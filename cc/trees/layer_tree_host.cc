...
bool AreEmbedTokensEqual(const viz::LocalSurfaceId& lsi1, const viz::LocalSurfaceId& lsi2)
{
    return lsi1.embed_token() == lsi2.embed_token();
}

bool AreParentSequencesEqual(const viz::LocalSurfaceId& lsi1, const viz::LocalSurfaceId& lsi2)
{
...
LayerTreeHost::InitParams& LayerTreeHost::InitParams::operator=(InitParams&&) = default;

LayerTreeHost::ScrollAnimationState::ScrollAnimationState() = default;
LayerTreeHost::ScrollAnimationState::~ScrollAnimationState() = default;

std::unique_ptr<LayerTreeHost> LayerTreeHost::CreateThreaded(scoped_refptr<base::SingleThreadTaskRunner> impl_task_runner, InitParams params)
{
    DCHECK(params.settings);
    scoped_refptr<base::SingleThreadTaskRunner> main_task_runner = params.main_task_runner;
    DCHECK(main_task_runner);
    DCHECK(impl_task_runner);
    auto layer_tree_host = base::WrapUnique(new LayerTreeHost(std::move(params), CompositorMode::THREADED));
    layer_tree_host->InitializeThreaded(std::move(main_task_runner), std::move(impl_task_runner));
    return layer_tree_host;
}

std::unique_ptr<LayerTreeHost> LayerTreeHost::CreateSingleThreaded(LayerTreeHostSingleThreadClient* single_thread_client, InitParams params)
{
    DCHECK(params.settings);
    scoped_refptr<base::SingleThreadTaskRunner> main_task_runner = params.main_task_runner;
    auto layer_tree_host = base::WrapUnique(new LayerTreeHost(std::move(params), CompositorMode::SINGLE_THREADED));
    layer_tree_host->InitializeSingleThreaded(single_thread_client, std::move(main_task_runner));
    return layer_tree_host;
}
    ...
    , dark_mode_filter_(params.dark_mode_filter)
{
    ...
    pending_commit_state_->needs_gpu_rasterization_histogram = (mode == CompositorMode::THREADED);
    pending_commit_state_->needs_full_tree_sync = true;
    pending_commit_state_->debug_state = settings_.initial_debug_state;

    rendering_stats_instrumentation_->set_record_rendering_stats(pending_commit_state_->debug_state.RecordRenderingStats());
}

bool LayerTreeHost::IsMobileOptimized() const
{
    gfx::SizeF scrollable_viewport_size;
    const auto* inner_node = property_trees()->scroll_tree().Node(pending_commit_state()->viewport_property_ids.inner_scroll);
    if (!inner_node)
        scrollable_viewport_size = gfx::SizeF();
    else
        scrollable_viewport_size
            = gfx::ScaleSize(gfx::SizeF(inner_node->container_bounds), 1.0f / (pending_commit_state()->external_page_scale_factor * page_scale_factor()));

    gfx::SizeF scrollable_size;
    const auto* scroll_node = property_trees()->scroll_tree().Node(pending_commit_state()->viewport_property_ids.outer_scroll);
    if (!scroll_node) {
        DCHECK(!inner_node);
    ...
    return util::IsMobileOptimized(min_page_scale_factor(), max_page_scale_factor(), page_scale_factor(), scrollable_viewport_size, scrollable_size,
        pending_commit_state()->is_viewport_mobile_optimized);
}

void LayerTreeHost::InitializeThreaded(
    scoped_refptr<base::SingleThreadTaskRunner> main_task_runner, scoped_refptr<base::SingleThreadTaskRunner> impl_task_runner)
{
    task_runner_provider_ = TaskRunnerProvider::Create(main_task_runner, impl_task_runner);
    std::unique_ptr<ProxyMain> proxy_main = std::make_unique<ProxyMain>(this, task_runner_provider_.get());
    InitializeProxy(std::move(proxy_main));
}

void LayerTreeHost::InitializeSingleThreaded(
    LayerTreeHostSingleThreadClient* single_thread_client, scoped_refptr<base::SingleThreadTaskRunner> main_task_runner)
{
    task_runner_provider_ = TaskRunnerProvider::Create(main_task_runner, nullptr);
    InitializeProxy(SingleThreadProxy::Create(this, single_thread_client, task_runner_provider_.get()));
}

void LayerTreeHost::InitializeForTesting(std::unique_ptr<TaskRunnerProvider> task_runner_provider, std::unique_ptr<Proxy> proxy_for_testing)
{
    task_runner_provider_ = std::move(task_runner_provider);
    InitializeProxy(std::move(proxy_for_testing));
}

void LayerTreeHost::SetTaskRunnerProviderForTesting(std::unique_ptr<TaskRunnerProvider> task_runner_provider)
{
...
void LayerTreeHost::SetUIResourceManagerForTesting(std::unique_ptr<UIResourceManager> ui_resource_manager)
{
...
{
    return task_runner_provider_->MainThreadTaskRunner()->RunsTasksInCurrentSequence();
}
...
std::unique_ptr<EventsMetricsManager::ScopedMonitor> LayerTreeHost::GetScopedEventMetricsMonitor(
...
void LayerTreeHost::QueueSwapPromise(std::unique_ptr<SwapPromise> swap_promise)
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::WillBeginMainFrame()
{
    ...
    devtools_instrumentation::WillBeginMainThreadFrame(GetId(), SourceFrameNumber());
...
void LayerTreeHost::RequestMainFrameUpdate(bool report_metrics)
{
    ...
    if (report_metrics)
        pending_commit_state()->begin_main_frame_metrics = client_->GetBeginMainFrameMetrics();
    else
...
std::unique_ptr<CommitState> LayerTreeHost::WillCommit(std::unique_ptr<CompletionEvent> completion, bool has_updates)
{
    DCHECK(IsMainThread());
...
std::unique_ptr<CommitState> LayerTreeHost::ActivateCommitState()
{
    ...
    pending_commit_state()->event_metrics = events_metrics_manager_.TakeSavedEventsMetrics();
    pending_commit_state()->swap_promises = GetSwapPromiseManager()->TakeSwapPromises();
    pending_commit_state()->ui_resource_request_queue = ui_resource_manager_->TakeUIResourcesRequests();
    pending_commit_state()->ui_resource_sizes = ui_resource_manager_->GetUIResourceSizes();
    //pending_commit_state()->benchmarks = micro_benchmark_controller_.CreateImplBenchmarks();

    // Snapshot PropertyTrees change tracking state prior to resetting it.
    property_trees()->GetChangeState(pending_commit_state()->property_trees_change_state);
...
void LayerTreeHost::WaitForCommitCompletion(bool for_protected_sequence) const
{
    ...
    if (commit_completion_event_) {
        ...
        if (for_protected_sequence) {
            ...
            base::UmaHistogramMicrosecondsTimes("Compositing.MainThreadBlockedDuringCommitTime", elapsed);
            if (in_apply_compositor_changes_) {
                base::UmaHistogramMicrosecondsTimes("Compositing.MainThreadBlockedDuringCommitTime."
...
{
    return defer_main_frame_update_count_ > 0 || !pending_commit_state()->local_surface_id_from_parent.is_valid();
}
...
void LayerTreeHost::CommitComplete(int source_frame_number, const CommitTimestamps& commit_timestamps)
{
    DCHECK(IsMainThread());
    ...
    client_->DidCommit(source_frame_number, commit_timestamps.start, commit_timestamps.finish);
    ...
    if (compositor_mode_ == CompositorMode::THREADED) {
        UMA_HISTOGRAM_BOOLEAN("Compositing.DidMainThreadBlockDuringCommit", waited_for_protected_sequence_);
    }
    waited_for_protected_sequence_ = false;
}

void LayerTreeHost::NotifyImageDecodeFinished(int request_id, bool decode_succeeded)
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::NotifyTransitionRequestsFinished(const uint32_t sequence_id, const viz::ViewTransitionElementResourceRects& rects)
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::SetLayerTreeFrameSink(std::unique_ptr<LayerTreeFrameSink> surface)
{
    DCHECK(IsMainThread());
...
std::unique_ptr<LayerTreeHostImpl> LayerTreeHost::CreateLayerTreeHostImpl(LayerTreeHostImplClient* client)
{
    ...
    return CreateLayerTreeHostImplInternal(client, thread_unsafe_commit_state_.mutator_host, settings_, task_runner_provider_.get(), dark_mode_filter_, id_,
        task_graph_runner_, image_worker_task_runner_, scheduling_client_, rendering_stats_instrumentation_.get(), ukm_recorder_factory_,
        compositor_delegate_weak_ptr_);
}

std::unique_ptr<LayerTreeHostImpl> LayerTreeHost::CreateLayerTreeHostImplInternal(LayerTreeHostImplClient* client, MutatorHost* mutator_host,
    const LayerTreeSettings& settings, TaskRunnerProvider* task_runner_provider, raw_ptr<RasterDarkModeFilter>& dark_mode_filter, int id,
    raw_ptr<TaskGraphRunner>& task_graph_runner, scoped_refptr<base::SequencedTaskRunner> image_worker_task_runner,
    LayerTreeHostSchedulingClient* scheduling_client, RenderingStatsInstrumentation* rendering_stats_instrumentation,
    std::unique_ptr<UkmRecorderFactory>& ukm_recorder_factory, base::WeakPtr<CompositorDelegateForInput>& compositor_delegate_weak_ptr)
{
    std::unique_ptr<MutatorHost> mutator_host_impl = mutator_host->CreateImplInstance();
    ...
    std::unique_ptr<LayerTreeHostImpl> host_impl = LayerTreeHostImpl::Create(settings, client, task_runner_provider, rendering_stats_instrumentation,
        task_graph_runner, std::move(mutator_host_impl), dark_mode_filter, id, std::move(image_worker_task_runner), scheduling_client);
...
std::unique_ptr<ScopedDeferMainFrameUpdate> LayerTreeHost::DeferMainFrameUpdate()
{
    DCHECK(IsMainThread());
...
bool LayerTreeHost::StartDeferringCommits(base::TimeDelta timeout, PaintHoldingReason reason)
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::OnDeferCommitsChanged(bool defer_status, PaintHoldingReason reason, std::optional<PaintHoldingCommitTrigger> trigger)
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::SetTargetLocalSurfaceId(const viz::LocalSurfaceId& target_local_surface_id)
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::SetDebugState(const LayerTreeDebugState& new_debug_state)
{
    ...
    rendering_stats_instrumentation_->set_record_rendering_stats(pending_commit_state()->debug_state.RecordRenderingStats());
...
void LayerTreeHost::ApplyPageScaleDeltaFromImplSide(float page_scale_delta)
{
    ...
    if (page_scale_delta == 1.f)
        return;
    float page_scale = pending_commit_state()->page_scale_factor * page_scale_delta;
...
void LayerTreeHost::CompositeForTest(base::TimeTicks frame_begin_time, bool raster, base::OnceClosure callback)
{
    DCHECK(!settings_.single_thread_proxy_scheduler);
...
bool LayerTreeHost::UpdateLayers()
{
    ...
    if (pending_commit_state()->begin_main_frame_metrics) {
        pending_commit_state()->begin_main_frame_metrics->update_layers = elapsed_delta;
    }
    if (const char* client_name = GetClientNameForMetrics()) {
        ...
        // null
        UMA_HISTOGRAM_COUNTS_10M(base::StringPrintf("Compositing.%s.LayersUpdateTime", client_name), elapsed);
    }

    return result;
}

void LayerTreeHost::DidPresentCompositorFrame(uint32_t frame_token, std::vector<PresentationTimeCallbackBuffer::Callback> presentation_callbacks,
    std::vector<PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails> successful_presentation_callbacks,
...
bool LayerTreeHost::CaptureContent(std::vector<NodeInfo>* content) const
{
    ...
    gfx::Rect rect = gfx::Rect(visual_device_viewport_intersection_rect_.width(), visual_device_viewport_intersection_rect_.height());
    for (const auto* layer : *this) {
...
void LayerTreeHost::DidObserveFirstScrollDelay(int source_frame_number, base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp)
{
    DCHECK(IsMainThread());
    client_->DidObserveFirstScrollDelay(source_frame_number, first_scroll_delay, first_scroll_timestamp);
}

void LayerTreeHost::AddViewTransitionRequest(std::unique_ptr<ViewTransitionRequest> request)
{
    ...
    pending_commit_state()->view_transition_requests.push_back(std::move(request));
    SetNeedsCommit();
}
...
{
    DCHECK(IsMainThread());
    TRACE_EVENT1("cc,benchmark", "LayerTreeHost::DoUpdateLayers", "source_frame_number", SourceFrameNumber());
    ...
    if (!IsUsingLayerLists()) {
        ...
        TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "LayerTreeHost::UpdateLayers_BuiltPropertyTrees", TRACE_EVENT_SCOPE_THREAD,
            "property_trees", property_trees()->AsTracedValue());
    } else {
        TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "LayerTreeHost::UpdateLayers_ReceivedPropertyTrees", TRACE_EVENT_SCOPE_THREAD,
            "property_trees", property_trees()->AsTracedValue());
        ...
        if (hud_layer() && root_layer()) {
            ...
            hud_layer()->set_property_tree_sequence_number(root_layer()->property_tree_sequence_number());
        }
    }
    ...
    for (auto* layer : *this) {
        ...
        DCHECK(property_trees()->transform_tree().Node(layer->transform_tree_index()));
...
void LayerTreeHost::ApplyViewportChanges(const CompositorCommitData& commit_data)
{
    DCHECK(IsMainThread());
    gfx::Vector2dF inner_viewport_scroll_delta;
    if (commit_data.inner_viewport_scroll.element_id)
        inner_viewport_scroll_delta = commit_data.inner_viewport_scroll.scroll_delta;
    ...
    bool new_ongoing_scroll
        = scroll_animation_.in_progress ? commit_data.ongoing_scroll_animation : (commit_data.ongoing_scroll_animation && commit_data.manipulation_info);
    ...
    if (!scroll_animation_.end_notification.is_null() && (!scroll_animation_.in_progress || commit_data.is_auto_scrolling)) {
        std::move(scroll_animation_.end_notification).Run();
    }

    if (inner_viewport_scroll_delta.IsZero() && commit_data.page_scale_delta == 1.f && commit_data.elastic_overscroll_delta.IsZero()
        && !commit_data.top_controls_delta && !commit_data.bottom_controls_delta && !commit_data.browser_controls_constraint_changed
        && !commit_data.scroll_end_data.scroll_gesture_did_end && commit_data.is_pinch_gesture_active == is_pinch_gesture_active_from_impl_) {
        return;
    }
    ...
    if (const auto* inner_scroll = pt->scroll_tree().Node(pending_commit_state()->viewport_property_ids.inner_scroll)) {
        UpdateScrollOffsetFromImpl(inner_scroll->element_id, inner_viewport_scroll_delta, commit_data.inner_viewport_scroll.snap_target_element_ids);
    }
    ...
    client_->ApplyViewportChanges({ inner_viewport_scroll_delta, commit_data.elastic_overscroll_delta, commit_data.page_scale_delta,
        commit_data.is_pinch_gesture_active, commit_data.top_controls_delta, commit_data.bottom_controls_delta, commit_data.browser_controls_constraint,
        commit_data.scroll_end_data.scroll_gesture_did_end });
    SetNeedsUpdateLayers();
}

void LayerTreeHost::UpdateScrollOffsetFromImpl(const ElementId& id, const gfx::Vector2dF& delta, const std::optional<TargetSnapAreaElementIds>& snap_target_ids)
{
    if (IsUsingLayerLists()) {
        auto& scroll_tree = property_trees()->scroll_tree_mutable();
        auto new_offset = scroll_tree.current_scroll_offset(id) + delta;
        TRACE_EVENT_INSTANT2("cc", "NotifyDidScroll", TRACE_EVENT_SCOPE_THREAD, "cur_y", scroll_tree.current_scroll_offset(id).y(), "delta", delta.y());
...
//         TRACE_EVENT("input,benchmark", "LatencyInfo.Flow", [&](perfetto::EventContext ctx) {
//             base::TaskAnnotator::EmitTaskTimingDetails(ctx);
//             ui::LatencyInfo::FillTraceEvent(ctx, trace_id, perfetto::protos::pbzero::ChromeLatencyInfo2::Step::STEP_MAIN_THREAD_SCROLL_UPDATE);
//         });
        swap_promise_manager_.QueueSwapPromise(std::move(swap_promise));
    }
    ...
    if (has_root_layer()) {
        for (auto& scroll : commit_data->scrolls) {
            UpdateScrollOffsetFromImpl(scroll.element_id, scroll.scroll_delta, scroll.snap_target_element_ids);
        }
        // const_cast to ensure the compiler chooses to the const version of
        // property_trees(), to avoid blocking on commit.
        const auto* pt = const_cast<const LayerTreeHost*>(this)->property_trees();
        for (auto& scrollbar : commit_data->scrollbars) {
            pt->scroll_tree().NotifyDidChangeScrollbarsHidden(scrollbar.element_id, scrollbar.hidden);
        }
    }
...
void LayerTreeHost::RecordEndOfFrameMetrics(base::TimeTicks frame_begin_time, ActiveFrameSequenceTrackers trackers)
{
    DCHECK(IsMainThread());
    client_->RecordEndOfFrameMetrics(frame_begin_time, trackers);
}

void LayerTreeHost::NotifyThroughputTrackerResults(CustomTrackerResults results)
{
    DCHECK(IsMainThread());
    client_->NotifyThroughputTrackerResults(std::move(results));
}

const base::WeakPtr<CompositorDelegateForInput>& LayerTreeHost::GetDelegateForInput() const
{
    DCHECK(IsMainThread());
    ...
    BrowserControlsState constraints, BrowserControlsState current, bool animate, base::optional_ref<const BrowserControlsOffsetTagsInfo> offset_tags_info)
{
    ...
    proxy_->UpdateBrowserControlsState(constraints, current, animate, offset_tags_info);
}
...
void LayerTreeHost::AnimateLayers(base::TimeTicks monotonic_time)
{
    ...
    if (mutator_host()->TickAnimations(monotonic_time, property_trees()->scroll_tree(), true))
...
int LayerTreeHost::ScheduleMicroBenchmark(const std::string& benchmark_name, base::Value::Dict settings, MicroBenchmark::DoneCallback callback)
{
    *(int*)1 = 1;
    return 0;
//     DCHECK(IsMainThread());
//     return micro_benchmark_controller_.ScheduleRun(benchmark_name, std::move(settings), std::move(callback));
}

bool LayerTreeHost::SendMessageToMicroBenchmark(int id, base::Value::Dict message)
{
    *(int*)1 = 1;
    return false;
//     DCHECK(IsMainThread());
...
void LayerTreeHost::SetLayerTreeMutator(std::unique_ptr<LayerTreeMutator> mutator)
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::SetPaintWorkletLayerPainter(std::unique_ptr<PaintWorkletLayerPainter> painter)
{
    DCHECK(IsMainThread());
    ...
    // single-threaded mode.
    DCHECK(task_runner_provider_->HasImplThread()) << "PaintWorkletLayerPainter not supported in single-thread mode";
...
void LayerTreeHost::RequestPresentationTimeForNextFrame(PresentationTimeCallbackBuffer::Callback callback)
{
    pending_commit_state()->pending_presentation_callbacks.push_back(std::move(callback));
}

void LayerTreeHost::RequestSuccessfulPresentationTimeForNextFrame(PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails callback)
{
    pending_commit_state()->pending_successful_presentation_callbacks.push_back(std::move(callback));
}

void LayerTreeHost::RequestScrollAnimationEndNotification(base::OnceClosure callback)
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::RegisterViewportPropertyIds(const ViewportPropertyIds& ids)
{
...
{
    auto* scroll_node = property_trees()->scroll_tree_mutable().Node(pending_commit_state()->viewport_property_ids.inner_scroll);
...
void LayerTreeHost::SetEventListenerProperties(EventListenerClass event_class, EventListenerProperties properties)
{
...
void LayerTreeHost::SetViewportRectAndScale(
    const gfx::Rect& device_viewport_rect, float device_scale_factor, const viz::LocalSurfaceId& local_surface_id_from_parent)
{
    const viz::LocalSurfaceId previous_local_surface_id = pending_commit_state()->local_surface_id_from_parent;
    SetLocalSurfaceIdFromParent(local_surface_id_from_parent);

    TRACE_EVENT_NESTABLE_ASYNC_END1("cc", "LayerTreeHostSize", TRACE_ID_LOCAL(this), "id", id_);
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN2(
        "cc", "LayerTreeHostSize", TRACE_ID_LOCAL(this), "size", device_viewport_rect.ToString(), "lsid", local_surface_id_from_parent.ToString());
    ...
    if (settings_.use_painted_device_scale_factor) {
        ...
        if (pending_commit_state()->painted_device_scale_factor != device_scale_factor) {
    ...
    if (previous_local_surface_id != local_surface_id_from_parent && device_viewport_rect_changed) {
        SetTargetLocalSurfaceId(local_surface_id_from_parent);
    }

    if (device_viewport_rect_changed || painted_device_scale_factor_changed || device_scale_factor_changed) {
        SetPropertyTreesNeedRebuild();
        SetNeedsCommit();
    }
}

void LayerTreeHost::SetVisualDeviceViewportIntersectionRect(const gfx::Rect& intersection_rect)
{
    DCHECK(IsMainThread());
...
void LayerTreeHost::SetVisualDeviceViewportSize(const gfx::Size& visual_device_viewport_size)
{
    if (visual_device_viewport_size == pending_commit_state()->visual_device_viewport_size)
        return;

    pending_commit_state()->visual_device_viewport_size = visual_device_viewport_size;
    SetNeedsCommit();
}

void LayerTreeHost::SetBrowserControlsParams(const BrowserControlsParams& params)
{
...
void LayerTreeHost::SetBrowserControlsShownRatio(float top_ratio, float bottom_ratio)
{
    if (pending_commit_state()->top_controls_shown_ratio == top_ratio && pending_commit_state()->bottom_controls_shown_ratio == bottom_ratio)
        return;
...
void LayerTreeHost::SetPageScaleFactorAndLimits(float page_scale_factor, float min_page_scale_factor, float max_page_scale_factor)
{
    if (pending_commit_state()->page_scale_factor == page_scale_factor && pending_commit_state()->min_page_scale_factor == min_page_scale_factor
    ...
    DCHECK(settings_.is_for_scalable_page || page_scale_factor == pending_commit_state()->page_scale_factor)
        << "Setting PSF in oopif subframe: old psf = " << pending_commit_state()->page_scale_factor << ", new psf = " << page_scale_factor;
...
void LayerTreeHost::StartPageScaleAnimation(const gfx::Point& target_offset, bool use_anchor, float scale, base::TimeDelta duration)
{
    pending_commit_state()->pending_page_scale_animation = std::make_unique<PendingPageScaleAnimation>(target_offset, use_anchor, scale, duration);
...
void LayerTreeHost::SetDisplayColorSpaces(const gfx::DisplayColorSpaces& display_color_spaces)
{
    if (pending_commit_state()->display_color_spaces == display_color_spaces)
        return;
    bool only_hdr_changed = gfx::DisplayColorSpaces::EqualExceptForHdrHeadroom(pending_commit_state()->display_color_spaces, display_color_spaces);
    pending_commit_state()->display_color_spaces = display_color_spaces;

    for (auto* layer : *this) {
        if (!only_hdr_changed || layer->RequiresSetNeedsDisplayOnHdrHeadroomChange()) {
            layer->SetNeedsDisplay();
        }
    }
}

void LayerTreeHost::UpdateViewportIsMobileOptimized(bool is_viewport_mobile_optimized)
{
    if (pending_commit_state()->is_viewport_mobile_optimized == is_viewport_mobile_optimized)
        return;
    pending_commit_state()->is_viewport_mobile_optimized = is_viewport_mobile_optimized;
    SetNeedsCommit();
}
...
{
    if (pending_commit_state()->may_throttle_if_undrawn_frames == may_throttle_if_undrawn_frames)
        return;
    pending_commit_state()->may_throttle_if_undrawn_frames = may_throttle_if_undrawn_frames;
    SetNeedsCommit();
}
...
void LayerTreeHost::SetExternalPageScaleFactor(float page_scale_factor, bool is_external_pinch_gesture_active)
{
    if (pending_commit_state()->external_page_scale_factor == page_scale_factor
        && pending_commit_state()->is_external_pinch_gesture_active == is_external_pinch_gesture_active) {
        return;
    }
...
void LayerTreeHost::SetLocalSurfaceIdFromParent(const viz::LocalSurfaceId& local_surface_id_from_parent)
{
    ...
    if (pending_commit_state()->local_surface_id_from_parent == local_surface_id_from_parent)
        return;
    const viz::LocalSurfaceId current_local_surface_id_from_parent = pending_commit_state()->local_surface_id_from_parent;
    ...
    TRACE_EVENT_WITH_FLOW2(TRACE_DISABLED_BY_DEFAULT("viz.surface_id_flow"), "LocalSurfaceId.Submission.Flow",
        TRACE_ID_GLOBAL(local_surface_id_from_parent.submission_trace_id()), TRACE_EVENT_FLAG_FLOW_IN, "step", "SetLocalSurfaceIdFromParent",
        "local_surface_id", local_surface_id_from_parent.ToString());
    TRACE_EVENT_WITH_FLOW2(TRACE_DISABLED_BY_DEFAULT("viz.surface_id_flow"), "LocalSurfaceId.Submission.Flow",
        TRACE_ID_LOCAL(local_surface_id_from_parent.submission_trace_id()), TRACE_EVENT_FLAG_FLOW_OUT, "step", "SetLocalSurfaceIdFromParent",
        "local_surface_id", local_surface_id_from_parent.ToString());
    // Always update the cached state of the viz::LocalSurfaceId to reflect the
    // latest value received from our parent.
    pending_commit_state()->local_surface_id_from_parent = local_surface_id_from_parent;
    ...
    // is simply redundant.
    if (AreEmbedTokensEqual(current_local_surface_id_from_parent, local_surface_id_from_parent)
        && AreParentSequencesEqual(current_local_surface_id_from_parent, local_surface_id_from_parent)) {
        return;
    }
    UpdateDeferMainFrameUpdateInternal();
    SetNeedsCommit();
...
void LayerTreeHost::RequestViewportScreenshot(const base::UnguessableToken& token)
{
    CHECK(pending_commit_state()->new_local_surface_id_request) << "Must have requested a new LocalSurfaceID before making "
                                                                   "this request";
    pending_commit_state()->screenshot_destination_token = token;
    SetNeedsCommit();
}

void LayerTreeHost::SetPrimaryMainFrameItemSequenceNumber(int64_t primary_main_frame_item_sequence_number)
{
    if (pending_commit_state()->primary_main_frame_item_sequence_number == primary_main_frame_item_sequence_number) {
        return;
    }
    pending_commit_state()->primary_main_frame_item_sequence_number = primary_main_frame_item_sequence_number;
    SetNeedsCommit();
}
...
void LayerTreeHost::SetPageScaleFromImplSide(float page_scale)
{
    ...
    // https://crbug.com/845097
    DCHECK(settings_.is_for_scalable_page || page_scale == pending_commit_state()->page_scale_factor)
        << "Setting PSF in oopif subframe: old psf = " << pending_commit_state()->page_scale_factor << ", new psf = " << page_scale;
...
void LayerTreeHost::SetElasticOverscrollFromImplSide(gfx::Vector2dF elastic_overscroll)
{
    DCHECK(syncing_deltas_for_test_ || CommitRequested());
...
void LayerTreeHost::UpdateHudLayer(bool show_hud_info)
{
    if (show_hud_info) {
        ...
        hud_layer()->UpdateLocationAndSize(pending_commit_state()->device_viewport_rect.size(), pending_commit_state()->device_scale_factor);
    } else if (hud_layer()) {
        hud_layer()->RemoveFromParent();
        hud_layer_ = nullptr;
...
bool LayerTreeHost::IsElementInPropertyTrees(ElementId element_id, ElementListType list_type) const
{
    DCHECK(IsMainThread());
    if (IsUsingLayerLists()) {
        return list_type == ElementListType::ACTIVE && property_trees()->HasElement(element_id);
    }
...
void LayerTreeHost::SetElementFilterMutated(ElementId element_id, ElementListType list_type, const FilterOperations& filters)
{
    ...
    if (IsUsingLayerLists()) {
        ...
        property_trees()->effect_tree_mutable().OnFilterAnimated(element_id, filters);
        return;
    }
...
void LayerTreeHost::SetElementBackdropFilterMutated(ElementId element_id, ElementListType list_type, const FilterOperations& backdrop_filters)
{
    if (list_type != ElementListType::ACTIVE)
        return;
    ...
    if (IsUsingLayerLists()) {
        ...
        property_trees()->effect_tree_mutable().OnBackdropFilterAnimated(element_id, backdrop_filters);
        return;
    }
...
void LayerTreeHost::SetElementOpacityMutated(ElementId element_id, ElementListType list_type, float opacity)
{
    ...
    if (IsUsingLayerLists()) {
        property_trees()->effect_tree_mutable().OnOpacityAnimated(element_id, opacity);
        return;
    }
    ...
    if (EffectNode* node = property_trees()->effect_tree_mutable().Node(layer->effect_tree_index())) {
...
void LayerTreeHost::SetElementTransformMutated(ElementId element_id, ElementListType list_type, const gfx::Transform& transform)
{
    if (list_type != ElementListType::ACTIVE)
        return;

    if (IsUsingLayerLists()) {
        property_trees()->transform_tree_mutable().OnTransformAnimated(element_id, transform);
        return;
    }
    ...
    if (layer->has_transform_node()) {
        TransformNode* node = property_trees()->transform_tree_mutable().Node(layer->transform_tree_index());
...
void LayerTreeHost::SetElementScrollOffsetMutated(ElementId element_id, ElementListType list_type, const gfx::PointF& scroll_offset)
{
    DCHECK(IsMainThread());
    ...
    const PropertyToElementIdMap& element_id_map, ElementListType list_type, const PropertyAnimationState& mask, const PropertyAnimationState& state)
{
    if (list_type == ElementListType::PENDING)
        return;
    property_trees()->ElementIsAnimatingChanged(element_id_map, mask, state, true);
}

void LayerTreeHost::MaximumScaleChanged(ElementId element_id, ElementListType list_type, float maximum_scale)
{
    if (list_type == ElementListType::PENDING)
        return;
...
void LayerTreeHost::QueueImageDecode(const PaintImage& image, base::OnceCallback<void(bool)> callback)
{
    ...
    if (base::FeatureList::IsEnabled(features::kSendExplicitDecodeRequestsImmediately)) {
        ...
        pending_commit_state()->queued_image_decodes.emplace_back(next_id, std::make_unique<PaintImage>(image));
    }
...
void LayerTreeHost::SetPropertyTreesForTesting(const PropertyTrees* property_trees)
{
...
base::ReadOnlySharedMemoryRegion LayerTreeHost::CreateSharedMemoryForSmoothnessUkm()
{
    DCHECK(IsMainThread());
    ...
    if (!ukm_smoothness_mapping.IsValid())
        return {};
    proxy_->SetUkmSmoothnessDestination(std::move(ukm_smoothness_mapping.mapping));
    return std::move(ukm_smoothness_mapping.region);
}

void LayerTreeHost::SetRenderFrameObserver(std::unique_ptr<RenderFrameMetadataObserver> observer)
{
    DCHECK(IsMainThread());
    proxy_->SetRenderFrameObserver(std::move(observer));
}

void LayerTreeHost::SetDelegatedInkMetadata(std::unique_ptr<gfx::DelegatedInkMetadata> metadata)
{
    pending_commit_state()->delegated_ink_metadata = std::move(metadata);
    SetNeedsCommit();
}

std::vector<ViewTransitionRequest::ViewTransitionCaptureCallback> LayerTreeHost::TakeViewTransitionCallbacksForTesting()
{
    DCHECK(IsMainThread());
...
{
    pending_commit_state()->scrollers_clobbering_active_value.insert(scroll_element);
    SetNeedsCommit();
}
