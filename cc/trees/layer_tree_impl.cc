...
public:
    ViewportAnchor(ScrollNode* inner_scroll, ScrollNode* outer_scroll, LayerTreeImpl* tree_impl)
        : inner_(inner_scroll)
        , outer_(outer_scroll)
        , tree_impl_(tree_impl)
    {
        viewport_in_content_coordinates_ = scroll_tree().current_scroll_offset(inner_->element_id).OffsetFromOrigin();

        if (outer_) {
            viewport_in_content_coordinates_ += scroll_tree().current_scroll_offset(outer_->element_id).OffsetFromOrigin();
        }
    }
    ...
    void ResetViewportToAnchoredPosition()
    {
        ...
        gfx::Vector2dF viewport_location = scroll_tree().current_scroll_offset(inner_->element_id).OffsetFromOrigin()
...
std::pair<gfx::PointF, gfx::PointF> GetVisibleSelectionEndPoints(const gfx::RectF& rect, const gfx::PointF& top, const gfx::PointF& bottom)
{
    gfx::PointF start(std::clamp(top.x(), rect.x(), rect.right()), std::clamp(top.y(), rect.y(), rect.bottom()));
...
LayerTreeImpl::LayerTreeImpl(LayerTreeHostImpl& host_impl, viz::BeginFrameArgs begin_frame_args, scoped_refptr<SyncedScale> page_scale_factor,
    scoped_refptr<SyncedBrowserControls> top_controls_shown_ratio, scoped_refptr<SyncedBrowserControls> bottom_controls_shown_ratio,
    ...
    , always_push_properties_on_picture_layers_(!base::FeatureList::IsEnabled(features::kDontAlwaysPushPictureLayerImpls))
...
{
    DetachLayers();
    BreakSwapPromises(IsActiveTree() ? SwapPromise::SWAP_FAILS : SwapPromise::ACTIVATION_FAILS);
    DCHECK(LayerListIsEmpty());
}
...
void LayerTreeImpl::DidUpdateScrollOffset(ElementId id, bool pushed_from_main_or_pending_tree)
{
    ...
    bool can_realize_on_active_tree = scroll_tree.CanRealizeScrollsOnActiveTree(*scroll_node);
    bool can_realize_on_pending_tree = scroll_tree.CanRealizeScrollsOnPendingTree(*scroll_node);
    ...
    if (IsActiveTree()) {
        ...
        if (host_impl_->pending_tree() && !pushed_from_main_or_pending_tree) {
            host_impl_->pending_tree()->DidUpdateScrollOffset(id, pushed_from_main_or_pending_tree);
        }
        if (host_impl_->recycle_tree()) {
            host_impl_->recycle_tree()->DidUpdateScrollOffset(id, pushed_from_main_or_pending_tree);
        }
        ...
        if (!pushed_from_main_or_pending_tree) {
            if (scroll_node == InnerViewportScrollNode() || scroll_node == OuterViewportScrollNode()) {
...
void LayerTreeImpl::UpdateAllScrollbarGeometries()
{
    ...
    for (auto& pair : element_id_to_scrollbar_layer_ids_) {
        if (const auto* scroll_node = property_trees()->scroll_tree().FindNodeFromElementId(pair.first)) {
            UpdateScrollbarGeometries(*scroll_node);
        }
    }
}
...
void LayerTreeImpl::UpdateScrollbarGeometries(const ScrollNode& scroll_node)
{
    ...
    bool is_viewport_scrollbar = &scroll_node == InnerViewportScrollNode() || &scroll_node == OuterViewportScrollNode();
    if (is_viewport_scrollbar) {
        ...
        if (&scroll_node == InnerViewportScrollNode()) {
            ...
            current_offset += scroll_tree.current_scroll_offset(outer_scroll_node->element_id).OffsetFromOrigin();
            gfx::SizeF outer_viewport_bounds(scroll_tree.container_bounds(outer_scroll_node->id));
            ...
            current_offset += scroll_tree.current_scroll_offset(inner_scroll_node->element_id).OffsetFromOrigin();
            gfx::SizeF inner_viewport_bounds(scroll_tree.container_bounds(inner_scroll_node->id));
    ...
    for (auto* scrollbar : ScrollbarsFor(scroll_node.element_id)) {
        ...
        if (is_viewport_scrollbar) {
            scrollbar->SetVerticalAdjust(property_trees_.inner_viewport_container_bounds_delta().y());
        }
    }
}
...
{
    return property_trees_.effect_tree().GetRenderSurface(kContentsRootPropertyNodeId);
}
...
void LayerTreeImpl::InvalidateRegionForImages(const PaintImageIdFlatSet& images_to_invalidate)
{
    TRACE_EVENT_BEGIN1("cc", "LayerTreeImpl::InvalidateRegionForImages", "total_layer_count", picture_layers_.size());
    DCHECK(IsSyncTree());
    ...
    if (!images_to_invalidate.empty()) {
        ...
        for (PictureLayerImpl* picture_layer : picture_layers_) {
            auto result = picture_layer->InvalidateRegionForImages(images_to_invalidate);
            switch (result) {
    ...
    TRACE_EVENT_END1("cc", "LayerTreeImpl::InvalidateRegionForImages", "counts",
        base::StringPrintf("no_images[%zu] no_invalidaton[%zu] invalidated[%zu]", no_images_count, no_invalidation_count, invalidated_count));
}

void LayerTreeImpl::InvalidateRasterInducingScrolls(const base::flat_set<ElementId>& scrolls_to_invalidate)
{
...
void LayerTreeImpl::UpdateViewportContainerSizes()
{
    ...
    ViewportAnchor anchor(InnerViewportScrollNode(), OuterViewportScrollNode(), this);

    float top_controls_shown_ratio = top_controls_shown_ratio_->Current(IsActiveTree());
    float bottom_controls_shown_ratio = bottom_controls_shown_ratio_->Current(IsActiveTree());
    float top_controls_layout_height = browser_controls_shrink_blink_size() ? top_controls_height() : top_controls_min_height();
    float top_content_offset = top_controls_height() > 0 ? top_controls_height() * top_controls_shown_ratio : 0.f;
    float delta_from_top_controls = top_controls_layout_height - top_content_offset;
    float bottom_controls_layout_height = browser_controls_shrink_blink_size() ? bottom_controls_height() : bottom_controls_min_height();
    float bottom_content_offset = bottom_controls_height() > 0 ? bottom_controls_height() * bottom_controls_shown_ratio : 0.f;
    delta_from_top_controls += bottom_controls_layout_height - bottom_content_offset;
    ...
    // clamping.
    gfx::Vector2dF scaled_bounds_delta = gfx::ScaleVector2d(bounds_delta, 1.f / min_page_scale_factor());
    ...
    DCHECK_EQ(scaled_bounds_delta, property_trees->inner_viewport_scroll_bounds_delta());

    if (auto* outer_clip_node = OuterViewportClipNode()) {
        float adjusted_container_height = OuterViewportScrollNode()->container_bounds.height() + scaled_bounds_delta.y();
        ...
        auto* outer_ancestor = property_trees->clip_tree_mutable().parent(outer_clip_node);
        while (outer_ancestor && outer_ancestor->id != kRootPropertyNodeId) {
            outer_ancestor->clip.Union(outer_clip_node->clip);
            outer_ancestor = property_trees->clip_tree_mutable().parent(outer_ancestor);
        }
    }
...
gfx::PointF LayerTreeImpl::TotalScrollOffset() const
{
    ...
    if (auto* inner_scroll = InnerViewportScrollNode()) {
        offset += scroll_tree.current_scroll_offset(inner_scroll->element_id).OffsetFromOrigin();
        DCHECK(OuterViewportScrollNode());
        offset += scroll_tree.current_scroll_offset(OuterViewportScrollNode()->element_id).OffsetFromOrigin();
    }
...
gfx::PointF LayerTreeImpl::TotalMaxScrollOffset() const
{
    ...
    if (viewport_property_ids_.inner_scroll != kInvalidPropertyNodeId) {
        offset += scroll_tree.MaxScrollOffset(viewport_property_ids_.inner_scroll).OffsetFromOrigin();
    }

    if (viewport_property_ids_.outer_scroll != kInvalidPropertyNodeId) {
        offset += scroll_tree.MaxScrollOffset(viewport_property_ids_.outer_scroll).OffsetFromOrigin();
    }
...
void LayerTreeImpl::SetPropertyTrees(PropertyTrees& property_trees, bool preserve_change_tracking)
{
...
void LayerTreeImpl::SetPropertyTrees(const PropertyTrees& property_trees, PropertyTreesChangeState& change_state, bool preserve_change_tracking)
{
    ...
    property_trees_.effect_tree_mutable().TakeRenderSurfaces(&old_render_surfaces);

    if (preserve_change_tracking) {
        change_state.full_tree_damaged |= property_trees_.full_tree_damaged();
        property_trees_.GetChangedNodes(change_state.changed_effect_nodes, change_state.changed_transform_nodes);
    }

    property_trees_ = property_trees;

    property_trees_.ApplyChangedNodes(change_state.changed_effect_nodes, change_state.changed_transform_nodes);
    ...
    property_trees_.effect_tree_mutable().ApplyRenderSurfaceChangedFlags(change_state.surface_property_changed_flags);
    bool render_surfaces_changed = property_trees_.effect_tree_mutable().CreateOrReuseRenderSurfaces(&old_render_surfaces, this);
    if (render_surfaces_changed)
        set_needs_update_draw_properties();
    property_trees_.effect_tree_mutable().PullCopyRequestsFrom(change_state.effect_tree_copy_requests);
...
void LayerTreeImpl::PullPropertiesFrom(CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
    ...
    AddPresentationCallbacks(std::move(commit_state.pending_presentation_callbacks));
    AddSuccessfulPresentationCallbacks(std::move(commit_state.pending_successful_presentation_callbacks));
    ...
    for (const ElementId& id : commit_state.scrollers_clobbering_active_value) {
        property_trees()->scroll_tree_mutable().SetScrollOffsetClobberActiveValue(id);
    }
    ...
    property_trees()->scroll_tree_mutable().PushScrollUpdatesFromMainThread(unsafe_state.property_trees, this, settings().commit_fractional_scroll_deltas);
    ...
    DCHECK(mutator_host());
    unsafe_state.mutator_host->PushPropertiesTo(mutator_host(), unsafe_state.property_trees);
...
void LayerTreeImpl::PullPropertyTreesFrom(CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
    ...
    if (unsafe_state.root_layer && IsActiveTree() && property_trees_.changed()) {
        if (unsafe_state.property_trees.sequence_number() == property_trees_.sequence_number()) {
    ...
    SetPropertyTrees(unsafe_state.property_trees, commit_state.property_trees_change_state, preserve_change_tracking);
}
...
void LayerTreeImpl::PullLayerTreePropertiesFrom(CommitState& commit_state)
{
    ...
    set_event_listener_properties(EventListenerClass::kTouchStartOrMove, commit_state.GetEventListenerProperties(EventListenerClass::kTouchStartOrMove));
    set_event_listener_properties(EventListenerClass::kMouseWheel, commit_state.GetEventListenerProperties(EventListenerClass::kMouseWheel));
    set_event_listener_properties(EventListenerClass::kTouchEndOrCancel, commit_state.GetEventListenerProperties(EventListenerClass::kTouchEndOrCancel));

    SetViewportPropertyIds(commit_state.viewport_property_ids);

    RegisterSelection(commit_state.selection);

    PushPageScaleFromMainThread(commit_state.page_scale_factor, commit_state.min_page_scale_factor, commit_state.max_page_scale_factor);

    SetBrowserControlsParams(commit_state.browser_controls_params);
    set_overscroll_behavior(commit_state.overscroll_behavior);
    PushBrowserControlsFromMainThread(commit_state.top_controls_shown_ratio, commit_state.bottom_controls_shown_ratio);
    ...
    set_primary_main_frame_item_sequence_number(commit_state.primary_main_frame_item_sequence_number);

    SetLocalSurfaceIdFromParent(commit_state.local_surface_id_from_parent);

    if (commit_state.pending_page_scale_animation) {
        SetPendingPageScaleAnimation(std::move(commit_state.pending_page_scale_animation));
    }
...
void LayerTreeImpl::PushPropertyTreesTo(LayerTreeImpl* target_tree)
{
    ...
    if (target_tree->property_trees()->changed()) {
        if (property_trees()->sequence_number() == target_tree->property_trees()->sequence_number()) {
            preserve_change_tracking = true;
        } else {
...
void LayerTreeImpl::PushPropertiesTo(LayerTreeImpl* target_tree)
{
    ...
    target_tree->property_trees()->scroll_tree_mutable().PushScrollUpdatesFromPendingTree(&property_trees_, target_tree);
    ...
    target_tree->PushPageScaleFactorAndLimits(nullptr, min_page_scale_factor(), max_page_scale_factor());
    ...
    target_tree->set_primary_main_frame_item_sequence_number(primary_main_frame_item_sequence_number());
    ...
    target_tree->set_event_listener_properties(EventListenerClass::kTouchStartOrMove, event_listener_properties(EventListenerClass::kTouchStartOrMove));
    target_tree->set_event_listener_properties(EventListenerClass::kMouseWheel, event_listener_properties(EventListenerClass::kMouseWheel));
    target_tree->set_event_listener_properties(EventListenerClass::kTouchEndOrCancel, event_listener_properties(EventListenerClass::kTouchEndOrCancel));

    if (hud_layer())
        target_tree->set_hud_layer(static_cast<HeadsUpDisplayLayerImpl*>(target_tree->LayerById(hud_layer()->id())));
    else
    ...
    target_tree->AddSuccessfulPresentationCallbacks(std::move(successful_presentation_callbacks_));
    successful_presentation_callbacks_.clear();

    if (delegated_ink_metadata_) {
        TRACE_EVENT_WITH_FLOW1("delegated_ink_trails", "Delegated ink metadata pushed to tree", TRACE_ID_GLOBAL(delegated_ink_metadata_->trace_id()),
            TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "metadata", delegated_ink_metadata_->ToString());
...
void LayerTreeImpl::HandleTickmarksVisibilityChange()
{
    ...
    ScrollbarAnimationController* controller = host_impl_->ScrollbarAnimationControllerForElementId(host_impl_->OuterViewportScrollNode()->element_id);
    ...
    for (ScrollbarLayerImplBase* scrollbar : controller->Scrollbars()) {
        ...
        controller->UpdateTickmarksVisibility(scrollbar->has_find_in_page_tickmarks());
    }
}
...
void LayerTreeImpl::HandleScrollbarShowRequests()
{
    for (ElementId scroll_element_id : show_scrollbar_requests_) {
        if (ScrollbarAnimationController* controller = host_impl_->ScrollbarAnimationControllerForElementId(scroll_element_id)) {
...
void LayerTreeImpl::MoveChangeTrackingToLayers()
{
    ...
    for (int id = kContentsRootPropertyNodeId; id < static_cast<int>(effect_tree.size()); ++id) {
...
void LayerTreeImpl::SetTransformMutated(ElementId element_id, const gfx::Transform& transform)
{
    DCHECK_EQ(1u, property_trees()->transform_tree().element_id_to_node_index().count(element_id));
    if (property_trees()->transform_tree_mutable().OnTransformAnimated(element_id, transform))
        set_needs_update_draw_properties();
}

void LayerTreeImpl::SetOpacityMutated(ElementId element_id, float opacity)
{
    DCHECK_EQ(1u, property_trees()->effect_tree().element_id_to_node_index().count(element_id));
    if (property_trees()->effect_tree_mutable().OnOpacityAnimated(element_id, opacity))
        set_needs_update_draw_properties();
}

void LayerTreeImpl::SetFilterMutated(ElementId element_id, const FilterOperations& filters)
{
    DCHECK_EQ(1u, property_trees()->effect_tree().element_id_to_node_index().count(element_id));
    if (property_trees()->effect_tree_mutable().OnFilterAnimated(element_id, filters))
        set_needs_update_draw_properties();
}

void LayerTreeImpl::SetBackdropFilterMutated(ElementId element_id, const FilterOperations& backdrop_filters)
{
    DCHECK_EQ(1u, property_trees()->effect_tree().element_id_to_node_index().count(element_id));
    if (property_trees()->effect_tree_mutable().OnBackdropFilterAnimated(element_id, backdrop_filters))
        set_needs_update_draw_properties();
}

void LayerTreeImpl::AddPresentationCallbacks(std::vector<PresentationTimeCallbackBuffer::Callback> callbacks)
{
    base::ranges::move(callbacks, std::back_inserter(presentation_callbacks_));
}

std::vector<PresentationTimeCallbackBuffer::Callback> LayerTreeImpl::TakePresentationCallbacks()
{
...
void LayerTreeImpl::AddSuccessfulPresentationCallbacks(std::vector<PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails> callbacks)
{
    base::ranges::move(callbacks, std::back_inserter(successful_presentation_callbacks_));
}

std::vector<PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails> LayerTreeImpl::TakeSuccessfulPresentationCallbacks()
{
    std::vector<PresentationTimeCallbackBuffer::SuccessfulCallbackWithDetails> callbacks;
...
void LayerTreeImpl::SetCurrentlyScrollingNode(const ScrollNode* node)
{
    ...
    scroll_tree.set_currently_scrolling_node(node ? node->id : kInvalidPropertyNodeId);
}
...
float LayerTreeImpl::ClampPageScaleFactorToLimits(float page_scale_factor) const
{
...
void LayerTreeImpl::UpdateTransformAnimation(ElementId element_id, int transform_node_index)
{
    ...
    // used.)
    const TargetProperty::Type transform_properties[] = { TargetProperty::TRANSFORM, TargetProperty::SCALE, TargetProperty::ROTATE, TargetProperty::TRANSLATE };
#if DCHECK_IS_ON()
    unsigned property_count = 0u;
#endif

    for (TargetProperty::Type property : transform_properties) {
        if (mutator_host()->HasAnyAnimationTargetingProperty(element_id, property)) {
#if DCHECK_IS_ON()
            ++property_count;
#endif
            TransformTree& transform_tree = property_trees()->transform_tree_mutable();
            if (TransformNode* node = transform_tree.Node(transform_node_index)) {
                ...
                bool has_potential_animation = mutator_host()->HasPotentiallyRunningAnimationForProperty(element_id, list_type, property);
                if (node->has_potential_animation != has_potential_animation) {
                    node->has_potential_animation = has_potential_animation;
                    node->maximum_animation_scale = mutator_host()->MaximumScale(element_id, list_type);
                    transform_tree.set_needs_update(true);
                    set_needs_update_draw_properties();
                }
            }
...
void LayerTreeImpl::UpdatePageScaleNode()
{
    ...
    draw_property_utils::UpdatePageScaleFactor(property_trees(), PageScaleTransformNode(), current_page_scale_factor());
}
...
void LayerTreeImpl::SetPageScaleOnActiveTree(float active_page_scale)
{
    ...
    if (!host_impl_->settings().is_for_scalable_page && clamped_page_scale != 1.f && !has_dumped_without_crashing) {
        has_dumped_without_crashing = true;
        static auto* psf_oopif_error = base::debug::AllocateCrashKeyString("psf_oopif_error", base::debug::CrashKeySize::Size32);
        base::debug::SetCrashKeyString(psf_oopif_error, base::StringPrintf("%f", clamped_page_scale));
...
void LayerTreeImpl::PushPageScaleFromMainThread(float page_scale_factor, float min_page_scale_factor, float max_page_scale_factor)
{
    PushPageScaleFactorAndLimits(&page_scale_factor, min_page_scale_factor, max_page_scale_factor);
}

void LayerTreeImpl::PushPageScaleFactorAndLimits(const float* page_scale_factor, float min_page_scale_factor, float max_page_scale_factor)
{
    ...
    if (page_scale_factor) {
        ...
        changed_page_scale |= page_scale_factor_->PushMainToPending(*page_scale_factor);
    }
...
void LayerTreeImpl::SetBrowserControlsParams(const BrowserControlsParams& params)
{
    ...
    if (IsActiveTree()) {
        host_impl_->browser_controls_manager()->OnBrowserControlsParamsChanged(params.animate_browser_controls_height_changes);
    }
}

void LayerTreeImpl::set_overscroll_behavior(const OverscrollBehavior& behavior)
{
...
bool LayerTreeImpl::ClampTopControlsShownRatio()
{
    ...
    if (IsActiveTree()) {
        ...
        range = host_impl_->browser_controls_manager()->TopControlsShownRatioRange();
    }
    return top_controls_shown_ratio_->SetCurrent(std::clamp(ratio, range.first, range.second));
}
...
bool LayerTreeImpl::ClampBottomControlsShownRatio()
{
    ...
    if (IsActiveTree()) {
        // BCOM might need to set ratios outside the [0, 1] range (e.g. animation
        // running). So, use the values it provides instead of clamping to [0, 1].
        range = host_impl_->browser_controls_manager()->BottomControlsShownRatioRange();
    }
    return bottom_controls_shown_ratio_->SetCurrent(std::clamp(ratio, range.first, range.second));
}

bool LayerTreeImpl::SetCurrentBrowserControlsShownRatio(float top_ratio, float bottom_ratio)
{
    TRACE_EVENT2("cc", "LayerTreeImpl::SetCurrentBrowserControlsShownRatio", "top_ratio", top_ratio, "bottom_ratio", bottom_ratio);
...
void LayerTreeImpl::PushBrowserControlsFromMainThread(float top_controls_shown_ratio, float bottom_controls_shown_ratio)
{
    PushBrowserControls(&top_controls_shown_ratio, &bottom_controls_shown_ratio);
}

void LayerTreeImpl::PushBrowserControls(const float* top_controls_shown_ratio, const float* bottom_controls_shown_ratio)
{
    DCHECK(top_controls_shown_ratio || bottom_controls_shown_ratio || IsActiveTree());
    ...
    if (top_controls_shown_ratio) {
        ...
        bottom_controls_shown_ratio_->PushMainToPending(*bottom_controls_shown_ratio);
        if (!IsActiveTree())
            UpdateViewportContainerSizes();
    }
    if (IsActiveTree()) {
...
bool LayerTreeImpl::SetPageScaleFactorLimits(float min_page_scale_factor, float max_page_scale_factor)
{
...
{
    if (IsActiveTree()) {
        page_scale_factor()->SetCurrent(ClampPageScaleFactorToLimits(current_page_scale_factor()));
        ...
        if (settings().scrollbar_flash_after_any_scroll_update) {
            ...
            if (ScrollbarAnimationController* controller = host_impl_->ScrollbarAnimationControllerForElementId(scroll_node->element_id))
...
void LayerTreeImpl::SetLocalSurfaceIdFromParent(const viz::LocalSurfaceId& local_surface_id_from_parent)
{
...
void LayerTreeImpl::SetScreenshotDestinationToken(base::UnguessableToken destination_token)
{
...
void LayerTreeImpl::SetDeviceViewportRect(const gfx::Rect& device_viewport_rect)
{
...
void LayerTreeImpl::SetDisplayColorSpaces(const gfx::DisplayColorSpaces& display_color_spaces)
{
...
void LayerTreeImpl::SetExternalPageScaleFactor(float external_page_scale_factor)
{
...
gfx::SizeF LayerTreeImpl::ScrollableViewportSize() const
{
    ...
    gfx::Size container_bounds = property_trees()->scroll_tree().container_bounds(viewport_property_ids_.inner_scroll);
    return gfx::ScaleSize(gfx::SizeF(container_bounds), 1.0f / page_scale_factor_for_scroll());
}
...
gfx::Rect LayerTreeImpl::RootScrollLayerDeviceViewportBounds() const
{
    ...
    return MathUtil::MapEnclosingClippedRect(property_trees()->transform_tree().ToScreen(root_scroll_node->transform_id), gfx::Rect(root_scroll_node->bounds));
}

void LayerTreeImpl::ApplySentScrollAndScaleDeltasFromAbortedCommit(bool next_bmf, bool main_frame_applied_deltas)
{
    DCHECK(IsActiveTree());
    ...
    bottom_controls_shown_ratio()->AbortCommit(next_bmf, main_frame_applied_deltas);
    elastic_overscroll()->AbortCommit(next_bmf, main_frame_applied_deltas);

    if (layer_list_.empty())
        return;

    property_trees()->scroll_tree_mutable().ApplySentScrollDeltasFromAbortedCommit(next_bmf, main_frame_applied_deltas);
}
...
void LayerTreeImpl::SetViewportPropertyIds(const ViewportPropertyIds& ids)
{
    ...
    DCHECK(ids.inner_scroll != kInvalidPropertyNodeId || (ids.outer_scroll == kInvalidPropertyNodeId && ids.outer_clip == kInvalidPropertyNodeId));
...
{
    return property_trees()->transform_tree().Node(viewport_property_ids_.overscroll_elasticity_transform);
}

const TransformNode* LayerTreeImpl::PageScaleTransformNode() const
{
    return property_trees()->transform_tree().Node(viewport_property_ids_.page_scale_transform);
}

const ScrollNode* LayerTreeImpl::InnerViewportScrollNode() const
{
    return property_trees()->scroll_tree().Node(viewport_property_ids_.inner_scroll);
}
...
{
    return property_trees()->scroll_tree().Node(viewport_property_ids_.outer_scroll);
}
...
bool LayerTreeImpl::UpdateDrawProperties(bool update_tiles, bool update_image_animation_controller, LayerImplList* output_update_layer_list_for_testing)
{
...

    {
        ...
        TRACE_EVENT2("cc,benchmark", "LayerTreeImpl::UpdateDrawProperties::CalculateDrawProperties", "IsActive", IsActiveTree(), "SourceFrameNumber",
        ...
        // us to skip it.
        draw_property_utils::CalculateDrawProperties(this, &render_surface_list_, output_update_layer_list_for_testing);

        if (settings().single_thread_proxy_scheduler) {
            // This metric is only recorded for the Browser.
            UMA_HISTOGRAM_COUNTS_1M("Compositing.Browser.LayerTreeImpl.CalculateDrawPropertiesUs", timer.Elapsed().InMicroseconds());
        } else {
            // This metric is only recorded for the Renderer.
            UMA_HISTOGRAM_COUNTS_100("Compositing.Renderer.NumRenderSurfaces", base::saturated_cast<int>(render_surface_list_.size()));
        }
    }

    TRACE_EVENT2("cc,benchmark", "LayerTreeImpl::UpdateDrawProperties::Occlusion", "IsActive", IsActiveTree(), "SourceFrameNumber", source_frame_number_);
    OcclusionTracker occlusion_tracker(RootRenderSurface()->content_rect());
    occlusion_tracker.set_minimum_tracking_size(settings().minimum_occlusion_tracking_size);

    for (EffectTreeLayerListIterator it(this); it.state() != EffectTreeLayerListIterator::State::kEnd; ++it) {
        occlusion_tracker.EnterLayer(it);

        if (it.state() == EffectTreeLayerListIterator::State::kLayer) {
            LayerImpl* layer = it.current_layer();
            layer->draw_properties().occlusion_in_content_space = occlusion_tracker.GetCurrentOcclusionForLayer(layer->DrawTransform());
        }
        ...
        if (it.state() == EffectTreeLayerListIterator::State::kContributingSurface) {
            ...
            if (occlusion_surface) {
                ...
                property_trees()->GetToTarget(render_surface->TransformTreeIndex(), occlusion_surface->EffectTreeIndex(), &draw_transform);
                const EffectNode* effect_node = property_trees()->effect_tree().Node(render_surface->EffectTreeIndex());
                draw_property_utils::ConcatInverseSurfaceContentsScale(effect_node, &draw_transform);
            }

            Occlusion occlusion = occlusion_tracker.GetCurrentOcclusionForContributingSurface(draw_transform);
...
bool LayerTreeImpl::UpdateTiles()
{
    ...
    TRACE_EVENT_BEGIN2("cc,benchmark", "LayerTreeImpl::UpdateTiles", "IsActive", IsActiveTree(), "SourceFrameNumber", source_frame_number_);
    size_t layers_updated_count = 0;
    bool tile_priorities_updated = false;
    const bool release_tile_resources_for_hidden_layers = settings().release_tile_resources_for_hidden_layers;
    for (PictureLayerImpl* layer : picture_layers_) {
    ...
    TRACE_EVENT_END1("cc,benchmark", "LayerTreeImpl::UpdateDrawProperties::UpdateTiles", "layers_updated_count", layers_updated_count);
...
{
    auto it = base::ranges::find(base::Reversed(*this), element_id, &LayerImpl::element_id);
    if (it == rend())
        return nullptr;
    return *it;
}

void LayerTreeImpl::SetSurfaceRanges(base::flat_set<viz::SurfaceRange> surface_ranges)
{
...
void LayerTreeImpl::AddLayerShouldPushProperties(LayerImpl* layer)
{
    ...
    DCHECK(
        !always_push_properties_on_picture_layers() || !base::Contains(picture_layers_, layer) || (IsActiveTree() && settings().UseLayerContextForDisplay()));
...
void LayerTreeImpl::DidBecomeActive()
{
    ...
    devtools_instrumentation::DidActivateLayerTree(host_impl_->id(), source_frame_number_);
}
...
{
    const gfx::Rect& viewport_rect_for_tile_priority = host_impl_->viewport_rect_for_tile_priority();
    return viewport_rect_for_tile_priority.IsEmpty() ? GetDeviceViewport() : viewport_rect_for_tile_priority;
}

std::unique_ptr<ScrollbarAnimationController> LayerTreeImpl::CreateScrollbarAnimationController(ElementId scroll_element_id, float initial_opacity)
{
    ...
    case LayerTreeSettings::ANDROID_OVERLAY: {
        return ScrollbarAnimationController::CreateScrollbarAnimationControllerAndroid(
            scroll_element_id, host_impl_, fade_delay, fade_duration, initial_opacity);
    }
    case LayerTreeSettings::AURA_OVERLAY: {
        ...
        return ScrollbarAnimationController::CreateScrollbarAnimationControllerAuraOverlay(
            scroll_element_id, host_impl_, fade_delay, fade_duration, thinning_duration, initial_opacity, idle_thickness_scale);
    }
...
void LayerTreeImpl::GetAllPrioritizedTilesForTracing(std::vector<PrioritizedTile>* prioritized_tiles) const
{
...
{
    viz::TracedValue::MakeDictIntoImplicitSnapshot(state, "cc::LayerTreeImpl", this);
...
void LayerTreeImpl::QueueSwapPromise(std::unique_ptr<SwapPromise> swap_promise)
{
    DCHECK(swap_promise);
    swap_promise_list_.push_back(std::move(swap_promise));
}

void LayerTreeImpl::QueuePinnedSwapPromise(std::unique_ptr<SwapPromise> swap_promise)
{
    DCHECK(IsActiveTree());
    DCHECK(swap_promise);
...
void LayerTreeImpl::PassSwapPromises(std::vector<std::unique_ptr<SwapPromise>> new_swap_promises)
{
    base::TimeTicks timestamp = base::TimeTicks::Now();
    for (auto& swap_promise : swap_promise_list_) {
        if (swap_promise->DidNotSwap(SwapPromise::SWAP_FAILS, timestamp) == SwapPromise::DidNotSwapAction::KEEP_ACTIVE) {
...
void LayerTreeImpl::AppendSwapPromises(std::vector<std::unique_ptr<SwapPromise>> new_swap_promises)
{
    std::move(new_swap_promises.begin(), new_swap_promises.end(), std::back_inserter(swap_promise_list_));
...
void LayerTreeImpl::BreakSwapPromises(SwapPromise::DidNotSwapReason reason)
{
    base::TimeTicks invocation_timestamp = base::TimeTicks::Now();
    {
        ...
        for (auto& swap_promise : swap_promise_list_) {
            if (swap_promise->DidNotSwap(reason, invocation_timestamp) == SwapPromise::DidNotSwapAction::KEEP_ACTIVE) {
...
void LayerTreeImpl::set_ui_resource_request_queue(UIResourceRequestQueue queue)
{
...
{
    TRACE_EVENT1("cc", "ProcessUIResourceRequestQueue", "queue_size", ui_resource_request_queue_.size());
...
void LayerTreeImpl::NotifyLayerHasPaintWorkletsChanged(PictureLayerImpl* layer, bool has_worklets)
{
    if (has_worklets) {
        ...
        CHECK(it != picture_layers_with_paint_worklets_.end(), base::NotFatalUntil::M130);
...
void LayerTreeImpl::RegisterScrollbar(ScrollbarLayerImplBase* scrollbar_layer)
{
    ...
    int* scrollbar_layer_id = scrollbar_layer->orientation() == ScrollbarOrientation::kHorizontal ? &scrollbar_ids->horizontal : &scrollbar_ids->vertical;
    ...
    if (*scrollbar_layer_id != Layer::INVALID_ID) {
        ...
        scrollbar_layer_id = scrollbar_layer->orientation() == ScrollbarOrientation::kHorizontal ? &scrollbar_ids->horizontal : &scrollbar_ids->vertical;
    }

    *scrollbar_layer_id = scrollbar_layer->id();

    if (IsActiveTree()) {
        host_impl_->DidRegisterScrollbarLayer(scroll_element_id, scrollbar_layer->orientation());
    }

    if (IsActiveTree() && scrollbar_layer->is_overlay_scrollbar() && scrollbar_layer->GetScrollbarAnimator() != LayerTreeSettings::NO_ANIMATOR) {
        // Fluent overlay scrollbars are invisible until the DidRequestShow gets
        // called.
        if (scrollbar_layer->IsFluentOverlayScrollbarEnabled()) {
            scrollbar_layer->SetOverlayScrollbarLayerOpacityAnimated(0.f, /*fade_out_animation=*/false);
        }
        host_impl_->RegisterScrollbarAnimationController(scroll_element_id, scrollbar_layer->Opacity());
    }
}

void LayerTreeImpl::UnregisterScrollbar(ScrollbarLayerImplBase* scrollbar_layer)
{
    ElementId scroll_element_id = scrollbar_layer->scroll_element_id();
    if (!scroll_element_id)
        return;
    ...
    if (scrollbar_ids.horizontal == Layer::INVALID_ID && scrollbar_ids.vertical == Layer::INVALID_ID) {
        element_id_to_scrollbar_layer_ids_.erase(scroll_element_id);
    }

    if (IsActiveTree()) {
        host_impl_->DidUnregisterScrollbarLayer(scroll_element_id, scrollbar_layer->orientation());
    }
}
...
static bool PointHitsRect(const gfx::PointF& screen_space_point, const gfx::Transform& local_space_to_screen_space_transform, const gfx::Rect& local_space_rect,
    float* distance_to_camera)
{
    ...
    // given rect.
    bool clipped = false;
    gfx::PointF hit_test_point_in_local_space = MathUtil::ProjectPoint(inverse_local_space_to_screen_space, screen_space_point, &clipped);
    ...
    if (distance_to_camera) {
        ...
        // z-axis.
        gfx::Point3F planar_point_in_screen_space = local_space_to_screen_space_transform.MapPoint(gfx::Point3F(hit_test_point_in_local_space));
...
static bool PointIsClippedByAncestorClipNode(const gfx::PointF& screen_space_point, const LayerImpl* layer)
{
    ...
    for (const ClipNode* clip_node = clip_tree.Node(layer->clip_tree_index()); clip_node->id > kViewportPropertyNodeId;
         clip_node = clip_tree.parent(clip_node)) {
        if (clip_node->AppliesLocalClip()) {
            clip = gfx::ToEnclosingRect(clip_node->clip);

            gfx::Transform screen_space_transform = transform_tree.ToScreen(clip_node->transform_id);
            if (!PointHitsRect(screen_space_point, screen_space_transform, clip, nullptr)) {
                return true;
            }
        }
    }
...
static bool PointIsClippedBySurfaceOrClipRect(const gfx::PointF& screen_space_point, const LayerImpl* layer)
{
...
static bool PointHitsRegion(
    const gfx::PointF& screen_space_point, const gfx::Transform& screen_space_transform, const Region& layer_space_region, const LayerImpl* layer_impl)
{
    ...
    // given region.
    bool clipped = false;
    gfx::PointF hit_test_point_in_layer_space = MathUtil::ProjectPoint(inverse_screen_space_transform, screen_space_point, &clipped);
    ...
    if (layer_impl && PointIsClippedBySurfaceOrClipRect(screen_space_point, layer_impl))
        return false;

    return layer_space_region.Contains(gfx::ToRoundedPoint(hit_test_point_in_layer_space));
}

static bool PointHitsLayer(const LayerImpl* layer, const gfx::PointF& screen_space_point, float* distance_to_intersection)
{
...
static void FindClosestMatchingLayer(const gfx::PointF& screen_space_point, LayerImpl* root_layer, const Functor& func, FindClosestMatchingLayerState* state)
{
    ...
    for (auto* layer : base::Reversed(*root_layer->layer_tree_impl())) {
        ...
        bool in_front_of_previous_candidate = state->closest_match && layer->GetSortingContextId() == state->closest_match->GetSortingContextId()
            && distance_to_intersection > state->closest_distance + std::numeric_limits<float>::epsilon();
...
LayerImpl* LayerTreeImpl::FindLayerThatIsHitByPoint(const gfx::PointF& screen_space_point)
{
    if (layer_list_.empty())
        return nullptr;
    ...
    FindClosestMatchingLayer(screen_space_point, layer_list_[0].get(), HitTestVisibleScrollableOrTouchableFunctor(), &state);
...
struct FindTouchEventLayerFunctor {
    bool operator()(LayerImpl* layer) const
    {
        if (!layer->has_touch_action_regions())
            return false;
        return PointHitsRegion(screen_space_point, layer->ScreenSpaceTransform(), layer->GetAllTouchActionRegions(), layer);
    }
...
struct FindWheelEventHandlerLayerFunctor {
    bool operator()(LayerImpl* layer) const
    {
        return PointHitsRegion(screen_space_point, layer->ScreenSpaceTransform(), layer->wheel_event_handler_region(), layer);
    }
    const gfx::PointF screen_space_point;
};

template <typename Functor> LayerImpl* LayerTreeImpl::FindLayerThatIsHitByPointInEventHandlerRegion(const gfx::PointF& screen_space_point, const Functor& func)
{
    if (layer_list_.empty())
        return nullptr;
    bool update_tiles = !features::IsCCSlimmingEnabled();
    ...
    FindClosestMatchingLayerState state;
    FindClosestMatchingLayer(screen_space_point, layer_list_[0].get(), func, &state);
    return state.closest_match;
}

LayerImpl* LayerTreeImpl::FindLayerThatIsHitByPointInTouchHandlerRegion(const gfx::PointF& screen_space_point)
{
    FindTouchEventLayerFunctor func = { screen_space_point };
    return FindLayerThatIsHitByPointInEventHandlerRegion(screen_space_point, func);
}

LayerImpl* LayerTreeImpl::FindLayerThatIsHitByPointInWheelEventHandlerRegion(const gfx::PointF& screen_space_point)
{
    FindWheelEventHandlerLayerFunctor func = { screen_space_point };
    return FindLayerThatIsHitByPointInEventHandlerRegion(screen_space_point, func);
}

std::vector<const LayerImpl*> LayerTreeImpl::FindLayersUpToFirstScrollableOrOpaqueToHitTest(const gfx::PointF& screen_space_point)
{
    ...
    for (const auto* layer : base::Reversed(*root_layer->layer_tree_impl())) {
        ...
        if (first_hit && layer->Is3dSorted() && layer->GetSortingContextId() != first_hit->GetSortingContextId()) {
        ...
        if (first_hit->Is3dSorted() && layer->Is3dSorted()) {
            layers_3d.emplace_back(std::pair<const LayerImpl*, float>(layer, distance_to_intersection));
        } else {
            layers.push_back(layer);
            if (settings().enable_hit_test_opaqueness ? layer->OpaqueToHitTest() : layer->IsScrollerOrScrollbar()) {
                break;
            }
        }
    }
    ...
    if (first_hit->Is3dSorted()) {
        ...
        std::sort(layers_3d.begin(), layers_3d.end(),
            [](const std::pair<const LayerImpl*, float>& a, const std::pair<const LayerImpl*, float>& b) { return a.second > b.second; });
...
bool LayerTreeImpl::PointHitsMainThreadScrollHitTestRegion(const gfx::PointF& screen_space_point, const LayerImpl& layer) const
{
    ...
    return PointHitsRegion(screen_space_point, layer.ScreenSpaceTransform(), layer.main_thread_scroll_hit_test_region(), &layer);
}

ElementId LayerTreeImpl::PointHitsNonCompositedScroll(const gfx::PointF& screen_space_point, const LayerImpl& layer) const
{
    const std::vector<ScrollHitTestRect>* hit_test_rects = layer.non_composited_scroll_hit_test_rects();
    if (!hit_test_rects) {
        return ElementId();
    }
    for (const ScrollHitTestRect& rect : base::Reversed(*hit_test_rects)) {
        if (PointHitsRect(screen_space_point, layer.ScreenSpaceTransform(), rect.hit_test_rect, /*distance_to_camera=*/nullptr)) {
...
{
    const auto& transform_tree = layer->layer_tree_impl()->property_trees()->transform_tree();
...
static void FindClosestMatchingLayerForAttribution(const gfx::PointF& screen_space_point, const LayerImpl* root_layer, FindClosestMatchingLayerState* state)
{
    ...
    if (auto* layer = state->closest_match) {
        const auto& transform_tree = layer->layer_tree_impl()->property_trees()->transform_tree();
        for (const auto* node = transform_tree.Node(layer->transform_tree_index()); node; node = transform_tree.Node(node->parent_frame_id)) {
...
ElementId LayerTreeImpl::FindFrameElementIdAtPoint(const gfx::PointF& screen_space_point)
{
    if (layer_list_.empty())
        return {};
    bool update_tiles = !features::IsCCSlimmingEnabled();
    ...
    FindClosestMatchingLayerState state;
    FindClosestMatchingLayerForAttribution(screen_space_point, layer_list_[0].get(), &state);
    ...
    if (const auto* layer = state.closest_match) {
        ...
        bool layer_hit_test_region_is_masked = property_trees()->effect_tree().HitTestMayBeAffectedByMask(layer->effect_tree_index());
...
static gfx::SelectionBound ComputeViewportSelectionBound(const LayerSelectionBound& layer_bound, LayerImpl* layer, float device_scale_factor)
{
    ...
    bool clipped = false;
    gfx::PointF screen_start = MathUtil::MapPoint(screen_space_transform, layer_start, &clipped);
    gfx::PointF screen_end = MathUtil::MapPoint(screen_space_transform, layer_end, &clipped);
    ...
    if (layer_bound.hidden) {
        ...
        viewport_bound.set_visible(PointHitsLayer(layer, visibility_point, &intersect_distance));
    }

    if (viewport_bound.visible()) {
        viewport_bound.SetVisibleEdge(viewport_bound.edge_start(), viewport_bound.edge_end());
    } else {
        ...
        if (!visible_layer_rect.Contains(visible_layer_start) && !visible_layer_rect.Contains(visible_layer_end))
            std::tie(visible_layer_start, visible_layer_end) = GetVisibleSelectionEndPoints(visible_layer_rect, layer_start, layer_end);

        gfx::PointF visible_screen_start = MathUtil::MapPoint(screen_space_transform, visible_layer_start, &clipped);
        gfx::PointF visible_screen_end = MathUtil::MapPoint(screen_space_transform, visible_layer_end, &clipped);

        viewport_bound.SetVisibleEdgeStart(gfx::ScalePoint(visible_screen_start, inv_scale));
...
void LayerTreeImpl::GetViewportSelection(viz::Selection<gfx::SelectionBound>* selection)
{
    DCHECK(selection);

    selection->start = ComputeViewportSelectionBound(
        selection_.start, selection_.start.layer_id ? LayerById(selection_.start.layer_id) : nullptr, device_scale_factor() * painted_device_scale_factor());
    if (selection->start.type() == gfx::SelectionBound::CENTER || selection->start.type() == gfx::SelectionBound::HIDDEN
        || selection->start.type() == gfx::SelectionBound::EMPTY) {
        selection->end = selection->start;
    } else {
        selection->end = ComputeViewportSelectionBound(
            selection_.end, selection_.end.layer_id ? LayerById(selection_.end.layer_id) : nullptr, device_scale_factor() * painted_device_scale_factor());
    }
}
...
VideoFrameControllerClient* LayerTreeImpl::GetVideoFrameControllerClient() const
{
    return host_impl_;
}

void LayerTreeImpl::UpdateImageDecodingHints(base::flat_map<PaintImage::Id, PaintImage::DecodingMode> decoding_mode_map)
{
    host_impl_->UpdateImageDecodingHints(std::move(decoding_mode_map));
}

int LayerTreeImpl::GetMSAASampleCountForRaster(const DisplayItemList& display_list) const
{
    return host_impl_->GetMSAASampleCountForRaster(display_list);
}

TargetColorParams LayerTreeImpl::GetTargetColorParams(gfx::ContentColorUsage content_color_usage) const
{
    return host_impl_->GetTargetColorParams(content_color_usage);
}

void LayerTreeImpl::SetPendingPageScaleAnimation(std::unique_ptr<PendingPageScaleAnimation> pending_animation)
{
    pending_page_scale_animation_ = std::move(pending_animation);
}

std::unique_ptr<PendingPageScaleAnimation> LayerTreeImpl::TakePendingPageScaleAnimation()
{
    return std::move(pending_page_scale_animation_);
}

void LayerTreeImpl::AppendEventsMetricsFromMainThread(EventMetrics::List events_metrics)
{
    events_metrics_from_main_thread_.reserve(events_metrics_from_main_thread_.size() + events_metrics.size());
    events_metrics_from_main_thread_.insert(
        events_metrics_from_main_thread_.end(), std::make_move_iterator(events_metrics.begin()), std::make_move_iterator(events_metrics.end()));
}
...
void LayerTreeImpl::AddViewTransitionRequest(std::unique_ptr<ViewTransitionRequest> request)
{
...
std::vector<std::unique_ptr<ViewTransitionRequest>> LayerTreeImpl::TakeViewTransitionRequests()
{
...
void LayerTreeImpl::SetViewTransitionContentRect(const viz::ViewTransitionElementResourceId& id, const gfx::RectF& rect)
{
    // TODO(noamr) this could be more efficient, but usually there would be only 1
    // or 2 anyway.
    for (const auto& request : view_transition_requests_) {
        if (request->type() == ViewTransitionRequest::Type::kSave && request->token() == id.transition_token()) {
            host_impl_->SetViewTransitionContentRect(request->sequence_id(), id, rect);
        }
    }
}
