...
public:
    ...
    scoped_refptr<DisplayItemList> PaintContentsToDisplayList() override
    {
        ...
        for (const auto& holder : holders_) {
            ...
            display_list->push<DrawTextBlobOp>(SkTextBlob::MakeFromString(holder.text().data(), font), static_cast<float>(holder.rect().x()),
                static_cast<float>(holder.rect().y()), holder.node_id(), PaintFlags());
...
protected:
    ...
    {
        PostCaptureContentToMainThread();
    }
    ...
    void SetupRootPictureLayer(const gfx::Size& size)
    {
        ...
        layer_tree_host()->SetVisualDeviceViewportIntersectionRect(gfx::Rect(device_bounds_));
    }
    ...
    void VerifyCapturedContent(std::vector<FakeTextHolder>* expected_result)
    {
        ...
        for (auto& c : captured_content_) {
            for (auto it = expected_result->begin(); it != expected_result->end(); ++it) {
...
private:
    ...
    {
        MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&LayerTreeHostCaptureContentTest::CaptureContent, weak_factory_.GetWeakPtr()));
    }
...
class LayerTreeHostCaptureContentTestBasic : public LayerTreeHostCaptureContentTest {
protected:
...
class LayerTreeHostCaptureContentTestOneVisible : public LayerTreeHostCaptureContentTestBasic {
protected:
...
class LayerTreeHostCaptureContentTestTwoVisible : public LayerTreeHostCaptureContentTestBasic {
protected:
    void SetupTree() override
    {
...
class LayerTreeHostCaptureContentTestTwoLayers : public LayerTreeHostCaptureContentTestBasic {
protected:
    void SetupTree() override
    {
    ...
    virtual bool IsSecondaryPictureLayerContentsOpaque()
    {
        return true;
    }
    ...
    void SetupTransform(const gfx::Vector2dF& translate)
    {
        ...
        transform_node.id = layer_tree_host()->property_trees()->transform_tree_mutable().Insert(transform_node, 0);
        picture_layer->SetTransformTreeIndex(transform_node.id);
        layer_tree_host()->property_trees()->transform_tree_mutable().UpdateTransforms(transform_node.id);
    }

    void SetupSecondaryPictureLayer(const gfx::Size& size)
    {
        // Add text to layer.
        text_holder_21_ = std::make_unique<FakeTextHolder>("Text21", gfx::Rect(0, 0, 10, 5), 21);
        client2_.addTextHolder(*text_holder_21_);
        text_holder_22_ = std::make_unique<FakeTextHolder>("Text22", gfx::Rect(0, 5, 10, 5), 22);
...
class LayerTreeHostCaptureContentTestOneLayerVisible : public LayerTreeHostCaptureContentTestTwoLayers {
...
class LayerTreeHostCaptureContentTestTwoLayersVisible : public LayerTreeHostCaptureContentTestTwoLayers {
    void OnSetupSecondaryLayTransform() override
    {
...
class LayerTreeHostCaptureContentTestTwoLayersVisibleAndTransparent : public LayerTreeHostCaptureContentTestTwoLayersVisible {
    bool IsSecondaryPictureLayerContentsOpaque() override
    {
        return false;
    }
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostCaptureContentTestTwoLayersVisibleAndTransparent);
...
class LayerTreeHostCaptureContentTestUpperLayerPartialOverlay : public LayerTreeHostCaptureContentTestTwoLayers {
    void OnSetupSecondaryLayTransform() override
    {
...
SINGLE_AND_MULTI_THREAD_TEST_F(LayerTreeHostCaptureContentTestUpperLayerPartialOverlay);
