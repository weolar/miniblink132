...
private:
    void BuildPropertyTreesInternal(Layer* layer, const DataForRecursion& data_from_parent) const;

    bool AddTransformNodeIfNeeded(
        const DataForRecursion& data_from_ancestor, Layer* layer, bool created_render_surface, DataForRecursion* data_for_children) const;

    void AddClipNodeIfNeeded(const DataForRecursion& data_from_ancestor, Layer* layer, bool created_transform_node, DataForRecursion* data_for_children) const;

    bool AddEffectNodeIfNeeded(const DataForRecursion& data_from_ancestor, Layer* layer, DataForRecursion* data_for_children) const;

    void AddScrollNodeIfNeeded(const DataForRecursion& data_from_ancestor, Layer* layer, DataForRecursion* data_for_children) const;

    bool UpdateRenderSurfaceIfNeeded(int parent_effect_tree_id, DataForRecursion* data_for_children, bool is_rounded_corner_layer_within_parent_bounds,
        bool subtree_has_overlapping_rounded_corner, bool subtree_has_gradient_mask, bool created_transform_node) const;
...
{
    return host.IsAnimatingProperty(layer->element_id(), layer->GetElementTypeForAnimation(), TargetProperty::OPACITY);
}

bool HasPotentiallyRunningOpacityAnimation(const MutatorHost& host, Layer* layer)
{
    return host.HasPotentiallyRunningAnimationForProperty(layer->element_id(), layer->GetElementTypeForAnimation(), TargetProperty::OPACITY);
}

bool HasPotentialOpacityAnimation(const MutatorHost& host, Layer* layer)
{
    return HasPotentiallyRunningOpacityAnimation(host, layer) || layer->OpacityCanAnimateOnImplThread();
}
...
bool HasPotentiallyRunningFilterAnimation(const MutatorHost& host, Layer* layer)
{
...
{
    DCHECK(!host.IsAnimatingProperty(layer->element_id(), layer->GetElementTypeForAnimation(), TargetProperty::SCALE)
        && !host.IsAnimatingProperty(layer->element_id(), layer->GetElementTypeForAnimation(), TargetProperty::ROTATE)
        && !host.IsAnimatingProperty(layer->element_id(), layer->GetElementTypeForAnimation(), TargetProperty::TRANSLATE))
...
bool HasPotentiallyRunningTransformAnimation(const MutatorHost& host, Layer* layer)
{
    DCHECK(!host.HasPotentiallyRunningAnimationForProperty(layer->element_id(), layer->GetElementTypeForAnimation(), TargetProperty::SCALE)
        && !host.HasPotentiallyRunningAnimationForProperty(layer->element_id(), layer->GetElementTypeForAnimation(), TargetProperty::ROTATE)
        && !host.HasPotentiallyRunningAnimationForProperty(layer->element_id(), layer->GetElementTypeForAnimation(), TargetProperty::TRANSLATE))
        << "individual transform properties only supported in layer lists mode";
...
{
    return host.MaximumScale(layer->element_id(), layer->GetElementTypeForAnimation());
}
...
bool HasAnyAnimationTargetingProperty(const MutatorHost& host, Layer* layer, TargetProperty::Type property)
{
...
{
    return LayerClipsSubtreeToItsBounds(layer) || layer->HasMaskFilter() || !layer->clip_rect().IsEmpty();
}

void PropertyTreeBuilderContext::AddClipNodeIfNeeded(
    const DataForRecursion& data_from_ancestor, Layer* layer, bool created_transform_node, DataForRecursion* data_for_children) const
{
    const int parent_id = data_from_ancestor.clip_tree_parent;

    bool layer_clips_subtree = LayerClipsSubtree(layer);
    bool requires_node = layer_clips_subtree || layer->filters().HasFilterThatMovesPixels();
    if (!requires_node) {
        ...
        node.transform_id = created_transform_node ? data_for_children->transform_tree_parent : data_from_ancestor.transform_tree_parent;
    ...
    const DataForRecursion& data_from_ancestor, Layer* layer, bool created_render_surface, DataForRecursion* data_for_children) const
{
    ...
    const bool is_snapped = is_scrollable || layer->IsSnappedToPixelGridInTarget();

    const bool has_significant_transform = !layer->transform().IsIdentityOr2dTranslation();

    const bool has_potentially_animated_transform = HasPotentiallyRunningTransformAnimation(*mutator_host_, layer);
    ...
    const bool has_any_transform_animation = HasAnyAnimationTargetingProperty(*mutator_host_, layer, TargetProperty::TRANSFORM);
    DCHECK(!HasAnyAnimationTargetingProperty(*mutator_host_, layer, TargetProperty::SCALE)
        && !HasAnyAnimationTargetingProperty(*mutator_host_, layer, TargetProperty::ROTATE)
        && !HasAnyAnimationTargetingProperty(*mutator_host_, layer, TargetProperty::TRANSLATE))
        << "individual transform properties only supported in layer lists mode";

    const bool has_surface = created_render_surface;

    DCHECK(!is_scrollable || is_snapped);
    bool requires_node = is_root || is_snapped || has_significant_transform || has_any_transform_animation || has_surface || layer->HasMaskFilter();
    ...
    if (is_root) {
        ...
        transform_tree_->SetRootScaleAndTransform(transform_tree_->device_scale_factor(), gfx::Transform());
    } else {
        node->local = layer->transform();
        node->origin = layer->transform_origin();
        node->post_translation = parent_offset + layer->position().OffsetFromOrigin();
    }
...
RenderSurfaceReason ComputeRenderSurfaceReason(const MutatorHost& mutator_host, Layer* layer, gfx::Transform current_transform, bool animation_axis_aligned)
{
    const bool preserves_2d_axis_alignment = current_transform.Preserves2dAxisAlignment() && animation_axis_aligned;
    const bool is_root = !layer->parent();
    if (is_root)
    ...
    if (!layer->is_fast_rounded_corner() && layer->HasRoundedCorner() && num_descendants_that_draw_content > 1) {
    ...
    bool at_least_two_layers_in_subtree_draw_content
        = num_descendants_that_draw_content > 0 && (layer->draws_content() || num_descendants_that_draw_content > 1);

    bool may_have_transparency = layer->EffectiveOpacity() != 1.f || HasPotentiallyRunningOpacityAnimation(mutator_host, layer);
...
bool UpdateSubtreeHasCopyRequestRecursive(Layer* layer)
{
    ...
    for (const scoped_refptr<Layer>& child : layer->children()) {
        subtree_has_copy_request |= UpdateSubtreeHasCopyRequestRecursive(child.get());
    }
    layer->SetSubtreeHasCopyRequest(subtree_has_copy_request);
    return subtree_has_copy_request;
}

bool PropertyTreeBuilderContext::AddEffectNodeIfNeeded(const DataForRecursion& data_from_ancestor, Layer* layer, DataForRecursion* data_for_children) const
{
    const bool is_root = !layer->parent();
    const bool has_transparency = layer->EffectiveOpacity() != 1.f;
    const bool has_potential_opacity_animation = HasPotentialOpacityAnimation(*mutator_host_, layer);
    const bool has_potential_filter_animation = HasPotentiallyRunningFilterAnimation(*mutator_host_, layer);

    data_for_children->animation_axis_aligned_since_render_target &= AnimationsPreserveAxisAlignment(*mutator_host_, layer);
    data_for_children->compound_transform_since_render_target *= layer->transform();
    auto render_surface_reason = ComputeRenderSurfaceReason(
        *mutator_host_, layer, data_for_children->compound_transform_since_render_target, data_for_children->animation_axis_aligned_since_render_target);
    bool should_create_render_surface = render_surface_reason != RenderSurfaceReason::kNone;

    bool not_axis_aligned_since_last_clip = data_from_ancestor.not_axis_aligned_since_last_clip
        ? true
        : !AnimationsPreserveAxisAlignment(*mutator_host_, layer) || !layer->transform().Preserves2dAxisAlignment();
    // A non-axis aligned clip may need a render surface. So, we create an effect
    // node.
    bool has_non_axis_aligned_clip = not_axis_aligned_since_last_clip && LayerClipsSubtree(layer);

    bool requires_node = is_root || has_transparency || has_potential_opacity_animation || has_potential_filter_animation || has_non_axis_aligned_clip
    ...
    node->element_id = layer->element_id() ? layer->element_id() : ElementId(layer->id());
    ...
    if (node->subtree_capture_id.is_valid()) {
        ...
        node->subtree_size = gfx::ScaleToFlooredSize(layer->bounds(), layer_tree_host_->device_scale_factor());
    }
    ...
    node->is_currently_animating_opacity = OpacityIsAnimating(*mutator_host_, layer);
    ...
    node->closest_ancestor_with_cached_render_surface_id
        = layer->cache_render_surface() ? node_id : data_from_ancestor.closest_ancestor_with_cached_render_surface;
    node->closest_ancestor_with_copy_request_id = layer->HasCopyRequest() ? node_id : data_from_ancestor.closest_ancestor_with_copy_request;
    node->closest_ancestor_being_captured_id = layer->subtree_capture_id().is_valid() ? node_id : data_from_ancestor.closest_ancestor_being_captured;
    ...
    if (layer->HasMaskFilter()) {
        ...
        node->mask_filter_info = gfx::MaskFilterInfo(layer->EffectiveClipRect(), layer->corner_radii(), layer->gradient_mask());
    ...
    data_for_children->closest_ancestor_with_cached_render_surface = node->closest_ancestor_with_cached_render_surface_id;
    data_for_children->closest_ancestor_with_copy_request = node->closest_ancestor_with_copy_request_id;
    data_for_children->closest_ancestor_being_captured = node->closest_ancestor_being_captured_id;
...
bool PropertyTreeBuilderContext::UpdateRenderSurfaceIfNeeded(int parent_effect_tree_id, DataForRecursion* data_for_children,
    bool is_rounded_corner_layer_within_parent_bounds, bool subtree_has_overlapping_rounded_corner, bool subtree_has_gradient_mask,
    bool created_transform_node) const
{
    ...
    if (parent_effect_tree_id == data_for_children->effect_tree_parent) {
        ...
        *data_for_children->subtree_has_overlapping_rounded_corner = subtree_has_overlapping_rounded_corner;
        return false;
    }

    EffectNode* effect_node = effect_tree_->Node(data_for_children->effect_tree_parent);
    const bool has_rounded_corner = effect_node->mask_filter_info.HasRoundedCorners();
    ...
    *data_for_children->subtree_has_overlapping_rounded_corner
        = (subtree_has_overlapping_rounded_corner && !effect_node->HasRenderSurface()) || (has_rounded_corner && !is_rounded_corner_layer_within_parent_bounds);
    *data_for_children->subtree_has_gradient_mask = (subtree_has_gradient_mask && !effect_node->HasRenderSurface()) || has_gradient_mask;
    return effect_node->HasRenderSurface();
}

bool PropertyTreeBuilderContext::IsRoundedCornerLayerWithinParentLayerBounds(const Layer* layer) const
{
    ...
    if (!layer->transform().IsIdentity() || layer->filters().HasFilterThatMovesPixels()) {
        return false;
    }
    ...
    const gfx::RectF current_layers_bounds = gfx::RectF(layer->position(), gfx::SizeF(layer->bounds()));
    const gfx::RectF parent_rect = gfx::RectF(gfx::PointF(0, 0), gfx::SizeF(parent_layer->bounds()));
    return parent_rect.Contains(current_layers_bounds);
}

void PropertyTreeBuilderContext::AddScrollNodeIfNeeded(const DataForRecursion& data_from_ancestor, Layer* layer, DataForRecursion* data_for_children) const
{
    ...
    if (!requires_node) {
        ...
        node.user_scrollable_horizontal = node.user_scrollable_vertical = scrollable;
        node.element_id = layer->element_id();
        ...
        scroll_tree_->SetBaseScrollOffset(layer->element_id(), layer->scroll_offset());
    }

    layer->SetScrollTreeIndex(node_id);
}

void SetSafeOpaqueBackgroundColor(const DataForRecursion& data_from_ancestor, Layer* layer, DataForRecursion* data_for_children)
{
    SkColor4f background_color = layer->background_color();
    data_for_children->safe_opaque_background_color = background_color.isOpaque() ? background_color : data_from_ancestor.safe_opaque_background_color;
    layer->SetSafeOpaqueBackgroundColor(data_for_children->safe_opaque_background_color);
}

void PropertyTreeBuilderContext::BuildPropertyTreesInternal(Layer* layer, const DataForRecursion& data_from_parent) const
{
    ...
    *data_for_children.subtree_has_gradient_mask = false;
    ...
    bool created_transform_node = AddTransformNodeIfNeeded(data_from_parent, layer, created_render_surface, &data_for_children);
    layer->SetHasTransformNode(created_transform_node);
    AddClipNodeIfNeeded(data_from_parent, layer, created_transform_node, &data_for_children);
    ...
    bool not_axis_aligned_since_last_clip = data_from_parent.not_axis_aligned_since_last_clip
        ? true
        : !AnimationsPreserveAxisAlignment(*mutator_host_, layer) || !layer->transform().Preserves2dAxisAlignment();
    bool has_non_axis_aligned_clip = not_axis_aligned_since_last_clip && LayerClipsSubtree(layer);
    data_for_children.not_axis_aligned_since_last_clip = !has_non_axis_aligned_clip;
    ...
    for (const scoped_refptr<Layer>& child : layer->children()) {
        ...
        subtree_has_overlapping_rounded_corner |= *data_for_children.subtree_has_overlapping_rounded_corner;
        subtree_has_gradient_mask |= *data_for_children.subtree_has_gradient_mask;
    }

    const bool is_rounded_corner_layer_within_parent_bounds = IsRoundedCornerLayerWithinParentLayerBounds(layer);
    created_render_surface = UpdateRenderSurfaceIfNeeded(data_from_parent.effect_tree_parent, &data_for_children, is_rounded_corner_layer_within_parent_bounds,
        subtree_has_overlapping_rounded_corner, subtree_has_gradient_mask, created_transform_node);
}
...
void PropertyTreeBuilderContext::BuildPropertyTrees()
{
    ...
    if (!property_trees_->needs_rebuild()) {
        clip_tree_->SetViewportClip(gfx::RectF(layer_tree_host_->device_viewport_rect()));
        ...
        transform_tree_->SetRootScaleAndTransform(layer_tree_host_->device_scale_factor(), gfx::Transform());
        return;
    }
    ...
    data_for_recursion.closest_ancestor_with_cached_render_surface = kInvalidPropertyNodeId;
    data_for_recursion.closest_ancestor_with_copy_request = kInvalidPropertyNodeId;
    ...
    transform_tree_->set_device_scale_factor(layer_tree_host_->device_scale_factor());
    ClipNode root_clip;
    root_clip.clip = gfx::RectF(layer_tree_host_->device_viewport_rect());
    root_clip.transform_id = kRootPropertyNodeId;
    data_for_recursion.clip_tree_parent = clip_tree_->Insert(root_clip, kRootPropertyNodeId);

    data_for_recursion.subtree_has_overlapping_rounded_corner = &subtree_has_overlapping_rounded_corner;
