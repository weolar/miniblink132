...
std::unique_ptr<Proxy> SingleThreadProxy::Create(
    LayerTreeHost* layer_tree_host, LayerTreeHostSingleThreadClient* client, TaskRunnerProvider* task_runner_provider)
{
    return base::WrapUnique(new SingleThreadProxy(layer_tree_host, client, task_runner_provider));
}

SingleThreadProxy::SingleThreadProxy(LayerTreeHost* layer_tree_host, LayerTreeHostSingleThreadClient* client, TaskRunnerProvider* task_runner_provider)
...
void SingleThreadProxy::Start()
{
    ...
    DCHECK(settings.single_thread_proxy_scheduler || !settings.enable_checker_imaging)
    ...
    if (settings.single_thread_proxy_scheduler && !scheduler_on_impl_thread_) {
        ...
        std::unique_ptr<CompositorTimingHistory> compositor_timing_history(
            new CompositorTimingHistory(CompositorTimingHistory::BROWSER_UMA, layer_tree_host_->rendering_stats_instrumentation()));
        scheduler_on_impl_thread_ = std::make_unique<Scheduler>(this, scheduler_settings, layer_tree_host_->GetId(),
            task_runner_provider_->MainThreadTaskRunner(), std::move(compositor_timing_history), host_impl_->compositor_frame_reporting_controller());
    }
}
...
void SingleThreadProxy::DidObserveFirstScrollDelay(int source_frame_number, base::TimeDelta first_scroll_delay, base::TimeTicks first_scroll_timestamp)
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
}
...
void SingleThreadProxy::SetLayerTreeFrameSink(LayerTreeFrameSink* layer_tree_frame_sink)
{
    DCHECK(task_runner_provider_->IsMainThread());
...
void SingleThreadProxy::DoCommit(const viz::BeginFrameArgs& commit_args)
{
    ...
    devtools_instrumentation::ScopedCommitTrace commit_task(layer_tree_host_->GetId(), commit_args.frame_id.sequence_number);
...
void SingleThreadProxy::IssueImageDecodeFinishedCallbacks()
{
    ...
    layer_tree_host_->ImageDecodesFinished(host_impl_->TakeCompletedImageDecodeRequests());
}
...
void SingleThreadProxy::CommitComplete()
{
    ...
    DCHECK(!host_impl_->pending_tree()) << "Activation is expected to have synchronously occurred by now.";
    CHECK_NE(source_frame_number_for_next_commit_, kInvalidSourceFrameNumber);

    DebugScopedSetMainThread main(task_runner_provider_);
    layer_tree_host_->DidBeginMainFrame();
    layer_tree_host_->CommitComplete(source_frame_number_for_next_commit_, { base::TimeTicks(), base::TimeTicks::Now() });
...
void SingleThreadProxy::SetTargetLocalSurfaceId(const viz::LocalSurfaceId& target_local_surface_id)
{
    DCHECK(task_runner_provider_->IsMainThread());
    if (!scheduler_on_impl_thread_)
        return;
...
{
    DCHECK(task_runner_provider_->IsMainThread());
    return animate_requested_ || update_layers_requested_ || commit_requested_ || needs_impl_frame_;
}
...
void SingleThreadProxy::SetDeferMainFrameUpdate(bool defer_main_frame_update)
{
    ...
    if (defer_main_frame_update) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("cc", "SingleThreadProxy::SetDeferMainFrameUpdate", TRACE_ID_LOCAL(this));
    } else {
        TRACE_EVENT_NESTABLE_ASYNC_END0("cc", "SingleThreadProxy::SetDeferMainFrameUpdate", TRACE_ID_LOCAL(this));
    }
...
void SingleThreadProxy::SetPauseRendering(bool pause_rendering)
{
    ...
    if (pause_rendering_) {
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("cc", "SingleThreadProxy::SetPauseRendering", TRACE_ID_LOCAL(this));
    } else {
        TRACE_EVENT_NESTABLE_ASYNC_END0("cc", "SingleThreadProxy::SetPauseRendering", TRACE_ID_LOCAL(this));
    }
...
bool SingleThreadProxy::StartDeferringCommits(base::TimeDelta timeout, PaintHoldingReason reason)
{
    DCHECK(task_runner_provider_->IsMainThread());
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("cc", "SingleThreadProxy::SetDeferCommits", TRACE_ID_LOCAL(this));
...
void SingleThreadProxy::StopDeferringCommits(PaintHoldingCommitTrigger trigger)
{
    DCHECK(task_runner_provider_->IsMainThread());
    if (!IsDeferringCommits())
        return;
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END0("cc", "SingleThreadProxy::SetDeferCommits", TRACE_ID_LOCAL(this));
...
void SingleThreadProxy::QueueImageDecode(int request_id, const PaintImage& image)
{
    DCHECK(task_runner_provider_->IsMainThread());
    DebugScopedSetImplThread impl(task_runner_provider_);
...
void SingleThreadProxy::SetPaintWorkletLayerPainter(std::unique_ptr<PaintWorkletLayerPainter> painter)
{
    NOTREACHED();
}
...
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
    TRACE_EVENT1("cc", "SingleThreadProxy::OnCanDrawStateChanged", "can_draw", can_draw);
    if (scheduler_on_impl_thread_)
...
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
    return scheduler_on_impl_thread_ && scheduler_on_impl_thread_->IsReadyToActivate();
}
...
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
    TRACE_EVENT1("cc", "SingleThreadProxy::SetVideoNeedsBeginFrames", "needs_begin_frames", needs_begin_frames);
...
void SingleThreadProxy::PostDelayedAnimationTaskOnImplThread(base::OnceClosure task, base::TimeDelta delay)
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
}
...
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
    TRACE_EVENT0("cc", "SingleThreadProxy::DidLoseLayerTreeFrameSinkOnImplThread");
    {
        DebugScopedSetMainThread main(task_runner_provider_);
...
void SingleThreadProxy::DidReceiveCompositorFrameAckOnImplThread()
{
    ...
    task_runner_provider_->MainThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&SingleThreadProxy::DidReceiveCompositorFrameAck, frame_sink_bound_weak_ptr_));
}

void SingleThreadProxy::OnDrawForLayerTreeFrameSink(bool resourceless_software_draw, bool skip_draw)
{
    NOTREACHED() << "Implemented by ThreadProxy for synchronous compositor.";
}

void SingleThreadProxy::SetNeedsImplSideInvalidation(bool needs_first_draw_on_activation)
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
    if (scheduler_on_impl_thread_) {
        scheduler_on_impl_thread_->SetNeedsImplSideInvalidation(needs_first_draw_on_activation);
    }
}

void SingleThreadProxy::NotifyImageDecodeRequestFinished(int request_id, bool decode_succeeded)
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
...
void SingleThreadProxy::NotifyTransitionRequestFinished(uint32_t sequence_id, const viz::ViewTransitionElementResourceRects& rects)
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
...
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
    host_impl_->NotifyDidPresentCompositorFrameOnImplThread(frame_token, std::move(callbacks.compositor_successful_callbacks), details);
    {
        DebugScopedSetMainThread main(task_runner_provider_);
        layer_tree_host_->DidPresentCompositorFrame(frame_token, std::move(callbacks.main_callbacks), std::move(callbacks.main_successful_callbacks), details);
    }
    if (scheduler_on_impl_thread_) {
...
void SingleThreadProxy::NotifyAnimationWorkletStateChange(AnimationWorkletMutationState state, ElementListType element_list_type)
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
    DebugScopedSetMainThread main(task_runner_provider_);
...
void SingleThreadProxy::NotifyPaintWorkletStateChange(Scheduler::PaintWorkletState state)
{
    // Off-Thread PaintWorklet is only supported on the threaded compositor.
    NOTREACHED();
}

void SingleThreadProxy::NotifyThroughputTrackerResults(CustomTrackerResults results)
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
...
void SingleThreadProxy::FrameSinksToThrottleUpdated(const base::flat_set<viz::FrameSinkId>& ids)
{
    DCHECK(!task_runner_provider_->HasImplThread() || task_runner_provider_->IsImplThread());
...
void SingleThreadProxy::RequestBeginMainFrameNotExpected(bool new_state)
{
    ...
    if (scheduler_on_impl_thread_) {
        DebugScopedSetImplThread impl(task_runner_provider_);
        scheduler_on_impl_thread_->SetMainThreadWantsBeginMainFrameNotExpected(new_state);
    }
}

viz::BeginFrameArgs SingleThreadProxy::BeginImplFrameForTest(base::TimeTicks frame_begin_time)
{
    viz::BeginFrameArgs begin_frame_args(viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, viz::BeginFrameArgs::kManualSourceId, begin_frame_sequence_number_++,
        frame_begin_time, base::TimeTicks(), viz::BeginFrameArgs::DefaultInterval(), viz::BeginFrameArgs::NORMAL));
...
void SingleThreadProxy::CompositeImmediatelyForTest(base::TimeTicks frame_begin_time, bool raster, base::OnceClosure callback)
{
    TRACE_EVENT0("cc,benchmark", "SingleThreadProxy::CompositeImmediatelyForTest");
    DCHECK(task_runner_provider_->IsMainThread());
#if DCHECK_IS_ON()
    ...
    viz::BeginFrameArgs begin_frame_args = BeginImplFrameForTest(frame_begin_time); // IN-TEST
    ...
    // Run the "main thread" and get it to commit.
    {
        ...
        DCHECK_EQ(0u, layer_tree_host_->GetSwapPromiseManager()->num_queued_swap_promises()) << "Commit should always succeed and transfer promises.";
    }
...
{
    if (layer_tree_frame_sink_creation_callback_.IsCancelled() && !layer_tree_frame_sink_creation_requested_) {
        layer_tree_frame_sink_creation_callback_.Reset(base::BindOnce(&SingleThreadProxy::RequestNewLayerTreeFrameSink, weak_factory_.GetWeakPtr()));
        task_runner_provider_->MainThreadTaskRunner()->PostTask(FROM_HERE, layer_tree_frame_sink_creation_callback_.callback());
    }
}
        ...
        if (draw_frame) {
            if (std::optional<SubmitInfo> submit_info = host_impl_->DrawLayers(frame)) {
                if (scheduler_on_impl_thread_) {
                    ...
                    scheduler_on_impl_thread_->DidSubmitCompositorFrame(submit_info.value());
                }
...
void SingleThreadProxy::SetUkmSmoothnessDestination(base::WritableSharedMemoryMapping ukm_smoothness_data)
{
    DCHECK(task_runner_provider_->IsMainThread());
}
...
void SingleThreadProxy::SetWaitingForScrollEvent(bool waiting_for_scroll_event)
{
    if (scheduler_on_impl_thread_) {
        NOTREACHED();
    }
...
size_t SingleThreadProxy::CommitDurationSampleCountForTesting() const
{
    ...
    return scheduler_on_impl_thread_->CommitDurationSampleCountForTesting(); // IN-TEST
}

void SingleThreadProxy::SetRenderFrameObserver(std::unique_ptr<RenderFrameMetadataObserver> observer)
{
    DCHECK(task_runner_provider_->IsMainThread());
    DebugScopedSetImplThread impl(task_runner_provider_);
...
{
    DebugScopedSetImplThread impl(task_runner_provider_);
    return host_impl_->dropped_frame_counter()->sliding_window_current_percent_dropped();
}
...
bool SingleThreadProxy::WillBeginImplFrame(const viz::BeginFrameArgs& args)
{
    DebugScopedSetImplThread impl(task_runner_provider_);
#if DCHECK_IS_ON()
    DCHECK(!inside_impl_frame_) << "WillBeginImplFrame called while already inside an impl frame!";
    inside_impl_frame_ = true;
#endif
    return host_impl_->WillBeginImplFrame(args);
}

void SingleThreadProxy::ScheduledActionSendBeginMainFrame(const viz::BeginFrameArgs& begin_frame_args)
{
    DebugScopedSetImplThread impl(task_runner_provider_);
    ...
    // synchronous commit.
    DCHECK(inside_impl_frame_) << "BeginMainFrame should only be sent inside a BeginImplFrame";
#endif

    host_impl_->WillSendBeginMainFrame();
    task_runner_provider_->MainThreadTaskRunner()->PostTask(
        FROM_HERE, base::BindOnce(&SingleThreadProxy::BeginMainFrame, weak_factory_.GetWeakPtr(), begin_frame_args));
}
...
void SingleThreadProxy::ScheduledActionBeginMainFrameNotExpectedUntil(base::TimeTicks time)
{
    // DebugScopedSetImplThread here is just a formality; all SchedulerClient
    // methods should have it.
    DebugScopedSetImplThread impl(task_runner_provider_);
...
void SingleThreadProxy::BeginMainFrame(const viz::BeginFrameArgs& begin_frame_args)
{
    ...
    if (defer_main_frame_update_) {
        TRACE_EVENT_INSTANT0("cc", "EarlyOut_DeferBeginMainFrame", TRACE_EVENT_SCOPE_THREAD);
        BeginMainFrameAbortedOnImplThread(CommitEarlyOutReason::kAbortedDeferredMainFrameUpdate);
        return;
    }
    ...
    // right now.
    if (defer_main_frame_update_ || IsDeferringCommits() || begin_frame_args.animate_only) {
        TRACE_EVENT_INSTANT0("cc", "EarlyOut_DeferCommit_InsideBeginMainFrame", TRACE_EVENT_SCOPE_THREAD);
        BeginMainFrameAbortedOnImplThread(CommitEarlyOutReason::kAbortedDeferredCommit);
...
void SingleThreadProxy::DoBeginMainFrame(const viz::BeginFrameArgs& begin_frame_args)
{
    ...
    const bool record_metrics = layer_tree_host_->GetSettings().is_layer_tree_for_ui;
#else
...
void SingleThreadProxy::DoPainting(const viz::BeginFrameArgs& commit_args)
{
    ...
    std::unique_ptr<BeginMainFrameMetrics> begin_main_frame_metrics = layer_tree_host_->TakeBeginMainFrameMetrics();
    ...
    // thread_proxy.cc
    if (scheduler_on_impl_thread_) {
        scheduler_on_impl_thread_->NotifyReadyToCommit(std::move(begin_main_frame_metrics));
    }
}

void SingleThreadProxy::BeginMainFrameAbortedOnImplThread(CommitEarlyOutReason reason)
{
    DebugScopedSetImplThread impl(task_runner_provider_);
    ...
    host_impl_->BeginMainFrameAborted(reason, std::move(empty_swap_promises), scheduler_on_impl_thread_->last_dispatched_begin_main_frame_args(),
...
DrawResult SingleThreadProxy::ScheduledActionDrawIfPossible()
{
    DebugScopedSetImplThread impl(task_runner_provider_);
    LayerTreeHostImpl::FrameData frame;
    frame.begin_frame_ack = scheduler_on_impl_thread_->CurrentBeginFrameAckForActiveTree();
    frame.origin_begin_main_frame_args = scheduler_on_impl_thread_->last_activate_origin_frame_args();
...
void SingleThreadProxy::ScheduledActionInvalidateLayerTreeFrameSink(bool needs_redraw)
{
...
void SingleThreadProxy::DidFinishImplFrame(const viz::BeginFrameArgs& last_activated_args)
{
    DebugScopedSetImplThread impl(task_runner_provider_);
    host_impl_->DidFinishImplFrame(last_activated_args);
#if DCHECK_IS_ON()
    DCHECK(inside_impl_frame_) << "DidFinishImplFrame called while not inside an impl frame!";
    inside_impl_frame_ = false;
#endif
}

void SingleThreadProxy::DidNotProduceFrame(const viz::BeginFrameAck& ack, FrameSkippedReason reason)
{
    DebugScopedSetImplThread impl(task_runner_provider_);
