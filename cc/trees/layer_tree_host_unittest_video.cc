...
class LayerTreeHostVideoTestSetNeedsDisplay : public LayerTreeHostVideoTest {
public:
    void SetupTree() override
    {
        ...
        scoped_refptr<VideoLayer> video = VideoLayer::Create(&video_frame_provider_, kTestTransform);
    ...
    DrawResult PrepareToDrawOnThread(LayerTreeHostImpl* host_impl, LayerTreeHostImpl::FrameData* frame, DrawResult draw_result) override
    {
        ...
        EXPECT_TRUE(root_surface->damage_tracker()->GetDamageRectIfValid(&damage_rect));
    ...
    {
        VideoLayerImpl* video = static_cast<VideoLayerImpl*>(host_impl->active_tree()->LayerById(video_layer_id_));
