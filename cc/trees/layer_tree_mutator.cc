...
AnimationWorkletInput::AddAndUpdateState::AddAndUpdateState(WorkletAnimationId worklet_animation_id, std::string name, double current_time,
...
AnimationWorkletInput::AddAndUpdateState::AddAndUpdateState(AddAndUpdateState&&) = default;
...
{
    return base::ranges::all_of(added_and_updated_animations, [worklet_id](auto& it) { return it.worklet_animation_id.worklet_id == worklet_id; })
        && base::ranges::all_of(updated_animations, [worklet_id](auto& it) { return it.worklet_animation_id.worklet_id == worklet_id; })
        && base::ranges::all_of(removed_animations, [worklet_id](auto& it) { return it.worklet_id == worklet_id; });
}
#endif
...
{
    AnimationWorkletInput& worklet_input = EnsureWorkletEntry(state.worklet_animation_id.worklet_id);
...
{
    AnimationWorkletInput& worklet_input = EnsureWorkletEntry(worklet_animation_id.worklet_id);
    worklet_input.removed_animations.push_back(worklet_animation_id);
}

std::unique_ptr<AnimationWorkletInput> MutatorInputState::TakeWorkletState(int worklet_id)
{
...
AnimationWorkletOutput::AnimationState::AnimationState(const AnimationState&) = default;
