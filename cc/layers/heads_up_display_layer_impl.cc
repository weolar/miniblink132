...
void DrawArc(PaintCanvas* canvas, const SkRect& oval, SkScalar start_angle, SkScalar sweep_angle, const PaintFlags& flags)
{
...
HeadsUpDisplayLayerImpl::HeadsUpDisplayLayerImpl(LayerTreeImpl* tree_impl, int id, const std::string& paused_localized_message)
...
std::unique_ptr<LayerImpl> HeadsUpDisplayLayerImpl::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
    return HeadsUpDisplayLayerImpl::Create(tree_impl, id(), paused_localized_message_);
}
...
public:
    ~HudGpuBacking() override
    {
        ...
        if (returned_sync_token.HasData())
            shared_image_interface->DestroySharedImage(returned_sync_token, std::move(shared_image));
        else if (mailbox_sync_token.HasData())
            shared_image_interface->DestroySharedImage(mailbox_sync_token, std::move(shared_image));
    }
...
public:
    ...
    void OnMemoryDump(base::trace_event::ProcessMemoryDump* pmd, const base::trace_event::MemoryAllocatorDumpGuid& buffer_dump_guid,
        uint64_t tracing_process_id, int importance) const override
    {
        pmd->CreateSharedMemoryOwnershipEdge(buffer_dump_guid, shared_mapping.guid(), importance);
    }
...
bool HeadsUpDisplayLayerImpl::WillDraw(DrawMode draw_mode, viz::ClientResourceProvider* resource_provider)
{
    if (draw_mode == DRAW_MODE_RESOURCELESS_SOFTWARE && !LayerImpl::WillDraw(draw_mode, resource_provider)) {
        return false;
    }
    ...
    internal_content_bounds_.SetToMin(gfx::Size(max_texture_size, max_texture_size));

    return true;
}

void HeadsUpDisplayLayerImpl::DidDraw(viz::ClientResourceProvider* resource_provider)
{
...
void HeadsUpDisplayLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    viz::SharedQuadState* shared_quad_state = render_pass->CreateAndAppendSharedQuadState();
    PopulateScaledSharedQuadState(shared_quad_state, internal_contents_scale_, contents_opaque());
    ...
    quad->SetNew(shared_quad_state, quad_rect, quad_rect, SkColors::kTransparent, false);
    ValidateQuadResources(quad);
    placeholder_quad_ = quad;
}

void HeadsUpDisplayLayerImpl::UpdateHudTexture(DrawMode draw_mode, LayerTreeFrameSink* layer_tree_frame_sink, viz::ClientResourceProvider* resource_provider,
    const RasterCapabilities& raster_caps, const viz::CompositorRenderPassList& list)
{
    ...
    if (!pool_) {
        scoped_refptr<base::SingleThreadTaskRunner> task_runner = layer_tree_impl()->task_runner_provider()->HasImplThread()
            ? layer_tree_impl()->task_runner_provider()->ImplThreadTaskRunner()
            : layer_tree_impl()->task_runner_provider()->MainThreadTaskRunner();
        pool_ = std::make_unique<ResourcePool>(resource_provider, layer_tree_frame_sink->context_provider(), std::move(task_runner),
            ResourcePool::kDefaultExpirationDelay, layer_tree_impl()->settings().disallow_non_exact_resource_reuse);
    }
    ...
    if (draw_mode == DRAW_MODE_HARDWARE) {
        pool_resource = pool_->AcquireResource(internal_content_bounds_, raster_caps.tile_format, gfx::ColorSpace());
        ...
        if (!pool_resource.gpu_backing()) {
            ...
            gpu::SharedImageUsageSet flags = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_RASTER_WRITE;
            ...
            backing->shared_image = sii->CreateSharedImage(
                { pool_resource.format(), pool_resource.size(), pool_resource.color_space(), flags, "HeadsUpDisplayLayer" }, gpu::kNullSurfaceHandle);
            ...
            auto* ri = raster_context_provider->RasterInterface();
            ri->WaitSyncTokenCHROMIUM(pool_resource.gpu_backing()->returned_sync_token.GetConstData());
        ...
        auto sii = layer_tree_frame_sink->shared_image_interface();
        if (sii) {
            pool_resource = pool_->AcquireResource(internal_content_bounds_, viz::SinglePlaneFormat::kBGRA_8888, gfx::ColorSpace());
            ...
            if (!pool_resource.software_backing()) {
                ...
                auto shared_image_mapping = sii->CreateSharedImage(
                    { pool_resource.format(), pool_resource.size(), pool_resource.color_space(), gpu::SHARED_IMAGE_USAGE_CPU_WRITE, "HeadsUpDisplayLayer" });
            ...
            pool_resource = pool_->AcquireResource(internal_content_bounds_, viz::SinglePlaneFormat::kRGBA_8888, gfx::ColorSpace());
            if (!pool_resource.software_backing()) {
                auto backing = std::make_unique<HudSoftwareBacking>();
                backing->layer_tree_frame_sink = layer_tree_frame_sink;
                backing->shared_bitmap_id = viz::SharedBitmap::GenerateId();
                base::MappedReadOnlyRegion shm = viz::bitmap_allocation::AllocateSharedBitmap(pool_resource.size(), pool_resource.format());
                backing->shared_mapping = std::move(shm.mapping);

                layer_tree_frame_sink->DidAllocateSharedBitmap(std::move(shm.region), backing->shared_bitmap_id);
                pool_resource.set_software_backing(std::move(backing));
            }
        }
    }
    ...
    if (draw_mode == DRAW_MODE_HARDWARE) {
        ...
        if (raster_caps.use_gpu_rasterization) {
            ...
            ri->BeginRasterCHROMIUM(background_color, needs_clear, msaa_sample_count, gpu::raster::kNoMSAA, can_use_lcd_text,
                /*visible=*/true, gfx::ColorSpace::CreateSRGB(),
                /*hdr_headroom=*/1.f, backing->shared_image->mailbox().name);
            ...
            ri->RasterCHROMIUM(display_item_list.get(), &image_provider, size, gfx::Rect(size), gfx::Rect(size), post_translate, post_scale,
            ...
            if (!staging_surface_ || gfx::SkISizeToSize(staging_surface_->getCanvas()->getBaseLayerSize()) != pool_resource.size()) {
                SkSurfaceProps props = skia::LegacyDisplayGlobals::GetSkSurfaceProps();
                staging_surface_ = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(pool_resource.size().width(), pool_resource.size().height()), &props);
            }
        ...
        backing->mailbox_sync_token = viz::ClientResourceProvider::GenerateSyncTokenHelper(ri);
    } else {
        ...
        SkImageInfo info = SkImageInfo::MakeN32Premul(pool_resource.size().width(), pool_resource.size().height());
        SkSurfaceProps props = skia::LegacyDisplayGlobals::GetSkSurfaceProps();
        const size_t row_bytes = info.minRowBytes();
        auto* backing = static_cast<HudSoftwareBacking*>(pool_resource.software_backing());
        base::span<uint8_t> mem(backing->shared_mapping.data(), backing->shared_mapping.size());
        CHECK_GE(mem.size(), info.computeByteSize(row_bytes));
        sk_sp<SkSurface> surface = SkSurfaces::WrapPixels(info, mem.data(), row_bytes, &props);
    ...
    bool exported = pool_->PrepareForExport(pool_resource, viz::TransferableResource::ResourceSource::kHeadsUpDisplay);
    DCHECK(exported);
    ...
    for (auto it = render_pass->quad_list.begin(); it != render_pass->quad_list.end(); ++it) {
        if (*it == hud_quad) {
            ...
            auto* quad = render_pass->quad_list.ReplaceExistingElement<viz::TextureDrawQuad>(it);
            ...
            if (in_flight_resource_.size() != internal_content_bounds_) {
                uv_bottom_right.set_x(static_cast<double>(internal_content_bounds_.width()) / static_cast<double>(in_flight_resource_.size().width()));
                uv_bottom_right.set_y(static_cast<double>(internal_content_bounds_.height()) / static_cast<double>(in_flight_resource_.size().height()));
            }
            quad->SetNew(sqs, quad_rect, visible_rect, /*needs_blending=*/true, resource_id, /*premultiplied_alpha=*/true,
                /*uv_top_left=*/gfx::PointF(),
                /*uv_bottom_right=*/uv_bottom_right,
                /*background_color=*/SkColors::kTransparent,
                /*flipped=*/false,
                /*nearest_neighbor=*/false, /*secure_output_only=*/false, gfx::ProtectedVideoType::kClear);
            ValidateQuadResources(quad);
            break;
        }
    }
...
gfx::Rect HeadsUpDisplayLayerImpl::GetEnclosingVisibleRectInTargetSpace() const
{
...
const std::vector<gfx::Rect>& HeadsUpDisplayLayerImpl::LayoutShiftRects() const
{
    return layout_shift_rects_;
}

void HeadsUpDisplayLayerImpl::SetLayoutShiftRects(const std::vector<gfx::Rect>& rects)
{
...
void HeadsUpDisplayLayerImpl::UpdateHudContents()
{
    ...
    if (base::TimeDelta(now - time_of_last_graph_update_).InSecondsF() > 0.25f) {
        ...
        if (debug_state.show_fps_counter) {
            throughput_value_ = layer_tree_impl()->dropped_frame_counter()->GetAverageThroughput();
...
void HeadsUpDisplayLayerImpl::DrawHudContents(PaintCanvas* canvas)
{
    ...
    if (debug_state.show_fps_counter) {
        area = DrawFrameThroughputDisplay(canvas, layer_tree_impl()->dropped_frame_counter(), 0, 0);
        area = DrawGpuRasterizationStatus(canvas, 0, area.bottom(), std::max<SkScalar>(area.width(), 150));
    }

    if (debug_state.ShowMemoryStats() && memory_entry_.total_bytes_used) {
        area = DrawMemoryDisplay(canvas, 0, area.bottom(), std::max<SkScalar>(area.width(), 150));
    }
    ...
    if (bounds_width_in_dips() < metrics_sizes.kWidth) {
        double scale_to_bounds = static_cast<double>(bounds_width_in_dips()) / static_cast<double>(metrics_sizes.kWidth);
...
void HeadsUpDisplayLayerImpl::DrawDebuggerPaused(PaintCanvas* canvas)
{
    ...
    const SkScalar label_text_width = label_font.measureText(paused_localized_message_.c_str(), paused_localized_message_.length(), SkTextEncoding::kUTF8);
    ...
    canvas->drawRect(SkRect::MakeWH(label_text_width + 2 * kPadding, kFontHeight + 2 * kPadding), label_flags);

    label_flags.setColor(SkColorSetARGB(255, 50, 50, 50));
    DrawText(canvas, label_flags, paused_localized_message_, TextAlign::kLeft, kFontHeight, kPadding, kFontHeight * 0.8f + kPadding);
    canvas->restore();
}

void HeadsUpDisplayLayerImpl::DrawText(PaintCanvas* canvas, const PaintFlags& flags, const std::string& text, TextAlign align, int size, int x, int y) const
{
    ...
    if (align == TextAlign::kCenter) {
        auto width = font.measureText(text.c_str(), text.length(), SkTextEncoding::kUTF8);
        x -= width * 0.5f;
    ...
    canvas->drawTextBlob(SkTextBlob::MakeFromText(text.c_str(), text.length(), font), x, y, flags);
}

void HeadsUpDisplayLayerImpl::DrawText(
    PaintCanvas* canvas, const PaintFlags& flags, const std::string& text, TextAlign align, int size, const SkPoint& pos) const
{
    DrawText(canvas, flags, text, align, size, pos.x(), pos.y());
}

void HeadsUpDisplayLayerImpl::DrawGraphBackground(PaintCanvas* canvas, PaintFlags* flags, const SkRect& bounds) const
{
    flags->setColor(DebugColors::HUDBackgroundColor());
    canvas->drawRect(bounds, *flags);
}

void HeadsUpDisplayLayerImpl::DrawGraphLines(PaintCanvas* canvas, PaintFlags* flags, const SkRect& bounds) const
{
    // Draw top and bottom line.
    flags->setColor(DebugColors::HUDSeparatorLineColor());
    canvas->drawLine(bounds.left(), bounds.top() - 1, bounds.right(), bounds.top() - 1, *flags);
    canvas->drawLine(bounds.left(), bounds.bottom(), bounds.right(), bounds.bottom(), *flags);
}

void HeadsUpDisplayLayerImpl::DrawSeparatorLine(PaintCanvas* canvas, PaintFlags* flags, const SkRect& bounds) const
{
    // Draw separator line as transparent white.
    flags->setColor({ 1.0f, 1.0f, 1.0f, 0.25f });
    canvas->drawLine(bounds.left(), bounds.top(), bounds.right(), bounds.top(), *flags);
}

SkRect HeadsUpDisplayLayerImpl::DrawFrameThroughputDisplay(PaintCanvas* canvas, const DroppedFrameCounter* dropped_frame_counter, int right, int top) const
{
    const int kPadding = 4;
    const int kGap = 6;
    ...
    const int kGraphWidth = base::saturated_cast<int>(dropped_frame_counter->frame_history_size());
    ...
    SkRect title_bounds = SkRect::MakeXYWH(left + kPadding, top + kPadding, kGraphWidth + kGap + 2, kTitleFontHeight);
    SkRect text_bounds = SkRect::MakeXYWH(left + kPadding, title_bounds.bottom() + 2 * kPadding, kGraphWidth + kGap + 2, kFontHeight);
    SkRect graph_bounds = SkRect::MakeXYWH(left + kPadding, text_bounds.bottom() + 2 * kPadding, kGraphWidth, kGraphHeight);
    ...
    if (frame_interval_.has_value()) {
        ...
        double frame_rate = static_cast<double>(throughput_value_) / (100 * frame_interval_.value().InSecondsF());
        value_text = base::StringPrintf("%5.1f fps", frame_rate);
    }
    VLOG(1) << value_text;

    flags.setColor(DebugColors::HUDTitleColor());
    DrawText(canvas, flags, title, TextAlign::kLeft, kTitleFontHeight, title_bounds.left(), title_bounds.bottom());

    flags.setColor(DebugColors::FPSDisplayTextAndGraphColor());
    DrawText(canvas, flags, value_text, TextAlign::kRight, kFontHeight, text_bounds.right(), text_bounds.bottom());
    ...
    for (auto it = dropped_frame_counter->End(); it; --it) {
        ...
        SkPath& path = state == DroppedFrameCounter::kFrameStateDropped ? dropped_path
            : state == DroppedFrameCounter::kFrameStateComplete         ? good_path
                                                                        : partial_path;
...
SkRect HeadsUpDisplayLayerImpl::DrawMemoryDisplay(PaintCanvas* canvas, int right, int top, int width) const
{
    const int kPadding = 4;
    const int kTitleFontHeight = 13;
    const int kFontHeight = 12;
    ...
    SkPoint stat1_pos = SkPoint::Make(left + width - kPadding - 1, top + kPadding + 2 * kFontHeight);
    SkPoint stat2_pos = SkPoint::Make(left + width - kPadding - 1, top + 2 * kPadding + 3 * kFontHeight);

    flags.setColor(DebugColors::HUDTitleColor());
    DrawText(canvas, flags, "GPU memory", TextAlign::kLeft, kTitleFontHeight, title_pos);

    flags.setColor(DebugColors::MemoryDisplayTextColor());
    std::string text = base::StringPrintf("%6.1f MB used", memory_entry_.total_bytes_used / kMegabyte);
    DrawText(canvas, flags, text, TextAlign::kRight, kFontHeight, stat1_pos);

    if (!memory_entry_.had_enough_memory)
        flags.setColor(SK_ColorRED);
    text = base::StringPrintf("%6.1f MB max ", memory_entry_.total_budget_in_bytes / kMegabyte);

    DrawText(canvas, flags, text, TextAlign::kRight, kFontHeight, stat2_pos);

    // Draw memory graph.
    int length = 2 * kFontHeight + kPadding + 12;
    SkRect oval = SkRect::MakeXYWH(left + kPadding * 6, top + kTitleFontHeight + kPadding * 3, length, length);
    flags.setAntiAlias(true);
    ...
    double angle = (static_cast<double>(memory_entry_.total_bytes_used) / memory_entry_.total_budget_in_bytes) * 180;

    SkColor4f colors[] = { SkColors::kRed, SkColors::kGreen, SkColors::kGreen, { 1.0f, 0.55f, 0.0f, 1.0f }, SkColors::kRed };
    const SkScalar pos[] = { SkFloatToScalar(0.2f), SkFloatToScalar(0.4f), SkFloatToScalar(0.6f), SkFloatToScalar(0.8f), SkFloatToScalar(1.0f) };
    flags.setShader(PaintShader::MakeSweepGradient(cx, cy, colors, pos, 5, SkTileMode::kClamp, 0, 360));
    flags.setAntiAlias(true);
...
SkRect HeadsUpDisplayLayerImpl::DrawGpuRasterizationStatus(PaintCanvas* canvas, int right, int top, int width) const
{
    std::string status;
    ...
    SkPoint gpu_status_pos = SkPoint::Make(left + width - kPadding, top + 2 * kFontHeight + 2 * kPadding);
    // TODO(crbug.com/40219248): Remove toSkColor and make all SkColor4f.
    flags.setColor(DebugColors::HUDTitleColor().toSkColor());
    DrawText(canvas, flags, "GPU raster", TextAlign::kLeft, kTitleFontHeight, left + kPadding, top + kFontHeight + kPadding);
    flags.setColor(color);
    DrawText(canvas, flags, status, TextAlign::kRight, kFontHeight, gpu_status_pos);

    return area;
}

void HeadsUpDisplayLayerImpl::DrawDebugRect(PaintCanvas* canvas, PaintFlags* flags, const DebugRect& rect, SkColor4f stroke_color, SkColor4f fill_color,
    float stroke_width, const std::string& label_text) const
{
    DCHECK(typeface_.get());
    gfx::Rect debug_layer_rect = gfx::ScaleToEnclosingRect(rect.rect, 1.0 / internal_contents_scale_, 1.0 / internal_contents_scale_);
    ...
    if (label_text.length()) {
        ...
        const SkScalar label_text_width = label_font.measureText(label_text.c_str(), label_text.length(), SkTextEncoding::kUTF8);
        canvas->drawRect(SkRect::MakeWH(label_text_width + 2 * kPadding, kFontHeight + 2 * kPadding), label_flags);

        label_flags.setColor(SkColorSetARGB(255, 50, 50, 50));
        DrawText(canvas, label_flags, label_text, TextAlign::kLeft, kFontHeight, kPadding, kFontHeight * 0.8f + kPadding);
        canvas->restore();
    }
}

void HeadsUpDisplayLayerImpl::DrawDebugRects(PaintCanvas* canvas, DebugRectHistory* debug_rect_history)
{
    PaintFlags flags;
    ...
    for (size_t i = 0; i < debug_rects.size(); ++i) {
        ...
        case DebugRectType::kMainThreadScrollRepaint:
            ...
            label_text.append(base::ToLowerASCII(MainThreadScrollingReason::AsText(debug_rects[i].main_thread_scroll_repaint_reasons)));
            break;
        ...
        DrawDebugRect(canvas, &flags, debug_rects[i], stroke_color, fill_color, stroke_width, label_text);
    }
    ...
    if (paint_rects_fade_step_ > 0) {
        ...
        for (auto& paint_rect : paint_rects_) {
            DrawDebugRect(canvas, &flags, paint_rect, DebugColors::PaintRectBorderColor(paint_rects_fade_step_),
                DebugColors::PaintRectFillColor(paint_rects_fade_step_), DebugColors::PaintRectBorderWidth(), "");
        }
    }
    ...
    if (layout_shift_rects_fade_step_ > 0) {
        ...
        for (auto& layout_shift_debug_rect : layout_shift_debug_rects_) {
            ...
            // make all SkColor4f.
            DrawDebugRect(canvas, &flags, layout_shift_debug_rect, DebugColors::LayoutShiftRectBorderColor(),
                DebugColors::LayoutShiftRectFillColor(layout_shift_rects_fade_step_), DebugColors::LayoutShiftRectBorderWidth(), "");
        }
    }
}

void HeadsUpDisplayLayerImpl::AsValueInto(base::trace_event::TracedValue* dict) const
{
