...
std::unique_ptr<PaintedScrollbarLayerImpl> PaintedScrollbarLayerImpl::Create(
    LayerTreeImpl* tree_impl, int id, ScrollbarOrientation orientation, bool is_left_side_vertical_scrollbar, bool is_overlay)
{
    return base::WrapUnique(new PaintedScrollbarLayerImpl(tree_impl, id, orientation, is_left_side_vertical_scrollbar, is_overlay));
}

PaintedScrollbarLayerImpl::PaintedScrollbarLayerImpl(
    LayerTreeImpl* tree_impl, int id, ScrollbarOrientation orientation, bool is_left_side_vertical_scrollbar, bool is_overlay)
    : ScrollbarLayerImplBase(tree_impl, id, orientation, is_left_side_vertical_scrollbar, is_overlay)
{
}
...
std::unique_ptr<LayerImpl> PaintedScrollbarLayerImpl::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
    return PaintedScrollbarLayerImpl::Create(tree_impl, id(), orientation(), is_left_side_vertical_scrollbar(), is_overlay_scrollbar());
}
...
void PaintedScrollbarLayerImpl::PushPropertiesTo(LayerImpl* layer)
{
    ...
    scrollbar_layer->set_internal_contents_scale_and_bounds(internal_contents_scale_, internal_content_bounds_);
    ...
    scrollbar_layer->set_track_and_buttons_ui_resource_id(track_and_buttons_ui_resource_id_);
    scrollbar_layer->set_thumb_ui_resource_id(thumb_ui_resource_id_);
    scrollbar_layer->set_uses_nine_patch_track_and_buttons(uses_nine_patch_track_and_buttons_);
    ...
    scrollbar_layer->SetTrackAndButtonsImageBounds(track_and_buttons_image_bounds_);
...
void PaintedScrollbarLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    AppendThumbQuads(render_pass, append_quads_data);
    AppendTrackAndButtonsQuads(render_pass, append_quads_data);
}

void PaintedScrollbarLayerImpl::AppendThumbQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data) const
{
    ...
    PopulateScaledSharedQuadState(shared_quad_state, internal_contents_scale_, kContentsOpaque);

    AppendDebugBorderQuad(render_pass, gfx::Rect(internal_content_bounds_), shared_quad_state, append_quads_data);

    const gfx::Rect thumb_quad_rect = ComputeThumbQuadRect();
    const gfx::Rect scaled_thumb_quad_rect = gfx::ScaleToEnclosingRect(thumb_quad_rect, internal_contents_scale_);
    const gfx::Rect visible_thumb_quad_rect = draw_properties().occlusion_in_content_space.GetUnoccludedContentRect(thumb_quad_rect);
    if (visible_thumb_quad_rect.IsEmpty()) {
        return;
    }
    const gfx::Rect scaled_visible_thumb_quad_rect = gfx::ScaleToEnclosingRect(visible_thumb_quad_rect, internal_contents_scale_);

    if (thumb_color_.has_value()) {
        gfx::MaskFilterInfo rounded_corners_mask = draw_properties().mask_filter_info;
        ...
        if (!is_web_test() && IsFluentScrollbarEnabled()) {
            const int rounded_corner_radius = orientation() == ScrollbarOrientation::kHorizontal ? thumb_quad_rect.height() : thumb_quad_rect.width();
            shared_quad_state->mask_filter_info = gfx::MaskFilterInfo(gfx::RRectF(gfx::RectF(thumb_quad_rect), rounded_corner_radius));
            shared_quad_state->mask_filter_info.ApplyTransform(draw_properties().target_space_transform);
        }
        auto* thumb_quad = render_pass->CreateAndAppendDrawQuad<viz::SolidColorDrawQuad>();
        thumb_quad->SetNew(shared_quad_state, scaled_thumb_quad_rect, scaled_visible_thumb_quad_rect, thumb_color_.value(),
    ...
    viz::ResourceId thumb_resource_id = layer_tree_impl()->ResourceIdForUIResource(thumb_ui_resource_id_);
    ...
    quad->SetNew(shared_quad_state, scaled_thumb_quad_rect, scaled_visible_thumb_quad_rect, /*needs_blending=*/true, thumb_resource_id, /*premultiplied=*/true,
...
void PaintedScrollbarLayerImpl::AppendTrackAndButtonsQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    if (IsFluentOverlayScrollbarEnabled() && thumb_thickness_scale_factor() <= GetIdleThicknessScale() && !has_find_in_page_tickmarks()) {
        return;
    }

    gfx::Rect track_and_buttons_quad_rect(bounds());
    gfx::Rect visible_track_and_buttons_quad_rect = draw_properties().occlusion_in_content_space.GetUnoccludedContentRect(track_and_buttons_quad_rect);
    viz::ResourceId track_and_buttons_resource_id = layer_tree_impl()->ResourceIdForUIResource(track_and_buttons_ui_resource_id_);

    if (!track_and_buttons_resource_id || track_and_buttons_quad_rect.IsEmpty()) {
        return;
    }

    viz::SharedQuadState* track_and_buttons_shared_quad_state = render_pass->CreateAndAppendSharedQuadState();
    PopulateScaledSharedQuadState(track_and_buttons_shared_quad_state, internal_contents_scale_, contents_opaque());
    if (IsFluentOverlayScrollbarEnabled()) {
        ...
        const float scaled_opacity = (thumb_thickness_scale_factor() - GetIdleThicknessScale()) / (1.f - GetIdleThicknessScale());
    ...
    if (uses_nine_patch_track_and_buttons_ && !has_find_in_page_tickmarks()) {
        AppendNinePatchScaledTrackAndButtons(render_pass, track_and_buttons_shared_quad_state, track_and_buttons_quad_rect);
        return;
    }

    gfx::Rect scaled_track_and_buttons_quad_rect(internal_content_bounds_);
    gfx::Rect scaled_visible_track_and_buttons_quad_rect = gfx::ScaleToEnclosingRect(visible_track_and_buttons_quad_rect, internal_contents_scale_);
    bool needs_blending = !contents_opaque();
    auto* quad = render_pass->CreateAndAppendDrawQuad<viz::TextureDrawQuad>();
    quad->SetNew(track_and_buttons_shared_quad_state, scaled_track_and_buttons_quad_rect, scaled_visible_track_and_buttons_quad_rect, needs_blending,
...
void PaintedScrollbarLayerImpl::AppendNinePatchScaledTrackAndButtons(
    viz::CompositorRenderPass* render_pass, viz::SharedQuadState* shared_quad_state, gfx::Rect& track_and_buttons_quad_rect)
{
    CHECK(uses_nine_patch_track_and_buttons_);
    gfx::Rect border(
        track_and_buttons_aperture_.x(), track_and_buttons_aperture_.y(), track_and_buttons_aperture_.x() * 2, track_and_buttons_aperture_.y() * 2);
    gfx::Rect layer_occlusion;
    bool layout_changed = track_and_buttons_patch_generator_.SetLayout(track_and_buttons_image_bounds_, track_and_buttons_quad_rect.size(),
    ...
    if (layout_changed) {
        ...
        track_and_buttons_patches_ = track_and_buttons_patch_generator_.GeneratePatches();
    ...
    track_and_buttons_patch_generator_.AppendQuadsForCc(this, track_and_buttons_ui_resource_id_, render_pass, shared_quad_state, track_and_buttons_patches_);
}

gfx::Rect PaintedScrollbarLayerImpl::GetEnclosingVisibleRectInTargetSpace() const
{
...
gfx::Rect PaintedScrollbarLayerImpl::ComputeHitTestableExpandedThumbQuadRect() const
{
    CHECK(is_overlay_scrollbar());
    gfx::Rect thumb_rect = ScrollbarLayerImplBase::ComputeHitTestableExpandedThumbQuadRect();
    if (thumb_color_.has_value()) {
...
gfx::Rect PaintedScrollbarLayerImpl::CenterSolidColorThumb(gfx::Rect thumb_rect) const
{
    ...
    const int thumb_offset = static_cast<int>((track_thickness - ThumbThickness()) / 2.0f);

    if (orientation() == ScrollbarOrientation::kHorizontal) {
        thumb_rect.Offset(0, thumb_offset);
    } else {
        thumb_rect.Offset(is_left_side_vertical_scrollbar() ? -thumb_offset : thumb_offset, 0);
    }
    return thumb_rect;
}

gfx::Rect PaintedScrollbarLayerImpl::ExpandSolidColorThumb(gfx::Rect thumb_rect) const
{
    CHECK(thumb_color_.has_value());
...
void PaintedScrollbarLayerImpl::SetSupportsDragSnapBack(bool supports_drag_snap_back)
{
...
void PaintedScrollbarLayerImpl::SetTrackAndButtonsImageBounds(const gfx::Size& bounds)
{
...
void PaintedScrollbarLayerImpl::SetTrackAndButtonsAperture(const gfx::Rect& aperture)
{
...
{
    return orientation() == ScrollbarOrientation::kVertical ? track_rect_.y() : track_rect_.x();
}
...
void PaintedScrollbarLayerImpl::SetForwardButtonRect(gfx::Rect forward_button_rect)
{
...
LayerTreeSettings::ScrollbarAnimator PaintedScrollbarLayerImpl::GetScrollbarAnimator() const
{
    return IsFluentOverlayScrollbarEnabled() ? LayerTreeSettings::AURA_OVERLAY : LayerTreeSettings::NO_ANIMATOR;
}
