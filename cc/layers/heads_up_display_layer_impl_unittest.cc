...
void CheckDrawLayer(HeadsUpDisplayLayerImpl* layer, LayerTreeFrameSink* frame_sink, viz::ClientResourceProvider* resource_provider,
    viz::RasterContextProvider* context_provider, DrawMode draw_mode)
{
    ...
    layer->UpdateHudTexture(draw_mode, frame_sink, resource_provider, raster_caps, pass_list);
    if (will_draw)
...
class HeadsUpDisplayLayerImplTest : public LayerTreeImplTestBase, public ::testing::Test {
public:
    HeadsUpDisplayLayerImplTest()
        : LayerTreeImplTestBase(FakeLayerTreeFrameSink::Create3dForGpuRasterization())
    {
    }
};
...
TEST_F(HeadsUpDisplayLayerImplTest, ResourcelessSoftwareDrawAfterResourceLoss)
{
    ...
    CheckDrawLayer(layer, layer_tree_frame_sink(), resource_provider(), layer_tree_frame_sink()->context_provider(), DRAW_MODE_HARDWARE);
    ...
    CheckDrawLayer(layer, layer_tree_frame_sink(), resource_provider(), layer_tree_frame_sink()->context_provider(), DRAW_MODE_RESOURCELESS_SOFTWARE);
}
...
TEST_F(HeadsUpDisplayLayerImplTest, CPUAndGPURasterCanvas)
{
    ...
    CheckDrawLayer(layer, layer_tree_frame_sink.get(), resource_provider(), nullptr, DRAW_MODE_SOFTWARE);
    host_impl()->ReleaseLayerTreeFrameSink();
}
