...
std::unique_ptr<LayerImpl> TextureLayerImpl::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
void TextureLayerImpl::PushPropertiesTo(LayerImpl* layer)
{
    ...
    if (own_resource_) {
        texture_layer->SetTransferableResource(transferable_resource_, std::move(release_callback_));
        own_resource_ = false;
    }
...
bool TextureLayerImpl::WillDraw(DrawMode draw_mode, viz::ClientResourceProvider* resource_provider)
{
    ...
    if (own_resource_) {
        ...
        if (!transferable_resource_.is_empty()) {
            ...
            if (base::FeatureList::IsEnabled(features::kEvictionUnlocksResources)
                && transferable_resource_.resource_source == viz::TransferableResource::ResourceSource::kCanvas) {
                evicted_cb = base::BindOnce(&TextureLayerImpl::OnResourceEvicted, base::Unretained(this));
                DCHECK(MayEvictResourceInBackground(transferable_resource_.resource_source));
            }
            resource_id_ = resource_provider->ImportResource(transferable_resource_,
...
void TextureLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    DCHECK(resource_id_);

    LayerTreeFrameSink* sink = layer_tree_impl()->layer_tree_frame_sink();
    for (const auto& pair : to_register_bitmaps_) {
        sink->DidAllocateSharedBitmap(pair.second->shared_region().Duplicate(), pair.first);
    }
    // All |to_register_bitmaps_| have been registered above, so we can move them
    // all to the |registered_bitmaps_|.
    registered_bitmaps_.insert(std::make_move_iterator(to_register_bitmaps_.begin()), std::make_move_iterator(to_register_bitmaps_.end()));
    to_register_bitmaps_.clear();

    SkColor4f bg_color = blend_background_color_ ? background_color() : SkColors::kTransparent;
    ...
    gfx::Rect visible_quad_rect = draw_properties().occlusion_in_content_space.GetUnoccludedContentRect(quad_rect);
    bool needs_blending = !are_contents_opaque;
    if (visible_quad_rect.IsEmpty())
        return;

    auto* quad = render_pass->CreateAndAppendDrawQuad<viz::TextureDrawQuad>();
    quad->SetNew(shared_quad_state, quad_rect, visible_quad_rect, needs_blending, resource_id_, premultiplied_alpha_, uv_top_left_, uv_bottom_right_, bg_color,
        flipped_, nearest_neighbor_,
...
void TextureLayerImpl::ReleaseResources()
{
    ...
    to_register_bitmaps_.insert(std::make_move_iterator(registered_bitmaps_.begin()), std::make_move_iterator(registered_bitmaps_.end()));
...
void TextureLayerImpl::RegisterSharedBitmapId(viz::SharedBitmapId id, scoped_refptr<CrossThreadSharedBitmap> bitmap)
{
...
void TextureLayerImpl::FreeTransferableResource()
{
    if (own_resource_) {
        ...
        if (release_callback_) {
            ...
            std::move(release_callback_).Run(transferable_resource_.sync_token(), false);
        }
...
void TextureLayerImpl::SetInInvisibleLayerTree()
{
    ...
    if (base::FeatureList::IsEnabled(features::kClearCanvasResourcesInBackground)
        && transferable_resource_.resource_source == viz::TransferableResource::ResourceSource::kCanvas && own_resource_) {
        DCHECK(MayEvictResourceInBackground(transferable_resource_.resource_source));
        if (!transferable_resource_.is_software) {
            FreeTransferableResource();
        }
...
// static
bool TextureLayerImpl::MayEvictResourceInBackground(viz::TransferableResource::ResourceSource source)
{
    return source == viz::TransferableResource::ResourceSource::kCanvas
        && (base::FeatureList::IsEnabled(features::kClearCanvasResourcesInBackground) || base::FeatureList::IsEnabled(features::kEvictionUnlocksResources));
}
