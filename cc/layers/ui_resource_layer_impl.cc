...
std::unique_ptr<LayerImpl> UIResourceLayerImpl::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
void UIResourceLayerImpl::SetUV(const gfx::PointF& top_left, const gfx::PointF& bottom_right)
{
...
bool UIResourceLayerImpl::WillDraw(DrawMode draw_mode, viz::ClientResourceProvider* resource_provider)
{
...
void UIResourceLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    DCHECK(!bounds().IsEmpty());

    viz::SharedQuadState* shared_quad_state = render_pass->CreateAndAppendSharedQuadState();

    viz::ResourceId resource = ui_resource_id_ ? layer_tree_impl()->ResourceIdForUIResource(ui_resource_id_) : viz::kInvalidResourceId;
    bool are_contents_opaque = resource ? (layer_tree_impl()->IsUIResourceOpaque(ui_resource_id_) || contents_opaque()) : false;
    ...
    quad->SetNew(shared_quad_state, quad_rect, visible_quad_rect, needs_blending, resource, premultiplied_alpha, uv_top_left_, uv_bottom_right_,
        SkColors::kTransparent, flipped, nearest_neighbor,
        /*secure_output_only=*/false, gfx::ProtectedVideoType::kClear);
    ValidateQuadResources(quad);
}

void UIResourceLayerImpl::AsValueInto(base::trace_event::TracedValue* state) const
{
