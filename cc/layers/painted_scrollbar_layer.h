...
public:
    ...
    static scoped_refptr<PaintedScrollbarLayer> CreateOrReuse(scoped_refptr<Scrollbar> scrollbar, PaintedScrollbarLayer* existing_layer);
    static scoped_refptr<PaintedScrollbarLayer> Create(scoped_refptr<Scrollbar> scrollbar);
...
private:
    ...
    template <typename T> bool UpdateProperty(T value, T* prop)
    {
        if (*prop == value)
            return false;
        *prop = value;
    ...
    ProtectedSequenceReadable<std::unique_ptr<ScopedUIResource>> track_and_buttons_resource_;
