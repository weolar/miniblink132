...
class SolidColorLayerImplTest : public LayerTreeImplTestBase, public ::testing::Test { };
...
TEST_F(SolidColorLayerImplTest, VerifyCorrectBackgroundColorInQuad)
{
    ...
    EXPECT_EQ(viz::SolidColorDrawQuad::MaterialCast(render_pass->quad_list.front())->color, test_color);
}
...
TEST_F(SolidColorLayerImplTest, VerifyCorrectOpacityInQuad)
{
    ...
    ASSERT_EQ(render_pass->quad_list.size(), 1U);
    EXPECT_EQ(opacity, viz::SolidColorDrawQuad::MaterialCast(render_pass->quad_list.front())->shared_quad_state->opacity);
...
TEST_F(SolidColorLayerImplTest, VerifyCorrectRenderSurfaceOpacityInQuad)
{
    ...
    EXPECT_EQ(1.f, viz::SolidColorDrawQuad::MaterialCast(render_pass->quad_list.front())->shared_quad_state->opacity);
...
TEST_F(SolidColorLayerImplTest, VerifyNeedsBlending)
{
    ...
    std::unique_ptr<FakeLayerTreeHost> host = FakeLayerTreeHost::Create(&client, &task_graph_runner, animation_host.get());
    ...
    std::unique_ptr<CommitState> commit_state = host->WillCommit(std::move(completion_event_ptr), /*has_updates=*/true);
    {
    ...
    completion_event_ptr = std::make_unique<CompletionEvent>(base::WaitableEvent::ResetPolicy::MANUAL);
    completion_event = completion_event_ptr.get();
    commit_state = host->WillCommit(std::move(completion_event_ptr), /*has_updates=*/true);
    {
        DebugScopedSetImplThread scoped_impl_thread(host->GetTaskRunnerProvider());
        host->host_impl()->FinishCommit(*commit_state, unsafe_state);
        completion_event->Signal();
        ...
        EXPECT_FALSE(render_pass->quad_list.front()->shared_quad_state->are_contents_opaque);
    }
    host->CommitComplete(commit_state->source_frame_number, { base::TimeTicks(), base::TimeTicks::Now() });
}
