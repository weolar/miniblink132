...
EffectTreeLayerListIterator::EffectTreeLayerListIterator(LayerTreeImpl* layer_tree_impl)
    ...
    , effect_tree_(&layer_tree_impl->property_trees()->effect_tree_mutable())
{
    ...
    while (layer_list_iterator_ != layer_tree_impl->rend() && !(*layer_list_iterator_)->contributes_to_drawn_render_surface()) {
    ...
    if (layer_list_iterator_ == layer_tree_impl->rend()) {
        ...
        state_ = State::kLayer;
        current_effect_tree_index_ = (*layer_list_iterator_)->render_target_effect_tree_index();
        next_effect_tree_index_ = current_effect_tree_index_;
        lowest_common_effect_tree_ancestor_index_ = current_effect_tree_index_;
    }
}

EffectTreeLayerListIterator::EffectTreeLayerListIterator(const EffectTreeLayerListIterator& iterator) = default;
...
void EffectTreeLayerListIterator::operator++()
{
    ...
    case State::kLayer:
        ...
        ++layer_list_iterator_;
        while (layer_list_iterator_ != layer_tree_impl_->rend() && !(*layer_list_iterator_)->contributes_to_drawn_render_surface()) {
            ++layer_list_iterator_;
        }
        ...
        next_effect_tree_index_ = (*layer_list_iterator_)->render_target_effect_tree_index();
        ...
        if (next_effect_tree_index_ != current_effect_tree_index_) {
            lowest_common_effect_tree_ancestor_index_
                = effect_tree_->LowestCommonAncestorWithRenderSurface(current_effect_tree_index_, next_effect_tree_index_);
            ...
            if (current_effect_tree_index_ == lowest_common_effect_tree_ancestor_index_) {
    ...
    case State::kContributingSurface:
        DCHECK(current_effect_tree_index_ != lowest_common_effect_tree_ancestor_index_);
        // Step towards the lowest common ancestor.
        current_effect_tree_index_ = effect_tree_->Node(current_effect_tree_index_)->target_id;
        if (current_effect_tree_index_ == next_effect_tree_index_) {
            state_ = State::kLayer;
        } else if (current_effect_tree_index_ == lowest_common_effect_tree_ancestor_index_) {
