...
void AddTiling(float scale, FakePictureLayerImpl* layer, std::vector<Tile*>* all_tiles)
{
    PictureLayerTiling* tiling = layer->AddTiling(gfx::AxisTransform2d(scale, gfx::Vector2dF()));
...
class PictureLayerImplPerfTest : public LayerTreeImplTestBase, public testing::Test {
public:
    ...
    void SetupPendingTree(const gfx::Size& layer_bounds)
    {
        ...
        std::unique_ptr<FakePictureLayerImpl> pending_layer = FakePictureLayerImpl::Create(pending_tree, 7, raster_source);
    ...
    void RunRasterQueueConstructAndIterateTest(const std::string& test_name, int num_tiles, const gfx::Rect& viewport_rect)
    {
        ...
        do {
            int count = num_tiles;
            std::unique_ptr<TilingSetRasterQueueAll> queue = TilingSetRasterQueueAll::Create(pending_layer_->picture_layer_tiling_set(), false, true);
            ASSERT_TRUE(queue);
            while (count--) {
        ...
        reporter.AddResult("_raster_queue_construct_and_iterate", timer_.LapsPerSecond());
    }
    ...
    void RunRasterQueueConstructTest(const std::string& test_name, const gfx::Rect& viewport)
    {
        ...
        host_impl()->pending_tree()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(
    ...
    void RunEvictionQueueConstructAndIterateTest(const std::string& test_name, int num_tiles, const gfx::Rect& viewport_rect)
    {
        host_impl()->active_tree()->SetDeviceViewportRect(viewport_rect);
        host_impl()->pending_tree()->UpdateDrawProperties(
            /*update_tiles=*/true, /*update_image_animation_controller=*/true);
        ...
        do {
            ...
            std::unique_ptr<TilingSetEvictionQueue> queue(
                new TilingSetEvictionQueue(pending_layer_->picture_layer_tiling_set(), pending_layer_->contributes_to_drawn_render_surface()));
            while (count--) {
                ASSERT_TRUE(!queue->IsEmpty()) << "count: " << count;
                ASSERT_TRUE(queue->Top().tile()) << "count: " << count;
                queue->Pop();
        ...
        perf_test::PerfResultReporter reporter = SetUpReporter(test_name);
        reporter.AddResult("_eviction_queue_construct_and_iterate", timer_.LapsPerSecond());
    }

    void RunEvictionQueueConstructTest(const std::string& test_name, const gfx::Rect& viewport)
    {
        host_impl()->active_tree()->SetDeviceViewportRect(viewport);
        host_impl()->pending_tree()->property_trees()->scroll_tree_mutable().UpdateScrollOffsetBaseForTesting(
            pending_layer_->element_id(), gfx::PointF(viewport.origin()));
...
protected:
    perf_test::PerfResultReporter SetUpReporter(const std::string& story_name)
    {
        ...
        reporter.RegisterImportantMetric("_raster_queue_construct_and_iterate", "runs/s");
        reporter.RegisterImportantMetric("_raster_queue_construct", "runs/s");
        reporter.RegisterImportantMetric("_eviction_queue_construct_and_iterate", "runs/s");
        return reporter;
    }
...
TEST_F(PictureLayerImplPerfTest, TilingSetRasterQueueConstructAndIterate)
{
    ...
    pending_layer_->AddTiling(gfx::AxisTransform2d(low_res_factor, gfx::Vector2dF()));
...
TEST_F(PictureLayerImplPerfTest, TilingSetEvictionQueueConstructAndIterate)
{
    ...
    RunEvictionQueueConstructAndIterateTest("32_100x100", 32, gfx::Rect(100, 100));
    RunEvictionQueueConstructAndIterateTest("32_500x500", 32, gfx::Rect(500, 500));
    RunEvictionQueueConstructAndIterateTest("64_100x100", 64, gfx::Rect(100, 100));
    RunEvictionQueueConstructAndIterateTest("64_500x500", 64, gfx::Rect(500, 500));
}
