...
TEST_F(LayerImplTest, VerifyPendingLayerChangesAreTrackedProperly)
{
    ...
    // they are used.
    EXECUTE_AND_VERIFY_SUBTREE_DID_NOT_CHANGE(root->UnionUpdateRect(arbitrary_rect));
    ...
    EXECUTE_AND_VERIFY_SUBTREE_CHANGED(host_impl()->pending_tree()->SetFilterMutated(root->element_id(), FilterOperations()));
    EXECUTE_AND_VERIFY_SUBTREE_CHANGED(host_impl()->pending_tree()->SetOpacityMutated(root->element_id(), arbitrary_number));
    EXECUTE_AND_VERIFY_SUBTREE_CHANGED(host_impl()->pending_tree()->SetTransformMutated(root->element_id(), arbitrary_transform));
...
TEST_F(LayerImplTest, VerifyNeedsUpdateDrawProperties)
{
    ...
    gfx::PointF arbitrary_scroll_offset(gfx::PointAtOffsetFromOrigin(arbitrary_vector2d));
    ...
    VERIFY_NEEDS_UPDATE_DRAW_PROPERTIES(host_impl()->active_tree()->SetFilterMutated(root->element_id(), FilterOperations()));
    VERIFY_NEEDS_UPDATE_DRAW_PROPERTIES(host_impl()->active_tree()->SetFilterMutated(root->element_id(), arbitrary_filters));
    ...
    VERIFY_NEEDS_UPDATE_DRAW_PROPERTIES(layer->layer_tree_impl()->DidUpdateScrollOffset(layer->element_id(), /*pushed_from_main_or_pending_tree=*/false));
    layer->layer_tree_impl()->property_trees()->scroll_tree_mutable().SetScrollOffsetDeltaForTesting(layer->element_id(), gfx::Vector2dF());
    VERIFY_NEEDS_UPDATE_DRAW_PROPERTIES(layer->SetCurrentScrollOffset(arbitrary_scroll_offset));
    VERIFY_NO_NEEDS_UPDATE_DRAW_PROPERTIES(layer->SetCurrentScrollOffset(arbitrary_scroll_offset));

    // Unrelated functions, always set to new values, always set needs update.
    host_impl()->active_tree()->set_needs_update_draw_properties();
    UpdateActiveTreeDrawProperties();
    CreateClipNode(layer);
    VERIFY_NEEDS_UPDATE_DRAW_PROPERTIES(layer->SetContentsOpaque(true); layer->NoteLayerPropertyChanged());
    VERIFY_NEEDS_UPDATE_DRAW_PROPERTIES(layer->SetBackgroundColor(arbitrary_color));
    VERIFY_NEEDS_UPDATE_DRAW_PROPERTIES(host_impl()->active_tree()->SetOpacityMutated(layer->element_id(), arbitrary_number));
    VERIFY_NEEDS_UPDATE_DRAW_PROPERTIES(host_impl()->active_tree()->SetTransformMutated(layer->element_id(), arbitrary_transform));
    VERIFY_NEEDS_UPDATE_DRAW_PROPERTIES(layer->SetBounds(arbitrary_size); layer->NoteLayerPropertyChanged());

    // Unrelated functions, set to the same values, no needs update.
    GetEffectNode(layer)->filters = arbitrary_filters;
    UpdateActiveTreeDrawProperties();
    VERIFY_NO_NEEDS_UPDATE_DRAW_PROPERTIES(host_impl()->active_tree()->SetFilterMutated(layer->element_id(), arbitrary_filters));
    ...
    VERIFY_NO_NEEDS_UPDATE_DRAW_PROPERTIES(layer->SetBackgroundColor(arbitrary_color));
...
TEST_F(LayerImplTest, PerspectiveTransformHasReasonableScale)
{
    ...
    {
        auto transform = gfx::Transform::RowMajor(-0.9397f, -0.7019f, 0.2796f, 2383.4521f, // row 1
...
TEST_F(LayerImplTest, GetDamageReasons)
{
    ...
    EXPECT_EQ(root->GetDamageReasons(), DamageReasonSet { DamageReason::kUntracked });
...
public:
    ...
    {
        return &layer_impl->layer_tree_impl()->property_trees()->scroll_tree_mutable();
    }
...
TEST_F(CommitToActiveTreeLayerImplScrollTest, ScrollByWithZeroOffset)
{
    ...
    EXPECT_POINTF_EQ(gfx::PointF(), scroll_tree(layer())->GetScrollOffsetBaseForTesting(layer()->element_id()));
    ...
    EXPECT_POINTF_EQ(gfx::PointAtOffsetFromOrigin(ScrollDelta(layer())), CurrentScrollOffset(layer()));
    EXPECT_POINTF_EQ(gfx::PointF(), scroll_tree(layer())->GetScrollOffsetBaseForTesting(layer()->element_id()));
...
TEST_F(CommitToActiveTreeLayerImplScrollTest, ScrollByWithNonZeroOffset)
{
    ...
    scroll_tree(layer())->UpdateScrollOffsetBaseForTesting(layer()->element_id(), scroll_offset);

    EXPECT_POINTF_EQ(scroll_offset, CurrentScrollOffset(layer()));
    EXPECT_POINTF_EQ(scroll_offset, scroll_tree(layer())->GetScrollOffsetBaseForTesting(layer()->element_id()));
    EXPECT_VECTOR2DF_EQ(gfx::Vector2dF(), ScrollDelta(layer()));

    layer()->ScrollBy(gfx::Vector2dF(-100, 100));
    EXPECT_POINTF_EQ(gfx::PointF(0, 80), CurrentScrollOffset(layer()));

    EXPECT_POINTF_EQ(scroll_offset + ScrollDelta(layer()), CurrentScrollOffset(layer()));
    EXPECT_POINTF_EQ(scroll_offset, scroll_tree(layer())->GetScrollOffsetBaseForTesting(layer()->element_id()));
...
TEST_F(CommitToActiveTreeLayerImplScrollTest, ApplySentScrollsNoListener)
{
    ...
    EXPECT_POINTF_EQ(scroll_offset + sent_scroll_delta, scroll_tree(layer())->GetScrollOffsetBaseForTesting(layer()->element_id()));
}
...
{
    host_impl()->CreatePendingTree();
    std::unique_ptr<LayerImpl> pending_layer = LayerImpl::Create(host_impl()->pending_tree(), 2);
    pending_layer->SetElementId(LayerIdToElementIdForTesting(pending_layer->id()));
...
TEST_F(LayerImplScrollTest, PushPropertiesToMirrorsCurrentScrollOffset)
{
    ...
    pending_layer->SetElementId(LayerIdToElementIdForTesting(pending_layer->id()));
    scroll_tree(pending_layer.get())->UpdateScrollOffsetBaseForTesting(pending_layer->element_id(), CurrentScrollOffset(layer()));

    pending_layer->PushPropertiesTo(layer());

    EXPECT_POINTF_EQ(gfx::PointF(22, 23), CurrentScrollOffset(layer()));
    EXPECT_POINTF_EQ(CurrentScrollOffset(layer()), CurrentScrollOffset(pending_layer.get()));
}
...
TEST_F(LayerImplTest, JitterTest)
{
    ...
    auto* scroll_layer = host_impl()->pending_tree()->InnerViewportScrollLayerForTesting();
    auto* content_layer = AddLayerInPendingTree<LayerImpl>();
    content_layer->SetBounds(gfx::Size(100, 100));
    content_layer->SetDrawsContent(true);
    CopyProperties(host_impl()->pending_tree()->OuterViewportScrollLayerForTesting(), content_layer);
    UpdatePendingTreeDrawProperties();
    ...
    for (int i = 0; i < LayerTreeImpl::kFixedPointHitsThreshold + 1; ++i) {
        ...
        SetScrollOffset(host_impl()->active_tree()->InnerViewportScrollLayerForTesting(), gfx::PointF(0, accumulated_scroll));
        UpdateActiveTreeDrawProperties();
        ...
        content_layer->SetOffsetToTransformParent(gfx::Vector2dF(0, accumulated_scroll));
        ...
        float expected_jitter = (i == pending_tree->kFixedPointHitsThreshold) ? 250 : 0;
