...
public:
    ...
    void set_is_web_test(bool is_web_test)
    {
        is_web_test_ = is_web_test;
    }
    ...
    void SetOverlayScrollbarLayerOpacityAnimated(float opacity, bool fade_out_animation);
    ...
    virtual ScrollbarPart IdentifyScrollbarPart(const gfx::PointF position_in_widget) const;
...
protected:
    ScrollbarLayerImplBase(LayerTreeImpl* tree_impl, int id, ScrollbarOrientation orientation, bool is_left_side_vertical_scrollbar, bool is_overlay);
...
private:
    ...
    gfx::Rect ComputeThumbQuadRectWithThumbThicknessScale(float thumb_thickness_scale_factor) const;
    ...
    FRIEND_TEST_ALL_PREFIXES(CommitToActiveTreeScrollbarLayerTest, ScrollElementIdPushedAcrossCommit);
};
