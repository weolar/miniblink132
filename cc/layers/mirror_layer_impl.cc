...
std::unique_ptr<LayerImpl> MirrorLayerImpl::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
    return MirrorLayerImpl::Create(tree_impl, id());
}

void MirrorLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    ...
    if (!mirrored_layer) {
        ...
        quad->SetNew(shared_quad_state, shared_quad_state->quad_layer_rect, shared_quad_state->visible_quad_layer_rect, SkColors::kBlack,
            /*anti_aliasing_off=*/false);

        return;
    }

    auto* mirrored_render_surface = GetEffectTree().GetRenderSurface(mirrored_layer->effect_tree_index());
    gfx::Rect content_rect = mirrored_render_surface->content_rect();

    gfx::Rect unoccluded_content_rect = draw_properties().occlusion_in_content_space.GetUnoccludedContentRect(content_rect);
    ...
    PopulateScaledSharedQuadStateWithContentRects(shared_quad_state, mirrored_layer->GetIdealContentsScaleKey(), content_rect, content_rect, contents_opaque);

    AppendDebugBorderQuad(render_pass, content_rect, shared_quad_state, append_quads_data);
    ...
    quad->SetNew(shared_quad_state, content_rect, unoccluded_content_rect, mirrored_layer_render_pass_id(), mask_resource_id, mask_uv_rect, mask_texture_size,
        mirrored_effect_node->surface_contents_scale, gfx::PointF(), gfx::RectF(gfx::Rect(content_rect.size())),
...
gfx::Rect MirrorLayerImpl::GetEnclosingVisibleRectInTargetSpace() const
{
    ...
    float scale = mirrored_layer ? mirrored_layer->GetIdealContentsScaleKey() : 1.0f;
