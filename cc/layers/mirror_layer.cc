...
std::unique_ptr<LayerImpl> MirrorLayer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
    return MirrorLayerImpl::Create(tree_impl, id());
}

void MirrorLayer::PushPropertiesTo(LayerImpl* layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
...
scoped_refptr<MirrorLayer> MirrorLayer::Create(scoped_refptr<Layer> mirrored_layer)
{
