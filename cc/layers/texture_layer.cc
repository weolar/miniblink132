...
scoped_refptr<TextureLayer> TextureLayer::CreateForMailbox(TextureLayerClient* client)
{
...
std::unique_ptr<LayerImpl> TextureLayer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
void TextureLayer::SetUV(const gfx::PointF& top_left, const gfx::PointF& bottom_right)
{
    if (uv_top_left_.Read(*this) == top_left && uv_bottom_right_.Read(*this) == bottom_right)
        return;
...
void TextureLayer::SetTransferableResourceInternal(const viz::TransferableResource& resource, viz::ReleaseCallback release_callback, bool requires_commit)
{
    DCHECK(resource.is_empty() || !resource_holder_.Read(*this) || resource != resource_holder_.Read(*this)->resource());
    DCHECK_EQ(resource.is_empty(), !release_callback);

    // If we never committed the resource, we need to release it here.
    if (!resource.is_empty()) {
        resource_holder_.Write(*this) = TransferableResourceHolder::Create(resource, std::move(release_callback));
    } else {
...
void TextureLayer::SetTransferableResource(const viz::TransferableResource& resource, viz::ReleaseCallback release_callback)
{
    bool requires_commit = true;
    SetTransferableResourceInternal(resource, std::move(release_callback), requires_commit);
}
...
void TextureLayer::SetLayerTreeHost(LayerTreeHost* host)
{
    ...
    if (host) {
        ...
        to_register_bitmaps_.Write(*this).insert(std::make_move_iterator(registered_bitmaps.begin()), std::make_move_iterator(registered_bitmaps.end()));
...
{
    return (client_.Read(*this) || resource_holder_.Read(*this)) && Layer::HasDrawableContent();
}
...
bool TextureLayer::Update()
{
    ...
    if (client_.Read(*this)) {
        ...
        if (client_.Write(*this)->PrepareTransferableResource(this, &resource, &release_callback)) {
...
void TextureLayer::PushPropertiesTo(LayerImpl* layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
    ...
    if (needs_set_resource_.Read(*this)) {
        ...
        if (auto& resource_holder = resource_holder_.Write(*this)) {
            ...
            release_callback = base::BindOnce(&TransferableResourceHolder::Return, resource_holder,
                base::RetainedRef(layer->layer_tree_impl()->task_runner_provider()->MainThreadTaskRunner()));
        }
        texture_layer->SetTransferableResource(resource, std::move(release_callback));
    ...
    registered_bitmaps_.Write(*this).insert(std::make_move_iterator(to_register_bitmaps.begin()), std::make_move_iterator(to_register_bitmaps.end()));
...
SharedBitmapIdRegistration TextureLayer::RegisterSharedBitmapId(const viz::SharedBitmapId& id, scoped_refptr<CrossThreadSharedBitmap> bitmap)
{
...
TextureLayer::TransferableResourceHolder::TransferableResourceHolder(const viz::TransferableResource& resource, viz::ReleaseCallback release_callback)
    : resource_(resource)
...
TextureLayer::TransferableResourceHolder::~TransferableResourceHolder()
{
    if (release_callback_) {
        if (!release_callback_task_runner_ || release_callback_task_runner_->RunsTasksInCurrentSequence()) {
            std::move(release_callback_).Run(sync_token_, is_lost_);
        } else {
            DCHECK(release_callback_task_runner_);
            release_callback_task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(release_callback_), sync_token_, is_lost_));
        }
    }
}

scoped_refptr<TextureLayer::TransferableResourceHolder> TextureLayer::TransferableResourceHolder::Create(
    const viz::TransferableResource& resource, viz::ReleaseCallback release_callback)
{
...
void TextureLayer::TransferableResourceHolder::Return(
    scoped_refptr<base::SequencedTaskRunner> main_thread_task_runner, const gpu::SyncToken& sync_token, bool is_lost)
{
