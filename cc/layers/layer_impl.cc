...
template <typename T> std::unique_ptr<T> ClonePtr(const std::unique_ptr<T>& value)
{
...
LayerImpl::LayerImpl(LayerTreeImpl* tree_impl, int id, bool will_always_push_properties)
    : layer_id_(id)
...
LayerImpl::~LayerImpl()
{
    ...
    TRACE_EVENT_OBJECT_DELETED_WITH_ID(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "cc::LayerImpl", this);
}
...
void LayerImpl::UpdateDebugInfo(LayerDebugInfo* debug_info)
{
    ...
    *debug_info_ = *debug_info;
    debug_info_->invalidations.insert(debug_info_->invalidations.begin(), existing_invalidations.begin(), existing_invalidations.end());
}
...
int LayerImpl::render_target_effect_tree_index() const
{
    ...
    return GetEffectTree().GetRenderSurface(effect_tree_index_) ? effect_node->id : effect_node->target_id;
}
...
void LayerImpl::PopulateSharedQuadState(viz::SharedQuadState* state, bool contents_opaque) const
{
    EffectNode* effect_node = GetEffectTree().Node(effect_tree_index_);
    ...
    state->SetAll(draw_properties_.target_space_transform, gfx::Rect(bounds()), draw_properties_.visible_layer_rect, draw_properties_.mask_filter_info,
        clip_rect, contents_opaque, draw_properties_.opacity, effect_node->HasRenderSurface() ? SkBlendMode::kSrcOver : effect_node->blend_mode,
        GetSortingContextId(), static_cast<uint32_t>(id()), draw_properties_.is_fast_rounded_corner);
}

void LayerImpl::PopulateScaledSharedQuadState(viz::SharedQuadState* state, float layer_to_content_scale, bool contents_opaque) const
{
    gfx::Size scaled_bounds = gfx::ScaleToCeiledSize(bounds(), layer_to_content_scale);
    gfx::Rect scaled_visible_layer_rect = gfx::ScaleToEnclosingRect(visible_layer_rect(), layer_to_content_scale);
    scaled_visible_layer_rect.Intersect(gfx::Rect(scaled_bounds));

    PopulateScaledSharedQuadStateWithContentRects(state, layer_to_content_scale, gfx::Rect(scaled_bounds), scaled_visible_layer_rect, contents_opaque);
}

void LayerImpl::PopulateScaledSharedQuadStateWithContentRects(
    viz::SharedQuadState* state, float layer_to_content_scale, const gfx::Rect& content_rect, const gfx::Rect& visible_content_rect, bool contents_opaque) const
{
    gfx::Transform scaled_draw_transform = GetScaledDrawTransform(layer_to_content_scale);
    ...
    state->SetAll(scaled_draw_transform, content_rect, visible_content_rect, draw_properties().mask_filter_info, clip_rect, contents_opaque,
        draw_properties().opacity, effect_node->HasRenderSurface() ? SkBlendMode::kSrcOver : effect_node->blend_mode, GetSortingContextId(),
        static_cast<uint32_t>(id()), draw_properties().is_fast_rounded_corner);
}

bool LayerImpl::WillDraw(DrawMode draw_mode, viz::ClientResourceProvider* resource_provider)
{
    if (visible_layer_rect().IsEmpty() || draw_properties().occlusion_in_content_space.IsOccluded(visible_layer_rect())) {
        return false;
    }
...
void LayerImpl::AppendDebugBorderQuad(
    viz::CompositorRenderPass* render_pass, const gfx::Rect& quad_rect, const viz::SharedQuadState* shared_quad_state, AppendQuadsData* append_quads_data) const
{
    SkColor4f color;
    float width;
    GetDebugBorderProperties(&color, &width);
    AppendDebugBorderQuad(render_pass, quad_rect, shared_quad_state, append_quads_data, color, width);
}

void LayerImpl::AppendDebugBorderQuad(viz::CompositorRenderPass* render_pass, const gfx::Rect& quad_rect, const viz::SharedQuadState* shared_quad_state,
    AppendQuadsData* append_quads_data, SkColor4f color, float width) const
{
    ...
    debug_border_quad->SetNew(shared_quad_state, quad_rect, visible_quad_rect, color, width);
    if (contents_opaque()) {
        ...
        auto* fill_quad = render_pass->CreateAndAppendDrawQuad<viz::DebugBorderDrawQuad>();
        fill_quad->SetNew(shared_quad_state, fill_rect, visible_fill_rect, fill_color, fill_width);
    }
}

void LayerImpl::GetContentsResourceId(viz::ResourceId* resource_id, gfx::Size* resource_size, gfx::SizeF* resource_uv_size) const
{
    NOTREACHED();
}
...
const Region& LayerImpl::GetAllTouchActionRegions() const
{
    if (!all_touch_action_regions_) {
        all_touch_action_regions_ = std::make_unique<Region>(touch_action_region_.GetAllRegions());
    } else {
...
std::unique_ptr<LayerImpl> LayerImpl::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
void LayerImpl::PushPropertiesTo(LayerImpl* layer)
{
    ...
    if (layer_property_changed_not_from_property_trees_ || layer_property_changed_from_property_trees_)
...
bool LayerImpl::IsAffectedByPageScale() const
{
    ...
    return transform_tree.Node(transform_tree_index())->in_subtree_of_page_scale_layer;
}

bool LayerImpl::LayerPropertyChanged() const
{
    return layer_property_changed_not_from_property_trees_ || LayerPropertyChangedFromPropertyTrees();
}

bool LayerImpl::LayerPropertyChangedFromPropertyTrees() const
{
    if (layer_property_changed_from_property_trees_ || GetPropertyTrees()->full_tree_damaged())
        return true;
...
{
#if DCHECK_IS_ON()
    const viz::ClientResourceProvider* resource_provider = layer_tree_impl_->resource_provider();
    for (viz::ResourceId resource_id : quad->resources)
        resource_provider->ValidateResource(resource_id);
#endif
}

gfx::Transform LayerImpl::GetScaledDrawTransform(float layer_to_content_scale) const
{
    gfx::Transform scaled_draw_transform = draw_properties_.target_space_transform;
    scaled_draw_transform.Scale(SK_Scalar1 / layer_to_content_scale, SK_Scalar1 / layer_to_content_scale);
...
gfx::Size LayerImpl::bounds() const
{
    ...
    return gfx::Size(bounds_.width() + viewport_bounds_delta.x(), bounds_.height() + viewport_bounds_delta.y());
}
...
bool LayerImpl::IsScrollerOrScrollbar() const
{
    ...
    return IsScrollbarLayer() || GetScrollTree().FindNodeFromElementId(element_id());
}
...
bool LayerImpl::OpaqueToHitTest() const
{
    return HitTestable() && hit_test_opaqueness_ == HitTestOpaqueness::kOpaque
        && !GetEffectTree().Node(effect_tree_index())->node_or_ancestor_has_fast_rounded_corner;
}
...
void LayerImpl::SetElementId(ElementId element_id)
{
    ...
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "LayerImpl::SetElementId", "element", element_id.ToString());
...
DamageReasonSet LayerImpl::GetDamageReasons() const
{
    ...
    if (LayerPropertyChanged() || !update_rect_.IsEmpty() || !GetDamageRect().IsEmpty()) {
...
void LayerImpl::SetCurrentScrollOffset(const gfx::PointF& scroll_offset)
{
    ...
    if (GetScrollTree().SetScrollOffset(element_id(), scroll_offset)) {
        layer_tree_impl()->DidUpdateScrollOffset(element_id(), /*pushed_from_main_or_pending_tree=*/false);
    }
}
...
void LayerImpl::SetNeedsPushProperties()
{
    ...
    if (layer_tree_impl()->IsActiveTree() && !layer_tree_impl()->settings().UseLayerContextForDisplay()) {
        return;
    }
...
void LayerImpl::GetAllPrioritizedTilesForTracing(std::vector<PrioritizedTile>* prioritized_tiles) const
{
}
...
void LayerImpl::AsValueInto(base::trace_event::TracedValue* state) const
{
    ...
    viz::TracedValue::MakeDictIntoImplicitSnapshotWithCategory(
        TRACE_DISABLED_BY_DEFAULT("cc.debug"), state, "cc::LayerImpl", LayerTypeAsString(GetLayerType()), this);
    ...
    state->SetInteger("gpu_memory_usage", base::saturated_cast<int>(GPUMemoryUsageInBytes()));

    if (element_id_)
        element_id_.AddToTracedValue(state);

    if (!ScreenSpaceTransform().IsIdentity())
        MathUtil::AddToTracedValue("screen_space_transform", ScreenSpaceTransform(), state);

    bool clipped;
    gfx::QuadF layer_quad = MathUtil::MapQuad(ScreenSpaceTransform(), gfx::QuadF(gfx::RectF(gfx::Rect(bounds()))), &clipped);
...
gfx::Transform LayerImpl::DrawTransform() const
{
    ...
    if (!contributes_to_drawn_render_surface()) {
        return draw_property_utils::DrawTransform(this, GetTransformTree(), GetEffectTree());
    }
...
gfx::Rect LayerImpl::GetScaledEnclosingVisibleRectInTargetSpace(float scale) const
{
    ...
    bool only_draws_visible_content = GetPropertyTrees()->effect_tree().Node(effect_tree_index())->only_draws_visible_content;
    ...
    return MathUtil::MapEnclosingClippedRect(scaled_draw_transform, scaled_bounds);
}
...
gfx::Vector2dF LayerImpl::GetIdealContentsScale() const
{
    ...
    std::optional<gfx::Vector2dF> transform_scales = gfx::TryComputeTransform2dScaleComponents(transform);
    if (transform_scales) {
    ...
    float page_scale = IsAffectedByPageScale() ? layer_tree_impl()->current_page_scale_factor() : 1.f;
    ...
    float scale_cap
        = std::min((layer_tree_impl()->settings().default_tile_size.width() - 2) * kMaxTilesToCoverLayerDimension / static_cast<float>(bounds().width()),
            (layer_tree_impl()->settings().default_tile_size.height() - 2) * kMaxTilesToCoverLayerDimension / static_cast<float>(bounds().height()));
...
float LayerImpl::GetPreferredRasterScale(gfx::Vector2dF raster_space_scale_factor)
{
    constexpr float kMaxScaleRatio = 5.f;
    float lower_scale = std::min(raster_space_scale_factor.x(), raster_space_scale_factor.y());
    float higher_scale = std::max(raster_space_scale_factor.x(), raster_space_scale_factor.y());
...
static float TranslationFromActiveTreeLayerScreenSpaceTransform(LayerImpl* pending_tree_layer)
{
    ...
    if (layer_tree_impl) {
        ...
        if (active_tree_layer) {
            ...
            if (active_tree_screen_space_transform.IsIdentity())
                return 0.f;
            if (active_tree_screen_space_transform.ApproximatelyEqual(pending_tree_layer->draw_properties().screen_space_transform))
                return 0.f;
            return (active_tree_layer->draw_properties().screen_space_transform.To2dTranslation()
                - pending_tree_layer->draw_properties().screen_space_transform.To2dTranslation())
                .Length();
        }
    }
    return 0.f;
...
int LayerImpl::CalculateJitter()
{
    ...
    performance_properties().last_commit_screen_space_transform = draw_properties().screen_space_transform;

    if (!visible_layer_rect().IsEmpty()) {
        if (draw_properties().screen_space_transform.ApproximatelyEqual(performance_properties().last_commit_screen_space_transform)) {
            float translation_from_last_commit = TranslationFromActiveTreeLayerScreenSpaceTransform(this);
            if (translation_from_last_commit > 0.f) {
                performance_properties().num_fixed_point_hits++;
                performance_properties().translation_from_last_frame = translation_from_last_commit;
                if (performance_properties().num_fixed_point_hits > LayerTreeImpl::kFixedPointHitsThreshold) {
                    ...
                    jitter += translation_from_last_commit * sqrt(visible_layer_rect().size().GetArea());
                }
            } else {
...
viz::ViewTransitionElementResourceId LayerImpl::ViewTransitionResourceId() const
{
