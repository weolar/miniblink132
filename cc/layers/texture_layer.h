...
public:
    class CC_EXPORT TransferableResourceHolder : public base::RefCountedThreadSafe<TransferableResourceHolder> {
    public:
        TransferableResourceHolder(const TransferableResourceHolder&) = delete;
        TransferableResourceHolder& operator=(const TransferableResourceHolder&) = delete;

        const viz::TransferableResource& resource() const
        {
            return resource_;
        }
        void Return(scoped_refptr<base::SequencedTaskRunner> main_thread_task_runner, const gpu::SyncToken& sync_token, bool is_lost);

        static scoped_refptr<TransferableResourceHolder> Create(const viz::TransferableResource& resource, viz::ReleaseCallback release_callback);
    ...
    private:
        ...
        explicit TransferableResourceHolder(const viz::TransferableResource& resource, viz::ReleaseCallback release_callback);
    ...
    static scoped_refptr<TextureLayer> CreateForMailbox(TextureLayerClient* client);
    ...
    void SetTransferableResource(const viz::TransferableResource& resource, viz::ReleaseCallback release_callback);
    ...
    void PushPropertiesTo(LayerImpl* layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state) override;
    ...
    SharedBitmapIdRegistration RegisterSharedBitmapId(const viz::SharedBitmapId& id, scoped_refptr<CrossThreadSharedBitmap> bitmap) override;
...
private:
    void SetTransferableResourceInternal(const viz::TransferableResource& resource, viz::ReleaseCallback release_callback, bool requires_commit);
    ...
    ProtectedSequenceForbidden<raw_ptr<TextureLayerClient, DanglingUntriaged>> client_;
    ...
    ProtectedSequenceWritable<scoped_refptr<TransferableResourceHolder>> resource_holder_;
    ProtectedSequenceWritable<bool> needs_set_resource_;

    typedef base::flat_map<viz::SharedBitmapId, scoped_refptr<CrossThreadSharedBitmap>> BitMapMap;
    ...
    // TextureLayerImpl.
    ProtectedSequenceWritable<std::vector<viz::SharedBitmapId>> to_unregister_bitmap_ids_;
