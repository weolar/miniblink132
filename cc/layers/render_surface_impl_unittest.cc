...
TEST(RenderSurfaceLayerImplTest, Occlusion)
{
    ...
    CreateEffectNode(owning_layer_impl).render_surface_reason = RenderSurfaceReason::kTest;
...
static std::unique_ptr<viz::CompositorRenderPass> DoAppendQuadsWithScaledMask(DrawMode draw_mode, float device_scale_factor)
{
    gfx::Size layer_size(1000, 1000);
    gfx::Rect viewport_rect(1000, 1000);
    float scale_factor = 2;
    scoped_refptr<FakeRasterSource> raster_source = FakeRasterSource::CreateFilledSolidColor(layer_size);
    ...
    auto* mask_layer = impl.AddLayerInActiveTree<FakeMaskLayerImpl>(raster_source);
    ...
    render_surface_impl->AppendQuads(draw_mode, render_pass.get(), &append_quads_data);
    return render_pass;
}

TEST(RenderSurfaceLayerImplTest, AppendQuadsWithScaledMask)
{
    std::unique_ptr<viz::CompositorRenderPass> render_pass = DoAppendQuadsWithScaledMask(DRAW_MODE_HARDWARE, 1.f);
    DCHECK(render_pass->quad_list.front());
    const viz::CompositorRenderPassDrawQuad* quad = viz::CompositorRenderPassDrawQuad::MaterialCast(render_pass->quad_list.front());
...
{
    std::unique_ptr<viz::CompositorRenderPass> render_pass = DoAppendQuadsWithScaledMask(DRAW_MODE_RESOURCELESS_SOFTWARE, 1.f);
    DCHECK(render_pass->quad_list.front());
    const viz::CompositorRenderPassDrawQuad* quad = viz::CompositorRenderPassDrawQuad::MaterialCast(render_pass->quad_list.front());
    EXPECT_EQ(viz::kInvalidResourceId, quad->mask_resource_id());
}

TEST(RenderSurfaceLayerImplTest, AppendQuadsWithSolidColorMaskAndDeviceScaleFactor)
{
    std::unique_ptr<viz::CompositorRenderPass> render_pass = DoAppendQuadsWithScaledMask(DRAW_MODE_HARDWARE, 2.f);
    DCHECK(render_pass->quad_list.front());
    const viz::CompositorRenderPassDrawQuad* quad = viz::CompositorRenderPassDrawQuad::MaterialCast(render_pass->quad_list.front());
    EXPECT_EQ(gfx::Transform(), quad->shared_quad_state->quad_to_target_transform);
    ...
    VerifyQuadsExactlyCoverRect(render_pass->quad_list, quad->shared_quad_state->visible_quad_layer_rect);
}
