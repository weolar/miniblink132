...
TEST_F(PaintedScrollbarLayerTest, NeedsPaint)
{
    ...
    scoped_refptr<PaintedScrollbarLayer> scrollbar_layer = PaintedScrollbarLayer::Create(scrollbar);
...
TEST_F(PaintedScrollbarLayerTest, InternalContentBounds)
{
    ...
    layer_tree_host_->SetViewportRectAndScale(layer_tree_host_->device_viewport_rect(), 2.0f, layer_tree_host_->local_surface_id_from_parent());
    UpdateDrawProperties(layer_tree_host_.get());
    scrollbar_layer->Update();
    EXPECT_EQ(gfx::Size(20, 200), scrollbar_layer->internal_content_bounds());

    layer_tree_host_->SetViewportRectAndScale(layer_tree_host_->device_viewport_rect(), 0.1f, layer_tree_host_->local_surface_id_from_parent());
    UpdateDrawProperties(layer_tree_host_.get());
    scrollbar_layer->Update();
    EXPECT_EQ(gfx::Size(10, 100), scrollbar_layer->internal_content_bounds());
}
...
TEST_F(PaintedScrollbarLayerTest, SolidColorThumbDoesntGenerateThumbBitmap)
{
    ...
    EXPECT_CALL(*scrollbar, PaintThumb(_, _)).Times(0);
    EXPECT_CALL(*scrollbar, PaintTrackAndButtons(_, _)).Times(1);
    EXPECT_CALL(*scrollbar, ThumbColor()).Times(1).WillOnce(testing::Return(SkColor4f::FromColor(SK_ColorRED)));
