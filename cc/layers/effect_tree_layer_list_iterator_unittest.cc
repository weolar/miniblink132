...
#define EXPECT_COUNT(layer, target, contrib, itself)                                                                                                           \
    if (GetRenderSurface(layer)) {                                                                                                                             \
        ...
        EXPECT_EQ(contrib, contributing_surface_count_[layer->effect_tree_index()]);                                                                           \
    }                                                                                                                                                          \
    EXPECT_EQ(itself, layer->count_);

class EffectTreeLayerListIteratorTest : public LayerTreeImplTestBase, public testing::Test {
public:
    void SetUp() override
    {
    ...
    void IterateFrontToBack()
    {
        ResetCounts();
        int count = 0;
        for (EffectTreeLayerListIterator it(host_impl()->active_tree()); it.state() != EffectTreeLayerListIterator::State::kEnd; ++it, ++count) {
            switch (it.state()) {
            ...
            case EffectTreeLayerListIterator::State::kTargetSurface:
                target_surface_count_[it.target_render_surface()->EffectTreeIndex()] = count;
                break;
            case EffectTreeLayerListIterator::State::kContributingSurface:
                contributing_surface_count_[it.current_render_surface()->EffectTreeIndex()] = count;
                break;
            default:
                NOTREACHED();
            }
    ...
    void ResetCounts()
    {
        ...
        target_surface_count_ = std::vector<int>(host_impl()->active_tree()->property_trees()->effect_tree().size(), -1);
        contributing_surface_count_ = std::vector<int>(host_impl()->active_tree()->property_trees()->effect_tree().size(), -1);
    }
