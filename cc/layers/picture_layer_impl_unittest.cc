...
public:
    ...
    void SetupDefaultTreesWithFixedTileSize(const gfx::Size& layer_bounds, const gfx::Size& tile_size, const Region& invalidation)
    {
        ...
        SetupTreesWithFixedTileSize(std::move(pending_raster_source), std::move(active_raster_source), tile_size, invalidation);
    }

    void SetupTreesWithFixedTileSize(scoped_refptr<RasterSource> pending_raster_source, scoped_refptr<RasterSource> active_raster_source,
        const gfx::Size& tile_size, const Region& pending_invalidation)
    {
        SetupPendingTree(std::move(active_raster_source), tile_size, Region());
        ActivateTree();
        SetupPendingTree(std::move(pending_raster_source), tile_size, pending_invalidation);
    }

    void SetupDefaultTreesWithInvalidation(const gfx::Size& layer_bounds, const Region& invalidation)
    {
        scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFilled(layer_bounds);
        scoped_refptr<FakeRasterSource> active_raster_source = FakeRasterSource::CreateFilled(layer_bounds);

        SetupTreesWithInvalidation(std::move(pending_raster_source), std::move(active_raster_source), invalidation);
    }

    void SetupTreesWithInvalidation(
        scoped_refptr<RasterSource> pending_raster_source, scoped_refptr<RasterSource> active_raster_source, const Region& pending_invalidation)
    {
        SetupPendingTree(std::move(active_raster_source), gfx::Size(), Region());
        ActivateTree();
        SetupPendingTree(std::move(pending_raster_source), gfx::Size(), pending_invalidation);
    }

    void SetupPendingTreeWithInvalidation(scoped_refptr<RasterSource> raster_source, const Region& invalidation)
    {
        SetupPendingTree(std::move(raster_source), gfx::Size(), invalidation);
    }

    void SetupPendingTreeWithFixedTileSize(scoped_refptr<RasterSource> raster_source, const gfx::Size& tile_size, const Region& invalidation)
    {
        SetupPendingTree(std::move(raster_source), tile_size, invalidation);
    }

    void SetupDrawProperties(FakePictureLayerImpl* layer, const gfx::Vector2dF& ideal_contents_scale, float device_scale_factor, float page_scale_factor)
    {
    ...
    void SetupDrawProperties(FakePictureLayerImpl* layer, float ideal_contents_scale, float device_scale_factor, float page_scale_factor)
    {
        SetupDrawProperties(layer, gfx::Vector2dF(ideal_contents_scale, ideal_contents_scale), device_scale_factor, page_scale_factor);
    }

    void SetupDrawPropertiesAndUpdateTiles(
        FakePictureLayerImpl* layer, const gfx::Vector2dF& ideal_contents_scale, float device_scale_factor, float page_scale_factor)
    {
        SetupDrawProperties(layer, ideal_contents_scale, device_scale_factor, page_scale_factor);
        layer->UpdateTiles();
    }

    void SetupDrawPropertiesAndUpdateTiles(FakePictureLayerImpl* layer, float ideal_contents_scale, float device_scale_factor, float page_scale_factor)
    {
        SetupDrawPropertiesAndUpdateTiles(layer, gfx::Vector2dF(ideal_contents_scale, ideal_contents_scale), device_scale_factor, page_scale_factor);
    }

    static void VerifyAllPrioritizedTilesExistAndHaveRasterSource(const PictureLayerTiling* tiling, RasterSource* raster_source)
    {
        auto prioritized_tiles = tiling->UpdateAndGetAllPrioritizedTilesForTesting();
        for (PictureLayerTiling::CoverageIterator iter(tiling, tiling->contents_scale_key(), tiling->tiling_rect()); iter; ++iter) {
            EXPECT_TRUE(*iter);
            EXPECT_EQ(raster_source, prioritized_tiles[*iter].raster_source());
        }
    }

    void SetMaximumAnimationToScreenScale(FakePictureLayerImpl* layer, float maximum_animation_to_screen_scale, bool affected_by_invalid_scale)
    {
        layer->layer_tree_impl()->property_trees()->SetMaximumAnimationToScreenScaleForTesting(
            layer->transform_tree_index(), maximum_animation_to_screen_scale, affected_by_invalid_scale);
        layer->draw_properties().screen_space_transform_is_animating = true;
    }

    void SetContentsScaleOnPendingLayer(float contents_scale, float device_scale_factor, float page_scale_factor)
    {
        ...
        pending_layer()->draw_properties().screen_space_transform_is_animating = false;
        SetupDrawPropertiesAndUpdateTiles(pending_layer(), contents_scale, device_scale_factor, page_scale_factor);
    }

    void SetContentsScaleOnActiveLayer(float contents_scale, float device_scale_factor, float page_scale_factor)
    {
        // Sets arbitrary animation scale to ensure it's not used in any way.
        constexpr float kArbitraryScale = 34567.0f;
        SetMaximumAnimationToScreenScale(active_layer(), kArbitraryScale, false);
        active_layer()->draw_properties().screen_space_transform_is_animating = false;
        SetupDrawPropertiesAndUpdateTiles(active_layer(), contents_scale, device_scale_factor, page_scale_factor);
    }

    void SetContentsScaleOnBothLayers(float contents_scale, float device_scale_factor, float page_scale_factor)
    {
        SetContentsScaleOnPendingLayer(contents_scale, device_scale_factor, page_scale_factor);
        SetContentsScaleOnActiveLayer(contents_scale, device_scale_factor, page_scale_factor);
    }
    ...
    {
        SetMaximumAnimationToScreenScale(pending_layer(), maximum_animation_scale, affected_by_invalid_scale);
        SetupDrawPropertiesAndUpdateTiles(pending_layer(), contents_scale, device_scale_factor, page_scale_factor);
        SetMaximumAnimationToScreenScale(active_layer(), maximum_animation_scale, affected_by_invalid_scale);
        SetupDrawPropertiesAndUpdateTiles(active_layer(), contents_scale, device_scale_factor, page_scale_factor);
    }
...
TEST_F(LegacySWPictureLayerImplTest, CloneNoInvalidation)
{
    ...
    EXPECT_EQ(pending_layer()->tilings()->num_tilings(), active_layer()->tilings()->num_tilings());
...
TEST_F(LegacySWPictureLayerImplTest, ExternalViewportRectForPrioritizingTiles)
{
    ...
    host_impl()->SetExternalTilePriorityConstraints(viewport_rect_for_tile_priority, transform_for_tile_priority);
    UpdateDrawProperties(host_impl()->active_tree());

    // Verify the viewport rect for tile priority is used in picture layer tiling.
    EXPECT_EQ(viewport_rect_for_tile_priority, active_layer()->viewport_rect_for_tile_priority_in_content_space());
    PictureLayerTilingSet* tilings = active_layer()->tilings();
    for (size_t i = 0; i < tilings->num_tilings(); i++) {
        PictureLayerTiling* tiling = tilings->tiling_at(i);
        EXPECT_EQ(tiling->GetCurrentVisibleRectForTesting(), gfx::ScaleToEnclosingRect(viewport_rect_for_tile_priority, tiling->contents_scale_key()));
    }
...
TEST_F(LegacySWPictureLayerImplTest, ViewportRectForTilePriorityIsCached)
{
    ...
    host_impl()->SetExternalTilePriorityConstraints(another_viewport_rect_for_tile_priority, transform_for_tile_priority);
    ...
    EXPECT_EQ(another_viewport_rect_for_tile_priority, active_layer()->viewport_rect_for_tile_priority_in_content_space());
}
...
TEST_F(LegacySWPictureLayerImplTest, ClonePartialInvalidation)
{
    ...
    SetupPendingTreeWithFixedTileSize(lost_raster_source, gfx::Size(50, 50), Region());
    ActivateTree();
    {
        // Add a unique tiling on the active tree.
        PictureLayerTiling* tiling = active_layer()->AddTiling(gfx::AxisTransform2d(3.f, gfx::Vector2dF()));
    ...
    SetupTreesWithFixedTileSize(pending_raster_source, active_raster_source, gfx::Size(50, 50), layer_invalidation);
    ...
    for (size_t i = 0; i < tilings->num_tilings(); ++i) {
        ...
        gfx::Rect content_invalidation = gfx::ScaleToEnclosingRect(layer_invalidation, tiling->contents_scale_key());
        auto prioritized_tiles = tiling->UpdateAndGetAllPrioritizedTilesForTesting();
        for (PictureLayerTiling::CoverageIterator iter(tiling, tiling->contents_scale_key(), tiling->tiling_rect()); iter; ++iter) {
            ...
            if (*iter) {
                ...
                EXPECT_EQ(pending_raster_source.get(), prioritized_tiles[*iter].raster_source());
    ...
    for (size_t i = 0; i < tilings->num_tilings(); ++i) {
        ...
        for (PictureLayerTiling::CoverageIterator iter(tiling, tiling->contents_scale_key(), tiling->tiling_rect()); iter; ++iter) {
            ...
            EXPECT_EQ(active_raster_source.get(), prioritized_tiles[*iter].raster_source());
        }
    }
}
...
TEST_F(LegacySWPictureLayerImplTest, CloneFullInvalidation)
{
    ...
    SetupTreesWithInvalidation(pending_raster_source, active_raster_source, gfx::Rect(layer_bounds));
    ...
    EXPECT_GT(tilings->num_tilings(), 0u);
    for (size_t i = 0; i < tilings->num_tilings(); ++i) {
        VerifyAllPrioritizedTilesExistAndHaveRasterSource(tilings->tiling_at(i), pending_raster_source.get());
    }
}
...
TEST_F(LegacySWPictureLayerImplTest, UpdateTilesCreatesTilings)
{
    ...
    EXPECT_FLOAT_EQ(6.f, active_layer()->tilings()->tiling_at(0)->contents_scale_key());
    ...
    EXPECT_FLOAT_EQ(6.6f, active_layer()->tilings()->tiling_at(0)->contents_scale_key());
    EXPECT_FLOAT_EQ(6.6f * low_res_factor, active_layer()->tilings()->tiling_at(2)->contents_scale_key());
    ...
    EXPECT_FLOAT_EQ(7.26f, active_layer()->tilings()->tiling_at(0)->contents_scale_key());
    EXPECT_FLOAT_EQ(7.26f * low_res_factor, active_layer()->tilings()->tiling_at(3)->contents_scale_key());
...
TEST_F(LegacySWPictureLayerImplTest, PendingLayerOnlyHasHighResTiling)
{
    ...
    EXPECT_FLOAT_EQ(6.f, pending_layer()->tilings()->tiling_at(0)->contents_scale_key());
    ...
    SetupDrawPropertiesAndUpdateTiles(pending_layer(),
        ...
        2.2f); // page scale
    ASSERT_EQ(1u, pending_layer()->tilings()->num_tilings());
    EXPECT_FLOAT_EQ(6.6f, pending_layer()->tilings()->tiling_at(0)->contents_scale_key());
    ...
    SetupDrawPropertiesAndUpdateTiles(pending_layer(),
        ...
        2.2f); // page scale
    ASSERT_EQ(1u, pending_layer()->tilings()->num_tilings());
    EXPECT_FLOAT_EQ(7.26f, pending_layer()->tilings()->tiling_at(0)->contents_scale_key());
...
TEST_F(LegacySWPictureLayerImplTest, ZoomOutCrash)
{
    ...
    host_impl()->GetInputHandler().PinchGestureBegin(gfx::Point(), ui::ScrollInputType::kTouchscreen);
...
TEST_F(LegacySWPictureLayerImplTest, ScaledBoundsOverflowInt)
{
    ...
    EXPECT_GT(static_cast<float>(layer_bounds.width()) * scale, static_cast<float>(std::numeric_limits<int>::max()));
    ...
    active_layer()->PopulateScaledSharedQuadState(&state, adjusted_scale, active_layer()->contents_opaque());
}
...
TEST_F(LegacySWPictureLayerImplTest, PinchGestureTilings)
{
    ...
    EXPECT_EQ(active_layer()->tilings()->tiling_at(1)->contents_scale_key(), 2.f * low_res_factor);
    // One of the tilings has to be a low resolution one.
    EXPECT_EQ(LOW_RESOLUTION, active_layer()->tilings()->tiling_at(1)->resolution());
    ...
    EXPECT_FLOAT_EQ(2.0f, active_layer()->tilings()->tiling_at(0)->contents_scale_key());
    EXPECT_FLOAT_EQ(1.0f, active_layer()->tilings()->tiling_at(1)->contents_scale_key());
    EXPECT_FLOAT_EQ(2.0f * low_res_factor, active_layer()->tilings()->tiling_at(2)->contents_scale_key());
    // Since we're pinching, we shouldn't create a low resolution tiling.
    EXPECT_FALSE(active_layer()->tilings()->FindTilingWithResolution(LOW_RESOLUTION));
    ...
    SetContentsScaleOnBothLayers(low_res_factor * 2.1f, 1.0f, low_res_factor * 2.1f);
    ASSERT_EQ(3u, active_layer()->tilings()->num_tilings());
    EXPECT_FALSE(active_layer()->tilings()->FindTilingWithResolution(LOW_RESOLUTION));
    ...
    ASSERT_EQ(4u, active_layer()->tilings()->num_tilings());
    EXPECT_FLOAT_EQ(4.0f, active_layer()->tilings()->tiling_at(0)->contents_scale_key());
...
TEST_F(LegacySWPictureLayerImplTest, SnappedTilingDuringZoom)
{
    ...
    ASSERT_EQ(2u, active_layer()->tilings()->num_tilings());
    EXPECT_FLOAT_EQ(0.24f, active_layer()->tilings()->tiling_at(0)->contents_scale_key());
    EXPECT_FLOAT_EQ(0.06f, active_layer()->tilings()->tiling_at(1)->contents_scale_key());
    ...
    EXPECT_FLOAT_EQ(0.06f, active_layer()->tilings()->tiling_at(2)->contents_scale_key());
    ...
    ASSERT_EQ(4u, active_layer()->tilings()->num_tilings());
    EXPECT_FLOAT_EQ(1.92f, active_layer()->tilings()->tiling_at(0)->contents_scale_key());
}
...
TEST_F(LegacySWPictureLayerImplTest, CleanUpTilings)
{
    ...
    EXPECT_FLOAT_EQ(1.f, active_layer()->tilings()->tiling_at(0)->contents_scale_key());
    ...
    ASSERT_EQ(4u, active_layer()->tilings()->num_tilings());
    EXPECT_FLOAT_EQ(1.2f, active_layer()->tilings()->tiling_at(0)->contents_scale_key());
    EXPECT_FLOAT_EQ(1.f, active_layer()->tilings()->tiling_at(1)->contents_scale_key());
    EXPECT_FLOAT_EQ(1.2f * low_res_factor, active_layer()->tilings()->tiling_at(2)->contents_scale_key());
    EXPECT_FLOAT_EQ(1.f * low_res_factor, active_layer()->tilings()->tiling_at(3)->contents_scale_key());
...
TEST_F(LegacySWPictureLayerImplTest, DontAddLowResDuringAnimation)
{
    ...
    SetContentsAndAnimationScalesOnBothLayers(contents_scale, device_scale, page_scale, maximum_animation_scale, affected_by_invalid_scale);
    ...
    EXPECT_BOTH_EQ(HighResTiling()->contents_scale_key(), 1.f);
    EXPECT_EQ(active_layer()->LowResTiling()->contents_scale_key(), low_res_factor);
    ...
    SetContentsScaleOnBothLayers(contents_scale, device_scale, page_scale);
    EXPECT_BOTH_EQ(HighResTiling()->contents_scale_key(), 2.f);
    EXPECT_EQ(active_layer()->LowResTiling()->contents_scale_key(), 2.f * low_res_factor);
...
TEST_F(LegacySWPictureLayerImplTest, DontAddLowResForSmallLayers)
{
    ...
    SetContentsScaleOnBothLayers(contents_scale, device_scale, page_scale);
    EXPECT_BOTH_EQ(HighResTiling()->contents_scale_key(), contents_scale);
    EXPECT_EQ(active_layer()->LowResTiling()->contents_scale_key(), contents_scale * low_res_factor);
    EXPECT_FALSE(pending_layer()->LowResTiling());
    EXPECT_EQ(active_layer()->num_tilings(), 2u);
    EXPECT_EQ(pending_layer()->num_tilings(), 1u);
    ...
    auto* mask = AddLayer<FakePictureLayerImpl>(host_impl()->pending_tree(), pending_raster_source);
...
TEST_F(LegacySWPictureLayerImplTest, HugeBackdropFilterMasksGetScaledDown)
{
    ...
    CreateEffectNode(pending_layer()).backdrop_filters.Append(FilterOperation::CreateInvertFilter(1.0));
    auto* pending_mask = AddLayer<FakePictureLayerImpl>(host_impl()->pending_tree(), valid_raster_source);
    ...
    host_impl()->tile_manager()->InitializeTilesWithResourcesForTesting(pending_mask->HighResTiling()->AllTilesForTesting());

    ActivateTree();

    FakePictureLayerImpl* active_mask = static_cast<FakePictureLayerImpl*>(host_impl()->active_tree()->LayerById(pending_mask->id()));
    ...
    active_mask->GetContentsResourceId(&mask_resource_id, &mask_texture_size, &mask_uv_size);
    ...
    ActivateTree();
    EXPECT_EQ(1u, active_mask->HighResTiling()->AllTilesForTesting().size());
    active_layer()->GetContentsResourceId(&mask_resource_id, &mask_texture_size, &mask_uv_size);
    EXPECT_EQ(expected_size, mask_texture_size);
...
TEST_F(LegacySWPictureLayerImplTest, ClampTilesToMaxTileSize)
{
    ...
    // The default value.
    EXPECT_EQ(gfx::Size(256, 256).ToString(), host_impl()->settings().default_tile_size.ToString());

    Tile* tile = pending_layer()->tilings()->tiling_at(0)->AllTilesForTesting()[0];
    EXPECT_EQ(gfx::Size(256, 256).ToString(), tile->content_rect().size().ToString());
    ...
    {
        viz::RasterContextProvider::ScopedRasterContextLock scoped_context(worker_context_provider.get());
        worker_context_provider->GetTestRasterInterface()->set_max_texture_size(140);
    }
    ResetLayerTreeFrameSink(FakeLayerTreeFrameSink::Create3d(viz::TestContextProvider::CreateRaster(), std::move(worker_context_provider)));
...
TEST_F(LegacySWPictureLayerImplTest, ClampSingleTileToToMaxTileSize)
{
    ...
    EXPECT_EQ(gfx::Size(512, 512).ToString(), host_impl()->settings().max_untiled_layer_size.ToString());
...
TEST_F(LegacySWPictureLayerImplTest, DisallowTileDrawQuads)
{
    ...
    EXPECT_EQ(viz::DrawQuad::Material::kPictureContent, render_pass->quad_list.front()->material);
    EXPECT_EQ(render_pass->quad_list.front()->rect, layer_rect);
    EXPECT_FALSE(render_pass->quad_list.front()->needs_blending);
    EXPECT_TRUE(render_pass->quad_list.front()->shared_quad_state->are_contents_opaque);
...
TEST_F(LegacySWPictureLayerImplTest, ResourcelessRecordingNotVisible)
{
    ...
    gfx::Size layer_bounds(1000, 1000);
    scoped_refptr<FakeRasterSource> active_raster_source = FakeRasterSource::CreatePartiallyFilled(layer_bounds, gfx::Rect(0, 0, 300, 300));
    SetupPendingTree(active_raster_source);
    ActivateTree();

    active_layer()->SetContentsOpaque(true);
    active_layer()->draw_properties().visible_layer_rect = gfx::Rect(500, 0, 1000, 500);
...
TEST_F(LegacySWPictureLayerImplTest, FarScrolledQuadsShifted)
{
    ...
    active_layer()->SetContentsOpaque(true);
    active_layer()->draw_properties().visible_layer_rect = gfx::Rect(0, 5000, 1000, 1000);
    ...
    for (auto* draw_quad : render_pass->quad_list) {
        ...
        auto transform = [draw_quad](const gfx::Rect& rect) { return draw_quad->shared_quad_state->quad_to_target_transform.MapRect(gfx::RectF(rect)); };
        ...
        gfx::RectF transformed_quad_layer_rect = transform(draw_quad->shared_quad_state->quad_layer_rect);
        EXPECT_RECTF_EQ(transformed_quad_layer_rect, gfx::RectF(0.f, 0.f, 1000.f, 10000.f));

        gfx::RectF transformed_visible_quad_layer_rect = transform(draw_quad->shared_quad_state->visible_quad_layer_rect);
        EXPECT_RECTF_EQ(transformed_visible_quad_layer_rect, gfx::RectF(0.f, 5000.f, 1000.f, 1000.f));
    }
    EXPECT_EQ(min_y, 0);
...
TEST_F(LegacySWPictureLayerImplTest, FarScrolledSolidColorQuadsShifted)
{
    ...
    active_layer()->SetContentsOpaque(true);
    active_layer()->draw_properties().visible_layer_rect = gfx::Rect(0, 9000, 1000, 1000);
    active_layer()->UpdateTiles();
    ...
    for (auto* draw_quad : render_pass->quad_list) {
        ...
        gfx::RectF transformed_visible_quad_layer_rect = transform(draw_quad->shared_quad_state->visible_quad_layer_rect);
        EXPECT_RECTF_EQ(transformed_visible_quad_layer_rect, gfx::RectF(0.f, 9000.f, 1000.f, 1000.f));
    }
    EXPECT_EQ(min_y, 0);
...
TEST_F(NoLowResPictureLayerImplTest, MarkRequiredOffscreenTiles)
{
    ...
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    SetupPendingTreeWithFixedTileSize(pending_raster_source, gfx::Size(100, 100), Region());

    EXPECT_EQ(1u, pending_layer()->num_tilings());
    EXPECT_EQ(viewport, pending_layer()->viewport_rect_for_tile_priority_in_content_space());
    ...
    std::unique_ptr<TilingSetRasterQueueAll> queue = TilingSetRasterQueueAll::Create(pending_layer()->picture_layer_tiling_set(), false, false);
    EXPECT_TRUE(queue);
...
TEST_F(NoLowResPictureLayerImplTest, TileOutsideOfViewportForTilePriorityNotRequired)
{
    ...
    gfx::Transform transform_for_tile_priority;
    host_impl()->SetExternalTilePriorityConstraints(external_viewport_for_tile_priority, transform_for_tile_priority);
    host_impl()->AdvanceToNextFrame(base::Milliseconds(1));
    UpdateDrawProperties(host_impl()->pending_tree());
    ...
    int num_inside = 0;
    int num_outside = 0;
    for (PictureLayerTiling::CoverageIterator iter(active_layer()->HighResTiling(), 1.f, gfx::Rect(layer_bounds)); iter; ++iter) {
        if (!*iter)
            continue;
        Tile* tile = *iter;
...
TEST_F(LegacySWPictureLayerImplTest, HighResTileIsIncompleteLowResComplete)
{
    ...
    host_impl()->tile_manager()->InitializeTilesWithResourcesForTesting(low_tiles);
...
TEST_F(LegacySWPictureLayerImplTest, LowResTileIsIncomplete)
{
    ...
    host_impl()->tile_manager()->InitializeTilesWithResourcesForTesting(high_tiles);
...
TEST_F(LegacySWPictureLayerImplTest, HighResAndIdealResTileIsCompleteWhenRasterScaleIsNotIdeal)
{
    host_impl()->AdvanceToNextFrame(base::Milliseconds(1));
    ...
    host_impl()->tile_manager()->InitializeTilesWithResourcesForTesting(ideal_tiles);
    ...
    EXPECT_EQ(gfx::RectF(0.f, 0.f, 99.f, 99.f), viz::TileDrawQuad::MaterialCast(render_pass->quad_list.front())->tex_coord_rect);
    EXPECT_EQ(gfx::Rect(99, 0, 100, 99), render_pass->quad_list.ElementAt(1)->rect);
    EXPECT_EQ(gfx::RectF(49.5f, 0.f, 50.f, 49.5f), viz::TileDrawQuad::MaterialCast(render_pass->quad_list.ElementAt(1))->tex_coord_rect);
...
TEST_F(LegacySWPictureLayerImplTest, AppendQuadsDataForCheckerboard)
{
    ...
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreatePartiallyFilled(layer_bounds, recorded_bounds);
    SetupPendingTreeWithFixedTileSize(pending_raster_source, tile_size, Region());
    pending_layer()->SetOffsetToTransformParent(layer_offset);
    host_impl()->pending_tree()->property_trees()->scroll_tree_mutable().SetScrollingContentsCullRect(
        pending_layer()->element_id(), gfx::Rect(layer_offset.x(), layer_offset.y(), 120, 120));
    ActivateTree();
    ...
    SetupPendingTree(FakeRasterSource::CreatePartiallyFilled(layer_bounds, recorded_bounds));
    ActivateTree();
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 180, 180), active_layer()->HighResTiling()->tiling_rect());
    EXPECT_EQ(1u, render_pass->quad_list.size());
    EXPECT_EQ(1, data.num_missing_tiles);
    EXPECT_TRUE(data.checkerboarded_needs_raster);
    EXPECT_TRUE(data.checkerboarded_needs_record);
    ...
    for (size_t i = 0; i < active_layer()->tilings()->num_tilings(); i++) {
        host_impl()->tile_manager()->InitializeTilesWithResourcesForTesting(active_layer()->tilings()->tiling_at(i)->AllTilesForTesting());
    }
    ...
    host_impl()->active_tree()->property_trees()->scroll_tree_mutable().SetScrollingContentsCullRect(
        active_layer()->element_id(), gfx::Rect(layer_offset.x(), layer_offset.y(), layer_bounds.width(), layer_bounds.height()));
...
TEST_F(LegacySWPictureLayerImplTest, RasterInducingScrollPaintCheckerboarding)
{
    ...
    scroll_list->EndPaintOfUnpaired(gfx::Rect(scroll_container_origin, scroll_contents_bounds));
    scroll_list->Finalize();
    auto display_list = base::MakeRefCounted<DisplayItemList>();
    display_list->PushDrawScrollingContentsOp(scroll_element_id, std::move(scroll_list), gfx::Rect(scroll_container_origin, scroll_container_bounds));
    ...
    SetupPendingTreeWithFixedTileSize(FakeRasterSource::CreateFromRecordingSource(recording), tile_size, Region());
    CreateScrollNodeForNonCompositedScroller(host_impl()->pending_tree()->property_trees(), pending_layer()->scroll_tree_index(), scroll_element_id,
        scroll_contents_bounds, scroll_container_bounds, scroll_container_origin);
    ScrollTree& pending_scroll_tree = host_impl()->pending_tree()->property_trees()->scroll_tree_mutable();
    pending_scroll_tree.SetScrollingContentsCullRect(scroll_element_id, cull_rect);
    ActivateTree();
    ...
    ScrollTree& active_scroll_tree = host_impl()->active_tree()->property_trees()->scroll_tree_mutable();
...
TEST_F(LegacySWPictureLayerImplTest, HighResRequiredWhenActiveAllReady)
{
    ...
    SetupDefaultTreesWithFixedTileSize(layer_bounds, tile_size, gfx::Rect(layer_bounds));
...
TEST_F(LegacySWPictureLayerImplTest, NothingRequiredIfActiveMissingTiles)
{
    ...
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    scoped_refptr<FakeRasterSource> active_raster_source = FakeRasterSource::CreatePartiallyFilled(layer_bounds, gfx::Rect(500, 500));

    SetupTreesWithFixedTileSize(pending_raster_source, active_raster_source, tile_size, Region());
    ...
    active_layer()->HighResTiling()->ComputeTilePriorityRects(visible_rect, visible_rect, visible_rect, visible_rect, 1, Occlusion());
...
TEST_F(LegacySWPictureLayerImplTest, HighResRequiredWhenActiveHasDifferentBounds)
{
...
TEST_F(LegacySWPictureLayerImplTest, ActivateUninitializedLayer)
{
    ...
    int kLayerId = 2;
    std::unique_ptr<FakePictureLayerImpl> pending_layer = FakePictureLayerImpl::Create(pending_tree, kLayerId, pending_raster_source);
    ...
    FakePictureLayerImpl* raw_active_layer = static_cast<FakePictureLayerImpl*>(host_impl()->active_tree()->LayerById(kLayerId));
...
TEST_F(LegacySWPictureLayerImplTest, ShareTilesOnNextFrame)
{
    ...
    active_layer()->tilings()->UpdateTilePriorities(gfx::Rect(), 1.f, 1.0, Occlusion(), true);
...
TEST_F(LegacySWPictureLayerImplTest, ShareInvalidActiveTreeTiles)
{
    ...
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    scoped_refptr<FakeRasterSource> active_raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    SetupTreesWithInvalidation(pending_raster_source, active_raster_source, gfx::Rect(1, 1));
...
TEST_F(LegacySWPictureLayerImplTest, HighResCreatedWhenBoundsShrinkOnActiveLayerWithUsedNonIdealScaleTiling)
{
    // Put 0.5 as high res.
    SetInitialDeviceScaleFactor(0.5f);
    ...
    ActivateTree();
    active_layer()->AddTiling(gfx::AxisTransform2d(1.0f, gfx::Vector2dF()))->set_resolution(NON_IDEAL_RESOLUTION);
    ...
    ActivateTree();
    active_layer()->AddLastAppendQuadsTilingForTesting(active_layer()->tilings()->FindTilingWithScaleKey(1.0f));
    active_layer()->UpdateTiles();
...
TEST_F(LegacySWPictureLayerImplTest, LowResTilingWithoutGpuRasterization)
{
    ...
    gfx::Size layer_bounds(default_tile_size.width() * 4, default_tile_size.height() * 4);
...
TEST_F(PictureLayerImplTest, RequiredTilesWithGpuRasterization)
{
    ...
    EXPECT_EQ(ALLOW_PREPAINT_ONLY, host_impl()->global_tile_state().memory_limit_policy);
    if (!features::IsCCSlimmingEnabled()) {
        ...
        EXPECT_EQ(15u, active_layer()->HighResTiling()->AllTilesForTesting().size());
    }
...
TEST_F(CommitToActiveTreePictureLayerImplTest, RequiredTilesWithGpuRasterizationAndFractionalDsf)
{
    ...
    SetContentsScaleOnActiveLayer(dsf /* contents_scale */, dsf /* device_scale_factor */, 1.0f /* page_scale_factor */);
...
TEST_F(LegacySWPictureLayerImplTest, PinchingTooSmall)
{
    ...
    SetContentsScaleOnBothLayers(high_res_scale, 1.f, high_res_scale);
    EXPECT_FLOAT_EQ(pending_layer()->MinimumContentsScale(), pending_layer()->HighResTiling()->contents_scale_key());
}
...
TEST_F(LegacySWPictureLayerImplTest, PinchingTooSmallWithContentsScale)
{
    ...
    EXPECT_FLOAT_EQ(contents_scale, pending_layer()->HighResTiling()->contents_scale_key());
    ...
    EXPECT_LT(page_scale * contents_scale, pending_layer()->MinimumContentsScale());
...
TEST_F(LegacySWPictureLayerImplTest, HighResTilingDuringAnimationSmallerAnimationScale)
{
    gfx::Size viewport_size(1000, 1000);
    host_impl()->active_tree()->SetDeviceViewportRect(gfx::Rect(viewport_size));
...
TEST_F(LegacySWPictureLayerImplTest, HighResTilingDuringAnimationSmallerAnimationScaleWithInvalidScale)
{
    gfx::Size viewport_size(1000, 1000);
    host_impl()->active_tree()->SetDeviceViewportRect(gfx::Rect(viewport_size));
...
TEST_F(LegacySWPictureLayerImplTest, AnimationTilingChangesWithWillChangeTransformHint)
{
    gfx::Size viewport_size(1000, 1000);
    host_impl()->active_tree()->SetDeviceViewportRect(gfx::Rect(viewport_size));
    ...
    // just use that.
    maximum_animation_scale = 2.f;
    LayerImpl* active_page_scale_layer = host_impl()->active_tree()->LayerById(active_layer()->id() - 1);
    LayerImpl* pending_page_scale_layer = host_impl()->pending_tree()->LayerById(pending_layer()->id() - 1);
    DCHECK_EQ(GetTransformNode(active_page_scale_layer)->id, GetTransformNode(active_layer())->parent_id);
    DCHECK_EQ(GetTransformNode(pending_page_scale_layer)->id, GetTransformNode(pending_layer())->parent_id);
...
TEST_F(LegacySWPictureLayerImplTest, HighResTilingDuringAnimationAspectRatioTooLarge)
{
    gfx::Size viewport_size(2000, 1000);
    host_impl()->active_tree()->SetDeviceViewportRect(gfx::Rect(viewport_size));
    ...
    SetContentsAndAnimationScalesOnBothLayers(contents_scale, device_scale, page_scale, maximum_animation_scale, affected_by_invalid_scale);
    EXPECT_NEAR(20.f, pending_layer()->HighResTiling()->contents_scale_key(), 0.001f);
    EXPECT_NEAR(20.f, active_layer()->HighResTiling()->contents_scale_key(), 0.001f);
    ...
    pending_layer()->UpdateRasterSource(FakeRasterSource::CreateFilled(layer_bounds), &invalidation);
    pending_layer()->PushPropertiesTo(active_layer());
    SetContentsAndAnimationScalesOnBothLayers(contents_scale, device_scale, page_scale, maximum_animation_scale, affected_by_invalid_scale);
    EXPECT_NEAR(10.f, pending_layer()->HighResTiling()->contents_scale_key(), 0.001f);
    EXPECT_NEAR(10.f, active_layer()->HighResTiling()->contents_scale_key(), 0.001f);
}
...
TEST_F(LegacySWPictureLayerImplTest, TilingSetRasterQueue)
{
    ...
    EXPECT_EQ(low_res_tile_count + high_res_tile_count + non_ideal_tile_count, static_cast<int>(unique_tiles.size()));

    std::unique_ptr<TilingSetRasterQueueRequired> required_queue
        = TilingSetRasterQueueRequired::Create(pending_layer()->picture_layer_tiling_set(), RasterTilePriorityQueue::Type::REQUIRED_FOR_DRAW);
    EXPECT_TRUE(required_queue);
    EXPECT_TRUE(required_queue->IsEmpty());

    required_queue = TilingSetRasterQueueRequired::Create(pending_layer()->picture_layer_tiling_set(), RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION);
    EXPECT_TRUE(required_queue);
    ...
    queue = TilingSetRasterQueueAll::Create(pending_layer()->picture_layer_tiling_set(), false, false);
    EXPECT_TRUE(queue);
    while (!queue->IsEmpty()) {
        PrioritizedTile prioritized_tile = queue->Top();
        TilePriority priority = prioritized_tile.priority();
    ...
    for (auto tile_it = high_res_tiles.begin(); tile_it != high_res_tiles.end(); ++tile_it) {
        Tile* tile = *tile_it;
        TileDrawInfo& draw_info = tile->draw_info();
        draw_info.SetSolidColorForTesting(SkColors::kRed);
    }

    queue = TilingSetRasterQueueAll::Create(pending_layer()->picture_layer_tiling_set(), true, false);
    EXPECT_TRUE(queue);
...
TEST_F(LegacySWPictureLayerImplTest, TilingSetRasterQueueActiveTree)
{
    ...
    std::unique_ptr<TilingSetRasterQueueRequired> queue
        = TilingSetRasterQueueRequired::Create(active_layer()->picture_layer_tiling_set(), RasterTilePriorityQueue::Type::REQUIRED_FOR_DRAW);
    EXPECT_TRUE(queue);
    ...
    queue = TilingSetRasterQueueRequired::Create(active_layer()->picture_layer_tiling_set(), RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION);
    if (features::IsCCSlimmingEnabled()) {
        EXPECT_FALSE(queue);
    } else {
        EXPECT_TRUE(queue);
...
TEST_F(LegacySWPictureLayerImplTest, TilingSetRasterQueueRequiredNoHighRes)
{
    ...
    SetupPendingTree(pending_raster_source);
    EXPECT_FALSE(pending_layer()->picture_layer_tiling_set()->FindTilingWithResolution(HIGH_RESOLUTION));

    std::unique_ptr<TilingSetRasterQueueRequired> queue
        = TilingSetRasterQueueRequired::Create(pending_layer()->picture_layer_tiling_set(), RasterTilePriorityQueue::Type::REQUIRED_FOR_ACTIVATION);
    if (features::IsCCSlimmingEnabled()) {
        EXPECT_FALSE(queue);
    } else {
        EXPECT_TRUE(queue);
...
TEST_F(LegacySWPictureLayerImplTest, TilingSetEvictionQueue)
{
    ...
    for (size_t i = 0; i < pending_layer()->num_tilings(); ++i) {
        PictureLayerTiling* tiling = pending_layer()->tilings()->tiling_at(i);
        for (PictureLayerTiling::CoverageIterator iter(tiling, 1.f, pending_layer()->visible_layer_rect()); iter; ++iter) {
            if (mark_required) {
    ...
    std::unique_ptr<TilingSetEvictionQueue> queue(
        new TilingSetEvictionQueue(pending_layer()->picture_layer_tiling_set(), pending_layer()->contributes_to_drawn_render_surface()));
    EXPECT_TRUE(queue->IsEmpty());

    host_impl()->tile_manager()->InitializeTilesWithResourcesForTesting(all_tiles);
    ...
    queue = std::make_unique<TilingSetEvictionQueue>(pending_layer()->picture_layer_tiling_set(), pending_layer()->contributes_to_drawn_render_surface());
    while (!queue->IsEmpty()) {
        PrioritizedTile prioritized_tile = queue->Top();
        ...
        while (std::abs(tile->contents_scale_key() - expected_scales[scale_index]) > std::numeric_limits<float>::epsilon()) {
            ++scale_index;
        ...
        if (tile->required_for_activation() == last_tile.tile()->required_for_activation()
            && std::abs(tile->contents_scale_key() - last_tile.tile()->contents_scale_key()) < std::numeric_limits<float>::epsilon()) {
...
TEST_F(LegacySWPictureLayerImplTest, Occlusion)
{
...

    {
        ...
        VerifyQuadsAreOccluded(impl.quad_list(), occluded, &partially_occluded_count);
...
TEST_F(LegacySWPictureLayerImplTest, ForceAdjustRasterScaleWillChangeTransform)
{
    gfx::Size layer_bounds(100, 100);
    SetupDefaultTrees(layer_bounds);
...
TEST_F(NoLowResPictureLayerImplTest, NothingRequiredIfActiveMissingTiles)
{
    ...
    // Active layer has tilings.
    EXPECT_TRUE(active_layer()->CanHaveTilings());
    EXPECT_EQ(active_layer()->tilings()->num_tilings(), host_impl()->settings().create_low_res_tiling ? 2u : 1u);
    // Simulate scrolling past the end of recorded content on the active layer,
    // where the recordings are so far away that no tiles are created.
    gfx::Rect visible_rect(0, 3000, 1000, 1000);
    active_layer()->HighResTiling()->ComputeTilePriorityRects(visible_rect, visible_rect, visible_rect, visible_rect, 1, Occlusion());
...
TEST_F(NoLowResPictureLayerImplTest, CleanUpTilings)
{
    ...
    // Now move the ideal scale to 1.1 on the active layer. Our target stays 1.2.
    SetupDrawPropertiesAndUpdateTiles(active_layer(), 1.1f, device_scale, page_scale);
...
TEST_F(LegacySWPictureLayerImplTest, SharedQuadStateContainsMaxTilingScale)
{
    ...
    scaled_draw_transform.Scale(SK_Scalar1 / max_contents_scale, SK_Scalar1 / max_contents_scale);
    ...
    EXPECT_EQ(scaled_draw_transform.ToString(), render_pass->shared_quad_state_list.front()->quad_to_target_transform.ToString());
    // The content_bounds should be scaled by the
    // MaximumTilingContentsScale on the layer.
    EXPECT_EQ(gfx::Rect(2500u, 5000u).ToString(), render_pass->shared_quad_state_list.front()->quad_layer_rect.ToString());
    // The visible_layer_rect should be scaled by the
    // MaximumTilingContentsScale on the layer.
    EXPECT_EQ(gfx::Rect(0u, 0u, 2500u, 5000u).ToString(), render_pass->shared_quad_state_list.front()->visible_quad_layer_rect.ToString());
}

class PictureLayerImplTestWithDelegatingRenderer : public LegacySWPictureLayerImplTest {
public:
    std::unique_ptr<LayerTreeFrameSink> CreateLayerTreeFrameSink() override
    {
...
TEST_F(PictureLayerImplTestWithDelegatingRenderer, DelegatingRendererWithTileOOM)
{
    ...
    ManagedMemoryPolicy policy(memory_limit, gpu::MemoryAllocation::CUTOFF_ALLOW_EVERYTHING, resource_limit);
    ...
    EXPECT_EQ(viz::DrawQuad::Material::kTiledContent, render_pass->quad_list.front()->material);
    EXPECT_EQ(viz::DrawQuad::Material::kSolidColor, render_pass->quad_list.back()->material);
}

class OcclusionTrackingPictureLayerImplTest : public LegacySWPictureLayerImplTest {
public:
    LayerTreeSettings CreateSettings() override
    {
        LayerTreeSettings settings = LegacySWPictureLayerImplTest::CreateSettings();
    ...
    void VerifyEvictionConsidersOcclusion(FakePictureLayerImpl* layer, WhichTree tree, size_t expected_occluded_tile_count, int source_line)
    {
        size_t occluded_tile_count = 0u;
        PrioritizedTile last_tile;

        std::unique_ptr<TilingSetEvictionQueue> queue(
            new TilingSetEvictionQueue(layer->picture_layer_tiling_set(), layer->contributes_to_drawn_render_surface()));
        while (!queue->IsEmpty()) {
            PrioritizedTile prioritized_tile = queue->Top();
            Tile* tile = prioritized_tile.tile();
            if (!last_tile.tile())
            ...
            if (tile_is_occluded) {
                ...
                if (!last_tile_is_occluded) {
                    ...
                    EXPECT_TRUE(tile_priority_bin < last_tile_priority_bin || tile->required_for_activation()
                        || tile->contents_scale_key() != last_tile.tile()->contents_scale_key())
                        << "line: " << source_line;
                }
            }
            last_tile = prioritized_tile;
        ...
        EXPECT_EQ(expected_occluded_tile_count, occluded_tile_count) << "line: " << source_line;
    }

    void TestOcclusionForScale(float scale, int expected_occluded_count);
};

TEST_F(OcclusionTrackingPictureLayerImplTest, OccludedTilesSkippedDuringRasterization)
{
    host_impl()->AdvanceToNextFrame(base::Milliseconds(1));
...
TEST_F(OcclusionTrackingPictureLayerImplTest, OccludedTilesNotMarkedAsRequired)
{
    host_impl()->AdvanceToNextFrame(base::Milliseconds(1));
    ...
    for (size_t i = 0; i < pending_layer()->num_tilings(); ++i) {
        ...
        for (PictureLayerTiling::CoverageIterator iter(tiling, 1.f, gfx::Rect(layer_bounds)); iter; ++iter) {
            if (!*iter)
                continue;
...
void OcclusionTrackingPictureLayerImplTest::TestOcclusionForScale(float scale, int expected_occluded_count)
{
    host_impl()->AdvanceToNextFrame(base::Milliseconds(1));
    ...
    float low_res_factor = host_impl()->settings().low_res_contents_scale_factor;
    active_layer()->AddTiling(gfx::AxisTransform2d(low_res_factor, gfx::Vector2dF()))->set_resolution(LOW_RESOLUTION);
    active_layer()->AddTiling(gfx::AxisTransform2d(scale, gfx::Vector2dF()))->set_resolution(HIGH_RESOLUTION);
    ...
    for (size_t i = 0; i < active_layer()->num_tilings(); ++i) {
        ...
        for (size_t j = 0; j < tiles.size(); ++j) {
            if (prioritized_tiles[tiles[j]].is_occluded()) {
                gfx::Rect scaled_content_rect = ScaleToEnclosingRect(tiles[j]->content_rect(), 1.f / tiles[j]->contents_scale_key());
...
TEST_F(OcclusionTrackingPictureLayerImplTest, DifferentOcclusionOnTrees)
{
    ...
    for (size_t i = 0; i < active_layer()->num_tilings(); ++i) {
        ...
        for (PictureLayerTiling::CoverageIterator iter(tiling, 1.f, gfx::Rect(layer_bounds)); iter; ++iter) {
            ...
            gfx::Rect scaled_content_rect = ScaleToEnclosingRect(tile->content_rect(), 1.f / tile->contents_scale_key());
            // Tiles are occluded on the active tree iff they lie beneath the
            // occluding layer.
            EXPECT_EQ(prioritized_tiles[tile].is_occluded(), scaled_content_rect.x() >= occluding_layer_position.x());
        }
    }
    ...
    for (size_t i = 0; i < active_layer()->num_tilings(); ++i) {
        ...
        for (PictureLayerTiling::CoverageIterator iter(tiling, 1.f, gfx::Rect(layer_bounds)); iter; ++iter) {
            ...
            Tile* twin_tile = active_layer()->GetPendingOrActiveTwinTiling(tiling)->TileAt(iter.i(), iter.j());
            gfx::Rect scaled_content_rect = ScaleToEnclosingRect(tile->content_rect(), 1.f / tile->contents_scale_key());
...
TEST_F(OcclusionTrackingPictureLayerImplTest, OccludedTilesConsideredDuringEviction)
{
    ...
    // Partially occlude the active layer.
    LayerImpl* active_occluding_layer = AddLayer<LayerImpl>(host_impl()->pending_tree());
    ...
    active_occluding_layer->SetOffsetToTransformParent(active_occluding_layer_position);
    ActivateTree();

    // Partially invalidate the pending layer. Tiles inside the invalidation rect
    // are created.
    SetupPendingTreeWithFixedTileSize(pending_raster_source, tile_size, invalidation_rect);

    // Partially occlude the pending layer in a different way.
    LayerImpl* pending_occluding_layer = host_impl()->pending_tree()->LayerById(active_occluding_layer->id());
    ASSERT_EQ(active_occluding_layer->bounds(), pending_occluding_layer->bounds());
    ASSERT_TRUE(pending_occluding_layer->draws_content());
    ASSERT_TRUE(pending_occluding_layer->contents_opaque());
    pending_occluding_layer->SetOffsetToTransformParent(pending_occluding_layer_position);
    ...
    float dest_scale = std::max(active_layer()->MaximumTilingContentsScale(), pending_layer()->MaximumTilingContentsScale());
    ...
    for (size_t i = 0; i < pending_layer()->num_tilings(); ++i) {
        ...
        for (PictureLayerTiling::CoverageIterator iter(tiling, dest_scale, dest_layer_bounds); iter; ++iter) {
            Tile* tile = *iter;
        ...
        EXPECT_EQ(expected_occluded_tile_count_on_pending[i], occluded_tile_count_on_pending) << tiling->contents_scale_key();
    }
    ...
    for (size_t i = 0; i < active_layer()->num_tilings(); ++i) {
        ...
        EXPECT_EQ(expected_occluded_tile_count_on_active[i], occluded_tile_count_on_active) << i;
    }
    ...
    VerifyEvictionConsidersOcclusion(pending_layer(), PENDING_TREE, total_expected_occluded_tile_count_on_trees[PENDING_TREE], __LINE__);
    VerifyEvictionConsidersOcclusion(active_layer(), ACTIVE_TREE, total_expected_occluded_tile_count_on_trees[ACTIVE_TREE], __LINE__);
...
void GetClientDataAndUpdateInvalidation(RecordingSource& recording_source, FakeContentLayerClient* client, gfx::Size layer_bounds)
{
    Region invalidation;
    recording_source.Update(layer_bounds, /*recording_scale_factor=*/1.f, *client, invalidation);
}

void PictureLayerImplTest::TestQuadsForSolidColor(bool test_for_solid, bool partial_opaque)
{
    host_impl()->AdvanceToNextFrame(base::Milliseconds(1));
    ...
    std::unique_ptr<FakeLayerTreeHost> host = FakeLayerTreeHost::Create(&host_client, &task_graph_runner, animation_host.get());
    ...
    scoped_refptr<RasterSource> pending_raster_source = recording_source.CreateRasterSource();
    ...
    if (test_for_solid) {
        ...
        host_impl()->tile_manager()->InitializeTilesWithResourcesForTesting(resource_tiles);
    }
    ...
    viz::DrawQuad::Material expected = test_for_solid ? viz::DrawQuad::Material::kSolidColor : viz::DrawQuad::Material::kTiledContent;
...
TEST_F(LegacySWPictureLayerImplTest, NonSolidToSolidNoTilings)
{
    ...
    scoped_refptr<RasterSource> raster_source1 = recording_source.CreateRasterSource();
    ...
    scoped_refptr<RasterSource> raster_source2 = recording_source.CreateRasterSource();
...
TEST_F(LegacySWPictureLayerImplTest, ChangeInViewportAllowsTilingUpdates)
{
    ...
    for (PictureLayerTiling::CoverageIterator iter(pending_layer()->HighResTiling(), 1.f, pending_layer()->HighResTiling()->GetCurrentVisibleRectForTesting());
         iter; ++iter) {
        if (*iter)
            tiles.push_back(*iter);
    }
    for (PictureLayerTiling::CoverageIterator iter(active_layer()->HighResTiling(), 1.f, active_layer()->HighResTiling()->GetCurrentVisibleRectForTesting());
         iter; ++iter) {
        if (*iter)
            tiles.push_back(*iter);
    }
...
TEST_F(LegacySWPictureLayerImplTest, CloneMissingRecordings)
{
    ...
    scoped_refptr<FakeRasterSource> partial_raster_source = FakeRasterSource::CreatePartiallyFilled(layer_bounds, gfx::Rect(100, 100, 300, 300));
    ...
    // back.
    SetupPendingTreeWithFixedTileSize(filled_raster_source, tile_size, Region(gfx::Rect(layer_bounds)));
...
TEST_F(LegacySWPictureLayerImplTest, DirectlyCompositedImageRasterSourceCoverage)
{
    gfx::Size tile_size(100, 100);
    gfx::Size layer_bounds(400, 400);

    scoped_refptr<FakeRasterSource> filled_raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    filled_raster_source->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(1, 1));

    scoped_refptr<FakeRasterSource> partial_raster_source = FakeRasterSource::CreatePartiallyFilled(layer_bounds, gfx::Rect(150, 150, 100, 100));
    partial_raster_source->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(1, 1));
...
TEST_F(LegacySWPictureLayerImplTest, ScrollPastLiveTilesRectAndBack)
{
    ...
    host_impl()->SetExternalTilePriorityConstraints(gfx::Rect(0, 5000, 100, 100), gfx::Transform());

    SetupPendingTreeWithFixedTileSize(pending_raster_source, tile_size, gfx::Rect());
    ...
    host_impl()->SetExternalTilePriorityConstraints(gfx::Rect(0, 110, 100, 100), gfx::Transform());
...
TEST_F(LegacySWPictureLayerImplTest, UpdateLCDTextPushToActiveTree)
{
    ...
    SetupDrawPropertiesAndUpdateTiles(pending_layer(), page_scale, 1.0f, page_scale);
    // LCD text is disallowed before SetContentsOpaque(true).
    EXPECT_FALSE(pending_layer()->can_use_lcd_text());
    pending_layer()->SetContentsOpaque(true);
    pending_layer()->UpdateTiles();
...
TEST_F(LegacySWPictureLayerImplTest, UpdateLCDTextPushToActiveTreeWith2dScale)
{
    ...
    float page_scale = 4.f;
    SetupDrawPropertiesAndUpdateTiles(pending_layer(), ideal_scale, 1.0f, page_scale);
    // LCD text is disallowed before SetContentsOpaque(true).
    EXPECT_FALSE(pending_layer()->can_use_lcd_text());
    pending_layer()->SetContentsOpaque(true);
    pending_layer()->UpdateTiles();
...
TEST_F(LegacySWPictureLayerImplTest, TilingAllTilesDone)
{
    ...
    pending_layer()->SetBounds(layer_bounds);
    ActivateTree();
    host_impl()->tile_manager()->InitializeTilesWithResourcesForTesting(active_layer()->HighResTiling()->AllTilesForTesting());
    host_impl()->SetTreePriority(SAME_PRIORITY_FOR_BOTH_TREES);
...

    {
        ...
        ManagedMemoryPolicy policy(memory_limit, gpu::MemoryAllocation::CUTOFF_ALLOW_EVERYTHING, max_tiles);
        host_impl()->SetMemoryPolicy(policy);
        host_impl()->PrepareTiles();
...
TEST_F(TileSizeTest, GPUTileSizes)
{
    ...
    layer->set_gpu_raster_max_texture_size(host_impl()->active_tree()->GetDeviceViewport().size());
    result = layer->CalculateTileSize(gfx::Size(10000, 10000));
    EXPECT_EQ(result.width(), MathUtil::UncheckedRoundUp(1000 + 2 * PictureLayerTiling::kBorderTexels, 32));
...
TEST_F(TileSizeTest, RawDrawTileSizes)
{
    ...
    layer->set_gpu_raster_max_texture_size(gfx::Size());
    host_impl()->NotifyReadyToActivate();
    content_bounds = gfx::Size(std::numeric_limits<int>::max(), std::numeric_limits<int>::max());
...
TEST_F(HalfWidthTileTest, TileSizes)
{
    ...
    layer->set_gpu_raster_max_texture_size(host_impl()->active_tree()->GetDeviceViewport().size());
    result = layer->CalculateTileSize(gfx::Size(10000, 10000));
    EXPECT_EQ(result.width(), MathUtil::UncheckedRoundUp(2000 / 2 + 2 * PictureLayerTiling::kBorderTexels, 32));
...
TEST_F(NoLowResPictureLayerImplTest, LowResWasHighResCollision)
{
    ...
    EXPECT_EQ(1u, pending_layer()->num_tilings());
    EXPECT_EQ(zoomed, pending_layer()->tilings()->tiling_at(0)->contents_scale_key());
}
...
TEST_F(LegacySWPictureLayerImplTest, HighResWasLowResCollision)
{
    ...
    SetupDrawPropertiesAndUpdateTiles(active_layer(), page_scale, 1.0f, page_scale);
    EXPECT_EQ(2u, active_layer()->tilings()->num_tilings());
    EXPECT_EQ(page_scale, active_layer()->tilings()->tiling_at(0)->contents_scale_key());
    ...
    EXPECT_TRUE(active_layer()->tilings()->tiling_at(1)->may_contain_low_resolution_tiles());
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageCalculateContentsScale)
{
    ...
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    pending_raster_source->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(1, 1));
    SetupPendingTree(pending_raster_source);
    ASSERT_TRUE(pending_layer()->IsDirectlyCompositedImage());
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageIgnoreIdealContentsScale)
{
    ...
    SetupDrawPropertiesAndUpdateTiles(pending_layer_ptr, suggested_ideal_contents_scale, device_scale_factor, page_scale_factor);
    EXPECT_EQ(1.f, pending_layer_ptr->tilings()->tiling_at(0)->contents_scale_key());

    // Push to active layer.
    host_impl()->ActivateSyncTree();

    FakePictureLayerImpl* active_layer = static_cast<FakePictureLayerImpl*>(host_impl()->active_tree()->LayerById(kLayerId));
    SetupDrawPropertiesAndUpdateTiles(active_layer, suggested_ideal_contents_scale, device_scale_factor, page_scale_factor);
    ...
    host_impl()->tile_manager()->InitializeTilesWithResourcesForTesting(active_layer->tilings()->tiling_at(0)->AllTilesForTesting());
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageRasterScaleChanges)
{
    ...
    for (int i = 1; i < 30; ++i) {
        ...
        SetupDrawPropertiesAndUpdateTiles(pending_layer(), ideal_contents_scale, 1.f, 1.f);
        EXPECT_FLOAT_EQ(expected_contents_scale, pending_layer()->picture_layer_tiling_set()->FindTilingWithResolution(HIGH_RESOLUTION)->contents_scale_key())
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageRasterOnChange)
{
    gfx::Size layer_bounds(400, 400);
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    pending_raster_source->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(0.5f, 0.5f));
    ...
    SetupDrawPropertiesAndUpdateTiles(pending_layer(), 1.f, 1.f, 1.f);
    EXPECT_FLOAT_EQ(0.5f, pending_layer()->picture_layer_tiling_set()->FindTilingWithResolution(HIGH_RESOLUTION)->contents_scale_key());
    ...
    pending_layer()->SetBounds(layer_bounds);
    pending_raster_source->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(5.f, 5.f));
    pending_layer()->UpdateDirectlyCompositedImageFromRasterSource();
    SetupDrawPropertiesAndUpdateTiles(pending_layer(), 1.f, 1.f, 1.f);
    EXPECT_FLOAT_EQ(1.25f, pending_layer()->picture_layer_tiling_set()->FindTilingWithResolution(HIGH_RESOLUTION)->contents_scale_key());
    ...
    EXPECT_FLOAT_EQ(5.f, pending_layer()->picture_layer_tiling_set()->FindTilingWithResolution(HIGH_RESOLUTION)->contents_scale_key());
    ...
    EXPECT_FLOAT_EQ(0.625f, pending_layer()->picture_layer_tiling_set()->FindTilingWithResolution(HIGH_RESOLUTION)->contents_scale_key());
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageTinyRasterScale)
{
    ...
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    pending_raster_source->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(1e-20f, 1e-20f));

    SetupPendingTree(pending_raster_source);
    ASSERT_TRUE(pending_layer()->IsDirectlyCompositedImage());

    SetupDrawPropertiesAndUpdateTiles(pending_layer(), 1.f, 1.f, 1.f);
    EXPECT_FLOAT_EQ(1.f / 512, pending_layer()->picture_layer_tiling_set()->FindTilingWithResolution(HIGH_RESOLUTION)->contents_scale_key());
}
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageHugeRasterScale)
{
    gfx::Size layer_bounds(400, 400);
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    pending_raster_source->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(1e20f, 1e20f));

    SetupPendingTree(pending_raster_source);
    ASSERT_TRUE(pending_layer()->IsDirectlyCompositedImage());

    SetupDrawPropertiesAndUpdateTiles(pending_layer(), 1.f, 1.f, 1.f);
    EXPECT_FLOAT_EQ(1.0f, pending_layer()->picture_layer_tiling_set()->FindTilingWithResolution(HIGH_RESOLUTION)->contents_scale_key());
}
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageRasterHighResScreen)
{
    gfx::Size layer_bounds(100, 100);
    scoped_refptr<FakeRasterSource> pending_raster_source = FakeRasterSource::CreateFilled(layer_bounds);
    pending_raster_source->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(5, 5));
...
TEST_F(LegacySWPictureLayerImplTest, CompositedImageRasterOptOutTransitions)
{
    ...
    EXPECT_FLOAT_EQ(1, pending_layer()->picture_layer_tiling_set()->FindTilingWithResolution(HIGH_RESOLUTION)->contents_scale_key());
    ...
    pending_raster_source->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(50, 50));
    pending_layer()->UpdateDirectlyCompositedImageFromRasterSource();
    SetupDrawPropertiesAndUpdateTiles(pending_layer(), 0.2f, 1.f, 1.f);
    EXPECT_FLOAT_EQ(0.2f, pending_layer()->picture_layer_tiling_set()->FindTilingWithResolution(HIGH_RESOLUTION)->contents_scale_key());
    ...
    pending_raster_source->SetDirectlyCompositedImageDefaultRasterScale(gfx::Vector2dF(2, 2));
    pending_layer()->UpdateDirectlyCompositedImageFromRasterSource();
    SetupDrawPropertiesAndUpdateTiles(pending_layer(), 0.2f, 1.f, 1.f);
    EXPECT_FLOAT_EQ(0.25f, pending_layer()->picture_layer_tiling_set()->FindTilingWithResolution(HIGH_RESOLUTION)->contents_scale_key());
}

TEST_F(LegacySWPictureLayerImplTest, ChangeRasterTranslationNukePendingLayerTiles)
{
    gfx::Size layer_bounds(200, 200);
    gfx::Size tile_size(256, 256);
    auto raster_source = FakeRasterSource::CreateFilledWithText(layer_bounds);
    SetupTreesWithFixedTileSize(raster_source, raster_source, tile_size, Region());
    ...
    pending_layer()->draw_properties().screen_space_transform.PostConcat(translate1);
    pending_layer()->draw_properties().target_space_transform = pending_layer()->draw_properties().screen_space_transform;
    pending_layer()->UpdateTiles();
    ASSERT_EQ(1u, pending_layer()->tilings()->num_tilings());
    {
        PictureLayerTiling* tiling = pending_layer()->tilings()->tiling_at(0);
        EXPECT_EQ(gfx::AxisTransform2d(2.25f, gfx::Vector2dF(0.25f, 0.5f)), tiling->raster_transform());
    ...
    pending_layer()->draw_properties().screen_space_transform.PostConcat(translate2);
    pending_layer()->draw_properties().target_space_transform = pending_layer()->draw_properties().screen_space_transform;
    pending_layer()->UpdateTiles();
    ASSERT_EQ(1u, pending_layer()->tilings()->num_tilings());
    {
        PictureLayerTiling* tiling = pending_layer()->tilings()->tiling_at(0);
        EXPECT_EQ(gfx::AxisTransform2d(2.25f, gfx::Vector2dF(0.75f, 0.25f)), tiling->raster_transform());
        EXPECT_EQ(4u, tiling->AllTilesForTesting().size());
        for (auto* tile : tiling->AllTilesForTesting())
            EXPECT_EQ(tile->raster_transform(), tiling->raster_transform());
    }
...
TEST_F(LegacySWPictureLayerImplTest, ChangeRasterTranslationNukeActiveLayerTiles)
{
    gfx::Size layer_bounds(200, 200);
    gfx::Size tile_size(256, 256);
    auto raster_source = FakeRasterSource::CreateFilledWithText(layer_bounds);
    ...
    gfx::Transform translate1;
    translate1.Translate(0.25f, 0.5f);
    active_layer()->draw_properties().screen_space_transform.PostConcat(translate1);
    active_layer()->draw_properties().target_space_transform = active_layer()->draw_properties().screen_space_transform;
    active_layer()->UpdateTiles();
    ASSERT_EQ(3u, active_layer()->tilings()->num_tilings());
    {
        PictureLayerTiling* tiling = active_layer()->tilings()->FindTilingWithScaleKey(2.25f);
        EXPECT_EQ(gfx::AxisTransform2d(2.25f, gfx::Vector2dF(0.25f, 0.5f)), tiling->raster_transform());
        EXPECT_EQ(4u, tiling->AllTilesForTesting().size());
        for (auto* tile : tiling->AllTilesForTesting())
            EXPECT_EQ(tile->raster_transform(), tiling->raster_transform());
...
TEST_F(LegacySWPictureLayerImplTest, AnimatedImages)
{
    ...
    std::vector<FrameMetadata> frames = { FrameMetadata(true, base::Milliseconds(1)), FrameMetadata(true, base::Milliseconds(1)) };
    ...
    EXPECT_EQ(controller->GetDriversForTesting(image1.stable_id()).count(pending_layer()), 1u);
    EXPECT_EQ(controller->GetDriversForTesting(image2.stable_id()).count(pending_layer()), 1u);
    ...
    EXPECT_EQ(controller->GetDriversForTesting(image1.stable_id()).count(active_layer()), 1u);
    EXPECT_EQ(controller->GetDriversForTesting(image2.stable_id()).count(active_layer()), 1u);
...
TEST_F(LegacySWPictureLayerImplTest, PaintWorkletInputPaintRecordInvalidation)
{
    ...
    PaintWorkletInput::PropertyKey key(PaintWorkletInput::NativePropertyType::kClipPath, ElementId());

    // Set up a raster source with a PaintWorkletInput.
    FakeRecordingSource recording_source(layer_bounds);
    scoped_refptr<TestPaintWorkletInput> input1 = base::MakeRefCounted<TestPaintWorkletInput>(key, gfx::SizeF(100, 100));
...
TEST_F(LegacySWPictureLayerImplTest, PaintWorkletInputs)
{
    ...
    FakeRecordingSource recording_source(layer_bounds);
    scoped_refptr<TestPaintWorkletInput> input1 = base::MakeRefCounted<TestPaintWorkletInput>(gfx::SizeF(100, 100));
    PaintImage image1 = CreatePaintWorkletPaintImage(input1);
    scoped_refptr<TestPaintWorkletInput> input2 = base::MakeRefCounted<TestPaintWorkletInput>(gfx::SizeF(50, 50));
    ...
    scoped_refptr<TestPaintWorkletInput> input3 = base::MakeRefCounted<TestPaintWorkletInput>(gfx::SizeF(12, 12));
...
TEST_F(LegacySWPictureLayerImplTest, PaintWorkletInputsIdenticalEntries)
{
    ...
    FakeRecordingSource recording_source(layer_bounds);
    scoped_refptr<TestPaintWorkletInput> input = base::MakeRefCounted<TestPaintWorkletInput>(gfx::SizeF(100, 100));
    ...
    // All inputs should be registered on the pending layer.
    SetupPendingTree(recording_source.CreateRasterSource(), gfx::Size(), Region(gfx::Rect(layer_bounds)));
    EXPECT_EQ(pending_layer()->GetPaintWorkletRecordMap().size(), 1u);
    ...
    pending_layer()->picture_layer_tiling_set()->RemoveAllTiles();
    recording_source.Rerecord();
    pending_layer()->SetRasterSource(recording_source.CreateRasterSource(), Region());
    EXPECT_EQ(pending_layer()->GetPaintWorkletRecordMap().size(), 1u);
...
TEST_F(LegacySWPictureLayerImplTest, NoTilingsUsesScaleOne)
{
    ...
    active_layer()->draw_properties().visible_layer_rect = gfx::Rect(0, 0, 1000, 1000);
    active_layer()->UpdateTiles();
...
TEST_F(LegacySWPictureLayerImplTest, TransformedRasterizationAndContentsOpaqueAndLCDText)
{
    auto raster_source = FakeRasterSource::CreateFilledWithText(gfx::Size(200, 200));
    ...
    EXPECT_EQ(LCDTextDisallowedReason::kNone, pending_layer()->ComputeLCDTextDisallowedReasonForTesting());
    ASSERT_TRUE(pending_layer()->HighResTiling());
    EXPECT_EQ(gfx::Vector2dF(0.2, 0.3), pending_layer()->HighResTiling()->raster_transform().translation());
    ...
    EXPECT_TRUE(pending_layer()->contents_opaque());
    EXPECT_TRUE(pending_layer()->contents_opaque_for_text());
    EXPECT_EQ(LCDTextDisallowedReason::kWillChangeTransform, pending_layer()->ComputeLCDTextDisallowedReasonForTesting());
    ASSERT_TRUE(pending_layer()->HighResTiling());
    EXPECT_EQ(gfx::Vector2dF(0.2, 0.3), pending_layer()->HighResTiling()->raster_transform().translation());
    ...
    EXPECT_EQ(LCDTextDisallowedReason::kNone, pending_layer()->ComputeLCDTextDisallowedReasonForTesting());
    ASSERT_TRUE(pending_layer()->HighResTiling());
    EXPECT_EQ(gfx::Vector2dF(0.4, 0.5), pending_layer()->HighResTiling()->raster_transform().translation());
}

TEST_F(LegacySWPictureLayerImplTest, TransformedRasterizationAndLCDTextWithoutText)
{
    ...
    EXPECT_TRUE(pending_layer()->contents_opaque());
    EXPECT_TRUE(pending_layer()->contents_opaque_for_text());
    EXPECT_EQ(LCDTextDisallowedReason::kNoText, pending_layer()->ComputeLCDTextDisallowedReasonForTesting());
    ASSERT_TRUE(pending_layer()->HighResTiling());
    EXPECT_EQ(gfx::Vector2dF(), pending_layer()->HighResTiling()->raster_transform().translation());
}
...
TEST_F(LegacySWPictureLayerImplTest, InvalidateRasterInducingScrolls)
{
    ...
    display_list->push<ClipRectOp>(SkRect::MakeWH(200, 200), SkClipOp::kIntersect, false);
    display_list->EndPaintOfPairedBegin();
    display_list->PushDrawScrollingContentsOp(scroll_element_id1, scroll_list1, gfx::Rect(200, 200));
    display_list->StartPaint();
    ...
    display_list->push<ClipRectOp>(SkRect::MakeWH(200, 200), SkClipOp::kIntersect, false);
    display_list->EndPaintOfPairedBegin();
    display_list->PushDrawScrollingContentsOp(scroll_element_id2, scroll_list2, gfx::Rect(100, 300, 200, 200));
    display_list->StartPaint();
    display_list->push<RestoreOp>();
    display_list->EndPaintOfPairedEnd();
    display_list->Finalize();
...
class LCDTextTest : public PictureLayerImplTest, public testing::WithParamInterface<unsigned> {
protected:
    LayerTreeSettings CreateSettings() override
    {
        auto settings = PictureLayerImplTest::CreateSettings();
        settings.can_use_lcd_text = GetParam() & kCanUseLCDText;
        settings.layers_always_allowed_lcd_text = GetParam() & kLayersAlwaysAllowedLCDText;
        return settings;
    }
    ...
    void CheckCanUseLCDText(LCDTextDisallowedReason expected_disallowed_reason, const char* description, PictureLayerImpl* layer = nullptr)
    {
        ...
        if (layer) {
            EXPECT_EQ(expected_disallowed_reason, layer->ComputeLCDTextDisallowedReasonForTesting());
        } else {
            EXPECT_EQ(expected_disallowed_reason, layer_->ComputeLCDTextDisallowedReasonForTesting());
            EXPECT_EQ(expected_disallowed_reason, descendant_->ComputeLCDTextDisallowedReasonForTesting());
        }
    }
...
INSTANTIATE_TEST_SUITE_P(All, LCDTextTest, testing::Values(0, kCanUseLCDText, kLayersAlwaysAllowedLCDText, kCanUseLCDText | kLayersAlwaysAllowedLCDText));
...
TEST_P(LCDTextTest, NonIntegralTranslation)
{
    ...
    CheckCanUseLCDText(LCDTextDisallowedReason::kNone, "non-integeral translation");
...
TEST_P(LCDTextTest, NonIntegralTranslationAboveRenderTarget)
{
    ...
    CheckCanUseLCDText(LCDTextDisallowedReason::kNonIntegralTranslation, "non-integeral translation above render target");
    SetTransform(layer_.get(), gfx::Transform());
    CheckCanUseLCDText(LCDTextDisallowedReason::kNone, "identity transform");
}
...
TEST_P(LCDTextTest, NonTranslation)
{
    ...
    CheckCanUseLCDText(LCDTextDisallowedReason::kNonIntegralTranslation, "Rotation transform");
    ...
    CheckCanUseLCDText(LCDTextDisallowedReason::kNonIntegralTranslation, "Skew transform");
...
TEST_P(LCDTextTest, NonTranslationAboveRenderTarget)
{
    ...
    rotation.Rotate(10.0);
    SetTransform(layer_.get(), rotation);
    CheckCanUseLCDText(LCDTextDisallowedReason::kNonIntegralTranslation, "rotation transform above render target");
    ...
    SetTransform(layer_.get(), scale);
    CheckCanUseLCDText(LCDTextDisallowedReason::kNonIntegralTranslation, "scale transform above render target");

    // Skew.
    gfx::Transform skew;
    skew.Skew(10.0, 0.0);
    SetTransform(layer_.get(), skew);
    CheckCanUseLCDText(LCDTextDisallowedReason::kNonIntegralTranslation, "skew transform above render target");
...
TEST_P(LCDTextTest, ContentsNotOpaque)
{
    ...
    CheckCanUseLCDText(LCDTextDisallowedReason::kContentsNotOpaque, "contents not opaque", layer_);
    CheckCanUseLCDText(LCDTextDisallowedReason::kNone, "descedant of contents not opaque", descendant_);

    // Non-opaque content and non-opaque background.
    layer_->SetBackgroundColor({ 1.0f, 1.0f, 1.0f, 0.5f });
    CheckCanUseLCDText(LCDTextDisallowedReason::kBackgroundColorNotOpaque, "background not opaque", layer_);
    CheckCanUseLCDText(LCDTextDisallowedReason::kNone, "descendant of contents not opaque", descendant_);
...
TEST_P(LCDTextTest, WillChangeTransform)
{
    ...
    CheckCanUseLCDText(LCDTextDisallowedReason::kWillChangeTransform, "will-change:transform");

    SetWillChangeTransform(layer_.get(), false);
    CheckCanUseLCDText(LCDTextDisallowedReason::kNone, "no will-change: transform");
}
...
TEST_P(LCDTextTest, FilterAnimation)
{
    ...
    SetFilter(layer_.get(), FilterOperations());
    CheckCanUseLCDText(LCDTextDisallowedReason::kPixelOrColorEffect, "filter animation");
...
TEST_P(LCDTextTest, BackdropFilter)
{
    ...
    UpdateDrawProperties(host_impl()->active_tree());
    CheckCanUseLCDText(LCDTextDisallowedReason::kPixelOrColorEffect, "backdrop-filter", layer_);
    CheckCanUseLCDText(LCDTextDisallowedReason::kNone, "backdrop-filter", descendant_);

    SetBackdropFilter(descendant_.get(), FilterOperations());
    UpdateDrawProperties(host_impl()->active_tree());
    CheckCanUseLCDText(LCDTextDisallowedReason::kNone, "no backdrop-filter", layer_);
}
...
TEST_P(LCDTextTest, BackdropFilterAnimation)
{
    ...
    GetEffectNode(descendant_.get())->has_potential_backdrop_filter_animation = true;
    SetBackdropFilter(descendant_.get(), FilterOperations());
    UpdateDrawProperties(host_impl()->active_tree());
    CheckCanUseLCDText(LCDTextDisallowedReason::kPixelOrColorEffect, "backdrop-filter animation", layer_);

    GetEffectNode(descendant_.get())->has_potential_backdrop_filter_animation = false;
    UpdateDrawProperties(host_impl()->active_tree());
    CheckCanUseLCDText(LCDTextDisallowedReason::kNone, "no backdrop-filter", layer_);
}
...
TEST_P(LCDTextTest, ContentsOpaqueForText)
{
    ...
    CheckCanUseLCDText(LCDTextDisallowedReason::kNone, "contents opaque for text", layer_);

    layer_->SetContentsOpaqueForText(false);
    CheckCanUseLCDText(LCDTextDisallowedReason::kContentsNotOpaque, "contents not opaque for text", layer_);
}
...
TEST_P(LCDTextTest, TransformAnimation)
{
    ...
    CheckCanUseLCDText(LCDTextDisallowedReason::kTransformAnimation, "transform animation");
