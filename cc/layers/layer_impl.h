...
public:
    ...
    {
        return draw_properties_.is_clipped;
    }

    LayerTreeImpl* layer_tree_impl() const
    {
        return layer_tree_impl_;
    }

    void PopulateSharedQuadState(viz::SharedQuadState* state, bool contents_opaque) const;
    ...
    void PopulateScaledSharedQuadState(viz::SharedQuadState* state, float layer_to_content_scale, bool contents_opaque) const;
    void PopulateScaledSharedQuadStateWithContentRects(viz::SharedQuadState* state, float layer_to_content_scale, const gfx::Rect& content_rect,
        const gfx::Rect& content_visible_rect, bool contents_opaque) const;

    // WillDraw must be called before AppendQuads. If WillDraw returns false,
    // AppendQuads and DidDraw will not be called. If WillDraw returns true,
    // DidDraw is guaranteed to be called before another WillDraw or before
    // the layer is destroyed.
    virtual bool WillDraw(DrawMode draw_mode, viz::ClientResourceProvider* resource_provider);
    virtual void AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
    {
    }
    ...
    virtual void GetContentsResourceId(viz::ResourceId* resource_id, gfx::Size* resource_size, gfx::SizeF* resource_uv_size) const;
    ...
    {
        return contents_opaque_for_text_;
    }
    ...
    bool Is3dSorted() const
    {
        return GetSortingContextId() != 0;
    }
    ...
    const DrawProperties& draw_properties() const
    {
        return draw_properties_;
    }
    ...
    // draw_properties_
    float draw_opacity() const
    {
        return draw_properties_.opacity;
    }
    ...
    {
        return draw_properties_.clip_rect;
    }
    ...
    {
        return rare_properties_ ? rare_properties_->main_thread_scroll_hit_test_region : Region::Empty();
    }

    void SetNonCompositedScrollHitTestRects(const std::vector<ScrollHitTestRect>& rects)
    {
    ...
    {
        return rare_properties_ ? &rare_properties_->non_composited_scroll_hit_test_rects : nullptr;
    }
    ...
    void SetWheelEventHandlerRegion(const Region& wheel_event_handler_region)
    {
        if (rare_properties_ || !wheel_event_handler_region.IsEmpty()) {
            EnsureRareProperties().wheel_event_handler_region = wheel_event_handler_region;
        }
    }
    const Region& wheel_event_handler_region() const
    {
        return rare_properties_ ? rare_properties_->wheel_event_handler_region : Region::Empty();
    }
    ...
    virtual void GetAllPrioritizedTilesForTracing(std::vector<PrioritizedTile>* prioritized_tiles) const;
    ...
    void set_may_contain_video(bool yes)
    {
    ...
    bool raster_even_if_not_drawn() const
    {
        return raster_even_if_not_drawn_;
    }
    ...
    virtual void NotifyKnownResourceIdsBeforeAppendQuads(const base::flat_set<viz::ViewTransitionElementResourceId>& known_resource_ids)
    {
    }
...
protected:
    ...
    LayerImpl(LayerTreeImpl* layer_impl, int id, bool will_always_push_properties = false);

    // Get the color and size of the layer's debug border.
    virtual void GetDebugBorderProperties(SkColor4f* color, float* width) const;

    void AppendDebugBorderQuad(viz::CompositorRenderPass* render_pass, const gfx::Rect& quad_rect, const viz::SharedQuadState* shared_quad_state,
        AppendQuadsData* append_quads_data) const;
    void AppendDebugBorderQuad(viz::CompositorRenderPass* render_pass, const gfx::Rect& quad_rect, const viz::SharedQuadState* shared_quad_state,
        AppendQuadsData* append_quads_data, SkColor4f color, float width) const;

    static float GetPreferredRasterScale(gfx::Vector2dF raster_space_scale_factor);
