...
protected:
    ...
    FakeLayerTreeHost* layer_tree_host()
    {
        return layer_impl_test_.host();
    }
...
TEST_F(UIResourceLayerTest, SetUIResourceId)
{
    ...
    std::unique_ptr<ScopedUIResource> resource
        = ScopedUIResource::Create(layer_tree_host()->GetUIResourceManager(), UIResourceBitmap(gfx::Size(10, 10), is_opaque));
    ...
    std::unique_ptr<ScopedUIResource> shared_resource
        = ScopedUIResource::Create(layer_tree_host()->GetUIResourceManager(), UIResourceBitmap(gfx::Size(5, 5), is_opaque));
