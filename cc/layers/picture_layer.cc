...
std::unique_ptr<LayerImpl> PictureLayer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
    return PictureLayerImpl::Create(tree_impl, id());
}

void PictureLayer::PushPropertiesTo(LayerImpl* base_layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
    ...
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "PictureLayer::PushPropertiesTo");
    DropRecordingSourceContentIfInvalid(base_layer->layer_tree_impl()->source_frame_number());

    layer_impl->set_gpu_raster_max_texture_size(commit_state.device_viewport_rect.size());
    layer_impl->SetIsBackdropFilterMask(is_backdrop_filter_mask());

    layer_impl->UpdateRasterSource(CreateRasterSource(), &last_updated_invalidation_.Write(*this));
...
void PictureLayer::SetLayerTreeHost(LayerTreeHost* host)
{
    ...
    recording_source_.Write(*this).SetSlowdownRasterScaleFactor(host->GetDebugState().slow_down_raster_scale_factor);
...
{
    update_source_frame_number_.Write(*this) = layer_tree_host()->SourceFrameNumber();
    ...
    recording_source.SetCanUseRecordedBounds(layer_tree_host()->GetSettings().enable_hit_test_opaqueness);

    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "PictureLayer::Update", "source_frame_number", layer_tree_host()->SourceFrameNumber());
    devtools_instrumentation::ScopedLayerTreeTask update_layer(devtools_instrumentation::kUpdateLayer, id(), layer_tree_host()->GetId());
    ...
    updated |= recording_source.Update(bounds(), layer_tree_host()->recording_scale_factor(), *client_, last_updated_invalidation_.Write(*this));
...
sk_sp<const SkPicture> PictureLayer::GetPicture() const
{
    ...
    const ScrollTree& scroll_tree = layer_tree_host()->property_trees()->scroll_tree();
    for (auto [element_id, _] : display_list->raster_inducing_scrolls()) {
        raster_inducing_scroll_offsets[element_id] = scroll_tree.current_scroll_offset(element_id);
    }
    display_list->Raster(canvas, /*image_provider=*/nullptr, &raster_inducing_scroll_offsets);
...
void PictureLayer::CaptureContent(const gfx::Rect& rect, std::vector<NodeInfo>* content) const
{
    if (!draws_content())
        return;
    ...
    if (!ScreenSpaceTransform().GetInverse(&inverse_screen_space_transform))
        return;
    gfx::Rect transformed = MathUtil::ProjectEnclosingClippedRect(inverse_screen_space_transform, rect);
    ...
    if (auto* outer_viewport_layer = layer_tree_host()->LayerByElementId(layer_tree_host()->OuterViewportScrollElementId())) {
        if (transform_tree_index() == outer_viewport_layer->transform_tree_index())
            return;
        gfx::Transform inverse_outer_screen_space_transform;
        if (!outer_viewport_layer->ScreenSpaceTransform().GetInverse(&inverse_outer_screen_space_transform)) {
            return;
        }
        gfx::Transform combined_transform = ScreenSpaceTransform() * inverse_outer_screen_space_transform;
        for (auto& i : *content) {
            i.visual_rect = MathUtil::ProjectEnclosingClippedRect(combined_transform, i.visual_rect);
        }
    }
}

void PictureLayer::DropRecordingSourceContentIfInvalid(int source_frame_number)
{
    ...
    // impl layer.
    DCHECK(update_source_frame_number_.Read(*this) != source_frame_number || layer_bounds == recording_source_size)
        << " bounds " << layer_bounds.ToString() << " recording source " << recording_source_size.ToString();

    if (update_source_frame_number_.Read(*this) != source_frame_number && recording_source_size != layer_bounds) {
