...
public:
    ...
    UIResourceId CreateUIResource(UIResourceClient* content) override
    {
        ...
        ui_resource_bitmap_map_.insert(std::make_pair(nid, content->GetBitmap(nid, false)));
        return nid;
    }
    ...
    size_t UIResourceCount()
    {
        return ui_resource_bitmap_map_.size();
    }
    int TotalUIResourceDeleted()
    {
        return total_ui_resource_deleted_;
    }
    int TotalUIResourceCreated()
    {
        return total_ui_resource_created_;
    }
...
public:
    explicit BaseScrollbarLayerTest(
        LayerTreeSettings::ScrollbarAnimator animator, bool enable_fluent_overlay_scrollbar = false, bool commit_to_active_tree = false)
    {
        ...
        layer_tree_settings_.enable_fluent_overlay_scrollbar = enable_fluent_overlay_scrollbar;
        ...
        std::unique_ptr<FakeResourceTrackingUIResourceManager> fake_ui_resource_manager = std::make_unique<FakeResourceTrackingUIResourceManager>();
        fake_ui_resource_manager_ = fake_ui_resource_manager.get();

        layer_tree_host_ = std::make_unique<FakeLayerTreeHost>(&fake_client_, std::move(params), CompositorMode::SINGLE_THREADED);
        layer_tree_host_->SetUIResourceManagerForTesting(std::move(fake_ui_resource_manager));
        layer_tree_host_->InitializeSingleThreaded(&single_thread_client_, base::SingleThreadTaskRunner::GetCurrentDefault());
        layer_tree_host_->SetVisible(true);
        fake_client_.SetLayerTreeHost(layer_tree_host_.get());
    }

protected:
    raw_ptr<FakeResourceTrackingUIResourceManager, DanglingUntriaged> fake_ui_resource_manager_;
...
class AuraScrollbarLayerTest : public BaseScrollbarLayerTest, public testing::WithParamInterface<bool> {
public:
    explicit AuraScrollbarLayerTest(bool enable_fluent_overlay_scrollbar = GetParam())
        : BaseScrollbarLayerTest(LayerTreeSettings::AURA_OVERLAY, enable_fluent_overlay_scrollbar)
    {
    }
};
...
TEST_F(ScrollbarLayerTest, RepaintNinePatchWhenResourceDisposed)
{
    ...
    scoped_refptr<NinePatchThumbScrollbarLayer> scrollbar_layer = NinePatchThumbScrollbarLayer::Create(fake_scrollbar);
...
TEST_F(ScrollbarLayerTest, SetNeedsDisplayDoesNotRequireUpdate_NonSolidColorThumb)
{
    scoped_refptr<Layer> layer_tree_root = Layer::Create();
    scoped_refptr<Layer> content_layer = Layer::Create();
    ...
    auto scrollbar_layer = base::MakeRefCounted<FakePaintedScrollbarLayer>(layer_tree_root->element_id(), std::move(scrollbar));
    ...
    scrollbar_layer->PushPropertiesTo(scrollbar_layer->CreateLayerImpl(layer_tree_host_->active_tree()).get(), *layer_tree_host_->GetPendingCommitState(),
...
TEST_F(ScrollbarLayerTest, SetNeedsDisplayDoesNotRequireUpdate_SolidColorThumb)
{
    scoped_refptr<Layer> layer_tree_root = Layer::Create();
    scoped_refptr<Layer> content_layer = Layer::Create();
    auto scrollbar = FakePaintedScrollbarLayer::CreateScrollbar();
...
TEST_F(CommitToActiveTreeScrollbarLayerTest, ScrollElementIdPushedAcrossCommit)
{
    scoped_refptr<Layer> layer_tree_root = Layer::Create();
    ...
    scoped_refptr<PaintedScrollbarLayer> painted_scrollbar_layer = PaintedScrollbarLayer::Create(base::MakeRefCounted<FakeScrollbar>());
    ...
    scoped_refptr<NinePatchThumbScrollbarLayer> nine_patch_thumb_scrollbar_layer
        = NinePatchThumbScrollbarLayer::Create(base::MakeRefCounted<FakeNinePatchScrollbar>());
    nine_patch_thumb_scrollbar_layer->SetScrollElementId(layer_a->element_id());
    scoped_refptr<SolidColorScrollbarLayer> solid_color_scrollbar_layer = SolidColorScrollbarLayer::Create(ScrollbarOrientation::kVertical, 1, 1, false);
    ...
    ScrollbarLayerImplBase* painted_scrollbar_layer_impl
        = static_cast<ScrollbarLayerImplBase*>(layer_impl_tree_root->layer_tree_impl()->LayerById(painted_scrollbar_layer->id()));
    ScrollbarLayerImplBase* nine_patch_thumb_scrollbar_layer_impl
        = static_cast<ScrollbarLayerImplBase*>(layer_impl_tree_root->layer_tree_impl()->LayerById(nine_patch_thumb_scrollbar_layer->id()));
    ScrollbarLayerImplBase* solid_color_scrollbar_layer_impl
        = static_cast<ScrollbarLayerImplBase*>(layer_impl_tree_root->layer_tree_impl()->LayerById(solid_color_scrollbar_layer->id()));

    ASSERT_EQ(painted_scrollbar_layer_impl->scroll_element_id_, layer_a->element_id());
    ASSERT_EQ(nine_patch_thumb_scrollbar_layer_impl->scroll_element_id_, layer_a->element_id());
    ASSERT_EQ(solid_color_scrollbar_layer_impl->scroll_element_id_, layer_a->element_id());
    ...
    std::unique_ptr<CommitState> commit_state = layer_tree_host_->WillCommit(std::move(completion_event_ptr),
        /*has_updates=*/true);
    {
        DebugScopedSetImplThread scoped_impl_thread(layer_tree_host_->GetTaskRunnerProvider());
    ...
    EXPECT_EQ(painted_scrollbar_layer_impl->scroll_element_id_, layer_b->element_id());
    EXPECT_EQ(nine_patch_thumb_scrollbar_layer_impl->scroll_element_id_, layer_b->element_id());
    EXPECT_EQ(solid_color_scrollbar_layer_impl->scroll_element_id_, layer_b->element_id());
}
...
TEST_F(CommitToActiveTreeScrollbarLayerTest, ScrollOffsetSynchronization)
{
    ...
    scoped_refptr<Layer> content_layer = Layer::Create();
    scoped_refptr<PaintedScrollbarLayer> scrollbar_layer = PaintedScrollbarLayer::Create(base::MakeRefCounted<FakeScrollbar>());
    ...
    ScrollbarLayerImplBase* cc_scrollbar_layer
        = static_cast<PaintedScrollbarLayerImpl*>(layer_impl_tree_root->layer_tree_impl()->LayerById(scrollbar_layer->id()));
...
#define UPDATE_AND_EXTRACT_LAYER_POINTERS()                                                                                                                    \
    do {                                                                                                                                                       \
        ...
        scrollbar_layer_impl = static_cast<decltype(scrollbar_layer_impl)>(root_layer_impl->layer_tree_impl()->LayerById(scrollbar_layer->id()));              \
    } while (false)

TEST_F(CommitToActiveTreeScrollbarLayerTest, UpdatePropertiesOfScrollBarWhenThumbRemoved)
{
    scoped_refptr<Layer> root_layer = Layer::Create();
    scoped_refptr<Layer> scroll_layer = Layer::Create();
    scoped_refptr<Layer> content_layer = Layer::Create();
    auto scrollbar_layer = base::MakeRefCounted<FakePaintedScrollbarLayer>(scroll_layer->element_id());
    ...
    EXPECT_EQ(gfx::Rect(10, 0, 4, 10).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());

    scrollbar_layer->fake_scrollbar()->set_has_thumb(false);

    UPDATE_AND_EXTRACT_LAYER_POINTERS();
    EXPECT_EQ(gfx::Rect(10, 5, 0, 0).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());
}
...
TEST_F(CommitToActiveTreeScrollbarLayerTest, NinePatchThumbRect)
{
    ...
    scroll_layer->SetElementId(LayerIdToElementIdForTesting(scroll_layer->id()));
    scoped_refptr<Layer> content_layer = Layer::Create();
    auto scrollbar_layer = base::MakeRefCounted<FakeNinePatchThumbScrollbarLayer>(scroll_layer->element_id());
    ...
    UPDATE_AND_EXTRACT_LAYER_POINTERS();
    EXPECT_EQ(gfx::Rect(56, 0, 4, 10).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());

    // Change thumb thickness and length.
    scrollbar_layer->fake_scrollbar()->set_thumb_size(gfx::Size(6, 4));

    UPDATE_AND_EXTRACT_LAYER_POINTERS();
    EXPECT_EQ(gfx::Rect(54, 0, 6, 4).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());
    ...
    UPDATE_AND_EXTRACT_LAYER_POINTERS();
    EXPECT_EQ(gfx::Rect(44, 0, 6, 4).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());
...
TEST_F(CommitToActiveTreeScrollbarLayerTest, PaintedThumbRect)
{
    ...
    UPDATE_AND_EXTRACT_LAYER_POINTERS();
    EXPECT_EQ(gfx::Rect(54, 3, 6, 4).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());
    ...
    UPDATE_AND_EXTRACT_LAYER_POINTERS();
    EXPECT_EQ(gfx::Rect(44, 3, 6, 4).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());
    ...
    UPDATE_AND_EXTRACT_LAYER_POINTERS();
    EXPECT_EQ(gfx::Rect(44, 1, 6, 4).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());
}

TEST_F(CommitToActiveTreeScrollbarLayerTest, ThumbRectForOverlayLeftSideVerticalScrollbar)
{
    scoped_refptr<Layer> root_layer = Layer::Create();
    scoped_refptr<Layer> scroll_layer = Layer::Create();
    ...
    auto scrollbar_layer = base::MakeRefCounted<FakeNinePatchThumbScrollbarLayer>(scroll_layer->element_id(), std::move(scrollbar));
    scroll_layer->SetScrollable(gfx::Size(20, 50));
    ...
    // position).
    UPDATE_AND_EXTRACT_LAYER_POINTERS();
    EXPECT_EQ(gfx::Rect(0, 0, 10, 4).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 5, 4).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());
    ...
    // thumb_thickness.
    EXPECT_EQ(gfx::Rect(6, 0, 2, 6).ToString(), scrollbar_layer_impl->ComputeThumbQuadRect().ToString());
}
...
TEST_F(CommitToActiveTreeScrollbarLayerTest, SolidColorDrawQuads)
{
    ...
    ScrollbarLayerImplBase* scrollbar_layer_impl = nullptr;
    {
        ...
        scoped_refptr<ScrollbarLayerBase> scrollbar_layer
            = SolidColorScrollbarLayer::Create(ScrollbarOrientation::kHorizontal, kThumbThickness, kTrackStart, false);
        root->AddChild(child);
        root->AddChild(scrollbar_layer);
        layer_tree_host_->SetRootLayer(root);
        layer_tree_host_->BuildPropertyTreesForTesting();
        auto* root_impl = layer_tree_host_->CommitToActiveTree();
        scrollbar_layer_impl = static_cast<SolidColorScrollbarLayerImpl*>(root_impl->layer_tree_impl()->LayerById(scrollbar_layer->id()));
    }
...
TEST_F(CommitToActiveTreeScrollbarLayerTest, LayerDrivenSolidColorDrawQuads)
{
    ...
    scoped_refptr<SolidColorScrollbarLayer> child2
        = SolidColorScrollbarLayer::Create(ScrollbarOrientation::kHorizontal, kThumbThickness, kTrackStart, kIsLeftSideVerticalScrollbar);
    ...
    auto* scrollbar_layer_impl = static_cast<ScrollbarLayerImplBase*>(scroll_layer_impl->layer_tree_impl()->LayerById(child2->id()));
...
TEST_F(ScrollbarLayerTest, ScrollbarLayerOpacity)
{
    ...
    const bool kIsLeftSideVerticalScrollbar = false;
    scrollbar_layer = SolidColorScrollbarLayer::Create(ScrollbarOrientation::kHorizontal, kThumbThickness, kTrackStart, kIsLeftSideVerticalScrollbar);
    scrollbar_layer->SetScrollElementId(scroll_layer->element_id());
    ...
    layer_tree_host_->UpdateLayers();
    const EffectNode* node = layer_tree_host_->property_trees()->effect_tree().Node(scrollbar_layer->effect_tree_index());
    ...
    node = layer_tree_impl->property_trees()->effect_tree().Node(scrollbar_layer->effect_tree_index());
    EXPECT_EQ(node->opacity, 0.f);
    ...
    ScrollbarLayerImplBase* scrollbar_layer_impl = static_cast<ScrollbarLayerImplBase*>(layer_tree_impl->LayerById(scrollbar_layer->id()));
    scrollbar_layer_impl->SetOverlayScrollbarLayerOpacityAnimated(0.25f, /*fade_out_animation=*/false);
    host_impl->CreatePendingTree();
...
TEST_P(AuraScrollbarLayerTest, ScrollbarLayerPushProperties)
{
    ...
    host_impl->ActivateSyncTree();
    EXPECT_TRUE(host_impl->ScrollbarAnimationControllerForElementId(scroll_layer->element_id()));
    ...
    layer_tree_host_->CommitToPendingTree();
    host_impl->ActivateSyncTree();
    const EffectNode* node = host_impl->active_tree()->property_trees()->effect_tree().Node(scrollbar_layer->effect_tree_index());
    // If Fluent overlay scrollbars are active, changing the bounds scrollable
    // content shouldn't make the scrollbars appear.
    EXPECT_EQ(node->opacity, layer_tree_settings_.enable_fluent_overlay_scrollbar ? 0.f : 1.f);
}
...
TEST_F(ScrollbarLayerTest, SubPixelCanScrollOrientation)
{
    ...
    SolidColorScrollbarLayerImpl* scrollbar_layer = impl.AddLayerInActiveTree<SolidColorScrollbarLayerImpl>(
        ScrollbarOrientation::kHorizontal, kThumbThickness, kTrackStart, kIsLeftSideVerticalScrollbar);
...
public:
    ScrollbarLayerSolidColorThumbTest()
    {
        ...
        host_impl_ = std::make_unique<FakeLayerTreeHostImpl>(layer_tree_settings, &task_runner_provider_, &task_graph_runner_);
        ...
        horizontal_scrollbar_layer_ = SolidColorScrollbarLayerImpl::Create(
            host_impl_->active_tree(), 1, ScrollbarOrientation::kHorizontal, kThumbThickness, kTrackStart, kIsLeftSideVerticalScrollbar);
        vertical_scrollbar_layer_ = SolidColorScrollbarLayerImpl::Create(
            host_impl_->active_tree(), 2, ScrollbarOrientation::kVertical, kThumbThickness, kTrackStart, kIsLeftSideVerticalScrollbar);
    }
...
TEST_F(ScrollbarLayerSolidColorThumbTest, SolidColorThumbVerticalAdjust)
{
    ...
    EXPECT_EQ(gfx::Rect(20, 0, 20, 3), horizontal_scrollbar_layer_->ComputeThumbQuadRect());
    EXPECT_EQ(gfx::Rect(0, 20, 3, 20), vertical_scrollbar_layer_->ComputeThumbQuadRect());
    ...
    EXPECT_EQ(gfx::Rect(20.f, 10.f, 20.f, 3.f), horizontal_scrollbar_layer_->ComputeThumbQuadRect());
    EXPECT_EQ(gfx::Rect(0.f, 22, 3.f, 22.f), vertical_scrollbar_layer_->ComputeThumbQuadRect());
}

class ScrollbarLayerTestResourceCreationAndRelease : public ScrollbarLayerTest {
public:
    void TestResourceUpload(int num_updates, size_t expected_resources, int expected_created, int expected_deleted, bool use_solid_color_scrollbar)
    {
        scoped_refptr<Layer> layer_tree_root = Layer::Create();
        scoped_refptr<Layer> content_layer = Layer::Create();
        scoped_refptr<ScrollbarLayerBase> scrollbar_layer;
        ...
        EXPECT_EQ(expected_created, fake_ui_resource_manager_->TotalUIResourceCreated());
        EXPECT_EQ(expected_deleted, fake_ui_resource_manager_->TotalUIResourceDeleted());
...
TEST_F(ScrollbarLayerTestResourceCreationAndRelease, ResourceUpload)
{
    ...
    for (int j = 0; j < 3; j++) {
        ...
        deleted = created - 2;
        TestResourceUpload(num_updates[j], 2, created, deleted, use_solid_color_scrollbars);
    }
}

TEST_F(ScrollbarLayerTestResourceCreationAndRelease, SolidColorNoResourceUpload)
{
...
TEST_F(ScrollbarLayerTestResourceCreationAndRelease, TestResourceUpdate)
{
    ...
    EXPECT_EQ(expected_created, fake_ui_resource_manager_->TotalUIResourceCreated());
    EXPECT_EQ(expected_deleted, fake_ui_resource_manager_->TotalUIResourceDeleted());
    EXPECT_EQ(gfx::Size(90, 15), fake_ui_resource_manager_->ui_resource_size(scrollbar_layer->track_and_buttons_resource_id()));
...
public:
    void TestResourceUpload(float test_scale)
    {
        ...
        scoped_refptr<Layer> layer_tree_root = Layer::Create();
        scoped_refptr<Layer> content_layer = Layer::Create();
        auto scrollbar_layer = base::MakeRefCounted<FakePaintedScrollbarLayer>(layer_tree_root->element_id());
        ...
        layer_tree_host_->SetViewportRectAndScale(layer_tree_host_->device_viewport_rect(), test_scale, layer_tree_host_->local_surface_id_from_parent());
        ...
        gfx::Size track_size = fake_ui_resource_manager_->ui_resource_size(scrollbar_layer->track_and_buttons_resource_id());
        gfx::Size thumb_size = fake_ui_resource_manager_->ui_resource_size(scrollbar_layer->thumb_resource_id());

        EXPECT_LE(track_size.width(), scrollbar_layer->internal_content_bounds().width());
        EXPECT_LE(track_size.height(), scrollbar_layer->internal_content_bounds().height());
        EXPECT_LE(thumb_size.width(), scrollbar_layer->internal_content_bounds().width());
        EXPECT_LE(thumb_size.height(), scrollbar_layer->internal_content_bounds().height());
    }
};
...
TEST_F(ScaledScrollbarLayerTestResourceCreation, ScaledResourceUpload)
{
    ...
    scoped_refptr<viz::TestContextProvider> context = viz::TestContextProvider::CreateRaster();
...
public:
    void TestScale(const gfx::Rect& scrollbar_rect, float test_scale)
    {
        ...
        scrollbar_layer->fake_scrollbar()->set_track_rect(gfx::Rect(scrollbar_rect.size()));
        ...
        UIResourceBitmap* bitmap = fake_ui_resource_manager_->ui_resource_bitmap(scrollbar_layer->track_and_buttons_resource_id());

        DCHECK(bitmap);

        const SkColor* pixels = reinterpret_cast<const SkColor*>(bitmap->GetPixels().data());
        SkColor color = argb_to_skia(scrollbar_layer->fake_scrollbar()->paint_fill_color());
        ...
        EXPECT_EQ(color, pixels[0]) << "Top left pixel doesn't match scrollbar color.";

        EXPECT_EQ(color, pixels[width - 1]) << "Top right pixel doesn't match scrollbar color.";

        EXPECT_EQ(color, pixels[width * (height - 1)]) << "Bottom left pixel doesn't match scrollbar color.";

        EXPECT_EQ(color, pixels[width * height - 1]) << "Bottom right pixel doesn't match scrollbar color.";
    }
