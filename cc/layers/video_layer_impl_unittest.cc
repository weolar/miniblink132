...
static void DebugSetImplThreadAndMainThreadBlocked(TaskRunnerProvider* task_runner_provider)
{
#if DCHECK_IS_ON()
...
TEST(VideoLayerImplTest, Occlusion)
{
    ...
    scoped_refptr<media::VideoFrame> video_frame
        = media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_I420, gfx::Size(10, 10), gfx::Rect(10, 10), gfx::Size(10, 10), base::TimeDelta());
    FakeVideoFrameProvider provider;
    provider.set_frame(video_frame);

    VideoLayerImpl* video_layer_impl = impl.AddLayerInActiveTree<VideoLayerImpl>(&provider, media::VIDEO_ROTATION_0);
...

    {
        ...
        VerifyQuadsExactlyCoverRect(impl.quad_list(), impl.quad_list().cbegin()->shared_quad_state->visible_quad_layer_rect);
...
TEST(VideoLayerImplTest, Rotated0)
{
    ...
    scoped_refptr<media::VideoFrame> video_frame
        = media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_I420, gfx::Size(20, 10), gfx::Rect(20, 10), gfx::Size(20, 10), base::TimeDelta());
    FakeVideoFrameProvider provider;
    provider.set_frame(video_frame);
    ...
    p1 = impl.quad_list().front()->shared_quad_state->quad_to_target_transform.MapPoint(p1);
    p2 = impl.quad_list().front()->shared_quad_state->quad_to_target_transform.MapPoint(p2);
...
TEST(VideoLayerImplTest, Rotated90)
{
    ...
    VideoLayerImpl* video_layer_impl = impl.AddLayerInActiveTree<VideoLayerImpl>(&provider, media::VIDEO_ROTATION_90);
    video_layer_impl->SetBounds(layer_size);
    video_layer_impl->SetDrawsContent(true);
    CopyProperties(impl.root_layer(), video_layer_impl);
...
TEST(VideoLayerImplTest, Rotated180)
{
    ...
    VideoLayerImpl* video_layer_impl = impl.AddLayerInActiveTree<VideoLayerImpl>(&provider, media::VIDEO_ROTATION_180);
    video_layer_impl->SetBounds(layer_size);
    video_layer_impl->SetDrawsContent(true);
    CopyProperties(impl.root_layer(), video_layer_impl);
    ...
    gfx::Point3F p1
        = impl.quad_list().front()->shared_quad_state->quad_to_target_transform.MapPoint(gfx::Point3F(0, impl.quad_list().front()->rect.height(), 0));
    gfx::Point3F p2
        = impl.quad_list().front()->shared_quad_state->quad_to_target_transform.MapPoint(gfx::Point3F(impl.quad_list().front()->rect.width(), 0, 0));
...
TEST(VideoLayerImplTest, Rotated270)
{
    ...
    VideoLayerImpl* video_layer_impl = impl.AddLayerInActiveTree<VideoLayerImpl>(&provider, media::VIDEO_ROTATION_270);
    video_layer_impl->SetBounds(layer_size);
    video_layer_impl->SetDrawsContent(true);
    CopyProperties(impl.root_layer(), video_layer_impl);
...
TEST(VideoLayerImplTest, SoftwareVideoFrameGeneratesYUVQuad)
{
    ...
    const auto* texture_draw_quad = static_cast<const viz::TextureDrawQuad*>(draw_quad);
...
TEST(VideoLayerImplTest, HibitSoftwareVideoFrameGeneratesYUVQuad)
{
    ...
    scoped_refptr<media::VideoFrame> video_frame
        = media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_YUV420P10, gfx::Size(20, 10), gfx::Rect(20, 10), gfx::Size(20, 10), base::TimeDelta());
...
TEST(VideoLayerImplTest, NativeYUVFrameGeneratesYUVQuad)
{
    ...
    scoped_refptr<media::VideoFrame> video_frame = media::VideoFrame::WrapSharedImage(media::PIXEL_FORMAT_I420, shared_image, gpu::SyncToken(),
        base::DoNothing(), gfx::Size(10, 10), gfx::Rect(10, 10), gfx::Size(10, 10), base::TimeDelta());
...
TEST(VideoLayerImplTest, NativeARGBFrameGeneratesTextureQuad)
{
    ...
    scoped_refptr<media::VideoFrame> video_frame = media::VideoFrame::WrapSharedImage(
        media::PIXEL_FORMAT_ARGB, shared_image, gpu::SyncToken(), base::DoNothing(), resource_size, gfx::Rect(10, 10), resource_size, base::TimeDelta());
    ASSERT_TRUE(video_frame);
    video_frame->metadata().allow_overlay = true;
    FakeVideoFrameProvider provider;
    provider.set_frame(video_frame);
    ...
    const viz::TextureDrawQuad* texture_draw_quad = viz::TextureDrawQuad::MaterialCast(draw_quad);
...
TEST(VideoLayerImplTest, GetDamageReasons)
{
    ...
    video_layer_impl->SetBounds(layer_size);
    EXPECT_EQ(video_layer_impl->GetDamageReasons(), DamageReasonSet { DamageReason::kUntracked });

    video_layer_impl->layer_tree_impl()->ResetAllChangeTracking();
    EXPECT_TRUE(video_layer_impl->GetDamageReasons().empty());
    video_layer_impl->SetNeedsRedraw();
    EXPECT_EQ(video_layer_impl->GetDamageReasons(), DamageReasonSet { DamageReason::kVideoLayer });
}
