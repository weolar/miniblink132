...
public:
    static std::unique_ptr<HeadsUpDisplayLayerImpl> Create(LayerTreeImpl* tree_impl, int id, const std::string& paused_localized_message)
    {
        return base::WrapUnique(new HeadsUpDisplayLayerImpl(tree_impl, id, paused_localized_message));
    }
    ...
    bool WillDraw(DrawMode draw_mode, viz::ClientResourceProvider* resource_provider) override;
    ...
    void UpdateHudTexture(DrawMode draw_mode, LayerTreeFrameSink* frame_sink, viz::ClientResourceProvider* resource_provider,
...
private:
    HeadsUpDisplayLayerImpl(LayerTreeImpl* tree_impl, int id, const std::string& paused_localized_message);
    ...
    void DrawText(PaintCanvas* canvas, const PaintFlags& flags, const std::string& text, TextAlign align, int size, int x, int y) const;
    void DrawText(PaintCanvas* canvas, const PaintFlags& flags, const std::string& text, TextAlign align, int size, const SkPoint& pos) const;
    void DrawGraphBackground(PaintCanvas* canvas, PaintFlags* flags, const SkRect& bounds) const;
    void DrawGraphLines(PaintCanvas* canvas, PaintFlags* flags, const SkRect& bounds) const;
    // Draw a separator line at top of bounds.
    void DrawSeparatorLine(PaintCanvas* canvas, PaintFlags* flags, const SkRect& bounds) const;

    SkRect DrawFrameThroughputDisplay(PaintCanvas* canvas, const DroppedFrameCounter* dropped_frame_counter, int right, int top) const;
    SkRect DrawMemoryDisplay(PaintCanvas* canvas, int top, int right, int width) const;
    SkRect DrawGpuRasterizationStatus(PaintCanvas* canvas, int right, int top, int width) const;
    void DrawDebugRect(PaintCanvas* canvas, PaintFlags* flags, const DebugRect& rect, SkColor4f stroke_color, SkColor4f fill_color, float stroke_width,
        const std::string& label_text) const;
    void DrawDebugRects(PaintCanvas* canvas, DebugRectHistory* debug_rect_history);
