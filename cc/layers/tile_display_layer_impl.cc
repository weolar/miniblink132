...
class TilingOrder {
public:
    bool operator()(const std::unique_ptr<TileDisplayLayerImpl::Tiling>& left, const std::unique_ptr<TileDisplayLayerImpl::Tiling>& right)
    {
...
TileDisplayLayerImpl::TileResource::TileResource(const viz::TransferableResource& resource, bool is_premultiplied, bool is_checkered)
    : resource(resource)
...
TileDisplayLayerImpl::TileResource& TileDisplayLayerImpl::TileResource::operator=(const TileResource&) = default;
...
TileDisplayLayerImpl::Tile& TileDisplayLayerImpl::Tile::operator=(Tile&&) = default;

TileDisplayLayerImpl::Tile::~Tile() = default;

TileDisplayLayerImpl::Tiling::Tiling(TileDisplayLayerImpl& layer, float scale_key)
    : layer_(layer)
    , scale_key_(scale_key)
{
}

TileDisplayLayerImpl::Tiling::~Tiling() = default;

TileDisplayLayerImpl::Tile* TileDisplayLayerImpl::Tiling::TileAt(const TileIndex& index) const
{
...
void TileDisplayLayerImpl::Tiling::SetRasterTransform(const gfx::AxisTransform2d& transform)
{
...
void TileDisplayLayerImpl::Tiling::SetTileContents(const TileIndex& key, const TileContents& contents)
{
...
TileDisplayLayerImpl::DisplayTilingCoverageIterator TileDisplayLayerImpl::Tiling::Cover(const gfx::Rect& coverage_rect, float coverage_scale) const
{
    return DisplayTilingCoverageIterator(this, coverage_scale, coverage_rect);
}

TileDisplayLayerImpl::TileDisplayLayerImpl(Client& client, LayerTreeImpl& tree, int id)
    : LayerImpl(&tree, id)
    , client_(client)
{
}
...
TileDisplayLayerImpl::Tiling& TileDisplayLayerImpl::GetOrCreateTilingFromScaleKey(float scale_key)
{
    auto it = std::find_if(tilings_.begin(), tilings_.end(), [scale_key](const auto& tiling) { return tiling->contents_scale_key() == scale_key; });
    if (it != tilings_.end()) {
        return **it;
    }
...
std::unique_ptr<LayerImpl> TileDisplayLayerImpl::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
    NOTREACHED();
}
...
void TileDisplayLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    if (tilings_.empty()) {
        return;
    }

    const float max_contents_scale = tilings_.empty() ? 1.0f : tilings_.back()->contents_scale_key();
    ...
    const Occlusion scaled_occlusion
        = draw_properties().occlusion_in_content_space.GetOcclusionWithGivenDrawTransform(shared_quad_state->quad_to_target_transform);
    ...
    if (const ScrollNode* scroll_node = scroll_tree.Node(scroll_tree_index())) {
        if (transform_tree_index() == scroll_node->transform_id) {
            if (const gfx::Rect* cull_rect = scroll_tree.ScrollingContentsCullRect(scroll_node->element_id)) {
                scaled_cull_rect = gfx::ScaleToEnclosingRect(*cull_rect, max_contents_scale);
            }
        }
    }
    ...
    for (auto iter = TilingSetCoverageIterator<Tiling>(tilings_, shared_quad_state->visible_quad_layer_rect, max_contents_scale, ideal_scale_key); iter;
         ++iter) {
        const gfx::Rect geometry_rect = iter.geometry_rect();
        const gfx::Rect visible_geometry_rect = scaled_occlusion.GetUnoccludedContentRect(geometry_rect);
        if (visible_geometry_rect.IsEmpty()) {
            continue;
        }

        const gfx::Rect offset_geometry_rect = geometry_rect + quad_offset;
        const gfx::Rect offset_visible_geometry_rect = visible_geometry_rect + quad_offset;
        const bool needs_blending = !contents_opaque();

        const uint64_t visible_geometry_area = visible_geometry_rect.size().Area64();
        ...
        if (*iter) {
            if (auto resource = iter->resource()) {
                ...
                quad->SetNew(shared_quad_state, offset_geometry_rect, offset_visible_geometry_rect, needs_blending, resource->resource.id, texture_rect,
                    iter.CurrentTiling()->tile_size(), resource->is_premultiplied, /*nearest_neighbor=*/false,
                ...
                if (alpha >= std::numeric_limits<float>::epsilon()) {
                    ...
                    quad->SetNew(shared_quad_state, offset_geometry_rect, offset_visible_geometry_rect, *color,
                        /*enable_edge_aa=*/false);
                }
            }
        }
