...
UIResourceLayerImpl* GenerateUIResourceLayer(
    FakeUIResourceLayerTreeHostImpl* host_impl, const gfx::Size& bitmap_size, const gfx::Size& layer_size, bool opaque, UIResourceId uid)
{
...
void QuadSizeTest(FakeUIResourceLayerTreeHostImpl* host_impl, UIResourceLayerImpl* layer, size_t expected_quad_size)
{
...
TEST(UIResourceLayerImplTest, VerifyDrawQuads)
{
    ...
    bool opaque = true;
    UIResourceId uid = 1;
    auto* layer = GenerateUIResourceLayer(&host_impl, bitmap_size, layer_size, opaque, uid);
    ...
    uid = 0;
    layer = GenerateUIResourceLayer(&host_impl, bitmap_size, layer_size, opaque, uid);
    QuadSizeTest(&host_impl, layer, expected_quad_size);
    host_impl.DeleteUIResource(uid);
}

void NeedsBlendingTest(FakeUIResourceLayerTreeHostImpl* host_impl, UIResourceLayerImpl* layer, bool needs_blending)
{
    auto render_pass = viz::CompositorRenderPass::Create();
    ...
    EXPECT_EQ(quads.front()->needs_blending, !quads.front()->shared_quad_state->are_contents_opaque);
...
TEST(UIResourceLayerImplTest, VerifySetOpaqueOnLayer)
{
    ...
    UIResourceId uid = 1;
    auto* layer = GenerateUIResourceLayer(&host_impl, bitmap_size, layer_size, skbitmap_opaque, uid);
    bool opaque = false;
    ...
    opaque = true;
    layer = GenerateUIResourceLayer(&host_impl, bitmap_size, layer_size, skbitmap_opaque, uid);
...
TEST(UIResourceLayerImplTest, Occlusion)
{
    ...
    UIResourceLayerImpl* ui_resource_layer_impl = impl.AddLayerInActiveTree<UIResourceLayerImpl>();
