...
LayerListIterator& LayerListIterator::operator++()
{
    ...
    for (Layer* parent = current_layer_->mutable_parent(); parent; parent = parent->mutable_parent()) {
...
LayerListConstIterator::LayerListConstIterator(const LayerListConstIterator& other) = default;
...
LayerListConstIterator& LayerListConstIterator::operator++()
{
    ...
    for (const Layer* parent = current_layer_->parent(); parent; parent = parent->parent()) {
        // We now try and advance in some list of siblings.
        // case 2: Advance to a sibling.
        if (list_indices_.back() + 1 < parent->children().size()) {
            ++list_indices_.back();
...
LayerListReverseIterator::LayerListReverseIterator(const LayerListReverseIterator& other) = default;
...
LayerListReverseIterator& LayerListReverseIterator::operator++()
{
    ...
    --list_indices_.back();
    this->current_layer_ = current_layer_->mutable_parent()->children()[list_indices_.back()].get();
    DescendToRightmostInSubtree();
    return *this;
}
...
LayerListReverseConstIterator::LayerListReverseConstIterator(const LayerListReverseConstIterator& other) = default;

LayerListReverseConstIterator::LayerListReverseConstIterator(const Layer* root_layer)
    : current_layer_(root_layer)
{
    DCHECK(!root_layer || !root_layer->parent());
    list_indices_.push_back(0);
...
LayerListReverseConstIterator& LayerListReverseConstIterator::operator++()
{
    ...
    CHECK(current_layer_->parent());
    --list_indices_.back();
    this->current_layer_ = current_layer_->parent()->children()[list_indices_.back()].get();
    DescendToRightmostInSubtree();
    return *this;
}
