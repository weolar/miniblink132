...
public:
    ...
    bool PaintTickmarksCalled()
    {
        return paint_tickmarks_called_;
    }
...
TEST(NinePatchThumbScrollbarLayerTest, PaintTickmarks)
{
    ...
    auto layer_tree_host = FakeLayerTreeHost::Create(&fake_client_, &task_graph_runner_, animation_host.get());

    auto scrollbar = base::MakeRefCounted<MockScrollbar>();
    scrollbar->set_has_tickmarks(false);

    scoped_refptr<NinePatchThumbScrollbarLayer> scrollbar_layer = NinePatchThumbScrollbarLayer::Create(scrollbar);
