...
class CC_EXPORT PictureLayerImpl : public LayerImpl, public PictureLayerTilingClient, public ImageAnimationController::AnimationDriver {
public:
    static std::unique_ptr<PictureLayerImpl> Create(LayerTreeImpl* tree_impl, int id)
    {
    ...
    void UpdateRasterSource(scoped_refptr<RasterSource> raster_source, Region* new_invalidation);
    bool UpdateTiles();

    // Mask-related functions.
    void GetContentsResourceId(viz::ResourceId* resource_id, gfx::Size* resource_size, gfx::SizeF* resource_uv_size) const override;
    ...
    PictureLayerTilingSet* picture_layer_tiling_set()
    {
        return tilings_.get();
    }
    ...
    RasterSource* GetRasterSource() const
    {
    ...
    const Region& InvalidationForTesting() const
    {
        return invalidation_;
    }

    // Set the paint result (PaintRecord) for a given PaintWorkletInput.
    void SetPaintWorkletRecord(scoped_refptr<const PaintWorkletInput>, PaintRecord);
    ...
    {
        ideal_contents_scale_ = raster_contents_scale_ = gfx::Vector2dF(scale, scale);
    }
...
protected:
    ...
    bool CanRecreateHighResTilingForLCDTextAndRasterTransform(const PictureLayerTiling& high_res) const;
    ...
    void AdjustRasterScaleForTransformAnimation(const gfx::Vector2dF& preserved_raster_contents_scale);
    ...
    void CleanUpTilingsOnActiveLayer(const std::vector<raw_ptr<PictureLayerTiling, VectorExperimental>>& used_tilings);
    ...
    void UpdateRasterSourceInternal(scoped_refptr<RasterSource> raster_source, Region* new_invalidation, const PictureLayerTilingSet* pending_set,
    ...
    void GetAllPrioritizedTilesForTracing(std::vector<PrioritizedTile>* prioritized_tiles) const override;
    ...
    void SetPaintWorkletInputs(const std::vector<DiscardableImageMap::PaintWorkletInputWithImageId>& inputs);

    LCDTextDisallowedReason ComputeLCDTextDisallowedReason(bool raster_translation_aligns_pixels) const;
    ...
    std::unique_ptr<PictureLayerTilingSet> tilings_ = CreatePictureLayerTilingSet();
    ...
    LCDTextDisallowedReason lcd_text_disallowed_reason_ = LCDTextDisallowedReason::kNoText;
    ...
    // exist.
    std::vector<raw_ptr<PictureLayerTiling, VectorExperimental>> last_append_quads_tilings_;
