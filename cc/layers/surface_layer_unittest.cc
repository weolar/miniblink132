...
public:
    ...
    void SynchronizeTrees()
    {
        ...
        std::unique_ptr<CommitState> commit_state = layer_tree_host_->ActivateCommitState();
        TreeSynchronizer::PushLayerProperties(*commit_state, unsafe_state, host_impl_.pending_tree());
        if (commit_state->needs_surface_ranges_sync) {
            host_impl_.pending_tree()->ClearSurfaceRanges();
            host_impl_.pending_tree()->SetSurfaceRanges(commit_state->SurfaceRanges());
        }
    }
...
protected:
    void SetUp() override
    {
        ...
        layer_tree_host_ = FakeLayerTreeHost::Create(&fake_client_, &task_graph_runner_, animation_host_.get());
...
TEST_F(SurfaceLayerTest, UseExistingDeadlineForNewSurfaceLayer)
{
    ...
    viz::SurfaceId primary_id(kArbitraryFrameSinkId, viz::LocalSurfaceId(1, base::UnguessableToken::Create()));
...
TEST_F(SurfaceLayerTest, ResetDeadlineOnInvalidSurfaceId)
{
    ...
    // is reset.
    layer->SetSurfaceId(viz::SurfaceId(), DeadlinePolicy::UseSpecifiedDeadline(3u));
    EXPECT_EQ(0u, layer->deadline_in_frames());
}
...
TEST_F(SurfaceLayerTest, PushProperties)
{
    ...
    EXPECT_TRUE(layer_tree_host_->GetPendingCommitState()->needs_surface_ranges_sync);
    EXPECT_EQ(layer_tree_host_->GetPendingCommitState()->SurfaceRanges().size(), 1u);
    ...
    std::unique_ptr<SurfaceLayerImpl> layer_impl = SurfaceLayerImpl::Create(host_impl_.pending_tree(), layer->id());
    SynchronizeTrees();
    ...
    EXPECT_FALSE(layer_tree_host_->GetPendingCommitState()->needs_surface_ranges_sync);
    ...
    viz::SurfaceId fallback_id(kArbitraryFrameSinkId, viz::LocalSurfaceId(2, base::UnguessableToken::Create()));
...
{
    const viz::SurfaceId old_surface_id(kArbitraryFrameSinkId, viz::LocalSurfaceId(1, base::UnguessableToken::Create()));
    ...
    std::unique_ptr<SurfaceLayerImpl> layer_impl1 = SurfaceLayerImpl::Create(host_impl_.pending_tree(), layer1->id());
    std::unique_ptr<SurfaceLayerImpl> layer_impl2 = SurfaceLayerImpl::Create(host_impl_.pending_tree(), layer2->id());

    SynchronizeTrees();

    // Verify that only |old_surface_id| is going to be referenced.
    EXPECT_THAT(layer_tree_host_->GetPendingCommitState()->SurfaceRanges(), ElementsAre(viz::SurfaceRange(old_surface_id)));
    EXPECT_THAT(host_impl_.pending_tree()->SurfaceRanges(), ElementsAre(viz::SurfaceRange(old_surface_id)));

    const viz::SurfaceId new_surface_id(kArbitraryFrameSinkId, viz::LocalSurfaceId(2, base::UnguessableToken::Create()));
    ...
    EXPECT_THAT(layer_tree_host_->GetPendingCommitState()->SurfaceRanges(), ElementsAre(viz::SurfaceRange(old_surface_id), viz::SurfaceRange(new_surface_id)));
    ...
    EXPECT_THAT(layer_tree_host_->GetPendingCommitState()->SurfaceRanges(), ElementsAre(viz::SurfaceRange(new_surface_id)));
...
{
    const viz::SurfaceId surface_id(kArbitraryFrameSinkId, viz::LocalSurfaceId(1, base::UnguessableToken::Create()));
    ...
    EXPECT_TRUE(layer_tree_host_->GetPendingCommitState()->needs_surface_ranges_sync);
    EXPECT_THAT(layer_tree_host_->GetPendingCommitState()->SurfaceRanges(), SizeIs(1));
    ...
    EXPECT_TRUE(layer_tree_host_->GetPendingCommitState()->needs_surface_ranges_sync);
    EXPECT_THAT(layer_tree_host_->GetPendingCommitState()->SurfaceRanges(), SizeIs(0));
}
