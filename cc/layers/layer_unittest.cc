...
#define EXPECT_CALL_MOCK_DELEGATE(obj, call) EXPECT_CALL((obj).mock_delegate(), call)

#define EXPECT_SET_NEEDS_UPDATE(expect, code_to_test)                                                                                                          \
    do {                                                                                                                                                       \
        EXPECT_CALL_MOCK_DELEGATE(*layer_tree_host_, SetNeedsUpdateLayers()).Times((expect));                                                                  \
        code_to_test;                                                                                                                                          \
...
#define EXPECT_SET_NEEDS_FULL_TREE_SYNC(expect, code_to_test)                                                                                                  \
    do {                                                                                                                                                       \
        EXPECT_CALL_MOCK_DELEGATE(*layer_tree_host_, SetNeedsFullTreeSync()).Times((expect));                                                                  \
        code_to_test;                                                                                                                                          \
        layer_tree_host_->VerifyAndClearExpectations();                                                                                                        \
    } while (false)
...
#define EXECUTE_AND_VERIFY_SUBTREE_CHANGED(code_to_test)                                                                                                       \
    ...
    EXPECT_TRUE(                                                                                                                                               \
        base::Contains(const_cast<const LayerTreeHost*>(top->layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, top.get()));      \
    EXPECT_TRUE(child->subtree_property_changed());                                                                                                            \
    EXPECT_TRUE(                                                                                                                                               \
        base::Contains(const_cast<const LayerTreeHost*>(child->layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, child.get()));  \
    EXPECT_TRUE(grand_child->subtree_property_changed());                                                                                                      \
    EXPECT_TRUE(base::Contains(                                                                                                                                \
        const_cast<const LayerTreeHost*>(grand_child->layer_tree_host())->pending_commit_state()->layers_that_should_push_properties, grand_child.get()));
...
#define EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(code_to_test)                                                                                                       \
    do {                                                                                                                                                       \
        EXPECT_CALL_MOCK_DELEGATE(*layer_tree_host_, SetNeedsCommit()).Times(AtLeast(1));                                                                      \
        code_to_test;                                                                                                                                          \
        layer_tree_host_->VerifyAndClearExpectations();                                                                                                        \
    } while (false)
...
static auto kArbitrarySourceId1 = base::UnguessableToken::CreateForTesting(0xdead, 0xbeef);
static auto kArbitrarySourceId2 = base::UnguessableToken::CreateForTesting(0xdead, 0xbee0);
...
public:
    FakeLayerTreeHost(LayerTreeHostSingleThreadClient* single_thread_client, LayerTreeHost::InitParams params)
    ...
    {
        InitializeSingleThreaded(single_thread_client, base::SingleThreadTaskRunner::GetCurrentDefault());
    }

    CommitState* GetPendingCommitState()
    {
...
public:
    LayerTest()
        : host_impl_(LayerTreeSettings(), &task_runner_provider_, &task_graph_runner_)
    {
...
protected:
    void SetUp() override
    {
        ...
        layer_tree_host_ = std::make_unique<FakeLayerTreeHost>(&single_thread_client_, std::move(params));
    }

    void TearDown() override
    {
        layer_tree_host_->VerifyAndClearExpectations();
        EXPECT_CALL_MOCK_DELEGATE(*layer_tree_host_, SetNeedsFullTreeSync()).Times(AnyNumber());
        parent_ = nullptr;
        child1_ = nullptr;
        child2_ = nullptr;
        child3_ = nullptr;
    ...
    void SimulateCommitForLayer(Layer* layer)
    {
        layer->PushPropertiesTo(
            layer->CreateLayerImpl(host_impl_.active_tree()).get(), *layer_tree_host_->GetPendingCommitState(), layer_tree_host_->GetThreadUnsafeCommitState());
    }
    ...
    void CommitAndPushProperties(Layer* layer, LayerImpl* layer_impl)
    {
        ...
        layer_tree_host_->CommitComplete(commit_state->source_frame_number, { base::TimeTicks(), base::TimeTicks::Now() });
    }
...
{
    EXPECT_CALL_MOCK_DELEGATE(*layer_tree_host_, SetNeedsFullTreeSync()).Times(AtLeast(1));
    ...
    EXPECT_SET_NEEDS_COMMIT_WAS_NOT_CALLED(mask_layer1->SetBounds(arbitrary_size));
    ...
    std::unique_ptr<LayerImpl> top_impl = LayerImpl::Create(host_impl_.active_tree(), top->id());
    ...
    std::unique_ptr<LayerImpl> mask_layer1_impl = mask_layer1->CreateLayerImpl(host_impl_.active_tree());

    host_impl_.active_tree()->set_source_frame_number(host_impl_.active_tree()->source_frame_number() + 1);

    auto& unsafe_state = layer_tree_host_->GetThreadUnsafeCommitState();
    layer_tree_host_->WillCommit(
        /*completion=*/nullptr, /*has_updates=*/true);
    EXECUTE_AND_VERIFY_SUBTREE_CHANGES_RESET(top->PushPropertiesTo(top_impl.get(), *commit_state, unsafe_state);
                                             ...
                                             grand_child->PushPropertiesTo(grand_child_impl.get(), *commit_state, unsafe_state);
                                             mask_layer1->PushPropertiesTo(mask_layer1_impl.get(), *commit_state, unsafe_state););
    layer_tree_host_->CommitComplete(commit_state->source_frame_number, { base::TimeTicks(), base::TimeTicks::Now() });
    ...
    EXPECT_CALL_MOCK_DELEGATE(*layer_tree_host_, SetNeedsCommit()).Times(2);
    EXECUTE_AND_VERIFY_SUBTREE_CHANGED(top->SetBackdropFilters(arbitrary_filters));
    ...
    TransformNode* node = layer_tree_host_->property_trees()->transform_tree_mutable().Node(top->transform_tree_index());
    EXPECT_TRUE(node->transform_changed);

    commit_state = layer_tree_host_->WillCommit(/*completion=*/nullptr,
        /*has_updates=*/true);
    EXECUTE_AND_VERIFY_SUBTREE_CHANGES_RESET(
        top->PushPropertiesTo(top_impl.get(), *commit_state, unsafe_state); child->PushPropertiesTo(child_impl.get(), *commit_state, unsafe_state);
        child2->PushPropertiesTo(child2_impl.get(), *commit_state, unsafe_state);
        grand_child->PushPropertiesTo(grand_child_impl.get(), *commit_state, unsafe_state); layer_tree_host_->property_trees()->ResetAllChangeTracking());
    layer_tree_host_->CommitComplete(commit_state->source_frame_number, { base::TimeTicks(), base::TimeTicks::Now() });
    ...
    node = layer_tree_host_->property_trees()->transform_tree_mutable().Node(child->transform_tree_index());
    EXPECT_TRUE(node->transform_changed);
    layer_tree_host_->VerifyAndClearExpectations();

    commit_state = layer_tree_host_->WillCommit(/*completion=*/nullptr,
        /*has_updates=*/true);
    EXECUTE_AND_VERIFY_SUBTREE_CHANGES_RESET(child->PushPropertiesTo(child_impl.get(), *commit_state, unsafe_state);
                                             grand_child->PushPropertiesTo(grand_child_impl.get(), *commit_state, unsafe_state);
    ...
    node = layer_tree_host_->property_trees()->transform_tree_mutable().Node(top->transform_tree_index());
    EXPECT_TRUE(node->transform_changed);
    layer_tree_host_->VerifyAndClearExpectations();
...
TEST_F(LayerTest, ReorderChildren)
{
    ...
    layer_tree_host_->GetPendingCommitState()->layers_that_should_push_properties.clear();
    ...
    for (const auto& child : parent->children()) {
        EXPECT_FALSE(base::Contains(layer_tree_host_->GetPendingCommitState()->layers_that_should_push_properties, child.get()));
        EXPECT_TRUE(child->subtree_property_changed());
    }
}
...
TEST_F(LayerTest, ReplaceChildWithNewChild)
{
    ...
    EXPECT_SET_NEEDS_FULL_TREE_SYNC(AtLeast(1), parent_->ReplaceChild(child2_.get(), child4));
...
TEST_F(LayerTest, CheckSetNeedsDisplayCausesCorrectBehavior)
{
    ...
    scoped_refptr<Layer> test_layer = Layer::Create();
    EXPECT_SET_NEEDS_FULL_TREE_SYNC(1, layer_tree_host_->SetRootLayer(test_layer));
    ...
    EXPECT_FALSE(LayerNeedsDisplay(test_layer.get()));
    EXPECT_SET_NEEDS_UPDATE(1, test_layer->SetNeedsDisplayRect(out_of_bounds_dirty_rect));
    ...
    EXPECT_FALSE(LayerNeedsDisplay(test_layer.get()));
    EXPECT_SET_NEEDS_COMMIT_WAS_NOT_CALLED(test_layer->SetNeedsDisplayRect(gfx::Rect()));
    EXPECT_FALSE(LayerNeedsDisplay(test_layer.get()));
    SimulateCommitForLayer(test_layer.get());
...
TEST_F(LayerTest, CheckPropertyChangeCausesCorrectBehavior)
{
    ...
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetTransformOrigin(gfx::Point3F(1.23f, 4.56f, 0.f)));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetBackgroundColor(SkColors::kLtGray));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetMasksToBounds(true));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetClipRect(gfx::Rect(1, 2, 3, 4)));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetRoundedCorner({ 1, 2, 3, 4 }));
    ...
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetBlendMode(SkBlendMode::kHue));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetContentsOpaque(true));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetPosition(gfx::PointF(4.f, 9.f)));
    // We can use any layer pointer here since we aren't syncing for real.
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetScrollable(gfx::Size(1, 1)));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetScrollOffset(gfx::PointF(10, 10)));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetMainThreadScrollHitTestRegion(Region(gfx::Rect(1, 1, 2, 2))));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetTransform(gfx::Transform::MakeScale(0.0)));
    TouchActionRegion touch_action_region;
    touch_action_region.Union(TouchAction::kNone, gfx::Rect(10, 10));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetTouchActionRegion(std::move(touch_action_region)));
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetForceRenderSurfaceForTesting(true));
    ...
    EXPECT_SET_NEEDS_COMMIT_WAS_CALLED(test_layer->SetCaptureBounds(
        viz::RegionCaptureBounds(base::flat_map<viz::RegionCaptureCropId, gfx::Rect> { { viz::RegionCaptureCropId(123u, 456u), gfx::Rect(0, 0, 640, 480) } })));
    EXPECT_CALL_MOCK_DELEGATE(*layer_tree_host_, SetNeedsCommit()).Times(1);
...
public:
    ...
    std::unique_ptr<LayerTreeHost> Create(LayerTreeSettings settings, MutatorHost* mutator_host)
    {
        LayerTreeHost::InitParams params;
        ...
        return LayerTreeHost::CreateSingleThreaded(&single_thread_client_, std::move(params));
    }
...
TEST_F(LayerLayerTreeHostTest, ChangeHost)
{
    ...
    AssertLayerTreeHostMatchesForSubtree(parent.get(), first_layer_tree_host.get());
    ...
    AssertLayerTreeHostMatchesForSubtree(parent.get(), second_layer_tree_host.get());
...
TEST_F(LayerLayerTreeHostTest, ChangeHostInSubtree)
{
    ...
    AssertLayerTreeHostMatchesForSubtree(first_parent.get(), first_layer_tree_host.get());
    ...
    EXPECT_EQ(second_layer_tree_host.get(), second_grand_child->layer_tree_host());
    ...
    for (int contents_opaque = 0; contents_opaque < 2; ++contents_opaque) {
        for (int layer_opaque = 0; layer_opaque < 2; ++layer_opaque) {
            for (int host_opaque = 0; host_opaque < 2; ++host_opaque) {
                ...
                layer->SetBackgroundColor(layer_opaque ? SkColors::kRed : SkColors::kTransparent);
                layer_tree_host->set_background_color(host_opaque ? SkColors::kRed : SkColors::kTransparent);

                layer_tree_host->property_trees()->set_needs_rebuild(true);
                layer_tree_host->BuildPropertyTreesForTesting();
                EXPECT_EQ(contents_opaque, layer->SafeOpaqueBackgroundColor().isOpaque())
                    << "Flags: " << contents_opaque << ", " << layer_opaque << ", " << host_opaque << "\n";
            }
        }
    }
}
...
TEST_F(LayerTest, PushUpdatesShouldHitTest)
{
    scoped_refptr<Layer> root_layer = Layer::Create();
    std::unique_ptr<LayerImpl> impl_layer = LayerImpl::Create(host_impl_.active_tree(), 1);
    EXPECT_SET_NEEDS_FULL_TREE_SYNC(1, layer_tree_host_->SetRootLayer(root_layer));
...
void ReceiveCopyOutputResult(int* result_count, std::unique_ptr<viz::CopyOutputResult> result)
{
    ++(*result_count);
}

void ReceiveCopyOutputResultAtomic(std::atomic<int>* result_count, std::unique_ptr<viz::CopyOutputResult> result)
{
    ++(*result_count);
}
...
TEST_F(LayerTest, DedupesCopyOutputRequestsBySource)
{
    ...
    std::unique_ptr<viz::CopyOutputRequest> request = std::make_unique<viz::CopyOutputRequest>(viz::CopyOutputRequest::ResultFormat::RGBA,
        viz::CopyOutputRequest::ResultDestination::kSystemMemory, base::BindOnce(&ReceiveCopyOutputResultAtomic, base::Unretained(&result_count)));
    ...
    request = std::make_unique<viz::CopyOutputRequest>(viz::CopyOutputRequest::ResultFormat::RGBA, viz::CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(&ReceiveCopyOutputResultAtomic, base::Unretained(&result_count)));
    layer->RequestCopyOfOutput(std::move(request));
    // Because RequestCopyOfOutput could run as a PostTask to return results
    // RunUntilIdle() to ensure that the result is not returned yet.
    CCTestSuite::RunUntilIdle();
    ...
    request = std::make_unique<viz::CopyOutputRequest>(viz::CopyOutputRequest::ResultFormat::RGBA, viz::CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(&ReceiveCopyOutputResult, &did_receive_first_result_from_this_source));
    ...
    request = std::make_unique<viz::CopyOutputRequest>(viz::CopyOutputRequest::ResultFormat::RGBA, viz::CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(&ReceiveCopyOutputResult, &did_receive_result_from_different_source));
    ...
    request = std::make_unique<viz::CopyOutputRequest>(viz::CopyOutputRequest::ResultFormat::RGBA, viz::CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(&ReceiveCopyOutputResult, &did_receive_second_result_from_this_source));
    request->set_source(kArbitrarySourceId1);
    layer->RequestCopyOfOutput(std::move(request)); // First request to be aborted.
    // Wait for any posted tasks to run so the results will be returned.
    CCTestSuite::RunUntilIdle();
...
TEST_F(LayerTest, AnimationSchedulesLayerUpdate)
{
    ...
    layer_tree_host_->SetElementOpacityMutated(element_id, ElementListType::ACTIVE, 0.5f);
    layer_tree_host_->VerifyAndClearExpectations();

    EXPECT_CALL_MOCK_DELEGATE(*layer_tree_host_, SetNeedsUpdateLayers());
    gfx::Transform transform;
    transform.Rotate(45.0);
    layer_tree_host_->SetElementTransformMutated(element_id, ElementListType::ACTIVE, transform);
    layer_tree_host_->VerifyAndClearExpectations();
    ...
    layer_tree_host_->SetElementScrollOffsetMutated(element_id, ElementListType::ACTIVE, gfx::PointF(10, 10));
    layer_tree_host_->VerifyAndClearExpectations();
}
...
TEST_F(LayerTest, SetLayerTreeHostNotUsingLayerListsManagesElementId)
{
    ...
    AddOpacityTransitionToElementWithAnimation(element_id, timeline, 10.0, 1.f, 0.f, false);
...
TEST_F(LayerTest, PushAnimationCountsLazily)
{
    ...
    animation_host_->PushPropertiesTo(host_impl_.animation_host(), *layer_tree_host_->property_trees());
...
TEST_F(LayerTest, UpdateMirrorCount)
{
    ...
    EXPECT_EQ(0u, layer_tree_host_->GetPendingCommitState()->layers_that_should_push_properties.size());
    ...
    EXPECT_TRUE(base::Contains(layer_tree_host_->GetPendingCommitState()->layers_that_should_push_properties, test_layer.get()));
...
TEST_F(LayerTest, UpdatingCaptureBounds)
{
    ...
    static const viz::RegionCaptureBounds kPopulatedBounds(
        base::flat_map<viz::RegionCaptureCropId, gfx::Rect> { { viz::RegionCaptureCropId(123u, 456u), gfx::Rect(0, 0, 640, 480) } });
    static const viz::RegionCaptureBounds kUpdatedBounds(
        base::flat_map<viz::RegionCaptureCropId, gfx::Rect> { { viz::RegionCaptureCropId(123u, 456u), gfx::Rect(0, 0, 1280, 720) } });
...
TEST_F(LayerTest, UpdatingClipRect)
{
    ...
    root->layer_tree_host()->BuildPropertyTreesForTesting();
    const ClipNode* node_1 = layer_tree_host_->property_trees()->clip_tree().Node(clipped_1->clip_tree_index());
    const ClipNode* node_2 = layer_tree_host_->property_trees()->clip_tree().Node(clipped_2->clip_tree_index());
    const ClipNode* node_3 = layer_tree_host_->property_trees()->clip_tree().Node(clipped_3->clip_tree_index());
    const ClipNode* node_4 = layer_tree_host_->property_trees()->clip_tree().Node(clipped_4->clip_tree_index());
    ...
    move_pixel_filters.Append(FilterOperation::CreateBlurFilter(2, SkTileMode::kClamp));
    ...
    node_1 = layer_tree_host_->property_trees()->clip_tree().Node(clipped_1->clip_tree_index());
    node_2 = layer_tree_host_->property_trees()->clip_tree().Node(clipped_2->clip_tree_index());
    node_3 = layer_tree_host_->property_trees()->clip_tree().Node(clipped_3->clip_tree_index());
    node_4 = layer_tree_host_->property_trees()->clip_tree().Node(clipped_4->clip_tree_index());

    EXPECT_EQ(node_1->clip, gfx::IntersectRects(gfx::RectF(kUpdatedClipRect_1), gfx::RectF(gfx::SizeF(kLayerSize))) + kParentOffset);
    EXPECT_EQ(node_2->clip, gfx::IntersectRects(gfx::RectF(kUpdatedClipRect_2), gfx::RectF(gfx::SizeF(kLayerSize))) + kParentOffset);
    EXPECT_EQ(node_3->clip, gfx::IntersectRects(gfx::RectF(kUpdatedClipRect_3), gfx::RectF(gfx::SizeF(kLayerSize))) + kParentOffset);
...
TEST_F(LayerTest, UpdatingRoundedCorners)
{
    ...
    root->layer_tree_host()->BuildPropertyTreesForTesting();
    const EffectNode* node_1 = layer_tree_host_->property_trees()->effect_tree().Node(layer_1->effect_tree_index());
    const EffectNode* node_2 = layer_tree_host_->property_trees()->effect_tree().Node(layer_2->effect_tree_index());
    const EffectNode* node_3 = layer_tree_host_->property_trees()->effect_tree().Node(layer_3->effect_tree_index());
    const EffectNode* node_4 = layer_tree_host_->property_trees()->effect_tree().Node(layer_4->effect_tree_index());
    const EffectNode* node_5 = layer_tree_host_->property_trees()->effect_tree().Node(layer_5->effect_tree_index());

    EXPECT_EQ(gfx::RRectF(gfx::RectF(kClipRect), kRoundedCorners), node_1->mask_filter_info.rounded_corner_bounds());
    ...
    EXPECT_EQ(gfx::RRectF(gfx::RectF(kClipRect), kRoundedCorners), node_4->mask_filter_info.rounded_corner_bounds());
    EXPECT_EQ(gfx::RRectF(gfx::RectF(gfx::Rect(kLayerSize)), kRoundedCorners), node_5->mask_filter_info.rounded_corner_bounds());
    ...
    node_1 = layer_tree_host_->property_trees()->effect_tree().Node(layer_1->effect_tree_index());
    node_2 = layer_tree_host_->property_trees()->effect_tree().Node(layer_2->effect_tree_index());
    node_3 = layer_tree_host_->property_trees()->effect_tree().Node(layer_3->effect_tree_index());
    node_4 = layer_tree_host_->property_trees()->effect_tree().Node(layer_4->effect_tree_index());
    node_5 = layer_tree_host_->property_trees()->effect_tree().Node(layer_5->effect_tree_index());

    EXPECT_EQ(gfx::RRectF(gfx::RectF(gfx::IntersectRects(gfx::Rect(kLayerSize), kClipRect)), kUpdatedRoundedCorners),
    ...
    EXPECT_EQ(gfx::RRectF(gfx::RectF(kUpdatedClipRect), kRoundedCorners), node_4->mask_filter_info.rounded_corner_bounds());
    EXPECT_EQ(gfx::RRectF(gfx::RectF(gfx::Rect(kLayerSize)), kUpdatedRoundedCorners), node_5->mask_filter_info.rounded_corner_bounds());
}
