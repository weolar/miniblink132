...
TEST(PictureLayerTest, NoTilesIfEmptyBounds)
{
    ...
    host->CommitComplete(host->SourceFrameNumber(), { base::TimeTicks(), base::TimeTicks::Now() });
    EXPECT_EQ(1, host->SourceFrameNumber());
    ...
    std::unique_ptr<FakeLayerTreeFrameSink> layer_tree_frame_sink = FakeLayerTreeFrameSink::CreateSoftware();
    ...
    layer->PushPropertiesTo(layer_impl.get(), *host->GetPendingCommitState(), host->GetThreadUnsafeCommitState());
...
TEST(PictureLayerTest, InvalidateRasterAfterUpdate)
{
    ...
    host_impl.InitializeFrameSink(layer_tree_frame_sink.get());
    host_impl.CreatePendingTree();
    host_impl.pending_tree()->SetRootLayerForTesting(FakePictureLayerImpl::Create(host_impl.pending_tree(), 1));
    FakePictureLayerImpl* layer_impl = static_cast<FakePictureLayerImpl*>(host_impl.pending_tree()->root_layer());
    layer->PushPropertiesTo(layer_impl, *host->GetPendingCommitState(), host->GetThreadUnsafeCommitState());

    EXPECT_EQ(invalidation_bounds, layer_impl->GetPendingInvalidation()->bounds());
}
...
TEST(PictureLayerTest, ClearVisibleRectWhenNoTiling)
{
    ...
    std::unique_ptr<LayerTreeFrameSink> layer_tree_frame_sink(FakeLayerTreeFrameSink::Create3d());
    FakeLayerTreeHostImpl host_impl(CommitToPendingTreeLayerListSettings(), &impl_task_runner_provider, &task_graph_runner);
    host_impl.SetVisible(true);
    ...
    std::unique_ptr<CommitState> commit_state = host->WillCommit(/*completion=*/nullptr, /*has_updates=*/true);
    layer->PushPropertiesTo(layer_impl, *commit_state, unsafe_state);
    host->CommitComplete(commit_state->source_frame_number, { base::TimeTicks(), base::TimeTicks::Now() });
    ...
    host_impl.CreatePendingTree();
    layer_impl = static_cast<FakePictureLayerImpl*>(host_impl.pending_tree()->root_layer());
...
TEST(PictureLayerTest, NonMonotonicSourceFrameNumber)
{
    ...
    std::unique_ptr<LayerTreeHost> host1 = LayerTreeHost::CreateSingleThreaded(&single_thread_client, std::move(params));
    ...
    std::unique_ptr<LayerTreeHost> host2 = LayerTreeHost::CreateSingleThreaded(&single_thread_client, std::move(params2));
...
TEST(PictureLayerTest, RecordingScaleIsCorrectlySet)
{
    ...
    client.add_draw_rect(gfx::ScaleToEnclosingRect(gfx::Rect(layer_bounds), recording_scale), solid_flags);

    // Add 1 pixel of non solid color.
    SkColor4f non_solid_color { 0.25f, 0.3f, 0.35f, 0.5f };
    PaintFlags non_solid_flags;
    non_solid_flags.setColor(non_solid_color);
    client.add_draw_rect(gfx::Rect(std::round(390 * recording_scale), std::round(390 * recording_scale), 1, 1), non_solid_flags);
