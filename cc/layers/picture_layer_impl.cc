...
gfx::Rect SafeIntersectRects(const gfx::Rect& one, const gfx::Rect& two)
{
    ...
    int64_t rr = std::min(static_cast<int64_t>(one.x()) + one.width(), static_cast<int64_t>(two.x()) + two.width());
    int64_t rb = std::min(static_cast<int64_t>(one.y()) + one.height(), static_cast<int64_t>(two.y()) + two.height());
    if (rx > rr || ry > rb)
        return gfx::Rect();
    return gfx::Rect(rx, ry, static_cast<int>(rr - rx), static_cast<int>(rb - ry));
}

} // namespace

PictureLayerImpl::PictureLayerImpl(LayerTreeImpl* tree_impl, int id)
    : LayerImpl(tree_impl, id, tree_impl->always_push_properties_on_picture_layers())
{
...
PictureLayerImpl::~PictureLayerImpl()
{
    ...
    if (!layer_tree_impl()->IsActiveTree()) {
        layer_tree_impl()->paint_worklet_tracker().UpdatePaintWorkletInputProperties({}, this);
    }
...
std::unique_ptr<LayerImpl> PictureLayerImpl::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
void PictureLayerImpl::PushPropertiesTo(LayerImpl* base_layer)
{
    ...
    layer_impl->has_non_animated_image_update_rect_ = has_non_animated_image_update_rect_;
    ...
    DCHECK_LE(tilings_->num_tilings(), layer_tree_impl()->create_low_res_tiling() ? 2u : 1u);

    layer_impl->set_gpu_raster_max_texture_size(gpu_raster_max_texture_size_);
    layer_impl->UpdateRasterSourceInternal(raster_source_, &invalidation_, tilings_.get(), &paint_worklet_records_, discardable_image_map_.get());
...
void PictureLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    ...
    if (raster_source_->IsSolidColor()) {
        ...
        PopulateScaledSharedQuadState(shared_quad_state, max_contents_scale, contents_opaque());

        AppendDebugBorderQuad(render_pass, gfx::Rect(bounds()), shared_quad_state, append_quads_data);

        gfx::Rect scaled_visible_layer_rect = shared_quad_state->visible_quad_layer_rect;
        Occlusion occlusion = draw_properties().occlusion_in_content_space;

        EffectNode* effect_node = GetEffectTree().Node(effect_tree_index());
        SolidColorLayerImpl::AppendSolidQuads(render_pass, occlusion, shared_quad_state, scaled_visible_layer_rect, raster_source_->GetSolidColor(),
            !layer_tree_impl()->settings().enable_edge_anti_aliasing, effect_node->blend_mode, append_quads_data);
        return;
    }
    ...
    float max_contents_scale = tilings_->num_tilings() ? MaximumTilingContentsScale() : 1.f;
    PopulateScaledSharedQuadState(shared_quad_state, max_contents_scale, contents_opaque());
    ...
    if (IsDirectlyCompositedImage()) {
        ...
        gfx::Rect bounds_in_target_space = MathUtil::MapEnclosingClippedRect(draw_properties().target_space_transform, gfx::Rect(bounds()));
        if (is_clipped())
    ...
    Occlusion scaled_occlusion = draw_properties().occlusion_in_content_space.GetOcclusionWithGivenDrawTransform(shared_quad_state->quad_to_target_transform);

    if (current_draw_mode_ == DRAW_MODE_RESOURCELESS_SOFTWARE) {
        DCHECK(shared_quad_state->quad_layer_rect.origin() == gfx::Point(0, 0));
        AppendDebugBorderQuad(render_pass, shared_quad_state->quad_layer_rect, shared_quad_state, append_quads_data, DebugColors::DirectPictureBorderColor(),
        ...
        gfx::Rect scaled_recorded_bounds = gfx::ScaleToEnclosingRect(raster_source_->recorded_bounds(), max_contents_scale);
        ...
        WhichTree tree = layer_tree_impl()->IsPendingTree() ? WhichTree::PENDING_TREE : WhichTree::ACTIVE_TREE;
        for (const auto& image_data : discardable_image_map_->animated_images_metadata()) {
            image_animation_map[image_data.paint_image_id] = controller->GetFrameIndexForImage(image_data.paint_image_id, tree);
        }

        auto* quad = render_pass->CreateAndAppendDrawQuad<viz::PictureDrawQuad>();
        quad->SetNew(shared_quad_state, geometry_rect, visible_geometry_rect, needs_blending, texture_rect, texture_size, nearest_neighbor_, quad_content_rect,
            max_contents_scale, std::move(image_animation_map), raster_source_->GetDisplayItemList(), GetRasterInducingScrollOffsets());
    ...
    AppendDebugBorderQuad(render_pass, debug_border_rect, shared_quad_state, append_quads_data);

    if (ShowDebugBorders(DebugBorderType::LAYER)) {
        for (auto iter = tilings_->Cover(shared_quad_state->visible_quad_layer_rect, max_contents_scale, ideal_contents_scale_key()); iter; ++iter) {
            SkColor4f color;
            float width;
            ...
            auto* debug_border_quad = render_pass->CreateAndAppendDrawQuad<viz::DebugBorderDrawQuad>();
            gfx::Rect geometry_rect = iter.geometry_rect();
            geometry_rect.Offset(quad_offset);
            gfx::Rect visible_geometry_rect = geometry_rect;
            debug_border_quad->SetNew(shared_quad_state, geometry_rect, visible_geometry_rect, color, width);
        }
    }

    if (layer_tree_impl()->debug_state().highlight_non_lcd_text_layers) {
        SkColor4f color = DebugColors::NonLCDTextHighlightColor(lcd_text_disallowed_reason());
        if (color != SkColors::kTransparent && GetRasterSource()->GetDisplayItemList()->AreaOfDrawText(gfx::Rect(bounds()))) {
            render_pass->CreateAndAppendDrawQuad<viz::SolidColorDrawQuad>()->SetNew(
                shared_quad_state, debug_border_rect, debug_border_rect, color, append_quads_data);
        }
    }
    ...
    gfx::Rect scaled_viewport_for_tile_priority = gfx::ScaleToEnclosingRect(viewport_rect_for_tile_priority_in_content_space_, max_contents_scale);

    std::optional<gfx::Rect> scaled_cull_rect;
    const ScrollTree& scroll_tree = layer_tree_impl()->property_trees()->scroll_tree();
    if (const ScrollNode* scroll_node = scroll_tree.Node(scroll_tree_index())) {
        if (transform_tree_index() == scroll_node->transform_id) {
            if (const gfx::Rect* cull_rect = scroll_tree.ScrollingContentsCullRect(scroll_node->element_id)) {
                scaled_cull_rect = gfx::ToEnclosingRect(gfx::ScaleRect(
                    // Convert into layer space.
                    gfx::RectF(*cull_rect) - offset_to_transform_parent(), max_contents_scale));
            }
        }
    }
    ...
    if (const auto& display_list = raster_source_->GetDisplayItemList()) {
        for (auto& [element_id, info] : display_list->raster_inducing_scrolls()) {
            ...
            if (const gfx::Rect* cull_rect = scroll_tree.ScrollingContentsCullRect(element_id)) {
                if (const auto* scroll_node = scroll_tree.FindNodeFromElementId(element_id)) {
                    gfx::RectF visible_rect(gfx::Rect(scroll_node->container_origin, scroll_tree.container_bounds(scroll_node->id)));
                    visible_rect.Offset(scroll_tree.current_scroll_offset(element_id).OffsetFromOrigin());
    ...
    for (auto iter = tilings_->Cover(shared_quad_state->visible_quad_layer_rect, max_contents_scale, ideal_contents_scale_key()); iter; ++iter) {
        ...
        if (*iter && iter->draw_info().IsReadyToDraw()) {
            ...
            case TileDrawInfo::RESOURCE_MODE: {
                ...
                if (iter->contents_scale_key() != raster_contents_scale_key() && iter->contents_scale_key() < ideal_contents_scale_key()
                ...
                quad->SetNew(shared_quad_state, offset_geometry_rect, offset_visible_geometry_rect, needs_blending, draw_info.resource_id_for_export(),
                    texture_rect, draw_info.resource_size(), draw_info.is_premultiplied(), nearest_neighbor_,
            ...
            case TileDrawInfo::SOLID_COLOR_MODE: {
                ...
                if (alpha >= std::numeric_limits<float>::epsilon()) {
                    ...
                    quad->SetNew(shared_quad_state, offset_geometry_rect, offset_visible_geometry_rect, draw_info.solid_color(),
                        !layer_tree_impl()->settings().enable_edge_anti_aliasing);
                    ValidateQuadResources(quad);
                }
                has_draw_quad = true;
        ...
        if (!append_quads_data->checkerboarded_needs_record && scaled_cull_rect && !scaled_cull_rect->Contains(visible_geometry_rect)) {
            append_quads_data->checkerboarded_needs_record = true;
        }
        ...
        if (!has_draw_quad) {
            ...
            auto* quad = render_pass->CreateAndAppendDrawQuad<viz::SolidColorDrawQuad>();
            quad->SetNew(shared_quad_state, offset_geometry_rect, offset_visible_geometry_rect, color, false);
            ValidateQuadResources(quad);
            ...
            // contentful image.
            if (*iter) {
                UMA_HISTOGRAM_BOOLEAN("Compositing.DecodeLCPCandidateImage.MissedDeadline", iter->HasMissingLCPCandidateImages());
            }
    ...
    if (missing_tile_count) {
        ...
        append_quads_data->checkerboarded_needs_raster = true;
        TRACE_EVENT_INSTANT1("cc", "PictureLayerImpl::AppendQuads checkerboard", TRACE_EVENT_SCOPE_THREAD, "missing_tile_count", missing_tile_count);
    }
...
bool PictureLayerImpl::UpdateTiles()
{
    ...
    if (contributes_to_drawn_render_surface()) {
        can_require_tiles_for_activation = !only_used_low_res_last_append_quads_ || RequiresHighResToDraw() || !layer_tree_impl()->SmoothnessTakesPriority();
    }

    static const base::NoDestructor<Occlusion> kEmptyOcclusion;
    const Occlusion& occlusion_in_content_space
        = layer_tree_impl()->settings().use_occlusion_for_tile_prioritization ? draw_properties().occlusion_in_content_space : *kEmptyOcclusion;

    // Pass |occlusion_in_content_space| for |occlusion_in_layer_space| since
    // they are the same space in picture layer, as contents scale is always 1.
    bool updated = tilings_->UpdateTilePriorities(viewport_rect_for_tile_priority_in_content_space_, ideal_contents_scale_key(), current_frame_time_in_seconds,
...
void PictureLayerImpl::UpdateViewportRectForTilePriorityInContentSpace()
{
    ...
    if (visible_rect_in_content_space.IsEmpty() || layer_tree_impl()->GetDeviceViewport() != viewport_rect_for_tile_priority) {
        gfx::Transform view_to_layer;
        if (ScreenSpaceTransform().GetInverse(&view_to_layer)) {
            // Transform from view space to content space.
            visible_rect_in_content_space = MathUtil::ProjectEnclosingClippedRect(view_to_layer, viewport_rect_for_tile_priority);
            ...
            int padding_amount = layer_tree_impl()->settings().skewport_extrapolation_limit_in_screen_pixels * MaximumTilingContentsScale();
...
void PictureLayerImpl::UpdateRasterSource(scoped_refptr<RasterSource> raster_source, Region* new_invalidation)
{
    CHECK(layer_tree_impl()->IsSyncTree());
    UpdateRasterSourceInternal(std::move(raster_source), new_invalidation,
        // These pointers being null indicates we are committing.
        nullptr, nullptr, nullptr);
}

void PictureLayerImpl::UpdateRasterSourceInternal(scoped_refptr<RasterSource> raster_source, Region* new_invalidation, const PictureLayerTilingSet* pending_set,
    const PaintWorkletRecordMap* pending_paint_worklet_records, const DiscardableImageMap* pending_discardable_image_map)
{
    CHECK(raster_source);
    ...
    DCHECK(raster_source->size().IsEmpty() || bounds() == raster_source->size())
        << " layer bounds " << bounds().ToString() << " raster_source size " << raster_source->size().ToString();
    ...
    // is different.
    const bool recording_updated = !raster_source_ || raster_source_->GetDisplayItemList() != raster_source->GetDisplayItemList();

    // If the MSAA sample count has changed, we need to re-raster the complete
    // layer.
    if (recording_updated && raster_source_) {
        const auto& current_display_item_list = raster_source_->GetDisplayItemList();
        const auto& new_display_item_list = raster_source->GetDisplayItemList();
        if (current_display_item_list && new_display_item_list) {
            bool needs_full_invalidation = layer_tree_impl()->GetMSAASampleCountForRaster(*current_display_item_list)
                != layer_tree_impl()->GetMSAASampleCountForRaster(*new_display_item_list);
            needs_full_invalidation |= layer_tree_impl()->GetTargetColorParams(current_display_item_list->content_color_usage())
                != layer_tree_impl()->GetTargetColorParams(new_display_item_list->content_color_usage());
    ...
    // crash.
    if (pending_set) {
        tilings_->UpdateTilingsToCurrentRasterSourceForActivation(raster_source_, pending_set, invalidation_, MinimumContentsScale(), MaximumContentsScale());
    } else {
        tilings_->UpdateTilingsToCurrentRasterSourceForCommit(raster_source_, invalidation_, MinimumContentsScale(), MaximumContentsScale());
    }
}
...
void PictureLayerImpl::RegenerateDiscardableImageMap()
{
    ...
    if (const auto* display_list = raster_source_->GetDisplayItemList().get()) {
        scoped_refptr<DiscardableImageMap> image_map = display_list->GenerateDiscardableImageMap();
        SetPaintWorkletInputs(image_map->paint_worklet_inputs());
        layer_tree_impl()->UpdateImageDecodingHints(image_map->TakeDecodingModeMap());
...
void PictureLayerImpl::UpdateCanUseLCDText(bool raster_translation_aligns_pixels)
{
    ...
    lcd_text_disallowed_reason_ = ComputeLCDTextDisallowedReason(raster_translation_aligns_pixels);
}
...
bool PictureLayerImpl::AffectedByWillChangeTransformHint() const
{
    ...
    return transform_node && transform_node->node_or_ancestors_will_change_transform;
}

LCDTextDisallowedReason PictureLayerImpl::ComputeLCDTextDisallowedReason(bool raster_translation_aligns_pixels) const
{
    // No need to use LCD text if there is no text.
    if (!raster_source_ || !raster_source_->GetDisplayItemList() || !raster_source_->GetDisplayItemList()->has_draw_text_ops()) {
    ...
    EffectNode* effect_node = GetEffectTree().Node(effect_tree_index());
    if (effect_node->node_or_ancestor_has_filters || effect_node->affected_by_backdrop_filter) {
...
LCDTextDisallowedReason PictureLayerImpl::ComputeLCDTextDisallowedReasonForTesting() const
{
    gfx::Vector2dF raster_translation;
    return ComputeLCDTextDisallowedReason(CalculateRasterTranslation(raster_translation));
}
...
void PictureLayerImpl::NotifyTileStateChanged(const Tile* tile)
{
    ...
    if (tile->draw_info().NeedsRaster()) {
        PictureLayerTiling* tiling = tilings_->FindTilingWithScaleKey(tile->contents_scale_key());
        if (tiling) {
    ...
    if (layer_tree_impl()->settings().UseLayerContextForDisplay() && !IsActive()) {
        // Accumulate tile changes on the pending tree only. These are pushed to the
        // active tree in PushPropertiesTo().
        updated_tiles_[tile->contents_scale_key()].emplace(tile->tiling_i_index(), tile->tiling_j_index());
    }
}
...
std::unique_ptr<Tile> PictureLayerImpl::CreateTile(const Tile::CreateInfo& info)
{
    ...
    return layer_tree_impl()->tile_manager()->CreateTile(info, id(), layer_tree_impl()->source_frame_number(), flags);
}
...
const PictureLayerTiling* PictureLayerImpl::GetPendingOrActiveTwinTiling(const PictureLayerTiling* tiling) const
{
    PictureLayerImpl* twin_layer = GetPendingOrActiveTwinLayer();
    if (!twin_layer)
        return nullptr;
    const PictureLayerTiling* twin_tiling = twin_layer->tilings_->FindTilingWithScaleKey(tiling->contents_scale_key());
    if (twin_tiling && twin_tiling->raster_transform() == tiling->raster_transform())
        return twin_tiling;
    return nullptr;
}
...
std::vector<const DrawImage*> PictureLayerImpl::GetDiscardableImagesInRect(const gfx::Rect& rect) const
{
...
ScrollOffsetMap PictureLayerImpl::GetRasterInducingScrollOffsets() const
{
    ...
    if (raster_source_) {
        const ScrollTree& scroll_tree = layer_tree_impl()->property_trees()->scroll_tree();
        for (auto [element_id, _] : raster_source_->GetDisplayItemList()->raster_inducing_scrolls()) {
...
const GlobalStateThatImpactsTilePriority& PictureLayerImpl::global_tile_state() const
{
...
{
    return GetScaledEnclosingVisibleRectInTargetSpace(MaximumTilingContentsScale());
}
...
void PictureLayerImpl::GetContentsResourceId(viz::ResourceId* resource_id, gfx::Size* resource_size, gfx::SizeF* resource_uv_size) const
{
    ...
    auto iter = tilings_->Cover(content_rect, dest_scale, ideal_contents_scale_key());
    ...
    DCHECK(iter.geometry_rect() == content_rect) << "iter rect " << iter.geometry_rect().ToString() << " content rect " << content_rect.ToString();
    ...
    *resource_size = draw_info.resource_size();
    ...
    // texture being used.
    gfx::SizeF requested_tile_size = gfx::SizeF(iter->tiling()->tiling_data()->tiling_rect().size());
    ...
    *resource_uv_size
        = gfx::SizeF(requested_tile_size.width() / draw_info.resource_size().width(), requested_tile_size.height() / draw_info.resource_size().height());
}
...
void PictureLayerImpl::UpdateDirectlyCompositedImageFromRasterSource()
{
    ...
    if (const auto& info = raster_source_->directly_composited_image_info()) {
        ...
        // images.
        new_default_raster_scale = GetPreferredRasterScale(info->default_raster_scale);
        new_nearest_neighbor = info->nearest_neighbor;
    }

    directly_composited_image_default_raster_scale_changed_ = new_default_raster_scale != directly_composited_image_default_raster_scale_;

    if (new_nearest_neighbor != nearest_neighbor_ || directly_composited_image_default_raster_scale_changed_) {
...
bool PictureLayerImpl::ShouldDirectlyCompositeImage(float raster_scale) const
{
    ...
    return std::abs(layer_bounds.width() - content_rect.width()) < 1.f && std::abs(layer_bounds.height() - content_rect.height()) < 1.f;
}
...
float PictureLayerImpl::CalculateDirectlyCompositedImageRasterScale() const
{
    ...
    float adjusted_raster_scale
        = directly_composited_image_default_raster_scale_changed_ ? directly_composited_image_default_raster_scale_ : raster_source_scale_key();
    ...
    float max_scale = std::max(directly_composited_image_default_raster_scale_, MinimumContentsScale());
    float min_scale = MinimumContentsScale();

    float clamped_ideal_source_scale = std::clamp(ideal_source_scale_key(), min_scale, max_scale);
    ...
    } else if (adjusted_raster_scale > clamped_ideal_source_scale * kFarAwayFactor) {
        adjusted_raster_scale = clamped_ideal_source_scale;
    } else {
    ...
    adjusted_raster_scale = std::clamp(adjusted_raster_scale, min_scale, max_scale);
    return adjusted_raster_scale;
}

PictureLayerTiling* PictureLayerImpl::AddTiling(const gfx::AxisTransform2d& raster_transform)
{
    ...
    DCHECK(raster_source_->HasRecordings());
    bool tiling_can_use_lcd_text = can_use_lcd_text() && raster_transform.scale() == raster_contents_scale_;
    return tilings_->AddTiling(raster_transform, raster_source_, tiling_can_use_lcd_text);
}
...
bool PictureLayerImpl::CanRecreateHighResTilingForLCDTextAndRasterTransform(const PictureLayerTiling& high_res) const
{
    ...
    if (high_res.can_use_lcd_text()
        && (lcd_text_disallowed_reason_ == LCDTextDisallowedReason::kBackgroundColorNotOpaque
            || lcd_text_disallowed_reason_ == LCDTextDisallowedReason::kContentsNotOpaque)) {
        ...
        // cause flickering.
        DCHECK(!layer_tree_impl()->IsSyncTree() || !layer_tree_impl()->IsReadyToActivate());
        return true;
    }
    ...
    if (draw_properties().screen_space_transform_is_animating || AffectedByWillChangeTransformHint())
        return false;
    ...
    if (lcd_text_disallowed_reason_ == LCDTextDisallowedReason::kNoText && high_res.raster_transform().scale() == raster_contents_scale_)
        return false;
    ...
    if (layer_tree_impl()->GetActivelyScrollingType() != ActivelyScrollingType::kNone) {
        return false;
    }
...
void PictureLayerImpl::UpdateTilingsForRasterScaleAndTranslation(bool has_adjusted_raster_scale)
{
    PictureLayerTiling* high_res = tilings_->FindTilingWithScaleKey(raster_contents_scale_key());

    gfx::Vector2dF raster_translation;
    bool raster_translation_aligns_pixels = CalculateRasterTranslation(raster_translation);
    UpdateCanUseLCDText(raster_translation_aligns_pixels);
    if (high_res) {
        bool raster_transform_is_not_ideal
            = high_res->raster_transform().scale() != raster_contents_scale_ || high_res->raster_transform().translation() != raster_translation;
        bool can_use_lcd_text_changed = high_res->can_use_lcd_text() != can_use_lcd_text();
        bool can_recreate_highres_tiling = CanRecreateHighResTilingForLCDTextAndRasterTransform(*high_res);
        // Only for the sync tree to avoid flickering.
        bool should_recreate_high_res
            = (raster_transform_is_not_ideal || can_use_lcd_text_changed) && layer_tree_impl()->IsSyncTree() && can_recreate_highres_tiling;
        // Only request an invalidation if we don't already have a pending tree.
        bool can_request_invalidation_for_high_res = (raster_transform_is_not_ideal || can_use_lcd_text_changed)
            && !layer_tree_impl()->settings().commit_to_active_tree && layer_tree_impl()->IsActiveTree() && can_recreate_highres_tiling
    ...
    if (!high_res) {
        ...
        high_res = AddTiling(gfx::AxisTransform2d::FromScaleAndTranslation(raster_contents_scale_, raster_translation));
    ...
    if (layer_tree_impl()->IsPendingTree() || (layer_tree_impl()->settings().commit_to_active_tree && IsDirectlyCompositedImage())) {
...
bool PictureLayerImpl::ShouldAdjustRasterScale() const
{
    ...
    if (IsDirectlyCompositedImage()) {
        ...
        float max_scale = std::max(directly_composited_image_default_raster_scale_, MinimumContentsScale());
        if (raster_source_scale_key() < std::min(ideal_source_scale_key(), max_scale))
            return true;
    ...
    if (was_screen_space_transform_animating_ != draw_properties().screen_space_transform_is_animating) {
        if (draw_properties().screen_space_transform_is_animating) {
            ...
            // raster scale.
            float maximum_animation_scale = layer_tree_impl()->property_trees()->MaximumAnimationToScreenScale(transform_tree_index());
            if ((maximum_animation_scale != raster_contents_scale_.x() || maximum_animation_scale != raster_contents_scale_.y())) {
                return true;
            }
        } else {
            // Exiting animation.
    ...
    if (raster_contents_scale_.x() > max_scale || raster_contents_scale_.y() > max_scale)
        return true;
    float min_scale = MinimumContentsScale();
    if (raster_contents_scale_.x() < min_scale || raster_contents_scale_.y() < min_scale)
        return true;
    ...
    if (draw_properties().screen_space_transform_is_animating) {
        ...
        if (ideal_contents_scale_.x() > raster_contents_scale_.x() * kRatioToAdjustRasterScaleForTransformAnimation
            || ideal_contents_scale_.y() > raster_contents_scale_.y() * kRatioToAdjustRasterScaleForTransformAnimation) {
            auto* property_trees = layer_tree_impl()->property_trees();
            int transform_id = transform_tree_index();
            if (property_trees->AnimationScaleCacheIsInvalid(transform_id) || !property_trees->AnimationAffectedByInvalidScale(transform_id)) {
                return true;
            }
        }
        return false;
    ...
    if (AffectedByWillChangeTransformHint()) {
        ...
        if (raster_contents_scale_.x() >= min_raster_scale && raster_contents_scale_.y() >= min_raster_scale)
            return false;
    }
...
void PictureLayerImpl::AddLowResolutionTilingIfNeeded()
{
    ...
    if (!is_pinching && !is_animating) {
        if (!low_res)
            low_res = AddTiling(gfx::AxisTransform2d(low_res_raster_contents_scale_, gfx::Vector2dF()));
...
void PictureLayerImpl::RecalculateRasterScales()
{
    if (IsDirectlyCompositedImage()) {
        ...
        if (ShouldDirectlyCompositeImage(used_raster_scale)) {
            raster_source_scale_ = gfx::Vector2dF(used_raster_scale, used_raster_scale);
    ...
    if (is_pinching && !old_raster_contents_scale.IsZero()) {
        ...
        float desired_contents_scale = std::max(old_raster_contents_scale.x(), old_raster_contents_scale.y());
        ...
        if (const auto* snapped_to_tiling = tilings_->FindTilingWithNearestScaleKey(desired_contents_scale, kSnapToExistingTilingRatio)) {
            raster_contents_scale_ = snapped_to_tiling->raster_transform().scale();
        } else {
            raster_contents_scale_ = old_raster_contents_scale;
            raster_contents_scale_.Scale(desired_contents_scale / raster_contents_scale_key());
        }
        preserved_raster_contents_scale = raster_contents_scale_;
        raster_page_scale_
            = std::max(raster_contents_scale_.x() / raster_source_scale_.x(), raster_contents_scale_.y() / raster_source_scale_.y()) / raster_device_scale_;
    }
    ...
    gfx::Size raster_bounds = gfx::ScaleToCeiledSize(raster_source_->recorded_bounds().size(), raster_contents_scale_.x(), raster_contents_scale_.y());
    ...
    low_res_raster_contents_scale_ = std::max(raster_contents_scale_key() * low_res_factor, min_scale);
...
void PictureLayerImpl::AdjustRasterScaleForTransformAnimation(const gfx::Vector2dF& preserved_raster_contents_scale)
{
    DCHECK(draw_properties().screen_space_transform_is_animating);

    float maximum_animation_scale = layer_tree_impl()->property_trees()->MaximumAnimationToScreenScale(transform_tree_index());
    raster_contents_scale_.SetToMax(gfx::Vector2dF(maximum_animation_scale, maximum_animation_scale));
    ...
    float max_viewport_dimension = std::max({ viewport.width(), viewport.height(), kMinViewportDimension });
    ...
    max_visible_bounds.SetToMin(gfx::SizeF(max_viewport_dimension, max_viewport_dimension));
    gfx::SizeF max_visible_bounds_at_max_scale = gfx::ScaleSize(max_visible_bounds, raster_contents_scale_.x(), raster_contents_scale_.y());
    float maximum_area = max_visible_bounds_at_max_scale.width() * max_visible_bounds_at_max_scale.height();
    // Clamp the scale to make the rastered content not larger than the viewport.
    if (maximum_area > squared_viewport_area) [[unlikely]] {
        raster_contents_scale_.Scale(1.f / std::sqrt(maximum_area / squared_viewport_area));
    }
}

void PictureLayerImpl::CleanUpTilingsOnActiveLayer(const std::vector<raw_ptr<PictureLayerTiling, VectorExperimental>>& used_tilings)
{
    DCHECK(layer_tree_impl()->IsActiveTree());
    if (tilings_->num_tilings() == 0)
        return;

    float min_acceptable_high_res_scale = std::min(raster_contents_scale_key(), ideal_contents_scale_key());
    float max_acceptable_high_res_scale = std::max(raster_contents_scale_key(), ideal_contents_scale_key());

    PictureLayerImpl* twin = GetPendingOrActiveTwinLayer();
    if (twin && twin->CanHaveTilings()) {
        min_acceptable_high_res_scale = std::min({ min_acceptable_high_res_scale, twin->raster_contents_scale_key(), twin->ideal_contents_scale_key() });
        max_acceptable_high_res_scale = std::max({ max_acceptable_high_res_scale, twin->raster_contents_scale_key(), twin->ideal_contents_scale_key() });
    }

    PictureLayerTilingSet* twin_set = twin ? twin->tilings_.get() : nullptr;
    tilings_->CleanUpTilings(min_acceptable_high_res_scale, max_acceptable_high_res_scale, used_tilings, twin_set);
}

float PictureLayerImpl::MinimumRasterContentsScaleForWillChangeTransform() const
{
...
bool PictureLayerImpl::CalculateRasterTranslation(gfx::Vector2dF& raster_translation) const
{
    ...
    if (!screen_transform.IsScaleOrTranslation() || !draw_transform.IsScaleOrTranslation()) {
        return false;
    }
    ...
    auto is_raster_scale = [this](const gfx::Transform& transform) -> bool {
        ...
        return std::abs(scale_diff.x()) <= kScaleErrorThreshold && std::abs(scale_diff.y()) <= kScaleErrorThreshold;
    };
    ...
    if (std::abs(screen_x_fraction - target_x_fraction) > kPixelErrorThreshold || std::abs(screen_y_fraction - target_y_fraction) > kPixelErrorThreshold) {
        return false;
    }
...
float PictureLayerImpl::MaximumContentsScale() const
{
    ...
    float max_dimension = static_cast<float>(is_backdrop_filter_mask_ ? layer_tree_impl()->max_texture_size() : std::numeric_limits<int>::max());
...
std::unique_ptr<PictureLayerTilingSet> PictureLayerImpl::CreatePictureLayerTilingSet()
{
    const LayerTreeSettings& settings = layer_tree_impl()->settings();
    return PictureLayerTilingSet::Create(IsActive() ? ACTIVE_TREE : PENDING_TREE, this, settings.tiling_interest_area_padding,
        layer_tree_impl()->use_gpu_rasterization() ? settings.gpu_rasterization_skewport_target_time_in_seconds : settings.skewport_target_time_in_seconds,
        settings.skewport_extrapolation_limit_in_screen_pixels, settings.max_preraster_distance_in_screen_pixels);
}
...
void PictureLayerImpl::UpdateIdealScales()
{
    ...
    if (layer_tree_impl()->PageScaleTransformNode()) {
        ...
        ideal_page_scale_ = IsAffectedByPageScale() ? layer_tree_impl()->current_page_scale_factor() : 1.f;
    }
    ...
    float external_page_scale_factor = layer_tree_impl() ? layer_tree_impl()->external_page_scale_factor() : 1.f;
    DCHECK(!layer_tree_impl() || layer_tree_impl()->settings().is_for_scalable_page || external_page_scale_factor == 1.f
    ...
    ideal_contents_scale_.SetToMax(gfx::Vector2dF(min_contents_scale, min_contents_scale));
    ideal_contents_scale_.SetToMin(gfx::Vector2dF(kMaxIdealContentsScale, kMaxIdealContentsScale));
    ideal_source_scale_ = { ideal_contents_scale_.x() / ideal_page_scale_, ideal_contents_scale_.y() / ideal_page_scale_ };
}

void PictureLayerImpl::GetDebugBorderProperties(SkColor4f* color, float* width) const
{
    float device_scale_factor = layer_tree_impl() ? layer_tree_impl()->device_scale_factor() : 1;
...
void PictureLayerImpl::GetAllPrioritizedTilesForTracing(std::vector<PrioritizedTile>* prioritized_tiles) const
{
    if (!tilings_)
        return;
...
void PictureLayerImpl::AsValueInto(base::trace_event::TracedValue* state) const
{
    ...
    MathUtil::AddToTracedValue("tile_priority_rect", viewport_rect_for_tile_priority_in_content_space_, state);
    MathUtil::AddToTracedValue("visible_rect", visible_layer_rect(), state);

    state->SetString("lcd_text_disallowed_reason", LCDTextDisallowedReasonToString(lcd_text_disallowed_reason_));
    ...
    for (auto iter = tilings_->Cover(gfx::Rect(bounds()), MaximumTilingContentsScale(), ideal_contents_scale_key()); iter; ++iter) {
    ...
    state->SetBoolean("raster_source_solid_color", raster_source_->IsSolidColor());
    state->SetBoolean("draws_content", draws_content());
    state->SetBoolean("raster_source_has_recordings", raster_source_->HasRecordings());
...
{
    return IsOnActiveOrPendingTree() && (contributes_to_drawn_render_surface() || raster_even_if_not_drawn());
}

PictureLayerImpl::ImageInvalidationResult PictureLayerImpl::InvalidateRegionForImages(const PaintImageIdFlatSet& images_to_invalidate)
{
...
void PictureLayerImpl::InvalidateRasterInducingScrolls(const base::flat_set<ElementId>& scrolls_to_invalidate)
{
    if (!raster_source_ || !raster_source_->GetDisplayItemList()) {
        return;
    }
    const DisplayItemList::RasterInducingScrollMap& raster_inducing_scrolls = raster_source_->GetDisplayItemList()->raster_inducing_scrolls();
    Region invalidation;
...
void PictureLayerImpl::SetPaintWorkletRecord(scoped_refptr<const PaintWorkletInput> input, PaintRecord record)
{
...
void PictureLayerImpl::SetPaintWorkletInputs(const std::vector<DiscardableImageMap::PaintWorkletInputWithImageId>& inputs)
{
    ...
    for (const auto& input_with_id : inputs) {
        ...
        new_records[input] = std::make_pair(paint_image_id, std::move(paint_worklet_records_[input].second));
    ...
    if ((has_paint_worklets != had_paint_worklets) && layer_tree_impl()->IsPendingTree()) {
        ...
        layer_tree_impl()->NotifyLayerHasPaintWorkletsChanged(this, has_paint_worklets);
    }
    if (layer_tree_impl()->IsPendingTree()) {
        layer_tree_impl()->paint_worklet_tracker().UpdatePaintWorkletInputProperties(inputs, this);
    }
}

void PictureLayerImpl::InvalidatePaintWorklets(
    const PaintWorkletInput::PropertyKey& key, const PaintWorkletInput::PropertyValue& prev, const PaintWorkletInput::PropertyValue& next)
{
    for (auto& entry : paint_worklet_records_) {
        const std::vector<PaintWorkletInput::PropertyKey>& prop_ids = entry.first->GetPropertyKeys();
        ...
        if (base::Contains(prop_ids, key) && entry.first->ValueChangeShouldCauseRepaint(prev, next)) {
...
DamageReasonSet PictureLayerImpl::GetDamageReasons() const
{
    ...
    if (LayerPropertyChanged() || has_non_animated_image_update_rect_ || !damage_rect_.IsEmpty()) {
