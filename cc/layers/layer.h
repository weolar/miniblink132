...
class CC_EXPORT Layer : public base::RefCounted<Layer>, public ProtectedSequenceSynchronizer {
public:
    ...
    // or null.
    Layer* mutable_parent()
    {
        return parent_.Write(*this);
    }
    const Layer* parent() const
    {
        return parent_.Read(*this);
    }
    ...
    {
        return inputs_.Read(*this).children;
    }
    ...
    {
        return inputs_.Read(*this).bounds;
    }
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->mask_layer.get() : nullptr;
    }
    ...
    void ResetUpdateRectForTesting()
    {
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->corner_radii : kNoRoundedCornersF;
    }
    // Returns true if any of the corner has a non-zero radius set.
    bool HasRoundedCorner() const
    {
        return !corner_radii().IsEmpty();
    }
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->gradient_mask : gfx::LinearGradient::GetEmpty();
    }
    ...
    {
        return HasRoundedCorner() || HasGradientMask();
    }
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->blend_mode : SkBlendMode::kSrcOver;
    }
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->filters : FilterOperations();
    }
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->backdrop_filters : FilterOperations();
    }
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->backdrop_filter_bounds : std::nullopt;
    }
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->backdrop_filter_quality : 1.0f;
    }
    ...
    {
        return inputs_.Read(*this).contents_opaque;
    }
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->transform : kIdentityTransform;
    }
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->transform_origin : gfx::Point3F();
    }
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->scroll_offset : gfx::PointF();
    }
    ...
    {
        return layer_tree_inputs() ? layer_tree_inputs()->scroll_container_bounds : gfx::Size();
    }
    ...
    // there.
    void SetMainThreadScrollHitTestRegion(const Region& main_thread_scroll_hit_test_region);
    ...
    const std::vector<ScrollHitTestRect>* non_composited_scroll_hit_test_rects() const
    {
        if (const auto& rare_inputs = inputs_.Read(*this).rare_inputs) {
    ...
    void SetDidScrollCallback(base::RepeatingCallback<void(const gfx::PointF&, const ElementId&)>);
    ...
    {
        return GetBitFlag(kDrawsContentFlagMask);
    }
    ...
    void SetHasClipNode(bool val)
    {
        SetBitFlag(val, kHasClipNodeFlagMask);
    }
    bool has_clip_node() const
    {
        return GetBitFlag(kHasClipNodeFlagMask);
    }
    ...
    {
        return inputs_.Read(*this).element_id;
    }
    ...
    // NodeInfo in |content|.
    virtual void CaptureContent(const gfx::Rect& rect, std::vector<NodeInfo>* content) const;
    ...
    virtual std::unique_ptr<LayerImpl> CreateLayerImpl(LayerTreeImpl* tree_impl) const;
    ...
    virtual void PushPropertiesTo(LayerImpl* layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state);
    ...
    void TakeCopyRequests(std::vector<std::unique_ptr<viz::CopyOutputRequest>>* requests);
...
protected:
    ...
    {
        return base::AutoReset<bool>(&ignore_set_needs_commit_for_test_.Write(*this), true);
    }
...
private:
    ...
    bool SetBitFlag(bool new_value, uint8_t mask, bool invalidate = false, bool needs_push = false);
    ...
    struct LayerTreeInputs {
        ...
        base::RepeatingCallback<void(const gfx::PointF&, const ElementId&)> did_scroll_callback;
    ...
    void UpdateMaskFilterInfo(const gfx::RoundedCornersF* corner_radii, const gfx::LinearGradient* gradient_mask);
    ...
    ProtectedSequenceReadable<std::unique_ptr<LayerTreeInputs>> layer_tree_inputs_;
