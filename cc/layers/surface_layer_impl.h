...
using UpdateSubmissionStateCB = base::RepeatingCallback<void(bool is_visible, base::WaitableEvent*)>;

class CC_EXPORT SurfaceLayerImpl : public LayerImpl {
public:
    static std::unique_ptr<SurfaceLayerImpl> Create(LayerTreeImpl* tree_impl, int id, UpdateSubmissionStateCB update_submission_state_callback);

    static std::unique_ptr<SurfaceLayerImpl> Create(LayerTreeImpl* tree_impl, int id);
    ...
    void SetRange(const viz::SurfaceRange& surface_range, std::optional<uint32_t> deadline_in_frames);
    const viz::SurfaceRange& range() const
    {
        return surface_range_;
    }
