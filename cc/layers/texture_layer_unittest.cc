...
{
    return gpu::SyncToken(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x123), value);
}

void AllocateCrossThreadSharedBitmap(gpu::SharedImageInterface* shared_image_interface, const gfx::Size& size, scoped_refptr<CrossThreadSharedBitmap>& bitmap,
    scoped_refptr<gpu::ClientSharedImage>& shared_image, gpu::SyncToken& sync_token)
{
    CHECK(shared_image_interface);
    viz::SharedImageFormat format = viz::SinglePlaneFormat::kBGRA_8888;

    auto shared_image_mapping
        = shared_image_interface->CreateSharedImage({ format, size, gfx::ColorSpace(), gpu::SHARED_IMAGE_USAGE_CPU_WRITE, "TextureLayerSharedBitmap" });
    bitmap = base::MakeRefCounted<CrossThreadSharedBitmap>(
        viz::SharedBitmapId(), base::ReadOnlySharedMemoryRegion(), std::move(shared_image_mapping.mapping), size, format);
...
public:
    static std::unique_ptr<MockLayerTreeHost> Create(FakeLayerTreeHostClient* client, TaskGraphRunner* task_graph_runner, MutatorHost* mutator_host)
    {
...
private:
    ...
    {
        InitializeSingleThreaded(&single_thread_client_, base::SingleThreadTaskRunner::GetCurrentDefault());
    }
...
public:
    MOCK_METHOD3(Release, void(const gpu::Mailbox& mailbox, const gpu::SyncToken& sync_token, bool lost_resource));
    MOCK_METHOD3(ReleaseSW, void(scoped_refptr<gpu::ClientSharedImage>, const gpu::SyncToken& token, bool lost));
};
...
struct CommonResourceObjects {
    CommonResourceObjects()
        ...
        , sync_token1_(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x234), 1)
        , sync_token2_(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x234), 2)
        , sync_token_sw_(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x235), 1)
    {
        release_callback1_ = base::BindRepeating(&MockReleaseCallback::Release, base::Unretained(&mock_callback_), mailbox_name1_);
        release_callback2_ = base::BindRepeating(&MockReleaseCallback::Release, base::Unretained(&mock_callback_), mailbox_name2_);
        ...
        resource1_ = viz::TransferableResource::MakeGpu(
            mailbox_name1_, arbitrary_target1, sync_token1_, size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
        resource2_ = viz::TransferableResource::MakeGpu(
            mailbox_name2_, arbitrary_target2, sync_token2_, size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);

        // For software rendering
        shared_image_sw_ = gpu::ClientSharedImage::CreateForTesting();
        resource_sw_ = viz::TransferableResource::MakeSoftwareSharedImage(
            shared_image_sw_, sync_token_sw_, size, viz::SinglePlaneFormat::kBGRA_8888, viz::TransferableResource::ResourceSource::kTest);
        release_callback_sw_ = base::BindRepeating(&MockReleaseCallback::ReleaseSW, base::Unretained(&mock_callback_), shared_image_sw_);
    }

    using RepeatingReleaseCallback = base::RepeatingCallback<void(const gpu::SyncToken& sync_token, bool is_lost)>;
...
protected:
    void SetUp() override
    {
        ...
        layer_tree_host_ = MockLayerTreeHost::Create(&fake_client_, &task_graph_runner_, animation_host_.get());
        EXPECT_CALL(*layer_tree_host_, SetNeedsCommit()).Times(AnyNumber());
        layer_tree_host_->SetViewportRectAndScale(gfx::Rect(10, 10), 1.f, viz::LocalSurfaceId());
        Mock::VerifyAndClearExpectations(layer_tree_host_.get());
    }
...
TEST_F(TextureLayerTest, ShutdownWithResource)
{
    for (int i = 0; i < 2; ++i) {
        ...
        auto host = LayerTreeHost::CreateSingleThreaded(&single_thread_client, std::move(params));
        ...
        if (gpu) {
            layer->SetTransferableResource(test_data_.resource1_, test_data_.release_callback1_);
        } else {
            layer->SetTransferableResource(test_data_.resource_sw_, test_data_.release_callback_sw_);
        }

        viz::ParentLocalSurfaceIdAllocator allocator;
        allocator.GenerateId();
        host->SetViewportRectAndScale(gfx::Rect(10, 10), 1.f, allocator.GetCurrentLocalSurfaceId());
        ...
        if (gpu) {
            EXPECT_CALL(test_data_.mock_callback_, Release(test_data_.mailbox_name1_, _, false)).Times(1);
        } else {
            EXPECT_CALL(test_data_.mock_callback_, ReleaseSW(test_data_.shared_image_sw_, test_data_.sync_token_sw_, false)).Times(1);
        }
        {
            base::RunLoop loop;
            loop.RunUntilIdle();
...
protected:
    void TearDown() override
    {
        ...
        EXPECT_CALL(test_data_.mock_callback_, Release(test_data_.mailbox_name1_, test_data_.sync_token1_, false)).Times(1);
...
TEST_F(TextureLayerWithResourceTest, ReplaceMailboxOnMainThreadBeforeCommit)
{
    ...
    test_layer->SetTransferableResource(test_data_.resource1_, test_data_.release_callback1_);
    Mock::VerifyAndClearExpectations(layer_tree_host_.get());

    EXPECT_CALL(*layer_tree_host_, SetNeedsCommit()).Times(AtLeast(1));
    EXPECT_CALL(test_data_.mock_callback_, Release(test_data_.mailbox_name1_, test_data_.sync_token1_, false)).Times(1);
    test_layer->SetTransferableResource(test_data_.resource2_, test_data_.release_callback2_);
    Mock::VerifyAndClearExpectations(layer_tree_host_.get());
    Mock::VerifyAndClearExpectations(&test_data_.mock_callback_);

    EXPECT_CALL(*layer_tree_host_, SetNeedsCommit()).Times(AtLeast(1));
    EXPECT_CALL(test_data_.mock_callback_, Release(test_data_.mailbox_name2_, test_data_.sync_token2_, false)).Times(1);
    test_layer->ClearTexture();
    Mock::VerifyAndClearExpectations(layer_tree_host_.get());
    Mock::VerifyAndClearExpectations(&test_data_.mock_callback_);

    EXPECT_CALL(*layer_tree_host_, SetNeedsCommit()).Times(AtLeast(1));
    test_layer->SetTransferableResource(test_data_.resource_sw_, test_data_.release_callback_sw_);
    Mock::VerifyAndClearExpectations(layer_tree_host_.get());
    Mock::VerifyAndClearExpectations(&test_data_.mock_callback_);
...
public:
    ...
    void Wait(const base::Thread& thread)
    {
        ...
        thread.task_runner()->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&event)));
        event.Wait();
    }

    void CreateMainRef()
    {
        resource_holder_ = TestMailboxHolder::Create(test_data_.resource1_, test_data_.release_callback1_);
    }
    ...
    void CreateImplRef(viz::ReleaseCallback* impl_ref, scoped_refptr<base::SequencedTaskRunner> main_thread_task_runner)
    {
        *impl_ref = base::BindOnce(&TextureLayer::TransferableResourceHolder::Return, resource_holder_, main_thread_task_runner);
    }
...
TEST_F(TextureLayerMailboxHolderTest, TwoCompositors_BothReleaseThenMain)
{
    ...
    main_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&TextureLayerMailboxHolderTest::CreateMainRef, base::Unretained(this)));
    ...
    main_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&TextureLayerMailboxHolderTest::CreateImplRef, base::Unretained(this), &compositor1, main_thread_.task_runner()));
    ...
    main_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&TextureLayerMailboxHolderTest::CreateImplRef, base::Unretained(this), &compositor2, main_thread_.task_runner()));
    ...
    EXPECT_CALL(test_data_.mock_callback_, Release(test_data_.mailbox_name1_, SyncTokenFromUInt(200), false)).Times(1);

    main_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&TextureLayerMailboxHolderTest::ReleaseMainRef, base::Unretained(this)));
    Wait(main_thread_);
    Mock::VerifyAndClearExpectations(&test_data_.mock_callback_);
}
...
TEST_F(TextureLayerMailboxHolderTest, TwoCompositors_MainReleaseBetween)
{
    ...
    EXPECT_CALL(test_data_.mock_callback_, Release(test_data_.mailbox_name1_, SyncTokenFromUInt(200), true)).Times(1);
...
TEST_F(TextureLayerMailboxHolderTest, TwoCompositors_MainReleasedFirst)
{
    ...
    // The second impl reference is destroyed last, causing the resource to be
    // released back to the embedder with the last sync point from the impl tree.
    EXPECT_CALL(test_data_.mock_callback_, Release(test_data_.mailbox_name1_, SyncTokenFromUInt(100), true)).Times(1);
...
public:
    ...
    void ReleaseCallback(char mailbox_char, const gpu::SyncToken& sync_token, bool lost_resource)
    {
        ...
        if (pending_callback_) {
            layer_tree_host()->GetTaskRunnerProvider()->MainThreadTaskRunner()->PostTask(
                FROM_HERE, base::BindOnce(&TextureLayerImplWithMailboxThreadedCallback::AdvanceTestCase, base::Unretained(this)));
        }
    }
    ...
    {
        EXPECT_EQ(true, main_thread_.CalledOnValidThread());
        viz::ReleaseCallback callback = base::BindOnce(&TextureLayerImplWithMailboxThreadedCallback::ReleaseCallback, base::Unretained(this), mailbox_char);

        const gfx::Size size(64, 64);
        auto resource = viz::TransferableResource::MakeGpu(MailboxFromChar(mailbox_char), GL_TEXTURE_2D, SyncTokenFromUInt(static_cast<uint32_t>(mailbox_char)),
            size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
...
protected:
    ...
    {
        bool will_draw = layer->WillDraw(mode, host_impl_.active_tree()->resource_provider());
        if (will_draw)
...
{
    EXPECT_CALL(test_data_.mock_callback_, Release(test_data_.mailbox_name1_, test_data_.sync_token1_, false)).Times(AnyNumber());
    EXPECT_CALL(test_data_.mock_callback_, ReleaseSW(test_data_.shared_image_sw_, test_data_.sync_token_sw_, false)).Times(AnyNumber());
    // Hardware mode.
    {
        std::unique_ptr<TextureLayerImpl> impl_layer = CreateTextureLayer();
        impl_layer->SetTransferableResource(test_data_.resource1_, test_data_.release_callback1_);
...

    {
        ...
        impl_layer->SetTransferableResource(viz::TransferableResource(), viz::ReleaseCallback());
...

    {
        // Software resource.
        std::unique_ptr<TextureLayerImpl> impl_layer = CreateTextureLayer();
        impl_layer->SetTransferableResource(test_data_.resource_sw_, test_data_.release_callback_sw_);
...
TEST_F(TextureLayerImplWithResourceTest, TestImplLayerCallbacks)
{
    ...
    std::unique_ptr<LayerImpl> active_layer(pending_layer->CreateLayerImpl(host_impl_.active_tree()));
    ASSERT_TRUE(active_layer);

    pending_layer->SetTransferableResource(test_data_.resource1_, test_data_.release_callback1_);

    // Test multiple commits without an activation. The resource wasn't used so
    // the original sync token is returned.
    EXPECT_CALL(test_data_.mock_callback_, Release(test_data_.mailbox_name1_, test_data_.sync_token1_, false)).Times(1);
    pending_layer->SetTransferableResource(test_data_.resource2_, test_data_.release_callback2_);
    Mock::VerifyAndClearExpectations(&test_data_.mock_callback_);
    ...
    EXPECT_CALL(test_data_.mock_callback_, Release(test_data_.mailbox_name2_, _, false)).Times(1);
    pending_layer->PushPropertiesTo(active_layer.get());
    active_layer->DidBecomeActive();
    Mock::VerifyAndClearExpectations(&test_data_.mock_callback_);
    ...
    EXPECT_CALL(test_data_.mock_callback_, Release(test_data_.mailbox_name1_, _, false)).Times(1);
    pending_layer->SetTransferableResource(viz::TransferableResource(), viz::ReleaseCallback());
    pending_layer->PushPropertiesTo(active_layer.get());
    active_layer->DidBecomeActive();
    Mock::VerifyAndClearExpectations(&test_data_.mock_callback_);
...
TEST_F(TextureLayerImplWithResourceTest, TestDestructorCallbackOnCreatedResource)
{
    std::unique_ptr<TextureLayerImpl> impl_layer = CreateTextureLayer();
    ...
    EXPECT_TRUE(impl_layer->WillDraw(DRAW_MODE_HARDWARE, host_impl_.active_tree()->resource_provider()));
...
// the texture mailbox.
class TextureLayerNoExtraCommitForMailboxTest : public LayerTreeTest, public TextureLayerClient {
public:
    // TextureLayerClient implementation.
    bool PrepareTransferableResource(
        SharedBitmapIdRegistrar* bitmap_registrar, viz::TransferableResource* resource, viz::ReleaseCallback* release_callback) override
    {
        ...
        *resource = viz::TransferableResource::MakeGpu(
            MailboxFromChar('1'), GL_TEXTURE_2D, SyncTokenFromUInt(0x123), size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
        *release_callback = base::BindOnce(&TextureLayerNoExtraCommitForMailboxTest::ResourceReleased, base::Unretained(this));
        return true;
    }
...
class TextureLayerChangeInvisibleMailboxTest : public LayerTreeTest, public TextureLayerClient {
public:
        ...
        SharedBitmapIdRegistrar* bitmap_registrar, viz::TransferableResource* resource, viz::ReleaseCallback* release_callback) override
    {
        ...
        *resource = resource_;
        *release_callback = base::BindOnce(&TextureLayerChangeInvisibleMailboxTest::ResourceReleased, base::Unretained(this));
        return true;
    }
    ...
    {
        constexpr gfx::Size size(64, 64);
        return viz::TransferableResource::MakeGpu(MailboxFromChar(name), GL_TEXTURE_2D, SyncTokenFromUInt(static_cast<uint32_t>(name)), size,
...
class TextureLayerReleaseResourcesBase : public LayerTreeTest, public TextureLayerClient {
public:
    // TextureLayerClient implementation.
    bool PrepareTransferableResource(
        SharedBitmapIdRegistrar* bitmap_registrar, viz::TransferableResource* resource, viz::ReleaseCallback* release_callback) override
    {
        constexpr gfx::Size size(64, 64);
        *resource = viz::TransferableResource::MakeGpu(
            MailboxFromChar('1'), GL_TEXTURE_2D, SyncTokenFromUInt(1), size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
        *release_callback = base::BindOnce(&TextureLayerReleaseResourcesBase::ResourceReleased, base::Unretained(this));
        return true;
    }
    ...
    {
        EXPECT_TRUE(resource_released_);
    }
...
class TextureLayerReleaseResourcesAfterCommit : public TextureLayerReleaseResourcesBase {
public:
...
class TextureLayerReleaseResourcesAfterActivate : public TextureLayerReleaseResourcesBase {
public:
...
public:
    ...
    {
        EXPECT_EQ(true, main_thread_.CalledOnValidThread());
        viz::ReleaseCallback callback = base::BindOnce(&TextureLayerWithResourceMainThreadDeleted::ReleaseCallback, base::Unretained(this));
        constexpr gfx::Size size(64, 64);
        auto resource = viz::TransferableResource::MakeGpu(MailboxFromChar(mailbox_char), GL_TEXTURE_2D, SyncTokenFromUInt(static_cast<uint32_t>(mailbox_char)),
            size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
        layer_->SetTransferableResource(resource, std::move(callback));
...
public:
    ...
    {
        EXPECT_EQ(true, main_thread_.CalledOnValidThread());
        viz::ReleaseCallback callback = base::BindOnce(&TextureLayerWithResourceImplThreadDeleted::ReleaseCallback, base::Unretained(this));
        constexpr gfx::Size size(64, 64);
        auto resource = viz::TransferableResource::MakeGpu(MailboxFromChar(mailbox_char), GL_TEXTURE_2D, SyncTokenFromUInt(static_cast<uint32_t>(mailbox_char)),
            size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
        layer_->SetTransferableResource(resource, std::move(callback));
...
public:
    ...
    FakeLayerTreeFrameSink* frame_sink() const
    {
        return frame_sink_;
    }
...
protected:
        ...
        scoped_refptr<viz::RasterContextProvider> compositor_context_provider, scoped_refptr<viz::RasterContextProvider> worker_context_provider) override
    {
        constexpr bool disable_display_vsync = false;
        bool synchronous_composite = !HasImplThread() && !layer_tree_host()->GetSettings().single_thread_proxy_scheduler;
        auto sink = std::make_unique<TestLayerTreeFrameSink>(nullptr, nullptr, gpu_memory_buffer_manager(), renderer_settings, &debug_settings_,
            task_runner_provider(), synchronous_composite, disable_display_vsync, refresh_rate);
        frame_sink_ = sink.get();
    ...
    scoped_refptr<TextureLayer> texture_layer_;
    raw_ptr<TestLayerTreeFrameSink, AcrossTasksDanglingUntriaged> frame_sink_ = nullptr;
...
protected:
    ...
    void DidCommitAndDrawFrame() override
    {
        ...
        case 1: {
            ...
            gfx::Size size(1, 1);
            AllocateCrossThreadSharedBitmap(shared_image_interface_.get(), size, bitmap_, shared_image_, sync_token_);
            auto transferable_resource = viz::TransferableResource::MakeSoftwareSharedImage(shared_image_, sync_token_, shared_image_->size(),
                viz::SinglePlaneFormat::kBGRA_8888, viz::TransferableResource::ResourceSource::kTileRasterTask);
            auto release_callback = base::BindOnce(&DeleteSharedImage, std::move(shared_image_));

            texture_layer_->SetTransferableResource(std::move(transferable_resource), std::move(release_callback));
        } break;
        case 2:
...
class SoftwareTextureLayerMultipleResourceTest : public SoftwareTextureLayerTest {
protected:
    void BeginTest() override
    {
        PostSetNeedsCommitToMainThread();
    ...
    void DidCommitAndDrawFrame() override
    {
        ...
        case 1: {
            ...
            gpu::SyncToken sync_token2_;
            gfx::Size size(1, 1);
            AllocateCrossThreadSharedBitmap(shared_image_interface_.get(), size, bitmap1_, shared_image1_, sync_token1_);
            AllocateCrossThreadSharedBitmap(shared_image_interface_.get(), size, bitmap2_, shared_image2_, sync_token2_);
            auto transferable_resource1 = viz::TransferableResource::MakeSoftwareSharedImage(shared_image1_, sync_token1_, shared_image1_->size(),
                viz::SinglePlaneFormat::kBGRA_8888, viz::TransferableResource::ResourceSource::kTileRasterTask);

            auto transferable_resource2 = viz::TransferableResource::MakeSoftwareSharedImage(shared_image2_, sync_token2_, shared_image2_->size(),
                viz::SinglePlaneFormat::kBGRA_8888, viz::TransferableResource::ResourceSource::kTileRasterTask);

            // Give the TextureLayer a resource so it contributes to the frame. It
            // doesn't need to register the SharedBitmapId otherwise.
            auto release_callback1 = base::BindOnce(&DeleteSharedImage, std::move(shared_image1_));
            auto release_callback2 = base::BindOnce(&DeleteSharedImage, std::move(shared_image2_));

            texture_layer_->SetTransferableResource(std::move(transferable_resource1), std::move(release_callback1));
            texture_layer_->SetTransferableResource(std::move(transferable_resource2), std::move(release_callback2));
        } break;
        case 2:
...
protected:
    ...
    void DidCommitAndDrawFrame() override
    {
        ...
        MainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&SoftwareTextureLayerLoseFrameSinkTest::NextStep, base::Unretained(this)));
    }
    ...
    void NextStep()
    {
        ...
        case 1: {
            ...
            auto transferable_resource = viz::TransferableResource::MakeSoftwareSharedImage(shared_image_, sync_token_, shared_image_->size(),
                viz::SinglePlaneFormat::kBGRA_8888, viz::TransferableResource::ResourceSource::kTileRasterTask);
            auto release_callback = base::BindOnce(&SoftwareTextureLayerLoseFrameSinkTest::ReleaseCallback, base::Unretained(this), std::move(shared_image_));
        ...
        case 2:
            ...
            first_frame_sink_ = layer_tree_host()->ReleaseLayerTreeFrameSink().get();
    ...
    void ReleaseCallback(scoped_refptr<gpu::ClientSharedImage> shared_image, const gpu::SyncToken& sync_token, bool lost)
    {
...
public:
    bool PrepareTransferableResource(
        SharedBitmapIdRegistrar* bitmap_registar, viz::TransferableResource* transferable_resource, viz::ReleaseCallback* release_callback) override
    {
        return false;
    }
    ...
    void DisplayReceivedCompositorFrameOnThread(const viz::CompositorFrame& frame) override
    {
        ...
        EXPECT_NE(viz::DrawQuad::Material::kTextureContent, quad_list.ElementAt(0)->material);
