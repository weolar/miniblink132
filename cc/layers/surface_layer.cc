...
{
    return base::WrapRefCounted(new SurfaceLayer(std::move(update_submission_state_callback)));
}
...
void SurfaceLayer::SetSurfaceId(const viz::SurfaceId& surface_id, const DeadlinePolicy& deadline_policy)
{
    if (surface_range_.Read(*this).end() == surface_id && deadline_policy.use_existing_deadline()) {
        return;
    }
    ...
    if (surface_id.local_surface_id().is_valid()) {
        TRACE_EVENT_WITH_FLOW2(TRACE_DISABLED_BY_DEFAULT("viz.surface_id_flow"), "LocalSurfaceId.Embed.Flow",
            TRACE_ID_GLOBAL(surface_id.local_surface_id().embed_trace_id()), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "step", "SetSurfaceId",
...
void SurfaceLayer::SetOldestAcceptableFallback(const viz::SurfaceId& surface_id)
{
    ...
    DCHECK(!surface_range_.Read(*this).start() || !surface_range_.Read(*this).start()->IsNewerThan(surface_id));
    ...
    surface_range = viz::SurfaceRange(surface_id.is_valid() ? std::optional<viz::SurfaceId>(surface_id) : std::nullopt, surface_range.end());
...
{
    if (stretch_content_to_fill_bounds_.Read(*this) == stretch_content_to_fill_bounds)
        return;
...
std::unique_ptr<LayerImpl> SurfaceLayer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
    auto layer_impl = SurfaceLayerImpl::Create(tree_impl, id(), update_submission_state_callback_.Read(*this));
    return layer_impl;
}
...
void SurfaceLayer::PushPropertiesTo(LayerImpl* layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
    Layer::PushPropertiesTo(layer, commit_state, unsafe_state);
    TRACE_EVENT0("cc", "SurfaceLayer::PushPropertiesTo");
    SurfaceLayerImpl* layer_impl = static_cast<SurfaceLayerImpl*>(layer);
    layer_impl->SetRange(surface_range_.Read(*this), std::move(deadline_in_frames_.Write(*this)));
    ...
    layer_impl->SetStretchContentToFillBounds(stretch_content_to_fill_bounds_.Read(*this));
