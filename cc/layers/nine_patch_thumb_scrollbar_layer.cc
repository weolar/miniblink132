...
std::unique_ptr<LayerImpl> NinePatchThumbScrollbarLayer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
    return NinePatchThumbScrollbarLayerImpl::Create(tree_impl, id(), orientation(), is_left_side_vertical_scrollbar());
}

scoped_refptr<NinePatchThumbScrollbarLayer> NinePatchThumbScrollbarLayer::CreateOrReuse(
    scoped_refptr<Scrollbar> scrollbar, NinePatchThumbScrollbarLayer* existing_layer)
{
...
scoped_refptr<NinePatchThumbScrollbarLayer> NinePatchThumbScrollbarLayer::Create(scoped_refptr<Scrollbar> scrollbar)
{
    return base::WrapRefCounted(new NinePatchThumbScrollbarLayer(std::move(scrollbar)));
}

NinePatchThumbScrollbarLayer::NinePatchThumbScrollbarLayer(scoped_refptr<Scrollbar> scrollbar)
...
void NinePatchThumbScrollbarLayer::PushPropertiesTo(LayerImpl* layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
    ...
    if (thumb_resource_.Read(*this)) {
        auto iter = commit_state.ui_resource_sizes.find(thumb_resource_.Read(*this)->id());
        gfx::Size image_bounds = (iter == commit_state.ui_resource_sizes.end()) ? gfx::Size() : iter->second;
...
bool NinePatchThumbScrollbarLayer::Update()
{
    ...
    updated |= UpdateProperty(scrollbar_.Read(*this)->TrackRect(), &track_rect_.Write(*this));
    // Ignore ThumbRect's location because the NinePatchThumbScrollbarLayerImpl
    // will compute it from scroll offset.
    updated |= UpdateProperty(scrollbar_.Read(*this)->ThumbRect().size(), &thumb_size_.Write(*this));
...
bool NinePatchThumbScrollbarLayer::PaintThumbIfNeeded()
{
    ...
    thumb_resource_.Write(*this) = ScopedUIResource::Create(layer_tree_host()->GetUIResourceManager(), UIResourceBitmap(skbitmap));
...
bool NinePatchThumbScrollbarLayer::PaintTickmarks()
{
    ...
    track_and_buttons_resource_.Write(*this) = ScopedUIResource::Create(layer_tree_host()->GetUIResourceManager(), UIResourceBitmap(skbitmap));

    SetNeedsPushProperties();
    return true;
}

ScrollbarLayerBase::ScrollbarLayerType NinePatchThumbScrollbarLayer::GetScrollbarLayerType() const
{
