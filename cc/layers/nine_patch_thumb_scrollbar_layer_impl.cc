...
std::unique_ptr<NinePatchThumbScrollbarLayerImpl> NinePatchThumbScrollbarLayerImpl::Create(
    LayerTreeImpl* tree_impl, int id, ScrollbarOrientation orientation, bool is_left_side_vertical_scrollbar)
{
    return base::WrapUnique(new NinePatchThumbScrollbarLayerImpl(tree_impl, id, orientation, is_left_side_vertical_scrollbar));
}

NinePatchThumbScrollbarLayerImpl::NinePatchThumbScrollbarLayerImpl(
    LayerTreeImpl* tree_impl, int id, ScrollbarOrientation orientation, bool is_left_side_vertical_scrollbar)
    : ScrollbarLayerImplBase(tree_impl, id, orientation, is_left_side_vertical_scrollbar, true)
{
}
...
std::unique_ptr<LayerImpl> NinePatchThumbScrollbarLayerImpl::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
    return NinePatchThumbScrollbarLayerImpl::Create(tree_impl, id(), orientation(), is_left_side_vertical_scrollbar());
}
...
void NinePatchThumbScrollbarLayerImpl::PushPropertiesTo(LayerImpl* layer)
{
    ...
    NinePatchThumbScrollbarLayerImpl* scrollbar_layer = static_cast<NinePatchThumbScrollbarLayerImpl*>(layer);
...
bool NinePatchThumbScrollbarLayerImpl::WillDraw(DrawMode draw_mode, viz::ClientResourceProvider* resource_provider)
{
    DCHECK(draw_mode != DRAW_MODE_RESOURCELESS_SOFTWARE);
    return LayerImpl::WillDraw(draw_mode, resource_provider);
}

void NinePatchThumbScrollbarLayerImpl::AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
...
void NinePatchThumbScrollbarLayerImpl::AppendThumbQuads(
    viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data, viz::SharedQuadState* shared_quad_state)
{
    if (aperture_.IsEmpty())
        return;

    bool is_resource = thumb_ui_resource_id_ && layer_tree_impl()->ResourceIdForUIResource(thumb_ui_resource_id_);
    bool are_contents_opaque = is_resource ? layer_tree_impl()->IsUIResourceOpaque(thumb_ui_resource_id_) || contents_opaque() : false;
    ...
    // and be symmetrical.
    gfx::Rect border(aperture_.x(), aperture_.y(), aperture_.x() * 2, aperture_.y() * 2);
    ...
    if (thumb_quad_rect.height() < border.height() || thumb_quad_rect.width() < border.width())
        return;

    const bool layout_changed
        = quad_generator_.SetLayout(image_bounds_, thumb_quad_rect.size(), aperture_, border, layer_occlusion, fill_center, nearest_neighbor);
    if (layout_changed) {
        quad_generator_.CheckGeometryLimitations();
        patches_ = quad_generator_.GeneratePatches();
    }

    quad_generator_.AppendQuadsForCc(this, thumb_ui_resource_id_, render_pass, shared_quad_state, patches_, thumb_quad_rect.OffsetFromOrigin());
}

void NinePatchThumbScrollbarLayerImpl::AppendTrackAndButtonsQuads(
    viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data, viz::SharedQuadState* shared_quad_state)
{
    viz::ResourceId track_resource_id = layer_tree_impl()->ResourceIdForUIResource(track_and_buttons_ui_resource_id_);
    ...
    gfx::Rect visible_track_quad_rect = draw_properties().occlusion_in_content_space.GetUnoccludedContentRect(track_quad_rect);
    gfx::Rect scaled_visible_track_quad_rect = gfx::ScaleToEnclosingRect(visible_track_quad_rect, 1.f);
    ...
    viz::TextureDrawQuad* quad = render_pass->CreateAndAppendDrawQuad<viz::TextureDrawQuad>();
    quad->SetNew(shared_quad_state, scaled_track_quad_rect, scaled_visible_track_quad_rect, needs_blending, track_resource_id, premultipled_alpha, uv_top_left,
...
{
    return track_length_ + (orientation() == ScrollbarOrientation::kVertical ? vertical_adjust() : 0);
}
