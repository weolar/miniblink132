...
RenderSurfaceImpl::RenderSurfaceImpl(LayerTreeImpl* layer_tree_impl, ElementId id)
...
{
    EffectTree& effect_tree = layer_tree_impl_->property_trees()->effect_tree_mutable();
...
gfx::RectF RenderSurfaceImpl::DrawableContentRect() const
{
    ...
    if (!filters.IsEmpty()) {
        surface_content_rect = filters.MapRect(surface_content_rect, gfx::TransformToFlattenedSkMatrix(SurfaceScale()));
    }
    gfx::RectF drawable_content_rect = MathUtil::MapClippedRect(draw_transform(), gfx::RectF(surface_content_rect));
    if (is_clipped()) {
    ...
    if (std::isnan(drawable_content_rect.x()) || std::isnan(drawable_content_rect.y()) || std::isnan(drawable_content_rect.right())
...
{
    return DebugColors::SurfaceBorderWidth(layer_tree_impl_ ? layer_tree_impl_->device_scale_factor() : 1);
}
...
gfx::Transform RenderSurfaceImpl::SurfaceScale() const
{
    ...
    surface_scale.Scale(OwningEffectNode()->surface_contents_scale.x(), OwningEffectNode()->surface_contents_scale.y());
...
{
    return HasCopyRequest() || ShouldCacheRenderSurface() || SubtreeCaptureId().is_valid() || OwningEffectNode()->view_transition_element_resource_id.IsValid();
}
...
{
    return layer_tree_impl_->property_trees()->effect_tree().Node(EffectTreeIndex());
}

EffectNode* RenderSurfaceImpl::OwningEffectNodeMutableForTest() const
{
    return layer_tree_impl_->property_trees()->effect_tree_mutable().Node(EffectTreeIndex());
}
...
gfx::Rect RenderSurfaceImpl::CalculateExpandedClipForFilters(const gfx::Transform& target_to_surface)
{
    gfx::Rect clip_in_surface_space = MathUtil::ProjectEnclosingClippedRect(target_to_surface, clip_rect());
    gfx::Rect expanded_clip_in_surface_space = Filters().MapRect(clip_in_surface_space, gfx::TransformToFlattenedSkMatrix(SurfaceScale()));
    gfx::Rect expanded_clip_in_target_space = MathUtil::MapEnclosingClippedRect(draw_transform(), expanded_clip_in_surface_space);
...
gfx::Rect RenderSurfaceImpl::CalculateClippedAccumulatedContentRect()
{
    ...
    gfx::Rect accumulated_rect_in_target_space = MathUtil::MapEnclosingClippedRect(draw_transform(), accumulated_content_rect());
    ...
    clipped_accumulated_rect_in_target_space.Intersect(accumulated_rect_in_target_space);

    if (clipped_accumulated_rect_in_target_space.IsEmpty())
        return gfx::Rect();

    gfx::Rect clipped_accumulated_rect_in_local_space = MathUtil::ProjectEnclosingClippedRect(target_to_surface, clipped_accumulated_rect_in_target_space);
...
void RenderSurfaceImpl::CalculateContentRectFromAccumulatedContentRect(int max_texture_size)
{
    ...
    // texture size.
    surface_content_rect.set_width(std::min(surface_content_rect.width(), max_texture_size));
    surface_content_rect.set_height(std::min(surface_content_rect.height(), max_texture_size));
...
void RenderSurfaceImpl::SetContentRectToViewport()
{
    ...
    gfx::Rect viewport = gfx::ToEnclosingRect(layer_tree_impl_->property_trees()->clip_tree().ViewportClip());
...
void RenderSurfaceImpl::AccumulateContentRectFromContributingLayer(LayerImpl* layer)
{
    DCHECK(layer->draws_content());
...
void RenderSurfaceImpl::AccumulateContentRectFromContributingRenderSurface(RenderSurfaceImpl* contributing_surface)
{
    ...
    if (contributing_surface->OwningEffectNode()->view_transition_element_resource_id.IsValid())
        return;
    ...
    accumulated_content_rect_.Union(gfx::ToEnclosedRect(contributing_surface->DrawableContentRect()));
}
...
bool RenderSurfaceImpl::AncestorPropertyChanged() const
{
    ...
    return ancestor_property_changed_ || property_trees->full_tree_damaged() || property_trees->transform_tree().Node(TransformTreeIndex())->transform_changed
...
RenderSurfacePropertyChangedFlags RenderSurfaceImpl::GetPropertyChangeFlags() const
{
    return { surface_property_changed_, ancestor_property_changed_ };
}

void RenderSurfaceImpl::ApplyPropertyChangeFlags(const RenderSurfacePropertyChangedFlags& flags)
{
...
{
    std::unique_ptr<viz::CompositorRenderPass> pass = viz::CompositorRenderPass::Create(num_contributors_);
    gfx::Rect damage_rect = GetDamageRect();
    damage_rect.Intersect(content_rect());
    pass->SetNew(render_pass_id(), content_rect(), damage_rect, draw_properties_.screen_space_transform);
    ...
    pass->has_damage_from_contributing_content = HasDamageFromeContributingContent();
    pass->view_transition_element_resource_id = OwningEffectNode()->view_transition_element_resource_id;
    return pass;
}

void RenderSurfaceImpl::AppendQuads(DrawMode draw_mode, viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data)
{
    gfx::Rect unoccluded_content_rect = occlusion_in_content_space().GetUnoccludedContentRect(content_rect());
    ...
    const PropertyTrees* property_trees = layer_tree_impl_->property_trees();
    int sorting_context_id = property_trees->transform_tree().Node(TransformTreeIndex())->sorting_context_id;
    ...
    shared_quad_state->SetAll(draw_transform(), content_rect(), content_rect(), mask_filter_info(), clip_rect, contents_opaque, draw_properties_.draw_opacity,
        BlendMode(), sorting_context_id, /*layer_id=*/0u, is_fast_rounded_corner());

    if (layer_tree_impl_->debug_state().show_debug_borders.test(DebugBorderType::RENDERPASS)) {
        auto* debug_border_quad = render_pass->CreateAndAppendDrawQuad<viz::DebugBorderDrawQuad>();
        debug_border_quad->SetNew(shared_quad_state, content_rect(), unoccluded_content_rect, GetDebugBorderColor(), GetDebugBorderWidth());
    }
    ...
    if (draw_mode != DRAW_MODE_RESOURCELESS_SOFTWARE && mask_layer && mask_layer->draws_content() && !mask_layer->bounds().IsEmpty()) {
        ...
        // mask layers.
        DCHECK(BlendMode() != SkBlendMode::kDstIn) << "kDstIn blend mode with mask layer is unsupported.";
        TRACE_EVENT1("cc", "RenderSurfaceImpl::AppendQuads", "mask_layer_gpu_memory_usage", mask_layer->GPUMemoryUsageInBytes());

        gfx::SizeF mask_uv_size;
        mask_layer->GetContentsResourceId(&mask_resource_id, &mask_texture_size, &mask_uv_size);
        gfx::SizeF unclipped_mask_target_size = gfx::ScaleSize(gfx::SizeF(mask_layer->bounds()), surface_contents_scale.x(), surface_contents_scale.y());
        gfx::Vector2dF mask_offset = gfx::ScaleVector2d(mask_layer->offset_to_transform_parent(), surface_contents_scale.x(), surface_contents_scale.y());
        ...
        mask_uv_rect = gfx::ScaleRect(
            ...
            gfx::RectF(content_rect()) - mask_offset, mask_uv_size.width() / unclipped_mask_target_size.width(),
            mask_uv_size.height() / unclipped_mask_target_size.height());
    }

    gfx::RectF tex_coord_rect(gfx::Rect(content_rect().size()));
    auto* quad = render_pass->CreateAndAppendDrawQuad<viz::CompositorRenderPassDrawQuad>();
    quad->SetAll(shared_quad_state, content_rect(), unoccluded_content_rect,
        /*needs_blending=*/true, render_pass_id(), mask_resource_id, mask_uv_rect, mask_texture_size, surface_contents_scale, gfx::PointF(), tex_coord_rect,
        !layer_tree_impl_->settings().enable_edge_anti_aliasing, OwningEffectNode()->backdrop_filter_quality, intersects_damage_under_);
}

bool RenderSurfaceImpl::ShouldClip() const
{
    return !HasCopyRequest() && !ShouldCacheRenderSurface() && !OwningEffectNode()->view_transition_element_resource_id.IsValid();
}
