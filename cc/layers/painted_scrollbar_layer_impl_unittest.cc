...
TEST(PaintedScrollbarLayerImplTest, Occlusion)
{
    ...
    impl.host_impl()->CreateUIResource(track_and_buttons_uid, track_and_buttons_bitmap);

    ScrollbarOrientation orientation = ScrollbarOrientation::kVertical;

    PaintedScrollbarLayerImpl* scrollbar_layer_impl = impl.AddLayerInActiveTree<PaintedScrollbarLayerImpl>(orientation, false, false);
    scrollbar_layer_impl->SetBounds(layer_size);
    scrollbar_layer_impl->SetContentsOpaque(true);
    scrollbar_layer_impl->set_internal_contents_scale_and_bounds(scale, scaled_layer_size);
    ...
    scrollbar_layer_impl->set_track_and_buttons_ui_resource_id(track_and_buttons_uid);
...

    {
        ...
        const viz::DrawQuad* track_and_buttons_draw_quad = impl.quad_list().ElementAt(1);

        EXPECT_EQ(viz::DrawQuad::Material::kTextureContent, thumb_draw_quad->material);
        EXPECT_EQ(viz::DrawQuad::Material::kTextureContent, track_and_buttons_draw_quad->material);

        const viz::TextureDrawQuad* thumb_quad = viz::TextureDrawQuad::MaterialCast(thumb_draw_quad);
        const viz::TextureDrawQuad* track_and_buttons_quad = viz::TextureDrawQuad::MaterialCast(track_and_buttons_draw_quad);
        ...
        EXPECT_EQ(scrollbar_layer_impl->contents_opaque(), track_and_buttons_quad->shared_quad_state->are_contents_opaque);
...
TEST(PaintedScrollbarLayerImplTest, Occlusion_SolidColorThumbNinePatchTrack)
{
...

    {
        ...
        const viz::SolidColorDrawQuad* thumb_quad = viz::SolidColorDrawQuad::MaterialCast(thumb_draw_quad);
        const viz::TextureDrawQuad* track_and_buttons_quad = viz::TextureDrawQuad::MaterialCast(track_and_buttons_draw_quad);
...
TEST(PaintedScrollbarLayerImplTest, SolidColorThumbNinePatchTrackUnderClipAndScale)
{
    gfx::Size layer_size(15, 1000);
    ...
    const viz::SolidColorDrawQuad* thumb_quad = viz::SolidColorDrawQuad::MaterialCast(impl.quad_list().ElementAt(0));
    EXPECT_EQ(thumb_quad->rect, scaled_thumb_rect);
    EXPECT_EQ(thumb_quad->visible_rect, scaled_thumb_rect);
    EXPECT_FALSE(thumb_quad->needs_blending);
    EXPECT_FALSE(thumb_quad->shared_quad_state->are_contents_opaque);
    ...
    const viz::TextureDrawQuad* track_and_buttons_quad = viz::TextureDrawQuad::MaterialCast(impl.quad_list().ElementAt(1));
...
TEST(PaintedScrollbarLayerImplTest, PaintedOpacityChangesInvalidate)
{
    ...
    ScrollbarOrientation orientation = ScrollbarOrientation::kVertical;
    PaintedScrollbarLayerImpl* scrollbar_layer_impl = impl.AddLayerInActiveTree<PaintedScrollbarLayerImpl>(orientation, false, false);
    EXPECT_FALSE(scrollbar_layer_impl->LayerPropertyChangedNotFromPropertyTrees());
...
protected:
    ...
    PaintedScrollbarLayerImpl* SetUpScrollbarLayerImpl(ScrollbarOrientation orientation, bool is_left_side_vertical_scrollbar, const gfx::Rect& track_rect,
        int thumb_thickness, int thumb_length, bool is_overlay = false)
    {
        PaintedScrollbarLayerImpl* scrollbar_layer_impl
            = impl_->AddLayerInActiveTree<PaintedScrollbarLayerImpl>(orientation, is_left_side_vertical_scrollbar, is_overlay);
...
TEST_F(PaintedScrollbarLayerImplSolidColorThumbTest, ComputeThumbQuadRect)
{
    const PaintedScrollbarLayerImpl* scrollbar_layer_impl_vertical
        = SetUpScrollbarLayerImpl(ScrollbarOrientation::kVertical, false, gfx::Rect(0, 0, 14, 100), 6, 30);
    EXPECT_EQ(scrollbar_layer_impl_vertical->ComputeThumbQuadRect(), gfx::Rect(4, 0, 6, 30));

    PaintedScrollbarLayerImpl* scrollbar_layer_impl_vertical_left
        = SetUpScrollbarLayerImpl(ScrollbarOrientation::kVertical, true, gfx::Rect(0, 0, 14, 100), 6, 30);
    scrollbar_layer_impl_vertical_left->SetBounds(gfx::Size(14, 100));
    EXPECT_EQ(scrollbar_layer_impl_vertical_left->ComputeThumbQuadRect(), gfx::Rect(4, 0, 6, 30));

    const PaintedScrollbarLayerImpl* scrollbar_layer_impl_horizontal
        = SetUpScrollbarLayerImpl(ScrollbarOrientation::kHorizontal, false, gfx::Rect(0, 0, 100, 14), 6, 30);
    EXPECT_EQ(scrollbar_layer_impl_horizontal->ComputeThumbQuadRect(), gfx::Rect(0, 4, 30, 6));
}

TEST_F(PaintedScrollbarLayerImplSolidColorThumbTest, ComputeHitTestableThumbQuadRect)
{
    const PaintedScrollbarLayerImpl* scrollbar_layer_impl_vertical
        = SetUpScrollbarLayerImpl(ScrollbarOrientation::kVertical, false, gfx::Rect(0, 0, 14, 100), 6, 30);
    EXPECT_EQ(scrollbar_layer_impl_vertical->ComputeHitTestableThumbQuadRect(), gfx::Rect(0, 0, 14, 30));

    PaintedScrollbarLayerImpl* scrollbar_layer_impl_vertical_left
        = SetUpScrollbarLayerImpl(ScrollbarOrientation::kVertical, true, gfx::Rect(0, 0, 14, 100), 6, 30);
    scrollbar_layer_impl_vertical_left->SetBounds(gfx::Size(14, 100));
    EXPECT_EQ(scrollbar_layer_impl_vertical_left->ComputeHitTestableThumbQuadRect(), gfx::Rect(0, 0, 14, 30));

    const PaintedScrollbarLayerImpl* scrollbar_layer_impl_horizontal
        = SetUpScrollbarLayerImpl(ScrollbarOrientation::kHorizontal, false, gfx::Rect(0, 0, 100, 14), 6, 30);
    EXPECT_EQ(scrollbar_layer_impl_horizontal->ComputeHitTestableThumbQuadRect(), gfx::Rect(0, 0, 30, 14));
}

class PaintedScrollbarLayerImplFluentOverlayTest : public PaintedScrollbarLayerImplSolidColorThumbTest {
protected:
    void SetUp() override
    {
...
TEST_F(PaintedScrollbarLayerImplFluentOverlayTest, OverlayFluentThumbHasNoMargin)
{
    {
        const PaintedScrollbarLayerImpl* scrollbar_layer_impl = SetUpScrollbarLayerImpl(ScrollbarOrientation::kVertical,
            /*is_left_side_vertical_scrollbar=*/false, gfx::Rect(0, 0, 14, 100), 6, 30,
            /*is_overlay=*/true);
        gfx::Rect thumb = scrollbar_layer_impl->ComputeHitTestableExpandedThumbQuadRect();
        EXPECT_EQ(scrollbar_layer_impl->bounds().width(), thumb.width());
    }
    {
        const PaintedScrollbarLayerImpl* scrollbar_layer_impl = SetUpScrollbarLayerImpl(ScrollbarOrientation::kHorizontal,
            /*is_left_side_vertical_scrollbar=*/false, gfx::Rect(0, 0, 100, 14), 6, 30,
            /*is_overlay=*/true);
        gfx::Rect thumb = scrollbar_layer_impl->ComputeHitTestableExpandedThumbQuadRect();
...
TEST_F(PaintedScrollbarLayerImplFluentOverlayTest, NoThumbColorDoesntAppendQuads)
{
    PaintedScrollbarLayerImpl* scrollbar_layer_impl = impl_->AddLayerInActiveTree<PaintedScrollbarLayerImpl>(ScrollbarOrientation::kVertical,
    ...
    // the minimum.
    scrollbar_layer_impl->SetThumbThickness(scrollbar_layer_impl->GetIdleThicknessScale());
