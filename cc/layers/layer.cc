...
struct SameSizeAsLayer : public base::RefCounted<SameSizeAsLayer>, public ProtectedSequenceSynchronizer {
private:
    SameSizeAsLayer();
...
static_assert(sizeof(Layer) == sizeof(SameSizeAsLayer), "Layer should stay small");
...
void Layer::SetLayerTreeHost(LayerTreeHost* host)
{
    ...
    if (host && !host->IsUsingLayerLists() && host->mutator_host()->IsElementAnimating(element_id)) {
        host->SetNeedsCommit();
    }
}
...
void Layer::CaptureContent(const gfx::Rect& rect, std::vector<NodeInfo>* content) const
{
}
...
void Layer::InsertChild(scoped_refptr<Layer> child, size_t index)
{
    ...
    AddDrawableDescendants(child->NumDescendantsThatDrawContent() + (child->draws_content() ? 1 : 0));
    ...
    if (layer_tree_inputs && layer_tree_inputs->mask_layer && index && index == inputs.children.size()) {
...
void Layer::RemoveChild(Layer* child, RemovalReason reason)
{
    ...
    auto& inputs = inputs_.Write(*this);
    for (auto iter = inputs.children.begin(); iter != inputs.children.end(); ++iter) {
        if (iter->get() != child)
            continue;
...
bool Layer::SetBitFlag(bool new_value, uint8_t mask, bool invalidate, bool needs_push)
{
...
void Layer::ReplaceChild(Layer* reference, scoped_refptr<Layer> new_layer)
{
    ...
    auto& inputs = inputs_.Write(*this);
    auto reference_it = base::ranges::find(inputs.children, reference, &scoped_refptr<Layer>::get);
...
void Layer::SetBounds(const gfx::Size& size)
{
    ...
    // size of this layer.
    if (!IsUsingLayerLists()) {
        if (subtree_capture_id().is_valid() || masks_to_bounds() || mask_layer() || HasMaskFilter()) {
            SetSubtreePropertyChanged();
            SetPropertyTreesNeedRebuild();
        }

        if (scrollable()) {
            auto& scroll_tree = layer_tree_host()->property_trees()->scroll_tree_mutable();
...
void Layer::RequestCopyOfOutput(std::unique_ptr<viz::CopyOutputRequest> request)
{
    ...
    if (request->has_source()) {
        ...
        auto it = base::ranges::find_if(
            inputs.copy_requests, [&source](const std::unique_ptr<viz::CopyOutputRequest>& x) { return x->has_source() && x->source() == source; });
...
void Layer::SetClipRect(const gfx::Rect& clip_rect)
{
    ...
    if (clip_tree_index() != kInvalidPropertyNodeId && !force_rebuild) {
        ...
        if (ClipNode* node = property_trees->clip_tree_mutable().Node(clip_tree_index())) {
        ...
        if (HasMaskFilter() && effect_tree_index() != kInvalidPropertyNodeId) {
            if (EffectNode* node = property_trees->effect_tree_mutable().Node(effect_tree_index())) {
                node->mask_filter_info = gfx::MaskFilterInfo(effective_clip_rect, corner_radii(), gradient_mask());
...
void Layer::UpdateMaskFilterInfo(const gfx::RoundedCornersF* corner_radii, const gfx::LinearGradient* gradient_mask)
{
    DCHECK(IsPropertyChangeAllowed());
    auto& inputs = EnsureLayerTreeInputs();
    ...
    SetSubtreePropertyChanged();
    SetNeedsCommit();
    PropertyTrees* property_trees = IsAttached() ? layer_tree_host()->property_trees() : nullptr;
    EffectNode* node = nullptr;
    if (property_trees && effect_tree_index() != kInvalidPropertyNodeId && (node = property_trees->effect_tree_mutable().Node(effect_tree_index()))) {
        gfx::RectF effective_clip_rect = EffectiveClipRect();
        effective_clip_rect += offset_to_transform_parent();
        node->mask_filter_info = gfx::MaskFilterInfo(effective_clip_rect, inputs.corner_radii, inputs.gradient_mask);
        node->effect_changed = true;
        property_trees->effect_tree_mutable().set_needs_update(true);
    } else {
        SetPropertyTreesNeedRebuild();
...
float Layer::EffectiveOpacity() const
{
    if (!layer_tree_inputs())
        return 1.0f;
    return layer_tree_inputs()->hide_layer_and_subtree ? 0.f : layer_tree_inputs()->opacity;
}
...
{
    SetHitTestOpaqueness(hit_testable ? HitTestOpaqueness::kMixed : HitTestOpaqueness::kTransparent);
}
...
void Layer::SetPosition(const gfx::PointF& position)
{
    ...
    if (has_transform_node()) {
        TransformNode* transform_node = layer_tree_host()->property_trees()->transform_tree_mutable().Node(transform_tree_index_.Read(*this));
        ...
        layer_tree_host()->property_trees()->transform_tree_mutable().set_needs_update(true);
    } else {
        SetPropertyTreesNeedRebuild();
    }
...
void Layer::SetTransform(const gfx::Transform& transform)
{
    ...
    if (IsAttached()) {
        if (has_transform_node()) {
            ...
            bool preserves_2d_axis_alignment = Are2dAxisAligned(inputs.transform, transform);
...
void Layer::UpdatePropertyTreeScrollOffset()
{
    ...
    property_trees.scroll_tree_mutable().SetScrollOffset(element_id(), scroll_offset());
    auto* transform_node = property_trees.transform_tree_mutable().Node(transform_tree_index());
    DCHECK_EQ(transform_tree_index(), transform_node->id);
...
void Layer::SetDidScrollCallback(base::RepeatingCallback<void(const gfx::PointF&, const ElementId&)> callback)
{
...
void Layer::SetSubtreeCaptureId(viz::SubtreeCaptureId subtree_id)
{
    ...
    DCHECK(!inputs.subtree_capture_id.is_valid() || !subtree_id.is_valid()) << "Not allowed to change from a valid ID to another valid ID, as it may "
                                                                               "already be in use.";
...
void Layer::SetMainThreadScrollHitTestRegion(const Region& region)
{
    ...
    if (!rare_inputs && region.IsEmpty())
        return;
    if (rare_inputs && rare_inputs->main_thread_scroll_hit_test_region == region) {
        return;
    }
...
void Layer::SetNonCompositedScrollHitTestRects(std::vector<ScrollHitTestRect> rects)
{
    DCHECK(IsPropertyChangeAllowed());
    const auto& rare_inputs = inputs_.Read(*this).rare_inputs;
    ...
    if (rare_inputs && rare_inputs->non_composited_scroll_hit_test_rects == rects) {
        return;
    }
...
RenderSurfaceReason Layer::GetRenderSurfaceReason() const
{
    ...
    const EffectNode* effect_node = property_trees->effect_tree().Node(this->effect_tree_index());
    ...
    if (!effect_node || (parent_.Read(*this) && this->effect_tree_index() == parent_.Read(*this)->effect_tree_index())) {
        return RenderSurfaceReason::kNone;
    }
...
{
    if (property_trees.sequence_number() != property_tree_sequence_number_.Read(*this)) {
        return kInvalidPropertyNodeId;
    }
    return transform_tree_index_.Read(*this);
}

bool Layer::transform_tree_index_is_valid(const PropertyTrees& property_trees) const
{
    return transform_tree_index_.Read(*this) != kInvalidPropertyNodeId && property_trees.sequence_number() == property_tree_sequence_number_.Read(*this);
}
...
bool Layer::clip_tree_index_is_valid(const PropertyTrees& property_trees) const
{
    return clip_tree_index_.Read(*this) != kInvalidPropertyNodeId && property_trees.sequence_number() == property_tree_sequence_number_.Read(*this);
}
...
bool Layer::effect_tree_index_is_valid(const PropertyTrees& property_trees) const
{
    return effect_tree_index_.Read(*this) != kInvalidPropertyNodeId && property_trees.sequence_number() == property_tree_sequence_number_.Read(*this);
}
...
bool Layer::scroll_tree_index_is_valid(const PropertyTrees& property_trees) const
{
    return scroll_tree_index_.Read(*this) != kInvalidPropertyNodeId && property_trees.sequence_number() == property_tree_sequence_number_.Read(*this);
}
...
{
    return base::StringPrintf("layer_id: %d\n"
                              "  name: %s\n"
                              "  Bounds: %s\n"
                              "  ElementId: %s\n"
                              "  HitTestOpaqueness: %s\n"
        ...
        id(), DebugName().c_str(), bounds().ToString().c_str(), element_id().ToString().c_str(), HitTestOpaquenessToString(hit_test_opaqueness()),
        offset_to_transform_parent().ToString().c_str(), clip_tree_index(), effect_tree_index(), scroll_tree_index(), transform_tree_index());
}
...
void Layer::SetNeedsDisplayRect(const gfx::Rect& dirty_rect)
{
    ...
    if (draws_content() && IsAttached() && !ignore_set_needs_commit_for_test_.Read(*this))
...
void Layer::PushPropertiesTo(LayerImpl* layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "Layer::PushPropertiesTo");
    DCHECK(IsAttached());
    ...
    if (unsafe_state.mutator_host->ScrollOffsetAnimationWasInterrupted(element_id())) {
        PropertyTrees* trees = layer->layer_tree_impl()->property_trees();
        trees->scroll_tree_mutable().SetScrollOffsetClobberActiveValue(layer->element_id());
    }
    ...
    if (inputs.rare_inputs) {
        layer->SetMainThreadScrollHitTestRegion(inputs.rare_inputs->main_thread_scroll_hit_test_region);
        layer->SetNonCompositedScrollHitTestRects(inputs.rare_inputs->non_composited_scroll_hit_test_rects);
...
void Layer::TakeCopyRequests(std::vector<std::unique_ptr<viz::CopyOutputRequest>>* requests)
{
    if (!layer_tree_inputs())
        return;

    auto& layer_tree_inputs = layer_tree_inputs_.Write(*this);
    for (std::unique_ptr<viz::CopyOutputRequest>& request : layer_tree_inputs->copy_requests) {
        // Ensure the result callback is not invoked on the compositing thread.
        if (!request->has_result_task_runner()) {
            request->set_result_task_runner(layer_tree_host()->GetTaskRunnerProvider()->MainThreadTaskRunner());
        }
        if (request->has_area()) {
            request->set_area(gfx::IntersectRects(request->area(), gfx::Rect(bounds())));
        }
...
std::unique_ptr<LayerImpl> Layer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
void Layer::SetElementId(ElementId id)
{
    ...
    if (inputs_.Read(*this).element_id == id)
        return;
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "Layer::SetElementId", "element", id.ToString());
    auto& inputs = inputs_.Write(*this);
...
gfx::Transform Layer::ScreenSpaceTransform() const
{
    ...
    return draw_property_utils::ScreenSpaceTransform(this, layer_tree_host()->property_trees()->transform_tree());
}
