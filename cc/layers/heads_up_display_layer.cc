    ...
    : typeface_(skia::MakeTypefaceFromName("Arial", SkFontStyle()))
{
    if (!typeface_.Read(*this)) {
        typeface_.Write(*this) = skia::MakeTypefaceFromName("monospace", SkFontStyle::Bold());
    }
...
void HeadsUpDisplayLayer::SetLayerTreeHost(LayerTreeHost* host)
{
    if (host && host != layer_tree_host()) {
        paused_debugger_message_ = host->client()->GetPausedDebuggerLocalizedMessage();
    }
    Layer::SetLayerTreeHost(host);
}

void HeadsUpDisplayLayer::UpdateLocationAndSize(const gfx::Size& device_viewport, float device_scale_factor)
{
    float multiplier = 1.f / (device_scale_factor * layer_tree_host()->painted_device_scale_factor());
    gfx::Size device_viewport_in_dips = gfx::ScaleToFlooredSize(device_viewport, multiplier);
    ...
    if (layer_tree_host()->GetDebugState().ShowDebugRects() || layer_tree_host()->GetDebugState().debugger_paused) {
    ...
    auto bounds_in_layout_space = gfx::ScaleToCeiledSize(bounds_in_dips, layer_tree_host()->painted_device_scale_factor());
...
std::unique_ptr<LayerImpl> HeadsUpDisplayLayer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
    return HeadsUpDisplayLayerImpl::Create(tree_impl, id(), paused_debugger_message_);
}
...
void HeadsUpDisplayLayer::SetLayoutShiftRects(const std::vector<gfx::Rect>& rects)
{
    layout_shift_rects_.Write(*this) = rects;
}

void HeadsUpDisplayLayer::PushPropertiesTo(LayerImpl* layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
