...
TEST(TextureLayerImplTest, VisibleOpaqueRegion)
{
    ...
    auto resource = viz::TransferableResource::MakeGpu(gpu::Mailbox::Generate(), GL_TEXTURE_2D,
        gpu::SyncToken(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x234), 0x456), layer_bounds,
...
TEST(TextureLayerImplTest, Occlusion)
{
    ...
    auto resource = viz::TransferableResource::MakeGpu(gpu::Mailbox::Generate(), GL_TEXTURE_2D,
        gpu::SyncToken(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x234), 0x456), layer_size,
        viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);

    TextureLayerImpl* texture_layer_impl = impl.AddLayerInActiveTree<TextureLayerImpl>();
    texture_layer_impl->SetBounds(layer_size);
    texture_layer_impl->SetDrawsContent(true);
    texture_layer_impl->SetTransferableResource(resource, base::BindOnce(&IgnoreCallback));
