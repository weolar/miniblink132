...
bool ScrollbarLayerImplBase::CanScrollOrientation() const
{
    ...
    const auto* scroll_node = property_trees->scroll_tree().FindNodeFromElementId(scroll_element_id_);
    DCHECK(scroll_node);
    ...
    return !MathUtil::IsFloatNearlyTheSame(clip_layer_length(), scroll_layer_length()) && clip_layer_length() < scroll_layer_length();
}
...
gfx::Rect ScrollbarLayerImplBase::ComputeThumbQuadRectWithThumbThicknessScale(float thumb_thickness_scale_factor) const
{
    ...
    float thumb_thickness_adjustment = thumb_thickness * (1.f - thumb_thickness_scale_factor);

    gfx::RectF thumb_rect;
    if (orientation_ == ScrollbarOrientation::kHorizontal) {
        thumb_rect = gfx::RectF(thumb_offset, vertical_adjust_ + thumb_thickness_adjustment, thumb_length, thumb_thickness - thumb_thickness_adjustment);
    } else {
        thumb_rect = gfx::RectF(is_left_side_vertical_scrollbar_ ? bounds().width() - thumb_thickness : thumb_thickness_adjustment, thumb_offset,
            thumb_thickness - thumb_thickness_adjustment, thumb_length);
    }

    return gfx::ToEnclosingRect(thumb_rect);
}

gfx::Rect ScrollbarLayerImplBase::ComputeHitTestableExpandedThumbQuadRect() const
{
...
{
    return ComputeThumbQuadRectWithThumbThicknessScale(thumb_thickness_scale_factor_);
}
...
void ScrollbarLayerImplBase::SetOverlayScrollbarLayerOpacityAnimated(float opacity, bool fade_out_animation)
{
    DCHECK(is_overlay_scrollbar());
    if (!layer_tree_impl())
        return;
    ...
    EffectNode* node = property_trees->effect_tree_mutable().Node(effect_tree_index());
...
LayerTreeSettings::ScrollbarAnimator ScrollbarLayerImplBase::GetScrollbarAnimator() const
{
...
void ScrollbarLayerImplBase::SetHasFindInPageTickmarks(bool has_find_in_page_tickmarks)
{
...
ScrollbarPart ScrollbarLayerImplBase::IdentifyScrollbarPart(const gfx::PointF position_in_widget) const
{
    const gfx::Point pointer_location(position_in_widget.x(), position_in_widget.y());
