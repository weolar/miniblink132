...
TEST(TouchActionRegionTest, GetAllowedTouchActionMapOverlapToZero)
{
    ...
    EXPECT_EQ(TouchAction::kPanLeft, touch_action_region.GetAllowedTouchAction(gfx::Point(10, 10)));
    // The point is in both PanLeft and PanRight, and those actions have no
    // common components, so the result is None.
    EXPECT_EQ(TouchAction::kNone, touch_action_region.GetAllowedTouchAction(gfx::Point(30, 30)));
    // The point is in neither PanLeft nor PanRight, so the result is Auto since
    // the default touch action is auto.
    EXPECT_EQ(TouchAction::kAuto, touch_action_region.GetAllowedTouchAction(gfx::Point(60, 60)));
}
...
TEST(TouchActionRegionTest, GetAllowedTouchActionMapOverlapToNonZero)
{
    ...
    EXPECT_EQ(TouchAction::kPanX, touch_action_region.GetAllowedTouchAction(gfx::Point(10, 10)));
    // The point is in both PanX and PanRight, and PanRight is a common component,
    // so the result is PanRight.
    EXPECT_EQ(TouchAction::kPanRight, touch_action_region.GetAllowedTouchAction(gfx::Point(30, 30)));
...
TEST(TouchActionRegionTest, GetAllowedTouchActionEmptyMap)
{
    ...
    // action is auto.
    EXPECT_EQ(TouchAction::kAuto, touch_action_region.GetAllowedTouchAction(gfx::Point(10, 10)));
}
...
TEST(TouchActionRegionTest, GetAllowedTouchActionSingleMapEntry)
{
    ...
    EXPECT_EQ(TouchAction::kPanUp, touch_action_region.GetAllowedTouchAction(gfx::Point(10, 10)));
