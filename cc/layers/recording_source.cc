...
void RecordingSource::UpdateInvalidationForRecordedBounds(const gfx::Rect& old_recorded_bounds, const gfx::Rect& new_recorded_bounds, Region& invalidation)
{
...
void RecordingSource::FinishDisplayItemListUpdate()
{
    ...
    directly_composited_image_info_ = display_list_->GetDirectlyCompositedImageInfo();
...
bool RecordingSource::Update(const gfx::Size& layer_size, float recording_scale_factor, ContentLayerClient& client, Region& invalidation)
{
    ...
    if (display_list_ && display_list->NeedsAdditionalInvalidationForLCDText(*display_list_)) {
        invalidation = layer_rect;
    } else if (new_recorded_bounds != recorded_bounds_) {
        UpdateInvalidationForRecordedBounds(recorded_bounds_, new_recorded_bounds, invalidation);
    }
...
void RecordingSource::DetermineIfSolidColor()
{
    ...
    TRACE_EVENT1("cc", "RecordingSource::DetermineIfSolidColor", "opcount", display_list_->TotalOpCount());
    is_solid_color_
        = display_list_->GetColorIfSolidInRect(gfx::ScaleToRoundedRect(gfx::Rect(size_), recording_scale_factor_), &solid_color_, kMaxOpsToAnalyzeForLayer);
}
