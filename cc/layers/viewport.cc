...
void Viewport::Pan(const gfx::Vector2dF& delta)
{
    ...
    scroll_tree().ScrollBy(*InnerScrollNode(), pending_delta, host_impl_->active_tree());
}

Viewport::ScrollResult Viewport::ScrollBy(const gfx::Vector2dF& physical_delta, const gfx::Point& viewport_point, bool is_direct_manipulation,
    bool affect_browser_controls, bool scroll_outer_viewport)
{
    if (!OuterScrollNode())
        return ScrollResult();
    ...
    if (affect_browser_controls && ShouldBrowserControlsConsumeScroll(physical_delta))
    ...
    gfx::Vector2dF inner_delta
        = host_impl_->GetInputHandler().ScrollSingleNode(*InnerScrollNode(), pending_scroll_node_delta, viewport_point, is_direct_manipulation);
    ...
    if (scroll_outer_viewport) {
        outer_delta = host_impl_->GetInputHandler().ScrollSingleNode(*OuterScrollNode(), pending_scroll_node_delta, viewport_point, is_direct_manipulation);
    ...
    result.consumed_delta = physical_delta - AdjustOverscroll(pending_scroll_node_delta);
    result.content_scrolled_delta = scroll_node_delta - pending_scroll_node_delta;
    return result;
}

bool Viewport::CanScroll(const ScrollNode& node, const ScrollState& scroll_state) const
{
    DCHECK(ShouldScroll(node));

    bool result = host_impl_->GetInputHandler().CanConsumeDelta(scroll_state, *InnerScrollNode());
    ...
    result |= host_impl_->GetInputHandler().CanConsumeDelta(scroll_state, *OuterScrollNode());
    return result;
}
...
void Viewport::SnapIfNeeded()
{
    ...
    SnapPositionData snap = data.FindSnapPosition(*SnapSelectionStrategy::CreateForTargetElement(current_position));
...
gfx::Vector2dF Viewport::ComputeClampedDelta(const gfx::Vector2dF& scroll_delta) const
{
    ...
    gfx::Vector2dF inner_delta = host_impl_->GetInputHandler().ComputeScrollDelta(*inner_node, scroll_delta);
    ...
    gfx::Vector2dF outer_delta = host_impl_->GetInputHandler().ComputeScrollDelta(*outer_node, remaining_delta);
...
{
    DCHECK(InnerScrollNode());
    gfx::Vector2dF unused_delta = scroll_tree().ScrollBy(*InnerScrollNode(), delta, host_impl_->active_tree());
...
bool Viewport::ShouldAnimateViewport(const gfx::Vector2dF& viewport_delta, const gfx::Vector2dF& pending_delta)
{
...
Viewport::ScrollResult Viewport::ScrollAnimated(const gfx::Vector2dF& delta, base::TimeDelta delayed_by)
{
    auto* outer_node = OuterScrollNode();
    if (!outer_node)
    ...
    ScrollNode* inner_node = InnerScrollNode();
    gfx::Vector2dF inner_delta = host_impl_->GetInputHandler().ComputeScrollDelta(*inner_node, delta);

    gfx::Vector2dF pending_delta = scaled_delta - inner_delta;
    pending_delta.Scale(scale_factor);

    gfx::Vector2dF outer_delta = host_impl_->GetInputHandler().ComputeScrollDelta(*outer_node, pending_delta);
...
void Viewport::PinchUpdate(float magnify_delta, const gfx::Point& anchor)
{
    ...
    move -= scroll_tree().ClampScrollToMaxScrollOffset(*InnerScrollNode(), host_impl_->active_tree());
...
void Viewport::PinchEnd(const gfx::Point& anchor, bool snap_to_min)
{
    if (snap_to_min) {
        ...
        const base::TimeDelta kSnapToMinZoomAnimationDuration = base::Milliseconds(200);
        float page_scale = active_tree->current_page_scale_factor();
        float min_scale = active_tree->min_page_scale_factor();

        // If the page is close to minimum scale at pinch end, snap to minimum.
        if (page_scale < min_scale * kMaxZoomForSnapToMin && page_scale != min_scale) {
            gfx::PointF adjusted_anchor = gfx::PointF(anchor + pinch_anchor_adjustment_);
            adjusted_anchor = gfx::ScalePoint(adjusted_anchor, min_scale / page_scale);
            adjusted_anchor += TotalScrollOffset().OffsetFromOrigin();
            host_impl_->StartPageScaleAnimation(gfx::ToRoundedPoint(adjusted_anchor), true, min_scale, kSnapToMinZoomAnimationDuration);
        }
    }
...
bool Viewport::ShouldScroll(const ScrollNode& scroll_node) const
{
    ...
    return scroll_node.scrolls_inner_viewport || scroll_node.scrolls_outer_viewport;
}
...
bool Viewport::ShouldBrowserControlsConsumeScroll(const gfx::Vector2dF& scroll_delta) const
{
...
gfx::PointF Viewport::TotalScrollOffset() const
{
    ...
    gfx::Vector2dF offset = scroll_tree().current_scroll_offset(InnerScrollNode()->element_id).OffsetFromOrigin();

    if (auto* outer_node = OuterScrollNode()) {
        offset += scroll_tree().current_scroll_offset(outer_node->element_id).OffsetFromOrigin();
    }
