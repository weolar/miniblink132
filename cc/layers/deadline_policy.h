...
public:
    enum Type { kUseExistingDeadline, kUseDefaultDeadline, kUseSpecifiedDeadline, kUseInfiniteDeadline };
    ...
    {
        DCHECK(policy_type_ == Type::kUseDefaultDeadline || policy_type_ == Type::kUseSpecifiedDeadline || policy_type_ == Type::kUseInfiniteDeadline);
        return deadline_in_frames_;
    }

    Type policy_type() const
    {
        return policy_type_;
    }
    ...
    {
        return other.policy_type_ == policy_type_ && other.deadline_in_frames_ == deadline_in_frames_;
    }
...
private:
    explicit DeadlinePolicy(Type policy_type, std::optional<uint32_t> deadline_in_frames = std::nullopt);
