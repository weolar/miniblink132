...
TEST(SurfaceLayerImplTest, Occlusion)
{
    ...
    const viz::LocalSurfaceId kArbitraryLocalSurfaceId(9, base::UnguessableToken::Create());

    LayerTreeImplTestBase impl;

    SurfaceLayerImpl* surface_layer_impl = impl.AddLayerInActiveTree<SurfaceLayerImpl>();
    surface_layer_impl->SetBounds(layer_size);
    surface_layer_impl->SetDrawsContent(true);
    viz::SurfaceId surface_id(kArbitraryFrameSinkId, kArbitraryLocalSurfaceId);
    surface_layer_impl->SetRange(viz::SurfaceRange(std::nullopt, surface_id), std::nullopt);
...
TEST(SurfaceLayerImplTest, SurfaceLayerImplWithTwoDifferentSurfaces)
{
    ...
    const viz::LocalSurfaceId kArbitraryLocalSurfaceId1(9, base::UnguessableToken::Create());
    viz::SurfaceId surface_id1(kArbitraryFrameSinkId, kArbitraryLocalSurfaceId1);

    // Populate the fallback viz::SurfaceId.
    const viz::LocalSurfaceId kArbitraryLocalSurfaceId2(7, kArbitraryLocalSurfaceId1.embed_token());
    ...
    {
        AppendQuadsData data;
        surface_layer_impl->SetRange(viz::SurfaceRange(std::nullopt, surface_id1), 0u);
        surface_layer_impl->AppendQuads(render_pass.get(), &data);
    ...
    {
        AppendQuadsData data;
        surface_layer_impl->SetRange(viz::SurfaceRange(surface_id2, surface_id1), 4u);
        surface_layer_impl->AppendQuads(render_pass.get(), &data);
        // The the primary viz::SurfaceInfo will be added to
        // activation_dependencies.
        EXPECT_THAT(data.activation_dependencies, UnorderedElementsAre(surface_id1));
    ...
    const viz::SurfaceDrawQuad* surface_draw_quad1 = viz::SurfaceDrawQuad::MaterialCast(render_pass->quad_list.ElementAt(0));
    ASSERT_TRUE(surface_draw_quad1);
    const viz::SurfaceDrawQuad* surface_draw_quad2 = viz::SurfaceDrawQuad::MaterialCast(render_pass->quad_list.ElementAt(1));
    ASSERT_TRUE(surface_draw_quad2);
    const viz::SurfaceDrawQuad* surface_draw_quad3 = viz::SurfaceDrawQuad::MaterialCast(render_pass->quad_list.ElementAt(2));
...
TEST(SurfaceLayerImplTest, SurfaceLayerImplsWithDeadlines)
{
    ...
    SurfaceLayerImpl* surface_layer_impl2 = impl.AddLayerInActiveTree<SurfaceLayerImpl>();
    CopyProperties(impl.root_layer(), surface_layer_impl2);

    const viz::LocalSurfaceId kArbitraryLocalSurfaceId1(1, base::UnguessableToken::Create());
    viz::SurfaceId surface_id1(kArbitraryFrameSinkId, kArbitraryLocalSurfaceId1);

    const viz::LocalSurfaceId kArbitraryLocalSurfaceId2(2, kArbitraryLocalSurfaceId1.embed_token());
    viz::SurfaceId surface_id2(kArbitraryFrameSinkId, kArbitraryLocalSurfaceId2);
    ...
    surface_layer_impl2->SetRange(viz::SurfaceRange(surface_id1, surface_id2), std::nullopt);
...
TEST(SurfaceLayerImplTest, GetEnclosingRectInTargetSpace)
{
    ...
    // pixels.
    EXPECT_EQ(surface_layer_impl->GetScaledEnclosingVisibleRectInTargetSpace(1.33), surface_layer_impl->GetEnclosingVisibleRectInTargetSpace());
}
...
TEST(SurfaceLayerImplTest, WillDrawNotifiesSynchronouslyInCompositeImmediate)
{
    ...
    bool updated = false;
    UpdateSubmissionStateCB callback = base::BindLambdaForTesting([task_runner, &updated](bool draw, base::WaitableEvent* done) {
    ...
    SurfaceLayerImpl* surface_layer_impl = impl.AddLayerInActiveTree<SurfaceLayerImpl>(std::move(callback));
...
{
    bool updated = false;
    UpdateSubmissionStateCB callback = base::BindLambdaForTesting([&updated](bool draw, base::WaitableEvent* done) {
        // SurfaceLayerImpl also notifies the callback on destruction with draw
        // = false. Ignore that call, since it's not really a part of the test.
        if (!draw)
            return;
...
public:
    ...
    {
        scoped_feature_list_.InitWithFeatures({ features::kAlignSurfaceLayerImplToPixelGrid }, {});
    }
...
TEST_F(SurfaceLayerImplAlignToPixelGridTest, FractionalOffsetSnapsToPixelGrid)
{
    ...
    viz::SharedQuadState* state = render_pass->shared_quad_state_list.ElementAt(0);
