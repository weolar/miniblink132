...
std::unique_ptr<LayerImpl> UIResourceLayer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
void UIResourceLayer::SetBitmap(const SkBitmap& bitmap)
{
    ...
    if (!layer_tree_host())
        return;
    SetUIResourceIdInternal(layer_tree_host()->GetUIResourceManager()->GetOrCreateUIResource(bitmap));
}
...
void UIResourceLayer::PushPropertiesTo(LayerImpl* layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
