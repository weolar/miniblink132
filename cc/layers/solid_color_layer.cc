...
std::unique_ptr<LayerImpl> SolidColorLayer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
sk_sp<const SkPicture> SolidColorLayer::GetPicture() const
{
    ...
    recorder.beginRecording(bounds().width(), bounds().height())->drawColor(background_color());
