...
{
    gfx::Size content_size_in_dips = gfx::ScaleToCeiledSize(device_pixels_size, 1.0f / dsf);
    gfx::Size content_size_in_dps = gfx::ScaleToCeiledSize(content_size_in_dips, dsf);
    return content_size_in_dps;
}

gfx::Size AdjustGpuTileSize(int tile_width, int tile_height, const gfx::Size& max_tile_size, int min_height_for_gpu_raster_tile)
{
...
// viewport vertically.
gfx::Size CalculateGpuTileSize(
    const gfx::Size& base_tile_size, const gfx::Size& content_bounds, const gfx::Size& max_tile_size, int min_height_for_gpu_raster_tile)
{
...
gfx::Size CalculateGpuRawDrawTileSize(const gfx::Size& base_tile_size, const gfx::Size& content_bounds, const gfx::Size& max_tile_size,
    int min_height_for_gpu_raster_tile, double raw_draw_tile_size_factor)
{
    ...
    int tile_size = std::max({ base_tile_size.width(), base_tile_size.height(), kMinBaseTileSize });
    ...
    if (content_bounds.Area64() <= static_cast<uint64_t>(tile_size) * static_cast<uint64_t>(tile_size)) {
        return AdjustGpuTileSize(content_bounds.width(), content_bounds.height(), max_tile_size, min_height_for_gpu_raster_tile);
    }
...
// AffectingParams.
bool TileSizeCalculator::AffectingParams::operator==(const AffectingParams& other) const = default;
...
TileSizeCalculator::AffectingParams TileSizeCalculator::GetAffectingParams(gfx::Size content_bounds) const
{
    ...
    params.min_height_for_gpu_raster_tile = layer_tree_impl->settings().min_height_for_gpu_raster_tile;
    params.gpu_raster_max_texture_size = layer_impl()->gpu_raster_max_texture_size();
    params.device_scale_factor = layer_tree_impl->device_scale_factor();
    params.max_untiled_layer_size = layer_tree_impl->settings().max_untiled_layer_size;
...
gfx::Size TileSizeCalculator::CalculateTileSize(gfx::Size content_bounds)
{
    if (layer_impl()->is_backdrop_filter_mask()) {
        ...
        DCHECK_LE(content_bounds.width(), layer_impl()->layer_tree_impl()->max_texture_size());
        DCHECK_LE(content_bounds.height(), layer_impl()->layer_tree_impl()->max_texture_size());
    ...
    if (affecting_params_.use_gpu_rasterization) {
        ...
        gfx::Size base_tile_size = ApplyDsfAdjustment(affecting_params_.gpu_raster_max_texture_size, affecting_params_.device_scale_factor);
        ...
        if (is_using_raw_draw_) {
            default_tile_size = CalculateGpuRawDrawTileSize(
                base_tile_size, content_bounds, max_tile_size, affecting_params_.min_height_for_gpu_raster_tile, raw_draw_tile_size_factor_);
        } else {
            default_tile_size = CalculateGpuTileSize(base_tile_size, content_bounds, max_tile_size, affecting_params_.min_height_for_gpu_raster_tile);
        ...
        int max_untiled_content_width = affecting_params_.max_untiled_layer_size.width();
        int max_untiled_content_height = affecting_params_.max_untiled_layer_size.height();
