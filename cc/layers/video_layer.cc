...
scoped_refptr<VideoLayer> VideoLayer::Create(VideoFrameProvider* provider, media::VideoTransformation transform)
{
    return base::WrapRefCounted(new VideoLayer(provider, transform));
}

VideoLayer::VideoLayer(VideoFrameProvider* provider, media::VideoTransformation transform)
    : provider_(provider)
...
std::unique_ptr<LayerImpl> VideoLayer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
    return VideoLayerImpl::Create(tree_impl, id(), provider_.Read(*this), transform_);
}
