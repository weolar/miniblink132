...
TEST(RecordingSourceTest, DiscardableImagesWithTransform)
{
    ...
    recording_source.add_draw_image_with_transform(discardable_image[0][0], identity_transform);
    recording_source.add_draw_image_with_transform(discardable_image[1][0], translate_transform);
    recording_source.add_draw_image_with_transform(discardable_image[1][1], rotate_transform);
    recording_source.Rerecord();

    scoped_refptr<RasterSource> raster_source = recording_source.CreateRasterSource();
    scoped_refptr<DiscardableImageMap> image_map = raster_source->GetDisplayItemList()->GenerateDiscardableImageMap();

    // Tile sized iterators. These should find only one pixel ref.
    {
        std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(0, 0, 128, 128));
        EXPECT_EQ(2u, images.size());
        EXPECT_TRUE(images[0]->paint_image().IsSameForTesting(discardable_image[0][0]));
        EXPECT_TRUE(images[1]->paint_image().IsSameForTesting(discardable_image[1][1]));
    }

    // Shifted tile sized iterators. These should find only one pixel ref.
    {
        std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(130, 140, 128, 128));
        EXPECT_EQ(1u, images.size());
        EXPECT_TRUE(images[0]->paint_image().IsSameForTesting(discardable_image[1][1]));
    }

    // The rotated bitmap would still be in the top right tile.
    {
        std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(130, 0, 128, 128));
        EXPECT_EQ(1u, images.size());
        EXPECT_TRUE(images[0]->paint_image().IsSameForTesting(discardable_image[1][1]));
    }
    ...
    // Layer sized iterators. These should find all pixel refs.
    {
        ...
        EXPECT_TRUE(images[0]->paint_image().IsSameForTesting(discardable_image[0][0]));
        EXPECT_TRUE(images[1]->paint_image().IsSameForTesting(discardable_image[1][0]));
        EXPECT_TRUE(images[2]->paint_image().IsSameForTesting(discardable_image[1][1]));
    }

    // Verify different raster scales
    for (float scale = 1.f; scale <= 5.f; scale += 0.5f) {
        std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(130, 0, 128, 128));
        DrawImage image(*images[0], scale, PaintImage::kDefaultFrameIndex, TargetColorParams());
        EXPECT_EQ(1u, images.size());
...
TEST(RecordingSourceTest, EmptyImages)
{
    ...
    {
        std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(140, 140, 128, 128));
        EXPECT_TRUE(images.empty());
    }
...
TEST(RecordingSourceTest, NoDiscardableImages)
{
    ...
    auto non_discardable_image = CreateNonDiscardablePaintImage(gfx::Size(128, 128));
    recording_source.add_draw_rect_with_flags(gfx::Rect(0, 0, 256, 256), simple_flags);
    recording_source.add_draw_rect_with_flags(gfx::Rect(128, 128, 512, 512), simple_flags);
    recording_source.add_draw_rect_with_flags(gfx::Rect(512, 0, 256, 256), simple_flags);
...
TEST(RecordingSourceTest, DiscardableImages)
{
    ...
    recording_source.add_draw_image(discardable_image[1][1], gfx::Point(140, 140));
    recording_source.Rerecord();
    ...
    {
        std::vector<const DrawImage*> images = image_map->GetDiscardableImagesInRect(gfx::Rect(140, 0, 128, 128));
        EXPECT_TRUE(images.empty());
    }
...
TEST(RecordingSourceTest, DiscardableImagesBaseNonDiscardable)
{
    ...
    PaintImage non_discardable_image = CreateNonDiscardablePaintImage(gfx::Size(512, 512));
    ...
    // Layer sized iterators. These should find three images.
    {
        ...
        EXPECT_EQ(3u, images.size());
        EXPECT_TRUE(images[0]->paint_image().IsSameForTesting(discardable_image[0][0]));
        EXPECT_TRUE(images[1]->paint_image().IsSameForTesting(discardable_image[0][1]));
        EXPECT_TRUE(images[2]->paint_image().IsSameForTesting(discardable_image[1][1]));
    }
}
...
TEST(RecordingSourceTest, AnalyzeIsSolid)
{
    ...
    const std::vector<float> recording_scales = { 1.f, 1.25f, 1.33f, 1.5f, 1.6f, 1.66f, 2.f, 2.25f, 2.5f };
    for (float recording_scale : recording_scales) {
        ...
        EXPECT_TRUE(raster->IsSolidColor()) << " recording scale: " << recording_scale;
        ...
        for (int y = 0; y < layer_bounds.height(); y += 50) {
            for (int x = 0; x < layer_bounds.width(); x += 50) {
                ...
                recording_source.add_draw_rect_with_flags(gfx::ScaleToEnclosingRect(gfx::Rect(layer_bounds), recording_scale), solid_flags);
                recording_source.add_draw_rect_with_flags(gfx::Rect(std::round(x * recording_scale), std::round(y * recording_scale), 1, 1), non_solid_flags);
                recording_source.Rerecord();
                raster = recording_source.CreateRasterSource();
                EXPECT_FALSE(raster->IsSolidColor()) << " recording scale: " << recording_scale << " pixel at: (" << x << ", " << y << ") was not solid.";
            }
        }
    }
}
