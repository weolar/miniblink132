...
std::unique_ptr<LayerImpl> NinePatchLayer::CreateLayerImpl(LayerTreeImpl* tree_impl) const
{
...
void NinePatchLayer::PushPropertiesTo(LayerImpl* layer, const CommitState& commit_state, const ThreadUnsafeCommitState& unsafe_state)
{
    ...
    if (resource_id()) {
        ...
        layer_impl->SetLayout(
            image_aperture_.Read(*this), border_.Read(*this), layer_occlusion_.Read(*this), fill_center_.Read(*this), nearest_neighbor_.Read(*this));
    }
}
