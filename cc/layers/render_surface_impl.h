...
public:
    ...
    RenderSurfacePropertyChangedFlags(bool self_changed, bool ancestor_changed)
        : self_changed_(self_changed)
        , ancestor_changed_(ancestor_changed)
    {
    }
    bool self_changed() const
    {
        return self_changed_;
    }
    bool ancestor_changed() const
    {
        return ancestor_changed_;
    }
...
public:
    ...
    {
        return draw_properties_.draw_opacity;
    }

    void SetMaskFilterInfo(const gfx::MaskFilterInfo& mask_filter_info, bool is_fast_rounded_corner)
    {
    ...
    {
        return common_ancestor_clip_id_ == kInvalidPropertyNodeId ? ClipTreeIndex() : common_ancestor_clip_id_;
    }

    // TODO(wangxianzhu): Remove this when removing the
    // RenderSurfaceCommonAncestorClip feature.
    void set_has_contributing_layer_that_escapes_clip(bool contributing_layer_escapes_clip)
    {
    ...
    {
        return intersects_damage_under_;
    }
    ...
    {
        return draw_properties_.content_rect;
    }

    void ClearAccumulatedContentRect();
    void AccumulateContentRectFromContributingLayer(LayerImpl* contributing_layer);
    void AccumulateContentRectFromContributingRenderSurface(RenderSurfaceImpl* contributing_surface);
    ...
    void increment_num_contributors()
    {
        num_contributors_++;
    }
    ...
    void reset_num_contributors()
    {
        num_contributors_ = 0;
    }
    int num_contributors() const
    {
        return num_contributors_;
    }
    ...
    DamageTracker* damage_tracker() const
    {
    ...
    viz::ResourceId GetMaskResourceFromLayer(PictureLayerImpl* mask_layer, gfx::Size* mask_texture_size, gfx::RectF* mask_uv_rect) const;
    void AppendQuads(DrawMode draw_mode, viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data);

    int TransformTreeIndex() const;
    int ClipTreeIndex() const;

    void set_effect_tree_index(int index)
    {
...
private:
    ...
    gfx::Rect CalculateExpandedClipForFilters(const gfx::Transform& target_to_surface);
    void TileMaskLayer(viz::CompositorRenderPass* render_pass, viz::SharedQuadState* shared_quad_state, const gfx::Rect& unoccluded_content_rect);
    ...
    raw_ptr<const RenderSurfaceImpl, AcrossTasksDanglingUntriaged> nearest_occlusion_immune_ancestor_ = nullptr;
