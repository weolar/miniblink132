...
// static
scoped_refptr<VideoFrameProviderClientImpl> VideoFrameProviderClientImpl::Create(VideoFrameProvider* provider, VideoFrameControllerClient* client)
{
    return base::WrapRefCounted(new VideoFrameProviderClientImpl(provider, client));
}

VideoFrameProviderClientImpl::VideoFrameProviderClientImpl(VideoFrameProvider* provider, VideoFrameControllerClient* client)
    : provider_(provider)
    , client_(client)
...
void VideoFrameProviderClientImpl::SetActiveVideoLayer(VideoLayerImpl* video_layer)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    DCHECK(video_layer);
...
scoped_refptr<media::VideoFrame> VideoFrameProviderClientImpl::AcquireLockAndCurrentFrame()
{
    DCHECK(thread_checker_.CalledOnValidThread());
...
std::optional<base::TimeDelta> VideoFrameProviderClientImpl::GetPreferredRenderInterval()
{
...
{
    TRACE_EVENT1("cc", "VideoFrameProviderClientImpl::DidReceiveFrame", "active_video_layer", !!active_video_layer_);
    DCHECK(thread_checker_.CalledOnValidThread());
...
void VideoFrameProviderClientImpl::OnBeginFrame(const viz::BeginFrameArgs& args)
{
    DCHECK(thread_checker_.CalledOnValidThread());
    DCHECK(rendering_);
    DCHECK(!stopped_);
