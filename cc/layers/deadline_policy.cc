...
// static
DeadlinePolicy DeadlinePolicy::UseSpecifiedDeadline(uint32_t deadline_in_frames)
{
...
{
    return DeadlinePolicy(Type::kUseInfiniteDeadline, std::numeric_limits<uint32_t>::max());
}

DeadlinePolicy::DeadlinePolicy(Type policy_type, std::optional<uint32_t> deadline_in_frames)
...
{
    return base::StringPrintf("DeadlinePolicy(%s, %d)", PolicyToString(policy_type_), deadline_in_frames_.value_or(-1));
}
