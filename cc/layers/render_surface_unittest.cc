...
public:
    static std::unique_ptr<FakePictureLayerImplForRenderSurfaceTest> Create(LayerTreeImpl* tree_impl, int id)
    {
        return base::WrapUnique(new FakePictureLayerImplForRenderSurfaceTest(tree_impl, id));
    }
    ...
    void AppendQuads(viz::CompositorRenderPass* render_pass, AppendQuadsData* append_quads_data) override
    {
        ...
        for (const auto& rect : quad_rects_) {
            ...
            quad->SetNew(shared_quad_state, rect, rect, needs_blending, viz::kInvalidResourceId, gfx::RectF(rect), bounds(), false, false, false);
        }
    }
...
TEST(RenderSurfaceTest, VerifySurfaceChangesAreTrackedProperly)
{
    ...
    EXECUTE_AND_VERIFY_SURFACE_CHANGED(render_surface->SetContentRectForTesting(test_rect));
    ...
    // considered "change".
    EXECUTE_AND_VERIFY_SURFACE_DID_NOT_CHANGE(render_surface->SetClipRect(test_rect));
    EXECUTE_AND_VERIFY_SURFACE_DID_NOT_CHANGE(render_surface->SetContentRectForTesting(test_rect));

    constexpr int kArbitraryLayerId = 200;
    std::unique_ptr<LayerImpl> dummy_mask = LayerImpl::Create(active_tree, kArbitraryLayerId);
    ...
    EXECUTE_AND_VERIFY_SURFACE_DID_NOT_CHANGE(render_surface->SetDrawOpacity(0.5f));
    EXECUTE_AND_VERIFY_SURFACE_DID_NOT_CHANGE(render_surface->SetDrawTransform(dummy_matrix));
}
...
TEST(RenderSurfaceTest, SanityCheckSurfaceCreatesCorrectSharedQuadState)
{
    ...
    EXPECT_EQ(content_rect, gfx::Rect(shared_quad_state->visible_quad_layer_rect));
...
TEST(RenderSurfaceTest, SanityCheckSurfaceCreatesCorrectRenderPass)
{
    ...
    EXPECT_EQ(viz::CompositorRenderPassId(layer->element_id().GetInternalValue()), pass->id);
...
TEST(RenderSurfaceTest, SanityCheckSurfaceIgnoreMaskLayerOcclusion)
{
    ...
    auto* mask_layer = impl.AddLayerInActiveTree<FakePictureLayerImplForRenderSurfaceTest>();
    scoped_refptr<FakeRasterSource> raster_source(FakeRasterSource::CreateFilled(mask_layer->bounds()));
    ...
    render_surface->SetContentRectForTesting(content_rect);
    mask_layer->draw_properties().occlusion_in_content_space = Occlusion(gfx::Transform(), SimpleEnclosedRegion(occluded), SimpleEnclosedRegion(occluded));
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 200, 100).ToString(), render_pass->quad_list.front()->rect.ToString());
}
