...
{
    bool called = false;
    auto callback = base::BindLambdaForTesting([&called](const viz::ViewTransitionElementResourceRects&) { called = true; });

    auto request = ViewTransitionRequest::CreateCapture(blink::ViewTransitionToken(),
    ...
    EXPECT_EQ(viz::CompositorFrameTransitionDirective::Type::kSave, directive.type());
...
{
    auto request = ViewTransitionRequest::CreateAnimateRenderer(blink::ViewTransitionToken(),
    ...
    auto directive = request->ConstructDirective({}, {});
    EXPECT_GT(directive.sequence_id(), 0u);
    EXPECT_EQ(viz::CompositorFrameTransitionDirective::Type::kAnimateRenderer, directive.type());
}
