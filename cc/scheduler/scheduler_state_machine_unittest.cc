...
#define EXPECT_ENUM_EQ(enum_tostring, expected, actual) EXPECT_THAT(enum_tostring(actual), testing::Eq(enum_tostring(expected)))

#define EXPECT_IMPL_FRAME_STATE(expected) EXPECT_ENUM_EQ(BeginImplFrameStateToString, expected, state.begin_impl_frame_state())

#define EXPECT_MAIN_FRAME_STATE(expected) EXPECT_ENUM_EQ(BeginMainFrameStateToString, expected, state.GetBeginMainFrameState())

#define EXPECT_NEXT_MAIN_FRAME_STATE(expected) EXPECT_ENUM_EQ(BeginMainFrameStateToString, expected, state.GetNextBeginMainFrameState())

#define EXPECT_ACTION(expected) EXPECT_ENUM_EQ(ActionToString, expected, state.NextAction())

#define EXPECT_ACTION_UPDATE_STATE(action)                                                                                                                     \
    EXPECT_ACTION(action);                                                                                                                                     \
    if (action == SchedulerStateMachine::Action::DRAW_IF_POSSIBLE || action == SchedulerStateMachine::Action::DRAW_FORCED) {                                   \
        EXPECT_IMPL_FRAME_STATE(SchedulerStateMachine::BeginImplFrameState::INSIDE_DEADLINE);                                                                  \
    }                                                                                                                                                          \
    PerformAction(&state, action);                                                                                                                             \
    if (action == SchedulerStateMachine::Action::NONE) {                                                                                                       \
        if (state.begin_impl_frame_state() == SchedulerStateMachine::BeginImplFrameState::INSIDE_DEADLINE)                                                     \
            state.OnBeginImplFrameIdle();                                                                                                                      \
    }

#define SET_UP_STATE(state)                                                                                                                                    \
    state.SetVisible(true);                                                                                                                                    \
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::BEGIN_LAYER_TREE_FRAME_SINK_CREATION);                                                           \
...
const char* BeginImplFrameStateToString(SchedulerStateMachine::BeginImplFrameState state)
{
...
const char* BeginMainFrameStateToString(SchedulerStateMachine::BeginMainFrameState state)
{
...
const SchedulerStateMachine::BeginMainFrameState begin_main_frame_states[] = { SchedulerStateMachine::BeginMainFrameState::IDLE,
    SchedulerStateMachine::BeginMainFrameState::SENT, SchedulerStateMachine::BeginMainFrameState::READY_TO_COMMIT };
...
public:
    ...
    {
        OnBeginImplFrame(viz::BeginFrameId(0, next_begin_frame_number_++), kAnimateOnly);
    }
    ...
    DrawResult draw_result_for_test()
    {
        return draw_result_for_test_;
    }
...
TEST(SchedulerStateMachineTest, BeginFrameNeeded)
{
    ...
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::NONE);
    state.CreateAndInitializeLayerTreeFrameSinkWithActivatedCommit();
    state.SetBeginMainFrameState(SchedulerStateMachine::BeginMainFrameState::IDLE);
...
TEST(SchedulerStateMachineTest, BeginMainFrameIsHighestPriorityAction)
{
    ...
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::SEND_BEGIN_MAIN_FRAME);
...
TEST(SchedulerStateMachineTest, TestNextActionNotifyBeginMainFrameNotExpectedUntil)
{
    SchedulerSettings default_scheduler_settings;
    StateMachine state(default_scheduler_settings);
    ...
    state.SetNeedsOneBeginImplFrame(true);
    EXPECT_TRUE(state.BeginFrameNeeded());
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::NOTIFY_BEGIN_MAIN_FRAME_NOT_EXPECTED_UNTIL);
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::NONE);
...
TEST(SchedulerStateMachineTest, TestNextActionNotifyBeginMainFrameNotExpectedSoon)
{
    SchedulerSettings default_scheduler_settings;
    StateMachine state(default_scheduler_settings);
    state.SetMainThreadWantsBeginMainFrameNotExpectedMessages(true);
    ...
    EXPECT_FALSE(state.BeginFrameNeeded());
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::NONE);
    state.SetBeginImplFrameState(SchedulerStateMachine::BeginImplFrameState::IDLE);
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::NOTIFY_BEGIN_MAIN_FRAME_NOT_EXPECTED_SOON);
...
TEST(SchedulerStateMachineTest, FailedDrawForAnimationCheckerboardSetsNeedsCommitAndRetriesDraw)
{
    SchedulerSettings default_scheduler_settings;
    StateMachine state(default_scheduler_settings);
    SET_UP_STATE(state);
...
TEST(SchedulerStateMachineTest, TestFailedDrawsEventuallyForceDrawAfterNextCommit)
{
    SchedulerSettings scheduler_settings;
...
TEST(SchedulerStateMachineTest, TestFailedDrawsDoNotRestartForcedDraw)
{
    ...
    EXPECT_TRUE(state.CommitPending());
    EXPECT_TRUE(state.ForcedRedrawState() == SchedulerStateMachine::ForcedRedrawOnTimeoutState::WAITING_FOR_COMMIT);
...
TEST(SchedulerStateMachineTest, TestNextActionDrawsOnBeginImplFrame)
{
    ...
    for (size_t i = 0; i < num_begin_main_frame_states; ++i) {
        for (size_t j = 0; j < num_begin_impl_frame_states; ++j) {
            ...
            bool visible = (all_begin_impl_frame_states[j] != SchedulerStateMachine::BeginImplFrameState::INSIDE_DEADLINE);
            state.SetVisible(visible);

            // Case 1: needs_begin_main_frame=false
            EXPECT_NE(SchedulerStateMachine::Action::DRAW_IF_POSSIBLE, state.NextAction());
    ...
    for (size_t i = 0; i < num_begin_main_frame_states; ++i) {
        ...
        state.SetCanDraw(true);
        state.SetBeginMainFrameState(begin_main_frame_states[i]);
        state.SetBeginImplFrameState(SchedulerStateMachine::BeginImplFrameState::INSIDE_DEADLINE);

        state.SetNeedsRedraw(true);

        SchedulerStateMachine::Action expected_action;
        if (begin_main_frame_states[i] == SchedulerStateMachine::BeginMainFrameState::READY_TO_COMMIT) {
...
TEST(SchedulerStateMachineTest, TestSetNeedsBeginMainFrameIsNotLost)
{
    ...
    state.NotifyReadyToCommit();
    EXPECT_MAIN_FRAME_STATE(SchedulerStateMachine::BeginMainFrameState::READY_TO_COMMIT);

    // Expect to commit regardless of BeginImplFrame state.
    EXPECT_IMPL_FRAME_STATE(SchedulerStateMachine::BeginImplFrameState::INSIDE_BEGIN_FRAME);
...
TEST(SchedulerStateMachineTest, TestFullCycleWithCommitToActive)
{
    ...
    frame_id.sequence_number++;
    state.OnBeginImplFrame(frame_id, kAnimateOnly);
    EXPECT_NE(SchedulerStateMachine::BeginImplFrameDeadlineMode::BLOCKED, state.CurrentBeginImplFrameDeadlineMode());
}
...
TEST(SchedulerStateMachineTest, TestMainFrameBeforeCommit)
{
    ...
    state.IssueNextBeginImplFrame();
    EXPECT_MAIN_FRAME_STATE(SchedulerStateMachine::BeginMainFrameState::IDLE);
    EXPECT_NEXT_MAIN_FRAME_STATE(SchedulerStateMachine::BeginMainFrameState::IDLE);
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::SEND_BEGIN_MAIN_FRAME);
    EXPECT_MAIN_FRAME_STATE(SchedulerStateMachine::BeginMainFrameState::SENT);
    EXPECT_NEXT_MAIN_FRAME_STATE(SchedulerStateMachine::BeginMainFrameState::IDLE);
    EXPECT_ACTION(SchedulerStateMachine::Action::NONE);
    ...
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::SEND_BEGIN_MAIN_FRAME);
    EXPECT_MAIN_FRAME_STATE(SchedulerStateMachine::BeginMainFrameState::READY_TO_COMMIT);
    EXPECT_NEXT_MAIN_FRAME_STATE(SchedulerStateMachine::BeginMainFrameState::SENT);
...
TEST(SchedulerStateMachineTest, TestContextLostWhenCompletelyIdle)
{
    ...
    EXPECT_NE(SchedulerStateMachine::Action::BEGIN_LAYER_TREE_FRAME_SINK_CREATION, state.NextAction());
    state.DidLoseLayerTreeFrameSink();
...
TEST(SchedulerStateMachineTest, TestContextLostWhenIdleAndCommitRequestedWhileRecreating)
{
    ...
    EXPECT_NE(SchedulerStateMachine::Action::BEGIN_LAYER_TREE_FRAME_SINK_CREATION, state.NextAction());
    state.DidLoseLayerTreeFrameSink();
    EXPECT_EQ(state.layer_tree_frame_sink_state(), SchedulerStateMachine::LayerTreeFrameSinkState::NONE);
    ...
    // Recreate the context
    state.DidCreateAndInitializeLayerTreeFrameSink();
    EXPECT_EQ(state.layer_tree_frame_sink_state(), SchedulerStateMachine::LayerTreeFrameSinkState::WAITING_FOR_FIRST_COMMIT);
    EXPECT_FALSE(state.RedrawPending());
    ...
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::COMMIT);
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::POST_COMMIT);
    EXPECT_EQ(state.layer_tree_frame_sink_state(), SchedulerStateMachine::LayerTreeFrameSinkState::WAITING_FOR_FIRST_ACTIVATION);
    state.NotifyReadyToActivate();
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::ACTIVATE_SYNC_TREE);
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::NONE);
    EXPECT_EQ(state.layer_tree_frame_sink_state(), SchedulerStateMachine::LayerTreeFrameSinkState::ACTIVE);
...
TEST(SchedulerStateMachineTest, TestContextLostWhileCommitInProgress)
{
    ...
    EXPECT_IMPL_FRAME_STATE(SchedulerStateMachine::BeginImplFrameState::IDLE);
    EXPECT_ACTION(SchedulerStateMachine::Action::BEGIN_LAYER_TREE_FRAME_SINK_CREATION);
...
TEST(SchedulerStateMachineTest, DontDrawBeforeCommitAfterLostLayerTreeFrameSink)
{
    SchedulerSettings default_scheduler_settings;
    StateMachine state(default_scheduler_settings);
    SET_UP_STATE(state);
...
TEST(SchedulerStateMachineTest, TestShouldAbortCurrentFrameAfterLostLayerTreeFrameSink)
{
    SchedulerSettings default_scheduler_settings;
    StateMachine state(default_scheduler_settings);
    SET_UP_STATE(state);

    state.SetBeginMainFrameState(SchedulerStateMachine::BeginMainFrameState::SENT);
...
TEST(SchedulerStateMachineTest, TestFinishCommitWhenCommitInProgressAndBeginFrameSourcePaused)
{
    SchedulerSettings default_scheduler_settings;
    StateMachine state(default_scheduler_settings);
    state.SetVisible(true);
...
TEST(SchedulerStateMachineTest, TestTriggerDeadlineImmediatelyAfterAbortedCommit)
{
    SchedulerSettings default_scheduler_settings;
    StateMachine state(default_scheduler_settings);
    SET_UP_STATE(state);
...
void FinishPreviousCommitAndDrawWithoutExitingDeadline(StateMachine* state_ptr)
{
...
TEST(SchedulerStateMachineTest, TestImplLatencyTakesPriorityImplInvalidations)
{
    ...
    state.SetTreePrioritiesAndScrollState(SMOOTHNESS_TAKES_PRIORITY, ScrollHandlerState::SCROLL_DOES_NOT_AFFECT_SCROLL_HANDLER);
    ...
    state.OnBeginImplFrame(frame_id, kAnimateOnly);
    state.OnBeginImplFrameDeadline();
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::PERFORM_IMPL_SIDE_INVALIDATION);
    state.NotifyReadyToActivate();
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::ACTIVATE_SYNC_TREE);
    state.OnBeginImplFrameIdle();
...
TEST(SchedulerStateMachineTest, TestTriggerDeadlineImmediatelyOnLostLayerTreeFrameSink)
{
    SchedulerSettings default_scheduler_settings;
    StateMachine state(default_scheduler_settings);
    SET_UP_STATE(state);
...
TEST(SchedulerStateMachineTest, TestTriggerDeadlineImmediatelyWhenBeginFrameSourcePaused)
{
    SchedulerSettings default_scheduler_settings;
    StateMachine state(default_scheduler_settings);
    SET_UP_STATE(state);
...
TEST(SchedulerStateMachineTest, NoLayerTreeFrameSinkCreationWhileCommitPending)
{
    SchedulerSettings settings;
    StateMachine state(settings);
    SET_UP_STATE(state);
...
TEST(SchedulerStateMachineTest, NoImplSideInvalidationsWhenBeginFrameSourcePaused)
{
    SchedulerSettings settings;
    StateMachine state(settings);
    SET_UP_STATE(state);
...
TEST(SchedulerStateMachineTest, ImplSideInvalidationAndMainFrame_NoMainFrameRequest)
{
...
TEST(SchedulerStateMachineTest, ImplSideInvalidationAndMainFrame_MainFrameRequest_FastMainThread)
{
...
TEST(SchedulerStateMachineTest, ImplSideInvalidationAndMainFrame_LastFrameCommit)
{
...
TEST(SchedulerStateMachineTest, ImplSideInvalidationAndMainFrame_LastFrameAborted)
{
...
TEST(SchedulerStateMachineTest, ImplSideInvalidationAndMainFrame_MainFrameRequest_SlowMainThread)
{
...
TEST(SchedulerStateMachineTest, ConsecutiveImplSideInvalidationsWaitForBeginFrame)
{
    SchedulerSettings settings;
    StateMachine state(settings);
    SET_UP_STATE(state);
...
TEST(SchedulerStateMachineTest, TestFullPipelineMode)
{
    ...
    viz::BeginFrameId frame_id = viz::BeginFrameId(0, 10);
    state.OnBeginImplFrame(frame_id, kAnimateOnly);
    EXPECT_EQ(SchedulerStateMachine::BeginImplFrameDeadlineMode::BLOCKED, state.CurrentBeginImplFrameDeadlineMode());

    // If main thread defers commits, don't wait for it.
    state.SetDeferBeginMainFrame(true);
    EXPECT_EQ(SchedulerStateMachine::BeginImplFrameDeadlineMode::IMMEDIATE, state.CurrentBeginImplFrameDeadlineMode());
    state.SetDeferBeginMainFrame(false);
...
TEST(SchedulerStateMachineTest, DelayDrawIfAnimationWorkletsPending)
{
    ...
    state.NotifyAnimationWorkletStateChange(SchedulerStateMachine::AnimationWorkletState::PROCESSING, SchedulerStateMachine::TreeType::ACTIVE);
    EXPECT_FALSE(state.ShouldTriggerBeginImplFrameDeadlineImmediately());
    EXPECT_EQ(SchedulerStateMachine::BeginImplFrameDeadlineMode::REGULAR, state.CurrentBeginImplFrameDeadlineMode());
    ...
    state.NotifyAnimationWorkletStateChange(SchedulerStateMachine::AnimationWorkletState::IDLE, SchedulerStateMachine::TreeType::ACTIVE);
    EXPECT_FALSE(state.ShouldTriggerBeginImplFrameDeadlineImmediately());
    EXPECT_EQ(SchedulerStateMachine::BeginImplFrameDeadlineMode::REGULAR, state.CurrentBeginImplFrameDeadlineMode());
...
TEST(SchedulerStateMachineTest, BlockActivationIfAnimationWorkletsPending)
{
    ...
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::COMMIT);
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::POST_COMMIT);
    state.NotifyAnimationWorkletStateChange(SchedulerStateMachine::AnimationWorkletState::PROCESSING, SchedulerStateMachine::TreeType::PENDING);
    state.NotifyReadyToActivate();
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::NONE);
    state.NotifyAnimationWorkletStateChange(SchedulerStateMachine::AnimationWorkletState::IDLE, SchedulerStateMachine::TreeType::PENDING);
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::ACTIVATE_SYNC_TREE);
}
...
TEST(SchedulerStateMachineTest, BlockActivationIfPaintWorkletsPending)
{
    ...
    // until they are done.
    state.NotifyPaintWorkletStateChange(SchedulerStateMachine::PaintWorkletState::PROCESSING);
    // We (correctly) cannot call state.NotifyReadyToActivate() here as it hits a
    // DCHECK because PaintWorklets are ongoing.
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::NONE);
    state.NotifyPaintWorkletStateChange(SchedulerStateMachine::PaintWorkletState::IDLE);
    state.NotifyReadyToActivate();
    EXPECT_ACTION_UPDATE_STATE(SchedulerStateMachine::Action::ACTIVATE_SYNC_TREE);
}

TEST(SchedulerStateMachineTest, BlockActivationIfPaintWorkletsPendingEvenWhenAbortingFrame)
{
    SchedulerSettings settings;
    StateMachine state(settings);
    SET_UP_STATE(state);
...
TEST(SchedulerStateMachineTest, PauseRenderingSuppressesCommitsAndInvalidations)
{
    SchedulerSettings settings;
    StateMachine state(settings);
    SET_UP_STATE(state);
...
class DisableFrameRateLimitSchedulerStateMachineTests : public testing::Test, public testing::WithParamInterface<bool> {
public:
...
TEST_P(DisableFrameRateLimitSchedulerStateMachineTests, TestImplLatencyTakesPriority)
{
...
INSTANTIATE_TEST_SUITE_P(DisableFrameRateLimitSchedulerStateMachineTests, DisableFrameRateLimitSchedulerStateMachineTests, testing::Bool());
...
class ScrollingSchedulerStateMachineTest : public DisableFrameRateLimitSchedulerStateMachineTests {
public:
...
{
    scheduler_settings_ = DisableFrameRateLimitSchedulerStateMachineTests::GetSchedulerSettings();
...
TEST_P(ScrollingSchedulerStateMachineTest, ScrollModeBlockedByNoImmediateMode)
{
    ...
    if (scheduler_settings_.disable_frame_rate_limit) {
        EXPECT_EQ(SchedulerStateMachine::BeginImplFrameDeadlineMode::WAIT_FOR_SCROLL, state.CurrentBeginImplFrameDeadlineMode());
    } else {
        EXPECT_NE(SchedulerStateMachine::BeginImplFrameDeadlineMode::WAIT_FOR_SCROLL, state.CurrentBeginImplFrameDeadlineMode());
    }
...
INSTANTIATE_TEST_SUITE_P(ScrollingSchedulerStateMachineTest, ScrollingSchedulerStateMachineTest, testing::Bool());
...
TEST_F(WarmUpCompositorSchedulerStateMachineTest, SetShouldWarmUpWillStartLayerTreeFrameSinkCreation)
{
    SchedulerSettings default_scheduler_settings;
    StateMachine state(default_scheduler_settings);
    state.SetVisible(false);
