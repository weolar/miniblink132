...
#define EXPECT_ACTIONS(...) EXPECT_THAT(client_->Actions(), ::testing::ElementsAre(__VA_ARGS__))
...
class FakeSchedulerClient : public SchedulerClient, public viz::FakeExternalBeginFrameSource::Client {
public:
    FakeSchedulerClient()
    {
        Reset();
    ...
    void set_scheduler(TestScheduler* scheduler)
    {
    ...
    {
        return scheduler_->begin_frames_expected();
    }
    int num_draws() const
    {
        return num_draws_;
    }
    bool invalidate_needs_redraw() const
    {
        return invalidate_needs_redraw_;
    }
    ...
    void DidFinishImplFrame(const viz::BeginFrameArgs& last_activated_args) override
    {
    ...
    void DidNotProduceFrame(const viz::BeginFrameAck& ack, FrameSkippedReason reason) override
    {
        EXPECT_FALSE(inside_action_);
        base::AutoReset<bool> mark_inside(&inside_action_, true);
    ...
    void ScheduledActionSendBeginMainFrame(const viz::BeginFrameArgs& args) override
    {
        EXPECT_FALSE(inside_action_);
        base::AutoReset<bool> mark_inside(&inside_action_, true);
    ...
    void ScheduledActionBeginMainFrameNotExpectedUntil(base::TimeTicks time) override
    {
        EXPECT_FALSE(inside_action_);
        base::AutoReset<bool> mark_inside(&inside_action_, true);
    ...
    bool IsInsideBeginImplFrame() const
    {
        return inside_begin_impl_frame_;
    }

    base::RepeatingCallback<bool(void)> InsideBeginImplFrame(bool state)
    {
        return base::BindRepeating(&FakeSchedulerClient::InsideBeginImplFrameCallback, base::Unretained(this), state);
    }
    ...
    {
        return base::BindRepeating(&FakeSchedulerClient::IsCurrentFrame, base::Unretained(this), scheduler_->current_frame_number());
    }
...
public:
    SchedulerTestTaskRunner()
        : base::TestMockTimeTaskRunner(base::TestMockTimeTaskRunner::Type::kStandalone)
    {
    ...
    void RunTasksWhile(base::RepeatingCallback<bool()> condition)
    {
        ...
        while (!tasks_to_requeue_.empty()) {
            ...
            PostDelayedTask(task.location, std::move(task.task), task.GetTimeToRun() - NowTicks());
        }
    }
...
protected:
    TestScheduler* CreateScheduler(BeginFrameSourceType bfs_type)
    {
        ...
        unthrottled_frame_source_ = std::make_unique<viz::BackToBackBeginFrameSource>(
            std::make_unique<viz::FakeDelayBasedTimeSource>(task_runner_->GetMockTickClock(), task_runner_.get()));
        fake_external_begin_frame_source_ = std::make_unique<viz::FakeExternalBeginFrameSource>(1.0, false);
        ...
        synthetic_frame_source_ = std::make_unique<viz::DelayBasedBeginFrameSource>(
            std::make_unique<viz::FakeDelayBasedTimeSource>(task_runner_->GetMockTickClock(), task_runner_.get()), viz::BeginFrameSource::kNotRestartableId);
        switch (bfs_type) {
        case EXTERNAL_BFS:
        ...
        std::unique_ptr<FakeCompositorTimingHistory> fake_compositor_timing_history
            = FakeCompositorTimingHistory::Create(scheduler_settings_.using_synchronous_renderer_compositor);
        fake_compositor_timing_history_ = fake_compositor_timing_history.get();
        reporting_controller = std::make_unique<FakeCompositorFrameReportingController>();
        reporting_controller->SetDroppedFrameCounter(&dropped_counter);

        scheduler_ = std::make_unique<TestScheduler>(task_runner_->GetMockTickClock(), client_.get(), scheduler_settings_, 0, task_runner_.get(),
            std::move(fake_compositor_timing_history), reporting_controller.get());
    ...
    void SetUpScheduler(BeginFrameSourceType bfs_type, std::unique_ptr<FakeSchedulerClient> client)
    {
    ...
    {
        TRACE_EVENT0("cc", "SchedulerUnitTest::InitializeLayerTreeFrameSinkAndFirstCommit");
        DCHECK(scheduler_);
...

        {
            ...
            if (scheduler_settings_.using_synchronous_renderer_compositor) {
                ...
                scheduler_->OnDrawForLayerTreeFrameSink(resourceless_software_draw, skip_draw);
            } else {
        ...
        {
            SCOPED_TRACE("Run second frame so Scheduler calls SetNeedsBeginFrame(false).");
            AdvanceFrame();
        ...
        if (scheduler_->begin_frame_source() == fake_external_begin_frame_source_.get()) {
            ...
            // didn't cause damage.
            uint64_t last_begin_frame_number = fake_external_begin_frame_source_->next_begin_frame_number() - 1;
            bool has_damage = false;
            EXPECT_EQ(viz::BeginFrameAck(fake_external_begin_frame_source_->source_id(), last_begin_frame_number, has_damage), client_->last_begin_frame_ack());
        }
    ...
    {
        TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("cc.debug.scheduler.frames"), "FakeSchedulerClient::AdvanceFrame");
    ...
    {
        DCHECK_EQ(scheduler_->begin_frame_source(), fake_external_begin_frame_source_.get());
        ...
        viz::BeginFrameArgs args = fake_external_begin_frame_source_->CreateBeginFrameArgs(BEGINFRAME_FROM_HERE, task_runner_->GetMockTickClock());
    ...
    void SetShouldDeferInvalidationForMainFrame(bool defer)
    {
        ...
        fake_compositor_timing_history_->SetBeginMainFrameStartToReadyToCommitDurationEstimate(delta);
        fake_compositor_timing_history_->SetBeginMainFrameQueueDurationCriticalEstimate(delta);
        fake_compositor_timing_history_->SetBeginMainFrameQueueDurationNotCriticalEstimate(delta);
    ...
    bool BeginMainFrameOnCriticalPath(TreePriority tree_priority, ScrollHandlerState scroll_handler_state, base::TimeDelta durations);

    scoped_refptr<SchedulerTestTaskRunner> task_runner_;
    std::unique_ptr<viz::FakeExternalBeginFrameSource> fake_external_begin_frame_source_;
...
public:
    ...
    void SetRequestRedrawsInsideDraw(bool enable)
    {
        request_redraws_ = enable;
    }
...
class SchedulerClientThatSetNeedsBeginMainFrameInsideDraw : public FakeSchedulerClient {
public:
...
TEST_F(SchedulerTest, PrepareTiles)
{
    ...
    EXPECT_LT(client->ActionIndex("ScheduledActionDrawIfPossible"), client->ActionIndex("ScheduledActionPrepareTiles"));
    EXPECT_FALSE(scheduler_->RedrawPending());
...
TEST_F(SchedulerTest, PrepareTilesOncePerFrame)
{
    ...
    EXPECT_LT(client_->ActionIndex("ScheduledActionDrawIfPossible"), client_->ActionIndex("ScheduledActionPrepareTiles"));
    EXPECT_FALSE(scheduler_->RedrawPending());
    EXPECT_FALSE(scheduler_->PrepareTilesPending());
    EXPECT_FALSE(client_->IsInsideBeginImplFrame());
...
TEST_F(SchedulerTest, DidPrepareTilesPreventsPrepareTilesForOneFrame)
{
    ...
    task_runner_->RunPendingTasks(); // Run posted deadline.
    EXPECT_FALSE(client_->IsInsideBeginImplFrame());
    EXPECT_ACTIONS("ScheduledActionDrawIfPossible", "ScheduledActionPrepareTiles");
...
TEST_F(SchedulerTest, WaitForReadyToDrawCancelledWhenLostLayerTreeFrameSink)
{
    ...
    EXPECT_TRUE(client_->IsInsideBeginImplFrame());
    task_runner_->RunPendingTasks(); // Run posted deadline.
    EXPECT_ACTIONS("ScheduledActionBeginLayerTreeFrameSinkCreation", "RemoveObserver(this)");
}
...
void SchedulerTest::AdvanceAndMissOneFrame()
{
    ...
    scheduler_->NotifyReadyToCommit(nullptr);
    scheduler_->NotifyReadyToActivate();
    EXPECT_ACTIONS("AddObserver(this)", "WillBeginImplFrame", "ScheduledActionSendBeginMainFrame", "ScheduledActionCommit", "ScheduledActionPostCommit",
...
TEST_F(SchedulerTest, MainFrameNotSkippedAfterLateBeginFrame)
{
    ...
    EXPECT_ACTIONS("WillBeginImplFrame", "ScheduledActionSendBeginMainFrame", "ScheduledActionDrawIfPossible");
}
...
TEST_F(SchedulerTest, FrameIntervalUpdated)
{
    ...
    base::TimeDelta interval = base::Microseconds(base::Time::kMicrosecondsPerSecond / 120.0);

    // Send BeginFrameArgs with 120hz refresh rate and confirm client gets update.
    scheduler_->SetNeedsRedraw();
    task_runner_->AdvanceMockTickClock(interval);
    viz::BeginFrameArgs args1 = viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, kSourceId, sequence_number++, task_runner_->NowTicks(),
        task_runner_->NowTicks() + interval, interval, viz::BeginFrameArgs::NORMAL);
    ...
    viz::BeginFrameArgs args2 = viz::BeginFrameArgs::Create(
        BEGINFRAME_FROM_HERE, kSourceId, sequence_number++, args1.deadline, args1.deadline + interval, interval, viz::BeginFrameArgs::NORMAL);
    ...
    viz::BeginFrameArgs args3 = viz::BeginFrameArgs::Create(
        BEGINFRAME_FROM_HERE, kSourceId, sequence_number++, args2.deadline, args2.deadline + interval, interval, viz::BeginFrameArgs::NORMAL);
    ...
    viz::BeginFrameArgs args4 = viz::BeginFrameArgs::Create(
        BEGINFRAME_FROM_HERE, kSourceId, sequence_number++, args3.deadline, args3.deadline + interval, base::TimeDelta(), viz::BeginFrameArgs::NORMAL);
...
TEST_F(SchedulerTest, MainFrameNotSkippedAfterLateCommit_LongMainFrameQueueDurationNotCritical)
{
    SetUpScheduler(EXTERNAL_BFS);
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);
    fake_compositor_timing_history_->SetBeginMainFrameQueueDurationNotCriticalEstimate(kSlowDuration);
...
TEST_F(SchedulerTest, MainFrameNotSkippedAfterLateCommit_LongMainFrameQueueDurationCritical)
{
    SetUpScheduler(EXTERNAL_BFS);
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);
    fake_compositor_timing_history_->SetBeginMainFrameQueueDurationCriticalEstimate(kSlowDuration);
    fake_compositor_timing_history_->SetBeginMainFrameQueueDurationNotCriticalEstimate(kSlowDuration);

    EXPECT_SCOPED(CheckMainFrameNotSkippedAfterLateCommit());
}

TEST_F(SchedulerTest, MainFrameNotSkippedAfterLateCommitInPreferImplLatencyMode)
{
    SetUpScheduler(EXTERNAL_BFS);
    scheduler_->SetTreePrioritiesAndScrollState(SMOOTHNESS_TAKES_PRIORITY, ScrollHandlerState::SCROLL_DOES_NOT_AFFECT_SCROLL_HANDLER);
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);

    EXPECT_SCOPED(CheckMainFrameNotSkippedAfterLateCommit());
}

TEST_F(SchedulerTest, MainFrameNotSkippedAfterLateCommit_CommitEstimateTooLong)
{
    SetUpScheduler(EXTERNAL_BFS);
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);
    fake_compositor_timing_history_->SetBeginMainFrameStartToReadyToCommitDurationEstimate(kSlowDuration);

    EXPECT_SCOPED(CheckMainFrameNotSkippedAfterLateCommit());
}

TEST_F(SchedulerTest, MainFrameNotSkippedAfterLateCommit_ReadyToActivateEstimateTooLong)
{
    SetUpScheduler(EXTERNAL_BFS);
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);
    fake_compositor_timing_history_->SetCommitToReadyToActivateDurationEstimate(kSlowDuration);

    EXPECT_SCOPED(CheckMainFrameNotSkippedAfterLateCommit());
}

TEST_F(SchedulerTest, MainFrameNotSkippedAfterLateCommit_ActivateEstimateTooLong)
{
    SetUpScheduler(EXTERNAL_BFS);
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);
...
TEST_F(SchedulerTest, MainFrameNotSkippedAfterLateBeginMainFrameAbort)
{
    ...
    EXPECT_FALSE(scheduler_->MainThreadMissedLastDeadline());
    EXPECT_SCOPED(AdvanceFrame());
    EXPECT_ACTIONS("AddObserver(this)", "WillBeginImplFrame", "ScheduledActionSendBeginMainFrame");
    EXPECT_FALSE(scheduler_->MainThreadMissedLastDeadline());
    task_runner_->RunTasksWhile(client_->InsideBeginImplFrame(true));
    EXPECT_TRUE(scheduler_->MainThreadMissedLastDeadline());
    scheduler_->NotifyBeginMainFrameStarted(task_runner_->NowTicks());
...
TEST_F(SchedulerTest, MainFrameNotSkippedAfterCanDrawChanges)
{
    ...
    scheduler_->NotifyReadyToCommit(nullptr);
    scheduler_->NotifyReadyToActivate();
    EXPECT_ACTIONS("ScheduledActionCommit", "ScheduledActionPostCommit", "ScheduledActionActivateSyncTree");
    EXPECT_TRUE(scheduler_->MainThreadMissedLastDeadline());
...
void SchedulerTest::ImplFrameNotSkippedAfterLateAck()
{
    ...
    EXPECT_FALSE(scheduler_->MainThreadMissedLastDeadline());
    task_runner_->RunTasksWhile(client_->InsideBeginImplFrame(true));
    EXPECT_ACTIONS("ScheduledActionCommit", "ScheduledActionPostCommit", "ScheduledActionActivateSyncTree", "ScheduledActionDrawIfPossible");
    ...
    for (int i = 0; i < 10; i++) {
        ...
        EXPECT_ACTIONS("ScheduledActionSendBeginMainFrame", "ScheduledActionCommit", "ScheduledActionPostCommit", "ScheduledActionActivateSyncTree",
            "ScheduledActionDrawIfPossible");
    }
}

TEST_F(SchedulerTest, ImplFrameNotSkippedAfterLateAck_MainFrameQueueDurationCriticalTooLong)
{
    SetUpScheduler(EXTERNAL_BFS);
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);
    fake_compositor_timing_history_->SetBeginMainFrameQueueDurationCriticalEstimate(kSlowDuration);
...
TEST_F(SchedulerTest, ImplFrameNotSkippedAfterLateAck_ReadyToActivateEstimateTooLong)
{
    SetUpScheduler(EXTERNAL_BFS);
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);
    fake_compositor_timing_history_->SetCommitToReadyToActivateDurationEstimate(kSlowDuration);
...
void SchedulerTest::BeginFramesNotFromClient_IsDrawThrottled(BeginFrameSourceType bfs_type)
{
    SetUpScheduler(bfs_type);
    ...
    EXPECT_LT(after_deadline, before_deadline + viz::BeginFrameArgs::DefaultInterval());
    EXPECT_FALSE(client_->IsInsideBeginImplFrame());
    client_->Reset();
...
TEST_F(SchedulerTest, DidLoseLayerTreeFrameSinkAfterLayerTreeFrameSinkIsInitialized)
{
    SetUpSchedulerWithNoLayerTreeFrameSink(EXTERNAL_BFS);
...
TEST_F(SchedulerTest, DidLoseLayerTreeFrameSinkAfterBeginFrameStartedWithHighLatency)
{
    SetUpScheduler(EXTERNAL_BFS);
    ...
    scheduler_->NotifyBeginMainFrameStarted(task_runner_->NowTicks());
    scheduler_->NotifyReadyToCommit(nullptr);
    EXPECT_ACTIONS("ScheduledActionCommit", "ScheduledActionPostCommit", "ScheduledActionActivateSyncTree", "ScheduledActionBeginLayerTreeFrameSinkCreation");
}
...
TEST_F(SchedulerTest, DidLoseLayerTreeFrameSinkAfterSetNeedsPrepareTiles)
{
    ...
    client_->Reset();
    task_runner_->RunTasksWhile(client_->InsideBeginImplFrame(true));
    EXPECT_ACTIONS("ScheduledActionPrepareTiles", "ScheduledActionBeginLayerTreeFrameSinkCreation", "RemoveObserver(this)");
}
...
TEST_F(SchedulerTest, ScheduledActionBeginMainFrameNotExpectedUntil)
{
    ...
    task_runner_->RunPendingTasks();
    EXPECT_ACTIONS("WillBeginImplFrame", "ScheduledActionBeginMainFrameNotExpectedUntil", "ScheduledActionDrawIfPossible");
}

// Tests to ensure that BeginMainFrameNotExpectedUntil is only sent once within
// the same frame.
TEST_F(SchedulerTest, ScheduledActionBeginMainFrameNotExpectedUntilSentOnlyOncePerFrame)
{
    SetUpScheduler(EXTERNAL_BFS);
...
TEST_F(SchedulerTest, SendBeginMainFrameNotExpectedSoon_Requested)
{
    ...
    scheduler_->NotifyReadyToActivate();
    task_runner_->RunPendingTasks();
    EXPECT_ACTIONS("WillBeginImplFrame", "ScheduledActionSendBeginMainFrame", "ScheduledActionCommit", "ScheduledActionPostCommit",
        "ScheduledActionActivateSyncTree", "ScheduledActionDrawIfPossible");
    client_->Reset();

    scheduler_->SetMainThreadWantsBeginMainFrameNotExpected(true);

    EXPECT_SCOPED(AdvanceFrame());
    EXPECT_ACTIONS("WillBeginImplFrame", "ScheduledActionBeginMainFrameNotExpectedUntil");
    EXPECT_TRUE(client_->IsInsideBeginImplFrame());
    client_->Reset();
...
TEST_F(SchedulerTest, ScheduledActionBeginMainFrameNotSoonSentDuringIdleIfNeededNotVisible)
{
    SetUpScheduler(EXTERNAL_BFS);
...
TEST_F(SchedulerTest, SynchronousCompositorAnimation)
{
    ...
    // Next vsync.
    AdvanceFrame();
    EXPECT_ACTIONS("WillBeginImplFrame", "ScheduledActionInvalidateLayerTreeFrameSink");
...
TEST_F(SchedulerTest, AbortEarlyIfNoDamage)
{
    ...
    EXPECT_ACTIONS("AddObserver(this)", "WillBeginImplFrame", "RemoveObserver(this)");
    EXPECT_EQ(0, client_->num_draws());
...
TEST_F(SchedulerTest, SynchronousCompositorCommitAndVerifyBeginFrameAcks)
{
    ...
    bool has_damage = false;
    EXPECT_EQ(viz::BeginFrameAck(args, has_damage), client_->last_begin_frame_ack());
    client_->Reset();
...
TEST_F(SchedulerTest, SynchronousCompositorAllowsActivateBeforeDraw)
{
    ...
    scheduler_->SetNeedsBeginMainFrame();
    EXPECT_SCOPED(AdvanceFrame());
    EXPECT_ACTIONS("WillBeginImplFrame", "ScheduledActionSendBeginMainFrame", "ScheduledActionInvalidateLayerTreeFrameSink");
    client_->Reset();
...
TEST_F(SchedulerTest, SetNeedsRedrawFromWillBeginImplFrame)
{
    ...
    AdvanceFrame();
    EXPECT_ACTIONS("ScheduledActionBeginLayerTreeFrameSinkCreation", "AddObserver(this)", "WillBeginImplFrame", "ScheduledActionSendBeginMainFrame");
    EXPECT_TRUE(scheduler_->RedrawPending());
...
TEST_F(SchedulerTest, AuthoritativeVSyncInterval)
{
    ...
    synthetic_frame_source_->OnUpdateVSyncParameters(task_runner_->NowTicks(), authoritative_interval);
...
TEST_F(SchedulerTest, ImplLatencyTakesPriority)
{
    ...
    scheduler_->SetTreePrioritiesAndScrollState(SMOOTHNESS_TAKES_PRIORITY, ScrollHandlerState::SCROLL_AFFECTS_SCROLL_HANDLER);
    scheduler_->SetCriticalBeginMainFrameToActivateIsFast(true);
    EXPECT_FALSE(scheduler_->ImplLatencyTakesPriority());
    scheduler_->SetCriticalBeginMainFrameToActivateIsFast(false);
    EXPECT_TRUE(scheduler_->ImplLatencyTakesPriority());

    scheduler_->SetTreePrioritiesAndScrollState(SAME_PRIORITY_FOR_BOTH_TREES, ScrollHandlerState::SCROLL_DOES_NOT_AFFECT_SCROLL_HANDLER);
    scheduler_->SetCriticalBeginMainFrameToActivateIsFast(true);
    EXPECT_FALSE(scheduler_->ImplLatencyTakesPriority());
    scheduler_->SetCriticalBeginMainFrameToActivateIsFast(false);
    EXPECT_FALSE(scheduler_->ImplLatencyTakesPriority());

    scheduler_->SetTreePrioritiesAndScrollState(SAME_PRIORITY_FOR_BOTH_TREES, ScrollHandlerState::SCROLL_AFFECTS_SCROLL_HANDLER);
    scheduler_->SetCriticalBeginMainFrameToActivateIsFast(true);
    EXPECT_FALSE(scheduler_->ImplLatencyTakesPriority());
    scheduler_->SetCriticalBeginMainFrameToActivateIsFast(false);
    EXPECT_FALSE(scheduler_->ImplLatencyTakesPriority());
...
TEST_F(SchedulerTest, NoLayerTreeFrameSinkCreationWhileCommitPending)
{
    ...
    client_->Reset();
    scheduler_->NotifyBeginMainFrameStarted(task_runner_->NowTicks());
    scheduler_->BeginMainFrameAborted(CommitEarlyOutReason::kAbortedDeferredCommit);
    EXPECT_ACTIONS("ScheduledActionBeginLayerTreeFrameSinkCreation");
}
...
TEST_F(SchedulerTest, ImplSideInvalidationInsideImplFrame)
{
    ...
    client_->Reset();
    EXPECT_SCOPED(AdvanceFrame());
    EXPECT_ACTIONS("WillBeginImplFrame", "ScheduledActionPerformImplSideInvalidation");
}
...
TEST_F(SchedulerTest, InvalidationNotBlockedOnMainFrame)
{
    ...
    client_->Reset();
    EXPECT_SCOPED(AdvanceFrame());
    EXPECT_ACTIONS("WillBeginImplFrame", "ScheduledActionSendBeginMainFrame", "ScheduledActionPerformImplSideInvalidation");
}
...
bool SchedulerTest::BeginMainFrameOnCriticalPath(TreePriority tree_priority, ScrollHandlerState scroll_handler_state, base::TimeDelta durations)
{
    SetUpScheduler(EXTERNAL_BFS);
    fake_compositor_timing_history_->SetAllEstimatesTo(durations);
    client_->Reset();
    scheduler_->SetTreePrioritiesAndScrollState(tree_priority, scroll_handler_state);
    scheduler_->SetNeedsBeginMainFrame();
...
{
    EXPECT_TRUE(BeginMainFrameOnCriticalPath(SAME_PRIORITY_FOR_BOTH_TREES, ScrollHandlerState::SCROLL_DOES_NOT_AFFECT_SCROLL_HANDLER, kFastDuration));
}

TEST_F(SchedulerTest, BeginMainFrameOnCriticalPath_BNS)
{
    EXPECT_TRUE(BeginMainFrameOnCriticalPath(SAME_PRIORITY_FOR_BOTH_TREES, ScrollHandlerState::SCROLL_DOES_NOT_AFFECT_SCROLL_HANDLER, kSlowDuration));
}

TEST_F(SchedulerTest, BeginMainFrameOnCriticalPath_BHF)
{
    EXPECT_TRUE(BeginMainFrameOnCriticalPath(SAME_PRIORITY_FOR_BOTH_TREES, ScrollHandlerState::SCROLL_AFFECTS_SCROLL_HANDLER, kFastDuration));
}

TEST_F(SchedulerTest, BeginMainFrameOnCriticalPath_BHS)
{
    EXPECT_TRUE(BeginMainFrameOnCriticalPath(SAME_PRIORITY_FOR_BOTH_TREES, ScrollHandlerState::SCROLL_AFFECTS_SCROLL_HANDLER, kSlowDuration));
}

TEST_F(SchedulerTest, BeginMainFrameOnCriticalPath_ANF)
{
    EXPECT_FALSE(BeginMainFrameOnCriticalPath(SMOOTHNESS_TAKES_PRIORITY, ScrollHandlerState::SCROLL_DOES_NOT_AFFECT_SCROLL_HANDLER, kFastDuration));
}

TEST_F(SchedulerTest, BeginMainFrameOnCriticalPath_ANS)
{
    EXPECT_FALSE(BeginMainFrameOnCriticalPath(SMOOTHNESS_TAKES_PRIORITY, ScrollHandlerState::SCROLL_DOES_NOT_AFFECT_SCROLL_HANDLER, kSlowDuration));
}

TEST_F(SchedulerTest, BeginMainFrameOnCriticalPath_AHF)
{
    EXPECT_TRUE(BeginMainFrameOnCriticalPath(SMOOTHNESS_TAKES_PRIORITY, ScrollHandlerState::SCROLL_AFFECTS_SCROLL_HANDLER, kFastDuration));
}

TEST_F(SchedulerTest, BeginMainFrameOnCriticalPath_AHS)
{
    EXPECT_FALSE(BeginMainFrameOnCriticalPath(SMOOTHNESS_TAKES_PRIORITY, ScrollHandlerState::SCROLL_AFFECTS_SCROLL_HANDLER, kSlowDuration));
}
...
TEST_F(SchedulerTest, BeginFrameAckForFinishedImplFrame)
{
    ...
    EXPECT_EQ(FrameSkippedReason::kWaitingOnMain, client_->last_frame_skipped_reason());
    client_->Reset();
...
TEST_F(SchedulerTest, BeginFrameAckForDroppedBeginFrame)
{
    ...
    // Latest ack should be for the dropped BeginFrame.
    bool has_damage = false;
    EXPECT_EQ(viz::BeginFrameAck(second_args, has_damage), client_->last_begin_frame_ack());
    client_->Reset();
    ...
    has_damage = false;
    EXPECT_EQ(viz::BeginFrameAck(first_args, has_damage), client_->last_begin_frame_ack());
    client_->Reset();
}
...
TEST_F(SchedulerTest, CriticalBeginMainFrameToActivateIsFast)
{
    ...
    viz::BeginFrameArgs args = viz::BeginFrameArgs::Create(
        BEGINFRAME_FROM_HERE, 0u, 1u, task_runner_->NowTicks(), task_runner_->NowTicks() + interval, interval, viz::BeginFrameArgs::NORMAL);
    fake_external_begin_frame_source_->TestOnBeginFrame(args);
    EXPECT_TRUE(scheduler_->ImplLatencyTakesPriority());
    ...
    args = viz::BeginFrameArgs::Create(
        BEGINFRAME_FROM_HERE, 0u, 2u, task_runner_->NowTicks(), task_runner_->NowTicks() + interval, interval, viz::BeginFrameArgs::NORMAL);
    fake_external_begin_frame_source_->TestOnBeginFrame(args);
    EXPECT_FALSE(scheduler_->ImplLatencyTakesPriority());
    ...
    scheduler_->SetNeedsRedraw();
    fake_compositor_timing_history_->SetDrawDurationEstimate(base::Milliseconds(7));
    task_runner_->AdvanceMockTickClock(interval);
    args = viz::BeginFrameArgs::Create(
        BEGINFRAME_FROM_HERE, 0u, 3u, task_runner_->NowTicks(), task_runner_->NowTicks() + interval, interval, viz::BeginFrameArgs::NORMAL);
    fake_external_begin_frame_source_->TestOnBeginFrame(args);
    EXPECT_TRUE(scheduler_->ImplLatencyTakesPriority());
}
...
TEST_F(SchedulerTest, WaitForAllPipelineStagesUsesMissedBeginFrames)
{
    ...
    viz::BeginFrameArgs args
        = viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, 0u, 1u, timestamp, timestamp + interval, interval, viz::BeginFrameArgs::MISSED);
    fake_external_begin_frame_source_->TestOnBeginFrame(args);
    EXPECT_TRUE(client_->IsInsideBeginImplFrame());
}
...
TEST_F(SchedulerTest, WaitForAllPipelineStagesAlwaysObservesBeginFrames)
{
    ...
    base::TimeTicks timestamp = task_runner_->NowTicks();
    viz::BeginFrameArgs args
        = viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, 0u, 1u, timestamp, timestamp + interval, interval, viz::BeginFrameArgs::NORMAL);
    fake_external_begin_frame_source_->TestOnBeginFrame(args);
    EXPECT_ACTIONS("WillBeginImplFrame");
    EXPECT_TRUE(client_->IsInsideBeginImplFrame());
    client_->Reset();
...
TEST_F(SchedulerTest, CriticalBeginMainFrameIsFast_CommitEstimateSlow)
{
    ...
    EXPECT_SCOPED(AdvanceFrame());
    EXPECT_FALSE(scheduler_->state_machine().critical_begin_main_frame_to_activate_is_fast());
}
...
TEST_F(SchedulerTest, CriticalBeginMainFrameIsFast_CommitEstimateFast)
{
    ...
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);
    EXPECT_SCOPED(AdvanceFrame());
    EXPECT_TRUE(scheduler_->state_machine().critical_begin_main_frame_to_activate_is_fast());
}
...
TEST_F(SchedulerTest, ShouldDeferInvalidation_AllEstimatesFast)
{
    ...
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);
    EXPECT_SCOPED(AdvanceFrame());
    EXPECT_TRUE(scheduler_->state_machine().should_defer_invalidation_for_fast_main_frame());
}
...
TEST_F(SchedulerTest, ShouldDeferInvalidation_BMFStartToReadyToCommitSlow)
{
    ...
    fake_compositor_timing_history_->SetBeginMainFrameStartToReadyToCommitDurationEstimate(kSlowDuration);
    EXPECT_SCOPED(AdvanceFrame());
    EXPECT_FALSE(scheduler_->state_machine().should_defer_invalidation_for_fast_main_frame());
}
...
TEST_F(SchedulerTest, ShouldDeferInvalidation_BMFQueueDurationNotCriticalSlow)
{
    SetUpScheduler(EXTERNAL_BFS);
    scheduler_->SetNeedsBeginMainFrame();
    scheduler_->SetTreePrioritiesAndScrollState(TreePriority::SMOOTHNESS_TAKES_PRIORITY, ScrollHandlerState::SCROLL_DOES_NOT_AFFECT_SCROLL_HANDLER);
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);
    fake_compositor_timing_history_->SetBeginMainFrameQueueDurationNotCriticalEstimate(kSlowDuration);
    EXPECT_SCOPED(AdvanceFrame());
    EXPECT_FALSE(scheduler_->state_machine().should_defer_invalidation_for_fast_main_frame());
...
TEST_F(SchedulerTest, SlowMainThreadRasterButEstimatedFastDoesNotTriggersInvalidations)
{
    SetUpScheduler(EXTERNAL_BFS);
    scheduler_->SetNeedsBeginMainFrame();
    scheduler_->SetNeedsImplSideInvalidation(true);
...
TEST_F(SchedulerTest, SendEarlyDidNotProduceFrameIfIdle)
{
    ...
    EXPECT_NE(client_->last_begin_frame_ack().frame_id.sequence_number, begin_main_frame_args.frame_id.sequence_number);
    ...
    scheduler_->SetNeedsBeginMainFrame();
    scheduler_->BeginMainFrameAborted(CommitEarlyOutReason::kFinishedNoUpdates);
    EXPECT_EQ(client_->last_begin_frame_ack().frame_id.sequence_number, begin_main_frame_args.frame_id.sequence_number);
}

TEST_F(SchedulerTest, HighImplLatencyModePrioritizesMainFramesOverImplInvalidation)
{
    SetUpScheduler(EXTERNAL_BFS);
    fake_compositor_timing_history_->SetAllEstimatesTo(kFastDuration);
    ...
    scheduler_->SetNeedsBeginMainFrame();
    EXPECT_ACTIONS("ScheduledActionSendBeginMainFrame");
    fake_compositor_timing_history_->SetBeginMainFrameSentTime(task_runner_->NowTicks() + base::Milliseconds(8));
    client_->Reset();
    scheduler_->NotifyReadyToActivate();
    task_runner_->RunTasksWhile(client_->InsideBeginImplFrame(true));
    EXPECT_ACTIONS("ScheduledActionActivateSyncTree", "ScheduledActionDrawIfPossible");
...
TEST_F(WarmUpCompositorSchedulerTest, SetShouldWarmUpWillStartLayerTreeFrameSinkCreation)
{
    SetUpSchedulerWithNoLayerTreeFrameSink(EXTERNAL_BFS);
    scheduler_->SetVisible(false);
