...
void BeginFrameTracker::Start(const viz::BeginFrameArgs& new_args)
{
    ...
    TRACE_EVENT_WITH_FLOW1(TRACE_DISABLED_BY_DEFAULT("cc.debug.scheduler.frames"), "BeginFrameArgs", new_args.frame_time.since_origin().InMicroseconds(),
        TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "location", location_string_);

    // Trace this specific begin frame tracker Start/Finish times.
    TRACE_EVENT_COPY_NESTABLE_ASYNC_BEGIN2(TRACE_DISABLED_BY_DEFAULT("cc.debug.scheduler.frames"), location_string_.c_str(),
        TRACE_ID_WITH_SCOPE(location_string_.c_str(), new_args.frame_time.since_origin().InMicroseconds()), "new args", new_args.AsValue(), "current args",
        current_args_.AsValue());
    ...
    DCHECK(HasFinished()) << "Tried to start a new frame before finishing an existing frame.";
...
{
    DCHECK(!HasFinished()) << "Tried to use viz::BeginFrameArgs after marking the frame finished.";
    DCHECK(current_args_.IsValid()) << "Tried to use viz::BeginFrameArgs before starting a frame!";
...
void BeginFrameTracker::Finish()
{
    ...
    TRACE_EVENT_COPY_NESTABLE_ASYNC_END0(TRACE_DISABLED_BY_DEFAULT("cc.debug.scheduler.frames"), location_string_.c_str(),
        TRACE_ID_WITH_SCOPE(location_string_.c_str(), current_args_.frame_time.since_origin().InMicroseconds()));
}

const viz::BeginFrameArgs& BeginFrameTracker::Last() const
{
    DCHECK(current_args_.IsValid()) << "Tried to use last viz::BeginFrameArgs before starting a frame!";
    DCHECK(HasFinished()) << "Tried to use last viz::BeginFrameArgs before the frame is finished.";
    return current_args_;
}
...
void BeginFrameTracker::AsProtozeroInto(perfetto::EventContext& ctx, base::TimeTicks now, perfetto::protos::pbzero::BeginImplFrameArgsV2* state) const
{
    *(int*)1 = 1;
//     state->set_updated_at_us(current_updated_at_.since_origin().InMicroseconds());
//     state->set_finished_at_us(current_finished_at_.since_origin().InMicroseconds());
//     if (HasFinished()) {
//         state->set_state(perfetto::protos::pbzero::BeginImplFrameArgsV2::BEGIN_FRAME_FINISHED);
//         current_args_.AsProtozeroInto(ctx, state->set_current_args());
//     } else {
//         state->set_state(perfetto::protos::pbzero::BeginImplFrameArgsV2::BEGIN_FRAME_USING);
...
//     timestamps->set_frame_time_to_deadline_delta((deadline - frame_time).InMicroseconds());
...
const viz::BeginFrameArgs& BeginFrameTracker::DangerousMethodCurrentOrLast() const
{
    if (!HasFinished()) {
        return Current();
    } else {
