...
double FastMainThreadThresholdAdjustment()
{
    if (base::FeatureList::IsEnabled(features::kAdjustFastMainThreadThreshold)) {
        double result = base::GetFieldTrialParamByFeatureAsDouble(features::kAdjustFastMainThreadThreshold, "Scalar", -1.0);
...
Scheduler::Scheduler(SchedulerClient* client, const SchedulerSettings& settings, int layer_tree_host_id, base::SingleThreadTaskRunner* task_runner,
    std::unique_ptr<CompositorTimingHistory> compositor_timing_history, CompositorFrameReportingController* compositor_frame_reporting_controller)
    : settings_(settings)
    , client_(client)
...
void Scheduler::SetNeedsImplSideInvalidation(bool needs_first_draw_on_activation)
{
    {
        TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug.scheduler"), "Scheduler::SetNeedsImplSideInvalidation", "needs_first_draw_on_activation",
...
base::TimeTicks Scheduler::Now() const
{
    ...
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("cc.debug.scheduler.now"), "Scheduler::Now", "now", now);
    return now;
}
...
void Scheduler::NotifyAnimationWorkletStateChange(AnimationWorkletState state, TreeType tree)
{
...
void Scheduler::DidSubmitCompositorFrame(SubmitInfo& submit_info)
{
    ...
    // software draw.
    if (!settings_.using_synchronous_renderer_compositor || !state_machine_.resourceless_draw()) {
        compositor_frame_reporting_controller_->DidSubmitCompositorFrame(
            submit_info, begin_main_frame_args_.frame_id, last_activate_origin_frame_args_.frame_id);
    }
...
void Scheduler::SetTreePrioritiesAndScrollState(TreePriority tree_priority, ScrollHandlerState scroll_handler_state)
{
    state_machine_.SetTreePrioritiesAndScrollState(tree_priority, scroll_handler_state);
    ProcessScheduledActions();
}

void Scheduler::NotifyReadyToCommit(std::unique_ptr<BeginMainFrameMetrics> details)
{
    {
        TRACE_EVENT0("cc", "Scheduler::NotifyReadyToCommit");
        compositor_timing_history_->NotifyReadyToCommit();
        compositor_frame_reporting_controller_->NotifyReadyToCommit(std::move(details));
...
{
    {
        TRACE_EVENT1("cc", "Scheduler::BeginMainFrameAborted", "reason", CommitEarlyOutReasonToString(reason));
        compositor_timing_history_->BeginMainFrameAborted();
        auto frame_id = last_dispatched_begin_main_frame_args_.frame_id;
        compositor_frame_reporting_controller_->BeginMainFrameAborted(frame_id, reason);
...
void Scheduler::DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& details)
{
    compositor_frame_reporting_controller_->DidPresentCompositorFrame(frame_token, details);
}
...
void Scheduler::NotifyBeginMainFrameStarted(base::TimeTicks main_thread_start_time)
{
    ...
    compositor_frame_reporting_controller_->BeginMainFrameStarted(main_thread_start_time);
}
...
{
    TRACE_EVENT1("cc", "Scheduler::BeginMainFrameNotExpectedUntil", "remaining_time", (time - Now()).InMillisecondsF());
...
{
    if (state_machine_.begin_impl_frame_state() != SchedulerStateMachine::BeginImplFrameState::IDLE) {
        return;
    }
...
void Scheduler::CancelPendingBeginFrameTask()
{
    if (pending_begin_frame_args_.IsValid()) {
        TRACE_EVENT_INSTANT0("cc", "Scheduler::BeginFrameDropped", TRACE_EVENT_SCOPE_THREAD);
        SendDidNotProduceFrame(pending_begin_frame_args_, FrameSkippedReason::kNoDamage);
...
{
    bool is_idle = state_machine_.begin_impl_frame_state() == SchedulerStateMachine::BeginImplFrameState::IDLE;
    ...
    bool has_no_pending_begin_frame_task = pending_begin_frame_task_.IsCancelled();

    if (is_idle && needs_begin_frames && has_pending_begin_frame_args && has_no_pending_begin_frame_task) {
        pending_begin_frame_task_.Reset(base::BindOnce(&Scheduler::HandlePendingBeginFrame, base::Unretained(this)));
...
void Scheduler::OnBeginFrameSourcePausedChanged(bool paused)
{
    if (state_machine_.begin_frame_source_paused() == paused)
        return;
    {
        TRACE_EVENT_INSTANT1("cc", "Scheduler::SetBeginFrameSourcePaused", TRACE_EVENT_SCOPE_THREAD, "paused", paused);
...
bool Scheduler::OnBeginFrameDerivedImpl(const viz::BeginFrameArgs& args)
{
    ...
    TRACE_EVENT_WITH_FLOW0(TRACE_DISABLED_BY_DEFAULT("cc.debug.scheduler.frames"), "viz::BeginFrameArgs", args.frame_time.since_origin().InMicroseconds(),
    ...
    bool inside_previous_begin_frame = state_machine_.begin_impl_frame_state() == SchedulerStateMachine::BeginImplFrameState::INSIDE_BEGIN_FRAME;

    if (inside_process_scheduled_actions_ || inside_previous_begin_frame || pending_begin_frame_args_.IsValid()) {
        ...
        if (pending_begin_frame_args_.IsValid()) {
            TRACE_EVENT_INSTANT0("cc", "Scheduler::BeginFrameDropped", TRACE_EVENT_SCOPE_THREAD);
            SendDidNotProduceFrame(pending_begin_frame_args_, FrameSkippedReason::kRecoverLatency);
        }
...
void Scheduler::OnDrawForLayerTreeFrameSink(bool resourceless_software_draw, bool skip_draw)
{
    DCHECK(settings_.using_synchronous_renderer_compositor);
    if (state_machine_.begin_impl_frame_state() == SchedulerStateMachine::BeginImplFrameState::INSIDE_BEGIN_FRAME) {
        DCHECK(needs_finish_frame_for_synchronous_compositor_);
    } else {
        DCHECK_EQ(state_machine_.begin_impl_frame_state(), SchedulerStateMachine::BeginImplFrameState::IDLE);
        DCHECK(!needs_finish_frame_for_synchronous_compositor_);
    }
    DCHECK(!begin_impl_frame_deadline_timer_.IsRunning());
...
void Scheduler::BeginImplFrameWithDeadline(const viz::BeginFrameArgs& args)
{
    ...
    TRACE_EVENT2("cc,benchmark", "Scheduler::BeginImplFrame", "args", args.AsValue(), "main_thread_missed_last_deadline", main_thread_is_in_high_latency_mode);
    TRACE_COUNTER1(TRACE_DISABLED_BY_DEFAULT("cc.debug.scheduler"), "MainThreadLatency", main_thread_is_in_high_latency_mode);

    base::TimeTicks now = Now();
    // Discard missed begin frames if they are too late. In full-pipe mode, we
    // ignore BeginFrame deadlines.
    if (!settings_.wait_for_all_pipeline_stages_before_draw && args.type == viz::BeginFrameArgs::MISSED && args.deadline < now) {
        TRACE_EVENT_INSTANT0("cc", "Scheduler::MissedBeginFrameDropped", TRACE_EVENT_SCOPE_THREAD);
    ...
    base::TimeDelta bmf_to_activate_threshold = adjusted_args.interval - compositor_timing_history_->DrawDurationEstimate() - kDeadlineFudgeFactor;

    base::TimeDelta bmf_to_activate_estimate_critical = compositor_timing_history_->BeginMainFrameQueueToActivateCriticalEstimate();
    base::TimeDelta fast_main_thread_threshold = bmf_to_activate_threshold * FastMainThreadThresholdAdjustment();
    state_machine_.SetCriticalBeginMainFrameToActivateIsFast(bmf_to_activate_estimate_critical < fast_main_thread_threshold);
    ...
    if (compositor_timing_history_->begin_main_frame_sent_time() != base::TimeTicks()) {
        time_since_main_frame_sent = now - compositor_timing_history_->begin_main_frame_sent_time();
    }
    base::TimeDelta bmf_sent_to_ready_to_commit_estimate;
    if (begin_main_frame_args_.on_critical_path) {
        bmf_sent_to_ready_to_commit_estimate = compositor_timing_history_->BeginMainFrameStartToReadyToCommitCriticalEstimate();
    } else {
        bmf_sent_to_ready_to_commit_estimate = compositor_timing_history_->BeginMainFrameStartToReadyToCommitNotCriticalEstimate();
    }
    ...
    bmf_to_activate_threshold += args.interval * settings_.delay_impl_invalidation_frames;
    if (time_since_main_frame_sent > bmf_to_activate_threshold) {
        ...
        main_thread_response_expected_soon = bmf_sent_to_ready_to_commit_estimate - time_since_main_frame_sent < bmf_to_activate_threshold;
    }
    state_machine_.set_should_defer_invalidation_for_fast_main_frame(main_thread_response_expected_soon);
...
void Scheduler::BeginImplFrameSynchronous(const viz::BeginFrameArgs& args)
{
    ...
    TRACE_EVENT1("cc,benchmark", "Scheduler::BeginImplFrame", "args", args.AsValue());
    ...
    compositor_frame_reporting_controller_->OnFinishImplFrame(adjusted_args.frame_id);
...
void Scheduler::FinishImplFrame()
{
    ...
    if (!state_machine_.did_submit_in_last_frame()) {
        ...
        bool is_waiting_on_main = state_machine_.begin_main_frame_state() != SchedulerStateMachine::BeginMainFrameState::IDLE;
        bool is_draw_throttled = state_machine_.needs_redraw() && state_machine_.IsDrawThrottled();
        ...
        if (begin_impl_frame_tracker_.Current().frame_id != last_activate_origin_frame_args_.frame_id && state_machine_.draw_succeeded_in_last_frame()) {
            compositor_frame_reporting_controller_->DidNotProduceFrame(last_activate_origin_frame_args_.frame_id, FrameSkippedReason::kNoDamage);
        }
    }
...
void Scheduler::SendDidNotProduceFrame(const viz::BeginFrameArgs& args, FrameSkippedReason reason)
{
    ...
    compositor_frame_reporting_controller_->DidNotProduceFrame(args.frame_id, reason);
}

// BeginImplFrame starts a compositor frame that will wait up until a deadline
// for a BeginMainFrame+activation to complete before it times out and draws
// any asynchronous animation and scroll/pinch updates.
void Scheduler::BeginImplFrame(const viz::BeginFrameArgs& args, base::TimeTicks now)
{
    DCHECK_EQ(state_machine_.begin_impl_frame_state(), SchedulerStateMachine::BeginImplFrameState::IDLE);
    DCHECK(!begin_impl_frame_deadline_timer_.IsRunning());
...

    {
        ...
        bool has_damage = client_->WillBeginImplFrame(begin_impl_frame_tracker_.Current());
...
void Scheduler::ScheduleBeginImplFrameDeadline()
{
    ...
    case DeadlineMode::LATE: {
        ...
        // have nothing to do.
        new_deadline = begin_impl_frame_tracker_.Current().frame_time + begin_impl_frame_tracker_.Current().interval;
        ...
        if (!state_machine_.NewActiveTreeLikely()) {
            SendDidNotProduceFrame(begin_impl_frame_tracker_.Current(), FrameSkippedReason::kNoDamage);
        }
        break;
    }
    ...
    case DeadlineMode::WAIT_FOR_SCROLL:
        new_deadline = begin_impl_frame_tracker_.Current().frame_time + begin_impl_frame_tracker_.Current().interval * settings_.scroll_deadline_ratio;
        break;
    }
    ...
    if (has_no_deadline_task || new_deadline != deadline_) {
        TRACE_EVENT2("cc", "Scheduler::ScheduleBeginImplFrameDeadline", "new deadline", new_deadline, "deadline mode",
            SchedulerStateMachine::BeginImplFrameDeadlineModeToString(deadline_mode_));
        deadline_ = new_deadline;
        static const unsigned char* debug_tracing_enabled = nullptr; // TRACE_EVENT_API_GET_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("cc.debug.scheduler"));
        if (debug_tracing_enabled)
            deadline_scheduled_at_ = Now();
        begin_impl_frame_deadline_timer_.Stop();
        begin_impl_frame_deadline_timer_.Start(
            FROM_HERE, deadline_, base::BindOnce(&Scheduler::OnBeginImplFrameDeadline, base::Unretained(this)), base::subtle::DelayPolicy::kPrecise);
    }
}
...
{
    {
        ...
        if (!settings_.using_synchronous_renderer_compositor) {
            compositor_timing_history_->WillFinishImplFrame(state_machine_.needs_redraw());
            compositor_frame_reporting_controller_->OnFinishImplFrame(begin_main_frame_args_.frame_id);
        }
...
void Scheduler::DrawForced()
{
    DCHECK(!inside_scheduled_action_);
    base::AutoReset<bool> mark_inside(&inside_scheduled_action_, true);
    bool drawing_with_new_active_tree = state_machine_.active_tree_needs_first_draw() && !state_machine_.previous_pending_tree_was_impl_side();
    if (drawing_with_new_active_tree) {
        TRACE_EVENT_WITH_FLOW1("viz,benchmark", "Graphics.Pipeline.DrawForced", TRACE_ID_GLOBAL(last_activate_origin_frame_args().trace_id),
            TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "trace_id", last_activate_origin_frame_args().trace_id);
    }
    compositor_timing_history_->WillDraw();
    state_machine_.WillDraw();
...
{
    {
        TRACE_EVENT1("cc", "Scheduler::SetDeferBeginMainFrame", "defer_begin_main_frame", defer_begin_main_frame);
...
{
    {
        TRACE_EVENT1("cc", "Scheduler::SetPauseRendering", "pause_rendering", pause_rendering);
...
void Scheduler::ProcessScheduledActions()
{
    ...
    do {
        ...
        TRACE_EVENT(TRACE_DISABLED_BY_DEFAULT("cc.debug.scheduler"), "SchedulerStateMachine", [this](perfetto::EventContext ctx) {
            this->AsProtozeroInto(ctx, ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>()->set_cc_scheduler_state());
        });
        base::AutoReset<SchedulerStateMachine::Action> mark_inside_action(&inside_action_, action);
        switch (action) {
        ...
        case SchedulerStateMachine::Action::SEND_BEGIN_MAIN_FRAME:
            ...
            compositor_frame_reporting_controller_->WillBeginMainFrame(begin_main_frame_args_);
            state_machine_.WillSendBeginMainFrame();
            client_->ScheduledActionSendBeginMainFrame(begin_main_frame_args_);
            last_dispatched_begin_main_frame_args_ = begin_main_frame_args_;
            break;
        case SchedulerStateMachine::Action::NOTIFY_BEGIN_MAIN_FRAME_NOT_EXPECTED_UNTIL:
            state_machine_.WillNotifyBeginMainFrameNotExpectedUntil();
            BeginMainFrameNotExpectedUntil(begin_main_frame_args_.frame_time + begin_main_frame_args_.interval);
            break;
        case SchedulerStateMachine::Action::NOTIFY_BEGIN_MAIN_FRAME_NOT_EXPECTED_SOON:
        ...
        case SchedulerStateMachine::Action::INVALIDATE_LAYER_TREE_FRAME_SINK:
            ...
            client_->ScheduledActionInvalidateLayerTreeFrameSink(state_machine_.RedrawPending());
            break;
        }
...
void Scheduler::AsProtozeroInto(perfetto::EventContext& ctx, perfetto::protos::pbzero::ChromeCompositorSchedulerStateV2* state) const
{
//     base::TimeTicks now = Now();
// 
...
//     state->set_begin_impl_frame_deadline_task(begin_impl_frame_deadline_timer_.IsRunning());
//     state->set_pending_begin_frame_task(!pending_begin_frame_task_.IsCancelled());
//     state->set_skipped_last_frame_missed_exceeded_deadline(skipped_last_frame_missed_exceeded_deadline_);
//     state->set_inside_action(SchedulerStateMachine::ActionToProtozeroEnum(inside_action_));
//     state->set_deadline_mode(SchedulerStateMachine::BeginImplFrameDeadlineModeToProtozeroEnum(deadline_mode_));
// 
//     state->set_deadline_us(deadline_.since_origin().InMicroseconds());
//     state->set_deadline_scheduled_at_us(deadline_scheduled_at_.since_origin().InMicroseconds());
// 
//     state->set_now_us(Now().since_origin().InMicroseconds());
//     state->set_now_to_deadline_delta_us((deadline_ - Now()).InMicroseconds());
//     state->set_now_to_deadline_scheduled_at_delta_us((deadline_scheduled_at_ - Now()).InMicroseconds());
// 
//     begin_impl_frame_tracker_.AsProtozeroInto(ctx, now, state->set_begin_impl_frame_args());
// 
//     BeginFrameObserverBase::AsProtozeroInto(ctx, state->set_begin_frame_observer_state());
// 
//     if (begin_frame_source_) {
//         begin_frame_source_->AsProtozeroInto(ctx, state->set_begin_frame_source_state());
//     }
}

void Scheduler::UpdateCompositorTimingHistoryRecordingEnabled()
{
    compositor_timing_history_->SetRecordingEnabled(state_machine_.HasInitializedLayerTreeFrameSink() && state_machine_.visible());
}

size_t Scheduler::CommitDurationSampleCountForTesting() const
{
    return compositor_timing_history_->CommitDurationSampleCountForTesting(); // IN-TEST
}
