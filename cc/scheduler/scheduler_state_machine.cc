...
const char* SchedulerStateMachine::BeginImplFrameDeadlineModeToString(BeginImplFrameDeadlineMode mode)
{
    switch (mode) {
...
// perfetto::protos::pbzero::ChromeCompositorSchedulerStateV2::BeginImplFrameDeadlineMode SchedulerStateMachine::BeginImplFrameDeadlineModeToProtozeroEnum(
//     BeginImplFrameDeadlineMode mode)
// {
//     using pbzeroSchedulerState = perfetto::protos::pbzero::ChromeCompositorSchedulerStateV2;
//     switch (mode) {
//     case BeginImplFrameDeadlineMode::NONE:
...
bool SchedulerStateMachine::PendingDrawsShouldBeAborted() const
{
    ...
    bool is_layer_tree_frame_sink_lost = (layer_tree_frame_sink_state_ == LayerTreeFrameSinkState::NONE);
    ...
    return is_layer_tree_frame_sink_lost || !can_draw_ || !visible_ || begin_frame_source_paused_ || waiting_for_activation_after_rendering_resumed_;
}
...
bool SchedulerStateMachine::ShouldBeginLayerTreeFrameSinkCreation() const
{
    ...
    if (begin_main_frame_state_ != BeginMainFrameState::IDLE || next_begin_main_frame_state_ != BeginMainFrameState::IDLE) {
        return false;
    }
...
bool SchedulerStateMachine::ShouldDraw() const
{
    ...
    if ((settings_.wait_for_all_pipeline_stages_before_draw || settings_.commit_to_active_tree) && !active_tree_is_ready_to_draw_) {
        return false;
    }
...
bool SchedulerStateMachine::ShouldActivateSyncTree() const
{
    ...
    if (!has_pending_tree_) {
        TRACE_EVENT_INSTANT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "Not activating sync tree due to no pending tree", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }
    ...
    if (!settings_.using_synchronous_renderer_compositor && active_tree_needs_first_draw_) {
        TRACE_EVENT_INSTANT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "Not activating before drawing active first", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }
    ...
    if (processing_paint_worklets_for_pending_tree_) {
        TRACE_EVENT_INSTANT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "Not activating due to processing paint worklets", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }
    ...
    if (processing_animation_worklets_for_pending_tree_) {
        TRACE_EVENT_INSTANT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "Not activating due to processing animation worklets", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }

    // At this point, only activate if we are ready to activate.
    if (!pending_tree_is_ready_for_activation_) {
        TRACE_EVENT_INSTANT0(TRACE_DISABLED_BY_DEFAULT("cc.debug"), "Not activating because pending tree not ready", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }
    return true;
}
...
bool SchedulerStateMachine::ShouldNotifyBeginMainFrameNotExpectedUntil() const
{
    ...
    // progress.
    if (needs_begin_main_frame_ || begin_main_frame_state_ != BeginMainFrameState::IDLE || next_begin_main_frame_state_ != BeginMainFrameState::IDLE) {
        return false;
    }
    ...
    if (!BeginFrameNeeded() && begin_impl_frame_state_ == BeginImplFrameState::IDLE) {
        return false;
    }
...
bool SchedulerStateMachine::ShouldSendBeginMainFrame() const
{
    ...
    case BeginMainFrameState::SENT:
        return false;
    case BeginMainFrameState::READY_TO_COMMIT:
        if (!settings_.main_frame_before_commit_enabled || next_begin_main_frame_state_ != BeginMainFrameState::IDLE) {
            return false;
        }
        break;
    }
    ...
    bool can_send_main_frame_with_pending_tree = settings_.main_frame_before_activation_enabled || current_pending_tree_is_impl_side_;
    ...
    if (ImplLatencyTakesPriority()
        && ((has_pending_tree_ && !current_pending_tree_is_impl_side_) || (active_tree_needs_first_draw_ && !previous_pending_tree_was_impl_side_))) {
        return false;
    }
    ...
    if (!settings_.using_synchronous_renderer_compositor && begin_impl_frame_state_ == BeginImplFrameState::IDLE) {
        return false;
    }
    ...
    bool just_submitted_in_deadline = begin_impl_frame_state_ == BeginImplFrameState::INSIDE_DEADLINE && did_submit_in_last_frame_;
...
bool SchedulerStateMachine::ShouldCommit() const
{
    ...
    if (has_pending_tree_) {
        DCHECK(settings_.main_frame_before_activation_enabled || settings_.main_frame_before_commit_enabled || current_pending_tree_is_impl_side_);
        return false;
    }
...
bool SchedulerStateMachine::ShouldInvalidateLayerTreeFrameSink() const
{
    ...
    return (needs_redraw_ && !PendingDrawsShouldBeAborted()) || needs_prepare_tiles_;
}
...
void SchedulerStateMachine::WillPerformImplSideInvalidationInternal()
{
    ...
    pending_tree_needs_first_draw_on_activation_ = next_invalidation_needs_first_draw_on_activation_;
...
{
    DCHECK(!has_pending_tree_ || settings_.main_frame_before_activation_enabled || current_pending_tree_is_impl_side_);
    DCHECK(visible_);
...
void SchedulerStateMachine::WillNotifyBeginMainFrameNotExpectedUntil()
{
    DCHECK(visible_);
    DCHECK(!begin_frame_source_paused_);
    DCHECK(BeginFrameNeeded() || begin_impl_frame_state_ != BeginImplFrameState::IDLE);
...
{
    bool can_have_pending_tree = commit_has_no_updates
        && (settings_.main_frame_before_activation_enabled || settings_.main_frame_before_commit_enabled || current_pending_tree_is_impl_side_);
    DCHECK(!has_pending_tree_ || can_have_pending_tree);
    DCHECK(settings_.main_frame_before_commit_enabled || next_begin_main_frame_state_ == BeginMainFrameState::IDLE);
    ...
    if (commit_has_no_updates) {
        ...
        DCHECK(settings_.main_frame_before_commit_enabled || next_begin_main_frame_state_ == BeginMainFrameState::IDLE);
        DCHECK_NE(next_begin_main_frame_state_, BeginMainFrameState::READY_TO_COMMIT);
    ...
    if (!commit_has_no_updates) {
        ...
        if (!active_tree_needs_first_draw_ || !settings_.wait_for_all_pipeline_stages_before_draw) {
    ...
    if (forced_redraw_state_ == ForcedRedrawOnTimeoutState::WAITING_FOR_COMMIT) {
        forced_redraw_state_ = has_pending_tree_ ? ForcedRedrawOnTimeoutState::WAITING_FOR_ACTIVATION : ForcedRedrawOnTimeoutState::WAITING_FOR_DRAW;
    }

    // Update the output surface state.
    if (layer_tree_frame_sink_state_ == LayerTreeFrameSinkState::WAITING_FOR_FIRST_COMMIT) {
        layer_tree_frame_sink_state_ = has_pending_tree_ ? LayerTreeFrameSinkState::WAITING_FOR_FIRST_ACTIVATION : LayerTreeFrameSinkState::ACTIVE;
    }
}
...
void SchedulerStateMachine::WillActivate()
{
    ...
    if (layer_tree_frame_sink_state_ == LayerTreeFrameSinkState::WAITING_FOR_FIRST_ACTIVATION)
...
void SchedulerStateMachine::WillDrawInternal()
{
    ...
    // of the deadline.
    main_thread_missed_last_deadline_ = CommitPending() || (has_pending_tree_ && !current_pending_tree_is_impl_side_);
...
void SchedulerStateMachine::DidDrawInternal(DrawResult draw_result)
{
    ...
    case DrawResult::kInvalidResult:
        NOTREACHED() << "Invalid return DrawResult:" << static_cast<int>(DrawResult::kInvalidResult);
    case DrawResult::kAbortedCantDraw:
        if (consecutive_cant_draw_count_++ < 3u) {
            needs_redraw_ = true;
        } else {
            DUMP_WILL_BE_NOTREACHED() << consecutive_cant_draw_count_ << " consecutve draws"
    ...
    case DrawResult::kAbortedCheckerboardAnimations:
        ...
        if (consecutive_checkerboard_animations_ >= settings_.maximum_number_of_failed_draws_before_draw_is_forced
            && forced_redraw_state_ == ForcedRedrawOnTimeoutState::IDLE) {
...
void SchedulerStateMachine::SetNeedsImplSideInvalidation(bool needs_first_draw_on_activation)
{
    needs_impl_side_invalidation_ = true;
    next_invalidation_needs_first_draw_on_activation_ |= needs_first_draw_on_activation;
}

void SchedulerStateMachine::SetMainThreadWantsBeginMainFrameNotExpectedMessages(bool new_state)
{
...
void SchedulerStateMachine::WillInvalidateLayerTreeFrameSink()
{
    ...
    last_frame_number_invalidate_layer_tree_frame_sink_performed_ = current_frame_number_;
}
...
void SchedulerStateMachine::SetVideoNeedsBeginFrames(bool video_needs_begin_frames)
{
    video_needs_begin_frames_ = video_needs_begin_frames;
}

void SchedulerStateMachine::SetDeferBeginMainFrame(bool defer_begin_main_frame)
{
...
bool SchedulerStateMachine::BeginFrameRequiredForAction() const
{
    ...
    return needs_redraw_ || needs_one_begin_impl_frame_ || (needs_begin_main_frame_ && !defer_begin_main_frame_) || needs_impl_side_invalidation_;
}
...
bool SchedulerStateMachine::ProactiveBeginFrameWanted() const
{
    ...
    // stay idle).
    if ((begin_main_frame_state_ != BeginMainFrameState::IDLE) && !defer_begin_main_frame_)
        return true;
...
void SchedulerStateMachine::OnBeginImplFrame(const viz::BeginFrameId& frame_id, bool animate_only)
{
...
SchedulerStateMachine::BeginImplFrameDeadlineMode SchedulerStateMachine::CurrentBeginImplFrameDeadlineMode() const
{
    const bool outside_begin_frame = begin_impl_frame_state_ != BeginImplFrameState::INSIDE_BEGIN_FRAME;
...
bool SchedulerStateMachine::ShouldWaitForScrollEvent() const
{
    ...
    if (settings_.scroll_deadline_mode_enabled && is_scrolling_ && waiting_for_scroll_event_) {
        return true;
    }
    return false;
}

bool SchedulerStateMachine::ShouldTriggerBeginImplFrameDeadlineImmediately() const
{
...
{
    if (!settings_.wait_for_all_pipeline_stages_before_draw && !settings_.commit_to_active_tree) {
        return false;
    }
    ...
    if (defer_begin_main_frame_ && settings_.wait_for_all_pipeline_stages_before_draw) {
        return false;
    }
...
{
    return pending_submit_frames_ >= kMaxPendingSubmitFrames && !settings_.disable_frame_rate_limit;
}
...
void SchedulerStateMachine::SetResourcelessSoftwareDraw(bool resourceless_draw)
{
...
{
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1("cc", "Scheduler:pending_submit_frames", TRACE_ID_LOCAL(this), "pending_frames", pending_submit_frames_);
...
{
    TRACE_EVENT_NESTABLE_ASYNC_END1("cc", "Scheduler:pending_submit_frames", TRACE_ID_LOCAL(this), "pending_frames", pending_submit_frames_);
    pending_submit_frames_--;
}

void SchedulerStateMachine::SetTreePrioritiesAndScrollState(TreePriority tree_priority, ScrollHandlerState scroll_handler_state)
{
    tree_priority_ = tree_priority;
    scroll_handler_state_ = scroll_handler_state;
}

void SchedulerStateMachine::SetCriticalBeginMainFrameToActivateIsFast(bool is_fast)
{
...
{
    if (layer_tree_frame_sink_state_ == LayerTreeFrameSinkState::NONE || layer_tree_frame_sink_state_ == LayerTreeFrameSinkState::CREATING)
        return;
...
void SchedulerStateMachine::NotifyAnimationWorkletStateChange(AnimationWorkletState state, TreeType tree)
{
    if (tree == TreeType::ACTIVE) {
        ...
        processing_animation_worklets_for_pending_tree_ = (state == AnimationWorkletState::PROCESSING);
    }
}

void SchedulerStateMachine::NotifyPaintWorkletStateChange(PaintWorkletState state)
{
    bool processing_paint_worklets_for_pending_tree = (state == PaintWorkletState::PROCESSING);
    DCHECK_NE(processing_paint_worklets_for_pending_tree, processing_paint_worklets_for_pending_tree_);
    processing_paint_worklets_for_pending_tree_ = processing_paint_worklets_for_pending_tree;
}
