...
public:
    ...
    virtual void ScheduledActionSendBeginMainFrame(const viz::BeginFrameArgs& args) = 0;
    ...
    virtual void DidNotProduceFrame(const viz::BeginFrameAck& ack, FrameSkippedReason reason) = 0;
    virtual void WillNotReceiveBeginFrame() = 0;
    virtual void SendBeginMainFrameNotExpectedSoon() = 0;
    virtual void ScheduledActionBeginMainFrameNotExpectedUntil(base::TimeTicks time) = 0;
...
public:
    Scheduler(SchedulerClient* client, const SchedulerSettings& scheduler_settings, int layer_tree_host_id, base::SingleThreadTaskRunner* task_runner,
    ...
    void OnDrawForLayerTreeFrameSink(bool resourceless_software_draw, bool skip_draw);

    const SchedulerSettings& settings() const
    {
        return settings_;
    }
    ...
    void NotifyAnimationWorkletStateChange(AnimationWorkletState state, TreeType tree);
    ...
    void SetTreePrioritiesAndScrollState(TreePriority tree_priority, ScrollHandlerState scroll_handler_state);
    ...
    void DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& details);
    ...
    {
        return state_machine_.RedrawPending();
    }
    ...
    void AsProtozeroInto(perfetto::EventContext& ctx, perfetto::protos::pbzero::ChromeCompositorSchedulerStateV2* state) const;
...
protected:
    ...
    // destroyed.
    raw_ptr<CompositorFrameReportingController, AcrossTasksDanglingUntriaged> compositor_frame_reporting_controller_;

    // What the latest deadline was, and when it was scheduled.
    base::TimeTicks deadline_;
    base::TimeTicks deadline_scheduled_at_;
    SchedulerStateMachine::BeginImplFrameDeadlineMode deadline_mode_ = SchedulerStateMachine::BeginImplFrameDeadlineMode::NONE;
    ...
    SchedulerStateMachine::Action inside_action_ = SchedulerStateMachine::Action::NONE;
...
private:
    ...
    void SendDidNotProduceFrame(const viz::BeginFrameArgs& args, FrameSkippedReason reason);
