...
InputMetricEvent GetInputMetricEvent(ScrollEventMetrics::ScrollType scroll_type, EventMetrics::EventType event_type)
{
...
void UkmManager::RecordCompositorLatencyUKM(const CompositorFrameReporter::FrameReportTypes& report_types,
    const std::vector<CompositorFrameReporter::StageData>& stage_history, const ActiveTrackers& active_trackers,
    const CompositorFrameReporter::ProcessedBlinkBreakdown& processed_blink_breakdown,
    const CompositorFrameReporter::ProcessedVizBreakdown& processed_viz_breakdown) const
{
    using StageType = CompositorFrameReporter::StageType;

    ukm::builders::Graphics_Smoothness_Latency builder(source_id_);

    if (report_types.test(static_cast<size_t>(CompositorFrameReporter::FrameReportType::kDroppedFrame))) {
...
#undef CASE_FOR_STAGE
    ...
    for (auto it = processed_blink_breakdown.CreateIterator(); it.IsValid(); it.Advance()) {
        switch (it.GetBreakdown()) {
#define CASE_FOR_BLINK_BREAKDOWN(name)                                                                                                                         \
    case CompositorFrameReporter::BlinkBreakdown::k##name:                                                                                                     \
        builder.SetSendBeginMainFrameToCommit_##name(it.GetLatency().InMicroseconds());                                                                        \
        break;
...
#undef CASE_FOR_BLINK_BREAKDOWN
    ...
    for (auto it = processed_viz_breakdown.CreateIterator(false); it.IsValid(); it.Advance()) {
        switch (it.GetBreakdown()) {
#define CASE_FOR_VIZ_BREAKDOWN(name)                                                                                                                           \
    case CompositorFrameReporter::VizBreakdown::k##name:                                                                                                       \
        builder.SetSubmitCompositorFrameToPresentationCompositorFrame_##name(it.GetDuration().InMicroseconds());                                               \
        break;
...
#undef CASE_FOR_VIZ_BREAKDOWN
    ...
    for (size_t type = 0; type < active_trackers.size(); ++type) {
        if (!active_trackers.test(type))
            continue;
        const auto frame_sequence_tracker_type = static_cast<FrameSequenceTrackerType>(type);
...
void UkmManager::RecordEventLatencyUKM(const EventMetrics::List& events_metrics, const std::vector<CompositorFrameReporter::StageData>& stage_history,
    const CompositorFrameReporter::ProcessedBlinkBreakdown& processed_blink_breakdown,
    const CompositorFrameReporter::ProcessedVizBreakdown& processed_viz_breakdown) const
{
    using StageType = CompositorFrameReporter::StageType;
    ...
    for (const auto& event_metrics : events_metrics) {
        ...
        base::TimeTicks generated_timestamp = event_metrics->GetDispatchStageTimestamp(EventMetrics::DispatchStage::kGenerated);
        ...
        if (ScrollEventMetrics* scroll_metrics = event_metrics->AsScroll()) {
            ...
            builder.SetScrollInputType(static_cast<int64_t>(scroll_metrics->scroll_type()));
        } else if (PinchEventMetrics* pinch_metrics = event_metrics->AsPinch()) {
            builder.SetPinchInputType(static_cast<int64_t>(pinch_metrics->pinch_type()));
        }
        ...
        while (dispatch_stage != EventMetrics::DispatchStage::kMaxValue) {
            ...
            auto end_stage = static_cast<EventMetrics::DispatchStage>(dispatch_index + 1);
            ...
            const int64_t dispatch_latency = (end_timestamp - dispatch_timestamp).InMicroseconds();
            ...
            case EventMetrics::DispatchStage::kArrivedInBrowserMain:
                ...
                builder.SetGenerationToRendererCompositor((end_timestamp - generated_timestamp).InMicroseconds());
                break;
            case EventMetrics::DispatchStage::kArrivedInRendererCompositor:
                switch (end_stage) {
...
#define CASE_FOR_STAGE(stage_name, metrics_suffix)                                                                                                             \
    case StageType::k##stage_name:                                                                                                                             \
        builder.SetRendererCompositorFinishedTo##metrics_suffix((stage_it->start_time - dispatch_timestamp).InMicroseconds());                                 \
        break;
                ...
                CASE_FOR_STAGE(SubmitCompositorFrameToPresentationCompositorFrame, SubmitCompositorFrame);
#undef CASE_FOR_STAGE
...
#define CASE_FOR_STAGE(stage_name, metrics_suffix)                                                                                                             \
    case StageType::k##stage_name:                                                                                                                             \
        builder.SetRendererMainFinishedTo##metrics_suffix((stage_it->start_time - dispatch_timestamp).InMicroseconds());                                       \
        break;
                CASE_FOR_STAGE(BeginImplFrameToSendBeginMainFrame, BeginImplFrame);
                CASE_FOR_STAGE(SendBeginMainFrameToCommit, SendBeginMainFrame);
                CASE_FOR_STAGE(Commit, Commit);
...
#undef CASE_FOR_STAGE
        ...
        for (; stage_it != stage_history.end(); ++stage_it) {
            ...
            const base::TimeTicks start_time = stage_it->stage_type == StageType::kTotalLatency ? generated_timestamp : stage_it->start_time;
            ...
            if (stage_it->stage_type == StageType::kTotalLatency) {
                ...
                if (scroll_type.has_value()) {
                    switch (GetInputMetricEvent(scroll_type.value(), event_metrics->type())) {
