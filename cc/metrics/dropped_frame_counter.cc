...
void SlidingWindowHistogram::AddPercentDroppedFrame(double percent_dropped_frame, size_t count)
{
...
uint32_t SlidingWindowHistogram::GetPercentDroppedFramePercentile(double percentile) const
{
    if (total_count_ == 0)
        return 0;
...
std::vector<double> SlidingWindowHistogram::GetPercentDroppedFrameBuckets() const
{
    if (total_count_ == 0)
    ...
    for (size_t i = 0; i < std::size(kBucketBounds); ++i) {
        buckets[i] = static_cast<double>(smoothness_buckets_[i]) * 100 / total_count_;
    }
    return buckets;
}
...
std::ostream& operator<<(std::ostream& stream, const DroppedFrameCounter::SlidingWindowHistogram& histogram)
{
    return histogram.Dump(stream);
}

DroppedFrameCounter::DroppedFrameCounter()
    : frame_sorter_(base::BindRepeating(&DroppedFrameCounter::NotifyFrameResult, base::Unretained(this)))
{
}
...
void DroppedFrameCounter::ResetPendingFrames(base::TimeTicks timestamp)
{
    ...
    if (!latest_sliding_window_start_.is_null()) {
        ...
        if (sliding_window_.empty()) {
            DCHECK_EQ(dropped_frame_count_in_window_[SmoothnessStrategy::kDefaultStrategy], 0u);
            DCHECK_EQ(dropped_frame_count_in_window_[SmoothnessStrategy::kCompositorFocusedStrategy], 0u);
            DCHECK_EQ(dropped_frame_count_in_window_[SmoothnessStrategy::kMainFocusedStrategy], 0u);
            DCHECK_EQ(dropped_frame_count_in_window_[SmoothnessStrategy::kScrollFocusedStrategy], 0u);
        }
        ...
        if (latest_sliding_window_start_ < report_until) {
            ...
            const size_t count = std::ceil(difference / latest_sliding_window_interval_);
            if (count > 0) {
                sliding_window_histogram_[SmoothnessStrategy::kDefaultStrategy].AddPercentDroppedFrame(0., count);
                sliding_window_histogram_[SmoothnessStrategy::kMainFocusedStrategy].AddPercentDroppedFrame(0., count);
                sliding_window_histogram_[SmoothnessStrategy::kCompositorFocusedStrategy].AddPercentDroppedFrame(0., count);
                sliding_window_histogram_[SmoothnessStrategy::kScrollFocusedStrategy].AddPercentDroppedFrame(0., count);
            }
        }
    }

    std::fill_n(dropped_frame_count_in_window_, SmoothnessStrategy::kStrategyCount, 0);
    sliding_window_ = {};
...
void DroppedFrameCounter::OnEndFrame(const viz::BeginFrameArgs& args, const FrameInfo& frame_info)
{
    ...
    if (is_dropped && fcp_received_ && args.frame_time >= time_fcp_received_ && !frame_sorter_.IsAlreadyReportedDropped(args.frame_id)) {
...
//     TRACE_EVENT2("cc,benchmark", "SmoothnessDroppedFrame", "total", total_frames, "smoothness", total_smoothness_dropped_);
//     if (sliding_window_max_percent_dropped_ != last_reported_metrics_.max_window) {
//         UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.MaxPercentDroppedFrames_1sWindow", sliding_window_max_percent_dropped_);
//         last_reported_metrics_.max_window = sliding_window_max_percent_dropped_;
//     }
// 
//     uint32_t sliding_window_95pct_percent_dropped = SlidingWindow95PercentilePercentDropped(SmoothnessStrategy::kDefaultStrategy);
//     if (sliding_window_95pct_percent_dropped != last_reported_metrics_.p95_window) {
//         UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.95pctPercentDroppedFrames_1sWindow", sliding_window_95pct_percent_dropped);
//         last_reported_metrics_.p95_window = sliding_window_95pct_percent_dropped;
//     }
// 
//     DCHECK_LE(sliding_window_95pct_percent_dropped, static_cast<uint32_t>(std::round(sliding_window_max_percent_dropped_)));
// 
//     // Emit trace event with most recent smoothness calculation. This matches
//     // the smoothness metrics displayed on HeadsUpDisplay.
//     TRACE_EVENT2("cc,benchmark", "SmoothnessDroppedFrame::MostRecentCalculation", "worst_smoothness", sliding_window_max_percent_dropped_,
...
//         smoothness_data.avg_smoothness = static_cast<double>(total_smoothness_dropped_) * 100 / total_frames;
...
//         smoothness_data.median_smoothness = SlidingWindowMedianPercentDropped(SmoothnessStrategy::kDefaultStrategy);
// 
//         uint32_t default_variance = static_cast<uint32_t>(SlidingWindowPercentDroppedVariance(SmoothnessStrategy::kDefaultStrategy));
//         DCHECK_LE(default_variance, 5000u);
//         DCHECK_LE(0u, default_variance);
//         smoothness_data.variance = default_variance;
// 
//         std::vector<double> sliding_window_buckets = sliding_window_histogram_[SmoothnessStrategy::kDefaultStrategy].GetPercentDroppedFrameBuckets();
//         DCHECK_EQ(sliding_window_buckets.size(), std::size(smoothness_data.buckets));
//         base::ranges::copy(sliding_window_buckets, smoothness_data.buckets);
// 
//         smoothness_data.main_focused_median = SlidingWindowMedianPercentDropped(SmoothnessStrategy::kMainFocusedStrategy);
//         smoothness_data.main_focused_percentile_95 = SlidingWindow95PercentilePercentDropped(SmoothnessStrategy::kMainFocusedStrategy);
//         smoothness_data.main_focused_variance = static_cast<uint32_t>(SlidingWindowPercentDroppedVariance(SmoothnessStrategy::kMainFocusedStrategy));
// 
//         smoothness_data.compositor_focused_median = SlidingWindowMedianPercentDropped(SmoothnessStrategy::kCompositorFocusedStrategy);
//         smoothness_data.compositor_focused_percentile_95 = SlidingWindow95PercentilePercentDropped(SmoothnessStrategy::kCompositorFocusedStrategy);
//         smoothness_data.compositor_focused_variance
//             = static_cast<uint32_t>(SlidingWindowPercentDroppedVariance(SmoothnessStrategy::kCompositorFocusedStrategy));
// 
//         smoothness_data.scroll_focused_median = SlidingWindowMedianPercentDropped(SmoothnessStrategy::kScrollFocusedStrategy);
//         smoothness_data.scroll_focused_percentile_95 = SlidingWindow95PercentilePercentDropped(SmoothnessStrategy::kScrollFocusedStrategy);
...
//             smoothness_data.worst_smoothness_after1sec = sliding_window_max_percent_dropped_After_1_sec_.value();
//         if (sliding_window_max_percent_dropped_After_2_sec_.has_value())
//             smoothness_data.worst_smoothness_after2sec = sliding_window_max_percent_dropped_After_2_sec_.value();
//         if (sliding_window_max_percent_dropped_After_5_sec_.has_value())
//             smoothness_data.worst_smoothness_after5sec = sliding_window_max_percent_dropped_After_5_sec_.value();
...
void DroppedFrameCounter::ReportFramesOnEveryFrameForUI()
{
    ...
    recorder->ReportPercentDroppedFramesInOneSecondWindow2(*sliding_window_current_percent_dropped_);
}

void DroppedFrameCounter::SetUkmSmoothnessDestination(UkmSmoothnessDataShared* smoothness_data)
{
...
void DroppedFrameCounter::Reset()
{
    ...
    sliding_window_histogram_[SmoothnessStrategy::kCompositorFocusedStrategy].Clear();
    ring_buffer_.Clear();
...
base::TimeDelta DroppedFrameCounter::ComputeCurrentWindowSize() const
{
    if (sliding_window_.empty())
        return {};
    return sliding_window_.back().first.frame_time + sliding_window_.back().first.interval - sliding_window_.front().first.frame_time;
}

void DroppedFrameCounter::NotifyFrameResult(const viz::BeginFrameArgs& args, const FrameInfo& frame_info)
{
    ...
    if (!in_dropping_ && is_dropped) {
        TRACE_EVENT_BEGIN("cc,benchmark,latency", "DroppedFrameDuration", perfetto::Track(reinterpret_cast<uint64_t>(this), perfetto::ThreadTrack::Current()),
            args.frame_time);
        in_dropping_ = true;
    } else if (in_dropping_ && !is_dropped) {
        TRACE_EVENT_END("cc,benchmark,latency" /* "DroppedFrameDuration" */,
            perfetto::Track(reinterpret_cast<uint64_t>(this), perfetto::ThreadTrack::Current()), args.frame_time);
        in_dropping_ = false;
    }
    ...
    DCHECK_GE(dropped_frame_count_in_window_[SmoothnessStrategy::kDefaultStrategy], 0u);
    DCHECK_GE(sliding_window_.size(), dropped_frame_count_in_window_[SmoothnessStrategy::kDefaultStrategy]);
...
void DroppedFrameCounter::PopSlidingWindow()
{
    ...
    const auto newest_was_dropped = sliding_window_.back().second.IsDroppedAffectingSmoothness();

    uint32_t invalidated_frames = 0;
    if (ComputeCurrentWindowSize() > kDefaultSlidingWindowInterval && newest_was_dropped) {
        invalidated_frames++;
    }
    ...
    const auto max_sliding_window_start = newest_args.frame_time - kDefaultSlidingWindowInterval;
    const auto max_difference = newest_args.interval * 1.5;
    const auto& remaining_oldest_args = sliding_window_.front().first;
    const auto last_timestamp = std::min(remaining_oldest_args.frame_time, max_sliding_window_start);
    const auto difference = last_timestamp - removed_args.frame_time;
    const size_t count = difference > max_difference ? std::ceil(difference / newest_args.interval) : 1;

    uint32_t dropped = dropped_frame_count_in_window_[SmoothnessStrategy::kDefaultStrategy] - invalidated_frames;
    const double percent_dropped_frame = std::min((dropped * 100.0) / total_frames_in_window_, 100.0);
    sliding_window_histogram_[SmoothnessStrategy::kDefaultStrategy].AddPercentDroppedFrame(percent_dropped_frame, count);

    uint32_t dropped_compositor = dropped_frame_count_in_window_[SmoothnessStrategy::kCompositorFocusedStrategy] - invalidated_frames;
    double percent_dropped_frame_compositor = std::min((dropped_compositor * 100.0) / total_frames_in_window_, 100.0);
    sliding_window_histogram_[SmoothnessStrategy::kCompositorFocusedStrategy].AddPercentDroppedFrame(percent_dropped_frame_compositor, count);

    uint32_t dropped_main = dropped_frame_count_in_window_[SmoothnessStrategy::kMainFocusedStrategy] - invalidated_frames;
    double percent_dropped_frame_main = std::min((dropped_main * 100.0) / total_frames_in_window_, 100.0);
    sliding_window_histogram_[SmoothnessStrategy::kMainFocusedStrategy].AddPercentDroppedFrame(percent_dropped_frame_main, count);

    uint32_t dropped_scroll = dropped_frame_count_in_window_[SmoothnessStrategy::kScrollFocusedStrategy] - invalidated_frames;
    double percent_dropped_frame_scroll = std::min((dropped_scroll * 100.0) / total_frames_in_window_, 100.0);
    sliding_window_histogram_[SmoothnessStrategy::kScrollFocusedStrategy].AddPercentDroppedFrame(percent_dropped_frame_scroll, count);
...
void DroppedFrameCounter::UpdateDroppedFrameCountInWindow(const FrameInfo& frame_info, int count)
{
    if (frame_info.IsDroppedAffectingSmoothness()) {
        DCHECK_GE(dropped_frame_count_in_window_[SmoothnessStrategy::kDefaultStrategy] + count, 0u);
        dropped_frame_count_in_window_[SmoothnessStrategy::kDefaultStrategy] += count;
    }
    if (frame_info.WasSmoothCompositorUpdateDropped()) {
        DCHECK_GE(dropped_frame_count_in_window_[SmoothnessStrategy::kCompositorFocusedStrategy] + count, 0u);
        dropped_frame_count_in_window_[SmoothnessStrategy::kCompositorFocusedStrategy] += count;
    }
    if (frame_info.WasSmoothMainUpdateDropped()) {
        DCHECK_GE(dropped_frame_count_in_window_[SmoothnessStrategy::kMainFocusedStrategy] + count, 0u);
        dropped_frame_count_in_window_[SmoothnessStrategy::kMainFocusedStrategy] += count;
    }
    if (frame_info.IsScrollPrioritizeFrameDropped()) {
        DCHECK_GE(dropped_frame_count_in_window_[SmoothnessStrategy::kScrollFocusedStrategy] + count, 0u);
        dropped_frame_count_in_window_[SmoothnessStrategy::kScrollFocusedStrategy] += count;
    }
}
...
void DroppedFrameCounter::UpdateMaxPercentDroppedFrame(double percent_dropped_frame)
{
    ...
    if (fcp_time_delta > base::Seconds(1))
        sliding_window_max_percent_dropped_After_1_sec_ = std::max(sliding_window_max_percent_dropped_After_1_sec_.value_or(0.0), percent_dropped_frame);
    if (fcp_time_delta > base::Seconds(2))
        sliding_window_max_percent_dropped_After_2_sec_ = std::max(sliding_window_max_percent_dropped_After_2_sec_.value_or(0.0), percent_dropped_frame);
    if (fcp_time_delta > base::Seconds(5))
        sliding_window_max_percent_dropped_After_5_sec_ = std::max(sliding_window_max_percent_dropped_After_5_sec_.value_or(0.0), percent_dropped_frame);
}
