...
constexpr base::TimeDelta kDrawIntervalTraceThreshold = base::Microseconds(34478);
...
#define UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED(name, sample)                                                                                                 \
    do {                                                                                                                                                       \
        UMA_HISTOGRAM_CUSTOM_ENUMERATION(                                                                                                                      \
            name "2", sample.InMicroseconds(), std::vector<int>(kUMAVSyncBuckets, kUMAVSyncBuckets + std::size(kUMAVSyncBuckets)));                            \
    } while (false)

} // namespace

CompositorTimingHistory::CompositorTimingHistory(UMACategory uma_category, RenderingStatsInstrumentation* rendering_stats_instrumentation)
    : enabled_(false)
    ...
    , begin_main_frame_queue_duration_not_critical_history_(kDurationHistorySize)
    , begin_main_frame_start_to_ready_to_commit_duration_history_(kDurationHistorySize)
...
base::TimeDelta CompositorTimingHistory::BeginMainFrameQueueDurationCriticalEstimate() const
{
...
base::TimeDelta CompositorTimingHistory::BeginMainFrameQueueDurationNotCriticalEstimate() const
{
    base::TimeDelta all = begin_main_frame_queue_duration_history_.Percentile(kBeginMainFrameQueueDurationEstimationPercentile);
    base::TimeDelta not_critical
        = begin_main_frame_queue_duration_not_critical_history_.Percentile(kBeginMainFrameQueueDurationNotCriticalEstimationPercentile);
    // Return the max since, non critical BeginMainFrames are likely slow if
    // the critical ones are.
    return std::max(not_critical, all);
}

base::TimeDelta CompositorTimingHistory::BeginMainFrameStartToReadyToCommitDurationEstimate() const
{
    return begin_main_frame_start_to_ready_to_commit_duration_history_.Percentile(kBeginMainFrameStartToReadyToCommitEstimationPercentile);
}
...
base::TimeDelta CompositorTimingHistory::CommitToReadyToActivateDurationEstimate() const
{
    return commit_to_ready_to_activate_duration_history_.Percentile(kCommitToReadyToActivateEstimationPercentile);
}
...
base::TimeDelta CompositorTimingHistory::BeginMainFrameStartToReadyToCommitCriticalEstimate() const
{
    return BeginMainFrameStartToReadyToCommitDurationEstimate() + BeginMainFrameQueueDurationCriticalEstimate();
}

base::TimeDelta CompositorTimingHistory::BeginMainFrameStartToReadyToCommitNotCriticalEstimate() const
{
    return BeginMainFrameStartToReadyToCommitDurationEstimate() + BeginMainFrameQueueDurationNotCriticalEstimate();
}

base::TimeDelta CompositorTimingHistory::BeginMainFrameQueueToActivateCriticalEstimate() const
{
    return BeginMainFrameStartToReadyToCommitDurationEstimate() + CommitDurationEstimate() + CommitToReadyToActivateDurationEstimate()
        + ActivateDurationEstimate() + BeginMainFrameQueueDurationCriticalEstimate();
}
...
void CompositorTimingHistory::WillBeginMainFrame(const viz::BeginFrameArgs& args)
{
...
void CompositorTimingHistory::BeginMainFrameStarted(base::TimeTicks main_thread_start_time)
{
...
void CompositorTimingHistory::NotifyReadyToCommit()
{
    ...
    base::TimeDelta bmf_duration = ready_to_commit_time_ - begin_main_frame_start_time_;
    DidBeginMainFrame(ready_to_commit_time_);
    begin_main_frame_start_to_ready_to_commit_duration_history_.InsertSample(bmf_duration);
}
...
void CompositorTimingHistory::DidBeginMainFrame(base::TimeTicks begin_main_frame_end_time)
{
    DCHECK_NE(base::TimeTicks(), begin_main_frame_sent_time_);
    ...
    base::TimeDelta bmf_sent_to_commit_duration = begin_main_frame_end_time - begin_main_frame_sent_time_;
    base::TimeDelta bmf_queue_duration = begin_main_frame_start_time_ - begin_main_frame_sent_time_;

    rendering_stats_instrumentation_->AddBeginMainFrameToCommitDuration(bmf_sent_to_commit_duration);

    if (enabled_) {
        begin_main_frame_queue_duration_history_.InsertSample(bmf_queue_duration);
        if (begin_main_frame_on_critical_path_) {
            begin_main_frame_queue_duration_critical_history_.InsertSample(bmf_queue_duration);
        } else {
            begin_main_frame_queue_duration_not_critical_history_.InsertSample(bmf_queue_duration);
        }
    }
...
void CompositorTimingHistory::ReadyToActivate()
{
    ...
    if (!pending_tree_is_impl_side_) {
        base::TimeDelta time_since_commit = pending_tree_ready_to_activate_time_ - pending_tree_creation_time_;
        ...
        rendering_stats_instrumentation_->AddCommitToActivateDuration(time_since_commit, commit_to_ready_to_activate_estimate);

        if (enabled_) {
            commit_to_ready_to_activate_duration_history_.InsertSample(time_since_commit);
        }
    }
}
...
void CompositorTimingHistory::DidDraw()
{
    ...
    rendering_stats_instrumentation_->AddDrawDuration(draw_duration, draw_estimate);
    ...
    if (!draw_end_time_prev_.is_null()) {
        ...
        if (uma_category_ == RENDERER_UMA) {
            UMA_HISTOGRAM_CUSTOM_TIMES_VSYNC_ALIGNED("Scheduling.Renderer.DrawInterval", draw_interval);
        }
        ...
        if (draw_interval > kDrawIntervalTraceThreshold) {
            TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP0(
                "latency", "Long Draw Interval", TRACE_ID_WITH_SCOPE("Long Draw Interval", g_num_long_draw_intervals), draw_start_time_);
            TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP0(
                "latency", "Long Draw Interval", TRACE_ID_WITH_SCOPE("Long Draw Interval", g_num_long_draw_intervals), draw_end_time);
