...
EventsMetricsManager::ScopedMonitor::DoneCallback CreateSimpleDoneCallback(std::unique_ptr<EventMetrics> metrics)
{
...
protected:
    std::unique_ptr<EventMetrics> CreateEventMetrics(ui::EventType type)
    {
        ...
        base::TimeTicks arrived_in_browser_main_timestamp = test_tick_clock_.NowTicks();
        test_tick_clock_.Advance(base::Microseconds(10));
        return EventMetrics::CreateForTesting(type, event_time, arrived_in_browser_main_timestamp, &test_tick_clock_, std::nullopt);
    }
...
TEST_F(EventsMetricsManagerTest, EventsMetricsSaved)
{
    ...
    std::pair<std::unique_ptr<EventMetrics>, Behavior> events[] = {
        ...
        { CreateEventMetrics(ui::EventType::kMousePressed), Behavior::kSaveInsideScope },

        // An interesting event type for which SaveActiveEventMetrics() is called
        // after its monitor scope is finished.
        { CreateEventMetrics(ui::EventType::kMousePressed), Behavior::kSaveOutsideScope },

        // A non-interesting event type for which SaveActiveEventMetrics() is
        // called inside its monitor scope.
        { CreateEventMetrics(ui::EventType::kMouseEntered), Behavior::kSaveInsideScope },
    };
    ...
    for (auto& event : events) {
        {
            auto monitor = manager_.GetScopedMonitor(CreateSimpleDoneCallback(std::move(event.first)));
    ...
    EXPECT_THAT(manager_.TakeSavedEventsMetrics(), UnorderedPointwise(UniquePtrMatches(), expected_saved_events));
...
TEST_F(EventsMetricsManagerTest, NestedEventsMetrics)
{
    ...
    for (size_t i = 0; i < std::size(configs); i++) {
        ...
        { // Start outer scope.
            ...
            auto outer_monitor = manager_.GetScopedMonitor(CreateSimpleDoneCallback(std::move(outer_metrics)));
            ...
            { // Start inner scope.
                ...
                auto inner_monitor = manager_.GetScopedMonitor(CreateSimpleDoneCallback(std::move(inner_metrics)));
        ...
        EXPECT_THAT(manager_.TakeSavedEventsMetrics(), UnorderedPointwise(UniquePtrMatches(), expected_saved_metrics));
    }
}
