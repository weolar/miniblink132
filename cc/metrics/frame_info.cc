...
{
    return thread == FrameInfo::SmoothThread::kSmoothCompositor || thread == FrameInfo::SmoothThread::kSmoothBoth;
}

bool IsMainSmooth(FrameInfo::SmoothThread thread)
{
    return thread == FrameInfo::SmoothThread::kSmoothMain || thread == FrameInfo::SmoothThread::kSmoothBoth;
}
...
void FrameInfo::MergeWith(const FrameInfo& other)
{
    ...
    if (was_merged)
        return;
    if (main_thread_response == MainThreadResponse::kIncluded && other.main_thread_response == MainThreadResponse::kIncluded) {
        return;
    }
#endif
    DCHECK(!was_merged);
    ...
    if (main_thread_response == MainThreadResponse::kIncluded) {
        ...
        bool compositor_only_change_included_new_main
            = other.final_state == FrameFinalState::kPresentedAll || other.final_state == FrameFinalState::kPresentedPartialNewMain;
        main_update_was_dropped = final_state == FrameFinalState::kDropped || !compositor_only_change_included_new_main;

        compositor_update_was_dropped = other.final_state == FrameFinalState::kDropped;
        raster_property_was_dropped = other.final_state_raster_property == FrameFinalState::kDropped;
        ...
        raster_property_was_dropped = final_state_raster_property == FrameFinalState::kDropped;
    ...
    const bool is_compositor_smooth = IsCompositorSmooth(smooth_thread) || IsCompositorSmooth(other.smooth_thread);
    const bool is_main_smooth = IsMainSmooth(smooth_thread) || IsMainSmooth(other.smooth_thread);
...
FrameInfo::FrameFinalState FrameInfo::GetFinalStateForThread(SmoothEffectDrivingThread thread) const
{
    if (!was_merged) {
        return final_state;
    }
...
base::TimeTicks FrameInfo::GetTerminationTimeForThread(SmoothEffectDrivingThread thread) const
{
    if (!was_merged) {
