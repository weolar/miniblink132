...
std::pair<float, bool> GetJankyThresholdAndScrollSpeed(float d1, float d2, float d3)
{
...
void PredictorJankTracker::ReportLatestScrollDelta(
    float next_delta, base::TimeTicks next_presentation_ts, base::TimeDelta vsync_interval, std::optional<EventMetrics::TraceId> trace_id)
{
    total_frames_++;
    ...
    // Maximum displacement in a sequence of 3 frames is used to decide
    // the janky threshold at which the user will start noticing jank.
    const auto [janky_threshold, slow_scroll] = GetJankyThresholdAndScrollSpeed(d1, d2, d3);
    ...
    bool contains_missed_vsyncs = ContainsMissedVSync(next_presentation_ts, vsync_interval);

    bool report_ukm = false;
    if (frame_janky_lower >= janky_threshold) {
        ReportJankyFrame(next_delta, frame_janky_lower - janky_threshold, contains_missed_vsyncs, slow_scroll, trace_id);
        report_ukm = true;
    }
    if (frame_janky_upper >= janky_threshold) {
        ReportJankyFrame(next_delta, frame_janky_upper - janky_threshold, contains_missed_vsyncs, slow_scroll, trace_id);
        report_ukm = true;
    }
...
void PredictorJankTracker::ReportJankyFrame(
    float next_delta, float janky_value, bool contains_missed_vsyncs, bool slow_scroll, std::optional<EventMetrics::TraceId> trace_id)
{
    janky_frames_++;
    if (scroll_jank_ukm_reporter_) {
        scroll_jank_ukm_reporter_->IncrementPredictorJankyFrames();
    }
    TRACE_EVENT_INSTANT("input.scrolling", "PredictorJankTracker::ReportJankyFrame", [&](perfetto::EventContext ctx) {
    ...
    if (contains_missed_vsyncs && slow_scroll) {
        UMA_HISTOGRAM_CUSTOM_COUNTS("Event.Jank.ScrollUpdate.SlowScroll.MissedVsync."
                                    "FrameAboveJankyThreshold2",
            janky_value_percentage, 1, 1500, 50);
    } else if (contains_missed_vsyncs && !slow_scroll) {
        UMA_HISTOGRAM_CUSTOM_COUNTS("Event.Jank.ScrollUpdate.FastScroll.MissedVsync."
                                    "FrameAboveJankyThreshold2",
            janky_value_percentage, 1, 1500, 50);
    } else if (slow_scroll) {
        UMA_HISTOGRAM_CUSTOM_COUNTS("Event.Jank.ScrollUpdate.SlowScroll.NoMissedVsync."
                                    "FrameAboveJankyThreshold2",
            janky_value_percentage, 1, 1500, 50);
    } else {
        UMA_HISTOGRAM_CUSTOM_COUNTS("Event.Jank.ScrollUpdate.FastScroll.NoMissedVsync."
                                    "FrameAboveJankyThreshold2",
    ...
    if (scroll_jank_ukm_reporter_) {
        if (contains_missed_vsyncs) {
            scroll_jank_ukm_reporter_->set_frame_with_missed_vsync(janky_value_percentage);
        } else {
            scroll_jank_ukm_reporter_->set_frame_with_no_missed_vsync(janky_value_percentage);
        }
    }
}

bool PredictorJankTracker::ContainsMissedVSync(base::TimeTicks& next_presentation_ts, base::TimeDelta& vsync_interval)
{
    ...
    return (next_presentation_ts - frame_data_.cur_presentation_ts_ > vsync_error_margin)
        || (frame_data_.cur_presentation_ts_ - frame_data_.prev_presentation_ts_ > vsync_error_margin);
}

void PredictorJankTracker::StoreLatestFrameData(float delta, base::TimeTicks presentation_ts, std::optional<EventMetrics::TraceId> trace_id)
{
...
{
    UMA_HISTOGRAM_PERCENTAGE("Event.Jank.PredictorJankyFramePercentage2", static_cast<int>(100 * (janky_frames_ / total_frames_)));
    total_frames_ = 0;
    janky_frames_ = 0;
}
