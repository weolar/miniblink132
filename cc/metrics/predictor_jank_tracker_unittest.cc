...
public:
    ...
    {
        predictor_jank_tracker_->ReportLatestScrollDelta(delta, presentation_ts, vsync_interval, EventMetrics::TraceId(++trace_id_));
    }
    ...
    const char* missed_fast_histogram_name = "Event.Jank.ScrollUpdate.FastScroll.MissedVsync."
                                             "FrameAboveJankyThreshold2";
    const char* fast_histogram_name = "Event.Jank.ScrollUpdate.FastScroll.NoMissedVsync."
                                      "FrameAboveJankyThreshold2";
    const char* missed_slow_histogram_name = "Event.Jank.ScrollUpdate.SlowScroll.MissedVsync."
                                             "FrameAboveJankyThreshold2";
    const char* slow_histogram_name = "Event.Jank.ScrollUpdate.SlowScroll.NoMissedVsync."
                                      "FrameAboveJankyThreshold2";
    const char* janky_percentage_name = "Event.Jank.PredictorJankyFramePercentage2";
...
TEST_F(PredictorJankTrackerTest, BasicNonMissedUpperJankCaseWithTracing)
{
    ...
    EXPECT_THAT(result.value(),
        ::testing::ElementsAre(std::vector<std::string> { "prev_delta", "cur_delta", "next_delta" }, std::vector<std::string> { "10", "50", "11" }));
}
...
TEST_F(PredictorJankTrackerTest, VerifySqlThresholds)
{
    ...
    EXPECT_EQ(static_cast<float>(slow_scroll_delta_threshold), PredictorJankTracker::GetSlowScrollDeltaThreshold());

    double slow_scroll_janky_threshold;
    base::StringToDouble(thresholds[1], &slow_scroll_janky_threshold);
    EXPECT_EQ(static_cast<float>(slow_scroll_janky_threshold), PredictorJankTracker::GetSlowScrollJankyThreshold());

    double fast_scroll_janky_threshold;
    base::StringToDouble(thresholds[2], &fast_scroll_janky_threshold);
    EXPECT_EQ(static_cast<float>(fast_scroll_janky_threshold), PredictorJankTracker::GetFastScrollJankyThreshold());
}
...
TEST_F(PredictorJankTrackerTest, VerifySqlPredictorJank)
{
    ...
    ASSERT_TRUE(result.has_value()) << result.error();
    EXPECT_THAT(result.value(),
        ::testing::ElementsAre(std::vector<std::string> { "prev_delta", "cur_delta", "next_delta", "janky_value_pixels", "sql_janky_value_pixels" },
            std::vector<std::string> { "10", "50", "11", "3.34545", "3.34545" }, std::vector<std::string> { "50", "11", "55", "3.34545", "3.34545" }));
}
