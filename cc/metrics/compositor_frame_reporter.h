...
struct GlobalMetricsTrackers {
    ...
    RAW_PTR_EXCLUSION FrameSequenceTrackerCollection* frame_sequence_trackers = nullptr;
    ...
    RAW_PTR_EXCLUSION ScrollJankDroppedFrameTracker* scroll_jank_dropped_frame_tracker = nullptr;
...
public:
    ...
    struct CC_EXPORT StageData {
        ...
        StageData();
        StageData(StageType stage_type, base::TimeTicks start_time, base::TimeTicks end_time);
    ...
    struct CC_EXPORT EventLatencyInfo {
        ...
        EventLatencyInfo(const int num_dispatch_stages, const int num_compositor_stages);
    ...
    public:
        ...
        ProcessedBlinkBreakdown(base::TimeTicks blink_start_time, base::TimeTicks begin_main_frame_start, const BeginMainFrameMetrics& blink_breakdown);
    ...
    public:
        class Iterator {
        public:
            Iterator(const ProcessedVizBreakdown* owner, bool skip_swap_start_to_swap_end);
            ~Iterator();
        ...
        ProcessedVizBreakdown(base::TimeTicks viz_start_time, const viz::FrameTimingDetails& viz_breakdown);
        ...
        Iterator CreateIterator(bool skip_swap_start_to_swap_end_if_breakdown_available) const;

        base::TimeTicks swap_start() const
        {
            return swap_start_;
        }

    private:
        std::optional<std::pair<base::TimeTicks, base::TimeTicks>> list_[static_cast<int>(VizBreakdown::kBreakdownCount)];
    ...
    CompositorFrameReporter(const ActiveTrackers& active_trackers, const viz::BeginFrameArgs& args, bool should_report_histograms, SmoothThread smooth_thread,
        FrameInfo::SmoothEffectDrivingThread scrolling_thread, int layer_tree_host_id, const GlobalMetricsTrackers& trackers);
    ~CompositorFrameReporter();

    CompositorFrameReporter(const CompositorFrameReporter& reporter) = delete;
    CompositorFrameReporter& operator=(const CompositorFrameReporter& reporter) = delete;

    // Name for `CompositorFrameReporter::StageType`, possibly suffixed with the
    // name of the appropriate breakdown.
    static const char* GetStageName(StageType stage_type, std::optional<VizBreakdown> viz_breakdown = std::nullopt,
        std::optional<BlinkBreakdown> blink_breakdown = std::nullopt, bool impl_only = false);
    ...
    void TerminateFrame(FrameTerminationStatus termination_status, base::TimeTicks termination_time);
    void SetBlinkBreakdown(std::unique_ptr<BeginMainFrameMetrics> blink_breakdown, base::TimeTicks begin_main_start);
    ...
    bool did_finish_impl_frame() const
    {
        return did_finish_impl_frame_;
    }
    ...
    FrameSkippedReason frame_skip_reason() const
    {
        return *frame_skip_reason_;
    }
    ...
    void set_is_accompanied_by_main_thread_update(bool is_accompanied_by_main_thread_update)
    {
        is_accompanied_by_main_thread_update_ = is_accompanied_by_main_thread_update;
    }
    ...
    void set_created_new_tree(bool new_tree)
    {
        created_new_tree_ = new_tree;
    }
    void set_want_new_tree(bool want_new_tree)
    {
        want_new_tree_ = want_new_tree;
    }

    const viz::BeginFrameId& frame_id() const
    {
        return args_.frame_id;
    }
    ...
    using FrameReportTypes = std::bitset<static_cast<size_t>(FrameReportType::kMaxValue) + 1>;
    ...
    void CalculateCompositorLatencyPrediction(CompositorLatencyInfo& previous_predictions, base::TimeDelta prediction_deviation_threshold);

    // Sets EventLatency stage duration predictions based on previous trace
    // durations using exponentially weighted averages.
    void CalculateEventLatencyPrediction(CompositorFrameReporter::EventLatencyInfo& predicted_event_latency, base::TimeDelta prediction_deviation_threshold);

    ReporterType get_reporter_type()
    {
        return reporter_type_;
    }

    void set_reporter_type_to_impl()
    {
        reporter_type_ = ReporterType::kImpl;
    }
    void set_reporter_type_to_main()
    {
        reporter_type_ = ReporterType::kMain;
    }
...
private:
    ...
    void ReportStageHistogramWithBreakdown(
        const StageData& stage, FrameSequenceTrackerType frame_sequence_tracker_type = FrameSequenceTrackerType::kMaxType) const;
    void ReportCompositorLatencyBlinkBreakdowns(FrameSequenceTrackerType frame_sequence_tracker_type) const;
    void ReportCompositorLatencyVizBreakdowns(FrameSequenceTrackerType frame_sequence_tracker_type) const;
    void ReportCompositorLatencyHistogram(FrameSequenceTrackerType intraction_type, StageType stage_type, std::optional<VizBreakdown> viz_breakdown,
        std::optional<BlinkBreakdown> blink_breakdown, base::TimeDelta time_delta) const;
    ...
    void FindHighLatencyAttribution(CompositorLatencyInfo& previous_predictions, CompositorLatencyInfo& current_stage_durations);

    void FindEventLatencyAttribution(EventMetrics* event_metrics, CompositorFrameReporter::EventLatencyInfo& predicted_event_latency,
    ...
    FrameTerminationStatus frame_termination_status_ = FrameTerminationStatus::kUnknown;
    ...
    raw_ptr<const base::TickClock> tick_clock_ = base::DefaultTickClock::GetInstance();
    ...
    std::queue<std::unique_ptr<CompositorFrameReporter>> owned_partial_update_dependents_;
