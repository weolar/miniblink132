...
constexpr int kFrameReportTypeCount = static_cast<int>(FrameReportType::kMaxValue) + 1;
constexpr int kStageTypeCount = static_cast<int>(StageType::kStageTypeCount);
constexpr int kAllBreakdownCount = static_cast<int>(VizBreakdown::kBreakdownCount) + static_cast<int>(BlinkBreakdown::kBreakdownCount);

constexpr int kVizBreakdownInitialIndex = kStageTypeCount;
constexpr int kBlinkBreakdownInitialIndex = kVizBreakdownInitialIndex + static_cast<int>(VizBreakdown::kBreakdownCount);

// For each possible FrameSequenceTrackerType there will be a UMA histogram
// plus one for general case.
constexpr int kFrameSequenceTrackerTypeCount = static_cast<int>(FrameSequenceTrackerType::kMaxType) + 1;
...
constexpr const char* kReportTypeNames[] { "", "MissedDeadlineFrame.", "DroppedFrame.", "CompositorOnlyFrame." };

static_assert(std::size(kReportTypeNames) == kFrameReportTypeCount, "Compositor latency report types has changed.");
...
constexpr int kMaxCompositorLatencyHistogramIndex = kFrameReportTypeCount * (kFrameSequenceTrackerTypeCount + kStagesWithBreakdownCount);

constexpr base::TimeDelta kCompositorLatencyHistogramMin = base::Microseconds(1);
constexpr base::TimeDelta kCompositorLatencyHistogramMax = base::Milliseconds(350);
constexpr int kCompositorLatencyHistogramBucketCount = 50;

constexpr const char kEventLatencyBaseHistogramName[] = "EventLatency";
constexpr int kEventLatencyEventTypeCount = static_cast<int>(EventMetrics::EventType::kMaxValue) + 1;
...
constexpr int kEventLatencyScrollTypeCount = static_cast<int>(ScrollEventMetrics::ScrollType::kMaxValue) + 1;
constexpr int kEventLatencyPinchTypeCount = static_cast<int>(PinchEventMetrics::PinchType::kMaxValue) + 1;
constexpr int kEventLatencyGestureTypeCount = std::max(kEventLatencyScrollTypeCount, kEventLatencyPinchTypeCount) + 1;

constexpr int kMaxEventLatencyHistogramIndex = kEventLatencyEventTypeCount * kEventLatencyGestureTypeCount;
...
constexpr int kNumOfCompositorStages = static_cast<int>(StageType::kStageTypeCount) - 1;
// Number of breakdown stages of the blink
constexpr int kNumOfBlinkStages = static_cast<int>(BlinkBreakdown::kBreakdownCount);
...
constexpr int kNumDispatchStages = static_cast<int>(EventMetrics::DispatchStage::kMaxValue);
...
std::string GetCompositorLatencyHistogramName(FrameReportType report_type, FrameSequenceTrackerType frame_sequence_tracker_type, StageType stage_type,
    std::optional<VizBreakdown> viz_breakdown, std::optional<BlinkBreakdown> blink_breakdown)
{
    DCHECK_LE(frame_sequence_tracker_type, FrameSequenceTrackerType::kMaxType);
    const char* tracker_type_name = FrameSequenceTracker::GetFrameSequenceTrackerTypeName(frame_sequence_tracker_type);
    DCHECK(tracker_type_name);
    bool impl_only_frame = report_type == FrameReportType::kCompositorOnlyFrame;
    return base::StrCat({ "CompositorLatency.", kReportTypeNames[static_cast<int>(report_type)], tracker_type_name, *tracker_type_name ? "." : "",
        CompositorFrameReporter::GetStageName(stage_type, viz_breakdown, blink_breakdown, impl_only_frame) });
}
...
void ReportEventLatencyMetric(
    const std::string& name, int index, base::TimeDelta latency, const std::optional<EventMetrics::HistogramBucketing>& bucketing, bool guiding_metric = false)
{
    // Various scrolling metrics have been updated to V2 bucketing
    if (bucketing) {
        std::string versioned_name = name + bucketing->version_suffix;
        STATIC_HISTOGRAM_POINTER_GROUP(versioned_name, index, kMaxEventLatencyHistogramIndex, AddTimeMicrosecondsGranularity(latency),
            base::Histogram::FactoryMicrosecondsTimeGet(
                versioned_name, bucketing->min, bucketing->max, bucketing->count, base::HistogramBase::kUmaTargetedHistogramFlag));
    }
    ...
    if (!bucketing || guiding_metric) {
        STATIC_HISTOGRAM_POINTER_GROUP(name, index, kMaxEventLatencyHistogramIndex, AddTimeMicrosecondsGranularity(latency),
            base::Histogram::FactoryMicrosecondsTimeGet(
                name, kEventLatencyHistogramMin, kEventLatencyHistogramMax, kEventLatencyHistogramBucketCount, base::HistogramBase::kUmaTargetedHistogramFlag));
    }
}

constexpr char kTraceCategory[] = "cc,benchmark," TRACE_DISABLED_BY_DEFAULT("devtools.timeline.frame");
...
base::TimeDelta CalculateWeightedAverage(base::TimeDelta previous_value, base::TimeDelta current_value)
{
    if (previous_value.is_negative())
        return current_value;
    return (kWeightOfCurStageInPercent * current_value + (100 - kWeightOfCurStageInPercent) * previous_value) / 100;
}

// Calculate new prediction of latency based on old prediction and current
// latency
base::TimeDelta PredictLatency(base::TimeDelta previous_prediction, base::TimeDelta current_latency)
{
    return (kWeightOfCurStageInPercent * current_latency + (100 - kWeightOfCurStageInPercent) * previous_prediction) / 100;
}

double DetermineHighestContribution(
    double contribution_change, double highest_contribution_change, const std::string& stage_name, std::vector<std::string>& high_latency_stages)
{
...
void TraceScrollJankMetrics(const EventMetrics::List& events_metrics, int32_t fling_input_count, int32_t normal_input_count, perfetto::EventContext& ctx)
{
    *(int*)1 = 1;
...
//         if (type != EventMetrics::EventType::kGestureScrollUpdate && type != EventMetrics::EventType::kFirstGestureScrollUpdate
//             && type != EventMetrics::EventType::kInertialGestureScrollUpdate) [[unlikely]] {
//             continue;
//         }
//         auto* scroll_update_event = event->AsScrollUpdate();
//         if (scroll_update_event->trace_id().has_value()) {
//             scroll_data->add_trace_ids_in_gpu_frame(scroll_update_event->trace_id()->value());
//             scroll_data->add_segregated_original_deltas_in_gpu_frame_y(scroll_update_event->delta());
//             scroll_data->add_segregated_predicted_deltas_in_gpu_frame_y(scroll_update_event->predicted_delta());
//         }
//         delta += scroll_update_event->delta();
//         predicted_delta += scroll_update_event->predicted_delta();
//     }
//     scroll_data->set_event_count_in_gpu_frame(fling_input_count + normal_input_count);
...
constexpr int kMaxVSyncRatioHistogramIndex = kMaxGestureScrollHistogramIndex * static_cast<int>(CompositorFrameReporter::VSyncRatioType::kVSyncRatioTypeCount);
const char* GetVSyncRatioTypeName(CompositorFrameReporter::VSyncRatioType type)
{
    switch (type) {
    case CompositorFrameReporter::VSyncRatioType::kArrivedInRendererVsVSyncRatioAfterVSync:
        return "ArrivedInRendererVsVSyncRatio.AfterVSync";
    case CompositorFrameReporter::VSyncRatioType::kArrivedInRendererVsVSyncRatioBeforeVSync:
        return "ArrivedInRendererVsVSyncRatio.BeforeVSync";
    case CompositorFrameReporter::VSyncRatioType::kGenerationVsVsyncRatioAfterVSync:
        return "GenerationVsVsyncRatio.AfterVSync";
    case CompositorFrameReporter::VSyncRatioType::kGenerationVsVsyncRatioBeforeVSync:
...
void ReportVSyncRatioMetric(const std::string& base_histogram_name, int gesture_scroll_index, CompositorFrameReporter::VSyncRatioType type, int percentage)
{
    const std::string vsync_ratio_type_name = GetVSyncRatioTypeName(type);
    const std::string histogram_name = base::JoinString({ base_histogram_name, vsync_ratio_type_name }, ".");
    STATIC_HISTOGRAM_POINTER_GROUP(histogram_name, gesture_scroll_index + static_cast<int>(type) * kMaxGestureScrollHistogramIndex,
        kMaxVSyncRatioHistogramIndex, Add(percentage),
        base::LinearHistogram::FactoryGet(histogram_name, 1, 100, 101, base::HistogramBase::kUmaTargetedHistogramFlag));
}
...
void ReportTopControlsMetric(const std::string& name, bool top_controls_moved, base::TimeDelta latency, EventMetrics::EventType type,
    const std::optional<EventMetrics::HistogramBucketing>& bucketing)
{
    ...
    if (top_controls_moved) {
        ...
        STATIC_HISTOGRAM_POINTER_GROUP(versioned_name, GetGestureScrollIndex(type), kMaxGestureScrollHistogramIndex, AddTimeMicrosecondsGranularity(latency),
            base::Histogram::FactoryMicrosecondsTimeGet(
                versioned_name, bucketing->min, bucketing->max, bucketing->count, base::HistogramBase::kUmaTargetedHistogramFlag));
...
CompositorFrameReporter::ProcessedBlinkBreakdown::Iterator::Iterator(const ProcessedBlinkBreakdown* owner)
    : owner_(owner)
{
}

CompositorFrameReporter::ProcessedBlinkBreakdown::Iterator::~Iterator() = default;

bool CompositorFrameReporter::ProcessedBlinkBreakdown::Iterator::IsValid() const
{
...
BlinkBreakdown CompositorFrameReporter::ProcessedBlinkBreakdown::Iterator::GetBreakdown() const
{
    DCHECK(IsValid());
    return static_cast<BlinkBreakdown>(index_);
}

base::TimeDelta CompositorFrameReporter::ProcessedBlinkBreakdown::Iterator::GetLatency() const
{
    DCHECK(IsValid());
...
CompositorFrameReporter::ProcessedBlinkBreakdown::ProcessedBlinkBreakdown(
    base::TimeTicks blink_start_time, base::TimeTicks begin_main_frame_start, const BeginMainFrameMetrics& blink_breakdown)
{
    if (blink_start_time.is_null())
        return;

    list_[static_cast<int>(BlinkBreakdown::kHandleInputEvents)] = blink_breakdown.handle_input_events;
    list_[static_cast<int>(BlinkBreakdown::kAnimate)] = blink_breakdown.animate;
    list_[static_cast<int>(BlinkBreakdown::kStyleUpdate)] = blink_breakdown.style_update;
    list_[static_cast<int>(BlinkBreakdown::kLayoutUpdate)] = blink_breakdown.layout_update;
    list_[static_cast<int>(BlinkBreakdown::kAccessibility)] = blink_breakdown.accessibility;
    list_[static_cast<int>(BlinkBreakdown::kPrepaint)] = blink_breakdown.prepaint;
    list_[static_cast<int>(BlinkBreakdown::kCompositingInputs)] = blink_breakdown.compositing_inputs;
    list_[static_cast<int>(BlinkBreakdown::kPaint)] = blink_breakdown.paint;
    list_[static_cast<int>(BlinkBreakdown::kCompositeCommit)] = blink_breakdown.composite_commit;
    list_[static_cast<int>(BlinkBreakdown::kUpdateLayers)] = blink_breakdown.update_layers;
    list_[static_cast<int>(BlinkBreakdown::kBeginMainSentToStarted)] = begin_main_frame_start - blink_start_time;
}

CompositorFrameReporter::ProcessedBlinkBreakdown::~ProcessedBlinkBreakdown() = default;

CompositorFrameReporter::ProcessedBlinkBreakdown::Iterator CompositorFrameReporter::ProcessedBlinkBreakdown::CreateIterator() const
{
...
CompositorFrameReporter::ProcessedVizBreakdown::Iterator::Iterator(const ProcessedVizBreakdown* owner, bool skip_swap_start_to_swap_end)
    : owner_(owner)
    , skip_swap_start_to_swap_end_(skip_swap_start_to_swap_end)
{
    DCHECK(owner_);
...
VizBreakdown CompositorFrameReporter::ProcessedVizBreakdown::Iterator::GetBreakdown() const
{
    DCHECK(HasValue());
    return static_cast<VizBreakdown>(index_);
}

base::TimeTicks CompositorFrameReporter::ProcessedVizBreakdown::Iterator::GetStartTime() const
{
    DCHECK(HasValue());
    return owner_->list_[index_]->first;
}

base::TimeTicks CompositorFrameReporter::ProcessedVizBreakdown::Iterator::GetEndTime() const
{
    DCHECK(HasValue());
    return owner_->list_[index_]->second;
}

base::TimeDelta CompositorFrameReporter::ProcessedVizBreakdown::Iterator::GetDuration() const
{
    DCHECK(HasValue());
    return owner_->list_[index_]->second - owner_->list_[index_]->first;
}

bool CompositorFrameReporter::ProcessedVizBreakdown::Iterator::HasValue() const
{
    DCHECK(IsValid());
    return owner_->list_[index_].has_value();
}

void CompositorFrameReporter::ProcessedVizBreakdown::Iterator::SkipBreakdownsIfNecessary()
{
    while (IsValid() && (!HasValue() || (GetBreakdown() == CompositorFrameReporter::VizBreakdown::kSwapStartToSwapEnd && skip_swap_start_to_swap_end_))) {
        index_++;
    }
}
...
CompositorFrameReporter::ProcessedVizBreakdown::ProcessedVizBreakdown(base::TimeTicks viz_start_time, const viz::FrameTimingDetails& viz_breakdown)
{
    ...
    if (viz_breakdown.received_compositor_frame_timestamp.is_null() || viz_breakdown.received_compositor_frame_timestamp < viz_start_time) {
        return;
    }
    list_[static_cast<int>(VizBreakdown::kSubmitToReceiveCompositorFrame)] = std::make_pair(viz_start_time, viz_breakdown.received_compositor_frame_timestamp);

    if (viz_breakdown.draw_start_timestamp.is_null())
        return;
    list_[static_cast<int>(VizBreakdown::kReceivedCompositorFrameToStartDraw)]
        = std::make_pair(viz_breakdown.received_compositor_frame_timestamp, viz_breakdown.draw_start_timestamp);

    if (viz_breakdown.swap_timings.is_null())
        return;
    list_[static_cast<int>(VizBreakdown::kStartDrawToSwapStart)] = std::make_pair(viz_breakdown.draw_start_timestamp, viz_breakdown.swap_timings.swap_start);

    list_[static_cast<int>(VizBreakdown::kSwapStartToSwapEnd)] = std::make_pair(viz_breakdown.swap_timings.swap_start, viz_breakdown.swap_timings.swap_end);

    list_[static_cast<int>(VizBreakdown::kSwapEndToPresentationCompositorFrame)]
        = std::make_pair(viz_breakdown.swap_timings.swap_end, viz_breakdown.presentation_feedback.timestamp);
    ...
    list_[static_cast<int>(VizBreakdown::kSwapStartToBufferAvailable)]
        = std::make_pair(viz_breakdown.swap_timings.swap_start, viz_breakdown.presentation_feedback.available_timestamp);
    list_[static_cast<int>(VizBreakdown::kBufferAvailableToBufferReady)]
        = std::make_pair(viz_breakdown.presentation_feedback.available_timestamp, viz_breakdown.presentation_feedback.ready_timestamp);
    list_[static_cast<int>(VizBreakdown::kBufferReadyToLatch)]
        = std::make_pair(viz_breakdown.presentation_feedback.ready_timestamp, viz_breakdown.presentation_feedback.latch_timestamp);
    list_[static_cast<int>(VizBreakdown::kLatchToSwapEnd)]
        = std::make_pair(viz_breakdown.presentation_feedback.latch_timestamp, viz_breakdown.swap_timings.swap_end);
}

CompositorFrameReporter::ProcessedVizBreakdown::~ProcessedVizBreakdown() = default;

CompositorFrameReporter::ProcessedVizBreakdown::Iterator CompositorFrameReporter::ProcessedVizBreakdown::CreateIterator(
    bool skip_swap_start_to_swap_end_if_breakdown_available) const
{
    return Iterator(this, skip_swap_start_to_swap_end_if_breakdown_available && buffer_ready_available_);
}

// CompositorFrameReporter::CompositorLatencyInfo ==============================

CompositorFrameReporter::CompositorLatencyInfo::CompositorLatencyInfo() = default;
CompositorFrameReporter::CompositorLatencyInfo::CompositorLatencyInfo(base::TimeDelta init_value)
...
CompositorFrameReporter::CompositorFrameReporter(const ActiveTrackers& active_trackers, const viz::BeginFrameArgs& args, bool should_report_histograms,
    SmoothThread smooth_thread, FrameInfo::SmoothEffectDrivingThread scrolling_thread, int layer_tree_host_id, const GlobalMetricsTrackers& trackers)
    ...
    , global_trackers_(trackers)
{
    ...
    DCHECK(IsScrollActive(active_trackers_) || scrolling_thread_ == FrameInfo::SmoothEffectDrivingThread::kUnknown);
    if (scrolling_thread_ == FrameInfo::SmoothEffectDrivingThread::kCompositor) {
        DCHECK(smooth_thread_ == SmoothThread::kSmoothCompositor || smooth_thread_ == SmoothThread::kSmoothBoth);
    } else if (scrolling_thread_ == FrameInfo::SmoothEffectDrivingThread::kMain) {
        DCHECK(smooth_thread_ == SmoothThread::kSmoothMain || smooth_thread_ == SmoothThread::kSmoothBoth);
    }
    // If we have a SET version of the animation, then we should also have a
    // non-SET version of the same animation.
    DCHECK(!active_trackers_.test(static_cast<size_t>(FrameSequenceTrackerType::kSETCompositorAnimation))
        || active_trackers_.test(static_cast<size_t>(FrameSequenceTrackerType::kCompositorNativeAnimation))
        || active_trackers_.test(static_cast<size_t>(FrameSequenceTrackerType::kCompositorRasterAnimation)));
    DCHECK(!active_trackers_.test(static_cast<size_t>(FrameSequenceTrackerType::kSETMainThreadAnimation))
        || active_trackers_.test(static_cast<size_t>(FrameSequenceTrackerType::kMainThreadAnimation)));
    is_forked_ = false;
    is_backfill_ = false;
}
...
const char* CompositorFrameReporter::GetStageName(
    StageType stage_type, std::optional<VizBreakdown> viz_breakdown, std::optional<BlinkBreakdown> blink_breakdown, bool impl_only)
{
    DCHECK(!viz_breakdown || stage_type == StageType::kSubmitCompositorFrameToPresentationCompositorFrame);
    DCHECK(!blink_breakdown || stage_type == StageType::kSendBeginMainFrameToCommit);
    switch (stage_type) {
    case StageType::kBeginImplFrameToSendBeginMainFrame:
        return impl_only ? "BeginImplFrameToFinishImpl" : "BeginImplFrameToSendBeginMainFrame";
    case StageType::kSendBeginMainFrameToCommit:
        if (!blink_breakdown) {
            return impl_only ? "SendBeginMainFrameToBeginMainAbort" : "SendBeginMainFrameToCommit";
        }
    ...
    case StageType::kEndActivateToSubmitCompositorFrame:
        return impl_only ? "ImplFrameDoneToSubmitCompositorFrame" : "EndActivateToSubmitCompositorFrame";
...
// static
const char* CompositorFrameReporter::GetVizBreakdownName(VizBreakdown breakdown)
{
    switch (breakdown) {
    case VizBreakdown::kSubmitToReceiveCompositorFrame:
...
std::unique_ptr<CompositorFrameReporter> CompositorFrameReporter::CopyReporterAtBeginImplStage()
{
    ...
    if (stage_history_.empty() || stage_history_.front().stage_type != StageType::kBeginImplFrameToSendBeginMainFrame
        || (!did_finish_impl_frame() && !did_not_produce_frame_time_.has_value())) {
        return nullptr;
    }
    auto new_reporter = std::make_unique<CompositorFrameReporter>(
        active_trackers_, args_, should_report_histograms_, smooth_thread_, scrolling_thread_, layer_tree_host_id_, global_trackers_);
    ...
    new_reporter->current_stage_.stage_type = StageType::kBeginImplFrameToSendBeginMainFrame;
...
CompositorFrameReporter::StageData::StageData(StageType stage_type, base::TimeTicks start_time, base::TimeTicks end_time)
...
CompositorFrameReporter::EventLatencyInfo::EventLatencyInfo(const int num_dispatch_stages, const int num_compositor_stages)
...
void CompositorFrameReporter::StartStage(CompositorFrameReporter::StageType stage_type, base::TimeTicks start_time)
{
...
void CompositorFrameReporter::TerminateFrame(FrameTerminationStatus termination_status, base::TimeTicks termination_time)
{
...
void CompositorFrameReporter::OnDidNotProduceFrame(FrameSkippedReason skip_reason)
{
...
void CompositorFrameReporter::SetBlinkBreakdown(std::unique_ptr<BeginMainFrameMetrics> blink_breakdown, base::TimeTicks begin_main_start)
{
...
void CompositorFrameReporter::SetVizBreakdown(const viz::FrameTimingDetails& viz_breakdown)
{
    DCHECK(viz_breakdown_.received_compositor_frame_timestamp.is_null());
    viz_breakdown_ = viz_breakdown;
}

void CompositorFrameReporter::AddEventsMetrics(EventMetrics::List events_metrics)
{
    events_metrics_.insert(events_metrics_.end(), std::make_move_iterator(events_metrics.begin()), std::make_move_iterator(events_metrics.end()));
}
...
{
    auto mid = std::partition(events_metrics_.begin(), events_metrics_.end(), [](std::unique_ptr<EventMetrics>& metrics) {
        DCHECK(metrics);
        bool is_blocked_on_main = metrics->requires_main_thread_update();
        // Invert so we can take from the end.
        return !is_blocked_on_main;
    });
    EventMetrics::List result(std::make_move_iterator(mid), std::make_move_iterator(events_metrics_.end()));
...
void CompositorFrameReporter::TerminateReporter()
{
    ...
    if (!processed_blink_breakdown_)
        processed_blink_breakdown_ = std::make_unique<ProcessedBlinkBreakdown>(blink_start_time_, begin_main_frame_start_, blink_breakdown_);
    if (!processed_viz_breakdown_)
        processed_viz_breakdown_ = std::make_unique<ProcessedVizBreakdown>(viz_start_time_, viz_breakdown_);
    ...
    if (report_types_.any() && (should_report_histograms_ || global_trackers_.latency_ukm_reporter || global_trackers_.event_latency_tracker)) {
        DCHECK(stage_history_.size());
        DCHECK_EQ(SumOfStageHistory(), stage_history_.back().end_time - stage_history_.front().start_time);
        stage_history_.emplace_back(StageType::kTotalLatency, stage_history_.front().start_time, stage_history_.back().end_time);
...
void CompositorFrameReporter::ReportCompositorLatencyMetrics() const
{
    ...
    if (global_trackers_.latency_ukm_reporter) {
        global_trackers_.latency_ukm_reporter->ReportCompositorLatencyUkm(
            report_types_, stage_history_, active_trackers_, *processed_blink_breakdown_, *processed_viz_breakdown_);
    }
        ...
        if (stage.stage_type == StageType::kTotalLatency) {
            for (size_t type = 0; type < active_trackers_.size(); ++type) {
                if (active_trackers_.test(type)) {
                    ...
                    ReportStageHistogramWithBreakdown(stage, static_cast<FrameSequenceTrackerType>(type));
                }
            }
        }
    }
    ...
    for (size_t type = 0; type < report_types_.size(); ++type) {
        ...
        for (size_t fst_type = 0; fst_type < active_trackers_.size(); ++fst_type) {
            ...
            if (!active_trackers_.test(fst_type) || tracker_type == FrameSequenceTrackerType::kCustom || tracker_type == FrameSequenceTrackerType::kMaxType) {
                continue;
            }
            any_active_interaction = true;
            switch (tracker_type) {
            case FrameSequenceTrackerType::kCompositorAnimation:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.CompositorAnimation", report_type);
                break;
            case FrameSequenceTrackerType::kMainThreadAnimation:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.MainThreadAnimation", report_type);
                break;
            case FrameSequenceTrackerType::kPinchZoom:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.PinchZoom", report_type);
                break;
            ...
            case FrameSequenceTrackerType::kTouchScroll:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.TouchScroll", report_type);
                break;
            case FrameSequenceTrackerType::kVideo:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.Video", report_type);
                break;
            case FrameSequenceTrackerType::kWheelScroll:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.WheelScroll", report_type);
                break;
            case FrameSequenceTrackerType::kScrollbarScroll:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.ScrollbarScroll", report_type);
                break;
            case FrameSequenceTrackerType::kCanvasAnimation:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.CanvasAnimation", report_type);
                break;
            case FrameSequenceTrackerType::kJSAnimation:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.JSAnimation", report_type);
                break;
            case FrameSequenceTrackerType::kSETCompositorAnimation:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.SETCompositorAnimation", report_type);
                break;
            case FrameSequenceTrackerType::kSETMainThreadAnimation:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.SETMainThreadAnimation", report_type);
                break;
            case FrameSequenceTrackerType::kCompositorNativeAnimation:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.NativePropertyAnimation", report_type);
                break;
            case FrameSequenceTrackerType::kCompositorRasterAnimation:
                UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.RasterAnimation", report_type);
                break;
        ...
        if (any_active_interaction) {
            UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.AnyInteraction", report_type);
        } else {
            UMA_HISTOGRAM_ENUMERATION("CompositorLatency.Type.NoInteraction", report_type);
        }
    }

    // Only report the IPC and Thread latency when we have valid timestamps.
    if (args_.frame_time.is_null() || args_.dispatch_time.is_null() || args_.client_arrival_time.is_null()) {
        return;
    }
    ...
    if (args_.dispatch_time > args_.frame_time) {
        ...
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("CompositorLatency.IpcThread.FrameTimeToDispatch", vsync_viz_delta, kCompositorLatencyHistogramMin,
            kCompositorLatencyHistogramMax, kCompositorLatencyHistogramBucketCount);
    }
    const base::TimeDelta viz_cc_delta = args_.client_arrival_time - args_.dispatch_time;
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("CompositorLatency.IpcThread.DispatchToRenderer", viz_cc_delta, kCompositorLatencyHistogramMin,
        kCompositorLatencyHistogramMax, kCompositorLatencyHistogramBucketCount);
    ...
    if (begin_main_frame_start_.is_null() || blink_start_time_.is_null()) {
        ...
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("CompositorLatency.IpcThread.ImplThreadTotalLatency", impl_total_latency, kCompositorLatencyHistogramMin,
            kCompositorLatencyHistogramMax, kCompositorLatencyHistogramBucketCount);
        return;
    }
    const base::TimeDelta impl_main_delta = begin_main_frame_start_ - blink_start_time_;
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("CompositorLatency.IpcThread.BeginMainFrameQueuing", impl_main_delta, kCompositorLatencyHistogramMin,
        kCompositorLatencyHistogramMax, kCompositorLatencyHistogramBucketCount);
    const base::TimeDelta main_total_latency = vsync_viz_delta + viz_cc_delta + impl_main_delta;
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("CompositorLatency.IpcThread.MainThreadTotalLatency", main_total_latency, kCompositorLatencyHistogramMin,
        kCompositorLatencyHistogramMax, kCompositorLatencyHistogramBucketCount);
}

void CompositorFrameReporter::ReportStageHistogramWithBreakdown(
    const CompositorFrameReporter::StageData& stage, FrameSequenceTrackerType frame_sequence_tracker_type) const
{
    base::TimeDelta stage_delta = stage.end_time - stage.start_time;
    ReportCompositorLatencyHistogram(frame_sequence_tracker_type, stage.stage_type,
...
void CompositorFrameReporter::ReportCompositorLatencyBlinkBreakdowns(FrameSequenceTrackerType frame_sequence_tracker_type) const
{
    DCHECK(processed_blink_breakdown_);
    for (auto it = processed_blink_breakdown_->CreateIterator(); it.IsValid(); it.Advance()) {
        ReportCompositorLatencyHistogram(frame_sequence_tracker_type, StageType::kSendBeginMainFrameToCommit,
            /*viz_breakdown=*/std::nullopt, it.GetBreakdown(), it.GetLatency());
    }
}

void CompositorFrameReporter::ReportCompositorLatencyVizBreakdowns(FrameSequenceTrackerType frame_sequence_tracker_type) const
{
    DCHECK(processed_viz_breakdown_);
    for (auto it = processed_viz_breakdown_->CreateIterator(false); it.IsValid(); it.Advance()) {
        ReportCompositorLatencyHistogram(frame_sequence_tracker_type, StageType::kSubmitCompositorFrameToPresentationCompositorFrame, it.GetBreakdown(),
            /*blink_breakdown=*/std::nullopt, it.GetDuration());
    }
}

void CompositorFrameReporter::ReportCompositorLatencyHistogram(FrameSequenceTrackerType frame_sequence_tracker_type, StageType stage_type,
    std::optional<VizBreakdown> viz_breakdown, std::optional<BlinkBreakdown> blink_breakdown, base::TimeDelta time_delta) const
{
    DCHECK(!viz_breakdown || stage_type == StageType::kSubmitCompositorFrameToPresentationCompositorFrame);
    DCHECK(!blink_breakdown || stage_type == StageType::kSendBeginMainFrameToCommit);
    for (size_t type = 0; type < report_types_.size(); ++type) {
        ...
        const int frame_sequence_tracker_type_index = static_cast<int>(frame_sequence_tracker_type);
        const int stage_type_index = blink_breakdown ? kBlinkBreakdownInitialIndex + static_cast<int>(*blink_breakdown)
            : viz_breakdown                          ? kVizBreakdownInitialIndex + static_cast<int>(*viz_breakdown)
                                                     : static_cast<int>(stage_type);
        const int histogram_index
            = (stage_type_index == static_cast<int>(StageType::kTotalLatency) ? kStagesWithBreakdownCount + frame_sequence_tracker_type_index
                                                                              : stage_type_index)
        ...
        // `STATIC_HISTOGRAM_POINTER_GROUP()` to cache histogram objects.
        STATIC_HISTOGRAM_POINTER_GROUP(GetCompositorLatencyHistogramName(report_type, frame_sequence_tracker_type, stage_type, viz_breakdown, blink_breakdown),
            histogram_index, kMaxCompositorLatencyHistogramIndex, AddTimeMicrosecondsGranularity(time_delta),
            base::Histogram::FactoryMicrosecondsTimeGet(
                GetCompositorLatencyHistogramName(report_type, frame_sequence_tracker_type, stage_type, viz_breakdown, blink_breakdown),
                kCompositorLatencyHistogramMin, kCompositorLatencyHistogramMax, kCompositorLatencyHistogramBucketCount,
...
void CompositorFrameReporter::ReportEventLatencyMetrics() const
{
    ...
    if (global_trackers_.latency_ukm_reporter) {
        global_trackers_.latency_ukm_reporter->ReportEventLatencyUkm(events_metrics_, stage_history_, *processed_blink_breakdown_, *processed_viz_breakdown_);
    }
    ...
    for (const auto& event_metrics : events_metrics_) {
        ...
        const base::TimeTicks generated_timestamp = event_metrics->GetDispatchStageTimestamp(EventMetrics::DispatchStage::kGenerated);
        ...
        const base::TimeDelta total_latency = total_latency_stage.end_time - generated_timestamp;

        if (should_report_histograms_) {
            const std::string histogram_base_name = base::JoinString({ kEventLatencyBaseHistogramName, event_metrics->GetTypeName() }, ".");
            const int event_histogram_index = static_cast<int>(event_metrics->type());
            const std::string total_latency_stage_name = GetStageName(StageType::kTotalLatency);

            // For pinch events, we only report metrics for each device type and not
            // the aggregate metric over all device types.
            if (!pinch_metrics) {
                const std::string event_total_latency_histogram_name = base::JoinString({ histogram_base_name, total_latency_stage_name }, ".");
                ReportEventLatencyMetric(event_total_latency_histogram_name, event_histogram_index, total_latency, event_metrics->GetHistogramBucketing());
            }
            ...
            if (scroll_metrics || pinch_metrics) {
                const int gesture_type_index
                    = 1 + (scroll_metrics ? static_cast<int>(scroll_metrics->scroll_type()) : static_cast<int>(pinch_metrics->pinch_type()));
                const int gesture_histogram_index = event_histogram_index * kEventLatencyGestureTypeCount + gesture_type_index;
                const std::string gesture_type_name = scroll_metrics ? scroll_metrics->GetScrollTypeName() : pinch_metrics->GetPinchTypeName();

                const std::string gesture_total_latency_histogram_name
                    = base::JoinString({ histogram_base_name, gesture_type_name, total_latency_stage_name }, ".");
                ...
                const bool guiding_metric = scroll_metrics && event_metrics->type() == EventMetrics::EventType::kGestureScrollUpdate
                    && scroll_metrics->scroll_type() == ScrollEventMetrics::ScrollType::kTouchscreen;
                ReportEventLatencyMetric(
                    gesture_total_latency_histogram_name, gesture_histogram_index, total_latency, event_metrics->GetHistogramBucketing(), guiding_metric);
            }

            if (scroll_metrics) {
                auto& original_args = scroll_metrics->begin_frame_args();
                const base::TimeTicks browser_main_timestamp = event_metrics->GetDispatchStageTimestamp(EventMetrics::DispatchStage::kArrivedInBrowserMain);
                const int gesture_scroll_index = GetGestureScrollIndex(scroll_metrics->type());
                if (!browser_main_timestamp.is_null()) {
                    const std::string generation_to_browser_main_name = base::JoinString({ histogram_base_name, kGenerationToBrowserMainName }, ".");
                    const base::TimeDelta browser_main_delay = browser_main_timestamp - generated_timestamp;
                    const std::optional<EventMetrics::HistogramBucketing>& bucketing = event_metrics->GetHistogramBucketing();
                    if (bucketing) {
                        STATIC_HISTOGRAM_POINTER_GROUP(generation_to_browser_main_name, gesture_scroll_index, kMaxGestureScrollHistogramIndex,
                            AddTimeMicrosecondsGranularity(browser_main_delay),
                            base::Histogram::FactoryMicrosecondsTimeGet(generation_to_browser_main_name, bucketing->min, bucketing->max, bucketing->count,
                                base::HistogramBase::kUmaTargetedHistogramFlag));
                    }
                    if (original_args.IsValid()) {
                        const base::TimeDelta generation_to_vsync_delta = original_args.frame_time - generated_timestamp;
                        const double generation_to_vsync_ratio = 100.f * generation_to_vsync_delta / original_args.interval;
                        if (generation_to_vsync_delta.is_negative()) {
                            ReportVSyncRatioMetric(histogram_base_name, gesture_scroll_index,
                                CompositorFrameReporter::VSyncRatioType::kGenerationVsVsyncRatioBeforeVSync, std::ceil(generation_to_vsync_ratio * -1));
                        } else {
                            ReportVSyncRatioMetric(histogram_base_name, gesture_scroll_index,
                                CompositorFrameReporter::VSyncRatioType::kGenerationVsVsyncRatioAfterVSync, std::ceil(generation_to_vsync_ratio));
                        }
                    }

#if BUILDFLAG(IS_ANDROID)
                    ReportTopControlsMetric(
                        histogram_base_name, top_controls_moved_, total_latency, event_metrics->type(), event_metrics->GetHistogramBucketing());
#endif // BUILDFLAG(IS_ANDROID)
                }

                const base::TimeTicks arrived_in_renderer_timestamp
                    = event_metrics->GetDispatchStageTimestamp(EventMetrics::DispatchStage::kArrivedInRendererCompositor);
                if (original_args.IsValid() && !arrived_in_renderer_timestamp.is_null()) {
                    const base::TimeDelta arrived_after_vsync_delta = arrived_in_renderer_timestamp - original_args.frame_time;
                    const double arrived_after_vsync_ratio = 100.f * arrived_after_vsync_delta / original_args.interval;
                    if (arrived_after_vsync_delta.is_negative()) {
                        ReportVSyncRatioMetric(histogram_base_name, gesture_scroll_index,
                            CompositorFrameReporter::VSyncRatioType::kArrivedInRendererVsVSyncRatioBeforeVSync, std::ceil(arrived_after_vsync_ratio * -1));
                    } else {
                        ReportVSyncRatioMetric(histogram_base_name, gesture_scroll_index,
                            CompositorFrameReporter::VSyncRatioType::kArrivedInRendererVsVSyncRatioAfterVSync, std::ceil(arrived_after_vsync_ratio));
                    }
                }
            }
            ...
            const std::string aggregate_total_latency_histogram_name = base::JoinString({ kEventLatencyBaseHistogramName, total_latency_stage_name }, ".");
            UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
                aggregate_total_latency_histogram_name, total_latency, kEventLatencyHistogramMin, kEventLatencyHistogramMax, kEventLatencyHistogramBucketCount);
        }

        if (global_trackers_.event_latency_tracker) {
            EventLatencyTracker::LatencyData& latency_data = latencies.emplace_back(event_metrics->type(), total_latency);
    ...
    if (!latencies.empty()) {
        ...
        global_trackers_.event_latency_tracker->ReportEventLatency(std::move(latencies));
    }
}
...
//     }
// 
//     const auto trace_track = perfetto::Track(base::trace_event::GetNextGlobalTraceId());
//     TRACE_EVENT_BEGIN(kTraceCategory, "PipelineReporter", trace_track, args_.frame_time, [&](perfetto::EventContext context) {
...
//         reporter->set_has_missing_content(info.checkerboarded_needs_raster || info.checkerboarded_needs_record);
//         reporter->set_checkerboarded_needs_raster(info.checkerboarded_needs_raster);
//         reporter->set_checkerboarded_needs_record(info.checkerboarded_needs_record);
//         if (info.IsDroppedAffectingSmoothness()) {
//             DCHECK(state == ChromeFrameReporter::STATE_DROPPED || state == ChromeFrameReporter::STATE_PRESENTED_PARTIAL);
//         }
...
//         reporter->set_has_main_animation(HasMainThreadAnimation(active_trackers_));
//         reporter->set_has_compositor_animation(HasCompositorThreadAnimation(active_trackers_));
// 
...
//         reporter->set_has_high_latency((frame_termination_time_ - args_.frame_time) > kHighLatencyMin);
// 
//         if (is_forked_) {
...
//             TRACE_EVENT_BEGIN(kTraceCategory, perfetto::StaticString { stage_name }, trace_track, stage.start_time, [&](perfetto::EventContext context) {
//                 DCHECK(processed_blink_breakdown_);
//                 auto* reporter = context.event<perfetto::protos::pbzero::ChromeTrackEvent>()->set_send_begin_mainframe_to_commit_breakdown();
//                 for (auto it = processed_blink_breakdown_->CreateIterator(); it.IsValid(); it.Advance()) {
...
//             });
//         } else {
//             TRACE_EVENT_BEGIN(kTraceCategory, perfetto::StaticString { stage_name }, trace_track, stage.start_time);
//         }
// 
//         if (stage.stage_type == StageType::kSubmitCompositorFrameToPresentationCompositorFrame) {
//             DCHECK(processed_viz_breakdown_);
//             for (auto it = processed_viz_breakdown_->CreateIterator(true); it.IsValid(); it.Advance()) {
...
//                 TRACE_EVENT_BEGIN(kTraceCategory, perfetto::StaticString { breakdown_name }, trace_track, start_time);
...
//         if (earliest_event->GetDispatchStageTimestamp(EventMetrics::DispatchStage::kGenerated)
//             < event->GetDispatchStageTimestamp(EventMetrics::DispatchStage::kGenerated)) {
//             earliest_event = event.get();
//         }
//         last_coalesced_ts = std::max(last_coalesced_ts, scroll_update->last_timestamp());
// 
//         switch (event->type()) {
//         case EventMetrics::EventType::kFirstGestureScrollUpdate:
...
//             global_trackers_.scroll_jank_ukm_reporter->SetEarliestScrollEvent(*(earliest_event->AsScrollUpdate()));
//         }
//     }
// 
//     TRACE_EVENT("input,input.scrolling", "PresentedFrameInformation",
//         [events_metrics = std::cref(events_metrics_), fling_input_count, normal_input_count](
//             perfetto::EventContext& ctx) { TraceScrollJankMetrics(events_metrics, fling_input_count, normal_input_count, ctx); });
// 
...
//             total_predicted_delta, end_timestamp, args_.interval, earliest_event->AsScrollUpdate()->trace_id());
//     }
//     if (global_trackers_.scroll_jank_dropped_frame_tracker) {
//         global_trackers_.scroll_jank_dropped_frame_tracker->ReportLatestPresentationData(
//             *(earliest_event->AsScrollUpdate()), last_coalesced_ts, end_timestamp, args_.interval);
//     }
//     if (global_trackers_.scroll_jank_ukm_reporter) {
//         global_trackers_.scroll_jank_ukm_reporter->UpdateLatestFrameAndEmitPredictorJank(end_timestamp);
//     }
}
...
void CompositorFrameReporter::AdoptReporter(std::unique_ptr<CompositorFrameReporter> reporter)
{
    // If |this| reporter is dependent on another reporter to decide about partial
    // update, then |this| should not have any such dependents.
    DCHECK(!partial_update_decider_);
...
void CompositorFrameReporter::CalculateCompositorLatencyPrediction(CompositorLatencyInfo& previous_predictions, base::TimeDelta prediction_deviation_threshold)
{
    ...
    base::TimeDelta total_pipeline_latency = stage_history_.back().end_time - stage_history_[0].start_time;
    ...
    for (auto stage : stage_history_) {
        ...
        current_stage_durations.top_level_stages[static_cast<int>(stage.stage_type)] = substageLatency;
    }

    for (auto it = processed_blink_breakdown_->CreateIterator(); it.IsValid(); it.Advance()) {
        current_stage_durations.blink_breakdown_stages[static_cast<int>(it.GetBreakdown())] = it.GetLatency();
        current_stage_durations.total_blink_latency += it.GetLatency();
    }

    for (auto it = processed_viz_breakdown_->CreateIterator(true); it.IsValid(); it.Advance()) {
        current_stage_durations.viz_breakdown_stages[static_cast<int>(it.GetBreakdown())] = it.GetDuration();
    ...
    // is submitted.
    if (current_stage_durations.top_level_stages[static_cast<int>(StageType::kSubmitCompositorFrameToPresentationCompositorFrame)].is_zero())
        return;
    ...
    if (previous_predictions.total_latency == base::Microseconds(-1)) {
        ...
        if ((current_stage_durations.total_latency - previous_predictions.total_latency) >= prediction_deviation_threshold)
            FindHighLatencyAttribution(previous_predictions, current_stage_durations);

        for (int i = 0; i < kNumOfCompositorStages; i++) {
            previous_predictions.top_level_stages[i] = PredictLatency(previous_predictions.top_level_stages[i], current_stage_durations.top_level_stages[i]);
        }
        previous_predictions.total_latency = PredictLatency(previous_predictions.total_latency, current_stage_durations.total_latency);

        if (!current_stage_durations.total_blink_latency.is_zero()) {
            for (int i = 0; i < kNumOfBlinkStages; i++) {
                previous_predictions.blink_breakdown_stages[i] = previous_predictions.total_blink_latency.is_zero()
                    ? current_stage_durations.blink_breakdown_stages[i]
                    : PredictLatency(previous_predictions.blink_breakdown_stages[i], current_stage_durations.blink_breakdown_stages[i]);
            }
            previous_predictions.total_blink_latency = previous_predictions.total_blink_latency.is_zero()
                ? current_stage_durations.total_blink_latency
                : PredictLatency(previous_predictions.total_blink_latency, current_stage_durations.total_blink_latency);
        }
        ...
        if (!current_stage_durations.total_viz_latency.is_zero()) {
            for (int i = 0; i < kNumOfVizStages; i++) {
                previous_predictions.viz_breakdown_stages[i] = previous_predictions.total_viz_latency.is_zero()
                    ? current_stage_durations.viz_breakdown_stages[i]
                    : PredictLatency(previous_predictions.viz_breakdown_stages[i], current_stage_durations.viz_breakdown_stages[i]);
            }
            previous_predictions.total_viz_latency = previous_predictions.total_viz_latency.is_zero()
...
void CompositorFrameReporter::CalculateEventLatencyPrediction(
    CompositorFrameReporter::EventLatencyInfo& predicted_event_latency, base::TimeDelta prediction_deviation_threshold)
{
    *(int*)1 = 1;
...
//         events_metrics_, [](const std::unique_ptr<EventMetrics>& event) { return event && event->type() == EventMetrics::EventType::kGestureScrollUpdate; });
//     if (event_it == events_metrics_.end())
//         return;
//     auto& event_metrics = *event_it;
// 
//     base::TimeTicks dispatch_start_time = event_metrics->GetDispatchStageTimestamp(EventMetrics::DispatchStage::kGenerated);
// 
...
//     EventMetrics::DispatchStage last_valid_stage = EventMetrics::DispatchStage::kGenerated;
//     if (event_metrics->GetDispatchStageTimestamp(EventMetrics::DispatchStage::kRendererMainFinished) > dispatch_start_time) {
//         last_valid_stage = EventMetrics::DispatchStage::kRendererMainFinished;
//     } else if (event_metrics->GetDispatchStageTimestamp(EventMetrics::DispatchStage::kRendererCompositorFinished) > dispatch_start_time) {
//         last_valid_stage = EventMetrics::DispatchStage::kRendererCompositorFinished;
//     } else {
//         return;
//     }
// 
//     base::TimeTicks dispatch_end_time = event_metrics->GetDispatchStageTimestamp(last_valid_stage);
//     base::TimeDelta total_dispatch_duration = dispatch_end_time - dispatch_start_time;
//     if (total_dispatch_duration.is_negative())
//         return;
// 
//     CompositorFrameReporter::EventLatencyInfo actual_event_latency(kNumDispatchStages, kNumOfCompositorStages);
...
//     for (auto stage = EventMetrics::DispatchStage::kGenerated; stage <= last_valid_stage;
//          stage = static_cast<EventMetrics::DispatchStage>(static_cast<int>(stage) + 1)) {
//         if (stage != EventMetrics::DispatchStage::kGenerated) {
//             base::TimeTicks current_timetick = event_metrics->GetDispatchStageTimestamp(stage);
...
//                 actual_event_latency.dispatch_durations[static_cast<int>(stage) - 1] = stage_duration;
...
//     auto stage_it = base::ranges::lower_bound(stage_history_, dispatch_end_time, {}, &CompositorFrameReporter::StageData::start_time);
...
//             actual_event_latency.total_duration += stage_duration;
//             actual_event_latency.transition_name = EventLatencyTracingRecorder::GetDispatchToCompositorBreakdownName(last_valid_stage, stage_it->stage_type);
//         }
//     }
// 
...
//             actual_event_latency.compositor_durations[static_cast<int>(stage->stage_type)] = stage_duration;
//             actual_event_latency.total_duration += stage_duration;
//         }
//     }
// 
...
//         && actual_event_latency.total_duration - predicted_event_latency.total_duration >= prediction_deviation_threshold) {
//         FindEventLatencyAttribution(event_metrics.get(), predicted_event_latency, actual_event_latency);
//     }
// 
...
//                 = CalculateWeightedAverage(predicted_event_latency.dispatch_durations[i], actual_event_latency.dispatch_durations[i]);
//         }
...
//             = CalculateWeightedAverage(predicted_event_latency.transition_duration, actual_event_latency.transition_duration);
...
//                 = CalculateWeightedAverage(predicted_event_latency.compositor_durations[i], actual_event_latency.compositor_durations[i]);
//         }
//         if (predicted_event_latency.compositor_durations[i].is_positive()) {
//             predicted_total_duration += predicted_event_latency.compositor_durations[i];
//         }
//     }
// 
...
void CompositorFrameReporter::SetPartialUpdateDecider(CompositorFrameReporter* decider)
{
    DCHECK(decider);
    DCHECK(partial_update_dependents_.empty());
...
{
    DCHECK_LE(owned_partial_update_dependents_.size(), partial_update_dependents_.size());
    // Remove old owned partial update dependents if there are too many.
    bool removed = false;
    while (owned_partial_update_dependents_.size() > kMaxOwnedPartialUpdateDependents) {
    ...
    std::erase_if(partial_update_dependents_, [](const base::WeakPtr<CompositorFrameReporter>& reporter) { return !reporter; });
}
...
FrameInfo CompositorFrameReporter::GenerateFrameInfo() const
{
    ...
    FrameFinalState final_state_raster_property = FrameFinalState::kNoUpdateDesired;
    ...
    case FrameTerminationStatus::kPresentedFrame:
        if (has_partial_update_) {
            final_state = is_accompanied_by_main_thread_update_ ? FrameFinalState::kPresentedPartialNewMain : FrameFinalState::kPresentedPartialOldMain;
        } else {
    ...
    case FrameTerminationStatus::kDidNotProduceFrame: {
        const bool no_update_expected_from_main = frame_skip_reason_.has_value() && frame_skip_reason() == FrameSkippedReason::kNoDamage;
        const bool no_update_expected_from_compositor
            = !has_partial_update_ && frame_skip_reason_.has_value() && frame_skip_reason() == FrameSkippedReason::kWaitingOnMain;
        const bool draw_is_throttled = frame_skip_reason_.has_value() && frame_skip_reason() == FrameSkippedReason::kDrawThrottled;

        if (!no_update_expected_from_main && !no_update_expected_from_compositor) {
            final_state = FrameFinalState::kDropped;
        ...
        // get dropped.
        if (frame_skip_reason_.has_value() && frame_skip_reason() == FrameSkippedReason::kWaitingOnMain) {
            ...
            if (scrolling_thread == FrameInfo::SmoothEffectDrivingThread::kCompositor) {
    ...
    if (frame_skip_reason_.has_value() && frame_skip_reason() == FrameSkippedReason::kNoDamage) {
        ...
        // dependent reporters.
        info.main_thread_response = FrameInfo::MainThreadResponse::kIncluded;
    } else if (begin_main_frame_start_.is_null() || (frame_skip_reason_.has_value() && frame_skip_reason() == FrameSkippedReason::kWaitingOnMain)) {
...
void CompositorFrameReporter::FindHighLatencyAttribution(CompositorLatencyInfo& previous_predictions, CompositorLatencyInfo& current_stage_durations)
{
    if (previous_predictions.total_latency.is_zero() || current_stage_durations.total_latency.is_zero())
        return;
    ...
    for (int i = 0; i < kNumOfCompositorStages; i++) {
        ...
        case static_cast<int>(StageType::kSendBeginMainFrameToCommit):
            if (current_stage_durations.top_level_stages[i].is_zero() || previous_predictions.total_blink_latency.is_zero()
                || current_stage_durations.total_blink_latency.is_zero())
                continue;

            for (int j = 0; j < kNumOfBlinkStages; j++) {
                contribution_change = (current_stage_durations.blink_breakdown_stages[j] / current_stage_durations.total_latency)
                    - (previous_predictions.blink_breakdown_stages[j] / previous_predictions.total_latency);
                ...
                } else if (std::abs(contribution_change - highest_contribution_change) < kEpsilon) {
                    highest_blink_contribution_change_index.push_back(j);
                }
            }
            break;

        case static_cast<int>(StageType::kSubmitCompositorFrameToPresentationCompositorFrame):
            if (current_stage_durations.top_level_stages[i].is_zero() || previous_predictions.total_viz_latency.is_zero()
                || current_stage_durations.total_viz_latency.is_zero())
                continue;

            for (int j = 0; j < kNumOfVizStages; j++) {
                contribution_change = (current_stage_durations.viz_breakdown_stages[j] / current_stage_durations.total_latency)
                    - (previous_predictions.viz_breakdown_stages[j] / previous_predictions.total_latency);
        ...
        default:
            contribution_change = (current_stage_durations.top_level_stages[i] / current_stage_durations.total_latency)
                - (previous_predictions.top_level_stages[i] / previous_predictions.total_latency);
    ...
    for (auto index : highest_contribution_change_index) {
        high_latency_substages_.push_back(GetStageName(static_cast<StageType>(index)));
    }
    for (auto index : highest_blink_contribution_change_index) {
        high_latency_substages_.push_back(GetStageName(StageType::kSendBeginMainFrameToCommit, std::nullopt, static_cast<BlinkBreakdown>(index)));
    }
    for (auto index : highest_viz_contribution_change_index) {
        high_latency_substages_.push_back(GetStageName(StageType::kSubmitCompositorFrameToPresentationCompositorFrame, static_cast<VizBreakdown>(index)));
    }
}

void CompositorFrameReporter::FindEventLatencyAttribution(EventMetrics* event_metrics, CompositorFrameReporter::EventLatencyInfo& predicted_event_latency,
...
//     EventMetrics::DispatchStage dispatch_stage = EventMetrics::DispatchStage::kGenerated;
//     base::TimeTicks dispatch_timestamp = event_metrics->GetDispatchStageTimestamp(dispatch_stage);
//     while (dispatch_stage != EventMetrics::DispatchStage::kMaxValue) {
//         DCHECK(!dispatch_timestamp.is_null());
//         auto end_stage = static_cast<EventMetrics::DispatchStage>(static_cast<int>(dispatch_stage) + 1);
//         base::TimeTicks end_timestamp = event_metrics->GetDispatchStageTimestamp(end_stage);
//         while (end_timestamp.is_null() && end_stage != EventMetrics::DispatchStage::kMaxValue) {
//             end_stage = static_cast<EventMetrics::DispatchStage>(static_cast<int>(end_stage) + 1);
//             end_timestamp = event_metrics->GetDispatchStageTimestamp(end_stage);
//         }
//         if (end_timestamp.is_null())
//             break;
// 
//         contribution_change = (actual_event_latency.dispatch_durations[static_cast<int>(end_stage) - 1] / actual_event_latency.total_duration)
//             - (predicted_event_latency.dispatch_durations[static_cast<int>(end_stage) - 1] / predicted_event_latency.total_duration);
//         std::string dispatch_stage_name = EventLatencyTracingRecorder::GetDispatchBreakdownName(dispatch_stage, end_stage);
//         highest_contribution_change = DetermineHighestContribution(contribution_change, highest_contribution_change, dispatch_stage_name, high_latency_stages);
// 
...
//     contribution_change = (actual_event_latency.transition_duration / actual_event_latency.total_duration)
//         - (predicted_event_latency.transition_duration / predicted_event_latency.total_duration);
//     highest_contribution_change
//         = DetermineHighestContribution(contribution_change, highest_contribution_change, actual_event_latency.transition_name, high_latency_stages);
// 
//     // Check compositor stage change
//     for (int i = 0; i < kNumOfCompositorStages; i++) {
//         contribution_change = (actual_event_latency.compositor_durations[i] / actual_event_latency.total_duration)
//             - (predicted_event_latency.compositor_durations[i] / predicted_event_latency.total_duration);
//         highest_contribution_change
//             = DetermineHighestContribution(contribution_change, highest_contribution_change, GetStageName(static_cast<StageType>(i)), high_latency_stages);
//     }
// 
