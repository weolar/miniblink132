...
public:
    FrameSorterTest()
        : frame_sorter_(base::BindRepeating(&FrameSorterTest::FlushFrame, base::Unretained(this)))
    {
        IncreaseSourceId();
    }
    ...
    viz::BeginFrameArgs GetNextFrameArgs()
    {
        ...
        return viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, next_frame_source_id_, sequence_number, last_begin_frame_time_,
    ...
    void SimulateQueries(std::vector<std::string> queries)
    {
        ...
        for (auto& query : queries) {
            ...
            case 'D': {
                ++end_counters[id];
                FrameInfo info = CreateFakeFrameInfo(FrameInfo::FrameFinalState::kDropped);
            ...
            case 'P': {
                ++end_counters[id];
                FrameInfo info = CreateFakeFrameInfo(FrameInfo::FrameFinalState::kPresentedAll);
                if (end_counters[id] == 1) {
                    // For the first response to the frame, mark it as not including
                    // update from the main-thread.
                    info.main_thread_response = FrameInfo::MainThreadResponse::kMissing;
    ...
    void ValidateResults(std::vector<std::pair<uint64_t, bool>> expected_results)
    {
...
private:
    ...
    uint64_t next_frame_sequence_number_ = viz::BeginFrameArgs::kStartingFrameNumber;
...
TEST_F(FrameSorterTest, TestSortingFrames)
{
    ...
    std::vector<std::string> queries = { "S0", "S1", "P0", "S2", "P2", "S3", "P1", "D3" };
    std::vector<std::pair<uint64_t, bool>> expected_results = { { 0, false }, { 1, false }, { 2, false }, { 3, true } };
...
TEST_F(FrameSorterTest, ResetInMiddleOfAcks)
{
    ...
    std::vector<std::string> queries = { "S0", "S1", "P0", "S2", "P2", "S3", "R", "S4", "P1", "D4", "D3" };
    std::vector<std::pair<uint64_t, bool>> expected_results = { { 0, false }, { 2, false }, { 4, true } };
...
TEST_F(FrameSorterTest, ExpectingMultipleAcks)
{
    ...
    std::vector<std::string> queries = { "S0", "S1", "P0", "S1", "S2", "P2", "S3", "S4", "P1", "D3", "D4", "D1" };
    std::vector<std::pair<uint64_t, bool>> expected_results = { { 0, false }, { 1, true }, { 2, false }, { 3, true }, { 4, true } };
...
TEST_F(FrameSorterTest, ExpectingMultipleAcksWithReset)
{
    ...
    std::vector<std::string> queries = { "S0", "S1", "P0", "S1", "S2", "P2", "S3", "R", "S4", "P1", "D1", "D4", "D3" };
    std::vector<std::pair<uint64_t, bool>> expected_results = { { 0, false }, { 2, false }, { 4, true } };
...
TEST_F(FrameSorterTest, ExpectingMultipleAcksWithSourceIdIncrease)
{
    ...
    std::vector<std::string> queries = { "S0", "S0", "S1", "S1", "I", "R", "S0", "S0", "P0", "S1", "P1", "D0" };
    std::vector<std::pair<uint64_t, bool>> expected_results = { { 0, true }, { 1, false } };
