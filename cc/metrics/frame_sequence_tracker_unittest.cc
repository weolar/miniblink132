...
public:
    ...
    FrameSequenceTrackerTest()
        ...
        , collection_(/*is_single_threaded=*/false, compositor_frame_reporting_controller_.get())
        , sorter_(base::BindRepeating(&FrameSequenceTrackerTest::OnFrameResult, base::Unretained(this)))
    {
        tracker_ = collection_.StartScrollSequence(FrameSequenceTrackerType::kTouchScroll, FrameInfo::SmoothEffectDrivingThread::kCompositor);
    }
    ~FrameSequenceTrackerTest() override = default;

    void CreateNewTracker(FrameInfo::SmoothEffectDrivingThread thread_type = FrameInfo::SmoothEffectDrivingThread::kCompositor)
    {
        tracker_ = collection_.StartScrollSequence(FrameSequenceTrackerType::kTouchScroll, thread_type);
    }

    viz::BeginFrameArgs CreateBeginFrameArgs(uint64_t source_id, uint64_t sequence_number, base::TimeTicks now = base::TimeTicks::Now())
    {
        auto interval = base::Milliseconds(16);
        auto deadline = now + interval;
        return viz::BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, source_id, sequence_number, now, deadline, interval, viz::BeginFrameArgs::NORMAL);
    }
    ...
    uint32_t DispatchCompleteFrame(const viz::BeginFrameArgs& args, uint32_t damage_type, bool has_missing_content = false)
    {
        StartFrames(args);
    ...
    {
        auto key = std::make_pair(type, FrameInfo::SmoothEffectDrivingThread::kUnknown);
        if (type == FrameSequenceTrackerType::kTouchScroll || type == FrameSequenceTrackerType::kWheelScroll
            || type == FrameSequenceTrackerType::kScrollbarScroll) {
            key = std::make_pair(type, FrameInfo::SmoothEffectDrivingThread::kCompositor);
    ...
    bool RemovalTrackerExists(unsigned index, FrameSequenceTrackerType type) const
    {
    ...
    void GenerateSequence(const char* str)
    {
        ...
        while (*str) {
            ...
            switch (command) {
            case 'b':
                collection_.NotifyBeginImplFrame(CreateBeginFrameArgs(source_id, sequence));
                break;
            ...
            case 'd': {
                ...
                frame_info.final_state_raster_property = FrameInfo::FrameFinalState::kDropped;
                frame_info.smooth_thread = FrameInfo::SmoothThread::kSmoothCompositor;
                frame_info.smooth_thread_raster_property = FrameInfo::SmoothThread::kSmoothCompositor;
    ...
    void ReportMetrics()
    {
        tracker_->metrics_->ReportMetrics();
    }
    ...
    {
        return tracker_->metrics_->v3_.frames_expected - tracker_->metrics_->v3_.frames_dropped;
    }

    uint32_t frames_produced_v4() const
    {
        return tracker_->metrics_->v3_.frames_expected - tracker_->metrics_->v4_.frames_dropped;
    }

    FrameSequenceTracker::TerminationStatus GetTerminationStatus(FrameSequenceTracker* tracker)
    {
    ...
    void OnFrameResult(const viz::BeginFrameArgs& args, const FrameInfo& frame_info)
    {
        collection_.AddSortedFrame(args, frame_info);
    }
...
{
    collection_.StartSequence(FrameSequenceTrackerType::kCompositorNativeAnimation);
    collection_.StartSequence(FrameSequenceTrackerType::kMainThreadAnimation);
    EXPECT_EQ(NumberOfTrackers(), 3u);

    collection_.StopSequence(FrameSequenceTrackerType::kCompositorNativeAnimation);
...
TEST_F(FrameSequenceTrackerTest, ReportMetricsAtFixedInterval)
{
    ...
    auto args = CreateBeginFrameArgs(source, ++sequence, base::TimeTicks::Now() + first_time_delta);
    ...
    args = CreateBeginFrameArgs(source, ++sequence, args.frame_time + TimeDeltaToReport());
    collection_.NotifyBeginImplFrame(args);
    collection_.NotifyFrameEnd(args, args);
    FrameInfo frame_info;
    frame_info.final_state = FrameInfo::FrameFinalState::kPresentedAll;
...
TEST_F(FrameSequenceTrackerTest, TrackLastImplFrame1)
{
    ...
    FrameSequenceTracker* removal_tracker = collection_.GetRemovalTrackerForTesting(FrameSequenceTrackerType::kTouchScroll);
    EXPECT_EQ(GetTerminationStatus(removal_tracker), FrameSequenceTracker::TerminationStatus::kScheduledForTermination);
...
{
    CreateNewTracker(FrameInfo::SmoothEffectDrivingThread::kMain);
    const char sequence[] = "b(1)n(1)e(1,0)b(2)n(2)e(2,1)b(3)n(3)e(3,1)b(4)n(4)e(4,1)b(8)n(8)e(8,2)";
    GenerateSequence(sequence);
    EXPECT_EQ(frames_expected(), 0u);
...
{
    CreateNewTracker(FrameInfo::SmoothEffectDrivingThread::kMain);
    const char sequence[] = "b(34)n(34)e(34,0)b(35)n(35)e(35,0)b(36)n(36)e(36,34)b(39)e(39,34)";
    GenerateSequence(sequence);
...
{
    CreateNewTracker(FrameInfo::SmoothEffectDrivingThread::kMain);
    const char sequence[] = "b(1)e(1,0)b(2)n(2)e(2,0)b(3)n(3)e(3,0)b(4)n(4)e(4,3)";
    GenerateSequence(sequence);
...
{
    CreateNewTracker(FrameInfo::SmoothEffectDrivingThread::kMain);
    const char sequence[] = "b(18)n(18)N(18,18)Re(18,18)b(20)N(20,20)n(20)N(0,20)e(20,18)b(21)e(21,"
                            "21)";
    GenerateSequence(sequence);
    // No damage frames are not expected.
    EXPECT_EQ(frames_expected(), 0u);
...
{
    CreateNewTracker(FrameInfo::SmoothEffectDrivingThread::kMain);
    const char sequence[] = "b(78)n(78)Re(78,0)Rb(82)n(82)N(82,82)Re(82,82)b(86)n(86)e(86,86)b(87)e("
                            "87,86)";
    GenerateSequence(sequence);
    // No damage frames are not expected.
    EXPECT_EQ(frames_expected(), 0u);
...
{
    CreateNewTracker(FrameInfo::SmoothEffectDrivingThread::kMain);
    const char sequence[] = "b(2)n(2)N(2,2)e(2,2)b(3)n(3)N(3,3)e(3,3)b(4)n(4)N(4,4)e(4,4)b(5)n(5)N(5,"
...
TEST_F(FrameSequenceTrackerTest, TrackerTypeEncoding)
{
    ...
    EXPECT_EQ(NumberOfTrackers(), 1u);
    ActiveFrameSequenceTrackers active_encoded = collection_.FrameSequenceTrackerActiveTypes();
...
TEST_F(FrameSequenceTrackerTest, CustomTrackers)
{
    ...
    collection_.set_custom_tracker_results_added_callback(base::BindLambdaForTesting([&](const CustomTrackerResults& reported) {
