...
const char* GetPerScrollHistogramName(int num_frames, PerScrollHistogramType type)
{
...
const char* GetPerVsyncScrollHistogramName(int num_vsyncs, PerScrollHistogramType type)
{
...
void ScrollJankDroppedFrameTracker::EmitPerScrollHistogramsAndResetCounters()
{
    ...
    int delayed_frames_percentage = (100 * per_scroll_->missed_frames) / per_scroll_->num_presented_frames;
    UMA_HISTOGRAM_PERCENTAGE(kDelayedFramesPerScrollHistogram, delayed_frames_percentage);
    UMA_HISTOGRAM_CUSTOM_COUNTS(kMissedVsyncsMaxPerScrollHistogram, per_scroll_->max_missed_vsyncs, kVsyncCountsMin, kVsyncCountsMax, kVsyncCountsBuckets);
    UMA_HISTOGRAM_CUSTOM_COUNTS(kMissedVsyncsSumPerScrollHistogram, per_scroll_->missed_vsyncs, kVsyncCountsMin, kVsyncCountsMax, kVsyncCountsBuckets);
    // Emit bucketed histogram.
    base::UmaHistogramPercentage(GetPerScrollHistogramName(per_scroll_->num_presented_frames, PerScrollHistogramType::kPercentage), delayed_frames_percentage);
    base::UmaHistogramCustomCounts(GetPerScrollHistogramName(per_scroll_->num_presented_frames, PerScrollHistogramType::kMax), per_scroll_->max_missed_vsyncs,
        kVsyncCountsMin, kVsyncCountsMax, kVsyncCountsBuckets);
    base::UmaHistogramCustomCounts(GetPerScrollHistogramName(per_scroll_->num_presented_frames, PerScrollHistogramType::kSum), per_scroll_->missed_vsyncs,
        kVsyncCountsMin, kVsyncCountsMax, kVsyncCountsBuckets);
...
void ScrollJankDroppedFrameTracker::EmitPerScrollVsyncHistogramsAndResetCounters()
{
    // There should be at least one presented frame given the method is only
    // called after we have a successful presentation.
    ...
    // Emit non-bucketed histograms.
    int missed_vsyncs_percentage = (100 * experimental_per_scroll_vsync_->missed_vsyncs) / experimental_per_scroll_vsync_->num_past_vsyncs;
    UMA_HISTOGRAM_PERCENTAGE(kMissedVsyncsPerScrollHistogram, missed_vsyncs_percentage);
    // Emit bucketed histogram.
    base::UmaHistogramPercentage(
        GetPerVsyncScrollHistogramName(experimental_per_scroll_vsync_->num_past_vsyncs, PerScrollHistogramType::kPercentage), missed_vsyncs_percentage);
...
void ScrollJankDroppedFrameTracker::EmitPerWindowHistogramsAndResetCounters()
{
    ...
    UMA_HISTOGRAM_PERCENTAGE(kDelayedFramesWindowHistogram, (100 * fixed_window_.missed_frames) / kHistogramEmitFrequency);
    UMA_HISTOGRAM_CUSTOM_COUNTS(kMissedVsyncsSumInWindowHistogram, fixed_window_.missed_vsyncs, kVsyncCountsMin, kVsyncCountsMax, kVsyncCountsBuckets);
    UMA_HISTOGRAM_CUSTOM_COUNTS(kMissedVsyncsMaxInWindowHistogram, fixed_window_.max_missed_vsyncs, kVsyncCountsMin, kVsyncCountsMax, kVsyncCountsBuckets);
...
// default.
void ScrollJankDroppedFrameTracker::EmitPerVsyncWindowHistogramsAndResetCounters()
{
    DCHECK_GE(experimental_vsync_fixed_window_.num_past_vsyncs, kHistogramEmitFrequency);

    UMA_HISTOGRAM_PERCENTAGE(
        kMissedVsyncsWindowHistogram, (100 * experimental_vsync_fixed_window_.missed_vsyncs) / experimental_vsync_fixed_window_.num_past_vsyncs);
    UMA_HISTOGRAM_CUSTOM_COUNTS(
        kMissedVsyncsSumInVsyncWindowHistogram, experimental_vsync_fixed_window_.missed_vsyncs, kVsyncCountsMin, kVsyncCountsMax, kVsyncCountsBuckets);
    UMA_HISTOGRAM_CUSTOM_COUNTS(kMissedVsyncsMaxInVsyncWindowHistogram, fixed_window_.max_missed_vsyncs, kVsyncCountsMin, kVsyncCountsMax, kVsyncCountsBuckets);
...
void ScrollJankDroppedFrameTracker::ReportLatestPresentationData(
    ScrollUpdateEventMetrics& earliest_event, base::TimeTicks last_input_generation_ts, base::TimeTicks presentation_ts, base::TimeDelta vsync_interval)
{
    base::TimeTicks first_input_generation_ts = earliest_event.GetDispatchStageTimestamp(EventMetrics::DispatchStage::kGenerated);
    if ((last_input_generation_ts < first_input_generation_ts) || (presentation_ts <= last_input_generation_ts)) {
    ...
    bool missed_frame = (presentation_ts - prev_presentation_ts_) > (vsync_interval + vsync_interval / 2);
    bool input_available = (first_input_generation_ts - prev_last_input_generation_ts_) < (vsync_interval + vsync_interval / 2);
    ...
    int curr_frame_total_vsyncs = (presentation_ts - prev_presentation_ts_ + (vsync_interval / 2)) / vsync_interval;
    ...
    if (missed_frame && input_available) {
        ...
        UMA_HISTOGRAM_CUSTOM_COUNTS(kMissedVsyncsPerFrameHistogram, curr_frame_missed_vsyncs, kVsyncCountsMin, kVsyncCountsMax, kVsyncCountsBuckets);
        ...
        if (curr_frame_missed_vsyncs > per_scroll_->max_missed_vsyncs) {
            ...
            if (scroll_jank_ukm_reporter_) {
                scroll_jank_ukm_reporter_->set_max_missed_vsyncs(curr_frame_missed_vsyncs);
            }
        }
        ...
        TRACE_EVENT_INSTANT("input,input.scrolling", "MissedFrame", "per_scroll_->missed_frames", per_scroll_->missed_frames, "per_scroll_->missed_vsyncs",
        ...
        UMA_HISTOGRAM_CUSTOM_COUNTS(kMissedVsyncsPerFrameHistogram, 0, kVsyncCountsMin, kVsyncCountsMax, kVsyncCountsBuckets);
    }
    ...
    if (input_available) {
        ...
        experimental_vsync_fixed_window_.max_missed_vsyncs = std::max(experimental_vsync_fixed_window_.max_missed_vsyncs, curr_frame_missed_vsyncs);
    ...
    // TODO(b/306611560): Cleanup experimental per vsync metric or promote to
    // default.
    if (experimental_vsync_fixed_window_.num_past_vsyncs >= kHistogramEmitFrequency) {
