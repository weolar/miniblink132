...
protected:
    ...
    template <class T> void SetReportingCallabck(T cb)
    {
        reporter_ = std::make_unique<VideoPlaybackRoughnessReporter>(base::BindLambdaForTesting(cb));
    }
    ...
    scoped_refptr<VideoFrame> MakeFrame(base::TimeDelta duration, int frame_size = 100)
    {
        scoped_refptr<VideoFrame> result = media::VideoFrame::CreateColorFrame(gfx::Size(frame_size, frame_size), 0x80, 0x80, 0x80, base::TimeDelta());
    ...
    {
        size_t max_frames = 2 * size_t { VideoPlaybackRoughnessReporter::kMaxWindowSize };
        if (reporter()->frames_.size() > max_frames)
            return ::testing::AssertionFailure();

        constexpr int max_worst_windows_size
            = 1 + VideoPlaybackRoughnessReporter::kMaxWindowsBeforeSubmit * (100 - VideoPlaybackRoughnessReporter::kPercentileToSubmit) / 100;
        if (reporter()->worst_windows_.size() > max_worst_windows_size)
            return ::testing::AssertionFailure() << "windows " << reporter()->worst_windows_.size();
        return ::testing::AssertionSuccess();
    }

    void NormalRun(double fps, double hz, std::vector<int> cadence, int frames, int frame_size = 100)
    {
    ...
    void BatchPresentationRun(double fps, double hz, std::vector<int> cadence, int frames)
    {
        base::TimeDelta vsync = base::Seconds(1 / hz);
        base::TimeDelta ideal_duration = base::Seconds(1 / fps);
    ...
    void FreezingRun(double fps, double hz, std::vector<int> cadence, int frames, int frame_size = 100, int freeze_on_frame = 50, int frozen_vsyncs = 10)
    {
        base::TimeDelta vsync = base::Seconds(1 / hz);
        base::TimeDelta ideal_duration = base::Seconds(1 / fps);
        for (int idx = 0; idx < frames; idx++) {
...
TEST_F(VideoPlaybackRoughnessReporterTest, BestCase24fps)
{
    int call_count = 0;
    int fps = 24;
    SetReportingCallabck([&](const VideoPlaybackRoughnessReporter::Measurement& measurement) {
    ...
    int frames_to_run = VideoPlaybackRoughnessReporter::kMaxWindowsBeforeSubmit * fps + 10;
...
TEST_F(VideoPlaybackRoughnessReporterTest, BestCase30fps)
{
    ...
    int frames_to_run = VideoPlaybackRoughnessReporter::kMaxWindowsBeforeSubmit * fps + 1;
...
TEST_F(VideoPlaybackRoughnessReporterTest, UserStudyOkay)
{
    ...
    int frames_to_run = VideoPlaybackRoughnessReporter::kMaxWindowsBeforeSubmit * fps + 1;
    NormalRun(fps, 60, { 2, 2, 2, 2, 2, 2, 1, 3, 2, 2, 2, 2, 2, 2, 2 }, frames_to_run);
    EXPECT_EQ(call_count, 1);
}
...
TEST_F(VideoPlaybackRoughnessReporterTest, UserStudyBad)
{
    ...
    int frames_to_run = VideoPlaybackRoughnessReporter::kMaxWindowsBeforeSubmit * fps + 1;
    NormalRun(fps, 60, { 2, 2, 2, 2, 2, 1, 2, 2, 3, 2, 2, 2, 2, 2, 2 }, frames_to_run);
    EXPECT_EQ(call_count, 1);
}
...
TEST_F(VideoPlaybackRoughnessReporterTest, PredictableRoughnessValue)
{
    ...
    base::TimeDelta error = base::Milliseconds(std::sqrt(intended_roughness * intended_roughness * frames_in_window));

    auto callback = [&](const VideoPlaybackRoughnessReporter::Measurement& measurement) {
...
TEST_F(VideoPlaybackRoughnessReporterTest, TakingPercentile)
{
    ...
    double expected_roughness = VideoPlaybackRoughnessReporter::kPercentileToSubmit * 0.1;
    std::mt19937 rnd(1);
    ...
    for (int win_idx = 0; win_idx < win_count; win_idx++) {
        ...
        // Calculating the error value that needs to be injected into one frame
        // in order to get desired roughness.
        base::TimeDelta error = base::Milliseconds(std::sqrt(roughness * roughness * frames_in_window));
...
TEST_F(VideoPlaybackRoughnessReporterTest, LongRunWithoutWindows)
{
    ...
    SetReportingCallabck([&](const VideoPlaybackRoughnessReporter::Measurement& measurement) { call_count++; });
...
TEST_F(VideoPlaybackRoughnessReporterTest, ReportingInReset)
{
    int call_count = 0;
    int fps = 60;
    auto callback = [&](const VideoPlaybackRoughnessReporter::Measurement& measurement) { call_count++; };
    SetReportingCallabck(callback);

    // Set number of frames insufficient for reporting in Reset()
    int frames_to_run = VideoPlaybackRoughnessReporter::kMinWindowsBeforeSubmit * fps - 1;
    NormalRun(fps, 60, { 1 }, frames_to_run);
    ...
    frames_to_run = VideoPlaybackRoughnessReporter::kMinWindowsBeforeSubmit * fps + 1;
    NormalRun(fps, 60, { 1 }, frames_to_run);
...
TEST_F(VideoPlaybackRoughnessReporterTest, ReportingAfterParameterChange)
{
    ...
    int fps = 60;
    auto callback = [&](const VideoPlaybackRoughnessReporter::Measurement& measurement) {
        reports.push_back({ measurement.refresh_rate_hz, measurement.frame_size.height(), measurement.roughness });
    };
    SetReportingCallabck(callback);

    int frames_to_run = (VideoPlaybackRoughnessReporter::kMinWindowsBeforeSubmit - 1) * fps + 3;
    NormalRun(fps, 59, { 1 }, frames_to_run, 480);
    ASSERT_TRUE(reports.empty());

    frames_to_run = (VideoPlaybackRoughnessReporter::kMinWindowsBeforeSubmit + 1) * fps + 3;
    NormalRun(fps, 60, { 1 }, frames_to_run, 480);
    // Check that if parameters change after only a few windows, nothing gets
    // reported.
    ASSERT_TRUE(reports.empty());

    frames_to_run = (VideoPlaybackRoughnessReporter::kMaxWindowsBeforeSubmit + 1) * fps + 3;
...
TEST_F(VideoPlaybackRoughnessReporterTest, BatchPresentation)
{
    ...
    fps = 24;
    frames_to_run = VideoPlaybackRoughnessReporter::kMaxWindowsBeforeSubmit * fps + 10;
