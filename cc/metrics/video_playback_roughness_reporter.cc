...
{
    constexpr int size
        = 1 + cc::VideoPlaybackRoughnessReporter::kMaxWindowsBeforeSubmit * (100 - cc::VideoPlaybackRoughnessReporter::kPercentileToSubmit) / 100;
    static_assert(size > 1, "worst_windows_ is too small");
    static_assert(size < 25, "worst_windows_ is too big");
    return size;
}

void RecordUmaVideoFrameSubmitter(bool is_media_stream, base::TimeDelta time_since_decode)
{
    if (is_media_stream) {
        base::UmaHistogramTimes("Media.VideoFrameSubmitter.Rtc.PresentationDelay", time_since_decode);
    } else {
        base::UmaHistogramTimes("Media.VideoFrameSubmitter.Video.PresentationDelay", time_since_decode);
    }
...
VideoPlaybackRoughnessReporter::VideoPlaybackRoughnessReporter(ReportingCallback reporting_cb)
...
double VideoPlaybackRoughnessReporter::ConsecutiveFramesWindow::roughness() const
{
    return root_mean_square_error.InMillisecondsF();
}

VideoPlaybackRoughnessReporter::FrameInfo::FrameInfo() = default;
VideoPlaybackRoughnessReporter::FrameInfo::FrameInfo(const FrameInfo&) = default;

void VideoPlaybackRoughnessReporter::FrameSubmitted(TokenType token, const media::VideoFrame& frame, base::TimeDelta render_interval)
{
    ...
    if (info.intended_duration) {
        ...
        const int win_size = base::ClampRound(info.intended_duration.value().ToHz());
...
void VideoPlaybackRoughnessReporter::FramePresented(TokenType token, base::TimeTicks timestamp, bool reliable_timestamp)
{
...
void VideoPlaybackRoughnessReporter::SubmitPlaybackRoughness()
{
    ...
    if (index_to_submit < 0 || index_to_submit >= static_cast<int>(worst_windows_.size())) {
        DCHECK(false);
        return;
    }
...
void VideoPlaybackRoughnessReporter::ReportWindow(const ConsecutiveFramesWindow& win)
{
...
void VideoPlaybackRoughnessReporter::ProcessFrameWindow()
{
    ...
    for (; next_frame_it != frames_.end(); cur_frame_it++, next_frame_it++) {
        ...
        if (!cur_frame.presentation_time.has_value() || !next_frame.presentation_time.has_value()) {
            // We reached a frame that hasn't been presented yet, there is
            // no way to keep processing the window.
            break;
        }

        cur_frame.actual_duration = next_frame.presentation_time.value() - cur_frame.presentation_time.value();
    }
    ...
    if (next_frame_it - frames_.begin() > frames_window_size_) {
        ...
        for (auto i = 0; i < frames_window_size_; i++) {
            ...
            if (win.frame_size != frame.size || win.refresh_rate_hz != frame.refresh_rate_hz) {
                observed_change_in_parameters = true;
                break;
            }

            if (frame.actual_duration.has_value() && frame.intended_duration.has_value()) {
                error = frame.actual_duration.value() - frame.intended_duration.value();
                win.intended_duration += frame.intended_duration.value();
            }
            total_error += error;
            max_single_frame_error_ = std::max(max_single_frame_error_, error.magnitude());
            mean_square_error_ms2 += total_error.InMillisecondsF() * total_error.InMillisecondsF();
        }
        win.size = frames_window_size_;
        win.root_mean_square_error = base::Milliseconds(std::sqrt(mean_square_error_ms2 / frames_window_size_));
