...
struct FrameTimestamps {
    ...
    FrameTimestamps(base::TimeTicks first_input, base::TimeTicks last_input, base::TimeTicks presentation)
...
constexpr int kHistogramEmitFrequency = ScrollJankDroppedFrameTracker::kHistogramEmitFrequency;
constexpr int kFirstWindowSize = kHistogramEmitFrequency + 1;
constexpr const char* kDelayedFramesWindowHistogram = ScrollJankDroppedFrameTracker::kDelayedFramesWindowHistogram;
constexpr const char* kMissedVsyncsWindowHistogram = ScrollJankDroppedFrameTracker::kMissedVsyncsWindowHistogram;
constexpr const char* kDelayedFramesPerScrollHistogram = ScrollJankDroppedFrameTracker::kDelayedFramesPerScrollHistogram;
constexpr const char* kMissedVsyncsPerScrollHistogram = ScrollJankDroppedFrameTracker::kMissedVsyncsPerScrollHistogram;
constexpr const char* kMissedVsyncsSumInWindowHistogram = ScrollJankDroppedFrameTracker::kMissedVsyncsSumInWindowHistogram;
constexpr const char* kMissedVsyncsSumInVsyncWindowHistogram = ScrollJankDroppedFrameTracker::kMissedVsyncsSumInVsyncWindowHistogram;
constexpr const char* kMissedVsyncsMaxInWindowHistogram = ScrollJankDroppedFrameTracker::kMissedVsyncsMaxInWindowHistogram;
constexpr const char* kMissedVsyncsMaxInVsyncWindowHistogram = ScrollJankDroppedFrameTracker::kMissedVsyncsMaxInVsyncWindowHistogram;
constexpr const char* kMissedVsyncsSumPerScrollHistogram = ScrollJankDroppedFrameTracker::kMissedVsyncsSumPerScrollHistogram;
constexpr const char* kMissedVsyncsMaxPerScrollHistogram = ScrollJankDroppedFrameTracker::kMissedVsyncsMaxPerScrollHistogram;
constexpr const char* kMissedVsyncsPerFrameHistogram = ScrollJankDroppedFrameTracker::kMissedVsyncsPerFrameHistogram;
} // namespace
...
public:
    ...
    FrameTimestamps ProduceAndReportMockFrames(FrameTimestamps prev_frame, int num_frames)
    {
        for (int i = 1; i <= num_frames; i++) {
            ...
            auto event = ScrollUpdateEventMetrics::CreateForTesting(ui::EventType::kGestureScrollUpdate, ui::ScrollInputType::kWheel,
                ...
                /*delta=*/10.0f, prev_frame.first_input_ts, base::TimeTicks(), &tick_clock, /*trace_id=*/std::nullopt);

            scroll_jank_dropped_frame_tracker_->ReportLatestPresentationData(
                *event.get(), prev_frame.last_input_ts, prev_frame.presentation_ts, kVsyncInterval);
        }
        return prev_frame;
    }
    ...
    void ReportLatestPresentationDataToTracker(const FrameTimestamps& frame)
    {
        ...
        scroll_jank_dropped_frame_tracker_->ReportLatestPresentationData(*event.get(), frame.last_input_ts, frame.presentation_ts, kVsyncInterval);
    }
...
TEST_F(ScrollJankDroppedFrameTrackerTest, EmitsHistograms)
{
    ...
    FrameTimestamps last_frame = ProduceAndReportMockFrames(f1, kHistogramEmitFrequency);
    ...
    histogram_tester->ExpectUniqueSample(kMissedVsyncsSumInVsyncWindowHistogram, 0, 0);
    ...
    histogram_tester->ExpectUniqueSample(kMissedVsyncsSumInVsyncWindowHistogram, 0, 1);
    ...
    histogram_tester->ExpectUniqueSample(kMissedVsyncsSumInVsyncWindowHistogram, 0, 2);
}
...
TEST_F(ScrollJankDroppedFrameTrackerTest, MissedVsyncWhenInputWasPresent)
{
    ...
    FrameTimestamps last_frame_ts = ProduceAndReportMockFrames(f3, frames_to_emit_histogram);
    ...
    const int expected_delayed_frames_percentage = (100 * expected_missed_frames) / kHistogramEmitFrequency;
    // Frame F2 missed 2 vsyncs, F3 missed 1 vsync.
    const int expected_max = 2;
    const int expected_sum = 3;
    histogram_tester->ExpectUniqueSample(kDelayedFramesWindowHistogram, expected_delayed_frames_percentage, 1);
    histogram_tester->ExpectUniqueSample(kMissedVsyncsSumInWindowHistogram, expected_sum, 1);
    histogram_tester->ExpectUniqueSample(kMissedVsyncsMaxInWindowHistogram, expected_max, 1);
...
TEST_F(ScrollJankDroppedFrameTrackerTest, MissedVsyncsPerVsyncWindow)
{
    ...
    const int expected_delayed_frames_percentage = (100 * expected_missed_vsyncs) / kHistogramEmitFrequency;
    // Frame F2 missed 2 vsyncs, F3 missed 1 vsync.
    const int expected_sum = 3;
    const int expected_max = 2;
    histogram_tester->ExpectUniqueSample(kMissedVsyncsWindowHistogram, expected_delayed_frames_percentage, 1);
    histogram_tester->ExpectUniqueSample(kMissedVsyncsSumInVsyncWindowHistogram, expected_sum, 1);
    histogram_tester->ExpectUniqueSample(kMissedVsyncsMaxInVsyncWindowHistogram, expected_max, 1);

    // The counters were reset for next set of `kHistogramEmitFrequency` frames.
    ProduceAndReportMockFrames(last_frame_ts, kHistogramEmitFrequency);

    histogram_tester->ExpectBucketCount(kMissedVsyncsWindowHistogram, 0, 1);
    histogram_tester->ExpectBucketCount(kMissedVsyncsSumInVsyncWindowHistogram, 0, 1);
    histogram_tester->ExpectBucketCount(kMissedVsyncsMaxInVsyncWindowHistogram, 0, 1);
}
...
class PerScrollTests : public ScrollJankDroppedFrameTrackerTest, public testing::WithParamInterface<ScrollTestCase> { };
...
TEST_P(PerScrollTests, MetricsEmittedPerScroll)
{
    ...
    FrameTimestamps last_ts = ProduceAndReportMockFrames(f3, params.num_frames - 3);
    ...
    const int expected_delayed_frames_percentage = (100 * expected_missed_frames) / total_frames;
    // Frame F2 missed 2 vsyncs, F3 missed 1 vsync.
    const int expected_max = 2;
    const int expected_sum = 3;

    // Emits non-bucketed histograms.
    histogram_tester->ExpectUniqueSample(kMissedVsyncsSumPerScrollHistogram, expected_sum, 1);
    histogram_tester->ExpectUniqueSample(kMissedVsyncsMaxPerScrollHistogram, expected_max, 1);
    histogram_tester->ExpectUniqueSample(kDelayedFramesPerScrollHistogram, expected_delayed_frames_percentage, 1);

    // Emits bucketed histograms.
    histogram_tester->ExpectUniqueSample(base::StrCat({ kMissedVsyncsSumPerScrollHistogram, params.suffix }), expected_sum, 1);
    histogram_tester->ExpectUniqueSample(base::StrCat({ kMissedVsyncsMaxPerScrollHistogram, params.suffix }), expected_max, 1);
    histogram_tester->ExpectUniqueSample(base::StrCat({ kDelayedFramesPerScrollHistogram, params.suffix }), expected_delayed_frames_percentage, 1);
...
TEST_P(PerScrollTests, VsyncMetricsEmittedPerScroll)
{
    ...
    const int expected_missed_vsyncs_percentage = (100 * expected_missed_vsyncs) / total_vsyncs;

    // Emits non-bucketed histograms.
    histogram_tester->ExpectUniqueSample(kMissedVsyncsSumPerScrollHistogram, expected_sum, 1);
    histogram_tester->ExpectUniqueSample(kMissedVsyncsMaxPerScrollHistogram, expected_max, 1);
    histogram_tester->ExpectUniqueSample(kMissedVsyncsPerScrollHistogram, expected_missed_vsyncs_percentage, 1);

    // Emits bucketed histograms.
    histogram_tester->ExpectUniqueSample(base::StrCat({ kMissedVsyncsPerScrollHistogram, params.suffix }), expected_missed_vsyncs_percentage, 1);
...
INSTANTIATE_TEST_SUITE_P(PerScrollTests, PerScrollTests,
    ...
    [](const testing::TestParamInfo<PerScrollTests::ParamType>& info) { return info.param.test_name; });
