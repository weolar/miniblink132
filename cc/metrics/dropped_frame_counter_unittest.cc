...
{
    return CreateFakeFrameInfo(is_dropped ? FrameInfo::FrameFinalState::kDropped : FrameInfo::FrameFinalState::kPresentedAll);
}
...
public:
    ...
    void BeginMainFrame(const viz::BeginFrameArgs& args) override
    {
        ...
        if (config_.should_drop_main_every > 0) {
            should_wait = args.frame_id.sequence_number % config_.should_drop_main_every == 0;
        }
...
class DroppedFrameCounterMainDropsNoSmoothness : public DroppedFrameCounterTestBase {
public:
...
class DroppedFrameCounterMainDropsSmoothnessTest : public DroppedFrameCounterTestBase {
public:
...
public:
    explicit DroppedFrameCounterTest(SmoothnessStrategy smoothness_strategy = SmoothnessStrategy::kDefaultStrategy)
    ...
    void SimulateFrameSequence(std::vector<bool> frame_states, int repeat)
    {
        for (int i = 0; i < repeat; i++) {
            for (auto is_dropped : frame_states) {
                ...
                dropped_frame_counter_.OnEndFrame(args_, CreateStubFrameInfo(is_dropped));
                sequence_number_++;
    ...
    {
        auto percent_dropped = dropped_frame_counter_.max_percent_dropped_After_1_sec();
    ...
    {
        auto percent_dropped = dropped_frame_counter_.max_percent_dropped_After_2_sec();
        EXPECT_TRUE(percent_dropped.has_value());
        return percent_dropped.value();
    }
    ...
    {
        auto percent_dropped = dropped_frame_counter_.max_percent_dropped_After_5_sec();
        EXPECT_TRUE(percent_dropped.has_value());
        return percent_dropped.value();
    }
    ...
    {
        return dropped_frame_counter_.SlidingWindow95PercentilePercentDropped(smoothness_strategy_);
    }

    double PercentDroppedFrameMedian()
    {
        return dropped_frame_counter_.SlidingWindowMedianPercentDropped(smoothness_strategy_);
    }

    double PercentDroppedFrameVariance()
    {
        return dropped_frame_counter_.SlidingWindowPercentDroppedVariance(smoothness_strategy_);
    }

    const DroppedFrameCounter::SlidingWindowHistogram* GetSlidingWindowHistogram()
    {
        return dropped_frame_counter_.GetSlidingWindowHistogram(smoothness_strategy_);
    }

    double GetTotalFramesInWindow()
    {
        return base::Seconds(1) / interval_;
    }
    ...
    base::TimeTicks GetNextFrameTime() const
    {
        return frame_time_ + interval_;
    }
    ...
    bool CheckSmoothnessBuckets(std::vector<double> expected_buckets)
    {
        ...
        std::vector<double> buckets = GetSlidingWindowHistogram()->GetPercentDroppedFrameBuckets();
...
// expectations.
class SmoothnessStrategyDroppedFrameCounterTest : public DroppedFrameCounterTest, public testing::WithParamInterface<SmoothnessStrategy> {
public:
    ...
    SmoothnessStrategyDroppedFrameCounterTest(const SmoothnessStrategyDroppedFrameCounterTest&) = delete;
    SmoothnessStrategyDroppedFrameCounterTest& operator=(const SmoothnessStrategyDroppedFrameCounterTest&) = delete;
};

INSTANTIATE_TEST_SUITE_P(DefaultStrategy, SmoothnessStrategyDroppedFrameCounterTest, ::testing::Values(SmoothnessStrategy::kDefaultStrategy));
...
TEST_P(SmoothnessStrategyDroppedFrameCounterTest, MaxPercentDroppedChanges)
{
    ...
    double expected_percent_dropped_frame1 = (12 / GetTotalFramesInWindow()) * 100;
    ...
    double expected_percent_dropped_frame2 = (24 / GetTotalFramesInWindow()) * 100;
    ...
    double expected_percent_dropped_frame3 = (42 / GetTotalFramesInWindow()) * 100;
...
TEST_P(SmoothnessStrategyDroppedFrameCounterTest, Percentile95WithIdleFrames)
{
    ...
    static_assert(kFps % 5 == 0, "kFps must be a multiple of 5 because this test depends on it.");
...
TEST_P(SmoothnessStrategyDroppedFrameCounterTest, Percentile95WithIdleFramesWhileHidden)
{
...
TEST_P(SmoothnessStrategyDroppedFrameCounterTest, Percentile95WithIdleFramesThenHide)
{
...
TEST_P(SmoothnessStrategyDroppedFrameCounterTest, ResetPendingFramesUpdatesMaxPercentDroppedFrames)
{
...
TEST_F(DroppedFrameCounterTest, ConsistentSmoothnessRatings)
{
    ...
    constexpr auto kInterval = base::Milliseconds(10);
    constexpr int kFps = base::Seconds(1).IntDiv(kInterval);
    static_assert(kFps == 100, "kFps must be 100 because this test depends on it.");
    SetInterval(kInterval);
...
TEST_F(DroppedFrameCounterTest, FramesInFlightWhenFcpReceived)
{
    ...
    base::TimeTicks time_fcp_sent = pending_frames[2].frame_time + pending_frames[2].interval / 2;
...
TEST_F(DroppedFrameCounterTest, WorstSmoothnessTiming)
{
    ...
    base::TimeTicks time_fcp_sent = last_frame.frame_time + last_frame.interval / 2;
    dropped_frame_counter_.OnFcpReceived();
    dropped_frame_counter_.SetTimeFcpReceivedForTesting(time_fcp_sent);
