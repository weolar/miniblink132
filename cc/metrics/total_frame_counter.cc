...
void TotalFrameCounter::UpdateTotalFramesSinceLastVisible(base::TimeTicks until)
{
    total_frames_ = ComputeTotalVisibleFrames(until);
}

size_t TotalFrameCounter::ComputeTotalVisibleFrames(base::TimeTicks until) const
{
    ...
    auto frames_since = std::ceil((until - last_shown_timestamp_) / latest_interval_);
