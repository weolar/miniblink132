...
public:
    ...
    enum PipelineStage { kBeginImplFrame = 0, kBeginMainFrame, kReadyToCommit, kCommit, kActivate, kNumPipelineStages };

    CompositorFrameReportingController(bool should_report_histograms, bool should_report_ukm, int layer_tree_host_id);
    virtual ~CompositorFrameReportingController();

    CompositorFrameReportingController(const CompositorFrameReportingController&) = delete;
    CompositorFrameReportingController& operator=(const CompositorFrameReportingController&) = delete;
    ...
    virtual void BeginMainFrameAborted(const viz::BeginFrameId& id, CommitEarlyOutReason reason);
    virtual void WillInvalidateOnImplSide();
    virtual void WillCommit();
    virtual void DidCommit();
    virtual void WillActivate();
    virtual void DidActivate();
    virtual void DidSubmitCompositorFrame(SubmitInfo& submit_info, const viz::BeginFrameId& current_frame_id, const viz::BeginFrameId& last_activated_frame_id);
    virtual void DidNotProduceFrame(const viz::BeginFrameId& id, FrameSkippedReason skip_reason);
    virtual void OnFinishImplFrame(const viz::BeginFrameId& id);
    virtual void DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& details);
    ...
    void SetThreadAffectsSmoothness(FrameInfo::SmoothEffectDrivingThread thread_type, bool affects_smoothness);
    ...
    std::unique_ptr<CompositorFrameReporter>* reporters()
    {
        return reporters_;
    }
    ...
    void SetFrameSequenceTrackerCollection(FrameSequenceTrackerCollection* frame_sequence_trackers)
    {
...
protected:
    struct SubmittedCompositorFrame {
        ...
        SubmittedCompositorFrame(uint32_t frame_token, std::unique_ptr<CompositorFrameReporter> reporter);
...
private:
    ...
    std::unique_ptr<CompositorFrameReporter> RestoreReporterAtBeginImpl(const viz::BeginFrameId& id);
    CompositorFrameReporter::SmoothThread GetSmoothThread() const;
    CompositorFrameReporter::SmoothThread GetSmoothThreadAtTime(base::TimeTicks timestamp) const;
    ...
    // returns nullptr.
    CompositorFrameReporter* GetOutstandingUpdatesFromMain(const viz::BeginFrameId& id) const;
    ...
    void MaybePassEventMetricsFromDroppedFrames(CompositorFrameReporter& reporter, uint32_t frame_token, bool next_reporter_from_same_frame);
    void StoreEventMetricsFromDroppedFrames(CompositorFrameReporter& reporter, uint32_t frame_token);
    void CreateReportersForDroppedFrames(const viz::BeginFrameArgs& old_args, const viz::BeginFrameArgs& new_args) const;
    ...
    void SetPartialUpdateDeciderWhenWaitingOnMain(std::unique_ptr<CompositorFrameReporter>& reporter);

    void AddSortedFrame(const viz::BeginFrameArgs& args, const FrameInfo& frame_info);
    ...
    FrameInfo::SmoothEffectDrivingThread scrolling_thread_ = FrameInfo::SmoothEffectDrivingThread::kUnknown;
    ...
    std::map<base::TimeTicks, CompositorFrameReporter::SmoothThread> smooth_thread_history_;
    ...
    std::unique_ptr<CompositorFrameReporter> reporters_[PipelineStage::kNumPipelineStages];
    ...
    struct {
        ...
        FrameInfo::SmoothEffectDrivingThread scrolling_thread = FrameInfo::SmoothEffectDrivingThread::kUnknown;
        ActiveTrackers active_trackers;
        CompositorFrameReporter::SmoothThread smooth_thread = CompositorFrameReporter::SmoothThread::kSmoothNone;
    ...
    CompositorFrameReporter::CompositorLatencyInfo previous_latency_predictions_main_;
    CompositorFrameReporter::CompositorLatencyInfo previous_latency_predictions_impl_;
