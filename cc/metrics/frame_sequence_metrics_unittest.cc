...
public:
    ...
    void SetFramesExpectedAndProduced(FrameSequenceMetrics& metrics, uint32_t frames_expected, uint32_t frames_dropped);
};

void FrameSequenceMetricsTest::SetFramesExpectedAndProduced(FrameSequenceMetrics& metrics, uint32_t frames_expected, uint32_t frames_dropped)
{
...
TEST_F(FrameSequenceMetricsTest, MergeMetrics)
{
    ...
    auto second = std::make_unique<FrameSequenceMetrics>(FrameSequenceTrackerType::kTouchScroll);
...
TEST_F(FrameSequenceMetricsTest, CheckerboardingMetricV4EmitsValidValue)
{
    ...
    histograms.ExpectTotalCount("Graphics.Smoothness.Checkerboarding4.MainThread.TouchScroll", 1u);
    histograms.ExpectTotalCount("Graphics.Smoothness.Checkerboarding4.CompositorThread.TouchScroll", 1u);
}
