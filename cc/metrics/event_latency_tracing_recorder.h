...
public:
    ...
    static const char* GetDispatchBreakdownName(EventMetrics::DispatchStage start_stage, EventMetrics::DispatchStage end_stage);

    // Returns the name of EventLatency breakdown between `dispatch_stage` and
    // `compositor_stage`.
    static const char* GetDispatchToCompositorBreakdownName(EventMetrics::DispatchStage dispatch_stage, CompositorFrameReporter::StageType compositor_stage);

    // Returns the name of EventLatency breakdown between `dispatch_stage` and
    // termination for events not associated with a frame update.
    static const char* GetDispatchToTerminationBreakdownName(EventMetrics::DispatchStage dispatch_stage);

    static void RecordEventLatencyTraceEvent(EventMetrics* event_metrics, base::TimeTicks termination_time, base::TimeDelta vsync_interval,
...
private:
    ...
    static void RecordEventLatencyTraceEventInternal(const EventMetrics* event_metrics, base::TimeTicks termination_time, base::TimeDelta vsync_interval,
        const std::vector<CompositorFrameReporter::StageData>* stage_history, const CompositorFrameReporter::ProcessedVizBreakdown* viz_breakdown);
};
