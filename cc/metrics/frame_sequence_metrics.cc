...
bool ShouldReportForAnimation(FrameSequenceTrackerType sequence_type, SmoothEffectDrivingThread thread_type)
{
    ...
    if (sequence_type == FrameSequenceTrackerType::kCompositorNativeAnimation || sequence_type == FrameSequenceTrackerType::kCompositorRasterAnimation) {
        return thread_type == SmoothEffectDrivingThread::kCompositor;
    }

    if (sequence_type == FrameSequenceTrackerType::kMainThreadAnimation || sequence_type == FrameSequenceTrackerType::kJSAnimation)
...
bool ShouldReportForInteraction(
    FrameSequenceTrackerType sequence_type, SmoothEffectDrivingThread reporting_thread_type, SmoothEffectDrivingThread metrics_effective_thread_type)
{
    // For scrollbar/touch/wheel scroll, the slower thread is the one we want to
    // report. For pinch-zoom, it's the compositor-thread.
    if (sequence_type == FrameSequenceTrackerType::kScrollbarScroll || sequence_type == FrameSequenceTrackerType::kTouchScroll
...
constexpr int kBuiltinSequenceNum = static_cast<int>(FrameSequenceTrackerType::kMaxType) + 1;
...
int GetIndexForMetric(SmoothEffectDrivingThread thread_type, FrameSequenceTrackerType type)
{
...
int GetIndexForJankV3Metric(SmoothEffectDrivingThread thread_type, FrameSequenceTrackerType type)
{
    if (thread_type == SmoothEffectDrivingThread::kMain) {
        return static_cast<int>(type);
    }
...
{
    return base::StrCat({ "Graphics.Smoothness.Checkerboarding3.", FrameSequenceTracker::GetFrameSequenceTrackerTypeName(type) });
}

std::string GetCheckerboardingV4HistogramName(FrameSequenceTrackerType type, const char* thread_name)
{
    return base::StrCat({ "Graphics.Smoothness.Checkerboarding4.", thread_name, ".", FrameSequenceTracker::GetFrameSequenceTrackerTypeName(type) });
}

std::string GetCheckerboardingV3ThreadedHistogramName(FrameSequenceTrackerType type, const char* thread_name)
{
    return base::StrCat({ "Graphics.Smoothness.Checkerboarding3.", thread_name, ".", FrameSequenceTracker::GetFrameSequenceTrackerTypeName(type) });
}

std::string GetJankV3HistogramName(FrameSequenceTrackerType type, const char* thread_name)
{
    return base::StrCat({ "Graphics.Smoothness.Jank3.", thread_name, ".", FrameSequenceTracker::GetFrameSequenceTrackerTypeName(type) });
}

std::string GetThroughputV3HistogramName(FrameSequenceTrackerType type, const char* thread_name)
{
    return base::StrCat({ "Graphics.Smoothness.PercentDroppedFrames3.", thread_name, ".", FrameSequenceTracker::GetFrameSequenceTrackerTypeName(type) });
}

std::string GetThroughputV4HistogramName(FrameSequenceTrackerType type, const char* thread_name)
{
    return base::StrCat({ "Graphics.Smoothness.PercentDroppedFrames4.", thread_name, ".", FrameSequenceTracker::GetFrameSequenceTrackerTypeName(type) });
}
...
FrameSequenceMetrics::CustomReportData::CustomReportData(uint32_t frames_expected, uint32_t frames_dropped, uint32_t jank_count, std::vector<Jank> janks)
...
FrameSequenceMetrics::CustomReportData::CustomReportData(const CustomReportData&) = default;
FrameSequenceMetrics::CustomReportData& FrameSequenceMetrics::CustomReportData::operator=(const CustomReportData&) = default;
...
void FrameSequenceMetrics::SetScrollingThread(SmoothEffectDrivingThread scrolling_thread)
{
    DCHECK(type_ == FrameSequenceTrackerType::kTouchScroll || type_ == FrameSequenceTrackerType::kWheelScroll
...
void FrameSequenceMetrics::Merge(std::unique_ptr<FrameSequenceMetrics> metrics)
{
    ...
    if (v3_.last_begin_frame_args.frame_time < metrics->v3_.last_begin_frame_args.frame_time) {
    ...
    v4_.frames_checkerboarded_need_raster += metrics->v4_.frames_checkerboarded_need_raster;
    v4_.frames_checkerboarded_need_record += metrics->v4_.frames_checkerboarded_need_record;
...
void FrameSequenceMetrics::AdoptTrace(FrameSequenceMetrics* adopt_from)
{
    ...
    trace_data_.last_presented_sequence_number = adopt_from->trace_data_.last_presented_sequence_number;
...
void FrameSequenceMetrics::ReportMetrics()
{
    ...
    if (type_ == FrameSequenceTrackerType::kCustom) {
        ...
        std::move(custom_reporter_).Run(CustomReportData(v3_.frames_expected, v3_.frames_dropped, v3_.jank_count, std::move(v3_.janks)));
    ...
    const bool is_interaction = ShouldReportForInteraction(type(), thread_type, thread_type);
    ...
    if (v3_.frames_expected >= kMinFramesForThroughputMetric) {
        auto get_percent = [this](uint32_t frames) -> int {
            ...
            return std::ceil(100. * frames / static_cast<double>(v3_.frames_expected));
        };
        ...
        const int percent_dropped_v4 = (type() == FrameSequenceTrackerType::kCompositorRasterAnimation) ? get_percent(v4_.frames_dropped) : percent_dropped;
        const int percent_jank = get_percent(v3_.jank_count);

        // v4.
        const int percent_checkerboarded = get_percent(v4_.frames_checkerboarded);
        const int percent_checkerboarded_need_raster = get_percent(v4_.frames_checkerboarded_need_raster);
        const int percent_checkerboarded_need_record = get_percent(v4_.frames_checkerboarded_need_record);

        if (is_animation) {
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Checkerboarding3.AllAnimations", percent_missing_content);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Checkerboarding4.AllAnimations", percent_checkerboarded);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Jank3.AllAnimations", percent_jank);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.PercentDroppedFrames3.AllAnimations", percent_dropped);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.PercentDroppedFrames4.AllAnimations", percent_dropped_v4);
        }
        if (is_interaction) {
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Checkerboarding3.AllInteractions", percent_missing_content);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Checkerboarding4.AllInteractions", percent_checkerboarded);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Jank3.AllInteractions", percent_jank);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.PercentDroppedFrames3.AllInteractions", percent_dropped);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.PercentDroppedFrames4.AllInteractions", percent_dropped_v4);
        }
        if (is_animation || is_interaction) {
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Checkerboarding3.AllSequences", percent_missing_content);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Checkerboarding4.AllSequences", percent_checkerboarded);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.CheckerboardingNeedRaster4.AllSequences", percent_checkerboarded_need_raster);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.CheckerboardingNeedRecord4.AllSequences", percent_checkerboarded_need_record);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.Jank3.AllSequences", percent_jank);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.PercentDroppedFrames3.AllSequences", percent_dropped);
            UMA_HISTOGRAM_PERCENTAGE("Graphics.Smoothness.PercentDroppedFrames4.AllSequences", percent_dropped_v4);
        }

        const char* thread_name = GetThreadTypeName(thread_type);

        if (type() == FrameSequenceTrackerType::kCompositorRasterAnimation) {
            STATIC_HISTOGRAM_POINTER_GROUP(GetThroughputV4HistogramName(type(), thread_name), GetIndexForMetric(thread_type, type_), kMaximumHistogramIndex,
                Add(percent_dropped_v4),
                base::LinearHistogram::FactoryGet(
                    GetThroughputV4HistogramName(type(), thread_name), 1, 100, 101, base::HistogramBase::kUmaTargetedHistogramFlag));
            ...
            STATIC_HISTOGRAM_POINTER_GROUP(GetThroughputV3HistogramName(type(), thread_name), GetIndexForMetric(thread_type, type_), kMaximumHistogramIndex,
                Add(percent_dropped),
                base::LinearHistogram::FactoryGet(
                    GetThroughputV3HistogramName(type(), thread_name), 1, 100, 101, base::HistogramBase::kUmaTargetedHistogramFlag));
        }

        STATIC_HISTOGRAM_POINTER_GROUP(GetCheckerboardingV3HistogramName(type_), static_cast<int>(type_), static_cast<int>(FrameSequenceTrackerType::kMaxType),
            Add(percent_missing_content),
            base::LinearHistogram::FactoryGet(GetCheckerboardingV3HistogramName(type_), 1, 100, 101, base::HistogramBase::kUmaTargetedHistogramFlag));
        STATIC_HISTOGRAM_POINTER_GROUP(GetCheckerboardingV4HistogramName(type_, thread_name), GetIndexForMetric(thread_type, type_), kMaximumHistogramIndex,
            Add(percent_checkerboarded),
            base::LinearHistogram::FactoryGet(
                GetCheckerboardingV4HistogramName(type_, thread_name), 1, 100, 101, base::HistogramBase::kUmaTargetedHistogramFlag));

        if (scrolling_thread_ != SmoothEffectDrivingThread::kUnknown) {
            STATIC_HISTOGRAM_POINTER_GROUP(GetCheckerboardingV3ThreadedHistogramName(type_, thread_name), GetIndexForMetric(thread_type, type_),
                kMaximumHistogramIndex, Add(percent_missing_content),
                base::LinearHistogram::FactoryGet(
                    GetCheckerboardingV3ThreadedHistogramName(type_, thread_name), 1, 100, 101, base::HistogramBase::kUmaTargetedHistogramFlag));
        }

        STATIC_HISTOGRAM_POINTER_GROUP(GetJankV3HistogramName(type_, thread_name), GetIndexForJankV3Metric(thread_type, type_), kMaximumJankV3HistogramIndex,
            Add(percent_jank),
            base::LinearHistogram::FactoryGet(GetJankV3HistogramName(type_, thread_name), 1, 100, 101, base::HistogramBase::kUmaTargetedHistogramFlag));
        v3_.frames_expected = 0u;
        v3_.frames_dropped = 0u;
        v3_.frames_missing_content = 0u;
        v3_.no_update_count = 0u;
...
void FrameSequenceMetrics::TraceData::Terminate(const V3& v3, const V4& v4, FrameInfo::SmoothEffectDrivingThread effective_thread)
{
    if (!enabled || !trace_id) {
        return;
    }
    ...
    dict->SetInteger("checkerboarded_need_raster", v4.frames_checkerboarded_need_raster);
    dict->SetInteger("checkerboarded_need_record", v4.frames_checkerboarded_need_record);
    dict->EndDictionary();
    base::TimeTicks termination_time = v3.last_presented_frame.GetTerminationTimeForThread(effective_thread);
    ...
    if (termination_time.is_null()) {
        termination_time = v3.last_frame.GetTerminationTimeForThread(effective_thread);
        if (termination_time.is_null()) {
            termination_time = base::TimeTicks::Now();
        }
    }
    TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP1(
        "cc,benchmark", "FrameSequenceTrackerV3", TRACE_ID_LOCAL(trace_id), termination_time, "args", std::move(dict));
    trace_id = 0u;
}

void FrameSequenceMetrics::TraceData::Advance(
    base::TimeTicks start_timestamp, base::TimeTicks new_timestamp, uint32_t expected, uint32_t dropped, uint64_t sequence_number, const char* histogram_name)
{
    if (!enabled)
        return;
    if (!trace_id) {
        ...
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP1("cc,benchmark", histogram_name, TRACE_ID_LOCAL(trace_id), start_timestamp, "name",
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP0("cc,benchmark", trace_names[++this->frame_count % 3], TRACE_ID_LOCAL(trace_id), start_timestamp);
    TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP1(
        "cc,benchmark", trace_names[this->frame_count % 3], TRACE_ID_LOCAL(trace_id), new_timestamp, "data", std::move(dict));
    this->last_presented_sequence_number = sequence_number;
    this->last_timestamp = new_timestamp;
}

void FrameSequenceMetrics::AddSortedFrame(const viz::BeginFrameArgs& args, const FrameInfo& frame_info)
{
    const auto effective_thread = GetEffectiveThread();
    const auto last_presented_termination_time = v3_.last_presented_frame.GetTerminationTimeForThread(effective_thread);
    const auto termination_time = frame_info.GetTerminationTimeForThread(effective_thread);
    ...
    case SmoothEffectDrivingThread::kMain:
        if (frame_info.WasSmoothMainUpdateExpected()) {
            ...
            IncrementJankIdleTimeV3(last_presented_termination_time, termination_time);
        }
        break;
    ...
    if (should_calculate_jank_and_checkerboarding) {
        ...
        CalculateJankV3(args, frame_info, final_state, last_presented_termination_time, termination_time);
...
void FrameSequenceMetrics::CalculateJankV3(const viz::BeginFrameArgs& args, const FrameInfo& frame_info, FrameInfo::FrameFinalState final_state,
    base::TimeTicks last_presented_termination_time, base::TimeTicks termination_time)
{
    switch (final_state) {
    ...
    case FrameInfo::FrameFinalState::kPresentedPartialNewMain:
        ...
        bool will_ignore_current_frame = v3_.no_update_count >= kMaxNoUpdateFrameCount;
        ...
        // traces.
        if (!last_presented_termination_time.is_null() && termination_time > last_presented_termination_time) {
            trace_data_.Advance(last_presented_termination_time, termination_time, v3_.frames_expected, v3_.frames_dropped, frame_info.sequence_number,
                "FrameSequenceTrackerV3");
        }

        const base::TimeDelta zero_delta = base::Milliseconds(0);
        base::TimeDelta current_frame_delta
            = will_ignore_current_frame ? zero_delta : termination_time - last_presented_termination_time - v3_.no_update_duration;
        ...
        if (!v3_.last_frame_delta.is_zero() && current_frame_delta > v3_.last_frame_delta + 0.5 * args.interval) {
            ++v3_.jank_count;
            if (type_ == FrameSequenceTrackerType::kCustom) {
                // Record `last_presented_termination_time` and `current_frame_delta`
                // as the timestamp and duration of the current jank.
                v3_.janks.push_back(Jank(last_presented_termination_time, current_frame_delta));
            }
            TraceJankV3(frame_info.sequence_number, last_presented_termination_time, termination_time);
        }
        ...
        if (v3_.last_presented_frame.GetTerminationTimeForThread(GetEffectiveThread()) > frame_info.GetTerminationTimeForThread(GetEffectiveThread())) {
        } else {
...
void FrameSequenceMetrics::CalculateCheckerboarding(const FrameInfo& frame_info, FrameInfo::FrameFinalState final_state)
{
    const FrameInfo& used_frame_info = final_state == FrameInfo::FrameFinalState::kDropped ? v3_.last_presented_frame : frame_info;
    ...
    if (used_frame_info.checkerboarded_needs_raster || used_frame_info.checkerboarded_needs_record) {
        ++v4_.frames_checkerboarded;
    }
}

void FrameSequenceMetrics::IncrementJankIdleTimeV3(base::TimeTicks last_presented_termination_time, base::TimeTicks termination_time)
{
    ...
    v3_.no_update_duration += termination_time - v3_.last_frame.GetTerminationTimeForThread(GetEffectiveThread());
    ++v3_.no_update_count;
}

void FrameSequenceMetrics::TraceJankV3(uint64_t sequence_number, base::TimeTicks last_termination_time, base::TimeTicks termination_time)
{
    ...
    dict->SetInteger("last_presented_frame_sequence_number", v3_.last_presented_frame.sequence_number);
    dict->SetString("thread-type", GetThreadTypeName(GetEffectiveThread()));
    dict->SetString("tracker-type", FrameSequenceTracker::GetFrameSequenceTrackerTypeName(type_));
    dict->EndDictionary();
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN_WITH_TIMESTAMP1("cc,benchmark", "JankV3", TRACE_ID_LOCAL(this), last_termination_time, "data", std::move(dict));
    TRACE_EVENT_NESTABLE_ASYNC_END_WITH_TIMESTAMP0("cc,benchmark", "JankV3", TRACE_ID_LOCAL(this), termination_time);
}
