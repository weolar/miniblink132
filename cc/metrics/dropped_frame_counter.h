...
public:
    enum FrameState { kFrameStateDropped, kFrameStatePartial, kFrameStateComplete };
    ...
    size_t frame_history_size() const
    {
        return ring_buffer_.BufferSize();
    }
    ...
    size_t total_partial() const
    {
        return total_partial_;
    }
    size_t total_smoothness_dropped() const
    {
        return total_smoothness_dropped_;
    }

    uint32_t GetAverageThroughput() const;

    using SortedFrameCallback = base::RepeatingCallback<void(const viz::BeginFrameArgs& args, const FrameInfo&)>;
    void SetSortedFrameCallback(SortedFrameCallback callback);

    typedef base::RingBuffer<FrameState, 180> RingBufferType;
    RingBufferType::Iterator Begin() const
    {
        return ring_buffer_.Begin();
    }
    // `End()` points to the last `FrameState`, not past it.
    RingBufferType::Iterator End() const
    {
    ...
    uint32_t SlidingWindow95PercentilePercentDropped(SmoothnessStrategy strategy) const
    {
        DCHECK_GT(SmoothnessStrategy::kStrategyCount, strategy);
        return sliding_window_histogram_[strategy].GetPercentDroppedFramePercentile(0.95);
    }

    uint32_t SlidingWindowMedianPercentDropped(SmoothnessStrategy strategy) const
    {
        DCHECK_GT(SmoothnessStrategy::kStrategyCount, strategy);
        return sliding_window_histogram_[strategy].GetPercentDroppedFramePercentile(0.5);
    }

    double SlidingWindowPercentDroppedVariance(SmoothnessStrategy strategy) const
    {
        DCHECK_GT(SmoothnessStrategy::kStrategyCount, strategy);
        return sliding_window_histogram_[strategy].GetPercentDroppedFrameVariance();
    }

    const SlidingWindowHistogram* GetSlidingWindowHistogram(SmoothnessStrategy strategy) const
    {
        DCHECK_GT(SmoothnessStrategy::kStrategyCount, strategy);
...
private:
    void NotifyFrameResult(const viz::BeginFrameArgs& args, const FrameInfo& frame_info);
    ...
    uint32_t dropped_frame_count_in_window_[SmoothnessStrategy::kStrategyCount] = { 0 };
    double total_frames_in_window_ = 60.0;
    SlidingWindowHistogram sliding_window_histogram_[SmoothnessStrategy::kStrategyCount];
...
CC_EXPORT std::ostream& operator<<(std::ostream&, const DroppedFrameCounter::SlidingWindowHistogram&);
