...
const char* FrameSequenceTracker::GetFrameSequenceTrackerTypeName(FrameSequenceTrackerType type)
{
    switch (type) {
...
FrameSequenceTracker::FrameSequenceTracker(int custom_sequence_id, FrameSequenceMetrics::CustomReporter custom_reporter)
    : custom_sequence_id_(custom_sequence_id)
    , metrics_(std::make_unique<FrameSequenceMetrics>(FrameSequenceTrackerType::kCustom))
{
...
void FrameSequenceTracker::ScheduleTerminate()
{
    ...
    if (!is_inside_frame_ && last_ended_frame_id_.sequence_number <= last_sorted_frame_id_.sequence_number) {
...
void FrameSequenceTracker::ReportBeginImplFrame(const viz::BeginFrameArgs& args)
{
...
void FrameSequenceTracker::ReportFrameEnd(const viz::BeginFrameArgs& args, const viz::BeginFrameArgs& main_args)
{
    ...
    bool should_ignore_sequence = args.frame_id.sequence_number != begin_impl_frame_data_.previous_sequence;
    ResetAllStateIfPaused();
...
bool FrameSequenceTracker::ShouldIgnoreBeginFrameSource(uint64_t source_id) const
{
...
bool FrameSequenceTracker::ShouldReportMetricsNow(const viz::BeginFrameArgs& args) const
{
    return metrics_->HasEnoughDataForReporting() && first_begin_frame_args_.IsValid()
        && args.frame_time - first_begin_frame_args_.frame_time >= time_delta_to_report_;
}
...
void FrameSequenceTracker::AddSortedFrame(const viz::BeginFrameArgs& args, const FrameInfo& frame_info)
{
    ...
    // sequence.
    if (!first_begin_frame_args_.IsValid() || args.frame_id.sequence_number < first_begin_frame_args_.frame_id.sequence_number) {
        return;
    }
    ...
    if (termination_status_ == TerminationStatus::kScheduledForTermination) {
        if (!last_ended_frame_id_.IsSequenceValid() || last_ended_frame_id_.source_id != args.frame_id.source_id) {
        ...
        if (frame_info.final_state != FrameInfo::FrameFinalState::kDropped) {
            if (last_ended_frame_id_.sequence_number < args.frame_id.sequence_number) {
                termination_status_ = TerminationStatus::kReadyForTermination;
                return;
            } else if (last_ended_frame_id_.sequence_number == args.frame_id.sequence_number) {
                // We still report the final `sequence_number`, but need to mark for
                // termination.
                termination_status_ = TerminationStatus::kReadyForTermination;
            }
        } else if (!base::FeatureList::IsEnabled(::features::kMetricsBackfillAdjustmentHoldback)
    ...
    if (base::FeatureList::IsEnabled(::features::kMetricsBackfillAdjustmentHoldback) && frame_info.final_state == FrameInfo::FrameFinalState::kDropped
        && last_ended_frame_id_.sequence_number < args.frame_id.sequence_number) {
        return;
    }
    if (metrics_)
