...
public:
    ...
    AverageLagTrackingManager& operator=(AverageLagTrackingManager const&) = delete;

    // Adds all the eligible events in the collection |infos| to the |frame_token|
    // wait list.
    void CollectScrollEventsFromFrame(uint32_t frame_token, const EventMetricsSet& events_metrics);

    // Sends all pending events in the |frame_token| list to the
    // AverageLagTracker, given its |frame_details|.
    void DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& frame_details);
...
private:
    ...
    base::circular_deque<std::pair<uint32_t, std::vector<AverageLagTracker::EventInfo>>> frame_token_to_info_;
};
