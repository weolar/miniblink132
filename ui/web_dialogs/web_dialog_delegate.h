...
public:
    ...
    void set_dialog_name(std::string dialog_name)
    {
        name_ = dialog_name;
    }
    ...
    void set_dialog_content_url(GURL content_url)
    {
    ...
    virtual void GetWebUIMessageHandlers(std::vector<content::WebUIMessageHandler*>* handlers);
    void AddWebUIMessageHandler(std::unique_ptr<content::WebUIMessageHandler> handler);
    ...
    void set_dialog_size(gfx::Size size)
    {
        size_ = size;
    }
    ...
    void set_dialog_args(std::string dialog_args)
    {
        args_ = dialog_args;
    }
    ...
    void set_web_view_element_id(const ui::ElementIdentifier web_view_element_id)
    {
    ...
    virtual void OnCloseContents(content::WebContents* source, bool* out_close_dialog);
    ...
    virtual bool HandleContextMenu(content::RenderFrameHost& render_frame_host, const content::ContextMenuParams& params);
    ...
    // any reason.
    virtual bool HandleOpenURLFromTab(content::WebContents* source, const content::OpenURLParams& params,
        base::OnceCallback<void(content::NavigationHandle&)> navigation_handle_callback, content::WebContents** out_new_contents);
    ...
    using AcceleratorHandler = base::RepeatingCallback<bool(WebDialogDelegate&, const Accelerator&)>;
    ...
    void set_dialog_frame_kind(FrameKind frame_kind)
    {
...
private:
    ...
    std::vector<std::unique_ptr<content::WebUIMessageHandler>> added_message_handlers_;
};
