...
public:
    explicit TestMessageHandler(bool* destroyed)
        : destroyed_(destroyed)
    {
    }
    ~TestMessageHandler() override
    {
        *destroyed_ = true;
    }
...
TEST(WebDialogDelegateTest, MessageHandlerOwnershipIsPassed)
{
    ...
    delegate.AddWebUIMessageHandler(std::make_unique<TestMessageHandler>(&destroyed_1));
    delegate.AddWebUIMessageHandler(std::make_unique<TestMessageHandler>(&destroyed_2));
...
TEST(WebDialogDelegateTest, CallbackOnClose)
{
    ...
    delegate->RegisterOnDialogClosedCallback(base::BindLambdaForTesting([&](const std::string&) { called = true; }));
...
public:
    explicit DeletionTestWebDialogDelegate(bool* deleted)
        : deleted_(deleted)
    {
    }
    ~DeletionTestWebDialogDelegate() override
    {
        *deleted_ = true;
    }
...
TEST(WebDialogDelegateTest, AcceleratorsAreHandled)
{
    ...
    delegate->RegisterAccelerator(
        accelerator, base::BindLambdaForTesting([&](WebDialogDelegate& provided_delegate, const ui::Accelerator& provided_accelerator) {
