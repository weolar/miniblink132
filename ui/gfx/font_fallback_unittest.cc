...
constexpr BaseFontTestOption styled_font = { nullptr, 1, Font::FontStyle::ITALIC, Font::Weight::BOLD };
constexpr BaseFontTestOption sans_font = { "sans", 2 };

using FallbackFontTestParamInfo = std::tuple<FallbackFontTestCase, FallbackFontTestOption, BaseFontTestOption>;

class GetFallbackFontTest : public ::testing::TestWithParam<FallbackFontTestParamInfo> {
public:
    ...
    static std::string ParamInfoToString(::testing::TestParamInfo<FallbackFontTestParamInfo> param_info)
    {
        ...
        if (base_font_option.delta || base_font_option.style || base_font_option.style) {
            font_option += base::StringPrintf("_d%ds%dw%d", base_font_option.delta, base_font_option.style, static_cast<int>(base_font_option.weight));
        }

        std::string language_tag = test_case.language_tag;
        base::RemoveChars(language_tag, "-", &language_tag);
        return std::string("S") + uscript_getName(test_case.script) + "L" + language_tag + font_option;
    }
...
protected:
    bool GetFallbackFont(const Font& font, const std::string& language_tag, Font* result)
    {
        return gfx::GetFallbackFont(font, language_tag, test_case_.text, result);
    }

    bool EnsuresScriptSupportCodePoints(const std::u16string& text, UScriptCode script, const std::string& script_name)
    {
        size_t i = 0;
        while (i < text.length()) {
            UChar32 code_point;
            ...
            if (!uscript_hasScript(code_point, script)) {
                ...
                UScriptCode codepoint_script = uscript_getScript(code_point, &script_error);

                ADD_FAILURE() << "CodePoint U+" << std::hex << code_point << " is not part of the script '" << script_name << "'. Script '"
                              << uscript_getName(codepoint_script) << "' detected.";
                return false;
            }
        }
        return true;
...
TEST_P(GetFallbackFontTest, GetFallbackFont)
{
    ...
    if (base_font_option_.delta != 0 || base_font_option_.style != 0 || base_font_option_.weight != gfx::Font::Weight::NORMAL) {
        base_font = base_font.Derive(base_font_option_.delta, base_font_option_.style, base_font_option_.weight);
    }
    ...
    if (!test_option_.skip_code_point_validation && !EnsuresScriptSupportCodePoints(test_case_.text, test_case_.script, script_name_)) {
        return;
    }
    ...
    Font fallback_font;
    bool result = GetFallbackFont(base_font, test_case_.language_tag, &fallback_font);
    if (!result) {
    ...
    if (!test_option_.skip_fallback_fonts_validation) {
        if (!base::Contains(test_case_.fallback_fonts, fallback_font.GetFontName())) {
            ADD_FAILURE() << "GetFallbackFont failed for '" << script_name_ << "' invalid fallback font: " << fallback_font.GetFontName()
                          << " not among valid options: " << base::JoinString(test_case_.fallback_fonts, ", ");
            return;
        }
    }
    ...
    if (!DoesFontSupportCodePoints(fallback_font, test_case_.text)) {
        ADD_FAILURE() << "Font '" << fallback_font.GetFontName() << "' does not matched every CodePoints.";
        return;
    }
}
...
std::vector<FallbackFontTestCase> GetSampleFontTestCases()
{
    ...
    for (unsigned int i = 0; i < script_max; i++) {
        ...
        int text_length = uscript_getSampleString(script, text, std::size(text), &errorCode);
...
INSTANTIATE_TEST_SUITE_P(KnownExpectedFonts, GetFallbackFontTest,
    testing::Combine(
        testing::ValuesIn(kGetFontFallbackTests), testing::Values(default_fallback_option), testing::Values(default_base_font, styled_font, sans_font)),
...
INSTANTIATE_TEST_SUITE_P(Glyphs, GetFallbackFontTest,
    testing::Combine(testing::ValuesIn(GetSampleFontTestCases()), testing::Values(untested_fallback_option), testing::Values(default_base_font)),
    GetFallbackFontTest::ParamInfoToString);
