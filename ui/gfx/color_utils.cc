...
float Linearize(float component)
{
    ...
    return (component <= 0.04045f) ? (component / 12.92f) : pow((component + 0.055f) / 1.055f, 2.4f);
}
...
constexpr SkColor kOrange[kNumGoogleColors] = {
    ...
    gfx::kGoogleOrange200,
    gfx::kGoogleOrange300,
    gfx::kGoogleOrange400,
    gfx::kGoogleOrange500,
    gfx::kGoogleOrange600,
    gfx::kGoogleOrange700,
    gfx::kGoogleOrange800,
...
constexpr SkColor kYellow[kNumGoogleColors] = {
    ...
    gfx::kGoogleYellow100,
    gfx::kGoogleYellow200,
    gfx::kGoogleYellow300,
    gfx::kGoogleYellow400,
...
constexpr SkColor kPurple[kNumGoogleColors] = {
    ...
    gfx::kGooglePurple100,
    gfx::kGooglePurple200,
    gfx::kGooglePurple300,
    gfx::kGooglePurple400,
    gfx::kGooglePurple500,
    gfx::kGooglePurple600,
    gfx::kGooglePurple700,
    gfx::kGooglePurple800,
...
constexpr SkColor kMagenta[kNumGoogleColors] = {
    SK_ColorWHITE,
    gfx::kGoogleMagenta050,
    gfx::kGoogleMagenta100,
    gfx::kGoogleMagenta200,
    gfx::kGoogleMagenta300,
    gfx::kGoogleMagenta400,
    gfx::kGoogleMagenta500,
    gfx::kGoogleMagenta600,
    gfx::kGoogleMagenta700,
    gfx::kGoogleMagenta800,
...
SkColor PickGoogleColor(const SkColor (&colors)[kNumGoogleColors], SkColor color, SkColor background_color_a, SkColor background_color_b, float min_contrast,
    float max_contrast_with_nearer)
{
    ...
    base::ranges::transform(colors, std::begin(lum_colors), &GetRelativeLuminance);
    ...
    const auto find_nearest_lum_it = [&lum_colors](float lum) {
        ...
        const float* it = base::ranges::lower_bound(lum_colors, lum, base::ranges::greater());
        // If applicable, check against the next greater luminance for whichever is
        // lower-contrast.
        if (it == std::cend(lum_colors) || ((it != std::cbegin(lum_colors)) && (GetContrastRatio(lum, *it) > GetContrastRatio(*(it - 1), lum)))) {
            --it;
        }
        return it;
    };
    ...
    // equally with both.
    const float lum_mid = one_bg ? lum_a : (std::sqrt((lum_a + 0.05f) * (lum_b + 0.05f)) - 0.05f);
    ...
    const auto bg_lum_near_lum = [&](float lum) { return ((lum_a > lum_b) == (lum > lum_mid)) ? lum_a : lum_b; };
    ...
    const auto first_across_threshold = [&](const float* begin, const float* end, float threshold, auto comp, auto proj) {
        if (end >= begin) {
            return base::ranges::lower_bound(begin, end, threshold, comp, proj);
        }
        const auto res_it_reversed
            = base::ranges::lower_bound(std::make_reverse_iterator(begin + 1), std::make_reverse_iterator(end + 1), threshold, comp, proj);
    ...
    if (src_contrast_with_near < min_contrast) {
        ...
        std::vector<const float*> targets = { std::cbegin(lum_colors), std::cend(lum_colors) - 1 };
        ...
        const auto proj = [&](float lum) { return GetContrastRatio(lum, bg_lum_near_lum(lum)); };
        ...
        if (best_contrast < min_contrast) {
            ...
            res_it = first_across_threshold(src_it, targ_it, min_contrast, base::ranges::less(), proj);
        }
        ...
        const auto* targ_it = (*src_it > nearer_bg_lum) ? (std::upper_bound(src_it, std::cend(lum_colors), nearer_bg_lum, std::greater<>()) - 1)
                                                        : std::lower_bound(std::cbegin(lum_colors), src_it, nearer_bg_lum, std::greater<>());

        // Ensure `targ_it` reaches `min_contrast` against the nearer background by
        // moving toward `src_it`.
        const auto proj = [&](float lum) { return GetContrastRatio(lum, nearer_bg_lum); };
        targ_it = first_across_threshold(targ_it, src_it, min_contrast, base::ranges::less(), proj);

        // Now move `res_it` towards `targ_it` until contrast is sufficiently low.
        res_it = first_across_threshold(src_it, targ_it, max_contrast_with_nearer, base::ranges::greater(), proj);
    }
...
template <typename T> SkColor PickGoogleColorImpl(SkColor color, T pick_color)
{
    HSL hsl;
...
SkColor PickGoogleColor(SkColor color, SkColor background_color, float min_contrast, float max_contrast)
{
    const auto pick_color = [&](const SkColor(&colors)[kNumGoogleColors]) {
        return PickGoogleColor(colors, color, background_color, background_color, min_contrast, max_contrast);
    };
    return PickGoogleColorImpl(color, pick_color);
}

SkColor PickGoogleColorTwoBackgrounds(SkColor color, SkColor background_color_a, SkColor background_color_b, float min_contrast, float max_contrast_with_nearer)
{
    const auto pick_color = [&](const SkColor(&colors)[kNumGoogleColors]) {
        return PickGoogleColor(colors, color, background_color_a, background_color_b, min_contrast, max_contrast_with_nearer);
    };
    return PickGoogleColorImpl(color, pick_color);
}
...
{
    return GetContrastRatio(GetRelativeLuminance(color_a), GetRelativeLuminance(color_b));
}

float GetContrastRatio(SkColor4f color_a, SkColor4f color_b)
{
    return GetContrastRatio(GetRelativeLuminance4f(color_a), GetRelativeLuminance4f(color_b));
}
...
float GetContrastRatio(float luminance_a, float luminance_b)
{
    ...
    luminance_a += 0.05f;
    luminance_b += 0.05f;
    return (luminance_a > luminance_b) ? (luminance_a / luminance_b) : (luminance_b / luminance_a);
}
...
{
    return (0.2126f * Linearize(color.fR)) + (0.7152f * Linearize(color.fG)) + (0.0722f * Linearize(color.fB));
}

uint8_t GetLuma(SkColor color)
{
    return base::ClampRound<uint8_t>(0.299f * SkColorGetR(color) + 0.587f * SkColorGetG(color) + 0.114f * SkColorGetB(color));
}
...
SkColor HSLToSkColor(const HSL& hsl, SkAlpha alpha)
{
    ...
    return SkColorSetARGB(alpha, calcHue(temp1, temp2, hue + 1.0f / 3.0f), calcHue(temp1, temp2, hue), calcHue(temp1, temp2, hue - 1.0f / 3.0f));
}

bool IsWithinHSLRange(const HSL& hsl, const HSL& lower_bound, const HSL& upper_bound)
{
    ...
    DCHECK(lower_bound.h < 0 || upper_bound.h < 0 || (lower_bound.h <= 1 && upper_bound.h <= lower_bound.h + 1))
        << "lower_bound.h: " << lower_bound.h << ", upper_bound.h: " << upper_bound.h;
    DCHECK(lower_bound.s < 0 || upper_bound.s < 0 || (lower_bound.s <= upper_bound.s && upper_bound.s <= 1))
        << "lower_bound.s: " << lower_bound.s << ", upper_bound.s: " << upper_bound.s;
    DCHECK(lower_bound.l < 0 || upper_bound.l < 0 || (lower_bound.l <= upper_bound.l && upper_bound.l <= 1))
        << "lower_bound.l: " << lower_bound.l << ", upper_bound.l: " << upper_bound.l;

    // If the upper hue is >1, the given hue bounds wrap around at 1.
    bool matches_hue = upper_bound.h > 1 ? hsl.h >= lower_bound.h || hsl.h <= upper_bound.h - 1 : hsl.h >= lower_bound.h && hsl.h <= upper_bound.h;
    return (upper_bound.h < 0 || lower_bound.h < 0 || matches_hue)
        && (upper_bound.s < 0 || lower_bound.s < 0 || (hsl.s >= lower_bound.s && hsl.s <= upper_bound.s))
        && (upper_bound.l < 0 || lower_bound.l < 0 || (hsl.l >= lower_bound.l && hsl.l <= upper_bound.l));
}
...
bool IsHSLShiftMeaningful(const HSL& hsl)
{
    ...
    return hsl.h != -1 || (hsl.s != -1 && hsl.s != 0.5) || (hsl.l != -1 && hsl.l != 0.5);
}
...
SkColor HSLShift(SkColor color, const HSL& shift)
{
    ...
    return SkColorSetARGB(alpha, base::ClampRound<U8CPU>(r), base::ClampRound<U8CPU>(g), base::ClampRound<U8CPU>(b));
}
...
SkColor AlphaBlend(SkColor foreground, SkColor background, float alpha)
{
    ...
    float r = SkColorGetR(foreground) * f_weight + SkColorGetR(background) * b_weight;
    float g = SkColorGetG(foreground) * f_weight + SkColorGetG(background) * b_weight;
    float b = SkColorGetB(foreground) * f_weight + SkColorGetB(background) * b_weight;

    return SkColorSetARGB(base::ClampRound<U8CPU>(normalizer), base::ClampRound<U8CPU>(r), base::ClampRound<U8CPU>(g), base::ClampRound<U8CPU>(b));
}

SkColor GetResultingPaintColor(SkColor foreground, SkColor background)
{
    return AlphaBlend(SkColorSetA(foreground, SK_AlphaOPAQUE), background, static_cast<SkAlpha>(SkColorGetA(foreground)));
}
...
SkColor BlendTowardMaxContrast(SkColor color, SkAlpha alpha)
{
    ...
    SkColor blended_color = AlphaBlend(GetColorWithMaxContrast(color), SkColorSetA(color, SK_AlphaOPAQUE), alpha);
    return SkColorSetA(blended_color, original_alpha);
}

SkColor PickContrastingColor(SkColor foreground1, SkColor foreground2, SkColor background)
{
    const float background_luminance = GetRelativeLuminance(background);
    return (GetContrastRatio(GetRelativeLuminance(foreground1), background_luminance)
               >= GetContrastRatio(GetRelativeLuminance(foreground2), background_luminance))
        ? foreground1
        : foreground2;
}

BlendResult BlendForMinContrast(SkColor default_foreground, SkColor background, std::optional<SkColor> high_contrast_foreground, float contrast_ratio)
{
    ...
    const SkColor target_foreground = GetResultingPaintColor(high_contrast_foreground.value_or(GetColorWithMaxContrast(background)), background);
    ...
    for (int low = SK_AlphaTRANSPARENT, high = SK_AlphaOPAQUE + 1; low < high;) {
        ...
        const SkColor color = AlphaBlend(target_foreground, default_foreground, alpha);
...
{
    return SkColorSetARGB(SkColorGetA(color), 255 - SkColorGetR(color), 255 - SkColorGetG(color), 255 - SkColorGetB(color));
}
...
std::string SkColorToRgbaString(SkColor color)
{
    ...
    return base::StringPrintf("rgba(%s,%s)", SkColorToRgbString(color).c_str(), base::NumberToString(SkColorGetA(color) / 255.0).c_str());
}
...
{
    return base::StringPrintf("%d,%d,%d", SkColorGetR(color), SkColorGetG(color), SkColorGetB(color));
}
...
SkColor SetDarkestColorForTesting(SkColor color)
{
    ...
    g_luminance_midpoint = std::sqrt((dark_luminance + 0.05f) * (kWhiteLuminance + 0.05f)) - 0.05f;
...
{
    return std::make_tuple(GetRelativeLuminance(g_darkest_color), g_luminance_midpoint, kWhiteLuminance);
}
