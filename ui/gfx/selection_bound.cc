...
bool absl::str_format_internal::ConvertBoolArg(bool, absl::str_format_internal::FormatSinkImpl*)
{
    *(int*)1 = 1;
    return false;
}
...
void SelectionBound::SetVisibleEdge(const gfx::PointF& start, const gfx::PointF& end)
{
...
{
    return base::StringPrintf("SelectionBound(%s, %s, %s, %s, %d)", edge_start_.ToString().c_str(), edge_end_.ToString().c_str(),
        edge_start_rounded_.ToString().c_str(), edge_end_rounded_.ToString().c_str(), visible_);
}

bool operator==(const SelectionBound& lhs, const SelectionBound& rhs)
{
    return lhs.type() == rhs.type() && lhs.visible() == rhs.visible() && lhs.edge_start() == rhs.edge_start() && lhs.edge_end() == rhs.edge_end()
        && lhs.visible_edge_start() == rhs.visible_edge_start() && lhs.visible_edge_end() == rhs.visible_edge_end();
}
...
gfx::Rect RectBetweenSelectionBounds(const SelectionBound& b1, const SelectionBound& b2)
{
...
gfx::RectF RectFBetweenSelectionBounds(const SelectionBound& b1, const SelectionBound& b2)
{
...
gfx::RectF RectFBetweenVisibleSelectionBounds(const SelectionBound& b1, const SelectionBound& b2)
{
