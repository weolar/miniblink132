...
void RetrieveCodepointsAndDecomposedCodepoints(std::u16string_view text, std::set<UChar32>* codepoints)
{
    size_t offset = 0;
...
size_t ComputeMissingGlyphsForGivenTypeface(std::u16string_view text, sk_sp<SkTypeface> typeface)
{
    ...
    while (i < text.length()) {
        ...
        // codepoints instead.
        icu::UnicodeString decomposed_text;
        if (UnicodeDecomposeCodepoint(codepoint, &decomposed_text) && !decomposed_text.isEmpty()) {
...
sk_sp<SkTypeface> GetSkiaFallbackTypeface(const Font& template_font, const std::string& locale, std::u16string_view text)
{
    if (text.empty())
        return nullptr;
    ...
    const int font_weight
        = (template_font.GetWeight() == Font::Weight::INVALID) ? static_cast<int>(Font::Weight::NORMAL) : static_cast<int>(template_font.GetWeight());
    const bool italic = (template_font.GetStyle() & Font::ITALIC) != 0;
    SkFontStyle skia_style(font_weight, SkFontStyle::kNormal_Width, italic ? SkFontStyle::kItalic_Slant : SkFontStyle::kUpright_Slant);
    ...
    for (UChar32 codepoint : codepoints) {
        sk_sp<SkTypeface> typeface(font_mgr->matchFamilyStyleCharacter(template_font.GetFontName().c_str(), skia_style, locales, num_locales, codepoint));
        ...
        size_t missing_glyphs = ComputeMissingGlyphsForGivenTypeface(text, typeface);
