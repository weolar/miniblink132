...
base::LazyInstance<std::string>::Leaky g_default_font_description = LAZY_INSTANCE_INITIALIZER;

// The default instance of gfx::FontListImpl.
base::LazyInstance<scoped_refptr<gfx::FontListImpl>>::Leaky g_default_impl = LAZY_INSTANCE_INITIALIZER;
bool g_default_impl_initialized = false;

#if !BUILDFLAG(IS_MAC)
bool IsFontFamilyAvailable(const std::string& family, SkFontMgr* font_manager)
{
    return !!sk_sp<SkTypeface>(font_manager->matchFamilyStyle(family.c_str(), SkFontStyle()));
}
#endif
...
// static
bool FontList::ParseDescription(
    const std::string& description, std::vector<std::string>* families_out, int* style_out, int* size_pixels_out, Font::Weight* weight_out)
{
    DCHECK(families_out);
    DCHECK(style_out);
    ...
    *families_out = base::SplitString(description, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    std::vector<std::string> styles = base::SplitString(families_out->back(), base::kWhitespaceASCII, base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    if (!base::StringToInt(size_string, size_pixels_out) || *size_pixels_out <= 0)
        return false;
...
FontList::FontList(const std::vector<std::string>& font_names, int font_style, int font_size, Font::Weight font_weight)
...
FontList FontList::Derive(int size_delta, int font_style, Font::Weight weight) const
{
...
FontList FontList::DeriveWithHeightUpperBound(int height) const
{
    ...
    for (int font_size = font_list.GetFontSize(); font_size > 1; --font_size) {
        const int internal_leading = font_list.GetBaseline() - font_list.GetCapHeight();
        ...
        const int space = height - ((internal_leading != 0) ? font_list.GetCapHeight() : font_list.GetHeight());
...
const scoped_refptr<FontListImpl>& FontList::GetDefaultImpl()
{
    ...
    DCHECK(g_default_font_description.IsCreated()) << "SetDefaultFontDescription has not been called.";

    if (!g_default_impl_initialized) {
        g_default_impl.Get() = g_default_font_description.Get().empty() ? new FontListImpl(Font()) : new FontListImpl(g_default_font_description.Get());
...
{
    std::vector<std::string> families = base::SplitString(font_name_list, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    if (families.empty())
        return std::string();
    if (families.size() == 1)
        return families[0];
