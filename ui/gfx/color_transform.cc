...
BASE_FEATURE(kHlgPqUnifiedTonemap, "HlgPqUnifiedTonemap", base::FEATURE_ENABLED_BY_DEFAULT);

BASE_FEATURE(kHlgPqSdrRelative, "HlgPqSdrRelative", base::FEATURE_ENABLED_BY_DEFAULT);
...
public:
    ...
    virtual ColorTransformFromLinear* GetFromLinear()
    {
        return nullptr;
    }
    virtual ColorTransformSkTransferFn* GetSkTransferFn()
    {
        return nullptr;
    }
    virtual ColorTransformMatrix* GetMatrix()
    {
        return nullptr;
    }
    virtual ColorTransformNull* GetNull()
    {
        return nullptr;
    }
    ...
    virtual bool Join(ColorTransformStep* next)
    {
        return false;
    }
    ...
    virtual void Transform(ColorTransform::TriStim* color, size_t num, const ColorTransform::RuntimeOptions& options) const = 0;
    virtual void AppendSkShaderSource(std::stringstream* src) const = 0;
    virtual void SetShaderUniforms(const ColorTransform::RuntimeOptions& options, SkShaderUniforms* uniforms) const
    {
    }
};
...
public:
    ColorTransformInternal(const ColorSpace& src, const ColorSpace& dst, const Options& options);
    ~ColorTransformInternal() override;

    gfx::ColorSpace GetSrcColorSpace() const override
    {
        return src_;
    }
    gfx::ColorSpace GetDstColorSpace() const override
    {
        return dst_;
    }
    ...
    void Transform(TriStim* colors, size_t num, const ColorTransform::RuntimeOptions& options) const override
    {
        for (const auto& step : steps_) {
            step->Transform(colors, num, options);
        }
    ...
    sk_sp<SkData> GetSkShaderUniforms(const RuntimeOptions& options) const override;
    bool IsIdentity() const override
    {
        return steps_.empty();
    }
    size_t NumberOfStepsForTesting() const override
    {
        return steps_.size();
    }

private:
    void AppendColorSpaceToColorSpaceTransform(const ColorSpace& src, const ColorSpace& dst, const Options& options);
    void Simplify();
...
public:
    ColorTransformNull* GetNull() override
    {
        return this;
    }
    ...
    void Transform(ColorTransform::TriStim* color, size_t num, const ColorTransform::RuntimeOptions& options) const override
    {
    }
...
public:
    explicit ColorTransformMatrix(const SkM44& matrix)
        : matrix_(matrix)
    {
    }
    ColorTransformMatrix* GetMatrix() override
    {
        return this;
    }
    ...
    bool IsNull() override
    {
        return SkM44IsApproximatelyIdentity(matrix_);
    }

    void Transform(ColorTransform::TriStim* colors, size_t num, const ColorTransform::RuntimeOptions& options) const override
    {
    ...
    {
        *src << "  color = half4x4(";
        *src << matrix_.rc(0, 0) << ", " << matrix_.rc(1, 0) << ", " << matrix_.rc(2, 0) << ", 0,";
        *src << endl;
        *src << "               ";
        *src << matrix_.rc(0, 1) << ", " << matrix_.rc(1, 1) << ", " << matrix_.rc(2, 1) << ", 0,";
        *src << endl;
        *src << "               ";
        *src << matrix_.rc(0, 2) << ", " << matrix_.rc(1, 2) << ", " << matrix_.rc(2, 2) << ", 0,";
        *src << endl;
        *src << "0, 0, 0, 1)";
        *src << " * color;" << endl;

        // Only print the translational component if it isn't the identity.
        if (matrix_.rc(0, 3) != 0.f || matrix_.rc(1, 3) != 0.f || matrix_.rc(2, 3) != 0.f) {
            *src << "  color += half4(";
            *src << matrix_.rc(0, 3) << ", " << matrix_.rc(1, 3) << ", " << matrix_.rc(2, 3);
            *src << ", 0);" << endl;
        }
    }
...
public:
    ...
    virtual void AppendTransferShaderSource(std::stringstream* src, bool is_glsl) const = 0;
...
public:
    static void GetParams(const gfx::ColorSpace color_space, skcms_TransferFunction* fn, float* p, float* q, float* r)
    {
        float sdr_joint = 1;
        float hdr_level = 1;
    ...
    static void InvertParams(skcms_TransferFunction* fn, float* p, float* q, float* r)
    {
    ...
    ColorTransformPiecewiseHDR(const skcms_TransferFunction fn, float p, float q, float r)
    ...
    void AppendTransferShaderSource(std::stringstream* result, bool is_glsl) const override
    {
        *result << "  if (v < 0.0) {\n";
        *result << "    v = 0.0;\n";
        *result << "  } else if (v < " << Str(fn_.d) << ") {\n";
        *result << "    v = " << Str(fn_.c) << " * v + " << Str(fn_.f) << ";" << endl;
...
public:
    explicit ColorTransformSkTransferFn(const skcms_TransferFunction& fn, bool extended)
        : ColorTransformPerChannelTransferFn(extended)
        , fn_(fn)
    {
    }
    // ColorTransformStep implementation.
    ColorTransformSkTransferFn* GetSkTransferFn() override
    {
        return this;
    }
    bool Join(ColorTransformStep* next_untyped) override
    ...
    bool IsNull() override
    {
        return SkTransferFnIsApproximatelyIdentity(fn_);
    }
...
public:
    ...
    void AppendTransferShaderSource(std::stringstream* src, bool is_glsl) const override
    {
...
public:
    ...
    ColorTransformFromLinear* GetFromLinear() override
    {
        return this;
    }
    bool IsNull() override
    {
        return transfer_ == ColorSpace::TransferID::LINEAR;
    }

    // ColorTransformPerChannelTransferFn implementation:
    float Evaluate(float v) const override
    {
        return FromLinear(transfer_, v);
    }
    void AppendTransferShaderSource(std::stringstream* src, bool is_glsl) const override
    {
        std::string scalar_type = is_glsl ? "float" : "half";
...
public:
    ...
    {
        *src << "{\n"
             << "  half4 luma_vec = half4(" << kLr << ", " << kLg << ", " << kLb << ", 0.0);\n"
    ...
    void SetShaderUniforms(const ColorTransform::RuntimeOptions& options, SkShaderUniforms* uniforms) const override
    {
        uniforms->hlg_dst_max_luminance_relative = options.dst_max_luminance_relative;
        ComputeHLGToneMapConstants(options, uniforms->hlg_ootf_gamma_minus_one);
    }

private:
    static void ComputeHLGToneMapConstants(const gfx::ColorTransform::RuntimeOptions& options, float& gamma_minus_one)
    {
        const float dst_max_luminance_nits = options.dst_sdr_max_luminance_nits * options.dst_max_luminance_relative;
        gamma_minus_one = 1.2f + 0.42f * logf(dst_max_luminance_nits / kHLGRefMaxLumNits) / logf(10.f) - 1.f;
    }
};
...
public:
    explicit ColorTransformToneMapInRec2020Linear(const gfx::ColorSpace& src)
        : use_ref_max_luminance_(src.GetTransferID() == ColorSpace::TransferID::HLG)
    {
    }
    ...
    void SetShaderUniforms(const ColorTransform::RuntimeOptions& options, SkShaderUniforms* uniforms) const override
    {
        ComputeToneMapConstants(options, uniforms->pq_tonemap_a, uniforms->pq_tonemap_b);
    }

private:
    float ComputeSrcMaxLumRelative(const ColorTransform::RuntimeOptions& options) const
    {
        float src_max_lum_nits = kHLGRefMaxLumNits;
        if (!use_ref_max_luminance_) {
            const auto hdr_metadata = gfx::HDRMetadata::PopulateUnspecifiedWithDefaults(options.src_hdr_metadata);
            src_max_lum_nits = (hdr_metadata.cta_861_3 && hdr_metadata.cta_861_3->max_content_light_level > 0) ? hdr_metadata.cta_861_3->max_content_light_level
    ...
    void ComputeToneMapConstants(const gfx::ColorTransform::RuntimeOptions& options, float& a, float& b) const
    {
        ...
        if (src_max_lum_relative > options.dst_max_luminance_relative) {
            a = options.dst_max_luminance_relative / (src_max_lum_relative * src_max_lum_relative);
...
public:
    ColorTransformSrcNitsToSdrRelative(float unity_nits, bool use_src_sdr_white)
        ...
        , use_src_sdr_white_(use_src_sdr_white)
    {
    }
    ...
    void SetShaderUniforms(const ColorTransform::RuntimeOptions& options, SkShaderUniforms* uniforms) const override
    {
        uniforms->nits_to_sdr_relative_factor = ComputeNitsToSdrRelativeFactor(options);
    }

private:
    float ComputeNitsToSdrRelativeFactor(const ColorTransform::RuntimeOptions& options) const
    {
        float sdr_white_nits = options.dst_sdr_max_luminance_nits;
...
public:
    ...
    void SetShaderUniforms(const ColorTransform::RuntimeOptions& options, SkShaderUniforms* uniforms) const override
    {
        uniforms->sdr_relative_to_nits_factor = ComputeSdrRelativeToNitsFactor(options);
    }

private:
    float ComputeSdrRelativeToNitsFactor(const ColorTransform::RuntimeOptions& options) const
    {
...
void ColorTransformInternal::AppendColorSpaceToColorSpaceTransform(const ColorSpace& src, const ColorSpace& dst, const Options& options)
{
    ...
    const bool src_matrix_is_identity_or_ycgco = src.GetMatrixID() == ColorSpace::MatrixID::GBR || src.GetMatrixID() == ColorSpace::MatrixID::YCOCG;
    auto src_range_adjust_matrix = std::make_unique<ColorTransformMatrix>(src.GetRangeAdjustMatrix(options.src_bit_depth));

    if (!src_matrix_is_identity_or_ycgco)
        steps_.push_back(std::move(src_range_adjust_matrix));

    steps_.push_back(std::make_unique<ColorTransformMatrix>(Invert(src.GetTransferMatrix(options.src_bit_depth))));
    ...
    case ColorSpace::TransferID::SCRGB_LINEAR_80_NITS:
        steps_.push_back(std::make_unique<ColorTransformSrcNitsToSdrRelative>(80.f, /*use_src_sdr_white=*/false));
        break;
    ...
    default: {
        ...
        if (src.GetTransferFunction(&src_to_linear_fn)) {
            steps_.push_back(std::make_unique<ColorTransformSkTransferFn>(src_to_linear_fn, src.HasExtendedSkTransferFn()));
        } else {
            steps_.push_back(std::make_unique<ColorTransformToLinear>(src.GetTransferID()));
        }
    }
    }

    steps_.push_back(std::make_unique<ColorTransformMatrix>(src.GetPrimaryMatrix()));

    // Perform tone mapping in a linear space
    const ColorSpace rec2020_linear(ColorSpace::PrimaryID::BT2020, ColorSpace::TransferID::LINEAR, ColorSpace::MatrixID::RGB, ColorSpace::RangeID::FULL);
    switch (src.GetTransferID()) {
    case ColorSpace::TransferID::HLG: {
        if (IsHlgPqUnifiedTonemapEnabled()) {
            // Convert from XYZ to Rec2020 primaries.
            steps_.push_back(std::make_unique<ColorTransformMatrix>(Invert(rec2020_linear.GetPrimaryMatrix())));
            ...
            steps_.push_back(std::make_unique<ColorTransformSrcNitsToSdrRelative>(kHLGRefMaxLumNits, IsHlgPqSdrRelative()));

            // If tone mapping is requested, tone map down to the available
            // headroom.
            if (options.tone_map_pq_and_hlg_to_dst) {
                steps_.push_back(std::make_unique<ColorTransformToneMapInRec2020Linear>(src));
            }

            // Convert back to XYZ.
            steps_.push_back(std::make_unique<ColorTransformMatrix>(rec2020_linear.GetPrimaryMatrix()));
        } else {
            if (options.tone_map_pq_and_hlg_to_dst) {
                ...
                steps_.push_back(std::make_unique<ColorTransformSrcNitsToSdrRelative>(12.f * ColorSpace::kDefaultSDRWhiteLevel,
    ...
    case ColorSpace::TransferID::PQ: {
        ...
        // SDR-relative space (where 1.0 is SDR white).
        steps_.push_back(std::make_unique<ColorTransformSrcNitsToSdrRelative>(kPQRefMaxLumNits, IsHlgPqSdrRelative()));
    ...
    steps_.push_back(std::make_unique<ColorTransformMatrix>(Invert(dst.GetPrimaryMatrix())));

    switch (dst.GetTransferID()) {
    case ColorSpace::TransferID::HLG:
        steps_.push_back(std::make_unique<ColorTransformSdrToDstNitsRelative>(gfx::ColorSpace::kDefaultSDRWhiteLevel));
        steps_.push_back(std::make_unique<ColorTransformHLG_OETF>());
        break;
    case ColorSpace::TransferID::PQ:
        steps_.push_back(std::make_unique<ColorTransformSdrToDstNitsRelative>(kPQRefMaxLumNits));
        steps_.push_back(std::make_unique<ColorTransformPQFromLinear>());
        break;
    case ColorSpace::TransferID::SCRGB_LINEAR_80_NITS:
        steps_.push_back(std::make_unique<ColorTransformSdrToDstNitsRelative>(80.f));
        break;
    case ColorSpace::TransferID::PIECEWISE_HDR: {
        skcms_TransferFunction fn;
        float p, q, r;
    ...
    default: {
        ...
        if (dst.GetInverseTransferFunction(&dst_from_linear_fn)) {
            steps_.push_back(std::make_unique<ColorTransformSkTransferFn>(dst_from_linear_fn, dst.HasExtendedSkTransferFn()));
        } else {
            steps_.push_back(std::make_unique<ColorTransformFromLinear>(dst.GetTransferID()));
        }
        break;
    }
    }
    ...
    // ITU-T H.273: If MatrixCoefficients is equal to 0 (Identity) or 8 (YCgCo),
    // range adjustment is performed on R,G,B samples rather than Y,U,V samples.
    const bool dst_matrix_is_identity_or_ycgco = dst.GetMatrixID() == ColorSpace::MatrixID::GBR || dst.GetMatrixID() == ColorSpace::MatrixID::YCOCG;
    auto dst_range_adjust_matrix = std::make_unique<ColorTransformMatrix>(Invert(dst.GetRangeAdjustMatrix(options.dst_bit_depth)));

    if (dst_matrix_is_identity_or_ycgco)
        steps_.push_back(std::move(dst_range_adjust_matrix));

    steps_.push_back(std::make_unique<ColorTransformMatrix>(dst.GetTransferMatrix(options.dst_bit_depth)));

    if (!dst_matrix_is_identity_or_ycgco)
        steps_.push_back(std::move(dst_range_adjust_matrix));
}

ColorTransformInternal::ColorTransformInternal(const ColorSpace& src, const ColorSpace& dst, const Options& options)
    : src_(src)
    , dst_(dst)
{
...
sk_sp<SkRuntimeEffect> ColorTransformInternal::GetSkRuntimeEffect() const
{
    ...
    auto result = SkRuntimeEffect::MakeForColorFilter(SkString(sksl_source.c_str(), sksl_source.size()),
        /*options=*/ {});
    DCHECK(result.effect) << '\n' << result.errorText.c_str() << "\n\nShader Source:\n" << sksl_source;
    return result.effect;
}

sk_sp<SkData> ColorTransformInternal::GetSkShaderUniforms(const RuntimeOptions& options) const
{
...
// static
std::unique_ptr<ColorTransform> ColorTransform::NewColorTransform(const ColorSpace& src, const ColorSpace& dst)
{
    Options options;
...
// static
std::unique_ptr<ColorTransform> ColorTransform::NewColorTransform(const ColorSpace& src, const ColorSpace& dst, const Options& options)
{
    return std::make_unique<ColorTransformInternal>(src, dst, options);
}
