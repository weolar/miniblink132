...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    const double upper_bound_hue = provider.ConsumeFloatingPointInRange(lower_bound_hue, lower_bound_hue + 1);
    ...
    color_utils::HSL upper_bound = { upper_bound_hue, std::max(s1, s2), std::max(l1, l2) };
    color_utils::HSL lower_bound = { lower_bound_hue, std::min(s1, s2), std::min(l1, l2) };

    bool find_closest = provider.ConsumeBool();

    // Ensure that we have enough data for this image.
    std::vector<uint8_t> image_data = provider.ConsumeBytes<uint8_t>(expected_size);
    ...
    color_utils::CalculateKMeanColorOfBitmap(bitmap, provider.ConsumeIntegralInRange<int>(-1, height + 2), lower_bound, upper_bound, find_closest);
