...
bool DelegatedInkPoint::MatchesDelegatedInkMetadata(const DelegatedInkMetadata* metadata) const
{
    if (!metadata) {
        return false;
    }
    ...
    TRACE_EVENT_INSTANT2("delegated_ink_trails", "DelegatedInkPoint::MatchesDelegatedInkMetadata", TRACE_EVENT_SCOPE_THREAD, "metadata", metadata->ToString(),
        "point", ToString());
    return timestamp_ == metadata->timestamp() && point_.IsWithinDistance(metadata->point(), kEpsilon);
}

std::string DelegatedInkPoint::ToString() const
{
    return base::StringPrintf(
        "point: %s, timestamp: %" PRId64 ", pointer_id: %d", point_.ToString().c_str(), timestamp_.since_origin().InMicroseconds(), pointer_id_);
}
