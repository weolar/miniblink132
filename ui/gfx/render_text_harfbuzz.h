...
enum class ShapeRunFallback { FAILED = 0, NO_FALLBACK = 1, FALLBACK = 2, FALLBACKS = 3, kMaxValue = FALLBACKS };
...
struct COMPONENT_EXPORT(GFX) TextRunHarfBuzz {
    ...
    RangeF GetGraphemeBounds(RenderTextHarfBuzz* render_text, size_t text_index) const;
    ...
    RangeF GetGraphemeSpanForCharRange(RenderTextHarfBuzz* render_text, const Range& char_range) const;
    ...
    struct COMPONENT_EXPORT(GFX) FontParams {
        ...
        bool SetRenderParamsRematchFont(const Font& font, const FontRenderParams& render_params);
        ...
        bool SetRenderParamsOverrideSkiaFaceFromFont(const Font& font, const FontRenderParams& render_params);
    ...
    void UpdateFontParamsAndShape(const FontParams& new_font_params, const ShapeOutput& new_shape);
...
protected:
    // RenderText:
    SelectionModel AdjacentCharSelectionModel(const SelectionModel& selection, VisualCursorDirection direction) override;
    SelectionModel AdjacentWordSelectionModel(const SelectionModel& selection, VisualCursorDirection direction) override;
    SelectionModel AdjacentLineSelectionModel(const SelectionModel& selection, VisualCursorDirection direction) override;
    void OnLayoutTextAttributeChanged() override;
    void OnDisplayTextAttributeChanged() override;
    void EnsureLayout() override;
    void DrawVisualText(internal::SkiaTextRenderer* renderer, const std::vector<Range>& selections) override;
...
private:
    ...
    SelectionModel FirstSelectionModelInsideRun(const internal::TextRunHarfBuzz* run);
    SelectionModel LastSelectionModelInsideRun(const internal::TextRunHarfBuzz* run);

    using CommonizedRunsMap
        = std::unordered_map<internal::TextRunHarfBuzz::FontParams, std::vector<internal::TextRunHarfBuzz*>, internal::TextRunHarfBuzz::FontParams::Hash>;
    ...
    void ItemizeTextToRuns(const std::u16string& string, internal::TextRunList* out_run_list, CommonizedRunsMap* out_commonized_run_map);
    ...
    // missing glyphs.
    bool ShapeRuns(const std::u16string& text, const internal::TextRunHarfBuzz::FontParams& base_font_params, std::vector<internal::TextRunHarfBuzz*> runs);
    ...
    void ShapeRunsWithFont(const std::u16string& text, const internal::TextRunHarfBuzz::FontParams& font_params,
        std::vector<internal::TextRunHarfBuzz*>* in_out_runs, std::vector<internal::TextRunHarfBuzz*>* sucessfully_shaped_runs = nullptr);
    ...
    void ItemizeAndShapeText(const std::u16string& text, internal::TextRunList* out_run_list);
    ...
    bool ItemizeAndShapeTextImpl(CommonizedRunsMap* commonized_run_map, const std::u16string& text, internal::TextRunList* run_list);
    ...
    void GetDecoratedTextForRange(const Range& range, DecoratedText* decorated_text) override;
