...
struct COLOR_SPACE_EXPORT HDRStaticMetadata {
    ...
    HDRStaticMetadata();
    HDRStaticMetadata(double max, double max_avg, double min, uint8_t supported_eotf_mask);
    ...
    {
        return ((max == rhs.max) && (max_avg == rhs.max_avg) && (min == rhs.min) && supported_eotf_mask == rhs.supported_eotf_mask);
    }
