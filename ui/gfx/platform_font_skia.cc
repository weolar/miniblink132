...
base::LazyInstance<scoped_refptr<PlatformFontSkia>>::Leaky g_default_font = LAZY_INSTANCE_INITIALIZER;
...
sk_sp<SkTypeface> CreateSkTypeface(bool italic, gfx::Font::Weight weight, std::string* family, bool* out_success)
{
    DCHECK(family);
    TRACE_EVENT0("fonts", "gfx::CreateSkTypeface");

    const int font_weight = (weight == Font::Weight::INVALID) ? static_cast<int>(Font::Weight::NORMAL) : static_cast<int>(weight);
    SkFontStyle sk_style(font_weight, SkFontStyle::kNormal_Width, italic ? SkFontStyle::kItalic_Slant : SkFontStyle::kUpright_Slant);
    sk_sp<SkTypeface> typeface;
    {
        TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("fonts"), "skia::MakeTypefaceFromName", "family", *family);
        typeface = skia::MakeTypefaceFromName(family->c_str(), sk_style);
    }
    if (!typeface) {
        TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("fonts"), "skia::MakeTypefaceFromName", "family", kFallbackFontFamilyName);
...
PlatformFontSkia::PlatformFontSkia(const std::string& font_name, int font_size_pixels)
{
    FontRenderParamsQuery query;
    query.families.push_back(font_name);
    query.pixel_size = font_size_pixels;
    query.weight = Font::Weight::NORMAL;
    InitFromDetails(nullptr, font_name, font_size_pixels, Font::NORMAL, query.weight, gfx::GetFontRenderParams(query, nullptr));
}

PlatformFontSkia::PlatformFontSkia(sk_sp<SkTypeface> typeface, int font_size_pixels, const std::optional<FontRenderParams>& params)
{
    DCHECK(typeface);
    ...
    InitFromDetails(std::move(typeface), family_name.c_str(), font_size_pixels, style, font_weight, actual_render_params);
}
...
void PlatformFontSkia::EnsuresDefaultFontIsInitialized()
{
    ...
    if (auto* linux_ui = ui::LinuxUi::instance()) {
        ...
        FontRenderParamsQuery query;
        CHECK(FontList::ParseDescription(*default_font_description_, &query.families, &query.style, &query.pixel_size, &query.weight))
    ...
    bool success = false;
    sk_sp<SkTypeface> typeface = CreateSkTypeface(style & Font::ITALIC, weight, &family, &success);
    ...
    g_default_font.Get() = new PlatformFontSkia(std::move(typeface), family, size_pixels, style, weight, params);
}
...
// static
void PlatformFontSkia::SetDefaultFontDescription(const std::string& font_description)
{
    delete default_font_description_;
    default_font_description_ = new std::string(font_description);
}

Font PlatformFontSkia::DeriveFont(int size_delta, int style, Font::Weight weight) const
{
#if BUILDFLAG(IS_WIN)
    ...
    bool success = true;
    sk_sp<SkTypeface> typeface = (weight == weight_ && style == style_) ? typeface_ : CreateSkTypeface(style, weight, &new_family, &success);
    if (!success) {
        LOG(ERROR) << "Could not find any font: " << new_family << ", " << kFallbackFontFamilyName << ". Falling back to the default";
    ...
    return Font(new PlatformFontSkia(std::move(typeface), new_family, new_size, style, weight, gfx::GetFontRenderParams(query, NULL)));
}
...
PlatformFontSkia::PlatformFontSkia(
    sk_sp<SkTypeface> typeface, const std::string& family, int size_pixels, int style, Font::Weight weight, const FontRenderParams& render_params)
{
    InitFromDetails(std::move(typeface), family, size_pixels, style, weight, render_params);
}
...
void PlatformFontSkia::InitFromDetails(
    sk_sp<SkTypeface> typeface, const std::string& font_family, int font_size_pixels, int style, Font::Weight weight, const FontRenderParams& render_params)
{
    ...
    bool success = true;
    typeface_ = typeface ? std::move(typeface) : CreateSkTypeface(style & Font::ITALIC, weight, &font_family_, &success);
...
void PlatformFontSkia::ComputeMetricsIfNecessary()
{
    if (metrics_need_computation_) {
        ...
        } else if (params.subpixel_rendering == FontRenderParams::SUBPIXEL_RENDERING_NONE) {
        ...
        font.setSkewX((Font::ITALIC & style_) && !typeface_->isItalic() ? -SK_Scalar1 / 4 : 0);
...
// static
PlatformFont* PlatformFont::CreateFromNameAndSize(const std::string& font_name, int font_size)
{
...
// static
PlatformFont* PlatformFont::CreateFromSkTypeface(sk_sp<SkTypeface> typeface, int font_size_pixels, const std::optional<FontRenderParams>& params)
{
