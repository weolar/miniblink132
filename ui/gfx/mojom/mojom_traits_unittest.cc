...
{
    return base::UnsafeSharedMemoryRegion::TakeHandleForSerialization(base::UnsafeSharedMemoryRegion::Create(1024)).PassPlatformHandle().fd;
}
#elif BUILDFLAG(IS_FUCHSIA)
zx::vmo CreateValidLookingBufferHandle()
{
    return base::UnsafeSharedMemoryRegion::TakeHandleForSerialization(base::UnsafeSharedMemoryRegion::Create(1024)).PassPlatformHandle();
}
#endif
...
private:
    // TraitsTestService:
    void EchoSelectionBound(const SelectionBound& s, EchoSelectionBoundCallback callback) override
    {
        std::move(callback).Run(s);
    }

    void EchoTransform(const Transform& t, EchoTransformCallback callback) override
    {
        std::move(callback).Run(t);
    }

    void EchoGpuMemoryBufferHandle(GpuMemoryBufferHandle handle, EchoGpuMemoryBufferHandleCallback callback) override
    {
...
TEST_F(StructTraitsTest, Transform)
{
    ...
    auto input = gfx::Transform::RowMajor(r0c0, r0c1, r0c2, r0c3, r1c0, r1c1, r1c2, r1c3, r2c0, r2c1, r2c2, r2c3, r3c0, r3c1, r3c2, r3c3);
    mojo::Remote<mojom::TraitsTestService> remote = GetTraitsTestRemote();
...
TEST_F(StructTraitsTest, AcceleratedWidget)
{
    ...
    mojo::test::SerializeAndDeserialize<gfx::mojom::AcceleratedWidget>(input, output);
...
TEST_F(StructTraitsTest, GpuMemoryBufferHandle)
{
    ...
    base::UnsafeSharedMemoryRegion shared_memory_region = base::UnsafeSharedMemoryRegion::Create(1024);
    ...
    handle2.native_pixmap_handle.buffer_collection_handle = std::move(client_handle);
    handle2.native_pixmap_handle.buffer_index = 4;
    handle2.native_pixmap_handle.ram_coherency = true;
#endif
    handle2.native_pixmap_handle.planes.emplace_back(kOffset, kStride, kSize, std::move(buffer_handle));
...
#elif BUILDFLAG(IS_FUCHSIA)
    EXPECT_EQ(handle_koid, base::GetKoid(output.native_pixmap_handle.buffer_collection_handle).value());
...
{
    using BufferFormatTraits = mojo::EnumTraits<gfx::mojom::BufferFormat, gfx::BufferFormat>;
    BufferFormat output;
    mojo::Remote<mojom::TraitsTestService> remote = GetTraitsTestRemote();
...
{
    using BufferUsageTraits = mojo::EnumTraits<gfx::mojom::BufferUsage, gfx::BufferUsage>;
    BufferUsage output;
    mojo::Remote<mojom::TraitsTestService> remote = GetTraitsTestRemote();
...
TEST_F(StructTraitsTest, PresentationFeedback)
{
    ...
    uint32_t flags = PresentationFeedback::kVSync | PresentationFeedback::kZeroCopy;
    ...
    mojo::test::SerializeAndDeserialize<gfx::mojom::PresentationFeedback>(input, output);
