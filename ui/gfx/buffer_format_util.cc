...
constexpr auto kBufferFormats = std::to_array<BufferFormat>({ BufferFormat::R_8, BufferFormat::R_16, BufferFormat::RG_88, BufferFormat::RG_1616,
    BufferFormat::BGR_565, BufferFormat::RGBA_4444, BufferFormat::RGBX_8888, BufferFormat::RGBA_8888, BufferFormat::BGRX_8888, BufferFormat::BGRA_1010102,
    BufferFormat::RGBA_1010102, BufferFormat::BGRA_8888, BufferFormat::RGBA_F16, BufferFormat::YUV_420_BIPLANAR, BufferFormat::YVU_420,
    BufferFormat::YUVA_420_TRIPLANAR, BufferFormat::P010 });

static_assert(std::size(kBufferFormats) == (static_cast<int>(BufferFormat::LAST) + 1), "BufferFormat::LAST must be last value of kBufferFormats");

} // namespace

base::span<const BufferFormat> GetBufferFormatsForTesting()
{
    return base::span<const BufferFormat>(kBufferFormats.data(), kBufferFormats.size());
}
...
base::CheckedNumeric<size_t> PlaneWidthForBufferFormatChecked(size_t width, BufferFormat format, size_t plane)
{
    const size_t subsample = SubsamplingFactorForBufferFormat(format, plane);
    return base::CheckDiv(base::CheckAdd(width, base::CheckSub(subsample, 1)), subsample);
}

base::CheckedNumeric<size_t> PlaneHeightForBufferFormatCheckedInternal(size_t height, BufferFormat format, size_t plane)
{
    const size_t subsample = SubsamplingFactorForBufferFormat(format, plane);
    return base::CheckDiv(base::CheckAdd(height, base::CheckSub(subsample, 1)), subsample);
}
...
bool RowSizeForBufferFormatChecked(size_t width, BufferFormat format, size_t plane, size_t* size_in_bytes)
{
    base::CheckedNumeric<size_t> checked_size = PlaneWidthForBufferFormatChecked(width, format, plane);
...
bool PlaneHeightForBufferFormatChecked(size_t height, BufferFormat format, size_t plane, size_t* height_in_pixels)
{
    base::CheckedNumeric<size_t> checked_height = PlaneHeightForBufferFormatCheckedInternal(height, format, plane);
...
size_t PlaneSizeForBufferFormat(const Size& size, BufferFormat format, size_t plane)
{
    size_t plane_size = 0;
    bool valid = PlaneSizeForBufferFormatChecked(size, format, plane, &plane_size);
    DCHECK(valid);
    return plane_size;
}

bool PlaneSizeForBufferFormatChecked(const Size& size, BufferFormat format, size_t plane, size_t* size_in_bytes)
{
    size_t row_size = 0;
    if (!RowSizeForBufferFormatChecked(base::checked_cast<size_t>(size.width()), format, plane, &row_size)) {
        return false;
    }
    base::CheckedNumeric<size_t> checked_plane_size = row_size;
    checked_plane_size *= PlaneHeightForBufferFormatCheckedInternal(base::checked_cast<size_t>(size.height()), format, plane);
...
bool BufferSizeForBufferFormatChecked(const Size& size, BufferFormat format, size_t* size_in_bytes)
{
...
size_t BufferOffsetForBufferFormat(const Size& size, BufferFormat format, size_t plane)
{
