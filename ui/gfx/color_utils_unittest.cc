...
TEST(ColorUtils, MidpointLuminanceMatches)
{
    ...
    EXPECT_FLOAT_EQ(GetContrastRatio(darkest, midpoint), GetContrastRatio(midpoint, lightest));
...
TEST(ColorUtils, GetColorWithMaxContrast)
{
    ...
    EXPECT_EQ(SK_ColorWHITE, GetColorWithMaxContrast(SkColorSetRGB(0x75, 0x75, 0x75)));
    EXPECT_EQ(SK_ColorBLACK, GetColorWithMaxContrast(SK_ColorWHITE));
    EXPECT_EQ(SK_ColorBLACK, GetColorWithMaxContrast(SkColorSetRGB(0x76, 0x76, 0x76)));
...
TEST(ColorUtils, GetEndpointColorWithMinContrast)
{
    ...
    EXPECT_EQ(SK_ColorBLACK, GetEndpointColorWithMinContrast(SkColorSetRGB(0x75, 0x75, 0x75)));
    EXPECT_EQ(SK_ColorWHITE, GetEndpointColorWithMinContrast(SK_ColorWHITE));
    EXPECT_EQ(SK_ColorWHITE, GetEndpointColorWithMinContrast(SkColorSetRGB(0x76, 0x76, 0x76)));

    SetDarkestColorForTesting(old_darkest_color);
    EXPECT_EQ(old_darkest_color, GetEndpointColorWithMinContrast(old_darkest_color));
}
...
TEST(ColorUtils, BlendForMinContrast_BlendDarker)
{
    ...
    EXPECT_GE(GetContrastRatio(result.color, SK_ColorWHITE), kMinimumReadableContrastRatio);
}
...
TEST(ColorUtils, BlendForMinContrast_BlendLighter)
{
    ...
    EXPECT_NE(SK_AlphaTRANSPARENT, result.alpha);
    EXPECT_NE(foreground, result.color);
    EXPECT_GE(GetContrastRatio(result.color, SK_ColorBLACK), kMinimumReadableContrastRatio);
}
...
TEST(ColorUtils, BlendForMinContrast_StopsAtDarkestColor)
{
    ...
    EXPECT_EQ(darkest_color, BlendForMinContrast(SkColorSetRGB(0x55, 0x55, 0x55), SkColorSetRGB(0xAA, 0xAA, 0xAA)).color);
...
{
    SkColor test_colors[]
        = { SK_ColorBLACK, SK_ColorWHITE, SK_ColorRED, SK_ColorYELLOW, SK_ColorMAGENTA, gfx::kGoogleGreen500, gfx::kGoogleRed050, gfx::kGoogleBlue800 };
    SkAlpha test_alphas[] = { SK_AlphaTRANSPARENT, 0x0F, gfx::kDisabledControlAlpha, 0xDD };
    SkAlpha blend_alpha = 0x7F;
    for (const SkColor color : test_colors) {
        SkColor opaque_result = color_utils::BlendTowardMaxContrast(color, blend_alpha);
...
TEST(ColorUtils, BlendForMinContrast_MatchesNaiveImplementation)
{
    ...
    const auto result = BlendForMinContrast(default_foreground, background, high_contrast_foreground, kContrastRatio);
...
TEST(ColorUtils, PickGoogleColor)
{
    ...
    EXPECT_EQ(gfx::kGoogleBlue800, PickGoogleColor(gfx::kGoogleBlue800, gfx::kGoogleBlue700, 1.1f));
    EXPECT_EQ(gfx::kGoogleBlue600, PickGoogleColor(gfx::kGoogleBlue600, gfx::kGoogleBlue700, 1.1f));

    // If it does not, it should stay on the same side of the background if
    // possible.
    EXPECT_EQ(gfx::kGoogleBlue900, PickGoogleColor(gfx::kGoogleBlue800, gfx::kGoogleBlue700, 1.25f));
    EXPECT_EQ(gfx::kGoogleBlue500, PickGoogleColor(gfx::kGoogleBlue600, gfx::kGoogleBlue700, 1.25f));

    // If even Blue 900 does not contrast enough, Grey 900 is a slightly darker
    // color.
    EXPECT_EQ(gfx::kGoogleGrey900, PickGoogleColor(gfx::kGoogleBlue800, gfx::kGoogleBlue700, 1.5f));

    // If no dark colors have enough contrast, the result should be a lighter
    // color instead.
    EXPECT_EQ(gfx::kGoogleBlue200, PickGoogleColor(gfx::kGoogleBlue800, gfx::kGoogleBlue700, 3.0f));

    // If the requested contrast is too high for any color to be sufficient, the
    // result should be the most-contrasting endpoint.
    EXPECT_EQ(SK_ColorWHITE, PickGoogleColor(gfx::kGoogleBlue800, gfx::kGoogleBlue700, kMaximumPossibleContrast));

    // Matching the background exactly is reasonable, if the minimum contrast is
    // zero.
    EXPECT_EQ(gfx::kGoogleBlue700, PickGoogleColor(gfx::kGoogleBlue800, gfx::kGoogleBlue700, 0.0f, 1.2f));
    ...
    EXPECT_EQ(gfx::kGoogleBlue800, PickGoogleColor(gfx::kGoogleBlue900, gfx::kGoogleBlue700, 1.18f, 1.2f));
}

TEST(ColorUtils, PickGoogleColorTwoBackgrounds)
{
    // If the input color already has sufficient contrast, it should be accepted.
    EXPECT_EQ(gfx::kGoogleBlue800, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue800, gfx::kGoogleBlue700, gfx::kGoogleBlue200, 1.1f));
    EXPECT_EQ(gfx::kGoogleBlue600, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue600, gfx::kGoogleBlue700, gfx::kGoogleBlue200, 1.1f));
    EXPECT_EQ(gfx::kGoogleBlue300, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue300, gfx::kGoogleBlue700, gfx::kGoogleBlue200, 1.1f));
    EXPECT_EQ(gfx::kGoogleBlue100, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue100, gfx::kGoogleBlue700, gfx::kGoogleBlue200, 1.1f));

    // If it does not, it should stay on the same side of the background if
    // possible.
    EXPECT_EQ(gfx::kGoogleBlue900, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue800, gfx::kGoogleBlue700, gfx::kGoogleBlue200, 1.25f));
    EXPECT_EQ(gfx::kGoogleBlue500, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue600, gfx::kGoogleBlue700, gfx::kGoogleBlue200, 1.25f));
    EXPECT_EQ(gfx::kGoogleBlue400, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue300, gfx::kGoogleBlue700, gfx::kGoogleBlue200, 1.3f));
    EXPECT_EQ(gfx::kGoogleBlue050, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue100, gfx::kGoogleBlue700, gfx::kGoogleBlue200, 1.3f));

    // If the blue endpoints do not contrast enough, the grey endpoints are
    // available.
    EXPECT_EQ(gfx::kGoogleGrey900, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue800, gfx::kGoogleBlue700, gfx::kGoogleBlue200, 1.5f));
    EXPECT_EQ(SK_ColorWHITE, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue100, gfx::kGoogleBlue700, gfx::kGoogleBlue200, 1.5f));
    ...
    EXPECT_EQ(gfx::kGoogleBlue500, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue100, gfx::kGoogleBlue200, gfx::kGoogleBlue900, 1.7f));
    EXPECT_EQ(gfx::kGoogleBlue100, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue800, gfx::kGoogleBlue700, gfx::kGoogleBlue600, 3.0f));

    // If the requested contrast is too high for any color to be sufficient, the
    // result should be the most-contrasting point.
    EXPECT_EQ(SK_ColorWHITE, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue800, gfx::kGoogleBlue700, gfx::kGoogleBlue600, kMaximumPossibleContrast));
    EXPECT_EQ(gfx::kGoogleGrey900, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue100, gfx::kGoogleBlue200, gfx::kGoogleBlue300, kMaximumPossibleContrast));
    EXPECT_EQ(gfx::kGoogleBlue400, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue100, gfx::kGoogleBlue900, gfx::kGoogleBlue050, kMaximumPossibleContrast));

    // Matching the background exactly is reasonable, if the minimum contrast is
    // zero.
    EXPECT_EQ(gfx::kGoogleBlue700, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue800, gfx::kGoogleBlue700, gfx::kGoogleBlue600, 0.0f, 1.2f));

    // Blue 600 is the only color that fits in the requested contrast window, but
    // it's on the other side of the background from the input, so something
    // closer to the input is used instead.
    EXPECT_EQ(gfx::kGoogleBlue800, PickGoogleColorTwoBackgrounds(gfx::kGoogleBlue900, gfx::kGoogleBlue700, gfx ::kGoogleBlue500, 1.18f, 1.2f));
}
