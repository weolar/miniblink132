...
// either 0, 1 or -1.
bool MassageRotationIfMultipleOfNinetyDegrees(gfx::Transform* rotation, float degrees)
{
...
InterpolatedTransform::InterpolatedTransform(float start_time, float end_time)
...
void InterpolatedTransform::SetChild(std::unique_ptr<InterpolatedTransform> child)
{
    child_ = std::move(child);
}

inline float InterpolatedTransform::ValueBetween(float time, float start_value, float end_value) const
{
    // can't handle NaN
    ...
    return static_cast<float>(gfx::Tween::DoubleValueBetween(t, start_value, end_value));
}
...
InterpolatedRotation::InterpolatedRotation(float start_degrees, float end_degrees)
    : InterpolatedTransform()
...
InterpolatedRotation::InterpolatedRotation(float start_degrees, float end_degrees, float start_time, float end_time)
...
InterpolatedAxisAngleRotation::InterpolatedAxisAngleRotation(
    const gfx::Vector3dF& axis, float start_degrees, float end_degrees, float start_time, float end_time)
    : InterpolatedTransform(start_time, end_time)
    , axis_(axis)
    , start_degrees_(start_degrees)
    , end_degrees_(end_degrees)
...
gfx::Transform InterpolatedAxisAngleRotation::InterpolateButDoNotCompose(float t) const
{
    gfx::Transform result;
...
InterpolatedScale::InterpolatedScale(float start_scale, float end_scale, float start_time, float end_time)
    : InterpolatedTransform(start_time, end_time)
    , start_scale_(gfx::Point3F(start_scale, start_scale, start_scale))
    , end_scale_(gfx::Point3F(end_scale, end_scale, end_scale))
{
...
InterpolatedScale::InterpolatedScale(const gfx::Point3F& start_scale, const gfx::Point3F& end_scale)
    : InterpolatedTransform()
...
InterpolatedScale::InterpolatedScale(const gfx::Point3F& start_scale, const gfx::Point3F& end_scale, float start_time, float end_time)
    : InterpolatedTransform(start_time, end_time)
    , start_scale_(start_scale)
    , end_scale_(end_scale)
{
...
InterpolatedTranslation::InterpolatedTranslation(const gfx::PointF& start_pos, const gfx::PointF& end_pos)
    : InterpolatedTransform()
...
InterpolatedTranslation::InterpolatedTranslation(const gfx::PointF& start_pos, const gfx::PointF& end_pos, float start_time, float end_time)
    : InterpolatedTransform(start_time, end_time)
    , start_pos_(start_pos)
    , end_pos_(end_pos)
{
...
InterpolatedTranslation::InterpolatedTranslation(const gfx::Point3F& start_pos, const gfx::Point3F& end_pos)
    : InterpolatedTransform()
    , start_pos_(start_pos)
    , end_pos_(end_pos)
{
...
InterpolatedTranslation::InterpolatedTranslation(const gfx::Point3F& start_pos, const gfx::Point3F& end_pos, float start_time, float end_time)
    : InterpolatedTransform(start_time, end_time)
    , start_pos_(start_pos)
    , end_pos_(end_pos)
{
...
gfx::Transform InterpolatedTranslation::InterpolateButDoNotCompose(float t) const
{
    gfx::Transform result;
    result.Translate3d(
        ValueBetween(t, start_pos_.x(), end_pos_.x()), ValueBetween(t, start_pos_.y(), end_pos_.y()), ValueBetween(t, start_pos_.z(), end_pos_.z()));
    return result;
}
...
InterpolatedConstantTransform::InterpolatedConstantTransform(const gfx::Transform& transform)
    : InterpolatedTransform()
    , transform_(transform)
{
}

gfx::Transform InterpolatedConstantTransform::InterpolateButDoNotCompose(float t) const
{
    return transform_;
}
...
InterpolatedTransformAboutPivot::InterpolatedTransformAboutPivot(const gfx::Point& pivot, std::unique_ptr<InterpolatedTransform> transform)
    : InterpolatedTransform()
{
...
InterpolatedTransformAboutPivot::InterpolatedTransformAboutPivot(
    const gfx::Point& pivot, std::unique_ptr<InterpolatedTransform> transform, float start_time, float end_time)
    : InterpolatedTransform()
{
    Init(pivot, std::move(transform));
}
...
gfx::Transform InterpolatedTransformAboutPivot::InterpolateButDoNotCompose(float t) const
{
    if (transform_.get()) {
...
void InterpolatedTransformAboutPivot::Init(const gfx::Point& pivot, std::unique_ptr<InterpolatedTransform> xform)
{
    ...
    std::unique_ptr<InterpolatedTransform> pre_transform = std::make_unique<InterpolatedConstantTransform>(to_pivot);
    std::unique_ptr<InterpolatedTransform> post_transform = std::make_unique<InterpolatedConstantTransform>(from_pivot);
...
InterpolatedMatrixTransform::InterpolatedMatrixTransform(const gfx::Transform& start_transform, const gfx::Transform& end_transform)
    : InterpolatedTransform()
{
...
InterpolatedMatrixTransform::InterpolatedMatrixTransform(
    const gfx::Transform& start_transform, const gfx::Transform& end_transform, float start_time, float end_time)
    : InterpolatedTransform()
{
    Init(start_transform, end_transform);
}
...
gfx::Transform InterpolatedMatrixTransform::InterpolateButDoNotCompose(float t) const
{
    gfx::DecomposedTransform blended = gfx::BlendDecomposedTransforms(end_decomp_, start_decomp_, t);
    return gfx::Transform::Compose(blended);
}

void InterpolatedMatrixTransform::Init(const gfx::Transform& start_transform, const gfx::Transform& end_transform)
{
