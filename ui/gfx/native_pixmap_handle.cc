...
NativePixmapPlane::NativePixmapPlane(int stride, int offset, uint64_t size
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    ,
    base::ScopedFD fd
...
NativePixmapPlane& NativePixmapPlane::operator=(NativePixmapPlane&& other) = default;
...
NativePixmapHandle& NativePixmapHandle::operator=(NativePixmapHandle&& other) = default;
...
#error Unsupported OS
    ...
    clone.supports_zero_copy_webgpu_import = handle.supports_zero_copy_webgpu_import;
#endif

#if BUILDFLAG(IS_FUCHSIA)
    if (handle.buffer_collection_handle) {
        zx_status_t status = handle.buffer_collection_handle.duplicate(ZX_RIGHT_SAME_RIGHTS, &clone.buffer_collection_handle);
        if (status != ZX_OK) {
            ZX_DLOG(ERROR, status) << "zx_handle_duplicate";
            return NativePixmapHandle();
        }
...
bool CanFitImageForSizeAndFormat(const gfx::NativePixmapHandle& handle, const gfx::Size& size, gfx::BufferFormat format, bool assume_single_memory_object)
{
    ...
    if (assume_single_memory_object) {
        if (!base::IsValueInRangeForNumericType<size_t>(handle.planes.back().offset)
            || !base::IsValueInRangeForNumericType<size_t>(handle.planes.back().size)) {
            return false;
        }
        const base::CheckedNumeric<size_t> total_size_checked
            = base::CheckAdd(base::checked_cast<size_t>(handle.planes.back().offset), base::checked_cast<size_t>(handle.planes.back().size));
    ...
    for (size_t i = 0; i < handle.planes.size(); ++i) {
        const size_t plane_stride = base::strict_cast<size_t>(handle.planes[i].stride);
        size_t min_stride = 0;
        if (!gfx::RowSizeForBufferFormatChecked(base::checked_cast<size_t>(size.width()), format, i, &min_stride) || plane_stride < min_stride) {
            return false;
        }

        const size_t subsample_factor = SubsamplingFactorForBufferFormat(format, i);
        const base::CheckedNumeric<size_t> plane_height
            = base::CheckDiv(base::CheckAdd(base::checked_cast<size_t>(size.height()), base::CheckSub(subsample_factor, 1)), subsample_factor);
        const base::CheckedNumeric<size_t> min_size = plane_height * plane_stride;
        if (!min_size.IsValid<uint64_t>() || handle.planes[i].size < min_size.ValueOrDie<uint64_t>()) {
            return false;
        }
        ...
        if (assume_single_memory_object) {
            if (!base::IsValueInRangeForNumericType<size_t>(handle.planes[i].offset) || !base::IsValueInRangeForNumericType<size_t>(handle.planes[i].size)) {
                return false;
            }
            base::CheckedNumeric<size_t> end_pos
                = base::CheckAdd(base::checked_cast<size_t>(handle.planes[i].offset), base::checked_cast<size_t>(handle.planes[i].size));
