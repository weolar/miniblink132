...
struct SwapTimings {
    ...
    bool is_null() const
    {
        return swap_start.is_null() && swap_end.is_null();
    }
};
...
struct COMPONENT_EXPORT(GFX) SwapCompletionResult {
    ...
    SwapCompletionResult(gfx::SwapResult swap_result, gfx::GpuFenceHandle release_fence);
    SwapCompletionResult(gfx::SwapResult swap_result, std::unique_ptr<gfx::CALayerParams> ca_layer_params);
