...
TEST(ICCProfile, Conversions)
{
    ...
    ICCProfile icc_profile_from_color_space = ICCProfile::FromColorSpace(color_space_from_icc_profile);
...
TEST(ICCProfile, SRGB)
{
    ...
    EXPECT_EQ(icc_profile.GetColorSpace().ToSkColorSpace().get(), sk_color_space.get());
...
TEST(ICCProfile, Equality)
{
    ...
    EXPECT_FALSE(SkColorSpace::Equals(spin_space.ToSkColorSpace().get(), adobe_space.ToSkColorSpace().get()));
}
...
TEST(ICCProfile, GarbageData)
{
    ...
    ICCProfile garbage_profile = ICCProfile::FromData(bad_data.data(), bad_data.size());
...
TEST(ICCProfile, GenericRGB)
{
    ...
    ColorSpace color_space(ColorSpace::PrimaryID::APPLE_GENERIC_RGB, ColorSpace::TransferID::GAMMA18);
