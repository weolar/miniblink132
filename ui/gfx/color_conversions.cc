...
{
    static skcms_Matrix3x3 adapt_XYZD65_to_srgb = skcms_Matrix3x3_concat(getXYZD50TosRGBLinearMatrix(), getXYDZ65toXYZD50matrix());
...
const skcms_Matrix3x3* getXYZToLMSMatrix()
{
    static const skcms_Matrix3x3 kXYZ_to_LMS = { { { 0.8190224432164319f, 0.3619062562801221f, -0.12887378261216414f },
        { 0.0329836671980271f, 0.9292868468965546f, 0.03614466816999844f }, { 0.048177199566046255f, 0.26423952494422764f, 0.6335478258136937f } } };
    return &kXYZ_to_LMS;
}
...
{
    static const skcms_Matrix3x3 kOklab_to_LMS = { { { 0.99999999845051981432f, 0.39633779217376785678f, 0.21580375806075880339f },
...
static skcms_Vector3 skcms_Matrix3x3_apply(const skcms_Matrix3x3* m, const skcms_Vector3* v)
{
    ...
    for (int row = 0; row < 3; ++row) {
        ...
        // float array.
        UNSAFE_BUFFERS(dst.vals[row] = m->vals[row][0] * v->vals[0] + m->vals[row][1] * v->vals[1] + m->vals[row][2] * v->vals[2]);
    }
    return dst;
}
...
{
    return std::make_tuple(skcmsTrFnEvalExt(getSRGBInverseTransferFunction(), r), skcmsTrFnEvalExt(getSRGBInverseTransferFunction(), g),
...
std::tuple<float, float, float> ApplyTransferFnsRGB(float r, float g, float b)
{
    return std::make_tuple(
        skcmsTrFnEvalExt(&SkNamedTransferFn::kSRGB, r), skcmsTrFnEvalExt(&SkNamedTransferFn::kSRGB, g), skcmsTrFnEvalExt(&SkNamedTransferFn::kSRGB, b));
}

std::tuple<float, float, float> ApplyTransferFnProPhoto(float r, float g, float b)
{
    return std::make_tuple(skcmsTrFnEvalExt(&SkNamedTransferFnExt::kProPhotoRGB, r), skcmsTrFnEvalExt(&SkNamedTransferFnExt::kProPhotoRGB, g),
...
{
    return std::make_tuple(
        skcmsTrFnEvalExt(&SkNamedTransferFn::k2Dot2, r), skcmsTrFnEvalExt(&SkNamedTransferFn::k2Dot2, g), skcmsTrFnEvalExt(&SkNamedTransferFn::k2Dot2, b));
}
...
{
    return std::make_tuple(skcmsTrFnEvalExt(getProPhotoInverseTransferFunction(), r), skcmsTrFnEvalExt(getProPhotoInverseTransferFunction(), g),
...
{
    return std::make_tuple(skcmsTrFnEvalExt(getAdobeRGBInverseTransferFunction(), r), skcmsTrFnEvalExt(getAdobeRGBInverseTransferFunction(), g),
...
{
    return std::make_tuple(skcmsTrFnEvalExt(&SkNamedTransferFn::kRec2020, r), skcmsTrFnEvalExt(&SkNamedTransferFn::kRec2020, g),
...
{
    return std::make_tuple(skcmsTrFnEvalExt(getRec2020nverseTransferFunction(), r), skcmsTrFnEvalExt(getRec2020nverseTransferFunction(), g),
...
std::tuple<float, float, float> SRGBToHSL(float r, float g, float b)
{
    ...
    if (d != 0.0f) {
        saturation = (lightness == 0.0f || lightness == 1.0f) ? 0.0f : (max - lightness) / std::min(lightness, 1 - lightness);
        if (max == r) {
