...
TEST(FontListTest, ParseDescription)
{
    ...
    EXPECT_TRUE(FontList::ParseDescription("Arial,Helvetica,Bold Italic 12px", &families, &style, &size_pixels, &weight));
    ...
    EXPECT_TRUE(FontList::ParseDescription("  Verdana , Italic  Bold   10px ", &families, &style, &size_pixels, &weight));
    ...
    EXPECT_FALSE(FontList::ParseDescription("", &families, &style, &size_pixels, &weight));
    EXPECT_FALSE(FontList::ParseDescription("Arial", &families, &style, &size_pixels, &weight));
    EXPECT_FALSE(FontList::ParseDescription("Arial,12", &families, &style, &size_pixels, &weight));
    EXPECT_FALSE(FontList::ParseDescription("Arial 12px", &families, &style, &size_pixels, &weight));
    EXPECT_FALSE(FontList::ParseDescription("Arial,12px,", &families, &style, &size_pixels, &weight));
    EXPECT_FALSE(FontList::ParseDescription("Arial,0px", &families, &style, &size_pixels, &weight));
    EXPECT_FALSE(FontList::ParseDescription("Arial,-1px", &families, &style, &size_pixels, &weight));
    EXPECT_FALSE(FontList::ParseDescription("Arial,foo 12px", &families, &style, &size_pixels, &weight));
}
...
{
    std::vector<Font> fonts;
    fonts.push_back(Font("Arial", 18).Derive(0, Font::ITALIC, Font::Weight::NORMAL));
    fonts.push_back(Font("Courier New", 18).Derive(0, Font::ITALIC, Font::Weight::NORMAL));
    FontList font_list = FontList(fonts);
...
TEST(FontListTest, Fonts_GetHeight_GetBaseline)
{
    ...
    ASSERT_EQ(base::ToLowerASCII(kTestFontName), base::ToLowerASCII(font1.GetActualFontName()));
    ...
    ASSERT_EQ(base::ToLowerASCII(kCJKFontName), base::ToLowerASCII(font2.GetActualFontName()));
    std::vector<Font> fonts;
    fonts.push_back(font1);
    fonts.push_back(font2);
    FontList font_list_mix(fonts);
    // ascent of FontList == max(ascent of Fonts)
    EXPECT_EQ(std::max(font1.GetBaseline(), font2.GetBaseline()), font_list_mix.GetBaseline());
    // descent of FontList == max(descent of Fonts)
    EXPECT_EQ(
        std::max(font1.GetHeight() - font1.GetBaseline(), font2.GetHeight() - font2.GetBaseline()), font_list_mix.GetHeight() - font_list_mix.GetBaseline());
}
...
TEST(FontListTest, FirstAvailableOrFirst)
{
    ...
    EXPECT_EQ("Arial", FontList::FirstAvailableOrFirst("not exist, Arial, not exist"));

    EXPECT_EQ("not exist", FontList::FirstAvailableOrFirst("not exist, not exist 2"));

    EXPECT_EQ("Arial", FontList::FirstAvailableOrFirst(", not exist, Arial"));
    EXPECT_EQ("not exist", FontList::FirstAvailableOrFirst(", not exist, not exist"));
}
