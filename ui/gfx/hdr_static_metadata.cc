...
HDRStaticMetadata::HDRStaticMetadata(double max, double max_avg, double min, uint8_t supported_eotf_mask)
    : max(max)
    , max_avg(max_avg)
    , min(min)
...
HDRStaticMetadata& HDRStaticMetadata::operator=(const HDRStaticMetadata& rhs) = default;
