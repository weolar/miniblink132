...
bool ColorConversionSkFilterCache::Key::Key::operator==(const Key& other) const
{
    return src == other.src && src_bit_depth == other.src_bit_depth && dst == other.dst && dst_sdr_max_luminance_nits == other.dst_sdr_max_luminance_nits;
}
...
bool ColorConversionSkFilterCache::Key::operator<(const Key& other) const
{
    return std::tie(src, src_bit_depth, dst, dst_sdr_max_luminance_nits)
        < std::tie(other.src, other.src_bit_depth, other.dst, other.dst_sdr_max_luminance_nits);
}

ColorConversionSkFilterCache::Key::Key(const gfx::ColorSpace& src, uint32_t src_bit_depth, const gfx::ColorSpace& dst, float dst_sdr_max_luminance_nits)
    : src(src)
...
ColorConversionSkFilterCache::Value& ColorConversionSkFilterCache::Value::operator=(Value&& other)
{
    transform = std::move(other.transform);
    effect = std::move(other.effect);
    return *this;
...
sk_sp<SkColorFilter> ColorConversionSkFilterCache::Get(const gfx::ColorSpace& src, const gfx::ColorSpace& dst, std::optional<uint32_t> src_bit_depth,
    std::optional<gfx::HDRMetadata> src_hdr_metadata, float dst_sdr_max_luminance_nits, float dst_max_luminance_relative)
{
    ...
    const Key key(src, src_bit_depth.value_or(0), dst, dst_sdr_max_luminance_nits);
    ...
    return value.effect->makeColorFilter(value.transform->GetSkShaderUniforms(options));
}
