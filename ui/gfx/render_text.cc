...
int CalculateFadeGradientWidth(const FontList& font_list, int display_width)
{
    ...
    const int gradient_width = std::min(narrow_width, base::ClampRound(display_width / 3.f));
...
void AddFadeEffect(const Rect& text_rect, const Rect& fade_rect, SkColor c0, SkColor c1, std::vector<SkScalar>* positions, std::vector<SkColor>* colors)
{
...
sk_sp<cc::PaintShader> CreateFadeShader(const FontList& font_list, const Rect& text_rect, const Rect& left_part, const Rect& right_part, SkColor color)
{
    ...
    const SkAlpha alpha = (width_fraction < 1) ? base::ClampRound<SkAlpha>((1 - width_fraction) * kAlphaAtZeroWidth) : 0;
    ...
    if (!left_part.IsEmpty())
        AddFadeEffect(text_rect, left_part, fade_color, color, &positions, &colors);
    if (!right_part.IsEmpty())
        AddFadeEffect(text_rect, right_part, color, fade_color, &positions, &colors);
    ...
    const SkPoint points[2] = { PointToSkPoint(text_rect.origin()), PointToSkPoint(text_rect.top_right()) };
    ...
    return cc::PaintShader::MakeLinearGradient(&points[0], &colors4f[0], &positions[0], static_cast<int>(colors4f.size()), SkTileMode::kClamp);
}

// Converts a FontRenderParams::Hinting value to the corresponding
// SkFontHinting value.
SkFontHinting FontRenderParamsHintingToSkFontHinting(FontRenderParams::Hinting params_hinting)
{
...
template <typename T> void RestoreBreakList(RenderText* render_text, BreakList<T>* break_list)
{
    ...
    while (range.end() < break_list->max()) {
        ...
        if (range.end() < break_list->max() && !render_text->IsValidCursorIndex(range.end())) {
            range.set_end(render_text->IndexOfAdjacentGrapheme(range.end(), CURSOR_FORWARD));
...
typename BreakList<T>::const_iterator IncrementBreakListIteratorToPosition(
    const BreakList<T>& break_list, typename BreakList<T>::const_iterator iter, size_t position)
{
...
UChar32 ReplaceControlCharacter(UChar32 codepoint)
{
    ...
    if (codepoint > 0x7F) {
        ...
        if (codepoint_category == U_PRIVATE_USE_CHAR || codepoint_category == U_CONTROL_CHAR) {
            return kReplacementCodepoint;
        }
    }
...
int GetLineSegmentContainingXCoord(const internal::Line& line, float line_x, float* offset_relative_segment)
{
...
void SkiaTextRenderer::SetFontRenderParams(const FontRenderParams& params, bool subpixel_rendering_suppressed)
{
...
void SkiaTextRenderer::DrawPosText(const SkPoint* pos, const uint16_t* glyphs, size_t glyph_count)
{
...
void SkiaTextRenderer::DrawUnderline(int x, int y, int width, SkScalar thickness_factor)
{
    SkScalar x_scalar = SkIntToScalar(x);
    const SkScalar text_size = font_.getSize();
    SkRect r = SkRect::MakeLTRB(x_scalar, y + text_size * kUnderlineOffset, x_scalar + width,
        y + (text_size * (kUnderlineOffset + (thickness_factor * RenderText::kLineThicknessFactor))));
    canvas_skia_->drawRect(r, flags_);
}

void SkiaTextRenderer::DrawStrike(int x, int y, int width, SkScalar thickness_factor)
{
    const SkScalar text_size = font_.getSize();
...
StyleIterator::StyleIterator(const BreakList<SkColor>* colors, const BreakList<BaselineStyle>* baselines, const BreakList<int>* font_size_overrides,
    const BreakList<Font::Weight>* weights, const BreakList<SkTypefaceID>* resolved_typefaces, const BreakList<cc::PaintFlags::Style>* fill_styles,
    const BreakList<SkScalar>* stroke_widths, const StyleArray* styles)
    : colors_(colors)
...
void StyleIterator::IncrementToPosition(size_t position)
{
    ...
    baseline_ = IncrementBreakListIteratorToPosition(*baselines_, baseline_, position);
    font_size_override_ = IncrementBreakListIteratorToPosition(*font_size_overrides_, font_size_override_, position);
    weight_ = IncrementBreakListIteratorToPosition(*weights_, weight_, position);
    resolved_typeface_ = IncrementBreakListIteratorToPosition(*resolved_typefaces_, resolved_typeface_, position);
    fill_style_ = IncrementBreakListIteratorToPosition(*fill_styles_, fill_style_, position);
    stroke_width_ = IncrementBreakListIteratorToPosition(*stroke_widths_, stroke_width_, position);
    for (size_t i = 0; i < styles_->size(); ++i) {
        style_[i] = IncrementBreakListIteratorToPosition((*styles_)[i], style_[i], position);
    }
}
...
void ApplyRenderParams(const FontRenderParams& params, bool subpixel_rendering_suppressed, SkFont* font)
{
    if (!params.antialiasing) {
        font->setEdging(SkFont::Edging::kAlias);
    } else if (subpixel_rendering_suppressed || params.subpixel_rendering == FontRenderParams::SUBPIXEL_RENDERING_NONE) {
...
std::unique_ptr<RenderText> RenderText::CreateInstanceOfSameStyle(const std::u16string& text) const
{
...
void RenderText::SetFontList(const FontList& font_list)
{
    ...
    styles_[TEXT_STYLE_ITALIC].ClearAndSetInitialValue((font_style & Font::ITALIC) != 0);
    styles_[TEXT_STYLE_UNDERLINE].ClearAndSetInitialValue((font_style & Font::UNDERLINE) != 0);
    styles_[TEXT_STYLE_HEAVY_UNDERLINE].ClearAndSetInitialValue(false);
    styles_[TEXT_STYLE_STRIKE].ClearAndSetInitialValue((font_style & Font::STRIKE_THROUGH) != 0);
...
void RenderText::SetCursorPosition(size_t position)
{
    ...
    if (IsValidCursorIndex(cursor)) {
        SetSelectionModel(SelectionModel(cursor, (cursor == 0) ? CURSOR_FORWARD : CURSOR_BACKWARD));
    }
}

void RenderText::MoveCursor(BreakType break_type, VisualCursorDirection direction, SelectionBehavior selection_behavior)
{
    ...
    if ((!has_directed_selection_ || selection_behavior == SELECTION_NONE) && !selection().is_empty()) {
        ...
        // different lines.
        const bool cursor_is_leading = (start.y() > end.y()) || ((start.y() == end.y()) && (start.x() > end.x()));
        const bool cursor_should_be_trailing = (direction == CURSOR_RIGHT) || (direction == CURSOR_DOWN);
        if (cursor_is_leading == cursor_should_be_trailing) {
            ...
            selection_model_ = SelectionModel(Range(range.end(), range.start()), selection_model_.caret_affinity());
            cursor = selection_start;
        }
    }

    // Cancelling a selection moves to the edge of the selection.
    if (break_type != FIELD_BREAK && break_type != LINE_BREAK && !selection().is_empty() && selection_behavior == SELECTION_NONE) {
    ...
    bool selection_reversed = !selection().is_empty() && min_end <= current_start && current_start <= max_end;
    ...
    case SELECTION_EXTEND:
        cursor.set_selection_start(selection_reversed ? selection().end() : current_start);
        break;
    case SELECTION_CARET:
        if (selection_reversed) {
            cursor = SelectionModel(current_start, selection_model_.caret_affinity());
        } else {
            cursor.set_selection_start(current_start);
        }
        break;
...
bool RenderText::SetSelection(const SelectionModel& model)
{
    ...
    for (auto& range : ranges) {
        range = { std::min(range.start(), text_length), std::min(range.end(), text_length) };
...
bool RenderText::MoveCursorToPoint(const Point& point, bool select, const Point& drag_origin)
{
    reset_cached_cursor_x();
...
{
    size_t text_length = text().length();
    Range sel(std::min(range.start(), text_length), std::min(range.end(), text_length));
...
bool RenderText::IsPointInSelection(const Point& point)
{
    ...
    return RangeContainsCaret(selection(), cursor.caret_pos(), cursor.caret_affinity());
}

void RenderText::ClearSelection()
{
    SetSelectionModel(SelectionModel(cursor_position(), selection_model_.caret_affinity()));
}
...
void RenderText::ApplyFontSizeOverride(int font_size_override, const Range& range)
{
...
void RenderText::ApplyFillStyle(cc::PaintFlags::Style style, const Range& range)
{
...
{
    return GetDisplayTextDirection() == base::i18n::RIGHT_TO_LEFT ? CURSOR_RIGHT : CURSOR_LEFT;
}

Size RenderText::GetStringSize()
{
    const SizeF size_f = GetStringSizeF();
    return Size(base::ClampCeil(size_f.width()), base::ClampCeil(size_f.height()));
}
...
int RenderText::GetBaseline()
{
    if (baseline_ == kInvalidBaseline) {
        const int centering_height = (vertical_alignment_ == ALIGN_MIDDLE) ? display_rect().height() : std::max(font_list().GetHeight(), min_line_height());
...
SelectionModel RenderText::FindCursorPosition(const Point& view_point, const Point& drag_origin)
{
    const internal::ShapedText* shaped_text = GetShapedText();
    ...
    if (RenderText::kDragToEndIfOutsideVerticalBounds && !multiline() && (line_index < 0 || line_index >= static_cast<int>(shaped_text->lines().size()))) {
        SelectionModel selection_start = GetSelectionModelForSelectionStart();
        int edge = drag_origin.x() == 0 ? GetCursorBounds(selection_start, true).x() : drag_origin.x();
    ...
    const int segment_index
        = GetLineSegmentContainingXCoord(line, (view_point - GetLineOffset(line_index) + newline_offset).x(), &point_offset_relative_segment);
    if (segment_index < 0)
    ...
    const size_t segment_min_glyph_index = run.CharRangeToGlyphRange(segment.char_range).GetMin();
    const float segment_offset_relative_run = segment_min_glyph_index != 0 ? SkScalarToFloat(run.shape.positions[segment_min_glyph_index].x()) : 0;
    const float point_offset_relative_run = point_offset_relative_segment + segment_offset_relative_run;
    ...
    for (size_t i = 0; i < run.shape.glyph_count; ++i) {
        const float end = i + 1 == run.shape.glyph_count ? run.shape.width : SkScalarToFloat(run.shape.positions[i + 1].x());
        const float middle = (end + SkScalarToFloat(run.shape.positions[i].x())) / 2;
        const size_t index = DisplayIndexToTextIndex(run.shape.glyph_to_char[i]);
        if (point_offset_relative_run < middle) {
            return run.font_params.is_rtl ? SelectionModel(IndexOfAdjacentGrapheme(index, CURSOR_FORWARD), CURSOR_BACKWARD)
        ...
        if (point_offset_relative_run < end) {
            return run.font_params.is_rtl ? SelectionModel(index, CURSOR_FORWARD)
                                          : SelectionModel(IndexOfAdjacentGrapheme(index, CURSOR_FORWARD), CURSOR_BACKWARD);
        }
    }
...
bool RenderText::IsValidLogicalIndex(size_t index) const
{
    ...
    return index == 0 || index == text().length()
        || (index < text().length() && (truncate_length_ == 0 || index < truncate_length_) && IsValidCodePointIndex(text(), index));
}

bool RenderText::IsValidCursorIndex(size_t index) const
{
    return index == 0 || index == text().length() || (IsValidLogicalIndex(index) && IsGraphemeBoundary(index));
}

Rect RenderText::GetCursorBounds(const SelectionModel& caret, bool insert_mode)
{
    EnsureLayout();
    ...
    const bool at_boundary = (insert_mode && caret_pos == 0) || caret_pos == (caret_affinity == CURSOR_BACKWARD ? 0 : text().length());
    if (at_boundary) {
        ...
        if (insert_mode) {
            ...
            // are always ceiled.
            width = base::ClampCeil(Clamp(xspan.GetMax())) - base::ClampCeil(Clamp(xspan.GetMin()));
        }
    }
...
internal::GraphemeIterator RenderText::GetGraphemeIteratorAtTextIndex(size_t index) const
{
    EnsureLayoutTextUpdated();
    return GetGraphemeIteratorAtIndex(text_, &internal::TextToDisplayIndex::text_index, index);
}

internal::GraphemeIterator RenderText::GetGraphemeIteratorAtDisplayTextIndex(size_t index) const
{
    EnsureLayoutTextUpdated();
    return GetGraphemeIteratorAtIndex(layout_text_, &internal::TextToDisplayIndex::display_index, index);
}

size_t RenderText::GetTextIndex(internal::GraphemeIterator iter) const
{
    DCHECK(layout_text_up_to_date_);
    return iter == text_to_display_indices_.end() ? text_.length() : iter->text_index;
}

size_t RenderText::GetDisplayTextIndex(internal::GraphemeIterator iter) const
{
    DCHECK(layout_text_up_to_date_);
    return iter == text_to_display_indices_.end() ? layout_text_.length() : iter->display_index;
}

bool RenderText::IsGraphemeBoundary(size_t index) const
{
    return index >= text_.length() || GetTextIndex(GetGraphemeIteratorAtTextIndex(index)) == index;
}

size_t RenderText::IndexOfAdjacentGrapheme(size_t index, LogicalCursorDirection direction) const
{
    ...
    internal::GraphemeIterator iter = index == text_.length() ? text_to_display_indices_.end() : GetGraphemeIteratorAtTextIndex(index);
...
SelectionModel RenderText::GetSelectionModelForSelectionStart() const
{
    ...
    return SelectionModel(sel.start(), sel.is_reversed() ? CURSOR_BACKWARD : CURSOR_FORWARD);
}
...
void RenderText::SetDisplayOffset(Vector2d offset)
{
    ...
    const base::ClampedNumeric<int> extra_content = base::ClampedNumeric<int>(GetContentWidth()) - display_rect_.width();
    ...
    const int vertical_offset = std::clamp(offset.y(), std::min(display_rect_.height() - GetStringSize().height(), 0), 0);
...
Vector2d RenderText::GetLineOffset(size_t line_number)
{
    ...
    if (!multiline()) {
        ...
        offset.Add(GetUpdatedDisplayOffset());
        offset.Add(Vector2d(0, shaped_text->lines()[line_number].preceding_heights));
    }
    offset.Add(GetAlignmentOffset(line_number));
    return offset;
}

bool RenderText::GetWordLookupDataAtPoint(const Point& point, DecoratedText* decorated_word, Rect* rect)
{
    if (obscured())
        return false;
    ...
    const size_t word_index = GetNearestWordStartBoundary(model_at_point.caret_pos());
...
bool RenderText::GetLookupDataForRange(const Range& range, DecoratedText* decorated_text, Rect* rect)
{
    const internal::ShapedText* shaped_text = GetShapedText();
    ...
    const auto left_rect = std::min_element(word_bounds.begin(), word_bounds.end(), [](const Rect& lhs, const Rect& rhs) { return lhs.x() < rhs.x(); });
    const int line_index = GetLineContainingYCoord(left_rect->CenterPoint().y() - GetLineOffset(0).y());
    if (line_index < 0 || line_index >= static_cast<int>(shaped_text->lines().size()))
        return false;
    *rect = Rect(left_rect->origin() + Vector2d(0, shaped_text->lines()[line_index].baseline), left_rect->size());
    return true;
}
...
{
    const auto snap_to_grapheme = [this](auto index, auto direction) { return IsValidCursorIndex(index) ? index : IndexOfAdjacentGrapheme(index, direction); };
...
bool RenderText::IsNewlineSegment(const std::u16string& text, const internal::LineSegment& segment) const
{
...
Range RenderText::GetLineRange(const std::u16string& text, const internal::Line& line) const
{
    ...
    // |max_index|.
    if (!line.segments.empty() && (IsNewlineSegment(text, line.segments.back()) || IsNewlineSegment(text, line.segments.front()))) {
        --max_index;
    }
...
{
    return internal::StyleIterator(&colors_, &baselines_, &font_size_overrides_, &weights_, &resolved_typefaces_, &fill_styles_, &stroke_widths_, &styles_);
}

internal::StyleIterator RenderText::GetLayoutTextStyleIterator() const
{
    EnsureLayoutTextUpdated();
    return internal::StyleIterator(&layout_colors_, &layout_baselines_, &layout_font_size_overrides_, &layout_weights_, &layout_resolved_typefaces_,
        &layout_fill_styles_, &layout_stroke_widths_, &layout_styles_);
}

bool RenderText::IsHomogeneous() const
{
    if (colors().breaks().size() > 1 || baselines().breaks().size() > 1 || font_size_overrides().breaks().size() > 1 || weights().breaks().size() > 1) {
        return false;
    }

    return base::ranges::none_of(styles(), [](const auto& style) { return style.breaks().size() > 1; });
}
...
SelectionModel RenderText::GetAdjacentSelectionModel(const SelectionModel& current, BreakType break_type, VisualCursorDirection direction)
{
    EnsureLayout();
...
SelectionModel RenderText::EdgeSelectionModel(VisualCursorDirection direction)
{
...
SelectionModel RenderText::LineSelectionModel(size_t line_index, VisualCursorDirection direction)
{
    ...
    if (line_index == (direction == GetVisualDirectionOfLogicalEnd() ? GetNumLines() - 1 : 0)) {
        return EdgeSelectionModel(direction);
    }
    ...
    return direction == GetVisualDirectionOfLogicalEnd() ? SelectionModel(DisplayIndexToTextIndex(line_range.end()), CURSOR_BACKWARD)
                                                         : SelectionModel(DisplayIndexToTextIndex(line_range.start()), CURSOR_FORWARD);
}
...
void RenderText::EnsureLayoutTextUpdated() const
{
    ...
    // graphemes.
    base::i18n::BreakIterator grapheme_iter(text_, base::i18n::BreakIterator::BREAK_CHARACTER);
    ...
    while (!text_iter.end() && !text_truncated) {
        ...
        do {
            ...
            text_iter.Advance();
        } while (!grapheme_iter.IsGraphemeBoundary(text_iter.array_pos()) && !text_iter.end());
        ...
        internal::TextToDisplayIndex mapping = { text_grapheme_start_position, layout_grapheme_start_position };
        text_to_display_indices_.push_back(mapping);

        // Flag telling if the current grapheme is a newline control sequence.
        const bool is_newline_grapheme = (grapheme_codepoints.size() == 1 && (grapheme_codepoints[0] == '\r' || grapheme_codepoints[0] == '\n'))
        ...
        if (obscured_ && (reveal_index < text_grapheme_start_position || reveal_index >= text_grapheme_end_position) && (!is_newline_grapheme || !multiline_)) {
        ...
        text_truncated = (truncate_length_ != 0
            && ((text_grapheme_end_position > truncate_length_) || (!text_iter.end() && (text_grapheme_end_position == truncate_length_))));

        // If the text is elided, replace it by an ellipsis. Do not append an
        // ellipsis if it was already inserted.
        eliding_iterator = IncrementBreakListIteratorToPosition(elidings_, eliding_iterator, text_grapheme_start_position);
        ...
        for (uint32_t codepoint : grapheme_codepoints) {
            ...
            for (size_t i = 0; i < layout_styles_.size(); ++i) {
                layout_styles_[i].ApplyValue(styles.style(static_cast<TextStyle>(i)), range);
            }

            // Apply an underline to the composition range in |underlines|.
            const Range grapheme_start_range(text_grapheme_start_position, text_grapheme_start_position + 1);
...
void RenderText::UpdateDisplayText(float text_width)
{
    ...
    if ((multiline_ && (!max_lines_ || elide_behavior() != ELIDE_TAIL)) || elide_behavior() == NO_ELIDE || elide_behavior() == FADE_TAIL
        || (text_width > 0 && text_width < display_rect_.width()) || layout_text_.empty()) {
        text_elided_ = false;
        display_text_.clear();
        return;
    }
    ...
    if (!multiline_) {
        ...
        display_text_.assign(Elide(layout_text_, text_width, static_cast<float>(display_rect_.width()), elide_behavior_));
    } else {
        text_elided_ = false;
        display_text_.clear();

        std::unique_ptr<RenderText> render_text(CreateInstanceOfSameStyle(layout_text_));
        ...
        if (render_text->GetShapedText()->lines().size() > max_lines_) {
            ...
            Range line_range = GetLineRange(layout_text_, render_text->GetShapedText()->lines()[max_lines_ - 1]);
            ...
            std::u16string text_to_elide = layout_text_.substr(line_range.start(), line_range.length()) + std::u16string(kEllipsisUTF16);
            display_text_.assign(layout_text_.substr(0, line_range.start()) + Elide(text_to_elide, 0, static_cast<float>(display_rect_.width()), ELIDE_TAIL));
        } else {
...
{
    if (GetNumLines() == 1) {
        return Point(base::ClampCeil(Clamp(point.x())), base::ClampRound(point.y())) + GetLineOffset(0);
    }
    ...
    return Point(base::ClampCeil(Clamp(x)), base::ClampRound(point.y())) + GetLineOffset(line);
}
...
Vector2d RenderText::GetAlignmentOffset(size_t line_number)
{
    ...
    if (horizontal_alignment != ALIGN_LEFT) {
        const int width
            = multiline_ ? base::ClampCeil(GetShapedText()->lines()[line_number].size.width() + (cursor_enabled_ ? 1.0f : 0.0f)) : GetContentWidth();
...
void RenderText::ApplyFadeEffects(internal::SkiaTextRenderer* renderer)
{
    ...
    if (horizontal_alignment != ALIGN_LEFT) {
        ...
        left_part.Inset(gfx::Insets::TLBR(0, 0, 0, solid_part.width() - gradient_width));
        solid_part.Inset(gfx::Insets::TLBR(0, gradient_width, 0, 0));
    }
    if (horizontal_alignment != ALIGN_RIGHT) {
        right_part = solid_part;
        right_part.Inset(gfx::Insets::TLBR(0, solid_part.width() - gradient_width, 0, 0));
    ...
    renderer->SetShader(CreateFadeShader(font_list(), text_rect, left_part, right_part, SkColorSetA(colors_.breaks().front().second, 0xff)));
}
...
base::i18n::TextDirection RenderText::GetTextDirectionForGivenText(const std::u16string& text) const
{
...
// static
bool RenderText::RangeContainsCaret(const Range& range, size_t caret_pos, LogicalCursorDirection caret_affinity)
{
...
// static
int RenderText::DetermineBaselineCenteringText(const int display_height, const FontList& font_list)
{
...
// static
Rect RenderText::ExpandToBeVerticallySymmetric(const Rect& rect, const Rect& display_rect)
{
...
void RenderText::MergeIntersectingRects(std::vector<Rect>& rects)
{
    ...
    for (size_t i = 1; i < rects.size(); i++) {
        if (rects[i].Intersects(rects[merge_candidate]) || rects[i].SharesEdgeWith(rects[merge_candidate])) {
...
std::u16string RenderText::Elide(const std::u16string& text, float text_width, float available_width, ElideBehavior behavior)
{
    ...
    StringSlicer slicer(text, ellipsis, elide_in_middle, elide_at_beginning, whitespace_elision_);
    ...
    while (lo <= hi) {
        ...
        if (hi_width != lo_width) {
            guess = lo + base::ClampRound<size_t>((available_width - lo_width) * (hi - lo) / (hi_width - lo_width));
        }
        ...
        render_text->colors_ = colors_;
        std::u16string new_text = slicer.CutString(guess, insert_ellipsis && behavior != ELIDE_TAIL);
        ...
        if (insert_ellipsis && behavior == ELIDE_TAIL) {
            ...
            if (trailing_text_direction != text_direction && new_text.length() + 2 > text.length() && guess >= 1) {
                new_text = slicer.CutString(guess - 1, false);
                trailing_text_direction = base::i18n::GetLastStrongCharacterDirection(new_text);
            }
...
std::u16string RenderText::ElideEmail(const std::u16string& email, float available_width)
{
    ...
    DCHECK(IsHomogeneous()) << "ElideEmail(...) doesn't work with non homogeneous styles.";
    ...
    const float available_domain_width = remaining_width - std::min(full_username_width, get_string_width(username.substr(0, 1) + kEllipsisUTF16));
    if (get_string_width(domain) > available_domain_width) {
        ...
        const float desired_domain_width = std::min<float>(available_domain_width, std::max<float>(remaining_width - full_username_width, remaining_width / 2));
...
internal::GraphemeIterator RenderText::GetGraphemeIteratorAtIndex(
    const std::u16string& text, const size_t internal::TextToDisplayIndex::*field, size_t index) const
{
    ...
    internal::GraphemeIterator iter = std::lower_bound(text_to_display_indices_.begin(), text_to_display_indices_.end(), index,
        [field](const internal::TextToDisplayIndex& lhs, size_t rhs) { return lhs.*field < rhs; });
...
void RenderText::DrawSelections(Canvas* canvas, const std::vector<Range>& selections)
{
