...
public:
    ...
    DelegatedInkPoint(const PointF& pt, base::TimeTicks timestamp, int32_t pointer_id = std::numeric_limits<int32_t>::min())
        : point_(pt)
    ...
    void set_paint_timestamp(base::TimeTicks ts)
    {
        paint_timestamp_ = ts;
    }
...
private:
    friend struct mojo::StructTraits<mojom::DelegatedInkPointDataView, DelegatedInkPoint>;
