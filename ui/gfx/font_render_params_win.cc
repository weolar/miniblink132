...
FontRenderParams::SubpixelRendering GetSubpixelRenderingGeometry()
{
    ...
    if (key.Valid() && key.ReadValueDW(L"PixelStructure", &pixel_structure) == ERROR_SUCCESS) {
...
public:
    ...
    const FontRenderParams& GetParams()
    {
        ...
        if (SystemParametersInfo(SPI_GETFONTSMOOTHING, 0, &enabled, 0) && enabled) {
            ...
            UINT type = 0;
            if (SystemParametersInfo(SPI_GETFONTSMOOTHINGTYPE, 0, &type, 0) && type == FE_FONTSMOOTHINGCLEARTYPE) {
                params_->subpixel_rendering = GetSubpixelRenderingGeometry();
            }
        }

        if (true/*base::FeatureList::IsEnabled(features::kUseGammaContrastRegistrySettings)*/) {
        ...
        skia::LegacyDisplayGlobals::SetCachedParams(
            FontRenderParams::SubpixelRenderingToSkiaPixelGeometry(params_->subpixel_rendering), params_->text_contrast, params_->text_gamma);

        singleton_hwnd_observer_ = std::make_unique<SingletonHwndObserver>(base::BindRepeating(&CachedFontRenderParams::OnWndProc, base::Unretained(this)));
        return *params_;
    }
