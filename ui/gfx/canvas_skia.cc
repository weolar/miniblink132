...
void ElideTextAndAdjustRange(const FontList& font_list, float width, std::u16string* text, Range* range)
{
    const char16_t start_char = (range->IsValid() ? text->at(range->start()) : u'\0');
...
void UpdateRenderText(const Rect& rect, const std::u16string& text, const FontList& font_list, int flags, SkColor color, RenderText* render_text)
{
    ...
    render_text->set_subpixel_rendering_suppressed((flags & Canvas::NO_SUBPIXEL_RENDERING) != 0);

    render_text->SetColor(color);
    const int font_style = font_list.GetFontStyle();
    render_text->SetStyle(TEXT_STYLE_ITALIC, (font_style & Font::ITALIC) != 0);
    render_text->SetStyle(TEXT_STYLE_UNDERLINE, (font_style & Font::UNDERLINE) != 0);
    render_text->SetStyle(TEXT_STYLE_STRIKE, (font_style & Font::STRIKE_THROUGH) != 0);
...
// static
void Canvas::SizeStringFloat(const std::u16string& text, const FontList& font_list, float* width, float* height, int line_height, int flags)
{
    DCHECK_GE(*width, 0);
    DCHECK_GE(*height, 0);
    ...
    if ((flags & MULTI_LINE) && *width != 0) {
        ...
        ElideRectangleText(text, font_list, *width, INT_MAX, wrap_behavior, &strings);
        Rect rect(base::saturated_cast<int>(*width), INT_MAX);

        std::unique_ptr<RenderText> render_text = RenderText::CreateRenderText();

        UpdateRenderText(rect, std::u16string(), font_list, flags, 0, render_text.get());
        ...
        for (size_t i = 0; i < strings.size(); ++i) {
            ...
            h += (i > 0 && line_height > 0) ? std::max(static_cast<float>(line_height), string_size.height()) : string_size.height();
        }
        *width = w;
        *height = h;
    } else {
        ...
        Rect rect(base::saturated_cast<int>(*width), base::saturated_cast<int>(*height));
        std::u16string adjusted_text = text;
        StripAcceleratorChars(flags, &adjusted_text);
        UpdateRenderText(rect, adjusted_text, font_list, flags, 0, render_text.get());
        const SizeF& string_size = render_text->GetStringSizeF();
...
void Canvas::DrawStringRectWithFlags(const std::u16string& text, const FontList& font_list, SkColor color, const Rect& text_bounds, int flags)
{
    ...
    if (flags & MULTI_LINE) {
        ...
        std::vector<std::u16string> strings;
        ElideRectangleText(text, font_list, static_cast<float>(text_bounds.width()), text_bounds.height(), wrap_behavior, &strings);

        for (size_t i = 0; i < strings.size(); i++) {
            Range range = StripAcceleratorChars(flags, &strings[i]);
            UpdateRenderText(rect, strings[i], font_list, flags, color, render_text.get());
            int line_padding = 0;
        ...
        if (elide_text) {
            ElideTextAndAdjustRange(font_list, static_cast<float>(text_bounds.width()), &adjusted_text, &range);
        }

        UpdateRenderText(rect, adjusted_text, font_list, flags, color, render_text.get());
        if (range.IsValid())
            render_text->ApplyStyle(TEXT_STYLE_UNDERLINE, true, range);
        render_text->Draw(this);
    }
