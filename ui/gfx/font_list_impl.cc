...
std::string BuildDescription(const std::vector<std::string>& families, int style, int size_pixels, Font::Weight weight)
{
...
FontListImpl::FontListImpl(const std::vector<std::string>& font_names, int font_style, int font_size, Font::Weight font_weight)
    : font_description_string_(BuildDescription(font_names, font_style, font_size, font_weight))
    , common_height_(-1)
    , common_baseline_(-1)
...
FontListImpl* FontListImpl::Derive(int size_delta, int font_style, Font::Weight weight) const
{
    ...
    CHECK(FontList::ParseDescription(font_description_string_, &font_names, &old_style, &old_size, &old_weight));
...
const std::vector<Font>& FontListImpl::GetFonts() const
{
    if (fonts_.empty()) {
        ...
        // valid.
        int style = 0;
        CHECK(FontList::ParseDescription(font_description_string_, &font_names, &style, &font_size_, &font_weight_));
        if (font_style_ == -1)
            font_style_ = style;
...
void FontListImpl::CacheFontStyleAndSize() const
{
    if (!fonts_.empty()) {
        ...
        std::vector<std::string> font_names;
        CHECK(FontList::ParseDescription(font_description_string_, &font_names, &font_style_, &font_size_, &font_weight_));
    }
}
