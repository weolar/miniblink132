...
public:
    MockGbmBuffer(
        uint32_t format, uint32_t flags, uint64_t modifier, const gfx::Size& size, std::vector<gfx::NativePixmapPlane> planes, std::vector<uint32_t> handles)
        : format_(format)
...
std::unique_ptr<GbmBuffer> MockGbmDevice::CreateBuffer(uint32_t format, const gfx::Size& size, uint32_t flags)
{
...
std::unique_ptr<GbmBuffer> MockGbmDevice::CreateBufferWithModifiers(
    uint32_t format, const gfx::Size& size, uint32_t flags, const std::vector<uint64_t>& modifiers)
{
    if (should_fail_allocations_)
        return nullptr;
    ...
    uint64_t format_modifier = modifiers.empty() ? DRM_FORMAT_MOD_NONE : modifiers.back();

    if (!base::Contains(supported_modifiers_, format_modifier)) {
        PLOG(ERROR) << "Unsupported format modifier: " << std::hex << format_modifier;
        return nullptr;
    }
    ...
    return std::make_unique<MockGbmBuffer>(format, flags, format_modifier, size, std::move(planes), std::move(handles));
}

std::unique_ptr<GbmBuffer> MockGbmDevice::CreateBufferFromHandle(uint32_t format, const gfx::Size& size, gfx::NativePixmapHandle handle)
{
    NOTREACHED();
}
