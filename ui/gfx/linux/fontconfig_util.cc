...
#if BUILDFLAG(IS_CHROMEOS)
constexpr base::FilePath::CharType kGoogleSansVariablePath[] = FILE_PATH_LITERAL("/usr/share/fonts/google-sans/variable");
constexpr base::FilePath::CharType kGoogleSansStaticPath[] = FILE_PATH_LITERAL("/usr/share/fonts/google-sans/static");

// This should match `imageloader::kImageloaderMountBase` from
// //third_party/cros_system_api/constants/imageloader.h.
constexpr base::FilePath::CharType kImageloaderMountBase[] = FILE_PATH_LITERAL("/run/imageloader/");
#endif // BUILDFLAG(IS_CHROMEOS)
...
public:
    GlobalFontConfig()
    {
        ...
        if (base::PathExists(base::FilePath(kGoogleSansVariablePath))) {
            const FcChar8* kVariableFontPath = reinterpret_cast<const FcChar8*>(kGoogleSansVariablePath);
            ...
            const FcChar8* kStaticFontPath = reinterpret_cast<const FcChar8*>(kGoogleSansStaticPath);
    ...
    ~GlobalFontConfig()
    {
        FcConfigDestroy(fc_config_.ExtractAsDangling());
    }
    ...
    bool AddAppFontDir(const base::FilePath& dir)
    {
        ...
        const FcChar8* dir_fcstring = reinterpret_cast<const FcChar8*>(dir.value().c_str());
...
std::string GetFontConfigPropertyAsString(FcPattern* pattern, const char* property)
{
    FcChar8* text = nullptr;
    if (FcPatternGetString(pattern, property, 0, &text) != FcResultMatch || text == nullptr) {
        return std::string();
    }
...
int GetFontConfigPropertyAsInt(FcPattern* pattern, const char* property, int default_value)
{
    int value = -1;
...
void InitializeGlobalFontConfigAsync()
{
    if (base::ThreadPoolInstance::Get()) {
        base::ThreadPool::PostTask(FROM_HERE, { base::MayBlock(), base::TaskPriority::USER_BLOCKING, base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN },
...
void GetFontRenderParamsFromFcPattern(FcPattern* pattern, FontRenderParams* param_out)
{
    ...
    FcBool fc_bitmap = 0;
    if (FcPatternGetBool(pattern, FC_EMBEDDED_BITMAP, 0, &fc_bitmap) == FcResultMatch) {
