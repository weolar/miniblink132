...
size_t GetSizeOfPlane(gbm_bo* bo, uint32_t format, const gfx::Size& size, size_t plane)
{
#if defined(MINIGBM)
    ...
    const base::CheckedNumeric<size_t> stride_for_plane = GetStrideForPlane(bo, plane);
    const base::CheckedNumeric<size_t> subsampled_height = size.height() / gfx::SubsamplingFactorForBufferFormat(buffer_format, plane);

    // Apply subsampling factor to get size in bytes.
    const base::CheckedNumeric<size_t> checked_plane_size = subsampled_height * stride_for_plane;
...
public:
    Buffer(struct gbm_bo* bo, uint32_t format, uint32_t flags, uint64_t modifier, const gfx::Size& size, gfx::NativePixmapHandle handle)
        : bo_(bo)
        , format_(format)
    ...
    sk_sp<SkSurface> GetSurface() override
    {
        ...
        uint32_t stride;
        void* addr;
        addr = gbm_bo_map(bo_, 0, 0, gbm_bo_get_width(bo_), gbm_bo_get_height(bo_), GBM_BO_TRANSFER_READ_WRITE, &stride, &mmap_data_);

        if (!addr)
            return nullptr;
        SkImageInfo info = SkImageInfo::MakeN32Premul(size_.width(), size_.height());
        SkSurfaceProps props = skia::LegacyDisplayGlobals::GetSkSurfaceProps();
        return SkSurfaces::WrapPixels(info, addr, stride, &Buffer::UnmapGbmBo, this, &props);
    }
...
std::unique_ptr<Buffer> CreateBufferForBO(struct gbm_bo* bo, uint32_t format, const gfx::Size& size, uint32_t flags)
{
    DCHECK(bo);
    ...
    for (size_t i = 0; i < static_cast<size_t>(plane_count); ++i) {
        ...
        handle.planes.emplace_back(GetStrideForPlane(bo, i), GetOffsetForPlane(bo, i), GetSizeOfPlane(bo, format, size, i), std::move(fd));
    }

    handle.modifier = modifier;
    return std::make_unique<Buffer>(bo, format, flags, modifier, size, std::move(handle));
}

class Device final : public ui::GbmDevice {
public:
    Device(gbm_device* device)
        : device_(device)
    {
    }
    ...
    std::unique_ptr<ui::GbmBuffer> CreateBuffer(uint32_t format, const gfx::Size& size, uint32_t flags) override
    {
        struct gbm_bo* bo = gbm_bo_create(device_.get(), size.width(), size.height(), format, flags);
        if (!bo) {
#if DCHECK_IS_ON()
            const char fourcc_as_string[5]
                = { static_cast<char>(format), static_cast<char>(format >> 8), static_cast<char>(format >> 16), static_cast<char>(format >> 24), 0 };

            DVLOG(2) << "Failed to create GBM BO, " << fourcc_as_string << ", " << size.ToString() << ", flags: 0x" << std::hex << flags
                     << "; gbm_device_is_format_supported() = " << gbm_device_is_format_supported(device_.get(), format, flags);
#endif
            return nullptr;
        }
    ...
    std::unique_ptr<ui::GbmBuffer> CreateBufferWithModifiers(
        uint32_t format, const gfx::Size& requested_size, uint32_t flags, const std::vector<uint64_t>& modifiers) override
    {
        if (modifiers.empty()) {
        ...
        while (!valid_modifiers && !filtered_modifiers.empty()) {
            created_bo = gbm_bo_create_with_modifiers(
                device_.get(), size_for_verification.width(), size_for_verification.height(), format, filtered_modifiers.data(), filtered_modifiers.size());
            if (!created_bo) {
                return nullptr;
            }

            const int planes_count = gbm_bo_get_plane_count(created_bo);
            struct gbm_import_fd_modifier_data fd_data = { .width = base::checked_cast<uint32_t>(size_for_verification.width()),
                .height = base::checked_cast<uint32_t>(size_for_verification.height()),
                .format = format,
            ...
            struct gbm_bo* imported_bo = gbm_bo_import(device_.get(), GBM_BO_IMPORT_FD_MODIFIER, &fd_data, flags);
            ...
            if (imported_bo) {
                ...
                filtered_modifiers = GetFilteredModifiers(format, flags, filtered_modifiers);
            }
        ...
        if (valid_modifiers && !created_bo) {
            created_bo = gbm_bo_create_with_modifiers(
                device_.get(), requested_size.width(), requested_size.height(), format, filtered_modifiers.data(), filtered_modifiers.size());
            PLOG_IF(ERROR, !created_bo) << "Failed to create BO with modifiers.";
        }

        // TODO(327768768): Add a test for this about size.
        return created_bo ? CreateBufferForBO(created_bo, format, requested_size, flags) : nullptr;
    }

    std::unique_ptr<ui::GbmBuffer> CreateBufferFromHandle(uint32_t format, const gfx::Size& size, gfx::NativePixmapHandle handle) override
    {
        ...
        // kept open for the lifetime of the buffer.
        struct gbm_bo* bo = gbm_bo_import(device_.get(), GBM_BO_IMPORT_FD_MODIFIER, &fd_data, gbm_flags);
        if (!bo) {
            LOG(ERROR) << "nullptr returned from gbm_bo_import";
            return nullptr;
        }

        return std::make_unique<Buffer>(bo, format, gbm_flags, handle.modifier, size, std::move(handle));
    }
    ...
    int GetSupportedGbmFlags(uint32_t format)
    {
        if (gbm_device_is_format_supported(device_.get(), format, GBM_BO_USE_SCANOUT)) {
...
private:
    std::vector<uint64_t> GetFilteredModifiers(uint32_t format, uint32_t flags, const std::vector<uint64_t>& modifiers)
    {
        std::vector<uint64_t> filtered_modifiers = modifiers;

        for (const auto& [entry_format, entry_flags, entry_modifier] : modifier_blocklist_) {
    ...
    void AddModifierToBlocklist(uint32_t format, uint32_t flags, uint64_t modifier)
    {
