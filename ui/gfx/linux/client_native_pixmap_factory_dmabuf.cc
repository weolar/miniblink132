...
public:
    ...
    ClientNativePixmapFactoryDmabuf(const ClientNativePixmapFactoryDmabuf&) = delete;
    ClientNativePixmapFactoryDmabuf& operator=(const ClientNativePixmapFactoryDmabuf&) = delete;
        ...
        gfx::NativePixmapHandle handle, const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage) override
    {
        ...
        case gfx::BufferUsage::SCANOUT_FRONT_RENDERING: {
            if (!CanFitImageForSizeAndFormat(handle, size, format, /*assume_single_memory_object=*/false)) {
                DLOG(ERROR) << "Failed to verify the size and format of the handle.";
                return nullptr;
            }

            for (size_t i = 0; i < handle.planes.size(); ++i) {
                if (!base::IsValueInRangeForNumericType<size_t>(handle.planes[i].offset)
                ...
                base::CheckedNumeric<uint64_t> size_to_map = base::CheckAdd(handle.planes[i].size, handle.planes[i].offset);
                if (!size_to_map.IsValid<size_t>())
                    return nullptr;
                if (size_to_map.ValueOrDie<size_t>() > base::checked_cast<size_t>(dma_buf_size)) {
                    return nullptr;
                }
            }
            return ClientNativePixmapDmaBuf::ImportFromDmabuf(std::move(handle), size, format);
        }
        ...
        case gfx::BufferUsage::PROTECTED_SCANOUT_VDA_WRITE:
            return base::WrapUnique(new ClientNativePixmapOpaque(std::move(handle)));
        }
        NOTREACHED();
    }
};
