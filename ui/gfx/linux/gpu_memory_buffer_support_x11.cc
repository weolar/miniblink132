...
std::vector<gfx::BufferUsageAndFormat> CreateSupportedConfigList(ui::GbmDevice* device)
{
    if (!device)
        return {};
    ...
    for (gfx::BufferUsage usage : {
        ...
        for (gfx::BufferFormat format : {
            ...
            if (device->CreateBuffer(GetFourCCFormatFromBufferFormat(format), gfx::Size(2, 2), BufferUsageToGbmFlags(usage))) {
...
std::unique_ptr<GbmBuffer> GpuMemoryBufferSupportX11::CreateBuffer(gfx::BufferFormat format, const gfx::Size& size, gfx::BufferUsage usage)
{
    if (!device_) {
        LOG(ERROR) << "Can't create buffer -- gbm  device is missing.";
        return nullptr;
    }
    if (!base::Contains(supported_configs_, gfx::BufferUsageAndFormat(usage, format))) {
        LOG(ERROR) << "Can't create buffer -- unsupported config: usage=" << gfx::BufferUsageToString(usage)
                   << ", format=" << gfx::BufferFormatToString(format);
        return nullptr;
    }

    static base::debug::CrashKeyString* crash_key_string = base::debug::AllocateCrashKeyString("buffer_usage_and_format", base::debug::CrashKeySize::Size64);
    std::string buffer_usage_and_format = gfx::BufferFormatToString(format) + std::string(",") + gfx::BufferUsageToString(usage);
    base::debug::ScopedCrashKeyString scoped_crash_key(crash_key_string, buffer_usage_and_format.c_str());

    return device_->CreateBuffer(GetFourCCFormatFromBufferFormat(format), size, BufferUsageToGbmFlags(usage));
}

bool GpuMemoryBufferSupportX11::CanCreateNativePixmapForFormat(gfx::BufferFormat format)
{
    return device_ && device_->CanCreateBufferForFormat(GetFourCCFormatFromBufferFormat(format));
}

std::unique_ptr<GbmBuffer> GpuMemoryBufferSupportX11::CreateBufferFromHandle(const gfx::Size& size, gfx::BufferFormat format, gfx::NativePixmapHandle handle)
{
    if (!device_) {
    ...
    static base::debug::CrashKeyString* crash_key_string = base::debug::AllocateCrashKeyString("buffer_from_handle_format", base::debug::CrashKeySize::Size64);
    std::string buffer_from_handle_format = gfx::BufferFormatToString(format);
    base::debug::ScopedCrashKeyString scoped_crash_key(crash_key_string, buffer_from_handle_format.c_str());

    return device_->CreateBufferFromHandle(GetFourCCFormatFromBufferFormat(format), size, std::move(handle));
}
