...
{
    std::optional<std::vector<uint8_t>> png_data = gfx::PNGCodec::EncodeBGRASkBitmap(bitmap,
        /*discard_transparency=*/false);
    return base::StrCat({ "data:image/png;base64,", base::Base64Encode(png_data.value_or(std::vector<uint8_t>())) });
}

void ExpectRedWithGreenRect(const SkBitmap& bitmap, const Rect& outer_rect, const Rect& green_rect)
{
    for (int y = outer_rect.y(); y < outer_rect.bottom(); y++) {
        ...
        for (int x = outer_rect.x(); x < outer_rect.right(); x++) {
            ...
            if (green_rect.Contains(x, y)) {
                ASSERT_EQ(SK_ColorGREEN, bitmap.getColor(x, y)) << "Output image:\n" << GetPNGDataUrl(bitmap);
            } else {
...
TEST(NineImagePainterTest, PaintHighDPI)
{
    ...
    gfx::Vector2d paint_offset = gfx::ToFlooredVector2d(gfx::ScaleVector2d(offset, image_scale));
