...
bool BuildResizedImageFamily(const gfx::ImageFamily& image_family, gfx::ImageFamily* resized_image_family)
{
...
void ConvertImageFamilyToBitmaps(const gfx::ImageFamily& image_family, std::vector<SkBitmap>* bitmaps, scoped_refptr<base::RefCountedMemory>* png_bytes)
{
    DCHECK(bitmaps);
    DCHECK(bitmaps->empty());

    for (gfx::ImageFamily::const_iterator it = image_family.begin(); it != image_family.end(); ++it) {
...
base::win::ScopedHICON IconUtil::CreateHICONFromSkBitmap(const SkBitmap& bitmap)
{
    // Only 32 bit ARGB bitmaps are supported. We also try to perform as many
    // validations as we can on the bitmap.
    if ((bitmap.colorType() != kN32_SkColorType) || (bitmap.width() <= 0) || (bitmap.height() <= 0) || (bitmap.getPixels() == nullptr))
    ...
    base::win::ScopedBitmap dib;
    {
        ...
        dib = base::win::ScopedBitmap(::CreateDIBSection(hdc, reinterpret_cast<BITMAPINFO*>(&bitmap_header), DIB_RGB_COLORS, &bits, nullptr, 0));
    }
    ...
    bool bitmap_has_alpha_channel = PixelsHaveAlpha(static_cast<const uint32_t*>(bitmap.getPixels()), bitmap.width() * bitmap.height());
    ...
    base::win::ScopedBitmap mono_bitmap(::CreateBitmap(bitmap.width(), bitmap.height(), 1, 1, mask_bits.get()));
...
// static
std::unique_ptr<gfx::ImageFamily> IconUtil::CreateImageFamilyFromIconResource(HMODULE module, int resource_id)
{
    ...
    if (!base::win::GetResourceFromModule(module, resource_id, RT_GROUP_ICON, &icon_dir_data, &icon_dir_size)) {
        return nullptr;
    }
    ...
    for (size_t i = 0; i < icon_dir->idCount; ++i) {
        ...
        if (entry->bWidth != 0 || entry->bHeight != 0) {
            ...
            base::win::ScopedHICON icon_handle(static_cast<HICON>(
                LoadImage(module, MAKEINTRESOURCE(resource_id), IMAGE_ICON, entry->bWidth, entry->bHeight, LR_DEFAULTCOLOR | LR_DEFAULTSIZE)));
            result->Add(gfx::Image::CreateFrom1xBitmap(IconUtil::CreateSkBitmapFromHICON(icon_handle.get())));
        } else {
            // 256x256 icons are stored with width and height set to 0.
            // See: http://en.wikipedia.org/wiki/ICO_(file_format)
            void* png_data = nullptr;
            size_t png_size = 0;
            if (!base::win::GetResourceFromModule(module, entry->nID, RT_ICON, &png_data, &png_size)) {
                return nullptr;
            }
            DCHECK(png_data);
            DCHECK_EQ(png_size, entry->dwBytesInRes);

            result->Add(gfx::Image::CreateFrom1xPNGBytes(new base::RefCountedStaticMemory(UNSAFE_TODO(base::span(static_cast<uint8_t*>(png_data), png_size)))));
        }
    }
    return result;
}
...
base::win::ScopedHICON IconUtil::CreateCursorFromSkBitmap(const SkBitmap& bitmap, const gfx::Point& hotspot)
{
    if (bitmap.empty())
        return base::win::ScopedHICON();
...
// static
SkBitmap IconUtil::CreateSkBitmapFromHICONHelper(HICON icon, const gfx::Size& s)
{
    DCHECK(icon);
    DCHECK(!s.IsEmpty());
    ...
    {
        base::win::ScopedGetDC hdc(nullptr);
        dib = base::win::ScopedBitmap(::CreateDIBSection(hdc, reinterpret_cast<BITMAPINFO*>(&h), DIB_RGB_COLORS, &bits, nullptr, 0));
    ...
    ::DrawIconEx(dib_dc.Get(), 0, 0, icon, s.width(), s.height(), 0, nullptr, DI_MASK);
    ...
    memset(bits, 0, num_pixels * 4);
    ::DrawIconEx(dib_dc.Get(), 0, 0, icon, s.width(), s.height(), 0, nullptr, DI_NORMAL);
    memcpy(bitmap.getPixels(), bits, num_pixels * 4);

    // Finding out whether the bitmap has an alpha channel.
    bool bitmap_has_alpha_channel = PixelsHaveAlpha(static_cast<const uint32_t*>(bitmap.getPixels()), num_pixels);
...
// static
bool IconUtil::CreateIconFileFromImageFamily(const gfx::ImageFamily& image_family, const base::FilePath& icon_path, WriteType write_type)
{
    ...
    DCHECK_LE(bitmaps.size(), static_cast<size_t>(USHRT_MAX - (png_bytes.get() ? 1 : 0)));
    WORD bitmap_count = static_cast<WORD>(bitmaps.size()); // Not including PNG image.
    ...
    for (size_t i = 0; i < bitmap_count; i++) {
        ...
        SetSingleIconImageInformation(bitmaps[i], i, icon_dir, image, offset, &icon_image_size);
    ...
    if (write_type == NORMAL_WRITE) {
        if (base::WriteFile(icon_path, base::span<const uint8_t>(buffer.data(), buffer.size())))
            return true;
...
void IconUtil::SetSingleIconImageInformation(
    const SkBitmap& bitmap, size_t index, ICONDIR* icon_dir, ICONIMAGE* icon_image, DWORD image_offset, size_t* image_byte_count)
{
    DCHECK(icon_dir);
    DCHECK(icon_image);
...
void IconUtil::CopySkBitmapBitsIntoIconBuffer(const SkBitmap& bitmap, unsigned char* buffer, size_t buffer_size)
{
    ...
    for (size_t i = 0; i < bitmap_size; i += bitmap.width() * 4) {
        memcpy(buffer + bitmap_size - bitmap.width() * 4 - i, bitmap_ptr + i, bitmap.width() * 4);
    }
}
...
void IconUtil::ComputeBitmapSizeComponents(const SkBitmap& bitmap, size_t* xor_mask_size, DWORD* bytes_in_resource)
{
    ...
    *bytes_in_resource = static_cast<DWORD>(masks_size + sizeof(BITMAPINFOHEADER));
}
