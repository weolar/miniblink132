...
public:
    ...
    // constructor.
    explicit PlatformFontMac(SystemFontType system_font_type, int font_size = kDefaultFontSize);
    ...
    PlatformFontMac(sk_sp<SkTypeface> typeface, int font_size_pixels, const std::optional<FontRenderParams>& params);

    PlatformFontMac(const PlatformFontMac&) = delete;
    PlatformFontMac& operator=(const PlatformFontMac&) = delete;

    // Overridden from PlatformFont:
    Font DeriveFont(int size_delta, int style, Font::Weight weight) const override;
...
private:
    ...
    PlatformFontMac(CTFontRef font, std::optional<SystemFontType> system_font_type);

    PlatformFontMac(CTFontRef font, std::optional<SystemFontType> system_font_type, FontSpec spec);
    ...
    static base::apple::ScopedCFTypeRef<CTFontRef> CTFontWithSpec(FontSpec font_spec);
