...
int FontWeightToFCWeight(Font::Weight weight)
{
    ...
    if (weight_number <= (static_cast<int>(Font::Weight::THIN) + static_cast<int>(Font::Weight::EXTRA_LIGHT)) / 2)
        return FC_WEIGHT_THIN;
    else if (weight_number <= (static_cast<int>(Font::Weight::EXTRA_LIGHT) + static_cast<int>(Font::Weight::LIGHT)) / 2)
        return FC_WEIGHT_ULTRALIGHT;
    else if (weight_number <= (static_cast<int>(Font::Weight::LIGHT) + static_cast<int>(Font::Weight::NORMAL)) / 2)
        return FC_WEIGHT_LIGHT;
    else if (weight_number <= (static_cast<int>(Font::Weight::NORMAL) + static_cast<int>(Font::Weight::MEDIUM)) / 2)
        return FC_WEIGHT_NORMAL;
    else if (weight_number <= (static_cast<int>(Font::Weight::MEDIUM) + static_cast<int>(Font::Weight::SEMIBOLD)) / 2)
        return FC_WEIGHT_MEDIUM;
    else if (weight_number <= (static_cast<int>(Font::Weight::SEMIBOLD) + static_cast<int>(Font::Weight::BOLD)) / 2)
        return FC_WEIGHT_DEMIBOLD;
    else if (weight_number <= (static_cast<int>(Font::Weight::BOLD) + static_cast<int>(Font::Weight::EXTRA_BOLD)) / 2)
        return FC_WEIGHT_BOLD;
    else if (weight_number <= (static_cast<int>(Font::Weight::EXTRA_BOLD) + static_cast<int>(Font::Weight::BLACK)) / 2)
...
{
    return base::StringPrintf("%d|%d|%d|%d|%s|%f", query.pixel_size, query.point_size, query.style, static_cast<int>(query.weight),
        base::JoinString(query.families, ",").c_str(), query.device_scale_factor);
}

} // namespace

bool QueryFontconfig(const FontRenderParamsQuery& query, FontRenderParams* params_out, std::string* family_out)
{
    ...
    for (auto it = query.families.begin(); it != query.families.end(); ++it) {
        FcPatternAddString(query_pattern.get(), FC_FAMILY, reinterpret_cast<const FcChar8*>(it->c_str()));
    }
    ...
    if (query.style >= 0) {
        FcPatternAddInteger(query_pattern.get(), FC_SLANT, (query.style & Font::ITALIC) ? FC_SLANT_ITALIC : FC_SLANT_ROMAN);
    }
    if (query.weight != Font::Weight::INVALID) {
        FcPatternAddInteger(query_pattern.get(), FC_WEIGHT, FontWeightToFCWeight(query.weight));
    }
    ...
    if (query.is_empty()) {
        ...
        FcConfigSubstituteWithPat(config, result_pattern.get(), query_pattern.get(), FcMatchFont);
    } else {
...
FontRenderParams GetFontRenderParams(const FontRenderParamsQuery& query, std::string* family_out)
{
    ...
    } else if (!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableFontSubpixelPositioning)) {
#if BUILDFLAG(IS_CHROMEOS)
        // We want to enable subpixel positioning for fractional dsf.
        params.subpixel_positioning
            = std::abs(std::round(actual_query.device_scale_factor) - actual_query.device_scale_factor) > std::numeric_limits<float>::epsilon();
#else
...

    {
        ...
        base::AutoLock lock(synchronized_cache->lock);
        synchronized_cache->cache.Put(query_key, QueryResult(params, family_out ? *family_out : std::string()));
    }
