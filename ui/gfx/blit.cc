...
void ScrollCanvas(SkCanvas* canvas, const gfx::Rect& in_clip, const gfx::Vector2d& offset)
{
    ...
    gfx::Rect clip = gfx::IntersectRects(in_clip, gfx::Rect(0, 0, pixmap.width(), pixmap.height()));
    ...
    if (offset.y() > 0) {
        ...
        for (int y = dest_rect.height() - 1; y >= 0; y--) {
            memcpy(pixmap.writable_addr32(dest_rect.x(), dest_rect.y() + y), pixmap.addr32(src_rect.x(), src_rect.y() + y), row_bytes);
        }
    } else if (offset.y() < 0) {
        ...
        for (int y = 0; y < dest_rect.height(); y++) {
            memmove(pixmap.writable_addr32(dest_rect.x(), dest_rect.y() + y), pixmap.addr32(src_rect.x(), src_rect.y() + y), row_bytes);
        }
    }
}
