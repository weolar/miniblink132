...
std::u16string ElideEmail(const std::u16string& email, const FontList& font_list, float available_pixel_width)
{
    ...
    const float available_domain_width
        = available_pixel_width - std::min(full_username_width, GetStringWidthF(username.substr(0, 1) + kEllipsisUTF16, font_list));
    if (GetStringWidthF(domain, font_list) > available_domain_width) {
        ...
        const float desired_domain_width = std::min(available_domain_width, std::max(available_pixel_width - full_username_width, available_pixel_width / 2));
...
bool GetDefaultWhitespaceElision(bool elide_in_middle, bool elide_at_beginning)
{
...
StringSlicer::StringSlicer(
    const std::u16string& text, const std::u16string& ellipsis, bool elide_in_middle, bool elide_at_beginning, std::optional<bool> elide_whitespace)
    : text_(text)
    , ellipsis_(ellipsis)
    , elide_in_middle_(elide_in_middle)
    , elide_at_beginning_(elide_at_beginning)
    , elide_whitespace_(elide_whitespace ? *elide_whitespace : GetDefaultWhitespaceElision(elide_in_middle, elide_at_beginning))
{
}

std::u16string StringSlicer::CutString(size_t length, bool insert_ellipsis) const
{
    const std::u16string ellipsis_text = insert_ellipsis ? *ellipsis_ : std::u16string();
    ...
    if (elide_at_beginning_) {
        return ellipsis_text + text_->substr(FindValidBoundaryAfter(*text_, text_->length() - length, elide_whitespace_));
    }

    if (!elide_in_middle_) {
        return text_->substr(0, FindValidBoundaryBefore(*text_, length, elide_whitespace_)) + ellipsis_text;
    }
    ...
    const size_t prefix_length = FindValidBoundaryBefore(*text_, length - half_length, elide_whitespace_);
    const size_t suffix_start = FindValidBoundaryAfter(*text_, text_->length() - half_length, elide_whitespace_);
    return text_->substr(0, prefix_length) + ellipsis_text + text_->substr(suffix_start);
}

std::u16string ElideFilename(const base::FilePath& filename, const FontList& font_list, float available_pixel_width)
{
#if BUILDFLAG(IS_WIN)
    std::u16string filename_utf16 = WideToUTF16(filename.value());
    std::u16string extension = WideToUTF16(filename.Extension());
    std::u16string rootname = WideToUTF16(filename.BaseName().RemoveExtension().value());
#elif BUILDFLAG(IS_POSIX) || BUILDFLAG(IS_FUCHSIA)
    std::u16string filename_utf16 = WideToUTF16(base::SysNativeMBToWide(filename.value()));
    std::u16string extension = WideToUTF16(base::SysNativeMBToWide(filename.Extension()));
    std::u16string rootname = WideToUTF16(base::SysNativeMBToWide(filename.BaseName().RemoveExtension().value()));
#endif
    ...
    if (rootname.empty() || extension.empty()) {
        const std::u16string elided_name = ElideText(filename_utf16, font_list, available_pixel_width, ELIDE_TAIL);
    ...
    if (ext_width >= available_pixel_width) {
        const std::u16string elided_name = ElideText(rootname + extension, font_list, available_pixel_width, ELIDE_MIDDLE);
        return base::i18n::GetDisplayStringInLTRDirectionality(elided_name);
    }

    float available_root_width = available_pixel_width - ext_width;
    std::u16string elided_name = ElideText(rootname, font_list, available_root_width, ELIDE_TAIL);
    elided_name += extension;
    return base::i18n::GetDisplayStringInLTRDirectionality(elided_name);
}

std::u16string ElideText(const std::u16string& text, const FontList& font_list, float available_pixel_width, ElideBehavior behavior)
{
#if !BUILDFLAG(IS_IOS)
    ...
    render_text->SetDisplayRect(gfx::ToEnclosingRect(gfx::RectF(gfx::SizeF(available_pixel_width, 1))));
    render_text->SetElideBehavior(behavior);
    render_text->SetText(text);
    return render_text->GetDisplayText();
#else
    DCHECK_NE(behavior, FADE_TAIL);
    if (text.empty() || behavior == FADE_TAIL || behavior == NO_ELIDE || GetStringWidthF(text, font_list) <= available_pixel_width) {
        return text;
    }
    ...
    if (insert_ellipsis && GetStringWidthF(ellipsis, font_list) > available_pixel_width)
...
bool ElideString(const std::u16string& input, size_t max_len, std::u16string* output)
{
    ...
    case 2:
        ...
        break;
    case 3:
        output->assign(input.substr(0, 1) + u"." + input.substr(input.length() - 1));
        break;
    case 4:
        output->assign(input.substr(0, 1) + u".." + input.substr(input.length() - 1));
        break;
    default: {
        ...
        output->assign(input.substr(0, lstr_len) + u"..." + input.substr(input.length() - rstr_len));
        break;
    }
    }
...
public:
    RectangleString(size_t max_rows, size_t max_cols, bool strict, std::u16string* output)
        : max_rows_(max_rows)
        , max_cols_(max_cols)
        , current_row_(0)
        , current_col_(0)
...
void RectangleString::AddLine(const std::u16string& line)
{
    if (line.length() < max_cols_) {
        ...
        base::i18n::BreakIterator words(line, base::i18n::BreakIterator::BREAK_SPACE);
        if (words.Init()) {
            while (words.Advance())
...
public:
    RectangleText(
        const FontList& font_list, float available_pixel_width, int available_pixel_height, WordWrapBehavior wrap_behavior, std::vector<std::u16string>* lines)
...
int RectangleText::Finalize()
{
    ...
    return (insufficient_width_ ? INSUFFICIENT_SPACE_HORIZONTAL : 0) | (insufficient_height_ ? INSUFFICIENT_SPACE_VERTICAL : 0)
            ...
            while (words.Advance()) {
                ...
                if (lines_added) {
                    if (truncate) {
                        ...
                        base::TrimWhitespace(lines_->at(new_line), base::TRIM_TRAILING, &lines_->at(new_line));
                    }
...
int RectangleText::WrapWord(const std::u16string& word)
{
    ...
    while (!insufficient_height_ && !text.empty()) {
        std::u16string fragment = ElideText(text, *font_list_, available_pixel_width_, TRUNCATE);
...
int RectangleText::AddWordOverflow(const std::u16string& word)
{
    ...
    if (wrap_behavior_ == IGNORE_LONG_WORDS) {
        ...
        const ElideBehavior elide_behavior = (wrap_behavior_ == ELIDE_LONG_WORDS ? ELIDE_TAIL : TRUNCATE);
        const std::u16string elided_word = ElideText(word, *font_list_, available_pixel_width_, elide_behavior);
...
int RectangleText::AddWord(const std::u16string& word)
{
    ...
    if (trimmed_width <= available_pixel_width_) {
        ...
        lines_added = AddWordOverflow(wrap_behavior_ == IGNORE_LONG_WORDS ? trimmed : word);
    }
    return lines_added;
}
...
void RectangleText::AddToCurrentLineWithWidth(const std::u16string& text, float text_width)
{
...
bool ElideRectangleString(const std::u16string& input, size_t max_rows, size_t max_cols, bool strict, std::u16string* output)
{
...
int ElideRectangleText(const std::u16string& input, const FontList& font_list, float available_pixel_width, int available_pixel_height,
    WordWrapBehavior wrap_behavior, std::vector<std::u16string>* lines)
{
    RectangleText rect(font_list, available_pixel_width, available_pixel_height, wrap_behavior, lines);
    rect.Init();
    rect.AddString(input);
    return rect.Finalize();
}

std::u16string TruncateString(const std::u16string& string, size_t length, BreakType break_type)
{
    ...
    if (word_break) {
        ...
        std::unique_ptr<icu::BreakIterator> bi(icu::RuleBasedBreakIterator::createWordInstance(icu::Locale::getDefault(), status));
        if (U_FAILURE(status))
    ...
    while (char_iterator.hasPrevious()) {
        ...
        if (!(u_isspace(char_iterator.current()) || u_charType(char_iterator.current()) == U_CONTROL_CHAR
