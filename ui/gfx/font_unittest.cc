...
TEST_F(FontTest, LoadArial)
{
    ...
    EXPECT_EQ(base::ToLowerASCII(kTestFontName), base::ToLowerASCII(cf.GetActualFontName()));
}
...
TEST_F(FontTest, GetActualFontName)
{
    ...
    EXPECT_EQ(base::ToLowerASCII(kTestFontName), base::ToLowerASCII(arial.GetActualFontName()))
        << "********\n"
    ...
    EXPECT_EQ(base::ToLowerASCII(kSymbolFontName), base::ToLowerASCII(symbol.GetActualFontName()))
        << "********\n"
        << "Your test environment seems to be missing the " << kSymbolFontName << " font, which is "
        << "needed for unittests.  Check if " << kSymbolFontName << " font is installed.\n"
        << "********";

    const char* const invalid_font_name = "no_such_font_name";
    Font fallback_font(invalid_font_name, 16);
    EXPECT_NE(invalid_font_name, base::ToLowerASCII(fallback_font.GetActualFontName()));
}
...
TEST_F(FontTest, DeriveFont)
{
    ...
    Font cf_underlined = cf.Derive(0, cf.GetStyle() | gfx::Font::UNDERLINE, cf.GetWeight());
    Font cf_underlined_resized = cf_underlined.Derive(kSizeDelta, cf_underlined.GetStyle(), cf_underlined.GetWeight());
    EXPECT_EQ(cf.GetStyle() | gfx::Font::UNDERLINE, cf_underlined_resized.GetStyle());
...
TEST_F(FontTest, WeightConversion)
{
    ...
    } expectations[] = { { -10, Font::Weight::INVALID }, { -1, Font::Weight::INVALID }, { 0, Font::Weight::THIN }, { 1, Font::Weight::THIN },
        { 100, Font::Weight::THIN }, { 350, Font::Weight::NORMAL }, { 400, Font::Weight::NORMAL }, { 899, Font::Weight::BLACK }, { 900, Font::Weight::BLACK },
        { 901, Font::Weight::INVALID } };
