...
{
    return std::max({ std::abs(u.x - v.x), std::abs(u.y - v.y), std::abs(u.z - v.z), std::abs(u.w - v.w) });
}
...
TEST(ColorSpace, RangeAdjust)
{
    ...
    ColorSpace color_spaces[kNumColorSpaces] = {
        ...
        ColorSpace(ColorSpace::PrimaryID::INVALID, ColorSpace::TransferID::INVALID, ColorSpace::MatrixID::YCOCG, ColorSpace::RangeID::LIMITED),
    };
...
TEST(ColorSpace, ConversionToAndFromSkColorSpace)
{
    ...
    ColorSpace color_spaces[] = {
        ...
        ColorSpace(ColorSpace::PrimaryID::ADOBE_RGB, ColorSpace::TransferID::SRGB),
    ...
    for (size_t i = 0; i < std::size(color_spaces); ++i) {
        EXPECT_TRUE(SkColorSpace::Equals(color_spaces[i].ToSkColorSpace().get(), sk_color_spaces[i].get())) << " on iteration i = " << i;
    }
    ...
    // representation.
    for (size_t i = 0; i < std::size(color_spaces); ++i) {
        const ColorSpace from_sk_color_space(*sk_color_spaces[i], color_spaces[i].IsHDR());
        EXPECT_EQ(color_spaces[i], from_sk_color_space);
        EXPECT_TRUE(SkColorSpace::Equals(sk_color_spaces[i].get(), from_sk_color_space.ToSkColorSpace().get()));
    }
}
...
TEST(ColorSpace, PQAndHLGToSkColorSpace)
{
    ...
    for (size_t i = 0; i < kNumCases; ++i) {
        ...
        EXPECT_NEAR(1.f, skcms_TransferFunction_eval(&pq_fn, pq_signal[i]), kEpsilon);
        ...
        EXPECT_NEAR(skcms_TransferFunction_eval(&pq_fn, kPQSignalFor203Nits), skcms_TransferFunction_eval(&hlg_fn, kHLGSignalFor203Nits), kEpsilon);
    }
}
...
TEST(ColorSpace, MixedInvalid)
{
    ...
    color_space = color_space.GetWithMatrixAndRange(ColorSpace::MatrixID::INVALID, ColorSpace::RangeID::INVALID);
    EXPECT_TRUE(!color_space.IsValid());
    color_space = color_space.GetWithMatrixAndRange(ColorSpace::MatrixID::SMPTE170M, ColorSpace::RangeID::LIMITED);
    EXPECT_TRUE(!color_space.IsValid());
}
...
TEST(ColorSpace, MixedSRGBWithRec601)
{
    const ColorSpace expected_color_space
        = ColorSpace(ColorSpace::PrimaryID::BT709, ColorSpace::TransferID::SRGB, ColorSpace::MatrixID::SMPTE170M, ColorSpace::RangeID::LIMITED);
...
{
    const ColorSpace expected_color_space
        = ColorSpace(ColorSpace::PrimaryID::BT2020, ColorSpace::TransferID::PQ, ColorSpace::MatrixID::BT709, ColorSpace::RangeID::LIMITED);
    ColorSpace color_space = ColorSpace::CreateHDR10();
    color_space = color_space.GetWithMatrixAndRange(ColorSpace::MatrixID::BT709, ColorSpace::RangeID::LIMITED);
    EXPECT_TRUE(expected_color_space.IsValid());
    EXPECT_EQ(color_space, expected_color_space);
}
...
{
    ColorSpace color_space(ColorSpace::PrimaryID::BT709, ColorSpace::TransferID::BT709, ColorSpace::MatrixID::BT709, ColorSpace::RangeID::LIMITED);
...
TEST(ColorSpace, ExpectationsMatchSRGB)
{
    ...
    ColorSpace custom_srgb = ColorSpace::CreateCustom(to_XYZD50, ColorSpace::TransferID::SRGB);
