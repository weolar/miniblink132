...
float COLOR_SPACE_EXPORT SkTransferFnEvalUnclamped(const skcms_TransferFunction& fn, float x);

// TODO(crbug.com/40944641): Replace this function with
// skcms_TransferFunction_invert.
skcms_TransferFunction COLOR_SPACE_EXPORT SkTransferFnInverse(const skcms_TransferFunction& fn);
...
// accurate.
bool COLOR_SPACE_EXPORT SkTransferFnsApproximatelyCancel(const skcms_TransferFunction& a, const skcms_TransferFunction& b);
...
bool COLOR_SPACE_EXPORT SkTransferFnIsApproximatelyIdentity(const skcms_TransferFunction& fn);

// Returns true if `m` is within `epsilon` of the identity in the L-infinity
// sense.
bool COLOR_SPACE_EXPORT SkM44IsApproximatelyIdentity(const SkM44& m, float epsilon = 1.f / 256.f);
