...
struct TestPixelMap {
    ...
    {
        std::transform(values.begin(), values.end(), pixels.begin(), &TestPixelMap::ExpandByte);
    }

    uint32_t AtXY(size_t x, size_t y) const
    {
...
{
    ASSERT_EQ(TestPixelMap::kHeight, base::checked_cast<size_t>(canvas->imageInfo().height()));
    ASSERT_EQ(TestPixelMap::kWidth, base::checked_cast<size_t>(canvas->imageInfo().width()));

    SkImageInfo info = SkImageInfo::MakeN32Premul(TestPixelMap::kWidth, TestPixelMap::kHeight);
    canvas->writePixels(info, values.pixels.data(), TestPixelMap::kWidth * 4, 0, 0);
}
...
TEST(Blit, ScrollCanvas)
{
    ...
    std::unique_ptr<SkCanvas> canvas = skia::CreatePlatformCanvas(kCanvasWidth, kCanvasHeight, false);
    const TestPixelMap initial_values({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x10, 0x11, 0x12, 0x13, 0x14, 0x20, 0x21, 0x22, 0x23, 0x24, 0x30, 0x31, 0x32, 0x33, 0x34,
    ...
    gfx::ScrollCanvas(canvas.get(), gfx::Rect(0, 0, kCanvasWidth, kCanvasHeight), gfx::Vector2d(0, 0));
    VerifyCanvasValues(canvas.get(), initial_values);
    ...
    const TestPixelMap scroll_up_expected({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x10, 0x21, 0x22, 0x23, 0x14, 0x20, 0x31, 0x32, 0x33, 0x24, 0x30, 0x31, 0x32, 0x33,
        0x34, 0x40, 0x41, 0x42, 0x43, 0x44 });
    VerifyCanvasValues(canvas.get(), scroll_up_expected);

    // Reset and scroll the center 3 pixels down one.
    SetToCanvas(canvas.get(), initial_values);
    gfx::ScrollCanvas(canvas.get(), center_three, gfx::Vector2d(0, 1));
    const TestPixelMap scroll_down_expected({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x10, 0x11, 0x12, 0x13, 0x14, 0x20, 0x11, 0x12, 0x13, 0x24, 0x30, 0x21, 0x22, 0x23,
        0x34, 0x40, 0x41, 0x42, 0x43, 0x44 });
    ...
    const TestPixelMap scroll_right_expected({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x10, 0x11, 0x11, 0x12, 0x14, 0x20, 0x21, 0x21, 0x22, 0x24, 0x30, 0x31, 0x31, 0x32,
        0x34, 0x40, 0x41, 0x42, 0x43, 0x44 });
    ...
    SetToCanvas(canvas.get(), initial_values);
    gfx::ScrollCanvas(canvas.get(), center_three, gfx::Vector2d(-1, 0));
    const TestPixelMap scroll_left_expected({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x10, 0x12, 0x13, 0x13, 0x14, 0x20, 0x22, 0x23, 0x23, 0x24, 0x30, 0x32, 0x33, 0x33,
        0x34, 0x40, 0x41, 0x42, 0x43, 0x44 });
    ...
    const TestPixelMap scroll_diagonal_expected({ 0x00, 0x01, 0x02, 0x03, 0x04, 0x10, 0x11, 0x12, 0x13, 0x14, 0x20, 0x21, 0x22, 0x23, 0x24, 0x30, 0x31, 0x32,
...
TEST(Blit, WithSharedMemory)
{
    const int kCanvasWidth = 5;
    const int kCanvasHeight = 5;
    base::subtle::PlatformSharedMemoryRegion section = base::subtle::PlatformSharedMemoryRegion::CreateWritable(kCanvasWidth * kCanvasHeight);
    ASSERT_TRUE(section.IsValid());
    std::unique_ptr<SkCanvas> canvas
        = skia::CreatePlatformCanvasWithSharedSection(kCanvasWidth, kCanvasHeight, false, section.GetPlatformHandle(), skia::RETURN_NULL_ON_FAILURE);
    ASSERT_TRUE(canvas);
