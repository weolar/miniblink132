...
TEST(SelectionModelTest, Construction)
{
    ...
    }
    {
        ...
        EXPECT_EQ(selection_model.secondary_selections(), std::vector<Range>({ { 5, 5 }, { 1, 0 } }));
    }
}
...
TEST(SelectionModelTest, AddSecondarySelection)
{
    ...
    EXPECT_EQ(selection_model.selection(), Range(0));
    EXPECT_EQ(selection_model.caret_pos(), 0u);
    EXPECT_EQ(selection_model.secondary_selections(), std::vector<Range>({ { 5, 6 }, { 7, 6 }, { 8, 8 } }));
}
...
TEST(SelectionModelTest, GetAllSelections)
{
    ...
    selection_model.AddSecondarySelection({ 7, 6 });
    selection_model.AddSecondarySelection({ 8, 8 });
    EXPECT_EQ(selection_model.GetAllSelections(), std::vector<Range>({ { 3, 2 }, { 5, 6 }, { 7, 6 }, { 8, 8 } }));
}
...
TEST(SelectionModelTest, EqualityOperators)
{
    ...
    // Equal
    EXPECT_EQ(selection_model, SelectionModel({ { 3, 2 }, { 5, 6 }, { 7, 6 }, { 8, 8 } }, CURSOR_BACKWARD));
    // Unequal selection
    EXPECT_NE(selection_model, SelectionModel({ { 3, 3 }, { 5, 6 }, { 7, 6 }, { 8, 8 } }, CURSOR_BACKWARD));
    // Unequal secondary selections
    EXPECT_NE(selection_model, SelectionModel({ { 3, 2 }, { 5, 6 }, { 7, 6 }, { 9, 8 } }, CURSOR_BACKWARD));
    // Unequal cursor affinity
    EXPECT_NE(selection_model, SelectionModel({ { 3, 2 }, { 5, 6 }, { 7, 6 }, { 8, 8 } }, CURSOR_FORWARD));
}
