...
COMPONENT_EXPORT(GFX) std::u16string RemoveAccelerator(const std::u16string& s);
...
COMPONENT_EXPORT(GFX)
size_t FindValidBoundaryBefore(const std::u16string& text, size_t index, bool trim_whitespace = false);

// Returns a valid cut boundary at or after |index|. The surrogate pair and
// combining characters should not be separated.
COMPONENT_EXPORT(GFX)
size_t FindValidBoundaryAfter(const std::u16string& text, size_t index, bool trim_whitespace = false);
