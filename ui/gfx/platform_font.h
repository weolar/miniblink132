...
class COMPONENT_EXPORT(GFX) PlatformFont : public base::RefCounted<PlatformFont> {
public:
...
// user preference for a larger or smaller font.
    ...
    static int GetFontSizeDeltaIgnoringUserOrLocaleSettings(int desired_font_size);
    ...
    static PlatformFont* CreateFromNameAndSize(const std::string& font_name, int font_size);
    ...
    static PlatformFont* CreateFromSkTypeface(sk_sp<SkTypeface> typeface, int font_size, const std::optional<FontRenderParams>& params);
    ...
    virtual Font DeriveFont(int size_delta, int style, Font::Weight weight) const = 0;
