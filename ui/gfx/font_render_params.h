...
struct COMPONENT_EXPORT(GFX) FontRenderParams {
    ...
    {
        return antialiasing == other.antialiasing && text_contrast == other.text_contrast && text_gamma == other.text_gamma
            && subpixel_positioning == other.subpixel_positioning && autohinter == other.autohinter && use_bitmaps == other.use_bitmaps
            && hinting == other.hinting && subpixel_rendering == other.subpixel_rendering;
    }
    ...
    static SkPixelGeometry SubpixelRenderingToSkiaPixelGeometry(SubpixelRendering subpixel_rendering);
};
...
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_FUCHSIA)
