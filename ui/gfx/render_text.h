...
public:
    ...
    void SetFontRenderParams(const FontRenderParams& params, bool subpixel_rendering_suppressed);
    ...
    virtual void DrawPosText(const SkPoint* pos, const uint16_t* glyphs, size_t glyph_count);
...
class StyleIterator {
public:
    StyleIterator(const BreakList<SkColor>* colors, const BreakList<BaselineStyle>* baselines, const BreakList<int>* font_size_overrides,
    ...
    BaselineStyle baseline() const
    {
        return baseline_->second;
    }
    int font_size_override() const
    {
        return font_size_override_->second;
    }
    bool style(TextStyle s) const
    {
        return style_[s]->second;
    }
    Font::Weight weight() const
    {
        return weight_->second;
    }
    SkTypefaceID resolved_typeface() const
    {
        return resolved_typeface_->second;
    }
    cc::PaintFlags::Style fill_style() const
    {
        return fill_style_->second;
    }
    SkScalar stroke_width() const
    {
        return stroke_width_->second;
    }
...
public:
    ...
    std::unique_ptr<RenderText> CreateInstanceOfSameStyle(const std::u16string& text) const;
    ...
    const FontList& font_list() const
    {
        return font_list_;
    }
    ...
    SkColor selection_color() const
    {
        return selection_color_;
    }
    void set_selection_color(SkColor color)
    {
    ...
    bool clip_to_display_rect() const
    {
        return clip_to_display_rect_;
    }
    void set_clip_to_display_rect(bool clip)
    {
        clip_to_display_rect_ = clip;
    }
    ...
    int obscured_glyph_spacing() const
    {
        return obscured_glyph_spacing_;
    }
    ...
    void set_truncate_length(size_t length)
    {
        truncate_length_ = length;
    }

    // The display text will be elided to fit |display_rect| using this behavior.
    void SetElideBehavior(ElideBehavior elide_behavior);
    ElideBehavior elide_behavior() const
    {
    ...
    std::optional<bool> whitespace_elision() const
    {
        return whitespace_elision_;
    }

    const Rect& display_rect() const
    {
    ...
    const SelectionModel& selection_model() const
    {
        return selection_model_;
    }
    const Range& selection() const
    {
        return selection_model_.selection();
    }
    size_t cursor_position() const
    {
        return selection_model_.caret_pos();
    }
    ...
    // behavior.
    void MoveCursor(BreakType break_type, VisualCursorDirection direction, SelectionBehavior selection_behavior);
    ...
    bool MoveCursorToPoint(const gfx::Point& point, bool select, const gfx::Point& drag_origin = gfx::Point());
    ...
    SelectionModel FindCursorPosition(const Point& point, const Point& drag_origin = gfx::Point());
    ...
    internal::GraphemeIterator GetGraphemeIteratorAtDisplayTextIndex(size_t index) const;
    ...
    size_t IndexOfAdjacentGrapheme(size_t index, LogicalCursorDirection direction) const;
    ...
    void set_shadows(const ShadowValues& shadows)
    {
        shadows_ = shadows;
    }
    const ShadowValues& shadows() const
    {
        return shadows_;
    }
    ...
    bool GetWordLookupDataAtPoint(const Point& point, DecoratedText* decorated_word, Rect* rect);
    ...
    bool GetLookupDataForRange(const Range& range, DecoratedText* decorated_text, Rect* rect);
    ...
    void set_glyph_width_for_test(float width)
    {
        glyph_width_for_test_ = width;
    }
...
protected:
    ...
    bool IsNewlineSegment(const std::u16string& text, const internal::LineSegment& segment) const;

    // Returns the character range of segments in |line| excluding the trailing
    // newline segment.
    Range GetLineRange(const std::u16string& text, const internal::Line& line) const;
    ...
    const std::u16string& display_text() const
    {
        return display_text_;
    }
    bool text_elided() const
    {
        return text_elided_;
    }
    ...
    const BreakList<SkColor>& colors() const
    {
        return colors_;
    }
    ...
    const internal::StyleArray& styles() const
    {
        return styles_;
    }
    ...
    {
        return stroke_widths_;
    }
    SkScalar strike_thickness_factor() const
    {
        return strike_thickness_factor_;
    }

    const BreakList<SkColor>& layout_colors() const
    {
        return layout_colors_;
    }
    ...
    bool has_shaped_text() const
    {
        return shaped_text_ != nullptr;
    }
    ...
    // The returned value represents a cursor/caret position without a selection.
    SelectionModel GetAdjacentSelectionModel(const SelectionModel& current, BreakType break_type, VisualCursorDirection direction);

    // Get the selection model visually left/right of |selection| by one grapheme.
    // The returned value represents a cursor/caret position without a selection.
    virtual SelectionModel AdjacentCharSelectionModel(const SelectionModel& selection, VisualCursorDirection direction) = 0;

    // Get the selection model visually left/right of |selection| by one word.
    // The returned value represents a cursor/caret position without a selection.
    virtual SelectionModel AdjacentWordSelectionModel(const SelectionModel& selection, VisualCursorDirection direction) = 0;

    // Get the selection model visually above/below |selection| by one line.
    // The returned value represents a cursor/caret position without a selection.
    virtual SelectionModel AdjacentLineSelectionModel(const SelectionModel& selection, VisualCursorDirection direction) = 0;
    ...
    // The returned value represents a cursor/caret position without a selection.
    SelectionModel LineSelectionModel(size_t line_index, gfx::VisualCursorDirection direction);
    ...
    virtual void DrawVisualText(internal::SkiaTextRenderer* renderer, const std::vector<Range>& selections) = 0;
    ...
    // |text|.
    base::i18n::TextDirection GetTextDirectionForGivenText(const std::u16string& text) const;
    ...
    static bool RangeContainsCaret(const Range& range, size_t caret_pos, LogicalCursorDirection caret_affinity);

    // Returns the baseline, with which the text best appears vertically centered.
    static int DetermineBaselineCenteringText(const int display_height, const FontList& font_list);

    // Returns an expanded version of |rect| that is vertically symmetric with
    // respect to the center of |display_rect|.
    static gfx::Rect ExpandToBeVerticallySymmetric(const gfx::Rect& rect, const gfx::Rect& display_rect);
    ...
    void reset_cached_cursor_x()
    {
        cached_cursor_x_.reset();
    }

    void set_cached_cursor_x(int x)
    {
        cached_cursor_x_ = x;
    }
    std::optional<int> cached_cursor_x() const
    {
        return cached_cursor_x_;
    }
...
private:
    ...
    std::u16string Elide(const std::u16string& text, float text_width, float available_width, ElideBehavior behavior);
    ...
    // Returns a grapheme iterator that contains the codepoint at |index|.
    internal::GraphemeIterator GetGraphemeIteratorAtIndex(const std::u16string& text, const size_t internal::TextToDisplayIndex::*field, size_t index) const;
    ...
    virtual void GetDecoratedTextForRange(const Range& text_range, DecoratedText* decorated_text) = 0;
    ...
    HorizontalAlignment horizontal_alignment_ { base::i18n::IsRTL() ? ALIGN_RIGHT : ALIGN_LEFT };
    ...
    mutable base::i18n::TextDirection text_direction_ = base::i18n::UNKNOWN_DIRECTION;
