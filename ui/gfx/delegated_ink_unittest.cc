...
TEST(DelegatedInkTest, PointAndMetadataMatch)
{
    ...
    DelegatedInkMetadata metadata(point, /*diameter=*/10, SK_ColorBLACK, timestamp, gfx::RectF(10, 10, 100, 100),
        /*hovering=*/false);
    ...
    DelegatedInkMetadata metadata_with_frame_time(point, /*diameter=*/7.777f, SK_ColorCYAN, timestamp, gfx::RectF(6, 14, 240, 307), base::TimeTicks::Now(),
        /*hovering=*/true,
        /*render_pass_id=*/0);

    EXPECT_TRUE(ink_point.MatchesDelegatedInkMetadata(&metadata_with_frame_time));
    EXPECT_TRUE(point_no_pointer_id.MatchesDelegatedInkMetadata(&metadata_with_frame_time));
}
...
TEST(DelegatedInkTest, PointAndMetadataAreClose)
{
    ...
    DelegatedInkMetadata metadata(metadata_location, /*diameter=*/3.5f, SK_ColorRED, timestamp, gfx::RectF(0, 3, 14.6f, 78.2f), /*hovering=*/false);
...
TEST(DelegatedInkTest, PointAndMetadataDoNotMatch)
{
    ...
    DelegatedInkMetadata close_metadata(
        metadata_location_within_tolerance, /*diameter=*/5, SK_ColorWHITE, timestamp, gfx::RectF(40, 30.6f, 43, 7.2f), /*hovering=*/true);
    ...
    DelegatedInkMetadata just_outside_metadata(
        metadata_location_outside_tolerance, /*diameter=*/5, SK_ColorWHITE, timestamp, gfx::RectF(40, 30.6f, 43, 7.2f), /*hovering=*/true);
    ...
    DelegatedInkMetadata far_metadata(
        metadata_location_far, /*diameter=*/1, SK_ColorYELLOW, timestamp, gfx::RectF(12.44f, 3, 1000, 1000.1f), /*hovering=*/false);
    EXPECT_FALSE(ink_point.MatchesDelegatedInkMetadata(&far_metadata));

    DelegatedInkMetadata metadata_different_timestamp(point_location, /*diameter=*/10, SK_ColorGREEN, timestamp + base::Milliseconds(10),
        gfx::RectF(0, 0, 0, 0),
        /*hovering*/ true);
    EXPECT_FALSE(ink_point.MatchesDelegatedInkMetadata(&metadata_different_timestamp));
}
