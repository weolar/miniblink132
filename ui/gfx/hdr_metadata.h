...
struct COLOR_SPACE_EXPORT HdrMetadataCta861_3 {
    ...
    constexpr HdrMetadataCta861_3(unsigned max_content_light_level, unsigned max_frame_average_light_level)
    ...
    {
        return max_content_light_level == rhs.max_content_light_level && max_frame_average_light_level == rhs.max_frame_average_light_level;
    }
    bool operator!=(const HdrMetadataCta861_3& rhs) const
    {
        return !(*this == rhs);
...
struct COLOR_SPACE_EXPORT HdrMetadataSmpteSt2086 {
    ...
    constexpr HdrMetadataSmpteSt2086(const SkColorSpacePrimaries& primaries, float luminance_max, float luminance_min)
        : primaries(primaries)
    ...
    bool IsValid() const
    {
        return primaries != SkNamedPrimariesExt::kInvalid || luminance_max != 0.f || luminance_min != 0.f;
    }

    bool operator==(const HdrMetadataSmpteSt2086& rhs) const
    {
        return (primaries == rhs.primaries && luminance_max == rhs.luminance_max && luminance_min == rhs.luminance_min);
    }
...
struct COLOR_SPACE_EXPORT HdrMetadataNdwl {
    ...
    constexpr explicit HdrMetadataNdwl(float nits)
        : nits(nits)
    {
    }
    ...
    bool operator==(const HdrMetadataNdwl& rhs) const
    {
        return nits == rhs.nits;
    }
...
struct COLOR_SPACE_EXPORT HdrMetadataExtendedRange {
    ...
    constexpr HdrMetadataExtendedRange(float current_headroom, float desired_headroom)
    ...
    {
        return (current_headroom == rhs.current_headroom && desired_headroom == rhs.desired_headroom);
    }
...
struct COLOR_SPACE_EXPORT HDRMetadata {
    ...
    HDRMetadata() = default;
    HDRMetadata(const HdrMetadataSmpteSt2086& smpte_st_2086, const HdrMetadataCta861_3& cta_861_3)
    ...
    bool IsValid() const
    {
        return (cta_861_3 && cta_861_3->IsValid()) || (smpte_st_2086 && smpte_st_2086->IsValid()) || extended_range;
    }
    ...
    // zero).
    static HDRMetadata PopulateUnspecifiedWithDefaults(const std::optional<gfx::HDRMetadata>& hdr_metadata);

    std::string ToString() const;

    bool operator==(const HDRMetadata& rhs) const
    {
        return cta_861_3 == rhs.cta_861_3 && smpte_st_2086 == rhs.smpte_st_2086 && ndwl == rhs.ndwl && extended_range == rhs.extended_range;
    }
