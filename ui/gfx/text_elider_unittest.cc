...
TEST(TextEliderTest, ElideEmail)
{
    ...
    const auto cases = std::to_array<StringMappingTestCase>({
        ...
        { u"messed\"up@whyanat\"++@notgoogley.com", u"messed\"up@whyanat\"++@notgoogley.com" },
        { u"messed\"up@whyanat\"++@notgoogley.com", u"messed\"up@why…@notgoogley.com" },
        { u"noca_messed\"up@whyanat\"++@notgoogley.ca", u"noca…@no…ca" },
        { u"at\"@@@@@@@@@...@@.@.@.@@@\"@madness.com", u"at\"@@@@@@@@@...@@.@.…@madness.com" },
    ...
    for (const auto& c : cases) {
        EXPECT_EQ(c.output, ElideText(c.input, font_list, GetStringWidthF(c.output, font_list), ELIDE_EMAIL));
    }
}
...
TEST(TextEliderTest, ElideEmailMoreSpace)
{
    ...
    for (const auto* test_email : test_emails) {
        ...
        for (int extra_space : test_widths_extra_spaces) {
            ...
            EXPECT_EQ(test_email, ElideText(test_email, font_list, mimimum_width + extra_space, ELIDE_EMAIL));
        }
    }
}
...
{
    const base::FilePath::StringType kPathSeparator = base::FilePath::StringType().append(1, base::FilePath::kSeparators[0]);
    ...
    const auto cases = std::to_array<Case>({ { FILE_PATH_LITERAL(""), u"" }, { FILE_PATH_LITERAL("."), u"." },
        { FILE_PATH_LITERAL("filename.exe"), u"filename.exe" }, { FILE_PATH_LITERAL(".longext"), u".longext" }, { FILE_PATH_LITERAL("pie"), u"pie" },
        { FILE_PATH_LITERAL("c:") + kPathSeparator + FILE_PATH_LITERAL("path") + kPathSeparator + FILE_PATH_LITERAL("filename.pie"), u"filename.pie" },
        { FILE_PATH_LITERAL("c:") + kPathSeparator + FILE_PATH_LITERAL("path") + kPathSeparator + FILE_PATH_LITERAL("longfilename.pie"), u"long….pie" },
        { FILE_PATH_LITERAL("http://path.com/filename.pie"), u"filename.pie" }, { FILE_PATH_LITERAL("http://path.com/longfilename.pie"), u"long….pie" },
        { FILE_PATH_LITERAL("piesmashingtacularpants"), u"pie…" }, { FILE_PATH_LITERAL(".piesmashingtacularpants"), u".pie…" },
        { FILE_PATH_LITERAL("cheese."), u"cheese." }, { FILE_PATH_LITERAL("file name.longext"), u"file….longext" },
        { FILE_PATH_LITERAL("fil ename.longext"), u"fil….longext", u"fil ….longext" }, { FILE_PATH_LITERAL("filename.longext"), u"file….longext" },
        { FILE_PATH_LITERAL("filename.middleext.longext"), u"filename.mid….longext" },
        { FILE_PATH_LITERAL("filename.superduperextremelylongext"), u"filename.sup…emelylongext" },
        { FILE_PATH_LITERAL("filenamereallylongtext.superdeduperextremelylongext"), u"filenamereall…emelylongext" },
        { FILE_PATH_LITERAL("file.name.really.long.text.superduperextremelylongext"), u"file.name.re…emelylongext" } });

    static const FontList font_list;
    for (const auto& c : cases) {
        std::u16string using_width_of = c.using_width_of.empty() ? c.output : c.using_width_of;
        EXPECT_EQ(base::i18n::GetDisplayStringInLTRDirectionality(c.output),
            ElideFilename(base::FilePath(c.input), font_list, GetStringWidthF(using_width_of, font_list)));
    }
}
...
static void CheckCodeUnitPairs(const std::u16string& text, char16_t first_char, char16_t second_char)
{
...
TEST(TextEliderTest, ElideTextAtomicSequences)
{
    ...
    base::test::SingleThreadTaskEnvironment task_environment(base::test::SingleThreadTaskEnvironment::MainThreadType::UI);
#endif
    const FontList font_list;
...
TEST(TextEliderTest, ElideTextLongStrings)
{
    ...
    std::u16string long_string_end(data_scheme + std::u16string(number_of_as, 'a') + u"…");
    ...
    for (const auto& c : cases_end) {
        ...
        EXPECT_EQ(c.output.size(), ElideText(c.input, font_list, GetStringWidthF(c.output, font_list), ELIDE_TAIL).size());
    ...
    std::u16string long_string_middle(
        data_scheme + std::u16string(number_of_as - number_of_trailing_as, 'a') + u"…" + std::u16string(number_of_trailing_as, 'a'));
#if !BUILDFLAG(IS_IOS)
    ...
    for (const auto& c : cases_middle) {
        // Compare sizes rather than actual contents because if the test fails,
        // output is rather long.
        EXPECT_EQ(c.output.size(), ElideText(c.input, font_list, GetStringWidthF(c.output, font_list), ELIDE_MIDDLE).size());
        EXPECT_EQ(u"…", ElideText(c.input, font_list, ellipsis_width, ELIDE_MIDDLE));
    }

    std::u16string long_string_beginning(u"…" + std::u16string(number_of_as, 'a'));
#if !BUILDFLAG(IS_IOS)
    ...
    for (const auto& c : cases_beginning) {
        EXPECT_EQ(c.output.size(), ElideText(c.input, font_list, GetStringWidthF(c.output, font_list), ELIDE_HEAD).size());
...
TEST(TextEliderTest, StringSlicerSurrogate)
{
    ...
    StringSlicer slicer_dangling_trailing(dangling_trailing_text, ellipsis, false, false);
    EXPECT_EQ(u"…", slicer_dangling_trailing.CutString(0, true));
    EXPECT_EQ(dangling_trailing_text + u"…", slicer_dangling_trailing.CutString(1, true));
}
...
TEST(TextEliderTest, StringSlicerCombining)
{
    ...
    StringSlicer slicer_mid(text, ellipsis, true, false);
    EXPECT_EQ(text.substr(0, 4) + u"…" + text.substr(6), slicer_mid.CutString(9, true));
...
TEST(TextEliderTest, ElideString)
{
    ...
    const auto cases = std::to_array<Case>({ { u"Hello", 0, true, u"" }, { u"", 0, false, u"" }, { u"Hello, my name is Tom", 1, true, u"H" },
        { u"Hello, my name is Tom", 2, true, u"He" }, { u"Hello, my name is Tom", 3, true, u"H.m" }, { u"Hello, my name is Tom", 4, true, u"H..m" },
        { u"Hello, my name is Tom", 5, true, u"H...m" }, { u"Hello, my name is Tom", 6, true, u"He...m" }, { u"Hello, my name is Tom", 7, true, u"He...om" },
        { u"Hello, my name is Tom", 10, true, u"Hell...Tom" }, { u"Hello, my name is Tom", 100, false, u"Hello, my name is Tom" } });
    for (const auto& c : cases) {
...
TEST(TextEliderTest, ElideRectangleText)
{
    ...
    const auto cases = std::to_array<Case>({
        ...
        { u"Test\n\n", 2 * test_width, line_height * 5, false, u"Test||" },
        { u"Test\n\n\n", 2 * test_width, line_height * 5, false, u"Test|||" },
        { u"Test\nTest\n\n", 2 * test_width, line_height * 5, false, u"Test|Test||" },
        { u"Test\n\nTest\n", 2 * test_width, line_height * 5, false, u"Test||Test|" },
        { u"Test\n\n\nTest", 2 * test_width, line_height * 5, false, u"Test|||Test" },
    ...
    for (const auto& c : cases) {
        ...
        EXPECT_EQ(c.truncated_y ? INSUFFICIENT_SPACE_VERTICAL : 0,
            ElideRectangleText(c.input, font_list, c.available_pixel_width, c.available_pixel_height, TRUNCATE_LONG_WORDS, &lines));
        if (c.output) {
...
TEST(TextEliderTest, ElideRectangleTextFirstWordTruncated)
{
    ...
    auto result_for_width = [&](const char16_t* input, float width) {
        lines.clear();
        return ElideRectangleText(input, font_list, width, line_height * 4, WRAP_LONG_WORDS, &lines);
    };
    ...
    EXPECT_EQ(INSUFFICIENT_SPACE_FOR_FIRST_WORD, result_for_width(u"Test", tes_width));
    EXPECT_EQ(2u, lines.size());
    EXPECT_EQ(u"Tes", lines[0]);
    EXPECT_EQ(u"t", lines[1]);

    // Two words truncated.
    EXPECT_EQ(INSUFFICIENT_SPACE_FOR_FIRST_WORD, result_for_width(u"Test\nTest", tes_width));
...
TEST(TextEliderTest, ElideRectangleTextPunctuation)
{
    ...
    constexpr int kResultMask = INSUFFICIENT_SPACE_HORIZONTAL | INSUFFICIENT_SPACE_VERTICAL;
    ...
    for (const auto& c : cases) {
        std::vector<std::u16string> lines;
        const WordWrapBehavior wrap_behavior = (c.wrap_words ? WRAP_LONG_WORDS : TRUNCATE_LONG_WORDS);
        EXPECT_EQ(c.truncated_x ? INSUFFICIENT_SPACE_HORIZONTAL : 0,
            ElideRectangleText(c.input, font_list, c.available_pixel_width, c.available_pixel_height, wrap_behavior, &lines) & kResultMask);
        if (c.output) {
            const std::u16string result = base::JoinString(lines, u"|");
            EXPECT_EQ(c.output, result);
        } else {
...
TEST(TextEliderTest, ElideRectangleTextLongWords)
{
    ...
    const auto cases = std::to_array<Case>({
        ...
        { u"TestTestTestT", test_width, WRAP_LONG_WORDS, false, u"Test|Test|Test|T" },
    });

    for (const auto& c : cases) {
        std::vector<std::u16string> lines;
        EXPECT_EQ(c.truncated_x ? INSUFFICIENT_SPACE_HORIZONTAL : 0,
            ElideRectangleText(c.input, font_list, c.available_pixel_width, kAvailableHeight, c.wrap_behavior, &lines) & kResultMask);
...
TEST(TextEliderTest, ElideRectangleTextCheckLineWidth)
{
    ...
    std::vector<std::u16string> lines;
    EXPECT_EQ(0, ElideRectangleText(text, font_list, kAvailableWidth, kAvailableHeight, WRAP_LONG_WORDS, &lines));
...
#define WIDTH(x) GetStringWidthF((x), font_list)
    EXPECT_EQ(WIDTH(u"The administrator for this account has"),
        WIDTH(u"The ") + WIDTH(u"administrator ") + WIDTH(u"for ") + WIDTH(u"this ") + WIDTH(u"account ") + WIDTH(u"has"));
#undef WIDTH
...
TEST(TextEliderTest, ElideRectangleString)
{
    ...
    std::u16string output;
    for (const auto& c : cases) {
        EXPECT_EQ(c.result, ElideRectangleString(c.input, c.max_rows, c.max_cols, true, &output));
        EXPECT_EQ(c.output, output);
    }
}
...
TEST(TextEliderTest, ElideRectangleStringNotStrict)
{
    ...
    std::u16string output;
    for (const auto& c : cases) {
        EXPECT_EQ(c.result, ElideRectangleString(c.input, c.max_rows, c.max_cols, false, &output));
        EXPECT_EQ(c.output, output);
    }
}
...
TEST(TextEliderTest, TruncateString)
{
    ...
    EXPECT_EQ(u"fooooey    bxxxar baz…", TruncateString(str, 22, CHARACTER_BREAK));
