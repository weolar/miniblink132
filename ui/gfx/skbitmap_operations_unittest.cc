...
{
    return ColorsClose(SkUnPreMultiply::PMColorToColor(a), SkUnPreMultiply::PMColorToColor(b));
}
...
SkBitmap ReferenceCreateHSLShiftedBitmap(const SkBitmap& bitmap, color_utils::HSL hsl_shift)
{
    SkBitmap shifted;
    ...
    for (int y = 0; y < bitmap.height(); ++y) {
        ...
        for (int x = 0; x < bitmap.width(); ++x) {
            tinted_pixels[x] = SkPreMultiplyColor(color_utils::HSLShift(SkUnPreMultiply::PMColorToColor(pixels[x]), hsl_shift));
        }
    }
...
TEST(SkBitmapOperationsTest, CreateInvertedBitmap)
{
    ...
    for (int y = 0; y < src_h; y++) {
        for (int x = 0; x < src_w; x++) {
            int i = y * src_w + x;
            EXPECT_EQ(static_cast<unsigned int>((255 - i) % 255), SkColorGetA(*inverted.getAddr32(x, y)));
            EXPECT_EQ(static_cast<unsigned int>(255 - (i % 255)), SkColorGetR(*inverted.getAddr32(x, y)));
            EXPECT_EQ(static_cast<unsigned int>(255 - (i * 4 % 255)), SkColorGetG(*inverted.getAddr32(x, y)));
            EXPECT_EQ(static_cast<unsigned int>(255), SkColorGetB(*inverted.getAddr32(x, y)));
        }
    }
}
...
TEST(SkBitmapOperationsTest, CreateBlendedBitmap)
{
    ...
    // Shift to red.
    SkBitmap blended = SkBitmapOperations::CreateBlendedBitmap(src_a, src_b, 0.5);

    for (int y = 0; y < src_h; y++) {
        for (int x = 0; x < src_w; x++) {
            int i = y * src_w + x;
            EXPECT_EQ(static_cast<unsigned int>((255 + ((255 - i) % 255)) / 2), SkColorGetA(*blended.getAddr32(x, y)));
            EXPECT_EQ(static_cast<unsigned int>(i % 255 / 2), SkColorGetR(*blended.getAddr32(x, y)));
            EXPECT_EQ((static_cast<unsigned int>((i * 2) % 255 + (i * 4) % 255) / 2), SkColorGetG(*blended.getAddr32(x, y)));
            EXPECT_EQ(static_cast<unsigned int>(i % 255 / 2), SkColorGetB(*blended.getAddr32(x, y)));
        }
    }
}
...
TEST(SkBitmapOperationsTest, CreateHSLShiftedBitmapToSame)
{
    ...
    for (int y = 0, i = 0; y < src_h; y++) {
        for (int x = 0; x < src_w; x++) {
            *src.getAddr32(x, y) = SkPreMultiplyColor(SkColorSetARGB((i + 128) % 255, (i + 128) % 255, (i + 64) % 255, (i + 0) % 255));
            i++;
        }
    }
    ...
    for (int y = 0; y < src_h; y++) {
        for (int x = 0; x < src_w; x++) {
            ...
            EXPECT_TRUE(MultipliedColorsClose(src_pixel, shifted_pixel))
                << "source: (a,r,g,b) = (" << SkColorGetA(src_pixel) << "," << SkColorGetR(src_pixel) << "," << SkColorGetG(src_pixel) << ","
                << SkColorGetB(src_pixel) << "); "
                << "shifted: (a,r,g,b) = (" << SkColorGetA(shifted_pixel) << "," << SkColorGetR(shifted_pixel) << "," << SkColorGetG(shifted_pixel) << ","
...
TEST(SkBitmapOperationsTest, CreateHSLShiftedBitmapHueOnly)
{
    ...
    for (int y = 0, i = 0; y < src_h; y++) {
        for (int x = 0; x < src_w; x++) {
            EXPECT_TRUE(ColorsClose(shifted.getColor(x, y), SkColorSetARGB(255, i % 255, 0, 0)));
            i++;
        }
    }
}
    ...
    for (double h = -0.1; h <= 1.0001; h += 0.1) {
        for (double s = -0.1; s <= 1.0001; s += 0.1) {
            for (double l = -0.1; l <= 1.0001; l += 0.1) {
                ...
                SkBitmap shifted = SkBitmapOperations::CreateHSLShiftedBitmap(src, hsl);
                EXPECT_TRUE(BitmapsClose(ref_shifted, shifted)) << "h = " << h << ", s = " << s << ", l = " << l;
            }
        }
    }
}
...
TEST(SkBitmapOperationsTest, CreateCroppedBitmap)
{
    ...
    SkBitmap cropped = SkBitmapOperations::CreateTiledBitmap(src, 4, 4, 8, 8);
    ASSERT_EQ(8, cropped.width());
    ASSERT_EQ(8, cropped.height());

    for (int y = 4; y < 12; y++) {
        for (int x = 4; x < 12; x++) {
            EXPECT_EQ(*src.getAddr32(x, y), *cropped.getAddr32(x - 4, y - 4));
        }
    }
}
...
TEST(SkBitmapOperationsTest, CreateCroppedBitmapWrapping)
{
    ...
    SkBitmap cropped = SkBitmapOperations::CreateTiledBitmap(src, src_w / 2, src_h / 2, src_w, src_h);
    ASSERT_EQ(src_w, cropped.width());
    ASSERT_EQ(src_h, cropped.height());

    for (int y = 0; y < src_h; y++) {
        for (int x = 0; x < src_w; x++) {
            EXPECT_EQ(*src.getAddr32(x, y), *cropped.getAddr32((x + src_w / 2) % src_w, (y + src_h / 2) % src_h));
        }
    }
}
...
TEST(SkBitmapOperationsTest, DownsampleByTwoUntilSize)
{
    ...
    SkBitmap result = SkBitmapOperations::DownsampleByTwoUntilSize(too_small, 16, 16);
...
void DrawRectWithColor(SkCanvas* canvas, int left, int top, int right, int bottom, SkColor color)
{
    SkPaint paint;
    paint.setColor(color);
    paint.setBlendMode(SkBlendMode::kSrc);
    canvas->drawRect(SkRect::MakeLTRB(SkIntToScalar(left), SkIntToScalar(top), SkIntToScalar(right), SkIntToScalar(bottom)), paint);
}
...
TEST(SkBitmapOperationsTest, RotateImage)
{
    ...
    DrawRectWithColor(&canvas, src_w / 2, src_h / 2, src_w, src_h, SK_ColorYELLOW);

    SkBitmap rotate90, rotate180, rotate270;
    rotate90 = SkBitmapOperations::Rotate(src, SkBitmapOperations::ROTATION_90_CW);
    rotate180 = SkBitmapOperations::Rotate(src, SkBitmapOperations::ROTATION_180_CW);
    rotate270 = SkBitmapOperations::Rotate(src, SkBitmapOperations::ROTATION_270_CW);
    ...
    for (int x = 0; x < src_w; ++x) {
        for (int y = 0; y < src_h; ++y) {
            ...
            ASSERT_EQ(*src.getAddr32(x, y), *rotate180.getAddr32(src_w - (x + 1), src_h - (y + 1)));
        }
    }
}
