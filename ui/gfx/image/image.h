...
public:
    ...
    // Creates an image from the PNG encoded input.
    static Image CreateFrom1xPNGBytes(const scoped_refptr<base::RefCountedMemory>& input);
...
private:
    ...
    const internal::ImageRep* GetRepresentation(RepresentationType rep_type, bool must_exist) const;
    ...
    // inside the map.
    const internal::ImageRep* AddRepresentation(std::unique_ptr<internal::ImageRep> rep) const;
    ...
    // constness is upheld.
    const internal::ImageStorage* storage() const
    {
        return storage_.get();
    }
