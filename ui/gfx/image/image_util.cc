...
{
    return ResizedImageForMaxDimensions(image, kSearchByImageMaxImageWidth, kSearchByImageMaxImageHeight, kSearchByImageMaxImageArea);
}

Image ResizedImageForMaxDimensions(const Image& image, int max_width, int max_height, int max_area)
{
    ...
    if (bitmap.height() * bitmap.width() > max_area && (bitmap.width() > max_width || bitmap.height() > max_height)) {
        double scale = std::min(static_cast<double>(max_width) / bitmap.width(), static_cast<double>(max_height) / bitmap.height());
        int width = std::clamp<int>(scale * bitmap.width(), 1, max_width);
        int height = std::clamp<int>(scale * bitmap.height(), 1, max_height);
        SkBitmap new_bitmap = skia::ImageOperations::Resize(bitmap, skia::ImageOperations::RESIZE_GOOD, width, height);
...
#if !BUILDFLAG(IS_MAC)
std::optional<std::vector<uint8_t>> JPEG1xEncodedDataFromImage(const Image& image, int quality)
{
    return JPEG1xEncodedDataFromSkiaRepresentation(image, quality);
}
#endif // !BUILDFLAG(IS_MAC)

std::optional<std::vector<uint8_t>> JPEG1xEncodedDataFromSkiaRepresentation(const Image& image, int quality)
{
    const gfx::ImageSkiaRep& image_skia_rep = image.AsImageSkia().GetRepresentation(1.0f);
    if (image_skia_rep.scale() != 1.0f) {
        return std::nullopt;
...
std::optional<std::vector<uint8_t>> WebpEncodedDataFromImage(const Image& image, int quality)
{
...
Image ResizedImage(const Image& image, const gfx::Size& size)
{
    ...
    SkBitmap new_bitmap = skia::ImageOperations::Resize(image_skia_rep.GetBitmap(), skia::ImageOperations::RESIZE_GOOD, size.width(), size.height());
    return Image(ImageSkia(ImageSkiaRep(new_bitmap, 0.0f)));
}
