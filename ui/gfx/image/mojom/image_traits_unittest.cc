...
public:
    explicit TestImageSkiaSource(const Size& dip_size)
        : dip_size_(dip_size)
    {
    }
...
mojo::StructPtr<mojom::ImageSkiaRep> ConstructImageSkiaRep(const SkBitmap& bitmap, float scale)
{
...
TEST(ImageTraitsTest, VerifyMojomConstruction)
{
    ...
    ImageSkiaRep output;
    EXPECT_TRUE(mojo::test::SerializeAndDeserialize<mojom::ImageSkiaRep>(input, output));
}
...
TEST(ImageTraitsTest, BadColorTypeImageSkiaRep_Deserialize)
{
    ...
    mojo::StructPtr<mojom::ImageSkiaRep> input = ConstructImageSkiaRep(a8_bitmap, 1);
    ImageSkiaRep output;
    EXPECT_FALSE(mojo::test::SerializeAndDeserialize<mojom::ImageSkiaRep>(input, output));
}
...
TEST(ImageTraitsTest, EmptyImageSkiaRep_Deserialize)
{
    ...
    mojo::StructPtr<mojom::ImageSkiaRep> input = ConstructImageSkiaRep(empty_bitmap, 1);
    ImageSkiaRep output;
    EXPECT_FALSE(mojo::test::SerializeAndDeserialize<mojom::ImageSkiaRep>(input, output));
}
...
TEST(ImageTraitsTest, ValidImageSkiaRep)
{
    ...
    ImageSkiaRep output;
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::ImageSkiaRep>(image_rep, output));
...
TEST(ImageTraitsTest, NullImageSkia)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::ImageSkia>(null_image, output));
...
TEST(ImageTraitsTest, ImageSkiaRepsAreCreatedAsNeeded)
{
    ...
    ImageSkia output;
    EXPECT_TRUE(output.isNull());
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::ImageSkia>(image, output));
...
TEST(ImageTraitsTest, ImageSkiaWithOperations)
{
    ...
    ImageSkia resized = ImageSkiaOperations::CreateResizedImage(image, skia::ImageOperations::RESIZE_BEST, kNewSize);
    resized.GetRepresentation(1.0f);
    resized.GetRepresentation(2.0f);

    ImageSkia output;
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::ImageSkia>(resized, output));
