...
public:
    ...
    static ImageSkia CreateBlendedImage(const ImageSkia& first, const ImageSkia& second, double alpha);
    ...
    static ImageSkia CreateSuperimposedImage(const ImageSkia& first, const ImageSkia& second);
    ...
    static ImageSkia CreateMaskedImage(const ImageSkia& first, const ImageSkia& alpha);
    ...
    static ImageSkia CreateTiledImage(const ImageSkia& image, int src_x, int src_y, int dst_w, int dst_h);
    ...
    static ImageSkia CreateHSLShiftedImage(const gfx::ImageSkia& image, const color_utils::HSL& hsl_shift);
    ...
    static ImageSkia CreateButtonBackground(SkColor color, const gfx::ImageSkia& image, const gfx::ImageSkia& mask);

    // Returns an image which is a subset of |image| with bounds |subset_bounds|.
    // The |image| cannot use kA1_Config config.
    static ImageSkia ExtractSubset(const gfx::ImageSkia& image, const gfx::Rect& subset_bounds);

    // Creates an image by resizing |source| to given |target_dip_size|.
    static ImageSkia CreateResizedImage(const ImageSkia& source, skia::ImageOperations::ResizeMethod methd, const Size& target_dip_size);

    // Creates an image with drop shadow defined in |shadows| for |source|.
    static ImageSkia CreateImageWithDropShadow(const ImageSkia& source, const ShadowValues& shadows);
    ...
    static ImageSkia CreateHorizontalShadow(const std::vector<ShadowValue>& shadows, bool fades_down);

    // Creates an image which is a rotation of the |source|. |rotation| is the
    // amount of clockwise rotation in degrees.
    static ImageSkia CreateRotatedImage(const ImageSkia& source, SkBitmapOperations::RotationAmount rotation);
    ...
    static ImageSkia CreateIconWithBadge(const ImageSkia& icon, const ImageSkia& badge);
    ...
    static ImageSkia CreateImageWithCircleBackground(int radius, SkColor color, const ImageSkia& image);

    // Creates an image with a rounded rect background of the specified `size`,
    // `color`, and `radius`.
    static ImageSkia CreateImageWithRoundRectBackground(const SizeF& size, int radius, SkColor color, const ImageSkia& image);

    // Creates an image with a roundrect clip path with `radius`.
    static ImageSkia CreateImageWithRoundRectClip(int radius, const ImageSkia& image);
    ...
    static ImageSkia CreateCroppedCenteredRoundRectImage(const Size& size, int border_radius, const ImageSkia& image);
