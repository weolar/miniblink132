...
ImageSkiaRep::ImageSkiaRep(const gfx::Size& size, float scale)
    ...
    , scale_(scale)
{
    bitmap_.allocN32Pixels(static_cast<int>(size.width() * this->scale()), static_cast<int>(size.height() * this->scale()));
...
ImageSkiaRep::ImageSkiaRep(cc::PaintRecord paint_record, const gfx::Size& pixel_size, float scale)
...
{
    if (type_ == ImageRepType::kImageTypeDrawable && bitmap_.isNull() && paint_record_) {
