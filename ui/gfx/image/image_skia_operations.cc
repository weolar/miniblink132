...
{
    return gfx::Rect(ScaleToFlooredPoint(dip_bounds.origin(), scale), DIPToPixelSize(dip_bounds.size(), scale));
}
...
protected:
    BinaryImageSource(const ImageSkia& first, const ImageSkia& second, const char* source_name)
        : first_(first)
        , second_(second)
    ...
    virtual ImageSkiaRep CreateImageSkiaRep(const ImageSkiaRep& first_rep, const ImageSkiaRep& second_rep) const = 0;
...
public:
    BlendingImageSource(const ImageSkia& first, const ImageSkia& second, double alpha)
    ...
    ImageSkiaRep CreateImageSkiaRep(const ImageSkiaRep& first_rep, const ImageSkiaRep& second_rep) const override
    {
        SkBitmap blended = SkBitmapOperations::CreateBlendedBitmap(first_rep.GetBitmap(), second_rep.GetBitmap(), alpha_);
...
public:
    SuperimposedImageSource(const ImageSkia& first, const ImageSkia& second)
        : gfx::CanvasImageSource(first.size())
        , first_(first)
        , second_(second)
    {
    }
    ...
    void Draw(Canvas* canvas) override
    {
        ...
        canvas->DrawImageInt(second_, (first_.width() - second_.width()) / 2, (first_.height() - second_.height()) / 2);
    }
...
private:
    ...
    ImageSkiaRep GetImageForScale(float scale) override
    {
        ...
        return ImageSkiaRep(SkBitmapOperations::CreateMaskedBitmap(image_rep.GetBitmap(), alpha), image_rep.scale());
    }
...
public:
    ...
    ImageSkiaRep CreateImageSkiaRep(const ImageSkiaRep& first_rep, const ImageSkiaRep& second_rep) const override
    {
        return ImageSkiaRep(SkBitmapOperations::CreateMaskedBitmap(first_rep.GetBitmap(), second_rep.GetBitmap()), first_rep.scale());
    }
};

class TiledImageSource : public gfx::ImageSkiaSource {
public:
    TiledImageSource(const ImageSkia& source, int src_x, int src_y, int dst_w, int dst_h)
        : source_(source)
        , src_x_(src_x)
        , src_y_(src_y)
        , dst_w_(dst_w)
    ...
    ImageSkiaRep GetImageForScale(float scale) override
    {
        ...
        gfx::Rect bounds = DIPToPixelBounds(gfx::Rect(src_x_, src_y_, dst_w_, dst_h_), source_rep.scale());
        return ImageSkiaRep(
            SkBitmapOperations::CreateTiledBitmap(source_rep.GetBitmap(), bounds.x(), bounds.y(), bounds.width(), bounds.height()), source_rep.scale());
    }
...
public:
    ...
    ImageSkiaRep GetImageForScale(float scale) override
    {
        ...
        return gfx::ImageSkiaRep(SkBitmapOperations::CreateHSLShiftedBitmap(image_rep.GetBitmap(), hsl_shift_), image_rep.scale());
    }
...
public:
    ButtonImageSource(SkColor color, const ImageSkia& image, const ImageSkia& mask)
        : color_(color)
        , image_(image)
        , mask_(mask)
    {
    ...
    ImageSkiaRep GetImageForScale(float scale) override
    {
        ...
        return gfx::ImageSkiaRep(SkBitmapOperations::CreateButtonBackground(color_, image_rep.GetBitmap(), mask_rep.GetBitmap()), image_rep.scale());
    }
...
public:
    ExtractSubsetImageSource(const gfx::ImageSkia& image, const gfx::Rect& subset_bounds)
        : image_(image)
    ...
    ImageSkiaRep GetImageForScale(float scale) override
    {
        ...
        SkIRect subset_bounds_in_pixel = RectToSkIRect(DIPToPixelBounds(subset_bounds_, image_rep.scale()));
        SkBitmap dst;
        bool success = image_rep.GetBitmap().extractSubset(&dst, subset_bounds_in_pixel);
        DCHECK(success);
...
public:
    ResizeSource(const ImageSkia& source, skia::ImageOperations::ResizeMethod method, const Size& target_dip_size)
        : source_(source)
    ...
    ImageSkiaRep GetImageForScale(float scale) override
    {
        ...
        const SkBitmap resized = skia::ImageOperations::Resize(image_rep.GetBitmap(), resize_method_, target_pixel_size.width(), target_pixel_size.height());
...
public:
    DropShadowSource(const ImageSkia& source, const ShadowValues& shadows_in_dip)
        : source_(source)
        , shadows_in_dip_(shadows_in_dip)
    {
    }
    ...
    ImageSkiaRep GetImageForScale(float scale) override
    {
        ...
        const SkBitmap shadow_bitmap = SkBitmapOperations::CreateDropShadow(image_rep.GetBitmap(), shadows_in_pixel);
...
public:
    HorizontalShadowSource(const std::vector<ShadowValue>& shadows, bool fades_down)
...
private:
    static int GetHeightForShadows(const std::vector<ShadowValue>& shadows)
    {
        ...
        for (const auto& shadow : shadows) {
            height = std::max(height, shadow.y() + base::ClampCeil(shadow.blur() / 2));
        }
        return height;
    }
...
public:
    RotatedSource(const ImageSkia& source, SkBitmapOperations::RotationAmount rotation)
        : source_(source)
        , rotation_(rotation)
    {
    }
    ...
    ImageSkiaRep GetImageForScale(float scale) override
    {
        ...
        const SkBitmap rotated_bitmap = SkBitmapOperations::Rotate(image_rep.GetBitmap(), rotation_);
...
public:
    IconWithBadgeSource(const ImageSkia& icon, const ImageSkia& badge)
        : gfx::CanvasImageSource(icon.size())
        , icon_(icon)
        , badge_(badge)
    {
    }
    ...
    void Draw(Canvas* canvas) override
    {
        ...
        canvas->DrawImageInt(badge_, (icon_.width() - badge_.width()), (icon_.height() - badge_.height()));
    }
...
public:
    ...
    ImageSkiaRep GetImageForScale(float scale) override
    {
        ...
        return ImageSkiaRep(SkBitmapOperations::CreateColorMask(image_rep.GetBitmap(), color_), image_rep.scale());
    }
...
public:
    ImageWithCircleBackgroundSource(int radius, SkColor color, const gfx::ImageSkia& image)
    ...
    ImageWithCircleBackgroundSource(const ImageWithCircleBackgroundSource&) = delete;
    ImageWithCircleBackgroundSource& operator=(const ImageWithCircleBackgroundSource&) = delete;
...
public:
    ImageWithRoundRectBackgroundSource(const SizeF& size, int radius, SkColor color, const gfx::ImageSkia& image)
    ...
    ImageWithRoundRectBackgroundSource(const ImageWithRoundRectBackgroundSource&) = delete;
    ImageWithRoundRectBackgroundSource& operator=(const ImageWithRoundRectBackgroundSource&) = delete;
...
public:
    ...
    ImageWithRoundRectClipSource& operator=(const ImageWithRoundRectClipSource&) = delete;
    ...
    void Draw(gfx::Canvas* canvas) override
    {
        canvas->ClipPath(SkPath().addRoundRect(gfx::RectToSkRect(gfx::Rect(image_.size())), radius_, radius_), true);
...
// static
ImageSkia ImageSkiaOperations::CreateBlendedImage(const ImageSkia& first, const ImageSkia& second, double alpha)
{
    if (first.isNull() || second.isNull())
        return ImageSkia();

    return ImageSkia(std::make_unique<BlendingImageSource>(first, second, alpha), first.size());
}

// static
ImageSkia ImageSkiaOperations::CreateSuperimposedImage(const ImageSkia& first, const ImageSkia& second)
{
    if (first.isNull() || second.isNull())
        return ImageSkia();

    return ImageSkia(std::make_unique<SuperimposedImageSource>(first, second), first.size());
}

// static
ImageSkia ImageSkiaOperations::CreateTransparentImage(const ImageSkia& image, double alpha)
{
    if (image.isNull())
        return ImageSkia();

    return ImageSkia(std::make_unique<TransparentImageSource>(image, alpha), image.size());
}

// static
ImageSkia ImageSkiaOperations::CreateMaskedImage(const ImageSkia& rgb, const ImageSkia& alpha)
{
...
// static
ImageSkia ImageSkiaOperations::CreateTiledImage(const ImageSkia& source, int src_x, int src_y, int dst_w, int dst_h)
{
    if (source.isNull())
        return ImageSkia();

    return ImageSkia(std::make_unique<TiledImageSource>(source, src_x, src_y, dst_w, dst_h), gfx::Size(dst_w, dst_h));
}

// static
ImageSkia ImageSkiaOperations::CreateHSLShiftedImage(const ImageSkia& image, const color_utils::HSL& hsl_shift)
{
    if (image.isNull())
        return ImageSkia();

    return ImageSkia(std::make_unique<HSLImageSource>(image, hsl_shift), image.size());
}

// static
ImageSkia ImageSkiaOperations::CreateButtonBackground(SkColor color, const ImageSkia& image, const ImageSkia& mask)
{
    if (image.isNull() || mask.isNull())
        return ImageSkia();

    return ImageSkia(std::make_unique<ButtonImageSource>(color, image, mask), mask.size());
}

// static
ImageSkia ImageSkiaOperations::ExtractSubset(const ImageSkia& image, const Rect& subset_bounds)
{
    ...
    return ImageSkia(std::make_unique<ExtractSubsetImageSource>(image, clipped_bounds), clipped_bounds.size());
}

// static
ImageSkia ImageSkiaOperations::CreateResizedImage(const ImageSkia& source, skia::ImageOperations::ResizeMethod method, const Size& target_dip_size)
{
    if (source.isNull())
        return ImageSkia();
    ...
    return ImageSkia(std::make_unique<ResizeSource>(source, method, target_dip_size), target_dip_size);
}

// static
ImageSkia ImageSkiaOperations::CreateImageWithDropShadow(const ImageSkia& source, const ShadowValues& shadows)
{
    if (source.isNull())
        return ImageSkia();
    ...
    return ImageSkia(std::make_unique<DropShadowSource>(source, shadows), shadow_image_size);
}

// static
ImageSkia ImageSkiaOperations::CreateHorizontalShadow(const std::vector<ShadowValue>& shadows, bool fades_down)
{
...
// static
ImageSkia ImageSkiaOperations::CreateRotatedImage(const ImageSkia& source, SkBitmapOperations::RotationAmount rotation)
{
    if (source.isNull())
        return ImageSkia();
...
// static
ImageSkia ImageSkiaOperations::CreateIconWithBadge(const ImageSkia& icon, const ImageSkia& badge)
{
    if (icon.isNull())
        return ImageSkia();
    ...
    return ImageSkia(std::make_unique<IconWithBadgeSource>(icon, badge), icon.size());
}

// static
ImageSkia ImageSkiaOperations::CreateColorMask(const ImageSkia& image, SkColor color)
{
    if (image.isNull())
        return ImageSkia();

    return ImageSkia(std::make_unique<ColorMaskSource>(image, color), image.size());
}

ImageSkia ImageSkiaOperations::CreateImageWithCircleBackground(int radius, SkColor color, const ImageSkia& image)
{
    DCHECK_GE(radius * 2, image.width());
    DCHECK_GE(radius * 2, image.height());
    return gfx::CanvasImageSource::MakeImageSkia<ImageWithCircleBackgroundSource>(radius, color, image);
}

ImageSkia ImageSkiaOperations::CreateImageWithRoundRectBackground(const SizeF& size, int radius, SkColor color, const ImageSkia& image)
{
    DCHECK_GE(size.width(), image.width());
    DCHECK_GE(size.height(), image.height());
    return gfx::CanvasImageSource::MakeImageSkia<ImageWithRoundRectBackgroundSource>(size, radius, color, image);
}

ImageSkia ImageSkiaOperations::CreateImageWithRoundRectClip(int radius, const ImageSkia& image)
{
    return gfx::CanvasImageSource::MakeImageSkia<ImageWithRoundRectClipSource>(radius, image);
}

ImageSkia ImageSkiaOperations::CreateCroppedCenteredRoundRectImage(const Size& size, int border_radius, const ImageSkia& image)
{
    float scale = std::min(static_cast<float>(image.width()) / size.width(), static_cast<float>(image.height()) / size.height());
    ...
    auto scaled_and_cropped_image = CreateTiledImage(image, bounds.x(), bounds.y(), bounds.width(), bounds.height());
    auto resized_image = CreateResizedImage(scaled_and_cropped_image, skia::ImageOperations::RESIZE_LANCZOS3, size);
