...
public:
    explicit FixedSource(const ImageSkiaRep& image)
        : image_(image)
    {
    }
...
public:
    explicit FixedScaleSource(const ImageSkiaRep& image)
        : image_(image)
    {
    }
...
private:
    ui::test::ScopedSetSupportedResourceScaleFactors scoped_set_supported_scale_factors_ { { ui::k100Percent, ui::k200Percent } };
};
...
TEST_F(ImageSkiaTest, FixedScaledSource)
{
    ...
    ImageSkia image_skia(std::make_unique<FixedScaleSource>(image), Size(100, 200));
    EXPECT_EQ(0U, image_skia.image_reps().size());
...
{
    ImageSkia image_skia(std::make_unique<DynamicSource>(Size(100, 200)), Size(100, 200));
    EXPECT_EQ(0U, image_skia.image_reps().size());
    const ImageSkiaRep& result_100p = image_skia.GetRepresentation(1.0f);
    EXPECT_EQ(100, result_100p.GetWidth());
    EXPECT_EQ(200, result_100p.GetHeight());
...
TEST_F(ImageSkiaTest, ManyRepsPerScaleFactor)
{
    ...
    ImageSkia image(std::make_unique<NullSource>(), gfx::Size(kSmallIcon1x, kSmallIcon1x));
    ...
    image.AddRepresentation(ImageSkiaRep(gfx::Size(kSmallIcon1x, kSmallIcon1x), 1.0f));
    image.AddRepresentation(ImageSkiaRep(gfx::Size(kSmallIcon2x, kSmallIcon2x), 2.0f));
    image.AddRepresentation(ImageSkiaRep(gfx::Size(kLargeIcon1x, kLargeIcon1x), 1.0f));
...
TEST_F(ImageSkiaTest, BackedBySameObjectAs)
{
    ...
    image.AddRepresentation(gfx::ImageSkiaRep(gfx::Size(10, 10), 1.0f));
    ImageSkia copy = image;
    copy.AddRepresentation(gfx::ImageSkiaRep(gfx::Size(10, 10), 2.0f));
    unrelated.AddRepresentation(gfx::ImageSkiaRep(gfx::Size(10, 10), 1.0f));
...
{
    ImageSkia image(std::make_unique<DynamicSource>(Size(100, 200)), Size(100, 200));
    EXPECT_FALSE(image.IsThreadSafe());
    ...
    EXPECT_EQ(ui::GetSupportedResourceScaleFactors().size(), image.image_reps().size());
    test::TestOnThread threadsafe_on_thread(&image);
    threadsafe_on_thread.StartAndJoin();
    EXPECT_TRUE(threadsafe_on_thread.can_read());
    EXPECT_FALSE(threadsafe_on_thread.can_modify());
...
{
    ImageSkia image(std::make_unique<FixedScaleSource>(ImageSkiaRep(Size(100, 200), 1.0f)), Size(100, 200));
...
{
    // 0.0f means unscaled.
    ImageSkia image(std::make_unique<FixedScaleSource>(ImageSkiaRep(Size(100, 200), 0.0f)), Size(100, 200));
...
TEST_F(ImageSkiaTest, SupportedScaleValue)
{
    ...
    // scale value is 2.0f).
    ui::test::ScopedSetSupportedResourceScaleFactors supported_scale_factors_override({ ui::k200Percent, ui::k300Percent, ui::kScaleFactorNone });
