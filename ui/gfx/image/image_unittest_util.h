...
const SkBitmap CreateBitmap(int width, int height, SkColor color = SK_ColorGREEN);
...
// data for an arbitrary scale factor.
gfx::ImageSkia CreateImageSkia(int width, int height, SkColor color = SK_ColorGREEN);

// Returns PNG encoded bytes for a bitmap of |edge_size|x|edge_size| and color
// `color`.
scoped_refptr<base::RefCountedMemory> CreatePNGBytes(int edge_size, SkColor color = SK_ColorGREEN);
...
bool AreImagesClose(const gfx::Image& image1, const gfx::Image& image2, int max_deviation);
...
bool AreBitmapsClose(const SkBitmap& bitmap1, const SkBitmap& bitmap2, int max_deviation);
...
//   scale_factor.
bool ImageSkiaStructureMatches(const gfx::ImageSkia& image_skia, int width, int height, const std::vector<float>& scale_factors);
