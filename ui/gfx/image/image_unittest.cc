...
{
    base::apple::ScopedCFTypeRef<CGColorSpaceRef> color_space(CGDisplayCopyColorSpace(CGMainDisplayID()));
    base::apple::ScopedCFTypeRef<CFStringRef> name(CGColorSpaceCopyName(color_space.get()));
    return name && CFStringCompare(name.get(), kCGColorSpaceSRGB, 0) == kCFCompareEqualTo;
}
...
private:
    ui::test::ScopedSetSupportedResourceScaleFactors scoped_set_supported_scale_factors_ { { ui::k100Percent,
#if !BUILDFLAG(IS_IOS)
        ui::k200Percent
#endif
    } };
...
TEST_F(ImageTest, EmptyImageFromObviouslyInvalidPNGImage)
{
    ...
    image_png_reps2.push_back(gfx::ImagePNGRep(new base::RefCountedBytes(), 1.0f));
...
TEST_F(ImageTest, ImageNo1xToPNG)
{
    ...
    gfx::ImageSkia image_skia;
    image_skia.AddRepresentation(gfx::ImageSkiaRep(gt::CreateBitmap(kSize2x, kSize2x), 2.0f));
    ...
    std::vector<gfx::ImagePNGRep> image_png_reps;
    image_png_reps.push_back(gfx::ImagePNGRep(gt::CreatePNGBytes(kSize2x), 2.0f));
...
TEST_F(ImageTest, CreateExtractPNGBytes)
{
    ...
    EXPECT_TRUE(std::ranges::equal(base::span(*bytes1x), base::span(*image.As1xPNGBytes())));
}
...
TEST_F(ImageTest, MultiResolutionImageSkiaToPNG)
{
    ...
    gfx::ImageSkia image_skia;
    image_skia.AddRepresentation(gfx::ImageSkiaRep(bitmap_1x, 1.0f));
    image_skia.AddRepresentation(gfx::ImageSkiaRep(gt::CreateBitmap(kSize2x, kSize2x), 2.0f));
    gfx::Image image(image_skia);

    EXPECT_TRUE(gt::ArePNGBytesCloseToBitmap(*image.As1xPNGBytes(), bitmap_1x, gt::MaxColorSpaceConversionColorShift()));
...
TEST_F(ImageTest, MultiResolutionPNGToImageSkia)
{
    ...
    EXPECT_TRUE(gt::ArePNGBytesCloseToBitmap(*bytes1x, image_skia.GetRepresentation(1.0f).GetBitmap(), gt::MaxColorSpaceConversionColorShift()));
    EXPECT_TRUE(gt::ArePNGBytesCloseToBitmap(*bytes2x, image_skia.GetRepresentation(2.0f).GetBitmap(), gt::MaxColorSpaceConversionColorShift()));
    EXPECT_TRUE(gt::ImageSkiaStructureMatches(image_skia, kSize1x, kSize1x, scales));
#if !BUILDFLAG(IS_IOS)
...
TEST_F(ImageTest, PreferDownscaleToUpscale)
{
    ...
    scoped_refptr<base::RefCountedMemory> bytes1x = gt::CreatePNGBytes(kSize1x, SK_ColorGREEN);
    scoped_refptr<base::RefCountedMemory> bytes4x = gt::CreatePNGBytes(kSize4x, SK_ColorBLUE);
...
TEST_F(ImageTest, MultiResolutionPNGToPlatform)
{
    ...
    const std::vector<ui::ResourceScaleFactor>& scales = ui::GetSupportedResourceScaleFactors();
    EXPECT_EQ(scales.size(), 1U);
    if (scales[0] == ui::k100Percent) {
        EXPECT_TRUE(gt::ArePNGBytesCloseToBitmap(*bytes1x, from_platform.AsBitmap(), gt::MaxColorSpaceConversionColorShift()));
    } else if (scales[0] == ui::k200Percent) {
        EXPECT_TRUE(gt::ArePNGBytesCloseToBitmap(*bytes2x, from_platform.AsBitmap(), gt::MaxColorSpaceConversionColorShift()));
    } else {
...
TEST_F(ImageTest, PNGEncodeFromSkiaDecodeToPlatform)
{
    ...
    gfx::Image from_bitmap = gfx::Image::CreateFrom1xBitmap(gt::CreateBitmap(25, 25));
    ...
    EXPECT_TRUE(gt::ArePNGBytesCloseToBitmap(*png_bytes, from_platform.AsBitmap(), gt::MaxColorSpaceConversionColorShift()));
}
...
TEST_F(ImageTest, PNGEncodeFromPlatformDecodeToSkia)
{
    ...
    EXPECT_TRUE(gt::AreBitmapsClose(from_platform.AsBitmap(), from_png.AsBitmap(), gt::MaxColorSpaceConversionColorShift()));
}

TEST_F(ImageTest, PNGDecodeToSkiaFailure)
{
    scoped_refptr<base::RefCountedBytes> invalid_bytes(new base::RefCountedBytes());
    invalid_bytes->as_vector().push_back('0');
    std::vector<gfx::ImagePNGRep> image_png_reps;
    image_png_reps.push_back(gfx::ImagePNGRep(invalid_bytes, 1.0f));
    gfx::Image image(image_png_reps);
...
{
#if BUILDFLAG(IS_MAC)
    LOG_IF(WARNING, !IsSystemColorSpaceSRGB()) << "This test is designed to pass with the sRGB color space, which is "
    ...
    gfx::Image from_skbitmap = gfx::Image::CreateFrom1xBitmap(bitmap);
    {
        ...
        gt::CheckColors(gt::GetPlatformImageColor(gt::ToPlatformType(from_skbitmap), 10, 10), SK_ColorRED);
        gt::CheckColors(gt::GetPlatformImageColor(gt::ToPlatformType(from_skbitmap), 10, 40), SK_ColorGREEN);
    }
...
TEST_F(ImageTest, SkBitmapConversionPreservesTransparency)
{
    ...
    gfx::Image from_skbitmap = gfx::Image::CreateFrom1xBitmap(bitmap);
    {
        SCOPED_TRACE("Checking color of the platform image");
        gt::CheckColors(gt::GetPlatformImageColor(gt::ToPlatformType(from_skbitmap), 10, 10), SK_ColorRED);
        gt::CheckIsTransparent(gt::GetPlatformImageColor(gt::ToPlatformType(from_skbitmap), 10, 40));
    }
...
TEST_F(ImageTest, Copy_PreservesRepresentation)
{
    ...
    std::vector<gfx::ImagePNGRep> image_png_reps;
    image_png_reps.push_back(gfx::ImagePNGRep(gt::CreatePNGBytes(kSize1x.width()), 1.0f));
    image_png_reps.push_back(gfx::ImagePNGRep(gt::CreatePNGBytes(kSize2x.width()), 2.0f));
    gfx::Image image(image_png_reps);
...
TEST_F(ImageTest, Copy_PreservesBackingStore)
{
    ...
    EXPECT_EQ(image_skia.bitmap()->getPixels(), image_skia2.bitmap()->getPixels());
}
...
TEST_F(ImageTest, MultiResolutionImageSkia)
{
    ...
    gfx::ImageSkia image_skia;
    image_skia.AddRepresentation(gfx::ImageSkiaRep(gt::CreateBitmap(kWidth1x, kHeight1x), 1.0f));
    image_skia.AddRepresentation(gfx::ImageSkiaRep(gt::CreateBitmap(kWidth2x, kHeight2x), 2.0f));

    std::vector<float> scales;
    scales.push_back(1.0f);
    scales.push_back(2.0f);
    EXPECT_TRUE(gt::ImageSkiaStructureMatches(image_skia, kWidth1x, kHeight1x, scales));
...
TEST_F(ImageTest, OwnershipTest)
{
    gfx::Image image;
    {
        ...
        image = gfx::Image(gfx::ImageSkia(gfx::ImageSkiaRep(bitmap, 1.0f)));
    }
