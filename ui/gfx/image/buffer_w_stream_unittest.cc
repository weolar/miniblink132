...
TEST(BufferWStreamTest, WriteBuffer)
{
    ...
    ASSERT_TRUE(stream.write(buffer2.data(), buffer2.size()));
    buffer_from_stream = stream.TakeBuffer();
    EXPECT_TRUE(base::ranges::equal(buffer.begin(), buffer.end(), buffer_from_stream.begin(), buffer_from_stream.begin() + buffer.size()));
    EXPECT_TRUE(base::ranges::equal(buffer2.begin(), buffer2.end(), buffer_from_stream.begin() + buffer.size(), buffer_from_stream.end()));
}
