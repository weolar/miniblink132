...
{
    return const_cast<ImageRepPNG*>(static_cast<const ImageRep*>(this)->AsImageRepPNG());
}
...
{
    return const_cast<ImageRepSkia*>(static_cast<const ImageRep*>(this)->AsImageRepSkia());
}
...
public:
    ...
    const std::vector<ImagePNGRep>& image_reps() const
    {
        return image_png_reps_;
    }
...
const ImageRep* ImageStorage::GetRepresentation(Image::RepresentationType rep_type, bool must_exist) const
{
    DCHECK(IsOnValidSequence());
...
const ImageRep* ImageStorage::AddRepresentation(std::unique_ptr<ImageRep> rep) const
{
    DCHECK(IsOnValidSequence());
...
Image Image::CreateFrom1xPNGBytes(base::span<const uint8_t> input)
{
    ...
    return CreateFrom1xPNGBytes(base::MakeRefCounted<base::RefCountedBytes>(input));
}

Image Image::CreateFrom1xPNGBytes(const scoped_refptr<base::RefCountedMemory>& input)
{
...
const ImageSkia* Image::ToImageSkia() const
{
    ...
    if (!rep) {
        ...
        case kImageRepPNG: {
            const internal::ImageRepPNG* png_rep = GetRepresentation(kImageRepPNG, true)->AsImageRepPNG();
            scoped_rep = std::make_unique<internal::ImageRepSkia>(internal::ImageSkiaFromPNG(png_rep->image_reps()));
            break;
        }
#if BUILDFLAG(IS_IOS)
        case kImageRepCocoaTouch: {
            const internal::ImageRepCocoaTouch* native_rep = GetRepresentation(kImageRepCocoaTouch, true)->AsImageRepCocoaTouch();
            scoped_rep = std::make_unique<internal::ImageRepSkia>(ImageSkiaFromUIImage(UIImageOfImageRepCocoaTouch(native_rep)));
            break;
        }
#elif BUILDFLAG(IS_MAC)
        case kImageRepCocoa: {
            const internal::ImageRepCocoa* native_rep = GetRepresentation(kImageRepCocoa, true)->AsImageRepCocoa();
            scoped_rep = std::make_unique<internal::ImageRepSkia>(ImageSkiaFromNSImage(NSImageOfImageRepCocoa(native_rep)));
            break;
        }
#endif
        default:
...
UIImage* Image::ToUIImage() const
{
    ...
    if (!rep) {
        ...
        case kImageRepPNG: {
            const internal::ImageRepPNG* png_rep = GetRepresentation(kImageRepPNG, true)->AsImageRepPNG();
            scoped_rep = internal::MakeImageRepCocoaTouch(internal::UIImageFromPNG(png_rep->image_reps()));
            break;
        }
        case kImageRepSkia: {
            const internal::ImageRepSkia* skia_rep = GetRepresentation(kImageRepSkia, true)->AsImageRepSkia();
...
NSImage* Image::ToNSImage() const
{
    ...
    if (!rep) {
        ...
        case kImageRepPNG: {
            const internal::ImageRepPNG* png_rep = GetRepresentation(kImageRepPNG, true)->AsImageRepPNG();
            scoped_rep = internal::MakeImageRepCocoa(internal::NSImageFromPNG(png_rep->image_reps()));
            break;
        }
        case kImageRepSkia: {
            const internal::ImageRepSkia* skia_rep = GetRepresentation(kImageRepSkia, true)->AsImageRepSkia();
...
scoped_refptr<base::RefCountedMemory> Image::As1xPNGBytes() const
{
...
#if BUILDFLAG(IS_IOS)
    case kImageRepCocoaTouch: {
        const internal::ImageRepCocoaTouch* cocoa_touch_rep = GetRepresentation(kImageRepCocoaTouch, true)->AsImageRepCocoaTouch();
        png_bytes = internal::Get1xPNGBytesFromUIImage(internal::UIImageOfImageRepCocoaTouch(cocoa_touch_rep));
        break;
    }
#elif BUILDFLAG(IS_MAC)
    case kImageRepCocoa: {
        const internal::ImageRepCocoa* cocoa_rep = GetRepresentation(kImageRepCocoa, true)->AsImageRepCocoa();
        png_bytes = internal::Get1xPNGBytesFromNSImage(internal::NSImageOfImageRepCocoa(cocoa_rep));
        break;
    }
#endif
    case kImageRepSkia: {
    ...
    AddRepresentation(base::WrapUnique(new internal::ImageRepPNG(image_png_reps)));
    return png_bytes;
}
...
const internal::ImageRep* Image::GetRepresentation(RepresentationType rep_type, bool must_exist) const
{
    CHECK(storage());
    return storage()->GetRepresentation(rep_type, must_exist);
}

const internal::ImageRep* Image::AddRepresentation(std::unique_ptr<internal::ImageRep> rep) const
{
    CHECK(storage());
