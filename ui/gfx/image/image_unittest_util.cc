...
scoped_refptr<base::RefCountedMemory> CreatePNGBytes(int edge_size, SkColor color)
{
    SkBitmap bitmap = CreateBitmap(edge_size, edge_size, color);
    std::optional<std::vector<uint8_t>> data = PNGCodec::EncodeBGRASkBitmap(bitmap, /*discard_transparency=*/false);
    return scoped_refptr<base::RefCountedBytes>(new base::RefCountedBytes(data.value_or(std::vector<uint8_t>())));
}
...
bool AreImagesClose(const gfx::Image& img1, const gfx::Image& img2, int max_deviation)
{
    ...
    for (size_t i = 0; i < img1_reps.size(); ++i) {
        ...
        if (image_rep2.scale() != scale || !AreBitmapsClose(img1_reps[i].GetBitmap(), image_rep2.GetBitmap(), max_deviation)) {
            return false;
        }
    }
    return true;
...
bool AreBitmapsClose(const SkBitmap& bmp1, const SkBitmap& bmp2, int max_deviation)
{
    if (bmp1.isNull() && bmp2.isNull())
        return true;

    if (bmp1.width() != bmp2.width() || bmp1.height() != bmp2.height() || bmp1.colorType() != kN32_SkColorType || bmp2.colorType() != kN32_SkColorType) {
        return false;
    }
...
bool ArePNGBytesCloseToBitmap(base::span<const uint8_t> bytes, const SkBitmap& bitmap, int max_deviation)
{
...
void CheckImageIndicatesPNGDecodeFailure(const gfx::Image& image)
{
    ...
    EXPECT_SKCOLOR_CLOSE(bitmap.getColor(10, 10), SK_ColorRED, MaxColorSpaceConversionColorShift());
}

bool ImageSkiaStructureMatches(const gfx::ImageSkia& image_skia, int width, int height, const std::vector<float>& scales)
{
    if (image_skia.isNull() || image_skia.width() != width || image_skia.height() != height || image_skia.image_reps().size() != scales.size()) {
        return false;
    }

    for (size_t i = 0; i < scales.size(); ++i) {
        gfx::ImageSkiaRep image_rep = image_skia.GetRepresentation(scales[i]);
        if (image_rep.is_null() || image_rep.scale() != scales[i])
            return false;

        if (image_rep.pixel_width() != static_cast<int>(width * scales[i]) || image_rep.pixel_height() != static_cast<int>(height * scales[i])) {
            return false;
        }
    }
    return true;
...
bool IsEmpty(const gfx::Image& image)
{
    ...
    return bmp.isNull() || (bmp.width() == 0 && bmp.height() == 0);
}
...
PlatformImage CreatePlatformImage()
{
    ...
    base::apple::ScopedCFTypeRef<CGColorSpaceRef> color_space(CGColorSpaceCreateDeviceRGB());
    UIImage* image = skia::SkBitmapToUIImageWithColorSpace(bitmap, scale, color_space.get());
    return image;
#elif BUILDFLAG(IS_MAC)
