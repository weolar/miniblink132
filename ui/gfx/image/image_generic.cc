...
public:
    ...
    ImageSkiaRep GetImageForScale(float scale) override
    {
        ...
        for (auto iter = image_skia_reps_.begin(); iter != image_skia_reps_.end(); ++iter) {
            if ((*iter).scale() == scale)
                return (*iter);
...
scoped_refptr<base::RefCountedMemory> Get1xPNGBytesFromImageSkia(const ImageSkia* image_skia)
{
    ...
    std::optional<std::vector<uint8_t>> result = PNGCodec::EncodeBGRASkBitmap(image_skia_rep.GetBitmap(), /*discard_transparency=*/false);
    if (!result) {
        return nullptr;
    }
