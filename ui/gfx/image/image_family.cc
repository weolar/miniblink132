...
ImageFamily::const_iterator::const_iterator(const std::map<MapKey, gfx::Image>::const_iterator& other)
    : map_iterator_(other)
{
}
...
gfx::Image ImageFamily::CreateExact(int width, int height) const
{
    ...
    SkBitmap resized_bitmap = skia::ImageOperations::Resize(bitmap, skia::ImageOperations::RESIZE_LANCZOS3, width, height);
...
const gfx::Image* ImageFamily::GetWithExactAspect(float aspect, int width) const
{
