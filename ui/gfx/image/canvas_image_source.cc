...
public:
    PaddedImageSource(const ImageSkia& image, const Insets& insets)
        : CanvasImageSource(Size(image.width() + insets.width(), image.height() + insets.height()))
        , image_(image)
        , insets_(insets)
    {
    }
...
// static
ImageSkia CanvasImageSource::CreatePadded(const ImageSkia& image, const Insets& insets)
{
    return MakeImageSkia<PaddedImageSource>(image, insets);
}

CanvasImageSource::CanvasImageSource(const Size& size)
    : size_(size)
{
}
...
ImageSkiaRep CanvasImageSource::GetImageForScale(float scale)
{
    ...
    return ImageSkiaRep(record_canvas.ReleaseAsRecord(), gfx::ScaleToCeiledSize(size_, scale), scale);
}
