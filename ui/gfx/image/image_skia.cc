...
ImageSkiaRep ScaleImageSkiaRep(const ImageSkiaRep& rep, float target_scale)
{
    ...
    return ImageSkiaRep(
        skia::ImageOperations::Resize(rep.GetBitmap(), skia::ImageOperations::RESIZE_LANCZOS3, scaled_size.width(), scaled_size.height()), target_scale);
}
...
public:
    ImageSkiaStorage(std::unique_ptr<ImageSkiaSource> source, const gfx::Size& size);
    ...
    std::vector<ImageSkiaRep>::const_iterator FindRepresentation(float scale, bool fetch_new_image) const;
...
ImageSkiaStorage::ImageSkiaStorage(std::unique_ptr<ImageSkiaSource> source, const gfx::Size& size)
...
ImageSkiaStorage::ImageSkiaStorage(std::unique_ptr<ImageSkiaSource> source, float scale)
    : source_(std::move(source))
    , read_only_(false)
{
    DCHECK(source_);
...
std::vector<ImageSkiaRep>::const_iterator ImageSkiaStorage::FindRepresentation(float scale, bool fetch_new_image) const
{
    ...
    if (fetch_new_image && source_) {
        DCHECK(sequence_checker_.CalledOnValidSequence()) << "An ImageSkia with the source must be accessed by the same "
                                                             "sequence.";
        ...
        if (!HasRepresentationAtAllScales()) {
            resource_scale = ui::GetScaleForResourceScaleFactor(ui::GetSupportedResourceScaleFactorForRescale(scale));
        }
        ...
        if (!image.is_null() && !base::Contains(image_reps_, image.scale(), &ImageSkiaRep::scale)) {
...
ImageSkia::ImageSkia(std::unique_ptr<ImageSkiaSource> source, const gfx::Size& size)
    : storage_(base::MakeRefCounted<internal::ImageSkiaStorage>(std::move(source), size))
{
...
ImageSkia::ImageSkia(std::unique_ptr<ImageSkiaSource> source, float scale)
    : storage_(base::MakeRefCounted<internal::ImageSkiaStorage>(std::move(source), scale))
{
...
std::vector<ImageSkiaRep> ImageSkia::image_reps() const
{
    ...
    for (auto it = internal_image_reps.begin(); it != internal_image_reps.end(); ++it) {
        if (!it->is_null())
...
void ImageSkia::EnsureRepsForSupportedScales() const
{
    ...
    const std::vector<ui::ResourceScaleFactor>& supported_scales = ui::GetSupportedResourceScaleFactors();
    ...
    if (storage_.get() && storage_->has_source()) {
        for (const auto scale : supported_scales) {
            storage_->FindRepresentation(ui::GetScaleForResourceScaleFactor(scale), true);
        }
    }
}
...
void ImageSkia::RemoveUnsupportedRepresentationsForScale(float scale)
{
    for (const ImageSkiaRep& image_rep_to_test : image_reps()) {
        ...
        if (test_scale != scale && ui::GetScaleForResourceScaleFactor(ui::GetSupportedResourceScaleFactorForRescale(test_scale)) == scale) {
...
{
    DCHECK(!image_rep.is_null());
    storage_ = new internal::ImageSkiaStorage(nullptr, gfx::Size(image_rep.GetWidth(), image_rep.GetHeight()));
