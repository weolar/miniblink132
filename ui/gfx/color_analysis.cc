...
public:
    ...
    static bool SortKMeanClusterByWeight(const KMeanCluster& a, const KMeanCluster& b)
    {
...
public:
    ...
    ColorBox Split()
    {
        ...
        const uint8_t long_dimension = std::max({ r_dimension, g_dimension, b_dimension });
        const enum {
            RED,
            GREEN,
            BLUE,
        } channel
            = long_dimension == r_dimension ? RED
            : long_dimension == g_dimension ? GREEN
                                            : BLUE;
        ...
        // sorted.
        std::sort(color_space_->begin() + color_range_.start(), color_space_->begin() + color_range_.end(), sort_function);
        ...
        for (uint32_t i = color_range_.start() + 1; i < color_range_.end() - 1; ++i) {
            bool past_midpoint = false;
            switch (channel) {
            case RED:
                past_midpoint = static_cast<uint8_t>(SkColorGetR((*color_space_)[i])) > (min_r_ + max_r_) / 2;
                break;
            case GREEN:
                past_midpoint = static_cast<uint8_t>(SkColorGetG((*color_space_)[i])) > (min_g_ + max_g_) / 2;
                break;
            case BLUE:
                past_midpoint = static_cast<uint8_t>(SkColorGetB((*color_space_)[i])) > (min_b_ + max_b_) / 2;
                break;
            }
            if (past_midpoint) {
                split_point = i;
    ...
    // |color_counts|.
    Swatch GetWeightedAverageColor(const std::unordered_map<SkColor, int>& color_counts) const
    {
        size_t sum_r = 0;
        size_t sum_g = 0;
        size_t sum_b = 0;
        ...
        return Swatch(SkColorSetRGB(std::round(static_cast<double>(sum_r) / total_count_in_box), std::round(static_cast<double>(sum_g) / total_count_in_box),
...
private:
    ...
    void RecomputeBounds()
    {
        ...
        volume_ = (max_r_ - min_r_ + 1) * (max_g_ - min_g_ + 1) * (max_b_ - min_b_ + 1);
    }
...
{
    const float average_channel_value = (SkColorGetR(color) + SkColorGetG(color) + SkColorGetB(color)) / 3.0f;
...
std::vector<Swatch> CalculateProminentColors(
    const SkBitmap& bitmap, const std::vector<ColorBracket>& color_brackets, const gfx::Rect& region, std::optional<ColorSwatchFilter> filter)
{
    DCHECK(!bitmap.empty());
    DCHECK(!bitmap.isNull());

    std::vector<Swatch> box_colors = CalculateColorSwatches(bitmap, 12, region, filter);
    ...
    for (size_t i = 0; i < color_brackets.size(); ++i) {
        ...
        for (const auto& box_color : box_colors) {
            ...
            if (!IsWithinHSLRange(hsl, color_brackets[i].lower_bound, color_brackets[i].upper_bound)) {
                continue;
            }

            double suitability = (1 - std::abs(hsl.s - color_brackets[i].goal.s)) * 3 + (1 - std::abs(hsl.l - color_brackets[i].goal.l)) * 6.5
...
int GridSampler::GetSample(int width, int height)
{
    ...
    int x = kInsetX + (calls_ / kNumberOfClusters) * ((width - 2 * kInsetX) / kNumberOfClusters);
    int y = kInsetY + (calls_ % kNumberOfClusters) * ((height - 2 * kInsetY) / kNumberOfClusters);
    int index = x + (y * width);
    ++calls_;
    return index % (width * height);
}

SkColor FindClosestColor(base::span<const uint8_t> image, int width, int height, SkColor color)
{
...
SkColor CalculateKMeanColorOfBuffer(base::span<const uint8_t> decoded_data, int img_width, int img_height, const HSL& lower_bound, const HSL& upper_bound,
    KMeanImageSampler* sampler, bool find_closest)
{
    ...
    if (img_width > 0 && img_height > 0) {
        ...
        while (new_cluster != clusters.end()) {
            ...
            for (int i = 0; i < 10; ++i) {
                ...
                // this color before.
                color_unique = true;
                for (auto cluster = clusters.begin(); cluster != new_cluster; ++cluster) {
        ...
        for (int iteration = 0; iteration < kNumberOfIterations && !convergence; ++iteration) {
            ...
            auto decoded_data_end = decoded_data.begin() + (img_width * img_height * 4);
    ...
    return find_closest ? FindClosestColor(decoded_data, img_width, img_height, color) : color;
}

SkColor CalculateKMeanColorOfPNG(base::span<const uint8_t> png, const HSL& lower_bound, const HSL& upper_bound, KMeanImageSampler* sampler)
{
    if (png.empty()) {
        return kDefaultBgColor;
    }

    std::optional<gfx::PNGCodec::DecodeOutput> output = gfx::PNGCodec::Decode(png, gfx::PNGCodec::FORMAT_BGRA);
    if (!output) {
        return kDefaultBgColor;
    }

    base::span<const uint8_t> decoded_data(output->output.data(), output->output.size());
    return CalculateKMeanColorOfBuffer(decoded_data, output->width, output->height, lower_bound, upper_bound, sampler, /*find_closest=*/true);
}

SkColor CalculateKMeanColorOfPNG(base::span<const uint8_t> png)
{
    GridSampler sampler;
    return CalculateKMeanColorOfPNG(png, kDefaultLowerHSLBound, kDefaultUpperHSLBound, &sampler);
}

SkColor CalculateKMeanColorOfBitmap(const SkBitmap& bitmap, int height, const HSL& lower_bound, const HSL& upper_bound, bool find_closest)
{
    ...
    base::HeapArray<uint32_t> image = base::HeapArray<uint32_t>::Uninit(pixel_count);
    ...
    UNSAFE_BUFFERS(base::span<uint32_t> in(static_cast<uint32_t*>(bitmap.getPixels()), base::checked_cast<size_t>(pixel_count)));

    // Un-premultiply into the out buffer.
    std::transform(in.begin(), in.end(), image.begin(), SkUnPreMultiply::PMColorToColor);

    GridSampler sampler;
    *(int*)1 = 1;
    base::span<const uint8_t> decoded_data((const uint8_t*)image.data(), image.size() * sizeof(uint32_t));
    return CalculateKMeanColorOfBuffer(decoded_data, bitmap.width(), height, lower_bound, upper_bound, &sampler, find_closest);
}

SkColor CalculateKMeanColorOfBitmap(const SkBitmap& bitmap)
{
    return CalculateKMeanColorOfBitmap(bitmap, bitmap.height(), kDefaultLowerHSLBound, kDefaultUpperHSLBound, true);
}
...
//   existing colors.
std::vector<Swatch> CalculateColorSwatches(const SkBitmap& bitmap, size_t max_swatches, const gfx::Rect& region, std::optional<ColorSwatchFilter> filter)
{
    DCHECK(!bitmap.empty());
    DCHECK(!bitmap.isNull());
    ...
    const float pixel_increment = std::max(1.0f, static_cast<float>(pixel_count) / kMaxConsideredPixelsForSwatches);
    std::unordered_map<SkColor, int> color_counts(kMaxConsideredPixelsForSwatches);
    ...
    std::priority_queue<ColorBox, std::vector<ColorBox>, bool (*)(const ColorBox&, const ColorBox&)> boxes(&ColorBox::CompareByVolume);
...
std::vector<color_utils::Swatch> CalculateProminentColorsOfBitmap(
    const SkBitmap& bitmap, const std::vector<ColorProfile>& color_profiles, gfx::Rect* region, ColorSwatchFilter filter)
{
    ...
    return CalculateProminentColors(bitmap, color_brackets, region ? *region : gfx::Rect(bitmap.width(), bitmap.height()),
