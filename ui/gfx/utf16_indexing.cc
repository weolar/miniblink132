...
{
    return index == 0 || index == s.length() || !(CBU16_IS_TRAIL(s[index]) && CBU16_IS_LEAD(s[index - 1]));
}
...
size_t UTF16OffsetToIndex(const std::u16string& s, size_t base, ptrdiff_t offset)
{
    DCHECK_LE(base, s.length());
