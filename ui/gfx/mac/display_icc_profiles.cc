...
base::apple::ScopedCFTypeRef<CFDataRef> DisplayICCProfiles::GetDataForColorSpace(const ColorSpace& color_space)
{
    UpdateIfNeeded();
...
{
    CGDisplayRegisterReconfigurationCallback(DisplayICCProfiles::DisplayReconfigurationCallBack, this);
}
...
void DisplayICCProfiles::UpdateIfNeeded()
{
    ...
    base::apple::ScopedCFTypeRef<CFDataRef> srgb_icc(CGColorSpaceCopyICCData(CGColorSpaceCreateWithName(kCGColorSpaceSRGB)));
    ...
    error = CGGetActiveDisplayList(displays.size(), displays.data(), &display_count);
    if (error != kCGErrorSuccess)
        return;

    for (uint32_t i = 0; i < display_count; ++i) {
        base::apple::ScopedCFTypeRef<CGColorSpaceRef> cg_color_space(CGDisplayCopyColorSpace(displays[i]));
        if (!cg_color_space)
            continue;
        base::apple::ScopedCFTypeRef<CFDataRef> icc_data(CGColorSpaceCopyICCData(cg_color_space.get()));
        if (!icc_data)
            continue;
        ICCProfile icc_profile = ICCProfile::FromData(CFDataGetBytePtr(icc_data.get()), CFDataGetLength(icc_data.get()));
...
// static
void DisplayICCProfiles::DisplayReconfigurationCallBack(CGDirectDisplayID display, CGDisplayChangeSummaryFlags flags, void* user_info)
{
    DisplayICCProfiles* profiles = reinterpret_cast<DisplayICCProfiles*>(user_info);
