...
BASE_FEATURE(kIOSurfaceUseNamedSRGBForREC709, "IOSurfaceUseNamedSRGBForREC709", base::FEATURE_ENABLED_BY_DEFAULT);

void AddIntegerValue(CFMutableDictionaryRef dictionary, const CFStringRef key, int32_t value)
{
    base::apple::ScopedCFTypeRef<CFNumberRef> number(CFNumberCreate(nullptr, kCFNumberSInt32Type, &value));
...
uint32_t BufferFormatToIOSurfacePixelFormat(gfx::BufferFormat format, bool override_rgba_to_bgra)
{
    switch (format) {
    case gfx::BufferFormat::R_8:
        return 'L008';
...
mach_port_t IOSurfaceMachPortTraits::Retain(mach_port_t port)
{
    ...
    MACH_LOG_IF(ERROR, kr != KERN_SUCCESS, kr) << "IOSurfaceMachPortTraits::Retain mach_port_mod_refs";
    return port;
}
...
{
    kern_return_t kr = mach_port_mod_refs(mach_task_self(), port, MACH_PORT_RIGHT_SEND, -1);
    MACH_LOG_IF(ERROR, kr != KERN_SUCCESS, kr) << "IOSurfaceMachPortTraits::Release mach_port_mod_refs";
}

// Common method used by IOSurfaceSetColorSpace and IOSurfaceCanSetColorSpace.
bool IOSurfaceSetColorSpace(IOSurfaceRef io_surface, const ColorSpace& color_space)
{
    // Allow but ignore invalid color spaces.
    if (!color_space.IsValid())
        return true;

    static const bool prefer_srgb_trfn = base::FeatureList::IsEnabled(kIOSurfaceUseNamedSRGBForREC709);

    // Prefer using named spaces.
    CFStringRef color_space_name = nullptr;
    if (color_space == ColorSpace::CreateSRGB() || (prefer_srgb_trfn && color_space == ColorSpace::CreateREC709())) {
    ...
    if (color_space == ColorSpace(ColorSpace::PrimaryID::BT2020, ColorSpace::TransferID::PQ, ColorSpace::MatrixID::BT2020_NCL, ColorSpace::RangeID::LIMITED)) {
        color_space_name = kCGColorSpaceITUR_2100_PQ;
    } else if (color_space
        == ColorSpace(ColorSpace::PrimaryID::BT2020, ColorSpace::TransferID::HLG, ColorSpace::MatrixID::BT2020_NCL, ColorSpace::RangeID::LIMITED)) {
    ...
    // TODO(b/304442486): Add gamma support here.
    {
        ...
        if (ColorSpaceToCVImageBufferKeys(color_space, prefer_srgb_trfn, &primaries, &transfer, &matrix)) {
            IOSurfaceSetValue(io_surface, CFSTR("IOSurfaceColorPrimaries"), primaries);
            IOSurfaceSetValue(io_surface, CFSTR("IOSurfaceTransferFunction"), transfer);
    ...
    base::apple::ScopedCFTypeRef<CFDataRef> cf_data_icc_profile(
        CFDataCreate(nullptr, reinterpret_cast<const UInt8*>(icc_profile_data.data()), icc_profile_data.size()));

    IOSurfaceSetValue(io_surface, CFSTR("IOSurfaceColorSpace"), cf_data_icc_profile.get());
    return true;
}

} // namespace internal

base::apple::ScopedCFTypeRef<IOSurfaceRef> CreateIOSurface(const gfx::Size& size, gfx::BufferFormat format, bool should_clear, bool override_rgba_to_bgra)
{
    ...
    AddIntegerValue(properties.get(), kIOSurfacePixelFormat, BufferFormatToIOSurfacePixelFormat(format, override_rgba_to_bgra));
    ...
    if (num_planes > 1) {
        base::apple::ScopedCFTypeRef<CFMutableArrayRef> planes(CFArrayCreateMutable(kCFAllocatorDefault, num_planes, &kCFTypeArrayCallBacks));
        size_t total_bytes_alloc = 0;
        for (size_t plane = 0; plane < num_planes; ++plane) {
            const size_t factor = gfx::SubsamplingFactorForBufferFormat(format, plane);
            ...
            const size_t plane_bytes_per_row
                = IOSurfaceAlignProperty(kIOSurfacePlaneBytesPerRow, base::bits::AlignUp(plane_width, size_t { 2 }) * plane_bytes_per_element);
            const size_t plane_bytes_alloc = IOSurfaceAlignProperty(kIOSurfacePlaneSize, base::bits::AlignUp(plane_height, size_t { 2 }) * plane_bytes_per_row);
            const size_t plane_offset = IOSurfaceAlignProperty(kIOSurfacePlaneOffset, total_bytes_alloc);
            ...
            AddIntegerValue(plane_info.get(), kIOSurfacePlaneBytesPerElement, plane_bytes_per_element);
            AddIntegerValue(plane_info.get(), kIOSurfacePlaneBytesPerRow, plane_bytes_per_row);
        ...
        const size_t bytes_per_row = IOSurfaceAlignProperty(kIOSurfaceBytesPerRow, base::bits::AlignUpDeprecatedDoNotUse(size.width(), 2) * bytes_per_element);
        const size_t bytes_alloc = IOSurfaceAlignProperty(kIOSurfaceAllocSize, base::bits::AlignUpDeprecatedDoNotUse(size.height(), 2) * bytes_per_row);
        AddIntegerValue(properties.get(), kIOSurfaceBytesPerElement, bytes_per_element);
    ...
    base::apple::ScopedCFTypeRef<IOSurfaceRef> surface(IOSurfaceCreate(properties.get()));
    if (!surface) {
...
void IOSurfaceSetColorSpace(IOSurfaceRef io_surface, const ColorSpace& color_space)
{
    if (!internal::IOSurfaceSetColorSpace(io_surface, color_space)) {
        DLOG(ERROR) << "Failed to set color space for IOSurface: " << color_space.ToString();
    }
}

COMPONENT_EXPORT(GFX)
base::apple::ScopedCFTypeRef<IOSurfaceRef> IOSurfaceMachPortToIOSurface(ScopedRefCountedIOSurfaceMachPort io_surface_mach_port)
{
