...
struct ScopedInUseIOSurfaceTraits {
    static IOSurfaceRef InvalidValue()
    {
        return nullptr;
    }
...
base::apple::ScopedCFTypeRef<IOSurfaceRef> CreateIOSurface(const Size& size, BufferFormat format, bool should_clear = true,
#if BUILDFLAG(IS_IOS)
...
// decremented.
using ScopedRefCountedIOSurfaceMachPort = base::apple::ScopedTypeRef<mach_port_t, internal::IOSurfaceMachPortTraits>;

// A scoper for holding a reference to an IOSurface and also incrementing its
// in-use counter while the scoper exists.
using ScopedInUseIOSurface = base::apple::ScopedTypeRef<IOSurfaceRef, internal::ScopedInUseIOSurfaceTraits>;
...
COMPONENT_EXPORT(GFX)
void IOSurfaceSetColorSpace(IOSurfaceRef io_surface, const gfx::ColorSpace& color_space);

// Return the expected four character code pixel format for an IOSurface with
// the specified gfx::BufferFormat.
COMPONENT_EXPORT(GFX)
uint32_t BufferFormatToIOSurfacePixelFormat(gfx::BufferFormat format, bool override_rgba_to_bgra = true);
