...
void ExtensionManager::Init(Connection* conn)
{
    ...
    screensaver_ = MakeExtension<ScreenSaver>(conn, std::move(screensaver_future));
...
void ExtensionManager::GetEventTypeAndOp(const void* raw_event, uint8_t* type_id, uint8_t* opcode) const
{
