...
void SetEventMask(Connection* connection, Window window, EventMask new_mask)
{
    ...
    // ignore the error.
    connection->ChangeWindowAttributes(ChangeWindowAttributesRequest { .window = window, .event_mask = static_cast<EventMask>(new_mask) }).IgnoreError();
}
...
ScopedEventSelector::ScopedEventSelector(Connection* connection, Window window, EventMask event_mask)
    : window_(window)
...
ScopedEventSelector& ScopedEventSelector::operator=(ScopedEventSelector&& other)
{
    Reset();
    Swap(other);
    return *this;
