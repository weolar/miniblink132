...
Window GetWindowAtPoint(const gfx::Point& point_px, const base::flat_set<Window>* ignore = nullptr);
...
public:
    ...
    Window GetWindowAtPoint(gfx::Point point_px, Window window, const base::flat_set<Window>* ignore = nullptr);
...
private:
    ...
    template <typename Future, typename Callback, typename... Args> void AddRequest(Future&& future, Callback&& callback, Args&&... args)
    {
        future.OnResponse(base::BindOnce(callback, weak_factory_.GetWeakPtr(), std::forward<Args>(args)...));
    ...
    void OnGetWindowAttributesResponse(Window window, GetWindowAttributesResponse response);
    ...
    void OnGetPropertyResponse(Window window, Atom atom, GetPropertyResponse response);

    void OnGetRectanglesResponse(Window window, Shape::Sk kind, Shape::GetRectanglesResponse response);
