...
protected:
    base::flat_map<Atom, PropertyCache::PropertyValue>& GetProperties(PropertyCache* property_cache)
    {
...
TEST_F(PropertyCacheTest, Event)
{
    ...
    connection()
        ...
        .OnResponse(base::BindOnce([](bool* have_response, Response<void> response) { *have_response = true; }, &have_response));
...
TEST_F(PropertyCacheTest, GetAs)
{
    ...
    connection()->SetProperty(window(), atom, Atom::CARDINAL, static_cast<uint8_t>(123));
    connection()->DispatchAll();
    ...
    connection()->SetArrayProperty(window(), atom, Atom::CARDINAL, std::vector<uint32_t>());
    connection()->DispatchAll();
    value = cache.GetAs<uint32_t>(atom, &size);
    EXPECT_EQ(size, 0u);
    EXPECT_FALSE(value);
