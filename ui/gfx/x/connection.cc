...
{
    LOG(WARNING) << "X error received.  Request: " << request_name << "Request, Error: " << error->ToString();
}
...
public:
    explicit UnknownError(RawError error_bytes)
    ...
    std::string ToString() const override
    {
        ...
        // here.
        UNSAFE_BUFFERS(base::span<const uint8_t> bytes(error_bytes_->bytes(), kMinimumErrorSize));
...
Connection::Connection(const std::string& address)
    : XProto(this)
    , display_string_(address.empty() ? base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kX11Display) : address)
    , connection_(xcb_connect(display_string_.empty() ? nullptr : display_string_.c_str(), &default_screen_id_), xcb_disconnect)
    ...
    , window_event_manager_(this)
{
    ...
    // sparse map.
    std::array<const Format*, std::numeric_limits<decltype(Format::depth)>::max()> formats {};
    ...
    for (const auto& depth : default_screen().allowed_depths) {
        ...
        for (const auto& visual : depth.visuals) {
            default_screen_visuals.emplace_back(visual.visual_id, VisualInfo { format, &visual });
        }
    }
    default_screen_visuals_ = base::flat_map<VisualId, VisualInfo>(std::move(default_screen_visuals));
    ...
    root_props_ = std::make_unique<PropertyCache>(this, default_root(), std::vector<Atom> { GetAtom("_NET_SUPPORTING_WM_CHECK"), GetAtom("_NET_SUPPORTED") },
        base::BindRepeating(&Connection::OnRootPropertyChanged, base::Unretained(this)));
}
...
{
    return 4 * std::max<size_t>(extended_max_request_length_, setup_.maximum_request_length);
}
...
void Connection::SetStringProperty(Window window, Atom property, Atom type, const std::string& value)
{
...
{
    ConfigureWindow(ConfigureWindowRequest { .window = window, .stack_mode = StackMode::Above });
}

void Connection::LowerWindow(Window window)
{
    ConfigureWindow(ConfigureWindowRequest { .window = window, .stack_mode = StackMode::Below });
}

void Connection::DefineCursor(Window window, Cursor cursor)
{
    ChangeWindowAttributes(ChangeWindowAttributesRequest { .window = window, .cursor = cursor });
}

ScopedEventSelector Connection::ScopedSelectEvent(Window window, EventMask event_mask)
{
...
std::string Connection::GetWmName() const
{
    if (WmSupportsEwmh()) {
        size_t size;
        if (const char* name = wm_props_->GetAs<char>(GetAtom("_NET_WM_NAME"), &size)) {
            std::string wm_name;
...
void Connection::Request::SetResponse(Connection* connection, void* raw_reply, void* raw_error)
{
    have_response = true;
    if (raw_reply) {
    ...
    if (raw_error) {
        error = connection->ParseError(base::MakeRefCounted<MallocedRefCountedMemory>(raw_error));
    }
}
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    auto* event = queued ? xcb_poll_for_queued_event(XcbConnection()) : xcb_poll_for_event(XcbConnection());
    if (event) {
        events_.emplace_back(base::MakeRefCounted<MallocedRefCountedMemory>(event), this);
    }
    return event;
}
...
const Connection::VisualInfo* Connection::GetVisualInfoFromId(VisualId id) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
uint32_t Connection::KeycodeToKeysym(KeyCode keycode, uint32_t modifiers) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void Connection::InitializeExtensions()
{
    ...
    auto randr_future = randr().QueryVersion(RandR::major_version, RandR::minor_version);
    auto render_future = render().QueryVersion(Render::major_version, Render::minor_version);
    auto screensaver_future = screensaver().QueryVersion(ScreenSaver::major_version, ScreenSaver::minor_version);
    shape().QueryVersion();
    auto shm_future = shm().QueryVersion();
    auto sync_future = sync().Initialize(Sync::major_version, Sync::minor_version);
    xfixes().QueryVersion(XFixes::major_version, XFixes::minor_version);
    auto xinput_future = xinput().XIQueryVersion(Input::major_version, Input::minor_version);
    ...
    if (auto response = screensaver_future.Sync()) {
        screensaver_version_ = { response->server_major_version, response->server_minor_version };
    }
...
void Connection::ProcessNextResponse()
{
    ...
    requests_.pop_front();
    if (last_non_void_request_id_.has_value() && last_non_void_request_id_.value() == first_request_id_) {
        last_non_void_request_id_ = std::nullopt;
    }
    first_request_id_++;
    if (request.callback) {
        std::move(request.callback).Run(std::move(request.reply), std::move(request.error));
    }
}

Future<void> Connection::SetArrayPropertyImpl(Window window, Atom name, Atom type, uint8_t format, base::span<const uint8_t> values)
{
    return ChangeProperty(ChangePropertyRequest { .window = static_cast<Window>(window),
        .property = name,
        .type = type,
        .format = format,
...
std::unique_ptr<FutureImpl> Connection::SendRequestImpl(WriteBuffer* buf, const char* request_name_for_tracing, bool generates_reply, bool reply_has_fds)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    auto callback = base::BindOnce(
        [](const char* request_name, RawReply raw_reply, std::unique_ptr<Error> error) {
            if (error) {
    ...
    return std::make_unique<FutureImpl>(this, sequence, generates_reply, request_name_for_tracing);
}
...
void Connection::WaitForResponse(FutureImpl* future)
{
    ...
    if (future->generates_reply()) {
        if (!xcb_poll_for_reply(XcbConnection(), future->sequence(), &reply, &error)) {
            TRACE_EVENT1("ui", "xcb_wait_for_reply", "request", future->request_name_for_tracing());
        ...
        if (!last_non_void_request_id_.has_value()) {
            ...
            SequenceType last_non_void_offset = last_non_void_request_id_.value() - first_request_id_;
        ...
        {
            TRACE_EVENT1("ui", "xcb_request_check", "request", future->request_name_for_tracing());
...
void Connection::PreDispatchEvent(const Event& event)
{
    if (auto* mapping = event.As<MappingNotifyEvent>()) {
        if (mapping->request == Mapping::Modifier || mapping->request == Mapping::Keyboard) {
            setup_.min_keycode = mapping->first_keycode;
            setup_.max_keycode = static_cast<KeyCode>(static_cast<int>(mapping->first_keycode) + mapping->count - 1);
    ...
    if (auto* configure = event.As<ConfigureNotifyEvent>()) {
        ...
        CHECK_GE(index, 0);
        bool portrait = static_cast<bool>(screen->rotation & (RandR::Rotation::Rotate_90 | RandR::Rotation::Rotate_270));
        if (portrait) {
...
void Connection::OnRootPropertyChanged(Atom property, const GetPropertyResponse& value)
{
    ...
    if (property == check_atom) {
        ...
        if (wm_window != Window::None) {
            wm_props_ = std::make_unique<PropertyCache>(this, wm_window, std::vector<Atom> { check_atom, GetAtom("_NET_WM_NAME") });
        }
    }
}
...
void Connection::AttemptSyncWithWm()
{
    ...
    wm_sync_ = std::make_unique<WmSync>(this, base::BindOnce(&Connection::OnWmSynced, base::Unretained(this)), true);
}
