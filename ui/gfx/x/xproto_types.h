...
// Undefined Behaviour.
class COMPONENT_EXPORT(X11) UnsizedRefCountedMemory : public base::RefCountedThreadSafe<UnsizedRefCountedMemory> {
public:
    uint8_t* bytes()
    {
...
class COMPONENT_EXPORT(X11) ThrowAwaySizeRefCountedMemory final : public UnsizedRefCountedMemory {
public:
    static scoped_refptr<ThrowAwaySizeRefCountedMemory> From(std::vector<uint8_t> data)
    {
    ...
    ThrowAwaySizeRefCountedMemory& operator=(const ThrowAwaySizeRefCountedMemory&) = delete;
...
class COMPONENT_EXPORT(X11) SizedRefCountedMemory final : public base::RefCountedMemory {
public:
    // SAFETY: The caller must ensure that the `mem` buffer points to at least
    // `size` many bytes or Undefined Behaviour can result.
    UNSAFE_BUFFER_USAGE static scoped_refptr<SizedRefCountedMemory> From(scoped_refptr<UnsizedRefCountedMemory> mem, size_t size)
    {
...
private:
    SizedRefCountedMemory(scoped_refptr<UnsizedRefCountedMemory> mem, size_t size);
...
using ResponseCallback = base::OnceCallback<void(RawReply reply, std::unique_ptr<Error> error)>;
...
template <typename T> void VerifyAlignment(T* t, size_t offset)
{
...
struct COMPONENT_EXPORT(X11) ReadBuffer {
    explicit ReadBuffer(scoped_refptr<UnsizedRefCountedMemory> data, bool setup_message = false);
...
public:
    ...
    UNSAFE_BUFFER_USAGE void AppendBuffer(scoped_refptr<UnsizedRefCountedMemory> buffer, size_t size);
...
template <typename Reply> std::unique_ptr<Reply> ReadReply(ReadBuffer* buffer);

} // namespace detail

template <class Reply> class Future;

template <typename T> T Read(ReadBuffer* buf);

template <typename T> WriteBuffer Write(const T& t);

template <typename T> void ReadEvent(T* event, ReadBuffer* buf);
...
private:
    ...
    explicit Response(std::unique_ptr<Error> error)
        : error(std::move(error))
    {
    }
};
