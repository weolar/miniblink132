...
template <typename T, typename U> auto CompareSequenceIds(T t, U u)
{
    ...
    using SmallerType = typename std::conditional<sizeof(T) <= sizeof(U), T, U>::type;
...
class COMPONENT_EXPORT(X11) Connection final : public XProto, public ExtensionManager {
public:
    ...
    template <typename T> T GenerateId()
    {
        DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
        return static_cast<T>(GenerateIdImpl());
    }

    template <typename Reply> Future<Reply> SendRequest(WriteBuffer* buf, const char* request_name, bool reply_has_fds)
    {
        bool generates_reply = !std::is_void<Reply>::value;
        return Future<Reply>(SendRequestImpl(buf, request_name, generates_reply, reply_has_fds));
    }
    ...
    WindowEventManager& window_event_manager()
    {
        return window_event_manager_;
    }

    // Indicates if the connection was able to successfully sync with the
    // window manager.
    bool synced_with_wm() const
    {
    ...
    template <typename T> Future<void> SendEvent(const T& event, Window target, EventMask mask)
    {
    ...
    template <typename T> bool GetArrayProperty(Window window, Atom name, std::vector<T>* value, Atom* out_type = nullptr, size_t amount = 0)
    {
        ...
        auto response = GetProperty(GetPropertyRequest { .window = static_cast<Window>(window),
                                        .property = name,
                                        .long_length = static_cast<uint32_t>(amount ? length : std::numeric_limits<lentype>::max()) })
                            .Sync();
    ...
    template <typename T> bool GetPropertyAs(Window window, const Atom name, T* value)
    {
        std::vector<T> values;
    ...
    template <typename T> Future<void> SetArrayProperty(Window window, Atom name, Atom type, const std::vector<T>& values)
    {
        static_assert(sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4, "");
        return SetArrayPropertyImpl(window, name, type, 8u * sizeof(T), base::as_byte_span(values));
    }

    template <typename T> Future<void> SetProperty(Window window, Atom name, Atom type, const T& value)
    {
    ...
    void SetStringProperty(Window window, Atom property, Atom type, const std::string& value);
...
private:
    ...
    template <typename Reply> friend class Future;
    ...
    Future<void> SetArrayPropertyImpl(Window window, Atom name, Atom type, uint8_t format, base::span<const uint8_t> values);
    ...
    std::unique_ptr<FutureImpl> SendRequestImpl(WriteBuffer* buf, const char* request_name_for_tracing, bool generates_reply, bool reply_has_fds);
    ...
    std::unique_ptr<xcb_connection_t, void (*)(xcb_connection_t*)> connection_ = { nullptr, nullptr };
    ...
    base::ObserverList<EventObserver>::UncheckedAndDanglingUntriaged event_observers_;
...
template <> struct ScopedObservationTraits<x11::Connection, x11::EventObserver> {
    static void AddObserver(x11::Connection* connection, x11::EventObserver* observer)
    {
        connection->AddEventObserver(observer);
    }
    static void RemoveObserver(x11::Connection* connection, x11::EventObserver* observer)
    {
