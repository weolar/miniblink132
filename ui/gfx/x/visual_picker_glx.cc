...
{
    auto bits = [](auto x) { return std::bitset<8 * sizeof(decltype(x))>(x).count(); };
    auto bits_rgb = bits(visual.visual_type->red_mask) + bits(visual.visual_type->green_mask) + bits(visual.visual_type->blue_mask);
...
VisualId PickBestGlVisual(
    Connection* connection, const Glx::GetVisualConfigsReply& configs, base::RepeatingCallback<bool(const Connection::VisualInfo&)> pred, bool want_alpha)
{
    int highest_score = -1;
    ...
    for (size_t cfg = 0; cfg < configs.num_visuals; cfg++) {
        ...
        const size_t extra_props = (configs.num_properties - std::size(static_props)) / 2;
...
VisualId PickBestSystemVisual(Connection* connection, const Glx::GetVisualConfigsReply& configs)
{
    Connection::VisualInfo default_visual_info = *connection->GetVisualInfoFromId(connection->default_root_visual().visual_id);

    auto is_compatible_with_root_visual = [](const Connection::VisualInfo& default_visual_info, const Connection::VisualInfo& visual_info) {
        const auto& dvt = *default_visual_info.visual_type;
        const auto& vt = *visual_info.visual_type;
        return vt.c_class == dvt.c_class && visual_info.format->depth == default_visual_info.format->depth && vt.red_mask == dvt.red_mask
            && vt.green_mask == dvt.green_mask && vt.blue_mask == dvt.blue_mask && vt.colormap_entries == dvt.colormap_entries
            && vt.bits_per_rgb_value == dvt.bits_per_rgb_value;
    };

    return PickBestGlVisual(connection, configs, base::BindRepeating(is_compatible_with_root_visual, default_visual_info), IsArgbVisual(default_visual_info));
}

VisualId PickBestRgbaVisual(Connection* connection, const Glx::GetVisualConfigsReply& configs)
{
    ...
    auto pred = [](int best_class_score, const Connection::VisualInfo& visual_info) {
        if (!IsArgbVisual(visual_info)) {
            return false;
        }
        return VisualScore(visual_info.visual_type->c_class) == best_class_score;
    };
    return PickBestGlVisual(connection, configs, base::BindRepeating(pred, best_class_score), true);
}

} // anonymous namespace

void PickBestVisuals(Connection* connection, VisualId& system_visual, VisualId& rgba_visual)
{
