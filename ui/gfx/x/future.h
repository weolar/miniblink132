...
public:
    FutureImpl(Connection* connection, SequenceType sequence, bool generates_reply, const char* request_name_for_tracing);
    ...
    SequenceType sequence() const
    {
        return sequence_;
    }

    bool generates_reply() const
    {
        return generates_reply_;
    }
...
template <typename Reply> class Future : public FutureBase {
public:
    ...
    void OnResponse(Callback callback)
    {
        ...
        // function.
        auto wrapper = [](Callback callback, RawReply raw_reply, std::unique_ptr<Error> error) {
            std::unique_ptr<Reply> reply;
            if (raw_reply) {
...
template <> inline Response<void> Future<void>::Sync()
{
    if (!impl()) {
...
template <> inline void Future<void>::OnResponse(Callback callback)
{
    if (!impl()) {
    ...
    auto wrapper = [](Callback callback, RawReply reply, std::unique_ptr<Error> error) {
        CHECK(!reply);
