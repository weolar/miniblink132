...
template <typename T, typename Enable = void> struct EnumBase {
    using type = T;
};

template <typename T> struct EnumBase<T, typename std::enable_if_t<std::is_enum<T>::value>> {
    using type = typename std::underlying_type<T>::type;
};

template <typename T> using EnumBaseType = typename EnumBase<T>::type;

template <typename T> void ReadError(T* error, ReadBuffer* buf);

// Calls free() on the underlying data when the count drops to 0.
class COMPONENT_EXPORT(X11) MallocedRefCountedMemory : public UnsizedRefCountedMemory {
public:
...
class COMPONENT_EXPORT(X11) OffsetRefCountedMemory : public UnsizedRefCountedMemory {
public:
    OffsetRefCountedMemory(scoped_refptr<UnsizedRefCountedMemory> memory, size_t offset, size_t size);
...
class COMPONENT_EXPORT(X11) UnretainedRefCountedMemory : public UnsizedRefCountedMemory {
public:
    explicit UnretainedRefCountedMemory(void* data);

    UnretainedRefCountedMemory(const UnretainedRefCountedMemory&) = delete;
    UnretainedRefCountedMemory& operator=(const UnretainedRefCountedMemory&) = delete;
...
template <typename T> void Read(T* t, ReadBuffer* buf)
{
...
template <typename T> size_t PopCount(T t)
{
...
template <typename F, typename T> auto SumOf(F&& f, T& t)
{
...
template <typename T, typename S> bool CaseEq(T t, S s)
{
...
template <typename T> auto SwitchVar(T enum_val, bool condition, bool is_bitcase, T* switch_var)
{
...
template <typename T> std::unique_ptr<T> MakeExtension(Connection* connection, Future<QueryExtensionReply> future)
{
    auto reply = future.Sync();
    return std::make_unique<T>(connection, reply ? *reply.reply : QueryExtensionReply {});
}
