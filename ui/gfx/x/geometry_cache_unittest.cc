...
TEST(GeometryCacheTest, ConstructAndDestruct)
{
    ...
    ScopedWindow window(connection, connection->default_root(), gfx::Rect(12, 34, 56, 78));
    GeometryCache geometry_cache(connection, window.id(), base::BindRepeating([](const std::optional<gfx::Rect>& old_bounds, const gfx::Rect& new_bounds) {}));
}
...
TEST(GeometryCacheTest, BoundsChangedCallback)
{
    ...
    auto bounds_changed_callback
        = [](std::optional<gfx::Rect>* last_bounds, const std::optional<gfx::Rect>& old_bounds, const gfx::Rect& new_bounds) { *last_bounds = new_bounds; };

    GeometryCache geometry_cache(connection, window.id(), base::BindRepeating(bounds_changed_callback, &last_bounds));
...
TEST(GeometryCacheTest, NestedWindows)
{
    ...
    ScopedWindow parent_window(connection, connection->default_root(), parent_bounds);
    ...
    GeometryCache child_geometry_cache(
        connection, child_window.id(), base::BindRepeating([](const std::optional<gfx::Rect>& old_bounds, const gfx::Rect& new_bounds) {}));

    // Initial bounds should be the sum of child window position and parent window
    // position.
    EXPECT_EQ(child_geometry_cache.GetBoundsPx(), child_bounds + gfx::Vector2d(parent_bounds.x(), parent_bounds.y()));
    ...
    EXPECT_EQ(child_geometry_cache.GetBoundsPx(), new_child_bounds + gfx::Vector2d(parent_bounds.x(), parent_bounds.y()));
}
