...
std::unique_ptr<std::vector<Rectangle>> CreateRegionFromSkRegion(const SkRegion& region);

// Creates a new XRegion given |path|. The caller is responsible for destroying
// the returned region.
COMPONENT_EXPORT(X11)
std::unique_ptr<std::vector<Rectangle>> CreateRegionFromSkPath(const SkPath& path);
