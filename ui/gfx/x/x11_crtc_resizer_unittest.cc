...
std::string PrettyPrint(const gfx::Rect& rect)
{
    ...
    return base::StringPrintf("%" PRId32 "x%" PRId32 "%+" PRId32 "%+" PRId32, rect.width(), rect.height(), rect.x(), rect.y());
}

void ExpectEqual(const gfx::Rect& rect1, const gfx::Rect& rect2)
{
    if (rect1 != (rect2)) {
        ADD_FAILURE() << "Expected equality of [" << PrettyPrint(rect1) << "] and [" << PrettyPrint(rect2) << "]";
    }
}
