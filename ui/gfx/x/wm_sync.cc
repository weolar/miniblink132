...
WmSync::WmSync(Connection* connection, base::OnceClosure on_synced, bool sync_with_wm)
    : connection_(connection)
    , on_synced_(std::move(on_synced))
{
    if (!sync_with_wm) {
        connection_->GetInputFocus().OnResponse(base::BindOnce(&WmSync::OnGetInputFocusResponse, weak_ptr_factory_.GetWeakPtr()));
        connection_->Flush();
        return;
    }

    constexpr EventMask event_mask = EventMask::StructureNotify | EventMask::PropertyChange;
