...
PropertyCache::PropertyCache(Connection* connection, Window window, const std::vector<Atom>& properties, OnChangeCallback on_change)
    : connection_(connection)
    , window_(window)
    , event_selector_(connection->ScopedSelectEvent(window_, EventMask::PropertyChange))
...
void PropertyCache::OnEvent(const Event& xev)
{
    ...
    if (prop->state == Property::NewValue) {
        ...
        // unnecessary request.
        OnGetPropertyResponse(it, GetPropertyResponse { std::make_unique<GetPropertyReply>(), nullptr });
    }
}

void PropertyCache::FetchProperty(PropertiesIterator it)
{
    it->second.future = static_cast<XProto*>(connection_)
                            ->GetProperty({
                                .window = window_,
                                .property = it->first,
                                .long_length = std::numeric_limits<uint32_t>::max(),
                            });
    it->second.future.OnResponse(base::BindOnce(&PropertyCache::OnGetPropertyResponse, weak_factory_.GetWeakPtr(), it));
}

void PropertyCache::OnGetPropertyResponse(PropertiesIterator it, GetPropertyResponse response)
{
...
PropertyCache::PropertyValue& PropertyCache::PropertyValue::operator=(PropertyValue&&) = default;
