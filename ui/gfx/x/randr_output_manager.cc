...
{
    return gfx::Vector2d(CalculateDpi(monitor.width, monitor.width_in_millimeters), CalculateDpi(monitor.height, monitor.height_in_millimeters));
}

x11::RandRMonitorConfig ToRandRMonitorConfig(const x11::RandR::MonitorInfo& monitor)
{
    return x11::RandRMonitorConfig(static_cast<intptr_t>(monitor.name), gfx::Rect(monitor.x, monitor.y, monitor.width, monitor.height), GetMonitorDpi(monitor));
}

// Gets current layout with context information from a list of monitors.
std::vector<x11::RandRMonitorConfigWithContext> GetLayoutWithContext(std::vector<x11::RandR::MonitorInfo>& monitors)
{
...
gfx::Size CalculateSizeInMmForGnome(const gfx::Size& dimensions, const gfx::Vector2d& dpi)
{
    ...
    constexpr std::pair<int, int> kBadSizes[] = { { 16, 9 }, { 16, 10 }, { 160, 90 }, { 160, 100 }, { 1600, 900 }, { 1600, 1000 } };
...
void SetOutputPhysicalSizeInMM(x11::Connection* connection, x11::RandR::Output output, int width, int height)
{
    ...
    x11::RandR::ChangeOutputPropertyRequest request = {
        ...
        .num_units = 1,
        .data = base::MakeRefCounted<base::RefCountedStaticMemory>(base::U32ToNativeEndian(width_32)),
    };
    connection->randr().ChangeOutputProperty(request).Sync();

    request.property = height_mm_atom;
    request.data = base::MakeRefCounted<base::RefCountedStaticMemory>(base::U32ToNativeEndian(height_32));
    connection->randr().ChangeOutputProperty(request).Sync();
}
...
DisplayLayoutDiff::DisplayLayoutDiff(x11::RandRMonitorLayout new_displays, std::vector<RandRMonitorConfigWithContext> updated_displays,
...
DisplayLayoutDiff CalculateDisplayLayoutDiff(const std::vector<RandRMonitorConfigWithContext>& current_displays, const x11::RandRMonitorLayout& new_layout)
{
    ...
    for (const x11::RandRMonitorConfig& new_config : new_layout.configs) {
        ...
        auto current_display_it = base::ranges::find(current_displays, new_config.id(), [](const auto& display) { return display.config.id(); });
        ...
        if (new_config.rect() != current_display_it->config.rect() || new_config.dpi() != current_display_it->config.dpi()) {
            VLOG(1) << "Video layout for screen id " << *new_config.id() << " has been changed.";
            diff.updated_displays.emplace_back(new_config, current_display_it->context);
        } else {
            VLOG(1) << "Video layout for screen id " << *new_config.id() << " has not been changed.";
        }
    }
...
{
    CHECK(resources_);
    base::span<const char> names = base::as_chars(base::span<uint8_t>(resources_->names));
    uint16_t idx = 0;
    for (const auto& mode_info : resources_->modes) {
        std::string mode_name(names.subspan(idx, mode_info.name_len).data(), mode_info.name_len);
...
RandRMonitorConfig::RandRMonitorConfig(std::optional<ScreenId> id, gfx::Rect rect, gfx::Vector2d dpi)
    : id_(id)
    , rect_(rect)
    , dpi_(dpi)
{
}
bool RandRMonitorConfig::operator==(const RandRMonitorConfig& rhs) const = default;
RandRMonitorConfig::RandRMonitorConfig(const RandRMonitorConfig& other) = default;
RandRMonitorConfig& RandRMonitorConfig::operator=(const RandRMonitorConfig& other) = default;
...
RandRMonitorLayout& RandRMonitorLayout::operator=(const RandRMonitorLayout&) = default;
RandRMonitorLayout::~RandRMonitorLayout() = default;
bool RandRMonitorLayout::operator==(const RandRMonitorLayout& rhs) const = default;

RandROutputManager::RandROutputManager(std::string output_name_prefix, uint32_t default_mode_dot_clock)
...
bool RandROutputManager::TryGetCurrentMonitors(std::vector<x11::RandR::MonitorInfo>& list)
{
    if (!has_randr_) {
        return false;
    }
    ...
    std::copy(reply->monitors.begin(), reply->monitors.end(), std::back_inserter(list));
    return true;
}
...
void RandROutputManager::SetLayout(const RandRMonitorLayout& layout)
{
    ...
    std::vector<RandRMonitorConfigWithContext> current_displays = GetLayoutWithContext(monitors);
    ...
    const std::vector<RandRMonitorConfig>& new_layouts = diff.new_displays.configs;
    // Add displays
    if (!new_layouts.empty()) {
        ...
        for (; i < outputs.size() && i < new_layouts.size(); i++) {
            ...
            if (output_info.crtcs.empty()) {
                LOG(ERROR) << "No available CRTC found associated with " << reinterpret_cast<char*>(output_info.name.data());
                continue;
            }
            ...
            auto mode = UpdateMode(output, new_layout.rect().width(), new_layout.rect().height());
            if (mode == kInvalidMode) {
                LOG(ERROR) << "Failed to create new mode.";
                continue;
            }
            resizer.AddActiveCrtc(crtc, mode, { output }, new_layout.rect());
            VLOG(0) << "Added display with crtc: " << base::to_underlying(crtc) << ", output: " << base::to_underlying(output);
        }
        if (i < diff.new_displays.configs.size()) {
            LOG(WARNING) << "Failed to create " << (diff.new_displays.configs.size() - i) << " display(s) due to insufficient resources.";
        }
    }
    ...
    for (const auto& updated_display : diff.updated_displays) {
        ...
        auto mode = UpdateMode(output, updated_layout.rect().width(), updated_layout.rect().height());
        if (mode == kInvalidMode) {
            LOG(ERROR) << "Failed to create new mode.";
            continue;
        }
        resizer.UpdateActiveCrtc(crtc, mode, updated_layout.rect());
        VLOG(0) << "Updated display with screen ID: " << updated_display.config.id().value_or(-1);
    }
    ...
    for (const auto& removed_display : diff.removed_displays) {
        ...
        VLOG(0) << "Removed display with screen ID: " << removed_display.config.id().value_or(-1);
    }
...
x11::RandR::Mode RandROutputManager::UpdateMode(x11::RandR::Output output, int width, int height)
{
    ...
    if (auto reply = randr_->CreateMode({ root_, mode, mode_name.c_str() }).Sync()) {
...
void RandROutputManager::DeleteMode(x11::RandR::Output output, const std::string& name)
{
...
void RandROutputManager::SetResolutionForOutput(x11::RandR::Output output, const gfx::Size& dimensions, const gfx::Vector2d& dpi)
{
    if (!resources_.Refresh(randr_, root_)) {
...
RandROutputManager::OutputInfoList RandROutputManager::GetDisabledOutputs()
{
    ...
    for (x11::RandR::Output output : resources_.get()->outputs) {
        auto reply = randr_->GetOutputInfo({ .output = output, .config_timestamp = resources_.get()->config_timestamp }).Sync();
        if (!reply) {
            continue;
        }
...
std::string RandROutputManager::GetModeNameForOutput(x11::RandR::Output output)
{
    ...
    return base::StringPrintf("%s%i", output_name_prefix_.c_str(), base::to_underlying(output));
}
