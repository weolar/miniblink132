...
public:
    ...
    RandRMonitorConfig(std::optional<ScreenId> id, gfx::Rect rect, gfx::Vector2d dpi);
    ...
    std::optional<ScreenId> id() const
    {
        return id_;
    }
...
struct COMPONENT_EXPORT(X11) DisplayLayoutDiff {
    DisplayLayoutDiff();
    DisplayLayoutDiff(x11::RandRMonitorLayout new_displays, std::vector<RandRMonitorConfigWithContext> updated_displays,
...
public:
    ...
    explicit RandROutputManager(std::string output_name_prefix, uint32_t default_mode_dot_clock = 60 * 1e6 /* Realistic default */);
    ~RandROutputManager();
    ...
    void SetResolutionForOutput(x11::RandR::Output output, const gfx::Size& dimensions, const gfx::Vector2d& dpi);

private:
    using OutputInfoList = std::vector<std::pair<x11::RandR::Output, x11::RandR::GetOutputInfoReply>>;
