...
X11CrtcResizer::CrtcInfo::CrtcInfo(x11::RandR::Crtc crtc, int16_t x, int16_t y, uint16_t width, uint16_t height, x11::RandR::Mode mode,
    x11::RandR::Rotation rotation, const std::vector<x11::RandR::Output>& outputs)
    : crtc(crtc)
    , old_x(x)
    , x(x)
    , old_y(y)
...
X11CrtcResizer::CrtcInfo& X11CrtcResizer::CrtcInfo::operator=(const X11CrtcResizer::CrtcInfo&) = default;
X11CrtcResizer::CrtcInfo& X11CrtcResizer::CrtcInfo::operator=(X11CrtcResizer::CrtcInfo&&) = default;
X11CrtcResizer::CrtcInfo::~CrtcInfo() = default;
...
X11CrtcResizer::X11CrtcResizer(x11::RandR::GetScreenResourcesCurrentReply* resources, x11::Connection* connection)
...
x11::RandR::Crtc X11CrtcResizer::GetCrtcForOutput(x11::RandR::Output output) const
{
    ...
    auto iter = base::ranges::find_if(active_crtcs_, [output](const CrtcInfo& crtc_info) { return base::Contains(crtc_info.outputs, output); });
...
void X11CrtcResizer::UpdateActiveCrtcs(x11::RandR::Crtc crtc, x11::RandR::Mode mode, const gfx::Size& new_size)
{
...
void X11CrtcResizer::UpdateActiveCrtc(x11::RandR::Crtc crtc, x11::RandR::Mode mode, const gfx::Rect& new_rect)
{
    updated_crtcs_.insert(crtc);
...
void X11CrtcResizer::AddActiveCrtc(x11::RandR::Crtc crtc, x11::RandR::Mode mode, const std::vector<x11::RandR::Output>& outputs, const gfx::Rect& new_rect)
{
    // |crtc| is not active so it must not be in |active_crtcs_|.
    DCHECK(base::ranges::find(active_crtcs_, crtc, &CrtcInfo::crtc) == active_crtcs_.end());

    active_crtcs_.emplace_back(crtc, new_rect.x(), new_rect.y(), new_rect.width(), new_rect.height(), mode, x11::RandR::Rotation::Rotate_0, outputs);
    updated_crtcs_.insert(crtc);
}
...
void X11CrtcResizer::RelayoutCrtcs(x11::RandR::Crtc crtc_to_resize, const gfx::Size& new_size)
{
    if (LayoutIsVertical()) {
...
void X11CrtcResizer::NormalizeCrtcs()
{
    ...
    gfx::Vector2d adjustment = gfx::Vector2d(-bounding_box.origin().x(), -bounding_box.origin().y());
...
void X11CrtcResizer::MoveApplicationWindows()
{
    ...
    auto query_response = connection_->QueryTree({ connection_->default_root() }).Sync();
    if (!query_response) {
        return;
    }
    for (const auto& window : query_response->children) {
        auto attributes_response = connection_->GetWindowAttributes({ window }).Sync();
        ...
        for (const auto& crtc_info : active_crtcs_) {
            ...
            auto old_rect = gfx::Rect(crtc_info.old_x, crtc_info.old_y, crtc_info.width, crtc_info.height);
            gfx::Vector2d window_top_left(geometry_response->x, geometry_response->y);
            if (!old_rect.Contains(gfx::Point(window_top_left.x(), window_top_left.y()))) {
                continue;
            }

            gfx::Vector2d adjustment(crtc_info.x - crtc_info.old_x, crtc_info.y - crtc_info.old_y);
            window_top_left = window_top_left + adjustment;

            MoveWindow(window, *attributes_response.reply, gfx::Point(window_top_left.x(), window_top_left.y()));
            break;
        }
    }
}
...
void X11CrtcResizer::UpdateRootWindow(x11::Window root)
{
    ...
    randr_->SetScreenSize({ root, static_cast<uint16_t>(dimensions.width()), static_cast<uint16_t>(dimensions.height()), width_mm, height_mm });
...
void X11CrtcResizer::SetCrtcsForTest(std::vector<x11::RandR::GetCrtcInfoReply> crtcs)
{
    int id = 1;
...
std::vector<gfx::Rect> X11CrtcResizer::GetCrtcsForTest() const
{
    ...
    for (const auto& crtc_info : active_crtcs_) {
        result.emplace_back(crtc_info.x, crtc_info.y, crtc_info.width, crtc_info.height);
    }
    return result;
}

void X11CrtcResizer::AddCrtcFromReply(x11::RandR::Crtc crtc, const x11::RandR::GetCrtcInfoReply& reply)
{
    active_crtcs_.emplace_back(crtc, reply.x, reply.y, reply.width, reply.height, reply.mode, reply.rotation, reply.outputs);
}
...
void X11CrtcResizer::PackVertically(const gfx::Size& new_size, x11::RandR::Crtc resized_crtc)
{
...
void X11CrtcResizer::PackHorizontally(const gfx::Size& new_size, x11::RandR::Crtc resized_crtc)
{
...
void X11CrtcResizer::MoveWindow(x11::Window window, const x11::GetWindowAttributesReply& attributes, gfx::Point top_left)
{
...
x11::Window X11CrtcResizer::FindAppWindow(x11::Window window, const x11::GetWindowAttributesReply& attributes)
{
    ...
    for (const auto& child_window : base::Reversed(query_response->children)) {
        auto attributes_response = connection_->GetWindowAttributes({ child_window }).Sync();
        if (!attributes_response) {
            return x11::Window::None;
        }
