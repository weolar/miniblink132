...
{
    connection()->ConfigureWindow({ .window = root(), .x = 10, .y = 10, .width = 20, .height = 20, .border_width = 5 });
    cache()->SyncForTest();
    const WindowCache::WindowInfo& info = cache()->windows().at(root());
...
TEST_F(WindowCacheTest, Restack)
{
    auto restack = [&](Window window, Window sibling, bool above) {
        connection()->ConfigureWindow({ .window = window, .sibling = sibling, .stack_mode = above ? StackMode::Above : StackMode::Below });
        cache()->SyncForTest();
    };
...
TEST_F(WindowCacheTest, GravityNotify)
{
    ...
    connection()->ConfigureWindow({ .window = parent, .width = 256, .height = 512 });
    cache()->SyncForTest();
...
TEST_F(WindowCacheTest, ShapeExtension)
{
    ...
    const WindowCache::WindowInfo& info = cache()->windows().at(root());
    EXPECT_EQ(info.bounding_rects_px, (std::vector<Rectangle> { { 0, 0, 512, 1024 } }));
    ...
    shape.Rectangles({ .operation = Shape::So::Set, .destination_kind = Shape::Sk::Bounding, .destination_window = root(), .rectangles = bounding_rects });
    shape.Rectangles({ .operation = Shape::So::Set, .destination_kind = Shape::Sk::Input, .destination_window = root(), .rectangles = input_rects });
    cache()->SyncForTest();
...
TEST_F(WindowCacheTest, GtkFrameExtents)
{
    ...
    connection()->SetArrayProperty(root(), gtk_frame_extents, Atom::CARDINAL, std::vector<uint32_t> { 1, 2, 3, 4 });
    cache()->SyncForTest();
    ...
    connection()->SetArrayProperty(root(), gtk_frame_extents, Atom::CARDINAL, std::vector<uint32_t> { 1, 2 });
    cache()->SyncForTest();
    EXPECT_EQ(info.gtk_frame_extents_px, gfx::Insets());

    connection()->SetArrayProperty(root(), gtk_frame_extents, Atom::CARDINAL, std::vector<uint8_t> { 1, 2, 3, 4 });
    cache()->SyncForTest();
    EXPECT_EQ(info.gtk_frame_extents_px, gfx::Insets());
}
...
TEST_F(WindowCacheTest, GetWindowAtPoint)
{
    ...
    EXPECT_EQ(cache()->GetWindowAtPoint({ 100, 100 }, does_not_exist), Window::None);
    ...
    if (shape.present()) {
        for (auto kind : { Shape::Sk::Bounding, Shape::Sk::Input }) {
            shape.Rectangles({ .destination_kind = kind, .destination_window = a, .rectangles = std::vector<Rectangle> { { 0, 0, 300, 300 } } });
        }
    }
    cache()->SyncForTest();
    ...
    const Atom gtk_frame_extents = GetAtom("_GTK_FRAME_EXTENTS");
    connection()->SetArrayProperty(a, gtk_frame_extents, Atom::CARDINAL, std::vector<uint32_t> { 40, 40, 40, 40 });
    cache()->SyncForTest();
