...
bool IsPublicOrPrivateKeypadKey(KeySym keysym)
{
    ...
    return (key >= XK_KP_Space && key <= XK_KP_Equal) || (key >= 0x11000000 && key <= 0x1100FFFF);
}
...
public:
    explicit CoreKeyboardState(Connection* connection)
    ...
    KeyCode KeysymToKeycode(uint32_t keysym) const override
    {
        ...
        CHECK_EQ(count * keyboard_mapping_.keysyms_per_keycode, static_cast<int>(keyboard_mapping_.keysyms.size()));
    ...
    {
        auto sym = static_cast<uint32_t>(KeycodeToKeysymCoreImpl(keycode, modifiers, connection_, keyboard_mapping_, lock_meaning_, mode_switch_, num_lock_));
...
private:
    ...
    {
        UpdateMappingImpl(connection_, &keyboard_mapping_, &lock_meaning_, &mode_switch_, &num_lock_);
    }
...
public:
    explicit XkbKeyboardState(Connection* connection)
        : connection_(connection)
    {
        UpdateMapping();
    }
...
private:
    void UpdateMapping() override
    {
        auto future = connection_->xkb().GetMap({ static_cast<Xkb::DeviceSpec>(Xkb::Id::UseCoreKbd), Xkb::MapPart::KeyTypes | Xkb::MapPart::KeySyms });
