...
ThrowAwaySizeRefCountedMemory::ThrowAwaySizeRefCountedMemory(std::vector<uint8_t> data)
    : data_(std::move(data))
{
}
...
SizedRefCountedMemory::SizedRefCountedMemory(scoped_refptr<UnsizedRefCountedMemory> mem, size_t size)
    : mem_(std::move(mem))
    , size_(size)
{
}
...
ReadBuffer::ReadBuffer(scoped_refptr<UnsizedRefCountedMemory> data, bool setup_message)
    : data(data)
{
...
scoped_refptr<UnsizedRefCountedMemory> ReadBuffer::ReadAndAdvance(size_t length)
{
...
void WriteBuffer::AppendBuffer(scoped_refptr<UnsizedRefCountedMemory> buffer, size_t size)
{
    AppendCurrentBuffer();
...
void WriteBuffer::AppendSizedBuffer(scoped_refptr<base::RefCountedMemory> buffer)
{
    AppendCurrentBuffer();
...
void WriteBuffer::AppendCurrentBuffer()
{
    ...
    owned_buffers_.push_back(ThrowAwaySizeRefCountedMemory::From(std::move(current_buffer_)));
