...
GeometryCache::GeometryCache(Connection* connection, Window window, BoundsChangedCallback bounds_changed_callback)
    : connection_(connection)
    , window_(window)
    , bounds_changed_callback_(bounds_changed_callback)
{
    scoped_observation_.Observe(connection_);
    window_events_ = connection_->ScopedSelectEvent(window_, EventMask::StructureNotify);

    parent_future_ = connection_->QueryTree(window_);
    parent_future_.OnResponse(base::BindOnce(&GeometryCache::OnQueryTreeResponse, weak_ptr_factory_.GetWeakPtr()));
    geometry_future_ = connection_->GetGeometry(window_);
    geometry_future_.OnResponse(base::BindOnce(&GeometryCache::OnGetGeometryResponse, weak_ptr_factory_.GetWeakPtr()));
}
...
{
    OnParentChanged(response ? response->parent : Window::None, geometry_.origin());
}

void GeometryCache::OnGetGeometryResponse(GetGeometryResponse response)
{
    OnGeometryChanged(response ? gfx::Rect(response->x, response->y, response->width, response->height) : gfx::Rect());
}
...
void GeometryCache::OnParentChanged(Window parent, const gfx::Point& position)
{
    ...
    if (parent == Window::None) {
        ...
        parent_ = std::make_unique<GeometryCache>(
            connection_, parent, base::BindRepeating(&GeometryCache::OnParentGeometryChanged, weak_ptr_factory_.GetWeakPtr()));
    }
...
void GeometryCache::OnParentGeometryChanged(const std::optional<gfx::Rect>& old_parent_bounds, const gfx::Rect& new_parent_bounds)
{
    NotifyGeometryChanged();
}
...
void GeometryCache::OnEvent(const Event& xevent)
{
    ...
    if (auto* configure = xevent.As<ConfigureNotifyEvent>()) {
        if (configure->window == window_) {
            OnGeometryChanged(gfx::Rect(configure->x, configure->y, configure->width, configure->height));
        }
