...
public:
    using OnChangeCallback = base::RepeatingCallback<void(Atom, const GetPropertyResponse&)>;

    PropertyCache(Connection* connection, Window window, const std::vector<Atom>& properties, OnChangeCallback on_change = {});
    ...
    template <typename T> static const T* GetAs(const GetPropertyResponse& response, size_t* size = nullptr)
    {
        if (size) {
            *size = 0;
        }
        if (!response || response->format != CHAR_BIT * sizeof(T) || !response->value_len) {
            return nullptr;
        }
        if (size) {
    ...
    template <typename T> const T* GetAs(Atom atom, size_t* size = nullptr)
    {
        return GetAs<T>(Get(atom), size);
    }

    template <typename T> static base::span<const T> GetAsSpan(const GetPropertyResponse& response)
    {
        size_t len;
    ...
    template <typename T> base::span<const T> GetAsSpan(Atom atom)
    {
...
private:
    ...
    void OnGetPropertyResponse(PropertiesIterator it, GetPropertyResponse response);
