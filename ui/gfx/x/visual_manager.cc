...
{
    auto atom_name = "_NET_WM_CM_S" + base::NumberToString(connection_->DefaultScreenId());
    ...
    if (xfixes.present()) {
        ...
        xfixes.SelectSelectionInput(connection_->default_root(), compositor_atom_, mask);
    ...
    for (const auto& depth : connection_->default_screen().allowed_depths) {
        for (const auto& visual : depth.visuals) {
            visuals_[visual.visual_id] = std::make_unique<XVisualData>(connection_, depth.depth, &visual);
        }
    }
    ...
    if (transparent_visual_id_ == VisualId {}) {
        for (const auto& pair : visuals_) {
            ...
            if (data.depth == 32 && data.info->red_mask == 0xff0000 && data.info->green_mask == 0x00ff00 && data.info->blue_mask == 0x0000ff) {
...
void VisualManager::ChooseVisualForWindow(bool want_argb_visual, VisualId* visual_id, uint8_t* depth, ColorMap* colormap, bool* visual_has_alpha)
{
...
bool VisualManager::GetVisualInfo(VisualId visual_id, uint8_t* depth, ColorMap* colormap, bool* visual_has_alpha)
{
    ...
    if (colormap) {
        bool is_default_visual = visual_id == connection_->default_root_visual().visual_id;
        *colormap = is_default_visual ? ColorMap {} : data.GetColormap(connection_);
    }
    if (visual_has_alpha) {
        auto popcount = [](auto x) { return std::bitset<8 * sizeof(decltype(x))>(x).count(); };
        *visual_has_alpha = popcount(info.red_mask) + popcount(info.green_mask) + popcount(info.blue_mask) < static_cast<std::size_t>(data.depth);
    }
    return true;
}
...
{
    return compositor_owner_ != x11::Window::None && transparent_visual_id_ != VisualId {};
}

VisualManager::XVisualData::XVisualData(Connection* connection, uint8_t depth, const VisualType* info)
    : depth(depth)
    , info(info)
{
}
...
ColorMap VisualManager::XVisualData::GetColormap(Connection* connection)
{
    if (colormap_ == ColorMap {}) {
        ...
        connection->CreateColormap({ ColormapAlloc::None, colormap_, connection->default_root(), info->visual_id });
