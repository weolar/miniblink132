...
public:
    template <typename T> Event(bool send_event, T&& xproto_event)
    {
    ...
    Event(scoped_refptr<UnsizedRefCountedMemory> event_bytes, Connection* connection);
    ...
    template <typename T> T* As()
    {
        ...
        if (!event_) {
            T* event = new T;
            Parse(
                event, [](void* e, ReadBuffer* r) { ReadEvent<T>(static_cast<T*>(e), r); }, [](void* e) { delete static_cast<T*>(e); });
