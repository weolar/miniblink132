    ...
    : connection_(connection)
{
    ...
    for (const char* name : kAtomsToCache) {
        requests.push_back(connection_->InternAtom(InternAtomRequest { .name = name }));
    }
...
Atom AtomCache::GetAtom(const char* name)
{
    ...
    Atom atom = Atom::None;
    if (auto response = connection_->InternAtom(InternAtomRequest { .name = name }).Sync()) {
        atom = response->atom;
        CHECK_GT(atom, x11::Atom::kLastPredefinedAtom) << " Use x11::Atom::" << name << " instead of x11::GetAtom(\"" << name << "\")";
        ...
        cached_atoms_.emplace(owned_strings_.emplace_back(std::move(owned_name))->c_str(), atom);
    }
    return atom;
}
