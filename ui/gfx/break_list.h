...
template <typename T> class BreakList {
public:
...
template <class T> bool BreakList<T>::ClearAndSetInitialValue(T value)
{
...
template <class T> bool BreakList<T>::ApplyValue(T value, const Range& range)
{
    ...
    const_iterator end = range.end() == max_ ? breaks_.cend() - 1 : GetBreak(range.end());
    T trailing_value = end->second;
    const_iterator i = start == breaks_.cend() ? start : breaks_.erase(start, end + 1);
...
template <class T> void BreakList<T>::SetMax(size_t max)
{
    if (max < max_) {
...
template <class T> typename BreakList<T>::const_iterator BreakList<T>::GetBreak(size_t position) const
{
    DCHECK(!breaks_.empty());
    DCHECK_LT(position, max_);
    // Find the iterator with a 'strictly greater' position and return the
    // previous one.
    return std::upper_bound(breaks_.cbegin(), breaks_.cend(), position, [](size_t offset, const Break& value) { return offset < value.first; }) - 1;
}

template <class T> Range BreakList<T>::GetRange(const typename BreakList<T>::const_iterator& i) const
{
...
template <class T> bool BreakList<T>::EqualsValueForTesting(T value) const
{
    return breaks_.size() == 1 && breaks_[0] == Break(0, value);
}

template <class T> bool BreakList<T>::EqualsForTesting(const std::vector<Break>& breaks) const
{
...
#ifndef NDEBUG
template <class T> void BreakList<T>::CheckBreaks()
{
