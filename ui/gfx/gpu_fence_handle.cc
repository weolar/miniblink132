...
{
    static bool is_enabled = base::FeatureList::IsEnabled(features::kUseSmartRefForGPUFenceHandle);
    return is_enabled;
} // namespace

gfx::GpuFenceHandle::ScopedPlatformFence PlatformDuplicate(const gfx::GpuFenceHandle::ScopedPlatformFence& scoped_fence)
{
    ...
    zx::event temp_event;
    zx_status_t status = scoped_fence.duplicate(ZX_RIGHT_SAME_RIGHTS, &temp_event);
    if (status != ZX_OK) {
    ...
    const BOOL result = ::DuplicateHandle(process, scoped_fence.Get(), process, &duplicated_handle, 0, FALSE, DUPLICATE_SAME_ACCESS);
    if (!result) {
...
GpuFenceHandle::RefCountedScopedFence::RefCountedScopedFence(ScopedPlatformFence scoped_fence)
...
{
    return is_null() ? base::ScopedFD().get() : smart_fence_.get()->scoped_fence_.get();
}
#elif BUILDFLAG(IS_WIN)
HANDLE GpuFenceHandle::Peek() const
{
    return is_null() ? INVALID_HANDLE_VALUE : smart_fence_.get()->scoped_fence_.Get();
}
#endif
...
void GpuFenceHandle::Adopt(ScopedPlatformFence scoped_fence)
{
    if (scoped_fence.is_valid()) {
        smart_fence_ = base::MakeRefCounted<RefCountedScopedFence>(std::move(scoped_fence));
    } else {
        Reset();
    }
}
