...
public:
    ...
    static std::unique_ptr<gfx::ImageFamily> CreateImageFamilyFromIconResource(HMODULE module, int resource_id);
    ...
    static bool CreateIconFileFromImageFamily(const gfx::ImageFamily& image_family, const base::FilePath& icon_path, WriteType write_type = ATOMIC_WRITE);

    // Creates a cursor of the specified size from the SkBitmap passed in.
    // Returns the cursor on success or NULL on failure.
    static base::win::ScopedHICON CreateCursorFromSkBitmap(const SkBitmap& bitmap, const gfx::Point& hotspot);
...
#pragma pack(pop)
    ...
    // |icon_image|.
    static void SetSingleIconImageInformation(
    ...
    static void CopySkBitmapBitsIntoIconBuffer(const SkBitmap& bitmap, unsigned char* buffer, size_t buffer_size);
    ...
    static void ComputeBitmapSizeComponents(const SkBitmap& bitmap, size_t* xor_mask_size, DWORD* bytes_in_resource);
