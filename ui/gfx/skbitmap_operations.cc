...
{
    return bitmap.isNull() && bitmap.colorType() == kUnknown_SkColorType && bitmap.alphaType() == kUnknown_SkAlphaType;
}
...
// static
SkBitmap SkBitmapOperations::CreateBlendedBitmap(const SkBitmap& first, const SkBitmap& second, double alpha)
{
...
// static
SkBitmap SkBitmapOperations::CreateMaskedBitmap(const SkBitmap& rgb, const SkBitmap& alpha)
{
...
// static
SkBitmap SkBitmapOperations::CreateButtonBackground(SkColor color, const SkBitmap& image, const SkBitmap& mask)
{
    CHECK_EQ(image.colorType(), kN32_SkColorType);
    ...
    for (int y = 0; y < mask.height(); ++y) {
        ...
        for (int x = 0; x < mask.width(); ++x) {
            ...
            dst_row[x] = SkColorSetARGB(
                ...
                static_cast<int>(std::min(255.0, bg_a + img_a) * mask_a), static_cast<int>(((bg_r * img_inv) + (img_r * img_alpha)) * mask_a),
                static_cast<int>(((bg_g * img_inv) + (img_g * img_alpha)) * mask_a), static_cast<int>(((bg_b * img_inv) + (img_b * img_alpha)) * mask_a));
        }
    }
...
typedef void (*LineProcessor)(const color_utils::HSL&, const SkPMColor*, SkPMColor*, int width);
...
void LineProcDefault(const color_utils::HSL& hsl_shift, const SkPMColor* in, SkPMColor* out, int width)
{
    for (int x = 0; x < width; x++) {
        out[x] = SkPreMultiplyColor(color_utils::HSLShift(SkUnPreMultiply::PMColorToColor(in[x]), hsl_shift));
    }
}

// Line processor: no-op (i.e., copy).
void LineProcCopy(const color_utils::HSL& hsl_shift, const SkPMColor* in, SkPMColor* out, int width)
{
...
void LineProcHnopSnopLdec(const color_utils::HSL& hsl_shift, const SkPMColor* in, SkPMColor* out, int width)
{
...
void LineProcHnopSnopLinc(const color_utils::HSL& hsl_shift, const SkPMColor* in, SkPMColor* out, int width)
{
    const uint32_t den = 65536;
...
void LineProcHnopSdecLnop(const color_utils::HSL& hsl_shift, const SkPMColor* in, SkPMColor* out, int width)
{
    DCHECK(hsl_shift.h < 0);
...
void LineProcHnopSdecLdec(const color_utils::HSL& hsl_shift, const SkPMColor* in, SkPMColor* out, int width)
{
    DCHECK(hsl_shift.h < 0);
    DCHECK(hsl_shift.s >= 0 && hsl_shift.s <= 0.5 - HSLShift::epsilon);
...
void LineProcHnopSdecLinc(const color_utils::HSL& hsl_shift, const SkPMColor* in, SkPMColor* out, int width)
{
    DCHECK(hsl_shift.h < 0);
    DCHECK(hsl_shift.s >= 0 && hsl_shift.s <= 0.5 - HSLShift::epsilon);
    DCHECK(hsl_shift.l >= 0.5 + HSLShift::epsilon && hsl_shift.l <= 1);
...
const LineProcessor kLineProcessors[kNumHOps][kNumSOps][kNumLOps] = { { // H: kOpHNone
                                                                          {
                                                                              // S: kOpSNone
...
// static
SkBitmap SkBitmapOperations::CreateHSLShiftedBitmap(const SkBitmap& bitmap, const color_utils::HSL& hsl_shift)
{
...
// static
SkBitmap SkBitmapOperations::CreateTiledBitmap(const SkBitmap& source, int src_x, int src_y, int dst_w, int dst_h)
{
...
// static
SkBitmap SkBitmapOperations::DownsampleByTwoUntilSize(const SkBitmap& bitmap, int min_w, int min_h)
{
...
SkBitmap SkBitmapOperations::UnPreMultiply(const SkBitmap& bitmap)
{
    ...
    const SkImageInfo& opaque_info = bitmap.info().makeAlphaType(kUnpremul_SkAlphaType);
...
// static
SkBitmap SkBitmapOperations::CreateColorMask(const SkBitmap& bitmap, SkColor c)
{
    CHECK_EQ(bitmap.colorType(), kN32_SkColorType);
...
// static
SkBitmap SkBitmapOperations::CreateDropShadow(const SkBitmap& bitmap, const gfx::ShadowValues& shadows)
{
    CHECK_EQ(bitmap.colorType(), kN32_SkColorType);
    ...
    image_with_shadow.allocN32Pixels(bitmap.width() + shadow_margin.width(), bitmap.height() + shadow_margin.height());
    image_with_shadow.eraseARGB(0, 0, 0, 0);

    SkCanvas canvas(image_with_shadow, SkSurfaceProps {});
    canvas.translate(SkIntToScalar(shadow_margin.left()), SkIntToScalar(shadow_margin.top()));

    SkPaint paint;
    for (size_t i = 0; i < shadows.size(); ++i) {
        const gfx::ShadowValue& shadow = shadows[i];
        SkBitmap shadow_image = SkBitmapOperations::CreateColorMask(bitmap, shadow.color());
        ...
        canvas.drawImage(shadow_image.asImage(), SkIntToScalar(shadow.x()), SkIntToScalar(shadow.y()));
        canvas.restore();
    }
...
// static
SkBitmap SkBitmapOperations::Rotate(const SkBitmap& source, RotationAmount rotation)
{
    ...
    canvas.translate(SkFloatToScalar(result.width() * 0.5f), SkFloatToScalar(result.height() * 0.5f));
    canvas.rotate(angle);
    canvas.translate(-SkFloatToScalar(source.width() * 0.5f), -SkFloatToScalar(source.height() * 0.5f));
