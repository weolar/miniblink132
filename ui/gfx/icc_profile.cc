...
public:
    DataToProfileCache()
        : DataToProfileCacheBase(kMaxCachedICCProfiles)
    {
    }
};
base::LazyInstance<DataToProfileCache>::Leaky g_data_to_profile_cache = LAZY_INSTANCE_INITIALIZER;

// Lock that must be held to access |g_data_to_profile_cache|.
base::LazyInstance<base::Lock>::Leaky g_icc_profile_lock = LAZY_INSTANCE_INITIALIZER;
...
void ICCProfile::Internals::Initialize()
{
    ...
    if (fabsf(wX - kD50_WhitePoint[0]) > 0.04f || fabsf(wY - kD50_WhitePoint[1]) > 0.04f || fabsf(wZ - kD50_WhitePoint[2]) > 0.04f) {
        return;
    }
...
ColorSpace ICCProfile::GetColorSpace() const
{
    ...
    return ColorSpace(ColorSpace::PrimaryID::CUSTOM, ColorSpace::TransferID::CUSTOM, ColorSpace::MatrixID::RGB, ColorSpace::RangeID::FULL,
        &internals_->to_XYZD50_, &internals_->transfer_fn_);
}
...
ColorSpace ICCProfile::GetPrimariesOnlyColorSpace() const
{
    ...
    return ColorSpace(
        ColorSpace::PrimaryID::CUSTOM, ColorSpace::TransferID::SRGB, ColorSpace::MatrixID::RGB, ColorSpace::RangeID::FULL, &internals_->to_XYZD50_, nullptr);
}
