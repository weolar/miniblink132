...
bool IsSpace(UChar32 c)
{
    ...
    if (!c)
        return false;
    const int8_t char_type = u_charType(c);
    return char_type == U_SPACE_SEPARATOR || char_type == U_LINE_SEPARATOR || char_type == U_PARAGRAPH_SEPARATOR || char_type == U_CONTROL_CHAR;
}

std::u16string RemoveAcceleratorChar(bool full_removal, const std::u16string& s, int* accelerated_char_pos, int* accelerated_char_span)
{
    bool escaped = false;
    ...
    enum { kFoundNothing, kFoundOpenParen, kFoundAcceleratorChar, kFoundAccelerator } cjk_state = kFoundNothing;
...
std::u16string LocateAndRemoveAcceleratorChar(const std::u16string& s, int* accelerated_char_pos, int* accelerated_char_span)
{
    return RemoveAcceleratorChar(false, s, accelerated_char_pos, accelerated_char_span);
}
...
size_t FindValidBoundaryBefore(const std::u16string& text, size_t index, bool trim_whitespace)
{
...
size_t FindValidBoundaryAfter(const std::u16string& text, size_t index, bool trim_whitespace)
{
    ...
    if (trim_whitespace) {
        ...
        while (!it.end() && IsSpace(it.get()) && !IsCombiningMark(it.NextCodePoint())) {
            it.Advance();
        }
    }
...
Insets AdjustVisualBorderForFont(const FontList& font_list, const Insets& desired_visual_padding)
{
...
int GetFontCapHeightCenterOffset(const gfx::FontList& original_font, const gfx::FontList& to_center)
{
    const int original_cap_height = original_font.GetCapHeight();
    const int original_cap_leading = original_font.GetBaseline() - original_cap_height;
    ...
    const int new_cap_top = original_cap_leading + base::ClampRound(cap_height_diff / 2.0f);
