...
public:
    ...
    // clang-format on
    static constexpr Transform RowMajor(double r0c0, double r0c1, double r0c2, double r0c3, double r1c0, double r1c1, double r1c2, double r1c3, double r2c0,
        double r2c1, double r2c2, double r2c3, double r3c0, double r3c1, double r3c2, double r3c3)
    {
    ...
    static constexpr Transform ColMajor(double r0c0, double r1c0, double r2c0, double r3c0, double r0c1, double r1c1, double r2c1, double r3c1, double r0c2,
        double r1c2, double r2c2, double r3c2, double r0c3, double r1c3, double r2c3, double r3c3)
    {
        return Transform(r0c0, r1c0, r2c0, r3c0, // col 0
            r0c1, r1c1, r2c1, r3c1, // col 1
            r0c2, r1c2, r2c2, r3c2, // col 2
    ...
    static constexpr Transform Make180degRotation()
    {
        return MakeScale(-1);
    }
    ...
    constexpr double rc(int row, int col) const
    {
        ...
        if (!full_matrix_) [[likely]] {
            float m[4][4] = { { axis_2d_.scale().x(), 0, 0, axis_2d_.translation().x() }, { 0, axis_2d_.scale().y(), 0, axis_2d_.translation().y() },
                { 0, 0, 1, 0 }, { 0, 0, 0, 1 } };
            return m[row][col];
        }
    ...
    double ColMajorData(int index) const
    {
        return rc(index % 4, index / 4);
    }
    ...
    void SkewX(double degrees)
    {
        Skew(degrees, 0);
    }
    void SkewY(double degrees)
    {
        Skew(0, degrees);
    }
    ...
    bool IsPositiveScaleOrTranslation() const
    {
        ...
        if (!matrix_.IsScaleOrTranslation())
            return false;
        return matrix_.rc(0, 0) > 0.0 && matrix_.rc(1, 1) > 0.0 && matrix_.rc(2, 2) > 0.0;
    }
    ...
    // be inverted.
    [[nodiscard]] std::optional<PointF> InverseMapPoint(const PointF& point) const;
    [[nodiscard]] std::optional<Point3F> InverseMapPoint(const Point3F& point) const;
    ...
    [[nodiscard]] PointF ProjectPoint(const PointF& point, bool* clamped = nullptr) const;
    ...
    bool ApproximatelyEqual(const gfx::Transform& transform, float abs_translation_tolerance, float abs_other_tolerance, float rel_scale_tolerance) const;
    // Checks approximate equality with one tolerance for all components.
    bool ApproximatelyEqual(const gfx::Transform& transform, float abs_tolerance) const
    {
    ...
    void EnsureFullMatrixForTesting()
    {
        EnsureFullMatrix();
    }
...
private:
  ...
  constexpr Transform(double r0c0, double r1c0, double r2c0, double r3c0,
    ...
    constexpr Transform(float scale_x, float scale_y, float trans_x, float trans_y)
        : axis_2d_(AxisTransform2d::FromScaleAndTranslation(Vector2dF(scale_x, scale_y), Vector2dF(trans_x, trans_y)))
    {
    }
