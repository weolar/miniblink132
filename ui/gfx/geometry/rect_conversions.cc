...
{
    Rect result;
    result.SetByBounds(base::ClampCeil(rect.x()), base::ClampCeil(rect.y()), base::ClampFloor(rect.right()), base::ClampFloor(rect.bottom()));
    return result;
}
...
bool IsNearestRectWithinDistance(const gfx::RectF& rect, float distance)
{
    ...
    return (std::abs(min_x - float_min_x) < distance) && (std::abs(min_y - float_min_y) < distance) && (std::abs(max_x - float_max_x) < distance)
...
{
    return Rect(base::ClampFloor(rect.x()), base::ClampFloor(rect.y()), base::ClampFloor(rect.width()), base::ClampFloor(rect.height()));
}
