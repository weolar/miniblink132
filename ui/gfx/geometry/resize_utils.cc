...
void SizeRectToAspectRatioWithExcludedMargin(ResizeEdge resize_edge, float aspect_ratio, const Size& original_min_window_size,
    std::optional<Size> max_window_size, const Size& excluded_margin, Rect& rect)
{
    ...
    if (max_window_size.has_value()) {
        ...
        DCHECK(Rect(rect.origin(), *max_window_size).Contains(rect)) << rect.ToString() << " is larger than the maximum size " << max_window_size->ToString();
    }
    DCHECK(rect.Contains(Rect(rect.origin(), original_min_window_size)))
        << rect.ToString() << " is smaller than the minimum size " << original_min_window_size.ToString();
    ...
    Size new_size(rect.width() - excluded_margin.width(), rect.height() - excluded_margin.height());
    if (max_window_size) {
        max_window_size.emplace(max_window_size->width() - excluded_margin.width(), max_window_size->height() - excluded_margin.height());
    }
    ...
    if (IsResizingHorizontally(resize_edge)) {
        ...
        if (min_window_size.height() > new_size.height() || (max_window_size.has_value() && new_size.height() > max_window_size->height())) {
            if (max_window_size.has_value()) {
                new_size.set_height(std::clamp(new_size.height(), min_window_size.height(), max_window_size->height()));
            } else {
        ...
        new_size.set_width(base::ClampRound(new_size.height() * aspect_ratio));
        if (min_window_size.width() > new_size.width() || (max_window_size.has_value() && new_size.width() > max_window_size->width())) {
            if (max_window_size.has_value()) {
                new_size.set_width(std::clamp(new_size.width(), min_window_size.width(), max_window_size->width()));
            } else {
...
void SizeRectToAspectRatio(ResizeEdge resize_edge, float aspect_ratio, const Size& min_window_size, std::optional<Size> max_window_size, Rect* rect)
{
    SizeRectToAspectRatioWithExcludedMargin(resize_edge, aspect_ratio, min_window_size, std::move(max_window_size), gfx::Size(), *rect);
}
