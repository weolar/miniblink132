...
public:
    ...
    constexpr Point3F(float x, float y, float z)
        : x_(x)
        , y_(y)
        , z_(z)
    {
    ...
    PointF AsPointF() const
    {
        return PointF(x_, y_);
    }

    Vector3dF OffsetFromOrigin() const
    {
        return Vector3dF(x_, y_, z_);
    }
...
inline Point3F ScalePoint(const Point3F& p, float x_scale, float y_scale, float z_scale)
{
