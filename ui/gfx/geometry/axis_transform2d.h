...
public:
    ...
    static constexpr AxisTransform2d FromScaleAndTranslation(const Vector2dF& scale, const Vector2dF& translation)
    {
    ...
    void PreScale(const Vector2dF& scale)
    {
    ...
    {
        return double { scale_.x() } * scale_.y();
    }
    ...
    {
        translation_.Scale(zoom_factor);
    }
    ...
    RectF MapRect(const RectF& r) const
    {
        ...
        return RectF(MapX(r.x()), MapY(r.y()), ClampFloatGeometry(r.width() * scale_.x()), ClampFloatGeometry(r.height() * scale_.y()));
    }
    RectF InverseMapRect(const RectF& r) const
    {
        ...
        return RectF(InverseMapX(r.x()), InverseMapY(r.y()),
            ...
            ClampFloatGeometry(r.width() * (1.f / scale_.x())), ClampFloatGeometry(r.height() * (1.f / scale_.y())));
    }
    ...
    constexpr const Vector2dF& scale() const
    {
        return scale_;
    }
    constexpr const Vector2dF& translation() const
    {
        return translation_;
    }
...
private:
    constexpr AxisTransform2d(const Vector2dF& scale, const Vector2dF& translation)
        : scale_(scale)
        , translation_(translation)
    {
    }
...
inline AxisTransform2d PreScaleAxisTransform2d(const AxisTransform2d& t, float scale)
{
...
inline AxisTransform2d PostScaleAxisTransform2d(const AxisTransform2d& t, float scale)
{
    AxisTransform2d result(t);
...
inline AxisTransform2d PreTranslateAxisTransform2d(const AxisTransform2d& t, const Vector2dF& translation)
{
    AxisTransform2d result(t);
...
inline AxisTransform2d PostTranslateAxisTransform2d(const AxisTransform2d& t, const Vector2dF& translation)
{
    AxisTransform2d result(t);
...
inline AxisTransform2d ConcatAxisTransform2d(const AxisTransform2d& post, const AxisTransform2d& pre)
{
