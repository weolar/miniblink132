...
Quaternion::Quaternion(const Vector3dF& from, const Vector3dF& to)
{
    ...
    if (real < kEpsilon * norm) {
        real = 0.0f;
        axis = std::abs(from.x()) > std::abs(from.z()) ? gfx::Vector3dF { -from.y(), from.x(), 0.0 } : gfx::Vector3dF { 0.0, -from.z(), from.y() };
    } else {
...
Quaternion Quaternion::FromAxisAngle(double x, double y, double z, double angle)
{
...
std::string Quaternion::ToString() const
{
    ...
    v.Scale(scale);
    return base::StringPrintf("[%f %f %f %f], v:", x_, y_, z_, w_) + v.ToString() + base::StringPrintf(", θ:%fπ", abs_theta / std::numbers::pi_v<float>);
}
