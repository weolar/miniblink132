...
void ExpectTransformOperationEqual(const TransformOperation& lhs, const TransformOperation& rhs)
{
...
TEST(TransformOperationTest, TransformTypesAreUnique)
{
    ...
    std::unique_ptr<TransformOperations> to_add(std::make_unique<TransformOperations>());
...
TEST(TransformOperationTest, NoneAlwaysMatches)
{
    ...
    for (const auto& operation : operations) {
        EXPECT_EQ(operation->size(), operation->MatchingPrefixLength(none_operation));
    }
}
...
TEST(TransformOperationTest, BlendOrder)
{
    ...
    operations_expected.AppendScale(gfx::Tween::FloatValueBetween(progress, sx1, sx2), gfx::Tween::FloatValueBetween(progress, sy1, sy2),
        gfx::Tween::FloatValueBetween(progress, sz1, sz2));

    operations_expected.AppendTranslate(gfx::Tween::FloatValueBetween(progress, dx1, dx2), gfx::Tween::FloatValueBetween(progress, dy1, dy2),
    ...
    operations_expected.AppendScale(
        gfx::Tween::FloatValueBetween(progress, 1, sx3), gfx::Tween::FloatValueBetween(progress, 1, sy3), gfx::Tween::FloatValueBetween(progress, 1, sz3));
...
static void CheckProgress(SkScalar progress, const gfx::Transform& from_matrix, const gfx::Transform& to_matrix, const TransformOperations& from_transform,
    const TransformOperations& to_transform)
{
    gfx::Transform expected_matrix = to_matrix;
    expected_matrix.Blend(from_matrix, progress);
    EXPECT_TRANSFORM_EQ(expected_matrix, to_transform.Blend(from_transform, progress).Apply());
}
...
TEST(TransformOperationTest, BlendWhenTypesDoNotMatch)
{
    ...
    EXPECT_TRANSFORM_EQ(expected, operations_to.Blend(operations_from, progress).Apply());
}
...
TEST(TransformOperationTest, BlendRotationFromIdentity)
{
    ...
    for (const auto& identity_operation : identity_operations) {
        ...
        EXPECT_TRANSFORM_EQ(expected, operations.Blend(*identity_operation, progress).Apply());
...
TEST(TransformOperationTest, BlendSkewFromEmpty)
{
    ...
    EXPECT_TRANSFORM_EQ(expected, operations.Blend(empty_operation, progress).Apply());
...
TEST(TransformOperationTest, BlendRotationToIdentity)
{
    ...
    for (const auto& identity_operation : identity_operations) {
        ...
        EXPECT_TRANSFORM_EQ(expected, identity_operation->Blend(operations, progress).Apply());
    }
}
...
TEST(TransformOperationTest, BlendSkewToEmpty)
{
    ...
    EXPECT_TRANSFORM_EQ(expected, empty_operation.Blend(operations, progress).Apply());
}
...
TEST(TransformOperationTest, NonDecomposableBlend)
{
    ...
    EXPECT_TRANSFORM_EQ(non_decomposible_matrix, identity_transform.Blend(non_decomposible_transform, 0.0f).Apply());
    EXPECT_TRANSFORM_EQ(non_decomposible_matrix, identity_transform.Blend(non_decomposible_transform, 0.49f).Apply());

    // After the half-way point, we should return the 'to' matrix.
    EXPECT_TRANSFORM_EQ(identity_matrix, identity_transform.Blend(non_decomposible_transform, 0.5f).Apply());
    EXPECT_TRANSFORM_EQ(identity_matrix, identity_transform.Blend(non_decomposible_transform, 1.0f).Apply());
}
...
TEST(TransformOperationTest, BlendedBoundsWhenTypesDoNotMatch)
{
    ...
    EXPECT_FALSE(operations_to.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
}
...
TEST(TransformOperationTest, BlendedBoundsForIdentity)
{
    ...
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
...
TEST(TransformOperationTest, BlendedBoundsForTranslate)
{
    ...
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(2.f, -6.f, -1.f, 12.f, 20.f, 12.f).ToString(), bounds.ToString());

    min_progress = 0.f;
    max_progress = 1.f;
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(4.f, -2.f, 1.f, 8.f, 12.f, 8.f).ToString(), bounds.ToString());

    TransformOperations identity;
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, identity, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(1.f, 2.f, 1.f, 11.f, 8.f, 6.f).ToString(), bounds.ToString());

    EXPECT_TRUE(identity.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(1.f, -2.f, 3.f, 7.f, 8.f, 6.f).ToString(), bounds.ToString());
}
...
TEST(TransformOperationTest, BlendedBoundsForScale)
{
    ...
    SkScalar max_progress = 1.5f;
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(1.f, -7.5f, -28.f, 44.f, 42.f, 56.f).ToString(), bounds.ToString());

    min_progress = 0.f;
    max_progress = 1.f;
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(3.f, 1.f, -14.f, 32.f, 23.f, 28.f).ToString(), bounds.ToString());

    TransformOperations identity;
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, identity, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(1.f, 2.f, -14.f, 34.f, 22.f, 21.f).ToString(), bounds.ToString());

    EXPECT_TRUE(identity.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(1.f, 1.f, 3.f, 14.f, 5.f, 11.f).ToString(), bounds.ToString());
}
...
TEST(TransformOperationTest, BlendedBoundsWithZeroScale)
{
    ...
    SkScalar min_progress = 0.f;
    SkScalar max_progress = 1.f;
    EXPECT_TRUE(zero_scale.BlendedBoundsForBox(box, non_zero_scale, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(0.f, -24.f, 0.f, 10.f, 24.f, 35.f).ToString(), bounds.ToString());

    EXPECT_TRUE(non_zero_scale.BlendedBoundsForBox(box, zero_scale, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(0.f, -24.f, 0.f, 10.f, 24.f, 35.f).ToString(), bounds.ToString());

    EXPECT_TRUE(zero_scale.BlendedBoundsForBox(box, zero_scale, min_progress, max_progress, &bounds));
...
TEST(TransformOperationTest, BlendedBoundsForRotationTrivial)
{
    ...
    for (float size : sizes) {
        ...
        SkScalar max_progress = 1.f;
        EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
        EXPECT_EQ(gfx::BoxF(-2.f, -2.f, 0.f, 4.f, 4.f, 0.f).ToString(), bounds.ToString());
    }
}
...
TEST(TransformOperationTest, BlendedBoundsForRotationAllExtrema)
{
    ...
    float max = 1.f;
    float size = max - min;
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, operations_from, 0.f, 1.f, &bounds));
    EXPECT_EQ(gfx::BoxF(min, min, min, size, size, size).ToString(), bounds.ToString());
}
...
TEST(TransformOperationTest, BlendedBoundsForRotationDifferentAxes)
{
    ...
    EXPECT_TRUE(operations_to_same.BlendedBoundsForBox(box, operations_from, 0.f, 1.f, &bounds));
    EXPECT_TRUE(operations_to_opposite.BlendedBoundsForBox(box, operations_from, 0.f, 1.f, &bounds));
    EXPECT_FALSE(operations_to_different.BlendedBoundsForBox(box, operations_from, 0.f, 1.f, &bounds));
}
...
TEST(TransformOperationTest, BlendedBoundsForRotationProblematicAxes)
{
    ...
    struct {
        ...
        float z;
        gfx::BoxF expected;
    } tests[] = { { 0.f, 0.f, 0.f, gfx::BoxF(1.f, 1.f, 1.f, 0.f, 0.f, 0.f) }, { 1.f, 0.f, 0.f, gfx::BoxF(1.f, -dim2, -dim2, 0.f, dim3, dim3) },
        { 0.f, 1.f, 0.f, gfx::BoxF(-dim2, 1.f, -dim2, dim3, 0.f, dim3) }, { 0.f, 0.f, 1.f, gfx::BoxF(-dim2, -dim2, 1.f, dim3, dim3, 0.f) },
        { 1.f, 1.f, 0.f, gfx::BoxF(dim1, dim1, -1.f, dim2, dim2, 2.f) }, { 0.f, 1.f, 1.f, gfx::BoxF(-1.f, dim1, dim1, 2.f, dim2, dim2) },
...
static void ExpectBoxesApproximatelyEqual(const gfx::BoxF& lhs, const gfx::BoxF& rhs, float tolerance)
{
...
static void EmpiricallyTestBounds(
    const TransformOperations& from, const TransformOperations& to, SkScalar min_progress, SkScalar max_progress, bool test_containment_only)
{
    ...
    if (test_containment_only) {
        ...
        gfx::Rect bounds_rect = ToEnclosingRect(gfx::RectF(bounds.x(), bounds.y(), bounds.width(), bounds.height()));
        gfx::Rect unified_bounds_rect = ToEnclosingRect(gfx::RectF(unified_bounds.x(), unified_bounds.y(), unified_bounds.width(), unified_bounds.height()));
...
static void EmpiricallyTestBoundsEquality(const TransformOperations& from, const TransformOperations& to, SkScalar min_progress, SkScalar max_progress)
{
    EmpiricallyTestBounds(from, to, min_progress, max_progress, false);
}

static void EmpiricallyTestBoundsContainment(const TransformOperations& from, const TransformOperations& to, SkScalar min_progress, SkScalar max_progress)
{
...
TEST(TransformOperationTest, BlendedBoundsForRotationEmpiricalTests)
{
    ...
    struct {
        ...
        float y;
        float z;
    } axes[] = { { 1.f, 1.f, 1.f }, { -1.f, -1.f, -1.f }, { -1.f, 2.f, 3.f }, { 1.f, -2.f, 3.f }, { 1.f, 2.f, -3.f }, { 0.f, 0.f, 0.f }, { 1.f, 0.f, 0.f },
        { 0.f, 1.f, 0.f }, { 0.f, 0.f, 1.f }, { 1.f, 1.f, 0.f }, { 0.f, 1.f, 1.f }, { 1.f, 0.f, 1.f }, { -1.f, 0.f, 0.f }, { 0.f, -1.f, 0.f },
        { 0.f, 0.f, -1.f }, { -1.f, -1.f, 0.f }, { 0.f, -1.f, -1.f }, { -1.f, 0.f, -1.f } };

    struct {
        float theta_from;
        float theta_to;
    } angles[] = { { 5.f, 10.f }, { 10.f, 5.f }, { 0.f, 360.f }, { 20.f, 180.f }, { -20.f, -180.f }, { 180.f, -220.f }, { 220.f, 320.f } };
    ...
    for (const auto& axis : axes) {
        for (const auto& angle : angles) {
            for (const auto& progress : progresses) {
                ...
                EmpiricallyTestBoundsContainment(operations_from, operations_to, progress.min_progress, progress.max_progress);
            }
        }
    }
}
...
TEST(TransformOperationTest, PerspectiveMatrixAndTransformBlendingEquivalency)
{
    ...
    for (int i = 0; i < steps; ++i) {
        ...
        gfx::Transform blended_transform = to_operations.Blend(from_operations, progress).Apply();
...
TEST(TransformOperationTest, NonCommutativeRotations)
{
    ...
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
    gfx::Transform blended_transform = operations_to.Blend(operations_from, max_progress).Apply();
...
TEST(TransformOperationTest, BlendedBoundsForSequence)
{
    ...
    SkScalar max_progress = 1.5f;
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(-57.f, -59.f, -1.f, 76.f, 112.f, 80.f).ToString(), bounds.ToString());

    min_progress = 0.f;
    max_progress = 1.f;
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(-32.f, -25.f, 7.f, 42.f, 44.f, 48.f).ToString(), bounds.ToString());

    TransformOperations identity;
    EXPECT_TRUE(operations_to.BlendedBoundsForBox(box, identity, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(-33.f, -13.f, 3.f, 57.f, 19.f, 52.f).ToString(), bounds.ToString());

    EXPECT_TRUE(identity.BlendedBoundsForBox(box, operations_from, min_progress, max_progress, &bounds));
    EXPECT_EQ(gfx::BoxF(-7.f, -3.f, 2.f, 15.f, 23.f, 20.f).ToString(), bounds.ToString());
}
