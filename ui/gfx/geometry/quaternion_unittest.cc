...
TEST(QuatTest, AxisAngleCommon)
{
    ...
    CompareQuaternions(Quaternion(std::sin(radians / 2), 0, 0, std::cos(radians / 2)), q);
}
...
TEST(QuatTest, Multiplication)
{
    struct {
        ...
        { Quaternion(5, 6, 7, 8), Quaternion(1, 2, 3, 4), Quaternion(32, 32, 56, -6) },
    };
...
TEST(QuatTest, Slerp360)
{
    ...
    Quaternion expected(0, 0, std::sin(expected_half_angle), std::cos(expected_half_angle));
    EXPECT_QUATERNION(expected, interpolated);
}
...
TEST(QuatTest, Equals)
{
    ...
    EXPECT_TRUE(Quaternion(1, 5.2, -8.5, 222.2) == Quaternion(1, 5.2, -8.5, 222.2));
...
TEST(QuatTest, NotEquals)
{
    ...
    EXPECT_FALSE(Quaternion(1, 5.2, -8.5, 222.2) != Quaternion(1, 5.2, -8.5, 222.2));
