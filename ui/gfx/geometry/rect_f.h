...
public:
    ...
    constexpr explicit RectF(const SizeF& size)
        : size_(size)
    {
    }
    constexpr explicit RectF(const Size& size)
        : size_(size)
    {
    }
    ...
    constexpr explicit RectF(const Rect& r)
        : RectF(static_cast<float>(r.x()), static_cast<float>(r.y()), static_cast<float>(r.width()), static_cast<float>(r.height()))
    {
    }
    ...
    {
        return PointF(right(), y());
    }
    constexpr PointF bottom_left() const
    {
        return PointF(x(), bottom());
    }
...
GEOMETRY_EXPORT RectF MapRect(const RectF& r, const RectF& src_rect, const RectF& dest_rect);
