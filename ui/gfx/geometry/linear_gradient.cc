...
LinearGradient::LinearGradient(int16_t angle)
    : angle_(angle)
{
}
...
void LinearGradient::AddStep(float fraction, uint8_t alpha)
{
    ...
    DCHECK(step_count_ ? steps_[step_count_ - 1].fraction < fraction : true)
        << base::StringPrintf("prev[%zu]=%f, next[%zu]=%f", step_count_ - 1, steps_[step_count_ - 1].fraction, step_count_, fraction);
...
{
    std::string result = base::StringPrintf("LinearGradient{angle=%d, step_count=%zu [", angle_, step_count_);
    for (size_t i = 0; i < step_count_; ++i) {
        if (i)
            result += " - ";
