...
TransformOperations& TransformOperations::operator=(const TransformOperations& other)
{
    operations_ = other.operations_;
    return *this;
}
...
TransformOperations TransformOperations::Blend(const TransformOperations& from, SkScalar progress) const
{
...
bool TransformOperations::BlendedBoundsForBox(
    const BoxF& box, const TransformOperations& from, SkScalar min_progress, SkScalar max_progress, BoxF* bounds) const
{
    *bounds = box;
    ...
    size_t num_operations = std::max(from_identity ? 0 : from.operations_.size(), to_identity ? 0 : operations_.size());
    ...
    for (size_t i = 0; i < num_operations; ++i) {
        ...
        if (!TransformOperation::BlendedBoundsForBox(*bounds, from_op, to_op, min_progress, max_progress, &bounds_for_operation)) {
            return false;
        }
    ...
    for (auto& operation : operations_) {
        ...
        case TransformOperation::TRANSFORM_OPERATION_MATRIX: {
            if (operation.matrix.HasPerspective())
                return false;
            Vector2dF scale_components = ComputeTransform2dScaleComponents(operation.matrix, 1.f);
        ...
        case TransformOperation::TRANSFORM_OPERATION_PERSPECTIVE:
            return false;
        case TransformOperation::TRANSFORM_OPERATION_SCALE:
            operations_scale *= std::max(std::abs(operation.scale.x), std::max(std::abs(operation.scale.y), std::abs(operation.scale.z)));
        }
    }
...
size_t TransformOperations::MatchingPrefixLength(const TransformOperations& other) const
{
    size_t num_operations = std::min(operations_.size(), other.operations_.size());
    for (size_t i = 0; i < num_operations; ++i) {
        if (operations_[i].type != other.operations_[i].type) {
...
void TransformOperations::AppendRotate(SkScalar x, SkScalar y, SkScalar z, SkScalar degrees)
{
    TransformOperation to_add;
...
bool TransformOperations::ApproximatelyEqual(const TransformOperations& other, SkScalar tolerance) const
{
    if (size() != other.size())
        return false;
    for (size_t i = 0; i < operations_.size(); ++i) {
...
bool TransformOperations::BlendInternal(const TransformOperations& from, SkScalar progress, TransformOperations* result) const
{
    bool from_identity = from.IsIdentity();
    bool to_identity = IsIdentity();
    if (from_identity && to_identity)
    ...
    for (size_t i = 0; i < matching_prefix_length; ++i) {
        ...
        if (!TransformOperation::BlendTransformOperations(
                i >= from_size ? nullptr : &from.operations_[i], i >= to_size ? nullptr : &operations_[i], progress, &blended)) {
            return false;
        }
    ...
    if (matching_prefix_length < num_operations) {
        if (!ComputeDecomposedTransform(matching_prefix_length) || !from.ComputeDecomposedTransform(matching_prefix_length)) {
            return false;
        }
        DecomposedTransform matrix_transform = BlendDecomposedTransforms(
            *decomposed_transforms_[matching_prefix_length].get(), *from.decomposed_transforms_[matching_prefix_length].get(), progress);
        result->AppendMatrix(Transform::Compose(matrix_transform));
    }
    return true;
}

bool TransformOperations::ComputeDecomposedTransform(size_t start_offset) const
{
    ...
    if (it == decomposed_transforms_.end()) {
        ...
        if (std::optional<DecomposedTransform> decomp = transform.Decompose()) {
            decomposed_transforms_[start_offset] = std::make_unique<DecomposedTransform>(*decomp);
        } else {
            return false;
        }
    }
