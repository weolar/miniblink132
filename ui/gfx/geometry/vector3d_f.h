...
public:
    ...
    constexpr Vector3dF(float x, float y, float z)
        : x_(x)
        , y_(y)
        , z_(z)
    {
    ...
    void operator+=(const Vector3dF& other)
    {
        Add(other);
    }
    void operator-=(const Vector3dF& other)
    ...
    void InvScale(float inv_scale)
    {
        InvScale(inv_scale, inv_scale, inv_scale);
    }
...
// axis.
GEOMETRY_EXPORT Vector3dF ScaleVector3d(const Vector3dF& v, float x_scale, float y_scale, float z_scale);
...
GEOMETRY_EXPORT float AngleBetweenVectorsInDegrees(const gfx::Vector3dF& base, const gfx::Vector3dF& other);
...
GEOMETRY_EXPORT float ClockwiseAngleBetweenVectorsInDegrees(const gfx::Vector3dF& base, const gfx::Vector3dF& other, const gfx::Vector3dF& normal);
