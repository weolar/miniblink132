...
public:
    ...
    constexpr BoxF(float x, float y, float z, float width, float height, float depth)
    ...
    void set_origin(const Point3F& origin)
    {
        origin_ = origin;
    }
...
inline BoxF ScaleBox(const BoxF& b, float x_scale, float y_scale, float z_scale)
{
    return BoxF(b.x() * x_scale, b.y() * y_scale, b.z() * z_scale, b.width() * x_scale, b.height() * y_scale, b.depth() * z_scale);
}
...
{
    return BoxF(b.x() + v.x(), b.y() + v.y(), b.z() + v.z(), b.width(), b.height(), b.depth());
}
