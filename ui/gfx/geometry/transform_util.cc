...
template <int n> void Combine(double* out, const double* a, const double* b, double scale_a, double scale_b)
{
    for (int i = 0; i < n; ++i)
...
DecomposedTransform BlendDecomposedTransforms(const DecomposedTransform& to, const DecomposedTransform& from, double progress)
{
...
DecomposedTransform AccumulateDecomposedTransforms(const DecomposedTransform& a, const DecomposedTransform& b)
{
    DecomposedTransform out;
...
AxisTransform2d OrthoProjectionTransform(float left, float right, float bottom, float top)
{
    ...
    return AxisTransform2d::FromScaleAndTranslation(Vector2dF(2.0f / delta_x, 2.0f / delta_y), Vector2dF(-(right + left) / delta_x, -(top + bottom) / delta_y));
}

AxisTransform2d WindowTransform(int x, int y, int width, int height)
{
    // Map from ([-1, -1] to [1, 1]) -> ([x, y] to [x + width, y + height]).
    return AxisTransform2d::FromScaleAndTranslation(Vector2dF(width * 0.5f, height * 0.5f), Vector2dF(x + width * 0.5f, y + height * 0.5f));
}
...
std::optional<Vector2dF> TryComputeTransform2dScaleComponents(const Transform& transform)
{
    ...
    return Vector2dF(ClampFloatGeometry(x_scale * w_scale), ClampFloatGeometry(y_scale * w_scale));
}

Vector2dF ComputeTransform2dScaleComponents(const Transform& transform, float fallback_value)
{
    std::optional<Vector2dF> scale = TryComputeTransform2dScaleComponents(transform);
    if (scale) {
        return *scale;
    }
