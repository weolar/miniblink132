...
public:
    ...
    constexpr RoundedCornersF()
        : RoundedCornersF(0.0f)
    {
    }
    ...
    constexpr RoundedCornersF(float upper_left, float upper_right, float lower_right, float lower_left)
    ...
    constexpr float upper_left() const
    {
        return upper_left_;
    }
    constexpr float upper_right() const
    {
        return upper_right_;
    }
    constexpr float lower_right() const
    {
        return lower_right_;
    }
    constexpr float lower_left() const
    {
        return lower_left_;
    }

    void set_upper_left(float upper_left)
    {
        upper_left_ = clamp(upper_left);
    }
    void set_upper_right(float upper_right)
    {
        upper_right_ = clamp(upper_right);
    }
    void set_lower_right(float lower_right)
    {
        lower_right_ = clamp(lower_right);
    }
    void set_lower_left(float lower_left)
    {
        lower_left_ = clamp(lower_left);
    }

    void Set(float upper_left, float upper_right, float lower_right, float lower_left)
    {
        upper_left_ = clamp(upper_left);
        upper_right_ = clamp(upper_right);
        lower_right_ = clamp(lower_right);
    ...
    bool IsEmpty() const
    {
        return upper_left_ == 0.0f && upper_right_ == 0.0f && lower_right_ == 0.0f && lower_left_ == 0.0f;
    }

    bool operator==(const RoundedCornersF& corners) const
    {
        return upper_left_ == corners.upper_left_ && upper_right_ == corners.upper_right_ && lower_right_ == corners.lower_right_
...
private:
    ...
    static constexpr float clamp(float f)
    {
        return f > kTrivial ? f : 0.f;
    }
