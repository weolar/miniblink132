...
TEST(RectTest, Intersects)
{
    static const struct {
        ...
        int h2;
        bool intersects;
    } tests[] = { { 0, 0, 0, 0, 0, 0, 0, 0, false }, { 0, 0, 0, 0, -10, -10, 20, 20, false }, { -10, 0, 0, 20, 0, -10, 20, 0, false },
        { 0, 0, 10, 10, 0, 0, 10, 10, true }, { 0, 0, 10, 10, 10, 10, 10, 10, false }, { 10, 10, 10, 10, 0, 0, 10, 10, false },
        { 10, 10, 10, 10, 5, 5, 10, 10, true }, { 10, 10, 10, 10, 15, 15, 10, 10, true }, { 10, 10, 10, 10, 20, 15, 10, 10, false },
...
TEST(RectTest, UnionEvenIfEmpty)
{
    ...
    EXPECT_EQ(Rect(0, 0, 8, 10), UnionRectsEvenIfEmpty(Rect(0, 0, 3, 4), Rect(3, 4, 5, 6)));
    EXPECT_EQ(Rect(0, 0, 8, 10), UnionRectsEvenIfEmpty(Rect(3, 4, 5, 6), Rect(0, 0, 3, 4)));
    EXPECT_EQ(Rect(2, 3, 6, 8), UnionRectsEvenIfEmpty(Rect(8, 9, 0, 2), Rect(2, 3, 4, 5)));
    EXPECT_EQ(Rect(2, 3, 8, 6), UnionRectsEvenIfEmpty(Rect(2, 3, 4, 5), Rect(8, 9, 2, 0)));
}
...
TEST(RectTest, AdjustToFit)
{
    static const struct Test {
        ...
        int w3;
        int h3;
    } tests[] = { { 0, 0, 2, 2, 0, 0, 2, 2, 0, 0, 2, 2 }, { 2, 2, 3, 3, 0, 0, 4, 4, 1, 1, 3, 3 }, { -1, -1, 5, 5, 0, 0, 4, 4, 0, 0, 4, 4 },
        { 2, 2, 4, 4, 0, 0, 3, 3, 0, 0, 3, 3 }, { 2, 2, 1, 1, 0, 0, 3, 3, 2, 2, 1, 1 } };
    for (size_t i = 0; i < std::size(tests); ++i) {
        Rect r1(tests[i].x1, tests[i].y1, tests[i].w1, tests[i].h1);
        Rect r2(tests[i].x2, tests[i].y2, tests[i].w2, tests[i].h2);
        Rect r3(tests[i].x3, tests[i].y3, tests[i].w3, tests[i].h3);
...
static void TestScaleRectOverflowClamp(Rect (*function)(const Rect&, float, float))
{
    // The whole rect is scaled out of kMinInt.
    Rect xy_underflow1(-100000, -123456, 10, 20);
    EXPECT_EQ(Rect(kMinInt, kMinInt, 0, 0), function(xy_underflow1, 100000, 100000));
    ...
    EXPECT_EQ(Rect(-kMaxInt, -kMaxInt, kMaxInt, kMaxInt), function(xy_underflow2, 100000, 100000));
    ...
    EXPECT_EQ(Rect(kMaxInt, kMaxInt, 0, 0), function(xy_overflow, 100000, 100000));
    ...
    EXPECT_EQ(Rect(100000, 200000, 0, 0), function(size_underflow, -100000, -100000));

    Rect size_overflow(-1, -2, 123456, 234567);
    EXPECT_EQ(Rect(-100000, -200000, kMaxInt, kMaxInt), function(size_overflow, 100000, 100000));
    // Verify width/right gets clamped properly too if x/y positive.
    Rect size_overflow2(1, 2, 123456, 234567);
    EXPECT_EQ(Rect(100000, 200000, kMaxInt - 100000, kMaxInt - 200000), function(size_overflow2, 100000, 100000));
    ...
    EXPECT_EQ(Rect(kMaxInt, kMaxInt, 0, 0), function(max_origin_rect, kMaxIntAsFloat, kMaxIntAsFloat));
    ...
    EXPECT_EQ(max_size_rect1, function(max_size_rect1, kMaxIntAsFloat, kMaxIntAsFloat));
    ...
    EXPECT_EQ(max_size_rect2, function(max_size_rect2, kMaxIntAsFloat, kMaxIntAsFloat));
    EXPECT_EQ(Rect(kMaxInt, kMaxInt, 0, 0), function(max_size_rect2, kMinInt, kMinInt));
}
...
TEST(RectTest, Offset)
{
    ...
    EXPECT_EQ(Rect(kMaxInt - 2, kMaxInt - 2, 2, 2), (Rect(0, 0, kMaxInt - 2, kMaxInt - 2) + Vector2d(kMaxInt - 2, kMaxInt - 2)));
    EXPECT_EQ(Rect(kMaxInt - 2, kMaxInt - 2, 2, 2), (Rect(0, 0, kMaxInt - 2, kMaxInt - 2) - Vector2d(2 - kMaxInt, 2 - kMaxInt)));
}
...
TEST(RectTest, MaximumCoveredRect)
{
    ...
    EXPECT_EQ(Rect(10, 20, 30, 60), MaximumCoveredRect(Rect(10, 20, 30, 40), Rect(10, 30, 30, 50)));
    // X aligned and adjacent: unite.
    EXPECT_EQ(Rect(10, 20, 30, 90), MaximumCoveredRect(Rect(10, 20, 30, 40), Rect(10, 60, 30, 50)));
    // X aligned and separate: choose the bigger one.
    EXPECT_EQ(Rect(10, 61, 30, 50), MaximumCoveredRect(Rect(10, 20, 30, 40), Rect(10, 61, 30, 50)));
    // Y aligned and intersect: unite.
    EXPECT_EQ(Rect(10, 20, 60, 40), MaximumCoveredRect(Rect(10, 20, 30, 40), Rect(30, 20, 40, 40)));
    // Y aligned and adjacent: unite.
    EXPECT_EQ(Rect(10, 20, 70, 40), MaximumCoveredRect(Rect(10, 20, 30, 40), Rect(40, 20, 40, 40)));
    // Y aligned and separate: choose the bigger one.
    EXPECT_EQ(Rect(41, 20, 40, 40), MaximumCoveredRect(Rect(10, 20, 30, 40), Rect(41, 20, 40, 40)));
    // Get the biggest expanded intersection.
    EXPECT_EQ(Rect(0, 0, 9, 19), MaximumCoveredRect(Rect(0, 0, 10, 10), Rect(0, 9, 9, 10)));
    EXPECT_EQ(Rect(0, 0, 19, 9), MaximumCoveredRect(Rect(0, 0, 10, 10), Rect(9, 0, 10, 9)));
    // Otherwise choose the bigger one.
    EXPECT_EQ(Rect(20, 30, 40, 50), MaximumCoveredRect(Rect(10, 20, 30, 40), Rect(20, 30, 40, 50)));
    EXPECT_EQ(Rect(10, 20, 40, 50), MaximumCoveredRect(Rect(10, 20, 40, 50), Rect(20, 30, 30, 40)));
    EXPECT_EQ(Rect(10, 20, 40, 50), MaximumCoveredRect(Rect(10, 20, 40, 50), Rect(20, 30, 40, 50)));
}
