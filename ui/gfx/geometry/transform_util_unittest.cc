...
TEST(TransformUtilTest, GetScaleTransform)
{
    ...
    for (int sign_x = -1; sign_x <= 1; ++sign_x) {
        for (int sign_y = -1; sign_y <= 1; ++sign_y) {
            Point test = scale.MapPoint(Point(kAnchor.x() + sign_x * kOffset, kAnchor.y() + sign_y * kOffset));

            EXPECT_EQ(Point(kAnchor.x() + sign_x * kOffset * kScale, kAnchor.y() + sign_y * kOffset * kScale), test);
        }
    }
}
...
{
    DecomposedTransform a { { 2.5, -3.25, 4.75 }, { 4.5, -5.25, 6.75 }, { 1.25, -2.5, 3.75 }, { 5, -4, 3, -2 }, { -5, 6, -7, 8 } };
    DecomposedTransform b { { -2, 3, 4 }, { -4, 5, 6 }, { -1, 2, 3 }, { 6, 7, -8, -9 }, { 5, 4, -3, -2 } };
    DecomposedTransform expected { { 0.5, -0.25, 8.75 }, { -0.5, -1.25, 11.75 }, { 0.25, -0.5, 6.75 }, { 11, 3, -5, -12 }, { +60, -30, -60, -36 } };
    EXPECT_DECOMPOSED_TRANSFORM_EQ(expected, AccumulateDecomposedTransforms(a, b));
}
...
TEST(TransformUtilTest, TransformBetweenRects)
{
    ...
    std::vector<std::pair<const RectF, const RectF>> test_cases { { RectF(0.f, 0.f, 2.f, 3.f), RectF(3.f, 5.f, 4.f, 9.f) },
        { RectF(10.f, 7.f, 2.f, 6.f), RectF(4.f, 2.f, 1.f, 12.f) }, { RectF(0.f, 0.f, 3.f, 5.f), RectF(0.f, 0.f, 6.f, 2.5f) } };
...
TEST(TransformUtilTest, Transform2dScaleComponents)
{
    ...
    const float quiet_NaN_or_zero = std::numeric_limits<float>::has_quiet_NaN ? std::numeric_limits<float>::quiet_NaN() : 0;
    const float infinity_or_zero = std::numeric_limits<float>::has_infinity ? std::numeric_limits<float>::infinity() : 0;
    const float denorm_min_or_zero = (std::numeric_limits<float>::has_denorm == std::denorm_present) ? std::numeric_limits<float>::denorm_min() : 0;
    ...
    const struct {
        ...
        { Transform::RowMajor(3, 0, 0, -23, 0, 7, 0, 31, 0, 0, 11, 47, 0, 0, 0, 1), Vector2dF(3, 7) },
        // Matrices like the first, but also with various
        // perspective-altering components.
        { Transform::RowMajor(3, 0, 0, -23, 0, 7, 0, 31, 0, 0, 11, 47, 0, 0, -0.5, 1), Vector2dF(3, 7) },
        // The result is always non-negative.
        { Transform::RowMajor(3, 0, 0, -23, 0, -7, 0, 31, 0, 0, 11, 47, 0, 0, -0.5, 1), Vector2dF(3, 7) },
        // Values are clamped.
        { Transform::RowMajor(std::numeric_limits<double>::max(), 0, 0, -23, 0, std::numeric_limits<double>::lowest(), 0, 31, 0, 0, 11, 47, 0, 0, -0.5f, 1),
            Vector2dF(FloatGeometrySaturationHandler<float>::max(), FloatGeometrySaturationHandler<float>::max()) },
        { Transform::RowMajor(3, 0, 0, -23, 0, 7, 0, 31, 0, 0, 11, 47, 0.2f, 0, -0.5f, 1), std::nullopt },
        { Transform::RowMajor(3, 0, 0, -23, 0, 7, 0, 31, 0, 0, 11, 47, 0.2f, -0.2f, -0.5f, 1), std::nullopt },
        { Transform::RowMajor(3, 0, 0, -23, 0, 7, 0, 31, 0, 0, 11, 47, 0.2f, -0.2f, -0.5f, 1), std::nullopt },
        { Transform::RowMajor(3, 0, 0, -23, 0, 7, 0, 31, 0, 0, 11, 47, 0, -0.2f, -0.5f, 1), std::nullopt },
        { Transform::RowMajor(3, 0, 0, -23, 0, 7, 0, 31, 0, 0, 11, 47, 0, 0, -0.5f, 0.25f), Vector2dF(12, 28) },
        // Matrices like the first, but with some types of rotation.
        { Transform::RowMajor(0, 3, 0, -23, 7, 0, 0, 31, 0, 0, 11, 47, 0, 0, 0, 1), Vector2dF(7, 3) },
        { Transform::RowMajor(3, 8, 0, -23, 4, 6, 0, 31, 0, 0, 11, 47, 0, 0, 0, 1), Vector2dF(5, 10) },
        // Combination of rotation and perspective
        { Transform::RowMajor(3, 8, 0, -23, 4, 6, 0, 31, 0, 0, 11, 47, 0, 0, 0, 0.25f), Vector2dF(20, 40) },
        // Error handling cases for final perspective component.
        { Transform::RowMajor(3, 0, 0, -23, 0, 7, 0, 31, 0, 0, 11, 47, 0, 0, 0, 0), std::nullopt },
        { Transform::RowMajor(3, 0, 0, -23, 0, 7, 0, 31, 0, 0, 11, 47, 0, 0, 0, quiet_NaN_or_zero), std::nullopt },
        { Transform::RowMajor(3, 0, 0, -23, 0, 7, 0, 31, 0, 0, 11, 47, 0, 0, 0, infinity_or_zero), std::nullopt },
        { Transform::RowMajor(3, 0, 0, -23, 0, 7, 0, 31, 0, 0, 11, 47, 0, 0, 0, denorm_min_or_zero), std::nullopt },
        // clang-format on
    };
    ...
    for (const auto& test : tests) {
        std::optional<Vector2dF> try_result = TryComputeTransform2dScaleComponents(test.transform);
        SCOPED_TRACE(test.transform.ToString());
        EXPECT_EQ(try_result, test.expected_scale);
        Vector2dF result = ComputeTransform2dScaleComponents(test.transform, fallback);
