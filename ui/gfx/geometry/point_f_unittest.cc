...
TEST(PointFTest, IsWithinDistance)
{
    ...
    pt = PointF(-10.f, -10.f);
    EXPECT_FALSE(pt.IsWithinDistance(PointF(10.f, 10.f), /*allowed_distance=*/10.f));
    EXPECT_TRUE(pt.IsWithinDistance(PointF(-9.9988f, -10.0013f), 0.0017689f));

    pt = PointF(std::numeric_limits<float>::max(), std::numeric_limits<float>::max());
    EXPECT_FALSE(pt.IsWithinDistance(PointF(std::numeric_limits<float>::min(), std::numeric_limits<float>::min()), 100.f));
}
