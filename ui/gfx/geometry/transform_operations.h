...
public:
    ...
    TransformOperations Blend(const TransformOperations& from, SkScalar progress) const;
    ...
    bool BlendedBoundsForBox(const BoxF& box, const TransformOperations& from, SkScalar min_progress, SkScalar max_progress, BoxF* bounds) const;
    ...
    bool ApproximatelyEqual(const TransformOperations& other, SkScalar tolerance) const;

private:
    FRIEND_TEST_ALL_PREFIXES(TransformOperationsTest, TestDecompositionCache);

    bool BlendInternal(const TransformOperations& from, SkScalar progress, TransformOperations* result) const;
    ...
    mutable std::unordered_map<size_t, std::unique_ptr<DecomposedTransform>> decomposed_transforms_;
};
