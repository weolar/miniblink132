...
TEST(SkiaConversionsTest, SkIRectToRectClamping)
{
    ...
    // can represent.
    Rect result = SkIRectToRect(SkIRect::MakeLTRB(Limits::min(), Limits::min(), Limits::max(), Limits::max()));
    ...
    result = SkIRectToRect(SkIRect::MakeLTRB(Limits::max(), Limits::max(), Limits::min(), Limits::min()));
