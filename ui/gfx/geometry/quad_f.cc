...
bool LineIntersectsCircle(const PointF& center, float radius, const PointF& p0, const PointF& p1)
{
...
{
    return base::StringPrintf("%s;%s;%s;%s", p1_.ToString().c_str(), p2_.ToString().c_str(), p3_.ToString().c_str(), p4_.ToString().c_str());
}
...
{
    return (WithinEpsilon(p1_.x(), p2_.x()) && WithinEpsilon(p2_.y(), p3_.y()) && WithinEpsilon(p3_.x(), p4_.x()) && WithinEpsilon(p4_.y(), p1_.y()))
        || (WithinEpsilon(p1_.y(), p2_.y()) && WithinEpsilon(p2_.x(), p3_.x()) && WithinEpsilon(p3_.y(), p4_.y()) && WithinEpsilon(p4_.x(), p1_.x()));
}
...
{
    return PointIsInTriangle(point, p1_, p2_, p3_) || PointIsInTriangle(point, p1_, p3_, p4_);
}

bool QuadF::ContainsQuad(const QuadF& other) const
{
    return Contains(other.p1()) && Contains(other.p2()) && Contains(other.p3()) && Contains(other.p4());
}
...
bool QuadF::IsToTheLeftOfOrTouchingLine(const PointF& base, const Vector2dF& vector) const
{
...
{
    return Contains(center) || LineIntersectsCircle(center, radius, p1_, p2_) || LineIntersectsCircle(center, radius, p2_, p3_)
        || LineIntersectsCircle(center, radius, p3_, p4_) || LineIntersectsCircle(center, radius, p4_, p1_);
}
