...
public:
    ...
    MaskFilterInfo(const RectF& bounds, const RoundedCornersF& radii, const gfx::LinearGradient& gradient_mask)
    ...
    RectF bounds() const
    {
        return rounded_corner_bounds_.rect();
    }

    // Defines the rounded corner bounds to clip.
    const RRectF& rounded_corner_bounds() const
    {
        return rounded_corner_bounds_;
    }
...
{
    return (lhs.rounded_corner_bounds() == rhs.rounded_corner_bounds()) && (lhs.gradient_mask() == rhs.gradient_mask());
}
