...
static bool ShareSameAxis(const TransformOperation* from, bool is_identity_from, const TransformOperation* to, bool is_identity_to, SkScalar* axis_x,
    SkScalar* axis_y, SkScalar* axis_z, SkScalar* angle_from)
{
    ...
    SkScalar length_2 = from->rotate.axis.x * from->rotate.axis.x + from->rotate.axis.y * from->rotate.axis.y + from->rotate.axis.z * from->rotate.axis.z;
    SkScalar other_length_2 = to->rotate.axis.x * to->rotate.axis.x + to->rotate.axis.y * to->rotate.axis.y + to->rotate.axis.z * to->rotate.axis.z;

    if (length_2 <= kAngleEpsilon || other_length_2 <= kAngleEpsilon)
        return false;

    SkScalar dot = to->rotate.axis.x * from->rotate.axis.x + to->rotate.axis.y * from->rotate.axis.y + to->rotate.axis.z * from->rotate.axis.z;
    SkScalar error = SkScalarAbs(SK_Scalar1 - (dot * dot) / (length_2 * other_length_2));
...
void TransformOperation::Bake()
{
    ...
    case TransformOperation::TRANSFORM_OPERATION_ROTATE:
        matrix.RotateAbout(gfx::Vector3dF(rotate.axis.x, rotate.axis.y, rotate.axis.z), rotate.angle);
        break;
...
bool TransformOperation::ApproximatelyEqual(const TransformOperation& other, SkScalar tolerance) const
{
    ...
    switch (type) {
    case TransformOperation::TRANSFORM_OPERATION_TRANSLATE:
        return base::IsApproximatelyEqual(translate.x, other.translate.x, tolerance) && base::IsApproximatelyEqual(translate.y, other.translate.y, tolerance)
            && base::IsApproximatelyEqual(translate.z, other.translate.z, tolerance);
    case TransformOperation::TRANSFORM_OPERATION_ROTATE:
        return base::IsApproximatelyEqual(rotate.axis.x, other.rotate.axis.x, tolerance)
            && base::IsApproximatelyEqual(rotate.axis.y, other.rotate.axis.y, tolerance)
            && base::IsApproximatelyEqual(rotate.axis.z, other.rotate.axis.z, tolerance)
            && base::IsApproximatelyEqual(rotate.angle, other.rotate.angle, tolerance);
    case TransformOperation::TRANSFORM_OPERATION_SCALE:
        return base::IsApproximatelyEqual(scale.x, other.scale.x, tolerance) && base::IsApproximatelyEqual(scale.y, other.scale.y, tolerance)
            && base::IsApproximatelyEqual(scale.z, other.scale.z, tolerance);
    case TransformOperation::TRANSFORM_OPERATION_SKEWX:
    case TransformOperation::TRANSFORM_OPERATION_SKEWY:
    case TransformOperation::TRANSFORM_OPERATION_SKEW:
        return base::IsApproximatelyEqual(skew.x, other.skew.x, tolerance) && base::IsApproximatelyEqual(skew.y, other.skew.y, tolerance);
    case TransformOperation::TRANSFORM_OPERATION_PERSPECTIVE:
        return base::IsApproximatelyEqual(perspective_m43, other.perspective_m43, tolerance);
    case TransformOperation::TRANSFORM_OPERATION_MATRIX:
...
bool TransformOperation::BlendTransformOperations(const TransformOperation* from, const TransformOperation* to, SkScalar progress, TransformOperation* result)
{
    ...
    case TransformOperation::TRANSFORM_OPERATION_TRANSLATE: {
        ...
        result->translate.x = BlendSkScalars(from_x, to_x, progress), result->translate.y = BlendSkScalars(from_y, to_y, progress),
        result->translate.z = BlendSkScalars(from_z, to_z, progress), result->Bake();
        break;
    }
    case TransformOperation::TRANSFORM_OPERATION_ROTATE: {
        SkScalar axis_x = 0;
        ...
        if (ShareSameAxis(from, is_identity_from, to, is_identity_to, &axis_x, &axis_y, &axis_z, &from_angle)) {
    ...
    case TransformOperation::TRANSFORM_OPERATION_PERSPECTIVE: {
        ...
        result->perspective_m43 = std::clamp(BlendSkScalars(from_perspective_m43, to_perspective_m43, progress), -1.0f, 0.0f);
        result->Bake();
        break;
    }
    case TransformOperation::TRANSFORM_OPERATION_MATRIX: {
...
static void FindCandidatesInPlane(float px, float py, float nz, double* candidates, int* num_candidates)
{
...
static void BoundingBoxForArc(
    const gfx::Point3F& point, const TransformOperation* from, const TransformOperation* to, SkScalar min_progress, SkScalar max_progress, gfx::BoxF* box)
{
    const TransformOperation* exemplar = from ? from : to;
    gfx::Vector3dF axis(exemplar->rotate.axis.x, exemplar->rotate.axis.y, exemplar->rotate.axis.z);
    ...
    // correspond to |from|.
    if (from && to) {
        gfx::Vector3dF other_axis(to->rotate.axis.x, to->rotate.axis.y, to->rotate.axis.z);
        if (gfx::DotProduct(axis, other_axis) < 0.f)
            to_angle *= -1.f;
    }

    float min_degrees = SkScalarToFloat(BlendSkScalars(from_angle, to_angle, min_progress));
    float max_degrees = SkScalarToFloat(BlendSkScalars(from_angle, to_angle, max_progress));
...
bool TransformOperation::BlendedBoundsForBox(
    const gfx::BoxF& box, const TransformOperation* from, const TransformOperation* to, SkScalar min_progress, SkScalar max_progress, gfx::BoxF* bounds)
{
    bool is_identity_from = IsOperationIdentity(from);
    bool is_identity_to = IsOperationIdentity(to);
    if (is_identity_from && is_identity_to) {
    ...
    case TransformOperation::TRANSFORM_OPERATION_SCALE: {
        ...
        if (!BlendTransformOperations(from, to, min_progress, &from_operation) || !BlendTransformOperations(from, to, max_progress, &to_operation))
            return false;
    ...
    case TransformOperation::TRANSFORM_OPERATION_ROTATE: {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            corner += gfx::Vector3dF(i & 1 ? box.width() : 0.f, i & 2 ? box.height() : 0.f, i & 4 ? box.depth() : 0.f);
