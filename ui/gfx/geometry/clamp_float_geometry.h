...
template <typename T> struct FloatGeometrySaturationHandler {
    static constexpr float NaN()
    {
        return 0;
    }
    static constexpr float Overflow()
    {
        return max();
    }
    static constexpr float Underflow()
    {
        return lowest();
    }
...
template <typename T> constexpr float ClampFloatGeometry(T value)
{
