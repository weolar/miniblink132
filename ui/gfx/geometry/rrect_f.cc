...
RRectF::RRectF(float x, float y, float width, float height, float x_rad, float y_rad)
    : skrrect_(SkRRect::MakeRectXY(SkRect::MakeXYWH(x, y, width, height), x_rad, y_rad))
{
    if (IsEmpty()) {
...
RRectF::RRectF(float x, float y, float width, float height, float upper_left_x, float upper_left_y, float upper_right_x, float upper_right_y,
    float lower_right_x, float lower_right_y, float lower_left_x, float lower_left_y)
{
    SkVector radii[4] = {
...
void RRectF::GetAllRadii(SkVector radii[4]) const
{
    ...
    radii[SkRRect::kUpperLeft_Corner] = skrrect_.radii(SkRRect::kUpperLeft_Corner);
    radii[SkRRect::kUpperRight_Corner] = skrrect_.radii(SkRRect::kUpperRight_Corner);
    radii[SkRRect::kLowerRight_Corner] = skrrect_.radii(SkRRect::kLowerRight_Corner);
    radii[SkRRect::kLowerLeft_Corner] = skrrect_.radii(SkRRect::kLowerLeft_Corner);
}
...
gfx::RectF RRectF::CornerBoundingRect(Corner corner) const
{
    ...
    case Corner::kLowerRight:
        bounding_box.Offset(rect().right() - radii.x(), rect().bottom() - radii.y());
        break;
...
std::string RRectF::ToString() const
{
    ...
    ss << rect().origin().x() << "," << rect().origin().y() << " " << rect().size().width() << "x" << rect().size().height();
...
{
    if (AboveTol(skrrect_.rect().x(), rect.skrrect_.rect().x(), tolerance) || AboveTol(skrrect_.rect().y(), rect.skrrect_.rect().y(), tolerance)
        || AboveTol(skrrect_.width(), rect.skrrect_.width(), tolerance) || AboveTol(skrrect_.height(), rect.skrrect_.height(), tolerance))
        return false;
    for (int i = 0; i < 4; i++) {
        SkVector r1 = skrrect_.radii(SkRRect::Corner(i));
        SkVector r2 = rect.skrrect_.radii(SkRRect::Corner(i));
        if (std::abs(r1.x() - r2.x()) > tolerance || std::abs(r1.y() - r2.y()) > tolerance) {
            return false;
        }
    }
    return true;
...
{
    return RRectF(gfx::RectF(ToEnclosingRect(rrect_f.rect())), rrect_f.GetRoundedCorners());
}

// static
RRectF RRectF::ToEnclosingRRectFIgnoringError(const RRectF& rrect_f, float error)
{
    return RRectF(gfx::RectF(ToEnclosingRectIgnoringError(rrect_f.rect(), error)), rrect_f.GetRoundedCorners());
}
