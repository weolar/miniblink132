...
void Vector3dF::InvScale(float inv_x_scale, float inv_y_scale, float inv_z_scale)
{
    x_ /= inv_x_scale;
    y_ /= inv_y_scale;
    z_ /= inv_z_scale;
...
Vector3dF ScaleVector3d(const Vector3dF& v, float x_scale, float y_scale, float z_scale)
{
...
float AngleBetweenVectorsInDegrees(const gfx::Vector3dF& base, const gfx::Vector3dF& other)
{
    // Clamp the resulting value to prevent potential NANs from floating point
    // precision issues.
    return base::RadToDeg(std::acos(fmax(fmin(gfx::DotProduct(base, other) / base.Length() / other.Length(), 1.f), -1.f)));
}

float ClockwiseAngleBetweenVectorsInDegrees(const gfx::Vector3dF& base, const gfx::Vector3dF& other, const gfx::Vector3dF& normal)
{
