...
{
    return SkRect::MakeLTRB(SkIntToScalar(rect.x()), SkIntToScalar(rect.y()), SkIntToScalar(rect.right()), SkIntToScalar(rect.bottom()));
}
...
{
    return SkRect::MakeLTRB(SkFloatToScalar(rect.x()), SkFloatToScalar(rect.y()), SkFloatToScalar(rect.right()), SkFloatToScalar(rect.bottom()));
}

RectF SkRectToRectF(const SkRect& rect)
{
    return BoundingRect(
        PointF(SkScalarToFloat(rect.left()), SkScalarToFloat(rect.top())), PointF(SkScalarToFloat(rect.right()), SkScalarToFloat(rect.bottom())));
}

SkSize SizeFToSkSize(const SizeF& size)
{
    return SkSize::Make(SkFloatToScalar(size.width()), SkFloatToScalar(size.height()));
}
...
{
    return SkMatrix::MakeAll(transform.scale().x(), 0, transform.translation().x(), // row 0
...
SkM44 TransformToSkM44(const Transform& matrix)
{
    ...
    return SkM44(matrix.rc(0, 0), matrix.rc(0, 1), matrix.rc(0, 2), matrix.rc(0, 3), matrix.rc(1, 0), matrix.rc(1, 1), matrix.rc(1, 2), matrix.rc(1, 3),
...
{
    return Transform::RowMajor(matrix.rc(0, 0), matrix.rc(0, 1), matrix.rc(0, 2), matrix.rc(0, 3), matrix.rc(1, 0), matrix.rc(1, 1), matrix.rc(1, 2),
        matrix.rc(1, 3), matrix.rc(2, 0), matrix.rc(2, 1), matrix.rc(2, 2), matrix.rc(2, 3), matrix.rc(3, 0), matrix.rc(3, 1), matrix.rc(3, 2),
        matrix.rc(3, 3));
}

// TODO(crbug.com/40237414): Remove this function in favor of the other form.
void TransformToFlattenedSkMatrix(const gfx::Transform& transform, SkMatrix* flattened)
{
...
SkMatrix TransformToFlattenedSkMatrix(const Transform& matrix)
{
    ...
    return SkMatrix::MakeAll(matrix.rc(0, 0), matrix.rc(0, 1), matrix.rc(0, 3), matrix.rc(1, 0), matrix.rc(1, 1), matrix.rc(1, 3), matrix.rc(3, 0),
        matrix.rc(3, 1), matrix.rc(3, 3));
}

Transform SkMatrixToTransform(const SkMatrix& matrix)
{
    return Transform::RowMajor(matrix.rc(0, 0), matrix.rc(0, 1), 0, matrix.rc(0, 2), // row 0
