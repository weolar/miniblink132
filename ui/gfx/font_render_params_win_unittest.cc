...
TEST_F(FontRenderParamsTest, SystemFontSettingsDisabled)
{
    ...
    scoped_features.InitWithFeatures({}, { features::kIncreaseWindowsTextContrast });
...
TEST_F(FontRenderParamsTest, DefaultRegistryState)
{
    ...
    // TODO(crbug.com/40037626)
    // Disable IncreaseWindowsTextContrast until SK_GAMMA_CONTRAST is set to 1.0f.
    scoped_features.InitWithFeatures({ features::kUseGammaContrastRegistrySettings }, { features::kIncreaseWindowsTextContrast });
    ...
    if (key.Valid()) {
        DWORD contrast;
        ASSERT_EQ(key.ReadValueDW(L"EnhancedContrastLevel", &contrast), ERROR_SUCCESS);
        DWORD gamma;
        ...
        EXPECT_FLOAT_EQ(params.text_contrast * kContrastMultiplier, FontUtilWin::ClampContrast(contrast));
        EXPECT_FLOAT_EQ(params.text_gamma * kGammaMultiplier, FontUtilWin::ClampGamma(contrast));
    } else {
...
TEST_F(FontRenderParamsTest, OverrideRegistryValues)
{
    ...
    base::test::ScopedFeatureList scoped_features(features::kUseGammaContrastRegistrySettings);

    // Override the registry to maintain test machine state.
    ASSERT_NO_FATAL_FAILURE(registry_override_manager_.OverrideRegistry(HKEY_CURRENT_USER));
    ...
    if (key.Valid()) {
        ...
        DWORD contrast = 75;
        ASSERT_EQ(key.WriteValue(L"EnhancedContrastLevel", contrast), ERROR_SUCCESS);
        DWORD gamma = 1900;
        ...
        EXPECT_FLOAT_EQ(FontUtilWin::GetContrastFromRegistry() * kContrastMultiplier, contrast);
        EXPECT_FLOAT_EQ(FontUtilWin::GetGammaFromRegistry() * kGammaMultiplier, gamma);
    }
}
...
TEST_F(FontRenderParamsTest, OverrideRegistryValuesAndIncreaseContrast)
{
    ...
    // flag.
    base::test::ScopedFeatureList scoped_features;
    scoped_features.InitWithFeatures({ features::kUseGammaContrastRegistrySettings }, {});
