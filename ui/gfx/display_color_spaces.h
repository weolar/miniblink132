...
public:
    ...
    void SetOutputColorSpaceAndBufferFormat(
    ...
    void SetOutputBufferFormats(gfx::BufferFormat buffer_format_no_alpha, gfx::BufferFormat buffer_format_with_alpha);

    // Retrieve parameters for a specific usage and alpha.
    ColorSpace GetOutputColorSpace(ContentColorUsage color_usage, bool needs_alpha) const;
    BufferFormat GetOutputBufferFormat(ContentColorUsage color_usage, bool needs_alpha) const;
    ...
    float GetSDRMaxLuminanceNits() const
    {
        return sdr_max_luminance_nits_;
    }
    ...
    // output.
    gfx::ColorSpace GetCompositingColorSpace(bool needs_alpha, ContentColorUsage color_usage) const;

    // Return true if the HDR color spaces are, indeed, HDR.
    bool SupportsHDR() const;

    // Return the primaries that define the color gamut of the display.
    const SkColorSpacePrimaries& GetPrimaries() const
    {
        return primaries_;
    }
    ...
    static bool EqualExceptForHdrHeadroom(const DisplayColorSpaces& a, const DisplayColorSpaces& b);

private:
    // Serialization of DisplayColorSpaces directly accesses members.
    friend struct IPC::ParamTraits<gfx::DisplayColorSpaces>;
    friend struct mojo::StructTraits<gfx::mojom::DisplayColorSpacesDataView, gfx::DisplayColorSpaces>;
