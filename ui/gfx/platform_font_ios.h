...
public:
    ...
    PlatformFontIOS(const std::string& font_name, int font_size);
    ...
    PlatformFontIOS(sk_sp<SkTypeface> typeface, int font_size_pixels, const std::optional<FontRenderParams>& params);
#endif
...
private:
    PlatformFontIOS(const std::string& font_name, int font_size, int style, Font::Weight weight);
    ~PlatformFontIOS() override = default;

    // Initialize the object with the specified parameters.
    void InitWithNameSizeAndStyle(const std::string& font_name, int font_size, int style, Font::Weight weight);
