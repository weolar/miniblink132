...
public:
    ...
    void SetFontSettings(const FontSettings& font_settings, const FontRenderParams& params)
    {
...
TEST(PlatformFontSkiaRenderParamsTest, DefaultFontRenderParams)
{
    ...
    scoped_refptr<PlatformFontSkia> named_font(new PlatformFontSkia(default_font->GetFontName(), default_font->GetFontSize()));

    // Ensures that both constructors are producing fonts with the same render
    // params.
    EXPECT_EQ(default_font->GetFontRenderParams(), named_font->GetFontRenderParams());
}
...
TEST(PlatformFontSkiaOnWindowsTest, SystemFont)
{
    ...
    EXPECT_EQ(system_font.GetBaseline(), default_font.GetBaseline());
    EXPECT_EQ(system_font.GetFontRenderParams(), default_font.GetFontRenderParams());
}
