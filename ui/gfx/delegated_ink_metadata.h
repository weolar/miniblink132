...
public:
    ...
    DelegatedInkMetadata(const PointF& pt, double diameter, SkColor color, base::TimeTicks timestamp, const RectF& area, bool hovering)
        : point_(pt)
        , diameter_(diameter)
        , color_(color)
    ...
    DelegatedInkMetadata(const PointF& pt, double diameter, SkColor color, base::TimeTicks timestamp, const RectF& area, base::TimeTicks frame_time,
        bool hovering, std::uint64_t render_pass_id)
        : point_(pt)
        , diameter_(diameter)
        , color_(color)
        , timestamp_(timestamp)
    ...
    const RectF& presentation_area() const
    {
        return presentation_area_;
    }
    base::TimeTicks frame_time() const
    {
        return frame_time_;
    }
    std::uint64_t render_pass_id() const
    {
        return render_pass_id_;
    }
    ...
    void set_frame_time(base::TimeTicks frame_time)
    {
        frame_time_ = frame_time;
    }
    uint64_t trace_id() const
    {
        ...
        return static_cast<uint64_t>(timestamp_.since_origin().InMicroseconds()) | (uint64_t { 1 } << 63);
    }
