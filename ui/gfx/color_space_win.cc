...
DXVA2_ExtendedFormat ColorSpaceWin::GetExtendedFormat(const ColorSpace& color_space)
{
...
DXGI_COLOR_SPACE_TYPE ColorSpaceWin::GetDXGIColorSpace(const ColorSpace& color_space, bool force_yuv)
{
    ...
    if (color_space.GetMatrixID() == gfx::ColorSpace::MatrixID::RGB && !force_yuv) {
        ...
        } else if (color_space.GetPrimaryID() == gfx::ColorSpace::PrimaryID::BT470BG || color_space.GetPrimaryID() == gfx::ColorSpace::PrimaryID::SMPTE170M) {
            // For YUV, we default to LIMITED
            switch (color_space.GetRangeID()) {
            case gfx::ColorSpace::RangeID::FULL:
...
D3D11_VIDEO_PROCESSOR_COLOR_SPACE ColorSpaceWin::GetD3D11ColorSpace(const ColorSpace& color_space)
{
