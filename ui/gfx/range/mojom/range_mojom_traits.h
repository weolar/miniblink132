...
template <> struct StructTraits<gfx::mojom::RangeDataView, gfx::Range> {
...
template <> struct StructTraits<gfx::mojom::RangeFDataView, gfx::RangeF> {
    static float start(const gfx::RangeF& r)
    {
        return r.start();
    }
    static float end(const gfx::RangeF& r)
    {
        return r.end();
    }
