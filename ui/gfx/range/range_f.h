...
public:
    ...
    constexpr RangeF(float start, float end)
        : start_(start)
        , end_(end)
    {
    }
    ...
    constexpr explicit RangeF(float position)
        : RangeF(position, position)
    {
    }
    ...
    constexpr RangeF Intersect(const RangeF& range) const
    {
        ...
        return (min < max || Contains(range) || range.Contains(*this)) ? RangeF(min, max) : InvalidRange();
    }
...
GFX_RANGE_EXPORT std::ostream& operator<<(std::ostream& os, const RangeF& range);
