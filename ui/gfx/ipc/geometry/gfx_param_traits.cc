...
bool ParamTraits<gfx::Point>::Read(const base::Pickle* m, base::PickleIterator* iter, gfx::Point* r)
{
    int x, y;
...
bool ParamTraits<gfx::PointF>::Read(const base::Pickle* m, base::PickleIterator* iter, gfx::PointF* r)
{
    float x, y;
    if (!ReadParam(m, iter, &x) || !ReadParam(m, iter, &y))
        return false;
...
bool ParamTraits<gfx::Point3F>::Read(const base::Pickle* m, base::PickleIterator* iter, gfx::Point3F* r)
{
    float x, y, z;
...
bool ParamTraits<gfx::Size>::Read(const base::Pickle* m, base::PickleIterator* iter, gfx::Size* r)
{
...
bool ParamTraits<gfx::SizeF>::Read(const base::Pickle* m, base::PickleIterator* iter, gfx::SizeF* r)
{
    const char* char_values;
...
void ParamTraits<gfx::Vector2d>::Write(base::Pickle* m, const gfx::Vector2d& p)
{
    int values[2] = { p.x(), p.y() };
    m->WriteBytes(&values, sizeof(int) * 2);
}

bool ParamTraits<gfx::Vector2d>::Read(const base::Pickle* m, base::PickleIterator* iter, gfx::Vector2d* r)
{
    const char* char_values;
    if (!iter->ReadBytes(&char_values, sizeof(int) * 2))
        return false;
...
void ParamTraits<gfx::Vector2dF>::Write(base::Pickle* m, const gfx::Vector2dF& p)
{
    float values[2] = { p.x(), p.y() };
    m->WriteBytes(&values, sizeof(float) * 2);
}

bool ParamTraits<gfx::Vector2dF>::Read(const base::Pickle* m, base::PickleIterator* iter, gfx::Vector2dF* r)
{
    const char* char_values;
    if (!iter->ReadBytes(&char_values, sizeof(float) * 2))
        return false;
...
bool ParamTraits<gfx::Rect>::Read(const base::Pickle* m, base::PickleIterator* iter, gfx::Rect* r)
{
    const char* char_values;
...
{
    l->append(base::StringPrintf("(%d, %d, %d, %d)", p.x(), p.y(), p.width(), p.height()));
}
...
bool ParamTraits<gfx::RectF>::Read(const base::Pickle* m, base::PickleIterator* iter, gfx::RectF* r)
{
    const char* char_values;
...
{
    l->append(base::StringPrintf("(%f, %f, %f, %f)", p.x(), p.y(), p.width(), p.height()));
}
