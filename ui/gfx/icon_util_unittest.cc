...
public:
    ...
    ScopedHICON LoadIconFromFile(const base::FilePath& filename, int width, int height)
    {
        HICON icon = static_cast<HICON>(LoadImage(NULL, filename.value().c_str(), IMAGE_ICON, width, height, LR_LOADTRANSPARENT | LR_LOADFROMFILE));
    ...
    void CheckAllIconSizes(const base::FilePath& icon_filename, int max_icon_size);
...
void IconUtilTest::CheckAllIconSizes(const base::FilePath& icon_filename, int max_icon_size)
{
    ...
    EXPECT_TRUE(LoadIconFromFile(icon_filename, kSmallIconWidth, kSmallIconHeight).is_valid());
    ...
    ASSERT_GE(icon_data.length(), sizeof(IconUtil::ICONDIR) + icon_dir->idCount * sizeof(IconUtil::ICONDIRENTRY));
    ...
    if (max_icon_size >= 256) {
        ...
        ASSERT_GE(icon_data.length(), png_entry->dwImageOffset + png_entry->dwBytesInRes);
        gfx::Image image = gfx::Image::CreateFrom1xPNGBytes(base::as_byte_span(icon_data).subspan(png_entry->dwImageOffset, png_entry->dwBytesInRes));
...
TEST_F(IconUtilTest, TestIconToBitmapInvalidParameters)
{
    ...
    ScopedHICON icon(LoadIconFromFile(icon_filename, icon_size.width(), icon_size.height()));
    ASSERT_TRUE(icon.is_valid());

    // Invalid size parameter.
    gfx::Size invalid_icon_size(kSmallIconHeight, 0);
    EXPECT_TRUE(IconUtil::CreateSkBitmapFromHICON(icon.get(), invalid_icon_size).isNull());

    // Invalid icon.
    EXPECT_TRUE(IconUtil::CreateSkBitmapFromHICON(nullptr, icon_size).isNull());

    // The following code should succeed.
    EXPECT_FALSE(IconUtil::CreateSkBitmapFromHICON(icon.get(), icon_size).drawsNothing());
}
...
TEST_F(IconUtilTest, TestBitmapToIconInvalidParameters)
{
    ...
    bitmap = std::make_unique<SkBitmap>();
    ASSERT_NE(bitmap.get(), static_cast<SkBitmap*>(NULL));
    bitmap->setInfo(SkImageInfo::MakeN32Premul(kSmallIconWidth, kSmallIconHeight));
    icon = IconUtil::CreateHICONFromSkBitmap(*bitmap);
    EXPECT_FALSE(icon.is_valid());
}
...
TEST_F(IconUtilTest, TestCreateIconFileInvalidParameters)
{
    ...
    base::FilePath valid_icon_filename = temp_directory_.GetPath().AppendASCII(kTempIconFilename);
    base::FilePath invalid_icon_filename = temp_directory_.GetPath().AppendASCII("<>?.ico");

    // Invalid file name.
    image_family.clear();
    image_family.Add(gfx::test::CreateImage(/*size=*/1));
    EXPECT_FALSE(IconUtil::CreateIconFileFromImageFamily(image_family, invalid_icon_filename));
...
{
    base::FilePath icon_filename = temp_directory_.GetPath().AppendASCII(kTempIconFilename);

    // Empty image family.
    EXPECT_FALSE(IconUtil::CreateIconFileFromImageFamily(gfx::ImageFamily(), icon_filename));
    EXPECT_FALSE(base::PathExists(icon_filename));

    // Image family with only an empty image.
    gfx::ImageFamily image_family;
    image_family.Add(gfx::Image());
    EXPECT_FALSE(IconUtil::CreateIconFileFromImageFamily(image_family, icon_filename));
    EXPECT_FALSE(base::PathExists(icon_filename));
}
...
{
    base::FilePath small_icon_filename = test_data_dir_.AppendASCII(kSmallIconName);
    gfx::Size small_icon_size(kSmallIconWidth, kSmallIconHeight);
    ScopedHICON small_icon(LoadIconFromFile(small_icon_filename, small_icon_size.width(), small_icon_size.height()));
    ASSERT_TRUE(small_icon.is_valid());
    SkBitmap bitmap = IconUtil::CreateSkBitmapFromHICON(small_icon.get(), small_icon_size);
    ...
    base::FilePath large_icon_filename = test_data_dir_.AppendASCII(kLargeIconName);
    gfx::Size large_icon_size(kLargeIconWidth, kLargeIconHeight);
    ScopedHICON large_icon(LoadIconFromFile(large_icon_filename, large_icon_size.width(), large_icon_size.height()));
...
TEST_F(IconUtilTest, TestBasicCreateHICONFromSkBitmap)
{
    ...
    HDC hdc = ::GetDC(NULL);
    int result = ::GetDIBits(hdc, icon_info.hbmColor, 0, kSmallIconWidth, NULL, &bitmap_info, DIB_RGB_COLORS);
    ASSERT_GT(result, 0);
...
TEST_F(IconUtilTest, TestCreateIconFileFromImageFamily)
{
    ...
    image_family.Add(gfx::Image::CreateFrom1xBitmap(CreateBlackSkBitmap(kSmallIconWidth, kSmallIconHeight)));
    ASSERT_TRUE(IconUtil::CreateIconFileFromImageFamily(image_family, icon_filename));
    ...
    image_family.Add(gfx::Image::CreateFrom1xBitmap(CreateBlackSkBitmap(256, 256)));
    ASSERT_TRUE(IconUtil::CreateIconFileFromImageFamily(image_family, icon_filename));
    CheckAllIconSizes(icon_filename, 256);
...
TEST_F(IconUtilTest, TestCreateImageFamilyFromIconResource)
{
    ...
    std::unique_ptr<gfx::ImageFamily> family(IconUtil::CreateImageFamilyFromIconResource(module, IDR_MAINFRAME));
...
{
    ASSERT_LE(IconUtil::kNumIconDimensionsUpToMediumSize, IconUtil::kNumIconDimensions);
    EXPECT_EQ(IconUtil::kMediumIconSize, IconUtil::kIconDimensions[IconUtil::kNumIconDimensionsUpToMediumSize - 1]);
}
...
TEST_F(IconUtilTest, TestTransparentIcon)
{
    ...
    ASSERT_TRUE(icon.is_valid());
    SkBitmap bitmap_loaded = IconUtil::CreateSkBitmapFromHICON(icon.get(), gfx::Size(size, size));
