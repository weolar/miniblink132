...
struct PresentationFeedback {
    ...
    PresentationFeedback(base::TimeTicks timestamp, base::TimeDelta interval, uint32_t flags)
        : timestamp(timestamp)
        , interval(interval)
        , flags(flags)
    {
...
inline bool operator==(const PresentationFeedback& lhs, const PresentationFeedback& rhs)
{
    return lhs.timestamp == rhs.timestamp && lhs.interval == rhs.interval && lhs.flags == rhs.flags && lhs.available_timestamp == rhs.available_timestamp
        && lhs.ready_timestamp == rhs.ready_timestamp && lhs.latch_timestamp == rhs.latch_timestamp &&
#if BUILDFLAG(IS_APPLE)
