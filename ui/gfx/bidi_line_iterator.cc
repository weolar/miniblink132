...
bool BiDiLineIterator::Open(const std::u16string& text, base::i18n::TextDirection direction)
{
    DCHECK(!bidi_);
    ...
    ubidi_setPara(bidi_.get(), text.data(), static_cast<int>(text.length()), GetParagraphLevelForDirection(direction), nullptr, &error);
...
UBiDiDirection BiDiLineIterator::GetVisualRun(int index, int* start, int* length) const
{
    DCHECK(bidi_ != nullptr);
    return ubidi_getVisualRun(bidi_.get(), index, start, length);
}

void BiDiLineIterator::GetLogicalRun(int start, int* end, UBiDiLevel* level) const
{
    DCHECK(bidi_ != nullptr);
