...
public:
    ...
    const base::FilePath& font_path() const
    {
        return font_path_;
    }
    int ttc_index() const
    ...
    {
        return std::tie(ttc_index_, font_path_) < std::tie(other.ttc_index_, other.font_path_);
    }
...
// these requests.
sk_sp<SkTypeface> GetSkTypefaceFromPathAndIndex(const base::FilePath& font_path, int ttc_index)
{
    ...
    sk_sp<SkTypeface> typeface = font_mgr->makeFromFile(filename.c_str(), ttc_index);
    (*cache)[key] = typeface;
...
public:
    FallbackFontEntry(const base::FilePath& font_path, int ttc_index, FontRenderParams font_params, FcCharSet* charset)
        : font_path_(font_path)
        , ttc_index_(ttc_index)
    ...
    FontRenderParams font_params() const
    {
        return font_params_;
    }
...
using FallbackFontEntriesCache = base::LRUCache<FallbackFontKey, FallbackFontEntries>;
...
FallbackFontEntriesCache* GetFallbackFontEntriesCacheInstance()
{
    ...
    static base::NoDestructor<FallbackFontEntriesCache> cache(kFallbackFontCacheSize);
    return cache.get();
}
...
FallbackFontListCache* GetFallbackFontListCacheInstance()
{
    ...
    static base::NoDestructor<FallbackFontListCache> fallback_cache(kFallbackCacheSize);
...
bool GetFallbackFont(const Font& font, const std::string& locale, std::u16string_view text, Font* result)
{
    ...
    if (cache_entry == cache->end()) {
        ...
        FcPatternAddString(pattern.get(), FC_FAMILY, reinterpret_cast<const FcChar8*>(font_family.c_str()));
        ...
        if (fonts) {
            ...
            for (int i = 0; i < fonts->nfont; ++i) {
                ...
                fallback_font_entries.push_back(FallbackFontEntry(font_path, font_ttc_index, font_params, char_set));
            }
    ...
    sk_sp<SkTypeface> typeface = GetSkTypefaceFromPathAndIndex(prefered_entry->font_path(), prefered_entry->ttc_index());
    // The file can't be parsed (e.g. corrupt). This font can't be used as a
    // fallback font.
    if (!typeface)
        return false;

    Font fallback_font(PlatformFont::CreateFromSkTypeface(typeface, font.GetFontSize(), prefered_entry->font_params()));
...
std::vector<Font> GetFallbackFonts(const Font& font)
{
    ...
    FcPatternAddString(pattern, FC_FAMILY, reinterpret_cast<const FcChar8*>(font_family.c_str()));
...
public:
    ...
    const FallbackFontData& fallback_font() const
    {
...
public:
    ...
    static std::unique_ptr<CachedFontSet> CreateForLocale(const std::string& locale)
    {
...
private:
    static FcFontSet* CreateFcFontSetForLocale(const std::string& locale)
    {
        ...
        if (!locale.empty()) {
            ...
            FcPatternAddString(pattern, FC_LANG, reinterpret_cast<const FcChar8*>(locale.c_str()));
        }
    ...
    CachedFontSet(FcFontSet* font_set)
        : font_set_(font_set)
    {
        FillFallbackList();
    }
    ...
    void FillFallbackList()
    {
        ...
        for (int i = 0; i < font_set_->nfont; ++i) {
            ...
            FcCharSet* char_set;
            if (FcPatternGetCharSet(pattern, FC_CHARSET, 0, &char_set) != FcResultMatch)
                continue;
...
base::LazyInstance<FontSetCache>::Leaky g_font_sets_by_locale = LAZY_INSTANCE_INITIALIZER;

} // namespace

FallbackFontData::FallbackFontData() = default;
FallbackFontData::FallbackFontData(const FallbackFontData& other) = default;
FallbackFontData& FallbackFontData::operator=(const FallbackFontData& other) = default;

bool GetFallbackFontForChar(UChar32 c, const std::string& locale, FallbackFontData* fallback_font)
{
