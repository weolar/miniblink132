...
DWRITE_READING_DIRECTION kReadingDirection = DWRITE_READING_DIRECTION_TOP_TO_BOTTOM;
...
public:
    TextAnalysisSourceTest()
    {
        ...
        factory->CreateNumberSubstitution(DWRITE_NUMBER_SUBSTITUTION_METHOD_NONE, kLocale, true /* ignoreUserOverride */, &number_substitution);

        TextAnalysisSource::Create(&source_, kText, kLocale, number_substitution.Get(), kReadingDirection);
    }
...
TEST_F(TextAnalysisSourceTest, TestGetNumberSubstitution)
{
    ...
    hr = source_->GetNumberSubstitution(kTextLength, &length, &number_substitution);
