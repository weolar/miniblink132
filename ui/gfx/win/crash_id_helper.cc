...
std::unique_ptr<CrashIdHelper::ScopedLogger> CrashIdHelper::OnWillProcessMessages(const std::string& id)
{
    if (main_thread_id_ == base::kInvalidThreadId || base::PlatformThread::CurrentId() != main_thread_id_) {
        return nullptr;
    }
