...
const GUID GUID_DEVICEINTERFACE_MONITOR = { 0xE6F07B5F, 0xEE97, 0x4A90, { 0xB0, 0x76, 0x33, 0xF5, 0x7B, 0xF4, 0xEA, 0xA7 } };

namespace {

struct DeviceInfoListScopedTraits {
    static HDEVINFO InvalidValue()
    {
        return INVALID_HANDLE_VALUE;
    }

    static void Free(HDEVINFO h)
    {
        SetupDiDestroyDeviceInfoList(h);
    }
};

bool GetSizeFromRegistry(HDEVINFO device_info_list, SP_DEVINFO_DATA* device_info, int* width_mm, int* height_mm)
{
    base::win::RegKey reg_key(SetupDiOpenDevRegKey(device_info_list, device_info, DICS_FLAG_GLOBAL, 0, DIREG_DEV, KEY_READ));
    if (!reg_key.Valid())
        return false;
...
bool GetInterfaceDetailAndDeviceInfo(HDEVINFO device_info_list, SP_DEVICE_INTERFACE_DATA* interface_data,
    std::unique_ptr<SP_DEVICE_INTERFACE_DETAIL_DATA, base::FreeDeleter>* interface_detail, SP_DEVINFO_DATA* device_info)
{
    ...
    SetupDiGetDeviceInterfaceDetail(device_info_list, interface_data, nullptr, 0, &buffer_size, device_info);
    if (GetLastError() != ERROR_INSUFFICIENT_BUFFER)
        return false;

    interface_detail->reset(reinterpret_cast<SP_DEVICE_INTERFACE_DETAIL_DATA*>(malloc(buffer_size)));
    (*interface_detail)->cbSize = sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA);
    return SetupDiGetDeviceInterfaceDetail(device_info_list, interface_data, interface_detail->get(), buffer_size, nullptr, nullptr) != 0;
}
...
std::vector<PhysicalDisplaySize> GetPhysicalSizeForDisplays()
{
    ...
    base::ScopedGeneric<HDEVINFO, DeviceInfoListScopedTraits> device_info_list(
        SetupDiGetClassDevs(&GUID_DEVICEINTERFACE_MONITOR, nullptr, nullptr, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE));
    ...
    while (SetupDiEnumDeviceInterfaces(device_info_list.get(), nullptr, &GUID_DEVICEINTERFACE_MONITOR, interface_index++, &interface_data)) {
        std::unique_ptr<SP_DEVICE_INTERFACE_DETAIL_DATA, base::FreeDeleter> interface_detail;
        SP_DEVINFO_DATA device_info = {};
        device_info.cbSize = sizeof(device_info);
        bool get_info_succeeded = GetInterfaceDetailAndDeviceInfo(device_info_list.get(), &interface_data, &interface_detail, &device_info);
        ...
        int display_index = 0;
        while (EnumDisplayDevices(nullptr, display_index++, &display_device, EDD_GET_DEVICE_INTERFACE_NAME)) {
            DISPLAY_DEVICE attached_device = {};
            attached_device.cb = sizeof(attached_device);
            int attached_index = 0;
            while (EnumDisplayDevices(display_device.DeviceName, attached_index++, &attached_device, EDD_GET_DEVICE_INTERFACE_NAME)) {
                ...
                if (wcsicmp(attached_device_id, setup_device_path) == 0) {
                    int width_mm;
                    int height_mm;
                    bool found = GetSizeFromRegistry(device_info_list.get(), &device_info, &width_mm, &height_mm);
                    if (found) {
                        out.push_back(PhysicalDisplaySize(base::WideToUTF8(display_device.DeviceName), width_mm, height_mm));
                    }
                    break;
                }
            }
