...
{
    return base::Singleton<ClassRegistrar, base::LeakySingletonTraits<ClassRegistrar>>::get();
}

void ClassRegistrar::UnregisterClasses()
{
    for (RegisteredClasses::iterator i = registered_classes_.begin(); i != registered_classes_.end(); ++i) {
        if (UnregisterClass(MAKEINTATOM(i->atom), i->instance)) {
            registered_classes_.erase(i);
        } else {
            LOG(ERROR) << "Failed to unregister class " << i->name << ". Error = " << GetLastError();
        }
    }
}
...
ATOM ClassRegistrar::RetrieveClassAtom(const ClassInfo& class_info)
{
    ...
    for (RegisteredClasses::const_iterator i = registered_classes_.begin(); i != registered_classes_.end(); ++i) {
        if (class_info.Equals(i->info))
            return i->atom;
    }

    // No class found, need to register one.
    std::wstring name = std::wstring(WindowImpl::kBaseClassName) + base::NumberToWString(registered_count_++);
    ...
    base::win::InitializeWindowClass(name.c_str(), &base::win::WrappedWindowProc<WindowImpl::WndProc>, class_info.style, 0, 0, NULL,
        reinterpret_cast<HBRUSH>(GetStockObject(BLACK_BRUSH)), NULL, class_info.icon, class_info.small_icon, &window_class);
...
ClassRegistrar::RegisteredClass::RegisteredClass(const ClassInfo& info, const std::wstring& name, ATOM atom, HMODULE instance)
    : info(info)
    , name(name)
    , atom(atom)
    , instance(instance)
...
{
    base::AtExitManager::RegisterTask(base::BindOnce(&ClassRegistrar::UnregisterClasses, base::Unretained(ClassRegistrar::GetInstance())));
}
...
void WindowImpl::Init(HWND parent, const Rect& bounds)
{
    ...
    HWND hwnd = CreateWindowEx(window_ex_style_, reinterpret_cast<wchar_t*>(atom), NULL, window_style_, x, y, width, height, parent, NULL, NULL, this);
    ...
    if (hwnd && (window_style_ & WS_CAPTION)) {
        SetWindowPos(hwnd, NULL, 0, 0, 0, 0, SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOREDRAW);
    }
    ...
    if (!hwnd_ && create_window_error == 0) {
        ...
        BOOL got_class = GetClassInfoEx(GetModuleHandle(NULL), reinterpret_cast<wchar_t*>(atom), &class_info);
        base::debug::Alias(&got_class);
        bool procs_match = got_class && class_info.lpfnWndProc == base::win::WrappedWindowProc<&WindowImpl::WndProc>;
...
// static
LRESULT CALLBACK WindowImpl::WndProc(HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param)
{
    ...
    //std::unique_ptr<ScopedLogger> logger = CrashIdHelper::Get()->OnWillProcessMessages(window->debugging_id_);
