...
public:
    ...
    static std::unique_ptr<SingletonHwndHotKeyObserver> Create(const SingletonHwndObserver::WndProc& wnd_proc, UINT key_code, int modifiers);

    SingletonHwndHotKeyObserver(const SingletonHwndHotKeyObserver&) = delete;
    SingletonHwndHotKeyObserver& operator=(const SingletonHwndHotKeyObserver&) = delete;

    ~SingletonHwndHotKeyObserver();

private:
    SingletonHwndHotKeyObserver(const SingletonHwndObserver::WndProc& wnd_proc, int hot_key_id);
