...
public:
    ...
    virtual BOOL ProcessWindowMessage(HWND window, UINT message, WPARAM w_param, LPARAM l_param, LRESULT& result, DWORD msg_map_id = 0) = 0;
};
...
public:
    ...
    void set_window_ex_style(DWORD style)
    {
        window_ex_style_ = style;
    }
    DWORD window_ex_style() const
    {
        return window_ex_style_;
    }
    ...
    UINT initial_class_style() const
    {
        return class_style_;
    }

    const std::string& debugging_id() const
    {
        return debugging_id_;
    }
