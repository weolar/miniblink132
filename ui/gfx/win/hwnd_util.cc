...
void AdjustWindowToFit(HWND hwnd, const RECT& bounds, bool fit_to_monitor)
{
    if (fit_to_monitor) {
        ...
        if (hmon) {
            ...
            if (new_window_rect != window_rect) {
                ...
                SetWindowPos(
                    hwnd, 0, new_window_rect.x(), new_window_rect.y(), new_window_rect.width(), new_window_rect.height(), SWP_NOACTIVATE | SWP_NOZORDER);
                return;
            }
            // Else fall through.
        } else {
    ...
    ::SetWindowPos(hwnd, 0, bounds.left, bounds.top, bounds.right - bounds.left, bounds.bottom - bounds.top, SWP_NOACTIVATE | SWP_NOZORDER);
}
...
std::wstring GetClassName(HWND window)
{
    ...
    while (true) {
        std::wstring output;
        DWORD size_ret = GetClassNameW(window, base::WriteInto(&output, buffer_size), buffer_size);
        if (size_ret == 0)
            break;
...
WNDPROC SetWindowProc(HWND hwnd, WNDPROC proc)
{
    ...
    WNDPROC oldwindow_proc = reinterpret_cast<WNDPROC>(GetWindowLongPtr(hwnd, GWLP_WNDPROC));
...
{
    return reinterpret_cast<void*>(SetWindowLongPtr(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(user_data)));
}
...
std::optional<bool> IsWindowOnCurrentVirtualDesktop(HWND window, Microsoft::WRL::ComPtr<IVirtualDesktopManager> virtual_desktop_manager)
{
    BOOL on_current_desktop;
    if (FAILED(virtual_desktop_manager->IsWindowOnCurrentVirtualDesktop(window, &on_current_desktop))) {
        return std::nullopt;
    }
    ...
    GUID workspace_guid;
    if (FAILED(virtual_desktop_manager->GetWindowDesktopId(window, &workspace_guid))) {
        return std::nullopt;
    }
...
void CenterAndSizeWindow(HWND parent, HWND window, const Size& pref)
{
    ...
    if (pref.width() < (center_bounds.right - center_bounds.left)) {
        window_bounds.left += (center_bounds.right - center_bounds.left - pref.width()) / 2;
    }
    ...
    if (pref.height() < (center_bounds.bottom - center_bounds.top)) {
        window_bounds.top += (center_bounds.bottom - center_bounds.top - pref.height()) / 2;
    }
