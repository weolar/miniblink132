...
TEST_F(FontFallbackWinTest, ParseFontFamilyString)
{
    ...
    internal::ParseFontFamilyString("Cambria & Cambria Math (TrueType)", &font_names);
    ASSERT_EQ(2U, font_names.size());
    EXPECT_EQ("Cambria", font_names[0]);
    EXPECT_EQ("Cambria Math", font_names[1]);
    font_names.clear();

    internal::ParseFontFamilyString("Meiryo & Meiryo Italic & Meiryo UI & Meiryo UI Italic (TrueType)", &font_names);
...
TEST_F(FontFallbackWinTest, NulTerminatedStringPiece)
{
    ...
    EXPECT_FALSE(GetFallbackFont(base_font, kDefaultApplicationLocale, std::u16string_view(kTest1, std::size(kTest1)), &fallback_font));
    // No ending NUL character.
    const char16_t kTest2[] = { 0x0540, 0x0541 };
    EXPECT_TRUE(GetFallbackFont(base_font, kDefaultApplicationLocale, std::u16string_view(kTest2, std::size(kTest2)), &fallback_font));

    // NUL only characters.
    const char16_t kTest3[] = { 0, 0, 0 };
    EXPECT_FALSE(GetFallbackFont(base_font, kDefaultApplicationLocale, std::u16string_view(kTest3, std::size(kTest3)), &fallback_font));
}
