...
void Fill(Canvas* c, const ImageSkiaRep& rep, int x, int y, int w, int h, const cc::PaintFlags& flags)
{
    if (rep.is_null())
        return;
...
NineImagePainter::NineImagePainter(const ImageSkia& image, const Insets& insets)
{
...
{
    return IsEmpty() ? Size()
                     : Size(images_[0].width() + images_[1].width() + images_[2].width(), images_[0].height() + images_[3].height() + images_[6].height());
}
...
void NineImagePainter::Paint(Canvas* canvas, const Rect& bounds, const uint8_t alpha)
{
    if (IsEmpty())
        return;
    ...
    Fill(canvas, image_reps[5], width_in_pixels - i5w, i2h, i5w, height_in_pixels - i2h - i8h, flags);
    Fill(canvas, image_reps[6], 0, height_in_pixels - i6h, i6w, i6h, flags);
    Fill(canvas, image_reps[7], i6w, height_in_pixels - i7h, width_in_pixels - i6w - i8w, i7h, flags);
    Fill(canvas, image_reps[8], width_in_pixels - i8w, height_in_pixels - i8h, i8w, i8h, flags);
}

// static
void NineImagePainter::GetSubsetRegions(const ImageSkia& image, const Insets& insets, std::vector<Rect>* regions)
{
    ...
    const int x[] = { 0, insets.left(), image.width() - insets.right(), image.width() };
    const int y[] = { 0, insets.top(), image.height() - insets.bottom(), image.height() };
