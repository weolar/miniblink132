...
public:
    ...
    static std::optional<std::vector<uint8_t>> Encode(
        const SkPixmap& input, int quality, SkJpegEncoder::Downsample downsample, const SkData* xmp_metadata = nullptr);
    ...
    //
    // quality: an integer in the range 0-100, where 100 is the highest quality.
    static std::optional<std::vector<uint8_t>> Encode(const SkPixmap& input, int quality);
    ...
    //
    // quality: an integer in the range 0-100, where 100 is the highest quality.
    static std::optional<std::vector<uint8_t>> Encode(const SkBitmap& input, int quality);
    ...
    [[nodiscard]] static bool Decode(const uint8_t* input, size_t input_size, SkColorType color_type, std::vector<uint8_t>* output, int* w, int* h);
