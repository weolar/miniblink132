...
PNGCodec::DecodeOutput& PNGCodec::DecodeOutput::operator=(PNGCodec::DecodeOutput&& other) = default;
PNGCodec::DecodeOutput::~DecodeOutput() = default;
...
std::unique_ptr<SkCodec> CreatePngDecoder(std::unique_ptr<SkStream> stream, SkCodec::Result* result)
{
...
std::optional<PreparationOutput> PrepareForPNGDecode(base::span<const uint8_t> input, PNGCodec::ColorFormat format)
{
    ...
    output.image_info = SkImageInfo::Make(codec_info.width(), codec_info.height(), color_type, alpha_type, codec_info.refColorSpace());
...
// static
std::optional<PNGCodec::DecodeOutput> PNGCodec::Decode(base::span<const uint8_t> input, ColorFormat format)
{
    SCOPED_UMA_HISTOGRAM_TIMER_MICROS("ImageDecoder.Png.UiGfxIntoVector");

    std::optional<PreparationOutput> preparation_output = PrepareForPNGDecode(input, format);
    ...
    SkImageInfo info_srgb = preparation_output->image_info.makeColorSpace(SkColorSpace::MakeSRGB());

    // Decode the pixels into the `output` vector.
    output.output.resize(info_srgb.computeMinByteSize());
    SkCodec::Result result = preparation_output->codec->getPixels(info_srgb, output.output.data(), preparation_output->image_info.minRowBytes());
...
SkBitmap PNGCodec::Decode(base::span<const uint8_t> input)
{
    ...
    std::optional<PreparationOutput> preparation_output = PrepareForPNGDecode(input, FORMAT_SkBitmap);
    if (!preparation_output) {
        return SkBitmap();
    }
    ...
    if (alpha == kUnpremul_SkAlphaType) {
        preparation_output->image_info = preparation_output->image_info.makeAlphaType(kPremul_SkAlphaType);
    }
    ...
    SkCodec::Result result = preparation_output->codec->getPixels(bitmap.pixmap());
    if (result == SkCodec::kSuccess) {
        return bitmap;
    } else {
        return SkBitmap();
...
void AddComments(SkPngEncoder::Options& options, const std::vector<PNGCodec::Comment>& comments)
{
    ...
    options.fComments
        = SkDataTable::MakeCopyArrays((void const* const*)comment_pointers.data(), comment_sizes.data(), static_cast<int>(comment_pointers.size()));
}

std::optional<std::vector<uint8_t>> EncodeSkPixmap(const SkPixmap& src, const std::vector<PNGCodec::Comment>& comments, int zlib_level, bool disable_filters)
{
...
std::optional<std::vector<uint8_t>> EncodeSkPixmap(
    const SkPixmap& src, bool discard_transparency, const std::vector<PNGCodec::Comment>& comments, int zlib_level, bool disable_filters)
{
    if (discard_transparency) {
        ...
        success = src.readPixels(opaque_info.makeAlphaType(kUnpremul_SkAlphaType), opaque_pixmap.writable_addr(), opaque_pixmap.rowBytes());
        DCHECK(success);
    ...
    if (src.info().alphaType() != kOpaque_SkAlphaType && src.computeIsOpaque()) {
        SkPixmap opaque_pixmap { src.info().makeAlphaType(kOpaque_SkAlphaType), src.addr(), src.rowBytes() };
        return EncodeSkPixmap(opaque_pixmap, comments, zlib_level, disable_filters);
    }
...
std::optional<std::vector<uint8_t>> EncodeSkBitmap(const SkBitmap& input, bool discard_transparency, int zlib_level, bool disable_filters)
{
    SkPixmap src;
    if (!input.peekPixels(&src)) {
        return std::nullopt;
    }
    return EncodeSkPixmap(src, discard_transparency, std::vector<PNGCodec::Comment>(), zlib_level, disable_filters);
}

} // namespace

std::optional<std::vector<uint8_t>> PNGCodec::Encode(
    const unsigned char* input, ColorFormat format, const Size& size, int row_byte_width, bool discard_transparency, const std::vector<Comment>& comments)
{
    ...
    auto alphaType = format == FORMAT_SkBitmap ? kPremul_SkAlphaType : kUnpremul_SkAlphaType;
    SkImageInfo info = SkImageInfo::Make(size.width(), size.height(), colorType, alphaType);
    SkPixmap src(info, input, row_byte_width);
    return EncodeSkPixmap(src, discard_transparency, comments, DEFAULT_ZLIB_COMPRESSION, /*disable_filters=*/false);
}

std::optional<std::vector<uint8_t>> PNGCodec::EncodeBGRASkBitmap(const SkBitmap& input, bool discard_transparency)
{
    return EncodeSkBitmap(input, discard_transparency, DEFAULT_ZLIB_COMPRESSION,
        /*disable_filters=*/false);
}

std::optional<std::vector<uint8_t>> PNGCodec::FastEncodeBGRASkBitmap(const SkBitmap& input, bool discard_transparency)
{
