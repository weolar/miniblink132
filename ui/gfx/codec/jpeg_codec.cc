...
std::optional<std::vector<uint8_t>> JPEGCodec::Encode(const SkPixmap& input, int quality, SkJpegEncoder::Downsample downsample, const SkData* xmp_metadata)
{
...
std::optional<std::vector<uint8_t>> JPEGCodec::Encode(const SkPixmap& input, int quality)
{
    return Encode(input, quality, SkJpegEncoder::Downsample::k420);
}

std::optional<std::vector<uint8_t>> JPEGCodec::Encode(const SkBitmap& src, int quality)
{
    SkPixmap pixmap;
...
std::optional<PreparationOutput> PrepareForJPEGDecode(base::span<const uint8_t> input, SkColorType color_type)
{
    PreparationOutput output;

    // We only support 8-bit RGBA and BGRA color types.
    CHECK(color_type == kRGBA_8888_SkColorType || color_type == kBGRA_8888_SkColorType) << "Invalid pixel format " << color_type;
...
bool JPEGCodec::Decode(const uint8_t* input, size_t input_size, SkColorType color_type, std::vector<uint8_t>* output, int* w, int* h)
{
    std::optional<PreparationOutput> preparation_output = PrepareForJPEGDecode(UNSAFE_BUFFERS(base::span(input, input_size)), color_type);
    ...
    *h = preparation_output->image_info.height();
    ...
    SkCodec::Result result
        = preparation_output->codec->getPixels(preparation_output->image_info, &output->front(), preparation_output->image_info.minRowBytes());
...
SkBitmap JPEGCodec::Decode(base::span<const uint8_t> input)
{
    ...
    std::optional<PreparationOutput> preparation_output = PrepareForJPEGDecode(input, kFormat);
    if (!preparation_output) {
        return SkBitmap();
    }
    ...
    SkBitmap bitmap;
    if (!bitmap.tryAllocN32Pixels(preparation_output->image_info.width(), preparation_output->image_info.height())) {
        return SkBitmap();
    }
