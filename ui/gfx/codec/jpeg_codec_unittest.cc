...
constexpr uint8_t kTopSitesMigrationTestImage[] = "\xff\xd8\xff\xe0\x00\x10\x4a\x46\x49\x46\x00\x01\x01\x00\x00\x01"
...
constexpr uint8_t kExtremelyLargeTestImage[] = "\xff\xd8\xff\xe0\x00\x10\x4a\x46\x49\x46\x00\x01\x01\x00\x00\x01"
                                               "\x00\x01\x00\x00\xff\xdb\x00\x43\x00\x03\x02\x02\x03\x02\x02\x03"
                                               "\x03\x03\x03\x04\x03\x03\x04\x05\x08\x05\x05\x04\x04\x05\x0a\x07"
                                               "\x07\x06\x08\x0c\x0a\x0c\x0c\x0b\x0a\x0b\x0b\x0d\x0e\x12\x10\x0d"
                                               "\x0e\x11\x0e\x0b\x0b\x10\x16\x10\x11\x13\x14\x15\x15\x15\x0c\x0f"
...
double AveragePixelDelta(base::span<const uint8_t> a, base::span<const uint8_t> b)
{
...
TEST(JPEGCodec, EncodeDecodeRGBA)
{
    ...
    SkImageInfo info = SkImageInfo::Make(kWidth, kHeight, kRGBA_8888_SkColorType, kOpaque_SkAlphaType);
    SkPixmap src(info, &original[0], kWidth * 4);
    std::optional<std::vector<uint8_t>> encoded = JPEGCodec::Encode(src, kJpegQuality);
    ASSERT_TRUE(encoded);
    ...
    int outw, outh;
    EXPECT_TRUE(JPEGCodec::Decode(encoded->data(), encoded->size(), kRGBA_8888_SkColorType, &decoded, &outw, &outh));
...
TEST(JPEGCodec, DecodeCorrupted)
{
    ...
    int outw, outh;
    ASSERT_FALSE(JPEGCodec::Decode(&original[0], original.size(), kRGBA_8888_SkColorType, &output, &outw, &outh));

    // Make some compressed data.
    SkImageInfo info = SkImageInfo::Make(kWidth, kHeight, kRGBA_8888_SkColorType, kOpaque_SkAlphaType);
    SkPixmap src(info, &original[0], kWidth * 4);
    std::optional<std::vector<uint8_t>> compressed = JPEGCodec::Encode(src, kJpegQuality);
    ASSERT_TRUE(compressed);

    // Try decompressing a truncated version.
    ASSERT_FALSE(JPEGCodec::Decode(compressed->data(), compressed->size() / 2, kRGBA_8888_SkColorType, &output, &outw, &outh));

    // Corrupt it and try decompressing that.
    for (int i = 10; i < 30; i++) {
        compressed.value()[i] = i;
    }
    ASSERT_FALSE(JPEGCodec::Decode(compressed->data(), compressed->size(), kRGBA_8888_SkColorType, &output, &outw, &outh));
}
...
TEST(JPEGCodec, InvalidRead)
{
    ...
    int outw, outh;
    ASSERT_TRUE(JPEGCodec::Decode(kTopSitesMigrationTestImage, std::size(kTopSitesMigrationTestImage), kRGBA_8888_SkColorType, &output, &outw, &outh));
}
...
TEST(JPEGCodec, ParallelEncoding)
{
    ...
    SkImageInfo info = SkImageInfo::Make(kImageSize, kImageSize, kRGBA_8888_SkColorType, kOpaque_SkAlphaType);
    ...
    base::RepeatingClosure encode_completion_closure = base::BarrierClosure(kNumCopies, encode_loop.QuitClosure());
    for (int i = 0; i < kNumCopies; ++i) {
        base::ThreadPool::PostTask(FROM_HERE, base::BindLambdaForTesting([&] {
            std::optional<std::vector<uint8_t>> output = JPEGCodec::Encode(src, kJpegQuality);
            ASSERT_TRUE(output);
...
TEST(JPEGCodec, ExtremelyLargeImage)
{
    std::vector<unsigned char> output;
    int outw, outh;
    bool ok = JPEGCodec::Decode(kExtremelyLargeTestImage, std::size(kExtremelyLargeTestImage), kRGBA_8888_SkColorType, &output, &outw, &outh);
    EXPECT_FALSE(ok);
}
