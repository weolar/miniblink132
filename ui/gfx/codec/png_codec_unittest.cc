...
std::vector<uint8_t> MakeRGBAImage(int width, int height, bool use_transparency)
{
...
{
    std::vector<uint8_t>& v = *static_cast<std::vector<uint8_t>*>(png_get_io_ptr(png_ptr));
...
std::tuple<uint8_t, uint8_t, uint8_t> Read3(const std::vector<uint8_t>& pixels, size_t base)
{
    return std::tie(pixels[base], pixels[base + 1], pixels[base + 2]);
}

std::tuple<uint8_t, uint8_t, uint8_t, uint8_t> Read4(const std::vector<uint8_t>& pixels, size_t base)
{
    return std::tie(pixels[base], pixels[base + 1], pixels[base + 2], pixels[base + 3]);
}

struct ImageSpec {
    ImageSpec(int width, int height, const std::vector<uint8_t>& bytes, ColorType type)
        : width(width)
        , height(height)
        , bytes(bytes)
        , type(type)
    ...
    ImageSpec(
        int width, int height, const std::vector<uint8_t>& bytes, ColorType type, const std::vector<png_color>& palette, const std::vector<uint8_t>& trans)
        : width(width)
        , height(height)
        , bytes(bytes)
        , type(type)
...
std::optional<std::vector<uint8_t>> EncodeImage(base::span<const uint8_t> input, const int width, const int height, ColorType output_color_type,
    const int interlace_type = PNG_INTERLACE_NONE, std::optional<base::span<const png_color>> palette = std::nullopt,
    std::optional<base::span<const uint8_t>> palette_alpha = std::nullopt)
{
    ...
    png_struct* png_ptr = png_create_write_struct(PNG_LIBPNG_VER_STRING, /*error_ptr=*/nullptr,
    ...
    png_set_IHDR(png_ptr, info_ptr, width, height, 8, output_color_type, interlace_type, PNG_COMPRESSION_TYPE_DEFAULT, PNG_FILTER_TYPE_DEFAULT);
    if (output_color_type == COLOR_TYPE_PALETTE) {
        png_set_PLTE(png_ptr, info_ptr, palette->data(), palette->size());
        if (palette_alpha) {
            png_set_tRNS(png_ptr, info_ptr, palette_alpha->data(), palette_alpha->size(), /*trans_color=*/nullptr);
        }
    }
...
{
    return abs(static_cast<int>(SkColorGetB(a) - SkColorGetB(b))) < 2 && abs(static_cast<int>(SkColorGetG(a) - SkColorGetG(b))) < 2
        && abs(static_cast<int>(SkColorGetR(a) - SkColorGetR(b))) < 2 && abs(static_cast<int>(SkColorGetA(a) - SkColorGetA(b))) < 2;
}
...
void MakeTestBGRASkBitmap(int width, int height, SkBitmap* bmp)
{
    ...
    for (int x = 0; x < width; ++x) {
        for (int y = 0; y < height; ++y) {
            int i = y * width + x;
            *bmp->getAddr32(x, y) = SkPreMultiplyARGB(i % 255, i % 250, i % 245, i % 240);
        }
    }
}
...
TEST_P(PNGCodecTest, EncodeDecodeRGBA)
{
    ...
    std::vector<uint8_t> original = MakeRGBAImage(kWidth, kHeight, /*use_transparency=*/true);

    // encode
    std::optional<std::vector<uint8_t>> encoded = PNGCodec::Encode(original.data(), PNGCodec::FORMAT_RGBA, Size(kWidth, kHeight), kWidth * 4,
    ...
    std::optional<PNGCodec::DecodeOutput> output;
    {
        ...
        ASSERT_TRUE(output);
        std::vector<base::Bucket> buckets = histograms.GetAllSamples("ImageDecoder.Png.UiGfxIntoVector");
        ASSERT_EQ(buckets.size(), 1u);
        ASSERT_GE(buckets[0].min, 0);
    }

    EXPECT_TRUE(
        ImagesExactlyEqual(ImageSpec(kWidth, kHeight, original, COLOR_TYPE_RGBA), ImageSpec(output->width, output->height, output->output, COLOR_TYPE_RGBA)));
}
...
TEST_P(PNGCodecTest, EncodeDecodeBGRA)
{
    ...
    // Encode.
    std::optional<std::vector<uint8_t>> encoded = PNGCodec::Encode(original.data(), PNGCodec::FORMAT_BGRA, Size(kWidth, kHeight), kWidth * 4,
        /*discard_transparency=*/false, std::vector<PNGCodec::Comment>());
    ASSERT_TRUE(encoded);

    // Decode, it should have the same size as the original.
    std::optional<PNGCodec::DecodeOutput> output = PNGCodec::Decode(encoded.value(), PNGCodec::FORMAT_BGRA);
    ASSERT_TRUE(output);

    EXPECT_TRUE(
        ImagesExactlyEqual(ImageSpec(kWidth, kHeight, original, COLOR_TYPE_BGRA), ImageSpec(output->width, output->height, output->output, COLOR_TYPE_BGRA)));
}
...
TEST_P(PNGCodecTest, DecodePalette)
{
    ...
    std::optional<std::vector<uint8_t>> encoded
        = EncodeImage(original.data, kWidth, kHeight, COLOR_TYPE_PALETTE, PNG_INTERLACE_NONE, original.palette, original.trans_chunk);
    ASSERT_TRUE(encoded);

    // decode
    std::optional<PNGCodec::DecodeOutput> output = PNGCodec::Decode(encoded.value(), PNGCodec::FORMAT_RGBA);
    ASSERT_TRUE(output);

    EXPECT_TRUE(ImagesExactlyEqual(ImageSpec(kWidth, kHeight, original.data, COLOR_TYPE_PALETTE, original.palette, original.trans_chunk),
        ImageSpec(output->width, output->height, output->output, COLOR_TYPE_RGBA)));
}
...
TEST_P(PNGCodecTest, DecodeInterlacedPalette)
{
    ...
    // encode
    std::optional<std::vector<uint8_t>> encoded
        = EncodeImage(original.data, kWidth, kHeight, COLOR_TYPE_PALETTE, PNG_INTERLACE_ADAM7, original.palette, original.trans_chunk);
    ASSERT_TRUE(encoded);
...
TEST_P(PNGCodecTest, DecodeGrayscale)
{
    ...
    // encode
    std::optional<std::vector<uint8_t>> encoded = EncodeImage(original, kWidth, kHeight, COLOR_TYPE_GRAY);
    ASSERT_TRUE(encoded);
    ...
    EXPECT_TRUE(
        ImagesExactlyEqual(ImageSpec(kWidth, kHeight, original, COLOR_TYPE_GRAY), ImageSpec(output->width, output->height, output->output, COLOR_TYPE_RGBA)));
}
...
TEST_P(PNGCodecTest, DecodeGrayscaleWithAlpha)
{
    ...
    // encode
    std::optional<std::vector<uint8_t>> encoded = EncodeImage(original, kWidth, kHeight, COLOR_TYPE_GRAY_ALPHA);
    ASSERT_TRUE(encoded);
    ...
    EXPECT_TRUE(ImagesExactlyEqual(
        ImageSpec(kWidth, kHeight, original, COLOR_TYPE_GRAY_ALPHA), ImageSpec(output->width, output->height, output->output, COLOR_TYPE_RGBA)));
}
...
TEST_P(PNGCodecTest, DecodeInterlacedGrayscaleWithAlpha)
{
    ...
    // encode
    std::optional<std::vector<uint8_t>> encoded = EncodeImage(original, kWidth, kHeight, COLOR_TYPE_GRAY_ALPHA, PNG_INTERLACE_ADAM7);
    ASSERT_TRUE(encoded);
...
TEST_P(PNGCodecTest, DecodeInterlacedRGBA)
{
    ...
    // create an image with known values
    std::vector<uint8_t> original = MakeRGBAImage(kWidth, kHeight, /*use_transparency=*/false);

    // encode
    std::optional<std::vector<uint8_t>> encoded = EncodeImage(original, kWidth, kHeight, COLOR_TYPE_RGBA, PNG_INTERLACE_ADAM7);
    ASSERT_TRUE(encoded);
...
TEST_P(PNGCodecTest, DecodeInterlacedBGR)
{
    ...
    // encode
    std::optional<std::vector<uint8_t>> encoded = EncodeImage(original, kWidth, kHeight, COLOR_TYPE_BGR, PNG_INTERLACE_ADAM7);
    ASSERT_TRUE(encoded);
    ...
    EXPECT_TRUE(
        ImagesExactlyEqual(ImageSpec(kWidth, kHeight, original, COLOR_TYPE_BGR), ImageSpec(output->width, output->height, output->output, COLOR_TYPE_BGRA)));
}
...
TEST_P(PNGCodecTest, DecodeInterlacedBGRA)
{
    ...
    // encode
    std::optional<std::vector<uint8_t>> encoded = EncodeImage(original, kWidth, kHeight, COLOR_TYPE_BGRA, PNG_INTERLACE_ADAM7);
    ASSERT_TRUE(encoded);
    ...
    EXPECT_TRUE(ImagesExactlyEqual(ImageSpec(kWidth, kHeight, original, COLOR_TYPE_BGRA), ImageSpec(output->width, output->height, original, COLOR_TYPE_BGRA)));
}
...
TEST_P(PNGCodecTest, DecodeInterlacedRGBtoSkBitmap)
{
    ...
    // encode
    std::optional<std::vector<uint8_t>> encoded = EncodeImage(original, kWidth, kHeight, COLOR_TYPE_RGB, PNG_INTERLACE_ADAM7);
    ASSERT_TRUE(encoded);
    ...
    SkBitmap decoded_bitmap;
    {
        ...
        std::vector<base::Bucket> buckets = histograms.GetAllSamples("ImageDecoder.Png.UiGfxIntoSkBitmap");
        ASSERT_EQ(buckets.size(), 1u);
        ASSERT_GE(buckets[0].min, 0);
    }

    EXPECT_EQ(decoded_bitmap.alphaType(), kOpaque_SkAlphaType);
    EXPECT_TRUE(ImageExactlyEqualsSkBitmap(ImageSpec(kWidth, kHeight, original, COLOR_TYPE_RGB), decoded_bitmap));
}
...
void DecodeInterlacedRGBAtoSkBitmap(bool use_transparency)
{
    ...
    const ColorType color_type = use_transparency ? COLOR_TYPE_RGBA : COLOR_TYPE_RGBX;

    // create an image with known values
    std::vector<uint8_t> original = MakeRGBAImage(kWidth, kHeight, use_transparency);

    // encode
    std::optional<std::vector<uint8_t>> encoded = EncodeImage(original, kWidth, kHeight, color_type, PNG_INTERLACE_ADAM7);
    ASSERT_TRUE(encoded);

    // Decode the encoded string.
    SkBitmap decoded_bitmap = PNGCodec::Decode(encoded.value());
    ASSERT_FALSE(decoded_bitmap.isNull());
    EXPECT_EQ(decoded_bitmap.alphaType(), use_transparency ? kPremul_SkAlphaType : kOpaque_SkAlphaType);

    EXPECT_TRUE(ImageExactlyEqualsSkBitmap(ImageSpec(kWidth, kHeight, original, color_type), decoded_bitmap));
}
...
TEST(PNGCodec, EncoderSavesImagesWithAllOpaquePixelsAsOpaque)
{
    ...
    std::optional<std::vector<uint8_t>> png_data = PNGCodec::Encode(original.data(), PNGCodec::FORMAT_RGBA, gfx::Size(kWidth, kHeight), kWidth * 4,
        /*discard_transparency=*/false, std::vector<PNGCodec::Comment> {});
...
TEST_P(PNGCodecTest, DecodeCorrupted)
{
    ...
    std::optional<PNGCodec::DecodeOutput> output = PNGCodec::Decode(original, PNGCodec::FORMAT_RGBA);
    ASSERT_FALSE(output);

    // Make some compressed data.
    std::optional<std::vector<uint8_t>> compressed
        = PNGCodec::Encode(&original[0], PNGCodec::FORMAT_RGBA, Size(kWidth, kHeight), kWidth * 4, false, std::vector<PNGCodec::Comment>());
    ASSERT_TRUE(compressed);

    // Try decompressing a truncated version.
    output = PNGCodec::Decode(base::span(compressed.value()).subspan(0, compressed.value().size() / 2), PNGCodec::FORMAT_RGBA);
    ASSERT_FALSE(output);
...
TEST_P(PNGCodecTest, DecodeGamma)
{
    ...
    base::FilePath data_dir = root_dir.AppendASCII("ui").AppendASCII("gfx").AppendASCII("test").AppendASCII("data").AppendASCII("codec");
    ...
    for (const auto& sf : kSourceFiles) {
        ...
        std::optional<const std::vector<uint8_t>> opt_input = base::ReadFileToBytes(filename);
        ...
        std::optional<PNGCodec::DecodeOutput> output = PNGCodec::Decode(input, PNGCodec::FORMAT_RGBA);
        ASSERT_TRUE(output);
...
TEST_P(PNGCodecTest, EncodeBGRASkBitmapStridePadded)
{
    ...
    // Encode the bitmap.
    std::optional<std::vector<uint8_t>> encoded = PNGCodec::EncodeBGRASkBitmap(original_bitmap, /*discard_transparency=*/false);
    ...
    for (int x = 0; x < kWidth; x++) {
        for (int y = 0; y < kHeight; y++) {
            ...
            ASSERT_TRUE(ColorsClose(original_pixel, decoded_pixel))
                << "; original_pixel = " << std::hex << std::setw(8) << original_pixel << "; decoded_pixel = " << std::hex << std::setw(8) << decoded_pixel;
        }
    }
}
...
TEST_P(PNGCodecTest, EncodeBGRASkBitmapDiscardTransparency)
{
    ...
    // Encode the bitmap.
    std::optional<std::vector<uint8_t>> encoded = PNGCodec::EncodeBGRASkBitmap(original_bitmap, /*discard_transparency=*/true);
    ...
    for (int x = 0; x < kWidth; x++) {
        for (int y = 0; y < kHeight; y++) {
            ...
            EXPECT_TRUE(NonAlphaColorsClose(unpremultiplied, unpremultiplied_decoded))
                << "Original_pixel: (" << SkColorGetR(unpremultiplied) << ", " << SkColorGetG(unpremultiplied) << ", " << SkColorGetB(unpremultiplied) << "), "
                << "Decoded pixel: (" << SkColorGetR(unpremultiplied_decoded) << ", " << SkColorGetG(unpremultiplied_decoded) << ", "
...
TEST_P(PNGCodecTest, EncodeWithComment)
{
    ...
    std::optional<std::vector<uint8_t>> encoded = PNGCodec::Encode(original.data(), PNGCodec::FORMAT_RGBA, Size(kWidth, kHeight), kWidth * 4, false, comments);
    ASSERT_TRUE(encoded);
    ...
    // results.
    const uint8_t kExpected1[] = "\x00\x00\x00\x08tEXtkey\x00text\x9e\xe7\x66\x51";
    const uint8_t kExpected2[] = "\x00\x00\x00\x0etEXttest\x00something\x29\xba\xef\xac";
    const uint8_t kExpected3[] = "\x00\x00\x00\x18tEXthave some\x00spaces in both\x8d\x69\x34\x2d";

    EXPECT_NE(base::ranges::search(encoded.value(), kExpected1), encoded.value().end());
    EXPECT_NE(base::ranges::search(encoded.value(), kExpected2), encoded.value().end());
    EXPECT_NE(base::ranges::search(encoded.value(), kExpected3), encoded.value().end());
}
...
TEST_P(PNGCodecTest, EncodeDecodeWithVaryingCompressionLevels)
{
    ...
    // Encode.
    std::optional<std::vector<uint8_t>> encoded_normal = PNGCodec::EncodeBGRASkBitmap(original_bitmap,
        /*discard_transparency=*/false);
    ASSERT_TRUE(encoded_normal);

    std::optional<std::vector<uint8_t>> encoded_fast = PNGCodec::FastEncodeBGRASkBitmap(original_bitmap,
        /*discard_transparency=*/false);
...
INSTANTIATE_TEST_SUITE_P(RustEnabled, PNGCodecTest, ::testing::Values(RustFeatureState::kRustEnabled));
#endif

INSTANTIATE_TEST_SUITE_P(RustDisabled, PNGCodecTest, ::testing::Values(RustFeatureState::kRustDisabled));
