...
struct PathElement {
    constexpr PathElement(CommandType value)
        : command(value)
    {
    }
    constexpr PathElement(SkScalar value)
        : arg(value)
    {
    }
...
struct VectorIcon {
    VectorIcon() = default;
    constexpr VectorIcon(const VectorIconRep* reps_data, size_t reps_size, const char* name)
        : reps(reps_data, reps_size)
        , name(name)
    {
    }
