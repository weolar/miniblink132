...
static bool FloatsEqualWithinTolerance(const float* a, const float* b, int n, float tol)
{
    for (int i = 0; i < n; ++i) {
...
{
    DCHECK(id != ColorSpace::PrimaryID::INVALID && id != ColorSpace::PrimaryID::CUSTOM);
...
float GetSDRWhiteLevelFromPQSkTransferFunction(const skcms_TransferFunction& fn)
{
...
ColorSpace::ColorSpace(PrimaryID primaries, TransferID transfer, MatrixID matrix, RangeID range, const skcms_Matrix3x3* custom_primary_matrix,
...
ColorSpace::ColorSpace(const SkColorSpace& sk_color_space, bool is_hdr)
    : ColorSpace(PrimaryID::INVALID, TransferID::INVALID, MatrixID::RGB, RangeID::FULL)
{
...
{
    return primaries_ != PrimaryID::INVALID && transfer_ != TransferID::INVALID && matrix_ != MatrixID::INVALID && range_ != RangeID::INVALID;
}

// static
ColorSpace ColorSpace::CreateExtendedSRGB10Bit()
{
    return ColorSpace(PrimaryID::P3, TransferID::CUSTOM_HDR, MatrixID::RGB, RangeID::FULL, nullptr, &SkNamedTransferFnExt::kSRGBExtended1023Over510);
}

// static
ColorSpace ColorSpace::CreatePiecewiseHDR(PrimaryID primaries, float sdr_joint, float hdr_level, const skcms_Matrix3x3* custom_primary_matrix)
{
    ...
    ColorSpace result(primaries, TransferID::PIECEWISE_HDR, MatrixID::RGB, RangeID::FULL, custom_primary_matrix, nullptr);
...
// static
ColorSpace ColorSpace::CreateCustom(const skcms_Matrix3x3& to_XYZD50, const skcms_TransferFunction& fn)
{
    ColorSpace result(ColorSpace::PrimaryID::CUSTOM, ColorSpace::TransferID::CUSTOM, ColorSpace::MatrixID::RGB, ColorSpace::RangeID::FULL, &to_XYZD50, &fn);
    return result;
}

// static
ColorSpace ColorSpace::CreateCustom(const skcms_Matrix3x3& to_XYZD50, TransferID transfer)
{
    ColorSpace result(ColorSpace::PrimaryID::CUSTOM, transfer, ColorSpace::MatrixID::RGB, ColorSpace::RangeID::FULL, &to_XYZD50, nullptr);
    return result;
}
...
void ColorSpace::SetCustomPrimaries(const skcms_Matrix3x3& to_XYZD50)
{
    ...
    for (PrimaryID id : kIDsToCheck) {
        ...
        if (FloatsEqualWithinTolerance(&to_XYZD50.vals[0][0], &matrix.vals[0][0], 9, 0.001f)) {
            primaries_ = id;
            return;
        }
    }
...
{
    DCHECK(transfer_ == TransferID::CUSTOM || transfer_ == TransferID::CUSTOM_HDR);
    ...
    if (transfer_ == TransferID::CUSTOM) {
        ...
        const TransferID kIDsToCheck[] = {
            ...
            TransferID::GAMMA24,
            TransferID::GAMMA28,
            TransferID::SMPTE240M,
            TransferID::BT709_APPLE,
...
{
    if (primaries_ != other.primaries_ || transfer_ != other.transfer_ || matrix_ != other.matrix_ || range_ != other.range_) {
        return false;
    }
    if (primaries_ == PrimaryID::CUSTOM) {
        if (memcmp(custom_primary_matrix_, other.custom_primary_matrix_, sizeof(custom_primary_matrix_))) {
            return false;
        }
    }
    if (size_t param_count = TransferParamCount(transfer_)) {
        if (memcmp(transfer_params_, other.transfer_params_, param_count * sizeof(float))) {
            return false;
        }
    }
    return true;
...
bool ColorSpace::IsWide() const
{
    ...
    if (transfer_ == TransferID::SRGB_HDR || transfer_ == TransferID::LINEAR_HDR || transfer_ == TransferID::CUSTOM_HDR)
        return true;

    if (primaries_ == PrimaryID::BT2020 || primaries_ == PrimaryID::SMPTEST431_2 || primaries_ == PrimaryID::P3 || primaries_ == PrimaryID::ADOBE_RGB
...
{
    return transfer_ == TransferID::PQ || transfer_ == TransferID::HLG || transfer_ == TransferID::LINEAR_HDR || transfer_ == TransferID::SRGB_HDR
        || transfer_ == TransferID::CUSTOM_HDR || transfer_ == TransferID::PIECEWISE_HDR || transfer_ == TransferID::SCRGB_LINEAR_80_NITS;
}
...
{
    return transfer_ == TransferID::LINEAR_HDR || transfer_ == TransferID::SRGB_HDR || transfer_ == TransferID::CUSTOM_HDR
        || transfer_ == TransferID::PIECEWISE_HDR || transfer_ == TransferID::SCRGB_LINEAR_80_NITS || transfer_ == TransferID::BT1361_ECG
...
bool ColorSpace::operator<(const ColorSpace& other) const
{
    ...
    if (range_ > other.range_)
        return false;
    if (primaries_ == PrimaryID::CUSTOM) {
        int primary_result = memcmp(custom_primary_matrix_, other.custom_primary_matrix_, sizeof(custom_primary_matrix_));
        if (primary_result < 0)
            return true;
        if (primary_result > 0)
            return false;
    ...
    if (size_t param_count = TransferParamCount(transfer_)) {
        int transfer_result = memcmp(transfer_params_, other.transfer_params_, param_count * sizeof(float));
...
{
    size_t result = (static_cast<size_t>(primaries_) << 0) | (static_cast<size_t>(transfer_) << 8) | (static_cast<size_t>(matrix_) << 16)
        | (static_cast<size_t>(range_) << 24);
    if (primaries_ == PrimaryID::CUSTOM) {
        const uint32_t* params = reinterpret_cast<const uint32_t*>(custom_primary_matrix_);
        result ^= params[0];
        result ^= params[4];
        result ^= params[8];
    }
    {
        ...
        const uint32_t* params = reinterpret_cast<const uint32_t*>(transfer_params_);
        result ^= params[3];
        result ^= params[6];
    }
    return result;
...
std::string ColorSpace::ToString() const
{
    ...
    case TransferID::CUSTOM_HDR: {
        skcms_TransferFunction fn;
        GetTransferFunction(&fn);
        if (fn.g == 1.0f && fn.a > 0.0f && fn.b == 0.0f && fn.c == 0.0f && fn.d == 0.0f && fn.e == 0.0f && fn.f == 0.0f) {
    ...
    case TransferID::PIECEWISE_HDR: {
        skcms_TransferFunction fn;
        GetTransferFunction(&fn);
        ss << "sRGB to 1 at " << transfer_params_[0] << ", linear to " << transfer_params_[1] << " at 1";
        break;
    }
    case TransferID::SCRGB_LINEAR_80_NITS:
...
ColorSpace ColorSpace::GetWithMatrixAndRange(MatrixID matrix, RangeID range) const
{
    ColorSpace result(*this);
    if (!IsValid())
        return result;
...
sk_sp<SkColorSpace> ColorSpace::ToSkColorSpace(std::optional<float> sdr_white_level) const
{
    ...
    case TransferID::LINEAR_HDR:
        ...
        break;
    case TransferID::HLG:
        transfer_fn = GetHLGSkTransferFunction(sdr_white_level.value_or(kDefaultSDRWhiteLevel));
        break;
    case TransferID::PQ:
        transfer_fn = GetPQSkTransferFunction(sdr_white_level.value_or(transfer_params_[0]));
        break;
    default:
    ...
    sk_sp<SkColorSpace> sk_color_space = SkColorSpace::MakeRGB(transfer_fn, gamut);
    if (!sk_color_space)
...
bool ColorSpace::IsTransferFunctionEqualTo(const skcms_TransferFunction& fn) const
{
    ...
    return fn.a == transfer_fn.a && fn.b == transfer_fn.b && fn.c == transfer_fn.c && fn.d == transfer_fn.d && fn.e == transfer_fn.e && fn.f == transfer_fn.f
        && fn.g == transfer_fn.g;
}

bool ColorSpace::Contains(const ColorSpace& other) const
{
    if (primaries_ == PrimaryID::INVALID || other.primaries_ == PrimaryID::INVALID)
        return false;
...
// static
SkColorSpacePrimaries ColorSpace::GetColorSpacePrimaries(PrimaryID primary_id, const skcms_Matrix3x3* custom_primary_matrix = nullptr)
{
...
// static
void ColorSpace::GetPrimaryMatrix(PrimaryID primary_id, skcms_Matrix3x3* to_XYZD50)
{
...
// static
bool ColorSpace::GetTransferFunction(TransferID transfer, skcms_TransferFunction* fn)
{
...
bool ColorSpace::GetTransferFunction(skcms_TransferFunction* fn, std::optional<float> sdr_white_level) const
{
    switch (transfer_) {
    case TransferID::CUSTOM:
    case TransferID::CUSTOM_HDR:
...
bool ColorSpace::GetInverseTransferFunction(skcms_TransferFunction* fn, std::optional<float> sdr_white_level) const
{
...
bool ColorSpace::GetPiecewiseHDRParams(float* sdr_joint, float* hdr_level) const
{
...
SkM44 ColorSpace::GetTransferMatrix(int bit_depth) const
{
    ...
    // used for YUV.
    const float chroma_0_5 = static_cast<float>(1 << (bit_depth - 1)) / ((1 << bit_depth) - 1);
    float Kr = 0;
    float Kb = 0;
    switch (matrix_) {
...
SkM44 ColorSpace::GetRangeAdjustMatrix(int bit_depth) const
{
    ...
    case MatrixID::YCOCG:
        return SkM44::Scale(scale_y, scale_y, scale_y).postTranslate(-16.0f / 219.0f, -16.0f / 219.0f, -16.0f / 219.0f);
    ...
    case MatrixID::YDZDX: {
        ...
        return SkM44::Scale(scale_y, scale_uv, scale_uv).postTranslate(-16.0f / 219.0f, translate_uv, translate_uv);
    }
    }
    NOTREACHED();
}
...
bool ColorSpace::ToSkYUVColorSpace(int bit_depth, SkYUVColorSpace* out) const
{
    switch (matrix_) {
    case MatrixID::BT709:
        *out = range_ == RangeID::FULL ? kRec709_Full_SkYUVColorSpace : kRec709_Limited_SkYUVColorSpace;
        return true;

    case MatrixID::BT470BG:
    case MatrixID::SMPTE170M:
        *out = range_ == RangeID::FULL ? kJPEG_SkYUVColorSpace : kRec601_Limited_SkYUVColorSpace;
        return true;

    case MatrixID::BT2020_NCL:
        if (bit_depth == 8) {
            *out = range_ == RangeID::FULL ? kBT2020_8bit_Full_SkYUVColorSpace : kBT2020_8bit_Limited_SkYUVColorSpace;
            return true;
        }
        if (bit_depth == 10) {
            *out = range_ == RangeID::FULL ? kBT2020_10bit_Full_SkYUVColorSpace : kBT2020_10bit_Limited_SkYUVColorSpace;
            return true;
        }
        if (bit_depth == 12) {
            *out = range_ == RangeID::FULL ? kBT2020_12bit_Full_SkYUVColorSpace : kBT2020_12bit_Limited_SkYUVColorSpace;
            return true;
        }
        if (bit_depth == 16) {
            *out = range_ == RangeID::FULL ? kBT2020_16bit_Full_SkYUVColorSpace : kBT2020_16bit_Limited_SkYUVColorSpace;
            return true;
        }
        return false;

    case MatrixID::FCC:
        *out = range_ == RangeID::FULL ? kFCC_Full_SkYUVColorSpace : kFCC_Limited_SkYUVColorSpace;
        return true;

    case MatrixID::SMPTE240M:
        *out = range_ == RangeID::FULL ? kSMPTE240_Full_SkYUVColorSpace : kSMPTE240_Limited_SkYUVColorSpace;
        return true;

    case MatrixID::YDZDX:
        *out = range_ == RangeID::FULL ? kYDZDX_Full_SkYUVColorSpace : kYDZDX_Limited_SkYUVColorSpace;
        return true;

    case MatrixID::GBR:
        *out = range_ == RangeID::FULL ? kGBR_Full_SkYUVColorSpace : kGBR_Limited_SkYUVColorSpace;
        return true;

    case MatrixID::YCOCG:
        if (bit_depth == 8) {
            *out = range_ == RangeID::FULL ? kYCgCo_8bit_Full_SkYUVColorSpace : kYCgCo_8bit_Limited_SkYUVColorSpace;
            return true;
        }
        if (bit_depth == 10) {
            *out = range_ == RangeID::FULL ? kYCgCo_10bit_Full_SkYUVColorSpace : kYCgCo_10bit_Limited_SkYUVColorSpace;
            return true;
        }
        if (bit_depth == 12) {
            *out = range_ == RangeID::FULL ? kYCgCo_12bit_Full_SkYUVColorSpace : kYCgCo_12bit_Limited_SkYUVColorSpace;
            return true;
        }
        if (bit_depth == 16) {
            *out = range_ == RangeID::FULL ? kYCgCo_16bit_Full_SkYUVColorSpace : kYCgCo_16bit_Limited_SkYUVColorSpace;
            return true;
        }
        return false;
    default:
