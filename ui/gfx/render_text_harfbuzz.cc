...
{
    return u_hasBinaryProperty(codepoint, UCHAR_EMOJI) || u_hasBinaryProperty(codepoint, UCHAR_EMOJI_PRESENTATION)
...
{
    return u_getIntPropertyValue(codepoint, UCHAR_BIDI_PAIRED_BRACKET_TYPE) != U_BPT_NONE;
}
...
std::vector<UScriptCode> GetScriptExtensions(UChar32 codepoint)
{
    ...
    size_t count = uscript_getScriptExtensions(codepoint, result.data(), result.size(), &icu_error);
...
void ScriptSetIntersect(UChar32 codepoint, std::vector<UScriptCode>& result)
{
    ...
    std::erase_if(result, [&](auto script) { return !base::Contains(extensions, script); });
}
...
GraphemeProperties RetrieveGraphemeProperties(std::u16string_view text, bool retrieve_block)
{
...
bool AreGraphemePropertiesCompatible(const GraphemeProperties& first, const GraphemeProperties& second)
{
    ...
    return !first.has_control && !second.has_control && first.has_bracket == second.has_bracket && first.has_pictographic == second.has_pictographic
...
size_t FindRunBreakingCharacter(const std::u16string& text, UScriptCode script, size_t run_start, size_t run_break, size_t run_end)
{
    ...
    base::i18n::BreakIterator grapheme_iterator(run_text, base::i18n::BreakIterator::BREAK_CHARACTER);
    if (!grapheme_iterator.Init() || !grapheme_iterator.Advance()) {
        NOTREACHED();
    }

    // Retrieve the first grapheme and its codepoint properties.
    const std::u16string_view first_grapheme_text = grapheme_iterator.GetStringView();
    const GraphemeProperties first_grapheme_properties = RetrieveGraphemeProperties(first_grapheme_text, is_common_script);
    ...
    while (grapheme_iterator.Advance()) {
        const std::u16string_view current_grapheme_text = grapheme_iterator.GetStringView();
        const GraphemeProperties current_grapheme_properties = RetrieveGraphemeProperties(current_grapheme_text, is_common_script);

        const size_t current_breaking_position = run_start + grapheme_iterator.prev();
        if (!AreGraphemePropertiesCompatible(first_grapheme_properties, current_grapheme_properties)) {
...
size_t ScriptInterval(const std::u16string& text, size_t start, size_t length, UScriptCode* script)
{
    DCHECK_GT(length, 0U);

    base::i18n::UTF16CharIterator char_iterator(std::u16string_view(text).substr(start, length));
...
bool FontWasAlreadyTried(sk_sp<SkTypeface> typeface, std::set<SkTypefaceID>* fallback_fonts)
{
    return fallback_fonts->count(typeface->uniqueID()) != 0;
}

void MarkFontAsTried(sk_sp<SkTypeface> typeface, std::set<SkTypefaceID>* fallback_fonts)
{
    fallback_fonts->insert(typeface->uniqueID());
}

// Whether |segment| corresponds to the newline character.
bool IsNewlineSegment(const std::u16string& text, const internal::LineSegment& segment)
{
...
size_t LineIndexForNewline(const size_t line_index, const std::u16string& text, const internal::LineSegment& segment, const SelectionModel& caret)
{
    bool at_newline = IsNewlineSegment(text, segment) && caret.caret_affinity() == CURSOR_BACKWARD;
...
bool GetClusterAtImpl(size_t pos, Range range, Iterator elements_begin, Iterator elements_end, bool reversed, Range* chars, Range* glyphs)
{
    ...
    glyphs->set_start(reversed ? elements_end - element : element - elements_begin);
    if (reversed)
...
public:
    HarfBuzzLineBreaker(size_t max_width, int min_baseline, float min_height, float glyph_height_for_test, WordWrapBehavior word_wrap_behavior,
        const std::u16string& text, const internal::TextRunList& run_list)
    ...
    void ConstructSingleLine()
    {
        for (size_t i = 0; i < run_list_->size(); i++) {
            ...
            segment.x_range = RangeF(SkScalarToFloat(text_x_), SkScalarToFloat(text_x_) + run.shape.width);
    ...
    void ConstructMultiLines()
    {
        ...
        base::i18n::BreakIterator words(*text_, base::i18n::BreakIterator::BREAK_LINE);
        ...
        while (words.Advance()) {
            ...
            bool new_line = false;
            if (!word_segments.empty() && IsNewlineSegment(*text_, word_segments.back())) {
                new_line = true;
...
private:
    ...
    void AdvanceLine()
    {
        if (!lines_.empty()) {
            ...
            std::sort(line->segments.begin(), line->segments.end(), [this](const internal::LineSegment& s1, const internal::LineSegment& s2) -> bool {
                return run_list_->logical_to_visual(s1.run) < run_list_->logical_to_visual(s2.run);
            });

            line->size.set_height(glyph_height_for_test_ ? glyph_height_for_test_ : std::max(min_height_, max_descent_ + max_ascent_));
            ...
            if (!line->segments.empty() && IsNewlineSegment(*text_, line->segments.back())) {
                line_width -= line->segments.back().width();
            }
            if (line->segments.size() > 1 && IsNewlineSegment(*text_, line->segments.front())) {
    ...
    void AddWordToLine(const std::vector<internal::LineSegment>& word_segments)
    {
        ...
        for (const internal::LineSegment& segment : word_segments) {
            ...
            if (IsNewlineSegment(*text_, segment) || segment.width() <= available_width_ || word_wrap_behavior_ == IGNORE_LONG_WORDS) {
                ...
                while (!remaining_segment.char_range.is_empty()) {
                    ...
                    SkScalar width = run.GetGlyphWidthForCharRange(Range(remaining_segment.char_range.start(), cutoff_pos));
                    if (remaining_segment.char_range.start() != cutoff_pos) {
                        ...
                        cut_segment.x_range = RangeF(SkScalarToFloat(text_x_), SkScalarToFloat(text_x_ + width));
    ...
    void AddLineSegment(const internal::LineSegment& segment, bool multiline)
    {
        ...
        if (!line->segments.empty()) {
            ...
            if (last_segment.run == segment.run) {
                ...
                DCHECK_LE(std::abs(last_segment.x_range.end() - segment.x_range.start()), 1.0f);
                last_segment.char_range.set_end(segment.char_range.end());
                last_segment.x_range.set_end(SkScalarToFloat(text_x_) + segment.width());
                if (run.font_params.is_rtl && last_segment.char_range.end() == run.range.end())
        ...
        if (!multiline || !IsNewlineSegment(*text_, segment)) {
            ...
            font.setEdging(run.font_params.render_params.antialiasing ? SkFont::Edging::kAntiAlias : SkFont::Edging::kAlias);
            SkFontMetrics metrics;
            font.getMetrics(&metrics);

            // max_descent_ is y-down, fDescent is y-down, baseline_offset is y-down
            max_descent_ = std::max(max_descent_, metrics.fDescent + run.font_params.baseline_offset);
            // max_ascent_ is y-up, fAscent is y-down, baseline_offset is y-down
            max_ascent_ = std::max(max_ascent_, -(metrics.fAscent + run.font_params.baseline_offset));
        }

        if (run.font_params.is_rtl) {
            rtl_segments_.push_back(SegmentHandle(lines_.size() - 1, line->segments.size() - 1));
    ...
    size_t GetCutoffPos(const internal::LineSegment& segment) const
    {
        ...
        const internal::TextRunHarfBuzz& run = *(run_list_->runs()[segment.run]).get();
        ...
        const size_t valid_end_pos = std::max(segment.char_range.start(), FindValidBoundaryBefore(*text_, end_pos));
        if (end_pos != valid_end_pos) {
            end_pos = valid_end_pos;
            width = run.GetGlyphWidthForCharRange(Range(segment.char_range.start(), end_pos));
        }
        ...
        if (width == 0 && available_width_ == max_width_ && end_pos < segment.char_range.end()) {
            end_pos = std::min(segment.char_range.end(), FindValidBoundaryAfter(*text_, end_pos + 1));
        }
    ...
    SkScalar GetWordWidth(const Range& word_range, std::vector<internal::LineSegment>* segments) const
    {
        ...
        for (size_t i = run_start_index; i <= run_end_index; i++) {
            ...
            segment.x_range = RangeF(SkScalarToFloat(text_x_ + width - char_width), SkScalarToFloat(text_x_ + width));
...
internal::TextRunHarfBuzz::FontParams CreateFontParams(
    const Font& primary_font, UBiDiLevel bidi_level, UScriptCode script, const internal::StyleIterator& style)
{
...
BASE_FEATURE(kRemoveFontLinkFallbacks, "RemoveFontLinkFallbacks", base::FEATURE_DISABLED_BY_DEFAULT);
...
BASE_FEATURE(kEnableFallbackFontsCrashReporting, "EnableFallbackFontsCrashReporting", base::FEATURE_DISABLED_BY_DEFAULT);
...
void AppendFontNameAndShapedTextToCrashDumpReport(
    const std::u16string& text, const std::vector<internal::TextRunHarfBuzz*>& shaped_runs, const std::string& font_name, std::u16string& report)
{
    ...
    for (internal::TextRunHarfBuzz* run : shaped_runs) {
        std::u16string text_substring = text.substr(run->range.start(), run->range.end());
...
sk_sp<SkTypeface> CreateSkiaTypeface(const Font& font, bool italic, Font::Weight weight)
{
#if BUILDFLAG(IS_APPLE)
...
#else
    SkFontStyle skia_style(static_cast<int>(weight), SkFontStyle::kNormal_Width, italic ? SkFontStyle::kItalic_Slant : SkFontStyle::kUpright_Slant);
...
TextRunHarfBuzz::FontParams::FontParams(const TextRunHarfBuzz::FontParams& other) = default;
TextRunHarfBuzz::FontParams& TextRunHarfBuzz::FontParams::operator=(const TextRunHarfBuzz::FontParams& other) = default;
...
bool TextRunHarfBuzz::FontParams::operator==(const FontParams& other) const
{
    ...
    return script == other.script && weight == other.weight && skia_face == other.skia_face && render_params == other.render_params
        && font_size == other.font_size && baseline_offset == other.baseline_offset && baseline_type == other.baseline_type && italic == other.italic
        && strike == other.strike && underline == other.underline && heavy_underline == other.heavy_underline && is_rtl == other.is_rtl && level == other.level
        && fill_style == other.fill_style && stroke_width == other.stroke_width;
}

void TextRunHarfBuzz::FontParams::ComputeRenderParamsFontSizeAndBaselineOffset()
{
    ...
    if (baseline_type != BaselineStyle::kNormalBaseline) {
        ...
        case BaselineStyle::kSuperior:
            baseline_offset = base::ClampRound(font.GetCapHeight() * ratio) - font.GetCapHeight();
            break;
...
size_t TextRunHarfBuzz::FontParams::Hash::operator()(const FontParams& key) const
{
    // In practice, |font|, |skia_face|, |render_params|, and |baseline_offset|
    // have not yet been set when this is called.
    return static_cast<size_t>(key.italic) << 0 ^ static_cast<size_t>(key.strike) << 1 ^ static_cast<size_t>(key.underline) << 2
        ^ static_cast<size_t>(key.heavy_underline) << 3 ^ static_cast<size_t>(key.is_rtl) << 4 ^ static_cast<size_t>(key.weight) << 8
        ^ static_cast<size_t>(key.font_size) << 12 ^ static_cast<size_t>(key.baseline_type) << 16 ^ static_cast<size_t>(key.level) << 20
        ^ static_cast<size_t>(key.script) << 24 ^ static_cast<size_t>(key.fill_style) << 28;
}

bool TextRunHarfBuzz::FontParams::SetRenderParamsRematchFont(const Font& new_font, const FontRenderParams& new_render_params)
{
    ...
    sk_sp<SkTypeface> new_skia_face(internal::CreateSkiaTypeface(new_font, italic, weight));
    if (!new_skia_face)
        return false;
...
bool TextRunHarfBuzz::FontParams::SetRenderParamsOverrideSkiaFaceFromFont(const Font& fallback_font, const FontRenderParams& new_render_params)
{
...
TextRunHarfBuzz::ShapeOutput::~ShapeOutput() = default;
TextRunHarfBuzz::ShapeOutput::ShapeOutput(const TextRunHarfBuzz::ShapeOutput& other) = default;
TextRunHarfBuzz::ShapeOutput& TextRunHarfBuzz::ShapeOutput::operator=(const TextRunHarfBuzz::ShapeOutput& other) = default;
TextRunHarfBuzz::ShapeOutput::ShapeOutput(TextRunHarfBuzz::ShapeOutput&& other) = default;
TextRunHarfBuzz::ShapeOutput& TextRunHarfBuzz::ShapeOutput::operator=(TextRunHarfBuzz::ShapeOutput&& other) = default;
...
Range TextRunHarfBuzz::CharRangeToGlyphRange(const Range& char_range) const
{
    ...
    return font_params.is_rtl ? Range(end_glyphs.start(), start_glyphs.end()) : Range(start_glyphs.start(), end_glyphs.end());
}
...
void TextRunHarfBuzz::GetClusterAt(size_t pos, Range* chars, Range* glyphs) const
{
    DCHECK(chars);
    DCHECK(glyphs);
    ...
    if (font_params.is_rtl) {
        success &= GetClusterAtImpl(pos, range, shape.glyph_to_char.rbegin(), shape.glyph_to_char.rend(), true, chars, glyphs);
    } else {
        success &= GetClusterAtImpl(pos, range, shape.glyph_to_char.begin(), shape.glyph_to_char.end(), false, chars, glyphs);
    }

    if (!success) {
        std::string glyph_to_char_string;
        for (size_t i = 0; i < shape.glyph_count && i < shape.glyph_to_char.size(); ++i) {
            glyph_to_char_string += base::NumberToString(i) + "->" + base::NumberToString(shape.glyph_to_char[i]) + ", ";
        }
        LOG(ERROR) << " TextRunHarfBuzz error, please report at crbug.com/724880:"
                   << " range: " << range.ToString() << ", rtl: " << font_params.is_rtl << ","
                   << " level: '" << font_params.level << "', script: " << font_params.script << ","
...
RangeF TextRunHarfBuzz::GetGraphemeBounds(RenderTextHarfBuzz* render_text, size_t text_index) const
{
    ...
    const float half_obscured_spacing = render_text->obscured() ? render_text->obscured_glyph_spacing() / 2.0f : 0.0f;
    const float cluster_begin_x = shape.positions[glyphs.start()].x() - half_obscured_spacing;
    const float cluster_end_x = glyphs.end() < shape.glyph_count ? shape.positions[glyphs.end()].x() - half_obscured_spacing : SkFloatToScalar(shape.width);
    ...
    ptrdiff_t code_point_count = UTF16IndexToOffset(render_text->GetDisplayText(), chars.start(), chars.end());
    if (code_point_count > 1) {
        ...
        if (total > 1) {
            ...
            return RangeF(cluster_start + average_width * before, cluster_start + average_width * (before + 1));
        }
    }

    return RangeF(preceding_run_widths + cluster_begin_x, preceding_run_widths + cluster_end_x);
}

RangeF TextRunHarfBuzz::GetGraphemeSpanForCharRange(RenderTextHarfBuzz* render_text, const Range& char_range) const
{
    ...
    size_t right_index = UTF16OffsetToIndex(render_text->GetDisplayText(), char_range.end(), -1);
    DCHECK_LE(left_index, right_index);
    if (font_params.is_rtl)
        std::swap(left_index, right_index);

    const RangeF left_span = GetGraphemeBounds(render_text, left_index);
    return left_index == right_index ? left_span : RangeF(left_span.start(), GetGraphemeBounds(render_text, right_index).end());
}

SkScalar TextRunHarfBuzz::GetGlyphWidthForCharRange(const Range& char_range) const
{
    if (char_range.is_empty())
        return 0;
    ...
    if (glyph_range.start() >= glyph_range.end()) {
        DUMP_WILL_BE_NOTREACHED() << "The glyph range is empty or invalid! Its char range: [" << char_range.start() << ", " << char_range.end()
                                  << "], and its glyph range: [" << glyph_range.start() << ", " << glyph_range.end() << "].";
        return 0;
    }

    return ((glyph_range.end() == shape.glyph_count) ? SkFloatToScalar(shape.width) : shape.positions[glyph_range.end()].x())
        - shape.positions[glyph_range.start()].x();
}

void TextRunHarfBuzz::UpdateFontParamsAndShape(const FontParams& new_font_params, const ShapeOutput& new_shape)
{
...
struct ShapeRunWithFontInput {
    ShapeRunWithFontInput(const std::u16string& full_text, const TextRunHarfBuzz::FontParams& font_params, Range full_range, bool obscured,
        float glyph_width_for_test, int obscured_glyph_spacing, bool subpixel_rendering_suppressed)
        ...
        , subpixel_rendering_suppressed(subpixel_rendering_suppressed)
    {
        ...
        size_t context_start = full_range.start() < kContextSize ? 0 : full_range.start() - kContextSize;
        size_t context_end = std::min(full_text.length(), full_range.end() + kContextSize);
        range = Range(full_range.start() - context_start, full_range.end() - context_start);
    ...
    {
        return text == other.text && skia_face == other.skia_face && render_params == other.render_params && font_size == other.font_size
            && range == other.range && script == other.script && is_rtl == other.is_rtl && obscured == other.obscured
            && glyph_width_for_test == other.glyph_width_for_test && obscured_glyph_spacing == other.obscured_glyph_spacing
...
using ShapeRunCacheBase = base::HashingLRUCache<ShapeRunWithFontInput, TextRunHarfBuzz::ShapeOutput, ShapeRunWithFontInput::Hash>;
class ShapeRunCache : public ShapeRunCacheBase {
public:
    ShapeRunCache()
        : ShapeRunCacheBase(kShapeRunCacheSize)
    {
    }
};
...
void ShapeRunWithFont(const ShapeRunWithFontInput& in, TextRunHarfBuzz::ShapeOutput* out)
{
    ...
    hb_font_t* harfbuzz_font = CreateHarfBuzzFont(in.skia_face, SkIntToScalar(in.font_size), in.render_params, in.subpixel_rendering_suppressed);
    ...
    hb_buffer_add_utf16(buffer, reinterpret_cast<const uint16_t*>(in.text.c_str()), static_cast<int>(in.text.length()), in.range.start(), in.range.length());
...
const std::u16string& RenderTextHarfBuzz::GetDisplayText()
{
    ...
    if ((multiline() && (max_lines() == 0 || elide_behavior() != ELIDE_TAIL)) || elide_behavior() == NO_ELIDE || elide_behavior() == FADE_TAIL) {
...
std::vector<Rect> RenderTextHarfBuzz::GetSubstringBounds(const Range& range)
{
    ...
    const Range display_range(TextIndexToDisplayIndex(grapheme_range.start()), TextIndexToDisplayIndex(grapheme_range.end()));
    ...
    const internal::ShapedText* shaped_text = GetShapedText();
    for (size_t line_index = 0; line_index < shaped_text->lines().size(); ++line_index) {
        const internal::Line& line = shaped_text->lines()[line_index];
        // Only the last line can be empty.
        DCHECK(!line.segments.empty() || (line_index == shaped_text->lines().size() - 1));
        float line_start_x = line.segments.empty() ? 0 : run_list->runs()[line.segments[0].run]->preceding_run_widths;
        ...
        for (const internal::LineSegment& segment : line.segments) {
            ...
            if (!intersection.is_empty()) {
                ...
                RangeF selected_span = run.GetGraphemeSpanForCharRange(this, intersection);
                ...
                Rect rect(start_x, 0, end_x - start_x, base::ClampCeil(line.size.height()));
                current_line_rects.push_back(rect + GetLineOffset(line_index));
            }
        }
        MergeIntersectingRects(current_line_rects);
        rects.insert(rects.end(), current_line_rects.begin(), current_line_rects.end());
    }
    return rects;
}
...
RangeF RenderTextHarfBuzz::GetCursorSpan(const Range& text_range)
{
    ...
    if (!IsValidCursorIndex(next_grapheme_start)) {
        next_grapheme_start = IndexOfAdjacentGrapheme(next_grapheme_start, CURSOR_FORWARD);
    }

    Range display_range(TextIndexToDisplayIndex(valid_range.start()), TextIndexToDisplayIndex(next_grapheme_start));
    DCHECK(IsValidDisplayRange(display_range));
    ...
    return run->font_params.is_rtl ? RangeF(bounds.end(), bounds.start()) : bounds;
}
...
SelectionModel RenderTextHarfBuzz::AdjacentCharSelectionModel(const SelectionModel& selection, VisualCursorDirection direction)
{
    DCHECK(!update_display_run_list_);
...
SelectionModel RenderTextHarfBuzz::AdjacentWordSelectionModel(const SelectionModel& selection, VisualCursorDirection direction)
{
    if (obscured())
        return EdgeSelectionModel(direction);
    ...
    for (;;) {
        ...
        if (iter.IsStartOfWord(cursor))
            break;
#else
        const bool is_forward = run_list->runs()[run]->font_params.is_rtl == (direction == CURSOR_LEFT);
...
SelectionModel RenderTextHarfBuzz::AdjacentLineSelectionModel(const SelectionModel& selection, VisualCursorDirection direction)
{
...
void RenderTextHarfBuzz::EnsureLayout()
{
    ...
    if (!has_shaped_text()) {
        ...
        HarfBuzzLineBreaker line_breaker(display_rect().width(), DetermineBaselineCenteringText(height, font_list()), height, glyph_height_for_test_,
            word_wrap_behavior(), GetDisplayText(), *run_list);
...
void RenderTextHarfBuzz::DrawVisualText(internal::SkiaTextRenderer* renderer, const std::vector<Range>& selections)
{
    ...
    for (auto selection : selections) {
        if (!selection.is_empty()) {
            ...
            colors.ApplyValue(selection_color(), Range(TextIndexToDisplayIndex(grapheme_range.GetMin()), TextIndexToDisplayIndex(grapheme_range.GetMax())));
        }
    }
    ...
    for (size_t i = 0; i < shaped_text->lines().size(); ++i) {
        ...
        for (const internal::LineSegment& segment : line.segments) {
            ...
            DEBUG_ALIAS_FOR_U16CSTR(alias_display_text, display_text.c_str(), crash_report_size);
            ...
            renderer->SetFontRenderParams(run.font_params.render_params, subpixel_rendering_suppressed());
            const Range glyphs_range = run.CharRangeToGlyphRange(segment.char_range);
            std::vector<SkPoint> positions(glyphs_range.length());
            SkScalar offset_x = preceding_segment_widths - ((glyphs_range.GetMin() != 0) ? run.shape.positions[glyphs_range.GetMin()].x() : 0);
            for (size_t j = 0; j < glyphs_range.length(); ++j) {
                positions[j] = run.shape.positions[(glyphs_range.is_reversed()) ? (glyphs_range.start() - j) : (glyphs_range.start() + j)];
                positions[j].offset(SkIntToScalar(origin.x()) + offset_x, SkIntToScalar(origin.y() + run.font_params.baseline_offset));
            }
            for (auto it = colors.GetBreak(segment.char_range.start()); it != colors.breaks().end() && it->first < segment.char_range.end(); ++it) {
                ...
                const size_t colored_pos = colored_glyphs.start() - glyphs_range.start();
                ...
                renderer->DrawPosText(
                    &positions[colored_glyphs.start() - glyphs_range.start()], &run.shape.glyphs[colored_glyphs.start()], colored_glyphs.length());
                int start_x = SkScalarRoundToInt(positions[colored_glyphs.start() - glyphs_range.start()].x());
                int end_x = SkScalarRoundToInt((colored_glyphs.end() == glyphs_range.end())
                ...
                if (run.font_params.strike)
                    renderer->DrawStrike(start_x, origin.y(), end_x - start_x, strike_thickness_factor());
            }
...
size_t RenderTextHarfBuzz::GetRunContainingCaret(const SelectionModel& caret)
{
    DCHECK(!update_display_run_list_);
    size_t layout_position = TextIndexToDisplayIndex(caret.caret_pos());
    LogicalCursorDirection affinity = caret.caret_affinity();
...
SelectionModel RenderTextHarfBuzz::FirstSelectionModelInsideRun(const internal::TextRunHarfBuzz* run)
{
...
SelectionModel RenderTextHarfBuzz::LastSelectionModelInsideRun(const internal::TextRunHarfBuzz* run)
{
...
bool RenderTextHarfBuzz::BuildResolvedTypefaceBreakList(internal::TextRunList* run_list)
{
        ...
        if (run->CountMissingGlyphs() > 0) {
            for (size_t offset = 0; offset < run->shape.glyphs.size(); ++offset) {
                ...
                if (run->shape.glyphs[offset] == kMissingGlyphId) {
                    ...
                    gfx::Range grapheme_range(ExpandRangeToGraphemeBoundary(gfx::Range(text_offset, text_offset + 1)));
                    ...
                    const std::u16string_view grapheme_text(&GetLayoutText()[display_range.start()], display_range.length());

                    Font fallback_font(primary_font);
                    const bool fallback_found = GetFallbackFont(primary_font, locale_, grapheme_text, &fallback_font);

                    // Only add to the breaklist if we found a fallback. This means that
                    // adjacent tofu will not be in isolated runs.
                    if (fallback_found) {
                        const SkTypefaceID fallback_font_id = fallback_font.platform_font()->GetNativeSkTypeface()->uniqueID();
                        if (layout_resolved_typefaces().ApplyValue(fallback_font_id, display_range)) {
...
void RenderTextHarfBuzz::ItemizeAndShapeText(const std::u16string& text, internal::TextRunList* run_list)
{
    CommonizedRunsMap commonized_run_map;
    const bool successfully_shaped_runs = ItemizeAndShapeTextImpl(&commonized_run_map, text, run_list);
    ...
    UMA_HISTOGRAM_COUNTS_1000("RenderTextHarfBuzz.MissingGlyphCount", run_list->MissingGlyphCount());
}

bool RenderTextHarfBuzz::ItemizeAndShapeTextImpl(CommonizedRunsMap* commonized_run_map, const std::u16string& text, internal::TextRunList* run_list)
{
    run_list->Reset();
    ...
    for (auto iter = commonized_run_map->begin(); iter != commonized_run_map->end(); ++iter) {
        internal::TextRunHarfBuzz::FontParams font_params = iter->first;
        font_params.ComputeRenderParamsFontSizeAndBaselineOffset();
        successfully_shaped_runs &= ShapeRuns(text, font_params, std::move(iter->second));
    }
    return successfully_shaped_runs;
}

void RenderTextHarfBuzz::ItemizeTextToRuns(const std::u16string& text, internal::TextRunList* out_run_list, CommonizedRunsMap* out_commonized_run_map)
{
    TRACE_EVENT1("ui", "RenderTextHarfBuzz::ItemizeTextToRuns", "text_length", text.length());
    DCHECK(!text.empty());
    const Font& primary_font = font_list().GetPrimaryFont();
    ...
    if (!bidi_iterator.Open(text, GetTextDirectionForGivenText(text))) {
        auto run = std::make_unique<internal::TextRunHarfBuzz>(font_list().GetPrimaryFont());
    ...
    for (size_t bidi_run_start = 0; bidi_run_start < text.length();) {
        ...
        for (size_t script_run_start = bidi_run_start; script_run_start < bidi_run_end;) {
            // Find the longest sequence of characters that have at least one common
            // UScriptCode value.
            UScriptCode script = USCRIPT_INVALID_CODE;
            size_t script_run_end = ScriptInterval(text, script_run_start, bidi_run_end - script_run_start, &script) + script_run_start;
            ...
            for (size_t breaking_run_start = script_run_start; breaking_run_start < script_run_end;) {
                ...
                size_t breaking_run_end = FindRunBreakingCharacter(text, script, breaking_run_start, text_style_end, script_run_end);
                ...
                internal::TextRunHarfBuzz::FontParams font_params = CreateFontParams(primary_font, bidi_level, script, style);
...
bool RenderTextHarfBuzz::ShapeRuns(
    const std::u16string& text, const internal::TextRunHarfBuzz::FontParams& font_params, std::vector<internal::TextRunHarfBuzz*> runs)
{
    ...
    for (internal::TextRunHarfBuzz*& run : runs) {
        if ((run->range.length() == 1 && (text[run->range.start()] == '\r' || text[run->range.start()] == '\n'))
    ...
    while (!runs.empty()) {
        ...
        internal::TextRunHarfBuzz* current_run = *runs.begin();
        {
            ...
            TRACE_EVENT1("ui", "RenderTextHarfBuzz::GetFallbackFont", "script", TRACE_STR_COPY(uscript_getShortName(font_params.script)));
            const std::u16string_view run_text(&text[current_run->range.start()], current_run->range.length());
            fallback_found = GetFallbackFont(primary_font, locale_, run_text, &fallback_font);
        }

        if (fallback_found) {
            internal::TextRunHarfBuzz::FontParams test_font_params = font_params;
            if (test_font_params.SetRenderParamsOverrideSkiaFaceFromFont(fallback_font, fallback_font.GetFontRenderParams())
                && !FontWasAlreadyTried(test_font_params.skia_face, &fallback_fonts_already_tried)) {
                ShapeRunsWithFont(text, test_font_params, &runs);
                MarkFontAsTried(test_font_params.skia_face, &fallback_fonts_already_tried);
            }
    ...
    if (!IsRemoveFontLinkFallbacks()) {
        ...
        std::u16string crash_report_string;
        {
            ...
            TRACE_EVENT1("ui", "RenderTextHarfBuzz::GetFallbackFonts", "script", TRACE_STR_COPY(uscript_getShortName(font_params.script)));
            ...
            for (const auto& fallback_font : fallback_font_candidates) {
                ...
                fallback_font_list.insert(fallback_font_list.end(), fallback_fonts.begin(), fallback_fonts.end());
            }
            ...
            if (!FontWasAlreadyTried(segoe.platform_font()->GetNativeSkTypeface(), &fallback_fonts_already_tried)) {
                std::vector<Font> default_fallback_families = GetFallbackFonts(segoe);
                fallback_font_list.insert(fallback_font_list.end(), default_fallback_families.begin(), default_fallback_families.end());
            }
#endif
        }

        // Use a set to track the fallback fonts and avoid duplicate entries.
        SCOPED_UMA_HISTOGRAM_LONG_TIMER("RenderTextHarfBuzz.ShapeRunsWithFallbackFontsTime");
        TRACE_EVENT1("ui", "RenderTextHarfBuzz::ShapeRunsWithFallbackFonts", "fonts_count", fallback_font_list.size());
        ...
        for (const auto& font : fallback_font_list) {
            ...
            FontRenderParams fallback_render_params = GetFontRenderParams(query, nullptr);
            internal::TextRunHarfBuzz::FontParams test_font_params = font_params;
            std::vector<internal::TextRunHarfBuzz*> fallback_fonts_shaped_runs;
            if (test_font_params.SetRenderParamsOverrideSkiaFaceFromFont(font, fallback_render_params)
                && !FontWasAlreadyTried(test_font_params.skia_face, &fallback_fonts_already_tried)) {
                ShapeRunsWithFont(text, test_font_params, &runs, &fallback_fonts_shaped_runs);
                MarkFontAsTried(test_font_params.skia_face, &fallback_fonts_already_tried);
                if (fallback_fonts_shaped_runs.size() > 0 && is_first_crash && IsEnableFallbackFontsCrashReporting()) {
                    AppendFontNameAndShapedTextToCrashDumpReport(text, fallback_fonts_shaped_runs, font_name, crash_report_string);
                }
            }
            if (runs.empty()) {
                TRACE_EVENT_INSTANT2("ui", "RenderTextHarfBuzz::FallbackFont", TRACE_EVENT_SCOPE_THREAD, "font_name", TRACE_STR_COPY(font_name.c_str()),
                ...
                if (is_first_crash && IsEnableFallbackFontsCrashReporting()) {
                    ...
                    const size_t crash_report_size = 256;
                    DEBUG_ALIAS_FOR_U16CSTR(aliased_crash_report_string, crash_report_string.c_str(), crash_report_size);
                    DEBUG_ALIAS_FOR_U16CSTR(aliased_full_text, text.c_str(), crash_report_size);
                    SCOPED_CRASH_KEY_STRING32("RenderTextFallbacks", "primaryfont_name", primary_font.GetFontName());
                    SCOPED_CRASH_KEY_STRING32("RenderTextFallbacks", "primaryfont_script", uscript_getShortName(font_params.script));
...
void RenderTextHarfBuzz::ShapeRunsWithFont(const std::u16string& text, const internal::TextRunHarfBuzz::FontParams& font_params,
    std::vector<internal::TextRunHarfBuzz*>* in_out_runs, std::vector<internal::TextRunHarfBuzz*>* successfully_shaped_runs)
{
    ...
    for (internal::TextRunHarfBuzz*& run : *in_out_runs) {
        ...
        bool can_use_cache = base::CurrentUIThread::IsSet() && run->range.length() <= kMaxRunLengthToCache;
        bool found_in_cache = false;
        const internal::ShapeRunWithFontInput cache_key(
            text, font_params, run->range, obscured(), glyph_width_for_test_, obscured_glyph_spacing(), subpixel_rendering_suppressed());
        if (can_use_cache) {
...
void RenderTextHarfBuzz::RecordShapeRunsFallback(internal::ShapeRunFallback fallback)
{
...
void RenderTextHarfBuzz::EmitShapeRunsFallback()
{
    if (last_shape_run_metric_.has_value()) {
        UMA_HISTOGRAM_ENUMERATION("RenderTextHarfBuzz.ShapeRunsFallback", last_shape_run_metric_.value());
    }
}

void RenderTextHarfBuzz::GetDecoratedTextForRange(const Range& text_range, DecoratedText* decorated_text)
{
    EnsureLayout();
    ...
    const Range display_range(TextIndexToDisplayIndex(text_range.start()), TextIndexToDisplayIndex(text_range.end()));
    ...
    for (size_t i = 0; i < run_list->size(); i++) {
        ...
        if (!intersection.is_empty()) {
            ...
            Range intersection_text_range = Range(DisplayIndexToTextIndex(intersection.start()), DisplayIndexToTextIndex(intersection.end()));
            DecoratedText::RangedAttribute attribute(
                Range(intersection_text_range.start() - text_range.GetMin(), intersection_text_range.end() - text_range.GetMin()),
