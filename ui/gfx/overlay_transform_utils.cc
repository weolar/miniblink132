...
Transform OverlayTransformToTransform(OverlayTransform overlay_transform, const SizeF& viewport_bounds)
{
    ...
    case OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_180:
        return Transform::Affine(-1, 0, 0, -1, viewport_bounds.width(), viewport_bounds.height());
    ...
    case OVERLAY_TRANSFORM_FLIP_VERTICAL_CLOCKWISE_270:
        return Transform::Affine(0, -1, -1, 0, viewport_bounds.height(), viewport_bounds.width());
    }
...
OverlayTransform OverlayTransformsConcat(OverlayTransform t1, OverlayTransform t2)
{
    ...
    VFlip result_vflip = static_cast<VFlip>(decomposed1.vflip != decomposed2.vflip);
    ...
    if (decomposed2.vflip == VFlip::kYes) {
        result_rotation = static_cast<Rotation>((decomposed2.rotation + 4 - decomposed1.rotation) % 4);
    } else {
        result_rotation = static_cast<Rotation>((decomposed2.rotation + decomposed1.rotation) % 4);
    }

    // Step 3: Reconstruct result.
    switch (result_rotation) {
    case Rotation::k0:
        return result_vflip == VFlip::kYes ? OVERLAY_TRANSFORM_FLIP_VERTICAL : OVERLAY_TRANSFORM_NONE;
    case Rotation::k90:
        return result_vflip == VFlip::kYes ? OVERLAY_TRANSFORM_FLIP_VERTICAL_CLOCKWISE_90 : OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90;
    case Rotation::k180:
        return result_vflip == VFlip::kYes ? OVERLAY_TRANSFORM_FLIP_HORIZONTAL : OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_180;
    case Rotation::k270:
        return result_vflip == VFlip::kYes ? OVERLAY_TRANSFORM_FLIP_VERTICAL_CLOCKWISE_270 : OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_270;
    }
