...
// static
std::unique_ptr<base::AutoReset<Animation::RichAnimationRenderMode>> AnimationTestApi::SetRichAnimationRenderMode(Animation::RichAnimationRenderMode mode)
{
    ...
    // unnecessary.
    if (Animation::rich_animation_rendering_mode_ != Animation::RichAnimationRenderMode::PLATFORM) {
        return nullptr;
    }
    return std::make_unique<base::AutoReset<Animation::RichAnimationRenderMode>>(&Animation::rich_animation_rendering_mode_, mode);
}
...
AnimationContainerTestApi::AnimationContainerTestApi(AnimationContainer* container)
...
{
    container_->runner_->SetAnimationTimeForTesting(container_->last_tick_time() + delta);
}
