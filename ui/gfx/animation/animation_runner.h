...
public:
    ...
    // animation.
    void Start(base::TimeDelta min_interval, base::TimeDelta elapsed, base::RepeatingCallback<void(base::TimeTicks)> step);

    // Called when subclasses don't need to call Step() anymore.
    virtual void Stop() = 0;

    bool step_is_null_for_testing() const
    {
        return step_.is_null();
    }
...
protected:
    ...
    // drive the animation.
    virtual void OnStart(base::TimeDelta min_interval, base::TimeDelta elapsed) = 0;
