...
{
    const double current_progress = direction_ == Direction::kShowing ? value_current_ : 1.0 - value_current_;
...
void SlideAnimation::AnimateToState(double state)
{
    ...
    if ((value_end_ >= value_start_) ? (value_current_ > value_end_) : (value_current_ < value_end_)) {
