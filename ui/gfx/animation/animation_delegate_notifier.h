...
template <class AnimationDelegateType = gfx::AnimationDelegate> class AnimationDelegateNotifier : public AnimationDelegateType {
public:
    template <typename... Args>
    AnimationDelegateNotifier(gfx::AnimationDelegate* owner, Args&&... args)
        : AnimationDelegateType(std::forward<Args>(args)...)
        , owner_(owner)
    {
        DCHECK(owner_);
    }
