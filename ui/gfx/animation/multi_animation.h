...
public:
    ...
    struct Part {
        Part(base::TimeDelta length, Tween::Type type, double start_value = 0.0, double end_value = 1.0)
            : length(length)
            , type(type)
    ...
    explicit MultiAnimation(const Parts& parts, base::TimeDelta timer_interval = kDefaultTimerInterval);
