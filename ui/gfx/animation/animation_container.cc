...
void AnimationContainer::Stop(AnimationContainerElement* element)
{
    ...
    if (!is_running()) {
        ...
        if (min_timer_interval_count_ == 0) {
            std::pair<base::TimeDelta, size_t> interval_count = GetMinIntervalAndCount();
...
void AnimationContainer::SetAnimationRunner(std::unique_ptr<AnimationRunner> runner)
{
    has_custom_animation_runner_ = !!runner;
    runner_ = has_custom_animation_runner_ ? std::move(runner) : AnimationRunner::CreateDefaultAnimationRunner();
    if (is_running())
...
{
    runner_->Start(min_timer_interval_, elapsed, base::BindRepeating(&AnimationContainer::Run, base::Unretained(this)));
}

std::pair<base::TimeDelta, size_t> AnimationContainer::GetMinIntervalAndCount() const
{
    DCHECK(is_running());
