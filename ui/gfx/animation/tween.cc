...
double Tween::CalculateValue(Tween::Type type, double state)
{
    ...
    case EASE_IN_OUT_EMPHASIZED:
        return gfx::ThreePointCubicBezier(0.05, 0, 0.133333, 0.06, 0.166666, 0.4, 0.208333, 0.82, 0.25, 1).Solve(state);
...
float BlendColorComponentsFloat(float start, float target, float start_alpha, float target_alpha, float blended_alpha, double progress)
{
    // Since progress can be outside [0, 1], blending can produce a value outside
    // [0, 1].
    float blended_premultiplied = Tween::FloatValueBetween(progress, start * start_alpha, target * target_alpha);
...
// static
SkColor4f Tween::ColorValueBetween(double value, SkColor4f start, SkColor4f target)
{
    ...
    auto blended_r = BlendColorComponentsFloat(start.fR, target.fR, start_a, target_a, blended_a, value);
    auto blended_g = BlendColorComponentsFloat(start.fG, target.fG, start_a, target_a, blended_a, value);
    auto blended_b = BlendColorComponentsFloat(start.fB, target.fB, start_a, target_a, blended_a, value);
...
// static
float Tween::ClampedFloatValueBetween(
    const base::TimeTicks& time, const base::TimeTicks& start_time, float start, const base::TimeTicks& target_time, float target)
{
    if (time <= start_time)
        return start;
    if (time >= target_time)
...
// static
gfx::Rect Tween::RectValueBetween(double value, const gfx::Rect& start, const gfx::Rect& target)
{
    ...
    const int bottom = LinearIntValueBetween(value, start.bottom(), target.bottom());
    return gfx::Rect(x, y, right - x, bottom - y);
}

// static
gfx::RectF Tween::RectFValueBetween(double value, const gfx::RectF& start, const gfx::RectF& target)
{
    ...
    const float bottom = FloatValueBetween(value, start.bottom(), target.bottom());
    return gfx::RectF(x, y, right - x, bottom - y);
}

// static
gfx::Transform Tween::TransformValueBetween(double value, const gfx::Transform& start, const gfx::Transform& target)
{
    if (value >= 1.0)
        return target;
    if (value <= 0.0)
...
// static
gfx::TransformOperations Tween::TransformOperationsValueBetween(double value, const gfx::TransformOperations& start, const gfx::TransformOperations& target)
{
    return target.Blend(start, value);
}

gfx::Size Tween::SizeValueBetween(double value, const gfx::Size& start, const gfx::Size& target)
{
    return gfx::Size(Tween::LinearIntValueBetween(value, start.width(), target.width()), Tween::LinearIntValueBetween(value, start.height(), target.height()));
}

gfx::SizeF Tween::SizeFValueBetween(double value, const gfx::SizeF& start, const gfx::SizeF& target)
{
    return gfx::SizeF(Tween::FloatValueBetween(value, start.width(), target.width()), Tween::FloatValueBetween(value, start.height(), target.height()));
}
