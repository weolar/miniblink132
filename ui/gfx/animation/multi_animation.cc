...
{
    return std::accumulate(parts.cbegin(), parts.cend(), base::TimeDelta(), [](base::TimeDelta total, const auto& part) { return total + part.length; });
}

// static
constexpr base::TimeDelta MultiAnimation::kDefaultTimerInterval;

MultiAnimation::MultiAnimation(const Parts& parts, base::TimeDelta timer_interval)
...
double MultiAnimation::GetCurrentValue() const
{
    ...
    return Tween::DoubleValueBetween(Tween::CalculateValue(current_part.type, current_part_state_), current_part.start_value, current_part.end_value);
}
...
void MultiAnimation::Step(base::TimeTicks time_now)
{
    ...
    if ((GetCurrentValue() != last_value || current_part_index_ != last_index) && delegate()) {
...
const MultiAnimation::Part& MultiAnimation::GetPart(base::TimeDelta* time, size_t* part_index)
{
