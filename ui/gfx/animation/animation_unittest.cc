...
public:
    CancelAnimation(base::TimeDelta duration, int frame_rate, AnimationDelegate* delegate)
...
public:
    EndAnimation(base::TimeDelta duration, int frame_rate, AnimationDelegate* delegate)
        : LinearAnimation(duration, frame_rate, delegate)
    {
    }
...
#if BUILDFLAG(IS_WIN)
    BOOL result;
    ASSERT_NE(0, ::SystemParametersInfo(SPI_GETCLIENTAREAANIMATION, 0, &result, 0));
...
{
    base::CommandLine::ForCurrentProcess()->AppendSwitchASCII(switches::kForcePrefersReducedMotion, "1");
