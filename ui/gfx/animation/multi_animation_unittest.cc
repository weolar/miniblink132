...
TEST(MultiAnimationTest, Basic)
{
    ...
    AnimationContainerElement* as_element = static_cast<AnimationContainerElement*>(&animation);
    ...
    EXPECT_DOUBLE_EQ(Tween::CalculateValue(Tween::EASE_OUT, .2), animation.GetCurrentValue());
...
public:
    ...
    double latest_current_value()
    {
        return latest_current_value_;
    }
...
TEST(MultiAnimationTest, GetCurrentValueDerivedFromStartAndEndOfCurrentPart)
{
    ...
    MultiAnimation::Parts parts;
    parts.push_back(MultiAnimation::Part(base::Milliseconds(100), Tween::LINEAR));
    parts.push_back(MultiAnimation::Part(base::Milliseconds(100), Tween::EASE_OUT, kSecondPartStart, kSecondPartEnd));
    ...
    const double current_animation_value = Tween::CalculateValue(Tween::EASE_OUT, .5);
    EXPECT_DOUBLE_EQ(Tween::DoubleValueBetween(current_animation_value, kSecondPartStart, kSecondPartEnd), animation.GetCurrentValue());
