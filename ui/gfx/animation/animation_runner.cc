...
void DefaultAnimationRunner::OnStart(base::TimeDelta min_interval, base::TimeDelta elapsed)
{
    min_interval_ = min_interval;
    timer_.Start(FROM_HERE, min_interval - elapsed, this, &DefaultAnimationRunner::OnTimerTick);
}
...
void DefaultAnimationRunner::OnTimerTick()
{
    ...
    timer_.Start(FROM_HERE, min_interval_, this, &DefaultAnimationRunner::OnTimerTick);
...
// static
std::unique_ptr<AnimationRunner> AnimationRunner::CreateDefaultAnimationRunner()
{
...
void AnimationRunner::Start(base::TimeDelta min_interval, base::TimeDelta elapsed, base::RepeatingCallback<void(base::TimeTicks)> step)
{
    step_ = std::move(step);
