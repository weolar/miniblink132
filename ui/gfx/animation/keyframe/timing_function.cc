...
std::unique_ptr<CubicBezierTimingFunction> CubicBezierTimingFunction::Create(double x1, double y1, double x2, double y2)
{
...
LinearTimingFunction::LinearTimingFunction(std::vector<LinearEasingPoint> points)
...
double LinearTimingFunction::GetValue(double input_progress, LimitDirection limit_direction) const
{
    ...
    auto it
        = std::upper_bound(points_.cbegin(), points_.cend(), input_progress, [](double progress, const auto& point) { return 100 * progress < point.input; });
