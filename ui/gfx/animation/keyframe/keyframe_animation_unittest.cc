...
public:
    ...
    const TransformOperations& layout_offset() const
    {
    ...
    void OnSizeAnimated(const SizeF& size, int target_property_id, KeyframeModel* keyframe_model) override
    {
        size_ = size;
    }

    void OnTransformAnimated(const TransformOperations& operations, int target_property_id, KeyframeModel* keyframe_model) override
    {
    ...
    void OnFloatAnimated(const float& opacity, int target_property_id, KeyframeModel* keyframe_model) override
    {
        opacity_ = opacity;
    }

    void OnColorAnimated(const SkColor& color, int target_property_id, KeyframeModel* keyframe_model) override
    {
        background_color_ = color;
    }

    void OnRectAnimated(const Rect& rect, int target_property_id, KeyframeModel* keyframe_model) override
    {
        rect_ = rect;
    }
...
TEST(KeyframeAnimationTest, AddRemoveKeyframeModels)
{
    ...
    animator.AddKeyframeModel(CreateSizeAnimation(&target, 1, kBoundsPropertyId, SizeF(10, 100), SizeF(20, 200), MicrosecondsToDelta(10000)));
    ...
    animator.AddKeyframeModel(CreateTransformAnimation(&target, 2, kTransformPropertyId, from_operations, to_operations, MicrosecondsToDelta(10000)));

    EXPECT_EQ(2ul, animator.keyframe_models().size());
    EXPECT_EQ(kTransformPropertyId, animator.keyframe_models()[1]->TargetProperty());

    animator.AddKeyframeModel(CreateTransformAnimation(&target, 3, kTransformPropertyId, from_operations, to_operations, MicrosecondsToDelta(10000)));
    EXPECT_EQ(3ul, animator.keyframe_models().size());
    EXPECT_EQ(kTransformPropertyId, animator.keyframe_models()[2]->TargetProperty());
...
TEST(KeyframeAnimationTest, AnimationLifecycle)
{
    ...
    EXPECT_EQ(1ul, animator.keyframe_models().size());
    EXPECT_EQ(kBoundsPropertyId, animator.keyframe_models()[0]->TargetProperty());
    EXPECT_EQ(KeyframeModel::WAITING_FOR_TARGET_AVAILABILITY, animator.keyframe_models()[0]->run_state());
...
TEST(KeyframeAnimationTest, AnimationQueue)
{
    ...
    animator.AddKeyframeModel(CreateSizeAnimation(&target, 2, kBoundsPropertyId, SizeF(10, 100), SizeF(20, 200), MicrosecondsToDelta(10000)));
    ...
    EXPECT_EQ(KeyframeModel::WAITING_FOR_TARGET_AVAILABILITY, animator.keyframe_models()[1]->run_state());
...
TEST(KeyframeAnimationTest, FinishedTransition)
{
    ...
    animator.TransitionFloatTo(&target, start_time + MsToDelta(1000), kOpacityPropertyId, target.opacity(), 1.0f);
...
TEST(KeyframeAnimationTest, OpacityTransitions)
{
    ...
    animator.TransitionFloatTo(&target, start_time, kOpacityPropertyId, from, nearby);
...
TEST(KeyframeAnimationTest, ReversedOpacityTransitions)
{
    ...
    animator.TransitionFloatTo(&target, start_time + MicrosecondsToDelta(1000), kOpacityPropertyId, target.opacity(), from);
    animator.Tick(start_time + MicrosecondsToDelta(1000));
...
TEST(KeyframeAnimationTest, RetargetOpacityTransition)
{
    ...
    std::unique_ptr<KeyframedFloatAnimationCurve> curve(gfx::KeyframedFloatAnimationCurve::Create());
    curve->AddKeyframe(FloatKeyframe::Create(base::TimeDelta(), 1.0f, nullptr));
    curve->AddKeyframe(FloatKeyframe::Create(MicrosecondsToDelta(10000), 0.0f, nullptr));
    curve->set_target(&target);
    animator.AddKeyframeModel(KeyframeModel::Create(std::move(curve), KeyframeEffect::GetNextKeyframeModelId(), kOpacityPropertyId));
    ...
    animator.GetKeyframeModel(kOpacityPropertyId)->Retarget(start_time + MicrosecondsToDelta(5000), kOpacityPropertyId, 1.f);
    animator.Tick(start_time + MicrosecondsToDelta(5000));
    EXPECT_FLOAT_EQ(0.5f, target.opacity());
...
TEST(KeyframeAnimationTest, RetargetTransitionBeforeLastKeyframe)
{
    ...
    std::unique_ptr<KeyframedFloatAnimationCurve> curve(gfx::KeyframedFloatAnimationCurve::Create());
    curve->AddKeyframe(FloatKeyframe::Create(base::TimeDelta(), 1.0f, nullptr));
    curve->AddKeyframe(FloatKeyframe::Create(MicrosecondsToDelta(5000), 0.5f, nullptr));
    curve->AddKeyframe(FloatKeyframe::Create(MicrosecondsToDelta(10000), 0.0f, nullptr));
    curve->set_target(&target);
    animator.AddKeyframeModel(KeyframeModel::Create(std::move(curve), KeyframeEffect::GetNextKeyframeModelId(), kOpacityPropertyId));
    ...
    animator.GetKeyframeModel(kOpacityPropertyId)->Retarget(start_time + MicrosecondsToDelta(4000), kOpacityPropertyId, 0.1f);
    animator.Tick(start_time + MicrosecondsToDelta(5000));
    EXPECT_FLOAT_EQ(0.5f, target.opacity());
...
TEST(KeyframeAnimationTest, LayoutOffsetTransitions)
{
    ...
    animator.TransitionTransformOperationsTo(&target, start_time, kLayoutOffsetPropertyId, from, to);
    ...
    animator.TransitionTransformOperationsTo(&target, start_time, kLayoutOffsetPropertyId, from, nearby);
    EXPECT_EQ(keyframe_model_id, animator.keyframe_models().front()->id());
...
TEST(KeyframeAnimationTest, TransformTransitions)
{
    ...
    animator.TransitionTransformOperationsTo(&target, start_time, kTransformPropertyId, from, to);
    ...
    TransformOperations nearby = to;
    nearby.at(0).translate.x += kNoise;
    animator.TransitionTransformOperationsTo(&target, start_time, kTransformPropertyId, from, nearby);
    EXPECT_EQ(keyframe_model_id, animator.keyframe_models().front()->id());
    ...
    animator.Tick(start_time + MicrosecondsToDelta(10000));
    EXPECT_TRUE(to.Apply().ApproximatelyEqual(target.operations().Apply(), tolerance));
}
...
TEST(KeyframeAnimationTest, ReversedTransformTransitions)
{
    ...
    animator.TransitionTransformOperationsTo(&target, start_time + MicrosecondsToDelta(1000), kTransformPropertyId, target.operations(), from);
    animator.Tick(start_time + MicrosecondsToDelta(1000));
    EXPECT_TRUE(value_before_reversing.Apply().ApproximatelyEqual(target.operations().Apply(), tolerance));

    animator.Tick(start_time + MicrosecondsToDelta(2000));
    EXPECT_TRUE(from.Apply().ApproximatelyEqual(target.operations().Apply(), tolerance));
}
...
TEST(KeyframeAnimationTest, RetargetTransformTransition)
{
    ...
    std::unique_ptr<KeyframedTransformAnimationCurve> curve(gfx::KeyframedTransformAnimationCurve::Create());
    curve->AddKeyframe(TransformKeyframe::Create(base::TimeDelta(), from, nullptr));
    curve->AddKeyframe(TransformKeyframe::Create(MicrosecondsToDelta(10000), to, nullptr));
    curve->set_target(&target);
    animator.AddKeyframeModel(KeyframeModel::Create(std::move(curve), KeyframeEffect::GetNextKeyframeModelId(), kTransformPropertyId));
    ...
    animator.GetKeyframeModel(kTransformPropertyId)->Retarget(start_time + MicrosecondsToDelta(5000), kTransformPropertyId, new_to);
    animator.Tick(start_time + MicrosecondsToDelta(5000));
    EXPECT_FLOAT_EQ(6.f, target.operations().at(0).scale.x);
    EXPECT_FLOAT_EQ(-5.f, target.operations().at(1).translate.x);
...
TEST(KeyframeAnimationTest, BoundsTransitions)
{
    ...
    animator.TransitionSizeTo(&target, start_time, kBoundsPropertyId, from, nearby);
    EXPECT_EQ(keyframe_model_id, animator.keyframe_models().front()->id());
...
TEST(KeyframeAnimationTest, RetargetSizeTransition)
{
    ...
    std::unique_ptr<KeyframedSizeAnimationCurve> curve(gfx::KeyframedSizeAnimationCurve::Create());
    curve->AddKeyframe(SizeKeyframe::Create(base::TimeDelta(), from, nullptr));
    curve->AddKeyframe(SizeKeyframe::Create(MicrosecondsToDelta(10000), to, nullptr));
    curve->set_target(&target);
    animator.AddKeyframeModel(KeyframeModel::Create(std::move(curve), KeyframeEffect::GetNextKeyframeModelId(), kBoundsPropertyId));
    ...
    animator.GetKeyframeModel(kBoundsPropertyId)->Retarget(start_time + MicrosecondsToDelta(5000), kRectPropertyId, new_to);
    animator.Tick(start_time + MicrosecondsToDelta(5000));
    EXPECT_SIZEF_EQ(SizeF(6, 12), target.size());
...
TEST(KeyframeAnimationTest, ReversedBoundsTransitions)
{
    ...
    animator.TransitionSizeTo(&target, start_time + MicrosecondsToDelta(1000), kBoundsPropertyId, target.size(), from);
    animator.Tick(start_time + MicrosecondsToDelta(1000));
...
TEST(KeyframeAnimationTest, BackgroundColorTransitions)
{
    ...
    animator.TransitionColorTo(&target, start_time, kBackgroundColorPropertyId, from, to);
...
TEST(KeyframeAnimationTest, ReversedBackgroundColorTransitions)
{
    ...
    animator.TransitionColorTo(&target, start_time + MicrosecondsToDelta(1000), kBackgroundColorPropertyId, target.background_color(), from);
    animator.Tick(start_time + MicrosecondsToDelta(1000));
...
TEST(KeyframeAnimationTest, RetargetColorTransition)
{
    ...
    std::unique_ptr<KeyframedColorAnimationCurve> curve(gfx::KeyframedColorAnimationCurve::Create());
    curve->AddKeyframe(ColorKeyframe::Create(base::TimeDelta(), from, nullptr));
    curve->AddKeyframe(ColorKeyframe::Create(MicrosecondsToDelta(10000), to, nullptr));
    curve->set_target(&target);
    animator.AddKeyframeModel(KeyframeModel::Create(std::move(curve), KeyframeEffect::GetNextKeyframeModelId(), kBackgroundColorPropertyId));
    ...
    animator.GetKeyframeModel(kBackgroundColorPropertyId)->Retarget(start_time + MicrosecondsToDelta(5000), kRectPropertyId, new_to);
    animator.Tick(start_time + MicrosecondsToDelta(5000));
    EXPECT_EQ(5u, SkColorGetR(target.background_color()));
...
TEST(KeyframeAnimationTest, DoubleReversedTransitions)
{
    ...
    animator.TransitionFloatTo(&target, start_time + MicrosecondsToDelta(1500), kOpacityPropertyId, target.opacity(), to);
    animator.Tick(start_time + MicrosecondsToDelta(1500));
    EXPECT_FLOAT_EQ(value_before_reversing, target.opacity());
}
...
TEST(KeyframeAnimationTest, RedundantTransition)
{
    ...
    animator.TransitionFloatTo(&target, start_time, kOpacityPropertyId, target.opacity(), to);
...
TEST(KeyframeAnimationTest, CorrectTargetValue)
{
    ...
    EXPECT_EQ(from_opacity, animator.GetTargetFloatValue(kOpacityPropertyId, from_opacity));
    EXPECT_SIZEF_EQ(from_bounds, animator.GetTargetSizeValue(kBoundsPropertyId, from_bounds));
    EXPECT_EQ(from_color, animator.GetTargetColorValue(kBackgroundColorPropertyId, from_color));
    EXPECT_TRUE(from_transform.ApproximatelyEqual(animator.GetTargetTransformOperationsValue(kTransformPropertyId, from_transform), kEpsilon));

    // Add keyframe_models.
    animator.AddKeyframeModel(CreateFloatAnimation(&target, 2, kOpacityPropertyId, from_opacity, to_opacity, duration));
    animator.AddKeyframeModel(CreateSizeAnimation(&target, 1, kBoundsPropertyId, from_bounds, to_bounds, duration));
    animator.AddKeyframeModel(CreateColorAnimation(&target, 3, kBackgroundColorPropertyId, from_color, to_color, duration));
    animator.AddKeyframeModel(CreateTransformAnimation(&target, 4, kTransformPropertyId, from_transform, to_transform, duration));

    base::TimeTicks start_time = MicrosecondsToTicks(1000000);
    animator.Tick(start_time);

    // Verify target value.
    EXPECT_EQ(to_opacity, animator.GetTargetFloatValue(kOpacityPropertyId, from_opacity));
    EXPECT_SIZEF_EQ(to_bounds, animator.GetTargetSizeValue(kBoundsPropertyId, from_bounds));
    EXPECT_EQ(to_color, animator.GetTargetColorValue(kBackgroundColorPropertyId, from_color));
    EXPECT_TRUE(to_transform.ApproximatelyEqual(animator.GetTargetTransformOperationsValue(kTransformPropertyId, from_transform), kEpsilon));
}
...
TEST(KeyframeAnimationTest, RetargetRectTransition)
{
    ...
    std::unique_ptr<KeyframedRectAnimationCurve> curve(gfx::KeyframedRectAnimationCurve::Create());
    curve->AddKeyframe(RectKeyframe::Create(base::TimeDelta(), from, nullptr));
    curve->AddKeyframe(RectKeyframe::Create(MicrosecondsToDelta(10000), to, nullptr));
    curve->set_target(&target);
    animator.AddKeyframeModel(KeyframeModel::Create(std::move(curve), KeyframeEffect::GetNextKeyframeModelId(), kRectPropertyId));
    ...
    animator.GetKeyframeModel(kRectPropertyId)->Retarget(start_time + MicrosecondsToDelta(5000), kRectPropertyId, new_to);
    animator.Tick(start_time + MicrosecondsToDelta(5000));
    EXPECT_EQ(Rect(6, 12, 18, 24), target.rect());
