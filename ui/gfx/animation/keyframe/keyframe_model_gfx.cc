...
static constexpr auto s_runStateNames = std::to_array<const char*>(
    { "WAITING_FOR_TARGET_AVAILABILITY", "WAITING_FOR_DELETION", "STARTING", "RUNNING", "PAUSED", "FINISHED", "ABORTED", "ABORTED_BUT_NEEDS_COMPLETION" });

static_assert(static_cast<int>(KeyframeModel::LAST_RUN_STATE) + 1 == std::size(s_runStateNames),
...
std::unique_ptr<KeyframeModel> KeyframeModel::Create(std::unique_ptr<AnimationCurve> curve, int keyframe_model_id, int target_property_id)
{
    return base::WrapUnique(new KeyframeModel(std::move(curve), keyframe_model_id, target_property_id));
}

KeyframeModel::KeyframeModel(std::unique_ptr<AnimationCurve> curve, int keyframe_model_id, int target_property_id)
...
void KeyframeModel::SetRunState(RunState run_state, base::TimeTicks monotonic_time)
{
...
void KeyframeModel::Pause(base::TimeDelta pause_offset)
{
    ...
    base::TimeTicks monotonic_time = pause_offset + start_time_.value_or(base::TimeTicks()) + total_paused_duration_;
    SetRunState(PAUSED, monotonic_time);
}

KeyframeModel::Phase KeyframeModel::CalculatePhaseForTesting(base::TimeDelta local_time) const
{
...
{
    base::TimeDelta opposite_time_offset = time_offset_ == base::TimeDelta::Min() ? base::TimeDelta::Max() : -time_offset_;
    base::TimeDelta before_active_boundary_time = std::max(opposite_time_offset, base::TimeDelta());
    if ((local_time < before_active_boundary_time) || (local_time == before_active_boundary_time && playback_rate_ < 0)) {
    ...
    if (std::isfinite(iterations_)) {
        ...
        base::TimeDelta active_duration = curve_->Duration() * iterations_ / std::abs(playback_rate_);
        active_after_boundary_time = std::max(opposite_time_offset + active_duration, base::TimeDelta());
    }
    if ((local_time > active_after_boundary_time) || (local_time == active_after_boundary_time && playback_rate_ > 0)) {
...
std::optional<base::TimeDelta> KeyframeModel::CalculateActiveTime(base::TimeDelta local_time, KeyframeModel::Phase phase) const
{
    ...
    case KeyframeModel::Phase::AFTER:
        if (fill_mode_ == FillMode::FORWARDS || fill_mode_ == FillMode::BOTH) {
            ...
            base::TimeDelta active_duration = curve_->Duration() * iterations_ / std::abs(playback_rate_);
            return std::max(std::min(local_time + time_offset_, active_duration), base::TimeDelta());
        }
        return std::nullopt;
    default:
        NOTREACHED();
...
bool KeyframeModel::IsFinishedAt(base::TimeTicks monotonic_time) const
{
    ...
    return run_state_ == RUNNING && std::isfinite(iterations_)
        && (curve_->Duration() * (iterations_ / std::abs(playback_rate_))) <= (ConvertMonotonicTimeToLocalTime(monotonic_time) + time_offset_);
}
...
base::TimeDelta KeyframeModel::TrimTimeToCurrentIteration(base::TimeTicks monotonic_time, TimingFunction::LimitDirection* limit_direction) const
{
    DCHECK(playback_rate_);
    ...
    base::TimeDelta repeated_duration = std::isfinite(iterations_) ? (curve_->Duration() * iterations_) : base::TimeDelta::Max();

    // Calculate the scaled active time
    base::TimeDelta scaled_active_time;
    if (playback_rate_ < 0) {
        DCHECK(std::isfinite(iterations_));
        base::TimeDelta active_duration = repeated_duration / std::abs(playback_rate_);
        scaled_active_time = ((active_time - active_duration) * playback_rate_) + start_offset;
    } else {
    ...
    bool has_defined_time_delta = (start_offset != scaled_active_time) || !(start_offset.is_max() || start_offset.is_min());
    if (has_defined_time_delta && scaled_active_time - start_offset == repeated_duration && fmod(iterations_ + iteration_start_, 1) == 0)
    ...
    // current iteration
    bool reverse = (direction_ == Direction::REVERSE) || (direction_ == Direction::ALTERNATE_NORMAL && iteration % 2 == 1)
...
base::TimeDelta KeyframeModel::ConvertMonotonicTimeToLocalTime(base::TimeTicks monotonic_time) const
{
    ...
    // the initial state.
    if ((run_state_ == STARTING && !has_set_start_time()) || StartShouldBeDeferred())
        return base::TimeDelta();

    // If we're paused, time is 'stuck' at the pause time.
    base::TimeTicks time = (run_state_ == PAUSED) ? pause_time_ : monotonic_time;
    return time - start_time_.value_or(base::TimeTicks()) - total_paused_duration_;
}
