...
public:
    const gfx::Font& GetFont(SystemFont system_font)
    {
        ...
        CHECK(it != system_fonts_.end(), base::NotFatalUntil::M130) << "System font #" << static_cast<int>(system_font) << " not found!";
        return it->second;
    }
    ...
    static void AdjustLOGFONT(const FontAdjustment& font_adjustment, LOGFONT* logfont)
    {
        DCHECK_GT(font_adjustment.font_scale, 0.0);
        LONG new_height = base::ClampRound<LONG>(logfont->lfHeight * font_adjustment.font_scale);
        ...
        if (!font_adjustment.font_family_override.empty()) {
            auto result = wcscpy_s(logfont->lfFaceName, font_adjustment.font_family_override.c_str());
            DCHECK_EQ(0, result) << "Font name " << font_adjustment.font_family_override << " cannot be copied into LOGFONT structure.";
        }
    }
    ...
    static Font GetFontFromLOGFONT(const LOGFONT& logfont)
    {
        ...
        DCHECK(font.get()) << "Font for '" << base::SysWideToUTF8(logfont.lfFaceName) << "' has an invalid handle.";
        ...
        const int font_size = std::max<int>(1, mapped_font_metrics.tmHeight - mapped_font_metrics.tmInternalLeading);

        Font system_font = Font(PlatformFont::CreateFromNameAndSize(font_name, font_size));

        // System fonts may have different styles when they are manually changed by
        // the users (see crbug.com/989476).
        Font::FontStyle style = logfont.lfItalic == 0 ? Font::FontStyle::NORMAL : Font::FontStyle::ITALIC;
        Font::Weight weight = logfont.lfWeight == 0 ? Font::Weight::NORMAL : static_cast<Font::Weight>(logfont.lfWeight);
    ...
    static void SetGetMinimumFontSizeCallback(GetMinimumFontSizeCallback callback)
    {
...
private:
    ...
    void Initialize()
    {
        ...
        AddFont(SystemFont::kSmallCaption, font_adjustment, &metrics.lfSmCaptionFont);
    ...
    void AddFont(SystemFont system_font, const FontAdjustment& font_adjustment, LOGFONT* logfont)
    {
...
// static
GetMinimumFontSizeCallback SystemFonts::get_minimum_font_size_callback_ = nullptr;
...
void AdjustLOGFONTForTesting(const FontAdjustment& font_adjustment, LOGFONT* logfont)
{
