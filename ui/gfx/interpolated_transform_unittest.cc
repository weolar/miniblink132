...
TEST(InterpolatedTransformTest, InterpolatedRotation)
{
    ...
    ui::InterpolatedRotation interpolated_rotation_diff_start_end(0, 100, 100, 200);
...
{
    ui::InterpolatedScale interpolated_scale(gfx::Point3F(0, 0, 0), gfx::Point3F(100, 100, 100));
    ui::InterpolatedScale interpolated_scale_diff_start_end(gfx::Point3F(0, 0, 0), gfx::Point3F(100, 100, 100), 100, 200);
...
{
    ui::InterpolatedTranslation interpolated_xform(gfx::PointF(), gfx::PointF(100.f, 100.f));

    ui::InterpolatedTranslation interpolated_xform_diff_start_end(gfx::PointF(), gfx::PointF(100.f, 100.f), 100, 200);
...
{
    ui::InterpolatedTranslation interpolated_xform(gfx::Point3F(0, 0, 0), gfx::Point3F(100, 100, 100));

    ui::InterpolatedTranslation interpolated_xform_diff_start_end(gfx::Point3F(0, 0, 0), gfx::Point3F(100, 100, 100), 100, 200);
...
TEST(InterpolatedTransformTest, InterpolatedRotationAboutPivot)
{
    ...
    ui::InterpolatedTransformAboutPivot interpolated_xform(pivot, std::make_unique<ui::InterpolatedRotation>(0, 90));
...
TEST(InterpolatedTransformTest, InterpolatedScaleAboutPivot)
{
    gfx::Point pivot(100, 100);
    gfx::Point above_pivot(100, 200);
    ui::InterpolatedTransformAboutPivot interpolated_xform(pivot, std::make_unique<ui::InterpolatedScale>(gfx::Point3F(1, 1, 1), gfx::Point3F(2, 2, 2)));
    gfx::Transform result = interpolated_xform.Interpolate(0.0f);
    EXPECT_TRANSFORM_EQ(gfx::Transform(), result);
    result = interpolated_xform.Interpolate(1.0f);
    gfx::Point expected_result = pivot;
...
ui::InterpolatedTransform* GetScreenRotation(int degrees, bool reversed)
{
    ...
    std::unique_ptr<ui::InterpolatedTransform> rotation = std::make_unique<ui::InterpolatedTransformAboutPivot>(
        old_pivot, std::make_unique<ui::InterpolatedRotation>(reversed ? degrees : 0, reversed ? 0 : degrees));

    std::unique_ptr<ui::InterpolatedTransform> translation
        = std::make_unique<ui::InterpolatedTranslation>(gfx::PointF(), gfx::PointF(new_pivot.x() - old_pivot.x(), new_pivot.y() - old_pivot.y()));

    float scale_factor = 0.9f;
    std::unique_ptr<ui::InterpolatedTransform> scale_down = std::make_unique<ui::InterpolatedScale>(1.0f, scale_factor, 0.0f, 0.5f);

    std::unique_ptr<ui::InterpolatedTransform> scale_up = std::make_unique<ui::InterpolatedScale>(1.0f, 1.0f / scale_factor, 0.5f, 1.0f);

    std::unique_ptr<ui::InterpolatedTransform> to_return = std::make_unique<ui::InterpolatedConstantTransform>(gfx::Transform());
...
TEST(InterpolatedTransformTest, ScreenRotationEndsCleanly)
{
    for (int i = 0; i < 2; ++i) {
        for (int degrees = -360; degrees <= 360; degrees += 90) {
            ...
            std::unique_ptr<ui::InterpolatedTransform> screen_rotation(GetScreenRotation(degrees, reversed));
...
ui::InterpolatedTransform* GetMaximize()
{
    ...
    float scale_x = static_cast<float>(target_bounds.height()) / initial_bounds.width();
    float scale_y = static_cast<float>(target_bounds.width()) / initial_bounds.height();

    std::unique_ptr<ui::InterpolatedTransform> scale = std::make_unique<ui::InterpolatedScale>(gfx::Point3F(1, 1, 1), gfx::Point3F(scale_x, scale_y, 1));

    std::unique_ptr<ui::InterpolatedTransform> translation = std::make_unique<ui::InterpolatedTranslation>(
        gfx::PointF(), gfx::PointF(target_bounds.x() - initial_bounds.x(), target_bounds.y() - initial_bounds.y()));

    std::unique_ptr<ui::InterpolatedTransform> rotation = std::make_unique<ui::InterpolatedRotation>(0, 4.0f);

    std::unique_ptr<ui::InterpolatedTransform> rotation_about_pivot(
        std::make_unique<ui::InterpolatedTransformAboutPivot>(gfx::Point(initial_bounds.width() * 0.5, initial_bounds.height() * 0.5), std::move(rotation)));
