...
public:
    ...
    constexpr GenericSharedMemoryId()
        : id(-1)
    {
    }
    constexpr explicit GenericSharedMemoryId(int id)
        : id(id)
    {
    }
...
base::trace_event::MemoryAllocatorDumpGuid GetGenericSharedGpuMemoryGUIDForTracing(uint64_t tracing_process_id, GenericSharedMemoryId generic_shared_memory_id);

} // namespace gfx

namespace std {

template <> struct hash<gfx::GenericSharedMemoryId> {
...
template <typename Second> struct hash<std::pair<gfx::GenericSharedMemoryId, Second>> {
    size_t operator()(const std::pair<gfx::GenericSharedMemoryId, Second>& pair) const
    {
