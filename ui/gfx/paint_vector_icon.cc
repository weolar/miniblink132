...
struct CompareIconDescription {
    bool operator()(const IconDescription& a, const IconDescription& b) const
    {
        ...
        return std::tie(a_icon, a.dip_size, a.color, a_badge) < std::tie(b_icon, b.dip_size, b.color, b_badge);
    }
};
...
class PathParser {
public:
    PathParser(base::span<const PathElement> elements)
        : elements_(elements)
    {
    }
...
std::vector<PathElement> PathFromSource(const std::string& source)
{
    ...
    std::vector<std::string> pieces = base::SplitString(source, "\n ,", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
{
    return command == CUBIC_TO || command == R_CUBIC_TO || command == CUBIC_TO_SHORTHAND || command == QUADRATIC_TO || command == R_QUADRATIC_TO
        || command == QUADRATIC_TO_SHORTHAND || command == R_QUADRATIC_TO_SHORTHAND;
}

void PaintPath(Canvas* canvas, const base::span<const PathElement>& path_elements, int dip_size, SkColor color)
{
    ...
    for (PathParser parser(path_elements); parser.HasCommandsRemaining(); parser.Advance()) {
        ...
        case PATH_COLOR_ARGB:
            flags.setColor(SkColorSetARGB(SkScalarFloorToInt(arg(0)), SkScalarFloorToInt(arg(1)), SkScalarFloorToInt(arg(2)), SkScalarFloorToInt(arg(3))));
            break;
        ...
        case R_ARC_TO: {
            ...
            SkScalar x = arg(5);
            SkScalar y = arg(6);
            SkPath::ArcSize arc_size = large_arc_flag ? SkPath::kLarge_ArcSize : SkPath::kSmall_ArcSize;
            SkPathDirection direction = arc_sweep_flag ? SkPathDirection::kCW : SkPathDirection::kCCW;
        ...
        case ROUND_RECT:
            path.addRoundRect(SkRect::MakeXYWH(arg(0), arg(1), arg(2), arg(3)), arg(4), arg(4));
            break;
...
public:
    explicit VectorIconSource(const IconDescription& data)
        : CanvasImageSource(Size(data.dip_size, data.dip_size))
        , data_(data)
    {
    }
    ...
    void Draw(Canvas* canvas) override
    {
        if (path_.empty()) {
            ...
            base::span<const PathElement> path_elements(path_.data(), path_.size());
            PaintPath(canvas, path_elements, size_.width(), data_.color);
        }
    }
...
public:
    ...
    ImageSkia GetOrCreateIcon(const IconDescription& description)
    {
        ...
        ImageSkia icon_image(std::make_unique<VectorIconSource>(description), Size(description.dip_size, description.dip_size));
...
static base::LazyInstance<VectorIconCache>::DestructorAtExit g_icon_cache = LAZY_INSTANCE_INITIALIZER;

} // namespace

IconDescription::IconDescription(const IconDescription& other) = default;

IconDescription::IconDescription(const VectorIcon& icon, int dip_size, SkColor color, const VectorIcon* badge_icon)
    : icon(icon)
    , dip_size(dip_size)
    , color(color)
...
void PaintVectorIcon(Canvas* canvas, const VectorIcon& icon, int dip_size, SkColor color)
{
    CHECK(!icon.is_empty());
...
ImageSkia CreateVectorIconWithBadge(const VectorIcon& icon, int dip_size, SkColor color, const VectorIcon& badge_icon)
{
    return CreateVectorIcon(IconDescription(icon, dip_size, color, &badge_icon));
}

ImageSkia CreateVectorIconFromSource(const std::string& source, int dip_size, SkColor color)
{
    return CanvasImageSource::MakeImageSkia<VectorIconSource>(source, dip_size, color);
}
