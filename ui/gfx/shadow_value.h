...
public:
    ...
    constexpr const gfx::Vector2d& offset() const
    {
        return offset_;
    }
    constexpr double blur() const
    {
        return blur_;
    }
    constexpr SkColor color() const
    {
        return color_;
    }
    ...
    {
        return offset_ == other.offset_ && blur_ == other.blur_ && color_ == other.color_;
    }

    constexpr bool operator<(const ShadowValue& other) const
    {
        return std::make_tuple(x(), y(), blur_, color_) < std::make_tuple(other.x(), other.y(), other.blur_, other.color_);
    }
    ...
    static ShadowValues MakeShadowValues(int elevation, SkColor key_shadow_color, SkColor ambient_shadow_color);
    // Makes ShadowValues for MD shadows. This style is deprecated.
    static ShadowValues MakeMdShadowValues(int elevation, SkColor color = SK_ColorBLACK);
    // Makes ShadowValues for MD shadows with customized key and ambient colors.
    static ShadowValues MakeMdShadowValues(int elevation, SkColor key_shadow_color, SkColor ambient_shadow_color);

#if BUILDFLAG(IS_CHROMEOS)
    // Makes ShadowValues for Chrome OS UI components with default colors.
    static ShadowValues MakeChromeOSSystemUIShadowValues(int elevation, SkColor color = SK_ColorBLACK);
    // Makes ShadowValues for chrome OS UI components with customized key and
    // ambient colors.
    static ShadowValues MakeChromeOSSystemUIShadowValues(int elevation, SkColor key_shadow_color, SkColor ambient_shadow_color);
#endif
