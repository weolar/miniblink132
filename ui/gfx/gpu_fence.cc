...
// static
GpuFence::FenceStatus GpuFence::GetStatusChangeTime(int fd, base::TimeTicks* time)
{
    DCHECK_NE(fd, -1);
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_ANDROID)
    auto info = std::unique_ptr<sync_fence_info_data, void (*)(sync_fence_info_data*)> { sync_fence_info(fd), sync_fence_info_free };
    if (!info) {
