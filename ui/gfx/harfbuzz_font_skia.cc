...
struct FontData {
    explicit FontData(GlyphCache* glyph_cache)
...
template <typename Type> void DeleteByType(void* data)
{
    Type* typed_data = reinterpret_cast<Type*>(data);
    delete typed_data;
}

template <typename Type> void DeleteArrayByType(void* data)
{
    Type* typed_data = reinterpret_cast<Type*>(data);
    delete[] typed_data;
}
...
// |paint|'s font.
void GetGlyphWidthAndExtents(const SkFont& font, hb_codepoint_t codepoint, hb_position_t* width, hb_glyph_extents_t* extents)
{
...
hb_bool_t GetGlyph(hb_font_t* font, void* data, hb_codepoint_t unicode, hb_codepoint_t variation_selector, hb_codepoint_t* glyph, void* user_data)
{
    ...
    if (iter == cache->end()) {
        auto result = cache->insert(std::make_pair(unicode, font_data->font_.unicharToGlyph(unicode)));
...
hb_bool_t GetNominalGlyph(hb_font_t* font, void* data, hb_codepoint_t unicode, hb_codepoint_t* glyph, void* user_data)
{
    return GetGlyph(font, data, unicode, 0, glyph, user_data);
}

// Returns the horizontal advance value of the |glyph|.
hb_position_t GetGlyphHorizontalAdvance(hb_font_t* font, void* data, hb_codepoint_t glyph, void* user_data)
{
    FontData* font_data = reinterpret_cast<FontData*>(data);
...
hb_bool_t GetGlyphHorizontalOrigin(hb_font_t* font, void* data, hb_codepoint_t glyph, hb_position_t* x, hb_position_t* y, void* user_data)
{
    // Just return true, like the HarfBuzz-FreeType implementation.
    return true;
}

hb_position_t GetGlyphKerning(FontData* font_data, hb_codepoint_t first_glyph, hb_codepoint_t second_glyph)
{
    SkTypeface* typeface = font_data->font_.getTypeface();
    const uint16_t glyphs[2] = { static_cast<uint16_t>(first_glyph), static_cast<uint16_t>(second_glyph) };
...
hb_position_t GetGlyphHorizontalKerning(hb_font_t* font, void* data, hb_codepoint_t left_glyph, hb_codepoint_t right_glyph, void* user_data)
{
    FontData* font_data = reinterpret_cast<FontData*>(data);
    return GetGlyphKerning(font_data, left_glyph, right_glyph);
}

hb_position_t GetGlyphVerticalKerning(hb_font_t* font, void* data, hb_codepoint_t top_glyph, hb_codepoint_t bottom_glyph, void* user_data)
{
    FontData* font_data = reinterpret_cast<FontData*>(data);
...
hb_bool_t GetGlyphExtents(hb_font_t* font, void* data, hb_codepoint_t glyph, hb_glyph_extents_t* extents, void* user_data)
{
    FontData* font_data = reinterpret_cast<FontData*>(data);
...
public:
        ...
        : font_funcs_(hb_font_funcs_create())
    {
        ...
        hb_font_funcs_set_glyph_h_advance_func(font_funcs_, GetGlyphHorizontalAdvance, 0, 0);
        hb_font_funcs_set_glyph_h_kerning_func(font_funcs_, GetGlyphHorizontalKerning, 0, 0);
        hb_font_funcs_set_glyph_h_origin_func(font_funcs_, GetGlyphHorizontalOrigin, 0, 0);
        hb_font_funcs_set_glyph_v_kerning_func(font_funcs_, GetGlyphVerticalKerning, 0, 0);
...
hb_blob_t* GetFontTable(hb_face_t* face, hb_tag_t tag, void* user_data)
{
    ...
    size_t actual_size = typeface->getTableData(tag, 0, buffer.size(), buffer.data());
    ...
    return hb_blob_create(
        buffer_raw, static_cast<uint32_t>(unowned_buffer.size()), HB_MEMORY_MODE_WRITABLE, buffer_raw, base::HeapArray<char>::DeleteLeakedData);
}

// For a given skia typeface, maps to its harfbuzz face and its glyphs cache.
class TypefaceData {
public:
    explicit TypefaceData(sk_sp<SkTypeface> skia_face)
        : sk_typeface_(skia_face)
    {
...
hb_font_t* CreateHarfBuzzFont(sk_sp<SkTypeface> skia_face, SkScalar text_size, const FontRenderParams& params, bool subpixel_rendering_suppressed)
{
    ...
    TypefaceCache::iterator typeface_data = typeface_cache->Get(skia_face->uniqueID());
    if (typeface_data == typeface_cache->end()) {
        TypefaceData new_typeface_data(skia_face);
        typeface_data = typeface_cache->Put(skia_face->uniqueID(), std::move(new_typeface_data));
    }
    ...
    internal::ApplyRenderParams(params, subpixel_rendering_suppressed, &hb_font_data->font_);
