...
protected:
    ...
    SurfaceControl::Transaction::OnCompleteCb CreateOnCompleteCb(bool* called, bool* destroyed)
    {
        return base::BindOnce(
            [](std::unique_ptr<CallbackContext> context, SurfaceControl::TransactionStats stats) {
    ...
    SurfaceControl::Transaction::OnCommitCb CreateOnCommitCb(bool* called, bool* destroyed)
    {
        return base::BindOnce(
    ...
    void RunRemainingTasks()
    {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, runloop.QuitClosure());
        runloop.Run();
    }
...
TEST_F(SurfaceControlTransactionTest, CallbackCalledAfterApply)
{
    ...
    transaction.SetOnCompleteCb(CreateOnCompleteCb(&on_complete_called, &on_complete_destroyed), base::SingleThreadTaskRunner::GetCurrentDefault());
    transaction.SetOnCommitCb(CreateOnCommitCb(&on_commit_called, &on_commit_destroyed), base::SingleThreadTaskRunner::GetCurrentDefault());
...
TEST(SurfaceControl, ColorSpaceToADataSpace)
{
    // Invalid color spaces are mapped to sRGB.
    {
        ...
        EXPECT_TRUE(SurfaceControl::ColorSpaceToADataSpace(gfx::ColorSpace(), 1.f, dataspace, extended_range_brightness_ratio));
    ...
    // sRGB.
    {
        ADataSpace dataspace = ADATASPACE_UNKNOWN;
        float extended_range_brightness_ratio = 0.f;
        EXPECT_TRUE(SurfaceControl::ColorSpaceToADataSpace(gfx::ColorSpace::CreateSRGB(), 1.f, dataspace, extended_range_brightness_ratio));
        EXPECT_EQ(dataspace, ADATASPACE_SRGB);
        EXPECT_EQ(extended_range_brightness_ratio, 1.f);
    }
    ...
    // Display P3.
    {
        ADataSpace dataspace = ADATASPACE_UNKNOWN;
        float extended_range_brightness_ratio = 0.f;
        EXPECT_TRUE(SurfaceControl::ColorSpaceToADataSpace(gfx::ColorSpace::CreateDisplayP3D65(), 1.f, dataspace, extended_range_brightness_ratio));
    ...
    {
        gfx::ColorSpace rec2020(gfx::ColorSpace::PrimaryID::BT2020, gfx::ColorSpace::TransferID::SRGB);
        ADataSpace dataspace = ADATASPACE_UNKNOWN;
        float extended_range_brightness_ratio = 0.f;
        EXPECT_TRUE(SurfaceControl::ColorSpaceToADataSpace(rec2020, 1.f, dataspace, extended_range_brightness_ratio));
    ...
    // brightness ratio.
    {
        ADataSpace dataspace = ADATASPACE_UNKNOWN;
        float extended_range_brightness_ratio = 0.f;
        EXPECT_TRUE(SurfaceControl::ColorSpaceToADataSpace(gfx::ColorSpace::CreateSRGB(), 4.f, dataspace, extended_range_brightness_ratio));
    ...
    // P3, extended by 2x.
    {
        skcms_TransferFunction trfn_srgb_scaled = skia::ScaleTransferFunction(SkNamedTransferFnExt::kSRGB, 2.f);
        gfx::ColorSpace p3_scaled(gfx::ColorSpace::PrimaryID::P3, gfx::ColorSpace::TransferID::CUSTOM_HDR, gfx::ColorSpace::MatrixID::RGB,
            gfx::ColorSpace::RangeID::FULL, nullptr, &trfn_srgb_scaled);
        ADataSpace dataspace = ADATASPACE_UNKNOWN;
        float extended_range_brightness_ratio = 0.f;
        EXPECT_TRUE(SurfaceControl::ColorSpaceToADataSpace(p3_scaled, 1.f, dataspace, extended_range_brightness_ratio));
