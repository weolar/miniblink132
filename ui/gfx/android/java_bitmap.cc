...
// RGBA_8888.
template <> SkBitmap FromJniType<SkBitmap>(JNIEnv* env, const JavaRef<jobject>& j_bitmap)
{
...
template <> ScopedJavaLocalRef<jobject> ToJniType<SkBitmap>(JNIEnv* env, const SkBitmap& skbitmap)
{
...
SkColorType BitmapFormatToSkColorType(BitmapFormat bitmap_format)
{
    ...
    default:
        SCOPED_CRASH_KEY_NUMBER("gfx", "bitmap_format", static_cast<int>(bitmap_format));
...
SkPixmap WrapJavaBitmapAsPixmap(const JavaBitmap& bitmap)
{
    ...
    auto image_info = SkImageInfo::Make(bitmap.size().width(), bitmap.size().height(), color_type, kPremul_SkAlphaType);
    return SkPixmap(image_info, bitmap.pixels(), bitmap.bytes_per_row());
}

} // namespace

#define ASSERT_ENUM_EQ(a, b) static_assert(static_cast<int>(a) == static_cast<int>(b), "")
...
JavaBitmap::JavaBitmap(const JavaRef<jobject>& bitmap)
    : bitmap_(bitmap)
{
    int err = AndroidBitmap_lockPixels(AttachCurrentThread(), bitmap_.obj(), &pixels_.AsEphemeralRawAddr());
    DCHECK(!err);
    DCHECK(pixels_);
...
ScopedJavaLocalRef<jobject> ConvertToJavaBitmap(const SkBitmap& skbitmap, OomBehavior reaction)
{
    ...
    ScopedJavaLocalRef<jobject> jbitmap = Java_BitmapHelper_createBitmap(
        AttachCurrentThread(), skbitmap.width(), skbitmap.height(), java_bitmap_format, reaction == OomBehavior::kReturnNullOnOom);
    if (!jbitmap) {
...
{
    BitmapFormat jbitmap_format = static_cast<BitmapFormat>(Java_BitmapHelper_getBitmapFormatForConfig(AttachCurrentThread(), bitmap_config));
