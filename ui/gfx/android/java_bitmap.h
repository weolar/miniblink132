...
public:
    ...
    inline BitmapFormat format() const
    {
        return format_;
    }
    inline uint32_t bytes_per_row() const
    {
...
base::android::ScopedJavaLocalRef<jobject> ConvertToJavaBitmap(const SkBitmap& skbitmap, OomBehavior reaction = OomBehavior::kCrashOnOom);
...
COMPONENT_EXPORT(GFX)
SkColorType ConvertToSkiaColorType(const base::android::JavaRef<jobject>& jbitmap_config);
...
// RGBA_8888.
template <> COMPONENT_EXPORT(GFX) SkBitmap FromJniType<SkBitmap>(JNIEnv* env, const JavaRef<jobject>& j_bitmap);

// Converts |skbitmap| to a Java-backed bitmap (android.graphics.Bitmap).
// Note: return nullptr jobject if |skbitmap| is null or empty.
template <> COMPONENT_EXPORT(GFX) ScopedJavaLocalRef<jobject> ToJniType<SkBitmap>(JNIEnv* env, const SkBitmap& skbitmap);
} // namespace jni_zero
