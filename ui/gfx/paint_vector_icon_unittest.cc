...
public:
    ...
    const std::vector<SkPath>& paths() const
    {
        return paths_;
    }
...
TEST(VectorIconTest, RelativeMoveToAfterClose)
{
    ...
    const PathElement elements[] = { MOVE_TO, 4, 5, LINE_TO, 10, 11, CLOSE,
...
TEST(VectorIconTest, FillRuleNonZero)
{
    ...
    +----<------+     +-----<-----+
       EvenOdd            Winding
  */
    const PathElement elements[] = { MOVE_TO, 0, 0, R_H_LINE_TO, 90, R_V_LINE_TO, 90, R_H_LINE_TO, -90, CLOSE, MOVE_TO, 20, 20, R_H_LINE_TO, 50, R_V_LINE_TO,
        50, R_H_LINE_TO, -50, CLOSE,
        // The next path is filled by kWinding.
        NEW_PATH, FILL_RULE_NONZERO, MOVE_TO, 110, 0, R_H_LINE_TO, 90, R_V_LINE_TO, 90, R_H_LINE_TO, -90, CLOSE, MOVE_TO, 130, 20, R_H_LINE_TO, 50, R_V_LINE_TO,
        50, R_H_LINE_TO, -50, CLOSE };
    const VectorIconRep rep_list[] = { { elements, std::size(elements) } };
    const VectorIcon icon(rep_list, 1u, nullptr);
...
TEST(VectorIconTest, FlipsInRtl)
{
    ...
    const PathElement elements[]
        = { CANVAS_DIMENSIONS, 2 * canvas_size, FLIPS_IN_RTL, MOVE_TO, 10, 10, R_H_LINE_TO, 20, R_V_LINE_TO, 20, R_H_LINE_TO, -20, CLOSE };
    const VectorIconRep rep_list[] = { { elements, std::size(elements) } };
    const VectorIcon icon(rep_list, 1u, nullptr);
...
TEST(VectorIconTest, CorrectSizePainted)
{
    ...
    const PathElement elements48[] = { CANVAS_DIMENSIONS, 48, PATH_COLOR_ARGB, 0xFF, SkColorGetR(kPath48Color), SkColorGetG(kPath48Color),
        SkColorGetB(kPath48Color), MOVE_TO, 0, 0, H_LINE_TO, 48, V_LINE_TO, 48, H_LINE_TO, 0, V_LINE_TO, 0, CLOSE };
    const SkColor kPath32Color = SK_ColorGREEN;
    const PathElement elements32[] = { CANVAS_DIMENSIONS, 32, PATH_COLOR_ARGB, 0xFF, SkColorGetR(kPath32Color), SkColorGetG(kPath32Color),
        SkColorGetB(kPath32Color), MOVE_TO, 0, 0, H_LINE_TO, 32, V_LINE_TO, 32, H_LINE_TO, 0, V_LINE_TO, 0, CLOSE };
    const SkColor kPath24Color = SK_ColorBLUE;
    const PathElement elements24[] = { CANVAS_DIMENSIONS, 24, PATH_COLOR_ARGB, 0xFF, SkColorGetR(kPath24Color), SkColorGetG(kPath24Color),
        SkColorGetB(kPath24Color), MOVE_TO, 0, 0, H_LINE_TO, 24, V_LINE_TO, 24, H_LINE_TO, 0, V_LINE_TO, 0, CLOSE };
    const SkColor kPath20Color = SK_ColorYELLOW;
    const PathElement elements20[] = { CANVAS_DIMENSIONS, 20, PATH_COLOR_ARGB, 0xFF, SkColorGetR(kPath20Color), SkColorGetG(kPath20Color),
        SkColorGetB(kPath20Color), MOVE_TO, 0, 0, H_LINE_TO, 20, V_LINE_TO, 20, H_LINE_TO, 0, V_LINE_TO, 0, CLOSE };
    const SkColor kPath16Color = SK_ColorCYAN;
    const PathElement elements16[] = { CANVAS_DIMENSIONS, 16, PATH_COLOR_ARGB, 0xFF, SkColorGetR(kPath16Color), SkColorGetG(kPath16Color),
        SkColorGetB(kPath16Color), MOVE_TO, 0, 0, H_LINE_TO, 16, V_LINE_TO, 16, H_LINE_TO, 0, V_LINE_TO, 0, CLOSE };
    // VectorIconReps are always sorted in descending order of size.
    const VectorIconRep rep_list[] = { { elements48, std::size(elements48) }, { elements32, std::size(elements32) }, { elements24, std::size(elements24) },
        { elements20, std::size(elements20) }, { elements16, std::size(elements16) } };
