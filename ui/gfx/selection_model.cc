...
SelectionModel::SelectionModel(const Range& selection, LogicalCursorDirection affinity)
    : selection_(selection)
    , caret_affinity_(affinity)
{
}

SelectionModel::SelectionModel(const std::vector<Range>& selections, LogicalCursorDirection affinity)
    : selection_(selections[0])
    , caret_affinity_(affinity)
{
...
std::vector<Range> SelectionModel::GetAllSelections() const
{
    ...
    selections.insert(selections.end(), secondary_selections_.begin(), secondary_selections_.end());
    return selections;
}

bool SelectionModel::operator==(const SelectionModel& sel) const
{
    return selection() == sel.selection() && caret_affinity() == sel.caret_affinity() && secondary_selections() == sel.secondary_selections();
}
