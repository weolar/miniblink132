...
RectInViewport ApplyOverlayTransform(OverlayTransform overlay_transform, const RectInViewport& original)
{
    auto transform = OverlayTransformToTransform(overlay_transform, SizeF(original.viewport));
    RectInViewport result;
    result.rect = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(transform, original.rect);
    result.viewport = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(transform, Rect(original.viewport)).size();
    return result;
}
...
TEST(OverlayTransformUtilTest, All)
{
    ...
    for (const auto& test_case : test_cases) {
        ...
        RectInViewport transformed = ApplyOverlayTransform(test_case.overlay_transform, { original, viewport_bounds });
        EXPECT_EQ(test_case.transformed, transformed.rect);

        RectInViewport inverted = ApplyOverlayTransform(InvertOverlayTransform(test_case.overlay_transform), { test_case.transformed, transformed.viewport });
...
TEST(OverlayTransformUtilTest, Concat)
{
    ...
    for (auto t1 : kTransforms) {
        ...
        for (auto t2 : kTransforms) {
            ...
            RectInViewport t1_then_t2_result = ApplyOverlayTransform(t2, ApplyOverlayTransform(t1, original));
            RectInViewport concat_result = ApplyOverlayTransform(concat_transform, original);
