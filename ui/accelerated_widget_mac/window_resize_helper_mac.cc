...
using EventTimedWaitCallback = base::RepeatingCallback<void(base::WaitableEvent*, base::TimeDelta)>;
...
public:
    ...
    const base::TimeTicks& can_run_time() const
    {
        return can_run_time_;
    }
...
public:
    PumpableTaskRunner(const EventTimedWaitCallback& event_timed_wait_callback, const scoped_refptr<base::SingleThreadTaskRunner>& target_task_runner);
    ...
    bool EnqueueAndPostWrappedTask(const base::Location& from_here, std::unique_ptr<WrappedTask> task, base::TimeDelta delay);
...
void WrappedTask::AddToTaskRunnerQueue(PumpableTaskRunner* pumpable_task_runner)
{
    ...
    iterator_ = pumpable_task_runner_->task_queue_.insert(pumpable_task_runner_->task_queue_.end(), this);
}
...
PumpableTaskRunner::PumpableTaskRunner(
    const EventTimedWaitCallback& event_timed_wait_callback, const scoped_refptr<base::SingleThreadTaskRunner>& target_task_runner)
...
bool PumpableTaskRunner::WaitForSingleWrappedTaskToRun(const base::TimeDelta& max_delay)
{
    ...
    for (;;) {
        ...
        WrappedTask* task_to_execute = NULL;
        {
            ...
            for (WrappedTaskQueue::iterator it = task_queue_.begin(); it != task_queue_.end(); ++it) {
...
bool PumpableTaskRunner::EnqueueAndPostWrappedTask(const base::Location& from_here, std::unique_ptr<WrappedTask> task, base::TimeDelta delay)
{
    ...
    return target_task_runner_->PostDelayedTask(from_here, base::BindOnce(&WrappedTask::Run, std::move(task)), delay);
}

////////////////////////////////////////////////////////////////////////////////
// PumpableTaskRunner, base::SingleThreadTaskRunner implementation:

bool PumpableTaskRunner::PostDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
    return EnqueueAndPostWrappedTask(from_here, std::make_unique<WrappedTask>(std::move(task), delay), delay);
}

bool PumpableTaskRunner::PostNonNestableDelayedTask(const base::Location& from_here, base::OnceClosure task, base::TimeDelta delay)
{
...
scoped_refptr<base::SingleThreadTaskRunner> WindowResizeHelperMac::task_runner() const
{
    return task_runner_;
}
...
void WindowResizeHelperMac::Init(const scoped_refptr<base::SingleThreadTaskRunner>& target_task_runner)
{
    DCHECK(!task_runner_);
    task_runner_ = new PumpableTaskRunner(base::BindRepeating(&WindowResizeHelperMac::EventTimedWait), target_task_runner);
}
...
bool WindowResizeHelperMac::WaitForSingleTaskToRun(const base::TimeDelta& max_delay)
{
...
void WindowResizeHelperMac::EventTimedWait(base::WaitableEvent* event, base::TimeDelta delay)
{
