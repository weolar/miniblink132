...
public:
    ...
    TestPaintingWindowDelegate& operator=(const TestPaintingWindowDelegate&) = delete;
    ...
    void OnPaint(const ui::PaintContext& context) override
    {
        ...
        for (int y = 0; y < window_size_.height(); ++y) {
            for (int x = 0; x < window_size_.width(); ++x) {
                recorder.canvas()->FillRect(gfx::Rect(x, y, 1, 1), GetExpectedColorForPoint(x, y));
            }
        }
    }
...
size_t GetFailedPixelsCountWithScaleFactor(const gfx::Image& image, int scale_factor)
{
...
public:
    ...
    void SetUp() override
    {
        ...
        task_environment_ = std::make_unique<base::test::TaskEnvironment>(base::test::TaskEnvironment::MainThreadType::UI);

        // The ContextFactory must exist before any Compositors are created.
        // Snapshot test tests real drawing and readback, so needs pixel output.
        const bool enable_pixel_output = true;
        context_factories_ = std::make_unique<ui::TestContextFactories>(enable_pixel_output);

        helper_ = std::make_unique<aura::test::AuraTestHelper>(context_factories_->GetContextFactory());
        helper_->SetUp();
    }
...
protected:
    aura::Window* test_window()
    {
        return test_window_.get();
    }
    aura::Window* root_window()
    {
        return helper_->GetContext();
    }
    aura::TestScreen* test_screen()
    {
        return helper_->GetTestScreen();
    }

    void WaitForDraw()
    {
        helper_->GetHost()->compositor()->ScheduleDraw();
        ui::DrawWaiterForTest::WaitForCompositingEnded(helper_->GetHost()->compositor());
    }

    void SetupTestWindow(const gfx::Rect& window_bounds)
    {
        delegate_ = std::make_unique<TestPaintingWindowDelegate>(window_bounds.size());
        test_window_.reset(aura::test::CreateTestWindowWithDelegate(delegate_.get(), 0, window_bounds, root_window()));
    }

    gfx::Image GrabSnapshotForTestWindow()
    {
        gfx::Rect source_rect(test_window_->bounds().size());
        aura::Window::ConvertRectToTarget(test_window(), root_window(), &source_rect);

        scoped_refptr<SnapshotHolder> holder(new SnapshotHolder);
        ui::GrabWindowSnapshot(root_window(), source_rect, base::BindOnce(&SnapshotHolder::SnapshotCallback, holder));
...
TEST_F(SnapshotAuraTest, MAYBE_FullScreenWindow)
{
    ...
    EXPECT_EQ(test_window()->bounds().size().ToString(), snapshot.Size().ToString());
...
TEST_F(SnapshotAuraTest, UIScale)
{
    ...
    gfx::Image snapshot = GrabSnapshotForTestWindow();
    EXPECT_EQ(gfx::ToRoundedSize(snapshot_size).ToString(), snapshot.Size().ToString());
