...
public:
    ...
    static std::unique_ptr<ScopedCursorHider> Create(aura::Window* window)
    {
        ...
        return std::unique_ptr<ScopedCursorHider>(base::WrapUnique(new ScopedCursorHider(window)));
    }
    ...
    ~ScopedCursorHider()
    {
        aura::client::CursorClient* cursor_client = aura::client::GetCursorClient(window_);
...
void ScreenshotGrabber::TakeScreenshot(gfx::NativeWindow window, const gfx::Rect& rect, ScreenshotCallback callback)
{
    ...
    ui::GrabWindowSnapshotAsPNG(
        window, rect, base::BindOnce(&ScreenshotGrabber::GrabSnapshotImageCallback, factory_.GetWeakPtr(), window_identifier, is_partial, std::move(callback)));
}

bool ScreenshotGrabber::CanTakeScreenshot()
{
    return last_screenshot_timestamp_.is_null() || base::TimeTicks::Now() - last_screenshot_timestamp_ > base::Milliseconds(kScreenshotMinimumIntervalInMS);
}

void ScreenshotGrabber::GrabSnapshotImageCallback(
    const std::string& window_identifier, bool is_partial, ScreenshotCallback callback, scoped_refptr<base::RefCountedMemory> png_data)
{
    DCHECK(base::CurrentUIThread::IsSet());
    ...
    if (!png_data.get()) {
        if (is_partial) {
            ...
            std::move(callback).Run(ScreenshotResult::GRABWINDOW_PARTIAL_FAILED, nullptr);
        } else {
            LOG(ERROR) << "Failed to grab the window screenshot for " << window_identifier;
            std::move(callback).Run(ScreenshotResult::GRABWINDOW_FULL_FAILED, nullptr);
        }
        return;
    }
