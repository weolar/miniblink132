...
using GrabSnapshotDataCallback = base::OnceCallback<void(scoped_refptr<base::RefCountedMemory> data)>;
...
SNAPSHOT_EXPORT void GrabWindowSnapshot(gfx::NativeWindow window, const gfx::Rect& source_rect, GrabSnapshotImageCallback callback);

SNAPSHOT_EXPORT void GrabViewSnapshot(gfx::NativeView view, const gfx::Rect& source_rect, GrabSnapshotImageCallback callback);
...
SNAPSHOT_EXPORT void GrabWindowSnapshotAndScale(
    gfx::NativeWindow window, const gfx::Rect& source_rect, const gfx::Size& target_size, GrabSnapshotImageCallback callback);
...
SNAPSHOT_EXPORT void GrabWindowSnapshotAsPNG(gfx::NativeWindow window, const gfx::Rect& source_rect, GrabSnapshotDataCallback callback);

// Takes a snapshot as with `GrabWindowSnapshot()` and encodes it as JPEG data.
//
// Returns the data via the provided `callback`. In case of an error, a null
// pointer will be returned.
SNAPSHOT_EXPORT void GrabWindowSnapshotAsJPEG(gfx::NativeWindow window, const gfx::Rect& source_rect, GrabSnapshotDataCallback callback);
