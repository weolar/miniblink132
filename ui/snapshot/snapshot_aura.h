...
SNAPSHOT_EXPORT void GrabWindowSnapshotAndScaleAura(
    aura::Window* window, const gfx::Rect& source_rect, const gfx::Size& target_size, GrabSnapshotImageCallback callback);

SNAPSHOT_EXPORT void GrabWindowSnapshotAura(aura::Window* window, const gfx::Rect& source_rect, GrabSnapshotImageCallback callback);

// Grabs a snapshot of a |layer| and all its descendants.
// |source_rect| is the bounds of the snapshot content relative to |layer|.
SNAPSHOT_EXPORT void GrabLayerSnapshot(Layer* layer, const gfx::Rect& source_rect, GrabSnapshotImageCallback callback);
