...
void Shadow::SetContentBounds(const gfx::Rect& content_bounds)
{
    ...
    if (content_bounds == content_bounds_ && layer()->bounds() == last_layer_bounds_) {
        return;
    }
...
void Shadow::SetElevation(int elevation)
{
...

    {
        ...
        settings.SetTransitionDuration(base::Milliseconds(kShadowAnimationDurationMs));
...
Shadow::ShadowLayerOwner::ShadowLayerOwner(Shadow* owner, std::unique_ptr<Layer> layer)
    : LayerOwner(std::move(layer))
    , owner_shadow_(owner)
{
}
...
void Shadow::UpdateShadowAppearance()
{
    ...
    const int smaller_dimension = std::min(content_bounds_.width(), content_bounds_.height());
    const int size_adjusted_elevation = std::min((smaller_dimension - 2 * rounded_corner_radius_) / 4, static_cast<int>(desired_elevation_));

    auto iter = color_map_.find(desired_elevation_);
    const auto& details = (iter == color_map_.end()) ? gfx::ShadowDetails::Get(size_adjusted_elevation, rounded_corner_radius_, style_)
    ...
    gfx::Insets blur_region = gfx::ShadowValue::GetBlurRegion(details.values) + gfx::Insets(rounded_corner_radius_);
    ...
    if (fading_layer()) {
        ...
        fading_layer_bounds.Offset(old_layer_bounds.origin() - combined_layer_bounds.origin());
        fading_layer()->SetBounds(fading_layer_bounds);

        shadow_layer_bounds.Offset(new_layer_bounds.origin() - combined_layer_bounds.origin());
    } else {
    ...
    shadow_layer()->UpdateNinePatchLayerBorder(gfx::Rect(blur_region.left(), blur_region.top(), blur_region.width(), blur_region.height()));
}
