...
public:
    ...
    // animations.
    ui::Layer* shadow_layer()
    {
        return shadow_layer_owner_.layer();
    }

    ui::Layer* fading_layer()
    {
        return fading_layer_owner_.layer();
    }
    ...
    int desired_elevation() const
    {
        return desired_elevation_;
    }
    const ElevationToColorsMap& color_map() const
    {
        return color_map_;
    }
...
private:
    ...
    public:
        explicit ShadowLayerOwner(Shadow* owner, std::unique_ptr<Layer> layer = nullptr);
