...
{
    return -gfx::Insets(2 * elevation) + gfx::Insets::TLBR(elevation, 0, -elevation, 0);
}

gfx::Size NineboxImageSizeForElevationAndCornerRadius(int elevation, int corner_radius)
{
...
// radius.
gfx::Size MinContentSizeForElevationAndCornerRadius(int elevation, int corner_radius)
{
...
{
    ScopedAnimationDurationScaleMode zero_duration_mode(ScopedAnimationDurationScaleMode::ZERO_DURATION);
...
TEST_F(ShadowTest, AdjustRoundedCornerRadius)
{
    ...
    shadow_bounds.Inset(InsetsForElevation(kElevationSmall));
    EXPECT_EQ(shadow_bounds, shadow.layer()->bounds());
    EXPECT_EQ(NineboxImageSizeForElevationAndCornerRadius(6, 0), shadow.details_for_testing()->nine_patch_image.size());
}
...
TEST_F(ShadowTest, EvictUniquelyOwnedDetail)
{
    ...
    // from the cache.
    {
        ...
        const gfx::Size min_content_size = MinContentSizeForElevationAndCornerRadius(kElevationUnique, 2);
        ...
        shadow_same.SetContentBounds(gfx::Rect(gfx::Point(10, 10), min_content_size + gfx::Size(50, 50)));
        ...
        shadow_small.SetContentBounds(gfx::Rect(MinContentSizeForElevationAndCornerRadius(kElevationSmall, 2)));
        EXPECT_EQ(2u, gfx::ShadowDetails::GetDetailsCacheSizeForTest());
    ...
    shadow_large.SetContentBounds(gfx::Rect(MinContentSizeForElevationAndCornerRadius(kElevationLarge, 2)));
    // The cache size is unchanged.
    EXPECT_EQ(1u, gfx::ShadowDetails::GetDetailsCacheSizeForTest());
}

class ShadowColorTest : public ShadowTest, public testing::WithParamInterface<gfx::ShadowStyle> {
public:
    ...
    static std::vector<gfx::ShadowStyle> GetTestParamValues()
    {
        ...
        return { gfx::ShadowStyle::kMaterialDesign, gfx::ShadowStyle::kChromeOSSystemUI };
#else
...
INSTANTIATE_TEST_SUITE_P(All, ShadowColorTest, testing::ValuesIn(ShadowColorTest::GetTestParamValues()));
...
TEST_P(ShadowColorTest, ElevationToColorsMap)
{
    ...
    shadow.SetContentBounds(gfx::Rect(MinContentSizeForElevationAndCornerRadius(kElevationLarge, 0)));
    ...
    color_map[kElevationSmall] = std::make_pair(small_key_color, small_ambient_color);
    color_map[kElevationLarge] = std::make_pair(large_key_color, large_ambient_color);
    shadow.SetElevationToColorsMap(color_map);

    // A lambda to get key and ambient shadow colors.
    auto get_colors = [](const ui::Shadow& shadow) -> std::pair<SkColor, SkColor> {
        const auto& values = shadow.details_for_testing()->values;
        return std::make_pair(values[0].color(), values[1].color());
    };

    // Check if shadow colors are updated.
    EXPECT_THAT(get_colors(shadow), FieldsAre(small_key_color, small_ambient_color));

    // Check if shadow colors are updated when the shadow changes to another
    // specified elevation.
    shadow.SetElevation(kElevationLarge);
    EXPECT_THAT(get_colors(shadow), FieldsAre(large_key_color, large_ambient_color));
    ...
    EXPECT_THAT(get_colors(shadow), FieldsAre(default_key_color, default_ambient_color));
}
