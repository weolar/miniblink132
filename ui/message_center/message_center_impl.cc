...
{
    return g_limit_overrider_instance_ ? g_limit_overrider_instance_->overriding_limit : kChromeOSNotificationLimit;
}

int GetTargetCountAfterRemoval()
{
    return g_limit_overrider_instance_ ? g_limit_overrider_instance_->overriding_target_count : kNotificationTargetCountAfterRemoval;
}
...
MessageCenterImpl::MessageCenterImpl(std::unique_ptr<LockScreenController> lock_screen_controller)
...
void MessageCenterImpl::RemoveNotificationBlocker(NotificationBlocker* blocker)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void MessageCenterImpl::OnBlockingStateChanged(NotificationBlocker* blocker)
{
    ...
    NotificationList::PopupNotifications popups = notification_list_->GetPopupNotifications(blockers_, &blocked);

    visible_notifications_ = notification_list_->GetVisibleNotifications(blockers_);

    for (const std::string& notification_id : blocked) {
        observer_list_.Notify(&MessageCenterObserver::OnNotificationUpdated, notification_id);
    }
    observer_list_.Notify(&MessageCenterObserver::OnBlockingStateChanged, blocker);
}
...
void MessageCenterImpl::SetVisibility(Visibility visibility)
{
    ...
    observer_list_.Notify(&MessageCenterObserver::OnCenterVisibilityChanged, visibility);
}
...
ExpandState MessageCenterImpl::GetNotificationExpandState(const std::string& id)
{
    return notification_list_->GetNotificationExpandState(id);
}

void MessageCenterImpl::SetNotificationExpandState(const std::string& id, const ExpandState expand_state)
{
...
Notification* MessageCenterImpl::FindParentNotification(Notification* notification)
{
...
#if BUILDFLAG(IS_CHROMEOS)
    is_privacy_indicators_notification = notification->notifier_id().id == ash::kPrivacyIndicatorsNotifierId;
    render_arc_notifications_by_chrome = ash::features::IsRenderArcNotificationsByChromeEnabled();
#endif // BUILDFLAG(IS_CHROMEOS)

    if (!is_privacy_indicators_notification && (notification->origin_url().is_empty() || notification->notifier_id().type != NotifierType::WEB_PAGE)
        && notification->notifier_id().type != NotifierType::ARC_APPLICATION) {
        return nullptr;
    }

    NotificationList::Notifications notifications = notification_list_->GetNotificationsByNotifierId(notification->notifier_id());
    ...
    auto parent_notification_it = base::ranges::find_if(notifications, [](Notification* notification) { return notification->group_parent(); });
...
Notification* MessageCenterImpl::FindPopupNotificationById(const std::string& id)
{
...
Notification* MessageCenterImpl::FindVisibleNotificationById(const std::string& id)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
NotificationList::Notifications MessageCenterImpl::FindNotificationsByAppId(const std::string& app_id)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
const NotificationList::Notifications& MessageCenterImpl::GetVisibleNotifications()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    return visible_notifications_;
}

NotificationList::Notifications MessageCenterImpl::GetVisibleNotificationsWithoutBlocker(const NotificationBlocker* blocker) const
{
    return notification_list_->GetVisibleNotificationsWithoutBlocker(blockers_, blocker);
}

NotificationList::PopupNotifications MessageCenterImpl::GetPopupNotifications()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    return notification_list_->GetPopupNotifications(blockers_, nullptr);
}

NotificationList::PopupNotifications MessageCenterImpl::GetPopupNotificationsWithoutBlocker(const NotificationBlocker& blocker) const
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    return notification_list_->GetPopupNotificationsWithoutBlocker(blockers_, blocker);
}

//------------------------------------------------------------------------------
// Client code interface.
void MessageCenterImpl::AddNotification(std::unique_ptr<Notification> notification)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(notification);
    ...
    if (auto* const parent = FindParentNotification(notification.get()); notification->allow_group() && parent && !notification->group_parent()) {
...
void MessageCenterImpl::UpdateNotification(const std::string& old_id, std::unique_ptr<Notification> new_notification)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    for (NotificationBlocker* blocker : blockers_) {
        blocker->CheckState();
    ...
    if (old_notification && old_notification->notifier_id() == new_notification->notifier_id()) {
    ...
    notification_list_->UpdateNotificationMessage(old_id, std::move(new_notification));
    visible_notifications_ = notification_list_->GetVisibleNotifications(blockers_);
...
void MessageCenterImpl::RemoveNotification(const std::string& id, bool by_user)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void MessageCenterImpl::RemoveNotificationsForNotifierId(const NotifierId& notifier_id)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void MessageCenterImpl::RemoveAllNotifications(bool by_user, RemoveType type)
{
    ...
    const NotificationBlockers& blockers = remove_pinned ? NotificationBlockers() /* empty blockers */
...
void MessageCenterImpl::SetNotificationIcon(const std::string& notification_id, const ui::ImageModel& image)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void MessageCenterImpl::SetNotificationImage(const std::string& notification_id, const gfx::Image& image)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void MessageCenterImpl::ClickOnNotification(const std::string& id)
{
    ...
    lock_screen_controller_->DismissLockScreenThenExecute(
        base::BindOnce(&MessageCenterImpl::ClickOnNotificationUnlocked, base::Unretained(this), id, std::nullopt, std::nullopt), base::OnceClosure());
}

void MessageCenterImpl::ClickOnNotificationButton(const std::string& id, int button_index)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (!FindVisibleNotificationById(id)) {
        return;
    ...
    lock_screen_controller_->DismissLockScreenThenExecute(
        base::BindOnce(&MessageCenterImpl::ClickOnNotificationUnlocked, base::Unretained(this), id, button_index, std::nullopt), base::OnceClosure());
}

void MessageCenterImpl::ClickOnNotificationButtonWithReply(const std::string& id, int button_index, const std::u16string& reply)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    if (!FindVisibleNotificationById(id)) {
        return;
    ...
    lock_screen_controller_->DismissLockScreenThenExecute(
        base::BindOnce(&MessageCenterImpl::ClickOnNotificationUnlocked, base::Unretained(this), id, button_index, reply), base::OnceClosure());
}

void MessageCenterImpl::ClickOnNotificationUnlocked(const std::string& id, const std::optional<int>& button_index, const std::optional<std::u16string>& reply)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (const Notification* notification = notification_list_->GetNotificationById(id);
...
{
    if (!ash::features::IsNotificationLimitEnabled() || notification_list_->size() <= GetNotificationLimit()) {
        return;
    }

    if (!overlimit_handler_timer_.IsRunning()) {
        overlimit_handler_timer_.Start(FROM_HERE, base::TimeDelta(), /*receiver=*/this, &MessageCenterImpl::RemoveNotificationsIfOverLimit);
    }
}
...
void MessageCenterImpl::RemoveNotificationsIfOverLimit()
{
    ...
    if (int notification_count = notification_list_->size(); notification_count > GetNotificationLimit()) {
        for (const std::string& id : notification_list_->GetTopKRemovableNotificationIds(notification_count - GetTargetCountAfterRemoval())) {
            RemoveNotification(id, /*by_user=*/false);
        }

        base::UmaHistogramBoolean("Ash.Notification.RemovedByLimitEnforcement", true);
    }
}
...
void MessageCenterImpl::ClickOnSettingsButton(const std::string& id)
{
    ...
    bool handled_by_delegate
        = notification && notification->delegate() && (notification->rich_notification_data().settings_button_handler == SettingsButtonHandler::DELEGATE);
...
void MessageCenterImpl::ClickOnSnoozeButton(const std::string& id)
{
    ...
    bool handled_by_delegate = notification && notification_list_->GetNotificationDelegate(id);
    if (handled_by_delegate) {
...
void MessageCenterImpl::MarkSinglePopupAsShown(const std::string& id, bool mark_notification_as_read)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void MessageCenterImpl::ResetPopupTimer(const std::string& id)
{
    ...
    popup_timers_controller_->StartTimer(id, popup_timers_controller_->GetTimeoutForNotification(FindPopupNotificationById(id)));
}
...
void MessageCenterImpl::DisplayedNotification(const std::string& id, const DisplaySource source)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void MessageCenterImpl::SetQuietMode(bool in_quiet_mode, QuietModeSourceType type)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
QuietModeSourceType MessageCenterImpl::GetLastQuietModeChangeSourceType() const
{
...
void MessageCenterImpl::EnterQuietModeWithExpire(const base::TimeDelta& expires_in)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    quiet_mode_timer_.Start(
        FROM_HERE, expires_in, base::BindOnce(&MessageCenterImpl::SetQuietMode, base::Unretained(this), false, QuietModeSourceType::kUserAction));
}
...
void MessageCenterImpl::SetSystemNotificationAppName(const std::u16string& name)
{
    system_notification_app_name_ = name;
}

void MessageCenterImpl::OnMessageViewHovered(const std::string& notification_id)
{
    for (MessageCenterObserver& observer : observer_list_) {
...
#if BUILDFLAG(IS_CHROMEOS)
ScopedNotificationLimitOverrider::ScopedNotificationLimitOverrider(size_t limit, size_t target_count)
    : overriding_limit(limit)
    , overriding_target_count(target_count)
{
