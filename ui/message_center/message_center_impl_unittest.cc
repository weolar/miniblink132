...
public:
    ...
    const std::string log(const std::string& id)
    {
        return logs_[id];
    }
    void reset_logs()
...
public:
    ...
    void Click(const std::optional<int>& button_index, const std::optional<std::u16string>& reply) override
    {
        if (button_index) {
            if (!reply) {
                log_ += "ButtonClick_";
...
public:
    DeleteOnCloseDelegate(MessageCenter* message_center, const std::string& notification_id)
        : message_center_(message_center)
...
public:
    ...
    MessageCenter* message_center() const
    {
    ...
    base::RunLoop* run_loop() const
    {
        return run_loop_.get();
    }
    base::RepeatingClosure closure() const
    {
        return closure_;
    }

protected:
    std::unique_ptr<Notification> CreateSimpleNotification(const std::string& id)
    {
        return CreateNotificationWithNotifierId(id, kDefaultAppId, NOTIFICATION_TYPE_SIMPLE);
    }

    std::unique_ptr<Notification> CreateSimpleNotificationWithNotifierId(const std::string& id, const std::string& notifier_id)
    {
        return CreateNotificationWithNotifierId(id, notifier_id, NOTIFICATION_TYPE_SIMPLE);
    }

    std::unique_ptr<Notification> CreateSimpleNotificationWithDelegate(const std::string& id, scoped_refptr<NotificationDelegate> delegate)
    {
        return CreateNotificationWithNotifierIdAndDelegate(id, kDefaultAppId, NOTIFICATION_TYPE_SIMPLE, delegate, RichNotificationData());
    }

    std::unique_ptr<Notification> CreateSimpleNotificationWithOptionalFields(const std::string& id, const RichNotificationData& optional_fields)
    {
        return CreateNotificationWithNotifierIdAndDelegate(id, kDefaultAppId, NOTIFICATION_TYPE_SIMPLE, base::MakeRefCounted<TestDelegate>(), optional_fields);
    }

    std::unique_ptr<Notification> CreateNotification(const std::string& id, NotificationType type)
    {
        return CreateNotificationWithNotifierId(id, kDefaultAppId, type);
    }

    std::unique_ptr<Notification> CreateNotificationWithNotifierId(const std::string& id, const std::string& notifier_id, NotificationType type)
    {
        return CreateNotificationWithNotifierIdAndDelegate(id, notifier_id, type, base::MakeRefCounted<TestDelegate>(), RichNotificationData());
    }

    std::unique_ptr<Notification> CreateNotificationWithNotifierIdAndDelegate(const std::string& id, const std::string& notifier_id, NotificationType type,
        scoped_refptr<NotificationDelegate> delegate, const RichNotificationData& optional_fields)
    {
        return std::make_unique<Notification>(type, id, u"title", UTF8ToUTF16(id), ui::ImageModel() /* icon */, std::u16string() /* display_source */, GURL(),
            NotifierId(NotifierType::APPLICATION, notifier_id), optional_fields, delegate);
    }
    ...
    {
        return static_cast<FakeLockScreenController*>(message_center_impl()->lock_screen_controller());
    }
...
public:
    ...
    ToggledNotificationBlocker& operator=(const ToggledNotificationBlocker&) = delete;
    ...
    bool ShouldShowNotificationAsPopup(const Notification& notification) const override
    {
...
public:
    PopupNotificationBlocker(MessageCenter* message_center, const NotifierId& allowed_notifier)
    ...
    bool ShouldShowNotificationAsPopup(const Notification& notification) const override
    {
        return (notification.notifier_id() == allowed_notifier_) || ToggledNotificationBlocker::ShouldShowNotificationAsPopup(notification);
    }
...
public:
    NearTotalNotificationBlocker(MessageCenter* message_center, const NotifierId& allowed_notifier)
    ...
    NearTotalNotificationBlocker& operator=(const NearTotalNotificationBlocker&) = delete;
    ...
    bool ShouldShowNotification(const Notification& notification) const override
    {
        return (notification.notifier_id() == allowed_notifier_) || ToggledNotificationBlocker::ShouldShowNotification(notification);
    }
};
...
bool PopupNotificationsContain(const NotificationList::PopupNotifications& popups, const std::string& id)
{
...
bool NotificationsContain(const NotificationList::Notifications& notifications, const std::string& id)
{
...
public:
    MockPopupTimersController(MessageCenter* message_center, base::RepeatingClosure quit_closure)
    ...
    {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, quit_closure_);
        timer_finished_++;
        last_id_ = id;
    }

    int timer_finished() const
    {
        return timer_finished_;
    }
...
{
    std::unique_ptr<PopupTimersController> popup_timers_controller = std::make_unique<PopupTimersController>(message_center());
...
{
    std::unique_ptr<MockPopupTimersController> popup_timers_controller = std::make_unique<MockPopupTimersController>(message_center(), closure());
...
{
    scoped_refptr<base::SingleThreadTaskRunner> old_task_runner = base::SingleThreadTaskRunner::GetCurrentDefault();

    scoped_refptr<base::TestMockTimeTaskRunner> task_runner = base::MakeRefCounted<base::TestMockTimeTaskRunner>(base::Time::Now(), base::TimeTicks::Now());
    base::CurrentThread::Get()->SetTaskRunner(task_runner);

    NotifierId notifier_id(GURL("https://example.com"));

    message_center()->AddNotification(std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "id1", u"title", u"message", ui::ImageModel() /* icon */,
        std::u16string() /* display_source */, GURL(), notifier_id, RichNotificationData(), nullptr));
    ...
    const int dismiss_time = popup_timers_controller->GetNotificationTimeoutDefault();
#else
...
TEST_F(MessageCenterImplTest, NotificationBlocker)
{
    ...
    message_center()->AddNotification(std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "id1", u"title", u"message", ui::ImageModel() /* icon */,
        std::u16string() /* display_source */, GURL(), notifier_id, RichNotificationData(), nullptr));
    message_center()->AddNotification(std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "id2", u"title", u"message", ui::ImageModel() /* icon */,
        std::u16string() /* display_source */, GURL(), notifier_id, RichNotificationData(), nullptr));
...
TEST_F(MessageCenterImplTest, VisibleNotificationsWithoutBlocker)
{
    ...
    message_center()->AddNotification(std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "id1", u"title", u"message", ui::ImageModel() /* icon */,
        std::u16string() /* display_source */, GURL(), notifier_id1, RichNotificationData(), nullptr));
    message_center()->AddNotification(std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "id2", u"title", u"message", ui::ImageModel() /* icon */,
        std::u16string() /* display_source */, GURL(), notifier_id2, RichNotificationData(), nullptr));
    ...
    NotificationList::Notifications notifications_1 = message_center()->GetVisibleNotificationsWithoutBlocker(&blocker_1);
    ...
    NotificationList::Notifications notifications_2 = message_center()->GetVisibleNotificationsWithoutBlocker(&blocker_2);
...
TEST_F(MessageCenterImplTest, PopupsWithoutBlocker)
{
    ...
    NotificationList::PopupNotifications initial_popups = message_center()->GetPopupNotificationsWithoutBlocker(blocker_3);
    NotificationList::PopupNotifications final_popups = message_center()->GetPopupNotificationsWithoutBlocker(blocker_3);
    ...
    NotificationList::PopupNotifications popups_1 = message_center()->GetPopupNotificationsWithoutBlocker(blocker_1);
    EXPECT_EQ(1u, popups_1.size());
    EXPECT_TRUE(PopupNotificationsContain(popups_1, "id2"));

    // Without blocker 2, popup 1 can be seen.
    NotificationList::PopupNotifications popups_2 = message_center()->GetPopupNotificationsWithoutBlocker(blocker_2);
...
TEST_F(MessageCenterImplTest, GetNotifications)
{
    ...
    blocker.SetNotificationsEnabled(false);
    message_center()->AddNotification(std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "id3", u"title", u"message", ui::ImageModel() /* icon */,
        std::u16string() /* display_source */, GURL(), notifier_id, RichNotificationData(), nullptr));
    EXPECT_EQ(0u, message_center()->GetPopupNotifications().size());
...
TEST_F(MessageCenterImplTest, NotificationBlockerAllowsPopups)
{
    ...
    message_center()->AddNotification(std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "id3", u"title", u"message", ui::ImageModel() /* icon */,
        std::u16string() /* display_source */, GURL(), notifier_id1, RichNotificationData(), nullptr));
    message_center()->AddNotification(std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "id4", u"title", u"message", ui::ImageModel() /* icon */,
        std::u16string() /* display_source */, GURL(), notifier_id2, RichNotificationData(), nullptr));
    popups = message_center()->GetPopupNotifications();
...
TEST_F(MessageCenterImplTest, NearTotalNotificationBlocker)
{
    ...
    blocker.SetNotificationsEnabled(false);
    message_center()->RemoveAllNotifications(false /* by_user */, MessageCenter::RemoveType::NON_PINNED);
    ...
    blocker.SetNotificationsEnabled(false);
    message_center()->RemoveAllNotifications(false /* by_user */, MessageCenter::RemoveType::ALL);
    EXPECT_EQ(0u, message_center()->NotificationCount());
}
...
TEST_F(MessageCenterImplTest, NotificationsUpdatedWhenBlockerAdded)
{
    ...
    NotifierId notifier_id1(NotifierType::APPLICATION, "app1");
    message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id1", notifier_id1.id));
    message_center()->DisplayedNotification("id1", DISPLAY_SOURCE_POPUP);
    ...
    EXPECT_EQ(0u, message_center()->NotificationCount());
    EXPECT_FALSE(NotificationsContain(message_center()->GetVisibleNotifications(), "id1"));
    popups = message_center()->GetPopupNotifications();
...
TEST_F(MessageCenterImplTest, NotificationsUpdatedWhenBlockerRemoved)
{
    ...
    NotifierId notifier_id2(NotifierType::APPLICATION, "app2");
    message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id1", notifier_id1.id));
    message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id2", notifier_id2.id));
    message_center()->DisplayedNotification("id1", DISPLAY_SOURCE_POPUP);
...

    {
        ...
        ASSERT_EQ(1u, message_center()->NotificationCount());
        ASSERT_FALSE(NotificationsContain(message_center()->GetVisibleNotifications(), "id1"));
        ASSERT_TRUE(NotificationsContain(message_center()->GetVisibleNotifications(), "id2"));
        popups = message_center()->GetPopupNotifications();
        ASSERT_EQ(1u, popups.size());
        ...
        message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id3", notifier_id1.id));
        message_center()->DisplayedNotification("id3", DISPLAY_SOURCE_POPUP);

        // Verify that "id3" is blocked.
        ASSERT_EQ(1u, message_center()->NotificationCount());
        ASSERT_FALSE(NotificationsContain(message_center()->GetVisibleNotifications(), "id3"));
        popups = message_center()->GetPopupNotifications();
        ASSERT_EQ(1u, popups.size());
...
TEST_F(MessageCenterImplTest, RemoveAllNotificationsWithPinned)
{
    ...
    auto notification3 = std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "id3", u"title", u"message", ui::ImageModel() /* icon */,
        std::u16string() /* display_source */, GURL(), notifier_id1, RichNotificationData(), nullptr);
    notification3->set_pinned(true);
    message_center()->AddNotification(std::move(notification3));

    // Notification 4: Invisible, pinned
    auto notification4 = std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "id4", u"title", u"message", ui::ImageModel() /* icon */,
        std::u16string() /* display_source */, GURL(), notifier_id2, RichNotificationData(), nullptr);
...
TEST_F(MessageCenterImplTest, NotifierEnabledChanged)
{
    ...
    message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id1-1", "app1"));
    message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id1-2", "app1"));
    message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id1-3", "app1"));
    message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id2-1", "app2"));
    message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id2-2", "app2"));
    message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id2-3", "app2"));
    message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id2-4", "app2"));
    message_center()->AddNotification(CreateSimpleNotificationWithNotifierId("id2-5", "app2"));
    ASSERT_EQ(8u, message_center()->NotificationCount());

    // Removing all of app2's notifications should only leave app1's.
    message_center()->RemoveNotificationsForNotifierId(NotifierId(NotifierType::APPLICATION, "app2"));
    ...
    message_center()->RemoveNotificationsForNotifierId(NotifierId(NotifierType::APPLICATION, "app1"));
    ASSERT_EQ(0u, message_center()->NotificationCount());
}
...
TEST_F(MessageCenterImplTest, RemoveInCloseHandler)
{
    ...
    auto notification = CreateSimpleNotificationWithDelegate(id, base::MakeRefCounted<DeleteOnCloseDelegate>(message_center(), id));
    message_center()->AddNotification(std::move(notification));
    EXPECT_TRUE(message_center()->FindVisibleNotificationById(id));
...
TEST_F(MessageCenterImplTest, RemoveInCloseHandlerCloseAll)
{
    ...
    auto notification1 = CreateSimpleNotificationWithDelegate(id1, base::MakeRefCounted<DeleteOnCloseDelegate>(message_center(), id1));
    auto notification2 = CreateSimpleNotificationWithDelegate(id2, base::MakeRefCounted<DeleteOnCloseDelegate>(message_center(), id2));
    ...
    message_center()->RemoveAllNotifications(true /* by_user */, message_center::MessageCenter::RemoveType::NON_PINNED);
    EXPECT_FALSE(message_center()->FindVisibleNotificationById(id1));
    EXPECT_FALSE(message_center()->FindVisibleNotificationById(id2));
}
...
TEST_F(MessageCenterImplTest, FindNotificationsByAppId)
{
...

    {
        ...
        const std::string id1("id1");
        std::unique_ptr<Notification> notification = CreateNotificationWithNotifierId(id1, app_id1, NOTIFICATION_TYPE_SIMPLE);
        message_center()->AddNotification(std::move(notification));
        ...
        notification = CreateNotificationWithNotifierId(id1, app_id1, NOTIFICATION_TYPE_SIMPLE);
        message_center()->AddNotification(std::move(notification));

        const std::string id2("id2");
        notification = CreateNotificationWithNotifierId(id2, app_id1, NOTIFICATION_TYPE_SIMPLE);
        message_center()->AddNotification(std::move(notification));
        ...
        notification = CreateNotificationWithNotifierId(id3, app_id2, NOTIFICATION_TYPE_SIMPLE);
        message_center()->AddNotification(std::move(notification));
        EXPECT_EQ(1u, message_center()->FindNotificationsByAppId(app_id1).size());
    ...
    for (std::string app_id : { app_id1, app_id2 }) {
        for (Notification* notification : message_center()->FindNotificationsByAppId(app_id)) {
            EXPECT_EQ(app_id, notification->notifier_id().id);
        }
    }

    // Remove all notifications.
    message_center()->RemoveAllNotifications(true, MessageCenterImpl::RemoveType::ALL);
...
TEST_F(MessageCenterImplTest, UpdateProgressNotificationWhenCenterVisible)
{
    ...
    message_center()->UpdateNotification("n", std::make_unique<Notification>(notification_copy));
...
TEST_F(MessageCenterImplTest, UpdateNonProgressNotificationWhenCenterVisible)
{
    ...
    message_center()->UpdateNotification(notification_copy.id(), std::make_unique<Notification>(notification_copy));
...
TEST_F(MessageCenterImplTest, UpdateNonProgressToProgressNotificationWhenCenterVisible)
{
    TestAddObserver observer(message_center());
...
TEST_F(MessageCenterImplTest, RemoveAfterClick)
{
    ...
    message_center()->AddNotification(CreateSimpleNotificationWithOptionalFields(id, notification_data));
