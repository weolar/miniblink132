...
public:
    ...
    virtual NotificationList::Notifications FindNotificationsByAppId(const std::string& app_id) = 0;
    ...
    virtual NotificationList::Notifications GetVisibleNotificationsWithoutBlocker(const NotificationBlocker* blocker) const = 0;
    ...
    virtual NotificationList::PopupNotifications GetPopupNotificationsWithoutBlocker(const NotificationBlocker& blocker) const = 0;
    ...
    virtual void UpdateNotification(const std::string& old_id, std::unique_ptr<Notification> new_notification) = 0;

    // Removes an existing notification.
    virtual void RemoveNotification(const std::string& id, bool by_user) = 0;
    virtual void RemoveNotificationsForNotifierId(const NotifierId& notifier_id) = 0;
    ...
    virtual void SetNotificationIcon(const std::string& notification_id, const ui::ImageModel& image) = 0;

    // Sets the large image for the notifications of type == TYPE_IMAGE. Specified
    // image will appear below of the notification.
    virtual void SetNotificationImage(const std::string& notification_id, const gfx::Image& image) = 0;
    ...
    // center observers.
    virtual void ClickOnNotificationButton(const std::string& id, int button_index) = 0;
    ...
    virtual void ClickOnNotificationButtonWithReply(const std::string& id, int button_index, const std::u16string& reply) = 0;
    ...
    virtual void MarkSinglePopupAsShown(const std::string& id, bool mark_notification_as_read) = 0;
    ...
    virtual void DisplayedNotification(const std::string& id, const DisplaySource source) = 0;

    // This can be called to change the quiet mode state (without a timeout).
    virtual void SetQuietMode(bool in_quiet_mode, QuietModeSourceType type = QuietModeSourceType::kUserAction) = 0;
    ...
    virtual void SetNotificationExpandState(const std::string& id, const ExpandState state) = 0;
