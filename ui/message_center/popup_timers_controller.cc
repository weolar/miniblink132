...
bool UseHighPriorityDelay(Notification* notification)
{
    ...
    const bool use_high_priority_delay = notification->never_timeout() && notification->priority() == SYSTEM_PRIORITY;
#else
    ...
    const bool use_high_priority_delay = notification->priority() > DEFAULT_PRIORITY || notification->notifier_id().type == NotifierType::WEB_PAGE;
#endif
...
int notification_timeout_high_priority_seconds_ = kAutocloseHighPriorityDelaySeconds;
...
void PopupTimersController::StartTimer(const std::string& id, const base::TimeDelta& timeout)
{
    ...
    auto timer = std::make_unique<PopupTimer>(id, timeout, weak_ptr_factory_.GetWeakPtr());
...
void PopupTimersController::SetNotificationTimeouts(int default_timeout, int high_priority_timeout)
{
...
base::TimeDelta PopupTimersController::GetTimeoutForNotification(Notification* notification)
{
    return base::Seconds(UseHighPriorityDelay(notification) ? notification_timeout_high_priority_seconds_ : notification_timeout_default_seconds_);
}
...
void PopupTimersController::OnNotificationDisplayed(const std::string& id, const DisplaySource source)
{
...
void PopupTimersController::OnNotificationUpdated(const std::string& id)
{
    ...
    // flag for now.
    const bool must_cancel_timer = (*iter)->never_timeout()
#if BUILDFLAG(IS_CHROMEOS)
...
void PopupTimersController::OnNotificationRemoved(const std::string& id, bool by_user)
{
    CancelTimer(id);
}
