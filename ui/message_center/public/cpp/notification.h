...
public:
    NotificationItem(const std::u16string& title, const std::u16string& message, ui::ImageModel icon = ui::ImageModel());
    ...
    const std::optional<ui::ImageModel>& icon() const
    {
        return icon_;
    }
...
struct MESSAGE_CENTER_PUBLIC_EXPORT ButtonInfo {
    ...
    ButtonInfo(const gfx::VectorIcon* vector_icon, const std::u16string& accessible_name);
    ButtonInfo();
...
public:
    ...
    // RAW_PTR_EXCLUSION: Never allocated by PartitionAlloc (always points to a
    // global), so there is no benefit to using a raw_ptr, only cost.
    RAW_PTR_EXCLUSION const gfx::VectorIcon* parent_vector_small_image = &gfx::kNoneIcon;
...
public:
    ...
    Notification(NotificationType type, const std::string& id, const std::u16string& title, const std::u16string& message, const ui::ImageModel& icon,
    ...
    // by |delegate|.
    Notification(scoped_refptr<NotificationDelegate> delegate, const Notification& other);
    ...
    void set_type(NotificationType type)
    {
        type_ = type;
    }
    ...
    bool allow_group() const
    {
        return allow_group_;
    }
    void set_allow_group(bool allow_group)
    {
        allow_group_ = allow_group;
    }

    bool group_child() const
    {
        return group_child_;
    }
    bool group_parent() const
    {
        return group_parent_;
    }
    ...
    int priority() const
    {
        return optional_fields_.priority;
    }
    void set_priority(int priority)
    {
        optional_fields_.priority = priority;
    }
    ...
    bool renotify() const
    {
        return optional_fields_.renotify;
    }
    void set_renotify(bool renotify)
    {
        optional_fields_.renotify = renotify;
    }

    // This property currently has no effect on non-Android platforms.
    bool silent() const
    {
        return optional_fields_.silent;
    }
    void set_silent(bool silent)
    {
        optional_fields_.silent = silent;
    }

    base::Time timestamp() const
    {
        return optional_fields_.timestamp;
    }
    ...
    int progress() const
    {
        return optional_fields_.progress;
    }
    void set_progress(int progress)
    {
        optional_fields_.progress = progress;
    }
    ...
    void set_icon(const ui::ImageModel& icon)
    {
        icon_ = icon;
    }
    ...
    const gfx::Image& small_image() const
    {
        return optional_fields_.small_image;
    }
    ...
    {
        optional_fields_.small_image_needs_additional_masking = needs_additional_masking;
    }
    ...
    // masking.
    gfx::Image GenerateMaskedSmallIcon(int dip_size, SkColor mask_color, SkColor background_color, SkColor foreground_color) const;

    gfx::Image GetMaskedSmallImage(const gfx::ImageSkia& small_image, SkColor background_color, SkColor foreground_color) const;
    ...
    bool never_timeout() const
    {
        return optional_fields_.never_timeout;
    }
    ...
    {
        optional_fields_.pinned = pinned;
    }
#endif // BUILDFLAG(IS_CHROMEOS)
    ...
    {
        return optional_fields_.settings_button_handler != SettingsButtonHandler::NONE;
    }
    ...
    {
        return optional_fields_.scenario;
    }
...
#if BUILDFLAG(IS_CHROMEOS)
    void set_system_notification_warning_level(SystemNotificationWarningLevel warning_level)
    {
    ...
    const std::string& custom_view_type() const
    {
        return custom_view_type_;
    }
    ...
    void set_host_view_element_id(const ui::ElementIdentifier host_view_element_id)
    {
...
private:
    ...
    SystemNotificationWarningLevel system_notification_warning_level_ = SystemNotificationWarningLevel::NORMAL;
#endif // BUILDFLAG(IS_CHROMEOS)
};
