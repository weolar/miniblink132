...
// notifications.
#if BUILDFLAG(IS_CHROMEOS)
    NotifierId(NotifierType type, const std::string& id, ash::NotificationCatalogName catalog_name = ash::NotificationCatalogName::kNone);
#else
    ...
    NotifierId(const GURL& url, std::optional<std::u16string> title, std::optional<std::string> web_app_id);
