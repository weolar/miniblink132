...
#if BUILDFLAG(IS_CHROMEOS)
NotifierId::NotifierId(NotifierType type, const std::string& id, ash::NotificationCatalogName catalog_name)
    : type(type)
    , id(id)
    , catalog_name(catalog_name)
{
    if (type == NotifierType::SYSTEM_COMPONENT) {
        DCHECK_NE(catalog_name, ash::NotificationCatalogName::kNone) << " System Notifications must include a `catalog_name`.";
    }
...
NotifierId::NotifierId(const GURL& url, std::optional<std::u16string> title, std::optional<std::string> web_app_id)
...
#if BUILDFLAG(IS_CHROMEOS)
    if (type == NotifierType::SYSTEM_COMPONENT && catalog_name != other.catalog_name) {
        return false;
    }
#endif // BUILDFLAG(IS_CHROMEOS)
