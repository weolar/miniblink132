...
TEST_F(NotificationBlockerTest, OnlyRegisteredAfterInit)
{
    ...
    EXPECT_EQ(initial_count, message_center.add_notification_blocker_called_count());

    // Initialize `blocker`.
    blocker.Init();

    // Now `blocker` should have been registered with the `MessageCenter`.
    EXPECT_EQ(initial_count + 1, message_center.add_notification_blocker_called_count());
}
