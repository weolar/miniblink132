...
class MESSAGE_CENTER_EXPORT MessagePopupCollection : public MessageCenterObserver, public NotificationViewController, public gfx::AnimationDelegate {
public:
    MessagePopupCollection();
    MessagePopupCollection(const MessagePopupCollection& other) = delete;
    MessagePopupCollection& operator=(const MessagePopupCollection& other) = delete;
    ...
    MessageView* GetMessageViewForNotificationId(const std::string& notification_id) override;
    void ConvertNotificationViewToGroupedNotificationView(
        const std::string& ungrouped_notification_id, const std::string& new_grouped_notification_id) override;
    void ConvertGroupedNotificationViewToNotificationView(const std::string& grouped_notification_id, const std::string& new_single_notification_id) override;
    void OnChildNotificationViewUpdated(const std::string& parent_notification_id, const std::string& child_notification_id) override;
    ...
    MessagePopupView* GetPopupViewForNotificationID(const std::string& notification_id);
    ...
    virtual void ConfigureWidgetInitParamsForContainer(views::Widget* widget, views::Widget::InitParams* init_params) = 0;

    size_t GetPopupItemsCount();

    gfx::Rect popup_collection_bounds()
    {
        return popup_collection_bounds_;
    }
...
protected:
    ...
    virtual bool BlockForMixedFullscreen(const Notification& notification) const = 0;
    ...
    const std::vector<PopupItem>& popup_items()
    {
        return popup_items_;
    }
...
private:
    ...
    void CloseAndRemovePopupFromPopupItem(MessagePopupView* popup, bool remove_only = false);
    ...
    base::ScopedObservation<MessageCenter, MessageCenterObserver> message_center_observation_ { this };
