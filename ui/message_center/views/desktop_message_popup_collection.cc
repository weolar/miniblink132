...
int DesktopMessagePopupCollection::GetPopupOriginX(const gfx::Rect& popup_bounds) const
{
    if (IsFromLeft())
...
{
    return IsTopDown() ? work_area_.y() + kMarginBetweenPopups : work_area_.bottom() - kMarginBetweenPopups;
}
...
bool DesktopMessagePopupCollection::RecomputeAlignment(const display::Display& display)
{
    ...
    alignment_ = work_area_.y() > display.bounds().y() ? POPUP_ALIGNMENT_TOP : POPUP_ALIGNMENT_BOTTOM;
    ...
    alignment_ |= (work_area_.x() > display.bounds().x() && work_area_.y() == display.bounds().y()) ? POPUP_ALIGNMENT_LEFT : POPUP_ALIGNMENT_RIGHT;

    return true;
}

void DesktopMessagePopupCollection::ConfigureWidgetInitParamsForContainer(views::Widget* widget, views::Widget::InitParams* init_params)
{
...
bool DesktopMessagePopupCollection::BlockForMixedFullscreen(const Notification& notification) const
{
...
void DesktopMessagePopupCollection::OnDisplayAdded(const display::Display& added_display)
{
    // The added display could be the new primary display.
    UpdatePrimaryDisplay();
}

void DesktopMessagePopupCollection::OnDisplaysRemoved(const display::Displays& removed_displays)
{
    // One of the removed displays may have been the primary display.
    UpdatePrimaryDisplay();
}

void DesktopMessagePopupCollection::OnDisplayMetricsChanged(const display::Display& display, uint32_t metrics)
{
