...
NotificationInputContainer::NotificationInputContainer(NotificationInputDelegate* delegate)
    : delegate_(delegate)
    , textfield_(new views::Textfield())
    , button_(new views::ImageButton(base::BindRepeating(
          [](NotificationInputContainer* container) {
              container->delegate_->OnNotificationInputSubmit(container->textfield_->GetProperty(kTextfieldIndexKey), container->textfield_->GetText());
          },
...
void NotificationInputContainer::Init()
{
    ...
    button_->GetViewAccessibility().SetName(l10n_util::GetStringUTF16(GetDefaultAccessibleNameStringId()));
    button_->SetTooltipText(l10n_util::GetStringUTF16(GetDefaultAccessibleNameStringId()));
...
void NotificationInputContainer::SetPlaceholderText(const std::optional<std::u16string>& placeholder)
{
    textfield_->SetPlaceholderText(placeholder->empty() ? l10n_util::GetStringUTF16(GetDefaultPlaceholderStringId()) : *placeholder);
}
...
void NotificationInputContainer::AnimateBackground(const ui::Event& event)
{
    ...
    DCHECK(ink_drop);
    ink_drop->AnimateToState(views::InkDropState::ACTION_PENDING, ui::LocatedEvent::FromIfValid(located_event.get()));
}

void NotificationInputContainer::AddLayerToRegion(ui::Layer* layer, views::LayerRegion region)
{
    if (!ink_drop_container_)
        return;
...
void NotificationInputContainer::OnThemeChanged()
{
    ...
    textfield_->SetTextColor(color_provider->GetColor(ui::kColorNotificationInputForeground));
    StyleTextfield();
    if (ink_drop_container_)
        textfield_->SetBackgroundColor(SK_ColorTRANSPARENT);
    textfield_->set_placeholder_text_color(color_provider->GetColor(ui::kColorNotificationInputPlaceholderForeground));
    UpdateButtonImage();
}

views::ProposedLayout NotificationInputContainer::CalculateProposedLayout(const views::SizeBounds& size_bounds) const
{
    ...
    layout.child_layouts.emplace_back(ink_drop_container_.get(), ink_drop_container_->GetVisible(), gfx::Rect(layout.host_size));
    return layout;
}

bool NotificationInputContainer::HandleKeyEvent(views::Textfield* sender, const ui::KeyEvent& event)
{
    if (event.type() == ui::EventType::kKeyPressed && event.key_code() == ui::VKEY_RETURN) {
        delegate_->OnNotificationInputSubmit(textfield_->GetProperty(kTextfieldIndexKey), textfield_->GetText());
...
{
    return SetLayoutManager(std::make_unique<views::BoxLayout>(views::BoxLayout::Orientation::kHorizontal,
...
views::InkDropContainerView* NotificationInputContainer::InstallInkDrop()
{
    ...
    views::InkDrop::Get(this)->SetBaseColorId(ui::kColorNotificationInputBackground);

    auto* ink_drop_container = AddChildView(std::make_unique<views::InkDropContainerView>());
...
void NotificationInputContainer::UpdateButtonImage()
{
    ...
    auto icon_color_id = textfield_->GetText().empty() ? ui::kColorNotificationInputPlaceholderForeground : ui::kColorNotificationInputForeground;
    button_->SetImageModel(views::Button::STATE_NORMAL,
        ui::ImageModel::FromVectorIcon(kNotificationInlineReplyIcon, GetColorProvider()->GetColor(icon_color_id), kInputReplyButtonSize));
}
