...
public:
    ...
    MockMessagePopupCollection& operator=(const MockMessagePopupCollection&) = delete;
    ...
    bool popup_timer_started() const
    {
        return popup_timer_started_;
    }
...
protected:
    ...
    void ConfigureWidgetInitParamsForContainer(views::Widget* widget, views::Widget::InitParams* init_params) override
    {
...
public:
    MockMessagePopupView(const std::string& id, int init_height, MockMessagePopupCollection* popup_collection)
        : MessagePopupView(popup_collection)
        , popup_collection_(popup_collection)
        , id_(id)
        , title_(base::UTF16ToUTF8(MessageCenter::Get()->FindVisibleNotificationById(id)->title()))
    {
    ...
    void UpdateContentsForChildNotification(const std::string& notification_id, const Notification& notification) override
    {
        child_updated_ = true;
    ...
    {
        if (expandable_)
            children().front()->SetPreferredSize(gfx::Size(GetNotificationWidth(), 42));
    }

    void SetPreferredHeight(int height)
    {
        children().front()->SetPreferredSize(gfx::Size(GetNotificationWidth(), height));
    }

    void SetHovered(bool is_hovered)
    {
        if (is_hovered) {
            ui::MouseEvent enter_event(ui::EventType::kMouseEntered, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), 0, 0);
    ...
    void SimulateFocused()
    {
        OnDidChangeFocus(nullptr, children().front());
    }
    ...
    bool child_updated() const
    {
        return child_updated_;
    }
    const std::string& child_updated_notification_id()
    {
        return child_updated_notification_id_;
    }
    const std::string& child_updated_title()
    {
        return child_updated_title_;
    }
...
MessagePopupView* MockMessagePopupCollection::CreatePopup(const Notification& notification)
{
    auto* popup = new MockMessagePopupView(notification.id(), new_popup_height_, this);
...
class MessagePopupCollectionTest : public views::ViewsTestBase, public MessageCenterObserver {
public:
    MessagePopupCollectionTest() = default;

    MessagePopupCollectionTest(const MessagePopupCollectionTest&) = delete;
    MessagePopupCollectionTest& operator=(const MessagePopupCollectionTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        popup_collection_ = std::make_unique<MockMessagePopupCollection>(GetContext());
    ...
    {
        MessageCenter::Get()->RemoveAllNotifications(false /* by_user */, MessageCenter::RemoveType::ALL);
        AnimateUntilIdle();
    ...
    void OnNotificationDisplayed(const std::string& notification_id, const DisplaySource source) override
    {
...
protected:
    ...
    std::unique_ptr<Notification> CreateNotification(const std::string& id, const std::string& title)
    {
        return std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, id, base::UTF8ToUTF16(title), u"test message", ui::ImageModel(),
            std::u16string() /* display_source */, GURL(), NotifierId(), RichNotificationData(), new NotificationDelegate());
    }

    std::unique_ptr<Notification> CreateLowPriorityNotification()
    {
        std::unique_ptr<Notification> notification = CreateNotification(base::NumberToString(id_++));
    ...
    {
        return popup_collection_->IsAnimating();
    }
    ...
    {
        popup_collection()->CloseAllPopupsNow();
    }

    size_t GetPopupCounts() const
    {
        return popup_collection_->popups().size();
    }

    void SetDisplayInfo(const gfx::Rect& work_area, const gfx::Rect& display_bounds)
    {
...
TEST_F(MessagePopupCollectionTest, FadeInMultipleNotifications)
{
    ...
    for (size_t i = 0; i < ids.size() - 1; ++i) {
        EXPECT_GT(GetPopupAt(i)->GetBoundsInScreen().x(), GetPopupAt(i + 1)->GetBoundsInScreen().bottom());
    }
}
...
TEST_F(MessagePopupCollectionTest, OnChildNotificationViewUpdated)
{
    ...
    MessageCenter::Get()->UpdateNotification(child_id, std::move(new_notification));
...
TEST_F(MessagePopupCollectionTest, PopupResized)
{
    ...
    EXPECT_EQ(previous_bounds[1].bottom(), GetPopup(ids[1])->GetBoundsInScreen().bottom());
    EXPECT_GT(previous_bounds[1].y(), GetPopup(ids[1])->GetBoundsInScreen().y());
    EXPECT_GT(previous_bounds[2].bottom(), GetPopup(ids[2])->GetBoundsInScreen().bottom());
...
TEST_F(MessagePopupCollectionTest, PopupCollectionBounds)
{
    ...
    int expected_height = r0.height() + kMarginBetweenPopups + r1.height() + kMarginBetweenPopups + r2.height();

    EXPECT_EQ(gfx::Rect(r2.x(), r2.y(), GetNotificationWidth(), expected_height), popup_collection()->popup_collection_bounds());
    ...
    EXPECT_EQ(
        gfx::Rect(r2.x(), r2.y(), GetNotificationWidth(), r1.height() + kMarginBetweenPopups + r2.height()), popup_collection()->popup_collection_bounds());
}
...
TEST_F(MessagePopupCollectionTest, RemoveNotificationWhileAnimating)
{
    ...
    auto old_notification = CreateNotification(notification_id, old_notification_title);
    ...
    auto new_notification = CreateNotification(notification_id, new_notification_title);
    EXPECT_TRUE(IsAnimating());
