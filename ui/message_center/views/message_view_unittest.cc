...
public:
        ...
        : MessageView(notification)
    {
        ...
        AddChildView(std::make_unique<views::Label>())->SetText(notification.title());
        AddChildView((std::make_unique<views::Label>()))->SetText(notification.message());
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        notification_ = std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "id", u"title", u"test message", ui::ImageModel(),
        ...
        message_view_ = widget_->SetContentsView(std::make_unique<TestMessageView>(*notification_.get()));
        widget_->Show();
    }
...
protected:
    TestMessageView* message_view()
    {
        return message_view_.get();
    }
...
{
    ui::test::EventGenerator event_generator(GetRootWindow(message_view()->GetWidget()));

    // Expect a call on the mouse entering the message view.
    EXPECT_CALL(*message_view(), UpdateControlButtonsVisibility);
    event_generator.MoveMouseTo(message_view()->GetBoundsInScreen().CenterPoint(), 10);

    // Expect a call on the mouse exiting the message view.
    EXPECT_CALL(*message_view(), UpdateControlButtonsVisibility);
    event_generator.MoveMouseTo(message_view()->GetBoundsInScreen().origin() - gfx::Vector2d(10, 10), 10);
}
...
TEST_F(MessageViewTest, AccessibleAttributes)
{
    ...
    auto notification = std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, "notification", u"", u"", ui::ImageModel(),
        /*display_source=*/std::u16string(), GURL(), NotifierId(), RichNotificationData(), /*delegate=*/nullptr);
    ...
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kRoleDescription), l10n_util::GetStringUTF16(IDS_MESSAGE_NOTIFICATION_ACCESSIBLE_NAME));
}
