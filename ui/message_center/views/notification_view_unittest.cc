...
constexpr SkColor kActionButtonBackgroundColor = SkColorSetRGB(0xF2, 0xF2, 0xF2);
...
{
    SkColor contrast_color = color_utils::BlendForMinContrast(foreground, background).color;
    float contrast_ratio = color_utils::GetContrastRatio(background, contrast_color);
...
public:
    ...
    {
        disable_notification_called_ = true;
    }

    bool disable_notification_called()
    {
...
public:
    ...
    std::unique_ptr<Notification> CreateSimpleNotificationWithRichData(const RichNotificationData& optional_fields) const
    {
        std::unique_ptr<Notification> notification = std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, std::string(kDefaultNotificationId), u"title",
            ...
            NotifierId(NotifierType::APPLICATION, "extension_id"), optional_fields, delegate_);
    ...
    gfx::Size GetImagePaintSize(ProportionalImageView* view)
    {
        ...
        static_assert(kBitmapColor != SK_ColorBLACK, "The bitmap color must match the background color");
...
protected:
    ...
    {
        return notification_view_->left_content();
    }
    views::Label* title_view()
    {
        return notification_view_->title_view_;
    }
    views::Label* message_label()
    {
        return notification_view_->message_label();
    }
    ...
    bool ink_drop_stopped() const
    {
        return ink_drop_stopped_;
    }
...
private:
    ...
    void OnNotificationRemoved(const std::string& notification_id, bool by_user) override
    {
        if (delete_on_notification_removed_) {
            views::InkDrop::Get(notification_view_)->SetMode(views::InkDropHost::InkDropMode::OFF);
            notification_view_.ExtractAsDangling()->GetWidget()->CloseNow();
            return;
        }
    }
...
TEST_F(NotificationViewTest, TestIconSizing)
{
    ...
    notification->set_icon(ui::ImageModel::FromImage(gfx::test::CreateImage(kIconSize / 2, kIconSize / 4)));
    UpdateNotificationViews(*notification);
    EXPECT_EQ(gfx::Size(kIconSize / 2, kIconSize / 4).ToString(), GetImagePaintSize(view).ToString());
    ...
    UpdateNotificationViews(*notification);
    EXPECT_EQ(gfx::Size(kIconSize, kIconSize).ToString(), GetImagePaintSize(view).ToString());

    // Icons over the maximum size should be scaled down, maintaining proportions.
    notification->set_icon(ui::ImageModel::FromImage(gfx::test::CreateImage(2 * kIconSize)));
    UpdateNotificationViews(*notification);
    EXPECT_EQ(gfx::Size(kIconSize, kIconSize).ToString(), GetImagePaintSize(view).ToString());

    notification->set_icon(ui::ImageModel::FromImage(gfx::test::CreateImage(4 * kIconSize, 2 * kIconSize)));
    UpdateNotificationViews(*notification);
    EXPECT_EQ(gfx::Size(kIconSize, kIconSize / 2).ToString(), GetImagePaintSize(view).ToString());
}
...
TEST_F(NotificationViewTest, ManuallyExpandedOrCollapsed)
{
    ...
    gfx::Point done_cursor_location = notification_view()->header_row_->GetBoundsInScreen().CenterPoint();
...
TEST_F(NotificationViewTest, InlineSettingsNotBlock)
{
    ...
    gfx::Point done_cursor_location = settings_done_button()->GetBoundsInScreen().CenterPoint();
    generator.MoveMouseTo(done_cursor_location);
...
TEST_F(NotificationViewTest, InlineSettingsBlockAll)
{
    ...
    gfx::Point block_cursor_location = block_all_button()->GetBoundsInScreen().CenterPoint();
    gfx::Point done_cursor_location = settings_done_button()->GetBoundsInScreen().CenterPoint();
...
TEST_F(NotificationViewTest, DISABLED_TestAccentColor)
{
    ...
    notification_view()->GetWidget()->GetNativeTheme()->set_use_dark_colors(false);
    UpdateNotificationViews(*notification);
    ...
    auto app_icon_color_matches = [&](SkColor color) {
        SkBitmap expected = notification
                                ->GenerateMaskedSmallIcon(kSmallImageSizeMD, color, color_provider->GetColor(ui::kColorNotificationIconBackground),
                                    color_provider->GetColor(ui::kColorNotificationIconForeground))
                                .AsBitmap();
        SkBitmap actual = *notification_view()->header_row_->app_icon_view_for_testing()->GetImage().bitmap();
    ...
    const SkColor kActionButtonTextColor = DeriveMinContrastColor(gfx::kGoogleBlue600, kActionButtonBackgroundColor);
    EXPECT_FALSE(notification_view()->header_row_->color_for_testing().has_value());
    EXPECT_EQ(kActionButtonTextColor, notification_view()->action_buttons_[0]->GetCurrentTextColor());
    EXPECT_EQ(kActionButtonTextColor, notification_view()->action_buttons_[1]->GetCurrentTextColor());
    ...
    EXPECT_EQ(kDarkCustomAccentColor, notification_view()->action_buttons_[0]->GetCurrentTextColor());
    EXPECT_EQ(kDarkCustomAccentColor, notification_view()->action_buttons_[1]->GetCurrentTextColor());
    ...
    SkColor expected_color_title = DeriveMinContrastColor(kBrightCustomAccentColor, kNotificationBackgroundColor);
    // Action buttons have a darker background.
    SkColor expected_color_actions = DeriveMinContrastColor(kBrightCustomAccentColor, kActionButtonBackgroundColor);
    ...
    EXPECT_EQ(expected_color_actions, notification_view()->action_buttons_[0]->GetCurrentTextColor());
    EXPECT_EQ(expected_color_actions, notification_view()->action_buttons_[1]->GetCurrentTextColor());
...
TEST_F(NotificationViewTest, TestAccentColorTextFlagAffectsHeaderRow)
{
    ...
    notification->set_type(NotificationType::NOTIFICATION_TYPE_SIMPLE);
    UpdateNotificationViews(*notification);
    EXPECT_EQ(*notification_view()->header_row_->color_for_testing(), data.accent_color);
}
...
TEST_F(NotificationViewTest, InkDropClipRect)
{
    ...
    auto* ink_drop = static_cast<views::InkDropImpl*>(views::InkDrop::Get(notification_view())->GetInkDrop());
    ...
    EXPECT_EQ(notification_view()->GetPreferredSize({}) - insets.size(), clip_rect.size());
...
TEST_F(NotificationViewTest, AppIconWebAppNotification)
{
    ...
    const SkBitmap* app_icon_view = header_row()->app_icon_view_for_testing()->GetImage().bitmap();

    EXPECT_EQ(color_utils::SkColorToRgbaString(SK_ColorTRANSPARENT), color_utils::SkColorToRgbaString(app_icon_view->getColor(8, 8)));

    EXPECT_EQ(color_utils::SkColorToRgbaString(SK_ColorYELLOW), color_utils::SkColorToRgbaString(app_icon_view->getColor(0, 0)));
}
...
TEST_F(NotificationViewTest, PreferredSize)
{
    ...
    EXPECT_EQ(GetNotificationWidth(), notification_view()->GetPreferredSize({}).width());
...
TEST_F(NotificationViewTest, ExpandLongMessage)
{
    ...
    const int collapsed_preferred_height = notification_view()->GetPreferredSize({}).height();
    ...
    EXPECT_LT(collapsed_preferred_height, notification_view()->GetPreferredSize({}).height());

    ToggleExpanded();
    EXPECT_FALSE(notification_view()->expanded_);
    EXPECT_EQ(collapsed_height, message_label()->height());
    EXPECT_EQ(collapsed_preferred_height, notification_view()->GetPreferredSize({}).height());
}
...
TEST_F(NotificationViewTest, InlineSettingsInkDropAnimation)
{
    ...
    ui::ScopedAnimationDurationScaleMode zero_duration_scope(ui::ScopedAnimationDurationScaleMode::NORMAL_DURATION);
    std::unique_ptr<Notification> notification = CreateSimpleNotification();
    notification->set_type(NOTIFICATION_TYPE_SIMPLE);
    UpdateNotificationViews(*notification);
...
TEST_F(NotificationViewTest, TestDeleteOnDisableNotification)
{
    ...
    views::test::ButtonTestApi(settings_done_button()).NotifyClick(ui::test::TestEvent());
}
...
TEST_F(NotificationViewTest, TestAccentColorTextFlagAffectsActionButtons)
{
    ...
    for (views::LabelButton* action_button : action_buttons()) {
        EXPECT_NE(notification_view()->GetActionButtonColorForTesting(action_button), data.accent_color);
    }
    ...
    EXPECT_EQ(action_buttons().size(), 2u);
    for (views::LabelButton* action_button : action_buttons()) {
        EXPECT_EQ(notification_view()->GetActionButtonColorForTesting(action_button), data.accent_color);
    }
}
...
TEST_F(NotificationViewTest, UpdateFiresAccessibilityEvents)
{
    ...
    EXPECT_EQ(0, counter.GetCount(ax::mojom::Event::kTextChanged, notification_view()));

    // Updating the view after changing the title should cause one text-changed
    // event to be fired on the notification view.
    UpdateNotificationViews(*notification);
    EXPECT_EQ(1, counter.GetCount(ax::mojom::Event::kTextChanged, notification_view()));
