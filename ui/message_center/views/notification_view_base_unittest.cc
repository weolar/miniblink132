...
public:
    ...
    void CreateOrUpdateInlineSettingsViews(const Notification& notification) override
    {
    ...
    void CreateOrUpdateSnoozeSettingsViews(const Notification& notification) override
    {
        set_snooze_settings_enabled(notification.notifier_id().type == message_center::NotifierType::ARC_APPLICATION);
    }
    bool IsExpandable() const override
    {
        return true;
    }
    std::unique_ptr<views::LabelButton> GenerateNotificationLabelButton(views::Button::PressedCallback callback, const std::u16string& label) override
    {
...
public:
    ...
    int clicked_button_index() const
    {
        return clicked_button_index_;
    }
    const std::u16string& submitted_reply_string() const
    {
        return submitted_reply_string_;
    }
    void set_expecting_click(bool expecting)
    {
        expecting_click_ = expecting;
    }
...
class NotificationViewBaseTest : public views::ViewsTestBase, public views::ViewObserver {
public:
    ...
    NotificationViewBase* notification_view() const
    {
        return notification_view_;
    }

    void set_delete_on_preferred_size_changed(bool delete_on_preferred_size_changed)
    {
...
protected:
    ...
    std::unique_ptr<Notification> CreateSimpleNotificationWithRichData(const RichNotificationData& optional_fields) const;
...
std::unique_ptr<Notification> NotificationViewBaseTest::CreateSimpleNotification() const
{
...
std::unique_ptr<Notification> NotificationViewBaseTest::CreateSimpleNotificationWithRichData(const RichNotificationData& data) const
{
    std::unique_ptr<Notification> notification = std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, std::string(kDefaultNotificationId), u"title",
        u"message", ui::ImageModel::FromImage(gfx::test::CreateImage(/*size=*/80)), u"display source", GURL(),
...
void NotificationViewBaseTest::OnViewPreferredSizeChanged(views::View* observed_view)
{
    ...
    notification_view_->GetWidget()->SetSize(notification_view()->GetPreferredSize({}));
}
...
void NotificationViewBaseTest::UpdateNotificationViews(const Notification& notification)
{
    MessageCenter::Get()->AddNotification(std::make_unique<Notification>(notification));
    ...
    if (!notification_view_) {
        ...
        auto notification_view = std::make_unique<TestNotificationView>(notification);
        static_cast<views::View*>(notification_view.get())->AddObserver(this);

        views::Widget::InitParams init_params(CreateParams(views::Widget::InitParams::TYPE_POPUP));
...
{
    return notification_view_->GetSlideOutLayer()->transform().To2dTranslation().x();
}

bool NotificationViewBaseTest::IsRemovedAfterIdle(const std::string& notification_id) const
{
    base::RunLoop().RunUntilIdle();
    return !MessageCenter::Get()->FindVisibleNotificationById(notification_id);
}

bool NotificationViewBaseTest::IsPopupRemovedAfterIdle(const std::string& notification_id) const
{
    base::RunLoop().RunUntilIdle();
    return !MessageCenter::Get()->FindPopupNotificationById(notification_id);
}

void NotificationViewBaseTest::DispatchGesture(const ui::GestureEventDetails& details)
{
    ui::test::EventGenerator generator(GetRootWindow(notification_view()->GetWidget()));
...
{
    DispatchGesture(ui::GestureEventDetails(ui::EventType::kGestureScrollUpdate, dx, 0));
}
...
TEST_F(NotificationViewBaseTest, UpdateButtonsStateTest)
{
    ...
    EXPECT_EQ(views::Button::STATE_NORMAL, notification_view()->action_buttons_[0]->GetState());

    // Now construct a mouse move event inside the boundary of the action button.
    gfx::Point cursor_location = notification_view()->action_buttons_[0]->GetBoundsInScreen().CenterPoint();
    ui::test::EventGenerator generator(GetRootWindow(notification_view()->GetWidget()));
    generator.MoveMouseTo(cursor_location);

    EXPECT_EQ(views::Button::STATE_HOVERED, notification_view()->action_buttons_[0]->GetState());
    ...
    cursor_location = notification_view()->action_buttons_[0]->GetBoundsInScreen().origin() + gfx::Vector2d(-1, -1);
    generator.MoveMouseTo(cursor_location);
...
TEST_F(NotificationViewBaseTest, UpdateButtonCountTest)
{
    ...
    EXPECT_EQ(views::Button::STATE_NORMAL, notification_view()->action_buttons_[0]->GetState());
    EXPECT_EQ(views::Button::STATE_NORMAL, notification_view()->action_buttons_[1]->GetState());
...
TEST_F(NotificationViewBaseTest, TestActionButtonClick)
{
    ...
    gfx::Point cursor_location = notification_view()->action_buttons_[1]->GetBoundsInScreen().CenterPoint();
    generator.MoveMouseTo(cursor_location);
...
TEST_F(NotificationViewBaseTest, MAYBE_TestInlineReply)
{
    ...
    // Now construct a mouse click event inside the boundary of the action button.
    cursor_location = notification_view()->action_buttons_[0]->GetBoundsInScreen().CenterPoint();
    generator.MoveMouseTo(cursor_location);
    generator.ClickLeftButton();
    ...
    cursor_location = notification_view()->inline_reply_->button()->GetBoundsInScreen().CenterPoint();
    generator.MoveMouseTo(cursor_location);
    generator.ClickLeftButton();
    EXPECT_EQ(1, delegate_->clicked_button_index());
    EXPECT_EQ(u"test", delegate_->submitted_reply_string());
...
TEST_F(NotificationViewBaseTest, MAYBE_SlideOut)
{
    ...
    ui::ScopedAnimationDurationScaleMode zero_duration_scope(ui::ScopedAnimationDurationScaleMode::ZERO_DURATION);
...
{
    MessageCenter::Get()->RemoveAllNotifications(/*by_user=*/false, MessageCenter::RemoveType::ALL);
    ...
    std::unique_ptr<Notification> notification
        = std::make_unique<Notification>(message_center::NOTIFICATION_TYPE_CUSTOM, kDefaultNotificationId, u"title", u"message", ui::ImageModel(),
            u"display source", GURL(), message_center::NotifierId(message_center::NotifierType::ARC_APPLICATION, "test_app_id"), rich_data, nullptr);

    UpdateNotificationViews(*notification);

    EXPECT_NE(nullptr, notification_view()->GetControlButtonsView()->snooze_button());
}
...
TEST_F(NotificationViewBaseTest, UseImageAsIcon)
{
    ...
    notification->set_icon(ui::ImageModel::FromImage(gfx::test::CreateImage(kIconSize)));
...
TEST_F(NotificationViewBaseTest, UpdateInSettings)
{
    ...
    gfx::Point settings_cursor_location = notification_view()->control_buttons_view_->settings_button()->GetBoundsInScreen().CenterPoint();
    ...
    views::Button* close_button = notification_view()->control_buttons_view_->close_button();
...
TEST_F(NotificationViewBaseTest, TestClick)
{
    ...
    gfx::Point cursor_location(notification_view()->size().width() / 2, notification_view()->size().height() - 2);
    gfx::Point cursor_in_screen = views::View::ConvertPointToScreen(notification_view(), cursor_location);
...
TEST_F(NotificationViewBaseTest, TestDeleteOnToggleExpanded)
{
    ...
    notification->set_title(std::u16string());
    notification->set_message(u"consectetur adipiscing elit, sed do eiusmod tempor incididunt ut "
    ...
    views::test::ButtonTestApi(notification_view()->header_row_).NotifyClick(ui::test::TestEvent());
}
...
TEST_F(NotificationViewBaseTest, TestLongTitleAndMessage)
{
    ...
    notification->set_title(u"consectetur adipiscing elit, sed do eiusmod tempor incididunt ut "
                            u"labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud "
                            u"exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.");
    UpdateNotificationViews(*notification);
...
TEST_F(NotificationViewBaseTest, AppNameSystemNotification)
{
    ...
    RichNotificationData data;
    data.settings_button_handler = SettingsButtonHandler::INLINE;
    auto notification = std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, std::string(kDefaultNotificationId), u"title", u"message", ui::ImageModel(),
        std::u16string(), GURL(),
#if BUILDFLAG(IS_CHROMEOS)
        NotifierId(NotifierType::SYSTEM_COMPONENT, "system", ash::NotificationCatalogName::kTestCatalogName),
#else
...
TEST_F(NotificationViewBaseTest, AppNameWebNotification)
{
    ...
    EXPECT_EQ(u"example.com", notification_view()->header_row_->app_name_for_testing());
}
...
TEST_F(NotificationViewBaseTest, AppNameWebAppNotification)
{
    ...
    std::unique_ptr<Notification> notification = std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, std::string(kDefaultNotificationId), u"title",
        u"message", ui::ImageModel::FromImage(gfx::test::CreateImage(/*size=*/80)), u"display source", GURL(), notifier_id, data, delegate_);
    ...
    EXPECT_EQ(u"web app title", notification_view()->header_row_->app_name_for_testing());
}
...
TEST_F(NotificationViewBaseTest, ShowProgress)
{
    ...
    EXPECT_TRUE(notification_view()->header_row_->summary_text_for_testing()->GetVisible());
}
...
TEST_F(NotificationViewBaseTest, ShowTimestamp)
{
    ...
    EXPECT_TRUE(notification_view()->header_row_->timestamp_view_for_testing()->GetVisible());

    // Expect timestamp view to hide for progress notifications.
    notification->set_type(NOTIFICATION_TYPE_PROGRESS);
    notification->set_progress(50);
    UpdateNotificationViews(*notification);
    EXPECT_FALSE(notification_view()->header_row_->timestamp_view_for_testing()->GetVisible());
}

#if BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_FUCHSIA)
#define MAYBE_SlideOutWithMessageCenterView DISABLED_SlideOutWithMessageCenterView
#else
...
TEST_F(NotificationViewBaseTest, MAYBE_SlideOutByTrackpad)
{
    ...
    ui::test::EventGenerator generator(GetRootWindow(notification_view()->GetWidget()));
    generator.ScrollSequence(gfx::Point(), base::TimeDelta(),
