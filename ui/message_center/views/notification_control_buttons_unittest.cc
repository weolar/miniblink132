...
public:
    ...
    NotificationControlButtonsTest(const NotificationControlButtonsTest&) = delete;
    NotificationControlButtonsTest& operator=(const NotificationControlButtonsTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        Notification notification(NOTIFICATION_TYPE_SIMPLE, "id", u"title", u"id", ui::ImageModel(), std::u16string(), GURL(),
            NotifierId(NotifierType::APPLICATION, "notifier_id"), RichNotificationData(), nullptr);
        message_view_ = std::make_unique<TestMessageView>(notification);
        message_view_->set_control_buttons_view(widget_->SetContentsView(std::make_unique<NotificationControlButtonsView>(message_view_.get())));
    }
    ...
    bool MatchesIcon(views::ImageButton* button, const gfx::VectorIcon& icon, SkColor color, int size = 0)
    {
    ...
    {
        EXPECT_TRUE(MatchesIcon(buttons_view()->close_button(), kNotificationCloseButtonIcon, color));
        EXPECT_TRUE(MatchesIcon(buttons_view()->settings_button(), kNotificationSettingsButtonIcon, color));
        EXPECT_TRUE(MatchesIcon(buttons_view()->snooze_button(), kNotificationSnoozeButtonIcon, color));
    }
...
TEST_F(NotificationControlButtonsTest, IconColor_ContrastEnforcement)
{
    ...
    buttons_view()->SetButtonIconColors(SK_ColorWHITE);
    ExpectIconColor(color_utils::BlendForMinContrast(SK_ColorWHITE, SK_ColorWHITE).color);

    // A dark background should enforce bright enough icons.
    buttons_view()->SetBackgroundColor(SK_ColorBLACK);
    buttons_view()->SetButtonIconColors(SK_ColorBLACK);
    ExpectIconColor(color_utils::BlendForMinContrast(SK_ColorBLACK, SK_ColorBLACK).color);
}
...
TEST_F(NotificationControlButtonsTest, SetIcons)
{
    ...
    const SkColor default_icon_color = buttons_view()->GetColorProvider()->GetColor(ui::kColorIcon);
    EXPECT_TRUE(MatchesIcon(buttons_view()->close_button(), NotificationControlButtonsView::kDefaultCloseIcon, default_icon_color));
    EXPECT_TRUE(MatchesIcon(buttons_view()->settings_button(), NotificationControlButtonsView::kDefaultSettingsIcon, default_icon_color));
    EXPECT_TRUE(MatchesIcon(buttons_view()->snooze_button(), NotificationControlButtonsView::kDefaultSnoozeIcon, default_icon_color));
    ...
    EXPECT_TRUE(MatchesIcon(buttons_view()->close_button(), test_icon, default_icon_color));
    EXPECT_TRUE(MatchesIcon(buttons_view()->settings_button(), test_icon, default_icon_color));
    EXPECT_TRUE(MatchesIcon(buttons_view()->snooze_button(), test_icon, default_icon_color));
}
...
TEST_F(NotificationControlButtonsTest, IconSize)
{
    ...
    buttons_view()->ShowSnoozeButton(true);
    const SkColor default_icon_color = buttons_view()->GetColorProvider()->GetColor(ui::kColorIcon);
    EXPECT_TRUE(MatchesIcon(buttons_view()->close_button(), NotificationControlButtonsView::kDefaultCloseIcon, default_icon_color, custom_size));
    EXPECT_TRUE(MatchesIcon(buttons_view()->settings_button(), NotificationControlButtonsView::kDefaultSettingsIcon, default_icon_color, custom_size));
    EXPECT_TRUE(MatchesIcon(buttons_view()->snooze_button(), NotificationControlButtonsView::kDefaultSnoozeIcon, default_icon_color, custom_size));
...
TEST_F(NotificationControlButtonsTest, BetweenButtonsSpacing)
{
    ...
    EXPECT_EQ(static_cast<views::BoxLayout*>(buttons_view()->GetLayoutManager())->between_child_spacing(),
        NotificationControlButtonsView::kDefaultBetweenButtonSpacing);

    // Use a non-default amount of horizontal spacing.
    const int new_spacing = NotificationControlButtonsView::kDefaultBetweenButtonSpacing + 1;
    ...
    EXPECT_EQ(static_cast<views::BoxLayout*>(buttons_view()->GetLayoutManager())->between_child_spacing(), new_spacing);
