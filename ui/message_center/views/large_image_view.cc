...
LargeImageView::LargeImageView(const gfx::Size& max_size)
    : max_size_(max_size)
    , min_size_(max_size_.width(), /*height=*/0)
{
    SetID(NotificationViewBase::kLargeImageView);

#if !BUILDFLAG(IS_CHROMEOS)
    SetBackground(views::CreateThemedSolidBackground(ui::kColorNotificationImageBackground));
#endif // !BUILDFLAG(IS_CHROMEOS)
}
...
gfx::Size LargeImageView::CalculateResizedImageSizeForWidth() const
{
    ...
    const double proportion = original_size.height() / static_cast<double>(original_size.width());
...
gfx::ImageSkia LargeImageView::CalculateDrawnImage() const
{
    ...
    const gfx::ImageSkia resized_image
        = gfx::ImageSkiaOperations::CreateResizedImage(original_image_, skia::ImageOperations::RESIZE_BEST, CalculateResizedImageSizeForWidth());
    ...
    const gfx::ImageSkia cropped_image = gfx::ImageSkiaOperations::ExtractSubset(resized_image, gfx::Rect(clamed_size));

    if (const std::optional<size_t> corner_radius = notification_view_util::GetLargeImageCornerRadius()) {
        // Return the cropped image decorated with rounded corners if necessary.
        return gfx::ImageSkiaOperations::CreateImageWithRoundRectClip(*corner_radius, cropped_image);
    }
