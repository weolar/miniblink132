...
MessagePopupCollection::PopupItem& MessagePopupCollection::PopupItem::operator=(PopupItem&& other) = default;
MessagePopupCollection::PopupItem::~PopupItem() = default;
...
void MessagePopupCollection::Update()
{
    ...
    if (state_ != State::kIdle) {
        ...
        animation_->SetDuration(animation_duration * ui::ScopedAnimationDurationScaleMode::duration_multiplier());
        animation_->Start();
        AnimationStarted();
        UpdateByAnimation();
    }
...
void MessagePopupCollection::AnimateResize()
{
    ...
    for (auto& popup : popup_items_) {
        auto target_bounds = gfx::Rect(popup.popup->GetWidget()->GetLayer()->bounds().x(), popup.bounds.y(), popup.bounds.width(), popup.bounds.height());
        animation_builder.Once()
            .SetDuration(base::Milliseconds(kNotificationResizeAnimationDurationMs))
            .SetBounds(popup.popup->GetWidget()->GetLayer(), target_bounds, gfx::Tween::EASE_OUT);
    }
}

MessageView* MessagePopupCollection::GetMessageViewForNotificationId(const std::string& notification_id)
{
...
void MessagePopupCollection::ConvertNotificationViewToGroupedNotificationView(
    const std::string& ungrouped_notification_id, const std::string& new_grouped_notification_id)
{
    auto it = base::ranges::find(popup_items_, ungrouped_notification_id, &PopupItem::id);
    if (it == popup_items_.end())
        return;
...
void MessagePopupCollection::ConvertGroupedNotificationViewToNotificationView(
    const std::string& grouped_notification_id, const std::string& new_single_notification_id)
{
    auto it = base::ranges::find(popup_items_, grouped_notification_id, &PopupItem::id);
    if (it == popup_items_.end())
        return;
...
void MessagePopupCollection::OnChildNotificationViewUpdated(const std::string& parent_notification_id, const std::string& child_notification_id)
{
    auto* notification = MessageCenter::Get()->FindNotificationById(child_notification_id);
    if (!notification) {
        return;
    }
    ...
    if (parent_popup) {
        parent_popup->UpdateContentsForChildNotification(child_notification_id, *notification);
    }
}

void MessagePopupCollection::OnNotificationAdded(const std::string& notification_id)
{
    ...
    const Notification* notification = message_center::MessageCenter::Get()->FindNotificationById(notification_id);
    if (notification && notification->priority() < DEFAULT_PRIORITY) {
        NotifySilentNotification(notification->id());
    }
}

void MessagePopupCollection::OnNotificationRemoved(const std::string& notification_id, bool by_user)
{
    Update();
}

void MessagePopupCollection::OnNotificationUpdated(const std::string& notification_id)
{
    if (is_updating_)
        return;
...
void MessagePopupCollection::OnBlockingStateChanged(NotificationBlocker* blocker)
{
    Update();
}
...
void MessagePopupCollection::AnimationProgressed(const gfx::Animation* animation)
{
    Update();
}

void MessagePopupCollection::AnimationCanceled(const gfx::Animation* animation)
{
    Update();
}

MessagePopupView* MessagePopupCollection::GetPopupViewForNotificationID(const std::string& notification_id)
{
    for (const auto& item : popup_items_) {
        if (item.id == notification_id)
            return item.popup;
...
MessagePopupView* MessagePopupCollection::CreatePopup(const Notification& notification)
{
    bool a11_feedback_on_init = notification.rich_notification_data().should_make_spoken_feedback_for_popup_updates;
    return new MessagePopupView(new NotificationView(notification), this, a11_feedback_on_init);
}

bool MessagePopupCollection::IsNextEdgeOutsideWorkArea(const PopupItem& item) const
{
    const int next_edge = GetNextEdge(item);

    const gfx::Rect work_area = GetWorkArea();
    return IsTopDown() ? next_edge > work_area.bottom() : next_edge < work_area.y();
}
...
void MessagePopupCollection::CloseAllPopupsNow()
{
    for (auto& item : popup_items_) {
        ...
        MessageCenter::Get()->MarkSinglePopupAsShown(item.id, /*mark_notification_as_read=*/true);
    }
...
void MessagePopupCollection::TransitionFromAnimation()
{
    ...
    if (state_ == State::kFadeIn || state_ == State::kMoveDown || (state_ == State::kFadeOut && popup_items_.empty())) {
...
void MessagePopupCollection::TransitionToAnimation()
{
    ...
    if (resize_requested_) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&MessagePopupCollection::ClosePopupsOutsideWorkArea, weak_ptr_factory_.GetWeakPtr()));
        return;
    }
}
...
void MessagePopupCollection::CalculateAndUpdateBounds()
{
    ...
    for (size_t i = 0; i < popup_items_.size(); ++i) {
        gfx::Size preferred_size(notification_width, GetPopupItem(i)->popup->GetHeightForWidth(notification_width));
        ...
        GetPopupItem(i)->bounds = gfx::Rect(gfx::Point(origin_x, origin_y), preferred_size);
    ...
    popup_collection_bounds_ = gfx::Rect(popup_bounds_origin_x, popup_bounds_origin_y, notification_width, popup_bounds_height - kMarginBetweenPopups);
...
void MessagePopupCollection::UpdateByAnimation()
{
    ...
    for (auto& item : popup_items_) {
        ...
        double value = gfx::Tween::CalculateValue(state_ == State::kFadeOut ? gfx::Tween::EASE_IN : gfx::Tween::EASE_OUT, animation_->GetCurrentValue());
        ...
        if (state_ == State::kFadeIn || state_ == State::kMoveDown) {
            item.popup->SetPopupBounds(gfx::Tween::RectValueBetween(value, item.start_bounds, item.bounds));
        }
    }
}

std::vector<Notification*> MessagePopupCollection::GetPopupNotifications() const
{
    ...
    for (auto* notification : MessageCenter::Get()->GetPopupNotifications()) {
        ...
        if (!IsPrimaryDisplayForNotification() && notification->type() == NOTIFICATION_TYPE_CUSTOM) {
            continue;
        }
...
bool MessagePopupCollection::AddPopup()
{
    ...
    MessageCenter::Get()->DisplayedNotification(new_notification->id(), DISPLAY_SOURCE_POPUP);
    ...
    if (!popup_items_.empty()) {
        ...
        item.start_bounds += gfx::Vector2d((IsFromLeft() ? -1 : 1) * item.bounds.width(), 0);
    }
...
{
    const int delta = item.popup->GetHeightForWidth(GetNotificationWidth()) + kMarginBetweenPopups;

    int base = 0;
    if (popup_items_.empty()) {
        base = GetBaseline();
    } else {
        base = IsTopDown() ? popup_items_.back().bounds.bottom() : popup_items_.back().bounds.y();
    }
...
void MessagePopupCollection::CloseAndRemovePopupFromPopupItem(MessagePopupView* popup, bool remove_only)
{
    for (auto& item : popup_items_) {
...
bool MessagePopupCollection::HasAddedPopup() const
{
    ...
    for (Notification* notification : GetPopupNotifications()) {
        if (!existing_ids.count(notification->id())) {
            ...
            if (notification->group_child()) {
                auto* parent_notification = MessageCenter::Get()->FindParentNotification(notification);
...
MessagePopupCollection::PopupItem* MessagePopupCollection::GetPopupItem(size_t index_from_top)
{
