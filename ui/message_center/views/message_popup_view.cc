...
MessagePopupView::MessagePopupView(MessageView* message_view, MessagePopupCollection* popup_collection, bool a11y_feedback_on_init)
    ...
    , a11y_feedback_on_init_(a11y_feedback_on_init)
{
    ...
    GetViewAccessibility().SetRoleDescription(message_view_->GetViewAccessibility().GetRoleDescription());
    UpdateAccessibleName(message_view_->GetViewAccessibility().GetCachedName());
    message_view_->SetUpdatedNameCallback(base::BindRepeating(&MessagePopupView::OnMessageViewNameUpdated, weak_ptr_factory_.GetWeakPtr()));
}
...
void MessagePopupView::UpdateAccessibleName(const std::u16string& new_name) const
{
    if (new_name.empty()) {
...
void MessagePopupView::OnMessageViewNameUpdated(bool should_make_spoken_feedback_for_popup_updates)
{
    const std::u16string old_name = GetViewAccessibility().GetCachedName();
    const std::u16string new_name = message_view_->GetViewAccessibility().GetCachedName();
...
void MessagePopupView::UpdateContentsForChildNotification(const std::string& notification_id, const Notification& notification)
{
    if (!IsWidgetValid()) {
        return;
    }

    auto* child_notification_view = static_cast<MessageView*>(message_view_->FindGroupNotificationView(notification_id));
...
{
    if (!IsWidgetValid() || is_hovered_ || message_view_->IsManuallyExpandedOrCollapsed()) {
        return;
    }
...
std::unique_ptr<views::Widget> MessagePopupView::Show()
{
    ...
    popup_collection_->ConfigureWidgetInitParamsForContainer(widget.get(), &params);
...
void MessagePopupView::OnWorkAreaChanged()
{
    ...
    if (popup_collection_->RecomputeAlignment(display::Screen::GetScreen()->GetDisplayNearestView(native_view))) {
