...
void InstallHighlightPathGenerator(views::View* view, float top_radius, float bottom_radius)
{
    auto corners = gfx::RoundedCornersF { top_radius, top_radius, bottom_radius, bottom_radius };
    ...
    views::HighlightPathGenerator::Install(
        view, std::make_unique<views::RoundRectHighlightPathGenerator>(gfx::Insets(-views::FocusRing::kDefaultHaloInset), corners));
}
    ...
    , slide_out_controller_(this, this)
{
    ...
    GetViewAccessibility().SetRoleDescription(l10n_util::GetStringUTF8(IDS_MESSAGE_NOTIFICATION_ACCESSIBLE_NAME));
    ...
    if (ShouldShowAeroShadowBorder()) {
        ...
        SetBorder(views::CreateBorderPainter(
            views::Painter::CreateImagePainter(shadow.nine_patch_image, ninebox_insets), -gfx::ShadowValue::GetMargin(shadow.values)));
    }
}
...
views::View* MessageView::FindGroupNotificationView(const std::string& notification_id)
{
...
// notification.
std::u16string MessageView::CreateAccessibleName(const Notification& notification)
{
...
void MessageView::UpdateWithNotification(const Notification& notification)
{
    ...
    if (updated_name_callback_) {
        updated_name_callback_.Run(notification.rich_notification_data().should_make_spoken_feedback_for_popup_updates);
    }
...
ui::Layer* MessageView::GetSlideOutLayer()
{
    ...
    // collapsed.
    auto* nested_layer = (ShouldParentHandleSlide() && parent_message_view_) ? parent_message_view_->layer() : layer();
    bool is_nested = (ShouldParentHandleSlide() && parent_message_view_) ? parent_message_view_->is_nested() : is_nested_;
...
void MessageView::OnSlideChanged(bool in_progress)
{
    ...
    if (scroller_ && !is_sliding_ && slide_out_controller_.GetGestureAmount()) {
        is_sliding_ = true;
        scroller_->SetVerticalScrollBarMode(views::ScrollView::ScrollBarMode::kDisabled);
    }

    if (scroller_ && !in_progress) {
        is_sliding_ = false;
        scroller_->SetVerticalScrollBarMode(views::ScrollView::ScrollBarMode::kEnabled);
    }
...
void MessageView::OnDidChangeFocus(views::View* before, views::View* now)
{
    if (Contains(before) || Contains(now)
        || (GetControlButtonsView() && (GetControlButtonsView()->Contains(before) || GetControlButtonsView()->Contains(now)))) {
        UpdateControlButtonsVisibility();
    }
}
...
{
    if (slide_out_controller_.mode() == views::SlideOutController::SlideMode::kNone) {
...
#if BUILDFLAG(IS_CHROMEOS)
base::TimeDelta MessageView::GetBoundsAnimationDuration(const Notification& notification) const
{
...
bool MessageView::ShouldShowControlButtons() const
{
    ...
    return control_buttons_view
        && (control_buttons_view->IsAnyButtonFocused() || (GetMode() != Mode::SETTING && IsMouseHovered()) || MessageCenter::Get()->IsSpokenFeedbackEnabled());
#else
    return true;
#endif
}
...
void MessageView::UpdateBackgroundPainter()
{
    ...
    SkColor background_color = color_provider->GetColor(ui::kColorNotificationBackgroundActive);

    SetBackground(views::CreateBackgroundFromPainter(std::make_unique<NotificationBackgroundPainter>(top_radius_, bottom_radius_, background_color)));
}
...
#else
    border_color = GetColorProvider()->GetColor(ui::kColorFocusableBorderUnfocused);
#endif

    SetBorder(views::CreateRoundedRectBorder(kNotificationBorderThickness, kNotificationCornerRadius, border_color));
}
...
void MessageView::UpdateControlButtonsVisibilityWithNotification(const Notification& notification)
{
    auto* control_buttons_view = GetControlButtonsView();
    if (control_buttons_view) {
        control_buttons_view->ShowButtons(ShouldShowControlButtons());
        control_buttons_view->ShowSettingsButton(notification.should_show_settings_button());
        control_buttons_view->ShowSnoozeButton(notification.should_show_snooze_button());
