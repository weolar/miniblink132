...
class MESSAGE_CENTER_EXPORT MessagePopupView : public views::FocusChangeListener, public views::WidgetDelegateView {
    METADATA_HEADER(MessagePopupView, views::WidgetDelegateView)

public:
    MessagePopupView(MessageView* message_view, MessagePopupCollection* popup_collection, bool a11y_feedback_on_init);
    ...
    virtual void UpdateContentsForChildNotification(const std::string& notification_id, const Notification& notification);
    ...
    bool view_added_to_widget()
    {
        return view_added_to_widget_;
    }
...
private:
    ...
    void OnMessageViewNameUpdated(bool should_make_spoken_feedback_for_popup_updates);
