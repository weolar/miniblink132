...
std::unique_ptr<views::View> CreateItemView(const NotificationItem& item)
{
    ...
    view->SetLayoutManager(std::make_unique<views::BoxLayout>(views::BoxLayout::Orientation::kHorizontal, gfx::Insets(), 0));

    auto* title = view->AddChildView(std::make_unique<views::Label>(item.title(), views::style::CONTEXT_DIALOG_BODY_TEXT));
    title->SetCollapseWhenHidden(true);
    title->SetHorizontalAlignment(gfx::ALIGN_LEFT);

    auto* message = view->AddChildView(
        std::make_unique<views::Label>(l10n_util::GetStringFUTF16(IDS_MESSAGE_CENTER_LIST_NOTIFICATION_MESSAGE_WITH_DIVIDER, item.message()),
...
{
    title_ = AddChildView(std::make_unique<views::Label>(std::u16string(), views::style::CONTEXT_DIALOG_BODY_TEXT));
    title_->SetHorizontalAlignment(gfx::ALIGN_LEFT);

    message_ = AddChildView(std::make_unique<views::Label>(std::u16string(), views::style::CONTEXT_DIALOG_BODY_TEXT, views::style::STYLE_SECONDARY));
    message_->SetHorizontalAlignment(gfx::ALIGN_RIGHT);
    SetLayoutManager(std::make_unique<views::DelegatingLayoutManager>(this));
}

gfx::Size CompactTitleMessageView::CalculatePreferredSize(const views::SizeBounds& /*available_size*/) const
{
    gfx::Size title_size = title_->GetPreferredSize();
    gfx::Size message_size = message_->GetPreferredSize();
    return gfx::Size(title_size.width() + message_size.width() + kCompactTitleMessageViewSpacing, std::max(title_size.height(), message_size.height()));
}

views::ProposedLayout CompactTitleMessageView::CalculateProposedLayout(const views::SizeBounds& size_bounds) const
{
    ...
    //   right-aligned.
    const int message_width = std::min(message_->GetPreferredSize().width(),
        title_->GetPreferredSize().width() > 0 ? static_cast<int>(kProgressNotificationMessageRatio * layout.host_size.width()) : layout.host_size.width());
    const int title_width = std::max(0, layout.host_size.width() - message_width - kCompactTitleMessageViewSpacing);
    layout.child_layouts.emplace_back(title_.get(), title_->GetVisible(), gfx::Rect(0, 0, title_width, layout.host_size.height()));
    layout.child_layouts.emplace_back(
        message_.get(), message_->GetVisible(), gfx::Rect(layout.host_size.width() - message_width, 0, message_width, layout.host_size.height()));
    return layout;
}
...
void NotificationViewBase::CreateOrUpdateViews(const Notification& notification)
{
...
NotificationViewBase::NotificationViewBase(const Notification& notification)
    ...
    , for_ash_notification_(IsForAshNotification())
{
    UpdateCornerRadius(kNotificationCornerRadius, kNotificationCornerRadius);
    SetProperty(views::kElementIdentifierKey, notification.host_view_element_id());
}
...
void NotificationViewBase::OnMouseReleased(const ui::MouseEvent& event)
{
    ...
    if (ui::EventTimeStampToSeconds(event.time_stamp()) - ui::EventTimeStampToSeconds(last_mouse_pressed_timestamp_)
        > ui::GetGestureProviderConfig(ui::GestureProviderConfigType::CURRENT_PLATFORM).gesture_detector_config.longpress_timeout.InSecondsF()) {
...
void NotificationViewBase::UpdateWithNotification(const Notification& notification)
{
...
void NotificationViewBase::OnNotificationInputSubmit(size_t index, const std::u16string& text)
{
    MessageCenter::Get()->ClickOnNotificationButtonWithReply(notification_id(), index, text);
}
...
views::Builder<NotificationControlButtonsView> NotificationViewBase::CreateControlButtonsBuilder()
{
    DCHECK(!control_buttons_view_);
    return views::Builder<NotificationControlButtonsView>().CopyAddressTo(&control_buttons_view_).SetMessageView(this);
}

views::Builder<NotificationHeaderView> NotificationViewBase::CreateHeaderRowBuilder()
{
    DCHECK(!header_row_);
    auto header_row_builder = views::Builder<NotificationHeaderView>().SetID(kHeaderRow).CopyAddressTo(&header_row_);
    return header_row_builder;
}

views::Builder<views::BoxLayoutView> NotificationViewBase::CreateLeftContentBuilder()
{
...
views::Builder<views::View> NotificationViewBase::CreateRightContentBuilder()
{
    ...
    return views::Builder<views::View>().CopyAddressTo(&right_content_).SetUseDefaultFillLayout(true);
}

views::Builder<views::View> NotificationViewBase::CreateContentRowBuilder()
{
    DCHECK(!content_row_);
    return views::Builder<views::View>().SetID(kContentRow).CopyAddressTo(&content_row_);
}

views::Builder<views::BoxLayoutView> NotificationViewBase::CreateInlineSettingsBuilder()
{
    DCHECK(!settings_row_);
    return views::Builder<views::BoxLayoutView>().SetID(ViewId::kInlineSettingsRow).CopyAddressTo(&settings_row_).SetVisible(false);
}

views::Builder<views::BoxLayoutView> NotificationViewBase::CreateSnoozeSettingsBuilder()
{
    CHECK(!snooze_row_);
    return views::Builder<views::BoxLayoutView>().CopyAddressTo(&snooze_row_).SetVisible(false);
}

views::Builder<views::View> NotificationViewBase::CreateImageContainerBuilder()
{
    DCHECK(!image_container_view_);
    return views::Builder<views::View>().SetID(ViewId::kImageContainerView).CopyAddressTo(&image_container_view_).SetUseDefaultFillLayout(true);
}

std::unique_ptr<views::View> NotificationViewBase::CreateActionsRow(std::unique_ptr<views::LayoutManager> layout_manager)
{
    DCHECK(!actions_row_);
    ...
    action_buttons_row->SetLayoutManager(
        std::make_unique<views::BoxLayout>(views::BoxLayout::Orientation::kHorizontal, kActionsRowPadding, kActionsRowHorizontalSpacing));
    action_buttons_row->SetVisible(false);
    action_buttons_row->SetID(kActionButtonsRow);
    action_buttons_row_ = actions_row->AddChildView(std::move(action_buttons_row));
...
// static
std::unique_ptr<views::Label> NotificationViewBase::GenerateTitleView(const std::u16string& title)
{
    auto title_view = std::make_unique<views::Label>(title, views::style::CONTEXT_DIALOG_BODY_TEXT);
    title_view->SetHorizontalAlignment(gfx::ALIGN_TO_HEAD);
    title_view->SetAllowCharacterBreak(true);
    return title_view;
}

std::unique_ptr<NotificationInputContainer> NotificationViewBase::GenerateNotificationInputContainer()
{
    return std::make_unique<NotificationInputContainer>(this);
}

void NotificationViewBase::CreateOrUpdateHeaderView(const Notification& notification)
{
    ...
    if (notification.notifier_id().title.has_value()) {
        ...
        app_name = url_formatter::FormatUrlForSecurityDisplay(notification.origin_url(), url_formatter::SchemeDisplay::OMIT_HTTP_AND_HTTPS);
        header_row_->SetAppNameElideBehavior(gfx::ELIDE_HEAD);
    } else if (notification.display_source().empty() && notification.notifier_id().type == NotifierType::SYSTEM_COMPONENT) {
...
void NotificationViewBase::CreateOrUpdateCompactTitleMessageView(const Notification& notification)
{
    if (notification.type() != NOTIFICATION_TYPE_PROGRESS) {
        DCHECK(!compact_title_message_view_ || left_content_->Contains(compact_title_message_view_));
    ...
    if (!compact_title_message_view_) {
        auto compact_title_message_view = std::make_unique<CompactTitleMessageView>();
        compact_title_message_view_ = AddViewToLeftContent(std::move(compact_title_message_view));
    } else {
...
void NotificationViewBase::CreateOrUpdateProgressBarView(const Notification& notification)
{
    if (notification.type() != NOTIFICATION_TYPE_PROGRESS) {
    ...
    if (!progress_bar_view_) {
        ...
        progress_bar_view->SetBorder(views::CreateEmptyBorder(gfx::Insets::TLBR(kProgressBarTopPadding, 0, 0, 0)));
...
void NotificationViewBase::CreateOrUpdateProgressStatusView(const Notification& notification)
{
    if (notification.type() != NOTIFICATION_TYPE_PROGRESS || notification.progress_status().empty()) {
        if (!status_view_)
            return;
    ...
    if (!status_view_) {
        auto status_view = std::make_unique<views::Label>(std::u16string(), views::style::CONTEXT_DIALOG_BODY_TEXT, views::style::STYLE_SECONDARY);
...
void NotificationViewBase::CreateOrUpdateMessageLabel(const Notification& notification)
{
    if (notification.type() == NOTIFICATION_TYPE_PROGRESS || notification.message().empty()) {
    ...
    const std::u16string text = gfx::TruncateString(notification.message(), GetMessageCharacterLimit(), gfx::WORD_BREAK);

    if (!message_label_) {
        auto message_label = std::make_unique<views::Label>(text, views::style::CONTEXT_DIALOG_BODY_TEXT, views::style::STYLE_SECONDARY);
...
void NotificationViewBase::CreateOrUpdateProgressViews(const Notification& notification)
{
...
void NotificationViewBase::CreateOrUpdateListItemViews(const Notification& notification)
{
    ...
    for (size_t i = 0; i < items.size() && i < kMaxLinesForExpandedMessageLabel; ++i) {
...
void NotificationViewBase::CreateOrUpdateIconView(const Notification& notification)
{
    const bool use_image_for_icon = notification.icon().IsEmpty();

    ui::ImageModel icon = use_image_for_icon ? ui::ImageModel::FromImage(notification.image()) : notification.icon();

    if (notification.type() == NOTIFICATION_TYPE_PROGRESS || notification.type() == NOTIFICATION_TYPE_MULTIPLE || icon.IsEmpty()) {
    ...
    if (!icon_view_) {
        icon_view_ = right_content_->AddChildView(std::make_unique<ProportionalImageView>(GetIconViewSize()));
...
void NotificationViewBase::CreateOrUpdateImageView(const Notification& notification)
{
    ...
    if (image_container_view_->children().empty()) {
        image_container_view_->AddChildView(std::make_unique<LargeImageView>(gfx::Size(GetLargeImageViewMaxWidth(), kLargeImageMaxHeight)));
        image_container_view_->SetVisible(true);
    }

    static_cast<LargeImageView*>(image_container_view_->children().front())->SetImage(notification.image().AsImageSkia());
}

void NotificationViewBase::CreateOrUpdateActionButtonViews(const Notification& notification)
{
    ...
    if (new_buttons || buttons.empty()) {
        ...
        actions_row_->SetVisible(expanded_ && (!buttons.empty() || (for_ash_notification_ && notification.should_show_snooze_button())));
    }

    // Hide inline reply field if it doesn't exist anymore.
    if (inline_reply_ && inline_reply_->GetVisible() && !HasInlineReply(notification)) {
    ...
    for (size_t i = 0; i < buttons.size(); ++i) {
        ...
        std::u16string label = for_ash_notification_ ? button_info.title : base::i18n::ToUpper(button_info.title);
        if (new_buttons) {
            action_buttons_.push_back(action_buttons_row_->AddChildView(
                GenerateNotificationLabelButton(base::BindRepeating(&NotificationViewBase::ActionButtonPressed, base::Unretained(this), i), label)));
            action_button_to_placeholder_map_[action_buttons_.back()] = button_info.placeholder;
            ...
            action_button_to_placeholder_map_[action_buttons_[i]] = button_info.placeholder;
        }

        bool use_accent_color = !for_ash_notification_ && !notification.rich_notification_data().ignore_accent_color_for_text;
        if (use_accent_color) {
...
void NotificationViewBase::ActionButtonPressed(size_t index, const ui::Event& event)
{
    const std::optional<std::u16string>& placeholder = action_button_to_placeholder_map_[action_buttons_[index]];
    ...
    MessageCenter::Get()->ClickOnNotificationButton(notification_id(), static_cast<int>(index));
...
bool NotificationViewBase::HasInlineReply(const Notification& notification) const
{
    if (!inline_reply_)
        return false;
...
void NotificationViewBase::UpdateViewForExpandedState(bool expanded)
{
    ...
    actions_row_->SetVisible(expanded && !action_buttons_row_->children().empty());
    if (!expanded) {
        action_buttons_row_->SetVisible(true);
        if (inline_reply_)
            inline_reply_->SetVisible(false);
...
NotificationControlButtonsView* NotificationViewBase::GetControlButtonsView() const
{
...
{
    return MessageCenter::Get()->GetNotificationExpandState(notification_id()) != ExpandState::DEFAULT;
}
...
void NotificationViewBase::InkDropRippleAnimationEnded(views::InkDropState ink_drop_state)
{
