...
public:
    virtual void OnNotificationInputSubmit(size_t index, const std::u16string& text) = 0;
    virtual ~NotificationInputDelegate() = default;
};

// A view which shows a textfield with a send button for notifications.
class MESSAGE_CENTER_EXPORT NotificationInputContainer : public views::View, public views::TextfieldController, public views::LayoutDelegate {
    METADATA_HEADER(NotificationInputContainer, views::View)

public:
    explicit NotificationInputContainer(NotificationInputDelegate* delegate = nullptr);
    NotificationInputContainer(const NotificationInputContainer&) = delete;
    NotificationInputContainer& operator=(const NotificationInputContainer&) = delete;
    ...
    views::ImageButton* button() const
    {
        return button_;
    }
