...
void ConfigureLabel(views::Label* label, const gfx::FontList& font_list, const gfx::Insets& text_view_padding, bool auto_color_readability)
{
...
ExpandButton::ExpandButton()
{
    ...
    if (GetTooltipText().empty()) {
        GetViewAccessibility().SetName(GetTooltipText(), ax::mojom::NameFrom::kAttributeExplicitlyEmpty);
    }
}
...
void ExpandButton::OnPaint(gfx::Canvas* canvas)
{
    ...
    if (HasFocus())
        views::Painter::PaintPainterAt(canvas, focus_painter_.get(), GetContentsBounds());
}
...
void ExpandButton::OnThemeChanged()
{
    ...
    focus_painter_ = views::Painter::CreateSolidFocusPainter(GetColorProvider()->GetColor(ui::kColorFocusableBorderFocused), gfx::Insets::TLBR(0, 0, 1, 1));
}
    ...
    : views::Button(std::move(callback))
{
    const views::FlexSpecification kAppNameFlex
        = views::FlexSpecification(views::MinimumFlexSizeRule::kScaleToZero, views::MaximumFlexSizeRule::kPreferred).WithOrder(1);

    const views::FlexSpecification kSpacerFlex
        = views::FlexSpecification(views::MinimumFlexSizeRule::kScaleToMinimum, views::MaximumFlexSizeRule::kUnbounded).WithOrder(2);
    ...
    auto* detail_layout = detail_views_->SetLayoutManager(std::make_unique<views::FlexLayout>());
    ...
    summary_text_divider_ = detail_views_->AddChildView(std::move(summary_text_divider));
    ...
    summary_text_view_ = detail_views_->AddChildView(std::move(summary_text_view));
    ...
    timestamp_divider_ = detail_views_->AddChildView(std::move(timestamp_divider));
    ...
    spacer->SetPreferredSize(gfx::Size(kControlButtonSpacing, kInnerHeaderHeight));
    ...
    if (app_name_view_->GetText().empty()) {
        GetViewAccessibility().SetName(app_name_view_->GetText(), ax::mojom::NameFrom::kAttributeExplicitlyEmpty);
    } else {
    ...
    summary_text_changed_callback_
        = summary_text_view_->AddTextChangedCallback(base::BindRepeating(&NotificationHeaderView::OnTextChanged, base::Unretained(this)));
    timestamp_changed_callback_ = timestamp_view_->AddTextChangedCallback(base::BindRepeating(&NotificationHeaderView::OnTextChanged, base::Unretained(this)));
}

NotificationHeaderView::~NotificationHeaderView() = default;

void NotificationHeaderView::ConfigureLabelsStyle(const gfx::FontList& font_list, const gfx::Insets& text_view_padding, bool auto_color_readability)
{
    ConfigureLabel(app_name_view_, font_list, text_view_padding, auto_color_readability);
    ConfigureLabel(summary_text_view_, font_list, text_view_padding, auto_color_readability);
    ConfigureLabel(summary_text_divider_, font_list, text_view_padding, auto_color_readability);
    ConfigureLabel(timestamp_divider_, font_list, text_view_padding, auto_color_readability);
    ConfigureLabel(timestamp_view_, font_list, text_view_padding, auto_color_readability);
}
...
void NotificationHeaderView::SetAppName(const std::u16string& name)
{
    ...
    if (name.empty()) {
        GetViewAccessibility().SetName(name, ax::mojom::NameFrom::kAttributeExplicitlyEmpty);
    } else {
        GetViewAccessibility().SetName(name);
    }
}

void NotificationHeaderView::SetAppNameElideBehavior(gfx::ElideBehavior elide_behavior)
{
...
{
    summary_text_view_->SetText(l10n_util::GetStringFUTF16Int(IDS_MESSAGE_CENTER_NOTIFICATION_PROGRESS_PERCENTAGE, progress));
    has_progress_ = true;
...
{
    summary_text_view_->SetText(l10n_util::GetStringFUTF16Int(IDS_MESSAGE_CENTER_LIST_NOTIFICATION_HEADER_OVERFLOW_INDICATOR, count));
    has_progress_ = false;
    UpdateSummaryTextAndTimestampVisibility();
}
...
void NotificationHeaderView::SetTimestamp(base::Time timestamp)
{
    ...
    GetRelativeTimeStringAndNextUpdateTime(timestamp - base::Time::Now(), &relative_time, &next_update);
    ...
    timestamp_update_timer_.Start(FROM_HERE, next_update, base::BindOnce(&NotificationHeaderView::SetTimestamp, base::Unretained(this), timestamp));
}
...
void NotificationHeaderView::SetExpanded(bool expanded)
{
    ...
    UpdateColors();
    expand_button_->SetTooltipText(l10n_util::GetStringUTF16(expanded ? IDS_MESSAGE_CENTER_COLLAPSE_NOTIFICATION : IDS_MESSAGE_CENTER_EXPAND_NOTIFICATION));
...
void NotificationHeaderView::SetIsInAshNotificationView(bool is_in_ash_notification)
{
    ...
    spacer_->SetPreferredSize(gfx::Size(kControlButtonSpacing, kHeaderHeightInAsh - kHeaderOuterPadding.height()));
    SetPreferredSize(gfx::Size(GetNotificationWidth(), kHeaderHeightInAsh));
}

void NotificationHeaderView::SetIsInGroupChildNotification(bool is_in_group_child_notification)
{
    ...
    app_icon_view_->SetVisible(!is_in_ash_notification_ && !is_in_group_child_notification_);
    expand_button_->SetVisible(!is_in_ash_notification_ && !is_in_group_child_notification_);
    UpdateSummaryTextAndTimestampVisibility();
}
...
{
    const bool summary_visible = !is_in_group_child_notification_ && !summary_text_view_->GetText().empty();
...
void NotificationHeaderView::UpdateColors()
{
    ...
    SkColor color = color_.value_or(GetColorProvider()->GetColor(ui::kColorNotificationHeaderForeground));
    ...
    if (expand_button_) {
        expand_button_->SetImage(
            ui::ImageModel::FromVectorIcon(is_expanded_ ? kNotificationExpandLessIcon : kNotificationExpandMoreIcon, actual_color, kExpandIconSize));
    }

    if (using_default_app_icon_ && app_icon_view_) {
        app_icon_view_->SetImage(ui::ImageModel::FromVectorIcon(kProductIcon, actual_color, kSmallImageSizeMD));
    }
}
...
{
    GetViewAccessibility().SetDescription(summary_text_view_->GetText() + u" " + timestamp_view_->GetText());
}
