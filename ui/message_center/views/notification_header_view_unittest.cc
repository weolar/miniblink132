...
public:
    ...
    NotificationHeaderViewTest& operator=(const NotificationHeaderViewTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        views::Widget::InitParams params = CreateParams(views::Widget::InitParams::TYPE_WINDOW_FRAMELESS);
        params.bounds = gfx::Rect(200, 200);
        params.ownership = views::Widget::InitParams::WIDGET_OWNS_NATIVE_WIDGET;
        widget_.Init(std::move(params));
        views::View* container = widget_.SetContentsView(std::make_unique<views::View>());

        notification_header_view_ = new NotificationHeaderView(views::Button::PressedCallback());
    ...
    {
        SkBitmap expected = *gfx::CreateVectorIcon(kProductIcon, kSmallImageSizeMD, color).bitmap();
        SkBitmap actual = *notification_header_view_->app_icon_for_testing().bitmap();
    ...
    bool MatchesExpandIconColor(SkColor color)
    {
        ...
        SkBitmap expected = *gfx::CreateVectorIcon(kNotificationExpandMoreIcon, kExpandIconSize, color).bitmap();
        SkBitmap actual = *notification_header_view_->expand_button()->GetImage().bitmap();
        return gfx::test::AreBitmapsEqual(expected, actual);
    }

protected:
    raw_ptr<NotificationHeaderView, DanglingUntriaged> notification_header_view_ = nullptr;
...
{
    auto* timestamp_view = notification_header_view_->timestamp_view_for_testing();

    notification_header_view_->SetTimestamp(base::Time::Now() + base::Hours(3) + base::Minutes(30));
    EXPECT_EQ(l10n_util::GetPluralStringFUTF16(IDS_MESSAGE_NOTIFICATION_DURATION_HOURS_SHORTEST_FUTURE, 3), timestamp_view->GetText());

    task_environment()->FastForwardBy(base::Hours(3));
    task_environment()->RunUntilIdle();

    EXPECT_EQ(l10n_util::GetPluralStringFUTF16(IDS_MESSAGE_NOTIFICATION_DURATION_MINUTES_SHORTEST_FUTURE, 30), timestamp_view->GetText());

    task_environment()->FastForwardBy(base::Minutes(30));
    task_environment()->RunUntilIdle();

    EXPECT_EQ(l10n_util::GetStringUTF16(IDS_MESSAGE_NOTIFICATION_NOW_STRING_SHORTEST), timestamp_view->GetText());

    task_environment()->FastForwardBy(base::Days(2));
    task_environment()->RunUntilIdle();

    EXPECT_EQ(l10n_util::GetPluralStringFUTF16(IDS_MESSAGE_NOTIFICATION_DURATION_DAYS_SHORTEST, 2), timestamp_view->GetText());
}

TEST_F(NotificationHeaderViewTest, AllowsHidingOfAppIcon)
{
    // The icon should be shown by default...
    EXPECT_TRUE(notification_header_view_->app_icon_view_for_testing()->IsDrawn());

    // ... though it can be explicitly hidden...
    notification_header_view_->SetAppIconVisible(false);
    EXPECT_FALSE(notification_header_view_->app_icon_view_for_testing()->IsDrawn());
...
{
    int progress = 50;
    std::u16string expected_summary_text = l10n_util::GetStringFUTF16Int(IDS_MESSAGE_CENTER_NOTIFICATION_PROGRESS_PERCENTAGE, progress);
...
{
    int count = 10;
    std::u16string expected_summary_text = l10n_util::GetStringFUTF16Int(IDS_MESSAGE_CENTER_LIST_NOTIFICATION_HEADER_OVERFLOW_INDICATOR, count);
...
TEST_F(NotificationHeaderViewTest, ColorContrastEnforcement)
{
    ...
    SkColor expected_color = color_utils::BlendForMinContrast(SK_ColorWHITE, SK_ColorWHITE).color;
    ...
    expected_color = color_utils::BlendForMinContrast(SK_ColorBLACK, SK_ColorBLACK).color;
    EXPECT_EQ(expected_color, summary_text->GetEnabledColor());
    EXPECT_TRUE(MatchesAppIconColor(expected_color));
    EXPECT_TRUE(MatchesExpandIconColor(expected_color));
}
...
{
    EXPECT_EQ(views::View::FocusBehavior::ACCESSIBLE_ONLY, notification_header_view_->GetFocusBehavior());
}
...
TEST_F(NotificationHeaderViewTest, AppIconAndExpandButtonNotVisible)
{
    ...
    // ash notification.
    auto notification_header_view = std::make_unique<NotificationHeaderView>(views::Button::PressedCallback());
    notification_header_view->SetIsInAshNotificationView(true);

    EXPECT_FALSE(notification_header_view->app_icon_view_for_testing()->GetVisible());
...
TEST_F(NotificationHeaderViewTest, GroupChildNotificationVisibility)
{
    ...
    EXPECT_TRUE(notification_header_view_->app_icon_view_for_testing()->GetVisible());
    EXPECT_TRUE(notification_header_view_->expand_button()->GetVisible());
    EXPECT_TRUE(notification_header_view_->summary_text_for_testing()->GetVisible());
    EXPECT_TRUE(notification_header_view_->summary_text_divider_->GetVisible());
    EXPECT_TRUE(notification_header_view_->timestamp_view_for_testing()->GetVisible());
    ...
    EXPECT_FALSE(notification_header_view_->app_icon_view_for_testing()->GetVisible());
    EXPECT_FALSE(notification_header_view_->expand_button()->GetVisible());
    EXPECT_FALSE(notification_header_view_->summary_text_for_testing()->GetVisible());
...
TEST_F(NotificationHeaderViewTest, AccessibleExpandAndCollapse)
{
    ...
    notification_header_view_->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_FALSE(data.HasState(ax::mojom::State::kExpanded));
    EXPECT_TRUE(data.HasState(ax::mojom::State::kCollapsed));
    EXPECT_EQ(ax_counter.GetCount(ax::mojom::Event::kExpandedChanged, notification_header_view_), 0);
    ...
    EXPECT_EQ(ax_counter.GetCount(ax::mojom::Event::kExpandedChanged, notification_header_view_), 1);
    ...
    bool expand_button_is_ignored = notification_header_view_->expand_button()->GetViewAccessibility().GetIsIgnored();
    EXPECT_EQ(ax_counter.GetCount(ax::mojom::Event::kChildrenChanged, notification_header_view_->expand_button()->parent()), expand_button_is_ignored ? 0 : 1);
    ...
    expand_button_is_ignored = notification_header_view_->expand_button()->GetViewAccessibility().GetIsIgnored();
    EXPECT_FALSE(expand_button_is_ignored);
    EXPECT_EQ(ax_counter.GetCount(ax::mojom::Event::kChildrenChanged, notification_header_view_->expand_button()->parent()), 1);
}
...
TEST_F(NotificationHeaderViewTest, AccessibleNameTest)
{
    ...
    notification_header_view_->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), u"Some app name");
}
...
TEST_F(NotificationHeaderViewTest, AccessibleRoleTest)
{
    ...
    data = ui::AXNodeData();
    notification_header_view_->expand_button()->GetViewAccessibility().GetAccessibleNodeData(&data);
...
{
    EXPECT_EQ(notification_header_view_->GetViewAccessibility().GetCachedDescription(), u" ");

    notification_header_view_->SetSummaryText(u"Example Summary");

    EXPECT_EQ(notification_header_view_->GetViewAccessibility().GetCachedDescription(), u"Example Summary ");

    auto* timestamp_view = notification_header_view_->timestamp_view_for_testing();

    notification_header_view_->SetTimestamp(base::Time::Now() + base::Hours(3) + base::Minutes(30));

    EXPECT_EQ(notification_header_view_->GetViewAccessibility().GetCachedDescription(), u"Example Summary " + timestamp_view->GetText());

    int progress = 1;
    notification_header_view_->SetProgress(progress);

    EXPECT_EQ(notification_header_view_->GetViewAccessibility().GetCachedDescription(),
        l10n_util::GetStringFUTF16Int(IDS_MESSAGE_CENTER_NOTIFICATION_PROGRESS_PERCENTAGE, progress) + u" " + timestamp_view->GetText());

    int count = 4;
    notification_header_view_->SetOverflowIndicator(count);

    EXPECT_EQ(notification_header_view_->GetViewAccessibility().GetCachedDescription(),
        l10n_util::GetStringFUTF16Int(IDS_MESSAGE_CENTER_LIST_NOTIFICATION_HEADER_OVERFLOW_INDICATOR, count) + u" " + timestamp_view->GetText());
}
