...
constexpr int kMessageLabelWidthWithIcon = kNotificationWidth - kIconViewSize.width() - kLeftContentPaddingWithIcon.left() - kLeftContentPaddingWithIcon.right()
    - kContentRowPadding.left() - kContentRowPadding.right();

constexpr int kMessageLabelWidth
    = kNotificationWidth - kLeftContentPadding.left() - kLeftContentPadding.right() - kContentRowPadding.left() - kContentRowPadding.right();
...
constexpr int kTitleCharacterLimit = kNotificationWidth * kMaxTitleLines / kMinPixelsPerTitleCharacter;
...
gfx::FontList GetHeaderTextFontList()
{
    ...
    const gfx::Font& font = default_font.Derive(font_size_delta, gfx::Font::NORMAL, gfx::Font::Weight::NORMAL);
...
public:
    ...
    void OnThemeChanged() override
    {
        ...
        label()->SetBackgroundColor(GetColorProvider()->GetColor(ui::kColorNotificationActionsBackground));
    }
    ...
    std::optional<SkColor> color() const
    {
        return color_;
    }
...
public:
    ...
    void OnThemeChanged() override
    {
        ...
        label()->SetAutoColorReadabilityEnabled(true);
        label()->SetBackgroundColor(GetColorProvider()->GetColor(ui::kColorNotificationBackgroundActive));
    }
...
public:
    NotificationInkDropImpl(views::InkDropHost* ink_drop_host, const gfx::Size& host_size)
        : views::InkDropImpl(ink_drop_host, host_size, views::InkDropImpl::AutoHighlightMode::SHOW_ON_RIPPLE)
    {
    }
...
class NotificationView::NotificationViewPathGenerator : public views::HighlightPathGenerator {
public:
    ...
    NotificationViewPathGenerator& operator=(const NotificationViewPathGenerator&) = delete;
    ...
    std::optional<gfx::RRectF> GetRoundRect(const gfx::RectF& rect) override
    {
        ...
        gfx::RoundedCornersF corner_radius(top_radius_, top_radius_, bottom_radius_, bottom_radius_);
        return gfx::RRectF(bounds, corner_radius);
    }

    void set_top_radius(int val)
    {
        top_radius_ = val;
    }
    void set_bottom_radius(int val)
    {
        bottom_radius_ = val;
    }
    void set_preferred_size(const gfx::Size& val)
    {
        preferred_size_ = val;
    }
...
NotificationView::NotificationView(const message_center::Notification& notification)
    : NotificationViewBase(notification)
    , ink_drop_container_(AddChildView(std::make_unique<views::InkDropContainerView>()))
{
    // Instantiate view instances and define layout and view hierarchy.
    SetLayoutManager(std::make_unique<views::BoxLayout>(views::BoxLayout::Orientation::kVertical, gfx::Insets(), 0));
    ...
    views::InkDrop::Get(this)->SetCreateInkDropCallback(base::BindRepeating(
        [](NotificationViewBase* host) -> std::unique_ptr<views::InkDrop> {
            auto ink_drop = std::make_unique<NotificationInkDropImpl>(views::InkDrop::Get(host), host->size());
    ...
    views::InkDrop::Get(this)->SetCreateRippleCallback(base::BindRepeating(
        [](NotificationViewBase* host) -> std::unique_ptr<views::InkDropRipple> {
            return std::make_unique<views::FloodFillInkDropRipple>(views::InkDrop::Get(host), host->GetPreferredSize({}),
                views::InkDrop::Get(host)->GetInkDropCenterBasedOnLastEvent(), views::InkDrop::Get(host)->GetBaseColor(),
                views::InkDrop::Get(host)->GetVisibleOpacity());
        },
        this));
    views::InkDrop::Get(this)->SetBaseColorId(ui::kColorNotificationBackgroundActive);
    ...
    header_row->SetPreferredSize(header_row->GetPreferredSize({}) - gfx::Size(GetInsets().width(), 0));
    header_row->SetCallback(base::BindRepeating(&NotificationView::HeaderRowPressed, base::Unretained(this)));
    header_row->AddChildView(CreateControlButtonsBuilder().SetNotificationControlButtonFactory(std::make_unique<NotificationControlButtonFactory>()).Build());

    auto content_row = CreateContentRowBuilder().SetLayoutManager(std::make_unique<views::BoxLayout>(views::BoxLayout::Orientation::kHorizontal)).Build();
    auto* content_row_layout = static_cast<views::BoxLayout*>(content_row->GetLayoutManager());
    content_row_layout->set_inside_border_insets(kContentRowPadding);
    content_row_layout->set_cross_axis_alignment(views::BoxLayout::CrossAxisAlignment::kStart);

    auto* left_content_ptr = content_row->AddChildView(CreateLeftContentBuilder().Build());
    static_cast<views::BoxLayout*>(content_row->GetLayoutManager())->SetFlexForView(left_content_ptr, 1);
    ...
    AddChildView(CreateImageContainerBuilder().SetBorder(views::CreateEmptyBorder(kLargeImageContainerPadding)).Build());
    ...
    auto highlight_path_generator = std::make_unique<NotificationViewPathGenerator>(GetInsets());
    highlight_path_generator_ = highlight_path_generator.get();
    views::HighlightPathGenerator::Install(this, std::move(highlight_path_generator));
}
...
SkColor NotificationView::GetActionButtonColorForTesting(views::LabelButton* action_button)
{
    NotificationTextButton* button = static_cast<NotificationTextButton*>(action_button);
    return button->color().value_or(SkColor());
}

void NotificationView::CreateOrUpdateHeaderView(const Notification& notification)
{
...
void NotificationView::CreateOrUpdateTitleView(const Notification& notification)
{
    if (notification.title().empty() || notification.type() == NOTIFICATION_TYPE_PROGRESS) {
        if (title_view_) {
    ...
    const std::u16string& title = gfx::TruncateString(notification.title(), kTitleCharacterLimit, gfx::WORD_BREAK);
    if (!title_view_) {
...
void NotificationView::CreateOrUpdateSmallIconView(const Notification& notification)
{
    ...
    SkColor accent_color = notification.accent_color().value_or(color_provider->GetColor(ui::kColorNotificationHeaderForeground));
    SkColor icon_color = color_utils::BlendForMinContrast(accent_color, GetNotificationHeaderViewBackgroundColor()).color;

    // TODO(crbug.com/40541732): figure out if this has a performance impact and
    // cache images if so.
    gfx::Image masked_small_icon = notification.GenerateMaskedSmallIcon(kSmallImageSizeMD, icon_color,
        color_provider->GetColor(ui::kColorNotificationIconBackground), color_provider->GetColor(ui::kColorNotificationIconForeground));
...
void NotificationView::CreateOrUpdateInlineSettingsViews(const Notification& notification)
{
    if (inline_settings_enabled()) {
        DCHECK_EQ(message_center::SettingsButtonHandler::INLINE, notification.rich_notification_data().settings_button_handler);
        return;
    }

    set_inline_settings_enabled(notification.rich_notification_data().settings_button_handler == message_center::SettingsButtonHandler::INLINE);
    ...
    case NotifierType::ARC_APPLICATION:
        block_notifications_message_id = IDS_MESSAGE_CENTER_BLOCK_ALL_NOTIFICATIONS_APP;
        break;
    case NotifierType::WEB_PAGE:
        block_notifications_message_id = IDS_MESSAGE_CENTER_BLOCK_ALL_NOTIFICATIONS_SITE;
        break;
    case NotifierType::SYSTEM_COMPONENT:
        block_notifications_message_id = IDS_MESSAGE_CENTER_BLOCK_ALL_NOTIFICATIONS;
        break;
    ...
    inline_settings_row()->SetOrientation(views::BoxLayout::Orientation::kVertical);
    inline_settings_row()->SetInsideBorderInsets(kSettingsRowPadding);

    auto block_all_button = std::make_unique<InlineSettingsRadioButton>(l10n_util::GetStringUTF16(block_notifications_message_id));
    block_all_button->SetBorder(views::CreateEmptyBorder(kSettingsRadioButtonPadding));
    block_all_button_ = inline_settings_row()->AddChildView(std::move(block_all_button));

    auto dont_block_button = std::make_unique<InlineSettingsRadioButton>(l10n_util::GetStringUTF16(IDS_MESSAGE_CENTER_DONT_BLOCK_NOTIFICATIONS));
    dont_block_button->SetBorder(views::CreateEmptyBorder(kSettingsRadioButtonPadding));
    dont_block_button_ = inline_settings_row()->AddChildView(std::move(dont_block_button));

    inline_settings_row()->SetVisible(false);

    auto settings_done_button = GenerateNotificationLabelButton(
        base::BindRepeating(&NotificationView::ToggleInlineSettings, base::Unretained(this)), l10n_util::GetStringUTF16(IDS_MESSAGE_CENTER_SETTINGS_DONE));

    auto settings_button_row = std::make_unique<views::View>();
    auto settings_button_layout = std::make_unique<views::BoxLayout>(views::BoxLayout::Orientation::kHorizontal, kSettingsButtonRowPadding, 0);
    settings_button_layout->set_main_axis_alignment(views::BoxLayout::MainAxisAlignment::kEnd);
    settings_button_row->SetLayoutManager(std::move(settings_button_layout));
    settings_done_button_ = settings_button_row->AddChildView(std::move(settings_done_button));
    inline_settings_row()->AddChildView(std::move(settings_button_row));
}

void NotificationView::CreateOrUpdateSnoozeSettingsViews(const Notification& notification)
{
    // Not implemented by default.
}

std::unique_ptr<views::LabelButton> NotificationView::GenerateNotificationLabelButton(views::Button::PressedCallback callback, const std::u16string& label)
{
...
{
    left_content()->SetBorder(views::CreateEmptyBorder(IsIconViewShown() ? kLeftContentPaddingWithIcon : kLeftContentPadding));
    ...
    const int message_label_width = (IsIconViewShown() ? kMessageLabelWidthWithIcon : kMessageLabelWidth) - GetInsets().width();
    if (title_view_)
        title_view_->SizeToFit(message_label_width);
    if (message_label()) {
        message_label()->SetMultiLine(true);
        message_label()->SetMaxLines(expanded ? kMaxLinesForExpandedMessageLabel : kMaxLinesForMessageLabel);
...
{
    return kNotificationWidth - kLargeImageContainerPadding.width() - GetInsets().width();
}
...
void NotificationView::ToggleInlineSettings(const ui::Event& event)
{
    ...
    // settings.
    bool disable_notification = !inline_settings_visible && block_all_button_->GetChecked();
...
bool NotificationView::IsExpandable() const
{
    ...
    if (message_label() && message_label()->GetVisible() && message_label()->GetRequiredLines() > 1) {
        return true;
    }
...
void NotificationView::AddLayerToRegion(ui::Layer* layer, views::LayerRegion region)
{
...
void NotificationView::UpdateHeaderViewBackgroundColor()
{
    ...
    auto* notification = MessageCenter::Get()->FindVisibleNotificationById(notification_id());
    if (notification)
...
{
    bool inline_settings_visible = inline_settings_row()->GetVisible();
    return GetColorProvider()->GetColor(inline_settings_visible ? ui::kColorNotificationBackgroundActive : ui::kColorNotificationBackgroundInactive);
}
...
void NotificationView::UpdateActionButtonsRowBackground()
{
    ...
    action_buttons_row()->SetBackground(views::CreateBackgroundFromPainter(std::make_unique<NotificationBackgroundPainter>(
        /*top_radius=*/0, bottom_radius(), GetColorProvider()->GetColor(ui::kColorNotificationActionsBackground))));
}

void NotificationView::AddBackgroundAnimation(const ui::Event& event)
{
    views::InkDrop::Get(this)->SetMode(views::InkDropHost::InkDropMode::ON_NO_GESTURE_HANDLER);
    std::unique_ptr<ui::Event> located_event = notification_view_util::ConvertToBoundedLocatedEvent(event, this);
    views::InkDrop::Get(this)->AnimateToState(views::InkDropState::ACTION_PENDING, ui::LocatedEvent::FromIfValid(located_event.get()));
}

void NotificationView::RemoveBackgroundAnimation()
{
    views::InkDrop::Get(this)->SetMode(views::InkDropHost::InkDropMode::OFF);
    views::InkDrop::Get(this)->AnimateToState(views::InkDropState::HIDDEN, nullptr);
}

std::vector<views::View*> NotificationView::GetChildrenForLayerAdjustment()
{
    return { header_row(), block_all_button_, dont_block_button_, settings_done_button_ };
}
...
void NotificationView::HeaderRowPressed()
{
    ...
    SetManuallyExpandedOrCollapsed(target_expanded_state ? message_center::ExpandState::USER_EXPANDED : message_center::ExpandState::USER_COLLAPSED);
