...
void ProportionalImageView::SetImage(const ui::ImageModel& image, const gfx::Size& max_image_size, bool apply_rounded_corners)
{
...
void ProportionalImageView::OnPaint(gfx::Canvas* canvas)
{
    ...
    gfx::ImageSkia image = (rasterized.size() == draw_size)
        ? rasterized
        : gfx::ImageSkiaOperations::CreateResizedImage(rasterized, skia::ImageOperations::RESIZE_BEST, draw_size);
    ...
    if (apply_rounded_corners_) {
        ...
        const SkScalar kRadius[8] = { corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius, corner_radius };
        path.addRoundRect(gfx::RectToSkRect(draw_bounds), kRadius);

        cc::PaintFlags flags;
        flags.setAntiAlias(true);

        canvas->DrawImageInPath(image, draw_bounds.x(), draw_bounds.y(), path, flags);
        return;
    }
