...
class CompactTitleMessageView : public views::View, public views::LayoutDelegate {
...
public:
    ...
    views::ProposedLayout CalculateProposedLayout(const views::SizeBounds& size_bounds) const override;
...
class MESSAGE_CENTER_EXPORT NotificationViewBase : public MessageView, public views::InkDropObserver, public NotificationInputDelegate {
...
public:
    ...
    void OnNotificationInputSubmit(size_t index, const std::u16string& text) override;

    // Whether the notification view is showing `icon_view_`.
    virtual bool IsIconViewShown() const;

    views::Label* message_label_for_testing()
    {
    ...
    views::Label* status_view_for_testing()
    {
        return status_view_;
    }
...
protected:
    ...
    std::unique_ptr<views::View> CreateActionsRow(std::unique_ptr<views::LayoutManager> layout_manager = std::make_unique<views::FillLayout>());

    // Generate a view to show notification title and other supporting views.
    static std::unique_ptr<views::Label> GenerateTitleView(const std::u16string& title);

    // Generate a view that shows the inline reply textfield and send message
    // button.
    virtual std::unique_ptr<NotificationInputContainer> GenerateNotificationInputContainer();

    // Generate a view that is a views::LabelButton, used in the `actions_row_`.
    // The base class uses an MdTextButton, but ash does not.
    virtual std::unique_ptr<views::LabelButton> GenerateNotificationLabelButton(views::Button::PressedCallback callback, const std::u16string& label) = 0;
    ...
    virtual void CreateOrUpdateCompactTitleMessageView(const Notification& notification);
    ...
    virtual void CreateOrUpdateSmallIconView(const Notification& notification) = 0;

    virtual void CreateOrUpdateInlineSettingsViews(const Notification& notification) = 0;

    virtual void CreateOrUpdateSnoozeSettingsViews(const Notification& notification) = 0;

    // Add view to `left_content_` in its appropriate position according to
    // `left_content_count_`. Return a pointer to added view.
    template <typename T> T* AddViewToLeftContent(std::unique_ptr<T> view)
    {
        return left_content_->AddChildViewAt(std::move(view), left_content_count_++);
    }
    ...
    views::View* content_row()
    {
        return content_row_;
    }
    const views::View* content_row() const
    {
        return content_row_;
    }

    views::BoxLayoutView* left_content()
    {
        return left_content_;
    }
    views::View* right_content()
    {
        return right_content_;
    }
    ...
    const views::Label* message_label() const
    {
        return message_label_;
    }

    ProportionalImageView* icon_view() const
    {
        return icon_view_;
    }

    views::BoxLayoutView* inline_settings_row()
    {
    ...
    views::BoxLayoutView* snooze_settings_row()
    {
        return snooze_row_;
    }
    ...
    {
        return action_buttons_row_;
    }
    const views::View* action_buttons_row() const
    {
        return action_buttons_row_;
    }
    ...
    views::ProgressBar* progress_bar_view() const
    {
        return progress_bar_view_;
    }

    NotificationInputContainer* inline_reply()
    {
        return inline_reply_;
    }

    views::Label* status_view()
    {
        return status_view_;
    }
    const views::Label* status_view() const
    {
        return status_view_;
    }
    const std::vector<raw_ptr<views::View, VectorExperimental>> item_views() const
    {
        return item_views_;
    }

    bool hide_icon_on_expanded() const
    {
        return hide_icon_on_expanded_;
    }
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(NotificationViewBaseTest, InlineSettingsInkDropAnimation);
    ...
    FRIEND_TEST_ALL_PREFIXES(NotificationViewBaseTest, TestDeleteOnDisableNotification);
    FRIEND_TEST_ALL_PREFIXES(NotificationViewBaseTest, TestDeleteOnToggleExpanded);
    FRIEND_TEST_ALL_PREFIXES(NotificationViewBaseTest, TestInlineReply);
    FRIEND_TEST_ALL_PREFIXES(NotificationViewBaseTest, TestInlineReplyActivateWithKeyPress);
    FRIEND_TEST_ALL_PREFIXES(NotificationViewBaseTest, TestInlineReplyRemovedByUpdate);
    ...
    FRIEND_TEST_ALL_PREFIXES(NotificationViewTest, TestAccentColorTextFlagAffectsHeaderRow);
    ...
    raw_ptr<CompactTitleMessageView, DanglingUntriaged> compact_title_message_view_ = nullptr;
    ...
    // placeholder strings.
    std::map<views::LabelButton*, std::optional<std::u16string>> action_button_to_placeholder_map_;
