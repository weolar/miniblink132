...
// timers expire.
class MESSAGE_CENTER_EXPORT PopupTimersController : public MessageCenterObserver, public PopupTimer::Delegate {
public:
    ...
    void OnNotificationDisplayed(const std::string& id, const DisplaySource source) override;
    ...
    void StartTimer(const std::string& id, const base::TimeDelta& timeout_in_seconds);
    ...
    static void SetNotificationTimeouts(int default_timeout, int high_priority_timeout);
...
private:
    ...
    using PopupTimerCollection = std::map<std::string, std::unique_ptr<PopupTimer>>;
