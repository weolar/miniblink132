...
public:
    NopNotificationBlocker()
        : NotificationBlocker(/*message_center=*/nullptr)
    {
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        notification_list_ = std::make_unique<NotificationList>(message_center_.get());
        counter_ = 0;
    }
...
protected:
    ...
    {
        std::string new_id;
        std::unique_ptr<Notification> notification(MakeNotification(optional_fields, &new_id));
    ...
    std::unique_ptr<Notification> MakeNotification(const RichNotificationData& optional_fields, std::string* id_out)
    {
        *id_out = base::StringPrintf(kIdFormat, counter_);
        std::unique_ptr<Notification> notification(
            new Notification(NOTIFICATION_TYPE_SIMPLE, *id_out, u"id" + base::NumberToString16(counter_), u"message" + base::NumberToString16(counter_),
                ui::ImageModel(), kDisplaySource, GURL(), NotifierId(NotifierType::APPLICATION, kExtensionId), optional_fields, nullptr));
        return notification;
    }
    ...
    {
        return notification_list_->GetPopupNotificationsWithoutBlocker(blockers_, NopNotificationBlocker()).size();
    }
...
bool IsInNotifications(const NotificationList::Notifications& notifications, const std::string& id)
{
...
TEST_F(NotificationListTest, UpdateNotification)
{
    ...
    EXPECT_EQ(1u, notification_list_->NotificationCount(blockers_));
    std::unique_ptr<Notification> notification(new Notification(NOTIFICATION_TYPE_SIMPLE, replaced, u"newtitle", u"newbody", ui::ImageModel(), kDisplaySource,
        GURL(), NotifierId(NotifierType::APPLICATION, kExtensionId), RichNotificationData(), nullptr));
    notification_list_->UpdateNotificationMessage(id0, std::move(notification));
    EXPECT_EQ(1u, notification_list_->NotificationCount(blockers_));
    const NotificationList::Notifications notifications = notification_list_->GetVisibleNotifications(blockers_);
...
TEST_F(NotificationListTest, UpdateNotificationWithRenotifyAndQuietMode)
{
    for (size_t quiet_mode = 0u; quiet_mode < 2u; ++quiet_mode) {
        ...
        const NotificationList::PopupNotifications popup_notifications = notification_list_->GetPopupNotifications(blockers_, nullptr);
        if (quiet_mode) {
...
#if BUILDFLAG(IS_CHROMEOS)
    NotifierId id3(NotifierType::SYSTEM_COMPONENT, "system-notifier", ash::NotificationCatalogName::kTestCatalogName);
#else
    NotifierId id3(NotifierType::SYSTEM_COMPONENT, "system-notifier");
#endif // BUILDFLAG(IS_CHROMEOS)
    std::unique_ptr<Notification> notification(
        new Notification(NOTIFICATION_TYPE_SIMPLE, "id0", u"title0", u"message0", ui::ImageModel(), u"source0", GURL(), id0, RichNotificationData(), nullptr));
    notification_list_->AddNotification(std::move(notification));
    notification = std::make_unique<Notification>(
        NOTIFICATION_TYPE_SIMPLE, "id1", u"title1", u"message1", ui::ImageModel(), u"source0", GURL(), id0, RichNotificationData(), nullptr);
    notification_list_->AddNotification(std::move(notification));
    notification = std::make_unique<Notification>(
        NOTIFICATION_TYPE_SIMPLE, "id2", u"title1", u"message1", ui::ImageModel(), u"source1", GURL(), id0, RichNotificationData(), nullptr);
    notification_list_->AddNotification(std::move(notification));
    notification = std::make_unique<Notification>(
        NOTIFICATION_TYPE_SIMPLE, "id3", u"title1", u"message1", ui::ImageModel(), u"source2", GURL(), id1, RichNotificationData(), nullptr);
    notification_list_->AddNotification(std::move(notification));
    notification = std::make_unique<Notification>(
        NOTIFICATION_TYPE_SIMPLE, "id4", u"title1", u"message1", ui::ImageModel(), u"source2", GURL(), id2, RichNotificationData(), nullptr);
    notification_list_->AddNotification(std::move(notification));
    notification = std::make_unique<Notification>(
        NOTIFICATION_TYPE_SIMPLE, "id5", u"title1", u"message1", ui::ImageModel(), u"source2", GURL(), id3, RichNotificationData(), nullptr);
    notification_list_->AddNotification(std::move(notification));

    NotificationList::Notifications by_notifier_id = notification_list_->GetNotificationsByNotifierId(id0);
...
TEST_F(NotificationListTest, Priority)
{
    ...
    for (size_t i = 0; i <= kMaxVisiblePopupNotifications; ++i)
        AddNotification();
    EXPECT_EQ(kMaxVisiblePopupNotifications + 1, notification_list_->NotificationCount(blockers_));
    ...
    EXPECT_EQ(kMaxVisiblePopupNotifications + 2, notification_list_->NotificationCount(blockers_));
    EXPECT_EQ(0u, GetPopupCounts());

    // Minimum priority: doesn't update the unread count.
    AddPriorityNotification(MIN_PRIORITY);
    EXPECT_EQ(kMaxVisiblePopupNotifications + 3, notification_list_->NotificationCount(blockers_));
    EXPECT_EQ(0u, GetPopupCounts());

    NotificationList::Notifications notifications = notification_list_->GetVisibleNotifications(blockers_);
    for (auto iter = notifications.begin(); iter != notifications.end(); ++iter) {
    ...
    EXPECT_EQ(kMaxVisiblePopupNotifications * 4, notification_list_->NotificationCount(blockers_));
...
TEST_F(NotificationListTest, GetNotificationsByAppId)
{
...

    {
        ...
        std::unique_ptr<Notification> notification(new Notification(NOTIFICATION_TYPE_PROGRESS, id1, u"updated", u"updated", ui::ImageModel(), std::u16string(),
            GURL(), NotifierId(NotifierType::APPLICATION, app_id1), RichNotificationData(), nullptr));
        notification_list_->AddNotification(std::move(notification));
        ...
        notification = std::make_unique<Notification>(NOTIFICATION_TYPE_PROGRESS, id1, u"updated", u"updated", ui::ImageModel(), std::u16string(), GURL(),
            NotifierId(NotifierType::APPLICATION, app_id1), RichNotificationData(), nullptr);
        notification_list_->AddNotification(std::move(notification));

        const std::string id2("id2");
        notification = std::make_unique<Notification>(NOTIFICATION_TYPE_PROGRESS, id2, u"updated", u"updated", ui::ImageModel(), std::u16string(), GURL(),
            NotifierId(NotifierType::APPLICATION, app_id1), RichNotificationData(), nullptr);
        notification_list_->AddNotification(std::move(notification));
        EXPECT_EQ(2u, notification_list_->GetNotificationsByAppId(app_id1).size());

        const std::string id3("id3");
        notification = std::make_unique<Notification>(NOTIFICATION_TYPE_PROGRESS, id3, u"updated", u"updated", ui::ImageModel(), std::u16string(), GURL(),
            NotifierId(NotifierType::APPLICATION, app_id2), RichNotificationData(), nullptr);
        notification_list_->AddNotification(std::move(notification));
        EXPECT_EQ(2u, notification_list_->GetNotificationsByAppId(app_id1).size());
    ...
    for (std::string app_id : { app_id1, app_id2 }) {
        for (Notification* notification : notification_list_->GetNotificationsByAppId(app_id)) {
...
TEST_F(NotificationListTest, GetNotificationsByOriginUrl)
{
...

    {
        ...
        std::unique_ptr<Notification> notification(new Notification(
            NOTIFICATION_TYPE_PROGRESS, id1, u"updated", u"updated", ui::ImageModel(), std::u16string(), kUrl1, NotifierId(), RichNotificationData(), nullptr));
        notification_list_->AddNotification(std::move(notification));
        EXPECT_EQ(1u, notification_list_->GetNotificationsByOriginUrl(kUrl1).size());
        ...
        // Remove the notification.
        notification_list_->RemoveNotification(id1);
        EXPECT_EQ(0u, notification_list_->GetNotificationsByOriginUrl(kUrl1).size());
        ...
        const std::string id2("id2");
        notification = std::make_unique<Notification>(
            NOTIFICATION_TYPE_PROGRESS, id2, u"updated", u"updated", ui::ImageModel(), std::u16string(), kUrl1, NotifierId(), RichNotificationData(), nullptr);
        notification_list_->AddNotification(std::move(notification));
        EXPECT_EQ(2u, notification_list_->GetNotificationsByOriginUrl(kUrl1).size());

        const std::string id3("id3");
        notification = std::make_unique<Notification>(
            NOTIFICATION_TYPE_PROGRESS, id3, u"updated", u"updated", ui::ImageModel(), std::u16string(), kUrl2, NotifierId(), RichNotificationData(), nullptr);
        notification_list_->AddNotification(std::move(notification));
        EXPECT_EQ(2u, notification_list_->GetNotificationsByOriginUrl(kUrl1).size());
        EXPECT_EQ(1u, notification_list_->GetNotificationsByOriginUrl(kUrl2).size());
    }

    for (GURL url : { kUrl1, kUrl2 }) {
        for (Notification* notification : notification_list_->GetNotificationsByOriginUrl(url)) {
...
TEST_F(NotificationListTest, UpdateWithoutMessageCenterView)
{
    auto run_test = [this](bool has_message_center_view) {
        ...
        RichNotificationData optional;
        std::unique_ptr<Notification> notification(new Notification(NOTIFICATION_TYPE_SIMPLE, replaced, u"newtitle", u"newbody", ui::ImageModel(),
            kDisplaySource, GURL(), NotifierId(NotifierType::APPLICATION, kExtensionId), optional, nullptr));
        notification_list_->UpdateNotificationMessage(id0, std::move(notification));
        EXPECT_EQ(1u, notification_list_->NotificationCount(blockers_));
        ...
        EXPECT_EQ(0U, notification_list_->GetVisibleNotifications(blockers_).size());
    };
...
TEST_F(NotificationListTest, Renotify)
{
    ...
    std::unique_ptr<Notification> notification(new Notification(NOTIFICATION_TYPE_SIMPLE, replaced, u"newtitle", u"newbody", ui::ImageModel(), kDisplaySource,
        GURL(), NotifierId(NotifierType::APPLICATION, kExtensionId), optional, nullptr));
    notification_list_->UpdateNotificationMessage(id0, std::move(notification));
    EXPECT_EQ(1u, notification_list_->NotificationCount(blockers_));
    EXPECT_EQ(1u, GetPopupCounts());
    const NotificationList::Notifications notifications = notification_list_->GetVisibleNotifications(blockers_);
...
TEST_F(NotificationListTest, PriorityAndRenotify)
{
    ...
    std::unique_ptr<Notification> notification(new Notification(NOTIFICATION_TYPE_SIMPLE, id0, u"newtitle", u"newbody", ui::ImageModel(), kDisplaySource,
        GURL(), NotifierId(NotifierType::APPLICATION, kExtensionId), priority, nullptr));
    notification_list_->UpdateNotificationMessage(id0, std::move(notification));
    EXPECT_EQ(1u, GetPopupCounts());
    notification_list_->MarkSinglePopupAsShown(id0, true);
    EXPECT_EQ(0u, GetPopupCounts());
    ...
    notification = std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, id0, u"newtitle2", u"newbody2", ui::ImageModel(), kDisplaySource, GURL(),
    ...
    notification = std::make_unique<Notification>(NOTIFICATION_TYPE_SIMPLE, id1, u"newtitle", u"newbody", ui::ImageModel(), kDisplaySource, GURL(),
        NotifierId(NotifierType::APPLICATION, kExtensionId), priority, nullptr);
...
TEST_F(NotificationListTest, MarkSinglePopupAsShown)
{
    ...
    ASSERT_EQ(std::min(static_cast<size_t>(3u), kMaxVisiblePopupNotifications), GetPopupCounts());
...
TEST_F(NotificationListTest, TestHasNotificationOfType)
{
    ...
    EXPECT_TRUE(notification_list_->HasNotificationOfType(id, NOTIFICATION_TYPE_SIMPLE));
    EXPECT_FALSE(notification_list_->HasNotificationOfType(id, NOTIFICATION_TYPE_PROGRESS));

    std::unique_ptr<Notification> updated_notification(new Notification(
        NOTIFICATION_TYPE_PROGRESS, id, u"updated", u"updated", ui::ImageModel(), std::u16string(), GURL(), NotifierId(), RichNotificationData(), nullptr));
    notification_list_->AddNotification(std::move(updated_notification));

    EXPECT_FALSE(notification_list_->HasNotificationOfType(id, NOTIFICATION_TYPE_SIMPLE));
    EXPECT_TRUE(notification_list_->HasNotificationOfType(id, NOTIFICATION_TYPE_PROGRESS));
}
