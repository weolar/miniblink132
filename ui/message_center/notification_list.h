...
template <typename PlainCompare> struct UniquePtrCompare {
    template <typename T> bool operator()(const std::unique_ptr<T>& n1, const std::unique_ptr<T>& n2) const
    {
...
public:
    ...
    // Notification Center.
    using Notifications = std::set<raw_ptr<Notification, SetExperimental>, ComparePriorityTimestampSerial>;
    using OwnedNotifications = std::map<std::unique_ptr<Notification>, NotificationState, UniquePtrCompare<ComparePriorityTimestampSerial>>;
    ...
    void SetNotificationsShown(const NotificationBlockers& blockers, std::set<std::string>* updated_ids);

    void AddNotification(std::unique_ptr<Notification> notification);

    void UpdateNotificationMessage(const std::string& old_id, std::unique_ptr<Notification> new_notification);
    ...
    Notifications GetNotificationsByNotifierId(const NotifierId& notifier_id) const;
    ...
    bool SetNotificationIcon(const std::string& notification_id, const ui::ImageModel& image);

    // Returns true if the notification exists and was updated.
    bool SetNotificationImage(const std::string& notification_id, const gfx::Image& image);

    // Returns true if |id| matches a notification in the list and that
    // notification's type matches the given type.
    bool HasNotificationOfType(const std::string& id, const NotificationType type) const;
    ...
    PopupNotifications GetPopupNotifications(const NotificationBlockers& blockers, std::list<std::string>* blocked);
    ...
    PopupNotifications GetPopupNotificationsWithoutBlocker(const NotificationBlockers& blockers, const NotificationBlocker& blocker) const;

    // Marks a specific popup item as shown. Set |mark_notification_as_read| to
    // true in case marking the notification as read too.
    void MarkSinglePopupAsShown(const std::string& id, bool mark_notification_as_read);
    ...
    void SetNotificationExpandState(const std::string& id, ExpandState expand_state);
    ...
    Notifications GetVisibleNotifications(const NotificationBlockers& blockers) const;

    // Returns all visible notifications if not for the provided blocker.
    Notifications GetVisibleNotificationsWithoutBlocker(const NotificationBlockers& blockers, const NotificationBlocker* ignored_blocker) const;
...
private:
    ...
    OwnedNotifications::const_iterator GetNotification(const std::string& id) const;
