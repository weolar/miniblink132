...
std::wstring GetDialogItemText(HWND window, int dialog_item_id)
{
    ...
    wchar_t buffer[256];
    UINT count = ::GetDlgItemText(window, dialog_item_id, buffer, std::size(buffer));
    return std::wstring(buffer, count);
}
...
class SelectFileDialogWinTest : public ::testing::Test, public ui::SelectFileDialog::Listener {
public:
...
TEST_F(SelectFileDialogWinTest, CancelAllDialogs)
{
    ...
    struct {
        ...
        },
        {
            ...
            kSaveFileDefaultTitle,
        },
        {
            ui::SelectFileDialog::SELECT_OPEN_FILE,
            kSelectFileDefaultTitle,
        },
        {
    ...
    for (size_t i = 0; i < std::size(kTestCases); ++i) {
        ...
        scoped_refptr<ui::SelectFileDialog> dialog = ui::SelectFileDialog::Create(this, nullptr);
        ...
        dialog->SelectFile(
            test_case.dialog_type, std::u16string(), base::FilePath(), file_type_info.get(), file_type_info_index, std::wstring(), native_window());
...
TEST_F(SelectFileDialogWinTest, UploadFolderCheckStrings)
{
    ...
    scoped_refptr<ui::SelectFileDialog> dialog = ui::SelectFileDialog::Create(this, nullptr);
    dialog->SelectFile(ui::SelectFileDialog::SELECT_UPLOAD_FOLDER, std::u16string(), default_path, nullptr, 0, L"", native_window());
    ...
    HWND window = WaitForDialogWindow(base::UTF16ToWide(l10n_util::GetStringUTF16(IDS_SELECT_UPLOAD_FOLDER_DIALOG_TITLE)));
    EXPECT_NE(GetWindowTitle(window), kSelectFolderDefaultTitle);

    // Check the OK button text.
    EXPECT_EQ(GetDialogItemText(window, 1), base::UTF16ToWide(l10n_util::GetStringUTF16(IDS_SELECT_UPLOAD_FOLDER_DIALOG_UPLOAD_BUTTON)));
    ...
    EXPECT_TRUE(base::FilePath::CompareEqualIgnoreCase(selected_paths()[0].value(), default_path.value()));
}
...
TEST_F(SelectFileDialogWinTest, SpecifyTitle)
{
    ...
    scoped_refptr<ui::SelectFileDialog> dialog = ui::SelectFileDialog::Create(this, nullptr);
    dialog->SelectFile(ui::SelectFileDialog::SELECT_OPEN_FILE, kTitle, default_path, nullptr, 0, L"", native_window());
...
TEST_F(SelectFileDialogWinTest, TestSelectFile)
{
    ...
    scoped_refptr<ui::SelectFileDialog> dialog = ui::SelectFileDialog::Create(this, nullptr);
    dialog->SelectFile(ui::SelectFileDialog::SELECT_OPEN_FILE, std::u16string(), default_path, nullptr, 0, L"", native_window());
...
TEST_F(SelectFileDialogWinTest, TestSaveFile)
{
    ...
    scoped_refptr<ui::SelectFileDialog> dialog = ui::SelectFileDialog::Create(this, nullptr);
    dialog->SelectFile(ui::SelectFileDialog::SELECT_SAVEAS_FILE, std::u16string(), default_path, &file_type_info, 1, L"", native_window());
    ...
    // normalized to C:\Windows by the file dialog, leading to spurious failures
    // if a case-sensitive comparison is used.
    EXPECT_TRUE(base::FilePath::CompareEqualIgnoreCase(selected_paths()[0].value(), default_path.AddExtension(L"html").value()));
}
...
TEST_F(SelectFileDialogWinTest, SaveAsDifferentExtension)
{
    ...
    scoped_refptr<ui::SelectFileDialog> dialog = ui::SelectFileDialog::Create(this, nullptr);
    dialog->SelectFile(ui::SelectFileDialog::SELECT_SAVEAS_FILE, std::u16string(), default_path, &file_type_info, 1, L"html", native_window());
...
TEST_F(SelectFileDialogWinTest, OpenFileDifferentExtension)
{
    ...
    scoped_refptr<ui::SelectFileDialog> dialog = ui::SelectFileDialog::Create(this, nullptr);
    dialog->SelectFile(ui::SelectFileDialog::SELECT_OPEN_FILE, std::u16string(), default_path, &file_type_info, 1, L"html", native_window());
...
TEST_F(SelectFileDialogWinTest, SelectNonExistingFile)
{
    ...
    base::FilePath default_path = scoped_temp_dir.GetPath().Append(L"does-not-exist.html");
