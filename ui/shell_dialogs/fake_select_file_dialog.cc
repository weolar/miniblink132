...
ui::SelectFileDialog* FakeSelectFileDialog::Factory::Create(ui::SelectFileDialog::Listener* listener, std::unique_ptr<ui::SelectFilePolicy> policy)
{
    FakeSelectFileDialog* dialog = new FakeSelectFileDialog(opened_callback_, listener, std::move(policy));
...
void FakeSelectFileDialog::Factory::SetOpenCallback(base::RepeatingClosure callback)
{
...
FakeSelectFileDialog::FakeSelectFileDialog(const base::RepeatingClosure& opened, Listener* listener, std::unique_ptr<ui::SelectFilePolicy> policy)
...
void FakeSelectFileDialog::SelectFileImpl(Type type, const std::u16string& title, const base::FilePath& default_path, const FileTypeInfo* file_types,
...
bool FakeSelectFileDialog::CallFileSelected(const base::FilePath& file_path, std::string_view filter_text)
{
    for (size_t index = 0; index < file_types_.extensions.size(); ++index) {
        for (const base::FilePath::StringType& ext : file_types_.extensions[index]) {
...
void FakeSelectFileDialog::CallMultiFilesSelected(const std::vector<base::FilePath>& files)
{
