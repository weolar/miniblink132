...
TEST(ShellDialogs, ShortenFileNameIfNeeded)
{
    struct ShortenFileNameTestCase {
    ...
    } test_cases[] = { // Paths with short paths/file names don't get shortened.
        { FILE_PATH_LITERAL("folder1111/folder2222/file1.html"), FILE_PATH_LITERAL("folder1111/folder2222/file1.html") },
        ...
        // file dialog.
        { FILE_PATH_LITERAL("folder1111/folder2222/folder3333/folder4444/folder5555/folder6666/"
    ...
    for (size_t i = 0; i < std::size(test_cases); ++i) {
        base::FilePath input = base::FilePath(test_cases[i].input).NormalizePathSeparators();
        base::FilePath output = base::FilePath(test_cases[i].expected).NormalizePathSeparators();
        EXPECT_EQ(output.value(), ui::SelectFileDialog::GetShortenedFilePath(input).value());
        EXPECT_LE(ui::SelectFileDialog::GetShortenedFilePath(input).BaseName().value().length(), 255u);
    }
}
