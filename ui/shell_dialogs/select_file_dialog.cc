...
void SelectFileDialog::Listener::MultiFilesSelected(const std::vector<SelectedFileInfo>& files)
{
    NOTREACHED();
}
...
SelectFileDialog::FileTypeInfo::FileTypeInfo(const FileTypeInfo& other) = default;
...
SelectFileDialog::FileTypeInfo::FileTypeInfo(std::vector<FileExtensionList> in_extensions)
    : extensions(std::move(in_extensions))
{
}

SelectFileDialog::FileTypeInfo::FileTypeInfo(std::vector<FileExtensionList> in_extensions, std::vector<std::u16string> in_descriptions)
    : extensions(std::move(in_extensions))
    , extension_description_overrides(std::move(in_descriptions))
{
    CHECK(extension_description_overrides.empty() || extension_description_overrides.size() == extensions.size());
}

SelectFileDialog::FileTypeInfo::~FileTypeInfo() = default;

// static
void SelectFileDialog::SetFactory(std::unique_ptr<ui::SelectFileDialogFactory> factory)
{
...
// static
scoped_refptr<SelectFileDialog> SelectFileDialog::Create(Listener* listener, std::unique_ptr<ui::SelectFilePolicy> policy)
{
    if (dialog_factory_)
...
base::FilePath SelectFileDialog::GetShortenedFilePath(const base::FilePath& path)
{
    ...
    if (file_string.length() < kMaxNameLength) {
        max_extension_length = std::max(max_extension_length, kMaxNameLength - file_string.length());
    }
...
void SelectFileDialog::SelectFile(Type type, const std::u16string& title, const base::FilePath& default_path, const FileTypeInfo* file_types,
    int file_type_index, const base::FilePath::StringType& default_extension, gfx::NativeWindow owning_window, const GURL* caller)
{
    ...
    if (select_file_policy_.get() && !select_file_policy_->CanOpenSelectFileDialog()) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&SelectFileDialog::CancelFileSelection, this));
        return;
    }
    ...
    SelectFileImpl(type, title, path, file_types, file_type_index, default_extension, owning_window, caller);
}
...
SelectFileDialog::SelectFileDialog(Listener* listener, std::unique_ptr<ui::SelectFilePolicy> policy)
    : listener_(listener)
    , select_file_policy_(std::move(policy))
{
    DCHECK(listener_);
}
