...
public:
    SelectFileDialogLinuxKde(
        Listener* listener, std::unique_ptr<ui::SelectFilePolicy> policy, base::nix::DesktopEnvironment desktop, const std::string& kdialog_version);
...
protected:
    ...
    void SelectFileImpl(Type type, const std::u16string& title, const base::FilePath& default_path, const FileTypeInfo* file_types, int file_type_index,
        const base::FilePath::StringType& default_extension, gfx::NativeWindow owning_window, const GURL* caller) override;

private:
    bool HasMultipleFileTypeChoicesImpl() override;

    struct KDialogParams {
        KDialogParams(const std::string& type, const std::string& title, const base::FilePath& default_path, gfx::AcceleratedWidget parent, bool file_operation,
    ...
    void GetKDialogCommandLine(const std::string& type, const std::string& title, const base::FilePath& default_path, gfx::AcceleratedWidget parent,
        bool file_operation, bool multiple_selection, base::CommandLine* command_line);

    // Call KDialog on the FILE thread and return the results.
    std::unique_ptr<KDialogOutputParams> CallKDialogOutput(const KDialogParams& params);
    ...
    void CreateSelectFolderDialog(Type type, const std::string& title, const base::FilePath& default_path, gfx::AcceleratedWidget parent);

    void CreateFileOpenDialog(const std::string& title, const base::FilePath& default_path, gfx::AcceleratedWidget parent);

    void CreateMultiFileOpenDialog(const std::string& title, const base::FilePath& default_path, gfx::AcceleratedWidget parent);

    void CreateSaveAsDialog(const std::string& title, const base::FilePath& default_path, gfx::AcceleratedWidget parent);

    // Common function for OnSelectSingleFileDialogResponse and
    // OnSelectSingleFolderDialogResponse.
    void SelectSingleFileHelper(bool allow_folder, std::unique_ptr<KDialogOutputParams> results);

    void OnSelectSingleFileDialogResponse(gfx::AcceleratedWidget parent, std::unique_ptr<KDialogOutputParams> results);
    void OnSelectMultiFileDialogResponse(gfx::AcceleratedWidget parent, std::unique_ptr<KDialogOutputParams> results);
    void OnSelectSingleFolderDialogResponse(gfx::AcceleratedWidget parent, std::unique_ptr<KDialogOutputParams> results);
...
// static
bool SelectFileDialogLinux::CheckKDEDialogWorksOnUIThread(std::string& kdialog_version)
{
...
SelectFileDialog* NewSelectFileDialogLinuxKde(SelectFileDialog::Listener* listener, std::unique_ptr<ui::SelectFilePolicy> policy,
    base::nix::DesktopEnvironment desktop, const std::string& kdialog_version)
{
    return new SelectFileDialogLinuxKde(listener, std::move(policy), desktop, kdialog_version);
}
...
SelectFileDialogLinuxKde::SelectFileDialogLinuxKde(
    ...
    , desktop_(desktop)
    , pipe_task_runner_(
          base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::TaskPriority::USER_BLOCKING, base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN }))
{
    DCHECK(desktop_ == base::nix::DESKTOP_ENVIRONMENT_KDE3 || desktop_ == base::nix::DESKTOP_ENVIRONMENT_KDE4 || desktop_ == base::nix::DESKTOP_ENVIRONMENT_KDE5
    ...
    std::vector<std::string_view> version_pieces = base::SplitStringPiece(kdialog_version, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    if (version_pieces.size() >= 2) {
        base::Version parsed_version(version_pieces[1]);
        if (parsed_version.IsValid()) {
            kdialog_supports_multiple_extensions_ = parsed_version >= base::Version("19.12");
        }
    }
}
...
bool SelectFileDialogLinuxKde::IsRunning(gfx::NativeWindow parent_window) const
{
...
void SelectFileDialogLinuxKde::SelectFileImpl(Type type, const std::u16string& title, const base::FilePath& default_path, const FileTypeInfo* file_types,
...
std::string SelectFileDialogLinuxKde::GetMimeTypeFilterString()
{
    ...
    if (!kdialog_supports_multiple_extensions_) {
        ...
        for (auto& extensions : file_types().extensions) {
            for (auto& extension : extensions) {
                if (!extension.empty()) {
                    std::string mime_type = base::nix::GetFileMimeType(base::FilePath("name").ReplaceExtension(extension));
                    filter_set.insert(mime_type);
                }
            }
        }
        std::vector<std::string> filter_vector(filter_set.cbegin(), filter_set.cend());
    ...
    for (size_t i = 0; i < file_types().extensions.size(); ++i) {
        ...
        std::vector<std::string> extension_filters_vector(extension_filters.begin(), extension_filters.end());
        ...
        if (i < file_types().extension_description_overrides.size()) {
            description = base::UTF16ToUTF8(file_types().extension_description_overrides[i]);
        ...
        filters.push_back(description + " (" + base::JoinString(extension_filters_vector, " ") + ")");
    }
...
std::unique_ptr<SelectFileDialogLinuxKde::KDialogOutputParams> SelectFileDialogLinuxKde::CallKDialogOutput(const KDialogParams& params)
{
    DCHECK(pipe_task_runner_->RunsTasksInCurrentSequence());
    base::CommandLine::StringVector cmd_vector;
    cmd_vector.push_back(kKdialogBinary);
    base::CommandLine command_line(cmd_vector);
    GetKDialogCommandLine(params.type, params.title, params.default_path, params.parent, params.file_operation, params.multiple_selection, &command_line);

    auto results = std::make_unique<KDialogOutputParams>();
    // Get output from KDialog
    base::GetAppOutputWithExitCode(command_line, &results->output, &results->exit_code);
    if (!results->output.empty())
        results->output.erase(results->output.size() - 1);
    return results;
}

void SelectFileDialogLinuxKde::GetKDialogCommandLine(const std::string& type, const std::string& title, const base::FilePath& path,
    gfx::AcceleratedWidget parent, bool file_operation, bool multiple_selection, base::CommandLine* command_line)
{
    CHECK(command_line);
    ...
    if (parent != gfx::kNullAcceleratedWidget) {
        command_line->AppendSwitchNative(
            desktop_ == base::nix::DESKTOP_ENVIRONMENT_KDE3 ? "--embed" : "--attach", base::NumberToString(static_cast<uint32_t>(parent)));
    }
...
void SelectFileDialogLinuxKde::FileSelected(const base::FilePath& path)
{
    ...
    } else if (type() == SELECT_FOLDER || type() == SELECT_UPLOAD_FOLDER || type() == SELECT_EXISTING_FOLDER) {
...
void SelectFileDialogLinuxKde::MultiFilesSelected(const std::vector<base::FilePath>& files)
{
...
void SelectFileDialogLinuxKde::CreateSelectFolderDialog(Type type, const std::string& title, const base::FilePath& default_path, gfx::AcceleratedWidget parent)
{
    int title_message_id = (type == SELECT_UPLOAD_FOLDER) ? IDS_SELECT_UPLOAD_FOLDER_DIALOG_TITLE : IDS_SELECT_FOLDER_DIALOG_TITLE;
    pipe_task_runner_->PostTaskAndReplyWithResult(FROM_HERE,
        base::BindOnce(&SelectFileDialogLinuxKde::CallKDialogOutput, this,
            KDialogParams(
                "--getexistingdirectory", GetTitle(title, title_message_id), default_path.empty() ? *last_opened_path() : default_path, parent, false, false)),
        base::BindOnce(&SelectFileDialogLinuxKde::OnSelectSingleFolderDialogResponse, this, parent));
}

void SelectFileDialogLinuxKde::CreateFileOpenDialog(const std::string& title, const base::FilePath& default_path, gfx::AcceleratedWidget parent)
{
    pipe_task_runner_->PostTaskAndReplyWithResult(FROM_HERE,
        base::BindOnce(&SelectFileDialogLinuxKde::CallKDialogOutput, this,
            KDialogParams("--getopenfilename", GetTitle(title, IDS_OPEN_FILE_DIALOG_TITLE), default_path.empty() ? *last_opened_path() : default_path, parent,
                true, false)),
        base::BindOnce(&SelectFileDialogLinuxKde::OnSelectSingleFileDialogResponse, this, parent));
}

void SelectFileDialogLinuxKde::CreateMultiFileOpenDialog(const std::string& title, const base::FilePath& default_path, gfx::AcceleratedWidget parent)
{
    pipe_task_runner_->PostTaskAndReplyWithResult(FROM_HERE,
        base::BindOnce(&SelectFileDialogLinuxKde::CallKDialogOutput, this,
            KDialogParams("--getopenfilename", GetTitle(title, IDS_OPEN_FILES_DIALOG_TITLE), default_path.empty() ? *last_opened_path() : default_path, parent,
                true, true)),
        base::BindOnce(&SelectFileDialogLinuxKde::OnSelectMultiFileDialogResponse, this, parent));
}

void SelectFileDialogLinuxKde::CreateSaveAsDialog(const std::string& title, const base::FilePath& default_path, gfx::AcceleratedWidget parent)
{
    pipe_task_runner_->PostTaskAndReplyWithResult(FROM_HERE,
        base::BindOnce(&SelectFileDialogLinuxKde::CallKDialogOutput, this,
            KDialogParams(
                "--getsavefilename", GetTitle(title, IDS_SAVE_AS_DIALOG_TITLE), default_path.empty() ? *last_saved_path() : default_path, parent, true, false)),
        base::BindOnce(&SelectFileDialogLinuxKde::OnSelectSingleFileDialogResponse, this, parent));
}

void SelectFileDialogLinuxKde::SelectSingleFileHelper(bool allow_folder, std::unique_ptr<KDialogOutputParams> results)
{
...
void SelectFileDialogLinuxKde::OnSelectSingleFileDialogResponse(gfx::AcceleratedWidget parent, std::unique_ptr<KDialogOutputParams> results)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void SelectFileDialogLinuxKde::OnSelectSingleFolderDialogResponse(gfx::AcceleratedWidget parent, std::unique_ptr<KDialogOutputParams> results)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    parents_.erase(parent);
...
void SelectFileDialogLinuxKde::OnSelectMultiFileDialogResponse(gfx::AcceleratedWidget parent, std::unique_ptr<KDialogOutputParams> results)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    for (std::string_view line : base::SplitStringPiece(results->output, "\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY)) {
