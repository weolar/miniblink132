...
constexpr char kXdgPortalRequestInterfaceName[] = "org.freedesktop.portal.Request";
constexpr char kXdgPortalResponseSignal[] = "Response";

constexpr char kFileChooserInterfaceName[] = "org.freedesktop.portal.FileChooser";
...
void AppendStringOption(dbus::MessageWriter* writer, const std::string& name, const std::string& value)
{
...
void AppendByteStringOption(dbus::MessageWriter* writer, const std::string& name, const std::string& value)
{
    dbus::MessageWriter option_writer(nullptr);
    writer->OpenDictEntry(&option_writer);
    ...
    value_writer.AppendArrayOfBytes(base::make_span(reinterpret_cast<const std::uint8_t*>(value.c_str()),
...
SelectFileDialogLinuxPortal::SelectFileDialogLinuxPortal(Listener* listener, std::unique_ptr<ui::SelectFilePolicy> policy)
...
SelectFileDialogLinuxPortal::~SelectFileDialogLinuxPortal()
{
    ...
    dbus_thread_linux::GetTaskRunner()->PostTask(FROM_HERE, base::BindOnce([](scoped_refptr<DialogInfo> info) {}, std::move(info_)));
}
...
void SelectFileDialogLinuxPortal::StartAvailabilityTestInBackground()
{
    ...
    dbus_thread_linux::GetTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&SelectFileDialogLinuxPortal::CheckPortalAvailabilityOnBusThread));
}
...
{
    dbus_thread_linux::GetTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&DestroyBusOnBusThread));
}

bool SelectFileDialogLinuxPortal::IsRunning(gfx::NativeWindow parent_window) const
{
    return parent_window && host_ && host_.get() == parent_window->GetHost();
}

void SelectFileDialogLinuxPortal::SelectFileImpl(Type type, const std::u16string& title, const base::FilePath& default_path, const FileTypeInfo* file_types,
    int file_type_index, const base::FilePath::StringType& default_extension, gfx::NativeWindow owning_window, const GURL* caller)
{
    info_ = base::MakeRefCounted<DialogInfo>(base::BindOnce(&SelectFileDialogLinuxPortal::DialogCreatedOnMainThread, weak_factory_.GetWeakPtr()),
        base::BindOnce(&SelectFileDialogLinuxPortal::CompleteOpenOnMainThread, weak_factory_.GetWeakPtr()),
        base::BindOnce(&SelectFileDialogLinuxPortal::CancelOpenOnMainThread, weak_factory_.GetWeakPtr()));
    info_->type = type;
    info_->main_task_runner = base::SequencedTaskRunner::GetCurrentDefault();

    if (owning_window) {
        if (auto* root = owning_window->GetRootWindow()) {
            if (auto* host = root->GetNativeWindowProperty(views::DesktopWindowTreeHostLinux::kWindowKey)) {
    ...
    if (host_) {
        ...
        if (delegate
            && delegate->ExportWindowHandle(host_->GetAcceleratedWidget(),
                base::BindOnce(&SelectFileDialogLinuxPortal::SelectFileImplWithParentHandle, this, title, default_path, filter_set, default_extension))) {
    ...
    SelectFileImplWithParentHandle(title, default_path, filter_set, default_extension, "");
}
...
void SelectFileDialogLinuxPortal::CheckPortalAvailabilityOnBusThread()
{
    ...
    base::AtomicFlag* availability_test_complete = GetAvailabilityTestCompletionFlag();
    if (availability_test_complete->IsSet())
        return;

    scoped_refptr<dbus::Bus> bus = AcquireBusOnBusThread();

    dbus_utils::CheckForServiceAndStart(bus, kXdgPortalService,
        base::BindOnce(
            [](scoped_refptr<dbus::Bus> bus, base::AtomicFlag* availability_test_complete, std::optional<bool> name_has_owner) {
                if (name_has_owner.value_or(false)) {
                    ...
                    dbus::ObjectProxy* portal = bus->GetObjectProxy(kXdgPortalService, portal_path);
                ...
                VLOG(1) << "File chooser portal available: " << (is_portal_available_ ? "yes" : "no");
...
// static
base::AtomicFlag* SelectFileDialogLinuxPortal::GetAvailabilityTestCompletionFlag()
{
    static base::NoDestructor<base::AtomicFlag> flag;
    return flag.get();
}

SelectFileDialogLinuxPortal::PortalFilterSet SelectFileDialogLinuxPortal::BuildFilterSet()
{
    ...
    for (size_t i = 0; i < file_types().extensions.size(); ++i) {
        ...
        if (i < file_types().extension_description_overrides.size()) {
            filter.name = base::UTF16ToUTF8(file_types().extension_description_overrides[i]);
        }
...
void SelectFileDialogLinuxPortal::SelectFileImplWithParentHandle(
    std::u16string title, base::FilePath default_path, PortalFilterSet filter_set, base::FilePath::StringType default_extension, std::string parent_handle)
{
    bool default_path_exists = CallDirectoryExistsOnUIThread(default_path);
    dbus_thread_linux::GetTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&SelectFileDialogLinuxPortal::DialogInfo::SelectFileImplOnBusThread, info_, std::move(title), std::move(default_path),
            default_path_exists, std::move(filter_set), std::move(default_extension), std::move(parent_handle)));
}

void SelectFileDialogLinuxPortal::DialogInfo::SelectFileImplOnBusThread(std::u16string title, base::FilePath default_path, const bool default_path_exists,
    PortalFilterSet filter_set, base::FilePath::StringType default_extension, std::string parent_handle)
{
    DCHECK(dbus_thread_linux::GetTaskRunner()->RunsTasksInCurrentSequence());
    ...
    std::string response_handle_token = base::StringPrintf("handle_%d", handle_token_counter_++);

    AppendOptions(&writer, response_handle_token, default_path, default_path_exists, filter_set);

    dbus::ObjectPath expected_handle_path(base::nix::XdgDesktopPortalRequestPath(bus->GetConnectionName(), response_handle_token));

    response_handle_ = bus->GetObjectProxy(kXdgPortalService, expected_handle_path);
    ConnectToHandle();

    dbus::ObjectPath portal_path(kXdgPortalObject);
    dbus::ObjectProxy* portal = bus->GetObjectProxy(kXdgPortalService, portal_path);
    portal->CallMethodWithErrorResponse(
        &method_call, dbus::ObjectProxy::TIMEOUT_USE_DEFAULT, base::BindOnce(&DialogInfo::OnCallResponse, weak_factory_.GetWeakPtr(), base::Unretained(bus)));
}

void SelectFileDialogLinuxPortal::DialogInfo::AppendOptions(dbus::MessageWriter* writer, const std::string& response_handle_token,
    const base::FilePath& default_path, const bool default_path_exists, const SelectFileDialogLinuxPortal::PortalFilterSet& filter_set)
{
    dbus::MessageWriter options_writer(nullptr);
    writer->OpenArray("{sv}", &options_writer);

    AppendStringOption(&options_writer, kFileChooserOptionHandleToken, response_handle_token);

    if (type == SelectFileDialog::Type::SELECT_UPLOAD_FOLDER) {
        AppendStringOption(&options_writer, kFileChooserOptionAcceptLabel, l10n_util::GetStringUTF8(IDS_SELECT_UPLOAD_FOLDER_DIALOG_UPLOAD_BUTTON));
    }

    if (type == SelectFileDialog::Type::SELECT_FOLDER || type == SelectFileDialog::Type::SELECT_UPLOAD_FOLDER
    ...
    if (!default_path.empty()) {
        if (default_path_exists) {
            ...
            // filename.
            AppendByteStringOption(&options_writer, kFileChooserOptionCurrentFolder, default_path.value());
        } else {
            ...
            AppendByteStringOption(&options_writer, kFileChooserOptionCurrentFolder, default_path.DirName().value());
            ...
            if (type == SelectFileDialog::Type::SELECT_SAVEAS_FILE) {
                AppendStringOption(&options_writer, kFileChooserOptionCurrentName, default_path.BaseName().value());
            }
        }
    }
...
void SelectFileDialogLinuxPortal::DialogInfo::AppendFiltersOption(dbus::MessageWriter* writer, const std::vector<PortalFilter>& filters)
{
    dbus::MessageWriter option_writer(nullptr);
    writer->OpenDictEntry(&option_writer);
...
void SelectFileDialogLinuxPortal::DialogInfo::AppendFilterStruct(dbus::MessageWriter* writer, const PortalFilter& filter)
{
...
{
    DCHECK(dbus_thread_linux::GetTaskRunner()->RunsTasksInCurrentSequence());
    response_handle_->ConnectToSignal(kXdgPortalRequestInterfaceName, kXdgPortalResponseSignal,
        base::BindRepeating(&DialogInfo::OnResponseSignalEmitted, weak_factory_.GetWeakPtr()),
        base::BindOnce(&DialogInfo::OnResponseSignalConnected, weak_factory_.GetWeakPtr()));
}

void SelectFileDialogLinuxPortal::DialogInfo::CompleteOpen(std::vector<base::FilePath> paths, std::string current_filter)
{
    DCHECK(dbus_thread_linux::GetTaskRunner()->RunsTasksInCurrentSequence());
    response_handle_->Detach();
    main_task_runner->PostTask(FROM_HERE, base::BindOnce(std::move(selected_callback_), std::move(paths), std::move(current_filter)));
}
...
void SelectFileDialogLinuxPortal::DialogCreatedOnMainThread()
{
    ...
    reenable_window_event_handling_ = static_cast<views::DesktopWindowTreeHostLinux*>(host_.get())->DisableEventListening();
}

void SelectFileDialogLinuxPortal::CompleteOpenOnMainThread(std::vector<base::FilePath> paths, std::string current_filter)
{
    UnparentOnMainThread();
...
void SelectFileDialogLinuxPortal::DialogInfo::OnCallResponse(dbus::Bus* bus, dbus::Response* response, dbus::ErrorResponse* error_response)
{
    DCHECK(dbus_thread_linux::GetTaskRunner()->RunsTasksInCurrentSequence());
    if (response) {
        ...
        if (!reader.PopObjectPath(&actual_handle_path)) {
            ...
            if (response_handle_->object_path() != actual_handle_path) {
                VLOG(1) << "Re-attaching response handle to " << actual_handle_path.value();

                response_handle_->Detach();
                response_handle_ = bus->GetObjectProxy(kXdgPortalService, actual_handle_path);
                ConnectToHandle();
            }
        ...
        LOG(ERROR) << "Portal returned error: " << error_name << ": " << error_message;
    } else {
        NOTREACHED();
    }
...
void SelectFileDialogLinuxPortal::DialogInfo::OnResponseSignalConnected(const std::string& interface, const std::string& signal, bool connected)
{
    DCHECK(dbus_thread_linux::GetTaskRunner()->RunsTasksInCurrentSequence());
    if (!connected) {
...
void SelectFileDialogLinuxPortal::DialogInfo::OnResponseSignalEmitted(dbus::Signal* signal)
{
    DCHECK(dbus_thread_linux::GetTaskRunner()->RunsTasksInCurrentSequence());
    ...
    if (!CheckResponseCode(&reader) || !ReadResponseResults(&reader, &uris, &current_filter)) {
        CancelOpen();
        return;
    }
...
bool SelectFileDialogLinuxPortal::DialogInfo::CheckResponseCode(dbus::MessageReader* reader)
{
...
bool SelectFileDialogLinuxPortal::DialogInfo::ReadResponseResults(dbus::MessageReader* reader, std::vector<std::string>* uris, std::string* current_filter)
{
    ...
    while (results_reader.HasMoreData()) {
        ...
        std::string key;
        if (!results_reader.PopDictEntry(&entry_reader) || !entry_reader.PopString(&key)) {
        ...
        if (key == "uris") {
            ...
            if (!entry_reader.PopVariant(&uris_reader) || !uris_reader.PopArrayOfStrings(uris)) {
        ...
        if (key == "current_filter") {
            ...
            if (!entry_reader.PopVariant(&current_filter_reader) || !current_filter_reader.PopStruct(&current_filter_struct_reader)
...
std::vector<base::FilePath> SelectFileDialogLinuxPortal::DialogInfo::ConvertUrisToPaths(const std::vector<std::string>& uris)
{
    ...
    for (const std::string& uri : uris) {
        ...
        url::DecodeURLEscapeSequences(encoded_path, url::DecodeURLMode::kUTF8OrIsomorphic, &decoded_path);
...
SelectFileDialogLinuxPortal::PortalFilter::PortalFilter(const PortalFilter& other) = default;
SelectFileDialogLinuxPortal::PortalFilter::PortalFilter(PortalFilter&& other) = default;
SelectFileDialogLinuxPortal::PortalFilter::~PortalFilter() = default;

SelectFileDialogLinuxPortal::PortalFilterSet::PortalFilterSet() = default;
SelectFileDialogLinuxPortal::PortalFilterSet::PortalFilterSet(const PortalFilterSet& other) = default;
SelectFileDialogLinuxPortal::PortalFilterSet::PortalFilterSet(PortalFilterSet&& other) = default;
SelectFileDialogLinuxPortal::PortalFilterSet::~PortalFilterSet() = default;

SelectFileDialogLinuxPortal::DialogInfo::DialogInfo(
    base::OnceClosure created_callback, OnSelectFileExecutedCallback selected_callback, OnSelectFileCanceledCallback canceled_callback)
