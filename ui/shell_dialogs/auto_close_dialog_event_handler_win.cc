...
HRESULT AutoCloseDialogEventHandler::Initialize(IFileDialog* file_dialog)
{
    ...
    CHECK(!event_hook_);
    event_hook_ = ::SetWinEventHook(EVENT_OBJECT_DESTROY, EVENT_OBJECT_DESTROY, nullptr, &EventHookCallback, process_id, thread_id, WINEVENT_OUTOFCONTEXT);
    if (!event_hook_) {
        return E_FAIL;
    }
...
// static
void CALLBACK AutoCloseDialogEventHandler::EventHookCallback(
    HWINEVENTHOOK handle, DWORD event, HWND hwnd, LONG id_object, LONG id_child, DWORD event_thread, DWORD event_time)
{
...
HRESULT AutoCloseDialogEventHandler::OnFolderChanging(IFileDialog*, IShellItem*)
{
    return E_NOTIMPL;
}
...
HRESULT AutoCloseDialogEventHandler::OnShareViolation(IFileDialog*, IShellItem*, FDE_SHAREVIOLATION_RESPONSE*)
{
    return E_NOTIMPL;
}
HRESULT AutoCloseDialogEventHandler::OnOverwrite(IFileDialog*, IShellItem*, FDE_OVERWRITE_RESPONSE*)
{
    return E_NOTIMPL;
}

// static
raw_ptr<AutoCloseDialogEventHandler> AutoCloseDialogEventHandler::instance_ = nullptr;
