...
BASE_FEATURE(kAutoCloseFileDialogs, "AutoCloseFileDialogs", base::FEATURE_ENABLED_BY_DEFAULT);
...
public:
        ...
        : file_dialog_(file_dialog)
    {
        ...
        Microsoft::WRL::ComPtr<IFileDialogEvents> dialog_event_handler = Microsoft::WRL::Make<AutoCloseDialogEventHandler>(owner_window);
...
bool SetDefaultPath(IFileDialog* file_dialog, const base::FilePath& default_path)
{
    ...
    if (IsDirectory(default_path)) {
        ...
        std::wstring sanitized = RemoveEnvVarFromFileName<wchar_t>(default_path.BaseName().value(), std::wstring(L"%"));
    ...
    if (SUCCEEDED(SHCreateItemFromParsingName(default_folder.value().c_str(), nullptr, IID_PPV_ARGS(&default_folder_shell_item)))) {
...
bool SetFilters(IFileDialog* file_dialog, const std::vector<FileFilterSpec>& filter, int filter_index)
{
    if (filter.empty())
        return true;
    ...
    return SUCCEEDED(file_dialog->SetFileTypes(comdlg_filterspec.size(), comdlg_filterspec.data())) && SUCCEEDED(file_dialog->SetFileTypeIndex(filter_index));
}
...
bool ConfigureDialog(IFileDialog* file_dialog, const std::u16string& title, const std::u16string& ok_button_label, const base::FilePath& default_path,
    const std::vector<FileFilterSpec>& filter, int filter_index, DWORD dialog_options)
{
    // Set title.
    if (!title.empty()) {
    ...
    return SetDefaultPath(file_dialog, default_path) && SetOptions(file_dialog, dialog_options) && SetFilters(file_dialog, filter, filter_index);
}
...
bool RunSaveFileDialog(HWND owner, const std::u16string& title, const base::FilePath& default_path, const std::vector<FileFilterSpec>& filter,
    DWORD dialog_options, const std::wstring& def_ext, int* filter_index, base::FilePath* path)
{
    Microsoft::WRL::ComPtr<IFileSaveDialog> file_save_dialog;
    if (FAILED(::CoCreateInstance(CLSID_FileSaveDialog, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&file_save_dialog)))) {
        return false;
    }

    if (!ConfigureDialog(file_save_dialog.Get(), title, std::u16string(), default_path, filter, *filter_index, dialog_options)) {
        return false;
    }
    ...
    auto auto_close_dialog_event_handler = std::make_unique<ScopedAutoCloseDialogEventHandler>(owner, file_save_dialog.Get());
    ...
    *filter_index = static_cast<int>(file_type_index);
    ...
    if (FAILED(result->GetDisplayName(SIGDN_DESKTOPABSOLUTEPARSING, &display_name))) {
        return false;
    }
...
// RunSaveFileDialog.
bool RunOpenFileDialog(HWND owner, const std::u16string& title, const std::u16string& ok_button_label, const base::FilePath& default_path,
    const std::vector<FileFilterSpec>& filter, DWORD dialog_options, int* filter_index, std::vector<base::FilePath>* paths)
{
    ...
    if (!ConfigureDialog(file_open_dialog.Get(), title, ok_button_label, default_path, filter, *filter_index, dialog_options)) {
        return false;
    }

    // This handler auto-closes the file dialog if its owner window is closed.
    auto auto_close_dialog_event_handler = std::make_unique<ScopedAutoCloseDialogEventHandler>(owner, file_open_dialog.Get());
    ...
    *filter_index = static_cast<int>(file_type_index);
    ...
    for (DWORD i = 0; i < result_count; ++i) {
        ...
        base::win::ScopedCoMem<wchar_t> display_name;
        if (FAILED(shell_item->GetDisplayName(SIGDN_DESKTOPABSOLUTEPARSING, &display_name))) {
            return false;
        }
...
bool ExecuteSelectFolder(
    HWND owner, SelectFileDialog::Type type, const std::u16string& title, const base::FilePath& default_path, std::vector<base::FilePath>* paths)
{
    DCHECK(paths);
    ...
    if (new_title.empty() && type == SelectFileDialog::SELECT_UPLOAD_FOLDER) {
        ...
        new_title = l10n_util::GetStringUTF16(IDS_SELECT_UPLOAD_FOLDER_DIALOG_TITLE);
    }

    std::u16string ok_button_label;
    if (type == SelectFileDialog::SELECT_UPLOAD_FOLDER) {
        ok_button_label = l10n_util::GetStringUTF16(IDS_SELECT_UPLOAD_FOLDER_DIALOG_UPLOAD_BUTTON);
    }
    ...
    return RunOpenFileDialog(owner, new_title, ok_button_label, default_path, no_filter, dialog_options, &filter_index, paths);
}

bool ExecuteSelectSingleFile(HWND owner, const std::u16string& title, const base::FilePath& default_path, const std::vector<FileFilterSpec>& filter,
    int* filter_index, std::vector<base::FilePath>* paths)
{
    return RunOpenFileDialog(owner, title, std::u16string(), default_path, filter, 0, filter_index, paths);
}

bool ExecuteSelectMultipleFile(HWND owner, const std::u16string& title, const base::FilePath& default_path, const std::vector<FileFilterSpec>& filter,
    int* filter_index, std::vector<base::FilePath>* paths)
{
    DWORD dialog_options = FOS_ALLOWMULTISELECT;
    return RunOpenFileDialog(owner, title, std::u16string(), default_path, filter, dialog_options, filter_index, paths);
}

bool ExecuteSaveFile(HWND owner, const std::u16string& title, const base::FilePath& default_path, const std::vector<FileFilterSpec>& filter,
    const std::wstring& def_ext, int* filter_index, base::FilePath* path)
{
    DCHECK(path);
    ...
    return RunSaveFileDialog(owner, title, default_path, filter, dialog_options, def_ext, filter_index, path);
}

} // namespace

void ExecuteSelectFile(SelectFileDialog::Type type, const std::u16string& title, const base::FilePath& default_path, const std::vector<FileFilterSpec>& filter,
    int file_type_index, const std::wstring& default_extension, HWND owner, OnSelectFileExecutedCallback on_select_file_executed_callback)
{
    ...
    case SelectFileDialog::SELECT_SAVEAS_FILE: {
        base::FilePath path;
        if (ExecuteSaveFile(owner, title, default_path, filter, default_extension, &file_type_index, &path)) {
            paths.push_back(std::move(path));
        }
        break;
    }
    case SelectFileDialog::SELECT_OPEN_FILE:
        ExecuteSelectSingleFile(owner, title, default_path, filter, &file_type_index, &paths);
        break;
    case SelectFileDialog::SELECT_OPEN_MULTI_FILE:
        ExecuteSelectMultipleFile(owner, title, default_path, filter, &file_type_index, &paths);
        break;
