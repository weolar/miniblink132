...
ScrollbarAnimationTimerMac::ScrollbarAnimationTimerMac(
    base::RepeatingCallback<void(double)> callback, double duration, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : start_time_(0.0)
    , duration_(duration)
    , callback_(std::move(callback))
{
    timing_function_ = gfx::CubicBezierTimingFunction::CreatePreset(gfx::CubicBezierTimingFunction::EaseType::EASE_IN_OUT);
}
...
void ScrollbarAnimationTimerMac::Start()
{
    ...
    timer_.Start(FROM_HERE, base::Seconds(1.0 / 60.0), this, &ScrollbarAnimationTimerMac::TimerFired);
}
...
OverlayScrollbarAnimatorMac::OverlayScrollbarAnimatorMac(
    Client* client, int thumb_width_expanded, int thumb_width_unexpanded, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : client_(client)
...
void OverlayScrollbarAnimatorMac::ExpandThumbAnimationStart()
{
    ...
    expand_thumb_animation_ = std::make_unique<ScrollbarAnimationTimerMac>(
        base::BindRepeating(&OverlayScrollbarAnimatorMac::ExpandThumbAnimationTicked, weak_factory_.GetWeakPtr()), kAnimationDurationSeconds, task_runner_);
    expand_thumb_animation_->Start();
}

void OverlayScrollbarAnimatorMac::ExpandThumbAnimationTicked(double progress)
{
    thumb_width_ = (1 - progress) * thumb_width_unexpanded_ + progress * thumb_width_expanded_;
    client_->SetThumbNeedsDisplay();
    if (progress == 1)
...
void OverlayScrollbarAnimatorMac::FadeInTrackAnimationStart()
{
    ...
    fade_in_track_animation_ = std::make_unique<ScrollbarAnimationTimerMac>(
        base::BindRepeating(&OverlayScrollbarAnimatorMac::FadeInTrackAnimationTicked, weak_factory_.GetWeakPtr()), kAnimationDurationSeconds, task_runner_);
...
void OverlayScrollbarAnimatorMac::FadeOutTimerUpdate()
{
    ...
    if (!start_scrollbar_fade_out_timer_) {
        start_scrollbar_fade_out_timer_ = std::make_unique<base::RetainingOneShotTimer>(
            FROM_HERE, kFadeOutDelay, base::BindRepeating(&OverlayScrollbarAnimatorMac::FadeOutAnimationStart, weak_factory_.GetWeakPtr()));
...
void OverlayScrollbarAnimatorMac::FadeOutAnimationStart()
{
    ...
    fade_out_animation_ = std::make_unique<ScrollbarAnimationTimerMac>(
        base::BindRepeating(&OverlayScrollbarAnimatorMac::FadeOutAnimationTicked, weak_factory_.GetWeakPtr()), kAnimationDurationSeconds, task_runner_);
...
const base::TimeDelta OverlayScrollbarAnimatorMac::kFadeOutDelay = base::Milliseconds(500);
