...
public:
    ScrollbarAnimationTimerMac(base::RepeatingCallback<void(double)> callback, double duration, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
...
public:
    ...
    OverlayScrollbarAnimatorMac(Client* client, int thumb_width_expanded, int thumb_width_unexpanded, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    float GetThumbAlpha() const
    {
        return thumb_alpha_;
    }
    float GetTrackAlpha() const
    {
        return track_alpha_;
    }
