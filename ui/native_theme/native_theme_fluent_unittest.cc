...
class NativeThemeFluentTest : public ::testing::Test, public ::testing::WithParamInterface<float> {
protected:
    ...
    void VerifyArrowRectIsCentered(const gfx::RectF& button_rect, const gfx::RectF& arrow_rect, NativeTheme::Part part) const
    {
        if (part == NativeTheme::kScrollbarUpArrow || part == NativeTheme::kScrollbarDownArrow) {
            ...
            EXPECT_NEAR(button_rect.CenterPoint().y(), arrow_rect.CenterPoint().y(), ScaleFromDIP() * 2);
        } else {
            EXPECT_EQ(button_rect.CenterPoint().y(), arrow_rect.CenterPoint().y());
            EXPECT_NEAR(button_rect.CenterPoint().x(), arrow_rect.CenterPoint().x(), ScaleFromDIP() * 2);
        }
    }
    ...
    void VerifyArrowRectLengthRatio(const gfx::RectF& button_rect, const gfx::RectF& arrow_rect, NativeTheme::State state) const
    {
        const int smaller_button_side = std::min(button_rect.width(), button_rect.height());
    ...
    {
        for (auto const& part : { NativeTheme::kScrollbarUpArrow, NativeTheme::kScrollbarLeftArrow }) {
            const gfx::RectF button_rect(ButtonRect(part));
            for (auto const& state : { NativeTheme::kNormal, NativeTheme::kPressed }) {
                const gfx::RectF arrow_rect = theme_.GetArrowRect(ToNearestRect(button_rect), part, state);
    ...
    {
        const int button_length = base::ClampFloor(kFluentScrollbarButtonSideLength * ScaleFromDIP());
        const int track_thickness = base::ClampFloor(kFluentScrollbarThickness * ScaleFromDIP());
...
TEST_F(NativeThemeFluentTest, PaintThumbRoundedCorners)
{
    ...
    EXPECT_EQ(canvas.ReleaseAsRecord().GetFirstOp().GetType(), cc::PaintOpType::kDrawRRect);
}
...
{
    const std::unique_ptr<ui::ColorProvider> color_provider = CreateDefaultColorProviderForBlink(/*dark_mode=*/false);
    NativeTheme::ScrollbarThumbExtraParams extra_params;
    const auto to_skcolor = [&](auto id) { return SkColor4f::FromColor(color_provider->GetColor(id)); };
    const auto scrollbar_color = [&](auto state) { return theme_.GetScrollbarThumbColor(*color_provider, state, extra_params); };

    const SkColor4f normal_thumb_color = to_skcolor(kColorWebNativeControlScrollbarThumb);
    const SkColor4f hovered_thumb_color = to_skcolor(kColorWebNativeControlScrollbarThumbHovered);
    const SkColor4f pressed_thumb_color = to_skcolor(kColorWebNativeControlScrollbarThumbPressed);
    const SkColor4f minimal_thumb_color = to_skcolor(kColorWebNativeControlScrollbarThumbOverlayMinimalMode);
...
INSTANTIATE_TEST_SUITE_P(All, NativeThemeFluentTest, ::testing::Values(1.f, 1.25f, 1.5f, 1.75f, 2.f));
