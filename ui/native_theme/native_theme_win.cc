...
void SetCheckerboardShader(SkPaint* paint, const RECT& align_rect)
{
    ...
    local_matrix.setTranslate(SkIntToScalar(align_rect.left), SkIntToScalar(align_rect.top));
    paint->setShader(bitmap.makeShader(SkTileMode::kRepeat, SkTileMode::kRepeat, SkSamplingOptions(), &local_matrix));
}
...
//
int ComputeAnimationProgress(int frame_width, int object_width, int pixels_per_second, double animated_seconds)
{
...
public:
    ...
    base::win::ScopedCreateDC::Handle Get() const
    {
        return dc_.Get();
    }
...
base::win::RegKey OpenColorFilteringRegKey(REGSAM access)
{
    ...
    // Validity is checked at time-of-use.
    std::ignore = hkcu_color_filtering_regkey.Open(HKEY_CURRENT_USER, L"Software\\Microsoft\\ColorFiltering", access);
...
{
    static bool system_supports_dark_mode = ([]() { return OpenThemeRegKey(KEY_READ).Valid(); })();
...
{
    static_cast<NativeThemeWin*>(NativeTheme::GetInstanceForNativeUi())->CloseHandlesInternal();
}

gfx::Size NativeThemeWin::GetPartSize(Part part, State state, const ExtraParams& extra) const
{
    ...
    if (handle && SUCCEEDED(GetThemePartSize(handle, screen_dc, part_id, state_id, nullptr, TS_TRUE, &size)))
...
void NativeThemeWin::Paint(cc::PaintCanvas* canvas, const ui::ColorProvider* color_provider, Part part, State state, const gfx::Rect& rect,
    const ExtraParams& extra, ColorScheme color_scheme, bool in_forced_colors, const std::optional<SkColor>& accent_color) const
{
    ...
    case kMenuPopupSeparator:
        PaintMenuSeparator(canvas, color_provider, absl::get<MenuSeparatorExtraParams>(extra));
        return;
    case kMenuPopupBackground:
        PaintMenuBackground(canvas, color_provider, rect);
        return;
    case kMenuItemBackground:
        CommonThemePaintMenuItemBackground(this, color_provider, canvas, state, rect, absl::get<MenuItemExtraParams>(extra));
        return;
    default:
...
NativeThemeWin::NativeThemeWin(bool configure_web_instance, bool should_only_use_dark_colors)
    ...
    , color_change_listener_(this)
{
    ...
    const bool observers_can_operate = base::SequencedTaskRunner::HasCurrentDefault();

    hkcu_themes_regkey_ = OpenThemeRegKey(KEY_READ | KEY_NOTIFY);
    if (hkcu_themes_regkey_.Valid()) {
        if (!should_only_use_dark_colors && !IsForcedDarkMode() && !IsForcedHighContrast()) {
    ...
    hkcu_color_filtering_regkey_ = OpenColorFilteringRegKey(KEY_READ | KEY_NOTIFY);
...
void NativeThemeWin::ConfigureWebInstance()
{
    ...
    // changes.
    color_scheme_observer_ = std::make_unique<NativeTheme::ColorSchemeNativeThemeObserver>(NativeTheme::GetInstanceForWeb());
    ...
    web_instance->set_prefers_reduced_transparency(GetPrefersReducedTransparency());
    web_instance->set_system_colors(GetSystemColors());
    web_instance->set_should_use_system_accent_color(should_use_system_accent_color());
}

std::optional<base::TimeDelta> NativeThemeWin::GetPlatformCaretBlinkInterval() const
{
    static const size_t system_value = ::GetCaretBlinkTime();
    if (system_value != 0) {
        return (system_value == INFINITE) ? base::TimeDelta() : base::Milliseconds(system_value);
    }
    return std::nullopt;
}
...
bool NativeThemeWin::IsUsingHighContrastThemeInternal() const
{
    ...
    return SystemParametersInfo(SPI_GETHIGHCONTRAST, result.cbSize, &result, 0) && (result.dwFlags & HCF_HIGHCONTRASTON) == HCF_HIGHCONTRASTON;
}
...
void NativeThemeWin::UpdateSystemColors()
{
    for (int sys_color : kSysColors)
        system_colors_[SysColorToSystemThemeColor(sys_color)] = color_utils::GetSysSkColor(sys_color);
}

void NativeThemeWin::PaintMenuSeparator(cc::PaintCanvas* canvas, const ColorProvider* color_provider, const MenuSeparatorExtraParams& params) const
{
...
void NativeThemeWin::PaintMenuGutter(cc::PaintCanvas* canvas, const ColorProvider* color_provider, const gfx::Rect& rect) const
{
    DCHECK(color_provider);
    cc::PaintFlags flags;
    flags.setColor(color_provider->GetColor(kColorMenuSeparator));
...
void NativeThemeWin::PaintMenuBackground(cc::PaintCanvas* canvas, const ColorProvider* color_provider, const gfx::Rect& rect) const
{
    DCHECK(color_provider);
    cc::PaintFlags flags;
...
void NativeThemeWin::PaintDirect(SkCanvas* destination_canvas, HDC hdc, Part part, State state, const gfx::Rect& rect, const ExtraParams& extra) const
{
    ...
    case kPushButton:
    case kRadio:
        PaintButtonClassic(hdc, part, state, &rect_win, absl::get<ButtonExtraParams>(extra));
        return;
    case kInnerSpinButton:
        DrawFrameControl(hdc, &rect_win, DFC_SCROLL, absl::get<InnerSpinButtonExtraParams>(extra).classic_state);
        return;
    case kMenuCheck: {
        const auto& menu_check = absl::get<MenuCheckExtraParams>(extra);
        PaintFrameControl(hdc, rect, DFC_MENU, menu_check.is_radio ? DFCS_MENUBULLET : DFCS_MENUCHECK, menu_check.is_selected, state);
        return;
    }
    case kMenuList:
        DrawFrameControl(hdc, &rect_win, DFC_SCROLL, DFCS_SCROLLCOMBOBOX | absl::get<MenuListExtraParams>(extra).classic_state);
        return;
    case kMenuPopupArrow: {
        ...
        PaintFrameControl(hdc, rect, DFC_MENU, menu_arrow.pointing_right ? DFCS_MENUARROW : DFCS_MENUARROWRIGHT, menu_arrow.is_selected, state);
        return;
    }
    case kProgressBar: {
        const auto& progress_bar = absl::get<ProgressBarExtraParams>(extra);
        RECT value_rect
            = gfx::Rect(progress_bar.value_rect_x, progress_bar.value_rect_y, progress_bar.value_rect_width, progress_bar.value_rect_height).ToRECT();
        if (handle) {
            PaintProgressBarOverlayThemed(hdc, handle, &rect_win, &value_rect, progress_bar);
        } else {
    ...
    case kScrollbarHorizontalTrack:
    case kScrollbarVerticalTrack:
        PaintScrollbarTrackClassic(destination_canvas, hdc, &rect_win, absl::get<ScrollbarTrackExtraParams>(extra));
        return;
    case kTabPanelBackground:
    ...
    case kTextField: {
        ...
        base::win::ScopedGDIObject<HBRUSH> bg_brush(CreateSolidBrush(skia::SkColorToCOLORREF(text_field.background_color)));
        if (handle) {
            PaintTextFieldThemed(hdc, handle, bg_brush.get(), part_id, state_id, &rect_win, text_field);
        } else {
    ...
    case kTrackbarThumb: {
        ...
        if (trackbar.vertical) {
            ...
            PaintHorizontalTrackbarThumbClassic(destination_canvas, hdc, rect_win, trackbar);
        }
        return;
    }
    case kTrackbarTrack:
...
NativeTheme::PreferredColorScheme NativeThemeWin::CalculatePreferredColorScheme() const
{
...
NativeTheme::PreferredContrast NativeThemeWin::CalculatePreferredContrast() const
{
    if (!InForcedColorsMode())
    ...
    return contrast_ratio <= 2.5 ? NativeTheme::PreferredContrast::kLess : NativeTheme::PreferredContrast::kCustom;
}

NativeTheme::ColorScheme NativeThemeWin::GetDefaultSystemColorScheme() const
{
    return InForcedColorsMode() ? ColorScheme::kPlatformHighContrast : NativeTheme::GetDefaultSystemColorScheme();
}

void NativeThemeWin::PaintIndirect(cc::PaintCanvas* destination_canvas, Part part, State state, const gfx::Rect& rect, const ExtraParams& extra) const
{
    ...
    if ((SelectClipRgn(offscreen_hdc.Get(), clip) == ERROR) || !DeleteObject(clip)) {
        return;
    }

    base::win::ScopedBitmap hbitmap = skia::CreateHBitmapXRGB8888(rect.width(), rect.height(), nullptr, nullptr);
    ...
    sk_sp<SkSurface> offscreen_surface = skia::MapPlatformSurface(offscreen_hdc.Get());
    ...
    case kScrollbarHorizontalTrack:
    case kScrollbarVerticalTrack: {
        auto scrollbar_track = absl::get<ScrollbarTrackExtraParams>(adjusted_extra);
    ...
    PaintDirect(offscreen_canvas, offscreen_hdc.Get(), part, state, adjusted_rect, adjusted_extra);
    ...
    for (int i = 0; i < pixel_count; i++) {
        if (pixels[i] == placeholder_value) {
            ...
            pixels[i] = SkPackARGB32(0xFF, SkGetPackedR32(pixels[i]), SkGetPackedG32(pixels[i]), SkGetPackedB32(pixels[i]));
        }
    }

    destination_canvas->drawImage(cc::PaintImage::CreateFromBitmap(std::move(offscreen_bitmap)), rect.x(), rect.y());
}

void NativeThemeWin::PaintButtonClassic(HDC hdc, Part part, State state, RECT* rect, const ButtonExtraParams& extra) const
{
...
void NativeThemeWin::PaintLeftMenuArrowThemed(HDC hdc, HANDLE handle, int part_id, int state_id, const gfx::Rect& rect) const
{
    ...
    base::win::ScopedBitmap mem_bitmap(CreateCompatibleBitmap(hdc, rect.width(), rect.height()));
    ...
    StretchBlt(mem_dc.Get(), 0, 0, rect.width(), rect.height(), hdc, rect.right() - 1, rect.y(), -rect.width(), rect.height(), SRCCOPY);
    // Draw the arrow.
    RECT theme_rect = { 0, 0, rect.width(), rect.height() };
    DrawThemeBackground(handle, mem_dc.Get(), part_id, state_id, &theme_rect, nullptr);
    // Copy and mirror the result back into mem_dc.
    StretchBlt(hdc, rect.x(), rect.y(), rect.width(), rect.height(), mem_dc.Get(), rect.width() - 1, 0, -rect.width(), rect.height(), SRCCOPY);
}

void NativeThemeWin::PaintScrollbarArrowClassic(HDC hdc, Part part, State state, RECT* rect) const
{
...
void NativeThemeWin::PaintScrollbarTrackClassic(SkCanvas* canvas, HDC hdc, RECT* rect, const ScrollbarTrackExtraParams& extra) const
{
    if ((system_colors_[SystemThemeColor::kScrollbar] != system_colors_[SystemThemeColor::kButtonFace])
        && (system_colors_[SystemThemeColor::kScrollbar] != system_colors_[SystemThemeColor::kWindow])) {
        FillRect(hdc, rect, reinterpret_cast<HBRUSH>(COLOR_SCROLLBAR + 1));
    } else {
        SkPaint paint;
        RECT align_rect = gfx::Rect(extra.track_x, extra.track_y, extra.track_width, extra.track_height).ToRECT();
...
void NativeThemeWin::PaintHorizontalTrackbarThumbClassic(SkCanvas* canvas, HDC hdc, const RECT& rect, const TrackbarExtraParams& extra) const
{
    ...
    DrawEdge(hdc, &top_section, EDGE_RAISED, BF_LEFT | BF_TOP | BF_RIGHT | BF_SOFT | BF_MIDDLE | BF_ADJUST);
    ...
    DrawEdge(hdc, &left_half, EDGE_RAISED, BF_DIAGONAL_ENDTOPLEFT | BF_SOFT | BF_MIDDLE | BF_ADJUST);
    DrawEdge(hdc, &right_half, EDGE_RAISED, BF_DIAGONAL_ENDBOTTOMLEFT | BF_SOFT | BF_MIDDLE | BF_ADJUST);
...
void NativeThemeWin::PaintProgressBarOverlayThemed(HDC hdc, HANDLE handle, RECT* bar_rect, RECT* value_rect, const ProgressBarExtraParams& extra) const
{
    ...
    if (!extra.determinate) {
        ...
        overlay_rect.left += ComputeAnimationProgress(width_with_margin, overlay_width, kIndeterminateOverlayPixelsPerSecond, extra.animated_seconds);
        overlay_rect.right = overlay_rect.left + overlay_width;
        DrawThemeBackground(handle, hdc, PP_MOVEOVERLAY, 0, &overlay_rect, bar_rect);
        return;
    }
    ...
    const bool mirror = bar_rect->right == value_rect->right && bar_rect->left != value_rect->left;
    const DTBGOPTS value_draw_options = { sizeof(DTBGOPTS), mirror ? DTBG_MIRRORDC : 0u, *bar_rect };
    ...
    DrawThemeBackgroundEx(handle, hdc, PP_FILL, 0, value_rect, &value_draw_options);

    RECT overlay_rect = *value_rect;
    overlay_rect.left += ComputeAnimationProgress(bar_width, kDeterminateOverlayWidth, kDeterminateOverlayPixelsPerSecond, extra.animated_seconds);
    overlay_rect.right = overlay_rect.left + kDeterminateOverlayWidth;
    DrawThemeBackground(handle, hdc, PP_MOVEOVERLAY, 0, &overlay_rect, value_rect);
}

void NativeThemeWin::PaintTextFieldThemed(
    HDC hdc, HANDLE handle, HBRUSH bg_brush, int part_id, int state_id, RECT* rect, const TextFieldExtraParams& extra) const
{
    static constexpr DTBGOPTS kOmitBorderOptions = { sizeof(DTBGOPTS), DTBG_OMITBORDER, { 0, 0, 0, 0 } };
    DrawThemeBackgroundEx(handle, hdc, part_id, state_id, rect, extra.draw_edges ? nullptr : &kOmitBorderOptions);

    if (extra.fill_content_area) {
        RECT content_rect;
        GetThemeBackgroundContentRect(handle, hdc, part_id, state_id, rect, &content_rect);
        FillRect(hdc, &content_rect, bg_brush);
    }
}

void NativeThemeWin::PaintTextFieldClassic(HDC hdc, HBRUSH bg_brush, RECT* rect, const TextFieldExtraParams& extra) const
{
    if (extra.draw_edges)
...
void NativeThemeWin::PaintScaledTheme(HANDLE theme, HDC hdc, int part_id, int state_id, const gfx::Rect& rect) const
{
...
// static
int NativeThemeWin::GetWindowsPart(Part part, State state, const ExtraParams& extra)
{
    switch (part) {
    case kCheckbox:
        return BP_CHECKBOX;
    ...
    case kScrollbarHorizontalTrack:
        return absl::get<ScrollbarTrackExtraParams>(extra).is_upper ? SBP_UPPERTRACKHORZ : SBP_LOWERTRACKHORZ;
    case kScrollbarVerticalTrack:
        return absl::get<ScrollbarTrackExtraParams>(extra).is_upper ? SBP_UPPERTRACKVERT : SBP_LOWERTRACKVERT;
    case kWindowResizeGripper:
        ...
        return SP_GRIPPER;
    case kInnerSpinButton:
        return absl::get<InnerSpinButtonExtraParams>(extra).spin_up ? SPNP_UP : SPNP_DOWN;
    case kTabPanelBackground:
        return TABP_BODY;
    case kTrackbarThumb:
        return absl::get<TrackbarExtraParams>(extra).vertical ? TKP_THUMBVERT : TKP_THUMBBOTTOM;
    case kTrackbarTrack:
        return absl::get<TrackbarExtraParams>(extra).vertical ? TKP_TRACKVERT : TKP_TRACK;
    case kMenuPopupBackground:
    case kMenuItemBackground:
    case kScrollbarCorner:
    case kSliderTrack:
...
int NativeThemeWin::GetWindowsState(Part part, State state, const ExtraParams& extra)
{
    switch (part) {
    case kScrollbarDownArrow:
        switch (state) {
        ...
        case kHovered:
            return absl::get<ScrollbarArrowExtraParams>(extra).is_hovering ? ABS_DOWNHOVER : ABS_DOWNHOT;
        case kNormal:
    ...
    case kScrollbarLeftArrow:
        ...
        case kHovered:
            return absl::get<ScrollbarArrowExtraParams>(extra).is_hovering ? ABS_LEFTHOVER : ABS_LEFTHOT;
        case kNormal:
    ...
    case kScrollbarRightArrow:
        ...
        case kHovered:
            return absl::get<ScrollbarArrowExtraParams>(extra).is_hovering ? ABS_RIGHTHOVER : ABS_RIGHTHOT;
        case kNormal:
    ...
    case kScrollbarUpArrow:
        ...
        case kHovered:
            return absl::get<ScrollbarArrowExtraParams>(extra).is_hovering ? ABS_UPHOVER : ABS_UPHOT;
        case kNormal:
            return ABS_UPNORMAL;
        case kPressed:
    ...
    case kCheckbox: {
        ...
        switch (state) {
        case kDisabled:
            return button.checked ? CBS_CHECKEDDISABLED : (button.indeterminate ? CBS_MIXEDDISABLED : CBS_UNCHECKEDDISABLED);
        case kHovered:
            return button.checked ? CBS_CHECKEDHOT : (button.indeterminate ? CBS_MIXEDHOT : CBS_UNCHECKEDHOT);
        case kNormal:
            return button.checked ? CBS_CHECKEDNORMAL : (button.indeterminate ? CBS_MIXEDNORMAL : CBS_UNCHECKEDNORMAL);
        case kPressed:
            return button.checked ? CBS_CHECKEDPRESSED : (button.indeterminate ? CBS_MIXEDPRESSED : CBS_UNCHECKEDPRESSED);
        case kNumStates:
            NOTREACHED();
        }
    }
    ...
    case kPushButton:
        ...
        case kHovered:
            return PBS_HOT;
        case kNormal:
            return absl::get<ButtonExtraParams>(extra).is_default ? PBS_DEFAULTED : PBS_NORMAL;
        case kPressed:
            return PBS_PRESSED;
        case kNumStates:
            NOTREACHED();
    ...
    case kScrollbarHorizontalThumb:
    case kScrollbarVerticalThumb:
        if ((state == kHovered) && !absl::get<ScrollbarThumbExtraParams>(extra).is_hovering) {
            return SCRBS_HOT;
        }
        [[fallthrough]];
    case kScrollbarHorizontalTrack:
    ...
    case kInnerSpinButton: {
        ...
        switch (state) {
        case kDisabled:
            return inner_spin.spin_up ? static_cast<int>(UPS_DISABLED) : static_cast<int>(DNS_DISABLED);
        case kHovered:
            return inner_spin.spin_up ? static_cast<int>(UPS_HOT) : static_cast<int>(DNS_HOT);
        case kNormal:
            return inner_spin.spin_up ? static_cast<int>(UPS_NORMAL) : static_cast<int>(DNS_NORMAL);
        case kPressed:
            return inner_spin.spin_up ? static_cast<int>(UPS_PRESSED) : static_cast<int>(DNS_PRESSED);
        case kNumStates:
            NOTREACHED();
        }
    }
...
HRESULT NativeThemeWin::PaintFrameControl(HDC hdc, const gfx::Rect& rect, UINT type, UINT state, bool is_selected, State control_state) const
{
    ...
    base::win::ScopedSelectObject select_bitmap(bitmap_dc.Get(), mask_bitmap.get());
    ...
    BitBlt(hdc, rect.x(), rect.y(), width, height, bitmap_dc.Get(), 0, 0, SRCCOPY);
...
HANDLE NativeThemeWin::GetThemeHandle(ThemeName theme_name) const
{
    ...
    case MENULIST:
        ...
        break;
    case SCROLLBAR:
        handle = OpenThemeData(nullptr, supports_windows_dark_mode_ ? L"Explorer::Scrollbar" : L"Scrollbar");
        break;
    case STATUS:
