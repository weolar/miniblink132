...
namespace {
static constexpr base::TimeDelta kDefaultCaretBlinkInterval = base::Milliseconds(500);
}

NativeTheme::MenuListExtraParams::MenuListExtraParams() = default;
NativeTheme::TextFieldExtraParams::TextFieldExtraParams() = default;

NativeTheme::MenuListExtraParams::MenuListExtraParams(const NativeTheme::MenuListExtraParams&) = default;

NativeTheme::TextFieldExtraParams::TextFieldExtraParams(const NativeTheme::TextFieldExtraParams&) = default;

NativeTheme::MenuListExtraParams& NativeTheme::MenuListExtraParams::operator=(const NativeTheme::MenuListExtraParams&) = default;
NativeTheme::TextFieldExtraParams& NativeTheme::TextFieldExtraParams::operator=(const NativeTheme::TextFieldExtraParams&) = default;
...
ColorProviderKey NativeTheme::GetColorProviderKey(scoped_refptr<ColorProviderKey::ThemeInitializerSupplier> custom_theme, bool use_custom_frame) const
{
    const auto get_forced_colors_key = [](bool forced_colors, PageColors page_colors) {
        if (!forced_colors) {
            return ColorProviderKey::ForcedColors::kNone;
        }
        static constexpr auto kForcedColorsMap = base::MakeFixedFlatMap<PageColors, ColorProviderKey::ForcedColors>({
            ...
            { PageColors::kHighContrast, ColorProviderKey::ForcedColors::kActive },
    ...
    case ColorScheme::kPlatformHighContrast:
        key.color_mode = GetPreferredColorScheme() == PreferredColorScheme::kDark ? ColorProviderKey::ColorMode::kDark : ColorProviderKey::ColorMode::kLight;
        break;
    default:
        NOTREACHED();
    }
    key.contrast_mode = UserHasContrastPreference() ? ColorProviderKey::ContrastMode::kHigh : ColorProviderKey::ContrastMode::kNormal;
    key.forced_colors = get_forced_colors_key(InForcedColorsMode(), page_colors_);
    key.system_theme = system_theme_;
    key.frame_type = use_custom_frame ? ColorProviderKey::FrameType::kChromium : ColorProviderKey::FrameType::kNative;
    key.user_color_source = should_use_system_accent_color_ ? ColorProviderKey::UserColorSource::kAccent : ColorProviderKey::UserColorSource::kBaseline;
...
float NativeTheme::GetBorderRadiusForPart(Part part, float width, float height) const
{
    return 0;
}
...
//     const size_t initial_providers_initialized = color_provider_manager.num_providers_initialized();
// 
...
//     native_theme_observers_.Notify(&NativeThemeObserver::OnNativeThemeUpdated, this);
// 
//     RecordNumColorProvidersInitializedDuringOnNativeThemeUpdated(color_provider_manager.num_providers_initialized() - initial_providers_initialized);
...
void NativeTheme::NotifyOnPreferredContrastUpdated()
{
    ...
    // listener, and those events may be delivered on unexpected sequences.
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    native_theme_observers_.Notify(&NativeThemeObserver::OnPreferredContrastChanged);
}

float NativeTheme::AdjustBorderWidthByZoom(float border_width, float zoom_level) const
{
    float zoomed = floorf(border_width * zoom_level);
    return std::max(1.0f, zoomed);
}

float NativeTheme::AdjustBorderRadiusByZoom(Part part, float border_radius, float zoom) const
{
...
base::TimeDelta NativeTheme::GetCaretBlinkInterval() const
{
    ...
    std::optional<base::TimeDelta> platform_interval = GetPlatformCaretBlinkInterval();
...
NativeTheme::NativeTheme(bool should_use_dark_colors, ui::SystemTheme system_theme)
...
{
    return GetPreferredContrast() != NativeTheme::PreferredContrast::kNoPreference;
}
...
NativeTheme::PlatformHighContrastColorScheme NativeTheme::GetPlatformHighContrastColorScheme() const
{
    if (GetDefaultSystemColorScheme() != ColorScheme::kPlatformHighContrast)
        return PlatformHighContrastColorScheme::kNone;
    return (GetPreferredColorScheme() == PreferredColorScheme::kDark) ? PlatformHighContrastColorScheme::kDark : PlatformHighContrastColorScheme::kLight;
}
...
NativeTheme::PreferredColorScheme NativeTheme::CalculatePreferredColorScheme() const
{
...
bool NativeTheme::CalculateUseOverlayScrollbar()
{
    ...
    use_overlay_scrollbar = use_overlay_scrollbar || features::IsOverlayScrollbarOSSettingEnabled();
#endif
    return use_overlay_scrollbar;
}

std::optional<base::TimeDelta> NativeTheme::GetPlatformCaretBlinkInterval() const
{
    return std::nullopt;
}
...
void NativeTheme::SetPreferredContrast(NativeTheme::PreferredContrast preferred_contrast)
{
...
{
    return false;
//     static bool kIsForcedDarkMode = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kForceDarkMode);
...
{
    return false;
//     static bool kIsForcedHighContrast = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kForceHighContrast);
//     return kIsForcedHighContrast;
}

NativeTheme::PreferredContrast NativeTheme::CalculatePreferredContrast() const
{
    return IsForcedHighContrast() ? PreferredContrast::kMore : PreferredContrast::kNoPreference;
}
...
const std::map<NativeTheme::SystemThemeColor, SkColor>& NativeTheme::GetSystemColors() const
{
    return system_colors_;
}

std::optional<SkColor> NativeTheme::GetSystemThemeColor(SystemThemeColor theme_color) const
{
...
bool NativeTheme::HasDifferentSystemColors(const std::map<NativeTheme::SystemThemeColor, SkColor>& colors) const
{
    return system_colors_ != colors;
}

void NativeTheme::set_system_colors(const std::map<NativeTheme::SystemThemeColor, SkColor>& colors)
{
    system_colors_ = colors;
}

NativeTheme::ColorSchemeNativeThemeObserver::ColorSchemeNativeThemeObserver(NativeTheme* theme_to_update)
    : theme_to_update_(theme_to_update)
{
}

NativeTheme::ColorSchemeNativeThemeObserver::~ColorSchemeNativeThemeObserver() = default;

void NativeTheme::ColorSchemeNativeThemeObserver::OnNativeThemeUpdated(ui::NativeTheme* observed_theme)
{
    const bool should_use_dark_colors = observed_theme->ShouldUseDarkColors();
    const PreferredColorScheme preferred_color_scheme = observed_theme->GetPreferredColorScheme();
    const bool inverted_colors = observed_theme->GetInvertedColors();
    const base::TimeDelta caret_blink_interval = observed_theme->GetCaretBlinkInterval();
    ...
    notify_observers |= theme_to_update_->UpdateContrastRelatedStates(*observed_theme);

    if (notify_observers) {
        DCHECK(theme_to_update_->UserHasContrastPreference() || !theme_to_update_->InForcedColorsMode());
...
bool NativeTheme::UpdateContrastRelatedStates(const NativeTheme& observed_theme)
{
    bool forced_colors = observed_theme.InForcedColorsMode();
    PageColors page_colors = observed_theme.GetPageColors();
    bool prefers_reduced_transparency = observed_theme.GetPrefersReducedTransparency();
    PreferredContrast preferred_contrast = observed_theme.GetPreferredContrast();
    bool states_updated = false;

    const auto default_page_colors = forced_colors ? PageColors::kHighContrast : PageColors::kOff;
    ...
    if (GetPageColors() != page_colors) {
        ...
        if (page_colors != PageColors::kOff && page_colors != PageColors::kHighContrast) {
            bool is_dark_color = page_colors == PageColors::kNightSky || page_colors == PageColors::kDusk || page_colors == PageColors::kAquatic;
            PreferredColorScheme page_colors_theme_scheme = is_dark_color ? PreferredColorScheme::kDark : PreferredColorScheme::kLight;
...
SkColor4f NativeTheme::GetScrollbarThumbColor(const ui::ColorProvider& color_provider, State state, const ScrollbarThumbExtraParams& extra_params) const
{
