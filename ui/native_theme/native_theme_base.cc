...
constexpr SkColor kThumbActiveColor[2] = { SkColorSetRGB(0xF4, 0xF4, 0xF4), gfx::kPlaceholderColor };
constexpr SkColor kThumbInactiveColor[2] = { SkColorSetRGB(0xEA, 0xEA, 0xEA), SK_ColorWHITE };
constexpr SkColor kTrackColor[2] = { SkColorSetRGB(0xD3, 0xD3, 0xD3), gfx::kPlaceholderColor };
...
constexpr SkColor kMenuPopupBackgroundColor[2] = { SkColorSetRGB(210, 225, 246), gfx::kPlaceholderColor };
...
SkColor GetColor(const SkColor colors[2], ui::NativeTheme::ColorScheme color_scheme)
{
...
SkColor CustomAccentColorForState(const SkColor& accent_color, ui::NativeTheme::State state, const ui::NativeTheme::ColorScheme& color_scheme)
{
    ...
    return AdjustLuminance(accent_color, (make_lighter ? 1 : -1) * kAccentLuminanceAdjust);
}
...
gfx::Size NativeThemeBase::GetPartSize(Part part, State state, const ExtraParams& extra) const
{
    switch (part) {
...
float NativeThemeBase::GetBorderRadiusForPart(Part part, float width, float height) const
{
    switch (part) {
    case kCheckbox:
        return 2.f;
...
void NativeThemeBase::Paint(cc::PaintCanvas* canvas, const ui::ColorProvider* color_provider, Part part, State state, const gfx::Rect& rect,
    const ExtraParams& extra, ColorScheme color_scheme, bool in_forced_colors, const std::optional<SkColor>& accent_color) const
{
    if (rect.IsEmpty())
        return;
    ...
    // Please keep these in the order of NativeTheme::Part.
    case kCheckbox:
        PaintCheckbox(canvas, color_provider, state, rect, absl::get<ButtonExtraParams>(extra), color_scheme, accent_color_opaque);
        break;
#if BUILDFLAG(IS_LINUX)
    case kFrameTopArea:
        PaintFrameTopArea(canvas, state, rect, absl::get<FrameTopAreaExtraParams>(extra), color_scheme);
        break;
#endif
    case kInnerSpinButton:
        PaintInnerSpinButton(canvas, color_provider, state, rect, absl::get<InnerSpinButtonExtraParams>(extra), color_scheme, in_forced_colors);
        break;
    case kMenuList:
        PaintMenuList(canvas, color_provider, state, rect, absl::get<MenuListExtraParams>(extra), color_scheme);
        break;
    case kMenuPopupBackground:
        PaintMenuPopupBackground(canvas, color_provider, rect.size(), absl::get<MenuBackgroundExtraParams>(extra), color_scheme);
        break;
    case kMenuPopupSeparator:
        PaintMenuSeparator(canvas, color_provider, state, rect, absl::get<MenuSeparatorExtraParams>(extra));
        break;
    case kMenuItemBackground:
        PaintMenuItemBackground(canvas, color_provider, state, rect, absl::get<MenuItemExtraParams>(extra), color_scheme);
        break;
    case kProgressBar:
        PaintProgressBar(canvas, color_provider, state, rect, absl::get<ProgressBarExtraParams>(extra), color_scheme, accent_color_opaque);
        break;
    case kPushButton:
        PaintButton(canvas, color_provider, state, rect, absl::get<ButtonExtraParams>(extra), color_scheme);
        break;
    case kRadio:
        PaintRadio(canvas, color_provider, state, rect, absl::get<ButtonExtraParams>(extra), color_scheme, accent_color_opaque);
        break;
    case kScrollbarDownArrow:
    case kScrollbarUpArrow:
    case kScrollbarLeftArrow:
    case kScrollbarRightArrow:
        if (scrollbar_button_length_ > 0)
            PaintArrowButton(canvas, color_provider, rect, part, state, color_scheme, in_forced_colors, absl::get<ScrollbarArrowExtraParams>(extra));
        break;
    case kScrollbarHorizontalThumb:
    case kScrollbarVerticalThumb:
        PaintScrollbarThumb(canvas, color_provider, part, state, rect, absl::get<ScrollbarThumbExtraParams>(extra), color_scheme);
        break;
    case kScrollbarHorizontalTrack:
    case kScrollbarVerticalTrack:
        PaintScrollbarTrack(canvas, color_provider, part, state, absl::get<ScrollbarTrackExtraParams>(extra), rect, color_scheme, in_forced_colors);
        break;
    case kScrollbarHorizontalGripper:
    case kScrollbarVerticalGripper:
    ...
    case kScrollbarCorner:
        PaintScrollbarCorner(canvas, color_provider, state, rect, absl::get<ScrollbarTrackExtraParams>(extra), color_scheme);
        break;
    case kSliderTrack:
        PaintSliderTrack(canvas, color_provider, state, rect, absl::get<SliderExtraParams>(extra), color_scheme, accent_color_opaque);
        break;
    case kSliderThumb:
        PaintSliderThumb(canvas, color_provider, state, rect, absl::get<SliderExtraParams>(extra), color_scheme, accent_color_opaque);
        break;
    case kTabPanelBackground:
        NOTIMPLEMENTED();
        break;
    case kTextField:
        PaintTextField(canvas, color_provider, state, rect, absl::get<TextFieldExtraParams>(extra), color_scheme);
        break;
    case kTrackbarThumb:
    case kTrackbarTrack:
    case kWindowResizeGripper:
...
NativeThemeBase::NativeThemeBase()
    : NativeThemeBase(false)
{
}

NativeThemeBase::NativeThemeBase(bool should_only_use_dark_colors, ui::SystemTheme system_theme)
...
void NativeThemeBase::PaintArrowButton(cc::PaintCanvas* canvas, const ColorProvider* color_provider, const gfx::Rect& rect, Part direction, State state,
    ColorScheme color_scheme, bool in_forced_colors, const ScrollbarArrowExtraParams& extra_params) const
{
    cc::PaintFlags flags;

    // Calculate button color.
    SkScalar track_hsv[3];
    SkColorToHSV(extra_params.track_color.value_or(GetColor(kTrackColor, color_scheme)), track_hsv);
    ...
    const SkColor arrow_color = extra_params.thumb_color.has_value() ? extra_params.thumb_color.value() : GetArrowColor(state, color_scheme, color_provider);
    PaintArrow(canvas, rect, direction, arrow_color);
}

void NativeThemeBase::PaintArrow(cc::PaintCanvas* gc, const gfx::Rect& rect, Part direction, SkColor color) const
{
    cc::PaintFlags flags;
...
SkPath NativeThemeBase::PathForArrow(const gfx::Rect& bounding_rect, Part direction) const
{
...
{
    const std::pair<int, int> rect_sides = std::minmax(rect.width(), rect.height());
    const int side_length_inset = 2 * std::ceil(rect_sides.second / 4.f);
    const int side_length = std::min(rect_sides.first, rect_sides.second - side_length_inset);
    // When there are an odd number of pixels, put the extra on the top/left.
    return gfx::Rect(rect.x() + (rect.width() - side_length + 1) / 2, rect.y() + (rect.height() - side_length + 1) / 2, side_length, side_length);
}

void NativeThemeBase::PaintScrollbarTrack(cc::PaintCanvas* canvas, const ColorProvider* color_provider, Part part, State state,
    const ScrollbarTrackExtraParams& extra_params, const gfx::Rect& rect, ColorScheme color_scheme, bool in_forced_colors) const
{
    cc::PaintFlags flags;
    SkIRect skrect;
...
void NativeThemeBase::PaintScrollbarThumb(cc::PaintCanvas* canvas, const ColorProvider* color_provider, Part part, State state, const gfx::Rect& rect,
    const ScrollbarThumbExtraParams& extra_params, ColorScheme color_scheme) const
{
    ...
    SkScalar thumb[3];
    SkColorToHSV(GetColor(hovered ? kThumbActiveColor : kThumbInactiveColor, color_scheme), thumb);
    ...
    if (vertical) {
        skrect.setLTRB(midx + 1, rect.y(), rect.x() + rect.width(), rect.y() + rect.height());
    } else {
        skrect.setLTRB(rect.x(), midy + 1, rect.x() + rect.width(), rect.y() + rect.height());
    }
    ...
    if (rect.height() > 10 && rect.width() > 10) {
        ...
        if (vertical) {
            DrawHorizLine(canvas, midx - grippy_half_width, midx + grippy_half_width, midy - inter_grippy_offset, flags);
            DrawHorizLine(canvas, midx - grippy_half_width, midx + grippy_half_width, midy, flags);
            DrawHorizLine(canvas, midx - grippy_half_width, midx + grippy_half_width, midy + inter_grippy_offset, flags);
        } else {
            DrawVertLine(canvas, midx - inter_grippy_offset, midy - grippy_half_width, midy + grippy_half_width, flags);
            DrawVertLine(canvas, midx, midy - grippy_half_width, midy + grippy_half_width, flags);
            DrawVertLine(canvas, midx + inter_grippy_offset, midy - grippy_half_width, midy + grippy_half_width, flags);
        }
    }
}

void NativeThemeBase::PaintScrollbarCorner(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const ScrollbarTrackExtraParams& extra_params, ColorScheme color_scheme) const
{
}

void NativeThemeBase::PaintCheckbox(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const ButtonExtraParams& button, ColorScheme color_scheme, const std::optional<SkColor>& accent_color) const
{
    const float border_radius = GetBorderRadiusForPart(kCheckbox, rect.width(), rect.height());

    SkRect skrect = PaintCheckboxRadioCommon(canvas, color_provider, state, rect, button, true, border_radius, color_scheme, accent_color);
    ...
    if (!skrect.isEmpty()) {
        ...
        if (button.indeterminate || button.checked) {
            ...
            if (accent_color && state != kDisabled) {
                flags.setColor(CustomAccentColorForState(*accent_color, state, color_scheme));
            } else {
                flags.setColor(ControlsAccentColorForState(state, color_scheme, color_provider));
            }
            canvas->drawRoundRect(skrect, border_radius, border_radius, flags);

            if (button.indeterminate) {
                // Draw the dash.
                flags.setColor(ControlsBackgroundColorForState(state, color_scheme, color_provider));
                skrect.inset(skrect.width() * kIndeterminateInsetWidthRatio, skrect.height() * kIndeterminateInsetHeightRatio);
                canvas->drawRoundRect(skrect, border_radius, border_radius, flags);
                ...
                check.lineTo(skrect.right() - skrect.width() * 0.2, skrect.y() + skrect.height() * 0.2);
                flags.setStyle(cc::PaintFlags::kStroke_Style);
                flags.setStrokeWidth(SkFloatToScalar(skrect.height() * 0.16));
                SkColor checkmark_color = ControlsBackgroundColorForState(state, color_scheme, color_provider);
...
SkRect NativeThemeBase::PaintCheckboxRadioCommon(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const ButtonExtraParams& button, bool is_checkbox, const SkScalar border_radius, ColorScheme color_scheme, const std::optional<SkColor>& accent_color) const
{
    ...
    const auto background_rect = skrect.makeInset(kBorderWidth * 0.2f, kBorderWidth * 0.2f);
    PaintLightenLayer(canvas, color_provider, background_rect, state, border_radius, color_scheme);
    flags.setColor(ControlsBackgroundColorForState(state, color_scheme, color_provider));
    flags.setStyle(cc::PaintFlags::kFill_Style);
    ...
    if (!is_checkbox || (!button.checked && !button.indeterminate)) {
        ...
        const auto border_rect = skrect.makeInset(kBorderWidth / 2, kBorderWidth / 2);

        SkColor border_color;
        if (button.checked) {
            if (accent_color && state != kDisabled) {
                border_color = CustomAccentColorForState(*accent_color, state, color_scheme);
            } else {
                border_color = ControlsAccentColorForState(state, color_scheme, color_provider);
            }
        } else {
            border_color = ControlsBorderColorForState(state, color_scheme, color_provider);
        }
...
void NativeThemeBase::PaintRadio(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const ButtonExtraParams& button, ColorScheme color_scheme, const std::optional<SkColor>& accent_color) const
{
    // Most of a radio button is the same as a checkbox, except the the rounded
    // square is a circle (i.e. border radius >= 100%).
    const float border_radius = GetBorderRadiusForPart(kRadio, rect.width(), rect.height());
    SkRect skrect = PaintCheckboxRadioCommon(canvas, color_provider, state, rect, button, false, border_radius, color_scheme, accent_color);
...
void NativeThemeBase::PaintButton(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const ButtonExtraParams& button, ColorScheme color_scheme) const
{
    cc::PaintFlags flags;
    SkRect skrect = gfx::RectToSkRect(rect);
    ...
    float border_radius = GetBorderRadiusForPart(kPushButton, rect.width(), rect.height());
    border_radius = AdjustBorderRadiusByZoom(kPushButton, border_radius, button.zoom);
    // Paint the background (is not visible behind the rounded corners).
    skrect.inset(border_width / 2, border_width / 2);
    PaintLightenLayer(canvas, color_provider, skrect, state, border_radius, color_scheme);
    flags.setColor(ButtonFillColorForState(state, color_scheme, color_provider));
    canvas->drawRoundRect(skrect, border_radius, border_radius, flags);
    ...
    if (button.has_border) {
        ...
        flags.setColor(ButtonBorderColorForState(state, color_scheme, color_provider));
        canvas->drawRoundRect(skrect, border_radius, border_radius, flags);
    }
}

void NativeThemeBase::PaintTextField(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const TextFieldExtraParams& text, ColorScheme color_scheme) const
{
    SkRect bounds = gfx::RectToSkRect(rect);
    float border_radius = GetBorderRadiusForPart(kTextField, rect.width(), rect.height());
    border_radius = AdjustBorderRadiusByZoom(kTextField, border_radius, text.zoom);
    ...
    if (text.background_color != 0) {
        PaintLightenLayer(canvas, color_provider, bounds, state, border_radius, color_scheme);
        SkColor text_field_background_color = ControlsBackgroundColorForState(state, color_scheme, color_provider);
        if (text.auto_complete_active && state != kDisabled) {
            text_field_background_color = GetControlColor(kAutoCompleteBackground, color_scheme, color_provider);
        }
    ...
    if (text.has_border) {
        ...
        stroke_flags.setColor(ControlsBorderColorForState(state, color_scheme, color_provider));
...
void NativeThemeBase::PaintMenuList(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const MenuListExtraParams& menu_list, ColorScheme color_scheme) const
{
    ...
    if (!menu_list.has_border_radius) {
        ...
        PaintTextField(canvas, color_provider, state, rect, text_field, color_scheme);
    }
    ...
    if (menu_list.arrow_direction == ui::NativeTheme::ArrowDirection::kDown) {
        ...
        gfx::Rect arrow(menu_list.arrow_x, menu_list.arrow_y - (arrow_height / 2), arrow_width, arrow_height);
        ...
        gfx::Rect arrow(menu_list.arrow_x - (arrow_width / 2), menu_list.arrow_y, arrow_width, arrow_height);
        arrow.Intersect(rect);
...
void NativeThemeBase::PaintMenuPopupBackground(cc::PaintCanvas* canvas, const ColorProvider* color_provider, const gfx::Size& size,
    const MenuBackgroundExtraParams& menu_background, ColorScheme color_scheme) const
{
    // We are currently only painting kMenuPopupBackground with the kDefault
    ...
    canvas->drawColor(SkColor4f::FromColor(GetColor(kMenuPopupBackgroundColor, color_scheme)), SkBlendMode::kSrc);
}

void NativeThemeBase::PaintMenuItemBackground(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const MenuItemExtraParams& menu_item, ColorScheme color_scheme) const
{
    // By default don't draw anything over the normal background.
}

void NativeThemeBase::PaintMenuSeparator(
    cc::PaintCanvas* canvas, const ui::ColorProvider* color_provider, State state, const gfx::Rect& rect, const MenuSeparatorExtraParams& menu_separator) const
{
...
void NativeThemeBase::PaintSliderTrack(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const SliderExtraParams& slider, ColorScheme color_scheme, const std::optional<SkColor>& accent_color) const
{
    // Paint the entire slider track.
    cc::PaintFlags flags;
    flags.setAntiAlias(true);
    flags.setColor(ControlsFillColorForState(state, color_scheme, color_provider));
    ...
    float border_radius = GetBorderRadiusForPart(kSliderTrack, rect.width(), rect.height());
    ...
    if (accent_color && state != kDisabled) {
        ...
        flags.setColor(ControlsSliderColorForState(state, color_scheme, color_provider));
    }
    ...
    flags.setStyle(cc::PaintFlags::kStroke_Style);
    flags.setStrokeWidth(border_width);
    SkColor border_color = ControlsBorderColorForState(state, color_scheme, color_provider);
    if (!UserHasContrastPreference() && state != kDisabled && color_scheme != ColorScheme::kDark)
...
void NativeThemeBase::PaintSliderThumb(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const SliderExtraParams& slider, ColorScheme color_scheme, const std::optional<SkColor>& accent_color) const
{
    const float radius = GetBorderRadiusForPart(kSliderThumb, rect.width(), rect.height());
...
void NativeThemeBase::PaintInnerSpinButton(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const InnerSpinButtonExtraParams& spin_button, ColorScheme color_scheme, bool in_forced_colors) const
{
    ...
    arrow.zoom = 1.0;
    if (spin_button.spin_arrows_direction == ui::NativeTheme::SpinArrowsDirection::kUpDown) {
        half.set_height(rect.height() / 2);
        PaintArrowButton(canvas, color_provider, half, kScrollbarUpArrow, north_state, color_scheme, in_forced_colors, arrow);

        half.set_y(rect.y() + rect.height() / 2);
        PaintArrowButton(canvas, color_provider, half, kScrollbarDownArrow, south_state, color_scheme, in_forced_colors, arrow);
    } else {
        half.set_width(rect.width() / 2);
        PaintArrowButton(canvas, color_provider, half, kScrollbarLeftArrow, south_state, color_scheme, in_forced_colors, arrow);

        half.set_x(rect.x() + rect.width() / 2);
        PaintArrowButton(canvas, color_provider, half, kScrollbarRightArrow, north_state, color_scheme, in_forced_colors, arrow);
    }
}

void NativeThemeBase::PaintProgressBar(cc::PaintCanvas* canvas, const ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const ProgressBarExtraParams& progress_bar, ColorScheme color_scheme, const std::optional<SkColor>& accent_color) const
{
    DCHECK(!rect.IsEmpty());
    // Paint the track.
    cc::PaintFlags flags;
    ...
    SkRect track_rect = AlignSliderTrack(rect, slider, false, track_block_thickness);
    float border_radius = GetBorderRadiusForPart(kProgressBar, rect.width(), rect.height());
    canvas->drawRoundRect(track_rect, border_radius, border_radius, flags);
    ...
    gfx::Rect original_value_rect(progress_bar.value_rect_x, progress_bar.value_rect_y, adjusted_width, adjusted_height);
    SkRect value_rect = AlignSliderTrack(original_value_rect, slider, false, track_block_thickness);
    if (accent_color) {
...
void NativeThemeBase::PaintFrameTopArea(
    cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const FrameTopAreaExtraParams& frame_top_area, ColorScheme color_scheme) const
{
    cc::PaintFlags flags;
...
void NativeThemeBase::AdjustCheckboxRadioRectForPadding(SkRect* rect) const
{
    ...
    rect->setLTRB(static_cast<int>(rect->x()), static_cast<int>(rect->y()), static_cast<int>(rect->right()) - 1, static_cast<int>(rect->bottom()) - 1);
}

SkColor NativeThemeBase::SaturateAndBrighten(SkScalar* hsv, SkScalar saturate_amount, SkScalar brighten_amount) const
{
    SkScalar color[3];
    color[0] = hsv[0];
...
SkColor NativeThemeBase::GetArrowColor(State state, ColorScheme color_scheme, const ColorProvider* color_provider) const
{
    if (state != kDisabled)
    ...
    SkScalar thumb_hsv[3];
    SkColorToHSV(GetControlColor(kScrollbarThumbInactive, color_scheme, color_provider), thumb_hsv);
    return OutlineColor(track_hsv, thumb_hsv);
}

void NativeThemeBase::DrawVertLine(cc::PaintCanvas* canvas, int x, int y1, int y2, const cc::PaintFlags& flags) const
{
    SkIRect skrect;
...
void NativeThemeBase::DrawHorizLine(cc::PaintCanvas* canvas, int x1, int x2, int y, const cc::PaintFlags& flags) const
{
    SkIRect skrect;
...
void NativeThemeBase::DrawBox(cc::PaintCanvas* canvas, const gfx::Rect& rect, const cc::PaintFlags& flags) const
{
...
SkColor NativeThemeBase::ControlsAccentColorForState(State state, ColorScheme color_scheme, const ColorProvider* color_provider) const
{
...
SkColor NativeThemeBase::ControlsSliderColorForState(State state, ColorScheme color_scheme, const ColorProvider* color_provider) const
{
    ControlColorId color_id;
    if (state == kHovered) {
...
SkColor NativeThemeBase::ControlsBorderColorForState(State state, ColorScheme color_scheme, const ColorProvider* color_provider) const
{
    ControlColorId color_id;
    if (state == kHovered) {
...
SkColor NativeThemeBase::ButtonBorderColorForState(State state, ColorScheme color_scheme, const ColorProvider* color_provider) const
{
    ControlColorId color_id;
    if (state == kHovered) {
...
SkColor NativeThemeBase::ControlsFillColorForState(State state, ColorScheme color_scheme, const ColorProvider* color_provider) const
{
    ControlColorId color_id;
    if (state == kHovered) {
...
SkColor NativeThemeBase::ButtonFillColorForState(State state, ColorScheme color_scheme, const ColorProvider* color_provider) const
{
    ControlColorId color_id;
    if (state == kHovered) {
...
SkColor NativeThemeBase::ControlsBackgroundColorForState(State state, ColorScheme color_scheme, const ColorProvider* color_provider) const
{
    ControlColorId color_id;
    if (state == kDisabled) {
...
SkColor NativeThemeBase::GetControlColor(ControlColorId color_id, ColorScheme color_scheme, const ColorProvider* color_provider) const
{
...
SkColor NativeThemeBase::GetDarkModeControlColor(ControlColorId color_id) const
{
    switch (color_id) {
    case kAccent:
...
SkColor NativeThemeBase::GetControlColorFromColorProvider(ControlColorId color_id, const ColorProvider* color_provider) const
{
    ...
    case kAutoCompleteBackground:
        return color_provider->GetColor(kColorWebNativeControlAutoCompleteBackground);
    case kScrollbarArrowBackground:
    case kScrollbarTrack:
        return color_provider->GetColor(kColorWebNativeControlScrollbarTrack);
    case kScrollbarArrowBackgroundHovered:
        return color_provider->GetColor(kColorWebNativeControlScrollbarArrowBackgroundHovered);
    case kScrollbarArrowBackgroundPressed:
        return color_provider->GetColor(kColorWebNativeControlScrollbarArrowBackgroundPressed);
    case kScrollbarArrow:
    case kScrollbarArrowHovered:
        return color_provider->GetColor(kColorWebNativeControlScrollbarArrowForeground);
    case kScrollbarArrowPressed:
        return color_provider->GetColor(kColorWebNativeControlScrollbarArrowForegroundPressed);
    case kScrollbarCornerControlColorId:
    ...
    case kScrollbarThumbHovered:
        return color_provider->GetColor(kColorWebNativeControlScrollbarThumbHovered);
    case kScrollbarThumbInactive:
        return color_provider->GetColor(kColorWebNativeControlScrollbarThumbInactive);
    case kScrollbarThumbPressed:
        return color_provider->GetColor(kColorWebNativeControlScrollbarThumbPressed);
    case kButtonBorder:
        return color_provider->GetColor(kColorWebNativeControlButtonBorder);
    case kButtonDisabledBorder:
        return color_provider->GetColor(kColorWebNativeControlButtonBorderDisabled);
    case kButtonHoveredBorder:
        return color_provider->GetColor(kColorWebNativeControlButtonBorderHovered);
    case kButtonPressedBorder:
        return color_provider->GetColor(kColorWebNativeControlButtonBorderPressed);
    case kButtonFill:
...
void NativeThemeBase::PaintLightenLayer(
    cc::PaintCanvas* canvas, const ColorProvider* color_provider, SkRect skrect, State state, SkScalar border_radius, ColorScheme color_scheme) const
{
    if (state == kDisabled) {
        cc::PaintFlags flags;
        flags.setAntiAlias(true);
        ...
        flags.setColor(GetControlColor(kLightenLayer, color_scheme, color_provider));
        canvas->drawRoundRect(skrect, border_radius, border_radius, flags);
    }
}

SkRect NativeThemeBase::AlignSliderTrack(
    const gfx::Rect& slider_rect, const NativeTheme::SliderExtraParams& slider, bool is_value, float track_block_thickness) const
{
    ...
    if (slider.vertical) {
        const float top = is_value && slider.right_to_left ? slider_rect.y() + slider.thumb_y + kAlignment : slider_rect.y();
        const float bottom = is_value && !slider.right_to_left ? slider_rect.y() + slider.thumb_y + kAlignment : slider_rect.bottom();
        aligned_rect.setLTRB(std::max(float(slider_rect.x()), mid_x - kAlignment), top, std::min(float(slider_rect.right()), mid_x + kAlignment), bottom);
    } else {
        const float right = is_value && !slider.right_to_left ? slider_rect.x() + slider.thumb_x + kAlignment : slider_rect.right();
        const float left = is_value && slider.right_to_left ? slider_rect.x() + slider.thumb_x + kAlignment : slider_rect.x();

        aligned_rect.setLTRB(left, std::max(float(slider_rect.y()), mid_y - kAlignment), right, std::min(float(slider_rect.bottom()), mid_y + kAlignment));
    }

    return aligned_rect;
}

bool NativeThemeBase::IsColorPipelineSupportedForControlColorId(const ColorProvider* color_provider, ControlColorId color_id) const
{
    ...
    static constexpr auto kControlColorIdsSet
        = base::MakeFixedFlatSet<ControlColorId>({ kBorder, kDisabledBorder, kHoveredBorder, kPressedBorder, kAccent, kDisabledAccent, kHoveredAccent,
            kPressedAccent, kBackground, kDisabledBackground, kFill, kDisabledFill, kHoveredFill, kPressedFill, kLightenLayer, kProgressValue, kSlider,
            kDisabledSlider, kHoveredSlider, kPressedSlider, kAutoCompleteBackground, kScrollbarArrowBackground, kScrollbarArrowBackgroundHovered,
            kScrollbarArrowBackgroundPressed, kScrollbarArrow, kScrollbarArrowHovered, kScrollbarArrowPressed, kScrollbarCornerControlColorId, kScrollbarTrack,
            kScrollbarThumb, kScrollbarThumbHovered, kScrollbarThumbPressed, kScrollbarThumbInactive, kButtonBorder, kButtonDisabledBorder,
            kButtonHoveredBorder, kButtonPressedBorder, kButtonFill, kButtonDisabledFill, kButtonHoveredFill, kButtonPressedFill });
