...
bool GLSurfaceEglReadback::Resize(const gfx::Size& size, float scale_factor, const gfx::ColorSpace& color_space, bool has_alpha)
{
...
gfx::SwapResult GLSurfaceEglReadback::SwapBuffers(PresentationCallback callback, gfx::FrameData data)
{
    ...
    task_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(callback), feedback));
    return swap_result;
}
...
void GLSurfaceEglReadback::ReadPixels(base::span<uint8_t> buffer)
{
    ...
    CHECK_GE(buffer.size() / base::checked_cast<size_t>(size.width()), base::checked_cast<size_t>(size.height()));
    glReadPixels(0, 0, size.width(), size.height(), GL_BGRA, GL_UNSIGNED_BYTE, buffer.data());
