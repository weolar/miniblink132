...
#define FOURCC(a, b, c, d) ((static_cast<uint32_t>(a)) | (static_cast<uint32_t>(b) << 8) | (static_cast<uint32_t>(c) << 16) | (static_cast<uint32_t>(d) << 24))
...
NativePixmapEGLBinding::NativePixmapEGLBinding(const gfx::Size& size, gfx::BufferFormat format, gfx::BufferPlane plane)
    : size_(size)
    , format_(format)
    , plane_(plane)
{
...
// static
std::unique_ptr<NativePixmapGLBinding> NativePixmapEGLBinding::Create(scoped_refptr<gfx::NativePixmap> pixmap, gfx::BufferFormat plane_format,
    gfx::BufferPlane plane, gfx::Size plane_size, const gfx::ColorSpace& color_space, GLenum target, GLuint texture_id)
{
    ...
    auto binding = std::make_unique<NativePixmapEGLBinding>(plane_size, plane_format, plane);

    if (!binding->InitializeFromNativePixmap(std::move(pixmap), color_space, target, texture_id)) {
...
bool NativePixmapEGLBinding::InitializeFromNativePixmap(
    scoped_refptr<gfx::NativePixmap> pixmap, const gfx::ColorSpace& color_space, GLenum target, GLuint texture_id)
{
    DCHECK(!pixmap_);
    ...
    if (format_ == gfx::BufferFormat::YUV_420_BIPLANAR || format_ == gfx::BufferFormat::YVU_420 || format_ == gfx::BufferFormat::P010) {
    ...
    if (plane_ == gfx::BufferPlane::DEFAULT) {
        ...
        bool has_dma_buf_import_modifier = gl::GLSurfaceEGL::GetGLDisplayEGL()->ext->b_EGL_EXT_image_dma_buf_import_modifiers;
        ...
        for (size_t attrs_plane = 0; attrs_plane < pixmap->GetNumberOfPlanes(); ++attrs_plane) {
            ...
            if (has_dma_buf_import_modifier && modifier != gfx::NativePixmapHandle::kNoModifier) {
    ...
    egl_image_ = gl::MakeScopedEGLImage(EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, static_cast<EGLClientBuffer>(nullptr), &attrs[0]);
    if (!egl_image_.get()) {
        return false;
    }
