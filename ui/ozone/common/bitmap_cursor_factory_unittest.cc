...
TEST(BitmapCursorFactoryTest, InvisibleCursor)
{
    ...
    EXPECT_EQ(BitmapCursor::FromPlatformCursor(cursor)->type(), CursorType::kNone);
}
...
TEST(BitmapCursorFactoryTest, DefaultCursorsHaveTypeInformation)
{
    ...
    EXPECT_EQ(BitmapCursor::FromPlatformCursor(cursor)->type(), CursorType::kPointer);

    cursor = factory.GetDefaultCursor(CursorType::kHand);
    EXPECT_NE(cursor, nullptr);
    EXPECT_EQ(BitmapCursor::FromPlatformCursor(cursor)->type(), CursorType::kHand);

    cursor = factory.GetDefaultCursor(CursorType::kIBeam);
    EXPECT_NE(cursor, nullptr);
    EXPECT_EQ(BitmapCursor::FromPlatformCursor(cursor)->type(), CursorType::kIBeam);
}
