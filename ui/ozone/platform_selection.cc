...
std::string GetPlatformName()
{
    ...
    if (!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kOzonePlatform) && kPlatformCount > 0)
        return kPlatformNames[0];
    return base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kOzonePlatform);
}
...
int GetOzonePlatformId()
{
    ...
    LOG(FATAL) << "Invalid ozone platform: " << platform_name << (platform_name == "auto" ? ", use --ozone-platform-hint=auto instead?" : ".");
}
