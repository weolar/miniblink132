...
protected:
    ...
    void SetUp() override
    {
        ...
        context_ = gl::init::CreateGLContext(nullptr, surface_.get(), GLContextAttribs());
...
protected:
    std::unique_ptr<ui::NativePixmapGLBinding> CreateSolidColorImage(const gfx::Size& size)
    {
        ui::SurfaceFactoryOzone* surface_factory = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone();
        scoped_refptr<gfx::NativePixmap> pixmap = surface_factory->CreateNativePixmap(gfx::kNullAcceleratedWidget, nullptr, size, kFormat, kUsage);
        DCHECK(pixmap) << "Offending format: " << gfx::BufferFormatToString(kFormat);
        ...
        auto binding = gl_ozone->ImportNativePixmap(
            std::move(pixmap), kFormat, gfx::BufferPlane::DEFAULT, size, gfx::ColorSpace(), GL_TEXTURE_EXTERNAL_OES, texture_id_);
        EXPECT_TRUE(binding);
        return binding;
    }
