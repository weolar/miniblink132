...
VkPipelineStageFlags GetPipelineStageFlags(const VkImageLayout layout)
{
    ...
    case VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL:
        return VK_PIPELINE_STAGE_TESSELLATION_CONTROL_SHADER_BIT | VK_PIPELINE_STAGE_TESSELLATION_EVALUATION_SHADER_BIT | VK_PIPELINE_STAGE_GEOMETRY_SHADER_BIT
...
VkAccessFlags GetAccessMask(const VkImageLayout layout)
{
    ...
    case VK_IMAGE_LAYOUT_GENERAL:
        ...
        return VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT | VK_ACCESS_TRANSFER_WRITE_BIT | VK_ACCESS_TRANSFER_READ_BIT
...
VulkanRenderer::VulkanRenderer(std::unique_ptr<PlatformWindowSurface> window_surface, std::unique_ptr<gpu::VulkanSurface> vulkan_surface,
    gpu::VulkanImplementation* vulkan_implementation, gfx::AcceleratedWidget widget, const gfx::Size& size)
...
bool VulkanRenderer::Initialize()
{
    ...
    device_queue_ = gpu::CreateVulkanDeviceQueue(
        vulkan_implementation_, gpu::VulkanDeviceQueue::GRAPHICS_QUEUE_FLAG | gpu::VulkanDeviceQueue::PRESENTATION_SUPPORT_QUEUE_FLAG);
    if (!device_queue_) {
        LOG(FATAL) << "Failed to init device queue";
    }

    if (!vulkan_surface_->Initialize(device_queue_.get(), gpu::VulkanSurface::DEFAULT_SURFACE_FORMAT)) {
...
void VulkanRenderer::RenderFrame()
{
    ...
    gpu::VulkanSwapChain* vulkan_swap_chain = vulkan_surface_->swap_chain();
    {
        ...
        if (!framebuffer) {
            framebuffer = Framebuffer::Create(device_queue_.get(), command_pool_.get(), render_pass_, vulkan_surface_.get(), scoped_write.image());
            CHECK(framebuffer);
        }
...

        {
            ...
            .subresourceRange =
                ...
                vkCmdPipelineBarrier(recorder.handle(), GetPipelineStageFlags(old_layout), GetPipelineStageFlags(layout), 0 /* dependencyFlags */,
                    0 /* memoryBarrierCount */, nullptr /* pMemoryBarriers */, 0 /* bufferMemoryBarrierCount */, nullptr /* pBufferMemoryBarriers */, 1,
...
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&VulkanRenderer::RenderFrame, weak_ptr_factory_.GetWeakPtr()));
}

VulkanRenderer::Framebuffer::Framebuffer(VkImageView vk_image_view, VkFramebuffer vk_framebuffer, std::unique_ptr<gpu::VulkanCommandBuffer> command_buffer)
...
std::unique_ptr<VulkanRenderer::Framebuffer> VulkanRenderer::Framebuffer::Create(gpu::VulkanDeviceQueue* vulkan_device_queue,
    gpu::VulkanCommandPool* vulkan_command_pool, VkRenderPass vk_render_pass, gpu::VulkanSurface* vulkan_surface, VkImage image)
{
    ...
    return std::make_unique<VulkanRenderer::Framebuffer>(vk_image_view, vk_framebuffer, std::move(command_buffer));
}
