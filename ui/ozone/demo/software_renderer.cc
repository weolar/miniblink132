...
SoftwareRenderer::SoftwareRenderer(gfx::AcceleratedWidget widget, std::unique_ptr<PlatformWindowSurface> window_surface, const gfx::Size& size)
...
{
    software_surface_ = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->CreateCanvasForWidget(widget_);
...
void SoftwareRenderer::RenderFrame()
{
    ...
    if (vsync_provider_) {
        vsync_provider_->GetVSyncParameters(base::BindOnce(&SoftwareRenderer::UpdateVSyncParameters, weak_ptr_factory_.GetWeakPtr()));
    }

    timer_.Start(FROM_HERE, vsync_period_, this, &SoftwareRenderer::RenderFrame);
}

void SoftwareRenderer::UpdateVSyncParameters(const base::TimeTicks timebase, const base::TimeDelta interval)
{
