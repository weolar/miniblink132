...
SkiaGlRenderer::SkiaGlRenderer(
    gfx::AcceleratedWidget widget, std::unique_ptr<PlatformWindowSurface> window_surface, const scoped_refptr<gl::GLSurface>& surface, const gfx::Size& size)
...
bool SkiaGlRenderer::Initialize()
{
    ...
    sk_sp<const GrGLInterface> native_interface = GrGLMakeAssembledInterface(nullptr, [](void* ctx, const char name[]) { return gl::GetGLProcAddress(name); });
...
void SkiaGlRenderer::RenderFrame()
{
    ...
    SkSurfaceProps surface_props = skia::LegacyDisplayGlobals::GetSkSurfaceProps();

    if (!sk_surface_) {
        GrGLFramebufferInfo framebuffer_info;
        framebuffer_info.fFBOID = 0;
        framebuffer_info.fFormat = GL_RGBA8;
        auto render_target = GrBackendRenderTargets::MakeGL(size_.width(), size_.height(), 0, 8, framebuffer_info);

        sk_surface_ = SkSurfaces::WrapBackendRenderTarget(
            gr_context_.get(), render_target, kBottomLeft_GrSurfaceOrigin, kRGBA_8888_SkColorType, nullptr, &surface_props);
    }
    ...
    if (gl_surface_->SupportsAsyncSwap()) {
        gl_surface_->SwapBuffersAsync(base::BindOnce(&SkiaGlRenderer::PostRenderFrameTask, weak_ptr_factory_.GetWeakPtr()),
            base::BindOnce(&SkiaGlRenderer::OnPresentation, weak_ptr_factory_.GetWeakPtr()), gfx::FrameData());
    } else {
        PostRenderFrameTask(gfx::SwapCompletionResult(
            gl_surface_->SwapBuffers(base::BindOnce(&SkiaGlRenderer::OnPresentation, weak_ptr_factory_.GetWeakPtr()), gfx::FrameData())));
    }
}
...
void SkiaGlRenderer::PostRenderFrameTask(gfx::SwapCompletionResult result)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&SkiaGlRenderer::RenderFrame, weak_ptr_factory_.GetWeakPtr()));
}
...
void SkiaGlRenderer::StartDDLRenderThreadIfNecessary(SkSurface* sk_surface)
{
    ...
    ddl_render_thread_ = std::make_unique<base::DelegateSimpleThread>(this, "DDLRenderThread");
...
void SkiaGlRenderer::Run()
{
    ...
    while (true) {
        ...
        while (surface_charaterization_.isValid() && ddls_.size() == kMaxPendingDDLS)
            condition_variable_.Wait();
