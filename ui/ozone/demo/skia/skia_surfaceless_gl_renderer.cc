...
public:
    ...
    bool Initialize(GrDirectContext* gr_context, gfx::AcceleratedWidget widget, const gfx::Size& size);
...
bool SurfacelessSkiaGlRenderer::BufferWrapper::Initialize(GrDirectContext* gr_context, gfx::AcceleratedWidget widget, const gfx::Size& size)
{
    ...
    auto backend_texture = GrBackendTextures::MakeGL(size_.width(), size_.height(), skgpu::Mipmapped::kNo, texture_info);
    sk_surface_ = SkSurfaces::WrapBackendTexture(gr_context, backend_texture, kTopLeft_GrSurfaceOrigin, 0, kBGRA_8888_SkColorType, nullptr, nullptr);
    if (!sk_surface_) {
...
SurfacelessSkiaGlRenderer::SurfacelessSkiaGlRenderer(gfx::AcceleratedWidget widget, std::unique_ptr<PlatformWindowSurface> window_surface,
    const scoped_refptr<gl::GLSurface>& offscreen_surface, const scoped_refptr<gl::Presenter>& presenter, const gfx::Size& size)
    : SkiaGlRenderer(widget, std::move(window_surface), std::move(offscreen_surface), size)
...
bool SurfacelessSkiaGlRenderer::Initialize()
{
    ...
    for (size_t i = 0; i < std::size(buffers_); ++i) {
        ...
        if (!buffers_[i]->Initialize(gr_context_.get(), widget_, primary_plane_rect_.size()))
            return false;
    }
    ...
    if (command_line->HasSwitch(kEnableOverlay)) {
        ...
        for (size_t i = 0; i < std::size(overlay_buffer_); ++i) {
            ...
            overlay_buffer_[i]->Initialize(gr_context_.get(), gfx::kNullAcceleratedWidget, overlay_size);
...
void SurfacelessSkiaGlRenderer::RenderFrame()
{
    ...
    if (overlay_buffer_[0]) {
        ...
        gfx::Vector2d offset(stepped_fraction * (size_.width() - overlay_rect.width()), (size_.height() - overlay_rect.height()) / 2);
        overlay_rect += offset;
    ...
    if (!disable_primary_plane_) {
        ...
        presenter_->ScheduleOverlayPlane(buffers_[back_buffer_]->image(), /* gpu_fence */ nullptr,
            gfx::OverlayPlaneData(0, gfx::OVERLAY_TRANSFORM_NONE, gfx::RectF(primary_plane_rect_), unity_rect,
    ...
    if (overlay_buffer_[0] && overlay_list.back().overlay_handled) {
        presenter_->ScheduleOverlayPlane(overlay_buffer_[back_buffer_]->image(), /* gpu_fence */ nullptr,
            gfx::OverlayPlaneData(1, gfx::OVERLAY_TRANSFORM_NONE, gfx::RectF(overlay_rect), unity_rect,
                /* enable_blend */ true, gfx::Rect(buffers_[back_buffer_]->size()),
                /* opacity */ 1.0f, gfx::OverlayPriorityHint::kNone,
                /* rounded_corners */ gfx::RRectF(), gfx::ColorSpace::CreateSRGB(),
                /*hdr_metadata=*/std::nullopt));
    ...
    back_buffer_ ^= 1;
    presenter_->Present(base::BindOnce(&SurfacelessSkiaGlRenderer::PostRenderFrameTask, weak_ptr_factory_.GetWeakPtr()), base::DoNothing(), gfx::FrameData());
}

void SurfacelessSkiaGlRenderer::PostRenderFrameTask(gfx::SwapCompletionResult result)
{
