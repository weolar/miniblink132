...
std::unique_ptr<Renderer> SkiaRendererFactory::CreateRenderer(gfx::AcceleratedWidget widget, const gfx::Size& size)
{
    ...
    if (auto presenter = CreatePresenter(display_, widget)) {
        return std::make_unique<SurfacelessSkiaGlRenderer>(
            widget, std::move(window_surface), gl::init::CreateOffscreenGLSurface(display_, gfx::Size(0, 0)), std::move(presenter), size);
    }
    scoped_refptr<gl::GLSurface> gl_surface = CreateGLSurface(display_, widget);
    if (!gl_surface)
        LOG(FATAL) << "Failed to create GL surface";
    return std::make_unique<SkiaGlRenderer>(widget, std::move(window_surface), std::move(gl_surface), size);
}
