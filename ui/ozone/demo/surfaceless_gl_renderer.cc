...
OverlaySurfaceCandidate MakeOverlayCandidate(int z_order, gfx::Rect bounds_rect, gfx::RectF crop_rect)
{
    ...
    gfx::RectF display_rect(bounds_rect.x(), bounds_rect.y(), bounds_rect.width(), bounds_rect.height());
...
scoped_refptr<gfx::NativePixmap> SurfacelessGlRenderer::BufferWrapper::image() const
{
    return pixmap_;
}

bool SurfacelessGlRenderer::BufferWrapper::Initialize(gfx::AcceleratedWidget widget, const gfx::Size& size)
{
    glGenFramebuffersEXT(1, &gl_fb_);
    glGenTextures(1, &gl_tex_);

    gfx::BufferFormat format = display::DisplaySnapshot::PrimaryFormat();
    pixmap_ = OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->CreateNativePixmap(widget, nullptr, size, format, gfx::BufferUsage::SCANOUT);

    glBindFramebufferEXT(GL_FRAMEBUFFER, gl_fb_);

    pixmap_gl_binding_ = OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->GetCurrentGLOzone()->ImportNativePixmap(
        pixmap_, format, gfx::BufferPlane::DEFAULT, size, gfx::ColorSpace(), GL_TEXTURE_2D, gl_tex_);
    ...
    glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, gl_tex_, 0);
    if (glCheckFramebufferStatusEXT(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        LOG(ERROR) << "Failed to create framebuffer " << glCheckFramebufferStatusEXT(GL_FRAMEBUFFER);
        return false;
    }
...
SurfacelessGlRenderer::SurfacelessGlRenderer(gfx::AcceleratedWidget widget, std::unique_ptr<PlatformWindowSurface> window_surface,
    const scoped_refptr<gl::GLSurface>& offscreen_surface, const scoped_refptr<gl::Presenter>& presenter, const gfx::Size& size)
    : RendererBase(widget, size)
    , overlay_checker_(ui::OzonePlatform::GetInstance()->GetOverlayManager()->CreateOverlayCandidates(widget))
...
{
    context_ = gl::init::CreateGLContext(nullptr, gl_surface_.get(), gl::GLContextAttribs());
    if (!context_.get()) {
    ...
    if (command_line->HasSwitch("enable-overlay")) {
        ...
        base::StringToInt(command_line->GetSwitchValueASCII("enable-overlay").c_str(), &requested_overlay_cnt);
        overlay_cnt_ = std::clamp(requested_overlay_cnt, 1, kMaxLayers);

        const gfx::Size overlay_size = gfx::Size(size_.width() / 8, size_.height() / 8);
        for (size_t i = 0; i < overlay_cnt_; ++i) {
            for (size_t j = 0; j < std::size(overlay_buffers_[i]); ++j) {
                overlay_buffers_[i][j] = std::make_unique<BufferWrapper>();
                overlay_buffers_[i][j]->Initialize(gfx::kNullAcceleratedWidget, overlay_size);
...
void SurfacelessGlRenderer::RenderFrame()
{
    ...
    if (!disable_primary_plane_) {
        overlay_list.push_back(MakeOverlayCandidate(1, gfx::Rect(size_), unity_rect));
    ...
    for (size_t i = 0; i < overlay_cnt_; ++i) {
        ...
        float stepped_fraction = std::floor((fraction + 0.5f / steps_num) * steps_num) / steps_num;
        gfx::Vector2d offset(
            stepped_fraction * (size_.width() - overlay_rect[i].width()), ((size_.height() / (overlay_cnt_ + 1)) * (i + 1) - overlay_rect[i].height() / 2));
        overlay_rect[i] += offset;
        overlay_list.push_back(MakeOverlayCandidate(1, overlay_rect[i], unity_rect));
    }
    ...
    if (!disable_primary_plane_) {
        ...
        std::unique_ptr<gl::GLFence> gl_fence = use_gpu_fences_ ? gl::GLFence::CreateForGpuFence() : nullptr;

        presenter_->ScheduleOverlayPlane(buffers_[back_buffer_]->image(), gl_fence ? gl_fence->GetGpuFence() : nullptr,
            gfx::OverlayPlaneData(0, gfx::OVERLAY_TRANSFORM_NONE, gfx::RectF(primary_plane_rect_), unity_rect, false, gfx::Rect(buffers_[back_buffer_]->size()),
                1.0f, gfx::OverlayPriorityHint::kNone, gfx::RRectF(), gfx::ColorSpace::CreateSRGB(), std::nullopt));
    }

    for (size_t i = 0; i < overlay_cnt_; ++i) {
        if (overlay_list.back().overlay_handled) {
            presenter_->ScheduleOverlayPlane(overlay_buffers_[i][back_buffer_]->image(), /* gpu_fence */ nullptr,
                gfx::OverlayPlaneData(1, gfx::OVERLAY_TRANSFORM_NONE, gfx::RectF(overlay_rect[i]), unity_rect, false,
                    gfx::Rect(overlay_buffers_[i][back_buffer_]->size()), 1.0f, gfx::OverlayPriorityHint::kNone, gfx::RRectF(), gfx::ColorSpace::CreateSRGB(),
                    std::nullopt));
        }
    }

    back_buffer_ ^= 1;
    presenter_->Present(base::BindOnce(&SurfacelessGlRenderer::PostRenderFrameTask, weak_ptr_factory_.GetWeakPtr()), base::DoNothing(), gfx::FrameData());
}

void SurfacelessGlRenderer::PostRenderFrameTask(gfx::SwapCompletionResult result)
{
    ...
    case gfx::SwapResult::SWAP_ACK:
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&SurfacelessGlRenderer::RenderFrame, weak_ptr_factory_.GetWeakPtr()));
        break;
...
void SurfacelessGlRenderer::OnPresentation(const gfx::PresentationFeedback& feedback)
{
