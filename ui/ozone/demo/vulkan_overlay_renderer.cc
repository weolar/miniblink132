...
VulkanOverlayRenderer::VulkanOverlayRenderer(std::unique_ptr<PlatformWindowSurface> window_surface, std::unique_ptr<OverlaySurface> overlay_surface,
    SurfaceFactoryOzone* surface_factory_ozone, gpu::VulkanImplementation* vulkan_implementation, gfx::AcceleratedWidget widget, const gfx::Size& size)
...
bool VulkanOverlayRenderer::Initialize()
{
    ...
    device_queue_ = gpu::CreateVulkanDeviceQueue(vulkan_implementation_, gpu::VulkanDeviceQueue::GRAPHICS_QUEUE_FLAG);
    if (!device_queue_) {
    ...
    VkAttachmentReference color_attachment_references[] = { { /* .attachment = */ 0,
    ...
    CHECK_EQ(vkCreateRenderPass(device_queue_->GetVulkanDevice(), &render_pass_create_info, nullptr, &render_pass_), VK_SUCCESS);
...
void VulkanOverlayRenderer::RecreateBuffers()
{
    ...
    for (auto& buffer : buffers_) {
        buffer = Buffer::Create(surface_factory_ozone_, vulkan_implementation_, device_queue_.get(), command_pool_.get(), render_pass_, widget_, size_);
        CHECK(buffer);
    }
}
...
void VulkanOverlayRenderer::RenderFrame()
{
    ...
    VkClearValue clear_value = { /* .color = */ { /* .float32 = */ { .5f, 1.f - NextFraction(), .5f, 1.f } } };
...

    {
        ...
        vkCmdBeginRenderPass(recorder.handle(), &begin_info, VK_SUBPASS_CONTENTS_INLINE);
...
std::unique_ptr<gfx::GpuFence> VulkanOverlayRenderer::SubmitFence(VkFence fence)
{
    VkResult result;
    VkFence fences[] = { fence };
    result = vkResetFences(device_queue_->GetVulkanDevice(), std::size(fences), fences);
    CHECK_EQ(result, VK_SUCCESS);

    result = vkQueueSubmit(device_queue_->GetVulkanQueue(), 0, nullptr, fence);
    CHECK_EQ(result, VK_SUCCESS);

    std::unique_ptr<gfx::GpuFence> gpu_fence = vulkan_implementation_->ExportVkFenceToGpuFence(device_queue_->GetVulkanDevice(), fence);
    if (!gpu_fence)
...
void VulkanOverlayRenderer::SubmitFrame(const VulkanOverlayRenderer::Buffer* buffer)
{
    ...
    primary_plane.overlay_plane_data.display_bounds = gfx::RectF(buffer->size().width(), buffer->size().height());
    ...
    overlay_surface_->SubmitFrame(std::move(overlay_planes),
...
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&VulkanOverlayRenderer::RenderFrame, weak_ptr_factory_.GetWeakPtr()));
}

void VulkanOverlayRenderer::OnFrameSubmitted(uint64_t frame_sequence, gfx::SwapResult swap_result)
{
    TRACE_EVENT1("ozone", "VulkanOverlayRenderer::OnFrameSubmitted", "frame", frame_sequence);
...
void VulkanOverlayRenderer::OnFramePresented(uint64_t frame_sequence, const gfx::PresentationFeedback& feedback)
{
    TRACE_EVENT1("ozone", "VulkanOverlayRenderer::OnFramePresented", "frame", frame_sequence);
}

void VulkanOverlayRenderer::OnFrameReleased(uint64_t frame_sequence)
{
    TRACE_EVENT1("ozone", "VulkanOverlayRenderer::OnFrameReleased", "frame", frame_sequence);
...
VulkanOverlayRenderer::Buffer::Buffer(const gfx::Size& size, scoped_refptr<gfx::NativePixmap> native_pixmap, VkDeviceMemory vk_device_memory, VkImage vk_image,
    VkImageView vk_image_view, VkFramebuffer vk_framebuffer, std::unique_ptr<gpu::VulkanCommandBuffer> command_buffer, VkFence fence)
...
std::unique_ptr<VulkanOverlayRenderer::Buffer> VulkanOverlayRenderer::Buffer::Create(SurfaceFactoryOzone* surface_factory_ozone,
    gpu::VulkanImplementation* vulkan_implementation, gpu::VulkanDeviceQueue* vulkan_device_queue, gpu::VulkanCommandPool* vulkan_command_pool,
    VkRenderPass vk_render_pass, gfx::AcceleratedWidget widget, const gfx::Size& size)
{
    ...
    scoped_refptr<gfx::NativePixmap> native_pixmap
        = surface_factory_ozone->CreateNativePixmapForVulkan(widget, size, format, gfx::BufferUsage::SCANOUT, vk_device, &vk_device_memory, &vk_image);
    if (!native_pixmap) {
        LOG(FATAL) << "Failed to create a presentable buffer for rendering with vulkan";
    }
    ...
    result = vkCreateImageView(vk_device, &vk_image_view_create_info, nullptr, &vk_image_view);
    ...
    result = vkCreateFramebuffer(vk_device, &vk_framebuffer_create_info, nullptr, &vk_framebuffer);
    if (result != VK_SUCCESS) {
        LOG(FATAL) << "Failed to create a Vulkan framebuffer.";
    }

    auto command_buffer = std::make_unique<gpu::VulkanCommandBuffer>(vulkan_device_queue, vulkan_command_pool, true /* primary */);
    ...
    return std::make_unique<VulkanOverlayRenderer::Buffer>(
        size, native_pixmap, vk_device_memory, vk_image, vk_image_view, vk_framebuffer, std::move(command_buffer), fence);
}
