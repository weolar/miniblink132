...
DemoWindow::DemoWindow(WindowManager* window_manager, RendererFactory* renderer_factory, const gfx::Rect& bounds)
    ...
    , renderer_factory_(renderer_factory)
{
    ...
    platform_window_ = OzonePlatform::GetInstance()->CreatePlatformWindow(this, std::move(properties));
...
gfx::AcceleratedWidget DemoWindow::GetAcceleratedWidget()
{
    ...
    DCHECK_NE(widget_, gfx::kNullAcceleratedWidget) << "Widget not available synchronously";
    return widget_;
}
...
void DemoWindow::OnWindowStateChanged(PlatformWindowState old_state, PlatformWindowState new_state)
{
}
