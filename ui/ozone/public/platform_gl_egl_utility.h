...
public:
    ...
    virtual void GetAdditionalEGLAttributes(EGLenum platform_type, std::vector<EGLAttrib>* display_attributes) = 0;

    // Chooses alpha and buffer size values.
    virtual void ChooseEGLAlphaAndBufferSize(EGLint* alpha_size, EGLint* buffer_size) = 0;
    ...
    virtual void CollectGpuExtraInfo(bool enable_native_gpu_memory_buffers, gfx::GpuExtraInfo& gpu_extra_info) const = 0;
    ...
    // is not supported.
    virtual std::optional<base::ScopedEnvironmentVariableOverride> MaybeGetScopedDisplayUnsetForVulkan();
};
