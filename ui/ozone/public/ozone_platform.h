...
public:
    ...
    struct PlatformProperties {
        ...
        // compositor thread.
        base::MessagePumpType message_pump_type_for_viz_compositor = base::MessagePumpType::DEFAULT;
    ...
    virtual void PostCreateMainMessageLoop(base::OnceCallback<void()> shutdown_cb, scoped_refptr<base::SingleThreadTaskRunner> user_input_task_runner);
    ...
    virtual std::unique_ptr<PlatformWindow> CreatePlatformWindow(PlatformWindowDelegate* delegate, PlatformWindowInitProperties properties) = 0;
    virtual std::unique_ptr<display::NativeDisplayDelegate> CreateNativeDisplayDelegate() = 0;
    ...
    virtual std::unique_ptr<InputMethod> CreateInputMethod(ImeKeyEventDispatcher* ime_key_event_dispatcher, gfx::AcceleratedWidget widget) = 0;
    ...
    virtual PlatformGlobalShortcutListener* GetPlatformGlobalShortcutListener(PlatformGlobalShortcutListenerDelegate* delegate);
    ...
    virtual std::unique_ptr<PlatformKeyboardHook> CreateKeyboardHook(PlatformKeyboardHookTypes type, base::RepeatingCallback<void(KeyEvent* event)> callback,
        std::optional<base::flat_set<DomCode>> dom_codes, gfx::AcceleratedWidget accelerated_widget);

    // Returns true if the specified buffer format is supported.
    virtual bool IsNativePixmapConfigSupported(gfx::BufferFormat format, gfx::BufferUsage usage) const;
    ...
    virtual std::unique_ptr<PlatformUserInputMonitor> GetPlatformUserInputMonitor(const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner);
...
protected:
    bool has_initialized_ui() const
    {
        return initialized_ui_;
    }
    bool has_initialized_gpu() const
    {
        return initialized_gpu_;
    }
