...
public:
    using GetTouchDeviceStatusReply = base::OnceCallback<void(const std::string&)>;
    // TODO(sky): convert this to value once mojo supports move for vectors.
    using GetTouchEventLogReply = base::OnceCallback<void(const std::vector<base::FilePath>&)>;
    ...
    virtual void SetAutoRepeatRate(const base::TimeDelta& delay, const base::TimeDelta& interval) = 0;
    virtual void GetAutoRepeatRate(base::TimeDelta* delay, base::TimeDelta* interval) = 0;
    // Callback is invoked when the keyboard layout is available and initialized.
    virtual void SetCurrentLayoutByName(const std::string& layout_name, base::OnceCallback<void(bool)> callback) = 0;
    virtual void SetKeyboardKeyBitsMapping(base::flat_map<int, std::vector<uint64_t>> key_bits_mapping) = 0;
    ...
    virtual void SetTouchpadSensitivity(std::optional<int> device_id, int value) = 0;
    virtual void SetTouchpadScrollSensitivity(std::optional<int> device_id, int value) = 0;
    ...
    virtual void SetTouchpadAcceleration(std::optional<int> device_id, bool enabled) = 0;
    virtual void SetTouchpadScrollAcceleration(std::optional<int> device_id, bool enabled) = 0;
    virtual void SetTouchpadHapticFeedback(std::optional<int> device_id, bool enabled) = 0;
    virtual void SetTouchpadHapticClickSensitivity(std::optional<int> device_id, int value) = 0;

    // Mouse settings.
    // If `nullopt` is passed instead of a `device_id`, settings will be applied
    // to all mice instead of per-device.
    virtual void SetMouseSensitivity(std::optional<int> device_id, int value) = 0;
    virtual void SetMouseScrollSensitivity(std::optional<int> device_id, int value) = 0;
    ...
    virtual void SetPrimaryButtonRight(std::optional<int> device_id, bool right) = 0;
    virtual void SetMouseReverseScroll(std::optional<int> device_id, bool enabled) = 0;
    virtual void SetMouseAcceleration(std::optional<int> device_id, bool enabled) = 0;
    virtual void SetMouseScrollAcceleration(std::optional<int> device_id, bool enabled) = 0;
    ...
    virtual void SetPointingStickSensitivity(std::optional<int> device_id, int value) = 0;
    ...
    virtual void SetPointingStickPrimaryButtonRight(std::optional<int> device_id, bool right) = 0;
    virtual void SetPointingStickAcceleration(std::optional<int> device_id, bool enabled) = 0;

    // Gamepad settings.
    virtual void SetGamepadKeyBitsMapping(base::flat_map<int, std::vector<uint64_t>> key_bits_mapping) = 0;
    ...
    virtual void GetTouchEventLog(const base::FilePath& out_dir, GetTouchEventLogReply reply) = 0;
    ...
    virtual void PlayVibrationEffect(int id, uint8_t amplitude, uint16_t duration_millis) = 0;
    virtual void StopVibration(int id) = 0;

    // Control haptic feedback for haptic-capable touchpad devices.
    virtual void PlayHapticTouchpadEffect(HapticTouchpadEffect effect, HapticTouchpadEffectStrength strength) = 0;
    virtual void SetHapticTouchpadEffectForNextButtonRelease(HapticTouchpadEffect effect, HapticTouchpadEffectStrength strength) = 0;

    // If |enable_filter| is true, all keys on the internal keyboard except
    // |allowed_keys| are disabled.
    virtual void SetInternalKeyboardFilter(bool enable_filter, std::vector<DomCode> allowed_keys) = 0;

    virtual void GetGesturePropertiesService(mojo::PendingReceiver<ui::ozone::mojom::GesturePropertiesService> receiver) = 0;
    ...
    [[nodiscard]] virtual std::unique_ptr<ScopedDisableInputDevices> DisableInputDevices() = 0;
