...
public:
    ...
    virtual display::Display GetDisplayForAcceleratedWidget(gfx::AcceleratedWidget widget) const = 0;
    ...
    virtual bool IsAcceleratedWidgetUnderCursor(gfx::AcceleratedWidget widget) const;

    virtual gfx::AcceleratedWidget GetAcceleratedWidgetAtScreenPoint(const gfx::Point& point_in_dip) const = 0;

    // Returns top level accelerated widget at |point| ignoring |ignore|.
    virtual gfx::AcceleratedWidget GetLocalProcessWidgetAtPoint(const gfx::Point& point_in_dip, const std::set<gfx::AcceleratedWidget>& ignore) const;

    // Returns the |Display| nearest the specified point. |point| must be in DIPs.
    virtual display::Display GetDisplayNearestPoint(const gfx::Point& point_in_dip) const = 0;

    // Returns the |Display| that most closely intersects the provided rect if one
    // exists.
    // TODO(rjk): Update the code to track this.
    virtual display::Display GetDisplayMatching(const gfx::Rect& match_rect) const = 0;
    ...
    public:
        ...
        PlatformScreenSaverSuspender& operator=(const PlatformScreenSaverSuspender&) = delete;
    ...
    virtual base::Value::List GetGpuExtraInfo(const gfx::GpuExtraInfo& gpu_extra_info);
    ...
    // available.
    virtual std::optional<float> GetPreferredScaleFactorForAcceleratedWidget(gfx::AcceleratedWidget widget) const;
...
protected:
    void StorePlatformNameIntoListOfValues(base::Value::List& values, const std::string& platform_name);
};
