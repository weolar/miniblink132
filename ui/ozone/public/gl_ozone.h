...
public:
    ...
    virtual bool InitializeStaticGLBindings(const gl::GLImplementationParts& implementation) = 0;

    // Performs any one off initialization for GL implementation.
    virtual gl::GLDisplay* InitializeGLOneOffPlatform(bool supports_angle, std::vector<gl::DisplayType> init_displays, gl::GpuPreference gpu_preference) = 0;
    ...
    // mechanism.
    virtual void SetDisabledExtensionsPlatform(const std::string& disabled_extensions) = 0;
    ...
    virtual bool InitializeExtensionSettingsOneOffPlatform(gl::GLDisplay* display) = 0;
    ...
    virtual std::unique_ptr<NativePixmapGLBinding> ImportNativePixmap(scoped_refptr<gfx::NativePixmap> pixmap, gfx::BufferFormat plane_format,
    ...
    virtual bool GetGLWindowSystemBindingInfo(const gl::GLVersionInfo& gl_info, gl::GLWindowSystemBindingInfo* info) = 0;
    ...
    virtual scoped_refptr<gl::GLContext> CreateGLContext(gl::GLShareGroup* share_group, gl::GLSurface* compatible_surface, const gl::GLContextAttribs& attribs)
        = 0;

    // Creates a GL surface that renders directly to a view.
    virtual scoped_refptr<gl::GLSurface> CreateViewGLSurface(gl::GLDisplay* display, gfx::AcceleratedWidget window) = 0;
    ...
    virtual scoped_refptr<gl::Presenter> CreateSurfacelessViewGLSurface(gl::GLDisplay* display, gfx::AcceleratedWidget window) = 0;

    // Creates a GL surface used for offscreen rendering.
    virtual scoped_refptr<gl::GLSurface> CreateOffscreenGLSurface(gl::GLDisplay* display, const gfx::Size& size) = 0;
};
