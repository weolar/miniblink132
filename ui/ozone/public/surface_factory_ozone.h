...
public:
    ...
    virtual std::unique_ptr<gpu::VulkanImplementation> CreateVulkanImplementation(bool use_swiftshader, bool allow_protected_memory);
    ...
    // available.
    virtual scoped_refptr<gfx::NativePixmap> CreateNativePixmapForVulkan(gfx::AcceleratedWidget widget, gfx::Size size, gfx::BufferFormat format,
    ...
    // window.
    virtual std::unique_ptr<PlatformWindowSurface> CreatePlatformWindowSurface(gfx::AcceleratedWidget window);

    // Creates an overlay surface for a platform window.
    virtual std::unique_ptr<OverlaySurface> CreateOverlaySurface(gfx::AcceleratedWidget window);
    ...
    virtual std::unique_ptr<SurfaceOzoneCanvas> CreateCanvasForWidget(gfx::AcceleratedWidget widget);
    ...
    virtual scoped_refptr<gfx::NativePixmap> CreateNativePixmap(gfx::AcceleratedWidget widget, gpu::VulkanDeviceQueue* device_queue, gfx::Size size,
        gfx::BufferFormat format, gfx::BufferUsage usage, std::optional<gfx::Size> framebuffer_size = std::nullopt);
    ...
    using NativePixmapCallback = base::OnceCallback<void(scoped_refptr<gfx::NativePixmap>)>;
    virtual void CreateNativePixmapAsync(gfx::AcceleratedWidget widget, gpu::VulkanDeviceQueue* device_queue, gfx::Size size, gfx::BufferFormat format,
    ...
    // instance for it.
    virtual scoped_refptr<gfx::NativePixmap> CreateNativePixmapForProtectedBufferHandle(
        gfx::AcceleratedWidget widget, gfx::Size size, gfx::BufferFormat format, gfx::NativePixmapHandle handle);
    ...
    using GetProtectedNativePixmapCallback = base::RepeatingCallback<scoped_refptr<gfx::NativePixmap>(const gfx::NativePixmapHandle& handle)>;
    ...
    virtual void SetGetProtectedNativePixmapDelegate(const GetProtectedNativePixmapCallback& get_protected_native_pixmap_callback);
    ...
    // in Chrome.
    virtual void SetDrmModifiersFilter(std::unique_ptr<DrmModifiersFilter> filter);
    ...
    virtual std::vector<gfx::BufferFormat> GetSupportedFormatsForTexturing() const;
    ...
    virtual std::optional<gfx::BufferFormat> GetPreferredFormatForSolidColor() const;
