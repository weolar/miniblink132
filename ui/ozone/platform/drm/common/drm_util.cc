...
bool IsCrtcInUse(uint32_t crtc, const std::vector<std::unique_ptr<HardwareDisplayControllerInfo>>& displays)
{
...
std::pair<uint32_t /* best_crtc */, uint32_t /* connected_crtc */> GetCrtcs(const DrmWrapper& drm, drmModeConnector* connector, drmModeRes* resources,
    const std::vector<std::unique_ptr<HardwareDisplayControllerInfo>>& displays, const std::vector<ScopedDrmPlanePtr>& planes)
{
    ...
    for (int i = 0; i < connector->count_encoders; ++i) {
        ...
        for (int j = 0; j < resources->count_crtcs; ++j) {
            ...
            int crtc_bit = 1 << j;
            if (!(encoder->possible_crtcs & crtc_bit) || IsCrtcInUse(resources->crtcs[j], displays))
                continue;

            int supported_planes = base::ranges::count_if(planes, [crtc_bit](const ScopedDrmPlanePtr& p) { return p->possible_crtcs & crtc_bit; });
            if (supported_planes > most_crtc_planes || (supported_planes == most_crtc_planes && connected_crtc == resources->crtcs[j])) {
...
display::DisplayConnectionType GetDisplayConnectionType(drmModeConnector* connector)
{
    ...
    case DRM_MODE_CONNECTOR_VIRTUAL:
        if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDRMVirtualConnectorIsExternal)) {
...
template <typename T> int GetDrmProperty(const DrmWrapper& drm, T* object, const std::string& name, ScopedDrmPropertyPtr* property)
{
...
ScopedDrmPropertyBlobPtr GetDrmPropertyBlob(const DrmWrapper& drm, drmModeConnector* connector, const std::string& name)
{
...
display::PrivacyScreenState GetPrivacyScreenState(const DrmWrapper& drm, drmModeConnector* connector)
{
    ScopedDrmPropertyPtr sw_property;
    const int sw_index = GetDrmProperty(drm, connector, kPrivacyScreenSwStatePropertyName, &sw_property);
    ScopedDrmPropertyPtr hw_property;
    const int hw_index = GetDrmProperty(drm, connector, kPrivacyScreenHwStatePropertyName, &hw_property);
    ...
    if (sw_index >= 0 && hw_index >= 0) {
        const std::string hw_enum_value = GetNameForEnumValue(hw_property.get(), connector->prop_values[hw_index]);
        const display::PrivacyScreenState* state = GetInternalTypeValueFromDrmEnum(hw_enum_value, kPrivacyScreenStates);
    ...
    const int legacy_index = GetDrmProperty(drm, connector, kPrivacyScreenPropertyNameLegacy, &legacy_property);
    if (legacy_index >= 0) {
        const std::string legacy_enum_value = GetNameForEnumValue(legacy_property.get(), connector->prop_values[legacy_index]);
        const display::PrivacyScreenState* state = GetInternalTypeValueFromDrmEnum(legacy_enum_value, kPrivacyScreenStates);
        return state ? *state : display::kNotSupported;
    }
...
bool HasContentProtectionKey(const DrmWrapper& drm, drmModeConnector* connector)
{
    ScopedDrmPropertyPtr content_protection_key_property;
    int idx = GetDrmProperty(drm, connector, kContentProtectionKey, &content_protection_key_property);
    return idx > -1;
}

std::vector<uint64_t> GetPathTopology(const DrmWrapper& drm, drmModeConnector* connector)
{
...
std::optional<TileProperty> GetTileProperty(const DrmWrapper& drm, const std::optional<display::EdidParser>& edid_parser, drmModeConnector* connector)
{
    const ScopedDrmPropertyBlobPtr tile_blob = drm.GetPropertyBlob(connector, "TILE");
    if (!tile_blob) {
        return std::nullopt;
    }
...
display::PanelOrientation GetPanelOrientation(const DrmWrapper& drm, drmModeConnector* connector)
{
    ScopedDrmPropertyPtr property;
...
std::optional<std::string> ReadFileAndTrim(const base::FilePath& path)
{
    ...
    return std::string(base::TrimWhitespaceASCII(data, base::TrimPositions::TRIM_ALL));
}

// Sort modes in |modes_in_out| from largest to smallest as defined by
// DisplayMode::operator>().
void SortDisplayModeListDesc(display::DisplaySnapshot::DisplayModeList& modes_in_out)
{
    std::stable_sort(modes_in_out.begin(), modes_in_out.end(),
        [](const std::unique_ptr<const display::DisplayMode>& left, const std::unique_ptr<const display::DisplayMode>& right) { return *left > *right; });
}
...
const HardwareDisplayControllerInfo* GetPrimaryTileInfo(const std::vector<std::unique_ptr<HardwareDisplayControllerInfo>>& tiled_infos)
{
    if (tiled_infos.empty()) {
        return nullptr;
    }
    // 1. Filter for tile switch scale to fit capability
    std::vector<const HardwareDisplayControllerInfo*> scalable_tiles, unscalable_tiles;
    ...
    const HardwareDisplayControllerInfo* tile_closest_to_origin = primary_eligible_tiles.front();
...
bool ContainsModePtr(const display::DisplaySnapshot::DisplayModeList& modes, const display::DisplayMode* target_mode)
{
...
bool ContainsModeEq(const display::DisplaySnapshot::DisplayModeList& modes, const display::DisplayMode& target_mode)
{
    for (const auto& mode : modes) {
        if (*mode == target_mode) {
            return true;
...
void PruneTileModesNotPresentInAllTiles(
    const HardwareDisplayControllerInfo& primary_tile_info, display::DisplaySnapshot::DisplayModeList& primary_tile_modes_in_out)
{
    const std::optional<TileProperty>& primary_tile_property = primary_tile_info.tile_property();
    if (!primary_tile_property.has_value()) {
        return;
    }

    const gfx::Size& tile_size = primary_tile_property->tile_size;
    for (auto primary_tile_mode_it = primary_tile_modes_in_out.begin(); primary_tile_mode_it != primary_tile_modes_in_out.end();) {
        // Skip non-tile modes.
        if (!(*primary_tile_mode_it) || (*primary_tile_mode_it)->size() != tile_size) {
            ++primary_tile_mode_it;
            continue;
        }

        bool mode_found_in_all_tiles = true;
        for (const auto& nonprimary_tile_info : primary_tile_info.nonprimary_tile_infos()) {
            const display::DisplaySnapshot::DisplayModeList nonprimary_tile_modes = nonprimary_tile_info->GetModesOfSize(tile_size);
        ...
        if (mode_found_in_all_tiles) {
            ...
            primary_tile_mode_it = primary_tile_modes_in_out.erase(primary_tile_mode_it);
        }
    }
}
...
void PruneTileModesForIncompleteGroup(const HardwareDisplayControllerInfo& tiled_display_info, display::DisplaySnapshot::DisplayModeList& modes_in_out)
{
    const std::optional<TileProperty>& tile_property = tiled_display_info.tile_property();
    if (!tile_property.has_value()) {
        return;
    }

    const std::vector<std::unique_ptr<HardwareDisplayControllerInfo>>& nonprimary_tiles = tiled_display_info.nonprimary_tile_infos();
    // Prune all tile modes if not all tiles in the display are connected yet.
    if (tile_property->tile_layout.GetArea() != static_cast<int>(nonprimary_tiles.size()) + 1) {
        modes_in_out.erase(std::remove_if(modes_in_out.begin(), modes_in_out.end(),
                               [&tile_property](const std::unique_ptr<const display::DisplayMode>& mode) { return mode->size() == tile_property->tile_size; }),
            modes_in_out.end());
        return;
    }
}
...
void ConvertTileModesToCompositedModes(const HardwareDisplayControllerInfo& tiled_display_info, display::DisplaySnapshot::DisplayModeList& modes_in_out,
    const display::DisplayMode*& current_mode_out, const display::DisplayMode*& native_mode_out)
{
    const std::optional<TileProperty>& tile_property = tiled_display_info.tile_property();
    if (!tile_property.has_value()) {
        return;
    ...
    for (auto& mode : modes_in_out) {
        ...
        std::unique_ptr<display::DisplayMode> tile_mode = mode->CopyWithSize(GetTotalTileDisplaySize(*tile_property));
...
std::unique_ptr<HardwareDisplayControllerInfo> PopPrimaryTileInfo(
    const HardwareDisplayControllerInfo* primary_tile_info_ptr, std::vector<std::unique_ptr<HardwareDisplayControllerInfo>>& infos)
{
...
ScopedDrmPropertyPtr FindDrmProperty(const DrmWrapper& drm, drmModeObjectProperties* properties, const char* name)
{
...
{
    ScopedDrmObjectPropertyPtr crtc_props = drm.GetObjectProperties(crtc->crtc_id, DRM_MODE_OBJECT_CRTC);
...
const std::optional<float> ModeVSyncRateMin(const drmModeModeInfo& mode, const std::optional<uint16_t>& vsync_rate_min_from_edid)
{
    if (!vsync_rate_min_from_edid.has_value() || vsync_rate_min_from_edid.value() == 0) {
        return std::nullopt;
    }
    ...
    float vtotal_extended = clock_hz / (htotal * vsync_rate_min_from_edid.value());
...
gfx::Size GetMaximumCursorSize(const DrmWrapper& drm)
{
    ...
    // OS kernels.
    if (!drm.GetCapability(DRM_CAP_CURSOR_WIDTH, &width) || !drm.GetCapability(DRM_CAP_CURSOR_HEIGHT, &height)) {
...
bool IsVrrCapable(const DrmWrapper& drm, drmModeConnector* connector)
{
    ...
    const int vrr_capable_index = GetDrmProperty(drm, connector, kVrrCapablePropertyName, &vrr_capable_property);
...
bool IsVrrEnabled(const DrmWrapper& drm, drmModeCrtc* crtc)
{
    ...
    const int vrr_enabled_index = GetDrmProperty(drm, crtc_props.get(), kVrrEnabledPropertyName, &vrr_enabled_property);
    return vrr_enabled_index >= 0 && crtc_props->prop_values[vrr_enabled_index];
}

display::VariableRefreshRateState GetVariableRefreshRateState(const DrmWrapper& drm, HardwareDisplayControllerInfo* info)
{
    if (!IsVrrCapable(drm, info->connector())) {
        return display::VariableRefreshRateState::kVrrNotCapable;
    }
    if (!info->edid_parser()->vsync_rate_min().has_value() || info->edid_parser()->vsync_rate_min().value() == 0) {
        return display::VariableRefreshRateState::kVrrNotCapable;
    }
...
std::pair<std::vector<std::unique_ptr<HardwareDisplayControllerInfo>>, std::vector<uint32_t>> GetDisplayInfosAndInvalidCrtcs(const DrmWrapper& drm)
{
    ...
    for (size_t i = 0; i < count_connectors; ++i) {
        if (i >= kMaxDrmConnectors) {
            LOG(WARNING) << "Reached the current limit of " << kMaxDrmConnectors << " connectors per DRM. Ignoring the remaining "
                         << count_connectors - kMaxDrmConnectors << " connectors.";
            break;
        }

        ScopedDrmConnectorPtr connector = drm.GetConnector(resources->connectors[i]);
        ...
        // connector ID yet.
        if (!connector || !drm.GetObjectProperties(resources->connectors[i], DRM_MODE_OBJECT_CONNECTOR)) {
            continue;
        }
        ...
        if (connector->connection == DRM_MODE_CONNECTED) {
            if (connector->count_modes != 0) {
                ...
                LOG(WARNING) << "[CONNECTOR:" << connector->connector_id << "] is connected but has no modes. Connector ignored.";
            }
        }
    ...
    for (auto* connector : available_connectors) {
        std::vector<uint32_t> encoder_ids(connector->encoders, connector->encoders + connector->count_encoders);
        connector_crtcs[connector] = GetPossibleCrtcsBitmaskFromEncoders(drm, encoder_ids);
    }
    // Make sure to start assigning a crtc to the connector that supports the
    // fewest crtcs first.
    std::stable_sort(available_connectors.begin(), available_connectors.end(), [&connector_crtcs](drmModeConnector* const c1, drmModeConnector* const c2) {
        ...
        return (c1_crtcs & c2_crtcs) == c1_crtcs && c1_crtcs != c2_crtcs;
    });
    ...
    for (auto* c : available_connectors) {
        ...
        std::tie(best_crtc, connected_crtc) = GetCrtcs(drm, c, resources.get(), displays, planes);
        if (!best_crtc)
            continue;
        ...
        auto connector_iter = base::ranges::find(connectors, c, &ScopedDrmConnectorPtr::get);
        ...
        ScopedDrmPropertyBlobPtr edid_blob(GetDrmPropertyBlob(drm, connector, "EDID"));
        std::optional<display::EdidParser> edid_parser;
        if (edid_blob) {
            uint8_t* edid_blob_ptr = static_cast<uint8_t*>(edid_blob->data);
            std::vector<uint8_t> edid(edid_blob_ptr, edid_blob_ptr + edid_blob->length);
            const bool is_external = GetDisplayConnectionType(connector) != display::DISPLAY_CONNECTION_TYPE_INTERNAL;
            edid_parser = display::EdidParser(std::move(edid), is_external);
        } else {
            VLOG(1) << "Failed to get EDID blob for connector " << connector->connector_id;
        }
        ...
        displays.push_back(std::make_unique<HardwareDisplayControllerInfo>(
            std::move(*connector_iter), std::move(crtc), index, std::move(edid_parser), std::move(tile_property)));
    }

    return std::make_pair(std::move(displays), std::move(invalid_crtcs));
}

std::vector<std::unique_ptr<HardwareDisplayControllerInfo>> GetAvailableDisplayControllerInfos(const DrmWrapper& drm)
{
    return GetDisplayInfosAndInvalidCrtcs(drm).first;
}

uint32_t GetPossibleCrtcsBitmaskFromEncoders(const DrmWrapper& drm, const std::vector<uint32_t>& encoder_ids)
{
...
std::vector<uint32_t> GetPossibleCrtcIdsFromBitmask(const DrmWrapper& drm, const uint32_t possible_crtcs_bitmask)
{
...
{
    return lhs.clock == rhs.clock && lhs.hdisplay == rhs.hdisplay && lhs.vdisplay == rhs.vdisplay && lhs.vrefresh == rhs.vrefresh
        && lhs.hsync_start == rhs.hsync_start && lhs.hsync_end == rhs.hsync_end && lhs.htotal == rhs.htotal && lhs.hskew == rhs.hskew
        && lhs.vsync_start == rhs.vsync_start && lhs.vsync_end == rhs.vsync_end && lhs.vtotal == rhs.vtotal && lhs.vscan == rhs.vscan && lhs.flags == rhs.flags
        && strcmp(lhs.name, rhs.name) == 0;
}

std::unique_ptr<display::DisplayMode> CreateDisplayMode(const drmModeModeInfo& mode, const std::optional<uint16_t>& vsync_rate_min_from_edid)
{
    return std::make_unique<display::DisplayMode>(gfx::Size { mode.hdisplay, mode.vdisplay }, mode.flags & DRM_MODE_FLAG_INTERLACE, GetRefreshRate(mode),
        ModeVSyncRateMin(mode, vsync_rate_min_from_edid));
}

std::unique_ptr<drmModeModeInfo> CreateVirtualMode(const drmModeModeInfo& base_mode, float virtual_refresh_rate)
{
    if (!base_mode.htotal) {
        return nullptr;
    }
    ...
    uint16_t virtual_vtotal = std::round(clock_hz / (htotal * virtual_refresh_rate));
...
display::DisplaySnapshot::DisplayModeList ExtractDisplayModes(HardwareDisplayControllerInfo* info, const gfx::Size& active_pixel_size,
    const display::DisplayMode** out_current_mode, const display::DisplayMode** out_native_mode)
{
    ...
    *out_native_mode = nullptr;
    ...
    for (int i = 0; i < info->connector()->count_modes; ++i) {
        ...
        modes.push_back(CreateDisplayMode(mode, info->edid_parser() ? info->edid_parser()->vsync_rate_min() : std::nullopt));
...
std::unique_ptr<display::DisplaySnapshot> CreateDisplaySnapshot(const DrmWrapper& drm, HardwareDisplayControllerInfo* info, uint8_t device_index)
{
    const uint8_t display_index = display::ConnectorIndex8(device_index, info->index());
    const uint16_t connector_index = display::ConnectorIndex16(device_index, info->index());
    const gfx::Size physical_size = gfx::Size(info->connector()->mmWidth, info->connector()->mmHeight);
    const display::DisplayConnectionType type = GetDisplayConnectionType(info->connector());
    ...
    const bool is_aspect_preserving_scaling = IsAspectPreserving(drm, info->connector());
    const display::PanelOrientation panel_orientation = GetPanelOrientation(drm, info->connector());
    const display::PrivacyScreenState privacy_screen_state = GetPrivacyScreenState(drm, info->connector());
    const bool has_content_protection_key = HasContentProtectionKey(drm, info->connector());
    display::DisplaySnapshot::ColorInfo color_info;
    color_info.supports_color_temperature_adjustment = HasColorCorrectionMatrix(drm, info->crtc());
    const gfx::Size maximum_cursor_size = GetMaximumCursorSize(drm);
    const display::VariableRefreshRateState variable_refresh_rate_state = GetVariableRefreshRateState(drm, info);
    ...
    base::UmaHistogramBoolean("DrmUtil.CreateDisplaySnapshot.HasEdidBlob", edid_parser.has_value());
    const std::vector<uint8_t>& edid = edid_parser.has_value() ? edid_parser->edid_blob() : std::vector<uint8_t>();
    if (edid_parser.has_value()) {
        ...
        has_overscan = edid_parser->has_overscan_flag() && edid_parser->overscan_flag();
        ...
        if (auto sk_color_space = color_info.color_space.ToSkColorSpace()) {
            ...
            color_info.edid_primaries = skia::GetD65PrimariesFromToXYZD50Matrix(to_xyzd50);
            color_info.edid_gamma = fn.g;
        }
        base::UmaHistogramBoolean("DrmUtil.CreateDisplaySnapshot.IsHDR", color_info.color_space.IsHDR());
        color_info.bits_per_channel = std::max(edid_parser->bits_per_channel(), 0);
        base::UmaHistogramCounts100("DrmUtil.CreateDisplaySnapshot.BitsPerChannel", color_info.bits_per_channel);
    ...
    display::DisplaySnapshot::DisplayModeList modes = ExtractDisplayModes(info, active_pixel_size, &current_mode, &native_mode);

    const display::DrmFormatsAndModifiers drm_formats_and_modifiers = drm.GetFormatsAndModifiersForCrtc(info->crtc()->crtc_id);
    ...
    return std::make_unique<display::DisplaySnapshot>(port_display_id, port_display_id, edid_display_id, connector_index, gfx::Point(), physical_size, type,
        base_connector_id, path_topology, is_aspect_preserving_scaling, has_overscan, privacy_screen_state, has_content_protection_key, color_info,
        display_name, drm.device_path(), std::move(modes), panel_orientation, edid, current_mode, native_mode, product_code, year_of_manufacture,
        maximum_cursor_size, variable_refresh_rate_state, drm_formats_and_modifiers);
}
...
uint64_t GetEnumValueForName(const DrmWrapper& drm, int property_id, const char* str)
{
...
{
    return gfx::Point(tiled_property.location.x() * tiled_property.tile_size.width(), tiled_property.location.y() * tiled_property.tile_size.height());
}
...
std::vector<uint64_t> ParsePathBlob(const drmModePropertyBlobRes& path_blob)
{
    ...
    std::string path_str(static_cast<char*>(path_blob.data), base::strict_cast<std::string::size_type>(path_blob.length));
    std::string_view path_string_piece(path_str);
    path_string_piece = base::TrimString(path_string_piece, std::string("\0", 1u), base::TRIM_TRAILING);

    const std::string prefix("mst:");
    if (!path_string_piece.starts_with(prefix)) {
        LOG(ERROR) << "Invalid PATH string prefix. Does not contain '" << prefix << "'. Input: '" << path_str << "'";
        return {};
    }
    path_string_piece.remove_prefix(prefix.length());

    std::vector<uint64_t> path;
    for (const auto& string_port : base::SplitStringPiece(path_string_piece, "-", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL)) {
        uint64_t int_port = 0;
        if (base::StringToUint64(string_port, &int_port) && int_port > 0) {
            path.push_back(int_port);
        } else {
            LOG(ERROR) << "One or more port values in the PATH string are invalid. Input: '" << path_str << "'";
            return {};
        }
    }

    if (path.size() < 2) {
        LOG(ERROR) << "Insufficient number of ports (should be at least 2 but found " << path.size() << "). Input: '" << path_str << "'";
        return {};
    }
...
std::optional<TileProperty> ParseTileBlob(const drmModePropertyBlobRes& tile_blob)
{
    if (!tile_blob.length) {
    ...
    const std::string tile_str(static_cast<char*>(tile_blob.data), base::strict_cast<std::string::size_type>(tile_blob.length));
    std::string_view tile_string_piece(tile_str);
    tile_string_piece = base::TrimString(tile_string_piece, std::string("\0", 1u), base::TRIM_TRAILING);

    std::vector<std::string_view> tile_properties = base::SplitStringPiece(tile_string_piece, ":", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    int num_tiles_horiz, num_tiles_vert, tile_loc_horiz, tile_loc_vert, tile_size_horiz, tile_size_vert;
    std::vector<std::pair<size_t /*tile properties index*/, int*>> tile_properties_ptrs = { { 0, &tile_property.group_id },
        // Skip {1, tile_is_single_monitor}
        { 2, &num_tiles_horiz }, { 3, &num_tiles_vert }, { 4, &tile_loc_horiz }, { 5, &tile_loc_vert }, { 6, &tile_size_horiz }, { 7, &tile_size_vert } };

    for (auto& [index, property_ptr] : tile_properties_ptrs) {
        if (!base::StringToInt(tile_properties[index], property_ptr)) {
            LOG(ERROR) << "Could not convert string \"" << tile_properties[index] << "\" at index #" << index
...
bool IsAddfb2ModifierCapable(const DrmWrapper& drm)
{
    ...
    return drm.GetCapability(DRM_CAP_ADDFB2_MODIFIERS, &addfb2_mod_cap) && addfb2_mod_cap;
}

std::string GetEnumNameForProperty(const drmModePropertyRes& property, const drmModeObjectProperties& property_values)
{
    for (uint32_t prop_idx = 0; prop_idx < property_values.count_props; ++prop_idx) {
...
std::optional<std::string> GetDrmDriverNameFromPath(const char* device_file_name)
{
...
std::vector<const char*> GetPreferredDrmDrivers()
{
    ...
    if (sys_vendor == "Apple Inc." && (product_name == "iMac12,1" || product_name == "iMac12,2")) {
        return { "radeon" };
    }
...
void ConsolidateTiledDisplayInfo(std::vector<std::unique_ptr<HardwareDisplayControllerInfo>>& display_infos)
{
    ...
    std::vector<std::unique_ptr<HardwareDisplayControllerInfo>> nontiled_display_infos;
    std::unordered_map<int /*tile_group_id*/, std::vector<std::unique_ptr<HardwareDisplayControllerInfo>>> tile_groups;
    ...
    for (auto& [_, tile_infos] : tile_groups) {
        const HardwareDisplayControllerInfo* primary_tile_info_ptr = GetPrimaryTileInfo(tile_infos);
        std::unique_ptr<HardwareDisplayControllerInfo> primary_tile_info = PopPrimaryTileInfo(primary_tile_info_ptr, tile_infos);

        for (auto& nonprimary_tile_info : tile_infos) {
            primary_tile_info->AcquireNonprimaryTileInfo(std::move(nonprimary_tile_info));
        }
...
gfx::Size GetTotalTileDisplaySize(const TileProperty& tile_property)
{
    ...
    return gfx::Size(tile_size.width() * layout.width(), tile_size.height() * layout.height());
}
