...
testing::Matcher<HardwareDisplayControllerInfo> InfoEqCrtcConnectorIds(uint32_t connector_id, uint32_t crtc_id)
{
    return AllOf(Property(&HardwareDisplayControllerInfo::connector, Pointee(Field(&drmModeConnector::connector_id, Eq(connector_id)))),
        Property(&HardwareDisplayControllerInfo::crtc, Pointee(Field(&drmModeCrtc::crtc_id, Eq(crtc_id)))));
}

testing::Matcher<HardwareDisplayControllerInfo> InfoHasTilePropertyWithLocation(const gfx::Point location)
{
    return Property(&HardwareDisplayControllerInfo::tile_property, Optional(Field(&TileProperty::location, Eq(location))));
}
...
TEST_F(DrmUtilTest, TestDisplayModesExtraction)
{
    ...
    drmModeModeInfo modes[kNumModes] = { { .hdisplay = 640, .vdisplay = 400 }, { .hdisplay = 640, .vdisplay = 480 }, { .hdisplay = 800, .vdisplay = 600 },
        { .hdisplay = 1024, .vdisplay = 768 }, { .hdisplay = 1280, .vdisplay = 768 } };
    drmModeModeInfoPtr modes_ptr = static_cast<drmModeModeInfoPtr>(drmMalloc(kNumModes * sizeof(drmModeModeInfo)));
    std::memcpy(modes_ptr, &modes[0], kNumModes * sizeof(drmModeModeInfo));

    // Initialize a connector.
    drmModeConnector connector = { .connection = DRM_MODE_CONNECTED, .subpixel = DRM_MODE_SUBPIXEL_UNKNOWN, .count_modes = 5, .modes = modes_ptr };
    drmModeConnector* connector_ptr = static_cast<drmModeConnector*>(drmMalloc(sizeof(drmModeConnector)));
    *connector_ptr = connector;

    // Initialize a CRTC.
    drmModeCrtc crtc = { .mode_valid = 1, .mode = modes[0] };
    drmModeCrtcPtr crtc_ptr = static_cast<drmModeCrtcPtr>(drmMalloc(sizeof(drmModeCrtc)));
    *crtc_ptr = crtc;

    HardwareDisplayControllerInfo info(ScopedDrmConnectorPtr(connector_ptr), ScopedDrmCrtcPtr(crtc_ptr), 0,
        /*edid_parser=*/std::nullopt);

    const display::DisplayMode* current_mode;
    const display::DisplayMode* native_mode;
    auto extracted_modes = ExtractDisplayModes(&info, gfx::Size(), &current_mode, &native_mode);
    ...
    extracted_modes = ExtractDisplayModes(&info, active_pixel_size, &current_mode, &native_mode);
    ASSERT_EQ(5u, extracted_modes.size());
    EXPECT_EQ(extracted_modes[0].get(), current_mode);
...
{
    std::unique_ptr<DrmDeviceGenerator> fake_device_generator = std::make_unique<FakeDrmDeviceGenerator>();
    scoped_refptr<DrmDevice> device = fake_device_generator->CreateDevice(base::FilePath("/test/dri/card0"), base::ScopedFD(),
    ...
    uint32_t first_encoder_id, second_encoder_id;
    {
        ...
        connector.encoders = std::vector<uint32_t> { first_encoder_id, second_encoder_id };
    }

    fake_drm->InitializeState(/*use_atomic*/ true);

    EXPECT_EQ(GetPossibleCrtcsBitmaskFromEncoders(*fake_drm, { first_encoder_id, second_encoder_id }), 0b11u);
}
...
TEST(GetPossibleCrtcsBitmaskFromEncoders, EmptyEncodersList)
{
    ...
    EXPECT_EQ(GetPossibleCrtcsBitmaskFromEncoders(*fake_drm, /*encoder_ids=*/ {}), 0u);
}
...
TEST(GetPossibleCrtcIdsFromBitmask, ValidBitmask)
{
    ...
    EXPECT_THAT(GetPossibleCrtcIdsFromBitmask(*fake_drm, 0b11), UnorderedElementsAre(crtc_1_id, crtc_2_id));
}
...
TEST(GetPossibleCrtcIdsFromBitmask, ZeroBitmask)
{
    ...
    EXPECT_THAT(GetPossibleCrtcIdsFromBitmask(*fake_drm, /*possible_crtcs_bitmask=*/0), IsEmpty());
}
...
TEST(ParseTileBlobTest, TileAtOrigin)
{
    ...
    drmModePropertyBlobRes blob = { .id = 100, .length = sizeof(tile_prop_cstr), .data = tile_prop_cstr };
...
TEST(CreateDisplaySnapshotTest, TiledDisplay)
{
    ...
    ScopedDrmPropertyBlob tile_property_blob = CreateTilePropertyBlob(*fake_drm, primary_tile_property);
    uint32_t primary_crtc_id = 0, primary_connector_id = 0;
    {
        ...
        connector.connection = true;
        connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(1920, 1080), 60 }, { gfx::Size(3840, 4320), 60 }, { gfx::Size(3840, 4320), 48 } };
        connector.encoders = std::vector<uint32_t> { encoder.id };
        fake_drm->AddProperty(primary_connector_id, { .id = kTileBlobPropId, .value = tile_property_blob->id() });
    }

    TileProperty nonprimary_tile_property = primary_tile_property;
    nonprimary_tile_property.location = gfx::Point(1, 0);
    ScopedDrmPropertyBlob nonprimary_tile_property_blob = CreateTilePropertyBlob(*fake_drm, nonprimary_tile_property);
    uint32_t nonprimary_crtc_id = 0, nonprimary_connector_id = 0;
    {
        ...
        nonprimary_connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(1920, 1080), 60 }, { gfx::Size(3840, 4320), 60 } };
        nonprimary_connector.encoders = std::vector<uint32_t> { nonprimary_encoder.id };
        fake_drm->AddProperty(nonprimary_connector_id, { .id = kTileBlobPropId, .value = nonprimary_tile_property_blob->id() });
    }

    fake_drm->InitializeState(/*use_atomic=*/true);

    HardwareDisplayControllerInfo info(fake_drm->GetConnector(primary_connector_id), fake_drm->GetCrtc(primary_crtc_id),
        /*index=*/0, std::nullopt, primary_tile_property);

    {
        std::unique_ptr<HardwareDisplayControllerInfo> nonprimary_info
            = std::make_unique<HardwareDisplayControllerInfo>(fake_drm->GetConnector(nonprimary_connector_id), fake_drm->GetCrtc(nonprimary_crtc_id),
                /*index=*/1, std::nullopt, nonprimary_tile_property);
        info.AcquireNonprimaryTileInfo(std::move(nonprimary_info));
    }
    std::unique_ptr<display::DisplaySnapshot> tile_snapshot = CreateDisplaySnapshot(*fake_drm, &info, /*device_index=*/0);

    ASSERT_NE(tile_snapshot, nullptr);
    EXPECT_THAT(tile_snapshot->modes(),
        UnorderedElementsAre(Pointee(AllOf(
                                 // The mode is transformed to tile-composited mode.
                                 Property(&display::DisplayMode::size, Eq(gfx::Size(7680, 4320))), Property(&display::DisplayMode::refresh_rate, Eq(60)))),
            Pointee(AllOf(Property(&display::DisplayMode::size, Eq(gfx::Size(1920, 1080))), Property(&display::DisplayMode::refresh_rate, Eq(60))))));
}
...
TEST(ConsolidateTiledDisplayInfoTest, OnlyNontiled)
{
    ...
    fake_drm->ResetStateWithAllProperties();
    TileProperty tile_property { .group_id = 1, .tile_size = gfx::Size(3840, 4320), .tile_layout = gfx::Size(2, 1), .location = gfx::Point(0, 0) };
    ...
    infos.push_back(std::make_unique<HardwareDisplayControllerInfo>(fake_drm->GetConnector(connector_1), fake_drm->GetCrtc(crtc_1),
        /*index=*/0, std::nullopt));
    infos.push_back(std::make_unique<HardwareDisplayControllerInfo>(fake_drm->GetConnector(connector_2), fake_drm->GetCrtc(crtc_2),
        /*index=*/1, std::nullopt));

    ConsolidateTiledDisplayInfo(infos);

    ASSERT_THAT(infos, UnorderedElementsAre(Pointee(InfoEqCrtcConnectorIds(connector_1, crtc_1)), Pointee(InfoEqCrtcConnectorIds(connector_2, crtc_2))));
}
...
TEST(ConsolidateTiledDisplayInfoTest, SingleTiled)
{
    ...
    uint32_t primary_crtc_id = 0, primary_connector_id = 0;
    {
        ...
        connector.connection = true;
        connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(1920, 1080), 60 }, { gfx::Size(3840, 4320), 60 } };
        connector.encoders = std::vector<uint32_t> { encoder.id };
        fake_drm->AddProperty(primary_connector_id, { .id = kTileBlobPropId, .value = tile_property_blob->id() });
    }
    ...
    std::vector<std::unique_ptr<HardwareDisplayControllerInfo>> infos;
    infos.push_back(std::make_unique<HardwareDisplayControllerInfo>(fake_drm->GetConnector(primary_connector_id), fake_drm->GetCrtc(primary_crtc_id),
        /*index=*/0, std::nullopt, tile_property));

    ConsolidateTiledDisplayInfo(infos);

    ASSERT_THAT(infos, UnorderedElementsAre(Pointee(InfoEqCrtcConnectorIds(primary_connector_id, primary_crtc_id))));
...
TEST(ConsolidateTiledDisplayInfoTest, AllTilesPresent)
{
    ...
    fake_drm->ResetStateWithAllProperties();
    TileProperty primary_tile_property { .group_id = 1, .tile_size = gfx::Size(3840, 4320), .tile_layout = gfx::Size(2, 1), .location = gfx::Point(0, 0) };
    uint32_t primary_crtc_id = 0, primary_connector_id = 0;
    {
        auto& crtc = fake_drm->AddCrtc();
        primary_crtc_id = crtc.id;
    ...
    nonprimary_tile_property.location = gfx::Point(1, 0);
    {
        ...
        ScopedDrmPropertyBlob tile_property_blob = CreateTilePropertyBlob(*fake_drm, nonprimary_tile_property);
        fake_drm->AddProperty(nonprimary_connector_id, { .id = kTileBlobPropId, .value = tile_property_blob->id() });
    }
    ...
    infos.push_back(std::make_unique<HardwareDisplayControllerInfo>(fake_drm->GetConnector(primary_connector_id), fake_drm->GetCrtc(primary_crtc_id),
        /*index=*/0, std::nullopt, primary_tile_property));
    infos.push_back(std::make_unique<HardwareDisplayControllerInfo>(fake_drm->GetConnector(nonprimary_connector_id), fake_drm->GetCrtc(nonprimary_crtc_id),
        /*index=*/1, std::nullopt, nonprimary_tile_property));
    ...
    EXPECT_THAT(infos[0]->nonprimary_tile_infos(), UnorderedElementsAre(Pointee(InfoHasTilePropertyWithLocation(gfx::Point(1, 0)))));
}
...
TEST(ConsolidateTiledDisplayInfoTest, AllTilesPresentMultipleGroups)
{
    ...
    fake_drm->ResetStateWithAllProperties();
    TileProperty group1_primary_tile_property {
        .group_id = 1, .tile_size = gfx::Size(3840, 4320), .tile_layout = gfx::Size(2, 1), .location = gfx::Point(0, 0)
    };
    uint32_t group1_primary_crtc_id = 0, group1_primary_connector_id = 0;
    {
        ...
        connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(1920, 1080), 60 }, { gfx::Size(3840, 4320), 60 } };
        connector.encoders = std::vector<uint32_t> { encoder.id };
        ScopedDrmPropertyBlob tile_property_blob = CreateTilePropertyBlob(*fake_drm, group1_primary_tile_property);
        fake_drm->AddProperty(group1_primary_connector_id, { .id = kTileBlobPropId, .value = tile_property_blob->id() });
    }
    ...
    group1_nonprimary_tile_property.location = gfx::Point(1, 0);
    {
        ...
        ScopedDrmPropertyBlob tile_property_blob = CreateTilePropertyBlob(*fake_drm, group1_nonprimary_tile_property);
        fake_drm->AddProperty(group1_nonprimary_connector_id, { .id = kTileBlobPropId, .value = tile_property_blob->id() });
    }

    TileProperty group2_primary_tile_property {
        .group_id = 2, .tile_size = gfx::Size(3840, 4320), .tile_layout = gfx::Size(2, 1), .location = gfx::Point(0, 0)
    };
    uint32_t group2_primary_crtc_id = 0, group2_primary_connector_id = 0;
    {
        ...
        connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(1920, 1080), 60 }, { gfx::Size(3840, 4320), 60 } };
        connector.encoders = std::vector<uint32_t> { encoder.id };
        ScopedDrmPropertyBlob tile_property_blob = CreateTilePropertyBlob(*fake_drm, group2_primary_tile_property);
        fake_drm->AddProperty(group2_primary_connector_id, { .id = kTileBlobPropId, .value = tile_property_blob->id() });
    }
    ...
    group2_nonprimary_tile_property.location = gfx::Point(1, 0);
    {
        ...
        ScopedDrmPropertyBlob tile_property_blob = CreateTilePropertyBlob(*fake_drm, group2_nonprimary_tile_property);
        fake_drm->AddProperty(group2_nonprimary_connector_id, { .id = kTileBlobPropId, .value = tile_property_blob->id() });
    }
    ...
    std::vector<std::unique_ptr<HardwareDisplayControllerInfo>> infos;
    infos.push_back(
        std::make_unique<HardwareDisplayControllerInfo>(fake_drm->GetConnector(group1_primary_connector_id), fake_drm->GetCrtc(group1_primary_crtc_id),
            /*index=*/0, std::nullopt, group1_primary_tile_property));
    infos.push_back(
        std::make_unique<HardwareDisplayControllerInfo>(fake_drm->GetConnector(group1_nonprimary_connector_id), fake_drm->GetCrtc(group1_nonprimary_crtc_id),
            /*index=*/1, std::nullopt, group1_nonprimary_tile_property));
    infos.push_back(
        std::make_unique<HardwareDisplayControllerInfo>(fake_drm->GetConnector(group2_primary_connector_id), fake_drm->GetCrtc(group2_primary_crtc_id),
            /*index=*/2, std::nullopt, group2_primary_tile_property));
    infos.push_back(
        std::make_unique<HardwareDisplayControllerInfo>(fake_drm->GetConnector(group2_nonprimary_connector_id), fake_drm->GetCrtc(group2_nonprimary_crtc_id),
            /*index=*/3, std::nullopt, group2_nonprimary_tile_property));

    ConsolidateTiledDisplayInfo(infos);

    ASSERT_THAT(infos,
        UnorderedElementsAre(Pointee(InfoEqCrtcConnectorIds(group1_primary_connector_id, group1_primary_crtc_id)),
            Pointee(InfoEqCrtcConnectorIds(group2_primary_connector_id, group2_primary_crtc_id))));
    EXPECT_EQ(infos[0]->tile_property()->location, gfx::Point(0, 0));
...
TEST(ConsolidateTiledDisplayInfoTest, PreferMoreModes)
{
    ...
    fake_drm->ResetStateWithAllProperties();
    TileProperty primary_tile_property { .group_id = 1, .tile_size = gfx::Size(3840, 4320), .tile_layout = gfx::Size(2, 1), .location = gfx::Point(1, 0) };
    uint32_t primary_crtc_id = 0, primary_connector_id = 0;
    {
        auto& crtc = fake_drm->AddCrtc();
        primary_crtc_id = crtc.id;
        ...
        connector.connection = true;
        connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(1920, 1080), 30 }, { gfx::Size(1920, 1080), 60 }, { gfx::Size(3840, 4320), 60 } };
        connector.encoders = std::vector<uint32_t> { encoder.id };
        ScopedDrmPropertyBlob tile_property_blob = CreateTilePropertyBlob(*fake_drm, primary_tile_property);
        fake_drm->AddProperty(primary_connector_id, { .id = kTileBlobPropId, .value = tile_property_blob->id() });
    }
    ...
    EXPECT_THAT(infos[0]->nonprimary_tile_infos(), UnorderedElementsAre(Pointee(InfoHasTilePropertyWithLocation(gfx::Point(0, 0)))));
}
...
TEST(ConsolidateTiledDisplayInfoTest, PreferScaleToFit)
{
    ...
    uint32_t primary_crtc_id = 0, primary_connector_id = 0;
    {
        ...
        connector.connection = true;
        connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(3840, 4320), 60 } };
        connector.encoders = std::vector<uint32_t> { encoder.id };
        ScopedDrmPropertyBlob tile_property_blob = CreateTilePropertyBlob(*fake_drm, primary_tile_property);
        fake_drm->AddProperty(primary_connector_id, { .id = kTileBlobPropId, .value = tile_property_blob->id() });
    }
...
TEST(CreateDisplaySnapshotTest, TiledDisplayWithoutOtherTilesConnected)
{
    ...
    HardwareDisplayControllerInfo info(fake_drm->GetConnector(connector.id), fake_drm->GetCrtc(crtc.id),
        /*index=*/0, std::nullopt, tile_property);
    std::unique_ptr<display::DisplaySnapshot> tile_snapshot = CreateDisplaySnapshot(*fake_drm, &info, /*device_index=*/0);
...
{
    TileProperty tile_property {
        .group_id = 1, .scale_to_fit_display = true, .tile_size = gfx::Size(200, 300), .tile_layout = gfx::Size(2, 4), .location = gfx::Point(0, 0)
    };
...
TEST(IsTileModeTest, TileMode)
{
    TileProperty property
        = { .group_id = 1, .scale_to_fit_display = true, .tile_size = gfx::Size(1000, 2000), .tile_layout = gfx::Size(2, 3), .location = gfx::Point(1, 1) };
...
{
    TileProperty property = { .tile_size = gfx::Size(1000, 2000), .location = gfx::Point(0, 0) };
    EXPECT_EQ(GetTileCrtcOffset(property), gfx::Point(0, 0));
}

TEST(TileCrtcOffset, NotOrigin)
{
    TileProperty property = { .tile_size = gfx::Size(1000, 2000), .location = gfx::Point(1, 2) };
