...
TEST_F(HardwareDisplayControllerInfoTest, BasicInfo)
{
    ...
    HardwareDisplayControllerInfo info(fake_drm_->GetConnector(connector_id), fake_drm_->GetCrtc(crtc_id),
        /*index=*/1, std::nullopt);
...
TEST_F(HardwareDisplayControllerInfoTest, HasEdidParser)
{
    ...
    display::EdidParser edid_parser(std::vector<uint8_t>(kNormalDisplay, kNormalDisplay + kNormalDisplayLength));
    HardwareDisplayControllerInfo info(fake_drm_->GetConnector(connector_id), fake_drm_->GetCrtc(crtc_id),
...
TEST_F(HardwareDisplayControllerInfoTest, TileInfo)
{
    ...
    EXPECT_EQ(actual_tile_property.scale_to_fit_display, tile_property.scale_to_fit_display);
...
TEST_F(HardwareDisplayControllerInfoTest, AcquireNonprimaryTileInfo)
{
    ...
    HardwareDisplayControllerInfo primary_info(fake_drm_->GetConnector(primary_connector_id), fake_drm_->GetCrtc(primary_crtc_id),
        /*index=*/0, std::nullopt, primary_tile_property);

    EXPECT_THAT(primary_info.nonprimary_tile_infos(), IsEmpty());

    auto nonprimary_info
        = std::make_unique<HardwareDisplayControllerInfo>(fake_drm_->GetConnector(nonprimary_connector_id), fake_drm_->GetCrtc(nonprimary_crtc_id),
...
TEST_F(HardwareDisplayControllerInfoTest, GetModesOfSize)
{
    uint32_t crtc_id = 0, connector_id = 0;
    {
        ...
        connector_id = connector.id;
        connector.connection = true;
        connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(1920, 1080), 60 }, { gfx::Size(1920, 1080), 30 }, { gfx::Size(1280, 720), 60 } };
        connector.encoders = std::vector<uint32_t> { encoder.id };
    }
