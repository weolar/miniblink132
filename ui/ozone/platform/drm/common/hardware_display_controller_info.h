...
public:
    HardwareDisplayControllerInfo(ScopedDrmConnectorPtr connector, ScopedDrmCrtcPtr crtc, uint8_t index, std::optional<display::EdidParser> edid_parser,
    ...
    HardwareDisplayControllerInfo& operator=(const HardwareDisplayControllerInfo&) = delete;
    ...
    void AcquireNonprimaryTileInfo(std::unique_ptr<HardwareDisplayControllerInfo> tile_info);

    const std::vector<std::unique_ptr<HardwareDisplayControllerInfo>>& nonprimary_tile_infos() const;

    std::vector<std::unique_ptr<HardwareDisplayControllerInfo>>& nonprimary_tile_infos();

    ScopedDrmConnectorPtr ReleaseConnector()
    {
        return std::move(connector_);
    }

    display::DisplaySnapshot::DisplayModeList GetModesOfSize(const gfx::Size& size);
...
private:
    ...
    std::vector<std::unique_ptr<HardwareDisplayControllerInfo>> nonprimary_tile_infos_;
};
