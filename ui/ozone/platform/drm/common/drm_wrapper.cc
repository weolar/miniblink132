...
bool DrmCreateDumbBuffer(int fd, const SkImageInfo& info, uint32_t* handle, uint32_t* stride)
{
...
DrmWrapper::DrmWrapper(const base::FilePath& device_path, base::ScopedFD fd, bool is_primary_device)
...
bool DrmWrapper::SetCrtc(uint32_t crtc_id, uint32_t framebuffer, std::vector<uint32_t> connectors, const drmModeModeInfo& mode)
{
    DCHECK(drm_fd_.is_valid());
    DCHECK(!connectors.empty());

    TRACE_EVENT2("drm", "DrmWrapper::SetCrtc", "crtc", crtc_id, "size", gfx::Size(mode.hdisplay, mode.vdisplay).ToString());

    return !drmModeSetCrtc(drm_fd_.get(), crtc_id, framebuffer, 0, 0, connectors.data(), connectors.size(), const_cast<drmModeModeInfo*>(&mode));
}
...
ScopedDrmConnectorPtr DrmWrapper::GetConnector(uint32_t connector_id) const
{
    ...
    return ScopedDrmConnectorPtr(drmModeGetConnector(drm_fd_.get(), connector_id));
}
...
bool DrmWrapper::SetCursor(uint32_t crtc_id, uint32_t handle, const gfx::Size& size)
{
    DCHECK(drm_fd_.is_valid());
    TRACE_EVENT2("drm", "DrmWrapper::SetCursor", "crtc_id", crtc_id, "handle", handle);
...
bool DrmWrapper::CreateDumbBuffer(const SkImageInfo& info, uint32_t* handle, uint32_t* stride)
{
    DCHECK(drm_fd_.is_valid());
...
bool DrmWrapper::MapDumbBuffer(uint32_t handle, size_t size, void** pixels)
{
    ...
    *pixels = mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED, drm_fd_.get(), map_request.offset);
...
{
    DCHECK(drm_fd_.is_valid());
    TRACE_EVENT1("drm", "DrmWrapper::RemoveFramebuffer", "framebuffer", framebuffer);
    return !drmModeRmFB(drm_fd_.get(), framebuffer);
}

bool DrmWrapper::AddFramebuffer2(uint32_t width, uint32_t height, uint32_t format, uint32_t handles[4], uint32_t strides[4], uint32_t offsets[4],
    uint64_t modifiers[4], uint32_t* framebuffer, uint32_t flags)
{
    DCHECK(drm_fd_.is_valid());
    TRACE_EVENT1("drm", "DrmWrapper::AddFramebuffer", "handle", handles[0]);
    return !drmModeAddFB2WithModifiers(drm_fd_.get(), width, height, format, handles, strides, offsets, modifiers, framebuffer, flags);
}
...
bool DrmWrapper::SetGammaRamp(uint32_t crtc_id, const display::GammaCurve& curve)
{
    ...
    return (drmModeCrtcSetGamma(drm_fd_.get(), crtc_id, r.size(), &r[0], &g[0], &b[0]) == 0);
}
...
ScopedDrmObjectPropertyPtr DrmWrapper::GetObjectProperties(uint32_t object_id, uint32_t object_type) const
{
    DCHECK(drm_fd_.is_valid());
    return ScopedDrmObjectPropertyPtr(drmModeObjectGetProperties(drm_fd_.get(), object_id, object_type));
}

bool DrmWrapper::SetObjectProperty(uint32_t object_id, uint32_t object_type, uint32_t property_id, uint32_t property_value)
{
    DCHECK(drm_fd_.is_valid());
    return !drmModeObjectSetProperty(drm_fd_.get(), object_id, object_type, property_id, property_value);
}

ScopedDrmPropertyPtr DrmWrapper::GetProperty(drmModeConnector* connector, const char* name) const
{
    DCHECK(drm_fd_.is_valid());
    TRACE_EVENT2("drm", "DrmWrapper::GetProperty", "connector", connector->connector_id, "name", name);
    for (int i = 0; i < connector->count_props; ++i) {
        ScopedDrmPropertyPtr property(drmModeGetProperty(drm_fd_.get(), connector->props[i]));
        if (!property) {
            continue;
        }
...
bool DrmWrapper::SetProperty(uint32_t connector_id, uint32_t property_id, uint64_t value)
{
    DCHECK(drm_fd_.is_valid());
    return !drmModeConnectorSetProperty(drm_fd_.get(), connector_id, property_id, value);
}
...
ScopedDrmPropertyBlob DrmWrapper::CreatePropertyBlob(const void* blob, size_t size)
{
    DCHECK(drm_fd_.is_valid());
    uint32_t id = 0;
...
ScopedDrmPropertyBlob DrmWrapper::CreatePropertyBlobWithFlags(const void* blob, size_t size, uint32_t flags)
{
...
// CrOS. Remove the #if once that happens.
    ...
    ret = drmModeCreatePropertyBlobWithFlags(drm_fd_.get(), blob, size, &id, flags);
    DCHECK(!ret && id);
    return std::make_unique<DrmPropertyBlobMetadata>(this, id);
#else
    return nullptr;
...
ScopedDrmPropertyBlobPtr DrmWrapper::GetPropertyBlob(uint32_t property_id) const
{
    DCHECK(drm_fd_.is_valid());
    return ScopedDrmPropertyBlobPtr(drmModeGetPropertyBlob(drm_fd_.get(), property_id));
}

ScopedDrmPropertyBlobPtr DrmWrapper::GetPropertyBlob(drmModeConnector* connector, const char* name) const
{
    DCHECK(drm_fd_.is_valid());
    TRACE_EVENT2("drm", "DrmWrapper::GetPropertyBlob", "connector", connector->connector_id, "name", name);
    for (int i = 0; i < connector->count_props; ++i) {
        ScopedDrmPropertyPtr property(drmModeGetProperty(drm_fd_.get(), connector->props[i]));
        if (!property) {
            continue;
        ...
        if (strcmp(property->name, name) == 0 && (property->flags & DRM_MODE_PROP_BLOB)) {
            return ScopedDrmPropertyBlobPtr(drmModeGetPropertyBlob(drm_fd_.get(), connector->prop_values[i]));
        }
    }
...
display::DrmFormatsAndModifiers DrmWrapper::GetFormatsAndModifiersForCrtc(uint32_t crtc_id) const
{
...
bool DrmWrapper::CommitProperties(drmModeAtomicReq* properties, uint32_t flags, uint64_t page_flip_id)
{
    DCHECK(drm_fd_.is_valid());
    TRACE_EVENT("drm", "DrmWrapper::CommitProperties", "test", IsTestOnly(flags), "modeset", IsModeset(flags), "blocking", IsBlocking(flags), "flags", flags,
        "page_flip_id", page_flip_id);
    int result = drmModeAtomicCommit(drm_fd_.get(), properties, flags, reinterpret_cast<void*>(page_flip_id));
    ...
    if (result && errno == EBUSY && (flags & DRM_MODE_ATOMIC_NONBLOCK)) {
        ...
        TRACE_EVENT("drm", "DrmWrapper::CommitProperties(retry)", "test", IsTestOnly(flags), "modeset", IsModeset(flags), "blocking", IsBlocking(flags),
            "flags", flags, "page_flip_id", page_flip_id);
        result = drmModeAtomicCommit(drm_fd_.get(), properties, flags, reinterpret_cast<void*>(page_flip_id));
    }

    return !result;
}

bool DrmWrapper::PageFlip(uint32_t crtc_id, uint32_t framebuffer, uint64_t page_flip_id)
{
    DCHECK(drm_fd_.is_valid());
    TRACE_EVENT2("drm", "DrmWrapper::PageFlip", "crtc", crtc_id, "framebuffer", framebuffer);

    return !drmModePageFlip(drm_fd_.get(), crtc_id, framebuffer, DRM_MODE_PAGE_FLIP_EVENT, reinterpret_cast<void*>(page_flip_id));
}
