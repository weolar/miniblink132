...
class DrmWrapper {
public:
    DrmWrapper(const base::FilePath& device_path, base::ScopedFD fd, bool is_primary_device);
   ...
   *******/
    ...
    virtual bool SetCrtc(uint32_t crtc_id, uint32_t framebuffer, std::vector<uint32_t> connectors, const drmModeModeInfo& mode);
   ...
   ********/
    ...
    virtual bool SetCursor(uint32_t crtc_id, uint32_t handle, const gfx::Size& size);
   ...
   **************/
    ...
    virtual bool CreateDumbBuffer(const SkImageInfo& info, uint32_t* handle, uint32_t* stride);
   ...
   **************/
    ...
    virtual bool AddFramebuffer2(uint32_t width, uint32_t height, uint32_t format, uint32_t handles[4], uint32_t strides[4], uint32_t offsets[4],
   ...
   ************/
    ...
    virtual ScopedDrmObjectPropertyPtr GetObjectProperties(uint32_t object_id, uint32_t object_type) const;
    ...
    virtual bool SetObjectProperty(uint32_t object_id, uint32_t object_type, uint32_t property_id, uint32_t property_value);
    ...
    virtual ScopedDrmPropertyPtr GetProperty(drmModeConnector* connector, const char* name) const;

    // Sets the value of property with ID |property_id| to |value|. The property
    // is applied to the connector with ID |connector_id|.
    virtual bool SetProperty(uint32_t connector_id, uint32_t property_id, uint64_t value);
   ...
   ****************/
    ...
    virtual ScopedDrmPropertyBlob CreatePropertyBlob(const void* blob, size_t size);
    // Creates a property blob with |size| for data |blob| which user space
    // can't read back.
    virtual ScopedDrmPropertyBlob CreatePropertyBlobWithFlags(const void* blob, size_t size, uint32_t flags);
    ...
    virtual ScopedDrmPropertyBlobPtr GetPropertyBlob(drmModeConnector* connector, const char* name) const;
   ...
   *********/
    ...
    virtual display::DrmFormatsAndModifiers GetFormatsAndModifiersForCrtc(uint32_t crtc_id) const;
    ...
    base::FilePath device_path() const
    {
        return device_path_;
    }
    ...
    bool is_primary_device() const
    {
        return is_primary_device_;
    }
...
protected:
    ...
    // specific.
    bool CommitProperties(drmModeAtomicReq* properties, uint32_t flags, uint64_t page_flip_id);
