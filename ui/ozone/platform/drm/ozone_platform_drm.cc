...
public:
    ...
    void AddInterfaces(mojo::BinderMap* binders) override
    {
        if (single_process()) {
            ...
            binders->Add<ozone::mojom::DrmDevice>(
                base::BindRepeating(&OzonePlatformDrm::CreateDrmDeviceReceiverOnGpuThread, weak_factory_.GetWeakPtr()), gpu_task_runner_);
        } else {
            ...
            binders->Add<ozone::mojom::DrmDevice>(base::BindRepeating(&OzonePlatformDrm::CreateDrmDeviceReceiverOnDrmThread, base::Unretained(this)),
                drm_thread_proxy_->GetDrmThreadTaskRunner());
        }
    }

    // Runs on the gpu thread. But the endpoint is always bound on the DRM thread.
    void CreateDrmDeviceReceiverOnGpuThread(mojo::PendingReceiver<ozone::mojom::DrmDevice> receiver)
    {
    ...
    void CreateDrmDeviceReceiverOnDrmThread(mojo::PendingReceiver<ozone::mojom::DrmDevice> receiver)
    {
        CHECK(!single_process());
        drm_thread_proxy_->AddDrmDeviceReceiver(std::move(receiver));
    }
    ...
    std::unique_ptr<PlatformWindow> CreatePlatformWindow(PlatformWindowDelegate* delegate, PlatformWindowInitProperties properties) override
    {
        ...
        auto platform_window = std::make_unique<DrmWindowHost>(
            delegate, properties.bounds, adapter, event_factory_ozone_.get(), cursor_.get(), window_manager_.get(), display_manager_.get());
    ...
    std::unique_ptr<InputMethod> CreateInputMethod(ImeKeyEventDispatcher* ime_key_event_dispatcher, gfx::AcceleratedWidget) override
    {
#if BUILDFLAG(IS_CHROMEOS_ASH)
    ...
    bool InitializeUI(const InitParams& args) override
    {
        ...
        event_factory_ozone_
            = std::make_unique<EventFactoryEvdev>(cursor_.get(), device_manager_.get(), KeyboardLayoutEngineManager::GetKeyboardLayoutEngine());
        ...
        display_manager_ = std::make_unique<DrmDisplayHostManager>(adapter, device_manager_.get(), &host_properties_, event_factory_ozone_->input_controller());
    ...
    void InitializeGPU(const InitParams& args) override
    {
        ...
        surface_factory_ = std::make_unique<GbmSurfaceFactory>(drm_thread_proxy_.get());
        ...
        overlay_manager_ = std::make_unique<DrmOverlayManagerGpu>(
            drm_thread_proxy_.get(), args.handle_overlays_swap_failure, args.allow_sync_and_real_buffer_page_flip_testing);
        ...
        if (host_thread_ == base::PlatformThread::CurrentRef()) {
            ...
            drm_thread_proxy_->AddDrmDeviceReceiver(drm_device.InitWithNewPipeAndPassReceiver());
            drm_device_connector_->ConnectSingleThreaded(std::move(drm_device));
        }
    }

    void PostCreateMainMessageLoop(base::OnceCallback<void()> shutdown_cb, scoped_refptr<base::SingleThreadTaskRunner> user_input_task_runner) override
    {
        event_factory_ozone_->SetUserInputTaskRunner(std::move(user_input_task_runner));
    }
...
private:
    ...
    void StartDrmThread(bool blocking)
    {
        if (blocking) {
            ...
            drm_thread_proxy_->StartDrmThread(base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&done_event)));
            done_event.Wait();
            ...
            // safe.
            auto safe_receiver_request_drainer = CreateSafeOnceCallback(base::BindOnce(&OzonePlatformDrm::DrainReceiverRequests, base::Unretained(this)));
            drm_thread_proxy_->StartDrmThread(std::move(safe_receiver_request_drainer));
        }
    }
    ...
    std::vector<mojo::PendingReceiver<ozone::mojom::DrmDevice>> pending_gpu_adapter_receivers_;
