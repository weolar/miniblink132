...
public:
    ...
    UIControlsSystemInputInjector& operator=(const UIControlsSystemInputInjector&) = delete;

    ~UIControlsSystemInputInjector() override = default;

    bool SendKeyEvents(gfx::NativeWindow window, ui::KeyboardCode key, int key_event_types, int accelerator_state) override;
    ...
    bool SendMouseMoveNotifyWhenDone(int screen_x, int screen_y, base::OnceClosure closure) override;

    bool SendMouseEvents(ui_controls::MouseButton type, int button_state, int accelerator_state) override;

    bool SendMouseEventsNotifyWhenDone(ui_controls::MouseButton type, int button_state, base::OnceClosure closure, int accelerator_state) override;
    ...
    bool SendTouchEventsNotifyWhenDone(int action, int id, int x, int y, base::OnceClosure task) override
    {
        NOTIMPLEMENTED();
        return false;
    }
...
bool UIControlsSystemInputInjector::SendKeyEvents(gfx::NativeWindow window, ui::KeyboardCode key, int key_event_types, int accelerator_state)
{
...
bool UIControlsSystemInputInjector::SendMouseMoveNotifyWhenDone(int screen_x, int screen_y, base::OnceClosure closure)
{
...
bool UIControlsSystemInputInjector::SendMouseEvents(ui_controls::MouseButton type, int button_state, int accelerator_state)
{
...
bool UIControlsSystemInputInjector::SendMouseEventsNotifyWhenDone(
    ui_controls::MouseButton type, int button_state, base::OnceClosure closure, int accelerator_state)
{
...
bool UIControlsSystemInputInjector::SendMouseClick(ui_controls::MouseButton type)
{
...
bool UIControlsSystemInputInjector::ScreenToHostCoord(gfx::PointF* location)
{
    ...
    const display::Display display = display::Screen::GetScreen()->GetDisplayNearestPoint(gfx::ToFlooredPoint(*location));
    if (!display.is_valid()) {
...
void UIControlsSystemInputInjector::InjectAcceleratorEvents(int accelerator_state, bool down)
{
...
{
    if (!input_injector_) {
        input_injector_ = ui::OzonePlatform::GetInstance()->CreateSystemInputInjector();
