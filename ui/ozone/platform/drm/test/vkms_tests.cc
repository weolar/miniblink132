...
public:
    ...
    void SetUp() override
    {
        ...
        drm_thread_proxy_->AddDrmDeviceReceiver(drm_device_.BindNewPipeAndPassReceiver());
        run_loop.Run();
...
protected:
    ui::MovableDisplaySnapshots RefreshDisplays()
    {
        ...
        base::RunLoop run_loop;
        auto callback = base::BindLambdaForTesting([&](ui::MovableDisplaySnapshots snapshots) {
    ...
    ui::MovableDisplaySnapshots SetupDisplaysHorizontally()
    {
        ...
        base::RunLoop run_loop;
        auto callback = base::BindLambdaForTesting([&run_loop](const std::vector<display::DisplayConfigurationParams>& request_results, bool success) {
            EXPECT_TRUE(success) << "Unable to set up displays.";
            run_loop.Quit();
        });
        drm_device_->ConfigureNativeDisplays(params, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset }, callback);
        run_loop.Run();

        return RefreshDisplays();
    }

    void FlipExpectingRecreateBuffers(const std::unique_ptr<ui::DrmWindowProxy>& window_proxy)
    {
        base::RunLoop run_loop;
        auto submission_callback
            = base::BindOnce([](gfx::SwapResult result, gfx::GpuFenceHandle handle) { EXPECT_EQ(result, gfx::SwapResult::SWAP_NAK_RECREATE_BUFFERS); });
        auto presentation_callback = base::BindLambdaForTesting([&run_loop](const gfx::PresentationFeedback& feedback) {
            EXPECT_TRUE(feedback.failed());
            run_loop.Quit();
        });
        auto planes = std::vector<ui::DrmOverlayPlane>();
        window_proxy->SchedulePageFlip(std::move(planes), std::move(submission_callback), std::move(presentation_callback));
        run_loop.Run();
    }
...
TEST_F(VKMSTest, SinglePlanePageFlip)
{
    ...
    const auto kWindowRect = gfx::Rect(displays[0]->origin(), displays[0]->current_mode()->size());
    ...
    drm_thread_proxy_->CreateBuffer(kWidget, kWindowRect.size(), kWindowRect.size(), gfx::BufferFormat::BGRX_8888, gfx::BufferUsage::SCANOUT_CPU_READ_WRITE,
        /*flags=*/0, &buffer, &framebuffer);

    auto planes = std::vector<ui::DrmOverlayPlane>();
    planes.push_back(ui::DrmOverlayPlane::TestPlane(framebuffer, gfx::ColorSpace::CreateSRGB(), std::make_unique<gfx::GpuFence>(gfx::GpuFenceHandle())));

    base::RunLoop run_loop;
    auto submission_callback = base::BindOnce([](gfx::SwapResult result, gfx::GpuFenceHandle handle) { EXPECT_EQ(result, gfx::SwapResult::SWAP_ACK); });
    auto presentation_callback = base::BindLambdaForTesting([&](const gfx::PresentationFeedback& feedback) {
