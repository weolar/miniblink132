...
public:
    ...
    static scoped_refptr<DrmFramebuffer> AddFramebuffer(scoped_refptr<DrmDevice> drm_device, AddFramebufferParams params);

    static scoped_refptr<DrmFramebuffer> AddFramebuffer(scoped_refptr<DrmDevice> drm_device, const GbmBuffer* buffer, const gfx::Size& framebuffer_size,
        std::vector<uint64_t> preferred_modifiers = std::vector<uint64_t>(), bool is_original_buffer = true);

    DrmFramebuffer(scoped_refptr<DrmDevice> drm_device, uint32_t framebuffer_id, uint32_t framebuffer_pixel_format, uint32_t opaque_framebuffer_id,
        uint32_t opaque_framebuffer_pixel_format, uint64_t format_modifier, std::vector<uint64_t> preferred_modifiers, const gfx::Size& size,
        bool is_original_buffer);

    // ID allocated by the KMS API when the buffer is registered (via the handle).
    uint32_t framebuffer_id() const
    {
    ...
    uint32_t is_original_buffer() const
    {
        return is_original_buffer_;
    }
    ...
    uint64_t format_modifier() const
    {
    ...
    const scoped_refptr<DrmDevice>& drm_device() const
    {
        return drm_device_;
    }
