...
void CompletePageFlip(base::WeakPtr<HardwareDisplayController> hardware_display_controller_, int modeset_sequence, PresentationOnceCallback callback,
    DrmOverlayPlaneList plane_list, const gfx::PresentationFeedback& presentation_feedback)
{
    if (hardware_display_controller_) {
        hardware_display_controller_->OnPageFlipComplete(modeset_sequence, std::move(plane_list), presentation_feedback);
    }
...
template <typename T> std::string NumberToHexString(const T value)
{
    static_assert(std::is_unsigned<T>::value, "Can only convert unsigned ints to hex");
...
{
    return modifier == DRM_FORMAT_MOD_ARM_AFBC(AFBC_FORMAT_MOD_BLOCK_SIZE_16x16 | AFBC_FORMAT_MOD_SPARSE | AFBC_FORMAT_MOD_YTR);
}

std::unique_ptr<DrmDumbBuffer> MakeCursorDrmBuffer(gfx::Size size, scoped_refptr<DrmDevice> drm_device)
{
...
HardwareDisplayController::HardwareDisplayController(
    std::unique_ptr<CrtcController> controller, const gfx::Point& origin, raw_ptr<DrmModifiersFilter> drm_modifiers_filter)
    : origin_(origin)
...
void HardwareDisplayController::GetModesetProps(
    CommitRequest* commit_request, const DrmOverlayPlaneList& modeset_planes, const drmModeModeInfo& mode, bool enable_vrr)
{
    GetModesetPropsForCrtcs(commit_request, modeset_planes,
        /*use_current_crtc_mode=*/false, mode, enable_vrr);
}

void HardwareDisplayController::GetEnableProps(CommitRequest* commit_request, const DrmOverlayPlaneList& modeset_planes)
{
...
void HardwareDisplayController::GetModesetPropsForCrtcs(CommitRequest* commit_request, const DrmOverlayPlaneList& modeset_planes, bool use_current_crtc_mode,
    const drmModeModeInfo& mode, std::optional<bool> enable_vrr)
{
    ...
    for (const auto& controller : crtc_controllers_) {
        drmModeModeInfo modeset_mode = use_current_crtc_mode ? controller->mode() : mode;

        if (ShouldDisableNonprimaryTileController(*controller, modeset_mode, use_current_crtc_mode)) {
            if (controller->is_enabled()) {
                CrtcCommitRequest request = CrtcCommitRequest::DisableCrtcRequest(controller->crtc(), controller->connector());
        ...
        CrtcCommitRequest request = CrtcCommitRequest::EnableCrtcRequest(controller->crtc(), controller->connector(), modeset_mode, origin_,
            &owned_hardware_planes_, std::move(overlays), enable_vrr.value_or(controller->vrr_enabled()));
        commit_request->push_back(std::move(request));
    }
}

bool HardwareDisplayController::ShouldDisableNonprimaryTileController(
    const CrtcController& controller, const drmModeModeInfo& mode, const bool use_current_crtc_mode) const
{
    const bool is_nonprimary_tile = IsTiled() &&
    ...
    const bool is_same_mode_as_primary_controller = SameMode(crtc_controllers_[0]->mode(), controller.mode());
    should_disable = should_disable || (use_current_crtc_mode && !is_same_mode_as_primary_controller);
...
{
    for (const auto& controller : crtc_controllers_) {
        CrtcCommitRequest request = CrtcCommitRequest::DisableCrtcRequest(controller->crtc(), controller->connector(), &owned_hardware_planes_);
        commit_request->push_back(std::move(request));
    }
}

void HardwareDisplayController::UpdateState(const CrtcCommitRequest& crtc_request)
{
    watchdog_.Disarm();
...
void HardwareDisplayController::SchedulePageFlip(
    DrmOverlayPlaneList plane_list, SwapCompletionOnceCallback submission_callback, PresentationOnceCallback presentation_callback)
{
    DCHECK(!page_flip_request_);
    TRACE_EVENT0("drm", "HDC::SchedulePageFlip");
    scoped_refptr<PageFlipRequest> page_flip_request = base::MakeRefCounted<PageFlipRequest>(GetRefreshInterval());
    gfx::GpuFenceHandle release_fence;

    PageFlipResult result = ScheduleOrTestPageFlip(plane_list, page_flip_request, &release_fence);
    UMA_HISTOGRAM_ENUMERATION("Compositing.Display.HardwareDisplayController.SchedulePageFlipResult", result);
    ...
    if (PageFlipResult::kFailedPlaneAssignment == result) {
        ...
        for (const auto& plane : plane_list) {
            ...
            if (plane.buffer && plane.buffer->modeset_sequence_id_at_allocation() < plane.buffer->drm_device()->modeset_sequence_id()) {
                std::move(submission_callback)
                    .Run(gfx::SwapResult::SWAP_NAK_RECREATE_BUFFERS,
                        /*release_fence=*/gfx::GpuFenceHandle());
                std::move(presentation_callback).Run(gfx::PresentationFeedback::Failure());
    ...
    std::move(submission_callback).Run(gfx::SwapResult::SWAP_ACK, std::move(release_fence));
    ...
    page_flip_request->TakeCallback(base::BindOnce(
        &CompletePageFlip, weak_ptr_factory_.GetWeakPtr(), GetDrmDevice()->modeset_sequence_id(), std::move(presentation_callback), std::move(plane_list)));
    page_flip_request_ = std::move(page_flip_request);
}

bool HardwareDisplayController::TestPageFlip(const DrmOverlayPlaneList& plane_list)
{
    TRACE_EVENT0("drm", "HDC::TestPageFlip");
    return PageFlipResult::kSuccess == ScheduleOrTestPageFlip(plane_list, nullptr, nullptr);
}

HardwareDisplayController::PageFlipResult HardwareDisplayController::ScheduleOrTestPageFlip(
    const DrmOverlayPlaneList& plane_list, scoped_refptr<PageFlipRequest> page_flip_request, gfx::GpuFenceHandle* release_fence)
{
    ...
    std::sort(pending_planes.begin(), pending_planes.end(), [](const DrmOverlayPlane& l, const DrmOverlayPlane& r) { return l.z_order < r.z_order; });
    GetDrmDevice()->plane_manager()->BeginFrame(&owned_hardware_planes_);
    ...
    for (const auto& controller : crtc_controllers_) {
        ...
        if (!controller->AssignOverlayPlanes(&owned_hardware_planes_, pending_planes, /*is_modesetting=*/false))
            return PageFlipResult::kFailedPlaneAssignment;
    }

    bool commit_success = GetDrmDevice()->plane_manager()->Commit(&owned_hardware_planes_, page_flip_request, release_fence);

    return commit_success ? PageFlipResult::kSuccess : PageFlipResult::kFailedCommit;
}

bool HardwareDisplayController::TestSeamlessMode(int32_t crtc_id, const drmModeModeInfo& mode)
{
    return GetDrmDevice()->plane_manager()->TestSeamlessMode(crtc_id, mode);
}

std::vector<uint64_t> HardwareDisplayController::GetFormatModifiers(uint32_t fourcc_format) const
{
    if (crtc_controllers_.empty())
        return std::vector<uint64_t>();

    std::vector<uint64_t> modifiers = crtc_controllers_[0]->GetFormatModifiers(fourcc_format);

    if (drm_modifiers_filter_) {
        gfx::BufferFormat buffer_format = GetBufferFormatFromFourCCFormat(fourcc_format);
        modifiers = drm_modifiers_filter_->Filter(buffer_format, modifiers);
    }

    for (size_t i = 1; i < crtc_controllers_.size(); ++i) {
        std::vector<uint64_t> other = crtc_controllers_[i]->GetFormatModifiers(fourcc_format);
        std::vector<uint64_t> intersection;

        std::set_intersection(modifiers.begin(), modifiers.end(), other.begin(), other.end(), std::back_inserter(intersection));
...
std::vector<uint64_t> HardwareDisplayController::GetSupportedModifiers(uint32_t fourcc_format, bool is_modeset) const
{
...
std::vector<uint64_t> HardwareDisplayController::GetFormatModifiersForTestModeset(uint32_t fourcc_format)
{
...
void HardwareDisplayController::UpdatePreferredModifierForFormat(gfx::BufferFormat buffer_format, uint64_t modifier)
{
    uint32_t fourcc_format = GetFourCCFormatFromBufferFormat(buffer_format);
    preferred_format_modifier_[fourcc_format] = modifier;

    uint32_t opaque_fourcc_format = GetFourCCFormatForOpaqueFramebuffer(buffer_format);
...
void HardwareDisplayController::AddCrtc(std::unique_ptr<CrtcController> controller)
{
    ...
    const std::vector<std::unique_ptr<HardwareDisplayPlane>>& all_planes = drm->plane_manager()->planes();
...
std::unique_ptr<CrtcController> HardwareDisplayController::RemoveCrtc(const scoped_refptr<DrmDevice>& drm, uint32_t crtc)
{
    auto controller_it = base::ranges::find_if(crtc_controllers_,
        [drm, crtc](const std::unique_ptr<CrtcController>& crtc_controller) { return crtc_controller->drm() == drm && crtc_controller->crtc() == crtc; });
    ...
    auto first_plane_to_disable_it = std::partition(owned_hardware_planes_.old_plane_list.begin(), owned_hardware_planes_.old_plane_list.end(),
        [crtc](const HardwareDisplayPlane* plane) { return plane->owning_crtc() != crtc; });
    ...
    std::copy(first_plane_to_disable_it, owned_hardware_planes_.old_plane_list.end(), std::back_inserter(hardware_plane_list.old_plane_list));
    drm->plane_manager()->DisableOverlayPlanes(&hardware_plane_list);

    // Remove the planes assigned to |crtc|.
    owned_hardware_planes_.old_plane_list.erase(first_plane_to_disable_it, owned_hardware_planes_.old_plane_list.end());

    return controller;
}

bool HardwareDisplayController::HasCrtc(const scoped_refptr<DrmDevice>& drm, uint32_t crtc) const
{
    for (const auto& controller : crtc_controllers_) {
...
void HardwareDisplayController::OnPageFlipComplete(
    int modeset_sequence, DrmOverlayPlaneList pending_planes, const gfx::PresentationFeedback& presentation_feedback)
{
    ...
    for (const auto& controller : crtc_controllers_) {
        ...
        if (modeset_sequence == GetDrmDevice()->modeset_sequence_id()) {
            GetDrmDevice()->plane_manager()->ResetModesetStateForCrtc(controller->crtc());
        }
    }
    page_flip_request_ = nullptr;
}

void HardwareDisplayController::WriteIntoTrace(perfetto::TracedValue context) const
{
...
void HardwareDisplayController::OnModesetComplete(const DrmOverlayPlaneList& modeset_planes)
{
...
void HardwareDisplayController::AllocateCursorBuffers()
{
    ...
    for (auto& size : supported_cursor_sizes_) {
        for (int i = 0; i < kActiveBufferCount; i++) {
            cursor_buffer_map_[size].push_back(MakeCursorDrmBuffer(size, GetDrmDevice()));
        }
    }
}

DrmDumbBuffer* HardwareDisplayController::NextCursorBuffer(const SkBitmap& image)
{
...
void HardwareDisplayController::InitSupportedCursorSizes()
{
    ...
    bool use_dynamic_cursor_size = IsUseDynamicCursorSizeEnabled() && driver.has_value() && *driver == "i915";
    if (use_dynamic_cursor_size) {
        const std::vector<std::unique_ptr<HardwareDisplayPlane>>& planes = GetDrmDevice()->plane_manager()->planes();
        for (const auto& plane : planes) {
            // Currently on Intel, if there are multiple CRTCs they should all have
            // the same supported cursor sizes.
            if (plane->type() == DRM_PLANE_TYPE_CURSOR) {
                const std::vector<gfx::Size>& supported_cursor_sizes = plane->supported_cursor_sizes();
                supported_cursor_sizes_.assign(supported_cursor_sizes.begin(), supported_cursor_sizes.end());
                break;
            }
        }
    }
    ...
    if (supported_cursor_sizes_.empty()) {
        ...
        const gfx::Size max_cursor_size_supported = GetMaximumCursorSize(*GetDrmDevice());
        ...
        // cursor.
        supported_cursor_sizes_.push_back(
            gfx::Size(std::min(max_cursor_size_supported.width(), kMaxCursorBufferSize), std::min(max_cursor_size_supported.height(), kMaxCursorBufferSize)));
    }

    // Sort the supported cursor sizes in ascending order so that we can use the
    // smallest buffer.
    DCHECK(!supported_cursor_sizes_.empty());
    std::sort(supported_cursor_sizes_.begin(), supported_cursor_sizes_.end(), CursorSizeComparator());
}
