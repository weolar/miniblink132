...
protected:
    ...
    void SetUp() override
    {
        drm_thread_.Start(base::DoNothing(), std::make_unique<FakeDrmDeviceGenerator>());
        drm_thread_.task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&DrmThread::AddDrmDeviceReceiver, base::Unretained(&drm_thread_), drm_device_.BindNewPipeAndPassReceiver()));
    ...
    std::unique_ptr<base::WaitableEvent> PostStubTaskWithWaitableEvent()
    {
        ...
        auto event = std::make_unique<base::WaitableEvent>(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED);
        drm_thread_.task_runner()->PostTask(
            FROM_HERE, base::BindOnce(&DrmThread::RunTaskAfterDeviceReady, base::Unretained(&drm_thread_), std::move(task), event.get()));
        return event;
    }
    ...
    void AddGraphicsDevice()
    {
        ...
        base::File file(file_path, base::File::FLAG_OPEN | base::File::FLAG_WRITE | base::File::FLAG_READ);
        drm_device_->AddGraphicsDevice(file_path, mojo::PlatformHandle(base::ScopedFD(file.TakePlatformFile())));
    }
...
TEST_F(DrmThreadTest, CheckOverlayCapabilitiesDestroyedWindow)
{
    ...
    drm_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&DrmThread::CheckOverlayCapabilitiesSync, base::Unretained(&drm_thread_), widget, candidates, &result));
    drm_thread_.FlushForTesting();
