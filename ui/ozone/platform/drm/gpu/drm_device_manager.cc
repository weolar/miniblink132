...
DrmDeviceManager::DrmDeviceManager(std::unique_ptr<DrmDeviceGenerator> drm_device_generator)
...
bool DrmDeviceManager::AddDrmDevice(const base::FilePath& path, base::ScopedFD fd)
{
    ...
    scoped_refptr<DrmDevice> device = drm_device_generator_->CreateDevice(path, std::move(fd), !primary_device_);
    if (!device) {
...
void DrmDeviceManager::UpdateDrmDevice(gfx::AcceleratedWidget widget, const scoped_refptr<DrmDevice>& device)
{
...
scoped_refptr<DrmDevice> DrmDeviceManager::GetDrmDevice(gfx::AcceleratedWidget widget)
{
    if (widget == gfx::kNullAcceleratedWidget)
        return primary_device_;

    auto it = drm_device_map_.find(widget);
    DLOG_IF(WARNING, it == drm_device_map_.end()) << "Attempting to get device for unknown widget " << widget;
