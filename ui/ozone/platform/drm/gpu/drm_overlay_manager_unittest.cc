...
public:
    explicit TestDrmOverlayManager(bool allow_sync_and_real_buffer_page_flip_testing)
        : DrmOverlayManager(/*handle_overlays_swap_failure=*/false, allow_sync_and_real_buffer_page_flip_testing)
    {
    }
    TestDrmOverlayManager(bool handle_overlays_swap_failure, bool allow_sync_and_real_buffer_page_flip_testing)
        : DrmOverlayManager(handle_overlays_swap_failure, allow_sync_and_real_buffer_page_flip_testing)
    {
    }
    TestDrmOverlayManager()
        : TestDrmOverlayManager(false)
    {
    }
    ...
    // DrmOverlayManager:
    void SendOverlayValidationRequest(const std::vector<OverlaySurfaceCandidate>& candidates, gfx::AcceleratedWidget widget) override
    {
    ...
    std::vector<OverlayStatus> SendOverlayValidationRequestSync(const std::vector<OverlaySurfaceCandidate>& candidates, gfx::AcceleratedWidget widget) override
    {
        ...
        for (size_t i = 0; i < candidates.size(); i++) {
            status.emplace_back(candidates[i].overlay_handled ? OverlayStatus::OVERLAY_STATUS_ABLE : OverlayStatus::OVERLAY_STATUS_NOT);
        }
        return status;
    }
    void GetHardwareCapabilities(gfx::AcceleratedWidget widget, HardwareCapabilitiesCallback& receive_callback) override
    {
...
OverlaySurfaceCandidate CreateCandidate(const gfx::Rect& rect, int plane_z_order)
{
...
public:
    ...
    void SetUp() override
    {
        manager_.SetSupportedBufferFormats(kPrimaryWidget, { gfx::BufferFormat::YUV_420_BIPLANAR });
        manager_.SetSupportedBufferFormats(kSecondaryWidget, { gfx::BufferFormat::YUV_420_BIPLANAR });
    }
...
TEST_F(DrmOverlayManagerTest, CacheLogic)
{
    ...
    std::vector<OverlaySurfaceCandidate> candidates = { CreateCandidate(gfx::Rect(0, 0, 100, 100), 0), CreateCandidate(gfx::Rect(10, 10, 20, 20), 1) };
    ...
    manager_.UpdateCacheForOverlayCandidates(manager_.requests().front(), kPrimaryWidget, std::vector<OverlayStatus>(candidates.size(), OVERLAY_STATUS_ABLE));
...
TEST_F(DrmOverlayManagerTest, CropRectCacheLogic)
{
    ...
    std::vector<OverlaySurfaceCandidate> candidates = { CreateCandidate(gfx::Rect(0, 0, 100, 100), 0) };
    ...
    manager_.UpdateCacheForOverlayCandidates(manager_.requests().front(), kPrimaryWidget, std::vector<OverlayStatus>(candidates.size(), OVERLAY_STATUS_NOT));
    manager_.requests().clear();
...
TEST_F(DrmOverlayManagerTest, MultipleWidgetCacheSupport)
{
    ...
    // work.
    manager_.UpdateCacheForOverlayCandidates(manager_.requests().front(), kSecondaryWidget, std::vector<OverlayStatus>(candidates.size(), OVERLAY_STATUS_ABLE));
    manager_.requests().clear();
...
TEST_F(DrmOverlayManagerTest, DifferentWidgetsSameCandidatesAreDistinct)
{
    ...
    // Request Size (3).
    for (int i = 0; i < 4; ++i) {
        manager_.CheckOverlaySupport(&candidates, i % 2 ? kPrimaryWidget : kSecondaryWidget);
    }
    EXPECT_EQ(manager_.requests().size(), 0u);
}
...
TEST_F(DrmOverlayManagerTest, RequiredOverlayMultiDisplay)
{
    ...
    // on the primary.
    std::vector<OverlaySurfaceCandidate> candidates1 = { CreateCandidate(gfx::Rect(0, 0, 100, 100), 0) };
    ...
    // on the secondary.
    std::vector<OverlaySurfaceCandidate> candidates2 = { CreateCandidate(gfx::Rect(0, 0, 200, 200), 0) };
...
TEST_F(DrmOverlayManagerTest, ObservingHardwareCapabilities)
{
    ...
    HardwareCapabilitiesCallback secondary_callback = base::BindRepeating(
        ...
        &secondary_calls);
    manager_.StartObservingHardwareCapabilities(kSecondaryWidget, secondary_callback);
...
TEST_F(DrmOverlayManagerTest, MultiClipRectUnderlaySupport)
{
    ...
    std::vector<OverlaySurfaceCandidate> candidates
        = { CreateCandidate(gfx::Rect(0, 0, 100, 100), 0), CreateCandidate(gfx::Rect(10, 10, 20, 20), -2), CreateCandidate(gfx::Rect(20, 20, 20, 20), -1) };
...
TEST_F(DrmOverlayManagerTest, SupportedBufferFormat)
{
    ...
    manager.SetSupportedBufferFormats(kPrimaryWidget, { gfx::BufferFormat::BGRA_8888, gfx::BufferFormat::RGBA_8888 });
    manager.SetSupportedBufferFormats(kSecondaryWidget, { gfx::BufferFormat::YUV_420_BIPLANAR });

    std::vector<OverlaySurfaceCandidate> candidates
        = { CreateCandidate(gfx::Rect(0, 0, 150, 150), -1), CreateCandidate(gfx::Rect(0, 0, 100, 100), 0), CreateCandidate(gfx::Rect(10, 10, 20, 20), 1) };
    ...
    manager.SetSupportedBufferFormats(kPrimaryWidget, { gfx::BufferFormat::YUV_420_BIPLANAR, gfx::BufferFormat::BGRA_8888, gfx::BufferFormat::RGBA_8888 });
...
{
    base::test::SingleThreadTaskEnvironment env(base::test::TaskEnvironment::TimeSource::MOCK_TIME);
    TestDrmOverlayManager manager(
        /*handle_overlays_swap_failure=*/true,
        /*allow_sync_and_real_buffer_page_flip_testing=*/true);
    manager.SetSupportedBufferFormats(kPrimaryWidget, { gfx::BufferFormat::YUV_420_BIPLANAR });
    ...
    manager.OnSwapBuffersComplete(gfx::SwapResult::SWAP_NON_SIMPLE_OVERLAYS_FAILED);
    ...
    std::vector<OverlaySurfaceCandidate> candidates2 = { CreateCandidate(gfx::Rect(0, 0, 100, 100), 0) };
    ...
    while (!manager.GetDisallowFullscreenOverlaysEndTime().is_null()) {
        env.FastForwardBy(manager.GetDisallowFullscreenOverlaysEndTime() - time_now);
        // Break in case if something goes very wrong.
        if (--kFastForwardAttempts <= 0) {
            break;
        }
    }
    std::vector<OverlaySurfaceCandidate> candidates3 = { CreateCandidate(gfx::Rect(0, 0, 100, 100), 0) };
