...
public:
    ...
    static CrtcCommitRequest EnableCrtcRequest(uint32_t crtc_id, uint32_t connector_id, drmModeModeInfo mode, gfx::Point origin,
        HardwareDisplayPlaneList* plane_list, DrmOverlayPlaneList overlays, bool enable_vrr);

    static CrtcCommitRequest DisableCrtcRequest(uint32_t crtc_id, uint32_t connector_id, HardwareDisplayPlaneList* plane_list = nullptr);

    bool should_enable_crtc() const
    {
        return should_enable_crtc_;
    }
    ...
    const DrmOverlayPlaneList& overlays() const
    {
        return overlays_;
    }
...
private:
    CrtcCommitRequest(uint32_t crtc_id, uint32_t connector_id, drmModeModeInfo mode, gfx::Point origin, HardwareDisplayPlaneList* plane_list,
