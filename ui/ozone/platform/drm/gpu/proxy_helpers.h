...
// executing.
void PostSyncTask(const scoped_refptr<base::SingleThreadTaskRunner>& task_runner, base::OnceCallback<void(base::WaitableEvent*)> callback);
...
base::RepeatingCallback<void(Args...)> CreateSafeRepeatingCallback(base::RepeatingCallback<void(Args...)> callback, const base::Location& location = FROM_HERE)
{
    return base::BindPostTask(base::SingleThreadTaskRunner::GetCurrentDefault(), std::move(callback), location);
}
...
template <typename... Args>
base::OnceCallback<void(Args...)> CreateSafeOnceCallback(base::OnceCallback<void(Args...)> callback, const base::Location& location = FROM_HERE)
{
    return base::BindPostTask(base::SingleThreadTaskRunner::GetCurrentDefault(), std::move(callback), location);
}
