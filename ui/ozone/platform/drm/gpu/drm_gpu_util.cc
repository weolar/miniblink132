...
std::vector<CrtcConnectorPairs> BuildCrtcConnectorPermutations(const std::vector<PossibleCrtcsForConnector>& connectors,
    std::vector<PossibleCrtcsForConnector>::iterator connectors_it, base::flat_set<uint32_t /*crtc_id*/>& crtcs_used_in_current_permutation)
{
    ...
    if (connectors_it == connectors.end() - 1) {
        ...
        for (const uint32_t crtc_id : connector.possible_crtcs) {
            if (!crtcs_used_in_current_permutation.contains(crtc_id)) {
                permutations.push_back({ CrtcConnectorPair { .crtc_id = crtc_id, .connector_id = connector.connector_id } });
            }
        }
        return permutations;
    }
    ...
    for (const uint32_t crtc_id : connector.possible_crtcs) {
        ...
        std::vector<CrtcConnectorPairs> next_connector_permutations
            = BuildCrtcConnectorPermutations(connectors, connectors_it + 1, crtcs_used_in_current_permutation);
        ...
        for (auto& permutation : next_connector_permutations) {
            permutation.push_back(CrtcConnectorPair { .crtc_id = crtc_id, .connector_id = connector.connector_id });
        }
        permutations.insert(permutations.end(), next_connector_permutations.begin(), next_connector_permutations.end());
    }
...
ControllerConfigParams::ControllerConfigParams(int64_t display_id, scoped_refptr<DrmDevice> drm, uint32_t crtc, uint32_t connector, gfx::Point origin,
    std::unique_ptr<drmModeModeInfo> pmode, bool enable_vrr, uint64_t base_connector)
    : display_id(display_id)
    , drm(drm)
    , crtc(crtc)
    , connector(connector)
    , base_connector_id(base_connector ? base_connector : static_cast<uint64_t>(connector))
    , origin(origin)
    , mode(std::move(pmode))
...
ControllerConfigParams::ControllerConfigParams(const ControllerConfigParams& other)
...
bool GetDrmPropertyForName(DrmWrapper* drm, drmModeObjectProperties* properties, const std::string& name, DrmWrapper::Property* property)
{
...
bool AddPropertyIfValid(drmModeAtomicReq* property_set, uint32_t object_id, const DrmWrapper::Property& property)
{
    if (!property.id)
        return true;

    int ret = drmModeAtomicAddProperty(property_set, object_id, property.id, property.value);
    if (ret < 0) {
        LOG(ERROR) << "Failed to set property object_id=" << object_id << " property_id=" << property.id << " property_value=" << property.value
                   << " error=" << -ret;
        return false;
    }
...
ScopedDrmColorLutPtr CreateLutBlob(const display::GammaCurve& source, size_t size)
{
    ...
    ScopedDrmColorLutPtr lut(static_cast<drm_color_lut*>(malloc(sizeof(drm_color_lut) * size)));
    drm_color_lut* p = lut.get();
    for (size_t i = 0; i < size; ++i) {
        // Be robust to `size` being 1, since some tests do this.
        source.Evaluate(i / std::max(size - 1.f, 1.f), p[i].red, p[i].green, p[i].blue);
    }
    return lut;
}
...
ScopedDrmColorCtmPtr CreateCTMBlob(const skcms_Matrix3x3& color_matrix, bool negative_values_broken)
{
    ScopedDrmColorCtmPtr ctm(static_cast<drm_color_ctm*>(drmMalloc(sizeof(drm_color_ctm))));
    for (size_t i = 0; i < 9; ++i) {
        ...
        if (value < 0) {
            if (negative_values_broken) {
                ...
                ctm->matrix[i] = static_cast<uint64_t>(-value * kCtmValueScale) & kCtmValueMask;
...
ScopedDrmModeRectPtr CreateDCBlob(const gfx::Rect& rect)
{
    ...
    ScopedDrmModeRectPtr dmg_rect(static_cast<drm_mode_rect*>(malloc(sizeof(drm_mode_rect))));
    dmg_rect->x1 = rect.x();
    dmg_rect->y1 = rect.y();
...
std::vector<std::unique_ptr<HardwareDisplayControllerInfo>> GetDisplayInfosAndUpdateCrtcs(DrmWrapper& drm)
{
...
void DrmWriteIntoTraceHelper(const drmModeModeInfo& mode_info, perfetto::TracedValue context)
{
...
std::vector<CrtcConnectorPairs> GetAllCrtcConnectorPermutations(const DrmDevice& drm, const std::vector<ControllerConfigParams>& controllers_params)
{
    ...
    for (auto params : controllers_params) {
        const uint32_t possible_crtcs_bitmask = drm.plane_manager()->GetPossibleCrtcsBitmaskForConnector(params.connector);
        std::vector<uint32_t> possible_crtc_ids = GetPossibleCrtcIdsFromBitmask(drm, possible_crtcs_bitmask);
        possible_crtcs_for_connectors.push_back({ .connector_id = params.connector, .possible_crtcs = std::move(possible_crtc_ids) });
    }

    base::flat_set<uint32_t /*crtc_id*/> crtcs_used_in_current_permutation;
    std::vector<CrtcConnectorPairs> permutations
        = BuildCrtcConnectorPermutations(possible_crtcs_for_connectors, possible_crtcs_for_connectors.begin(), crtcs_used_in_current_permutation);

    return permutations;
}

void ApplyCrtcColorSpaceConversion(DrmWrapper* drm, uint32_t crtc_id, float rgb[3])
{
    // Look up all properties on this CRTC and create a helper lambda to look up
    // their blobs.
    ScopedDrmObjectPropertyPtr props(drm->GetObjectProperties(crtc_id, DRM_MODE_OBJECT_CRTC));
    if (!props) {
        return;
    }
