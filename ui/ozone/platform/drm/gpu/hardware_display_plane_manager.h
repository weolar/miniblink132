...
public:
    ...
    HardwareDisplayPlaneManager& operator=(const HardwareDisplayPlaneManager&) = delete;
    ...
    // of display hardware.
    virtual bool TestSeamlessMode(int32_t crtc_id, const drmModeModeInfo& mode) = 0;
    ...
    void SetOutputColorSpace(uint32_t crtc_id, const SkColorSpacePrimaries& primaries);

    // Sets the color temperature adjustment for a given CRTC.
    void SetColorTemperatureAdjustment(uint32_t crtc_id, const display::ColorTemperatureAdjustment& cta);

    // Sets the color calibration information for a given CRTC.
    void SetColorCalibration(uint32_t crtc_id, const display::ColorCalibration& calibration);

    // Sets the gamma adjustment for a given CRTC.
    void SetGammaAdjustment(uint32_t crtc_id, const display::GammaAdjustment& adjustment);
    ...
    virtual bool AssignOverlayPlanes(
        HardwareDisplayPlaneList* plane_list, const DrmOverlayPlaneList& overlay_list, uint32_t crtc_id, std::optional<gfx::Point> crtc_offset = std::nullopt);
    ...
    virtual bool Commit(HardwareDisplayPlaneList* plane_list, scoped_refptr<PageFlipRequest> page_flip_request, gfx::GpuFenceHandle* release_fence) = 0;
    ...
    virtual bool ValidatePrimarySize(const DrmOverlayPlane& primary, const drmModeModeInfo& mode) = 0;
    ...
    // sequence or thread).
    virtual void RequestPlanesReadyCallback(DrmOverlayPlaneList planes, base::OnceCallback<void(DrmOverlayPlaneList planes)> callback) = 0;

    // Returns all formats which can be scanned out by this PlaneManager.
    const std::vector<uint32_t>& GetSupportedFormats() const;

    std::vector<uint64_t> GetFormatModifiers(uint32_t crtc_id, uint32_t format) const;
    ...
    base::flat_set<uint32_t> ResetConnectorsCacheAndGetValidIds(const ScopedDrmResourcesPtr& resources);
...
protected:
    ...
    void UpdateCrtcAndPlaneStatesAfterModeset(const CommitRequest& commit_request);
    ...
    void DisableConnectedConnectorsToCrtcs(const ScopedDrmResourcesPtr& resources);

    virtual bool InitializePlanes() = 0;

    virtual bool SetPlaneData(HardwareDisplayPlaneList* plane_list, HardwareDisplayPlane* hw_plane, const DrmOverlayPlane& overlay, uint32_t crtc_id,
        std::optional<gfx::Point> crtc_offset, const gfx::Rect& src_rect)
        = 0;
    ...
    // |crtc_id|.
    virtual bool IsCompatible(HardwareDisplayPlane* plane, const DrmOverlayPlane& overlay, uint32_t crtc_id) const;
