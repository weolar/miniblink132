...
constexpr base::TimeDelta kDisallowSkipFullscreenOverlaysDRMTestTime = base::Hours(2);
...
std::vector<OverlaySurfaceCandidate> ToCacheKey(const std::vector<OverlaySurfaceCandidate>& candidates)
{
...
DrmOverlayManager::DrmOverlayManager(bool handle_overlays_swap_failure, bool allow_sync_and_real_buffer_page_flip_testing)
    : handle_overlays_swap_failure_(handle_overlays_swap_failure)
{
    allow_sync_and_real_buffer_page_flip_testing_ = allow_sync_and_real_buffer_page_flip_testing;
    DETACH_FROM_THREAD(thread_checker_);
}

DrmOverlayManager::~DrmOverlayManager() = default;

std::unique_ptr<OverlayCandidatesOzone> DrmOverlayManager::CreateOverlayCandidates(gfx::AcceleratedWidget widget)
{
...
void DrmOverlayManager::StartObservingHardwareCapabilities(gfx::AcceleratedWidget widget, HardwareCapabilitiesCallback receive_callback)
{
    GetHardwareCapabilities(widget, receive_callback);
    hardware_capabilities_callbacks_.emplace(widget, std::move(receive_callback));
}

void DrmOverlayManager::StopObservingHardwareCapabilities(gfx::AcceleratedWidget widget)
{
    hardware_capabilities_callbacks_.erase(widget);
}

void DrmOverlayManager::CheckOverlaySupport(std::vector<OverlaySurfaceCandidate>* candidates, gfx::AcceleratedWidget widget)
{
    ...
    if (!widgets_with_required_overlays_.empty() && !widgets_with_required_overlays_.contains(widget)) {
        return;
    }
    ...
    for (size_t i = 0; i < candidates->size(); i++) {
        overlay_reindex.emplace_back((OverlayReindexZOrder { i, (*candidates)[i].plane_z_order }));
    }
    // Create a mapping for sorted z order to candidate index.
    std::sort(overlay_reindex.begin(), overlay_reindex.end(), [](const auto& a, const auto& b) { return a.plane_z_order > b.plane_z_order; });
    ...
    for (auto& reindex : overlay_reindex) {
        ...
        if (candidate.plane_z_order < 0 && candidate.clip_rect && !gfx::RectF(*candidate.clip_rect).Contains(candidate.display_rect)) {
    ...
    if (!allow_skip_fullscreen_overlay_drm_test_ && base::TimeTicks::Now() >= disallow_fullscreen_overlays_end_time_) {
    ...
    for (size_t i = 0; i < candidates->size(); i++) {
        ...
        bool can_handle = !underlay_fail_clip[i] && CanHandleCandidate(candidate, widget);
        ...
        result_candidates.push_back(can_handle ? candidate : OverlaySurfaceCandidate());
        result_candidates.back().overlay_handled = can_handle;

        can_skip_fullscreen_overlay_drm_test = handle_overlays_swap_failure_ && allow_skip_fullscreen_overlay_drm_test_
    ...
    if (allow_sync_and_real_buffer_page_flip_testing_ && features::IsSynchronousPageFlipTestingEnabled()) {
        std::vector<OverlayStatus> status = SendOverlayValidationRequestSync(result_candidates, widget);
        size_t size = candidates->size();
        DCHECK_EQ(size, status.size());
        for (size_t i = 0; i < size; i++) {
            DCHECK(status[i] == OVERLAY_STATUS_ABLE || status[i] == OVERLAY_STATUS_NOT);
    ...
    if (widget_cache_map_it == widget_cache_map_.end()) {
        widget_cache_map_it = widget_cache_map_.emplace(widget, kMaxCacheSize).first;
    }
    OverlayCandidatesListCache& cache = widget_cache_map_it->second;
    std::vector<OverlaySurfaceCandidate> cache_key = ToCacheKey(result_candidates);
    auto iter = cache.Get(cache_key);
    if (iter == cache.end()) {
        // We can skip GPU side validation in case all candidates are invalid.
        bool needs_gpu_validation = base::ranges::any_of(result_candidates, [](OverlaySurfaceCandidate& c) { return c.overlay_handled; });
        OverlayValidationCacheValue value;
        value.status.resize(result_candidates.size(), needs_gpu_validation ? OVERLAY_STATUS_PENDING : OVERLAY_STATUS_NOT);
...
void DrmOverlayManager::RegisterOverlayRequirement(gfx::AcceleratedWidget widget, bool requires_overlay)
{
    if (requires_overlay)
...
void DrmOverlayManager::OnSwapBuffersComplete(gfx::SwapResult swap_result)
{
    ...
    const bool is_fullscreen_overlay = committed_overlay_types.size() == 1U && committed_overlay_types.front() == gfx::kFullScreen;

    const bool allow_skip_fullscreen_overlay_drm_test_prev = allow_skip_fullscreen_overlay_drm_test_;
    if (swap_result == gfx::SwapResult::SWAP_NON_SIMPLE_OVERLAYS_FAILED) {
        LOG_IF(FATAL, !handle_overlays_swap_failure_) << "Handling non-simple overlays' swap failure requires the "
        ...
        if (is_fullscreen_overlay) {
            if (allow_skip_fullscreen_overlay_drm_test_) {
                ...
                disallow_fullscreen_overlays_end_time_ = base::TimeTicks::Now() + kDisallowSkipFullscreenOverlaysDRMTestTime;
            } else {
    ...
    if (is_fullscreen_overlay && handle_overlays_swap_failure_ && allow_skip_fullscreen_overlay_drm_test_prev
        && (swap_result == gfx::SwapResult::SWAP_NON_SIMPLE_OVERLAYS_FAILED || swap_result == gfx::SwapResult::SWAP_ACK)) {
        // Record how many times fullscreen overlays failed if skipping drm test was
        // allowed.
        UMA_HISTOGRAM_BOOLEAN("Compositing.Display.DrmOverlayManager.FullscreenOverlayFailed", swap_result == gfx::SwapResult::SWAP_NON_SIMPLE_OVERLAYS_FAILED);
    }
}

void DrmOverlayManager::SetSupportedBufferFormats(gfx::AcceleratedWidget widget, base::flat_set<gfx::BufferFormat> supported_buffer_formats)
{
    per_widget_overlay_supported_buffer_formats_.insert_or_assign(widget, std::move(supported_buffer_formats));
}

void DrmOverlayManager::OnPromotedOverlayTypes(std::vector<gfx::OverlayType> promoted_overlay_types)
{
...
bool DrmOverlayManager::CanHandleCandidate(const OverlaySurfaceCandidate& candidate, gfx::AcceleratedWidget widget) const
{
    if (candidate.buffer_size.IsEmpty())
        return false;

    if (!absl::holds_alternative<gfx::OverlayTransform>(candidate.transform)
        || absl::get<gfx::OverlayTransform>(candidate.transform) == gfx::OVERLAY_TRANSFORM_INVALID) {
        return false;
    }
    ...
    if (!gfx::IsNearestRectWithinDistance(candidate.display_rect, 0.01f)) {
        VLOG(3) << "Overlay Rejected: display_rect=" << candidate.display_rect.ToString();
        return false;
    }
    ...
    if (candidate.plane_z_order >= 0 && candidate.clip_rect && !candidate.clip_rect->Contains(gfx::ToNearestRect(candidate.display_rect))) {
        VLOG(3) << "Overlay Rejected: clip_rect=" << candidate.clip_rect->ToString() << ", display_rect=" << candidate.display_rect.ToString();
        return false;
    }
...
bool DrmOverlayManager::IsBufferFormatSupported(gfx::BufferFormat required_overlay_buffer_format, gfx::AcceleratedWidget widget) const
{
    auto supported_formats_it = per_widget_overlay_supported_buffer_formats_.find(widget);
    if (supported_formats_it == per_widget_overlay_supported_buffer_formats_.end()) {
        // Supported formats are unknown.
        return false;
    }

    auto format_it = base::ranges::find_if(supported_formats_it->second,
        [required_overlay_buffer_format](const auto& supported_format) { return required_overlay_buffer_format == supported_format; });
    return format_it != supported_formats_it->second.end();
}

void DrmOverlayManager::UpdateCacheForOverlayCandidates(
    const std::vector<OverlaySurfaceCandidate>& candidates, const gfx::AcceleratedWidget widget, const std::vector<OverlayStatus>& status)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
DrmOverlayManager::OverlayValidationCacheValue::OverlayValidationCacheValue() = default;
DrmOverlayManager::OverlayValidationCacheValue::OverlayValidationCacheValue(OverlayValidationCacheValue&&) = default;
DrmOverlayManager::OverlayValidationCacheValue::~OverlayValidationCacheValue() = default;
DrmOverlayManager::OverlayValidationCacheValue& DrmOverlayManager::OverlayValidationCacheValue::operator=(OverlayValidationCacheValue&& other) = default;
