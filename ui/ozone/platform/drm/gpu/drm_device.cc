...
using DrmEventHandler = base::RepeatingCallback<void(uint32_t /* frame */, base::TimeTicks /* timestamp */, uint64_t /* id */)>;
...
bool ProcessDrmEvent(int fd, const DrmEventHandler& callback)
{
    ...
    while (idx < len) {
        ...
        case DRM_EVENT_FLIP_COMPLETE: {
            ...
            std::unique_ptr<base::trace_event::TracedValue> drm_data(new base::trace_event::TracedValue());
            ...
            TRACE_EVENT_INSTANT1("benchmark,drm", "DrmEventFlipComplete", TRACE_EVENT_SCOPE_THREAD, "data", std::move(drm_data));
            ...
            const base::TimeTicks timestamp
                = base::TimeTicks() + base::Microseconds(static_cast<int64_t>(vblank.tv_sec) * base::Time::kMicrosecondsPerSecond + vblank.tv_usec);
...
public:
    ...
    void RegisterCallback(uint64_t id, uint64_t pending_calls, DrmDevice::PageFlipCallback callback)
    {
        callbacks_.push_back({ id, static_cast<uint32_t>(pending_calls), std::move(callback) });
    }
...
private:
    void Register()
    {
        ...
        base::CurrentIOThread::Get()->WatchFileDescriptor(fd_, true, base::MessagePumpForIO::WATCH_READ, &controller_, this);
    }
    ...
    void OnFileCanReadWithoutBlocking(int fd) override
    {
        ...
        if (!ProcessDrmEvent(fd, base::BindRepeating(&DrmDevice::PageFlipManager::OnPageFlip, base::Unretained(page_flip_manager_))))
            Unregister();
    }
...
DrmDevice::DrmDevice(const base::FilePath& device_path, base::ScopedFD fd, bool is_primary_device, std::unique_ptr<GbmDevice> gbm)
...
bool DrmDevice::Initialize()
{
    ...
    if (!plane_manager_->Initialize()) {
        LOG(ERROR) << "Failed to initialize the plane manager for " << device_path().value();
...
bool DrmDevice::SetCrtc(uint32_t crtc_id, uint32_t framebuffer, std::vector<uint32_t> connectors, const drmModeModeInfo& mode)
{
...
bool DrmDevice::PageFlip(uint32_t crtc_id, uint32_t framebuffer, scoped_refptr<PageFlipRequest> page_flip_request)
{
...
bool DrmDevice::CommitProperties(drmModeAtomicReq* properties, uint32_t flags, uint32_t crtc_count, scoped_refptr<PageFlipRequest> page_flip_request)
{
    uint64_t id = 0;
    ...
    if (page_flip_request) {
        page_flip_manager_->RegisterCallback(id, crtc_count, page_flip_request->AddPageFlip());
    }
...
display::DrmFormatsAndModifiers DrmDevice::GetFormatsAndModifiersForCrtc(uint32_t crtc_id) const
{
    display::DrmFormatsAndModifiers drm_formats_and_modifiers;
    for (uint32_t format : plane_manager_->GetSupportedFormats()) {
        std::vector<uint64_t> modifiers = plane_manager_->GetFormatModifiers(crtc_id, format);
        drm_formats_and_modifiers.emplace(format, modifiers);
    }
    return drm_formats_and_modifiers;
}

int DrmDevice::modeset_sequence_id() const
{
