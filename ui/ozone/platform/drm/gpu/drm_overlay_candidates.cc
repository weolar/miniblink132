...
DrmOverlayCandidates::DrmOverlayCandidates(DrmOverlayManager* manager, gfx::AcceleratedWidget widget)
...
void DrmOverlayCandidates::CheckOverlaySupport(std::vector<OverlaySurfaceCandidate>* candidates)
{
    overlay_manager_->CheckOverlaySupport(candidates, widget_);
}

void DrmOverlayCandidates::ObserveHardwareCapabilities(HardwareCapabilitiesCallback receive_callback)
{
    overlay_manager_->StartObservingHardwareCapabilities(widget_, std::move(receive_callback));
}
...
void DrmOverlayCandidates::SetSupportedBufferFormats(base::flat_set<gfx::BufferFormat> supported_buffer_formats)
{
    overlay_manager_->SetSupportedBufferFormats(widget_, std::move(supported_buffer_formats));
}

void DrmOverlayCandidates::NotifyOverlayPromotion(std::vector<gfx::OverlayType> promoted_overlay_types)
{
