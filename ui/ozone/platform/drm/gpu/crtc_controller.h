...
public:
    CrtcController(const scoped_refptr<DrmDevice>& drm, uint32_t crtc, uint32_t connector, std::optional<TileProperty> tile_property = std::nullopt);
    ...
    const scoped_refptr<DrmDevice>& drm() const
    {
        return drm_;
    }
    bool is_enabled() const
    {
        return state_->properties.active.value;
    }
    bool vrr_enabled() const
    {
        return state_->properties.vrr_enabled.value;
    }

    std::optional<TileProperty> tile_property() const
    {
    ...
    bool AssignOverlayPlanes(HardwareDisplayPlaneList* plane_list, const DrmOverlayPlaneList& planes, bool is_modesetting);
