...
constexpr uint32_t kTestModesetFlags = DRM_MODE_ATOMIC_TEST_ONLY | DRM_MODE_ATOMIC_ALLOW_MODESET;
...
const ResolutionAndRefreshRate kStandardMode = ResolutionAndRefreshRate { gfx::Size(1920, 1080), 60u };
...
template <class T> uint32_t GetNextId(const std::vector<T>& collection, uint32_t base)
{
    uint32_t max = 0;
...
ScopedDrmObjectPropertyPtr CreatePropertyObject(const std::vector<DrmDevice::Property>& properties)
{
    ScopedDrmObjectPropertyPtr drm_properties(DrmAllocator<drmModeObjectProperties>());
    drm_properties->count_props = properties.size();
    drm_properties->props = static_cast<uint32_t*>(drmMalloc(sizeof(uint32_t) * drm_properties->count_props));
    drm_properties->prop_values = static_cast<uint64_t*>(drmMalloc(sizeof(uint64_t) * drm_properties->count_props));
...
template <class Type> Type* FindObjectById(uint32_t id, std::vector<Type>& properties)
{
...
template <class Type> const Type* FindObjectById(uint32_t id, const std::vector<Type>& properties)
{
    auto it = base::ranges::find(properties, id, &Type::id);
    return it != properties.end() ? &(*it) : nullptr;
}
...
FakeDrmDevice::ConnectorProperties::ConnectorProperties(const ConnectorProperties&) = default;
FakeDrmDevice::ConnectorProperties::~ConnectorProperties() = default;

FakeDrmDevice::EncoderProperties::EncoderProperties() = default;
FakeDrmDevice::EncoderProperties::EncoderProperties(const EncoderProperties&) = default;
FakeDrmDevice::EncoderProperties::~EncoderProperties() = default;

FakeDrmDevice::PlaneProperties::PlaneProperties() = default;
FakeDrmDevice::PlaneProperties::PlaneProperties(const PlaneProperties&) = default;
FakeDrmDevice::PlaneProperties::~PlaneProperties() = default;
...
void FakeDrmDevice::ResetStateWithAllProperties()
{
    ...
    drm_state_.property_names.insert(kCrtcRequiredPropertyNames.begin(), kCrtcRequiredPropertyNames.end());
    drm_state_.property_names.insert(kConnectorRequiredPropertyNames.begin(), kConnectorRequiredPropertyNames.end());
    drm_state_.property_names.insert(kPlaneRequiredPropertyNames.begin(), kPlaneRequiredPropertyNames.end());
    ...
    drm_state_.property_names.insert(kCrtcOptionalPropertyNames.begin(), kCrtcOptionalPropertyNames.end());
    drm_state_.property_names.insert(kConnectorOptionalPropertyNames.begin(), kConnectorOptionalPropertyNames.end());
    drm_state_.property_names.insert(kPlaneOptionalPropertyNames.begin(), kPlaneOptionalPropertyNames.end());
}

FakeDrmDevice::FakeDrmState& FakeDrmDevice::ResetStateWithDefaultObjects(size_t crtc_count, size_t planes_per_crtc, size_t movable_planes,
    std::vector<uint32_t> plane_supported_formats, std::vector<drm_format_modifier> plane_supported_format_modifiers)
{
    ...
    for (size_t i = 0; i < crtc_count; ++i) {
        ...
        AddPlane(crtc_id, DRM_PLANE_TYPE_PRIMARY, plane_supported_formats, plane_supported_format_modifiers);
        for (size_t j = 0; j < planes_per_crtc - 1; ++j) {
            AddPlane(crtc_id, DRM_PLANE_TYPE_OVERLAY, plane_supported_formats, plane_supported_format_modifiers);
        }
        AddPlane(crtc_id, DRM_PLANE_TYPE_CURSOR, plane_supported_formats, plane_supported_format_modifiers);
    }

    for (size_t i = 0; i < movable_planes; ++i) {
        AddPlane(crtc_ids, DRM_PLANE_TYPE_OVERLAY, plane_supported_formats, plane_supported_format_modifiers);
    }
...
FakeDrmDevice::ConnectorProperties& FakeDrmDevice::AddConnector()
{
    ...
    uint32_t next_connector_id = GetNextId(drm_state_.connector_properties, kConnectorIdBase);
...
{
    DCHECK(!IsInitialized());
    uint32_t next_encoder_id = GetNextId(drm_state_.encoder_properties, kEncoderIdBase);
...
std::pair<FakeDrmDevice::CrtcProperties&, FakeDrmDevice::ConnectorProperties&> FakeDrmDevice::AddCrtcAndConnector()
{
    DCHECK(!IsInitialized());
...
FakeDrmDevice::PlaneProperties& FakeDrmDevice::AddPlane(
    uint32_t crtc_id, uint32_t type, std::vector<uint32_t> supported_formats, std::vector<drm_format_modifier> supported_format_modifiers)
{
    DCHECK(!IsInitialized());
    return AddPlane(std::vector<uint32_t> { crtc_id }, type, supported_formats, supported_format_modifiers);
}

FakeDrmDevice::PlaneProperties& FakeDrmDevice::AddPlane(
    const std::vector<uint32_t>& crtc_ids, uint32_t type, std::vector<uint32_t> supported_formats, std::vector<drm_format_modifier> supported_format_modifiers)
{
    DCHECK(!IsInitialized());
    ...
    auto in_formats_blob = CreateInFormatsBlob(supported_formats, supported_format_modifiers);
    AddProperty(plane.id, { .id = kInFormatsPropId, .value = in_formats_blob->id() });
...
{
    return !connector_properties.empty() || !crtc_properties.empty() || !encoder_properties.empty();
}

FakeDrmDevice::CrtcProperties& FakeDrmDevice::AddCrtcWithPrimaryAndCursorPlanes()
{
    DCHECK(!IsInitialized());
    auto& crtc = AddCrtc();
...
FakeDrmDevice::FakeDrmDevice(std::unique_ptr<GbmDevice> gbm_device)
    : DrmDevice(base::FilePath(), base::ScopedFD(), true /* is_primary_device */, std::move(gbm_device))
{
}

FakeDrmDevice::FakeDrmDevice(const base::FilePath& path, std::unique_ptr<GbmDevice> gbm_device, bool is_primary_device)
    : DrmDevice(std::move(path), base::ScopedFD(), is_primary_device, std::move(gbm_device))
{
}
...
ScopedDrmPropertyBlob FakeDrmDevice::CreateInFormatsBlob(
    const std::vector<uint32_t>& supported_formats, const std::vector<drm_format_modifier>& supported_format_modifiers)
{
    ...
    header.modifiers_offset = header.formats_offset + sizeof(uint32_t) * header.count_formats;

    std::vector<uint8_t> data(header.modifiers_offset + sizeof(drm_format_modifier) * header.count_modifiers);
    memcpy(data.data(), &header, sizeof(header));
    memcpy(data.data() + header.formats_offset, supported_formats.data(), sizeof(uint32_t) * header.count_formats);
    memcpy(data.data() + header.modifiers_offset, supported_format_modifiers.data(), sizeof(drm_format_modifier) * header.count_modifiers);

    return CreatePropertyBlob(data.data(), data.size());
}

ScopedDrmPropertyBlob FakeDrmDevice::CreateSizeHintsBlob(const std::vector<gfx::Size>& sizes)
{
...
bool FakeDrmDevice::InitializeStateWithResult(bool use_atomic)
{
    ...
    for (FakeDrmDevice::ConnectorProperties& connector : drm_state_.connector_properties) {
        if (connector.connection && connector.modes.empty()) {
            DrmWrapper::Property* connector_link_status = FindObjectById(kLinkStatusPropId, connector.properties);
    ...
    for (auto& mock_connector : drm_state_.connector_properties) {
        ...
        auto edid_property_blob = CreatePropertyBlob(edid_blob.data(), edid_blob.size());
        UpdateProperty(mock_connector.id, kEdidBlobPropId, edid_property_blob->id());
    }

    return plane_manager_->Initialize();
}

void FakeDrmDevice::SetModifiersOverhead(base::flat_map<uint64_t /*modifier*/, int /*overhead*/> modifiers_overhead)
{
...
ScopedDrmResourcesPtr FakeDrmDevice::GetResources() const
{
    ...
    resources->crtcs = static_cast<uint32_t*>(drmMalloc(sizeof(uint32_t) * resources->count_crtcs));
    for (size_t i = 0; i < drm_state_.crtc_properties.size(); ++i)
        resources->crtcs[i] = drm_state_.crtc_properties[i].id;

    resources->count_connectors = drm_state_.connector_properties.size();
    resources->connectors = static_cast<uint32_t*>(drmMalloc(sizeof(uint32_t) * resources->count_connectors));
    ...
    resources->encoders = static_cast<uint32_t*>(drmMalloc(sizeof(uint32_t) * resources->count_encoders));
...
ScopedDrmPlaneResPtr FakeDrmDevice::GetPlaneResources() const
{
    ...
    resources->planes = static_cast<uint32_t*>(drmMalloc(sizeof(uint32_t) * resources->count_planes));
...
ScopedDrmObjectPropertyPtr FakeDrmDevice::GetObjectProperties(uint32_t object_id, uint32_t object_type) const
{
    if (object_type == DRM_MODE_OBJECT_PLANE) {
        const PlaneProperties* properties = FindObjectById(object_id, drm_state_.plane_properties);
        if (properties)
            return CreatePropertyObject(properties->properties);
    } else if (object_type == DRM_MODE_OBJECT_CRTC) {
        const CrtcProperties* properties = FindObjectById(object_id, drm_state_.crtc_properties);
        if (properties)
            return CreatePropertyObject(properties->properties);
    } else if (object_type == DRM_MODE_OBJECT_CONNECTOR) {
        const ConnectorProperties* properties = FindObjectById(object_id, drm_state_.connector_properties);
        if (properties)
            return CreatePropertyObject(properties->properties);
    }
...
{
    const CrtcProperties* mock_crtc = FindObjectById(crtc_id, drm_state_.crtc_properties);
    if (!mock_crtc)
        return nullptr;
...
bool FakeDrmDevice::SetCrtc(uint32_t crtc_id, uint32_t framebuffer, std::vector<uint32_t> connectors, const drmModeModeInfo& mode)
{
...
{
    const ConnectorProperties* mock_connector = FindObjectById(connector_id, drm_state_.connector_properties);
    if (!mock_connector)
        return nullptr;

    ScopedDrmConnectorPtr connector(DrmAllocator<drmModeConnector>());
    connector->connector_id = mock_connector->id;
    connector->connection = mock_connector->connection ? DRM_MODE_CONNECTED : DRM_MODE_DISCONNECTED;
...
{
    const EncoderProperties* mock_encoder = FindObjectById(encoder_id, drm_state_.encoder_properties);
    if (!mock_encoder)
        return nullptr;
...
bool FakeDrmDevice::AddFramebuffer2(uint32_t width, uint32_t height, uint32_t format, uint32_t handles[4], uint32_t strides[4], uint32_t offsets[4],
    uint64_t modifiers[4], uint32_t* framebuffer, uint32_t flags)
{
...
ScopedDrmFramebufferPtr FakeDrmDevice::GetFramebuffer(uint32_t framebuffer) const
{
    return ScopedDrmFramebufferPtr();
}

bool FakeDrmDevice::PageFlip(uint32_t crtc_id, uint32_t framebuffer, scoped_refptr<PageFlipRequest> page_flip_request)
{
...
{
    const PlaneProperties* properties = FindObjectById(plane_id, drm_state_.plane_properties);
    if (!properties)
        return nullptr;
...
ScopedDrmPropertyPtr FakeDrmDevice::GetProperty(drmModeConnector* connector, const char* name) const
{
...
bool FakeDrmDevice::SetProperty(uint32_t connector_id, uint32_t property_id, uint64_t value)
{
    return true;
}

ScopedDrmPropertyBlob FakeDrmDevice::CreatePropertyBlob(const void* blob, size_t size)
{
...
ScopedDrmPropertyBlobPtr FakeDrmDevice::GetPropertyBlob(uint32_t property_id) const
{
...
ScopedDrmPropertyBlobPtr FakeDrmDevice::GetPropertyBlob(drmModeConnector* connector, const char* name) const
{
    const ConnectorProperties* mock_connector = FindObjectById(connector->connector_id, drm_state_.connector_properties);
    if (!mock_connector)
        return nullptr;
...
bool FakeDrmDevice::SetObjectProperty(uint32_t object_id, uint32_t object_type, uint32_t property_id, uint32_t property_value)
{
...
bool FakeDrmDevice::SetCursor(uint32_t crtc_id, uint32_t handle, const gfx::Size& size)
{
...
bool FakeDrmDevice::CreateDumbBuffer(const SkImageInfo& info, uint32_t* handle, uint32_t* stride)
{
    ...
    *stride = info.minRowBytes();
    ...
    buffers_[*handle] = SkSurfaces::WrapPixels(
        info, pixels, *stride, [](void* pixels, void* context) { delete[] static_cast<char*>(pixels); },
...
bool FakeDrmDevice::CommitProperties(drmModeAtomicReq* request, uint32_t flags, uint32_t crtc_count, scoped_refptr<PageFlipRequest> page_flip_request)
{
    commit_count_++;
    ...
    if ((!test_only && !set_crtc_expectation_) || (flags & DRM_MODE_ATOMIC_NONBLOCK && !commit_expectation_)) {
        return false;
    }
    ...
    for (uint32_t i = 0; i < request->cursor; ++i) {
        ...
        if (item.property_id == kPlaneCrtcId) {
            if (++crtc_planes_counter[item.value] > 1 && !modeset_with_overlays_expectation_)
                return false;
        }
    }
    ...
    for (uint32_t i = 0; i < request->cursor; ++i) {
        bool res = UpdateProperty(request->items[i].object_id, request->items[i].property_id, request->items[i].value);
        if (!res)
            return false;
    }
...
bool FakeDrmDevice::SetGammaRamp(uint32_t crtc_id, const display::GammaCurve& curve)
{
...
void FakeDrmDevice::AddProperty(uint32_t object_id, const DrmWrapper::Property& property)
{
    DCHECK(!IsInitialized());
...
void FakeDrmDevice::SetPossibleValuesForEnumProperty(uint32_t property_id, std::vector<std::pair<uint64_t /* value */, std::string /* name */>> values)
{
    DCHECK(!IsInitialized());
    DCHECK(!values.empty());
...
void FakeDrmDevice::FillPossibleValuesForEnumProperty(drmModePropertyRes* property) const
{
    DCHECK(IsPropertyValueEnum(property->prop_id));

    const std::vector<std::pair<uint64_t, std::string>>& enum_values = drm_state_.enum_values.find(property->prop_id)->second;
...
bool FakeDrmDevice::UpdateProperty(uint32_t id, uint64_t value, std::vector<DrmDevice::Property>* properties, bool add_property_if_needed)
{
...
bool FakeDrmDevice::UpdateProperty(uint32_t object_id, uint32_t property_id, uint64_t value, bool add_property_if_needed)
{
    PlaneProperties* plane_properties = FindObjectById(object_id, drm_state_.plane_properties);
    if (plane_properties) {
        return UpdateProperty(property_id, value, &plane_properties->properties, add_property_if_needed);
    }

    CrtcProperties* crtc_properties = FindObjectById(object_id, drm_state_.crtc_properties);
    if (crtc_properties) {
        return UpdateProperty(property_id, value, &crtc_properties->properties, add_property_if_needed);
    }

    ConnectorProperties* connector_properties = FindObjectById(object_id, drm_state_.connector_properties);
    if (connector_properties) {
        return UpdateProperty(property_id, value, &connector_properties->properties, add_property_if_needed);
    }
...
bool FakeDrmDevice::ValidatePropertyValue(uint32_t id, uint64_t value)
{
    ...
    std::vector<std::string> blob_properties = { "CTM", "DEGAMMA_LUT", "GAMMA_LUT", "PLANE_CTM" };
