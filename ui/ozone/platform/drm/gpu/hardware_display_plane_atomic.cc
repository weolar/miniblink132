...
uint32_t OverlayTransformToDrmRotationPropertyValue(gfx::OverlayTransform transform)
{
    switch (transform) {
...
bool IsRotationTransformSupported(gfx::OverlayTransform transform, uint32_t format_fourcc, bool is_original_buffer)
{
    if ((transform == gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90) || (transform == gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_270)
        ...
        if (is_original_buffer && (format_fourcc == DRM_FORMAT_NV12 || format_fourcc == DRM_FORMAT_P010)) {
            return true;
        }
        return false;
    }
...
bool HardwareDisplayPlaneAtomic::Initialize(DrmDevice* drm)
{
    ...
    bool ret = properties_.crtc_id.id && properties_.crtc_x.id && properties_.crtc_y.id && properties_.crtc_w.id && properties_.crtc_h.id
        && properties_.fb_id.id && properties_.src_x.id && properties_.src_y.id && properties_.src_w.id && properties_.src_h.id;
    LOG_IF(ERROR, !ret) << "Failed to find all required properties for plane=" << id_;

    ret &= (properties_.plane_color_encoding.id == 0) == (properties_.plane_color_range.id == 0);
    LOG_IF(ERROR, !ret) << "Inconsistent color management properties for plane=" << id_;

    return ret;
}

bool HardwareDisplayPlaneAtomic::AssignPlaneProps(DrmDevice* drm, uint32_t crtc_id, uint32_t framebuffer, const gfx::Rect& crtc_rect, const gfx::Rect& src_rect,
    ...
    bool is_original_buffer)
{
    ...
    if (!IsRotationTransformSupported(transform, format_fourcc, is_original_buffer))
        return false;
    ...
    if (assigned_props_.rotation.id) {
        assigned_props_.rotation.value = OverlayTransformToDrmRotationPropertyValue(transform);
    }
    ...
    if (!clip_in_bounds && assigned_props_.plane_fb_damage_clips.id) {
        LOG(ERROR) << "Damage clip dmg: " << damage_rect.ToString() << " not contained inside display bounds"
    ...
    if (drm && assigned_props_.plane_fb_damage_clips.id) {
        ...
        static ScopedDrmPropertyBlob dmg_clip_blob = drm->CreatePropertyBlob(dmg_clip_blob_data.get(), sizeof(drm_mode_rect));
    ...
    if (assigned_props_.plane_color_encoding.id) {
        assigned_props_.plane_color_encoding.value
            = color_space.GetMatrixID() == gfx::ColorSpace::MatrixID::BT709 ? color_encoding_bt709_ : color_encoding_bt601_;
    }
...
{
    bool plane_set_succeeded = AddPropertyIfValid(property_set, id_, assigned_props_.crtc_id) && AddPropertyIfValid(property_set, id_, assigned_props_.crtc_x)
        && AddPropertyIfValid(property_set, id_, assigned_props_.crtc_y) && AddPropertyIfValid(property_set, id_, assigned_props_.crtc_w)
        && AddPropertyIfValid(property_set, id_, assigned_props_.crtc_h) && AddPropertyIfValid(property_set, id_, assigned_props_.fb_id)
        && AddPropertyIfValid(property_set, id_, assigned_props_.src_x) && AddPropertyIfValid(property_set, id_, assigned_props_.src_y)
        && AddPropertyIfValid(property_set, id_, assigned_props_.src_w) && AddPropertyIfValid(property_set, id_, assigned_props_.src_h);

    if (assigned_props_.rotation.id) {
        plane_set_succeeded &= AddPropertyIfValid(property_set, id_, assigned_props_.rotation);
    }

    if (assigned_props_.plane_fb_damage_clips.id) {
        plane_set_succeeded &= AddPropertyIfValid(property_set, id_, assigned_props_.plane_fb_damage_clips);
    }

    if (assigned_props_.in_fence_fd.id) {
        plane_set_succeeded &= AddPropertyIfValid(property_set, id_, assigned_props_.in_fence_fd);
    }
    ...
    if (assigned_props_.plane_color_encoding.id) {
        ...
        plane_set_succeeded &= AddPropertyIfValid(property_set, id_, assigned_props_.plane_color_encoding);
        plane_set_succeeded &= AddPropertyIfValid(property_set, id_, assigned_props_.plane_color_range);
    }
...
void HardwareDisplayPlaneAtomic::AssignDisableProps()
{
    set_in_use(false);
    set_owning_crtc(0);
    AssignPlaneProps(nullptr, 0, 0, gfx::Rect(), gfx::Rect(), gfx::Rect(), gfx::OVERLAY_TRANSFORM_NONE, gfx::ColorSpace(), base::kInvalidPlatformFile,
