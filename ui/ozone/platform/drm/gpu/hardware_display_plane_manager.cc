...
gfx::Rect OverlayPlaneToDrmSrcRect(const DrmOverlayPlane& plane)
{
    ...
    return gfx::Rect(crop_rect.x() << 16, crop_rect.y() << 16, crop_rect.width() << 16, crop_rect.height() << 16);
}

skcms_Matrix3x3 PlaneToOutputMatrix(const HardwareDisplayPlaneManager::CrtcState& crtc_state)
{
...
HardwareDisplayPlaneList::PageFlipInfo::PageFlipInfo(uint32_t crtc_id, uint32_t framebuffer)
    : crtc_id(crtc_id)
    , framebuffer(framebuffer)
{
}

HardwareDisplayPlaneList::PageFlipInfo::PageFlipInfo(const PageFlipInfo& other) = default;

HardwareDisplayPlaneList::PageFlipInfo::~PageFlipInfo() = default;

void HardwareDisplayPlaneList::WriteIntoTrace(perfetto::TracedValue context) const
{
...
HardwareDisplayPlaneManager::CrtcProperties::CrtcProperties(const CrtcProperties& other) = default;
HardwareDisplayPlaneManager::CrtcProperties::~CrtcProperties() = default;
...
bool HardwareDisplayPlaneManager::Initialize()
{
    ...
    uint64_t value = 0;
    has_universal_planes_ = drm_->GetCapability(DRM_CLIENT_CAP_UNIVERSAL_PLANES, &value) && value;
    ...
    std::sort(planes_.begin(), planes_.end(),
        [](const std::unique_ptr<HardwareDisplayPlane>& l, const std::unique_ptr<HardwareDisplayPlane>& r) { return l->id() < r->id(); });

    PopulateSupportedFormats();
    return true;
}

std::unique_ptr<HardwareDisplayPlane> HardwareDisplayPlaneManager::CreatePlane(uint32_t id)
{
    return std::make_unique<HardwareDisplayPlane>(id);
}

std::optional<int> HardwareDisplayPlaneManager::LookupCrtcIndex(uint32_t crtc_id) const
{
...
std::optional<int> HardwareDisplayPlaneManager::LookupConnectorIndex(uint32_t connector_id) const
{
...
base::flat_set<uint32_t> HardwareDisplayPlaneManager::CrtcMaskToCrtcIds(uint32_t crtc_mask) const
{
...
bool HardwareDisplayPlaneManager::IsCompatible(HardwareDisplayPlane* plane, const DrmOverlayPlane& overlay, uint32_t crtc_id) const
{
    bool ownership_compatible = plane->owning_crtc() == 0 || plane->owning_crtc() == crtc_id;
    if (plane->in_use() || !ownership_compatible || plane->type() == DRM_PLANE_TYPE_CURSOR || !plane->CanUseForCrtcId(crtc_id)) {
        return false;
    }

    const uint32_t format = overlay.enable_blend ? overlay.buffer->framebuffer_pixel_format() : overlay.buffer->opaque_framebuffer_pixel_format();
...
void HardwareDisplayPlaneManager::ResetCurrentPlaneList(HardwareDisplayPlaneList* plane_list) const
{
...
void HardwareDisplayPlaneManager::RestoreCurrentPlaneList(HardwareDisplayPlaneList* plane_list) const
{
...
void HardwareDisplayPlaneManager::BeginFrame(HardwareDisplayPlaneList* plane_list)
{
    for (auto* plane : plane_list->old_plane_list) {
        plane->set_in_use(false);
    }
...
bool HardwareDisplayPlaneManager::AssignOverlayPlanes(
    HardwareDisplayPlaneList* plane_list, const DrmOverlayPlaneList& overlay_list, uint32_t crtc_id, std::optional<gfx::Point> crtc_offset)
{
    ...
    for (const auto& plane : overlay_list) {
        ...
        if (!SetPlaneData(plane_list, hw_plane, plane, crtc_id, crtc_offset, OverlayPlaneToDrmSrcRect(plane))) {
            RestoreCurrentPlaneList(plane_list);
            return false;
        }
...
const std::vector<uint32_t>& HardwareDisplayPlaneManager::GetSupportedFormats() const
{
    return supported_formats_;
}

std::vector<uint64_t> HardwareDisplayPlaneManager::GetFormatModifiers(uint32_t crtc_id, uint32_t format) const
{
    for (const auto& plane : planes_) {
        if (plane->CanUseForCrtcId(crtc_id) && plane->type() == DRM_PLANE_TYPE_PRIMARY) {
...
base::flat_set<uint32_t> HardwareDisplayPlaneManager::ResetConnectorsCacheAndGetValidIds(const ScopedDrmResourcesPtr& resources)
{
    ...
    for (int i = 0; i < resources->count_connectors; ++i) {
        ...
        ScopedDrmObjectPropertyPtr props(drm_->GetObjectProperties(connector_id, DRM_MODE_OBJECT_CONNECTOR));
        if (!props) {
            PLOG(ERROR) << "Failed to get Connector properties for connector=" << connector_id;
            continue;
        }
        ...
        GetDrmPropertyForName(drm_, props.get(), "link-status", &state_props.link_status);

        const std::vector<uint32_t> possible_encoder_ids(connector->encoders, connector->encoders + connector->count_encoders);
        state_props.possible_crtcs_bitmask = GetPossibleCrtcsBitmaskFromEncoders(*drm_, possible_encoder_ids);
...
void HardwareDisplayPlaneManager::SetOutputColorSpace(uint32_t crtc_id, const SkColorSpacePrimaries& primaries)
{
...
void HardwareDisplayPlaneManager::SetColorSpaceForAllPlanes(uint32_t crtc_id, const SkColorSpacePrimaries& primaries)
{
    if (primaries == SkNamedPrimariesExt::kInvalid) {
...
void HardwareDisplayPlaneManager::SetColorTemperatureAdjustment(uint32_t crtc_id, const display::ColorTemperatureAdjustment& cta)
{
    CrtcState& crtc_state = CrtcStateForCrtcId(crtc_id);
...
void HardwareDisplayPlaneManager::SetColorCalibration(uint32_t crtc_id, const display::ColorCalibration& calibration)
{
    CrtcState& crtc_state = CrtcStateForCrtcId(crtc_id);
...
void HardwareDisplayPlaneManager::SetGammaAdjustment(uint32_t crtc_id, const display::GammaAdjustment& adjustment)
{
    CrtcState& crtc_state = CrtcStateForCrtcId(crtc_id);
...
void HardwareDisplayPlaneManager::SetBackgroundColor(uint32_t crtc_id, const uint64_t background_color)
{
    CrtcState& crtc_state = CrtcStateForCrtcId(crtc_id);
...
bool HardwareDisplayPlaneManager::InitializeCrtcState()
{
    ...
    for (int i = 0; i < resources->count_crtcs; ++i) {
        ...
        ScopedDrmObjectPropertyPtr props(drm_->GetObjectProperties(resources->crtcs[i], DRM_MODE_OBJECT_CRTC));
        if (!props) {
            PLOG(ERROR) << "Failed to get CRTC properties for crtc_id=" << state.properties.id;
            continue;
        }

        GetDrmPropertyForName(drm_, props.get(), "ACTIVE", &state.properties.active);
        DCHECK(!drm_->is_atomic() || state.properties.active.id);
        GetDrmPropertyForName(drm_, props.get(), "MODE_ID", &state.properties.mode_id);
        ...
        GetDrmPropertyForName(drm_, props.get(), "GAMMA_LUT", &state.properties.gamma_lut);
        GetDrmPropertyForName(drm_, props.get(), "GAMMA_LUT_SIZE", &state.properties.gamma_lut_size);
        GetDrmPropertyForName(drm_, props.get(), "DEGAMMA_LUT", &state.properties.degamma_lut);
        GetDrmPropertyForName(drm_, props.get(), "DEGAMMA_LUT_SIZE", &state.properties.degamma_lut_size);
        GetDrmPropertyForName(drm_, props.get(), "OUT_FENCE_PTR", &state.properties.out_fence_ptr);
        GetDrmPropertyForName(drm_, props.get(), "BACKGROUND_COLOR", &state.properties.background_color);
        GetDrmPropertyForName(drm_, props.get(), kVrrEnabledPropertyName, &state.properties.vrr_enabled);
    ...
    // crtcs.
    if (num_crtcs_with_out_fence_ptr != 0 && num_crtcs_with_out_fence_ptr != crtc_state_.size()) {
...
void HardwareDisplayPlaneManager::DisableConnectedConnectorsToCrtcs(const ScopedDrmResourcesPtr& resources)
{
    ...
    for (int i = 0; i < resources->count_connectors; ++i) {
        ScopedDrmConnectorPtr connector = drm_->GetConnector(resources->connectors[i]);
        if (!connector)
            continue;
        // Disable Zombie connectors (disconnected connectors but holding to an
        // encoder).
        if (connector->encoder_id && connector->connection == DRM_MODE_DISCONNECTED) {
...
const HardwareDisplayPlaneManager::CrtcState& HardwareDisplayPlaneManager::GetCrtcStateForCrtcId(uint32_t crtc_id)
{
    return CrtcStateForCrtcId(crtc_id);
}

HardwareDisplayPlaneManager::CrtcState& HardwareDisplayPlaneManager::CrtcStateForCrtcId(uint32_t crtc_id)
{
...
void HardwareDisplayPlaneManager::UpdateCrtcAndPlaneStatesAfterModeset(const CommitRequest& commit_request)
{
    ...
    for (const auto& crtc_request : commit_request) {
        ...
        if (is_enabled) {
            ...
            if (!drm_->SetCursor(crtc_request.crtc_id(), 0, gfx::Size())) {
                PLOG(ERROR) << "Failed to drmModeSetCursor: device:" << drm_->device_path().value() << " crtc:" << crtc_request.crtc_id();
            }
        }
    }
...
HardwareCapabilities HardwareDisplayPlaneManager::GetHardwareCapabilities(uint32_t crtc_id)
{
    ...
    base::ranges::for_each(planes_,
        [crtc_id, &num_overlay_planes = hc.num_overlay_capable_planes, &buffer_formats = hc.supported_buffer_formats](
            const std::unique_ptr<HardwareDisplayPlane>& plane) {
            if (plane->type() != DRM_PLANE_TYPE_CURSOR && plane->CanUseForCrtcId(crtc_id)) {
                num_overlay_planes++;
...
uint32_t HardwareDisplayPlaneManager::GetPossibleCrtcsBitmaskForConnector(uint32_t connector_id) const
{
    const auto& connector_prop
        = std::find_if(connectors_props_.begin(), connectors_props_.end(), [connector_id](const ConnectorProperties& prop) { return prop.id == connector_id; });
    if (connector_prop == connectors_props_.end()) {
        LOG(WARNING) << __func__ << ": Failed to retrieve connector property for id " << connector_id;
        return {};
    }
    return connector_prop->possible_crtcs_bitmask;
}

void HardwareDisplayPlaneManager::UpdatePendingCrtcState(CrtcState& crtc_state)
{
    ...
    const skcms_Matrix3x3 plane_to_device_matrix = base::FeatureList::IsEnabled(display::features::kCtmColorManagement)
        ? PlaneToOutputMatrix(crtc_state)
        : crtc_state.color_calibration.srgb_to_device_matrix;
    const skcms_Matrix3x3 ctm = skcms_Matrix3x3_concat(&plane_to_device_matrix, &crtc_state.color_temperature_adjustment.srgb_matrix);
    if (crtc_state.properties.ctm.id) {
        ScopedDrmColorCtmPtr ctm_blob_data = CreateCTMBlob(ctm, ctm_negative_values_broken_);
        crtc_state.pending_ctm_blob = drm_->CreatePropertyBlob(ctm_blob_data.get(), sizeof(drm_color_ctm));
    }
    ...
    if (crtc_props.gamma_lut.id && crtc_props.gamma_lut_size.id && crtc_props.degamma_lut.id && crtc_props.degamma_lut_size.id) {
        ...
        if (degamma_curve.IsDefaultIdentity()) {
            ...
            ScopedDrmColorLutPtr degamma_blob_data = CreateLutBlob(degamma_curve, crtc_props.degamma_lut_size.value);
            crtc_state.pending_degamma_lut_blob = drm_->CreatePropertyBlob(degamma_blob_data.get(), sizeof(drm_color_lut) * crtc_props.degamma_lut_size.value);
        }
    }
    ...
    // is reliable.
    const auto gamma_curve = display::GammaCurve::MakeConcat(crtc_state.color_calibration.linear_to_device, crtc_state.gamma_adjustment.curve);
    if (crtc_props.gamma_lut.id && crtc_props.gamma_lut_size.id) {
        if (gamma_curve.IsDefaultIdentity()) {
            ...
            ScopedDrmColorLutPtr gamma_blob_data = CreateLutBlob(gamma_curve, crtc_props.gamma_lut_size.value);
            crtc_state.pending_gamma_lut_blob = drm_->CreatePropertyBlob(gamma_blob_data.get(), sizeof(drm_color_lut) * crtc_props.gamma_lut_size.value);
        }
    } else {
