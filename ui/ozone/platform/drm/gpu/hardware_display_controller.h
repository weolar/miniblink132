...
public:
    HardwareDisplayController(std::unique_ptr<CrtcController> controller, const gfx::Point& origin, raw_ptr<DrmModifiersFilter> drm_modifiers_filter);

    HardwareDisplayController(const HardwareDisplayController&) = delete;
    HardwareDisplayController& operator=(const HardwareDisplayController&) = delete;

    ~HardwareDisplayController();

    // Gets the props required to modeset a CRTC with a |mode| onto
    // |commit_request|.
    void GetModesetProps(CommitRequest* commit_request, const DrmOverlayPlaneList& modeset_planes, const drmModeModeInfo& mode, bool enable_vrr);
    // Gets the props required to enable/disable a CRTC onto |commit_request|.
    void GetEnableProps(CommitRequest* commit_request, const DrmOverlayPlaneList& modeset_planes);
    ...
    void SchedulePageFlip(DrmOverlayPlaneList plane_list, SwapCompletionOnceCallback submission_callback, PresentationOnceCallback presentation_callback);
    ...
    std::vector<uint64_t> GetSupportedModifiers(uint32_t fourcc_format, bool is_modeset = false) const;

    std::vector<uint64_t> GetFormatModifiersForTestModeset(uint32_t fourcc_format);

    void UpdatePreferredModifierForFormat(gfx::BufferFormat buffer_format, uint64_t modifier);
    ...
    std::unique_ptr<CrtcController> RemoveCrtc(const scoped_refptr<DrmDevice>& drm, uint32_t crtc);
    ...
    void set_origin(const gfx::Point& origin)
    {
        origin_ = origin;
    }
    ...
    void OnPageFlipComplete(int modeset_sequence, DrmOverlayPlaneList pending_planes, const gfx::PresentationFeedback& presentation_feedback);
...
private:
    ...
    void GetModesetPropsForCrtcs(CommitRequest* commit_request, const DrmOverlayPlaneList& modeset_planes, bool use_current_crtc_mode,
    ...
    bool ShouldDisableNonprimaryTileController(const CrtcController& controller, const drmModeModeInfo& mode, const bool use_current_crtc_mode) const;
    ...
    base::flat_map<gfx::Size, std::vector<std::unique_ptr<DrmDumbBuffer>>, CursorSizeComparator> cursor_buffer_map_;
    ...
    base::flat_map<uint32_t /*fourcc_format*/, uint64_t /*preferred_modifier*/> preferred_format_modifier_;
