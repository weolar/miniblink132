...
// HP z32x monitor.
constexpr unsigned char kHPz32x[] = "\x00\xFF\xFF\xFF\xFF\xFF\xFF\x00\x22\xF0\x75\x32\x01\x01\x01\x01"
...
constexpr unsigned char kInternalDisplay[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x4c\xa3\x42\x31\x00\x00\x00\x00"
...
// display IDs.
constexpr unsigned char kNoSerialNumberDisplay[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x22\xf0\x6c\x28\x00\x00\x00\x00"
...
constexpr size_t kNoSerialNumberDisplayLength = std::size(kNoSerialNumberDisplay);

constexpr unsigned char kTiledDisplay[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x10\xac\x47\x41\x4c\x34\x37\x41"
...
constexpr char kTestOnlyModesetOutcomeOneDisplay[] = "ConfigureDisplays.Modeset.Test.OneDisplay.Outcome";
constexpr char kTestOnlyModesetOutcomeTwoDisplays[] = "ConfigureDisplays.Modeset.Test.TwoDisplays.Outcome";
constexpr char kTestOnlyModesetFallbacksAttemptedTwoDisplaysMetric[] = "ConfigureDisplays.Modeset.Test.DynamicCRTCs.TwoDisplays."
                                                                       "PermutationsAttempted";

const std::vector<ResolutionAndRefreshRate> kStandardModes
    = { ResolutionAndRefreshRate { gfx::Size(3840, 2160), 60u }, ResolutionAndRefreshRate { gfx::Size(3840, 2160), 50u },
          ResolutionAndRefreshRate { gfx::Size(3840, 2160), 30u }, ResolutionAndRefreshRate { gfx::Size(1920, 1080), 60u },
          ResolutionAndRefreshRate { gfx::Size(1920, 1080), 50u }, ResolutionAndRefreshRate { gfx::Size(1920, 1080), 30u } };
...
MATCHER_P(AtomicRequestHasCrtcConnectorPairs, pairings, "")
{
    ...
    for (const auto& [crtc, connector] : pairings) {
        if (std::find_if(arg_items.begin(), arg_items.end(),
                [crtc, connector](const drmModeAtomicReqItem& item) { return item.object_id == connector && item.value == crtc; })
            == arg_items.end()) {
            return false;
        }
    }
...
class MockDrmDeviceGenerator : public DrmDeviceGenerator {
    scoped_refptr<DrmDevice> CreateDevice(const base::FilePath& path, base::ScopedFD fd, bool is_primary_device) override
    {
        auto gbm_device = std::make_unique<MockGbmDevice>();
        DCHECK(!path.empty());
        return base::MakeRefCounted<MockDrmDevice>(std::move(path), std::move(gbm_device), is_primary_device);
    }
};

ScopedDrmPropertyBlob CreateTilePropertyBlob(FakeDrmDevice& drm, const TileProperty& property)
{
    // "group_id:tile_is_single_monitor:num_h_tile:num_v_tile:tile_h_loc
    // :tile_v_loc:tile_h_size:tile_v_size"
    std::string tile_property_str = base::StringPrintf("%d:1:%d:%d:%d:%d:%d:%d\0", property.group_id, property.tile_layout.width(),
        property.tile_layout.height(), property.location.x(), property.location.y(), property.tile_size.width(), property.tile_size.height());

    return drm.CreatePropertyBlob(tile_property_str.data(), tile_property_str.size());
}

testing::Matcher<display::DisplayMode> EqResAndRefresh(const ResolutionAndRefreshRate& mode)
{
    return AllOf(Property(&display::DisplayMode::size, Eq(mode.first)), Property(&display::DisplayMode::refresh_rate, Eq(mode.second)));
}
...
void ExpectEqualRequestsWithExceptions(const std::vector<display::DisplayConfigurationParams>& a, const std::vector<display::DisplayConfigurationParams>& b)
{
...
protected:
    void SetUp() override
    {
        std::unique_ptr<FakeDrmDeviceGenerator> fake_device_generator = std::make_unique<FakeDrmDeviceGenerator>();
        device_manager_ = std::make_unique<DrmDeviceManager>(std::move(fake_device_generator));
        screen_manager_ = std::make_unique<ScreenManager>();
        drm_gpu_display_manager_ = std::make_unique<DrmGpuDisplayManager>(screen_manager_.get(), device_manager_.get());
    }
    ...
    {
        std::string card_path = base::StringPrintf(kDefaultTestGraphicsCardPattern, next_drm_device_number_++);
        base::FilePath file_path(card_path);

        device_manager_->AddDrmDevice(file_path, base::ScopedFD());
        FakeDrmDevice* fake_drm = static_cast<FakeDrmDevice*>(device_manager_->GetDrmDevices().back().get());
        return scoped_refptr<FakeDrmDevice>(fake_drm);
    }

    bool ConfigureDisplays(const MovableDisplaySnapshots& display_snapshots, display::ModesetFlags modeset_flag)
    {
        std::vector<display::DisplayConfigurationParams> config_requests;
        for (const auto& snapshot : display_snapshots) {
            config_requests.emplace_back(snapshot->display_id(), snapshot->origin(), snapshot->native_mode());
        }
        std::vector<display::DisplayConfigurationParams> out_requests;
        return drm_gpu_display_manager_->ConfigureDisplays(config_requests, modeset_flag, out_requests);
    }
...
TEST_F(DrmGpuDisplayManagerTest, FindAndConfigureDisplaysOnSameDrmDevice)
{
    ...
    for (size_t i = 0; i < 3; ++i) {
        ...
        connector.connection = true;
        connector.modes = std::vector<ResolutionAndRefreshRate> { kStandardModes[i % kStandardModes.size()] };
        connector.encoders = std::vector<uint32_t> { encoder.id };
        connector.edid_blob = std::vector<uint8_t>(kHPz32x, kHPz32x + kHPz32xLength);
    }
    drm->InitializeState(/* use_atomic */ true);

    MovableDisplaySnapshots display_snapshots = drm_gpu_display_manager_->GetDisplays();
    ...
    ASSERT_TRUE(ConfigureDisplays(display_snapshots, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset }));
}

// This case tests scenarios in which a display ID is searched across multiple
// DRM devices, such as in DisplayLink hubs.
TEST_F(DrmGpuDisplayManagerTest, FindAndConfigureDisplaysAcrossDifferentDrmDevices)
{
...
TEST_F(DrmGpuDisplayManagerTest, OriginsPersistThroughSimilarExtendedModeConfigurations)
{
    // One DRM device.
    auto fake_drm = AddDrmDevice();
    fake_drm->ResetStateWithAllProperties();
    ...
    DrmDisplay* display2 = FindDisplay(display_snapshots[1]->display_id());
    ASSERT_TRUE(display2);
    const gfx::Point display2_origin = gfx::Point(0, display_snapshots[0]->native_mode()->size().height());
    ...
    const gfx::Point display3_origin
        = gfx::Point(0, display_snapshots[0]->native_mode()->size().height() + display_snapshots[1]->native_mode()->size().height());
...
TEST_F(DrmGpuDisplayManagerTest, TestEdidIdConflictResolution)
{
    ...
    // First, add the internal display.
    {
        ...
        connector.encoders = std::vector<uint32_t> { encoder.id };
        connector.edid_blob = std::vector<uint8_t>(kInternalDisplay, kInternalDisplay + kInternalDisplayLength);
    }
    ...
    // collision, since their EDIDs do not include viable serial numbers.
    {
        ...
        connector.encoders = std::vector<uint32_t> { encoder.id };
        connector.edid_blob = std::vector<uint8_t>(kNoSerialNumberDisplay, kNoSerialNumberDisplay + kNoSerialNumberDisplayLength);
    }
    ...
    ASSERT_NE(display_snapshots[0]->edid_display_id(), display_snapshots[1]->edid_display_id());
    ASSERT_NE(display_snapshots[0]->edid_display_id(), display_snapshots[2]->edid_display_id());
    ASSERT_NE(display_snapshots[1]->edid_display_id(), display_snapshots[2]->edid_display_id());
    ...
    const int64_t unresolved_display_id1 = display_snapshots[1]->edid_display_id() & 0xffffffff;
    const int64_t unresolved_display_id2 = display_snapshots[2]->edid_display_id() & 0xffffffff;
...
protected:
    void SetUp() override
    {
        std::unique_ptr<MockDrmDeviceGenerator> fake_device_generator = std::make_unique<MockDrmDeviceGenerator>();
        device_manager_ = std::make_unique<DrmDeviceManager>(std::move(fake_device_generator));
        screen_manager_ = std::make_unique<ScreenManager>();
        drm_gpu_display_manager_ = std::make_unique<DrmGpuDisplayManager>(screen_manager_.get(), device_manager_.get());
    }
...
TEST_F(DrmGpuDisplayManagerMockedDeviceTest, ConfigureDisplaysAlternateCrtcFallbackSuccess)
{
    auto drm = AddDrmDevice();
    drm->ResetStateWithAllProperties();
    ...
    // First, add a "big" display with high bandwidth mode.
    {
        ...
        auto& connector = drm->AddConnector();
        connector.connection = true;
        connector.modes = std::vector<ResolutionAndRefreshRate> { ResolutionAndRefreshRate { gfx::Size(7680, 4320), 144u } };
        connector.encoders = std::vector<uint32_t> { encoder.id };
    ...
    const DrmDisplay::CrtcConnectorPair* original_big_display_crtc_connector_pair = big_display->GetCrtcConnectorPairForConnectorId(big_display_connector_id);
    ASSERT_NE(original_big_display_crtc_connector_pair, nullptr);

    DrmDisplay* small_display = FindDisplayByConnectorId(small_display_connector_id);
    const DrmDisplay::CrtcConnectorPair* original_small_display_crtc_connector_pair
        = small_display->GetCrtcConnectorPairForConnectorId(small_display_connector_id);
    ...
    EXPECT_CALL(*mock_drm, CommitProperties).Times(AtLeast(1)).WillRepeatedly(Return(false));

    CrtcConnectorPairs desired_pairings = { { crtc_1, big_display_connector_id }, { crtc_3, small_display_connector_id } };
    EXPECT_CALL(*mock_drm, CommitProperties(AtomicRequestHasCrtcConnectorPairs(desired_pairings), _, _, _)).WillOnce(Return(true));

    EXPECT_TRUE(ConfigureDisplays(display_snapshots, { display::ModesetFlag::kTestModeset }));

    histogram_tester_.ExpectBucketCount(kTestOnlyModesetOutcomeTwoDisplays, TestOnlyModesetOutcome::kFallbackSuccess,
        /*expected_count=*/1);
    EXPECT_THAT(histogram_tester_.GetAllSamples(kTestOnlyModesetFallbacksAttemptedTwoDisplaysMetric),
        UnorderedElementsAre(AllOf(Field(&base::Bucket::min, Gt(0)), Field(&base::Bucket::count, Eq(1)))));
    ...
    EXPECT_EQ(big_display->GetCrtcConnectorPairForConnectorId(big_display_connector_id)->crtc_id, original_big_display_crtc_connector_pair->crtc_id);
    small_display = FindDisplayByConnectorId(small_display_connector_id);
    EXPECT_EQ(small_display->GetCrtcConnectorPairForConnectorId(small_display_connector_id)->crtc_id, original_small_display_crtc_connector_pair->crtc_id);
    ...
    EXPECT_TRUE(ConfigureDisplays(display_snapshots, { display::ModesetFlag::kCommitModeset }));
...
TEST_F(DrmGpuDisplayManagerMockedDeviceTest, ConfigureDisplaysFallbackTestSuccessButCommitFailure)
{
    auto drm = AddDrmDevice();
    drm->ResetStateWithAllProperties();
    ...
    CrtcConnectorPairs desired_pairings = { { crtc_1, primary_connector_id }, { crtc_3, secondary_connector_id } };
    EXPECT_CALL(*mock_drm, CommitProperties(AtomicRequestHasCrtcConnectorPairs(desired_pairings), _, _, _)).WillRepeatedly(Return(true));
    ...
    EXPECT_FALSE(ConfigureDisplays(display_snapshots, { display::ModesetFlag::kCommitModeset }));

    DrmDisplay* primary_display = FindDisplayByConnectorId(primary_connector_id);
    EXPECT_TRUE(primary_display->ContainsCrtc(crtc_1));
    DrmDisplay* secondary_display = FindDisplayByConnectorId(secondary_connector_id);
    EXPECT_TRUE(secondary_display->ContainsCrtc(crtc_3));
    histogram_tester_.ExpectBucketCount(kTestOnlyModesetOutcomeTwoDisplays, TestOnlyModesetOutcome::kFailure,
        /*expected_count=*/0);
}

TEST_F(DrmGpuDisplayManagerMockedDeviceTest, ConfigureDisplaysAlternateCrtcFallbackAllFailed)
{
    auto drm = AddDrmDevice();
    drm->ResetStateWithAllProperties();
    ...
    EXPECT_FALSE(ConfigureDisplays(display_snapshots, { display::ModesetFlag::kTestModeset }));
    histogram_tester_.ExpectBucketCount(kTestOnlyModesetOutcomeTwoDisplays, TestOnlyModesetOutcome::kFailure,
        /*expected_count=*/1);
    EXPECT_THAT(histogram_tester_.GetAllSamples(kTestOnlyModesetFallbacksAttemptedTwoDisplaysMetric),
        UnorderedElementsAre(AllOf(Field(&base::Bucket::min, Gt(0)), Field(&base::Bucket::count, Eq(1)))));
...
TEST_F(DrmGpuDisplayManagerMockedDeviceTest, NoConfigureDisplaysAlternateCrtcFallbackWithHardwareMirroring)
{
    // Enable hardware mirroring
    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(display::features::kEnableHardwareMirrorMode);
    ...
    EXPECT_FALSE(ConfigureDisplays(display_snapshots, { display::ModesetFlag::kTestModeset }));
    histogram_tester_.ExpectBucketCount(kTestOnlyModesetOutcomeOneDisplay, TestOnlyModesetOutcome::kFailure,
        /*expected_count=*/1);
    EXPECT_THAT(histogram_tester_.GetAllSamples(kTestOnlyModesetFallbacksAttemptedTwoDisplaysMetric), IsEmpty());
}

TEST_F(DrmGpuDisplayManagerMockedDeviceTest, ConfigureDisplaysUseSuccessfulStateForCommit)
{
    auto drm = AddDrmDevice();
    drm->ResetStateWithAllProperties();
    ...
    // DrmGpuDisplayManager with a failed CRTC-connector pairings.
    {
        ...
        for (const auto& snapshot : display_snapshots) {
            failing_request.emplace_back(snapshot->display_id(), snapshot->origin(), snapshot->modes().back().get());
        }

        std::vector<display::DisplayConfigurationParams> out_requests;
        EXPECT_CALL(*mock_drm, CommitProperties).Times(AtLeast(1)).WillRepeatedly(Return(false));
        EXPECT_FALSE(drm_gpu_display_manager_->ConfigureDisplays(failing_request, { display::ModesetFlag::kTestModeset }, out_requests));

        histogram_tester_.ExpectBucketCount(kTestOnlyModesetOutcomeTwoDisplays, TestOnlyModesetOutcome::kFailure,
            /*expected_count=*/1);
        EXPECT_THAT(histogram_tester_.GetAllSamples(kTestOnlyModesetFallbacksAttemptedTwoDisplaysMetric),
            UnorderedElementsAre(AllOf(Field(&base::Bucket::min, Gt(0)), Field(&base::Bucket::count, Eq(1))),
                AllOf(Field(&base::Bucket::min, Gt(0)), Field(&base::Bucket::count, Eq(1)))));
        ASSERT_TRUE(testing::Mock::VerifyAndClearExpectations(mock_drm));
    }
...
class DrmGpuDisplayManagerGetSeamlessRefreshRateTest : public DrmGpuDisplayManagerMockedDeviceTest {
protected:
    void SetUp() override
    {
        ...
        // capability.
        {
            ...
            // VRR capability.
            connector.edid_blob = std::vector<uint8_t>(kHPz32x, kHPz32x + kHPz32xLength);
            fake_drm_device_->AddProperty(connector.id, { .id = kVrrCapablePropId, .value = 1 });
        }
        ...
        CHECK(ConfigureDisplays(display_snapshots, { display::ModesetFlag::kCommitModeset }));
    }
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, GetSeamlessRefreshRates)
{
    ...
    EXPECT_CALL(*mock_drm_device_, CommitProperties(_, seamless_test_flags, _, _)).Times(2).WillRepeatedly(Return(true));

    std::optional<std::vector<float>> refresh_rates = drm_gpu_display_manager_->GetSeamlessRefreshRates(display_id);
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, GetSeamlessRefreshRatesWithInvalidId)
{
    ...
    std::optional<std::vector<float>> refresh_rates = drm_gpu_display_manager_->GetSeamlessRefreshRates(wrong_display_id);

    ASSERT_FALSE(refresh_rates);
}

TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureDisplaysModeMatching_UnsetMode)
{
    ...
    const std::vector<display::DisplayConfigurationParams> config_requests
        = { display::DisplayConfigurationParams(snapshot->display_id(), snapshot->origin(), nullptr) };

    std::vector<display::DisplayConfigurationParams> out_requests;
    EXPECT_TRUE(drm_gpu_display_manager_->ConfigureDisplays(config_requests, flags, out_requests));
    EXPECT_EQ(config_requests, out_requests);
}

TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureDisplaysModeMatching_ExistingMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[0];
    const display::ModesetFlags flags = { display::ModesetFlag::kTestModeset };
    const std::vector<display::DisplayConfigurationParams> config_requests
        = { display::DisplayConfigurationParams(snapshot->display_id(), snapshot->origin(), snapshot->native_mode()) };
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureDisplaysModeMatching_NonExistingMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[0];
    const display::ModesetFlags flags = { display::ModesetFlag::kTestModeset };
    const display::DisplayMode nonmatching_mode(snapshot->native_mode()->size(), false, 600);
    const std::vector<display::DisplayConfigurationParams> config_requests
        = { display::DisplayConfigurationParams(snapshot->display_id(), snapshot->origin(), &nonmatching_mode) };

    std::vector<display::DisplayConfigurationParams> out_requests;
    EXPECT_FALSE(drm_gpu_display_manager_->ConfigureDisplays(config_requests, flags, out_requests));
    EXPECT_EQ(config_requests, out_requests);
}

TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureDisplaysSeamlessModeMatching_UnsetMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[0];
    const display::ModesetFlags flags = { display::ModesetFlag::kTestModeset, display::ModesetFlag::kSeamlessModeset };
    const std::vector<display::DisplayConfigurationParams> config_requests
        = { display::DisplayConfigurationParams(snapshot->display_id(), snapshot->origin(), nullptr) };
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureDisplaysSeamlessModeMatching_ExistingSeamlessMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[0];
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureDisplaysSeamlessModeMatching_ExistingModeFailingSeamlessVerification)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[0];
    ...
    const std::vector<display::DisplayConfigurationParams> config_requests
        = { display::DisplayConfigurationParams(snapshot->display_id(), snapshot->origin(), failing_mode) };

    const uint32_t seamless_test_flags = DRM_MODE_ATOMIC_TEST_ONLY;
    std::vector<display::DisplayConfigurationParams> out_requests;
    // Override mock behavior to fail seamless verification.
    EXPECT_CALL(*mock_drm_device_, CommitProperties(_, seamless_test_flags, _, _)).Times(1).WillOnce(Return(false));
    EXPECT_FALSE(drm_gpu_display_manager_->ConfigureDisplays(config_requests, flags, out_requests));
    EXPECT_EQ(config_requests, out_requests);
}

TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureDisplaysSeamlessModeMatching_NonExistingMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[0];
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureVrrDisplaysModeMatching_UnsetMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureVrrDisplaysModeMatching_ExistingMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[1];
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureVrrDisplaysModeMatching_NonExistingFasterMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[1];
    const display::ModesetFlags flags = { display::ModesetFlag::kTestModeset };
    const display::DisplayMode nonmatching_mode = display::DisplayMode(snapshot->native_mode()->size(), false, 600, std::nullopt);
    const std::vector<display::DisplayConfigurationParams> config_requests
        = { display::DisplayConfigurationParams(snapshot->display_id(), snapshot->origin(), &nonmatching_mode) };
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureVrrDisplaysModeMatching_NonExistingSlowerMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[1];
    const display::ModesetFlags flags = { display::ModesetFlag::kTestModeset };
    const display::DisplayMode nonmatching_mode = display::DisplayMode(snapshot->native_mode()->size(), false, 51, std::nullopt);
    const std::vector<display::DisplayConfigurationParams> config_requests
        = { display::DisplayConfigurationParams(snapshot->display_id(), snapshot->origin(), &nonmatching_mode) };
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureVrrDisplaysModeMatching_NonExistingSlowerModeBelowVSyncMin)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[1];
    const display::ModesetFlags flags = { display::ModesetFlag::kTestModeset };
    const display::DisplayMode nonmatching_mode = display::DisplayMode(snapshot->native_mode()->size(), false, 20, std::nullopt);
    const std::vector<display::DisplayConfigurationParams> config_requests
        = { display::DisplayConfigurationParams(snapshot->display_id(), snapshot->origin(), &nonmatching_mode) };
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureVrrDisplaysSeamlessModeMatching_UnsetMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[1];
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureVrrDisplaysSeamlessModeMatching_ExistingSeamlessMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[1];
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureVrrDisplaysSeamlessModeMatching_ExistingModeFailingSeamlessVerification)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[1];
    ...
    std::vector<display::DisplayConfigurationParams> out_requests;
    // Override mock behavior to fail seamless verification.
    EXPECT_CALL(*mock_drm_device_, CommitProperties(_, seamless_test_flags, _, _)).Times(3).WillRepeatedly(Return(false));
    EXPECT_FALSE(drm_gpu_display_manager_->ConfigureDisplays(config_requests, flags, out_requests));
    EXPECT_EQ(config_requests, out_requests);
}

TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureVrrDisplaysSeamlessModeMatching_NonExistingFasterMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[1];
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureVrrDisplaysSeamlessModeMatching_NonExistingSlowerMode)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[1];
    const display::ModesetFlags flags = { display::ModesetFlag::kTestModeset, display::ModesetFlag::kSeamlessModeset };
    const display::DisplayMode nonmatching_mode = display::DisplayMode(snapshot->native_mode()->size(), false, 50, std::nullopt);
    const std::vector<display::DisplayConfigurationParams> config_requests
        = { display::DisplayConfigurationParams(snapshot->display_id(), snapshot->origin(), &nonmatching_mode) };
...
TEST_F(DrmGpuDisplayManagerGetSeamlessRefreshRateTest, ConfigureVrrDisplaysSeamlessModeMatching_NonExistingSlowerModeBelowVSyncMin)
{
    const auto snapshots = drm_gpu_display_manager_->GetDisplays();
    ASSERT_FALSE(snapshots.empty());
    const auto& snapshot = snapshots[1];
...
TEST_F(TiledDisplayGetDisplaysTest, SingleTile)
{
    ...
    connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(3840, 4320), 60 }, { gfx::Size(1920, 1080), 60 } };
    connector.encoders = std::vector<uint32_t> { encoder.id };

    const TileProperty tile_property
        = { .group_id = 1, .scale_to_fit_display = true, .tile_size = gfx::Size(3840, 4320), .tile_layout = gfx::Size(2, 1), .location = gfx::Point(0, 0) };
    ScopedDrmPropertyBlob tile_property_blob = CreateTilePropertyBlob(*fake_drm, tile_property);
    fake_drm->AddProperty(connector.id, { .id = kTileBlobPropId, .value = tile_property_blob->id() });
    ...
    EXPECT_THAT(displays[0]->modes(), UnorderedElementsAre(Pointee(EqResAndRefresh({ gfx::Size(1920, 1080), 60 }))));
}
...
TEST_F(TiledDisplayGetDisplaysTest, AllTilesPresent)
{
    ...
    // Primary tile at (0,0)
    ScopedDrmPropertyBlob primary_tile_property_blob = CreateTilePropertyBlob(*fake_drm, expected_primary_tile_prop);
    {
        fake_drm->AddCrtcWithPrimaryAndCursorPlanes();
        ...
        primary_connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(3840, 4320), 60 }, { gfx::Size(1920, 1080), 60 } };
        primary_connector.encoders = std::vector<uint32_t> { primary_encoder.id };
        fake_drm->AddProperty(primary_connector.id, { .id = kTileBlobPropId, .value = primary_tile_property_blob->id() });
    }
    ...
    ScopedDrmPropertyBlob nonprimary_tile_property_blob = CreateTilePropertyBlob(*fake_drm, expected_nonprimary_tile_prop);
    {
        fake_drm->AddCrtcWithPrimaryAndCursorPlanes();
        ...
        nonprimary_connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(3840, 4320), 60 }, { gfx::Size(1920, 1080), 60 } };
        nonprimary_connector.encoders = std::vector<uint32_t> { nonprimary_encoder.id };
        fake_drm->AddProperty(nonprimary_connector.id, { .id = kTileBlobPropId, .value = nonprimary_tile_property_blob->id() });
    }
    ...
    EXPECT_THAT(displays[0]->modes(),
        UnorderedElementsAre(Pointee(EqResAndRefresh({ gfx::Size(7680, 4320), 60 })), Pointee(EqResAndRefresh({ gfx::Size(1920, 1080), 60 }))));
}
...
TEST_F(TiledDisplayGetDisplaysTest, PrimaryCanStretchToFit)
{
    ...
    // Primary tile at (1,0)
    const TileProperty expected_primary_tile_prop
        = { .group_id = 1, .scale_to_fit_display = true, .tile_size = gfx::Size(3840, 4320), .tile_layout = gfx::Size(2, 1), .location = gfx::Point(1, 0) };
    ScopedDrmPropertyBlob primary_tile_property_blob = CreateTilePropertyBlob(*fake_drm, expected_primary_tile_prop);
    { // Add 1 CRTC
        fake_drm->AddCrtcWithPrimaryAndCursorPlanes();
        ...
        primary_connector.edid_blob = std::vector<uint8_t>(kTiledDisplay, kTiledDisplay + kTiledDisplayLength);
        fake_drm->AddProperty(primary_connector.id, { .id = kTileBlobPropId, .value = primary_tile_property_blob->id() });
    }
    ...
    ScopedDrmPropertyBlob nonprimary_tile_property_blob = CreateTilePropertyBlob(*fake_drm, expected_nonprimary_tile_prop);
    {
        ...
        // be false.
        nonprimary_connector.edid_blob = std::vector<uint8_t>(kHPz32x, kHPz32x + kHPz32xLength);
        fake_drm->AddProperty(nonprimary_connector.id, { .id = kTileBlobPropId, .value = nonprimary_tile_property_blob->id() });
    }
...
TEST_F(TiledDisplayGetDisplaysTest, MoreModesInOneTile)
{
    ...
    const TileProperty expected_primary_tile_prop
        = { .group_id = 1, .scale_to_fit_display = false, .tile_size = gfx::Size(3840, 4320), .tile_layout = gfx::Size(2, 1), .location = gfx::Point(1, 0) };
    ScopedDrmPropertyBlob primary_tile_property_blob = CreateTilePropertyBlob(*fake_drm, expected_primary_tile_prop);
    {
        fake_drm->AddCrtcWithPrimaryAndCursorPlanes();
        ...
        primary_connector.modes
            = std::vector<ResolutionAndRefreshRate> { { gfx::Size(3840, 4320), 60 }, { gfx::Size(1920, 1080), 60 }, { gfx::Size(2560, 1440), 60 } };
        primary_connector.encoders = std::vector<uint32_t> { primary_encoder.id };
        fake_drm->AddProperty(primary_connector.id, { .id = kTileBlobPropId, .value = primary_tile_property_blob->id() });
    }
    ...
    ScopedDrmPropertyBlob nonprimary_tile_property_blob = CreateTilePropertyBlob(*fake_drm, expected_nonprimary_tile_prop);
    {
        ...
        nonprimary_connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(3840, 4320), 60 } };
        nonprimary_connector.encoders = std::vector<uint32_t> { nonprimary_encoder.id };
        nonprimary_connector.properties.push_back({ .id = kTileBlobPropId, .value = kTileBlobId + 1 });
        fake_drm->AddProperty(nonprimary_connector.id, { .id = kTileBlobPropId, .value = nonprimary_tile_property_blob->id() });
    }
    ...
    // the primary tile.
    EXPECT_THAT(displays[0]->modes(),
        UnorderedElementsAre(Pointee(EqResAndRefresh({ gfx::Size(7680, 4320), 60 })),
            // The non-tile mode in the primary tile (but not in
            // non-primary) should live.
            Pointee(EqResAndRefresh({ gfx::Size(1920, 1080), 60 })), Pointee(EqResAndRefresh({ gfx::Size(2560, 1440), 60 }))));
}
...
TEST_F(TiledDisplayGetDisplaysTest, PruneTileModesNotInAllTiles)
{
    ...
    ScopedDrmPropertyBlob primary_tile_property_blob = CreateTilePropertyBlob(*fake_drm, expected_primary_tile_prop);
    {
        ...
        auto& primary_connector = fake_drm->AddConnector();
        primary_connector.connection = true;
        primary_connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(3840, 4320), 60 }, { gfx::Size(3840, 4320), 48 } };
        primary_connector.encoders = std::vector<uint32_t> { primary_encoder.id };
        primary_connector.edid_blob = std::vector<uint8_t>(kHPz32x, kHPz32x + kHPz32xLength);
        fake_drm->AddProperty(primary_connector.id, { .id = kTileBlobPropId, .value = primary_tile_property_blob->id() });
    }
    ...
    EXPECT_THAT(displays[0]->modes(), UnorderedElementsAre(Pointee(EqResAndRefresh({ gfx::Size(7680, 4320), 60 }))));
}
...
TEST_F(TiledDisplayGetDisplaysTest, NonTileModeNotPruned)
{
    ...
    ScopedDrmPropertyBlob primary_tile_property_blob = CreateTilePropertyBlob(*fake_drm, expected_primary_tile_prop);
    {
        ...
        primary_connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(7680, 4320), 30 }, { gfx::Size(3840, 4320), 60 } };
        primary_connector.encoders = std::vector<uint32_t> { primary_encoder.id };
        primary_connector.edid_blob = std::vector<uint8_t>(kHPz32x, kHPz32x + kHPz32xLength);
        fake_drm->AddProperty(primary_connector.id, { .id = kTileBlobPropId, .value = primary_tile_property_blob->id() });
    }
    ...
    EXPECT_THAT(displays[0]->modes(), UnorderedElementsAre(Pointee(EqResAndRefresh({ gfx::Size(7680, 4320), 30 }))));
}
...
TEST_F(TiledDisplayGetDisplaysTest, ConfigureTileDisplayTileCompositeMode)
{
    ...
    ScopedDrmPropertyBlob nonprimary_tile_property_blob = CreateTilePropertyBlob(*fake_drm, expected_nonprimary_tile_prop);
    {
        ...
        nonprimary_connector.modes = std::vector<ResolutionAndRefreshRate> { { gfx::Size(3840, 4320), 60 }, { gfx::Size(1920, 1080), 60 } };
        nonprimary_connector.encoders = std::vector<uint32_t> { nonprimary_encoder.id };
        nonprimary_connector.edid_blob = std::vector<uint8_t>(kTiledDisplay, kTiledDisplay + kTiledDisplayLength);
        fake_drm->AddProperty(nonprimary_connector.id, { .id = kTileBlobPropId, .value = nonprimary_tile_property_blob->id() });
    }
    ...
    std::vector<display::DisplayConfigurationParams> config_requests;
    config_requests.emplace_back(displays[0]->display_id(), displays[0]->origin(), native_tile_mode);
    std::vector<display::DisplayConfigurationParams> out_requests;
    EXPECT_TRUE(drm_gpu_display_manager_->ConfigureDisplays(
        config_requests, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset }, out_requests));
    ExpectEqualRequestsWithExceptions(config_requests, out_requests);
}
