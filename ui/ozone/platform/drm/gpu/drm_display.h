...
public:
    ...
    public:
        explicit PrivacyScreenProperty(const scoped_refptr<DrmDevice>& drm, drmModeConnector* connector);
    ...
    private:
        ...
        display::PrivacyScreenState property_last_ = display::kPrivacyScreenStateLast;
    ...
    struct CrtcConnectorPair {
        CrtcConnectorPair(uint32_t crtc_id, ScopedDrmConnectorPtr connector, std::optional<gfx::Point> tile_location);
    ...
    // DrmDisplay instance.
    explicit DrmDisplay(const scoped_refptr<DrmDevice>& drm, HardwareDisplayControllerInfo* info, const display::DisplaySnapshot& display_snapshot);
    ...
    int64_t base_connector_id() const
    {
        return base_connector_id_;
    }
    scoped_refptr<DrmDevice> drm() const
    {
        return drm_;
    }
    ...
    const std::vector<drmModeModeInfo>& modes() const
    {
        return modes_;
    }
    ...
    bool GetHDCPState(display::HDCPState* state, display::ContentProtectionMethod* protection_method);
    bool SetHDCPState(display::HDCPState state, display::ContentProtectionMethod protection_method);
    void SetColorTemperatureAdjustment(const display::ColorTemperatureAdjustment& cta);
    ...
    // call.
    bool ReplaceCrtcs(const base::flat_map<uint32_t /*current_crtc*/, uint32_t /*new_crtc*/>& current_to_new_crtc_ids);

    std::optional<TileProperty> GetTileProperty() const;
    const CrtcConnectorPair* GetCrtcConnectorPairForConnectorId(uint32_t connector_id) const;

private:
    gfx::HDRStaticMetadata::Eotf GetEotf(const gfx::ColorSpace::TransferID transfer_id);
