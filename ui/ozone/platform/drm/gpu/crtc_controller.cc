...
CrtcController::CrtcController(const scoped_refptr<DrmDevice>& drm, uint32_t crtc, uint32_t connector, std::optional<TileProperty> tile_property)
    : drm_(drm)
    , crtc_(crtc)
...
{
    if (is_enabled()) {
        const std::vector<std::unique_ptr<HardwareDisplayPlane>>& all_planes = drm_->plane_manager()->planes();
...
bool CrtcController::AssignOverlayPlanes(HardwareDisplayPlaneList* plane_list, const DrmOverlayPlaneList& overlays, bool is_modesetting)
{
    ...
    if (primary && !drm_->plane_manager()->ValidatePrimarySize(*primary, state_->mode)) {
        VLOG(2) << "Trying to pageflip a buffer with the wrong size. Expected " << ModeSize(state_->mode).ToString() << " got "
    ...
    if (!drm_->plane_manager()->AssignOverlayPlanes(plane_list, overlays, crtc_, crtc_offset)) {
        return false;
    }
...
void CrtcController::SetCursor(uint32_t handle, const gfx::Size& size)
{
    if (is_enabled() && !drm_->SetCursor(crtc_, handle, size)) {
        PLOG(ERROR) << "drmModeSetCursor: device " << drm_->device_path().value() << " crtc " << crtc_ << " handle " << handle << " size " << size.ToString();
    }
}
...
void CrtcController::MoveCursor(const gfx::Point& location)
{
    ...
    if (CurrentModeIsTiled()) {
        ...
        gfx::Point translated_location(location.x() - tiled_offset.x(), location.y() - tiled_offset.y());
...
bool CrtcController::CurrentModeIsTiled() const
{
    ...
    return mode().hdisplay == tile_property_->tile_size.width() && mode().vdisplay == tile_property_->tile_size.height();
}
