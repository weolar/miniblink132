...
void PageFlipWatchdog::CrashOnFailedPlaneAssignment()
{
    ...
    // too early.
    if (page_flip_status_tracker_.CurrentIndex() < page_flip_status_tracker_.BufferSize())
        return;
    ...
    if (flakes >= kPlaneAssignmentFlakeThreshold) {
        LOG(FATAL) << "Plane assignment has flaked " << flakes << " times, but the threshold is " << kPlaneAssignmentFlakeThreshold
                   << ". Crashing the GPU process.";
    }

    if (failures >= kPlaneAssignmentMaximumFailures) {
        LOG(FATAL) << "Plane assignment has failed " << failures << "/" << page_flip_status_tracker_.BufferSize() << " times, but the threshold is "
                   << kPlaneAssignmentMaximumFailures << ". Crashing the GPU process.";
    }
}
...
void PageFlipWatchdog::Disarm()
{
    ...
    if (crash_gpu_timer_.IsRunning()) {
        ...
        SYSLOG(INFO) << "Detected a modeset attempt after " << failed_page_flip_counter_ << " failed page flips. Aborting GPU process self-destruct with "
                     << crash_gpu_timer_.desired_run_time() - base::TimeTicks::Now() << " to spare.";
    }
}
...
void PageFlipWatchdog::StartCrashGpuTimer()
{
    if (!crash_gpu_timer_.IsRunning()) {
        ...
        LOG(WARNING) << "Initiating GPU process self-destruct in " << kWaitForModesetTimeout << " unless a modeset attempt is detected.";

        crash_gpu_timer_.Start(FROM_HERE, kWaitForModesetTimeout, base::BindOnce([] {
