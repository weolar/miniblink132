...
public:
    DrmOverlayManager(bool handle_overlays_swap_failure, bool allow_sync_and_real_buffer_page_flip_testing);
    ...
    // OverlayManagerOzone:
    std::unique_ptr<OverlayCandidatesOzone> CreateOverlayCandidates(gfx::AcceleratedWidget w) override;
    ...
    void CheckOverlaySupport(std::vector<OverlaySurfaceCandidate>* candidates, gfx::AcceleratedWidget widget);

    void StartObservingHardwareCapabilities(gfx::AcceleratedWidget widget, HardwareCapabilitiesCallback receive_callback);
    void StopObservingHardwareCapabilities(gfx::AcceleratedWidget widget);

    virtual void GetHardwareCapabilities(gfx::AcceleratedWidget widget, HardwareCapabilitiesCallback& receive_callback) = 0;
    ...
    void RegisterOverlayRequirement(gfx::AcceleratedWidget widget, bool requires_overlay);
    ...
    // capabilities.
    void SetSupportedBufferFormats(gfx::AcceleratedWidget widget, base::flat_set<gfx::BufferFormat> supported_buffer_formats);
    ...
    void OnPromotedOverlayTypes(std::vector<gfx::OverlayType> promoted_overlay_types);
...
protected:
    ...
    virtual void SendOverlayValidationRequest(const std::vector<OverlaySurfaceCandidate>& candidates, gfx::AcceleratedWidget widget) = 0;
    ...
    virtual std::vector<OverlayStatus> SendOverlayValidationRequestSync(const std::vector<OverlaySurfaceCandidate>& candidates, gfx::AcceleratedWidget widget)
        = 0;

    // Perform basic validation to see if |candidate| is a valid request.
    bool CanHandleCandidate(const OverlaySurfaceCandidate& candidate, gfx::AcceleratedWidget widget) const;

    // Checks if gfx::BufferFormat that overlay candidate requires is supported
    // by hardware.
    bool IsBufferFormatSupported(gfx::BufferFormat required_overlay_buffer_format, gfx::AcceleratedWidget widget) const;
...
private:
    ...
    using OverlayCandidatesListCache = base::LRUCache<std::vector<OverlaySurfaceCandidate>, OverlayValidationCacheValue>;
    ...
    std::map<gfx::AcceleratedWidget, OverlayCandidatesListCache> widget_cache_map_;

    base::flat_set<gfx::AcceleratedWidget> widgets_with_required_overlays_;

    std::map<gfx::AcceleratedWidget, HardwareCapabilitiesCallback> hardware_capabilities_callbacks_;

    base::flat_map<gfx::AcceleratedWidget, base::flat_set<gfx::BufferFormat>> per_widget_overlay_supported_buffer_formats_;
