...
DrmWindow::DrmWindow(gfx::AcceleratedWidget widget, DrmDeviceManager* device_manager, ScreenManager* screen_manager)
    : widget_(widget)
...
{
    TRACE_EVENT2("drm", "DrmWindow::SetBounds", "widget", widget_, "bounds", bounds.ToString());
...
void DrmWindow::SetCursor(const std::vector<SkBitmap>& bitmaps, const std::optional<gfx::Point>& location, base::TimeDelta frame_delay)
{
    ...
    if (!frame_delay.is_zero()) {
        cursor_timer_.Start(FROM_HERE, frame_delay, this, &DrmWindow::OnCursorAnimationTimeout);
    }
    ...
    std::vector<DrmOverlayPlane> planes, SwapCompletionOnceCallback submission_callback, PresentationOnceCallback presentation_callback)
{
    ...
    controller_->SchedulePageFlip(std::move(planes), std::move(submission_callback), std::move(presentation_callback));
}

OverlayStatusList DrmWindow::TestPageFlip(const OverlaySurfaceCandidateList& overlay_params)
{
    return overlay_validator_->TestPageFlip(overlay_params, last_submitted_planes_);
}
...
void DrmWindow::SetController(HardwareDisplayController* controller)
{
    ...
    device_manager_->UpdateDrmDevice(widget_, controller ? controller->GetDrmDevice() : nullptr);
