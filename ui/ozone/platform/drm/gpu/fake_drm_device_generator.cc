...
namespace ui {
scoped_refptr<DrmDevice> FakeDrmDeviceGenerator::CreateDevice(const base::FilePath& path, base::ScopedFD fd, bool is_primary_device)
{
    auto gbm_device = std::make_unique<MockGbmDevice>();
    if (path.empty())
        return base::MakeRefCounted<FakeDrmDevice>(std::move(gbm_device));

    return base::MakeRefCounted<FakeDrmDevice>(std::move(path), std::move(gbm_device), is_primary_device);
}
