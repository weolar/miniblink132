...
bool FillModesetBuffer(const scoped_refptr<DrmDevice>& drm, HardwareDisplayController* controller, SkSurface* surface, const std::vector<uint64_t>& modifiers)
{
    ...
    // changes modes.
    if (saved_buffer.GetCanvas()->getBaseLayerSize() != surface->getCanvas()->getBaseLayerSize()) {
    ...
    surface->getCanvas()->drawImage(saved_buffer.surface()->makeImageSnapshot(), 0, 0, SkSamplingOptions(), &paint);
    return true;
}

CrtcController* GetCrtcController(HardwareDisplayController* controller, const scoped_refptr<DrmDevice>& drm, uint32_t crtc)
{
...
void ParamsToTracedValue(perfetto::TracedValue context, const std::vector<ControllerConfigParams>& controllers_params, display::ModesetFlags modeset_flags)
{
...
std::string GenerateConfigurationLogForController(const std::vector<ControllerConfigParams>& controllers_params)
{
    ...
    for (const auto& param : controllers_params) {
        ...
        if (it == base_connectors_to_keys.end()) {
            base_connector_key = base::StrCat({ "base_connector=", base::NumberToString(next_base_connector) });
            drm_device.Set(base_connector_key, base::Value::List());

            base_connectors_to_keys.insert(std::make_pair(next_base_connector, base_connector_key));
        } else {
        ...
        if (param.mode) {
            ...
            const std::string refresh_rate = base::NumberToString(ModeRefreshRate(*param.mode));
            mode = base::StrCat({ size, "@", refresh_rate });
        } else {
            mode = "Disabled";
        }

        const std::string display = base::StrCat({ "{connector=", base::NumberToString(param.connector), " ", "crtc=", base::NumberToString(param.crtc),
            " mode=", mode, "+(", param.origin.ToString(), ")}" });
        drm_device.FindList(base_connector_key)->Append(display);
    }
    const std::string device_name = controllers_params.back().drm->device_path().BaseName().value();
    base::Value::Dict drm_config;
    drm_config.Set(device_name, std::move(drm_device));
    std::string drm_config_log;
    const int json_writer_options = IsPrettyPrintDrmModesetConfigLogsEnabled() ? base::JSONWriter::OPTIONS_PRETTY_PRINT : 0;
    bool json_status = base::JSONWriter::WriteWithOptions(drm_config, json_writer_options, &drm_config_log);
    DCHECK(json_status);
...
bool ControllerContainsCrtcConnectorPair(const HardwareDisplayController& controller, const DrmDisplay::CrtcConnectorPair& crtc_connector_pair)
{
    for (const auto& crtc_controller : controller.crtc_controllers()) {
        const std::optional<TileProperty>& tile_property = crtc_controller->tile_property();
        std::optional<gfx::Point> tile_location;
        if (tile_property.has_value()) {
            tile_location = tile_property->location;
        }

        if (crtc_controller->crtc() == crtc_connector_pair.crtc_id && crtc_controller->connector() == crtc_connector_pair.connector->connector_id
...
void AddNonPrimaryTileControllers(const DrmDisplay& display, HardwareDisplayController& controller)
{
    TRACE_EVENT1("drm", "ScreenManager::AddNonPrimaryTileControllers", "display_id", display.display_id());
    ...
    for (const auto& crtc_connector_pair : display.crtc_connector_pairs()) {
        ...
        if (crtc_connector_pair.crtc_id == display.GetPrimaryCrtcId() && crtc_connector_pair.connector->connector_id == display.GetPrimaryConnectorId()) {
            continue;
        }
        ...
        controller.AddCrtc(
            std::make_unique<CrtcController>(display.drm(), crtc_connector_pair.crtc_id, crtc_connector_pair.connector->connector_id, nonprimary_tile_prop));
    }
}
...
void ScreenManager::AddDisplayController(const scoped_refptr<DrmDevice>& drm, uint32_t crtc, uint32_t connector, std::optional<TileProperty> tile_property)
{
    ...
    controllers_.push_back(std::make_unique<HardwareDisplayController>(
        std::make_unique<CrtcController>(drm, crtc, connector, tile_property), gfx::Point(), drm_modifiers_filter_.get()));
}
...
void ScreenManager::AddDisplayControllersForDisplay(const DrmDisplay& display)
{
    ...
    AddDisplayController(display.drm(), display.GetPrimaryCrtcId(), display.GetPrimaryConnectorId(), tile_property);

    if (!tile_property.has_value()) {
        return;
    }

    HardwareDisplayController& controller = **FindDisplayController(display.drm(), display.GetPrimaryCrtcId());
    AddNonPrimaryTileControllers(display, controller);
}

void ScreenManager::RemoveDisplayControllers(const CrtcsWithDrmList& controllers_to_remove)
{
    TRACE_EVENT1("drm", "ScreenManager::RemoveDisplayControllers", "display_count", controllers_to_remove.size());

    // Split them to different lists unique to each DRM Device.
    base::flat_map<scoped_refptr<DrmDevice>, CrtcsWithDrmList> controllers_for_drm_devices;
    for (const auto& controller : controllers_to_remove) {
        ...
        if (it == controllers_for_drm_devices.end()) {
            controllers_for_drm_devices.insert(std::make_pair(drm, CrtcsWithDrmList()));
        }
    ...
    for (const auto& controllers_on_drm : controllers_for_drm_devices) {
        ...
        for (const auto& controller : drm_controllers_to_remove) {
            ...
            if (crtc->is_enabled()) {
                commit_request.push_back(CrtcCommitRequest::DisableCrtcRequest(crtc->crtc(), crtc->connector()));
            }
        ...
        if (!commit_request.empty()) {
            drm->plane_manager()->Commit(std::move(commit_request), DRM_MODE_ATOMIC_ALLOW_MODESET);
        }
    }
...
bool ScreenManager::ConfigureDisplayControllers(const std::vector<ControllerConfigParams>& controllers_params, display::ModesetFlags modeset_flags)
{
    TRACE_EVENT_BEGIN2("drm", "ScreenManager::ConfigureDisplayControllers", "params",
        ([modeset_flags, &controllers_params](
             perfetto::TracedValue context) -> void { ParamsToTracedValue(std::move(context), controllers_params, modeset_flags); }),
        "before", this);

    // At least one of these flags must be set.
    DCHECK(modeset_flags.HasAny({ display::ModesetFlag::kCommitModeset, display::ModesetFlag::kTestModeset }));

    // Split them to different lists unique to each DRM Device.
    base::flat_map<scoped_refptr<DrmDevice>, std::vector<ControllerConfigParams>> displays_for_drm_devices;

    for (auto& params : controllers_params) {
        auto it = displays_for_drm_devices.find(params.drm);
        if (it == displays_for_drm_devices.end()) {
            displays_for_drm_devices.insert(std::make_pair(params.drm, std::vector<ControllerConfigParams>()));
        }
        displays_for_drm_devices[params.drm].emplace_back(params);
    }

    const bool commit_modeset = modeset_flags.Has(display::ModesetFlag::kCommitModeset);
    const bool is_seamless_modeset = modeset_flags.Has(display::ModesetFlag::kSeamlessModeset);
    bool config_success = true;
    // Perform display configurations together for the same DRM only.
    for (const auto& configs_on_drm : displays_for_drm_devices) {
        const std::vector<ControllerConfigParams>& drm_controllers_params = configs_on_drm.second;
        VLOG(1) << "DRM " << (is_seamless_modeset ? "seamlessly " : "") << (commit_modeset ? "configuring: " : "testing: ")
                << GenerateConfigurationLogForController(drm_controllers_params);

        if (modeset_flags.Has(display::ModesetFlag::kTestModeset)) {
            bool test_modeset = TestAndSetPreferredModifiers(drm_controllers_params, is_seamless_modeset)
        ...
        if (commit_modeset) {
            bool can_modeset_with_overlays = TestModesetWithOverlays(drm_controllers_params, is_seamless_modeset);
            bool modeset_commit_result = Modeset(drm_controllers_params, can_modeset_with_overlays, is_seamless_modeset);
    ...
    TRACE_EVENT_END2("drm", "ScreenManager::ConfigureDisplayControllers", "after", this, "success", config_success);
    return config_success;
}

bool ScreenManager::TestAndSetPreferredModifiers(const std::vector<ControllerConfigParams>& controllers_params, bool is_seamless_modeset)
{
    TRACE_EVENT1("drm", "ScreenManager::TestAndSetPreferredModifiers", "display_count", controllers_params.size());
    ...
    for (const auto& params : controllers_params) {
        ...
        if (params.mode) {
            uint32_t fourcc_format = GetFourCCFormatForOpaqueFramebuffer(display::DisplaySnapshot::PrimaryFormat());
            std::vector<uint64_t> modifiers = controller->GetFormatModifiersForTestModeset(fourcc_format);
            // Test with no overlays to go for a lower bandwidth usage.
            DrmOverlayPlaneList modeset_planes
                = GetModesetPlanes(controller, gfx::Rect(params.origin, ModeSize(*params.mode)), modifiers, /*include_overlays=*/false, /*is_testing=*/true);
            if (modeset_planes.empty())
                return false;

            uint64_t primary_modifier = DrmOverlayPlane::GetPrimaryPlane(modeset_planes)->buffer->format_modifier();
            crtcs_preferred_modifier[params.crtc] = std::make_pair(modifiers.empty(), primary_modifier);

            GetModesetControllerProps(&commit_request, controller, params.origin, *params.mode, modeset_planes, params.enable_vrr);
        } else {
...
bool ScreenManager::TestAndSetLinearModifier(const std::vector<ControllerConfigParams>& controllers_params, bool is_seamless_modeset)
{
    TRACE_EVENT1("drm", "ScreenManager::TestAndSetLinearModifier", "display_count", controllers_params.size());
    ...
    for (const auto& params : controllers_params) {
        ...
        crtcs_preferred_modifier[params.crtc] = std::make_pair(modifiers.empty(), DRM_FORMAT_MOD_LINEAR);
...
void ScreenManager::SetPreferredModifiers(
    const std::vector<ControllerConfigParams>& controllers_params, const CrtcPreferredModifierMap& crtcs_preferred_modifier)
{
    for (const auto& params : controllers_params) {
        if (params.mode) {
            bool was_modifiers_list_empty = crtcs_preferred_modifier.at(params.crtc).first;
            ...
            if (!was_modifiers_list_empty) {
                uint64_t picked_modifier = crtcs_preferred_modifier.at(params.crtc).second;
                auto it = FindDisplayController(params.drm, params.crtc);
                DCHECK(*it);
                it->get()->UpdatePreferredModifierForFormat(display::DisplaySnapshot::PrimaryFormat(), picked_modifier);
            }
        }
    }
}

bool ScreenManager::TestModesetWithOverlays(const std::vector<ControllerConfigParams>& controllers_params, bool is_seamless_modeset)
{
    TRACE_EVENT1("drm", "ScreenManager::TestModesetWithOverlays", "display_count", controllers_params.size());
    ...
    for (const auto& params : controllers_params) {
        ...
        if (params.mode) {
            uint32_t fourcc_format = GetFourCCFormatForOpaqueFramebuffer(display::DisplaySnapshot::PrimaryFormat());
            std::vector<uint64_t> modifiers = controller->GetSupportedModifiers(fourcc_format);

            DrmOverlayPlaneList modeset_planes
                = GetModesetPlanes(controller, gfx::Rect(params.origin, ModeSize(*params.mode)), modifiers, /*include_overlays=*/true, /*is_testing=*/true);
...
bool ScreenManager::Modeset(const std::vector<ControllerConfigParams>& controllers_params, bool can_modeset_with_overlays, bool is_seamless_modeset)
{
    TRACE_EVENT2("drm", "ScreenManager::Modeset", "display_count", controllers_params.size(), "modeset_with_overlays", can_modeset_with_overlays);
    ...
    for (const auto& params : controllers_params) {
        if (params.mode) {
            ...
            uint32_t fourcc_format = GetFourCCFormatForOpaqueFramebuffer(display::DisplaySnapshot::PrimaryFormat());
            std::vector<uint64_t> modifiers = controller->GetSupportedModifiers(fourcc_format, /*is_modeset=*/true);
            ...
            DrmOverlayPlaneList modeset_planes = GetModesetPlanes(controller, bounds, modifiers, can_modeset_with_overlays, /*is_testing=*/false);

            SetDisplayControllerForEnableAndGetProps(
                &commit_request, params.drm, params.crtc, params.connector, params.origin, *params.mode, modeset_planes, params.enable_vrr);

        } else {
            bool disable_set = SetDisableDisplayControllerForDisableAndGetProps(&commit_request, params.drm, params.crtc);
            if (!disable_set)
                return false;
        }
    }
...
void ScreenManager::SetDisplayControllerForEnableAndGetProps(CommitRequest* commit_request, const scoped_refptr<DrmDevice>& drm, uint32_t crtc,
    uint32_t connector, const gfx::Point& origin, const drmModeModeInfo& mode, const DrmOverlayPlaneList& modeset_planes, bool enable_vrr)
{
    HardwareDisplayControllers::iterator it = FindDisplayController(drm, crtc);
    CHECK(controllers_.end() != it, base::NotFatalUntil::M130) << "Display controller (crtc=" << crtc << ") doesn't exist.";
    ...
    if (SameMode(mode, crtc_controller->mode()) && origin == controller->origin() && enable_vrr == crtc_controller->vrr_enabled()) {
        if (!controller->IsEnabled()) {
            ...
            GetModesetControllerProps(commit_request, controller, controller->origin(), mode, modeset_planes, enable_vrr);
        } else {
    ...
    if (controller->IsMirrored()) {
        controllers_.push_back(
            std::make_unique<HardwareDisplayController>(controller->RemoveCrtc(drm, crtc), controller->origin(), drm_modifiers_filter_.get()));
        it = controllers_.end() - 1;
        controller = it->get();
    }

    GetModesetControllerProps(commit_request, controller, origin, mode, modeset_planes, enable_vrr);
}

bool ScreenManager::SetDisableDisplayControllerForDisableAndGetProps(CommitRequest* commit_request, const scoped_refptr<DrmDevice>& drm, uint32_t crtc)
{
    HardwareDisplayControllers::iterator it = FindDisplayController(drm, crtc);
    if (it != controllers_.end()) {
        HardwareDisplayController* controller = it->get();
...
void ScreenManager::UpdateControllerStateAfterModeset(const scoped_refptr<DrmDevice>& drm, const CommitRequest& commit_request, bool did_succeed)
{
    for (const CrtcCommitRequest& crtc_request : commit_request) {
        bool was_enabled = (crtc_request.should_enable_crtc());

        HardwareDisplayControllers::iterator it = FindDisplayController(drm, crtc_request.crtc_id());
        if (it != controllers_.end()) {
...
void ScreenManager::HandleMirrorIfExists(
    const scoped_refptr<DrmDevice>& drm, const CrtcCommitRequest& crtc_request, const HardwareDisplayControllers::iterator& controller)
{
    gfx::Rect modeset_bounds(crtc_request.origin(), ModeSize(crtc_request.mode()));
    HardwareDisplayControllers::iterator mirror = FindActiveDisplayControllerByLocation(drm, modeset_bounds);
...
HardwareDisplayController* ScreenManager::GetDisplayController(const gfx::Rect& bounds)
{
...
HardwareDisplayController* ScreenManager::GetDisplayController(const scoped_refptr<DrmDevice>& drm, int32_t crtc_id)
{
...
void ScreenManager::AddWindow(gfx::AcceleratedWidget widget, std::unique_ptr<DrmWindow> window)
{
    std::pair<WidgetToWindowMap::iterator, bool> result = window_map_.emplace(widget, std::move(window));
    DCHECK(result.second) << "Window already added.";
    UpdateControllerToWindowMapping();
}

std::unique_ptr<DrmWindow> ScreenManager::RemoveWindow(gfx::AcceleratedWidget widget)
{
...
ScreenManager::HardwareDisplayControllers::iterator ScreenManager::FindDisplayController(const scoped_refptr<DrmDevice>& drm, uint32_t crtc)
{
...
ScreenManager::HardwareDisplayControllers::iterator ScreenManager::FindActiveDisplayControllerByLocation(const gfx::Rect& bounds)
{
    for (auto it = controllers_.begin(); it != controllers_.end(); ++it) {
...
ScreenManager::HardwareDisplayControllers::iterator ScreenManager::FindActiveDisplayControllerByLocation(
    const scoped_refptr<DrmDevice>& drm, const gfx::Rect& bounds)
{
    for (auto it = controllers_.begin(); it != controllers_.end(); ++it) {
        gfx::Rect controller_bounds((*it)->origin(), (*it)->GetModeSize());
        if ((*it)->GetDrmDevice() == drm && controller_bounds == bounds && (*it)->IsEnabled())
            return it;
    }
...
void ScreenManager::UpdateControllerToWindowMapping()
{
    ...
    for (const auto& controller : controllers_) {
        ...
        DrmWindow* window = FindWindowAt(gfx::Rect(controller->origin(), controller->GetModeSize()));
        if (!window)
            continue;
    ...
    for (auto& pair : window_map_) {
        ...
        bool should_enable = controller && pair.second->GetController() && pair.second->GetController() != controller;
        ...
        if (should_enable) {
            uint32_t fourcc_format = GetFourCCFormatForOpaqueFramebuffer(display::DisplaySnapshot::PrimaryFormat());
            std::vector<uint64_t> modifiers = controller->GetSupportedModifiers(fourcc_format);
            DrmOverlayPlaneList modeset_planes = GetModesetPlanes(controller, gfx::Rect(controller->origin(), controller->GetModeSize()), modifiers,
                /*include_overlays=*/true, /*is_testing=*/false);
            DCHECK(!modeset_planes.empty());

            CommitRequest commit_request;
            GetEnableControllerProps(&commit_request, controller, modeset_planes);
            controller->GetDrmDevice()->plane_manager()->Commit(std::move(commit_request), DRM_MODE_ATOMIC_ALLOW_MODESET);
        }
    }
}
...
DrmOverlayPlaneList ScreenManager::GetModesetPlanes(
    HardwareDisplayController* controller, const gfx::Rect& bounds, const std::vector<uint64_t>& modifiers, bool include_overlays, bool is_testing)
{
    ...
    std::unique_ptr<GbmBuffer> buffer = drm->gbm_device()->CreateBufferWithModifiers(fourcc_format, bounds.size(), GBM_BO_USE_SCANOUT, modifiers);
    if (!buffer) {
    ...
    if (window) {
        const DrmOverlayPlaneList& last_submitted_planes = window->last_submitted_planes();
        const DrmOverlayPlane* primary = DrmOverlayPlane::GetPrimaryPlane(last_submitted_planes);
        if (primary && primary->buffer->size() == bounds.size() && primary->buffer->drm_device() == controller->GetDrmDevice().get()
    ...
    scoped_refptr<DrmFramebuffer> framebuffer = DrmFramebuffer::AddFramebuffer(drm, buffer.get(), buffer->GetSize(), modifiers);
    if (!framebuffer) {
...
void ScreenManager::GetEnableControllerProps(CommitRequest* commit_request, HardwareDisplayController* controller, const DrmOverlayPlaneList& modeset_planes)
{
    DCHECK(!controller->crtc_controllers().empty());
...
void ScreenManager::GetModesetControllerProps(CommitRequest* commit_request, HardwareDisplayController* controller, const gfx::Point& origin,
    const drmModeModeInfo& mode, const DrmOverlayPlaneList& modeset_planes, bool enable_vrr)
{
    DCHECK(!controller->crtc_controllers().empty());
...
void ScreenManager::SetDrmModifiersFilter(std::unique_ptr<DrmModifiersFilter> filter)
{
...
bool ScreenManager::ReplaceDisplayControllersCrtcs(
    const scoped_refptr<DrmDevice>& drm, const ConnectorCrtcMap& current_pairings, const ConnectorCrtcMap& new_pairings)
{
    std::vector<std::pair<uint32_t /*connector_id*/, HardwareDisplayController*>> connector_to_controllers;
    for (const auto& [connector_id, crtc_id] : current_pairings) {
        ...
        if (hdc_it == controllers_.end()) {
            LOG(DFATAL) << __func__ << " controller not found for connector ID: " << connector_id << " crtc ID: " << crtc_id;
            return false;
        }
    ...
    // old CrtcController.
    base::flat_map<uint32_t /*connector_id*/, std::optional<TileProperty>> connector_tile_properties;
    // First, remove the CRTC.
    for (auto& [connector_id, hdc] : connector_to_controllers) {
        auto crtc_controller = hdc->RemoveCrtc(drm, current_pairings.at(connector_id));
    ...
    // HDCs share a CRTC.
    for (auto& [connector_id, hdc] : connector_to_controllers) {
        hdc->AddCrtc(std::make_unique<CrtcController>(drm, new_pairings.at(connector_id), connector_id, connector_tile_properties[connector_id]));
    }
