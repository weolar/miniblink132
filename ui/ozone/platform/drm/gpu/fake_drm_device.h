...
template <class Object> Object* DrmAllocator(size_t num_of_objects = 1)
{
...
public:
    ...
    struct FakeDrmState {
        ...
        std::map<uint32_t, std::vector<std::pair<uint64_t, std::string>>> enum_values;
    };

    explicit FakeDrmDevice(std::unique_ptr<GbmDevice> gbm_device);
    explicit FakeDrmDevice(const base::FilePath& path, std::unique_ptr<GbmDevice> gbm_device, bool is_primary_device);
    ...
    ScopedDrmPropertyBlob CreateSizeHintsBlob(const std::vector<gfx::Size>& sizes);
    int get_set_crtc_call_count() const
    {
        return set_crtc_call_count_;
    }
    ...
    int get_page_flip_call_count() const
    {
        return page_flip_call_count_;
    }
    int get_overlay_clear_call_count() const
    {
        return overlay_clear_call_count_;
    }
    int get_test_modeset_count() const
    {
        return test_modeset_count_;
    }
    int get_commit_modeset_count() const
    {
        return commit_modeset_count_;
    }
    int get_seamless_modeset_count() const
    {
        return seamless_modeset_count_;
    }
    ...
    int get_set_gamma_ramp_count() const
    {
        return set_gamma_ramp_count_;
    }
    ...
    void set_commit_expectation(bool state)
    {
        commit_expectation_ = state;
    }
    ...
    uint32_t current_framebuffer() const
    {
    ...
    // `drm_state_`.
    FakeDrmDevice::FakeDrmState& ResetStateWithDefaultObjects(size_t crtc_count, size_t planes_per_crtc, size_t movable_planes = 0u,
        std::vector<uint32_t> plane_supported_formats = { DRM_FORMAT_XRGB8888 }, std::vector<drm_format_modifier> plane_supported_format_modifiers = {});
    ...
    void SetPossibleValuesForEnumProperty(uint32_t property_id, std::vector<std::pair<uint64_t /* value */, std::string /* name */>> values);
    ...
    PlaneProperties& AddPlane(uint32_t crtc_id, uint32_t type, std::vector<uint32_t> supported_formats = { DRM_FORMAT_XRGB8888 },
        std::vector<drm_format_modifier> supported_format_modifiers = {});
    PlaneProperties& AddPlane(const std::vector<uint32_t>& crtc_ids, uint32_t type, std::vector<uint32_t> supported_formats = { DRM_FORMAT_XRGB8888 },
        std::vector<drm_format_modifier> supported_format_modifiers = {});
    ...
    ScopedDrmObjectPropertyPtr GetObjectProperties(uint32_t object_id, uint32_t object_type) const override;
    ScopedDrmCrtcPtr GetCrtc(uint32_t crtc_id) const override;
    bool SetCrtc(uint32_t crtc_id, uint32_t framebuffer, std::vector<uint32_t> connectors, const drmModeModeInfo& mode) override;
    ...
    bool AddFramebuffer2(uint32_t width, uint32_t height, uint32_t format, uint32_t handles[4], uint32_t strides[4], uint32_t offsets[4], uint64_t modifiers[4],
        uint32_t* framebuffer, uint32_t flags) override;
    ...
    bool PageFlip(uint32_t crtc_id, uint32_t framebuffer, scoped_refptr<PageFlipRequest> page_flip_request) override;
    ScopedDrmPlanePtr GetPlane(uint32_t plane_id) const override;
    ScopedDrmPropertyPtr GetProperty(drmModeConnector* connector, const char* name) const override;
    ScopedDrmPropertyPtr GetProperty(uint32_t id) const override;
    bool SetProperty(uint32_t connector_id, uint32_t property_id, uint64_t value) override;
    ScopedDrmPropertyBlob CreatePropertyBlob(const void* blob, size_t size) override;
    ...
    ScopedDrmPropertyBlobPtr GetPropertyBlob(drmModeConnector* connector, const char* name) const override;
    bool SetObjectProperty(uint32_t object_id, uint32_t object_type, uint32_t property_id, uint32_t property_value) override;
    bool SetCursor(uint32_t crtc_id, uint32_t handle, const gfx::Size& size) override;
    bool MoveCursor(uint32_t crtc_id, const gfx::Point& point) override;
    bool CommitProperties(drmModeAtomicReq* request, uint32_t flags, uint32_t crtc_count, scoped_refptr<PageFlipRequest> callback) override;
    bool CreateDumbBuffer(const SkImageInfo& info, uint32_t* handle, uint32_t* stride) override;
    ...
    bool SetGammaRamp(uint32_t crtc_id, const display::GammaCurve& curve) override;
...
private:
    ...
    bool UpdateProperty(uint32_t id, uint64_t value, std::vector<DrmWrapper::Property>* properties, bool add_property_if_needed = false);

    bool UpdateProperty(uint32_t object_id, uint32_t property_id, uint64_t value, bool add_property_if_needed = false);
