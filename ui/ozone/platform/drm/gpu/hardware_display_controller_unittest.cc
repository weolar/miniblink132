...
const gfx::Size kOverlaySize(kDefaultMode.hdisplay / 2, kDefaultMode.vdisplay / 2);
const gfx::SizeF kDefaultModeSizeF(1.0, 1.0);

const std::string kGpuCrashLogTimeout
    = "Failed to modeset within " + base::NumberToString(kWaitForModesetTimeout.InSeconds()) + " s of the first page flip failure. Crashing GPU process.";
...
public:
    ...
    HardwareDisplayControllerTest& operator=(const HardwareDisplayControllerTest&) = delete;
    ...
    void InitializeDrmDevice(bool use_atomic, size_t movable_planes = 0, const std::vector<uint64_t>& supported_modifiers = {},
        std::unique_ptr<DrmModifiersFilter> modifiers_filter = nullptr, bool has_size_hints = false);
    void SchedulePageFlip(DrmOverlayPlaneList planes);
    void OnSubmission(gfx::SwapResult swap_result, gfx::GpuFenceHandle release_fence);
    ...
    {
        std::unique_ptr<GbmBuffer> buffer = drm_->gbm_device()->CreateBuffer(DRM_FORMAT_XRGB8888, kDefaultModeSize, GBM_BO_USE_SCANOUT);
        return DrmFramebuffer::AddFramebuffer(drm_, buffer.get(), kDefaultModeSize);
    }

    scoped_refptr<DrmFramebuffer> CreateOverlayBuffer()
    {
        std::unique_ptr<GbmBuffer> buffer = drm_->gbm_device()->CreateBuffer(DRM_FORMAT_XRGB8888, kOverlaySize, GBM_BO_USE_SCANOUT);
...
protected:
    bool ModesetWithPlanes(const DrmOverlayPlaneList& modeset_planes, const drmModeModeInfo& mode = kDefaultMode);
...
void HardwareDisplayControllerTest::InitializeDrmDevice(bool use_atomic, size_t movable_planes, const std::vector<uint64_t>& supported_modifiers,
    std::unique_ptr<DrmModifiersFilter> modifiers_filter, bool has_size_hints)
{
    ...
    drm_->ResetStateWithDefaultObjects(
        /*crtc_count=*/2, /*planes_per_crtc*/ 2, movable_planes, { DRM_FORMAT_XRGB8888 }, drm_format_modifiers);

    if (has_size_hints) {
        std::vector<gfx::Size> supported_cursor_sizes = { gfx::Size(64, 64), gfx::Size(128, 128), gfx::Size(256, 256) };
        for (FakeDrmDevice::PlaneProperties plane : drm_->plane_properties()) {
            for (DrmWrapper::Property property : plane.properties) {
                if (property.id == kTypePropId && property.value == DRM_PLANE_TYPE_CURSOR) {
                    ScopedDrmPropertyBlob size_hints_blob = drm_->CreateSizeHintsBlob(supported_cursor_sizes);
                    drm_->AddProperty(plane.id, { .id = kSizeHintsPropId, .value = size_hints_blob->id() });
                }
            }
        }
    }
    ...
    drm_->AddProperty(drm_->crtc_property(0).id, { .id = kVrrEnabledPropId, .value = 0 });
    ...
    controller_ = std::make_unique<HardwareDisplayController>(
        std::make_unique<CrtcController>(drm_.get(), primary_crtc_, kConnectorIdBase), gfx::Point(), modifiers_filter_.get());
}

bool HardwareDisplayControllerTest::ModesetWithPlanes(const DrmOverlayPlaneList& modeset_planes, const drmModeModeInfo& mode)
{
    CommitRequest commit_request;
    controller_->GetModesetProps(&commit_request, modeset_planes, mode,
        /*enable_vrr=*/false);
    CommitRequest request_for_update = commit_request;
    bool status = drm_->plane_manager()->Commit(std::move(commit_request), DRM_MODE_ATOMIC_ALLOW_MODESET);
...
void HardwareDisplayControllerTest::SchedulePageFlip(DrmOverlayPlaneList planes)
{
    controller_->SchedulePageFlip(std::move(planes), base::BindOnce(&HardwareDisplayControllerTest::OnSubmission, base::Unretained(this)),
        base::BindOnce(&HardwareDisplayControllerTest::OnPresentation, base::Unretained(this)));
}

void HardwareDisplayControllerTest::OnSubmission(gfx::SwapResult result, gfx::GpuFenceHandle release_fence)
{
    last_swap_result_ = result;
}

void HardwareDisplayControllerTest::OnPresentation(const gfx::PresentationFeedback& feedback)
{
    if (!feedback.failed()) {
...
uint64_t HardwareDisplayControllerTest::GetPlanePropertyValue(uint32_t plane, const std::string& property_name)
{
    DrmWrapper::Property p {};
    ScopedDrmObjectPropertyPtr properties(drm_->GetObjectProperties(plane, DRM_MODE_OBJECT_PLANE));
    EXPECT_TRUE(GetDrmPropertyForName(drm_.get(), properties.get(), property_name, &p));
    return p.value;
}

class HardwareDisplayControllerMockedDeviceTest : public HardwareDisplayControllerTest {
    void SetUp() override
    {
        successful_page_flips_count_ = 0;
        last_swap_result_ = gfx::SwapResult::SWAP_FAILED;
        ...
        auto gbm_device = std::make_unique<MockGbmDevice>();
        drm_ = new testing::NiceMock<MockDrmDevice>(base::FilePath(), std::move(gbm_device), true);
        InitializeDrmDevice(/* use_atomic= */ true);
    }
};
...
TEST_F(HardwareDisplayControllerTest, CheckModesettingResult)
{
    ...
    EXPECT_FALSE(DrmOverlayPlane::GetPrimaryPlane(modeset_planes)->buffer->HasOneRef());
}
...
TEST_F(HardwareDisplayControllerTest, CrtcPropsAfterModeset)
{
    ...
    ScopedDrmObjectPropertyPtr crtc_props = drm_->GetObjectProperties(primary_crtc_, DRM_MODE_OBJECT_CRTC);
    {
...
TEST_F(HardwareDisplayControllerTest, ConnectorPropsAfterModeset)
{
    ...
    ScopedDrmObjectPropertyPtr connector_props = drm_->GetObjectProperties(kConnectorIdBase, DRM_MODE_OBJECT_CONNECTOR);
    ...
    {
        DrmWrapper::Property prop = {};
        GetDrmPropertyForName(drm_.get(), connector_props.get(), "link-status", &prop);
...
TEST_F(HardwareDisplayControllerTest, BadLinkStatusConnectorPropsAfterModeset)
{
    ...
    ScopedDrmObjectPropertyPtr bad_link_connector_props = drm_->GetObjectProperties(kNoModesConnectorId, DRM_MODE_OBJECT_CONNECTOR);
    {
        DrmWrapper::Property prop = {};
        GetDrmPropertyForName(drm_.get(), bad_link_connector_props.get(), "link-status", &prop);
        EXPECT_EQ(kLinkStatusPropId, prop.id);
...
TEST_F(HardwareDisplayControllerTest, PlanePropsAfterModeset)
{
    ...
    DrmOverlayPlaneList modeset_planes;
    modeset_planes.push_back(DrmOverlayPlane::TestPlane(CreateBuffer(), gfx::ColorSpace::CreateSRGB(), fake_fence_fd.GetGpuFence()));
    EXPECT_TRUE(ModesetWithPlanes(modeset_planes));

    ScopedDrmObjectPropertyPtr plane_props = drm_->GetObjectProperties(kPlaneOffset, DRM_MODE_OBJECT_PLANE);
    const DrmOverlayPlane* primary_plane = DrmOverlayPlane::GetPrimaryPlane(modeset_planes);
    ...
    }
    {
        ...
        EXPECT_EQ(primary_plane->buffer->opaque_framebuffer_id(), static_cast<uint32_t>(prop.value));
    }

    gfx::RectF crop_rectf = primary_plane->crop_rect;
    crop_rectf.Scale(primary_plane->buffer->size().width(), primary_plane->buffer->size().height());
    gfx::Rect crop_rect = gfx::ToNearestRect(crop_rectf);
    gfx::Rect fixed_point_rect = gfx::Rect(crop_rect.x() << 16, crop_rect.y() << 16, crop_rect.width() << 16, crop_rect.height() << 16);
    {
        DrmWrapper::Property prop = {};
...
TEST_F(HardwareDisplayControllerTest, FenceFdValueChange)
{
    ...
    // Test invalid fence fd
    {
        ...
        ScopedDrmObjectPropertyPtr plane_props = drm_->GetObjectProperties(kPlaneOffset, DRM_MODE_OBJECT_PLANE);
        GetDrmPropertyForName(drm_.get(), plane_props.get(), "IN_FENCE_FD", &fence_fd_prop);
    ...
    // Verify fence FD after a GPU Fence is added to the plane.
    {
        ...
        GetDrmPropertyForName(drm_.get(), plane_props.get(), "IN_FENCE_FD", &fence_fd_prop);
        EXPECT_EQ(kInFencePropId, fence_fd_prop.id);
        EXPECT_LT(base::kInvalidPlatformFile, static_cast<int>(fence_fd_prop.value));
    }
...
TEST_F(HardwareDisplayControllerTest, CheckStateAfterPageFlip)
{
    ...
    drm_->RunCallbacks();
    EXPECT_TRUE(DrmOverlayPlane::GetPrimaryPlane(modeset_planes)->buffer->HasOneRef());
...
TEST_F(HardwareDisplayControllerTest, CheckOverlayPresent)
{
    ...
    planes.emplace_back(CreateOverlayBuffer(), gfx::ColorSpace::CreateSRGB(), 1, gfx::OVERLAY_TRANSFORM_NONE, gfx::Rect(), gfx::Rect(kOverlaySize),
...
TEST_F(HardwareDisplayControllerTest, AcceptUnderlays)
{
    DrmOverlayPlaneList planes;
    planes.push_back(DrmOverlayPlane::TestPlane(CreateBuffer()));
    planes.emplace_back(CreateBuffer(), gfx::ColorSpace::CreateSRGB(), -1, gfx::OVERLAY_TRANSFORM_NONE, gfx::Rect(), gfx::Rect(kDefaultModeSize),
        gfx::RectF(kDefaultModeSizeF), true, nullptr);
...
{
    controller_->AddCrtc(std::make_unique<CrtcController>(drm_.get(), secondary_crtc_, drm_->connector_property(1).id));
...
TEST_F(HardwareDisplayControllerTest, PlaneStateAfterRemoveCrtc)
{
    ...
    std::unique_ptr<CrtcController> crtc = controller_->RemoveCrtc(drm_, primary_crtc_);
...
TEST_F(HardwareDisplayControllerTest, PlaneStateAfterAddCrtc)
{
    ...
    auto hdc_controller = std::make_unique<HardwareDisplayController>(controller_->RemoveCrtc(drm_, primary_crtc_), controller_->origin(), nullptr);
    SchedulePageFlip(DrmOverlayPlane::Clone(planes));
    drm_->RunCallbacks();
    EXPECT_EQ(gfx::SwapResult::SWAP_ACK, last_swap_result_);
    EXPECT_EQ(2, successful_page_flips_count_);
    ...
    hdc_controller->SchedulePageFlip(DrmOverlayPlane::Clone(planes), base::BindOnce(&HardwareDisplayControllerTest::OnSubmission, base::Unretained(this)),
        base::BindOnce(&HardwareDisplayControllerTest::OnPresentation, base::Unretained(this)));
    drm_->RunCallbacks();
    EXPECT_EQ(gfx::SwapResult::SWAP_ACK, last_swap_result_);
...
TEST_F(HardwareDisplayControllerTest, FailPageFlippingWithNoSavingModeset)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(task_environment_.FastForwardBy(kWaitForModesetTimeout), kGpuCrashLogTimeout);
}
...
TEST_F(HardwareDisplayControllerTest, RecreateBuffersOnOldPlanesPageFlipFailure)
{
    DrmOverlayPlaneList planes;
    planes.push_back(DrmOverlayPlane::TestPlane(CreateBuffer()));
...
TEST_F(HardwareDisplayControllerTest, CheckNoPrimaryPlaneOnFlip)
{
    ...
    std::vector<DrmOverlayPlane> page_flip_planes;
    page_flip_planes.emplace_back(CreateBuffer(), gfx::ColorSpace::CreateSRGB(), 1, gfx::OVERLAY_TRANSFORM_NONE, gfx::Rect(), gfx::Rect(kDefaultModeSize),
...
TEST_F(HardwareDisplayControllerTest, SomePlaneAssignmentFailuresAreOk)
{
    ...
    for (size_t i = 0; i < kUnassignableFlips; ++i) {
        {
            std::vector<DrmOverlayPlane> page_flip_planes;
            page_flip_planes.emplace_back(CreateBuffer(), gfx::ColorSpace::CreateSRGB(), 1, gfx::OVERLAY_TRANSFORM_NONE, gfx::Rect(),
    ...
    for (size_t i = 0; i < kPageFlipWatcherHistorySize - kUnassignableFlips; ++i) {
        drm_->set_commit_expectation(true);
        {
            std::vector<DrmOverlayPlane> page_flip_planes;
            page_flip_planes.emplace_back(CreateBuffer(), gfx::ColorSpace::CreateSRGB(), 1, gfx::OVERLAY_TRANSFORM_NONE, gfx::Rect(),
...
TEST_F(HardwareDisplayControllerTest, CrashOnTooManyFlakyPlaneAssignments)
{
    ...
    ASSERT_GT(kPageFlipWatcherHistorySize, flakes) << "Page flip history is too small to account for the maximum number of "
                                                      "flakes";
    ...
    EXPECT_DEATH_IF_SUPPORTED(do_flake(),
...
TEST_F(HardwareDisplayControllerTest, CrashOnTooManyFailedPlaneAssignments)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(do_failed_flip(),
        base::StringPrintf("Plane assignment has failed %d/%d times, but the "
                           "threshold is %d. Crashing the GPU process.",
            failures, kPageFlipWatcherHistorySize, kPlaneAssignmentMaximumFailures));
}
...
TEST_F(HardwareDisplayControllerTest, AddCrtcMidPageFlip)
{
    ...
    controller_->AddCrtc(std::make_unique<CrtcController>(drm_.get(), secondary_crtc_, kConnectorIdBase + 1));
...
TEST_F(HardwareDisplayControllerTest, PageflipAfterModeset)
{
    ...
    for (const auto& plane : planes) {
        EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(primary_crtc_).modeset_framebuffers, plane.buffer));
    }
    ...
    EXPECT_TRUE(drm_->plane_manager()->GetCrtcStateForCrtcId(primary_crtc_).modeset_framebuffers.empty());
}
...
TEST_F(HardwareDisplayControllerTest, PageflipBeforeModeset)
{
    ...
    // after a modeset
    drm_->RunCallbacks();
    EXPECT_FALSE(drm_->plane_manager()->GetCrtcStateForCrtcId(primary_crtc_).modeset_framebuffers.empty());
    for (const auto& plane : planes) {
        EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(primary_crtc_).modeset_framebuffers, plane.buffer));
    }
}
...
TEST_F(HardwareDisplayControllerTest, MultiplePlanesModeset)
{
    ...
    modeset_planes.push_back(DrmOverlayPlane::TestPlane(CreateBuffer()));
    ASSERT_TRUE(ModesetWithPlanes(modeset_planes));
    EXPECT_EQ(drm_->plane_manager()->GetCrtcStateForCrtcId(primary_crtc_).modeset_framebuffers.size(), 2UL);
...
TEST_F(HardwareDisplayControllerTest, CheckPinningAfterPageFlip)
{
    ...
    for (auto& plane : drm_->plane_manager()->planes()) {
        if (plane->in_use()) {
            EXPECT_EQ(controller_->crtc_controllers()[0]->crtc(), plane->owning_crtc());
            in_use_planes++;
        }
    }
...
TEST_F(HardwareDisplayControllerTest, CheckPinningAfterFailedPageFlip)
{
    ...
    size_t in_use_planes = base::ranges::count_if(drm_->plane_manager()->planes(), [](const auto& plane) { return plane->in_use(); });
    EXPECT_EQ(0u, in_use_planes) << "Planes, including pinned planes, should not "
                                    "be in use after a failed flip.";
}

TEST_F(HardwareDisplayControllerTest, PinnedPlanesAreRespectedDuringModesetting)
{
    InitializeDrmDevice(/*use_atomic=*/true, /*movable_planes=*/1);
    ...
    EXPECT_FALSE(ModesetWithPlanes(modeset_planes)) << "Modesetting should fail if it requires a movable plane that is "
    ...
    EXPECT_TRUE(ModesetWithPlanes(modeset_planes)) << "Modesetting with movable planes should work once those movable "
...
TEST_F(HardwareDisplayControllerTest, AddingAndRemovingCrtcsWithMovablePlanes)
{
    ...
    controller_->AddCrtc(std::make_unique<CrtcController>(drm_, secondary_crtc_, drm_->connector_property(1).id));
    ...
    EXPECT_FALSE(ModesetWithPlanes(modeset_planes)) << "Should not modeset when two CRTCs both need the movable overlay "
                                                       "plane.";

    modeset_planes.pop_back();
    EXPECT_TRUE(ModesetWithPlanes(modeset_planes)) << "Modesetting should work when neither CRTC needs the movable overlay "
                                                      "plane";

    {
        ...
        drm_->RunCallbacks();
        EXPECT_TRUE(last_presentation_feedback_.failed()) << "Only one of the CRTCs should be able to use an additional plane.";
    }

    {
        ...
        SchedulePageFlip(std::move(flip_planes));
        drm_->RunCallbacks();
        EXPECT_FALSE(last_presentation_feedback_.failed()) << "Both CRTCs should be able to flip with their own overlays.";
    }
    ...
    EXPECT_TRUE(removed_crtc);
    {
        ...
        SchedulePageFlip(std::move(flip_planes));
        drm_->RunCallbacks();
        EXPECT_FALSE(last_presentation_feedback_.failed()) << "With only one CRTC to flip, we should be able to use the movable "
                                                              "plane again.";
    }
}

TEST_F(HardwareDisplayControllerTest, ModesettingWithMirroringAndMultipleMovablePlanes)
{
    ...
    EXPECT_TRUE(ModesetWithPlanes(modeset_planes)) << "Should be able modeset with two CRTCs and two movable planes.";
}

TEST_F(HardwareDisplayControllerTest, ModifiersFilter)
{
    std::vector<uint64_t> filter_modifiers = { DRM_FORMAT_MOD_LINEAR, I915_FORMAT_MOD_X_TILED };
    std::unique_ptr<MockDrmModifiersFilter> filter = std::make_unique<MockDrmModifiersFilter>(filter_modifiers);
    InitializeDrmDevice(/*use_atomic=*/true, /*movable_planes=*/0, { I915_FORMAT_MOD_X_TILED, I915_FORMAT_MOD_Y_TILED }, std::move(filter));

    std::vector<uint64_t> valid_modifiers = controller_->GetFormatModifiersForTestModeset(DRM_FORMAT_XRGB8888);
...
TEST_F(HardwareDisplayControllerMockedDeviceTest, TestSeamlessMode_MatchingSizeFailedProbe)
{
    ...
    // TestSeamlessMode.
    EXPECT_CALL(*mock_drm, CommitProperties(_, DRM_MODE_ATOMIC_TEST_ONLY, 1, _)).Times(1).WillRepeatedly(Return(false));
    EXPECT_FALSE(controller_->TestSeamlessMode(primary_crtc_, matching_size_mode));
}
...
TEST_F(HardwareDisplayControllerTest, GetTileProperty)
{
    TileProperty tile_property
        = { .group_id = 123, .scale_to_fit_display = true, .tile_size = gfx::Size(300, 200), .tile_layout = gfx::Size(3, 2), .location = gfx::Point(1, 2) };

    auto tiled_crtc_controller = std::make_unique<CrtcController>(drm_.get(), secondary_crtc_, drm_->connector_property(1).id, tile_property);
    auto hdc_controller = std::make_unique<HardwareDisplayController>(std::move(tiled_crtc_controller), gfx::Point(), nullptr);

    EXPECT_TRUE(hdc_controller->IsTiled());
    EXPECT_THAT(hdc_controller->GetTileProperty(), Optional(EqTileProperty(tile_property)));
}

TEST_F(HardwareDisplayControllerTest, TileCompositedModeset)
{
    InitializeDrmDevice(/*use_atomic=*/true, /*movable_planes=*/2);
    TileProperty primary_tile_property
        = { .group_id = 123, .scale_to_fit_display = true, .tile_size = gfx::Size(300, 200), .tile_layout = gfx::Size(2, 1), .location = gfx::Point(0, 0) };

    auto primary_tiled_crtc_controller = std::make_unique<CrtcController>(drm_.get(), primary_crtc_, drm_->connector_property(0).id, primary_tile_property);

    TileProperty nonprimary_tile_property = primary_tile_property;
    nonprimary_tile_property.location = gfx::Point(1, 0);
    auto nonprimary_tiled_crtc_controller
        = std::make_unique<CrtcController>(drm_.get(), secondary_crtc_, drm_->connector_property(1).id, nonprimary_tile_property);

    controller_ = std::make_unique<HardwareDisplayController>(std::move(primary_tiled_crtc_controller), gfx::Point(), nullptr);
    ...
    EXPECT_THAT(controller_->crtc_controllers(),
        testing::UnorderedElementsAre(
            Pointee(AllOf(Property(&CrtcController::crtc, Eq(primary_crtc_)), Property(&CrtcController::CurrentModeIsTiled, Eq(true)),
                Property(&CrtcController::is_enabled, Eq(true)), Property(&CrtcController::tile_property, Optional(EqTileProperty(primary_tile_property))))),
            Pointee(AllOf(Property(&CrtcController::crtc, Eq(secondary_crtc_)), Property(&CrtcController::CurrentModeIsTiled, Eq(true)),
                Property(&CrtcController::is_enabled, Eq(true)),
                Property(&CrtcController::tile_property, Optional(EqTileProperty(nonprimary_tile_property)))))));
}
...
TEST_F(HardwareDisplayControllerTest, ModesetTileToNonTileMode)
{
    ...
    EXPECT_THAT(controller_->crtc_controllers(),
        testing::UnorderedElementsAre(Pointee(AllOf(Property(&CrtcController::crtc, Eq(primary_crtc_)),
                                          Property(&CrtcController::CurrentModeIsTiled, Eq(false)), Property(&CrtcController::is_enabled, Eq(true)))),
            Pointee(AllOf(Property(&CrtcController::crtc, Eq(secondary_crtc_)), Property(&CrtcController::is_enabled, Eq(false))))));
}
...
TEST_F(HardwareDisplayControllerTest, TileDisplayPageflipWithCrtcOffset)
{
    ...
    std::unordered_map<uint32_t, std::vector<HardwareDisplayPlane*>> crtc_planes_map;
    for (const auto& plane : drm_->plane_manager()->planes()) {
    ...
    std::unordered_map<uint32_t, gfx::Point> crtc_offset_map = { { primary_crtc_, gfx::Point(0, 0) }, { secondary_crtc_, gfx::Point(-300, 0) } };
    for (const auto& [crtc, crtc_planes] : crtc_planes_map) {
        ...
        for (const auto* plane : crtc_planes) {
            ScopedDrmObjectPropertyPtr plane_props = drm_->GetObjectProperties(plane->id(), DRM_MODE_OBJECT_PLANE);
            {
                DrmWrapper::Property prop;
                GetDrmPropertyForName(drm_.get(), plane_props.get(), "CRTC_X", &prop);
...
TEST_F(HardwareDisplayControllerTest, TileDisplayMoveCursor)
{
    ...
    EXPECT_EQ(drm_->get_crtc_cursor_location(secondary_crtc_), gfx::Point(kCursorLocation.x() - 300, kCursorLocation.y() - 0));
}
