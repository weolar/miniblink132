...
public:
    ...
    scoped_refptr<DrmFramebuffer> ReturnNullBuffer(const gfx::Size& size, uint32_t format)
    {
        return nullptr;
    }
    ...
    {
        auto gbm_buffer = drm_->gbm_device()->CreateBuffer(DRM_FORMAT_XRGB8888, primary_rect_.size(), GBM_BO_USE_SCANOUT);
        return DrmFramebuffer::AddFramebuffer(drm_, gbm_buffer.get(), primary_rect_.size());
    }

    scoped_refptr<DrmFramebuffer> CreateOverlayBuffer(uint32_t format, const gfx::Size& size)
    {
        auto gbm_buffer = drm_->gbm_device()->CreateBuffer(format, size, GBM_BO_USE_SCANOUT);
...
protected:
    ...
    void InitDrmStatesAndControllers(const std::vector<CrtcState>& crtc_states, const std::vector<PlaneState>& movable_planes = {});
...
void DrmOverlayValidatorTest::InitDrmStatesAndControllers(const std::vector<CrtcState>& crtc_states, const std::vector<PlaneState>& movable_planes)
{
    size_t plane_count = crtc_states[0].planes.size();
    for (const auto& crtc_state : crtc_states) {
        ASSERT_EQ(plane_count, crtc_state.planes.size()) << "FakeDrmDevice::CreateStateWithDefaultObjects currently expects the "
    ...
    for (const auto& crtc_state : crtc_states) {
        ...
        for (size_t i = 0; i < crtc_state.planes.size(); ++i) {
            auto in_formats_blob = drm_->CreateInFormatsBlob(crtc_state.planes[i].formats, {});

            auto& plane = drm_->AddPlane(crtc_id, i == 0 ? DRM_PLANE_TYPE_PRIMARY : DRM_PLANE_TYPE_OVERLAY);
...
void DrmOverlayValidatorTest::SetupControllers()
{
    ...
    controllers_to_enable.emplace_back(
        1 /*display_id*/, drm_, primary_crtc_id, primary_connector_id, gfx::Point(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    ...
    std::unique_ptr<DrmWindow> window(new DrmWindow(kDefaultWidgetHandle, drm_device_manager_.get(), screen_manager_.get()));
    window->Initialize();
    window->SetBounds(gfx::Rect(gfx::Size(kDefaultMode.hdisplay, kDefaultMode.vdisplay)));
...
void DrmOverlayValidatorTest::AddPlane(const OverlaySurfaceCandidate& params)
{
    ...
    scoped_refptr<DrmFramebuffer> drm_framebuffer = CreateOverlayBuffer(GetFourCCFormatFromBufferFormat(params.format), params.buffer_size);
    plane_list_.emplace_back(std::move(drm_framebuffer), params.color_space, params.plane_z_order, absl::get<gfx::OverlayTransform>(params.transform),
        gfx::Rect(), gfx::ToNearestRect(params.display_rect), params.crop_rect, true, nullptr);
}

void DrmOverlayValidatorTest::TearDown()
{
    std::unique_ptr<DrmWindow> window = screen_manager_->RemoveWindow(kDefaultWidgetHandle);
    window->Shutdown();
...
TEST_F(DrmOverlayValidatorTest, WindowWithNoController)
{
    ...
    std::vector<OverlayStatus> returns = overlay_validator_->TestPageFlip(overlay_params_, DrmOverlayPlaneList());
...
TEST_F(DrmOverlayValidatorTest, OverlayFormat_XRGB)
{
    ...
    CrtcState state = { .planes = { { .formats = { DRM_FORMAT_XRGB8888, DRM_FORMAT_NV12 } }, { .formats = { DRM_FORMAT_XRGB8888 } } } };
...
TEST_F(DrmOverlayValidatorTest, OverlayFormat_YUV)
{
    ...
    // needed.
    CrtcState state = { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888, DRM_FORMAT_NV12 } } } };
    InitDrmStatesAndControllers(std::vector<CrtcState>(1, state));
...
TEST_F(DrmOverlayValidatorTest, RejectYUVBuffersIfNotSupported)
{
    ...
    // support it.
    CrtcState state = { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } } } };
    InitDrmStatesAndControllers(std::vector<CrtcState>(1, state));
    ...
    std::vector<OverlayStatus> returns = overlay_validator_->TestPageFlip(validated_params, DrmOverlayPlaneList());
    EXPECT_EQ(2u, returns.size());
    EXPECT_EQ(returns.back(), OVERLAY_STATUS_NOT);
}
...
TEST_F(DrmOverlayValidatorTest, RejectYUVBuffersIfNotSupported_MirroredControllers)
{
    ...
    HardwareDisplayController* controller = window_->GetController();
    controller->AddCrtc(std::make_unique<CrtcController>(drm_.get(), drm_->crtc_property(1).id, drm_->connector_property(1).id));
...
TEST_F(DrmOverlayValidatorTest, RejectYUVBuffersIfNotSupported_NoPackedFormatsInMirroredCrtc)
{
    ...
    std::vector<CrtcState> crtc_states = {
        { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888, DRM_FORMAT_NV12 } } } },
        { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } } } },
    };
    InitDrmStatesAndControllers(crtc_states);
...
TEST_F(DrmOverlayValidatorTest, RejectYUVBuffersIfNotSupported_NoPackedFormatsInPrimaryDisplay)
{
...
TEST_F(DrmOverlayValidatorTest, OptimalFormatXRGB_NoPackedFormatInMirroredCrtc)
{
    std::vector<CrtcState> crtc_states = {
        { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888, DRM_FORMAT_NV12 } } } },
        { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } } } },
...
TEST_F(DrmOverlayValidatorTest, OptimalFormatXRGB_NoPackedFormatInPrimaryDisplay)
{
    std::vector<CrtcState> crtc_states = { { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } } } },
        { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888, DRM_FORMAT_NV12 } } } } };
    InitDrmStatesAndControllers(crtc_states);
...
class TestAtOnceDrmOverlayValidatorTest : public DrmOverlayValidatorTest, public testing::WithParamInterface<bool> { };

TEST_F(DrmOverlayValidatorTest, FourCandidates_OneCommit)
{
    // Four planes.
    CrtcState crtc_state = { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } },
...
{
    // Only two planes.
    CrtcState crtc_state = { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } } } };
    InitDrmStatesAndControllers({ crtc_state });
    int setup_commits = drm_->get_commit_count();
...
TEST_F(DrmOverlayValidatorTest, TwoOfSixIgnored_OneCommit)
{
    // Six planes.
    CrtcState crtc_state = { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } },
                                 { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } } } };
    InitDrmStatesAndControllers({ crtc_state });
    int setup_commits = drm_->get_commit_count();
...
TEST_F(DrmOverlayValidatorTest, PinnedPlanesCantBeReused)
{
    ...
    auto results = overlay_validator_->TestPageFlip(overlay_params_, DrmOverlayPlaneList());
    EXPECT_EQ(results[0], OVERLAY_STATUS_ABLE) << "The primary plane should still be usable.";
    EXPECT_EQ(results[1], OVERLAY_STATUS_NOT) << "The overlay plane should not be available.";
}
...
TEST_F(DrmOverlayValidatorTest, UnpinnedMovablePlanesCanBeUsed)
{
    ...
    auto results = overlay_validator_->TestPageFlip(overlay_params_, DrmOverlayPlaneList());
    EXPECT_EQ(results[0], OVERLAY_STATUS_ABLE) << "The primary plane should still be usable.";
    EXPECT_EQ(results[1], OVERLAY_STATUS_ABLE) << "The overlay plane should be available since it is not pinned.";
}
