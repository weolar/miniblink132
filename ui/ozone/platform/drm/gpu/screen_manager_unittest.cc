...
Matcher<const CrtcController&> EqualsCrtcConnectorIds(uint32_t crtc, uint32_t connector)
{
    return AllOf(Property(&CrtcController::crtc, Eq(crtc)), Property(&CrtcController::connector, Eq(connector)));
}
...
{
    return AllOf(Field(&TileProperty::group_id, Eq(expected.group_id)), Field(&TileProperty::scale_to_fit_display, Eq(expected.scale_to_fit_display)),
        Field(&TileProperty::tile_size, Eq(expected.tile_size)), Field(&TileProperty::tile_layout, Eq(expected.tile_layout)),
        Field(&TileProperty::location, Eq(expected.location)));
}

std::unique_ptr<HardwareDisplayControllerInfo> GetDisplayInfo(
    uint32_t connector_id, uint32_t crtc_id, uint8_t index, const std::optional<TileProperty>& tile_property = std::nullopt)
{
    // Initialize a list of display modes.
    constexpr size_t kNumModes = 5;
    drmModeModeInfo modes[kNumModes] = { { .hdisplay = 640, .vdisplay = 400 }, { .hdisplay = 640, .vdisplay = 480 }, { .hdisplay = 800, .vdisplay = 600 },
    ...
    return std::make_unique<HardwareDisplayControllerInfo>(std::move(connector), std::move(crtc), index,
...
public:
    ...
    {
        return gfx::Rect(kDefaultMode.hdisplay, 0, kDefaultMode.hdisplay, kDefaultMode.vdisplay);
    }

    void InitializeDrmState(FakeDrmDevice* drm, const std::vector<CrtcState>& crtc_states, bool is_atomic, bool use_modifiers_list = false,
        const std::vector<PlaneState>& movable_planes = {})
    {
        ...
        for (const auto& crtc_state : crtc_states) {
            ASSERT_EQ(plane_count, crtc_state.planes.size()) << "FakeDrmDevice::CreateStateWithAllProperties currently expects "
        ...
        if (use_modifiers_list) {
            for (const auto modifier : supported_modifiers_) {
                drm_format_modifiers.push_back({ .formats = 1, .offset = 0, .pad = 0, .modifier = modifier });
            }
        }
        ...
        for (const auto& crtc_state : crtc_states) {
            ...
            for (size_t i = 0; i < crtc_state.planes.size(); ++i) {
                auto in_formats_blob = drm->CreateInFormatsBlob(crtc_state.planes[i].formats, drm_format_modifiers);

                auto& plane = drm->AddPlane(crtc_id, i == 0 ? DRM_PLANE_TYPE_PRIMARY : DRM_PLANE_TYPE_OVERLAY);
                drm->AddProperty(plane.id, { .id = kInFormatsPropId, .value = in_formats_blob->id() });
            }
        }
        for (const auto& movable_plane : movable_planes) {
            auto in_formats_blob = drm->CreateInFormatsBlob(movable_plane.formats, drm_format_modifiers);
    ...
    void AddPlaneToCrtc(uint32_t crtc_id, uint32_t plane_type)
    {
        ...
        drm_->AddProperty(plane.id, { .id = kInFormatsPropId, .value = in_formats_blob->id() });
    }

    void InitializeDrmStateWithDefault(FakeDrmDevice* drm, bool is_atomic, bool use_modifiers_list = false)
    {
        // A Sample of CRTC states.
        std::vector<CrtcState> crtc_states
            = { { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } } } }, { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } } } } };
    ...
    scoped_refptr<DrmFramebuffer> CreateBuffer(uint32_t format, const gfx::Size& size)
    {
        return CreateBufferWithModifier(format, DRM_FORMAT_MOD_NONE, size);
    }

    scoped_refptr<DrmFramebuffer> CreateBufferWithModifier(uint32_t format, uint64_t format_modifier, const gfx::Size& size)
    {
        std::vector<uint64_t> modifiers;
        if (format_modifier != DRM_FORMAT_MOD_NONE) {
            modifiers.push_back(format_modifier);
        }
        auto buffer = drm_->gbm_device()->CreateBufferWithModifiers(format, size, GBM_BO_USE_SCANOUT, modifiers);
...
protected:
    ...
    base::flat_map<uint64_t /*modifier*/, int /*overhead*/> modifiers_overhead_ { { DRM_FORMAT_MOD_LINEAR, 1 }, { I915_FORMAT_MOD_Yf_TILED_CCS, 100 } };
};
...
TEST_F(ScreenManagerTest, CheckWithValidController)
{
    ...
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, crtc_id, connector_id, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
    EXPECT_EQ(drm_->get_test_modeset_count(), 1);
    EXPECT_EQ(drm_->get_commit_modeset_count(), 1);

    HardwareDisplayController* controller = screen_manager_->GetDisplayController(GetPrimaryBounds());
...
TEST_F(ScreenManagerTest, CheckWithSeamlessModeset)
{
    ...
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, crtc_id, connector_id, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kCommitModeset, display::ModesetFlag::kSeamlessModeset });
...
TEST_F(ScreenManagerTest, CheckForSecondValidController)
{
    ...
    screen_manager_->AddDisplayController(drm_, primary_crtc_id, primary_connector_id);
    screen_manager_->AddDisplayController(drm_, secondary_crtc_id, secondary_connector_id);

    std::vector<ControllerConfigParams> controllers_to_enable;
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, primary_crtc_id, primary_connector_id, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    drmModeModeInfo secondary_mode = kDefaultMode;
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm_, secondary_crtc_id, secondary_connector_id, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(secondary_mode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
...
TEST_F(ScreenManagerTest, CheckMultipleDisplaysWithinModifiersLimit)
{
    ...
    drm_->SetSystemLimitOfModifiers(modifiers_overhead_[I915_FORMAT_MOD_Yf_TILED_CCS] * max_supported_displays_with_modifier);
...
{
    int max_supported_displays_with_modifier = 2;
    drm_->SetSystemLimitOfModifiers(modifiers_overhead_[DRM_FORMAT_MOD_LINEAR] * max_supported_displays_with_modifier);
...
TEST_F(ScreenManagerTest, CheckControllerAfterDisabled)
{
    ...
    // Disable
    std::vector<ControllerConfigParams> controllers_to_enable;
    controllers_to_enable.emplace_back(kPrimaryDisplayId, drm_, crtc_id, connector_id, gfx::Point(), nullptr);
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });

    EXPECT_EQ(drm_->get_test_modeset_count(), test_modeset_count_before_disable + 1);
    EXPECT_EQ(drm_->get_commit_modeset_count(), commit_modeset_count_before_disable + 1);
...
TEST_F(ScreenManagerTest, CheckMultipleControllersAfterBeingRemoved)
{
    ...
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, primary_crtc_id, primary_connector_id, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm_, secondary_crtc_id, secondary_connector_id, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
...
TEST_F(ScreenManagerTest, CheckMultipleControllersAfterBeingDisabled)
{
    ...
    // Enable
    {
        ...
        controllers_to_enable.emplace_back(
            kPrimaryDisplayId, drm_, primary_crtc_id, primary_connector_id, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
        controllers_to_enable.emplace_back(kSecondaryDisplayId, drm_, secondary_crtc_id, secondary_connector_id, GetSecondaryBounds().origin(),
    ...
    // Disable
    std::vector<ControllerConfigParams> controllers_to_enable;
    controllers_to_enable.emplace_back(kPrimaryDisplayId, drm_, primary_crtc_id, primary_connector_id, gfx::Point(), nullptr);
    controllers_to_enable.emplace_back(kSecondaryDisplayId, drm_, secondary_crtc_id, secondary_connector_id, gfx::Point(), nullptr);
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
...
TEST_F(ScreenManagerTest, CheckChangingMode)
{
    ...
    // Modeset with a changed Mode.
    {
        std::vector<ControllerConfigParams> controllers_to_enable;
        controllers_to_enable.emplace_back(
            kPrimaryDisplayId, drm_, crtc_id, connector_id, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(new_mode));
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
    }
    ...
    EXPECT_FALSE(screen_manager_->GetDisplayController(GetSecondaryBounds()));
    drmModeModeInfo mode = screen_manager_->GetDisplayController(new_bounds)->crtc_controllers()[0]->mode();
...
TEST_F(ScreenManagerTest, CheckChangingVrrState)
{
    ...
    // Modeset with default VRR state.
    {
        std::vector<ControllerConfigParams> controllers_to_enable;
        controllers_to_enable.emplace_back(
            kPrimaryDisplayId, drm_, crtc_id, connector_id, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode), /*enable_vrr=*/false);
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });

        const HardwareDisplayController* hdc = screen_manager_->GetDisplayController(GetPrimaryBounds());
    ...
    // Modeset with a changed VRR state.
    {
        std::vector<ControllerConfigParams> controllers_to_enable;
        controllers_to_enable.emplace_back(
            kPrimaryDisplayId, drm_, crtc_id, connector_id, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode), /*enable_vrr=*/true);
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
...
TEST_F(ScreenManagerTest, CheckForControllersInMirroredMode)
{
    ...
    drmModeModeInfo secondary_mode = kDefaultMode;
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm_, secondary_crtc_id, secondary_connector_id, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(secondary_mode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
...
{
    std::vector<CrtcState> crtc_states = { { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888, DRM_FORMAT_NV12 } } } },
        { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888, DRM_FORMAT_NV12 } } } } };
    ...
    controllers_to_enable.emplace_back(kPrimaryDisplayId, drm_, primary_crtc_id, primary_connector_id, GetPrimaryBounds().origin(),
        std::make_unique<drmModeModeInfo>(transition1_primary_mode));
    drmModeModeInfo transition1_secondary_mode = kDefaultMode;
    controllers_to_enable.emplace_back(kSecondaryDisplayId, drm_, secondary_crtc_id, secondary_connector_id, GetPrimaryBounds().origin(),
    ...
    controllers_to_enable.emplace_back(kPrimaryDisplayId, drm_, primary_crtc_id, primary_connector_id, GetSecondaryBounds().origin(),
...
TEST_F(ScreenManagerTest, MonitorDisabledInMirrorMode)
{
    ...
    std::vector<ControllerConfigParams> controllers_to_enable;
    controllers_to_enable.emplace_back(0, drm_, secondary_crtc_id, 0, gfx::Point(), nullptr);
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
...
TEST_F(ScreenManagerTest, DoNotEnterMirrorModeUnlessSameBounds)
{
...

    {
        ...
        std::vector<ControllerConfigParams> controllers_to_enable;
        controllers_to_enable.emplace_back(
            kSecondaryDisplayId, drm_, secondary_crtc_id, secondary_connector_id, GetPrimaryBounds().origin(), std::move(new_mode));
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
    }

    EXPECT_FALSE(screen_manager_->GetDisplayController(GetPrimaryBounds())->IsMirrored());
}
...
TEST_F(ScreenManagerTest, ReuseFramebufferIfDisabledThenReEnabled)
{
    ...
    controllers_to_enable.clear();
    // Disable display controller.
    controllers_to_enable.emplace_back(0, drm_, crtc_id, 0, gfx::Point(), nullptr);
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
    ...
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, crtc_id, connector_id, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(reenable_mode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
...
TEST_F(ScreenManagerTest, CheckMirrorModeAfterBeginReEnabled)
{
    ...
    {
        std::vector<ControllerConfigParams> controllers_to_enable;
        controllers_to_enable.emplace_back(0, drm_, primary_crtc_id, 0, gfx::Point(), nullptr);
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
    }
...

    {
        ...
        drmModeModeInfo reenable_mode = kDefaultMode;
        controllers_to_enable.emplace_back(
            kPrimaryDisplayId, drm_, primary_crtc_id, primary_connector_id, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(reenable_mode));
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
    }
...
{
    InitializeDrmStateWithDefault(drm_.get(), /*is_atomic=*/false);
    std::vector<CrtcState> crtc_states = { { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } } } }, { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } } } },
    ...
    std::vector<ControllerConfigParams> controllers_to_enable;
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, drm_1_crtc_1, drm_1_connector_1, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    drmModeModeInfo secondary_mode = kDefaultMode;
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm2_, drm_2_crtc_1, drm_2_connector_1, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(secondary_mode));
    drmModeModeInfo secondary_mode2 = kDefaultMode;
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId + 1, drm2_, drm_2_crtc_1, drm_2_connector_1, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(secondary_mode2));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
...
TEST_F(ScreenManagerTest, CheckProperConfigurationWithDifferentDeviceAndSameCrtc)
{
    InitializeDrmStateWithDefault(drm_.get(), /*is_atomic=*/true);
    uint32_t crtc_id = drm_->crtc_property(0).id;
    uint32_t connector_id = drm_->connector_property(0).id;
    ...
    drmModeModeInfo secondary_mode = kDefaultMode;
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm2_, crtc_id, connector_id, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(secondary_mode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });

    HardwareDisplayController* controller1 = screen_manager_->GetDisplayController(GetPrimaryBounds());
    HardwareDisplayController* controller2 = screen_manager_->GetDisplayController(GetSecondaryBounds());
...
TEST_F(ScreenManagerTest, CheckControllerToWindowMappingWithSameBounds)
{
    ...
    std::unique_ptr<DrmWindow> window(new DrmWindow(1, device_manager_.get(), screen_manager_.get()));
    window->Initialize();
...
TEST_F(ScreenManagerTest, CheckControllerToWindowMappingWithOverlappingWindows)
{
    InitializeDrmStateWithDefault(drm_.get(), /*is_atomic=*/true);
    uint32_t crtc_id = drm_->crtc_property(0).id;
    uint32_t connector_id = drm_->connector_property(0).id;
    ...
    for (size_t i = 1; i < kWindowCount + 1; ++i) {
        std::unique_ptr<DrmWindow> window(new DrmWindow(i, device_manager_.get(), screen_manager_.get()));
        window->Initialize();
        window->SetBounds(GetPrimaryBounds());
...
TEST_F(ScreenManagerTest, ShouldDissociateWindowOnControllerRemoval)
{
    ...
    std::unique_ptr<DrmWindow> window(new DrmWindow(window_id, device_manager_.get(), screen_manager_.get()));
    window->Initialize();
    window->SetBounds(GetPrimaryBounds());
...
TEST_F(ScreenManagerTest, EnableControllerWhenWindowHasBuffer)
{
    ...
    window->Initialize();
    window->SetBounds(GetPrimaryBounds());
    scoped_refptr<DrmFramebuffer> buffer = CreateBuffer(DRM_FORMAT_XRGB8888, GetPrimaryBounds().size());
    DrmOverlayPlaneList planes;
    planes.push_back(DrmOverlayPlane::TestPlane(buffer));
    window->SchedulePageFlip(std::move(planes), base::DoNothing(), base::DoNothing());
    screen_manager_->AddWindow(1, std::move(window));
...
TEST_F(ScreenManagerTest, DISABLED_RejectBufferWithIncompatibleModifiers)
{
    ...
    window->Initialize();
    window->SetBounds(GetPrimaryBounds());
    auto buffer = CreateBufferWithModifier(DRM_FORMAT_XRGB8888, I915_FORMAT_MOD_X_TILED, GetPrimaryBounds().size());
    DrmOverlayPlaneList planes;
    planes.push_back(DrmOverlayPlane::TestPlane(buffer));
    window->SchedulePageFlip(std::move(planes), base::DoNothing(), base::DoNothing());
    screen_manager_->AddWindow(1, std::move(window));
...
TEST_F(ScreenManagerTest, ShouldNotHardwareMirrorDifferentDrmDevices)
{
    ...
    {
        auto window1 = std::make_unique<DrmWindow>(1, device_manager_.get(), screen_manager_.get());
        window1->Initialize();
        ...
        std::vector<ControllerConfigParams> controllers_to_enable;
        std::unique_ptr<drmModeModeInfo> primary_mode = std::make_unique<drmModeModeInfo>(k1920x1080Screen);
        std::unique_ptr<drmModeModeInfo> secondary_mode = std::make_unique<drmModeModeInfo>(k1920x1080Screen);
        controllers_to_enable.emplace_back(kPrimaryDisplayId, drm_, crtc1, connector1, gfx::Point(0, 0), std::move(primary_mode));
        controllers_to_enable.emplace_back(kSecondaryDisplayId, drm2_, crtc2, connector2, gfx::Point(0, 1140), std::move(secondary_mode));
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });

        auto window2 = std::make_unique<DrmWindow>(2, device_manager_.get(), screen_manager_.get());
        window2->Initialize();
    ...
    {
        HardwareDisplayController* controller1 = screen_manager_->GetWindow(1)->GetController();
        HardwareDisplayController* controller2 = screen_manager_->GetWindow(2)->GetController();
        EXPECT_NE(controller1, controller2);
    ...
    // Disconnect first display. Second display moves to origin.
    {
        ...
        std::vector<ControllerConfigParams> controllers_to_enable;
        std::unique_ptr<drmModeModeInfo> secondary_mode = std::make_unique<drmModeModeInfo>(k1920x1080Screen);
        controllers_to_enable.emplace_back(kSecondaryDisplayId, drm2_, crtc2, connector2, gfx::Point(0, 0), std::move(secondary_mode));
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
    ...
    // Reconnect first display. Original configuration restored.
    {
        ...
        {
            HardwareDisplayController* controller = screen_manager_->GetWindow(1)->GetController();
            EXPECT_FALSE(controller->IsMirrored());
            // We don't really care which crtc it has, but it should have just
            EXPECT_EQ(1U, controller->crtc_controllers().size());
            EXPECT_TRUE(controller->HasCrtc(drm_, crtc1) || controller->HasCrtc(drm2_, crtc2));
        }
        controllers_to_enable.clear();
        std::unique_ptr<drmModeModeInfo> secondary_mode = std::make_unique<drmModeModeInfo>(k1920x1080Screen);
        controllers_to_enable.emplace_back(kSecondaryDisplayId, drm2_, crtc2, connector2, gfx::Point(0, 1140), std::move(secondary_mode));
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
        auto window3 = std::make_unique<DrmWindow>(3, device_manager_.get(), screen_manager_.get());
        window3->Initialize();
    ...
    {
        HardwareDisplayController* controller1 = screen_manager_->GetWindow(1)->GetController();
        HardwareDisplayController* controller3 = screen_manager_->GetWindow(3)->GetController();
...
TEST_F(ScreenManagerTest, ShouldNotUnbindFramebufferOnJoiningMirror)
{
    ...
    // Both displays connect at startup.
    {
        ...
        std::vector<ControllerConfigParams> controllers_to_enable;
        std::unique_ptr<drmModeModeInfo> primary_mode = std::make_unique<drmModeModeInfo>(k1080p60Screen);
        std::unique_ptr<drmModeModeInfo> secondary_mode = std::make_unique<drmModeModeInfo>(k1080p60Screen);
        controllers_to_enable.emplace_back(kPrimaryDisplayId, drm_, crtc1, connector1, gfx::Point(0, 0), std::move(primary_mode));
        controllers_to_enable.emplace_back(kSecondaryDisplayId, drm_, crtc2, connector2, gfx::Point(0, 0), std::move(secondary_mode));
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });
    }
...
TEST_F(ScreenManagerTest, DrmFramebufferSequenceIdIncrementingAtModeset)
{
    ...
    scoped_refptr<DrmFramebuffer> pre_modeset_buffer = CreateBuffer(DRM_FORMAT_XRGB8888, GetPrimaryBounds().size());
    ...
    scoped_refptr<DrmFramebuffer> first_post_modeset_buffer = CreateBuffer(DRM_FORMAT_XRGB8888, GetPrimaryBounds().size());
    ...
    // Unsuccessful modeset
    {
        ...
        std::vector<ControllerConfigParams> controllers_to_enable;
        controllers_to_enable.emplace_back(
            kPrimaryDisplayId, drm_, crtc_id, connector_id, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
        ASSERT_FALSE(
            screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset }));
    }

    scoped_refptr<DrmFramebuffer> second_post_modeset_buffer = CreateBuffer(DRM_FORMAT_XRGB8888, GetPrimaryBounds().size());
...
TEST_F(ScreenManagerTest, CloningPlanesOnModeset)
{
    ...
    EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(crtc_id).modeset_framebuffers, buffer));
...
{
    std::vector<CrtcState> crtc_states = { { .planes = { { .formats = { DRM_FORMAT_XRGB8888 } }, { .formats = { DRM_FORMAT_XRGB8888 } } } } };
    InitializeDrmState(drm_.get(), crtc_states, /*is_atomic=*/true);
    uint32_t crtc_id = drm_->crtc_property(0).id;
    uint32_t connector_id = drm_->connector_property(0).id;
    ...
    window->Initialize();
    window->SetBounds(GetPrimaryBounds());
    scoped_refptr<DrmFramebuffer> primary = CreateBuffer(DRM_FORMAT_XRGB8888, GetPrimaryBounds().size());
    scoped_refptr<DrmFramebuffer> overlay = CreateBuffer(DRM_FORMAT_XRGB8888, GetPrimaryBounds().size());
    DrmOverlayPlaneList planes;
    ...
    EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(crtc_id).modeset_framebuffers, primary));
    EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(crtc_id).modeset_framebuffers, overlay));
...
TEST_F(ScreenManagerTest, ModesetWithClonedPlanesNoOverlays)
{
    ...
    ASSERT_TRUE(
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset }));
    ASSERT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(crtc_id).modeset_framebuffers, buffer));
...
TEST_F(ScreenManagerTest, ModesetWithClonedPlanesWithOverlayFailing)
{
    ...
    EXPECT_TRUE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(crtc_id).modeset_framebuffers, primary));
    EXPECT_FALSE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(crtc_id).modeset_framebuffers, overlay));
...
TEST_F(ScreenManagerTest, ModesetWithNewBuffersOnModifiersChange)
{
    ...
    EXPECT_FALSE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(crtc_id).modeset_framebuffers, primary));
    EXPECT_FALSE(base::Contains(drm_->plane_manager()->GetCrtcStateForCrtcId(crtc_id).modeset_framebuffers, overlay));
...
TEST_F(ScreenManagerTest, PinnedPlanesAndHwMirroring)
{
    ...
    std::vector<ControllerConfigParams> controllers_to_enable;
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, crtc_id_1, connector_id_1, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    EXPECT_TRUE(
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset }));
    ...
    {
        DrmOverlayPlaneList planes;
        planes.push_back(DrmOverlayPlane::TestPlane(CreateBuffer(DRM_FORMAT_XRGB8888, GetPrimaryBounds().size())));
        planes.push_back(DrmOverlayPlane::TestPlane(CreateBuffer(DRM_FORMAT_XRGB8888, GetPrimaryBounds().size())));
        screen_manager_->GetWindow(1)->SchedulePageFlip(std::move(planes), base::DoNothing(), base::DoNothing());
        drm_->RunCallbacks();
    }

    // We should now be able to set up a HW mirrored display:
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm_, crtc_id_2, connector_id_2, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    EXPECT_TRUE(
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset }));
...
TEST_F(ScreenManagerTest, PinnedPlanesAndModesetting)
{
    ...
    {
        std::unique_ptr<DrmWindow> window(new DrmWindow(1, device_manager_.get(), screen_manager_.get()));
        std::unique_ptr<DrmWindow> window2(new DrmWindow(2, device_manager_.get(), screen_manager_.get()));
        window->Initialize();
        window->SetBounds(GetPrimaryBounds());
        screen_manager_->AddWindow(1, std::move(window));
    ...
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm_, crtc_id_2, connector_id_2, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    EXPECT_TRUE(
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset }));
...
TEST_F(ScreenManagerTest, ReplaceDisplayControllersCrtcs)
{
    ...
    ASSERT_TRUE(screen_manager_->ReplaceDisplayControllersCrtcs(drm_, current_pairings, new_pairings));

    controllers_to_enable.back().crtc = new_crtc_id;
    EXPECT_TRUE(
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset }));

    HardwareDisplayController* new_controller = screen_manager_->GetDisplayController(GetPrimaryBounds());
...
#define MAYBE_ReplaceDisplayControllersCrtcsNonexistent DISABLED_ReplaceDisplayControllersCrtcsNonexistent
#else
#define MAYBE_ReplaceDisplayControllersCrtcsNonexistent ReplaceDisplayControllersCrtcsNonexistent
#endif
TEST_F(ScreenManagerTest, MAYBE_ReplaceDisplayControllersCrtcsNonexistent)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(screen_manager_->ReplaceDisplayControllersCrtcs(drm_, current_pairings, new_pairings), "controller not found for connector");
}
...
TEST_F(ScreenManagerTest, ReplaceDisplayControllersCrtcsComplex)
{
    ...
    std::vector<ControllerConfigParams> controllers_to_enable;
    controllers_to_enable.emplace_back(
        kPrimaryDisplayId, drm_, crtc_1, connector_1, GetPrimaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    controllers_to_enable.emplace_back(
        kSecondaryDisplayId, drm_, crtc_2, connector_2, GetSecondaryBounds().origin(), std::make_unique<drmModeModeInfo>(kDefaultMode));
    EXPECT_TRUE(
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset }));

    HardwareDisplayController* controller_1 = screen_manager_->GetDisplayController(GetPrimaryBounds());
    ASSERT_NE(controller_1, nullptr);
    EXPECT_THAT(controller_1->crtc_controllers(), UnorderedElementsAre(Pointee(EqualsCrtcConnectorIds(crtc_1, connector_1))));

    HardwareDisplayController* controller_2 = screen_manager_->GetDisplayController(GetSecondaryBounds());
    ASSERT_NE(controller_2, nullptr);
    EXPECT_THAT(controller_2->crtc_controllers(), UnorderedElementsAre(Pointee(EqualsCrtcConnectorIds(crtc_2, connector_2))));

    ConnectorCrtcMap current_pairings = { { connector_1, crtc_1 }, { connector_2, crtc_2 } };
    ConnectorCrtcMap new_pairings = { { connector_1, crtc_2 }, { connector_2, crtc_3 } };
    ASSERT_TRUE(screen_manager_->ReplaceDisplayControllersCrtcs(drm_, current_pairings, new_pairings));

    // Check that the HDCs now reflect the replaced CRTCs, and that no old
    // CrtcControllers remain.
    EXPECT_THAT(controller_1->crtc_controllers(), UnorderedElementsAre(Pointee(EqualsCrtcConnectorIds(crtc_2, connector_1))));
    EXPECT_THAT(controller_2->crtc_controllers(), UnorderedElementsAre(Pointee(EqualsCrtcConnectorIds(crtc_3, connector_2))));
}
...
TEST_F(ScreenManagerTest, TileDisplay)
{
    ...
    TileProperty primary_tile_prop
        = { .group_id = 1, .scale_to_fit_display = true, .tile_size = gfx::Size(3840, 4320), .tile_layout = gfx::Size(2, 1), .location = gfx::Point(0, 0) };
    std::unique_ptr<ui::HardwareDisplayControllerInfo> primary_info = GetDisplayInfo(connector_id_1, crtc_id_1, /*index=*/1, primary_tile_prop);

    TileProperty nonprimary_tile_prop = primary_tile_prop;
    nonprimary_tile_prop.location = gfx::Point(1, 0);
    primary_info->AcquireNonprimaryTileInfo(GetDisplayInfo(connector_id_2, crtc_id_2, /*index=*/2, nonprimary_tile_prop));

    std::unique_ptr<display::FakeDisplaySnapshot> snapshot = display::FakeDisplaySnapshot::Builder()
    ...
    std::vector<ControllerConfigParams> controllers_to_enable;
    controllers_to_enable.emplace_back(kPrimaryDisplayId, drm_, crtc_id_1, connector_id_1, gfx::Point(0, 0),
        std::make_unique<drmModeModeInfo>(drmModeModeInfo { .hdisplay = 3840, .vdisplay = 4320 }));
    ASSERT_TRUE(
        screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset }));
    ...
    EXPECT_THAT(hdc->GetTileProperty(), Optional(EqTileProperty(primary_tile_prop)));

    EXPECT_THAT(hdc->crtc_controllers(),
        UnorderedElementsAre(
            Pointee(AllOf(Property(&CrtcController::crtc, Eq(crtc_id_1)), Property(&CrtcController::connector, Eq(connector_id_1)),
                Property(&CrtcController::is_tiled, Eq(true)), Property(&CrtcController::tile_property, Optional(EqTileProperty(primary_tile_prop))))),
            Pointee(AllOf(Property(&CrtcController::crtc, Eq(crtc_id_2)), Property(&CrtcController::connector, Eq(connector_id_2)),
                Property(&CrtcController::is_tiled, Eq(true)), Property(&CrtcController::tile_property, Optional(EqTileProperty(nonprimary_tile_prop)))))));
}
} // namespace ui
