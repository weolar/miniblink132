...
GbmOverlaySurface::Frame& GbmOverlaySurface::Frame::operator=(GbmOverlaySurface::Frame&& frame) = default;

GbmOverlaySurface::Frame::~Frame() = default;

void GbmOverlaySurface::SubmitFrame(std::vector<OverlayPlane> overlay_planes, SubmissionCallback submission_callback,
    PresentationCallback presentation_callback, ReleaseCallback release_callback)
{
    ...
    for (auto& plane : overlay_planes) {
        unsubmitted_frame.overlay_planes.emplace_back(
            static_cast<GbmPixmap*>(plane.pixmap.get())->framebuffer(), plane.overlay_plane_data, std::move(plane.gpu_fence));
    }
...
void GbmOverlaySurface::SubmitFrame()
{
    ...
    window_->SchedulePageFlip(std::move(unsubmitted_frame_.overlay_planes), base::BindOnce(&GbmOverlaySurface::OnSubmission, weak_ptr_factory_.GetWeakPtr()),
        base::BindOnce(&GbmOverlaySurface::OnPresentation, weak_ptr_factory_.GetWeakPtr()));
    page_flip_pending_ = true;
}

void GbmOverlaySurface::OnSubmission(gfx::SwapResult swap_result, gfx::GpuFenceHandle release_fence)
{
...
void GbmOverlaySurface::OnPresentation(const gfx::PresentationFeedback& presentation_feedback)
{
    DCHECK(page_flip_pending_);
