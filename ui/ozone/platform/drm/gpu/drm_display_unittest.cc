...
std::unique_ptr<HardwareDisplayControllerInfo> GetDisplayInfo(
    uint32_t connector_id = 123, uint32_t crtc_id = 456, uint8_t index = 0, const std::optional<TileProperty>& tile_property = std::nullopt)
{
    ...
    drmModeModeInfo modes[kNumModes] = { { .hdisplay = 640, .vdisplay = 400 }, { .hdisplay = 640, .vdisplay = 480 }, { .hdisplay = 800, .vdisplay = 600 },
        ...
        { .hdisplay = kNativeDisplaySize.width(), .vdisplay = kNativeDisplaySize.height() } };
...
TEST(DrmDisplayTest, TiledDisplay)
{
    ...
    uint32_t primary_crtc_id, primary_connector_id;
    {
        ...
        primary_connector.properties.push_back({ .id = kTileBlobPropId, .value = kTileBlobId });
    ...
    std::unique_ptr<ui::HardwareDisplayControllerInfo> primary_info = GetDisplayInfo(primary_connector_id, primary_crtc_id, /*index=*/1, primary_tile_prop);

    TileProperty nonprimary_tile_prop = primary_tile_prop;
    nonprimary_tile_prop.location = gfx::Point(1, 0);
    primary_info->AcquireNonprimaryTileInfo(GetDisplayInfo(nonprimary_connector_id, nonprimary_crtc_id, /*index=*/2, nonprimary_tile_prop));
    ...
    EXPECT_THAT(drm_display.crtc_connector_pairs(),
        UnorderedElementsAre(AllOf(Field(&DrmDisplay::CrtcConnectorPair::connector, Pointee(Field(&drmModeConnector::connector_id, Eq(primary_connector_id)))),
                                 Field(&DrmDisplay::CrtcConnectorPair::crtc_id, Eq(primary_crtc_id)),
                                 Field(&DrmDisplay::CrtcConnectorPair::tile_location, Eq(gfx::Point(0, 0)))),
            AllOf(Field(&DrmDisplay::CrtcConnectorPair::connector, Pointee(Field(&drmModeConnector::connector_id, Eq(nonprimary_connector_id)))),
                Field(&DrmDisplay::CrtcConnectorPair::crtc_id, Eq(nonprimary_crtc_id)),
                Field(&DrmDisplay::CrtcConnectorPair::tile_location, Eq(gfx::Point(1, 0))))));
    EXPECT_EQ(drm_display.GetPrimaryConnectorId(), primary_connector_id);
    EXPECT_EQ(drm_display.GetPrimaryCrtcId(), primary_crtc_id);

    std::optional<TileProperty> actual_tile_property = drm_display.GetTileProperty();
