...
constexpr char kMultipleDisplayIdsCollisionDetected[] = "Display.MultipleDisplays.GenerateId.CollisionDetection";
...
using CrtcConnectorPairTileLocations = std::vector<std::pair<CrtcConnectorPair, std::optional<gfx::Point> /*tile_location*/>>;

CrtcConnectorPairTileLocations CreateCrtcConnectorPairTileLocationsForDisplay(const DrmDisplay& display)
{
    CrtcConnectorPairTileLocations crtc_connector_pairs;
    for (const DrmDisplay::CrtcConnectorPair& crtc_connector_pair : display.crtc_connector_pairs()) {
        crtc_connector_pairs.push_back(
            { { .crtc_id = crtc_connector_pair.crtc_id, .connector_id = crtc_connector_pair.connector->connector_id }, crtc_connector_pair.tile_location });
    }
...
public:
    explicit DisplayComparator(const DrmDisplay* display)
        : drm_(display->drm())
    {
        crtc_connector_pair_tile_locations_ = CreateCrtcConnectorPairTileLocationsForDisplay(*display);
    }

    DisplayComparator(const scoped_refptr<DrmDevice>& drm, const HardwareDisplayControllerInfo& display_info)
        : drm_(drm)
    {
        CrtcConnectorPair primary_crtc_connector { .crtc_id = display_info.crtc()->crtc_id, .connector_id = display_info.connector()->connector_id };
        std::optional<gfx::Point> primary_tile_location;
        const std::optional<TileProperty>& primary_tile_property = display_info.tile_property();
        ...
        crtc_connector_pair_tile_locations_.push_back({ primary_crtc_connector, primary_tile_location });

        for (const std::unique_ptr<HardwareDisplayControllerInfo>& nonprimary_tile : display_info.nonprimary_tile_infos()) {
            CrtcConnectorPair crtc_connector_pair { .crtc_id = nonprimary_tile->crtc()->crtc_id, .connector_id = nonprimary_tile->connector()->connector_id };
            crtc_connector_pair_tile_locations_.push_back({ crtc_connector_pair, nonprimary_tile->tile_property()->location });
        }
    }
    ...
    bool operator()(const std::unique_ptr<DrmDisplay>& other) const
    {
        ...
        const std::optional<TileProperty>& other_tile_property = other->GetTileProperty();
        std::optional<int> other_tile_group_id = other_tile_property.has_value() ? other_tile_property->group_id : std::optional<int>(std::nullopt);
        ...
        CrtcConnectorPairTileLocations other_crtc_connector_tile_locations = CreateCrtcConnectorPairTileLocationsForDisplay(*other);
        return std::is_permutation(crtc_connector_pair_tile_locations_.begin(), crtc_connector_pair_tile_locations_.end(),
            other_crtc_connector_tile_locations.begin(),
            [](const std::pair<CrtcConnectorPair, std::optional<gfx::Point>>& lhs, const std::pair<CrtcConnectorPair, std::optional<gfx::Point>>& rhs) {
                return (lhs.first.crtc_id == rhs.first.crtc_id) && (lhs.first.connector_id == rhs.first.connector_id) && (lhs.second == rhs.second);
            });
    }
...
bool MatchMode(const display::DisplayMode& display_mode, const drmModeModeInfo& m)
{
    return display_mode.size() == ModeSize(m) && display_mode.refresh_rate() == ModeRefreshRate(m) && display_mode.is_interlaced() == ModeIsInterlaced(m);
}
...
std::vector<const drmModeModeInfo*> FindMatchingModes(const display::DisplayMode& request_mode, const std::vector<drmModeModeInfo>& modes)
{
...
const drmModeModeInfo* FindClosestModeWithGreaterRefreshRate(
    const display::DisplayMode& request_mode, const DrmDisplay& display, bool is_seamless, HardwareDisplayController* controller)
{
    if (!display.IsVrrCapable()) {
        return nullptr;
    }
    if (is_seamless && !controller) {
        LOG(ERROR) << "Could not find HardwareDisplayController for display_id: " << display.display_id() << " required to perform seamless verification.";
        return nullptr;
    }

    const drmModeModeInfo* closest_mode = nullptr;
    for (const auto& m : display.modes()) {
        if (request_mode.size() != ModeSize(m) || request_mode.is_interlaced() != ModeIsInterlaced(m)) {
            continue;
        }
        if (request_mode.refresh_rate() < ModeVSyncRateMin(m, display.vsync_rate_min_from_edid())) {
            continue;
        }
        if (is_seamless && !controller->TestSeamlessMode(display.GetPrimaryCrtcId(), m)) {
            continue;
        }
        if (ModeRefreshRate(m) >= request_mode.refresh_rate() && (!closest_mode || ModeRefreshRate(m) < ModeRefreshRate(*closest_mode))) {
            closest_mode = &m;
        }
    }
    return closest_mode;
...
std::string GetEventPropertyByKey(const std::string& key, const EventPropertyMap event_props)
{
...
ControllerConfigParams* FindConfigParamsForConnector(std::vector<ControllerConfigParams>& config_list, uint32_t connector_id)
{
...
std::string ConfigRequestToString(const std::vector<display::DisplayConfigurationParams>& config_requests)
{
    std::string signature;
    for (const auto& config : config_requests) {
        if (config.id <= 0) {
            LOG(WARNING) << __func__ << ": potentially invalid display ID: " << config.id;
        }

        signature += base::NumberToString(config.id) + ":" + config.origin.ToString() + ":" + (config.mode ? config.mode->ToString() : "Disabled") + ":"
            + (config.enable_vrr ? "vrr" : "no_vrr") + ";";
    }
    if (signature.empty()) {
        LOG(WARNING) << __func__ << ": empty return value with request of size: " << config_requests.size();
    }
    return signature;
}

TestOnlyModesetOutcome GetTestOnlyModesetOutcome(bool config_success, bool did_test_modeset_with_fallback)
{
    if (!config_success) {
        return TestOnlyModesetOutcome::kFailure;
    }
    return did_test_modeset_with_fallback ? TestOnlyModesetOutcome::kFallbackSuccess : TestOnlyModesetOutcome::kSuccess;
}

std::string NumDisplaysToHistogramString(int num_displays)
{
    DCHECK(num_displays >= 0) << __func__ << ": " << num_displays << " displays detected.";
    switch (num_displays) {
    case 1:
        return "OneDisplay";
    case 2:
...
{
    return base::StrCat({ "ConfigureDisplays.Modeset.Test.DynamicCRTCs.", NumDisplaysToHistogramString(num_displays), ".PermutationsAttempted" });
}
std::string GetTestOnlyModesetOutcomeName(int num_displays)
{
    return base::StrCat({ "ConfigureDisplays.Modeset.Test.", NumDisplaysToHistogramString(num_displays), ".Outcome" });
}
} // namespace

DrmGpuDisplayManager::DrmGpuDisplayManager(ScreenManager* screen_manager, DrmDeviceManager* drm_device_manager)
...
void DrmGpuDisplayManager::SetDisplaysConfiguredCallback(base::RepeatingClosure callback)
{
...
MovableDisplaySnapshots DrmGpuDisplayManager::GetDisplays()
{
    ...
    for (const auto& drm : devices) {
        if (device_index >= kMaxDrmCount) {
            LOG(WARNING) << "Reached the current limit of " << kMaxDrmCount << " connected DRM devices. Ignoring the remaining "
        ...
        base::flat_set<uint32_t> valid_connector_ids = drm->plane_manager()->ResetConnectorsCacheAndGetValidIds(drm->GetResources());
        ...
        std::erase_if(display_infos,
            [&valid_connector_ids](const auto& display_info) { return !base::Contains(valid_connector_ids, display_info->connector()->connector_id); });
        ...
        for (auto& display_info : display_infos) {
            display_snapshots.emplace_back(CreateDisplaySnapshot(*drm, display_info.get(), static_cast<uint8_t>(device_index)));

            display::DisplaySnapshot* current_display_snapshot = display_snapshots.back().get();
            const auto colliding_display_snapshot_iter = edid_id_collision_map.find(current_display_snapshot->edid_display_id());
            if (colliding_display_snapshot_iter != edid_id_collision_map.end()) {
                collision_detected = true;

                current_display_snapshot->AddIndexToDisplayId();

                display::DisplaySnapshot* colliding_display_snapshot = colliding_display_snapshot_iter->second;
                colliding_display_snapshot->AddIndexToDisplayId();
                edid_id_collision_map[colliding_display_snapshot->edid_display_id()] = colliding_display_snapshot;
            }
            edid_id_collision_map[current_display_snapshot->edid_display_id()] = current_display_snapshot;

            // Ownership of |display_info| is handed over.
            displays_to_create.push_back({ drm, std::move(display_info), current_display_snapshot });
        }
        device_index++;
    }
    ...
    for (const DrmDisplayParams& params : displays_to_create) {
        ...
        auto old_drm_display_it = base::ranges::find_if(old_displays, DisplayComparator(params.drm, *params.display_info));
        ...
        displays_.emplace_back(std::make_unique<DrmDisplay>(params.drm, params.display_info.get(), *params.snapshot));
    }

    const bool multiple_connected_displays = display_snapshots.size() > 1;
    if (multiple_connected_displays) {
        base::UmaHistogramBoolean(kMultipleDisplayIdsCollisionDetected, collision_detected);
    }
...
bool DrmGpuDisplayManager::ShouldDisplayEventTriggerConfiguration(const EventPropertyMap& event_props)
{
    DCHECK(!event_props.empty());

    const std::string event_seq_num = GetEventPropertyByKey("SEQNUM", event_props);
    std::string log_prefix = "Display event CHANGE" + (event_seq_num.empty() ? "" : "(SEQNUM:" + event_seq_num + ") ");
    std::string trigger_prop_log;

    const std::string event_dev_path = GetEventPropertyByKey("DEVPATH", event_props);
    const DrmDeviceVector& devices = drm_device_manager_->GetDrmDevices();
    for (const auto& drm : devices) {
        ...
        const std::string connector_id_str = GetEventPropertyByKey("CONNECTOR", event_props);
        if (connector_id_str.empty()) {
            break;
        }
        uint32_t connector_id;
        {
            const bool conversion_success = base::StringToUint(connector_id_str, &connector_id);
            DCHECK(conversion_success);
        }

        // Get the trigger property's ID and convert to an int.
        const std::string trigger_prop_id_str = GetEventPropertyByKey("PROPERTY", event_props);
        if (trigger_prop_id_str.empty())
            break;

        uint32_t trigger_prop_id;
        {
            const bool conversion_success = base::StringToUint(trigger_prop_id_str, &trigger_prop_id);
            DCHECK(conversion_success);
        }

        ScopedDrmObjectPropertyPtr property_values(drm->GetObjectProperties(connector_id, DRM_MODE_OBJECT_CONNECTOR));
        ...
        DCHECK(drm_property);
        const std::string enum_value = GetEnumNameForProperty(*drm_property, *property_values);
        DCHECK(!enum_value.empty());

        trigger_prop_log = "[CONNECTOR:" + connector_id_str + "] trigger property: " + std::string(drm_property->name) + "=" + enum_value + ", ";
    ...
    VLOG(1) << log_prefix << trigger_prop_log << "resolution: allowed; display configuration task triggered.";
    return true;
}

bool DrmGpuDisplayManager::ConfigureDisplays(const std::vector<display::DisplayConfigurationParams>& config_requests, display::ModesetFlags modeset_flags,
    std::vector<display::DisplayConfigurationParams>& out_requests)
{
    const bool is_commit = modeset_flags.Has(display::ModesetFlag::kCommitModeset);
    const bool is_seamless = modeset_flags.Has(display::ModesetFlag::kSeamlessModeset);
    ...
    if (controllers_to_configure.empty()) {
        for (const auto& request : config_requests) {
            ...
            if (request.mode) {
                ...
                std::unique_ptr<display::DisplayMode> out_mode = CreateDisplayMode(*found_mode, display->vsync_rate_min_from_edid());
                ...
                if (tile_property.has_value() && IsTileMode(out_mode->size(), *tile_property)) {
                    out_mode = out_mode->CopyWithSize(GetTotalTileDisplaySize(*tile_property));
                }
                out_requests.emplace_back(request.id, request.origin, out_mode.get(), request.enable_vrr);
            } else {
                out_requests.emplace_back(request);
            }

            scoped_refptr<DrmDevice> drm = display->drm();
            ControllerConfigParams params(display->display_id(), drm, display->GetPrimaryCrtcId(), display->GetPrimaryConnectorId(), request.origin,
                std::move(found_mode), request.enable_vrr, display->base_connector_id());
            controllers_to_configure.push_back(std::move(params));
        }
    }

    bool config_success = screen_manager_->ConfigureDisplayControllers(controllers_to_configure, modeset_flags);
    ...
    const bool should_try_test_fallback = !is_commit && !config_success && !display::features::IsHardwareMirrorModeEnabled();
    bool did_test_modeset_with_fallback = false;
    if (should_try_test_fallback) {
        did_test_modeset_with_fallback = true;
        config_success = RetryTestConfigureDisplaysWithAlternateCrtcs(config_requests, controllers_to_configure);
    }
    ...
    if (is_commit) {
        ...
        const std::string test_modest_outcome_histogram = GetTestOnlyModesetOutcomeName(config_requests.size());
        const TestOnlyModesetOutcome test_modeset_outcome = GetTestOnlyModesetOutcome(config_success, did_test_modeset_with_fallback);
        base::UmaHistogramEnumeration(test_modest_outcome_histogram, test_modeset_outcome);
    }

    return config_success;
}

bool DrmGpuDisplayManager::SetHdcpKeyProp(int64_t display_id, const std::string& key)
{
    DrmDisplay* display = FindDisplay(display_id);
    if (!display) {
...
bool DrmGpuDisplayManager::GetHDCPState(int64_t display_id, display::HDCPState* state, display::ContentProtectionMethod* protection_method)
{
    DrmDisplay* display = FindDisplay(display_id);
    if (!display) {
        LOG(WARNING) << __func__ << ": there is no display with ID " << display_id;
...
bool DrmGpuDisplayManager::SetHDCPState(int64_t display_id, display::HDCPState state, display::ContentProtectionMethod protection_method)
{
    DrmDisplay* display = FindDisplay(display_id);
    if (!display) {
        LOG(WARNING) << __func__ << ": there is no display with ID " << display_id;
...
void DrmGpuDisplayManager::SetColorTemperatureAdjustment(int64_t display_id, const display::ColorTemperatureAdjustment& cta)
{
    DrmDisplay* display = FindDisplay(display_id);
    if (!display) {
        LOG(WARNING) << __func__ << ": there is no display with ID " << display_id;
...
void DrmGpuDisplayManager::SetColorCalibration(int64_t display_id, const display::ColorCalibration& calibration)
{
    DrmDisplay* display = FindDisplay(display_id);
    if (!display) {
        LOG(WARNING) << __func__ << ": there is no display with ID " << display_id;
...
void DrmGpuDisplayManager::SetGammaAdjustment(int64_t display_id, const display::GammaAdjustment& adjustment)
{
    DrmDisplay* display = FindDisplay(display_id);
    if (!display) {
        LOG(WARNING) << __func__ << ": there is no display with ID " << display_id;
...
void DrmGpuDisplayManager::SetBackgroundColor(int64_t display_id, const uint64_t background_color)
{
    DrmDisplay* display = FindDisplay(display_id);
    if (!display) {
        LOG(WARNING) << __func__ << ": there is no display with ID" << display_id;
...
std::optional<std::vector<float>> DrmGpuDisplayManager::GetSeamlessRefreshRates(int64_t display_id) const
{
    TRACE_EVENT1("drm", "DrmGpuDisplayManager::GetSeamlessRefreshRates", "display_id", display_id);
    ...
    HardwareDisplayController* controller = screen_manager_->GetDisplayController(display->drm(), display->GetPrimaryCrtcId());
    if (!controller) {
        LOG(ERROR) << "Could not find HardwareDisplayController for display_id: " << display_id;
        return std::nullopt;
    }
...
DrmDisplay* DrmGpuDisplayManager::FindDisplayByConnectorId(uint32_t connector_id) const
{
    for (const auto& display : displays_) {
...
void DrmGpuDisplayManager::NotifyScreenManager(
    const std::vector<std::unique_ptr<DrmDisplay>>& new_displays, const std::vector<std::unique_ptr<DrmDisplay>>& old_displays) const
{
    ScreenManager::CrtcsWithDrmList controllers_to_remove;
    for (const auto& old_display : old_displays) {
        if (base::ranges::none_of(new_displays, DisplayComparator(old_display.get()))) {
            for (const auto& crtc_connector_pair : old_display->crtc_connector_pairs()) {
                controllers_to_remove.emplace_back(crtc_connector_pair.crtc_id, old_display->drm());
            }
        }
    }
    ...
    for (const auto& new_display : new_displays) {
        if (base::ranges::none_of(old_displays, DisplayComparator(new_display.get()))) {
...
bool DrmGpuDisplayManager::RetryTestConfigureDisplaysWithAlternateCrtcs(
    const std::vector<display::DisplayConfigurationParams>& config_requests, const std::vector<ControllerConfigParams>& controllers_to_configure)
{
    // Separate individual params of |controllers_to_configure| into multiple
    // std::vector<ControllerConfigParams> by their DrmDevice.
    base::flat_map<scoped_refptr<DrmDevice>, std::vector<ControllerConfigParams>> drm_device_controllers_to_configure;
    ...
    for (auto& [drm, configs_list] : drm_device_controllers_to_configure) {
        std::vector<CrtcConnectorPairs> crtc_connector_permutations = GetAllCrtcConnectorPermutations(*drm, configs_list);

        VLOG(1) << "Number of possible fallback CRTC-connector permutations: " << crtc_connector_permutations.size();
        ...
        for (const auto& permutation : crtc_connector_permutations) {
            ...
            for (const auto& crtc_connector_pair : permutation) {
                ...
                ControllerConfigParams* param = FindConfigParamsForConnector(configs_list, connector_id);
                if (!param) {
                    LOG(ERROR) << __func__
            ...
            if (screen_manager_->ConfigureDisplayControllers(configs_list, { display::ModesetFlag::kTestModeset })) {
        ...
        if (!fallback_successful_for_all_devices) {
            ...
            if (!UpdateDisplaysWithNewCrtcs(controllers_to_configure)) {
                LOG(ERROR) << __func__ << ": Failed to revert to the original CRTC-connector pairings.";
            }

            const std::string num_fallback_histogram = GetNumFallbackHistogramName(config_requests.size());
            base::UmaHistogramCounts1000(num_fallback_histogram, num_permutations_attempted);
            return false;
        }
    }

    if (fallback_successful_for_all_devices) {
        const std::string config_request_string = ConfigRequestToString(config_requests);
        successful_test_config_params_.insert({ config_request_string, successful_config_list });
    }
...
bool DrmGpuDisplayManager::UpdateDisplaysWithNewCrtcs(const std::vector<ControllerConfigParams>& controllers_to_configure)
{
    base::flat_map<scoped_refptr<DrmDevice>, std::vector<ControllerConfigParams>> drm_device_to_configs;
    for (const auto& config : controllers_to_configure) {
        scoped_refptr<DrmDevice> drm = config.drm;
    ...
    // ScreenManager.
    base::flat_map<DrmDisplay*, base::flat_map<uint32_t /*current_crtc_id*/, uint32_t /*new_crtc_id*/>> display_to_new_crtcs_pairs;
    for (const auto& [drm, config_list] : drm_device_to_configs) {
        ...
        for (const auto& config_param : config_list) {
            ...
            if (!display) {
                LOG(DFATAL) << "DrmDisplay with connector ID " << connector_id << " not found.";
                return false;
            }

            const DrmDisplay::CrtcConnectorPair* crtc_connector_pair = display->GetCrtcConnectorPairForConnectorId(connector_id);
            if (!crtc_connector_pair) {
                LOG(DFATAL) << "CrtcConnectorPair with connector ID " << connector_id << " not found.";
                return false;
            }
        ...
        if (!screen_manager_->ReplaceDisplayControllersCrtcs(drm, current_connector_to_crtc_pairings, new_connector_to_crtc_pairings)) {
            return false;
        }
    }
...
std::vector<ControllerConfigParams> DrmGpuDisplayManager::GetLatestModesetTestConfig(const std::vector<display::DisplayConfigurationParams>& config_requests)
{
    const std::string config_request_string = ConfigRequestToString(config_requests);
    const auto& config_param_it = successful_test_config_params_.find(config_request_string);
...
std::unique_ptr<drmModeModeInfo> DrmGpuDisplayManager::FindModeForDisplay(const display::DisplayMode& request_mode, const DrmDisplay& display, bool is_seamless)
{
    std::vector<const drmModeModeInfo*> matching_modes = FindMatchingModes(request_mode, display.modes());
    ...
    if (matching_modes.empty() && tile_property.has_value()) {
        if (request_mode.size() == GetTotalTileDisplaySize(*tile_property)) {
            matching_modes = FindMatchingModes(*request_mode.CopyWithSize(tile_property->tile_size), display.modes());
        }
    }
    ...
    if (is_seamless) {
        if (!controller) {
            LOG(ERROR) << "Could not find HardwareDisplayController for display_id: " << display.display_id() << ". Continuing without seamless verification.";
        } else {
            std::erase_if(matching_modes,
                [&display, &controller](const drmModeModeInfo* mode) { return !controller->TestSeamlessMode(display.GetPrimaryCrtcId(), *mode); });
        }
    }
    ...
    if (matching_modes.empty() && display.IsVrrCapable()) {
        const drmModeModeInfo* closest_mode = FindClosestModeWithGreaterRefreshRate(request_mode, display, is_seamless, controller);

        // Use the closest mode to create and return a virtual mode.
        if (closest_mode) {
            std::unique_ptr<drmModeModeInfo> out_virtual_mode = CreateVirtualMode(*closest_mode, request_mode.refresh_rate());
            if (out_virtual_mode) {
                VLOG(3) << "Using virtual mode to achieve refresh_rate=" << request_mode.refresh_rate();
    ...
    if (matching_modes.empty()) {
        LOG(ERROR) << "Failed to find mode: size=" << request_mode.size().ToString() << " is_interlaced=" << request_mode.is_interlaced()
