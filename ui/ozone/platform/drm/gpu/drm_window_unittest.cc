...
std::vector<sk_sp<SkSurface>> GetCursorBuffers(const scoped_refptr<FakeDrmDevice> drm)
{
    std::vector<sk_sp<SkSurface>> cursor_buffers;
    for (const auto& pair : drm->buffers()) {
        const auto& cursor_buffer = pair.second;
        if (cursor_buffer && cursor_buffer->width() == kDefaultCursorSize && cursor_buffer->height() == kDefaultCursorSize) {
...
{
    SkBitmap image;
    SkImageInfo info = SkImageInfo::Make(size.width(), size.height(), kN32_SkColorType, kPremul_SkAlphaType);
...
public:
    ...
    scoped_refptr<DrmFramebuffer> CreateBuffer()
    {
        ...
        std::unique_ptr<GbmBuffer> buffer = drm_->gbm_device()->CreateBuffer(DRM_FORMAT_XRGB8888, window_size, GBM_BO_USE_SCANOUT);
...
void DrmWindowTest::SetUp()
{
    ...
    controllers_to_enable.emplace_back(1 /*display_id*/, drm_, crtc_id_, connector_id_, gfx::Point(), std::make_unique<drmModeModeInfo>(kDefaultMode));
...
TEST_F(DrmWindowTest, SetCursorImage)
{
    ...
    screen_manager_->GetWindow(kDefaultWidgetHandle)->SetCursor(std::vector<SkBitmap>(1, AllocateBitmap(cursor_size)), gfx::Point(4, 2), base::TimeDelta());
    ...
    for (int i = 0; i < cursor.height(); ++i) {
        for (int j = 0; j < cursor.width(); ++j) {
            if (j < cursor_size.width() && i < cursor_size.height())
                ...
                EXPECT_EQ(static_cast<SkColor>(SK_ColorTRANSPARENT), cursor.getColor(j, i));
        }
    }
}
...
TEST_F(DrmWindowTest, CheckCursorSurfaceAfterChangingDevice)
{
    ...
    controllers_to_enable.emplace_back(
        /*display_id=*/2, drm2_, crtc_id_, connector_id_, gfx::Point(0, kDefaultMode.vdisplay), std::make_unique<drmModeModeInfo>(kDefaultMode));
    screen_manager_->ConfigureDisplayControllers(controllers_to_enable, { display::ModesetFlag::kTestModeset, display::ModesetFlag::kCommitModeset });

    // Move window to the display on the new device.
    screen_manager_->GetWindow(kDefaultWidgetHandle)->SetBounds(gfx::Rect(0, kDefaultMode.vdisplay, kDefaultMode.hdisplay, kDefaultMode.vdisplay));
...
TEST_F(DrmWindowTest, CheckPageflipSuccessOnSuccessfulSwap)
{
    ...
    window->SchedulePageFlip(DrmOverlayPlane::Clone(planes), base::BindOnce(&DrmWindowTest::OnSubmission, base::Unretained(this)),
        base::BindOnce(&DrmWindowTest::OnPresentation, base::Unretained(this)));
    drm_->RunCallbacks();
    EXPECT_EQ(0, on_successful_swap_buffers_count_);
    EXPECT_EQ(gfx::SwapResult::SWAP_NAK_RECREATE_BUFFERS, last_swap_buffers_result_);
    EXPECT_EQ(static_cast<uint32_t>(gfx::PresentationFeedback::Flags::kFailure), last_presentation_feedback_.flags);
...
TEST_F(DrmWindowTest, CheckPageflipFailureOnFailedSwap)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(task_environment_.FastForwardBy(kWaitForModesetTimeout), gpu_crash_log);
}
