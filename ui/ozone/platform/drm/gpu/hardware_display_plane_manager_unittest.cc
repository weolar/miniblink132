...
class HardwareDisplayPlaneManagerTest : public testing::Test, public testing::WithParamInterface<bool> {
public:
    HardwareDisplayPlaneManagerTest() = default;

    HardwareDisplayPlaneManagerTest(const HardwareDisplayPlaneManagerTest&) = delete;
    HardwareDisplayPlaneManagerTest& operator=(const HardwareDisplayPlaneManagerTest&) = delete;

    uint64_t GetObjectPropertyValue(uint32_t object_id, uint32_t object_type, const std::string& property_name);
    uint64_t GetCrtcPropertyValue(uint32_t crtc, const std::string& property_name);
    uint64_t GetPlanePropertyValue(uint32_t plane, const std::string& property_name);

    void PerformPageFlip(size_t crtc_idx, HardwareDisplayPlaneList* state);
    void PerformPageFlip(size_t crtc_idx, HardwareDisplayPlaneList* state, DrmOverlayPlaneList& assigns);
    void PerformFailingPageFlip(size_t crtc_idx, HardwareDisplayPlaneList* state, DrmOverlayPlaneList& assigns);
    ...
    scoped_refptr<DrmFramebuffer> CreateBufferWithFormat(const gfx::Size& size, uint32_t format)
    {
        std::unique_ptr<GbmBuffer> buffer = fake_drm_->gbm_device()->CreateBuffer(format, size, GBM_BO_USE_SCANOUT);
...
void HardwareDisplayPlaneManagerTest::PerformPageFlip(size_t crtc_idx, HardwareDisplayPlaneList* state)
{
...
void HardwareDisplayPlaneManagerTest::PerformPageFlip(size_t crtc_idx, HardwareDisplayPlaneList* state, DrmOverlayPlaneList& assigns)
{
    fake_drm_->plane_manager()->BeginFrame(state);
    ASSERT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(state, assigns, fake_drm_->crtc_property(crtc_idx).id));
    scoped_refptr<PageFlipRequest> page_flip_request = base::MakeRefCounted<PageFlipRequest>(base::TimeDelta());

    fake_drm_->set_commit_expectation(true);

    ASSERT_TRUE(fake_drm_->plane_manager()->Commit(state, page_flip_request, nullptr));
}

void HardwareDisplayPlaneManagerTest::PerformFailingPageFlip(size_t crtc_idx, HardwareDisplayPlaneList* state, DrmOverlayPlaneList& assigns)
{
    fake_drm_->plane_manager()->BeginFrame(state);
    ASSERT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(state, assigns, fake_drm_->crtc_property(crtc_idx).id));
    scoped_refptr<PageFlipRequest> page_flip_request = base::MakeRefCounted<PageFlipRequest>(base::TimeDelta());
    ...
    ASSERT_FALSE(fake_drm_->plane_manager()->Commit(state, page_flip_request, nullptr));
}

uint64_t HardwareDisplayPlaneManagerTest::GetObjectPropertyValue(uint32_t object_id, uint32_t object_type, const std::string& property_name)
{
    DrmDevice::Property p {};
    ScopedDrmObjectPropertyPtr properties(fake_drm_->GetObjectProperties(object_id, object_type));
    EXPECT_TRUE(GetDrmPropertyForName(fake_drm_.get(), properties.get(), property_name, &p));
    return p.value;
}

uint64_t HardwareDisplayPlaneManagerTest::GetCrtcPropertyValue(uint32_t crtc, const std::string& property_name)
{
    return GetObjectPropertyValue(crtc, DRM_MODE_OBJECT_CRTC, property_name);
}

uint64_t HardwareDisplayPlaneManagerTest::GetPlanePropertyValue(uint32_t plane, const std::string& property_name)
{
...
TEST_P(HardwareDisplayPlaneManagerTest, ResettingConnectorCache)
{
    ...
    auto& drm_state = fake_drm_->ResetStateWithDefaultObjects(connector_and_crtc_count,
        /*planes_per_crtc=*/1);
    ...
    for (size_t i = 0; i < connector_and_crtc_count; ++i) {
        ...
        connector_properties.properties.push_back({ .id = kCrtcIdPropId, .value = 0 });
    }
...

    {
        ...
        for (size_t i = 0; i < connector_and_crtc_count; ++i) {
            ...
            CrtcCommitRequest request = CrtcCommitRequest::EnableCrtcRequest(fake_drm_->crtc_property(i).id, fake_drm_->connector_property(i).id, kDefaultMode,
                gfx::Point(), &state, std::move(overlays),
                /*enable_vrr=*/false);
            commit_request.push_back(std::move(request));
        }

        EXPECT_TRUE(fake_drm_->plane_manager()->Commit(std::move(commit_request), DRM_MODE_ATOMIC_ALLOW_MODESET));
    }

    // Replace last connector and update state.
    drm_state.connector_properties[connector_and_crtc_count - 1].id = kConnectorIdBase + 3;
    fake_drm_->plane_manager()->ResetConnectorsCacheAndGetValidIds(fake_drm_->GetResources());

    {
        ...
        fake_drm_->plane_manager()->BeginFrame(&state);
        {
            ...
            commit_request.push_back(CrtcCommitRequest::EnableCrtcRequest(
                fake_drm_->crtc_property(0).id, kConnectorIdBase, kDefaultMode, gfx::Point(), &state, std::move(overlays), /*enable_vrr=*/false));
        }
        {
            DrmOverlayPlaneList overlays;
            overlays.push_back(DrmOverlayPlane::TestPlane(fake_buffer_));
            commit_request.push_back(CrtcCommitRequest::EnableCrtcRequest(
                fake_drm_->crtc_property(1).id, kConnectorIdBase + 1, kDefaultMode, gfx::Point(), &state, std::move(overlays), /*enable_vrr=*/false));
        }
        {
            DrmOverlayPlaneList overlays;
            overlays.push_back(DrmOverlayPlane::TestPlane(fake_buffer_));
            commit_request.push_back(CrtcCommitRequest::EnableCrtcRequest(
                fake_drm_->crtc_property(2).id, kConnectorIdBase + 3, kDefaultMode, gfx::Point(), &state, std::move(overlays), /*enable_vrr=*/false));
        }
...
TEST_P(HardwareDisplayPlaneManagerTest, SequenceIncrementOnModesetOnly)
{
    ...
    // Modeset Test
    {
        ...
        ASSERT_TRUE(fake_drm_->plane_manager()->Commit(CommitRequest(), DRM_MODE_ATOMIC_TEST_ONLY | DRM_MODE_ATOMIC_ALLOW_MODESET));
    ...
    // Successful Modeset
    {
        ...
        ASSERT_TRUE(fake_drm_->plane_manager()->Commit(CommitRequest(), DRM_MODE_ATOMIC_ALLOW_MODESET));
    ...
    // Failed Modeset
    {
        ...
        ASSERT_FALSE(fake_drm_->plane_manager()->Commit(CommitRequest(), DRM_MODE_ATOMIC_ALLOW_MODESET));
    ...
    // Page Flip
    {
        ...
        ASSERT_TRUE(fake_drm_->plane_manager()->Commit(CommitRequest(), DRM_MODE_ATOMIC_NONBLOCK));
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, Modeset)
{
    ...
    commit_request.push_back(CrtcCommitRequest::EnableCrtcRequest(fake_drm_->crtc_property(0).id, fake_drm_->connector_property(0).id, kDefaultMode,
        gfx::Point(), &state, std::move(overlays),
        /*enable_vrr=*/false));
    EXPECT_FALSE(fake_drm_->plane_manager()->Commit(std::move(commit_request), DRM_MODE_ATOMIC_ALLOW_MODESET));
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, DisableModeset)
{
    ...
    HardwareDisplayPlaneList state;
    CommitRequest commit_request;
    commit_request.push_back(CrtcCommitRequest::DisableCrtcRequest(fake_drm_->crtc_property(0).id, fake_drm_->connector_property(0).id, &state));
    EXPECT_TRUE(fake_drm_->plane_manager()->Commit(std::move(commit_request), DRM_MODE_ATOMIC_ALLOW_MODESET));
}
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, SinglePlaneAssignment)
{
    ...
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, fake_drm_->crtc_property(0).id));
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, BadCrtc)
{
    ...
    EXPECT_FALSE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, 0));
}
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, NotEnoughPlanes)
{
    ...
    EXPECT_FALSE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, fake_drm_->crtc_property(0).id));
}
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, MultipleCrtcs)
{
    ...
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, fake_drm_->crtc_property(0).id));
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, fake_drm_->crtc_property(1).id));
...
TEST_P(HardwareDisplayPlaneManagerLegacyTest, MultiplePlanesAndCrtcs)
{
    ...
    EXPECT_FALSE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, fake_drm_->crtc_property(0).id));
    EXPECT_FALSE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns, fake_drm_->crtc_property(1).id));
...
{
    DrmOverlayPlaneList assigns;
    scoped_refptr<DrmFramebuffer> buffer = CreateBufferWithFormat(kDefaultBufferSize, DRM_FORMAT_NV12);
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, CheckPropsAfterModeset)
{
    ...
    ScopedDrmObjectPropertyPtr connector_props = fake_drm_->GetObjectProperties(kConnectorIdBase, DRM_MODE_OBJECT_CONNECTOR);
    GetDrmPropertyForName(fake_drm_.get(), connector_props.get(), "CRTC_ID", &connector_prop_crtc_id);
    EXPECT_EQ(kCrtcIdPropId, connector_prop_crtc_id.id);

    DrmDevice::Property crtc_prop_for_name;
    ScopedDrmObjectPropertyPtr crtc_props = fake_drm_->GetObjectProperties(kCrtcIdBase, DRM_MODE_OBJECT_CRTC);
    GetDrmPropertyForName(fake_drm_.get(), crtc_props.get(), "ACTIVE", &crtc_prop_for_name);
    EXPECT_EQ(kActivePropId, crtc_prop_for_name.id);
    EXPECT_EQ(1U, crtc_prop_for_name.value);

    GetDrmPropertyForName(fake_drm_.get(), crtc_props.get(), "MODE_ID", &crtc_prop_for_name);
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, CheckVrrAfterModeset)
{
    ...
    fake_drm_->AddProperty(fake_drm_->crtc_property(0).id, { .id = kVrrEnabledPropId, .value = 0 });
    fake_drm_->InitializeState(/*use_atomic=*/true);
    HardwareDisplayPlaneList state;
    ...
    // Check initial VRR_ENABLED state.
    {
        ...
        ScopedDrmObjectPropertyPtr crtc_props = fake_drm_->GetObjectProperties(kCrtcIdBase, DRM_MODE_OBJECT_CRTC);
        GetDrmPropertyForName(fake_drm_.get(), crtc_props.get(), "VRR_ENABLED", &crtc_prop_vrr_enabled);
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, SharedPlanes)
{
    ...
    auto in_formats_blob = fake_drm_->CreateInFormatsBlob({ DRM_FORMAT_XRGB8888 }, {});

    auto plane_prop = fake_drm_->AddPlane({ fake_drm_->crtc_property(0).id, fake_drm_->crtc_property(1).id }, DRM_PLANE_TYPE_OVERLAY);
    fake_drm_->InitializeState(use_atomic_);
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, UnusedPlanesAreReleased)
{
    ...
    DrmOverlayPlaneList assigns;
    scoped_refptr<DrmFramebuffer> primary_buffer = CreateBuffer(kDefaultBufferSize);
    ...
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&hdpl, assigns, fake_drm_->crtc_property(0).id));
    EXPECT_TRUE(fake_drm_->plane_manager()->Commit(&hdpl, page_flip_request, nullptr));
    assigns.clear();
    assigns.push_back(DrmOverlayPlane::TestPlane(primary_buffer));
    fake_drm_->plane_manager()->BeginFrame(&hdpl);
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&hdpl, assigns, fake_drm_->crtc_property(0).id));
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, AssignPlanesRestoresInUse)
{
    ...
    EXPECT_FALSE(fake_drm_->plane_manager()->AssignOverlayPlanes(&hdpl, assigns, fake_drm_->crtc_property(0).id));
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, PageFlipOnlySwapsPlaneListsOnSuccess)
{
    fake_drm_->ResetStateWithDefaultObjects(
        /*crtc_count=*/1, /*planes_per_crtc=*/2);
    fake_drm_->InitializeState(use_atomic_);
    ...
    single_assign.push_back(DrmOverlayPlane::TestPlane(CreateBuffer(kDefaultBufferSize)));

    DrmOverlayPlaneList overlay_assigns;
    overlay_assigns.push_back(DrmOverlayPlane::TestPlane(CreateBuffer(kDefaultBufferSize)));
    overlay_assigns.push_back(DrmOverlayPlane::TestPlane(CreateBuffer(kDefaultBufferSize)));

    HardwareDisplayPlaneList hdpl;

    auto flip_with_assigns = [&](bool commit_status, const auto& assigns) -> bool {
        auto page_flip_request = base::MakeRefCounted<PageFlipRequest>(base::TimeDelta());
        fake_drm_->plane_manager()->BeginFrame(&hdpl);
        EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&hdpl, assigns, fake_drm_->crtc_property(0).id));
        fake_drm_->set_commit_expectation(commit_status);
        return fake_drm_->plane_manager()->Commit(&hdpl, page_flip_request, nullptr);
    };
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, PlanePinningAndUnpinning)
{
    ...
    assigns_with_overlay.push_back(DrmOverlayPlane::TestPlane(CreateBuffer(gfx::Size(1, 1))));
    ...
    EXPECT_EQ(crtc_0, get_overlay_owner()) << "Assigning a plane should pin it to the CRTC.";
    fake_drm_->RunCallbacks();

    EXPECT_FALSE(fake_drm_->plane_manager()->AssignOverlayPlanes(&list_1, assigns_with_overlay, crtc_1))
        << "Pinned planes should be unassignable while they're pinned.";

    PerformPageFlip(0, &list_0, assigns);
    EXPECT_EQ(0u, get_overlay_owner()) << "Assigning without overlays should unpin the overlay.";

    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&list_1, assigns_with_overlay, crtc_1))
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, PlanesUnpinnedOnFailedFlip)
{
    ...
    EXPECT_EQ(0u, get_overlay_owner()) << "A failed flip should result in the overlay being freed again.";
}
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, PlanesUnpinnedOnDisable)
{
    ...
    EXPECT_EQ(0u, get_overlay_owner()) << "After disabling, the pinned overlay owner should be reset.";
}
...
TEST_P(HardwareDisplayPlaneManagerTest, ColorManagement_Temperature)
{
    ...
    fake_drm_->AddProperty(fake_drm_->crtc_property(0).id, { .id = kCtmPropId, .value = 0 });
    fake_drm_->AddProperty(fake_drm_->crtc_property(0).id, { .id = kDegammaLutSizePropId, .value = 1 });
    fake_drm_->AddProperty(fake_drm_->crtc_property(0).id, { .id = kDegammaLutPropId, .value = 0 });
    fake_drm_->AddProperty(fake_drm_->crtc_property(0).id, { .id = kGammaLutSizePropId, .value = 1 });
    fake_drm_->AddProperty(fake_drm_->crtc_property(0).id, { .id = kGammaLutPropId, .value = 0 });
    ...
    fake_drm_->plane_manager()->SetColorTemperatureAdjustment(fake_drm_->crtc_property(0).id, cta);
    ...
    if (use_atomic_) {
        ...
        EXPECT_NE(0u, GetCrtcPropertyValue(fake_drm_->crtc_property(0).id, "CTM"));
        EXPECT_EQ(0u, GetCrtcPropertyValue(fake_drm_->crtc_property(0).id, "GAMMA_LUT"));
        EXPECT_EQ(0u, GetCrtcPropertyValue(fake_drm_->crtc_property(0).id, "DEGAMMA_LUT"));
    } else {
...
TEST_P(HardwareDisplayPlaneManagerTest, ColorManagement_Profile)
{
    ...
    fake_drm_->plane_manager()->SetColorCalibration(fake_drm_->crtc_property(0).id, calibration);
    ...
    if (use_atomic_) {
        ...
        EXPECT_EQ(2, fake_drm_->get_commit_count());
        EXPECT_NE(0u, GetCrtcPropertyValue(fake_drm_->crtc_property(0).id, "CTM"));
        EXPECT_NE(0u, GetCrtcPropertyValue(fake_drm_->crtc_property(0).id, "GAMMA_LUT"));
        EXPECT_NE(0u, GetCrtcPropertyValue(fake_drm_->crtc_property(0).id, "DEGAMMA_LUT"));
    } else {
        EXPECT_EQ(3, fake_drm_->get_set_object_property_count());
    }
}
...
TEST_P(HardwareDisplayPlaneManagerTest, CtmColorManagement_ColorTemperatureAdjustment)
{
...
// the CTM.
TEST_P(HardwareDisplayPlaneManagerAtomicTest, CtmColorManagement_ColorConversion)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitWithFeatures({ display::features::kCtmColorManagement }, {});
    ...
    fake_drm_->plane_manager()->SetOutputColorSpace(crtc_id, SkNamedPrimariesExt::kP3);
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, CtmColorManagement_Combined)
{
    ...
    {
        fake_drm_->plane_manager()->SetOutputColorSpace(crtc_id, SkNamedPrimariesExt::kSRGB);
    ...
    {
        DrmOverlayPlaneList planes;
        planes.push_back(DrmOverlayPlane::TestPlane(CreateBuffer(kDefaultBufferSize)));
        planes[0].color_space = gfx::ColorSpace::CreateSRGB();
        planes[0].z_order = 1;
    ...
    {
        display::GammaAdjustment gamma_adjustment;
        gamma_adjustment.curve = display::GammaCurve::MakeScale(0.5, 0.25 / 0.75, 0.1 / 0.25);
        fake_drm_->plane_manager()->SetGammaAdjustment(crtc_id, gamma_adjustment);
...
TEST_P(HardwareDisplayPlaneManagerTest, ColorManagement_GammaAdjustment)
{
    ...
    fake_drm_->plane_manager()->SetGammaAdjustment(fake_drm_->crtc_property(0).id, gamma_adjustment);
...
{
    {
        fake_drm_->ResetStateWithDefaultObjects(
            /*crtc_count=*/1, /*planes_per_crtc=*/1);
        fake_drm_->AddProperty(fake_drm_->crtc_property(0).id, { .id = kBackgroundColorPropId, .value = 0 });
    }
    fake_drm_->InitializeState(use_atomic_);
    fake_drm_->plane_manager()->SetBackgroundColor(fake_drm_->crtc_property(0).id, 0);
    if (use_atomic_) {
        HardwareDisplayPlaneList state;
        PerformPageFlip(/*crtc_idx=*/0, &state);
        EXPECT_EQ(1, fake_drm_->get_commit_count());
        EXPECT_EQ(0u, GetCrtcPropertyValue(fake_drm_->crtc_property(0).id, "BACKGROUND_COLOR"));
    } else {
...

    {
        fake_drm_->ResetStateWithDefaultObjects(
            /*crtc_count=*/1, /*planes_per_crtc=*/1);
        fake_drm_->AddProperty(fake_drm_->crtc_property(0).id, { .id = kBackgroundColorPropId, .value = 1 });
    }
    fake_drm_->InitializeState(use_atomic_);
    fake_drm_->plane_manager()->SetBackgroundColor(fake_drm_->crtc_property(0).id, 1);
    if (use_atomic_) {
        HardwareDisplayPlaneList state;
        PerformPageFlip(/*crtc_idx=*/0, &state);
        EXPECT_EQ(2, fake_drm_->get_commit_count());
        EXPECT_EQ(1u, GetCrtcPropertyValue(fake_drm_->crtc_property(0).id, "BACKGROUND_COLOR"));
    } else {
        EXPECT_EQ(0, fake_drm_->get_set_object_property_count());
    }
}

TEST_P(HardwareDisplayPlaneManagerAtomicTest, CommitReturnsNullOutFenceIfOutFencePtrNotSupported)
{
    ...
    fake_drm_->plane_manager()->BeginFrame(&state_);
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns1, fake_drm_->crtc_property(0).id));
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&state_, assigns2, fake_drm_->crtc_property(1).id));

    scoped_refptr<PageFlipRequest> page_flip_request = base::MakeRefCounted<PageFlipRequest>(base::TimeDelta());

    gfx::GpuFenceHandle release_fence;
    EXPECT_TRUE(fake_drm_->plane_manager()->Commit(&state_, page_flip_request, &release_fence));
...
TEST_P(HardwareDisplayPlaneManagerTest, InitializationFailsIfSupportForOutFencePropertiesIsPartial)
{
    ...
    fake_drm_->AddProperty(fake_drm_->crtc_property(0).id, { .id = kOutFencePtrPropId, .value = 1 });
    fake_drm_->AddProperty(fake_drm_->crtc_property(2).id, { .id = kOutFencePtrPropId, .value = 2 });

    EXPECT_FALSE(fake_drm_->InitializeStateWithResult(use_atomic_));
}

TEST_P(HardwareDisplayPlaneManagerTest, InitializationSucceedsIfSupportForOutFencePropertiesIsComplete)
{
    fake_drm_->ResetStateWithDefaultObjects(
        /*crtc_count=*/3, /*planes_per_crtc=*/1);
    fake_drm_->AddProperty(fake_drm_->crtc_property(0).id, { .id = kOutFencePtrPropId, .value = 1 });
    fake_drm_->AddProperty(fake_drm_->crtc_property(1).id, { .id = kOutFencePtrPropId, .value = 2 });
    fake_drm_->AddProperty(fake_drm_->crtc_property(2).id, { .id = kOutFencePtrPropId, .value = 3 });
...
TEST_P(HardwareDisplayPlaneManagerTest, ForceOpaqueFormatsForAddFramebuffer)
{
    ...
    for (const auto& format_pair : kFourCCFormats) {
        scoped_refptr<DrmFramebuffer> drm_fb = CreateBufferWithFormat(kDefaultBufferSize, format_pair.input_fourcc);

        EXPECT_EQ(drm_fb->framebuffer_pixel_format(), format_pair.used_fourcc);
        EXPECT_EQ(drm_fb->opaque_framebuffer_pixel_format(), format_pair.used_fourcc);
    }
    ...
    fake_drm_->ResetStateWithDefaultObjects(
        /*crtc_count=*/3, /*planes_per_crtc=*/1, /*movable_planes=*/0, { DRM_FORMAT_ARGB2101010, DRM_FORMAT_ABGR2101010 }, {});
...
TEST_P(HardwareDisplayPlaneManagerTest, GetHardwareCapabilities)
{
    ...
    for (int i = 0; i < 4; ++i) {
        auto hc = fake_drm_->plane_manager()->GetHardwareCapabilities(kCrtcIdBase + i);
...

    {
        ...
        auto& last_props = drm_state.plane_properties[drm_state.plane_properties.size() - 1];
...
INSTANTIATE_TEST_SUITE_P(All, HardwareDisplayPlaneManagerTest, testing::Values(false, true));

// TODO(dnicoara): Migrate as many tests as possible to the general list above.
INSTANTIATE_TEST_SUITE_P(All, HardwareDisplayPlaneManagerLegacyTest, testing::Values(false));

INSTANTIATE_TEST_SUITE_P(All, HardwareDisplayPlaneManagerAtomicTest, testing::Values(true));
...
public:
    HardwareDisplayPlaneManagerPlanesReadyTest(const HardwareDisplayPlaneManagerPlanesReadyTest&) = delete;
    HardwareDisplayPlaneManagerPlanesReadyTest& operator=(const HardwareDisplayPlaneManagerPlanesReadyTest&) = delete;
...
protected:
    ...
    scoped_refptr<DrmFramebuffer> CreateBuffer(const gfx::Size& size)
    {
        std::unique_ptr<GbmBuffer> buffer = fake_drm_->gbm_device()->CreateBuffer(DRM_FORMAT_XRGB8888, size, GBM_BO_USE_SCANOUT);
        return DrmFramebuffer::AddFramebuffer(fake_drm_, buffer.get(), size);
    }
    ...
    {
        DrmOverlayPlaneList planes;
        planes.push_back(DrmOverlayPlane::TestPlane(CreateBuffer(kDefaultBufferSize), gfx::ColorSpace::CreateSRGB(), fake_fence_fd1_.GetGpuFence()));
        planes.push_back(DrmOverlayPlane::TestPlane(CreateBuffer(kDefaultBufferSize), gfx::ColorSpace::CreateSRGB(), fake_fence_fd2_.GetGpuFence()));
        return planes;
    }
    ...
    base::test::TaskEnvironment task_env_ { base::test::TaskEnvironment::MainThreadType::DEFAULT,
...
void HardwareDisplayPlaneManagerPlanesReadyTest::RequestPlanesReady(DrmOverlayPlaneList planes)
{
    auto set_true = [](bool* b, DrmOverlayPlaneList planes) { *b = true; };
    plane_manager_->RequestPlanesReadyCallback(std::move(planes), base::BindOnce(set_true, &callback_called));
}

void HardwareDisplayPlaneManagerPlanesReadyTest::UseLegacyManager()
{
    plane_manager_ = std::make_unique<HardwareDisplayPlaneManagerLegacy>(fake_drm_.get());
}

void HardwareDisplayPlaneManagerPlanesReadyTest::UseAtomicManager()
{
    plane_manager_ = std::make_unique<HardwareDisplayPlaneManagerAtomic>(fake_drm_.get());
}

TEST_F(HardwareDisplayPlaneManagerPlanesReadyTest, LegacyWithoutFencesIsAsynchronousWithoutFenceWait)
{
    UseLegacyManager();
...
TEST_F(HardwareDisplayPlaneManagerPlanesReadyTest, LegacyWithFencesIsAsynchronousWithFenceWait)
{
    UseLegacyManager();
...
TEST_F(HardwareDisplayPlaneManagerPlanesReadyTest, AtomicWithoutFencesIsAsynchronousWithoutFenceWait)
{
    UseAtomicManager();
    RequestPlanesReady(DrmOverlayPlane::Clone(planes_without_fences_));
...
TEST_F(HardwareDisplayPlaneManagerPlanesReadyTest, AtomicWithFencesIsAsynchronousWithoutFenceWait)
{
    UseAtomicManager();
    RequestPlanesReady(DrmOverlayPlane::Clone(planes_with_fences_));
...
TEST_P(HardwareDisplayPlaneManagerTest, GetPossibleCrtcsBitmaskForConnector)
{
    ...
    EXPECT_EQ(fake_drm_->plane_manager()->GetPossibleCrtcsBitmaskForConnector(connector_1_id), 0b101u);
    EXPECT_EQ(fake_drm_->plane_manager()->GetPossibleCrtcsBitmaskForConnector(connector_2_id), 0b110u);
    EXPECT_EQ(fake_drm_->plane_manager()->GetPossibleCrtcsBitmaskForConnector(connector_3_id), 0b011u);
}

TEST_P(HardwareDisplayPlaneManagerTest, GetPossibleCrtcsBitmaskForConnectorInvalidConnector)
{
    fake_drm_->ResetStateWithAllProperties();
    fake_drm_->AddCrtc();
    FakeDrmDevice::EncoderProperties& encoder = fake_drm_->AddEncoder();
    ...
    EXPECT_EQ(fake_drm_->plane_manager()->GetPossibleCrtcsBitmaskForConnector(connector_id + 1), 0u);
}

TEST_P(HardwareDisplayPlaneManagerAtomicTest, OriginalModifiersSupportOnly)
{
    fake_drm_->ResetStateWithDefaultObjects(
        /*crtc_count=*/1, /*planes_per_crtc=*/1, /*movable_planes=*/0, { DRM_FORMAT_NV12 }, {});
    fake_drm_->InitializeState(use_atomic_);

    {
        DrmOverlayPlaneList assigns;
        // Create as NV12 since this is required for rotation support.
        std::unique_ptr<GbmBuffer> buffer = fake_drm_->gbm_device()->CreateBuffer(DRM_FORMAT_NV12, kDefaultBufferSize, GBM_BO_USE_SCANOUT);
        scoped_refptr<DrmFramebuffer> framebuffer_original = DrmFramebuffer::AddFramebuffer(fake_drm_, buffer.get(), kDefaultBufferSize, {}, true);
        assigns.push_back(DrmOverlayPlane::TestPlane(framebuffer_original));
        assigns.back().plane_transform = gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_270;
...

    {
        ...
        // fail rotation.
        std::unique_ptr<GbmBuffer> buffer = fake_drm_->gbm_device()->CreateBuffer(DRM_FORMAT_NV12, kDefaultBufferSize, GBM_BO_USE_SCANOUT);
        scoped_refptr<DrmFramebuffer> framebuffer_non_original = DrmFramebuffer::AddFramebuffer(fake_drm_, buffer.get(), kDefaultBufferSize, {}, false);
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, OverlaySourceCrop)
{
    ...
    {
        DrmOverlayPlaneList assigns;
        assigns.emplace_back(fake_buffer_, gfx::ColorSpace::CreateSRGB(), 0, gfx::OverlayTransform::OVERLAY_TRANSFORM_NONE, gfx::Rect(),
...

    {
        DrmOverlayPlaneList assigns;
        assigns.emplace_back(fake_buffer_, gfx::ColorSpace::CreateSRGB(), 0, gfx::OverlayTransform::OVERLAY_TRANSFORM_NONE, gfx::Rect(),
            gfx::Rect(kDefaultBufferSize), gfx::RectF(0, 0, .999, .501), false, nullptr);
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, ColorEncodingAndRange)
{
    ...
    fake_drm_->AddProperty(
        /*object_id=*/fake_drm_->plane_property(0).id, { .id = kColorEncodingPropId, .value = kColorEncodingBT601 });
    ...
    fake_drm_->AddProperty(/*object_id=*/fake_drm_->plane_property(0).id, { .id = kColorRangePropId, .value = kColorRangeFull });
    ...
    std::vector<TestCase> test_cases = {
        {
            .color_space = gfx::ColorSpace(
                gfx::ColorSpace::PrimaryID::BT709, gfx::ColorSpace::TransferID::BT709, gfx::ColorSpace::MatrixID::BT709, gfx::ColorSpace::RangeID::LIMITED),
            .expected_color_encoding = kColorEncodingBT709,
            .expected_color_range = kColorRangeLimited,
        },
        {
            .color_space = gfx::ColorSpace(
                gfx::ColorSpace::PrimaryID::BT709, gfx::ColorSpace::TransferID::BT709, gfx::ColorSpace::MatrixID::BT709, gfx::ColorSpace::RangeID::FULL),
            .expected_color_encoding = kColorEncodingBT709,
            .expected_color_range = kColorRangeLimited,
        },
        {
            .color_space = gfx::ColorSpace(gfx::ColorSpace::PrimaryID::SMPTE170M, gfx::ColorSpace::TransferID::SMPTE170M, gfx::ColorSpace::MatrixID::SMPTE170M,
    ...
    for (const auto& test_case : test_cases) {
        ...
        EXPECT_EQ(test_case.expected_color_encoding, GetPlanePropertyValue(kPlaneOffset, "COLOR_ENCODING"));
        EXPECT_EQ(test_case.expected_color_range, GetPlanePropertyValue(kPlaneOffset, "COLOR_RANGE"));
    }
}
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, OldPlaneInAnotherList)
{
    ...
    plane_list_1.old_plane_list.push_back(plane_0);
    {
        ...
        CrtcCommitRequest request = CrtcCommitRequest::EnableCrtcRequest(crtc_0_id, fake_drm_->connector_property(0).id, kDefaultMode, gfx::Point(),
            &plane_list_0, std::move(overlays),
            /*enable_vrr=*/false);
        commit_request.push_back(std::move(request));
    }
    {
        ...
        CrtcCommitRequest request = CrtcCommitRequest::EnableCrtcRequest(crtc_1_id, fake_drm_->connector_property(1).id, kDefaultMode, gfx::Point(),
            &plane_list_1, std::move(overlays),
            /*enable_vrr=*/false);
        commit_request.push_back(std::move(request));
    }

    ASSERT_TRUE(fake_drm_->plane_manager()->Commit(std::move(commit_request), DRM_MODE_ATOMIC_TEST_ONLY | DRM_MODE_ATOMIC_ALLOW_MODESET));
...
public:
    HardwareDisplayPlaneAtomicMock()
        : HardwareDisplayPlaneAtomic(1)
    {
    }
    ~HardwareDisplayPlaneAtomicMock() override = default;

    bool AssignPlaneProps(DrmDevice* drm, uint32_t crtc_id, uint32_t framebuffer, const gfx::Rect& crtc_rect, const gfx::Rect& src_rect,
        const gfx::Rect& damage_rect, const gfx::OverlayTransform transform, const gfx::ColorSpace& color_space, int in_fence_fd, uint32_t format_fourcc,
...
TEST(HardwareDisplayPlaneManagerAtomic, EnableBlend)
{
    ...
    auto plane_manager = std::make_unique<HardwareDisplayPlaneManagerAtomic>(drm_device.get());
    HardwareDisplayPlaneList plane_list;
    HardwareDisplayPlaneAtomicMock hw_plane;
    std::unique_ptr<GbmBuffer> buffer = drm_device->gbm_device()->CreateBuffer(DRM_FORMAT_XRGB8888, kDefaultBufferSize, GBM_BO_USE_SCANOUT);
    scoped_refptr<DrmFramebuffer> framebuffer = DrmFramebuffer::AddFramebuffer(drm_device, buffer.get(), kDefaultBufferSize);
    DrmOverlayPlane overlay(DrmOverlayPlane::TestPlane(framebuffer));
    overlay.enable_blend = true;
    plane_manager->SetPlaneData(&plane_list, &hw_plane, overlay, 1, std::nullopt, gfx::Rect());
...
protected:
    void SetUp() override
    {
        ...
        plane_manager_ = std::make_unique<HardwareDisplayPlaneManagerAtomic>(drm_device_.get());
...
TEST_F(HardwareDisplayPlaneManagerSeamlessModeTest, TestSeamlessMode)
{
    ...
    drmModeModeInfo arbitrary_mode = { .hdisplay = 1234, .vdisplay = 567, .vrefresh = 19u };
    ...
    EXPECT_CALL(*drm_device_, CommitProperties(_, DRM_MODE_ATOMIC_TEST_ONLY, 1, _)).Times(1).WillRepeatedly(Return(false));
    EXPECT_FALSE(plane_manager_->TestSeamlessMode(crtc_id_, arbitrary_mode));

    // CommitProperties returns true.
    EXPECT_CALL(*drm_device_, CommitProperties(_, DRM_MODE_ATOMIC_TEST_ONLY, 1, _)).Times(1).WillRepeatedly(Return(true));
    EXPECT_TRUE(plane_manager_->TestSeamlessMode(crtc_id_, arbitrary_mode));
}

TEST_F(HardwareDisplayPlaneManagerSeamlessModeTest, TestSeamlessMode_InvalidCrtcId)
{
    // Any arbitrary mode to be tested for seamless configuration.
    drmModeModeInfo arbitrary_mode = { .hdisplay = 1234, .vdisplay = 567, .vrefresh = 19u };
    ...
    EXPECT_DCHECK_DEATH(plane_manager_->TestSeamlessMode(wrong_crtc_id, arbitrary_mode));
}
...
TEST_P(HardwareDisplayPlaneManagerAtomicTest, CrtcOffsetPageFlip)
{
    ...
    scoped_refptr<PageFlipRequest> page_flip_request = base::MakeRefCounted<PageFlipRequest>(base::TimeDelta());
    fake_drm_->plane_manager()->BeginFrame(&hdpl);
    EXPECT_TRUE(fake_drm_->plane_manager()->AssignOverlayPlanes(&hdpl, assigns, fake_drm_->crtc_property(0).id, crtc_offset));
    EXPECT_TRUE(fake_drm_->plane_manager()->Commit(&hdpl, page_flip_request, nullptr));

    // DrmWrapper::Property::value is a uint64_t, even though the crtc offset is
    // negative.
    EXPECT_EQ(GetPlanePropertyValue(kPlaneOffset, "CRTC_X"), static_cast<uint64_t>(-100));
    EXPECT_EQ(GetPlanePropertyValue(kPlaneOffset, "CRTC_Y"), static_cast<uint64_t>(-200));
}
} // namespace ui
