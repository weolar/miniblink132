...
bool DestroyDumbBuffer(const scoped_refptr<DrmDevice>& drm_device, uint32_t handle, DrmDumbBuffer::HandleCloser handle_closer)
{
...
DrmDumbBuffer::DrmDumbBuffer(const scoped_refptr<DrmDevice>& drm)
    : drm_(drm)
{
}
...
bool DrmDumbBuffer::Initialize(const SkImageInfo& info)
{
    ...
    if (!drm_->CreateDumbBuffer(info, &handle_, &stride_)) {
        PLOG(ERROR) << "DrmDumbBuffer: CreateDumbBuffer: width " << info.width() << " height " << info.height();
        return false;
    }
...
bool DrmDumbBuffer::InitializeFromFramebuffer(uint32_t framebuffer_id)
{
    ...
    SkImageInfo info = SkImageInfo::MakeN32Premul(framebuffer->width, framebuffer->height);
...
bool DrmDumbBuffer::MapDumbBuffer(const SkImageInfo& info)
{
    ...
    if (!surface_) {
        LOG(ERROR) << "DrmDumbBuffer: Failed to create SkSurface: handle " << handle_;
        return false;
    }
