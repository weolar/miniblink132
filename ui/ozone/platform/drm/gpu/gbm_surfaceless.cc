...
GbmSurfaceless::GbmSurfaceless(
    GbmSurfaceFactory* surface_factory, gl::GLDisplayEGL* display, std::unique_ptr<DrmWindowProxy> window, gfx::AcceleratedWidget widget)
...
bool GbmSurfaceless::ScheduleOverlayPlane(gl::OverlayImage image, std::unique_ptr<gfx::GpuFence> gpu_fence, const gfx::OverlayPlaneData& overlay_plane_data)
{
    unsubmitted_frames_.back()->overlays.emplace_back(std::move(image), std::move(gpu_fence), overlay_plane_data);
    return true;
}

bool GbmSurfaceless::Resize(const gfx::Size& size, float scale_factor, const gfx::ColorSpace& color_space, bool has_alpha)
{
    return true;
}
...
void GbmSurfaceless::Present(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, gfx::FrameData data)
{
    ...
    if (supports_plane_gpu_fences_ || (!use_egl_fence_sync_ && !is_on_external_drm_device_)) {
        frame->ready = true;
        SubmitFrame();
        return;
    }
    ...
    base::OnceClosure fence_wait_task = base::BindOnce(&WaitForFence, GetEGLDisplay(), fence);

    base::OnceClosure fence_retired_callback = base::BindOnce(&GbmSurfaceless::FenceRetired, weak_factory_.GetWeakPtr(), frame);
...
bool GbmSurfaceless::PendingFrame::ScheduleOverlayPlanes(gfx::AcceleratedWidget widget)
{
    ...
    if (unsubmitted_frames_.front()->ready && !submitted_frame_) {
        ...
        for (auto& overlay : unsubmitted_frames_.front()->overlays) {
            if (overlay.z_order() == 0 && overlay.gpu_fence()) {
                submitted_frame_gpu_fence_ = std::make_unique<gfx::GpuFence>(overlay.gpu_fence()->GetGpuFenceHandle().Clone());
            }
            ...
            if (overlay.overlay_type() == gfx::OverlayType::kFullScreen && should_handle_non_simple_overlay_failure) {
        ...
        bool schedule_planes_succeeded = submitted_frame_->ScheduleOverlayPlanes(widget_);

        if (!schedule_planes_succeeded) {
            OnSubmission(should_handle_non_simple_overlay_failure, gfx::SwapResult::SWAP_FAILED,
        ...
        window_->SchedulePageFlip(std::move(planes_),
            base::BindOnce(&GbmSurfaceless::OnSubmission, weak_factory_.GetWeakPtr(), should_handle_non_simple_overlay_failure),
            base::BindOnce(&GbmSurfaceless::OnPresentation, weak_factory_.GetWeakPtr()));
        planes_.clear();
    }
}
...
EGLSyncKHR GbmSurfaceless::InsertFence()
{
    ...
    const bool has_implicit_external_fence = display_->ext->b_EGL_ARM_implicit_external_sync;
    ...
    const EGLenum syncType = has_global_fence ? EGL_SYNC_GLOBAL_FENCE_ANGLE : EGL_SYNC_FENCE_KHR;
    const bool use_implicit_external_sync = has_implicit_external_fence && !has_global_fence;
    const EGLint attrib_list[] = { EGL_SYNC_CONDITION_KHR, EGL_SYNC_PRIOR_COMMANDS_IMPLICIT_EXTERNAL_ARM, EGL_NONE };

    return eglCreateSyncKHR(GetEGLDisplay(), syncType, use_implicit_external_sync ? attrib_list : nullptr);
}
...
void GbmSurfaceless::OnSubmission(bool should_handle_fullscreen_overlay_failure, gfx::SwapResult result, gfx::GpuFenceHandle release_fence)
{
    ...
    if (should_handle_fullscreen_overlay_failure && result == gfx::SwapResult::SWAP_FAILED) {
        result = gfx::SwapResult::SWAP_NON_SIMPLE_OVERLAYS_FAILED;
    }
    submitted_frame_->swap_result = result;
    if (!release_fence.is_null()) {
        std::move(submitted_frame_->completion_callback).Run(gfx::SwapCompletionResult(result, std::move(release_fence)));
    }
}
...
void GbmSurfaceless::OnPresentation(const gfx::PresentationFeedback& feedback)
{
    ...
    if (submitted_frame_gpu_fence_ && !feedback.failed()) {
        feedback_copy.ready_timestamp = submitted_frame_gpu_fence_->GetMaxTimestamp();
    }
    ...
    if (submitted_frame_->completion_callback)
        std::move(submitted_frame_->completion_callback).Run(gfx::SwapCompletionResult(result));
