...
gfx::GpuFenceHandle CreateMergedGpuFenceFromFDs(std::vector<base::ScopedFD> fence_fds)
{
...
std::vector<uint32_t> GetCrtcIdsOfPlanes(const HardwareDisplayPlaneList& plane_list)
{
...
bool AddPendingCrtcProperty(drmModeAtomicReq* property_set, uint32_t crtc_id, DrmWrapper::Property& prop, std::optional<ScopedDrmPropertyBlob>& pending_blob,
    std::vector<ScopedDrmPropertyBlob>& pending_blobs)
{
    ...
    int ret = drmModeAtomicAddProperty(property_set, crtc_id, prop.id, prop.value);
    if (ret < 0) {
...
bool AddAllPendingCrtcProperties(
    drmModeAtomicReq* property_set, HardwareDisplayPlaneManager::CrtcState& crtc_state, std::vector<ScopedDrmPropertyBlob>& pending_blobs)
{
    bool result = true;
    auto& crtc_props = crtc_state.properties;

    if (!AddPendingCrtcProperty(property_set, crtc_props.id, crtc_props.ctm, crtc_state.pending_ctm_blob, pending_blobs)) {
        LOG(ERROR) << "Failed to set CTM property for crtc=" << crtc_props.id;
        result = false;
    }

    if (!AddPendingCrtcProperty(property_set, crtc_props.id, crtc_props.degamma_lut, crtc_state.pending_degamma_lut_blob, pending_blobs)) {
        LOG(ERROR) << "Failed to set DEGAMMA_LUT property for crtc=" << crtc_props.id;
        result = false;
    }

    if (!AddPendingCrtcProperty(property_set, crtc_props.id, crtc_props.gamma_lut, crtc_state.pending_gamma_lut_blob, pending_blobs)) {
...
bool ResetPlanePropsIfUnused(const base::flat_set<HardwareDisplayPlaneList*>& plane_lists, HardwareDisplayPlane* plane)
{
...
HardwareDisplayPlaneManagerAtomic::HardwareDisplayPlaneManagerAtomic(DrmDevice* drm)
    : HardwareDisplayPlaneManager(drm)
{
}

HardwareDisplayPlaneManagerAtomic::~HardwareDisplayPlaneManagerAtomic() = default;

bool HardwareDisplayPlaneManagerAtomic::SetCrtcProps(drmModeAtomicReq* atomic_request, uint32_t crtc_id, bool set_active, uint32_t mode_id, bool enable_vrr)
{
    ...
    bool status = AddPropertyIfValid(atomic_request, crtc_id, modeset_props.active);
    status &= AddPropertyIfValid(atomic_request, crtc_id, modeset_props.mode_id);
    status &= AddPropertyIfValid(atomic_request, crtc_id, modeset_props.vrr_enabled);
    return status;
}

bool HardwareDisplayPlaneManagerAtomic::SetConnectorProps(drmModeAtomicReq* atomic_request, uint32_t connector_id, uint32_t crtc_id)
{
    ...
    if (connector_props.connection == DRM_MODE_CONNECTED && connector_props.count_modes != 0) {
        connector_props.link_status.value = DRM_MODE_LINK_STATUS_GOOD;
    }

    bool status = AddPropertyIfValid(atomic_request, connector_id, connector_props.crtc_id);
    status &= AddPropertyIfValid(atomic_request, connector_id, connector_props.link_status);
    return status;
}

bool HardwareDisplayPlaneManagerAtomic::Commit(CommitRequest commit_request, uint32_t flags)
{
    ...
    for (const auto& crtc_request : commit_request) {
        ...
        if (crtc_request.should_enable_crtc()) {
            auto mode_blob = drm_->CreatePropertyBlob(&crtc_request.mode(), sizeof(crtc_request.mode()));
        ...
        status &= SetCrtcProps(atomic_request.get(), crtc_id, crtc_request.should_enable_crtc(), mode_id, crtc_request.enable_vrr());
        status &= SetConnectorProps(atomic_request.get(), crtc_request.connector_id(), crtc_request.should_enable_crtc() * crtc_id);

        if (crtc_request.should_enable_crtc()) {
            DCHECK(crtc_request.plane_list());
            if (!AssignOverlayPlanes(crtc_request.plane_list(), crtc_request.overlays(), crtc_id)) {
    ...
    std::vector<ScopedDrmPropertyBlob> pending_blobs;
    SetAtomicPropsForCommit(enable_planes_lists, atomic_request.get(), pending_blobs, is_testing);
    ...
    if (!status || !drm_->CommitProperties(atomic_request.get(), flags, commit_request.size(), nullptr)) {
        if (is_testing)
...
void HardwareDisplayPlaneManagerAtomic::SetAtomicPropsForCommit(const base::flat_set<HardwareDisplayPlaneList*>& plane_lists, drmModeAtomicReq* atomic_request,
    std::vector<ScopedDrmPropertyBlob>& pending_blobs, bool test_only)
{
    for (HardwareDisplayPlaneList* plane_list : plane_lists) {
        ...
        for (uint32_t crtc : crtcs) {
            ...
            AddPropertyIfValid(atomic_request, crtc, crtc_state_[*idx].properties.background_color);
            if (base::FeatureList::IsEnabled(display::features::kCtmColorManagement)) {
                AddAllPendingCrtcProperties(atomic_request, crtc_state_[*idx], pending_blobs);
            }
        }
...
bool HardwareDisplayPlaneManagerAtomic::Commit(
    HardwareDisplayPlaneList* plane_list, scoped_refptr<PageFlipRequest> page_flip_request, gfx::GpuFenceHandle* release_fence)
{
    bool test_only = !page_flip_request;

    ScopedDrmAtomicReqPtr atomic_property_set(drmModeAtomicAlloc());
    std::vector<ScopedDrmPropertyBlob> pending_blobs;
    SetAtomicPropsForCommit(base::flat_set<HardwareDisplayPlaneList*> { plane_list }, atomic_property_set.get(), pending_blobs, test_only);
    ...
    std::vector<base::ScopedFD> out_fence_fds;
    {
        ...
        if (release_fence) {
            if (!AddOutFencePtrProperties(atomic_property_set.get(), crtcs, &out_fence_fds, &out_fence_fd_receivers)) {
                ResetCurrentPlaneList(plane_list);
                return false;
            }
        }

        uint32_t flags = test_only ? DRM_MODE_ATOMIC_TEST_ONLY : DRM_MODE_ATOMIC_NONBLOCK;

        if (!drm_->CommitProperties(atomic_property_set.get(), flags, crtcs.size(), page_flip_request)) {
            if (!test_only) {
...
bool HardwareDisplayPlaneManagerAtomic::TestSeamlessMode(int32_t crtc_id, const drmModeModeInfo& mode)
{
    TRACE_EVENT1("drm", "HardwareDisplayPlaneManagerAtomic::TestSeamlessMode", "crtc_id", crtc_id);
    ScopedDrmAtomicReqPtr atomic_request(drmModeAtomicAlloc());

    ScopedDrmPropertyBlob mode_blob = drm_->CreatePropertyBlob(&mode, sizeof(mode));
    ...
    if (!AddPropertyIfValid(atomic_request.get(), crtc_id, modeset_props.mode_id)) {
    ...
    return drm_->CommitProperties(atomic_request.get(), seamless_test_flags, num_crtcs, nullptr);
}

bool HardwareDisplayPlaneManagerAtomic::DisableOverlayPlanes(HardwareDisplayPlaneList* plane_list)
{
    bool ret = true;
    ...
    if (!plane_list->old_plane_list.empty()) {
        ...
        for (HardwareDisplayPlane* plane : plane_list->old_plane_list) {
            ...
            HardwareDisplayPlaneAtomic* atomic_plane = static_cast<HardwareDisplayPlaneAtomic*>(plane);
            atomic_plane->AssignPlaneProps(nullptr, 0, 0, gfx::Rect(), gfx::Rect(), gfx::Rect(), gfx::OVERLAY_TRANSFORM_NONE, gfx::ColorSpace(),
...
bool HardwareDisplayPlaneManagerAtomic::ValidatePrimarySize(const DrmOverlayPlane& primary, const drmModeModeInfo& mode)
{
...
void HardwareDisplayPlaneManagerAtomic::RequestPlanesReadyCallback(DrmOverlayPlaneList planes, base::OnceCallback<void(DrmOverlayPlaneList planes)> callback)
{
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), std::move(planes)));
}

bool HardwareDisplayPlaneManagerAtomic::SetPlaneData(HardwareDisplayPlaneList*, HardwareDisplayPlane* hw_plane, const DrmOverlayPlane& overlay,
    uint32_t crtc_id, std::optional<gfx::Point> crtc_offset, const gfx::Rect& src_rect)
{
    HardwareDisplayPlaneAtomic* atomic_plane = static_cast<HardwareDisplayPlaneAtomic*>(hw_plane);
    uint32_t framebuffer_id = overlay.enable_blend ? overlay.buffer->framebuffer_id() : overlay.buffer->opaque_framebuffer_id();
    ...
    if (!atomic_plane->AssignPlaneProps(drm_, crtc_id, framebuffer_id, crtc_rect, src_rect, overlay.damage_rect, overlay.plane_transform, overlay.color_space,
            fence_fd, overlay.buffer->framebuffer_pixel_format(), overlay.buffer->is_original_buffer())) {
        return false;
    }
    return true;
}
...
bool HardwareDisplayPlaneManagerAtomic::InitializePlanes()
{
    ...
    for (uint32_t i = 0; i < plane_resources->count_planes; ++i) {
        std::unique_ptr<HardwareDisplayPlane> plane(CreatePlane(plane_resources->planes[i]));
...
std::unique_ptr<HardwareDisplayPlane> HardwareDisplayPlaneManagerAtomic::CreatePlane(uint32_t plane_id)
{
    return std::make_unique<HardwareDisplayPlaneAtomic>(plane_id);
}

bool HardwareDisplayPlaneManagerAtomic::CommitPendingCrtcState(CrtcState& crtc_state)
{
    std::vector<ScopedDrmPropertyBlob> pending_blobs;
    ScopedDrmAtomicReqPtr property_set(drmModeAtomicAlloc());

    bool result = AddAllPendingCrtcProperties(property_set.get(), crtc_state, pending_blobs);
    ...
    if (!drm_->CommitProperties(property_set.get(), 0, 0, nullptr)) {
        LOG(ERROR) << "Failed to commit properties for crtc=" << crtc_state.properties.id;
        result = false;
    }
...
bool HardwareDisplayPlaneManagerAtomic::AddOutFencePtrProperties(drmModeAtomicReq* property_set, const std::vector<uint32_t>& crtcs,
    std::vector<base::ScopedFD>* out_fence_fds, std::vector<base::ScopedFD::Receiver>* out_fence_fd_receivers)
{
    ...
    for (uint32_t crtc : crtcs) {
        ...
        const auto out_fence_ptr_id = crtc_state_[*crtc_index].properties.out_fence_ptr.id;
        ...
        if (out_fence_ptr_id > 0) {
            ...
            // until then.
            int ret = drmModeAtomicAddProperty(property_set, crtc, out_fence_ptr_id, reinterpret_cast<uint64_t>(out_fence_fd_receivers->back().get()));
            if (ret < 0) {
                LOG(ERROR) << "Failed to set OUT_FENCE_PTR property for crtc=" << crtc << " error=" << -ret;
