...
public:
    ...
    void CursorSet(
        gfx::AcceleratedWidget window, const std::vector<SkBitmap>& bitmaps, const std::optional<gfx::Point>& point, base::TimeDelta frame_delay) override
    {
    }
...
gfx::Point DrmCursor::GetBitmapLocationLocked() EXCLUSIVE_LOCKS_REQUIRED(lock_)
{
    return gfx::ToFlooredPoint(location_) - cursor_->hotspot().OffsetFromOrigin();
}

void DrmCursor::SetCursor(gfx::AcceleratedWidget window, scoped_refptr<BitmapCursor> platform_cursor)
{
...
void DrmCursor::OnWindowAdded(gfx::AcceleratedWidget window, const gfx::Rect& bounds_in_screen, const gfx::Rect& cursor_confined_bounds)
{
...
void DrmCursor::CommitBoundsChange(gfx::AcceleratedWidget window, const gfx::Rect& new_display_bounds_in_screen, const gfx::Rect& new_confined_bounds)
{
...
void DrmCursor::MoveCursorTo(gfx::AcceleratedWidget window, const gfx::PointF& location)
{
...
void DrmCursor::MoveCursorToOnUiThread(const gfx::PointF& screen_location)
{
    ...
    const auto* window = window_manager_->GetWindowAt(gfx::ToRoundedPoint(screen_location));
    if (!window) {
    ...
    auto location_in_window = screen_location - window->GetBoundsInPixels().OffsetFromOrigin();
...
void DrmCursor::MoveCursor(const gfx::Vector2dF& delta)
{
    ...
    DCHECK(evdev_thread_checker_.CalledOnValidThread() || ui_thread_checker_.CalledOnValidThread());
#endif
    ...
    ui::CursorController::GetInstance()->ApplyCursorConfigForWindow(window_, &transformed_delta);
...
void DrmCursor::SetCursorLocationLocked(const gfx::PointF& location) EXCLUSIVE_LOCKS_REQUIRED(lock_)
{
    gfx::PointF clamped_location = location;
    clamped_location.SetToMax(gfx::PointF(confined_bounds_.origin()));
    // Right and bottom edges are exclusive.
    clamped_location.SetToMin(gfx::PointF(confined_bounds_.right() - 1, confined_bounds_.bottom() - 1));
...
void DrmCursor::SendCursorShowLocked() EXCLUSIVE_LOCKS_REQUIRED(lock_)
{
    ...
    CursorSetLockTested(window_, cursor_->bitmaps(), GetBitmapLocationLocked(), cursor_->frame_delay());
}

void DrmCursor::SendCursorHideLocked() EXCLUSIVE_LOCKS_REQUIRED(lock_)
{
    CursorSetLockTested(window_, std::vector<SkBitmap>(), std::nullopt, base::TimeDelta());
}
...
void DrmCursor::CursorSetLockTested(
    gfx::AcceleratedWidget window, const std::vector<SkBitmap>& bitmaps, const std::optional<gfx::Point>& point, base::TimeDelta frame_delay)
{
    lock_.AssertAcquired();
    proxy_->CursorSet(window, bitmaps, point, frame_delay);
}

void DrmCursor::MoveLockTested(gfx::AcceleratedWidget window, const gfx::Point& point)
{
    lock_.AssertAcquired();
