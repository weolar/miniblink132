...
typedef base::OnceCallback<void(const base::FilePath&, const base::FilePath&, std::unique_ptr<DrmWrapper>)> OnOpenDeviceReplyCallback;
...
base::FilePath MapDevPathToSysPath(const base::FilePath& device_path)
{
    ...
    base::FilePath sys_path = base::MakeAbsoluteFilePath(base::FilePath("/sys/class/drm").Append(device_path.BaseName()));
    ...
    for (const auto& component : sys_path.GetComponents()) {
        ...
        if (component.starts_with("evdi")) {
            ...
            if (base::ReadSymbolicLink(path_thus_far.Append("device"), &usb_device_path)) {
                return base::MakeAbsoluteFilePath(path_thus_far.Append(usb_device_path)).Append(device_path.BaseName());
            }
            break;
        }
    }
...
std::unique_ptr<DrmWrapper> OpenDrmDevice(const base::FilePath& dev_path, const base::FilePath& sys_path, bool is_primary_device)
{
    ...
    while (true) {
        ...
        const bool should_log_error = (base::TimeTicks::Now() - start_time).InMilliseconds() >= kLogAuthFailDelayMs;
        drm_magic_t magic;
        ...
        if (drm_errno) {
            LOG_IF(ERROR, should_log_error) << "Failed to get magic cookie to authenticate: " << dev_path.value() << " with errno: " << drm_errno << " after "
        ...
        if (drm_errno) {
            LOG_IF(ERROR, should_log_error) << "Failed to authenticate: " << dev_path.value() << " with errno: " << drm_errno << " after " << num_auth_attempts
                                            << " attempt(s)";
            usleep(kAuthFailSleepMs * 1000);
            continue;
        }
    ...
    VLOG(1) << "Succeeded authenticating " << dev_path.value() << " in " << (base::TimeTicks::Now() - start_time).InMilliseconds() << " ms "
            << "with " << num_auth_attempts << " attempt(s)";

    auto drm = std::make_unique<DrmWrapper>(sys_path, std::move(scoped_fd), is_primary_device);
...
void OpenDeviceAsync(const base::FilePath& device_path, const scoped_refptr<base::TaskRunner>& reply_runner, OnOpenDeviceReplyCallback callback)
{
    base::FilePath sys_path = MapDevPathToSysPath(device_path);

    std::unique_ptr<DrmWrapper> drm = OpenDrmDevice(device_path, sys_path, /*is_primary_device=*/false);
    reply_runner->PostTask(FROM_HERE, base::BindOnce(std::move(callback), device_path, sys_path, std::move(drm)));
}
...
std::vector<DisplayCard> GetValidDisplayCards()
{
    ...
    for (int card_number = 0; /* end on first card# that does not exist */; card_number++) {
        std::string card_path = base::StringPrintf(kDefaultGraphicsCardPattern, card_number);
        ...
        if (ret == 0 && res.count_crtcs > 0) {
            cards.push_back({ base::FilePath(card_path), GetDrmDriverNameFromFd(fd.get()) });
        }
    }
...
DrmDisplayHostManager::DrmDisplayHostManager(
    GpuThreadAdapter* proxy, DeviceManager* device_manager, OzonePlatform::PlatformRuntimeProperties* host_properties, InputControllerEvdev* input_controller)
    : proxy_(proxy)
...
{
    {
        ...
        primary_drm_device_ = OpenDrmDevice(primary_graphics_card_path_, primary_graphics_card_path_sysfs,
    ...
    for (auto& display_info : display_infos) {
        ...
        std::unique_ptr<display::DisplaySnapshot> current_display_snapshot = CreateDisplaySnapshot(*primary_drm_device_, display_info.get(), 0);
        ...
        edid_id_collision_map[current_display_snapshot->edid_display_id()] = current_display_snapshot.get();

        displays_.push_back(std::make_unique<DrmDisplayHost>(proxy_, std::move(current_display_snapshot), true /* is_dummy */));
    }
}
...
DrmDisplayHostManager::DisplayEvent::DisplayEvent(DeviceEvent::ActionType action_type, const base::FilePath& path, const EventPropertyMap& properties)
    : action_type(action_type)
    , path(path)
    , display_event_props(properties)
{
}

DrmDisplayHostManager::DisplayEvent::DisplayEvent(const DisplayEvent&) = default;
DrmDisplayHostManager::DisplayEvent& DrmDisplayHostManager::DisplayEvent::operator=(const DisplayEvent&) = default;
...
{
    auto it = base::ranges::find(displays_, display_id, [](const std::unique_ptr<DrmDisplayHost>& display) { return display->snapshot()->display_id(); });
    if (it == displays_.end())
        return nullptr;
...
void DrmDisplayHostManager::TakeDisplayControl(display::DisplayControlCallback callback)
{
...
void DrmDisplayHostManager::RelinquishDisplayControl(display::DisplayControlCallback callback)
{
    TRACE_EVENT0("drm", "DrmDisplayHostManager::RelinquishDisplayControl");
    if (display_control_change_pending_) {
        LOG(ERROR) << "RelinquishDisplayControl called while change already pending";
        std::move(callback).Run(false);
        return;
    }
...
void DrmDisplayHostManager::UpdateDisplays(display::GetDisplaysCallback callback)
{
...
void DrmDisplayHostManager::ConfigureDisplays(
    const std::vector<display::DisplayConfigurationParams>& config_requests, display::ConfigureCallback callback, display::ModesetFlags modeset_flags)
{
    ...
    proxy_->GpuConfigureNativeDisplays(config_requests, std::move(callback), modeset_flags);
}
...
void DrmDisplayHostManager::ProcessEvent()
{
    while (!event_queue_.empty() && !task_pending_) {
        ...
        const std::string seqnum = seqnum_it == event.display_event_props.end() ? "" : ("(SEQNUM:" + seqnum_it->second + ")");
        VLOG(1) << "Got display event " << kDisplayActionString[event.action_type] << seqnum << " for " << event.path.value();
        switch (event.action_type) {
        case DeviceEvent::ADD:
            if (drm_devices_.find(event.path) == drm_devices_.end()) {
                base::ThreadPool::PostTask(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN },
                    base::BindOnce(&OpenDeviceAsync, event.path, base::SingleThreadTaskRunner::GetCurrentDefault(),
                        base::BindOnce(&DrmDisplayHostManager::OnAddGraphicsDevice, weak_ptr_factory_.GetWeakPtr())));
                task_pending_ = true;
            }
            break;
        case DeviceEvent::CHANGE:
            task_pending_ = base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
                FROM_HERE, base::BindOnce(&DrmDisplayHostManager::OnUpdateGraphicsDevice, weak_ptr_factory_.GetWeakPtr(), event.display_event_props));
            break;
        case DeviceEvent::REMOVE:
            DCHECK(event.path != primary_graphics_card_path_) << "Removing primary graphics card";
            auto it = drm_devices_.find(event.path);
            if (it != drm_devices_.end()) {
                task_pending_ = base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
                    FROM_HERE, base::BindOnce(&DrmDisplayHostManager::OnRemoveGraphicsDevice, weak_ptr_factory_.GetWeakPtr(), it->second));
...
void DrmDisplayHostManager::OnAddGraphicsDevice(const base::FilePath& dev_path, const base::FilePath& sys_path, std::unique_ptr<DrmWrapper> drm)
{
    if (drm) {
        drm_devices_[dev_path] = sys_path;
        proxy_->GpuAddGraphicsDevice(sys_path, DrmWrapper::ToScopedFD(std::move(drm)));
...
void DrmDisplayHostManager::OnUpdateGraphicsDevice(const EventPropertyMap& udev_event_props)
{
    proxy_->GpuShouldDisplayEventTriggerConfiguration(udev_event_props);
}

void DrmDisplayHostManager::OnRemoveGraphicsDevice(const base::FilePath& sys_path)
{
...
{
    std::unique_ptr<DrmWrapper> primary_drm_device = std::move(primary_drm_device_);
    {
        base::ScopedAllowBlocking scoped_allow_blocking;
        ...
        if (!primary_drm_device) {
            primary_drm_device = OpenDrmDevice(primary_graphics_card_path_, drm_devices_[primary_graphics_card_path_],
                /*is_primary_device=*/true);
            if (!primary_drm_device) {
    ...
    // state.
    proxy_->GpuAddGraphicsDevice(drm_devices_[primary_graphics_card_path_], DrmWrapper::ToScopedFD(std::move(primary_drm_device)));
}
...
void DrmDisplayHostManager::OnGpuThreadReady()
{
    ...
    if (!get_displays_callback_.is_null()) {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&DrmDisplayHostManager::RunUpdateDisplaysCallback, weak_ptr_factory_.GetWeakPtr(), std::move(get_displays_callback_)));
        get_displays_callback_.Reset();
    }
...
void DrmDisplayHostManager::GpuHasUpdatedNativeDisplays(MovableDisplaySnapshots displays)
{
    if (delegate_)
    ...
    for (auto& display : displays) {
        auto it = base::ranges::find(
            old_displays, display->display_id(), [](const std::unique_ptr<DrmDisplayHost>& display) { return display->snapshot()->display_id(); });
        if (it == old_displays.end()) {
            displays_.push_back(std::make_unique<DrmDisplayHost>(proxy_, std::move(display), false /* is_dummy */));
        } else {
...
void DrmDisplayHostManager::GpuSetHdcpKeyProp(int64_t display_id, bool success)
{
...
void DrmDisplayHostManager::GpuReceivedHDCPState(int64_t display_id, bool status, display::HDCPState state, display::ContentProtectionMethod protection_method)
{
    DrmDisplayHost* display = GetDisplay(display_id);
    if (display)
...
void DrmDisplayHostManager::GpuUpdatedHDCPState(int64_t display_id, bool status)
{
    DrmDisplayHost* display = GetDisplay(display_id);
    if (display)
...
void DrmDisplayHostManager::GpuTookDisplayControl(bool status)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(take_display_control_callback_), status));
...
void DrmDisplayHostManager::GpuRelinquishedDisplayControl(bool status)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(relinquish_display_control_callback_), status));
    relinquish_display_control_callback_.Reset();
    display_control_change_pending_ = false;
}

void DrmDisplayHostManager::GpuShouldDisplayEventTriggerConfiguration(bool should_trigger)
{
    if (should_trigger)
        NotifyDisplayDelegate();
...
void DrmDisplayHostManager::RunUpdateDisplaysCallback(display::GetDisplaysCallback callback) const
{
