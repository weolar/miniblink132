...
DrmWindowHost::DrmWindowHost(PlatformWindowDelegate* delegate, const gfx::Rect& bounds, GpuThreadAdapter* sender, EventFactoryEvdev* event_factory,
    DrmCursor* cursor, DrmWindowHostManager* window_manager, DrmDisplayHostManager* display_manager)
    : delegate_(delegate)
    , sender_(sender)
...
{
    return cursor_confined_bounds_.IsEmpty() ? gfx::Rect(bounds_.size()) : cursor_confined_bounds_;
}
...
void DrmWindowHost::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
    NOTREACHED();
}
...
bool DrmWindowHost::CanDispatchEvent(const PlatformEvent& event)
{
    ...
    if (event->IsTouchEvent()) {
        ...
        // |bounds_|.
        int64_t display_id = DeviceDataManager::GetInstance()->GetTargetDisplayForTouchDevice(event->source_device_id());
        ...
        gfx::Rect display_bounds(snapshot->origin(), snapshot->current_mode()->size());
...
uint32_t DrmWindowHost::DispatchEvent(const PlatformEvent& event)
{
    ...
    if (event->IsLocatedEvent()) {
        ...
        if (event->IsMouseEvent()) {
            DrmWindowHost* window_on_mouse = window_manager_->GetWindowAt(located_event->location());
            if (window_on_mouse)
    ...
    DispatchEventFromNativeUiEvent(event, base::BindOnce(&PlatformWindowDelegate::DispatchEvent, base::Unretained(delegate_)));
