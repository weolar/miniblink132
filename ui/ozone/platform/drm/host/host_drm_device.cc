...
void HostDrmDevice::RegisterHandlerForDrmDisplayHostManager(DrmDisplayHostManager* handler)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
...
bool HostDrmDevice::GpuCreateWindow(gfx::AcceleratedWidget widget, const gfx::Rect& initial_bounds)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    if (!IsConnected())
        return false;
...
bool HostDrmDevice::GpuWindowBoundsChanged(gfx::AcceleratedWidget widget, const gfx::Rect& bounds)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    if (!IsConnected())
        return false;
...
bool HostDrmDevice::GpuRefreshNativeDisplays()
{
    ...
    auto callback = base::BindOnce(&HostDrmDevice::GpuRefreshNativeDisplaysCallback, this);
    ...
    const std::vector<display::DisplayConfigurationParams>& config_requests, display::ConfigureCallback callback, display::ModesetFlags modeset_flags)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    if (IsConnected()) {
        drm_device_->ConfigureNativeDisplays(config_requests, modeset_flags, std::move(callback));
    } else {
        ...
        base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), config_requests, false));
    }
}
...
bool HostDrmDevice::GpuTakeDisplayControl()
{
    ...
    auto callback = base::BindOnce(&HostDrmDevice::GpuTakeDisplayControlCallback, this);
...
bool HostDrmDevice::GpuRelinquishDisplayControl()
{
    ...
    auto callback = base::BindOnce(&HostDrmDevice::GpuRelinquishDisplayControlCallback, this);
...
void HostDrmDevice::GpuAddGraphicsDevice(const base::FilePath& path, base::ScopedFD fd)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
...
void HostDrmDevice::GpuShouldDisplayEventTriggerConfiguration(const EventPropertyMap& event_props)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    ...
    auto callback = base::BindOnce(&HostDrmDevice::GpuShouldDisplayEventTriggerConfigurationCallback, this);
    drm_device_->ShouldDisplayEventTriggerConfiguration(event_props, std::move(callback));
}

bool HostDrmDevice::GpuSetHdcpKeyProp(int64_t display_id, const std::string& key)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    if (!IsConnected()) {
        return false;
    ...
    auto callback = base::BindOnce(&HostDrmDevice::GpuSetHdcpKeyPropCallback, this);
...
bool HostDrmDevice::GpuSetHDCPState(int64_t display_id, display::HDCPState state, display::ContentProtectionMethod protection_method)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    if (!IsConnected())
        return false;
    ...
    drm_device_->SetHDCPState(display_id, state, protection_method, std::move(callback));

    return true;
}

void HostDrmDevice::GpuSetColorTemperatureAdjustment(int64_t display_id, const display::ColorTemperatureAdjustment& cta)
{
    if (!IsConnected()) {
        return;
    }
...
void HostDrmDevice::GpuSetColorCalibration(int64_t display_id, const display::ColorCalibration& calibration)
{
    if (!IsConnected()) {
        return;
    }
...
void HostDrmDevice::GpuSetGammaAdjustment(int64_t display_id, const display::GammaAdjustment& adjustment)
{
    if (!IsConnected()) {
        return;
    }
...
void HostDrmDevice::GpuSetPrivacyScreen(int64_t display_id, bool enabled, display::SetPrivacyScreenCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    if (IsConnected()) {
...
void HostDrmDevice::GpuGetSeamlessRefreshRates(int64_t display_id, display::GetSeamlessRefreshRatesCallback callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    if (!IsConnected()) {
...
void HostDrmDevice::GpuRefreshNativeDisplaysCallback(MovableDisplaySnapshots displays) const
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
...
void HostDrmDevice::GpuShouldDisplayEventTriggerConfigurationCallback(bool should_trigger) const
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    display_manager_->GpuShouldDisplayEventTriggerConfiguration(should_trigger);
}

void HostDrmDevice::GpuSetHdcpKeyPropCallback(int64_t display_id, bool success) const
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
...
void HostDrmDevice::GpuGetHDCPStateCallback(
    int64_t display_id, bool success, display::HDCPState state, display::ContentProtectionMethod protection_method) const
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    display_manager_->GpuReceivedHDCPState(display_id, success, state, protection_method);
}

void HostDrmDevice::GpuSetHDCPStateCallback(int64_t display_id, bool success) const
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    display_manager_->GpuUpdatedHDCPState(display_id, success);
}

void HostDrmDevice::OnGpuServiceLaunched(mojo::PendingRemote<ui::ozone::mojom::DrmDevice> drm_device)
{
    DCHECK_CALLED_ON_VALID_THREAD(on_ui_thread_);
    ...
    // IO thread.
    mojo::PendingAssociatedRemote<ui::ozone::mojom::DeviceCursor> cursor_ui, cursor_io;
    ...
    // GpuProcessHost.
    cursor_proxy_ = std::make_unique<HostCursorProxy>(std::move(cursor_ui), std::move(cursor_io));
