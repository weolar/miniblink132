...
public:
    DrmDisplayHost(GpuThreadAdapter* sender, std::unique_ptr<display::DisplaySnapshot> params, bool is_dummy);
    ...
    display::DisplaySnapshot* snapshot() const
    {
    ...
    void SetHdcpKeyProp(const std::string& key, display::SetHdcpKeyPropCallback callback);
    void GetHDCPState(display::GetHDCPStateCallback callback);
    void SetHDCPState(display::HDCPState state, display::ContentProtectionMethod protection_method, display::SetHDCPStateCallback callback);
    ...
    void SetPrivacyScreen(bool enabled, display::SetPrivacyScreenCallback callback);
    void GetSeamlessRefreshRates(display::GetSeamlessRefreshRatesCallback callback) const;

    // Called when the IPC from the GPU process arrives to answer the above
    // commands.
    void OnHdcpKeyPropSetReceived(bool success);
    void OnHDCPStateReceived(bool status, display::HDCPState state, display::ContentProtectionMethod protection_method);
