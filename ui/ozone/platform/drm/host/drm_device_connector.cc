...
DrmDeviceConnector::DrmDeviceConnector(scoped_refptr<HostDrmDevice> host_drm_device)
...
void DrmDeviceConnector::OnGpuServiceLaunched(int host_id, GpuHostBindInterfaceCallback binder, GpuHostTerminateCallback terminate_callback)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&HostDrmDevice::OnGpuServiceLaunched, host_drm_device_, std::move(drm_device)));
}

void DrmDeviceConnector::BindInterfaceDrmDevice(mojo::PendingRemote<ui::ozone::mojom::DrmDevice>* drm_device) const
{
    binder_callback_.Run(ui::ozone::mojom::DrmDevice::Name_, drm_device->InitWithNewPipeAndPassReceiver().PassPipe());
}

void DrmDeviceConnector::ConnectSingleThreaded(mojo::PendingRemote<ui::ozone::mojom::DrmDevice> drm_device)
{
