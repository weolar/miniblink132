...
void WaylandGLEGLUtility::GetAdditionalEGLAttributes(EGLenum platform_type, std::vector<EGLAttrib>* display_attributes)
{
    ...
    if (std::find(display_attributes->begin(), display_attributes->end(), EGL_PLATFORM_ANGLE_TYPE_OPENGL_ANGLE) != display_attributes->end()
        || std::find(display_attributes->begin(), display_attributes->end(), EGL_PLATFORM_ANGLE_TYPE_OPENGLES_ANGLE) != display_attributes->end()) {
...
void WaylandGLEGLUtility::ChooseEGLAlphaAndBufferSize(EGLint* alpha_size, EGLint* buffer_size)
{
}

void WaylandGLEGLUtility::CollectGpuExtraInfo(bool enable_native_gpu_memory_buffers, gfx::GpuExtraInfo& gpu_extra_info) const
{
}
