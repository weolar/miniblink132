...
GLSurfaceEglReadbackWayland::PixelBuffer::PixelBuffer(base::WritableSharedMemoryMapping shm_mapping, uint32_t buffer_id)
    : shm_mapping_(std::move(shm_mapping))
...
GLSurfaceEglReadbackWayland::GLSurfaceEglReadbackWayland(gl::GLDisplayEGL* display, gfx::AcceleratedWidget widget, WaylandBufferManagerGpu* buffer_manager)
...
bool GLSurfaceEglReadbackWayland::Resize(const gfx::Size& size, float scale_factor, const gfx::ColorSpace& color_space, bool has_alpha)
{
    DestroyBuffers();
    ...
    for (size_t i = 0; i < kMaxBuffers; ++i) {
        ...
        base::UnsafeSharedMemoryRegion shm_region = base::UnsafeSharedMemoryRegion::Create(checked_length.ValueOrDie());
        ...
        available_buffers_.push_back(std::make_unique<PixelBuffer>(std::move(shm_mapping), buffer_id));

        buffer_manager_->CreateShmBasedBuffer(std::move(fd_pair.fd), checked_length.ValueOrDie(), size, buffer_id);
    }
...
gfx::SwapResult GLSurfaceEglReadbackWayland::SwapBuffers(PresentationCallback callback, gfx::FrameData data)
{
    NOTREACHED();
}

void GLSurfaceEglReadbackWayland::SwapBuffersAsync(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, gfx::FrameData data)
{
    ...
    buffer_manager_->CommitBuffer(widget_, next_buffer->buffer_id_,
        /*frame_id*/ next_buffer->buffer_id_, data, bounds, enable_blend_for_shadow, gfx::RoundedCornersF(), surface_scale_factor_, bounds);
}
...
void GLSurfaceEglReadbackWayland::OnSubmission(uint32_t frame_id, const gfx::SwapResult& swap_result, gfx::GpuFenceHandle release_fence)
{
    ...
    std::move(completion_callbacks_.front()).Run(gfx::SwapCompletionResult(swap_result));
    completion_callbacks_.erase(completion_callbacks_.begin());
}

void GLSurfaceEglReadbackWayland::OnPresentation(uint32_t frame_id, const gfx::PresentationFeedback& feedback)
{
