...
public:
    ...
    sk_sp<SkSurface> sk_surface() const
    {
        return sk_surface_;
    }
    ...
    void Initialize(const gfx::Size& size)
    {
        ...
        // bytes per pixel.
        size_t size_in_bytes = viz::ResourceSizes::CheckedSizeInBytes<size_t>(size, viz::SinglePlaneFormat::kRGBA_8888);

        base::UnsafeSharedMemoryRegion shm_region = base::UnsafeSharedMemoryRegion::Create(size_in_bytes);
        ...
        base::subtle::PlatformSharedMemoryRegion platform_shm = base::UnsafeSharedMemoryRegion::TakeHandleForSerialization(std::move(shm_region));
        base::subtle::ScopedFDPair fd_pair = platform_shm.PassPlatformHandle();
        buffer_manager_->CreateShmBasedBuffer(std::move(fd_pair.fd), size_in_bytes, size, buffer_id_);

        SkSurfaceProps props = skia::LegacyDisplayGlobals::GetSkSurfaceProps();
        sk_surface_
            = SkSurfaces::WrapPixels(SkImageInfo::MakeN32Premul(size.width(), size.height()), shm_mapping_.memory(), CalculateStride(size.width()), &props);
        DCHECK(sk_surface_);
    ...
    void CopyDirtyRegionFrom(SharedMemoryBuffer* buffer)
    {
        ...
        for (SkRegion::Iterator i(dirty_region_); !i.done(); i.next()) {
            auto offset = i.rect().x() * SkColorTypeBytesPerPixel(kN32_SkColorType) + i.rect().y() * stride;
            auto dst_subspan = dst_span.subspan(offset, i.rect().width() * i.rect().height() * SkColorTypeBytesPerPixel(kN32_SkColorType));

            UNSAFE_TODO(buffer->sk_surface_->readPixels(
                SkImageInfo::MakeN32Premul(i.rect().width(), i.rect().height()), dst_subspan.data(), stride, i.rect().x(), i.rect().y()));
        }
...
WaylandCanvasSurface::PendingFrame::PendingFrame(
    uint32_t frame_id, const gfx::Size& surface_size, SwapBuffersCallback callback, gfx::FrameData frame_data, SharedMemoryBuffer* frame_buffer)
    : frame_id(frame_id)
...
WaylandCanvasSurface::PendingFrame::~PendingFrame()
{
    if (swap_ack_callback) {
        ...
        // the draw start time).
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(swap_ack_callback), surface_size));
    }
}

WaylandCanvasSurface::WaylandCanvasSurface(WaylandBufferManagerGpu* buffer_manager, gfx::AcceleratedWidget widget)
    : buffer_manager_(buffer_manager)
    , widget_(widget)
{
...
{
    DCHECK(!pending_buffer_) << "The previous pending buffer has not been presented yet";
...
void WaylandCanvasSurface::ResizeCanvas(const gfx::Size& viewport_size, float scale)
{
...
void WaylandCanvasSurface::OnSwapBuffers(SwapBuffersCallback swap_ack_callback, gfx::FrameData data)
{
    unsubmitted_frames_.push_back(std::make_unique<PendingFrame>(next_frame_id(), size_, std::move(swap_ack_callback), std::move(data), pending_buffer_));
    pending_buffer_ = nullptr;

    MaybeProcessUnsubmittedFrames();
}

std::unique_ptr<gfx::VSyncProvider> WaylandCanvasSurface::CreateVSyncProvider()
{
    return std::make_unique<WaylandCanvasSurface::VSyncProvider>(weak_factory_.GetWeakPtr());
}
...
void WaylandCanvasSurface::MaybeProcessUnsubmittedFrames()
{
    ...
    buffer_manager_->CommitBuffer(widget_, frame->frame_id, frame_buffer->buffer_id(), std::move(frame->data), gfx::Rect(size_), enable_blend_for_shadow,
        gfx::RoundedCornersF(), viewport_scale_, damage);

    submitted_frame_ = std::move((frame));
}

void WaylandCanvasSurface::OnSubmission(uint32_t frame_id, const gfx::SwapResult& swap_result, gfx::GpuFenceHandle release_fence)
{
...
void WaylandCanvasSurface::OnPresentation(uint32_t frame_id, const gfx::PresentationFeedback& feedback)
{
...
std::unique_ptr<WaylandCanvasSurface::SharedMemoryBuffer> WaylandCanvasSurface::CreateSharedMemoryBuffer()
{
