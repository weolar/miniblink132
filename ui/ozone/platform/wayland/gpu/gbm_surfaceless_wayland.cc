...
GbmSurfacelessWayland::SolidColorBufferHolder::SolidColorBufferHolder() = default;
GbmSurfacelessWayland::SolidColorBufferHolder::~SolidColorBufferHolder() = default;

BufferId GbmSurfacelessWayland::SolidColorBufferHolder::GetOrCreateSolidColorBuffer(SkColor4f color, WaylandBufferManagerGpu* buffer_manager)
{
    BufferId next_buffer_id = 0;

    // First try for an existing buffer.
    auto it = base::ranges::find(available_solid_color_buffers_, color, &SolidColorBuffer::color);
    if (it != available_solid_color_buffers_.end()) {
        ...
        if (buffer_manager->supports_non_backed_solid_color_buffers()) {
            buffer_manager->CreateSolidColorBuffer(color, kSolidColorBufferSize, next_buffer_id);
        } else {
        ...
        inflight_solid_color_buffers_.emplace_back(SolidColorBuffer(color, next_buffer_id));
    }
    DCHECK_GT(next_buffer_id, 0u);
    return next_buffer_id;
}

void GbmSurfacelessWayland::SolidColorBufferHolder::OnSubmission(BufferId buffer_id, WaylandBufferManagerGpu* buffer_manager)
{
    ...
    auto it = base::ranges::find(inflight_solid_color_buffers_, buffer_id, &SolidColorBuffer::buffer_id);
    if (it != inflight_solid_color_buffers_.end()) {
        ...
        while (available_solid_color_buffers_.size() > kMaxSolidColorBuffers) {
            buffer_manager->DestroyBuffer(available_solid_color_buffers_.begin()->buffer_id);
            available_solid_color_buffers_.erase(available_solid_color_buffers_.begin());
        }
    }
}

void GbmSurfacelessWayland::SolidColorBufferHolder::EraseBuffers(WaylandBufferManagerGpu* buffer_manager)
{
...
GbmSurfacelessWayland::GbmSurfacelessWayland(gl::GLDisplayEGL* display, WaylandBufferManagerGpu* buffer_manager, gfx::AcceleratedWidget widget)
    ...
    , weak_factory_(this)
{
    ...
    unsubmitted_frames_.push_back(std::make_unique<PendingFrame>(next_frame_id()));
}

void GbmSurfacelessWayland::QueueWaylandOverlayConfig(wl::WaylandOverlayConfig config)
{
    auto* frame = unsubmitted_frames_.back().get();
    DCHECK(frame);
    TRACE_EVENT("wayland", "GbmSurfacelessWayland::QueueWaylandOverlayConfig", "frame_id", frame->frame_id, "buffer_id", config.buffer_id);
    frame->configs.emplace_back(std::move(config));
}

bool GbmSurfacelessWayland::ScheduleOverlayPlane(
    gl::OverlayImage image, std::unique_ptr<gfx::GpuFence> gpu_fence, const gfx::OverlayPlaneData& overlay_plane_data)
{
    auto* frame = unsubmitted_frames_.back().get();
    ...
    if (!image) {
        ...
        BufferId buf_id = solid_color_buffers_holder_->GetOrCreateSolidColorBuffer(overlay_plane_data.color.value(), buffer_manager_);
        // Invalid buffer id.
        if (buf_id == 0) {
            frame->schedule_planes_succeeded = false;
            return false;
        ...
        QueueWaylandOverlayConfig({ overlay_plane_data, nullptr, buf_id, surface_scale_factor() });
    } else {
        std::vector<gfx::GpuFence> acquire_fences;
        if (gpu_fence && (buffer_manager_->supports_acquire_fence() || use_egl_fence_sync_)) {
            acquire_fences.push_back(std::move(*gpu_fence));
        }

        frame->schedule_planes_succeeded = image->ScheduleOverlayPlane(widget_, overlay_plane_data, std::move(acquire_fences), {});
    }
    return frame->schedule_planes_succeeded;
}
void GbmSurfacelessWayland::Present(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, gfx::FrameData data)
{
    ...
    if (buffer_manager_->supports_acquire_fence() || !use_egl_fence_sync_ || !frame->schedule_planes_succeeded) {
        frame->ready = true;
        MaybeSubmitFrames();
        return;
    }
    ...
    for (auto& config : frame->configs) {
        if (!config.access_fence_handle.is_null()) {
            fences.push_back(std::make_unique<gfx::GpuFence>(std::move(config.access_fence_handle)));
    ...
    base::OnceClosure fence_retired_callback = base::BindOnce(&GbmSurfacelessWayland::FenceRetired, weak_factory_.GetWeakPtr(), frame);

    base::ThreadPool::PostTaskAndReply(
        FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, std::move(fence_wait_task), std::move(fence_retired_callback));
}
...
bool GbmSurfacelessWayland::Resize(const gfx::Size& size, float scale_factor, const gfx::ColorSpace& color_space, bool has_alpha)
{
...
void GbmSurfacelessWayland::MaybeSubmitFrames()
{
    while (!unsubmitted_frames_.empty() && unsubmitted_frames_.front()->ready) {
        ...
        if (!submitted_frame->schedule_planes_succeeded) {
            ...
            std::move(submitted_frame->completion_callback).Run(gfx::SwapCompletionResult(gfx::SwapResult::SWAP_FAILED));
            // Notify the caller, the buffer is never presented on a screen.
            std::move(submitted_frame->presentation_callback).Run(gfx::PresentationFeedback::Failure());

            submitted_frame.reset();
            return;
        }

        buffer_manager_->CommitOverlays(widget_, submitted_frame->frame_id, submitted_frame->data, std::move(submitted_frame->configs));
...
void GbmSurfacelessWayland::OnSubmission(uint32_t frame_id, const gfx::SwapResult& swap_result, gfx::GpuFenceHandle release_fence)
{
    // If the frame_id is stale, the gpu process just recovered from a crash so
    // this frame_id can be ignored.
    if (submitted_frames_.empty() || submitted_frames_.front()->frame_id != frame_id) {
        return;
    }

    auto submitted_frame = std::move(submitted_frames_.front());

    TRACE_EVENT("wayland", "GbmSurfacelessWayland::OnSubmission", "frame_id", submitted_frame->frame_id);
    ...
    if (!release_fence.is_null()) {
        ...
        auto status = gfx::GpuFence::GetStatusChangeTime(release_fence.Peek(), &ticks);
        if (status == gfx::GpuFence::kSignaled)
            release_fence = {};
    }

    std::move(submitted_frame->completion_callback).Run(gfx::SwapCompletionResult(swap_result, std::move(release_fence)));

    pending_presentation_frames_.push_back(std::move(submitted_frame));

    if (swap_result != gfx::SwapResult::SWAP_ACK && swap_result != gfx::SwapResult::SWAP_NAK_RECREATE_BUFFERS) {
        last_swap_buffers_result_ = false;
        return;
    }
...
void GbmSurfacelessWayland::OnPresentation(uint32_t frame_id, const gfx::PresentationFeedback& feedback)
{
    if (pending_presentation_frames_.empty() || pending_presentation_frames_.front()->frame_id != frame_id) {
        return;
    }

    std::move(pending_presentation_frames_.front()->presentation_callback).Run(feedback);
