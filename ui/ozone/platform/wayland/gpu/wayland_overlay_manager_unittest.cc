...
public:
    ...
    WaylandOverlayManagerTest& operator=(const WaylandOverlayManagerTest&) = delete;

    ~WaylandOverlayManagerTest() override = default;

    void SetUp() override
    {
        const base::flat_map<gfx::BufferFormat, std::vector<uint64_t>> kSupportedFormatsWithModifiers { { gfx::BufferFormat::YUV_420_BIPLANAR,
...
TEST_P(WaylandOverlayManagerTest, MultipleOverlayCandidates)
{
    ...
    std::vector<OverlaySurfaceCandidate> candidates = { CreateCandidate(gfx::Rect(10, 10, 20, 20), -2), CreateCandidate(gfx::Rect(30, 30, 10, 10), -1),
        CreateCandidate(gfx::Rect(0, 0, 100, 100), 0), CreateCandidate(gfx::Rect(40, 40, 20, 20), 1), CreateCandidate(gfx::Rect(60, 60, 20, 20), 2) };
...
void NonIntegerDisplayRectTestHelper(WaylandBufferManagerGpu* manager_gpu, bool is_context_delegated, bool expect_candidates_handled)
{
...
{
    constexpr std::array<std::array<bool, 2>, 2> test_data = { { { false, false }, { true, false } } };
    for (const auto& data : test_data) {
        NonIntegerDisplayRectTestHelper(buffer_manager_gpu_.get(), data[0] /* is_delegated_context */, data[1] /* expect_candidates_handled */);
    }
}
