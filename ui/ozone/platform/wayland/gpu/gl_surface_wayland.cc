...
std::unique_ptr<wl_egl_window, EGLWindowDeleter> CreateWaylandEglWindow(WaylandWindow* window)
{
...
GLSurfaceWayland::GLSurfaceWayland(gl::GLDisplayEGL* display, WaylandEglWindowPtr egl_window, WaylandWindow* window)
    : NativeViewGLSurfaceEGL(display, reinterpret_cast<EGLNativeWindowType>(egl_window.get()), nullptr)
...
bool GLSurfaceWayland::Resize(const gfx::Size& size, float scale_factor, const gfx::ColorSpace& color_space, bool has_alpha)
{
    if (size_ == size) {
        return true;
    }
...
{
    if (!config_) {
        EGLint config_attribs[] = { EGL_BUFFER_SIZE, 32, EGL_ALPHA_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_RED_SIZE, 8, EGL_RENDERABLE_TYPE,
...
gfx::SwapResult GLSurfaceWayland::SwapBuffers(PresentationCallback callback, gfx::FrameData data)
{
    ...
    if (!window_->IsSurfaceConfigured()) {
        ...
        gl::GLSurfacePresentationHelper::ScopedSwapBuffers scoped_swap_buffers(presentation_helper(), std::move(callback));
...
gfx::SwapResult GLSurfaceWayland::PostSubBuffer(int x, int y, int width, int height, PresentationCallback callback, gfx::FrameData data)
{
    OnSequencePoint(data.seq);
    if (!window_->IsSurfaceConfigured()) {
        // The presentation |callback| must be called after gfx::SwapResult is sent.
    ...
    window_->root_surface()->set_surface_buffer_scale(scale_factor_);
    return gl::NativeViewGLSurfaceEGL::PostSubBuffer(x, y, width, height, std::move(callback), data);
}
...
{
    window_->ui_task_runner()->PostTask(FROM_HERE, base::BindOnce(&WaylandWindow::OnSequencePoint, window_->AsWeakPtr(), seq));
}
