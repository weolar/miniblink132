...
void NotifyOverlayDelegationLimitedCapabilityOnce()
{
    ...
    if (!logged_once) {
        DLOG(ERROR) << "Subpixel accurate position is not available. Only some quads "
...
WaylandOverlayManager::WaylandOverlayManager(WaylandBufferManagerGpu* manager_gpu)
    : manager_gpu_(manager_gpu)
{
}
WaylandOverlayManager::~WaylandOverlayManager() = default;

std::unique_ptr<OverlayCandidatesOzone> WaylandOverlayManager::CreateOverlayCandidates(gfx::AcceleratedWidget widget)
{
...
void WaylandOverlayManager::CheckOverlaySupport(std::vector<OverlaySurfaceCandidate>* candidates, gfx::AcceleratedWidget widget)
{
...
bool WaylandOverlayManager::CanHandleCandidate(const OverlaySurfaceCandidate& candidate, gfx::AcceleratedWidget widget) const
{
    ...
    constexpr auto kMaxWaylandRect = gfx::RectF(-kMaxWaylandFixed, -kMaxWaylandFixed, kMaxWaylandFixed * 2, kMaxWaylandFixed * 2);
    ...
    if (wl_fixed_from_double(candidate.display_rect.width() / kAssumedMaxDeviceScaleFactor) == 0
        || wl_fixed_from_double(candidate.display_rect.height() / kAssumedMaxDeviceScaleFactor) == 0)
        return false;

    if (absl::holds_alternative<gfx::OverlayTransform>(candidate.transform)) {
        if (absl::get<gfx::OverlayTransform>(candidate.transform) == gfx::OVERLAY_TRANSFORM_INVALID) {
            return false;
        }
    } else if (!manager_gpu_->supports_affine_transform() || absl::get<gfx::Transform>(candidate.transform).HasPerspective()) {
        // Wayland supports only 2d matrix transforms.
        return false;
    }

    if (candidate.background_color.has_value() && !manager_gpu_->supports_surface_background_color()) {
        return false;
    }

    // If clipping isn't supported, reject candidates with a clip rect, unless
    // that clip wouldn't have any effect.
    if (!manager_gpu_->supports_clip_rect() && candidate.clip_rect && !candidate.clip_rect->Contains(gfx::ToNearestRect(candidate.display_rect))) {
        return false;
    }
