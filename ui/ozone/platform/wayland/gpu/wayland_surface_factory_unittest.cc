...
public:
    OverlayImageHolder(scoped_refptr<gfx::NativePixmap> pixmap, const gfx::Size& size)
        : pixmap_(pixmap)
    {
    }
    ...
    uint32_t GetAssociateWithSwapId()
    {
        return swap_id_;
    }
...
public:
    ...
    uint32_t GetLastFinishedSwapId() const
    {
        return last_finish_swap_id_;
    }
    ...
    // simpler way.
    void FinishPresent(uint32_t local_swap_id, std::vector<scoped_refptr<OverlayImageHolder>> overlay_images, gfx::SwapCompletionResult result)
    {
    ...
    void BufferPresented(uint64_t local_swap_id, const gfx::PresentationFeedback& feedback)
    {
        ...
        ASSERT_TRUE(pending_local_swap_ids_.empty() || pending_local_swap_ids_.front() > local_swap_id);
    }

    // Corresponds to SoftwareOutputDevice::SwapBuffersCallback so that it can be
    // used with canvas surfaces.
    void CanvasSwapBuffersCallback(uint64_t local_swap_id, const gfx::Size& pixel_size)
    {
        last_finish_swap_id_ = pending_local_swap_ids_.front();
        pending_local_swap_ids_.pop();
...
public:
    ...
    WaylandSurfaceFactoryTest& operator=(const WaylandSurfaceFactoryTest&) = delete;
    ~WaylandSurfaceFactoryTest() override = default;

    void SetUp() override
    {
        const base::flat_map<gfx::BufferFormat, std::vector<uint64_t>> kSupportedFormatsWithModifiers { { gfx::BufferFormat::BGRA_8888,
            { DRM_FORMAT_MOD_LINEAR } } };

        WaylandTest::SetUp();

        auto manager_ptr = connection_->buffer_manager_host()->BindInterface();
        buffer_manager_gpu_->Initialize(std::move(manager_ptr), kSupportedFormatsWithModifiers,
...
protected:
    std::unique_ptr<SurfaceOzoneCanvas> CreateCanvas(gfx::AcceleratedWidget widget)
    {
    ...
    void ScheduleOverlayPlane(gl::Presenter* presenter, gl::OverlayImage image, int z_order)
    {
        presenter->ScheduleOverlayPlane(image, nullptr,
            gfx::OverlayPlaneData(z_order, gfx::OverlayTransform::OVERLAY_TRANSFORM_NONE, gfx::RectF(window_->GetBoundsInPixels()), {}, false,
                gfx::Rect(window_->applied_state().size_px), 1.0f, gfx::OverlayPriorityHint::kNone, gfx::RRectF(), gfx::ColorSpace::CreateSRGB(),
                std::nullopt));
    }
...
TEST_P(WaylandSurfaceFactoryTest, GbmSurfacelessWaylandCommitOverlaysCallbacksTest)
{
    ...
    auto* gl_ozone = surface_factory_->GetGLOzone(gl::GLImplementationParts(gl::kGLImplementationEGLGLES2));
    auto presenter = gl_ozone->CreateSurfacelessViewGLSurface(gl::GetDefaultDisplay(), widget_);
    EXPECT_TRUE(presenter);
    presenter->SetRelyOnImplicitSync();
    static_cast<ui::GbmSurfacelessWayland*>(presenter.get())->SetNoGLFlushForTests();
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        auto native_pixmap = surface_factory_->CreateNativePixmap(widget_, nullptr, size_px, gfx::BufferFormat::BGRA_8888, gfx::BufferUsage::SCANOUT);
        fake_overlay_image.push_back(base::MakeRefCounted<OverlayImageHolder>(native_pixmap, size_px));
    }
    ...
    // Submit a frame with an overlay and background.
    {
        ...
        // Prepare background.
        ScheduleOverlayPlane(presenter.get(), fake_overlay_image[0]->GetNativePixmap(),
            /*z_order=*/INT32_MIN);
        ...
        ScheduleOverlayPlane(presenter.get(), fake_overlay_image[1]->GetNativePixmap(),
            /*z_order=*/1);
        ...
        presenter->Present(base::BindOnce(&CallbacksHelper::FinishPresent, base::Unretained(&cbs_helper), swap_id, overlay_images),
            base::BindOnce(&CallbacksHelper::BufferPresented, base::Unretained(&cbs_helper), swap_id), gfx::FrameData());
    }
    ...
    const uint32_t primary_subsurface_id = window_->primary_subsurface()->wayland_surface()->get_surface_id();
    PostToServerAndWait([main_surface_id = surface_id_, primary_subsurface_id](wl::TestWaylandServerThread* server) {
        auto* root_surface = server->GetObject<wl::MockSurface>(main_surface_id);
        auto* mock_primary_surface = server->GetObject<wl::MockSurface>(primary_subsurface_id);
    ...
    PostToServerAndWait([main_surface_id = surface_id_, primary_subsurface_id](wl::TestWaylandServerThread* server) {
        testing::Mock::VerifyAndClearExpectations(server->GetObject<wl::MockSurface>(primary_subsurface_id));
        testing::Mock::VerifyAndClearExpectations(server->GetObject<wl::MockSurface>(main_surface_id));
    });
    ...
    // Submit another frame with only an overlay.
    {
        ...
        // Prepare overlay plane.
        ScheduleOverlayPlane(presenter.get(), fake_overlay_image[2]->GetNativePixmap(),
            /*z_order=*/1);
    ...
    const uint32_t overlay_surface_id = (*window_->wayland_subsurfaces().begin())->wayland_surface()->get_surface_id();
    // The fake server must have dmabuf params created. Set expectations and
    // notify the client about created buffers.
    PostToServerAndWait([overlay_surface_id, main_surface_id = surface_id_, primary_subsurface_id](wl::TestWaylandServerThread* server) {
        auto* mock_overlay_surface = server->GetObject<wl::MockSurface>(overlay_surface_id);
        auto* root_surface = server->GetObject<wl::MockSurface>(main_surface_id);
        auto* mock_primary_surface = server->GetObject<wl::MockSurface>(primary_subsurface_id);
    ...
    PostToServerAndWait([overlay_surface_id, main_surface_id = surface_id_, primary_subsurface_id](wl::TestWaylandServerThread* server) {
        testing::Mock::VerifyAndClearExpectations(server->GetObject<wl::MockSurface>(primary_subsurface_id));
        testing::Mock::VerifyAndClearExpectations(server->GetObject<wl::MockSurface>(overlay_surface_id));
        testing::Mock::VerifyAndClearExpectations(server->GetObject<wl::MockSurface>(main_surface_id));
    });
    ...
    EXPECT_EQ(cbs_helper.GetLastFinishedSwapId(), std::numeric_limits<uint32_t>::max());
    ...
    // Submit another frame with 0 overlays, 1 primary plane.
    {
        ...
        ScheduleOverlayPlane(presenter.get(), fake_overlay_image[3]->GetNativePixmap(),
            /*z_order=*/0);
    ...
    // and swapped.
    PostToServerAndWait([overlay_surface_id](wl::TestWaylandServerThread* server) {
    ...
    PostToServerAndWait([overlay_surface_id, main_surface_id = surface_id_, primary_subsurface_id](wl::TestWaylandServerThread* server) {
        ...
        // id=1u.
        mock_overlay_surface->ReleaseBuffer(mock_overlay_surface->prev_attached_buffer());
    });
...
TEST_P(WaylandSurfaceFactoryTest, GbmSurfacelessWaylandGroupOnSubmissionCallbacksTest)
{
    if (!connection_->ShouldUseOverlayDelegation()) {
        GTEST_SKIP();
    }
    ...
    PostToServerAndWait([main_surface_id = surface_id_, primary_subsurface_id](wl::TestWaylandServerThread* server) {
        ...
        for (wl::TestZwpLinuxBufferParamsV1* param : params_vector) {
            zwp_linux_buffer_params_v1_send_created(param->resource(), param->buffer_resource());
        }
    });
    ...
    const uint32_t overlay_surface_id = subsurface->wayland_surface()->get_surface_id();
    ...
    // Submit another frame with 1 primary plane and 1 overlay
    {
        ...
        // Prepare overlay plane.
        ScheduleOverlayPlane(presenter.get(), fake_overlay_image[4]->GetNativePixmap(),
            /*z_order=*/1);
    ...
    PostToServerAndWait([main_surface_id = surface_id_, overlay_surface_id, primary_subsurface_id](wl::TestWaylandServerThread* server) {
        auto* root_surface = server->GetObject<wl::MockSurface>(main_surface_id);
        auto* mock_primary_surface = server->GetObject<wl::MockSurface>(primary_subsurface_id);
        auto* mock_overlay_surface = server->GetObject<wl::MockSurface>(overlay_surface_id);
    ...
    PostToServerAndWait([main_surface_id = surface_id_](wl::TestWaylandServerThread* server) {
        auto* root_surface = server->GetObject<wl::MockSurface>(main_surface_id);
        // Send the frame callback so that pending buffer for swap id=1u is
        // processed and swapped.
        root_surface->SendFrameCallback();
    ...
    PostToServerAndWait([primary_subsurface_id](wl::TestWaylandServerThread* server) {
        testing::Mock::VerifyAndClearExpectations(server->GetObject<wl::MockSurface>(primary_subsurface_id));
    });
    ...
    PostToServerAndWait([primary_subsurface_id](wl::TestWaylandServerThread* server) {
        ...
        mock_primary_surface->ReleaseBuffer(mock_primary_surface->prev_attached_buffer());
    });
...
TEST_P(WaylandSurfaceFactoryTest, Canvas)
{
    ...
    for (auto scale_factor : scale_factors) {
        ...
        const gfx::Rect expected_damage = ScaleToEnclosingRect(gfx::Rect(5, 10, 20, 15), 1.f / std::ceil(scale_factor));

        const uint32_t surface_id = window_->root_surface()->get_surface_id();
        PostToServerAndWait([surface_id, expected_damage](wl::TestWaylandServerThread* server) {
            auto* mock_surface = server->GetObject<wl::MockSurface>(surface_id);
            ASSERT_FALSE(mock_surface->attached_buffer());
            Expectation damage
                = EXPECT_CALL(*mock_surface, Damage(expected_damage.x(), expected_damage.y(), expected_damage.width(), expected_damage.height())).Times(1);
        ...
        PostToServerAndWait([surface_id, bounds_px](wl::TestWaylandServerThread* server) {
            auto* mock_surface = server->GetObject<wl::MockSurface>(surface_id);
...
TEST_P(WaylandSurfaceFactoryTest, CanvasResize)
{
    ...
    PostToServerAndWait([surface_id](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(surface_id);
        Expectation damage = EXPECT_CALL(*mock_surface, Damage(0, 0, 100, 50)).Times(1);
        Expectation attach = EXPECT_CALL(*mock_surface, Attach(_, 0, 0)).Times(1);
        EXPECT_CALL(*mock_surface, Commit()).After(damage, attach);
    });
...
TEST_P(WaylandSurfaceFactoryTest, CanvasBufferSwapAck)
{
    ...
    // Send the first buffer. OnSubmission must be received immediately.
    {
        ...
        canvas->OnSwapBuffers(
            base::BindOnce(&CallbacksHelper::CanvasSwapBuffersCallback, base::Unretained(&cbs_helper), cbs_helper.GetNextLocalSwapId()), gfx::FrameData());
...
TEST_P(WaylandSurfaceFactoryTest, CanvasBufferSwapAck2)
{
    ...
    // another for the invalid frame.
    {
        ...
        canvas->OnSwapBuffers(
            base::BindOnce(&CallbacksHelper::CanvasSwapBuffersCallback, base::Unretained(&cbs_helper), second_buffer_swap_id), gfx::FrameData());
        ...
        canvas->OnSwapBuffers(
            base::BindOnce(&CallbacksHelper::CanvasSwapBuffersCallback, base::Unretained(&cbs_helper), invalid_frame_swap_id), gfx::FrameData());
        ...
        canvas->OnSwapBuffers(
            base::BindOnce(&CallbacksHelper::CanvasSwapBuffersCallback, base::Unretained(&cbs_helper), invalid_frame_swap_id2), gfx::FrameData());
...
TEST_P(WaylandSurfaceFactoryTest, CreateSurfaceCheckGbm)
{
    ...
    EXPECT_FALSE(buffer_manager_gpu_->GetGbmDevice());
    presenter = gl_ozone->CreateSurfacelessViewGLSurface(gl::GetDefaultDisplay(), widget_);
    EXPECT_FALSE(presenter);
...
TEST_P(WaylandSurfaceFactoryCompositorV3, SurfaceDamageTest)
{
    ...
    gfx::Rect expected_src = gfx::ToEnclosingRect(gfx::ScaleRect({ 0.2f, 0.4f, 0.5f, 0.5f }, test_buffer_size.height(), test_buffer_size.width()));
    gfx::Rect surface_damage_rect = window_->GetBoundsInPixels();

    // Create buffer and FakeGlImageNativePixmap.
    std::vector<scoped_refptr<OverlayImageHolder>> fake_overlay_image;
    auto native_pixmap = surface_factory_->CreateNativePixmap(widget_, nullptr, test_buffer_size, gfx::BufferFormat::BGRA_8888, gfx::BufferUsage::SCANOUT);
    ASSERT_TRUE(native_pixmap);
    fake_overlay_image.push_back(base::MakeRefCounted<OverlayImageHolder>(native_pixmap, test_buffer_size));
    ...
    // Submit a frame with an overlay and background.
    {
        ...
        // Prepare background.
        presenter->ScheduleOverlayPlane(fake_overlay_image[0]->GetNativePixmap(), nullptr,
            gfx::OverlayPlaneData(INT32_MIN, gfx::OverlayTransform::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_270, gfx::RectF(window_->GetBoundsInPixels()), crop_uv,
                false, surface_damage_rect, 1.0f, gfx::OverlayPriorityHint::kNone, gfx::RRectF(), gfx::ColorSpace::CreateSRGB(), std::nullopt));
    ...
    PostToServerAndWait([surface_id, expected_src, bounds_dip = window_->GetBoundsInDIP(), surface_damage_rect](wl::TestWaylandServerThread* server) {
        ...
        EXPECT_CALL(*test_viewport,
            ...
            SetSource(_, _, _, _))
            .Times(1)
            .WillOnce([expected_src](float x, float y, float width, float height) {
                auto matches_with_precision_loss = [](float expected, float actual) {
                    // Allows for a precision loss of 1/256
                    bool match = std::abs(wl_fixed_from_double(expected) - wl_fixed_from_double(actual)) <= 1;
                    if (!match) {
                ...
                EXPECT_TRUE(matches_with_precision_loss(expected_src.width(), width));
                EXPECT_TRUE(matches_with_precision_loss(expected_src.height(), height));
            });
        EXPECT_CALL(*test_viewport, SetDestination(bounds_dip.width(), bounds_dip.height())).Times(1);
        EXPECT_CALL(*root_surface, SetBufferTransform(WL_OUTPUT_TRANSFORM_90)).Times(1);
        Expectation damage = EXPECT_CALL(*root_surface,
            Damage(surface_damage_rect.origin().x(), surface_damage_rect.origin().y(), surface_damage_rect.width(), surface_damage_rect.height()))
                                 .Times(1);
        Expectation attach = EXPECT_CALL(*root_surface, Attach(_, 0, 0)).Times(1);
        EXPECT_CALL(*root_surface, Commit()).After(damage, attach);

        auto params_vector = server->zwp_linux_dmabuf_v1()->buffer_params();
        ASSERT_EQ(params_vector.size(), 1u);

        zwp_linux_buffer_params_v1_send_created(params_vector.front()->resource(), params_vector.front()->buffer_resource());
    });

    PostToServerAndWait(
        [surface_id](wl::TestWaylandServerThread* server) { testing::Mock::VerifyAndClearExpectations(server->GetObject<wl::MockSurface>(surface_id)); });
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandSurfaceFactoryTest, Values(wl::ServerConfig {}));

INSTANTIATE_TEST_SUITE_P(
    CompositorVersionV3Test, WaylandSurfaceFactoryCompositorV3, Values(wl::ServerConfig { .compositor_version = wl::TestCompositor::Version::kV3 }));
