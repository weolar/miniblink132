...
class OzonePlatformWayland : public OzonePlatform, public OSExchangeDataProviderFactoryOzone {
public:
    OzonePlatformWayland()
        : old_synthesize_key_repeat_enabled_(KeyEvent::IsSynthesizeKeyRepeatEnabled())
    {
        ...
        if (command_line->HasSwitch(switches::kForceDeviceScaleFactor)) {
            LOG(WARNING) << "--" << switches::kForceDeviceScaleFactor << " on Wayland is TEST ONLY.  Use it at your own risk.";
        }
    ...
    {
        return buffer_manager_connector_ ? buffer_manager_connector_.get() : gpu_platform_support_host_.get();
    }
    ...
    {
        return WaylandWindow::Create(delegate, connection_.get(), std::move(properties));
    }
    ...
    void InitScreen(PlatformScreen* screen) override
    {
        ...
        connection_->wayland_output_manager()->InitWaylandScreen(static_cast<WaylandScreen*>(screen));
    }
    ...
    WaylandUtils* GetPlatformUtils() override
    {
        return wayland_utils_.get();
    }
    ...
    bool IsNativePixmapConfigSupported(gfx::BufferFormat format, gfx::BufferUsage usage) const override
    {
        ...
        if (buffer_manager_) {
            ...
            if (!gbm_device || !gbm_device->CanCreateBufferForFormat(GetFourCCFormatFromBufferFormat(format))) {
                return false;
            }
        } else {
            if (supported_buffer_formats_.find(format) == supported_buffer_formats_.end()) {
                return false;
            }
        }
    ...
    bool InitializeUI(const InitParams& args) override
    {
...
#if BUILDFLAG(USE_XKBCOMMON)
        keyboard_layout_engine_ = std::make_unique<XkbKeyboardLayoutEngine>(xkb_evdev_code_converter_);
#else
        keyboard_layout_engine_ = std::make_unique<StubKeyboardLayoutEngine>();
#endif
        KeyboardLayoutEngineManager::SetKeyboardLayoutEngine(keyboard_layout_engine_.get());
        ...
        if (drm_render_node_fd.is_valid()) {
            connection_->buffer_manager_host()->SetDrmSyncobjWrapper(std::make_unique<DrmSyncobjIoctlWrapper>(std::move(drm_render_node_fd)));
        }

        buffer_manager_connector_ = std::make_unique<WaylandBufferManagerConnector>(connection_->buffer_manager_host());
#if BUILDFLAG(IS_LINUX)
        ...
        supported_buffer_formats_ = connection_->buffer_manager_host()->GetSupportedBufferFormats();
#if BUILDFLAG(IS_LINUX)
        linux_ui_delegate_ = std::make_unique<LinuxUiDelegateWayland>(connection_.get());
#endif
        ...
        GetInputMethodContextFactoryForOzone() = base::BindRepeating(
            [](WaylandConnection* connection, WaylandKeyboard::Delegate* key_delegate, LinuxInputMethodContextDelegate* ime_delegate)
                -> std::unique_ptr<LinuxInputMethodContext> { return std::make_unique<WaylandInputMethodContext>(connection, key_delegate, ime_delegate); },
            base::Unretained(connection_.get()), base::Unretained(connection_->event_source()));
    ...
    void InitializeGPU(const InitParams& args) override
    {
        ...
        surface_factory_ = std::make_unique<WaylandSurfaceFactory>(connection_.get(), buffer_manager_.get());
        overlay_manager_ = std::make_unique<WaylandOverlayManager>(buffer_manager_.get());
    }
    ...
    const PlatformProperties& GetPlatformProperties() override
    {
        ...
        if (!initialised) {
            ...
            // arbitrary position.
            properties->supports_global_screen_coordinates = kDefaultScreenCoordinateEnabled;
    ...
    {
        using SupportsForTest = OzonePlatform::PlatformRuntimeProperties::SupportsForTest;
        const auto& override_supports_ssd_for_test = OzonePlatform::PlatformRuntimeProperties::override_supports_ssd_for_test;
        const auto& override_supports_per_window_scaling_for_test = OzonePlatform::PlatformRuntimeProperties::override_supports_per_window_scaling_for_test;
        ...
        if (connection_) {
            ...
            properties.supports_server_side_window_decorations
                = (connection_->xdg_decoration_manager_v1() != nullptr && override_supports_ssd_for_test == SupportsForTest::kNotSet)
                || override_supports_ssd_for_test == SupportsForTest::kYes;
            properties.supports_overlays = connection_->ShouldUseOverlayDelegation() && connection_->viewporter();
            properties.supports_non_backed_solid_color_buffers
                = connection_->ShouldUseOverlayDelegation() && connection_->buffer_manager_host()->SupportsNonBackedSolidColorBuffers();
            properties.supports_single_pixel_buffer
                = ui::IsWaylandOverlayDelegationEnabled() && connection_->buffer_manager_host()->SupportsSinglePixelBuffer();
            ...
            properties.needs_background_image = connection_->ShouldUseOverlayDelegation() && connection_->viewporter();
            properties.supports_activation
                = connection_->zaura_shell() && zaura_shell_get_version(connection_->zaura_shell()->wl_object()) >= ZAURA_TOPLEVEL_ACTIVATE_SINCE_VERSION;
            properties.supports_subwindows_as_accelerated_widgets = connection_->ShouldUseOverlayDelegation()
                ? connection_->surface_augmenter() && connection_->surface_augmenter()->SupportsCompositingOnlySurface()
                : true;
            properties.supports_per_window_scaling
                = (connection_->UsePerSurfaceScaling() && override_supports_per_window_scaling_for_test == SupportsForTest::kNotSet)
                || (override_supports_per_window_scaling_for_test == SupportsForTest::kYes);

            if (surface_factory_) {
                DCHECK(has_initialized_gpu());
                properties.supports_native_pixmaps = surface_factory_->SupportsNativePixmaps();
            }
            ...
            properties.supports_non_backed_solid_color_buffers
                = buffer_manager_->supports_overlays() && buffer_manager_->supports_non_backed_solid_color_buffers();
            properties.supports_single_pixel_buffer = ui::IsWaylandOverlayDelegationEnabled() && buffer_manager_->supports_single_pixel_buffer();
            // See the comment above.
            properties.needs_background_image = buffer_manager_->supports_overlays() && buffer_manager_->supports_viewporter();
            properties.supports_native_pixmaps = surface_factory_->SupportsNativePixmaps();
            properties.supports_clip_rect = buffer_manager_->supports_clip_rect();
            properties.supports_affine_transform = buffer_manager_->supports_affine_transform();
            properties.supports_out_of_window_clip_rect = buffer_manager_->supports_out_of_window_clip_rect();
            properties.has_transformation_fix = buffer_manager_->has_transformation_fix();
        }
        return properties;
    }
    ...
    void AddInterfaces(mojo::BinderMap* binders) override
    {
        ...
        binders->Add<ozone::mojom::WaylandBufferManagerGpu>(
            base::BindRepeating(&OzonePlatformWayland::CreateWaylandBufferManagerGpuBinding, base::Unretained(this)), gpu_task_runner);
    }
    ...
    void CreateWaylandBufferManagerGpuBinding(mojo::PendingReceiver<ozone::mojom::WaylandBufferManagerGpu> receiver)
    {
        buffer_manager_->AddBindingWaylandBufferManagerGpu(std::move(receiver));
    }

    void PostCreateMainMessageLoop(base::OnceCallback<void()> shutdown_cb, scoped_refptr<base::SingleThreadTaskRunner>) override
    {
        DCHECK(connection_);
    ...
    std::unique_ptr<PlatformKeyboardHook> CreateKeyboardHook(PlatformKeyboardHookTypes type, base::RepeatingCallback<void(KeyEvent* event)> callback,
        std::optional<base::flat_set<DomCode>> dom_codes, gfx::AcceleratedWidget accelerated_widget) override
    {
        DCHECK(connection_);
        ...
        case PlatformKeyboardHookTypes::kModifier:
            return seat->keyboard()->CreateKeyboardHook(window, std::move(dom_codes), std::move(callback));
