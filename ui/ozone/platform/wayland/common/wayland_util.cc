...
uint32_t IdentifyDirection(int hittest)
{
    ...
    case HTBOTTOM:
        ...
        break;
    case HTBOTTOMLEFT:
        direction = xdg_toplevel_resize_edge::XDG_TOPLEVEL_RESIZE_EDGE_BOTTOM_LEFT;
        break;
    case HTBOTTOMRIGHT:
        direction = xdg_toplevel_resize_edge::XDG_TOPLEVEL_RESIZE_EDGE_BOTTOM_RIGHT;
        break;
    case HTLEFT:
...
bool DrawBitmap(const SkBitmap& bitmap, ui::WaylandShmBuffer* out_buffer)
{
    ...
    DCHECK(gfx::Rect(out_buffer->size()).Contains(gfx::Rect(bitmap.width(), bitmap.height())));

    auto* mapped_memory = out_buffer->GetMemory();
    auto size = out_buffer->size();
    sk_sp<SkSurface> sk_surface
        = SkSurfaces::WrapPixels(SkImageInfo::Make(size.width(), size.height(), kColorType, kOpaque_SkAlphaType), mapped_memory, out_buffer->stride());
...
gfx::Rect TranslateBoundsToParentCoordinates(const gfx::Rect& child_bounds, const gfx::Rect& parent_bounds)
{
    return gfx::Rect((child_bounds.origin() - parent_bounds.origin().OffsetFromOrigin()), child_bounds.size());
}

gfx::RectF TranslateBoundsToParentCoordinatesF(const gfx::RectF& child_bounds, const gfx::RectF& parent_bounds)
{
    return gfx::RectF((child_bounds.origin() - parent_bounds.origin().OffsetFromOrigin()), child_bounds.size());
}

gfx::Rect TranslateBoundsToTopLevelCoordinates(const gfx::Rect& child_bounds, const gfx::Rect& parent_bounds)
{
    return gfx::Rect((child_bounds.origin() + parent_bounds.origin().OffsetFromOrigin()), child_bounds.size());
}
...
gfx::RectF ApplyWaylandTransform(const gfx::RectF& rect, const gfx::SizeF& bounds, wl_output_transform transform)
{
...
gfx::Rect ApplyWaylandTransform(const gfx::Rect& rect, const gfx::Size& bounds, wl_output_transform transform)
{
...
gfx::SizeF ApplyWaylandTransform(const gfx::SizeF& size, wl_output_transform transform)
{
...
ui::WaylandWindow* RootWindowFromWlSurface(wl_surface* surface)
{
    if (!surface)
        return nullptr;
    auto* wayland_surface = static_cast<ui::WaylandSurface*>(wl_proxy_get_user_data(reinterpret_cast<wl_proxy*>(surface)));
    if (!wayland_surface)
        return nullptr;
    return wayland_surface->root_window();
}

gfx::Rect TranslateWindowBoundsToParentDIP(ui::WaylandWindow* window, ui::WaylandWindow* parent_window)
{
    DCHECK(window);
    DCHECK(parent_window);
    DCHECK_EQ(window->applied_state().window_scale, parent_window->applied_state().window_scale);
    return wl::TranslateBoundsToParentCoordinates(window->GetBoundsInDIP(), parent_window->GetBoundsInDIP());
}
...
void TransformToWlArray(const absl::variant<gfx::OverlayTransform, gfx::Transform>& transform, wl_array& array)
{
    ...
    constexpr std::array<std::array<int, 2>, 6> rcs = { { { 0, 0 }, { 1, 0 }, { 0, 1 }, { 1, 1 }, { 0, 3 }, { 1, 3 } } };
...
bool MaybeHandlePlatformEventForDrag(const ui::PlatformEvent& event, bool start_drag_ack_received, base::OnceClosure cancel_drag_cb)
{
    ...
    if (!event->IsSynthesized() && (event->type() == ui::EventType::kMouseReleased || event->type() == ui::EventType::kTouchReleased)) {
...
void RecordConnectionMetrics(wl_display* display)
{
    ...
    auto get_compositor = [&]() {
        ...
        std::string name;
        if (!base::ReadFileToStringNonBlocking(base::FilePath(base::StringPrintf("/proc/%d/comm", credentials.pid)), &name)) {
            return WaylandCompositor::kUnknown;
        }

        for (const auto& [name_key, compositor] : kCompositors) {
            if (base::StartsWith(name, name_key, base::CompareCase::INSENSITIVE_ASCII)) {
                return compositor;
            }
        }
