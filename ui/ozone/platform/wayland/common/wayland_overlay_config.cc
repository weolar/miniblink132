...
WaylandOverlayConfig::WaylandOverlayConfig(WaylandOverlayConfig&& other) = default;

WaylandOverlayConfig::WaylandOverlayConfig(const gfx::OverlayPlaneData& data, std::unique_ptr<gfx::GpuFence> fence, BufferId buffer_id, float scale_factor)
    ...
    , access_fence_handle(fence ? fence->GetGpuFenceHandle().Clone() : gfx::GpuFenceHandle())
    , color_space(data.color_space == gfx::ColorSpace::CreateSRGB() ? std::nullopt : std::optional<gfx::ColorSpace>(data.color_space))
    , rounded_clip_bounds(data.rounded_corners.IsEmpty() ? std::nullopt : std::optional<gfx::RRectF>(data.rounded_corners))
    ,
