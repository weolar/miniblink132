...
using GlobalObjectFactory = void (*)(ui::WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version);
...
template <typename T> class GlobalObjectRegistrar {
public:
...
template <typename T> class Object : public std::unique_ptr<T, Deleter> {
public:
    Object()
    {
    }
    ...
    uint32_t id() const
    {
        return wl_proxy_get_id(reinterpret_cast<wl_proxy*>(std::unique_ptr<T, Deleter>::get()));
    }
};

template <typename T> wl::Object<T> Bind(wl_registry* registry, uint32_t name, uint32_t version)
{
    DCHECK(ObjectTraits<T>::interface);
    return wl::Object<T>(wl::bind_registry<T>(registry, name, ObjectTraits<T>::interface, version));
}

template <> struct ObjectTraits<wl_proxy> {
...
#define DECLARE_WAYLAND_OBJECT_TRAITS(TYPE)                                                                                                                    \
    struct TYPE;                                                                                                                                               \
    namespace wl {                                                                                                                                             \
    template <> struct ObjectTraits<TYPE> {                                                                                                                    \
        static const wl_interface* interface;                                                                                                                  \
