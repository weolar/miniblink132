...
{
    if (wl::get_version_of_object(surface) >= GTK_SURFACE1_RELEASE_SINCE_VERSION) {
...
{
    if (wl::get_version_of_object(data_device) >= WL_DATA_DEVICE_RELEASE_SINCE_VERSION) {
...
{
    if (wl::get_version_of_object(toplevel) >= ZAURA_TOPLEVEL_RELEASE_SINCE_VERSION) {
...
bool CanBind(const std::string& interface, uint32_t available_version, uint32_t min_version, uint32_t max_version)
{
    if (available_version < min_version) {
        LOG(WARNING) << "Unable to bind to " << interface << " version " << available_version << ".  The minimum supported version is " << min_version << ".";
        return false;
    }

    if (available_version > max_version) {
        LOG(WARNING) << "Binding to " << interface << " version " << max_version << " but version " << available_version << " is available.";
    }

    return true;
}

void (*ObjectTraits<wl_cursor_theme>::deleter)(wl_cursor_theme*) = &wl_cursor_theme_destroy;
...
void (*ObjectTraits<wl_event_queue>::deleter)(wl_event_queue*) = &wl_event_queue_destroy;
...
#define IMPLEMENT_WAYLAND_OBJECT_TRAITS_WITH_DELETER(TYPE, DELETER)                                                                                            \
    const wl_interface* ObjectTraits<struct TYPE>::interface = &TYPE##_interface;                                                                              \
    void (*ObjectTraits<struct TYPE>::deleter)(struct TYPE*) = &DELETER;

#define IMPLEMENT_WAYLAND_OBJECT_TRAITS(TYPE) IMPLEMENT_WAYLAND_OBJECT_TRAITS_WITH_DELETER(TYPE, TYPE##_destroy)
...
IMPLEMENT_WAYLAND_OBJECT_TRAITS_WITH_DELETER(zaura_output_manager, delete_zaura_output_manager)
IMPLEMENT_WAYLAND_OBJECT_TRAITS_WITH_DELETER(zaura_output_manager_v2, delete_zaura_output_manager_v2)
IMPLEMENT_WAYLAND_OBJECT_TRAITS_WITH_DELETER(zaura_shell, delete_zaura_shell)
IMPLEMENT_WAYLAND_OBJECT_TRAITS_WITH_DELETER(zaura_surface, delete_zaura_surface)
IMPLEMENT_WAYLAND_OBJECT_TRAITS_WITH_DELETER(zaura_output, delete_zaura_output)
IMPLEMENT_WAYLAND_OBJECT_TRAITS_WITH_DELETER(zaura_toplevel, delete_zaura_toplevel)
