...
template <typename T> uint32_t get_version_of_object(T* obj)
{
    return wl_proxy_get_version(reinterpret_cast<wl_proxy*>(obj));
}

template <typename T> T* bind_registry(struct wl_registry* registry, uint32_t name, const struct wl_interface* interface, uint32_t version)
{
