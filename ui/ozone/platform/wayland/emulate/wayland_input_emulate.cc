...
WaylandInputEmulate::PendingRequest::PendingRequest(PendingRequestType request_type, uint32_t request_id)
    : type(request_type)
...
WaylandInputEmulate::WaylandInputEmulate(base::RepeatingCallback<void(uint32_t)> request_processed)
    : request_processed_callback_(std::move(request_processed))
{
    ...
    static constexpr wl_registry_listener kRegistryListener = { .global = &OnGlobal, .global_remove = &OnGlobalRemove };
    ...
    static constexpr zcr_ui_controls_v1_listener kUiControlsListener = { .request_processed = &OnRequestProcessed };
...
void WaylandInputEmulate::EmulateKeyboardKey(ui::DomCode dom_code, int key_state, int accelerator_state, uint32_t request_id)
{
    if (AnyWindowWaitingForBufferCommit()) {
        auto pending_request = std::make_unique<PendingRequest>(PendingRequestType::KeyPress, request_id);
    ...
    VLOG(1) << "Requesting keyboard key: dom_code=" << ui::KeycodeConverter::DomCodeToCodeString(dom_code) << " state=" << key_state
            << " accelerator_state=" << accelerator_state;

    zcr_ui_controls_v1_send_key_events(ui_controls_, ui::KeycodeConverter::DomCodeToEvdevCode(dom_code), key_state, accelerator_state, request_id);
...
void WaylandInputEmulate::EmulatePointerMotion(
    gfx::AcceleratedWidget widget, const gfx::Point& mouse_surface_location, const gfx::Point& mouse_screen_location, uint32_t request_id)
{
    if (AnyWindowWaitingForBufferCommit()) {
        auto pending_request = std::make_unique<PendingRequest>(PendingRequestType::MouseMove, request_id);
    ...
    if (widget) {
        ...
        if (auto* toplevel_window = window->AsWaylandToplevelWindow()) {
            xdg_surface = toplevel_window->shell_toplevel()->AsXDGToplevelWrapper()->xdg_surface_wrapper()->xdg_surface();
        } else if (auto* popup = window->AsWaylandPopup()) {
            xdg_surface = popup->shell_popup()->AsXDGPopupWrapper()->xdg_surface_wrapper()->xdg_surface();
        }
        ...
        // true.
        target_location = window->IsScreenCoordinatesEnabled() ? mouse_screen_location : mouse_surface_location;
    }

    VLOG(1) << "Requesting pointer motion: location=" << target_location.ToString();

    zcr_ui_controls_v1_send_mouse_move(ui_controls_, target_location.x(), target_location.y(), target_surface, request_id);
    wayland_proxy->FlushForTesting();
}

void WaylandInputEmulate::EmulatePointerButton(ui_controls::MouseButton button, int button_state, int accelerator_state, uint32_t request_id)
{
    if (AnyWindowWaitingForBufferCommit()) {
        auto pending_request = std::make_unique<PendingRequest>(PendingRequestType::MouseButton, request_id);
    ...
    VLOG(1) << "Requesting pointer button: button=" << button << " button_state=" << button_state;

    zcr_ui_controls_v1_send_mouse_button(ui_controls_, button, button_state, accelerator_state, request_id);

    auto* wayland_proxy = wl::WaylandProxy::GetInstance();
    wayland_proxy->FlushForTesting();
}

void WaylandInputEmulate::EmulateTouch(int action, const gfx::Point& touch_screen_location, int touch_id, uint32_t request_id)
{
    if (AnyWindowWaitingForBufferCommit()) {
        auto pending_request = std::make_unique<PendingRequest>(PendingRequestType::Touch, request_id);
    ...
    VLOG(1) << "Requesting touch: location=" << touch_screen_location.ToString() << " action=" << action << " touch_id=" << touch_id;

    zcr_ui_controls_v1_send_touch(ui_controls_, action, touch_id, touch_screen_location.x(), touch_screen_location.y(), /*surface=*/nullptr, request_id);
    auto* wayland_proxy = wl::WaylandProxy::GetInstance();
    wayland_proxy->FlushForTesting();
}

#if BUILDFLAG(IS_CHROMEOS)
void WaylandInputEmulate::EmulateUpdateDisplay(const std::string& display_specs, uint32_t request_id)
{
    VLOG(1) << "Updating display specs to: " << display_specs;
    if (zcr_ui_controls_v1_get_version(ui_controls_) >= ZCR_UI_CONTROLS_V1_DISPLAY_INFO_LIST_DONE_SINCE_VERSION) {
        const std::vector<display::Display>& existing_displays = display::Screen::GetScreen()->GetAllDisplays();
        auto info_list = display::CreateDisplayInfoListFromSpecs(display_specs, std::vector<display::Display>(), false);
        ...
        for (const auto& pending_display : info_list) {
            if (existing_display_index < existing_displays.size()) {
                auto id_pair = ui::wayland::ToWaylandDisplayIdPair(existing_displays[existing_display_index].id());
                zcr_ui_controls_v1_set_display_info_id(ui_controls_, id_pair.high, id_pair.low);
                ++existing_display_index;
            }

            zcr_ui_controls_v1_set_display_info_size(ui_controls_, pending_display.bounds_in_native().width(), pending_display.bounds_in_native().height());

            float device_scale_factor = pending_display.device_scale_factor();
            uint32_t scale_factor_value = *reinterpret_cast<const uint32_t*>(&device_scale_factor);
            zcr_ui_controls_v1_set_display_info_device_scale_factor(ui_controls_, scale_factor_value);
...
void WaylandInputEmulate::OnWindowConfigured(gfx::AcceleratedWidget widget, bool is_configured)
{
    auto it = windows_.find(widget);
    ...
    auto* wayland_window = wayland_proxy->GetWaylandWindowForAcceleratedWidget(widget);
    ...
    wl_callback_add_listener(test_surface->frame_callback, &kFrameCallbackListener, this);
...
// static
void WaylandInputEmulate::OnRequestProcessed(void* data, zcr_ui_controls_v1* ui_controls, uint32_t id)
{
...
// static
void WaylandInputEmulate::OnGlobal(void* data, wl_registry* registry, uint32_t name, const char* interface, uint32_t version)
{
    auto* self = static_cast<WaylandInputEmulate*>(data);
    if (strcmp(interface, "zcr_ui_controls_v1") == 0 && version >= kMinVersion) {
        const wl_interface* wayland_interface = static_cast<const wl_interface*>(&zcr_ui_controls_v1_interface);
        self->ui_controls_ = static_cast<zcr_ui_controls_v1*>(wl_registry_bind(registry, name, wayland_interface, version));
    }
}

// static
void WaylandInputEmulate::OnGlobalRemove(void* data, wl_registry* registry, uint32_t name)
{
}

// static
void WaylandInputEmulate::OnFrameDone(void* data, wl_callback* callback, uint32_t time)
{
    auto* self = static_cast<WaylandInputEmulate*>(data);
    CHECK(self) << "WaylandInputEmulate was destroyed before a frame callback arrived";
...
void WaylandInputEmulate::DispatchPendingRequests()
{
    while (!pending_requests_.empty()) {
        ...
        case PendingRequestType::KeyPress:
            EmulateKeyboardKey(event->key_dom_code, event->key_state, event->accelerator_state, event->request_id);
            break;
        case PendingRequestType::MouseMove:
            ...
            EmulatePointerMotion(event->widget, event->mouse_surface_location, event->mouse_screen_location, event->request_id);
            break;
        case PendingRequestType::MouseButton:
            EmulatePointerButton(event->button, event->button_state, event->accelerator_state, event->request_id);
            break;
        case PendingRequestType::Touch:
            EmulateTouch(event->action, event->touch_screen_location, event->touch_id, event->request_id);
            break;
        }
    }
}
