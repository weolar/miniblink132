...
public:
    ...
    void EmulateKeyboardKey(ui::DomCode dom_code, int key_state, int accelerator_state, uint32_t request_id);

    // Both |mouse_surface_location| and |mouse_screen_location| are in DIP.
    void EmulatePointerMotion(
    ...
    // |accelerator_state| is a bit-mask of ui_controls::AcceleratorState.
    void EmulatePointerButton(ui_controls::MouseButton button, int button_state, int accelerator_state, uint32_t request_id);

    // |touch_screen_location| is in DIP.
    void EmulateTouch(int action, const gfx::Point& touch_screen_location, int touch_id, uint32_t request_id);

#if BUILDFLAG(IS_CHROMEOS)
    // |display_specs| is the spec for the display(s).
    void EmulateUpdateDisplay(const std::string& display_specs, uint32_t request_id);
#endif
...
private:
    ...
    void OnWindowConfigured(gfx::AcceleratedWidget widget, bool is_configured) override;
    void OnWindowRoleAssigned(gfx::AcceleratedWidget widget) override;

    // zcr_ui_controls_v1_listener callbacks:
    static void OnRequestProcessed(void* data, zcr_ui_controls_v1* ui_controls, uint32_t id);
    ...
    // created.
    base::flat_map<gfx::AcceleratedWidget, std::unique_ptr<WaylandInputEmulate::TestWindow>> windows_;
