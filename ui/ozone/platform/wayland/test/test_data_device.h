...
public:
    ...
    void StartDrag(TestDataSource* data_source, MockSurface* origin, uint32_t serial);
    void SendOfferAndEnter(MockSurface* origin, const gfx::Point& location);

    void OnEnter(uint32_t serial, wl_resource* surface, wl_fixed_t x, wl_fixed_t y, TestDataOffer* data_offer);
    void OnLeave();
    void OnMotion(uint32_t time, wl_fixed_t x, wl_fixed_t y);
    void OnDrop();

    uint32_t drag_serial() const
    {
        return drag_serial_;
    }
