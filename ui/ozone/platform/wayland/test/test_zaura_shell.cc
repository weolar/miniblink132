...
void GetAuraSurface(wl_client* client, wl_resource* resource, uint32_t id, wl_resource* surface_resource)
{
    CreateResourceWithImpl<TestZAuraSurface>(client, &zaura_surface_interface, kZAuraShellVersion, &kTestZAuraSurfaceImpl, id);
}

void GetAuraOutput(wl_client* client, wl_resource* resource, uint32_t id, wl_resource* output_resource)
{
    wl_resource* zaura_output_resource
        = CreateResourceWithImpl<TestZAuraOutput>(client, &zaura_output_interface, kZAuraOutputVersion, &kTestZAuraOutputImpl, id);
...
void SurfaceSubmissionInPixelCoordinates(wl_client* client, wl_resource* resource)
{
...
void GetAuraToplevelForXdgToplevel(wl_client* client, wl_resource* resource, uint32_t id, wl_resource* toplevel)
{
    wl_resource* zaura_toplevel_resource
        = CreateResourceWithImpl<TestZAuraToplevel>(client, &zaura_toplevel_interface, kZAuraShellVersion, &kTestZAuraToplevelImpl, id);
    auto* xdg_toplevel = GetUserDataAs<MockXdgTopLevel>(toplevel);
    xdg_toplevel->set_zaura_toplevel(GetUserDataAs<TestZAuraToplevel>(zaura_toplevel_resource));
}

void GetAuraPopupForXdgPopup(wl_client* client, wl_resource* resource, uint32_t id, wl_resource* popup)
{
    CreateResourceWithImpl<TestZAuraPopup>(client, &zaura_popup_interface, kZAuraShellVersion, &kTestZAuraPopupImpl, id);
}
...
TestZAuraShell::TestZAuraShell()
    : GlobalObject(&zaura_shell_interface, &kTestZAuraShellImpl, kZAuraShellVersion)
{
}
...
{
    if (resource() && wl_resource_get_version(resource()) >= ZAURA_SHELL_ALL_BUG_FIXES_SENT_SINCE_VERSION) {
...
{
    if (resource() && wl_resource_get_version(resource()) >= ZAURA_SHELL_COMPOSITOR_VERSION_SINCE_VERSION && !compositor_version_string_.empty()) {
        zaura_shell_send_compositor_version(resource(), compositor_version_string_.c_str());
    }
}

void TestZAuraShell::SendBugFixes()
{
    if (resource() && wl_resource_get_version(resource()) >= ZAURA_SHELL_BUG_FIX_SINCE_VERSION) {
