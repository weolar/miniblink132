...
extern const struct zwp_linux_buffer_params_v1_interface kTestZwpLinuxBufferParamsV1Impl;
...
public:
    ...
    TestZwpLinuxBufferParamsV1& operator=(const TestZwpLinuxBufferParamsV1&) = delete;
    ...
    void Create(wl_client* client, wl_resource* resource, int32_t width, int32_t height, uint32_t format, uint32_t flags);
    void CreateImmed(wl_client* client, wl_resource* resource, uint32_t buffer_id, int32_t width, int32_t height, uint32_t format, uint32_t flags);

    wl_resource* buffer_resource() const
    {
        return buffer_resource_;
    }
