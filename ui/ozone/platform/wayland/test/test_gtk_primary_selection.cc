...
struct GtkPrimarySelectionOffer final : public TestSelectionOffer::Delegate {
    ...
    {
        gtk_primary_selection_offer_send_offer(offer->resource(), mime_type.c_str());
    }
...
struct GtkPrimarySelectionDevice final : public TestSelectionDevice::Delegate {
    ...
    {
        static const struct gtk_primary_selection_offer_interface kOfferImpl = { &TestSelectionOffer::Receive, &Destroy };
        ...
        wl_resource* new_offer_resource = CreateResourceWithImpl<TestSelectionOffer>(
            wl_resource_get_client(device->resource()), &gtk_primary_selection_offer_interface, version, &kOfferImpl, 0, std::move(owned_delegate));
        delegate->offer = GetUserDataAs<TestSelectionOffer>(new_offer_resource);
        gtk_primary_selection_device_send_data_offer(device_resource, new_offer_resource);
    ...
    {
        CHECK(offer);
        gtk_primary_selection_device_send_selection(device->resource(), offer->resource());
    }
...
struct GtkPrimarySelectionSource : public TestSelectionSource::Delegate {
    ...
    {
        gtk_primary_selection_source_send_send(source->resource(), mime_type.c_str(), write_fd.get());
...
struct GtkPrimarySelectionDeviceManager : public TestSelectionDeviceManager::Delegate {
    ...
    {
        static const struct gtk_primary_selection_device_interface kTestSelectionDeviceImpl = { &TestSelectionDevice::SetSelection, &Destroy };
        ...
        wl_resource* resource = CreateResourceWithImpl<TestSelectionDevice>(
            client, &gtk_primary_selection_device_interface, version_, &kTestSelectionDeviceImpl, id, std::move(owned_delegate));
    ...
    {
        static const struct gtk_primary_selection_source_interface kTestSelectionSourceImpl = { &TestSelectionSource::Offer, &Destroy };
        ...
        wl_resource* resource = CreateResourceWithImpl<TestSelectionSource>(
            client, &gtk_primary_selection_source_interface, version_, &kTestSelectionSourceImpl, id, std::move(owned_delegate));
...
std::unique_ptr<TestSelectionDeviceManager> CreateTestSelectionManagerGtk()
{
    ...
    static const struct gtk_primary_selection_device_manager_interface kTestSelectionManagerImpl
    ...
    static const TestSelectionDeviceManager::InterfaceInfo interface_info
        = { .interface = &gtk_primary_selection_device_manager_interface, .implementation = &kTestSelectionManagerImpl, .version = kVersion };
    return std::make_unique<TestSelectionDeviceManager>(interface_info, std::make_unique<GtkPrimarySelectionDeviceManager>(kVersion));
}
