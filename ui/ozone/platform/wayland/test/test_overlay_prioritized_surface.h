...
extern const struct overlay_prioritized_surface_interface kTestOverlayPrioritizedSurfaceImpl;
...
public:
    ...
    TestOverlayPrioritizedSurface(const TestOverlayPrioritizedSurface& rhs) = delete;
    TestOverlayPrioritizedSurface& operator=(const TestOverlayPrioritizedSurface& rhs) = delete;

    void set_overlay_priority(uint32_t priority)
    {
        overlay_priority_ = priority;
    }
    uint32_t overlay_priority()
    {
        return overlay_priority_;
    }
...
private:
    ...
    uint32_t overlay_priority_ = OVERLAY_PRIORITIZED_SURFACE_OVERLAY_PRIORITY_NONE;
};
