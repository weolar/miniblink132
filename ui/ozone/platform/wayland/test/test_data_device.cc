...
void DataDeviceStartDrag(wl_client* client, wl_resource* resource, wl_resource* source, wl_resource* origin, wl_resource* icon, uint32_t serial)
{
    auto* data_source = GetUserDataAs<TestDataSource>(source);
    auto* origin_surface = GetUserDataAs<MockSurface>(origin);

    GetUserDataAs<TestDataDevice>(resource)->StartDrag(data_source, origin_surface, serial);
}
...
struct WlDataDeviceImpl : public TestSelectionDevice::Delegate {
    explicit WlDataDeviceImpl(TestDataDevice* device)
        : device_(device)
    {
    }
    ...
    TestSelectionOffer* CreateAndSendOffer() override
    {
        ...
        wl_resource* new_offer_resource = CreateResourceWithImpl<TestDataOffer>(
            wl_resource_get_client(device_resource), &wl_data_offer_interface, wl_resource_get_version(device_resource), &kTestDataOfferImpl, 0);
    ...
    void SendSelection(TestSelectionOffer* selection_offer) override
    {
        ...
        wl_data_device_send_selection(device_->resource(), selection_offer->resource());
    }
...
const struct wl_data_device_interface kTestDataDeviceImpl = { &DataDeviceStartDrag, &TestSelectionDevice::SetSelection, &DataDeviceRelease };

TestDataDevice::TestDataDevice(wl_resource* resource, TestDataDeviceManager* manager)
...
void TestDataDevice::SetSelection(TestDataSource* data_source, uint32_t serial)
{
    CHECK(manager_);
...
void TestDataDevice::StartDrag(TestDataSource* source, MockSurface* origin, uint32_t serial)
{
    DCHECK(source);
    DCHECK(origin);
...
void TestDataDevice::SendOfferAndEnter(MockSurface* origin, const gfx::Point& location)
{
    ...
    DCHECK(display);
    wl_data_device_send_enter(resource(), wl_display_get_serial(display), origin->resource(), wl_fixed_from_int(location.x()), wl_fixed_from_int(location.y()),
        data_offer->resource());
}

void TestDataDevice::OnEnter(uint32_t serial, wl_resource* surface, wl_fixed_t x, wl_fixed_t y, TestDataOffer* data_offer)
{
    wl_data_device_send_enter(resource(), serial, surface, x, y, data_offer->resource());
}
