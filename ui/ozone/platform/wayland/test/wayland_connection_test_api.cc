...
void WaylandConnectionTestApi::SetCursorShape(std::unique_ptr<WaylandCursorShape> obj)
{
    impl_->cursor_shape_ = std::move(obj);
}

void WaylandConnectionTestApi::SetZcrCursorShapes(std::unique_ptr<WaylandZcrCursorShapes> obj)
{
...
void WaylandConnectionTestApi::SyncDisplay()
{
    ...
    wl::Object<wl_callback> sync_callback(wl_display_sync(impl_->display_wrapper()));
    wl_callback_listener listener = { [](void* data, struct wl_callback* cb, uint32_t time) { static_cast<base::RunLoop*>(data)->Quit(); } };
