...
void Attach(wl_client* client, wl_resource* resource, wl_resource* buffer_resource, int32_t x, int32_t y)
{
    auto* surface = GetUserDataAs<MockSurface>(resource);
    surface->AttachNewBuffer(buffer_resource, x, y);
}

void SetOpaqueRegion(wl_client* client, wl_resource* resource, wl_resource* region)
{
    GetUserDataAs<MockSurface>(resource)->SetOpaqueRegionImpl(region);
}

void SetInputRegion(wl_client* client, wl_resource* resource, wl_resource* region)
{
...
void Frame(struct wl_client* client, struct wl_resource* resource, uint32_t callback)
{
    ...
    wl_resource* callback_resource = wl_resource_create(client, &wl_callback_interface, 1, callback);
...
void SetBufferTransform(struct wl_client* client, struct wl_resource* resource, int32_t transform)
{
    auto* mock_surface = GetUserDataAs<MockSurface>(resource);
    mock_surface->SetBufferTransform(transform);
}

void DamageBuffer(struct wl_client* client, struct wl_resource* resource, int32_t x, int32_t y, int32_t width, int32_t height)
{
...
{
    auto* linux_buffer_release_resource = wl_resource_create(client, &zwp_linux_buffer_release_v1_interface, 1, id);
    auto* linux_surface_synchronization = GetUserDataAs<TestLinuxSurfaceSynchronization>(resource);
    auto* surface = GetUserDataAs<MockSurface>(linux_surface_synchronization->surface_resource());
    surface->set_linux_buffer_release(surface->attached_buffer(), linux_buffer_release_resource);
}
...
const struct zwp_linux_surface_synchronization_v1_interface kMockZwpLinuxSurfaceSynchronizationImpl = {
    DestroyResource,
    SetAcquireFence,
    GetRelease,
};
...
{
    if (!ResourceHasImplementation(resource, &wl_surface_interface, &kMockSurfaceImpl))
        return nullptr;
...
void MockSurface::SetOpaqueRegionImpl(wl_resource* region)
{
    ...
    opaque_region_ = gfx::Rect(bounds.fLeft, bounds.fTop, bounds.fRight - bounds.fLeft, bounds.fBottom - bounds.fTop);
...
void MockSurface::SetInputRegionImpl(wl_resource* region)
{
    ...
    auto bounds = GetUserDataAs<TestRegion>(region)->getBounds();
    input_region_ = gfx::Rect(bounds.fLeft, bounds.fTop, bounds.fRight - bounds.fLeft, bounds.fBottom - bounds.fTop);

    SetInputRegion(region);
}

void MockSurface::AttachNewBuffer(wl_resource* buffer_resource, int32_t x, int32_t y)
{
    if (attached_buffer_)
...
void MockSurface::ReleaseBufferFenced(wl_resource* buffer, gfx::GpuFenceHandle release_fence)
{
    DCHECK(buffer);
    ...
    if (!release_fence.is_null()) {
        zwp_linux_buffer_release_v1_send_fenced_release(linux_buffer_release, release_fence.Peek());
    } else {
...
void MockSurface::SendFrameCallback()
{
    ...
    wl_callback_send_done(frame_callback_, 0 /* trequest-specific data for the callback. not used */);
