...
{
    auto request_processed_callback = base::BindRepeating(&WaylandOzoneUIControlsTestHelper::RequestProcessed, weak_factory_.GetWeakPtr());
    input_emulate_ = std::make_unique<WaylandInputEmulate>(std::move(request_processed_callback));
}
...
{
    CHECK(!(accelerator_state & ui_controls::kCommand)) << "No Command key on Wayland";

    uint32_t request_id = GetNextRequestId();
    pending_closures_.insert_or_assign(request_id, std::move(closure));
    input_emulate_->EmulateKeyboardKey(UsLayoutKeyboardCodeToDomCode(key), key_event_types, accelerator_state, request_id);
}

void WaylandOzoneUIControlsTestHelper::SendMouseMotionNotifyEvent(
    gfx::AcceleratedWidget widget, const gfx::Point& mouse_loc, const gfx::Point& mouse_loc_in_screen, base::OnceClosure closure)
{
    uint32_t request_id = GetNextRequestId();

    pending_closures_.insert_or_assign(request_id, std::move(closure));
    input_emulate_->EmulatePointerMotion(widget, mouse_loc, mouse_loc_in_screen, request_id);
}

void WaylandOzoneUIControlsTestHelper::SendMouseEvent(gfx::AcceleratedWidget widget, ui_controls::MouseButton type, int button_state, int accelerator_state,
    const gfx::Point& mouse_loc, const gfx::Point& mouse_loc_in_screen, base::OnceClosure closure)
{
    uint32_t request_id = GetNextRequestId();

    pending_closures_.insert_or_assign(request_id, std::move(closure));
    input_emulate_->EmulatePointerButton(type, button_state, accelerator_state, request_id);
}

void WaylandOzoneUIControlsTestHelper::RunClosureAfterAllPendingUIEvents(base::OnceClosure closure)
{
    NOTREACHED();
}
...
void WaylandOzoneUIControlsTestHelper::RequestProcessed(uint32_t request_id)
{
    ...
    if (base::Contains(pending_closures_, request_id)) {
        if (!pending_closures_[request_id].is_null()) {
            ...
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(pending_closures_[request_id]));
        }
