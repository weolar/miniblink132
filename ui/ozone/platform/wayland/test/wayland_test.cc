...
{
#if BUILDFLAG(USE_XKBCOMMON)
    auto keyboard_layout_engine = std::make_unique<XkbKeyboardLayoutEngine>(xkb_evdev_code_converter_);
#else
    auto keyboard_layout_engine = std::make_unique<StubKeyboardLayoutEngine>();
#endif
    scoped_keyboard_layout_engine_ = std::make_unique<ScopedKeyboardLayoutEngine>(std::move(keyboard_layout_engine));
    connection_ = std::make_unique<WaylandConnection>();
    buffer_manager_gpu_ = std::make_unique<WaylandBufferManagerGpu>();
    surface_factory_ = std::make_unique<WaylandSurfaceFactory>(connection_.get(), buffer_manager_gpu_.get());
}
...
void WaylandTestBase::SetUp()
{
    ...
    EXPECT_CALL(delegate_, OnAcceleratedWidgetAvailable(_)).WillOnce(SaveArg<0>(&widget_));
    ...
    EXPECT_EQ(0u, DeviceDataManager::GetInstance()->GetTouchscreenDevices().size());
...
void WaylandTestBase::PostToServerAndWait(base::OnceCallback<void(wl::TestWaylandServerThread* server)> callback, bool no_nested_runloops)
{
    PostToServerAndWait(base::BindOnce(std::move(callback), base::Unretained(&server_)), no_nested_runloops);
}

void WaylandTestBase::PostToServerAndWait(base::OnceClosure closure, bool no_nested_runloops)
{
...
void WaylandTestBase::SendConfigureEvent(uint32_t surface_id, const gfx::Size& size, const wl::ScopedWlArray& states, std::optional<uint32_t> serial)
{
    PostToServerAndWait([size, surface_id, states, serial](wl::TestWaylandServerThread* server) {
        ...
        if (xdg_surface->xdg_toplevel()) {
            xdg_toplevel_send_configure(xdg_surface->xdg_toplevel()->resource(), width, height, surface_states.get());
        } else {
            ASSERT_TRUE(xdg_surface->xdg_popup()->resource());
            xdg_popup_send_configure(xdg_surface->xdg_popup()->resource(), 0, 0, width, height);
        }
        xdg_surface_send_configure(xdg_surface->resource(), serial.has_value() ? serial.value() : server->GetNextSerial());
    });
}

void WaylandTestBase::ActivateSurface(uint32_t surface_id, std::optional<uint32_t> serial)
{
...
void WaylandTestBase::MaybeSetUpXkb()
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        std::unique_ptr<xkb_context, ui::XkbContextDeleter> xkb_context(xkb_context_new(XKB_CONTEXT_NO_FLAGS));
        std::unique_ptr<xkb_keymap, ui::XkbKeymapDeleter> xkb_keymap(
            xkb_keymap_new_from_names(xkb_context.get(), nullptr /*names*/, XKB_KEYMAP_COMPILE_NO_FLAGS));
        std::unique_ptr<xkb_state, ui::XkbStateDeleter> xkb_state(xkb_state_new(xkb_keymap.get()));

        std::unique_ptr<char, base::FreeDeleter> keymap_string(xkb_keymap_get_as_string(xkb_keymap.get(), XKB_KEYMAP_FORMAT_TEXT_V1));
        ASSERT_TRUE(keymap_string.get());

        size_t keymap_size = strlen(keymap_string.get()) + 1;
        base::UnsafeSharedMemoryRegion shared_keymap_region = base::UnsafeSharedMemoryRegion::Create(keymap_size);
        ...
        base::subtle::PlatformSharedMemoryRegion platform_shared_keymap
            = base::UnsafeSharedMemoryRegion::TakeHandleForSerialization(std::move(shared_keymap_region));
        ...
        wl_keyboard_send_keymap(keyboard, WL_KEYBOARD_KEYMAP_FORMAT_XKB_V1, platform_shared_keymap.GetPlatformHandle().fd, keymap_size);
    });
#endif
}
...
void WaylandTestBase::WaitForAllDisplaysReady()
{
    ...
    timer.Start(FROM_HERE, base::Milliseconds(1), base::BindLambdaForTesting([&]() {
...
std::unique_ptr<WaylandWindow> WaylandTestBase::CreateWaylandWindowWithParams(
    PlatformWindowType type, const gfx::Rect bounds, MockWaylandPlatformWindowDelegate* delegate, gfx::AcceleratedWidget parent_widget)
{
    PlatformWindowInitProperties properties;
...
WaylandTestSimpleWithAuraShell::WaylandTestSimpleWithAuraShell()
    : WaylandTestBase({ .enable_aura_shell = wl::EnableAuraShellProtocol::kEnabled })
{
}
