...
void Feedback(struct wl_client* client, struct wl_resource* resource, struct wl_resource* surface, uint32_t callback)
{
    ...
    wl_resource* presentation_feedback_resource = wl_resource_create(client, &wp_presentation_feedback_interface, wl_resource_get_version(resource), callback);
...
{
    PresentationFeedbackParams params { .tv_sec_hi = 1,
        .tv_sec_lo = 1,
        .tv_nsec = 1,
        .refresh = 1,
        .seq_hi = 1,
...
void MockWpPresentation::SendPresentationFeedbackToClient(bool last, std::optional<PresentationFeedbackParams> params)
{
    wl_resource* callback_resource = GetPresentationCallbackResource(last);
    ...
    if (!params.has_value()) {
        ...
        wp_presentation_feedback_send_presented(
            callback_resource, params->tv_sec_hi, params->tv_sec_lo, params->tv_nsec, params->refresh, params->seq_hi, params->seq_lo, params->flags);
    }
