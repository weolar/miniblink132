...
public:
    ...
    const std::optional<TestRegion>& shape() const
    {
        return shape_;
    }
    void set_shape(const std::optional<TestRegion>& shape)
    {
        shape_ = shape;
    }
    ...
    void set_top_inset(int top_inset)
    {
        top_inset_ = top_inset;
    }

    using AckRotateFocusCallback = base::RepeatingCallback<void(uint32_t serial, uint32_t handled)>;
    ...
    {
        return can_fullscreen_;
    }

    using SetUnsetFloatCallback = base::RepeatingCallback<void(bool floated, uint32_t start_location)>;
