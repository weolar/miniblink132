...
constexpr struct wp_fractional_scale_v1_interface kTestFractionalScaleImpl = { .destroy = DestroyResource };

void GetFractionalScale(struct wl_client* client, struct wl_resource* resource, uint32_t id, struct wl_resource* surface)
{
    ...
    wl_resource* fractional_scale_resource = CreateResourceWithImpl<::testing::NiceMock<TestFractionalScale>>(
        client, &wp_fractional_scale_v1_interface, wl_resource_get_version(resource), &kTestFractionalScaleImpl, id, surface);
    CHECK(fractional_scale_resource);
    mock_surface->set_fractional_scale(GetUserDataAs<TestFractionalScale>(fractional_scale_resource));
}

constexpr struct wp_fractional_scale_manager_v1_interface kTestFractionalScaleManagerImpl = {
...
TestFractionalScaleManager::TestFractionalScaleManager()
    : GlobalObject(&wp_fractional_scale_manager_v1_interface, &kTestFractionalScaleManagerImpl, kTestFractionalScaleManagerVersion)
{
}

TestFractionalScaleManager::~TestFractionalScaleManager() = default;

TestFractionalScale::TestFractionalScale(wl_resource* resource, wl_resource* surface)
