...
void handle_client_destroyed(struct wl_listener* listener, void* data)
{
    ...
    destroy_listener->test_server->OnClientDestroyed(static_cast<struct wl_client*>(data));
}
...
TestWaylandServerThread::~TestWaylandServerThread()
{
    ...
    auto stop_controller_on_server_thread = [](wl::TestWaylandServerThread* server) { server->controller_.StopWatchingFileDescriptor(); };
...
bool TestWaylandServerThread::Start()
{
    ...
    if (!SetupExplicitSynchronizationProtocol(config_.use_explicit_synchronization)) {
        return false;
    }
    ...
    protocol_logger_ = wl_display_add_protocol_logger(display_.get(), TestWaylandServerThread::ProtocolLogger, this);
    ...
    options.message_pump_factory = base::BindRepeating(&TestWaylandServerThread::CreateMessagePump, base::Unretained(this), run_loop.QuitClosure());
    if (!base::Thread::StartWithOptions(std::move(options)))
        return false;

    run_loop.Run();

    setenv("WAYLAND_SOCKET", base::NumberToString(client_fd.release()).c_str(), 1);

    return true;
}

void TestWaylandServerThread::RunAndWait(base::OnceCallback<void(TestWaylandServerThread*)> callback)
{
    base::OnceClosure closure = base::BindOnce(std::move(callback), base::Unretained(this));
...
void TestWaylandServerThread::RunAndWait(base::OnceClosure closure)
{
    ...
    task_runner()->PostTaskAndReply(
        FROM_HERE, base::BindOnce(&TestWaylandServerThread::DoRun, base::Unretained(this), std::move(closure)), run_loop.QuitClosure());
    run_loop.Run();
}

void TestWaylandServerThread::Post(base::OnceCallback<void(TestWaylandServerThread*)> callback)
{
    base::OnceClosure closure = base::BindOnce(std::move(callback), base::Unretained(this));
...
{
    task_runner()->PostTask(FROM_HERE, base::BindOnce(&TestWaylandServerThread::DoRun, weak_ptr_factory_.GetWeakPtr(), std::move(closure)));
}
...
void TestWaylandServerThread::OnTestOutputFlush(TestOutput* test_output, const TestOutputMetrics& metrics)
{
...
void TestWaylandServerThread::OnTestOutputGlobalDestroy(TestOutput* test_output)
{
    if (zaura_output_manager_v2_.resource()) {
...
bool TestWaylandServerThread::SetupPrimarySelectionManager(PrimarySelectionProtocol protocol)
{
    switch (protocol) {
...
bool TestWaylandServerThread::SetupExplicitSynchronizationProtocol(ShouldUseExplicitSynchronizationProtocol usage)
{
    switch (usage) {
...
bool TestWaylandServerThread::SetupLinuxDrmSyncobjProtocol(ShouldUseLinuxDrmSyncobjProtocol usage)
{
    switch (usage) {
...
std::unique_ptr<base::MessagePump> TestWaylandServerThread::CreateMessagePump(base::OnceClosure closure)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    auto pump = std::make_unique<base::MessagePumpEpoll>();
    pump->WatchFileDescriptor(wl_event_loop_get_fd(event_loop_), true, base::MessagePumpEpoll::WATCH_READ, &controller_, this);
...
// static
void TestWaylandServerThread::ProtocolLogger(void* user_data, enum wl_protocol_logger_type direction, const struct wl_protocol_logger_message* message)
{
