...
TestWaylandOSExchangeDataProvideFactory::TestWaylandOSExchangeDataProvideFactory()
{
    SetInstance(this);
}
...
std::unique_ptr<OSExchangeDataProvider> TestWaylandOSExchangeDataProvideFactory::CreateProvider()
{
...
void WaylandDragDropTest::SendDndEnter(WaylandWindow* window, const gfx::Point& location)
{
    ...
    PostToServerAndWait([surface_id, location](wl::TestWaylandServerThread* server) {
...
void WaylandDragDropTest::SendDndMotion(const gfx::Point& location)
{
    PostToServerAndWait([location](wl::TestWaylandServerThread* server) {
        ...
        server->data_device_manager()->data_device()->OnMotion(server->GetNextTime(), x, y);
    });
}
...
void WaylandDragDropTest::ReadAndCheckData(const std::string& mime_type, const std::string& expected_data)
{
    PostToServerAndWait([mime_type, expected_data](wl::TestWaylandServerThread* server) {
        ...
        server_data_source->ReadData(mime_type, base::BindLambdaForTesting([&run_loop, &expected_data](std::vector<uint8_t>&& data) {
...
void WaylandDragDropTest::SendPointerEnter(WaylandWindow* window, MockPlatformWindowDelegate* delegate)
{
    const uint32_t surface_id = window->root_surface()->get_surface_id();
...
void WaylandDragDropTest::SendPointerLeave(WaylandWindow* window, MockPlatformWindowDelegate* delegate)
{
    const uint32_t surface_id = window->root_surface()->get_surface_id();
    PostToServerAndWait([surface_id](wl::TestWaylandServerThread* server) {
        auto* surface = server->GetObject<wl::MockSurface>(surface_id);
...
void WaylandDragDropTest::SendPointerButton(WaylandWindow* window, MockPlatformWindowDelegate* delegate, int button, bool pressed)
{
    PostToServerAndWait([pressed, button](wl::TestWaylandServerThread* server) {
        ...
        auto* pointer = server->seat()->pointer();
        ASSERT_TRUE(pointer);
        wl_pointer_send_button(pointer->resource(), server->GetNextSerial(), server->GetNextTime(), button, state);
        wl_pointer_send_frame(pointer->resource());
    });
}

void WaylandDragDropTest::SendTouchDown(WaylandWindow* window, MockPlatformWindowDelegate* delegate, int id, const gfx::Point& location)
{
    const uint32_t surface_id = window->root_surface()->get_surface_id();
    PostToServerAndWait([surface_id, id, location](wl::TestWaylandServerThread* server) {
        auto* surface = server->GetObject<wl::MockSurface>(surface_id);
        ASSERT_TRUE(surface);
        auto* touch = server->seat()->touch();
        ASSERT_TRUE(touch);
        wl_touch_send_down(touch->resource(), server->GetNextSerial(), server->GetNextTime(), surface->resource(), id, wl_fixed_from_double(location.x()),
...
void WaylandDragDropTest::SendTouchUp(int id)
{
    PostToServerAndWait([id](wl::TestWaylandServerThread* server) {
        auto* touch = server->seat()->touch();
        ASSERT_TRUE(touch);
        wl_touch_send_up(touch->resource(), server->GetNextSerial(), server->GetNextTime(), id);
        wl_touch_send_frame(touch->resource());
    });
}

void WaylandDragDropTest::SendTouchMotion(WaylandWindow* window, MockPlatformWindowDelegate* delegate, int id, const gfx::Point& location)
{
    PostToServerAndWait([id, location](wl::TestWaylandServerThread* server) {
        auto* touch = server->seat()->touch();
        ASSERT_TRUE(touch);
        wl_touch_send_motion(touch->resource(), server->GetNextSerial(), id, wl_fixed_from_double(location.x()), wl_fixed_from_double(location.y()));
        wl_touch_send_frame(touch->resource());
    });
}
...
void WaylandDragDropTest::MaybeRunScheduledTasks()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&WaylandDragDropTest::RunTestTask, base::Unretained(this), std::move(next_task)));
}
