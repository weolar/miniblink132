...
struct ZwpPrimarySelectionOffer final : public TestSelectionOffer::Delegate {
    ...
    {
        zwp_primary_selection_offer_v1_send_offer(offer->resource(), mime_type.c_str());
    }
...
struct ZwpPrimarySelectionDevice final : public TestSelectionDevice::Delegate {
    ...
    {
        static const struct zwp_primary_selection_offer_v1_interface kOfferImpl = { &TestSelectionOffer::Receive, &Destroy };
        ...
        wl_resource* new_offer_resource = CreateResourceWithImpl<TestSelectionOffer>(
            wl_resource_get_client(device->resource()), &zwp_primary_selection_offer_v1_interface, version, &kOfferImpl, 0, std::move(owned_delegate));
        delegate->offer = GetUserDataAs<TestSelectionOffer>(new_offer_resource);
        zwp_primary_selection_device_v1_send_data_offer(device_resource, new_offer_resource);
    ...
    {
        CHECK(offer);
        zwp_primary_selection_device_v1_send_selection(device->resource(), offer->resource());
    }

    void HandleSetSelection(TestSelectionSource* source, uint32_t serial) override
    {
        NOTIMPLEMENTED();
    }
...
struct ZwpPrimarySelectionSource : public TestSelectionSource::Delegate {
    void SendSend(const std::string& mime_type, base::ScopedFD write_fd) override
    {
        zwp_primary_selection_source_v1_send_send(source->resource(), mime_type.c_str(), write_fd.get());
...
struct ZwpPrimarySelectionDeviceManager : public TestSelectionDeviceManager::Delegate {
    ...
    {
        static const struct zwp_primary_selection_device_v1_interface kTestSelectionDeviceImpl = { &TestSelectionDevice::SetSelection, &Destroy };
        ...
        wl_resource* resource = CreateResourceWithImpl<TestSelectionDevice>(
            client, &zwp_primary_selection_device_v1_interface, version_, &kTestSelectionDeviceImpl, id, std::move(owned_delegate));
    ...
    {
        static const struct zwp_primary_selection_source_v1_interface kTestSelectionSourceImpl = { &TestSelectionSource::Offer, &Destroy };
        ...
        wl_resource* resource = CreateResourceWithImpl<TestSelectionSource>(
            client, &zwp_primary_selection_source_v1_interface, version_, &kTestSelectionSourceImpl, id, std::move(owned_delegate));
...
std::unique_ptr<TestSelectionDeviceManager> CreateTestSelectionManagerZwp()
{
    ...
    static const struct zwp_primary_selection_device_manager_v1_interface kTestSelectionManagerImpl
        = { &TestSelectionDeviceManager::CreateSource, &TestSelectionDeviceManager::GetDevice, &Destroy };
    static const TestSelectionDeviceManager::InterfaceInfo interface_info
        = { .interface = &zwp_primary_selection_device_manager_v1_interface, .implementation = &kTestSelectionManagerImpl, .version = kVersion };
    return std::make_unique<TestSelectionDeviceManager>(interface_info, std::make_unique<ZwpPrimarySelectionDeviceManager>(kVersion));
}
