...
extern const struct zcr_extended_text_input_v1_interface kMockZcrExtendedTextInputV1Impl;
...
public:
    ...
    MOCK_METHOD(void, DeprecatedSetInputType, (uint32_t input_type, uint32_t input_mode, uint32_t input_flags, uint32_t learning_mode));
    MOCK_METHOD(
        void, SetInputType, (uint32_t input_type, uint32_t input_mode, uint32_t input_flags, uint32_t learning_mode, uint32_t inline_composition_support));
    MOCK_METHOD(void, SetGrammarFragmentAtCursor, (const gfx::Range& range, const std::string& suggestion));
    MOCK_METHOD(void, SetAutocorrectInfo, (const gfx::Range& range, const gfx::Rect& bounds));
