...
std::vector<uint8_t> ReadDataOnWorkerThread(base::ScopedFD fd)
{
    ...
    while (true) {
        ...
        ssize_t bytes_read = HANDLE_EINTR(read(fd.get(), chunk.data(), chunk.size()));
        if (bytes_read > 0) {
...
void WriteDataOnWorkerThread(base::ScopedFD fd, ui::PlatformClipboard::Data data)
{
...
TestSelectionOffer::TestSelectionOffer(wl_resource* resource, std::unique_ptr<Delegate> delegate)
...
void TestSelectionOffer::OnOffer(const std::string& mime_type, ui::PlatformClipboard::Data data)
{
    data_to_offer_[mime_type] = data;
    delegate_->SendOffer(mime_type);
}

void TestSelectionOffer::Receive(wl_client* client, wl_resource* resource, const char* mime_type, int fd)
{
    CHECK(GetUserDataAs<TestSelectionOffer>(resource));
    auto* self = GetUserDataAs<TestSelectionOffer>(resource);
    self->task_runner_->PostTask(FROM_HERE, base::BindOnce(&WriteDataOnWorkerThread, base::ScopedFD(fd), self->data_to_offer_[mime_type]));
}

// TestSelectionSource implementation.
TestSelectionSource::TestSelectionSource(wl_resource* resource, std::unique_ptr<Delegate> delegate)
    : ServerObject(resource)
    , delegate_(std::move(delegate))
    , task_runner_(base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock() }))
{
...
void TestSelectionSource::ReadData(const std::string& mime_type, ReadDataCallback callback)
{
    ...
    task_runner_->PostTaskAndReplyWithResult(FROM_HERE, base::BindOnce(&ReadDataOnWorkerThread, std::move(read_fd)), std::move(callback));
}
...
void TestSelectionSource::Offer(struct wl_client* client, struct wl_resource* resource, const char* mime_type)
{
...
TestSelectionDevice::TestSelectionDevice(wl_resource* resource, std::unique_ptr<Delegate> delegate)
    : ServerObject(resource)
    , delegate_(std::move(delegate))
{
}
...
void TestSelectionDevice::SetSelection(struct wl_client* client, struct wl_resource* resource, struct wl_resource* source, uint32_t serial)
{
...
TestSelectionDeviceManager::TestSelectionDeviceManager(const InterfaceInfo& info, std::unique_ptr<Delegate> delegate)
...
void TestSelectionDeviceManager::CreateSource(wl_client* client, wl_resource* manager_resource, uint32_t id)
{
...
void TestSelectionDeviceManager::GetDevice(wl_client* client, wl_resource* manager_resource, uint32_t id, wl_resource* seat_resource)
{
    CHECK(GetUserDataAs<TestSelectionDeviceManager>(manager_resource));
    auto* manager = GetUserDataAs<TestSelectionDeviceManager>(manager_resource);
