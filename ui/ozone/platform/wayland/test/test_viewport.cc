...
void SetSource(wl_client* client, wl_resource* resource, wl_fixed_t x, wl_fixed_t y, wl_fixed_t width, wl_fixed_t height)
{
    auto* test_vp = GetUserDataAs<TestViewport>(resource);
    DCHECK(test_vp);
    test_vp->SetSource(wl_fixed_to_double(x), wl_fixed_to_double(y), wl_fixed_to_double(width), wl_fixed_to_double(height));
}

void SetDestination(wl_client* client, wl_resource* resource, int32_t width, int32_t height)
{
    auto* test_vp = GetUserDataAs<TestViewport>(resource);
    DCHECK(test_vp);
