...
struct ServerConfig {
    TestZcrTextInputExtensionV1::Version text_input_extension_version = TestZcrTextInputExtensionV1::Version::kV14;
    ZWPTextInputWrapperType text_input_wrapper_type = ZWPTextInputWrapperType::kV1;
    TestCompositor::Version compositor_version = TestCompositor::Version::kV4;
    PrimarySelectionProtocol primary_selection_protocol = PrimarySelectionProtocol::kNone;
    ShouldUseExplicitSynchronizationProtocol use_explicit_synchronization = ShouldUseExplicitSynchronizationProtocol::kUse;
    ShouldUseLinuxDrmSyncobjProtocol use_linux_drm_syncobj = ShouldUseLinuxDrmSyncobjProtocol::kNone;
    EnableAuraShellProtocol enable_aura_shell = EnableAuraShellProtocol::kDisabled;
...
class TestWaylandServerThread : public TestOutput::Delegate, public base::Thread, base::MessagePumpEpoll::FdWatcher {
public:
    ...
    template <typename T> T* GetObject(uint32_t id)
    {
    ...
    // TestOutput::Delegate:
    void OnTestOutputFlush(TestOutput* test_output, const TestOutputMetrics& metrics) override;
    void OnTestOutputGlobalDestroy(TestOutput* test_output) override;

    TestDataDeviceManager* data_device_manager()
    {
    ...
    TestZwpLinuxExplicitSynchronizationV1* zwp_linux_explicit_synchronization_v1()
    {
    ...
    TestWpPointerGestures& wp_pointer_gestures()
    {
        return wp_pointer_gestures_;
    }
    ...
    MockXdgActivationV1* xdg_activation_v1()
    {
...
private:
    ...
    bool SetupExplicitSynchronizationProtocol(ShouldUseExplicitSynchronizationProtocol usage);
    bool SetupLinuxDrmSyncobjProtocol(ShouldUseLinuxDrmSyncobjProtocol usage);

    std::unique_ptr<base::MessagePump> CreateMessagePump(base::OnceClosure closure);
    ...
    // server tread.
    static void ProtocolLogger(void* user_data, enum wl_protocol_logger_type direction, const struct wl_protocol_logger_message* message);
