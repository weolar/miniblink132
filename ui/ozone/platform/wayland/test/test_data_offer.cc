...
void DataOfferAccept(wl_client* client, wl_resource* resource, uint32_t serial, const char* mime_type)
{
    NOTIMPLEMENTED();
}
...
void DataOfferSetActions(wl_client* client, wl_resource* resource, uint32_t dnd_actions, uint32_t preferred_action)
{
    GetUserDataAs<TestDataOffer>(resource)->SetActions(dnd_actions, preferred_action);
}

struct WlDataOfferImpl : public TestSelectionOffer::Delegate {
    explicit WlDataOfferImpl(TestDataOffer* offer)
        : offer_(offer)
    {
    }
...
const struct wl_data_offer_interface kTestDataOfferImpl
    = { DataOfferAccept, &TestSelectionOffer::Receive, DataOfferDestroy, DataOfferFinish, DataOfferSetActions };
...
{
    if (!ResourceHasImplementation(resource, &wl_data_offer_interface, &kTestDataOfferImpl)) {
        return nullptr;
    }
    return GetUserDataAs<TestDataOffer>(resource);
}

void TestDataOffer::SetActions(uint32_t dnd_actions, uint32_t preferred_action)
{
