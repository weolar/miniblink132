...
public:
    ...
    MockDrmSyncobjIoctlWrapper& operator=(const MockDrmSyncobjIoctlWrapper&) = delete;

    MOCK_METHOD(int, SyncobjTransfer, (uint32_t dst_handle, uint64_t dst_point, uint32_t src_handle, uint64_t src_point, uint32_t flags), (override));
    MOCK_METHOD(int, SyncobjImportSyncFile, (uint32_t handle, int sync_file_fd), (override));
    MOCK_METHOD(int, SyncobjExportSyncFile, (uint32_t handle, int* sync_file_fd), (override));
    MOCK_METHOD(int, SyncobjEventfd, (uint32_t handle, uint64_t point, int ev_fd, uint32_t flags), (override));
