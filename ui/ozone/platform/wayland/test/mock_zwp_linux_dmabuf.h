...
public:
    ...
    MOCK_METHOD3(CreateParams, void(wl_client* client, wl_resource* resource, uint32_t params_id));

    const std::vector<raw_ptr<TestZwpLinuxBufferParamsV1, VectorExperimental>>& buffer_params() const
    {
...
private:
    std::vector<raw_ptr<TestZwpLinuxBufferParamsV1, VectorExperimental>> buffer_params_;
};
