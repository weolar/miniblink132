...
void SetWindowGeometry(wl_client* client, wl_resource* resource, int32_t x, int32_t y, int32_t width, int32_t height)
{
    GetUserDataAs<MockXdgSurface>(resource)->SetWindowGeometry({ x, y, width, height });
}
...
void SetMaxSize(wl_client* client, wl_resource* resource, int32_t width, int32_t height)
{
    auto* toplevel = GetUserDataAs<MockXdgTopLevel>(resource);
...
void SetMinSize(wl_client* client, wl_resource* resource, int32_t width, int32_t height)
{
    auto* toplevel = GetUserDataAs<MockXdgTopLevel>(resource);
...
void GetTopLevel(wl_client* client, wl_resource* resource, uint32_t id)
{
    ...
    if (surface->xdg_toplevel()) {
        wl_resource_post_error(resource, XDG_SURFACE_ERROR_ALREADY_CONSTRUCTED, "surface has already been constructed");
        return;
    }
    wl_resource* xdg_toplevel_resource = wl_resource_create(client, &xdg_toplevel_interface, 1, id);
...
void GetXdgPopup(struct wl_client* client, struct wl_resource* resource, uint32_t id, struct wl_resource* parent, struct wl_resource* positioner_resource)
{
    auto* mock_xdg_surface = GetUserDataAs<MockXdgSurface>(resource);
    wl_resource* current_resource = mock_xdg_surface->resource();
    if (current_resource
        && (ResourceHasImplementation(current_resource, &xdg_popup_interface, &kXdgPopupImpl)
            || ResourceHasImplementation(current_resource, &xdg_positioner_interface, &kTestXdgPositionerImpl))) {
        wl_resource_post_error(resource, XDG_SURFACE_ERROR_ALREADY_CONSTRUCTED, "surface has already been constructed");
        return;
    }

    wl_resource* xdg_popup_resource = CreateResourceWithImpl<::testing::NiceMock<TestXdgPopup>>(
        client, &xdg_popup_interface, wl_resource_get_version(resource), &kXdgPopupImpl, id, resource);
    ...
    if (test_xdg_popup->size().IsEmpty() || test_xdg_popup->anchor_rect().IsEmpty()) {
        wl_resource_post_error(resource, XDG_WM_BASE_ERROR_INVALID_POSITIONER, "Positioner object is not complete");
        return;
    }
...
MockXdgTopLevel::MockXdgTopLevel(wl_resource* resource, const void* implementation)
    : ServerObject(resource)
{
