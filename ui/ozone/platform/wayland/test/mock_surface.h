...
extern const struct zwp_linux_surface_synchronization_v1_interface kMockZwpLinuxSurfaceSynchronizationImpl;
...
public:
    ...
    MOCK_METHOD4(Damage, void(int32_t x, int32_t y, int32_t width, int32_t height));
    MOCK_METHOD0(Commit, void());
    MOCK_METHOD1(SetBufferScale, void(int32_t scale));
    MOCK_METHOD1(SetBufferTransform, void(int32_t transform));
    MOCK_METHOD4(DamageBuffer, void(int32_t x, int32_t y, int32_t width, int32_t height));

    void set_xdg_surface(MockXdgSurface* xdg_surface)
    {
        xdg_surface_ = xdg_surface;
    }
    MockXdgSurface* xdg_surface() const
    {
        return xdg_surface_;
    }
    ...
    TestSubSurface* sub_surface() const
    {
        return sub_surface_;
    }

    void set_viewport(TestViewport* viewport)
    {
        viewport_ = viewport;
    }
    ...
    TestFractionalScale* fractional_scale()
    {
        return fractional_scale_;
    }

    void set_overlay_prioritized_surface(TestOverlayPrioritizedSurface* prioritized_surface)
    {
    ...
    TestAugmentedSurface* augmented_surface()
    {
        return augmented_surface_;
    }
    ...
    void set_blending(TestAlphaBlending* blending)
    {
        blending_ = blending;
    }
    TestAlphaBlending* blending()
    {
        return blending_;
    }

    gfx::Rect opaque_region() const
    {
        return opaque_region_;
    }
    gfx::Rect input_region() const
    {
    ...
    void set_linux_buffer_release(wl_resource* buffer, wl_resource* linux_buffer_release)
    {
    ...
    wl_resource* attached_buffer() const
    {
        return attached_buffer_;
    }
    wl_resource* prev_attached_buffer() const
    {
        return prev_attached_buffer_;
    }

    bool has_role() const
    {
        return !!xdg_surface_ || !!sub_surface_;
    }
    ...
    void ReleaseBufferFenced(wl_resource* buffer, gfx::GpuFenceHandle release_fence);
    void SendFrameCallback();

    int32_t buffer_scale() const
    {
        return buffer_scale_;
    }
    void set_buffer_scale(int32_t buffer_scale)
    {
...
private:
    ...
    raw_ptr<TestOverlayPrioritizedSurface, AcrossTasksDanglingUntriaged> prioritized_surface_ = nullptr;
    raw_ptr<TestAugmentedSurface, AcrossTasksDanglingUntriaged> augmented_surface_ = nullptr;
    ...
    base::flat_map<wl_resource*, raw_ptr<wl_resource, CtnExperimental>> linux_buffer_releases_;

    raw_ptr<wl_resource, AcrossTasksDanglingUntriaged> attached_buffer_ = nullptr;
    raw_ptr<wl_resource, AcrossTasksDanglingUntriaged> prev_attached_buffer_ = nullptr;
