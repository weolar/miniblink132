...
{
    if (wl_resource_get_version(resource()) >= ZAURA_OUTPUT_DISPLAY_ID_SINCE_VERSION) {
        auto display_id = ui::wayland::ToWaylandDisplayIdPair(metrics.aura_display_id);
        zaura_output_send_display_id(resource(), display_id.high, display_id.low);
    }

    const auto insets = metrics.aura_logical_insets;
    zaura_output_send_insets(resource(), insets.top(), insets.left(), insets.bottom(), insets.right());

    zaura_output_send_device_scale_factor(resource(), base::bit_cast<uint32_t>(metrics.aura_device_scale_factor));

    zaura_output_send_logical_transform(resource(), metrics.aura_logical_transform);
}
