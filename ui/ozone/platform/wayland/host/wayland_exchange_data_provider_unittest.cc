...
TEST(WaylandExchangeDataProviderTest, ExtractPickledData)
{
    ...
    EXPECT_FALSE(provider.ExtractData(kMimeTypeDataTransferCustomData, &extracted));
    ...
    provider.SetPickledData(ClipboardFormatType::DataTransferCustomType(), pickle);
    EXPECT_TRUE(provider.ExtractData(kMimeTypeDataTransferCustomData, &extracted));

    // Ensure Pickle "reconstruction" works as expected.
    std::string read_pickled_str;
    base::Pickle read_pickle = base::Pickle::WithData(base::as_byte_span(extracted));
...
TEST(WaylandExchangeDataProviderTest, FileContents)
{
    ...
    std::optional<OSExchangeDataProvider::FileContentsInfo> file_contents = provider.GetFileContents();
