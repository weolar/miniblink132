...
public:
    using WaylandZcrColorSpaceDoneCallback = base::OnceCallback<void(const gfx::ColorSpace&)>;
    ...
    zcr_color_space_v1* zcr_color_space() const
    {
        return zcr_color_space_.get();
    }
...
private:
    ...
    static void OnIccFile(void* data, zcr_color_space_v1* cs, int32_t icc, uint32_t icc_size);
    static void OnNames(void* data, zcr_color_space_v1* cs, uint32_t eotf, uint32_t chromaticity, uint32_t whitepoint);
    static void OnParams(void* data, zcr_color_space_v1* cs, uint32_t eotf, uint32_t primary_r_x, uint32_t primary_r_y, uint32_t primary_g_x,
        uint32_t primary_g_y, uint32_t primary_b_x, uint32_t primary_b_y, uint32_t whitepoint_x, uint32_t whitepoint_y);
    static void OnCompleteNames(void* data, zcr_color_space_v1* cs, uint32_t eotf, uint32_t chromaticity, uint32_t whitepoint, uint32_t matrix, uint32_t range);
    static void OnCompleteParams(void* data, zcr_color_space_v1* cs, uint32_t eotf, uint32_t matrix, uint32_t range, uint32_t primary_r_x, uint32_t primary_r_y,
    ...
    std::array<std::optional<gfx::ColorSpace>, static_cast<uint8_t>(InformationType::kMaxValue) + 1> gathered_information;
