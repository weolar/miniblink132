...
// static
void WaylandCursorShape::Instantiate(WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    CHECK_EQ(interface, kInterfaceName) << "Expected \"" << kInterfaceName << "\" but got \"" << interface << "\"";

    if (connection->cursor_shape_ || !wl::CanBind(interface, version, kMinVersion, kMinVersion)) {
        return;
    }

    auto cursor_shape_manager = wl::Bind<wp_cursor_shape_manager_v1>(registry, name, kMinVersion);
    if (!cursor_shape_manager) {
        LOG(ERROR) << "Failed to bind wp_cursor_shape_manager_v1";
        return;
    }
    connection->cursor_shape_ = std::make_unique<WaylandCursorShape>(cursor_shape_manager.release(), connection);
}

WaylandCursorShape::WaylandCursorShape(wp_cursor_shape_manager_v1* cursor_shape, WaylandConnection* connection)
    : wp_cursor_shape_manager_v1_(cursor_shape)
...
wp_cursor_shape_device_v1* WaylandCursorShape::GetShapeDevice()
{
    ...
    if (!wp_cursor_shape_device_v1_.get()) {
        ...
        wp_cursor_shape_device_v1_.reset(wp_cursor_shape_manager_v1_get_pointer(wp_cursor_shape_manager_v1_.get(), pointer));
    }
...
void WaylandCursorShape::SetCursorShape(uint32_t shape)
{
    ...
    wp_cursor_shape_device_v1_set_shape(GetShapeDevice(), pointer_enter_serial->value, shape);
}
