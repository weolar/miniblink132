...
public:
    ...
    WaylandBufferBacking(const WaylandConnection* connection, uint32_t buffer_id, const gfx::Size& size, uint32_t format = DRM_FORMAT_INVALID);
...
private:
    ...
    virtual void RequestBufferHandle(base::OnceCallback<void(wl::Object<wl_buffer>)> callback) = 0;
    ...
    base::flat_map<WaylandSurface*, std::unique_ptr<WaylandBufferHandle>> buffer_handles_;
};
