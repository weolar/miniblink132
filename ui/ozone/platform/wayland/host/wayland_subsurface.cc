...
gfx::RectF AdjustSubsurfaceBounds(const gfx::RectF& bounds_px, const gfx::RectF& parent_bounds_px, float buffer_scale)
{
    const auto bounds_dip = gfx::ScaleRect(bounds_px, 1.0f / buffer_scale);
    const auto parent_bounds_dip = gfx::ScaleRect(parent_bounds_px, 1.0f / buffer_scale);
...
WaylandSubsurface::WaylandSubsurface(WaylandConnection* connection, WaylandWindow* parent)
...
void WaylandSubsurface::CreateSubsurface()
{
    ...
    if (connection_->surface_augmenter()) {
        ...
        // high enough.
        augmented_subsurface_ = connection_->surface_augmenter()->CreateAugmentedSubSurface(subsurface_.get());
    }
}

bool WaylandSubsurface::ConfigureAndShowSurface(const gfx::RectF& bounds_px, const gfx::RectF& parent_bounds_px, const std::optional<gfx::Rect>& clip_rect_px,
    const absl::variant<gfx::OverlayTransform, gfx::Transform>& transform, float buffer_scale, WaylandSubsurface* new_below, WaylandSubsurface* new_above)
{
    ...
    auto bounds_dip_in_parent_surface = AdjustSubsurfaceBounds(bounds_px, parent_bounds_px, buffer_scale);
    if (bounds_dip_in_parent_surface.origin() != position_dip_) {
        ...
        if (augmented_subsurface_) {
            DCHECK(connection_->surface_augmenter()->SupportsSubpixelAccuratePosition());
            augmented_sub_surface_set_position(
                augmented_subsurface_.get(), wl_fixed_from_double(bounds_dip_in_parent_surface.x()), wl_fixed_from_double(bounds_dip_in_parent_surface.y()));
        } else {
            gfx::Point origin_in_parent = gfx::ToCeiledPoint(bounds_dip_in_parent_surface.origin());
            wl_subsurface_set_position(subsurface_.get(), origin_in_parent.x(), origin_in_parent.y());
        }
    ...
    if (augmented_subsurface_ && connection_->surface_augmenter()->SupportsTransform()) {
        // If the old and new transforms are both enums, there's no need to update
        // the matrix transform.
        if ((absl::holds_alternative<gfx::Transform>(transform_) || absl::holds_alternative<gfx::Transform>(transform)) && transform_ != transform) {
            transform_ = transform;
            wl_array transform_data;
            wl_array_init(&transform_data);
            wl::TransformToWlArray(transform_, transform_data);

            augmented_sub_surface_set_transform(augmented_subsurface_.get(), &transform_data);
            needs_commit = true;
