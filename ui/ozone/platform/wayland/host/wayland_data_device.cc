...
WaylandDataDevice::WaylandDataDevice(WaylandConnection* connection, wl_data_device* data_device)
    : WaylandDataDeviceBase(connection)
    , data_device_(data_device)
{
    static constexpr wl_data_device_listener kDataDeviceListener
        = { .data_offer = &OnDataOffer, .enter = &OnEnter, .leave = &OnLeave, .motion = &OnMotion, .drop = &OnDrop, .selection = &OnSelection };
...
void WaylandDataDevice::StartDrag(
    const WaylandDataSource& data_source, const WaylandWindow& origin_window, uint32_t serial, wl_surface* icon_surface, DragDelegate* delegate)
{
    DCHECK(delegate);
    CHECK(!drag_delegate_);
    drag_delegate_ = delegate;

    wl_data_device_start_drag(data_device_.get(), data_source.data_source(), origin_window.root_surface()->surface(), icon_surface, serial);
...
void WaylandDataDevice::SetSelectionSource(WaylandDataSource* source, uint32_t serial)
{
...
// static
void WaylandDataDevice::OnDataOffer(void* data, wl_data_device* data_device, wl_data_offer* offer)
{
...
void WaylandDataDevice::OnEnter(void* data, wl_data_device* data_device, uint32_t serial, wl_surface* surface, wl_fixed_t x, wl_fixed_t y, wl_data_offer* offer)
{
    auto* self = static_cast<WaylandDataDevice*>(data);
    ...
    self->drag_delegate_->OnDragEnter(window, gfx::PointF(wl_fixed_to_double(x), wl_fixed_to_double(y)), timestamp, serial);

    self->connection()->Flush();
}

void WaylandDataDevice::OnMotion(void* data, wl_data_device* data_device, uint32_t time, wl_fixed_t x, wl_fixed_t y)
{
    auto* self = static_cast<WaylandDataDevice*>(data);
    if (self->drag_delegate_) {
        self->drag_delegate_->OnDragMotion(gfx::PointF(wl_fixed_to_double(x), wl_fixed_to_double(y)), wl::EventMillisecondsToTimeTicks(time));
    }
}
...
void WaylandDataDevice::OnSelection(void* data, wl_data_device* data_device, wl_data_offer* offer)
{
    auto* self = static_cast<WaylandDataDevice*>(data);
    DCHECK(self);
