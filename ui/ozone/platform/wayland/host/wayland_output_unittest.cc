...
public:
    MOCK_METHOD(void, OnOutputHandleMetrics, (const WaylandOutput::Metrics& metrics), (override));
};
...
TEST_F(WaylandOutputTest, ScaleFactorCalculation)
{
    ...
    wl_output->xdg_output_ = std::make_unique<XDGOutput>(nullptr);
    wl_output->xdg_output_->logical_size_ = gfx::Size(kLogicalSideLength, kLogicalSideLength);
    wl_output->physical_size_ = gfx::Size(compositor_scale * kLogicalSideLength, compositor_scale * kLogicalSideLength);
...
TEST_F(WaylandOutputTest, WaylandOutputIsReady)
{
    ...
    PostToServerAndWait([&test_output](wl::TestWaylandServerThread* server) {
        test_output = server->CreateAndInitializeOutput(wl::TestOutputMetrics({ 0, 0, 800, 600 }));
    ...
    auto pair_it = base::ranges::find_if_not(all_outputs, [&](auto& pair) { return pair.first == primary_output->output_id(); });
    ...
    PostToServerAndWait([&test_output](wl::TestWaylandServerThread* server) { test_output->Flush(); });
