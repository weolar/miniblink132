...
public:
    ...
    WaylandZcrColorManagerTest& operator=(const WaylandZcrColorManagerTest&) = delete;
...
TEST_P(WaylandZcrColorManagerTest, DISABLED_CreateColorManagementOutput)
{
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        auto params_vector = server->zcr_color_manager_v1()->color_management_outputs();
        for (wl::TestZcrColorManagementOutputV1* mock_params : params_vector) {
            mock_params->SetGfxColorSpace(gfx::ColorSpace::CreateHDR10());
            zcr_color_management_output_v1_send_color_space_changed(mock_params->resource());
        }
    });
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        ...
        for (wl::TestZcrColorManagementOutputV1* mock_params : params_vector) {
            ...
            EXPECT_EQ(zcr_color_space->GetGfxColorSpace(), gfx::ColorSpace::CreateHDR10());
...
TEST_P(WaylandZcrColorManagerTest, DISABLED_CreateColorManagementSurface)
{
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        auto params_vector = server->zcr_color_manager_v1()->color_management_surfaces();
    ...
    connection_->zcr_color_manager()->GetColorSpace(gfx::ColorSpace::CreateHDR10());
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        auto params_vector = server->zcr_color_manager_v1()->color_management_surfaces();
        EXPECT_EQ(gfx::ColorSpace::CreateHDR10(), params_vector.front()->GetGfxColorSpace());
    });
}
...
TEST_P(WaylandZcrColorManagerTest, DISABLED_DoNotSetInvaliColorSpace)
{
    ...
    gfx::ColorSpace invalid_space = gfx::ColorSpace(gfx::ColorSpace::PrimaryID::INVALID, gfx::ColorSpace::TransferID::INVALID);
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        auto params_vector = server->zcr_color_manager_v1()->color_management_surfaces();
        EXPECT_EQ(gfx::ColorSpace::CreateSRGB(), params_vector.front()->GetGfxColorSpace());
    });
}

INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandZcrColorManagerTest, Values(wl::ServerConfig {}));
