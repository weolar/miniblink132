...
TEST_P(WaylandWindowManagerTest, GetWindow)
{
    ...
    auto window2 = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::Rect(0, 0, 300, 300), &delegate);
...
TEST_P(WaylandWindowManagerTest, GetWindowWithLargestBounds)
{
    ...
    auto window2 = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::Rect(0, 0, 3000, 3000), &delegate);
...
TEST_P(WaylandWindowManagerTest, GetAllWindows)
{
    ...
    auto window2 = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, kDefaultBounds, &delegate);

    windows = manager_->GetAllWindows();
    EXPECT_EQ(2u, windows.size());
}

INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandWindowManagerTest, Values(wl::ServerConfig {}));
