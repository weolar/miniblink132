...
PlatformWindowOcclusionState WaylandOcclusionStateToPlatformWindowOcclusionState(uint32_t mode)
{
    switch (mode) {
...
WaylandZAuraSurface::WaylandZAuraSurface(zaura_shell* zaura_shell, wl_surface* wl_surface, WaylandConnection* connection)
...
bool WaylandZAuraSurface::SetFullscreenMode(zaura_surface_fullscreen_mode mode)
{
...
bool WaylandZAuraSurface::IntentToSnap(zaura_surface_snap_direction snap_direction)
{
...
bool WaylandZAuraSurface::SetInitialWorkspace(int workspace)
{
    if (IsSupported(ZAURA_SURFACE_SET_INITIAL_WORKSPACE_SINCE_VERSION)) {
        zaura_surface_set_initial_workspace(zaura_surface_.get(), base::NumberToString(workspace).c_str());
        return true;
    }
    return false;
}
...
bool WaylandZAuraSurface::ShowTooltip(const std::u16string& text, const gfx::Point& position, zaura_surface_tooltip_trigger trigger,
    const base::TimeDelta& show_delay, const base::TimeDelta& hide_delay)
{
    if (IsSupported(ZAURA_SURFACE_SHOW_TOOLTIP_SINCE_VERSION)) {
        zaura_surface_show_tooltip(zaura_surface_.get(), base::UTF16ToUTF8(text).c_str(), position.x(), position.y(), trigger,
            // Cast `show_delay` and `hide_delay` into int32_t as TimeDelta should
            // not be larger than what can be handled in int32_t
            base::saturated_cast<uint32_t>(show_delay.InMilliseconds()), base::saturated_cast<uint32_t>(hide_delay.InMilliseconds()));
        return true;
    }
    return false;
}
...
// static.
void WaylandZAuraSurface::OnOcclusionChanged(void* data, zaura_surface* surface, wl_fixed_t occlusion_fraction, uint32_t occlusion_reason)
{
...
// static.
void WaylandZAuraSurface::OnLockFrameNormal(void* data, zaura_surface* surface)
{
    auto* self = static_cast<WaylandZAuraSurface*>(data);
    if (self && self->delegate_) {
...
// static.
void WaylandZAuraSurface::OnUnlockFrameNormal(void* data, zaura_surface* surface)
{
    auto* self = static_cast<WaylandZAuraSurface*>(data);
    if (self && self->delegate_) {
...
// static.
void WaylandZAuraSurface::OnOcclusionStateChanged(void* data, zaura_surface* surface, uint32_t mode)
{
    auto* self = static_cast<WaylandZAuraSurface*>(data);
    if (self && self->delegate_) {
        self->delegate_->OcclusionStateChanged(WaylandOcclusionStateToPlatformWindowOcclusionState(mode));
    }
}

// static.
void WaylandZAuraSurface::OnDeskChanged(void* data, zaura_surface* surface, int state)
{
    auto* self = static_cast<WaylandZAuraSurface*>(data);
    if (self && self->delegate_) {
...
// static.
void WaylandZAuraSurface::OnTooltipShown(void* data, zaura_surface* surface, const char* text, int32_t x, int32_t y, int32_t width, int32_t height)
{
    auto* self = static_cast<WaylandZAuraSurface*>(data);
    if (self && self->delegate_) {
