...
public:
    explicit WaylandZcrColorManagementOutput(WaylandOutput* wayland_output, zcr_color_management_output_v1* management_output);
    WaylandZcrColorManagementOutput(const WaylandZcrColorManagementOutput&) = delete;
    WaylandZcrColorManagementOutput& operator=(const WaylandZcrColorManagementOutput&) = delete;
    ~WaylandZcrColorManagementOutput();

    gfx::ColorSpace* gfx_color_space() const
    {
        return gfx_color_space_.get();
    }
    WaylandZcrColorSpace* color_space() const
    {
...
private:
    ...
    static void OnColorSpaceChanged(void* data, zcr_color_management_output_v1* cmo);
    static void OnExtendedDynamicRange(void* data, zcr_color_management_output_v1* cmo, uint32_t value);
