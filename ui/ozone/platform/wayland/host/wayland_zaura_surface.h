...
public:
    ...
    class Delegate {
    public:
        virtual void OcclusionChanged(wl_fixed_t occlusion_fraction, uint32_t occlusion_reason)
        {
        }
        ...
        virtual void OcclusionStateChanged(PlatformWindowOcclusionState occlusion_state)
        {
        }
        ...
        virtual void TooltipShown(const char* text, int32_t x, int32_t y, int32_t width, int32_t height)
        {
        }
    ...
    WaylandZAuraSurface(zaura_shell* zaura_shell, wl_surface* wl_surface, WaylandConnection* connection);
    ...
    bool UnsetPin();
    bool ShowTooltip(const std::u16string& text, const gfx::Point& position, zaura_surface_tooltip_trigger trigger, const base::TimeDelta& show_delay,
...
private:
    ...
    static void OnOcclusionChanged(void* data, zaura_surface* surface, wl_fixed_t occlusion_fraction, uint32_t occlusion_reason);
    ...
    static void OnOcclusionStateChanged(void* data, zaura_surface* surface, uint32_t mode);
    ...
    static void OnTooltipShown(void* data, zaura_surface* surface, const char* text, int32_t x, int32_t y, int32_t width, int32_t height);
