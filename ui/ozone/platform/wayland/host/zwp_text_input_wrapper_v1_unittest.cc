...
public:
    void SetUp() override
    {
        ...
        wrapper_ = std::make_unique<ZWPTextInputWrapperV1>(
            connection_.get(), &test_client_, connection_->text_input_manager_v1(), connection_->text_input_extension_v1());
    }
...
TEST_F(ZWPTextInputWrapperV1Test, OnPreeditString)
{
    ...
    EXPECT_CALL(test_client_,
        OnPreeditString(kPreeditString,
            std::vector<ZWPTextInputWrapperClient::SpanStyle> {
                { 0, static_cast<uint32_t>(kPreeditString.size()), { { ImeTextSpan::Type::kComposition, ImeTextSpan::Thickness::kThin } } } },
    ...
    PostToServerAndWait([kPreeditString](wl::TestWaylandServerThread* server) {
        ...
        zwp_text_input_v1_send_preedit_cursor(text_input->resource(), kPreeditCursor);
        zwp_text_input_v1_send_preedit_styling(text_input->resource(), 0, kPreeditString.size(), ZWP_TEXT_INPUT_V1_PREEDIT_STYLE_UNDERLINE);
        zwp_text_input_v1_send_preedit_string(text_input->resource(), server->GetNextSerial(), kPreeditString.data(), "");
    });
}

TEST_F(ZWPTextInputWrapperV1Test, FinalizeVirtualKeyboardChangesShowInputPanel)
{
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        InSequence s;
        EXPECT_CALL(*server->text_input_manager_v1()->text_input(), ShowInputPanel());
        EXPECT_CALL(*server->text_input_extension_v1()->extended_text_input(), FinalizeVirtualKeyboardChanges());
    });
...
TEST_F(ZWPTextInputWrapperV1Test, FinalizeVirtualKeyboardChangesHideInputPanel)
{
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        InSequence s;
        EXPECT_CALL(*server->text_input_manager_v1()->text_input(), HideInputPanel());
        EXPECT_CALL(*server->text_input_extension_v1()->extended_text_input(), FinalizeVirtualKeyboardChanges());
    });
...
TEST_F(ZWPTextInputWrapperV1Test, FinalizeVirtualKeyboardChangesMultipleInputPanelChanges)
{
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
...
TEST_F(ZWPTextInputWrapperV1Test, OnKeySym_TimestampPropagated)
{
    ...
    PostToServerAndWait([test_time](wl::TestWaylandServerThread* server) {
        zwp_text_input_v1_send_keysym(server->text_input_manager_v1()->text_input()->resource(), 0, test_time, 0, 0, 0);
    });
}
...
TEST_F(ZWPTextInputWrapperV1Test, OnInsertImageWithLargeURL)
{
    ...
    PostToServerAndWait([&mime_type, &charset, &raw_bytes, &memfd](wl::TestWaylandServerThread* server) {
        zcr_extended_text_input_v1_send_insert_image_with_large_url(
            server->text_input_extension_v1()->extended_text_input()->resource(), mime_type.c_str(), charset.c_str(), memfd.get(), raw_bytes.size());
    });
}
