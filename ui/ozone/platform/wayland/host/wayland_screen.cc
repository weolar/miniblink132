    ...
    , weak_factory_(this)
{
    ...
    // BGRA_8888).
    auto buffer_formats = connection_->buffer_manager_host()->GetSupportedBufferFormats();
...
void WaylandScreen::OnOutputAddedOrUpdated(const WaylandOutput::Metrics& metrics)
{
    ...
    DISPLAY_LOG(EVENT) << "Displays updated, count: " << display_list_.displays().size();
    ...
    if (display_id == GetPrimaryDisplay().id()) {
        ...
        for (const auto& display : display_list_.displays()) {
            if (display.id() != display_id) {
                display_list_.AddOrUpdateDisplay(display, display::DisplayList::Type::PRIMARY);
                break;
            }
        }
    }
...
void WaylandScreen::AddOrUpdateDisplay(const WaylandOutput::Metrics& metrics)
{
    ...
    display::Display::Rotation panel_rotation = WaylandTransformToRotation(metrics.panel_transform);
    ...
    display::Display::Rotation rotation = WaylandTransformToRotation(metrics.logical_transform);
    changed_display.set_rotation(rotation);

    gfx::Size size_in_pixels(metrics.physical_size);
    if (panel_rotation == display::Display::Rotation::ROTATE_90 || panel_rotation == display::Display::Rotation::ROTATE_270) {
        size_in_pixels.Transpose();
    }
    size_in_pixels.Enlarge(-metrics.physical_overscan_insets.width(), -metrics.physical_overscan_insets.height());
    ...
    if (!metrics.logical_size.IsEmpty()) {
        ...
        changed_display.SetScaleAndBounds(metrics.scale_factor, gfx::Rect(size_in_pixels));
    ...
    color_spaces.SetOutputBufferFormats(image_format_no_alpha_.value(), image_format_alpha_.value());
...
{
    auto iter = std::find_if(display_id_map_.begin(), display_id_map_.end(), [display_id](auto pair) { return pair.second == display_id; });
    if (iter != display_id_map_.end())
        return iter->first;
    return 0;
}
...
display::Display WaylandScreen::GetPrimaryDisplay() const
{
    ...
    return display_list_.displays().empty() ? display::Display::GetDefaultDisplay() : *display_list_.GetPrimaryDisplayIterator();
}

display::Display WaylandScreen::GetDisplayForAcceleratedWidget(gfx::AcceleratedWidget widget) const
{
    ...
    if (display_id_map_.find(entered_output_id.value()) == display_id_map_.end()) {
...
gfx::Point WaylandScreen::GetCursorScreenPoint() const
{
    ...
    if (connection_->window_manager()->GetCurrentPointerOrTouchFocusedWindow() && cursor_position)
...
gfx::AcceleratedWidget WaylandScreen::GetAcceleratedWidgetAtScreenPoint(const gfx::Point& point) const
{
    // It is safe to check only for focused windows and test if they contain the
    // point or not.
    auto* window = connection_->window_manager()->GetCurrentPointerOrTouchFocusedWindow();
    if (window && window->GetBoundsInDIP().Contains(point))
        return window->GetWidget();
    return gfx::kNullAcceleratedWidget;
}

gfx::AcceleratedWidget WaylandScreen::GetLocalProcessWidgetAtPoint(const gfx::Point& point, const std::set<gfx::AcceleratedWidget>& ignore) const
{
    auto widget = GetAcceleratedWidgetAtScreenPoint(point);
    return !widget || base::Contains(ignore, widget) ? gfx::kNullAcceleratedWidget : widget;
}

display::Display WaylandScreen::GetDisplayNearestPoint(const gfx::Point& point) const
{
...
display::Display WaylandScreen::GetDisplayMatching(const gfx::Rect& match_rect) const
{
    if (match_rect.IsEmpty())
        return GetDisplayNearestPoint(match_rect.origin());

    const display::Display* display_matching = display::FindDisplayWithBiggestIntersection(display_list_.displays(), match_rect);
    return display_matching ? *display_matching : GetPrimaryDisplay();
}

std::unique_ptr<WaylandScreen::WaylandScreenSaverSuspender> WaylandScreen::WaylandScreenSaverSuspender::Create(WaylandScreen& screen)
{
...
WaylandScreen::WaylandScreenSaverSuspender::WaylandScreenSaverSuspender(WaylandScreen& screen)
...
std::unique_ptr<PlatformScreen::PlatformScreenSaverSuspender> WaylandScreen::SuspendScreenSaver()
{
...
bool WaylandScreen::SetScreenSaverSuspended(bool suspend)
{
    ...
    if (suspend) {
        ...
        idle_inhibitor_ = connection_->zwp_idle_inhibit_manager()->CreateInhibitor(current_window->root_surface()->surface());
    } else {
...
base::TimeDelta WaylandScreen::CalculateIdleTime() const
{
    ...
    if (!org_gnome_mutter_idle_monitor_)
        org_gnome_mutter_idle_monitor_ = std::make_unique<OrgGnomeMutterIdleMonitor>();
...
base::Value::List WaylandScreen::GetGpuExtraInfo(const gfx::GpuExtraInfo& gpu_extra_info)
{
    auto values = GetDesktopEnvironmentInfo();
    std::vector<std::string> protocols;
    for (const auto& protocol_and_version : connection_->available_globals()) {
        protocols.push_back(base::StringPrintf("%s:%u", protocol_and_version.first.c_str(), protocol_and_version.second));
    }
    values.Append(display::BuildGpuInfoEntry("Interfaces exposed by the Wayland compositor", base::JoinString(protocols, " ")));
    StorePlatformNameIntoListOfValues(values, "wayland");
    return values;
}

std::optional<float> WaylandScreen::GetPreferredScaleFactorForAcceleratedWidget(gfx::AcceleratedWidget widget) const
{
    if (const auto* window = connection_->window_manager()->GetWindow(widget)) {
        ...
        return window->GetPreferredScaleFactor().value_or(1.0f) * window->applied_state().ui_scale;
    }
    return std::nullopt;
}
...
bool WaylandScreen::VerifyOutputStateConsistentForTesting() const
{
    ...
    for (const auto& pair : display_id_map_) {
        if (base::ranges::find(displays, pair.second, &display::Display::id) == displays.end()) {
            return false;
        }
    }
    return true;
...
void WaylandScreen::OnDeviceScaleFactorChanged()
{
    ...
    connection_->window_manager()->SetFontScale(linux_ui.display_config().font_scale);
}
...
void WaylandScreen::DumpState(std::ostream& out) const
{
    ...
    for (const auto& display : display_list_.displays()) {
        out << "  display[" << display.id() << "]:" << display.ToString() << std::endl;
    }
    out << "  id_map=";
