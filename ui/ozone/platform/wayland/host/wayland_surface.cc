...
uint32_t TranslatePriority(gfx::OverlayPriorityHint priority_hint)
{
    ...
    case gfx::OverlayPriorityHint::kLowLatencyCanvas:
        priority = OVERLAY_PRIORITIZED_SURFACE_OVERLAY_PRIORITY_PREFERRED_LOW_LATENCY_CANVAS;
        break;
    case gfx::OverlayPriorityHint::kHardwareProtection:
        priority = OVERLAY_PRIORITIZED_SURFACE_OVERLAY_PRIORITY_REQUIRED_HARDWARE_PROTECTION;
        break;
    }
    return priority;
}
...
WaylandSurface::ExplicitReleaseInfoLegacy::ExplicitReleaseInfoLegacy(
    wl::Object<zwp_linux_buffer_release_v1>&& linux_buffer_release, wl_buffer* buffer, ExplicitReleaseCallback explicit_release_callback)
...
WaylandSurface::ExplicitReleaseInfoLegacy::~ExplicitReleaseInfoLegacy() = default;

WaylandSurface::ExplicitReleaseInfoLegacy::ExplicitReleaseInfoLegacy(ExplicitReleaseInfoLegacy&&) = default;

WaylandSurface::ExplicitReleaseInfoLegacy& WaylandSurface::ExplicitReleaseInfoLegacy::operator=(ExplicitReleaseInfoLegacy&&) = default;
...
WaylandSurface::WaylandSurface(WaylandConnection* connection, WaylandWindow* root_window)
    ...
    , use_viewporter_surface_scaling_(connection->supports_viewporter_surface_scaling())
{
    ...
    if (root_window_ && root_window_->parent_window()) {
        preferred_scale_factor_ = root_window_->parent_window()->GetPreferredScaleFactor();
    }
}
...
WaylandSurface::~WaylandSurface()
{
    for (auto& release : linux_buffer_releases_legacy_) {
        ...
        std::move(release.second.explicit_release_callback).Run(release.second.buffer.get(), base::ScopedFD());
    }
}
...
WaylandZAuraSurface* WaylandSurface::CreateZAuraSurface()
{
    ...
    if (!zaura_surface_ && zaura_shell) {
        zaura_surface_ = std::make_unique<WaylandZAuraSurface>(zaura_shell->wl_object(), surface(), connection_);
    }
...
bool WaylandSurface::Initialize()
{
    ...
    if (connection_->fractional_scale_manager_v1()) {
        ...
        fractional_scale_.reset(wp_fractional_scale_manager_v1_get_fractional_scale(connection_->fractional_scale_manager_v1(), surface_.get()));
        wp_fractional_scale_v1_add_listener(fractional_scale_.get(), &kFractionalScaleListener, this);
    }

    if (connection_->viewporter()) {
        viewport_.reset(wp_viewporter_get_viewport(connection_->viewporter(), surface()));
        if (!viewport_) {
    ...
    if (connection_->alpha_compositing()) {
        blending_.reset(zcr_alpha_compositing_v1_get_blending(connection_->alpha_compositing(), surface()));
        if (!blending_) {
    ...
    if (auto* overlay_prioritizer = connection_->overlay_prioritizer()) {
        overlay_priority_surface_ = overlay_prioritizer->CreateOverlayPrioritizedSurface(surface());
    ...
    if (surface_augmenter && root_window() && root_window()->root_surface() != this) {
    ...
    if (auto* content_type_manager = connection_->content_type_manager_v1()) {
        content_type_.reset(wp_content_type_manager_v1_get_surface_content_type(content_type_manager, surface()));
        if (!content_type_) {
            LOG(ERROR) << "Failed to create wp_content_type_v1. Continuing without it.";
        }
    } else {
        static bool log_once = false;
        if (!log_once) {
    ...
    if (auto* zcr_color_manager = connection_->zcr_color_manager()) {
        zcr_color_management_surface_
            = std::make_unique<WaylandZcrColorManagementSurface>(zcr_color_manager->CreateColorManagementSurface(surface()).release(), connection_);
...
void WaylandSurface::set_acquire_fence(gfx::GpuFenceHandle acquire_fence)
{
    ...
    DCHECK(connection_->SupportsExplicitSync() || connection_->UseImplicitSyncInterop());
    if (!acquire_fence.is_null() &&
        ...
        auto status = gfx::GpuFence::GetStatusChangeTime(acquire_fence.Peek(), &ticks);
...
bool WaylandSurface::AttachBuffer(WaylandBufferHandle* buffer_handle)
{
    ...
    if (state_.buffer_id == pending_state_.buffer_id && buffer_handle->released(this)) {
...
void WaylandSurface::Commit(bool flush)
{
    ...
    if (surface_sync_ && acquire_timeline_ && !next_explicit_release_request_.is_null()) {
        auto* buffer_handle = connection_->buffer_manager_host()->GetBufferHandle(this, state_.buffer_id);
        CHECK(buffer_handle);
        ...
        release_timeline->WaitForFenceAvailableAtCurrentSyncPoint(
            base::BindOnce(&WaylandSurface::OnFenceAvailable, weak_factory_.GetWeakPtr(), state_.buffer_id, std::move(next_explicit_release_request_)));
    }
}
...
void WaylandSurface::set_surface_buffer_scale(float scale)
{
    ...
    if (apply_state_immediately_) {
        ...
        if (!use_viewporter_surface_scaling_) {
            ...
            wl_surface_set_buffer_scale(surface_.get(), static_cast<int32_t>(GetWaylandScale(state_)));
        }
        if (root_window_)
...
void WaylandSurface::set_opaque_region(std::optional<std::vector<gfx::Rect>> region_px)
{
    ...
    if (apply_state_immediately_) {
        ...
        wl_surface_set_opaque_region(surface_.get(),
            pending_state_.opaque_region_px.empty() ? nullptr : CreateAndAddRegion(pending_state_.opaque_region_px, GetWaylandScale(pending_state_)).get());
    }
}

void WaylandSurface::set_input_region(std::optional<std::vector<gfx::Rect>> region_px)
{
    pending_state_.input_region_px.clear();
    if (!root_window_)
        return;
    if (root_window_->root_surface() == this && root_window_->ShouldUseNativeFrame()) {
        return;
    }
    if (region_px)
    ...
    if (apply_state_immediately_) {
        ...
        wl_surface_set_input_region(surface_.get(),
            pending_state_.input_region_px.empty() ? nullptr : CreateAndAddRegion(pending_state_.input_region_px, GetWaylandScale(pending_state_)).get());
    }
}

float WaylandSurface::GetWaylandScale(const State& state)
{
    return wl::ClampScale(use_viewporter_surface_scaling_ ? state.buffer_scale_float : std::ceil(state.buffer_scale_float));
}
...
bool WaylandSurface::IsViewportScaled(const State& state)
{
    ...
    gfx::SizeF src_size_px = wl::ApplyWaylandTransform(gfx::SizeF(state.buffer_size_px), wl::ToWaylandTransform(state.buffer_transform));
    if (!state.crop.IsEmpty()) {
        const gfx::RectF crop_transformed = wl::ApplyWaylandTransform(state.crop, gfx::SizeF(1, 1), wl::ToWaylandTransform(state.buffer_transform));
        src_size_px = gfx::ScaleRect(crop_transformed, src_size_px.width(), src_size_px.height()).size();
    }
...
wl::Object<wl_region> WaylandSurface::CreateAndAddRegion(const std::vector<gfx::Rect>& region_px, float buffer_scale)
{
    DCHECK(root_window_);
    wl::Object<wl_region> region(wl_compositor_create_region(connection_->compositor()));
    ...
    for (const auto& rect_px : region_px) {
        ...
        wl_region_add(region.get(), rect.x(), rect.y(), rect.width(), rect.height());
    }
    return region;
}
...
bool WaylandSurface::SetExplicitSyncLegacy()
{
    ...
    if (!surface_sync_legacy_) {
        surface_sync_legacy_.reset(zwp_linux_explicit_synchronization_v1_get_synchronization(connection_->linux_explicit_synchronization_v1(), surface_.get()));
    }
    ...
    if (!pending_state_.acquire_fence.is_null()) {
        zwp_linux_surface_synchronization_v1_set_acquire_fence(surface_sync, pending_state_.acquire_fence.Peek());
    }

    if (!next_explicit_release_request_.is_null()) {
        auto* linux_buffer_release = zwp_linux_surface_synchronization_v1_get_release(surface_sync);
        ...
        LOG_IF(FATAL, !linux_buffer_release) << "Unable to get an explicit release object.";

        static constexpr zwp_linux_buffer_release_v1_listener kBufferReleaseListener = {
            .fenced_release = &OnFencedRelease,
            .immediate_release = &OnImmediateRelease,
        };
        zwp_linux_buffer_release_v1_add_listener(linux_buffer_release, &kBufferReleaseListener, this);

        linux_buffer_releases_legacy_.emplace(linux_buffer_release,
            ExplicitReleaseInfoLegacy(
                wl::Object<zwp_linux_buffer_release_v1>(linux_buffer_release), pending_state_.buffer, std::move(next_explicit_release_request_)));
    }
    return true;
}
...
{
    if (!surface_sync_) {
        surface_sync_.reset(wp_linux_drm_syncobj_manager_v1_get_surface(connection_->linux_drm_syncobj_manager_v1(), surface_.get()));
    }
}
...
std::optional<bool> WaylandSurface::SetExplicitSync()
{
    ...
    auto* buffer_handle = connection_->buffer_manager_host()->GetBufferHandle(this, pending_state_.buffer_id);
    CHECK(buffer_handle);
    auto* release_timeline = buffer_handle->release_timeline();
    ...
    if (!pending_state_.acquire_fence.is_null()) {
        ...
        if (!acquire_syncobj->ImportSyncFdAtCurrentSyncPoint(pending_state_.acquire_fence.Peek())) {
    ...
    wp_linux_drm_syncobj_surface_v1_set_acquire_point(
        surface_sync_.get(), acquire_timeline_->timeline(), acquire_sync_point >> kPointHiShift, acquire_sync_point & kPointLoMask);
    ...
    wp_linux_drm_syncobj_surface_v1_set_release_point(
        surface_sync_.get(), release_timeline->timeline(), release_sync_point >> kPointHiShift, release_sync_point & kPointLoMask);

    return true;
}

void WaylandSurface::OnFenceAvailable(uint32_t buffer_id, ExplicitReleaseCallback callback, base::ScopedFD fd)
{
    auto* buffer_handle = connection_->buffer_manager_host()->GetBufferHandle(this, buffer_id);
    DVLOG(3) << __func__ << " surface=" << surface_.id() << " buffer=" << wl_proxy_get_id(reinterpret_cast<wl_proxy*>(buffer_handle->buffer()))
             << " fence fd=" << fd.get();
    CHECK(buffer_handle);
    std::move(callback).Run(buffer_handle->buffer(), std::move(fd));
}

wl::Object<wl_subsurface> WaylandSurface::CreateSubsurface(WaylandSurface* parent)
{
    DCHECK(parent);
    wl_subcompositor* subcompositor = connection_->subcompositor();
    DCHECK(subcompositor);
    wl::Object<wl_subsurface> subsurface(wl_subcompositor_get_subsurface(subcompositor, surface_.get(), parent->surface_.get()));
    return subsurface;
}
...
std::optional<bool> WaylandSurface::ApplyPendingState()
{
    ...
    if (pending_state_.buffer_id != state_.buffer_id) {
        ...
        // sync can exist.
        if (pending_state_.buffer && !explicit_sync_success.value() && !SetExplicitSyncLegacy() && connection_->UseImplicitSyncInterop()
            && !pending_state_.acquire_fence.is_null()) {
            connection_->buffer_manager_host()->InsertAcquireFence(pending_state_.buffer_id, pending_state_.acquire_fence.Peek());
        }
    }
    ...
    if (pending_state_.buffer_transform != state_.buffer_transform) {
        wl_output_transform wl_transform = wl::ToWaylandTransform(pending_state_.buffer_transform);
    ...
    if (pending_state_.opacity != state_.opacity) {
        DCHECK(blending());
        zcr_blending_v1_set_alpha(blending(), wl_fixed_from_double(pending_state_.opacity));
        needs_commit = true;
    }
    if (pending_state_.use_blending != state_.use_blending) {
        DCHECK(blending());
        zcr_blending_v1_set_blending(
            blending(), pending_state_.use_blending ? ZCR_BLENDING_V1_BLENDING_EQUATION_PREMULT : ZCR_BLENDING_V1_BLENDING_EQUATION_NONE);
        needs_commit = true;
    }

    if (pending_state_.priority_hint != state_.priority_hint) {
        DCHECK(overlay_priority_surface());
        overlay_prioritized_surface_set_overlay_priority(overlay_priority_surface(), TranslatePriority(pending_state_.priority_hint));
        needs_commit = true;
    }

    // Don't set input region when use_native_frame is enabled.
    if (pending_state_.input_region_px != state_.input_region_px || GetWaylandScale(pending_state_) != GetWaylandScale(state_)) {
    ...
    if (pending_state_.opaque_region_px != state_.opaque_region_px || GetWaylandScale(pending_state_) != GetWaylandScale(state_)) {
        wl_surface_set_opaque_region(surface_.get(),
            pending_state_.opaque_region_px.empty() ? nullptr : CreateAndAddRegion(pending_state_.opaque_region_px, GetWaylandScale(pending_state_)).get());
        needs_commit = true;
    }
    ...
    if (pending_state_.background_color != state_.background_color) {
        ...
        if (augmented_surface_get_version(get_augmented_surface()) >= static_cast<uint32_t>(AUGMENTED_SURFACE_SET_BACKGROUND_COLOR_SINCE_VERSION)) {
            wl_array color_data;
            wl_array_init(&color_data);
            if (pending_state_.background_color.has_value())
                wl::SkColorToWlArray(pending_state_.background_color.value(), color_data);

            augmented_surface_set_background_color(get_augmented_surface(), &color_data);
            needs_commit = true;
    ...
    if (pending_state_.rounded_clip_bounds != state_.rounded_clip_bounds) {
        ...
        if (augmented_surface_get_version(get_augmented_surface()) >= AUGMENTED_SURFACE_SET_ROUNDED_CORNERS_CLIP_BOUNDS_SINCE_VERSION) {
            augmented_surface_set_rounded_corners_clip_bounds(get_augmented_surface(), wl_fixed_from_double(rounded_clip_bounds.rect().x()),
                wl_fixed_from_double(rounded_clip_bounds.rect().y()), wl_fixed_from_double(rounded_clip_bounds.rect().width()),
                wl_fixed_from_double(rounded_clip_bounds.rect().height()),
                wl_fixed_from_double(rounded_clip_bounds.GetCornerRadii(gfx::RRectF::Corner::kUpperLeft).x()),
                wl_fixed_from_double(rounded_clip_bounds.GetCornerRadii(gfx::RRectF::Corner::kUpperRight).x()),
                wl_fixed_from_double(rounded_clip_bounds.GetCornerRadii(gfx::RRectF::Corner::kLowerRight).x()),
                wl_fixed_from_double(rounded_clip_bounds.GetCornerRadii(gfx::RRectF::Corner::kLowerLeft).x()));
            needs_commit = true;
        } else if (augmented_surface_get_version(get_augmented_surface()) >= AUGMENTED_SURFACE_SET_ROUNDED_CLIP_BOUNDS_SINCE_VERSION) {
    ...
    if (pending_state_.clip_rect != state_.clip_rect) {
        DCHECK(get_augmented_surface());
        if (connection_->surface_augmenter()->SupportsClipRectOnAugmentedSurface()) {
            std::optional<gfx::RectF> clip_rect = pending_state_.clip_rect;
            if (clip_rect) {
                clip_rect->Scale(1.f / GetWaylandScale(pending_state_));
                augmented_surface_set_clip_rect(get_augmented_surface(), wl_fixed_from_double(clip_rect->x()), wl_fixed_from_double(clip_rect->y()),
                    wl_fixed_from_double(clip_rect->width()), wl_fixed_from_double(clip_rect->height()));
            } else {
                augmented_surface_set_clip_rect(get_augmented_surface(), kMinusOne, kMinusOne, kMinusOne, kMinusOne);
            }
            needs_commit = true;
        }
    }

    if (content_type_ && (pending_state_.contains_video != state_.contains_video)) {
        wp_content_type_v1_set_content_type(content_type_.get(), pending_state_.contains_video ? WP_CONTENT_TYPE_V1_TYPE_VIDEO : WP_CONTENT_TYPE_V1_TYPE_NONE);
        needs_commit = true;
    }
    ...
    gfx::SizeF bounds = wl::ApplyWaylandTransform(gfx::SizeF(pending_state_.buffer_size_px), wl::ToWaylandTransform(pending_state_.buffer_transform));
    ...
    if (connection_->fractional_scale_manager_v1() && root_window_ && root_window_->root_surface() == this && !IsViewportScaled(pending_state_)) {
        gfx::Size old_size_px = crop.IsEmpty()
            ? pending_state_.buffer_size_px
            : gfx::ToFlooredSize(gfx::ScaleRect(crop, pending_state_.buffer_size_px.width(), pending_state_.buffer_size_px.height()).size());
        gfx::Size new_size_dip = gfx::ScaleToFlooredSize(old_size_px, 1.f / pending_state_.buffer_scale_float);
        gfx::Size new_size_px = gfx::ScaleToRoundedSize(new_size_dip, pending_state_.buffer_scale_float);
        if (new_size_px != old_size_px) {
            crop.set_width(static_cast<float>(new_size_px.width()) / static_cast<float>(pending_state_.buffer_size_px.width()));
            crop.set_height(static_cast<float>(new_size_px.height()) / static_cast<float>(pending_state_.buffer_size_px.height()));
            viewport_px = wl::ApplyWaylandTransform(gfx::SizeF(new_size_px), wl::ToWaylandTransform(pending_state_.buffer_transform));
        }
    }
    ...
    if (!use_viewporter_surface_scaling_ && surface_scale_set_ != applying_surface_scale) {
    ...
    wl_fixed_t src_to_set[4] = { wl_fixed_from_int(-1), wl_fixed_from_int(-1), wl_fixed_from_int(-1), wl_fixed_from_int(-1) };
    if (crop.IsEmpty()) {
        ...
        gfx::RectF crop_transformed = wl::ApplyWaylandTransform(crop, gfx::SizeF(1, 1), wl::ToWaylandTransform(pending_state_.buffer_transform));
        viewport_src_dip = gfx::ScaleRect(crop_transformed, bounds.width(), bounds.height());
        DCHECK(viewport());
        ...
        // entire window width/height.
        {
            ...
            const float kViewPortSizeMinFloat = static_cast<float>(wl_fixed_to_double(kViewportSizeMin));
        ...
        if (wl_fixed_from_double(viewport_src_dip.x()) < 0 || wl_fixed_from_double(viewport_src_dip.y()) < 0) {
            ...
            LOG(ERROR) << "viewport_src_dip=" << viewport_src_dip.ToString() << " crop=" << crop.ToString() << " bounds=" << bounds.ToString()
                       << "  pending_state_.buffer_size_px=" << pending_state_.buffer_size_px.ToString();

            viewport_src_dip.set_x(std::max(viewport_src_dip.x(), 0.f));
            viewport_src_dip.set_y(std::max(viewport_src_dip.y(), 0.f));
        }

        src_to_set[0] = wl_fixed_from_double(viewport_src_dip.x()), src_to_set[1] = wl_fixed_from_double(viewport_src_dip.y());
    }
    // Apply crop (wp_viewport.set_source).
    if (viewport() && !base::ranges::equal(src_to_set, src_set_)) {
        wp_viewport_set_source(viewport(), src_to_set[0], src_to_set[1], src_to_set[2], src_to_set[3]);
        memcpy(src_set_, src_to_set, 4 * sizeof(*src_to_set));
        needs_commit = true;
    }

    gfx::SizeF viewport_dst_dip = viewport_px.IsEmpty() ? viewport_src_dip.size() : gfx::ScaleSize(viewport_px, 1.f / GetWaylandScale(pending_state_));
    ...
    if (!base::ranges::equal(dst_to_set, dst_set_)) {
        ...
        if (dst_to_set[0] > 0.f && augmented_surface && connection_->surface_augmenter()->SupportsSubpixelAccuratePosition()) {
            ...
            DCHECK_EQ(AUGMENTED_SURFACE_SET_DESTINATION_SIZE_SINCE_VERSION, SURFACE_AUGMENTER_GET_AUGMENTED_SUBSURFACE_SINCE_VERSION);
            DCHECK(augmented_surface_get_version(get_augmented_surface()) >= AUGMENTED_SURFACE_SET_DESTINATION_SIZE_SINCE_VERSION);
            augmented_surface_set_destination_size(
                augmented_surface, wl_fixed_from_double(viewport_dst_dip.width()), wl_fixed_from_double(viewport_dst_dip.height()));
            needs_commit = true;
        } else if (viewport()) {
            wp_viewport_set_destination(viewport(), dst_to_set[0] > 0.f ? base::ClampRound(viewport_dst_dip.width()) : static_cast<int>(dst_to_set[0]),
                dst_to_set[1] > 0.f ? base::ClampRound(viewport_dst_dip.height()) : static_cast<int>(dst_to_set[1]));
            needs_commit = true;
        }
    ...
    if (pending_state_.frame_trace_id >= 0) {
        ...
        TRACE_EVENT_CATEGORY_GROUP_ENABLED("viz,benchmark,graphics.pipeline", &is_frame_tracing_enabled);
        if (is_frame_tracing_enabled) {
            auto* augmented_surface = get_augmented_surface();
            if (augmented_surface && augmented_surface_get_version(augmented_surface) >= AUGMENTED_SURFACE_SET_FRAME_TRACE_ID_SINCE_VERSION) {
                augmented_surface_set_frame_trace_id(augmented_surface, pending_state_.frame_trace_id >> 32, pending_state_.frame_trace_id & 0xffffffff);
            }
        }
    ...
    if (pending_state_.damage_px.empty() || pending_state_.damage_px.back().IsEmpty()) {
        pending_state_.damage_px.clear();
    ...
    gfx::Rect damage = ScaleToEnclosingRect(pending_state_.damage_px.back(), 1.f / GetWaylandScale(pending_state_));
    ...
    wl_surface_damage(surface_.get(), damage.x(), damage.y(), damage.width(), damage.height());
    needs_commit = true;
...
{
    if (get_augmented_surface() && augmented_surface_get_version(get_augmented_surface()) >= AUGMENTED_SURFACE_SET_TRUSTED_DAMAGE_SINCE_VERSION) {
        augmented_surface_set_trusted_damage(get_augmented_surface(), true);
    }
}

void WaylandSurface::ExplicitRelease(zwp_linux_buffer_release_v1* linux_buffer_release, base::ScopedFD fence)
{
    ...
    std::move(iter->second.explicit_release_callback).Run(iter->second.buffer.get(), std::move(fence));
...
WaylandSurface::State& WaylandSurface::State::operator=(const WaylandSurface::State& other)
{
...
// static
void WaylandSurface::OnEnter(void* data, wl_surface* surface, wl_output* output)
{
    ...
    auto* wayland_output = static_cast<WaylandOutput*>(wl_output_get_user_data(output));

    DCHECK_NE(self->connection_->wayland_output_manager()->GetOutput(wayland_output->output_id()), nullptr);

    if (auto it = base::ranges::find(self->entered_outputs_, wayland_output->output_id()); it == self->entered_outputs_.end()) {
...
// static
void WaylandSurface::OnLeave(void* data, wl_surface* surface, wl_output* output)
{
    auto* self = static_cast<WaylandSurface*>(data);
    DCHECK(self);
...
// static
void WaylandSurface::OnPreferredScale(void* data, wp_fractional_scale_v1* fractional_scale, uint32_t scale)
{
    auto* self = static_cast<WaylandSurface*>(data);
    DCHECK(self);
    ...
    const float scale_factor = scale == 0 ? 1.0f : (scale / kFractionalScaleDenominator);
...
void WaylandSurface::set_color_space(gfx::ColorSpace color_space)
{
    ...
    auto wayland_zcr_color_space = connection_->zcr_color_manager()->GetColorSpace(color_space);
    if (wayland_zcr_color_space != nullptr)
        pending_state_.color_space = wayland_zcr_color_space;
}

// static
void WaylandSurface::OnFencedRelease(void* data, zwp_linux_buffer_release_v1* buffer_release, int32_t fence)
{
    auto* self = static_cast<WaylandSurface*>(data);
...
// static
void WaylandSurface::OnImmediateRelease(void* data, zwp_linux_buffer_release_v1* buffer_release)
{
    auto* self = static_cast<WaylandSurface*>(data);
