...
public:
    ...
    DryRunningWaylandCursorFactory(const DryRunningWaylandCursorFactory&) = delete;
...
protected:
    ...
    wl_cursor* GetCursorFromTheme(wl_cursor_theme* theme, const std::string& name) override
    {
...
protected:
    void TriggerThemeLoad(WaylandCursorFactory* cursor_factory, float scale = 1.0)
    {
        ASSERT_FALSE(theme_loading_in_progress_);
        theme_loading_in_progress_ = true;
        cursor_factory->GetThemeForScale(scale)->AddThemeLoadedCallback(base::BindOnce(&WaylandCursorFactoryTest::OnThemeLoaded, weak_factory_.GetWeakPtr()));
    }
...
{
    std::unique_ptr<WaylandCursorFactory> cursor_factory = std::make_unique<DryRunningWaylandCursorFactory>(connection_.get());
    ...
    {
        auto* const current_theme_object = cursor_factory->theme_cache_->begin()->second.get()->theme();
        cursor_factory->OnCursorThemeNameChanged("Theme1");
        TriggerThemeLoad(cursor_factory.get());
        EXPECT_EQ(cursor_factory->theme_cache_->size(), 1U);
        EXPECT_NE(cursor_factory->theme_cache_->begin()->second.get()->theme(), current_theme_object);
        EXPECT_EQ(cursor_factory->unloaded_theme_, nullptr);
    ...
    // that theme.
    {
        ...
        cursor_factory->OnCursorBufferAttached(static_cast<wl_cursor*>(WaylandAsyncCursor::FromPlatformCursor(cursor)->bitmap_cursor()->platform_data()));
        EXPECT_EQ(cursor_factory->unloaded_theme_.get(), used_current_theme);
    }
...
TEST_F(WaylandCursorFactoryTest, CachesSizesUntilThemeNameIsChanged)
{
    ...
    EXPECT_EQ(cursor_factory->size_, 24);
    {
        for (const auto size : { 24, 36, 48 }) {
            for (const auto scale : { 1.0, 1.5, 2.0 }) {
                ...
                EXPECT_EQ(cursor_factory->theme_cache_->count(static_cast<int>(cursor_factory->GetScaledSize(scale))), 1U);
                WaitForThemeLoaded();
            }
        }
    ...
    // cursor used currently.
    {
        ...
        auto* const new_current_theme = cursor_factory->theme_cache_->begin()->second.get();
