...
TEST_P(WaylandSurfaceTest, SurfaceReenterOutput)
{
    ...
    const uint32_t output_id = screen_->GetOutputIdForDisplayId(screen_->GetPrimaryDisplay().id());
    ...
    PostToServerAndWait([surface_id_](wl::TestWaylandServerThread* server) {
        wl_surface_send_enter(server->GetObject<wl::MockSurface>(surface_id_)->resource(), server->output()->resource());
    });
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandSurfaceTest, ::testing::Values(wl::ServerConfig {}));
#else
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTestWithAuraShell, WaylandSurfaceTest,
    ::testing::Values(wl::ServerConfig { .enable_aura_shell = wl::EnableAuraShellProtocol::kEnabled },
...
public:
    WaylandSurfaceExplicitSyncTest()
        : WaylandTestSimple(wl::ServerConfig { .use_linux_drm_syncobj = wl::ShouldUseLinuxDrmSyncobjProtocol::kUse })
    {
    }
    ...
    void CreateDmabufBasedBuffer(uint32_t buffer_id, const gfx::Size& size = kDefaultSize, const std::vector<uint32_t>& strides = { 1 },
        const std::vector<uint32_t>& offsets = { 2 }, const std::vector<uint64_t>& modifiers = { 3 }, uint32_t format = DRM_FORMAT_R8,
        uint32_t planes_count = 1)
    {
        buffer_manager_gpu_->CreateDmabufBasedBuffer(GetFdFactory()->CreateFd(), kDefaultSize, strides, offsets, modifiers, format, planes_count, buffer_id);
...
protected:
    ...
    void VerifyAndClearExpectations()
    {
        ...
        PostToServerAndWait([this](wl::TestWaylandServerThread* server) {
            Mock::VerifyAndClearExpectations(server->GetObject<wl::MockSurface>(surface_id_)->linux_drm_syncobj_surface());
        });
    }
...
TEST_F(WaylandSurfaceExplicitSyncTest, ConfigureWithExplicitSync)
{
    ...
    EXPECT_TRUE(surface->AttachBuffer(connection_->buffer_manager_host()->EnsureBufferHandle(surface, kBufferId)));
    ...
    PostToServerAndWait([this, surface_acquire_syncobj, buffer_release_syncobj](wl::TestWaylandServerThread* server) {
        auto* syncobj_surface = server->GetObject<wl::MockSurface>(surface_id_)->linux_drm_syncobj_surface();
        EXPECT_TRUE(syncobj_surface);
        EXPECT_CALL(*syncobj_surface, SetAcquirePoint(IsSameFile(surface_acquire_syncobj), 1));
        EXPECT_CALL(*syncobj_surface, SetReleasePoint(IsSameFile(buffer_release_syncobj), 1));
    });
...
TEST_F(WaylandSurfaceExplicitSyncTest, ConfigureWithExplicitSyncAndCommit)
{
    ...
    auto callback = base::BindOnce(
        [](base::RepeatingClosure quit_closure, wl_buffer** buffer_out, base::ScopedFD* fd_out, wl_buffer* buffer, base::ScopedFD fd) {
            *buffer_out = buffer;
            *fd_out = std::move(fd);
            quit_closure.Run();
        },
    ...
    EXPECT_CALL(*drm_, SyncobjEventfd(_, kCurrentSyncPoint, _, _)).WillOnce([](uint32_t, uint64_t, int ev_fd, uint32_t) {
        uint64_t value = 1;
    ...
    EXPECT_CALL(*drm_, SyncobjExportSyncFile(_, _)).WillOnce([fd = sync_file_fd.get()](uint32_t, int* sync_file_fd) {
    ...
    connection_->Flush();
    run_loop.Run();
    EXPECT_EQ(connection_->buffer_manager_host()->GetBufferHandle(surface, kBufferId)->buffer(), buffer);
    EXPECT_TRUE(fd.is_valid());
}

// Tests that if acquire timeline creation fails, explicit sync is not set and
// explicit release callback is cleared.
TEST_F(WaylandSurfaceExplicitSyncTest, ExplicitSyncNotSet_AcquireTimelineCreationFailed)
{
    constexpr uint32_t kBufferId = 1;
    ...
    surface->RequestExplicitRelease(base::BindOnce([](wl_buffer*, base::ScopedFD) {}));
    ...
    EXPECT_FALSE(surface->acquire_timeline_);
    {
        ...
        // the first time.
        base::AutoReset<bool> fail_on_create(&MockDrmSyncobjIoctlWrapper::fail_on_syncobj_create, true);
        EXPECT_TRUE(surface->ApplyPendingState().value());
    }
    VerifyAndClearExpectations();
...
// explicit release callback is cleared.
TEST_F(WaylandSurfaceExplicitSyncTest, ExplicitSyncNotSet_ReleaseTimelineCreationFailed)
{
    constexpr uint32_t kBufferId = 1;
    {
...
// explicit release callback is cleared.
TEST_F(WaylandSurfaceExplicitSyncTest, ExplicitSyncNotSet_InitialAcquireFenceNotSet)
{
    constexpr uint32_t kBufferId = 1;
    CreateDmabufBasedBuffer(kBufferId);
...
TEST_F(WaylandSurfaceExplicitSyncTest, ExplicitNotSyncSet_InitialAcquireFenceImportFail)
{
    constexpr uint32_t kBufferId = 1;
    CreateDmabufBasedBuffer(kBufferId);
    ...
    EXPECT_CALL(*drm_, SyncobjImportSyncFile(_, _)).WillOnce([] { return ENOENT; });
    EXPECT_TRUE(surface->ApplyPendingState().value());
    VerifyAndClearExpectations();
    EXPECT_TRUE(surface->next_explicit_release_request_.is_null());
}
...
// subsequent frame.
TEST_F(WaylandSurfaceExplicitSyncTest, ExplicitSyncSet_SubsequentAcquireFenceNotSet)
{
    constexpr uint32_t kBufferId = 1;
    CreateDmabufBasedBuffer(kBufferId);
    constexpr uint32_t kBufferReleaseSyncObj = 1;
    ...
    surface->EnsureSurfaceSync();
    constexpr uint32_t kSurfaceAcquireSyncObj = 2;
    int surface_acquire_syncobj = drm_->syncobjs().at(kSurfaceAcquireSyncObj).get();
...
TEST_F(WaylandSurfaceExplicitSyncTest, ExplicitSyncNotSet_SubsequentAcquireFenceImportFail)
{
    constexpr uint32_t kBufferId = 1;
    CreateDmabufBasedBuffer(kBufferId);
