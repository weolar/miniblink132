...
constexpr base::TimeDelta kPresentationFlushTimerDuration = base::Milliseconds(160);
constexpr base::TimeDelta kPresentationFlushTimerStopThreshold = kPresentationFlushTimerDuration / 10;

constexpr char kBoundsRectNanOrInf[] = "Overlay bounds_rect is invalid (NaN or infinity).";

bool potential_compositor_buffer_lock = false;

bool ValidateRect(const gfx::RectF& rect)
{
    return !std::isnan(rect.x()) && !std::isnan(rect.y()) && !std::isnan(rect.width()) && !std::isnan(rect.height()) && !std::isinf(rect.x())
        && !std::isinf(rect.y()) && !std::isinf(rect.width()) && !std::isinf(rect.height());
}
...
WaylandFrame::WaylandFrame(uint32_t frame_id, const gfx::FrameData& data, WaylandSurface* root_surface, wl::WaylandOverlayConfig root_config,
...
WaylandFrame::WaylandFrame(WaylandSurface* root_surface, wl::WaylandOverlayConfig root_config,
    base::circular_deque<std::pair<WaylandSubsurface*, wl::WaylandOverlayConfig>> subsurfaces_to_overlays)
    : root_surface(root_surface)
    , root_config(std::move(root_config))
    , subsurfaces_to_overlays(std::move(subsurfaces_to_overlays))
...
WaylandFrameManager::WaylandFrameManager(WaylandWindow* window, WaylandConnection* connection)
    : window_(window)
...
void WaylandFrameManager::RecordFrame(std::unique_ptr<WaylandFrame> frame)
{
    ...
    TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(frame->trace_id),
        [swap_trace_id = frame->trace_id, frame_id = frame->frame_id](perfetto::EventContext ctx) {
            auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
            auto* data = event->set_chrome_graphics_pipeline();
            data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_BACKEND_SEND_BUFFER_SWAP);
            data->set_display_trace_id(swap_trace_id), data->set_backend_frame_id(frame_id);
        });
    ...
    if (!connection_->buffer_factory()->CanCreateDmabufImmed() && !pending_frames_.empty()) {
        buffer_pending_creation = EnsureWlBuffersExist(*frame) && !frame->buffer_lost;
    }
...
void WaylandFrameManager::MaybeProcessPendingFrame()
{
    ...
    if (!submitted_frames_.empty() && submitted_frames_.back()->wl_frame_callback) {
        TRACE_EVENT_INSTANT("wayland", "WaitForFrameCallback", "cb_owner_frame_id", submitted_frames_.back()->frame_id);
        return;
    }
    ...
    if (has_buffer_pending_creation && !frame->buffer_lost) {
        DLOG_IF(FATAL, has_buffer_pending_creation && connection_->buffer_factory()->CanCreateDmabufImmed()) << "Buffers should have been created immediately.";
        return;
    }
    ...
    if (should_ack_swap_without_commit_ || (!frame->buffer_lost && !!config.buffer_id)) {
    ...
    if (!fatal_error_message_.empty() || frame->buffer_lost || !window_->IsSurfaceConfigured())
    ...
    if (!fatal_error_message_.empty()) {
        connection_->buffer_manager_host()->OnCommitOverlayError(fatal_error_message_);
        return;
    }
...
void WaylandFrameManager::PlayBackFrame(std::unique_ptr<WaylandFrame> frame)
{
    ...
    TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(frame->trace_id),
        [swap_trace_id = frame->trace_id, frame_id = frame->frame_id](perfetto::EventContext ctx) {
            auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
            auto* data = event->set_chrome_graphics_pipeline();
            data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_BACKEND_SEND_BUFFER_POST_SUBMIT);
            data->set_display_trace_id(swap_trace_id), data->set_backend_frame_id(frame_id);
        });
    ...
    if (empty_frame) {
        ...
        auto result = ApplySurfaceConfigure(frame.get(), root_surface, root_config, false);
        if (!result.has_value()) {
    ...
    for (auto& [subsurface, config] : base::Reversed(frame->subsurfaces_to_overlays)) {
        DCHECK(subsurface);
        auto* surface = subsurface->wayland_surface();
        if (empty_frame || !config.buffer_id || wl_fixed_from_double(config.opacity) == 0) {
            ...
            bool needs_commit = subsurface->ConfigureAndShowSurface(
                config.bounds_rect, root_config.bounds_rect, config.clip_rect, config.transform, root_config.surface_scale_factor, nullptr, reference_above);
    ...
    DCHECK(empty_frame || !connection_->presentation() || frame->pending_feedback || frame->feedback.has_value());
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("wayland", "WaylandFrameManager.PlaybackFrame", frame->frame_id);
    ...
    if (!empty_frame) {
        if (potential_compositor_buffer_lock && ++frames_in_flight_ >= kMaxFramesInFlight) {
            if (freeze_timeout_timer_.IsRunning()) {
                freeze_timeout_timer_.Reset();
            } else {
                freeze_timeout_timer_.Start(FROM_HERE, base::Milliseconds(500), this, &WaylandFrameManager::FreezeTimeout);
            }
        }
        submitted_frames_.push_back(std::move(frame));
    }
...
std::optional<bool> WaylandFrameManager::ApplySurfaceConfigure(
    WaylandFrame* frame, WaylandSurface* surface, wl::WaylandOverlayConfig& config, bool set_opaque_region)
{
    DCHECK(surface);
    if (!config.buffer_id)
        return true;
    ...
    const float surface_buffer_scale = config.surface_scale_factor / window_->latched_state().ui_scale;

    surface->set_buffer_transform(absl::holds_alternative<gfx::OverlayTransform>(config.transform) ? absl::get<gfx::OverlayTransform>(config.transform)
    ...
    surface->set_contains_video(
        config.priority_hint == gfx::OverlayPriorityHint::kHardwareProtection || config.priority_hint == gfx::OverlayPriorityHint::kVideo);
    surface->set_color_space(config.color_space.value_or(gfx::ColorSpace::CreateSRGB()));
    surface->set_frame_trace_id(frame->trace_id);
    if (set_opaque_region) {
        auto region_px = config.enable_blend
            ? std::nullopt
            : std::optional<std::vector<gfx::Rect>>({ gfx::Rect(gfx::ToEnclosingRectIgnoringError(config.bounds_rect).size()) });
        surface->set_opaque_region(region_px);
    }

    WaylandBufferHandle* buffer_handle = connection_->buffer_manager_host()->GetBufferHandle(surface, config.buffer_id);
    ...
    surface->UpdateBufferDamageRegion(gfx::ToEnclosingRectIgnoringError(surface_damage));
    if (config.rounded_clip_bounds) {
        ...
        auto rounded_corners_offset = (connection_->surface_augmenter() && connection_->surface_augmenter()->NeedsRoundedClipBoundsInLocalSurfaceCoordinates())
            ? config.bounds_rect.OffsetFromOrigin()
            : gfx::Vector2d();
        surface->set_rounded_clip_bounds(*config.rounded_clip_bounds - rounded_corners_offset);
    } else {
    ...
    const bool is_solid_color_buffer = buffer_handle->backing_type() == WaylandBufferBacking::BufferBackingType::kSolidColor;
    if (will_attach) {
        ...
        if (!frame->wl_frame_callback && !should_skip_frame_callbacks_) {
            static constexpr wl_callback_listener kFrameCallbackListener = { .done = &OnFrameDone };
            TRACE_EVENT_INSTANT("wayland", "CreateFrameCallback", "cb_owner_frame_id", frame->frame_id);
            frame->wl_frame_callback.reset(wl_surface_frame(surface->surface()));
            wl_callback_add_listener(frame->wl_frame_callback.get(), &kFrameCallbackListener, this);
            needs_commit = true;
        }

        if (!is_solid_color_buffer) {
            if (connection_->SupportsExplicitSync()) {
                surface->RequestExplicitRelease(base::BindOnce(&WaylandFrameManager::OnExplicitBufferRelease, weak_factory_.GetWeakPtr(), surface));
            }
            buffer_handle->set_buffer_released_callback(base::BindOnce(&WaylandFrameManager::OnWlBufferRelease, weak_factory_.GetWeakPtr(), surface), surface);
        }
    }

    if (connection_->presentation() && !frame->pending_feedback) {
        static constexpr wp_presentation_feedback_listener kPresentationFeedbackListener
            = { .sync_output = &OnSyncOutput, .presented = &OnPresented, .discarded = &OnDiscarded };
        frame->pending_feedback.reset(wp_presentation_feedback(connection_->presentation(), surface->surface()));
        wp_presentation_feedback_add_listener(frame->pending_feedback.get(), &kPresentationFeedbackListener, this);
        needs_commit = true;
    }
    ...
    if (!is_solid_color_buffer) {
        ...
        for (auto& submitted_frames : submitted_frames_) {
            ...
            if (result != submitted_frames->submitted_buffers.end() && result->second->buffer() == buffer_handle->buffer()) {
...
// static
void WaylandFrameManager::OnFrameDone(void* data, wl_callback* callback, uint32_t time)
{
...
void WaylandFrameManager::HandleFrameCallback(wl_callback* callback)
{
    ...
    submitted_frames_.back()->wl_frame_callback.reset();
    TRACE_EVENT("wayland", "HandleFrameCallback", "cb_owner_frame_id", submitted_frames_.back()->frame_id);
    MaybeProcessPendingFrame();
}

// static
void WaylandFrameManager::OnSyncOutput(void* data, struct wp_presentation_feedback* presentation_feedback, wl_output* output)
{
}

// static
void WaylandFrameManager::OnPresented(void* data, struct wp_presentation_feedback* presentation_feedback, uint32_t tv_sec_hi, uint32_t tv_sec_lo,
    uint32_t tv_nsec, uint32_t refresh, uint32_t seq_hi, uint32_t seq_lo, uint32_t flags)
{
    ...
    self->HandlePresentationFeedback(presentation_feedback,
        gfx::PresentationFeedback(
            self->connection_->ConvertPresentationTime(tv_sec_hi, tv_sec_lo, tv_nsec), base::Nanoseconds(refresh), GetPresentationKindFlags(flags)));
}

// static
void WaylandFrameManager::OnDiscarded(void* data, struct wp_presentation_feedback* presentation_feedback)
{
    auto* self = static_cast<WaylandFrameManager*>(data);
    DCHECK(self);
    self->HandlePresentationFeedback(presentation_feedback, gfx::PresentationFeedback::Failure(), true /* discarded */);
}

void WaylandFrameManager::HandlePresentationFeedback(
    struct wp_presentation_feedback* presentation_feedback, const gfx::PresentationFeedback& feedback, bool discarded)
{
    TRACE_EVENT("wayland", "WaylandFrameManager::HandleFeedback", "discarded", discarded);
    for (auto& frame : submitted_frames_) {
        if (frame->pending_feedback.get() == presentation_feedback) {
            TRACE_EVENT_INSTANT("wayland", "StoreFeedback", "frame_id", frame->frame_id, "feedback_flags", feedback.flags);
...
void WaylandFrameManager::VerifyNumberOfSubmittedFrames()
{
    ...
    if (submitted_frames_.size() >= kMaxNumberOfFrames) {
        LOG(ERROR) << "The server has buggy presentation feedback. Discarding all "
        ...
        // (unlikely).
        for (auto it = submitted_frames_.begin(); it < (submitted_frames_.end() - kMaxFramesInFlight); it++) {
...
bool WaylandFrameManager::EnsureWlBuffersExist(WaylandFrame& frame)
{
    ...
    for (auto& subsurface_to_overlay : frame.subsurfaces_to_overlays) {
        if (subsurface_to_overlay.second.buffer_id) {
            auto* handle = connection_->buffer_manager_host()->EnsureBufferHandle(
                subsurface_to_overlay.first->wayland_surface(), subsurface_to_overlay.second.buffer_id);
    ...
    if (frame.root_config.buffer_id) {
        auto* handle = connection_->buffer_manager_host()->EnsureBufferHandle(frame.root_surface, frame.root_config.buffer_id);
        if (!handle) {
            frame.buffer_lost = true;
            frame.root_config = wl::WaylandOverlayConfig();
        } else if (!handle->buffer() && !handle_pending_creation) {
    ...
    if (handle_pending_creation) {
        handle_pending_creation->set_buffer_created_callback(base::BindOnce(&WaylandFrameManager::MaybeProcessPendingFrame, weak_factory_.GetWeakPtr()));
    }
    return !!handle_pending_creation;
}

void WaylandFrameManager::OnExplicitBufferRelease(WaylandSurface* surface, wl_buffer* buffer, base::ScopedFD fence)
{
    DCHECK(buffer);
    ...
    for (const auto& frame : submitted_frames_) {
        ...
        if (result != frame->submitted_buffers.end() && result->second->buffer() == buffer) {
            ...
            if (fence.is_valid()) {
                if (frame->merged_release_fence_fd.is_valid()) {
                    frame->merged_release_fence_fd.reset(sync_merge("", frame->merged_release_fence_fd.get(), fence.get()));
                } else {
            ...
            TRACE_EVENT_INSTANT("wayland", "OnExplicitBufferRelease", "frame_id", frame->frame_id, "buffer_id", result->second->id());
...
void WaylandFrameManager::OnWlBufferRelease(WaylandSurface* surface, wl_buffer* buffer)
{
    DCHECK(buffer);
    ...
    for (const auto& frame : submitted_frames_) {
        ...
        if (result != frame->submitted_buffers.end() && result->second->buffer() == buffer) {
            if (connection_->UseImplicitSyncInterop()) {
                base::ScopedFD fence = connection_->buffer_manager_host()->ExtractReleaseFence(result->second->id());
            ...
            TRACE_EVENT_INSTANT("wayland", "OnWlBufferRelease", "frame_id", frame->frame_id, "buffer_id", result->second->id());
            frame->submitted_buffers.erase(result);
            break;
        }
    }
...
void WaylandFrameManager::MaybeProcessSubmittedFrames()
{
    ...
    base::circular_deque<std::unique_ptr<WaylandFrame>>::iterator on_submission_begin;
    ...
    if (submitted_frames_.size() == 1u && !submitted_frames_.front()->submission_acked) {
    ...
    for (auto iter = submitted_frames_.begin(); iter < submitted_frames_.end() - 1; ++iter) {
    ...
    TRACE_EVENT_INSTANT("wayland", "Submission count", "count", on_submission_count);

    if (on_submission_count > 0) {
        std::vector<wl::WaylandPresentationInfo> presentation_infos = GetReadyPresentations();
        ...
        for (int32_t i = 0; i < on_submission_count; ++i) {
            ...
            if (iter != submitted_frames_.begin()) {
                ...
                if ((*prev_iter)->merged_release_fence_fd.is_valid()) {
                    release_fence_handle.Adopt(std::move((*prev_iter)->merged_release_fence_fd));
                }
            }

            TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global((*iter)->trace_id),
                [swap_trace_id = (*iter)->trace_id, frame_id = (*iter)->frame_id](perfetto::EventContext ctx) {
                    auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
                    auto* data = event->set_chrome_graphics_pipeline();
                    data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_BACKEND_FINISH_BUFFER_SWAP);
                    data->set_display_trace_id(swap_trace_id), data->set_backend_frame_id(frame_id);
                });

            // The presentation info entries are sent with the last OnSubmission()
            // call.
            TRACE_EVENT_NESTABLE_ASYNC_END0("wayland", "WaylandFrameManager.PlaybackFrame", (*iter)->frame_id);
            auto swap_result = (*iter)->swap_result_recreate_buffers ? gfx::SwapResult::SWAP_NAK_RECREATE_BUFFERS : gfx::SwapResult::SWAP_ACK;
            connection_->buffer_manager_host()->OnSubmission(window_->GetWidget(), (*iter)->frame_id, swap_result, std::move(release_fence_handle),
                (i != on_submission_count - 1) ? std::vector<wl::WaylandPresentationInfo>() : presentation_infos);
        }
    }
...
void WaylandFrameManager::SetFakeFeedback(WaylandFrame* frame)
{
    ...
    frame->feedback = frame->feedback.value_or(gfx::PresentationFeedback(base::TimeTicks::Now(), base::TimeDelta(), GetPresentationKindFlags(0)));
}
...
void WaylandFrameManager::ProcessOldSubmittedFrame(WaylandFrame* frame)
{
    ...
    DCHECK(frame->pending_feedback || frame->feedback.has_value() || !connection_->presentation());
    frame->submission_acked = true;

    if (potential_compositor_buffer_lock && frame != submitted_frames_.front().get()) {
        --frames_in_flight_;
...
std::vector<wl::WaylandPresentationInfo> WaylandFrameManager::GetReadyPresentations()
{
...
{
    while (submitted_frames_.size() > 1 && submitted_frames_.front()->submitted_buffers.empty() && submitted_frames_.front()->presentation_acked) {
...
void WaylandFrameManager::FreezeTimeout()
{
    ...
    for (auto& frame : submitted_frames_) {
        if (frame->submitted_buffers.empty())
            continue;
        TRACE_EVENT_INSTANT("wayland", "FreezeTimeout", "frame_id", frame->frame_id);
        frame->submitted_buffers.clear();
        MaybeProcessSubmittedFrames();
        return;
    }
...
void WaylandFrameManager::EvaluateShouldAckSwapWithoutCommit()
{
    ...
    should_ack_swap_without_commit_ = video_capture_count_ > 0 && window_->IsSuspended();
...
void WaylandFrameManager::EvaluateShouldSkipFrameCallbacks()
{
    ...
    should_skip_frame_callbacks_ = video_capture_count_ > 0 && !window_->IsActive();
...
void WaylandFrameManager::ClearStates()
{
    ...
    for (auto& frame : pending_frames_) {
        DCHECK(frame) << "Can't perform OnChannelDestroyed() during a frame playback.";
    }
    pending_frames_.clear();
...
// static
base::TimeDelta WaylandFrameManager::GetPresentationFlushTimerDurationForTesting()
{
...
void WaylandFrameManager::UpdatePresentationFlushTimer()
{
    if (HaveReadyPresentations()) {
        if (!presentation_flush_timer_.IsRunning()) {
            presentation_flush_timer_.Start(FROM_HERE, kPresentationFlushTimerDuration, this, &WaylandFrameManager::OnPresentationFlushTimerFired);
        }
    ...
    if (presentation_flush_timer_.IsRunning()) {
        ...
        const base::TimeDelta remaining_delay = presentation_flush_timer_.desired_run_time() - base::TimeTicks::Now();
...
void WaylandFrameManager::OnPresentationFlushTimerFired()
{
    ...
    connection_->buffer_manager_host()->OnPresentation(window_->GetWidget(), presentation_infos);
