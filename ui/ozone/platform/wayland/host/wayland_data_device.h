...
public:
    ...
    public:
        ...
        virtual void OnDragEnter(WaylandWindow* window, const gfx::PointF& location, base::TimeTicks timestamp, uint32_t serial) = 0;
        virtual void OnDragMotion(const gfx::PointF& location, base::TimeTicks timestamp) = 0;
    ...
    void StartDrag(const WaylandDataSource& data_source, const WaylandWindow& origin_window, uint32_t serial, wl_surface* icon_surface, DragDelegate* delegate);
    ...
    wl_data_device* data_device() const
    {
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(WaylandDataDragControllerTest, DestroyWindowWhileFetchingForeignData);
    FRIEND_TEST_ALL_PREFIXES(WaylandDataDragControllerTest, LeaveWindowWhileFetchingData);

    // wl_data_device_listener callbacks:
    static void OnDataOffer(void* data, wl_data_device* data_device, wl_data_offer* offer);
    static void OnEnter(void* data, wl_data_device* data_device, uint32_t serial, wl_surface* surface, wl_fixed_t x, wl_fixed_t y, wl_data_offer* offer);
    static void OnMotion(void* data, struct wl_data_device* data_device, uint32_t time, wl_fixed_t x, wl_fixed_t y);
    ...
    static void OnSelection(void* data, wl_data_device* data_device, wl_data_offer* offer);
