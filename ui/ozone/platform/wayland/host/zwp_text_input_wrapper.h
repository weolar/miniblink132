...
public:
    ...
    virtual void OnPreeditString(std::string_view text, const std::vector<SpanStyle>& spans, const gfx::Range& preedit_cursor) = 0;
    ...
    virtual void OnKeysym(uint32_t key, uint32_t state, uint32_t modifiers, uint32_t time) = 0;
    ...
    virtual void OnSetPreeditRegion(int32_t index, uint32_t length, const std::vector<SpanStyle>& spans) = 0;
    ...
    virtual void OnSetVirtualKeyboardOccludedBounds(const gfx::Rect& screen_bounds) = 0;
...
public:
    ...
    virtual void Activate(WaylandWindow* window, ui::TextInputClient::FocusReason reason) = 0;
    ...
    virtual void SetSurroundingText(const std::string& text, const gfx::Range& preedit_range, const gfx::Range& selection_range) = 0;
    virtual bool HasAdvancedSurroundingTextSupport() const = 0;
    virtual void SetSurroundingTextOffsetUtf16(uint32_t offset_utf16) = 0;
    virtual void SetContentType(ui::TextInputType type, ui::TextInputMode mode, uint32_t flags, bool should_do_learning, bool can_compose_inline) = 0;

    virtual void SetGrammarFragmentAtCursor(const ui::GrammarFragment& fragment) = 0;
    virtual void SetAutocorrectInfo(const gfx::Range& autocorrect_range, const gfx::Rect& autocorrect_bounds) = 0;
};
