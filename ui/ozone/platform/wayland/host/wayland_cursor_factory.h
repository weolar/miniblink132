...
class WaylandCursorFactory : public BitmapCursorFactory, public CursorThemeManagerObserver, public WaylandCursorBufferListener {
public:
    ...
    // CursorFactory:
    scoped_refptr<PlatformCursor> GetDefaultCursor(mojom::CursorType type) override;
    scoped_refptr<PlatformCursor> GetDefaultCursor(mojom::CursorType type, float scale) override;
    scoped_refptr<PlatformCursor> CreateImageCursor(mojom::CursorType type, const SkBitmap& bitmap, const gfx::Point& hotspot, float scale) override;
    scoped_refptr<PlatformCursor> CreateAnimatedCursor(
        mojom::CursorType type, const std::vector<SkBitmap>& bitmaps, const gfx::Point& hotspot, float scale, base::TimeDelta frame_delay) override;

protected:
    // Returns the actual wl_cursor record from the currently loaded theme.
    // Virtual for tests where themes can be empty.
    virtual wl_cursor* GetCursorFromTheme(wl_cursor_theme* theme, const std::string& name);

private:
    friend class DryRunningWaylandCursorFactory;
    friend class WaylandCursorFactoryTest;
    FRIEND_TEST_ALL_PREFIXES(WaylandCursorFactoryTest, RetainOldThemeUntilNewBufferIsAttached);
    FRIEND_TEST_ALL_PREFIXES(WaylandCursorFactoryTest, CachesSizesUntilThemeNameIsChanged);
    ...
    public:
        ...
        base::WeakPtr<ThemeData> GetWeakPtr()
        {
    ...
    void FinishThemeLoad(base::WeakPtr<ThemeData> cache_entry, wl_cursor_theme* loaded_theme);
    void FinishCursorLoad(scoped_refptr<WaylandAsyncCursor> cursor, mojom::CursorType type, float scale, wl_cursor_theme* loaded_theme);

    const raw_ptr<WaylandConnection> connection_;

    base::ScopedObservation<LinuxUi, CursorThemeManagerObserver> cursor_theme_observer_ { this };
