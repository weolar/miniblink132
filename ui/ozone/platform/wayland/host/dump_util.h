...
template <typename M> std::string ToMatchingKeyMaskString(int mask, const M& map)
{
    std::string str;
...
// `defualt_value`.
template <typename M, typename K> const char* GetMapValueOrDefault(const M& map, const K& key, const char* default_value = "unknown")
{
