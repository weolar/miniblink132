...
public:
    ...
    WaylandInputMethodContext(WaylandConnection* connection, WaylandKeyboard::Delegate* key_delegate, LinuxInputMethodContextDelegate* ime_delegate);
    WaylandInputMethodContext(const WaylandInputMethodContext&) = delete;
    WaylandInputMethodContext& operator=(const WaylandInputMethodContext&) = delete;
    ~WaylandInputMethodContext() override;

    void Init(bool initialize_for_testing = false, std::unique_ptr<ZWPTextInputWrapper> wrapper_for_testing = nullptr,
        std::optional<base::nix::DesktopEnvironment> desktop_for_testing = std::nullopt);
    ...
    void UpdateFocus(
        bool has_client, TextInputType old_type, const TextInputClientAttributes& new_client_attributes, TextInputClient::FocusReason reason) override;
    void Reset() override;
    ...
    void OnPreeditString(std::string_view text, const std::vector<SpanStyle>& spans, const gfx::Range& preedit_cursor) override;
    ...
    void OnKeysym(uint32_t keysym, uint32_t state, uint32_t modifiers, uint32_t time) override;
    void OnSetPreeditRegion(int32_t index, uint32_t length, const std::vector<SpanStyle>& spans) override;
...
private:
    ...
    void Focus(bool skip_virtual_keyboard_update, TextInputClient::FocusReason reason);
    ...
    void MaybeUpdateActivated(bool skip_virtual_keyboard_update, TextInputClient::FocusReason reason);

    const raw_ptr<WaylandConnection> connection_; // TODO(jani) Handle this better
    ...
    base::flat_map<TextInputClient*, base::WeakPtr<TextInputClient>> past_clients_;
