...
// static
void GtkShell1::Instantiate(WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    CHECK_EQ(interface, kInterfaceName) << "Expected \"" << kInterfaceName << "\" but got \"" << interface << "\"";

    if (connection->gtk_shell1_ || !wl::CanBind(interface, version, kMinVersion, kMaxVersion)) {
        return;
    }

    auto gtk_shell1 = wl::Bind<::gtk_shell1>(registry, name, std::min(version, kMaxVersion));
    if (!gtk_shell1) {
...
std::unique_ptr<GtkSurface1> GtkShell1::GetGtkSurface1(wl_surface* top_level_window_surface)
{
    return std::make_unique<GtkSurface1>(gtk_shell1_get_gtk_surface(shell1_.get(), top_level_window_surface));
}
