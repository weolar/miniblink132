...
const std::vector<std::string>& WaylandDataDeviceBase::GetAvailableMimeTypes() const
{
    if (!data_offer_) {
...
PlatformClipboard::Data WaylandDataDeviceBase::ReadSelectionData(const std::string& mime_type)
{
    if (!data_offer_)
        return {};
...
void WaylandDataDeviceBase::NotifySelectionOffer(WaylandDataOfferBase* offer) const
{
