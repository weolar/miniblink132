...
public:
    ...
    bool ExportWindowHandle(gfx::AcceleratedWidget parent, base::OnceCallback<void(const std::string&)> callback) override;
    bool ExportWindowHandle(gfx::AcceleratedWidget window_id, base::OnceCallback<void(std::string)> callback) override;

private:
    void OnHandleForward(base::OnceCallback<void(std::string)> callback, const std::string& handle);
