...
public:
    ZWPTextInputWrapperV3Test()
        : WaylandTestSimple({ .text_input_wrapper_type = wl::ZWPTextInputWrapperType::kV3 })
    {
    }
    ...
    void SetUp() override
    {
        ...
        wrapper_ = std::make_unique<ZWPTextInputWrapperV3>(connection_.get(), &test_client_, connection_->text_input_manager_v3());
    }
...
TEST_F(ZWPTextInputWrapperV3Test, Activate)
{
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        InSequence s;
        EXPECT_CALL(*server->text_input_manager_v3()->text_input(), Enable()).Times(1);
        EXPECT_CALL(*server->text_input_manager_v3()->text_input(), Commit()).Times(1);
    });
...
TEST_F(ZWPTextInputWrapperV3Test, Deactivate)
{
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        InSequence s;
        EXPECT_CALL(*server->text_input_manager_v3()->text_input(), Disable()).Times(1);
        EXPECT_CALL(*server->text_input_manager_v3()->text_input(), Commit()).Times(1);
    });
    wrapper_->Deactivate();
}
...
{
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        EXPECT_CALL(*server->text_input_manager_v3()->text_input(), Enable()).Times(0);
        EXPECT_CALL(*server->text_input_manager_v3()->text_input(), Disable()).Times(0);
        EXPECT_CALL(*server->text_input_manager_v3()->text_input(), Commit()).Times(0);
    });
...
TEST_F(ZWPTextInputWrapperV3Test, SetContentType)
{
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        EXPECT_CALL(*zwp_text_input, SetContentType(ZWP_TEXT_INPUT_V3_CONTENT_HINT_SPELLCHECK, ZWP_TEXT_INPUT_V3_CONTENT_PURPOSE_EMAIL)).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    });
    wrapper_->SetContentType(TEXT_INPUT_TYPE_EMAIL, TEXT_INPUT_MODE_NONE, TEXT_INPUT_FLAG_AUTOCORRECT_ON, false, false);
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetContentType(ZWP_TEXT_INPUT_V3_CONTENT_HINT_AUTO_CAPITALIZATION, ZWP_TEXT_INPUT_V3_CONTENT_PURPOSE_NUMBER)).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    });
    wrapper_->SetContentType(TEXT_INPUT_TYPE_NUMBER, TEXT_INPUT_MODE_NONE, TEXT_INPUT_FLAG_AUTOCAPITALIZE_WORDS, false, false);
}
...
TEST_F(ZWPTextInputWrapperV3Test, SetCursorRect)
{
    ...
    PostToServerAndWait([kRect](wl::TestWaylandServerThread* server) {
        InSequence s;
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetCursorRect(kRect.x(), kRect.y(), kRect.width(), kRect.height()));
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    });
    ...
    PostToServerAndWait([kRect2](wl::TestWaylandServerThread* server) {
        InSequence s;
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetCursorRect(kRect2.x(), kRect2.y(), kRect2.width(), kRect2.height()));
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    });
...
TEST_F(ZWPTextInputWrapperV3Test, SetSurroundingText)
{
    ...
    PostToServerAndWait([kSurroundingText](wl::TestWaylandServerThread* server) {
        EXPECT_CALL(*server->text_input_manager_v3()->text_input(), SetSurroundingText(kSurroundingText, gfx::Range { 11, 11 })).Times(1);
        EXPECT_CALL(*server->text_input_manager_v3()->text_input(), Commit()).Times(1);
    });
    ...
    PostToServerAndWait([kSurroundingText](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetSurroundingText(kSurroundingText, gfx::Range { 8, 9 })).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    });
    wrapper_->SetSurroundingText(text, kPreeditRange, selection_range);
    VerifyAndClearExpectations();
    ...
    PostToServerAndWait([kSurroundingText](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetSurroundingText(kSurroundingText, gfx::Range { 11, 11 })).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    ...
    PostToServerAndWait([kSurroundingText](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetSurroundingText(kSurroundingText, gfx::Range { 11, 17 })).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    ...
    PostToServerAndWait([kSurroundingText](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetSurroundingText(kSurroundingText, gfx::Range { 17, 11 })).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    ...
    PostToServerAndWait([kSurroundingText](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetSurroundingText(kSurroundingText, gfx::Range { 8, 11 })).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    ...
    PostToServerAndWait([kSurroundingText](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetSurroundingText(kSurroundingText, gfx::Range { 11, 8 })).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    ...
    PostToServerAndWait([kSurroundingText](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetSurroundingText(kSurroundingText, gfx::Range { 8, 13 })).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    ...
    PostToServerAndWait([kSurroundingText](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetSurroundingText(kSurroundingText, gfx::Range { 14, 17 })).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
    ...
    PostToServerAndWait([kSurroundingText](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetSurroundingText(kSurroundingText, gfx::Range { 12, 12 })).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
        zwp_text_input_v3_send_done(zwp_text_input->resource(), 9);
    });
    wrapper_->SetSurroundingText(kSurroundingText, gfx::Range::InvalidRange(), selection_range);
    VerifyAndClearExpectations();
    ...
    PostToServerAndWait([text](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        EXPECT_CALL(*zwp_text_input, SetSurroundingText(text, gfx::Range { 42, 42 })).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
        zwp_text_input_v3_send_done(zwp_text_input->resource(), 11);
    });
    wrapper_->SetSurroundingText(text, gfx::Range::InvalidRange(), gfx::Range::InvalidRange());
    VerifyAndClearExpectations();
}
...
TEST_F(ZWPTextInputWrapperV3Test, PendingRequestsSentOnDone)
{
    ...
    // matches.
    PostToServerAndWait([kRect, kSurroundingText](wl::TestWaylandServerThread* server) {
        auto* zwp_text_input = server->text_input_manager_v3()->text_input();
        InSequence s;
        EXPECT_CALL(*zwp_text_input, SetContentType(ZWP_TEXT_INPUT_V3_CONTENT_HINT_SPELLCHECK, ZWP_TEXT_INPUT_V3_CONTENT_PURPOSE_EMAIL)).Times(1);
        EXPECT_CALL(*server->text_input_manager_v3()->text_input(), SetCursorRect(kRect.x(), kRect.y(), kRect.width(), kRect.height()));
        EXPECT_CALL(*zwp_text_input, SetSurroundingText(kSurroundingText, gfx::Range { 11, 11 })).Times(1);
        EXPECT_CALL(*zwp_text_input, Commit()).Times(1);
...
TEST_F(ZWPTextInputWrapperV3Test, OnPreeditString)
{
    ...
    EXPECT_CALL(test_client_, OnPreeditString(kPreeditString, std::vector<ZWPTextInputWrapperClient::SpanStyle> {}, kPreeditCursor));
    PostToServerAndWait([kPreeditString, kPreeditCursor](wl::TestWaylandServerThread* server) {
        auto* text_input = server->text_input_manager_v3()->text_input();
        zwp_text_input_v3_send_preedit_string(text_input->resource(), kPreeditString.data(), kPreeditCursor.start(), kPreeditCursor.end());
        zwp_text_input_v3_send_done(text_input->resource(), 0);
    });
    VerifyAndClearExpectations();

    // Invalid range if negative cursor begin
    EXPECT_CALL(test_client_, OnPreeditString(kPreeditString, std::vector<ZWPTextInputWrapperClient::SpanStyle> {}, gfx::Range::InvalidRange()));
    PostToServerAndWait([kPreeditString, kPreeditCursor](wl::TestWaylandServerThread* server) {
        auto* text_input = server->text_input_manager_v3()->text_input();
        zwp_text_input_v3_send_preedit_string(text_input->resource(), kPreeditString.data(), -1, kPreeditCursor.end());
        zwp_text_input_v3_send_done(text_input->resource(), 0);
    });
    VerifyAndClearExpectations();
    ...
    PostToServerAndWait([kPreeditString, kPreeditCursor](wl::TestWaylandServerThread* server) {
        auto* text_input = server->text_input_manager_v3()->text_input();
        zwp_text_input_v3_send_preedit_string(text_input->resource(), kPreeditString.data(), kPreeditCursor.start(), -1);
        zwp_text_input_v3_send_done(text_input->resource(), 0);
    });
    VerifyAndClearExpectations();
}
...
TEST_F(ZWPTextInputWrapperV3Test, OnCommitString)
{
    ...
    PostToServerAndWait([kCommitString](wl::TestWaylandServerThread* server) {
        auto* text_input = server->text_input_manager_v3()->text_input();
        zwp_text_input_v3_send_commit_string(text_input->resource(), kCommitString.c_str());
        zwp_text_input_v3_send_done(text_input->resource(), 0);
    });
    VerifyAndClearExpectations();
}
...
TEST_F(ZWPTextInputWrapperV3Test, OnDoneWithCommitAndPreedit)
{
    ...
    EXPECT_CALL(test_client_, OnCommitString(kCommitString));
    EXPECT_CALL(test_client_, OnPreeditString(kPreeditString, std::vector<ZWPTextInputWrapperClient::SpanStyle> {}, kPreeditCursor));
    PostToServerAndWait([kPreeditString, kPreeditCursor, kCommitString](wl::TestWaylandServerThread* server) {
        auto* text_input = server->text_input_manager_v3()->text_input();
        zwp_text_input_v3_send_preedit_string(text_input->resource(), kPreeditString.c_str(), kPreeditCursor.start(), kPreeditCursor.end());
        zwp_text_input_v3_send_commit_string(text_input->resource(), kCommitString.c_str());
        zwp_text_input_v3_send_done(text_input->resource(), 0);
    });
    VerifyAndClearExpectations();
