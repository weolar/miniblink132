...
WaylandWindow* WaylandWindowManager::GetWindow(gfx::AcceleratedWidget widget) const
{
...
WaylandWindow* WaylandWindowManager::GetCurrentFocusedWindow() const
{
    for (const auto& entry : window_map_) {
        WaylandWindow* window = entry.second;
        if (window == pointer_focused_window_ || window->has_touch_focus() || window == keyboard_focused_window_) {
            return window;
        }
    }
    return nullptr;
...
WaylandWindow* WaylandWindowManager::GetCurrentPointerOrTouchFocusedWindow() const
{
    ...
    if (connection_->window_drag_controller()) {
        ...
        if (auto drag_source = connection_->window_drag_controller()->drag_source()) {
            return *drag_source == mojom::DragEventSource::kMouse ? GetCurrentPointerFocusedWindow() : GetCurrentTouchFocusedWindow();
        }
    }
...
void WaylandWindowManager::AddWindow(gfx::AcceleratedWidget widget, WaylandWindow* window)
{
...
void WaylandWindowManager::AddSubsurface(gfx::AcceleratedWidget widget, WaylandSubsurface* subsurface)
{
    auto* window = window_map_[widget].get();
    DCHECK(window);

    observers_.Notify(&WaylandWindowObserver::OnSubsurfaceAdded, window, subsurface);
}

void WaylandWindowManager::RemoveSubsurface(gfx::AcceleratedWidget widget, WaylandSubsurface* subsurface)
{
    auto* window = window_map_[widget].get();
    DCHECK(window);

    observers_.Notify(&WaylandWindowObserver::OnSubsurfaceRemoved, window, subsurface);
}

void WaylandWindowManager::RecycleSubsurface(std::unique_ptr<WaylandSubsurface> subsurface)
{
...
float WaylandWindowManager::DetermineUiScale() const
{
    ...
    const float ui_scale = Display::HasForceDeviceScaleFactor() && connection_->IsUiScaleEnabled() ? Display::GetForcedDeviceScaleFactor() : font_scale_;
