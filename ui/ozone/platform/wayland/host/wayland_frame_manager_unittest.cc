...
public:
    void SetUp() override
    {
        ...
        frame_manager_ = std::make_unique<WaylandFrameManager>(window_.get(), connection_.get());
    }
...
protected:
    void ApplySurfaceConfigureAndCheckFrameCallback(bool expect_frame_callback)
    {
        ...
        buffer_manager_gpu_->Initialize(std::move(interface_ptr), {},
            ...
            /*server_version=*/ {});
        buffer_manager_gpu_->CreateShmBasedBuffer(MakeFD(), kLength, kBufferSize, kBufferId);
        ...
        frame_manager_->ApplySurfaceConfigure(&frame, surface, frame.root_config, false);
    ...
    void RecordFrameAndCheckSurfaceCommits(bool expect_surface_commits)
    {
        ...
        auto* surface = window_->root_surface();
        std::unique_ptr<WaylandFrame> frame = std::make_unique<WaylandFrame>(surface, std::move(config));

        PostToServerAndWait([id = window_->root_surface()->get_surface_id(), expect_surface_commits](wl::TestWaylandServerThread* server) {
    ...
    size_t NumPendingFrames()
    {
        return frame_manager_->pending_frames_.size();
    }
...
// captured.
TEST_F(WaylandFrameManagerTest, FrameCallbackSet_WindowInactiveVideoNotCapturing)
{
    WaylandWindow::WindowStates window_states;
...
// capture.
TEST_F(WaylandFrameManagerTest, FrameCallbackNotSet_WindowInactiveVideoCapturing)
{
    // Make window inactive
    WaylandWindow::WindowStates window_states;
    window_states.is_activated = false;
...
TEST_F(WaylandFrameManagerTest, UnblockFrames_BothInactiveAndVideoCaptureBecomeTrue)
{
    constexpr uint32_t kBufferId = 1;
    // Setup wl_buffers.
    EXPECT_TRUE(connection_->buffer_manager_host());
...
// not being captured.
TEST_F(WaylandFrameManagerTest, SurfaceCommitted_WindowInactiveVideoNotCapturing)
{
    WaylandWindow::WindowStates window_states;
    window_states.is_activated = true;
...
TEST_F(WaylandFrameManagerTest, AckSwapWithoutCommit_WindowSuspendedVideoCapturing)
{
    ...
    buffer_manager_gpu_->CreateShmBasedBuffer(MakeFD(), kLength, kBufferSize, kBufferId2);
    base::RunLoop().RunUntilIdle();

    wl::WaylandOverlayConfig config2;
    config2.buffer_id = kBufferId2;
    config2.bounds_rect = { 0, 0, kWidth, kHeight };
    auto frame2 = std::make_unique<WaylandFrame>(window_->root_surface(), std::move(config2));

    PostToServerAndWait([id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(id);
...
TEST_F(WaylandFrameManagerTest, UnblockFrames_BothSuspendedAndVideoCaptureBecomeTrue)
{
    constexpr uint32_t kBufferId = 1;
    // Setup wl_buffers.
    EXPECT_TRUE(connection_->buffer_manager_host());
