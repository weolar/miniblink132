...
WaylandPopup::WaylandPopup(PlatformWindowDelegate* delegate, WaylandConnection* connection, WaylandWindow* parent)
    : WaylandWindow(delegate, connection)
{
    ...
    LOG_IF(WARNING, !parent->AsWaylandToplevelWindow() && !parent->AsWaylandPopup()) << "Popup's parent is a bubble. Wayland shell popup is not guaranteed to "
                                                                                        "show up.";
#endif
}
...
void WaylandPopup::TooltipShown(const char* text, int32_t x, int32_t y, int32_t width, int32_t height)
{
...
bool WaylandPopup::CreateShellPopup()
{
    ...
    if (applied_state().window_scale != parent_window()->applied_state().window_scale) {
    ...
    auto bounds_dip = wl::TranslateWindowBoundsToParentDIP(this, xdg_parent_window);
    ...
    if (params.anchor.has_value()) {
        ...
        gfx::Rect anchor_rect(wl::TranslateBoundsToParentCoordinates(params.anchor->anchor_rect, xdg_parent_window->GetBoundsInDIP())
...
void WaylandPopup::SetBoundsInDIP(const gfx::Rect& bounds_dip)
{
    ...
    if (shell_popup_ && old_bounds_dip != bounds_dip) {
        auto bounds_dip_in_parent = wl::TranslateWindowBoundsToParentDIP(this, xdg_parent_window);
        bounds_dip_in_parent.Inset(delegate()->CalculateInsetsInDIP(GetPlatformWindowState()));
...
void WaylandPopup::HandlePopupConfigure(const gfx::Rect& bounds_dip)
{
    ...
    gfx::Rect pending_bounds_dip(bounds_dip.IsEmpty() ? GetBoundsInDIP() : gfx::ScaleToEnclosingRectIgnoringError(bounds_dip, 1.0f / applied_state().ui_scale));

    // The origin is relative to parent's window geometry.
    // See https://crbug.com/1292486.
    pending_configure_state_.bounds_dip
        = wl::TranslateBoundsToTopLevelCoordinates(pending_bounds_dip, xdg_parent_window->GetBoundsInDIP()) + xdg_parent_window->GetWindowGeometryOffsetInDIP();
    ...
    const auto insets = delegate()->CalculateInsetsInDIP(pending_configure_state_.window_state.value_or(PlatformWindowState::kNormal));
    pending_configure_state_.bounds_dip->Inset(-insets);
    pending_configure_state_.size_px = delegate()->ConvertRectToPixels(pending_bounds_dip).size();
}
...
void WaylandPopup::UpdateWindowMask()
{
    ...
    auto region = IsOpaqueWindow() ? std::optional<std::vector<gfx::Rect>>({ gfx::Rect(latched_state().size_px) }) : std::nullopt;
...
void WaylandPopup::ShowTooltip(const std::u16string& text, const gfx::Point& position, const PlatformWindowTooltipTrigger trigger,
...
bool WaylandPopup::OnInitialize(PlatformWindowInitProperties properties, PlatformWindowDelegate::State* state)
{
    DCHECK(parent_window());
...
void WaylandPopup::SetWindowGeometry(const PlatformWindowDelegate::State& state)
{
    if (!shell_popup_) {
        return;
    }
