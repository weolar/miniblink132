...
public:
    ...
    double latest_scale_update() const
    {
        return latest_scale_update_;
    }
...
protected:
    ...
    void WillProcessEvent(const PlatformEvent& event) override
    {
        ...
        if (!gesture->IsPinchEvent() || gesture->type() != EventType::kGesturePinchUpdate) {
            return;
        }
...
protected:
    ...
    void EnterPoint()
    {
        PostToServerAndWait([surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
            ...
            wl_pointer_send_enter(pointer, server->GetNextSerial(), surface, wl_fixed_from_int(50), wl_fixed_from_int(50));
...
TEST_F(WaylandPointerGesturesTest, PinchZoomScale)
{
    ...
    PostToServerAndWait([surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
        ...
        zwp_pointer_gesture_pinch_v1_send_begin(pinch, server->GetNextSerial(), server->GetNextTime(), surface,
            /* fingers */ 2);
    });

    constexpr double kScales[] = { 1.0, 1.1, 1.2, 1.3, 1.4, 1.5, 1.4, 1.3, 1.2, 1.1, 1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0 };
    ...
    for (auto scale : kScales) {
        PostToServerAndWait([scale](wl::TestWaylandServerThread* server) {
            ...
            zwp_pointer_gesture_pinch_v1_send_update(pinch, /* time */ 0, /* dx */ 0, /* dy */ 0, wl_fixed_from_double(scale), /* rotation */ 0);
        });
#if BUILDFLAG(IS_CHROMEOS_LACROS)
        EXPECT_FLOAT_EQ(observer.latest_scale_update(), wl_fixed_to_double(wl_fixed_from_double(scale)));
#else
        ...
        EXPECT_FLOAT_EQ(observer.latest_scale_update(), wl_fixed_to_double(wl_fixed_from_double(scale)) / previous_scale);
...
TEST_F(WaylandPointerGesturesTest, HoldEventCancelsFling)
{
    ...
    EXPECT_CALL(delegate_, DispatchEvent(testing::_)).Times(3).WillOnce(CloneEvent(&event1)).WillOnce(CloneEvent(&event2)).WillOnce(CloneEvent(&event3));
    // Send vertical scroll event.
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        auto* const pointer = server->seat()->pointer()->resource();
        wl_pointer_send_axis_source(pointer, WL_POINTER_AXIS_SOURCE_FINGER);
        wl_pointer_send_axis(pointer, server->GetNextTime(), WL_POINTER_AXIS_VERTICAL_SCROLL, wl_fixed_from_int(50));
        wl_pointer_send_frame(pointer);
    });
    ...
    PostToServerAndWait([surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
        ...
        auto* const surface = server->GetObject<wl::MockSurface>(surface_id)->resource();
        zwp_pointer_gesture_hold_v1_send_begin(hold, server->GetNextSerial(), server->GetNextTime(), surface,
            /* fingers */ 1);
    });
    ...
    PostToServerAndWait([surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
        auto* const hold = server->wp_pointer_gestures().hold()->resource();
        zwp_pointer_gesture_hold_v1_send_end(hold, server->GetNextSerial(), server->GetNextTime(),
            /* cancelled */ 1);
    });
