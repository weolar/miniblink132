...
OrgGnomeMutterIdleMonitor::OrgGnomeMutterIdleMonitor()
    : task_runner_(base::ThreadPool::CreateSequencedTaskRunner(
        base::TaskTraits(base::MayBlock(), base::TaskPriority::USER_VISIBLE, base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN)))
{
...
std::optional<base::TimeDelta> OrgGnomeMutterIdleMonitor::GetIdleTime() const
{
    ...
    case ServiceState::kUnknown:
        ...
        {
            dbus::ObjectProxy* dbus_proxy = bus_->GetObjectProxy(DBUS_SERVICE_DBUS, dbus::ObjectPath(DBUS_PATH_DBUS));
            ...
            dbus_proxy->CallMethod(&name_has_owner_call, dbus::ObjectProxy::TIMEOUT_USE_DEFAULT,
                base::BindOnce(&OrgGnomeMutterIdleMonitor::OnServiceHasOwner, weak_factory_.GetMutableWeakPtr()));
        }
...
void OrgGnomeMutterIdleMonitor::OnServiceHasOwner(dbus::Response* response)
{
    ...
    proxy_->ConnectToSignal(kInterface, kSignalWatchFired, base::BindRepeating(&OrgGnomeMutterIdleMonitor::OnWatchFired, weak_factory_.GetWeakPtr()),
        base::BindOnce(&OrgGnomeMutterIdleMonitor::OnWatchFiredSignalConnected, weak_factory_.GetWeakPtr()));
}

void OrgGnomeMutterIdleMonitor::OnWatchFiredSignalConnected(const std::string& interface, const std::string& signal, bool succeeded)
{
    DCHECK_CALLED_ON_VALID_THREAD(main_thread_checker_);
    ...
    if (!succeeded) {
        LOG(WARNING) << "Cannot connect to " << kSignalWatchFired << " signal of " << kInterface << " D-Bus service";
        return Shutdown();
    }
    ...
    proxy_->CallMethod(&call, dbus::ObjectProxy::TIMEOUT_USE_DEFAULT, base::BindOnce(&OrgGnomeMutterIdleMonitor::OnAddIdleWatch, weak_factory_.GetWeakPtr()));
}
...
void OrgGnomeMutterIdleMonitor::OnAddIdleWatch(dbus::Response* response)
{
    ...
    if (!response || !ReadWatchId(response, &idle_watch_id_)) {
        LOG(WARNING) << "Call to " << kMethodAddIdleWatch << " method of " << kInterface << " D-Bus service failed";
        return Shutdown();
    }
    ...
    proxy_->CallMethod(
        &call, dbus::ObjectProxy::TIMEOUT_USE_DEFAULT, base::BindOnce(&OrgGnomeMutterIdleMonitor::OnAddUserActiveWatch, weak_factory_.GetWeakPtr()));
}
...
void OrgGnomeMutterIdleMonitor::OnAddUserActiveWatch(dbus::Response* response)
{
    ...
    if (!response || !ReadWatchId(response, &active_watch_id_)) {
        LOG(WARNING) << "Call to " << kMethodAddUserActiveWatch << " method of " << kInterface << " D-Bus service failed";
        return Shutdown();
    }
    ...
    if (service_state_ == ServiceState::kInitializing) {
        ...
        proxy_->CallMethod(
            &call, dbus::ObjectProxy::TIMEOUT_USE_DEFAULT, base::BindOnce(&OrgGnomeMutterIdleMonitor::OnGetIdletime, weak_factory_.GetWeakPtr()));
    }
}
...
void OrgGnomeMutterIdleMonitor::OnGetIdletime(dbus::Response* response)
{
    ...
    if (!response || !UpdateIdleTime(response)) {
        LOG(WARNING) << "Call to " << kMethodGetIdletime << " method of " << kInterface << " D-Bus service failed";
        return Shutdown();
    }
...
bool OrgGnomeMutterIdleMonitor::ReadWatchId(dbus::Message* message, uint32_t* watch_id)
{
    if (!message)
        return false;
