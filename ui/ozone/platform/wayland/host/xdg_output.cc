...
void XDGOutput::UpdateMetrics(bool compute_scale_from_size, WaylandOutput::Metrics& metrics)
{
    if (!IsReady()) {
        return;
    }
    ...
    compute_scale_from_size &= (!logical_size.IsEmpty() && logical_size != physical_size);
    if (compute_scale_from_size) {
        const float max_physical_side = std::max(physical_size.width(), physical_size.height());
        const float max_logical_side = std::max(logical_size.width(), logical_size.height());
...
// static
void XDGOutput::OnLogicalPosition(void* data, zxdg_output_v1* output, int32_t x, int32_t y)
{
...
// static
void XDGOutput::OnLogicalSize(void* data, zxdg_output_v1* output, int32_t width, int32_t height)
{
    if (auto* self = static_cast<XDGOutput*>(data)) {
...
// static
void XDGOutput::OnDescription(void* data, zxdg_output_v1* output, const char* description)
{
    if (auto* self = static_cast<XDGOutput*>(data)) {
