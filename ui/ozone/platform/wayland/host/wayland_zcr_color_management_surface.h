...
public:
    explicit WaylandZcrColorManagementSurface(zcr_color_management_surface_v1* management_surface, WaylandConnection* connection);
    WaylandZcrColorManagementSurface(const WaylandZcrColorManagementSurface&) = delete;
    WaylandZcrColorManagementSurface& operator=(const WaylandZcrColorManagementSurface&) = delete;
    ~WaylandZcrColorManagementSurface();

    void SetDefaultColorSpace();
    void SetColorSpace(scoped_refptr<WaylandZcrColorSpace> wayland_zcr_color_space);

private:
    // zcr_color_management_surface_v1_listener callbacks:
    static void OnPreferredColorSpace(void* data, zcr_color_management_surface_v1* cms, wl_output* output);
