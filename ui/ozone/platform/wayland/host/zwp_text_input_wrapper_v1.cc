...
{
    return base::SplitString(std::string_view(static_cast<char*>(array->data),
...
// supported.
std::optional<ZWPTextInputWrapperClient::SpanStyle::Style> ConvertStyle(uint32_t style)
{
    switch (style) {
    ...
    case ZWP_TEXT_INPUT_V1_PREEDIT_STYLE_HIGHLIGHT:
        return { { ImeTextSpan::Type::kComposition, ImeTextSpan::Thickness::kThick } };
    ...
    case ZWP_TEXT_INPUT_V1_PREEDIT_STYLE_INCORRECT:
        return { { ImeTextSpan::Type::kMisspellingSuggestion, ImeTextSpan::Thickness::kNone } };
...
ZWPTextInputWrapperV1::ZWPTextInputWrapperV1(WaylandConnection* connection, ZWPTextInputWrapperClient* client, zwp_text_input_manager_v1* text_input_manager,
    ...
    , client_(client)
{
    ...
    static constexpr zcr_extended_text_input_v1_listener kExtendedTextInputListener = {
        ...
        .set_virtual_keyboard_occluded_bounds = &OnSetVirtualKeyboardOccludedBounds,
        .confirm_preedit = &OnConfirmPreedit,
        .insert_image = &OnInsertImage,
        .insert_image_with_large_url = &OnInsertImageWithLargeURL,
    };

    obj_ = wl::Object<zwp_text_input_v1>(zwp_text_input_manager_v1_create_text_input(text_input_manager));
    DCHECK(obj_.get());
    zwp_text_input_v1_add_listener(obj_.get(), &kTextInputListener, this);

    if (text_input_extension) {
        extended_obj_ = wl::Object<zcr_extended_text_input_v1>(zcr_text_input_extension_v1_get_extended_text_input(text_input_extension, obj_.get()));
        DCHECK(extended_obj_.get());
        zcr_extended_text_input_v1_add_listener(extended_obj_.get(), &kExtendedTextInputListener, this);
    }
}
...
void ZWPTextInputWrapperV1::Activate(WaylandWindow* window, TextInputClient::FocusReason reason)
{
    DCHECK(connection_->seat());
    if (extended_obj_.get() && wl::get_version_of_object(extended_obj_.get()) >= ZCR_EXTENDED_TEXT_INPUT_V1_SET_FOCUS_REASON_SINCE_VERSION) {
        std::optional<uint32_t> wayland_focus_reason;
        switch (reason) {
        case ui::TextInputClient::FocusReason::FOCUS_REASON_NONE:
            wayland_focus_reason = ZCR_EXTENDED_TEXT_INPUT_V1_FOCUS_REASON_TYPE_NONE;
            break;
        case ui::TextInputClient::FocusReason::FOCUS_REASON_MOUSE:
            wayland_focus_reason = ZCR_EXTENDED_TEXT_INPUT_V1_FOCUS_REASON_TYPE_MOUSE;
            break;
        case ui::TextInputClient::FocusReason::FOCUS_REASON_TOUCH:
            wayland_focus_reason = ZCR_EXTENDED_TEXT_INPUT_V1_FOCUS_REASON_TYPE_TOUCH;
            break;
        ...
        case ui::TextInputClient::FocusReason::FOCUS_REASON_OTHER:
            wayland_focus_reason = ZCR_EXTENDED_TEXT_INPUT_V1_FOCUS_REASON_TYPE_OTHER;
            break;
        }

        if (wayland_focus_reason.has_value()) {
            zcr_extended_text_input_v1_set_focus_reason(extended_obj_.get(), wayland_focus_reason.value());
        }
    }
    zwp_text_input_v1_activate(obj_.get(), connection_->seat()->wl_object(), window->root_surface()->surface());
}
...
{
    zwp_text_input_v1_set_cursor_rectangle(obj_.get(), rect.x(), rect.y(), rect.width(), rect.height());
}

void ZWPTextInputWrapperV1::SetSurroundingText(const std::string& text, const gfx::Range& preedit_range, const gfx::Range& selection_range)
{
    ...
    if (HasAdvancedSurroundingTextSupport() && text.length() > kSizeLimit) {
        ...
        zcr_extended_text_input_v1_set_large_surrounding_text(extended_obj_.get(), memfd.get(), text.length(), selection_range.start(), selection_range.end());
    } else {
        zwp_text_input_v1_set_surrounding_text(obj_.get(), text.c_str(), selection_range.start(), selection_range.end());
    }
}

bool ZWPTextInputWrapperV1::HasAdvancedSurroundingTextSupport() const
{
    return extended_obj_.get() && wl::get_version_of_object(extended_obj_.get()) >= ZCR_EXTENDED_TEXT_INPUT_V1_SET_LARGE_SURROUNDING_TEXT_SINCE_VERSION;
}

void ZWPTextInputWrapperV1::SetSurroundingTextOffsetUtf16(uint32_t offset_utf16)
{
    if (HasAdvancedSurroundingTextSupport()) {
        zcr_extended_text_input_v1_set_surrounding_text_offset_utf16(extended_obj_.get(), offset_utf16);
    }
}

void ZWPTextInputWrapperV1::SetContentType(ui::TextInputType type, ui::TextInputMode mode, uint32_t flags, bool should_do_learning, bool can_compose_inline)
{
    ...
    if (extended_obj_.get()) {
        ...
        if (wl_server_version >= ZCR_EXTENDED_TEXT_INPUT_V1_SET_INPUT_TYPE_SINCE_VERSION) {
            zcr_extended_text_input_v1_set_input_type(extended_obj_.get(), ui::wayland::ConvertFromTextInputType(type),
                ui::wayland::ConvertFromTextInputMode(mode), ui::wayland::ConvertFromTextInputFlags(flags),
                should_do_learning ? ZCR_EXTENDED_TEXT_INPUT_V1_LEARNING_MODE_ENABLED : ZCR_EXTENDED_TEXT_INPUT_V1_LEARNING_MODE_DISABLED,
                can_compose_inline ? ZCR_EXTENDED_TEXT_INPUT_V1_INLINE_COMPOSITION_SUPPORT_SUPPORTED
                                   : ZCR_EXTENDED_TEXT_INPUT_V1_INLINE_COMPOSITION_SUPPORT_UNSUPPORTED);
            return;
        }
        if (wl_server_version >= ZCR_EXTENDED_TEXT_INPUT_V1_DEPRECATED_SET_INPUT_TYPE_SINCE_VERSION) {
            ...
            zcr_extended_text_input_v1_deprecated_set_input_type(extended_obj_.get(), ui::wayland::ConvertFromTextInputType(type),
                ui::wayland::ConvertFromTextInputMode(mode), ui::wayland::ConvertFromTextInputFlags(flags),
                should_do_learning ? ZCR_EXTENDED_TEXT_INPUT_V1_LEARNING_MODE_ENABLED : ZCR_EXTENDED_TEXT_INPUT_V1_LEARNING_MODE_DISABLED);
            return;
        }
...
void ZWPTextInputWrapperV1::SetGrammarFragmentAtCursor(const ui::GrammarFragment& fragment)
{
    if (extended_obj_.get() && wl::get_version_of_object(extended_obj_.get()) >= ZCR_EXTENDED_TEXT_INPUT_V1_SET_GRAMMAR_FRAGMENT_AT_CURSOR_SINCE_VERSION) {
        zcr_extended_text_input_v1_set_grammar_fragment_at_cursor(
            extended_obj_.get(), fragment.range.start(), fragment.range.end(), fragment.suggestion.c_str());
    }
}

void ZWPTextInputWrapperV1::SetAutocorrectInfo(const gfx::Range& autocorrect_range, const gfx::Rect& autocorrect_bounds)
{
    if (extended_obj_.get() && wl::get_version_of_object(extended_obj_.get()) >= ZCR_EXTENDED_TEXT_INPUT_V1_SET_AUTOCORRECT_INFO_SINCE_VERSION) {
        zcr_extended_text_input_v1_set_autocorrect_info(extended_obj_.get(), autocorrect_range.start(), autocorrect_range.end(), autocorrect_bounds.x(),
            autocorrect_bounds.y(), autocorrect_bounds.width(), autocorrect_bounds.height());
    }
}
...
{
    if (!SupportsFinalizeVirtualKeyboardChanges() || send_vk_finalize_timer_.IsRunning()) {
        return;
    }

    send_vk_finalize_timer_.Start(FROM_HERE, base::Microseconds(0), this, &ZWPTextInputWrapperV1::FinalizeVirtualKeyboardChanges);
}

void ZWPTextInputWrapperV1::FinalizeVirtualKeyboardChanges()
{
    DCHECK(SupportsFinalizeVirtualKeyboardChanges());
    zcr_extended_text_input_v1_finalize_virtual_keyboard_changes(extended_obj_.get());
}

bool ZWPTextInputWrapperV1::SupportsFinalizeVirtualKeyboardChanges()
{
    return extended_obj_.get() && wl::get_version_of_object(extended_obj_.get()) >= ZCR_EXTENDED_TEXT_INPUT_V1_FINALIZE_VIRTUAL_KEYBOARD_CHANGES_SINCE_VERSION;
}

// static
void ZWPTextInputWrapperV1::OnEnter(void* data, struct zwp_text_input_v1* text_input, struct wl_surface* surface)
{
    NOTIMPLEMENTED_LOG_ONCE();
}

// static
void ZWPTextInputWrapperV1::OnLeave(void* data, struct zwp_text_input_v1* text_input)
{
    NOTIMPLEMENTED_LOG_ONCE();
}

// static
void ZWPTextInputWrapperV1::OnModifiersMap(void* data, struct zwp_text_input_v1* text_input, struct wl_array* map)
{
...
// static
void ZWPTextInputWrapperV1::OnInputPanelState(void* data, struct zwp_text_input_v1* text_input, uint32_t state)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
...
// static
void ZWPTextInputWrapperV1::OnPreeditString(void* data, struct zwp_text_input_v1* text_input, uint32_t serial, const char* text, const char* commit)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
    auto spans = std::move(self->spans_);
    int32_t preedit_cursor = self->preedit_cursor_;
    self->ResetInputEventState();
    self->client_->OnPreeditString(text, spans, preedit_cursor < 0 ? gfx::Range::InvalidRange() : gfx::Range(preedit_cursor));
}

// static
void ZWPTextInputWrapperV1::OnPreeditStyling(void* data, struct zwp_text_input_v1* text_input, uint32_t index, uint32_t length, uint32_t style)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
    self->spans_.push_back(ZWPTextInputWrapperClient::SpanStyle { index, length, ConvertStyle(style) });
}

// static
void ZWPTextInputWrapperV1::OnPreeditCursor(void* data, struct zwp_text_input_v1* text_input, int32_t index)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
...
// static
void ZWPTextInputWrapperV1::OnCommitString(void* data, struct zwp_text_input_v1* text_input, uint32_t serial, const char* text)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
    self->ResetInputEventState();
...
// static
void ZWPTextInputWrapperV1::OnCursorPosition(void* data, struct zwp_text_input_v1* text_input, int32_t index, int32_t anchor)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
...
// static
void ZWPTextInputWrapperV1::OnDeleteSurroundingText(void* data, struct zwp_text_input_v1* text_input, int32_t index, uint32_t length)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
...
// static
void ZWPTextInputWrapperV1::OnKeysym(
    void* data, struct zwp_text_input_v1* text_input, uint32_t serial, uint32_t time, uint32_t key, uint32_t state, uint32_t modifiers)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
...
// static
void ZWPTextInputWrapperV1::OnLanguage(void* data, struct zwp_text_input_v1* text_input, uint32_t serial, const char* language)
{
    NOTIMPLEMENTED_LOG_ONCE();
}

// static
void ZWPTextInputWrapperV1::OnTextDirection(void* data, struct zwp_text_input_v1* text_input, uint32_t serial, uint32_t direction)
{
    NOTIMPLEMENTED_LOG_ONCE();
}

// static
void ZWPTextInputWrapperV1::OnSetPreeditRegion(void* data, struct zcr_extended_text_input_v1* extended_text_input, int32_t index, uint32_t length)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
    auto spans = std::move(self->spans_);
    self->ResetInputEventState();
...
// static
void ZWPTextInputWrapperV1::OnClearGrammarFragments(void* data, struct zcr_extended_text_input_v1* extended_text_input, uint32_t start, uint32_t end)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
...
void ZWPTextInputWrapperV1::OnAddGrammarFragment(
    void* data, struct zcr_extended_text_input_v1* extended_text_input, uint32_t start, uint32_t end, const char* suggestion)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
    self->client_->OnAddGrammarFragment(ui::GrammarFragment(gfx::Range(start, end), suggestion));
}

// static
void ZWPTextInputWrapperV1::OnSetAutocorrectRange(void* data, struct zcr_extended_text_input_v1* extended_text_input, uint32_t start, uint32_t end)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
...
void ZWPTextInputWrapperV1::OnSetVirtualKeyboardOccludedBounds(
    void* data, struct zcr_extended_text_input_v1* extended_text_input, int32_t x, int32_t y, int32_t width, int32_t height)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
...
// static
void ZWPTextInputWrapperV1::OnConfirmPreedit(void* data, struct zcr_extended_text_input_v1* extended_text_input, uint32_t selection_behavior)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
...
// static
void ZWPTextInputWrapperV1::OnInsertImage(void* data, struct zcr_extended_text_input_v1* extended_text_input, const char* src)
{
    auto* self = static_cast<ZWPTextInputWrapperV1*>(data);
...
void ZWPTextInputWrapperV1::OnInsertImageWithLargeURL(
    void* data, struct zcr_extended_text_input_v1* extended_text_input, const char* mime_type, const char* charset, const int32_t raw_fd, const uint32_t size)
{
