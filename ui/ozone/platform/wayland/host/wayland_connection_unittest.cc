...
TEST_P(WaylandConnectionTest, CompositorVersionTest)
{
    ...
    uint32_t expected_compositor_version = static_cast<uint32_t>(config.compositor_version);

    EXPECT_EQ(expected_compositor_version, wl::get_version_of_object(connection_->compositor()));
}

INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandConnectionTest, Values(wl::ServerConfig { .compositor_version = wl::TestCompositor::Version::kV3 }));
INSTANTIATE_TEST_SUITE_P(
    XdgVersionStableTestCompositorV4, WaylandConnectionTest, Values(wl::ServerConfig { .compositor_version = wl::TestCompositor::Version::kV4 }));
