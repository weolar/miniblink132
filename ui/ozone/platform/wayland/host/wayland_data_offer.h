...
public:
    ...
    uint32_t id() const
    {
        return data_offer_.id();
    }

private:
    // wl_data_offer_listener callbacks:
    static void OnOffer(void* data, wl_data_offer* data_offer, const char* mime_type);
    // Notifies the source-side available actions
    static void OnSourceActions(void* data, wl_data_offer* offer, uint32_t source_actions);
