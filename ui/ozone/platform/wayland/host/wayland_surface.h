...
public:
    using ExplicitReleaseCallback = base::OnceCallback<void(wl_buffer*, base::ScopedFD)>;
    ...
    WaylandWindow* root_window() const
    {
        return root_window_;
    }
    ...
    wp_viewport* viewport() const
    {
        return viewport_.get();
    }
    zcr_blending_v1* blending() const
    {
        return blending_.get();
    }

    uint32_t buffer_id() const
    {
        return state_.buffer_id;
    }
    float opacity() const
    {
    ...
    WaylandZAuraSurface* zaura_surface()
    {
    ...
    uint32_t get_surface_id() const
    {
        return surface_ ? surface_.id() : 0u;
    }
...
private:
    FRIEND_TEST_ALL_PREFIXES(WaylandWindowTest, DoesNotCreateSurfaceSyncOnCommitWithoutBuffers);
    FRIEND_TEST_ALL_PREFIXES(PerSurfaceScaleWaylandWindowTest, UiScale_HandleFontScaleChange);
    FRIEND_TEST_ALL_PREFIXES(PerSurfaceScaleWaylandWindowTest, UiScale_HandleServerTriggeredBoundsChange);
    FRIEND_TEST_ALL_PREFIXES(PerSurfaceScaleWaylandWindowTest, UiScale_InitScaleAndBounds);
    FRIEND_TEST_ALL_PREFIXES(PerSurfaceScaleWaylandWindowTest, UiScale_HandlePopupGeometry);
    FRIEND_TEST_ALL_PREFIXES(WaylandSurfaceTest, SetExplicitSyncSuccess);
    FRIEND_TEST_ALL_PREFIXES(WaylandSurfaceExplicitSyncTest, ConfigureWithExplicitSync);
    FRIEND_TEST_ALL_PREFIXES(WaylandSurfaceExplicitSyncTest, ExplicitSyncNotSet_AcquireTimelineCreationFailed);
    FRIEND_TEST_ALL_PREFIXES(WaylandSurfaceExplicitSyncTest, ExplicitSyncNotSet_ReleaseTimelineCreationFailed);
    FRIEND_TEST_ALL_PREFIXES(WaylandSurfaceExplicitSyncTest, ExplicitSyncNotSet_InitialAcquireFenceNotSet);
    FRIEND_TEST_ALL_PREFIXES(WaylandSurfaceExplicitSyncTest, ExplicitNotSyncSet_InitialAcquireFenceImportFail);
    FRIEND_TEST_ALL_PREFIXES(WaylandSurfaceExplicitSyncTest, ExplicitSyncSet_SubsequentAcquireFenceNotSet);
    FRIEND_TEST_ALL_PREFIXES(WaylandSurfaceExplicitSyncTest, ExplicitSyncNotSet_SubsequentAcquireFenceImportFail);
    ...
    struct ExplicitReleaseInfoLegacy {
        ...
        ExplicitReleaseInfoLegacy& operator=(const ExplicitReleaseInfoLegacy&) = delete;
    ...
    void OnFenceAvailable(uint32_t buffer_id, ExplicitReleaseCallback callback, base::ScopedFD fd);

    // Tracks the last sent src and dst values across wayland protocol s.t. we
    // skip resending them when possible.
    wl_fixed_t src_set_[4] = { wl_fixed_from_int(-1), wl_fixed_from_int(-1), wl_fixed_from_int(-1), wl_fixed_from_int(-1) };
    ...
    wl::Object<wl_region> CreateAndAddRegion(const std::vector<gfx::Rect>& region_px, float buffer_scale);
    ...
    std::unique_ptr<WaylandZcrColorManagementSurface> zcr_color_management_surface_;
    std::unique_ptr<WaylandZAuraSurface> zaura_surface_;
    base::flat_map<zwp_linux_buffer_release_v1*, ExplicitReleaseInfoLegacy> linux_buffer_releases_legacy_;
    ...
    void ExplicitRelease(zwp_linux_buffer_release_v1* linux_buffer_release, base::ScopedFD fence);
    ...
    static void OnPreferredScale(void* data, wp_fractional_scale_v1* fractional_scale, uint32_t scale);

    // zwp_linux_buffer_release_v1_listener callbacks:
    static void OnFencedRelease(void* data, zwp_linux_buffer_release_v1* buffer_release, int32_t fence);
    static void OnImmediateRelease(void* data, zwp_linux_buffer_release_v1* buffer_release);
