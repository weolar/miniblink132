...
protected:
    ...
    {
        PostToServerAndWait([x, y, surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
            auto* const pointer = server->seat()->pointer()->resource();
            auto* const surface = server->GetObject<wl::MockSurface>(surface_id)->resource();

            wl_pointer_send_enter(pointer, server->GetNextSerial(), surface, wl_fixed_from_int(x), wl_fixed_from_int(y));
    ...
    void SendLeave()
    {
        PostToServerAndWait([surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
            auto* const pointer = server->seat()->pointer()->resource();
            auto* const surface = server->GetObject<wl::MockSurface>(surface_id)->resource();
    ...
    void SendAxisStopEvents()
    {
        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            ...
            wl_pointer_send_axis_stop(pointer, server->GetNextTime(), WL_POINTER_AXIS_VERTICAL_SCROLL);
            wl_pointer_send_axis_stop(pointer, server->GetNextTime(), WL_POINTER_AXIS_HORIZONTAL_SCROLL);
            wl_pointer_send_frame(pointer);
        });
    }
    ...
    void SendRightButtonPress()
    {
        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            ...
            wl_pointer_send_button(pointer, server->GetNextSerial(), server->GetNextTime(), BTN_RIGHT, WL_POINTER_BUTTON_STATE_PRESSED);
            wl_pointer_send_frame(pointer);
        });
    }

    void CheckEventType(ui::EventType event_type, ui::Event* event, ui::EventPointerType pointer_type = ui::EventPointerType::kMouse,
        float force = std::numeric_limits<float>::quiet_NaN(), float tilt_x = 0.0, float tilt_y = 0.0)
    {
        ASSERT_TRUE(event);
...
void SendAxisEvents(struct wl_resource* resource, uint32_t time_ms, uint32_t axis_source, uint32_t axis, int offset)
{
...
void SendDiagonalAxisEvents(struct wl_resource* resource, uint32_t time_ms, uint32_t axis_source, int offset_x, int offset_y)
{
    wl_pointer_send_axis_source(resource, axis_source);
    wl_pointer_send_axis(resource, time_ms, WL_POINTER_AXIS_VERTICAL_SCROLL, wl_fixed_from_int(offset_y));
    wl_pointer_send_axis(resource, time_ms, WL_POINTER_AXIS_HORIZONTAL_SCROLL, wl_fixed_from_int(offset_x));
    wl_pointer_send_frame(resource);
}
...
TEST_F(WaylandPointerTest, Leave)
{
    ...
    PostToServerAndWait([surface_id = window_->root_surface()->get_surface_id(), other_surface_id = other_window->root_surface()->get_surface_id()](
                            wl::TestWaylandServerThread* server) {
        auto* const pointer = server->seat()->pointer()->resource();
        auto* const surface = server->GetObject<wl::MockSurface>(surface_id)->resource();
        auto* const other_surface = server->GetObject<wl::MockSurface>(other_surface_id)->resource();
        ...
        wl_pointer_send_button(pointer, 4, 1004, BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);
        wl_pointer_send_frame(pointer);
    });
}
...
TEST_F(WaylandPointerTest, Motion)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        wl_pointer_send_motion(pointer, 1002, wl_fixed_from_double(10.75), wl_fixed_from_double(20.375));
        wl_pointer_send_frame(pointer);
    });
...
TEST_F(WaylandPointerTest, MotionDragged)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        wl_pointer_send_button(pointer, server->GetNextSerial(), server->GetNextTime(), BTN_MIDDLE, WL_POINTER_BUTTON_STATE_PRESSED);
        wl_pointer_send_frame(pointer);
    });
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        wl_pointer_send_motion(pointer, 1003, wl_fixed_from_int(400), wl_fixed_from_int(500));
        wl_pointer_send_frame(pointer);
    });
...
TEST_F(WaylandPointerTest, MotionDraggedWithStylus)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        auto* const pointer = server->seat()->pointer()->resource();
        auto* const stylus = server->seat()->pointer()->pointer_stylus()->resource();

        wl_pointer_send_button(pointer, server->GetNextSerial(), server->GetNextTime(), BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);

        // Stylus data.
        zcr_pointer_stylus_v2_send_tool(stylus, ZCR_POINTER_STYLUS_V2_TOOL_TYPE_PEN);
        zcr_pointer_stylus_v2_send_force(stylus, server->GetNextTime(), wl_fixed_from_double(1.0f));
        zcr_pointer_stylus_v2_send_tilt(stylus, server->GetNextTime(), wl_fixed_from_double(-45), wl_fixed_from_double(45));
        wl_pointer_send_frame(pointer);
    });

    CheckEventType(ui::EventType::kMousePressed, event.get(), ui::EventPointerType::kPen, 1.0f /* force */, -45.0f /* tilt_x */, 45.0f /* tilt_y */);

    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        auto* const pointer = server->seat()->pointer()->resource();

        wl_pointer_send_motion(pointer, server->GetNextTime(), wl_fixed_from_int(400), wl_fixed_from_int(500));
        wl_pointer_send_frame(pointer);
    });
...
TEST_F(WaylandPointerTest, AxisSourceTypes)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        for (const auto source :
            { WL_POINTER_AXIS_SOURCE_WHEEL, WL_POINTER_AXIS_SOURCE_FINGER, WL_POINTER_AXIS_SOURCE_CONTINUOUS, WL_POINTER_AXIS_SOURCE_WHEEL_TILT }) {
            SendAxisEvents(pointer, server->GetNextTime(), source, WL_POINTER_AXIS_VERTICAL_SCROLL, rand() % 20);
        }
    });
...
TEST_F(WaylandPointerTest, SpuriousAxisSourceAndStylusToolEvents)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        // Stylus data.
        zcr_pointer_stylus_v2_send_tool(stylus, ZCR_POINTER_STYLUS_V2_TOOL_TYPE_NONE);
        wl_pointer_send_frame(pointer);
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        wl_pointer_send_button(pointer, 2, 1, BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);
        wl_pointer_send_frame(pointer);
    });
...
TEST_F(WaylandPointerTest, Axis)
{
    ...
    for (uint32_t axis : { WL_POINTER_AXIS_VERTICAL_SCROLL, WL_POINTER_AXIS_HORIZONTAL_SCROLL }) {
        for (bool send_axis_source : { false, true }) {
            std::unique_ptr<Event> event;
            EXPECT_CALL(delegate_, DispatchEvent(_)).WillOnce(CloneEvent(&event));

            PostToServerAndWait([axis, send_axis_source](wl::TestWaylandServerThread* server) {
                auto* const pointer = server->seat()->pointer()->resource();
            ...
            EXPECT_EQ(
                axis == WL_POINTER_AXIS_VERTICAL_SCROLL ? gfx::Vector2d(0, -MouseWheelEvent::kWheelDelta) : gfx::Vector2d(-MouseWheelEvent::kWheelDelta, 0),
...
TEST_F(WaylandPointerTest, SetBitmap)
{
    ...
    dummy_cursor.setInfo(SkImageInfo::Make(16, 16, kUnknown_SkColorType, kUnknown_SkAlphaType));
...
TEST_F(WaylandPointerTest, SetBitmapAndScaleOnPointerFocus)
{
    for (float scale : { 1.0, 1.2, 1.5, 1.75, 2.0, 2.5, 3.0 }) {
        gfx::Size size = { static_cast<int>(10 * scale), static_cast<int>(10 * scale) };
        SkBitmap dummy_cursor;
        SkImageInfo info = SkImageInfo::Make(size.width(), size.height(), SkColorType::kBGRA_8888_SkColorType, SkAlphaType::kPremul_SkAlphaType);
        dummy_cursor.allocPixels(info, size.width() * 4);

        const gfx::Point hotspot_px = { 5, 8 };
        const gfx::Point hotspot_dip = gfx::ScaleToRoundedPoint(hotspot_px, 1 / scale);
...
#endif
        auto cursor = cursor_factory.CreateImageCursor(mojom::CursorType::kCustom, dummy_cursor, hotspot_px, scale);
        ...
        PostToServerAndWait([&surface_resource, hotspot_dip](wl::TestWaylandServerThread* server) {
            auto* const pointer = server->seat()->pointer();

            // Allow up to 1 DIP of precision loss.
            EXPECT_CALL(*pointer,
                SetCursor(Ne(nullptr), AllOf(Ge(hotspot_dip.x() - 1), Le(hotspot_dip.x() + 1)), AllOf(Ge(hotspot_dip.y() - 1), Le(hotspot_dip.y() + 1))))
        ...
        PostToServerAndWait([surface_resource, scale, hotspot_dip](wl::TestWaylandServerThread* server) {
            auto* const pointer = server->seat()->pointer();

            auto* mock_pointer_surface = wl::MockSurface::FromResource(surface_resource);
#if BUILDFLAG(IS_CHROMEOS_LACROS)
...
TEST_F(WaylandPointerTest, FlingVertical)
{
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).Times(3).WillOnce(CloneEvent(&event1)).WillOnce(CloneEvent(&event2)).WillOnce(CloneEvent(&event3));
    ...
    for (int n = 0; n < 2; ++n) {
        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            ...
            SendAxisEvents(pointer, server->GetNextTime(), WL_POINTER_AXIS_SOURCE_FINGER, WL_POINTER_AXIS_VERTICAL_SCROLL, 10);
        });
...
TEST_F(WaylandPointerTest, FlingHorizontal)
{
    ...
    for (int n = 0; n < 2; ++n) {
        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            ...
            SendAxisEvents(pointer, server->GetNextTime(), WL_POINTER_AXIS_SOURCE_FINGER, WL_POINTER_AXIS_HORIZONTAL_SCROLL, 10);
        });
...
TEST_F(WaylandPointerTest, FlingCancel)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        auto* const pointer = server->seat()->pointer()->resource();
        SendAxisEvents(pointer, server->GetNextTime(), WL_POINTER_AXIS_SOURCE_FINGER, WL_POINTER_AXIS_VERTICAL_SCROLL, 0);
    });
#endif
...
TEST_F(WaylandPointerTest, FlingDiagonal)
{
    ...
    for (int n = 0; n < 2; ++n) {
        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            ...
            SendDiagonalAxisEvents(pointer, server->GetNextTime(), WL_POINTER_AXIS_SOURCE_FINGER, 20, 10);
        });
    ...
    EXPECT_GT(std::abs(scroll_event->x_offset()), std::abs(scroll_event->y_offset()));
    EXPECT_GT(std::abs(scroll_event->x_offset_ordinal()), std::abs(scroll_event->y_offset_ordinal()));
}
...
TEST_F(WaylandPointerTest, FlingVelocityWithoutLeadingAxis)
{
    ...
    std::unique_ptr<Event> event;
    EXPECT_CALL(delegate_, DispatchEvent(_)).Times(1).WillOnce(CloneEvent(&event));
...
TEST_F(WaylandPointerTest, FlingVelocityWithSingleLeadingAxis)
{
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).Times(2).WillOnce(CloneEvent(&event1)).WillOnce(CloneEvent(&event2));
