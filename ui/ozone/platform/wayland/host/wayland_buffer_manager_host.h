...
public:
    ...
    void SetTerminateGpuCallback(base::OnceCallback<void(std::string)> terminate_gpu_cb);
    ...
    void SetWaylandBufferManagerGpu(mojo::PendingAssociatedRemote<ozone::mojom::WaylandBufferManagerGpu> buffer_manager_gpu_associated) override;
    //
    ...
    void CreateDmabufBasedBuffer(mojo::PlatformHandle dmabuf_fd, const gfx::Size& size, const std::vector<uint32_t>& strides,
        const std::vector<uint32_t>& offsets, const std::vector<uint64_t>& modifiers, uint32_t format, uint32_t planes_count, uint32_t buffer_id) override;
    ...
    // ui/ozone/platform/wayland/mojom/wayland_buffer_manager.mojom.
    void CreateShmBasedBuffer(mojo::PlatformHandle shm_fd, uint64_t length, const gfx::Size& size, uint32_t buffer_id) override;
    ...
    void CreateSolidColorBuffer(const gfx::Size& size, const SkColor4f& color, uint32_t buffer_id) override;
    ...
    void CreateSinglePixelBuffer(const SkColor4f& color, uint32_t buffer_id) override;
    ...
    void CommitOverlays(gfx::AcceleratedWidget widget, uint32_t frame_id, const gfx::FrameData& data, std::vector<wl::WaylandOverlayConfig> overlays) override;
    ...
    // buffer handle.
    WaylandBufferHandle* EnsureBufferHandle(WaylandSurface* requestor, uint32_t buffer_id);
    ...
    void OnSubmission(gfx::AcceleratedWidget widget, uint32_t frame_id, const gfx::SwapResult& swap_result, gfx::GpuFenceHandle release_fence,
        const std::vector<wl::WaylandPresentationInfo>& presentation_infos);
    void OnPresentation(gfx::AcceleratedWidget widget, const std::vector<wl::WaylandPresentationInfo>& presentation_infos);
...
private:
    ...
    bool ValidateDataFromGpu(const base::ScopedFD& file, const gfx::Size& size, const std::vector<uint32_t>& strides, const std::vector<uint32_t>& offsets,
        const std::vector<uint64_t>& modifiers, uint32_t format, uint32_t planes_count, uint32_t buffer_id);
    bool ValidateBufferIdFromGpu(uint32_t buffer_id);
    bool ValidateDataFromGpu(const base::ScopedFD& file, size_t length, const gfx::Size& size, uint32_t buffer_id);
    ...
    mojo::AssociatedRemote<ozone::mojom::WaylandBufferManagerGpu> buffer_manager_gpu_associated_;
    ...
    base::flat_map<uint32_t, std::unique_ptr<WaylandBufferBacking>> buffer_backings_;
