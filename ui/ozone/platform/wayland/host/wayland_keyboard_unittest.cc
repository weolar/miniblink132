...
protected:
    ...
    {
        PostToServerAndWait([rate, delay, surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
...
TEST_F(WaylandKeyboardTest, ControlShiftModifiers)
{
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).Times(3).WillRepeatedly(AppendEvent(&events));
    ...
    PostToServerAndWait([surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
        ...
        wl_keyboard_send_key(keyboard, server->GetNextSerial(), server->GetNextTime(), 42 /* Shift */, WL_KEYBOARD_KEY_STATE_PRESSED);

        wl_keyboard_send_modifiers(keyboard, server->GetNextSerial(), 5 /* mods_depressed*/, 0 /* mods_latched */, 0 /* mods_locked */, 0 /* group */);
...
TEST_F(WaylandKeyboardTest, CapsLockModifier)
{
    ...
    PostToServerAndWait([surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
        ...
        wl_keyboard_send_key(keyboard, server->GetNextSerial(), server->GetNextTime(), 58 /* Capslock */, WL_KEYBOARD_KEY_STATE_PRESSED);

        wl_keyboard_send_modifiers(keyboard, server->GetNextSerial(), 2 /* mods_depressed*/, 0 /* mods_latched */, 2 /* mods_locked */, 0 /* group */);

        // Releasing capslock (led ON).
        wl_keyboard_send_key(keyboard, server->GetNextSerial(), server->GetNextTime(), 58 /* Capslock */, WL_KEYBOARD_KEY_STATE_RELEASED);

        wl_keyboard_send_modifiers(keyboard, server->GetNextSerial(), 0 /* mods_depressed*/, 0 /* mods_latched */, 2 /* mods_locked */, 0 /* group */);
...
TEST_F(WaylandKeyboardTest, EventAutoRepeat)
{
    ...
    std::vector<std::unique_ptr<Event>> events;
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillRepeatedly(AppendEventAndQuitLoop(&events, 3u, run_loop.QuitClosure()));
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        wl_keyboard_send_key(keyboard, server->GetNextSerial(), server->GetNextTime(), 30 /* a */, WL_KEYBOARD_KEY_STATE_RELEASED);
    });
    ...
    const auto first_repeat_delay = first_repeat_event.time_stamp() - press_timestamp;
    ...
    const auto second_repeat_delay = second_repeat_event.time_stamp() - press_timestamp - first_repeat_delay;
...
TEST_F(WaylandKeyboardTest, NoEventAutoRepeatOnLeave)
{
    ...
    base::RunLoop run_loop;
    std::vector<std::unique_ptr<Event>> events;
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillRepeatedly(AppendEventAndQuitLoop(&events, 2u, run_loop.QuitClosure()));
...
TEST_F(WaylandKeyboardTest, NoEventAutoRepeatForModifiers)
{
    ...
    for (auto values : kIncomeModifiersAndExpectedResults) {
        ...
        base::RunLoop run_loop;
        std::vector<std::unique_ptr<Event>> events;
        EXPECT_CALL(delegate_, DispatchEvent(_)).WillRepeatedly(AppendEvent(&events));

        PostToServerAndWait([&values](wl::TestWaylandServerThread* server) {
            auto* const keyboard = server->seat()->keyboard()->resource();

            wl_keyboard_send_key(keyboard, server->GetNextSerial(), server->GetNextTime(), values.evdev_key, WL_KEYBOARD_KEY_STATE_PRESSED);
            wl_keyboard_send_modifiers(
                keyboard, server->GetNextSerial(), values.mods_depressed /* mods_depressed*/, 0 /* mods_latched */, 0 /* mods_locked */, 0 /* group */);
        });

        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), base::Milliseconds(1000));
        run_loop.Run();
        Mock::VerifyAndClearExpectations(&delegate_);
        ...
        EXPECT_CALL(delegate_, DispatchEvent(_)).WillRepeatedly(AppendEventAndQuitLoop(&events2, 5u, run_loop2.QuitClosure()));
        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            auto* const keyboard = server->seat()->keyboard()->resource();
        ...
        PostToServerAndWait([&values](wl::TestWaylandServerThread* server) {
            ...
            wl_keyboard_send_key(keyboard, server->GetNextSerial(), server->GetNextTime(), values.evdev_key, WL_KEYBOARD_KEY_STATE_RELEASED);
            wl_keyboard_send_key(keyboard, server->GetNextSerial(), server->GetNextTime(), 30 /* a */, WL_KEYBOARD_KEY_STATE_RELEASED);
        });
    }
}
