...
WaylandShmBuffer::WaylandShmBuffer(WaylandBufferFactory* buffer_factory, const gfx::Size& size)
    : size_(size)
{
...
WaylandShmBuffer& WaylandShmBuffer::operator=(WaylandShmBuffer&& buffer) = default;
...
void WaylandShmBuffer::Initialize(WaylandBufferFactory* buffer_factory)
{
    ...
    base::subtle::PlatformSharedMemoryRegion platform_shared_memory
        = base::UnsafeSharedMemoryRegion::TakeHandleForSerialization(std::move(shared_memory_region));

    base::subtle::ScopedFDPair fd_pair = platform_shared_memory.PassPlatformHandle();
    buffer_ = buffer_factory->CreateShmBuffer(std::move(fd_pair.fd), buffer_size, size_);
    if (!buffer_) {
