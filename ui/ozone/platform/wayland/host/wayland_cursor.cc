...
WaylandCursor::WaylandCursor(WaylandPointer* pointer, WaylandConnection* connection)
    : pointer_(pointer)
...
void WaylandCursor::UpdateBitmap(const std::vector<SkBitmap>& cursor_image, const gfx::Point& hotspot_in_dips, int buffer_scale)
{
    if (!pointer_)
        return;
    ...
    static constexpr wl_buffer_listener wl_buffer_listener { &WaylandCursor::OnBufferRelease };
    wl_buffer_add_listener(buffer.get(), &wl_buffer_listener, this);

    wl::DrawBitmap(image, &buffer);

    AttachAndCommit(buffer.get(), image_size.width(), image_size.height(), hotspot_in_dips.x(), hotspot_in_dips.y());
...
{
    auto pointer_enter_serial = connection_->serial_tracker().GetSerial(wl::SerialType::kMouseEnter);
    ...
    DCHECK(pointer_);
    wl_pointer_set_cursor(pointer_->wl_object(), pointer_enter_serial->value, nullptr, 0, 0);
...
void WaylandCursor::SetPlatformShapeInternal()
{
    ...
    // valid span.
    auto image_span = UNSAFE_BUFFERS(base::span(cursor_data_->images, cursor_data_->image_count));
    wl_cursor_image* const cursor_image = image_span[current_image_index_];

    AttachAndCommit(wl_cursor_image_get_buffer(cursor_image), cursor_image->width, cursor_image->height, cursor_image->hotspot_x / buffer_scale_,
    ...
    if (cursor_data_->image_count > 1 && cursor_image->delay > 0) {
        ...
        animation_timer_.Start(FROM_HERE, base::Milliseconds(cursor_image->delay), this, &WaylandCursor::SetPlatformShapeInternal);
        ++current_image_index_;
    }
}

void WaylandCursor::AttachAndCommit(wl_buffer* buffer, uint32_t buffer_width, uint32_t buffer_height, uint32_t hotspot_x_dip, uint32_t hotspot_y_dip)
{
    auto pointer_enter_serial = connection_->serial_tracker().GetSerial(wl::SerialType::kMouseEnter);
    if (!pointer_enter_serial) {
        VLOG(1) << "Failed to hide cursor. No mouse enter serial found.";
    ...
    wl_pointer_set_cursor(pointer_->wl_object(), pointer_enter_serial->value, pointer_surface_.get(), hotspot_x_dip, hotspot_y_dip);
    wl_surface_commit(pointer_surface_.get());
