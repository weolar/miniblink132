...
TEST_F(WaylandSeatTest, TestMultipleCapabilityEvents)
{
    ...
    for (size_t _ : { 0, 1 }) {
        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            wl_seat_send_capabilities(server->seat()->resource(), WL_SEAT_CAPABILITY_TOUCH | WL_SEAT_CAPABILITY_KEYBOARD | WL_SEAT_CAPABILITY_POINTER);
        });
        ...
        validate_new_device_obtained(connection_->seat()->keyboard(), prev_keyboard_id);
        validate_new_device_obtained(connection_->seat()->pointer(), prev_pointer_id);
