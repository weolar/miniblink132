...
public:
    WaylandZAuraOutputManagerV2Test()
        : WaylandTestSimple(wl::ServerConfig { .enable_aura_shell = wl::EnableAuraShellProtocol::kEnabled })
    {
    }
...
protected:
    ...
    void SendMetricsOnServerThread(const WaylandOutput::Metrics& metrics)
    {
        PostToServerAndWait([&metrics](wl::TestWaylandServerThread* server) {
            wl_resource* manager_resource = server->zaura_output_manager_v2()->resource();
            const WaylandOutput::Id output_id = metrics.output_id;

            const auto wayland_display_id = ui::wayland::ToWaylandDisplayIdPair(metrics.display_id);
            zaura_output_manager_v2_send_display_id(manager_resource, output_id, wayland_display_id.high, wayland_display_id.low);
            zaura_output_manager_v2_send_logical_position(manager_resource, output_id, metrics.origin.x(), metrics.origin.y());
            zaura_output_manager_v2_send_logical_size(manager_resource, output_id, metrics.logical_size.width(), metrics.logical_size.height());
            zaura_output_manager_v2_send_physical_size(manager_resource, output_id, metrics.physical_size.width(), metrics.physical_size.height());
            zaura_output_manager_v2_send_work_area_insets(
                manager_resource, output_id, metrics.insets.top(), metrics.insets.left(), metrics.insets.bottom(), metrics.insets.right());
            zaura_output_manager_v2_send_overscan_insets(manager_resource, output_id, metrics.physical_overscan_insets.top(),
                metrics.physical_overscan_insets.left(), metrics.physical_overscan_insets.bottom(), metrics.physical_overscan_insets.right());
            zaura_output_manager_v2_send_device_scale_factor(manager_resource, output_id, base::bit_cast<uint32_t>(metrics.scale_factor));
            zaura_output_manager_v2_send_logical_transform(manager_resource, output_id, metrics.logical_transform);
            zaura_output_manager_v2_send_panel_transform(manager_resource, output_id, metrics.panel_transform);
            zaura_output_manager_v2_send_name(manager_resource, output_id, metrics.name.c_str());
            zaura_output_manager_v2_send_description(manager_resource, output_id, metrics.description.c_str());
        });
    }
    ...
    {
        PostToServerAndWait([&](wl::TestWaylandServerThread* server) { server->zaura_output_manager_v2()->set_send_done_on_config_change(false); });
    }
...
TEST_F(WaylandZAuraOutputManagerV2Test, ServerEventsPopulateOutputMetrics)
{
    ...
    const auto get_pending_client_metrics = [&]() { return aura_output_manager_v2()->pending_output_metrics_map_for_testing().at(output_id); };
    const auto get_client_metrics = [&]() { return aura_output_manager_v2()->output_metrics_map_for_testing().at(output_id); };
...
TEST_F(WaylandZAuraOutputManagerV2Test, ProcessesOutputGlobalAdd)
{
    ...
    PostToServerAndWait(
        [&](wl::TestWaylandServerThread* server) { secondary_output_id = server->CreateAndInitializeOutput()->GetOutputName(server->client()); });
    ...
    WaylandOutput* secondary_output = wayland_output_manager()->GetOutput(secondary_output_id);
...
TEST_F(WaylandZAuraOutputManagerV2Test, MultipleOutputChangeTransaction)
{
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        ...
        secondary_output_id = secondary_test_output->GetOutputName(server->client());
    });

    WaitForAllDisplaysReady();
    EXPECT_EQ(2u, wayland_output_manager()->GetAllOutputs().size());
    EXPECT_TRUE(wayland_output_manager()->GetOutput(primary_output_id)->IsReady());
    EXPECT_TRUE(wayland_output_manager()->GetOutput(secondary_output_id)->IsReady());
    ...
    DisableSendImplicitDoneOnServerThread();
    PostToServerAndWait(
        [&](wl::TestWaylandServerThread* server) { tertiary_output_id = server->CreateAndInitializeOutput()->GetOutputName(server->client()); });

    // 2. Remove the secondary output.
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) { secondary_test_output->DestroyGlobal(); });

    // 3. Create updated output metrics for the primary output.
    WaylandOutput* primary_output = wayland_output_manager()->GetOutput(primary_output_id);
    ...
    WaylandOutput* tertiary_output = wayland_output_manager()->GetOutput(tertiary_output_id);
    ...
    const auto get_primary_client_metrics = [&]() { return aura_output_manager_v2()->output_metrics_map_for_testing().at(primary_output_id); };
...
TEST_F(WaylandZAuraOutputManagerV2Test, ActiveDisplay)
{
    ...
    const int64_t secondary_display_id = platform_screen->GetAllDisplays()[1].id();

    // Activate the secondary output.
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) { server->zaura_output_manager_v2()->SendActivated(secondary_output); });
    EXPECT_EQ(secondary_display_id, display::Screen::GetScreen()->GetDisplayForNewWindows().id());

    // Activate the primary output.
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) { server->zaura_output_manager_v2()->SendActivated(primary_output); });
    EXPECT_EQ(primary_display_id, display::Screen::GetScreen()->GetDisplayForNewWindows().id());
}
