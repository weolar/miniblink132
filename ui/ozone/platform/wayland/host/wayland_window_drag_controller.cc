...
constexpr uint32_t kDndActionWindowDrag = WL_DATA_DEVICE_MANAGER_DND_ACTION_MOVE;
...
public:
        ...
        : connection_(connection)
    {
        ...
        uint32_t options
            = ZCR_EXTENDED_DRAG_V1_OPTIONS_ALLOW_SWALLOW | ZCR_EXTENDED_DRAG_V1_OPTIONS_ALLOW_DROP_NO_TARGET | ZCR_EXTENDED_DRAG_V1_OPTIONS_LOCK_CURSOR;
        source_.reset(zcr_extended_drag_v1_get_extended_drag_source(connection.extended_drag_v1(), source, options));
        DCHECK(source_);
    }

    void SetDraggedWindow(WaylandToplevelWindow* window, const gfx::Vector2d& offset)
    {
        auto* surface = window ? window->root_surface()->surface() : nullptr;
        zcr_extended_drag_source_v1_drag(source_.get(), surface, offset.x(), offset.y());
        connection_->Flush();
    }
...
public:
        ...
        : connection_(connection)
    {
        ...
        drag_.reset(xdg_toplevel_drag_manager_v1_get_xdg_toplevel_drag(connection.toplevel_drag_manager_v1(), source));
        DCHECK(drag_);
    }
    ...
    void SetDraggedWindow(WaylandToplevelWindow* window, const gfx::Vector2d& offset)
    {
        ...
        DCHECK(window->shell_toplevel() && window->shell_toplevel()->AsXDGToplevelWrapper());

        auto* toplevel = window->shell_toplevel()->AsXDGToplevelWrapper()->xdg_toplevel_.get();
        DCHECK(toplevel);
        ...
        xdg_toplevel_drag_v1_attach(drag_.get(), toplevel, toplevel_offset.x(), toplevel_offset.y());
        connection_->Flush();
    }
...
WaylandWindowDragController::WaylandWindowDragController(WaylandConnection* connection, WaylandDataDeviceManager* device_manager,
    WaylandPointer::Delegate* pointer_delegate, WaylandTouch::Delegate* touch_delegate, WaylandKeyboard::Delegate* keyboard_delegate)
    : connection_(connection)
...
bool WaylandWindowDragController::StartDragSession(WaylandToplevelWindow* origin, DragEventSource drag_source)
{
    ...
    if (!serial) {
        LOG(ERROR) << "Failed to retrieve dnd serial. origin=" << origin << " drag_source=" << drag_source;
        return false;
    }

    VLOG(1) << "Starting Window Drag session. tracker_id=" << serial->value << ", data_source=" << (drag_source == DragEventSource::kMouse ? "mouse" : "touch")
    ...
    if (connection_->toplevel_drag_manager_v1()) {
        xdg_toplevel_drag_ = std::make_unique<XdgToplevelDrag>(*connection_, data_source_->data_source());
    } else if (connection_->extended_drag_v1()) {
        extended_drag_source_ = std::make_unique<ExtendedDragSource>(*connection_, data_source_->data_source());
    } else {
        LOG(ERROR) << "zcr_extended_drag_v1 and xdg_toplevel_drag_v1 extensions "
...
bool WaylandWindowDragController::Drag(WaylandToplevelWindow* window, const gfx::Vector2d& offset)
{
    ...
    DCHECK(state_ == State::kAttaching || state_ == State::kDropped || state_ == State::kCancelled) << "Drag state: " << int(state_);
...
void WaylandWindowDragController::StopDragging()
{
    ...
    pointer_grab_owner_ = window_manager_->GetCurrentPointerOrTouchFocusedWindow();
...
void WaylandWindowDragController::OnDragOffer(std::unique_ptr<WaylandDataOffer> offer)
{
    DCHECK_GE(state_, State::kAttached);
    DCHECK(offer);
    DCHECK(!data_offer_);
...
void WaylandWindowDragController::OnDragEnter(WaylandWindow* window, const gfx::PointF& location, base::TimeTicks timestamp, uint32_t serial)
{
...
void WaylandWindowDragController::OnDragMotion(const gfx::PointF& location, base::TimeTicks timestamp)
{
    // Drag-and-drop sessions may be terminated by the client while drag-and-drop
    // server events are still in-flight. No-op if this is the case.
    if (!IsActiveDragAndDropSession()) {
    ...
    if (*drag_source_ == DragEventSource::kMouse) {
        pointer_delegate_->OnPointerMotionEvent(location, timestamp, wl::EventDispatchPolicy::kImmediate,
            /*is_synthesized=*/true);
    } else {
        const auto touch_pointer_ids = touch_delegate_->GetActiveTouchPointIds();
        LOG_IF(WARNING, touch_pointer_ids.size() < 1u) << "Unexpected touch drag motion. Active touch_points: " << touch_pointer_ids.size();
        for (auto id : touch_pointer_ids) {
            touch_delegate_->OnTouchMotionEvent(location, timestamp, id, wl::EventDispatchPolicy::kImmediate,
                /*is_synthesized=*/true);
        }
    }
}
...
void WaylandWindowDragController::OnDragLeave(base::TimeTicks timestamp)
{
    ...
    if (*drag_source_ == DragEventSource::kMouse) {
        pointer_delegate_->OnPointerMotionEvent({ pointer_location_.x(), -1 }, timestamp, wl::EventDispatchPolicy::kImmediate, /*is_synthesized=*/true);
    } else {
        const auto touch_pointer_ids = touch_delegate_->GetActiveTouchPointIds();
        for (auto id : touch_pointer_ids) {
            ...
            touch_delegate_->OnTouchMotionEvent(
                { pointer_location_.x(), kHorizontalRailExitThreshold }, timestamp, id, wl::EventDispatchPolicy::kImmediate, /*is_synthesized=*/true);
        }
    }
}
...
void WaylandWindowDragController::OnDataSourceDropPerformed(WaylandDataSource* source, base::TimeTicks timestamp)
{
    CHECK_GE(state_, State::kAttached);
...
void WaylandWindowDragController::OnDataSourceFinish(WaylandDataSource* source, base::TimeTicks timestamp, bool completed)
{
...
void WaylandWindowDragController::HandleDragEnd(bool completed, base::TimeTicks timestamp)
{
    ...
    State state_when_dropped = std::exchange(state_, completed || !IsWindowDragProtocolAvailable() ? State::kDropped : State::kCancelled);
...
void WaylandWindowDragController::OnDataSourceSend(WaylandDataSource* source, const std::string& mime_type, std::string* contents)
{
...
uint32_t WaylandWindowDragController::DispatchEvent(const PlatformEvent& event)
{
    ...
    // once.
    auto cancel_drag_cb
        = base::BindOnce(&WaylandWindowDragController::OnDataSourceFinish, base::Unretained(this), data_source_.get(), EventTimeForNow(), /*completed=*/false);
    if (wl::MaybeHandlePlatformEventForDrag(event, /*start_drag_ack_received=*/has_received_enter_, std::move(cancel_drag_cb))) {
        return POST_DISPATCH_STOP_PROPAGATION;
    }

    if (state_ == State::kDetached
        && (event->type() == EventType::kMouseMoved || event->type() == EventType::kMouseDragged || event->type() == EventType::kTouchMoved)) {
...
// about to finish.
void WaylandWindowDragController::HandleDropAndResetState(base::TimeTicks timestamp)
{
    ...
    if (state_ == State::kCancelled && IsWindowDragProtocolAvailable()) {
        ...
        if (*drag_source_ == DragEventSource::kMouse) {
            if (pointer_grab_owner_) {
                pointer_delegate_->ReleasePressedPointerButtons(pointer_grab_owner_, timestamp);
            }
        } else {
            const auto touch_pointer_ids = touch_delegate_->GetActiveTouchPointIds();
            for (auto id : touch_pointer_ids) {
                touch_delegate_->OnTouchReleaseEvent(timestamp, id, wl::EventDispatchPolicy::kImmediate,
                    /*is_synthesized=*/true);
            }
        }
    }
...
void WaylandWindowDragController::RunLoop()
{
    ...
    VLOG(1) << "Starting drag loop. widget=" << dragged_window_->GetWidget() << " offset=" << drag_offset_.ToString();
...
void WaylandWindowDragController::SetDraggedWindow(WaylandToplevelWindow* window, const gfx::Vector2d& offset)
{
...
{
    return !!connection_->extended_drag_v1() || !!connection_->toplevel_drag_manager_v1() || window_drag_protocol_available_for_testing_;
}
...
{
    constexpr auto kStateToString = base::MakeFixedFlatMap<State, const char*>({ { State::kIdle, "idle" }, { State::kAttached, "attached" },
        { State::kDetached, "detached" }, { State::kDropped, "dropped" }, { State::kCancelled, "canceled" }, { State::kAttaching, "attaching" } });

    out << "WaylandWindowDragController:"
        << " state=" << GetMapValueOrDefault(kStateToString, state_) << ", drag_offset=" << drag_offset_.ToString()
        << ", pointer_position=" << pointer_location_.ToString() << ", data_source=" << !!data_source_
        << ", dragged_window=" << GetWindowName(dragged_window_.get()) << ", pointer_grab_owner=" << GetWindowName(pointer_grab_owner_.get())
        << ", origin_window=" << GetWindowName(origin_window_.get()) << ", drag_target_window=" << GetWindowName(drag_target_window_.get())
        << ", nested_dispatcher=" << !!nested_dispatcher_;
}

std::ostream& operator<<(std::ostream& out, WaylandWindowDragController::State state)
{
    return out << static_cast<int>(state);
}

std::optional<wl::Serial> WaylandWindowDragController::GetSerial(DragEventSource drag_source, WaylandToplevelWindow* origin)
{
    auto* focused
        = drag_source == DragEventSource::kMouse ? window_manager_->GetCurrentPointerFocusedWindow() : window_manager_->GetCurrentTouchFocusedWindow();
    if (!origin || focused != origin) {
        return std::nullopt;
    }
    return connection_->serial_tracker().GetSerial(drag_source == DragEventSource::kMouse ? wl::SerialType::kMousePress : wl::SerialType::kTouchPress);
}
