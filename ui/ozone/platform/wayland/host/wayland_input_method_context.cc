...
std::optional<size_t> OffsetFromUTF8Offset(std::string_view text, uint32_t offset)
{
...
bool IsImeEnabled()
{
    ...
    DCHECK(!cmd_line->HasSwitch(switches::kEnableWaylandIme) || !cmd_line->HasSwitch(switches::kDisableWaylandIme));
    ...
    const chromeos::BrowserParamsProxy* init_params = chromeos::BrowserParamsProxy::Get();
    if (init_params->ExoImeSupport() != crosapi::mojom::ExoImeSupport::kUnsupported) {
        return true;
    }
#endif
...
std::optional<OffsetText> TrimSurroundingTextForStandard(std::string_view text_utf8, gfx::Range selection_utf8)
{
    ...
    uint32_t selection_range_utf8_center = selection_utf8.start() + selection_utf8.length() / 2;
    ...
    if (selection_range_utf8_center <= kWaylandMessageDataMaxLength / 2) {
        ...
        start_index = 0;
    } else if (text_utf8.size() - selection_range_utf8_center < kWaylandMessageDataMaxLength / 2) {
        ...
        // range.
        start_index = selection_range_utf8_center - kWaylandMessageDataMaxLength / 2;
    }

    gfx::Range truncated_range = AdjustUtf8Alignment(text_utf8, gfx::Range(start_index, start_index + kWaylandMessageDataMaxLength));

    return OffsetText { std::string(text_utf8.substr(truncated_range.start(), truncated_range.length())), truncated_range.start() };
}

std::optional<OffsetText> TrimSurroundingTextForExtension(std::string_view text_utf8, const base::span<size_t> offsets)
{
    ...
    size_t start_index = min_offset - std::min(min_offset, kMaxSurroundingTextBytes);
    size_t end_index = max_offset + kMaxSurroundingTextBytes;

    gfx::Range truncated_range = AdjustUtf8Alignment(text_utf8, gfx::Range(start_index, end_index));
...
WaylandInputMethodContext::WaylandInputMethodContext(
    WaylandConnection* connection, WaylandKeyboard::Delegate* key_delegate, LinuxInputMethodContextDelegate* ime_delegate)
...
void WaylandInputMethodContext::CreateTextInputWrapper()
{
    ...
    const std::string version_from_cmd_line = cmd_line->GetSwitchValueASCII(switches::kWaylandTextInputVersion);
    const bool enable_using_cmd_line_version = cmd_line->HasSwitch(switches::kEnableWaylandIme) && !version_from_cmd_line.empty();

    if (base::FeatureList::IsEnabled(features::kWaylandTextInputV3) || (enable_using_cmd_line_version && version_from_cmd_line == kWaylandTextInputVersion3)) {
        if (connection_->text_input_manager_v3()) {
            text_input_ = std::make_unique<ZWPTextInputWrapperV3>(connection_, this, connection_->text_input_manager_v3());
        } else {
            LOG(WARNING) << "text-input-v3 not available.";
        }
        return;
    } else if (enable_using_cmd_line_version && version_from_cmd_line != kWaylandTextInputVersion1) {
        LOG(WARNING) << "text input version should be either 1 or 3. Defaulting to "
                        "text-input-v1.";
    }
    if (connection_->text_input_manager_v1()) {
        text_input_ = std::make_unique<ZWPTextInputWrapperV1>(connection_, this, connection_->text_input_manager_v1(), connection_->text_input_extension_v1());
    } else {
...
void WaylandInputMethodContext::Init(
    bool initialize_for_testing, std::unique_ptr<ZWPTextInputWrapper> wrapper_for_testing, std::optional<base::nix::DesktopEnvironment> desktop_for_testing)
{
    desktop_environment_ = desktop_for_testing.value_or(base::nix::GetDesktopEnvironment(base::Environment::Create().get()));
...
void WaylandInputMethodContext::UpdatePreeditText(const std::u16string& preedit_text)
{
    ...
    preedit.ime_text_spans.push_back(
        ImeTextSpan(ImeTextSpan::Type::kComposition, 0, length, ImeTextSpan::Thickness::kThin, ImeTextSpan::UnderlineStyle::kSolid, SK_ColorTRANSPARENT));
...
void WaylandInputMethodContext::WillUpdateFocus(TextInputClient* old_client, TextInputClient* new_client)
{
    if (old_client) {
...
void WaylandInputMethodContext::UpdateFocus(
    bool has_client, TextInputType old_type, const TextInputClientAttributes& new_client_attributes, TextInputClient::FocusReason reason)
{
    ...
    bool skip_vk_update = old_type != TEXT_INPUT_TYPE_NONE && new_type != TEXT_INPUT_TYPE_NONE;
...
void WaylandInputMethodContext::Focus(bool skip_virtual_keyboard_update, TextInputClient::FocusReason reason)
{
    focused_ = true;
...
{
    focused_ = false;
    MaybeUpdateActivated(skip_virtual_keyboard_update, TextInputClient::FOCUS_REASON_NONE);
}
...
void WaylandInputMethodContext::SetCursorLocation(const gfx::Rect& rect)
{
    ...
    WaylandWindow* focused_window = connection_->window_manager()->GetCurrentKeyboardFocusedWindow();
    if (!focused_window) {
        return;
    }
    text_input_->SetCursorRect(rect - focused_window->GetBoundsInDIP().OffsetFromOrigin());
}

void WaylandInputMethodContext::SetSurroundingText(
    const std::u16string& text, const gfx::Range& text_range, const gfx::Range& composition_range, const gfx::Range& selection_range)
{
    DVLOG(1) << __func__ << " text=" << text << " text_range=" << text_range << " composition_range=" << composition_range
    ...
    if (!selection_range.IsBoundedBy(text_range)) {
        ...
        LOG(ERROR) << "selection_range is not bounded by text_range: " << selection_range.ToString() << ", " << text_range.ToString();
    ...
    std::vector<size_t> offsets_for_adjustment = { selection_range.start() - utf16_offset, selection_range.end() - utf16_offset };
    std::string text_utf8 = base::UTF16ToUTF8AndAdjustOffsets(text, &offsets_for_adjustment);
    if (offsets_for_adjustment[0] == std::u16string::npos || offsets_for_adjustment[1] == std::u16string::npos) {
        LOG(DFATAL) << "The selection range is invalid.";
        return;
    }
    gfx::Range selection_range_utf8 = { static_cast<uint32_t>(offsets_for_adjustment[0]), static_cast<uint32_t>(offsets_for_adjustment[1]) };

    // To ensure trimming around cursor position, selection is used and not
    // preedit.
    auto trimmed = text_input_->HasAdvancedSurroundingTextSupport() ? TrimSurroundingTextForExtension(text_utf8, offsets_for_adjustment)
    ...
    const size_t extra_offset_utf16 = base::UTF8ToUTF16(std::string_view(text_utf8).substr(0, trimmed->offset)).length();
    ...
    if (composition_range.IsValid()) {
        if (!composition_range.IsBoundedBy(text_range)) {
            ...
            // ignore this request.
            LOG(ERROR) << "composition_range is not bounded by text_range: " << composition_range.ToString() << ", " << text_range.ToString();
            return;
        }
        std::vector<size_t> preedit_range = { composition_range.start() - utf16_offset, composition_range.end() - utf16_offset };
        base::UTF16ToUTF8AndAdjustOffsets(text, &preedit_range);
        if (preedit_range[0] < surrounding_text_offset_ || preedit_range[1] < surrounding_text_offset_
            || preedit_range[0] > (surrounding_text_offset_ + text_utf8.size()) || preedit_range[1] > (surrounding_text_offset_ + text_utf8.size())) {
            ...
            relocated_preedit_range = { preedit_range[0] - surrounding_text_offset_, preedit_range[1] - surrounding_text_offset_ };
        }
    } else {
        relocated_preedit_range = gfx::Range::InvalidRange();
    }

    gfx::Range relocated_selection_range(selection_range_utf8.start() - surrounding_text_offset_, selection_range_utf8.end() - surrounding_text_offset_);
    text_input_->SetSurroundingText(text_utf8, relocated_preedit_range, relocated_selection_range);
}

VirtualKeyboardController* WaylandInputMethodContext::GetVirtualKeyboardController()
{
    if (!text_input_)
        return nullptr;
    return this;
...
void WaylandInputMethodContext::AddObserver(VirtualKeyboardControllerObserver* observer)
{
    NOTIMPLEMENTED_LOG_ONCE();
}

void WaylandInputMethodContext::RemoveObserver(VirtualKeyboardControllerObserver* observer)
{
    NOTIMPLEMENTED_LOG_ONCE();
}
...
void WaylandInputMethodContext::OnPreeditString(std::string_view text, const std::vector<SpanStyle>& spans, const gfx::Range& preedit_cursor)
{
    ...
    for (const auto& span : spans) {
        ...
        if (!style.has_value())
            continue;
        composition_text.ime_text_spans.emplace_back(style->type, *start_offset, *end_offset, style->thickness);
    }
    if (!preedit_cursor.IsValid()) {
        ...
        std::vector<size_t> offsets = { static_cast<uint32_t>(preedit_cursor.start()), static_cast<uint32_t>(preedit_cursor.end()) };
        ...
        if (offsets[0] == std::u16string::npos || offsets[1] == std::u16string::npos) {
            DVLOG(1) << "got invalid cursor position (byte offset)=" << preedit_cursor.start() << "-" << preedit_cursor.end();
...
void WaylandInputMethodContext::OnCommitString(std::string_view text)
{
    ...
    surrounding_text_tracker_.OnInsertText(text_utf16, TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    ime_delegate_->OnCommit(text_utf16);
}

void WaylandInputMethodContext::OnCursorPosition(int32_t index, int32_t anchor)
{
    const auto& [surrounding_text, utf16_offset, selection, unused_composition_text] = surrounding_text_tracker_.predicted_state();
    ...
    std::vector<size_t> offsets = { index + surrounding_text_offset_, anchor + surrounding_text_offset_ };
    base::UTF8ToUTF16AndAdjustOffsets(base::UTF16ToUTF8(surrounding_text), &offsets);
    if (offsets[0] == std::u16string::npos || offsets[0] > surrounding_text.size()) {
        LOG(ERROR) << "Invalid index is specified.";
        return;
    }
    if (offsets[1] == std::u16string::npos || offsets[1] > surrounding_text.size()) {
        LOG(ERROR) << "Invalid anchor is specified.";
        return;
    }

    const gfx::Range new_selection_range = gfx::Range(offsets[1] + utf16_offset, offsets[0] + utf16_offset);
#if BUILDFLAG(IS_CHROMEOS_LACROS)
...
void WaylandInputMethodContext::OnDeleteSurroundingText(int32_t index, uint32_t length)
{
    const auto& [surrounding_text, utf16_offset, selection, unsused_composition] = surrounding_text_tracker_.predicted_state();
    ...
    base::UTF8ToUTF16AndAdjustOffsets(base::UTF16ToUTF8(surrounding_text), &offsets_for_adjustment);
    if (base::Contains(offsets_for_adjustment, std::u16string::npos)) {
        LOG(DFATAL) << "The selection range for surrounding text is invalid.";
        return;
    }

    if (selection.GetMin() < offsets_for_adjustment[0] + utf16_offset || selection.GetMax() > offsets_for_adjustment[1] + utf16_offset) {
...
void WaylandInputMethodContext::OnKeysym(uint32_t keysym, uint32_t state, uint32_t modifiers_bits, uint32_t time)
{
#if BUILDFLAG(USE_XKBCOMMON)
    ...
    if (!connection_->text_input_extension_v1() || base::Contains(modifiers_map_, XKB_MOD_NAME_CAPS)) {
    ...
    DomCode dom_code = static_cast<XkbKeyboardLayoutEngine*>(layout_engine)->GetDomCodeByKeysym(keysym, modifiers);
    if (dom_code == DomCode::NONE)
        return;

    // Keyboard might not exist.
    int device_id = connection_->seat()->keyboard() ? connection_->seat()->keyboard()->device_id() : 0;

    EventType type = state == WL_KEYBOARD_KEY_STATE_PRESSED ? EventType::kKeyPressed : EventType::kKeyReleased;
    key_delegate_->OnKeyboardKeyEvent(
        type, dom_code, /*repeat=*/false, std::nullopt, wl::EventMillisecondsToTimeTicks(time), device_id, WaylandKeyboard::KeyEventKind::kKey);
#else
    NOTIMPLEMENTED();
#endif
}

void WaylandInputMethodContext::OnSetPreeditRegion(int32_t index, uint32_t length, const std::vector<SpanStyle>& spans)
{
    const auto& [surrounding_text, utf16_offset, selection, unused_composition_text] = surrounding_text_tracker_.predicted_state();

    std::vector<size_t> selection_utf8_offsets = { selection.start() - utf16_offset, selection.end() - utf16_offset };
    std::string surrounding_text_utf8 = base::UTF16ToUTF8AndAdjustOffsets(surrounding_text, &selection_utf8_offsets);
    ...
    size_t begin_utf8 = static_cast<size_t>(static_cast<ssize_t>(selection_utf8_offsets[1]) + index);
    ...
    for (size_t i = 0; i < spans.size(); ++i) {
        ...
        if (begin_span == std::u16string::npos || end_span == std::u16string::npos)
            continue;
        if (begin_span < offsets[0] || end_span < offsets[0] || begin_span > offsets[1] || end_span > offsets[1]) {
        ...
        if (!style.has_value())
            continue;
        ime_text_spans.emplace_back(style->type, begin_span - offsets[0], end_span - offsets[0], style->thickness);
    }

    surrounding_text_tracker_.OnSetCompositionFromExistingText(gfx::Range(offsets[0] + utf16_offset, offsets[1] + utf16_offset));
    ime_delegate_->OnSetPreeditRegion(gfx::Range(offsets[0] + utf16_offset, offsets[1] + utf16_offset), ime_text_spans);
}

void WaylandInputMethodContext::OnClearGrammarFragments(const gfx::Range& range)
{
...
void WaylandInputMethodContext::OnAddGrammarFragment(const GrammarFragment& fragment)
{
    // TODO(crbug.com/374244479): remove these when cleaning Wayland code from
    // Lacros. Grammar and autocorrect are features of CrOS.
    NOTREACHED();
...
void WaylandInputMethodContext::OnSetVirtualKeyboardOccludedBounds(const gfx::Rect& screen_bounds)
{
...
void WaylandInputMethodContext::OnModifiersMap(std::vector<std::string> modifiers_map)
{
...
void WaylandInputMethodContext::MaybeUpdateActivated(bool skip_virtual_keyboard_update, TextInputClient::FocusReason reason)
{
    if (!text_input_)
        return;

    WaylandWindow* window = connection_->window_manager()->GetCurrentKeyboardFocusedWindow();
    ...
    if (activated) {
        ...
        text_input_->SetContentType(
            attributes_.input_type, attributes_.input_mode, attributes_.flags, attributes_.should_do_learning, attributes_.can_compose_inline);
