...
TEST_F(XdgActivationTest, RequestNewToken)
{
    ...
    PostToServerAndWait([surface_id2](wl::TestWaylandServerThread* server) {
        ...
        auto* const surface2 = server->GetObject<wl::MockSurface>(surface_id2)->resource();

        wl::ScopedWlArray empty({});
        wl_keyboard_send_enter(keyboard, server->GetNextSerial(), surface2, empty.get());
    ...
    {
        ::testing::StrictMock<base::MockCallback<base::nix::XdgActivationTokenCallback>> callback;
        ...
        PostToServerAndWait(
            [](wl::TestWaylandServerThread* server) {
                ...
                xdg_activation_token_v1_send_done(xdg_activation->get_token()->resource(), kMockStaticTestToken);
            },
            true);
    }
    ...
    // Expect a successful token request from a non-sequenced task.
    {
        ::testing::StrictMock<base::MockCallback<base::nix::XdgActivationTokenCallback>> callback;
        EXPECT_CALL(callback, Run(std::string(kMockStaticTestToken)));
        base::ThreadPool::PostTask(FROM_HERE, { base::MayBlock(), base::TaskPriority::USER_BLOCKING, base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN },
            base::BindOnce(&XdgActivation::RequestNewToken, base::Unretained(connection_->xdg_activation()), callback.Get()));
...
TEST_F(XdgActivationTest, RequestNewToken_TooManyRequests)
{
    ...
    for (int i = 0; i < 100; ++i) {
        connection_->xdg_activation()->RequestNewToken(base::BindOnce([](std::string _) {}));
    }
