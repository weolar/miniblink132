...
// static
std::unique_ptr<WaylandWindow> WaylandWindow::Create(PlatformWindowDelegate* delegate, WaylandConnection* connection, PlatformWindowInitProperties properties)
{
    ...
    case PlatformWindowType::kBubble:
        ...
        if (auto* parent = connection->window_manager()->GetWindow(properties.parent_widget)) {
    ...
    return window && window->Initialize(std::move(properties)) ? std::move(window) : nullptr;
}
