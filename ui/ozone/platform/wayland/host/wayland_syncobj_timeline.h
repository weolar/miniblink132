...
public:
    ...
    wp_linux_drm_syncobj_timeline_v1* timeline() const
    {
        return timeline_.get();
    }
    uint64_t sync_point() const
    {
        return sync_point_;
    }
...
protected:
    ...
    WaylandSyncobjTimeline(DrmSyncobjIoctlWrapper* drm, std::unique_ptr<DrmSyncobj> syncobj, wp_linux_drm_syncobj_timeline_v1* timeline);

    static std::pair<std::unique_ptr<DrmSyncobj>, wp_linux_drm_syncobj_timeline_v1*> CreateHelper(const WaylandConnection* connection);
...
public:
    ...
    WaylandSyncobjAcquireTimeline& operator=(const WaylandSyncobjAcquireTimeline&) = delete;

    ~WaylandSyncobjAcquireTimeline() override;

    static std::unique_ptr<WaylandSyncobjAcquireTimeline> Create(const WaylandConnection* connection);
...
private:
    WaylandSyncobjAcquireTimeline(DrmSyncobjIoctlWrapper* drm, std::unique_ptr<DrmSyncobj> syncobj, wp_linux_drm_syncobj_timeline_v1* timeline);
};

class WaylandSyncobjReleaseTimeline : public WaylandSyncobjTimeline, public base::MessagePumpForUI::FdWatcher {
public:
    ...
    WaylandSyncobjReleaseTimeline& operator=(const WaylandSyncobjReleaseTimeline&) = delete;

    ~WaylandSyncobjReleaseTimeline() override;

    void IncrementSyncPoint() override;

    static std::unique_ptr<WaylandSyncobjReleaseTimeline> Create(const WaylandConnection* connection);
...
private:
    WaylandSyncobjReleaseTimeline(DrmSyncobjIoctlWrapper* drm, std::unique_ptr<DrmSyncobj> syncobj, wp_linux_drm_syncobj_timeline_v1* timeline);
    base::ScopedFD ExportCurrentSyncPointToSyncFd();
    FRIEND_TEST_ALL_PREFIXES(WaylandSyncobjTimelineTest, ExportCurrentSyncPointToSyncFd);
    FRIEND_TEST_ALL_PREFIXES(WaylandSyncobjTimelineTest, ExportCurrentSyncPointToSyncFd_FailOnSyncobjTransfer);
    FRIEND_TEST_ALL_PREFIXES(WaylandSyncobjTimelineTest, ExportCurrentSyncPointToSyncFd_FailOnSyncobjExportSyncFile);
