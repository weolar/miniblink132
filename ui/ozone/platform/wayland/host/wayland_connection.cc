...
int64_t ConvertTimespecResultToMicros(uint32_t tv_sec_hi, uint32_t tv_sec_lo, uint32_t tv_nsec)
{
...
bool WaylandConnection::Initialize(bool use_threaded_polling)
{
    ...
    RegisterGlobalObjectFactory(FractionalScaleManager::kInterfaceName, &FractionalScaleManager::Instantiate);
    RegisterGlobalObjectFactory(GtkPrimarySelectionDeviceManager::kInterfaceName, &GtkPrimarySelectionDeviceManager::Instantiate);
    RegisterGlobalObjectFactory(GtkShell1::kInterfaceName, &GtkShell1::Instantiate);
    RegisterGlobalObjectFactory(OrgKdeKwinIdle::kInterfaceName, &OrgKdeKwinIdle::Instantiate);
    RegisterGlobalObjectFactory(OverlayPrioritizer::kInterfaceName, &OverlayPrioritizer::Instantiate);
    RegisterGlobalObjectFactory(SinglePixelBuffer::kInterfaceName, &SinglePixelBuffer::Instantiate);
    RegisterGlobalObjectFactory(SurfaceAugmenter::kInterfaceName, &SurfaceAugmenter::Instantiate);
    RegisterGlobalObjectFactory(ToplevelIconManager::kInterfaceName, &ToplevelIconManager::Instantiate);
    RegisterGlobalObjectFactory(WaylandZAuraOutputManagerV2::kInterfaceName, &WaylandZAuraOutputManagerV2::Instantiate);
    RegisterGlobalObjectFactory(WaylandDataDeviceManager::kInterfaceName, &WaylandDataDeviceManager::Instantiate);
    RegisterGlobalObjectFactory(WaylandDrm::kInterfaceName, &WaylandDrm::Instantiate);
    RegisterGlobalObjectFactory(WaylandOutput::kInterfaceName, &WaylandOutput::Instantiate);
    RegisterGlobalObjectFactory(WaylandSeat::kInterfaceName, &WaylandSeat::Instantiate);
    RegisterGlobalObjectFactory(WaylandShm::kInterfaceName, &WaylandShm::Instantiate);
    RegisterGlobalObjectFactory(WaylandZAuraShell::kInterfaceName, &WaylandZAuraShell::Instantiate);
    RegisterGlobalObjectFactory(WaylandCursorShape::kInterfaceName, &WaylandCursorShape::Instantiate);
    RegisterGlobalObjectFactory(WaylandZcrCursorShapes::kInterfaceName, &WaylandZcrCursorShapes::Instantiate);
    RegisterGlobalObjectFactory(WaylandZcrTouchpadHaptics::kInterfaceName, &WaylandZcrTouchpadHaptics::Instantiate);
    RegisterGlobalObjectFactory(WaylandZwpLinuxDmabuf::kInterfaceName, &WaylandZwpLinuxDmabuf::Instantiate);
    RegisterGlobalObjectFactory(WaylandZwpPointerConstraints::kInterfaceName, &WaylandZwpPointerConstraints::Instantiate);
    RegisterGlobalObjectFactory(WaylandZwpPointerGestures::kInterfaceName, &WaylandZwpPointerGestures::Instantiate);
    RegisterGlobalObjectFactory(WaylandZwpRelativePointerManager::kInterfaceName, &WaylandZwpRelativePointerManager::Instantiate);
    RegisterGlobalObjectFactory(XdgActivation::kInterfaceName, &XdgActivation::Instantiate);
    RegisterGlobalObjectFactory(XdgForeignWrapper::kInterfaceNameV1, &XdgForeignWrapper::Instantiate);
    RegisterGlobalObjectFactory(XdgForeignWrapper::kInterfaceNameV2, &XdgForeignWrapper::Instantiate);
    RegisterGlobalObjectFactory(ZwpIdleInhibitManager::kInterfaceName, &ZwpIdleInhibitManager::Instantiate);
    RegisterGlobalObjectFactory(ZwpPrimarySelectionDeviceManager::kInterfaceName, &ZwpPrimarySelectionDeviceManager::Instantiate);
    ...
    wrapped_display_.reset(reinterpret_cast<wl_proxy*>(wl_proxy_create_wrapper(display())));
    ...
    event_source_ = std::make_unique<WaylandEventSource>(display(), event_queue_.get(), window_manager(), this, use_threaded_polling);
...
{
    return zaura_shell() ? zaura_shell()->server_version().value_or(base::Version {}) : base::Version {};
}

void WaylandConnection::SetPlatformCursor(wl_cursor* cursor_data, int buffer_scale)
{
    if (!cursor_)
        return;
...
void WaylandConnection::SetCursorBufferListener(WaylandCursorBufferListener* listener)
{
    listener_ = listener;
    if (!cursor_)
        return;
...
void WaylandConnection::SetCursorBitmap(const std::vector<SkBitmap>& bitmaps, const gfx::Point& hotspot_in_dips, int buffer_scale)
{
    if (!cursor_)
        return;
...
bool WaylandConnection::IsDragInProgress() const
{
    ...
    return seat_ && data_device_manager_ && data_device_manager_->GetDevice()->IsDragInProgress();
}
...
{
    DCHECK(compositor_);
    return wl::Object<wl_surface>(wl_compositor_create_surface(compositor_.get()));
}

void WaylandConnection::RegisterGlobalObjectFactory(const char* interface_name, wl::GlobalObjectFactory factory)
{
...
bool WaylandConnection::WlObjectsReady() const
{
    ...
    if (zaura_shell_ && zaura_shell_get_version(zaura_shell_->wl_object()) >= ZAURA_SHELL_COMPOSITOR_VERSION_SINCE_VERSION) {
...
std::vector<InputDevice> WaylandConnection::CreateMouseDevices() const
{
    ...
    if (const auto* pointer = seat_->pointer()) {
        devices.emplace_back(pointer->id(), InputDeviceType::INPUT_DEVICE_UNKNOWN, "pointer");
    }
    return devices;
}
...
std::vector<KeyboardDevice> WaylandConnection::CreateKeyboardDevices() const
{
    ...
    if (const auto* keyboard = seat_->keyboard()) {
        devices.emplace_back(keyboard->id(), InputDeviceType::INPUT_DEVICE_UNKNOWN, "keyboard");
    }
    return devices;
}

std::vector<TouchscreenDevice> WaylandConnection::CreateTouchscreenDevices() const
{
    ...
    if (const auto* touch = seat_->touch()) {
        ...
        devices.emplace_back(touch->id(), InputDeviceType::INPUT_DEVICE_UNKNOWN, "touch", gfx::Size(),
            /*touch_points=*/10);
    }
    return devices;
}
...
void WaylandConnection::CreateDataObjectsIfReady()
{
    if (data_device_manager_ && seat_) {
        ...
        data_drag_controller_ = std::make_unique<WaylandDataDragController>(this, data_device_manager_.get(), event_source(), event_source());

        DCHECK(!window_drag_controller_);
        window_drag_controller_
            = std::make_unique<WaylandWindowDragController>(this, data_device_manager_.get(), event_source(), event_source(), event_source());

        DCHECK(!clipboard_);
        clipboard_ = std::make_unique<WaylandClipboard>(this, data_device_manager_.get());
    }
}

base::TimeTicks WaylandConnection::ConvertPresentationTime(uint32_t tv_sec_hi, uint32_t tv_sec_lo, uint32_t tv_nsec)
{
    DCHECK(presentation());
    ...
    if (presentation_clk_id_ == CLOCK_MONOTONIC) {
        return base::TimeTicks() + base::Microseconds(ConvertTimespecResultToMicros(tv_sec_hi, tv_sec_lo, tv_nsec));
    }
    ...
    if (ret < 0) {
        ...
        PLOG(ERROR) << "Error: failure to read the wp_presentation clock " << presentation_clk_id_;
        return base::TimeTicks::Now();
    }

    int64_t delta_us = ConvertTimespecResultToMicros(tv_sec_hi, tv_sec_lo, tv_nsec) - ConvertTimespecToMicros(presentation_now);
...
bool WaylandConnection::ShouldUseOverlayDelegation() const
{
    ...
    bool should_use_overlay_delegation = IsWaylandOverlayDelegationEnabled() && !fractional_scale_manager_v1();
...
// static
void WaylandConnection::OnGlobal(void* data, wl_registry* registry, uint32_t name, const char* interface, uint32_t version)
{
...
// static
void WaylandConnection::OnGlobalRemove(void* data, wl_registry* registry, uint32_t name)
{
    auto* self = static_cast<WaylandConnection*>(data);
...
// static
void WaylandConnection::OnPing(void* data, xdg_wm_base* shell, uint32_t serial)
{
...
// static
void WaylandConnection::OnClockId(void* data, wp_presentation* presentation, uint32_t clk_id)
{
    DCHECK_EQ(base::TimeTicks::GetClock(), base::TimeTicks::Clock::LINUX_CLOCK_MONOTONIC);
    auto* connection = static_cast<WaylandConnection*>(data);
    connection->presentation_clk_id_ = clk_id;
}

void WaylandConnection::HandleGlobal(wl_registry* registry, uint32_t name, const char* interface, uint32_t version)
{
    ...
    if (factory_it != global_object_factories_.end()) {
        ...
        compositor_ = wl::Bind<wl_compositor>(registry, name, std::min(version, kMaxCompositorVersion));
        ...
        shell_ = wl::Bind<xdg_wm_base>(registry, name, std::min(version, kMaxXdgShellVersion));
        if (!shell_) {
    ...
    } else if (!alpha_compositing_ && (strcmp(interface, "zcr_alpha_compositing_v1") == 0)) {
        alpha_compositing_ = wl::Bind<zcr_alpha_compositing_v1>(registry, name, std::min(version, kMaxAlphaCompositingVersion));
        if (!alpha_compositing_) {
            LOG(ERROR) << "Failed to bind zcr_alpha_compositing_v1";
            return;
        }
    } else if (!linux_explicit_synchronization_ && (strcmp(interface, "zwp_linux_explicit_synchronization_v1") == 0)) {
        linux_explicit_synchronization_ = wl::Bind<zwp_linux_explicit_synchronization_v1>(registry, name, std::min(version, kMaxExplicitSyncVersion));
        if (!linux_explicit_synchronization_) {
            LOG(ERROR) << "Failed to bind zwp_linux_explicit_synchronization_v1";
            return;
        }
    } else if (!linux_drm_syncobj_manager_ && (strcmp(interface, "wp_linux_drm_syncobj_manager_v1") == 0)) {
        if (enable_linux_drm_syncobj_for_testing_
            || (base::FeatureList::IsEnabled(features::kWaylandLinuxDrmSyncobj) && MinSupportedKernelForLinuxDrmSyncobj())) {
            linux_drm_syncobj_manager_ = wl::Bind<wp_linux_drm_syncobj_manager_v1>(registry, name, std::min(version, kMaxLinuxDrmSyncobjVersion));
            if (!linux_drm_syncobj_manager_) {
                LOG(ERROR) << "Failed to bind wp_linux_drm_syncobj_manager_v1";
                return;
            }
        }
    } else if (!content_type_manager_v1_ && (strcmp(interface, "wp_content_type_manager_v1") == 0)) {
        content_type_manager_v1_ = wl::Bind<wp_content_type_manager_v1>(registry, name, std::min(version, kMaxWpContentTypeVersion));
        ...
        presentation_ = wl::Bind<wp_presentation>(registry, name, std::min(version, kMaxWpPresentationVersion));
        if (!presentation_) {
        ...
        wp_presentation_add_listener(presentation_.get(), &kPresentationListener, this);
    } else if (!viewporter_ && (strcmp(interface, "wp_viewporter") == 0)) {
        viewporter_ = wl::Bind<wp_viewporter>(registry, name, std::min(version, kMaxWpViewporterVersion));
        if (!viewporter_) {
            LOG(ERROR) << "Failed to bind wp_viewporter";
            return;
        }
    } else if (!keyboard_extension_v1_ && strcmp(interface, "zcr_keyboard_extension_v1") == 0) {
        keyboard_extension_v1_ = wl::Bind<zcr_keyboard_extension_v1>(registry, name, std::min(version, kMaxKeyboardExtensionVersion));
    ...
    } else if (!keyboard_shortcuts_inhibit_manager_v1_ && strcmp(interface, "zwp_keyboard_shortcuts_inhibit_manager_v1") == 0) {
        keyboard_shortcuts_inhibit_manager_v1_
            = wl::Bind<zwp_keyboard_shortcuts_inhibit_manager_v1>(registry, name, std::min(version, kMaxKeyboardShortcutsInhibitManagerVersion));
        if (!keyboard_shortcuts_inhibit_manager_v1_) {
            LOG(ERROR) << "Failed to bind zwp_keyboard_shortcuts_inhibit_manager_v1";
            return;
        }
    } else if (!text_input_manager_v1_ && strcmp(interface, "zwp_text_input_manager_v1") == 0) {
        text_input_manager_v1_ = wl::Bind<zwp_text_input_manager_v1>(registry, name, std::min(version, kMaxTextInputManagerV1Version));
        if (!text_input_manager_v1_) {
            LOG(ERROR) << "Failed to bind to zwp_text_input_manager_v1 global";
            return;
        }
    } else if (!text_input_extension_v1_ && strcmp(interface, "zcr_text_input_extension_v1") == 0) {
        text_input_extension_v1_ = wl::Bind<zcr_text_input_extension_v1>(registry, name, std::min(version, kMaxTextInputExtensionVersion));
    } else if (!text_input_manager_v3_ && strcmp(interface, "zwp_text_input_manager_v3") == 0) {
        text_input_manager_v3_ = wl::Bind<zwp_text_input_manager_v3>(registry, name, std::min(version, kMaxTextInputManagerV3Version));
        if (!text_input_manager_v3_) {
            LOG(ERROR) << "Failed to bind to zwp_text_input_manager_v3 global";
            return;
        }
    } else if (!xdg_decoration_manager_ && strcmp(interface, "zxdg_decoration_manager_v1") == 0) {
        xdg_decoration_manager_ = wl::Bind<zxdg_decoration_manager_v1>(registry, name, std::min(version, kMaxXdgDecorationVersion));
        if (!xdg_decoration_manager_) {
            LOG(ERROR) << "Failed to bind zxdg_decoration_manager_v1";
            return;
        }
    } else if (!extended_drag_v1_ && strcmp(interface, "zcr_extended_drag_v1") == 0) {
        extended_drag_v1_ = wl::Bind<zcr_extended_drag_v1>(registry, name, std::min(version, kMaxExtendedDragVersion));
        if (!extended_drag_v1_) {
            LOG(ERROR) << "Failed to bind to zcr_extended_drag_v1 global";
            return;
        }
    } else if (!xdg_toplevel_drag_manager_v1_ && strcmp(interface, "xdg_toplevel_drag_manager_v1") == 0 && IsWaylandXdgToplevelDragEnabled()) {
        xdg_toplevel_drag_manager_v1_ = wl::Bind<::xdg_toplevel_drag_manager_v1>(registry, name, std::min(version, kMaxXdgToplevelDragVersion));
        if (!xdg_toplevel_drag_manager_v1_) {
            LOG(ERROR) << "Failed to bind to xdg_toplevel_drag_manager_v1 global";
            return;
        }
    } else if (!xdg_output_manager_ && strcmp(interface, "zxdg_output_manager_v1") == 0) {
        ...
        xdg_output_manager_ = wl::Bind<zxdg_output_manager_v1>(registry, name, std::min(version, kMaxXdgOutputManagerVersion));
        ...
        zcr_stylus_v2_ = wl::Bind<zcr_stylus_v2>(registry, name, std::min(version, kMaxStylusVersion));
        if (!zcr_stylus_v2_) {
...
{
    return gl::EGLDisplayPlatform(reinterpret_cast<EGLNativeDisplayType>(display()));
}
