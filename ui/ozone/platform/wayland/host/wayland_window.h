...
public:
    ...
    static std::unique_ptr<WaylandWindow> Create(PlatformWindowDelegate* delegate, WaylandConnection* connection, PlatformWindowInitProperties properties);
    ...
    WaylandSurface* root_surface() const
    {
        return root_surface_.get();
    }
    ...
    const PlatformWindowDelegate* delegate() const
    {
        return delegate_;
    }
    ...
    bool CommitOverlays(uint32_t frame_id, const gfx::FrameData& data, std::vector<wl::WaylandOverlayConfig>& overlays);
    ...
    void set_touch_focus(bool focus)
    {
        has_touch_focus_ = focus;
    }
    bool has_touch_focus() const
    {
    ...
    void set_child_popup(WaylandPopup* window)
    {
        child_popup_ = window;
    }
    WaylandPopup* child_popup() const
    {
        return child_popup_;
    }
    ...
    {
        return received_configure_event_;
    }
    ...
    // WmDragHandler
    bool StartDrag(const ui::OSExchangeData& data, int operations, mojom::DragEventSource source, gfx::NativeCursor cursor, bool can_grab_pointer,
        base::OnceClosure drag_started_callback, WmDragHandler::DragFinishedCallback drag_finished_callback,
    ...
    void OcclusionStateChanged(PlatformWindowOcclusionState occlusion_state) override;
    ...
    // Configure related:
    virtual void HandleToplevelConfigure(int32_t width, int32_t height, const WindowStates& window_states);
    virtual void HandleToplevelConfigureWithOrigin(int32_t x, int32_t y, int32_t width, int32_t height, const WindowStates& window_states);
...
protected:
    WaylandWindow(PlatformWindowDelegate* delegate, WaylandConnection* connection);

    WaylandConnection* connection()
    {
        return connection_;
    }
    const WaylandConnection* connection() const
    {
        return connection_;
    }
    ...
    const gfx::Rect& restored_bounds_dip() const
    {
    ...
    void RequestStateFromServer(PlatformWindowDelegate::State state, int64_t serial);
    ...
    void RequestState(PlatformWindowDelegate::State state, int64_t serial, bool force);
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(WaylandBufferManagerTest, CommitOverlaysNonsensicalBoundsRect);
    FRIEND_TEST_ALL_PREFIXES(WaylandWindowTest, ServerInitiatedRestoreFromMinimizedState);
    ...
    virtual bool OnInitialize(PlatformWindowInitProperties properties, PlatformWindowDelegate::State* state) = 0;
...
#if BUILDFLAG(IS_LINUX)
    void OnCursorLoaded(scoped_refptr<WaylandAsyncCursor> cursor, scoped_refptr<BitmapCursor> bitmap_cursor);
#endif
    ...
    std::list<raw_ptr<WaylandSubsurface, CtnExperimental>> subsurface_stack_above_;
    std::list<raw_ptr<WaylandSubsurface, CtnExperimental>> subsurface_stack_below_;
    ...
    wl::Object<zwp_keyboard_shortcuts_inhibitor_v1> permanent_keyboard_shortcuts_inhibitor_;
#endif
    ...
    std::vector<std::tuple<PlatformWindowDelegate::State, int64_t, bool>> reentrant_requests_;
