...
public:
    using CreatorResultCallback = base::OnceCallback<void(scoped_refptr<WaylandZcrColorSpace>, std::optional<uint32_t>)>;
    WaylandZcrColorSpaceCreator(wl::Object<zcr_color_space_creator_v1> creator, CreatorResultCallback on_creation);
    WaylandZcrColorSpaceCreator(const WaylandZcrColorSpaceCreator&) = delete;
    WaylandZcrColorSpaceCreator& operator=(const WaylandZcrColorSpaceCreator&) = delete;
    ~WaylandZcrColorSpaceCreator();

private:
    // zcr_color_space_creator_v1_listener callbacks:
    static void OnCreated(void* data, zcr_color_space_creator_v1* csc, zcr_color_space_v1* color_space);
    static void OnError(void* data, zcr_color_space_creator_v1* csc, uint32_t error);
