...
std::unique_ptr<XdgForeignWrapper> CreateWrapper(WaylandConnection* connection, wl_registry* registry, uint32_t name, uint32_t version)
{
    ...
    return std::make_unique<XdgForeignWrapper>(connection, std::move(zxdg_exporter));
}

} // namespace

template <typename ExportedType> struct ExportedSurface {
...
public:
    ...
    virtual void ExportSurfaceToForeign(wl_surface* surface, OnHandleExported cb) = 0;

    // WaylandWindowObserver:
    virtual void OnWindowRemoved(WaylandWindow* window) = 0;
};

template <typename ExporterType, typename ExportedType> class XdgForeignWrapperImpl : public XdgForeignWrapper::XdgForeignWrapperInternal {
public:
    XdgForeignWrapperImpl(WaylandConnection* connection, wl::Object<ExporterType> exporter)
    ...
    void ExportSurfaceToForeign(wl_surface* surface, OnHandleExported cb) override
    {
    ...
    {
        auto it = base::ranges::find(exported_surfaces_, window->root_surface()->surface(), &ExportedSurface<ExportedType>::surface_for_export);
...
private:
    ...
    {
        auto* self = static_cast<XdgForeignWrapperImpl<ExporterType, ExportedType>*>(data);
        DCHECK(self);

        auto exported_surface_it = base::ranges::find(self->exported_surfaces_, exported, [](const auto& item) { return item.exported.get(); });
        CHECK(exported_surface_it != self->exported_surfaces_.end(), base::NotFatalUntil::M130);
...
template <> void XdgForeignWrapperImpl<zxdg_exporter_v1, zxdg_exported_v1>::ExportSurfaceInternal(wl_surface* surface, OnHandleExported cb)
{
    ExportedSurface<zxdg_exported_v1> exported_surface(surface, std::move(cb));
    exported_surface.exported.reset(zxdg_exporter_v1_export(exporter_.get(), surface));

    static constexpr zxdg_exported_v1_listener kXdgExportedListener = { .handle = &OnHandle };
    zxdg_exported_v1_add_listener(exported_surface.exported.get(), &kXdgExportedListener, this);

    exported_surfaces_.emplace_back(std::move(exported_surface));
    connection_->Flush();
}

template <> void XdgForeignWrapperImpl<zxdg_exporter_v2, zxdg_exported_v2>::ExportSurfaceInternal(wl_surface* surface, OnHandleExported cb)
{
    ExportedSurface<zxdg_exported_v2> exported_surface(surface, std::move(cb));
    exported_surface.exported.reset(zxdg_exporter_v2_export_toplevel(exporter_.get(), surface));

    static constexpr zxdg_exported_v2_listener kXdgExportedListener = { .handle = &OnHandle };
    zxdg_exported_v2_add_listener(exported_surface.exported.get(), &kXdgExportedListener, this);
...
// static
void XdgForeignWrapper::Instantiate(WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    if (connection->xdg_foreign_ || !wl::CanBind(interface, version, kMinVersion, kMinVersion)) {
        return;
    }

    if (interface == kInterfaceNameV1) {
        connection->xdg_foreign_ = CreateWrapper<zxdg_exporter_v1>(connection, registry, name, kMinVersion);
    } else if (interface == kInterfaceNameV2) {
        connection->xdg_foreign_ = CreateWrapper<zxdg_exporter_v2>(connection, registry, name, kMinVersion);
    } else {
        NOTREACHED() << " unexpected interface name: " << interface;
    }
}

XdgForeignWrapper::XdgForeignWrapper(WaylandConnection* connection, wl::Object<zxdg_exporter_v1> exporter_v1)
{
    impl_ = std::make_unique<XdgForeignWrapperImpl<zxdg_exporter_v1, zxdg_exported_v1>>(connection, std::move(exporter_v1));
}

XdgForeignWrapper::XdgForeignWrapper(WaylandConnection* connection, wl::Object<zxdg_exporter_v2> exporter_v2)
{
    impl_ = std::make_unique<XdgForeignWrapperImpl<zxdg_exporter_v2, zxdg_exported_v2>>(connection, std::move(exporter_v2));
}

XdgForeignWrapper::~XdgForeignWrapper() = default;

void XdgForeignWrapper::ExportSurfaceToForeign(WaylandWindow* window, OnHandleExported cb)
{
