...
KernelVersion KernelVersionNumbers()
{
    ...
    int num_read = sscanf(info.release, "%d.%d.%d", &ver.major, &ver.minor, &ver.bugfix);
    if (num_read < 1) {
        ver.major = 0;
    }
    if (num_read < 2) {
...
WaylandBufferManagerHost::WaylandBufferManagerHost(WaylandConnection* connection)
...
void WaylandBufferManagerHost::SetTerminateGpuCallback(base::OnceCallback<void(std::string)> terminate_callback)
{
    terminate_gpu_cb_ = std::move(terminate_callback);
}

mojo::PendingRemote<ozone::mojom::WaylandBufferManagerHost> WaylandBufferManagerHost::BindInterface()
{
    // Allow to rebind the interface if it hasn't been destroyed yet.
    if (receiver_.is_bound())
        OnChannelDestroyed();

    mojo::PendingRemote<ozone::mojom::WaylandBufferManagerHost> buffer_manager_host;
...
void WaylandBufferManagerHost::OnCommitOverlayError(const std::string& message)
{
...
wl::BufferFormatsWithModifiersMap WaylandBufferManagerHost::GetSupportedBufferFormats() const
{
...
{
    return connection_->SupportsExplicitSync() || connection_->UseImplicitSyncInterop();
}
...
void WaylandBufferManagerHost::SetWaylandBufferManagerGpu(mojo::PendingAssociatedRemote<ozone::mojom::WaylandBufferManagerGpu> buffer_manager_gpu_associated)
{
    buffer_manager_gpu_associated_.Bind(std::move(buffer_manager_gpu_associated));
}

void WaylandBufferManagerHost::CreateDmabufBasedBuffer(mojo::PlatformHandle dmabuf_fd, const gfx::Size& size, const std::vector<uint32_t>& strides,
    const std::vector<uint32_t>& offsets, const std::vector<uint64_t>& modifiers, uint32_t format, uint32_t planes_count, uint32_t buffer_id)
{
    DCHECK(base::CurrentUIThread::IsSet());
    DCHECK(error_message_.empty());

    TRACE_EVENT2("wayland", "WaylandBufferManagerHost::CreateDmabufBasedBuffer", "Format", format, "Buffer id", buffer_id);

    base::ScopedFD fd = dmabuf_fd.TakeFD();

    // Validate data and ask surface to create a buffer associated with the
    // |buffer_id|.
    if (!ValidateDataFromGpu(fd, size, strides, offsets, modifiers, format, planes_count, buffer_id)) {
        TerminateGpuProcess();
        return;
    }
    ...
    auto result = buffer_backings_.emplace(buffer_id,
        std::make_unique<WaylandBufferBackingDmabuf>(
            connection_, std::move(fd), size, std::move(strides), std::move(offsets), std::move(modifiers), format, planes_count, buffer_id));

    if (!result.second) {
        error_message_ = base::StrCat({ "A buffer with id= ", NumberToString(buffer_id), " already exists" });
        TerminateGpuProcess();
        return;
    }
...
void WaylandBufferManagerHost::CreateShmBasedBuffer(mojo::PlatformHandle shm_fd, uint64_t length, const gfx::Size& size, uint32_t buffer_id)
{
    DCHECK(base::CurrentUIThread::IsSet());
    DCHECK(error_message_.empty());

    TRACE_EVENT1("wayland", "WaylandBufferManagerHost::CreateShmBasedBuffer", "Buffer id", buffer_id);
    ...
    // Check if any of the surfaces has already had a buffer with the same id.
    auto result = buffer_backings_.emplace(buffer_id, std::make_unique<WaylandBufferBackingShm>(connection_, std::move(fd), length, size, buffer_id));
...
void WaylandBufferManagerHost::CreateSolidColorBuffer(const gfx::Size& size, const SkColor4f& color, uint32_t buffer_id)
{
    DCHECK(base::CurrentUIThread::IsSet());
    DCHECK(error_message_.empty());
    TRACE_EVENT1("wayland", "WaylandBufferManagerHost::CreateSolidColorBuffer", "Buffer id", buffer_id);
    ...
    auto result = buffer_backings_.emplace(buffer_id, std::make_unique<WaylandBufferBackingSolidColor>(connection_, color, size, buffer_id));
...
void WaylandBufferManagerHost::CreateSinglePixelBuffer(const SkColor4f& color, uint32_t buffer_id)
{
    DCHECK(base::CurrentUIThread::IsSet());
    DCHECK(error_message_.empty());
    TRACE_EVENT1("wayland", "WaylandBufferManagerHost::CreateSinglePixelBuffer", "Buffer id", buffer_id);
    ...
    auto result = buffer_backings_.emplace(buffer_id, std::make_unique<WaylandBufferBackingSinglePixel>(connection_, color, buffer_id));
...
WaylandBufferHandle* WaylandBufferManagerHost::EnsureBufferHandle(WaylandSurface* requestor, uint32_t buffer_id)
{
    DCHECK(base::CurrentUIThread::IsSet());
    DCHECK(error_message_.empty());
    DCHECK(requestor);
...
WaylandBufferHandle* WaylandBufferManagerHost::GetBufferHandle(WaylandSurface* requestor, uint32_t buffer_id)
{
    DCHECK(base::CurrentUIThread::IsSet());
    DCHECK(requestor);
...
uint32_t WaylandBufferManagerHost::GetBufferFormat(WaylandSurface* requestor, uint32_t buffer_id)
{
    DCHECK(base::CurrentUIThread::IsSet());
    DCHECK(requestor);
...
void WaylandBufferManagerHost::CommitOverlays(
    gfx::AcceleratedWidget widget, uint32_t frame_id, const gfx::FrameData& data, std::vector<wl::WaylandOverlayConfig> overlays)
{
    DCHECK(base::CurrentUIThread::IsSet());
...
void WaylandBufferManagerHost::DestroyBuffer(uint32_t buffer_id)
{
    ...
    TRACE_EVENT1("wayland", "WaylandBufferManagerHost::DestroyBuffer", "Buffer id", buffer_id);
...
bool WaylandBufferManagerHost::ValidateDataFromGpu(const base::ScopedFD& fd, const gfx::Size& size, const std::vector<uint32_t>& strides,
    const std::vector<uint32_t>& offsets, const std::vector<uint64_t>& modifiers, uint32_t format, uint32_t planes_count, uint32_t buffer_id)
{
    if (!ValidateDataFromGpu(size, buffer_id))
        return false;
    ...
    if (planes_count != strides.size() || planes_count != offsets.size() || planes_count != modifiers.size()) {
        reason = base::StrCat({ "Number of strides(", NumberToString(strides.size()), ")/offsets(", NumberToString(offsets.size()), ")/modifiers(",
            NumberToString(modifiers.size()), ") does not correspond to the number of planes(", NumberToString(planes_count), ")" });
    }
...
bool WaylandBufferManagerHost::ValidateDataFromGpu(const base::ScopedFD& fd, size_t length, const gfx::Size& size, uint32_t buffer_id)
{
    if (!ValidateDataFromGpu(size, buffer_id))
        return false;
...
bool WaylandBufferManagerHost::ValidateDataFromGpu(const gfx::Size& size, uint32_t buffer_id)
{
...
bool WaylandBufferManagerHost::ValidateBufferExistence(uint32_t buffer_id)
{
    ...
    auto it = buffer_backings_.find(buffer_id);
    if (it == buffer_backings_.end()) {
        error_message_ = base::StrCat({ "Buffer with ", NumberToString(buffer_id), " id does not exist" });
    }

    return error_message_.empty();
}

void WaylandBufferManagerHost::OnSubmission(gfx::AcceleratedWidget widget, uint32_t frame_id, const gfx::SwapResult& swap_result,
    gfx::GpuFenceHandle release_fence, const std::vector<wl::WaylandPresentationInfo>& presentation_infos)
{
    ...
    buffer_manager_gpu_associated_->OnSubmission(widget, frame_id, swap_result, std::move(release_fence), presentation_infos);
}

void WaylandBufferManagerHost::OnPresentation(gfx::AcceleratedWidget widget, const std::vector<wl::WaylandPresentationInfo>& presentation_infos)
{
    DCHECK(base::CurrentUIThread::IsSet());
...
void WaylandBufferManagerHost::InsertAcquireFence(uint32_t buffer_id, int sync_fd)
{
    ...
    int rv = HANDLE_EINTR(ioctl(it->second.get(), DMA_BUF_IOCTL_IMPORT_SYNC_FILE, &req));
    PLOG_IF(ERROR, rv) << "Failed DMA_BUF_IOCTL_IMPORT_SYNC_FILE";
}

base::ScopedFD WaylandBufferManagerHost::ExtractReleaseFence(uint32_t buffer_id)
{
    DCHECK(connection_->UseImplicitSyncInterop());
    auto it = dma_buffers_.find(buffer_id);
    if (it == dma_buffers_.end()) {
    ...
    if (HANDLE_EINTR(ioctl(it->second.get(), DMA_BUF_IOCTL_EXPORT_SYNC_FILE, &req)) < 0) {
        return base::ScopedFD();
    }
...
void WaylandBufferManagerHost::SetDrmSyncobjWrapper(std::unique_ptr<DrmSyncobjIoctlWrapper> wrapper)
{
