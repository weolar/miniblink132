...
public:
    ...
    WaylandKeyboard(wl_keyboard* keyboard, zcr_keyboard_extension_v1* keyboard_extension_v1, WaylandConnection* connection,
        KeyboardLayoutEngine* keyboard_layout_engine, Delegate* delegate);
    ...
    wl::Object<zwp_keyboard_shortcuts_inhibitor_v1> CreateShortcutsInhibitor(WaylandWindow* window);
...
private:
    ...
    static void OnKeymap(void* data, wl_keyboard* keyboard, uint32_t format, int32_t fd, uint32_t size);
    static void OnEnter(void* data, wl_keyboard* keyboard, uint32_t serial, wl_surface* surface, wl_array* keys);
    static void OnLeave(void* data, wl_keyboard* keyboard, uint32_t serial, wl_surface* surface);
    ...
    static void OnModifiers(
        void* data, wl_keyboard* keyboard, uint32_t serial, uint32_t mods_depressed, uint32_t mods_latched, uint32_t mods_locked, uint32_t group);
    static void OnRepeatInfo(void* data, wl_keyboard* keyboard, int32_t rate, int32_t delay);

    // wl_callback_listener callbacks:
    static void OnSyncDone(void* data, struct wl_callback* callback, uint32_t time);

    // Callback for wl_keyboard::key and extended_keyboard::peek_key.
    void ProcessKey(uint32_t serial, uint32_t time, uint32_t key, uint32_t state, KeyEventKind kind);

    // Dispatches the key event.
    void DispatchKey(unsigned int key, unsigned int scan_code, bool down, bool repeat, std::optional<uint32_t> serial, base::TimeTicks timestamp, int device_id,
        int flags, KeyEventKind kind);

    // EventAutoRepeatHandler::Delegate
    void FlushInput(base::OnceClosure closure) override;
    void DispatchKey(unsigned int key, unsigned int scan_code, bool down, bool repeat, base::TimeTicks timestamp, int device_id, int flags) override;
...
public:
    ...
    // dispatched.
    virtual uint32_t OnKeyboardKeyEvent(EventType type, DomCode dom_code, bool repeat, std::optional<uint32_t> serial, base::TimeTicks timestamp, int device_id,
        WaylandKeyboard::KeyEventKind kind)
        = 0;
    virtual void OnSynthesizedKeyPressEvent(DomCode dom_code, base::TimeTicks timestamp) = 0;
