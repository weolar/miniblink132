...
void WaylandCursorFactory::ThemeData::AddThemeLoadedCallback(Callback callback)
{
    if (loaded_) {
...
scoped_refptr<PlatformCursor> WaylandCursorFactory::CreateImageCursor(mojom::CursorType type, const SkBitmap& bitmap, const gfx::Point& hotspot, float scale)
{
    ...
    if (std::abs(rounded_scale - scale) > std::numeric_limits<float>::epsilon()) {
        const SkBitmap scaled_bitmap = skia::ImageOperations::Resize(bitmap, skia::ImageOperations::RESIZE_LANCZOS3,
            std::round(bitmap.width() * (rounded_scale / scale)), std::round(bitmap.height() * (rounded_scale / scale)));
        const gfx::Point scaled_hotspot = gfx::ScaleToRoundedPoint(hotspot, rounded_scale / scale);
        bitmap_cursor = base::MakeRefCounted<BitmapCursor>(type, scaled_bitmap, scaled_hotspot, rounded_scale);
    } else {
        bitmap_cursor = base::MakeRefCounted<BitmapCursor>(type, bitmap, hotspot, scale);
    }
...
scoped_refptr<PlatformCursor> WaylandCursorFactory::CreateAnimatedCursor(
    mojom::CursorType type, const std::vector<SkBitmap>& bitmaps, const gfx::Point& hotspot, float scale, base::TimeDelta frame_delay)
{
    scoped_refptr<BitmapCursor> bitmap_cursor;
    ...
    if (std::abs(rounded_scale - scale) > std::numeric_limits<float>::epsilon()) {
        ...
        for (const auto& bitmap : bitmaps) {
            scaled_bitmaps.push_back(skia::ImageOperations::Resize(bitmap, skia::ImageOperations::RESIZE_LANCZOS3,
                std::round(bitmap.width() * (rounded_scale / scale)), std::round(bitmap.height() * (rounded_scale / scale))));
        }
        const gfx::Point scaled_hotspot = gfx::ScaleToRoundedPoint(hotspot, rounded_scale / scale);
        bitmap_cursor = base::MakeRefCounted<BitmapCursor>(type, scaled_bitmaps, scaled_hotspot, frame_delay, rounded_scale);
    } else {
        bitmap_cursor = base::MakeRefCounted<BitmapCursor>(type, bitmaps, hotspot, frame_delay, scale);
    }
    return base::MakeRefCounted<WaylandAsyncCursor>(bitmap_cursor);
}

void WaylandCursorFactory::FinishCursorLoad(scoped_refptr<WaylandAsyncCursor> cursor, mojom::CursorType type, float scale, wl_cursor_theme* loaded_theme)
{
    for (const auto& name : CursorNamesFromType(type)) {
        wl_cursor* theme_cursor = GetCursorFromTheme(loaded_theme, name);
        if (theme_cursor) {
            cursor->SetBitmapCursor(base::MakeRefCounted<BitmapCursor>(type, theme_cursor, GetRoundedScale(scale)));
            return;
        }
    }
    // Fall back to the BitmapCursorFactory implementation if the theme has't
    // provided a shape for the requested type.
    cursor->SetBitmapCursor(BitmapCursor::FromPlatformCursor(BitmapCursorFactory::GetDefaultCursor(type)));
}

scoped_refptr<PlatformCursor> WaylandCursorFactory::GetDefaultCursor(mojom::CursorType type)
{
    // Fall back to 1x for scale if not provided.
    return GetDefaultCursor(type, 1.0);
}

scoped_refptr<PlatformCursor> WaylandCursorFactory::GetDefaultCursor(mojom::CursorType type, float scale)
{
    ...
    if (current_theme->cache.count(type) == 0) {
        ...
        current_theme->AddThemeLoadedCallback(base::BindOnce(&WaylandCursorFactory::FinishCursorLoad, weak_factory_.GetWeakPtr(), async_cursor, type, scale));
    }
    return current_theme->cache[type];
}

wl_cursor* WaylandCursorFactory::GetCursorFromTheme(wl_cursor_theme* theme, const std::string& name)
{
...
void WaylandCursorFactory::OnCursorThemeNameChanged(const std::string& cursor_theme_name)
{
...
WaylandCursorFactory::ThemeData* WaylandCursorFactory::GetThemeForScale(float scale)
{
    ...
    theme_cache_->insert_or_assign(GetScaledSize(scale), std::make_unique<ThemeData>());
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN },
        base::BindOnce(wl_cursor_theme_load, name_.empty() ? nullptr : name_.c_str(), GetScaledSize(scale), connection_->buffer_factory()->shm()),
        base::BindOnce(&WaylandCursorFactory::FinishThemeLoad, weak_factory_.GetWeakPtr(), cache_entry->GetWeakPtr()));
...
void WaylandCursorFactory::FinishThemeLoad(base::WeakPtr<ThemeData> cache_entry, wl_cursor_theme* loaded_theme)
{
