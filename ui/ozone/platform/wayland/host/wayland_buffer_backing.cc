...
WaylandBufferBacking::WaylandBufferBacking(const WaylandConnection* connection, uint32_t buffer_id, const gfx::Size& size, uint32_t format)
...
WaylandBufferHandle* WaylandBufferBacking::EnsureBufferHandle(WaylandSurface* requestor)
{
    ...
    RequestBufferHandle(base::BindOnce(&WaylandBufferHandle::OnWlBufferCreated, buffer_handle->AsWeakPtr()));

    return buffer_handle.get();
}

WaylandBufferHandle* WaylandBufferBacking::GetBufferHandle(WaylandSurface* requestor)
{
    DCHECK(requestor);
    if (UseExplicitSyncRelease()) {
        requestor = nullptr;
