...
std::string GetApplicationOctetStreamName(const std::string& mime_type)
{
    ...
    if (net::MatchesMimeType(std::string(ui::kMimeTypeOctetStream), mime_type) && net::ParseMimeType(mime_type, nullptr, &params)) {
...
template <typename StringType> StringType BytesTo(PlatformClipboard::Data bytes)
{
    using ValueType = typename StringType::value_type;
    const size_t bytes_size = bytes->size();
    const size_t rounded_bytes_size = bytes_size - (bytes_size % sizeof(ValueType));
    if (bytes_size != rounded_bytes_size) {
        // This is suspicious.
        LOG(WARNING) << "Data is possibly truncated, or a wrong conversion is requested.";
    }

    StringType result;
    result.resize(rounded_bytes_size / sizeof(ValueType));
    base::as_writable_byte_span(result).copy_from(base::span(*bytes).first(rounded_bytes_size));
    return result;
}
...
void AddFiles(PlatformClipboard::Data data, OSExchangeDataProvider* provider)
{
    ...
    const auto lines = base::SplitString(data_as_string, "\r\n", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    for (const auto& line : lines) {
        ...
        url::DecodeURLEscapeSequences(url.path_piece(), url::DecodeURLMode::kUTF8OrIsomorphic, &unescaped);
...
void AddFileContents(const std::string& filename, PlatformClipboard::Data data, OSExchangeDataProvider* provider)
{
    DCHECK(provider);
    ...
    provider->SetFileContents(base::FilePath(filename), BytesTo<std::string>(data));
}
...
void AddUrl(PlatformClipboard::Data data, OSExchangeDataProvider* provider)
{
    ...
    const auto lines = base::SplitString(data_as_string16, u"\r\n", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    if (lines.size() != 2U) {
...
std::unique_ptr<OSExchangeDataProvider> WaylandExchangeDataProvider::Clone() const
{
...
std::vector<std::string> WaylandExchangeDataProvider::BuildMimeTypesList() const
{
    ...
    if (HasFileContents()) {
        ...
        base::ReplaceChars(filename, "\"", "\\\"", &filename);
        const std::string mime_type = base::StrCat({ ui::kMimeTypeOctetStream, ";name=\"", filename, "\"" });
...
void WaylandExchangeDataProvider::AddData(PlatformClipboard::Data data, const std::string& mime_type)
{
    DCHECK(data);
...
// TODO(crbug.com/40192823): Support custom formats/pickled data.
bool WaylandExchangeDataProvider::ExtractData(const std::string& mime_type, std::string* out_content) const
{
    DCHECK(out_content);
    DCHECK(IsMimeTypeSupported(mime_type));
    if (std::optional<ui::OSExchangeData::UrlInfo> url_info;
        mime_type == ui::kMimeTypeMozillaURL && (url_info = GetURLAndTitle(kFilenameToURLPolicy)).has_value()) {
        out_content->append(url_info->url.spec());
        return true;
    }
    if ((mime_type == ui::kMimeTypeHTML || mime_type == ui::kMimeTypeHTMLUtf8) && HasHtml()) {
    ...
    if (mime_type == ui::kMimeTypeDataTransferCustomData && HasCustomFormat(ui::ClipboardFormatType::DataTransferCustomType())) {
        std::optional<base::Pickle> pickle = GetPickledData(ui::ClipboardFormatType::DataTransferCustomType());
        *out_content = std::string(reinterpret_cast<const char*>(pickle->data()), pickle->size());
        return true;
    }
