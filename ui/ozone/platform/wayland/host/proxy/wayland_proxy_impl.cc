...
wl_surface* WaylandProxyImpl::GetWlSurfaceForAcceleratedWidget(gfx::AcceleratedWidget widget)
{
...
ui::WaylandWindow* WaylandProxyImpl::GetWaylandWindowForAcceleratedWidget(gfx::AcceleratedWidget widget)
{
    auto* window = connection_->window_manager()->GetWindow(widget);
    DCHECK(window);
    return window;
...
wl_buffer* WaylandProxyImpl::CreateShmBasedWlBuffer(const gfx::Size& buffer_size)
{
...
{
    auto it = base::ranges::find(shm_buffers_, buffer, &ui::WaylandShmBuffer::get);
...
ui::PlatformWindowType WaylandProxyImpl::GetWindowType(gfx::AcceleratedWidget widget)
{
    auto* window = connection_->window_manager()->GetWindow(widget);
    DCHECK(window);
    return window->type();
...
{
    if (delegate_) {
        delegate_->OnWindowConfigured(window->GetWidget(), window->IsSurfaceConfigured());
    }
}
