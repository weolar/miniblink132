...
{
    return state == PlatformWindowState::kNormal || state == PlatformWindowState::kSnappedPrimary || state == PlatformWindowState::kSnappedSecondary
...
WaylandToplevelWindow::WaylandToplevelWindow(PlatformWindowDelegate* delegate, WaylandConnection* connection)
...
void WaylandToplevelWindow::DispatchHostWindowDragMovement(int hittest, const gfx::Point& pointer_location_in_px)
{
...
bool WaylandToplevelWindow::IsVisible() const
{
    ...
    // the same.
    return !!shell_toplevel_ || GetPlatformWindowState() == PlatformWindowState::kMinimized;
}
...
void WaylandToplevelWindow::SetFullscreen(bool fullscreen, int64_t target_display_id)
{
    ...
    if (base::FeatureList::IsEnabled(features::kAsyncFullscreenWindowState)) {
        if (fullscreen) {
            shell_toplevel_->SetFullscreen(GetWaylandOutputForDisplayId(target_display_id));
        } else {
...
void WaylandToplevelWindow::Minimize()
{
    ...
    if (!SupportsConfigureMinimizedState() && IsSurfaceConfigured()) {
        ...
        delegate()->OnWindowStateChanged(previous_state, PlatformWindowState::kMinimized);
    }
}
...
void WaylandToplevelWindow::Activate()
{
    ...
    if (shell_toplevel_ && shell_toplevel_->SupportsActivation()) {
        ...
        if (auto token = base::nix::TakeXdgActivationToken()) {
            ...
            connection()->xdg_activation()->RequestNewToken(base::BindOnce(&WaylandToplevelWindow::ActivateWithToken, weak_ptr_factory_.GetWeakPtr()));
        }
...
void WaylandToplevelWindow::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
    if (!shell_toplevel_) {
        return;
    }
...
void WaylandToplevelWindow::SetShape(std::unique_ptr<ShapeRects> native_shape, const gfx::Transform& transform)
{
    if (shell_toplevel_) {
...
void WaylandToplevelWindow::SetOpaqueRegion(std::optional<std::vector<gfx::Rect>> region_px)
{
    opaque_region_px_ = region_px;
    root_surface()->set_opaque_region(region_px);
}

void WaylandToplevelWindow::SetInputRegion(std::optional<std::vector<gfx::Rect>> region_px)
{
    input_region_px_ = region_px;
    root_surface()->set_input_region(region_px);
}

void WaylandToplevelWindow::NotifyStartupComplete(const std::string& startup_id)
{
...
{
    return shell_toplevel_ && shell_toplevel_->IsSupportedOnAuraToplevel(ZAURA_TOPLEVEL_STATE_MINIMIZED_SINCE_VERSION);
}

bool WaylandToplevelWindow::SupportsConfigurePinnedState() const
{
    return shell_toplevel_ && shell_toplevel_->IsSupportedOnAuraToplevel(ZAURA_TOPLEVEL_STATE_TRUSTED_PINNED_SINCE_VERSION);
}
...
void WaylandToplevelWindow::OnRotateFocus(uint32_t serial, uint32_t direction, bool restart)
{
    if (!is_active_ || !HasKeyboardFocus()) {
        VLOG(1) << "requested focus rotation when surface is not active or does "
                ...
                << is_active_ << ", " << HasKeyboardFocus() << "}. This might be caused by delay in exo. Ignoring request.";
        shell_toplevel()->AckRotateFocus(serial, ZAURA_TOPLEVEL_ROTATE_HANDLED_STATE_NOT_HANDLED);
        return;
    }

    auto platform_direction = direction == ZAURA_TOPLEVEL_ROTATE_DIRECTION_FORWARD ? PlatformWindowDelegate::RotateDirection::kForward
                                                                                   : PlatformWindowDelegate::RotateDirection::kBackward;
    bool rotated = delegate()->OnRotateFocus(platform_direction, restart);
    shell_toplevel()->AckRotateFocus(serial, rotated ? ZAURA_TOPLEVEL_ROTATE_HANDLED_STATE_HANDLED : ZAURA_TOPLEVEL_ROTATE_HANDLED_STATE_NOT_HANDLED);
}

void WaylandToplevelWindow::OnOverviewChange(uint32_t in_overview_as_int)
{
#if BUILDFLAG(IS_CHROMEOS_LACROS)
    const bool in_overview = in_overview_as_int == ZAURA_TOPLEVEL_IN_OVERVIEW_IN_OVERVIEW;
...
void WaylandToplevelWindow::OcclusionStateChanged(PlatformWindowOcclusionState occlusion_state)
{
...
void WaylandToplevelWindow::TooltipShown(const char* text, int32_t x, int32_t y, int32_t width, int32_t height)
{
    delegate()->OnTooltipShownOnServer(base::UTF8ToUTF16(text), gfx::Rect(x, y, width, height));
}
...
void WaylandToplevelWindow::HandleToplevelConfigure(int32_t width_dip, int32_t height_dip, const WindowStates& window_states)
{
    HandleToplevelConfigureWithOrigin(0, 0, width_dip, height_dip, window_states);
}

void WaylandToplevelWindow::HandleToplevelConfigureWithOrigin(int32_t x, int32_t y, int32_t width_dip, int32_t height_dip, const WindowStates& window_states)
{
    ...
    if ((!SupportsConfigureMinimizedState() && GetLatestRequestedState().window_state == PlatformWindowState::kMinimized && !window_states.is_activated)
    ...
    // bounds.
    gfx::Rect bounds_dip(pending_configure_state_.bounds_dip.value_or(gfx::Rect()));
    if (width_dip > 1 && height_dip > 1) {
        ...
        bounds_dip = gfx::ScaleToEnclosingRectIgnoringError(bounds_dip, 1.f / applied_state().ui_scale);
    } else if (ShouldSetBounds(window_state)) {
        bounds_dip = !restored_bounds_dip().IsEmpty() ? restored_bounds_dip() : GetBoundsInDIP();
    }

    bounds_dip = AdjustBoundsToConstraintsDIP(bounds_dip);
    pending_configure_state_.bounds_dip = bounds_dip;
    pending_configure_state_.size_px = delegate()->ConvertRectToPixels(bounds_dip).size();
    ...
    if (prev_suspended != is_suspended_) {
        ...
        OcclusionStateChanged(is_suspended_ ? PlatformWindowOcclusionState::kOccluded : PlatformWindowOcclusionState::kUnknown);
    }
}
...
bool WaylandToplevelWindow::OnInitialize(PlatformWindowInitProperties properties, PlatformWindowDelegate::State* state)
{
    state->window_state = PlatformWindowState::kNormal;

#if BUILDFLAG(IS_CHROMEOS_LACROS)
    auto token = base::UnguessableToken::Create();
    window_unique_id_ = std::string(crosapi::kLacrosAppIdPrefix) + token.ToString();
#else
...
void WaylandToplevelWindow::SetWindowGeometry(const PlatformWindowDelegate::State& state)
{
    ...
    gfx::Rect geometry_dip = gfx::ScaleToEnclosingRectIgnoringError(gfx::Rect(state.bounds_dip.size()), state.ui_scale);
...
void WaylandToplevelWindow::PropagateBufferScale(float new_scale)
{
    ...
    if (!last_sent_buffer_scale_ || last_sent_buffer_scale_.value() != new_scale) {
...
void WaylandToplevelWindow::ShowTooltip(const std::u16string& text, const gfx::Point& position, const PlatformWindowTooltipTrigger trigger,
    const base::TimeDelta show_delay, const base::TimeDelta hide_delay)
{
    auto* zaura_surface = GetZAuraSurface();
    const auto zaura_shell_trigger
        = trigger == PlatformWindowTooltipTrigger::kCursor ? ZAURA_SURFACE_TOOLTIP_TRIGGER_CURSOR : ZAURA_SURFACE_TOOLTIP_TRIGGER_KEYBOARD;
    if (zaura_surface && zaura_surface->ShowTooltip(text, position, zaura_shell_trigger, show_delay, hide_delay)) {
        connection()->Flush();
    }
}
...
bool WaylandToplevelWindow::ShouldReleaseCaptureForDrag(ui::OSExchangeData* data) const
{
...
void WaylandToplevelWindow::StartWindowDraggingSessionIfNeeded(ui::mojom::DragEventSource event_source, bool allow_system_drag)
{
    DCHECK(connection()->window_drag_controller());
...
void WaylandToplevelWindow::ShowSnapPreview(WaylandWindowSnapDirection snap_direction, bool allow_haptic_feedback)
{
    if (shell_toplevel_ && shell_toplevel_->IsSupportedOnAuraToplevel(ZAURA_TOPLEVEL_INTENT_TO_SNAP_SINCE_VERSION)) {
        shell_toplevel_->ShowSnapPreview(snap_direction, allow_haptic_feedback);
        return;
    }

    auto* zaura_surface = GetZAuraSurface();
    zaura_surface_snap_direction zaura_shell_snap_direction = ZAURA_SURFACE_SNAP_DIRECTION_NONE;
    ...
    if (zaura_surface && zaura_surface->IntentToSnap(zaura_shell_snap_direction)) {
        return;
    }
...
void WaylandToplevelWindow::CommitSnap(WaylandWindowSnapDirection snap_direction, float snap_ratio)
{
    // If aura_toplevel does not support `WaylandWindowSnapDirection::kNone` let
    // it fallthrough to `zaura_surface_unset_snap()`.
    const bool use_shell_toplevel = shell_toplevel_
        && (shell_toplevel_->IsSupportedOnAuraToplevel(ZAURA_TOPLEVEL_UNSET_SNAP_SINCE_VERSION)
            || (snap_direction != WaylandWindowSnapDirection::kNone
                && shell_toplevel_->IsSupportedOnAuraToplevel(ZAURA_TOPLEVEL_SET_SNAP_PRIMARY_SINCE_VERSION)));
...
{
    return !!connection()->zwp_pointer_constraints() && !!connection()->zwp_relative_pointer_manager();
}
...
{
    auto* zaura_shell = connection()->zaura_shell();
    return zaura_shell ? base::UTF8ToUTF16(zaura_shell->GetDeskName(index)) : std::u16string();
}
...
void WaylandToplevelWindow::Pin(bool trusted)
{
    if (SupportsConfigurePinnedState()) {
        auto new_state = trusted ? PlatformWindowState::kTrustedPinnedFullscreen : PlatformWindowState::kPinnedFullscreen;
...
{
    if (SupportsConfigurePinnedState()) {
        auto new_state = previously_maximized_ ? PlatformWindowState::kMaximized : PlatformWindowState::kNormal;
        SetWindowState(new_state, display::kInvalidDisplayId);
    } else {
        if (auto* zaura_surface = GetZAuraSurface()) {
...
void WaylandToplevelWindow::DumpState(std::ostream& out) const
{
    ...
    out << ", title=" << window_title_ << ", is_active=" << ToBoolString(is_active_) << ", persistable=" << ToBoolString(persistable_)
...
{
    return workspace_.has_value() ? base::NumberToString(workspace_.value()) : std::string();
}

void WaylandToplevelWindow::SetVisibleOnAllWorkspaces(bool always_visible)
{
    SendToDeskAtIndex(always_visible ? kVisibleOnAllWorkspaces : GetActiveDeskIndex());
}
...
void WaylandToplevelWindow::SetWorkspaceExtensionDelegate(WorkspaceExtensionDelegate* delegate)
{
    workspace_extension_delegate_ = delegate;
}

void WaylandToplevelWindow::TriggerStateChanges(PlatformWindowState window_state)
{
    if (shell_toplevel_) {
        ...
        if (window_state == PlatformWindowState::kMinimized) {
            ...
            shell_toplevel_->SetFullscreen(GetWaylandOutputForDisplayId(fullscreen_display_id_));
        } else if (window_state == PlatformWindowState::kPinnedFullscreen || window_state == PlatformWindowState::kTrustedPinnedFullscreen) {
            if (auto* zaura_surface = GetZAuraSurface()) {
                zaura_surface->SetPin(window_state == PlatformWindowState::kTrustedPinnedFullscreen);
            }
        } else if (GetLatestRequestedState().window_state == PlatformWindowState::kFullScreen) {
            shell_toplevel_->UnSetFullscreen();
        } else if (GetLatestRequestedState().window_state == PlatformWindowState::kPinnedFullscreen
            || GetLatestRequestedState().window_state == PlatformWindowState::kTrustedPinnedFullscreen) {
            if (auto* zaura_surface = GetZAuraSurface()) {
                zaura_surface->UnsetPin();
            }
...
void WaylandToplevelWindow::SetWindowState(PlatformWindowState window_state, int64_t target_display_id)
{
    ...
    if (ShouldTriggerStateChange(window_state, target_display_id)) {
        ...
        fullscreen_display_id_ = (window_state == PlatformWindowState::kFullScreen) ? target_display_id : display::kInvalidDisplayId;

        TriggerStateChanges(window_state);
    }
}

bool WaylandToplevelWindow::ShouldTriggerStateChange(PlatformWindowState window_state, int64_t target_display_id) const
{
    ...
    if (window_state == PlatformWindowState::kFullScreen && target_display_id != display::kInvalidDisplayId && target_display_id != fullscreen_display_id_) {
        return true;
    }
...
WaylandOutput* WaylandToplevelWindow::GetWaylandOutputForDisplayId(int64_t display_id)
{
...
void WaylandToplevelWindow::SetUpShellIntegration()
{
    ...
    if (connection()->zaura_shell()) {
        if (auto* zaura_surface = root_surface()->CreateZAuraSurface()) {
            ...
            // occlusion.
            if (!shell_toplevel_->IsSupportedOnAuraToplevel(ZAURA_TOPLEVEL_CONFIGURE_OCCLUSION_STATE_SINCE_VERSION)) {
...
#if BUILDFLAG(IS_CHROMEOS_LACROS)
        if (shell_toplevel_->IsSupportedOnAuraToplevel(ZAURA_TOPLEVEL_SET_PERSISTABLE_SINCE_VERSION)) {
    ...
    if (connection()->gtk_shell1() && !gtk_surface1_) {
        gtk_surface1_ = connection()->gtk_shell1()->GetGtkSurface1(root_surface()->surface());
    }
}
...
void WaylandToplevelWindow::OnDecorationModeChanged()
{
    ...
    if (use_native_frame_) {
        ...
        // e.g. taskmanager
        shell_toplevel_->SetDecoration(ShellToplevelWrapper::DecorationMode::kServerSide);
        ...
        shell_toplevel_->SetDecoration(ShellToplevelWrapper::DecorationMode::kClientSide);
    }
}
...
void WaylandToplevelWindow::UpdateWindowMask()
{
    ...
    root_surface()->set_opaque_region(
        opaque_region_px_.has_value() ? opaque_region_px_ : (IsOpaqueWindow() ? std::optional<std::vector<gfx::Rect>>(region) : std::nullopt));
    root_surface()->set_input_region(input_region_px_ ? input_region_px_ : region);
}
...
void WaylandToplevelWindow::SetFloatToLocation(WaylandFloatStartLocation float_start_location)
{
