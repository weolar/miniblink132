...
ZwpPrimarySelectionOffer::ZwpPrimarySelectionOffer(zwp_primary_selection_offer_v1* data_offer)
    : data_offer_(data_offer)
{
    static constexpr zwp_primary_selection_offer_v1_listener kPrimarySelectionOfferListener = { .offer = &OnOffer };
    zwp_primary_selection_offer_v1_add_listener(data_offer, &kPrimarySelectionOfferListener, this);
}
...
base::ScopedFD ZwpPrimarySelectionOffer::Receive(const std::string& mime_type)
{
    ...
    zwp_primary_selection_offer_v1_receive(data_offer_.get(), effective_mime_type.data(), write_fd.get());
    return read_fd;
}

// static
void ZwpPrimarySelectionOffer::OnOffer(void* data, zwp_primary_selection_offer_v1* data_offer, const char* mime_type)
{
