...
{
    base::AutoReset<bool> fail_on_convert_to_fd(&MockDrmSyncobjIoctlWrapper::fail_on_syncobj_handle_to_fd, true);
    ASSERT_FALSE(CreateAcquireTimeline());
    ASSERT_FALSE(CreateReleaseTimeline());
}
...
TEST_F(WaylandSyncobjTimelineTest, ImportSyncFdAtCurrentSyncPoint_FailOnSyncobjImportSyncFile)
{
    constexpr int kSyncFileFd = 123;
    auto timeline = CreateAcquireTimeline();
    EXPECT_EQ(timeline->sync_point(), static_cast<unsigned>(0));
    ...
    EXPECT_CALL(*drm_, SyncobjImportSyncFile(2, kSyncFileFd)).WillOnce([] { return ENOENT; });
...
TEST_F(WaylandSyncobjTimelineTest, ImportSyncFdAtCurrentSyncPoint_FailOnSyncobjTransfer)
{
    constexpr int kSyncFileFd = 123;
    auto timeline = CreateAcquireTimeline();
    EXPECT_EQ(timeline->sync_point(), static_cast<unsigned>(0));
    ...
    EXPECT_CALL(*drm_, SyncobjTransfer(1, 1, 2, 0, 0)).WillOnce([] { return ENOENT; });
...
TEST_F(WaylandSyncobjTimelineTest, ExportCurrentSyncPointToSyncFd)
{
    ...
    EXPECT_CALL(*drm_, SyncobjExportSyncFile(2, _)).WillOnce([fd = sync_file_fd.get()](uint32_t, int* sync_file_fd) {
...
TEST_F(WaylandSyncobjTimelineTest, ExportCurrentSyncPointToSyncFd_FailOnSyncobjTransfer)
{
    ...
    EXPECT_CALL(*drm_, SyncobjTransfer(2, 0, 1, kCurrentSyncPoint, 0)).WillOnce([] { return ENOENT; });
...
TEST_F(WaylandSyncobjTimelineTest, ExportCurrentSyncPointToSyncFd_FailOnSyncobjExportSyncFile)
{
    constexpr int kCurrentSyncPoint = 1;
    auto timeline = CreateReleaseTimeline();
    timeline->IncrementSyncPoint();
    ...
    EXPECT_CALL(*drm_, SyncobjExportSyncFile(2, _)).WillOnce([](uint32_t, int* sync_file_fd) { return ENOENT; });
...
TEST_F(WaylandSyncobjTimelineTest, WaitForFenceAvailableAtCurrentSyncPoint)
{
    ...
    auto callback = base::BindOnce(
        [](base::RepeatingClosure quit_closure, base::ScopedFD* fd_out, base::ScopedFD fd) {
            *fd_out = std::move(fd);
            quit_closure.Run();
        },
    ...
    EXPECT_CALL(*drm_, SyncobjEventfd(1, kCurrentSyncPoint, _, DRM_SYNCOBJ_WAIT_FLAGS_WAIT_AVAILABLE)).WillOnce([](uint32_t, uint64_t, int ev_fd, uint32_t) {
        uint64_t value = 1;
    ...
    fd.reset();
    auto callback2 = base::BindOnce([](base::ScopedFD* fd_out, base::ScopedFD fd) { *fd_out = std::move(fd); }, &fd);
    EXPECT_CALL(*drm_, SyncobjTransfer(3, 0, 1, kCurrentSyncPoint, 0));
    EXPECT_CALL(*drm_, SyncobjExportSyncFile(3, _)).WillOnce([fd = sync_file_fd.get()](uint32_t, int* sync_file_fd) {
        *sync_file_fd = HANDLE_EINTR(dup(fd));
        return 0;
    });
...
TEST_F(WaylandSyncobjTimelineTest, WaitForFenceAvailableAtCurrentSyncPoint_RecoverFromOnSyncobjEventfdFailure)
{
    constexpr int kCurrentSyncPoint = 1;
    auto timeline = CreateReleaseTimeline();
    timeline->IncrementSyncPoint();
