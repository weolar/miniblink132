    ...
    WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    ...
    if (connection->zwp_relative_pointer_manager_ || !wl::CanBind(interface, version, kMinVersion, kMinVersion)) {
        return;
    }

    auto new_pointer_manager = wl::Bind<zwp_relative_pointer_manager_v1>(registry, name, kMinVersion);
    if (!new_pointer_manager) {
        LOG(ERROR) << "Failed to bind zwp_relative_pointer_manager_v1";
        return;
    }
    connection->zwp_relative_pointer_manager_ = std::make_unique<WaylandZwpRelativePointerManager>(new_pointer_manager.release(), connection);
}
...
{
    relative_pointer_.reset(zwp_relative_pointer_manager_v1_get_relative_pointer(obj_.get(), connection_->seat()->pointer()->wl_object()));

    static constexpr zwp_relative_pointer_v1_listener kRelativePointerListener = {
        .relative_motion = &OnRelativeMotion,
    };
    zwp_relative_pointer_v1_add_listener(relative_pointer_.get(), &kRelativePointerListener, this);
...
// static
void WaylandZwpRelativePointerManager::OnRelativeMotion(void* data, zwp_relative_pointer_v1* pointer, uint32_t utime_hi, uint32_t utime_lo, wl_fixed_t dx,
    wl_fixed_t dy, wl_fixed_t dx_unaccel, wl_fixed_t dy_unaccel)
{
    ...
    gfx::Vector2dF delta_unaccel = { static_cast<float>(wl_fixed_to_double(dx_unaccel)), static_cast<float>(wl_fixed_to_double(dy_unaccel)) };
    ...
    CHECK(!utime_hi);
#endif
    self->delegate_->OnRelativePointerMotion(delta, wl::EventMillisecondsToTimeTicks(utime_lo));
}
