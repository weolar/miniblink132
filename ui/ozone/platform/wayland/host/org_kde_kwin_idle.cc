...
// static
void OrgKdeKwinIdle::Instantiate(WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    CHECK_EQ(interface, kInterfaceName) << "Expected \"" << kInterfaceName << "\" but got \"" << interface << "\"";

    if (connection->org_kde_kwin_idle_ || !wl::CanBind(interface, version, kMinVersion, kMinVersion)) {
        return;
    }
    ...
    connection->org_kde_kwin_idle_ = std::make_unique<OrgKdeKwinIdle>(idle.release(), connection);
}

OrgKdeKwinIdle::OrgKdeKwinIdle(org_kde_kwin_idle* idle, WaylandConnection* connection)
    : idle_(idle)
...
std::optional<base::TimeDelta> OrgKdeKwinIdle::GetIdleTime() const
{
    ...
    if (!idle_timeout_) {
        idle_timeout_ = std::make_unique<Timeout>(org_kde_kwin_idle_get_idle_timeout(idle_.get(), connection_->seat()->wl_object(), kIdleThresholdMs));
    }
...
OrgKdeKwinIdle::Timeout::Timeout(org_kde_kwin_idle_timeout* timeout)
    : timeout_(timeout)
{
    static constexpr org_kde_kwin_idle_timeout_listener kIdleTimeoutListener = { .idle = &OnIdle, .resumed = &OnResumed };
...
// static
void OrgKdeKwinIdle::Timeout::OnIdle(void* data, org_kde_kwin_idle_timeout* idle_timeout)
{
    auto* self = static_cast<OrgKdeKwinIdle::Timeout*>(data);
    self->idle_timestamp_ = base::Time::Now() - base::Microseconds(kIdleThresholdMs);
}

// static
void OrgKdeKwinIdle::Timeout::OnResumed(void* data, org_kde_kwin_idle_timeout* idle_timeout)
{
    auto* self = static_cast<OrgKdeKwinIdle::Timeout*>(data);
