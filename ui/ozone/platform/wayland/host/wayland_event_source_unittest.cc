...
class WaylandEventSourceTest : public ::testing::WithParamInterface<FeatureState>, public WaylandTestSimple {
public:
    void SetUp() override
    {
        CHECK(!base::Contains(enabled_features_, kDispatchPointerEventsOnFrameEvent));
        CHECK(!base::Contains(disabled_features_, kDispatchPointerEventsOnFrameEvent));
...
TEST_P(WaylandEventSourceTest, CheckPointerButtonHandling)
{
    ...
    EXPECT_FALSE(pointer_delegate_->IsPointerButtonPressed(EF_RIGHT_MOUSE_BUTTON));
    EXPECT_FALSE(pointer_delegate_->IsPointerButtonPressed(EF_MIDDLE_MOUSE_BUTTON));
    EXPECT_FALSE(pointer_delegate_->IsPointerButtonPressed(EF_BACK_MOUSE_BUTTON));
    EXPECT_FALSE(pointer_delegate_->IsPointerButtonPressed(EF_FORWARD_MOUSE_BUTTON));
    ...
    PostToServerAndWait([surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
        ...
        wl_pointer_send_button(pointer, server->GetNextSerial(), server->GetNextTime(), BTN_LEFT, WL_POINTER_BUTTON_STATE_RELEASED);
        wl_pointer_send_frame(pointer);
        wl_pointer_send_button(pointer, server->GetNextSerial(), server->GetNextTime(), BTN_RIGHT, WL_POINTER_BUTTON_STATE_RELEASED);
        wl_pointer_send_frame(pointer);
    });
...
{
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { wl_seat_send_capabilities(server->seat()->resource(), WL_SEAT_CAPABILITY_TOUCH); });
    ASSERT_TRUE(connection_->seat()->touch());

    MockWaylandPlatformWindowDelegate delegate;
    auto window1 = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, kDefaultBounds, &delegate);

    PostToServerAndWait([surface_id = window1->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
        auto* const surface = server->GetObject<wl::MockSurface>(surface_id)->resource();
        auto* const touch = server->seat()->touch()->resource();

        wl_touch_send_down(touch, server->GetNextSerial(), server->GetNextTime(), surface, /*id=*/0, 0, 0);
        wl_touch_send_down(touch, server->GetNextSerial(), server->GetNextTime(), surface, /*id=*/1, 0, 0);
    });
...
TEST_P(WaylandEventSourceTest, ReleasesAllPressedPointerButtons)
{
    ...
    EXPECT_TRUE(pointer_delegate_->IsPointerButtonPressed(EF_LEFT_MOUSE_BUTTON));
    EXPECT_TRUE(pointer_delegate_->IsPointerButtonPressed(EF_RIGHT_MOUSE_BUTTON));
    EXPECT_TRUE(pointer_delegate_->IsPointerButtonPressed(EF_MIDDLE_MOUSE_BUTTON));

    // Verify release buttons are synthesized for mouse pressed events.
    EXPECT_CALL(delegate_, DispatchEvent(_)).Times(3);
    pointer_delegate_->ReleasePressedPointerButtons(window_.get(), base::TimeTicks::Now());
...
INSTANTIATE_TEST_SUITE_P(EventsDispatchPolicyTest, WaylandEventSourceTest,
    ::testing::Values(FeatureState { .dispatch_mouse_events_on_frame_event = false, .dispatch_touch_events_on_frame_event = false },
        FeatureState { .dispatch_mouse_events_on_frame_event = true, .dispatch_touch_events_on_frame_event = true }));
