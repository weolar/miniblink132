...
TEST_F(WaylandEventWatcherTest, CrashKeyResourceError)
{
    ...
    std::string text;
    auto callback = base::BindLambdaForTesting([&text](const std::string& data) { text = data; });

    server_.RunAndWait(
        base::BindLambdaForTesting([&kTestErrorString, callback, surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
            auto* const xdg_surface = server->GetObject<wl::MockSurface>(surface_id)->xdg_surface()->resource();

            // Prepare the expectation error string.
            const std::string expected_error_code = base::StrCat({ wl_resource_get_class(xdg_surface), "#", NumberToString(wl_resource_get_id(xdg_surface)),
                ": error ", NumberToString(static_cast<uint32_t>(XDG_SURFACE_ERROR_UNCONFIGURED_BUFFER)), ": ", kTestErrorString });

            callback.Run(expected_error_code);
            wl_resource_post_error(xdg_surface, XDG_SURFACE_ERROR_UNCONFIGURED_BUFFER, "%s", kTestErrorString.c_str());
        }));
...
TEST_F(WaylandEventWatcherTest, CrashKeyResourceNoMemory)
{
    ...
    const std::string expected_error_code
        = base::StrCat({ "wl_display#1: error ", NumberToString(static_cast<uint32_t>(WL_DISPLAY_ERROR_NO_MEMORY)), ": no memory" });

    server_.RunAndWait(base::BindLambdaForTesting([surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
        auto* const xdg_surface = server->GetObject<wl::MockSurface>(surface_id)->xdg_surface()->resource();
    ...
    EXPECT_EQ(expected_error_code, crash_reporter::GetCrashKeyValue("wayland_error"));
}
...
TEST_F(WaylandEventWatcherTest, CrashKeyClientNoMemoryError)
{
    ...
    server_.RunAndWait(base::BindLambdaForTesting([](wl::TestWaylandServerThread* server) { wl_client_post_no_memory(server->client()); }));
...
TEST_F(WaylandEventWatcherTest, CrashKeyClientImplementationError)
{
    ...
    const std::string expected_error_code
        = base::StrCat({ "wl_display#1: error ", NumberToString(static_cast<uint32_t>(WL_DISPLAY_ERROR_IMPLEMENTATION)), ": ", kError });

    server_.RunAndWait(base::BindLambdaForTesting(
        [&kError](wl::TestWaylandServerThread* server) { wl_client_post_implementation_error(server->client(), "%s", kError.c_str()); }));
...
TEST_F(WaylandEventWatcherTest, CrashKeyCompositorNameSet)
{
    ...
    base::Environment::Create()->SetVar(base::nix::kXdgCurrentDesktopEnvVar, kTestWaylandCompositor);

    server_.RunAndWait(
        base::BindLambdaForTesting([](wl::TestWaylandServerThread* server) { wl_client_post_implementation_error(server->client(), "%s", "stub error"); }));

    base::RunLoop().RunUntilIdle();

    EXPECT_EQ(kTestWaylandCompositor, crash_reporter::GetCrashKeyValue("wayland_compositor"));
}
