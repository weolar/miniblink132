...
constexpr char kSampleTextForDragAndDrop[] = "This is a sample text for drag-and-drop.";
constexpr char16_t kSampleTextForDragAndDrop16[] = u"This is a sample text for drag-and-drop.";

constexpr FilenameToURLPolicy kFilenameToURLPolicy = FilenameToURLPolicy::CONVERT_FILENAMES;

template <typename StringType> PlatformClipboard::Data ToClipboardData(const StringType& data_string)
{
    return base::MakeRefCounted<base::RefCountedBytes>(base::as_byte_span(data_string));
}
...
public:
    ...
    MOCK_METHOD3(OnDragEnter, void(const gfx::PointF& point, int operation, int modifiers));
    MOCK_METHOD0(MockOnDragDataAvailable, void());
    MOCK_METHOD3(MockDragMotion, void(const gfx::PointF& point, int operation, int modifiers));
    ...
    OSExchangeData* dropped_data()
    {
        return dropped_data_.get();
    }

    int available_operations() const
    {
...
protected:
    ...
    int OnDragMotion(const gfx::PointF& point, int operation, int modifiers) override
    {
...
public:
    ...
    MockDropHandler* drop_handler()
    {
        return drop_handler_.get();
    }
    ...
    void RunMouseDragWithSampleData(WaylandWindow* origin_window, int operations)
    {
        ...
        origin_window->StartDrag(os_exchange_data, operations, DragEventSource::kMouse, /*cursor=*/ {},
            /*can_grab_pointer=*/true, drag_started_callback_.Get(), drag_finished_callback_.Get(),
            /*loation delegate=*/nullptr);
    }

    void RunTouchDragWithSampleData(WaylandWindow* origin_window, int operations)
    {
        ASSERT_TRUE(origin_window);
        OSExchangeData os_exchange_data;
        os_exchange_data.SetString(sample_text_for_dnd());
        EXPECT_CALL(drag_started_callback_, Run()).Times(1);
        origin_window->StartDrag(os_exchange_data, operations, DragEventSource::kTouch, /*cursor=*/ {},
            /*can_grab_pointer=*/true, drag_started_callback_.Get(), drag_finished_callback_.Get(),
            /*loation delegate=*/nullptr);
    }

    std::unique_ptr<WaylandWindow> CreateTestWindow(
        PlatformWindowType type, const gfx::Size& size, MockPlatformWindowDelegate* delegate, gfx::AcceleratedWidget context)
    {
        CHECK(delegate);
        ...
        auto window = WaylandWindow::Create(delegate, connection_.get(), std::move(properties));
    ...
    void ScheduleDragCancel()
    {
        ScheduleTestTask(base::BindOnce(
            [](WaylandDataDragControllerTest* self) {
                ...
                EXPECT_CALL(self->drag_finished_callback_, Run(DragOperation::kNone)).Times(1);
    ...
    {
        ScheduleTestTask(base::BindLambdaForTesting([this, action]() { SendDndAction(action); }));
    }

    void FocusAndPressLeftPointerButton(WaylandWindow* window, MockPlatformWindowDelegate* delegate)
    {
...
protected:
    ...
    {
        int64_t time = (EventTimeForNow() - base::TimeTicks()).InMilliseconds() & UINT32_MAX;

        PostToServerAndWait([motion_point, time](wl::TestWaylandServerThread* server) {
            auto* data_device = server->data_device_manager()->data_device();
            ASSERT_TRUE(data_device);
            data_device->OnMotion(time, wl_fixed_from_int(motion_point.x()), wl_fixed_from_int(motion_point.y()));
        });
    }
    ...
    base::MockCallback<ui::WmDragHandler::DragFinishedCallback> drag_finished_callback_;
};
...
TEST_P(WaylandDataDragControllerTest, StartDrag)
{
    ...
    RunMouseDragWithSampleData(window_.get(), DragDropTypes::DRAG_COPY | DragDropTypes::DRAG_MOVE);
...
TEST_P(WaylandDataDragControllerTest, StartDragWithWrongMimeType)
{
    ...
    drag_controller()->StartSession(os_exchange_data, operations, DragEventSource::kMouse);
...
TEST_P(WaylandDataDragControllerTest, StartDragWithCustomFormats)
{
    ...
    ClipboardFormatType kCustomFormats[] = { ClipboardFormatType::DataTransferCustomType(), ClipboardFormatType::GetType("chromium/x-bookmark-entries"),
    ...
    drag_controller()->StartSession(data, DragDropTypes::DRAG_MOVE, DragEventSource::kMouse);
    ...
    PostToServerAndWait([kCustomFormats](wl::TestWaylandServerThread* server) {
        ...
        for (auto format : kCustomFormats) {
            EXPECT_TRUE(base::Contains(mime_types, format.GetName())) << "Format '" << format.GetName() << "' should be offered.";
        }
    });
}
...
TEST_P(WaylandDataDragControllerTest, StartDragWithFileContents)
{
    ...
    OSExchangeData os_exchange_data;
    os_exchange_data.SetFileContents(base::FilePath(FILE_PATH_LITERAL("t\\est\".jpg")), kSampleTextForDragAndDrop);
    int operations = DragDropTypes::DRAG_COPY | DragDropTypes::DRAG_MOVE;
    drag_controller()->StartSession(os_exchange_data, operations, DragEventSource::kMouse);
...
TEST_P(WaylandDataDragControllerTest, StartDragWithEmptyMimeTypeList)
{
    ...
    OSExchangeData os_exchange_data;
    bool started = drag_controller()->StartSession(os_exchange_data, DragDropTypes::DRAG_MOVE, DragEventSource::kMouse);
    ASSERT_TRUE(started);

    PostToServerAndWait([kExpectedMimeType = "chromium/x-empty-drag-data"](wl::TestWaylandServerThread* server) {
...
TEST_P(WaylandDataDragControllerTest, CancelOutgoingDrag)
{
    ...
    EXPECT_CALL(*drop_handler_, MockOnDragDataAvailable()).WillOnce([&]() { drag_controller()->CancelSession(); });
...
TEST_P(WaylandDataDragControllerTest, CancelOutgoingDragOutsideWindow)
{
    ...
    EXPECT_CALL(*drop_handler_, MockOnDragDataAvailable()).WillOnce([&]() { SendDndLeave(); });
...
TEST_P(WaylandDataDragControllerTest, CancelIncomingDrag)
{
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        ...
        data_offer->OnOffer(kMimeTypeText, ToClipboardData(std::string(kSampleTextForDragAndDrop)));

        const uint32_t surface_id = window_->root_surface()->get_surface_id();
        auto* surface = server->GetObject<wl::MockSurface>(surface_id);
        data_device->OnEnter(
            server->GetNextSerial(), surface->resource(), wl_fixed_from_int(pointer_location.x()), wl_fixed_from_int(pointer_location.y()), data_offer);
    });
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        server->data_device_manager()->data_device()->OnMotion(
            server->GetNextTime(), wl_fixed_from_int(pointer_location.x()), wl_fixed_from_int(pointer_location.y()));
    });
    SendDndLeave();
}
...
TEST_P(WaylandDataDragControllerTest, ReceiveDrag)
{
    ...
    EXPECT_CALL(*drop_handler_, MockDragMotion(PointFNear(gfx::PointF(10, 10)), _, _));

    PostToServerAndWait(
        [surface_id, mime_type_text = std::string(kMimeTypeText), sample_text = std::string(kSampleTextForDragAndDrop)](wl::TestWaylandServerThread* server) {
            // HiDPI
            ...
            data_device->OnEnter(
                server->GetNextSerial(), surface->resource(), wl_fixed_from_int(entered_point.x()), wl_fixed_from_int(entered_point.y()), data_offer);
        });
    WaitForDragDropTasks();
    ...
    std::optional<std::u16string> result = drop_handler_->dropped_data()->GetString();
    ASSERT_TRUE(result.has_value());
    EXPECT_EQ(kSampleTextForDragAndDrop16, result.value());

    // In 2x window scale, we expect received coordinates still be in DIP.
    EXPECT_CALL(*drop_handler_, MockDragMotion(PointFNear(gfx::PointF(30, 30)), _, _));
...
TEST_P(WaylandDataDragControllerTest, DropSeveralMimeTypes)
{
    ...
    base::RunLoop loop;
    EXPECT_CALL(*drop_handler_, MockOnDragDataAvailable()).WillOnce([&loop]() { loop.Quit(); });
    EXPECT_CALL(*drop_handler_, OnDragEnter(_, _, _)).Times(1);
    PostToServerAndWait([surface_id](wl::TestWaylandServerThread* server) {
        auto* data_offer = server->data_device_manager()->data_device()->CreateAndSendDataOffer();
        data_offer->OnOffer(kMimeTypeText, ToClipboardData(std::string(kSampleTextForDragAndDrop)));
        data_offer->OnOffer(kMimeTypeMozillaURL, ToClipboardData(std::u16string(u"https://sample.com/\r\nSample")));
        data_offer->OnOffer(kMimeTypeURIList, ToClipboardData(std::string("file:///home/user/file\r\n")));

        gfx::Point entered_point(10, 10);
        wl::MockSurface* surface = server->GetObject<wl::MockSurface>(surface_id);
        server->data_device_manager()->data_device()->OnEnter(
            1002, surface->resource(), wl_fixed_from_int(entered_point.x()), wl_fixed_from_int(entered_point.y()), data_offer);
    });
    loop.Run();
    Mock::VerifyAndClearExpectations(drop_handler_.get());

    EXPECT_CALL(*drop_handler_, MockOnDragDrop()).Times(1);
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { server->data_device_manager()->data_device()->OnDrop(); });
    Mock::VerifyAndClearExpectations(drop_handler_.get());
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { server->data_device_manager()->data_device()->OnLeave(); });
}
...
TEST_P(WaylandDataDragControllerTest, ValidateDroppedUriList)
{
    ...
    for (const auto& kCase : kCases) {
        ...
        const uint32_t surface_id = window_->root_surface()->get_surface_id();
        PostToServerAndWait([surface_id, content = kCase.content](wl::TestWaylandServerThread* server) {
            auto* data_offer = server->data_device_manager()->data_device()->CreateAndSendDataOffer();
        ...
        if (kCase.expected_uris.empty()) {
            ...
            EXPECT_TRUE(drop_handler_->dropped_data()->HasFile());
            std::optional<std::vector<FileInfo>> filenames = drop_handler_->dropped_data()->GetFilenames();
...
TEST_P(WaylandDataDragControllerTest, ValidateDroppedXMozUrl)
{
    ...
    } kCases[] = { { {}, {}, {} }, { u"http://sample.com/\r\nSample", "http://sample.com/", u"Sample" }, { u"http://title.must.be.set/", {}, {} },
        { u"url.must.be.valid/and/have.scheme\r\nInvalid URL", {}, {} },
        { u"file:///files/are/ok\r\nThe policy allows that", "file:///files/are/ok", u"The policy allows that" } };
    ...
    for (const auto& kCase : kCases) {
        ...
        if (kCase.expected_url.empty()) {
            ...
            std::optional<ui::OSExchangeData::UrlInfo> url_info = dropped_data->GetURLAndTitle(kFilenameToURLPolicy);
...
TEST_P(WaylandDataDragControllerTest, ForeignDragHandleAskAction)
{
    ...
    PostToServerAndWait([surface_id](wl::TestWaylandServerThread* server) {
        auto* data_offer = server->data_device_manager()->data_device()->CreateAndSendDataOffer();
        data_offer->OnOffer(kMimeTypeText, ToClipboardData(std::string(kSampleTextForDragAndDrop)));
        data_offer->OnSourceActions(WL_DATA_DEVICE_MANAGER_DND_ACTION_MOVE | WL_DATA_DEVICE_MANAGER_DND_ACTION_COPY);
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        server->data_device_manager()->data_device()->OnMotion(server->GetNextTime(), wl_fixed_from_int(motion_point.x()), wl_fixed_from_int(motion_point.y()));
    });
    WaitForDragDropTasks();
    ...
    PostToServerAndWait([client_data_offer_id](wl::TestWaylandServerThread* server) {
        auto* data_offer = server->GetObject<wl::TestDataOffer>(client_data_offer_id);
        ASSERT_TRUE(data_offer);
        EXPECT_EQ(WL_DATA_DEVICE_MANAGER_DND_ACTION_COPY, data_offer->preferred_action());
        EXPECT_EQ(WL_DATA_DEVICE_MANAGER_DND_ACTION_COPY, data_offer->supported_actions());
...
TEST_P(WaylandDataDragControllerTest, DestroyEnteredSurface)
{
    ...
    auto test = [](WaylandDataDragControllerTest* self) {
        ...
        auto window_2 = self->CreateTestWindow(PlatformWindowType::kWindow, gfx::Size(80, 80), &delegate_2, gfx::kNullAcceleratedWidget);
...
TEST_P(WaylandDataDragControllerTest, DestroyOriginSurface)
{
    ...
    auto test = [](WaylandDataDragControllerTest* self, std::unique_ptr<WaylandWindow>* origin) {
    ...
    // Init and open |target_window|.
    MockPlatformWindowDelegate delegate_2;
    auto window_2 = CreateTestWindow(PlatformWindowType::kPopup, gfx::Size(80, 80), &delegate_2, window_1->GetWidget());
    FocusAndPressLeftPointerButton(window_2.get(), &delegate_);

    // Post test task to be performed asynchronously once the drag session gets
    // started.
    ScheduleTestTask(base::BindOnce(test, base::Unretained(this), base::Unretained(&window_2)));

    // Request to start the drag session, which spins a nested run loop.
    OSExchangeData os_exchange_data;
    os_exchange_data.SetString(sample_text_for_dnd());
    window_2->StartDrag(
        os_exchange_data, DragDropTypes::DRAG_COPY, DragEventSource::kMouse, {}, true, drag_started_callback_.Get(), drag_finished_callback_.Get(), nullptr);
...
TEST_P(WaylandDataDragControllerTest, DragToNonToplevelWindows)
{
    ...
    auto test = [](WaylandDataDragControllerTest* self, PlatformWindowType window_type, gfx::AcceleratedWidget context, bool should_schedule_cancel) {
        // Init and open |target_window|.
        MockPlatformWindowDelegate aux_window_delegate;
        auto aux_window = self->CreateTestWindow(window_type, gfx::Size(100, 40), &aux_window_delegate, context);
    ...
    constexpr PlatformWindowType kNonToplevelWindowTypes[] { PlatformWindowType::kPopup, PlatformWindowType::kMenu, PlatformWindowType::kTooltip,
    ...
    for (auto window_type : kNonToplevelWindowTypes) {
        ...
        const bool should_schedule_end_of_the_test = kNonToplevelWindowTypes[3] == window_type;
        ScheduleTestTask(base::BindOnce(test, base::Unretained(this), window_type,
            window_type == PlatformWindowType::kBubble ? gfx::kNullAcceleratedWidget : origin_window->GetWidget(), should_schedule_end_of_the_test));
    }
...
TEST_P(WaylandDataDragControllerTest, PopupRequestCreatesPopupWindow)
{
    ...
    ScheduleTestTask(base::BindLambdaForTesting([&]() {
        popup_window = CreateTestWindow(PlatformWindowType::kPopup, gfx::Size(100, 40), &delegate, origin_window->GetWidget());
...
TEST_P(WaylandDataDragControllerTest, MenuRequestCreatesPopupWindow)
{
    ...
    auto test = [](WaylandDataDragControllerTest* self, gfx::AcceleratedWidget context) {
        MockPlatformWindowDelegate delegate;
        auto menu_window = self->CreateTestWindow(PlatformWindowType::kMenu, gfx::Size(100, 40), &delegate, context);
        menu_window->Show(false);

        const uint32_t surface_id = menu_window->root_surface()->get_surface_id();
        self->PostToServerAndWait([surface_id](wl::TestWaylandServerThread* server) {
            auto* surface = server->GetObject<wl::MockSurface>(surface_id);
            ASSERT_TRUE(surface);
    ...
    ScheduleTestTask(base::BindOnce(test, base::Unretained(this), origin_window->GetWidget()));
...
TEST_P(WaylandDataDragControllerTest, AsyncNoopStartDrag)
{
    ...
    ASSERT_FALSE(window_->StartDrag(os_exchange_data, DragDropTypes::DRAG_COPY, DragEventSource::kMouse,
        /*cursor=*/ {},
        /*can_grab_pointer=*/true, drag_started_callback_.Get(), drag_finished_callback_.Get(), nullptr));
    Mock::VerifyAndClearExpectations(drop_handler_.get());
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        server->data_device_manager()->data_device()->disable_auto_send_start_drag_events();
    });
    ...
    ScheduleTestTask(base::BindLambdaForTesting([&]() {
        EXPECT_CALL(*drop_handler_, OnDragLeave).Times(1);
        EXPECT_CALL(drag_finished_callback_, Run(Eq(DragOperation::kNone))).Times(1);

        SendPointerButton(window_.get(), &delegate_, BTN_LEFT, /*pressed=*/false);
    }));

    bool result = window_->StartDrag(os_exchange_data, DragDropTypes::DRAG_COPY, DragEventSource::kMouse,
        /*cursor=*/ {},
        /*can_grab_pointer=*/true, drag_started_callback_.Get(), drag_finished_callback_.Get(), nullptr);
...
TEST_P(WaylandDataDragControllerTest, SuppressPointerButtonReleasesAfterEnter)
{
    ...
    FocusAndPressLeftPointerButton(window_.get(), &delegate_);
    drag_controller()->StartSession(os_exchange_data, DragDropTypes::DRAG_COPY, DragEventSource::kMouse);
...
{
    FocusAndPressLeftPointerButton(window_.get(), &delegate_);
    std::optional<wl::Serial> mouse_press_serial = connection()->serial_tracker().GetSerial(wl::SerialType::kMousePress);
    ...
    const uint32_t surface_id = window_->root_surface()->get_surface_id();
    PostToServerAndWait([mouse_serial = mouse_press_serial->value, surface_id](wl::TestWaylandServerThread* server) {
        auto* keyboard = server->seat()->keyboard();
        ASSERT_TRUE(keyboard);
        wl::ScopedWlArray empty({});
        auto* surface = server->GetObject<wl::MockSurface>(surface_id);
        wl_keyboard_send_enter(keyboard->resource(), 1, surface->resource(), empty.get());
        wl_keyboard_send_key(keyboard->resource(), (mouse_serial + 1), 0, 30 /* a */, WL_KEYBOARD_KEY_STATE_PRESSED);
    });
    ...
    RunMouseDragWithSampleData(window_.get(), DragDropTypes::DRAG_COPY);
    Mock::VerifyAndClearExpectations(drop_handler_.get());
    PostToServerAndWait([serial = mouse_press_serial->value](wl::TestWaylandServerThread* server) {
        // Drag must be started. Availability of data_source can be used to
        // determine if the client has initiated a drag session.
        EXPECT_EQ(serial, server->data_device_manager()->data_device()->drag_serial());
        ASSERT_TRUE(server->data_device_manager()->data_source());
    });
}
...
TEST_P(WaylandDataDragControllerTest, StartDragWithCorrectSerialForDragSource)
{
    ...
    bool drag_started = window_->StartDrag(os_exchange_data, DragDropTypes::DRAG_COPY, DragEventSource::kTouch,
        /*cursor=*/ {}, /*can_grab_pointer=*/true, drag_started_callback_.Get(), drag_finished_callback_.Get(), nullptr);
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        // Drag must be started. Availability of data_source can be used to
        // determine if the client has initiated a drag session.
        ASSERT_FALSE(server->data_device_manager()->data_source());
    ...
    bool success = window_->StartDrag(os_exchange_data, DragDropTypes::DRAG_COPY, DragEventSource::kMouse,
        /*cursor=*/ {}, /*can_grab_pointer=*/true, drag_started_callback_.Get(), drag_finished_callback_.Get(), nullptr);
    EXPECT_TRUE(success);
    Mock::VerifyAndClearExpectations(drop_handler_.get());
    PostToServerAndWait([mouse_serial = mouse_press_serial->value](wl::TestWaylandServerThread* server) {
        // Drag must be started. Availability of data_source can be used to
        // determine if the client has initiated a drag session.
        EXPECT_EQ(mouse_serial, server->data_device_manager()->data_device()->drag_serial());
        ASSERT_TRUE(server->data_device_manager()->data_source());
    });
}
...
TEST_P(WaylandDataDragControllerTest, DropWhileFetchingData)
{
    ...
    PostToServerAndWait(
        [surface_id](wl::TestWaylandServerThread* server) {
            ...
            auto* surface = server->GetObject<wl::MockSurface>(surface_id);
            data_device->OnEnter(server->GetNextSerial(), surface->resource(), wl_fixed_from_int(0), wl_fixed_from_int(0), data_offer);
...
TEST_P(WaylandDataDragControllerTest, DndActionsToDragOperations)
{
    ...
    PostToServerAndWait([surface_id](wl::TestWaylandServerThread* server) {
        ...
        auto* data_device = server->data_device_manager()->data_device();
        auto* data_offer = data_device->CreateAndSendDataOffer();
        data_offer->OnSourceActions(WL_DATA_DEVICE_MANAGER_DND_ACTION_MOVE | WL_DATA_DEVICE_MANAGER_DND_ACTION_COPY | WL_DATA_DEVICE_MANAGER_DND_ACTION_ASK);
    ...
    EXPECT_CALL(*drop_handler_, MockDragMotion(_, DragDropTypes::DRAG_COPY | DragDropTypes::DRAG_MOVE | DragDropTypes::DRAG_LINK, _));
...
TEST_P(WaylandDataDragControllerTest, DISABLED_DestroyWindowWhileFetchingForeignData)
{
    auto main_thread_test_task_runnner = task_environment_.GetMainThreadTaskRunner();
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        ...
        const uint32_t surface_id = window_->root_surface()->get_surface_id();
        auto* surface = server->GetObject<wl::MockSurface>(surface_id);
        server_device->OnEnter(server->GetNextSerial(), surface->resource(), wl_fixed_from_int(0), wl_fixed_from_int(0), server_offer);
    });
    ASSERT_EQ(drag_controller(), data_device()->drag_delegate_);
    ...
    EXPECT_EQ(drag_controller_state(), WaylandDataDragController::State::kStarted);
...
TEST_P(WaylandDataDragControllerTest, CursorPositionShouldBeUpdatedDuringMouseDrag)
{
    FocusAndPressLeftPointerButton(window_.get(), &delegate_);
...
TEST_P(WaylandDataDragControllerTest, CursorPositionShouldNotBeUpdatedDuringTouchDrag)
{
    ...
    RunTouchDragWithSampleData(window_.get(), DragDropTypes::DRAG_COPY | DragDropTypes::DRAG_MOVE);
}
...
TEST_P(WaylandDataDragControllerTest, OutgoingSessionWithoutDndFinished)
{
    ...
    ScheduleTestTask(base::BindLambdaForTesting([&]() { SendDndDropPerformed(); }));
    ...
    OSExchangeData os_exchange_data;
    os_exchange_data.SetHtml(sample_text_for_dnd(), {});
    bool started = drag_controller()->StartSession(os_exchange_data, DragDropTypes::DRAG_COPY | DragDropTypes::DRAG_MOVE, DragEventSource::kMouse);
    WaylandTestBase::SyncDisplay();
    ASSERT_TRUE(started);
...
class PerSurfaceScaleWaylandDataDragControllerTest : public WaylandDataDragControllerTest {
public:
    PerSurfaceScaleWaylandDataDragControllerTest() = default;
    ~PerSurfaceScaleWaylandDataDragControllerTest() override = default;

    PerSurfaceScaleWaylandDataDragControllerTest(const PerSurfaceScaleWaylandDataDragControllerTest&) = delete;
    PerSurfaceScaleWaylandDataDragControllerTest& operator=(const PerSurfaceScaleWaylandDataDragControllerTest&) = delete;
...
TEST_P(PerSurfaceScaleWaylandDataDragControllerTest, ScaleEnterAndMotionEventsLocation)
{
    ...
    EXPECT_CALL(*drop_handler_, MockDragMotion(PointFNear(gfx::PointF(0, 80)), _, _));
    PostToServerAndWait([surface_id, location = gfx::Point(0, 100), mime_type_text = std::string(kMimeTypeText),
                            sample_text = std::string(kSampleTextForDragAndDrop)](wl::TestWaylandServerThread* server) {
        auto* data_device = server->data_device_manager()->data_device();
        auto* data_offer = data_device->CreateAndSendDataOffer();
        CHECK(data_offer);
        ...
        wl::MockSurface* surface = server->GetObject<wl::MockSurface>(surface_id);
        data_device->OnEnter(server->GetNextSerial(), surface->resource(), wl_fixed_from_int(location.x()), wl_fixed_from_int(location.y()), data_offer);
    });
    WaitForDragDropTasks();
    Mock::VerifyAndClearExpectations(drop_handler_.get());

    // Ensure a subsequent motion event location also gets properly scaled.
    EXPECT_CALL(*drop_handler_, MockDragMotion(PointFNear(gfx::PointF(160, 160)), _, _));
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandDataDragControllerTest, Values(wl::ServerConfig {}));
INSTANTIATE_TEST_SUITE_P(
    XdgVersionStableTest, PerSurfaceScaleWaylandDataDragControllerTest, Values(wl::ServerConfig { .supports_viewporter_surface_scaling = true }));
#else
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTestWithAuraShell, WaylandDataDragControllerTest,
    Values(wl::ServerConfig { .enable_aura_shell = wl::EnableAuraShellProtocol::kEnabled },
        wl::ServerConfig { .enable_aura_shell = wl::EnableAuraShellProtocol::kEnabled }));
#endif
