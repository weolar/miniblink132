...
bool OverlayStackOrderCompare(const wl::WaylandOverlayConfig& i, const wl::WaylandOverlayConfig& j)
{
...
WaylandWindow::WaylandWindow(PlatformWindowDelegate* delegate, WaylandConnection* connection)
    : delegate_(delegate)
    , connection_(connection)
    , accelerated_widget_(connection->window_manager()->AllocateAcceleratedWidget())
    , frame_manager_(std::make_unique<WaylandFrameManager>(this, connection))
    , wayland_overlay_delegation_enabled_(connection->viewporter() && connection->ShouldUseOverlayDelegation())
...
WaylandWindow::~WaylandWindow()
{
    ...
    if (wayland_overlay_delegation_enabled_) {
        connection_->window_manager()->RemoveSubsurface(GetWidget(), primary_subsurface_.get());
        connection_->window_manager()->RecycleSubsurface(std::move(primary_subsurface_));
    }
    for (const auto& widget_subsurface : wayland_subsurfaces()) {
        connection_->window_manager()->RemoveSubsurface(GetWidget(), widget_subsurface.get());
    }
    if (root_surface_) {
...
void WaylandWindow::UpdateWindowScale(bool update_bounds)
{
    ...
    const auto window_scale = connection_->UsePerSurfaceScaling() ? GetPreferredScaleFactor() : GetScaleFactorFromEnteredOutputs();
...
void WaylandWindow::RemoveBubble(WaylandBubble* window)
{
    ...
    child_bubbles_.erase(std::find(child_bubbles_.begin(), child_bubbles_.end(), window));
}

void WaylandWindow::ActivateBubble(WaylandBubble* window)
{
    CHECK(!window || base::Contains(child_bubbles_, window));
    CHECK(!window || (window->AsWaylandBubble() && window->AsWaylandBubble()->activatable()));
    if (active_bubble_ == window) {
        return;
    }
    if (active_bubble_) {
...
std::optional<WaylandOutput::Id> WaylandWindow::GetPreferredEnteredOutputId()
{
    ...
    if (root_surface_->entered_outputs().empty()) {
        ...
        if (IsScreenCoordinatesEnabled() && connection_->wayland_output_manager()->wayland_screen()) {
            ...
            return connection_->wayland_output_manager()->wayland_screen()->GetOutputIdMatching(GetBoundsInDIP());
        }
        return std::nullopt;
    }

    // PlatformWindowType::kPopup are created as toplevel windows as well.
    DCHECK(type() == PlatformWindowType::kWindow || type() == PlatformWindowType::kPopup);
...
void WaylandWindow::OnPointerFocusChanged(bool focused)
{
    ...
    if (focused && async_cursor_) {
        async_cursor_->AddCursorLoadedCallback(base::BindOnce(&WaylandWindow::OnCursorLoaded, AsWeakPtr(), async_cursor_));
    }
#else
    if (focused && cursor_) {
...
{
    return this == connection_->window_manager()->GetCurrentPointerFocusedWindow();
}

bool WaylandWindow::HasKeyboardFocus() const
{
    return this == connection_->window_manager()->GetCurrentKeyboardFocusedWindow();
}
...
bool WaylandWindow::StartDrag(const ui::OSExchangeData& data, int operations, mojom::DragEventSource source, gfx::NativeCursor cursor, bool can_grab_pointer,
...
{
    if (!connection_->data_drag_controller()->StartSession(data, operations, source)) {
        return false;
    }
...
void WaylandWindow::UpdateDragImage(const gfx::ImageSkia& image, const gfx::Vector2d& offset)
{
...
void WaylandWindow::OnChannelDestroyed()
{
    ...
    base::circular_deque<std::pair<WaylandSubsurface*, wl::WaylandOverlayConfig>> subsurfaces_to_overlays;
    subsurfaces_to_overlays.reserve(wayland_subsurfaces_.size() + (primary_subsurface() ? 1 : 0));
    if (primary_subsurface()) {
        subsurfaces_to_overlays.emplace_back(primary_subsurface(), wl::WaylandOverlayConfig());
    }
    for (auto& subsurface : wayland_subsurfaces_) {
        subsurfaces_to_overlays.emplace_back(subsurface.get(), wl::WaylandOverlayConfig());
    }

    frame_manager_->RecordFrame(std::make_unique<WaylandFrame>(root_surface(), wl::WaylandOverlayConfig(), std::move(subsurfaces_to_overlays)));
}
...
{
    constexpr auto kWindowTypeToString = base::MakeFixedFlatMap<PlatformWindowType, const char*>(
        { { PlatformWindowType::kWindow, "window" }, { PlatformWindowType::kPopup, "popup" }, { PlatformWindowType::kMenu, "menu" },
            { PlatformWindowType::kTooltip, "tooltip" }, { PlatformWindowType::kDrag, "drag" }, { PlatformWindowType::kBubble, "bubble" } });
    out << "type=" << GetMapValueOrDefault(kWindowTypeToString, type_) << ", bounds_in_dip=" << GetBoundsInDIP().ToString()
        << ", bounds_in_pixels=" << GetBoundsInPixels().ToString() << ", restore_bounds_dip=" << restored_bounds_dip_.ToString()
        << ", overlay_delegation=" << (wayland_overlay_delegation_enabled_ ? "enabled" : "disabled");
    if (has_touch_focus_) {
        out << ", has_touch_focus";
    }
    constexpr auto kOpacityToString = base::MakeFixedFlatMap<PlatformWindowOpacity, const char*>({ { PlatformWindowOpacity::kInferOpacity, "infer" },
        { PlatformWindowOpacity::kOpaqueWindow, "opaque" }, { PlatformWindowOpacity::kTranslucentWindow, "translucent" } });
...
gfx::Rect WaylandWindow::GetBoundsInPixels() const
{
    ...
    auto origin = delegate_->ConvertRectToPixels(applied_state_.bounds_dip).origin();
...
void WaylandWindow::SetCursor(scoped_refptr<PlatformCursor> platform_cursor)
{
    ...
    async_cursor->AddCursorLoadedCallback(base::BindOnce(&WaylandWindow::OnCursorLoaded, AsWeakPtr(), async_cursor));
#else
...
void WaylandWindow::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
    NOTIMPLEMENTED_LOG_ONCE();
}
...
uint32_t WaylandWindow::DispatchEvent(const PlatformEvent& native_event)
{
    ...
    if (event->IsLocatedEvent()) {
        auto* event_grabber = connection_->window_manager()->located_events_grabber();
        ...
        //
        bool send_to_grabber = event_grabber && root_parent_window == event_grabber->GetRootParentWindow();
        if (send_to_grabber) {
            WaylandEventSource::ConvertEventToTarget(event_grabber, event->AsLocatedEvent());
        ...
        event->AsLocatedEvent()->set_location_f(gfx::ScalePoint(event->AsLocatedEvent()->location_f(), scale, scale));
...
void WaylandWindow::OcclusionStateChanged(PlatformWindowOcclusionState occlusion_state)
{
...
{
    NOTREACHED() << "Only shell surfaces must receive HandleSurfaceConfigure calls.";
}
...
void WaylandWindow::HandleToplevelConfigure(int32_t widht, int32_t height, const WindowStates& window_states)
{
    NOTREACHED() << "Only shell toplevels must receive HandleToplevelConfigure calls.";
}

void WaylandWindow::HandleToplevelConfigureWithOrigin(int32_t x, int32_t y, int32_t width, int32_t height, const WindowStates& window_states)
{
    NOTREACHED() << "Only shell toplevels must receive HandleAuraToplevelConfigure calls.";
}
...
void WaylandWindow::OnDragEnter(const gfx::PointF& point, int operations)
{
    ...
    const gfx::PointF scaled_point_dip = gfx::ScalePoint(point, 1.0f / applied_state().ui_scale);
...
int WaylandWindow::OnDragMotion(const gfx::PointF& point, int operations)
{
    ...
    // hence the translation below.
    const gfx::PointF scaled_point_dip = gfx::ScalePoint(point, 1.0f / applied_state().ui_scale);
    return drop_handler->OnDragMotion(scaled_point_dip, operations, kWaylandDndModifiers);
}
...
void WaylandWindow::OnDragSessionClose(DragOperation operation)
{
    ...
    if (!connection()->data_drag_controller()->IsWindowDragSessionRunning()) {
        connection()->event_source()->ReleasePressedPointerButtons(this, EventTimeForNow());
    }
...
bool WaylandWindow::Initialize(PlatformWindowInitProperties properties)
{
    ...
    if (state.bounds_dip.IsEmpty()) {
        ...
        const gfx::Point origin = IsScreenCoordinatesEnabled() ? screen->GetDisplayForNewWindows().work_area().CenterPoint() : gfx::Point(0, 0);
    ...
    WaylandKeyboard* keyboard = connection_->seat() ? connection_->seat()->keyboard() : nullptr;
    if (keyboard && properties.inhibit_keyboard_shortcuts) {
        permanent_keyboard_shortcuts_inhibitor_ = keyboard->CreateShortcutsInhibitor(this);
    }
#endif
    ...
    state.size_px = gfx::ScaleToEnclosingRectIgnoringError(gfx::Rect(state.bounds_dip.size()), state.window_scale * state.ui_scale).size();
    ...
    if (wayland_overlay_delegation_enabled_) {
        ...
        connection_->window_manager()->AddSubsurface(GetWidget(), primary_subsurface_.get());
    }
...
void WaylandWindow::SetWindowGeometry(const PlatformWindowDelegate::State& state)
{
}

gfx::Vector2d WaylandWindow::GetWindowGeometryOffsetInDIP() const
{
    const auto& insets_dip = delegate()->CalculateInsetsInDIP(GetPlatformWindowState());
...
WaylandWindow* WaylandWindow::GetXdgParentWindow()
{
    ...
    while (xdg_parent_window && !xdg_parent_window->AsWaylandToplevelWindow() && !xdg_parent_window->AsWaylandPopup()) {
...
uint32_t WaylandWindow::DispatchEventToDelegate(const PlatformEvent& native_event)
{
    EventResult result = DispatchEventFromNativeUiEvent(native_event, base::BindOnce(&PlatformWindowDelegate::DispatchEvent, base::Unretained(delegate_)));
    if (result == ER_UNHANDLED) {
        return POST_DISPATCH_NONE;
    }

    return !!(result & ER_SKIPPED) ? POST_DISPATCH_PERFORM_DEFAULT : POST_DISPATCH_STOP_PROPAGATION;
}
...
bool WaylandWindow::ArrangeSubsurfaceStack(size_t above, size_t below)
{
    ...
    DCHECK(subsurface_stack_below_.size() + subsurface_stack_above_.size() >= above + below);
    ...
    if (subsurface_stack_above_.size() < above) {
        ...
        subsurface_stack_above_.splice(subsurface_stack_above_.end(), subsurface_stack_below_, splice_start, subsurface_stack_below_.end());
        ...
        subsurface_stack_below_.splice(subsurface_stack_below_.end(), subsurface_stack_above_, splice_start, subsurface_stack_above_.end());
    }
...
bool WaylandWindow::CommitOverlays(uint32_t frame_id, const gfx::FrameData& data, std::vector<wl::WaylandOverlayConfig>& overlays)
{
    if (overlays.empty()) {
        return true;
    }
    ...
    if (!std::is_sorted(overlays.begin(), overlays.end(), OverlayStackOrderCompare)) {
    ...
    auto split = std::lower_bound(overlays.begin(), overlays.end(), value, OverlayStackOrderCompare);
    DCHECK(split == overlays.end() || (*split).z_order >= 0);
    size_t num_primary_planes = (split != overlays.end() && (*split).z_order == 0) ? 1 : 0;
    size_t num_background_planes = (overlays.front().z_order == INT32_MIN) ? 1 : 0;
    ...
    if (!wayland_overlay_delegation_enabled_) {
        ...
        frame_manager_->RecordFrame(std::make_unique<WaylandFrame>(frame_id, data, root_surface(), std::move(*overlays.begin())));
        return true;
    }

    base::circular_deque<std::pair<WaylandSubsurface*, wl::WaylandOverlayConfig>> subsurfaces_to_overlays;
    subsurfaces_to_overlays.reserve(std::max(overlays.size() - num_background_planes, wayland_subsurfaces_.size() + 1));

    subsurfaces_to_overlays.emplace_back(primary_subsurface(), num_primary_planes ? std::move(*split) : wl::WaylandOverlayConfig());

    {
        ...
        for (auto iter = subsurface_stack_below_.begin(); iter != subsurface_stack_below_.end(); ++iter, --overlay_iter) {
            if (overlay_iter >= overlays.begin() + num_background_planes) {
                ...
                // moment, hide them.
                subsurfaces_to_overlays.emplace_front(*iter, wl::WaylandOverlayConfig());
            }
        }
        ...
        for (auto iter = subsurface_stack_above_.begin(); iter != subsurface_stack_above_.end(); ++iter, ++overlay_iter) {
    ...
    if (num_background_planes) {
        ...
        root_config
            = wl::WaylandOverlayConfig(gfx::OverlayPlaneData(INT32_MIN, gfx::OverlayTransform::OVERLAY_TRANSFORM_NONE, gfx::RectF(visual_size), gfx::RectF(),
                                           root_surface()->use_blending(), gfx::Rect(), root_surface()->opacity(), gfx::OverlayPriorityHint::kNone,
                                           rounded_clip_bounds.value_or(gfx::RRectF()), gfx::ColorSpace::CreateSRGB(), std::nullopt),
                nullptr, root_surface()->buffer_id(), buffer_scale);
    }

    frame_manager_->RecordFrame(std::make_unique<WaylandFrame>(frame_id, data, root_surface(), std::move(root_config), std::move(subsurfaces_to_overlays)));
...
{
    DCHECK(cursor);
    CHECK(cursor->type() == CursorType::kNone || base::IsValueInRangeForNumericType<int>(cursor->cursor_image_scale_factor()));

    std::optional<uint32_t> shape = WaylandCursorShape::ShapeFromType(cursor->type());
    std::optional<int32_t> zcr_shape = WaylandZcrCursorShapes::ShapeFromType(cursor->type());

    // Round cursor scale factor to ceil as wl_surface.set_buffer_scale accepts
    // only integers.
    if (cursor->type() == CursorType::kNone) { // Hide the cursor.
        connection_->SetCursorBitmap({}, gfx::Point(), std::ceil(cursor->cursor_image_scale_factor()));
        ...
        connection_->SetPlatformCursor(reinterpret_cast<wl_cursor*>(cursor->platform_data()), std::ceil(cursor->cursor_image_scale_factor()));
    } else if (connection_->zcr_cursor_shapes() && zcr_shape.has_value()) { // Check for Exo server-side cursor
        // support.
#if BUILDFLAG(IS_CHROMEOS_LACROS)
    ...
    } else if (!cursor->bitmaps().empty()) { // Use client-side bitmap cursors as fallback.
...
#if BUILDFLAG(IS_LINUX)
void WaylandWindow::OnCursorLoaded(scoped_refptr<WaylandAsyncCursor> cursor, scoped_refptr<BitmapCursor> bitmap_cursor)
{
...
void WaylandWindow::ProcessPendingConfigureState(uint32_t serial)
{
    ...
    if (state.bounds_dip.IsEmpty() && GetPlatformWindowState() == PlatformWindowState::kMinimized && in_flight_requests_.empty()) {
    ...
    if (state == applied_state_ && state == latched_state_ && in_flight_requests_.empty()) {
        root_surface_->Commit(/*flush=*/true);
    }
}

void WaylandWindow::RequestStateFromServer(PlatformWindowDelegate::State state, int64_t serial)
{
    bool force = false;
...
void WaylandWindow::RequestStateFromClient(PlatformWindowDelegate::State state)
{
...
void WaylandWindow::RequestState(PlatformWindowDelegate::State state, int64_t serial, bool force)
{
    LOG_IF(WARNING, in_flight_requests_.size() > 100u) << "The queue of configures is longer than 100!";
    ...
    state.bounds_dip = gfx::ScaleToEnclosingRectIgnoringError(state.bounds_dip, state.ui_scale / new_ui_scale);
    ...
    state.size_px = gfx::ScaleToEnclosingRectIgnoringError(gfx::Rect(state.bounds_dip.size()), scale).size();
...
void WaylandWindow::ProcessSequencePoint(int64_t viz_seq)
{
    ...
    for (auto i = in_flight_requests_.begin(); i != in_flight_requests_.end(); ++i) {
    ...
    if (UseTestConfigForPlatformWindows()) {
        ...
        for (auto i = in_flight_requests_.begin(); i != end; ++i) {
            ...
            latest_latched_viz_seq_for_testing_ = std::max(i->viz_seq, latest_latched_viz_seq_for_testing_);
        }
    }
...
gfx::Rect WaylandWindow::AdjustBoundsToConstraintsPx(const gfx::Rect& bounds_px)
{
    gfx::Rect adjusted_bounds_px = bounds_px;
    if (const auto min_size = delegate_->GetMinimumSizeForWindow()) {
        gfx::Size min_size_in_px = delegate()->ConvertRectToPixels(gfx::Rect(*min_size)).size();
        if (min_size_in_px.width() > 0 && adjusted_bounds_px.width() < min_size_in_px.width()) {
            adjusted_bounds_px.set_width(min_size_in_px.width());
        }
        if (min_size_in_px.height() > 0 && adjusted_bounds_px.height() < min_size_in_px.height()) {
            adjusted_bounds_px.set_height(min_size_in_px.height());
        }
    }
    if (const auto max_size = delegate_->GetMaximumSizeForWindow()) {
        gfx::Size max_size_in_px = delegate()->ConvertRectToPixels(gfx::Rect(*max_size)).size();
        if (max_size_in_px.width() > 0 && adjusted_bounds_px.width() > max_size_in_px.width()) {
            adjusted_bounds_px.set_width(max_size_in_px.width());
        }
        if (max_size_in_px.height() > 0 && adjusted_bounds_px.height() > max_size_in_px.height()) {
...
gfx::Rect WaylandWindow::AdjustBoundsToConstraintsDIP(const gfx::Rect& bounds_dip)
{
    gfx::Rect adjusted_bounds_dip = bounds_dip;
    if (const auto min_size_dip = delegate_->GetMinimumSizeForWindow()) {
        if (min_size_dip->width() > 0 && adjusted_bounds_dip.width() < min_size_dip->width()) {
            adjusted_bounds_dip.set_width(min_size_dip->width());
        }
        if (min_size_dip->height() > 0 && adjusted_bounds_dip.height() < min_size_dip->height()) {
            adjusted_bounds_dip.set_height(min_size_dip->height());
        }
    }
    if (const auto max_size_dip = delegate_->GetMaximumSizeForWindow()) {
        if (max_size_dip->width() > 0 && adjusted_bounds_dip.width() > max_size_dip->width()) {
            adjusted_bounds_dip.set_width(max_size_dip->width());
        }
        if (max_size_dip->height() > 0 && adjusted_bounds_dip.height() > max_size_dip->height()) {
...
void WaylandWindow::LatchStateRequest(const StateRequest& req)
{
    ...
    if (req.state.bounds_dip.size() != old_state.bounds_dip.size()
        || delegate()->CalculateInsetsInDIP(req.state.window_state) != delegate()->CalculateInsetsInDIP(old_state.window_state)) {
...
void WaylandWindow::MaybeApplyLatestStateRequest(bool force)
{
    ...
    // `in_flight_requests_`.
    CHECK(!applying_state_) << "MaybeApplyLatestStateRequest called re-entrantly.";
    auto setter = std::make_optional<base::AutoReset<bool>>(&applying_state_, true);
    ...
    if (!force) {
        int in_flight_applied = base::ranges::count_if(in_flight_requests_, [](const StateRequest& req) { return req.applied; });
    ...
    if (UseTestConfigForPlatformWindows()) {
        latest_applied_viz_seq_for_testing_ = std::max(
            latest_applied_viz_seq_for_testing_, base::ranges::max(in_flight_requests_, {}, [](const StateRequest& req) { return req.viz_seq; }).viz_seq);
    }
...
{
    return in_flight_requests_.empty() ? applied_state_ : in_flight_requests_.back().state;
}
...
void WaylandWindow::ForceApplyWindowStateDoNotUse(PlatformWindowState window_state)
{
