...
public:
    ...
    // created.
    void CreateDmabufBuffer(const base::ScopedFD& fd, const gfx::Size& size, const std::vector<uint32_t>& strides, const std::vector<uint32_t>& offsets,
        const std::vector<uint64_t>& modifiers, uint32_t format, uint32_t planes_count, wl::OnRequestBufferCallback callback) const;
    ...
    wl::Object<struct wl_buffer> CreateShmBuffer(const base::ScopedFD& fd, size_t length, const gfx::Size& size, bool with_alpha_channel = true) const;
    ...
    wl_shm* shm() const
    {
        return wayland_shm_ ? wayland_shm_->get() : nullptr;
    }
