...
WaylandBubble::WaylandBubble(PlatformWindowDelegate* delegate, WaylandConnection* connection, WaylandWindow* parent)
...
void WaylandBubble::SetInputRegion(std::optional<std::vector<gfx::Rect>> region_px)
{
    if (accept_events_) {
...
void WaylandBubble::ShowTooltip(const std::u16string& text, const gfx::Point& position, const PlatformWindowTooltipTrigger trigger,
...
{
    bool is_active = activatable_ && parent_window() && parent_window()->IsActive() && parent_window()->active_bubble() == this;
...
void WaylandBubble::AddToParentAsSubsurface()
{
    ...
    subsurface_ = root_surface()->CreateSubsurface(parent_window()->root_surface());
    DCHECK(subsurface_);
...
void WaylandBubble::SetSubsurfacePosition()
{
    ...
    const auto bounds_dip_in_parent = wl::TranslateWindowBoundsToParentDIP(this, parent_window());
    wl_subsurface_set_position(subsurface_.get(), bounds_dip_in_parent.x(), bounds_dip_in_parent.y());
    parent_window()->root_surface()->Commit();
}

bool WaylandBubble::OnInitialize(PlatformWindowInitProperties properties, PlatformWindowDelegate::State* state)
{
    ...
    if (!accept_events_) {
        root_surface()->set_input_region(std::optional<std::vector<gfx::Rect>>({ gfx::Rect() }));
    }
...
bool WaylandBubble::IsSurfaceConfigured()
{
    ...
    return parent_window() && parent_window()->IsSurfaceConfigured() && subsurface_;
}
