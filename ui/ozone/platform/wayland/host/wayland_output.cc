...
// static
void WaylandOutput::Instantiate(WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    ...
    auto output = wl::Bind<wl_output>(registry, name, std::min(version, kMaxVersion));
    if (!output) {
    ...
    if (!connection->output_manager_) {
        connection->output_manager_ = std::make_unique<WaylandOutputManager>(connection);
    }
    connection->output_manager_->AddWaylandOutput(name, output.release());
}

Metrics::Metrics() = default;
Metrics::Metrics(Id output_id, int64_t display_id, gfx::Point origin, gfx::Size logical_size, gfx::Size physical_size, gfx::Insets insets,
    gfx::Insets physical_overscan_insets, float scale_factor, int32_t panel_transform, int32_t logical_transform, const std::string& description)
    : output_id(output_id)
...
void Metrics::DumpState(std::ostream& out) const
{
    ...
    out << "output_id=" << output_id << ", display_id=" << display_id << ", description=" << description << ", origin=" << origin.ToString()
        << ", logical_size=" << logical_size.ToString() << ", physicacl_size=" << physical_size.ToString() << ", scale_factor=" << scale_factor
        << ", work_area_insets=" << insets.ToString() << ", overscacn_insets=" << physical_overscan_insets.ToString()
        << ", panel_transform=" << GetMapValueOrDefault(kTransformMap, panel_transform)
        << ", logical_transform=" << GetMapValueOrDefault(kTransformMap, logical_transform);
}

WaylandOutput::WaylandOutput(Id output_id, wl_output* output, WaylandConnection* connection)
...
void WaylandOutput::InitializeXdgOutput(zxdg_output_manager_v1* xdg_output_manager)
{
    DCHECK(!xdg_output_);
    xdg_output_ = std::make_unique<XDGOutput>(zxdg_output_manager_v1_get_xdg_output(xdg_output_manager, output_.get()));
}

void WaylandOutput::InitializeColorManagementOutput(WaylandZcrColorManager* zcr_color_manager)
{
    DCHECK(!color_management_output_);
    color_management_output_ = std::make_unique<WaylandZcrColorManagementOutput>(this, zcr_color_manager->CreateColorManagementOutput(output_.get()).release());
}
...
void WaylandOutput::UpdateMetrics()
{
    ...
    if (xdg_output_) {
        xdg_output_->UpdateMetrics(connection_->supports_viewporter_surface_scaling(), metrics_);
    }
}

// static
void WaylandOutput::OnGeometry(void* data, wl_output* output, int32_t x, int32_t y, int32_t physical_width, int32_t physical_height, int32_t subpixel,
...
// static
void WaylandOutput::OnMode(void* data, wl_output* wl_output, uint32_t flags, int32_t width, int32_t height, int32_t refresh)
{
...
// static
void WaylandOutput::OnDescription(void* data, wl_output* output, const char* description)
{
    if (auto* self = static_cast<WaylandOutput*>(data)) {
