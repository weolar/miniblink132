...
// static
void WaylandSeat::Instantiate(WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    CHECK_EQ(interface, kInterfaceName) << "Expected \"" << kInterfaceName << "\" but got \"" << interface << "\"";

    if (connection->seat_ || !wl::CanBind(interface, version, kMinVersion, kMaxVersion)) {
        return;
    }
...
bool WaylandSeat::RefreshKeyboard()
{
    ...
    keyboard_ = std::make_unique<WaylandKeyboard>(keyboard, connection_->keyboard_extension_v1_.get(), connection_, layout_engine, connection_->event_source());
    return true;
}

// static
void WaylandSeat::OnCapabilities(void* data, wl_seat* seat, uint32_t capabilities)
{
...
void WaylandSeat::HandleCapabilities(void* data, wl_seat* seat, uint32_t capabilities)
{
    ...
    if (capabilities & WL_SEAT_CAPABILITY_POINTER) {
        if (wl_pointer* pointer = wl_seat_get_pointer(seat)) {
            pointer_ = std::make_unique<WaylandPointer>(pointer, connection_, connection_->event_source());
        } else {
    ...
    if (capabilities & WL_SEAT_CAPABILITY_TOUCH) {
        if (wl_touch* touch = wl_seat_get_touch(seat)) {
            touch_ = std::make_unique<WaylandTouch>(touch, connection_, connection_->event_source());
        } else {
