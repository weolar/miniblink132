...
// static
void WaylandZcrCursorShapes::Instantiate(WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    CHECK_EQ(interface, kInterfaceName) << "Expected \"" << kInterfaceName << "\" but got \"" << interface << "\"";

    if (connection->zcr_cursor_shapes_ || !wl::CanBind(interface, version, kMinVersion, kMinVersion)) {
        return;
    }

    auto zcr_cursor_shapes = wl::Bind<zcr_cursor_shapes_v1>(registry, name, kMinVersion);
    if (!zcr_cursor_shapes) {
        LOG(ERROR) << "Failed to bind zcr_cursor_shapes_v1";
        return;
    }
    connection->zcr_cursor_shapes_ = std::make_unique<WaylandZcrCursorShapes>(zcr_cursor_shapes.release(), connection);
}

WaylandZcrCursorShapes::WaylandZcrCursorShapes(zcr_cursor_shapes_v1* zcr_cursor_shapes, WaylandConnection* connection)
    : zcr_cursor_shapes_v1_(zcr_cursor_shapes)
...
void WaylandZcrCursorShapes::SetCursorShape(int32_t shape)
{
    ...
    if (!connection_->seat()->pointer())
        return;
    zcr_cursor_shapes_v1_set_cursor_shape(zcr_cursor_shapes_v1_.get(), connection_->seat()->pointer()->wl_object(), shape);
}
