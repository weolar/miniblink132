...
uint32_t TranslateConstraintAdjustment(OwnedWindowConstraintAdjustment constraint_adjustment)
{
    uint32_t res = 0;
    if ((constraint_adjustment & OwnedWindowConstraintAdjustment::kAdjustmentSlideX) != OwnedWindowConstraintAdjustment::kAdjustmentNone)
        res |= XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_SLIDE_X;
    if ((constraint_adjustment & OwnedWindowConstraintAdjustment::kAdjustmentSlideY) != OwnedWindowConstraintAdjustment::kAdjustmentNone)
        res |= XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_SLIDE_Y;
    if ((constraint_adjustment & OwnedWindowConstraintAdjustment::kAdjustmentFlipX) != OwnedWindowConstraintAdjustment::kAdjustmentNone)
        res |= XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_FLIP_X;
    if ((constraint_adjustment & OwnedWindowConstraintAdjustment::kAdjustmentFlipY) != OwnedWindowConstraintAdjustment::kAdjustmentNone)
        res |= XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_FLIP_Y;
    if ((constraint_adjustment & OwnedWindowConstraintAdjustment::kAdjustmentResizeX) != OwnedWindowConstraintAdjustment::kAdjustmentNone)
        res |= XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_RESIZE_X;
    if ((constraint_adjustment & OwnedWindowConstraintAdjustment::kAdjustmentRezizeY) != OwnedWindowConstraintAdjustment::kAdjustmentNone)
        res |= XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_RESIZE_Y;
    return res;
}

zaura_popup_decoration_type TranslateDecorationType(ui::PlatformWindowShadowType platformWindowShadowType)
{
...
XDGPopupWrapperImpl::XDGPopupWrapperImpl(std::unique_ptr<XDGSurfaceWrapperImpl> surface, WaylandWindow* wayland_window, WaylandConnection* connection)
...
bool XDGPopupWrapperImpl::Initialize(const ShellPopupParams& params)
{
    ...
    if (auto* parent_popup = xdg_parent->AsWaylandPopup()) {
        parent_xdg_surface = static_cast<XDGPopupWrapperImpl*>(parent_popup->shell_popup())->xdg_surface_wrapper();
    } else if (auto* parent_toplevel = xdg_parent->AsWaylandToplevelWindow()) {
        parent_xdg_surface = static_cast<XDGToplevelWrapperImpl*>(parent_toplevel->shell_toplevel())->xdg_surface_wrapper();
    }
    ...
    xdg_popup_.reset(xdg_surface_get_popup(xdg_surface_wrapper_->xdg_surface(), parent_xdg_surface->xdg_surface(), positioner.get()));
    if (!xdg_popup_)
        return false;
    ...
    if (connection_->zaura_shell()) {
        ...
        if (version >= ZAURA_SHELL_GET_AURA_POPUP_FOR_XDG_POPUP_SINCE_VERSION) {
            aura_popup_.reset(zaura_shell_get_aura_popup_for_xdg_popup(connection_->zaura_shell()->wl_object(), xdg_popup_.get()));
            if (version >= ZAURA_POPUP_SET_MENU_SINCE_VERSION && wayland_window_->type() == PlatformWindowType::kMenu) {
    ...
    if (auto* parent_popup = xdg_parent->AsWaylandPopup()) {
        parent_shell_popup_has_grab.emplace(parent_popup->shell_popup()->has_grab());
    }
...
{
    if (xdg_popup_get_version(xdg_popup_.get()) < XDG_POPUP_REPOSITIONED_SINCE_VERSION) {
        return false;
    }
    ...
    // meaning.
    xdg_popup_reposition(xdg_popup_.get(), positioner.get(), ++next_reposition_token_);
...
{
    xdg_surface_set_window_geometry(xdg_surface_wrapper_->xdg_surface(), bounds.x(), bounds.y(), bounds.width(), bounds.height());
}
...
{
    zaura_popup_set_decoration(aura_popup_.get(), TranslateDecorationType(shadow_type));
}

void XDGPopupWrapperImpl::SetScaleFactor(float scale_factor)
{
    if (aura_popup_ && zaura_popup_get_version(aura_popup_.get()) >= ZAURA_POPUP_SET_SCALE_FACTOR_SINCE_VERSION) {
...
{
    wl::Object<xdg_positioner> positioner(xdg_wm_base_create_positioner(connection_->shell()));
    if (!positioner)
        return {};
    ...
    FillAnchorData(params_, &anchor_rect, &anchor_position, &anchor_gravity, &constraint_adjustment);
    ...
    xdg_positioner_set_anchor_rect(positioner.get(), anchor_rect.x(), anchor_rect.y(), anchor_rect.width(), anchor_rect.height());
    // XDG protocol does not allow empty geometries, but Chrome does. Set a dummy
    // {1, 1} size to prevent protocol error.
    if (params_.bounds.IsEmpty()) {
        params_.bounds.set_size({ 1, 1 });
    }
    xdg_positioner_set_size(positioner.get(), params_.bounds.width(), params_.bounds.height());
    xdg_positioner_set_anchor(positioner.get(), TranslateAnchor(anchor_position));
    xdg_positioner_set_gravity(positioner.get(), TranslateGravity(anchor_gravity));
    xdg_positioner_set_constraint_adjustment(positioner.get(), TranslateConstraintAdjustment(constraint_adjustment));
    return positioner;
}

// static
void XDGPopupWrapperImpl::OnConfigure(void* data, xdg_popup* popup, int32_t x, int32_t y, int32_t width, int32_t height)
{
...
// static
void XDGPopupWrapperImpl::OnRepositioned(void* data, xdg_popup* popup, uint32_t token)
{
