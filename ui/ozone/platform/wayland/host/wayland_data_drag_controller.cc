...
WaylandDataDragController::WaylandDataDragController(WaylandConnection* connection, WaylandDataDeviceManager* data_device_manager,
    WaylandPointer::Delegate* pointer_delegate, WaylandTouch::Delegate* touch_delegate)
...
bool WaylandDataDragController::StartSession(const OSExchangeData& data, int operations, DragEventSource source)
{
    ...
    auto* origin_window
        = source == DragEventSource::kTouch ? window_manager_->GetCurrentTouchFocusedWindow() : window_manager_->GetCurrentPointerFocusedWindow();
    if (!origin_window) {
    ...
    VLOG(1) << "Starting Drag-and-Drop session. tracker_id=" << serial->value << ", data_source=" << (source == DragEventSource::kMouse ? "mouse" : "touch")
    ...
    data_device_->StartDrag(*data_source_, *origin_window, serial->value, icon_surface_ ? icon_surface_->surface() : nullptr, this);

    SetUpWindowDraggingSessionIfNeeded(data);

    // Monitor mouse events so that the session can be aborted if needed.
    nested_dispatcher_ = PlatformEventSource::GetInstance()->OverrideDispatcher(this);
    return true;
}
...
void WaylandDataDragController::UpdateDragImage(const gfx::ImageSkia& image, const gfx::Vector2d& offset)
{
    icon_image_ = image;
...
bool WaylandDataDragController::ShouldReleaseCaptureForDrag(ui::OSExchangeData* data) const
{
    DCHECK(data);
...
void WaylandDataDragController::DumpState(std::ostream& out) const
{
    constexpr auto kStateToString
        = base::MakeFixedFlatMap<State, const char*>({ { State::kIdle, "idle" }, { State::kStarted, "started" }, { State::kFetching, "fetching" } });
    out << "WaylandDataDragController: state=" << GetMapValueOrDefault(kStateToString, state_) << ", drag_source=" << !!drag_source_
        << ", data_source=" << !!data_source_ << ", origin_window=" << GetWindowName(origin_window_) << ", current_window=" << GetWindowName(window_)
        << ", last_drag_location=" << last_drag_location_.ToString() << ", icon_surface_bufer_scale=" << icon_surface_buffer_scale_
        << ", pending_icon_offset=" << pending_icon_offset_.ToString() << ", current_icon_offset=" << current_icon_offset_.ToString();
}
...
void WaylandDataDragController::DrawIcon()
{
    ...
    static const wl_callback_listener kFrameListener { .done = WaylandDataDragController::OnDragSurfaceFrame };
...
void WaylandDataDragController::OnDragSurfaceFrame(void* data, struct wl_callback* callback, uint32_t time)
{
...
void WaylandDataDragController::DrawIconInternal()
{
    ...
    const gfx::Size size_dip = gfx::ScaleToCeiledSize({ icon_bitmap.width(), icon_bitmap.height() }, 1.0f / icon_surface_buffer_scale_);
    const gfx::Size size_px = gfx::ScaleToCeiledSize(size_dip, icon_surface_buffer_scale_);

    icon_buffer_ = std::make_unique<WaylandShmBuffer>(connection_->buffer_factory(), size_px);
    ...
    if (wl::get_version_of_object(surface) < WL_SURFACE_OFFSET_SINCE_VERSION) {
        wl_surface_attach(
            surface, icon_buffer_->get(), pending_icon_offset_.x() - current_icon_offset_.x(), pending_icon_offset_.y() - current_icon_offset_.y());
    } else {
        wl_surface_attach(surface, icon_buffer_->get(), 0, 0);
        wl_surface_offset(surface, pending_icon_offset_.x() - current_icon_offset_.x(), pending_icon_offset_.y() - current_icon_offset_.y());
    }
    if (connection_->supports_viewporter_surface_scaling() && icon_surface_->viewport()) {
        wp_viewport_set_destination(icon_surface_->viewport(), size_dip.width(), size_dip.height());
    }
...
void WaylandDataDragController::OnDragOffer(std::unique_ptr<WaylandDataOffer> offer)
{
    DCHECK(!data_offer_);
    VLOG(1) << __FUNCTION__;
...
void WaylandDataDragController::OnDragEnter(WaylandWindow* window, const gfx::PointF& location, base::TimeTicks timestamp, uint32_t serial)
{
    DCHECK(window);
    DCHECK(data_offer_);
    ...
    if (pointer_grabber_for_window_drag_) {
        ...
        if (*drag_source_ == DragEventSource::kMouse) {
            pointer_delegate_->OnPointerFocusChanged(window, location, timestamp, wl::EventDispatchPolicy::kImmediate);
        } else {
            touch_delegate_->OnTouchFocusChanged(window);
        }

        pointer_grabber_for_window_drag_ = window_manager_->GetCurrentPointerOrTouchFocusedWindow();
        DCHECK(pointer_grabber_for_window_drag_);
    }

    window_->OnDragEnter(location, DndActionsToDragOperations(data_offer_->source_actions()));
    ...
    if (IsDragSource()) {
        ...
        window_->OnDragDataAvailable(std::make_unique<OSExchangeData>(offered_exchange_data_provider_->Clone()));
    } else {
...
void WaylandDataDragController::OnDragMotion(const gfx::PointF& location, base::TimeTicks timestamp)
{
    VLOG(2) << __FUNCTION__ << " window=" << !!window_ << " location=" << location.ToString() << " fetching=" << (state_ == State::kFetching);
    ...
    if (drag_source_.has_value()) {
        ...
        if (*drag_source_ == mojom::DragEventSource::kMouse) {
            ...
            if (cursor_position) {
                ...
                auto location_in_screen = gfx::ToRoundedPoint(location) + window_->GetBoundsInDIP().origin().OffsetFromOrigin();
    ...
    DCHECK(data_offer_);
    const int client_operations = window_->OnDragMotion(location, DndActionsToDragOperations(data_offer_->source_actions()));
...
{
    VLOG(1) << __FUNCTION__ << " window=" << !!window_ << " fetching=" << (state_ == State::kFetching) << " is_source=" << IsDragSource();
...
void WaylandDataDragController::OnDataSourceFinish(WaylandDataSource* source, base::TimeTicks timestamp, bool completed)
{
    CHECK_EQ(data_source_.get(), source);
    VLOG(1) << __FUNCTION__ << " window=" << !!window_ << " origin=" << !!origin_window_ << " nested_dispatcher=" << !!nested_dispatcher_;
    ...
    HandleDragEnd(completed ? DragResult::kCompleted : DragResult::kCancelled, timestamp);
    Reset();
}

void WaylandDataDragController::OnDataSourceDropPerformed(WaylandDataSource* source, base::TimeTicks timestamp)
{
    CHECK_EQ(data_source_.get(), source);
    VLOG(1) << __FUNCTION__ << " window=" << !!window_ << " origin=" << !!origin_window_ << " nested_dispatcher=" << !!nested_dispatcher_;
...
void WaylandDataDragController::OnDataSourceSend(WaylandDataSource* source, const std::string& mime_type, std::string* buffer)
{
    CHECK_EQ(data_source_.get(), source);
    CHECK(buffer);
    ...
    if (!GetOfferedExchangeDataProvider()->ExtractData(mime_type, buffer)) {
        LOG(WARNING) << "Cannot deliver data of type " << mime_type << " and no text representation is available.";
    }
}
...
void WaylandDataDragController::PostDataFetchingTask(const gfx::PointF& location, base::TimeTicks start_time, const scoped_refptr<CancelFlag>& cancel_flag)
{
    ...
    auto fetch_data_closure = [](FetchingInfo offered_data, const scoped_refptr<CancelFlag>& cancel_flag) -> std::unique_ptr<OSExchangeData> {
        base::ScopedBlockingCall blocking_call(FROM_HERE, base::BlockingType::MAY_BLOCK);
        auto fetched_data = std::make_unique<WaylandExchangeDataProvider>();

        VLOG(1) << "Starting data fetching for " << offered_data.size() << " mime types.";
        ...
        for (const auto& [mime_type, fd_handle] : offered_data) {
            ...
            fetched_data->AddData(base::MakeRefCounted<base::RefCountedBytes>(std::move(contents)), mime_type);
        }
    ...
    GetDataFetchTaskRunner().PostTaskAndReplyWithResult(FROM_HERE, base::BindOnce(fetch_data_closure, std::move(offered_data), cancel_flag),
        base::BindOnce(&WaylandDataDragController::OnDataFetchingFinished, weak_factory_.GetWeakPtr(), start_time));
}

void WaylandDataDragController::OnDataFetchingFinished(base::TimeTicks timestamp, std::unique_ptr<OSExchangeData> received_data)
{
    // This is expected to be called only in incoming drag sessions.
    DCHECK(!IsDragSource());
    VLOG(1) << __func__ << " fetching=" << (state_ == State::kFetching) << " window=" << !!window_;
...
void WaylandDataDragController::CancelDataFetchingIfNeeded()
{
    ...
    if (data_fetch_cancel_flag_) {
        VLOG_IF(1, data_fetch_cancel_flag_->data.IsSet()) << "Cancelling data fetching.";
...
void WaylandDataDragController::HandleDragEnd(DragResult result, base::TimeTicks timestamp)
{
    if (origin_window_) {
        DragOperation operation = (result == DragResult::kCompleted) ? DndActionToDragOperation(data_source_->dnd_action()) : DragOperation::kNone;
...
std::optional<wl::Serial> WaylandDataDragController::GetAndValidateSerialForDrag(DragEventSource source)
{
    ...
    return should_drag ? connection_->serial_tracker().GetSerial(serial_type) : std::nullopt;
}

const WaylandExchangeDataProvider* WaylandDataDragController::GetOfferedExchangeDataProvider() const
{
    DCHECK(offered_exchange_data_provider_);
    return static_cast<const WaylandExchangeDataProvider*>(offered_exchange_data_provider_.get());
}

bool WaylandDataDragController::IsWindowDraggingSession(const ui::OSExchangeData& data) const
{
    auto custom_format = ui::ClipboardFormatType::GetType(ui::kMimeTypeWindowDrag);
    return data.provider().HasCustomFormat(custom_format);
}

void WaylandDataDragController::SetUpWindowDraggingSessionIfNeeded(const ui::OSExchangeData& data)
{
...
void WaylandDataDragController::DispatchPointerRelease(base::TimeTicks timestamp)
{
    DCHECK(pointer_grabber_for_window_drag_);
    pointer_delegate_->OnPointerButtonEvent(EventType::kMouseReleased, EF_LEFT_MOUSE_BUTTON, timestamp, pointer_grabber_for_window_drag_,
...
uint32_t WaylandDataDragController::DispatchEvent(const PlatformEvent& event)
{
    ...
    if (wl::MaybeHandlePlatformEventForDrag(event, /*start_drag_ack_received=*/has_received_enter_, std::move(cancel_drag_cb))) {
...
base::TaskRunner& WaylandDataDragController::GetDataFetchTaskRunner()
{
    if (!data_fetch_task_runner_) {
        data_fetch_task_runner_ = base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_VISIBLE, base::MayBlock() });
    }
