...
// and aura_output.
class WaylandZAuraOutputManagerV2 : public wl::GlobalObjectRegistrar<WaylandZAuraOutputManagerV2> {
public:
    using OutputMetricsMap = base::flat_map<WaylandOutput::Id, WaylandOutput::Metrics>;

    static constexpr char kInterfaceName[] = "zaura_output_manager_v2";

    static void Instantiate(WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version);

    WaylandZAuraOutputManagerV2(zaura_output_manager_v2* output_manager, WaylandConnection* connection);
    WaylandZAuraOutputManagerV2(const WaylandZAuraOutputManagerV2&) = delete;
    WaylandZAuraOutputManagerV2& operator=(const WaylandZAuraOutputManagerV2&) = delete;
...
private:
    ...
    static void OnDisplayId(void* data, zaura_output_manager_v2* output_manager, uint32_t output_id, uint32_t display_id_hi, uint32_t display_id_lo);
    static void OnLogicalPosition(void* data, zaura_output_manager_v2* output_manager, uint32_t output_id, int32_t x, int32_t y);
    static void OnLogicalSize(void* data, zaura_output_manager_v2* output_manager, uint32_t output_id, int32_t width, int32_t height);
    static void OnPhysicalSize(void* data, zaura_output_manager_v2* output_manager, uint32_t output_id, int32_t width, int32_t height);
    static void OnWorkAreaInsets(
    ...
    static void OnDeviceScaleFactor(void* data, zaura_output_manager_v2* output_manager, uint32_t output_id, uint32_t scale_as_uint);
    static void OnLogicalTransform(void* data, zaura_output_manager_v2* output_manager, uint32_t output_id, int32_t transform);
    static void OnPanelTransform(void* data, zaura_output_manager_v2* output_manager, uint32_t output_id, int32_t transform);
    static void OnName(void* data, zaura_output_manager_v2* output_manager, uint32_t output_id, const char* name);
    static void OnDescription(void* data, zaura_output_manager_v2* output_manager, uint32_t output_id, const char* description);
    static void OnOverscanInsets(
        void* data, zaura_output_manager_v2* output_manager, uint32_t output_id, int32_t top, int32_t left, int32_t bottom, int32_t right);
    static void OnActivated(void* data, zaura_output_manager_v2* output_manager, uint32_t output_id);
