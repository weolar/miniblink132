...
{
    return base::MakeRefCounted<base::RefCountedBytes>(base::ToVector(base::as_byte_span(data_string)));
}

// This must be called on the server thread.
wl::TestSelectionDevice* GetSelectionDevice(wl::TestWaylandServerThread* server, ClipboardBuffer buffer)
{
    DCHECK(server->task_runner()->BelongsToCurrentThread());

    return buffer == ClipboardBuffer::kSelection ? server->primary_selection_device_manager()->device() : server->data_device_manager()->data_device();
}

// This must be called on the server thread.
wl::TestSelectionSource* GetSelectionSource(wl::TestWaylandServerThread* server, ClipboardBuffer buffer)
{
    DCHECK(server->task_runner()->BelongsToCurrentThread());

    return buffer == ClipboardBuffer::kSelection ? server->primary_selection_device_manager()->source() : server->data_device_manager()->data_source();
}
...
public:
    ...
    void SetUp() override
    {
        ...
        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            wl_seat_send_capabilities(server->seat()->resource(), WL_SEAT_CAPABILITY_POINTER | WL_SEAT_CAPABILITY_TOUCH | WL_SEAT_CAPABILITY_KEYBOARD);
...
protected:
    ...
    {
        PostToServerAndWait([&location, surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
            auto* const pointer = server->seat()->pointer()->resource();
            auto* const surface = server->GetObject<wl::MockSurface>(surface_id)->resource();

            wl_pointer_send_enter(pointer, server->GetNextSerial(), surface, wl_fixed_from_int(location.x()), wl_fixed_from_int(location.y()));
    ...
    void SendTouchDown(const gfx::Point& location)
    {
        PostToServerAndWait([location, surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
            ...
            wl_touch_send_down(
                touch, server->GetNextSerial(), server->GetNextTime(), surface, 0 /* id */, wl_fixed_from_int(location.x()), wl_fixed_from_int(location.y()));
    ...
    void SendTouchUp()
    {
        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            ...
            wl_touch_send_up(touch, server->GetNextSerial(), server->GetNextTime(), 0 /* id */);
        });
    }
    ...
    void SendKeyboardKey()
    {
        PostToServerAndWait([surface_id = window_->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
            ...
            wl_keyboard_send_enter(keyboard, server->GetNextSerial(), surface, empty.get());
            wl_keyboard_send_key(keyboard, server->GetNextSerial(), server->GetNextTime(), 30 /* a */, WL_KEYBOARD_KEY_STATE_PRESSED);
        });
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        PostToServerAndWait([primary_selection_protocol = GetParam().primary_selection_protocol](wl::TestWaylandServerThread* server) {
            ASSERT_TRUE(primary_selection_protocol == wl::PrimarySelectionProtocol::kNone || server->primary_selection_device_manager());
        });
        ...
        ASSERT_EQ(WhichBufferToUse() == ClipboardBuffer::kSelection, !!clipboard_->GetClipboard(ClipboardBuffer::kSelection));
        WaylandTestBase::SyncDisplay();
...
protected:
    ...
    {
        return GetParam().primary_selection_protocol != wl::PrimarySelectionProtocol::kNone ? ClipboardBuffer::kSelection : ClipboardBuffer::kCopyPaste;
    }

    // Fill the clipboard backing store with sample data.
    void OfferData(ClipboardBuffer buffer, std::string_view data, const std::string& mime_type)
    {
        offered_data_[mime_type] = base::MakeRefCounted<base::RefCountedBytes>(base::ToVector(base::as_byte_span(data)));
...
public:
    void SetUp() override
    {
        ...
        ASSERT_EQ(wl::PrimarySelectionProtocol::kNone, GetParam().primary_selection_protocol);
...
{
    const base::RepeatingClosure send_input_event_closures[] { // Mouse button press
    ...
    for (auto send_input_event : send_input_event_closures) {
        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            if (GetParam().primary_selection_protocol != wl::PrimarySelectionProtocol::kNone) {
        ...
        auto client_selection_serial
            = connection_->serial_tracker().GetSerial({ wl::SerialType::kTouchPress, wl::SerialType::kMousePress, wl::SerialType::kKeyPress });
        ...
        PostToServerAndWait([&delivered_text, &callback, buffer = WhichBufferToUse(), serial = client_selection_serial->value,
                                quit_closure = run_loop.QuitClosure()](wl::TestWaylandServerThread* server) {
            ASSERT_TRUE(GetSelectionSource(server, buffer));

            EXPECT_EQ(serial, GetSelectionDevice(server, buffer)->selection_serial());

            EXPECT_CALL(callback, Run(_)).WillOnce([&delivered_text, quit_closure](std::vector<uint8_t>&& data) {
                delivered_text = std::string(data.begin(), data.end());
                quit_closure.Run();
            });

            GetSelectionSource(server, buffer)->ReadData(kMimeTypeTextUtf8, callback.Get());
        });

        WaitForClipboardTasks();

        PostToServerAndWait([&delivered_text](wl::TestWaylandServerThread* server) { EXPECT_EQ(kSampleClipboardText, delivered_text); });
...
TEST_P(WaylandClipboardTest, ReadFromClipboard)
{
    ...
    PostToServerAndWait([buffer = WhichBufferToUse()](wl::TestWaylandServerThread* server) {
        auto* device = GetSelectionDevice(server, buffer);
        auto* data_offer = device->OnDataOffer();
        data_offer->OnOffer(kMimeTypeTextUtf8, ToClipboardData(kSampleClipboardText));
    ...
    clipboard_->RequestClipboardData(WhichBufferToUse(), kMimeTypeTextUtf8, callback.Get());
...
TEST_P(WaylandClipboardTest, IsSelectionOwner)
{
    ...
    PostToServerAndWait([buffer = WhichBufferToUse()](wl::TestWaylandServerThread* server) { ASSERT_TRUE(GetSelectionSource(server, buffer)); });
    ...
    PostToServerAndWait([buffer = WhichBufferToUse()](wl::TestWaylandServerThread* server) { GetSelectionSource(server, buffer)->OnCancelled(); });
...
TEST_P(WaylandClipboardTest, OverlapReadingFromDifferentBuffers)
{
    ...
    auto other_buffer = WhichBufferToUse() == ClipboardBuffer::kSelection ? ClipboardBuffer::kCopyPaste : ClipboardBuffer::kSelection;
    base::MockCallback<PlatformClipboard::RequestDataClosure> callback;
    EXPECT_CALL(callback, Run(Eq(nullptr))).Times(1);
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&PlatformClipboard::RequestClipboardData, base::Unretained(clipboard_), other_buffer, kMimeTypeTextUtf8, callback.Get()));
    ...
    clipboard_->RequestClipboardData(WhichBufferToUse(), kMimeTypeTextUtf8, got_text.Get());
...
{
    base::MockCallback<PlatformClipboard::ClipboardDataChangedCallback> clipboard_changed_callback;
...
TEST_P(CopyPasteOnlyClipboardTest, DISABLED_OverlappingReadRequests)
{
    ...
    EXPECT_CALL(got_html, Run(_)).WillOnce([&](PlatformClipboard::Data data) { html = std::string(base::as_string_view(*data)); });
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE,
        base::BindOnce(&PlatformClipboard::RequestClipboardData, base::Unretained(clipboard_), ClipboardBuffer::kCopyPaste, kMimeTypeHTML, got_html.Get()));

    // Instantly start a read request for text/plain mime type.
    std::string text;
    base::MockCallback<PlatformClipboard::RequestDataClosure> got_text;
    EXPECT_CALL(got_text, Run(_)).WillOnce([&](PlatformClipboard::Data data) { text = std::string(base::as_string_view(*data)); });
    clipboard_->RequestClipboardData(ClipboardBuffer::kCopyPaste, kMimeTypeText, got_text.Get());
...
INSTANTIATE_TEST_SUITE_P(
    WithZwpPrimarySelection, WaylandClipboardTest, Values(wl::ServerConfig { .primary_selection_protocol = wl::PrimarySelectionProtocol::kZwp }));

INSTANTIATE_TEST_SUITE_P(
    WithGtkPrimarySelection, WaylandClipboardTest, Values(wl::ServerConfig { .primary_selection_protocol = wl::PrimarySelectionProtocol::kGtk }));

INSTANTIATE_TEST_SUITE_P(
    WithoutPrimarySelection, WaylandClipboardTest, Values(wl::ServerConfig { .primary_selection_protocol = wl::PrimarySelectionProtocol::kNone }));

INSTANTIATE_TEST_SUITE_P(
    WithoutPrimarySelection, CopyPasteOnlyClipboardTest, Values(wl::ServerConfig { .primary_selection_protocol = wl::PrimarySelectionProtocol::kNone }));
