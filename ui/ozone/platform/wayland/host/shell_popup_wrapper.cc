...
bool IsGnomeShell()
{
    ...
    return base::nix::GetDesktopEnvironment(env.get()) == base::nix::DESKTOP_ENVIRONMENT_GNOME;
}
...
ShellPopupParams& ShellPopupParams::operator=(const ShellPopupParams&) = default;
ShellPopupParams::~ShellPopupParams() = default;

void ShellPopupWrapper::FillAnchorData(const ShellPopupParams& params, gfx::Rect* anchor_rect, OwnedWindowAnchorPosition* anchor_position,
    OwnedWindowAnchorGravity* anchor_gravity, OwnedWindowConstraintAdjustment* constraints) const
{
...
void ShellPopupWrapper::GrabIfPossible(WaylandConnection* connection, std::optional<bool> parent_shell_popup_has_grab)
{
#if !BUILDFLAG(IS_CHROMEOS_LACROS)
    ...
    auto serial = connection->serial_tracker().GetSerial({ wl::SerialType::kTouchPress, wl::SerialType::kMousePress, wl::SerialType::kKeyPress });
    if (!serial.has_value())
        return;
