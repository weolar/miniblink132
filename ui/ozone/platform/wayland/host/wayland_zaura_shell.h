...
private:
    ...
    static void OnBugFix(void* data, struct zaura_shell* zaura_shell, uint32_t id);
    static void OnDesksChanged(void* data, struct zaura_shell* zaura_shell, struct wl_array* states);
    static void OnDeskActivationChanged(void* data, struct zaura_shell* zaura_shell, int active_desk_index);
    static void OnActivated(void* data, struct zaura_shell* zaura_shell, struct wl_surface* gained_active, struct wl_surface* lost_active);
    ...
    static void OnCompositorVersion(void* data, struct zaura_shell* zaura_shell, const char* version_label);
    static void OnAllBugFixesSent(void* data, struct zaura_shell* zaura_shell);
    static void OnSetWindowCornersRadii(void* data, struct zaura_shell* zaura_shell, uint32_t upper_left_radius, uint32_t upper_right_radius,
