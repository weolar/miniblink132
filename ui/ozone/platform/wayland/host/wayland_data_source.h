...
public:
    class Delegate {
    public:
        virtual void OnDataSourceFinish(DataSource<T>* source, base::TimeTicks timestamp, bool completed) = 0;
        virtual void OnDataSourceSend(DataSource<T>* source, const std::string& mime_type, std::string* contents) = 0;
        // Optional callback intended to be implemented only by dnd-capable delegate
        // implementations.
        virtual void OnDataSourceDropPerformed(DataSource<T>* source, base::TimeTicks timestamp)
        {
        }
    ...
    DataSource(T* data_source, ui::WaylandConnection* connection, Delegate* delegate);
