...
public:
    WaylandToplevelWindow(PlatformWindowDelegate* delegate, WaylandConnection* connection);
    ...
    ShellToplevelWrapper* shell_toplevel() const
    {
        return shell_toplevel_.get();
    }
    ...
    void TooltipShown(const char* text, int32_t x, int32_t y, int32_t width, int32_t height) override;
    void TooltipHidden() override;
    WaylandToplevelWindow* AsWaylandToplevelWindow() override;

    // Configure related:
    void HandleToplevelConfigure(int32_t width, int32_t height, const WindowStates& window_states) override;
    void HandleToplevelConfigureWithOrigin(int32_t x, int32_t y, int32_t width, int32_t height, const WindowStates& window_states) override;
    void HandleSurfaceConfigure(uint32_t serial) override;
    void OnSequencePoint(int64_t seq) override;
    bool IsSurfaceConfigured() override;
    void AckConfigure(uint32_t serial) override;

    bool OnInitialize(PlatformWindowInitProperties properties, PlatformWindowDelegate::State* state) override;
    ...
    void ShowTooltip(const std::u16string& text, const gfx::Point& position, const PlatformWindowTooltipTrigger trigger, const base::TimeDelta show_delay,
    ...
    void StartWindowDraggingSessionIfNeeded(ui::mojom::DragEventSource event_source, bool allow_system_drag) override;
    void ShowSnapPreview(WaylandWindowSnapDirection snap, bool allow_haptic_feedback) override;
    ...
    void SetFloatToLocation(WaylandFloatStartLocation float_start_location) override;
...
private:
    ...
    bool ShouldTriggerStateChange(PlatformWindowState state, int64_t target_display_id) const;
