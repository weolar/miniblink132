...
public:
    ...
    {
        return std::move(display_);
    }
    display::Display GetRemovedDisplay()
    {
        return std::move(removed_display_);
    }
...
protected:
    std::unique_ptr<WaylandWindow> CreateWaylandWindowWithProperties(
        const gfx::Rect& bounds, PlatformWindowType window_type, gfx::AcceleratedWidget parent_widget, MockWaylandPlatformWindowDelegate* delegate)
    {
        PlatformWindowInitProperties properties;
        properties.bounds = bounds;
        properties.type = window_type;
        properties.parent_widget = parent_widget;
        return delegate->CreateWaylandWindow(connection_.get(), std::move(properties));
    }

    void ValidateTheDisplayForWidget(gfx::AcceleratedWidget widget, int64_t expected_display_id)
    {
        display::Display display_for_widget = platform_screen_->GetDisplayForAcceleratedWidget(widget);
...
TEST_P(WaylandScreenTest, OutputBaseTest)
{
    ...
    EXPECT_EQ(platform_screen_->GetPrimaryDisplay().bounds(), gfx::Rect(0, 0, kOutputWidth, kOutputHeight));
}
...
TEST_P(WaylandScreenTest, EnteredOutputListAfterDisplayRemoval)
{
    ...
    // display.
    PostToServerAndWait([&output2, &output1](wl::TestWaylandServerThread* server) {
        output2 = server->CreateAndInitializeOutput(wl::TestOutputMetrics({ GetRightX(output1), 0, 800, 600 }));
        ASSERT_TRUE(output2);
    });

    // Add a third display. The third display is located to the right of second
    // display.
    PostToServerAndWait([&output3, &output2](wl::TestWaylandServerThread* server) {
        output3 = server->CreateAndInitializeOutput(wl::TestOutputMetrics({ GetRightX(output2), 0, 800, 600 }));
        ASSERT_TRUE(output3);
    });
    ...
    PostToServerAndWait([output1, output2, surface_id](wl::TestWaylandServerThread* server) {
    ...
    PostToServerAndWait([output3, surface_id](wl::TestWaylandServerThread* server) {
        auto* surface = server->GetObject<wl::MockSurface>(surface_id);
    ...
    PostToServerAndWait([output2, surface_id](wl::TestWaylandServerThread* server) {
        auto* surface = server->GetObject<wl::MockSurface>(surface_id);
        wl_surface_send_enter(surface->resource(), output2->resource());
    });
...
TEST_P(WaylandScreenTest, MultipleOutputsAddedAndRemoved)
{
    ...
    const int64_t old_primary_display_id = platform_screen_->GetPrimaryDisplay().id();
    PostToServerAndWait([&output1](wl::TestWaylandServerThread* server) { output1 = server->output(); });
    ...
    EXPECT_EQ(platform_screen_->GetOutputIdForDisplayId(removed_display_id), WaylandOutput::Id(0));
...
TEST_P(WaylandScreenTest, OutputPropertyChangesMissingLogicalSize)
{
    ...
    platform_screen_->OnOutputAddedOrUpdated(
        { output_id, display_id, origin, gfx::Size(), physical_size, insets, overscan_insets, scale, panel_transform, logical_transform, "display" });
...
TEST_P(WaylandScreenTest, GetAcceleratedWidgetAtScreenPoint)
{
    ...
    // Wayland.
    gfx::AcceleratedWidget widget_at_screen_point = platform_screen_->GetAcceleratedWidgetAtScreenPoint(gfx::Point(10, 10));
    ...
    widget_at_screen_point = platform_screen_->GetAcceleratedWidgetAtScreenPoint(gfx::Point(10, 10));
    ...
    widget_at_screen_point = platform_screen_->GetAcceleratedWidgetAtScreenPoint(gfx::Point(window_bounds.width() + 1, window_bounds.height() + 1));
    EXPECT_EQ(widget_at_screen_point, gfx::kNullAcceleratedWidget);

    MockWaylandPlatformWindowDelegate delegate;
    auto menu_window_bounds = gfx::Rect(window_->GetBoundsInDIP().width() - 10, window_->GetBoundsInDIP().height() - 10, 100, 100);
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithProperties(menu_window_bounds, PlatformWindowType::kMenu, window_->GetWidget(), &delegate);
    ...
    widget_at_screen_point
        = platform_screen_->GetAcceleratedWidgetAtScreenPoint(gfx::Point(menu_window->GetBoundsInDIP().x() + 1, menu_window->GetBoundsInDIP().y() + 1));
    ...
    SetPointerFocusedWindow(window_.get());
    widget_at_screen_point = platform_screen_->GetAcceleratedWidgetAtScreenPoint(gfx::Point(0, 0));
    EXPECT_EQ(widget_at_screen_point, window_->GetWidget());
    ...
    SetPointerFocusedWindow(menu_window.get());
    widget_at_screen_point = platform_screen_->GetAcceleratedWidgetAtScreenPoint(point_in_screen);
    EXPECT_EQ(widget_at_screen_point, menu_window->GetWidget());
}
...
TEST_P(WaylandScreenTest, GetLocalProcessWidgetAtPoint)
{
    ...
    EXPECT_EQ(platform_screen_->GetLocalProcessWidgetAtPoint(point, {}), gfx::kNullAcceleratedWidget);

    // Set a focus to the main window. Now, that focused window must be returned.
    SetPointerFocusedWindow(window_.get());
    EXPECT_EQ(platform_screen_->GetLocalProcessWidgetAtPoint(point, {}), window_->GetWidget());

    // Null widget must be returned when the focused window is part of the
    // |ignore| list.
    gfx::AcceleratedWidget w = window_->GetWidget();
    EXPECT_EQ(platform_screen_->GetLocalProcessWidgetAtPoint(point, { w - 1, w, w + 1 }), gfx::kNullAcceleratedWidget);
}
...
TEST_P(WaylandScreenTest, GetDisplayMatching)
{
    ...
    const display::Display primary_display = platform_screen_->GetPrimaryDisplay();

    // This has to be stored on the client thread, but must be used only on the
    // server thread. Place it on the right side of the primary display.
    const auto outout2_bounds = gfx::Rect(primary_display.bounds().width(), 0, 1024, 768);
    wl::TestOutput* output2 = nullptr;
    PostToServerAndWait([&output2, &outout2_bounds](wl::TestWaylandServerThread* server) {
        output2 = server->CreateAndInitializeOutput(wl::TestOutputMetrics(outout2_bounds));
        ASSERT_TRUE(output2);
    });
    ...
    EXPECT_EQ(primary_display.id(), platform_screen_->GetDisplayMatching(gfx::Rect(0, 0, 100, 100)).id());
    EXPECT_EQ(second_display.id(), platform_screen_->GetDisplayMatching(gfx::Rect(1024, 0, 10, 10)).id());

    // More pixels on second display.
    EXPECT_EQ(second_display.id(), platform_screen_->GetDisplayMatching(gfx::Rect(1020, 0, 10, 10)).id());

    // More pixels on first display.
    EXPECT_EQ(primary_display.id(), platform_screen_->GetDisplayMatching(gfx::Rect(1018, 0, 10, 10)).id());

    // Half pixels on second and half on primary.
    EXPECT_EQ(primary_display.id(), platform_screen_->GetDisplayMatching(gfx::Rect(1019, 0, 10, 10)).id());

    // Place second display 700 pixels below along y axis (1024:700,1024x768)
    PostToServerAndWait([&output2, &outout2_bounds](wl::TestWaylandServerThread* server) {
        output2->SetOrigin(gfx::Point(outout2_bounds.x(), outout2_bounds.y() + 700));
        output2->SetLogicalOrigin(gfx::Point(outout2_bounds.x(), outout2_bounds.y() + 700));
        output2->Flush();
    });

    // The match rect is located outside the displays. Primary display must be
    // returned.
    EXPECT_EQ(primary_display.id(), platform_screen_->GetDisplayMatching(gfx::Rect(1024, 0, 10, 10)).id());

    // At least some of the pixels are located on the display.
    EXPECT_EQ(primary_display.id(), platform_screen_->GetDisplayMatching(gfx::Rect(1023, 0, 10, 10)).id());

    // Most of pixels are located on second display.
    EXPECT_EQ(second_display.id(), platform_screen_->GetDisplayMatching(gfx::Rect(1023, 695, 10, 10)).id());

    // Empty rect results in primary display.
    EXPECT_EQ(primary_display.id(), platform_screen_->GetDisplayMatching(gfx::Rect(0, 0, 0, 0)).id());
...
TEST_P(WaylandScreenTest, GetPrimaryDisplayAfterRemoval)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { server->output()->DestroyGlobal(); });
...
TEST_P(WaylandScreenTest, GetDisplayForAcceleratedWidget)
{
    ...
    // the primary display.
    const gfx::Rect output2_bounds = gfx::Rect(primary_display.bounds().width(), 0, 1024, 768);
    wl::TestOutput* output2 = nullptr;
    PostToServerAndWait([&output2, &output2_bounds](wl::TestWaylandServerThread* server) {
        output2 = server->CreateAndInitializeOutput(wl::TestOutputMetrics(output2_bounds));
        ASSERT_TRUE(output2);
    });
    ...
    PostToServerAndWait([output2, surface_id](wl::TestWaylandServerThread* server) {
        wl_surface_send_enter(server->GetObject<wl::MockSurface>(surface_id)->resource(), output2->resource());
    });
    ...
    PostToServerAndWait([surface_id](wl::TestWaylandServerThread* server) {
        wl_surface_send_leave(server->GetObject<wl::MockSurface>(surface_id)->resource(), server->output()->resource());
    });
...
TEST_P(WaylandScreenTest, GetCursorScreenPoint)
{
    ...
    std::unique_ptr<WaylandWindow> second_window
        = CreateWaylandWindowWithProperties(gfx::Rect(0, 0, 1920, 1080), PlatformWindowType::kWindow, gfx::kNullAcceleratedWidget, &delegate);
    ...
    PostToServerAndWait([surface_id](wl::TestWaylandServerThread* server) {
        ...
        // client side.
        wl_seat_send_capabilities(server->seat()->resource(), WL_SEAT_CAPABILITY_POINTER);
    });
    ...
    PostToServerAndWait([surface_id](wl::TestWaylandServerThread* server) {
        ...
        auto* surface = server->GetObject<wl::MockSurface>(surface_id);
        wl_pointer_send_enter(pointer->resource(), server->GetNextSerial(), surface->resource(), 0, 0);
        wl_pointer_send_frame(pointer->resource());
        wl_pointer_send_motion(pointer->resource(), server->GetNextTime(), wl_fixed_from_int(10), wl_fixed_from_int(20));
        wl_pointer_send_frame(pointer->resource());
    });
    ...
    const uint32_t second_surface_id = second_window->root_surface()->get_surface_id();
    PostToServerAndWait([surface_id, second_surface_id](wl::TestWaylandServerThread* server) {
        wl::MockPointer* pointer = server->seat()->pointer();
        auto* surface = server->GetObject<wl::MockSurface>(surface_id);
        auto* second_surface = server->GetObject<wl::MockSurface>(second_surface_id);
        ASSERT_TRUE(second_surface);
        // Now, leave the first surface and enter second one.
        wl_pointer_send_leave(pointer->resource(), server->GetNextSerial(), surface->resource());
        wl_pointer_send_frame(pointer->resource());
        wl_pointer_send_enter(pointer->resource(), server->GetNextSerial(), second_surface->resource(), 0, 0);
        wl_pointer_send_frame(pointer->resource());
        wl_pointer_send_motion(pointer->resource(), server->GetNextTime(), wl_fixed_from_int(20), wl_fixed_from_int(10));
        wl_pointer_send_frame(pointer->resource());
    });
    ...
    PostToServerAndWait([second_surface_id](wl::TestWaylandServerThread* server) {
        wl::MockPointer* pointer = server->seat()->pointer();
        auto* second_surface = server->GetObject<wl::MockSurface>(second_surface_id);
        wl_pointer_send_leave(pointer->resource(), server->GetNextSerial(), second_surface->resource());
        wl_pointer_send_frame(pointer->resource());
    });
    ...
    EXPECT_EQ(gfx::Point(second_window_bounds.width() + 10, second_window_bounds.height() + 10), platform_screen_->GetCursorScreenPoint());

    // Create a menu window now and ensure cursor position is always sent in
    // regards to that window bounds.
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithProperties(gfx::Rect(second_window_bounds.width() - 10, second_window_bounds.height() - 10, 10, 20), PlatformWindowType::kMenu,
            second_window->GetWidget(), &delegate);
    ...
    PostToServerAndWait([menu_surface_id](wl::TestWaylandServerThread* server) {
        ...
        wl_pointer_send_enter(pointer->resource(), server->GetNextSerial(), menu_surface->resource(), 0, 0);
        wl_pointer_send_frame(pointer->resource());
        wl_pointer_send_motion(pointer->resource(), server->GetNextTime(), wl_fixed_from_int(2), wl_fixed_from_int(1));
        wl_pointer_send_frame(pointer->resource());
    });
    ...
    PostToServerAndWait([menu_surface_id, second_surface_id](wl::TestWaylandServerThread* server) {
        auto* menu_surface = server->GetObject<wl::MockSurface>(menu_surface_id);
        ASSERT_TRUE(menu_surface);
        auto* second_surface = server->GetObject<wl::MockSurface>(second_surface_id);

        wl::MockPointer* pointer = server->seat()->pointer();
        wl_pointer_send_leave(pointer->resource(), server->GetNextSerial(), menu_surface->resource());
        wl_pointer_send_frame(pointer->resource());
        wl_pointer_send_enter(pointer->resource(), server->GetNextSerial(), second_surface->resource(), 0, 0);
        wl_pointer_send_frame(pointer->resource());
        wl_pointer_send_motion(pointer->resource(), server->GetNextTime(), wl_fixed_from_int(1912), wl_fixed_from_int(1071));
        wl_pointer_send_frame(pointer->resource());
    });
    ...
    std::unique_ptr<WaylandWindow> nested_menu_window
        = CreateWaylandWindowWithProperties(gfx::Rect(menu_window_bounds.x() + menu_window_bounds.width(), menu_window_bounds.y() + 2, 10, 20),
            PlatformWindowType::kMenu, second_window->GetWidget(), &delegate);

    const uint32_t nested_menu_surface_id = nested_menu_window->root_surface()->get_surface_id();
    PostToServerAndWait([nested_menu_surface_id](wl::TestWaylandServerThread* server) {
        wl::MockPointer* pointer = server->seat()->pointer();
        auto* nested_menu_surface = server->GetObject<wl::MockSurface>(nested_menu_surface_id);
        ASSERT_TRUE(nested_menu_surface);

        wl_pointer_send_enter(pointer->resource(), server->GetNextSerial(), nested_menu_surface->resource(), 0, 0);
        wl_pointer_send_frame(pointer->resource());
        wl_pointer_send_motion(pointer->resource(), server->GetNextTime(), wl_fixed_from_int(2), wl_fixed_from_int(3));
        wl_pointer_send_frame(pointer->resource());
    });
    ...
    PostToServerAndWait([nested_menu_surface_id, menu_surface_id](wl::TestWaylandServerThread* server) {
        wl::MockPointer* pointer = server->seat()->pointer();
        auto* nested_menu_surface = server->GetObject<wl::MockSurface>(nested_menu_surface_id);
        auto* menu_surface = server->GetObject<wl::MockSurface>(menu_surface_id);
        wl_pointer_send_leave(pointer->resource(), server->GetNextSerial(), nested_menu_surface->resource());
        wl_pointer_send_frame(pointer->resource());
        wl_pointer_send_enter(pointer->resource(), server->GetNextSerial(), menu_surface->resource(), 0, 0);
        wl_pointer_send_frame(pointer->resource());
        wl_pointer_send_motion(pointer->resource(), server->GetNextTime(), wl_fixed_from_int(2), wl_fixed_from_int(1));
...
TEST_P(WaylandScreenTest, SetWindowScale)
{
    ...
    PostToServerAndWait([surface_id](wl::TestWaylandServerThread* server) {
        ...
        wl_surface_send_enter(server->GetObject<wl::MockSurface>(surface_id)->resource(), output->resource());
    ...
    command_line.GetProcessCommandLine()->AppendSwitchASCII(switches::kForceDeviceScaleFactor, base::StringPrintf("%.1f", kForcedUIScale));
...
{
    constexpr std::pair<wl_output_transform, display::Display::Rotation> kTestData[] = {
    ...
    for (const auto& [transform, expected_rotation] : kTestData) {
        PostToServerAndWait([new_transform = transform](wl::TestWaylandServerThread* server) {
            auto* output = server->output();
...
TEST_P(WaylandScreenTest, DualOutput)
{
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        output_1 = server_.CreateAndInitializeOutput(wl::TestOutputMetrics({ 800, 0, 800, 600 }));
        output_2 = server_.CreateAndInitializeOutput(wl::TestOutputMetrics({ 1600, 0, 800, 600 }));
    ...
    // after output_1.
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) { output_2->Flush(); });
    ...
    // before output_2.
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) { output_1->Flush(); });
...
TEST_P(WaylandScreenTest, OutputStateIsConsistentWhenNotifyingObservers)
{
    ...
    TestDisplayObserver observer;
    observer.set_display_metrics_changed_closure(base::BindLambdaForTesting([&]() { EXPECT_TRUE(platform_screen_->VerifyOutputStateConsistentForTesting()); }));
    platform_screen_->AddObserver(&observer);
...
TEST_P(WaylandAuraShellScreenTest, OutputPropertyChanges)
{
    ...
    uint32_t changed_values = display::DisplayObserver::DISPLAY_METRIC_BOUNDS | display::DisplayObserver::DISPLAY_METRIC_WORK_AREA;
    ...
    const gfx::Size scaled_logical_size = gfx::ScaleToRoundedSize(kPhysicalBounds.size(), 1.f / kNewScaleValue);
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        auto* output = server->output();
    ...
    changed_values = display::DisplayObserver::DISPLAY_METRIC_DEVICE_SCALE_FACTOR | display::DisplayObserver::DISPLAY_METRIC_WORK_AREA
    ...
    changed_values = display::DisplayObserver::DISPLAY_METRIC_WORK_AREA | display::DisplayObserver::DISPLAY_METRIC_BOUNDS
    ...
    EXPECT_EQ(observer.GetDisplay().panel_rotation(), display::Display::Rotation::ROTATE_270);
    EXPECT_EQ(observer.GetDisplay().rotation(), display::Display::Rotation::ROTATE_270);
...
TEST_P(WaylandAuraShellScreenTest, OutputPropertyChangesWithPortraitPanelRotation)
{
    TestDisplayObserver observer;
    platform_screen_->AddObserver(&observer);
    ...
    const gfx::Size scaled_logical_size = gfx::ScaleToRoundedSize(kPhysicalSize, 0.5);
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        auto* output = server->output();
        output->SetLogicalSize(scaled_logical_size);
    ...
    uint32_t changed_values = display::DisplayObserver::DISPLAY_METRIC_BOUNDS | display::DisplayObserver::DISPLAY_METRIC_WORK_AREA
        | display::DisplayObserver::DISPLAY_METRIC_DEVICE_SCALE_FACTOR | display::DisplayObserver::DISPLAY_METRIC_ROTATION;
    EXPECT_EQ(observer.GetAndClearChangedMetrics(), changed_values);
    ...
    EXPECT_EQ(observer.GetDisplay().panel_rotation(), display::Display::Rotation::ROTATE_270);
    EXPECT_EQ(observer.GetDisplay().rotation(), display::Display::Rotation::ROTATE_0);
    ...
    changed_values = display::DisplayObserver::DISPLAY_METRIC_BOUNDS | display::DisplayObserver::DISPLAY_METRIC_WORK_AREA
        | display::DisplayObserver::DISPLAY_METRIC_ROTATION;
    EXPECT_EQ(observer.GetAndClearChangedMetrics(), changed_values);
    ...
    EXPECT_EQ(observer.GetDisplay().panel_rotation(), display::Display::Rotation::ROTATE_180);
    EXPECT_EQ(observer.GetDisplay().rotation(), display::Display::Rotation::ROTATE_270);
...
TEST_P(WaylandAuraShellScreenTest, UseCorrectScreenBeforeEnterEvent)
{
    ...
    PostToServerAndWait([&output1, &output2](wl::TestWaylandServerThread* server) {
        output2 = server->CreateAndInitializeOutput(wl::TestOutputMetrics({ GetRightX(output1), 0, 800, 600 }));
        // Scale Factor 2.
    ...
    window_ = delegate_.CreateWaylandWindow(connection_.get(), std::move(properties));
...
#if !BUILDFLAG(IS_CHROMEOS_LACROS)
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandScreenTest, Values(wl::ServerConfig {}));

#else

INSTANTIATE_TEST_SUITE_P(
    XdgVersionStableTestWithAuraShell, WaylandScreenTest, Values(wl::ServerConfig { .enable_aura_shell = wl::EnableAuraShellProtocol::kEnabled }));

INSTANTIATE_TEST_SUITE_P(
    XdgVersionStableTest, WaylandAuraShellScreenTest, Values(wl::ServerConfig { .enable_aura_shell = wl::EnableAuraShellProtocol::kEnabled }));
