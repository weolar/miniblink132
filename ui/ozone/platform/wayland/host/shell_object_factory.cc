...
std::unique_ptr<ShellToplevelWrapper> ShellObjectFactory::CreateShellToplevelWrapper(WaylandConnection* connection, WaylandWindow* wayland_window)
{
    if (!connection->shell()) {
    ...
    auto surface = std::make_unique<XDGSurfaceWrapperImpl>(wayland_window, connection);
    if (!surface->Initialize())
        return nullptr;

    auto toplevel = std::make_unique<XDGToplevelWrapperImpl>(std::move(surface), wayland_window, connection);
...
std::unique_ptr<ShellPopupWrapper> ShellObjectFactory::CreateShellPopupWrapper(
    WaylandConnection* connection, WaylandWindow* wayland_window, const ShellPopupParams& params)
{
    if (!connection->shell()) {
        LOG(WARNING) << "Shell protocol is not available.";
        return nullptr;
    ...
    auto popup = std::make_unique<XDGPopupWrapperImpl>(std::move(surface), wayland_window, connection);
