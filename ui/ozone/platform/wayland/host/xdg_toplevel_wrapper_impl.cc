...
static_assert(sizeof(uint32_t) == sizeof(float), "Sizes much match for reinterpret cast to be meaningful");
...
std::optional<wl::Serial> GetSerialForMoveResize(WaylandConnection* connection)
{
    return connection->serial_tracker().GetSerial({ wl::SerialType::kTouchPress, wl::SerialType::kMousePress, wl::SerialType::kKeyPress });
}

zaura_toplevel_z_order_level ToZauraToplevelZOrderLevel(ZOrderLevel z_order_level)
{
...
XDGToplevelWrapperImpl::XDGToplevelWrapperImpl(std::unique_ptr<XDGSurfaceWrapperImpl> surface, WaylandWindow* wayland_window, WaylandConnection* connection)
...
bool XDGToplevelWrapperImpl::Initialize()
{
    ...
    xdg_toplevel_.reset(xdg_surface_get_toplevel(xdg_surface_wrapper_->xdg_surface()));
    if (!xdg_toplevel_) {
    ...
    if (connection_->zaura_shell()) {
        uint32_t version = zaura_shell_get_version(connection_->zaura_shell()->wl_object());
        if (version >= ZAURA_SHELL_GET_AURA_TOPLEVEL_FOR_XDG_TOPLEVEL_SINCE_VERSION) {
            aura_toplevel_.reset(zaura_shell_get_aura_toplevel_for_xdg_toplevel(connection_->zaura_shell()->wl_object(), xdg_toplevel_.get()));
        }
    }
...
{
    return aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= version;
}

void XDGToplevelWrapperImpl::SetCanMaximize(bool can_maximize)
{
    if (!aura_toplevel_ || zaura_toplevel_get_version(aura_toplevel_.get()) < ZAURA_TOPLEVEL_SET_CAN_MAXIMIZE_SINCE_VERSION) {
        return;
    }
...
{
    if (!aura_toplevel_ || zaura_toplevel_get_version(aura_toplevel_.get()) < ZAURA_TOPLEVEL_SET_CAN_FULLSCREEN_SINCE_VERSION) {
        return;
    }
...
{
    DCHECK(xdg_toplevel_);
    xdg_toplevel_set_fullscreen(xdg_toplevel_.get(), wayland_output ? wayland_output->get_output() : nullptr);
}
...
void XDGToplevelWrapperImpl::SurfaceMove(WaylandConnection* connection)
{
    ...
    if (auto serial = GetSerialForMoveResize(connection)) {
        xdg_toplevel_move(xdg_toplevel_.get(), connection->seat()->wl_object(), serial->value);
    }
}

void XDGToplevelWrapperImpl::SurfaceResize(WaylandConnection* connection, uint32_t hittest)
{
    DCHECK(xdg_toplevel_);
    if (auto serial = GetSerialForMoveResize(connection)) {
        xdg_toplevel_resize(xdg_toplevel_.get(), connection->seat()->wl_object(), serial->value, wl::IdentifyDirection(hittest));
    }
}
...
// static
void XDGToplevelWrapperImpl::OnToplevelConfigure(void* data, xdg_toplevel* toplevel, int32_t width, int32_t height, wl_array* states)
{
    auto* self = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(self);

    WaylandWindow::WindowStates window_states;
    window_states.is_maximized = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_MAXIMIZED);
    window_states.is_fullscreen = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_FULLSCREEN);
    window_states.is_activated = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_ACTIVATED);

    if (xdg_toplevel_get_version(toplevel) >= XDG_TOPLEVEL_STATE_SUSPENDED_SINCE_VERSION) {
        window_states.is_suspended = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_SUSPENDED);
    }
    if (xdg_toplevel_get_version(toplevel) >= XDG_TOPLEVEL_STATE_TILED_LEFT_SINCE_VERSION) {
        // All four tiled states have the same since version, so it is enough to
        // check only one.
        window_states.tiled_edges = { .left = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_TILED_LEFT),
            ...
            .bottom = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_TILED_BOTTOM) };
    }
...
// static
void XDGToplevelWrapperImpl::OnToplevelClose(void* data, xdg_toplevel* toplevel)
{
    auto* self = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(self);
...
// static
void XDGToplevelWrapperImpl::OnConfigureBounds(void* data, xdg_toplevel* toplevel, int32_t width, int32_t height)
{
    NOTIMPLEMENTED_LOG_ONCE();
}

// static
void XDGToplevelWrapperImpl::OnWmCapabilities(void* data, xdg_toplevel* toplevel, wl_array* capabilities)
{
    NOTIMPLEMENTED_LOG_ONCE();
}

// static
void XDGToplevelWrapperImpl::OnDecorationConfigure(void* data, zxdg_toplevel_decoration_v1* decoration, uint32_t mode)
{
    auto* self = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(self);
...
void XDGToplevelWrapperImpl::OnAuraToplevelConfigure(
    void* data, zaura_toplevel* aura_toplevel, int32_t x, int32_t y, int32_t width, int32_t height, wl_array* states)
{
    auto* self = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(self);
    ...
    window_states.is_fullscreen = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_FULLSCREEN);
    window_states.is_activated = CheckIfWlArrayHasValue(states, XDG_TOPLEVEL_STATE_ACTIVATED);
    window_states.is_minimized = CheckIfWlArrayHasValue(states, ZAURA_TOPLEVEL_STATE_MINIMIZED);
    window_states.is_snapped_primary = CheckIfWlArrayHasValue(states, ZAURA_TOPLEVEL_STATE_SNAPPED_PRIMARY);
    window_states.is_snapped_secondary = CheckIfWlArrayHasValue(states, ZAURA_TOPLEVEL_STATE_SNAPPED_SECONDARY);
    window_states.is_floated = CheckIfWlArrayHasValue(states, ZAURA_TOPLEVEL_STATE_FLOATED);
    window_states.is_pip = CheckIfWlArrayHasValue(states, ZAURA_TOPLEVEL_STATE_PIP);

    self->wayland_window_->HandleToplevelConfigureWithOrigin(x, y, width, height, window_states);
}

// static
void XDGToplevelWrapperImpl::OnOriginChange(void* data, zaura_toplevel* aura_toplevel, int32_t x, int32_t y)
{
    auto* self = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(self);
    auto* toplevel_window = static_cast<WaylandToplevelWindow*>(self->wayland_window_);
    toplevel_window->SetOrigin(gfx::Point(x, y));
}

// static
void XDGToplevelWrapperImpl::OnConfigureRasterScale(void* data, zaura_toplevel* aura_toplevel, uint32_t scale_as_uint)
{
    auto* self = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(self);
...
// static
void XDGToplevelWrapperImpl::OnRotateFocus(void* data, zaura_toplevel* aura_toplevel, uint32_t serial, uint32_t direction, uint32_t restart)
{
    auto* self = static_cast<XDGToplevelWrapperImpl*>(data);
    DCHECK(self);
    auto* toplevel_window = static_cast<WaylandToplevelWindow*>(self->wayland_window_);
...
// static
void XDGToplevelWrapperImpl::OnOverviewChange(void* data, zaura_toplevel* aura_toplevel, uint32_t in_overview_as_uint)
{
    auto* self = static_cast<XDGToplevelWrapperImpl*>(data);
    CHECK(self);
    self->wayland_window_->AsWaylandToplevelWindow()->OnOverviewChange(in_overview_as_uint);
}

// static
void XDGToplevelWrapperImpl::OnConfigureOcclusionState(void* data, struct zaura_toplevel* zaura_toplevel, uint32_t mode)
{
    ...
    auto occlusion_state = WaylandOcclusionStateToPlatformWindowOcclusionState(mode);
    wayland_window->SetPendingOcclusionState(occlusion_state);
}

void XDGToplevelWrapperImpl::SetTopLevelDecorationMode(DecorationMode requested_mode)
{
    if (!zxdg_toplevel_decoration_ || requested_mode == decoration_mode_) {
        return;
    }

    zxdg_toplevel_decoration_v1_set_mode(zxdg_toplevel_decoration_.get(), ToInt32(requested_mode));
}
...
void XDGToplevelWrapperImpl::InitializeXdgDecoration()
{
    if (connection_->xdg_decoration_manager_v1()) {
        ...
        zxdg_toplevel_decoration_.reset(zxdg_decoration_manager_v1_get_toplevel_decoration(connection_->xdg_decoration_manager_v1(), xdg_toplevel_.get()));

        static constexpr zxdg_toplevel_decoration_v1_listener kToplevelDecorationListener = {
            .configure = &OnDecorationConfigure,
        };
        zxdg_toplevel_decoration_v1_add_listener(zxdg_toplevel_decoration_.get(), &kToplevelDecorationListener, this);
    }
}

wl::Object<wl_region> XDGToplevelWrapperImpl::CreateAndAddRegion(const std::vector<gfx::Rect>& shape)
{
...
zaura_toplevel_orientation_lock ToZauraSurfaceOrientationLock(WaylandOrientationLockType lock_type)
{
    switch (lock_type) {
...
{
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_ORIENTATION_LOCK_SINCE_VERSION) {
        zaura_toplevel_set_orientation_lock(aura_toplevel_.get(), ToZauraSurfaceOrientationLock(lock_type));
    }
}
...
{
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_ORIENTATION_LOCK_SINCE_VERSION) {
        zaura_toplevel_set_orientation_lock(aura_toplevel_.get(), ZAURA_TOPLEVEL_ORIENTATION_LOCK_NONE);
    }
}

void XDGToplevelWrapperImpl::RequestWindowBounds(const gfx::Rect& bounds, int64_t display_id)
{
    const WaylandOutputManager* manager = connection_->wayland_output_manager();

    WaylandOutput* target_output = nullptr;
    if (display_id != display::kInvalidDisplayId) {
        auto output_id_for_display_id = manager->wayland_screen()->GetOutputIdForDisplayId(display_id);
        // the output for the valid display_id should exist.
        LOG_IF(WARNING, !output_id_for_display_id) << "No output found for display id:" << display_id;

        target_output = manager->GetOutput(output_id_for_display_id);
    }

    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_WINDOW_BOUNDS_SINCE_VERSION) {
        zaura_toplevel_set_window_bounds(
            aura_toplevel_.get(), bounds.x(), bounds.y(), bounds.width(), bounds.height(), target_output ? target_output->get_output() : nullptr);
    }
}

void XDGToplevelWrapperImpl::SetSystemModal(bool modal)
{
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_SYSTEM_MODAL_SINCE_VERSION) {
        if (modal) {
...
{
    return aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_SUPPORTS_SCREEN_COORDINATES_SINCE_VERSION;
}
...
void XDGToplevelWrapperImpl::EnableScreenCoordinates()
{
    ...
    static constexpr zaura_toplevel_listener kAuraToplevelListener = { .configure = &OnAuraToplevelConfigure,
    ...
    zaura_toplevel_add_listener(aura_toplevel_.get(), &kAuraToplevelListener, this);
}
#endif

void XDGToplevelWrapperImpl::SetZOrder(ZOrderLevel z_order)
{
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_Z_ORDER_SINCE_VERSION) {
        zaura_toplevel_set_z_order(aura_toplevel_.get(), ToZauraToplevelZOrderLevel(z_order));
    }
}

bool XDGToplevelWrapperImpl::SupportsActivation()
{
    static_assert(ZAURA_TOPLEVEL_ACTIVATE_SINCE_VERSION == ZAURA_TOPLEVEL_DEACTIVATE_SINCE_VERSION,
        "Support for activation and deactivation was added in the same version.");
    return aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_ACTIVATE_SINCE_VERSION;
}
...
{
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_SCALE_FACTOR_SINCE_VERSION) {
        uint32_t value = base::bit_cast<uint32_t>(scale_factor);
        zaura_toplevel_set_scale_factor(aura_toplevel_.get(), value);
    }
}

void XDGToplevelWrapperImpl::SetFloatToLocation(WaylandFloatStartLocation float_start_location)
{
    if (!aura_toplevel_) {
        return;
    }
...
{
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_UNSET_FLOAT_SINCE_VERSION) {
        zaura_toplevel_unset_float(aura_toplevel_.get());
    }
}

void XDGToplevelWrapperImpl::CommitSnap(WaylandWindowSnapDirection snap_direction, float snap_ratio)
{
    if (!aura_toplevel_) {
        return;
    }

    if (zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_UNSET_SNAP_SINCE_VERSION && snap_direction == WaylandWindowSnapDirection::kNone) {
        zaura_toplevel_unset_snap(aura_toplevel_.get());
        return;
    }

    if (zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_SNAP_PRIMARY_SINCE_VERSION) {
...
{
    auto persistable_enum = persistable ? ZAURA_TOPLEVEL_PERSISTABLE_PERSISTABLE : ZAURA_TOPLEVEL_PERSISTABLE_NOT_PERSISTABLE;

    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_PERSISTABLE_SINCE_VERSION) {
...
{
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_SET_SHAPE_SINCE_VERSION) {
        zaura_toplevel_set_shape(aura_toplevel_.get(), shape_rects ? CreateAndAddRegion(*shape_rects).get() : nullptr);
    }
}

void XDGToplevelWrapperImpl::ShowSnapPreview(WaylandWindowSnapDirection snap_direction, bool allow_haptic_feedback)
{
    if (aura_toplevel_ && zaura_toplevel_get_version(aura_toplevel_.get()) >= ZAURA_TOPLEVEL_INTENT_TO_SNAP_SINCE_VERSION) {
        ...
        zaura_toplevel_intent_to_snap(aura_toplevel_.get(), zaura_shell_snap_direction);
        return;
    }
}
...
void XDGToplevelWrapperImpl::SetIcon(const gfx::ImageSkia& icon)
{
    ...
    if (icon.isNull()) {
        xdg_toplevel_icon_manager_v1_set_icon(manager, xdg_toplevel_.get(), nullptr);
        return;
    }
