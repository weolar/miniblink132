...
constexpr auto kMouseButtonToStringMap = base::MakeFixedFlatMap<int, const char*>({
...
{
    return (flags & (EF_LEFT_MOUSE_BUTTON | EF_MIDDLE_MOUSE_BUTTON | EF_RIGHT_MOUSE_BUTTON | EF_BACK_MOUSE_BUTTON | EF_FORWARD_MOUSE_BUTTON)) != 0;
}

std::vector<uint8_t> ToLittleEndianByteVector(uint32_t value)
{
    return { static_cast<uint8_t>(value), static_cast<uint8_t>(value >> 8), static_cast<uint8_t>(value >> 16), static_cast<uint8_t>(value >> 24) };
}
...
{
    auto* root_window = static_cast<WaylandWindow*>(GetRootTarget(event->target()));
    return event->root_location() + root_window->GetBoundsInDIP().origin().OffsetFromOrigin();
}
...
WaylandEventSource::TouchPoint::TouchPoint(gfx::PointF location, WaylandWindow* current_window)
...
WaylandEventSource::PointerScrollData& WaylandEventSource::PointerScrollData::operator=(const PointerScrollData&) = default;
...
void WaylandEventSource::PointerScrollData::DumpState(std::ostream& out) const
{
    ...
    out << ", d=(" << dx << ", " << dy << "), dt=" << dt << ", is_axis_stop=" << ToBoolString(is_axis_stop);
}

// WaylandEventSource::FrameData implementation
WaylandEventSource::FrameData::FrameData(const Event& e, base::OnceCallback<void()> cb)
    : event(e.Clone())
...
{
    out << "event=" << (event ? event->ToString() : "none") << ", callback=" << !!completion_cb;
}

// WaylandEventSource implementation

// static
void WaylandEventSource::ConvertEventToTarget(EventTarget* new_target, LocatedEvent* event)
{
    ...
    auto diff = gfx::ScaleVector2d(GetOriginInScreen(current_target) - GetOriginInScreen(new_target_window), new_target_window->applied_state().ui_scale);
    event->set_location_f(event->location_f() + diff);
}

WaylandEventSource::WaylandEventSource(
    wl_display* display, wl_event_queue* event_queue, WaylandWindowManager* window_manager, WaylandConnection* connection, bool use_threaded_polling)
    : window_manager_(window_manager)
    , connection_(connection)
    , event_watcher_(WaylandEventWatcher::CreateWaylandEventWatcher(display, event_queue, use_threaded_polling))
{
...
void WaylandEventSource::OnKeyboardFocusChanged(WaylandWindow* window, bool focused)
{
    DCHECK(window);
#if DCHECK_IS_ON()
    if (!focused)
...
uint32_t WaylandEventSource::OnKeyboardKeyEvent(
    EventType type, DomCode dom_code, bool repeat, std::optional<uint32_t> serial, base::TimeTicks timestamp, int device_id, WaylandKeyboard::KeyEventKind kind)
{
    ...
    if (!layout_engine || !layout_engine->Lookup(dom_code, keyboard_modifiers_, &dom_key, &key_code)) {
    ...
    KeyEvent event(type, key_code, dom_code, keyboard_modifiers_ | (repeat ? EF_IS_REPEAT : 0), dom_key, timestamp);
    ...
    uint32_t converted_key_code = ui::KeycodeConverter::DomCodeToXkbKeycode(dom_code);
    properties.emplace(kPropertyKeyboardHwKeyCode, std::vector<uint8_t> { static_cast<unsigned char>(converted_key_code) });
    ...
    // conversion.
    properties.emplace(kPropertyKeyboardState, ToLittleEndianByteVector(state_before_event));
#endif

    if (serial.has_value()) {
        properties.emplace(WaylandKeyboard::kPropertyWaylandSerial, ToLittleEndianByteVector(serial.value()));
    }
...
void WaylandEventSource::OnSynthesizedKeyPressEvent(DomCode dom_code, base::TimeTicks timestamp)
{
    std::ignore = OnKeyboardKeyEvent(EventType::kKeyPressed, dom_code, /*repeat=*/false,
        /*serial=*/std::nullopt, timestamp,
        /*device_id=*/0, WaylandKeyboard::KeyEventKind::kKey);
}

void WaylandEventSource::OnPointerFocusChanged(
    WaylandWindow* window, const gfx::PointF& location, base::TimeTicks timestamp, wl::EventDispatchPolicy dispatch_policy)
{
    bool focused = !!window;
    if (focused) {
    ...
    auto closure = focused ? base::NullCallback() : base::BindOnce([](WaylandWindowManager* wwm) { wwm->SetPointerFocusedWindow(nullptr); }, window_manager_);

    auto* target = window_manager_->GetCurrentPointerFocusedWindow();
    if (target) {
        EventType type = focused ? EventType::kMouseEntered : EventType::kMouseExited;
        MouseEvent event(type, pointer_location_, pointer_location_, timestamp, pointer_flags_, 0);
        if (dispatch_policy == wl::EventDispatchPolicy::kImmediate) {
            SetTargetAndDispatchEvent(&event, target);
        } else {
            pointer_frames_.push_back(std::make_unique<FrameData>(event, std::move(closure)));
            return;
        }
    }
...
void WaylandEventSource::OnPointerButtonEvent(EventType type, int changed_button, base::TimeTicks timestamp, WaylandWindow* window,
    wl::EventDispatchPolicy dispatch_policy, bool allow_release_of_unpressed_button, bool is_synthesized)
{
    ...
    if (!allow_release_of_unpressed_button && type == EventType::kMouseReleased && (pointer_flags_ & changed_button) == 0) {
        return;
    }

    WaylandWindow* prev_focused_window = window_manager_->GetCurrentPointerFocusedWindow();
    if (window)
        window_manager_->SetPointerFocusedWindow(window);

    auto closure = base::BindOnce(&WaylandEventSource::OnPointerButtonEventInternal, base::Unretained(this), (window ? prev_focused_window : nullptr), type);

    pointer_flags_ = type == EventType::kMousePressed ? (pointer_flags_ | changed_button) : (pointer_flags_ & ~changed_button);
    ...
    if (target) {
        ...
        MouseEvent event(type, pointer_location_, pointer_location_, timestamp, flags, changed_button);
        if (dispatch_policy == wl::EventDispatchPolicy::kImmediate) {
            SetTargetAndDispatchEvent(&event, target);
        } else {
            pointer_frames_.push_back(std::make_unique<FrameData>(event, std::move(closure)));
...
void WaylandEventSource::OnPointerButtonEventInternal(WaylandWindow* window, EventType type)
{
    if (window)
        window_manager_->SetPointerFocusedWindow(window);
...
void WaylandEventSource::OnPointerMotionEvent(
    const gfx::PointF& location, base::TimeTicks timestamp, wl::EventDispatchPolicy dispatch_policy, bool is_synthesized)
{
    pointer_location_ = location;
    ...
    MouseEvent event(EventType::kMouseMoved, pointer_location_, pointer_location_, timestamp, flags, 0);
    auto* target = window_manager_->GetCurrentPointerFocusedWindow();
    ...
    if (dispatch_policy == wl::EventDispatchPolicy::kImmediate) {
        ...
        pointer_frames_.push_back(std::make_unique<FrameData>(event, base::NullCallback()));
    }
}

void WaylandEventSource::OnPointerAxisEvent(const gfx::Vector2dF& offset, base::TimeTicks timestamp)
{
...
void WaylandEventSource::DumpState(std::ostream& out) const
{
    ...
    out << "  pointer_location=" << pointer_location_.ToString() << ", flags=" << ToPointerFlagsString(pointer_flags_)
        << ", last button pressed=" << last_pointer_button_pressed_ << ", keyboard modifiers=" << ToKeyboardModifierStrings(keyboard_modifiers_) << std::endl;
    if (relative_pointer_location_) {
        out << "  relative_poniter_location=" << relative_pointer_location_->ToString() << std::endl;
    }
...
void WaylandEventSource::OnPointerFrameEvent()
{
    ...
    while (!pointer_frames_.empty()) {
        ...
        if (pointer_details_with_stylus_data && pointer_frame->event->IsMouseEvent() && pointer_frame->event->AsMouseEvent()->IsOnlyLeftMouseButton()) {
            auto old_event = std::move(pointer_frame->event);
            pointer_frame->event
                = std::make_unique<MouseEvent>(old_event->type(), old_event->AsMouseEvent()->location(), old_event->AsMouseEvent()->root_location(),
                    old_event->time_stamp(), old_event->flags(), old_event->AsMouseEvent()->changed_button_flags(), pointer_details_with_stylus_data.value());
        }
...
void WaylandEventSource::OnPointerAxisStopEvent(uint32_t axis, base::TimeTicks timestamp)
{
    EnsurePointerScrollData(timestamp);
...
void WaylandEventSource::OnTouchPressEvent(
    WaylandWindow* window, const gfx::PointF& location, base::TimeTicks timestamp, PointerId id, wl::EventDispatchPolicy dispatch_policy)
{
    DCHECK(window);
    HandleTouchFocusChange(window, true);

    // Make sure this touch point wasn't present before.
    auto success = touch_points_.try_emplace(id, std::make_unique<TouchPoint>(location, window));
    if (!success.second) {
    ...
    TouchEvent event(EventType::kTouchPressed, location, location, timestamp, details, keyboard_modifiers_);
    touch_frames_.push_back(std::make_unique<FrameData>(event, base::NullCallback()));
}

void WaylandEventSource::OnTouchReleaseEvent(base::TimeTicks timestamp, PointerId id, wl::EventDispatchPolicy dispatch_policy, bool is_synthesized)
{
    ...
    TouchEvent event(EventType::kTouchReleased, location, location, timestamp, details, flags);
    if (dispatch_policy == wl::EventDispatchPolicy::kImmediate) {
        SetTouchTargetAndDispatchTouchEvent(&event);
        OnTouchReleaseInternal(id);
    } else {
        touch_frames_.push_back(std::make_unique<FrameData>(event, base::BindOnce(&WaylandEventSource::OnTouchReleaseInternal, base::Unretained(this), id)));
    }
}
...
void WaylandEventSource::SetTargetAndDispatchEvent(Event* event, EventTarget* target)
{
...
void WaylandEventSource::SetTouchTargetAndDispatchTouchEvent(TouchEvent* event)
{
...
void WaylandEventSource::OnTouchMotionEvent(
    const gfx::PointF& location, base::TimeTicks timestamp, PointerId id, wl::EventDispatchPolicy dispatch_policy, bool is_synthesized)
{
    const auto it = touch_points_.find(id);
    // Make sure this touch point was present before.
    if (it == touch_points_.end()) {
    ...
    TouchEvent event(EventType::kTouchMoved, location, location, timestamp, details, flags);
    if (dispatch_policy == wl::EventDispatchPolicy::kImmediate) {
        SetTouchTargetAndDispatchTouchEvent(&event);
    } else {
        touch_frames_.push_back(std::make_unique<FrameData>(event, base::NullCallback()));
...
void WaylandEventSource::OnTouchCancelEvent()
{
    ...
    for (auto& touch_point : touch_points_) {
        ...
        TouchEvent event(EventType::kTouchCancelled, location, location, timestamp, PointerDetails(EventPointerType::kTouch, id));
        SetTouchTargetAndDispatchTouchEvent(&event);
...
void WaylandEventSource::OnTouchFrame()
{
    while (!touch_frames_.empty()) {
        ...
        auto pointer_details_with_stylus_data = AmendStylusData(touch_frame->event->AsTouchEvent()->pointer_details().id);
        if (pointer_details_with_stylus_data) {
            auto old_event = std::move(touch_frame->event);
            touch_frame->event = std::make_unique<TouchEvent>(old_event->type(), old_event->AsTouchEvent()->location_f(),
                old_event->AsTouchEvent()->root_location_f(), old_event->time_stamp(), pointer_details_with_stylus_data.value(), old_event->flags());
        }
...
void WaylandEventSource::OnTouchStylusToolChanged(PointerId pointer_id, EventPointerType pointer_type)
{
    StylusData stylus_data = { .type = pointer_type, .tilt = gfx::Vector2dF(), .force = std::numeric_limits<float>::quiet_NaN() };
    bool inserted = last_touch_stylus_data_.try_emplace(pointer_id, stylus_data).second;
    DCHECK(inserted);
}

void WaylandEventSource::OnTouchStylusForceChanged(PointerId pointer_id, float force)
{
    DCHECK(last_touch_stylus_data_[pointer_id].has_value());
    last_touch_stylus_data_[pointer_id]->force = force;
}

void WaylandEventSource::OnTouchStylusTiltChanged(PointerId pointer_id, const gfx::Vector2dF& tilt)
{
    DCHECK(last_touch_stylus_data_[pointer_id].has_value());
...
void WaylandEventSource::OnPinchEvent(
    EventType event_type, const gfx::Vector2dF& delta, base::TimeTicks timestamp, int device_id, std::optional<float> scale_delta)
{
    ...
    GestureEvent event(location.x(), location.y(), 0 /* flags */, timestamp, details);
    event.set_source_device_id(device_id);
...
void WaylandEventSource::OnHoldEvent(
    EventType event_type, uint32_t finger_count, base::TimeTicks timestamp, int device_id, wl::EventDispatchPolicy dispatch_policy)
{
    ...
    ScrollEvent event(EventType::kScrollFlingCancel, pointer_location_, pointer_location_, timestamp, pointer_flags_, 0, 0, 0, 0, finger_count);
...
void WaylandEventSource::OnRelativePointerMotion(const gfx::Vector2dF& delta, base::TimeTicks timestamp)
{
    DCHECK(relative_pointer_location_.has_value());
    relative_pointer_location_ = *relative_pointer_location_ + delta;
    OnPointerMotionEvent(*relative_pointer_location_, timestamp, wl::EventDispatchPolicy::kImmediate,
        /*is_sythesized=*/false);
}
...
void WaylandEventSource::ReleasePressedPointerButtons(WaylandWindow* window, base::TimeTicks timestamp)
{
    ...
    for (const auto& [button, name] : kMouseButtonToStringMap) {
        if (button & pointer_flags_) {
            ...
            OnPointerButtonEvent(EventType::kMouseReleased, button, timestamp, window, wl::EventDispatchPolicy::kImmediate,
...
void WaylandEventSource::OnPointerStylusToolChanged(EventPointerType pointer_type)
{
    ...
    last_pointer_stylus_data_ = { .type = pointer_type, .tilt = gfx::Vector2dF(), .force = std::numeric_limits<float>::quiet_NaN() };
}
...
void WaylandEventSource::OnPointerStylusForceChanged(float force)
{
    if (!last_pointer_stylus_data_.has_value()) {
        ...
        LOG(WARNING) << "Cannot handle force for the default tool!  (the value is " << force << ")";
        return;
    }
...
void WaylandEventSource::OnPointerStylusTiltChanged(const gfx::Vector2dF& tilt)
{
    if (!last_pointer_stylus_data_.has_value()) {
        // This is a stray tilt event that the default tool cannot accept.
        LOG(WARNING) << "Cannot handle tilt for the default tool!  (the value is [" << tilt.x() << "," << tilt.y() << "])";
        return;
    }
...
void WaylandEventSource::OnWindowRemoved(WaylandWindow* window)
{
    ...
    base::EraseIf(touch_points_, [window](const auto& point) { return point.second->window == window; });
}

void WaylandEventSource::HandleTouchFocusChange(WaylandWindow* window, bool focused, std::optional<PointerId> id)
{
    DCHECK(window);
...
bool WaylandEventSource::ShouldUnsetTouchFocus(WaylandWindow* win, PointerId id)
{
    return base::ranges::none_of(touch_points_, [win, id](auto& p) { return p.second->window == win && p.first != id; });
}
...
gfx::Vector2dF WaylandEventSource::ComputeFlingVelocity()
{
    ...
    if (count == 1) {
        ...
        const float dt = pointer_frame.dt.InSecondsF() + pointer_scroll_data_->dt.InSecondsF();
    ...
    for (const auto& frame : pointer_scroll_data_set_) {
        if (frame.axis_source && *frame.axis_source != WL_POINTER_AXIS_SOURCE_FINGER) {
            break;
        }
    ...
    return gfx::Vector2dF((count * sums.tx_ - sums.t_ * sums.x_) * det_inv, (count * sums.ty_ - sums.t_ * sums.y_) * det_inv);
}
...
std::optional<PointerDetails> WaylandEventSource::AmendStylusData() const
{
    ...
    return PointerDetails(last_pointer_stylus_data_->type, /*pointer_id=*/0,
        ...
        /*twist=*/0.0f, last_pointer_stylus_data_->tilt.x(), last_pointer_stylus_data_->tilt.y());
}

std::optional<PointerDetails> WaylandEventSource::AmendStylusData(PointerId pointer_id) const
{
    const auto it = last_touch_stylus_data_.find(pointer_id);
    if (it == last_touch_stylus_data_.end() || !it->second || it->second->type == EventPointerType::kTouch) {
        return std::nullopt;
    }
    ...
    return PointerDetails(it->second->type, pointer_id,
        ...
        /*twist=*/0.0f, it->second->tilt.x(), it->second->tilt.y());
}

void WaylandEventSource::EnsurePointerScrollData(const std::optional<base::TimeTicks>& timestamp)
{
    if (!pointer_scroll_data_)
...
#if BUILDFLAG(IS_CHROMEOS_LACROS)
        ScrollEvent event(pointer_scroll_data_->is_axis_stop ? EventType::kScrollFlingStart : EventType::kScrollFlingCancel, pointer_location_,
            pointer_location_, timestamp, flags, vx, vy, vx, vy, kGestureScrollFingerCount);
#else
        // In Linux there is no axis event with 0 delta when start scrolling.
        // A fling is therefore always started at this point.
        ScrollEvent event(EventType::kScrollFlingStart, pointer_location_, pointer_location_, timestamp, flags, vx, vy, vx, vy, kGestureScrollFingerCount);
        is_fling_active_ = true;
#endif
        pointer_frames_.push_back(std::make_unique<FrameData>(event, base::NullCallback()));
    } else if (pointer_scroll_data_->axis_source) {
        if (*pointer_scroll_data_->axis_source == WL_POINTER_AXIS_SOURCE_WHEEL || *pointer_scroll_data_->axis_source == WL_POINTER_AXIS_SOURCE_WHEEL_TILT) {
            MouseWheelEvent event(gfx::Vector2d(pointer_scroll_data_->dx, pointer_scroll_data_->dy), pointer_location_, pointer_location_, timestamp, flags, 0);
            pointer_frames_.push_back(std::make_unique<FrameData>(event, base::NullCallback()));
        } else if (*pointer_scroll_data_->axis_source == WL_POINTER_AXIS_SOURCE_FINGER
            || *pointer_scroll_data_->axis_source == WL_POINTER_AXIS_SOURCE_CONTINUOUS) {
#if !BUILDFLAG(IS_CHROMEOS_LACROS)
            ...
            if (is_fling_active_) {
                ...
                ScrollEvent stop_fling_event(
                    EventType::kScrollFlingCancel, pointer_location_, pointer_location_, timestamp, flags, 0, 0, 0, 0, kGestureScrollFingerCount);
                pointer_frames_.push_back(std::make_unique<FrameData>(stop_fling_event, base::NullCallback()));
            }
#endif
            ScrollEvent event(EventType::kScroll, pointer_location_, pointer_location_, timestamp, flags, pointer_scroll_data_->dx, pointer_scroll_data_->dy,
                pointer_scroll_data_->dx, pointer_scroll_data_->dy, kGestureScrollFingerCount);
            pointer_frames_.push_back(std::make_unique<FrameData>(event, base::NullCallback()));
        }
