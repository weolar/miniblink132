...
public:
    ...
    wl::SerialTracker& serial_tracker()
    {
        return connection_->serial_tracker();
    }
    ...
    void SendPointerMotion(WaylandWindow* window, MockPlatformWindowDelegate* delegate, gfx::Point location, bool ensure_dispatched = true)
    {
        ...
        if (ensure_dispatched) {
            ...
            EXPECT_EQ(window->GetWidget(), screen_->GetLocalProcessWidgetAtPoint(location, {}));
        }
    }
    ...
    void SendDndMotionForWindowDrag(const gfx::Point& location)
    {
        ...
        if (drag_controller_state() != WaylandWindowDragController::State::kDetached) {
            return;
        }
        ...
        const uint32_t surface_id = dragged_window->root_surface()->get_surface_id();
        PostToServerAndWait([new_origin, surface_id](wl::TestWaylandServerThread* server) {
            ...
            auto* aura_toplevel = surface->xdg_surface()->xdg_toplevel()->zaura_toplevel();
            ASSERT_TRUE(aura_toplevel);
            zaura_toplevel_send_origin_change(aura_toplevel->resource(), new_origin.x(), new_origin.y());
        });
#endif
    }
...
protected:
    ...
    void SendPointerEnter(WaylandWindow* window, MockPlatformWindowDelegate* delegate) override
    {
        EXPECT_CALL(*delegate, DispatchEvent(_)).Times(1);
        WaylandDragDropTest::SendPointerEnter(window, delegate);
        Mock::VerifyAndClearExpectations(delegate);
        EXPECT_EQ(window, window_manager()->GetCurrentPointerOrTouchFocusedWindow());
    }

    void SendPointerLeave(WaylandWindow* window, MockPlatformWindowDelegate* delegate) override
    {
        EXPECT_CALL(*delegate, DispatchEvent(_)).Times(1);
        WaylandDragDropTest::SendPointerLeave(window, delegate);
        Mock::VerifyAndClearExpectations(delegate);
        EXPECT_EQ(nullptr, window_manager()->GetCurrentPointerOrTouchFocusedWindow());
    }

    void SendPointerPress(WaylandWindow* window, MockPlatformWindowDelegate* delegate, int button)
    {
        EXPECT_CALL(*delegate, DispatchEvent(_)).Times(1);
    ...
    void SendTouchDown(WaylandWindow* window, MockPlatformWindowDelegate* delegate, int id, const gfx::Point& location) override
    {
        EXPECT_CALL(*delegate, DispatchEvent(_)).WillOnce([](Event* event) { EXPECT_EQ(EventType::kTouchPressed, event->type()); });
        WaylandDragDropTest::SendTouchDown(window, delegate, id, location);
        Mock::VerifyAndClearExpectations(delegate);
        EXPECT_EQ(window, window_manager()->GetCurrentPointerOrTouchFocusedWindow());
    }
    void SendTouchMotion(WaylandWindow* window, MockPlatformWindowDelegate* delegate, int id, const gfx::Point& location) override
    {
        EXPECT_CALL(*delegate, DispatchEvent(_)).WillOnce([](Event* event) { EXPECT_EQ(EventType::kTouchMoved, event->type()); });
...
TEST_P(WaylandWindowDragControllerTest, DragInsideWindowAndDrop)
{
    ...
    EXPECT_EQ(gfx::kNullAcceleratedWidget, screen_->GetLocalProcessWidgetAtPoint({ 10, 10 }, {}));
    ...
    wayland_extension->StartWindowDraggingSessionIfNeeded(DragEventSource::kMouse,
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillRepeatedly([&](Event* event) {
        ...
        case kStarted:
            ...
            EXPECT_EQ(window_->GetWidget(), screen_->GetLocalProcessWidgetAtPoint({ 10, 10 }, {}));
            ...
            ScheduleTestTask(base::BindOnce(&WaylandWindowDragControllerTest::SendDndMotionForWindowDrag, base::Unretained(this), gfx::Point(20, 20)));
            test_step = kDragging;
            break;
        case kDropping: {
            ...
            // Ensure PlatformScreen keeps consistent.
            EXPECT_EQ(window_->GetWidget(), screen_->GetLocalProcessWidgetAtPoint({ 20, 20 }, {}));
            test_step = kDone;
            break;
        }
        case kDragging:
        default:
            FAIL() << " event=" << event->GetName() << " state=" << drag_controller_state() << " step=" << static_cast<int>(test_step);
        }
    });

    EXPECT_CALL(delegate_, OnBoundsChanged(_)).WillOnce([&](const PlatformWindowDelegate::BoundsChange& change) {
...
TEST_P(WaylandWindowDragControllerTest, DragInsideWindowAndDrop_TOUCH)
{
    ...
    SendTouchMotion(window_.get(), &delegate_, 0 /*point id*/, { 10, 10 } /*location*/);

    // Set up an "interaction flow", start the drag session and run move loop:
    //  - Event dispatching and bounds changes are monitored
    //  - At each event, emulates a new event at server side and proceeds to the
    //  next test step.
    GetWaylandToplevelExtension(*window_)->StartWindowDraggingSessionIfNeeded(DragEventSource::kTouch,
        /*allow_system_drag=*/false);
    ...
    EXPECT_CALL(delegate_, OnBoundsChanged(_)).WillOnce([&](const PlatformWindowDelegate::BoundsChange& change) {
        ...
        test_step = kDropping;
        ScheduleTestTask(base::BindOnce(&WaylandWindowDragControllerTest::SendDndDropAndFinished, base::Unretained(this)));
    });
    ...
    EXPECT_EQ(State::kIdle, drag_controller_state());
    EXPECT_EQ(window_.get(), window_manager()->GetCurrentPointerOrTouchFocusedWindow());
    EXPECT_EQ(window_->GetWidget(), screen_->GetLocalProcessWidgetAtPoint({ 20, 20 }, {}));
}
...
TEST_P(WaylandWindowDragControllerTest, DragInsideWindowAndDropTwoFingerTouch)
{
    ...
    SendTouchMotion(window_.get(), &delegate_, 0 /*point id*/, { 10, 10 } /*location*/);
    SendTouchMotion(window_.get(), &delegate_, 1 /*point id*/, { 15, 10 } /*location*/);
    ...
    enum TestStep { kDragging, kDropping, kFirstFingerReleased, kDone } test_step = kDragging;
...
TEST_P(WaylandWindowDragControllerTest, DestroyWindowDuringDragAndDrop_TOUCH)
{
    ...
    auto window_2 = delegate_.CreateWaylandWindow(connection_.get(), std::move(properties));
    ...
    // dragged
    SendTouchDown(window_2.get(), &delegate_, 0 /*point id*/, { 0, 0 } /*location*/);

    auto* wayland_extension = GetWaylandToplevelExtension(*window_2);
    wayland_extension->StartWindowDraggingSessionIfNeeded(DragEventSource::kTouch,
        /*allow_system_drag=*/false);

    // Verify that the proper window is being dragged.
    EXPECT_EQ(window_2.get(), window_manager()->GetCurrentPointerOrTouchFocusedWindow());
...
TEST_P(WaylandWindowDragControllerTest, DragAndDropWithExtraneousPointerEnterEvent_TOUCH)
{
    ...
    GetWaylandToplevelExtension(*window_2)->StartWindowDraggingSessionIfNeeded(DragEventSource::kTouch,
        /*allow_system_drag=*/false);
...
TEST_P(WaylandWindowDragControllerTest, DragToOtherWindowSnapDragDrop)
{
    ...
    enum { kStarted, kDragging, kEnteredTarget, kSnapped, kDone } test_step = kStarted;
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillRepeatedly([&](Event* event) {
        ...
        case kStarted:
            ...
            EXPECT_EQ(State::kDetached, drag_controller_state());
            // Ensure PlatformScreen keeps consistent.
            EXPECT_EQ(source_window->GetWidget(), screen_->GetLocalProcessWidgetAtPoint({ 10, 10 }, {}));
            ...
            // WaylandWindowDragController will crash.
            ScheduleTestTask(base::BindOnce(&WaylandWindowDragControllerTest::SendDndMotionForWindowDrag, base::Unretained(this), gfx::Point(50, 50)));
            test_step = kDragging;
            break;
        case kEnteredTarget:
            EXPECT_EQ(EventType::kMouseEntered, event->type());
            EXPECT_EQ(State::kDetached, drag_controller_state());
            // Ensure PlatformScreen keeps consistent.
            EXPECT_EQ(target_window->GetWidget(), screen_->GetLocalProcessWidgetAtPoint({ 10, 10 }, {}));
    ...
    EXPECT_EQ(target_window->GetWidget(), screen_->GetLocalProcessWidgetAtPoint({ 50, 50 }, {}));
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillOnce([&](Event* event) {
        ...
        case kSnapped:
            EXPECT_EQ(EventType::kMouseReleased, event->type());
            EXPECT_EQ(State::kDropped, drag_controller_state());
            EXPECT_EQ(target_window, window_manager()->GetCurrentPointerOrTouchFocusedWindow());
            test_step = kDone;
            break;
        default:
            FAIL() << " event=" << event->GetName() << " state=" << drag_controller_state() << " step=" << static_cast<int>(test_step);
    ...
    EXPECT_EQ(target_window, window_manager()->GetCurrentPointerOrTouchFocusedWindow());
    EXPECT_EQ(target_window->GetWidget(), screen_->GetLocalProcessWidgetAtPoint({ 20, 20 }, {}));
}
...
TEST_P(WaylandWindowDragControllerTest, DragToOtherWindowSnapDragDrop_TOUCH)
{
    ...
    EXPECT_CALL(delegate_, OnBoundsChanged(_)).WillOnce([&](const PlatformWindowDelegate::BoundsChange& change) {
        ...
        EXPECT_EQ(target_window, window_manager()->GetCurrentPointerOrTouchFocusedWindow());
        EXPECT_EQ(target_window, static_cast<WaylandTouch::Delegate*>(connection_->event_source())->GetTouchTarget(0 /*point id*/));
        move_loop_handler->EndMoveLoop();
    });
...
TEST_P(WaylandWindowDragControllerTest, DragExitAttached)
{
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillOnce([&](Event* event) {
        EXPECT_EQ(EventType::kMouseDragged, event->type());
        EXPECT_EQ(gfx::Point(50, -1).ToString(), event->AsMouseEvent()->location().ToString());
    });
    SendDndLeave();
    ...
    if (window_->IsScreenCoordinatesEnabled()) {
        EXPECT_EQ(window_->GetWidget(), screen_->GetLocalProcessWidgetAtPoint({ 70, 70 }, {}));
    } else {
        EXPECT_EQ(window_->GetWidget(), screen_->GetLocalProcessWidgetAtPoint({ 20, 20 }, {}));
    }
}
...
TEST_P(WaylandWindowDragControllerTest, DragExitAttached_TOUCH)
{
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillOnce([&](Event* event) {
        EXPECT_EQ(EventType::kTouchMoved, event->type());
        EXPECT_EQ(gfx::Point(50, -10).ToString(), event->AsTouchEvent()->location().ToString());
    });
    SendDndLeave();
...
TEST_P(WaylandWindowDragControllerTest, RestoreDuringWindowDragSession)
{
    ...
    SendConfigureEvent(window_->root_surface()->get_surface_id(), kMaximizedBounds.size(), states);
    ...
    wayland_extension->StartWindowDraggingSessionIfNeeded(DragEventSource::kMouse,
        /*allow_system_drag=*/false);
    EXPECT_EQ(WaylandWindowDragController::State::kAttached, drag_controller_state());
...
TEST_P(WaylandWindowDragControllerTest, MotionEventsSkippedWhileReattaching)
{
    ...
    auto test = [](WaylandWindowDragControllerTest* self, WmMoveLoopHandler* move_loop_handler) {
        // While in |kDetached| state, motion events are expected to be propagated
        // by window drag controller as bounds changes.
        EXPECT_EQ(State::kDetached, self->drag_controller_state());
        EXPECT_CALL(self->delegate(), OnBoundsChanged(_)).WillOnce([&](const PlatformWindowDelegate::BoundsChange& change) {
            EXPECT_EQ(gfx::Point(30, 30), self->window()->GetBoundsInDIP().origin());
            EXPECT_TRUE(change.origin_changed);
        });
    ...
    ScheduleTestTask(base::BindOnce(test, base::Unretained(this), base::Unretained(move_loop_handler)));
...
TEST_P(WaylandWindowDragControllerTest, CursorPositionIsUpdatedOnMotion)
{
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        ...
        auto* output2 = server->CreateAndInitializeOutput(wl::TestOutputMetrics(kOutputBounds));
        output2->SetScale(2);
    ...
    auto* move_loop_handler = GetWmMoveLoopHandler(*window_);
    ASSERT_TRUE(move_loop_handler);
    auto test = [](WaylandWindowDragControllerTest* self, WaylandScreen* screen, const WaylandOutputManager::OutputList* outputs,
                    wl::TestWaylandServerThread* server, WaylandWindow* window, WmMoveLoopHandler* move_loop_handler) {
        ASSERT_TRUE(outputs);
        for (const auto& output : *outputs) {
            SCOPED_TRACE(base::StringPrintf("Output Scale=%f", output.second->scale_factor()));
            gfx::Point p0 { 10, 10 };
            ...
            const uint32_t output_id = wl_proxy_get_id(reinterpret_cast<wl_proxy*>(output.second->get_output()));
            self->PostToServerAndWait([surface_id, output_id](wl::TestWaylandServerThread* server) {
            ...
            EXPECT_EQ(output.second->scale_factor(), window->applied_state().window_scale);
    ...
    const WaylandOutputManager::OutputList* outputs = &connection_->wayland_output_manager()->GetAllOutputs();
    ScheduleTestTask(base::BindOnce(test, base::Unretained(this), base::Unretained(screen_.get()), base::Unretained(outputs), base::Unretained(&server_),
...
TEST_P(WaylandWindowDragControllerTest, HandleDraggedWindowDestructionAfterMoveLoop)
{
    ...
    auto* move_loop_handler = GetWmMoveLoopHandler(*window_);
    ASSERT_TRUE(move_loop_handler);
    ScheduleTestTask(base::BindLambdaForTesting([&]() { move_loop_handler->EndMoveLoop(); }));
    ...
    window_.reset();
    EXPECT_NE(dangling_window_ptr, TestApi(drag_controller()).pointer_grab_owner());
    EXPECT_EQ(State::kIdle, drag_controller_state());
    ...
    EXPECT_EQ(State::kIdle, drag_controller_state());
    EXPECT_FALSE(window_manager()->GetCurrentPointerOrTouchFocusedWindow());
    EXPECT_EQ(gfx::kNullAcceleratedWidget, screen_->GetLocalProcessWidgetAtPoint({ 20, 20 }, {}));
}
...
TEST_P(WaylandWindowDragControllerTest, HandleWindowsDestructionDuringMoveLoop)
{
    ...
    auto window_2 = delegate_2.CreateWaylandWindow(connection_.get(), std::move(properties));
    ASSERT_NE(gfx::kNullAcceleratedWidget, window_2->GetWidget());
...
TEST_P(WaylandWindowDragControllerTest, ExtendedDragUnavailable)
{
    ...
    GetWaylandToplevelExtension(*window_)->StartWindowDraggingSessionIfNeeded(DragEventSource::kMouse,
        /*allow_system_drag=*/false);
    EXPECT_EQ(State::kAttached, drag_controller_state());
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillRepeatedly([&](Event* event) {
        ...
        case kStarted:
            ...
            // Otherwise, the data offer will be reset and
            // WaylandWindowDragController will crash.
            ScheduleTestTask(base::BindOnce(&WaylandDragDropTest::SendDndCancelled, base::Unretained(this)));
            test_step = kDropping;
            break;
        case kDropping: {
            EXPECT_EQ(EventType::kMouseReleased, event->type());
...
TEST_P(WaylandWindowDragControllerTest, GetSerial)
{
    ...
    { // No serial, no window focused.
        auto serial = TestApi(drag_controller()).GetSerial(DragEventSource::kMouse, origin);
    ...
    { // Serial available, but no window focused.
        ...
        auto serial = TestApi(drag_controller()).GetSerial(DragEventSource::kTouch, origin);
        EXPECT_FALSE(serial.has_value());
    }
    { // Both serial and focused window available.
...
TEST_P(WaylandWindowDragControllerTest, NoopUnlessPointerOrTouchPressed)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { ASSERT_FALSE(server->data_device_manager()->data_source()); });
...
TEST_P(WaylandWindowDragControllerTest, HandleTargetWindowDestruction_DetachedState)
{
...
// state).
// Regression test for crbug.com/1433577.
TEST_P(WaylandWindowDragControllerTest, HandleTargetWindowDestruction_AttachedState)
{
    // Send some initial pointer events to |window_|.
    ASSERT_FALSE(window_manager()->GetCurrentPointerOrTouchFocusedWindow());
    SendPointerEnter(window_.get(), &delegate_);
...
TEST_P(WaylandWindowDragControllerTest, PointerReleaseBeforeServerAckCancellsDrag)
{
    // Ensure there is no window currently focused.
    EXPECT_FALSE(window_manager()->GetCurrentPointerOrTouchFocusedWindow());
    EXPECT_EQ(gfx::kNullAcceleratedWidget, screen_->GetLocalProcessWidgetAtPoint({ 10, 10 }, {}));
    ...
    pointer_delegate()->OnPointerButtonEvent(EventType::kMouseReleased, EF_LEFT_MOUSE_BUTTON, base::TimeTicks::Now(), window_.get(),
...
TEST_P(WaylandWindowDragControllerTest, TouchReleaseBeforeServerAckCancellsDrag)
{
    // Ensure there is no window currently focused.
    EXPECT_FALSE(window_manager()->GetCurrentPointerOrTouchFocusedWindow());
    EXPECT_EQ(gfx::kNullAcceleratedWidget, screen_->GetLocalProcessWidgetAtPoint({ 10, 10 }, {}));
    ...
    // has acknowledged the drag request. This should cancel the drag.
    touch_delegate()->OnTouchReleaseEvent(base::TimeTicks::Now(), /*id=*/0, wl::EventDispatchPolicy::kImmediate,
        /*is_synthesized=*/false);
    EXPECT_EQ(State::kIdle, drag_controller_state());
}
...
TEST_P(WaylandWindowDragControllerTest, AllPointersReleasedAfterDragEnd)
{
    ...
    EXPECT_TRUE(pointer_delegate()->IsPointerButtonPressed(EF_RIGHT_MOUSE_BUTTON));
    EXPECT_TRUE(pointer_delegate()->IsPointerButtonPressed(EF_MIDDLE_MOUSE_BUTTON));
    ...
    EXPECT_FALSE(pointer_delegate()->IsPointerButtonPressed(EF_LEFT_MOUSE_BUTTON));
    EXPECT_FALSE(pointer_delegate()->IsPointerButtonPressed(EF_RIGHT_MOUSE_BUTTON));
    EXPECT_FALSE(pointer_delegate()->IsPointerButtonPressed(EF_MIDDLE_MOUSE_BUTTON));
}
...
TEST_P(WaylandWindowDragControllerTest, OutgoingSessionWithoutDndFinished)
{
    ...
    window_->StartDrag(os_exchange_data, DragDropTypes::DRAG_COPY | DragDropTypes::DRAG_MOVE, DragEventSource::kMouse, /*cursor=*/ {},
        /*can_grab_pointer=*/true, start_callback.Get(), completion_callback.Get(),
        /*loation delegate=*/nullptr);
    EXPECT_NE(connection_->data_drag_controller()->state_, WaylandDataDragController::State::kIdle);
    ...
    SendPointerEnter(window_.get(), &delegate_);
    SendPointerPress(window_.get(), &delegate_, BTN_LEFT);
    EXPECT_TRUE(drag_controller()->StartDragSession(window_->AsWaylandToplevelWindow(), DragEventSource::kMouse));
    EXPECT_EQ(State::kAttached, drag_controller_state());
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandWindowDragControllerTest, Values(wl::ServerConfig {}));
#else
// Linux shouldn't use aura shell.
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTestWithAuraShell, WaylandWindowDragControllerTest,
