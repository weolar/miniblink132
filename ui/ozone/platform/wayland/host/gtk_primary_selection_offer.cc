...
GtkPrimarySelectionOffer::GtkPrimarySelectionOffer(gtk_primary_selection_offer* data_offer)
    : data_offer_(data_offer)
{
    static constexpr gtk_primary_selection_offer_listener kPrimarySelectionOfferListener = { .offer = &OnOffer };
    gtk_primary_selection_offer_add_listener(data_offer, &kPrimarySelectionOfferListener, this);
}
...
base::ScopedFD GtkPrimarySelectionOffer::Receive(const std::string& mime_type)
{
    ...
    gtk_primary_selection_offer_receive(data_offer_.get(), effective_mime_type.data(), write_fd.get());
    return read_fd;
}

// static
void GtkPrimarySelectionOffer::OnOffer(void* data, gtk_primary_selection_offer* selection_offer, const char* mime_type)
{
