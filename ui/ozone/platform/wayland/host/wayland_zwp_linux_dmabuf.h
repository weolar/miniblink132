...
class WaylandZwpLinuxDmabuf : public wl::GlobalObjectRegistrar<WaylandZwpLinuxDmabuf> {
public:
    ...
    WaylandZwpLinuxDmabuf(zwp_linux_dmabuf_v1* zwp_linux_dmabuf, WaylandConnection* connection);
    ...
    // |wl_buffer| is sent.
    void CreateBuffer(const base::ScopedFD& fd, const gfx::Size& size, const std::vector<uint32_t>& strides, const std::vector<uint32_t>& offsets,
        const std::vector<uint64_t>& modifiers, uint32_t format, uint32_t planes_count, wl::OnRequestBufferCallback callback);
...
private:
    ...
    void AddSupportedFourCCFormatAndModifier(uint32_t fourcc_format, std::optional<uint64_t> modifier);
    ...
    void NotifyRequestCreateBufferDone(zwp_linux_buffer_params_v1* params, wl_buffer* new_buffer);

    // zwp_linux_dmabuf_v1_listener callbacks:
    static void OnModifiers(void* data, zwp_linux_dmabuf_v1* linux_dmabuf, uint32_t format, uint32_t modifier_hi, uint32_t modifier_lo);
    static void OnFormat(void* data, zwp_linux_dmabuf_v1* linux_dmabuf, uint32_t format);

    // zwp_linux_buffer_params_v1_listener callbacks:
    static void OnCreated(void* data, zwp_linux_buffer_params_v1* params, wl_buffer* new_buffer);
    ...
    base::flat_map<wl::Object<zwp_linux_buffer_params_v1>, wl::OnRequestBufferCallback> pending_params_;
};
