...
void SerialTracker::UpdateSerial(SerialType type, uint32_t serial)
{
    ...
    serials_.at(type) = { .value = serial, .type = type, .timestamp = base::TimeTicks::Now() - base_time_ };
}
...
std::optional<Serial> SerialTracker::GetSerial(const std::vector<SerialType>& types) const
{
    DCHECK(!types.empty());
...
{
    auto tostring = [this](const SerialType serial_type, const std::string& label, std::ostringstream& out) {
