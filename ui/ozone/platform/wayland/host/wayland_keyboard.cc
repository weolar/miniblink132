...
public:
    explicit WaylandKeyboardHook(wl::Object<zwp_keyboard_shortcuts_inhibitor_v1> inhibitor)
...
public:
    ...
    ZCRExtendedKeyboard(WaylandKeyboard* keyboard, zcr_extended_keyboard_v1* extended_keyboard)
        : keyboard_(keyboard)
        , obj_(extended_keyboard)
    {
        static constexpr zcr_extended_keyboard_v1_listener kExtendedKeyboardListener = {
            .peek_key = &OnPeekKey,
        };
        zcr_extended_keyboard_v1_add_listener(obj_.get(), &kExtendedKeyboardListener, this);
    }
    ...
    {
        return wl::get_version_of_object(obj_.get()) >= ZCR_EXTENDED_KEYBOARD_V1_PEEK_KEY_SINCE_VERSION;
    }

private:
    static void OnPeekKey(void* data, zcr_extended_keyboard_v1* extended_keyboard, uint32_t serial, uint32_t time, uint32_t key, uint32_t state)
    {
        auto* self = static_cast<ZCRExtendedKeyboard*>(data);
        DCHECK(self);
        self->keyboard_->ProcessKey(serial, time, key, state, WaylandKeyboard::KeyEventKind::kPeekKey);
    }
...
WaylandKeyboard::WaylandKeyboard(wl_keyboard* keyboard, zcr_keyboard_extension_v1* keyboard_extension_v1, WaylandConnection* connection,
    KeyboardLayoutEngine* layout_engine, Delegate* delegate)
    : obj_(keyboard)
    ...
    , layout_engine_(static_cast<LayoutEngine*>(layout_engine))
{
    ...
    if (keyboard_extension_v1) {
        extended_keyboard_ = std::make_unique<ZCRExtendedKeyboard>(this, zcr_keyboard_extension_v1_get_extended_keyboard(keyboard_extension_v1, obj_.get()));
    }
}
...
void WaylandKeyboard::OnUnhandledKeyEvent(const KeyEvent& key_event)
{
    ...
    uint32_t serial = it->second[0] | (it->second[1] << 8) | (it->second[2] << 16) | (it->second[3] << 24);
...
std::unique_ptr<PlatformKeyboardHook> WaylandKeyboard::CreateKeyboardHook(
    WaylandWindow* window, std::optional<base::flat_set<DomCode>> dom_codes, PlatformKeyboardHook::KeyEventCallback callback)
{
    DCHECK(window);
#if BUILDFLAG(IS_CHROMEOS_LACROS)
    return std::make_unique<BaseKeyboardHook>(std::move(dom_codes), std::move(callback));
#else
    return std::make_unique<WaylandKeyboardHook>(CreateShortcutsInhibitor(window));
#endif
}

wl::Object<zwp_keyboard_shortcuts_inhibitor_v1> WaylandKeyboard::CreateShortcutsInhibitor(WaylandWindow* window)
{
    DCHECK(window);
    ...
    if (auto* manager = connection_->keyboard_shortcuts_inhibit_manager_v1()) {
        return wl::Object<zwp_keyboard_shortcuts_inhibitor_v1>(
            zwp_keyboard_shortcuts_inhibit_manager_v1_inhibit_shortcuts(manager, window->root_surface()->surface(), connection_->seat()->wl_object()));
    }
    return {};
}

// static
void WaylandKeyboard::OnKeymap(void* data, wl_keyboard* keyboard, uint32_t format, int32_t fd, uint32_t size)
{
    ...
    int map_flags = wl_keyboard_get_version(keyboard) >= 7 ? MAP_PRIVATE : MAP_SHARED;
    ...
    if (!self->layout_engine_->SetCurrentLayoutFromBuffer(keymap_string, strnlen(keymap_string, size))) {
...
// static
void WaylandKeyboard::OnEnter(void* data, wl_keyboard* keyboard, uint32_t serial, wl_surface* surface, wl_array* keys)
{
...
// static
void WaylandKeyboard::OnLeave(void* data, wl_keyboard* keyboard, uint32_t serial, wl_surface* surface)
{
    // wl_surface might have been destroyed by this time.
    auto* self = static_cast<WaylandKeyboard*>(data);
    if (auto* window = wl::RootWindowFromWlSurface(surface))
...
void WaylandKeyboard::OnKey(void* data, wl_keyboard* keyboard, uint32_t serial, uint32_t time, uint32_t key, uint32_t state)
{
    auto* self = static_cast<WaylandKeyboard*>(data);
    DCHECK(self);
...
// static
void WaylandKeyboard::OnModifiers(void* data, wl_keyboard* keyboard, uint32_t serial, uint32_t depressed, uint32_t latched, uint32_t locked, uint32_t group)
{
#if BUILDFLAG(USE_XKBCOMMON)
    auto* self = static_cast<WaylandKeyboard*>(data);
    DCHECK(self);

    int modifiers = self->layout_engine_->UpdateModifiers(depressed, latched, locked, group);
    self->delegate_->OnKeyboardModifiersChanged(modifiers);
#endif
}

// static
void WaylandKeyboard::OnRepeatInfo(void* data, wl_keyboard* keyboard, int32_t rate, int32_t delay)
{
    ...
    if (handler.IsAutoRepeatEnabled()) {
        ...
        handler.SetAutoRepeatRate(base::Milliseconds(delay), base::Seconds(1.0 / rate));
    }
}

// static
void WaylandKeyboard::OnSyncDone(void* data, struct wl_callback* callback, uint32_t time)
{
    auto* self = static_cast<WaylandKeyboard*>(data);
    DCHECK(self);
...
void WaylandKeyboard::DispatchKey(unsigned int key, unsigned int scan_code, bool down, bool repeat, base::TimeTicks timestamp, int device_id, int flags)
{
    // Key repeat is only triggered by wl_keyboard::key event, but not by
    // extended_keyboard::peek_key.
    DispatchKey(key, scan_code, down, repeat, std::nullopt, timestamp, device_id, flags, KeyEventKind::kKey);
}

void WaylandKeyboard::ProcessKey(uint32_t serial, uint32_t time, uint32_t key, uint32_t state, KeyEventKind kind)
{
    bool down = state == WL_KEYBOARD_KEY_STATE_PRESSED;
    if (down) {
        connection_->serial_tracker().UpdateSerial(wl::SerialType::kKeyPress, serial);
    }

    if (kind == KeyEventKind::kKey && !IsModifierKey(key)) {
        auto_repeat_handler_.UpdateKeyRepeat(key, 0 /*scan_code*/, down,
            /*suppress_auto_repeat=*/false, device_id(), wl::EventMillisecondsToTimeTicks(time));
    }
    ...
    if (kind == KeyEventKind::kKey && !down && extended_keyboard_ && extended_keyboard_->IsPeekKeySupported()) {
        return;
    }

    DispatchKey(key, 0 /*scan_code*/, down, false /*repeat*/, std::make_optional(serial), wl::EventMillisecondsToTimeTicks(time), device_id(), EF_NONE, kind);
}

void WaylandKeyboard::DispatchKey(unsigned int key, unsigned int scan_code, bool down, bool repeat, std::optional<uint32_t> serial, base::TimeTicks timestamp,
    int device_id, int flags, KeyEventKind kind)
{
    ...
    uint32_t result
        = delegate_->OnKeyboardKeyEvent(down ? EventType::kKeyPressed : EventType::kKeyReleased, dom_code, repeat, serial, timestamp, device_id, kind);

    if (extended_keyboard_ && !(result & POST_DISPATCH_STOP_PROPAGATION) && serial.has_value()) {
