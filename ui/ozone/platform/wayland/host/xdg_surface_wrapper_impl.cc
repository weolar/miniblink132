...
XDGSurfaceWrapperImpl::XDGSurfaceWrapperImpl(WaylandWindow* wayland_window, WaylandConnection* connection)
...
bool XDGSurfaceWrapperImpl::Initialize()
{
    ...
    xdg_surface_.reset(xdg_wm_base_get_xdg_surface(connection_->shell(), wayland_window_->root_surface()->surface()));
    if (!xdg_surface_) {
...
void XDGSurfaceWrapperImpl::SetWindowGeometry(const gfx::Rect& bounds)
{
    ...
    CHECK(!bounds.IsEmpty()) << "The xdg-shell protocol specification forbids "
                             ...
                             << bounds.ToString();
    xdg_surface_set_window_geometry(xdg_surface_.get(), bounds.x(), bounds.y(), bounds.width(), bounds.height());
}
...
// static
void XDGSurfaceWrapperImpl::OnConfigure(void* data, struct xdg_surface* surface, uint32_t serial)
{
