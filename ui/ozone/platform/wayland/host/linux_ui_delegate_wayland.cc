...
bool LinuxUiDelegateWayland::ExportWindowHandle(gfx::AcceleratedWidget parent, base::OnceCallback<void(const std::string&)> callback)
{
...
bool LinuxUiDelegateWayland::ExportWindowHandle(gfx::AcceleratedWidget window_id, base::OnceCallback<void(std::string)> callback)
{
    return ui::LinuxUiDelegate::GetInstance()->ExportWindowHandle(
        window_id, base::BindOnce(&LinuxUiDelegateWayland::OnHandleForward, weak_factory_.GetWeakPtr(), std::move(callback)));
}

void LinuxUiDelegateWayland::OnHandleForward(base::OnceCallback<void(std::string)> callback, const std::string& handle)
{
