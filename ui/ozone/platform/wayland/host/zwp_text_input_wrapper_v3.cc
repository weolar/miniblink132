...
ZWPTextInputWrapperV3::ZWPTextInputWrapperV3(WaylandConnection* connection, ZWPTextInputWrapperClient* client, zwp_text_input_manager_v3* text_input_manager)
    ...
    , client_(client)
{
    ...
    auto* text_input = zwp_text_input_manager_v3_get_text_input(text_input_manager, connection_->seat()->wl_object());
...
void ZWPTextInputWrapperV3::Activate(WaylandWindow* window, TextInputClient::FocusReason reason)
{
...
void ZWPTextInputWrapperV3::SendCursorRect(const gfx::Rect& rect)
{
    ...
    zwp_text_input_v3_set_cursor_rectangle(obj_.get(), rect.x(), rect.y(), rect.width(), rect.height());
    last_sent_cursor_rect_ = rect;
}

void ZWPTextInputWrapperV3::SetSurroundingText(const std::string& text_with_preedit, const gfx::Range& preedit_range, const gfx::Range& selection_range)
{
    ...
    if (!preedit_range.is_empty()) {
        ...
        if (selection_range.IsValid()) {
            ...
            anchor = selection_start - (selection_start <= preedit_min ? 0 : std::min(selection_start, preedit_max) - preedit_min);
            cursor = selection_end - (selection_end <= preedit_min ? 0 : std::min(selection_end, preedit_max) - preedit_min);
        ...
        anchor = base::checked_cast<int32_t>(selection_range.IsValid() ? selection_range.start() : text.length());
        cursor = base::checked_cast<int32_t>(selection_range.IsValid() ? selection_range.end() : text.length());
    }
...
void ZWPTextInputWrapperV3::SendSurroundingText(const SetSurroundingTextData& data)
{
    CHECK_EQ(commit_count_, last_done_serial_);
    zwp_text_input_v3_set_surrounding_text(obj_.get(), data.text.c_str(), data.cursor, data.anchor);
    last_sent_surrounding_text_data_ = data;
}

void ZWPTextInputWrapperV3::SetContentType(ui::TextInputType type, ui::TextInputMode mode, uint32_t flags, bool should_do_learning, bool can_compose_inline)
{
...
{
    CHECK_EQ(commit_count_, last_done_serial_);
    zwp_text_input_v3_set_content_type(obj_.get(), content_type.content_hint, content_type.content_purpose);
...
void ZWPTextInputWrapperV3::OnEnter(void* data, struct zwp_text_input_v3* text_input, struct wl_surface* surface)
{
...
void ZWPTextInputWrapperV3::OnLeave(void* data, struct zwp_text_input_v3* text_input, struct wl_surface* surface)
{
    // Same as text-input-v1, we don't use this for text-input focus changes and
    // instead use wayland keyboard enter/leave events to activate or deactivate
    // text-input.
...
void ZWPTextInputWrapperV3::OnPreeditString(void* data, struct zwp_text_input_v3* text_input, const char* text, int32_t cursor_begin, int32_t cursor_end)
{
    auto* self = static_cast<ZWPTextInputWrapperV3*>(data);
    self->pending_preedit_ = { text ? text : std::string(), cursor_begin, cursor_end };
}

void ZWPTextInputWrapperV3::OnCommitString(void* data, struct zwp_text_input_v3* text_input, const char* text)
{
    auto* self = static_cast<ZWPTextInputWrapperV3*>(data);
    self->pending_commit_ = text ? text : std::string();
}

void ZWPTextInputWrapperV3::OnDeleteSurroundingText(void* data, struct zwp_text_input_v3* text_input, uint32_t before_length, uint32_t after_length)
{
    NOTIMPLEMENTED_LOG_ONCE();
}

void ZWPTextInputWrapperV3::OnDone(void* data, struct zwp_text_input_v3* text_input, uint32_t serial)
{
    ...
    if (const auto& preedit_data = self->pending_preedit_) {
        gfx::Range preedit_cursor = (preedit_data->cursor_begin < 0 || preedit_data->cursor_end < 0)
            ? gfx::Range::InvalidRange()
            : gfx::Range(preedit_data->cursor_begin, preedit_data->cursor_end);
        self->client_->OnPreeditString(preedit_data->text.c_str(), {}, preedit_cursor);
    }
...
void ZWPTextInputWrapperV3::SetSurroundingTextOffsetUtf16(uint32_t offset_utf16)
{
    NOTIMPLEMENTED_LOG_ONCE();
}

void ZWPTextInputWrapperV3::SetGrammarFragmentAtCursor(const ui::GrammarFragment& fragment)
{
    NOTIMPLEMENTED_LOG_ONCE();
}

void ZWPTextInputWrapperV3::SetAutocorrectInfo(const gfx::Range& autocorrect_range, const gfx::Rect& autocorrect_bounds)
{
    NOTIMPLEMENTED_LOG_ONCE();
}
