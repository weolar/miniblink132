...
base::ScopedFD MakeFD()
{
    ...
    auto file = base::File(temp_path, base::File::FLAG_READ | base::File::FLAG_WRITE | base::File::FLAG_CREATE_ALWAYS);
    return base::ScopedFD(file.TakePlatformFile());
}

// Must happen on the server thread.
wl::TestXdgPopup* GetTestXdgPopupByWindow(wl::TestWaylandServerThread* server, const uint32_t surface_id)
{
    DCHECK(server->task_runner()->BelongsToCurrentThread());
    wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(surface_id);
    if (mock_surface) {
...
public:
    MockCursorShape()
        : WaylandCursorShape(nullptr, nullptr)
    {
    }
...
public:
    MockZcrCursorShapes()
        : WaylandZcrCursorShapes(nullptr, nullptr)
    {
    }
...
scoped_refptr<PlatformCursor> AsPlatformCursor(scoped_refptr<BitmapCursor> bitmap_cursor)
{
#if BUILDFLAG(IS_LINUX)
...
public:
    ...
    void OnWindowStateChanged(ui::PlatformWindowState old_state, ui::PlatformWindowState new_state) override
    {
    }
...
public:
    WaylandWindowTest()
        : test_mouse_event_(EventType::kMousePressed, gfx::Point(10, 15), gfx::Point(10, 15), ui::EventTimeStampFromSeconds(123456),
            EF_LEFT_MOUSE_BUTTON | EF_RIGHT_MOUSE_BUTTON, EF_LEFT_MOUSE_BUTTON)
    {
    }
...
protected:
    void SendConfigureEventPopup(WaylandWindow* menu_window, const gfx::Rect& bounds)
    {
        const uint32_t surface_id = menu_window->root_surface()->get_surface_id();
        PostToServerAndWait([surface_id, bounds](wl::TestWaylandServerThread* server) {
            auto* popup = GetTestXdgPopupByWindow(server, surface_id);
            ASSERT_TRUE(popup);
            xdg_popup_send_configure(popup->resource(), bounds.x(), bounds.y(), bounds.width(), bounds.height());
        });
    }
    ...
    // state to update.
    void AdvanceFrameToCurrent(WaylandWindow* window, const MockWaylandPlatformWindowDelegate& delegate)
    {
        AdvanceFrameToGivenVizSequenceId(window, delegate, delegate.viz_seq());
    }

    void AdvanceFrameToGivenVizSequenceId(WaylandWindow* window, const MockWaylandPlatformWindowDelegate& delegate, int64_t viz_seq)
    {
    ...
    // wp-viewport.
    void VerifyAndClearExpectations(MockWaylandPlatformWindowDelegate& delegate, uint32_t surface_id)
    {
    ...
    void VerifyXdgPopupPosition(WaylandWindow* menu_window, const PopupPosition& position)
    {
        const uint32_t surface_id = menu_window->root_surface()->get_surface_id();
        PostToServerAndWait([surface_id, position](wl::TestWaylandServerThread* server) {
            auto* popup = GetTestXdgPopupByWindow(server, surface_id);
            ASSERT_TRUE(popup);
    ...
    void VerifyCanDispatchMouseEvents(WaylandWindow* dispatching_window, const std::vector<WaylandWindow*>& non_dispatching_windows)
    {
        auto* pointer_focused_window = connection_->window_manager()->GetCurrentPointerFocusedWindow();
    ...
    void VerifyCanDispatchTouchEvents(const std::vector<WaylandWindow*>& dispatching_windows, const std::vector<WaylandWindow*>& non_dispatching_windows)
    {
        ASSERT_LT(dispatching_windows.size(), 2u);
        auto* touch_focused_window = connection_->window_manager()->GetCurrentTouchFocusedWindow();
        ...
        TouchEvent test_touch_event(EventType::kTouchPressed, { 1, 1 }, base::TimeTicks(), pointer_details);
    ...
    void VerifyCanDispatchKeyEvents(const std::vector<WaylandWindow*>& dispatching_windows, const std::vector<WaylandWindow*>& non_dispatching_windows)
    {
        ASSERT_LT(dispatching_windows.size(), 2u);
        auto* keyboard_focused_window = connection_->window_manager()->GetCurrentKeyboardFocusedWindow();
    ...
    uint32_t GetObjIdForOutput(WaylandOutput::Id id)
    {
        ...
        if (wayland_output) {
            return wl_proxy_get_id(reinterpret_cast<wl_proxy*>(wayland_output->get_output()));
        }
        return 0;
    }

    WaylandBufferHandle* CreateTestShmBuffer(const gfx::Size& buffer_size, WaylandSurface* surface)
    {
        ...
        buffer_manager_gpu_->CreateShmBasedBuffer(MakeFD(), length, buffer_size, buffer_id);
        task_environment_.RunUntilIdle();
        return connection_->buffer_manager_host()->EnsureBufferHandle(surface, buffer_id);
    }

    // Emulates a new frame being received from Viz being processed by `window`s
    // frame manager.
    void CreateBufferAndPresentAsNewFrame(
        WaylandWindow* window, const MockWaylandPlatformWindowDelegate& delegate, const gfx::Size& buffer_size, float buffer_scale)
    {
        CHECK(window);
        ...
        window->CommitOverlays(frame_id, gfx::FrameData(delegate.viz_seq()), configs);
    }
...
TEST_P(WaylandWindowTest, OnSequencePointConfiguresWaylandWindow)
{
    ...
    PostToServerAndWait([id = surface_id_, bounds = kNormalBounds](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        auto* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(gfx::Rect(bounds.size()))).Times(0);
    ...
    SendConfigureEvent(surface_id_, kNormalBounds.size(), state, kConfigureSerial);
...
TEST_P(WaylandWindowTest, HandleTiledEdges)
{
    ...
    struct {
        ...
        { { XDG_TOPLEVEL_STATE_TILED_LEFT, XDG_TOPLEVEL_STATE_TILED_TOP }, { true, false, true, false } },
        { { XDG_TOPLEVEL_STATE_TILED_LEFT, XDG_TOPLEVEL_STATE_TILED_BOTTOM }, { true, false, false, true } },
        { { XDG_TOPLEVEL_STATE_TILED_RIGHT, XDG_TOPLEVEL_STATE_TILED_TOP }, { false, true, true, false } },
        { { XDG_TOPLEVEL_STATE_TILED_RIGHT, XDG_TOPLEVEL_STATE_TILED_BOTTOM }, { false, true, false, true } },
        { { XDG_TOPLEVEL_STATE_TILED_LEFT, XDG_TOPLEVEL_STATE_TILED_TOP, XDG_TOPLEVEL_STATE_TILED_BOTTOM }, { true, false, true, true } },
        { { XDG_TOPLEVEL_STATE_TILED_RIGHT, XDG_TOPLEVEL_STATE_TILED_TOP, XDG_TOPLEVEL_STATE_TILED_BOTTOM }, { false, true, true, true } },
    };
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_CALL(delegate_, OnWindowTiledStateChanged(test_case.expected_tiled_edges)).Times(1);
...
TEST_P(WaylandWindowTest, DisregardUnpassedWindowConfigure)
{
    ...
    // applied.
    PostToServerAndWait([id = surface_id_, bounds1 = kNormalBounds1, bounds2 = kNormalBounds2, bounds3 = kNormalBounds3](wl::TestWaylandServerThread* server) {
        ...
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(gfx::Rect(bounds2.size()))).Times(0);
...
TEST_P(WaylandWindowTest, OnSequencePointClearsPreviousUnackedConfigures)
{
    ...
    PostToServerAndWait([id = surface_id_, bounds = kNormalBounds1](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        wl::MockXdgSurface* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(gfx::Rect(bounds.size()))).Times(0);
    ...
    PostToServerAndWait([id = surface_id_, bounds = kNormalBounds2](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        wl::MockXdgSurface* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(gfx::Rect(bounds.size()))).Times(0);
    ...
    PostToServerAndWait([id = surface_id_, bounds = kNormalBounds3](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        wl::MockXdgSurface* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(gfx::Rect(bounds.size())));
...
TEST_P(WaylandWindowTest, RestoredBoundsSetWithCorrectOrigin)
{
    ...
    window_->HandleToplevelConfigure(kMaximizedBounds.width(), kMaximizedBounds.height(), window_states);
...
TEST_P(WaylandWindowTest, MaximizeAndRestore)
{
    ...
    auto active_maximized = MakeStateArray({ XDG_TOPLEVEL_STATE_ACTIVATED, XDG_TOPLEVEL_STATE_MAXIMIZED });
    PostToServerAndWait([id = surface_id_, bounds = kMaximizedBounds](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        wl::MockXdgSurface* xdg_surface = mock_surface->xdg_surface();
...
TEST_P(WaylandWindowTest, MaximizeAndRestoreWithInsets)
{
    ...
    auto active_maximized = MakeStateArray({ XDG_TOPLEVEL_STATE_ACTIVATED, XDG_TOPLEVEL_STATE_MAXIMIZED });
    PostToServerAndWait([id = surface_id_, bounds = maximized_geometry](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        wl::MockXdgSurface* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface->xdg_toplevel(), SetMaximized());
    ...
    // change of the window state.
    // See https://crbug.com/1223005.
    EXPECT_CALL(delegate_, CalculateInsetsInDIP(PlatformWindowState::kMaximized)).WillRepeatedly(Return(kMaximizedInsets));
    EXPECT_CALL(delegate_, OnWindowStateChanged(_, _)).Times(1);
    window_->Maximize();
    ...
    EXPECT_CALL(delegate_, OnActivationChanged(Eq(false)));
    EXPECT_CALL(delegate_, OnBoundsChanged(_)).Times(0);
    SendConfigureEvent(surface_id_, maximized_geometry.size(), inactive_maximized);
    AdvanceFrameToCurrent(window_.get(), delegate_);
    VerifyAndClearExpectations();
    ...
    // change of the window state.
    // See https://crbug.com/1223005.
    EXPECT_CALL(delegate_, CalculateInsetsInDIP(PlatformWindowState::kNormal)).WillRepeatedly(Return(kNormalInsets));
    EXPECT_CALL(delegate_, OnWindowStateChanged(_, _)).Times(1);
    EXPECT_CALL(delegate_, OnActivationChanged(_)).Times(0);
    EXPECT_CALL(delegate_, OnBoundsChanged(Eq(kDefaultBoundsChange)));
    PostToServerAndWait([id = surface_id_, bounds = normal_geometry](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        wl::MockXdgSurface* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface->xdg_toplevel(), UnsetMaximized());
...
TEST_P(WaylandWindowTest, ClientInitiatedMinimize)
{
    ...
    EXPECT_CALL(delegate_, OnWindowStateChanged(_, PlatformWindowState::kMinimized)).Times(1);
    {
        WaylandWindow::WindowStates window_states;
...
TEST_P(WaylandWindowTest, Minimize)
{
    ...
    EXPECT_CALL(delegate_, OnWindowStateChanged(_, _)).WillRepeatedly(DoAll(SaveArg<0>(&state), InvokeWithoutArgs([&]() {
...
TEST_P(WaylandWindowTest, SetFullscreenAndRestore)
{
    ...
    EXPECT_EQ(window_->GetPlatformWindowState(), PlatformWindowState::kFullScreen);
...
TEST_P(WaylandWindowTest, StartWithFullscreen)
{
    ...
    auto window = delegate.CreateWaylandWindow(connection_.get(), std::move(properties));
    ...
    EXPECT_CALL(delegate, OnWindowStateChanged(Eq(PlatformWindowState::kNormal), Eq(PlatformWindowState::kFullScreen))).Times(1);
...
TEST_P(WaylandWindowTest, StartMaximized)
{
    ...
    EXPECT_CALL(delegate, OnWindowStateChanged(Eq(PlatformWindowState::kNormal), Eq(PlatformWindowState::kMaximized))).Times(1);
...
TEST_P(WaylandWindowTest, CompositorSideStateChanges)
{
    ...
    EXPECT_CALL(delegate_, CalculateInsetsInDIP(PlatformWindowState::kMaximized)).WillRepeatedly(Return(gfx::Insets()));
    EXPECT_CALL(delegate_, OnWindowStateChanged(_, Eq(PlatformWindowState::kMaximized))).Times(1);
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        wl::MockXdgSurface* xdg_surface = mock_surface->xdg_surface();
    ...
    // Unmaximize
    EXPECT_CALL(delegate_, CalculateInsetsInDIP(PlatformWindowState::kNormal)).WillRepeatedly(Return(kInsets));
    EXPECT_CALL(delegate_, OnWindowStateChanged(_, Eq(PlatformWindowState::kNormal))).Times(1);
    PostToServerAndWait([id = surface_id_, insets = kInsets, bounds = kNormalBounds](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        wl::MockXdgSurface* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface,
            SetWindowGeometry(
                gfx::Rect(insets.left(), insets.top(), bounds.width() - (insets.left() + insets.right()), bounds.height() - (insets.top() + insets.bottom()))));
    });
    states = InitializeWlArrayWithActivatedState();
    SendConfigureEvent(surface_id_, { 0, 0 }, states);
    AdvanceFrameToCurrent(window_.get(), delegate_);
    ...
    EXPECT_CALL(delegate_, CalculateInsetsInDIP(PlatformWindowState::kFullScreen)).WillRepeatedly(Return(gfx::Insets()));
    EXPECT_CALL(delegate_, OnWindowStateChanged(_, Eq(PlatformWindowState::kFullScreen))).Times(1);
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        wl::MockXdgSurface* xdg_surface = mock_surface->xdg_surface();
...
TEST_P(WaylandWindowTest, SetMaximizedFullscreenAndRestore)
{
    ...
    window_->SetFullscreen(true, display::kInvalidDisplayId);
    // State changes are synchronous.
    EXPECT_EQ(PlatformWindowState::kFullScreen, window_->GetPlatformWindowState());
...
TEST_P(WaylandWindowTest, RestoreBoundsAfterMaximize)
{
    ...
    PostToServerAndWait([id = surface_id_, current_bounds](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        wl::MockXdgSurface* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(gfx::Rect { current_bounds.size() }));
    });
    window_->Restore();
    // Reinitialize wl_array, which removes previous old states.
    states = InitializeWlArrayWithActivatedState();
...
TEST_P(WaylandWindowTest, SetCanMaximize)
{
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        ...
        wl::TestZAuraToplevel* zaura_toplevel = surface->xdg_surface()->xdg_toplevel()->zaura_toplevel();
...
TEST_P(WaylandWindowTest, SendsBoundsOnRequest)
{
    ...
    const gfx::Rect new_bounds = gfx::Rect(initial_bounds.width() + 10, initial_bounds.height() + 10);
    EXPECT_CALL(delegate_, OnBoundsChanged(kDefaultBoundsChange));

    PostToServerAndWait([id = surface_id_, new_bounds](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        wl::MockXdgSurface* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(gfx::Rect(new_bounds.size()))).Times(1);
    });
...
TEST_P(WaylandWindowTest, UpdateWindowRegion)
{
    ...
    PostToServerAndWait([id = surface_id_, kMaximizedBounds](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
    ...
    PostToServerAndWait([id = surface_id_, restored_bounds](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
...
TEST_P(WaylandWindowTest, CanDispatchMouseEventFocus)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { wl_seat_send_capabilities(server->seat()->resource(), WL_SEAT_CAPABILITY_POINTER); });
...
TEST_P(WaylandWindowTest, SetCursorUsesCursorShapeForCommonTypes)
{
    ...
    EXPECT_CALL(*mock_cursor_shape, SetCursorShape(WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_DEFAULT));
    auto pointer_cursor = AsPlatformCursor(base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kPointer));
    window_->SetCursor(pointer_cursor.get());
    VerifyAndClearExpectations();

    EXPECT_CALL(*mock_cursor_shape, SetCursorShape(WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_POINTER));
    auto hand_cursor = AsPlatformCursor(base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kHand));
    window_->SetCursor(hand_cursor.get());
    VerifyAndClearExpectations();

    EXPECT_CALL(*mock_cursor_shape, SetCursorShape(WP_CURSOR_SHAPE_DEVICE_V1_SHAPE_TEXT));
    auto ibeam_cursor = AsPlatformCursor(base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kIBeam));
...
TEST_P(WaylandWindowTest, SetCursorDoesNotUseCursorShapeForNoneCursor)
{
    ...
    auto none_cursor = AsPlatformCursor(base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kNone));
...
TEST_P(WaylandWindowTest, SetCursorDoesNotUseCursorShapeForCustomCursors)
{
    ...
    EXPECT_CALL(*mock_cursor_shape, SetCursorShape(_)).Times(0);
    auto custom_cursor = AsPlatformCursor(base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kCustom, SkBitmap(), gfx::Point(), kDefaultCursorScale));
...
TEST_P(WaylandWindowTest, SetCursorUsesZcrCursorShapesForCommonTypes)
{
    ...
    // Verify some commonly-used cursors.
    EXPECT_CALL(*mock_cursor_shapes, SetCursorShape(ZCR_CURSOR_SHAPES_V1_CURSOR_SHAPE_TYPE_POINTER));
    auto pointer_cursor = AsPlatformCursor(base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kPointer));
    window_->SetCursor(pointer_cursor.get());
    VerifyAndClearExpectations();

    EXPECT_CALL(*mock_cursor_shapes, SetCursorShape(ZCR_CURSOR_SHAPES_V1_CURSOR_SHAPE_TYPE_HAND));
    auto hand_cursor = AsPlatformCursor(base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kHand));
    window_->SetCursor(hand_cursor.get());
    VerifyAndClearExpectations();

    EXPECT_CALL(*mock_cursor_shapes, SetCursorShape(ZCR_CURSOR_SHAPES_V1_CURSOR_SHAPE_TYPE_IBEAM));
    auto ibeam_cursor = AsPlatformCursor(base::MakeRefCounted<BitmapCursor>(mojom::CursorType::kIBeam));
    window_->SetCursor(ibeam_cursor.get());
}
...
TEST_P(WaylandWindowTest, DispatchEvent)
{
    ...
    EXPECT_EQ(mouse_event->root_location_f(), test_mouse_event_.root_location_f());
    ...
    EXPECT_EQ(mouse_event->changed_button_flags(), test_mouse_event_.changed_button_flags());
}
...
TEST_P(WaylandWindowTest, DispatchEventResult)
{
    ...
    auto window = WaylandWindow::Create(&window_delegate, connection_.get(), std::move(properties));

    KeyEvent event_1(EventType::kKeyPressed, VKEY_0, 0);
    window_delegate.SetDispatchEventCallback(base::BindOnce([](Event* event) { event->SetSkipped(); }));
    EXPECT_EQ(window->DispatchEvent(&event_1), POST_DISPATCH_PERFORM_DEFAULT);

    KeyEvent event_2(EventType::kKeyPressed, VKEY_0, 0);
    window_delegate.SetDispatchEventCallback(base::BindOnce([](Event* event) { event->StopPropagation(); }));
    EXPECT_EQ(window->DispatchEvent(&event_2), POST_DISPATCH_STOP_PROPAGATION);

    KeyEvent event_3(EventType::kKeyPressed, VKEY_0, 0);
    window_delegate.SetDispatchEventCallback(base::BindOnce([](Event* event) { event->SetHandled(); }));
...
TEST_P(WaylandWindowTest, ConfigureEvent)
{
    ...
    PostToServerAndWait([id = surface_id_, kSize, serial](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        auto* xdg_surface = mock_surface->xdg_surface();
    ...
    EXPECT_CALL(delegate_, OnBoundsChanged(Eq(kDefaultBoundsChange)));
    PostToServerAndWait([id = surface_id_, kNewSize, serial](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        auto* xdg_surface = mock_surface->xdg_surface();
...
TEST_P(WaylandWindowTest, ConfigureEventIsNotAckedMultipleTimes)
{
    ...
    PostToServerAndWait([id = surface_id_, bounds = kSecondBounds](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        auto* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(bounds));
...
TEST_P(WaylandWindowTest, ThrottledConfigureEventsDoNotGetStuckOnHiddenOcclusion)
{
    uint32_t serial = 2u;
    ...
    window_->HandleToplevelConfigureWithOrigin(0, 0, size.width(), size.height(), window_states);
...
TEST_P(WaylandWindowTest, InitialConfigureFollowedByBoundsChangeCompletesAck)
{
...

    {
        WaylandWindow::WindowStates window_states;
        window_states.is_activated = true;
        window_->HandleToplevelConfigureWithOrigin(kSecondBounds.x(), kSecondBounds.y(), kSecondBounds.width(), kSecondBounds.height(), window_states);
    }
...
TEST_P(WaylandWindowTest, CanCreateMenuWindow)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        wl_seat_send_capabilities(server->seat()->resource(), WL_SEAT_CAPABILITY_POINTER | WL_SEAT_CAPABILITY_TOUCH);
    });
    ASSERT_TRUE(connection_->seat()->pointer());
    ASSERT_TRUE(connection_->seat()->touch());
    SetPointerFocusedWindow(window_.get());

    std::unique_ptr<WaylandWindow> menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(10, 10), &menu_window_delegate);
    ...
    menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(10, 10), &menu_window_delegate);
    EXPECT_TRUE(menu_window);
...
TEST_P(WaylandWindowTest, CreateAndDestroyNestedMenuWindow)
{
    ...
    EXPECT_CALL(menu_window_delegate, OnAcceleratedWidgetAvailable(_)).WillOnce(SaveArg<0>(&menu_window_widget));

    std::unique_ptr<WaylandWindow> menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(10, 10), &menu_window_delegate, widget_);
    EXPECT_TRUE(menu_window);
    ...
    std::unique_ptr<WaylandWindow> nested_menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(20, 0, 10, 10), &nested_menu_window_delegate, menu_window_widget);
...
TEST_P(WaylandWindowTest, DispatchesLocatedEventsToCapturedWindow)
{
    ...
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(10, 10, 10, 10), &menu_window_delegate, widget_);
    EXPECT_TRUE(menu_window);
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        wl_pointer_send_motion(server->seat()->pointer()->resource(), server->GetNextSerial(), wl_fixed_from_double(10.75), wl_fixed_from_double(20.375));
    ...
    EXPECT_CALL(menu_window_delegate, DispatchEvent(_)).WillOnce(CloneEvent(&event2));
    ...
    EXPECT_CALL(menu_window_delegate, DispatchEvent(_)).WillOnce(CloneEvent(&event3));

    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        // The event is sent in local surface coordinates of the |window|.
        wl_pointer_send_motion(server->seat()->pointer()->resource(), server->GetNextSerial(), wl_fixed_from_double(2.75), wl_fixed_from_double(8.375));
        wl_pointer_send_frame(server->seat()->pointer()->resource());
    });
    ...
    MockWaylandPlatformWindowDelegate nested_menu_window_delegate;
    std::unique_ptr<WaylandWindow> nested_menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(15, 18, 10, 10), &nested_menu_window_delegate, menu_window->GetWidget());
    EXPECT_TRUE(nested_menu_window);
    ...
    VerifyCanDispatchMouseEvents(nested_menu_window.get(), { window_.get(), menu_window.get() });
    ...
    EXPECT_CALL(menu_window_delegate, DispatchEvent(_)).WillOnce(CloneEvent(&event4));
...
TEST_P(WaylandWindowTest, ConvertEventToTarget)
{
    ...
    EXPECT_CALL(delegate_, CalculateInsetsInDIP(PlatformWindowState::kNormal)).WillRepeatedly(Return(kMainWindowInsets));
    PostToServerAndWait([id = surface_id_, bounds_with_insets](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        auto* xdg_surface = mock_surface->xdg_surface();
    ...
    SendConfigureEvent(surface_id_, bounds_with_insets.size(), MakeStateArray({ XDG_TOPLEVEL_STATE_ACTIVATED }));
    AdvanceFrameToCurrent(window_.get(), delegate_);
    VerifyAndClearExpectations();
    ...
    MockWaylandPlatformWindowDelegate menu_window_delegate;
    std::unique_ptr<WaylandWindow> menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, kMenuBounds, &menu_window_delegate, widget_);
    EXPECT_TRUE(menu_window);
    ...
    ui::MouseEvent event(ui::EventType::kMouseMoved, kParentPoint, kParentPoint, {}, ui::EF_NONE, ui::EF_NONE);
...
TEST_P(WaylandWindowTest, DispatchesLocatedEventsToCapturedWindowInTheSameStack)
{
    MockWaylandPlatformWindowDelegate menu_window_delegate;
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(30, 40, 20, 50), &menu_window_delegate, widget_);
    EXPECT_TRUE(menu_window);
    ...
    std::unique_ptr<WaylandWindow> toplevel_window2
        = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, window_->GetBoundsInDIP(), &toplevel_window2_delegate);
    ...
    // Make sure the events are handled by the window that has the pointer focus.
    VerifyCanDispatchMouseEvents(window_.get(), { menu_window.get(), toplevel_window2.get() });
    ...
    std::unique_ptr<Event> event;
    EXPECT_CALL(menu_window_delegate, DispatchEvent(_)).WillOnce(CloneEvent(&event));
    ...
    VerifyCanDispatchMouseEvents(toplevel_window2.get(), { menu_window.get(), window_.get() });

    // The |toplevel_window2| that has capture and must receive the event.
    EXPECT_CALL(delegate_, DispatchEvent(_)).Times(0);
    EXPECT_CALL(menu_window_delegate, DispatchEvent(_)).Times(0);
    event.reset();
    EXPECT_CALL(toplevel_window2_delegate, DispatchEvent(_)).WillOnce(CloneEvent(&event));
...
TEST_P(WaylandWindowTest, DispatchesKeyboardEventToToplevelWindow)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { wl_seat_send_capabilities(server->seat()->resource(), WL_SEAT_CAPABILITY_KEYBOARD); });
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { wl_keyboard_send_repeat_info(server->seat()->keyboard()->resource(), 0, 0); });
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        wl_keyboard_send_key(server->seat()->keyboard()->resource(), server->GetNextSerial(), server->GetNextTime(), 30 /* a */, WL_KEYBOARD_KEY_STATE_PRESSED);
    });
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        wl_keyboard_send_key(
            server->seat()->keyboard()->resource(), server->GetNextSerial(), server->GetNextTime(), 30 /* a */, WL_KEYBOARD_KEY_STATE_RELEASED);
    });
...
TEST_P(WaylandWindowTest, CanDispatchEvent)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        wl_seat_send_capabilities(server->seat()->resource(), WL_SEAT_CAPABILITY_POINTER | WL_SEAT_CAPABILITY_KEYBOARD | WL_SEAT_CAPABILITY_TOUCH);
    });
    ASSERT_TRUE(connection_->seat()->pointer());
    ASSERT_TRUE(connection_->seat()->touch());
    ASSERT_TRUE(connection_->seat()->keyboard());
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        wl_pointer_send_enter(server->seat()->pointer()->resource(), server->GetNextSerial(), toplevel_surface->resource(), 0, 0);
    });

    // Only |window_| can dispatch MouseEvents.
    VerifyCanDispatchMouseEvents(window_.get(), { menu_window.get(), nested_menu_window.get() });
    VerifyCanDispatchTouchEvents({}, { window_.get(), menu_window.get(), nested_menu_window.get() });
    VerifyCanDispatchKeyEvents({}, { window_.get(), menu_window.get(), nested_menu_window.get() });

    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        wl::MockSurface* toplevel_surface = server->GetObject<wl::MockSurface>(id);
        wl::ScopedWlArray empty({});
        wl_keyboard_send_enter(server->seat()->keyboard()->resource(), server->GetNextSerial(), toplevel_surface->resource(), empty.get());
    });
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        wl::MockSurface* toplevel_surface = server->GetObject<wl::MockSurface>(id);
        wl_touch_send_down(server->seat()->touch()->resource(), server->GetNextSerial(), 0, toplevel_surface->resource(), 0 /* id */, wl_fixed_from_int(50),
    ...
    const uint32_t menu_window_surface_id = menu_window->root_surface()->get_surface_id();
    PostToServerAndWait([toplevel_surface_id = surface_id_, menu_window_surface_id](wl::TestWaylandServerThread* server) {
        wl::MockSurface* toplevel_surface = server->GetObject<wl::MockSurface>(toplevel_surface_id);
        wl::MockSurface* menu_window_surface = server->GetObject<wl::MockSurface>(menu_window_surface_id);
        wl_pointer_send_leave(server->seat()->pointer()->resource(), server->GetNextSerial(), toplevel_surface->resource());
        wl_pointer_send_enter(server->seat()->pointer()->resource(), server->GetNextSerial(), menu_window_surface->resource(), 0, 0);
        wl_touch_send_up(server->seat()->touch()->resource(), server->GetNextSerial(), 1000, 0 /* id */);
        wl_touch_send_frame(server->seat()->touch()->resource());

        wl_keyboard_send_leave(server->seat()->keyboard()->resource(), server->GetNextSerial(), toplevel_surface->resource());
    });

    // Only |menu_window| can dispatch MouseEvents.
    VerifyCanDispatchMouseEvents(menu_window.get(), { window_.get(), nested_menu_window.get() });
    VerifyCanDispatchTouchEvents({}, { window_.get(), menu_window.get(), nested_menu_window.get() });
    VerifyCanDispatchKeyEvents({}, { window_.get(), menu_window.get(), nested_menu_window.get() });

    const uint32_t nested_menu_window_surface_id = nested_menu_window->root_surface()->get_surface_id();
    PostToServerAndWait([nested_menu_window_surface_id, menu_window_surface_id](wl::TestWaylandServerThread* server) {
        wl::MockSurface* menu_window_surface = server->GetObject<wl::MockSurface>(menu_window_surface_id);
        wl::MockSurface* nested_menu_window_surface = server->GetObject<wl::MockSurface>(nested_menu_window_surface_id);

        wl_pointer_send_leave(server->seat()->pointer()->resource(), server->GetNextSerial(), menu_window_surface->resource());
        wl_pointer_send_enter(server->seat()->pointer()->resource(), server->GetNextSerial(), nested_menu_window_surface->resource(), 0, 0);
    });
...
TEST_P(WaylandWindowTest, DispatchWindowMove)
{
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        wl_pointer_send_enter(pointer_resource, server->GetNextSerial(), surface->resource(), 0, 0);
        wl_pointer_send_button(pointer_resource, server->GetNextSerial(), server->GetNextTime(), BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);
    });
    ...
    for (const int value : hit_test_values) {
        {
            ...
            PostToServerAndWait([id = surface_id_, direction](wl::TestWaylandServerThread* server) {
                wl::MockSurface* surface = server->GetObject<wl::MockSurface>(id);
                auto* xdg_surface = surface->xdg_surface();
                EXPECT_CALL(*xdg_surface->xdg_toplevel(), Resize(_, Eq(direction)));
            });
            wm_move_resize_handler->DispatchHostWindowDragMovement(value, gfx::Point());
        }
    }
}
...
TEST_P(WaylandWindowTest, ToplevelWindowUpdateWindowScale)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        auto* output2 = server->CreateAndInitializeOutput(wl::TestOutputMetrics({ 1920, 1080 }));
        output2->SetScale(2);
    ...
    const uint32_t output_id1 = GetObjIdForOutput(output_manager->GetAllOutputs().begin()->first);
    PostToServerAndWait([id = surface_id_, output_id1](wl::TestWaylandServerThread* server) {
    ...
    const uint32_t output_id2 = GetObjIdForOutput(output_manager->GetAllOutputs().rbegin()->first);

    PostToServerAndWait([id = surface_id_, output_id2, output_id1](wl::TestWaylandServerThread* server) {
        wl::MockSurface* surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(surface);
...
TEST_P(WaylandWindowTest, ToplevelWindowOnRotateFocus)
{
    ...
    PostToServerAndWait(
        [&](wl::TestWaylandServerThread* server) {
            server->GetObject<wl::MockSurface>(surface_id_)->xdg_surface()->xdg_toplevel()->zaura_toplevel()->set_ack_rotate_focus_callback(ack_cb.Get());
        },
        /*no_nested_runloops=*/false);
    SendConfigureEvent(surface_id_, { 10, 10 }, InitializeWlArrayWithActivatedState());
    ...
    // Forward, restart
    EXPECT_CALL(delegate_, OnRotateFocus(Direction::kForward, true)).WillOnce(Return(true));
    PostToServerAndWait(
        [&](wl::TestWaylandServerThread* server) {
            EXPECT_CALL(ack_cb, Run(serial, ZAURA_TOPLEVEL_ROTATE_HANDLED_STATE_HANDLED));

            auto* surface = server->GetObject<wl::MockSurface>(surface_id_);
            auto* toplevel = surface->xdg_surface()->xdg_toplevel()->zaura_toplevel();
            zaura_toplevel_send_rotate_focus(
                toplevel->resource(), serial++, ZAURA_TOPLEVEL_ROTATE_DIRECTION_FORWARD, ZAURA_TOPLEVEL_ROTATE_RESTART_STATE_RESTART);
        },
        /*no_nested_runloops=*/false);

    // Backward, no restart
    EXPECT_CALL(delegate_, OnRotateFocus(Direction::kBackward, false)).WillOnce(Return(true));
    PostToServerAndWait(
        [&](wl::TestWaylandServerThread* server) {
            EXPECT_CALL(ack_cb, Run(serial, ZAURA_TOPLEVEL_ROTATE_HANDLED_STATE_HANDLED));
            ...
            auto* toplevel = surface->xdg_surface()->xdg_toplevel()->zaura_toplevel();
            zaura_toplevel_send_rotate_focus(
                toplevel->resource(), serial++, ZAURA_TOPLEVEL_ROTATE_DIRECTION_BACKWARD, ZAURA_TOPLEVEL_ROTATE_RESTART_STATE_NO_RESTART);
        },
        /*no_nested_runloops=*/false);
    ...
    PostToServerAndWait(
        [&](wl::TestWaylandServerThread* server) {
            EXPECT_CALL(ack_cb, Run(serial, ZAURA_TOPLEVEL_ROTATE_HANDLED_STATE_NOT_HANDLED));
...
TEST_P(WaylandWindowTest, WaylandPopupSurfaceScale)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        // Creating an output with scale 2.
        auto* output2 = server->CreateAndInitializeOutput(wl::TestOutputMetrics({ 1920, 0, 1920, 1080 }));
        output2->SetScale(2);
        output2->SetDeviceScaleFactor(2);
    });
    WaitForAllDisplaysReady();
    ...
    std::vector<PlatformWindowType> window_types { PlatformWindowType::kMenu, PlatformWindowType::kTooltip };
    for (const auto& type : window_types) {
        ...
        auto wayland_popup = CreateWaylandWindowWithParams(type, wayland_popup_bounds, &delegate_, window_->GetWidget());
        ...
        // window.
        EXPECT_EQ(1, window_->applied_state().window_scale);
        EXPECT_EQ(window_->applied_state().window_scale, wayland_popup->applied_state().window_scale);
        EXPECT_EQ(wayland_popup_bounds.size(), wayland_popup->applied_state().size_px);
        EXPECT_EQ(wayland_popup_bounds, wayland_popup->GetBoundsInDIP());
        wayland_popup->Hide();

        PostToServerAndWait([id = surface_id_, output_id1, output_id2](wl::TestWaylandServerThread* server) {
            wl::MockSurface* surface = server->GetObject<wl::MockSurface>(id);
            ASSERT_TRUE(surface);
            wl::TestOutput* output1 = server->GetObject<wl::TestOutput>(output_id1);
            wl::TestOutput* output2 = server->GetObject<wl::TestOutput>(output_id2);
        ...
        // scale is changed.
        EXPECT_EQ(window_->applied_state().window_scale, wayland_popup->applied_state().window_scale);
        EXPECT_EQ(gfx::ScaleToCeiledSize(wayland_popup_bounds.size(), wayland_popup->applied_state().window_scale), wayland_popup->applied_state().size_px);

        wayland_popup->Hide();
        SetPointerFocusedWindow(nullptr);

        PostToServerAndWait([id = surface_id_, output_id2](wl::TestWaylandServerThread* server) {
            wl::MockSurface* surface = server->GetObject<wl::MockSurface>(id);
            ASSERT_TRUE(surface);
            wl::TestOutput* output2 = server->GetObject<wl::TestOutput>(output_id2);
...
TEST_P(WaylandWindowTest, WaylandPopupInitialBufferScale)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        server->CreateAndInitializeOutput(wl::TestOutputMetrics({ 1921, 0, 1920, 1080 }));
    });

    auto* output_manager = connection_->wayland_output_manager();
    EXPECT_EQ(2u, output_manager->GetAllOutputs().size());

    const WaylandOutput* main_output = output_manager->GetAllOutputs().begin()->second.get();
    const WaylandOutput* secondary_output = output_manager->GetAllOutputs().rbegin()->second.get();

    struct {
        raw_ptr<const WaylandOutput> output;
        const char* label;
    } screen[] = { { main_output, "main output" }, { secondary_output, "secondary output" } };

    for (const auto& entered_output : screen) {
        PostToServerAndWait([id = surface_id_, entered_output_id = GetObjIdForOutput(entered_output.output->output_id())](wl::TestWaylandServerThread* server) {
            wl::MockSurface* surface = server->GetObject<wl::MockSurface>(id);
            ASSERT_TRUE(surface);
            wl::TestOutput* output = server->GetObject<wl::TestOutput>(entered_output_id);
            wl_surface_send_enter(surface->resource(), output->resource());
        });
        for (auto main_output_scale = 1; main_output_scale < 5; main_output_scale++) {
            for (auto secondary_output_scale = 1; secondary_output_scale < 5; secondary_output_scale++) {
                PostToServerAndWait(
                    [main_output_id = GetObjIdForOutput(main_output->output_id()), secondary_output_id = GetObjIdForOutput(secondary_output->output_id()),
                        main_output_scale, secondary_output_scale](wl::TestWaylandServerThread* server) {
                        wl::TestOutput* main_output = server->GetObject<wl::TestOutput>(main_output_id);
                        wl::TestOutput* secondary_output = server->GetObject<wl::TestOutput>(secondary_output_id);
                ...
                std::unique_ptr<WaylandWindow> wayland_popup
                    = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, bounds_dip, &delegate_, window_->GetWidget());
                EXPECT_TRUE(wayland_popup);
                ...
                if (entered_output.output == secondary_output) {
                    expected_px_size = gfx::ScaleToCeiledSize(bounds_dip.size(), secondary_output_scale);
                }

                EXPECT_EQ(expected_px_size, wayland_popup->applied_state().size_px)
                    << " when the window is on " << entered_output.label << " that has scale " << entered_output.output->scale_factor();
            }
        }
        PostToServerAndWait([id = surface_id_, entered_output_id = GetObjIdForOutput(main_output->output_id())](wl::TestWaylandServerThread* server) {
            wl::MockSurface* surface = server->GetObject<wl::MockSurface>(id);
            ASSERT_TRUE(surface);
            wl::TestOutput* output = server->GetObject<wl::TestOutput>(entered_output_id);
...
TEST_P(WaylandWindowTest, WaylandPopupInitialBufferUsesParentScale)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        // Creating an output with scale 2.
        auto* output2 = server->CreateAndInitializeOutput(wl::TestOutputMetrics({ 1921, 0, 1920, 1080 }));
        output2->SetScale(2);
        output2->SetDeviceScaleFactor(2);
    });
    WaitForAllDisplaysReady();
    ...
    const uint32_t secondary_output_id = GetObjIdForOutput(output_manager->GetAllOutputs().rbegin()->first);

    PostToServerAndWait([id = surface_id_, secondary_output_id](wl::TestWaylandServerThread* server) {
        // Send the window to |output2|.
        wl::MockSurface* surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(surface);
        wl::TestOutput* output = server->GetObject<wl::TestOutput>(secondary_output_id);

        wl_surface_send_enter(surface->resource(), output->resource());
    });

    constexpr gfx::Rect kBoundsDip { 50, 50, 100, 100 };
    const gfx::Size expected_size_px = gfx::ScaleToCeiledSize(kBoundsDip.size(), 2);

    std::unique_ptr<WaylandWindow> wayland_popup = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, kBoundsDip, &delegate_, window_->GetWidget());
    EXPECT_TRUE(wayland_popup);
...
TEST_P(WaylandWindowTest, GetPreferredOutput)
{
    ...
    const uint32_t output1_id = screen_->GetOutputIdForDisplayId(screen_->GetAllDisplays().at(0).id());
    const uint32_t output2_id = screen_->GetOutputIdForDisplayId(screen_->GetAllDisplays().at(1).id());
    ...
    PostToServerAndWait([id = surface_id_, wl_output1_id, wl_output2_id](wl::TestWaylandServerThread* server) {
        // Send the window to |output1| and |output2|.
        wl_surface_send_enter(server->GetObject<wl::MockSurface>(id)->resource(), server->GetObject<wl::TestOutput>(wl_output1_id)->resource());
        wl_surface_send_enter(server->GetObject<wl::MockSurface>(id)->resource(), server->GetObject<wl::TestOutput>(wl_output2_id)->resource());
    });

    // The window entered two outputs.
    EXPECT_THAT(wayland_surface->entered_outputs(), ElementsAre(output1_id, output2_id));
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) {
        ...
        server->CreateAndInitializeOutput(wl::TestOutputMetrics({ 0, 1081, 1920, 1080 }));
    });

    WaitForAllDisplaysReady();

    ASSERT_EQ(3u, screen_->GetAllDisplays().size());
    const uint32_t output3_id = screen_->GetOutputIdForDisplayId(screen_->GetAllDisplays().at(2).id());

    EXPECT_EQ(3u, output_manager->GetAllOutputs().size());
    const uint32_t wl_output3_id = GetObjIdForOutput(output3_id);

    PostToServerAndWait([id = surface_id_, wl_output3_id](wl::TestWaylandServerThread* server) {
        // Send window into 3rd output.
        wl_surface_send_enter(server->GetObject<wl::MockSurface>(id)->resource(), server->GetObject<wl::TestOutput>(wl_output3_id)->resource());
    });

    // The window entered three outputs...
    EXPECT_THAT(wayland_surface->entered_outputs(), ElementsAre(output1_id, output2_id, output3_id));
    ...
    EXPECT_EQ(2, connection_->wayland_output_manager()->GetOutput(output2_id)->scale_factor());
...
TEST_P(WaylandWindowTest, GetChildrenPreferredOutput)
{
    ...
    MockWaylandPlatformWindowDelegate menu_window_delegate;
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(10, 10, 10, 10), &menu_window_delegate, window_->GetWidget());
    ...
    PostToServerAndWait([id = surface_id_, wl_output1_id](wl::TestWaylandServerThread* server) {
        wl_surface_send_enter(server->GetObject<wl::MockSurface>(id)->resource(), server->GetObject<wl::TestOutput>(wl_output1_id)->resource());
    });
    ...
    EXPECT_EQ(window_->GetPreferredEnteredOutputId(), output1_id);
    EXPECT_EQ(window_->GetPreferredEnteredOutputId(), menu_window->GetPreferredEnteredOutputId());

    // Send the menu window into output2, while the toplevel is still on output1.
    PostToServerAndWait([menu_surface_id, wl_output2_id](wl::TestWaylandServerThread* server) {
        wl_surface_send_enter(server->GetObject<wl::MockSurface>(menu_surface_id)->resource(), server->GetObject<wl::TestOutput>(wl_output2_id)->resource());
    });
    ...
    PostToServerAndWait([id = surface_id_, wl_output2_id](wl::TestWaylandServerThread* server) {
        wl_surface_send_enter(server->GetObject<wl::MockSurface>(id)->resource(), server->GetObject<wl::TestOutput>(wl_output2_id)->resource());
    });

    // The toplevel window has now entered 2 outputs, in chronological order.
    EXPECT_THAT(root_surface->entered_outputs(), ElementsAre(output1_id, output2_id));
    EXPECT_THAT(menu_surface->entered_outputs(), ElementsAre(output2_id));
...
TEST_P(WaylandWindowTest, PopupPassesDefaultAnchorInformation)
{
    ...
    menu_window_positioner = { gfx::Rect(439, 46, 1, 1), gfx::Size(287, 409), XDG_POSITIONER_ANCHOR_TOP_LEFT, XDG_POSITIONER_GRAVITY_BOTTOM_RIGHT,
        XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_FLIP_Y };
    nested_menu_window_positioner = { gfx::Rect(285, 1, 1, 1), gfx::Size(305, 99), XDG_POSITIONER_ANCHOR_TOP_LEFT, XDG_POSITIONER_GRAVITY_BOTTOM_RIGHT,
        XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_FLIP_Y };
    ...
    // be normal.
    MockWaylandPlatformWindowDelegate menu_window_delegate;
    EXPECT_CALL(menu_window_delegate, GetOwnedWindowAnchorAndRectInDIP()).WillOnce(Return(std::nullopt));
    gfx::Rect menu_window_bounds(gfx::Point(439, 46), menu_window_positioner.size);
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window_bounds, &menu_window_delegate, toplevel_window->GetWidget());
    EXPECT_TRUE(menu_window);
    ...
    MockWaylandPlatformWindowDelegate nested_menu_window_delegate;
    gfx::Rect nested_menu_window_bounds(gfx::Point(724, 47), nested_menu_window_positioner.size);
    std::unique_ptr<WaylandWindow> nested_menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, nested_menu_window_bounds, &nested_menu_window_delegate, menu_window->GetWidget());
    EXPECT_TRUE(nested_menu_window);

    VerifyXdgPopupPosition(nested_menu_window.get(), nested_menu_window_positioner);
}
...
TEST_P(WaylandWindowTest, PopupPassesSetAnchorInformation)
{
    ...
    menu_window_positioner = { gfx::Rect(468, 46, 28, 28), gfx::Size(320, 404), XDG_POSITIONER_ANCHOR_BOTTOM_RIGHT, XDG_POSITIONER_GRAVITY_BOTTOM_LEFT,
        XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_FLIP_Y };
    nested_menu_window_positioner = { gfx::Rect(4, 83, 312, 1), gfx::Size(480, 294), XDG_POSITIONER_ANCHOR_TOP_RIGHT, XDG_POSITIONER_GRAVITY_BOTTOM_RIGHT,
        XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_FLIP_Y | XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_FLIP_X };

    auto* toplevel_window = window_.get();
    toplevel_window->SetBoundsInDIP(gfx::Rect(508, 212));

    MockWaylandPlatformWindowDelegate menu_window_delegate;
    ui::OwnedWindowAnchor anchor = { gfx::Rect(menu_window_positioner.anchor_rect), OwnedWindowAnchorPosition::kBottomRight,
        OwnedWindowAnchorGravity::kBottomLeft, OwnedWindowConstraintAdjustment::kAdjustmentFlipY };
    EXPECT_CALL(menu_window_delegate, GetOwnedWindowAnchorAndRectInDIP()).WillOnce(Return(anchor));
    gfx::Rect menu_window_bounds(gfx::Point(176, 74), menu_window_positioner.size);
    std::unique_ptr<WaylandWindow> menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window_bounds, &menu_window_delegate, toplevel_window->GetWidget());
    EXPECT_TRUE(menu_window);
    ...
    MockWaylandPlatformWindowDelegate nested_menu_window_delegate;
    anchor = { { 180, 157, 312, 1 }, OwnedWindowAnchorPosition::kTopRight, OwnedWindowAnchorGravity::kBottomRight,
        OwnedWindowConstraintAdjustment::kAdjustmentFlipY | OwnedWindowConstraintAdjustment::kAdjustmentFlipX };
    EXPECT_CALL(nested_menu_window_delegate, GetOwnedWindowAnchorAndRectInDIP()).WillOnce(Return(anchor));
    gfx::Rect nested_menu_window_bounds(gfx::Point(492, 157), nested_menu_window_positioner.size);
    std::unique_ptr<WaylandWindow> nested_menu_window
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, nested_menu_window_bounds, &nested_menu_window_delegate, menu_window->GetWidget());
    EXPECT_TRUE(nested_menu_window);
...
TEST_P(WaylandWindowTest, SetBoundsResizesEmptySizes)
{
    ...
    std::unique_ptr<WaylandWindow> popup
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window_bounds, &popup_delegate, toplevel_window->GetWidget());
    EXPECT_TRUE(popup);

    popup->SetBoundsInDIP({ 0, 0, 0, 0 });

    VerifyXdgPopupPosition(popup.get(),
        { gfx::Rect(0, 0, 1, 1), gfx::Size(1, 1), XDG_POSITIONER_ANCHOR_TOP_LEFT, XDG_POSITIONER_GRAVITY_BOTTOM_RIGHT,
            XDG_POSITIONER_CONSTRAINT_ADJUSTMENT_FLIP_Y });
}
...
TEST_P(WaylandWindowTest, SetOpaqueRegion)
{
    ...
    SkIRect rect = SkIRect::MakeXYWH(0, 0, new_bounds.width(), new_bounds.height());
...
TEST_P(WaylandWindowTest, WaylandPopupSimpleParent)
{
    ...
    std::unique_ptr<WaylandWindow> wayland_popup
        = CreateWaylandWindowWithParams(PlatformWindowType::kTooltip, wayland_popup_bounds, &delegate_, window_->GetWidget());
    EXPECT_TRUE(wayland_popup);

    wayland_popup->Show(false);

    PostToServerAndWait([id = wayland_popup->root_surface()->get_surface_id(), wayland_popup_bounds](wl::TestWaylandServerThread* server) {
        auto* mock_surface_popup = server->GetObject<wl::MockSurface>(id);
        auto* mock_xdg_popup = mock_surface_popup->xdg_surface()->xdg_popup();

        EXPECT_EQ(mock_xdg_popup->anchor_rect().origin(), wayland_popup_bounds.origin());
        EXPECT_EQ(mock_surface_popup->opaque_region(), gfx::Rect(wayland_popup_bounds.size()));
    });
...
TEST_P(WaylandWindowTest, WaylandPopupNestedParent)
{
    ...
    auto menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window_bounds, &delegate_, window_->GetWidget());
    EXPECT_TRUE(menu_window);
    ...
    std::vector<PlatformWindowType> window_types { PlatformWindowType::kMenu, PlatformWindowType::kTooltip };
    for (const auto& type : window_types) {
        gfx::Rect nested_wayland_popup_bounds(gfx::Point(15, 15), gfx::Size(10, 10));
        auto nested_wayland_popup = CreateWaylandWindowWithParams(type, nested_wayland_popup_bounds, &delegate_, menu_window->GetWidget());
        ...
        PostToServerAndWait([id = nested_wayland_popup->root_surface()->get_surface_id(), nested_wayland_popup_bounds, menu_window_bounds](
                                wl::TestWaylandServerThread* server) {
            auto* mock_surface_nested = server->GetObject<wl::MockSurface>(id);
            auto* mock_xdg_popup_nested = mock_surface_nested->xdg_surface()->xdg_popup();

            auto new_origin = nested_wayland_popup_bounds.origin() - menu_window_bounds.origin().OffsetFromOrigin();
            EXPECT_EQ(mock_xdg_popup_nested->anchor_rect().origin(), new_origin);
            EXPECT_EQ(mock_surface_nested->opaque_region(), gfx::Rect(nested_wayland_popup_bounds.size()));
        });
...
TEST_P(WaylandWindowTest, SizeConstraintsExternal)
{
    ...
    EXPECT_CALL(delegate_, GetMinimumSizeForWindow()).WillRepeatedly(Return(kMinSize));
    EXPECT_CALL(delegate_, GetMaximumSizeForWindow()).WillRepeatedly(Return(kMaxSize));
...
TEST_P(WaylandWindowTest, OnSizeConstraintsChanged)
{
    ...
    for (bool has_min_size : kBooleans) {
        for (bool has_max_size : kBooleans) {
            std::optional<gfx::Size> min_size = has_min_size ? std::optional<gfx::Size>(gfx::Size(100, 200)) : std::nullopt;
            std::optional<gfx::Size> max_size = has_max_size ? std::optional<gfx::Size>(gfx::Size(300, 400)) : std::nullopt;
            EXPECT_CALL(delegate_, GetMinimumSizeForWindow()).WillOnce(Return(min_size));
            EXPECT_CALL(delegate_, GetMaximumSizeForWindow()).WillOnce(Return(max_size));

            PostToServerAndWait([id = surface_id_, has_min_size, has_max_size](wl::TestWaylandServerThread* server) {
                auto* mock_surface = server->GetObject<wl::MockSurface>(id);
                auto* mock_xdg_surface = mock_surface->xdg_surface();
                EXPECT_CALL(*mock_xdg_surface->xdg_toplevel(), SetMinSize(100, 200)).Times(has_min_size ? 1 : 0);
                EXPECT_CALL(*mock_xdg_surface->xdg_toplevel(), SetMaxSize(300, 400)).Times(has_max_size ? 1 : 0);
            });
...
{
    MockWaylandPlatformWindowDelegate delegate;
    auto window = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::Rect(100, 100), &delegate);
    ASSERT_TRUE(window);
...
{
    MockWaylandPlatformWindowDelegate delegate;
    auto window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(50, 50), &delegate, window_->GetWidget());
    ASSERT_TRUE(window);
...
TEST_P(WaylandWindowTest, ReattachesBackgroundOnShow)
{
    ...
    buffer_manager_gpu_->CreateShmBasedBuffer(MakeFD(), length, buffer_size, buffer_id1);
    buffer_manager_gpu_->CreateShmBasedBuffer(MakeFD(), length, buffer_size, buffer_id2);
    ...
    buffer_manager_gpu_->CommitOverlays(window->GetWidget(), 1u, gfx::FrameData(), std::move(overlays));
    ...
    buffer_manager_gpu_->CommitOverlays(window->GetWidget(), 2u, gfx::FrameData(), std::move(overlays));
...
TEST_P(WaylandWindowTest, SetsPropertiesOnShow)
{
    ...
    const uint32_t surface_id = window->root_surface()->get_surface_id();
    PostToServerAndWait([surface_id, app_id = window->GetWindowUniqueId()](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(surface_id);
    ...
    EXPECT_CALL(delegate, GetMinimumSizeForWindow()).WillRepeatedly(Return(min_size));
    EXPECT_CALL(delegate, GetMaximumSizeForWindow()).WillRepeatedly(Return(max_size));

    PostToServerAndWait([surface_id, min_size, max_size, kTitle](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(surface_id);
        auto* mock_xdg_toplevel = mock_surface->xdg_surface()->xdg_toplevel();
        EXPECT_CALL(*mock_xdg_toplevel, SetMinSize(min_size.value().width(), min_size.value().height()));
        EXPECT_CALL(*mock_xdg_toplevel, SetMaxSize(max_size.value().width(), max_size.value().height()));
    ...
    PostToServerAndWait([surface_id, min_size, max_size, kTitle, app_id = window->GetWindowUniqueId()](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(surface_id);
        auto* mock_xdg_toplevel = mock_surface->xdg_surface()->xdg_toplevel();
...
TEST_P(WaylandWindowTest, CreatesPopupOnButtonPressSerial)
{
    for (bool use_explicit_grab : { false, true }) {
        ...
        if (use_explicit_grab) {
            command_line_.GetProcessCommandLine()->AppendSwitch(switches::kUseWaylandExplicitGrab);
        }

        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            wl_seat_send_capabilities(server->seat()->resource(), WL_SEAT_CAPABILITY_POINTER | WL_SEAT_CAPABILITY_KEYBOARD);
        });
        ...
        PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
            wl::MockSurface* toplevel_surface = server->GetObject<wl::MockSurface>(id);
            wl::ScopedWlArray empty({});
            wl_keyboard_send_enter(server->seat()->keyboard()->resource(), keyboard_enter_serial, toplevel_surface->resource(), empty.get());

            wl_pointer_send_enter(
                server->seat()->pointer()->resource(), pointer_enter_serial, toplevel_surface->resource(), wl_fixed_from_int(0), wl_fixed_from_int(0));

            // Send two events - button down and button up.
            wl_pointer_send_button(server->seat()->pointer()->resource(), button_press_serial, 1002, BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);
            wl_pointer_send_button(server->seat()->pointer()->resource(), button_release_serial, 1004, BTN_LEFT, WL_POINTER_BUTTON_STATE_RELEASED);
        });

        // Create a popup window and verify the client used correct serial.
        MockWaylandPlatformWindowDelegate delegate;
        auto popup = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(50, 50), &delegate, window_->GetWidget());
        ASSERT_TRUE(popup);
...
TEST_P(WaylandWindowTest, CreatesPopupOnTouchDownSerial)
{
    for (bool use_explicit_grab : { false, true }) {
        ...
        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            wl_seat_send_capabilities(server->seat()->resource(), WL_SEAT_CAPABILITY_TOUCH | WL_SEAT_CAPABILITY_KEYBOARD);
        });
        ...
        PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
            wl::MockSurface* toplevel_surface = server->GetObject<wl::MockSurface>(id);
            wl::ScopedWlArray empty({});
            wl_keyboard_send_enter(server->seat()->keyboard()->resource(), enter_serial, toplevel_surface->resource(), empty.get());

            // Send two events - touch down and touch up.
            wl_touch_send_down(server->seat()->touch()->resource(), touch_down_serial, 0, toplevel_surface->resource(), 0 /* id */, wl_fixed_from_int(50),
                wl_fixed_from_int(100));
            wl_touch_send_frame(server->seat()->touch()->resource());

            wl_touch_send_up(server->seat()->touch()->resource(), touch_up_serial, 1000, 0 /* id */);
            wl_touch_send_frame(server->seat()->touch()->resource());
        });
        ...
        PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
            ...
            if (base::nix::GetDesktopEnvironment(env.get()) == base::nix::DESKTOP_ENVIRONMENT_GNOME) {
...
TEST_P(WaylandWindowTest, NestedPopupWindowsGetCorrectParent)
{
    ...
    std::unique_ptr<WaylandWindow> menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window_bounds, &delegate_, window_->GetWidget());
    EXPECT_TRUE(menu_window);

    EXPECT_TRUE(menu_window->parent_window() == window_.get());

    gfx::Rect menu_window_bounds2(gfx::Rect(20, 40, 30, 20));
    std::unique_ptr<WaylandWindow> menu_window2
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window_bounds2, &delegate_, menu_window->GetWidget());
    ...
    std::unique_ptr<WaylandWindow> menu_window3
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window_bounds3, &delegate_, menu_window2->GetWidget());
    ...
    std::unique_ptr<WaylandWindow> menu_window4
        = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, menu_window_bounds4, &delegate_, menu_window3->GetWidget());
...
TEST_P(WaylandWindowTest, DoesNotGrabPopupIfNoSeat)
{
    ...
    PostToServerAndWait([surface_id = popup->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
        auto* test_popup = GetTestXdgPopupByWindow(server, surface_id);
        ASSERT_TRUE(test_popup);
        EXPECT_EQ(test_popup->grab_serial(), 0u);
    });
...
TEST_P(WaylandWindowTest, DoesNotGrabPopupUnlessParentHasGrab)
{
    ...
    root_menu = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(50, 50), &delegate, window_->GetWidget());
    VerifyAndClearExpectations();
    Mock::VerifyAndClearExpectations(&delegate);
    ASSERT_TRUE(root_menu);

    PostToServerAndWait([surface_id = root_menu->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
    ...
    PostToServerAndWait([surface_id = root_menu->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
        auto* server_root_menu_surface = server->GetObject<wl::MockSurface>(surface_id);
        ASSERT_TRUE(server_root_menu_surface);

        auto* pointer_resource = server->seat()->pointer()->resource();
        wl_pointer_send_enter(pointer_resource, 3u /*serial*/, server_root_menu_surface->resource(), 0, 0);
        wl_pointer_send_frame(pointer_resource);
        wl_pointer_send_button(pointer_resource, 4u /*serial*/, 1, BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);
        wl_pointer_send_frame(pointer_resource);
    });
    Mock::VerifyAndClearExpectations(&delegate);
    ...
    child_menu = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(10, 10), &delegate_2, root_menu->GetWidget());
    VerifyAndClearExpectations();
    Mock::VerifyAndClearExpectations(&delegate_2);
    ASSERT_TRUE(child_menu);

    PostToServerAndWait([surface_id = child_menu->root_surface()->get_surface_id(), root_menu_id = root_menu->root_surface()->get_surface_id()](
                            wl::TestWaylandServerThread* server) {
        ...
        auto* pointer_resource = server->seat()->pointer()->resource();
        auto* server_root_menu_surface = server->GetObject<wl::MockSurface>(root_menu_id);
        wl_pointer_send_leave(pointer_resource, 5u /*serial*/, server_root_menu_surface->resource());
        wl_pointer_send_frame(pointer_resource);
    });
}
...
TEST_P(WaylandWindowTest, InitialBounds)
{
    ...
    auto toplevel = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::Rect(10, 10, 200, 200), &delegate_2);
    {
        WaylandWindow::WindowStates window_states;
...
{
    testing::NiceMock<MockWaylandPlatformWindowDelegate> delegate_2;
    auto toplevel = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::Rect(0, 0, 200, 200), &delegate_2);
    {
        WaylandWindow::WindowStates window_states;
        window_states.is_maximized = false;
        window_states.is_fullscreen = false;
...
TEST_P(WaylandWindowTest, SecondarySnappedState)
{
    ...
    auto toplevel = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::Rect(0, 0, 200, 200), &delegate_2);
    {
        ...
        toplevel->HandleToplevelConfigureWithOrigin(100, 0, 100, 200, window_states);
    }
    toplevel->HandleSurfaceConfigure(2);
...
protected:
    void OneWaylandSubsurfaceTestHelper(const gfx::RectF& subsurface_bounds, const gfx::RectF& expected_subsurface_bounds)
    {
        VerifyAndClearExpectations();

        std::unique_ptr<WaylandWindow> window = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::Rect(640, 480), &delegate_);
        EXPECT_TRUE(window);
        ...
        WaylandSubsurface* wayland_subsurface = window->wayland_subsurfaces().begin()->get();

        PostToServerAndWait([subsurface_id = wayland_subsurface->wayland_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
            auto* mock_surface_subsurface = server->GetObject<wl::MockSurface>(subsurface_id);
            EXPECT_TRUE(mock_surface_subsurface);
        });
        wayland_subsurface->ConfigureAndShowSurface(subsurface_bounds, gfx::RectF(0, 0, 640, 480) /*parent_bounds_px*/, std::nullopt /*clip_rect_px*/,
            gfx::OVERLAY_TRANSFORM_NONE, 1.f /*buffer_scale*/, nullptr, nullptr);
        connection_->Flush();

        PostToServerAndWait([surface_id = window->root_surface()->get_surface_id(), subsurface_id = wayland_subsurface->wayland_surface()->get_surface_id(),
                                expected_subsurface_bounds](wl::TestWaylandServerThread* server) {
            auto* mock_surface_root_window = server->GetObject<wl::MockSurface>(surface_id);
            auto* mock_surface_subsurface = server->GetObject<wl::MockSurface>(subsurface_id);
            EXPECT_TRUE(mock_surface_subsurface);
            ...
            gfx::PointF expected_position(wl_fixed_to_double(wl_fixed_from_double(expected_subsurface_bounds.x())),
                wl_fixed_to_double(wl_fixed_from_double(expected_subsurface_bounds.y())));
    ...
    {
        VerifyAndClearExpectations();
        std::vector<WaylandSubsurface*> res = std::vector<WaylandSubsurface*> { window_->primary_subsurface() };
...
TEST_P(WaylandSubsurfaceTest, OneWaylandSubsurfaceInteger)
{
    ...
    constexpr std::array<std::array<gfx::RectF, 2>, 2> test_data
        = { { { gfx::RectF({ 15.12, 15.912 }, { 10.351, 10.742 }), gfx::RectF({ 16, 16 }, { 11, 11 }) },
            { gfx::RectF({ 7.041, 8.583 }, { 13.452, 20.231 }), gfx::RectF({ 7.041, 8.583 }, { 13.452, 20.231 }) } } };

    for (const auto& item : test_data) {
        OneWaylandSubsurfaceTestHelper(item[0] /* subsurface_bounds */, item[1] /* expected_subsurface_bounds */);
...
TEST_P(WaylandSubsurfaceTest, OneWaylandSubsurfaceNonInteger)
{
    ...
    constexpr std::array<std::array<gfx::RectF, 2>, 2> test_data = { { { gfx::RectF({ 15, 15 }, { 10, 10 }), gfx::RectF({ 15, 15 }, { 10, 10 }) },
...
TEST_P(WaylandSubsurfaceTest, NoDuplicateSubsurfaceRequests)
{
    ...
    for (auto* subsurface : subsurfaces) {
        subsurface->ConfigureAndShowSurface(
            gfx::RectF(1.f, 2.f, 10.f, 20.f), gfx::RectF(0.f, 0.f, 800.f, 600.f), std::nullopt, gfx::OVERLAY_TRANSFORM_NONE, 1.f, nullptr, nullptr);
    }
    connection_->Flush();

    PostToServerAndWait(
        [subsurface_id1 = subsurfaces[0]->wayland_surface()->get_surface_id(), subsurface_id2 = subsurfaces[1]->wayland_surface()->get_surface_id(),
            subsurface_id3 = subsurfaces[2]->wayland_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
            // From top to bottom: subsurfaces[2], subsurfaces[1], subsurfaces[0].
            wl::TestSubSurface* test_subs[3] = { server->GetObject<wl::MockSurface>(subsurface_id1)->sub_surface(),
                server->GetObject<wl::MockSurface>(subsurface_id2)->sub_surface(), server->GetObject<wl::MockSurface>(subsurface_id3)->sub_surface() };
    ...
    subsurfaces[0]->ConfigureAndShowSurface(
        gfx::RectF(0.f, 0.f, 10.f, 20.f), gfx::RectF(0.f, 0.f, 800.f, 600.f), std::nullopt, gfx::OVERLAY_TRANSFORM_NONE, 1.f, subsurfaces[2], nullptr);
    subsurfaces[1]->ConfigureAndShowSurface(
        gfx::RectF(1.f, 2.f, 10.f, 20.f), gfx::RectF(0.f, 0.f, 800.f, 600.f), std::nullopt, gfx::OVERLAY_TRANSFORM_NONE, 1.f, nullptr, subsurfaces[2]);
    subsurfaces[2]->ConfigureAndShowSurface(
        gfx::RectF(1.f, 2.f, 10.f, 20.f), gfx::RectF(0.f, 0.f, 800.f, 600.f), std::nullopt, gfx::OVERLAY_TRANSFORM_NONE, 1.f, nullptr, subsurfaces[0]);
    connection_->Flush();
...
TEST_P(WaylandWindowTest, NoDuplicateViewporterRequests)
{
    ...
    PostToServerAndWait([surface_id = surface->get_surface_id()](wl::TestWaylandServerThread* server) {
        auto* test_viewport = server->GetObject<wl::MockSurface>(surface_id)->viewport();
    ...
    surface->AttachBuffer(connection_->buffer_manager_host()->EnsureBufferHandle(surface, buffer_id));
...
TEST_P(WaylandWindowTest, RepositionPopups)
{
    ...
    PostToServerAndWait([id = menu_window->root_surface()->get_surface_id(), menu_window_bounds](wl::TestWaylandServerThread* server) {
        auto* mock_surface_popup = server->GetObject<wl::MockSurface>(id);
        auto* mock_xdg_popup = mock_surface_popup->xdg_surface()->xdg_popup();

        EXPECT_EQ(mock_xdg_popup->anchor_rect().origin(), menu_window_bounds.origin());
        EXPECT_EQ(mock_xdg_popup->size(), menu_window_bounds.size());
        EXPECT_EQ(mock_surface_popup->opaque_region(), gfx::Rect(menu_window_bounds.size()));
    });

    VerifyAndClearExpectations();

    const gfx::Rect damage_rect { menu_window_bounds.width(), menu_window_bounds.height() };
    ...
    SendConfigureEvent(menu_window->root_surface()->get_surface_id(), { 0, 0 }, wl::ScopedWlArray({}));
...
TEST_P(WaylandWindowTest, StartWithMinimized)
{
    ...
    SendConfigureEvent(surface_id_, { 0, 0 }, InitializeWlArrayWithActivatedState());
...
public:
    BlockableWaylandToplevelWindow(MockWaylandPlatformWindowDelegate* delegate, WaylandConnection* connection)
        : WaylandToplevelWindow(delegate, connection)
    {
    }

    static std::unique_ptr<BlockableWaylandToplevelWindow> Create(
        const gfx::Rect& bounds, WaylandConnection* connection, MockWaylandPlatformWindowDelegate* delegate)
    {
        auto window = std::make_unique<BlockableWaylandToplevelWindow>(delegate, connection);
    ...
    uint32_t DispatchEvent(const PlatformEvent& platform_event) override
    {
        ...
        if (event->type() == EventType::kTouchReleased && !blocked_) {
            ...
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(async_task_), run_loop.QuitClosure()));
            run_loop.Run();
            blocked_ = false;
        }
    ...
    void SetAsyncTask(base::RepeatingCallback<void(base::OnceClosure)> async_task)
    {
...
TEST_P(WaylandWindowTest, BlockingTouchDownUp_NoCrash)
{
    ...
    MockWaylandPlatformWindowDelegate delegate;
    auto window = BlockableWaylandToplevelWindow::Create(gfx::Rect(800, 600), connection_.get(), &delegate);
    ...
    auto async_task = base::BindLambdaForTesting([&](base::OnceClosure closure) {
        PostToServerAndWait([id = window->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
            wl::MockSurface* toplevel_surface = server->GetObject<wl::MockSurface>(id);
            wl_touch_send_down(server->seat()->touch()->resource(), server->GetNextSerial(), 0, toplevel_surface->resource(), 0 /* id */,
                wl_fixed_from_int(100), wl_fixed_from_int(100));
            wl_touch_send_up(server->seat()->touch()->resource(), server->GetNextSerial(), 2000, 0 /* id */);
            wl_touch_send_frame(server->seat()->touch()->resource());
        });
...
TEST_P(WaylandWindowTest, ChangeFocusDuringDispatch)
{
    ...
    EXPECT_CALL(other_delegate, OnAcceleratedWidgetAvailable(_)).WillOnce(SaveArg<0>(&other_widget));

    PlatformWindowInitProperties properties;
    properties.bounds = gfx::Rect(10, 10);
    properties.type = PlatformWindowType::kWindow;
    auto other_window = WaylandWindow::Create(&other_delegate, connection_.get(), std::move(properties));
    ...
    EXPECT_CALL(delegate_, DispatchEvent(_)).WillRepeatedly([&](Event* event) {
        ...
        if (event->type() == ui::EventType::kMousePressed) {
            PostToServerAndWait([id = surface_id_, other_id = other_window->root_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
                wl::MockSurface* surface = server->GetObject<wl::MockSurface>(id);
                ASSERT_TRUE(surface);
                wl::MockSurface* other_surface = server->GetObject<wl::MockSurface>(other_id);
                ...
                wl_pointer_send_enter(pointer->resource(), 4, other_surface->resource(), 0, 0);
                wl_pointer_send_frame(pointer->resource());
            });
        }
    });
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        wl_pointer_send_frame(pointer->resource());
        wl_pointer_send_button(pointer->resource(), 2, 1004, BTN_LEFT, WL_POINTER_BUTTON_STATE_PRESSED);
        wl_pointer_send_frame(pointer->resource());
    });
...
TEST_P(WaylandWindowTest, WindowMovedResized)
{
    ...
    PostToServerAndWait([id = surface_id_, new_bounds](wl::TestWaylandServerThread* server) {
        wl::MockSurface* surface = server->GetObject<wl::MockSurface>(id);
        auto* xdg_surface = surface->xdg_surface();
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(gfx::Rect(new_bounds.size()))).Times(0);
    });
    window_->SetBoundsInDIP(new_bounds);
    AdvanceFrameToCurrent(window_.get(), delegate_);
    VerifyAndClearExpectations();
...
{
    VerifyAndClearExpectations();
    auto* primary_output = connection_->wayland_output_manager()->GetPrimaryOutput();
    constexpr float kScales[] = { display::kDsf_1_777, display::kDsf_2_252, display::kDsf_2_666, display::kDsf_1_8 };
    for (float scale : kScales) {
        ...
        std::unique_ptr<WaylandWindow> wayland_window = CreateWaylandWindowWithParams(PlatformWindowType::kWindow, gfx::Rect(20, 0, 100, 100), &delegate);
        for (int i = 100; i < 3000; i++) {
            const gfx::Rect kBoundsDip { 20, 0, i, 3000 - i };
            const gfx::Rect bounds_in_px = delegate_.ConvertRectToPixels(gfx::Rect(kBoundsDip.size()));
...
TEST_P(WaylandWindowTest, ScaleChangeWhenStateRequestThrottoled)
{
    ...
    EXPECT_EQ(bounds_dip.size(), delegate_.ConvertRectToPixels(bounds_dip).size());
    ...
    EXPECT_NE(bounds_dip.size(), delegate_.ConvertRectToPixels(bounds_dip).size());
    EXPECT_EQ(window_->applied_state().size_px, delegate_.ConvertRectToPixels(bounds_dip).size());
    VerifyAndClearExpectations();
}
...
TEST_P(WaylandWindowTest, SetShape)
{
    ...
    const PlatformWindow::ShapeRects shape_rects = { { 10, 10, 40, 40 }, { 20, 20, 50, 50 } };
    wl::TestRegion shape_region;
    for (const auto& rect : shape_rects) {
        shape_region.op(SkIRect::MakeXYWH(rect.x(), rect.y(), rect.width(), rect.height()), SkRegion::kUnion_Op);
    }
...
TEST_P(WaylandWindowTest, OverviewMode)
{
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        auto* surface = server->GetObject<wl::MockSurface>(surface_id_);
        auto* toplevel = surface->xdg_surface()->xdg_toplevel()->zaura_toplevel();
        zaura_toplevel_send_overview_change(toplevel->resource(), ZAURA_TOPLEVEL_IN_OVERVIEW_IN_OVERVIEW);
    });
    ...
    PostToServerAndWait([&](wl::TestWaylandServerThread* server) {
        auto* surface = server->GetObject<wl::MockSurface>(surface_id_);
        auto* toplevel = surface->xdg_surface()->xdg_toplevel()->zaura_toplevel();
        zaura_toplevel_send_overview_change(toplevel->resource(), ZAURA_TOPLEVEL_IN_OVERVIEW_NOT_IN_OVERVIEW);
    });
}
#endif
...
TEST_P(WaylandWindowTest, SetUnsetFloat)
{
    ...
    window_->AsWaylandToplevelWindow()->SetFloatToLocation(ui::WaylandFloatStartLocation::kBottomRight);
    EXPECT_CALL(set_unset_float_cb, Run(/*floated=*/true, 0));
    post_to_server_and_wait();

    window_->AsWaylandToplevelWindow()->SetFloatToLocation(ui::WaylandFloatStartLocation::kBottomLeft);
...
TEST_P(WaylandWindowTest, UnsynchronizedOcclusionStateNotOverridenByConfigureOcclusionState)
{
    if (GetParam().enable_aura_shell != wl::EnableAuraShellProtocol::kEnabled) {
        GTEST_SKIP();
    }
    ...
    EXPECT_CALL(delegate_, OnOcclusionStateChanged(ui::PlatformWindowOcclusionState::kHidden)).Times(2);
    EXPECT_CALL(delegate_, OnOcclusionStateChanged(ui::PlatformWindowOcclusionState::kVisible)).Times(0);
...
TEST_P(WaylandWindowTest, ReentrantApplyStateWorks)
{
    ...
    PostToServerAndWait([id = surface_id_, bounds = kBounds1](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        auto* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(gfx::Rect(bounds.size()))).Times(1);
        EXPECT_CALL(*xdg_surface, AckConfigure(_)).Times(0);
    });
    ...
    PostToServerAndWait([id = surface_id_, bounds = kBounds3](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        auto* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(gfx::Rect(bounds.size()))).Times(1);
    ...
    delegate_.set_on_state_update_callback(base::BindLambdaForTesting([&]() { window_->SetBoundsInDIP(kBounds3); }));
...
TEST_P(WaylandWindowTest, ConfigureWithSameStateAcksAndCommitsImmediately)
{
    ...
    PostToServerAndWait([id = surface_id_, bounds = kBounds](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface);
        auto* xdg_surface = mock_surface->xdg_surface();
        EXPECT_CALL(*xdg_surface, SetWindowGeometry(gfx::Rect(bounds.size()))).Times(1);
...
public:
    ...
    void SetUp() override
    {
        test_screen_.display_list().AddDisplay(display::Display(kPrimaryDisplayId, kPrimaryDisplayBounds), display::DisplayList::Type::PRIMARY);
        test_screen_.display_list().AddDisplay(display::Display(kSecondaryDisplayId, kSecondaryDisplayBounds), display::DisplayList::Type::NOT_PRIMARY);
...
TEST_P(MultiDisplayWaylandWindowTest, SetsNewWindowBoundsToCorrectDisplay)
{
    ...
    auto window = delegate.CreateWaylandWindow(connection_.get(), std::move(init_properties));
    ASSERT_TRUE(window);
    ...
    // Init a new window with empty init bounds.
    window = delegate.CreateWaylandWindow(connection_.get(), std::move(init_properties));
    ASSERT_TRUE(window);
...
TEST_P(MultiDisplayWaylandWindowTest, NewWindowsRespectInitParamBounds)
{
    ...
    // Set the secondary display as the new window target.
    const display::ScopedDisplayForNewWindows scoped_display_new_windows(kSecondaryDisplayId);

    // Init a new window with non-empty bounds.
    constexpr gfx::Rect kInitBounds(100, 100, 100, 100);
    const auto window = delegate.CreateWaylandWindow(connection_.get(), PlatformWindowInitProperties(kInitBounds));
    ASSERT_TRUE(window);
...
public:
    ...
    PerSurfaceScaleWaylandWindowTest(const PerSurfaceScaleWaylandWindowTest&) = delete;
    PerSurfaceScaleWaylandWindowTest& operator=(const PerSurfaceScaleWaylandWindowTest&) = delete;

    void SetUp() override
    {
        CHECK(!base::Contains(enabled_features_, features::kWaylandPerSurfaceScale));
...
TEST_P(PerSurfaceScaleWaylandWindowTest, UsePreferredSurfaceScale)
{
    ...
    EXPECT_EQ(1.0f, screen_->GetDisplayForAcceleratedWidget(window_->GetWidget()).device_scale_factor());
    ...
    EXPECT_EQ(1.0f, screen_->GetPreferredScaleFactorForAcceleratedWidget(window_->GetWidget()).value_or(0.f));
    ...
    EXPECT_EQ(1.5f, screen_->GetPreferredScaleFactorForAcceleratedWidget(window_->GetWidget()).value_or(0.f));
...
TEST_P(PerSurfaceScaleWaylandWindowTest, UiScale_HandleFontScaleChange)
{
    ...
    EXPECT_EQ(1.0f, window_->GetPreferredScaleFactor().value_or(0.f));
    EXPECT_EQ(1.25f, screen_->GetPreferredScaleFactorForAcceleratedWidget(window_->GetWidget()).value_or(0.f));
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        EXPECT_CALL(*mock_surface->xdg_surface(), SetWindowGeometry(gfx::Rect(800, 600)));
...
TEST_P(PerSurfaceScaleWaylandWindowTest, UiScale_HandleServerTriggeredBoundsChange)
{
    base::test::ScopedFeatureList enable_ui_scaling(features::kWaylandUiScale);
    ASSERT_TRUE(connection_->IsUiScaleEnabled());
    ...
    EXPECT_CALL(delegate_, OnBoundsChanged(Eq(kDefaultBoundsChange))).Times(1);
    SendConfigureEvent(surface_id_, gfx::Size(1000, 1000), wl::ScopedWlArray({}), kConfigureSerial);
    VerifyAndClearExpectations();
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        EXPECT_CALL(*mock_surface->xdg_surface(), SetWindowGeometry(gfx::Rect(1000, 1000)));
        EXPECT_CALL(*mock_surface->xdg_surface(), AckConfigure(Eq(kConfigureSerial)));
    });
    CreateBufferAndPresentAsNewFrame(window_.get(), delegate_,
...
TEST_P(PerSurfaceScaleWaylandWindowTest, UiScale_InitScaleAndBounds)
{
    ...
    auto new_window = new_window_delegate.CreateWaylandWindow(connection_.get(), std::move(properties));
    WaylandTestBase::SyncDisplay();
    Mock::VerifyAndClearExpectations(&new_window_delegate);
    const uint32_t new_window_surface_id = new_window->root_surface()->get_surface_id();
    ...
    // received yet.
    EXPECT_EQ(1.25f, screen_->GetPreferredScaleFactorForAcceleratedWidget(new_window->GetWidget()).value_or(0.f));
    ...
    PostToServerAndWait([id = new_window_surface_id](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(id);
        ASSERT_TRUE(mock_surface->fractional_scale());
    ...
    EXPECT_EQ(2.5, screen_->GetPreferredScaleFactorForAcceleratedWidget(new_window->GetWidget()).value_or(0.f));
    ...
    SendConfigureEvent(new_window_surface_id, gfx::Size(0, 0), wl::ScopedWlArray({ XDG_TOPLEVEL_STATE_ACTIVATED }), kConfigureSerial);
    CreateBufferAndPresentAsNewFrame(new_window.get(), new_window_delegate,
...
TEST_P(PerSurfaceScaleWaylandWindowTest, UiScale_HandlePopupGeometry)
{
    ...
    ui::OwnedWindowAnchor anchor { .anchor_rect = gfx::Rect(100, 100, 20, 20),
        .anchor_position = OwnedWindowAnchorPosition::kBottomRight,
        .anchor_gravity = OwnedWindowAnchorGravity::kBottomLeft };
    EXPECT_CALL(menu_delegate, GetOwnedWindowAnchorAndRectInDIP()).WillOnce(Return(anchor));
    auto menu_window = CreateWaylandWindowWithParams(PlatformWindowType::kMenu, gfx::Rect(100, 100, 20, 80), &menu_delegate, toplevel->GetWidget());
    ASSERT_TRUE(menu_window);
    const uint32_t menu_surface_id = menu_window->root_surface()->get_surface_id();
    ...
    PostToServerAndWait([menu_surface_id](wl::TestWaylandServerThread* server) {
        wl::MockSurface* mock_surface = server->GetObject<wl::MockSurface>(menu_surface_id);
        ASSERT_TRUE(mock_surface->xdg_surface());
        EXPECT_CALL(*mock_surface, Damage(0, 0, 100, 200));
        EXPECT_CALL(*mock_surface->xdg_surface(), SetWindowGeometry(gfx::Rect(100, 200)));
        EXPECT_CALL(*mock_surface->xdg_surface(), AckConfigure(Eq(kLatestConfigureSerial)));
    });
    SendConfigureEvent(menu_surface_id, gfx::Size(100, 200), wl::ScopedWlArray({}), kLatestConfigureSerial);
...
TEST_P(PerSurfaceScaleWaylandWindowTest, UiScale_SanitizeFontScale)
{
    ...
    auto test_font_scale = [&](float requested_font_scale, float sanitized_font_scale) {
        EXPECT_CALL(delegate_, OnBoundsChanged(Eq(kDefaultBoundsChange))).Times(1);
...
#if !BUILDFLAG(IS_CHROMEOS_LACROS)
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandWindowTest, Values(wl::ServerConfig {}));
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, MultiDisplayWaylandWindowTest, Values(wl::ServerConfig {}));
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, PerSurfaceScaleWaylandWindowTest, Values(wl::ServerConfig { .supports_viewporter_surface_scaling = true }));
#else
INSTANTIATE_TEST_SUITE_P(
    XdgVersionStableTestWithAuraShell, WaylandWindowTest, Values(wl::ServerConfig { .enable_aura_shell = wl::EnableAuraShellProtocol::kEnabled }));
INSTANTIATE_TEST_SUITE_P(
    XdgVersionStableTestWithAuraShell, MultiDisplayWaylandWindowTest, Values(wl::ServerConfig { .enable_aura_shell = wl::EnableAuraShellProtocol::kEnabled }));
#endif

#if !BUILDFLAG(IS_CHROMEOS_LACROS)
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandSubsurfaceTest, Values(wl::ServerConfig {}));

#else
INSTANTIATE_TEST_SUITE_P(
    XdgVersionStableTestWithAuraShell, WaylandSubsurfaceTest, Values(wl::ServerConfig { .enable_aura_shell = wl::EnableAuraShellProtocol::kEnabled }));
#endif
