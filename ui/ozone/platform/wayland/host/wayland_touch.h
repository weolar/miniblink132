...
public:
    ...
    WaylandTouch(wl_touch* touch, WaylandConnection* connection, Delegate* delegate);
...
private:
    ...
    static void OnTouchDown(void* data, wl_touch* touch, uint32_t serial, uint32_t time, struct wl_surface* surface, int32_t id, wl_fixed_t x, wl_fixed_t y);
    ...
    static void OnTouchStylusTool(void* data, struct zcr_touch_stylus_v2* stylus, uint32_t id, uint32_t type);
    static void OnTouchStylusForce(void* data, struct zcr_touch_stylus_v2* stylus, uint32_t time, uint32_t id, wl_fixed_t force);
    static void OnTouchStylusTilt(void* data, struct zcr_touch_stylus_v2* stylus, uint32_t time, uint32_t id, wl_fixed_t tilt_x, wl_fixed_t tilt_y);
...
public:
    virtual void OnTouchPressEvent(
        ...
        = 0;
    virtual void OnTouchReleaseEvent(base::TimeTicks timestamp, PointerId id, wl::EventDispatchPolicy dispatch_policy, bool is_synthesized) = 0;
    virtual void OnTouchMotionEvent(
    ...
    virtual void OnTouchStylusToolChanged(PointerId pointer_id, EventPointerType pointer_type) = 0;
    virtual void OnTouchStylusForceChanged(PointerId pointer_id, float force) = 0;
    virtual void OnTouchStylusTiltChanged(PointerId pointer_id, const gfx::Vector2dF& tilt) = 0;
};
