...
void WaylandOutputManager::AddWaylandOutput(WaylandOutput::Id output_id, wl_output* output)
{
    ...
    auto wayland_output = std::make_unique<WaylandOutput>(output_id, output, connection_);
    ...
    if (connection_->zcr_color_manager()) {
        wayland_output->InitializeColorManagementOutput(connection_->zcr_color_manager());
    }
...
void WaylandOutputManager::InitializeAllColorManagementOutputs()
{
    ...
    for (const auto& output : output_list_)
        output.second->InitializeColorManagementOutput(connection_->zcr_color_manager());
}
...
WaylandOutput::Id WaylandOutputManager::GetOutputId(wl_output* output_resource) const
{
    auto it = base::ranges::find(output_list_, output_resource, [](const auto& pair) { return pair.second->get_output(); });
...
{
    if (wayland_screen_) {
        auto output_id = wayland_screen_->GetOutputIdForDisplayId(wayland_screen_->GetPrimaryDisplay().id());
        return GetOutput(output_id);
    }
    return nullptr;
}

const WaylandOutputManager::OutputList& WaylandOutputManager::GetAllOutputs() const
{
    return output_list_;
}
...
void WaylandOutputManager::OnOutputHandleMetrics(const WaylandOutput::Metrics& metrics)
{
    if (wayland_screen_) {
    ...
    const bool is_primary = wayland_screen_ && metrics.display_id == wayland_screen_->GetPrimaryDisplay().id();
