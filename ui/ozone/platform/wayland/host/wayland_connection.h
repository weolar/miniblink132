...
public:
    ...
    {
        return viewporter_.get();
    }
    zcr_alpha_compositing_v1* alpha_compositing() const
    {
        return alpha_compositing_.get();
    }
    xdg_wm_base* shell() const
    {
        return shell_.get();
    }
    wp_presentation* presentation() const
    {
    ...
    zwp_keyboard_shortcuts_inhibit_manager_v1* keyboard_shortcuts_inhibit_manager_v1() const
    {
        return keyboard_shortcuts_inhibit_manager_v1_.get();
    }
    zcr_stylus_v2* stylus_v2() const
    {
        return zcr_stylus_v2_.get();
    }
    ...
    zwp_linux_explicit_synchronization_v1* linux_explicit_synchronization_v1() const
    {
    ...
    {
        return !!linux_explicit_synchronization_v1() || !!linux_drm_syncobj_manager_v1();
    }
    ...
    void SetCursorBitmap(const std::vector<SkBitmap>& bitmaps, const gfx::Point& hotspot_in_dips, int buffer_scale);

    WaylandEventSource* event_source() const
    {
    ...
    WaylandClipboard* clipboard() const
    {
    ...
    WaylandZAuraShell* zaura_shell() const
    {
        return zaura_shell_.get();
    }
    ...
    WaylandBufferFactory* buffer_factory() const
    {
        return buffer_factory_.get();
    }
    ...
    GtkPrimarySelectionDeviceManager* gtk_primary_selection_device_manager() const
    {
        return gtk_primary_selection_device_manager_.get();
    }

    GtkShell1* gtk_shell1()
    {
        return gtk_shell1_.get();
    }

    OrgKdeKwinIdle* org_kde_kwin_idle()
    {
        return org_kde_kwin_idle_.get();
    }
    ...
    const XdgActivation* xdg_activation() const
    {
        return xdg_activation_.get();
    }

    XdgForeignWrapper* xdg_foreign() const
    {
        return xdg_foreign_.get();
    }
    ...
    base::TimeTicks ConvertPresentationTime(uint32_t tv_sec_hi, uint32_t tv_sec_lo, uint32_t tv_nsec);

    const std::vector<std::pair<std::string, uint32_t>>& available_globals() const
    {
    ...
    {
        return base::FeatureList::IsEnabled(features::kWaylandPerSurfaceScale) && supports_viewporter_surface_scaling();
    }

    bool IsUiScaleEnabled() const
    {
        return base::FeatureList::IsEnabled(features::kWaylandUiScale) && UsePerSurfaceScaling();
    }
    ...
    bool GetTabletMode()
    {
        return tablet_layout_state_ == display::TabletState::kInTabletMode || tablet_layout_state_ == display::TabletState::kEnteringTabletMode;
    }
    display::TabletState GetTabletState()
    {
        return tablet_layout_state_;
    }
    ...
    {
        return !SupportsExplicitSync() && WaylandBufferManagerHost::SupportsImplicitSyncInterop();
    }
...
private:
    ...
    void RegisterGlobalObjectFactory(const char* interface_name, wl::GlobalObjectFactory factory);
    ...
    static void OnClockId(void* data, wp_presentation* presentation, uint32_t clk_id);

    void HandleGlobal(wl_registry* registry, uint32_t name, const char* interface, uint32_t version);
    ...
    wl::Object<zwp_keyboard_shortcuts_inhibit_manager_v1> keyboard_shortcuts_inhibit_manager_v1_;
    ...
    wl::Object<zwp_linux_explicit_synchronization_v1> linux_explicit_synchronization_;
    ...
    std::unique_ptr<WaylandZwpRelativePointerManager> zwp_relative_pointer_manager_;
    ...
    std::unique_ptr<GtkPrimarySelectionDeviceManager> gtk_primary_selection_device_manager_;
    std::unique_ptr<ZwpPrimarySelectionDeviceManager> zwp_primary_selection_device_manager_;
    ...
    display::TabletState tablet_layout_state_ = display::TabletState::kInClamshellMode;
