...
public:
    XDGToplevelWrapperImpl(std::unique_ptr<XDGSurfaceWrapperImpl> surface, WaylandWindow* wayland_window, WaylandConnection* connection);
    ...
    void RequestWindowBounds(const gfx::Rect& bounds, int64_t display_id) override;
    ...
    void CommitSnap(WaylandWindowSnapDirection snap_direction, float snap_ratio) override;
    void ShowSnapPreview(WaylandWindowSnapDirection snap_direction, bool allow_haptic_feedback) override;
...
private:
    ...
    static void OnWmCapabilities(void* data, xdg_toplevel* toplevel, wl_array* capabilities);
    ...
    static void OnAuraToplevelConfigure(void* data, zaura_toplevel* aura_toplevel, int32_t x, int32_t y, int32_t width, int32_t height, wl_array* states);
    static void OnOriginChange(void* data, zaura_toplevel* aura_toplevel, int32_t x, int32_t y);

    static void OnConfigureRasterScale(void* data, struct zaura_toplevel* zaura_toplevel, uint32_t scale_as_uint);

    static void OnConfigureOcclusionState(void* data, struct zaura_toplevel* zaura_toplevel, uint32_t mode);
    static void OnRotateFocus(void* data, zaura_toplevel* aura_toplevel, uint32_t serial, uint32_t direction, uint32_t restart);
    static void OnOverviewChange(void* data, zaura_toplevel* aura_toplevel, uint32_t in_overview_as_uint);
