...
class Clipboard {
public:
    using ClipboardDataChangedCallback = ui::PlatformClipboard::ClipboardDataChangedCallback;
    ...
    virtual void SetClipboardDataChangedCallback(ClipboardDataChangedCallback callback) = 0;
};
...
template <typename Manager, typename DataSource = typename Manager::DataSource, typename DataDevice = typename Manager::DataDevice>
class ClipboardImpl final : public Clipboard, public DataSource::Delegate {
public:
    ClipboardImpl(Manager* manager, ui::ClipboardBuffer buffer, ui::WaylandConnection* connection)
        : manager_(manager)
        , buffer_(buffer)
        , connection_(connection)
    {
        GetDevice()->set_selection_offer_callback(base::BindRepeating(&ClipboardImpl::HandleNewSelectionOffer, weak_factory_.GetWeakPtr()));
    }
    ...
    void Write(const ui::PlatformClipboard::DataMap* data) final
    {
        if (!data || data->empty()) {
            ...
            auto serial = serial_tracker.GetSerial({ wl::SerialType::kTouchPress, wl::SerialType::kMousePress, wl::SerialType::kKeyPress });
            if (serial.has_value()) {
    ...
    bool IsSelectionOwner() const final
    {
        return !!source_;
    }

    void SetClipboardDataChangedCallback(ClipboardDataChangedCallback callback) final
    {
        clipboard_changed_callback_ = std::move(callback);
    }

private:
    DataDevice* GetDevice()
    {
        return manager_->GetDevice();
    }
    ...
    std::string GetMimeTypeForRequest(const std::string& mime_type)
    {
        ...
        for (const auto& t : GetDevice()->GetAvailableMimeTypes()) {
            if (t == ui::kMimeTypeTextUtf8 || t == ui::kMimeTypeLinuxString || t == ui::kMimeTypeLinuxUtf8String || t == ui::kMimeTypeLinuxText) {
                return t;
            }
        }
        return mime_type;
    ...
    void OnDataSourceFinish(DataSource* source, base::TimeTicks timestamp, bool completed) override
    {
        if (!completed)
            Write(nullptr);
    }

    void OnDataSourceSend(DataSource* source, const std::string& mime_type, std::string* contents) override
    {
        DCHECK(contents);
...
WaylandClipboard::WaylandClipboard(WaylandConnection* connection, WaylandDataDeviceManager* manager)
    : connection_(connection)
    , copypaste_clipboard_(std::make_unique<wl::ClipboardImpl<WaylandDataDeviceManager>>(manager, ClipboardBuffer::kCopyPaste, connection))
{
    DCHECK(manager);
    DCHECK(connection_);
...
void WaylandClipboard::OfferClipboardData(ClipboardBuffer buffer, const PlatformClipboard::DataMap& data_map, PlatformClipboard::OfferDataClosure callback)
{
...
void WaylandClipboard::RequestClipboardData(ClipboardBuffer buffer, const std::string& mime_type, PlatformClipboard::RequestDataClosure callback)
{
...
void WaylandClipboard::GetAvailableMimeTypes(ClipboardBuffer buffer, PlatformClipboard::GetMimeTypesClosure callback)
{
    std::vector<std::string> mime_types;
    if (auto* clipboard = GetClipboard(buffer))
...
void WaylandClipboard::SetClipboardDataChangedCallback(ClipboardDataChangedCallback data_changed_callback)
{
...
{
    return (connection_->zwp_primary_selection_device_manager() != nullptr) || (connection_->gtk_primary_selection_device_manager() != nullptr);
}
...
wl::Clipboard* WaylandClipboard::GetClipboard(ClipboardBuffer buffer)
{
    ...
    if (buffer == ClipboardBuffer::kSelection) {
        ...
        if (zwp_manager) {
            if (!primary_selection_clipboard_) {
                primary_selection_clipboard_
                    = std::make_unique<wl::ClipboardImpl<ZwpPrimarySelectionDeviceManager>>(zwp_manager, ClipboardBuffer::kSelection, connection_);
            }
        ...
        if (gtk_manager) {
            if (!primary_selection_clipboard_) {
                primary_selection_clipboard_
                    = std::make_unique<wl::ClipboardImpl<GtkPrimarySelectionDeviceManager>>(gtk_manager, ClipboardBuffer::kSelection, connection_);
            }
            return primary_selection_clipboard_.get();
        }
