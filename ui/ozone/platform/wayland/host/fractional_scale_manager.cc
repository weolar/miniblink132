...
// static
void FractionalScaleManager::Instantiate(WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    ...
    auto instance = wl::Bind<::wp_fractional_scale_manager_v1>(registry, name, std::min(version, kMaxVersion));
    if (!instance) {
