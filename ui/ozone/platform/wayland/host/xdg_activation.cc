...
public:
    TokenRequest(xdg_activation_v1* xdg_activation, WaylandConnection* connection, base::nix::XdgActivationTokenCallback callback);
...
private:
    ...
    static void OnTokenReceived(void* data, xdg_activation_token_v1* activation_token, const char* token);
...
// static
void XdgActivation::Instantiate(WaylandConnection* connection, wl_registry* registry, uint32_t name, const std::string& interface, uint32_t version)
{
    ...
    auto instance = wl::Bind<::xdg_activation_v1>(registry, name, std::min(version, kMaxVersion));
    if (!instance) {
        LOG(ERROR) << "Failed to bind " << kInterfaceName;
        return;
    }
    connection->xdg_activation_ = std::make_unique<XdgActivation>(std::move(instance), connection);
}

XdgActivation::XdgActivation(wl::Object<xdg_activation_v1> xdg_activation_v1, WaylandConnection* connection)
    : xdg_activation_v1_(std::move(xdg_activation_v1))
    , connection_(connection)
{
    base::nix::SetXdgActivationTokenCreator(base::BindRepeating(&XdgActivation::RequestNewToken, base::Unretained(this)));
}
...
void XdgActivation::Activate(wl_surface* surface, const std::string& token) const
{
    xdg_activation_v1_activate(xdg_activation_v1_.get(), token.c_str(), surface);
}

void XdgActivation::RequestNewToken(base::nix::XdgActivationTokenCallback callback) const
{
    if (!task_runner_->RunsTasksInCurrentSequence()) {
        ...
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&XdgActivation::RequestNewToken, weak_ptr_factory_.GetMutableWeakPtr(), std::move(callback)));
        return;
    }
    ...
    token_request_queue_.push(std::make_unique<TokenRequest>(xdg_activation_v1_.get(), connection_,
        base::BindOnce(&XdgActivation::OnTokenRequestCompleted, base::Unretained(const_cast<XdgActivation*>(this)), std::move(callback))));
...
void XdgActivation::OnTokenRequestCompleted(base::nix::XdgActivationTokenCallback callback, std::string token)
{
...
XdgActivation::TokenRequest::TokenRequest(xdg_activation_v1* xdg_activation, WaylandConnection* connection, base::nix::XdgActivationTokenCallback callback)
    : xdg_activation_(xdg_activation)
    , connection_(connection)
    , callback_(std::move(callback))
{
    constexpr auto kMaxTokenRequestDelay = base::Milliseconds(500);
    timer_.Start(FROM_HERE, kMaxTokenRequestDelay, base::BindOnce(&XdgActivation::TokenRequest::OnTimeout, base::Unretained(this)));
}
...
void XdgActivation::TokenRequest::InitiateRequest()
{
    ...
    static constexpr xdg_activation_token_v1_listener kXdgActivationListener = { .done = &OnTokenReceived };
    xdg_activation_token_v1_add_listener(token_.get(), &kXdgActivationListener, this);
    ...
    const WaylandWindow* const keyboard_focused_window = connection_->window_manager()->GetCurrentKeyboardFocusedWindow();
    wl_surface* const keyboard_focused_surface = keyboard_focused_window ? keyboard_focused_window->root_surface()->surface() : nullptr;
    ...
    if (auto serial = connection_->serial_tracker().GetSerial(
            { wl::SerialType::kTouchPress, wl::SerialType::kMousePress, wl::SerialType::kMouseEnter, wl::SerialType::kKeyPress });
        serial.has_value()) {
        xdg_activation_token_v1_set_serial(token_.get(), serial->value, connection_->seat()->wl_object());
    }
...
// static
void XdgActivation::TokenRequest::OnTokenReceived(void* data, xdg_activation_token_v1* activation_token, const char* token)
{
