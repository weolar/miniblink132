...
// static
std::pair<std::unique_ptr<WaylandSyncobjTimeline::DrmSyncobj>, wp_linux_drm_syncobj_timeline_v1*> WaylandSyncobjTimeline::CreateHelper(
    const WaylandConnection* connection)
{
    ...
    auto* timeline = wp_linux_drm_syncobj_manager_v1_import_timeline(connection->linux_drm_syncobj_manager_v1(), syncobj_fd);
    return { std::move(syncobj), timeline };
}

WaylandSyncobjTimeline::WaylandSyncobjTimeline(DrmSyncobjIoctlWrapper* drm, std::unique_ptr<DrmSyncobj> syncobj, wp_linux_drm_syncobj_timeline_v1* timeline)
    : drm_(drm)
    , syncobj_(std::move(syncobj))
    , timeline_(timeline)
{
}
...
WaylandSyncobjAcquireTimeline::WaylandSyncobjAcquireTimeline(
    DrmSyncobjIoctlWrapper* drm, std::unique_ptr<DrmSyncobj> syncobj, wp_linux_drm_syncobj_timeline_v1* timeline)
...
// static
std::unique_ptr<WaylandSyncobjAcquireTimeline> WaylandSyncobjAcquireTimeline::Create(const WaylandConnection* connection)
{
    auto pair = CreateHelper(connection);
    if (pair == std::pair<std::unique_ptr<DrmSyncobj>, wp_linux_drm_syncobj_timeline_v1*>()) {
        return nullptr;
    }

    return base::WrapUnique<WaylandSyncobjAcquireTimeline>(
        new WaylandSyncobjAcquireTimeline(connection->buffer_manager_host()->drm_syncobj_wrapper(), std::move(pair.first), pair.second));
}

bool WaylandSyncobjAcquireTimeline::ImportSyncFdAtCurrentSyncPoint(int sync_fd)
{
    ...
    if (drm_->SyncobjTransfer(syncobj_->handle, sync_point_, temp->handle, 0, 0)) {
...
// static
std::unique_ptr<WaylandSyncobjReleaseTimeline> WaylandSyncobjReleaseTimeline::Create(const WaylandConnection* connection)
{
    auto pair = CreateHelper(connection);
    if (pair == std::pair<std::unique_ptr<DrmSyncobj>, wp_linux_drm_syncobj_timeline_v1*>()) {
        return nullptr;
    ...
    return base::WrapUnique<WaylandSyncobjReleaseTimeline>(
        new WaylandSyncobjReleaseTimeline(connection->buffer_manager_host()->drm_syncobj_wrapper(), std::move(pair.first), pair.second));
}
...
base::ScopedFD WaylandSyncobjReleaseTimeline::ExportCurrentSyncPointToSyncFd()
{
    ...
    if (drm_->SyncobjTransfer(temp->handle, 0, syncobj_->handle, sync_point_, 0)) {
        DVPLOG(3) << "Unable to transfer timeline syncobj=" << syncobj_->handle << " at sync point=" << sync_point_ << " to binary syncobj=" << temp->handle;
...
{
    DVLOG(3) << "Fence available for syncobj=" << syncobj_->handle << " sync point=" << sync_point_;
...
void WaylandSyncobjReleaseTimeline::WaitForFenceAvailableAtCurrentSyncPoint(FenceAvailableCallback callback)
{
    DVLOG(3) << __func__ << " syncobj=" << syncobj_->handle << " sync point=" << sync_point_;
    // It is illegal to call this before the first sync point.
    CHECK(sync_point_ > 0);
    ...
    if (!event_fd_.is_valid()) {
        ...
        base::CurrentUIThread::Get()->WatchFileDescriptor(event_fd_.get(), true, base::MessagePumpForUI::WATCH_READ, &controller_, this);
    }
    if (drm_->SyncobjEventfd(syncobj_->handle, sync_point_, event_fd_.get(), DRM_SYNCOBJ_WAIT_FLAGS_WAIT_AVAILABLE)) {
