...
using MockTerminateGpuCallback = base::MockCallback<base::OnceCallback<void(std::string)>>;
...
public:
    MockSurfaceGpu(WaylandBufferManagerGpu* buffer_manager, gfx::AcceleratedWidget widget)
    ...
    ~MockSurfaceGpu() override
    {
        buffer_manager_->UnregisterSurface(widget_);
    }

    MOCK_METHOD3(OnSubmission, void(uint32_t buffer_id, const gfx::SwapResult& swap_result, gfx::GpuFenceHandle release_fence));
    MOCK_METHOD2(OnPresentation, void(uint32_t buffer_id, const gfx::PresentationFeedback& feedback));
...
protected:
    ...
    wl_resource* GetSurfaceAttachedBuffer(uint32_t surface_id)
    {
        ...
        PostToServerAndWait([&wl_buffer, surface_id](wl::TestWaylandServerThread* server) {
            auto* mock_surface = server->GetObject<wl::MockSurface>(surface_id);
            ASSERT_TRUE(mock_surface);
    ...
    // for convenience.
    void SetTerminateCallbackExpectationAndDestroyChannel(MockTerminateGpuCallback* callback, bool fail)
    {
        ...
        if (!fail) {
            ...
            EXPECT_CALL(*callback, Run(_)).Times(1).WillRepeatedly(::testing::Invoke([this, callback](std::string error_string) {
    ...
    void CreateDmabufBasedBufferAndSetTerminateExpectation(bool fail, uint32_t buffer_id, base::ScopedFD fd = base::ScopedFD(),
        const gfx::Size& size = kDefaultSize, const std::vector<uint32_t>& strides = { 1 }, const std::vector<uint32_t>& offsets = { 2 },
        const std::vector<uint64_t>& modifiers = { 3 }, uint32_t format = DRM_FORMAT_R8, uint32_t planes_count = 1)
    {
        if (!fd.is_valid())
            fd = MakeFD();

        SetTerminateCallbackExpectationAndDestroyChannel(&callback_, fail);
        buffer_manager_gpu_->CreateDmabufBasedBuffer(std::move(fd), kDefaultSize, strides, offsets, modifiers, format, planes_count, buffer_id);

        base::RunLoop().RunUntilIdle();
    }

    void CreateShmBasedBufferAndSetTerminateExpecation(bool fail, uint32_t buffer_id, const gfx::Size& size = kDefaultSize, size_t length = 0)
    {
        SetTerminateCallbackExpectationAndDestroyChannel(&callback_, fail);

        if (!length)
            length = size.width() * size.height() * 4;
        buffer_manager_gpu_->CreateShmBasedBuffer(MakeFD(), length, size, buffer_id);
    ...
    void ProcessCreatedBufferResourcesWithExpectation(size_t expected_size, bool fail)
    {
        PostToServerAndWait([expected_size, fail](wl::TestWaylandServerThread* server) {
            ...
            for (wl::TestZwpLinuxBufferParamsV1* mock_params : params_vector) {
                if (!fail) {
                    zwp_linux_buffer_params_v1_send_created(mock_params->resource(), mock_params->buffer_resource());
                } else {
    ...
    std::unique_ptr<WaylandWindow> CreateWindow(
        PlatformWindowType type = PlatformWindowType::kWindow, gfx::AcceleratedWidget parent_widget = gfx::kNullAcceleratedWidget)
    {
        ...
        auto new_window = WaylandWindow::Create(&delegate_, connection_.get(), std::move(properties));
    ...
    wl::WaylandOverlayConfig CreateBasicWaylandOverlayConfig(int z_order, uint32_t buffer_id, const gfx::Rect& bounds_rect)
    {
        return CreateBasicWaylandOverlayConfig(z_order, buffer_id, gfx::RectF(bounds_rect));
    }

    wl::WaylandOverlayConfig CreateBasicWaylandOverlayConfig(int z_order, uint32_t buffer_id, const gfx::RectF& bounds_rect)
    {
    ...
    void CommitBuffer(gfx::AcceleratedWidget widget, uint32_t frame_id, uint32_t buffer_id, gfx::FrameData data, const gfx::Rect& bounds_rect,
        const gfx::RoundedCornersF& corners, float surface_scale_factor, const gfx::Rect& damage_region)
    {
        buffer_manager_gpu_->CommitBuffer(widget, frame_id, buffer_id, data, bounds_rect, /*enable_blend=*/false, corners, surface_scale_factor, damage_region);
...
TEST_P(WaylandBufferManagerTest, CreateDmabufBasedBuffers)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { EXPECT_CALL(*server->zwp_linux_dmabuf_v1(), CreateParams(_, _, _)).Times(1); });

    CreateDmabufBasedBufferAndSetTerminateExpectation(false /*fail*/, kDmabufBufferId);
...
TEST_P(WaylandBufferManagerTest, VerifyModifiers)
{
    ...
    const std::vector<uint64_t> kFormatModifiers { DRM_FORMAT_MOD_INVALID, kFormatModiferLinear };
    ...
    for (const auto& modifier : kFormatModifiers) {
        PostToServerAndWait([modifier](wl::TestWaylandServerThread* server) {
            ...
            zwp_linux_dmabuf_v1_send_modifier(server->zwp_linux_dmabuf_v1()->resource(), kFourccFormatR8, modifier_hi, modifier_lo);
        });

        auto buffer_formats = connection_->buffer_factory()->GetSupportedBufferFormats();
        ASSERT_EQ(buffer_formats.size(), 1u);
        ASSERT_EQ(buffer_formats.begin()->first, GetBufferFormatFromFourCCFormat(kFourccFormatR8));
    ...
    CreateDmabufBasedBufferAndSetTerminateExpectation(
        false /*fail*/, kDmabufBufferId, base::ScopedFD(), kDefaultSize, { 1 }, { 2 }, { kFormatModiferLinear }, kFourccFormatR8, 1);
...
TEST_P(WaylandBufferManagerTest, ValidateDataFromGpu)
{
    ...
    for (const auto& bad : kBadInputs) {
        ...
        base::ScopedFD dummy;
        CreateDmabufBasedBufferAndSetTerminateExpectation(true /*fail*/, bad.buffer_id, bad.has_file ? MakeFD() : std::move(dummy), bad.size, bad.strides,
...
TEST_P(WaylandBufferManagerTest, CreateAndDestroyBuffer)
{
    ...
    {
        PostToServerAndWait([](wl::TestWaylandServerThread* server) { EXPECT_CALL(*server->zwp_linux_dmabuf_v1(), CreateParams(_, _, _)).Times(2); });
        ...
        CreateDmabufBasedBufferAndSetTerminateExpectation(true /*fail*/, kBufferId2);
    }
    ...
    // has already been attached to a surface.
    {
        ...
        CommitBuffer(widget, kBufferId1, kBufferId1, gfx::FrameData(delegate_.viz_seq()), window_->GetBoundsInPixels(), gfx::RoundedCornersF(), kDefaultScale,
            gfx::Rect(window_->applied_state().size_px));

        CreateDmabufBasedBufferAndSetTerminateExpectation(true /*fail*/, kBufferId1);
    }
    ...
    {
        PostToServerAndWait([](wl::TestWaylandServerThread* server) { EXPECT_CALL(*server->zwp_linux_dmabuf_v1(), CreateParams(_, _, _)).Times(3); });
        CreateDmabufBasedBufferAndSetTerminateExpectation(false /*fail*/, kBufferId1);
        // Attach to a surface.
        CommitBuffer(widget, kBufferId1, kBufferId1, gfx::FrameData(delegate_.viz_seq()), window_->GetBoundsInPixels(), gfx::RoundedCornersF(), kDefaultScale,
            gfx::Rect(window_->applied_state().size_px));
...
TEST_P(WaylandBufferManagerTest, CommitBufferNonExistingBufferId)
{
    ...
    ProcessCreatedBufferResourcesWithExpectation(1u /* expected size */, false /* fail */);
    ...
    CommitBuffer(window_->GetWidget(), 1u, 5u, gfx::FrameData(delegate_.viz_seq()), window_->GetBoundsInPixels(), gfx::RoundedCornersF(), kDefaultScale,
        gfx::Rect(window_->applied_state().size_px));
    ...
    PostToServerAndWait(
        [id = surface_id_](wl::TestWaylandServerThread* server) { testing::Mock::VerifyAndClearExpectations(server->GetObject<wl::MockSurface>(id)); });
}
...
TEST_P(WaylandBufferManagerTest, CommitOverlaysNonExistingBufferId)
{
    ...
    if (connection_->ShouldUseOverlayDelegation()) {
        overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(INT32_MIN, 1u, window_->GetBoundsInPixels()));
    }
    // Non-existing buffer id
    overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(0, 2u, window_->GetBoundsInPixels()));
    buffer_manager_gpu_->CommitOverlays(window_->GetWidget(), 1u, gfx::FrameData(delegate_.viz_seq()), std::move(overlay_configs));
...
TEST_P(WaylandBufferManagerTest, CommitOverlaysWithSameBufferId)
{
    ...
    const size_t expected_number_of_buffers = connection_->linux_explicit_synchronization_v1() ? 1 : 2;

    PostToServerAndWait([expected_number_of_buffers](wl::TestWaylandServerThread* server) {
        EXPECT_CALL(*server->zwp_linux_dmabuf_v1(), CreateParams(_, _, _)).Times(expected_number_of_buffers);
    });
    CreateDmabufBasedBufferAndSetTerminateExpectation(false /*fail*/, 1u);
    ...
    std::vector<wl::WaylandOverlayConfig> overlay_configs;
    overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(0, 1u, window_->GetBoundsInPixels()));
    overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(1, 1u, window_->GetBoundsInPixels()));
    ...
    ProcessCreatedBufferResourcesWithExpectation(expected_number_of_buffers /* expected size */, false /* fail */);
}
...
TEST_P(WaylandBufferManagerTest, CommitBufferNullWidget)
{
    ...
    CommitBuffer(gfx::kNullAcceleratedWidget, 1u, kBufferId, gfx::FrameData(delegate_.viz_seq()), window_->GetBoundsInPixels(), gfx::RoundedCornersF(),
...
TEST_P(WaylandBufferManagerTest, CommitOverlaysNonsensicalBoundsRect)
{
    ...
    const std::vector<gfx::RectF> bounds_rect_test_data
        = { gfx::RectF(std::nanf(""), window_->GetBoundsInPixels().y(), std::nanf(""), window_->GetBoundsInPixels().height()),
              gfx::RectF(window_->GetBoundsInPixels().x(), std::numeric_limits<float>::infinity(), window_->GetBoundsInPixels().width(),
    ...
    for (bool should_root_have_nan_bounds : config) {
        for (const auto& faulty_bounds_rect : bounds_rect_test_data) {
            ...
            ProcessCreatedBufferResourcesWithExpectation(3u /* expected size */, false /* fail */);
            ...
            if (should_root_have_nan_bounds) {
                ...
                overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(INT32_MIN, kBufferId1, faulty_bounds_rect));
                overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(z_order++, kBufferId2, window_->GetBoundsInPixels()));
                overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(z_order++, kBufferId3, window_->GetBoundsInPixels()));
            } else {
                ...
                // doesn't crash.
                overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(INT32_MIN, kBufferId1, window_->GetBoundsInPixels()));
                overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(z_order++, kBufferId2, window_->GetBoundsInPixels()));
                overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(z_order++, kBufferId3, faulty_bounds_rect));
                overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(z_order++, kBufferId2, window_->GetBoundsInPixels()));
            }
            buffer_manager_gpu_->CommitOverlays(window_->GetWidget(), 1u, gfx::FrameData(delegate_.viz_seq()), std::move(overlay_configs));
            ...
            if (!should_root_have_nan_bounds && !connection_->linux_explicit_synchronization_v1()) {
            ...
            EXPECT_EQ("Overlay bounds_rect is invalid (NaN or infinity).", channel_destroyed_error_message_);
            ...
            for (auto& subsurface : window_->wayland_subsurfaces_) {
                ids_of_subsurfaces.emplace_back(subsurface->wayland_surface()->get_surface_id());
            }

            PostToServerAndWait([id = surface_id_, ids_of_subsurfaces](wl::TestWaylandServerThread* server) {
                // Clear all the possible frame and release callbacks.
                auto* mock_surface = server->GetObject<wl::MockSurface>(id);
                for (auto subsurface_id : ids_of_subsurfaces) {
                    auto* mock_surface_of_subsurface = server->GetObject<wl::MockSurface>(subsurface_id);
...
TEST_P(WaylandBufferManagerTest, CommitOverlaysNonsensicalBoundsRectSingleOverlay)
{
    if (connection_->ShouldUseOverlayDelegation()) {
        GTEST_SKIP();
    }
    ...
    for (const auto& faulty_bounds_rect : bounds_rect_test_data) {
        ...
        overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(z_order, kBufferId1, faulty_bounds_rect));
...
TEST_P(WaylandBufferManagerTest, EnsureCorrectOrderOfCallbacks)
{
    ...
    ProcessCreatedBufferResourcesWithExpectation(2u /* expected size */, false /* fail */);
    ...
    EXPECT_CALL(mock_surface_gpu, OnSubmission(kBufferId1, gfx::SwapResult::SWAP_ACK, _)).Times(1);
    ...
    CommitBuffer(widget, kBufferId1, kBufferId1, gfx::FrameData(delegate_.viz_seq()), bounds, gfx::RoundedCornersF(), kDefaultScale, bounds);
    ...
    CommitBuffer(widget, kBufferId2, kBufferId2, gfx::FrameData(delegate_.viz_seq()), bounds, gfx::RoundedCornersF(), kDefaultScale, bounds);

    base::RunLoop().RunUntilIdle();

    EXPECT_CALL(mock_surface_gpu, OnSubmission(kBufferId2, gfx::SwapResult::SWAP_ACK, _)).Times(1);
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        EXPECT_CALL(*mock_wp_presentation, Feedback(_, _, mock_surface->resource(), _)).Times(1);
    });
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { server->EnsureAndGetWpPresentation()->SendPresentationCallback(); });
...
TEST_P(WaylandBufferManagerTest, DestroyedBuffersGeneratePresentationFeedbackFailure)
{
    constexpr uint32_t kBufferId1 = 1;
    constexpr uint32_t kBufferId2 = 2;
    constexpr uint32_t kBufferId3 = 3;
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        EXPECT_CALL(*mock_surface, Frame(_)).Times(kNumberOfCommits);
        EXPECT_CALL(*mock_surface, Commit()).Times(kNumberOfCommits);
        EXPECT_CALL(*mock_wp_presentation, Feedback(_, _, mock_surface->resource(), _)).Times(3);
    });
    ...
    EXPECT_CALL(mock_surface_gpu,
        OnPresentation(kBufferId1, ::testing::Field(&gfx::PresentationFeedback::flags, ::testing::Eq(gfx::PresentationFeedback::Flags::kFailure))))
        .Times(1);
    EXPECT_CALL(mock_surface_gpu,
        OnPresentation(kBufferId2, ::testing::Field(&gfx::PresentationFeedback::flags, ::testing::Eq(gfx::PresentationFeedback::Flags::kFailure))))
        .Times(1);
    EXPECT_CALL(mock_surface_gpu, OnPresentation(_, _)).Times(0);
    CommitBuffer(widget, kBufferId3, kBufferId3, gfx::FrameData(delegate_.viz_seq()), bounds, gfx::RoundedCornersF(), kDefaultScale, bounds);
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
    ...
    task_environment_.FastForwardBy(WaylandFrameManager::GetPresentationFlushTimerDurationForTesting());
    ...
    EXPECT_CALL(mock_surface_gpu, OnSubmission(kBufferId3, gfx::SwapResult::SWAP_ACK, _)).Times(1);
...
TEST_P(WaylandBufferManagerTest, EnsureDiscardedPresentationDoesNotMakePreviousFeedbacksFailed)
{
    constexpr uint32_t kBufferId1 = 1;
    constexpr uint32_t kBufferId2 = 2;
    constexpr uint32_t kBufferId3 = 3;
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        EXPECT_EQ(1u, server->EnsureAndGetWpPresentation()->num_of_presentation_callbacks());
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        // Verify we have a presentation callback now. This will be sent later.
        EXPECT_EQ(2u, server->EnsureAndGetWpPresentation()->num_of_presentation_callbacks());
    ...
    EXPECT_CALL(mock_surface_gpu,
        OnPresentation(kBufferId1, ::testing::Field(&gfx::PresentationFeedback::flags, ::testing::Eq(gfx::PresentationFeedback::Flags::kVSync))))
        .Times(1);
    ...
    EXPECT_CALL(mock_surface_gpu,
        OnPresentation(kBufferId2, ::testing::Field(&gfx::PresentationFeedback::flags, ::testing::Eq(gfx::PresentationFeedback::Flags::kVSync))))
        .Times(1);
    EXPECT_CALL(mock_surface_gpu,
        OnPresentation(kBufferId3, ::testing::Field(&gfx::PresentationFeedback::flags, ::testing::Eq(gfx::PresentationFeedback::Flags::kFailure))))
        .Times(1);
...
TEST_P(WaylandBufferManagerTest, EnsureFailedPresentationIsSent)
{
    ...
    wl::MockWpPresentation::PresentationFeedbackParams params {
        .tv_sec_hi = 1, .tv_sec_lo = 1, .tv_nsec = 1, .refresh = 1, .seq_hi = 1, .seq_lo = 1, .flags = gfx::PresentationFeedback::kFailure
    };

    PostToServerAndWait([params](wl::TestWaylandServerThread* server) {
        auto* mock_wp_presentation = server->EnsureAndGetWpPresentation();
        EXPECT_EQ(1u, mock_wp_presentation->num_of_presentation_callbacks());
        mock_wp_presentation->SendPresentationFeedbackToClient(/*last=*/false, params);
    });
...
TEST_P(WaylandBufferManagerTest, TestCommitBufferConditions)
{
    ...
    CommitBuffer(widget, kDmabufBufferId, kDmabufBufferId, gfx::FrameData(delegate_.viz_seq()), window_->GetBoundsInPixels(), gfx::RoundedCornersF(),
        kDefaultScale, gfx::Rect(window_->applied_state().size_px));
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        testing::Mock::VerifyAndClearExpectations(server->GetObject<wl::MockSurface>(id));
    });
    ...
    CommitBuffer(widget, kDmabufBufferId2, kDmabufBufferId2, gfx::FrameData(delegate_.viz_seq()), window_->GetBoundsInPixels(), gfx::RoundedCornersF(),
        kDefaultScale, gfx::Rect(window_->applied_state().size_px));
...
TEST_P(WaylandBufferManagerTest, TestCommitBufferConditionsAckConfigured)
{
    ...
    std::vector<PlatformWindowType> window_types { PlatformWindowType::kWindow, PlatformWindowType::kTooltip };

    for (const auto& type : window_types) {
        // If the type is not kWindow, provide default created window as parent of
        // the newly created window.
        auto temp_window = CreateWindow(type, type != PlatformWindowType::kWindow ? widget_ : gfx::kNullAcceleratedWidget);
        auto widget = temp_window->GetWidget();

        temp_window->Show(false);

        const uint32_t temp_window_surface_id = temp_window->root_surface()->get_surface_id();
        ...
        PostToServerAndWait([temp_window_surface_id](wl::TestWaylandServerThread* server) {
            auto* mock_surface = server->GetObject<wl::MockSurface>(temp_window_surface_id);
        ...
        PostToServerAndWait([temp_window_surface_id](wl::TestWaylandServerThread* server) {
            testing::Mock::VerifyAndClearExpectations(server->GetObject<wl::MockSurface>(temp_window_surface_id));
        });
...
TEST_P(WaylandBufferManagerTest, CommitBufferConditionsWithDeferredAckConfigure)
{
    ...
    gfx::Insets insets;
    EXPECT_CALL(delegate_, CalculateInsetsInDIP(PlatformWindowState::kNormal)).WillRepeatedly(testing::Return(insets));
    auto window = WaylandWindow::Create(&delegate_, connection_.get(), std::move(properties));
    ASSERT_TRUE(window);
    ...
    PostToServerAndWait([surface_id, bounds = kRestoredBounds](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(surface_id);
        auto* xdg_surface = mock_surface->xdg_surface();
    ...
    CommitBuffer(widget, kDmabufBufferId, kDmabufBufferId, gfx::FrameData(delegate_.viz_seq() - 1), gfx::Rect { 55, 55 }, gfx::RoundedCornersF(), kDefaultScale,
        gfx::Rect { 55, 55 });
    ActivateSurface(surface_id, kActivateSerial);

    CommitBuffer(
        widget, kDmabufBufferId, kDmabufBufferId, gfx::FrameData(delegate_.viz_seq()), kRestoredBounds, gfx::RoundedCornersF(), kDefaultScale, kRestoredBounds);
    SetPointerFocusedWindow(nullptr);
    window.reset();
    DestroyBufferAndSetTerminateExpectation(kDmabufBufferId, false /*fail*/);
}
...
TEST_P(WaylandBufferManagerTest, AnonymousBufferAttachedAndReleased)
{
    ...
    EXPECT_CALL(mock_surface_gpu, OnSubmission(kBufferId3, gfx::SwapResult::SWAP_ACK, _)).Times(0);
...
TEST_P(WaylandBufferManagerTest, DestroyBufferForDestroyedWindow)
{
    ...
    CommitBuffer(widget, kBufferId, kBufferId, gfx::FrameData(delegate_.viz_seq()), temp_window->GetBoundsInPixels(), gfx::RoundedCornersF(), kDefaultScale,
...
TEST_P(WaylandBufferManagerTest, DestroyedWindowNoSubmissionSingleBuffer)
{
    ...
    CommitBuffer(widget, kBufferId, kBufferId, gfx::FrameData(delegate_.viz_seq()), bounds, gfx::RoundedCornersF(), kDefaultScale, bounds);
...
TEST_P(WaylandBufferManagerTest, DestroyedWindowNoSubmissionMultipleBuffers)
{
    ...
    const uint32_t temp_window_surface_id = temp_window->root_surface()->get_surface_id();
    PostToServerAndWait(
        [temp_window_surface_id](wl::TestWaylandServerThread* server) { ASSERT_TRUE(server->GetObject<wl::MockSurface>(temp_window_surface_id)); });
    ...
    PostToServerAndWait([temp_window_surface_id](wl::TestWaylandServerThread* server) {
        server->GetObject<wl::MockSurface>(temp_window_surface_id)->SendFrameCallback();
        EXPECT_CALL(*server->zwp_linux_dmabuf_v1(), CreateParams(_, _, _)).Times(1);
    });
...
TEST_P(WaylandBufferManagerTest, DestroyBufferCommittedTwiceInARow)
{
    ...
    EXPECT_CALL(mock_surface_gpu, OnSubmission(kBufferId2, gfx::SwapResult::SWAP_ACK, _)).Times(2);
...
TEST_P(WaylandBufferManagerTest, ReleaseBufferCommittedTwiceInARow)
{
    ...
    EXPECT_CALL(mock_surface_gpu, OnPresentation(kBufferId2, _)).Times(2);
    PostToServerAndWait(
        [wl_buffer1, id = surface_id_](wl::TestWaylandServerThread* server) { server->GetObject<wl::MockSurface>(id)->ReleaseBuffer(wl_buffer1); });
...
TEST_P(WaylandBufferManagerTest, ReleaseOrderDifferentToCommitOrder)
{
    ...
    PostToServerAndWait(
        [id = surface_id_](wl::TestWaylandServerThread* server) { EXPECT_CALL(*server->GetObject<wl::MockSurface>(id), Attach(_, _, _)).Times(1); });
    ...
    EXPECT_CALL(mock_surface_gpu, OnPresentation(_, _)).Times(0);
    PostToServerAndWait(
        [id = surface_id_](wl::TestWaylandServerThread* server) { EXPECT_CALL(*server->GetObject<wl::MockSurface>(id), Attach(_, _, _)).Times(2); });
    ...
    EXPECT_CALL(mock_surface_gpu, OnPresentation(_, _)).Times(0);
    PostToServerAndWait(
        [wl_buffer2, id = surface_id_](wl::TestWaylandServerThread* server) { server->GetObject<wl::MockSurface>(id)->ReleaseBuffer(wl_buffer2); });
...
TEST_P(WaylandBufferManagerTest, OnSubmissionCalledForBufferCommitedTwiceInARow)
{
    constexpr uint32_t kBufferId1 = 1;
    constexpr uint32_t kBufferId2 = 2;
    ...
    PostToServerAndWait([id = surface_id_, bounds](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(id);
        EXPECT_CALL(*mock_surface, Attach(_, _, _)).Times(1);
        EXPECT_CALL(*mock_surface, Frame(_)).Times(1);
        EXPECT_CALL(*mock_surface, Damage(0, 0, bounds.width(), bounds.height())).Times(1);
        EXPECT_CALL(*mock_surface, Commit()).Times(1);
    });
...
TEST_P(WaylandBufferManagerTest, RootSurfaceIsCommittedLast)
{
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        zwp_linux_buffer_params_v1_send_created(linux_dmabuf->buffer_params()[0]->resource(), linux_dmabuf->buffer_params()[0]->buffer_resource());
        zwp_linux_buffer_params_v1_send_created(linux_dmabuf->buffer_params()[1]->resource(), linux_dmabuf->buffer_params()[1]->buffer_resource());
    ...
    std::vector<wl::WaylandOverlayConfig> overlay_configs;
    overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(INT32_MIN, kBufferId1, bounds));
    overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(0, kBufferId2, bounds));
    overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(1, kBufferId3, bounds));
    buffer_manager_gpu_->CommitOverlays(window_->GetWidget(), 1u, gfx::FrameData(delegate_.viz_seq()), std::move(overlay_configs));
...
TEST_P(WaylandBufferManagerTest, FencedRelease)
{
    ...
    EXPECT_CALL(mock_surface_gpu, OnSubmission(kBufferId1, gfx::SwapResult::SWAP_ACK, Truly([](const auto& fence) { return fence.is_null(); }))).Times(1);
    CommitBuffer(widget, kBufferId1, kBufferId1, gfx::FrameData(delegate_.viz_seq()), bounds, gfx::RoundedCornersF(), kDefaultScale, bounds);
    ...
    EXPECT_CALL(mock_surface_gpu, OnSubmission(kBufferId2, gfx::SwapResult::SWAP_ACK, Truly([](const auto& fence) { return !fence.is_null(); }))).Times(1);
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        auto* mock_surface = server->GetObject<wl::MockSurface>(id);
        mock_surface->ReleaseBufferFenced(mock_surface->prev_attached_buffer(), std::move(handle));
        mock_surface->SendFrameCallback();
    });
    ...
    EXPECT_CALL(mock_surface_gpu, OnSubmission(kBufferId3, gfx::SwapResult::SWAP_ACK, Truly([](const auto& fence) { return fence.is_null(); }))).Times(1);

    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        auto* mock_surface = server->GetObject<wl::MockSurface>(id);
        mock_surface->ReleaseBufferFenced(mock_surface->prev_attached_buffer(), gfx::GpuFenceHandle());
        mock_surface->SendFrameCallback();
    });
...
TEST_P(WaylandBufferManagerTest, CanSubmitBufferAfterChannelDestroyedAndInitialized)
{
    constexpr uint32_t kBufferId1 = 1;
    ...
    auto mock_surface_gpu = std::make_unique<MockSurfaceGpu>(buffer_manager_gpu_.get(), widget);

    CreateDmabufBasedBufferAndSetTerminateExpectation(false /*fail*/, kBufferId1);

    ProcessCreatedBufferResourcesWithExpectation(1u /* expected size */, false /* fail */);

    EXPECT_CALL(*mock_surface_gpu.get(), OnSubmission(kBufferId1, gfx::SwapResult::SWAP_ACK, _)).Times(1);
    ...
    // that memory.
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) { server->GetObject<wl::MockSurface>(id)->ClearBufferReleases(); });
...
TEST_P(WaylandBufferManagerTest, HidesSubsurfacesOnChannelDestroyed)
{
    ...
    PostToServerAndWait(
        [id = surface_id_, primary_subsurface_id = window_->primary_subsurface()->wayland_surface()->get_surface_id(),
            overlay_surface_id = window_->wayland_subsurfaces().begin()->get()->wayland_surface()->get_surface_id()](wl::TestWaylandServerThread* server) {
            EXPECT_TRUE(server->GetObject<wl::MockSurface>(id)->attached_buffer());
            EXPECT_TRUE(server->GetObject<wl::MockSurface>(primary_subsurface_id)->attached_buffer());
            EXPECT_TRUE(server->GetObject<wl::MockSurface>(overlay_surface_id)->attached_buffer());
        });
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) { EXPECT_TRUE(server->GetObject<wl::MockSurface>(id)->attached_buffer()); });
    ...
    overlay_configs2.push_back(CreateBasicWaylandOverlayConfig(INT32_MIN, kBufferId1, bounds));
    buffer_manager_gpu_->CommitOverlays(window_->GetWidget(), 2u, gfx::FrameData(delegate_.viz_seq()), std::move(overlay_configs2));
...
TEST_P(WaylandBufferManagerTest, DoesNotAttachAndCommitOnHideIfNoBuffersAttached)
{
...
TEST_P(WaylandBufferManagerTest, CanSubmitOverlayPriority)
{
    ...
    MockSurfaceGpu mock_surface_gpu(buffer_manager_gpu_.get(), window_->GetWidget());
    ...
    PostToServerAndWait([size = kBufferIds.size()](wl::TestWaylandServerThread* server) {
        auto* linux_dmabuf = server->zwp_linux_dmabuf_v1();
        for (size_t i = 0; i < size; i++) {
            zwp_linux_buffer_params_v1_send_created(linux_dmabuf->buffer_params()[i]->resource(), linux_dmabuf->buffer_params()[i]->buffer_resource());
        }
    });

    std::vector<std::pair<gfx::OverlayPriorityHint, uint32_t>> priorities
        = { { gfx::OverlayPriorityHint::kNone, OVERLAY_PRIORITIZED_SURFACE_OVERLAY_PRIORITY_NONE },
              { gfx::OverlayPriorityHint::kRegular, OVERLAY_PRIORITIZED_SURFACE_OVERLAY_PRIORITY_REGULAR },
              { gfx::OverlayPriorityHint::kVideo, OVERLAY_PRIORITIZED_SURFACE_OVERLAY_PRIORITY_REGULAR },
    ...
    for (const auto& priority : priorities) {
        std::vector<wl::WaylandOverlayConfig> overlay_configs;
        for (auto id : kBufferIds) {
            overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(id == 1 ? INT32_MIN : id, id, window_->GetBoundsInPixels()));
            overlay_configs.back().priority_hint = priority.first;
        }

        buffer_manager_gpu_->CommitOverlays(window_->GetWidget(), ++frame_id, gfx::FrameData(delegate_.viz_seq()), std::move(overlay_configs));

        base::RunLoop().RunUntilIdle();

        for (auto& subsurface : window_->wayland_subsurfaces_) {
            PostToServerAndWait([subsurface_id = subsurface->wayland_surface()->get_surface_id(), &priority](wl::TestWaylandServerThread* server) {
                auto* mock_surface_of_subsurface = server->GetObject<wl::MockSurface>(subsurface_id);
                EXPECT_TRUE(mock_surface_of_subsurface);
...
TEST_P(WaylandBufferManagerTest, CanSetRoundedCorners)
{
    ...
    std::vector<gfx::RRectF> rounded_corners_vec = {
        { { 10, 10, 200, 200 }, { 1, 1, 1, 1 } },
        { { 10, 10, 200, 200 }, { 0, 1, 0, 1 } },
        { { 10, 10, 200, 200 }, { 1, 0, 1, 0 } },
        { { 10, 10, 200, 200 }, { 5, 10, 0, 1 } },
        { { 10, 10, 200, 200 }, { 0, 2, 20, 3 } },
        { { 10, 10, 200, 200 }, { 2, 3, 4, 5 } },
    ...
    for (auto scale_factor : scale_factors) {
        ...
        for (const auto& rounded_corners : rounded_corners_vec) {
            ...
            buffer_manager_gpu_->CommitOverlays(window_->GetWidget(), ++frame_id, gfx::FrameData(), std::move(overlay_configs));
            ...
            for (auto& subsurface : window_->wayland_subsurfaces_) {
                ...
                PostToServerAndWait(
                    [subsurface_id = subsurface->wayland_surface()->get_surface_id(), &rounded_clip_bounds_dip](wl::TestWaylandServerThread* server) {
                        auto* mock_surface_of_subsurface = server->GetObject<wl::MockSurface>(subsurface_id);
                        EXPECT_TRUE(mock_surface_of_subsurface);

                        EXPECT_EQ(mock_surface_of_subsurface->augmented_surface()->rounded_clip_bounds(), rounded_clip_bounds_dip);
                        mock_surface_of_subsurface->SendFrameCallback();
                    });
            }
...
{
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { ASSERT_TRUE(server->EnsureAndGetWpPresentation()); });
    ...
    PostToServerAndWait([id = surface_id_](wl::TestWaylandServerThread* server) {
        ...
        EXPECT_CALL(*mock_surface, Attach(_, _, _)).Times(kNumberOfCommits);
        EXPECT_CALL(*mock_surface, Frame(_)).Times(kNumberOfCommits);
        EXPECT_CALL(*server->EnsureAndGetWpPresentation(), Feedback(_, _, _, _)).Times(kNumberOfCommits);
        EXPECT_CALL(*mock_surface, Commit()).Times(kNumberOfCommits);
    });
    ...
    for (auto commit_seq = 1u; commit_seq <= kNumberOfCommits; commit_seq++) {
        ...
        EXPECT_CALL(mock_surface_gpu, OnSubmission(next_buffer_id_commit, gfx::SwapResult::SWAP_ACK, _)).Times(1);

        if (commit_seq % expect_presentation_failure_on_commit_seq == 0) {
            EXPECT_CALL(mock_surface_gpu, OnPresentation(_, gfx::PresentationFeedback::Failure())).Times(kExpectedFailedFeedbacks);
        ...
        CommitBuffer(
            widget, next_buffer_id_commit, next_buffer_id_commit, gfx::FrameData(delegate_.viz_seq()), bounds, gfx::RoundedCornersF(), kDefaultScale, bounds);
...
TEST_P(WaylandBufferManagerTest, ExecutesTasksAfterInitialization)
{
    ...
    constexpr uint32_t kDmabufBufferId = 1;
    CreateDmabufBasedBufferAndSetTerminateExpectation(false /*fail*/, kDmabufBufferId);
    CommitBuffer(window_->GetWidget(), kDmabufBufferId, kDmabufBufferId, gfx::FrameData(delegate_.viz_seq()), window_->GetBoundsInPixels(),
...
TEST_P(WaylandBufferManagerTest, DoesNotRequestReleaseForSolidColorBuffers)
{
    ...
    PostToServerAndWait([](wl::TestWaylandServerThread* server) { server->EnsureSurfaceAugmenter(); });

    MockSurfaceGpu mock_surface_gpu(buffer_manager_gpu_.get(), widget_);

    const auto solid_color_buffer_id = buffer_manager_gpu_->AllocateBufferID();
    buffer_manager_gpu_->CreateSolidColorBuffer(SkColor4f::FromColor(SK_ColorBLUE), gfx::Size(1, 1), solid_color_buffer_id);
    ...
    overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(INT32_MIN, solid_color_buffer_id, bounds));
    buffer_manager_gpu_->CommitOverlays(widget_, 1u, gfx::FrameData(delegate_.viz_seq()), std::move(overlay_configs));

    base::RunLoop().RunUntilIdle();

    PostToServerAndWait(
        [id = surface_id_](wl::TestWaylandServerThread* server) { EXPECT_FALSE(server->GetObject<wl::MockSurface>(id)->has_linux_buffer_release()); });
}
...
protected:
    void ViewportDestinationTestHelper(const gfx::RectF& bounds_rect, const gfx::RectF& expected_bounds_rect)
    {
        auto temp_window = CreateWindow();
        temp_window->Show(false);
        const uint32_t surface_id = temp_window->root_surface()->get_surface_id();

        PostToServerAndWait([surface_id](wl::TestWaylandServerThread* server) { ASSERT_TRUE(server->GetObject<wl::MockSurface>(surface_id)); });
        ...
        MockSurfaceGpu mock_surface_gpu(buffer_manager_gpu_.get(), temp_window->GetWidget());
        ...
        auto bounds = temp_window->GetBoundsInPixels();
        overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(INT32_MIN, kBufferId1, bounds));
        overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(0, kBufferId1, bounds));
        overlay_configs.emplace_back(CreateBasicWaylandOverlayConfig(1, kBufferId1, bounds_rect));
        buffer_manager_gpu_->CommitOverlays(temp_window->GetWidget(), 1u, gfx::FrameData(delegate_.viz_seq()), std::move(overlay_configs));

        base::RunLoop().RunUntilIdle();

        PostToServerAndWait([](wl::TestWaylandServerThread* server) {
            // Creates a handle for a subsurface.
            const std::vector<raw_ptr<wl::TestZwpLinuxBufferParamsV1, VectorExperimental>>& params_vector = server->zwp_linux_dmabuf_v1()->buffer_params();
            for (wl::TestZwpLinuxBufferParamsV1* mock_params : params_vector) {
                zwp_linux_buffer_params_v1_send_created(mock_params->resource(), mock_params->buffer_resource());
            }
        });
        ...
        WaylandSubsurface* subsurface = temp_window->wayland_subsurfaces_.begin()->get();
        ASSERT_TRUE(subsurface);
        const uint32_t subsurface_id = subsurface->wayland_surface()->get_surface_id();
        PostToServerAndWait([subsurface_id](wl::TestWaylandServerThread* server) { ASSERT_TRUE(server->GetObject<wl::MockSurface>(subsurface_id)); });
        ...
        gfx::SizeF expected_size(wl_fixed_to_double(wl_fixed_from_double(expected_bounds_rect.size().width())),
            wl_fixed_to_double(wl_fixed_from_double(expected_bounds_rect.size().height())));

        PostToServerAndWait([surface_id, subsurface_id, &expected_size](wl::TestWaylandServerThread* server) {
            auto* mock_surface_of_subsurface = server->GetObject<wl::MockSurface>(subsurface_id);
...
TEST_P(WaylandBufferManagerViewportTest, ViewportDestinationNonInteger)
{
    ...
    constexpr std::array<std::array<gfx::RectF, 2>, 2> test_data = { { { gfx::RectF({ 21, 18 }, { 7, 11 }), gfx::RectF({ 21, 18 }, { 7, 11 }) },
        { gfx::RectF({ 7, 8 }, { 43, 63 }), gfx::RectF({ 7, 8 }, { 43, 63 }) } } };

    for (const auto& data : test_data) {
        ViewportDestinationTestHelper(data[0] /* display_rect */, data[1] /* expected_rect */);
...
TEST_P(WaylandBufferManagerViewportTest, ViewportDestinationInteger)
{
    ...
    constexpr std::array<std::array<gfx::RectF, 2>, 2> test_data = { { { gfx::RectF({ 21, 18 }, { 7.423, 11.854 }), gfx::RectF({ 21, 18 }, { 7, 12 }) },
        { gfx::RectF({ 7, 8 }, { 43.562, 63.76 }), gfx::RectF({ 7, 8 }, { 43.562, 63.76 }) } } };
...
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandBufferManagerTest, Values(wl::ServerConfig {}));

INSTANTIATE_TEST_SUITE_P(XdgVersionStableTest, WaylandBufferManagerViewportTest, Values(wl::ServerConfig {}));

INSTANTIATE_TEST_SUITE_P(XdgVersionStableTestWithoutExplicitSync, WaylandBufferManagerTest,
    Values(wl::ServerConfig { .use_explicit_synchronization = wl::ShouldUseExplicitSynchronizationProtocol::kNone }));
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTestWithExplicitSync, WaylandBufferManagerTest,
    Values(wl::ServerConfig { .use_explicit_synchronization = wl::ShouldUseExplicitSynchronizationProtocol::kUse }));

INSTANTIATE_TEST_SUITE_P(XdgVersionStableTestWithViewporterSurfaceScalingDisabled, WaylandBufferManagerTest,
    Values(wl::ServerConfig { .supports_viewporter_surface_scaling = false }));
INSTANTIATE_TEST_SUITE_P(XdgVersionStableTestWithViewporterSurfaceScalingEnabled, WaylandBufferManagerTest,
