...
public:
    ...
    MockPlatformWindowDelegate& operator=(const MockPlatformWindowDelegate&) = delete;
    ...
    MOCK_METHOD2(OnWindowStateChanged, void(ui::PlatformWindowState old_state, ui::PlatformWindowState new_state));
    MOCK_METHOD0(OnLostCapture, void());
    MOCK_METHOD1(OnAcceleratedWidgetAvailable, void(gfx::AcceleratedWidget widget));
...
struct Environment {
    ...
    {
        DCHECK(host);
        host->SetTerminateGpuCallback(base::BindOnce(&Environment::OnTerminateCallbackFired, base::Unretained(this)));
    }

    void OnTerminateCallbackFired(std::string message)
    {
        terminated = true;
    }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    std::vector<uint32_t> known_fourccs { DRM_FORMAT_R8, DRM_FORMAT_GR88, DRM_FORMAT_ABGR8888, DRM_FORMAT_XBGR8888, DRM_FORMAT_ARGB8888, DRM_FORMAT_XRGB8888,
        DRM_FORMAT_XRGB2101010, DRM_FORMAT_XBGR2101010, DRM_FORMAT_RGB565, DRM_FORMAT_NV12, DRM_FORMAT_YVU420 };

    wl::TestWaylandServerThread server;
    CHECK(server.Start());

    std::unique_ptr<ui::WaylandConnection> connection = std::make_unique<ui::WaylandConnection>();
    ...
    EXPECT_CALL(delegate, OnAcceleratedWidgetAvailable(_)).WillOnce(testing::SaveArg<0>(&widget));
    ui::PlatformWindowInitProperties properties;
    properties.bounds = gfx::Rect(0, 0, 800, 600);
    properties.type = ui::PlatformWindowType::kWindow;
    std::unique_ptr<ui::WaylandWindow> window = ui::WaylandWindow::Create(&delegate, connection.get(), std::move(properties));
    ...
    base::ScopedFD fd = base::CreateAndOpenFdForTemporaryFileInDir(temp_dir, &temp_path);
    EXPECT_TRUE(fd.is_valid());

    // 10K screens are reality these days.
    const gfx::Size buffer_size(data_provider.ConsumeIntegralInRange(1U, 20000U), data_provider.ConsumeIntegralInRange(1U, 20000U));
    ...
    const uint32_t kFormat = known_fourccs[data_provider.ConsumeIntegralInRange(0UL, static_cast<unsigned long>(known_fourccs.size() - 1))];
    ...
    for (uint32_t i = 0; i < kPlaneCount; ++i) {
        ...
        modifiers[i] = data_provider.ConsumeIntegralInRange(uint64_t(0), UINT64_MAX);
    ...
    manager_host->CreateDmabufBasedBuffer(mojo::PlatformHandle(std::move(fd)), buffer_size, strides, offsets, modifiers, kFormat, kPlaneCount, kBufferId);

    // Wait until the buffers are created.
    test_api.SyncDisplay();

    if (!env.terminated) {
        server.RunAndWait(base::BindLambdaForTesting([](wl::TestWaylandServerThread* server) {
