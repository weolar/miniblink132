...
GLSurfaceEglReadbackX11::GLSurfaceEglReadbackX11(gl::GLDisplayEGL* display, gfx::AcceleratedWidget window)
...
bool GLSurfaceEglReadbackX11::Initialize(gl::GLSurfaceFormat format)
{
    ...
    if (auto attributes = connection_->GetWindowAttributes({ window_ }).Sync()) {
        ...
        DLOG(ERROR) << "Failed to get attributes for window " << static_cast<uint32_t>(window_);
        Destroy();
        return false;
    }
...
{
    SkImageInfo image_info = SkImageInfo::Make(GetSize().width(), GetSize().height(), kBGRA_8888_SkColorType, kPremul_SkAlphaType);
    ...
    DrawPixmap(connection_, visual_, window_, window_graphics_context_, pixmap, 0, 0, 0, 0, size.width(), size.height());
