...
struct MockDisplayObserver : public display::DisplayObserver {
    ...
    MOCK_METHOD1(OnDisplaysRemoved, void(const display::Displays& removed_displays));
    MOCK_METHOD2(OnDisplayMetricsChanged, void(const display::Display& display, uint32_t changed_metrics));
};
...
public:
    ...
    void SetUp() override
    {
        ...
        primary_display_ = std::make_unique<display::Display>(NextDisplayId(), kPrimaryDisplayBounds);
...
protected:
    X11ScreenOzone* screen() const
    {
        return screen_.get();
    }
    const display::Display& primary_display() const
    {
    ...
    void RemoveDisplayForTest(const display::Display& display_to_remove)
    {
        ...
        std::remove_copy(display_list.begin(), display_list.end(), new_displays.begin(), display_to_remove);
        UpdateDisplayListForTest(std::move(new_displays));
    }
    ...
    void UpdateDisplayListForTest(std::vector<display::Display> displays)
    {
        ...
        manager->change_notifier_.NotifyDisplaysChanged(old_displays, manager->displays_);
    }

    std::unique_ptr<X11Window> CreatePlatformWindow(MockPlatformWindowDelegate* delegate, const gfx::Rect& bounds, gfx::AcceleratedWidget* widget = nullptr)
    {
        EXPECT_CALL(*delegate, OnAcceleratedWidgetAvailable(_)).WillOnce(StoreWidget(widget));
...
TEST_F(X11ScreenOzoneTest, GetDisplayForWidgetSingleDisplay)
{
    ...
    EXPECT_EQ(primary, screen()->GetDisplayForAcceleratedWidget(gfx::kNullAcceleratedWidget));

    MockPlatformWindowDelegate delegate_1;
    gfx::AcceleratedWidget widget_1;
    constexpr gfx::Rect bounds_1(kPrimaryDisplayBounds.width() + 100, kPrimaryDisplayBounds.height() + 100, 200, 200);
...
{
    auto display_2 = CreateDisplay(gfx::Rect(kPrimaryDisplayBounds.width(), 0, 1280, 720));
    AddDisplayForTest(*display_2);
    ...
    window->SetBoundsInPixels(gfx::Rect(kPrimaryDisplayBounds.width() - 250, 0, 400, 300));
    EXPECT_EQ(primary_display(), screen()->GetDisplayForAcceleratedWidget(widget));
}
...
TEST_F(X11ScreenOzoneTest, GetDisplayNearestPointTwoDisplays)
{
    ...
    EXPECT_EQ(primary_display(), screen()->GetDisplayNearestPoint(gfx::Point(10, 10)));
    EXPECT_EQ(primary_display(), screen()->GetDisplayNearestPoint(gfx::Point(790, 100)));
    EXPECT_EQ(*display_2, screen()->GetDisplayNearestPoint(gfx::Point(1000, 10)));
    EXPECT_EQ(*display_2, screen()->GetDisplayNearestPoint(gfx::Point(10000, 10000)));
}
...
TEST_F(X11ScreenOzoneTest, GetDisplayMatchingMultiple)
{
    ...
    EXPECT_EQ(primary, screen()->GetDisplayMatching(gfx::Rect(1000, 600, 100, 100)));

    auto second = CreateDisplay(gfx::Rect(kPrimaryDisplayBounds.width(), 0, 1280, 720));
    AddDisplayForTest(*second);
    EXPECT_EQ(primary, screen()->GetDisplayMatching(gfx::Rect(50, 50, 100, 100)));
    EXPECT_EQ(*second, screen()->GetDisplayMatching(gfx::Rect(1000, 100, 100, 100)));
    EXPECT_EQ(*second, screen()->GetDisplayMatching(gfx::Rect(1000, 600, 100, 100)));

    // Check rectangle overlapping 2 displays
    EXPECT_EQ(primary, screen()->GetDisplayMatching(gfx::Rect(740, 0, 100, 100)));
    EXPECT_EQ(*second, screen()->GetDisplayMatching(gfx::Rect(760, 100, 100, 100)));
}
...
TEST_F(X11ScreenOzoneTest, GetDisplayNearestPoint)
{
    ...
    EXPECT_EQ(kFirstDisplay, screen()->GetDisplayNearestPoint(gfx::Point(630, 10)).id());
    EXPECT_EQ(kSecondDisplay, screen()->GetDisplayNearestPoint(gfx::Point(650, 10)).id());
    EXPECT_EQ(kFirstDisplay, screen()->GetDisplayNearestPoint(gfx::Point(10, 10)).id());
    EXPECT_EQ(kSecondDisplay, screen()->GetDisplayNearestPoint(gfx::Point(10000, 10000)).id());
    EXPECT_EQ(kFirstDisplay, screen()->GetDisplayNearestPoint(gfx::Point(639, -10)).id());
    EXPECT_EQ(kSecondDisplay, screen()->GetDisplayNearestPoint(gfx::Point(641, -20)).id());
    EXPECT_EQ(kSecondDisplay, screen()->GetDisplayNearestPoint(gfx::Point(600, 760)).id());
    EXPECT_EQ(kFirstDisplay, screen()->GetDisplayNearestPoint(gfx::Point(-1000, 760)).id());
}
...
TEST_F(X11ScreenOzoneTest, GetDisplayMatchingBasic)
{
    ...
    EXPECT_EQ(kSecondDisplay, screen()->GetDisplayMatching(gfx::Rect(700, 20, 100, 100)).id());
}
...
TEST_F(X11ScreenOzoneTest, GetDisplayMatchingOverlap)
{
    ...
    EXPECT_EQ(kSecondDisplay, screen()->GetDisplayMatching(gfx::Rect(630, 20, 100, 100)).id());
}
...
TEST_F(X11ScreenOzoneTest, GetDisplayNearestWindow)
{
    ...
    auto window_two = CreatePlatformWindow(&delegate2, gfx::Rect(650, 50, 10, 10));

    EXPECT_EQ(kFirstDisplay, screen()->GetDisplayForAcceleratedWidget(window_one->GetWidget()).id());
    EXPECT_EQ(kSecondDisplay, screen()->GetDisplayForAcceleratedWidget(window_two->GetWidget()).id());
