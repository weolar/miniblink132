...
bool CoalesceEventsIfNeeded(const x11::Event& xev, EventType type, x11::Event* out)
{
    if (xev.As<x11::MotionNotifyEvent>()
        || (xev.As<x11::Input::DeviceEvent>()
            && (type == ui::EventType::kTouchMoved || type == ui::EventType::kMouseMoved || type == ui::EventType::kMouseDragged))) {
...
{
    gfx::Size sanitized_size(std::max(bounds.width(), 1), std::max(bounds.height(), 1));
    gfx::Rect sanitized_bounds(bounds.origin(), sanitized_size);
    return sanitized_bounds;
}

void SerializeImageRepresentation(const gfx::ImageSkiaRep& rep, std::vector<uint32_t>* data)
{
...
void SyncSetCounter(x11::Connection* connection, x11::Sync::Counter counter, int64_t value)
{
    x11::Sync::Int64 sync_value { .hi = static_cast<int32_t>(value >> 32), .lo = static_cast<uint32_t>(value) };
    connection->sync().SetCounter({ counter, sync_value });
}

// Returns the whole path from |window| to the root.
std::vector<x11::Window> GetParentsList(x11::Connection* connection, x11::Window window)
{
...
void X11Window::Initialize(PlatformWindowInitProperties properties)
{
    ...
    // ourselves.
    if (properties.wm_role_name == kStatusIconWmRoleName && !InitializeAsStatusIcon()) {
        CloseXWindow();
        return;
    }
    ...
    x11_window_move_client_ = std::make_unique<ui::X11DesktopWindowMoveClient>(this);
    ...
    auto event_mask = x11::EventMask::ButtonPress | x11::EventMask::ButtonRelease | x11::EventMask::FocusChange | x11::EventMask::KeyPress
        | x11::EventMask::KeyRelease | x11::EventMask::EnterWindow | x11::EventMask::LeaveWindow | x11::EventMask::Exposure | x11::EventMask::VisibilityChange
        | x11::EventMask::StructureNotify | x11::EventMask::PropertyChange | x11::EventMask::PointerMotion;
    ...
    connection_->SetArrayProperty(xwindow_, x11::GetAtom("WM_PROTOCOLS"), x11::Atom::ATOM, protocols);

    // We need a WM_CLIENT_MACHINE value so we integrate with the desktop
    // environment.
    connection_->SetStringProperty(xwindow_, x11::Atom::WM_CLIENT_MACHINE, x11::Atom::STRING, net::GetHostName());

    // Likewise, the X server needs to know this window's pid so it knows which
    // program to kill if the window hangs.
    // XChangeProperty() expects "pid" to be long.
    static_assert(sizeof(uint32_t) >= sizeof(pid_t), "pid_t should not be larger than uint32_t");
    uint32_t pid = getpid();
    connection_->SetProperty(xwindow_, x11::GetAtom("_NET_WM_PID"), x11::Atom::CARDINAL, pid);
    ...
    connection_->SetProperty(xwindow_, x11::GetAtom("_NET_WM_WINDOW_TYPE"), x11::Atom::ATOM, window_type);
    ...
    if ((properties.type == PlatformWindowType::kPopup || properties.type == PlatformWindowType::kBubble) && !properties.force_show_in_taskbar) {
    ...
    if (properties.visible_on_all_workspaces) {
        ...
        connection_->SetProperty(xwindow_, x11::GetAtom("_NET_WM_DESKTOP"), x11::Atom::CARDINAL, kAllWorkspaces);
    } else if (!properties.workspace.empty()) {
        int32_t workspace;
        if (base::StringToInt(properties.workspace, &workspace)) {
            connection_->SetProperty(xwindow_, x11::GetAtom("_NET_WM_DESKTOP"), x11::Atom::CARDINAL, workspace);
        }
    }

    if (!properties.wm_class_name.empty() || !properties.wm_class_class.empty()) {
        SetWindowClassHint(&connection_.get(), xwindow_, properties.wm_class_name, properties.wm_class_class);
    }
    ...
    if (properties.remove_standard_frame) {
        ...
        SetHideTitlebarWhenMaximizedProperty(xwindow_, HIDE_TITLEBAR_WHEN_MAXIMIZED);
    }

    if (properties.prefer_dark_theme) {
        connection_->SetStringProperty(xwindow_, x11::GetAtom("_GTK_THEME_VARIANT"), x11::GetAtom("UTF8_STRING"), kDarkGtkThemeVariant);
    }
    ...
    if (IsSyncExtensionAvailable()) {
        ...
        std::vector<x11::Sync::Counter> counters { update_counter_, extended_update_counter_ };

        // Set XSyncCounter as window property _NET_WM_SYNC_REQUEST_COUNTER. the
        // compositor will listen on them during resizing.
        connection_->SetArrayProperty(xwindow_, x11::GetAtom("_NET_WM_SYNC_REQUEST_COUNTER"), x11::Atom::CARDINAL, counters);
    }
    ...
    connection_->SetProperty<uint32_t>(xwindow_, x11::GetAtom("_NET_WM_BYPASS_COMPOSITOR"), x11::Atom::CARDINAL, 2);
...
{
    gfx::Rect new_bounds_in_pixels(bounds.origin(), AdjustSizeForDisplay(bounds.size()));

    const bool size_changed = bounds_in_pixels_.size() != new_bounds_in_pixels.size();
    const bool origin_changed = bounds_in_pixels_.origin() != new_bounds_in_pixels.origin();
    ...
    if (size_changed) {
        ...
        if (new_bounds_in_pixels.width() < min_size_in_pixels_.width() || new_bounds_in_pixels.height() < min_size_in_pixels_.height()
            || (!max_size_in_pixels_.IsEmpty()
                && (new_bounds_in_pixels.width() > max_size_in_pixels_.width() || new_bounds_in_pixels.height() > max_size_in_pixels_.height()))) {
...
{
    SetBoundsInPixels(platform_window_delegate_->ConvertRectToPixels(bounds_in_dip));
}
...
void X11Window::SetTitle(const std::u16string& title)
{
    ...
    connection_->SetStringProperty(xwindow_, x11::GetAtom("_NET_WM_NAME"), x11::GetAtom("UTF8_STRING"), utf8str);
    connection_->SetStringProperty(xwindow_, x11::Atom::WM_NAME, x11::GetAtom("UTF8_STRING"), utf8str);
}
...
void X11Window::SetCapture()
{
    ...
    has_pointer_grab_ |= (ui::GrabPointer(xwindow_, true, nullptr) == x11::GrabStatus::Success);
}
...
void X11Window::Maximize()
{
    if (IsFullscreen()) {
        ...
        gfx::Rect adjusted_bounds_in_pixels(bounds_in_pixels.origin(), AdjustSizeForDisplay(bounds_in_pixels.size()));
    ...
    SetWMSpecState(true, x11::GetAtom("_NET_WM_STATE_MAXIMIZED_VERT"), x11::GetAtom("_NET_WM_STATE_MAXIMIZED_HORZ"));
}

void X11Window::Minimize()
{
    if (window_mapped_in_client_) {
        SendClientMessage(xwindow_, x_root_window_, x11::GetAtom("WM_CHANGE_STATE"), { x11::WM_STATE_ICONIC, 0, 0, 0, 0 });
    } else {
...
void X11Window::Restore()
{
    if (IsMinimized()) {
        ...
        SetWMSpecState(false, x11::GetAtom("_NET_WM_STATE_MAXIMIZED_VERT"), x11::GetAtom("_NET_WM_STATE_MAXIMIZED_HORZ"));
    }
}
...
void X11Window::Activate()
{
    ...
    static bool wm_supports_active_window = GuessWindowManager() != WM_WMII && connection_->WmSupportsHint(x11::GetAtom("_NET_ACTIVE_WINDOW"));
    ...
    if (wm_supports_active_window && !override_redirect_) {
        ...
        SendClientMessage(xwindow_, x_root_window_, x11::GetAtom("_NET_ACTIVE_WINDOW"), data);
    } else {
        ...
        connection_->SetInputFocus({ x11::InputFocus::Parent, xwindow_, timestamp }).IgnoreError();
...
void X11Window::SetCursor(scoped_refptr<PlatformCursor> cursor)
{
    ...
    on_cursor_loaded_.Reset(base::BindOnce(&x11::Connection::DefineCursor, base::Unretained(connection_), xwindow_));
...
void X11Window::ConfineCursorToBounds(const gfx::Rect& bounds)
{
    ...
    auto make_barrier = [&](uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, x11::XFixes::BarrierDirections directions) {
        x11::XFixes::Barrier barrier = connection_->GenerateId<x11::XFixes::Barrier>();
        connection_->xfixes().CreatePointerBarrier({ barrier, x_root_window_, x1, y1, x2, y2, directions });
        return barrier;
    };

    // Top horizontal barrier.
    pointer_barriers_[0] = make_barrier(barrier.x(), barrier.y(), barrier.right(), barrier.y(), x11::XFixes::BarrierDirections::PositiveY);
    // Bottom horizontal barrier.
    pointer_barriers_[1] = make_barrier(barrier.x(), barrier.bottom(), barrier.right(), barrier.bottom(), x11::XFixes::BarrierDirections::NegativeY);
    // Left vertical barrier.
    pointer_barriers_[2] = make_barrier(barrier.x(), barrier.y(), barrier.x(), barrier.bottom(), x11::XFixes::BarrierDirections::PositiveX);
    // Right vertical barrier.
    pointer_barriers_[3] = make_barrier(barrier.right(), barrier.y(), barrier.right(), barrier.bottom(), x11::XFixes::BarrierDirections::NegativeX);
...
{
    restored_bounds_in_pixels_ = platform_window_delegate_->ConvertRectToPixels(bounds);
}

gfx::Rect X11Window::GetRestoredBoundsInDIP() const
{
    return platform_window_delegate_->ConvertRectToDIP(restored_bounds_in_pixels_);
}
...
void X11Window::SetZOrderLevel(ZOrderLevel order)
{
    ...
    SetWMSpecState(is_always_on_top_, x11::GetAtom("_NET_WM_STATE_ABOVE"), x11::Atom::None);
}
...
void X11Window::StackAbove(gfx::AcceleratedWidget widget)
{
    ...
    std::vector<x11::Window> window_below_parents = GetParentsList(&connection_.get(), window);
    std::vector<x11::Window> window_above_parents = GetParentsList(&connection_.get(), xwindow_);
...
void X11Window::SetShape(std::unique_ptr<ShapeRects> native_shape, const gfx::Transform& transform)
{
    ...
    if (native_shape) {
        ...
        if (!transform.IsIdentity() && !native_region.isEmpty()) {
            ...
            if (native_region.getBoundaryPath(&path_in_dip)) {
                ...
                path_in_dip.transform(gfx::TransformToFlattenedSkMatrix(transform), &path_in_pixels);
...
void X11Window::SetAspectRatio(const gfx::SizeF& aspect_ratio)
{
    ...
    if (aspect_ratio.IsEmpty()) {
        ...
        size_hints.flags |= x11::SIZE_HINT_P_ASPECT;
        size_hints.min_aspect_num = size_hints.max_aspect_num = aspect_ratio.width();
        size_hints.min_aspect_den = size_hints.max_aspect_den = aspect_ratio.height();
    }
    connection_->SetWmNormalHints(xwindow_, size_hints);
}

void X11Window::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
    ...
    if (!data.empty()) {
        connection_->SetArrayProperty(xwindow_, x11::GetAtom("_NET_WM_ICON"), x11::Atom::CARDINAL, data);
    }
}
...
void X11Window::SetOpacity(float opacity)
{
    ...
    if (cardinality == 0xffffffff) {
        connection_->DeleteProperty(xwindow_, x11::GetAtom("_NET_WM_WINDOW_OPACITY"));
    } else {
        connection_->SetProperty(xwindow_, x11::GetAtom("_NET_WM_WINDOW_OPACITY"), x11::Atom::CARDINAL, cardinality);
    }
}
...
void X11Window::SetOpaqueRegion(std::optional<std::vector<gfx::Rect>> region_px)
{
...
void X11Window::SetInputRegion(std::optional<std::vector<gfx::Rect>> region_px)
{
    ...
    connection_->shape().Rectangles(x11::Shape::RectanglesRequest {
        ...
        .destination_window = xwindow_,
        .rectangles = { { static_cast<int16_t>((*region_px)[0].x()), static_cast<int16_t>((*region_px)[0].y()), static_cast<uint16_t>((*region_px)[0].width()),
...
void X11Window::NotifyStartupComplete(const std::string& startup_id)
{
    ...
    for (size_t offset = 0; offset < data_size; offset += kChunkSize) {
        ...
        connection_->SendEvent(event, x_root_window_, x11::EventMask::PropertyChange);
...
std::string X11Window::GetWorkspace() const
{
    ...
    return workspace_id.has_value() ? base::NumberToString(workspace_id.value()) : std::string();
}

void X11Window::SetVisibleOnAllWorkspaces(bool always_visible)
{
    SetWMSpecState(always_visible, x11::GetAtom("_NET_WM_STATE_STICKY"), x11::Atom::None);
    ...
    SendClientMessage(xwindow_, x_root_window_, x11::GetAtom("_NET_WM_DESKTOP"), { static_cast<uint32_t>(new_desktop), 0, 0, 0, 0 });
}
...
void X11Window::SetWorkspaceExtensionDelegate(WorkspaceExtensionDelegate* delegate)
{
...
void X11Window::OnCompleteSwapAfterResize()
{
    if (configure_counter_value_is_extended_) {
        if ((current_counter_value_ % 2) == 1) {
            ...
            SyncSetCounter(&connection_.get(), extended_update_counter_, current_counter_value_);
        }
        return;
    }

    if (configure_counter_value_ != 0) {
        SyncSetCounter(&connection_.get(), update_counter_, configure_counter_value_);
...
bool X11Window::HandleAsAtkEvent(const x11::KeyEvent& key_event, bool send_event, bool transient)
{
#if !BUILDFLAG(USE_ATK)
...
void X11Window::OnEvent(const x11::Event& xev)
{
    ...
    if (prop && target_current_context && prop->window == target_current_context->source_window()) {
...
{
    TRACE_EVENT1("views", "X11PlatformWindow::Dispatch", "event->type()", event->type());
    ...
    if (auto* key = current_xevent.As<x11::KeyEvent>()) {
        if (HandleAsAtkEvent(*key, current_xevent.send_event(), key->event == transient_window_)) {
...
void X11Window::DispatchUiEvent(ui::Event* event, const x11::Event& xev)
{
    ...
    if (event->IsLocatedEvent() && located_events_grabber && located_events_grabber != this) {
        if (event->IsMouseEvent() || (event->IsTouchEvent() && event->type() == ui::EventType::kTouchPressed)) {
            // Another X11Window has installed itself as capture. Translate the
            // event's location and dispatch to the other.
            ConvertEventLocationToTargetWindowLocation(
                located_events_grabber->GetBoundsInPixels().origin(), GetBoundsInPixels().origin(), event->AsLocatedEvent());
        }
    ...
    DispatchEventFromNativeUiEvent(event, base::BindOnce(&PlatformWindowDelegate::DispatchEvent, base::Unretained(platform_window_delegate())));
}

void X11Window::UpdateDecorationInsets()
{
    auto atom = x11::GetAtom("_GTK_FRAME_EXTENTS");
    auto insets_dip = platform_window_delegate_->CalculateInsetsInDIP(GetPlatformWindowState());
    ...
    CHECK(GetPlatformWindowState() == PlatformWindowState::kNormal || GetPlatformWindowState() == PlatformWindowState::kUnknown);
    ...
    connection_->SetArrayProperty(xwindow_, atom, x11::Atom::CARDINAL,
        std::vector<uint32_t> { static_cast<uint32_t>(insets_px.left()), static_cast<uint32_t>(insets_px.right()), static_cast<uint32_t>(insets_px.top()),
...
std::optional<gfx::Size> X11Window::GetMinimumSizeForXWindow()
{
    if (auto max_size = platform_window_delegate_->GetMinimumSizeForWindow()) {
        return platform_window_delegate_->ConvertRectToPixels(gfx::Rect(*max_size)).size();
    }
    return std::nullopt;
}
...
void X11Window::DispatchHostWindowDragMovement(int hittest, const gfx::Point& pointer_location_in_px)
{
    ...
    DoWMMoveResize(&connection_.get(), x_root_window_, xwindow_, pointer_location_in_px, direction);
}
...
bool X11Window::StartDrag(const OSExchangeData& data, int operations, mojom::DragEventSource source, gfx::NativeCursor cursor, bool can_grab_pointer,
    base::OnceClosure drag_started_callback, WmDragHandler::DragFinishedCallback drag_finished_callback, WmDragHandler::LocationDelegate* location_delegate)
{
    DCHECK(drag_drop_client_);
    ...
    const bool dropped = drag_loop_->RunMoveLoop(can_grab_pointer, last_cursor_, last_cursor_, std::move(drag_started_callback));
    if (!alive) {
        return false;
    }
...
void X11Window::UpdateDragImage(const gfx::ImageSkia& image, const gfx::Vector2d& offset)
{
    NOTIMPLEMENTED();
}
...
int X11Window::UpdateDrag(const gfx::Point& connection_point)
{
    ...
    auto data = std::make_unique<OSExchangeData>(std::make_unique<XOSExchangeDataProvider>(
        drag_drop_client_->xwindow(), target_current_context->source_window(), target_current_context->fetched_targets()));
    ...
    if (data->HasFile() && (suggested_operations & (DragDropTypes::DRAG_MOVE | DragDropTypes::DRAG_LINK))) {
        suggested_operations |= DragDropTypes::DRAG_COPY;
    }

    XDragDropClient* source_client = XDragDropClient::GetForWindow(target_current_context->source_window());
    gfx::PointF local_point_in_dip = platform_window_delegate_->ConvertScreenPointToLocalDIP(connection_point);
    if (!notified_enter_) {
        drop_handler->OnDragEnter(local_point_in_dip, suggested_operations, GetKeyModifiers(source_client));
    ...
    allowed_drag_operations_ = drop_handler->OnDragMotion(local_point_in_dip, suggested_operations, GetKeyModifiers(source_client));
...
{
    notified_enter_ = false;
    source_window_events_ = connection_->ScopedSelectEvent(window, x11::EventMask::PropertyChange);
}
...
DragOperation X11Window::PerformDrop()
{
    ...
    drop_handler->OnDragDrop(GetKeyModifiers(XDragDropClient::GetForWindow(target_current_context->source_window())));
    notified_enter_ = false;
...
void X11Window::EndDragLoop()
{
    ...
    std::move(drag_finished_callback_).Run(PreferredDragOperation(allowed_drag_operations_));
    drag_loop_->EndMoveLoop();
}

void X11Window::OnMouseMovement(const gfx::Point& connection_point, int flags, base::TimeTicks event_time)
{
...
gfx::Size X11Window::AdjustSizeForDisplay(const gfx::Size& requested_size_in_pixels)
{
    ...
    if (screen && !UseTestConfigForPlatformWindows()) {
        ...
        for (const auto& display : displays) {
            if (requested_size_in_pixels == display.GetSizeInPixel()) {
                return gfx::Size(requested_size_in_pixels.width() - 1, requested_size_in_pixels.height() - 1);
            }
        }
    }
...
{
    auto bounds = platform_window_delegate_->ConvertRectToPixels(properties.bounds);

    gfx::Size adjusted_size_in_pixels = AdjustSizeForDisplay(bounds.size());
    bounds.set_size(adjusted_size_in_pixels);
    const auto override_redirect = properties.x11_extension_delegate && properties.x11_extension_delegate->IsOverrideRedirect();
    ...
    req.background_pixel = properties.background_color.has_value() ? properties.background_color.value() : connection_->default_screen().white_pixel;
    ...
    if (properties.wm_role_name == kStatusIconWmRoleName) {
        std::string atom_name = "_NET_SYSTEM_TRAY_S" + base::NumberToString(connection_->DefaultScreenId());
        auto selection = connection_->GetSelectionOwner({ x11::GetAtom(atom_name.c_str()) });
        if (auto reply = selection.Sync()) {
            connection_->GetPropertyAs(reply->owner, x11::GetAtom("_NET_SYSTEM_TRAY_VISUAL"), &visual_id_);
        }
    }
    ...
    if (visual_id_ == x11::VisualId {} || !visual_manager.GetVisualInfo(visual_id_, &depth, &colormap, &visual_has_alpha_)) {
        visual_manager.ChooseVisualForWindow(enable_transparent_visuals, &visual_id, &depth, &colormap, &visual_has_alpha_);
    }
...
void X11Window::CloseXWindow()
{
    ...
    if (is_security_surface_) {
        ...
        security_surfaces.erase(base::ranges::find(security_surfaces, xwindow_), security_surfaces.end());
    }
...
void X11Window::Map(bool inactive)
{
    ...
    auto wm_user_time_ms = ignore_keyboard_input_ ? x11::Time::CurrentTime : X11EventSource::GetInstance()->GetTimestamp();
    if (inactive || wm_user_time_ms != x11::Time::CurrentTime) {
        connection_->SetProperty(xwindow_, x11::GetAtom("_NET_WM_USER_TIME"), x11::Atom::CARDINAL, wm_user_time_ms);
    }
    ...
    if (window_properties_.empty()) {
        ...
        connection_->SetArrayProperty(
            xwindow_, x11::GetAtom("_NET_WM_STATE"), x11::Atom::ATOM, std::vector<x11::Atom>(std::begin(window_properties_), std::end(window_properties_)));
    }
...
{
    SetWMSpecState(fullscreen, x11::GetAtom("_NET_WM_STATE_FULLSCREEN"), x11::Atom::None);
}
...
{
    return HasWMSpecProperty(window_properties_, x11::GetAtom("_NET_WM_STATE_HIDDEN"));
}
...
bool X11Window::IsMaximized() const
{
    ...
    return (HasWMSpecProperty(window_properties_, x11::GetAtom("_NET_WM_STATE_MAXIMIZED_VERT"))
        && HasWMSpecProperty(window_properties_, x11::GetAtom("_NET_WM_STATE_MAXIMIZED_HORZ")));
}

bool X11Window::IsFullscreen() const
{
    return HasWMSpecProperty(window_properties_, x11::GetAtom("_NET_WM_STATE_FULLSCREEN"));
}
...
void X11Window::UpdateMinAndMaxSize()
{
    ...
    if ((!minimum_in_pixels || min_size_in_pixels_ == minimum_in_pixels.value()) && (!maximum_in_pixels || max_size_in_pixels_ == maximum_in_pixels.value())) {
        return;
    }
...
{
    PlatformWindowOcclusionState occlusion_state = is_occluded_ ? PlatformWindowOcclusionState::kOccluded : PlatformWindowOcclusionState::kVisible;
...
void X11Window::OnCrossingEvent(bool enter, bool focus_in_window_or_ancestor, x11::NotifyMode mode, x11::NotifyDetail detail)
{
...
void X11Window::OnFocusEvent(bool focus_in, x11::NotifyMode mode, x11::NotifyDetail detail)
{
    ...
    bool notify_grab = mode == x11::NotifyMode::Grab || mode == x11::NotifyMode::Ungrab;
...
void X11Window::HandleEvent(const x11::Event& xev)
{
    ...
    if (auto* crossing = xev.As<x11::CrossingEvent>()) {
        ...
        OnCrossingEvent(crossing->opcode == x11::CrossingEvent::EnterNotify, focus, crossing->mode, crossing->detail);
    } else if (auto* expose = xev.As<x11::ExposeEvent>()) {
        gfx::Rect damage_rect_in_pixels(expose->x, expose->y, expose->width, expose->height);
        OnXWindowDamageEvent(damage_rect_in_pixels);
    } else if (auto* focus = xev.As<x11::FocusEvent>()) {
        OnFocusEvent(focus->opcode == x11::FocusEvent::In, focus->mode, focus->detail);
        ...
        if (message_type == x11::GetAtom("WM_PROTOCOLS")) {
            ...
            if (protocol == x11::GetAtom("WM_DELETE_WINDOW")) {
                ...
                connection_->SendEvent(reply_event, x_root_window_, x11::EventMask::SubstructureNotify | x11::EventMask::SubstructureRedirect);
            } else if (protocol == x11::GetAtom("_NET_WM_SYNC_REQUEST")) {
                pending_counter_value_ = client->data.data32[2] + (static_cast<int64_t>(client->data.data32[3]) << 32);
...
void X11Window::UpdateWMUserTime(Event* event)
{
    ...
    if (type == EventType::kMousePressed || type == EventType::kKeyPressed || type == EventType::kTouchPressed) {
        uint32_t wm_user_time_ms = (event->time_stamp() - base::TimeTicks()).InMilliseconds();
        connection_->SetProperty(xwindow_, x11::GetAtom("_NET_WM_USER_TIME"), x11::Atom::CARDINAL, wm_user_time_ms);
    }
}
...
void X11Window::OnConfigureEvent(const x11::ConfigureNotifyEvent& configure, bool send_event)
{
    ...
    if (!send_event && !configure.override_redirect) {
        auto future = connection_->TranslateCoordinates({ xwindow_, x_root_window_, 0, 0 });
        if (auto coords = future.Sync()) {
            translated_x_in_pixels = coords->dst_x;
            translated_y_in_pixels = coords->dst_y;
        }
    }
    gfx::Rect new_bounds_px(translated_x_in_pixels, translated_y_in_pixels, configure.width, configure.height);
    const bool size_changed = bounds_in_pixels_.size() != new_bounds_px.size();
    const bool origin_changed = bounds_in_pixels_.origin() != new_bounds_px.origin();
...
void X11Window::SetWMSpecState(bool enabled, x11::Atom state1, x11::Atom state2)
{
    if (window_mapped_in_client_) {
...
void X11Window::OnWMStateUpdated()
{
    ...
    if (connection_->GetArrayProperty(xwindow_, x11::GetAtom("_NET_WM_STATE"), &atom_list) || window_mapped_in_client_) {
        UpdateWindowProperties(base::flat_set<x11::Atom>(std::begin(atom_list), std::end(atom_list)));
    }
}

WindowTiledEdges X11Window::GetTiledState() const
{
    const bool vert = HasWMSpecProperty(window_properties_, x11::GetAtom("_NET_WM_STATE_MAXIMIZED_VERT"));
    const bool horz = HasWMSpecProperty(window_properties_, x11::GetAtom("_NET_WM_STATE_MAXIMIZED_HORZ"));
    return WindowTiledEdges { vert, vert, horz, horz };
}

void X11Window::UpdateWindowProperties(const base::flat_set<x11::Atom>& new_window_properties)
{
    ...
    is_always_on_top_ = HasWMSpecProperty(window_properties_, x11::GetAtom("_NET_WM_STATE_ABOVE"));
...
void X11Window::OnFrameExtentsUpdated()
{
    ...
    if (connection_->GetArrayProperty(xwindow_, x11::GetAtom("_NET_FRAME_EXTENTS"), &insets) && insets.size() == 4) {
        // |insets| are returned in the order: [left, right, top, bottom].
        native_window_frame_borders_in_pixels_ = gfx::Insets::TLBR(insets[2], insets[0], insets[3], insets[1]);
    } else {
...
{
    if (update_counter_ == x11::Sync::Counter {} || configure_counter_value_ == 0) {
        // WM doesn't support _NET_WM_SYNC_REQUEST. Or we are too slow, so
        // _NET_WM_SYNC_REQUEST is disabled by the compositor.
        delayed_resize_task_.Reset(base::BindOnce(&X11Window::DelayedResize, base::Unretained(this), origin_changed));
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, delayed_resize_task_.callback());
        return;
    }
...
{
    if (configure_counter_value_is_extended_ && (current_counter_value_ % 2) == 0) {
...
void X11Window::UpdateWindowRegion(std::unique_ptr<std::vector<x11::Rectangle>> region)
{
...
{
    std::string atom_name = "_NET_SYSTEM_TRAY_S" + base::NumberToString(connection_->DefaultScreenId());
    auto reply = connection_->GetSelectionOwner({ x11::GetAtom(atom_name.c_str()) }).Sync();
    ...
    connection_->SetArrayProperty(
        xwindow_, x11::GetAtom("_XEMBED_INFO"), x11::Atom::CARDINAL, std::vector<uint32_t> { kXembedInfoProtocolVersion, kXembedInfoFlags });

    x11::ChangeWindowAttributesRequest req { xwindow_ };
    if (visual_has_alpha_) {
        req.background_pixel = 0;
    } else {
        connection_->SetProperty(xwindow_, x11::GetAtom("CHROMIUM_COMPOSITE_WINDOW"), x11::Atom::CARDINAL, static_cast<uint32_t>(1));
        req.background_pixmap = static_cast<x11::Pixmap>(x11::BackPixmap::ParentRelative);
    }
    connection_->ChangeWindowAttributes(req);

    auto future = SendClientMessage(manager, manager, x11::GetAtom("_NET_SYSTEM_TRAY_OPCODE"),
        { static_cast<uint32_t>(X11EventSource::GetInstance()->GetTimestamp()), kSystemTrayRequestDock, static_cast<uint32_t>(xwindow_), 0, 0 },
