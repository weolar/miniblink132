...
std::unique_ptr<AtkKeyEventStruct> AtkKeyEventFromXEvent(const x11::KeyEvent& xkey, bool send_event)
{
    auto atk_key_event = std::make_unique<AtkKeyEventStruct>();

    atk_key_event->type = xkey.opcode == x11::KeyEvent::Press ? ATK_KEY_EVENT_PRESS : ATK_KEY_EVENT_RELEASE;
