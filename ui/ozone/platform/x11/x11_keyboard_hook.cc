...
const x11::ModMask kModifierMasks[] = { {}, // No additional modifier.
...
// locked.
const DomCode kDomCodesForLockAllKeys[] = { DomCode::ESCAPE, DomCode::CONTEXT_MENU, DomCode::CONTROL_LEFT, DomCode::SHIFT_LEFT, DomCode::ALT_LEFT,
    DomCode::META_LEFT, DomCode::CONTROL_RIGHT, DomCode::SHIFT_RIGHT, DomCode::ALT_RIGHT, DomCode::META_RIGHT };

} // namespace

X11KeyboardHook::X11KeyboardHook(
    std::optional<base::flat_set<DomCode>> dom_codes, BaseKeyboardHook::KeyEventCallback callback, gfx::AcceleratedWidget accelerated_widget)
...
X11KeyboardHook::~X11KeyboardHook()
{
    ...
    for (int native_key_code : grabbed_keys_) {
        for (auto modifier : kModifierMasks) {
            connection_->UngrabKey({ static_cast<x11::KeyCode>(native_key_code), window_, modifier });
        }
    }
}

void X11KeyboardHook::RegisterHook(const std::optional<base::flat_set<DomCode>>& dom_codes)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void X11KeyboardHook::CaptureSpecificKeys(const std::optional<base::flat_set<DomCode>>& dom_codes)
{
