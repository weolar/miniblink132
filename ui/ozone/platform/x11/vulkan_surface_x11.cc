...
// static
std::unique_ptr<VulkanSurfaceX11> VulkanSurfaceX11::Create(VkInstance vk_instance, x11::Window parent_window)
{
    auto* connection = x11::Connection::Get();
    auto geometry = connection->GetGeometry(parent_window).Sync();
    if (!geometry) {
        LOG(ERROR) << "GetGeometry failed for window " << static_cast<uint32_t>(parent_window) << ".";
        return nullptr;
    }
    ...
    VkResult result = vkCreateXcbSurfaceKHR(vk_instance, &surface_create_info, nullptr, &vk_surface);
    if (VK_SUCCESS != result) {
        DLOG(ERROR) << "vkCreateXcbSurfaceKHR() failed: " << result;
        return nullptr;
    }
    return std::make_unique<VulkanSurfaceX11>(vk_instance, vk_surface, parent_window, window);
}
...
// be created.
VulkanSurfaceX11::VulkanSurfaceX11(VkInstance vk_instance, VkSurfaceKHR vk_surface, x11::Window parent_window, x11::Window window)
    : gpu::VulkanSurface(vk_instance, static_cast<gfx::AcceleratedWidget>(window), vk_surface,
        /*acquire_next_image_timeout_ns=*/base::Time::kNanosecondsPerSecond * 2, std::make_unique<ui::XrandrIntervalOnlyVSyncProvider>())
    , parent_window_(parent_window)
    , window_(window)
    , event_selector_(x11::Connection::Get()->ScopedSelectEvent(window, x11::EventMask::Exposure))
{
...
bool VulkanSurfaceX11::Reshape(const gfx::Size& size, gfx::OverlayTransform pre_transform)
{
    ...
    x11::Connection::Get()->ConfigureWindow(x11::ConfigureWindowRequest { .window = window_, .width = size.width(), .height = size.height() }).Sync();
...
void VulkanSurfaceX11::OnEvent(const x11::Event& event)
{
    ...
    x11::Connection::Get()->SendEvent(forwarded_event, parent_window_, x11::EventMask::Exposure);
