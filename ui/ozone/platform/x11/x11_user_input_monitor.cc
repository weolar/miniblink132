...
X11UserInputMonitor::X11UserInputMonitor(const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner)
    : user_input_monitor_(std::make_unique<ui::XUserInputMonitor>(io_task_runner))
{
}
...
void X11UserInputMonitor::StartMonitor(PlatformUserInputMonitor::WriteKeyPressCallback callback)
{
    user_input_monitor_->StartMonitor(callback);
}

void X11UserInputMonitor::StartMonitorWithMapping(PlatformUserInputMonitor::WriteKeyPressCallback callback, base::WritableSharedMemoryMapping mapping)
{
