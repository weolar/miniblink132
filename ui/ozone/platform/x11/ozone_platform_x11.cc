...
class OzonePlatformX11 : public OzonePlatform, public OSExchangeDataProviderFactoryOzone {
public:
    OzonePlatformX11()
    {
        SetInstance(this);
    ...
    PlatformUtils* GetPlatformUtils() override
    {
        return x11_utils_.get();
    }

    PlatformGlobalShortcutListener* GetPlatformGlobalShortcutListener(PlatformGlobalShortcutListenerDelegate* delegate) override
    {
        if (!global_shortcut_listener_) {
            global_shortcut_listener_ = std::make_unique<X11GlobalShortcutListenerOzone>(delegate);
        }
        ...
        std::optional<base::flat_set<DomCode>> dom_codes, gfx::AcceleratedWidget accelerated_widget) override
    {
        ...
        case PlatformKeyboardHookTypes::kModifier:
            return std::make_unique<X11KeyboardHook>(std::move(dom_codes), std::move(callback), accelerated_widget);
    ...
    const PlatformProperties& GetPlatformProperties() override
    {
        ...
        if (!initialised) {
            ...
            properties->message_pump_type_for_viz_compositor = base::MessagePumpType::UI;
    ...
    const PlatformRuntimeProperties& GetPlatformRuntimeProperties() override
    {
        ...
        if (has_initialized_gpu() && ui::GpuMemoryBufferSupportX11::GetInstance()->has_gbm_device()) {
    ...
    {
        return x11::Connection::Get()->GetOrCreateVisualManager().ArgbVisualAvailable();
    }
    ...
    bool InitializeUI(const InitParams& params) override
    {
        ...
        // cannot initialise.
        if (!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kHeadless) && !x11::Connection::Get()->Ready()) {
    ...
    void InitializeGPU(const InitParams& params) override
    {
        ...
        if (params.enable_native_gpu_memory_buffers) {
            base::ThreadPool::PostTask(FROM_HERE, base::BindOnce([]() { ui::GpuMemoryBufferSupportX11::GetInstance(); }));
        }
    ...
    std::unique_ptr<PlatformUserInputMonitor> GetPlatformUserInputMonitor(const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner) override
    {
