...
public:
    ...
    void SetShape(std::unique_ptr<ShapeRects> native_shape, const gfx::Transform& transform) override;
    ...
    void SetOpaqueRegion(std::optional<std::vector<gfx::Rect>> region_px) override;
    ...
    void SetWorkspaceExtensionDelegate(WorkspaceExtensionDelegate* delegate) override;
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(X11WindowTest, ToggleMinimizePropogateToPlatformWindowDelegate);
    ...
    // WmDragHandler:
    bool StartDrag(const OSExchangeData& data, int operations, mojom::DragEventSource source, gfx::NativeCursor cursor, bool can_grab_pointer,
    ...
    bool HandleAsAtkEvent(const x11::KeyEvent& key_event, bool send_event, bool transient);
    ...
    x11::Window root_window() const
    {
        return x_root_window_;
    }
    std::vector<x11::Rectangle>* shape() const
    {
    ...
    // XIFocusOutEvent.
    void OnFocusEvent(bool focus_in, x11::NotifyMode mode, x11::NotifyDetail detail);

    // Called on an XEnterWindowEvent, XLeaveWindowEvent, XIEnterEvent, or an
    // XILeaveEvent.
    void OnCrossingEvent(bool enter, bool focus_in_window_or_ancestor, x11::NotifyMode mode, x11::NotifyDetail detail);
    ...
    void OnConfigureEvent(const x11::ConfigureNotifyEvent& event, bool send_event);
    ...
    void UpdateWindowProperties(const base::flat_set<x11::Atom>& new_window_properties);
    ...
    raw_ptr<WorkspaceExtensionDelegate, DanglingUntriaged> workspace_extension_delegate_ = nullptr;

    raw_ptr<X11ExtensionDelegate, DanglingUntriaged> x11_extension_delegate_ = nullptr;
    ...
    raw_ptr<WmDragHandler::LocationDelegate, DanglingUntriaged> drag_location_delegate_ = nullptr;
    ...
    PlatformWindowOcclusionState occlusion_state_ = PlatformWindowOcclusionState::kUnknown;
