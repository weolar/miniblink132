...
x11::Pixmap XPixmapFromNativePixmap(const gfx::NativePixmap& native_pixmap, gfx::BufferFormat buffer_format)
{
    ...
    if (!base::IsValueInRangeForNumericType<uint32_t>(native_pixmap.GetDmaBufPlaneSize(0))) {
        VLOG(1) << "Could not import the dma-buf as an XPixmap because the "
                   "dma-buf's byte size is out-of-range";
        return x11::Pixmap::None;
    }
    buffer_byte_size = base::checked_cast<uint32_t>(native_pixmap.GetDmaBufPlaneSize(0));

    uint16_t width;
    if (!base::IsValueInRangeForNumericType<uint16_t>(native_pixmap.GetBufferSize().width())) {
        VLOG(1) << "Could not import the dma-buf as an XPixmap because the "
    ...
    uint16_t height;
    if (!base::IsValueInRangeForNumericType<uint16_t>(native_pixmap.GetBufferSize().height())) {
        VLOG(1) << "Could not import the dma-buf as an XPixmap because the "
                   "dma-buf's width is out-of-range";
        return x11::Pixmap::None;
    }
    ...
    uint16_t stride;
    if (!base::IsValueInRangeForNumericType<uint16_t>(native_pixmap.GetDmaBufPitch(0))) {
        VLOG(1) << "Could not import the dma-buf as an XPixmap because the "
                   "dma-buf's width is out-of-range";
        return x11::Pixmap::None;
    }
    ...
    auto response = connection->dri3()
                        .PixmapFromBuffer(pixmap_id, connection->default_root(), buffer_byte_size, width, height, stride, depth, bpp, ref_counted_fd)
                        .Sync();
    if (response.error) {
...
bool NativePixmapEGLX11Binding::IsBufferFormatSupported(gfx::BufferFormat format)
{
...
bool NativePixmapEGLX11Binding::Initialize(x11::Pixmap pixmap)
{
    ...
    EGLint attribs[] = { EGL_BUFFER_SIZE, 32, EGL_ALPHA_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_RED_SIZE, 8, EGL_SURFACE_TYPE, EGL_PIXMAP_BIT,
        EGL_BIND_TO_TEXTURE_RGBA, EGL_TRUE, EGL_NONE };

    EGLint num_configs;
    EGLConfig config = nullptr;

    if ((eglChooseConfig(display_, attribs, &config, 1, &num_configs) != EGL_TRUE) || !num_configs) {
        return false;
    }

    std::vector<EGLint> attrs = { EGL_TEXTURE_FORMAT, EGL_TEXTURE_RGBA, EGL_TEXTURE_TARGET, EGL_TEXTURE_2D, EGL_NONE };

    surface_ = eglCreatePixmapSurface(display_, config, static_cast<EGLNativePixmapType>(pixmap), attrs.data());
    return surface_ != EGL_NO_SURFACE;
}

// static
std::unique_ptr<NativePixmapGLBinding> NativePixmapEGLX11Binding::Create(
    scoped_refptr<gfx::NativePixmap> native_pixmap, gfx::BufferFormat plane_format, gfx::Size plane_size, GLenum target, GLuint texture_id)
{
    if (native_pixmap->GetBufferFormat() != plane_format || !gl::IsFormatSupported(plane_format)) {
        VLOG(1) << "Format " << gfx::BufferFormatToString(plane_format) << " is unsupported or does not match the NativePixmap's format ("
                << gfx::BufferFormatToString(native_pixmap->GetBufferFormat()) << ")";
        return nullptr;
    }

    if (native_pixmap->GetBufferSize() != plane_size) {
        VLOG(1) << "The native pixmap size (" << native_pixmap->GetBufferSize().ToString() << ") does not match |plane_size| (" << plane_size.ToString() << ")";
        return nullptr;
    }
    ...
    x11::Pixmap pixmap = gl::XPixmapFromNativePixmap(*native_pixmap, plane_format);
...
{
    gl::ScopedTextureBinder binder(base::strict_cast<unsigned int>(target), base::strict_cast<unsigned int>(texture_id));
