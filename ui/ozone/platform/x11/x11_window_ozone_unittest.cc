...
protected:
    std::unique_ptr<PlatformWindow> CreatePlatformWindow(
        MockPlatformWindowDelegate* delegate, const gfx::Rect& bounds, gfx::AcceleratedWidget* widget, X11ExtensionDelegate* x11_extension_delegate)
    {
...
TEST_F(X11WindowOzoneTest, SendPlatformEventToRightTarget)
{
    ...
    xi_event.InitGenericButtonEvent(kPointerDeviceId, EventType::kMousePressed, gfx::Point(218, 290), EF_NONE);
    ...
    auto window_2 = CreatePlatformWindow(&delegate_2, bounds_2, &widget_2, nullptr);
...
TEST_F(X11WindowOzoneTest, MouseEnterAndDelete)
{
    ...
    gfx::AcceleratedWidget widget_1;
    auto window_1 = CreatePlatformWindow(&delegate_1, bounds_1, &widget_1, nullptr);
    ...
    EXPECT_EQ(window_2.get(), window_manager()->window_mouse_currently_on_for_test());
    ...
    window_2->SetCapture();
    ScopedXI2Event xi_event;
    xi_event.InitGenericButtonEvent(kPointerDeviceId, EventType::kMousePressed, gfx::Point(0, 0), EF_NONE);
    DispatchXEvent(xi_event, widget_1);
    EXPECT_EQ(window_1.get(), window_manager()->window_mouse_currently_on_for_test());
...
TEST_F(X11WindowOzoneTest, SetFullscreen)
{
    ...
    auto window = CreatePlatformWindow(&delegate, bounds, &widget, &x11_extension_delegate);
    EXPECT_CALL(delegate, OnBoundsChanged(testing::Eq(PlatformWindowDelegate::BoundsChange { true })));
