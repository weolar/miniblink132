...
public:
    X11KeyboardHook(std::optional<base::flat_set<DomCode>> dom_codes, BaseKeyboardHook::KeyEventCallback callback, gfx::AcceleratedWidget accelerated_widget);
...
private:
    ...
    void CaptureAllKeys();
    void CaptureSpecificKeys(const std::optional<base::flat_set<DomCode>>& dom_codes);
