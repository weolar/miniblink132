...
public:
    ...
    PlatformWindowState state() const
    {
        return state_;
    }

    void WaitForBoundsChange(const PlatformWindowDelegate::BoundsChange& expected_change)
    {
    ...
    void set_window(X11Window* window)
    {
        window_ = window;
    }
...
private:
    ...
    bool ShouldKeepOnWaiting() override
    {
        ...
        if (x11::Connection::Get()->GetArrayProperty(xwindow(), x11::GetAtom("_NET_WM_STATE"), &hints)) {
...
public:
    ...
    void SetScaleAndBoundsForPrimaryDisplay(float scale, const gfx::Rect& bounds_in_pixels)
    {
...
std::vector<gfx::Rect> GetShapeRects(x11::Window window)
{
    ...
    if (auto shape = x11::Connection::Get()->shape().GetRectangles({ window, x11::Shape::Sk::Bounding }).Sync()) {
...
bool ShapeRectContainsPoint(const std::vector<gfx::Rect>& shape_rects, int x, int y)
{
    gfx::Point point(x, y);
    return base::ranges::any_of(shape_rects, [&point](const auto& rect) { return rect.Contains(point); });
}
...
protected:
    ...
    std::unique_ptr<X11Window> CreateX11Window(PlatformWindowDelegate* delegate, const gfx::Rect& bounds, X11ExtensionDelegate* x11_extension_delegate)
    {
...
TEST_F(X11WindowTest, DISABLED_Shape)
{
    ...
    if (connection->WmSupportsHint(x11::GetAtom("_NET_WM_STATE_MAXIMIZED_VERT"))) {
...
#define MAYBE_WindowManagerTogglesFullscreen DISABLED_WindowManagerTogglesFullscreen
#else
...
TEST_F(X11WindowTest, MAYBE_WindowManagerTogglesFullscreen)
{
    ...
    // accelerator key.
    {
        ui::SendClientMessage(
            x11_window, ui::GetX11RootWindow(), x11::GetAtom("_NET_WM_STATE"), { 0, static_cast<uint32_t>(x11::GetAtom("_NET_WM_STATE_FULLSCREEN")), 0, 1, 0 });
...
// PlatformWindowDelegate.
TEST_F(X11WindowTest, DISABLED_ToggleMinimizePropogateToPlatformWindowDelegate)
{
    TestPlatformWindowDelegate delegate;
    ...
    // Minimize by iconifying.
    {
        ...
        SendClientMessage(x11_window, GetX11RootWindow(), x11::GetAtom("WM_CHANGE_STATE"), { x11::WM_STATE_ICONIC, 0, 0, 0, 0 });
        // Wait till set.
    ...
    {
        SendClientMessage(x11_window, GetX11RootWindow(), x11::GetAtom("WM_CHANGE_STATE"), { x11::WM_STATE_NORMAL, 0, 0, 0, 0 });
        // Wait till unset.
