...
public:
    ...
    ClientMessageEventCollector& operator=(const ClientMessageEventCollector&) = delete;
...
public:
    ...
    // X11MoveLoop:
    bool RunMoveLoop(
        bool can_grab_pointer, scoped_refptr<X11Cursor> old_cursor, scoped_refptr<X11Cursor> new_cursor, base::OnceClosure started_callback) override;
...
class SimpleTestDragDropClient : public XDragDropClient, public XDragDropClient::Delegate, public X11MoveLoopDelegate {
public:
    ...
    DragOperation StartDragAndDrop(std::unique_ptr<OSExchangeData> data, X11Window* source_window, int allowed_operations, mojom::DragEventSource source);
...
private:
    ...
    // X11MoveLoopDelegate:
    void OnMouseMovement(const gfx::Point& screen_point, int flags, base::TimeTicks event_time) override;
...
public:
    ...
    void SetEventCollectorFor(x11::Window window, ClientMessageEventCollector* collector);

    // Builds an XdndStatus message and sends it to
    // XDragDropClient.
    void OnStatus(x11::Window target_window, bool will_accept_drop, x11::Atom accepted_action);

    // Builds an XdndFinished message and sends it to
    // XDragDropClient.
    void OnFinished(x11::Window target_window, bool accepted_drop, x11::Atom performed_action);
...
private:
    // XDragDropClient:
    void SendXClientEvent(x11::Window window, const x11::ClientMessageEvent& event) override;
    ...
    std::map<x11::Window, raw_ptr<ClientMessageEventCollector, CtnExperimental>> collectors_;
};

///////////////////////////////////////////////////////////////////////////////
// ClientMessageEventCollector

ClientMessageEventCollector::ClientMessageEventCollector(x11::Window window, TestDragDropClient* client)
    : window_(window)
    , client_(client)
{
...
std::vector<x11::ClientMessageEvent> ClientMessageEventCollector::PopAllEvents()
{
...
void ClientMessageEventCollector::RecordEvent(const x11::ClientMessageEvent& event)
{
...
bool TestMoveLoop::RunMoveLoop(
    bool can_grab_pointer, scoped_refptr<X11Cursor> old_cursor, scoped_refptr<X11Cursor> new_cursor, base::OnceClosure started_callback)
{
    is_running_ = true;
...
DragOperation SimpleTestDragDropClient::StartDragAndDrop(
    std::unique_ptr<OSExchangeData> data, X11Window* source_window, int allowed_operations, mojom::DragEventSource source)
{
...
std::optional<gfx::AcceleratedWidget> SimpleTestDragDropClient::GetDragWidget()
{
    return std::nullopt;
}
...
void SimpleTestDragDropClient::UpdateCursor(DragOperation negotiated_operation)
{
}
...
x11::Window SimpleTestDragDropClient::FindWindowFor(const gfx::Point& screen_point)
{
    return target_window_;
}

void SimpleTestDragDropClient::OnMouseMovement(const gfx::Point& screen_point, int flags, base::TimeTicks event_time)
{
...
bool TestDragDropClient::MessageHasType(const x11::ClientMessageEvent& event, const char* type)
{
    return event.type == GetAtom(type);
}

void TestDragDropClient::SetEventCollectorFor(x11::Window window, ClientMessageEventCollector* collector)
{
    if (collector)
...
void TestDragDropClient::OnStatus(x11::Window target_window, bool will_accept_drop, x11::Atom accepted_action)
{
...
void TestDragDropClient::OnFinished(x11::Window target_window, bool accepted_drop, x11::Atom performed_action)
{
    x11::ClientMessageEvent event;
...
void TestDragDropClient::SetTopmostXWindowAndMoveMouse(x11::Window window)
{
    ...
    HandleMouseMovement(gfx::Point(kMouseMoveX, kMouseMoveY), EF_NONE, EventTimeForNow());
}

void TestDragDropClient::SendXClientEvent(x11::Window window, const x11::ClientMessageEvent& event)
{
...
public:
    ...
    TestPlatformWindowDelegate& operator=(const TestPlatformWindowDelegate&) = delete;
    ~TestPlatformWindowDelegate() override = default;

    // PlatformWindowDelegate:
    void OnBoundsChanged(const PlatformWindowDelegate::BoundsChange& change) override
    {
    }
    ...
    void OnWindowStateChanged(PlatformWindowState old_state, PlatformWindowState new_state) override
    {
    }
...
class TestOSExchangeDataProvideFactory : public OSExchangeDataProviderFactoryOzone {
public:
    TestOSExchangeDataProvideFactory()
    {
        SetInstance(this);
    }
...
public:
    X11DragDropClientTest()
        : task_env_(std::make_unique<base::test::TaskEnvironment>(base::test::TaskEnvironment::MainThreadType::UI))
    {
    }
    ...
    DragOperation StartDragAndDrop()
    {
        ...
        return client_->StartDragAndDrop(std::move(data), window_.get(), DragDropTypes::DRAG_COPY, mojom::DragEventSource::kMouse);
    }
...
void BasicStep2(TestDragDropClient* client, x11::Window toplevel)
{
    ...
    EXPECT_EQ(client->source_xwindow(), static_cast<x11::Window>(events[0].data.data32[0]));
    EXPECT_EQ(1u, events[0].data.data32[1] & 1);
    std::vector<x11::Atom> targets;
    x11::Connection::Get()->GetArrayProperty(client->source_xwindow(), x11::GetAtom("XdndTypeList"), &targets);
    EXPECT_FALSE(targets.empty());

    EXPECT_TRUE(client->MessageHasType(events[1], "XdndPosition"));
    EXPECT_EQ(client->source_xwindow(), static_cast<x11::Window>(events[0].data.data32[0]));
    const uint32_t kCoords = TestDragDropClient::kMouseMoveX << 16 | TestDragDropClient::kMouseMoveY;
    EXPECT_EQ(kCoords, events[1].data.data32[2]);
    EXPECT_EQ(client->GetAtom("XdndActionCopy"), static_cast<x11::Atom>(events[1].data.data32[4]));
...
TEST_F(X11DragDropClientTest, Basic)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&BasicStep2, client(), toplevel));
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&BasicStep3, client(), toplevel));
...
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&TargetDoesNotRespondStep2, client()));
    DragOperation result = StartDragAndDrop();
...
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&QueuePositionStep2, client()));
    DragOperation result = StartDragAndDrop();
    EXPECT_EQ(DragOperation::kCopy, result);
}
...
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&TargetChangesStep2, client()));
    DragOperation result = StartDragAndDrop();
    EXPECT_EQ(DragOperation::kCopy, result);
}
...
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&RejectAfterMouseReleaseStep2, client()));
    DragOperation result = StartDragAndDrop();
    EXPECT_EQ(DragOperation::kNone, result);

    // Repeat the test but reject the drop in the XdndFinished message instead.
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&RejectAfterMouseReleaseStep3, client()));
    result = StartDragAndDrop();
    EXPECT_EQ(DragOperation::kNone, result);
}
