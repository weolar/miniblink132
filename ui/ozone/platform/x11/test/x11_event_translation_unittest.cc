...
TEST(XEventTranslationTest, KeyEventXEventPropertiesSet)
{
    ...
    uint32_t state = XkbBuildCoreState(static_cast<uint32_t>(xev->As<x11::KeyEvent>()->state), 2u);
    ...
    EXPECT_EQ(static_cast<uint8_t>(connection->KeysymToKeycode(XK_a)), hw_keycode_it->second[0]);
...
TEST(XEventTranslationTest, ChangedMouseButtonFlags)
{
    ...
    event.InitButtonEvent(ui::EventType::kMousePressed, gfx::Point(500, 500), ui::EF_LEFT_MOUSE_BUTTON);
...
TEST(XEventTranslationTest, KeyModifiersCounterpartRepeat)
{
    ...
    shift_r_pressed.InitKeyEvent(EventType::kKeyPressed, VKEY_RSHIFT, EF_SHIFT_DOWN);
    ...
    shift_r_released.InitKeyEvent(EventType::kKeyReleased, VKEY_RSHIFT, EF_SHIFT_DOWN);
...
TEST(XEventTranslationTest, ScrollEventType)
{
    ...
    const std::vector<ScrollEventTestData> test_data = { // Ordinary horizontal scrolling remains EventType::kScroll.
    ...
    for (const auto& data : test_data) {
        ...
        xev.InitScrollEvent(device_id, data.x_offset_, data.y_offset_, data.x_offset_ordinal_, data.y_offset_ordinal_, 2);
