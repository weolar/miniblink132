...
const display::Display& GetDisplayForRect(const DisplayList& displays, const gfx::Rect& rect, bool rect_in_px)
{
    ...
    for (const auto& display : displays) {
        auto bounds = rect_in_px ? DisplayBoundsInPixels(display) : display.bounds();
        min_dist_display = std::min(min_dist_display, std::make_pair(RectDistance(bounds, rect), &display));
    }
...
{
    const auto& display = GetDisplayForRect(displays, gfx::Rect(point_px, gfx::Size(1, 1)), true);
    gfx::Vector2d delta_px = point_px - display.native_origin();
    gfx::Vector2dF delta_dip = gfx::ScaleVector2d(delta_px, 1.0 / display.device_scale_factor());
    return gfx::PointF(display.bounds().origin()) + delta_dip;
}

gfx::PointF PointDipToPx(const DisplayList& displays, gfx::Point point_dip)
{
    const auto& display = GetDisplayForRect(displays, gfx::Rect(point_dip, gfx::Size(1, 1)), false);
    gfx::Rect bounds_dip = display.bounds();
    gfx::Vector2d delta_dip = point_dip - bounds_dip.origin();
    gfx::Vector2dF delta_px = gfx::ScaleVector2d(delta_dip, display.device_scale_factor());
...
gfx::Point GetCursorScreenPointPx(x11::Connection& connection)
{
    if (ui::X11EventSource::HasInstance()) {
        auto point_in_pixels = ui::X11EventSource::GetInstance()->last_cursor_location();
    ...
    auto point_in_pixels = response ? gfx::Point(response->root_x, response->root_y) : gfx::Point();
    if (ui::X11EventSource::HasInstance()) {
        ui::X11EventSource::GetInstance()->set_last_cursor_location(point_in_pixels);
    }
...
display::Display X11ScreenOzone::GetDisplayForAcceleratedWidget(gfx::AcceleratedWidget widget) const
{
...
gfx::Point X11ScreenOzone::GetCursorScreenPoint() const
{
    ...
    return gfx::ToFlooredPoint(PointPxToDip(GetAllDisplays(), GetCursorScreenPointPx(*connection_)));
}

bool X11ScreenOzone::IsAcceleratedWidgetUnderCursor(gfx::AcceleratedWidget widget) const
{
...
gfx::AcceleratedWidget X11ScreenOzone::GetAcceleratedWidgetAtScreenPoint(const gfx::Point& point) const
{
    gfx::Point point_in_pixels = gfx::ToFlooredPoint(PointDipToPx(GetAllDisplays(), point));
    return static_cast<gfx::AcceleratedWidget>(x11::GetWindowAtPoint(point_in_pixels));
}

gfx::AcceleratedWidget X11ScreenOzone::GetLocalProcessWidgetAtPoint(const gfx::Point& point, const std::set<gfx::AcceleratedWidget>& ignore) const
{
    ...
    if (ignore.empty()) {
        ...
        widget = static_cast<gfx::AcceleratedWidget>(x11::GetWindowAtPoint(point_in_pixels, &ignore_windows));
    }
    return window_manager_->GetWindow(widget) ? widget : gfx::AcceleratedWidget {};
}

display::Display X11ScreenOzone::GetDisplayNearestPoint(const gfx::Point& point) const
{
...
display::Display X11ScreenOzone::GetDisplayMatching(const gfx::Rect& match_rect) const
{
    const display::Display* matching_display = display::FindDisplayWithBiggestIntersection(x11_display_manager_->displays(), match_rect);
...
std::unique_ptr<X11ScreenOzone::X11ScreenSaverSuspender> X11ScreenOzone::X11ScreenSaverSuspender::Create()
{
...
std::unique_ptr<PlatformScreen::PlatformScreenSaverSuspender> X11ScreenOzone::SuspendScreenSaver()
{
...
base::Value::List X11ScreenOzone::GetGpuExtraInfo(const gfx::GpuExtraInfo& gpu_extra_info)
{
...
{
    float scale_factor = x11_display_manager_->GetPrimaryDisplay().device_scale_factor();
