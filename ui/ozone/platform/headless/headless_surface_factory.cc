...
base::FilePath GetPathForWidget(const base::FilePath& base_path, gfx::AcceleratedWidget widget)
{
...
void WriteDataToFile(const base::FilePath& location, const SkBitmap& bitmap)
{
    ...
    std::optional<std::vector<uint8_t>> png_data = gfx::PNGCodec::FastEncodeBGRASkBitmap(bitmap,
        /*discard_transparency=*/true);
    if (!png_data || !base::WriteFile(location, png_data.value())) {
        static bool logged_once = false;
        LOG_IF(ERROR, !logged_once) << "Failed to write frame to file. "
...
public:
    explicit FileSurface(const base::FilePath& location)
        : base_path_(location)
    {
    }
    ...
    void ResizeCanvas(const gfx::Size& viewport_size, float scale) override
    {
        ...
        surface_ = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(viewport_size.width(), viewport_size.height()), &props);
    }
    ...
    void PresentCanvas(const gfx::Rect& damage) override
    {
        ...
        if (surface_->getCanvas()->readPixels(bitmap, 0, 0)) {
            base::ThreadPool::PostTask(
                FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, base::BindOnce(&WriteDataToFile, base_path_, bitmap));
        }
    }
...
private:
    ...
    bool HandlePixels(uint8_t* pixels) override
    {
        ...
        base::ThreadPool::PostTask(
            FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN }, base::BindOnce(&WriteDataToFile, location_, bitmap));
        return true;
    }
...
public:
    explicit TestPixmap(gfx::BufferFormat format)
        : format_(format)
    {
    }
    ...
    bool ScheduleOverlayPlane(gfx::AcceleratedWidget widget, const gfx::OverlayPlaneData& overlay_plane_data, std::vector<gfx::GpuFence> acquire_fences,
...
public:
    GLOzoneEGLHeadless(const base::FilePath& base_path)
    ...
    {
        return gl::InitializeGLSurface(base::MakeRefCounted<FileGLSurface>(display->GetAs<gl::GLDisplayEGL>(), GetPathForWidget(base_path_, window)));
    }
...
HeadlessSurfaceFactory::HeadlessSurfaceFactory(base::FilePath base_path)
    : base_path_(base_path)
    , swiftshader_implementation_(std::make_unique<GLOzoneEGLHeadless>(base_path))
{
    CheckBasePath();
}
...
std::vector<gl::GLImplementationParts> HeadlessSurfaceFactory::GetAllowedGLImplementations()
{
...
GLOzone* HeadlessSurfaceFactory::GetGLOzone(const gl::GLImplementationParts& implementation)
{
...
std::unique_ptr<SurfaceOzoneCanvas> HeadlessSurfaceFactory::CreateCanvasForWidget(gfx::AcceleratedWidget widget)
{
    return std::make_unique<FileSurface>(GetPathForWidget(base_path_, widget));
}

scoped_refptr<gfx::NativePixmap> HeadlessSurfaceFactory::CreateNativePixmap(gfx::AcceleratedWidget widget, gpu::VulkanDeviceQueue* device_queue, gfx::Size size,
...
void HeadlessSurfaceFactory::CheckBasePath() const
{
    ...
    if (!DirectoryExists(base_path_) && !base::CreateDirectory(base_path_) && base_path_ != base::FilePath(kDevNull))
...
std::unique_ptr<gpu::VulkanImplementation> HeadlessSurfaceFactory::CreateVulkanImplementation(bool use_swiftshader, bool allow_protected_memory)
{
