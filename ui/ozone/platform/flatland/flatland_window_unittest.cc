...
Matcher<FakeGraph> IsWindowGraph(const fuchsia::ui::composition::ParentViewportWatcherPtr& parent_viewport_watcher,
    const fuchsia::ui::views::ViewportCreationToken& viewport_creation_token, Matcher<std::vector<FakeTransformPtr>> children_transform_matcher)
{
    std::optional<zx_koid_t> view_token_koid = base::GetRelatedKoid(viewport_creation_token.value);
    EXPECT_TRUE(view_token_koid.has_value());
    std::optional<zx_koid_t> watcher_koid = base::GetRelatedKoid(parent_viewport_watcher.channel());
    EXPECT_TRUE(watcher_koid.has_value());

    return AllOf(Field("root_transform", &FakeGraph::root_transform,
                     Pointee(AllOf(Field("translation", &FakeTransform::translation, ::base::test::FidlEq(FakeTransform::kDefaultTranslation)),
                         ...
                         Field("children", &FakeTransform::children, children_transform_matcher)))),
...
Matcher<fuchsia::ui::composition::ViewportProperties> IsViewportProperties(const fuchsia::math::SizeU& logical_size)
{
    return AllOf(Property("has_logical_size", &fuchsia::ui::composition::ViewportProperties::has_logical_size, true),
        Property("logical_size", &fuchsia::ui::composition::ViewportProperties::logical_size, ::base::test::FidlEq(logical_size)));
}

Matcher<FakeTransformPtr> IsViewport(const fuchsia::ui::views::ViewCreationToken& view_token, const fuchsia::math::SizeU& viewport_logical_size)
{
    auto viewport_koid = base::GetRelatedKoid(view_token.value);

    return Pointee(AllOf(Field("translation", &FakeTransform::translation, ::base::test::FidlEq(FakeTransform::kDefaultTranslation)),
        Field("scale", &FakeTransform::scale, ::base::test::FidlEq(FakeTransform::kDefaultScale)),
        ...
        Field("content", &FakeTransform::content,
            Pointee(
                VariantWith<FakeViewport>(AllOf(Field("viewport_properties", &FakeViewport::viewport_properties, IsViewportProperties(viewport_logical_size)),
                    Field("viewport_token", &FakeViewport::viewport_token, viewport_koid)))))));
}
...
Matcher<FakeTransformPtr> IsHitShield()
{
    return Pointee(AllOf(
        ...
        Field("clip_bounds", &FakeTransform::clip_bounds, testing::Eq(std::nullopt)),
        // Hit region must be "infinite".
        Field("hit_regions", &FakeTransform::hit_regions, testing::Contains(::base::test::FidlEq(scenic::kInfiniteHitRegion)))));
}
...
protected:
    ...
    void CreateWindow()
    {
        ...
        auto status = zx::channel::create(0, &viewport_token.value, &view_token.value);
        CHECK_EQ(ZX_OK, status);
        viewport_token_ = std::move(viewport_token);

        EXPECT_CALL(window_delegate_, OnAcceleratedWidgetAvailable(_)).WillOnce(SaveArg<0>(&window_widget_));

        PlatformWindowInitProperties properties;
        properties.view_ref_pair = ViewRefPair::New();
        properties.view_creation_token = std::move(view_token);
        flatland_window_ = std::make_unique<FlatlandWindow>(&window_manager_, &window_delegate_, std::move(properties));
    }

    void SetLayoutInfo(uint32_t width, uint32_t height, float dpr, fuchsia::math::Inset inset = { 0, 0, 0, 0 })
    {
    ...
    void SetViewRefFocusedHandle(fuchsia::ui::views::ViewRefFocusedHandle view_ref_focused_handle)
    {
        flatland_window_->view_ref_focused_.Bind(std::move(view_ref_focused_handle));
        flatland_window_->view_ref_focused_->Watch(fit::bind_member(flatland_window_.get(), &FlatlandWindow::OnViewRefFocusedWatchResult));
    }

    void SetTouchSource(fidl::ClientEnd<fuchsia_ui_pointer::TouchSource> touch_source)
    {
        auto mouse_endpoints = fidl::CreateEndpoints<fuchsia_ui_pointer::MouseSource>();
        EXPECT_TRUE(mouse_endpoints.is_ok()) << mouse_endpoints.status_string();
        flatland_window_->pointer_handler_ = std::make_unique<PointerEventsHandler>(std::move(touch_source), std::move(mouse_endpoints->client));
        flatland_window_->pointer_handler_->StartWatching(base::BindRepeating(&FlatlandWindow::DispatchEvent,
...
TEST_F(FlatlandWindowTest, PresentsOnShow)
{
    ...
    EXPECT_THAT(fake_flatland_.graph(), IsWindowGraph(parent_viewport_watcher(), viewport_token_, IsEmpty()));
}
...
TEST_F(FlatlandWindowTest, ProcessesFocusedSignal)
{
    ...
    fidl::Binding<fuchsia::ui::views::ViewRefFocused> fake_view_ref_focused_binding(&fake_view_ref_focused);
    ...
    EXPECT_CALL(window_delegate_, OnActivationChanged(_)).WillRepeatedly(SaveArg<0>(&focus_delegated));
...
TEST_F(FlatlandWindowTest, AppliesDevicePixelRatio)
{
    ...
    fidl::Binding<fuchsia::ui::pointer::TouchSource> fake_touch_source_binding(&fake_touch_source);
    ...
    EXPECT_CALL(window_delegate_, DispatchEvent(_)).WillOnce([&event_received, kLocationX, kLocationY](ui::Event* event) {
    ...
    events.push_back(
        TouchEventBuilder().SetPosition({ kLocationX, kLocationY }).SetTouchInteractionStatus(fuchsia_ui_pointer::TouchInteractionStatus::kGranted).Build());
    ...
    EXPECT_CALL(window_delegate_, DispatchEvent(_)).WillOnce([&event_received, kLocationX, kLocationY, kDPR](ui::Event* event) {
...
TEST_F(FlatlandWindowTest, WaitsForNonZeroSizeToAttachSurfaceContent)
{
    ...
    SetWindowStatus(fuchsia::ui::composition::ParentViewportStatus::CONNECTED_TO_DISPLAY);
    ...
    fake_flatland_.FireOnNextFrameBeginEvent(std::move(on_next_frame_begin_values));

    // Spin the loop to process Present().
    task_environment_.RunUntilIdle();
    EXPECT_EQ(2u, presents_called);
    EXPECT_THAT(fake_flatland_.graph(), IsWindowGraph(parent_viewport_watcher(), viewport_token_, Contains(IsViewport(view_token, expected_size))));
}
...
TEST_F(FlatlandWindowTest, ResetSurfaceOnDisconnect)
{
    ...
    EXPECT_THAT(fake_flatland_.graph(), IsWindowGraph(parent_viewport_watcher(), viewport_token_, _));
    ...
    SetWindowStatus(fuchsia::ui::composition::ParentViewportStatus::DISCONNECTED_FROM_DISPLAY);
...
TEST_F(FlatlandWindowTest, SurfaceHasHitTestHitShield)
{
    ...
    EXPECT_THAT(fake_flatland_.graph(),
        Field("root_transform", &FakeGraph::root_transform,
            Pointee(Field("children", &FakeTransform::children, ElementsAre(IsViewport(view_token, expected_size), IsHitShield())))));
}

class ParameterizedViewInsetTest : public FlatlandWindowTest, public testing::WithParamInterface<float> { };

INSTANTIATE_TEST_SUITE_P(ViewInsetTest, ParameterizedViewInsetTest, testing::Values(1.f, 2.f, 3.f));
...
TEST_P(ParameterizedViewInsetTest, ViewInsetsTest)
{
    ...
    EXPECT_CALL(window_delegate_, OnBoundsChanged(_)).WillOnce(SaveArg<0>(&bounds));
