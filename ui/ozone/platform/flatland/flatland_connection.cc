...
FlatlandConnection::FlatlandConnection(std::string_view debug_name, OnErrorCallback error_callback)
{
    zx_status_t status = base::ComponentContextForProcess()->svc()->Connect<fuchsia::ui::composition::Flatland>(flatland_.NewRequest());
    if (status != ZX_OK) {
    ...
    flatland_.events().OnError
        = [callback = std::move(error_callback)](fuchsia::ui::composition::FlatlandError error) mutable { std::move(callback).Run(std::move(error)); };
    flatland_.events().OnFramePresented = fit::bind_member(this, &FlatlandConnection::OnFramePresented);
    flatland_.events().OnNextFrameBegin = fit::bind_member(this, &FlatlandConnection::OnNextFrameBegin);
}
...
void FlatlandConnection::Present()
{
    ...
    Present(std::move(present_args), base::BindOnce([](base::TimeTicks, base::TimeDelta) {}));
}

void FlatlandConnection::Present(fuchsia::ui::composition::PresentArgs present_args, OnFramePresentedCallback callback)
{
    if (present_credits_ == 0) {
        pending_presents_.emplace(std::move(present_args), std::move(callback));
        DCHECK_LE(pending_presents_.size(), 3u) << "Renderer is queueing up more frames than expected.";
        return;
    }
    --present_credits_;
...
void FlatlandConnection::OnNextFrameBegin(fuchsia::ui::composition::OnNextFrameBeginValues values)
{
    ...
    // presentation times.
    if (values.has_future_presentation_infos() && values.future_presentation_infos().size() > 1) {
        presentation_interval_ = base::TimeTicks::FromZxTime(values.future_presentation_infos()[1].presentation_time())
            - base::TimeTicks::FromZxTime(values.future_presentation_infos()[0].presentation_time());
    }
...
void FlatlandConnection::OnFramePresented(fuchsia::scenic::scheduling::FramePresentedInfo info)
{
    for (size_t i = 0; i < info.presentation_infos.size(); ++i) {
        std::move(presented_callbacks_.front()).Run(base::TimeTicks::FromZxTime(info.actual_presentation_time), presentation_interval_);
        presented_callbacks_.pop();
    }
}

FlatlandConnection::PendingPresent::PendingPresent(fuchsia::ui::composition::PresentArgs present_args, OnFramePresentedCallback callback)
    : present_args(std::move(present_args))
...
FlatlandConnection::PendingPresent& FlatlandConnection::PendingPresent::operator=(PendingPresent&&) = default;
