...
public:
    ...
    FlatlandPlatformEventSource& operator=(const FlatlandPlatformEventSource&) = delete;
};

// OzonePlatform for Flatland.
class OzonePlatformFlatland : public OzonePlatform, public base::CurrentThread::DestructionObserver {
public:
    ...
    std::unique_ptr<PlatformWindow> CreatePlatformWindow(PlatformWindowDelegate* delegate, PlatformWindowInitProperties properties) override
    {
        ...
        if (!properties.view_creation_token.value) {
            ...
            properties.view_controller = ::ui::fuchsia::GetFlatlandViewPresenter().Run(std::move(parent_token));
        }

        CHECK(properties.view_creation_token.value.is_valid());
        return std::make_unique<FlatlandWindow>(window_manager_.get(), delegate, std::move(properties));
    }
    ...
    {
        return std::make_unique<InputMethodFuchsia>(window_manager_->GetWindow(widget)->virtual_keyboard_enabled(), ime_key_event_dispatcher,
            fidl::HLCPPToNatural(window_manager_->GetWindow(widget)->CloneViewRef()));
    }
    ...
    bool InitializeUI(const InitParams& params) override
    {
        ...
        flatland_gpu_host_ = std::make_unique<FlatlandGpuHost>(window_manager_.get());
    ...
    void InitializeGPU(const InitParams& params) override
    {
        ...
        if (!params.single_process) {
            ...
            flatland_gpu_service_ = std::make_unique<FlatlandGpuService>(flatland_gpu_host_remote.InitWithNewPipeAndPassReceiver());
    ...
    {
        binders->Add<mojom::ScenicGpuService>(flatland_gpu_service_->GetBinderCallback(), base::SingleThreadTaskRunner::GetCurrentDefault());
    }

    bool IsNativePixmapConfigSupported(gfx::BufferFormat format, gfx::BufferUsage usage) const override
    {
        return FlatlandSysmemBufferCollection::IsNativePixmapConfigSupported(format, usage);
    }
...
private:
    ...
    void BindInMainProcessIfNecessary()
    {
        ...
        flatland_gpu_host_->Initialize(gpu_host_remote.InitWithNewPipeAndPassReceiver());
