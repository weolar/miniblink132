...
public:
    ...
    bool GetVSyncParametersIfAvailable(base::TimeTicks* timebase, base::TimeDelta* interval) override
    {
        DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void FlatlandSurfaceCanvas::Frame::InitializeBuffer(fuchsia::sysmem2::VmoBuffer vmo, gfx::Size size, size_t stride)
{
    size_t buffer_size = stride * size.height();
    base::WritableSharedMemoryRegion memory_region = base::WritableSharedMemoryRegion::Deserialize(base::subtle::PlatformSharedMemoryRegion::Take(
        std::move(*vmo.mutable_vmo()), base::subtle::PlatformSharedMemoryRegion::Mode::kWritable, buffer_size, base::UnguessableToken::Create()));
    ...
    surface = SkSurfaces::WrapPixels(SkImageInfo::MakeN32Premul(size.width(), size.height()), memory_mapping.memory(), stride, &props);
...
void FlatlandSurfaceCanvas::Frame::CopyDirtyRegionFrom(const Frame& frame)
{
    ...
    for (SkRegion::Iterator i(dirty_region); !i.done(); i.next()) {
        uint8_t* dst_ptr = static_cast<uint8_t*>(memory_mapping.memory()) + i.rect().x() * SkColorTypeBytesPerPixel(kN32_SkColorType) + i.rect().y() * stride;
        frame.surface->readPixels(SkImageInfo::MakeN32Premul(i.rect().width(), i.rect().height()), dst_ptr, stride, i.rect().x(), i.rect().y());
    }
    dirty_region.setEmpty();
}

FlatlandSurfaceCanvas::FlatlandSurfaceCanvas(fuchsia::sysmem2::Allocator_Sync* sysmem_allocator, fuchsia::ui::composition::Allocator* flatland_allocator)
    : sysmem_allocator_(sysmem_allocator)
    , flatland_allocator_(flatland_allocator)
    , flatland_("Chromium FlatlandSurface", base::BindOnce(&FlatlandSurfaceCanvas::OnFlatlandError, base::Unretained(this)))
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void FlatlandSurfaceCanvas::ResizeCanvas(const gfx::Size& viewport_size, float scale)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    zx_status_t status = zx::eventpair::create(0, &export_token.value, &import_token_.value);
    ZX_DCHECK(status == ZX_OK, status);

    fuchsia::ui::composition::RegisterBufferCollectionArgs args;
    args.set_export_token(std::move(export_token));
    args.set_buffer_collection_token(fuchsia::sysmem::BufferCollectionTokenHandle(collection_token_for_flatland.Unbind().TakeChannel()));
    args.set_usage(fuchsia::ui::composition::RegisterBufferCollectionUsage::DEFAULT);

    flatland_allocator_->RegisterBufferCollection(std::move(args), [](fuchsia::ui::composition::Allocator_RegisterBufferCollection_Result result) {
        if (result.is_err()) {
    ...
    image_constraints.set_min_size(fuchsia::math::SizeU { static_cast<uint32_t>(viewport_size_.width()), static_cast<uint32_t>(viewport_size_.height()) });
...
void FlatlandSurfaceCanvas::FinalizeBufferAllocation()
{
    ...
    zx_status_t status = buffer_collection_->WaitForAllBuffersAllocated(&wait_result);
    ZX_LOG_IF(FATAL, status != ZX_OK, status) << "Sysmem connection failed (status).";
    if (!wait_result.is_response()) {
        if (wait_result.is_framework_err()) {
            LOG(ERROR) << "WaitForBuffersAllocated (framework_err): " << fidl::ToUnderlying(wait_result.framework_err());
        } else {
            LOG(ERROR) << "WaitForBuffersAllocated (err): " << static_cast<uint32_t>(wait_result.err());
        }
        return;
    }
    auto buffer_info = std::move(*wait_result.response().mutable_buffer_collection_info());
    ...
    for (size_t i = 0; i < kNumBuffers; ++i) {
        ...
        image_properties.set_size(fuchsia::math::SizeU { static_cast<uint32_t>(viewport_size_.width()), static_cast<uint32_t>(viewport_size_.height()) });
        ...
        flatland_.flatland()->CreateImage(frames_[i].image_id, std::move(token), i, std::move(image_properties));

        // TODO(crbug.com/42050483): We should set SRC blend mode when Chrome has a
        // reliable signal for opaque background.
        flatland_.flatland()->SetImageBlendingFunction(frames_[i].image_id, fuchsia::ui::composition::BlendMode::SRC_OVER);
    }
    import_token_.value.reset();

    const fuchsia::sysmem2::ImageFormatConstraints& format = buffer_info.settings().image_format_constraints();
    size_t stride = RoundUp(std::max(format.min_bytes_per_row(), viewport_size_.width() * 4U), format.bytes_per_row_divisor());

    for (size_t i = 0; i < kNumBuffers; ++i) {
        frames_[i].InitializeBuffer(std::move(buffer_info.mutable_buffers()->at(i)), viewport_size_, stride);
    }
}
...
SkCanvas* FlatlandSurfaceCanvas::GetCanvas()
{
    ...
    if (frames_[current_frame_].release_fence) {
        auto status = frames_[current_frame_].release_fence.wait_one(
            ZX_EVENT_SIGNALED, zx::deadline_after(zx::duration(kFrameReleaseTimeout.InNanoseconds())), nullptr);
...
void FlatlandSurfaceCanvas::PresentCanvas(const gfx::Rect& damage)
{
    ...
    // is up to date.
    int prev_frame = current_frame_ == 0 ? (kNumBuffers - 1) : (current_frame_ - 1);
    ...
    flatland_.flatland()->SetContent(root_transform_id_, frames_[current_frame_].image_id);
    ...
    auto status = frames_[current_frame_].release_fence.duplicate(ZX_RIGHT_SAME_RIGHTS, &release_fence_dup);
    ZX_CHECK(status == ZX_OK, status);

    fuchsia::ui::composition::PresentArgs present_args;
    present_args.set_requested_presentation_time(0);
    present_args.mutable_release_fences()->push_back(std::move(release_fence_dup));
    present_args.set_unsquashable(false);
    flatland_.Present(std::move(present_args), base::BindOnce(&FlatlandSurfaceCanvas::OnFramePresented, base::Unretained(this)));

    // Move to the next buffer.
    current_frame_ = (current_frame_ + 1) % kNumBuffers;
}

std::unique_ptr<gfx::VSyncProvider> FlatlandSurfaceCanvas::CreateVSyncProvider()
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void FlatlandSurfaceCanvas::OnFramePresented(base::TimeTicks actual_presentation_time, base::TimeDelta future_presentation_interval)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);

    if (vsync_provider_) {
        vsync_provider_->SetState(actual_presentation_time, future_presentation_interval);
    }
}

void FlatlandSurfaceCanvas::OnFlatlandError(fuchsia::ui::composition::FlatlandError error)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
