...
FlatlandScreen::FlatlandScreen()
    : displays_({ display::Display(display::kDefaultDisplayId, gfx::Rect(kDefaultDisplaySize)) })
{
}
...
display::Display FlatlandScreen::GetDisplayForAcceleratedWidget(gfx::AcceleratedWidget widget) const
{
    return displays_[0];
}
...
gfx::AcceleratedWidget FlatlandScreen::GetAcceleratedWidgetAtScreenPoint(const gfx::Point& point) const
{
    NOTIMPLEMENTED_LOG_ONCE();
    return gfx::kNullAcceleratedWidget;
}

display::Display FlatlandScreen::GetDisplayNearestPoint(const gfx::Point& point) const
{
    return displays_[0];
}

display::Display FlatlandScreen::GetDisplayMatching(const gfx::Rect& match_rect) const
{
    return displays_[0];
}
