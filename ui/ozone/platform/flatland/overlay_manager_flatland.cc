    ...
    void CheckOverlaySupport(std::vector<OverlaySurfaceCandidate>* candidates) override
    {
        for (auto& candidate : *candidates) {
            if (!candidate.native_pixmap)
                continue;
            FlatlandSysmemNativePixmap* sysmem_native_pixmap = static_cast<FlatlandSysmemNativePixmap*>(candidate.native_pixmap.get());
...
std::unique_ptr<OverlayCandidatesOzone> OverlayManagerFlatland::CreateOverlayCandidates(gfx::AcceleratedWidget widget)
{
