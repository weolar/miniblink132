...
public:
    using OnFramePresentedCallback = base::OnceCallback<void(base::TimeTicks actual_presentation_time, base::TimeDelta future_presentation_interval)>;
    using OnErrorCallback = base::OnceCallback<void(fuchsia::ui::composition::FlatlandError error)>;

    explicit FlatlandConnection(std::string_view debug_name, OnErrorCallback callback);
    ~FlatlandConnection();
...
private:
    ...
    base::TimeDelta presentation_interval_ = base::Hertz(kInitialFramesPerSecondEstimate);

    struct PendingPresent {
        PendingPresent(fuchsia::ui::composition::PresentArgs present_args, OnFramePresentedCallback callback);
        ~PendingPresent();
