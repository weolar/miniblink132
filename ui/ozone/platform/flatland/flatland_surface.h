...
public:
    FlatlandSurface(FlatlandSurfaceFactory* flatland_surface_factory, gfx::AcceleratedWidget window);
    ...
    void Present(scoped_refptr<gfx::NativePixmap> primary_plane_pixmap, std::vector<ui::OverlayPlane> overlays, std::vector<gfx::GpuFenceHandle> acquire_fences,
        std::vector<gfx::GpuFenceHandle> release_fences, SwapCompletionCallback completion_callback, BufferPresentedCallback presentation_callback) override;
...
private:
    template <typename T> friend class FlatlandSurfaceTestBase;

    struct PresentedFrame {
        PresentedFrame(fuchsia::ui::composition::ContentId image_id, scoped_refptr<gfx::NativePixmap> primary_plane, SwapCompletionCallback completion_callback,
    ...
    void OnPresentComplete(base::TimeTicks actual_presentation_time, base::TimeDelta presentation_interval);

    FlatlandIds CreateOrGetFlatlandIds(gfx::NativePixmap* pixmap, bool is_primary_plane);
    ...
    std::map<int /* z_order */, fuchsia::ui::composition::TransformId> child_transforms_;
