...
TEST_F(FlatlandConnectionTest, Initialization)
{
    ...
    FlatlandConnection flatland_connection(debug_name, base::BindLambdaForTesting([](fuchsia::ui::composition::FlatlandError) { FAIL(); }));
...
TEST_F(FlatlandConnectionTest, BasicPresent)
{
    ...
    fake_flatland_.SetPresentHandler([&presents_called](fuchsia::ui::composition::PresentArgs present_args) { presents_called++; });

    // Create the FlatlandConnection but don't pump the loop.  No FIDL calls are
    // completed yet.
    FlatlandConnection flatland_connection(GetCurrentTestName(), base::BindLambdaForTesting([](fuchsia::ui::composition::FlatlandError) { FAIL(); }));
...
TEST_F(FlatlandConnectionTest, RespectsPresentCredits)
{
    ...
    flatland_connection.Present(
        std::move(present_args), base::BindLambdaForTesting([&on_presented_called](base::TimeTicks, base::TimeDelta) { on_presented_called = true; }));
    task_environment_.RunUntilIdle();
    EXPECT_EQ(presents_called, 1u);
...
TEST_F(FlatlandConnectionTest, CalculatesPresentationInterval)
{
    ...
    flatland_connection.Present(
        std::move(present_args), base::BindLambdaForTesting([&on_presented_called, &presentation_interval](base::TimeTicks, base::TimeDelta interval) {
...
TEST_F(FlatlandConnectionTest, ReleaseFences)
{
    ...
    fake_flatland_.SetPresentHandler([&presents_called, &release_fence_handle](fuchsia::ui::composition::PresentArgs present_args) {
        presents_called++;
        release_fence_handle = present_args.release_fences().empty() ? ZX_HANDLE_INVALID : present_args.release_fences().front().get();
    });
    ...
    present_args.set_unsquashable(false);
    flatland_connection.Present(std::move(present_args), FlatlandConnection::OnFramePresentedCallback());
    task_environment_.RunUntilIdle();
    EXPECT_EQ(presents_called, 1u);
