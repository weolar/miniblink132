...
using BinderCallback = base::RepeatingCallback<void(const std::string&, mojo::ScopedMessagePipeHandle)>;

template <typename Interface> void BindInterface(mojo::PendingReceiver<Interface> receiver, const BinderCallback& binder_callback)
{
...
void FlatlandGpuHost::Initialize(mojo::PendingReceiver<mojom::ScenicGpuHost> host_receiver)
{
...
void FlatlandGpuHost::AttachSurfaceToWindow(int32_t window_id, mojo::PlatformHandle surface_view_holder_token_mojo)
{
    DCHECK_CALLED_ON_VALID_THREAD(ui_thread_checker_);
    FlatlandWindow* flatland_window = flatland_window_manager_->GetWindow(window_id);
    if (!flatland_window)
        return;
    fuchsia::ui::views::ViewportCreationToken surface_view_holder_token;
    surface_view_holder_token.value = zx::channel(surface_view_holder_token_mojo.TakeHandle());
...
void FlatlandGpuHost::OnGpuServiceLaunched(int host_id, GpuHostBindInterfaceCallback binder, GpuHostTerminateCallback terminate_callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(ui_thread_checker_);
