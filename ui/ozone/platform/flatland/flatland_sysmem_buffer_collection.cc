...
VkImageFormatConstraintsInfoFUCHSIA GetDefaultImageFormatConstraintsInfo(const VkImageCreateInfo& create_info)
{
    ...
    static const VkSysmemColorSpaceFUCHSIA kSrgbColorSpace
        = { VK_STRUCTURE_TYPE_SYSMEM_COLOR_SPACE_FUCHSIA, nullptr, static_cast<uint32_t>(fuchsia::images2::ColorSpace::SRGB) };

    static const VkSysmemColorSpaceFUCHSIA kYuvDefaultColorSpaces[] = {
        { VK_STRUCTURE_TYPE_SYSMEM_COLOR_SPACE_FUCHSIA, nullptr, static_cast<uint32_t>(fuchsia::images2::ColorSpace::REC709) },
        { VK_STRUCTURE_TYPE_SYSMEM_COLOR_SPACE_FUCHSIA, nullptr, static_cast<uint32_t>(fuchsia::images2::ColorSpace::REC601_NTSC) },
        { VK_STRUCTURE_TYPE_SYSMEM_COLOR_SPACE_FUCHSIA, nullptr, static_cast<uint32_t>(fuchsia::images2::ColorSpace::REC601_NTSC_FULL_RANGE) },
        { VK_STRUCTURE_TYPE_SYSMEM_COLOR_SPACE_FUCHSIA, nullptr, static_cast<uint32_t>(fuchsia::images2::ColorSpace::REC601_PAL) },
        { VK_STRUCTURE_TYPE_SYSMEM_COLOR_SPACE_FUCHSIA, nullptr, static_cast<uint32_t>(fuchsia::images2::ColorSpace::REC601_PAL_FULL_RANGE) },
    };
    ...
    VkImageFormatConstraintsInfoFUCHSIA format_info = {
        ...
        .requiredFormatFeatures = GetFormatFeatureFlagsFromUsage(create_info.usage),
        .sysmemPixelFormat = 0u,
        .colorSpaceCount = static_cast<uint32_t>(is_yuv ? std::size(kYuvDefaultColorSpaces) : 1u),
...
struct ImageConstraintsInfo {
    ...
    ImageConstraintsInfo(const VkImageConstraintsInfoFUCHSIA& image_constraints_in, const VkImageFormatConstraintsInfoFUCHSIA& format_constraints_in)
...
std::unique_ptr<ImageConstraintsInfo> InitializeImageConstraintsInfo(const VkImageCreateInfo& vk_image_info, bool allow_protected_memory)
{
    VkImageFormatConstraintsInfoFUCHSIA format_constraints = GetDefaultImageFormatConstraintsInfo(vk_image_info);
    VkImageConstraintsInfoFUCHSIA image_constraints = {
    ...
    return std::make_unique<ImageConstraintsInfo>(image_constraints, format_constraints);
}

} // namespace

// static
bool FlatlandSysmemBufferCollection::IsNativePixmapConfigSupported(gfx::BufferFormat format, gfx::BufferUsage usage)
{
    switch (format) {
    case gfx::BufferFormat::YUV_420_BIPLANAR:
    case gfx::BufferFormat::R_8:
...
bool FlatlandSysmemBufferCollection::Initialize(fuchsia::sysmem2::Allocator_Sync* sysmem_allocator, fuchsia::ui::composition::Allocator* flatland_allocator,
    FlatlandSurfaceFactory* flatland_surface_factory, zx::eventpair handle, zx::channel sysmem_token, gfx::Size size, gfx::BufferFormat format,
    gfx::BufferUsage usage, VkDevice vk_device, size_t min_buffer_count, bool register_with_flatland_allocator)
{
    ...
    if (sysmem_token) {
        ...
        zx_status_t status = sysmem_allocator->AllocateSharedCollection(
            std::move(fuchsia::sysmem2::AllocatorAllocateSharedCollectionRequest {}.set_token_request(collection_token.NewRequest())));
        if (status != ZX_OK) {
            ZX_DLOG(ERROR, status) << "fuchsia.sysmem.Allocator.AllocateSharedCollection()";
            return false;
        }
    }

    return InitializeInternal(sysmem_allocator, flatland_allocator, std::move(collection_token), register_with_flatland_allocator, min_buffer_count);
}

void FlatlandSysmemBufferCollection::InitializeForTesting(zx::eventpair handle, gfx::BufferUsage usage)
{
    handle_ = std::move(handle);
    ...
    if (usage == gfx::BufferUsage::SCANOUT) {
        ...
        zx::eventpair::create(0, &export_token.value, &flatland_import_token_.value);
    }
}

scoped_refptr<gfx::NativePixmap> FlatlandSysmemBufferCollection::CreateNativePixmap(gfx::NativePixmapHandle handle, gfx::Size size)
{
    ...
    handle.ram_coherency = buffers_info_.settings().buffer_settings().coherency_domain() == fuchsia::sysmem2::CoherencyDomain::RAM;
    ...
    zx_status_t status = buffers_info_.buffers()[handle.buffer_index].vmo().duplicate(ZX_RIGHT_SAME_RIGHTS, &main_plane_vmo);
    if (status != ZX_OK) {
        ZX_DLOG(ERROR, status) << "zx_handle_duplicate";
        return nullptr;
    }

    const fuchsia::sysmem2::ImageFormatConstraints& format = buffers_info_.settings().image_format_constraints();

    // The logic should match LogicalBufferCollection::Allocate().
    size_t stride
        = RoundUp(std::max(static_cast<size_t>(format.min_bytes_per_row()), size.width() * GetBytesPerPixel(format_)), format.bytes_per_row_divisor());
    size_t plane_offset = buffers_info_.buffers()[handle.buffer_index].vmo_usable_start();
    size_t plane_size = stride * size.height();
    handle.planes.emplace_back(stride, plane_offset, plane_size, std::move(main_plane_vmo));
    ...
    if (format_ == gfx::BufferFormat::YUV_420_BIPLANAR) {
        ...
        zx::vmo uv_plane_vmo;
        status = handle.planes[0].vmo.duplicate(ZX_RIGHT_SAME_RIGHTS, &uv_plane_vmo);
        if (status != ZX_OK) {
            ZX_DLOG(ERROR, status) << "zx_handle_duplicate";
            return nullptr;
        }

        handle.planes.emplace_back(stride, uv_plane_offset, uv_plane_size, std::move(uv_plane_vmo));
...
bool FlatlandSysmemBufferCollection::CreateVkImage(size_t buffer_index, VkDevice vk_device, gfx::Size size, VkImage* vk_image, VkImageCreateInfo* vk_image_info,
    VkDeviceMemory* vk_device_memory, VkDeviceSize* mem_allocation_size)
{
    ...
    VkBufferCollectionPropertiesFUCHSIA properties = { VK_STRUCTURE_TYPE_BUFFER_COLLECTION_PROPERTIES_FUCHSIA };
    if (vkGetBufferCollectionPropertiesFUCHSIA(vk_device_, vk_buffer_collection_, &properties) != VK_SUCCESS) {
    ...
    if (vkCreateImage(vk_device_, vk_image_info, nullptr, vk_image) != VK_SUCCESS) {
    ...
    VkMemoryDedicatedAllocateInfoKHR dedicated_allocate = { VK_STRUCTURE_TYPE_MEMORY_DEDICATED_ALLOCATE_INFO_KHR };
    dedicated_allocate.image = *vk_image;
    VkImportMemoryBufferCollectionFUCHSIA buffer_collection_info = { VK_STRUCTURE_TYPE_IMPORT_MEMORY_BUFFER_COLLECTION_FUCHSIA, &dedicated_allocate };
    buffer_collection_info.collection = vk_buffer_collection_;
    buffer_collection_info.index = buffer_index;

    VkMemoryAllocateInfo alloc_info = { VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO, &buffer_collection_info };
    alloc_info.allocationSize = requirements.size;
    alloc_info.memoryTypeIndex = memory_type;

    if (vkAllocateMemory(vk_device_, &alloc_info, nullptr, vk_device_memory) != VK_SUCCESS) {
    ...
    if (vkBindImageMemory(vk_device_, *vk_image, *vk_device_memory, 0u) != VK_SUCCESS) {
...
fuchsia::ui::composition::BufferCollectionImportToken FlatlandSysmemBufferCollection::GetFlatlandImportToken() const
{
    DCHECK(HasFlatlandImportToken());

    fuchsia::ui::composition::BufferCollectionImportToken import_dup;
    zx_status_t status = flatland_import_token_.value.duplicate(ZX_RIGHT_SAME_RIGHTS, &import_dup.value);
    if (status != ZX_OK) {
...
void FlatlandSysmemBufferCollection::AddOnReleasedCallback(base::OnceClosure on_released)
{
...
FlatlandSysmemBufferCollection::~FlatlandSysmemBufferCollection()
{
    if (vk_buffer_collection_ != VK_NULL_HANDLE) {
        vkDestroyBufferCollectionFUCHSIA(vk_device_, vk_buffer_collection_, nullptr);
    }
...
bool FlatlandSysmemBufferCollection::InitializeInternal(fuchsia::sysmem2::Allocator_Sync* sysmem_allocator,
    fuchsia::ui::composition::Allocator* flatland_allocator, fuchsia::sysmem2::BufferCollectionTokenSyncPtr collection_token,
    bool register_with_flatland_allocator, size_t min_buffer_count)
{
    fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> collection_token_for_vulkan;
    collection_token->Duplicate(std::move(fuchsia::sysmem2::BufferCollectionTokenDuplicateRequest {}
                                              .set_rights_attenuation_mask(ZX_RIGHT_SAME_RIGHTS)
                                              .set_token_request(collection_token_for_vulkan.NewRequest())));

    fidl::InterfaceHandle<fuchsia::sysmem2::BufferCollectionToken> collection_token_for_flatland;
    ...
    status = sysmem_allocator->BindSharedCollection(std::move(fuchsia::sysmem2::AllocatorBindSharedCollectionRequest {}
    ...
    if (is_mappable()) {
        constraints.mutable_usage()->set_cpu(fuchsia::sysmem2::CPU_USAGE_READ | fuchsia::sysmem2::CPU_USAGE_WRITE);
    ...
    status = collection_->SetConstraints(std::move(fuchsia::sysmem2::BufferCollectionSetConstraintsRequest {}.set_constraints(std::move(constraints))));
    if (status != ZX_OK) {
        ZX_DLOG(ERROR, status) << "fuchsia.sysmem.BufferCollection.SetConstraints()";
        return false;
    }
    ...
    if (register_with_flatland_allocator) {
        ...
        fuchsia::ui::composition::BufferCollectionExportToken export_token;
        status = zx::eventpair::create(0, &export_token.value, &flatland_import_token_.value);

        fuchsia::ui::composition::RegisterBufferCollectionArgs args;
        args.set_export_token(std::move(export_token));
        args.set_buffer_collection_token(fuchsia::sysmem::BufferCollectionTokenHandle(collection_token_for_flatland.TakeChannel()));
        args.set_usage(fuchsia::ui::composition::RegisterBufferCollectionUsage::DEFAULT);
        flatland_allocator->RegisterBufferCollection(std::move(args), [](fuchsia::ui::composition::Allocator_RegisterBufferCollection_Result result) {
            if (result.is_err()) {
    ...
    VkBufferCollectionCreateInfoFUCHSIA buffer_collection_create_info = { VK_STRUCTURE_TYPE_BUFFER_COLLECTION_CREATE_INFO_FUCHSIA };
    buffer_collection_create_info.collectionToken = token_channel.get();
    if (vkCreateBufferCollectionFUCHSIA(vk_device_, &buffer_collection_create_info, nullptr, &vk_buffer_collection_) != VK_SUCCESS) {
    ...
    auto image_constraints_info = InitializeImageConstraintsInfo(image_create_info, /* allow_protected_memory */ true);

    auto result = vkSetBufferCollectionImageConstraintsFUCHSIA(vk_device_, vk_buffer_collection_, &image_constraints_info->image_constraints);
    if (result != VK_SUCCESS) {
        DLOG(ERROR) << "vkSetBufferCollectionConstraintsFUCHSIA() failed:" << result;
        return false;
    }
    ...
    buffers_info_ = std::move(*wait_result.response().mutable_buffer_collection_info());
    ...
    handle_watch_ = std::make_unique<base::MessagePumpForIO::ZxHandleWatchController>(FROM_HERE);
    bool watch_result = base::CurrentIOThread::Get()->WatchZxHandle(handle_.get(), /*persistent=*/false, ZX_EVENTPAIR_PEER_CLOSED, handle_watch_.get(), this);
...
void FlatlandSysmemBufferCollection::InitializeImageCreateInfo(VkImageCreateInfo* vk_image_info, gfx::Size size)
{
    *vk_image_info = { VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO };
    ...
    vk_image_info->extent = VkExtent3D { static_cast<uint32_t>(size.width()), static_cast<uint32_t>(size.height()), 1 };
    vk_image_info->mipLevels = 1;
    vk_image_info->arrayLayers = 1;
    vk_image_info->samples = VK_SAMPLE_COUNT_1_BIT;
    vk_image_info->tiling = is_mappable() ? VK_IMAGE_TILING_LINEAR : VK_IMAGE_TILING_OPTIMAL;

    vk_image_info->usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
...
void FlatlandSysmemBufferCollection::OnZxHandleSignalled(zx_handle_t handle, zx_signals_t signals)
{
