...
gfx::Size GetDisplaySize()
{
    ...
    int width, height;
    if (base::StringToInt(cmd_line->GetSwitchValueASCII(switches::kCastInitialScreenWidth), &width)
        && base::StringToInt(cmd_line->GetSwitchValueASCII(switches::kCastInitialScreenHeight), &height)) {
...
scoped_refptr<gl::GLSurface> GLOzoneEglCast::CreateViewGLSurface(gl::GLDisplay* display, gfx::AcceleratedWidget widget)
{
    ...
    return gl::InitializeGLSurface(new GLSurfaceCast(display->GetAs<gl::GLDisplayEGL>(), widget, this));
}

scoped_refptr<gl::GLSurface> GLOzoneEglCast::CreateOffscreenGLSurface(gl::GLDisplay* display, const gfx::Size& size)
{
...
gl::EGLDisplayPlatform GLOzoneEglCast::GetNativeDisplay()
{
    ...
    return gl::EGLDisplayPlatform(reinterpret_cast<EGLNativeDisplayType>(display_type_));
}
...
bool GLOzoneEglCast::LoadGLES2Bindings(const gl::GLImplementationParts& implementation)
{
    InitializeHardwareIfNeeded();
