...
SurfaceFactoryCast::SurfaceFactoryCast(std::unique_ptr<chromecast::CastEglPlatform> egl_platform)
{
    if (egl_platform) {
        egl_implementation_ = std::make_unique<GLOzoneEglCast>(std::move(egl_platform));
    }
}
...
std::vector<gl::GLImplementationParts> SurfaceFactoryCast::GetAllowedGLImplementations()
{
...
GLOzone* SurfaceFactoryCast::GetGLOzone(const gl::GLImplementationParts& implementation)
{
...
std::unique_ptr<SurfaceOzoneCanvas> SurfaceFactoryCast::CreateCanvasForWidget(gfx::AcceleratedWidget widget)
{
...
scoped_refptr<gfx::NativePixmap> SurfaceFactoryCast::CreateNativePixmap(gfx::AcceleratedWidget widget, gpu::VulkanDeviceQueue* device_queue, gfx::Size size,
