...
constexpr auto colors = std::to_array<ExamplesColorIds>({ ExamplesColorIds::kColorBubbleExampleBackground1, ExamplesColorIds::kColorBubbleExampleBackground2,
    ExamplesColorIds::kColorBubbleExampleBackground3, ExamplesColorIds::kColorBubbleExampleBackground4 });

constexpr auto arrows = std::to_array<BubbleBorder::Arrow>({ BubbleBorder::TOP_LEFT, BubbleBorder::TOP_CENTER, BubbleBorder::TOP_RIGHT, BubbleBorder::RIGHT_TOP,
    BubbleBorder::RIGHT_CENTER, BubbleBorder::RIGHT_BOTTOM, BubbleBorder::BOTTOM_RIGHT, BubbleBorder::BOTTOM_CENTER, BubbleBorder::BOTTOM_LEFT,
    BubbleBorder::LEFT_BOTTOM, BubbleBorder::LEFT_CENTER, BubbleBorder::LEFT_TOP });
...
protected:
    void Init() override
    {
        SetLayoutManager(std::make_unique<BoxLayout>(BoxLayout::Orientation::kVertical, gfx::Insets(50)));
...
void BubbleExample::CreateExampleView(View* container)
{
    ...
    standard_shadow_ = container->AddChildView(std::make_unique<LabelButton>(
        base::BindRepeating(&BubbleExample::ShowBubble, base::Unretained(this), &standard_shadow_, BubbleBorder::STANDARD_SHADOW, false), u"Standard Shadow"));
    no_shadow_ = container->AddChildView(std::make_unique<LabelButton>(
        base::BindRepeating(&BubbleExample::ShowBubble, base::Unretained(this), &no_shadow_, BubbleBorder::NO_SHADOW, false), u"No Shadow"));
    persistent_ = container->AddChildView(std::make_unique<LabelButton>(
        base::BindRepeating(&BubbleExample::ShowBubble, base::Unretained(this), &persistent_, BubbleBorder::NO_SHADOW, true), u"Persistent"));
}

void BubbleExample::ShowBubble(raw_ptr<Button>* button, BubbleBorder::Shadow shadow, bool persistent, const ui::Event& event)
{
    ...
    bubble->set_color(provider->GetColor(colors[(color_index++) % std::size(colors)]));
    ...
    LogStatus("Click with optional modifiers: [Ctrl] for set_arrow(NONE), "
