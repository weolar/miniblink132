...
protected:
    ...
    {
        View::OnThemeChanged();
        SetBorder(CreateSolidBorder(1, GetColorProvider()->GetColor(ui::kColorFocusableBorderUnfocused)));
    }
};
...
std::unique_ptr<Textfield> CreateCommonTextfield(TextfieldController* container)
{
...
std::unique_ptr<Textfield> CreateCommonTextfieldWithAXName(TextfieldController* container, std::u16string name)
{
...
bool LayoutExampleBase::ChildPanel::OnMousePressed(const ui::MouseEvent& event)
{
...
ProposedLayout LayoutExampleBase::ChildPanel::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
    ...
    if (selected_) {
        ...
        layout.child_layouts.emplace_back(margin_.top.get(), margin_.top->GetVisible(),
            gfx::Rect((layout.host_size.width() - preferred_size.width()) / 2, kSpacing, preferred_size.width(), preferred_size.height()));
        preferred_size = margin_.left->GetPreferredSize({});
        layout.child_layouts.emplace_back(margin_.left.get(), margin_.left->GetVisible(),
            gfx::Rect(kSpacing, (layout.host_size.height() - preferred_size.height()) / 2, preferred_size.width(), preferred_size.height()));
        preferred_size = margin_.bottom->GetPreferredSize({});
        layout.child_layouts.emplace_back(margin_.bottom.get(), margin_.bottom->GetVisible(),
            gfx::Rect((layout.host_size.width() - preferred_size.width()) / 2, layout.host_size.height() - preferred_size.height() - kSpacing,
                preferred_size.width(), preferred_size.height()));
        preferred_size = margin_.right->GetPreferredSize({});
        layout.child_layouts.emplace_back(margin_.right.get(), margin_.right->GetVisible(),
            gfx::Rect(layout.host_size.width() - preferred_size.width() - kSpacing, (layout.host_size.height() - preferred_size.height()) / 2,
                preferred_size.width(), preferred_size.height()));
        preferred_size = flex_->GetPreferredSize({});
        layout.child_layouts.emplace_back(flex_.get(), flex_->GetVisible(),
            gfx::Rect((layout.host_size.width() - preferred_size.width()) / 2, (layout.host_size.height() - preferred_size.height()) / 2,
                preferred_size.width(), preferred_size.height()));
    }
    return layout;
}
...
{
    View::OnThemeChanged();
    SetBorder(CreateSolidBorder(1, GetColorProvider()->GetColor(selected_ ? ui::kColorFocusableBorderFocused : ui::kColorFocusableBorderUnfocused)));
}

void LayoutExampleBase::ChildPanel::ContentsChanged(Textfield* sender, const std::u16string& new_contents)
{
...
Textfield* LayoutExampleBase::ChildPanel::CreateTextfield(const std::u16string& name)
{
...
gfx::Insets LayoutExampleBase::TextfieldsToInsets(const InsetTextfields& textfields, const gfx::Insets& default_insets)
{
    ...
    return gfx::Insets::TLBR(std::max(0, top), std::max(0, left), std::max(0, bottom), std::max(0, right));
}

Combobox* LayoutExampleBase::CreateAndAddCombobox(
    const std::u16string& label_text, base::span<const char* const> items, base::RepeatingClosure combobox_callback)
{
    auto* const row = control_panel_->AddChildView(std::make_unique<View>());
    row->SetLayoutManager(std::make_unique<BoxLayout>(BoxLayout::Orientation::kHorizontal, gfx::Insets(), kLayoutExampleVerticalSpacing));
    row->AddChildView(std::make_unique<Label>(label_text));
    auto* const combobox = row->AddChildView(std::make_unique<Combobox>(std::make_unique<ExampleComboboxModel>(items)));
    combobox->SetCallback(std::move(combobox_callback));
    combobox->GetViewAccessibility().SetName(label_text);
    return combobox;
}

Textfield* LayoutExampleBase::CreateAndAddTextfield(const std::u16string& label_text)
{
    auto* const row = control_panel_->AddChildView(std::make_unique<View>());
    row->SetLayoutManager(std::make_unique<BoxLayout>(BoxLayout::Orientation::kHorizontal, gfx::Insets(), kLayoutExampleVerticalSpacing));
...
void LayoutExampleBase::CreateMarginsTextFields(const std::u16string& label, InsetTextfields* textfields)
{
    auto* const row = control_panel_->AddChildView(std::make_unique<View>());
    row->SetLayoutManager(std::make_unique<BoxLayout>(BoxLayout::Orientation::kHorizontal, gfx::Insets(), kLayoutExampleVerticalSpacing));
    ...
    container->SetLayoutManager(std::make_unique<BoxLayout>(BoxLayout::Orientation::kVertical, gfx::Insets(), kLayoutExampleVerticalSpacing))
        ->set_cross_axis_alignment(BoxLayout::CrossAxisAlignment::kCenter);
    textfields->top = container->AddChildView(CreateCommonTextfieldWithAXName(this, label + u" " + l10n_util::GetStringUTF16(IDS_LAYOUT_BASE_TOP_LABEL)));
    auto* const middle_row = container->AddChildView(std::make_unique<View>());
    middle_row->SetLayoutManager(std::make_unique<BoxLayout>(BoxLayout::Orientation::kHorizontal, gfx::Insets(), kLayoutExampleVerticalSpacing));
    textfields->left = middle_row->AddChildView(CreateCommonTextfieldWithAXName(this, label + u" " + l10n_util::GetStringUTF16(IDS_LAYOUT_BASE_LEFT_LABEL)));
    textfields->right = middle_row->AddChildView(CreateCommonTextfieldWithAXName(this, label + u" " + l10n_util::GetStringUTF16(IDS_LAYOUT_BASE_RIGHT_LABEL)));
    textfields->bottom = container->AddChildView(CreateCommonTextfieldWithAXName(this, label + u" " + l10n_util::GetStringUTF16(IDS_LAYOUT_BASE_BOTTOM_LABEL)));
}

Checkbox* LayoutExampleBase::CreateAndAddCheckbox(const std::u16string& label_text, base::RepeatingClosure checkbox_callback)
{
    return control_panel_->AddChildView(std::make_unique<Checkbox>(label_text, std::move(checkbox_callback)));
}
...
void LayoutExampleBase::CreateExampleView(View* container)
{
    ...
    auto* const manager = full_panel->SetLayoutManager(std::make_unique<BoxLayout>(views::BoxLayout::Orientation::kHorizontal));
    ...
    control_panel_
        ->SetLayoutManager(std::make_unique<BoxLayout>(
            BoxLayout::Orientation::kVertical, gfx::Insets::VH(kLayoutExampleVerticalSpacing, kLayoutExampleLeftPadding), kLayoutExampleVerticalSpacing))
    ...
    add_button_ = row->AddChildView(std::make_unique<MdTextButton>(
        base::BindRepeating(&LayoutExampleBase::AddButtonPressed, base::Unretained(this)), l10n_util::GetStringUTF16(IDS_LAYOUT_BASE_ADD_LABEL)));

    preferred_width_view_ = row->AddChildView(CreateCommonTextfieldWithAXName(this, l10n_util::GetStringUTF16(IDS_LAYOUT_BASE_PREFERRED_WIDTH_LABEL)));
    preferred_width_view_->SetText(base::NumberToString16(kLayoutExampleDefaultChildSize.width()));

    preferred_height_view_ = row->AddChildView(CreateCommonTextfieldWithAXName(this, l10n_util::GetStringUTF16(IDS_LAYOUT_BASE_PREFERRED_HEIGHT_LABEL)));
    preferred_height_view_->SetText(base::NumberToString16(kLayoutExampleDefaultChildSize.height()));
...
{
    auto* const panel = layout_panel_->AddChildView(std::make_unique<ChildPanel>(this));
