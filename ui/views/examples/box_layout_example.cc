...
void BoxLayoutExample::ContentsChanged(Textfield* textfield, const std::u16string& new_contents)
{
    ...
    } else if (textfield == border_insets_.left || textfield == border_insets_.top || textfield == border_insets_.right || textfield == border_insets_.bottom) {
        UpdateBorderInsets();
    }
...
void BoxLayoutExample::CreateAdditionalControls()
{
    ...
    orientation_
        = CreateAndAddCombobox(u"Orientation", kOrientationValues, base::BindRepeating(&LayoutExampleBase::RefreshLayoutPanel, base::Unretained(this), true));

    constexpr auto kMainAxisValues = std::to_array<const char* const>({ "Start", "Center", "End" });
    main_axis_alignment_
        = CreateAndAddCombobox(u"Main axis", kMainAxisValues, base::BindRepeating(&BoxLayoutExample::MainAxisAlignmentChanged, base::Unretained(this)));

    constexpr auto kCrossAxisValues = std::to_array<const char* const>({ "Start", "Center", "End", "Stretch" });
    cross_axis_alignment_
        = CreateAndAddCombobox(u"Cross axis", kCrossAxisValues, base::BindRepeating(&BoxLayoutExample::CrossAxisAlignmentChanged, base::Unretained(this)));
    ...
    collapse_margins_ = CreateAndAddCheckbox(u"Collapse margins", base::BindRepeating(&LayoutExampleBase::RefreshLayoutPanel, base::Unretained(this), true));
...
void BoxLayoutExample::UpdateLayoutManager()
{
    ...
    layout_ = nullptr;
    layout_ = panel->SetLayoutManager(
        std::make_unique<BoxLayout>(orientation_->GetSelectedIndex() == 0u ? BoxLayout::Orientation::kHorizontal : BoxLayout::Orientation::kVertical,
            gfx::Insets(), child_spacing, collapse_margins_->GetChecked()));

    layout_->set_cross_axis_alignment(static_cast<BoxLayout::CrossAxisAlignment>(cross_axis_alignment_->GetSelectedIndex().value()));
    layout_->set_main_axis_alignment(static_cast<BoxLayout::MainAxisAlignment>(main_axis_alignment_->GetSelectedIndex().value()));
