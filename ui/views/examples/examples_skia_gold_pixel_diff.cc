...
ExamplesExitCode ExamplesSkiaGoldPixelDiff::CompareScreenshot(const std::string& screenshot_name, const views::Widget* widget) const
{
    ...
    ui::GrabWindowSnapshot(
        ...
        base::BindOnce(
            [](gfx::Image* screenshot, base::OnceClosure quit_loop, gfx::Image image) {
                *screenshot = image;
    ...
    return pixel_diff_->CompareScreenshot(
               ui::test::SkiaGoldPixelDiff::GetGoldenImageName(screenshot_prefix_, screenshot_name, ui::test::SkiaGoldPixelDiff::GetPlatform()),
...
void ExamplesSkiaGoldPixelDiff::DoScreenshot(views::Widget* widget)
{
    ...
    result_ = CompareScreenshot(test_info ? test_info->name() : "ExampleWindow", widget);
    widget->Close();
}
