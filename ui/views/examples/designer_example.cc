...
{
    const SkColor grid_color = GetColorProvider()->GetColor(ExamplesColorIds::kColorDesignerGrid);
    gfx::Size grid_size = grid_size_ <= 8 ? gfx::Size(grid_size_ * 8, grid_size_ * 8) : gfx::Size(grid_size_, grid_size_);
...
template <typename C> class ClassRegistration : public BaseClassRegistration {
public:
...
template <> class ClassRegistration<Combobox> : public BaseClassRegistration, public ui::ComboboxModel {
public:
    ClassRegistration() = default;
    ~ClassRegistration() override = default;
    std::unique_ptr<View> CreateView() override
    {
        return Builder<Combobox>().SetModel(this).SetAccessibleName(l10n_util::GetStringUTF16(IDS_DESIGNER_COMBOBOX_NAME)).Build();
    }
    std::u16string GetViewClassName() override
    {
...
template <> class ClassRegistration<MdTextButton> : public BaseClassRegistration {
public:
    ClassRegistration() = default;
    ~ClassRegistration() override = default;
    std::unique_ptr<View> CreateView() override
...
template <> class ClassRegistration<Textfield> : public BaseClassRegistration {
public:
    ClassRegistration() = default;
    ~ClassRegistration() override = default;
    std::unique_ptr<View> CreateView() override
    {
        ...
        return Builder<Textfield>()
            ...
            .SetAccessibleName(l10n_util::GetStringUTF16(IDS_DESIGNER_TEXTFIELD_NAME))
            .Build();
    }
    std::u16string GetViewClassName() override
    {
...
template <> class ClassRegistration<Checkbox> : public BaseClassRegistration {
public:
    ClassRegistration() = default;
    ~ClassRegistration() override = default;
    std::unique_ptr<View> CreateView() override
...
template <> class ClassRegistration<RadioButton> : public BaseClassRegistration {
public:
    ClassRegistration() = default;
    ~ClassRegistration() override = default;
    std::unique_ptr<View> CreateView() override
...
template <> class ClassRegistration<ToggleButton> : public BaseClassRegistration {
public:
    ClassRegistration() = default;
    ~ClassRegistration() override = default;
    std::unique_ptr<View> CreateView() override
    {
        return Builder<ToggleButton>().SetAccessibleName(l10n_util::GetStringUTF16(IDS_DESIGNER_IMAGEBUTTON_NAME)).Build();
    }
    std::u16string GetViewClassName() override
    {
...
template <> class ClassRegistration<ImageButton> : public BaseClassRegistration {
public:
    ClassRegistration() = default;
    ~ClassRegistration() override = default;
    std::unique_ptr<View> CreateView() override
    {
        return Builder<ImageButton>()
            .SetImageModel(Button::ButtonState::STATE_NORMAL, ui::ImageModel::FromVectorIcon(kPinIcon, ui::kColorIcon))
            .SetAccessibleName(l10n_util::GetStringUTF16(IDS_DESIGNER_IMAGEBUTTON_NAME))
            .Build();
    }
    std::u16string GetViewClassName() override
    {
...
{
    while (child_view && child_view != designer && child_view->parent() != designer) {
...
DesignerExample::GrabHandle::GrabHandle(GrabHandles* grab_handles, GrabHandlePosition position)
    : position_(position)
    , grab_handles_(grab_handles)
{
}
...
gfx::Size DesignerExample::GrabHandle::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
...
void DesignerExample::GrabHandle::OnPaint(gfx::Canvas* canvas)
{
    ...
    cc::PaintFlags flags;
    flags.setColor(GetColorProvider()->GetColor(ExamplesColorIds::kColorDesignerGrabHandle));
...
bool DesignerExample::GrabHandle::OnMouseDragged(const ui::MouseEvent& event)
{
    ...
    if (position_ == GrabHandlePosition::kTop || position_ == GrabHandlePosition::kBottom) {
        new_location.set_x(mouse_drag_pos_.x());
    }
    if (position_ == GrabHandlePosition::kLeft || position_ == GrabHandlePosition::kRight) {
...
void DesignerExample::GrabHandle::UpdateViewSize()
{
    ...
    if (IsTop(position_)) {
        view_bounds.set_height(view_bounds.height() - (view_center.y() - view_bounds.y()));
        view_bounds.set_y(view_center.y());
    }
    if (IsBottom(position_))
        view_bounds.set_height(view_center.y() - view_bounds.y());
    if (IsLeft(position_)) {
        view_bounds.set_width(view_bounds.width() - (view_center.x() - view_bounds.x()));
...
void DesignerExample::GrabHandles::Initialize(View* layout_panel)
{
    static constexpr GrabHandlePosition positions[] = {
        GrabHandlePosition::kTop,
        GrabHandlePosition::kBottom,
        GrabHandlePosition::kLeft,
        GrabHandlePosition::kRight,
        GrabHandlePosition::kTopLeft,
        GrabHandlePosition::kTopRight,
        GrabHandlePosition::kBottomLeft,
        GrabHandlePosition::kBottomRight,
    };
            ...
            Builder<BoxLayoutView>()
                ...
                .AddChildren(
                    Builder<DesignerSurface>()
                        ...
                        .CustomConfigure(base::BindOnce([](DesignerExample* designer_example,
                                                            DesignerSurface* designer_surface) { designer_surface->AddPreTargetHandler(designer_example); },
                    ...
                    Builder<BoxLayoutView>()
                        ...
                        .AddChildren(Builder<Combobox>().CopyAddressTo(&view_type_).SetAccessibleName(u"View Type").SetModel(this),
                            Builder<MdTextButton>().SetCallback(base::BindRepeating(&DesignerExample::CreateView, base::Unretained(this))).SetText(u"Add"),
                            TableView::CreateScrollViewBuilderWithTable(Builder<TableView>()
                                                                            .CopyAddressTo(&inspector_)
                                                                            .SetColumns({ MakeColumn(0, u"Name", true), MakeColumn(1, u"Value", false) })
                                                                            .SetModel(this)
...
void DesignerExample::OnEvent(ui::Event* event)
{
    if (event->IsMouseEvent() && event->target()) {
        ...
        if (IsViewParent(designer_panel_, view->parent()) || view == designer_panel_) {
...
void DesignerExample::HandleDesignerMouseEvent(ui::Event* event)
{
    ...
    case ui::EventType::kMousePressed:
        if (mouse_event->IsOnlyLeftMouseButton()) {
            DCHECK(!dragging_);
            View* event_view = GetDesignerChild(static_cast<View*>(event->target()), designer_panel_);
    ...
    case ui::EventType::kMouseDragged:
        if (dragging_) {
            if (grab_handles_.IsGrabHandle(dragging_))
                return;
            gfx::Point new_position = selected_->origin() + SnapToGrid(mouse_event->location() - last_mouse_pos_);
            new_position.SetToMax(gfx::Point());
            new_position.SetToMin(designer_panel_->GetLocalBounds().bottom_right() - gfx::Vector2d(dragging_->size().width(), dragging_->size().height()));
...
{
    return gfx::Vector2d(SnapToInterval(distance.x(), designer_panel_->GetGridSize()), SnapToInterval(distance.y(), designer_panel_->GetGridSize()));
}

void DesignerExample::CreateView(const ui::Event& event)
{
    std::unique_ptr<View> new_view = class_registrations_[view_type_->GetSelectedRow().value()]->CreateView();
    ...
    child_rect.set_origin(gfx::Point() + SnapToGrid(child_rect.OffsetFromOrigin()));
