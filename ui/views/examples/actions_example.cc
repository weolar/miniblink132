...
// clang-format on
constexpr auto kActionIdStrings = base::MakeFixedFlatMap<actions::ActionId, const char*>({ ACTION_IDS EXAMPLES_ACTION_IDS });
// clang-format on
...
ActionItemComboboxModel::ActionItemComboboxModel(actions::ActionItem* action_scope)
{
...
actions::ActionItem* ActionItemComboboxModel::GetActionItemAt(size_t index) const
{
    return items_[index];
}
...
ProposedLayout FlowLayout::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
    ...
    for (views::View* view : host_view()->children()) {
        ...
        ChildLayout child_layout = { view, view_visible, proposed_bounds, available_bounds };
...
ActionsExample::ActionsExample()
    : ExampleBase("Actions")
{
    subscriptions_.push_back(
        actions::ActionManager::Get().AppendActionItemInitializer(base::BindRepeating(&ActionsExample::CreateActions, base::Unretained(this))));
}
...
void ActionsExample::CreateExampleView(View* container)
{
    Builder<View>(container)
        ...
        .AddChild(Builder<BoxLayoutView>()
                      .SetOrientation(BoxLayout::Orientation::kHorizontal)
                      .AddChildren(Builder<View>()
                                       .CopyAddressTo(&action_panel_)
                                       .SetLayoutManager(std::make_unique<FlowLayout>())
                                       .SetBorder(CreateThemedSolidBorder(1, ui::kColorFocusableBorderUnfocused)),
                          Builder<BoxLayoutView>()
                              .CopyAddressTo(&control_panel_)
                              .SetOrientation(BoxLayout::Orientation::kVertical)
                              .SetInsideBorderInsets(gfx::Insets::VH(kActionExampleVerticalSpacing, kActionExampleLeftPadding))
                              .SetBetweenChildSpacing(kActionExampleVerticalSpacing))
                      .AfterBuild(base::BindOnce(
                          [](raw_ptr<View>* action_panel, raw_ptr<BoxLayoutView>* control_panel, BoxLayoutView* box) {
    ...
    auto add_row = [this]() {
        auto* row = control_panel_->AddChildView(Builder<BoxLayoutView>()
                                                     .SetOrientation(BoxLayout::Orientation::kHorizontal)
                                                     .SetBetweenChildSpacing(kActionExampleVerticalSpacing)
    ...
    auto add_combobox_row = [&add_row](std::unique_ptr<ui::ComboboxModel> model, int label_text) -> std::pair<View*, Combobox*> {
        auto* row = add_row();
        Label* label = nullptr;
        Combobox* combobox = nullptr;
        Builder<BoxLayoutView>(row)
            .AddChildren(Builder<Label>().CopyAddressTo(&label).SetText(l10n_util::GetStringUTF16(label_text)),
                Builder<Combobox>(std::make_unique<Combobox>(std::move(model))).CopyAddressTo(&combobox))
            .BuildChildren();
        combobox->GetViewAccessibility().SetName(label->GetText());
        return { row, combobox };
    };

    auto add_combobox_and_button = [&add_combobox_row, this](std::unique_ptr<ui::ComboboxModel> model, int label_text, actions::ActionId action_id) {
        auto pair = add_combobox_row(std::move(model), label_text);
        views::MdTextButton* action_button;
        pair.first->AddChildView(Builder<MdTextButton>().CopyAddressTo(&action_button).Build());
        action_view_controller_.CreateActionViewRelationship(action_button, actions::ActionManager::Get().FindAction(action_id)->GetAsWeakPtr());
        return pair.second;
    };

    auto add_checkbox_row = [this, &add_row](int checkbox_text, PropertyChangedCallback callback) {
        auto* row = add_row();
        auto* checkbox = row->AddChildView(Builder<Checkbox>().SetText(l10n_util::GetStringUTF16(checkbox_text)).Build());
        subscriptions_.push_back(checkbox->AddCheckedChangedCallback(std::move(callback)));
        return checkbox;
    };

    auto add_textfield_row = [this, &add_row](int label_text, PropertyChangedCallback callback) {
        auto* row = add_row();
        Label* label = nullptr;
        ...
        Builder<BoxLayoutView>(row)
            .AddChildren(Builder<Label>().CopyAddressTo(&label).SetText(l10n_util::GetStringUTF16(label_text)),
                Builder<Textfield>()
                    ...
                    .CustomConfigure(
                        base::BindOnce([](std::vector<base::CallbackListSubscription>* subscriptions, PropertyChangedCallback callback,
                                           Textfield* textfield) { subscriptions->push_back(textfield->AddTextChangedCallback(std::move(callback))); },
    ...
    available_controls_ = add_combobox_and_button(std::make_unique<ControlTypeComboboxModel>(), IDS_AVAILABLE_CONTROLS, kActionCreateControl);

    control_panel_->AddChildView(Builder<Separator>().SetProperty(kMarginsKey, kSeparatorPadding).Build());

    controls_ = add_combobox_row(std::make_unique<ViewsComboboxModel>(action_panel_.get()), IDS_CONTROLS).second;

    available_actions_ = add_combobox_and_button(std::make_unique<ActionItemComboboxModel>(example_actions_.get()), IDS_AVAILABLE_ACTIONS, kActionAssignAction);
    subscriptions_.push_back(available_actions_->AddSelectedIndexChangedCallback(base::BindRepeating(&ActionsExample::ActionSelected, base::Unretained(this))));

    action_visible_ = add_checkbox_row(IDS_ACTION_VISIBLE, base::BindRepeating(&ActionsExample::VisibleChanged, base::Unretained(this)));
    action_enabled_ = add_checkbox_row(IDS_ACTION_ENABLED, base::BindRepeating(&ActionsExample::EnabledChanged, base::Unretained(this)));
    action_checked_ = add_checkbox_row(IDS_ACTION_CHECKED, base::BindRepeating(&ActionsExample::CheckedChanged, base::Unretained(this)));
    action_text_ = add_textfield_row(IDS_ACTION_TEXT, base::BindRepeating(&ActionsExample::TextChanged, base::Unretained(this)));
    action_tooltip_text_ = add_textfield_row(IDS_ACTION_TOOLTIP_TEXT, base::BindRepeating(&ActionsExample::TooltipTextChanged, base::Unretained(this)));
    auto* row = add_row();
    control_panel_->SetFlexForView(row, 1);
    actions_trigger_info_
        = row->AddChildView(Builder<Textarea>().SetReadOnly(true).SetAccessibleName(l10n_util::GetStringUTF16(IDS_ACTIONS_TRIGGER_INFO)).Build());
...
{
    manager->AddActions(actions::ActionItem::Builder()
                            .CopyAddressTo(&example_actions_)
                            .AddChildren(actions::ActionItem::Builder(base::BindRepeating(&ActionsExample::ActionInvoked, base::Unretained(this)))
                                             .SetText(u"Test Action 1")
                                             .SetActionId(kActionTest1),
                                actions::ActionItem::Builder(base::BindRepeating(&ActionsExample::ActionInvoked, base::Unretained(this)))
        ...
        actions::ActionItem::Builder()
            .AddChildren(actions::ActionItem::Builder(base::BindRepeating(&ActionsExample::AssignAction, base::Unretained(this)))
                             .SetActionId(kActionAssignAction)
                             .SetText(l10n_util::GetStringUTF16(IDS_ASSIGN_ACTION_TO_CONTROL)),
                actions::ActionItem::Builder(base::BindRepeating(&ActionsExample::CreateControl, base::Unretained(this)))
                    .SetActionId(kActionCreateControl)
                    .SetText(l10n_util::GetStringUTF16(IDS_CREATE_CONTROL)))
            .Build());
}

void ActionsExample::ActionInvoked(actions::ActionItem* action, actions::ActionInvocationContext context)
{
    auto bool_to_string = [](bool value) { return value ? kBoolStrings[1] : kBoolStrings[0]; };
    std::u16string text = actions_trigger_info_->GetText();
    if (!text.empty()) {
        text.append(u"\n");
    }
    text.append(base::ASCIIToUTF16(
        base::StringPrintf("Action ID: %s; Text: \"%s\"; Visible: %s; Enabled: %s", kActionIdStrings.at(action->GetActionId().value_or(actions::kActionsStart)),
            base::UTF16ToASCII(action->GetText()).c_str(), bool_to_string(action->GetVisible()), bool_to_string(action->GetEnabled()))));
...
void ActionsExample::AssignAction(actions::ActionItem* action, actions::ActionInvocationContext context)
{
    auto index = controls_->GetSelectedIndex();
    if (!index || controls_->GetModel()->GetItemCount() == 0) {
        return;
    }
    ViewsComboboxModel* model = static_cast<ViewsComboboxModel*>(controls_->GetModel());
    size_t index_val = index.value();
    View* view = model->GetViewItemAt(index_val);
    if (IsViewClass<MdTextButton>(view)) {
        action_view_controller_.CreateActionViewRelationship(static_cast<MdTextButton*>(view), GetSelectedAction()->GetAsWeakPtr());
    } else if (IsViewClass<Checkbox>(view)) {
        action_view_controller_.CreateActionViewRelationship(static_cast<Checkbox*>(view), GetSelectedAction()->GetAsWeakPtr());
    }
}

void ActionsExample::CreateControl(actions::ActionItem* action, actions::ActionInvocationContext context)
{
    static int control_num = 0;
    std::unique_ptr<View> new_view;
    const std::optional<size_t> selected_index = available_controls_->GetSelectedIndex();
    switch (selected_index.value_or(0)) {
    case 0:
        new_view = Builder<MdTextButton>().SetText(u"Button " + base::NumberToString16(control_num)).Build();
        break;
    case 1:
        new_view = Builder<Checkbox>().SetText(u"Checkbox " + base::NumberToString16(control_num)).Build();
        break;
    default:
        NOTREACHED();
    }
...
{
    ActionItemComboboxModel* action_item_model = static_cast<ActionItemComboboxModel*>(available_actions_->GetModel());
    return action_item_model->GetActionItemAt(available_actions_->GetSelectedIndex().value_or(0));
}
