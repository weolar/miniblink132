...
private:
    ...
    {
        return base::UTF8ToUTF16(base::StringPrintf("%c item", static_cast<char>('A' + index)));
    }
};
...
void ComboboxExample::CreateExampleView(View* container)
{
    ...
    auto view = Builder<BoxLayoutView>()
                    ...
                    .AddChildren(Builder<Combobox>()
                                     .CopyAddressTo(&combobox_)
                                     .SetOwnedModel(std::make_unique<ComboboxModelExample>())
                                     .SetSelectedIndex(3)
                                     .SetAccessibleName(l10n_util::GetStringUTF16(IDS_COMBOBOX_NAME_1))
                                     .SetCallback(base::BindRepeating(&ComboboxExample::ValueChanged, base::Unretained(this))),
                        Builder<Combobox>()
                            .SetOwnedModel(std::make_unique<ComboboxModelExample>())
                            .SetEnabled(false)
                            .SetSelectedIndex(4)
                            .SetAccessibleName(l10n_util::GetStringUTF16(IDS_COMBOBOX_NAME_2))
                            .SetCallback(base::BindRepeating(&ComboboxExample::ValueChanged, base::Unretained(this))),
                        Builder<EditableCombobox>()
                            ...
                            .SetEnabled(true)
                            .SetAccessibleName(l10n_util::GetStringUTF16(IDS_COMBOBOX_NAME_2))
                            .SetCallback(base::BindRepeating(&ComboboxExample::EditableValueChanged, base::Unretained(this))))
                    .Build();
...
{
    PrintStatus("Selected: %s", base::UTF16ToUTF8(combobox_->GetModel()->GetItemAt(combobox_->GetSelectedIndex().value())).c_str());
}

void ComboboxExample::EditableValueChanged()
{
    PrintStatus("Changed: %s", base::UTF16ToUTF8(editable_combobox_->GetText()).c_str());
}
