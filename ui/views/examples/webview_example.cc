...
{
    webview_ = container->AddChildView(std::make_unique<WebView>(browser_context_));
...
bool WebViewExample::HandleKeyboardEvent(content::WebContents* source, const input::NativeWebKeyboardEvent& event)
{
    return unhandled_keyboard_event_handler_.HandleKeyboardEvent(event, webview_->GetFocusManager());
}
