...
base::LazyInstance<base::TestDiscardableMemoryAllocator>::DestructorAtExit g_discardable_memory_allocator = LAZY_INSTANCE_INITIALIZER;
...
ExamplesExitCode ExamplesMainProc(bool under_test, ExampleVector examples)
{
    ...
    base::FeatureList::InitInstance(
        command_line->GetSwitchValueASCII(switches::kEnableFeatures), command_line->GetSwitchValueASCII(switches::kDisableFeatures));
    ...
    ui::ColorProviderManager::Get().AppendColorProviderInitializer(base::BindRepeating(&AddExamplesColorMixers));
...

    {
        ...
        auto disable_timeout = std::make_unique<base::test::ScopedDisableRunLoopTimeout>();

        if (under_test) {
            pixel_diff.Init("ViewsExamples");
            widget_observer.set_shown_callback(base::BindRepeating(&ExamplesSkiaGoldPixelDiff::OnExamplesWindowShown, base::Unretained(&pixel_diff)));
        ...
        if (examples.empty()) {
            ...
            views::examples::ShowExamplesWindow(run_loop.QuitClosure(), std::move(examples));
        }
