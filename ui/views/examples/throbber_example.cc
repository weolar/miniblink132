...
public:
    ...
    {
        throbber_ = diameter ? AddChildView(std::make_unique<Throbber>(diameter.value())) : AddChildView(std::make_unique<Throbber>());
        throbber_->Start();
    ...
    {
        return gfx::Size(available_size.width().value_or(width()), available_size.height().value_or(height()));
    }
    ...
    ProposedLayout CalculateProposedLayout(const SizeBounds& size_bounds) const override
    {
        ...
        layout.child_layouts.emplace_back(throbber_.get(), throbber_->GetVisible(),
            gfx::Rect((layout.host_size.width() - diameter) / 2, (layout.host_size.height() - diameter) / 2, diameter, diameter));
        return layout;
    }
