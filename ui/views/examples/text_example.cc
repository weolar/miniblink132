...
public:
    ...
    void OnPaint(gfx::Canvas* canvas) override
    {
        ...
        const SkColor color = GetColorProvider()->GetColor(ui::kColorTextfieldForeground);
    ...
    {
        return font_list_.GetFontWeight();
    }
...
{
    return parent->AddChildView(std::make_unique<Checkbox>(base::ASCIIToUTF16(name), base::BindRepeating(&TextExample::UpdateStyle, base::Unretained(this))));
}

Combobox* TextExample::AddCombobox(View* parent, std::u16string name, base::span<const char* const> items, void (TextExample::*combobox_callback)())
{
    ...
    combobox->SetCallback(base::BindRepeating(combobox_callback, base::Unretained(this)));
...
{
    auto* box_layout = container->SetLayoutManager(std::make_unique<BoxLayout>(BoxLayout::Orientation::kVertical, gfx::Insets(8), 20, false));
    auto* table_container = container->AddChildView(std::make_unique<View>());
    TableLayout* layout = table_container->SetLayoutManager(std::make_unique<TableLayout>());
    layout->AddColumn(LayoutAlignment::kStart, LayoutAlignment::kStretch, 0.1f, TableLayout::ColumnSize::kUsePreferred, 0, 0);
    for (int i = 0; i < kNumColumns - 1; i++) {
        layout->AddColumn(LayoutAlignment::kStretch, LayoutAlignment::kStretch, 0.1f, TableLayout::ColumnSize::kUsePreferred, 0, 0);
    }
    ...
    h_align_cb_ = AddCombobox(table_container, u"H-Align", kHorizontalAligments, &TextExample::AlignComboboxChanged);

    constexpr auto kElideBehaviors = std::to_array<const char* const>({ "Elide", "No Elide" });
    eliding_cb_ = AddCombobox(table_container, u"Eliding", kElideBehaviors, &TextExample::ElideComboboxChanged);
    ...
    prefix_cb_ = AddCombobox(table_container, u"Prefix", kPrefixOptions, &TextExample::PrefixComboboxChanged);
    ...
    text_cb_ = AddCombobox(table_container, u"Example Text", kTextExamples, &TextExample::TextComboboxChanged);
    ...
    weight_cb_ = AddCombobox(table_container, u"Font Weight", kWeightLabels, &TextExample::WeightComboboxChanged);
    ...
    text_view_ = fill_container->AddChildView(std::make_unique<TextExampleView>());
...
void TextExample::UpdateStyle()
{
    ...
    SetFlagFromCheckbox(break_checkbox_, &flags, gfx::Canvas::CHARACTER_BREAKABLE);
...
{
    int flags = text_view_->GetFlags() & ~(gfx::Canvas::TEXT_ALIGN_LEFT | gfx::Canvas::TEXT_ALIGN_CENTER | gfx::Canvas::TEXT_ALIGN_RIGHT);
...
void TextExample::TextComboboxChanged()
{
    ...
    case 0:
        ...
        break;
    case 1:
        text_view_->SetText(u"Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do "
    ...
    case 2:
        ...
        break;
    case 3:
        text_view_->SetText(u"\x5e9\x5dc\x5d5\x5dd \x5d4\x5e2\x5d5\x5dc\x5dd! "
...
{
    int flags = text_view_->GetFlags() & ~(gfx::Canvas::SHOW_PREFIX | gfx::Canvas::HIDE_PREFIX);
