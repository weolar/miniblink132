...
public:
    VectorIconGallery()
    {
        ...
        BoxLayout* box = SetLayoutManager(std::make_unique<BoxLayout>(BoxLayout::Orientation::kVertical, gfx::Insets(10), 10));
        ...
        std::vector<std::u16string> icon_dir = {
            base::FilePath(test_dir.AppendASCII("ash").AppendASCII("resources").AppendASCII("vector_icons")).AsUTF16Unsafe(),
            base::FilePath(test_dir.AppendASCII("chrome").AppendASCII("app").AppendASCII("vector_icons")).AsUTF16Unsafe(),
            base::FilePath(test_dir.AppendASCII("chromeos").AppendASCII("ui").AppendASCII("vector_icons")).AsUTF16Unsafe(),
            base::FilePath(test_dir.AppendASCII("components").AppendASCII("vector_icons")).AsUTF16Unsafe(),
            base::FilePath(test_dir.AppendASCII("components").AppendASCII("omnibox").AppendASCII("browser").AppendASCII("vector_icons")).AsUTF16Unsafe(),
            base::FilePath(test_dir.AppendASCII("ui").AppendASCII("views").AppendASCII("vector_icons")).AsUTF16Unsafe(),
        };
        auto editable_combobox = std::make_unique<views::EditableCombobox>();
        editable_combobox->SetModel(std::make_unique<ui::SimpleComboboxModel>(std::vector<ui::SimpleComboboxModel::Item>(icon_dir.begin(), icon_dir.end())));
        editable_combobox->SetPlaceholderText(GetStringUTF16(IDS_VECTOR_FILE_SELECT_LABEL));
        editable_combobox->GetViewAccessibility().SetName(u"Editable Combobox");

        auto file_container = std::make_unique<View>();
        BoxLayout* file_box = file_container->SetLayoutManager(std::make_unique<BoxLayout>(BoxLayout::Orientation::kHorizontal, gfx::Insets(10), 10));
        file_chooser_combobox_ = file_container->AddChildView(std::move(editable_combobox));

        file_go_button_ = file_container->AddChildView(std::make_unique<MdTextButton>(
            base::BindRepeating(&VectorIconGallery::FileGoButtonPressed, base::Unretained(this)), GetStringUTF16(IDS_VECTOR_RENDER_LABEL)));
        file_box->SetFlexForView(file_chooser_combobox_, 1);
        AddChildView(std::move(file_container));

        size_input_->SetPlaceholderText(GetStringUTF16(IDS_VECTOR_DIP_SIZE_DESC_LABEL));
        size_input_->set_controller(this);
        color_input_->SetPlaceholderText(GetStringUTF16(IDS_VECTOR_COLOR_DESC_LABEL));
    ...
    void ContentsChanged(Textfield* sender, const std::u16string& new_contents) override
    {
...
private:
    ...
    void GenerateSingleIcon(const base::FilePath& path)
    {
        ...
        image_view_ = image_view_container_->AddChildView(std::make_unique<ImageView>());
        image_view_->SetBorder(CreateThemedSolidBorder(1, ExamplesColorIds::kColorVectorExampleImageBorder));

        auto image_layout = std::make_unique<BoxLayout>(BoxLayout::Orientation::kHorizontal);
        image_layout->set_cross_axis_alignment(BoxLayout::CrossAxisAlignment::kCenter);
        image_layout->set_main_axis_alignment(BoxLayout::MainAxisAlignment::kCenter);
    ...
    void GenerateAllIconInFolder(const base::FilePath& path)
    {
        ...
        views::TableLayout* layout = image_view_container_->SetLayoutManager(std::make_unique<views::TableLayout>());
        for (int i = 0; i < nCols; ++i) {
            layout->AddColumn(
                LayoutAlignment::kStretch, LayoutAlignment::kStretch, TableLayout::kFixedSize, TableLayout::ColumnSize::kFixed, kColumnWidth, kColumnWidth);
        }
        ...
        {
            base::ScopedAllowBlockingForTesting allow_blocking;
            base::FileEnumerator file_iter(path, false, base::FileEnumerator::FILES, FILE_PATH_LITERAL("*.icon"));
            std::vector<base::FilePath> files;
            for (base::FilePath input_file = file_iter.Next(); !input_file.empty(); input_file = file_iter.Next()) {
            ...
            for (base::FilePath file : files) {
                ...
                ImageView* icon_view = image_view_container_->AddChildView(std::make_unique<ImageView>());
                icon_view->SetImage(ui::ImageModel::FromImageSkia(gfx::CreateVectorIconFromSource(CleanUpContents(file_content), size_, color_)));
    ...
    void Update()
    {
        if (!contents_.empty() && image_view_ != nullptr) {
            image_view_->SetImage(ui::ImageModel::FromImageSkia(gfx::CreateVectorIconFromSource(contents_, size_, color_)));
        }
        InvalidateLayout();
    }
    ...
    std::string CleanUpContents(const std::string& file_content)
    {
        ...
        for (size_t slashes = output.find("//"); slashes != std::string::npos; slashes = output.find("//")) {
