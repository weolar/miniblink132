...
ExampleVector GetExamplesToShow(ExampleVector examples)
{
    ...
    std::string enable_examples = command_line->GetSwitchValueASCII(kEnableExamples);

    if (!enable_examples.empty()) {
        // Filter examples to show based on the command line switch.
        StringVector enabled = base::SplitString(enable_examples, ";,", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);

        // Transform list of examples to just the list of names.
        StringVector example_names;
        base::ranges::transform(examples, std::back_inserter(example_names), &ExampleBase::example_title);

        base::ranges::sort(enabled);

        // Get an intersection of list of titles between the full list and the list
        // from the command-line.
        StringVector valid_examples = base::STLSetIntersection<StringVector>(enabled, example_names);

        // If there are still example names in the list, only include the examples
        // from the list.
        if (!valid_examples.empty()) {
            std::erase_if(examples, [valid_examples](auto& example) { return !base::Contains(valid_examples, example->example_title()); });
        }
        ...
        std::cout << "You may want to specify the example(s) you want to run:" << titles;
    }
    return examples;
}
...
public:
        ...
        : on_close_(std::move(on_close))
    {
        ...
        auto* layout = SetLayoutManager(std::make_unique<views::BoxLayout>(BoxLayout::Orientation::kVertical, gfx::Insets(0)));

        auto tabbed_pane = std::make_unique<TabbedPane>(TabbedPane::Orientation::kVertical, TabbedPane::TabStripStyle::kBorder, true);
    ...
    static ExamplesWindowContents* instance()
    {
        return instance_;
    }
...
private:
    ...
    gfx::Size CalculatePreferredSize(const SizeBounds& /*available_size*/) const override
    {
        ...
        for (size_t i = 0; i < tabbed_pane_->GetTabCount(); i++) {
            size.set_height(std::max(size.height(), tabbed_pane_->GetTabAt(i)->contents()->GetHeightForWidth(800)));
        }
        return size;
    }
    ...
    void CreateSidePanel(ExampleVector examples)
    {
        for (auto& example : examples) {
            ...
            example->SetContainer(tabbed_pane_->AddTab(base::UTF8ToUTF16(example->example_title()), std::move(tab_contents)));
        }
...
{
    return ExamplesWindowContents::instance() ? ExamplesWindowContents::instance()->GetWidget() : nullptr;
}

void ShowExamplesWindow(base::OnceClosure on_close, ExampleVector examples, gfx::NativeWindow window_context)
{
    if (ExamplesWindowContents::instance()) {
        ...
        Widget::InitParams params(Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
        params.delegate = new ExamplesWindowContents(std::move(on_close), std::move(examples));
