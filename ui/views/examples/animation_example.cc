...
private:
    ...
    gfx::FontList font_list_ = TypographyProvider::Get().GetFont(style::CONTEXT_DIALOG_TITLE, style::STYLE_PRIMARY);
};
...
void AnimatingSquare::OnPaint(gfx::Canvas* canvas)
{
    ...
    const SkColor4f colors[2] = { SkColor4f::FromColor(color), SkColor4f::FromColor(color_utils::HSLShift(color, { -1.0, -1.0, 0.75 })) };
    cc::PaintFlags flags;
    ...
    flags.setShader(
        cc::PaintShader::MakeRadialGradient(SkPoint::Make(bounds.centerX(), bounds.centerY()), bounds.width() / 2, colors, nullptr, 2, SkTileMode::kClamp));
    canvas->DrawRect(gfx::ToEnclosingRect(local_bounds_f), flags);
    int width = 0;
    int height = 0;
    std::u16string counter = base::NumberToString16(++paint_counter_);
    canvas->SizeStringInt(counter, font_list_, &width, &height, 0, gfx::Canvas::TEXT_ALIGN_CENTER);
    local_bounds.ClampToCenteredSize(gfx::Size(width, height));
    canvas->DrawStringRectWithFlags(
        counter, font_list_, GetColorProvider()->GetColor(ExamplesColorIds::kColorAnimationExampleForeground), local_bounds, gfx::Canvas::TEXT_ALIGN_CENTER);
}
...
ProposedLayout SquaresLayoutManager::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
    ...
    const int bounds_width = std::max(kPadding + item_width, size_bounds.width().min_of(max_width));
    ...
    for (size_t i = 0; i < children.size(); ++i) {
        ...
        const gfx::Point origin(kPadding + column * item_width, kPadding + row * item_height);
        layout.child_layouts.push_back({ children[i].get(), true, gfx::Rect(origin, kSize), SizeBounds(kSize) });
    }

    const size_t num_rows = (children.size() + views_per_row - 1) / views_per_row;
    const int max_height = kPadding + (num_rows * item_height);
    const int bounds_height = std::max(kPadding + item_height, size_bounds.height().min_of(max_height));
...
void SquaresLayoutManager::LayoutImpl()
{
    ...
    for (auto child_layout : proposed_layout.child_layouts) {
        bounds_animator_->AnimateViewTo(child_layout.child_view, child_layout.bounds);
    }
}
...
void AnimationExample::CreateExampleView(View* container)
{
    ...
    squares_container->SetBackground(CreateThemedSolidBackground(ExamplesColorIds::kColorAnimationExampleBackground));
...

    {
        ...
        for (views::View* view : squares_container->children()) {
            b.Once().SetDuration(base::Seconds(10)).SetRoundedCorners(view, rounded_corners);
            b.Repeatedly()
    ...
    container->AddChildView(
        std::make_unique<MdTextButton>(base::BindRepeating([](std::unique_ptr<AnimationAbortHandle>* abort_handle) { abort_handle->reset(); }, &abort_handle_),
