...
FadingView::FadingView()
{
    Builder<FadingView>(this)
        ...
        .AddChildren(Builder<BoxLayoutView>()
                         .CopyAddressTo(&primary_view_)
                         .SetBorder(CreateThemedRoundedRectBorder(1, kCornerRadius, ExamplesColorIds::kColorFadeAnimationExampleBorder))
                         .SetBackground(CreateThemedRoundedRectBackground(ExamplesColorIds::kColorFadeAnimationExampleBackground, kCornerRadius, 1))
                         .SetPaintToLayer()
            ...
            Builder<BoxLayoutView>()
                ...
                .AddChild(Builder<ThemedLabel>()
                              ...
                              .SetTextStyle(style::STYLE_PRIMARY)
                              .SetVerticalAlignment(gfx::ALIGN_MIDDLE)
                              .SetEnabledColorId(ExamplesColorIds::kColorFadeAnimationExampleForeground)))
        .BuildChildren();
    primary_view_->layer()->SetRoundedCornerRadius(gfx::RoundedCornersF(kCornerRadiusF));
    secondary_view_->layer()->SetRoundedCornerRadius(gfx::RoundedCornersF(kCornerRadiusF));
...
ProposedLayout CenteringLayoutManager::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
    ProposedLayout layout;
    const auto& children = host_view()->children();

    gfx::Rect host_bounds(size_bounds.width().min_of(host_view()->width()), size_bounds.height().min_of(host_view()->height()));
    for (views::View* child : children) {
        ...
        layout.child_layouts.push_back({ child, true, child_bounds, SizeBounds(preferred_size) });
    }
...
{
    container->SetBackground(CreateThemedSolidBackground(ExamplesColorIds::kColorFadeAnimationExampleBackground));
