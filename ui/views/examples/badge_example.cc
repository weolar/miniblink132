...
void BadgeExample::CreateExampleView(View* container)
{
    ...
    auto show_menu = [](BadgeExample* example) {
        ...
        auto menu_item_view = std::make_unique<views::MenuItemView>(&example->menu_delegate_);
        ...
        menu_item_view->AppendMenuItem(3, GetStringUTF16(IDS_BADGE_MENU_ITEM_3))->set_is_new(true);

        example->menu_runner_ = std::make_unique<MenuRunner>(std::move(menu_item_view), 0);
        ...
        example->menu_runner_->RunMenuAt(menu_button->GetWidget(), nullptr, bounds, MenuAnchorPosition::kTopLeft, ui::mojom::MenuSourceType::kNone);
    };
    ...
    auto view = Builder<BoxLayoutView>()
                    ...
                    .AddChildren(Builder<BoxLayoutView>().SetBetweenChildSpacing(10).AddChildren(Builder<Link>().SetText(GetStringUTF16(IDS_BADGE_LINK_TEXT)),
                                     Builder<Badge>().SetText(GetStringUTF16(IDS_BADGE_BADGE_TEXT))),
                        Builder<MdTextButton>()
                            .CopyAddressTo(&menu_button_)
                            .SetText(GetStringUTF16(IDS_BADGE_MENU_BUTTON))
                            .SetCallback(base::BindRepeating(show_menu, base::Unretained(this))))
                    .Build();
