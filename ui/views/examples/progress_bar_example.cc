...
void ProgressBarExample::CreateExampleView(View* container)
{
    container->SetLayoutManager(std::make_unique<views::TableLayout>())
        ->AddColumn(LayoutAlignment::kEnd, LayoutAlignment::kCenter, TableLayout::kFixedSize, TableLayout::ColumnSize::kUsePreferred, 0, 0)
        .AddPaddingColumn(0, 8)
        .AddColumn(LayoutAlignment::kStretch, LayoutAlignment::kCenter, 1.0f, TableLayout::ColumnSize::kFixed, 200, 0)
        .AddPaddingColumn(0, 8)
    ...
    container->AddChildView(std::make_unique<views::MdTextButton>(base::BindRepeating(&ProgressBarExample::ButtonPressed, base::Unretained(this), -0.1), u"-"));
    progress_bar_ = container->AddChildView(std::make_unique<ProgressBar>());
    container->AddChildView(std::make_unique<views::MdTextButton>(base::BindRepeating(&ProgressBarExample::ButtonPressed, base::Unretained(this), 0.1), u"+"));

    container->AddChildView(std::make_unique<Label>(GetStringUTF16(IDS_PROGRESS_LOADER_LABEL)));
    container->AddChildView(std::make_unique<ProgressBar>())->SetValue(-1);
    container->AddChildView(std::make_unique<View>());

    container->AddChildView(std::make_unique<Label>(GetStringUTF16(IDS_PROGRESS_LOADER_SHORT_LABEL)));
