...
public:
    ...
    {
        ++observed_event_count_;
    }

    size_t observed_event_count() const
    {
        return observed_event_count_;
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        widget_->Show();
        generator_ = std::make_unique<ui::test::EventGenerator>(GetContext(), widget_->GetNativeWindow());
...
TEST_F(EventMonitorTest, ShouldReceiveAppEventsWhileInstalled)
{
    std::unique_ptr<EventMonitor> monitor(
        EventMonitor::CreateApplicationMonitor(&observer_, widget_->GetNativeWindow(), { ui::EventType::kMousePressed, ui::EventType::kMouseReleased }));
...
{
    std::unique_ptr<EventMonitor> monitor(
        EventMonitor::CreateWindowMonitor(&observer_, widget_->GetNativeWindow(), { ui::EventType::kMousePressed, ui::EventType::kMouseReleased }));
...
TEST_F(EventMonitorTest, ShouldNotReceiveEventsFromOtherWindow)
{
    ...
    std::unique_ptr<EventMonitor> monitor(
        EventMonitor::CreateWindowMonitor(&observer_, widget2->GetNativeWindow(), { ui::EventType::kMousePressed, ui::EventType::kMouseReleased }));
...
TEST_F(EventMonitorTest, ShouldOnlyReceiveRequestedEventTypes)
{
    ...
    std::unique_ptr<EventMonitor> monitor(EventMonitor::CreateWindowMonitor(&observer_, widget_->GetNativeWindow(), { ui::EventType::kMousePressed }));
...
TEST_F(EventMonitorTest, WindowMonitorTornDownOnWindowClose)
{
    ...
    std::unique_ptr<EventMonitor> monitor(EventMonitor::CreateWindowMonitor(&observer_, widget2->GetNativeWindow(), { ui::EventType::kMousePressed }));
...
public:
    ...
    {
        monitor_ = EventMonitor::CreateApplicationMonitor(this, context, { ui::EventType::kMousePressed, ui::EventType::kMouseReleased });
    }

    DeleteOtherOnEventObserver(const DeleteOtherOnEventObserver&) = delete;
    DeleteOtherOnEventObserver& operator=(const DeleteOtherOnEventObserver&) = delete;

    bool DidDelete() const
    {
        return !observer_to_delete_;
    }

    void set_monitor_to_delete(std::unique_ptr<DeleteOtherOnEventObserver> observer_to_delete)
    {
