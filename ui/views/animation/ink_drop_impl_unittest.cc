...
public:
    explicit InkDropImplTest(InkDropImpl::AutoHighlightMode auto_highlight_mode = InkDropImpl::AutoHighlightMode::NONE)
    ...
    void SetUp() override
    {
        ...
        widget_->SetContentsView(std::make_unique<TestInkDropHost>(auto_highlight_mode_));
...
protected:
    ...
    {
        return static_cast<InkDropImpl*>(InkDrop::Get(ink_drop_host())->GetInkDrop());
    }
...
private:
    ...
    gfx::AnimationTestApi::RenderModeResetter animation_mode_reset_
        = gfx::AnimationTestApi::SetRichAnimationRenderMode(gfx::Animation::RichAnimationRenderMode::FORCE_DISABLED);
};

// AutoHighlightMode parameterized test fixture.
class InkDropImplAutoHighlightTest : public InkDropImplTest, public testing::WithParamInterface<testing::tuple<InkDropImpl::AutoHighlightMode>> {
public:
    InkDropImplAutoHighlightTest();

    InkDropImplAutoHighlightTest(const InkDropImplAutoHighlightTest&) = delete;
    InkDropImplAutoHighlightTest& operator=(const InkDropImplAutoHighlightTest&) = delete;
...
TEST_F(InkDropImplTest, VerifyInkDropLayersRemovedWhenPresentDuringDestruction)
{
    test_api().SetShouldHighlight(true);
...
TEST_F(InkDropImplTest, SettingHighlightStateDuringStateExitIsntAllowedDeathTest)
{
    GTEST_FLAG_SET(death_test_style, "threadsafe");

    test::InkDropImplTestApi::SetStateOnExitHighlightState::Install(test_api().state_factory());
    EXPECT_DCHECK_DEATH(test::InkDropImplTestApi::AccessFactoryOnExitHighlightState::Install(test_api().state_factory()));
...
TEST_F(InkDropImplTest, TearingDownHighlightStateThatAccessesTheStateFactoryIsSafe)
{
    test::InkDropImplTestApi::AccessFactoryOnExitHighlightState::Install(test_api().state_factory());
    test::InkDropImplTestApi::AccessFactoryOnExitHighlightState::Install(test_api().state_factory());
}
...
TEST_F(InkDropImplTest, SuccessfulAnimationEndedDuringDestruction)
{
    ...
    ink_drop()->AnimateToState(InkDropState::ACTION_PENDING);
    {
        ...
        ui::ScopedAnimationDurationScaleMode duration_mode(ui::ScopedAnimationDurationScaleMode::ZERO_DURATION);
...
TEST_F(InkDropImplTest, HostTracksHighlightState)
{
    ...
    auto subscription
        = InkDrop::Get(ink_drop_host())->AddHighlightedChangedCallback(base::BindRepeating([](bool* called) { *called = true; }, &callback_called));
...
INSTANTIATE_TEST_SUITE_P(All, InkDropImplCommonAutoHighlightTest,
    testing::Values(InkDropImpl::AutoHighlightMode::NONE, InkDropImpl::AutoHighlightMode::HIDE_ON_RIPPLE, InkDropImpl::AutoHighlightMode::SHOW_ON_RIPPLE));

// Verifies InkDropImplTestApi::SetShouldHighlight() works as expected.
TEST_P(InkDropImplCommonAutoHighlightTest, ShouldHighlightCausesHighlightToBeVisible)
{
    test_api().SetShouldHighlight(true);
...
TEST_P(InkDropImplCommonAutoHighlightTest, HighlightVisibilityForFocusAndHoverStates)
{
    ink_drop()->SetShowHighlightOnHover(true);
    ink_drop()->SetShowHighlightOnFocus(true);
...
// Note: First argument is optional and intentionally left blank.
// (it's a prefix for the generated test cases)
INSTANTIATE_TEST_SUITE_P(All, InkDropImplNoAutoHighlightTest, testing::Values(InkDropImpl::AutoHighlightMode::NONE));
...
// Note: First argument is optional and intentionally left blank.
// (it's a prefix for the generated test cases)
INSTANTIATE_TEST_SUITE_P(All, InkDropImplHideAutoHighlightTest, testing::Values(InkDropImpl::AutoHighlightMode::HIDE_ON_RIPPLE));

TEST_P(InkDropImplHideAutoHighlightTest, VisibleHighlightDuringRippleAnimations)
{
    test_api().SetShouldHighlight(true);
...
TEST_P(InkDropImplHideAutoHighlightTest, HiddenHighlightDuringRippleAnimations)
{
    test_api().SetShouldHighlight(false);
...
TEST_P(InkDropImplHideAutoHighlightTest, HighlightDoesntFadeInAfterAnimationIfHighlightNotSet)
{
    test_api().SetShouldHighlight(true);
...
TEST_P(InkDropImplHideAutoHighlightTest, HighlightFadesInAfterAnimationIfHovered)
{
    ink_drop()->SetShowHighlightOnHover(true);
    ink_drop()->SetHovered(true);
...
TEST_P(InkDropImplHideAutoHighlightTest, HighlightSnapsInAfterAnimationWhenHostIsFocused)
{
    ink_drop()->SetShowHighlightOnFocus(true);
    ink_drop()->SetFocused(true);
...
TEST_P(InkDropImplHideAutoHighlightTest, DeactivatedAnimationSkippedWhenFocused)
{
    ink_drop()->SetShowHighlightOnFocus(true);
    ink_drop()->SetFocused(true);
...
TEST_P(InkDropImplHideAutoHighlightTest, FocusAndHoverChangesDontShowHighlightWhenRippleIsVisible)
{
    test_api().SetShouldHighlight(true);
    ink_drop()->AnimateToState(InkDropState::ACTION_PENDING);
    test_api().CompleteAnimations();
...
// Note: First argument is optional and intentionally left blank.
// (it's a prefix for the generated test cases)
INSTANTIATE_TEST_SUITE_P(All, InkDropImplShowAutoHighlightTest, testing::Values(InkDropImpl::AutoHighlightMode::SHOW_ON_RIPPLE));

TEST_P(InkDropImplShowAutoHighlightTest, VisibleHighlightDuringRippleAnimations)
{
    test_api().SetShouldHighlight(true);
...
TEST_P(InkDropImplShowAutoHighlightTest, HiddenHighlightDuringRippleAnimations)
{
    test_api().SetShouldHighlight(false);
...
TEST_P(InkDropImplShowAutoHighlightTest, FocusAndHoverChangesDontHideHighlightWhenRippleIsVisible)
{
    test_api().SetShouldHighlight(true);
    ink_drop()->AnimateToState(InkDropState::ACTION_PENDING);
    test_api().CompleteAnimations();
