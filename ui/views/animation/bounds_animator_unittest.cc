...
public:
    ...
    const gfx::Rect& dirty_rect() const
    {
        return dirty_rect_;
    }

    void set_repaint_count(int val)
    {
        repaint_count_ = val;
    }
    int repaint_count() const
    {
        return repaint_count_;
    }
...
public:
    RTLAnimationTestDelegate(const gfx::Rect& start, const gfx::Rect& target, View* view, base::RepeatingClosure quit_closure)
        : start_(start)
        , target_(target)
        , view_(view)
...
private:
    ...
    void AnimationProgressed(const Animation* animation) override
    {
        ...
        gfx::Rect current_bounds_in_screen = transform.MapRect(parent->GetMirroredRect(view_->bounds()));

        // Verify that |view_|'s current bounds in screen are valid.
        EXPECT_GE(current_bounds_in_screen.x(), std::min(start_rect_in_screen.x(), target_rect_in_screen.x()));
        EXPECT_LE(current_bounds_in_screen.right(), std::max(start_rect_in_screen.right(), target_rect_in_screen.right()));
...
public:
    BoundsAnimatorTest()
        : task_environment_(base::test::TaskEnvironment::TimeSource::MOCK_TIME, base::test::SingleThreadTaskEnvironment::MainThreadType::UI)
    {
...
protected:
    ...
    // created.
    int GetRepaintTimeFromBoundsAnimation(const gfx::Rect& target_bounds, bool use_long_duration)
    {
        base::RunLoop loop;
        child()->set_repaint_count(0);

        const base::TimeDelta animation_duration = base::Milliseconds(use_long_duration ? 2000 : 10);
        animator()->SetAnimationDuration(animation_duration);

        animator()->AnimateViewTo(child(), target_bounds);
        animator()->SetAnimationDelegate(child(), std::make_unique<TestAnimationDelegate>(loop.QuitWhenIdleClosure()));
...
TEST_F(BoundsAnimatorTest, AnimateViewTo)
{
    ...
    EXPECT_EQ(gfx::UnionRects(target_bounds, initial_bounds), parent()->dirty_rect());
}
...
TEST_F(BoundsAnimatorTest, DuplicateAnimationsCanReplaceDelegate)
{
    ...
    // delegate.
    animator()->AnimateViewTo(child(), target_bounds, std::make_unique<OwnedDelegate>());
...
TEST_F(BoundsAnimatorTest, UseTransformsAnimateViewTo)
{
    ...
    const gfx::Rect target_bounds_without_resize(gfx::Point(10, 10), initial_bounds.size());

    const int repaint_time_from_short_animation = GetRepaintTimeFromBoundsAnimation(target_bounds_without_resize,
        /*use_long_duration=*/false);
    const int repaint_time_from_long_animation = GetRepaintTimeFromBoundsAnimation(initial_bounds,
        /*use_long_duration=*/true);

    // The number of repaints in long animation should be the same as with the
    // short animation.
    EXPECT_EQ(repaint_time_from_short_animation, repaint_time_from_long_animation);
}
...
TEST_F(BoundsAnimatorTest, NoTransformForScalingAnimation)
{
    ...
    const gfx::Rect target_bounds_with_reize(gfx::Point(10, 10), gfx::Size(20, 20));

    const int repaint_time_from_short_animation = GetRepaintTimeFromBoundsAnimation(target_bounds_with_reize,
        /*use_long_duration=*/false);
    const int repaint_time_from_long_animation = GetRepaintTimeFromBoundsAnimation(initial_bounds,
        /*use_long_duration=*/true);
    ...
    EXPECT_GT(repaint_time_from_long_animation, repaint_time_from_short_animation);
}
...
TEST_F(BoundsAnimatorTest, UseTransformsCancelAnimation)
{
    ...
    animator()->AnimateViewTo(child(), target_bounds);
    animator()->SetAnimationDelegate(child(), std::make_unique<TestAnimationDelegate>());
    EXPECT_TRUE(animator()->IsAnimating());
    EXPECT_TRUE(animator()->IsAnimating(child()));
...
TEST_F(BoundsAnimatorTest, VerifyBoundsAnimatorUnderRTL)
{
    ...
    animator()->SetAnimationDelegate(child(), std::make_unique<RTLAnimationTestDelegate>(initial_bounds, target_bounds, child(), run_loop.QuitClosure()));
