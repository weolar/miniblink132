...
WidgetFadeAnimator::WidgetFadeAnimator(Widget* widget)
    : AnimationDelegateViews(widget->GetRootView())
    , widget_(widget)
{
...
base::CallbackListSubscription WidgetFadeAnimator::AddFadeCompleteCallback(FadeCompleteCallback callback)
{
...
void WidgetFadeAnimator::AnimationProgressed(const gfx::Animation* animation)
{
    ...
    double value = gfx::Tween::CalculateValue(tween_type_, animation->GetCurrentValue());
    float opacity = 0.0f;
    if (IsFadingOut())
