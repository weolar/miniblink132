...
public:
    ...
    ui::EventHandler* GetTargetHandler()
    {
        return target_handler();
    }

    TestInkDrop* last_created_inkdrop() const
    {
        return last_created_inkdrop_;
    }
    void ClearLastCreatedInkDrop()
    {
        last_created_inkdrop_ = nullptr;
    }
...
InkDropHostTest::InkDropHostTest()
    ...
    , animation_mode_reset_(gfx::AnimationTestApi::SetRichAnimationRenderMode(gfx::Animation::RichAnimationRenderMode::FORCE_DISABLED))
{
}

InkDropHostTest::~InkDropHostTest() = default;

void InkDropHostTest::MouseEventTriggersInkDropHelper(InkDropMode ink_drop_mode)
{
    ...
    ui::MouseEvent mouse_event(ui::EventType::kMouseEntered, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), ui::EF_IS_SYNTHESIZED, 0);
...
TEST_F(InkDropHostTest, GetInkDropCenterBasedOnLastEventForNullEvent)
{
    ...
    EXPECT_EQ(gfx::Point(10, 10), InkDrop::Get(&host_view_)->GetInkDropCenterBasedOnLastEvent());
}
...
TEST_F(InkDropHostTest, GetInkDropCenterBasedOnLastEventForLocatedEvent)
{
    ...
    ui::MouseEvent located_event(ui::EventType::kMousePressed, gfx::Point(5, 6), gfx::Point(5, 6), ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, 0);

    test_api_.AnimateToState(InkDropState::ACTION_PENDING, &located_event);
    EXPECT_EQ(gfx::Point(5, 6), InkDrop::Get(&host_view_)->GetInkDropCenterBasedOnLastEvent());
}
...
TEST_F(InkDropHostTest, MouseEventsTriggerInkDropsWhenInkDropIsOnNoGestureHandler)
{
    MouseEventTriggersInkDropHelper(views::InkDropHost::InkDropMode::ON_NO_GESTURE_HANDLER);
}
...
TEST_F(InkDropHostTest, GestureEventsDontTriggerInkDropsWhenHostIsDisabled)
{
    ...
    ui::GestureEvent gesture_event(0.f, 0.f, 0, ui::EventTimeForNow(), ui::GestureEventDetails(ui::EventType::kGestureTapDown));

    host_view_.GetTargetHandler()->OnEvent(&gesture_event);

    EXPECT_EQ(test_api_.GetInkDrop()->GetTargetInkDropState(), InkDropState::HIDDEN);
}

// Verifies that ink drops are not triggered by gesture events when ink drop
// mode is ON_NO_GESTURE_EVENT or OFF.
TEST_F(InkDropHostTest, GestureEventsDontTriggerInkDropsWhenInkDropModeIsNotOn)
{
    for (auto ink_drop_mode : { views::InkDropHost::InkDropMode::ON_NO_GESTURE_HANDLER, views::InkDropHost::InkDropMode::OFF }) {
        test_api_.SetInkDropMode(ink_drop_mode);
        ui::GestureEvent gesture_event(0.f, 0.f, 0, ui::EventTimeForNow(), ui::GestureEventDetails(ui::EventType::kGestureTapDown));
...
TEST_F(InkDropHostTest, NoInkDropOnTouchOrGestureEvents)
{
    ...
    test_api_.SetInkDropMode(views::InkDropHost::InkDropMode::ON_NO_GESTURE_HANDLER);

    // Ensure the target ink drop is in the expected state.
    EXPECT_EQ(test_api_.GetInkDrop()->GetTargetInkDropState(), InkDropState::HIDDEN);

    ui::TouchEvent touch_event(ui::EventType::kTouchPressed, gfx::Point(5, 6), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));

    test_api_.AnimateToState(InkDropState::ACTION_PENDING, &touch_event);
    EXPECT_EQ(test_api_.GetInkDrop()->GetTargetInkDropState(), InkDropState::HIDDEN);

    test_api_.AnimateToState(InkDropState::ALTERNATE_ACTION_PENDING, &touch_event);
    EXPECT_EQ(test_api_.GetInkDrop()->GetTargetInkDropState(), InkDropState::HIDDEN);

    ui::GestureEvent gesture_event(5.0f, 6.0f, 0, ui::EventTimeForNow(), ui::GestureEventDetails(ui::EventType::kGestureTap));

    test_api_.AnimateToState(InkDropState::ACTION_PENDING, &gesture_event);
    EXPECT_EQ(test_api_.GetInkDrop()->GetTargetInkDropState(), InkDropState::HIDDEN);

    test_api_.AnimateToState(InkDropState::ALTERNATE_ACTION_PENDING, &gesture_event);
    EXPECT_EQ(test_api_.GetInkDrop()->GetTargetInkDropState(), InkDropState::HIDDEN);
}
...
TEST_F(InkDropHostTest, DismissInkDropOnTouchOrGestureEvents)
{
    ...
    ui::MouseEvent mouse_event(ui::EventType::kMousePressed, gfx::Point(5, 6), gfx::Point(5, 6), ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, 0);

    test_api_.AnimateToState(InkDropState::ACTION_PENDING, &mouse_event);
    EXPECT_EQ(test_api_.GetInkDrop()->GetTargetInkDropState(), InkDropState::ACTION_PENDING);

    ui::TouchEvent touch_event(ui::EventType::kTouchPressed, gfx::Point(5, 6), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));

    test_api_.AnimateToState(InkDropState::ACTION_TRIGGERED, &touch_event);
    EXPECT_EQ(test_api_.GetInkDrop()->GetTargetInkDropState(), InkDropState::ACTION_TRIGGERED);
}
#endif
...
TEST_F(InkDropHostTest, HighlightedChangedFired)
{
    ...
    auto subscription = InkDrop::Get(&host_view_)->AddHighlightedChangedCallback(base::BindRepeating([](bool* called) { *called = true; }, &callback_called));
...
public:
        ...
        : host_view_test_api_(InkDrop::Get(&host_view_))
    {
        ...
        ink_drop_ = static_cast<InkDropImpl*>(InkDrop::Get(&host_view_)->GetInkDrop());
    ...
    {
        return ink_drop_test_api_->GetRootLayer();
    }
...
TEST_F(InkDropHostClippingTest, HighlightPathGeneratorClipsRootLayerWithoutMask)
{
...
TEST_F(InkDropHostClippingTest, EmptyHighlightPathGeneratorUsesNeitherMaskNorClipsRootLayer)
{
...
protected:
    void SetUp() override
    {
        ...
        view_ = widget_->SetContentsView(std::make_unique<BasicTestViewWithInkDrop>());
    }
    ...
    InkDropHost& ink_drop()
    {
        return *InkDrop::Get(view_);
    }
...
TEST_F(InkDropInWidgetTest, SetBaseColorId)
{
    ...
    EXPECT_EQ(ink_drop().GetBaseColor(), color_provider().GetColor(ui::kColorSeparator));
...
protected:
    void SetUp() override
    {
        ...
        InkDrop::Install(host_view_.get(), std::make_unique<InkDropHost>(host_view_.get()));
        ink_drop_host_ = InkDrop::Get(host_view_.get());
        widget_ = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
        ink_drop_host_test_api_ = std::make_unique<InkDropHostTestApi>(ink_drop_host_);
        ...
        // The root layer of the ink drop is created the first time GetInkDrop is
        // called and then kept alive until the host view is destroyed.
        ink_drop_ = static_cast<InkDropImpl*>(InkDrop::Get(host_view_.get())->GetInkDrop());
        ink_drop_test_api_ = std::make_unique<test::InkDropImplTestApi>(ink_drop_);
...
TEST_F(InkDropHostAttentionTest, ToggleAttentionColor)
{
    ...
    EXPECT_EQ(ink_drop_host()->GetBaseColor(), color_provider().GetColor(ui::kColorSeparator));

    // Flipping attention state to true triggers attention color.
    ink_drop_host()->ToggleAttentionState(true);
    EXPECT_EQ(ink_drop_host()->GetBaseColor(), color_provider().GetColor(ui::kColorButtonFeatureAttentionHighlight));
