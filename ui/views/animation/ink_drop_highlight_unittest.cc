...
protected:
    ...
    {
        return ink_drop_highlight_.get();
    }
    ...
    TestInkDropHighlightObserver* observer()
    {
        return &observer_;
    }
...
{
    InitHighlight(std::make_unique<InkDropHighlight>(gfx::Size(10, 10), 3, gfx::PointF(), SK_ColorBLACK));
}
...
void InkDropHighlightTest::InitHighlight(std::unique_ptr<InkDropHighlight> new_highlight)
{
...
TEST_F(InkDropHighlightTest, VerifyObserversAreNotifiedWithCorrectAnimationType)
{
    ink_drop_highlight()->FadeIn(base::Seconds(1));

    EXPECT_TRUE(observer()->AnimationHasStarted());
    EXPECT_EQ(InkDropHighlight::AnimationType::kFadeIn, observer()->last_animation_started_context());
    ...
    ink_drop_highlight()->FadeOut(base::Seconds(1));
    EXPECT_EQ(InkDropHighlight::AnimationType::kFadeOut, observer()->last_animation_started_context());
...
TEST_F(InkDropHighlightTest, VerifyObserversAreNotifiedOfSuccessfulAnimations)
{
    ...
    EXPECT_EQ(2, observer()->last_animation_ended_ordinal());
    EXPECT_EQ(InkDropAnimationEndedReason::SUCCESS, observer()->last_animation_ended_reason());
}
...
TEST_F(InkDropHighlightTest, VerifyObserversAreNotifiedOfPreemptedAnimations)
{
    ...
    EXPECT_EQ(2, observer()->last_animation_ended_ordinal());
    EXPECT_EQ(InkDropHighlight::AnimationType::kFadeIn, observer()->last_animation_ended_context());
...
{
    InitHighlight(std::make_unique<InkDropHighlight>(gfx::Size(0, 0), 3, gfx::PointF(), SK_ColorBLACK));
    ink_drop_highlight()->FadeOut(base::Milliseconds(0));
}
...
TEST_F(InkDropHighlightTest, TransformIsPixelAligned)
{
    ...
    InitHighlight(std::make_unique<InkDropHighlight>(kHighlightSize, 3, gfx::PointF(3.5f, 3.5f), SK_ColorYELLOW));
    for (auto dsf : { 1.25, 1.33, 1.5, 1.6, 1.75, 1.8, 2.25 }) {
        ...
        gfx::PointF transformed_layer_origin = transform.MapPoint(gfx::PointF(ink_drop_highlight()->layer()->bounds().origin()));
        ...
        EXPECT_NEAR(transformed_layer_origin.x(), std::round(transformed_layer_origin.x()), kEpsilon);
        EXPECT_NEAR(transformed_layer_origin.y(), std::round(transformed_layer_origin.y()), kEpsilon);
    }
}
