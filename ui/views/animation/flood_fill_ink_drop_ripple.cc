...
gfx::Rect CalculateClipBounds(const gfx::Size& host_size, const gfx::Insets& clip_insets)
{
...
FloodFillInkDropRipple::FloodFillInkDropRipple(InkDropHost* ink_drop_host, const gfx::Size& host_size, const gfx::Insets& clip_insets,
    const gfx::Point& center_point, SkColor color, float visible_opacity)
    ...
    , circle_layer_delegate_(color, CalculateCircleLayerRadius(CalculateClipBounds(host_size, clip_insets)))
{
    ...
    root_callback_subscription_ = root_layer_.GetAnimator()->AddSequenceScheduledCallback(
        base::BindRepeating(&FloodFillInkDropRipple::OnLayerAnimationSequenceScheduled, base::Unretained(this)));

    const int painted_size_length = std::max(clip_bounds.width(), clip_bounds.height());
    ...
    painted_layer_callback_subscription_ = painted_layer_.GetAnimator()->AddSequenceScheduledCallback(
        base::BindRepeating(&FloodFillInkDropRipple::OnLayerAnimationSequenceScheduled, base::Unretained(this)));
...
FloodFillInkDropRipple::FloodFillInkDropRipple(
    InkDropHost* ink_drop_host, const gfx::Size& host_size, const gfx::Point& center_point, SkColor color, float visible_opacity)
    : FloodFillInkDropRipple(ink_drop_host, host_size, gfx::Insets(), center_point, color, visible_opacity)
{
}
...
void FloodFillInkDropRipple::AnimateStateChange(InkDropState old_ink_drop_state, InkDropState new_ink_drop_state)
{
    ...
    case InkDropState::HIDDEN:
        if (!IsVisible()) {
            ...
            AnimationBuilder()
                ...
                .SetTransform(&painted_layer_, CalculateTransform(kMinRadius), gfx::Tween::EASE_IN_OUT);
        }
        break;
    case InkDropState::ACTION_PENDING: {
        ...
        AnimationBuilder()
            ...
            .SetTransform(&painted_layer_, GetMaxSizeTargetTransform(), gfx::Tween::FAST_OUT_SLOW_IN);
        break;
    }
    ...
    case InkDropState::ALTERNATE_ACTION_PENDING: {
        ...
        AnimationBuilder()
            ...
            .SetOpacity(&root_layer_, visible_opacity_, gfx::Tween::EASE_IN)
            .SetTransform(&painted_layer_, GetMaxSizeTargetTransform(), gfx::Tween::EASE_IN_OUT);
        break;
    }
    case InkDropState::ALTERNATE_ACTION_TRIGGERED:
        ...
        AnimationBuilder()
            .SetPreemptionStrategy(ui::LayerAnimator::ENQUEUE_NEW_ANIMATION)
            .Once()
            .SetDuration(GetAnimationDuration(ALTERNATE_ACTION_TRIGGERED_FADE_OUT))
            .SetOpacity(&root_layer_, kHiddenOpacity, gfx::Tween::EASE_IN_OUT);
        break;
...
gfx::Transform FloodFillInkDropRipple::CalculateTransform(float target_radius) const
{
    const float target_scale = target_radius / circle_layer_delegate_.radius();

    gfx::Transform transform = gfx::Transform();
    transform.Translate(center_point_.x() - root_layer_.bounds().x(), center_point_.y() - root_layer_.bounds().y());
    transform.Scale(target_scale, target_scale);

    const gfx::Vector2dF drawn_center_offset = circle_layer_delegate_.GetCenteringOffset();
    transform.Translate(-drawn_center_offset.x(), -drawn_center_offset.y());

    // Add subpixel correction to the transform.
    transform.PostConcat(GetTransformSubpixelCorrection(transform, painted_layer_.device_scale_factor()));
...
float FloodFillInkDropRipple::MaxDistanceToCorners(const gfx::Point& point) const
{
    ...
    const float distance_to_bottom_right = (bounds.bottom_right() - point).Length();

    float largest_distance = std::max(distance_to_top_left, distance_to_top_right);
...
base::TimeDelta FloodFillInkDropRipple::GetAnimationDuration(AnimationSubState state)
{
    if (!PlatformStyle::kUseRipples || !gfx::Animation::ShouldRenderRichAnimation()
        || (GetInkDropHost() && GetInkDropHost()->GetMode() == InkDropHost::InkDropMode::ON_NO_ANIMATE)) {
        return base::TimeDelta();
    }

    // Override the requested state if needed.
    if (use_hide_transform_duration_for_hide_fade_out_ && state == HIDDEN_FADE_OUT) {
...
void FloodFillInkDropRipple::OnLayerAnimationSequenceScheduled(ui::LayerAnimationSequence* sequence)
{
