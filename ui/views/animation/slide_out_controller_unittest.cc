...
public:
    explicit TestSlideOutControllerDelegate(View* target)
        : target_(target)
    {
    }
    ...
    {
        return target_->layer();
    }

    void OnSlideStarted() override
    {
        ++slide_started_count_;
    }
    ...
    bool IsOnSlideChangedCalled() const
    {
        return (slide_changed_count_ > 0);
    }
...
public:
    ...
    void SetUp() override
    {
        ...
        slide_out_controller_ = std::make_unique<SlideOutController>(target_, delegate_.get());
    }
...
protected:
    ...
    TestSlideOutControllerDelegate* delegate()
    {
    ...
    void PostSequentialGestureEvent(const ui::GestureEventDetails& details)
    {
        ...
        ui::GestureEvent gesture_event(0, 0, ui::EF_NONE, base::TimeTicks() + sequential_event_timestamp_, details);
        slide_out_controller()->OnGestureEvent(&gesture_event);
    }

    void PostSequentialSwipeEvent(int swipe_amount)
    {
        PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollBegin));
        PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollUpdate, swipe_amount, 0));
        PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollEnd));
    }

    void PostTrackPadSwipeEvent(ui::EventType type, int swipe_amount, int finger_count)
    {
        auto scroll_event = ui::ScrollEvent(type, gfx::PointF(), gfx::PointF(), base::TimeTicks(), 0, swipe_amount, 0, 0, 0, finger_count);
...
TEST_F(SlideOutControllerTest, OnGestureEventAndDelegate)
{
    ...
    PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollUpdate));
...
TEST_F(SlideOutControllerTest, SlideOutAndClose)
{
    ...
    // target width 200 px)
    PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollUpdate, kTargetWidth / 2 + 1, 0));
    ...
    // width (200px).
    EXPECT_EQ(kTargetWidth, delegate()->GetSlideOutLayer()->transform().To2dTranslation().x());
...
TEST_F(SlideOutControllerTest, SlideLittleAmountAndNotClose)
{
    ...
    // target width 200 px)
    PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollUpdate, 99, 0));
    ...
    EXPECT_EQ(0.f, delegate()->GetSlideOutLayer()->transform().To2dTranslation().x());
...
class SwipeControlTest : public SlideOutControllerTest, public testing::WithParamInterface<SlideOutController::SlideMode> {
public:
...
INSTANTIATE_TEST_SUITE_P(All, SwipeControlTest, ::testing::Values(SlideOutController::SlideMode::kFull, SlideOutController::SlideMode::kPartial));
...
TEST_P(SwipeControlTest, SetSwipeControlWidth_SwipeLessThanControlWidth)
{
    ...
    // control width).
    PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollUpdate, kSwipeControlWidth - 1, 0));
...
TEST_P(SwipeControlTest, SwipeControlWidth_SwipeMoreThanControlWidth)
{
    ...
    EXPECT_EQ(kSwipeControlWidth, delegate()->GetSlideOutLayer()->transform().To2dTranslation().x());
...
{
    const bool swipe_out_supported = slide_out_controller()->mode() == SlideOutController::SlideMode::kFull;
    ...
    EXPECT_EQ(0, delegate()->slide_started_count_);
    EXPECT_TRUE(delegate()->IsOnSlideChangedCalled());
    EXPECT_EQ(delegate()->slide_changed_last_value_.value(), !swipe_out_supported);
    EXPECT_EQ(0, delegate()->slide_out_count_);
    EXPECT_EQ(swipe_out_supported ? kTargetWidth : kSwipeControlWidth, delegate()->GetSlideOutLayer()->transform().To2dTranslation().x());
...
TEST_P(SwipeControlTest, SwipeControlWidth_SnapAndSwipeOut)
{
    ...
    PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollBegin));
    PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollUpdate, kSwipeControlWidth, 0));
    PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollEnd));
    EXPECT_EQ(1, delegate()->slide_started_count_);
    EXPECT_TRUE(delegate()->IsOnSlideChangedCalled());
    EXPECT_TRUE(delegate()->slide_changed_last_value_.value());
    ...
    PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollBegin));
    PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollUpdate, 70, 0));
    PostSequentialGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollEnd));
...
TEST_P(SwipeControlTest, SwipeControlWidth_SnapAndBackToOrigin)
{
    ...
    EXPECT_FALSE(delegate()->slide_changed_last_value_.value());
    EXPECT_EQ(0, delegate()->slide_out_count_);
    EXPECT_EQ(0, delegate()->GetSlideOutLayer()->transform().To2dTranslation().x());
