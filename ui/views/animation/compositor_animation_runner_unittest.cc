...
TEST_F(CompositorAnimationRunnerTest, BasicCoverageTest)
{
    ...
    gfx::LinearAnimation animation(kDuration, gfx::LinearAnimation::kDefaultFrameRate, &delegate);
    ...
    EXPECT_STREQ(base::Location::Current().file_name(), delegate.location_for_test().file_name());
}
...
TEST_F(CompositorAnimationRunnerTest, ThroughputTracker)
{
    ...
    ui::ThroughputTracker tracker1 = widget->GetCompositor()->RequestNewThroughputTracker();
    tracker1.Start(base::BindLambdaForTesting([&](const cc::FrameSequenceMetrics::CustomReportData& data) {
        ++report_count;
        run_loop.Quit();
    }));
    ...
    ui::ThroughputTracker tracker2 = widget->GetCompositor()->RequestNewThroughputTracker();
    tracker2.Start(base::BindLambdaForTesting([&](const cc::FrameSequenceMetrics::CustomReportData& data) {
        ++report_count2;
        run_loop2.Quit();
    }));
