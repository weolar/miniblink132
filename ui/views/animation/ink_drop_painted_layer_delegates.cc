...
void BasePaintedLayerDelegate::OnDeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor)
{
}
...
void CircleLayerDelegate::OnPaintLayer(const ui::PaintContext& context)
{
    ...
    ui::PaintRecorder recorder(context, gfx::ToEnclosingRect(GetPaintedBounds()).size());
...
RoundedRectangleLayerDelegate::RoundedRectangleLayerDelegate(SkColor color, const gfx::SizeF& size, int corner_radius)
    : BasePaintedLayerDelegate(color)
    , size_(size)
...
void RoundedRectangleLayerDelegate::OnPaintLayer(const ui::PaintContext& context)
{
    cc::PaintFlags flags;
    flags.setColor(color());
    flags.setAntiAlias(true);
    ...
    rect.Scale(dsf);
    recorder.canvas()->DrawRoundRect(gfx::ToEnclosingRect(rect), dsf * corner_radius_, flags);
}
...
BorderShadowLayerDelegate::BorderShadowLayerDelegate(
    const std::vector<gfx::ShadowValue>& shadows, const gfx::Rect& shadowed_area_bounds, SkColor fill_color, int corner_radius)
...
void BorderShadowLayerDelegate::OnPaintLayer(const ui::PaintContext& context)
{
    ...
    gfx::RectF rrect_bounds = gfx::RectF(bounds_) - GetPaintedBounds().OffsetFromOrigin();
    SkRRect r_rect = SkRRect::MakeRectXY(gfx::RectFToSkRect(rrect_bounds), corner_radius_, corner_radius_);

    // First the fill color.
    ui::PaintRecorder recorder(context, gfx::ToCeiledSize(GetPaintedBounds().size()));
    recorder.canvas()->sk_canvas()->drawRRect(r_rect, flags);

    // Now the shadow.
    flags.setLooper(gfx::CreateShadowDrawLooper(shadows_));
    recorder.canvas()->sk_canvas()->clipRRect(r_rect, SkClipOp::kDifference, true);
    recorder.canvas()->sk_canvas()->drawRRect(r_rect, flags);
}
