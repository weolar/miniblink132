...
SlideOutController::SlideOutController(ui::EventTarget* target, SlideOutControllerDelegate* delegate)
...
void SlideOutController::CaptureControlOpenState()
{
    if (!has_swipe_control_)
        return;
    if ((mode_ == SlideMode::kFull || mode_ == SlideMode::kPartial) && fabs(gesture_amount_) >= swipe_control_width_) {
        control_open_state_ = gesture_amount_ < 0 ? SwipeControlOpenState::kOpenOnRight : SwipeControlOpenState::kOpenOnLeft;
    } else {
...
void SlideOutController::OnGestureEvent(ui::GestureEvent* event)
{
    ...
    float scroll_amount_for_closing_notification = has_swipe_control_ ? swipe_control_width_ + kSwipeCloseMargin : width * 0.5;
    ...
    if (event->type() == ui::EventType::kScrollFlingStart) {
        ...
        if (mode_ == SlideMode::kFull && fabsf(event->details().velocity_x()) > kFlingThresholdForClose) {
    ...
    if (event->type() == ui::EventType::kGestureScrollBegin) {
        ...
        case SlideMode::kPartial:
            if (gesture_amount_ >= 0) {
                scroll_amount = std::min(0.5f * gesture_amount_, scroll_amount_for_closing_notification);
            } else {
                scroll_amount = std::max(0.5f * gesture_amount_, -1.f * scroll_amount_for_closing_notification);
            }
            opacity = 1.f;
            break;
        }
        ...
        if (mode_ == SlideMode::kFull && scrolled_ratio >= scroll_amount_for_closing_notification / width) {
...
void SlideOutController::OnScrollEvent(ui::ScrollEvent* event)
{
    ...
    if (event->type() == ui::EventType::kScrollFlingCancel) {
        ...
        if (abs(gesture_amount_) > width) {
            ...
            AnimationBuilder()
                .OnEnded(base::BindOnce(&SlideOutController::OnSlideOut, weak_ptr_factory_.GetWeakPtr()))
                .Once()
...
void SlideOutController::SlideOutAndClose(int direction)
{
    ...
    SetTransformWithAnimationIfNecessary(transform, base::Milliseconds(swipe_out_duration));
}
...
void SlideOutController::SetTransformWithAnimationIfNecessary(const gfx::Transform& transform, base::TimeDelta animation_duration)
{
    ui::Layer* layer = delegate_->GetSlideOutLayer();
    if (layer->transform() != transform) {
        ...
        AnimationBuilder()
            .OnEnded(base::BindOnce(&SlideOutController::OnAnimationsCompleted, weak_ptr_factory_.GetWeakPtr()))
            .Once()
...
void SlideOutController::OnAnimationsCompleted()
{
    ...
    const bool in_progress = !delegate_->GetSlideOutLayer()->transform().IsIdentity() && !is_completely_slid_out;
    delegate_->OnSlideChanged(in_progress);
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&SlideOutController::OnSlideOut, weak_ptr_factory_.GetWeakPtr()));
}
