...
template <typename ContextType> class TestInkDropAnimationObserverHelper {
public:
    ...
    TestInkDropAnimationObserverHelper(const TestInkDropAnimationObserverHelper&) = delete;
    TestInkDropAnimationObserverHelper& operator=(const TestInkDropAnimationObserverHelper&) = delete;
    ...
    void OnAnimationEnded(ContextType context, InkDropAnimationEndedReason reason)
    {
    ...
    testing::AssertionResult AnimationHasStarted()
    {
        if (last_animation_started_ordinal() > 0) {
            return testing::AssertionSuccess() << "Animations were started at ordinal=" << last_animation_started_ordinal() << ".";
        }
    ...
    testing::AssertionResult AnimationHasNotStarted()
    {
        ...
        return testing::AssertionFailure() << "Animations were started at ordinal=" << last_animation_started_ordinal() << ".";
    }
    ...
    testing::AssertionResult AnimationHasEnded()
    {
        if (last_animation_ended_ordinal() > 0) {
            return testing::AssertionSuccess() << "Animations were ended at ordinal=" << last_animation_ended_ordinal() << ".";
        }
    ...
    testing::AssertionResult AnimationHasNotEnded()
    {
        if (last_animation_ended_ordinal() < 0)
            return testing::AssertionSuccess();
        return testing::AssertionFailure() << "Animations were ended at ordinal=" << last_animation_ended_ordinal() << ".";
    }

    // Passes *_TRUE assertions when |animation_started_context_| is the same as
    // |expected_contexts|.
    testing::AssertionResult AnimationStartedContextsMatch(const std::vector<ContextType>& expected_contexts)
    {
    ...
    // |expected_contexts|.
    testing::AssertionResult AnimationEndedContextsMatch(const std::vector<ContextType>& expected_contexts)
    {
...
private:
    ...
    testing::AssertionResult ContextsMatch(const std::vector<ContextType>& expected_contexts, const std::vector<ContextType>& actual_contexts)
    {
        const bool match = base::ranges::equal(expected_contexts, actual_contexts);
        testing::AssertionResult result
            = match ? (testing::AssertionSuccess() << "Expected == Actual: {") : (testing::AssertionFailure() << "Expected != Actual: {");
        for (auto eit = expected_contexts.begin(), ait = actual_contexts.begin(); eit != expected_contexts.end() || ait != actual_contexts.end();) {
            ...
            result << (eexists ? ToString(*eit) : "<none>") << (item_match ? " == " : " != ") << (aexists ? ToString(*ait) : "<none>");
            if (eexists)
                eit++;
            if (aexists)
                ait++;
    ...
    {
        return std::max(1, std::max(last_animation_started_ordinal_, last_animation_ended_ordinal_) + 1);
    }
    ...
    InkDropAnimationEndedReason last_animation_ended_reason_ = InkDropAnimationEndedReason::SUCCESS;
};
