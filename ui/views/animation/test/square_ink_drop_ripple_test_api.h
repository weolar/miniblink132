...
public:
    ...
    SquareInkDropRippleTestApi& operator=(const SquareInkDropRippleTestApi&) = delete;

    ~SquareInkDropRippleTestApi() override;

    // Wrapper functions to the wrapped InkDropRipple:
    void CalculateCircleTransforms(const gfx::Size& size, InkDropTransforms* transforms_out) const;
    void CalculateRectTransforms(const gfx::Size& size, float corner_radius, InkDropTransforms* transforms_out) const;
...
private:
    ...
    {
        return static_cast<const SquareInkDropRippleTestApi*>(this)->ink_drop_ripple();
    }

    SquareInkDropRipple* ink_drop_ripple() const
    {
        return static_cast<SquareInkDropRipple*>(InkDropRippleTestApi::ink_drop_ripple());
    }
};
