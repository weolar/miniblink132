...
public:
    ...
    void SetInkDrop(std::unique_ptr<InkDrop> ink_drop, bool handles_gesture_events);
    void SetInkDrop(std::unique_ptr<InkDrop> ink_drop);

    InkDrop* ink_drop()
    {
        return ink_drop_host_->ink_drop_.get();
    }
    ...
    InkDropMode ink_drop_mode() const
    {
        return ink_drop_host_->ink_drop_mode_;
    }
