...
InkDropHost::InkDropHostEventHandlerDelegate::InkDropHostEventHandlerDelegate(InkDropHost* ink_drop_host)
...
bool InkDropHost::InkDropHostEventHandlerDelegate::SupportsGestureEvents() const
{
    return ink_drop_host_->ink_drop_mode_ == InkDropMode::ON;
}

InkDropHost::ViewLayerTransformObserver::ViewLayerTransformObserver(InkDropHost* ink_drop_host, View* host_view)
    : ink_drop_host_(ink_drop_host)
{
    observation_.Observe(host_view);
}

InkDropHost::ViewLayerTransformObserver::~ViewLayerTransformObserver() = default;

void InkDropHost::ViewLayerTransformObserver::OnViewLayerTransformed(View* observed_view)
{
    // Notify the ink drop that we have transformed so it can adapt
    // accordingly.
    if (ink_drop_host_->HasInkDrop()) {
        ink_drop_host_->GetInkDrop()->HostTransformChanged(observed_view->GetTransform());
    }
}
...
void InkDropHost::SetCreateInkDropCallback(base::RepeatingCallback<std::unique_ptr<InkDrop>()> callback)
{
...
std::unique_ptr<InkDropRipple> InkDropHost::CreateInkDropRipple() const
{
    ...
    return std::make_unique<views::FloodFillInkDropRipple>(
        InkDrop::Get(host_view_), host_view_->size(), GetInkDropCenterBasedOnLastEvent(), GetBaseColor(), GetVisibleOpacity());
}

void InkDropHost::SetCreateRippleCallback(base::RepeatingCallback<std::unique_ptr<InkDropRipple>()> callback)
{
...
{
    return GetEventHandler()->GetLastRippleTriggeringEvent() ? GetEventHandler()->GetLastRippleTriggeringEvent()->location()
                                                             : host_view_->GetMirroredRect(host_view_->GetContentsBounds()).CenterPoint();
}
...
std::unique_ptr<InkDropHighlight> InkDropHost::CreateInkDropHighlight() const
{
    ...
    auto highlight = std::make_unique<views::InkDropHighlight>(
        host_view_->size(), 0, gfx::RectF(host_view_->GetMirroredRect(host_view_->GetLocalBounds())).CenterPoint(), GetBaseColor());
...
void InkDropHost::SetCreateHighlightCallback(base::RepeatingCallback<std::unique_ptr<InkDropHighlight>()> callback)
{
...
std::unique_ptr<views::InkDropMask> InkDropHost::CreateInkDropMask() const
{
    ...
    return std::make_unique<views::PathInkDropMask>(host_view_->size(), GetHighlightPath(host_view_));
}

void InkDropHost::SetCreateMaskCallback(base::RepeatingCallback<std::unique_ptr<InkDropMask>()> callback)
{
...
SkColor InkDropHost::GetBaseColor() const
{
    ...
    if (absl::holds_alternative<ui::ColorId>(ink_drop_base_color_)) {
        ...
        CHECK(color_provider);
        return color_provider->GetColor(absl::get<ui::ColorId>(ink_drop_base_color_));
    }
    ...
    if (host_view_->GetWidget()) {
        return absl::get<base::RepeatingCallback<SkColor()>>(ink_drop_base_color_).Run();
    }
...
void InkDropHost::SetBaseColorCallback(base::RepeatingCallback<SkColor()> callback)
{
    CHECK(callback);
...
void InkDropHost::AnimateToState(InkDropState state, const ui::LocatedEvent* event)
{
...
base::CallbackListSubscription InkDropHost::AddHighlightedChangedCallback(base::RepeatingClosure callback)
{
...
std::unique_ptr<InkDropRipple> InkDropHost::CreateSquareRipple(const gfx::Point& center_point, const gfx::Size& size) const
{
    auto ripple = std::make_unique<SquareInkDropRipple>(InkDrop::Get(host_view_), GetLargeSize(size), ink_drop_large_corner_radius_, size,
        ink_drop_small_corner_radius_, center_point, GetBaseColor(), GetVisibleOpacity());
    return ripple;
}
...
{
    return const_cast<InkDropEventHandler*>(const_cast<const InkDropHost*>(this)->GetEventHandler());
}

bool InkDropHost::AddInkDropClip(ui::Layer* ink_drop_layer)
{
    std::optional<gfx::RRectF> clipping_data = HighlightPathGenerator::GetRoundRectForView(host_view_);
    if (!clipping_data) {
        return false;
    }

    ink_drop_layer->SetClipRect(gfx::ToEnclosingRect(clipping_data->rect()));
    auto get_corner_radii = [&clipping_data](gfx::RRectF::Corner corner) -> float { return clipping_data.value().GetCornerRadii(corner).x(); };
    gfx::RoundedCornersF rounded_corners;
    rounded_corners.set_upper_left(get_corner_radii(gfx::RRectF::Corner::kUpperLeft));
    rounded_corners.set_upper_right(get_corner_radii(gfx::RRectF::Corner::kUpperRight));
    rounded_corners.set_lower_right(get_corner_radii(gfx::RRectF::Corner::kLowerRight));
    rounded_corners.set_lower_left(get_corner_radii(gfx::RRectF::Corner::kLowerLeft));
