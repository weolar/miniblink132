...
enum InkDropRippleTestTypes { SQUARE_INK_DROP_RIPPLE, FLOOD_FILL_INK_DROP_RIPPLE };
...
class InkDropRippleTest : public testing::TestWithParam<InkDropRippleTestTypes> {
public:
    InkDropRippleTest();
    ...
    : animation_mode_reset_(gfx::AnimationTestApi::SetRichAnimationRenderMode(gfx::Animation::RichAnimationRenderMode::FORCE_DISABLED))
{
    ...
    case SQUARE_INK_DROP_RIPPLE: {
        SquareInkDropRipple* square_ink_drop_ripple
            = new SquareInkDropRipple(nullptr, gfx::Size(10, 10), 2, gfx::Size(8, 8), 1, gfx::Point(), SK_ColorBLACK, kVisibleOpacity);
        ink_drop_ripple_.reset(square_ink_drop_ripple);
        test_api_ = std::make_unique<SquareInkDropRippleTestApi>(square_ink_drop_ripple);
        break;
    }
    case FLOOD_FILL_INK_DROP_RIPPLE: {
        FloodFillInkDropRipple* flood_fill_ink_drop_ripple
            = new FloodFillInkDropRipple(nullptr, gfx::Size(10, 10), gfx::Point(), SK_ColorBLACK, kVisibleOpacity);
        ink_drop_ripple_.reset(flood_fill_ink_drop_ripple);
        test_api_ = std::make_unique<FloodFillInkDropRippleTestApi>(flood_fill_ink_drop_ripple);
        break;
    }
    }
...
INSTANTIATE_TEST_SUITE_P(All, InkDropRippleTest, testing::Values(SQUARE_INK_DROP_RIPPLE, FLOOD_FILL_INK_DROP_RIPPLE));

TEST_P(InkDropRippleTest, InitialStateAfterConstruction)
{
    EXPECT_EQ(views::InkDropState::HIDDEN, ink_drop_ripple_->target_ink_drop_state());
}
...
{
    ink_drop_ripple_->AnimateToState(views::InkDropState::ACTION_PENDING);
    ink_drop_ripple_->AnimateToState(views::InkDropState::ALTERNATE_ACTION_PENDING);
    test_api_->CompleteAnimations();
...
TEST_P(InkDropRippleTest, SlowActionOpacity)
{
    ink_drop_ripple_->AnimateToState(views::InkDropState::ACTION_PENDING);
    ink_drop_ripple_->AnimateToState(views::InkDropState::ALTERNATE_ACTION_PENDING);
    ink_drop_ripple_->AnimateToState(views::InkDropState::ALTERNATE_ACTION_TRIGGERED);
    test_api_->CompleteAnimations();
...
TEST_P(InkDropRippleTest, AnimationsAbortedDuringDeletion)
{
    ...
    EXPECT_EQ(1, observer_.last_animation_started_ordinal());
    EXPECT_EQ(2, observer_.last_animation_ended_ordinal());
    EXPECT_EQ(views::InkDropState::ACTION_PENDING, observer_.last_animation_ended_context());
    EXPECT_EQ(InkDropAnimationEndedReason::PRE_EMPTED, observer_.last_animation_ended_reason());
}
...
TEST_P(InkDropRippleTest, VerifyObserversAreNotified)
{
    ...
    EXPECT_EQ(InkDropState::ACTION_PENDING, observer_.last_animation_started_context());
    ...
    EXPECT_EQ(1, observer_.last_animation_started_ordinal());
    EXPECT_EQ(2, observer_.last_animation_ended_ordinal());
    EXPECT_EQ(InkDropState::ACTION_PENDING, observer_.last_animation_ended_context());
}
...
TEST_P(InkDropRippleTest, VerifyObserversAreNotifiedOfSuccessfulAnimations)
{
    ...
    EXPECT_EQ(2, observer_.last_animation_ended_ordinal());
    EXPECT_EQ(InkDropAnimationEndedReason::SUCCESS, observer_.last_animation_ended_reason());
}
...
TEST_P(InkDropRippleTest, InkDropStatesPersistWhenCallingAnimateToState)
{
    ink_drop_ripple_->AnimateToState(views::InkDropState::ACTION_PENDING);
    ink_drop_ripple_->AnimateToState(views::InkDropState::ACTIVATED);
    EXPECT_EQ(views::InkDropState::ACTIVATED, ink_drop_ripple_->target_ink_drop_state());
}
...
TEST_P(InkDropRippleTest, TargetInkDropStateOnAnimationStarted)
{
    ...
    EXPECT_EQ(views::InkDropState::ACTION_PENDING, observer_.target_state_at_last_animation_started());
    ...
    EXPECT_TRUE(observer_.AnimationHasStarted());
    EXPECT_EQ(views::InkDropState::HIDDEN, observer_.target_state_at_last_animation_started());
}
...
TEST_P(InkDropRippleTest, TargetInkDropStateOnAnimationEnded)
{
    ...
    EXPECT_EQ(views::InkDropState::HIDDEN, observer_.target_state_at_last_animation_ended());
}
...
TEST_P(InkDropRippleTest, RipplePendingToActivatedObserverOrder)
{
    ...
    EXPECT_TRUE(observer_.AnimationStartedContextsMatch({ InkDropState::ACTION_PENDING, InkDropState::ACTIVATED }));
    EXPECT_TRUE(observer_.AnimationEndedContextsMatch({ InkDropState::ACTION_PENDING, InkDropState::ACTIVATED }));
}
