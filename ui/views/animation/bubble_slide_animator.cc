...
BubbleSlideAnimator::BubbleSlideAnimator(BubbleDialogDelegateView* bubble_delegate)
    ...
    , bubble_delegate_(bubble_delegate)
{
    ...
    constexpr base::TimeDelta kDefaultBubbleSlideAnimationTime = base::Milliseconds(200);
...
void BubbleSlideAnimator::AnimateToAnchorView(View* desired_anchor_view)
{
    ...
    starting_bubble_bounds_ = bubble_delegate_->GetWidget()->GetWindowBoundsInScreen();
...
base::CallbackListSubscription BubbleSlideAnimator::AddSlideProgressedCallback(SlideProgressedCallback callback)
{
    return slide_progressed_callbacks_.Add(callback);
}

base::CallbackListSubscription BubbleSlideAnimator::AddSlideCompleteCallback(SlideCompleteCallback callback)
{
...
{
    double value = gfx::Tween::CalculateValue(tween_type_, slide_animation_.GetCurrentValue());

    const gfx::Rect current_bounds = gfx::Tween::RectValueBetween(value, starting_bubble_bounds_, target_bubble_bounds_);
...
gfx::Rect BubbleSlideAnimator::CalculateTargetBounds(const View* desired_anchor_view) const
{
    return bubble_delegate_->GetBubbleFrameView()->GetUpdatedWindowBounds(
        desired_anchor_view->GetAnchorBoundsInScreen(), bubble_delegate_->arrow(), bubble_delegate_->GetWidget()->client_view()->GetPreferredSize({}), true);
}
