...
void PulsingInkDropMask::OnPaintLayer(const ui::PaintContext& context)
{
    ...
    const gfx::Insets* const internal_padding = layer_container_->GetProperty(views::kInternalPaddingKey);
    if (internal_padding) {
    ...
    const float current_inset = throb_animation_.CurrentValueBetween(0.0f, kFeaturePromoPulseInsetDip);
