...
public:
    explicit TestBubbleView(View* anchor_view)
        : BubbleDialogDelegateView(anchor_view, BubbleBorder::TOP_LEFT, BubbleBorder::DIALOG_SHADOW, true)
    {
...
public:
    ...
    void AnimationContainerWasSet(gfx::AnimationContainer* container) override
    {
        ...
        if (container) {
            container_test_api_ = std::make_unique<gfx::AnimationContainerTestApi>(container);
        }
    }
...
public:
    void SetUp() override
    {
        ...
        anchor_widget_ = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::Type::TYPE_WINDOW);
        auto* const contents_view = anchor_widget_->GetRootView()->AddChildView(std::make_unique<FlexLayoutView>());
...
TEST_F(BubbleSlideAnimatorTest, SlideCallbacksCalled)
{
    ...
    auto progress_sub = delegate_->AddSlideProgressedCallback(base::BindLambdaForTesting([&](BubbleSlideAnimator*, double progress) {
        last_progress = progress;
        ++progress_count;
    }));
    auto completed_sub = delegate_->AddSlideCompleteCallback(base::BindLambdaForTesting([&](BubbleSlideAnimator*) { ++complete_count; }));
    delegate_->AnimateToAnchorView(view2_);
