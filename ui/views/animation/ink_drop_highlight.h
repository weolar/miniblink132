...
public:
    ...
    InkDropHighlight(const gfx::PointF& center_point, std::unique_ptr<BasePaintedLayerDelegate> layer_delegate);

    // Creates a highlight that paints a partially transparent roundrect with
    // color |color|.
    InkDropHighlight(const gfx::SizeF& size, int corner_radius, const gfx::PointF& center_point, SkColor color);

    // Deprecated version of the above that takes a Size instead of SizeF.
    // TODO(estade): remove. See crbug.com/706228
    InkDropHighlight(const gfx::Size& size, int corner_radius, const gfx::PointF& center_point, SkColor color);
...
private:
    ...
    // |duration|.
    void AnimateFade(AnimationType animation_type, const base::TimeDelta& duration);
    ...
    void AnimationEndedCallback(AnimationType animation_type, InkDropAnimationEndedReason reason);
...
VIEWS_EXPORT std::string ToString(InkDropHighlight::AnimationType animation_type);

// This is declared here for use in gtest-based unit tests but is defined in
// the views_test_support target. Depend on that to use this in your unit test.
// This should not be used in production code - call ToString() instead.
void PrintTo(InkDropHighlight::AnimationType animation_type, ::std::ostream* os);
