...
InkDropEventHandler::InkDropEventHandler(View* host_view, Delegate* delegate)
    : target_handler_(std::make_unique<ui::ScopedTargetHandler>(host_view, this))
...
void InkDropEventHandler::AnimateToState(InkDropState state, const ui::LocatedEvent* event)
{
#if BUILDFLAG(IS_WIN)
    ...
    if (event && (event->IsTouchEvent() || event->IsGestureEvent()) && delegate_->GetInkDrop()->GetTargetInkDropState() == InkDropState::HIDDEN
        && state != InkDropState::ACTIVATED) {
        return;
    }
#endif
    last_ripple_triggering_event_.reset(event ? event->Clone().release()->AsLocatedEvent() : nullptr);
...
void InkDropEventHandler::OnGestureEvent(ui::GestureEvent* event)
{
    ...
    InkDropState current_ink_drop_state = delegate_->GetInkDrop()->GetTargetInkDropState();
    ...
    if (ink_drop_state == InkDropState::HIDDEN
        && (current_ink_drop_state == InkDropState::ACTION_TRIGGERED || current_ink_drop_state == InkDropState::ALTERNATE_ACTION_TRIGGERED
            || current_ink_drop_state == InkDropState::DEACTIVATED || current_ink_drop_state == InkDropState::HIDDEN)) {
...
void InkDropEventHandler::OnMouseEvent(ui::MouseEvent* event)
{
    ...
    case ui::EventType::kMouseDragged:
        delegate_->GetInkDrop()->SetHovered(host_view_->GetLocalBounds().Contains(event->location()));
        break;
    default:
        break;
    }
...
void InkDropEventHandler::OnViewVisibilityChanged(View* observed_view, View* starting_view)
{
    ...
    const bool is_visible = host_view_->IsDrawn() && host_view_->GetWidget() && host_view_->GetWidget()->IsVisible();
...
void InkDropEventHandler::OnViewHierarchyChanged(View* observed_view, const ViewHierarchyChangedDetails& details)
{
    DCHECK_EQ(host_view_, observed_view);
    ...
    if (!details.is_add && details.child == host_view_ && delegate_->HasInkDrop()) {
