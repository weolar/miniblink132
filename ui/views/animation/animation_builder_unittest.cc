...
public:
    TestAnimatibleLayerOwner()
        : ui::LayerOwner(std::make_unique<ui::Layer>())
    {
...
AnimationBuilder BuildLayerOpacityAnimationAndReturnBuilder(ui::LayerOwner* layer_owner, const base::TimeDelta& duration)
{
...
public:
    ...
    TestAnimatibleLayerOwner* CreateTestLayerOwner()
    {
        ...
        animator_controllers_.push_back(std::make_unique<ui::LayerAnimatorTestController>(layer_owners_.back()->layer()->GetAnimator()));
    ...
    void Step(const base::TimeDelta& duration)
    {
        ...
        for (const auto& controller : animator_controllers_) {
            controller->StartThreadedAnimationsIfNeeded(controller->animator()->last_step_time());
...
protected:
    ...
    {
        testing::Test::SetUp();
        AnimationBuilder::SetObserverDeletedCallbackForTesting(base::BindRepeating([](int* deleted_count) { ++(*deleted_count); }, &deleted_observers_));
    }
    ...
    void TearDown() override
    {
        ...
        AnimationBuilder::SetObserverDeletedCallbackForTesting(base::NullCallback());
...
private:
    ...
    std::vector<std::unique_ptr<ui::LayerAnimatorTestController>> animator_controllers_;
...
TEST_F(AnimationBuilderTest, SimpleAnimation)
{
    ...
    ui::LayerAnimationDelegate* second_delegate = second_animating_view->delegate();
    ...
    EXPECT_FLOAT_EQ(first_delegate->GetRoundedCornersForAnimation().upper_left(), 0.0);
    ...
    EXPECT_FLOAT_EQ(first_delegate->GetRoundedCornersForAnimation().upper_left(), 12.0f);
...
{
    ui::ScopedAnimationDurationScaleMode scoped_animation_duration_scale_mode(ui::ScopedAnimationDurationScaleMode::SLOW_DURATION);
    TestAnimatibleLayerOwner* first_animating_view = CreateTestLayerOwner();
    TestAnimatibleLayerOwner* second_animating_view = CreateTestLayerOwner();
    ui::LayerAnimationDelegate* first_delegate = first_animating_view->delegate();
    ui::LayerAnimationDelegate* second_delegate = second_animating_view->delegate();
...
{
    ui::ScopedAnimationDurationScaleMode scoped_animation_duration_scale_mode(ui::ScopedAnimationDurationScaleMode::FAST_DURATION);
    TestAnimatibleLayerOwner* first_animating_view = CreateTestLayerOwner();
    TestAnimatibleLayerOwner* second_animating_view = CreateTestLayerOwner();
    ui::LayerAnimationDelegate* first_delegate = first_animating_view->delegate();
    ui::LayerAnimationDelegate* second_delegate = second_animating_view->delegate();
...
{
    ui::ScopedAnimationDurationScaleMode scoped_animation_duration_scale_mode(ui::ScopedAnimationDurationScaleMode::ZERO_DURATION);
    TestAnimatibleLayerOwner* first_animating_view = CreateTestLayerOwner();
    TestAnimatibleLayerOwner* second_animating_view = CreateTestLayerOwner();
    ui::LayerAnimationDelegate* first_delegate = first_animating_view->delegate();
    ui::LayerAnimationDelegate* second_delegate = second_animating_view->delegate();
...
TEST_F(AnimationBuilderTest, ModifiedZeroAnimationDurationWithOnEndedCallback)
{
    ...
    b.OnEnded(base::BindRepeating(
                  [](TestAnimatibleLayerOwner* layer_owner, TestAnimatibleLayerOwner* second_layer_owner) {
                      delete layer_owner;
...
TEST_F(AnimationBuilderTest, ZeroDurationBlock)
{
    ...
    EXPECT_FLOAT_EQ(first_delegate->GetRoundedCornersForAnimation().upper_left(), 6.0f);
    Step(kDelay / 2);
    EXPECT_FLOAT_EQ(first_delegate->GetRoundedCornersForAnimation().upper_left(), 9.0f);
    Step(kDelay / 2);
    EXPECT_FLOAT_EQ(first_delegate->GetRoundedCornersForAnimation().upper_left(), 6.0f);
}
...
TEST_F(AnimationBuilderTest, CheckTweenType)
{
    ...
    // Set initial opacity.
    first_animating_view->delegate()->SetOpacityFromAnimation(0.0f, ui::PropertyChangeReason::NOT_FROM_ANIMATION);

    constexpr float opacity_end_val = 0.5f;
    {
        AnimationBuilder().Once().SetDuration(kDelay).SetOpacity(first_animating_view, opacity_end_val, tween_type);
    }
    EXPECT_TRUE(first_animating_view->layer()->GetAnimator()->is_animating());
    Step(kDelay / 2);
    ...
    EXPECT_NEAR(gfx::Tween::CalculateValue(tween_type, 0.5) * opacity_end_val, first_animating_view->delegate()->GetOpacityForAnimation(), 0.001f);
}
...
TEST_F(AnimationBuilderTest, DestroyLayerBeforeAnimationEnd)
{
    ...
    std::unique_ptr<AnimationAbortHandle> abort_handle;
    {
        ...
        builder.Once().SetDuration(base::Seconds(3)).SetOpacity(first_animating_view, 0.5f).SetOpacity(second_animating_view, 0.5f);
    }
...
TEST_F(AnimationBuilderTest, DestroyLayerWhenAnimationEnd)
{
    ...
    auto end_callback = [](TestAnimatibleLayerOwner* first_animating_view, TestAnimatibleLayerOwner* second_animating_view) {
        first_animating_view->ReleaseLayer();
        second_animating_view->ReleaseLayer();
    };
    ...
    std::unique_ptr<AnimationAbortHandle> abort_handle;
    {
        AnimationBuilder builder;
        abort_handle = builder.GetAbortHandle();
        builder.OnEnded(base::BindOnce(end_callback, first_animating_view, second_animating_view))
            .Once()
            .SetDuration(kDelay)
...
TEST_F(AnimationBuilderTest, DestroyLayerWhenAnimationAborted)
{
    ...
    auto abort_callback = [](TestAnimatibleLayerOwner* first_animating_view, TestAnimatibleLayerOwner* second_animating_view) {
        first_animating_view->ReleaseLayer();
        second_animating_view->ReleaseLayer();
    };
    ...
    std::unique_ptr<AnimationAbortHandle> abort_handle;
    {
        AnimationBuilder builder;
        abort_handle = builder.GetAbortHandle();
        builder.OnAborted(base::BindOnce(abort_callback, first_animating_view, second_animating_view))
            .Once()
            .SetDuration(kDelay)
            .SetOpacity(first_animating_view, 0.5f)
            .SetOpacity(second_animating_view, 0.5f);
...
TEST_F(AnimationBuilderTest, CheckStartEndCallbacks)
{
    ...
    {
        AnimationBuilder()
            .OnStarted(base::BindOnce([](bool* started) { *started = true; }, &started))
...
TEST_F(AnimationBuilderTest, CheckOnWillRepeatCallbacks)
{
    ...
    {
        AnimationBuilder b;
        b.OnWillRepeat(base::BindRepeating([](int& repeat) { repeat = repeat + 1; }, std::ref(first_repeat_count)))
            .Repeatedly()
            .SetDuration(kDelay)
            .SetOpacity(first_animating_view, 0.4f)
            .Then()
        ...
        b.OnWillRepeat(base::BindRepeating([](int& repeat) { repeat = repeat + 1; }, std::ref(second_repeat_count)))
            .Repeatedly()
            .SetDuration(kDelay)
...
TEST_F(AnimationBuilderTest, DelayedStart)
{
    ...
    {
        // clang-format on
        AnimationBuilder().Once().At(kDelay).SetDuration(kDuration).SetOpacity(view, 0.4f);
        // clang-format on
    }
...
TEST_F(AnimationBuilderTest, TwoKeyFrame)
{
    ...
    {
        AnimationBuilder().Once().SetDuration(kDuration).SetOpacity(view, 0.4f).Then().SetDuration(kDuration).SetOpacity(view, 0.9f);
    }
...
TEST_F(AnimationBuilderTest, PauseInTheMiddle)
{
    ...
    {
        AnimationBuilder().Once().SetDuration(kDuration).SetOpacity(view, 0.4f).Then().Offset(kDuration).SetDuration(kDuration).SetOpacity(view, 0.9f);
    }
...
TEST_F(AnimationBuilderTest, TwoPropertiesOfDifferentDuration)
{
    ...
    EXPECT_FLOAT_EQ(delegate->GetOpacityForAnimation(), 0.4f);
    EXPECT_FLOAT_EQ(delegate->GetRoundedCornersForAnimation().upper_left(), 12.0f);
}
...
TEST_F(AnimationBuilderTest, TwoPropertiesOfDifferentStartTime)
{
    ...
    {
        AnimationBuilder().Once().SetDuration(kDuration).SetOpacity(view, 0.4f).At(kDelay).SetDuration(kDuration).SetRoundedCorners(view, rounded_corners);
    }
...
TEST_F(AnimationBuilderTest, Repeat)
{
    ...
    {
        AnimationBuilder().Repeatedly().SetDuration(kDuration).SetOpacity(view, 0.4f).Then().SetDuration(kDuration).SetOpacity(view, 0.9f);
    }
...
TEST_F(AnimationBuilderTest, RepeatedBlocks)
{
...

    {
        ...
        for (const auto& opacity : kOpacity) {
            builder.GetCurrentSequence().SetDuration(kDuration).SetOpacity(view, opacity).Then();
        }
    }
...
TEST_F(AnimationBuilderTest, PreemptionStrategyTest)
{
    ...
    delegate->SetBrightnessFromAnimation(1.0f, ui::PropertyChangeReason::NOT_FROM_ANIMATION);
    ...
    // Now start a new animation to a different target.
    {
        AnimationBuilder()
            ...
            .Once()
            .SetDuration(kStepSize) // We only moved previous animation by kStepSize
    ...
    {
        AnimationBuilder().SetPreemptionStrategy(ps::IMMEDIATELY_ANIMATE_TO_NEW_TARGET).Once().SetDuration(kStepSize).SetBrightness(view, 1.0f);
    }
...
TEST_F(AnimationBuilderTest, AbortHandle)
{
    ...
    {
        delegate->SetBrightnessFromAnimation(1.0f, ui::PropertyChangeReason::NOT_FROM_ANIMATION);
        delegate->SetOpacityFromAnimation(1.0f, ui::PropertyChangeReason::NOT_FROM_ANIMATION);

        {
            AnimationBuilder b;
            abort_handle = b.GetAbortHandle();
            b.Once().SetDuration(kDuration).SetOpacity(view, 0.4f).At(base::TimeDelta()).SetDuration(kDuration).SetBrightness(view, 0.4f);
        }
    ...
    // The handle shouldn't abort animations subsequent to the builder.
    {
        ...
        EXPECT_EQ(abort_handle->animation_state(), AnimationAbortHandle::AnimationState::kRunning);
        // Step to the end of the animation.
        Step(kDuration);
        EXPECT_EQ(abort_handle->animation_state(), AnimationAbortHandle::AnimationState::kEnded);

        {
            AnimationBuilder b;
            b.Once().SetDuration(kDuration).SetOpacity(view, 0.8f).At(base::TimeDelta()).SetDuration(kDuration).SetBrightness(view, 0.8f);
        }
...
TEST_F(AnimationBuilderTest, BuildAnimationWithBuilderFromScope)
{
    ...
    constexpr auto kDuration = base::Seconds(3);
    {
        ...
        AnimationBuilder builder = BuildLayerOpacityAnimationAndReturnBuilder(first_animating_view, kDuration);
...
// same target.
TEST_F(AnimationBuilderTest, DisallowMultipleSameBlockSameTargetTransformPropertyAnimations)
{
    TestAnimatibleLayerOwner* target = CreateTestLayerOwner();
    EXPECT_DCHECK_DEATH_WITH(AnimationBuilder()
                                 .Once()
                                 .SetInterpolatedTransform(target,
                                     std::make_unique<ui::InterpolatedMatrixTransform>(
...
TEST_F(AnimationBuilderTest, SetInterpolatedTransform)
{
    ...
    AnimationBuilder().Once().SetDuration(kDuration).SetInterpolatedTransform(target, std::move(transform));

    // Verify state at animation start.
    EXPECT_TRUE(target->layer()->GetAnimator()->is_animating());
    EXPECT_EQ(target->delegate()->GetTransformForAnimation(), expected_start_transform);

    // Verify state at animation midpoint.
    Step(kDuration / 2);
    EXPECT_TRUE(target->layer()->GetAnimator()->is_animating());
    EXPECT_EQ(target->delegate()->GetTransformForAnimation(), expected_mid_transform);

    // Verify state at animation end.
    Step(kDuration / 2);
    EXPECT_FALSE(target->layer()->GetAnimator()->is_animating());
    EXPECT_EQ(target->delegate()->GetTransformForAnimation(), expected_end_transform);
}
