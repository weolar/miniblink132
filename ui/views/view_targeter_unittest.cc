...
public:
    ...
    void SetGestureHandlerSetBeforeProcessing(internal::RootView* root_view, bool set)
    {
...
TEST_F(ViewTargeterTest, ViewTargeterForKeyEvents)
{
    ...
    ui::KeyEvent key_event = ui::KeyEvent::FromCharacter('a', ui::VKEY_A, ui::DomCode::NONE, ui::EF_NONE);

    // The focused view should be the initial target of the event.
    ui::EventTarget* current_target = targeter->FindTargetForEvent(root_view, &key_event);
    ...
    current_target = targeter->FindNextBestTarget(widget->GetRootView(), &key_event);
...
TEST_F(ViewTargeterTest, ViewTargeterForScrollEvents)
{
    ...
    ui::ScrollEvent scroll(ui::EventType::kScroll, gfx::Point(60, 60), ui::EventTimeForNow(), 0, 0, 3, 0, 3, 2);
    ui::EventTarget* current_target = targeter->FindTargetForEvent(root_view, &scroll);
    EXPECT_EQ(child, static_cast<View*>(current_target));
    ...
    scroll = ui::ScrollEvent(ui::EventType::kScroll, gfx::Point(150, 150), ui::EventTimeForNow(), 0, 0, 3, 0, 3, 2);
...
{
    return ui::GestureEvent(details.bounding_box().CenterPoint().x(), details.bounding_box().CenterPoint().y(), 0, base::TimeTicks(), details);
}
...
TEST_F(ViewTargeterTest, TargetContentsAndRootView)
{
    ...
    EXPECT_EQ(widget->GetRootView(), targeter->FindTargetForEvent(root_view, &tap));
...
TEST_F(ViewTargeterTest, GestureEventCoordinateConversion)
{
    ...
    gfx::Point location_in_great_grandchild(ConvertPointFromWidgetToView(great_grandchild, location_in_root));
    EXPECT_EQ(gfx::Point(2, 2), location_in_great_grandchild);
    gfx::Point location_in_grandchild(ConvertPointFromWidgetToView(grandchild, location_in_root));
    EXPECT_EQ(gfx::Point(5, 5), location_in_grandchild);
    gfx::Point location_in_child(ConvertPointFromWidgetToView(child, location_in_root));
    EXPECT_EQ(gfx::Point(10, 10), location_in_child);
    gfx::Point location_in_content(ConvertPointFromWidgetToView(content, location_in_root));
...
TEST_F(ViewTargeterTest, DoesIntersectRect)
{
    ...
    EXPECT_TRUE(view_targeter->DoesIntersectRect(root_view, gfx::Rect(0, 0, 50, 50)));
    EXPECT_FALSE(view_targeter->DoesIntersectRect(root_view, gfx::Rect(-20, -20, 10, 10)));
}
...
TEST_F(ViewTargeterTest, HitTestCallsOnView)
{
    ...
    TestMaskedView* v2 = root_view->AddChildView(std::make_unique<TestMaskedView>());
    ...
    EXPECT_TRUE(v1->HitTestPoint(ConvertPointFromWidgetToView(v1, v1_centerpoint)));
    EXPECT_TRUE(v2->HitTestPoint(ConvertPointFromWidgetToView(v2, v2_centerpoint)));
