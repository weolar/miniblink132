...
{
    return AnyViewMatchingPredicate(widget, [&](const View* view) { return view->GetClassName() == classname; });
}

WidgetTest::WidgetTest() = default;

WidgetTest::WidgetTest(std::unique_ptr<base::test::TaskEnvironment> task_environment)
...
Widget* WidgetTest::CreateTopLevelPlatformWidget(Widget::InitParams::Ownership ownership)
{
    auto widget = std::make_unique<Widget>();
    Widget::InitParams params = CreateParams(ownership, Widget::InitParams::TYPE_WINDOW);
...
Widget* WidgetTest::CreateTopLevelPlatformDesktopWidget(Widget::InitParams::Ownership ownership)
{
    auto widget = std::make_unique<Widget>();
    Widget::InitParams params = CreateParams(ownership, Widget::InitParams::TYPE_WINDOW);
    params.native_widget = CreatePlatformDesktopNativeWidgetImpl(widget.get(), kStubCapture, base::DoNothing());
    widget->Init(std::move(params));
    return widget.release();
}
#endif

Widget* WidgetTest::CreateTopLevelFramelessPlatformWidget(Widget::InitParams::Ownership ownership)
{
    auto widget = std::make_unique<Widget>();
    Widget::InitParams params = CreateParams(ownership, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    params.native_widget = CreatePlatformNativeWidgetImpl(widget.get(), kStubCapture, nullptr);
    widget->Init(std::move(params));
    return widget.release();
}

Widget* WidgetTest::CreateChildPlatformWidget(gfx::NativeView parent_native_view, Widget::InitParams::Ownership ownership)
{
    Widget::InitParams params = CreateParams(ownership, Widget::InitParams::TYPE_CONTROL);
    params.parent = parent_native_view;
    auto child = std::make_unique<Widget>();
    params.native_widget = CreatePlatformNativeWidgetImpl(child.get(), kStubCapture, nullptr);
    child->Init(std::move(params));
    child->SetContentsView(std::make_unique<View>());
    return child.release();
}

Widget* WidgetTest::CreateTopLevelNativeWidget(Widget::InitParams::Ownership ownership)
{
...
Widget* WidgetTest::CreateChildNativeWidgetWithParent(Widget* parent, Widget::InitParams::Ownership ownership)
{
    auto child = std::make_unique<Widget>();
    Widget::InitParams params = CreateParams(ownership, Widget::InitParams::TYPE_CONTROL);
...
{
    return contents_view_ ? contents_view_.get() : WidgetDelegate::GetContentsView();
}

bool TestDesktopWidgetDelegate::OnCloseRequested(Widget::ClosedReason close_reason)
{
    last_closed_reason_ = close_reason;
    return can_close_;
}

TestInitialFocusWidgetDelegate::TestInitialFocusWidgetDelegate(gfx::NativeWindow context)
{
    ...
    view_ = GetWidget()->GetContentsView()->AddChildView(std::make_unique<View>());
...
void WidgetVisibleWaiter::OnWidgetVisibilityChanged(Widget* widget, bool visible)
{
