...
public:
    explicit ViewSkiaGoldPixelDiff(const std::string& screenshot_prefix, const std::optional<std::string>& corpus = {});
    ...
    bool CompareViewScreenshot(
        const std::string& screenshot_name, const views::View* view, const ui::test::SkiaGoldMatchingAlgorithm* algorithm = nullptr) const;
    ...
    // local coordinates.
    bool CompareNativeWindowScreenshot(const std::string& screenshot_name, gfx::NativeWindow window, const gfx::Rect& snapshot_bounds,
    ...
    bool CompareNativeWindowScreenshotInRects(const std::string& screenshot_name, gfx::NativeWindow window, const gfx::Rect& snapshot_bounds,
...
protected:
    ...
    virtual bool GrabWindowSnapshotInternal(gfx::NativeWindow window, const gfx::Rect& snapshot_bounds, gfx::Image* image) const;

private:
    // Updates `image` so that only the pixels within `rects` are kept.
    void KeepPixelsInRects(const std::vector<gfx::Rect>& rects, gfx::Image* image) const;
