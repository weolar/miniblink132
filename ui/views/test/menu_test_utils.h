...
public:
    ...
    int show_context_menu_count() const
    {
        return show_context_menu_count_;
    }
    MenuItemView* show_context_menu_source()
    {
        return show_context_menu_source_;
    }
    int execute_command_id() const
    {
        return execute_command_id_;
    }
    int on_menu_closed_called() const
    {
        return on_menu_closed_called_count_;
    }
    MenuItemView* on_menu_closed_menu() const
    {
        return on_menu_closed_menu_;
    }
    bool is_drop_performed() const
    {
        return is_drop_performed_;
    }
    int will_hide_menu_count() const
    {
        return will_hide_menu_count_;
    }
    MenuItemView* will_hide_menu()
    {
        return will_hide_menu_;
    }
    ...
    // MenuDelegate:
    bool ShowContextMenu(MenuItemView* source, int id, const gfx::Point& p, ui::mojom::MenuSourceType source_type) override;
    void ExecuteCommand(int id) override;
    void OnMenuClosed(MenuItemView* menu) override;
    views::View::DropCallback GetDropCallback(MenuItemView* menu, DropPosition position, const ui::DropTargetEvent& event) override;
...
private:
    ...
    void PerformDrop(const ui::DropTargetEvent& event, ui::mojom::DragOperation& output_drag_op, std::unique_ptr<ui::LayerTreeOwner> drag_image_layer_owner);
