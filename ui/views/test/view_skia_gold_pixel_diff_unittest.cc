...
public:
    ...
    bool GrabWindowSnapshotInternal(gfx::NativeWindow window, const gfx::Rect& snapshot_bounds, gfx::Image* image) const override
    {
        SkBitmap bitmap;
...
public:
    using MockLaunchProcess = testing::MockFunction<ui::test::SkiaGoldPixelDiff::LaunchProcessCallback::RunType>;
    ...
    ViewSkiaGoldPixelDiffTest& operator=(const ViewSkiaGoldPixelDiffTest&) = delete;
...
protected:
    void SetUp() override
    {
        ...
        auto_reset_custom_launch_process_.emplace(
            ui::test::SkiaGoldPixelDiff::OverrideLaunchProcessForTesting(base::BindLambdaForTesting(mock_launch_process_->AsStdFunction())));
    }
...
private:
    std::optional<ui::test::SkiaGoldPixelDiff::ScopedSessionCacheForTesting> session_cache_;
    std::optional<MockLaunchProcess> mock_launch_process_;
    std::optional<base::AutoReset<ui::test::SkiaGoldPixelDiff::LaunchProcessCallback>> auto_reset_custom_launch_process_;
};
    ...
    EXPECT_CALL(mock_launch_process(),
        Call(ResultOf(
            ...
            [&](const base::CommandLine& cmdline) {
                return cmdline.argv()[1] == FILE_PATH_LITERAL("imgtest") && cmdline.argv()[2] == FILE_PATH_LITERAL("add")
                    && cmdline.GetSwitchValueASCII("test-name") == base::StrCat({ kPrefix, ui::test::SkiaGoldPixelDiff::GetPlatform() });
            },
            IsTrue())))
        .Times(1);
