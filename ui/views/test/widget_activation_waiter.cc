...
public:
    ...
    void Wait()
    {
        if (!observed_) {
            ...
            const base::test::ScopedRunLoopTimeout increased_run_timeout(FROM_HERE, TestTimeouts::action_max_timeout(), base::BindLambdaForTesting([&]() {
                return "Requested activation state: " + base::NumberToString(active_) + ", actual: " + base::NumberToString(widget_->native_widget_active());
            }));
#endif
            run_loop_.Run();
        }
