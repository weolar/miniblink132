...
void ExpectClose(Widget::Widgets* expected, std::vector<Widget*> widgets, const char* message)
{
    SCOPED_TRACE(message);
...
TEST_F(DesktopWidgetTestTest, GetAllWidgets)
{
    ...
    Widget* desktop_cousin = CreateChildPlatformWidget(desktop->GetNativeView()->GetRootWindow());
