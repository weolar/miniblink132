...
{
    Widget::SetDisableActivationChangeHandling(Widget::DisableActivationChangeHandlingType::kIgnore);
}
    ...
    : widget_(widget)
{
    ...
    if (ui::OzonePlatform::GetPlatformNameForTest() == "wayland") {
        ...
        // processed.
        auto* host = aura::WindowTreeHostPlatform::GetHostForWindow(widget_->GetNativeWindow());
...
{
    CHECK(waited_) << "AsyncWidgetRequestWaiter has no effect unless `Wait` is called.";
}
    ...
    if (ui::OzonePlatform::GetPlatformNameForTest() == "wayland") {
        ...
        CHECK(base::test::RunUntil([&]() {
            return !wayland_extension->HasInFlightRequestsForState()
                && wayland_extension->GetVizSequenceIdForAppliedState() == wayland_extension->GetVizSequenceIdForLatchedState();
        })) << "Has in flight requests: "
            << wayland_extension->HasInFlightRequestsForState() << ", applied sequence ID: " << wayland_extension->GetVizSequenceIdForAppliedState()
            << ", latched sequence ID:" << wayland_extension->GetVizSequenceIdForLatchedState();
