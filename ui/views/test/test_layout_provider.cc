...
void TestLayoutProvider::SetFontDetails(int context, int style, const ui::ResourceBundle::FontDetails& details)
{
...
ui::ResourceBundle::FontDetails TestLayoutProvider::GetFontDetailsImpl(int context, int style) const
{
    auto it = details_.find({ context, style });
    return it != details_.end() ? it->second : TypographyProvider::GetFontDetailsImpl(context, style);
}
