...
BOOL CALLBACK FindAllWindowsCallback(HWND hwnd, LPARAM param)
{
    ...
    if (aura::WindowTreeHost* host = aura::WindowTreeHost::GetForAcceleratedWidget(hwnd))
...
std::vector<aura::Window*> GetAllTopLevelWindows()
{
...
#if BUILDFLAG(IS_WIN)
    {
        ...
        EnumThreadWindows(GetCurrentThreadId(), FindAllWindowsCallback, reinterpret_cast<LPARAM>(&data));
    }
#elif BUILDFLAG(ENABLE_DESKTOP_AURA)
    roots = DesktopWindowTreeHostPlatform::GetAllOpenWindows();
#endif
    aura::test::AuraTestHelper* aura_test_helper = aura::test::AuraTestHelper::GetInstance();
...
// static
ui::ImeKeyEventDispatcher* WidgetTest::GetImeKeyEventDispatcherForWidget(Widget* widget)
{
