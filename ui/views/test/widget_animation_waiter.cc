...
WidgetAnimationWaiter::WidgetAnimationWaiter(Widget* widget, const gfx::Rect& target_bounds)
...
void WidgetAnimationWaiter::OnLayerAnimationEnded(ui::LayerAnimationSequence* sequence)
{
    Widget* widget = widget_observation_.GetSource();
    if (!widget->GetLayer()->GetAnimator()->is_animating() && animation_scheduled_) {
...
void WidgetAnimationWaiter::OnLayerAnimationAborted(ui::LayerAnimationSequence* sequence)
{
}

void WidgetAnimationWaiter::OnLayerAnimationScheduled(ui::LayerAnimationSequence* sequence)
{
    animation_scheduled_ = true;
    if (!target_bounds_.IsEmpty())
        EXPECT_NE(widget_observation_.GetSource()->GetLayer()->transform(), gfx::Transform());
}
