...
template <typename Pred> View* AnyViewMatchingPredicate(View* root, Pred predicate)
{
...
template <typename Pred> View* AnyViewMatchingPredicate(Widget* widget, Pred predicate)
{
    return AnyViewMatchingPredicate(widget, base::BindLambdaForTesting(predicate));
}
...
public:
    WidgetTest();
    explicit WidgetTest(std::unique_ptr<base::test::TaskEnvironment> task_environment);
    ...
    Widget* CreateTopLevelPlatformWidget(Widget::InitParams::Ownership ownership = Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET);
    Widget* CreateTopLevelFramelessPlatformWidget(Widget::InitParams::Ownership ownership = Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET);
    Widget* CreateChildPlatformWidget(
        gfx::NativeView parent_native_view, Widget::InitParams::Ownership ownership = Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET);

#if BUILDFLAG(ENABLE_DESKTOP_AURA)
    // Create Widgets with |native_widget| in InitParams set to an instance of
    // platform specific widget type that has stubbled capture calls. This will
    // create a desktop widget.
    Widget* CreateTopLevelPlatformDesktopWidget(Widget::InitParams::Ownership ownership = Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET);
#endif
    ...
    Widget* CreateTopLevelNativeWidget(Widget::InitParams::Ownership ownership = Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET);
    Widget* CreateChildNativeWidgetWithParent(Widget* parent, Widget::InitParams::Ownership ownership = Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET);
    ...
    static ui::ImeKeyEventDispatcher* GetImeKeyEventDispatcherForWidget(Widget* widget);
...
public:
    ...
    DesktopWidgetTestInteractive& operator=(const DesktopWidgetTestInteractive&) = delete;
...
public:
    ...
    TestDesktopWidgetDelegate& operator=(const TestDesktopWidgetDelegate&) = delete;
    ...
    int window_closing_count() const
    {
        return window_closing_count_;
    }
...
public:
    ...
    TestInitialFocusWidgetDelegate(const TestInitialFocusWidgetDelegate&) = delete;
    TestInitialFocusWidgetDelegate& operator=(const TestInitialFocusWidgetDelegate&) = delete;
