...
public:
    ...
    NOINLINE explicit ViewsTestBase(TaskEnvironmentTraits&&... traits)
        : ViewsTestBase(
            std::make_unique<base::test::TaskEnvironment>(base::test::TaskEnvironment::MainThreadType::UI, std::forward<TaskEnvironmentTraits>(traits)...))
    {
    }

    // Alternatively a subclass may pass a TaskEnvironment directly.
    explicit ViewsTestBase(std::unique_ptr<base::test::TaskEnvironment> task_environment);
    ...
    // GetContext().
    virtual Widget::InitParams CreateParams(Widget::InitParams::Ownership ownership, Widget::InitParams::Type type);
    ...
    virtual std::unique_ptr<Widget> CreateTestWidget(
        Widget::InitParams::Ownership ownership, Widget::InitParams::Type type = Widget::InitParams::TYPE_WINDOW_FRAMELESS);
...
protected:
    ...
    template <typename T> T* set_views_delegate(std::unique_ptr<T> views_delegate)
    {
        DCHECK(!setup_called_);
    ...
    ui::EventSink* GetEventSink()
    {
        return host()->GetEventSink();
    }
    ...
    // kDesktop.
    NativeWidget* CreateNativeWidgetForTest(const Widget::InitParams& init_params, internal::NativeWidgetDelegate* delegate);
    ...
    Widget::InitParams CreateParamsForTestWidget(
        views::Widget::InitParams::Ownership ownership, views::Widget::InitParams::Type type = views::Widget::InitParams::TYPE_WINDOW_FRAMELESS);

    // TODO(crbug.com/339619005): Remove once all uses are explicitly specifying
    // Widget ownership.
    Widget::InitParams CreateParamsForTestWidget(views::Widget::InitParams::Type type = views::Widget::InitParams::TYPE_WINDOW_FRAMELESS);
...
public:
    ...
    ViewsTestWithDesktopNativeWidget(const ViewsTestWithDesktopNativeWidget&) = delete;
    ViewsTestWithDesktopNativeWidget& operator=(const ViewsTestWithDesktopNativeWidget&) = delete;
