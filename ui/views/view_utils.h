...
VIEWS_EXPORT extern const ui::ClassProperty<base::debug::StackTrace*>* const kViewStackTraceKey;
...
template <typename V> bool IsViewClass(const View* view)
{
    return ui::metadata::IsClass<V, View>(view);
}

template <typename V> V* AsViewClass(View* view)
{
    return IsViewClass<V>(view) ? static_cast<V*>(view) : nullptr;
}

template <typename V> const V* AsViewClass(const View* view)
{
    return IsViewClass<V>(view) ? static_cast<const V*>(view) : nullptr;
}

VIEWS_EXPORT void PrintViewHierarchy(View* view, bool verbose = false, int depth = -1);
