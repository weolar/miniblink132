...
public:
        ...
        : mouse_watcher_(mouse_watcher)
    {
        event_monitor_ = EventMonitor::CreateApplicationMonitor(
            this, window, { ui::EventType::kMousePressed, ui::EventType::kMouseMoved, ui::EventType::kMouseExited, ui::EventType::kMouseDragged });
    }
...
private:
    MouseWatcherHost* host() const
    {
        return mouse_watcher_->host_.get();
    }
        ...
        if (!host()->Contains(event_monitor_->GetLastMouseLocation(), event_type)) {
            if (event_type == EventType::kPress) {
                ...
                base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE,
                    base::BindOnce(&Observer::NotifyListener, notify_listener_factory_.GetWeakPtr()),
                    event_type == EventType::kMove ? kNotifyListenerTime : mouse_watcher_->notify_on_exit_time_);
            }
        } else {
...
MouseWatcher::MouseWatcher(std::unique_ptr<MouseWatcherHost> host, MouseWatcherListener* listener)
    : host_(std::move(host))
    , listener_(listener)
