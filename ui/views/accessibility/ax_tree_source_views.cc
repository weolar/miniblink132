...
AXTreeSourceViews::AXTreeSourceViews(ui::AXNodeID root_id, const ui::AXTreeID& tree_id, views::AXAuraObjCache* cache)
    : root_id_(root_id)
    , tree_id_(tree_id)
    , cache_(cache)
{
...
AXAuraObjWrapper* AXTreeSourceViews::ChildAt(AXAuraObjWrapper* node, size_t index) const
{
    std::vector<raw_ptr<AXAuraObjWrapper, VectorExperimental>> children;
    node->GetChildren(&children);
...
bool AXTreeSourceViews::IsEqual(AXAuraObjWrapper* node1, AXAuraObjWrapper* node2) const
{
...
void AXTreeSourceViews::SerializeNode(AXAuraObjWrapper* node, ui::AXNodeData* out_data) const
{
    node->Serialize(out_data);

    if (out_data->role == ax::mojom::Role::kWindow || out_data->role == ax::mojom::Role::kDialog) {
    ...
    out_data->relative_bounds.bounds.Offset(-parent_data.relative_bounds.bounds.OffsetFromOrigin());
    out_data->relative_bounds.offset_container_id = parent->GetUniqueId();
}

std::string AXTreeSourceViews::ToString(AXAuraObjWrapper* root, std::string prefix)
{
    ui::AXNodeData data;
    root->Serialize(&data);
