...
template <typename T, typename U> bool IsSameObject(T* left, U* right)
{
    if (!left && !right)
        return true;
...
protected:
    ...
    ComPtr<IRawElementProviderSimple> GetIRawElementProviderSimple(View* view)
    {
        ...
        EXPECT_HRESULT_SUCCEEDED(view->GetNativeViewAccessible()->QueryInterface(__uuidof(IRawElementProviderSimple), &result));
        return result;
    }
...
TEST_F(ViewAXPlatformNodeDelegateWinTest, TextfieldAccessibility)
{
    ...
    ASSERT_EQ(S_OK, content_accessible->get_accChild(child_index, &textfield_dispatch));
    ...
    EXPECT_EQ(0, child_count) << "Text fields should be leaf nodes on this platform, otherwise no "
    ...
    ASSERT_EQ(S_OK, textfield_accessible->put_accValue(childid_self, new_value.Get()));
...
TEST_F(ViewAXPlatformNodeDelegateWinTest, TextfieldAssociatedLabel)
{
    ...
    IUnknown** targets;
    LONG n_targets;
    EXPECT_EQ(S_OK, textfield_ia2->get_relationTargetsOfType(type.Get(), 0, &targets, &n_targets));
...
class ViewAXPlatformNodeDelegateWinTestWithBoolChildFlag : public ViewAXPlatformNodeDelegateWinTest, public testing::WithParamInterface<bool> {
public:
    ViewAXPlatformNodeDelegateWinTestWithBoolChildFlag() = default;
    ViewAXPlatformNodeDelegateWinTestWithBoolChildFlag(const ViewAXPlatformNodeDelegateWinTestWithBoolChildFlag&) = delete;
    ViewAXPlatformNodeDelegateWinTestWithBoolChildFlag& operator=(const ViewAXPlatformNodeDelegateWinTestWithBoolChildFlag&) = delete;
    ~ViewAXPlatformNodeDelegateWinTestWithBoolChildFlag() override = default;
};

INSTANTIATE_TEST_SUITE_P(All, ViewAXPlatformNodeDelegateWinTestWithBoolChildFlag, testing::Bool());
...
TEST_P(ViewAXPlatformNodeDelegateWinTestWithBoolChildFlag, AuraChildWidgets)
{
    ...
    ComPtr<IAccessible> root_view_accessible(widget->GetRootView()->GetNativeViewAccessible());
    LONG child_count = 0;
    ...
    Widget::InitParams child_init_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_BUBBLE);
    ...
    ASSERT_EQ(S_OK, root_view_accessible->accLocation(&x, &y, &width, &height, childid_self));
    EXPECT_EQ(0, x);
    EXPECT_EQ(0, y);
    EXPECT_EQ(400, width);
    ...
    ASSERT_EQ(S_OK, root_view_accessible->get_accChild(child_index_2, &child_widget_dispatch));
    ...
    ASSERT_EQ(S_OK, child_widget_accessible->accLocation(&x, &y, &width, &height, childid_self));
    EXPECT_EQ(30, x);
    EXPECT_EQ(40, y);
    EXPECT_EQ(100, width);
    EXPECT_EQ(50, height);
    ...
    ASSERT_EQ(S_OK, child_widget_accessible->get_accParent(&child_widget_parent_dispatch));
    ASSERT_EQ(S_OK, child_widget_parent_dispatch.As(&child_widget_parent_accessible));
...
TEST_F(ViewAXPlatformNodeDelegateWinTest, DISABLED_RetrieveAllAlerts)
{
    ...
    IUnknown** targets;
    LONG n_targets;
    ASSERT_EQ(S_FALSE, root_view_accessible->get_relationTargetsOfType(alerts_bstr.Get(), 0, &targets, &n_targets));
    ...
    ASSERT_EQ(S_OK, root_view_accessible->get_relationTargetsOfType(alerts_bstr.Get(), 0, &targets, &n_targets));
    ASSERT_EQ(2, n_targets);
    {
        ...
        UNSAFE_BUFFERS(base::span<IUnknown*> targets_span(targets, base::checked_cast<size_t>(n_targets));)
    ...
    ASSERT_EQ(S_OK, root_view_accessible->get_relationTargetsOfType(alerts_bstr.Get(), 1, &targets, &n_targets));
    ASSERT_EQ(1, n_targets);
...
TEST_F(ViewAXPlatformNodeDelegateWinTest, GetAllOwnedWidgetsCrash)
{
    ...
    LONG child_count = 0;
    ComPtr<IAccessible> content_accessible(widget.GetRootView()->GetNativeViewAccessible());
...
TEST_F(ViewAXPlatformNodeDelegateWinTest, WindowHasRoleApplication)
{
    ...
    ComPtr<IAccessible> accessible(widget->GetRootView()->GetNativeViewAccessible());
    ScopedVariant childid_self(CHILDID_SELF);
    ScopedVariant role;
...
TEST_F(ViewAXPlatformNodeDelegateWinTest, Overrides)
{
    ...
    alert_view->GetViewAccessibility().SetName(u"Name", ax::mojom::NameFrom::kAttribute);
    ...
    ComPtr<IAccessible> content_accessible(contents_view->GetNativeViewAccessible());
    ScopedVariant child_index(1);
    ...
    ASSERT_EQ(S_OK, content_accessible->get_accDescription(child_index, description.Receive()));
    ...
    ASSERT_EQ(S_OK, content_accessible->get_accChild(child_index, &alert_dispatch));
    ...
    ASSERT_EQ(E_INVALIDARG, alert_accessible->get_accChild(child_index, &child_dispatch));
...
TEST_F(ViewAXPlatformNodeDelegateWinTest, GridRowColumnCount)
{
    ...
    // table row/column count = not set
    grid->SetAriaTableSize(ax::mojom::kUnknownAriaColumnOrRowCount, ax::mojom::kUnknownAriaColumnOrRowCount);
    EXPECT_EQ(E_UNEXPECTED, grid_provider->get_RowCount(&row_count));
    ...
    grid->SetTableSize(ax::mojom::kUnknownAriaColumnOrRowCount, ax::mojom::kUnknownAriaColumnOrRowCount);
    EXPECT_EQ(E_UNEXPECTED, grid_provider->get_RowCount(&row_count));
    EXPECT_EQ(E_UNEXPECTED, grid_provider->get_ColumnCount(&column_count));
}
...
TEST_F(ViewAXPlatformNodeDelegateWinTest, IsUIAControlIsTrueEvenWhenReadonly)
{
    ...
    ComPtr<IRawElementProviderSimple> textfield_provider = GetIRawElementProviderSimple(text_field);
...
TEST_F(ViewAXPlatformNodeDelegateWinTest, UIAGetPropertyValue_Histograms)
{
    ...
    histogram_tester.ExpectTotalCount("Accessibility.Performance.WinAPIs2.View.UMA_API_GET_PROPERTY_VALUE", 0);

    ASSERT_HRESULT_SUCCEEDED(textfield_provider->GetPropertyValue(UIA_IsControlElementPropertyId, actual.Receive()));

    histogram_tester.ExpectTotalCount("Accessibility.Performance.WinAPIs2.View.UMA_API_GET_PROPERTY_VALUE", 1);
}
...
TEST_F(ViewAXPlatformNodeDelegateWinTest, TextPositionAt)
{
    ...
    ViewAXPlatformNodeDelegate* label_accessibility = static_cast<ViewAXPlatformNodeDelegate*>(&label->GetViewAccessibility());
    label_accessibility->GetData();

    ui::AXNodePosition::AXPositionInstance actual_position = label_accessibility->CreateTextPositionAt(0, ax::mojom::TextAffinity::kDownstream);
...
class ViewAXPlatformNodeDelegateWinTableTest : public ViewAXPlatformNodeDelegateWinTest {
    void SetUp() override
    {
        ...
        auto table = std::make_unique<TableView>(model_.get(), columns, TableType::kTextOnly, true);
        table_ = table.get();
        ...
        content->AddChildView(TableView::CreateScrollViewWithTable(std::move(table)));
        widget_->Show();
    }
...
TEST_F(ViewAXPlatformNodeDelegateWinTableTest, TableCellAttributes)
{
    ...
    auto get_attributes = [&](int row_child, int cell_child) -> std::wstring {
        ...
        CHECK_EQ(S_OK, table_accessible->get_accChild(ScopedVariant(row_child), &row_dispatch));
        ...
        CHECK_EQ(S_OK, row->get_accChild(ScopedVariant(cell_child), &cell_dispatch));
    ...
    EXPECT_EQ(get_attributes(1, 1), L"name-from:attribute;explicit-name:true;sort:none;class:AXVirtualView;");
    EXPECT_EQ(get_attributes(1, 2), L"name-from:attribute;explicit-name:true;sort:none;class:AXVirtualView;");
...
class ViewAXPlatformNodeDelegateWinInnerTextRangeTest : public test::ViewAXPlatformNodeDelegateWinTest {
public:
    void SetUp() override
    {
        ViewAXPlatformNodeDelegateWinTest::SetUp();
    ...
    {
        return static_cast<ViewAXPlatformNodeDelegate*>(&textfield_->GetViewAccessibility());
    }
    ViewAXPlatformNodeDelegate* label_delegate()
    {
        return static_cast<ViewAXPlatformNodeDelegate*>(&label_->GetViewAccessibility());
    }
...
TEST_F(ViewAXPlatformNodeDelegateWinInnerTextRangeTest, EmptyTextfield_NonEmptyRect)
{
    ...
    gfx::Rect bounds
        = textfield_delegate()->GetInnerTextRangeBoundsRect(0, 1, ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kClipped, &offscreen_result);
    EXPECT_EQ(gfx::Rect(insets.left(), insets.top(), 1, textfield_bounds.height() - insets.top() - insets.bottom()), bounds);
...
TEST_F(ViewAXPlatformNodeDelegateWinInnerTextRangeTest, EmptyLabel_EmptyRect)
{
    ...
    // An empty text field should expose bounds with a fixed width of 1.
    bounds = label_delegate()->GetInnerTextRangeBoundsRect(0, 1, ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kClipped, &offscreen_result);
...
TEST_F(ViewAXPlatformNodeDelegateWinInnerTextRangeTest, Textfield_LTR)
{
    ...
    constexpr gfx::Range kRange2 = gfx::Range(1, 2); // Range of the middle glyph.
    ...
    bounds = textfield_delegate()->GetInnerTextRangeBoundsRect(
        kRange1.start(), kRange1.end(), ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kClipped, &offscreen_result);
    EXPECT_EQ(gfx::Rect(initial_x, insets.top(), kGlyphWidth, height), bounds);
    EXPECT_EQ(offscreen_result, ui::AXOffscreenResult::kOnscreen);

    // Range 2: middle glyph.
    bounds = textfield_delegate()->GetInnerTextRangeBoundsRect(
        kRange2.start(), kRange2.end(), ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kClipped, &offscreen_result);
    EXPECT_EQ(gfx::Rect(initial_x + kGlyphWidth, insets.top(), kGlyphWidth, height), bounds);
    EXPECT_EQ(offscreen_result, ui::AXOffscreenResult::kOnscreen);

    // Range 3: 'b'.
    bounds = textfield_delegate()->GetInnerTextRangeBoundsRect(
        kRange3.start(), kRange3.end(), ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kClipped, &offscreen_result);
    EXPECT_EQ(gfx::Rect(initial_x + 2 * kGlyphWidth, insets.top(), kGlyphWidth, height), bounds);
    EXPECT_EQ(offscreen_result, ui::AXOffscreenResult::kOnscreen);

    // Range 4: all text.
    bounds = textfield_delegate()->GetInnerTextRangeBoundsRect(
        kRange4.start(), kRange4.end(), ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kClipped, &offscreen_result);
    EXPECT_EQ(gfx::Rect(initial_x, insets.top(), 3 * kGlyphWidth, height), bounds);
    EXPECT_EQ(offscreen_result, ui::AXOffscreenResult::kOnscreen);
}

TEST_F(ViewAXPlatformNodeDelegateWinInnerTextRangeTest, Textfield_TextOverflow)
{
    const ::ui::ScopedAXModeSetter ax_mode_setter(ui::AXMode::kNativeAPIs);
    MockAXModeAdded();
    ...
    EXPECT_EQ(textfield_delegate()->GetData().GetIntAttribute(ax::mojom::IntAttribute::kScrollX), 0);
    ...
    int scroll_x = textfield_delegate()->GetData().GetIntAttribute(ax::mojom::IntAttribute::kScrollX);
    ...
    bounds = textfield_delegate()->GetInnerTextRangeBoundsRect(
        kRange2.start(), kRange2.end(), ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kClipped, &offscreen_result);
    EXPECT_EQ(gfx::Rect(initial_x + kRange2.start() * kGlyphWidth + scroll_x, insets.top(), kGlyphWidth, height), bounds);
    EXPECT_EQ(offscreen_result, ui::AXOffscreenResult::kOnscreen);
}
...
TEST_F(ViewAXPlatformNodeDelegateWinInnerTextRangeTest, Label_LTR)
{
    ...
    gfx::test::RenderTextTestApi render_text_test_api(label_->display_text_.get());
...
TEST_F(ViewAXPlatformNodeDelegateWinInnerTextRangeTest, Textfield_RTL)
{
    ...
    bounds = textfield_delegate()->GetInnerTextRangeBoundsRect(
        kRange4.start(), kRange4.end(), ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kClipped, &offscreen_result);
    EXPECT_EQ(gfx::Rect(initial_x - 3 * kGlyphWidth, insets.top(), kGlyphWidth, height), bounds);
    EXPECT_EQ(offscreen_result, ui::AXOffscreenResult::kOnscreen);

    // Range 5.
    bounds = textfield_delegate()->GetInnerTextRangeBoundsRect(
        kRange5.start(), kRange5.end(), ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kClipped, &offscreen_result);
    // TODO(accessibility): The x offset here should be smaller than the one of
    // the previous glyph, but it's not. Investigate.
    EXPECT_EQ(gfx::Rect(initial_x - 4 * kGlyphWidth, insets.top(), kGlyphWidth, height), bounds);
    EXPECT_EQ(offscreen_result, ui::AXOffscreenResult::kOnscreen);

    // Range 6.
    bounds = textfield_delegate()->GetInnerTextRangeBoundsRect(
        kRange6.start(), kRange6.end(), ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kClipped, &offscreen_result);
    EXPECT_EQ(gfx::Rect(initial_x - 4 * kGlyphWidth, insets.top(), 5 * kGlyphWidth, height), bounds);
    EXPECT_EQ(offscreen_result, ui::AXOffscreenResult::kOnscreen);
}
...
TEST_F(ViewAXPlatformNodeDelegateWinInnerTextRangeTest, Textfield_CreatePositionAt)
{
    ...
    position = textfield_delegate()->CreatePositionAt(0, ax::mojom::TextAffinity::kDownstream);
    ...
    position = textfield_delegate()->CreatePositionAt(kText.length(), ax::mojom::TextAffinity::kDownstream);
...
TEST_F(ViewAXPlatformNodeDelegateWinInnerTextRangeTest, Textfield_ScreenPhysicalPixels)
{
    const ::ui::ScopedAXModeSetter ax_mode_setter(ui::AXMode::kNativeAPIs);
    MockAXModeAdded();
    DCHECK(label_delegate()->is_initialized());
    ...
    bounds = textfield_delegate()->GetInnerTextRangeBoundsRect(
        0, 1, ui::AXCoordinateSystem::kScreenPhysicalPixels, ui::AXClippingBehavior::kClipped, &offscreen_result);
    EXPECT_EQ(gfx::Rect(2 * insets.left(), 0 + insets.top(), kGlyphWidth, textfield_bounds.height() - insets.top() - insets.bottom()), bounds);
    EXPECT_EQ(offscreen_result, ui::AXOffscreenResult::kOnscreen);
}
