...
base::LazyInstance<std::vector<QueuedEvent>>::Leaky g_event_queue = LAZY_INSTANCE_INITIALIZER;
...
{
    return view->GetFocusBehavior() != View::FocusBehavior::NEVER && view->IsDrawn();
}
...
ui::AXPlatformNode* FromNativeWindow(gfx::NativeWindow native_window)
{
    ...
    gfx::NativeViewAccessible native_view_accessible = view->GetNativeViewAccessible();
...
void PostFlushEventQueueTaskIfNecessary()
{
    if (!g_is_queueing_events) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(cb));
    }
}
...
ViewAXPlatformNodeDelegate::ChildWidgetsResult::ChildWidgetsResult(std::vector<raw_ptr<Widget, VectorExperimental>> child_widgets, bool is_tab_modal_showing)
    : child_widgets(child_widgets)
    , is_tab_modal_showing(is_tab_modal_showing)
{
}
ViewAXPlatformNodeDelegate::ChildWidgetsResult::ChildWidgetsResult(const ViewAXPlatformNodeDelegate::ChildWidgetsResult& other) = default;

ViewAXPlatformNodeDelegate::ChildWidgetsResult::~ChildWidgetsResult() = default;

ViewAXPlatformNodeDelegate::ChildWidgetsResult& ViewAXPlatformNodeDelegate::ChildWidgetsResult::operator=(
    const ViewAXPlatformNodeDelegate::ChildWidgetsResult& other)
    = default;
...
void ViewAXPlatformNodeDelegate::Init()
{
    ...
    if (first_time) {
        ui::AXPlatformNode::RegisterNativeWindowHandler(base::BindRepeating(&FromNativeWindow));
        first_time = false;
    }
}
...
{
    if (ui::AXPlatformNode::GetPopupFocusOverride() == ax_platform_node_->GetNativeViewAccessible()) {
...
bool ViewAXPlatformNodeDelegate::IsFocusedForTesting() const
{
    if (ui::AXPlatformNode::GetPopupFocusOverride()) {
        return ui::AXPlatformNode::GetPopupFocusOverride() == GetNativeViewAccessible();
    }
...
{
    ui::AXPlatformNodeBase* focused_node = static_cast<ui::AXPlatformNodeBase*>(ax_platform_node_);
    ...
    while (focused_node) {
        ui::AXPlatformNodeBase* deeper_focus = static_cast<ui::AXPlatformNodeBase*>(ui::AXPlatformNode::FromNativeViewAccessible(focused_node->GetFocus()));
    ...
    if (focused_node) {
        ...
        if (accessibility_events_callback_) {
            accessibility_events_callback_.Run(this, ax::mojom::Event::kFocusAfterMenuClose);
        }

        focused_node->NotifyAccessibilityEvent(ax::mojom::Event::kFocusAfterMenuClose);
    }
}
...
void ViewAXPlatformNodeDelegate::FireNativeEvent(ax::mojom::Event event_type)
{
    ...
    case ax::mojom::Event::kFocusAfterMenuClose: {
        DCHECK(!ui::AXPlatformNode::GetPopupFocusOverride()) << "Must call ViewAccessibility::EndPopupFocusOverride() as menu "
                                                                "closes.";
        break;
    }
    case ax::mojom::Event::kFocus: {
        if (ui::AXPlatformNode::GetPopupFocusOverride()) {
            DCHECK_EQ(ui::AXPlatformNode::GetPopupFocusOverride(), GetNativeViewAccessible())
...
#if BUILDFLAG(IS_MAC)
void ViewAXPlatformNodeDelegate::AnnounceTextAs(const std::u16string& text, ui::AXPlatformNode::AnnouncementType announcement_type)
{
...
size_t ViewAXPlatformNodeDelegate::GetChildCount() const
{
    ...
    if (!virtual_children().empty()) {
        ...
        for (const std::unique_ptr<AXVirtualView>& virtual_child : virtual_children()) {
    ...
    for (View* child : view()->children()) {
        ...
        if (view_accessibility.GetIsIgnored()) {
            const auto* child_view_delegate = static_cast<const ViewAXPlatformNodeDelegate*>(&view_accessibility);
...
gfx::NativeViewAccessible ViewAXPlatformNodeDelegate::ChildAtIndex(size_t index) const
{
    DCHECK_LT(index, GetChildCount()) << "|index| should be less than the unignored child count.";
    if (IsLeaf()) {
        return nullptr;
    }
    ...
    const ChildWidgetsResult child_widgets_result = GetChildWidgets();
    const std::vector<raw_ptr<Widget, VectorExperimental>>& child_widgets = child_widgets_result.child_widgets;
    ...
    for (View* child : view()->children()) {
        ...
        if (view_accessibility.GetIsIgnored()) {
            auto* child_view_delegate = static_cast<ViewAXPlatformNodeDelegate*>(&view_accessibility);
            DCHECK(child_view_delegate);
    ...
    CHECK_LT(index, child_widgets_result.child_widgets.size()) << "|index| should be less than the unignored child count.";
...
std::wstring ViewAXPlatformNodeDelegate::ComputeListItemNameFromContent() const
{
...
const ui::AXSelection ViewAXPlatformNodeDelegate::GetUnignoredSelection() const
{
    const ui::AXNodeData& data = GetData();
    ui::AXSelection selection;
    selection.is_backward = false;
    selection.anchor_object_id = GetUniqueId();
    selection.anchor_offset = data.GetIntAttribute(ax::mojom::IntAttribute::kTextSelStart);
    selection.anchor_affinity = ax::mojom::TextAffinity::kDownstream;
    selection.focus_object_id = GetUniqueId();
    selection.focus_offset = data.GetIntAttribute(ax::mojom::IntAttribute::kTextSelEnd);
...
ui::AXNodePosition::AXPositionInstance ViewAXPlatformNodeDelegate::CreatePositionAt(int offset, ax::mojom::TextAffinity affinity) const
{
    return CreateTextPositionAt(offset, affinity);
}

ui::AXNodePosition::AXPositionInstance ViewAXPlatformNodeDelegate::CreateTextPositionAt(int offset, ax::mojom::TextAffinity affinity) const
{
    if (!atomic_view_ax_tree_manager_) {
        return ui::AXNodePosition::CreateNullPosition();
    }

    return ui::AXNodePosition::CreateTextPosition(*atomic_view_ax_tree_manager_->GetRoot(), offset, affinity);
}
...
gfx::NativeViewAccessible ViewAXPlatformNodeDelegate::GetNativeViewAccessible() const
{
    // TODO(nektar): Make "GetNativeViewAccessible" const throughout the codebase.
    return const_cast<ViewAXPlatformNodeDelegate*>(this)->GetNativeViewAccessible();
}
...
gfx::NativeViewAccessible ViewAXPlatformNodeDelegate::GetParent() const
{
    if (View* parent_view = view()->parent()) {
        ...
        auto* parent_view_delegate = static_cast<ViewAXPlatformNodeDelegate*>(&view_accessibility);
...
gfx::Rect ViewAXPlatformNodeDelegate::GetBoundsRect(
    const ui::AXCoordinateSystem coordinate_system, const ui::AXClippingBehavior clipping_behavior, ui::AXOffscreenResult* offscreen_result) const
{
...
gfx::Rect ViewAXPlatformNodeDelegate::GetInnerTextRangeBoundsRect(const int start_offset, const int end_offset, const ui::AXCoordinateSystem coordinate_system,
    const ui::AXClippingBehavior clipping_behavior, ui::AXOffscreenResult* offscreen_result) const
{
    switch (coordinate_system) {
    case ui::AXCoordinateSystem::kScreenDIPs: {
        ...
        if (ui::IsTextField(data_.role)) {
            ...
            if (bounds.IsEmpty() && offscreen_result && *offscreen_result == ui::AXOffscreenResult::kOnscreen) {
...
gfx::RectF ViewAXPlatformNodeDelegate::GetInlineTextRect(const int start_offset, const int end_offset) const
{
    DCHECK(start_offset >= 0 && end_offset >= 0 && start_offset <= end_offset);
    const std::vector<int32_t>& character_offsets = data_.GetIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets);
    if (character_offsets.empty()) {
        return gfx::RectF();
    }

    const size_t adjusted_start = std::min(static_cast<size_t>(start_offset), character_offsets.size() - 1);
    const size_t adjusted_end = std::min(static_cast<size_t>(end_offset), character_offsets.size() - 1);

    // Finding the left/right most offsets allow us to return the right bounds
    // whether we're in LTR or RTL.
    const int left_most_offset = std::min(character_offsets[adjusted_start], character_offsets[adjusted_end]);
    const int right_most_offset = std::max(character_offsets[adjusted_start], character_offsets[adjusted_end]);

    if (left_most_offset == right_most_offset) {
        return gfx::RectF();
    }

    return gfx::RectF(left_most_offset, 0, right_most_offset - left_most_offset, view()->GetContentsBounds().height());
}

gfx::RectF ViewAXPlatformNodeDelegate::RelativeToContainerBounds(const gfx::RectF& bounds, ui::AXOffscreenResult* offscreen_result) const
{
    ...
    if (offscreen_result) {
        *offscreen_result = !bounds.IsEmpty() && intersection.IsEmpty() ? ui::AXOffscreenResult::kOffscreen : ui::AXOffscreenResult::kOnscreen;
    }
    return intersection;
}

gfx::NativeViewAccessible ViewAXPlatformNodeDelegate::HitTestSync(int screen_physical_pixel_x, int screen_physical_pixel_y) const
{
    ...
    gfx::Point point = ScreenToDIPPoint(gfx::Point(screen_physical_pixel_x, screen_physical_pixel_y));
    ...
    if (!virtual_children().empty()) {
        ...
        for (const std::unique_ptr<AXVirtualView>& child : base::Reversed(virtual_children())) {
            gfx::NativeViewAccessible result = child->HitTestSync(screen_physical_pixel_x, screen_physical_pixel_y);
            if (result)
                return result;
        }
    ...
    const auto i = base::ranges::find_if(base::Reversed(v->children()), is_point_in_child);
    // If it's not inside any of our children, it's inside this view.
    return (i == v->children().rend()) ? GetNativeViewAccessible() : (*i)->GetNativeViewAccessible();
}

gfx::NativeViewAccessible ViewAXPlatformNodeDelegate::GetFocus() const
{
    gfx::NativeViewAccessible focus_override = ui::AXPlatformNode::GetPopupFocusOverride();
    if (focus_override)
...
ui::AXPlatformNode* ViewAXPlatformNodeDelegate::GetFromTreeIDAndNodeID(const ui::AXTreeID& ax_tree_id, int32_t id)
{
    if (atomic_view_ax_tree_manager_) {
...
bool ViewAXPlatformNodeDelegate::AccessibilityPerformAction(const ui::AXActionData& data)
{
...
AtomicViewAXTreeManager* ViewAXPlatformNodeDelegate::GetAtomicViewAXTreeManagerForTesting() const
{
    return atomic_view_ax_tree_manager_.get();
}

gfx::Point ViewAXPlatformNodeDelegate::ScreenToDIPPoint(const gfx::Point& screen_point) const
{
    if (!view() || !view()->GetWidget()) {
        return screen_point;
    }
    ...
    return gfx::Point(screen_point.x() / scale_factor, screen_point.y() / scale_factor);
}
...
std::vector<int32_t> ViewAXPlatformNodeDelegate::GetColHeaderNodeIds() const
{
    ...
    if (!virtual_children().empty()) {
        for (const std::unique_ptr<AXVirtualView>& header_cell : virtual_children().front()->children()) {
...
std::vector<int32_t> ViewAXPlatformNodeDelegate::GetColHeaderNodeIds(int col_index) const
{
...
std::optional<int32_t> ViewAXPlatformNodeDelegate::GetCellId(int row_index, int col_index) const
{
    if (virtual_children().empty() || !GetAncestorTableView())
        return std::nullopt;

    AXVirtualView* ax_cell = GetAncestorTableView()->GetVirtualAccessibilityCell(static_cast<size_t>(row_index), static_cast<size_t>(col_index));
    if (!ax_cell)
        return std::nullopt;

    const ui::AXNodeData& cell_data = ax_cell->GetData();
    if (cell_data.role == ax::mojom::Role::kCell || cell_data.role == ax::mojom::Role::kGridCell) {
        return cell_data.id;
    }
...
{
    const ui::AXNodeData& data = GetData();
    return (view()->GetGroup() >= 0) || (data.HasIntAttribute(ax::mojom::IntAttribute::kPosInSet) && data.HasIntAttribute(ax::mojom::IntAttribute::kSetSize));
}

bool ViewAXPlatformNodeDelegate::IsOrderedSet() const
{
    return (view()->GetGroup() >= 0) || GetData().HasIntAttribute(ax::mojom::IntAttribute::kSetSize);
}
...
std::optional<int> ViewAXPlatformNodeDelegate::GetPosInSet() const
{
    ...
    int pos_in_set = base::checked_cast<int>(std::distance(views_in_group.begin(), found_view));
...
void ViewAXPlatformNodeDelegate::GetViewsInGroupForSet(std::vector<raw_ptr<View, VectorExperimental>>* views_in_group) const
{
    ...
    std::erase_if(*views_in_group, [](View* view) { return view->GetViewAccessibility().GetIsIgnored(); });
}

bool ViewAXPlatformNodeDelegate::TableHasColumnOrRowHeaderNodeForTesting() const
{
...
ViewAXPlatformNodeDelegate::ChildWidgetsResult ViewAXPlatformNodeDelegate::GetChildWidgets() const
{
    ...
    base::ranges::copy_if(owned_widgets, std::back_inserter(visible_widgets), &Widget::IsVisible);
    ...
    const View* focused_view = focus_manager ? focus_manager->GetFocusedView() : nullptr;
    const auto is_focused_child = [focused_view](Widget* child_widget) { return ViewAccessibilityUtils::IsFocusedChildWidget(child_widget, focused_view); };
