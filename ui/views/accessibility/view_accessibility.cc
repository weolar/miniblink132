...
#define RETURN_IF_UNAVAILABLE()                                                                                                                                \
    ...
    CHECK(initialization_state_ != State::kInitializing) << "Accessibility cache setters must not be used during complete "                                    \
...
void ViewAccessibility::AddVirtualChildView(std::unique_ptr<AXVirtualView> virtual_view)
{
    AddVirtualChildViewAt(std::move(virtual_view), virtual_children_.size());
}

void ViewAccessibility::AddVirtualChildViewAt(std::unique_ptr<AXVirtualView> virtual_view, size_t index)
{
    DCHECK(virtual_view);
    ...
    auto insert_iterator = virtual_children_.begin() + static_cast<ptrdiff_t>(index);
    virtual_children_.insert(insert_iterator, std::move(virtual_view));
}

std::unique_ptr<AXVirtualView> ViewAccessibility::RemoveVirtualChildView(AXVirtualView* virtual_view)
{
    DCHECK(virtual_view);
    ...
    std::unique_ptr<AXVirtualView> child = std::move(virtual_children_[cur_index.value()]);
    virtual_children_.erase(virtual_children_.begin() + static_cast<ptrdiff_t>(cur_index.value()));
...
std::optional<size_t> ViewAccessibility::GetIndexOf(const AXVirtualView* virtual_view) const
{
    DCHECK(virtual_view);
    const auto iter = base::ranges::find(virtual_children_, virtual_view, &std::unique_ptr<AXVirtualView>::get);
    return iter != virtual_children_.end() ? std::make_optional(static_cast<size_t>(iter - virtual_children_.begin())) : std::nullopt;
}

void ViewAccessibility::GetAccessibleNodeData(ui::AXNodeData* data) const
{
    data->id = GetUniqueId();
    data->AddStringAttribute(ax::mojom::StringAttribute::kClassName, view_->GetClassName());
    ...
    if (!data->HasStringAttribute(ax::mojom::StringAttribute::kDescription)) {
        ...
        if (!tooltip.empty() && tooltip != data->GetString16Attribute(ax::mojom::StringAttribute::kName)) {
...
void ViewAccessibility::NotifyEvent(ax::mojom::Event event_type, bool send_native_event)
{
...
{
    DCHECK(!virtual_view || Contains(virtual_view)) << "|virtual_view| must be nullptr or a descendant of this view.";
...
{
    if ((read_only && data_.GetRestriction() == ax::mojom::Restriction::kReadOnly)
        || (!read_only && data_.GetRestriction() != ax::mojom::Restriction::kReadOnly)) {
        return;
    }
...
void ViewAccessibility::SetCharacterOffsets(const std::vector<int32_t>& offsets)
{
    data_.AddIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets, offsets);

    OnIntListAttributeChanged(ax::mojom::IntListAttribute::kCharacterOffsets, offsets);
}

const std::vector<int32_t>& ViewAccessibility::GetCharacterOffsets() const
{
    return data_.GetIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets);
}
...
void ViewAccessibility::ClearTextOffsets()
{
    ...
    OnIntListAttributeChanged(ax::mojom::IntListAttribute::kCharacterOffsets, std::nullopt);
    OnIntListAttributeChanged(ax::mojom::IntListAttribute::kWordStarts, std::nullopt);
    OnIntListAttributeChanged(ax::mojom::IntListAttribute::kWordEnds, std::nullopt);
}

void ViewAccessibility::SetClipsChildren(bool clips_children)
{
    data_.AddBoolAttribute(ax::mojom::BoolAttribute::kClipsChildren, clips_children);

    OnBoolAttributeChanged(ax::mojom::BoolAttribute::kClipsChildren, clips_children);
}
...
void ViewAccessibility::SetRole(const ax::mojom::Role role, const std::u16string& role_description)
{
    RETURN_IF_UNAVAILABLE();
...
void ViewAccessibility::SetName(std::u16string name, ax::mojom::NameFrom name_from)
{
    RETURN_IF_UNAVAILABLE();
    ...
    OnStringAttributeChanged(ax::mojom::StringAttribute::kName, base::UTF16ToUTF8(name));

    NotifyEvent(ax::mojom::Event::kTextChanged, true);
}

void ViewAccessibility::SetName(const std::string& name, ax::mojom::NameFrom name_from)
{
...
void ViewAccessibility::SetName(View& naming_view)
{
    ...
    if (naming_view.GetViewAccessibility().GetCachedName().empty()) {
        ...
        const std::string& name = label_data.GetStringAttribute(ax::mojom::StringAttribute::kName);
        DCHECK(!name.empty());
    ...
    data_.AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, { naming_view.GetViewAccessibility().GetUniqueId() });
}
...
{
    return static_cast<ax::mojom::NameFrom>(data_.GetIntAttribute(ax::mojom::IntAttribute::kNameFrom));
}
...
void ViewAccessibility::SetRoleDescription(const std::u16string& role_description)
{
    if (role_description == data_.GetString16Attribute(ax::mojom::StringAttribute::kRoleDescription)) {
        return;
    }

    if (!role_description.empty()) {
        data_.AddStringAttribute(ax::mojom::StringAttribute::kRoleDescription, base::UTF16ToUTF8(role_description));
    } else {
        RemoveRoleDescription();
    }

    OnStringAttributeChanged(ax::mojom::StringAttribute::kRoleDescription, base::UTF16ToUTF8(role_description));
}

void ViewAccessibility::SetRoleDescription(const std::string& role_description)
{
...
{
    return data_.GetString16Attribute(ax::mojom::StringAttribute::kRoleDescription);
}
...
void ViewAccessibility::RemoveRoleDescription()
{
    ...
    OnStringAttributeChanged(ax::mojom::StringAttribute::kRoleDescription, std::nullopt);
}
...
{
    if (data_.GetIntAttribute(ax::mojom::IntAttribute::kActivedescendantId) == id) {
        return;
    }
...
void ViewAccessibility::ClearActiveDescendant()
{
    ...
    OnIntAttributeChanged(ax::mojom::IntAttribute::kActivedescendantId, std::nullopt);
...
void ViewAccessibility::SetIsEnabled(bool is_enabled)
{
    ...
    OnIntAttributeChanged(ax::mojom::IntAttribute::kRestriction, static_cast<int32_t>(data_.GetRestriction()));
...
{
    data_.AddIntAttribute(ax::mojom::IntAttribute::kTableColumnCount, column_count);

    OnIntAttributeChanged(ax::mojom::IntAttribute::kTableColumnCount, column_count);
}
...
void ViewAccessibility::ClearDescriptionAndDescriptionFrom()
{
    ...
    OnStringAttributeChanged(ax::mojom::StringAttribute::kDescription, std::nullopt);
}
...
void ViewAccessibility::RemoveDescription()
{
    ...
    OnStringAttributeChanged(ax::mojom::StringAttribute::kDescription, std::nullopt);
    OnIntAttributeChanged(ax::mojom::IntAttribute::kDescriptionFrom, std::nullopt);
}

void ViewAccessibility::SetDescription(const std::string& description, const ax::mojom::DescriptionFrom description_from)
{
    // TODO(crbug.com/325137417): Remove once we initialize the cache when a
    // platform accessibility API is used.
    InitializeRoleIfNeeded();
    if (description.empty() && description_from != ax::mojom::DescriptionFrom::kAttributeExplicitlyEmpty) {
        data_.RemoveStringAttribute(ax::mojom::StringAttribute::kDescription);
        data_.RemoveIntAttribute(ax::mojom::IntAttribute::kDescriptionFrom);
        return;
    }
    ...
    OnStringAttributeChanged(ax::mojom::StringAttribute::kDescription, description);
    OnIntAttributeChanged(ax::mojom::IntAttribute::kDescriptionFrom, static_cast<int>(description_from));
}

void ViewAccessibility::SetDescription(const std::u16string& description, const ax::mojom::DescriptionFrom description_from)
{
...
void ViewAccessibility::SetDescription(View& describing_view)
{
    ...
    DCHECK(!name.empty()) << "The describing view must have an accessible name set.";
    SetDescription(name, ax::mojom::DescriptionFrom::kRelatedElement);

    std::vector<int32_t> ids = { describing_view.GetViewAccessibility().GetUniqueId() };
...
std::u16string ViewAccessibility::GetCachedDescription() const
{
    if (data_.HasStringAttribute(ax::mojom::StringAttribute::kDescription)) {
        return base::UTF8ToUTF16(data_.GetStringAttribute(ax::mojom::StringAttribute::kDescription));
    }
...
{
    data_.AddStringAttribute(ax::mojom::StringAttribute::kPlaceholder, placeholder);
}
...
void ViewAccessibility::SetCheckedState(ax::mojom::CheckedState checked_state)
{
    ...
    OnIntAttributeChanged(ax::mojom::IntAttribute::kCheckedState, static_cast<int>(checked_state));
...
{
    data_.AddStringAttribute(ax::mojom::StringAttribute::kKeyShortcuts, key_shortcuts);
}
...
{
    data_.AddStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId, app_id);
}
...
{
    if (data_.HasBoolAttribute(ax::mojom::BoolAttribute::kSelected) && selected == data_.GetBoolAttribute(ax::mojom::BoolAttribute::kSelected)) {
        return;
    }
...
void ViewAccessibility::AddHTMLAttributes(std::pair<std::string, std::string> attribute)
{
...
{
    data_.AddStringAttribute(ax::mojom::StringAttribute::kContainerLiveStatus, status);
}

void ViewAccessibility::RemoveContainerLiveStatus()
{
    if (!data_.HasStringAttribute(ax::mojom::StringAttribute::kContainerLiveStatus)) {
        return;
    }
...
{
    return base::UTF8ToUTF16(data_.GetStringAttribute(ax::mojom::StringAttribute::kValue));
}

void ViewAccessibility::SetDefaultActionVerb(const ax::mojom::DefaultActionVerb default_action_verb)
{
...
void ViewAccessibility::RemoveDefaultActionVerb()
{
    ...
    OnIntAttributeChanged(ax::mojom::IntAttribute::kDefaultActionVerb, std::nullopt);
}

void ViewAccessibility::SetAutoComplete(const std::string& autocomplete)
{
    data_.AddStringAttribute(ax::mojom::StringAttribute::kAutoComplete, autocomplete);
}

void ViewAccessibility::UpdateFocusableState()
{
    bool is_focusable = view_->GetFocusBehavior() != View::FocusBehavior::NEVER && GetIsEnabled() && view_->IsDrawn() && !ViewAccessibility::GetIsIgnored();
...
{
    CHECK(!view_->parent()) << "This method should only be called on the RootView.";
    ready_to_notify_events_ = true;
}

void ViewAccessibility::UpdateInvisibleState()
{
    bool is_invisible = !view_->GetVisible() && data_.role != ax::mojom::Role::kAlert;
    SetState(ax::mojom::State::kInvisible, is_invisible);
}
...
void ViewAccessibility::SetChildTreeID(ui::AXTreeID tree_id)
{
    if (tree_id != ui::AXTreeIDUnknown()) {
        ...
        if (widget && widget->GetNativeView() && display::Screen::GetScreen()) {
            ...
            const float scale_factor = display::Screen::GetScreen()->GetDisplayNearestView(widget->GetNativeView()).device_scale_factor();
            SetChildTreeScaleFactor(scale_factor);
        }

        OnStringAttributeChanged(ax::mojom::StringAttribute::kChildTreeId, tree_id.ToString());
    }
}
...
void ViewAccessibility::RemoveChildTreeID()
{
    ...
    OnStringAttributeChanged(ax::mojom::StringAttribute::kChildTreeId, std::string());
}

void ViewAccessibility::SetChildTreeScaleFactor(float scale_factor)
{
    if (data_.HasChildTreeID()) {
        data_.AddFloatAttribute(ax::mojom::FloatAttribute::kChildTreeScale, scale_factor);
    }
}
...
void ViewAccessibility::AnnounceAlert(const std::u16string& text)
{
    if (auto* const widget = view_->GetWidget()) {
        if (auto* const root_view = static_cast<internal::RootView*>(widget->GetRootView())) {
            root_view->AnnounceTextAs(text, ui::AXPlatformNode::AnnouncementType::kAlert);
        }
    }
}
...
void ViewAccessibility::AnnouncePolitely(const std::u16string& text)
{
    if (auto* const widget = view_->GetWidget()) {
        if (auto* const root_view = static_cast<internal::RootView*>(widget->GetRootView())) {
            root_view->AnnounceTextAs(text, ui::AXPlatformNode::AnnouncementType::kPolite);
        }
    }
}
...
AtomicViewAXTreeManager* ViewAccessibility::GetAtomicViewAXTreeManagerForTesting() const
{
    return nullptr;
}
...
const ViewAccessibility::AccessibilityEventsCallback& ViewAccessibility::accessibility_events_callback() const
{
    return accessibility_events_callback_;
}

void ViewAccessibility::set_accessibility_events_callback(ViewAccessibility::AccessibilityEventsCallback callback)
{
...
{
    bool is_ignored = should_be_ignored_ || pruned_ || data_.role == ax::mojom::Role::kNone;
...
base::CallbackListSubscription ViewAccessibility::AddRoleChangedCallback(RoleCallbackList::CallbackType callback)
{
    return GetOrCreateAXAttributeChangedCallbacks()->AddRoleChangedCallback(callback);
}

void ViewAccessibility::OnStringAttributeChanged(ax::mojom::StringAttribute attribute, const std::optional<std::string>& value)
{
    GetOrCreateAXAttributeChangedCallbacks()->NotifyStringAttributeChanged(attribute, value);
}

base::CallbackListSubscription ViewAccessibility::AddStringAttributeChangedCallback(
    ax::mojom::StringAttribute attribute, StringAttributeCallbackList::CallbackType callback)
{
    return GetOrCreateAXAttributeChangedCallbacks()->AddStringAttributeChangedCallback(attribute, callback);
}

void ViewAccessibility::OnIntAttributeChanged(ax::mojom::IntAttribute attribute, std::optional<int> value)
{
    GetOrCreateAXAttributeChangedCallbacks()->NotifyIntAttributeChanged(attribute, value);
}

base::CallbackListSubscription ViewAccessibility::AddIntAttributeChangedCallback(
    ax::mojom::IntAttribute attribute, IntAttributeCallbackList::CallbackType callback)
{
    return GetOrCreateAXAttributeChangedCallbacks()->AddIntAttributeChangedCallback(attribute, callback);
}

void ViewAccessibility::OnBoolAttributeChanged(ax::mojom::BoolAttribute attribute, std::optional<bool> value)
{
    GetOrCreateAXAttributeChangedCallbacks()->NotifyBoolAttributeChanged(attribute, value);
}

base::CallbackListSubscription ViewAccessibility::AddBoolAttributeChangedCallback(
    ax::mojom::BoolAttribute attribute, BoolAttributeCallbackList::CallbackType callback)
{
    return GetOrCreateAXAttributeChangedCallbacks()->AddBoolAttributeChangedCallback(attribute, callback);
}
...
base::CallbackListSubscription ViewAccessibility::AddStateChangedCallback(ax::mojom::State state, StateCallbackList::CallbackType callback)
{
    return GetOrCreateAXAttributeChangedCallbacks()->AddStateChangedCallback(state, callback);
}

void ViewAccessibility::OnIntListAttributeChanged(ax::mojom::IntListAttribute attribute, const std::optional<std::vector<int>>& value)
{
    GetOrCreateAXAttributeChangedCallbacks()->NotifyIntListAttributeChanged(attribute, value);
}

base::CallbackListSubscription ViewAccessibility::AddIntListAttributeChangedCallback(
    ax::mojom::IntListAttribute attribute, IntListAttributeCallbackList::CallbackType callback)
{
    return GetOrCreateAXAttributeChangedCallbacks()->AddIntListAttributeChangedCallback(attribute, callback);
}

void ViewAccessibility::SetHierarchicalLevel(int hierarchical_level)
{
    data_.AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, hierarchical_level);
}
...
{
    CHECK_GE(text_direction, static_cast<int32_t>(ax::mojom::WritingDirection::kMinValue));
    CHECK_LE(text_direction, static_cast<int32_t>(ax::mojom::WritingDirection::kMaxValue));
    data_.AddIntAttribute(ax::mojom::IntAttribute::kTextDirection, text_direction);
}
...
{
    data_.AddStringAttribute(ax::mojom::StringAttribute::kLiveStatus, live_status);
}

void ViewAccessibility::SetLiveRelevant(const std::string& live_relevant)
{
    data_.AddStringAttribute(ax::mojom::StringAttribute::kLiveRelevant, live_relevant);
}
...
void ViewAccessibility::SetContainerLiveRelevant(const std::string& live_relevant)
{
    data_.AddStringAttribute(ax::mojom::StringAttribute::kContainerLiveRelevant, live_relevant);
}

void ViewAccessibility::RemoveContainerLiveRelevant()
{
    data_.RemoveStringAttribute(ax::mojom::StringAttribute::kContainerLiveRelevant);
}

ui::AXAttributeChangedCallbacks* ViewAccessibility::GetOrCreateAXAttributeChangedCallbacks()
{
    if (!attribute_changed_callbacks_) {
        attribute_changed_callbacks_ = std::make_unique<ui::AXAttributeChangedCallbacks>();
    }
