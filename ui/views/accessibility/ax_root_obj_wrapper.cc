...
AXRootObjWrapper::AXRootObjWrapper(views::AXAuraObjCache::Delegate* delegate, views::AXAuraObjCache* cache)
...
void AXRootObjWrapper::GetChildren(std::vector<raw_ptr<views::AXAuraObjWrapper, VectorExperimental>>* out_children)
{
...
void AXRootObjWrapper::Serialize(ui::AXNodeData* out_node_data)
{
    ...
    out_node_data->relative_bounds.bounds = gfx::RectF(display.bounds().width(), display.bounds().height());
...
void AXRootObjWrapper::OnDisplayMetricsChanged(const display::Display& display, uint32_t changed_metrics)
{
