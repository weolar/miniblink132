...
{
    if (view_->parent()) {
        if (view_->parent()->GetViewAccessibility().GetChildTreeID() != ui::AXTreeIDUnknown())
            return nullptr;
...
void AXViewObjWrapper::GetChildren(std::vector<raw_ptr<AXAuraObjWrapper, VectorExperimental>>* out_children)
{
...
void AXViewObjWrapper::Serialize(ui::AXNodeData* out_node_data)
{
    ...
    out_node_data->relative_bounds.bounds = gfx::RectF(view_->GetBoundsInScreen());

    if (view_accessibility.GetNextWindowFocus()) {
        out_node_data->AddIntAttribute(
            ax::mojom::IntAttribute::kNextWindowFocusId, aura_obj_cache_->GetOrCreate(view_accessibility.GetNextWindowFocus())->GetUniqueId());
    }

    if (view_accessibility.GetPreviousWindowFocus()) {
        out_node_data->AddIntAttribute(
            ax::mojom::IntAttribute::kPreviousWindowFocusId, aura_obj_cache_->GetOrCreate(view_accessibility.GetPreviousWindowFocus())->GetUniqueId());
    }
}
