...
// static
std::unique_ptr<AtomicViewAXTreeManager> AtomicViewAXTreeManager::Create(ViewAXPlatformNodeDelegate* delegate, ui::AXNodeData node_data)
{
    auto view_tree_manager = base::WrapUnique<AtomicViewAXTreeManager>(new AtomicViewAXTreeManager(delegate, node_data));
...
AtomicViewAXTreeManager::AtomicViewAXTreeManager(ViewAXPlatformNodeDelegate* delegate, ui::AXNodeData node_data)
    : AXPlatformTreeManager(nullptr)
    , delegate_(delegate)
{
    DCHECK(delegate);
...
ui::AXNode* AtomicViewAXTreeManager::GetNode(ui::AXNodeID node_id) const
{
    ...
    DCHECK_EQ(node_id, ax_tree_->root()->id()) << "The AtomicViewAXTreeManager should only allow callers to get the "
...
ui::AXPlatformNode* AtomicViewAXTreeManager::GetPlatformNodeFromTree(ui::AXNodeID node_id) const
{
    return delegate_->GetFromNodeID(node_id);
}

ui::AXPlatformNode* AtomicViewAXTreeManager::GetPlatformNodeFromTree(const ui::AXNode& node) const
{
