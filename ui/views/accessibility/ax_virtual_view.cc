...
AXVirtualView::AXVirtualView()
{
    ...
    custom_data_.AddStringAttribute(ax::mojom::StringAttribute::kClassName, GetViewClassName());
}

AXVirtualView::~AXVirtualView()
{
    GetIdMap().erase(unique_id_.Get());
    DCHECK(!parent_view_ || !virtual_parent_view_) << "Either |parent_view_| or |virtual_parent_view_| could be set but "
                                                      "not both.";
...
void AXVirtualView::AddChildViewAt(std::unique_ptr<AXVirtualView> view, size_t index)
{
    DCHECK(view);
    CHECK_NE(view.get(), this) << "You cannot add an AXVirtualView as its own child.";
    ...
    children_.insert(children_.begin() + static_cast<ptrdiff_t>(index), std::move(view));
    if (GetOwnerView()) {
        GetOwnerView()->NotifyAccessibilityEvent(ax::mojom::Event::kChildrenChanged, true);
    }
}
...
void AXVirtualView::ReorderChildView(AXVirtualView* view, size_t index)
{
    ...
    std::unique_ptr<AXVirtualView> child = std::move(children_[cur_index.value()]);
    children_.erase(children_.begin() + static_cast<ptrdiff_t>(cur_index.value()));
...
std::unique_ptr<AXVirtualView> AXVirtualView::RemoveFromParentView()
{
    ...
    CHECK(virtual_parent_view_) << "Cannot remove from parent view if there is no parent.";
    return virtual_parent_view_->RemoveChildView(this);
}

std::unique_ptr<AXVirtualView> AXVirtualView::RemoveChildView(AXVirtualView* view)
{
    DCHECK(view);
    auto cur_index = GetIndexOf(view);
    if (!cur_index.has_value())
    ...
    if (GetOwnerView()) {
        ViewAccessibility& view_accessibility = GetOwnerView()->GetViewAccessibility();
        if (view_accessibility.FocusedVirtualChild() && Contains(view_accessibility.FocusedVirtualChild())) {
            focus_changed = true;
        }
    }
...
std::optional<size_t> AXVirtualView::GetIndexOf(const AXVirtualView* view) const
{
    DCHECK(view);
    const auto iter = base::ranges::find(children_, view, &std::unique_ptr<AXVirtualView>::get);
    return iter != children_.end() ? std::make_optional(static_cast<size_t>(iter - children_.begin())) : std::nullopt;
}
...
void AXVirtualView::NotifyAccessibilityEvent(ax::mojom::Event event_type)
{
    ...
    if (event_type == ax::mojom::Event::kAlert) {
        CHECK(ui::IsAlert(GetRole())) << "On some platforms, the alert event does not work correctly unless "
                                         "it is fired on an object with an alert role. Role was "
                                      << GetRole();
    }
    if (GetOwnerView()) {
        const ViewAccessibility::AccessibilityEventsCallback& events_callback = GetOwnerView()->GetViewAccessibility().accessibility_events_callback();
        if (events_callback)
...
void AXVirtualView::SetPopulateDataCallback(base::RepeatingCallback<void(ui::AXNodeData*)> callback)
{
...
const ui::AXNodeData& AXVirtualView::GetData() const
{
    ...
    // and operable.
    if (node_data.HasState(ax::mojom::State::kIgnored) && node_data.HasState(ax::mojom::State::kFocusable)) {
...
{
    DCHECK_LT(index, GetChildCount()) << "|index| should be less than the child count.";
...
gfx::NativeViewAccessible AXVirtualView::HitTestSync(int screen_physical_pixel_x, int screen_physical_pixel_y) const
{
    ...
    for (const std::unique_ptr<AXVirtualView>& child : base::Reversed(children_)) {
    ...
    gfx::Rect bounds_in_screen_physical_pixels = GetBoundsRect(ui::AXCoordinateSystem::kScreenPhysicalPixels, ui::AXClippingBehavior::kUnclipped);
    if (bounds_in_screen_physical_pixels.Contains(static_cast<float>(screen_physical_pixel_x), static_cast<float>(screen_physical_pixel_y)) && !IsIgnored()) {
        return GetNativeObject();
    }
...
std::optional<int32_t> AXVirtualView::GetCellId(int row_index, int col_index) const
{
    return GetDelegate()->GetCellId(row_index, col_index);
}

bool AXVirtualView::HandleAccessibleAction(const ui::AXActionData& action_data)
{
    if (!GetOwnerView())
        return false;
    ...
    case ax::mojom::Action::kShowContextMenu: {
        const gfx::Rect screen_bounds = GetBoundsRect(ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kClipped, nullptr /* offscreen_result */);
        if (!screen_bounds.IsEmpty()) {
            GetOwnerView()->ShowContextMenu(screen_bounds.CenterPoint(), ui::mojom::MenuSourceType::kKeyboard);
            return true;
        }
        break;
    }
...
bool AXVirtualView::HandleAccessibleActionInOwnerView(const ui::AXActionData& action_data)
{
    DCHECK(GetOwnerView());
...
ViewAXPlatformNodeDelegate* AXVirtualView::GetDelegate() const
{
    ...
    return static_cast<ViewAXPlatformNodeDelegate*>(&GetOwnerView()->GetViewAccessibility());
#else
    return nullptr;
#endif
}

AXVirtualViewWrapper* AXVirtualView::GetOrCreateWrapper(views::AXAuraObjCache* cache)
{
#if defined(USE_AURA)
