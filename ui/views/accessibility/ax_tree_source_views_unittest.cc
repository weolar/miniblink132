...
{
    AXAuraObjCache cache;
    TestAXTreeSourceViews tree(cache.GetOrCreate(widget_.get())->GetUniqueId(), &cache);
...
TEST_F(AXTreeSourceViewsTest, ViewWithChildTreeHasNoChildren)
{
    ...
    contents_view->GetViewAccessibility().SetChildTreeID(ui::AXTreeID::CreateNewAXTreeID());
...
{
    AXAuraObjCache cache;
    AXAuraObjWrapper* root_wrapper = cache.GetOrCreate(widget_->GetNativeWindow()->GetRootWindow());
