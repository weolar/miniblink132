...
public:
    ...
    virtual void GetChildren(std::vector<raw_ptr<AXAuraObjWrapper, VectorExperimental>>* out_children) = 0;
    ...
    const AXAuraObjCache* cache() const
    {
        return aura_obj_cache_;
    }

protected:
    std::optional<std::vector<raw_ptr<AXAuraObjWrapper, VectorExperimental>>> cached_children_;
