...
AXWidgetObjWrapper::AXWidgetObjWrapper(AXAuraObjCache* aura_obj_cache, Widget* widget)
...
void AXWidgetObjWrapper::GetChildren(std::vector<raw_ptr<AXAuraObjWrapper, VectorExperimental>>* out_children)
{
    if (!widget_->IsVisible() || !widget_->GetRootView() || !widget_->GetRootView()->GetVisible()) {
        return;
    }
...
void AXWidgetObjWrapper::Serialize(ui::AXNodeData* out_node_data)
{
    ...
    out_node_data->AddStringAttribute(ax::mojom::StringAttribute::kName, base::UTF16ToUTF8(widget_->widget_delegate()->GetAccessibleWindowTitle()));
    out_node_data->AddStringAttribute(ax::mojom::StringAttribute::kClassName, "Widget");
    out_node_data->relative_bounds.bounds = gfx::RectF(widget_->GetWindowBoundsInScreen());
