...
// the DCHECK elsewhere.
    ...
    DCHECK(!node_data.HasState(ax::mojom::State::kIgnored)) << "View is focusable and should not be ignored.\n" << GetViewDebugInfo(view);

    DCHECK(!node_data.IsInvisible()) << "View is focusable and should not be invisible.\n" << GetViewDebugInfo(view);
    ...
    if (node_data.GetRestriction() != ax::mojom::Restriction::kDisabled) {
        ...
        DCHECK(node_data.role != ax::mojom::Role::kNone && node_data.role != ax::mojom::Role::kUnknown) << "View is focusable but lacks a valid role.\n"
                                                                                                        << GetViewDebugInfo(view);
    }
    ...
    if (node_data.role == ax::mojom::Role::kTextField && !node_data.GetStringAttribute(ax::mojom::StringAttribute::kPlaceholder).empty()) {
        return;
    }

    // Finally, a view is allowed to explicitly state that it has no name.
    DCHECK_EQ(node_data.GetNameFrom(), ax::mojom::NameFrom::kAttributeExplicitlyEmpty) << "View is focusable but has no accessible name or placeholder, and is "
