...
public:
    ...
    void SetUp() override
    {
        ...
        delegate_ = static_cast<ViewAXPlatformNodeDelegate*>(&textfield_->GetViewAccessibility());
...
{
    ui::AXNodeData previous = delegate_->GetAtomicViewAXTreeManagerForTesting()->GetRoot()->data();
    textfield_->SetText(u"text");
    ui::AXNodeData actual = delegate_->GetAtomicViewAXTreeManagerForTesting()->GetRoot()->data();
    CompareNodeData(delegate_data(), actual);
    EXPECT_EQ(actual.GetString16Attribute(ax::mojom::StringAttribute::kValue), u"text");
    EXPECT_EQ(previous.GetString16Attribute(ax::mojom::StringAttribute::kValue), u"");
}

TEST_F(AtomicViewAXTreeManagerTest, GetRootAsAXNode)
{
    CompareNodeData(delegate_data(), delegate_->GetAtomicViewAXTreeManagerForTesting()->GetRoot()->data());
}

TEST_F(AtomicViewAXTreeManagerTest, GetNode)
{
    CompareNodeData(delegate_data(),
        delegate_->GetAtomicViewAXTreeManagerForTesting()->GetNode(delegate_->GetAtomicViewAXTreeManagerForTesting()->GetRoot()->id())->data());
}
