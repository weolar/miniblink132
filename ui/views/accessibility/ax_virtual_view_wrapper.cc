...
AXVirtualViewWrapper::AXVirtualViewWrapper(AXAuraObjCache* cache, AXVirtualView* virtual_view)
    : AXAuraObjWrapper(cache)
...
AXAuraObjWrapper* AXVirtualViewWrapper::GetParent()
{
    if (virtual_view_->virtual_parent_view()) {
        return const_cast<AXVirtualView*>(virtual_view_->virtual_parent_view())->GetOrCreateWrapper(aura_obj_cache_);
    }
...
void AXVirtualViewWrapper::GetChildren(std::vector<raw_ptr<AXAuraObjWrapper, VectorExperimental>>* out_children)
{
...
bool AXVirtualViewWrapper::HandleAccessibleAction(const ui::AXActionData& action)
{
