...
std::vector<int32_t> ComputeTextOffsets(gfx::RenderText* render_text)
{
    ...
    if (!render_text || render_text->multiline() || render_text->elide_behavior() == gfx::ELIDE_MIDDLE || render_text->elide_behavior() == gfx::ELIDE_HEAD
    ...
    while (begin_position < render_text->text().length()) {
        size_t end_position = render_text->IndexOfAdjacentGrapheme(begin_position, gfx::CURSOR_FORWARD);
