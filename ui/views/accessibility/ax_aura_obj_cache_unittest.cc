...
TEST_F(AXAuraObjCacheTest, TestViewRemoval)
{
    ...
    std::unique_ptr<Widget> widget(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
...
public:
    ViewBlurObserver(AXAuraObjCache* cache, View* view)
        : cache_(cache)
    {
...
TEST_F(AXAuraObjCacheTest, ViewDestruction)
{
    ...
    std::unique_ptr<Widget> widget(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    auto button = std::make_unique<LabelButton>(Button::PressedCallback(), u"button");
...
TEST_F(AXAuraObjCacheTest, ValidTree)
{
    ...
    params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_BUBBLE);
    ...
    // Create a child view.
    auto button = std::make_unique<LabelButton>(Button::PressedCallback(), u"ChildButton");
    ...
    AXTreeSourceViews tree_source(cache.GetOrCreate(parent_widget->GetNativeWindow())->GetUniqueId(), tree_id, &cache);
    ui::AXTreeSerializer<AXAuraObjWrapper*, std::vector<AXAuraObjWrapper*>, ui::AXTreeUpdate*, ui::AXTreeData*, ui::AXNodeData> serializer(&tree_source);
...
TEST_F(AXAuraObjCacheTest, GetFocusIsUnignoredAncestor)
{
    ...
    client_child->GetViewAccessibility().SetChildTreeID(ui::AXTreeID::CreateNewAXTreeID());
...
public:
    ...
    TestingWidgetDelegateView& operator=(const TestingWidgetDelegateView&) = delete;
...
public:
    explicit TestingAXEventObserver(AXAuraObjCache* cache)
        : cache_(cache)
    {
