...
// |window|.
void FireEventOnWindowChildWidgetAndRootView(aura::Window* window, ax::mojom::Event event, AXAuraObjCache* cache)
{
...
void FireLocationChangesRecursively(aura::Window* window, AXAuraObjCache* cache)
{
    FireEventOnWindowChildWidgetAndRootView(window, ax::mojom::Event::kLocationChanged, cache);
...
AXWindowObjWrapper::AXWindowObjWrapper(AXAuraObjCache* aura_obj_cache, aura::Window* window)
...
bool AXWindowObjWrapper::HandleAccessibleAction(const ui::AXActionData& action)
{
...
AXAuraObjWrapper* AXWindowObjWrapper::GetParent()
{
    ...
    if (parent->GetProperty(ui::kChildAXTreeID) && ui::AXTreeID::FromString(*(parent->GetProperty(ui::kChildAXTreeID))) != ui::AXTreeIDUnknown()) {
        return nullptr;
    }
...
void AXWindowObjWrapper::GetChildren(std::vector<raw_ptr<AXAuraObjWrapper, VectorExperimental>>* out_children)
{
    // Ignore this window's descendants if it has a child tree.
    if (window_->GetProperty(ui::kChildAXTreeID) && ui::AXTreeID::FromString(*(window_->GetProperty(ui::kChildAXTreeID))) != ui::AXTreeIDUnknown()) {
        return;
    }
...
void AXWindowObjWrapper::Serialize(ui::AXNodeData* out_node_data)
{
    if (window_->IsRootWindow() && !window_->parent() && window_->GetHost()) {
        ui::TextInputClient* client = window_->GetHost()->GetInputMethod()->GetTextInputClient();
        ...
        if (client && client->GetTextInputType() != ui::TEXT_INPUT_TYPE_NONE) {
            ...
            out_node_data->AddIntListAttribute(ax::mojom::IntListAttribute::kCaretBounds,
                { caret_bounds_in_screen.x(), caret_bounds_in_screen.y(), caret_bounds_in_screen.width(), caret_bounds_in_screen.height() });
        }
    }
    ...
    out_node_data->AddStringAttribute(ax::mojom::StringAttribute::kName, base::UTF16ToUTF8(window_->GetTitle()));
    if (!window_->IsVisible() || window_->GetProperty(ui::kAXConsiderInvisibleAndIgnoreChildren)) {
        out_node_data->AddState(ax::mojom::State::kInvisible);
    }

    out_node_data->relative_bounds.bounds = gfx::RectF(window_->GetBoundsInScreen());

    out_node_data->AddStringAttribute(ax::mojom::StringAttribute::kClassName, GetWindowName(window_));

    std::string* child_ax_tree_id_ptr = window_->GetProperty(ui::kChildAXTreeID);
    if (child_ax_tree_id_ptr) {
        ui::AXTreeID child_ax_tree_id = ui::AXTreeID::FromString(*child_ax_tree_id_ptr);
        if (child_ax_tree_id != ui::AXTreeIDUnknown()) {
            ...
            if (!window_->GetToplevelWindow() || GetWidgetForWindow(window_->GetToplevelWindow()) || !window_->IsVisible()
            ...
            const float scale_factor = window_->GetToplevelWindow()->layer()->device_scale_factor();
            out_node_data->AddFloatAttribute(ax::mojom::FloatAttribute::kChildTreeScale, scale_factor);
        }
    }
}
...
void AXWindowObjWrapper::OnCaretBoundsChanged(const ui::TextInputClient* client)
{
...
void AXWindowObjWrapper::OnWindowHierarchyChanged(const HierarchyChangeParams& params)
{
...
void AXWindowObjWrapper::OnWindowBoundsChanged(aura::Window* window, const gfx::Rect& old_bounds, const gfx::Rect& new_bounds, ui::PropertyChangeReason reason)
{
...
void AXWindowObjWrapper::OnWindowPropertyChanged(aura::Window* window, const void* key, intptr_t old)
{
    if (window_destroying_ || window != window_)
        return;

    if (key == ui::kChildAXTreeID || key == ui::kAXConsiderInvisibleAndIgnoreChildren) {
        FireEvent(ax::mojom::Event::kChildrenChanged);
    }
}

void AXWindowObjWrapper::OnWindowVisibilityChanged(aura::Window* window, bool visible)
{
    if (window_destroying_)
        return;
...
void AXWindowObjWrapper::OnWindowTransformed(aura::Window* window, ui::PropertyChangeReason reason)
{
    if (window_destroying_)
        return;
...
void AXWindowObjWrapper::OnWindowTitleChanged(aura::Window* window)
{
    ...
    FireEventOnWindowChildWidgetAndRootView(window_, ax::mojom::Event::kTreeChanged, aura_obj_cache_);
}
