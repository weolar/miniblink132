...
class AuraLinuxApplication : public ui::AXPlatformNodeDelegate, public WidgetObserver, public aura::WindowObserver {
public:
    ...
    void RegisterWidget(Widget* widget)
    {
        ...
        if (!widget || !widget->native_widget() || base::Contains(widgets_, widget)) {
            return;
        }
    ...
    void OnWindowVisibilityChanged(aura::Window* window, bool visible) override
    {
        for (Widget* widget : widgets_) {
            ...
            root_view->NotifyAccessibilityEvent(ax::mojom::Event::kWindowVisibilityChanged, true);
        }
    }
    ...
    const ui::AXNodeData& GetData() const override
    {
        ...
        // retrieved.
        if (data_.GetStringAttribute(ax::mojom::StringAttribute::kName).empty() && ViewsDelegate::GetInstance()) {
...
private:
    ...
    base::ScopedMultiSourceObservation<Widget, WidgetObserver> widget_observations_ { this };
...
ViewAXPlatformNodeDelegateAuraLinux::ViewAXPlatformNodeDelegateAuraLinux(View* view)
...
gfx::NativeViewAccessible ViewAXPlatformNodeDelegateAuraLinux::GetParent() const
{
    if (gfx::NativeViewAccessible parent = ViewAXPlatformNodeDelegate::GetParent()) {
        return parent;
    }
...
void ViewAXPlatformNodeDelegateAuraLinux::OnViewHierarchyChanged(View* observed_view, const ViewHierarchyChangedDetails& details)
{
    if (view() != details.child || !details.is_add)
        return;
    static_cast<ui::AXPlatformNodeAuraLinux*>(ax_platform_node())->OnParentChanged();
}
