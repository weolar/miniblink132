...
public:
    explicit A11yOverrideWindowObserver(AXAuraObjCache* cache)
        : cache_(cache)
    {
    }
    A11yOverrideWindowObserver(const A11yOverrideWindowObserver&) = delete;
    A11yOverrideWindowObserver& operator=(const A11yOverrideWindowObserver&) = delete;
...
{
    if (!virtual_view->GetOwnerView() || !virtual_view->GetOwnerView()->GetWidget())
        return nullptr;
    return CreateInternal<AXVirtualViewWrapper>(virtual_view, &virtual_view_to_id_map_);
}
...
void AXAuraObjCache::GetTopLevelWindows(std::vector<raw_ptr<AXAuraObjWrapper, VectorExperimental>>* children)
{
...
AXAuraObjWrapper* AXAuraObjCache::GetFocus()
{
    ...
    while (focused_view && (focused_view->GetViewAccessibility().GetIsIgnored() || focused_view->GetViewAccessibility().propagate_focus_to_ancestor())) {
    ...
    if (focused_view->GetViewAccessibility().FocusedVirtualChild()) {
        return focused_view->GetViewAccessibility().FocusedVirtualChild()->GetOrCreateWrapper(this);
    }
...
void AXAuraObjCache::FireEvent(AXAuraObjWrapper* aura_obj, ax::mojom::Event event_type)
{
    if (delegate_)
...
AXAuraObjCache::AXAuraObjCache()
    : a11y_override_window_observer_(std::make_unique<A11yOverrideWindowObserver>(this))
{
}
...
View* AXAuraObjCache::GetFocusedView()
{
    ...
    aura::Window* focused_window = focused_widget ? focused_widget->GetNativeWindow() : nullptr;
    if (!focused_widget) {
        // Uses the a11y override window for focus if it exists, otherwise gets the
        // last focused window.
        focused_window = a11y_override_window_ ? a11y_override_window_.get() : focused_window_.get();

        // Finally, fallback to searching for the focus.
        if (!focused_window) {
            for (aura::Window* window : root_windows_) {
                auto* focus_client = GetFocusClient(window);
                if (focus_client && (focused_window = GetFocusClient(window)->GetFocusedWindow())) {
                    break;
                }
            }
        }
    ...
    if (focused_window) {
        ...
        if (client && !client->children().empty()) {
            const ViewAccessibility& host_accessibility = client->children().front()->GetViewAccessibility();
            ...
            if (host_accessibility.GetChildTreeID() != ui::AXTreeIDUnknown()
                || !host_data.GetStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId).empty()) {
...
void AXAuraObjCache::OnWindowFocused(aura::Window* gained_focus, aura::Window* lost_focus)
{
...
{
    std::erase_if(root_windows_, [window](aura::Window* current_window) { return current_window == window; });
    if (root_windows_.empty() && GetFocusClient(window))
...
AXAuraObjWrapper* AXAuraObjCache::CreateInternal(AuraView* aura_view, std::map<AuraView*, int32_t>* aura_view_to_id_map)
{
    if (!aura_view)
        return nullptr;
...
template <typename AuraView> int32_t AXAuraObjCache::GetIDInternal(AuraView* aura_view, const std::map<AuraView*, int32_t>& aura_view_to_id_map) const
{
    if (!aura_view)
...
template <typename AuraView> void AXAuraObjCache::RemoveInternal(AuraView* aura_view, std::map<AuraView*, int32_t>* aura_view_to_id_map)
{
