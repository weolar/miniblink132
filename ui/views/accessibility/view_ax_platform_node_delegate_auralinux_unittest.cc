...
TEST_F(ViewAXPlatformNodeDelegateAuraLinuxTest, TextfieldAccessibility)
{
    ...
    ASSERT_EQ(0, atk_object_get_n_accessible_children(textfield->GetNativeViewAccessible()))
    ...
    GCallback callback = G_CALLBACK(+[](AtkText*, int position, int length, char* text, gpointer data) {
...
TEST_F(ViewAXPlatformNodeDelegateAuraLinuxTest, ExpandedChangedNotFiredOnNonExpandableViews)
{
    auto widget = std::make_unique<Widget>();
    Widget::InitParams init_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP);
    widget->Init(std::move(init_params));
    ...
    View* content = widget->SetContentsView(std::make_unique<View>());
    EXPECT_DCHECK_DEATH(content->NotifyAccessibilityEvent(ax::mojom::Event::kExpandedChanged, true));
}
...
TEST_F(ViewAXPlatformNodeDelegateAuraLinuxTest, AuraChildWidgets)
{
    ...
    AtkObject* root_view_accessible = widget->GetRootView()->GetNativeViewAccessible();
    ...
    // the original parent.
    AtkObject* child_widget_accessible = child_widget->GetRootView()->GetNativeViewAccessible();
    ASSERT_EQ(atk_object_get_parent(child_widget_accessible), root_view_accessible);

    // Make sure that querying the second child of the parent is the child widget
    // accessible as well.
    AtkObject* second_child = atk_object_ref_accessible_child(root_view_accessible, 1);
...
TEST_F(ViewAXPlatformNodeDelegateAuraLinuxTest, IndexInParent)
{
    ...
    auto widget = std::make_unique<Widget>();
    Widget::InitParams child_init_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP);
    ...
    std::unique_ptr<View> contents_unique = widget->GetRootView()->RemoveChildViewT(contents);
