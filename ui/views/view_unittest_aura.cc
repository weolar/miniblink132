...
std::unique_ptr<Widget> CreateControlWidget(aura::Window* parent, const gfx::Rect& bounds)
{
...
std::unique_ptr<View> CreateViewWithLayer(const gfx::Rect& bounds, const char* layer_name)
{
...
{
    std::unique_ptr<Widget> w1 = CreateControlWidget(GetContext(), gfx::Rect(0, 0, 100, 100));
    w1->GetNativeView()->layer()->SetName("w1");

    auto* v1 = w1->GetRootView()->AddChildView(CreateViewWithLayer(gfx::Rect(0, 3, 100, 103), "v1"));
    ...
    auto* v4 = v2->AddChildView(CreateViewWithLayer(gfx::Rect(0, 4, 100, 104), "v4"));
    ui::Layer* v4_layer = v4->layer();

    auto* w2_host_view = w1->GetRootView()->AddChildView(std::make_unique<View>());
    auto* v7 = w1->GetRootView()->AddChildView(CreateViewWithLayer(gfx::Rect(0, 4, 100, 104), "v7"));
    ui::Layer* v7_layer = v7->layer();

    auto* v8 = v7->AddChildView(CreateViewWithLayer(gfx::Rect(0, 4, 100, 104), "v8"));
    ui::Layer* v8_layer = v8->layer();

    auto* v9 = v7->AddChildView(CreateViewWithLayer(gfx::Rect(0, 4, 100, 104), "v9"));
    ui::Layer* v9_layer = v9->layer();

    std::unique_ptr<Widget> w2 = CreateControlWidget(w1->GetNativeView(), gfx::Rect(0, 5, 100, 105));
    w2->GetNativeView()->layer()->SetName("w2");
    w2->GetNativeView()->SetProperty(kHostViewKey, w2_host_view);

    auto* v5 = w2->GetRootView()->AddChildView(CreateViewWithLayer(gfx::Rect(0, 6, 100, 106), "v5"));
    auto* v6 = v5->AddChildView(CreateViewWithLayer(gfx::Rect(0, 7, 100, 107), "v6"));
    ...
    {
        std::unique_ptr<ui::LayerTreeOwner> cloned_owner(wm::RecreateLayers(w1->GetNativeView()));
