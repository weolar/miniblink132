...
public:
    ...
    BubbleBorder(Arrow arrow, Shadow shadow, ui::ColorId color_id = ui::kColorDialogBackground);
    ...
    {
        return (a == TOP_CENTER || a == BOTTOM_CENTER || a >= NONE) ? a : static_cast<Arrow>(int { a } ^ RIGHT);
    }

    static Arrow vertical_mirror(Arrow a)
    {
        return (a == LEFT_CENTER || a == RIGHT_CENTER || a >= NONE) ? a : static_cast<Arrow>(int { a } ^ BOTTOM);
    }
    ...
    static gfx::Insets GetBorderAndShadowInsets(const std::optional<int>& shadow_elevation = std::nullopt,
        const std::optional<bool>& draw_border_stroke = std::nullopt, Shadow shadow_type = Shadow::STANDARD_SHADOW);
    ...
    static void DrawBorderAndShadow(SkRect rect, gfx::Canvas* canvas, const ui::ColorProvider* color_provider);
    ...
    void set_visible_arrow(bool visible_arrow)
    {
        visible_arrow_ = visible_arrow;
    }
    bool visible_arrow() const
    {
        return visible_arrow_;
    }
    ...
    void set_avoid_shadow_overlap(bool value)
    {
        avoid_shadow_overlap_ = value;
    }

    // Sets an explicit insets value to be used.
    void set_insets(const gfx::Insets& insets)
    {
        insets_ = insets;
    }
    ...
    virtual gfx::Rect GetBounds(const gfx::Rect& anchor_rect, const gfx::Size& contents_size) const;
    ...
    // bubble border.
    bool AddArrowToBubbleCornerAndPointTowardsAnchor(const gfx::Rect& anchor_rect, gfx::Rect& popup_bounds, int popup_min_y);
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(BubbleBorderTest, MoveContentsBoundsToPlaceVisibleArrow);
    ...
    // kVisibleArrowGap.
    static gfx::Vector2d GetContentsBoundsOffsetToPlaceVisibleArrow(BubbleBorder::Arrow arrow, bool include_gap = true);
    ...
    void CalculateVisibleArrowRect(const gfx::Rect& contents_bounds, const gfx::Point& anchor_point) const;
