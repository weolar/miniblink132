...
InfoBubble::InfoBubble(View* anchor, BubbleBorder::Arrow arrow, const std::u16string& message)
    : BubbleDialogDelegateView(anchor, arrow, views::BubbleBorder::DIALOG_SHADOW, true)
{
...
std::unique_ptr<NonClientFrameView> InfoBubble::CreateNonClientFrameView(Widget* widget)
{
    DCHECK(!frame_);
...
gfx::Size InfoBubble::CalculatePreferredSize(const SizeBounds& available_size) const
{
    ...
    return gfx::Size(pref_width, GetLayoutManager()->GetPreferredHeightForWidth(this, pref_width));
}

void InfoBubble::OnWidgetBoundsChanged(Widget* widget, const gfx::Rect& new_bounds)
{
...
void InfoBubble::UpdatePosition()
{
    ...
    if (anchor_widget()->IsVisible() && !GetAnchorView()->GetVisibleBounds().IsEmpty()) {
