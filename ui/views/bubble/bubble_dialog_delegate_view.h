...
public:
    BubbleDialogDelegate(View* anchor_view, BubbleBorder::Arrow arrow, BubbleBorder::Shadow shadow = BubbleBorder::DIALOG_SHADOW, bool autosize = false);
    ...
    static Widget* CreateBubble(
        std::unique_ptr<BubbleDialogDelegate> bubble_delegate, Widget::InitParams::Ownership ownership = Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET);
    ...
    const ui::ImageModel& GetMainImage() const
    {
        return main_image_;
    }
    ...
    const std::optional<gfx::Rect>& anchor_rect() const
    {
        return anchor_rect_;
    }
    ...
    BubbleBorder::Arrow arrow() const
    {
        return arrow_;
    }
    ...
    bool adjust_if_offscreen() const
    {
        return adjust_if_offscreen_;
    }
    void set_adjust_if_offscreen(bool adjust)
    {
        adjust_if_offscreen_ = adjust;
    }
    ...
    gfx::NativeView parent_window() const
    {
        return parent_window_;
    }
    void set_parent_window(gfx::NativeView window)
    {
    ...
    void set_accept_events(bool accept_events)
    {
    ...
    void set_force_create_contents_background(bool force_create_contents_background)
    {
    ...
    virtual void OnBeforeBubbleWidgetInit(Widget::InitParams* params, Widget* widget) const
    {
    }
    ...
    static gfx::Size GetAvailableSpaceToPlaceBubble(BubbleBorder::Arrow arrow, gfx::Rect anchor_rect, gfx::Rect screen_rect);
...
protected:
    ...
    public:
        ...
        void set_bubble_view(views::View* view)
        {
            bubble_view_ = view;
        }

        void set_allowed_class_names_for_testing(const base::span<const char* const>& value)
        {
        ...
        template <typename Value> void LogMetric(void (*uma_func)(const std::string&, Value), const std::string& histogram_name, Value value) const;

    private:
        std::optional<raw_ptr<views::View>> bubble_view_;
        std::optional<raw_ptr<views::BubbleDialogDelegate>> delegate_;
        std::optional<base::raw_span<const char* const>> allowed_class_names_for_testing_;
    ...
    void set_color_internal(SkColor color)
    {
        color_ = color;
    }

    bool color_explicitly_set() const
    {
        return color_explicitly_set_;
    }

    BubbleUmaLogger& bubble_uma_logger()
    {
        return bubble_uma_logger_;
    }
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(BubbleDialogDelegateViewTest, VisibleWidgetShowsInkDropOnAttaching);
    FRIEND_TEST_ALL_PREFIXES(BubbleDialogDelegateViewTest, AttachedWidgetShowsInkDropWhenVisible);
    FRIEND_TEST_ALL_PREFIXES(BubbleDialogDelegateViewTest, MultipleBubbleAnchorHighlightTestInOrder);
    FRIEND_TEST_ALL_PREFIXES(BubbleDialogDelegateViewTest, MultipleBubbleAnchorHighlightTestOutOfOrder);
...
class VIEWS_EXPORT BubbleDialogDelegateView : public View, public BubbleDialogDelegate {
    METADATA_HEADER(BubbleDialogDelegateView, View)

public:
    template <typename T> static bool IsBubbleDialogDelegateView(const BubbleDialogDelegateView* view)
    {
    ...
    template <typename T>
    static Widget* CreateBubble(std::unique_ptr<T> delegate, Widget::InitParams::Ownership ownership = Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET)
    {
        CHECK(IsBubbleDialogDelegateView<T>(delegate.get()));
        return BubbleDialogDelegate::CreateBubble(std::move(delegate), ownership);
    }
    static Widget* CreateBubble(
        BubbleDialogDelegateView* bubble_delegate, Widget::InitParams::Ownership ownership = Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET);
    ...
    BubbleDialogDelegateView(View* anchor_view, BubbleBorder::Arrow arrow, BubbleBorder::Shadow shadow = BubbleBorder::DIALOG_SHADOW, bool autosize = false);
...
VIEW_BUILDER_OVERLOAD_METHOD_CLASS(WidgetDelegate, SetTitle, const std::u16string&)
