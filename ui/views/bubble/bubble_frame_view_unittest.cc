...
public:
    ...
    {
        if (!contents_view_) {
            StaticSizedView* contents_view = new StaticSizedView(kPreferredClientSize);
...
public:
    ...
    {
        SetBubbleBorder(std::make_unique<BubbleBorder>(kArrow, BubbleBorder::STANDARD_SHADOW));
        widget_ = std::make_unique<Widget>();
        widget_delegate_ = std::make_unique<TestBubbleFrameViewWidgetDelegate>();
        Widget::InitParams params = test_base->CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_BUBBLE);
    ...
    {
        return bubble_border()->GetInsets();
    }
...
TEST_F(BubbleFrameViewTest, GetBoundsForClientViewWithClose)
{
    ...
    const gfx::Insets content_margins = frame.GetContentMargins();
    const gfx::Insets insets = frame.GetBorderInsets();
    const int close_margin = frame.close_button()->height() + LayoutProvider::Get()->GetDistanceMetric(DISTANCE_CLOSE_BUTTON_MARGIN);
    const gfx::Rect client_view_bounds = frame.GetBoundsForClientView();
    EXPECT_EQ(insets.left() + content_margins.left(), client_view_bounds.x());
    EXPECT_EQ(insets.top() + content_margins.top() + close_margin, client_view_bounds.y());
}
...
TEST_F(BubbleFrameViewTest, FootnoteContainerViewShouldMatchVisibilityOfFirstChild)
{
    TestBubbleFrameView frame(this);
    std::unique_ptr<View> footnote = std::make_unique<StaticSizedView>(gfx::Size(200, 200));
...
TEST_F(BubbleFrameViewTest, GetUpdatedWindowBounds)
{
    ...
    frame.SetBubbleBorder(std::make_unique<BubbleBorder>(kArrow, BubbleBorder::NO_SHADOW));

    // Test that the info bubble displays normally when it fits.
    frame.SetArrow(BubbleBorder::TOP_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(100, 100, 0, 0), // |anchor_rect|
    ...
    frame.SetArrow(BubbleBorder::BOTTOM_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(900, 100, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::BOTTOM_LEFT, // |delegate_arrow|
        gfx::Size(500, 500), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    ...
    frame.SetArrow(BubbleBorder::TOP_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(900, 900, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_LEFT, // |delegate_arrow|
        gfx::Size(500, 500), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    ...
    frame.SetArrow(BubbleBorder::TOP_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(100, 900, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_LEFT, // |delegate_arrow|
        gfx::Size(500, 500), // |client_size|
        true); // |adjust_to_fit_available_bounds|
...
{
    TestBubbleFrameView frame(this);
    frame.SetBubbleBorder(std::make_unique<BubbleBorder>(BubbleBorder::TOP_RIGHT, BubbleBorder::NO_SHADOW));
    gfx::Rect window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(100, 900, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_RIGHT, // |delegate_arrow|
        gfx::Size(500, 500), // |client_size|
...
TEST_F(BubbleFrameViewTest, GetUpdatedWindowBoundsCenterArrows)
{
    ...
    frame.SetArrow(BubbleBorder::BOTTOM_CENTER);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(500, 900, 50, 50), // |anchor_rect|
        BubbleBorder::Arrow::BOTTOM_CENTER, // |delegate_arrow|
        gfx::Size(500, 500), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::BOTTOM_CENTER, frame.GetBorderArrow());
    ...
    frame.SetArrow(BubbleBorder::LEFT_CENTER);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(100, 400, 50, 50), // |anchor_rect|
        BubbleBorder::Arrow::LEFT_CENTER, // |delegate_arrow|
        gfx::Size(500, 500), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::LEFT_CENTER, frame.GetBorderArrow());
    ...
    frame.SetArrow(BubbleBorder::RIGHT_CENTER);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(900, 400, 50, 50), // |anchor_rect|
        BubbleBorder::Arrow::RIGHT_CENTER, // |delegate_arrow|
        gfx::Size(500, 500), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::RIGHT_CENTER, frame.GetBorderArrow());
    ...
    frame.SetArrow(BubbleBorder::BOTTOM_CENTER);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(100, 900, 50, 50), // |anchor_rect|
        BubbleBorder::Arrow::BOTTOM_CENTER, // |delegate_arrow|
        gfx::Size(500, 500), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::BOTTOM_CENTER, frame.GetBorderArrow());
    ...
    frame.SetArrow(BubbleBorder::BOTTOM_CENTER);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(900, 900, 50, 50), // |anchor_rect|
        BubbleBorder::Arrow::BOTTOM_CENTER, // |delegate_arrow|
        gfx::Size(500, 500), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::BOTTOM_CENTER, frame.GetBorderArrow());
...
TEST_F(BubbleFrameViewTest, GetUpdatedWindowBoundsForBubbleWithAnchorWindow)
{
    ...
    // Test that the bubble displays normally when it fits.
    frame.SetArrow(BubbleBorder::TOP_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(200, 200, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_LEFT, // |delegate_arrow|
    ...
    frame.SetArrow(BubbleBorder::BOTTOM_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(500, 200, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::BOTTOM_LEFT, // |delegate_arrow|
        gfx::Size(250, 250), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::TOP_RIGHT, frame.GetBorderArrow());
    ...
    // rect.
    frame.SetArrow(BubbleBorder::TOP_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(500, 500, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_LEFT, // |delegate_arrow|
        gfx::Size(250, 250), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::BOTTOM_RIGHT, frame.GetBorderArrow());
    ...
    // anchor rect.
    frame.SetArrow(BubbleBorder::TOP_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(200, 500, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_LEFT, // |delegate_arrow|
        gfx::Size(250, 250), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::BOTTOM_LEFT, frame.GetBorderArrow());
...
// Tests that the arrow is mirrored as needed to better fit the screen.
TEST_F(BubbleFrameViewTest, GetUpdatedWindowBoundsForBubbleWithAnchorWindowExitingScreen)
{
    TestBubbleFrameView frame(this);
    gfx::Rect window_bounds;
    ...
    frame.SetArrow(BubbleBorder::TOP_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(800, 300, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_LEFT, // |delegate_arrow|
        gfx::Size(250, 250), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::TOP_RIGHT, frame.GetBorderArrow());
    ...
    frame.SetArrow(BubbleBorder::TOP_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(800, 800, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_LEFT, // |delegate_arrow|
        gfx::Size(250, 250), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::BOTTOM_RIGHT, frame.GetBorderArrow());
    ...
    frame.SetAvailableAnchorWindowBounds(gfx::Rect(700, 200, 400, 400));
    frame.SetArrow(BubbleBorder::TOP_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(800, 500, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_LEFT, // |delegate_arrow|
        gfx::Size(250, 250), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::BOTTOM_RIGHT, frame.GetBorderArrow());
...
TEST_F(BubbleFrameViewTest, MirroringNotStickyForGetUpdatedWindowBounds)
{
    ...
    // original anchor.
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(700, 300, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_LEFT, // |delegate_arrow|
        gfx::Size(250, 250), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::TOP_LEFT, frame.GetBorderArrow());
...
TEST_F(BubbleFrameViewTest, GetUpdatedWindowBoundsForBubbleSetToOffset)
{
    TestBubbleFrameView frame(this);
    frame.SetAvailableAnchorWindowBounds(gfx::Rect(100, 100, 500, 500));
    frame.SetPreferredArrowAdjustment(BubbleFrameView::PreferredArrowAdjustment::kOffset);
    gfx::Rect window_bounds;
    ...
    frame.SetArrow(BubbleBorder::TOP_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(900, 500, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_LEFT, // |delegate_arrow|
        gfx::Size(250, 250), // |client_size|
        true); // |adjust_to_fit_available_bounds|
    EXPECT_EQ(BubbleBorder::TOP_LEFT, frame.GetBorderArrow());
...
TEST_F(BubbleFrameViewTest, GetUpdatedWindowBoundsForBubbleSetToOffsetLargerThanAvailableBounds)
{
    TestBubbleFrameView frame(this);
    ...
    frame.SetArrow(BubbleBorder::TOP_LEFT);
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(300, 300, 0, 0), // |anchor_rect|
        BubbleBorder::Arrow::TOP_LEFT, // |delegate_arrow|
    ...
    window_bounds = frame.GetUpdatedWindowBounds(gfx::Rect(400, 400, 0, 0), // |anchor_rect|
...
TEST_F(BubbleFrameViewTest, GetPreferredSizeWithFootnote)
{
    ...
    with_footnote_size.Enlarge(0, kFootnoteHeight + kFootnoteTopBorderThickness + frame.GetContentMargins().height());
...
TEST_F(BubbleFrameViewTest, LayoutWithHeader)
{
    ...
    std::unique_ptr<View> header = std::make_unique<StaticSizedView>(gfx::Size(10, kHeaderHeight));
...
TEST_F(BubbleFrameViewTest, LayoutWithHeaderAndCloseButton)
{
    ...
    // close button.
    frame.SetHeaderView(std::make_unique<StaticSizedView>(gfx::Size(10, close_margin - 1)));
    ...
    EXPECT_EQ(insets.top() + content_margins.top() + close_margin + 1, client_view_bounds.y());
}
...
public:
    TestBubbleDialogDelegateView()
        : BubbleDialogDelegateView(nullptr, BubbleBorder::NONE, BubbleBorder::NO_SHADOW, true)
    {
    ...
    {
        return static_cast<BubbleFrameView*>(GetWidget()->non_client_view()->frame_view());
    }
...
public:
    ...
    {
        DialogDelegate::SetButtons(should_snap ? static_cast<int>(ui::mojom::DialogButton::kOk) : static_cast<int>(ui::mojom::DialogButton::kNone));
...
TEST_F(BubbleFrameViewTest, WidthSnaps)
{
    ...
    {
        TestWidthSnapDelegate* const delegate = new TestWidthSnapDelegate(&anchor, true);
        WidgetAutoclosePtr widget(delegate->GetWidget());
        EXPECT_EQ(delegate->GetPreferredSize({}).width(), delegate->GetWidget()->GetWindowBoundsInScreen().width());
    }
...

    {
        ...
        // LayoutProvider.
        EXPECT_EQ(kTestWidth, delegate->GetWidget()->GetWindowBoundsInScreen().width());
    }

    {
        // If the DialogDelegate asks not to snap, it should not snap.
        TestWidthSnapDelegate* const delegate = new TestWidthSnapDelegate(&anchor, false);
        WidgetAutoclosePtr widget(delegate->GetWidget());
        EXPECT_EQ(delegate->GetPreferredSize({}).width(), delegate->GetWidget()->GetWindowBoundsInScreen().width());
    }
}
...
TEST_F(BubbleFrameViewTest, LayoutEdgeCases)
{
    ...
    Widget* bubble = BubbleDialogDelegateView::CreateBubble(std::move(delegate_unique));
    bubble->Show();
...
TEST_F(BubbleFrameViewTest, LayoutEdgeCasesWithHeader)
{
    ...
    frame->SetHeaderView(std::make_unique<StaticSizedView>(gfx::Size(10, close_margin)));
...
TEST_F(BubbleFrameViewTest, LayoutSubtitleEdgeCases)
{
    ...
    int line_height_diff = bubble->GetWindowBoundsInScreen().height() - min_bubble_height;
    ...
    line_height_diff = bubble->GetWindowBoundsInScreen().height() - min_bubble_height;
    EXPECT_GT(line_height_diff, 12);
    EXPECT_LT(line_height_diff, 18);
...
TEST_F(BubbleFrameViewTest, LayoutWithIcon)
{
    ...
    delegate->SetIcon(ui::ImageModel::FromImageSkia(gfx::ImageSkia::CreateFrom1xBitmap(bitmap)));
    delegate->SetShowIcon(true);

    Widget* widget = BubbleDialogDelegateView::CreateBubble(std::move(delegate_unique));
    widget->Show();
...
TEST_F(BubbleFrameViewTest, NoElideTitle)
{
    ...
    delegate->ChangeTitle(title);
    Label* title_label = static_cast<Label*>(delegate->GetBubbleFrameView()->title());
    ...
    EXPECT_GE(bubble->GetClientAreaBoundsInScreen().width(), title_label->GetPreferredSize({ title_label->width(), {} }).width());
    EXPECT_LE(title_label->GetPreferredSize({ title_label->width(), {} }).width(), title_label->width());
...
TEST_F(BubbleFrameViewTest, LabelWithHeadingLevel)
{
    ...
    EXPECT_TRUE(node_data.HasIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel));
    EXPECT_EQ(node_data.GetIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel), 1);
}
...
TEST_F(BubbleFrameViewTest, IgnorePossiblyUnintendedClicksClose)
{
    ...
    test::ButtonTestApi(frame->close_)
        .NotifyClick(ui::MouseEvent(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(),
            ui::EventTimeForNow() + base::Milliseconds(GetDoubleClickInterval()), ui::EF_NONE, ui::EF_NONE));
...
TEST_F(BubbleFrameViewTest, IgnorePossiblyUnintendedClicksMinimize)
{
    ...
    views::test::PropertyWaiter minimize_waiter(base::BindRepeating(&Widget::IsMinimized, base::Unretained(bubble)), true);
    test::ButtonTestApi(frame->minimize_)
        .NotifyClick(ui::MouseEvent(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(),
            ui::EventTimeForNow() + base::Milliseconds(GetDoubleClickInterval()), ui::EF_NONE, ui::EF_NONE));
...
TEST_F(BubbleFrameViewTest, IgnorePossiblyUnintendedClicksAnchorBoundsChanged)
{
    ...
    Widget* bubble = BubbleDialogDelegateView::CreateBubble(std::move(delegate_unique));
    bubble->Show();
    ui::MouseEvent mouse_event(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    BubbleFrameView* frame = delegate->GetBubbleFrameView();
    ...
    task_environment()->FastForwardBy(base::Milliseconds(GetDoubleClickInterval()));
    anchor.widget().SetBounds(gfx::Rect(10, 10, 100, 100));

    ui::MouseEvent mouse_event_1(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    test::ButtonTestApi(ok_button).NotifyClick(mouse_event_1);
    test::ButtonTestApi(frame->minimize_).NotifyClick(mouse_event_1);
    EXPECT_FALSE(widget->IsClosed());
    EXPECT_FALSE(bubble->IsMinimized());

    test::ButtonTestApi(ok_button).NotifyClick(ui::MouseEvent(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(),
        ui::EventTimeForNow() + base::Milliseconds(GetDoubleClickInterval()), ui::EF_NONE, ui::EF_NONE));
...
TEST_F(BubbleFrameViewTest, LayoutWithProgressIndicator)
{
    ...
    EXPECT_EQ(progress_indicator->width(), bubble->GetWindowBoundsInScreen().width());
}
...
TEST_F(BubbleFrameViewTest, MinimizeBeforeClose)
{
    ...
    auto minimze_iter = std::find_if(delegate->GetBubbleFrameView()->children().begin(), delegate->GetBubbleFrameView()->children().end(),
        [](views::View* child) { return child->GetProperty(views::kElementIdentifierKey) == BubbleFrameView::kMinimizeButtonElementId; });
    ASSERT_NE(minimze_iter, delegate->GetBubbleFrameView()->children().end());
    EXPECT_EQ((*++minimze_iter)->GetProperty(views::kElementIdentifierKey), BubbleFrameView::kCloseButtonElementId);
}
