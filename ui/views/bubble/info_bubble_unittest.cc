...
public:
    ...
    void SetUp() override
    {
        ...
        Widget::InitParams params = CreateParamsForTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
...
TEST_F(InfoBubbleTest, CreateInfoBubble)
{
    ...
    InfoBubble* info_bubble = new InfoBubble(anchor_widget()->GetContentsView(), BubbleBorder::Arrow::TOP_LEFT, text);
    info_bubble->Show();
...
{
    InfoBubble* info_bubble = new InfoBubble(anchor_widget()->GetContentsView(), BubbleBorder::Arrow::TOP_LEFT, std::u16string());
...
{
    InfoBubble* info_bubble = new InfoBubble(anchor_widget()->GetContentsView(), BubbleBorder::Arrow::TOP_LEFT, u"");
    info_bubble->Show();

    gfx::Rect original_bounds = info_bubble->GetWidget()->GetWindowBoundsInScreen();

    anchor_widget()->SetBounds(original_bounds - gfx::Vector2d(5, 5));

    EXPECT_NE(original_bounds, info_bubble->GetWidget()->GetWindowBoundsInScreen());
    info_bubble->Hide();
    RunPendingMessages();
}
