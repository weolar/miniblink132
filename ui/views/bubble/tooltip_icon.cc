    ...
    bubble_(nullptr)
{
    ...
    SetBorder(CreateEmptyBorder(LayoutProvider::Get()->GetInsetsMetric(INSETS_VECTOR_IMAGE_BUTTON)));
...
{
    mouse_inside_ = true;
    show_timer_.Start(FROM_HERE, base::Milliseconds(150), this, &TooltipIcon::ShowBubble);
}
...
void TooltipIcon::SetDrawAsHovered(bool hovered)
{
    SetImage(ui::ImageModel::FromVectorIcon(
        vector_icons::kInfoOutlineIcon, GetColorProvider()->GetColor(hovered ? ui::kColorHelpIconActive : ui::kColorHelpIconInactive), tooltip_icon_size_));
}
...
void TooltipIcon::ShowBubble()
{
    ...
    if (mouse_inside_) {
        ...
        mouse_watcher_ = std::make_unique<MouseWatcher>(std::make_unique<MouseWatcherViewHost>(frame, gfx::Insets()), this);
        mouse_watcher_->Start(GetWidget()->GetNativeWindow());
    }
