...
BubbleDialogModelHost::FieldType GetFieldTypeForField(ui::DialogModelField* field)
{
    DCHECK(field);
    switch (field->type()) {
    ...
    case ui::DialogModelField::kCustom:
        return static_cast<BubbleDialogModelHost::CustomView*>(field->AsCustomField()->field())->field_type();
    }
}

int GetFieldTopMargin(
    LayoutProvider* layout_provider, const BubbleDialogModelHost::FieldType& field_type, const BubbleDialogModelHost::FieldType& last_field_type)
{
    DCHECK(layout_provider);
    // Menu item preceded by non-menu item should have margin
    if (field_type == BubbleDialogModelHost::FieldType::kMenuItem && last_field_type == BubbleDialogModelHost::FieldType::kMenuItem) {
        return 0;
    }
    if (field_type == BubbleDialogModelHost::FieldType::kControl && last_field_type == BubbleDialogModelHost::FieldType::kControl) {
        // TODO(pbos): Move DISTANCE_CONTROL_LIST_VERTICAL to views::LayoutProvider
        // and replace "12" here.
        return 12;
    }
    return layout_provider->GetDistanceMetric(DISTANCE_UNRELATED_CONTROL_VERTICAL);
}

int GetDialogTopMargins(LayoutProvider* layout_provider, ui::DialogModelField* first_field)
{
    CHECK(first_field);

    const BubbleDialogModelHost::FieldType field_type = GetFieldTypeForField(first_field);
    switch (field_type) {
    case BubbleDialogModelHost::FieldType::kMenuItem:
        return 0;
    case BubbleDialogModelHost::FieldType::kControl:
        return layout_provider->GetDistanceMetric(DISTANCE_DIALOG_CONTENT_MARGIN_TOP_CONTROL);
    case BubbleDialogModelHost::FieldType::kText:
        return layout_provider->GetDistanceMetric(DISTANCE_DIALOG_CONTENT_MARGIN_TOP_TEXT);
    }
}

int GetDialogBottomMargins(LayoutProvider* layout_provider, ui::DialogModelField* last_field, bool has_buttons)
{
    CHECK(last_field);
    const BubbleDialogModelHost::FieldType field_type = GetFieldTypeForField(last_field);
    switch (field_type) {
    case BubbleDialogModelHost::FieldType::kMenuItem:
        return has_buttons ? layout_provider->GetDistanceMetric(DISTANCE_DIALOG_CONTENT_MARGIN_BOTTOM_CONTROL) : 0;
    case BubbleDialogModelHost::FieldType::kControl:
        return layout_provider->GetDistanceMetric(DISTANCE_DIALOG_CONTENT_MARGIN_BOTTOM_CONTROL);
    case BubbleDialogModelHost::FieldType::kText:
        return layout_provider->GetDistanceMetric(DISTANCE_DIALOG_CONTENT_MARGIN_BOTTOM_TEXT);
    }
}
...
public:
        ...
        : label_line_height_(label_line_height)
    {
        ...
        layout->set_between_child_spacing(LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_LABEL_HORIZONTAL));
    ...
    void OnThemeChanged() override
    {
        ...
        image_container_view()->SetBorder(
            CreateEmptyBorder(gfx::Insets::TLBR((label_line_height_ - image_container_view()->GetPreferredSize({}).height()) / 2, 0, 0, 0)));
    }
...
{
    return field_view_info.focusable_view ? field_view_info.focusable_view.get() : field_view_info.field_view.get();
}
...
public:
    ...
    gfx::Size GetMaxPreferredSize(const SizeBounds& available_size) const
    {
        ...
        for (View* child : children_) {
            ...
            size.SetToMax(child->children().front()->GetPreferredSize(available_size));
        }
        return size;
    }
...
public:
    ...
    ~LayoutConsensusView() override
    {
        group_->RemoveView(this);
    }

    gfx::Size CalculatePreferredSize(const SizeBounds& available_size) const override
    {
        const gfx::Size group_preferred_size = group_->GetMaxPreferredSize(available_size);
        DCHECK_EQ(1u, children().size());
        const gfx::Size child_preferred_size = children()[0]->GetPreferredSize(available_size);
        // TODO(pbos): This uses the max width, but could be configurable to use
        // either direction.
        return gfx::Size(group_preferred_size.width(), child_preferred_size.height());
    }
...
BubbleDialogModelHost::CustomView::CustomView(std::unique_ptr<View> view, FieldType field_type, View* focusable_view)
    : view_(std::move(view))
...
public:
    ...
    BubbleDialogModelHostContentsView(ui::DialogModelSection* contents, ui::ElementIdentifier initially_focused_field_id)
        : contents_(contents)
        , initially_focused_field_id_(initially_focused_field_id)
        , on_field_added_subscription_(
              contents->AddOnFieldAddedCallback(base::BindRepeating(&BubbleDialogModelHostContentsView::OnFieldAdded, base::Unretained(this))))
        , on_field_changed_subscription_(
              contents->AddOnFieldChangedCallback(base::BindRepeating(&BubbleDialogModelHostContentsView::OnFieldChanged, base::Unretained(this))))
    {
    ...
    [[nodiscard]] base::CallbackListSubscription AddOnContentsChangedCallback(base::RepeatingClosure on_contents_changed)
    {
    ...
    void AddOrUpdateParagraph(ui::DialogModelParagraph* model_field)
    {
        ...
        std::unique_ptr<View> view = model_field->header().empty() ? CreateViewForLabel(model_field->label())
                                                                   : CreateViewForParagraphWithHeader(model_field->label(), model_field->header());
    ...
    void AddOrUpdateCheckbox(ui::DialogModelCheckbox* model_field)
    {
        ...
        checkbox->SetCallback(base::BindRepeating([](ui::DialogModelCheckbox* model_field, base::PassKey<DialogModelFieldHost> pass_key, Checkbox* checkbox,
                                                      const ui::Event& event) { model_field->OnChecked(pass_key, checkbox->GetChecked()); },
    ...
    void AddOrUpdateCombobox(ui::DialogModelCombobox* model_field)
    {
        ...
        combobox->GetViewAccessibility().SetName(model_field->accessible_name().empty() ? model_field->label() : model_field->accessible_name());
        combobox->SetCallback(base::BindRepeating([](ui::DialogModelCombobox* model_field, base::PassKey<DialogModelFieldHost> pass_key,
        ...
        property_changed_subscriptions_.push_back(combobox->AddSelectedIndexChangedCallback(
            base::BindRepeating([](ui::DialogModelCombobox* model_field, base::PassKey<DialogModelFieldHost> pass_key,
                                    Combobox* combobox) { model_field->OnSelectedIndexChanged(pass_key, combobox->GetSelectedIndex().value()); },
                model_field, GetPassKey(), combobox.get())));
        AddViewForLabelAndField(model_field, model_field->label(), std::move(combobox));
    }
    ...
    void AddOrUpdateMenuItem(ui::DialogModelMenuItem* model_field)
    {
        ...
        auto item = std::make_unique<LabelButton>(base::BindRepeating([](ui::DialogModelMenuItem* model_field, base::PassKey<DialogModelFieldHost> pass_key,
                                                                          const ui::Event& event) { model_field->OnActivated(pass_key, event.flags()); },
        ...
        item->SetBorder(views::CreateEmptyBorder(gfx::Insets::VH(12 / 2, LayoutProvider::Get()->GetDistanceMetric(DISTANCE_BUTTON_HORIZONTAL_PADDING))));
    ...
    void AddOrUpdatePasswordField(ui::DialogModelPasswordField* model_field)
    {
        ...
        if (!model_field->incorrect_password_text().empty()) {
            ...
            error_label = std::make_unique<Label>(model_field->incorrect_password_text(), style::CONTEXT_TEXTFIELD_SUPPORTING_TEXT, style::STYLE_INVALID);
        ...
        auto password_combobox
            = std::make_unique<views::EditablePasswordCombobox>(std::make_unique<ui::SimpleComboboxModel>(std::vector<ui::SimpleComboboxModel::Item>()),
                views::style::CONTEXT_TEXTFIELD, style::STYLE_PRIMARY_MONOSPACED, false);
        password_combobox->SetPasswordIconTooltips(
            l10n_util::GetStringUTF16(IDS_SETTINGS_PASSWORD_SHOW), l10n_util::GetStringUTF16(IDS_SETTINGS_PASSWORD_HIDE));
        password_combobox->GetViewAccessibility().SetName(model_field->accessible_name().empty() ? model_field->label() : model_field->accessible_name());

        password_combobox->SetCallback(base::BindRepeating(
            [](ui::DialogModelPasswordField* model_field, base::PassKey<DialogModelFieldHost> pass_key, EditablePasswordCombobox* combobox,
                Label* error_label) {
        ...
        property_changed_subscriptions_.push_back(model_field->AddOnInvalidateCallback(GetPassKey(),
            base::BindRepeating(
    ...
    void AddOrUpdateTextfield(ui::DialogModelTextfield* model_field)
    {
        ...
        textfield->GetViewAccessibility().SetName(model_field->accessible_name().empty() ? model_field->label() : model_field->accessible_name());
        ...
        if (initially_focused_field_id_ && model_field->id() == initially_focused_field_id_) {
            textfield->SelectAll(true);
        }

        property_changed_subscriptions_.push_back(
            textfield->AddTextChangedCallback(base::BindRepeating([](ui::DialogModelTextfield* model_field, base::PassKey<DialogModelFieldHost> pass_key,
                                                                      Textfield* textfield) { model_field->OnTextChanged(pass_key, textfield->GetText()); },
                model_field, GetPassKey(), textfield.get())));

        AddViewForLabelAndField(model_field, model_field->label(), std::move(textfield));
    }

    void AddOrUpdateCustomField(ui::DialogModelCustomField* model_field)
    {
        auto* custom_view = static_cast<BubbleDialogModelHost::CustomView*>(model_field->field());
    ...
    void AddViewForLabelAndField(ui::DialogModelField* model_field, const std::u16string& label_text, std::unique_ptr<View> field)
    {
        auto box_layout = std::make_unique<BoxLayoutView>();

        box_layout->SetBetweenChildSpacing(LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_CONTROL_HORIZONTAL));

        DialogModelHostField info { model_field, box_layout.get(), field.get() };

        auto label = std::make_unique<Label>(label_text, style::CONTEXT_LABEL, style::STYLE_PRIMARY);
        label->SetHorizontalAlignment(gfx::ALIGN_LEFT);

        box_layout->AddChildView(std::make_unique<LayoutConsensusView>(&textfield_first_column_group_, std::move(label)));
        box_layout->SetFlexForView(box_layout->AddChildView(std::make_unique<LayoutConsensusView>(&textfield_second_column_group_, std::move(field))), 1);

        AddDialogModelHostField(std::move(box_layout), info);
    }

    static bool DialogModelLabelRequiresStyledLabel(const ui::DialogModelLabel& dialog_label)
    {
        return !dialog_label.replacements().empty();
    }

    static std::unique_ptr<View> CreateViewForLabel(const ui::DialogModelLabel& dialog_label)
    {
    ...
    static std::unique_ptr<StyledLabel> CreateStyledLabelForDialogModelLabel(const ui::DialogModelLabel& dialog_label)
    {
        DCHECK(DialogModelLabelRequiresStyledLabel(dialog_label));
        const std::vector<ui::DialogModelLabel::TextReplacement>& replacements = dialog_label.replacements();
        ...
        const std::u16string text = l10n_util::GetStringFUTF16(dialog_label.message_id(), string_replacements, &offsets);

        auto styled_label = std::make_unique<StyledLabel>();
        styled_label->SetText(text);
        styled_label->SetDefaultTextStyle(dialog_label.is_secondary() ? style::STYLE_SECONDARY : style::STYLE_PRIMARY);
        ...
        for (size_t i = 0; i < replacements.size(); ++i) {
            ...
            if (replacement.callback().has_value()) {
                style_info = StyledLabel::RangeStyleInfo::CreateForLink(replacement.callback().value());
            ...
            styled_label->AddStyleRange(gfx::Range(offset, offset + replacement.text().length()), style_info);
        }

        return styled_label;
    }

    static std::unique_ptr<Label> CreateLabelForDialogModelLabel(const ui::DialogModelLabel& dialog_label)
    {
        DCHECK(!DialogModelLabelRequiresStyledLabel(dialog_label));

        auto text_label = std::make_unique<Label>(
            dialog_label.GetString(), style::CONTEXT_DIALOG_BODY_TEXT, dialog_label.is_secondary() ? style::STYLE_SECONDARY : style::STYLE_PRIMARY);
        text_label->SetMultiLine(true);
        text_label->SetHorizontalAlignment(gfx::ALIGN_LEFT);
        return text_label;
    }

    std::unique_ptr<View> CreateViewForParagraphWithHeader(const ui::DialogModelLabel& dialog_label, const std::u16string& header)
    {
        auto view = std::make_unique<BoxLayoutView>();
        view->SetOrientation(BoxLayout::Orientation::kVertical);

        auto* header_label = view->AddChildView(std::make_unique<Label>(header, style::CONTEXT_DIALOG_BODY_TEXT, style::STYLE_PRIMARY));
    ...
    void AddDialogModelHostField(std::unique_ptr<View> view, const DialogModelHostField& field_view_info)
    {
        ...
        target->SetProperty(kElementIdentifierKey, field_view_info.dialog_model_field->id());
        for (const auto& accelerator : field_view_info.dialog_model_field->accelerators()) {
...
std::unique_ptr<DialogModelSectionHost> DialogModelSectionHost::Create(ui::DialogModelSection* section, ui::ElementIdentifier initially_focused_field_id)
{
    return std::make_unique<BubbleDialogModelHostContentsView>(section, initially_focused_field_id);
}

BEGIN_METADATA(DialogModelSectionHost)
END_METADATA

BubbleDialogModelHost::ThemeChangedObserver::ThemeChangedObserver(BubbleDialogModelHost* parent, BubbleDialogModelHostContentsView* contents_view)
    : parent_(parent)
{
...
BubbleDialogModelHost::BubbleDialogModelHost(std::unique_ptr<ui::DialogModel> model, View* anchor_view, BubbleBorder::Arrow arrow, bool autosize)
    : BubbleDialogModelHost(base::PassKey<BubbleDialogModelHost>(), std::move(model), anchor_view, arrow, ui::mojom::ModalType::kNone, autosize)
{
}

BubbleDialogModelHost::BubbleDialogModelHost(base::PassKey<BubbleDialogModelHost>, std::unique_ptr<ui::DialogModel> model, View* anchor_view,
    BubbleBorder::Arrow arrow, ui::mojom::ModalType modal_type, bool autosize)
    : BubbleDialogDelegate(anchor_view, arrow, views::BubbleBorder::DIALOG_SHADOW, autosize)
    , model_(std::move(model))
    ,
    // Make sure the modal type is set before calling InitContentsView which
    // uses IsModalDialog().
    contents_view_((SetModalType(modal_type), InitContentsView(model_->contents())))
    , on_contents_changed_subscription_(
          contents_view_->AddOnContentsChangedCallback(base::BindRepeating(&BubbleDialogModelHost::OnContentsViewChanged, base::Unretained(this))))
    , theme_observer_(this, contents_view_)
{
    ...
    SetAcceptCallbackWithClose(base::BindRepeating(&ui::DialogModel::OnDialogAcceptAction, base::Unretained(model_.get()), DialogModelHost::GetPassKey()));
    SetCancelCallbackWithClose(base::BindRepeating(&ui::DialogModel::OnDialogCancelAction, base::Unretained(model_.get()), DialogModelHost::GetPassKey()));
    SetCloseCallback(base::BindOnce(&ui::DialogModel::OnDialogCloseAction, base::Unretained(model_.get()), DialogModelHost::GetPassKey()));
    ...
    // already been closed.
    RegisterWindowClosingCallback(base::BindOnce(&BubbleDialogModelHost::OnWindowClosing, base::Unretained(this)));
    ...
    if (ok_button) {
        ...
        ConfigureBubbleButtonForParams(*this, /*button_view=*/nullptr, ui::mojom::DialogButton::kOk, *ok_button);
    }
    ...
    if (cancel_button) {
        ...
        ConfigureBubbleButtonForParams(*this, /*button_view=*/nullptr, ui::mojom::DialogButton::kCancel, *cancel_button);
    }
    ...
    if (ui::DialogModel::Button* extra_button = model_->extra_button(DialogModelHost::GetPassKey())) {
        DCHECK(!model_->extra_link(DialogModelHost::GetPassKey()));
        auto builder = views::Builder<MdTextButton>()
                           .SetCallback(base::BindRepeating(&ui::DialogModel::Button::OnPressed, base::Unretained(extra_button), DialogModelHost::GetPassKey()))
    ...
    } else if (ui::DialogModelLabel::TextReplacement* extra_link = model_->extra_link(DialogModelHost::GetPassKey())) {
    ...
    if (model_->override_default_button(DialogModelHost::GetPassKey())) {
        SetDefaultButton(static_cast<int>(model_->override_default_button(DialogModelHost::GetPassKey()).value()));
    }
    ...
    if (model_->override_show_close_button(DialogModelHost::GetPassKey())) {
        SetShowCloseButton(*model_->override_show_close_button(DialogModelHost::GetPassKey()));
    } else {
    ...
    set_close_on_deactivate(model_->close_on_deactivate(DialogModelHost::GetPassKey()));
    ...
    set_fixed_width(LayoutProvider::Get()->GetDistanceMetric(anchor_view ? DISTANCE_BUBBLE_PREFERRED_WIDTH : DISTANCE_MODAL_DIALOG_PREFERRED_WIDTH));

    if (model_->footnote_label()) {
        SetFootnoteView(BubbleDialogModelHostContentsView::CreateViewForLabel(*model_->footnote_label()));
    }
...
std::unique_ptr<BubbleDialogModelHost> BubbleDialogModelHost::CreateModal(
    std::unique_ptr<ui::DialogModel> model, ui::mojom::ModalType modal_type, bool autosize)
{
    DCHECK_NE(modal_type, ui::mojom::ModalType::kNone);
    return std::make_unique<BubbleDialogModelHost>(
        base::PassKey<BubbleDialogModelHost>(), std::move(model), nullptr, BubbleBorder::Arrow::NONE, modal_type, autosize);
}
...
View* BubbleDialogModelHost::GetInitiallyFocusedView()
{
    ...
    ui::ElementIdentifier unique_id = model_->initially_focused_field(DialogModelHost::GetPassKey());

    if (!unique_id)
        return BubbleDialogDelegate::GetInitiallyFocusedView();

    if (ui::DialogModel::Button* const ok_button = model_->ok_button(DialogModelHost::GetPassKey()); ok_button && unique_id == ok_button->id()) {
        return GetOkButton();
    }

    if (ui::DialogModel::Button* const cancel_button = model_->cancel_button(DialogModelHost::GetPassKey());
        cancel_button && unique_id == cancel_button->id()) {
        return GetCancelButton();
    }

    if (ui::DialogModel::Button* const extra_button = model_->extra_button(DialogModelHost::GetPassKey()); extra_button && unique_id == extra_button->id()) {
        return GetExtraView();
    }

    return GetTargetView(contents_view_->FindDialogModelHostField(model_->GetFieldByUniqueId(unique_id)));
}
...
void BubbleDialogModelHost::OnWidgetInitialized()
{
    ...
    if (const ui::ImageModel& banner = model_->banner(DialogModelHost::GetPassKey()); !banner.IsEmpty()) {
        const ui::ImageModel& dark_mode_banner = model_->dark_mode_banner(DialogModelHost::GetPassKey());
        auto banner_view = std::make_unique<ThemeTrackingImageView>(banner.Rasterize(contents_view_->GetColorProvider()),
            (dark_mode_banner.IsEmpty() ? banner : dark_mode_banner).Rasterize(contents_view_->GetColorProvider()),
            base::BindRepeating(&views::BubbleDialogDelegate::GetBackgroundColor, base::Unretained(this)));
...
BubbleDialogModelHostContentsView* BubbleDialogModelHost::InitContentsView(ui::DialogModelSection* contents)
{
    auto contents_view_unique = std::make_unique<BubbleDialogModelHostContentsView>(contents, model_->initially_focused_field(DialogModelHost::GetPassKey()));

    BubbleDialogModelHostContentsView* const contents_view = contents_view_unique.get();
    ...
    if (IsModalDialog()) {
        ...
        contents_view_unique->SetInsideBorderInsets(gfx::Insets::VH(kScrollViewVerticalMargin, 0));
        ...
        const int kMaxDialogHeight = LayoutProvider::Get()->GetDistanceMetric(DISTANCE_MODAL_DIALOG_SCROLLABLE_AREA_MAX_HEIGHT);
        auto scroll_view = std::make_unique<views::ScrollView>();
        scroll_view->ClipHeightTo(0, kMaxDialogHeight);
        scroll_view->SetHorizontalScrollBarMode(views::ScrollView::ScrollBarMode::kDisabled);
...
void BubbleDialogModelHost::UpdateWindowIcon()
{
    ...
    const ui::ImageModel dark_mode_icon = model_->dark_mode_icon(DialogModelHost::GetPassKey());
...
void BubbleDialogModelHost::UpdateSpacingAndMargins()
{
    ...
    gfx::Insets dialog_side_insets = layout_provider->GetInsetsMetric(InsetsMetric::INSETS_DIALOG);
    if (GetWindowTitle().empty()) {
        ...
        dialog_side_insets.set_top(layout_provider->GetInsetsMetric(InsetsMetric::INSETS_DIALOG_TITLE).top());
    } else {
    ...
    auto* scroll_view = views::ScrollView::GetScrollViewForContents(contents_view_);
    const views::View::Views& children = scroll_view ? scroll_view->contents()->children() : contents_view_->children();

    if (children.empty()) {
        // TODO(pbos): Copied from the BubbleDialogDelegate constructor. Maybe there
        // should be a way to reset them if we become empty?
        set_margins(layout_provider->GetDialogInsetsForContentType(DialogContentType::kText, DialogContentType::kText));
        return;
    }

    for (View* const view : children) {
        ui::DialogModelField* const field = contents_view_->FindDialogModelHostField(view).dialog_model_field;

        const FieldType field_type = GetFieldTypeForField(field);

        gfx::Insets side_insets = field_type == FieldType::kMenuItem ? gfx::Insets() : dialog_side_insets;
        ...
        if (!first_field) {
            ...
            side_insets.set_top(GetFieldTopMargin(layout_provider, field_type, last_field_type));
            view->SetProperty(kMarginsKey, side_insets);
        }
        last_field = field;
    }
    ...
    const int top_margin = GetDialogTopMargins(layout_provider, first_field) - extra_margin;
    const int bottom_margin = GetDialogBottomMargins(layout_provider, last_field, buttons() != static_cast<int>(ui::mojom::DialogButton::kNone)) - extra_margin;
    set_margins(gfx::Insets::TLBR(top_margin >= 0 ? top_margin : 0, 0, bottom_margin >= 0 ? bottom_margin : 0, 0));
}
...
{
    if (ui::DialogModel::Button* const ok_button = model_->ok_button(DialogModelHost::GetPassKey())) {
        ConfigureBubbleButtonForParams(*this, GetOkButton(), ui::mojom::DialogButton::kOk, *ok_button);
    }
    if (ui::DialogModel::Button* const cancel_button = model_->cancel_button(DialogModelHost::GetPassKey())) {
        ConfigureBubbleButtonForParams(*this, GetCancelButton(), ui::mojom::DialogButton::kCancel, *cancel_button);
    }
    if (ui::DialogModel::Button* const extra_button = model_->extra_button(DialogModelHost::GetPassKey())) {
