...
DEFINE_OWNED_UI_CLASS_PROPERTY_KEY(std::vector<views::BubbleDialogDelegate*>, kAnchorVector, nullptr)

std::vector<BubbleDialogDelegate*>& GetAnchorVector(View* view)
{
    if (!view->GetProperty(kAnchorVector)) {
        return *(view->SetProperty(kAnchorVector, std::make_unique<std::vector<BubbleDialogDelegate*>>()));
    }
...
public:
    ...
    ui::ColorProviderKey::ThemeInitializerSupplier* GetCustomTheme() const override
    {
        const Widget* const anchor = GetAnchorWidget();
    ...
    Widget* GetPrimaryWindowWidget() override
    {
        ...
        return anchor ? anchor->GetPrimaryWindowWidget() : Widget::GetPrimaryWindowWidget();
    }

    const ui::ColorProvider* GetColorProvider() const override
    {
        const Widget* const primary = GetPrimaryWindowWidget();
        return (primary && primary != this) ? primary->GetColorProvider() : Widget::GetColorProvider();
    }
...
private:
    const Widget* GetAnchorWidget() const
    {
        ...
        BubbleDialogDelegate* const bubble_delegate = static_cast<BubbleDialogDelegate*>(widget_delegate());
...
Widget* CreateBubbleWidget(BubbleDialogDelegate* bubble, Widget::InitParams::Ownership ownership = Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET)
{
    ...
    // autosize.
    CHECK(!bubble->is_autosized() || bubble->use_custom_frame()) << "Autosizing native frame dialogs is not supported.";
    ...
    bubble_params.activatable = bubble->CanActivate() ? Widget::InitParams::Activatable::kYes : Widget::InitParams::Activatable::kNo;
...
#else
class BubbleDialogDelegate::AnchorWidgetObserver : public WidgetObserver, public aura::WindowObserver {
#endif
...
#if !BUILDFLAG(IS_MAC)
    // aura::WindowObserver:
    void OnWindowTransformed(aura::Window* window, ui::PropertyChangeReason reason) override
    {
        ...
        if (!window->layer()->GetAnimator()->IsAnimatingOnePropertyOf(ui::LayerAnimationElement::TRANSFORM)) {
            owner_->OnAnchorBoundsChanged();
        }
    }
...
#if !BUILDFLAG(IS_MAC)
    base::ScopedObservation<aura::Window, aura::WindowObserver> window_observation_ { this };
#endif
};
...
private:
    ...
    base::ScopedObservation<views::Widget, views::WidgetObserver> observation_ { this };
};

class BubbleDialogDelegate::ThemeObserver : public ViewObserver {
public:
    explicit ThemeObserver(BubbleDialogDelegate* delegate)
        : delegate_(delegate)
    {
...
BubbleDialogDelegate::CloseOnDeactivatePin::CloseOnDeactivatePin(base::WeakPtr<Pins> pins)
    : pins_(pins)
{
    pins_->AddPin(this);
}
...
BubbleDialogDelegate::BubbleDialogDelegate(View* anchor_view, BubbleBorder::Arrow arrow, BubbleBorder::Shadow shadow, bool autosize)
    : arrow_(arrow)
    , shadow_(shadow)
    , autosize_(autosize)
    ...
    , bubble_created_time_(base::TimeTicks::Now())
{
    ...
    set_footnote_margins(layout_provider->GetInsetsMetric(INSETS_DIALOG_FOOTNOTE));

    set_desired_bounds_delegate(base::BindRepeating(&BubbleDialogDelegate::GetDesiredBubbleBounds, base::Unretained(this)));

    RegisterWidgetInitializedCallback(base::BindOnce(
        [](BubbleDialogDelegate* bubble_delegate) {
            bubble_delegate->theme_observer_ = std::make_unique<ThemeObserver>(bubble_delegate);
    ...
    RegisterWidgetInitializedCallback(base::BindOnce(
        [](BubbleDialogDelegate* delegate, base::TimeTicks bubble_created_time) {
            delegate->GetWidget()->GetCompositor()->RequestSuccessfulPresentationTimeForNextFrame(base::BindOnce(
                [](base::WeakPtr<BubbleDialogDelegate::BubbleUmaLogger> uma_logger, base::TimeTicks bubble_created_time,
                    const viz::FrameTimingDetails& frame_timing_details) {
                    base::TimeTicks presentation_timestamp = frame_timing_details.presentation_feedback.timestamp;
                    if (!uma_logger) {
                        return;
                    }
                    uma_logger->LogMetric(base::UmaHistogramTimes, "CreateToPresentationTime", presentation_timestamp - bubble_created_time);
                },
                delegate->bubble_uma_logger().GetWeakPtr(), bubble_created_time));
        },
...
// static
Widget* BubbleDialogDelegate::CreateBubble(std::unique_ptr<BubbleDialogDelegate> bubble_delegate_unique, Widget::InitParams::Ownership ownership)
{
    ...
    Widget* const bubble_widget = CreateBubbleWidget(bubble_delegate_unique.release(), ownership);

    bubble_delegate->set_adjust_if_offscreen(PlatformStyle::kAdjustBubbleIfOffscreen);

    bubble_delegate->SizeToContents();
    bubble_delegate->bubble_widget_observer_ = std::make_unique<BubbleWidgetObserver>(bubble_delegate, bubble_widget);
    return bubble_widget;
}

Widget* BubbleDialogDelegateView::CreateBubble(BubbleDialogDelegateView* delegate_view, Widget::InitParams::Ownership ownership)
{
...
BubbleDialogDelegateView::BubbleDialogDelegateView(View* anchor_view, BubbleBorder::Arrow arrow, BubbleBorder::Shadow shadow, bool autosize)
...
std::unique_ptr<NonClientFrameView> BubbleDialogDelegate::CreateNonClientFrameView(Widget* widget)
{
    ...
    std::unique_ptr<BubbleBorder> border = std::make_unique<BubbleBorder>(arrow(), GetShadow());
...
ClientView* BubbleDialogDelegate::CreateClientView(Widget* widget)
{
    ...
    if (paint_client_to_layer_) {
        ...
        client_view_->layer()->SetRoundedCornerRadius(gfx::RoundedCornersF(GetCornerRadius()));
...
void BubbleDialogDelegate::OnBubbleWidgetClosing()
{
    ...
    if (GetAnchorView() && GetAnchorView()->GetProperty(kAnchoredDialogKey) == this) {
    ...
    bubble_uma_logger().LogMetric(base::UmaHistogramLongTimes, "TimeVisible", bubble_shown_duration_);

    bubble_uma_logger().LogMetric(base::UmaHistogramEnumeration, "CloseReason", GetWidget()->closed_reason());
}
...
void BubbleDialogDelegate::OnBubbleWidgetActivationChanged(bool active)
{
    ...
    if (active && !mac_bubble_closer_) {
        mac_bubble_closer_ = std::make_unique<ui::BubbleCloser>(
            GetWidget()->GetNativeWindow().GetNativeNSWindow(), base::BindRepeating(&BubbleDialogDelegate::OnDeactivate, base::Unretained(this)));
    }
#endif
...
std::unique_ptr<BubbleDialogDelegate::CloseOnDeactivatePin> BubbleDialogDelegate::PreventCloseOnDeactivate()
{
    return base::WrapUnique(new CloseOnDeactivatePin(close_on_deactivate_pins_->GetWeakPtr()));
}
...
gfx::Rect BubbleDialogDelegate::GetAnchorRect() const
{
    ...
    // offset.
    if (anchor_widget_) {
        gfx::Transform transform = anchor_widget_->GetNativeWindow()->layer()->GetTargetTransform();
...
gfx::Size BubbleDialogDelegate::GetMaxAvailableScreenSpaceToPlaceBubble(
    View* anchor_view, BubbleBorder::Arrow arrow, bool adjust_if_offscreen, BubbleFrameView::PreferredArrowAdjustment arrow_adjustment)
{
    // TODO(sanchit.abrol@microsoft.com): Implement for other arrows.
    DCHECK(arrow == BubbleBorder::TOP_LEFT || arrow == BubbleBorder::TOP_RIGHT || arrow == BubbleBorder::BOTTOM_RIGHT || arrow == BubbleBorder::BOTTOM_LEFT);
    DCHECK_EQ(arrow_adjustment, BubbleFrameView::PreferredArrowAdjustment::kMirror);

#if BUILDFLAG(IS_OZONE)
    // This function should not be called in ozone platforms where global screen
    // coordinates are not available.
    DCHECK(ui::OzonePlatform::GetInstance()->GetPlatformProperties().supports_global_screen_coordinates);
#endif

    gfx::Rect anchor_rect = anchor_view->GetAnchorBoundsInScreen();
    gfx::Rect screen_rect = display::Screen::GetScreen()->GetDisplayNearestPoint(anchor_rect.CenterPoint()).work_area();

    gfx::Size max_available_space;

    if (adjust_if_offscreen) {
        max_available_space = GetAvailableSpaceToPlaceBubble(BubbleBorder::TOP_LEFT, anchor_rect, screen_rect);
        max_available_space.SetToMax(GetAvailableSpaceToPlaceBubble(BubbleBorder::TOP_RIGHT, anchor_rect, screen_rect));
        max_available_space.SetToMax(GetAvailableSpaceToPlaceBubble(BubbleBorder::BOTTOM_RIGHT, anchor_rect, screen_rect));
        max_available_space.SetToMax(GetAvailableSpaceToPlaceBubble(BubbleBorder::BOTTOM_LEFT, anchor_rect, screen_rect));
    } else {
        max_available_space = GetAvailableSpaceToPlaceBubble(arrow, anchor_rect, screen_rect);
    }
...
// static
gfx::Size BubbleDialogDelegate::GetAvailableSpaceToPlaceBubble(BubbleBorder::Arrow arrow, gfx::Rect anchor_rect, gfx::Rect screen_rect)
{
    ...
    return { BubbleBorder::is_arrow_on_left(arrow) ? available_width_on_right : available_width_on_left,
        BubbleBorder::is_arrow_on_top(arrow) ? available_height_below : available_height_above };
}
...
base::WeakPtr<BubbleDialogDelegate::BubbleUmaLogger> BubbleDialogDelegate::BubbleUmaLogger::GetWeakPtr()
{
    return weak_factory_.GetWeakPtr();
}

std::optional<std::string> BubbleDialogDelegate::BubbleUmaLogger::GetBubbleName() const
{
    ...
    if (delegate_.has_value()) {
        std::string class_name = delegate_.value()->GetContentsView()->GetClassName();
        if (class_name != "View") {
            return class_name;
        }
    }
...
void BubbleDialogDelegate::BubbleUmaLogger::LogMetric(void (*uma_func)(const std::string&, Value), const std::string& histogram_name, Value value) const
{
    ...
    if (allowed_class_names_for_testing_.has_value()) {
        if (!base::Contains(allowed_class_names_for_testing_.value(), bubble_name.value())) {
            return;
        }
    ...
    uma_func(base::StrCat({ "Bubble.", bubble_name.value(), ".", histogram_name }), value);
}

// Instantiate template function to be able to use in views_unittests.
template VIEWS_EXPORT void BubbleDialogDelegate::BubbleUmaLogger::LogMetric<base::TimeDelta>(
    void (*uma_func)(const std::string&, base::TimeDelta), const std::string& histogram_name, base::TimeDelta value) const;
...
gfx::Rect BubbleDialogDelegate::GetBubbleBounds()
{
    ...
    return GetBubbleFrameView()->GetUpdatedWindowBounds(
        anchor_rect, arrow(), GetWidget()->client_view()->GetPreferredSize({}), adjust_if_offscreen_ && !anchor_minimized && has_anchor);
}

ax::mojom::Role BubbleDialogDelegate::GetAccessibleWindowRole()
{
    const ax::mojom::Role accessible_role = WidgetDelegate::GetAccessibleWindowRole();
...
{
    CHECK(use_custom_frame()) << "GetBubbleBounds() for native frame dialogs is not supported.";
    ...
    gfx::Size actual_size = GetWindowSizeForClientSize(GetWidget(), bubble_bounds.size());
...
void BubbleDialogDelegate::SetAnchorView(View* anchor_view)
{
    if (anchor_view && anchor_view->GetWidget()) {
        anchor_widget_observer_ = std::make_unique<AnchorWidgetObserver>(this, anchor_view->GetWidget());
    } else {
    ...
    if (anchor_view) {
        anchor_view_observer_ = std::make_unique<AnchorViewObserver>(this, anchor_view);
...
void BubbleDialogDelegate::UpdateColorsFromTheme()
{
    ...
    if (!color_explicitly_set()) {
        set_color_internal(contents_view->GetColorProvider()->GetColor(ui::kColorBubbleBackground));
    }
    BubbleFrameView* frame_view = GetBubbleFrameView();
    if (frame_view)
    ...
    const bool contents_layer_opaque = contents_view->layer() && contents_view->layer()->fills_bounds_opaquely();
    contents_view->SetBackground(contents_layer_opaque || force_create_contents_background_ ? CreateSolidBackground(color()) : nullptr);
}
...
void BubbleDialogDelegate::OnBubbleWidgetVisibilityChanged(bool visible)
{
    ...
    if (visible) {
        ...
        if (bubble_created_time_.has_value()) {
            GetWidget()->GetCompositor()->RequestSuccessfulPresentationTimeForNextFrame(base::BindOnce(
                [](base::WeakPtr<BubbleDialogDelegate::BubbleUmaLogger> uma_logger, base::TimeTicks bubble_created_time,
                    const viz::FrameTimingDetails& frame_timing_details) {
                    base::TimeTicks presentation_timestamp = frame_timing_details.presentation_feedback.timestamp;
                    if (!uma_logger) {
                    ...
                    uma_logger->LogMetric(base::UmaHistogramMediumTimes, "CreateToVisibleTime", presentation_timestamp - bubble_created_time);
                },
    ...
    if (visible && ui::IsAlert(GetAccessibleWindowRole())) {
        GetWidget()->GetRootView()->GetViewAccessibility().SetRole(GetAccessibleWindowRole());
        GetWidget()->GetRootView()->NotifyAccessibilityEvent(ax::mojom::Event::kAlert, true);
    }
}
...
void BubbleDialogDelegate::SetAnchoredDialogKey()
{
    ...
    if (focus_traversable_from_anchor_view_) {
        ...
        anchor_view->SetProperty(kAnchoredDialogKey, static_cast<DialogDelegate*>(this));
    }
}
