...
// |window_bounds|.
int GetOverflowLength(const gfx::Rect& available_bounds, const gfx::Rect& window_bounds, bool vertical)
{
...
BubbleFrameView::BubbleFrameView(const gfx::Insets& title_margins, const gfx::Insets& content_margins)
    ...
    , default_title_(title_container_->AddChildView(CreateDefaultTitleLabel(std::u16string())))
    , subtitle_(title_container_->AddChildView(CreateLabelWithContextAndStyle(std::u16string(), style::CONTEXT_LABEL, style::STYLE_SECONDARY)))
{
    ...
    auto close = CreateCloseButton(base::BindRepeating(
        [](BubbleFrameView* view, const ui::Event& event) {
            if (view->input_protector_.IsPossiblyUnintendedInteraction(event))
                return;
            view->GetWidget()->CloseWithReason(Widget::ClosedReason::kCloseButtonClicked);
        },
        this));
    ...
    progress_indicator->SetProperty(views::kElementIdentifierKey, kProgressIndicatorElementId);
...
// static
std::unique_ptr<Label> BubbleFrameView::CreateDefaultTitleLabel(const std::u16string& title_text)
{
    std::unique_ptr<Label> label = CreateLabelWithContextAndStyle(title_text, style::CONTEXT_DIALOG_TITLE, style::STYLE_PRIMARY);
...
// static
std::unique_ptr<Button> BubbleFrameView::CreateCloseButton(Button::PressedCallback callback)
{
    auto close_button = CreateVectorImageButtonWithNativeTheme(std::move(callback), vector_icons::kCloseChromeRefreshIcon);
    close_button->SetTooltipText(l10n_util::GetStringUTF16(IDS_APP_CLOSE));
    close_button->GetViewAccessibility().SetName(l10n_util::GetStringUTF16(IDS_APP_CLOSE));
...
// static
std::unique_ptr<Button> BubbleFrameView::CreateMinimizeButton(Button::PressedCallback callback)
{
    auto minimize_button = CreateVectorImageButtonWithNativeTheme(std::move(callback), kWindowControlMinimizeIcon);
    minimize_button->SetTooltipText(l10n_util::GetStringUTF16(IDS_APP_ACCNAME_MINIMIZE));
    minimize_button->GetViewAccessibility().SetName(l10n_util::GetStringUTF16(IDS_APP_ACCNAME_MINIMIZE));
...
gfx::Rect BubbleFrameView::GetWindowBoundsForClientBounds(const gfx::Rect& client_bounds) const
{
...
bool BubbleFrameView::GetClientMask(const gfx::Size& size, SkPath* path) const
{
    ...
    SkScalar radii[] { corner_radii.upper_left(), corner_radii.upper_left(), corner_radii.upper_right(), corner_radii.upper_right(), corner_radii.lower_right(),
        corner_radii.lower_right(), corner_radii.lower_left(), corner_radii.lower_left() };
...
int BubbleFrameView::NonClientHitTest(const gfx::Point& point)
{
    ...
    gfx::RRectF round_contents_bounds(gfx::RectF(GetContentsBounds()), bubble_border_->corner_radius());
...
void BubbleFrameView::GetWindowMask(const gfx::Size& size, SkPath* window_mask)
{
    if (bubble_border_->shadow() != BubbleBorder::STANDARD_SHADOW && bubble_border_->shadow() != BubbleBorder::NO_SHADOW)
        return;

    // We don't return a mask for windows with arrows unless they use
    // BubbleBorder::NO_SHADOW.
    if (bubble_border_->shadow() != BubbleBorder::NO_SHADOW && bubble_border_->arrow() != BubbleBorder::NONE && bubble_border_->arrow() != BubbleBorder::FLOAT)
        return;

    // Use a window mask roughly matching the border in the image assets.
    const int kBorderStrokeSize = bubble_border_->shadow() == BubbleBorder::NO_SHADOW ? 0 : 1;
    ...
    SkRect rect = { SkIntToScalar(border_insets.left() - kBorderStrokeSize), SkIntToScalar(border_insets.top() - kBorderStrokeSize),
        SkIntToScalar(size.width() - border_insets.right() + kBorderStrokeSize), SkIntToScalar(size.height() - border_insets.bottom() + kBorderStrokeSize) };
...
void BubbleFrameView::ResetWindowControls()
{
    ...
    bool close_is_visible = GetWidget()->widget_delegate()->ShouldShowCloseButton();
    ...
    bool minimize_visible_changed = minimize_->GetVisible() != minimize_is_visible;
...
void BubbleFrameView::UpdateWindowTitle()
{
    if (default_title_) {
        ...
        default_title_->SetVisible(delegate->ShouldShowWindowTitle() && !delegate->GetWindowTitle().empty());
...
void BubbleFrameView::InsertClientView(ClientView* client_view)
{
    ...
    footnote_container_ ? AddChildViewAt(client_view, GetIndexOf(footnote_container_).value()) : AddChildView(client_view);
}
...
void BubbleFrameView::UpdateSubtitle()
{
    ...
    views::BubbleDialogDelegate* const bubble_delegate = GetWidget()->widget_delegate()->AsBubbleDialogDelegate();
    if (!bubble_delegate) {
        return;
    }
    // Subtitle anchors and margins rely heavily on Title being visible.
    subtitle_->SetVisible(!bubble_delegate->GetSubtitle().empty() && default_title_->GetVisible());
    subtitle_->SetText(bubble_delegate->GetSubtitle());
    subtitle_->SetAllowCharacterBreak(bubble_delegate->GetSubtitleAllowCharacterBreak());
    InvalidateLayout();
}
...
void BubbleFrameView::UpdateMainImage()
{
    ...
    if (model.IsEmpty()) {
        ...
        const int main_image_dimension = kMainImageDialogWidthIncrease - border_insets.left() - border_margin_left - kBorderStrokeThickness;
        ...
        const int border_radius = LayoutProvider::Get()->GetCornerRadiusMetric(Emphasis::kHigh, gfx::Size());
        main_image_->SetImage(ui::ImageModel::FromImageSkia(gfx::ImageSkiaOperations::CreateCroppedCenteredRoundRectImage(
            gfx::Size(main_image_dimension, main_image_dimension), border_radius - 2 * kBorderStrokeThickness, model.GetImage().AsImageSkia())));
        main_image_->SetBorder(views::CreateRoundedRectBorder(kBorderStrokeThickness, border_radius, image_insets,
            GetColorProvider() ? GetColorProvider()->GetColor(ui::kColorBubbleBorder) : gfx::kPlaceholderColor));
...
gfx::Size BubbleFrameView::CalculatePreferredSize(const SizeBounds& available_size) const
{
    // Get the preferred size of the client area.
    gfx::Size client_size = GetWidget()->client_view()->GetPreferredSize(available_size);
...
void BubbleFrameView::Layout(PassKey)
{
    ...
    // there's no title.
    DCHECK(!title_margins_.IsEmpty() || (!custom_title_ && !default_title_->GetVisible()));

    const gfx::Rect contents_bounds = GetContentsBounds();

    // Lay out the progress bar.
    progress_indicator_->SetBounds(contents_bounds.x(), contents_bounds.y(), contents_bounds.width(), kProgressIndicatorHeight);
    ...
    for (Button* button : { close_, minimize_ }) {
        ...
        if (button == minimize_ && close_->GetVisible()) {
            button_area_rect.Outset(gfx::Outsets::TLBR(0, LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_BUTTON_HORIZONTAL), 0, 0));
        }
        button->SetPosition(gfx::Point(button_area_rect.x() - button->width(), button_area_rect.y()));
        button_area_rect.Union(button->bounds());
    }

    // Add spacing between the title and buttons.
    if (!button_area_rect.IsEmpty()) {
        button_area_rect.Outset(gfx::Outsets::TLBR(0, LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_LABEL_HORIZONTAL), 0, 0));
    }
    ...
    const int title_icon_padding = title_icon_pref_size.width() > 0 ? title_margins_.left() : 0;
    const int title_label_x = bounds.x() + title_icon_pref_size.width() + title_icon_padding + GetMainImageLeftInsets();
    int title_label_right = bounds.right();
    if (!button_area_rect.IsEmpty() && button_area_rect.bottom() > header_rect.bottom()) {
    ...
    if (DCHECK_IS_ON() && HasTitle() && !bounds.IsEmpty()) {
        const gfx::Insets title_insets = GetTitleLabelInsetsFromFrame() + GetInsets();
        DCHECK_EQ(title_insets.left(), title_label_x);
        DCHECK_EQ(title_insets.right(), width() - title_label_right);
    }

    const int title_available_width = std::max(1, title_label_right - title_label_x);
    const int title_preferred_height = title_container_->GetHeightForWidth(title_available_width);
    const int title_height = std::max(title_icon_pref_size.height(), title_preferred_height);

    title_container_->SetBounds(title_label_x, bounds.y() + (title_height - title_preferred_height) / 2, title_available_width, title_preferred_height);
    ...
    if (default_title_ && title_icon_pref_size.height() <= default_title_->GetLineHeight()) {
        ...
        // first line.
        title_icon_->SetBounds(bounds.x(), bounds.y() + (default_title_->GetLineHeight() - title_icon_pref_size.height()) / 2, title_icon_pref_size.width(),
            title_icon_pref_size.height());
    } else {
        title_icon_->SetBounds(bounds.x(), bounds.y(), title_icon_pref_size.width(), title_height);
    }

    main_image_->SetBounds(0, 0, main_image_->GetPreferredSize({}).width(), main_image_->GetPreferredSize({}).height());
    ...
    if (footnote_container_ && footnote_container_->GetVisible()) {
        ...
        footnote_container_->SetBounds(contents_bounds.x(), contents_bounds.bottom() - height, width, height);
    }
...
void BubbleFrameView::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details)
{
    ...
    if (!details.is_add && details.parent == footnote_container_ && footnote_container_->children().size() == 1
...
void BubbleFrameView::PaintChildren(const PaintInfo& paint_info)
{
    ...
    ui::PaintRecorder recorder(
        paint_info.context(), paint_info.paint_recording_size(), paint_info.paint_recording_scale_x(), paint_info.paint_recording_scale_y(), &paint_cache);
...
void BubbleFrameView::SetFootnoteView(std::unique_ptr<View> view)
{
    ...
    if (view) {
        ...
        footnote_container_ = AddChildView(std::make_unique<FootnoteContainerView>(footnote_margins_, std::move(view), radius));
    }
    InvalidateLayout();
}
...
void BubbleFrameView::SetPreferredArrowAdjustment(BubbleFrameView::PreferredArrowAdjustment adjustment)
{
...
BubbleFrameView::PreferredArrowAdjustment BubbleFrameView::GetPreferredArrowAdjustment() const
{
...
gfx::Rect BubbleFrameView::GetUpdatedWindowBounds(
    const gfx::Rect& anchor_rect, const BubbleBorder::Arrow delegate_arrow, const gfx::Size& client_size, bool adjust_to_fit_available_bounds)
{
    ...
    if (adjust_to_fit_available_bounds && BubbleBorder::has_arrow(delegate_arrow)) {
        ...
        // bounds.
        if (bubble_border_->is_arrow_at_center(delegate_arrow) || preferred_arrow_adjustment_ == PreferredArrowAdjustment::kOffset) {
            const bool mirror_vertical = BubbleBorder::is_arrow_on_horizontal(delegate_arrow);
            if (use_anchor_window_bounds_) {
                MirrorArrowIfOutOfBounds(mirror_vertical, anchor_rect, size, GetAvailableAnchorWindowBounds());
            }
            MirrorArrowIfOutOfBounds(mirror_vertical, anchor_rect, size, GetAvailableScreenBounds(anchor_rect));
            if (use_anchor_window_bounds_) {
                OffsetArrowIfOutOfBounds(anchor_rect, size, GetAvailableAnchorWindowBounds());
            }
            OffsetArrowIfOutOfBounds(anchor_rect, size, GetAvailableScreenBounds(anchor_rect));
        } else {
            if (use_anchor_window_bounds_) {
                MirrorArrowIfOutOfBounds(true, anchor_rect, size, GetAvailableAnchorWindowBounds());
            }
            MirrorArrowIfOutOfBounds(true, anchor_rect, size, GetAvailableScreenBounds(anchor_rect));
            if (use_anchor_window_bounds_) {
                MirrorArrowIfOutOfBounds(false, anchor_rect, size, GetAvailableAnchorWindowBounds());
            }
            MirrorArrowIfOutOfBounds(false, anchor_rect, size, GetAvailableScreenBounds(anchor_rect));
        }
    }

    // Check to see if any of the positioning values have changed.
    if (bubble_border_->arrow() != old_arrow || bubble_border_->arrow_offset() != old_offset) {
        InvalidateLayout();
        SchedulePaint();
    }
...
gfx::Rect BubbleFrameView::GetAvailableScreenBounds(const gfx::Rect& rect) const
{
    // The bubble attempts to fit within the current screen bounds.
    return display::Screen::GetScreen()->GetDisplayNearestPoint(rect.CenterPoint()).work_area();
}

gfx::Rect BubbleFrameView::GetAvailableAnchorWindowBounds() const
{
    views::BubbleDialogDelegate* bubble_delegate_view = GetWidget()->widget_delegate()->AsBubbleDialogDelegate();
...
gfx::RoundedCornersF BubbleFrameView::GetClientCornerRadii() const
{
    ...
    const gfx::Insets insets = GetClientInsetsForFrameWidth(GetContentsBounds().width());
    ...
    if ((insets.top() > radius && insets.bottom() > radius) || (insets.left() > radius && insets.right() > radius)) {
...
void BubbleFrameView::MirrorArrowIfOutOfBounds(bool vertical, const gfx::Rect& anchor_rect, const gfx::Size& client_size, const gfx::Rect& available_bounds)
{
    ...
    if (GetOverflowLength(available_bounds, window_bounds, vertical) > 0) {
        ...
        bubble_border_->set_arrow(vertical ? BubbleBorder::vertical_mirror(arrow) : BubbleBorder::horizontal_mirror(arrow));
        ...
        // new bubble border.
        if (GetOverflowLength(available_bounds, mirror_bounds, vertical) >= GetOverflowLength(available_bounds, window_bounds, vertical)) {
            bubble_border_->set_arrow(arrow);
        }
    }
}

void BubbleFrameView::OffsetArrowIfOutOfBounds(const gfx::Rect& anchor_rect, const gfx::Size& client_size, const gfx::Rect& available_bounds)
{
    BubbleBorder::Arrow arrow = bubble_border_->arrow();
    DCHECK(BubbleBorder::is_arrow_at_center(arrow) || preferred_arrow_adjustment_ == PreferredArrowAdjustment::kOffset);
    ...
    bubble_border_->set_arrow_offset(bubble_border_->arrow_offset() - offscreen_adjust);
}
...
int BubbleFrameView::GetFrameWidthForClientWidth(int client_width) const
{
    ...
    const int title_bar_width = GetTitleLabelInsetsFromFrame().width() + (HasWindowTitle() ? title()->GetMinimumSize().width() : 0);
    const int client_area_width = client_width + content_margins_.width();
    const int frame_width = std::max(title_bar_width, client_area_width) + GetMainImageLeftInsets();

    DialogDelegate* const dialog_delegate = GetWidget()->widget_delegate()->AsDialogDelegate();
    bool snapping = dialog_delegate && dialog_delegate->buttons() != static_cast<int>(ui::mojom::DialogButton::kNone);
    return snapping ? LayoutProvider::Get()->GetSnappedDialogWidth(frame_width) : frame_width;
}

gfx::Size BubbleFrameView::GetFrameSizeForClientSize(const gfx::Size& client_size) const
{
    ...
    if (main_image_->GetVisible()) {
        size.set_height(std::max(size.height(), main_image_->GetPreferredSize({}).height()));
    }
...
{
    return (custom_title_ != nullptr && GetWidget()->widget_delegate()->ShouldShowWindowTitle())
        || (default_title_ != nullptr && default_title_->GetHeightForWidth(default_title_->width()) > 0) || title_icon_->GetPreferredSize({}).height() > 0;
}

BubbleFrameView::ButtonsPositioning BubbleFrameView::GetButtonsPositioning() const
{
    // Positions the buttons in the title row when there's no header row.
    return HasTitle() && !(header_view_ && header_view_->GetVisible()) ? ButtonsPositioning::kInTitleRow : ButtonsPositioning::kOnFrameEdge;
}

bool BubbleFrameView::TitleRowHasButtons() const
{
    return GetButtonsPositioning() == ButtonsPositioning::kInTitleRow
        && (GetWidget()->widget_delegate()->ShouldShowCloseButton() || GetWidget()->widget_delegate()->CanMinimize());
}

gfx::Insets BubbleFrameView::GetTitleLabelInsetsFromFrame() const
{
    const gfx::Rect content_bounds = GetContentsBounds();
    const int header_height = GetHeaderHeightForFrameWidth(content_bounds.width());
    const gfx::Size button_area_size = GetButtonAreaSize();
    const gfx::Rect button_area_bounds(GetButtonAreaTopRight() - gfx::Vector2d(button_area_size.width(), 0), button_area_size);
    // Only reserve space if the button vertically extends over the header.
    int insets_right = button_area_bounds.bottom() > header_height ? content_bounds.right() - button_area_bounds.x() : 0;
    ...
    const int title_icon_padding = title_icon_pref_size.width() > 0 ? title_margins_.left() : 0;
    const int insets_left = GetMainImageLeftInsets() + title_margins_.left() + title_icon_pref_size.width() + title_icon_padding;
    return gfx::Insets::TLBR(header_height + title_margins_.top(), insets_left, title_margins_.bottom(), insets_right);
}

gfx::Insets BubbleFrameView::GetClientInsetsForFrameWidth(int frame_width) const
{
    int header_height = GetHeaderHeightForFrameWidth(frame_width);
    int close_height = 0;
    if (!ExtendClientIntoTitle() && GetButtonsPositioning() == ButtonsPositioning::kOnFrameEdge && GetWidget()->widget_delegate()->ShouldShowCloseButton()) {
        const int close_margin = LayoutProvider::Get()->GetDistanceMetric(DISTANCE_CLOSE_BUTTON_MARGIN);
        // Note: |close_margin| is not applied on the bottom of the icon.
        close_height = close_margin + close_->height();
    }

    if (!HasTitle()) {
        return content_margins_ + gfx::Insets::TLBR(std::max(header_height, close_height), GetMainImageLeftInsets(), 0, 0);
    }

    const int icon_height = title_icon_->GetPreferredSize({}).height();
    const int label_height = title_container_->GetHeightForWidth(frame_width - GetTitleLabelInsetsFromFrame().width());
    const int title_height = std::max(icon_height, label_height) + title_margins_.height();

    return content_margins_ + gfx::Insets::TLBR(std::max(title_height + header_height, close_height), GetMainImageLeftInsets(), 0, 0);
}

int BubbleFrameView::GetHeaderHeightForFrameWidth(int frame_width) const
{
    return header_view_ && header_view_->GetVisible() ? header_view_->GetHeightForWidth(frame_width) : 0;
}
...
void BubbleFrameView::UpdateClientLayerCornerRadius()
{
    ...
    if (GetWidget() && GetWidget()->client_view()->layer()) {
        GetWidget()->client_view()->layer()->SetRoundedCornerRadius(GetClientCornerRadii());
    }
}
...
int BubbleFrameView::GetMainImageLeftInsets() const
{
    ...
    return main_image_->GetPreferredSize({}).width() - main_image_->GetBorder()->GetInsets().right();
}
...
gfx::Point BubbleFrameView::GetButtonAreaTopRight() const
{
    ...
    if (GetButtonsPositioning() == ButtonsPositioning::kOnFrameEdge) {
        const int distance_to_edge = LayoutProvider::Get()->GetDistanceMetric(DISTANCE_CLOSE_BUTTON_MARGIN);
        return gfx::Point(contents_bounds.right() - distance_to_edge, contents_bounds.y() + distance_to_edge);
    }
    ...
    return inner_bounds.top_right() + gfx::Vector2d(close_->GetInsets().right(), 0);
}
...
gfx::Size BubbleFrameView::GetButtonAreaSize() const
{
    ...
    // Add left padding.
    total_width += LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_LABEL_HORIZONTAL);
    // Add spacing between buttons.
    if (button_count == 2) {
        total_width += LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_BUTTON_HORIZONTAL);
    }
...
std::unique_ptr<Label> BubbleFrameView::CreateLabelWithContextAndStyle(
    const std::u16string& label_text, style::TextContext text_context, style::TextStyle text_style)
{
...
ADD_PROPERTY_METADATA(BubbleFrameView::PreferredArrowAdjustment, PreferredArrowAdjustment)
...
DEFINE_CLASS_ELEMENT_IDENTIFIER_VALUE(BubbleFrameView, kMinimizeButtonElementId);
DEFINE_CLASS_ELEMENT_IDENTIFIER_VALUE(BubbleFrameView, kCloseButtonElementId);
DEFINE_CLASS_ELEMENT_IDENTIFIER_VALUE(BubbleFrameView, kProgressIndicatorElementId);
