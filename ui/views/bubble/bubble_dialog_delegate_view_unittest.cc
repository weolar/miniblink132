...
public:
    explicit TestBubbleDialogDelegateView(View* anchor_view)
        : BubbleDialogDelegateView(anchor_view, BubbleBorder::TOP_LEFT, BubbleBorder::NO_SHADOW, true)
    {
    ...
    TestBubbleDialogDelegateView& operator=(const TestBubbleDialogDelegateView&) = delete;
    ...
    template <typename T> T* set_title_view(std::unique_ptr<T> title_view)
    {
...
class TestAlertBubbleDialogDelegateView : public TestBubbleDialogDelegateView {
    METADATA_HEADER(TestAlertBubbleDialogDelegateView, TestBubbleDialogDelegateView)
...
public:
    ...
    BubbleDialogDelegateViewTest& operator=(const BubbleDialogDelegateViewTest&) = delete;

    ~BubbleDialogDelegateViewTest() override = default;

    std::unique_ptr<views::Widget> CreateTestWidget(views::Widget::InitParams::Ownership ownership, views::Widget::InitParams::Type type) override
    {
...
{
    std::unique_ptr<Widget> anchor_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    TestBubbleDialogDelegateView* bubble_delegate = new TestBubbleDialogDelegateView(anchor_widget->GetContentsView());
    bubble_delegate->set_color(SK_ColorGREEN);
    Widget* bubble_widget = BubbleDialogDelegateView::CreateBubble(bubble_delegate);
...
{
    std::unique_ptr<Widget> anchor_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    BubbleDialogDelegateView* bubble_delegate = new TestBubbleDialogDelegateView(anchor_widget->GetContentsView());
    ...
    std::unique_ptr<Widget> smoke_and_mirrors_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    EXPECT_FALSE(bubble_observer.widget_closed());
...
TEST_F(BubbleDialogDelegateViewTest, CloseAnchorViewTest)
{
    ...
    std::unique_ptr<Widget> anchor_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    View* anchor_view = anchor_widget->SetContentsView(std::make_unique<View>());
    TestBubbleDialogDelegateView* bubble_delegate = new TestBubbleDialogDelegateView(anchor_view);
...
TEST_F(BubbleDialogDelegateViewTest, ResetAnchorWidget)
{
    ...
    std::unique_ptr<Widget> parent_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
...
{
    std::unique_ptr<Widget> anchor_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    LabelButton* button = anchor_widget->SetContentsView(std::make_unique<LabelButton>(Button::PressedCallback(), std::u16string()));
    TestInkDrop* ink_drop = new TestInkDrop();
    test::InkDropHostTestApi(InkDrop::Get(button)).SetInkDrop(base::WrapUnique(ink_drop));
    TestBubbleDialogDelegateView* bubble_delegate_first = new TestBubbleDialogDelegateView(button);
    bubble_delegate_first->set_parent_window(anchor_widget->GetNativeView());
    bubble_delegate_first->set_close_on_deactivate(false);

    Widget* bubble_widget_first = BubbleDialogDelegateView::CreateBubble(bubble_delegate_first);
    ...
    TestBubbleDialogDelegateView* bubble_delegate_second = new TestBubbleDialogDelegateView(button);
    bubble_delegate_second->set_parent_window(anchor_widget->GetNativeView());
    Widget* bubble_widget_second = BubbleDialogDelegateView::CreateBubble(bubble_delegate_second);
...
TEST_F(BubbleDialogDelegateViewTest, MultipleBubbleAnchorHighlightTestOutOfOrder)
{
    std::unique_ptr<Widget> anchor_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    LabelButton* button = anchor_widget->SetContentsView(std::make_unique<LabelButton>(Button::PressedCallback(), std::u16string()));
    TestInkDrop* ink_drop = new TestInkDrop();
...
#endif
    BubbleDialogDelegateView* bubble_delegate = new TestBubbleDialogDelegateView(nullptr);
    bubble_delegate->set_has_parent(false);
    WidgetAutoclosePtr bubble_widget(BubbleDialogDelegateView::CreateBubble(bubble_delegate));
    EXPECT_EQ(bubble_delegate, bubble_widget->widget_delegate());
...
TEST_F(BubbleDialogDelegateViewTest, InitiallyFocusedView)
{
    ...
    Widget* bubble_widget = BubbleDialogDelegateView::CreateBubble(bubble_delegate);
    bubble_widget->Show();
    EXPECT_EQ(bubble_delegate->GetInitiallyFocusedView(), bubble_widget->GetFocusManager()->GetFocusedView());
    bubble_widget->CloseNow();
}
...
TEST_F(BubbleDialogDelegateViewTest, NonClientHitTest)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_EQ(test_case.hit, frame->NonClientHitTest(point)) << " at point " << test_case.point;
    }
}
...
TEST_F(BubbleDialogDelegateViewTest, CloseMethods)
{
...

    {
        ...
        ui::KeyEvent escape_event(ui::EventType::kKeyPressed, ui::VKEY_ESCAPE, ui::EF_NONE);
...

    {
        ...
        test::ButtonTestApi(close_button)
            .NotifyClick(ui::MouseEvent(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE));
        EXPECT_TRUE(bubble_widget->IsClosed());
    }
}
...
TEST_F(BubbleDialogDelegateViewTest, CustomTitle)
{
    ...
    View* title_view = bubble_delegate->set_title_view(std::make_unique<StaticSizedView>(gfx::Size(10, kTitleHeight)));
    Widget* bubble_widget = BubbleDialogDelegateView::CreateBubble(bubble_delegate);
    bubble_widget->Show();

    BubbleFrameView* bubble_frame = static_cast<BubbleFrameView*>(bubble_widget->non_client_view()->frame_view());
    ...
    const gfx::Insets content_margins = provider->GetDialogInsetsForContentType(views::DialogContentType::kText, views::DialogContentType::kText);
    const gfx::Insets title_margins = provider->GetInsetsMetric(INSETS_DIALOG_TITLE);
    ...
    EXPECT_EQ(content_margins.height() + kContentSize.height() + title_margins.height() + kTitleHeight, frame_size.height());
    ...
    EXPECT_EQ(content_margins.height() + kContentSize.height() + title_margins.height(), frame_size.height());
    EXPECT_EQ(content_margins.width() + kContentSize.width(), frame_size.width());
    ...
    frame_size = bubble_frame->GetContentsBounds();
    EXPECT_EQ(content_margins.height() + kContentSize.height(), frame_size.height());
    EXPECT_EQ(content_margins.width() + kContentSize.width(), frame_size.width());
}
...
TEST_F(BubbleDialogDelegateViewTest, StyledLabelTitle)
{
    std::unique_ptr<Widget> anchor_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    TestBubbleDialogDelegateView* bubble_delegate = new TestBubbleDialogDelegateView(anchor_widget->GetContentsView());
    StyledLabel* title_view = bubble_delegate->set_title_view(std::make_unique<StyledLabel>());
    title_view->SetText(u"123");

    Widget* bubble_widget = BubbleDialogDelegateView::CreateBubble(bubble_delegate);
    bubble_widget->Show();

    const gfx::Size size_before_new_title = bubble_widget->GetWindowBoundsInScreen().size();
    title_view->SetText(u"12");

    // A shorter title should change nothing, since both will be within the
    // minimum dialog width.
    EXPECT_EQ(size_before_new_title, bubble_widget->GetWindowBoundsInScreen().size());

    title_view->SetText(base::UTF8ToUTF16(std::string(200, '0')));

    // A (much) longer title should increase the height, but not the width.
    EXPECT_EQ(size_before_new_title.width(), bubble_widget->GetWindowBoundsInScreen().width());
    EXPECT_LT(size_before_new_title.height(), bubble_widget->GetWindowBoundsInScreen().height());
}
...
TEST_F(BubbleDialogDelegateViewTest, AttachedWidgetShowsInkDropWhenVisible)
{
    ...
    TestInkDrop* ink_drop = new TestInkDrop();
    test::InkDropHostTestApi(InkDrop::Get(button)).SetInkDrop(base::WrapUnique(ink_drop));
    TestBubbleDialogDelegateView* bubble_delegate = new TestBubbleDialogDelegateView(nullptr);
...
TEST_F(BubbleDialogDelegateViewTest, GetThemeProvider_FromAnchorWidget)
{
    ...
    Widget* bubble_widget = BubbleDialogDelegateView::CreateBubble(bubble_delegate);
    bubble_widget->Show();
    EXPECT_NE(bubble_widget->GetThemeProvider(), anchor_widget->GetThemeProvider());

    bubble_delegate->SetAnchorView(anchor_widget->GetRootView());
    EXPECT_EQ(bubble_widget->GetThemeProvider(), anchor_widget->GetThemeProvider());
}
...
struct ArrowTestParameters {
    ...
    gfx::Size ExpectedSpace() const
    {
        ...
        adjusted_anchor_rect.Offset(0, ViewsTestBase::GetSystemReservedHeightAtTopOfScreen());
        gfx::Rect screen_rect = gfx::Rect(0, 0, kScreenWidth, kScreenHeight);

        return BubbleDialogDelegate::GetAvailableSpaceToPlaceBubble(expected_arrow, adjusted_anchor_rect, screen_rect);
    }
};

class BubbleDialogDelegateViewArrowTest : public BubbleDialogDelegateViewTest, public testing::WithParamInterface<ArrowTestParameters> {
public:
    ...
    BubbleDialogDelegateViewArrowTest(const BubbleDialogDelegateViewArrowTest&) = delete;
    BubbleDialogDelegateViewArrowTest& operator=(const BubbleDialogDelegateViewArrowTest&) = delete;
...
private:
    void SetUpTestScreen()
    {
        ...
        test_screen_->display_list().AddDisplay(display, display::DisplayList::Type::PRIMARY);
    }
...
{
#if BUILDFLAG(IS_OZONE)
    if (!ui::OzonePlatform::GetInstance()->GetPlatformProperties().supports_global_screen_coordinates) {
        GTEST_SKIP() << "Global screen coordinates unavailable";
    }
#endif
    std::unique_ptr<Widget> anchor_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    auto* bubble_delegate = new TestBubbleDialogDelegateView(anchor_widget->GetContentsView());
    BubbleDialogDelegateView::CreateBubble(bubble_delegate);
    EXPECT_EQ(bubble_delegate->adjust_if_offscreen(), PlatformStyle::kAdjustBubbleIfOffscreen);
    ...
    anchor_widget->GetContentsView()->SetBounds(kParam.anchor_rect.x(), kParam.anchor_rect.y(), kParam.anchor_rect.width(), kParam.anchor_rect.height());
    gfx::Size available_space = BubbleDialogDelegate::GetMaxAvailableScreenSpaceToPlaceBubble(
        bubble_delegate->GetAnchorView(), bubble_delegate->arrow(), bubble_delegate->adjust_if_offscreen(), BubbleFrameView::PreferredArrowAdjustment::kMirror);
...
const ArrowTestParameters kAnchorAtFarScreenCornersParams[] = {
    ...
    { views::BubbleBorder::Arrow::TOP_RIGHT, true, gfx::Rect(kAnchorFarRightX, kAnchorFarTopY, kAnchorLength, kAnchorLength),
        views::BubbleBorder::Arrow::TOP_RIGHT },
    // Max size available when arrow is changed to Bottom Right
    { views::BubbleBorder::Arrow::TOP_RIGHT, true, gfx::Rect(kAnchorFarRightX, kAnchorFarBottomY, kAnchorLength, kAnchorLength),
        views::BubbleBorder::Arrow::BOTTOM_RIGHT },
    // Max size available when arrow is changed to Bottom Left
    { views::BubbleBorder::Arrow::TOP_RIGHT, true, gfx::Rect(kAnchorFarLeftX, kAnchorFarBottomY, kAnchorLength, kAnchorLength),
        views::BubbleBorder::Arrow::BOTTOM_LEFT },
    // Max size available when arrow is changed to Top Left
    { views::BubbleBorder::Arrow::TOP_RIGHT, true, gfx::Rect(kAnchorFarLeftX, kAnchorFarTopY, kAnchorLength, kAnchorLength),
        views::BubbleBorder::Arrow::TOP_LEFT },
    // Offscreen adjustment is off, available size is per Bottom Left
    // arrow
    { views::BubbleBorder::Arrow::BOTTOM_LEFT, false, gfx::Rect(kAnchorFarRightX, kAnchorFarTopY, kAnchorLength, kAnchorLength),
        views::BubbleBorder::Arrow::BOTTOM_LEFT },
    // Offscreen adjustment is off, available size is per Top Left arrow
    { views::BubbleBorder::Arrow::TOP_LEFT, false, gfx::Rect(kAnchorFarRightX, kAnchorFarBottomY, kAnchorLength, kAnchorLength),
        views::BubbleBorder::Arrow::TOP_LEFT },
    // Offscreen adjustment is off, available size is per Top Right arrow
    { views::BubbleBorder::Arrow::TOP_RIGHT, false, gfx::Rect(kAnchorFarLeftX, kAnchorFarBottomY, kAnchorLength, kAnchorLength),
        views::BubbleBorder::Arrow::TOP_RIGHT },
    // Offscreen adjustment is off, available size is per Bottom Right
    // arrow
    { views::BubbleBorder::Arrow::BOTTOM_RIGHT, false, gfx::Rect(kAnchorFarLeftX, kAnchorFarTopY, kAnchorLength, kAnchorLength),
        views::BubbleBorder::Arrow::BOTTOM_RIGHT }
};

INSTANTIATE_TEST_SUITE_P(AnchorAtFarScreenCorners, BubbleDialogDelegateViewArrowTest, testing::ValuesIn(kAnchorAtFarScreenCornersParams));
...
{
    std::unique_ptr<Widget> anchor_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    auto bubble_delegate = std::make_unique<BubbleDialogDelegateView>(nullptr, BubbleBorder::TOP_LEFT);
    bubble_delegate->SetPaintClientToLayer(true);
    bubble_delegate->set_parent_window(anchor_widget->GetNativeView());

    WidgetAutoclosePtr bubble_widget(BubbleDialogDelegateView::CreateBubble(std::move(bubble_delegate)));
...
TEST_F(BubbleDialogDelegateViewTest, AlertAccessibleEvent)
{
    ...
    std::unique_ptr<Widget> anchor_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    auto bubble_delegate = std::make_unique<TestBubbleDialogDelegateView>(anchor_widget->GetContentsView());

    EXPECT_EQ(0, counter.GetCount(ax::mojom::Event::kAlert));
    Widget* bubble_widget = BubbleDialogDelegateView::CreateBubble(std::move(bubble_delegate));
    bubble_widget->Show();
    // Bubbles with kDialog accessible role don't produce this event
    EXPECT_EQ(0, counter.GetCount(ax::mojom::Event::kAlert));

    auto alert_bubble_delegate = std::make_unique<TestAlertBubbleDialogDelegateView>(anchor_widget->GetContentsView());
    Widget* alert_bubble_widget = BubbleDialogDelegateView::CreateBubble(std::move(alert_bubble_delegate));
...
public:
    ...
    AnchorTestBubbleDialogDelegateView(const AnchorTestBubbleDialogDelegateView&) = delete;
    AnchorTestBubbleDialogDelegateView& operator=(const AnchorTestBubbleDialogDelegateView&) = delete;
...
public:
    ...
    BubbleDialogDelegateViewAnchorTest(const BubbleDialogDelegateViewAnchorTest&) = delete;
    BubbleDialogDelegateViewAnchorTest& operator=(const BubbleDialogDelegateViewAnchorTest&) = delete;
    ...
    {
        Widget::ReparentNativeView(bubble_widget->GetNativeView(), anchor_to->GetNativeView());
        static_cast<AnchorTestBubbleDialogDelegateView*>(bubble_widget->widget_delegate())->SetAnchorView(GetAnchorView(anchor_to));
    }
...
protected:
    ...
    Widget* dummy_widget() const
    {
        return dummy_widget_.get();
    }
...
TEST_F(BubbleDialogDelegateViewAnchorTest, AnchoredToWidgetShouldPaintAsActive)
{
...
TEST_F(BubbleDialogDelegateViewAnchorTest, AnchoredToWidgetBecomesActiveWhenBubbleIsShown)
{
    auto widget = CreateTopLevelWidget();
    widget->ShowInactive();
    EXPECT_FALSE(widget->ShouldPaintAsActive());
...
TEST_F(BubbleDialogDelegateViewAnchorTest, ActiveStatePersistsAcrossAnchorWidgetAndBubbleActivation)
{
    auto widget = CreateTopLevelWidget();
    widget->ShowInactive();
    auto* bubble = CreateBubble(widget.get());
...
TEST_F(BubbleDialogDelegateViewAnchorTest, AnchoringAlreadyActiveBubbleChangesAnchorWidgetState)
{
    auto widget = CreateTopLevelWidget();
    auto* bubble = CreateBubble();
    widget->Show();
...
TEST_F(BubbleDialogDelegateViewAnchorTest, ActivationPassesToRemainingWidgetOnBubbleClose)
{
    auto widget = CreateTopLevelWidget();
    auto* bubble = CreateBubble();
    widget->ShowInactive();
...
TEST_F(BubbleDialogDelegateViewAnchorTest, ActivationPassesToOtherWidgetOnReanchor)
{
    auto widget = CreateTopLevelWidget();
    auto* bubble = CreateBubble();
    widget->Show();
...
TEST_F(BubbleDialogDelegateViewAnchorTest, ActivationPassesAcrossChainOfAnchoredBubbles)
{
    auto widget = CreateTopLevelWidget();
...
TEST_F(BubbleDialogDelegateViewAnchorTest, DestroyingAnchoredToWidgetDoesNotCrash)
{
    auto widget = CreateTopLevelWidget();
    auto* bubble = CreateBubble(widget.get());
    widget->Show();
...
TEST_F(BubbleDialogDelegateViewAnchorTest, DestroyingMiddleWidgetOfAnchorChainDoesNotCrash)
{
    auto widget = CreateTopLevelWidget();
    auto* bubble = CreateBubble();
    auto* bubble2 = CreateBubble();
...
TEST_F(BubbleDialogDelegateViewAnchorTest, WidgetForAnchoring)
{
    ...
    GetAnchorView(widget.get())->SetProperty(kWidgetForAnchoringKey, widget_for_anchoring.get());
    auto* bubble = CreateBubble(widget.get());
...
TEST_F(BubbleDialogDelegateViewTest, BubbleMetrics)
{
    ...
    bubble_delegate->GetWidget()->GetCompositor()->RequestSuccessfulPresentationTimeForNextFrame(
        base::BindOnce([](base::RunLoop* run_loop, const viz::FrameTimingDetails& frame_timing_details) { run_loop->Quit(); }, &run_loop));
    run_loop.Run();
...
TEST_F(BubbleUmaLoggerTest, LogMetricFromDelegate)
{
    ...
    BubbleDialogDelegate delegate(anchored_view.get(), BubbleBorder::Arrow::TOP_LEFT);
