...
class VIEWS_EXPORT ViewMaskLayer : public ui::LayerDelegate, public ViewObserver {
public:
...
void ViewMaskLayer::OnDeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor)
{
}
...
View::View()
{
    ...
    static bool capture_stack_trace = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kViewStackTraces);
    if (capture_stack_trace) {
        SetProperty(kViewStackTraceKey, std::make_unique<base::debug::StackTrace>());
    }
}
...
View::~View()
{
    if (layouts_since_last_paint_) {
        UMA_HISTOGRAM_COUNTS_100("Views.UnnecessaryLayouts", layouts_since_last_paint_);
    }
...
void View::ReorderChildView(View* view, size_t index)
{
    ...
    const auto pos = std::next(children_.begin(), static_cast<ptrdiff_t>(std::min(index, children_.size() - 1)));
    if (i == pos) {
        return;
    }
...
std::optional<size_t> View::GetIndexOf(const View* view) const
{
    ...
    return i == children_.cend() ? std::nullopt : std::make_optional(static_cast<size_t>(std::distance(children_.cbegin(), i)));
}
...
void View::SetBoundsRect(const gfx::Rect& bounds)
{
    if (bounds == bounds_) {
        if (needs_layout_) {
            needs_layout_ = false;
            TRACE_EVENT1("views", "View::Layout(set_bounds)", "class", GetClassName());
            LayoutImmediately();
        }
        return;
    }
    ...
    if (layer()) {
        if (parent_) {
            LayerOffsetData offset_data(parent_->CalculateOffsetToAncestorWithLayer(nullptr));
            offset_data += GetMirroredPosition().OffsetFromOrigin();
            SetLayerBounds(size(), offset_data);
        } else {
            SetLayerBounds(bounds_.size(), LayerOffsetData() + bounds_.OffsetFromOrigin());
        }
        ...
        if (GetMirrored() && bounds_.width() != prev.width()) {
            for (View* child : children_) {
                child->UpdateChildLayerBounds(LayerOffsetData(layer()->device_scale_factor(), child->GetMirroredPosition().OffsetFromOrigin()));
            }
        }
    } else {
    ...
    if (needs_layout_ || is_size_changed) {
        needs_layout_ = false;
        TRACE_EVENT1("views", "View::Layout(bounds_changed)", "class", GetClassName());
        LayoutImmediately();
    }
    ...
    // redo them here.
    if (prev.x() != bounds_.x()) {
        OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&bounds_, kXChangedKey), kPropertyEffectsNone);
    }
    if (prev.y() != bounds_.y()) {
        OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&bounds_, kYChangedKey), kPropertyEffectsNone);
    }
    if (prev.width() != bounds_.width()) {
        OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&bounds_, kWidthChangedKey), kPropertyEffectsNone);
    }
    if (prev.height() != bounds_.height()) {
        OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&bounds_, kHeightChangedKey), kPropertyEffectsNone);
    }
}
...
gfx::Rect View::GetVisibleBounds() const
{
    ...
    while (view != nullptr && !vis_bounds.IsEmpty()) {
        ...
        translation.Translate(static_cast<float>(view->GetMirroredX()), static_cast<float>(view->y()));
    ...
    if (was_visible != visible) {
        ...
        if (parent_) {
            ...
            if (!view_accessibility_ || !view_accessibility_->GetIsIgnored()) {
                parent_->NotifyAccessibilityEvent(ax::mojom::Event::kChildrenChanged, true);
            }
        }
...
base::CallbackListSubscription View::AddVisibleChangedCallback(PropertyChangedCallback callback)
{
...
base::CallbackListSubscription View::AddEnabledChangedCallback(PropertyChangedCallback callback)
{
...
gfx::Transform View::GetTransform() const
{
    ...
    transform.Translate((GetMirrored() ? 1 : -1) * scroll_offset.x(), -scroll_offset.y());
    return transform;
}
...
{
    AddLayerToRegionImpl(new_layer, region == LayerRegion::kAbove ? layers_above_ : layers_below_);
}
...
{
    auto remove_layer = [old_layer, this](std::vector<raw_ptr<ui::Layer, VectorExperimental>>& layer_vector) {
    ...
    const bool layer_removed = remove_layer(layers_below_) || remove_layer(layers_above_);
...
void View::Layout(PassKey)
{
    ...
    internal::ScopedChildrenLock lock(this);
    for (views::View* child : children_) {
        if (child->needs_layout_ || !HasLayoutManager() || child->GetProperty(kViewIgnoredByLayoutKey)) {
            TRACE_EVENT1("views", "View::LayoutChildren", "class", child->GetClassName());
...
void View::SetLayoutManagerUseConstrainedSpace(bool layout_manager_use_constrained_space)
{
    if (layout_manager_use_constrained_space == layout_manager_use_constrained_space_) {
        return;
    }
...
base::CallbackListSubscription View::AddIDChangedCallback(PropertyChangedCallback callback)
{
...
base::CallbackListSubscription View::AddGroupChangedCallback(PropertyChangedCallback callback)
{
...
// static
void View::ConvertPointToTarget(const View* source, const View* target, gfx::Point* point)
{
    DCHECK(source);
    DCHECK(target);
    if (source == target) {
    ...
    if (GetHierarchyRoot(source) != root) {
        const Widget* source_top_level_widget = source->GetWidget()->GetTopLevelWidget();
        const Widget* target_top_level_widget = target->GetWidget()->GetTopLevelWidget();
...
// static
gfx::Point View::ConvertPointToTarget(const View* source, const View* target, const gfx::Point& point)
{
...
// static
void View::ConvertRectToTarget(const View* source, const View* target, gfx::RectF* rect)
{
    DCHECK(source);
    DCHECK(target);
    if (source == target) {
...
// static
gfx::RectF View::ConvertRectToTarget(const View* source, const View* target, const gfx::RectF& rect)
{
...
// static
gfx::Rect View::ConvertRectToTarget(const View* source, const View* target, const gfx::Rect& rect)
{
    constexpr float kDefaultAllowedConversionError = 0.00001f;
    return gfx::ToEnclosedRectIgnoringError(ConvertRectToTarget(source, target, gfx::RectF(rect)), kDefaultAllowedConversionError);
}
...
void View::Paint(const PaintInfo& parent_paint_info)
{
    CHECK_EQ(life_cycle_state_, LifeCycleState::kAlive)
        ...
        << "life_cycle_state_: 0x" << IntToHex(static_cast<uint32_t>(life_cycle_state_)) << " Classname: " << GetClassName();
    ...
    const gfx::Rect& parent_bounds = !parent() ? GetMirroredBounds() : parent()->GetMirroredBounds();

    PaintInfo paint_info
        = PaintInfo::CreateChildPaintInfo(parent_paint_info, GetMirroredBounds(), parent_bounds.size(), GetPaintScaleType(), !!layer(), needs_paint_);

    needs_paint_ = false;

    const ui::PaintContext& context = paint_info.context();
    bool is_invalidated = true;
    if (paint_info.context().CanCheckInvalid() || base::FeatureList::IsEnabled(features::kEnableViewPaintOptimization)) {
    ...
    if (layouts_since_last_paint_) {
        UMA_HISTOGRAM_COUNTS_100("Views.UnnecessaryLayouts", layouts_since_last_paint_ - 1);
    ...
    if (!layer()) {
        ...
        if (clip_path_.isEmpty()) {
            clip_recorder.ClipRect(gfx::Rect(paint_info.paint_recording_size()) + paint_info.offset_from_parent());
        } else {
            ...
            to_parent_recording_space.Scale(SkFloatToScalar(paint_info.paint_recording_scale_x()), SkFloatToScalar(paint_info.paint_recording_scale_y()));

            clip_path_in_parent.transform(gfx::TransformToFlattenedSkMatrix(to_parent_recording_space));
            clip_recorder.ClipPathWithAntiAliasing(clip_path_in_parent);
        }
    }

    ui::TransformRecorder transform_recorder(context);
    SetUpTransformRecorderForPainting(paint_info.offset_from_parent(), &transform_recorder);
    ...
    if (is_invalidated || !paint_cache_.UseCache(context, paint_info.paint_recording_size())) {
        ui::PaintRecorder recorder(
            context, paint_info.paint_recording_size(), paint_info.paint_recording_scale_x(), paint_info.paint_recording_scale_y(), &paint_cache_);
...
base::CallbackListSubscription View::AddFlipCanvasOnPaintForRTLUIChangedCallback(PropertyChangedCallback callback)
{
    return AddPropertyChangedCallback(&flip_canvas_on_paint_for_rtl_ui_, std::move(callback));
}
...
{
    bool consumed = (event->type() == ui::EventType::kKeyPressed) ? OnKeyPressed(*event) : OnKeyReleased(*event);
    if (consumed) {
...
std::unique_ptr<ViewTargeter> View::SetEventTargeter(std::unique_ptr<ViewTargeter> targeter)
{
...
void View::ConvertEventToTarget(const ui::EventTarget* target, ui::LocatedEvent* event) const
{
...
{
    return GetFocusBehavior() == FocusBehavior::ALWAYS && GetEnabled() && IsDrawn();
}
...
void View::RequestFocus()
{
    ...
    if (focus_manager) {
        bool focusable = focus_manager->keyboard_accessible() ? GetViewAccessibility().IsAccessibilityFocusable() : IsFocusable();
        if (focusable) {
...
void View::ShowContextMenu(const gfx::Point& p, ui::mojom::MenuSourceType source_type)
{
...
gfx::Point View::GetKeyboardContextMenuLocation()
{
    ...
    gfx::Point screen_point(vis_bounds.x() + vis_bounds.width() / 2, vis_bounds.y() + vis_bounds.height() / 2);
...
bool View::GetDropFormats(int* formats, std::set<ui::ClipboardFormatType>* format_types)
{
    return false;
}
...
void View::SetAccessibleName(std::u16string name, ax::mojom::NameFrom name_from)
{
...
void View::SetAccessibleRole(const ax::mojom::Role role, const std::u16string& role_description)
{
...
void View::SetAccessibleDescription(const std::u16string& description, ax::mojom::DescriptionFrom description_from)
{
...
bool View::HandleAccessibleAction(const ui::AXActionData& action_data)
{
    ...
    case ax::mojom::Action::kDoDefault: {
        ...
        OnEvent(&press);
        ui::MouseEvent release(ui::EventType::kMouseReleased, center, center, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
        OnEvent(&release);
        return true;
    }
...
{
    if (HasLayoutManager()) {
        return GetLayoutManager()->GetPreferredSize(this, layout_manager_use_constrained_space_ ? available_size : SizeBounds());
    }
    return gfx::Size();
}
...
View::LayerOffsetData View::CalculateOffsetToAncestorWithLayer(ui::Layer** layer_parent)
{
    if (layer()) {
        if (layer_parent) {
            *layer_parent = layer();
    ...
    LayerOffsetData offset_data = parent_->CalculateOffsetToAncestorWithLayer(layer_parent);
...
void View::MoveLayerToParent(ui::Layer* parent_layer, const LayerOffsetData& offset_data)
{
...
void View::DestroyLayerImpl(LayerChangeNotifyBehavior notify_parents)
{
    ...
    std::vector<raw_ptr<ui::Layer, VectorExperimental>> children = layer()->children();
...
void View::UpdateChildLayerBounds(const LayerOffsetData& offset_data)
{
    if (layer()) {
        ...
        internal::ScopedChildrenLock lock(this);
        for (views::View* child : children_) {
            child->UpdateChildLayerBounds(offset_data + child->GetMirroredPosition().OffsetFromOrigin());
        }
    }
}
...
void View::OnLayerTransformed(const gfx::Transform& old_transform, ui::PropertyChangeReason reason)
{
...
void View::OnLayerClipRectChanged(const gfx::Rect& old_rect, ui::PropertyChangeReason reason)
{
    observers_.Notify(&ViewObserver::OnViewLayerClipRectChanged, this);
}

void View::OnDeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor)
{
    snap_layer_to_pixel_boundary_ = (new_device_scale_factor - std::floor(new_device_scale_factor)) != 0.0f;
    ...
    if (layer()->parent() && layer()->GetCompositor() && layer()->GetCompositor()->is_pixel_canvas()) {
        LayerOffsetData offset_data(parent()->CalculateOffsetToAncestorWithLayer(nullptr));
        offset_data += GetMirroredPosition().OffsetFromOrigin();
...
{
    if (paint_to_layer_explicitly_set_ || paint_to_layer_for_transform_ || !layers_below_.empty() || !layers_above_.empty()) {
...
void View::AddLayerToRegionImpl(ui::Layer* new_layer, std::vector<raw_ptr<ui::Layer, VectorExperimental>>& layer_vector)
{
    DCHECK(new_layer);
    ...
    if (layer()) {
        ...
        new_layer->SetBounds(gfx::Rect(new_layer->size()) + layer()->bounds().OffsetFromOrigin());
        if (parent()) {
...
void View::Focus()
{
    ...
    if (!suppress_default_focus_handling_) {
        ...
        AXVirtualView* const focused_virtual_child = view_accessibility_ ? view_accessibility_->FocusedVirtualChild() : nullptr;
...
{
    if (base::FeatureList::IsEnabled(::views::features::kKeyboardAccessibleTooltipInViews) && !kShouldDisableKeyboardTooltipsForTesting) {
        Widget* widget = GetWidget();
        if (widget && widget->GetTooltipManager()) {
...
void View::AfterPropertyChange(const void* key, int64_t old_value)
{
    if (key == kElementIdentifierKey) {
        const ui::ElementIdentifier old_element_id = ui::ElementIdentifier::FromRawValue(base::checked_cast<intptr_t>(old_value));
        if (old_element_id) {
            views::ElementTrackerViews::GetInstance()->UnregisterView(old_element_id, this);
        }
        const ui::ElementIdentifier new_element_id = GetProperty(kElementIdentifierKey);
        if (new_element_id) {
            views::ElementTrackerViews::GetInstance()->RegisterView(new_element_id, this);
        }
    }
    observers_.Notify(&ViewObserver::OnViewPropertyChanged, this, key, old_value);
}

void View::OnPropertyChanged(ui::metadata::PropertyKey property, PropertyEffects property_effects)
{
...
void View::SetUpTransformRecorderForPainting(const gfx::Vector2d& offset_from_parent, ui::TransformRecorder* recorder) const
{
    // If the view is backed by a layer, it should paint with itself as the origin
    // rather than relative to its parent.
    if (layer()) {
    ...
    transform_from_parent.Translate(offset_from_parent.x(), offset_from_parent.y());
    recorder->Transform(transform_from_parent);
}

void View::RecursivePaintHelper(void (View::*func)(const PaintInfo&), const PaintInfo& info)
{
    View::Views children = GetChildrenInZOrder();
    DCHECK_EQ(children_.size(), children.size());
...
{
    PaintInfo paint_info = PaintInfo::CreateRootPaintInfo(parent_context, layer() ? layer()->size() : size());
    Paint(paint_info);
    static bool draw_view_bounds_rects = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDrawViewBoundsRects);
...
void View::PaintDebugRects(const PaintInfo& parent_paint_info)
{
    ...
    const gfx::Rect& parent_bounds = (layer() || !parent()) ? GetMirroredBounds() : parent()->GetMirroredBounds();
    PaintInfo paint_info = PaintInfo::CreateChildPaintInfo(parent_paint_info, GetMirroredBounds(), parent_bounds.size(), GetPaintScaleType(), !!layer());
    ...
    ui::PaintRecorder recorder(context, paint_info.paint_recording_size(), paint_info.paint_recording_scale_x(), paint_info.paint_recording_scale_y(),
        /*cache*/ nullptr);
    gfx::Canvas* canvas = recorder.canvas();
    const float scale = canvas->UndoDeviceScaleFactor();
    gfx::RectF outline_rect(ScaleToEnclosedRect(GetLocalBounds(), scale));
    gfx::RectF content_outline_rect(ScaleToEnclosedRect(GetContentsBounds(), scale));
    const auto* color_provider = GetColorProvider();
    if (content_outline_rect != outline_rect) {
        content_outline_rect.Inset(0.5f);
        canvas->DrawRect(content_outline_rect, color_provider->GetColor(ui::kColorDebugContentOutline));
    }
    outline_rect.Inset(0.5f);
    canvas->DrawRect(outline_rect, color_provider->GetColor(ui::kColorDebugBoundsOutline));
}
...
void View::AddChildViewAtImpl(View* view, size_t index)
{
    ...
    DCHECK(!iterating_);
#endif
    const auto pos = children_.insert(std::next(children_.cbegin(), static_cast<ptrdiff_t>(index)), view);
...
void View::DoRemoveChildView(View* view, bool update_tool_tip, bool delete_removed_view, View* new_parent)
{
    DCHECK(view);
...
void View::PropagateRemoveNotifications(View* old_parent, View* new_parent, bool is_removed_from_widget)
{
    {
        internal::ScopedChildrenLock lock(this);
        for (views::View* child : children_) {
            child->PropagateRemoveNotifications(old_parent, new_parent, is_removed_from_widget);
        }
    }
...
void View::PropagateAddNotifications(const ViewHierarchyChangedDetails& details, bool is_added_to_widget)
{
...
void View::ViewHierarchyChangedImpl(const ViewHierarchyChangedDetails& details)
{
...
void View::SnapLayerToPixelBoundary(const LayerOffsetData& offset_data)
{
    ...
    if (layer()->GetCompositor() && layer()->GetCompositor()->is_pixel_canvas()) {
        gfx::Vector2dF offset = snap_layer_to_pixel_boundary_ && layer()->parent() ? offset_data.GetSubpixelOffset() : gfx::Vector2dF();
...
void View::SetLayerBounds(const gfx::Size& size, const LayerOffsetData& offset_data)
{
...
bool View::GetTransformRelativeTo(const View* ancestor, gfx::Transform* transform) const
{
    const View* p = this;
    ...
    while (p && p != ancestor) {
        ...
        gfx::Transform translation;
        translation.Translate(static_cast<float>(p->GetMirroredX()), static_cast<float>(p->y()));
        transform->PostConcat(translation);
...
bool View::ConvertPointForAncestor(const View* ancestor, gfx::Point* point) const
{
    gfx::Transform trans;
...
bool View::ConvertPointFromAncestor(const View* ancestor, gfx::Point* point) const
{
    gfx::Transform trans;
    bool result = GetTransformRelativeTo(ancestor, &trans);
    if (const std::optional<gfx::PointF> transformed_point = trans.InverseMapPoint(gfx::PointF(*point))) {
        *point = gfx::ToFlooredPoint(transformed_point.value());
    }
    return result;
}

bool View::ConvertRectForAncestor(const View* ancestor, gfx::RectF* rect) const
{
    gfx::Transform trans;
    // TODO(sad): Have some way of caching the transformation results.
    bool result = GetTransformRelativeTo(ancestor, &trans);
...
bool View::ConvertRectFromAncestor(const View* ancestor, gfx::RectF* rect) const
{
    gfx::Transform trans;
    bool result = GetTransformRelativeTo(ancestor, &trans);
    ...
    if (layer()) {
        if (ui::Layer* parent = layer()->parent()) {
            for (ui::Layer* layer : GetLayersInOrder()) {
                ...
                SCOPED_CRASH_KEY_BOOL("OrphanLayers", "layer_is_sibling", layer->parent() == parent);
                SCOPED_CRASH_KEY_STRING256("OrphanLayers", "this_layer_name", this->layer()->name());
                SCOPED_CRASH_KEY_STRING256("OrphanLayers", "parent_layer_name", parent->name());
                SCOPED_CRASH_KEY_STRING256("OrphanLayers", "sibling_layer_name", layer->name());
                SCOPED_CRASH_KEY_STRING256("OrphanLayers", "widget_name", GetWidget() ? GetWidget()->GetName() : "");
                SCOPED_CRASH_KEY_STRING256("OrphanLayers", "view_class_name", GetClassName());
                parent->Remove(layer);
            }
        }
...
void View::ReparentLayer(ui::Layer* parent_layer)
{
    ...
    LayerOffsetData offset = parent_ ? parent_->CalculateOffsetToAncestorWithLayer(nullptr) : LayerOffsetData(layer()->device_scale_factor());
...
void View::LayoutImmediately()
{
    ...
    Layout(PassKey());
    UMA_HISTOGRAM_COUNTS_100("Views.InvalidatesDuringLayout", invalidates_during_layout_);
    --current_layout_call_depth_;
    if (current_layout_call_depth_ == 0) {
        UMA_HISTOGRAM_EXACT_LINEAR("Views.LayoutCallDepth", max_layout_call_depth_, 6);
...
bool View::ProcessMousePressed(const ui::MouseEvent& event)
{
    ...
    ContextMenuController* context_menu_controller = event.IsRightMouseButton() ? context_menu_controller_.get() : nullptr;
...
void View::ProcessMouseDragged(ui::MouseEvent* event)
{
    ...
    if (possible_drag && ExceededDragThreshold(GetDragInfo()->start_pt - event->location())
        && (!drag_controller_ || drag_controller_->CanStartDragForView(this, GetDragInfo()->start_pt, event->location()))) {
        if (DoDrag(*event, GetDragInfo()->start_pt, ui::mojom::DragEventSource::kMouse)) {
            event->StopPropagation();
            return;
        }
    } else {
...
{
    if (!accelerators_ || registered_accelerator_count_ == accelerators_->size()) {
    ...
    for (std::vector<ui::Accelerator>::const_iterator i = accelerators_->begin() + static_cast<ptrdiff_t>(registered_accelerator_count_);
         i != accelerators_->end(); ++i) {
        accelerator_focus_manager_->RegisterAccelerator(*i, ui::AcceleratorManager::kNormalPriority, this);
    }
...
void View::SetFocusSiblings(View* view, Views::const_iterator pos)
{
    ...
    if (children_.size() > 1) {
        if (std::next(pos) == children_.cend()) {
            ...
            // child.
            View* const old_last = *base::ranges::find_if_not(children_.cbegin(), pos, &View::next_focusable_view_);
...
void View::AdvanceFocusIfNecessary()
{
    ...
    if (GetViewAccessibility().ViewAccessibility::IsAccessibilityFocusable() || !HasFocus()) {
        return;
    }
...
#if DCHECK_IS_ON()
    DCHECK(on_theme_changed_called_) << "views::View::OnThemeChanged() has not been called. This means that "
...
void View::PropagateDeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor)
{
    {
        internal::ScopedChildrenLock lock(this);
        for (views::View* child : base::Reversed(children_)) {
            child->PropagateDeviceScaleFactorChanged(old_device_scale_factor, new_device_scale_factor);
        }
    }
    ...
    if (!layer()) {
        OnDeviceScaleFactorChanged(old_device_scale_factor, new_device_scale_factor);
        return;
    }
...
bool View::DoDrag(const ui::LocatedEvent& event, const gfx::Point& press_pt, ui::mojom::DragEventSource source)
{
    ...
    widget->RunShellDrag(this, std::move(data), widget_location, drag_operations, source);
    // WARNING: we may have been deleted.
    return true;
}
...
base::CallbackListSubscription View::RegisterNotifyViewControllerCallback(base::RepeatingClosureList::CallbackType callback)
{
...
void BaseActionViewInterface::ActionItemChangedImpl(actions::ActionItem* action_item)
{
...
DEFINE_ENUM_CONVERTERS(views::View::FocusBehavior, { views::View::FocusBehavior::ACCESSIBLE_ONLY, u"ACCESSIBLE_ONLY" },
    { views::View::FocusBehavior::ALWAYS, u"ALWAYS" }, { views::View::FocusBehavior::NEVER, u"NEVER" })
