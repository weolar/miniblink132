...
TEST_F(ViewTrackerTest, TestTrackEntireViewHierarchy)
{
    ...
    ViewTracker tracker;
    {
        ...
        tracker.SetView(tracked_view.get());
        tracker.SetIsDeletingCallback(base::BindOnce(hierarchy_deleting_callback, base::Unretained(tracked_view.get())));
    }
    ...
    };
    {
        ...
        tracked_view->AddChildView(std::make_unique<View>());
        tracker.SetView(tracked_view.get());
        tracker.SetIsDeletingCallback(base::BindOnce(view_deleting_callback, base::Unretained(tracked_view.get())));
    }
}
