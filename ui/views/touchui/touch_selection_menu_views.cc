...
TouchSelectionMenuViews::TouchSelectionMenuViews(
    TouchSelectionMenuRunnerViews* owner, base::WeakPtr<ui::TouchSelectionMenuClient> client, aura::Window* context)
...
void TouchSelectionMenuViews::ShowMenu(const gfx::Rect& anchor_rect, const gfx::Size& handle_image_size)
{
    CreateButtons();
    ...
    if (menu_width > anchor_rect.width() - handle_image_size.width())
        adjusted_anchor_rect.Inset(gfx::Insets::TLBR(0, 0, -handle_image_size.height(), 0));
    if (::features::IsTouchTextEditingRedesignEnabled()) {
    ...
    gfx::Rect work_area = display::Screen::GetScreen()->GetDisplayNearestPoint(bounds.origin()).work_area();
...
bool TouchSelectionMenuViews::IsMenuAvailable(const ui::TouchSelectionMenuClient* client)
{
    DCHECK(client);

    const auto is_enabled = [client](MenuCommand command) { return client->IsCommandIdEnabled(command.command_id); };
    bool is_available = base::ranges::any_of(kMenuCommands, is_enabled);
    is_available |= ::features::IsTouchTextEditingRedesignEnabled() && base::ranges::any_of(kMenuSelectCommands, is_enabled);
    return is_available;
}
...
void TouchSelectionMenuViews::CreateButtons()
{
    ...
    for (const auto& command : kMenuCommands) {
        ...
        CreateButton(l10n_util::GetStringUTF16(command.message_id),
            base::BindRepeating(&TouchSelectionMenuViews::ButtonPressed, base::Unretained(this), command.command_id));
        CreateSeparator();
    }
    ...
    CreateButton(u"...", base::BindRepeating(&TouchSelectionMenuViews::EllipsisPressed, base::Unretained(this)))->SetID(ButtonViewId::kEllipsisButton);
    InvalidateLayout();
}

LabelButton* TouchSelectionMenuViews::CreateButton(const std::u16string& title, Button::PressedCallback callback)
{
    std::u16string label = gfx::RemoveAccelerator(title);
    auto* button = AddChildView(std::make_unique<LabelButton>(std::move(callback), label, style::CONTEXT_TOUCH_MENU));
    if (::features::IsTouchTextEditingRedesignEnabled()) {
        button->SetBorder(CreateEmptyBorder(gfx::Insets::VH(0, kButtonHorizontalPadding)));
...
void TouchSelectionMenuViews::ButtonPressed(int command, const ui::Event& event)
{
