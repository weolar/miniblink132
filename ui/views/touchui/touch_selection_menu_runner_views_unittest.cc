...
class TouchSelectionMenuRunnerViewsTest : public ViewsTestBase, public ui::TouchSelectionMenuClient {
public:
    TouchSelectionMenuRunnerViewsTest() = default;

    TouchSelectionMenuRunnerViewsTest(const TouchSelectionMenuRunnerViewsTest&) = delete;
    TouchSelectionMenuRunnerViewsTest& operator=(const TouchSelectionMenuRunnerViewsTest&) = delete;
...
protected:
    ...
    int last_executed_command_id() const
    {
        return last_executed_command_id_;
    }
...
TEST_F(TouchSelectionMenuRunnerViewsTest, InstalledAndWorksProperly)
{
    ...
    ui::TouchSelectionMenuRunner::GetInstance()->OpenMenu(GetWeakPtr(), menu_anchor, handle_size, GetContext());
...
{
    TouchSelectionMenuRunnerViews::TestApi test_api(static_cast<TouchSelectionMenuRunnerViews*>(ui::TouchSelectionMenuRunner::GetInstance()));
    ...
    ui::TouchSelectionMenuRunner::GetInstance()->OpenMenu(GetWeakPtr(), anchor_rect, kHandleSize, GetContext());
    EXPECT_GE(test_api.GetAnchorRect().bottom(), anchor_rect.bottom() + kHandleSize.height());
    ...
    anchor_rect = gfx::Rect(test_api.GetMenuWidth() + kHandleSize.width() + 10, 20);
    ui::TouchSelectionMenuRunner::GetInstance()->OpenMenu(GetWeakPtr(), anchor_rect, kHandleSize, GetContext());
    EXPECT_GE(test_api.GetAnchorRect().bottom(), anchor_rect.bottom());
    EXPECT_LE(test_api.GetAnchorRect().bottom(), anchor_rect.bottom() + kHandleSize.height());
...
TEST_F(TouchSelectionMenuRunnerViewsTest, RunningActionClosesProperly)
{
    ...
    ui::GestureEvent tap(button_center.x(), button_center.y(), 0, ui::EventTimeForNow(), details);
...
TEST_F(TouchSelectionMenuRunnerViewsTest, ShowMenuTwiceOpensOneMenu)
{
    gfx::Rect menu_anchor(0, 0, 10, 10);
    gfx::Size handle_size(10, 10);
    auto* menu_runner = static_cast<TouchSelectionMenuRunnerViews*>(ui::TouchSelectionMenuRunner::GetInstance());
    ...
    auto* menu1 = new TouchSelectionMenuViews(menu_runner, GetWeakPtr(), GetContext());
    test_api.ShowMenu(menu1, menu_anchor, handle_size);
    auto* widget1 = test_api.GetWidget();

    auto* menu2 = new TouchSelectionMenuViews(menu_runner, GetWeakPtr(), GetContext());
...
TEST_F(TouchSelectionMenuRunnerViewsTest, MenuActionMetrics)
{
    ...
    // Open the menu.
    ui::TouchSelectionMenuRunner::GetInstance()->OpenMenu(GetWeakPtr(), /*anchor_rect=*/gfx::Rect(20, 30),
        /*handle_image_size=*/gfx::Size(10, 10), GetContext());

    EXPECT_TRUE(ui::TouchSelectionMenuRunner::GetInstance()->IsRunning());
    histogram_tester.ExpectTotalCount(ui::kTouchSelectionMenuActionHistogramName, 0);

    // Tap the first action on the menu.
    ui::test::EventGenerator generator(test_api.GetWidget()->GetNativeView()->GetRootWindow());
    gfx::Point button_center = test_api.GetFirstButton()->bounds().CenterPoint();
    generator.delegate()->ConvertPointFromTarget(test_api.GetWidget()->GetNativeView(), &button_center);
    generator.GestureTapAt(button_center);

    histogram_tester.ExpectTotalCount(ui::kTouchSelectionMenuActionHistogramName, 1);
}
