...
// the same location.
int CompareTextSelectionBounds(const gfx::SelectionBound& b1, const gfx::SelectionBound& b2)
{
    if (b1.edge_start().y() < b2.edge_start().y() || b1.edge_start().x() < b2.edge_start().x()) {
        return -1;
    }
    if (b1 == b2)
        return 0;
...
public:
    ...
    TouchSelectionControllerImplTest(const TouchSelectionControllerImplTest&) = delete;
    TouchSelectionControllerImplTest& operator=(const TouchSelectionControllerImplTest&) = delete;
    ...
    {
        ViewsTestBase::SetUp();
        test_cursor_client_ = std::make_unique<aura::test::TestCursorClient>(GetContext());
    }
    ...
    void CreateTextfield()
    {
        ...
        textfield_ = textfield_widget_->SetContentsView(std::make_unique<View>())
                         ->AddChildView(Builder<Textfield>().SetPlaceholderText(u"Foo").SetID(1).SetBoundsRect(gfx::Rect(0, 0, 200, 21)).Build());
...
protected:
    ...
    {
        return TextfieldTestApi(textfield_).GetRenderText()->GetCursorBounds(sel, true);
    }
    ...
    {
        return static_cast<TouchSelectionControllerImpl*>(TextfieldTestApi(textfield_).touch_selection_controller());
    }
    ...
    void SimulateSelectionHandleDrag(gfx::Vector2d v, int selection_handle)
    {
        ...
        gfx::Point grip_location = gfx::Point(handle->size().width() / 2, handle->size().height() / 2);
        base::TimeTicks time_stamp = base::TimeTicks();
        {
            ui::GestureEventDetails details(ui::EventType::kGestureScrollBegin);
            ui::GestureEvent scroll_begin(grip_location.x(), grip_location.y(), 0, time_stamp, details);
        ...
        test_cursor_client_->DisableMouseEvents();
        {
            ...
            ui::GestureEvent scroll_update(update_location.x(), update_location.y(), 0, time_stamp, details);
            handle->OnGestureEvent(&scroll_update);
        }
        {
            ui::GestureEventDetails details(ui::EventType::kGestureScrollEnd);
            ui::GestureEvent scroll_end(grip_location.x(), grip_location.y(), 0, time_stamp, details);
    ...
    bool IsQuickMenuVisible()
    {
        ...
        return ui::TouchSelectionMenuRunner::GetInstance() && ui::TouchSelectionMenuRunner::GetInstance()->IsRunning();
    }
    ...
    void VerifyHandlePositions(bool cursor_at_selection_handle_1, bool check_direction, const base::Location& from_here)
    {
    ...
    void SetupSelectionInvisibleHandle(uint32_t selection_start)
    {
        ...
        textfield_->SetSelectedRange(gfx::Range(selection_start, static_cast<uint32_t>(textfield_text.length())));

        // Check that one selection handle is hidden.
        EXPECT_FALSE(IsSelectionHandle1Visible());
        EXPECT_TRUE(IsSelectionHandle2Visible());
        EXPECT_EQ(gfx::Range(selection_start, static_cast<uint32_t>(textfield_text.length())), textfield_->GetSelectedRange());
    }

    void CloseHandle1Widget()
    {
        GetSelectionController()->GetHandle1View()->GetWidget()->CloseWithReason(views::Widget::ClosedReason::kUnspecified);
    }
...
TEST_F(TouchSelectionControllerImplTest, SelectionInBidiTextfieldTest)
{
    ...
    textfield_->SelectSelectionModel(gfx::SelectionModel(3, gfx::CURSOR_BACKWARD));
    VerifyHandlePositions(false, true, FROM_HERE);
...
{
    CreateTextfield();
    textfield_->SetText(u"abc\x05e1\x05e2\x05e3"
                        u"def");
    // Tap the textfield to invoke touch selection.
    ui::GestureEventDetails details(ui::EventType::kGestureTap);
    details.set_tap_count(1);
...
TEST_F(TouchSelectionControllerImplTest, HiddenSelectionHandleRetainsCursorPosition)
{
...
TEST_F(TouchSelectionControllerImplTest, DoubleTapInTextfieldWithCursorHandleShouldSelectText)
{
    CreateTextfield();
    textfield_->SetText(u"some text");
    ui::test::EventGenerator generator(textfield_->GetWidget()->GetNativeView()->GetRootWindow());
...
TEST_F(TouchSelectionControllerImplTest, MenuAppearsAfterDraggingSelectionHandles)
{
    CreateTextfield();
...
TEST_F(TouchSelectionControllerImplTest, MagnifierShownWhenDraggingCursorHandle)
{
    ...
    generator.GestureScrollSequenceWithCallback(drag_start, drag_end, /*duration=*/base::Milliseconds(50),
        /*steps=*/5, base::BindLambdaForTesting([&](ui::EventType event_type, const gfx::Vector2dF& offset) {
...
TEST_F(TouchSelectionControllerImplTest, MagnifierShownWhenDraggingSelectionHandles)
{
    base::test::ScopedFeatureList feature_list;
    feature_list.InitWithFeatures(
        /*enabled_features=*/ { ::features::kTouchTextEditingRedesign },
...
TEST_F(TouchSelectionControllerImplTest, MagnifierShownWhenDraggingCursor)
{
    ...
    // dragging ends.
    const gfx::Point drag_start = GetCursorPosition(gfx::SelectionModel(6, gfx::CURSOR_FORWARD));
...
TEST_F(TouchSelectionControllerImplTest, TapOnCursorTogglesMenu)
{
    ...
    const gfx::Point cursor_position = GetCursorRect(textfield_->GetSelectionModel()).CenterPoint();
    ui::test::EventGenerator generator(textfield_->GetWidget()->GetNativeView()->GetRootWindow());
...
TEST_F(TouchSelectionControllerImplTest, SelectCommands)
{
    ...
    menu_client->ExecuteCommand(ui::TouchEditable::kSelectWord, ui::EF_FROM_TOUCH);
...
TEST_F(TouchSelectionControllerImplTest, CursorHandleDraggingMetrics)
{
    ...
    generator.GestureScrollSequence(drag_start, drag_end,
        ...
        /*steps=*/5);
    histogram_tester.ExpectBucketCount(ui::kTouchSelectionDragTypeHistogramName, ui::TouchSelectionDragType::kCursorHandleDrag, 1);
}
...
TEST_F(TouchSelectionControllerImplTest, SelectionHandleDraggingMetrics)
{
    ...
    generator.GestureScrollSequence(drag_start, drag_end,
        /*duration=*/base::Milliseconds(50),
        /*steps=*/5);
    histogram_tester.ExpectBucketCount(ui::kTouchSelectionDragTypeHistogramName, ui::TouchSelectionDragType::kSelectionHandleDrag, 1);
    ...
    generator.GestureScrollSequence(drag_start, drag_end,
        /*duration=*/base::Milliseconds(50),
        /*steps=*/5);
    histogram_tester.ExpectBucketCount(ui::kTouchSelectionDragTypeHistogramName, ui::TouchSelectionDragType::kSelectionHandleDrag, 2);
}
...
private:
    ...
    void SelectBetweenCoordinates(const gfx::Point& base, const gfx::Point& extent) override
    {
        NOTREACHED();
    }
    void GetSelectionEndPoints(gfx::SelectionBound* anchor, gfx::SelectionBound* focus) override
    {
...
TEST_F(TouchSelectionControllerImplTest, VisibilityOfHandleRegardingClientBounds)
{
    ...
    std::unique_ptr<TouchSelectionController> touch_selection_controller = std::make_unique<TouchSelectionControllerImpl>(&touch_editable);
    ...
    touch_editable.set_cursor_rect(gfx::RectF(2.f, kBarMinHeight - 20.f, 1.f, 20.f));
    touch_selection_controller->SelectionChanged();
    EXPECT_TRUE(IsCursorHandleVisibleFor(touch_selection_controller.get()));

    // Move the cursor up such that less than |kBarMinHeight| pixels are in the
    // client bounds. Handle should be hidden.
    touch_editable.set_cursor_rect(gfx::RectF(2.f, kBarMinHeight - 20.f - 1.f, 1.f, 20.f));
    touch_selection_controller->SelectionChanged();
    ...
    touch_editable.set_cursor_rect(gfx::RectF(2.f, kBarBottomAllowance, 1.f, 20.f));
    touch_selection_controller->SelectionChanged();
    EXPECT_TRUE(IsCursorHandleVisibleFor(touch_selection_controller.get()));

    // Move the cursor down such that more than |kBarBottomAllowance| pixels are
    // out of the client bounds. Handle should be hidden.
    touch_editable.set_cursor_rect(gfx::RectF(2.f, kBarBottomAllowance + 1.f, 1.f, 20.f));
    touch_selection_controller->SelectionChanged();
    EXPECT_FALSE(IsCursorHandleVisibleFor(touch_selection_controller.get()));
...
TEST_F(TouchSelectionControllerImplTest, HandlesStackAboveParent)
{
    ...
    ui::EventTargeter* targeter = root->GetHost()->dispatcher()->GetDefaultEventTargeter();
    ...
    ui::MouseEvent test_event1(ui::EventType::kMouseMoved, test_point, test_point, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    EXPECT_EQ(GetCursorHandleNativeView(), targeter->FindTargetForEvent(root, &test_event1));
    EndTouchEditing();
    ...
    StartTouchEditing();
    ui::MouseEvent test_event2(ui::EventType::kMouseMoved, test_point, test_point, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ...
    ui::MouseEvent test_event3(ui::EventType::kMouseMoved, test_point, test_point, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    EXPECT_EQ(GetCursorHandleNativeView(), targeter->FindTargetForEvent(root, &test_event3));
}
...
TEST_F(TouchSelectionControllerImplTest, MouseEventDeactivatesTouchSelection)
{
    ...
    ui::test::EventGenerator generator(textfield_widget_->GetNativeView()->GetRootWindow());
...
TEST_F(TouchSelectionControllerImplTest, MouseCaptureChangedEventIgnored)
{
    ...
    StartTouchEditing();
    EXPECT_TRUE(GetSelectionController());
    ui::MouseEvent capture_changed(ui::EventType::kMouseCaptureChanged, gfx::Point(5, 5), gfx::Point(5, 5), base::TimeTicks(), 0, 0);
...
TEST_F(TouchSelectionControllerImplTest, DestroyingEditingHandleWidgetDoesNotCrash)
{
    CreateTextfield();
    textfield_->SetText(u"some text in a textfield");
    textfield_->SetSelectedRange(gfx::Range(7, 7));
