...
{
    return ::features::IsTouchTextEditingRedesignEnabled() ? 0 : kSelectionHandleVerticalOffset;
}
...
{
    return ::features::IsTouchTextEditingRedesignEnabled() ? kSelectionHandlePadding : kSelectionHandleHorizontalPadding;
}
int GetSelectionHandleBottomPadding()
{
    return ::features::IsTouchTextEditingRedesignEnabled() ? kSelectionHandlePadding : kSelectionHandleBottomPadding;
}
...
gfx::Image* GetCenterHandleImage()
{
    ...
    if (!handle_image) {
        handle_image = &ui::ResourceBundle::GetSharedInstance().GetImageNamed(IDR_TEXT_SELECTION_HANDLE_CENTER);
    }
    return handle_image;
}
...
gfx::Image* GetLeftHandleImage()
{
    static gfx::Image* handle_image = nullptr;
    if (!handle_image) {
        handle_image = &ui::ResourceBundle::GetSharedInstance().GetImageNamed(IDR_TEXT_SELECTION_HANDLE_LEFT);
    }
    return handle_image;
}
...
gfx::Image* GetRightHandleImage()
{
    static gfx::Image* handle_image = nullptr;
    if (!handle_image) {
        handle_image = &ui::ResourceBundle::GetSharedInstance().GetImageNamed(IDR_TEXT_SELECTION_HANDLE_RIGHT);
    }
    return handle_image;
}
...
{
    return ::features::IsTouchTextEditingRedesignEnabled() ? GetHandleVectorIcon(bound_type) : ui::ImageModel::FromImage(*GetHandleImage(bound_type));
    ;
}
...
gfx::Rect GetSelectionWidgetBounds(const gfx::SelectionBound& bound)
{
    ...
    const int widget_width = image_size.width() + 2 * GetSelectionHandleHorizontalPadding();
    // Extend the widget height to handle touch events below the painted image.
    const int widget_height = bound.GetHeight() + image_size.height() + GetSelectionHandleVerticalOffset() + GetSelectionHandleBottomPadding();
    ...
    switch (bound.type()) {
    case gfx::SelectionBound::LEFT:
        widget_left = bound.edge_start_rounded().x() - image_size.width() - GetSelectionHandleHorizontalPadding();
        break;
    case gfx::SelectionBound::RIGHT:
        widget_left = bound.edge_start_rounded().x() - GetSelectionHandleHorizontalPadding();
        break;
    case gfx::SelectionBound::CENTER:
    ...
    return gfx::Rect(widget_left, bound.edge_start_rounded().y(), widget_width, widget_height);
}
...
gfx::SelectionBound ConvertFromScreen(ui::TouchEditable* client, const gfx::SelectionBound& bound)
{
...
gfx::SelectionBound ConvertToScreen(ui::TouchEditable* client, const gfx::SelectionBound& bound)
{
    gfx::SelectionBound result = bound;
    gfx::Point edge_end = bound.edge_end_rounded();
    gfx::Point edge_start = bound.edge_start_rounded();
...
{
    return gfx::BoundingRect(bound.edge_start_rounded(), bound.edge_end_rounded());
}

std::unique_ptr<views::Widget> CreateHandleWidget(gfx::NativeView parent)
{
    views::Widget::InitParams params(views::Widget::InitParams::CLIENT_OWNS_WIDGET, views::Widget::InitParams::TYPE_POPUP);
    params.opacity = views::Widget::InitParams::WindowOpacity::kTranslucent;
    params.shadow_type = views::Widget::InitParams::ShadowType::kNone;
    params.parent = parent;

    auto widget = std::make_unique<views::Widget>(std::move(params));
    widget->GetNativeWindow()->SetEventTargeter(std::make_unique<aura::WindowTargeter>());
...
public:
    EditingHandleView(TouchSelectionControllerImpl* controller, bool is_cursor_handle)
    ...
    {
        canvas->DrawImageInt(handle_image_.Rasterize(GetColorProvider()), GetSelectionHandleHorizontalPadding(),
    ...
    void OnGestureEvent(ui::GestureEvent* event) override
    {
        ...
        case ui::EventType::kGestureScrollBegin: {
            ...
            drag_offset_ = selection_bound_.edge_end_rounded() - gfx::Vector2d(0, kSelectionHandleVerticalDragOffset) - event->location();
            break;
        }
        ...
        case ui::EventType::kScrollFlingStart: {
            ...
            ui::RecordTouchSelectionDrag(is_cursor_handle_ ? ui::TouchSelectionDragType::kCursorHandleDrag : ui::TouchSelectionDragType::kSelectionHandleDrag);
            break;
        }
        default:
            break;
    ...
    void SetBoundInScreen(const gfx::SelectionBound& bound, bool is_visible)
    {
        ...
        const auto insets = gfx::Insets::TLBR(selection_bound_.GetHeight() + GetSelectionHandleVerticalOffset(), 0, 0, 0);
    ...
    gfx::Size GetHandleImageSize() const
    {
        return handle_image_.Size();
    }
...
TouchSelectionControllerImpl::TouchSelectionControllerImpl(ui::TouchEditable* client_view)
    : client_view_(client_view)
{
    ...
    std::set<ui::EventType> types = { ui::EventType::kMousePressed, ui::EventType::kMouseMoved, ui::EventType::kKeyPressed, ui::EventType::kMousewheel };
...
void TouchSelectionControllerImpl::SelectionChanged()
{
    ...
    gfx::SelectionBound screen_bound_anchor = ConvertToScreen(client_view_, anchor);
    ...
    gfx::SelectionBound screen_bound_anchor_clipped = ConvertToScreen(client_view_, anchor);
    gfx::SelectionBound screen_bound_focus_clipped = ConvertToScreen(client_view_, focus);
    const bool is_client_selection_dragging = client_view_->IsSelectionDragging();
    if (is_client_selection_dragging == is_client_selection_dragging_ && screen_bound_anchor_clipped == selection_bound_1_clipped_
    ...
    if (is_client_selection_dragging) {
        ...
        if (screen_bound_anchor.edge_start() == screen_bound_focus.edge_start() && screen_bound_anchor.edge_end() == screen_bound_focus.edge_end()) {
...
void TouchSelectionControllerImpl::OnDragUpdate(EditingHandleView* handle, const gfx::Point& drag_pos)
{
    DCHECK_EQ(handle, GetDraggingHandle());
...
void TouchSelectionControllerImpl::ConvertPointToClientView(EditingHandleView* source, gfx::Point* point)
{
    View::ConvertPointToScreen(source, point);
    client_view_->ConvertPointFromScreen(point);
}

void TouchSelectionControllerImpl::SetHandleBound(EditingHandleView* handle, const gfx::SelectionBound& bound, const gfx::SelectionBound& bound_in_screen)
{
    handle->SetWidgetVisible(ShouldShowHandleFor(bound));
    handle->SetBoundInScreen(bound_in_screen, handle->GetWidgetVisible());
}

bool TouchSelectionControllerImpl::ShouldShowHandleFor(const gfx::SelectionBound& bound) const
{
    if (bound.GetHeight() < kSelectionHandleBarMinHeight)
        return false;
    gfx::Rect client_bounds = client_view_->GetBounds();
    client_bounds.Inset(gfx::Insets::TLBR(0, 0, -kSelectionHandleBarBottomAllowance, 0));
...
void TouchSelectionControllerImpl::ExecuteCommand(int command_id, int event_flags)
{
...
void TouchSelectionControllerImpl::OnWidgetBoundsChanged(Widget* widget, const gfx::Rect& new_bounds)
{
    DCHECK_EQ(client_widget_, widget);
    SelectionChanged();
}
...
void TouchSelectionControllerImpl::OnEvent(const ui::Event& event)
{
    if (event.IsMouseEvent()) {
        auto* cursor = aura::client::GetCursorClient(client_view_->GetNativeView()->GetRootWindow());
        ...
        if ((event.flags() & (int { ui::EF_IS_SYNTHESIZED } | ui::EF_FROM_TOUCH))
            || event.AsMouseEvent()->pointer_details().pointer_type == ui::EventPointerType::kPen) {
            return;
        }
    }
...
void TouchSelectionControllerImpl::QuickMenuTimerFired()
{
    ...
    if (::features::IsTouchTextEditingRedesignEnabled()) {
        if (selection_handle_1_widget_->IsClosed() || selection_handle_2_widget_->IsClosed() || cursor_handle_widget_->IsClosed()) {
            return;
        }
        handle_image_size = cursor_handle_widget_->IsVisible() ? GetCursorHandle()->GetHandleImageSize() : GetSelectionHandle1()->GetHandleImageSize();
    } else {
        handle_image_size = GetMaxHandleImageSize();
    }

    menu_runner->OpenMenu(GetWeakPtr(), menu_anchor, handle_image_size, client_view_->GetNativeView());
}
...
void TouchSelectionControllerImpl::StartQuickMenuTimer()
{
    if (quick_menu_timer_.IsRunning())
        return;
    quick_menu_timer_.Start(FROM_HERE, base::Milliseconds(kQuickMenuDelayInMs), this, &TouchSelectionControllerImpl::QuickMenuTimerFired);
}

void TouchSelectionControllerImpl::UpdateQuickMenu()
{
    HideQuickMenu();
    if (quick_menu_requested_ && GetDraggingHandle() == nullptr && !client_view_->IsSelectionDragging()) {
        StartQuickMenuTimer();
    }
}
...
gfx::Rect TouchSelectionControllerImpl::GetQuickMenuAnchorRect() const
{
    ...
    gfx::SelectionBound b2_in_screen = cursor_handle_widget_ && cursor_handle_widget_->IsVisible() ? b1_in_screen : selection_bound_2_clipped_;
...
void TouchSelectionControllerImpl::ShowMagnifier(const gfx::SelectionBound& focus_bound_in_screen)
{
    if (!::features::IsTouchTextEditingRedesignEnabled()) {
        return;
    }
    ...
    if (!touch_selection_magnifier_) {
        touch_selection_magnifier_ = std::make_unique<ui::TouchSelectionMagnifierAura>();
    }
    ...
    touch_selection_magnifier_->ShowFocusBound(root_window->layer(), focus_start, focus_end);
}
...
void TouchSelectionControllerImpl::CreateHandleWidgets()
{
    ...
    selection_handle_1_widget_ = CreateHandleWidget(client_view_->GetNativeView());
    selection_handle_1_widget_->SetContentsView(std::make_unique<EditingHandleView>(this, false));

    selection_handle_2_widget_ = CreateHandleWidget(client_view_->GetNativeView());
    selection_handle_2_widget_->SetContentsView(std::make_unique<EditingHandleView>(this, false));

    cursor_handle_widget_ = CreateHandleWidget(client_view_->GetNativeView());
    cursor_handle_widget_->SetContentsView(std::make_unique<EditingHandleView>(this, true));
}

EditingHandleView* TouchSelectionControllerImpl::GetSelectionHandle1()
{
    return selection_handle_1_widget_->IsClosed() ? nullptr : AsViewClass<EditingHandleView>(selection_handle_1_widget_->GetContentsView());
}

EditingHandleView* TouchSelectionControllerImpl::GetSelectionHandle2()
{
    return selection_handle_1_widget_->IsClosed() ? nullptr : AsViewClass<EditingHandleView>(selection_handle_2_widget_->GetContentsView());
}

EditingHandleView* TouchSelectionControllerImpl::GetCursorHandle()
{
    return selection_handle_1_widget_->IsClosed() ? nullptr : AsViewClass<EditingHandleView>(cursor_handle_widget_->GetContentsView());
}
...
gfx::SelectionBound::Type TouchSelectionControllerImpl::GetSelectionHandle1Type()
{
...
gfx::Rect TouchSelectionControllerImpl::GetExpectedHandleBounds(const gfx::SelectionBound& bound)
{
...
DEFINE_ENUM_CONVERTERS(gfx::SelectionBound::Type, { gfx::SelectionBound::Type::LEFT, u"LEFT" }, { gfx::SelectionBound::Type::RIGHT, u"RIGHT" },
    { gfx::SelectionBound::Type::CENTER, u"CENTER" }, { gfx::SelectionBound::Type::EMPTY, u"EMPTY" })
