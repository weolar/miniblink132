...
public:
    ...
    LabelButton* child_button() const
    {
        return child_button_;
    }
    LabelButton* second_child_button() const
    {
        return second_child_button_;
    }
    LabelButton* third_child_button() const
    {
        return third_child_button_;
    }
    LabelButton* not_child_button() const
    {
        return not_child_button_.get();
    }
...
TEST_F(AccessiblePaneViewTest, SimpleSetPaneFocus)
{
    ...
    EXPECT_EQ(test_view->child_button(), test_view->GetWidget()->GetFocusManager()->GetFocusedView());
...
TEST_F(AccessiblePaneViewTest, SetPaneFocusAndRestore)
{
    ...
    Widget::InitParams params_main = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    ...
    Widget::InitParams params_bar = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    ...
    EXPECT_EQ(test_view_bar->child_button(), test_view_bar->GetWidget()->GetFocusManager()->GetFocusedView());
...
TEST_F(AccessiblePaneViewTest, TwoSetPaneFocus)
{
    ...
    EXPECT_EQ(test_view_2->second_child_button(), test_view->GetWidget()->GetFocusManager()->GetFocusedView());
    widget->CloseNow();
    widget.reset();
}
...
TEST_F(AccessiblePaneViewTest, PaneFocusTraversal)
{
    ...
    widget->Init(std::move(params));
    View* root = widget->GetRootView();
    auto* original_test_view = root->AddChildView(std::make_unique<TestBarView>());
    auto* test_view = root->AddChildView(std::make_unique<TestBarView>());
    widget->Show();
    widget->Activate();

    // Set pane focus on first view.
    EXPECT_TRUE(original_test_view->SetPaneFocus(original_test_view->third_child_button()));
    ...
    EXPECT_EQ(test_view->third_child_button(), test_view->GetWidget()->GetFocusManager()->GetFocusedView());
    // left
    test_view->AcceleratorPressed(test_view->left_key());
    EXPECT_EQ(test_view->second_child_button(), test_view->GetWidget()->GetFocusManager()->GetFocusedView());
    // right, right
    ...
    EXPECT_EQ(original_test_view->third_child_button(), test_view->GetWidget()->GetFocusManager()->GetFocusedView());
    widget->CloseNow();
    widget.reset();
}
...
#define MAYBE_DoesntCrashOnEscapeWithRemovedView DISABLED_DoesntCrashOnEscapeWithRemovedView
#else
#define MAYBE_DoesntCrashOnEscapeWithRemovedView DoesntCrashOnEscapeWithRemovedView
#endif
...
TEST_F(AccessiblePaneViewTest, AccessibleProperties)
{
    ...
    EXPECT_EQ(test_view->GetViewAccessibility().GetCachedDescription(), u"Description");
    EXPECT_EQ(test_view->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kPane);
