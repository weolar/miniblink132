...
bool ViewTargeter::DoesIntersectRect(const View* target, const gfx::Rect& rect) const
{
...
ui::EventTarget* ViewTargeter::FindTargetForEvent(ui::EventTarget* root, ui::Event* event)
{
    ...
    CHECK(event->IsGestureEvent()) << "ViewTargeter does not yet support this event type.";
...
ui::EventTarget* ViewTargeter::FindNextBestTarget(ui::EventTarget* previous_target, ui::Event* event)
{
    if (!previous_target)
        return nullptr;
...
View* ViewTargeter::FindTargetForScrollEvent(View* root, const ui::ScrollEvent& scroll)
{
    gfx::Rect rect(scroll.location(), gfx::Size(1, 1));
    return root->GetEffectiveViewTargeter()->TargetForRect(root, rect);
}

View* ViewTargeter::FindTargetForGestureEvent(View* root, const ui::GestureEvent& gesture)
{
...
ui::EventTarget* ViewTargeter::FindNextBestTargetForGestureEvent(ui::EventTarget* previous_target, const ui::GestureEvent& gesture)
{
    NOTREACHED();
}
