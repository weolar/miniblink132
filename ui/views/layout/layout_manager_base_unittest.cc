...
public:
    std::vector<const View*> GetIncludedChildViews() const
    {
        ...
        base::ranges::copy_if(host_view()->children(), std::back_inserter(included), [=, this](const View* child) { return IsChildIncludedInLayout(child); });
        return included;
    }
    ...
    ProposedLayout CalculateProposedLayout(const SizeBounds& size_bounds) const override
    {
        ...
        layout.host_size.set_width(std::clamp<SizeBound>(size_bounds.width(), kMinimumSize.width(), kPreferredSize.width()).value());
        layout.host_size.set_height(std::clamp<SizeBound>(size_bounds.height(), kMinimumSize.height(), kPreferredSize.height()).value());
        return layout;
    }
...
public:
    ...
    int num_invalidations() const
    {
        return num_invalidations_;
    }
    int num_layouts_generated() const
    {
        return num_layouts_generated_;
    }
    ...
    ProposedLayout CalculateProposedLayout(const SizeBounds& size_bounds) const override
    {
        ...
        for (views::View* it : host_view()->children()) {
            ...
            const int required_height = preferred_size.height() + 2 * kChildViewPadding;
            if ((required_width <= size_bounds.width()) && (required_height <= size_bounds.height())) {
                visible = true;
                bounds = gfx::Rect(kChildViewPadding, kChildViewPadding, preferred_size.width(), preferred_size.height());
                layout.host_size.set_width(std::max(layout.host_size.width(), bounds.right() + kChildViewPadding));
                layout.host_size.set_height(std::max(layout.host_size.height(), bounds.bottom() + kChildViewPadding));
            }
...
void ExpectSameViews(const std::vector<const View*>& expected, const std::vector<const View*>& actual)
{
...
TEST(LayoutManagerBaseTest, GetPreferredHeightForWidth)
{
    ...
    TestLayoutManagerBase layout;
    EXPECT_EQ(kPreferredSize.height(), layout.GetPreferredHeightForWidth(nullptr, kWidth));
}
...
TEST(LayoutManagerBaseTest, InvalidateHost_NotInstalled)
{
    ...
    MockLayoutManagerBase* const child1 = root_layout.AddOwnedLayout(std::make_unique<MockLayoutManagerBase>());
    MockLayoutManagerBase* const child2 = root_layout.AddOwnedLayout(std::make_unique<MockLayoutManagerBase>());
    MockLayoutManagerBase* const grandchild = child1->AddOwnedLayout(std::make_unique<MockLayoutManagerBase>());
...
{
    View view;
    MockLayoutManagerBase* const root_layout = view.SetLayoutManager(std::make_unique<MockLayoutManagerBase>());
    MockLayoutManagerBase* const child1 = root_layout->AddOwnedLayout(std::make_unique<MockLayoutManagerBase>());
    MockLayoutManagerBase* const child2 = root_layout->AddOwnedLayout(std::make_unique<MockLayoutManagerBase>());
    MockLayoutManagerBase* const grandchild = child1->AddOwnedLayout(std::make_unique<MockLayoutManagerBase>());
...
public:
    void SetUp() override
    {
        ...
        layout_manager_ = host_view_->SetLayoutManager(std::make_unique<MockLayoutManagerBase>());
    }
    ...
    void DoLayoutManagerLayout()
    {
        layout_manager_->Layout(host_view_.get());
    }

    View* host_view()
    {
        return host_view_.get();
    }
    MockLayoutManagerBase* layout_manager()
    {
        return layout_manager_;
    }
    View* child(int index)
    {
        return host_view_->children().at(index);
    }
...
TEST_F(LayoutManagerBaseManagerTest, ProposedLayout_GetLayoutFor)
{
    ...
    EXPECT_EQ(&const_layout.child_layouts[0], const_layout.GetLayoutFor(child(0)));
    EXPECT_EQ(&layout.child_layouts[1], layout.GetLayoutFor(child(1)));
    EXPECT_EQ(&const_layout.child_layouts[1], const_layout.GetLayoutFor(child(1)));
...
TEST_F(LayoutManagerBaseManagerTest, GetMinimumSize)
{
    ...
    EXPECT_EQ(gfx::Size(kChildViewPadding, kChildViewPadding), host_view()->GetMinimumSize());
}
...
TEST_F(LayoutManagerBaseManagerTest, GetPreferredSize)
{
    ...
    AddChildView(kLongSize);
    AddChildView(kTallSize);
    const gfx::Size expected(kLongSize.width() + 2 * kChildViewPadding, kTallSize.height() + 2 * kChildViewPadding);
...
TEST_F(LayoutManagerBaseManagerTest, GetPreferredHeightForWidth)
{
    ...
    EXPECT_EQ(expected, layout_manager()->GetPreferredHeightForWidth(host_view(), 20));
...
TEST_F(LayoutManagerBaseManagerTest, Layout)
{
    ...
    EXPECT_EQ(1, layout_manager()->num_layouts_generated());
    EXPECT_EQ(gfx::Rect(kUpperLeft, child(0)->GetPreferredSize({})), child(0)->bounds());
    EXPECT_TRUE(child(0)->GetVisible());
    EXPECT_EQ(gfx::Rect(kUpperLeft, child(1)->GetPreferredSize({})), child(1)->bounds());
    EXPECT_TRUE(child(1)->GetVisible());
    EXPECT_EQ(gfx::Rect(kUpperLeft, child(2)->GetPreferredSize({})), child(2)->bounds());
    EXPECT_TRUE(child(2)->GetVisible());
...
TEST_F(LayoutManagerBaseManagerTest, ViewRemoved)
{
    ...
    std::unique_ptr<View> owned_child_view = host_view()->RemoveChildViewT(child_view);
...
TEST_F(LayoutManagerBaseManagerTest, IgnoredView)
{
    ...
    TestIgnoredView* ignored_view = host_view()->AddChildView(std::make_unique<TestIgnoredView>());
...
public:
    void SetUp() override
    {
        ...
        layout_ = view_->SetLayoutManager(std::make_unique<TestLayoutManagerBase>());
    }
...
TEST_F(LayoutManagerBaseAvailableSizeTest, ReturnsCorrectValues)
{
    ...
    layout()->OverrideProposedLayout({ { 10, 10 }, { { child1, true, { 1, 1, 1, 1 }, kChild1Bounds }, { child2, true, { 2, 2, 2, 2 }, kChild2Bounds } } });
...
TEST_F(LayoutManagerBaseAvailableSizeTest, AvailableSizesInNestedValuesAdd)
{
    ...
    auto* const child_layout = child->SetLayoutManager(std::make_unique<TestLayoutManagerBase>());
    ...
    layout()->OverrideProposedLayout({ kViewSize, { { child, true, { { 3, 3 }, kChildSize }, kChildAvailableSize } } });
    child_layout->OverrideProposedLayout({ kChildSize, { { grandchild, true, { { 2, 2 }, kGrandchildSize }, kGrandchildAvailableSize } } });
    view()->SizeToPreferredSize();

    EXPECT_EQ(kChildAvailableSize, view()->GetAvailableSize(child));
    SizeBounds expected;
    expected.set_width(kGrandchildAvailableSize.width() + kChildAvailableSize.width() - kChildSize.width());
    expected.set_height(kGrandchildAvailableSize.height() + kChildAvailableSize.height() - kChildSize.height());
    EXPECT_EQ(expected, child->GetAvailableSize(grandchild));
}

TEST_F(LayoutManagerBaseAvailableSizeTest, PartiallySpecifiedAvailableSizesInNestedLayoutsAddPartially)
{
    View* const child = view()->AddChildView(std::make_unique<View>());
    View* const grandchild = child->AddChildView(std::make_unique<View>());
    auto* const child_layout = child->SetLayoutManager(std::make_unique<TestLayoutManagerBase>());
...
TEST_F(LayoutManagerBaseAvailableSizeTest, MismatchedAvailableSizesInNestedLayoutsDoNotAdd)
{
    View* const child = view()->AddChildView(std::make_unique<View>());
    View* const grandchild = child->AddChildView(std::make_unique<View>());
    auto* const child_layout = child->SetLayoutManager(std::make_unique<TestLayoutManagerBase>());
...
TEST_F(LayoutManagerBaseAvailableSizeTest, AvaialbleSizeChangeTriggersDescendantLayout)
{
    View* const child = view()->AddChildView(std::make_unique<View>());
    TestLayoutManagerBase* const child_layout = child->SetLayoutManager(std::make_unique<TestLayoutManagerBase>());
    View* const grandchild = child->AddChildView(std::make_unique<View>());
    TestLayoutManagerBase* const grandchild_layout = grandchild->SetLayoutManager(std::make_unique<TestLayoutManagerBase>());
    View* const great_grandchild = grandchild->AddChildView(std::make_unique<View>());
    TestLayoutManagerBase* const great_grandchild_layout = great_grandchild->SetLayoutManager(std::make_unique<TestLayoutManagerBase>());
    ...
    grandchild_layout->OverrideProposedLayout({ {}, { { great_grandchild, false, {}, { 0, 0 } } } });

    test::RunScheduledLayout(view());

    const size_t num_grandchild_layouts = grandchild_layout->layout_count();
    const size_t num_great_grandchild_layouts = great_grandchild_layout->layout_count();
    ...
    EXPECT_EQ(num_great_grandchild_layouts, great_grandchild_layout->layout_count());
    ...
    EXPECT_EQ(num_great_grandchild_layouts + 1, great_grandchild_layout->layout_count());
}
