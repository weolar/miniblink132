...
TEST_F(FillLayoutTest, GetPreferredSizeWithInsets)
{
    ...
    SetHostInsets(gfx::Insets::TLBR(kTopInset, kLeftInset, kBottomInset, kRightInset));

    EXPECT_EQ(gfx::Size(kChildWidth + kLeftInset + kRightInset, kChildHeight + kTopInset + kBottomInset), GetPreferredSize());
}
...
TEST_F(FillLayoutTest, LayoutWithOneChild)
{
    ...
    EXPECT_EQ(gfx::Rect(0, 0, kDefaultHostWidth, kDefaultHostHeight), child->bounds());
}
...
TEST_F(FillLayoutTest, LayoutWithInsets)
{
    ...
    EXPECT_EQ(gfx::Rect(kLeftInset, kTopInset, kDefaultHostWidth - kLeftInset - kRightInset, kDefaultHostHeight - kTopInset - kBottomInset), child->bounds());
}
