...
const ChildLayout* FindChildViewInLayout(const ProposedLayout& layout, const View* view)
{
    if (!view)
        return nullptr;
...
ProposedLayout WithOnlyVisibleViews(const ProposedLayout layout)
{
    ...
    base::ranges::copy_if(layout.child_layouts, std::back_inserter(result.child_layouts), &ChildLayout::visible);
    return result;
}
...
class AnimatingLayoutManager::AnimationDelegate : public AnimationDelegateViews {
public:
    ...
    // target layout.
    bool ready_to_animate() const
    {
        return ready_to_animate_;
    }
...
private:
    ...
    base::ScopedObservation<View, ViewObserver> scoped_observation_ { &view_widget_observer_ };
};

AnimatingLayoutManager::AnimationDelegate::AnimationDelegate(AnimatingLayoutManager* layout_manager)
...
void AnimatingLayoutManager::AnimationDelegate::UpdateAnimationParameters()
{
    ...
    base::TimeDelta opacity_animation_duration = std::min(target_layout_manager_->animation_duration(), target_layout_manager_->opacity_animation_duration());
    if (!opacity_animation_duration.is_zero()) {
        fade_in_opacity_animation_ = std::make_unique<gfx::MultiAnimation>(std::vector<gfx::MultiAnimation::Part> {
            gfx::MultiAnimation::Part(target_layout_manager_->animation_duration() - opacity_animation_duration, gfx::Tween::Type::LINEAR, 0.0, 0.0),
            gfx::MultiAnimation::Part(opacity_animation_duration, target_layout_manager_->opacity_tween_type(), 0.0, 1.0) });
        fade_in_opacity_animation_->SetContainer(container_);
        fade_in_opacity_animation_->set_continuous(false);
        const base::TimeDelta fade_out_opacity_duration = target_layout_manager_->animation_duration() == opacity_animation_duration
            ? opacity_animation_duration
            : target_layout_manager_->animation_duration() - opacity_animation_duration;
        fade_out_opacity_animation_ = std::make_unique<gfx::MultiAnimation>(std::vector<gfx::MultiAnimation::Part> {
            gfx::MultiAnimation::Part(fade_out_opacity_duration, target_layout_manager_->opacity_tween_type(), 1.0, 0.0),
            gfx::MultiAnimation::Part(target_layout_manager_->animation_duration() - fade_out_opacity_duration, gfx::Tween::Type::LINEAR, 0.0, 0.0) });
...
void AnimatingLayoutManager::AnimationDelegate::Animate()
{
    ...
    Reset();
    animation_->Show();
    if (target_layout_manager_->opacity_animation_duration() > base::Milliseconds(0)) {
...
void AnimatingLayoutManager::AnimationDelegate::AnimationProgressed(const gfx::Animation* animation)
{
    DCHECK(animation_.get() == animation);
    double fade_in_opacity = fade_in_opacity_animation_.get() ? fade_in_opacity_animation_->GetCurrentValue() : 1.0;
    double fade_out_opacity = fade_out_opacity_animation_.get() ? fade_out_opacity_animation_->GetCurrentValue() : 0.0;
    target_layout_manager_->AnimateTo(animation->GetCurrentValue(), fade_in_opacity, fade_out_opacity);
}

void AnimatingLayoutManager::AnimationDelegate::AnimationCanceled(const gfx::Animation* animation)
{
    AnimationEnded(animation);
}

void AnimatingLayoutManager::AnimationDelegate::AnimationEnded(const gfx::Animation* animation)
{
...
AnimatingLayoutManager& AnimatingLayoutManager::SetBoundsAnimationMode(BoundsAnimationMode bounds_animation_mode)
{
...
AnimatingLayoutManager& AnimatingLayoutManager::SetAnimationDuration(base::TimeDelta animation_duration)
{
...
AnimatingLayoutManager& AnimatingLayoutManager::SetTweenType(gfx::Tween::Type tween_type)
{
...
AnimatingLayoutManager& AnimatingLayoutManager::SetOpacityAnimationDuration(base::TimeDelta animation_duration)
{
    DCHECK_GE(animation_duration, base::TimeDelta());
...
AnimatingLayoutManager& AnimatingLayoutManager::SetOpacityTweenType(gfx::Tween::Type tween_type)
{
...
AnimatingLayoutManager& AnimatingLayoutManager::SetOrientation(LayoutOrientation orientation)
{
...
AnimatingLayoutManager& AnimatingLayoutManager::SetDefaultFadeMode(FadeInOutMode default_fade_mode)
{
...
void AnimatingLayoutManager::FadeOut(View* child_view)
{
    ...
    // view immediately.
    const ChildLayout* const current_layout = FindChildViewInLayout(current_layout_, child_view);
    ...
    static_cast<LayoutManager*>(this)->ViewVisibilitySet(host_view(), child_view, child_view->GetVisible(), false);
}
...
void AnimatingLayoutManager::FadeIn(View* child_view)
{
    ...
    // This method is typically only called from View and has a private final
    // implementation in LayoutManagerBase so we have to cast to call it.
    static_cast<LayoutManager*>(this)->ViewVisibilitySet(host_view(), child_view, child_view->GetVisible(), true);
}
...
gfx::Size AnimatingLayoutManager::GetPreferredSize(const View* host) const
{
    ...
    case BoundsAnimationMode::kAnimateMainAxis: {
        ...
        SetCrossAxis(&result, orientation(), GetCrossAxis(orientation(), target_layout_manager()->GetPreferredSize(host)));
        return result;
    }
...
gfx::Size AnimatingLayoutManager::GetPreferredSize(const View* host, const SizeBounds& available_size) const
{
    if (!target_layout_manager()) {
        return gfx::Size();
    }
    ...
    case BoundsAnimationMode::kAnimateMainAxis: {
        // Animating only main axis, so cross axis is preferred size.
        gfx::Size result = current_layout_.host_size;
        SetCrossAxis(&result, orientation(), GetCrossAxis(orientation(), target_layout_manager()->GetPreferredSize(host, available_size)));
        return result;
    }
    case BoundsAnimationMode::kAnimateBothAxes:
        return current_layout_.host_size;
...
gfx::Size AnimatingLayoutManager::GetMinimumSize(const View* host) const
{
    ...
    case BoundsAnimationMode::kUseHostBounds:
        ...
        break;
    case BoundsAnimationMode::kAnimateMainAxis:
        SetMainAxis(&minimum_size, orientation(), std::min(GetMainAxis(orientation(), minimum_size), GetMainAxis(orientation(), current_layout_.host_size)));
        break;
    case BoundsAnimationMode::kAnimateBothAxes:
...
int AnimatingLayoutManager::GetPreferredHeightForWidth(const View* host, int width) const
{
    if (!target_layout_manager())
        return 0;
    ...
    if (bounds_animation_mode_ == BoundsAnimationMode::kAnimateBothAxes
        || (bounds_animation_mode_ == BoundsAnimationMode::kAnimateMainAxis && orientation() == LayoutOrientation::kVertical)) {
        return current_layout_.host_size.height();
    }
    return target_layout_manager()->GetPreferredHeightForWidth(host, width);
}

std::vector<raw_ptr<View, VectorExperimental>> AnimatingLayoutManager::GetChildViewsInPaintOrder(const View* host) const
{
...
bool AnimatingLayoutManager::OnViewRemoved(View* host, View* view)
{
    ...
    std::erase_if(fade_infos_, [view](const LayoutFadeInfo& fade_info) { return fade_info.child_view == view; });
    ...
    // view.
    std::erase_if(current_layout_.child_layouts, [view](const ChildLayout& child_layout) { return child_layout.child_view == view; });
...
{
    return base::BindRepeating(&AnimatingLayoutManager::DefaultFlexRuleImpl, base::Unretained(this));
}

gfx::AnimationContainer* AnimatingLayoutManager::GetAnimationContainerForTesting()
{
...
ProposedLayout AnimatingLayoutManager::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
...
bool AnimatingLayoutManager::OnViewAdded(View* host, View* view)
{
    ...
    if (IsChildIncludedInLayout(view) && cached_layout_size() && !is_animating_) {
        ...
        ProposedLayout proposed_layout = target_layout_manager()->GetProposedLayout(target_size);
...
void AnimatingLayoutManager::LayoutImpl()
{
    ...
    if (bounds_animation_mode_ == BoundsAnimationMode::kUseHostBounds) {
        ...
        if (bounds_animation_mode_ == BoundsAnimationMode::kAnimateMainAxis
            && (!cached_layout_size() || GetCrossAxis(orientation(), host_size) != GetCrossAxis(orientation(), *cached_layout_size()))) {
            ...
            const int current_main = GetMainAxis(orientation(), current_layout_.host_size);
...
void AnimatingLayoutManager::EndAnimation()
{
    ...
    // |fade_infos_|.
    for (auto fade_info : fade_infos_) {
        if (fade_info.fade_type != LayoutFadeType::kFadingOut && fade_info.child_view->layer()) {
...
bool AnimatingLayoutManager::RecalculateTarget()
{
    ...
    if (!cached_layout_size() || !animation_delegate_ || !animation_delegate_->ready_to_animate()) {
        ResetLayoutToTargetSize();
        return true;
    }
    ...
    const ProposedLayout proposed_layout = target_layout_manager()->GetProposedLayout(target_size);
...
void AnimatingLayoutManager::AnimateTo(double value, double fade_in_opacity, double fade_out_opacity)
{
    DCHECK_GE(value, 0.0);
    DCHECK_LE(value, 1.0);
    ...
    const double percent = (current_offset_ - starting_offset_) / (1.0 - starting_offset_);
...
{
    observers_.Notify(&Observer::OnLayoutIsAnimatingChanged, this, is_animating());
}
...
void AnimatingLayoutManager::PostQueuedActions()
{
    ...
    run_queued_actions_is_pending_ = base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&AnimatingLayoutManager::RunQueuedActions, weak_ptr_factory_.GetWeakPtr()));
}

void AnimatingLayoutManager::UpdateCurrentLayout(double percent, double fade_in_opacity, double fade_out_opacity)
{
    // This drops out any child view elements that don't exist in the target
    // layout. We'll add them back in later.
    current_layout_ = ProposedLayoutBetween(percent, starting_layout_, target_layout_);
    ...
    for (const LayoutFadeInfo& fade_info : fade_infos_) {
        ...
        if (percent == 1.0) {
            ...
            case LayoutFadeType::kFadingIn:
                ...
                child_layout.bounds = Denormalize(orientation(), fade_info.reference_bounds);
            ...
            const double scale_percent = fade_info.fade_type == LayoutFadeType::kFadingIn ? percent : 1.0 - percent;
            double opacity_value = fade_info.fade_type == LayoutFadeType::kFadingIn ? fade_in_opacity : fade_out_opacity;
            ...
            case FadeInOutMode::kFadeAndSlideFromTrailingEdge:
                child_layout = CalculateFadeAndSlideFade(fade_info, scale_percent, opacity_value, false);
                break;
            }
        }

        ChildLayout* const to_overwrite = FindChildViewInLayout(&current_layout_, fade_info.child_view);
        if (to_overwrite)
...
void AnimatingLayoutManager::CalculateFadeInfos()
{
    ...
    for (View* child : host_view()->children()) {
        ...
        if (index.start_visible && index.target_visible && !base::Contains(previously_fading, child)) {
    ...
    const NormalizedSize start_host_size = Normalize(orientation(), starting_layout_.host_size);
    const NormalizedSize target_host_size = Normalize(orientation(), target_layout_.host_size);
    ...
    for (View* child : host_view()->children()) {
        ...
        if (current.start_visible && !current.target_visible) {
            ...
            auto next = start_leading_edges.upper_bound(current.start_bounds.origin_main());
            if (next == start_leading_edges.end()) {
                fade_info.next_view = nullptr;
                fade_info.offsets.set_trailing(start_host_size.main() - current.start_bounds.max_main());
            } else {
                fade_info.next_view = next->second;
                fade_info.offsets.set_trailing(next->first - current.start_bounds.max_main());
            }
            if (next == start_leading_edges.begin()) {
                ...
                fade_info.offsets.set_leading(current.start_bounds.origin_main() - prev_info.start_bounds.max_main());
            }
            ...
            auto next = target_leading_edges.upper_bound(current.target_bounds.origin_main());
            if (next == target_leading_edges.end()) {
                fade_info.next_view = nullptr;
                fade_info.offsets.set_trailing(target_host_size.main() - current.target_bounds.max_main());
            } else {
                fade_info.next_view = next->second;
                fade_info.offsets.set_trailing(next->first - current.target_bounds.max_main());
            }
            if (next == target_leading_edges.begin()) {
                ...
                const auto& prev_info = child_to_info[prev->second];
                fade_info.prev_view = prev->second;
                fade_info.offsets.set_leading(current.target_bounds.origin_main() - prev_info.target_bounds.max_main());
            }
            fade_infos_.push_back(fade_info);
...
void AnimatingLayoutManager::ResolveFades()
{
    ...
    for (const LayoutFadeInfo& fade_info : fade_infos_) {
        ...
        if (fade_info.fade_type == LayoutFadeType::kFadingOut && host_view()->GetIndexOf(child).has_value() && !child->GetProperty(kViewIgnoredByLayoutKey)
            && !IsChildIncludedInLayout(child)) {
            SetViewVisibility(child, false);
        }
        if (default_fade_mode_ == FadeInOutMode::kFadeAndSlideFromTrailingEdge && fade_info.fade_type == LayoutFadeType::kFadingIn
...
ChildLayout AnimatingLayoutManager::CalculateScaleFade(const LayoutFadeInfo& fade_info, double scale_percent, bool scale_from_zero) const
{
    ChildLayout child_layout;
    ...
    if (fade_info.prev_view) {
        ...
        // null.
        const ChildLayout* const prev_layout = FindChildViewInLayout(current_layout_, fade_info.prev_view);
        leading_reference_point = Normalize(orientation(), prev_layout->bounds).max_main();
    }
    ...
    if (fade_info.next_view) {
        ...
        // should also be in the current layout. Therefore this should never return
        // null.
        const ChildLayout* const next_layout = FindChildViewInLayout(current_layout_, fade_info.next_view);
        trailing_reference_point = Normalize(orientation(), next_layout->bounds).origin_main();
    } else {
        trailing_reference_point = Normalize(orientation(), current_layout_.host_size).main();
    }
    trailing_reference_point -= fade_info.offsets.trailing();

    const int new_size = std::min(base::ClampRound(scale_percent * fade_info.reference_bounds.size_main()), trailing_reference_point - leading_reference_point);

    child_layout.child_view = fade_info.child_view;
    if (new_size > 0 && (scale_from_zero || new_size >= Normalize(orientation(), fade_info.child_view->GetMinimumSize()).main())) {
        child_layout.visible = true;
...
ChildLayout AnimatingLayoutManager::CalculateSlideFade(const LayoutFadeInfo& fade_info, double scale_percent, bool slide_from_leading) const
{
    ...
    if (slide_from_leading) {
        ...
        const ChildLayout* const leading_child = FindChildViewInLayout(*fully_faded_layout, fade_info.prev_view);

        // This is the right side of the leading control that will eclipse the
        // sliding view at the start/end of the animation.
        const int initial_trailing = Normalize(orientation(), leading_child->bounds).max_main();

        // Interpolate between initial and final trailing edge.
        const int new_trailing = gfx::Tween::IntValueBetween(scale_percent, initial_trailing, new_bounds.max_main());
        ...
        const ChildLayout* const trailing_child = FindChildViewInLayout(*fully_faded_layout, fade_info.next_view);

        // This is the left side of the trailing control that will eclipse the
        // sliding view at the start/end of the animation.
        const int initial_leading = Normalize(orientation(), trailing_child->bounds).origin_main();

        // Interpolate between initial and final leading edge.
        const int new_leading = gfx::Tween::IntValueBetween(scale_percent, initial_leading, new_bounds.origin_main());
...
ChildLayout AnimatingLayoutManager::CalculateFadeAndSlideFade(
    const LayoutFadeInfo& fade_info, double scale_percent, double opacity_value, bool slide_from_leading) const
{
    ...
    if (!slide_from_leading) {
        ...
        const ChildLayout* const trailing_child = FindChildViewInLayout(*fully_faded_layout, fade_info.next_view);
        const int host_trailing = Normalize(orientation(), fully_faded_layout->host_size).main();
        int leading_bound = !trailing_child ? host_trailing : Normalize(orientation(), trailing_child->bounds).origin_main();
        // Interpolate between initial and final leading edge.
        const int new_leading = gfx::Tween::IntValueBetween(scale_percent, leading_bound, new_bounds.origin_main());
...
gfx::Size AnimatingLayoutManager::GetAvailableTargetLayoutSize()
{
    ...
    const gfx::Size preferred_size = target_layout_manager()->GetPreferredSize(host_view());

    int width;

    if (orientation() == LayoutOrientation::kVertical && bounds_animation_mode_ == BoundsAnimationMode::kAnimateMainAxis) {
    ...
    if (orientation() == LayoutOrientation::kHorizontal && bounds_animation_mode_ == BoundsAnimationMode::kAnimateMainAxis) {
        height = host_view()->height();
    } else {
        height = width < preferred_size.width() ? target_layout_manager()->GetPreferredHeightForWidth(host_view(), width) : preferred_size.height();
...
// static
gfx::Size AnimatingLayoutManager::DefaultFlexRuleImpl(const AnimatingLayoutManager* animating_layout, const View* view, const SizeBounds& size_bounds)
{
    ...
    if (!preferred_size.IsEmpty() && CanFitInBounds(preferred_size, size_bounds)) {
    ...
    const LayoutManagerBase* const target_layout = animating_layout->target_layout_manager();
    ...
    if (size_bounds.width().is_bounded() && size_bounds.height().is_bounded()) {
        ...
        size = gfx::Size(width, target_layout->GetPreferredHeightForWidth(view, width));
    } else {
