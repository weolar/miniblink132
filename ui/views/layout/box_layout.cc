...
struct BoxChildData {
    ...
    {
        return base::StrCat({ "{ preferred ", preferred_size.ToString(), " margins ", margins.ToString(), " bounds ", actual_bounds.ToString(), " }" });
    }
...
struct BoxLayout::BoxLayoutData {
    ...
    std::string ToString() const
    {
        std::string result = base::StrCat(
            { "{ host_size ", total_size.ToString(), ", cross_center_pos ", base::NumberToString(cross_center_pos), "\n", layout.ToString(), " {\n" });
        bool first = true;
...
BoxLayoutFlexSpecification BoxLayoutFlexSpecification::WithWeight(int weight) const
{
    DCHECK_GE(weight, 0);
...
BoxLayoutFlexSpecification BoxLayoutFlexSpecification::UseMinSize(bool use_min_size) const
{
    BoxLayoutFlexSpecification spec = *this;
...
BoxLayout::BoxLayout(BoxLayout::Orientation orientation, const gfx::Insets& inside_border_insets, int between_child_spacing, bool collapse_margins_spacing)
...
void BoxLayout::set_cross_axis_alignment(CrossAxisAlignment cross_axis_alignment)
{
...
void BoxLayout::SetFlexForView(const View* view, int flex_weight, bool use_min_size)
{
    DCHECK(view);
    DCHECK_EQ(host_view(), view->parent());
    DCHECK_GE(flex_weight, 0);
    const_cast<View*>(view)->SetProperty(kBoxLayoutFlexKey, BoxLayoutFlexSpecification().WithWeight(flex_weight).UseMinSize(use_min_size));
}
...
ProposedLayout BoxLayout::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
    BoxLayoutData data;
    ...
    if (orientation_ == Orientation::kVertical && cross_axis_alignment_ == CrossAxisAlignment::kStretch && !size_bounds.width().is_bounded()) {
    ...
    if (collapse_margins_spacing_) {
        Inset1D main_axis_inset(data.child_data.empty() ? 0 : data.child_data.front().margins.main_leading(),
            data.child_data.empty() ? 0 : data.child_data.back().margins.main_trailing());
        data.host_insets = Normalize(orientation_, insets + Denormalize(orientation_, NormalizedInsets(main_axis_inset, Inset1D())));
    } else {
...
int BoxLayout::GetMinimumSizeForView(const View* view) const
{
    ...
    return (orientation_ == Orientation::kHorizontal) ? view->GetMinimumSize().width() : view->GetMinimumSize().height();
}

gfx::Size BoxLayout::GetPreferredSizeForView(const View* view, const NormalizedSizeBounds& size_bounds) const
{
...
void BoxLayout::EnsureCrossSize(BoxLayoutData& data) const
{
    if (minimum_cross_axis_size_ > data.total_size.cross()) {
        if (cross_axis_alignment_ == CrossAxisAlignment::kCenter) {
            data.cross_center_pos += (minimum_cross_axis_size_ - data.total_size.cross()) / 2;
        }
...
SizeBound BoxLayout::CalculateMaxChildWidth(BoxLayoutData& data) const
{
    ...
    for (size_t i = 0; i < data.num_children(); ++i) {
        ...
        gfx::Size child_size = child_layout.child_view->GetPreferredSize({ /* Unbounded */ });
        ...
        if (cross_axis_alignment_ == CrossAxisAlignment::kStart) {
            width = std::max<SizeBound>(width, child_size.width() + child_margins.cross_trailing());
        } else if (cross_axis_alignment_ == CrossAxisAlignment::kEnd) {
            width = std::max<SizeBound>(width, child_size.width() + child_margins.cross_leading());
        } else {
            ...
            gfx::Rect child_bounds = gfx::Rect(-(child_size.width() / 2), 0, child_size.width(), child_size.height());
            child_bounds.Inset(gfx::Insets::TLBR(0, -child_margins.cross_leading(), 0, -child_margins.cross_trailing()));
    ...
    return std::max<SizeBound>(width + extra_cross_margin, minimum_cross_axis_size_);
}

void BoxLayout::CalculatePreferredSize(const SizeBounds& bounds, BoxLayoutData& data) const
{
    if (orientation_ == Orientation::kVertical) {
        for (size_t i = 0; i < data.num_children(); ++i) {
            BoxChildData& box_child = data.child_data[i];
            ChildLayout& child_layout = data.layout.child_layouts[i];
            SizeBound available_width = std::max<SizeBound>(0, bounds.width() - box_child.margins.cross_size());

            box_child.preferred_size = Normalize(orientation_, child_layout.child_view->GetPreferredSize(SizeBounds(available_width, bounds.height())));
        }
    } else {
        for (size_t i = 0; i < data.num_children(); ++i) {
            BoxChildData& box_child = data.child_data[i];
            ...
            box_child.preferred_size = Normalize(orientation_, child_layout.child_view->GetPreferredSize(bounds));
        }
    }
}

void BoxLayout::UpdateChildMarginsIfCollapseMarginsSpacing(BoxLayoutData& data) const
{
    if (!collapse_margins_spacing_) {
        return;
    }
    ...
    for (size_t i = 0; i < num_child; ++i) {
        BoxChildData& box_child = data.child_data[i];
        int prev_trailing = i == 0 ? data.interior_margin.main_leading() : data.child_data[i - 1].margins.main_trailing();
        int next_leading = i == num_child - 1 ? data.interior_margin.main_trailing() : data.child_data[i + 1].margins.main_leading();
        box_child.margins = NormalizedInsets(std::max(box_child.margins.main_leading(), prev_trailing),
            std::max(box_child.margins.cross_leading(), data.interior_margin.cross_leading()), std::max(box_child.margins.main_trailing(), next_leading),
            std::max(box_child.margins.cross_trailing(), data.interior_margin.cross_trailing()));
    }
}
...
void BoxLayout::CalculatePreferredTotalSize(BoxLayoutData& data) const
{
    for (size_t i = 0; i < data.num_children(); ++i) {
        ...
        if (i < data.num_children() - 1) {
            if (collapse_margins_spacing_) {
                main_size += std::max(between_child_spacing_, child_margins.main_trailing());
            } else {
        ...
        if (cross_axis_alignment_ == CrossAxisAlignment::kStart) {
            cross_size += data.max_cross_margin.leading() + child_margins.cross_trailing();
        } else if (cross_axis_alignment_ == CrossAxisAlignment::kEnd) {
            cross_size += data.max_cross_margin.trailing() + child_margins.cross_leading();
        } else {
            ...
            data.cross_center_pos = std::max(data.cross_center_pos, child_margins.cross_leading() + view_center);
            cross_size = data.cross_center_pos + box_child.preferred_size.cross() - view_center + child_margins.cross_trailing();
            ...
            data.total_size.Enlarge(0, std::max(0, data.cross_center_pos - old_cross_center_pos));
        }
        data.total_size.SetSize(data.total_size.main() + main_size, std::max(data.total_size.cross(), cross_size));
    }

    EnsureCrossSize(data);
}

void BoxLayout::UpdateFlexLayout(const NormalizedSizeBounds& bounds, BoxLayoutData& data) const
{
    ...
    int flex_sum = std::accumulate(data.child_data.cbegin(), data.child_data.cend(), 0, [](int total, const BoxChildData& data) { return total + data.flex; });
    ...
    for (size_t i = 0; i < num_child; ++i) {
        ...
        SizeBound cross_axis_size = bounds.cross().is_bounded() && bounds.cross().value() > 0 ? bounds.cross() : preferred_cross;
        if (cross_axis_alignment_ == CrossAxisAlignment::kStretch || cross_axis_alignment_ == CrossAxisAlignment::kCenter) {
        ...
        box_child.actual_bounds.set_size_main(
            std::max(child_min_size, GetActualMainSizeAndUpdateChildPreferredSizeIfNeeded(bounds, data, i, current_padding, cross_axis_size)));

        if (box_child.actual_bounds.size_main() > 0 || box_child.flex > 0) {
            data.total_size.set_main(box_child.actual_bounds.max_main());
            if (i < num_child - 1) {
                if (collapse_margins_spacing_) {
                    data.total_size.Enlarge(std::max(between_child_spacing_, child_margins.main_trailing()), 0);
                } else {
    ...
    const SizeBound cross_total_size = bounds.cross().is_bounded() && bounds.cross().value() > 0 ? bounds.cross() : data.total_size.cross();
    if (cross_axis_alignment_ == CrossAxisAlignment::kCenter) {
        const int center_offset = std::floor((cross_total_size.value() - data.total_size.cross()) / 2.0);
        for (BoxChildData& child : data.child_data) {
            const SizeBound cross_axis_size = cross_total_size - child.margins.cross_size();

            child.actual_bounds.set_size_cross(std::min(cross_axis_size.value(), child.preferred_size.cross()));
            int view_center = child.actual_bounds.size_cross() / 2 + (child.actual_bounds.size_cross() & 1);
            child.actual_bounds.set_origin_cross(std::max(0, data.cross_center_pos - view_center + center_offset));
        }
    } else {
        for (BoxChildData& child : data.child_data) {
            const SizeBound cross_axis_size = cross_total_size - child.margins.cross_size();
            ...
            Span new_cross(0, std::min(cross_axis_size.value(), child.preferred_size.cross()));
...
int BoxLayout::GetActualMainSizeAndUpdateChildPreferredSizeIfNeeded(
    const NormalizedSizeBounds& bounds, BoxLayoutData& data, size_t index, int current_padding, SizeBound cross_axis_size) const
{
    BoxChildData& box_child = data.child_data[index];
    ChildLayout& child_layout = data.layout.child_layouts[index];

    SizeBound avaible_main_size = std::max<SizeBound>(0, bounds.main() - data.total_size.main());
    ...
    bool need_shrink = current_padding < 0 || avaible_main_size < box_child.preferred_size.main();
    if (need_shrink) {
        avaible_main_size = std::max<SizeBound>(0, avaible_main_size.min_of(box_child.preferred_size.main() + current_padding));

        // Calculate the preferred size given the current size.
        box_child.preferred_size
            = Normalize(orientation_, GetPreferredSizeForView(child_layout.child_view, NormalizedSizeBounds(avaible_main_size, cross_axis_size)));
        return box_child.flex > 0 ? avaible_main_size.value() : box_child.preferred_size.main();
    } else {
        return box_child.preferred_size.main() + current_padding;
    }
}

void BoxLayout::CalculateChildBounds(const SizeBounds& size_bounds, BoxLayoutData& data) const
{
    // Apply main axis alignment (we've already done cross-axis alignment above).
    const NormalizedSizeBounds normalized_bounds = Normalize(orientation_, size_bounds);
    const NormalizedSize normalized_host_size = Normalize(orientation_, data.layout.host_size);
    ...
    for (size_t i = 0; i < data.num_children(); ++i) {
        ...
        if (actual.origin_main() < data.host_insets.main_leading()) {
            actual.SetByBounds(data.host_insets.main_leading(), actual.origin_cross(), actual.max_main(), actual.max_cross());
        }

        if (actual.max_main() > data.host_insets.main_leading() + available_main) {
            actual.SetByBounds(actual.origin_main(), actual.origin_cross(), data.host_insets.main_leading() + available_main, actual.max_cross());
        }
