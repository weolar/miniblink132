...
public:
    void SetUp() override
    {
        host_ = std::make_unique<FlexLayoutView>();
    }
...
TEST_F(FlexLayoutViewTest, LayoutInvalidationWhenPropertyChanged)
{
    ...
    EXPECT_NE(include_host_insets_in_layout, host()->GetIncludeHostInsetsInLayout());
    host()->SetIncludeHostInsetsInLayout(include_host_insets_in_layout);
    reset_layout();

    constexpr bool ignore_default_main_axis_margins = true;
    EXPECT_NE(ignore_default_main_axis_margins, host()->GetIgnoreDefaultMainAxisMargins());
    host()->SetIgnoreDefaultMainAxisMargins(ignore_default_main_axis_margins);
    reset_layout();

    constexpr FlexAllocationOrder flex_allocation_order = FlexAllocationOrder::kReverse;
...
TEST_F(FlexLayoutViewTest, NoLayoutInvalidationWhenPropertyUnchanged)
{
    ...
    host()->SetIgnoreDefaultMainAxisMargins(host()->GetIgnoreDefaultMainAxisMargins());
