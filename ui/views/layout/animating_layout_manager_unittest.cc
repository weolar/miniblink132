...
gfx::Size ConstrainSizeToBounds(const gfx::Size& size, const SizeBounds& bounds)
{
...
public:
    ...
    void SetMinimumSize(gfx::Size minimum_size)
    {
    ...
    gfx::Size CalculatePreferredSize(const SizeBounds& available_size) const override
    {
        ...
        if (fix_area_) {
            ...
            const int width = std::max(min_width, available_size.width().value_or(preferred_size.width()));
            const int height = preferred_size.height() * preferred_size.width() / std::max(1, width);
...
protected:
    ProposedLayout CalculateProposedLayout(const SizeBounds& size_bounds) const override
    {
...
public:
    ...
    gfx::Size GetPreferredSize(const View* host, const SizeBounds& available_size) const override
    {
        if (host->children().empty()) {
            return gfx::Size();
        }
...
public:
    ...
    void OnLayoutIsAnimatingChanged(AnimatingLayoutManager* source, bool is_animating) override
    {
        events_.push_back(is_animating);
    }

    const std::vector<bool> events() const
    {
        return events_;
    }
...
private:
    ...
    base::ScopedObservation<AnimatingLayoutManager, Observer> scoped_observation_ { this };
};
...
public:
    ...
    void SetUp() override
    {
        render_mode_lock_ = gfx::AnimationTestApi::SetRichAnimationRenderMode(
            enable_animations_ ? gfx::Animation::RichAnimationRenderMode::FORCE_ENABLED : gfx::Animation::RichAnimationRenderMode::FORCE_DISABLED);
        ...
        if (UseContainerTestApi()) {
            container_test_api_ = std::make_unique<gfx::AnimationContainerTestApi>(layout()->GetAnimationContainerForTesting());
        }

        // These can't be constructed statically since they depend on the child
        // views.
        layout1_ = { { 100, 100 },
            { { children_[0].get(), true, { 5, 5, 10, 10 } }, { children_[1].get(), false }, { children_[2].get(), true, { 20, 20, 20, 20 } } } };
        layout2_ = { { 200, 200 },
            { { children_[0].get(), true, { 10, 20, 20, 30 } }, { children_[1].get(), false }, { children_[2].get(), true, { 10, 100, 10, 10 } } } };
    }
    ...
    TestView* child(size_t index) const
    {
    ...
    const ProposedLayout& layout1() const
    {
        return layout1_;
    }
    const ProposedLayout& layout2() const
    {
        return layout2_;
    }
    ...
    {
        base::RunLoop loop;
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, loop.QuitClosure());
        loop.Run();
    }
    ...
    void ReplaceChild(int index)
    {
        ...
        delete old_view;
        children_[index] = view_->AddChildViewAt(std::make_unique<TestView>(), index);
    }
    ...
    void EnsureLayout(const ProposedLayout& expected, const char* message = "")
    {
        for (size_t i = 0; i < expected.child_layouts.size(); ++i) {
            ...
            EXPECT_EQ(view_, child->parent()) << " view " << i << " parent differs " << message;
            EXPECT_EQ(expected_child.visible, child->GetVisible()) << " view " << i << " visibility " << message;
            if (expected_child.visible) {
                EXPECT_EQ(expected_child.bounds, child->bounds()) << " view " << i << " bounds " << message;
            }
        }
    }
    ...
    virtual bool UseContainerTestApi() const
    {
        return true;
    }
...
const FlexSpecification AnimatingLayoutManagerTest::kDropOut
    = FlexSpecification(MinimumFlexSizeRule::kPreferredSnapToZero, MaximumFlexSizeRule::kPreferred).WithWeight(0);

const FlexSpecification AnimatingLayoutManagerTest::kFlex = FlexSpecification(MinimumFlexSizeRule::kScaleToZero, MaximumFlexSizeRule::kUnbounded).WithOrder(2);
...
TEST_F(AnimatingLayoutManagerTest, SetLayoutManager_NoAnimation)
{
    ...
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
...
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const test_layout = layout()->SetTargetLayoutManager(std::make_unique<TestLayoutManager>());
    test_layout->SetLayout(layout1());
    layout()->ResetLayout();
...
TEST_F(AnimatingLayoutManagerTest, HostInvalidate_AnimateBounds_AnimationProgresses)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const test_layout = layout()->SetTargetLayoutManager(std::make_unique<TestLayoutManager>());
    test_layout->SetLayout(layout1());
...
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kUseHostBounds);
    auto* const test_layout = layout()->SetTargetLayoutManager(std::make_unique<TestLayoutManager>());
    test_layout->SetLayout(layout1());
    SizeAndLayout();
    layout()->ResetLayout();
...
TEST_F(AnimatingLayoutManagerTest, HostInvalidate_NoAnimateBounds_NewLayoutTriggersAnimation)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kUseHostBounds);
    auto* const test_layout = layout()->SetTargetLayoutManager(std::make_unique<TestLayoutManager>());
    test_layout->SetLayout(layout1());
...
TEST_F(AnimatingLayoutManagerTest, HostInvalidate_NoAnimateBounds_AnimationProgresses)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kUseHostBounds);
    auto* const test_layout = layout()->SetTargetLayoutManager(std::make_unique<TestLayoutManager>());
    test_layout->SetLayout(layout1());
...
TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_MiddleView_ScaleFromZero)
{
    const ProposedLayout initial_layout { { 50, 20 },
        { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 20, 5, 10, 10 } }, { child(2), true, { 35, 5, 10, 10 } } } };
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    layout()->SetDefaultFadeMode(AnimatingLayoutManager::FadeInOutMode::kScaleFromZero);
    ...
    const ProposedLayout final_layout { { 35, 20 }, { { child(0), true, { 5, 5, 10, 10 } }, { child(1), false }, { child(2), true, { 20, 5, 10, 10 } } } };
    ...
    animation_api()->IncrementTime(base::Milliseconds(250));
    SizeAndLayout();
    ProposedLayout expected = ProposedLayoutBetween(0.25, initial_layout, final_layout);
    DCHECK_EQ(expected.child_layouts[1].child_view, child(1));
    expected.child_layouts[1].visible = true;
    expected.child_layouts[1].bounds = { expected.child_layouts[0].bounds.right() + 5, initial_layout.child_layouts[1].bounds.y(),
        expected.child_layouts[2].bounds.x() - expected.child_layouts[0].bounds.right() - 10, initial_layout.child_layouts[1].bounds.height() };
    EXPECT_TRUE(layout()->is_animating());
    EXPECT_EQ(expected.host_size, view()->size());
    EnsureLayout(expected);
...
TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_MiddleView_ScaleFromMinimum)
{
    const ProposedLayout initial_layout { { 50, 20 },
        { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 20, 5, 10, 10 } }, { child(2), true, { 35, 5, 10, 10 } } } };
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    layout()->SetDefaultFadeMode(AnimatingLayoutManager::FadeInOutMode::kScaleFromMinimum);
    layout()->SetOrientation(LayoutOrientation::kHorizontal);
    auto* const test_layout = layout()->SetTargetLayoutManager(std::make_unique<TestLayoutManager>());
    test_layout->SetLayout(initial_layout);
    layout()->ResetLayout();
...
TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_LeadingView_ScaleFromMinimum)
{
    ...
    const ProposedLayout final_layout { { 35, 20 }, { { child(0), false }, { child(1), true, { 5, 5, 10, 10 } }, { child(2), true, { 20, 5, 10, 10 } } } };
    ...
    expected.child_layouts[0].bounds
        = { 5, initial_layout.child_layouts[0].bounds.y(), expected.child_layouts[1].bounds.x() - 10, initial_layout.child_layouts[0].bounds.height() };
    EXPECT_TRUE(layout()->is_animating());
    EXPECT_EQ(expected.host_size, view()->size());
    EnsureLayout(expected);
...
TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_TrailingView_ScaleFromMinimum_FadeIn)
{
    const ProposedLayout initial_layout { { 35, 20 }, { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 20, 5, 10, 10 } }, { child(2), false } } };
    ...
    animation_api()->IncrementTime(base::Milliseconds(500));
    SizeAndLayout();
    ProposedLayout expected = ProposedLayoutBetween(0.5, initial_layout, final_layout);
    // At this point the layout is still animating but the middle view is below
    ...
    expected.child_layouts[2].bounds = { expected.child_layouts[1].bounds.right() + 5, final_layout.child_layouts[2].bounds.y(),
        expected.host_size.width() - expected.child_layouts[1].bounds.right() - 10, final_layout.child_layouts[2].bounds.height() };
    EXPECT_TRUE(layout()->is_animating());
    EXPECT_EQ(expected.host_size, view()->size());
    EnsureLayout(expected);
...
TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_TrailingView_ScaleFromMinimum)
{
    const ProposedLayout initial_layout { { 50, 20 },
        { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 20, 5, 10, 10 } }, { child(2), true, { 35, 5, 10, 10 } } } };
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    ...
    const ProposedLayout final_layout { { 35, 20 }, { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 20, 5, 10, 10 } }, { child(2), false } } };
    ...
    DCHECK_EQ(expected.child_layouts[2].child_view, child(2));
    expected.child_layouts[2].visible = true;
    expected.child_layouts[2].bounds = { expected.child_layouts[1].bounds.right() + 5, initial_layout.child_layouts[2].bounds.y(),
        expected.host_size.width() - expected.child_layouts[1].bounds.right() - 10, initial_layout.child_layouts[2].bounds.height() };
    EXPECT_TRUE(layout()->is_animating());
    EXPECT_EQ(expected.host_size, view()->size());
    EnsureLayout(expected);
...
TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_TrailingView_ScaleFromMinimum_Vertical)
{
    const ProposedLayout initial_layout { { 20, 50 },
        { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 5, 20, 10, 10 } }, { child(2), true, { 5, 35, 10, 10 } } } };
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    layout()->SetDefaultFadeMode(AnimatingLayoutManager::FadeInOutMode::kScaleFromMinimum);
    ...
    const ProposedLayout final_layout { { 20, 35 }, { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 5, 20, 10, 10 } }, { child(2), false } } };
    ...
    DCHECK_EQ(expected.child_layouts[2].child_view, child(2));
    expected.child_layouts[2].visible = true;
    expected.child_layouts[2].bounds = { initial_layout.child_layouts[2].bounds.x(), expected.child_layouts[1].bounds.bottom() + 5,
        initial_layout.child_layouts[2].bounds.width(), expected.host_size.height() - expected.child_layouts[1].bounds.bottom() - 10 };
    EXPECT_TRUE(layout()->is_animating());
    EnsureLayout(expected);
...
TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_Hide_HidesViewDuringAnimation)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kUseHostBounds);
    layout()->SetDefaultFadeMode(AnimatingLayoutManager::FadeInOutMode::kHide);
    layout()->SetOrientation(LayoutOrientation::kVertical);
    FlexLayout* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
    ...
    // Sanity check...
    const ProposedLayout initial_layout { { 20, 50 }, { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 5, 20, 10, 10 } }, { child(2), false } } };
    EnsureLayout(initial_layout);
    ...
    const ProposedLayout middle_layout { { 20, 35 }, { { child(0), true, { 5, 5, 10, 10 } }, { child(1), false }, { child(2), false } } };
    EnsureLayout(middle_layout);

    animation_api()->IncrementTime(base::Milliseconds(500));
    test::RunScheduledLayout(view());
    const ProposedLayout final_layout { { 20, 35 }, { { child(0), true, { 5, 5, 10, 10 } }, { child(1), false }, { child(2), true, { 5, 20, 10, 10 } } } };
    EnsureLayout(final_layout);
}

TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_Hide_HidesViewDuringAnimation_OneFrame)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kUseHostBounds);
    layout()->SetDefaultFadeMode(AnimatingLayoutManager::FadeInOutMode::kHide);
    layout()->SetOrientation(LayoutOrientation::kVertical);
...
TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_Hide_AnimationResetDuringHide)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kUseHostBounds);
    layout()->SetDefaultFadeMode(AnimatingLayoutManager::FadeInOutMode::kHide);
    layout()->SetOrientation(LayoutOrientation::kVertical);
...
TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_SlideFromLeading_LastView)
{
    ...
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    layout()->SetDefaultFadeMode(AnimatingLayoutManager::FadeInOutMode::kSlideFromLeadingEdge);
    layout()->SetOrientation(LayoutOrientation::kHorizontal);
    auto* const test_layout = layout()->SetTargetLayoutManager(std::make_unique<TestLayoutManager>());
    test_layout->SetLayout(initial_layout);
    layout()->ResetLayout();
...
{
    const ProposedLayout initial_layout { { 20, 35 }, { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 5, 20, 10, 10 } }, { child(2), false } } };
...
{
    const ProposedLayout initial_layout { { 50, 20 },
        { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 20, 5, 5, 10 } }, { child(2), true, { 35, 5, 10, 10 } } } };
...
TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_SlideFromLeading_LeadingView_SlidesFromTrailing)
{
    const ProposedLayout initial_layout { { 50, 20 },
        { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 20, 5, 5, 10 } }, { child(2), true, { 35, 5, 10, 10 } } } };

    const ProposedLayout final_layout { { 35, 20 }, { { child(0), false }, { child(1), true, { 5, 5, 5, 10 } }, { child(2), true, { 20, 5, 10, 10 } } } };
...
TEST_F(AnimatingLayoutManagerTest, FadeInOutMode_SlideFromTrailing_MiddleView)
{
    ...
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    layout()->SetDefaultFadeMode(AnimatingLayoutManager::FadeInOutMode::kSlideFromTrailingEdge);
    layout()->SetOrientation(LayoutOrientation::kHorizontal);
    auto* const test_layout = layout()->SetTargetLayoutManager(std::make_unique<TestLayoutManager>());
    test_layout->SetLayout(initial_layout);
    layout()->ResetLayout();
...
TEST_F(AnimatingLayoutManagerTest, FlexLayout_FadeOutOnVisibilitySet)
{
    ...
    layout()->SetOrientation(LayoutOrientation::kHorizontal);
    layout()->SetDefaultFadeMode(AnimatingLayoutManager::FadeInOutMode::kScaleFromZero);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
    ...
    child(1)->SetProperty(kFlexBehaviorKey, FlexSpecification(MinimumFlexSizeRule::kPreferred, MaximumFlexSizeRule::kUnbounded));

    const ProposedLayout expected_start { { 50, 20 },
        { { child(0), true, { { 5, 5 }, kChildViewSize } }, { child(1), true, { { 20, 5 }, kChildViewSize } },
            { child(2), true, { { 35, 5 }, kChildViewSize } } } };

    const ProposedLayout expected_end { { 50, 20 },
        { { child(0), false }, { child(1), true, { 5, 5, 25, 10 } }, { child(2), true, { { 35, 5 }, kChildViewSize } } } };
    ...
    test::RunScheduledLayout(view());
    EXPECT_TRUE(layout()->is_animating());
    ProposedLayout expected = ProposedLayoutBetween(0.5, expected_start, expected_end);
    expected.child_layouts[0].visible = true;
    expected.child_layouts[0].bounds = expected_start.child_layouts[0].bounds;
    expected.child_layouts[0].bounds.set_width(expected.child_layouts[1].bounds.x() - 10);
    EnsureLayout(expected);
...
TEST_F(AnimatingLayoutManagerTest, FlexLayout_AnimateOutOnDescendentVisibilitySet)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kUseHostBounds);
    layout()->SetOrientation(LayoutOrientation::kHorizontal);
...
// Regression test for issues: crbug.com/1021332, crbug.com/1003500
TEST_F(AnimatingLayoutManagerTest, FlexLayout_AnimateInOnDescendentVisibilitySet)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kUseHostBounds);
    layout()->SetOrientation(LayoutOrientation::kHorizontal);
...
TEST_F(AnimatingLayoutManagerTest, FlexLayout_DoubleSlide)
{
    ...
    const ProposedLayout expected_start { { 20, 10 },
        { { child(0), true, { { 0, 0 }, kChildViewSize } }, { child(1), false }, { child(2), true, { { 10, 0 }, kChildViewSize } } } };
    ...
    const ProposedLayout expected_middle { { 20, 10 },
        { { child(0), true, { { 5, 0 }, kChildViewSize } }, { child(1), true, { { 5, 0 }, kChildViewSize } },
    ...
    EXPECT_FALSE(layout()->is_animating());
    const ProposedLayout expected_end { { 20, 10 },
        { { child(0), false }, { child(1), true, { { 0, 0 }, kChildViewSize } }, { child(2), true, { { 10, 0 }, kChildViewSize } } } };
...
TEST_F(AnimatingLayoutManagerTest, FlexLayout_PostDelayedActionAfterFadeIn_AnimateNewViewIn)
{
    child(0)->SetVisible(false);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    layout()->SetOrientation(LayoutOrientation::kHorizontal);
    ...
    layout()->PostOrQueueAction(base::BindOnce([](bool* var) { *var = true; }, &action_run));
...
// PostOrQueueAction does not delay an action after FadeIn is called.
TEST_F(AnimatingLayoutManagerTest, FlexLayout_PostDelayedActionAfterFadeIn_SwapTwoViews)
{
    child(0)->SetVisible(false);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kUseHostBounds);
    layout()->SetOrientation(LayoutOrientation::kHorizontal);
...
TEST_F(AnimatingLayoutManagerTest, FlexLayout_PostDelayedActionAfterFadeIn_FadeInHiddenView)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kUseHostBounds);
    layout()->SetOrientation(LayoutOrientation::kHorizontal);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
{
    const ProposedLayout initial_layout { { 35, 20 }, { { child(0), true, { 5, 5, 10, 10 } }, { child(1), true, { 20, 5, 10, 10 } }, { child(2), false } } };
    const ProposedLayout final_layout { { 20, 20 }, { { child(0), true, { 5, 5, 10, 10 } }, { child(1), false }, { child(2), false } } };
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    layout()->SetDefaultFadeMode(AnimatingLayoutManager::FadeInOutMode::kSlideFromLeadingEdge);
    layout()->SetOrientation(LayoutOrientation::kHorizontal);
    auto* const test_layout = layout()->SetTargetLayoutManager(std::make_unique<TestLayoutManager>());
...
TEST_F(AnimatingLayoutManagerTest, FlexLayout_FadeInOnAdded)
{
    ...
    const ProposedLayout expected_start { { 50, 20 }, { { child(1), true, { 5, 5, 25, 10 } }, { child(2), true, { { 35, 5 }, kChildViewSize } } } };
...
TEST_F(AnimatingLayoutManagerTest, FlexLayout_FadeOut_NoCrashOnRemove)
{
    ...
    const ProposedLayout after_remove { { 50, 20 }, { { child(1), true, { { 20, 5 }, kChildViewSize } }, { child(2), true, { { 35, 5 }, kChildViewSize } } } };

    const ProposedLayout expected_end { { 50, 20 }, { { child(1), true, { 5, 5, 25, 10 } }, { child(2), true, { { 35, 5 }, kChildViewSize } } } };
    ...
    test::RunScheduledLayout(view());
    EXPECT_TRUE(layout()->is_animating());
    ProposedLayout expected = ProposedLayoutBetween(0.5, after_remove, expected_end);
    EnsureLayout(expected);
...
TEST_F(AnimatingLayoutManagerTest, FlexLayout_FadeOut_IgnoreChildView)
{
    ...
    const ProposedLayout expected_end { { 50, 20 },
        { { child(0), true, { { 5, 5 }, kChildViewSize } }, { child(1), true, { 5, 5, 25, 10 } }, { child(2), true, { { 35, 5 }, kChildViewSize } } } };
...
TEST_F(AnimatingLayoutManagerTest, FlexLayout_SlideAfterViewHidden)
{
    ...
    test::RunScheduledLayout(view());
    EXPECT_TRUE(layout()->is_animating());
    ProposedLayout expected = ProposedLayoutBetween(0.0, expected_start, expected_end);
    EnsureLayout(expected);
...
TEST_F(AnimatingLayoutManagerTest, FlexLayout_RedirectAnimation)
{
    ...
    const ProposedLayout expected_end1 { { 50, 20 },
        { { child(0), false }, { child(1), true, { 5, 5, 25, 10 } }, { child(2), true, { { 35, 5 }, kChildViewSize } } } };

    const ProposedLayout expected_end2 { { 50, 20 }, { { child(0), false }, { child(1), true, { 5, 5, 40, 10 } }, { child(2), false } } };
    ...
    test::RunScheduledLayout(view());
    EXPECT_TRUE(layout()->is_animating());
    ProposedLayout expected = ProposedLayoutBetween(0.5, expected_start, expected_end1);
    EnsureLayout(expected);
...
TEST_F(AnimatingLayoutManagerTest, FlexLayout_ResetAnimation)
{
    ...
    test::RunScheduledLayout(view());
    EXPECT_TRUE(layout()->is_animating());
    ProposedLayout expected = ProposedLayoutBetween(0.9, expected_start, expected_end1);
    EnsureLayout(expected);
...
TEST_F(AnimatingLayoutManagerTest, PostOrQueueAction)
{
    ...
    // Invalidating the layout forces a recalculation, which starts the animation.
    view()->InvalidateLayout();
    layout()->PostOrQueueAction(base::BindOnce([](bool* var) { *var = true; }, &action1_called));
    layout()->PostOrQueueAction(base::BindOnce([](bool* var) { *var = true; }, &action2_called));
    EXPECT_TRUE(layout()->is_animating());
...
TEST_F(AnimatingLayoutManagerTest, PostOrQueueAction_MayPostImmediately)
{
    ...
    test_layout->SetLayout(layout1());
    layout()->ResetLayout();
    layout()->PostOrQueueAction(base::BindOnce([](bool* var) { *var = true; }, &action3_called));
    RunCurrentTasks();
...
TEST_F(AnimatingLayoutManagerTest, ZOrder_FadingOutViewMovedToBack)
{
    const ProposedLayout starting_layout { { 10, 10 },
        { { child(0), true, { 1, 1, 2, 2 } }, { child(1), true, { 3, 3, 2, 2 } }, { child(2), true, { 7, 7, 2, 2 } } } };

    const ProposedLayout ending_layout { { 8, 8 }, { { child(0), true, { 1, 1, 2, 2 } }, { child(1), false }, { child(2), true, { 5, 5, 2, 2 } } } };

    const std::vector<raw_ptr<View, VectorExperimental>> expected_order { child(1), child(0), child(2) };
...
{
    const ProposedLayout starting_layout { { 8, 8 }, { { child(0), true, { 1, 1, 2, 2 } }, { child(1), false }, { child(2), true, { 5, 5, 2, 2 } } } };
...
TEST_F(AnimatingLayoutManagerTest, ConstrainedSpace_TriggersDelayedAction)
{
    ...
    layout()->PostOrQueueAction(base::BindOnce([](bool* var) { *var = true; }, &action_called));
    RunCurrentTasks();
...
class AnimatingLayoutManagerNoAnimationsTest : public AnimatingLayoutManagerTest {
public:
...
protected:
    ...
    {
        TestLayoutManager* const test_layout = layout()->target_layout_manager() ? static_cast<TestLayoutManager*>(layout()->target_layout_manager())
    ...
    static const std::vector<std::pair<AnimatingLayoutManager::FadeInOutMode, const char*>> kFadeModes;
};

const std::vector<std::pair<AnimatingLayoutManager::FadeInOutMode, const char*>> AnimatingLayoutManagerNoAnimationsTest::kFadeModes = {
    { AnimatingLayoutManager::FadeInOutMode::kHide, "Hide" },
    { AnimatingLayoutManager::FadeInOutMode::kScaleFromZero, "Scale" },
    { AnimatingLayoutManager::FadeInOutMode::kSlideFromTrailingEdge, "Slide" },
};
...
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    UseFlexLayout()->SetOrientation(LayoutOrientation::kHorizontal).SetCollapseMargins(true).SetDefault(kMarginsKey, gfx::Insets(5));
    const ProposedLayout expected_layout { { 50, 20 },
        { { child(0), true, { { 5, 5 }, kChildViewSize } }, { child(1), true, { { 20, 5 }, kChildViewSize } },
            { child(2), true, { { 35, 5 }, kChildViewSize } } } };
    SizeAndLayout();
    EXPECT_FALSE(layout()->is_animating());
    ...
    for (const auto& fade_mode : kFadeModes) {
        ...
        const ProposedLayout expected_layout2
            = { { 35, 20 }, { { child(0), false }, { child(1), true, { { 5, 5 }, kChildViewSize } }, { child(2), true, { { 20, 5 }, kChildViewSize } } } };
...
TEST_F(AnimatingLayoutManagerNoAnimationsTest, ActionsPostedAfterLayout)
{
    ...
    layout()->PostOrQueueAction(base::BindLambdaForTesting([&]() { cb1 = true; }));
    RunCurrentTasks();
    EXPECT_TRUE(cb1);
    ...
    UseFixedLayout(layout2());
    layout()->PostOrQueueAction(base::BindLambdaForTesting([&]() { cb2 = true; }));
    view()->InvalidateLayout();
    layout()->PostOrQueueAction(base::BindLambdaForTesting([&]() { cb3 = true; }));
    RunCurrentTasks();
    EXPECT_FALSE(cb2);
    EXPECT_FALSE(cb3);
    ...
    layout()->PostOrQueueAction(base::BindLambdaForTesting([&]() { cb4 = true; }));
    RunCurrentTasks();
    EXPECT_TRUE(cb4);
}
...
public:
    ImmediateLayoutManager()
        : ImmediateLayoutManager(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes, LayoutOrientation::kHorizontal)
    {
    }

    ImmediateLayoutManager(
        AnimatingLayoutManager::BoundsAnimationMode bounds_animation_mode, LayoutOrientation orientation, SizeBounds size_bounds = SizeBounds())
    ...
    ProposedLayout CalculateProposedLayout(const SizeBounds& bounds) const override
    {
        ...
        for (View* child : host_view()->children()) {
            ...
            case AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes:
                child_layout.bounds = gfx::Rect(ConstrainSizeToBounds(child->GetPreferredSize(bounds), size_bounds_));
                break;
            case AnimatingLayoutManager::BoundsAnimationMode::kAnimateMainAxis: {
                ...
                // the cross axis.
                gfx::Size size = ConstrainSizeToBounds(child->GetPreferredSize(bounds), size_bounds_);
                SetCrossAxis(&size, orientation_, GetCrossAxis(orientation_, child->bounds().size()));
...
public:
    ...
    void OnLayoutIsAnimatingChanged(AnimatingLayoutManager*, bool is_animating) override
    {
...
private:
    ...
    base::ScopedObservation<AnimatingLayoutManager, AnimatingLayoutManager::Observer> observation_ { this };
...
class AnimatingLayoutManagerAvailableSizeTest : public AnimatingLayoutManagerRootViewTest {
protected:
    void InitRootView()
    {
        root_layout_ = root_view()->SetLayoutManager(std::make_unique<ImmediateLayoutManager>(layout()->bounds_animation_mode(), layout()->orientation()));
    }

    ImmediateLayoutManager* root_layout()
    {
        return root_layout_;
    }
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AvailableSize_RestartsAnimation)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AvailableSize_RestartsAnimation_Vertical)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AvailableSize_RedirectsAnimation)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AvailableSize_ConstraintRemovedStartsAnimation)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AvailableSize_LimitsExpansion_WithFlex)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AvailableSize_RestartsAnimation_WithFlex)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AvailableSize_RedirectsAnimation_WithFlex)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AvailableSize_StopsAnimation_WithFlex)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AvailableSize_ImmediateResize_WithFlex)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AvailableSize_StepDownStepUp_WithFlex)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AvailableSize_ConstraintRemovedStartsAnimation_WithFlex)
{
    constexpr gfx::Insets kChildMargins(5);
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AnimateMainAxis_Horizontal_MainAxisAnimates)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateMainAxis);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
    flex_layout->SetOrientation(LayoutOrientation::kHorizontal)
        .SetCrossAxisAlignment(LayoutAlignment::kStart)
        .SetDefault(kFlexBehaviorKey,
            FlexSpecification(
                LayoutOrientation::kHorizontal, MinimumFlexSizeRule::kPreferred, MaximumFlexSizeRule::kPreferred, false, MinimumFlexSizeRule::kScaleToZero));
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AnimateMainAxis_Vertical_MainAxisAnimates)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateMainAxis).SetOrientation(LayoutOrientation::kVertical);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
    flex_layout->SetOrientation(LayoutOrientation::kVertical)
        .SetCrossAxisAlignment(LayoutAlignment::kStart)
        .SetDefault(kFlexBehaviorKey,
            FlexSpecification(
                LayoutOrientation::kVertical, MinimumFlexSizeRule::kPreferred, MaximumFlexSizeRule::kPreferred, false, MinimumFlexSizeRule::kScaleToZero));
    view()->SetBoundsRect(gfx::Rect(0, 0, 5, 5));
    InitRootView();
    layout()->ResetLayout();
    test::RunScheduledLayout(root_view());
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AnimateMainAxis_Horizontal_CrossAxisSizeChangeResetsLayout)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateMainAxis);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
    flex_layout->SetOrientation(LayoutOrientation::kHorizontal)
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AnimateMainAxis_Vertical_CrossAxisSizeChangeResetsLayout)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateMainAxis).SetOrientation(LayoutOrientation::kVertical);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
    flex_layout->SetOrientation(LayoutOrientation::kVertical)
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AnimateMainAxis_Horizontal_CrossAxisAlignmentWorks)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateMainAxis);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
    flex_layout->SetOrientation(LayoutOrientation::kHorizontal).SetCrossAxisAlignment(LayoutAlignment::kStart);
...
TEST_F(AnimatingLayoutManagerAvailableSizeTest, AnimateMainAxis_Vertical_CrossAxisAlignmentWorks)
{
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateMainAxis).SetOrientation(LayoutOrientation::kVertical);
    auto* const flex_layout = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
    flex_layout->SetOrientation(LayoutOrientation::kVertical).SetCrossAxisAlignment(LayoutAlignment::kStart);
    // Pick an arbitrary (wrong) main-axis size.
    view()->SetBoundsRect(gfx::Rect(0, 0, 20, 20));
    InitRootView();
    layout()->ResetLayout();
...
public:
    ...
    void InitLayout(LayoutOrientation orientation, const FlexSpecification& default_flex, const std::optional<gfx::Size>& minimum_size, bool fix_child_size)
    {
...
const FlexSpecification AnimatingLayoutManagerFlexRuleTest::kScaleToMinimumSnapToZero
    = FlexSpecification(MinimumFlexSizeRule::kScaleToMinimumSnapToZero, MaximumFlexSizeRule::kUnbounded, true).WithOrder(2);

TEST_F(AnimatingLayoutManagerFlexRuleTest, ReturnsPreferredSize)
{
    InitLayout(LayoutOrientation::kHorizontal, kScaleToMinimumSnapToZero, gfx::Size(5, 5), false);
    EXPECT_EQ(flex_layout()->GetPreferredSize(view()), RunFlexRule(SizeBounds()));
}

TEST_F(AnimatingLayoutManagerFlexRuleTest, VerticalBounded_ReturnsPreferredSize)
{
    InitLayout(LayoutOrientation::kVertical, kScaleToMinimumSnapToZero, gfx::Size(5, 5), true);
    const gfx::Size preferred = flex_layout()->GetPreferredSize(view());
    const gfx::Size result = RunFlexRule(SizeBounds(preferred.width() + 5, SizeBound()));
    EXPECT_EQ(preferred, result);
    EXPECT_EQ(3U, GetVisibleChildCount(result));
}

TEST_F(AnimatingLayoutManagerFlexRuleTest, VerticalBounded_ReturnsHeightForWidth)
{
    InitLayout(LayoutOrientation::kVertical, kScaleToMinimumSnapToZero, gfx::Size(5, 5), true);

    const gfx::Size preferred = flex_layout()->GetPreferredSize(view());
    const int width = preferred.width() - 5;
    const int height_for_width = flex_layout()->GetPreferredHeightForWidth(view(), width);
...
{
    InitLayout(LayoutOrientation::kVertical, kScaleToMinimumSnapToZero, gfx::Size(5, 5), false);
...
{
    InitLayout(LayoutOrientation::kHorizontal, kScaleToMinimumSnapToZero, gfx::Size(10, 5), true);
...
{
    InitLayout(LayoutOrientation::kVertical, kScaleToMinimumSnapToZero, gfx::Size(5, 10), true);
...
{
    InitLayout(LayoutOrientation::kHorizontal, kScaleToMinimumSnapToZero, gfx::Size(5, 5), true);
...
class AnimatingLayoutManagerInFlexLayoutTest : public AnimatingLayoutManagerRootViewTest {
protected:
    ...
    void SetUp() override
    {
        ...
        layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
        root_layout_ = root_view()->SetLayoutManager(std::make_unique<FlexLayout>());
        root_layout_->SetOrientation(LayoutOrientation::kHorizontal)
            .SetMainAxisAlignment(LayoutAlignment::kStart)
            .SetCrossAxisAlignment(LayoutAlignment::kStart);
        view()->SetProperty(kFlexBehaviorKey, FlexSpecification(layout()->GetDefaultFlexRule()).WithOrder(2));
        target_layout_ = layout()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(AnimatingLayoutManagerInFlexLayoutTest, AnimateFullyWithinAvailableSpace)
{
    other_view()->set_preferred_size(gfx::Size());
    const gfx::Size preferred = target_layout()->GetPreferredSize(view());
    root_view()->SetSize(preferred);
...
TEST_F(AnimatingLayoutManagerInFlexLayoutTest, GrowWithinConstrainedSpace_NoAnimationRestart)
{
    other_view()->set_preferred_size(gfx::Size(5, 5));
    const gfx::Size preferred = target_layout()->GetPreferredSize(view());
    root_view()->SetSize(preferred);
...
TEST_F(AnimatingLayoutManagerInFlexLayoutTest, GrowWithinConstrainedSpace_AnimationRedirected)
{
    const gfx::Size preferred = target_layout()->GetPreferredSize(view());
    root_view()->SetSize(preferred);
    layout()->ResetLayout();
...
TEST_F(AnimatingLayoutManagerInFlexLayoutTest, GrowWithinConstrainedSpace_AnimationInterrupted)
{
    const gfx::Size preferred = target_layout()->GetPreferredSize(view());
    root_view()->SetSize(preferred);
    layout()->ResetLayout();
...
TEST_F(AnimatingLayoutManagerInFlexLayoutTest, ShrinkWithinConstrainedSpace_AnimationProceeds)
{
    const gfx::Size preferred = target_layout()->GetPreferredSize(view());
    root_view()->SetSize(preferred);
    layout()->ResetLayout();
...
TEST_F(AnimatingLayoutManagerInFlexLayoutTest, ShrinkWithinConstrainedSpace_ExpandedSpaceHasNoEffect)
{
    const gfx::Size preferred = target_layout()->GetPreferredSize(view());
    root_view()->SetSize(preferred);
    layout()->ResetLayout();
...
TEST_F(AnimatingLayoutManagerInFlexLayoutTest, ShrinkWithinConstrainedSpace_SnapToFinalLayout)
{
    const gfx::Size preferred = target_layout()->GetPreferredSize(view());
    root_view()->SetSize(preferred);
    layout()->ResetLayout();
...
TEST_F(AnimatingLayoutManagerInFlexLayoutTest, ShrinkWithinConstrainedSpace_SnapToConstrainedLayout)
{
    const gfx::Size preferred = target_layout()->GetPreferredSize(view());
    root_view()->SetSize(preferred);
    layout()->ResetLayout();
...
TEST_F(AnimatingLayoutManagerInFlexLayoutTest, ShrinkWithinConstrainedSpace_NoRestartOnLargerPreferredSize)
{
    const gfx::Size preferred = target_layout()->GetPreferredSize(view());
    root_view()->SetSize(preferred);
    layout()->ResetLayout();
...
class AnimatingLayoutManagerInFlexLayoutNoAnimationTest : public AnimatingLayoutManagerInFlexLayoutTest {
public:
...
{
    other_view()->SetProperty(
        kFlexBehaviorKey, FlexSpecification(LayoutOrientation::kHorizontal, MinimumFlexSizeRule::kPreferred, MaximumFlexSizeRule::kUnbounded).WithOrder(3));
...
class AnimatingLayoutManagerRealtimeTest : public AnimatingLayoutManagerRootViewTest {
public:
    ...
    bool UseContainerTestApi() const override
    {
        return false;
    }
...
protected:
    ...
    {
        root_view()->SetLayoutManager(std::make_unique<ImmediateLayoutManager>(layout()->bounds_animation_mode(), layout()->orientation(), std::move(bounds)));
        layout()->EnableAnimationForTesting();
    }

    AnimationWatcher* animation_watcher()
    {
        return animation_watcher_.get();
    }
...
TEST_F(AnimatingLayoutManagerRealtimeTest, TestAnimateSlide)
{
    ...
    const ProposedLayout expected_end { { 35, 20 }, { { child(1), true, { { 5, 5 }, kChildViewSize } }, { child(2), true, { { 20, 5 }, kChildViewSize } } } };
...
TEST_F(AnimatingLayoutManagerRealtimeTest, TestAnimateStretch)
{
    ...
    const ProposedLayout expected_end { { 50, 20 },
        { { child(1), true, { { 5, 5 }, { 25, kChildViewSize.height() } } }, { child(2), true, { { 35, 5 }, kChildViewSize } } } };
...
TEST_F(AnimatingLayoutManagerRealtimeTest, TestConstrainedSpaceStopsAnimation)
{
    ...
    child(0)->SetProperty(kFlexBehaviorKey, FlexSpecification(MinimumFlexSizeRule::kScaleToZero, MaximumFlexSizeRule::kPreferred));
    child(0)->SetVisible(false);
    layout()->ResetLayout();
    view()->InvalidateLayout();

    const ProposedLayout starting_layout { { 35, 20 },
        { { child(1), true, { { 5, 5 }, kChildViewSize } }, { child(2), true, { { 20, 5 }, kChildViewSize } } } };

    const ProposedLayout ending_layout { { 45, 20 },
        { { child(0), true, { { 5, 5 }, { 5, 10 } } }, { child(1), true, { { 15, 5 }, kChildViewSize } }, { child(2), true, { { 30, 5 }, kChildViewSize } } } };
...
TEST_F(AnimatingLayoutManagerRealtimeTest, TestConstrainedSpaceRestartedAnimationSucceeds)
{
    constexpr gfx::Insets kChildMargins(5);
    constexpr SizeBounds kSizeBounds(45, SizeBound());
    layout()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
...
public:
    ...
    void SetUp() override
    {
        render_mode_lock_ = gfx::AnimationTestApi::SetRichAnimationRenderMode(gfx::Animation::RichAnimationRenderMode::FORCE_ENABLED);
        ViewsTestBase::SetUp();
        ...
        parent_view_ptr_->SetLayoutManager(std::make_unique<ImmediateLayoutManager>());
    ...
    void ConfigureLayoutView()
    {
        ...
        auto* const flex_layout = layout_manager()->SetTargetLayoutManager(std::make_unique<FlexLayout>());
        flex_layout->SetOrientation(LayoutOrientation::kHorizontal);
        flex_layout->SetCollapseMargins(true);
        flex_layout->SetCrossAxisAlignment(LayoutAlignment::kStart);
        flex_layout->SetDefault(kMarginsKey, gfx::Insets(5));
        layout_manager()->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateBothAxes);
    }
    ...
    {
        return static_cast<AnimatingLayoutManager*>(layout_view_->GetLayoutManager());
    }

private:
    struct WidgetCloser {
        inline void operator()(Widget* widget) const
        {
            widget->CloseNow();
        }
    };
...
TEST_F(AnimatingLayoutManagerSequenceTest, AddChild_AddToParent_Configure_AddToWidget)
{
...
TEST_F(AnimatingLayoutManagerSequenceTest, AddChild_Configure_AddToParent_AddToWidget)
{
    AddChildToLayoutView();
    ConfigureLayoutView();
    AddViewToParent();
...
TEST_F(AnimatingLayoutManagerSequenceTest, Configure_AddChild_AddToParent_AddToWidget)
{
    ConfigureLayoutView();
    AddChildToLayoutView();
    AddViewToParent();
...
TEST_F(AnimatingLayoutManagerSequenceTest, Configure_AddToParent_AddChild_AddToWidget)
{
    ConfigureLayoutView();
    AddViewToParent();
    AddChildToLayoutView();
...
TEST_F(AnimatingLayoutManagerSequenceTest, AddToParent_Configure_AddChild_AddToWidget)
{
    AddViewToParent();
    ConfigureLayoutView();
    AddChildToLayoutView();
...
TEST_F(AnimatingLayoutManagerSequenceTest, AddToParent_AddChild_Configure_AddToWidget)
{
    AddViewToParent();
    AddChildToLayoutView();
    ConfigureLayoutView();
...
TEST_F(AnimatingLayoutManagerSequenceTest, AddToWidget_AddToParent_Configure_AddChild)
{
    AddParentToWidget();
    AddViewToParent();
    ConfigureLayoutView();
...
TEST_F(AnimatingLayoutManagerSequenceTest, AddToParent_AddToWidget_Configure_AddChild)
{
    AddViewToParent();
    AddParentToWidget();
    ConfigureLayoutView();
...
TEST_F(AnimatingLayoutManagerSequenceTest, Configure_AddToParent_AddToWidget_AddChild)
{
    ConfigureLayoutView();
    AddViewToParent();
    AddParentToWidget();
...
TEST_F(AnimatingLayoutManagerSequenceTest, AddToWidget_AddToParent_AddChild_Configure)
{
    AddParentToWidget();
    AddViewToParent();
    AddChildToLayoutView();
