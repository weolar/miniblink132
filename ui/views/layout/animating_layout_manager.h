...
public:
    ...
    public:
        virtual void OnLayoutIsAnimatingChanged(AnimatingLayoutManager* source, bool is_animating) = 0;
    };
    ...
    AnimatingLayoutManager& SetBoundsAnimationMode(BoundsAnimationMode bounds_animation_mode);

    base::TimeDelta animation_duration() const
    {
        return animation_duration_;
    }
    AnimatingLayoutManager& SetAnimationDuration(base::TimeDelta animation_duration);
    ...
    AnimatingLayoutManager& SetOpacityAnimationDuration(base::TimeDelta animation_duration);

    gfx::Tween::Type opacity_tween_type() const
    {
        return opacity_tween_type_;
    }
    ...
    FadeInOutMode default_fade_mode() const
    {
        return default_fade_mode_;
    }
    ...
    // only be set once.
    template <class T> T* SetTargetLayoutManager(std::unique_ptr<T> layout_manager)
    {
    ...
    const ProposedLayout& target_layout() const
    {
...
private:
    ...
    // and target layouts.
    void UpdateCurrentLayout(double percent, double fade_in_opacity, double fade_out_opacity);
    ...
    // layout info.
    ChildLayout CalculateScaleFade(const LayoutFadeInfo& fade_info, double scale_percent, bool scale_from_zero) const;

    // Calculates a kSlideFrom[Leading|Trailing]Edge fade and returns the
    // resulting child layout info.
    ChildLayout CalculateSlideFade(const LayoutFadeInfo& fade_info, double scale_percent, bool slide_from_leading) const;

    ChildLayout CalculateFadeAndSlideFade(const LayoutFadeInfo& fade_info, double scale_percent, double opacity_value, bool slide_from_leading) const;
    ...
    static gfx::Size DefaultFlexRuleImpl(const AnimatingLayoutManager* animating_layout, const View* view, const SizeBounds& size_bounds);

    // How to animate bounds of the host view when the preferred size of the
    // layout changes.
    BoundsAnimationMode bounds_animation_mode_ = BoundsAnimationMode::kUseHostBounds;
