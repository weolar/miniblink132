...
SizeBound SizeBoundValueBetween(double value, const SizeBound& start, const SizeBound& target)
{
    return (start.is_bounded() && target.is_bounded()) ? gfx::Tween::IntValueBetween(value, start.value(), target.value()) : target;
}

SizeBounds SizeBoundsBetween(double value, const SizeBounds& start, const SizeBounds& target)
{
    return { SizeBoundValueBetween(value, start.width(), target.width()), SizeBoundValueBetween(value, start.height(), target.height()) };
}
...
bool ChildLayout::operator==(const ChildLayout& other) const
{
    ...
    // be set.
    return child_view == other.child_view && visible == other.visible && (!visible || bounds == other.bounds);
}

ChildLayout* ProposedLayout::GetLayoutFor(const View* child_view)
{
    // Defer to the const implementation and then cast back.
    return const_cast<ChildLayout*>(const_cast<const ProposedLayout*>(this)->GetLayoutFor(child_view));
}

const ChildLayout* ProposedLayout::GetLayoutFor(const View* child_view) const
{
    const auto found = base::ranges::find_if(child_layouts, [child_view](const auto& child_layout) { return child_view == child_layout.child_view; });
...
{
    return base::StrCat(
        { "{", child_view->GetClassName(), (visible ? " visible " : " not visible "), bounds.ToString(), " / ", available_size.ToString(), "}" });
}
...
ProposedLayout::ProposedLayout(const gfx::Size& size, const std::initializer_list<ChildLayout>& children)
    : host_size(size)
    , child_layouts(children)
{
}
ProposedLayout::~ProposedLayout() = default;
ProposedLayout& ProposedLayout::operator=(const ProposedLayout& other) = default;
...
ProposedLayout ProposedLayoutBetween(double value, const ProposedLayout& start, const ProposedLayout& target)
{
    if (value >= 1.0)
        return target;
    ...
    layout.host_size = gfx::Tween::SizeValueBetween(value, start.host_size, target.host_size);
    ...
    for (const ChildLayout& target_child : target.child_layouts) {
        ...
        if (start_match == start_view_to_index.end()) {
            ...
            layout.child_layouts.push_back({ target_child.child_view, start_child.visible && target_child.visible,
                gfx::Tween::RectValueBetween(value, start_child.bounds, target_child.bounds),
                SizeBoundsBetween(value, start_child.available_size, target_child.available_size) });
        }
    }
    return layout;
}
