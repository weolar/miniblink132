...
ProposedLayout FillLayout::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
    ...
    for (View* child : host_view()->children()) {
        if (!child->GetProperty(kViewIgnoredByLayoutKey)) {
            layout.child_layouts.push_back(ChildLayout { child, child->GetVisible(), contents_bounds, SizeBounds(contents_bounds.size()) });
        }
    }
...
gfx::Size FillLayout::GetPreferredSize(const View* host, const SizeBounds& available_size) const
{
    DCHECK_EQ(host_view(), host);
    ...
    bool has_child = false;
    const SizeBounds new_available_size = include_insets_ ? available_size.Inset(host->GetInsets()) : available_size;
    for (const View* child : host->children()) {
        if (!child->GetProperty(kViewIgnoredByLayoutKey)) {
            has_child = true;
...
int FillLayout::GetPreferredHeightForWidth(const View* host, int width) const
{
    ...
    for (const View* child : host->children()) {
        if (!child->GetProperty(kViewIgnoredByLayoutKey)) {
            height = std::max(height, insets.height() + child->GetHeightForWidth(width));
        }
    }
...
{
    return host->bounds().IsEmpty() ? SizeBounds() : SizeBounds(host->GetContentsBounds().size());
}
