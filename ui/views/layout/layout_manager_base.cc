...
SizeBounds AdjustAvailableSizeForParentAvailableSize(const View* host, const SizeBounds& child_available_size)
{
...
gfx::Size LayoutManagerBase::GetPreferredSize(const View* host, const SizeBounds& available_size) const
{
    DCHECK_EQ(host_view_, host);
...
int LayoutManagerBase::GetPreferredHeightForWidth(const View* host, int width) const
{
    if (!cached_height_for_width_ || cached_height_for_width_->width() != width) {
        const int height = CalculateProposedLayout(SizeBounds(width, SizeBound())).host_size.height();
...
SizeBounds LayoutManagerBase::GetAvailableSize(const View* host, const View* view) const
{
    DCHECK_EQ(host_view_, host);
    ...
    if (cached_layout_size_) {
        for (const auto& child_layout : cached_layout_.child_layouts)
            if (child_layout.child_view == view) {
                return AdjustAvailableSizeForParentAvailableSize(host, child_layout.available_size);
            }
    }
    return SizeBounds();
}
...
{
    DCHECK_EQ(host_view_, host);
    TRACE_EVENT1("ui", "LayoutManagerBase::Layout", "class", host->GetClassName());
...
void LayoutManagerBase::OnViewPropertyChanged(View* observed_view, const void* key, int64_t old_value)
{
...
std::vector<raw_ptr<View, VectorExperimental>> LayoutManagerBase::GetChildViewsInPaintOrder(const View* host) const
{
    DCHECK_EQ(host_view_, host);
    return LayoutManager::GetChildViewsInPaintOrder(host);
}

ProposedLayout LayoutManagerBase::GetProposedLayout(const gfx::Size& host_size) const
{
...
ProposedLayout LayoutManagerBase::GetProposedLayout(const SizeBounds& size_bounds, PassKeyType) const
{
...
bool LayoutManagerBase::IsChildIncludedInLayout(const View* child, bool include_hidden) const
{
    ...
    return it->second.included_in_layout && (include_hidden || it->second.can_be_visible);
}
...
void LayoutManagerBase::ApplyLayout(const ProposedLayout& layout)
{
    ...
    for (auto& child_layout : layout.child_layouts) {
        ...
        if (new_available_size != cached_available_size_ || child_layout.visible || !child_layout.bounds.IsEmpty()) {
            const bool size_changed = child_view->bounds().size() != child_layout.bounds.size();
...
bool LayoutManagerBase::OnChildViewIncludedInLayoutSet(View* child_view, bool ignored)
{
    OnLayoutChanged();
    return false;
}
...
bool LayoutManagerBase::OnViewVisibilitySet(View* host, View* view, bool visible)
{
    OnLayoutChanged();
    return false;
}
...
void LayoutManagerBase::ViewRemoved(View* host, View* view)
{
    ...
    const bool removed_visible = it->second.can_be_visible && it->second.included_in_layout;
...
void LayoutManagerBase::ViewVisibilitySet(View* host, View* view, bool old_visibility, bool new_visibility)
{
    DCHECK_EQ(host_view_, host);
    auto it = child_infos_.find(view);
    CHECK(it != child_infos_.end(), base::NotFatalUntil::M130);
    ...
    base::AutoReset<bool> setter(&suppress_invalidate_, true);
    const bool invalidate = PropagateViewVisibilitySet(host, view, new_visibility);
    if (invalidate || it->second.included_in_layout) {
        InvalidateHost(false);
    }
}

void LayoutManagerBase::AddOwnedLayoutInternal(std::unique_ptr<LayoutManagerBase> owned_layout)
{
    DCHECK(owned_layout);
    ...
    base::ranges::for_each(
        owned_layout->view_observations_.sources(), [&](View* source) { DCHECK(root_layout->view_observations_.IsObservingSource(source)); });
#endif
    ...
    if (host_view_) {
        ...
        for (View* child_view : host_view_->children()) {
            ...
            owned_layout->PropagateChildViewIncludedInLayout(child_view, child_info.included_in_layout);
            owned_layout->PropagateViewVisibilitySet(host_view_, child_view, child_info.can_be_visible);
        }
    }
...
bool LayoutManagerBase::PropagateChildViewIncludedInLayout(View* child_view, bool include)
{
    child_infos_[child_view].included_in_layout = include;

    bool result = false;
    for (auto& owned_layout : owned_layouts_) {
        result |= owned_layout->PropagateChildViewIncludedInLayout(child_view, include);
    }
...
{
    child_infos_.emplace(view, ChildInfo { .can_be_visible = view->GetVisible(), .included_in_layout = IncludeInLayout(view) });
...
bool LayoutManagerBase::PropagateViewVisibilitySet(View* host, View* view, bool visible)
{
...
void LayoutManagerBase::PropagateInstalled(View* host)
{
    ...
    for (View* child : host->children()) {
        child_infos_.emplace(child, ChildInfo { .can_be_visible = child->GetVisible(), .included_in_layout = IncludeInLayout(child) });
    }
...
void ManualLayoutUtil::SetViewHidden(View* child_view, bool hidden)
{
    ...
    if (can_be_visible != it->second.can_be_visible) {
        layout_manager_->PropagateViewVisibilitySet(layout_manager_->host_view(), child_view, can_be_visible);
    }
}

ManualLayoutUtil::TemporaryExclusion ManualLayoutUtil::TemporarilyExcludeFromLayout(View* child_view)
{
    const auto it = layout_manager_->child_infos_.find(child_view);
    CHECK(it != layout_manager_->child_infos_.end());
    CHECK_EQ(IncludeInLayout(child_view), it->second.included_in_layout) << "Cannot nest exclusions for the same child view.";
...
void ManualLayoutUtil::TemporaryExclusionDeleter::operator()(TemporaryExclusionData* data) const
{
    if (data) {
