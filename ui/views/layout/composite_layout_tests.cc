...
protected:
    ...
    {
        auto* const animating_layout = SetLayoutManager(std::make_unique<AnimatingLayoutManager>());
        animating_layout->SetBoundsAnimationMode(AnimatingLayoutManager::BoundsAnimationMode::kAnimateMainAxis)
            .SetOrientation(LayoutOrientation::kHorizontal)
            .SetAnimationDuration(kDefaultAnimationDuration);
        animating_layout->SetTargetLayoutManager(std::make_unique<FlexLayout>())->SetOrientation(LayoutOrientation::kHorizontal);
    }
...
public:
    SimulatedAvatarButton()
    {
        ...
        auto* const status = AddChildView(std::make_unique<StaticSizedView>(kLabelSize));
    ...
    void EnsureLayout() const
    {
        ...
        if (layout()->is_animating()) {
            ...
            EXPECT_EQ(gfx::Rect(gfx::Point(kIconDimension, 0), kLabelSize), label()->bounds());
        } else {
...
public:
    ...
    {
        auto* const main_button = AddChildView(std::make_unique<StaticSizedView>(kIconSize));
        ...
        target_layout()->SetDefault(kFlexBehaviorKey, FlexSpecification(LayoutOrientation::kHorizontal, MinimumFlexSizeRule::kPreferredSnapToZero));
    }
    ...
    void EnsureLayout(std::optional<int> expected_num_icons) const
    {
        if (layout()->is_animating()) {
            ...
            if (available_size.width().is_bounded()) {
                num_visible = std::min(num_visible, (available_size.width().value() - kIconDimension) / kIconDimension);
            }
            ...
            for (int i = 0; i < static_cast<int>(children().size()) - 1; ++i) {
                ...
                if (base::Contains(visible_views_, child)) {
                    if (num_visible > 0) {
                        ...
                        EXPECT_EQ(gfx::Rect(gfx::Point(x, 0), kIconSize), child->bounds()) << " icon " << i;
                        x += kIconDimension;
                    } else {
                        // This is a pinned extension that overflowed the available space
                        // and therefore should be hidden.
                        EXPECT_FALSE(child->GetVisible()) << " icon " << i << " should have been hidden; available size is " << available_size.ToString();
                    }
                } else {
...
public:
    ...
    {
        AddChildView(std::make_unique<StaticSizedView>(kIconSize));
        auto* const bar = AddChildView(std::make_unique<StaticSizedView>(kBarPreferredSize));
        bar->set_minimum_size(kBarMinimumSize);
        extensions_ = AddChildView(std::make_unique<SimulatedExtensionsContainer>());
        avatar_ = AddChildView(std::make_unique<SimulatedAvatarButton>());
        AddChildView(std::make_unique<StaticSizedView>(kIconSize));

        SetLayoutManager(std::make_unique<FlexLayout>())->SetOrientation(LayoutOrientation::kHorizontal);
        avatar_->SetProperty(kFlexBehaviorKey, FlexSpecification(avatar_->layout()->GetDefaultFlexRule()).WithOrder(1));
        bar->SetProperty(kFlexBehaviorKey,
            FlexSpecification(LayoutOrientation::kHorizontal, MinimumFlexSizeRule::kScaleToMinimum, MaximumFlexSizeRule::kUnbounded).WithOrder(2));
        extensions_->SetProperty(kFlexBehaviorKey, FlexSpecification(extensions_->layout()->GetDefaultFlexRule()).WithOrder(3));
    }

    SimulatedExtensionsContainer* extensions()
    {
        return extensions_;
    }
    ...
    SimulatedAvatarButton* avatar()
    {
        return avatar_;
    }
    const SimulatedAvatarButton* avatar() const
    {
        return avatar_;
    }
    View* location()
    ...
    void EnsureLayout(std::optional<int> expected_num_extension_icons) const
    {
        ...
        EXPECT_EQ(gfx::Point(location()->bounds().right(), 0), extensions_->origin());
        ...
        EXPECT_EQ(gfx::Rect(gfx::Point(avatar_->bounds().right(), 0), kIconSize), children()[4]->bounds());
...
public:
    void SetUp() override
    {
        ...
        animation_lock_ = gfx::AnimationTestApi::SetRichAnimationRenderMode(gfx::Animation::RichAnimationRenderMode::FORCE_ENABLED);

        toolbar_ = std::make_unique<SimulatedToolbar>();
        toolbar_->SetSize(kDefaultToolbarSize);
        extensions_test_api_ = std::make_unique<gfx::AnimationContainerTestApi>(extensions()->layout()->GetAnimationContainerForTesting());
        avatar_test_api_ = std::make_unique<gfx::AnimationContainerTestApi>(avatar()->layout()->GetAnimationContainerForTesting());
    }

    SimulatedAvatarButton* avatar()
    {
        return toolbar_->avatar();
    }
    const SimulatedAvatarButton* avatar() const
    {
        return toolbar_->avatar();
    ...
    SimulatedToolbar* toolbar()
    {
        return toolbar_.get();
    }
    const SimulatedToolbar* toolbar() const
    {
        return toolbar_.get();
    }
    ...
    {
        toolbar()->SetSize(gfx::Size(std::max(0, toolbar()->width() + delta), kIconDimension));
    }
    ...
    {
        return avatar()->layout()->is_animating() || extensions()->layout()->is_animating();
    }
    ...
    void FinishAnimations()
    {
        ...
        GTEST_FAIL() << "Animations did not complete in a reasonable amount of time.";
    }
    ...
    // visible.
    void EnsureLayout(std::optional<int> expected_num_extension_icons = std::nullopt)
    {
...
{
    toolbar()->SetSize(gfx::Size(4 * kIconDimension + kBarMinimumWidth, kIconDimension));
    EnsureLayout();
}

TEST_F(CompositeLayoutTest, ShrinkLocationBarTooSmall)
{
    toolbar()->SetSize(gfx::Size(4 * kIconDimension + kBarMinimumWidth - 20, kIconDimension));
    EnsureLayout();
}
...
TEST_F(CompositeLayoutTest, ExtensionsShowingAnimationRedirectsDueToSmallerAvailableSpace)
{
    // Provide room for both icons.
    SetWidth(toolbar()->GetPreferredSize({}).width() + 2 * kIconDimension);
    extensions()->AddIcons({ true, true });
...
TEST_F(CompositeLayoutTest, ExtensionsShowingAnimationCancelsDueToSmallerAvailableSpace)
{
    // Provide room for both icons.
    SetWidth(toolbar()->GetPreferredSize({}).width() + 2 * kIconDimension);
    extensions()->AddIcons({ true, true });
...
TEST_F(CompositeLayoutTest, ExtensionsShowingAnimationRedirectsDueToLargerAvailableSpace)
{
    // Provide room for one of two icons.
    SetWidth(toolbar()->GetPreferredSize({}).width() + kIconDimension);
    extensions()->AddIcons({ true, true });
