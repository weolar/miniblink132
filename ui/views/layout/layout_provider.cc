...
// static
int LayoutProvider::GetControlHeightForFont(int context, int style, const gfx::FontList& font)
{
    return std::max(TypographyProvider::Get().GetLineHeight(context, style), font.GetHeight())
...
gfx::Insets LayoutProvider::GetInsetsMetric(int metric) const
{
    ...
    case InsetsMetric::INSETS_DIALOG_BUTTON_ROW: {
        ...
        return gfx::Insets::TLBR(0, dialog_insets.left(), dialog_insets.bottom(), dialog_insets.right());
    }
    case InsetsMetric::INSETS_DIALOG_TITLE: {
        const gfx::Insets dialog_insets = GetInsetsMetric(INSETS_DIALOG);
        return gfx::Insets::TLBR(dialog_insets.top(), dialog_insets.left(), 0, dialog_insets.right());
    }
...
int LayoutProvider::GetDistanceMetric(int metric) const
{
    ...
    case DISTANCE_DIALOG_CONTENT_MARGIN_BOTTOM_TEXT:
        ...
        return GetDistanceMetric(DISTANCE_DIALOG_CONTENT_MARGIN_BOTTOM_CONTROL) - 8;
...
gfx::Insets LayoutProvider::GetDialogInsetsForContentType(DialogContentType leading, DialogContentType trailing) const
{
    const int top_margin = leading == DialogContentType::kControl ? GetDistanceMetric(DISTANCE_DIALOG_CONTENT_MARGIN_TOP_CONTROL)
                                                                  : GetDistanceMetric(DISTANCE_DIALOG_CONTENT_MARGIN_TOP_TEXT);
    const int bottom_margin = trailing == DialogContentType::kControl ? GetDistanceMetric(DISTANCE_DIALOG_CONTENT_MARGIN_BOTTOM_CONTROL)
                                                                      : GetDistanceMetric(DISTANCE_DIALOG_CONTENT_MARGIN_BOTTOM_TEXT);
    const gfx::Insets dialog_insets = GetInsetsMetric(INSETS_DIALOG);
    return gfx::Insets::TLBR(top_margin, dialog_insets.left(), bottom_margin, dialog_insets.right());
}

int LayoutProvider::GetCornerRadiusMetric(Emphasis emphasis, const gfx::Size& size) const
{
    switch (emphasis) {
    case Emphasis::kNone:
        return 0;
...
{
    static constexpr auto shape_token_map = base::MakeFixedFlatMap<ShapeContextTokens, ShapeSysTokens>({
        ...
        { ShapeContextTokens::kSidePanelContentRadius, ShapeSysTokens::kMedium },
        { ShapeContextTokens::kSidePanelPageContentRadius, ShapeSysTokens::kSmall },
    });
    const auto it = shape_token_map.find(id);
    return it == shape_token_map.end() ? ShapeSysTokens::kDefault : it->second;
}

int LayoutProvider::GetCornerRadiusMetric(ShapeContextTokens id, const gfx::Size& size) const
{
    ShapeSysTokens token = GetShapeSysToken(id);
    DCHECK_NE(token, ShapeSysTokens::kDefault) << "kDefault token means there is a missing mapping between shape tokens";
    switch (token) {
