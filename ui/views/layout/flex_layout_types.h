...
using FlexRule = base::RepeatingCallback<gfx::Size(const View*, const SizeBounds&)>;
...
public:
    ...
    explicit FlexSpecification(
        MinimumFlexSizeRule minimum_size_rule, MaximumFlexSizeRule maximum_size_rule = MaximumFlexSizeRule::kPreferred, bool adjust_height_for_width = false);
    ...
    FlexSpecification(LayoutOrientation orientation, MinimumFlexSizeRule minimum_main_axis_rule,
        MaximumFlexSizeRule maximum_main_axis_rule = MaximumFlexSizeRule::kPreferred, bool adjust_height_for_width = false,
        MinimumFlexSizeRule minimum_cross_axis_rule = MinimumFlexSizeRule::kPreferred);
...
public:
    ...
    constexpr explicit Inset1D(int all)
        : leading_(all)
        , trailing_(all)
    {
    }
    ...
    constexpr int trailing() const
    {
        return trailing_;
    }
    void set_trailing(int trailing)
    {
        trailing_ = trailing;
    }
...
public:
    ...
    constexpr Span(int start, int length)
        : start_(start)
        , length_(length)
    {
    }
    ...
    void Align(const Span& container, LayoutAlignment alignment, const Inset1D& margins = Inset1D());
