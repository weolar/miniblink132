...
public:
    ...
    // marked as resizable.
    TableLayout& AddColumn(LayoutAlignment h_align, LayoutAlignment v_align, float horizontal_resize, ColumnSize size_type, int fixed_width, int min_width);
...
private:
    ...
    void CalculateSize(SizeCalculationType type, const std::vector<raw_ptr<ViewState, VectorExperimental>>& view_states) const;
    ...
    mutable std::vector<raw_ptr<ViewState, VectorExperimental>> view_states_by_col_span_;
};
