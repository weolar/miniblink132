...
public:
    ...
    FlexLayout& SetIgnoreDefaultMainAxisMargins(bool ignore_default_main_axis_margins);
    ...
    bool collapse_margins() const
    {
        return collapse_margins_;
    }
    LayoutAlignment main_axis_alignment() const
    {
    ...
    const gfx::Insets& interior_margin() const
    {
        return interior_margin_;
    }
    int minimum_cross_axis_size() const
    {
    ...
    template <class T, class U> FlexLayout& SetDefault(const ui::ClassProperty<T>* key, U&& value)
    {
    ...
    template <class T, class U> FlexLayout& SetDefault(const ui::ClassProperty<T>* key, const U& value)
    {
        layout_defaults_.SetProperty(key, value);
        return *this;
    }

    template <class T> T* GetDefaultForTesting(const ui::ClassProperty<T*>* key) const
    {
...
private:
    ...
    static constexpr LayoutAlignment kDefaultMainAxisAlignment = LayoutAlignment::kStart;

    // Layout used when the cross-axis alignment is not specified.
    static constexpr LayoutAlignment kDefaultCrossAxisAlignment = LayoutAlignment::kStretch;
    ...
    NormalizedSize GetPreferredSizeForRule(const FlexRule& rule, const View* child, const SizeBound& available_cross) const;

    // Returns the size for a given |rule| and |child| with |available| space.
    NormalizedSize GetCurrentSizeForRule(const FlexRule& rule, const View* child, const NormalizedSizeBounds& available) const;

    // Returns the combined margins across the cross axis of the host view, for a
    // particular child view.
    Inset1D GetCrossAxisMargins(const FlexLayoutData& layout, size_t child_index) const;
    ...
    SizeBound GetAvailableCrossAxisSize(const FlexLayoutData& layout, size_t child_index, const NormalizedSizeBounds& bounds) const;

    // Calculates the preferred spacing between two child views, or between a
    // view edge and the first or last visible child views.
    int CalculateChildSpacing(const FlexLayoutData& layout, std::optional<size_t> child1_index, std::optional<size_t> child2_index) const;
    ...
    void UpdateLayoutFromChildren(const NormalizedSizeBounds& bounds, FlexLayoutData& data, ChildViewSpacing& child_spacing) const;
    ...
    void InitializeChildData(const NormalizedSizeBounds& bounds, FlexLayoutData& data, FlexOrderToViewIndexMap& flex_order_to_index) const;

    // Caclulates the child bounds (in screen coordinates) for each visible child
    // in the layout.
    void CalculateChildBounds(const SizeBounds& size_bounds, FlexLayoutData& data) const;
    ...
    // in |child_spacing|.
    void CalculateFlexAvailableSpace(
    ...
    NormalizedSize ClampSizeToMinAndMax(FlexLayoutData& data, const size_t view_index, SizeBound size) const;

    // Allocate space for flexible views
    void AllocateFlexItem(const NormalizedSizeBounds& bounds, const FlexOrderToViewIndexMap& order_to_index, FlexLayoutData& data,
    ...
    SizeBound AllocateZeroWeightFlex(const NormalizedSizeBounds& bounds, ChildIndices& child_list, FlexLayoutData& data, ChildViewSpacing& child_spacing) const;
    ...
    void FilterZeroSizeChildreIfNeeded(
    ...
    bool ResolveFlexibleLengths(const NormalizedSizeBounds& bounds, SizeBound& remaining_free_space, ChildIndices& child_list, FlexLayoutData& data,
    ...
    // force a restart.
    bool FreezeViolations(ChildIndices& child_list, SizeBound& remaining_free_space, ChildIndices& freeze_child_list, ChildViewSpacing& child_spacing,
    ...
    static int CalculateFlexTotal(const FlexLayoutData& data, const ChildIndices& child_indices);
    ...
    // kMarginsKey).
    template <class T> T* GetDefault(const ui::ClassProperty<T*>* key) const
    {
        return layout_defaults_.GetProperty(key);
    }

    static gfx::Size DefaultFlexRuleImpl(const FlexLayout* flex_layout, const View* view, const SizeBounds& size_bounds);
