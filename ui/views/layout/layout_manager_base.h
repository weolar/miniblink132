...
class VIEWS_EXPORT LayoutManagerBase : public LayoutManager, public ViewObserver {
public:
    ...
    // other LayoutManager.
    ProposedLayout GetProposedLayout(const SizeBounds& size_bounds, PassKeyType) const;
    ...
    SizeBounds GetAvailableSize(const View* host, const View* view) const override;
    void Layout(View* host) final;

    // ViewObserver:
    void OnViewPropertyChanged(View* observed_view, const void* key, int64_t old_value) final;
...
protected:
    ...
    PassKeyType PassKey() const
    {
        return PassKeyType();
    }

    // LayoutManager:
    std::vector<raw_ptr<View, VectorExperimental>> GetChildViewsInPaintOrder(const View* host) const override;
    ...
    void set_cached_minimum_size(const std::optional<gfx::Size>& minimum_size) const
    {
    ...
    void set_cached_preferred_size(const std::optional<gfx::Size>& preferred_size) const
    {
    ...
    void set_cached_height_for_width(const std::optional<gfx::Size>& height_for_width) const
    {
    ...
    void set_cached_layout_size(const std::optional<gfx::Size>& layout_size) const
    {
        cached_layout_size_ = layout_size;
    }
    const ProposedLayout& cached_layout() const
    {
    ...
    bool IsChildIncludedInLayout(const View* child, bool include_hidden = false) const;
    ...
    virtual ProposedLayout CalculateProposedLayout(const SizeBounds& size_bounds) const = 0;
    ...
    template <class T> T* AddOwnedLayout(std::unique_ptr<T> owned_layout)
    {
    ...
    size_t num_owned_layouts() const
    {
        return owned_layouts_.size();
    }
...
private:
    ...
    void ViewVisibilitySet(View* host, View* view, bool old_visibility, bool new_visibility) final;
    ...
    // managers as needed.
    base::ScopedMultiSourceObservation<View, ViewObserver> view_observations_ { this };
...
public:
    ...
    using TemporaryExclusionData = std::pair<const raw_ptr<ManualLayoutUtil>, const raw_ptr<View>>;
    struct VIEWS_EXPORT TemporaryExclusionDeleter {
        void operator()(TemporaryExclusionData*) const;
    };
    using TemporaryExclusion = std::unique_ptr<TemporaryExclusionData, ManualLayoutUtil::TemporaryExclusionDeleter>;
    ...
    [[nodiscard]] TemporaryExclusion TemporarilyExcludeFromLayout(View* child_view);
