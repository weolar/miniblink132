...
constexpr MaximumFlexSizeRule kDefaultMaximumFlexSizeRule = MaximumFlexSizeRule::kPreferred;
...
int InterpolateSize(MinimumFlexSizeRule minimum_size_rule, MaximumFlexSizeRule maximum_size_rule, int minimum_size, int preferred_size,
    LazyDimension maximum_size, int available_size)
{
...
gfx::Size GetPreferredSize(MinimumFlexSizeRule minimum_width_rule, MaximumFlexSizeRule maximum_width_rule, MinimumFlexSizeRule minimum_height_rule,
    MaximumFlexSizeRule maximum_height_rule, bool adjust_height_for_width, const View* view, const SizeBounds& size_bounds)
{
    ...
    if (!size_bounds.width().is_bounded()) {
        ...
        width = InterpolateSize(
            minimum_width_rule, maximum_width_rule, minimum_size->width(), preferred.width(), maximum_size.width(), size_bounds.width().value());
    }
    ...
    if (adjust_height_for_width) {
        ...
        if (width < preferred.width()) {
            ...
            // point.
            preferred_height = std::max(preferred_height, view->GetHeightForWidth(width));
        }
    }
    ...
    if (!size_bounds.height().is_bounded()) {
        ...
        height = InterpolateSize(
            minimum_height_rule, maximum_height_rule, minimum_size->height(), preferred_height, maximum_size.height(), size_bounds.height().value());
    }
...
FlexSpecification::FlexSpecification()
    : rule_(base::BindRepeating(&GetPreferredSize, MinimumFlexSizeRule::kPreferred, MaximumFlexSizeRule::kPreferred, MinimumFlexSizeRule::kPreferred,
...
FlexSpecification::FlexSpecification(MinimumFlexSizeRule minimum_size_rule, MaximumFlexSizeRule maximum_size_rule, bool adjust_height_for_width)
    : FlexSpecification(
        base::BindRepeating(&GetPreferredSize, minimum_size_rule, maximum_size_rule, minimum_size_rule, maximum_size_rule, adjust_height_for_width))
{
}

FlexSpecification::FlexSpecification(LayoutOrientation orientation, MinimumFlexSizeRule minimum_main_axis_rule, MaximumFlexSizeRule maximum_main_axis_rule,
    bool adjust_height_for_width, MinimumFlexSizeRule minimum_cross_axis_rule)
    : FlexSpecification(base::BindRepeating(&GetPreferredSize, orientation == LayoutOrientation::kHorizontal ? minimum_main_axis_rule : minimum_cross_axis_rule,
        orientation == LayoutOrientation::kHorizontal ? maximum_main_axis_rule : kDefaultMaximumFlexSizeRule,
        orientation == LayoutOrientation::kVertical ? minimum_main_axis_rule : minimum_cross_axis_rule,
        orientation == LayoutOrientation::kVertical ? maximum_main_axis_rule : kDefaultMaximumFlexSizeRule, adjust_height_for_width))
{
}

FlexSpecification::FlexSpecification(const FlexSpecification& other) = default;

FlexSpecification& FlexSpecification::operator=(const FlexSpecification& other) = default;
...
FlexSpecification FlexSpecification::WithAlignment(LayoutAlignment alignment) const
{
    FlexSpecification spec = *this;
...
{
    return std::tie(leading_, trailing_) == std::tie(other.leading_, other.trailing_);
}
...
void Span::Align(const Span& container, LayoutAlignment alignment, const Inset1D& margins)
{
    switch (alignment) {
    ...
    case LayoutAlignment::kStretch:
        SetSpan(container.start() + margins.leading(), std::max(0, container.length() - margins.size()));
        break;
