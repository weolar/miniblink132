...
class LayoutElement {
public:
    explicit LayoutElement(float resize)
        : resize_(resize)
    {
    ...
    // preceding elements.
    template <class T> static void CalculateLocationsFromSize(std::vector<T>& elements)
    {
        int location = 0;
...
template <class T> void ResetSizes(std::vector<T>& elements)
{
    for (auto& element : elements)
        element.ResetSize();
}
...
template <class T> void DistributeDelta(int delta, std::vector<T>& elements)
{
    if (delta == 0)
        return;
    ...
    for (auto& element : elements) {
        if (element.resize() > 0) {
            ...
            if (--resize_count != 0) {
                element_delta = base::ClampFloor(delta * (element.resize() / total_resize));
...
// `start` + `length`.
template <class T> int TotalSize(size_t start, size_t length, const std::vector<T>& elements)
{
    DCHECK_GT(length, 0u);
    DCHECK_LE(start + length, elements.size());
    const auto begin = elements.cbegin() + static_cast<ptrdiff_t>(start);
    return std::accumulate(begin, begin + static_cast<ptrdiff_t>(length), 0, [](int size, const auto& elem) { return size + elem.size(); });
}

// Advances `index` past any padding elements.
template <class T> void SkipPadding(size_t& index, const std::vector<T>& elements)
{
    while (index < elements.size() && elements[index].is_padding())
        ++index;
}

void CalculateLocationAndSize(int pref_size, LayoutAlignment alignment, int* location, int* size)
{
...
public:
    Column(LayoutAlignment h_align, LayoutAlignment v_align, float horizontal_resize, ColumnSize size_type, int fixed_width, int min_width, bool is_padding)
    ...
    void set_linked_columns(const std::vector<raw_ptr<Column, VectorExperimental>>& linked_columns)
    {
        DCHECK(linked_columns_.empty()) << "Cannot link a column twice";
        linked_columns_ = linked_columns;
    }

    LayoutAlignment h_align() const
    {
        return h_align_;
    }
    LayoutAlignment v_align() const
    {
        return v_align_;
    }
    ColumnSize size_type() const
    {
        return size_type_;
    }
    int min_width() const
...
struct TableLayout::ViewState {
    ViewState() = default;
    ViewState(View* view, size_t start_col, size_t start_row, size_t col_span, size_t row_span, LayoutAlignment h_align, LayoutAlignment v_align)
        : view(view)
        , start_col(start_col)
        , start_row(start_row)
        , col_span(col_span)
...
TableLayout& TableLayout::AddColumn(
    LayoutAlignment h_align, LayoutAlignment v_align, float horizontal_resize, ColumnSize size_type, int fixed_width, int min_width)
{
    columns_.emplace_back(h_align, v_align, horizontal_resize, size_type, fixed_width, min_width, false);
    return *this;
}

TableLayout& TableLayout::AddPaddingColumn(float horizontal_resize, int width)
{
    columns_.emplace_back(LayoutAlignment::kStretch, LayoutAlignment::kStretch, horizontal_resize, ColumnSize::kFixed, width, width, true);
    return *this;
}
...
TableLayout& TableLayout::LinkColumnSizes(std::vector<size_t> columns)
{
    if (columns.size() > 1) {
        ...
        DCHECK_LT(columns.back(), columns_.size()) << "Cannot link an unspecified column";

        std::vector<raw_ptr<Column, VectorExperimental>> linked_columns;
        base::ranges::transform(columns, std::back_inserter(linked_columns), [&](size_t index) { return &columns_[index]; });
...
ProposedLayout TableLayout::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
    ...
    for (const auto& view_state : view_states_by_row_span_) {
        ...
        int width = TotalSize(view_state->start_col, view_state->col_span, columns_);
        CalculateLocationAndSize(view_state->width, view_state->h_align, &x, &width);
        int y = rows_[view_state->start_row].location() + insets.top();
        int height = TotalSize(view_state->start_row, view_state->row_span, rows_);
        if (view_state->v_align == LayoutAlignment::kBaseline && view_state->baseline) {
            y += rows_[view_state->start_row].max_ascent() - *view_state->baseline;
            height = view_state->height;
        } else {
            CalculateLocationAndSize(view_state->height, view_state->v_align, &y, &height);
        }

        auto it = base::ranges::find(layout.child_layouts, view, &ChildLayout::child_view);
...
void TableLayout::SetViewStates() const
{
    ...
    for (View* child : GetChildViewsInPaintOrder(host_view())) {
        ...
        CHECK_LT(row, rows_.size()) << "There're not enough cells for layout. Did you forget to "
                                       "call AddRows()?";
        ...
        LayoutAlignment* const child_h_align = child->GetProperty(kTableHorizAlignKey);
        const LayoutAlignment h_align = child_h_align ? *child_h_align : columns_[col].h_align();
        LayoutAlignment* const child_v_align = child->GetProperty(kTableVertAlignKey);
        const LayoutAlignment v_align = child_v_align ? *child_v_align : columns_[col].v_align();
        auto view_state = std::make_unique<ViewState>(child, col, row, col_span, row_span, h_align, v_align);

        // Add `view_state` to the relevant vectors.
        ViewState* ptr;
        {
            auto it = base::ranges::lower_bound(view_states_by_row_span_, view_state->row_span, std::less<>(), &ViewState::row_span);
            ptr = view_states_by_row_span_.insert(it, std::move(view_state))->get();
        }
        {
            auto it = base::ranges::lower_bound(view_states_by_col_span_, ptr->col_span, std::less<>(), &ViewState::col_span);
            view_states_by_col_span_.insert(it, ptr);
        }
        if (ptr->row_span > 1) {
            DCHECK_LE(row + ptr->row_span, rows_.size()) << "row_span extends past trailing edge";
            auto it = base::ranges::lower_bound(row_spans, ptr->start_col, std::less<>(), &ViewState::start_col);
...
gfx::Size TableLayout::SizeRowsAndColumns(const SizeBounds& bounds) const
{
    ...
    const int bounded_width = bounds.width().is_bounded() ? bounds.width().value() : pref.width();
    ...
    for (auto& view_state : view_states_by_row_span_) {
        ...
        int actual_width = TotalSize(view_state->start_col, view_state->col_span, columns_);
        int x = 0; // Not used in this stage.
        CalculateLocationAndSize(view_state->width, view_state->h_align, &x, &actual_width);
    ...
    for (; view_states_iterator != view_states_by_row_span_.end() && (*view_states_iterator)->row_span == 1; ++view_states_iterator) {
        auto& view_state = *view_states_iterator;
        Row& row = rows_[view_state->start_row];
        row.AdjustSize(view_state->remaining_height);
        if (view_state->baseline.has_value() && *view_state->baseline <= view_state->height) {
            row.AdjustSizeForBaseline(*view_state->baseline, view_state->height - *view_state->baseline);
        }
        view_state->remaining_height = 0;
    }

    // Distribute the height of each view with a row_span > 1.
    for (; view_states_iterator != view_states_by_row_span_.end(); ++view_states_iterator) {
        auto& view_state = *view_states_iterator;
        view_state->remaining_height -= TotalSize(view_state->start_row, view_state->row_span, rows_);
    ...
    pref.set_height(rows_.back().location() + rows_.back().size() + insets.height());
...
void TableLayout::DistributeRemainingHeight(ViewState& view_state) const
{
    ...
    const base::span<Row> rows_to_resize = base::span(rows_).subspan(view_state.start_row, view_state.row_span);
    const auto resizable_rows = static_cast<size_t>(base::ranges::count_if(rows_to_resize, &Row::resizable));
    size_t remaining_rows = resizable_rows ? resizable_rows : rows_to_resize.size();
    for (Row& row : rows_to_resize) {
        if (!resizable_rows || row.resizable()) {
            ...
            // the last row.
            const int delta = base::ClampRound(static_cast<float>(view_state.remaining_height) / remaining_rows);
...
void TableLayout::DistributeRemainingWidth(ViewState& view_state) const
{
    ...
    if (resizable_columns > 0) {
        ...
        for (size_t i = start_col, resize_i = 0; i < max_col; ++i) {
            if (columns_[i].resizable()) {
                ++resize_i;
                const int column_delta = (resize_i == resizable_columns) ? remaining_width : base::ClampFloor(width * columns_[i].resize() / total_resize);
...
{
    return std::accumulate(columns_.cbegin(), columns_.cend(), 0, [](int size, const auto& elem) { return size + elem.size(); });
}

void TableLayout::CalculateSize(SizeCalculationType type, const std::vector<raw_ptr<ViewState, VectorExperimental>>& view_states) const
{
    ...
    for (; view_state_iterator != view_states.end() && (*view_state_iterator)->col_span == 1; ++view_state_iterator) {
    ...
    for (; view_state_iterator != view_states.end(); ++view_state_iterator) {
        ...
        view_state->remaining_width -= TotalSize(view_state->start_col, view_state->col_span, columns_);
...
void TableLayout::ResizeUsingMin(int total_delta) const
{
    ...
    std::vector<raw_ptr<ViewState, VectorExperimental>> view_state_ptrs(num_states);
    ...
    for (size_t i = 0; i < columns_.size(); ++i) {
        ...
        const int available = std::max(0, preferred_column_sizes[i] - std::max(column.min_width(), column.size()));
    ...
    while (!resize_data.empty() && total_delta > 0) {
        ...
        for (size_t i = resize_data.size(); i > 0; --i) {
            ...
            int delta = std::min(data.available, base::ClampFloor(total_delta * data.column->resize() / total_resize));
...
{
    const auto begin = columns_.cbegin() + static_cast<ptrdiff_t>(view_state.start_col);
    return std::any_of(
        begin, begin + static_cast<ptrdiff_t>(view_state.col_span), [](const auto& col) { return col.resizable() && col.size_type() != ColumnSize::kFixed; });
}
