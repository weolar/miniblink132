...
int LayoutManager::GetPreferredHeightForWidth(const View* host, int width) const
{
    return GetPreferredSize(host).height();
}

SizeBounds LayoutManager::GetAvailableSize(const View* host, const View* view) const
{
    return SizeBounds();
}
...
void LayoutManager::ViewVisibilitySet(View* host, View* view, bool old_visibility, bool new_visibility)
{
...
{
    DCHECK(!view->parent() || view->parent()->GetLayoutManager() == this || view->parent()->GetLayoutManager() == nullptr);
    base::AutoReset<raw_ptr<View>> setter(&view_setting_visibility_on_, view);
    view->SetVisible(visible);
}

std::vector<raw_ptr<View, VectorExperimental>> LayoutManager::GetChildViewsInPaintOrder(const View* host) const
{
