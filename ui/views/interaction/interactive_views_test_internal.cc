...
// receives focus.
class NativeViewWidgetFocusSupplier : public WidgetFocusSupplier, public WidgetFocusChangeListener {
public:
...
protected:
    Widget::Widgets GetAllWidgets() const override
    {
        ...
        if (aura::test::AuraTestHelper* const aura_test_helper = aura::test::AuraTestHelper::GetInstance()) {
...
private:
    base::ScopedObservation<WidgetFocusManager, WidgetFocusChangeListener> observation_ { this };
};
...
InteractiveViewsTestPrivate::DebugTreeNodeViews::List DebugDumpViewHierarchy(std::vector<const TrackedElementViews*> views)
{
    ...
    for (const auto& view : views) {
        ...
        for (const View* ancestor = view->view()->parent(); ancestor != nullptr; ancestor = ancestor->parent()) {
        ...
        for (const auto* view_to_add : views_to_add) {
            ...
            const auto add_child_result = to_add_to->children.emplace(view_to_add->view(), view_to_add);
            ...
            const auto add_entry_result = view_nodes.emplace(view_to_add->view(), ptr);
...
InteractiveViewsTestPrivate::DebugTreeNodeViews::DebugTreeNodeViews(const View* view, const ui::TrackedElement* el)
    : impl(view)
    , element(el)
    , bounds(view->GetBoundsInScreen())
{
}
InteractiveViewsTestPrivate::DebugTreeNodeViews::DebugTreeNodeViews(const Widget* widget)
    : impl(widget)
    , bounds(widget->GetWindowBoundsInScreen())
{
}
InteractiveViewsTestPrivate::DebugTreeNodeViews::DebugTreeNodeViews(DebugTreeNodeViews&&) noexcept = default;
InteractiveViewsTestPrivate::DebugTreeNodeViews& InteractiveViewsTestPrivate::DebugTreeNodeViews::operator=(DebugTreeNodeViews&&) noexcept = default;
InteractiveViewsTestPrivate::DebugTreeNodeViews::~DebugTreeNodeViews() = default;

InteractiveViewsTestPrivate::DebugTreeNode InteractiveViewsTestPrivate::DebugTreeNodeViews::ToNode(const InteractiveViewsTestPrivate& owner) const
{
    ...
    if (std::holds_alternative<const View*>(impl)) {
        ...
        result = DebugTreeNode(owner.DebugDumpWidget(*std::get<const Widget*>(impl)));
    }
...
std::strong_ordering InteractiveViewsTestPrivate::DebugTreeNodeViews::operator<=>(const DebugTreeNodeViews& other) const
{
...
InteractiveViewsTestPrivate::InteractiveViewsTestPrivate(std::unique_ptr<ui::test::InteractionTestUtil> test_util)
...
void InteractiveViewsTestPrivate::OnSequenceAborted(const ui::InteractionSequence::AbortedData& data)
{
    if (mouse_util_) {
        mouse_util_->CancelAllGestures();
    }
...
gfx::NativeWindow InteractiveViewsTestPrivate::GetWindowHintFor(ui::TrackedElement* el)
{
    ...
    if (window) {
        ...
        auto result = window_hint_cache_.try_emplace(el->context(), WindowHintCacheEntry());
...
gfx::NativeWindow InteractiveViewsTestPrivate::GetNativeWindowFromElement(ui::TrackedElement* el) const
{
...
gfx::NativeWindow InteractiveViewsTestPrivate::GetNativeWindowFromContext(ui::ElementContext context) const
{
    // Used the cached value, if one exists.
    const auto it = window_hint_cache_.find(context);
    return it != window_hint_cache_.end() ? it->second.GetWindow() : gfx::NativeWindow();
}

std::string InteractiveViewsTestPrivate::DebugDumpWidget(const Widget& widget) const
{
    std::string description = widget.GetName();
    return base::StringPrintf("%s \"%s\" at %s", widget.GetClassName(), widget.GetName().c_str(), DebugDumpBounds(widget.GetWindowBoundsInScreen()).c_str());
}

InteractiveViewsTestPrivate::DebugTreeNode InteractiveViewsTestPrivate::DebugDumpElement(const ui::TrackedElement* el) const
{
    if (const auto* view = el->AsA<TrackedElementViews>()) {
        return DebugTreeNode(base::StringPrintf("%s%s - %s at %s", (view->view()->HasFocus() ? "[FOCUSED] " : ""), view->view()->GetClassName(),
            el->identifier().GetName().c_str(), DebugDumpBounds(el->GetScreenBounds())));
    }
    return InteractiveTestPrivate::DebugDumpElement(el);
}

InteractiveViewsTestPrivate::DebugTreeNode InteractiveViewsTestPrivate::DebugDumpContext(ui::ElementContext context) const
{
