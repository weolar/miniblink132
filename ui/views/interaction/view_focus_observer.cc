...
{
    CHECK_EQ(focus_manager_, widget->GetTopLevelWidget()->GetFocusManager()) << "Expect focus manager to be owned by top level widget.";
...
ui::ElementIdentifier ViewFocusObserverByIdentifier::GetStateObserverInitialState() const
{
    auto* const focused = GetFocusedView();
    return focused ? focused->GetProperty(kElementIdentifierKey) : ui::ElementIdentifier();
}

void ViewFocusObserverByIdentifier::OnFocusChanged(View* new_focused_view)
{
    OnStateObserverStateChanged(new_focused_view ? new_focused_view->GetProperty(kElementIdentifierKey) : ui::ElementIdentifier());
}

DEFINE_STATE_IDENTIFIER_VALUE(ViewFocusObserverByView, kCurrentFocusedView);
DEFINE_STATE_IDENTIFIER_VALUE(ViewFocusObserverByIdentifier, kCurrentFocusedViewId);
