    ...
    bool Wait()
    {
        if (!active_) {
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
                FROM_HERE, base::BindOnce(&WidgetActivationWaiterWayland::OnTimeout, weak_ptr_factory_.GetWeakPtr()), kTimeout);
            run_loop_.Run();
        }
        return active_;
    }
...
public:
    ...
    DropdownItemSelector(InteractionTestUtilSimulatorViews* simulator, ui::test::InteractionTestUtil::InputType input_type, size_t item_index)
        : simulator_(simulator)
        , input_type_(input_type)
        , item_index_(item_index)
    {
        observer_.set_shown_callback(base::BindRepeating(&DropdownItemSelector::OnWidgetShown, weak_ptr_factory_.GetWeakPtr()));
        observer_.set_hidden_callback(base::BindRepeating(&DropdownItemSelector::OnWidgetHidden, weak_ptr_factory_.GetWeakPtr()));
    }
...
private:
    ...
    void OnWidgetShown(Widget* widget)
    {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&DropdownItemSelector::SelectItemImpl, weak_ptr_factory_.GetWeakPtr()));
    }
    ...
    void SelectItemImpl()
    {
        ...
        if (auto* const menu_item = FindMenuItem(widget_->GetContentsView(), index)) {
            ...
            const auto result = simulator_->SelectMenuItem(ElementTrackerViews::GetInstance()->GetElementForView(menu_item, true), input_type_);
            SetResult(result);
            switch (result) {
            ...
            case ui::test::ActionResult::kKnownIncompatible:
                LOG(WARNING) << "Select dropdown item not available on this platform with "
                                "input type "
                             << input_type_;
                break;
...
template <class T> void SendMouseClick(T* target, const gfx::Point& point)
{
    ui::MouseEvent mouse_down(ui::EventType::kMousePressed, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    target->OnMouseEvent(&mouse_down);
    ui::MouseEvent mouse_up(ui::EventType::kMouseReleased, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
...
// Views are EventHandlers but Widgets are not despite having the same API for
// event handling, so use a templated approach to support both cases.
template <class T> void SendTapGesture(T* target, const gfx::Point& point)
{
    ...
    ui::GestureEvent press_event(point.x(), point.y(), ui::EF_NONE, ui::EventTimeForNow(), press_details);
    ...
    ui::GestureEvent release_event(point.x(), point.y(), ui::EF_NONE, ui::EventTimeForNow(), release_details);
...
bool SendKeyPress(View* view, ui::KeyboardCode code, int flags = ui::EF_NONE)
{
    ...
    view->OnKeyPressed(ui::KeyEvent(ui::EventType::kKeyPressed, code, flags, ui::EventTimeForNow()));
    ...
    tracker.view()->OnKeyReleased(ui::KeyEvent(ui::EventType::kKeyReleased, code, flags, ui::EventTimeForNow()));
...
InteractionTestUtilSimulatorViews::InteractionTestUtilSimulatorViews() = default;
InteractionTestUtilSimulatorViews::~InteractionTestUtilSimulatorViews() = default;

ui::test::ActionResult InteractionTestUtilSimulatorViews::PressButton(ui::TrackedElement* element, InputType input_type)
{
    if (!element->IsA<TrackedElementViews>())
        return ui::test::ActionResult::kNotAttempted;
    auto* const button = Button::AsButton(element->AsA<TrackedElementViews>()->view());
    if (!button)
        return ui::test::ActionResult::kNotAttempted;
...
ui::test::ActionResult InteractionTestUtilSimulatorViews::SelectMenuItem(ui::TrackedElement* element, InputType input_type)
{
    if (!element->IsA<TrackedElementViews>())
        return ui::test::ActionResult::kNotAttempted;
    auto* const menu_item = AsViewClass<MenuItemView>(element->AsA<TrackedElementViews>()->view());
    if (!menu_item)
        return ui::test::ActionResult::kNotAttempted;
    ...
    case ui::test::InteractionTestUtil::InputType::kDontCare: {
        ...
        ui::KeyEvent key_event(ui::EventType::kKeyPressed, kSelectMenuKeyboardCode, ui::EF_NONE, ui::EventTimeForNow());
...
ui::test::ActionResult InteractionTestUtilSimulatorViews::DoDefaultAction(ui::TrackedElement* element, InputType input_type)
{
    if (!element->IsA<TrackedElementViews>())
        return ui::test::ActionResult::kNotAttempted;
    if (!DoDefaultAction(element->AsA<TrackedElementViews>()->view(), input_type)) {
...
ui::test::ActionResult InteractionTestUtilSimulatorViews::SelectTab(ui::TrackedElement* tab_collection, size_t index, InputType input_type)
{
    ...
    if (!tab_collection->IsA<TrackedElementViews>())
        return ui::test::ActionResult::kNotAttempted;
    auto* const pane = views::AsViewClass<TabbedPane>(tab_collection->AsA<TrackedElementViews>()->view());
    if (!pane)
        return ui::test::ActionResult::kNotAttempted;
    ...
    if (!tab) {
        LOG(ERROR) << "Tab index " << index << " out of range, there are " << pane->GetTabCount() << " tabs.";
        return ui::test::ActionResult::kFailed;
    }
    switch (input_type) {
    case ui::test::InteractionTestUtil::InputType::kDontCare:
    ...
    case ui::test::InteractionTestUtil::InputType::kKeyboard: {
        ...
        if (current_index != index) {
            const auto code = ((current_index > index) ^ base::i18n::IsRTL()) ? ui::VKEY_LEFT : ui::VKEY_RIGHT;
            const int count = std::abs(static_cast<int>(index) - static_cast<int>(current_index));
            LOG_IF(WARNING, count > 1) << "SelectTab via keyboard from " << current_index << " to " << index << " will pass through intermediate tabs.";
...
ui::test::ActionResult InteractionTestUtilSimulatorViews::SelectDropdownItem(ui::TrackedElement* dropdown, size_t index, InputType input_type)
{
    ...
    if (!combobox && !editable_combobox)
        return ui::test::ActionResult::kNotAttempted;
    auto* const model = combobox ? combobox->GetModel() : editable_combobox->GetComboboxModel();
    if (index >= model->GetItemCount()) {
        LOG(ERROR) << "Item index " << index << " is out of range, there are " << model->GetItemCount() << " items.";
        return ui::test::ActionResult::kFailed;
    }
    ...
    auto* const arrow = combobox ? combobox->arrow_button_.get() : editable_combobox->arrow_.get();
    if (arrow) {
...
ui::test::ActionResult InteractionTestUtilSimulatorViews::EnterText(ui::TrackedElement* element, std::u16string text, TextEntryMode mode)
{
    if (!element->IsA<TrackedElementViews>())
        return ui::test::ActionResult::kNotAttempted;
    ...
    textfield->InsertText(text, ui::TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    return ui::test::ActionResult::kSucceeded;
}

ui::test::ActionResult InteractionTestUtilSimulatorViews::ActivateSurface(ui::TrackedElement* element)
{
    if (!element->IsA<TrackedElementViews>())
        return ui::test::ActionResult::kNotAttempted;
...
ui::test::ActionResult InteractionTestUtilSimulatorViews::SendAccelerator(ui::TrackedElement* element, ui::Accelerator accelerator)
{
    if (!element->IsA<TrackedElementViews>())
        return ui::test::ActionResult::kNotAttempted;

    element->AsA<TrackedElementViews>()->view()->GetFocusManager()->ProcessAccelerator(accelerator);
    return ui::test::ActionResult::kSucceeded;
}

ui::test::ActionResult InteractionTestUtilSimulatorViews::Confirm(ui::TrackedElement* element)
{
    if (!element->IsA<TrackedElementViews>())
        return ui::test::ActionResult::kNotAttempted;
    auto* const view = element->AsA<TrackedElementViews>()->view();
...
// static
ui::test::ActionResult InteractionTestUtilSimulatorViews::ActivateWidget(Widget* widget)
{
#if HANDLE_WAYLAND_FAILURE
    if (IsWayland()) {
        ...
        widget->Activate();
        if (!waiter.Wait()) {
            LOG(WARNING) << "Unable to activate widget due to lack of Wayland support for "
...
// static
bool InteractionTestUtilSimulatorViews::DoDefaultAction(View* view, InputType input_type)
{
    switch (input_type) {
    case ui::test::InteractionTestUtil::InputType::kDontCare:
...
// static
void InteractionTestUtilSimulatorViews::PressButton(Button* button, InputType input_type)
{
    switch (input_type) {
    case ui::test::InteractionTestUtil::InputType::kMouse:
