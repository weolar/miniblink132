...
base::CallbackListSubscription WidgetFocusSupplier::AddWidgetFocusChangedCallback(WidgetFocusChangedCallback callback)
{
...
{
    LOG_IF(ERROR, g_current_supplier_frame) << "Old WidgetFocusSupplierFrame was not cleaned up.";
...
{
    for (auto& supplier : internal::WidgetFocusSupplierFrame::GetCurrentFrame()->supplier_list()) {
        subscriptions_.emplace_back(
            supplier.AddWidgetFocusChangedCallback(base::BindRepeating(&WidgetFocusObserver::OnWidgetFocusChanged, base::Unretained(this))));
    }
}
...
{
    auto* const widget = internal::WidgetFocusSupplierFrame::GetCurrentFrame()->GetActiveWidget();
