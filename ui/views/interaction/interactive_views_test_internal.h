...
class InteractiveViewsTestPrivate : public ui::test::internal::InteractiveTestPrivate {
public:
    explicit InteractiveViewsTestPrivate(std::unique_ptr<ui::test::InteractionTestUtil> test_util);
    ~InteractiveViewsTestPrivate() override;

    // base::test::internal::InteractiveTestPrivate:
    void OnSequenceComplete() override;
    void OnSequenceAborted(const ui::InteractionSequence::AbortedData& data) override;
...
protected:
    ...
    virtual gfx::NativeWindow GetNativeWindowFromElement(ui::TrackedElement* el) const;

    // Retrieves the native window from a context. Used by GetWindowHintFor().
    virtual gfx::NativeWindow GetNativeWindowFromContext(ui::ElementContext context) const;
    ...
    DebugTreeNode DebugDumpContext(const ui::ElementContext context) const override;
...
template <size_t N, typename F, typename V = std::remove_cv_t<std::remove_pointer_t<ui::test::internal::NthArgumentOf<N, F>>>>
    requires IsView<V>
using ViewArgType = V;
