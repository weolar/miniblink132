...
TrackedElementViews::TrackedElementViews(View* view, ui::ElementIdentifier identifier, ui::ElementContext context)
...
class ElementTrackerViews::ElementDataViews : public ViewObserver, public WidgetObserver {
public:
    ElementDataViews(ElementTrackerViews* tracker, ui::ElementIdentifier identifier)
        : tracker_(tracker)
        , id_(identifier)
    {
    }
    ...
    void AddView(View* view)
    {
        ...
        const auto it = view_data_.insert(view_data_.end(), ViewData(view, GetContextForView(view)));
    ...
    void RemoveView(View* view)
    {
        ...
        if (it->second->visible()) {
            ui::ElementTracker::GetFrameworkDelegate()->NotifyElementHidden(it->second->element.get());
        }
    ...
    void NotifyViewActivated(View* view)
    {
        ...
        CHECK(it != view_data_lookup_.end(), base::NotFatalUntil::M130);
        if (it->second->visible()) {
            ui::ElementTracker::GetFrameworkDelegate()->NotifyElementActivated(it->second->element.get());
        } else {
            DLOG(WARNING) << "View " << view->GetClassName()
    ...
    {
        ViewList result;
        base::ranges::transform(view_data_lookup_, std::back_inserter(result), &ViewDataMap::value_type::first);
        return result;
    }
...
private:
    ...
    // ViewObserver:
    void OnViewVisibilityChanged(View* observed_view, View* starting_view) override
    {
        UpdateVisible(observed_view, starting_view->parent() ? UpdateReason::kGeneral : UpdateReason::kVisbilityFromRoot);
    }
    ...
    void UpdateVisible(View* view, UpdateReason update_reason = UpdateReason::kGeneral)
    {
        const auto it = view_data_lookup_.find(view);
        CHECK(it != view_data_lookup_.end(), base::NotFatalUntil::M130);
        ViewData& data = *it->second;
        const ui::ElementContext old_context = data.context;
        data.context = (update_reason == UpdateReason::kRemoveFromWidget) ? ui::ElementContext() : GetContextForView(view);
        const bool was_visible = data.visible();
        const bool visible = it->second->context && IsViewVisibleToUser(view);
        if (visible && !was_visible) {
            data.element = std::make_unique<TrackedElementViews>(view, id_, data.context);
            ui::ElementTracker::GetFrameworkDelegate()->NotifyElementShown(data.element.get());
        } else if (!visible && was_visible) {
            ui::ElementTracker::GetFrameworkDelegate()->NotifyElementHidden(data.element.get());
            data.element.reset();
        } else if (visible && old_context != data.context) {
            CHECK(update_reason == UpdateReason::kVisbilityFromRoot) << "We should always get a removed-from-widget notification before "
...
// static
void ElementTrackerViews::SetContextOverrideCallback(ContextOverrideCallback callback)
{
...
TrackedElementViews* ElementTrackerViews::GetElementForView(View* view, bool assign_temporary_id)
{
...
const TrackedElementViews* ElementTrackerViews::GetElementForView(const View* view) const
{
    // Const casts are justified as `assign_temporary_id` = false will not result
    // in any modification to existing data.
    return const_cast<ElementTrackerViews*>(this)->GetElementForView(const_cast<View*>(view), false);
}

View* ElementTrackerViews::GetUniqueView(ui::ElementIdentifier id, ui::ElementContext context)
{
    ui::TrackedElement* const element = ui::ElementTracker::GetElementTracker()->GetUniqueElement(id, context);
...
View* ElementTrackerViews::GetFirstMatchingView(ui::ElementIdentifier id, ui::ElementContext context)
{
...
ElementTrackerViews::ViewList ElementTrackerViews::GetAllMatchingViews(ui::ElementIdentifier id, ui::ElementContext context)
{
    const auto it = element_data_.find(id);
    if (it == element_data_.end())
        return ViewList();
...
ElementTrackerViews::ViewList ElementTrackerViews::GetAllMatchingViewsInAnyContext(ui::ElementIdentifier id)
{
    const auto it = element_data_.find(id);
    if (it == element_data_.end())
        return ViewList();
...
bool ElementTrackerViews::NotifyCustomEvent(ui::CustomElementEventType event_type, View* view)
{
    auto* const element = GetElementForView(view, /* assign_temporary_id =*/true);
    if (!element)
        return false;
    ui::ElementTracker::GetFrameworkDelegate()->NotifyCustomEvent(element, event_type);
    return true;
}

void ElementTrackerViews::RegisterView(ui::ElementIdentifier element_id, View* view)
{
    const auto [it, added] = element_data_.try_emplace(element_id, this, element_id);
    it->second.AddView(view);
}

void ElementTrackerViews::UnregisterView(ui::ElementIdentifier element_id, View* view)
{
    DCHECK(view);
...
void ElementTrackerViews::NotifyViewActivated(ui::ElementIdentifier element_id, View* view)
{
    DCHECK(view);
    const auto it = element_data_.find(element_id);
    CHECK(it != element_data_.end(), base::NotFatalUntil::M130);
...
// static
ElementTrackerViews::ContextOverrideCallback& ElementTrackerViews::GetContextOverrideCallback()
{
