...
template <typename T, typename V, typename = std::enable_if<std::is_base_of_v<View, V>>>
...
public:
    ...
    template <typename C>
    PollingViewObserver(ui::ElementIdentifier view_id, std::optional<ui::ElementContext> context, C&& poll_view_callback,
        base::TimeDelta polling_interval = ui::test::PollingStateObserver<std::optional<T>>::kDefaultPollingInterval)
        : ui::test::PollingElementStateObserver<T>(view_id, context,
            base::BindRepeating(
                [](PollViewCallback callback, const ui::TrackedElement* el) {
                    const auto* const view_el = el->AsA<TrackedElementViews>();
                    CHECK(view_el);
                    const auto* const view = views::AsViewClass<V>(view_el->view());
                    CHECK(view);
                    return callback.Run(view);
                },
                ui::test::internal::MaybeBindRepeating(std::forward<C>(poll_view_callback))),
            polling_interval)
    {
    }
...
//
// See `PollingElementStateObserver<T>` for more information on usage.
template <typename T, typename V, typename = std::enable_if<std::is_base_of_v<View, V>>> class PollingViewPropertyObserver : public PollingViewObserver<T, V> {
public:
    template <typename R>
    PollingViewPropertyObserver(ui::ElementIdentifier view_id, std::optional<ui::ElementContext> context, R (V::*property)() const,
        base::TimeDelta polling_interval = ui::test::PollingStateObserver<std::optional<T>>::kDefaultPollingInterval)
        : PollingViewObserver<T, V>(
            view_id, context, base::BindRepeating([](R (V::*property)() const, const V* view) -> T { return (view->*property)(); }, property), polling_interval)
    {
    }
