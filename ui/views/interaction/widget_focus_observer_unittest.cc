...
TEST_F(WidgetFocusObserverTest, Bubble)
{
    ...
    auto bubble = std::make_unique<BubbleDialogDelegateView>(widget->GetRootView(), BubbleBorder::LEFT_CENTER);
    auto* const bubble_widget = BubbleDialogDelegate::CreateBubble(std::move(bubble));
    ...
    test::WidgetFocusObserver observer;
    observer.SetStateObserverStateChangedCallback(base::DoNothing());
    EXPECT_EQ(bubble_widget->GetNativeView(), observer.GetStateObserverInitialState());
}
