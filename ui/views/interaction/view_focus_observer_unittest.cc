...
public:
    ...
    void SetUp() override
    {
        ...
        button1_ = contents->AddChildView(std::make_unique<LabelButton>(Button::PressedCallback(), u"Button1"));
        button1_->SetProperty(kElementIdentifierKey, kButton1Id);
        button2_ = contents->AddChildView(std::make_unique<LabelButton>(Button::PressedCallback(), u"Button2"));
...
{
    RunTestSequence(ObserveState(kCurrentFocusedView, widget_.get()), ObserveState(kCurrentFocusedViewId, widget_.get()), Focus(button1_),
        WaitForState(kCurrentFocusedView, button1_.get()), WaitForState(kCurrentFocusedViewId, kButton1Id), Focus(button2_),
        WaitForState(kCurrentFocusedView, button2_.get()), WaitForState(kCurrentFocusedViewId, ui::ElementIdentifier()), Focus(text_),
        WaitForState(kCurrentFocusedView, text_.get()), WaitForState(kCurrentFocusedViewId, kTextFieldId),
        Do([this]() { widget_->GetFocusManager()->ClearFocus(); }),
        WaitForState(kCurrentFocusedView, testing::Matcher<View*>(testing::Not(testing::AnyOf(button1_.get(), button2_.get(), text_.get())))),
