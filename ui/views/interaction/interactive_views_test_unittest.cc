...
public:
    ...
    void SetUp() override
    {
        ...
        auto contents
            = Builder<FlexLayoutView>()
                  ...
                  .AddChildren(Builder<TabbedPane>()
                                   .CopyAddressTo(&tabs_)
                      ...
                      Builder<FlexLayoutView>()
                          ...
                          .AddChildren(Builder<LabelButton>()
                              ...
                              Builder<LabelButton>().CopyAddressTo(&button2_).SetText(kButton2Caption).SetCallback(button2_callback_.Get())),
                      Builder<ScrollView>()
                          .CopyAddressTo(&scroll_)
                          .SetPreferredSize(gfx::Size(100, 90))
                          .SetVerticalScrollBarMode(ScrollView::ScrollBarMode::kEnabled)
                          .SetContents(
                              Builder<FlexLayoutView>()
                                  .SetOrientation(LayoutOrientation::kVertical)
                                  .SetSize(gfx::Size(100, 200))
                                  .AddChildren(Builder<View>().SetProperty(kElementIdentifierKey, kScrollChild1Id).SetPreferredSize(gfx::Size(100, 100)),
                                      Builder<View>().SetProperty(kElementIdentifierKey, kScrollChild2Id).SetPreferredSize(gfx::Size(100, 100)))));
    ...
    {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(closure));
    }

    auto Post(base::OnceClosure closure)
    {
        return Do(base::BindOnce([](base::OnceClosure closure) { DoPost(std::move(closure)); }, std::move(closure)));
    }

protected:
    using ButtonCallbackMock = testing::StrictMock<base::MockCallback<Button::PressedCallback::Callback>>;
...
{
    RunTestSequence(WithView(kButton1Id, base::BindOnce([](LabelButton* button) { EXPECT_TRUE(button->GetVisible()); })),
        // Check version with arbitrary return value and matcher.
        WithView(kTabbedPaneId, base::BindOnce([](TabbedPane* tabs) { EXPECT_EQ(3U, tabs->GetTabCount()); })));
}
...
TEST_F(InteractiveViewsTestTest, CheckView)
{
    RunTestSequence(
        ...
        CheckView(kButton1Id, base::BindOnce([](LabelButton* button) { return button->GetVisible(); })),
        // Check version with arbitrary return value and matcher.
        CheckView(kTabbedPaneId, base::BindOnce([](TabbedPane* tabs) { return tabs->GetTabCount(); }), testing::Gt(2U)));
}
...
{
    RunTestSequence(CheckViewProperty(kButton1Id, &LabelButton::GetText,
...
TEST_F(InteractiveViewsTestTest, CheckViewPropertyFails)
{
    UNCALLED_MOCK_CALLBACK(ui::InteractionSequence::AbortedCallback, aborted);
    private_test_impl().set_aborted_callback_for_testing(aborted.Get());
    EXPECT_CALL_IN_SCOPE(aborted, Run, RunTestSequence(CheckViewProperty(kTabbedPaneId, &TabbedPane::GetSelectedTabIndex, testing::Eq(1U))));
}
...
TEST_F(InteractiveViewsTestTest, PollView)
{
    ...
    DoPost(base::BindLambdaForTesting([this]() { button1_->SetText(kButton2Caption); }));
    RunTestSequence(PollView(kButtonTextState, kButton1Id, [](const LabelButton* b) -> std::u16string { return b->GetText(); }),
...
TEST_F(InteractiveViewsTestTest, PollViewProperty)
{
    ...
    DEFINE_LOCAL_STATE_IDENTIFIER_VALUE(Observer, kButtonTextState);
    DoPost(base::BindLambdaForTesting([this]() { button1_->SetText(kButton2Caption); }));
    RunTestSequence(PollViewProperty(kButtonTextState, kButton1Id, &LabelButton::GetText), WaitForState(kButtonTextState, kButton2Caption));
}
...
TEST_F(InteractiveViewsTestTest, WaitForViewPropertyFails)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run, RunTestSequence(WaitForViewProperty(kButton1Id, View, Enabled, true)));
}
...
TEST_F(InteractiveViewsTestTest, WaitForViewPropertyInParallel)
{
    ...
    RunTestSequence(
        InParallel(Steps(
                       ...
                       Post(base::BindLambdaForTesting([this]() { tabs_->SetEnabled(true); })), WaitForViewProperty(kButton1Id, View, Enabled, true),
                       Post(base::BindLambdaForTesting([this]() { button1_->SetID(998); })), WaitForViewProperty(kButton1Id, View, ID, 998)),
            Steps(Post(base::BindLambdaForTesting([this]() { button1_->SetEnabled(true); })), WaitForViewProperty(kTabbedPaneId, View, Enabled, true),
                Post(base::BindLambdaForTesting([this]() { tabs_->SetID(999); })), WaitForViewProperty(kTabbedPaneId, View, ID, 999))));
}

TEST_F(InteractiveViewsTestTest, NameViewAbsoluteValue)
{
    RunTestSequence(NameView(kViewName, button2_.get()),
        WithElement(kViewName, base::BindLambdaForTesting([&](ui::TrackedElement* el) { EXPECT_EQ(button2_.get(), AsView<LabelButton>(el)); })));
}

TEST_F(InteractiveViewsTestTest, NameViewAbsoluteDeferred)
{
    View* view = nullptr;
    RunTestSequence(Do(base::BindLambdaForTesting([&]() { view = button2_.get(); })), NameView(kViewName, std::ref(view)),
        WithElement(kViewName, base::BindLambdaForTesting([&](ui::TrackedElement* el) { EXPECT_EQ(view, AsView(el)); })));
}

TEST_F(InteractiveViewsTestTest, NameViewAbsoluteCallback)
{
    RunTestSequence(NameView(kViewName, base::BindLambdaForTesting([&]() -> View* { return button2_.get(); })),
        WithElement(kViewName, base::BindLambdaForTesting([&](ui::TrackedElement* el) { EXPECT_EQ(button2_.get(), AsView<LabelButton>(el)); })));
}

TEST_F(InteractiveViewsTestTest, NameChildViewByIndex)
{
    RunTestSequence(NameChildView(kButtonsId, kViewName, 1U), WithElement(kViewName, base::BindLambdaForTesting([&](ui::TrackedElement* el) {
...
{
    EXPECT_CALL_IN_SCOPE(button2_callback_, Run,
        RunTestSequence(NameChildView(kButtonsId, kViewName, base::BindRepeating([](const View* view) {
...
{
    EXPECT_CALL_IN_SCOPE(button1_callback_, Run,
        RunTestSequence(NameDescendantView(kContentsId, kViewName,
                            base::BindRepeating([&](const View* view) { return view->GetProperty(kElementIdentifierKey) == kButton1Id; })),
            PressButton(kViewName, InputType::kMouse)));
}

TEST_F(InteractiveViewsTestTest, NameViewRelative)
{
    RunTestSequence(SelectTab(kTabbedPaneId, 1U, InputType::kTouch),
        NameViewRelative(kTabbedPaneId, kViewName, base::BindRepeating([&](TabbedPane* tabs) { return tabs->GetTabAt(1U)->contents(); })),
        WithElement(kViewName, base::BindLambdaForTesting([&](ui::TrackedElement* el) { EXPECT_EQ(kTab2Contents, AsView<Label>(el)->GetText()); })));
}
...
TEST_F(InteractiveViewsTestTest, NameChildViewFails)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run,
        RunTestSequence(NameChildView(kButtonsId, kViewName, base::BindRepeating([](const View* view) {
            const auto* const button = AsViewClass<LabelButton>(view);
            return button && button->GetText() == u"This is not a valid button caption.";
        })),
            PressButton(kViewName, InputType::kKeyboard)));
}
...
{
    EXPECT_CALLS_IN_SCOPE_2(button1_callback_, Run, button2_callback_, Run,
        RunTestSequence(NameChildViewByType<views::LabelButton>(kButtonsId, kViewName), NameChildViewByType<views::LabelButton>(kButtonsId, kViewName2, 1),
            PressButton(kViewName), PressButton(kViewName2)));
}

TEST_F(InteractiveViewsTestTest, NameDescendantViewByTypeAndIndex)
{
    RunTestSequence(NameDescendantViewByType<views::TabbedPaneTab>(kContentsId, kViewName),
        NameDescendantViewByType<views::TabbedPaneTab>(kContentsId, kViewName2, 2),
        CheckViewProperty(kViewName, &views::TabbedPaneTab::GetTitleText, kTab1Title),
        CheckViewProperty(kViewName2, &views::TabbedPaneTab::GetTitleText, kTab3Title));
}

TEST_F(InteractiveViewsTestTest, IfViewTrue)
{
    UNCALLED_MOCK_CALLBACK(base::OnceCallback<bool(const LabelButton*)>, condition);
    ...
    RunTestSequence(IfView(kButton1Id, condition.Get(), Do(step1.Get()), Do(step2.Get())));
}
...
{
    UNCALLED_MOCK_CALLBACK(base::OnceCallback<int(const LabelButton*)>, condition);
    UNCALLED_MOCK_CALLBACK(base::OnceClosure, step1);
    UNCALLED_MOCK_CALLBACK(base::OnceClosure, step2);

    EXPECT_CALL(condition, Run(button1_.get())).WillOnce(testing::Return(1));
    EXPECT_CALL(step1, Run);
    RunTestSequence(IfViewMatches(kButton1Id, condition.Get(), 1, Do(step1.Get()), Do(step2.Get())));
}
...
TEST_F(InteractiveViewsTestTest, IfViewPropertyMatchesTrue)
{
    ...
    EXPECT_CALL(step1, Run);
    RunTestSequence(IfViewPropertyMatches(kButton1Id, &LabelButton::GetText, std::u16string(kButton1Caption), Do(step1.Get()), Do(step2.Get())));
}
...
TEST_F(InteractiveViewsTestTest, IfViewPropertyMatchesFalse)
{
    ...
    EXPECT_CALL(step2, Run);
    RunTestSequence(IfViewPropertyMatches(kButton1Id, &LabelButton::GetText, testing::Ne(kButton1Caption), Do(step1.Get()), Do(step2.Get())));
}
...
{
    auto is_view = []() { return base::BindOnce([](View* actual) { return actual; }); };
    ...
    RunTestSequence(
        ...
        InParallel(CheckView(kViewName, is_view(), button1_), CheckView(kViewName2, is_view(), button2_)));
}
...
TEST_F(InteractiveViewsTestTest, BindingMethods)
{
    ...
    RunTestSequence(SelectTab(kTabbedPaneId, 1U), NameViewRelative(kTabbedPaneId, kViewName, get_second_tab),
        WithView(kViewName, [](TabbedPaneTab* tab) { EXPECT_NE(nullptr, tab); }),
        IfView(
            kViewName, [](const TabbedPaneTab* tab) { return tab != nullptr; }, Do(correct.Get()), Do(incorrect.Get())),
        IfViewMatches(
            kViewName, [this](const TabbedPaneTab* tab) { return tabs_->GetIndexOf(tab); }, 0U, Do(incorrect.Get()), Do(correct.Get())));
}
...
TEST_F(InteractiveViewsTestTest, ScrollIntoView)
{
    ...
    RunTestSequence(CheckView(kScrollChild1Id, visible, true), CheckView(kScrollChild2Id, visible, false), ScrollIntoView(kScrollChild2Id),
        CheckView(kScrollChild2Id, visible, true), ScrollIntoView(kScrollChild1Id), CheckView(kScrollChild1Id, visible, true));
}
...
class InteractiveViewsTestCompileTest : public views::test::InteractiveViewsTestTest {
public:
    ...
    {
        (void)WaitForViewProperty(views::test::kButton1Id, views::View, Enabled, true);
    }
};
