...
class InteractionTestUtilMouse::DragEnder : public aura::client::DragDropClientObserver, public aura::WindowObserver {
public:
    ...
    bool CancelDragNow()
    {
        if (auto* const client = GetClient()) {
            if (client->IsDragDropInProgress()) {
                LOG(WARNING) << "InteractionTestUtilMouse: Force-canceling spurious drag "
                                "operation.\n"
...
private:
    ...
    {
        drag_client_observation_.Reset();
        PostTask(base::BindOnce(base::IgnoreResult(&DragEnder::CancelDragNow), weak_ptr_factory_.GetWeakPtr()));
    }
    ...
    {
        return window_ ? aura::client::GetDragDropClient(window_->GetRootWindow()) : nullptr;
    }
    ...
    base::ScopedObservation<aura::client::DragDropClient, aura::client::DragDropClientObserver> drag_client_observation_ { this };
...
{
    CHECK(!performing_gestures_) << "InteractionTestUtilMouse destroyed with pending actions.";
    LOG_IF(ERROR, g_current_mouse_util != this) << "Expected |this| to be current InteractionTestUtilMouse.";
    g_current_mouse_util = nullptr;
}

// static
InteractionTestUtilMouse::MouseGesture InteractionTestUtilMouse::MoveTo(gfx::Point point)
{
    return MouseGesture(point);
}

// static
InteractionTestUtilMouse::MouseGesture InteractionTestUtilMouse::MouseDown(ui_controls::MouseButton button)
{
    return MouseGesture(std::make_pair(button, ui_controls::DOWN));
}

// static
InteractionTestUtilMouse::MouseGesture InteractionTestUtilMouse::MouseUp(ui_controls::MouseButton button)
{
    return MouseGesture(std::make_pair(button, ui_controls::UP));
}

// static
InteractionTestUtilMouse::MouseGestures InteractionTestUtilMouse::Click(ui_controls::MouseButton button)
{
    return MouseGestures { MouseDown(button), MouseUp(button) };
}

// static
InteractionTestUtilMouse::MouseGestures InteractionTestUtilMouse::DragAndHold(gfx::Point destination)
{
    return MouseGestures { MouseDown(ui_controls::LEFT), MoveTo(destination) };
}

// static
InteractionTestUtilMouse::MouseGestures InteractionTestUtilMouse::DragAndRelease(gfx::Point destination)
{
    return MouseGestures { MouseDown(ui_controls::LEFT), MoveTo(destination), MouseUp(ui_controls::LEFT) };
}
...
bool InteractionTestUtilMouse::SendButtonPress(const MouseButtonGesture& gesture, gfx::NativeWindow window_hint, bool sync, base::OnceClosure on_complete)
{
    if (sync) {
#if TOUCH_INPUT_SUPPORTED
        if (touch_mode_) {
            return ui_controls::SendTouchEventsNotifyWhenDone(
                GetTouchAction(gesture.second), GetTouchCount(gesture.first), touch_hover_point_.x(), touch_hover_point_.y(), std::move(on_complete));
        }
#endif // TOUCH_INPUT_SUPPORTED
        return ui_controls::SendMouseEventsNotifyWhenDone(gesture.first, gesture.second, std::move(on_complete), ui_controls::kNoAccelerator, window_hint);
    }

#if TOUCH_INPUT_SUPPORTED
    if (touch_mode_) {
        PostTask(base::BindOnce(
            [](base::WeakPtr<InteractionTestUtilMouse> util, base::OnceClosure on_complete, MouseButtonGesture gesture, gfx::Point target) {
                if (!util) {
                    return;
                }
                CHECK(ui_controls::SendTouchEventsNotifyWhenDone(
                    GetTouchAction(gesture.second), GetTouchCount(gesture.first), target.x(), target.y(), std::move(on_complete)));
            },
            weak_ptr_factory_.GetWeakPtr(), std::move(on_complete), gesture, touch_hover_point_));
        return true;
    }
#endif // TOUCH_INPUT_SUPPORTED

    PostTask(base::BindOnce(
        [](base::WeakPtr<InteractionTestUtilMouse> util, base::OnceClosure on_complete, MouseButtonGesture gesture, gfx::NativeWindow window_hint) {
            if (!util) {
                return;
            }
            CHECK(ui_controls::SendMouseEventsNotifyWhenDone(gesture.first, gesture.second, std::move(on_complete), ui_controls::kNoAccelerator, window_hint));
        },
        weak_ptr_factory_.GetWeakPtr(), std::move(on_complete), gesture, window_hint));

    return true;
}

bool InteractionTestUtilMouse::SendMove(const MouseMoveGesture& gesture, gfx::NativeWindow window_hint, bool sync, base::OnceClosure on_complete)
{
#if TOUCH_INPUT_SUPPORTED
    if (touch_mode_) {
    ...
    if (sync) {
        ...
        if (touch_mode_) {
            return ui_controls::SendTouchEventsNotifyWhenDone(
                ui_controls::kTouchMove, GetTouchCount(*buttons_down_.begin()), gesture.x(), gesture.y(), std::move(on_complete));
        }
#endif // TOUCH_INPUT_SUPPORTED
        return ui_controls::SendMouseMoveNotifyWhenDone(gesture.x(), gesture.y(), std::move(on_complete), window_hint);
    }

#if TOUCH_INPUT_SUPPORTED
    if (touch_mode_) {
        PostTask(base::BindOnce(
            [](base::WeakPtr<InteractionTestUtilMouse> util, base::OnceClosure on_complete, MouseMoveGesture gesture) {
                if (!util) {
                    return;
                }
                const int touch_count = GetTouchCount(*util->buttons_down_.begin());
                CHECK(ui_controls::SendTouchEventsNotifyWhenDone(ui_controls::kTouchMove, touch_count, gesture.x(), gesture.y(), std::move(on_complete)));
            },
    ...
    PostTask(base::BindOnce(
        [](base::WeakPtr<InteractionTestUtilMouse> util, base::OnceClosure on_complete, MouseMoveGesture gesture, gfx::NativeWindow window_hint) {
            if (!util) {
                return;
            }
            CHECK(ui_controls::SendMouseMoveNotifyWhenDone(gesture.x(), gesture.y(), std::move(on_complete), window_hint));
        },
        weak_ptr_factory_.GetWeakPtr(), std::move(on_complete), gesture, window_hint));
...
bool InteractionTestUtilMouse::SetTouchMode(bool touch_mode)
{
    ...
    CHECK(buttons_down_.empty()) << "Cannot toggle touch mode when buttons or fingers are down.";
#if TOUCH_INPUT_SUPPORTED
...
bool InteractionTestUtilMouse::PerformGesturesImpl(MouseGestures gestures, gfx::NativeWindow window_hint)
{
    ...
    for (auto& gesture : gestures) {
        ...
        if (MouseButtonGesture* const button = std::get_if<MouseButtonGesture>(&gesture)) {
            switch (button->second) {
            case ui_controls::UP: {
                CHECK(buttons_down_.erase(button->first));
                base::OnceClosure on_complete = force_async ? base::DoNothing() : run_loop.QuitClosure();
                if (ShouldCancelDrag()) {
                    ...
                    on_complete = std::move(on_complete).Then(base::BindOnce(&InteractionTestUtilMouse::CancelFutureDrag, weak_ptr_factory_.GetWeakPtr()));
                }
#if defined(USE_AURA)
                dragging_ = false;
#endif
                if (!SendButtonPress(*button, window_hint, !force_async, std::move(on_complete))) {
            ...
            case ui_controls::DOWN:
#if TOUCH_INPUT_SUPPORTED
                CHECK(!touch_mode_ || buttons_down_.empty()) << "In touch mode, only one set of fingers may be down at any "
                                                                "given time.";
#endif
                ...
                if (!force_async && button->first == ui_controls::RIGHT) {
                    force_async = true;
                    LOG(WARNING) << "InteractionTestUtilMouse::PerformGestures(): "
                                    "Important note:\n"
...
#endif
                CancelDragNow();
                if (!SendButtonPress(*button, window_hint, !force_async, force_async ? base::DoNothing() : run_loop.QuitClosure())) {
...
#endif
            if (!SendMove(move, window_hint, !force_async, force_async ? base::DoNothing() : run_loop.QuitClosure())) {
...
void InteractionTestUtilMouse::CancelAllGestures()
{
    ...
    if (touch_mode_ && !buttons_down_.empty()) {
        ...
        if (!ui_controls::SendTouchEvents(ui_controls::kTouchRelease, GetTouchCount(button), touch_hover_point_.x(), touch_hover_point_.y())) {
...
{
    CHECK(window);
    CHECK(!g_current_mouse_util) << "Cannot have multiple overlapping InteractionTestUtilMouse instances";
    g_current_mouse_util = this;
}

// static
void InteractionTestUtilMouse::AddGestures(MouseGestures& gestures, MouseGesture to_add)
{
    gestures.emplace_back(std::move(to_add));
}

// static
void InteractionTestUtilMouse::AddGestures(MouseGestures& gestures, MouseGestures to_add)
{
