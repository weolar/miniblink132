...
class InteractionSequenceViewsTest : public ViewsTestBase, public testing::WithParamInterface<ui::InteractionSequence::StepStartMode> {
public:
    ...
    {
        return view ? ElementTrackerViews::GetInstance()->GetElementForView(view) : nullptr;
    }
    ...
    {
        auto delegate = std::make_unique<BubbleDialogDelegateView>(contents(), BubbleBorder::Arrow::TOP_LEFT);
        ...
        bubble_widget_ = BubbleDialogDelegateView::CreateBubble(std::move(delegate));
    ...
    {
        ui::ElementTracker::GetFrameworkDelegate()->NotifyElementActivated(ElementTrackerViews::GetInstance()->GetElementForView(view));
    }
    ...
    {
        return std::move(ui::InteractionSequence::Builder().SetDefaultStepStartMode(GetParam()));
    }
...
INSTANTIATE_TEST_SUITE_P(, InteractionSequenceViewsTest,
    testing::Values(ui::InteractionSequence::StepStartMode::kImmediate, ui::InteractionSequence::StepStartMode::kAsynchronous),
    [](const testing::TestParamInfo<ui::InteractionSequence::StepStartMode>& mode) {
...
TEST_P(InteractionSequenceViewsTest, DestructWithInitialViewAborts)
{
    ...
    auto* const starting_view = contents()->AddChildView(std::make_unique<View>());
    ...
    auto sequence
        = Builder()
              ...
              .AddStep(ui::InteractionSequence::StepBuilder().SetElementID(kTestElementID).SetType(ui::InteractionSequence::StepType::kActivated).Build())
              .Build();
    sequence->Start();
    EXPECT_ASYNC_CALL_IN_SCOPE(aborted, Run, contents()->RemoveChildViewT(starting_view));
}
...
TEST_P(InteractionSequenceViewsTest, SequenceNotCanceledDueToViewDestroyedIfRequirementChanged)
{
    UNCALLED_MOCK_CALLBACK(ui::InteractionSequence::AbortedCallback, aborted);
    UNCALLED_MOCK_CALLBACK(ui::InteractionSequence::CompletedCallback, completed);
    UNCALLED_MOCK_CALLBACK(ui::InteractionSequence::StepStartCallback, step2_start);
    UNCALLED_MOCK_CALLBACK(ui::InteractionSequence::StepEndCallback, step2_end);
    UNCALLED_MOCK_CALLBACK(ui::InteractionSequence::StepStartCallback, step3_start);
    auto* const starting_view = contents()->AddChildView(std::make_unique<View>());
    starting_view->SetProperty(kElementIdentifierKey, kTestElementID);
    auto sequence
        = Builder()
              ...
              .SetCompletedCallback(completed.Get())
              .AddStep(InteractionSequenceViews::WithInitialView(starting_view))
              .AddStep(ui::InteractionSequence::StepBuilder().SetElementID(kTestElementID2).SetType(ui::InteractionSequence::StepType::kShown).Build())
    ...
    EXPECT_ASYNC_CALL_IN_SCOPE(step2_start, Run(sequence.get(), ViewToElement(second_view)), Activate(second_view));
    ...
    EXPECT_ASYNC_CALLS_IN_SCOPE_3(step2_end, Run, step3_start, Run, completed, Run, third_view->SetVisible(true));
}
...
TEST_P(InteractionSequenceViewsTest, MAYBE_TransitionToBubble)
{
    ...
    auto sequence = Builder()
                        ...
                        .AddStep(ui::InteractionSequence::StepBuilder()
                                     ...
                                     .Build())
                        .Build();
    auto* const button = contents()->AddChildView(std::make_unique<LabelButton>(
        Button::PressedCallback(base::BindRepeating(&InteractionSequenceViewsTest::ShowBubble, base::Unretained(this), kTestElementID2))));
    button->SetProperty(kElementIdentifierKey, kTestElementID);
    button->SetAccessibleName(u"Button");
    sequence->Start();

    EXPECT_ASYNC_CALLS_IN_SCOPE_2(step, Run, step2, Run, views::test::InteractionTestUtilSimulatorViews::PressButton(button));

    EXPECT_ASYNC_CALLS_IN_SCOPE_2(step3, Run, completed, Run, views::test::InteractionTestUtilSimulatorViews::PressButton(label_button_));
}
...
// TODO(crbug.com/42050042): Re-enable the tests once we get rid of swiftshader.
#if BUILDFLAG(IS_FUCHSIA) && !defined(NDEBUG) && defined(ARCH_CPU_ARM64)
#define MAYBE_NameView_NameViewWithIdentifier DISABLED_NameView_NameViewWithIdentifier
#else
...
TEST_P(InteractionSequenceViewsTest, MAYBE_NameView_NameViewWithIdentifier)
{
    ...
    UNCALLED_MOCK_CALLBACK(ui::InteractionSequence::StepStartCallback, step);
    UNCALLED_MOCK_CALLBACK(ui::InteractionSequence::StepStartCallback, step3);
    auto step2 = base::BindLambdaForTesting([&](ui::InteractionSequence* sequence, ui::TrackedElement* element) {
    ...
    EXPECT_ASYNC_CALL_IN_SCOPE(step, Run, views::test::InteractionTestUtilSimulatorViews::PressButton(button));
...
// TODO(crbug.com/42050042): Re-enable the tests once we get rid of swiftshader.
#if BUILDFLAG(IS_FUCHSIA) && !defined(NDEBUG) && defined(ARCH_CPU_ARM64)
#define MAYBE_NameView_NameViewWithNoIdentifier DISABLED_NameView_NameViewWithNoIdentifier
#else
#define MAYBE_NameView_NameViewWithNoIdentifier NameView_NameViewWithNoIdentifier
#endif
TEST_P(InteractionSequenceViewsTest, MAYBE_NameView_NameViewWithNoIdentifier)
{
    ...
    EXPECT_ASYNC_CALLS_IN_SCOPE_2(step3, Run, completed, Run, views::test::InteractionTestUtilSimulatorViews::PressButton(no_id_view_));
}
