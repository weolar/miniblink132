...
public:
    using NativeWidgetFactory = base::RepeatingCallback<NativeWidget*(const Widget::InitParams&, internal::NativeWidgetDelegate*)>;
#if BUILDFLAG(IS_WIN)
    ...
    // specified name.
    virtual void SaveWindowPlacement(const Widget* widget, const std::string& window_name, const gfx::Rect& bounds, ui::mojom::WindowShowState show_state);

    // Retrieves the saved position and size and "show" state for the window with
    // the specified name.
    virtual bool GetSavedWindowPlacement(const Widget* widget, const std::string& window_name, gfx::Rect* bounds, ui::mojom::WindowShowState* show_state) const;
    ...
    virtual void NotifyMenuItemFocused(const std::u16string& menu_name, const std::u16string& menu_item_name, int item_index, int item_count, bool has_submenu);
    ...
    // will be returned.
    virtual ProcessMenuAcceleratorResult ProcessAcceleratorWhileMenuShowing(const ui::Accelerator& accelerator);
...
#elif BUILDFLAG(ENABLE_DESKTOP_AURA) && (BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS))
    ...
    virtual std::unique_ptr<NonClientFrameView> CreateDefaultNonClientFrameView(Widget* widget);
    ...
    virtual void OnBeforeWidgetInit(Widget::InitParams* params, internal::NativeWidgetDelegate* delegate);
    ...
    virtual int GetAppbarAutohideEdges(HMONITOR monitor, base::OnceClosure callback);
#endif
...
#if defined(USE_AURA)
    void SetTouchSelectionMenuRunner(std::unique_ptr<TouchSelectionMenuRunnerViews> menu_runner);
#endif
