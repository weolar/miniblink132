...
gfx::Rect NativeFrameView::GetWindowBoundsForClientBounds(const gfx::Rect& client_bounds) const
{
#if BUILDFLAG(IS_WIN)
    return views::GetWindowBoundsForClientBounds(static_cast<View*>(const_cast<NativeFrameView*>(this)), client_bounds);
#else
...
void NativeFrameView::GetWindowMask(const gfx::Size& size, SkPath* window_mask)
{
...
gfx::Size NativeFrameView::CalculatePreferredSize(const SizeBounds& available_size) const
{
    gfx::Size client_preferred_size = frame_->client_view()->GetPreferredSize(available_size);
#if BUILDFLAG(IS_WIN)
...
#else
    return frame_->non_client_view()->GetWindowBoundsForClientBounds(gfx::Rect(client_preferred_size)).size();
#endif
}
