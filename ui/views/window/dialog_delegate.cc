...
public:
    ...
    Widget* GetPrimaryWindowWidget() override
    {
        ...
        // no parent.
        return parent() ? parent()->GetPrimaryWindowWidget() : Widget::GetPrimaryWindowWidget();
    }
...
bool HasCallback(const absl::variant<base::OnceClosure, base::RepeatingCallback<bool()>>& callback)
{
    return absl::visit([](const auto& variant) { return static_cast<bool>(variant); }, callback);
}
...
{
    WidgetDelegate::RegisterWindowWillCloseCallback(base::BindOnce(&DialogDelegate::WindowWillClose, base::Unretained(this)));
}

// static
Widget* DialogDelegate::CreateDialogWidget(WidgetDelegate* delegate, gfx::NativeWindow context, gfx::NativeView parent)
{
    views::Widget* widget = new DialogWidget;
    views::Widget::InitParams params = GetDialogWidgetInitParams(delegate, context, parent, gfx::Rect());
    widget->Init(std::move(params));
    return widget;
}

// static
Widget* DialogDelegate::CreateDialogWidget(std::unique_ptr<WidgetDelegate> delegate, gfx::NativeWindow context, gfx::NativeView parent)
{
...
Widget::InitParams DialogDelegate::GetDialogWidgetInitParams(
    WidgetDelegate* delegate, gfx::NativeWindow context, gfx::NativeView parent, const gfx::Rect& bounds)
{
    DialogDelegate* dialog = delegate->AsDialogDelegate();

    views::Widget::InitParams params(dialog ? dialog->ownership_of_new_widget_ : Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET);
    ...
    params.child = parent && (delegate->GetModalType() == ui::mojom::ModalType::kChild);
#endif
...
std::u16string DialogDelegate::GetDialogButtonLabel(ui::mojom::DialogButton button) const
{
    ...
    return buttons() & static_cast<int>(ui::mojom::DialogButton::kOk) ? l10n_util::GetStringUTF16(IDS_APP_CANCEL) : l10n_util::GetStringUTF16(IDS_APP_CLOSE);
}

ui::ButtonStyle DialogDelegate::GetDialogButtonStyle(ui::mojom::DialogButton button) const
{
    std::optional<ui::ButtonStyle> style = GetParams().button_styles[static_cast<size_t>(button)];
    if (style.has_value()) {
        return *style;
    }

    return GetIsDefault(button) ? ui::ButtonStyle::kProminent : ui::ButtonStyle::kDefault;
}

bool DialogDelegate::GetIsDefault(ui::mojom::DialogButton button) const
{
    return GetDefaultDialogButton() == static_cast<int>(button)
        && (button != ui::mojom::DialogButton::kCancel || PlatformStyle::kDialogDefaultButtonCanBeCancel);
}

bool DialogDelegate::IsDialogButtonEnabled(ui::mojom::DialogButton button) const
{
    return params_.enabled_buttons & static_cast<int>(button);
}

bool DialogDelegate::ShouldIgnoreButtonPressedEventHandling(View* button, const ui::Event& event) const
{
    return false;
}
...
bool DialogDelegate::RunCloseCallback(absl::variant<base::OnceClosure, base::RepeatingCallback<bool()>>& callback)
{
    DCHECK(!already_started_close_);
    if (absl::holds_alternative<base::OnceClosure>(callback)) {
        ...
        already_started_close_ = absl::get<base::RepeatingCallback<bool()>>(callback).Run();
    }
...
std::unique_ptr<NonClientFrameView> DialogDelegate::CreateNonClientFrameView(Widget* widget)
{
    return use_custom_frame() ? CreateDialogFrameView(widget) : WidgetDelegate::CreateNonClientFrameView(widget);
}
...
void DialogDelegate::WindowWillClose()
{
    ...
    const bool new_callback_present = close_callback_ || HasCallback(cancel_callback_) || HasCallback(accept_callback_);
    ...
    if (close_callback_) {
        ...
        absl::variant<base::OnceClosure, base::RepeatingCallback<bool()>> close_callback_wrapped(std::move(close_callback_));
...
// static
std::unique_ptr<NonClientFrameView> DialogDelegate::CreateDialogFrameView(Widget* widget)
{
    LayoutProvider* provider = LayoutProvider::Get();
    auto frame = std::make_unique<BubbleFrameView>(provider->GetInsetsMetric(INSETS_DIALOG_TITLE), gfx::Insets());

    const BubbleBorder::Shadow kShadow = BubbleBorder::DIALOG_SHADOW;
    std::unique_ptr<BubbleBorder> border = std::make_unique<BubbleBorder>(BubbleBorder::FLOAT, kShadow);
...
{
    return const_cast<DialogClientView*>(const_cast<const DialogDelegate*>(this)->GetDialogClientView());
}
...
BubbleFrameView* DialogDelegate::GetBubbleFrameView() const
{
    ...
    const NonClientView* view = GetWidget() ? GetWidget()->non_client_view() : nullptr;
...
void DialogDelegate::SetButtonEnabled(ui::mojom::DialogButton dialog_button, bool enabled)
{
...
void DialogDelegate::SetButtonLabel(ui::mojom::DialogButton button, std::u16string label)
{
...
void DialogDelegate::SetButtonStyle(ui::mojom::DialogButton button, std::optional<ui::ButtonStyle> style)
{
...
void DialogDelegate::SetAcceptCallbackWithClose(base::RepeatingCallback<bool()> callback)
{
    accept_callback_ = std::move(callback);
}
...
void DialogDelegate::SetCancelCallbackWithClose(base::RepeatingCallback<bool()> callback)
{
    cancel_callback_ = std::move(callback);
}
...
void DialogDelegate::SetOwnershipOfNewWidget(Widget::InitParams::Ownership ownership)
{
    CHECK(!GetWidget());
...
void DialogDelegate::CancelDialog()
{
    ...
    already_started_close_ = true;
    GetWidget()->CloseWithReason(views::Widget::ClosedReason::kCancelButtonClicked);
}
...
int DialogDelegate::GetCornerRadius() const
{
    ...
    return LayoutProvider::Get()->GetCornerRadiusMetric(views::ShapeContextTokens::kDialogRadius);
}
