...
// rings.
class FrameCaptionButton::HighlightPathGenerator : public views::HighlightPathGenerator {
public:
    ...
    std::optional<gfx::RRectF> GetRoundRect(const gfx::RectF& rect) override
    {
        ...
        return gfx::RRectF(gfx::RectF(bounds), frame_caption_button_->GetInkDropCornerRadius());
    }
...
FrameCaptionButton::FrameCaptionButton(PressedCallback callback, CaptionButtonIcon icon, int hit_test_type)
    ...
    , swap_images_animation_(std::make_unique<gfx::SlideAnimation>(this))
{
    ...
    InkDrop::Get(this)->SetCreateRippleCallback(base::BindRepeating(
        [](FrameCaptionButton* host) -> std::unique_ptr<views::InkDropRipple> {
            return std::make_unique<views::FloodFillInkDropRipple>(InkDrop::Get(host), host->size(), host->GetInkdropInsets(host->size()),
                InkDrop::Get(host)->GetInkDropCenterBasedOnLastEvent(), InkDrop::Get(host)->GetBaseColor(), InkDrop::Get(host)->GetVisibleOpacity());
        },
        this));

    views::HighlightPathGenerator::Install(this, std::make_unique<HighlightPathGenerator>(this));
...
SkColor FrameCaptionButton::GetButtonColor(SkColor background_color)
{
    ...
    // or dark.
    const SkColor default_foreground = color_utils::IsDark(background_color) ? gfx::kGoogleGrey200 : gfx::kGoogleGrey700;
    const SkColor high_contrast_foreground = color_utils::GetColorWithMaxContrast(background_color);
    return color_utils::BlendForMinContrast(default_foreground, background_color, high_contrast_foreground, color_utils::kMinimumVisibleContrastRatio).color;
}
...
void FrameCaptionButton::SetImage(CaptionButtonIcon icon, Animate animate, const gfx::VectorIcon& icon_definition)
{
    ...
    gfx::ImageSkia new_icon_image = gfx::CreateVectorIcon(icon_definition, icon_color);
    ...
    if (icon == icon_ && (animate == Animate::kYes || !swap_images_animation_->is_animating()) && new_icon_image.BackedBySameObjectAs(icon_image_)) {
        return;
    }
...
void FrameCaptionButton::OnGestureEvent(ui::GestureEvent* event)
{
    ...
    if (event->type() == ui::EventType::kGestureScrollBegin || event->type() == ui::EventType::kGestureScrollUpdate) {
...
{
    if (absl::holds_alternative<SkColor>(color_) && absl::get<SkColor>(color_) == background_color) {
        return;
    }
...
{
    if (absl::holds_alternative<ui::ColorId>(color_) && absl::get<ui::ColorId>(color_) == icon_color_id) {
        return;
    }
...
base::CallbackListSubscription FrameCaptionButton::AddBackgroundColorChangedCallback(PropertyChangedCallback callback)
{
...
void FrameCaptionButton::DrawHighlight(gfx::Canvas* canvas, cc::PaintFlags flags)
{
...
void FrameCaptionButton::DrawIconContents(gfx::Canvas* canvas, gfx::ImageSkia image, int x, int y, cc::PaintFlags flags)
{
...
gfx::Insets FrameCaptionButton::GetInkdropInsets(const gfx::Size& button_size) const
{
    return gfx::Insets::VH((button_size.height() - GetInkDropSize().height()) / 2, (button_size.width() - GetInkDropSize().width()) / 2);
}
...
void FrameCaptionButton::PaintButtonContents(gfx::Canvas* canvas)
{
    ...
    if (hover_animation().is_animating()) {
        highlight_alpha = static_cast<SkAlpha>(hover_animation().CurrentValueBetween(SK_AlphaTRANSPARENT, kHighlightVisibleOpacity));
    ...
    SkAlpha icon_alpha = static_cast<SkAlpha>(swap_images_animation_->CurrentValueBetween(SK_AlphaTRANSPARENT, SK_AlphaOPAQUE));
    SkAlpha crossfade_icon_alpha = 0;
    if (icon_alpha < base::ClampRound<SkAlpha>(kFadeOutRatio * SK_AlphaOPAQUE)) {
        crossfade_icon_alpha = base::ClampRound<SkAlpha>(SK_AlphaOPAQUE - icon_alpha / kFadeOutRatio);
    }
    ...
    if (crossfade_icon_alpha > 0 && !crossfade_icon_image_.isNull()) {
        ...
        DrawIconContents(canvas, crossfade_icon_image_, icon_bounds_x, icon_bounds_y, flags);
        canvas->Restore();
    } else {
...
SkAlpha FrameCaptionButton::GetAlphaForIcon(SkAlpha base_alpha) const
{
    ...
    if (hover_animation().is_animating()) {
        inactive_alpha = hover_animation().CurrentValueBetween(inactive_alpha, 1.0f);
...
void FrameCaptionButton::UpdateInkDropBaseColor()
{
    ...
    const SkColor button_color = absl::holds_alternative<ui::ColorId>(color_) ? GetColorProvider()->GetColor(absl::get<ui::ColorId>(color_))
...
DEFINE_ENUM_CONVERTERS(views::CaptionButtonIcon, { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_MINIMIZE, u"CAPTION_BUTTON_ICON_MINIMIZE" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_MAXIMIZE_RESTORE, u"CAPTION_BUTTON_ICON_MAXIMIZE_RESTORE" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_CLOSE, u"CAPTION_BUTTON_ICON_CLOSE" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_LEFT_TOP_SNAPPED, u"CAPTION_BUTTON_ICON_LEFT_TOP_SNAPPED" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_RIGHT_BOTTOM_SNAPPED, u"CAPTION_BUTTON_ICON_RIGHT_BOTTOM_SNAPPED" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_BACK, u"CAPTION_BUTTON_ICON_BACK" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_LOCATION, u"CAPTION_BUTTON_ICON_LOCATION" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_MENU, u"CAPTION_BUTTON_ICON_MENU" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_ZOOM, u"CAPTION_BUTTON_ICON_ZOOM" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_CENTER, u"CAPTION_BUTTON_ICON_CENTER" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_FLOAT, u"CAPTION_BUTTON_ICON_FLOAT" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_CUSTOM, u"CAPTION_BUTTON_ICON_CUSTOM" },
    { views::CaptionButtonIcon::CAPTION_BUTTON_ICON_COUNT, u"CAPTION_BUTTON_ICON_COUNT" })
