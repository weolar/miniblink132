...
// atop the other.
gfx::Size GetBoundingSizeForVerticalStack(const gfx::Size& size1, const gfx::Size& size2)
{
    return gfx::Size(std::max(size1.width(), size2.width()), size1.height() + size2.height());
}
...
public:
    explicit ButtonRowContainer(DialogClientView* owner)
        : owner_(owner)
    {
    }
    ...
    , input_protector_(std::make_unique<views::InputEventActivationProtector>())
{
    ...
    button_row_container_ = AddChildView(std::make_unique<ButtonRowContainer>(this));
...
gfx::Size DialogClientView::CalculatePreferredSize(const SizeBounds& available_size) const
{
    ...
    if (fixed_width) {
        ...
        contents_size = ClientView::CalculatePreferredSize(views::SizeBounds(content_width, {}));
        contents_size.set_width(content_width);
    } else {
        SizeBounds content_available_size(available_size);
        content_available_size.Enlarge(-content_margins.width(), -content_margins.height());
...
gfx::Size DialogClientView::GetMinimumSize() const
{
    if (GetDialogDelegate()->fixed_width()) {
        return CalculatePreferredSize(SizeBounds(GetDialogDelegate()->fixed_width(), {}));
    }

    return GetBoundingSizeForVerticalStack(ClientView::GetMinimumSize(), button_row_container_->GetMinimumSize());
}

gfx::Size DialogClientView::GetMaximumSize() const
{
    constexpr int kUnconstrained = 0;
    DCHECK(gfx::Size(kUnconstrained, kUnconstrained) == button_row_container_->GetMaximumSize());
...
ProposedLayout DialogClientView::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
    ProposedLayout layouts;
    DCHECK(size_bounds.is_fully_bounded());
    const int container_height = button_row_container_->GetHeightForWidth(size_bounds.width().value());
    const int container_y = size_bounds.height().value() - container_height;
    layouts.child_layouts.emplace_back(button_row_container_.get(), button_row_container_->GetVisible(),
        gfx::Rect(0, container_y, size_bounds.width().value(), container_height), size_bounds);
    if (contents_view()) {
        gfx::Rect contents_bounds(size_bounds.width().value(), container_y);
        contents_bounds.Inset(GetDialogDelegate()->margins());
        layouts.child_layouts.emplace_back(contents_view(), contents_view()->GetVisible(), contents_bounds, size_bounds);
    }
...
void DialogClientView::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details)
{
...
void DialogClientView::UpdateBackground()
{
    ...
    if (dialog && !dialog->use_custom_frame()) {
        SetBackground(views::CreateRoundedRectBackground(GetColorProvider()->GetColor(background_color_id_), background_radii_));
    }
}
...
void DialogClientView::UpdateDialogButton(raw_ptr<MdTextButton>* member, ui::mojom::DialogButton type)
{
    DialogDelegate* const delegate = GetDialogDelegate();
    ...
    const int minimum_width = LayoutProvider::Get()->GetDistanceMetric(views::DISTANCE_DIALOG_BUTTON_MINIMUM_WIDTH);

    Builder<View>(button_row_container_)
        .AddChild(Builder<MdTextButton>()
                      .CopyAddressTo(member)
                      .SetCallback(base::BindRepeating(&DialogClientView::ButtonPressed, base::Unretained(this), type))
                      .SetText(title)
...
void DialogClientView::ButtonPressed(ui::mojom::DialogButton type, const ui::Event& event)
{
    DialogDelegate* const delegate = GetDialogDelegate();
    ...
    DCHECK(type == ui::mojom::DialogButton::kOk || type == ui::mojom::DialogButton::kCancel);
    if (type == ui::mojom::DialogButton::kOk && !delegate->ShouldIgnoreButtonPressedEventHandling(ok_button_, event)) {
        delegate->AcceptDialog();
    }

    if (type == ui::mojom::DialogButton::kCancel && !delegate->ShouldIgnoreButtonPressedEventHandling(cancel_button_, event)) {
        delegate->CancelDialog();
    }
}
...
int DialogClientView::GetExtraViewSpacing() const
{
    ...
    return LayoutProvider::Get()->GetDistanceMetric(views::DISTANCE_RELATED_BUTTON_HORIZONTAL);
}

std::array<View*, DialogClientView::kNumButtons> DialogClientView::GetButtonRowViews()
{
...
void DialogClientView::UpdateExtraViewFromDelegate()
{
    ...
    extra_view_ = button_row_container_->AddChildViewAt(std::move(new_extra_view), 0);
...
void DialogClientView::SetupLayout()
{
    ...
    // it may already have.
    auto* layout = button_row_container_->SetLayoutManager(std::make_unique<views::TableLayout>());
    ...
    if (extra_view_) {
        ...
        extra_view_->SetProperty(kViewIgnoredByLayoutKey, !extra_view_->GetVisible());
    }
    ...
    const int button_spacing = (ok_button_ && cancel_button_) ? layout_provider->GetDistanceMetric(DISTANCE_RELATED_BUTTON_HORIZONTAL) : 0;
    ...
    layout->SetMinimumSize(minimum_size_)
        ...
        .AddColumn(LayoutAlignment::kStretch, LayoutAlignment::kStretch, kFixed, TableLayout::ColumnSize::kUsePreferred, 0, 0)
        .AddPaddingColumn(kStretchy, GetExtraViewSpacing())
        .AddColumn(LayoutAlignment::kStretch, LayoutAlignment::kEnd, kFixed, TableLayout::ColumnSize::kUsePreferred, 0, 0)
        ...
        .SetLinkedColumnSizeLimit(layout_provider->GetDistanceMetric(DISTANCE_BUTTON_MAX_LINKABLE_WIDTH));
    ...
    auto should_link = [](views::View* view) { return IsViewClass<Button>(view) && !IsViewClass<Checkbox>(view) && !IsViewClass<ImageButton>(view); };
    ...
    if (previously_focused_view && !focus_manager->GetFocusedView() && Contains(previously_focused_view)) {
...
void DialogClientView::AddFillerView(size_t view_index)
{
    ...
    if (!filler) {
        filler = button_row_container_->AddChildViewAt(std::make_unique<View>(), std::min(button_row_container_->children().size(), view_index));
    }
}
