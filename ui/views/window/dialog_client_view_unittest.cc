...
public:
    ...
    void SetUp() override
    {
        ...
        layout_provider_->SetDistanceMetric(DISTANCE_BUTTON_MAX_LINKABLE_WIDTH, 200);
    }
...
protected:
    ...
    void SetDialogButtonLabel(ui::mojom::DialogButton button, const std::u16string& label)
    {
    ...
    template <typename T> T* SetExtraView(std::unique_ptr<T> view)
    {
    ...
    void SetSizeConstraints(const gfx::Size& min_size, const gfx::Size& preferred_size, const gfx::Size& max_size)
    {
        min_size_ = min_size;
    ...
    View* FocusableViewAfter(View* view)
    {
        ...
        return delegate_->GetFocusManager()->GetNextFocusableView(view, delegate_->GetWidget(), reverse, dont_loop);
    }
    ...
    {
        delegate_->SetButtonLabel(ui::mojom::DialogButton::kCancel, u"Cancel Cancel Cancel");
        delegate_->DialogModelChanged();
    }
...
private:
    ...
    public:
        ...
        {
            return parent_->min_size_;
        }
        gfx::Size GetMaximumSize() const override
        {
            return parent_->max_size_;
        }
...
TEST_F(DialogClientViewTest, UpdateButtons)
{
    ...
    SetSizeConstraints(gfx::Size(200, 100), gfx::Size(300, 200), gfx::Size(400, 300));
    // This dialog should start with no buttons.
    EXPECT_EQ(delegate()->buttons(), static_cast<int>(ui::mojom::DialogButton::kNone));
    ...
    SetDialogButtons(static_cast<int>(ui::mojom::DialogButton::kOk) | static_cast<int>(ui::mojom::DialogButton::kCancel));
    ...
    EXPECT_EQ(nullptr, client_view()->ok_button());
    EXPECT_EQ(client_view()->cancel_button()->GetIsDefault(), PlatformStyle::kDialogDefaultButtonCanBeCancel);
    EXPECT_EQ(GetUpdatedClientBounds().height(), height_with_buttons);
}
...
TEST_F(DialogClientViewTest, SetupFocusChain)
{
    ...
    EXPECT_EQ(delegate()->GetContentsView(), FocusableViewAfter(delegate()->GetContentsView()));

    // Add OK and cancel buttons.
    SetDialogButtons(static_cast<int>(ui::mojom::DialogButton::kOk) | static_cast<int>(ui::mojom::DialogButton::kCancel));

    if (kIsOkButtonOnLeftSide) {
        EXPECT_EQ(client_view()->ok_button(), FocusableViewAfter(delegate()->GetContentsView()));
        EXPECT_EQ(client_view()->cancel_button(), FocusableViewAfter(client_view()->ok_button()));
        EXPECT_EQ(delegate()->GetContentsView(), FocusableViewAfter(client_view()->cancel_button()));
    } else {
        EXPECT_EQ(client_view()->cancel_button(), FocusableViewAfter(delegate()->GetContentsView()));
        EXPECT_EQ(client_view()->ok_button(), FocusableViewAfter(client_view()->cancel_button()));
        EXPECT_EQ(delegate()->GetContentsView(), FocusableViewAfter(client_view()->ok_button()));
    }

    // Add extra view and remove OK button.
    View* extra_view = SetExtraView(std::make_unique<StaticSizedView>(gfx::Size(200, 200)));
    ...
    EXPECT_TRUE(!client_view()->cancel_button()->GetNextFocusableView()
        || client_view()->cancel_button()->GetNextFocusableView()->GetFocusBehavior() == View::FocusBehavior::NEVER);
}
...
TEST_F(DialogClientViewTest, LayoutWithButtons)
{
    ...
    EXPECT_LT(delegate()->GetContentsView()->bounds().bottom(), client_view()->bounds().bottom());
...
TEST_F(DialogClientViewTest, MinMaxPreferredSize)
{
    ...
    SetDialogButtons(static_cast<int>(ui::mojom::DialogButton::kOk) | static_cast<int>(ui::mojom::DialogButton::kCancel));
    EXPECT_EQ(gfx::Size(buttons_size.width(), 15 + buttons_size.height()), client_view()->GetMinimumSize());
    EXPECT_EQ(gfx::Size(buttons_size.width(), 25 + buttons_size.height()), client_view()->GetPreferredSize({}));
    EXPECT_EQ(gfx::Size(300, 350 + buttons_size.height()), client_view()->GetMaximumSize());

    // If the contents view gets bigger, it should take over the width.
    SetSizeConstraints(gfx::Size(400, 450), gfx::Size(500, 550), gfx::Size(600, 650));
    EXPECT_EQ(gfx::Size(400, 450 + buttons_size.height()), client_view()->GetMinimumSize());
    EXPECT_EQ(gfx::Size(500, 550 + buttons_size.height()), client_view()->GetPreferredSize({}));
    EXPECT_EQ(gfx::Size(600, 650 + buttons_size.height()), client_view()->GetMaximumSize());
}
...
TEST_F(DialogClientViewTest, LinkedWidthDoesLink)
{
    ...
    SetDialogButtons(static_cast<int>(ui::mojom::DialogButton::kCancel) | static_cast<int>(ui::mojom::DialogButton::kOk));
    CheckContentsIsSetToPreferredSize();
    ...
    View* extra_button = SetExtraView(std::make_unique<LabelButton>(Button::PressedCallback(), std::u16string()));
    CheckContentsIsSetToPreferredSize();
...
TEST_F(DialogClientViewTest, LinkedWidthDoesntLink)
{
    ...
    View* extra_button = SetExtraView(std::make_unique<Checkbox>(std::u16string()));
    CheckContentsIsSetToPreferredSize();
...
TEST_F(DialogClientViewTest, ButtonPosition)
{
    ...
    SetSizeConstraints(gfx::Size(), gfx::Size(contents_width, contents_height), gfx::Size(666, 666));
    SetDialogButtons(static_cast<int>(ui::mojom::DialogButton::kOk));
    SizeAndLayoutWidget();
    EXPECT_EQ(contents_width - button_row_inset, client_view()->ok_button()->bounds().right());
    EXPECT_EQ(contents_height + button_row_inset, delegate()->height() + client_view()->ok_button()->y());
}
...
TEST_F(DialogClientViewTest, IgnorePossiblyUnintendedClicks_ClickAfterShown)
{
    ...
    ui::MouseEvent mouse_event(ui::EventType::kMousePressed, gfx::PointF(), gfx::PointF(), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ...
    cancel_button.NotifyClick(ui::MouseEvent(ui::EventType::kMousePressed, gfx::PointF(), gfx::PointF(),
...
TEST_F(DialogClientViewTest, IgnorePossiblyUnintendedClicks_TapAfterShown)
{
    ...
    ui::GestureEvent tap_event(0, 0, 0, ui::EventTimeForNow(), ui::GestureEventDetails(ui::EventType::kGestureTap));
    ...
    ui::GestureEvent tap_event2(
        0, 0, 0, ui::EventTimeForNow() + base::Milliseconds(GetDoubleClickInterval()), ui::GestureEventDetails(ui::EventType::kGestureTap));
...
TEST_F(DialogClientViewTest, IgnorePossiblyUnintendedClicks_TouchAfterShown)
{
    ...
    ui::TouchEvent touch_event(
        ui::EventType::kTouchPressed, gfx::PointF(), gfx::PointF(), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch));
    test::ButtonTestApi(client_view()->ok_button()).NotifyClick(touch_event);
    test::ButtonTestApi cancel_button(client_view()->cancel_button());
    cancel_button.NotifyClick(touch_event);
    EXPECT_FALSE(widget()->IsClosed());

    ui::TouchEvent touch_event2(ui::EventType::kTouchPressed, gfx::PointF(), gfx::PointF(),
        ui::EventTimeForNow() + base::Milliseconds(GetDoubleClickInterval()), ui::PointerDetails(ui::EventPointerType::kTouch));
...
TEST_F(DesktopDialogClientViewTest, IgnorePossiblyUnintendedClicks_TopLevelWindowBoundsChanged)
{
    SetDialogButtons(static_cast<int>(ui::mojom::DialogButton::kCancel) | static_cast<int>(ui::mojom::DialogButton::kOk));
    SizeAndLayoutWidget();
    widget()->Show();
    task_environment()->FastForwardBy(base::Milliseconds(GetDoubleClickInterval() * 2));
    ...
    cancel_button.NotifyClick(ui::MouseEvent(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(),
        ui::EventTimeForNow() + base::Milliseconds(GetDoubleClickInterval()), ui::EF_NONE, ui::EF_NONE));
    EXPECT_TRUE(widget()->IsClosed());
    widget1->CloseNow();
}
...
// been closed.
TEST_F(DesktopDialogClientViewTest, IgnorePossiblyUnintendedClicks_CloseRootWindow)
{
    SetDialogButtons(static_cast<int>(ui::mojom::DialogButton::kCancel) | static_cast<int>(ui::mojom::DialogButton::kOk));
    SizeAndLayoutWidget();
    widget()->Show();
...
TEST_F(DialogClientViewTest, IgnorePossiblyUnintendedClicks_ClickAfterClosingTooltip)
{
    SetDialogButtons(static_cast<int>(ui::mojom::DialogButton::kCancel) | static_cast<int>(ui::mojom::DialogButton::kOk));
    SizeAndLayoutWidget();
    widget()->Show();
...
TEST_F(DialogClientViewTest, IgnorePossiblyUnintendedClicks_RepeatedClicks)
{
    ...
    const base::TimeDelta kShortClickInterval = base::Milliseconds(GetDoubleClickInterval());

    // Should ignore clicks right after the dialog is shown.
    ui::MouseEvent mouse_event(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(), kNow, ui::EF_NONE, ui::EF_NONE);
    test::ButtonTestApi(client_view()->ok_button()).NotifyClick(mouse_event);
    test::ButtonTestApi cancel_button(client_view()->cancel_button());
    cancel_button.NotifyClick(mouse_event);
    EXPECT_FALSE(widget()->IsClosed());
    ...
    for (size_t i = 0; i < kNumClicks; i++) {
        cancel_button.NotifyClick(ui::MouseEvent(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(), event_time, ui::EF_NONE, ui::EF_NONE));
        EXPECT_FALSE(widget()->IsClosed());
...
TEST_F(DialogClientViewTest, ButtonLayoutWithExtra)
{
    ...
    SetExtraView(std::make_unique<LabelButton>(Button::PressedCallback(), u"extra"));
    ...
    int right_inset = bounds_right(delegate()) - std::max(bounds_right(ok), bounds_right(cancel));
    ...
    // changes:
    auto get_margin = [&]() { return std::max(bounds_left(ok), bounds_left(cancel)) - std::min(bounds_right(ok), bounds_right(cancel)); };

    // (5): Flex margin between ok/cancel and extra should vary with dialog width
    // (it should absorb 100% of the change in width)
    auto get_flex_margin = [&]() { return std::min(bounds_left(ok), bounds_left(cancel)) - bounds_right(extra); };

    int old_margin = get_margin();
    int old_flex_margin = get_flex_margin();

    SetSizeConstraints(gfx::Size(), gfx::Size(delegate()->width() + 100, 0), gfx::Size());
    SizeAndLayoutWidget();
