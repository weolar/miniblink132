...
public:
    ...
    void InitializeDialog()
    {
        ...
        dialog_->SetAcceptCallback(base::BindLambdaForTesting([&]() { accepted_ = true; }));
        dialog_->SetCancelCallback(base::BindLambdaForTesting([&]() { cancelled_ = true; }));
        dialog_->SetCloseCallback(base::BindLambdaForTesting([&]() { closed_ = true; }));
    }

    views::Widget* CreateDialogWidget(std::unique_ptr<WidgetDelegate> dialog)
    {
        views::Widget* widget = DialogDelegate::CreateDialogWidget(std::move(dialog), GetContext(), parent_widget_->GetNativeView());
        return widget;
    }

    void ShowDialog()
    {
        CreateDialogWidget(std::move(dialog_))->Show();
    }
    ...
    TestDialog* dialog() const
    {
        return dialog_raw_;
    }
    views::Widget* parent_widget()
    {
        return parent_widget_.get();
    }
...
{
    const ui::KeyEvent return_event(ui::EventType::kKeyPressed, ui::VKEY_RETURN, ui::EF_NONE);
...
TEST_F(DialogTest, HitTest_CloseButton)
{
    ...
    // tooltips.
    EXPECT_NE(HTCLOSE, frame->NonClientHitTest(close_button_bounds.CenterPoint()));
#else
    EXPECT_EQ(HTCLOSE, frame->NonClientHitTest(close_button_bounds.CenterPoint()));
#endif
}
...
TEST_F(DialogTest, BoundsAccommodateTitle)
{
    ...
    EXPECT_LT(frame1->GetPreferredSize({}).height(), frame2->GetPreferredSize({}).height());
    ...
    dialog()->GetWidget()->UpdateWindowTitle();
    EXPECT_EQ(frame1->GetPreferredSize({}).height(), frame2->GetPreferredSize({}).height());
...
{
    dialog()->set_title(u"La la la look at me I'm a really really long title that needs to be "
...
TEST_F(DialogTest, InitialFocusWithDeactivatedWidget)
{
    ...
    EXPECT_EQ(dialog->GetOkButton(), dialog_widget->GetFocusManager()->GetStoredFocusView());
    dialog_widget->Show();
    // After activation, the initially focused View should have focus as intended.
    EXPECT_EQ(dialog->GetOkButton(), dialog_widget->GetFocusManager()->GetFocusedView());
...
TEST_F(DialogTest, UnfocusableInitialFocus)
{
    ...
    ui::test::ScopedFakeFullKeyboardAccess::GetInstance()->set_full_keyboard_access_state(false);
#endif
    ...
    dialog->GetBubbleFrameView()->close_button()->SetFocusBehavior(View::FocusBehavior::NEVER);
#endif
...
{
    test::WidgetTest::WidgetAutoclosePtr widget(CreateDialogWidget(std::make_unique<DialogDelegateView>()));
...
TEST_F(DialogDelegateCloseTest, AnyCallbackInhibitsDefaultClose)
{
    ...
    dialog.SetCancelCallback(base::BindLambdaForTesting([&]() { cancelled = true; }));
    dialog.SetAcceptCallback(base::BindLambdaForTesting([&]() { accepted = true; }));
...
TEST_F(DialogDelegateCloseTest, RecursiveCloseFromAcceptCallbackDoesNotTriggerSecondCallback)
{
    DialogDelegateView dialog;
...
TEST_F(DialogDelegateCloseTest, OldClosePathDoesNotDoubleClose)
{
    ...
    auto dialog_owned = std::make_unique<TestDialogDelegateView>(&accepted, &cancelled);
    TestDialogDelegateView* dialog = dialog_owned.get();
    Widget* widget = DialogDelegate::CreateDialogWidget(std::move(dialog_owned), GetContext(), nullptr);
    widget->Show();
...
TEST_F(DialogDelegateCloseTest, CloseParentWidgetDoesNotInvokeCloseCallback)
{
    ...
    Widget* widget = DialogDelegate::CreateDialogWidget(std::move(dialog_owned), GetContext(), parent->GetNativeView());

    bool closed = false;
    dialog->SetCloseCallback(base::BindLambdaForTesting([&closed]() { closed = true; }));
