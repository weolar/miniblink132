...
public:
    ...
    FrameCaptionButton(PressedCallback callback, CaptionButtonIcon icon, int hit_test_type);
    ...
    void SetImage(CaptionButtonIcon icon, Animate animate, const gfx::VectorIcon& icon_image);
    ...
    base::CallbackListSubscription AddBackgroundColorChangedCallback(PropertyChangedCallback callback);

    CaptionButtonIcon GetIcon() const
    {
        return icon_;
    }
...
protected:
    ...
    virtual void DrawIconContents(gfx::Canvas* canvas, gfx::ImageSkia image, int x, int y, cc::PaintFlags flags);
