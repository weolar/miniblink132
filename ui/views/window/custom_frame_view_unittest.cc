...
public:
    ...
    {
        return static_cast<CustomFrameView*>(widget()->non_client_view()->frame_view());
    }
...
protected:
    ...
    {
        return custom_frame_view()->restore_button_;
    }

    ImageButton* close_button()
    {
        return custom_frame_view()->close_button_;
    }

    gfx::Rect title_bounds()
    {
        return custom_frame_view()->title_bounds_;
    }

    void SetWindowButtonOrder(const std::vector<views::FrameButton> leading_buttons, const std::vector<views::FrameButton> trailing_buttons);
...
void CustomFrameViewTest::SetUp()
{
    ...
    widget_->non_client_view()->SetFrameView(std::make_unique<CustomFrameView>(widget_));
}
...
void CustomFrameViewTest::SetWindowButtonOrder(const std::vector<views::FrameButton> leading_buttons, const std::vector<views::FrameButton> trailing_buttons)
{
...
TEST_F(CustomFrameViewTest, DefaultButtonLayout)
{
    ...
    EXPECT_GT(minimize_button()->x(), title_bounds().x() + title_bounds().width());
}
...
TEST_F(CustomFrameViewTest, LeadingButtonLayout)
{
    ...
    EXPECT_LT(minimize_button()->x(), maximize_button()->x());
    EXPECT_FALSE(restore_button()->GetVisible());
    EXPECT_LT(maximize_button()->x() + maximize_button()->width(), title_bounds().x());
}
...
TEST_F(CustomFrameViewTest, LargerEdgeButtonsWhenMaximized)
{
    ...
    EXPECT_EQ(close_button()->bounds().width(), close_button_initial_bounds.width());
    EXPECT_EQ(minimize_button()->bounds().width(), minimize_button_initial_bounds.width());
#else
    EXPECT_GT(close_button()->bounds().width(), close_button_initial_bounds.width());
    EXPECT_GT(minimize_button()->bounds().width(), minimize_button_initial_bounds.width());
#endif
}
