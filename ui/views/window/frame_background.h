...
public:
    ...
    void set_frame_color(SkColor color)
    {
        frame_color_ = color;
    }
    ...
    void set_theme_image(const gfx::ImageSkia& image)
    {
        theme_image_ = image;
    }
    ...
    void PaintMaximized(gfx::Canvas* canvas, const ui::NativeTheme* native_theme, const ui::ColorProvider* color_provider, int x, int y, int width) const;

    // Fills the frame side and bottom borders with the frame color.
    void FillFrameBorders(gfx::Canvas* canvas, const View* view, int left_edge_width, int right_edge_width, int bottom_edge_height) const;
