...
const gfx::FontList& GetFontForDetails(const ui::ResourceBundle::FontDetails& details)
{
...
{
    return (context == style::CONTEXT_TEXTFIELD) ? ui::kColorTextfieldForegroundPlaceholder : ui::kColorLabelForegroundSecondary;
}
...
ui::ResourceBundle::FontDetails TypographyProvider::GetFontDetails(int context, int style) const
{
...
gfx::Font::Weight TypographyProvider::MediumWeightForUI()
{
    ...
    if (ui::ResourceBundle::GetSharedInstance().GetFontListForDetails(ui::ResourceBundle::FontDetails()).GetFontWeight() < gfx::Font::Weight::MEDIUM) {
        return gfx::Font::Weight::MEDIUM;
    }
...
ui::ResourceBundle::FontDetails TypographyProvider::GetFontDetailsImpl(int context, int style) const
{
    ...
    case style::STYLE_DIALOG_BUTTON_DEFAULT:
        ...
        if (context == style::CONTEXT_BUTTON) {
            details.weight = GetValueBolderThan(ui::ResourceBundle::GetSharedInstance().GetFontListForDetails(details).GetFontWeight());
        }
        break;
...
ui::ColorId TypographyProvider::GetColorIdImpl(int context, int style) const
{
    ...
    case style::STYLE_LINK_3:
    case style::STYLE_LINK_5:
        return (context == style::CONTEXT_BUBBLE_FOOTER) ? ui::kColorLinkForegroundOnBubbleFooter : ui::kColorLinkForeground;
    ...
    case style::CONTEXT_TEXTFIELD_SUPPORTING_TEXT:
        return (style == style::STYLE_INVALID) ? ui::kColorTextfieldForegroundPlaceholderInvalid : ui::kColorTextfieldForegroundPlaceholder;
    case style::CONTEXT_MENU:
    case style::CONTEXT_TOUCH_MENU:
...
int TypographyProvider::GetLineHeightImpl(int context, int style) const
{
    static constexpr auto kLineHeights = base::MakeFixedFlatMap<int, int>({
        ...
        { style::STYLE_HEADLINE_4_BOLD, 24 },
        { style::STYLE_HEADLINE_5, 20 },
        { style::STYLE_BODY_1, 24 },
        { style::STYLE_BODY_1_MEDIUM, 24 },
        { style::STYLE_BODY_1_BOLD, 24 },
        { style::STYLE_BODY_2, 20 },
        { style::STYLE_BODY_2_MEDIUM, 20 },
        { style::STYLE_BODY_2_BOLD, 20 },
        { style::STYLE_BODY_3, 20 },
        { style::STYLE_BODY_3_MEDIUM, 20 },
        { style::STYLE_BODY_3_BOLD, 20 },
        { style::STYLE_BODY_4, 18 },
        { style::STYLE_BODY_4_MEDIUM, 18 },
        { style::STYLE_BODY_4_BOLD, 18 },
        { style::STYLE_BODY_5, 16 },
        { style::STYLE_BODY_5_MEDIUM, 16 },
        { style::STYLE_BODY_5_BOLD, 16 },
        { style::STYLE_LINK_3, 20 },
        { style::STYLE_LINK_5, 16 },
        { style::STYLE_CAPTION, 12 },
    });
    const auto it = kLineHeights.find(style);
    return (it == kLineHeights.end()) ? GetFontForDetails(GetFontDetailsImpl(context, style)).GetHeight() : it->second;
}

void TypographyProvider::AssertContextAndStyleAreValid(int context, int style) const
{
    ...
    CHECK(StyleAllowedForContext(context, style)) << "context: " << context << " style: " << style;
}
