...
const Button::KeyClickAction PlatformStyle::kKeyClickActionOnSpace = Button::KeyClickAction::kOnKeyRelease;
...
const View::FocusBehavior PlatformStyle::kDefaultFocusBehavior = View::FocusBehavior::ALWAYS;
...
// static
std::unique_ptr<ScrollBar> PlatformStyle::CreateScrollBar(ScrollBar::Orientation orientation)
{
#if BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_LINUX)
...
// static
gfx::Range PlatformStyle::RangeToDeleteBackwards(const std::u16string& text, size_t cursor_position)
{
    // Delete one code point, which may be two UTF-16 words.
    size_t previous_grapheme_index = gfx::UTF16OffsetToIndex(text, cursor_position, -1);
