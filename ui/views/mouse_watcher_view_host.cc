...
MouseWatcherViewHost::MouseWatcherViewHost(const View* view, const gfx::Insets& hot_zone_insets)
    : view_(view)
...
bool MouseWatcherViewHost::Contains(const gfx::Point& screen_point, EventType type)
{
...
bool MouseWatcherViewHost::IsCursorInViewZone(const gfx::Point& screen_point)
{
    ...
    bounds.SetRect(view_topleft.x() - hot_zone_insets_.left(), view_topleft.y() - hot_zone_insets_.top(), bounds.width() + hot_zone_insets_.width(),
...
bool MouseWatcherViewHost::IsMouseOverWindow()
{
    ...
    return display::Screen::GetScreen()->IsWindowUnderCursor(widget->GetNativeWindow());
}
