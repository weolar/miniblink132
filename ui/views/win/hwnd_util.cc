...
{
    return view && view->GetRootWindow() ? view->GetHost()->GetAcceleratedWidget() : nullptr;
}

HWND HWNDForNativeWindow(const gfx::NativeWindow window)
{
    return window && window->GetRootWindow() ? window->GetHost()->GetAcceleratedWidget() : nullptr;
}

gfx::Rect GetWindowBoundsForClientBounds(View* view, const gfx::Rect& client_bounds)
{
    DCHECK(view);
...
{
    gfx::Rect bounds;
    if (aura::WindowTreeHost* host = aura::WindowTreeHost::GetForAcceleratedWidget(window)) {
        if (gfx::Rect* headless_bounds = host->window()->GetProperty(aura::client::kHeadlessBoundsKey)) {
...
void ShowSystemMenuAtScreenPixelLocation(HWND window, const gfx::Point& point)
{
    ...
    const int command = ::TrackPopupMenu(menu, flags, point.x(), point.y(), 0, window, nullptr);
