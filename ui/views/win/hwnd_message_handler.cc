...
public:
    MoveLoopMouseWatcher(base::WeakPtr<HWNDMessageHandler> host, bool hide_on_escape);
    ...
    bool got_mouse_up() const
    {
        return got_mouse_up_;
    }
...
MoveLoopMouseWatcher::MoveLoopMouseWatcher(base::WeakPtr<HWNDMessageHandler> host, bool hide_on_escape)
    : host_(std::move(host))
    , hide_on_escape_(hide_on_escape)
{
    ...
    mouse_hook_ = SetWindowsHookEx(WH_MOUSE, &MouseHook, nullptr, GetCurrentThreadId());
    if (mouse_hook_) {
        instance_ = this;
        // We don't care if setting the key hook succeeded.
        key_hook_ = SetWindowsHookEx(WH_KEYBOARD, &KeyHook, nullptr, GetCurrentThreadId());
    }
    if (instance_ != this) {
...
// static
LRESULT CALLBACK MoveLoopMouseWatcher::MouseHook(int n_code, WPARAM w_param, LPARAM l_param)
{
    DCHECK(instance_);
...
// static
LRESULT CALLBACK MoveLoopMouseWatcher::KeyHook(int n_code, WPARAM w_param, LPARAM l_param)
{
    if (n_code == HC_ACTION && w_param == VK_ESCAPE) {
        int value = TRUE;
        DwmSetWindowAttribute(instance_->host_->hwnd(), DWMWA_TRANSITIONS_FORCEDISABLED, &value, sizeof(value));
...
{
    return hittest == HTRIGHT || hittest == HTLEFT || hittest == HTTOP || hittest == HTBOTTOM || hittest == HTTOPLEFT || hittest == HTTOPRIGHT
...
bool IsWindowArranged(HWND window)
{
    ...
    static const auto is_window_arranged_func = reinterpret_cast<IsWindowArrangedFuncType>(base::win::GetUser32FunctionPointer("IsWindowArranged"));
...
public:
    explicit ScopedRedrawLock(HWNDMessageHandler* owner)
        : owner_(owner)
        , hwnd_(owner_->hwnd())
        , should_lock_(owner_->IsVisible() && !owner->HasChildRenderingWindow() && ::IsWindow(hwnd_) && !owner_->IsHeadless()
...
base::LazyInstance<HWNDMessageHandler::FullscreenWindowMonitorMap>::DestructorAtExit HWNDMessageHandler::fullscreen_monitor_map_ = LAZY_INSTANCE_INITIALIZER;
...
// static
std::unique_ptr<HWNDMessageHandler> HWNDMessageHandler::Create(HWNDMessageHandlerDelegate* delegate, const std::string& debugging_id, bool headless_mode)
{
    HWNDMessageHandler* message_handler
        = headless_mode ? new HWNDMessageHandlerHeadless(delegate, debugging_id) : new HWNDMessageHandler(delegate, debugging_id);
    return base::WrapUnique(message_handler);
}

HWNDMessageHandler::HWNDMessageHandler(HWNDMessageHandlerDelegate* delegate, const std::string& debugging_id)
...
void HWNDMessageHandler::Init(HWND parent, const gfx::Rect& bounds)
{
    ...
    GetMonitorAndRects(bounds.ToRECT(), &last_monitor_, &last_monitor_rect_, &last_work_area_);
...
void HWNDMessageHandler::Close()
{
    ...
    if (!waiting_for_close_now_) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&HWNDMessageHandler::CloseNow, msg_handler_weak_factory_.GetWeakPtr()));
    }
}
...
void HWNDMessageHandler::GetWindowPlacement(gfx::Rect* bounds, ui::mojom::WindowShowState* show_state) const
{
    WINDOWPLACEMENT wp;
    wp.length = sizeof(wp);
    ...
    if (bounds != nullptr) {
        if (wp.showCmd == SW_SHOWNORMAL) {
            ...
            *bounds = gfx::Rect(wp.rcNormalPosition);
            ...
            MONITORINFO mi;
            mi.cbSize = sizeof(mi);
            succeeded = GetMonitorInfo(MonitorFromWindow(hwnd(), MONITOR_DEFAULTTONEAREST), &mi) != 0;
            DCHECK(succeeded);

            *bounds = gfx::Rect(wp.rcNormalPosition);
            // Convert normal position from workarea coordinates to screen
            // coordinates.
            bounds->Offset(mi.rcWork.left - mi.rcMonitor.left, mi.rcWork.top - mi.rcMonitor.top);
        }
    }
...
void HWNDMessageHandler::SetBounds(const gfx::Rect& bounds_in_pixels, bool force_size_changed)
{
    background_fullscreen_hack_ = false;
...
void HWNDMessageHandler::SetParentOrOwner(HWND new_parent)
{
    ...
    if (parent) {
        ...
        SetWindowLongPtr(hwnd(), GWLP_HWNDPARENT, reinterpret_cast<LONG_PTR>(new_parent));
    }
}
...
{
    SetWindowPos(hwnd(), nullptr, 0, 0, size.width(), size.height(), SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOMOVE);
}
...
void HWNDMessageHandler::StackAbove(HWND other_hwnd)
{
    ...
    SetWindowPos(hwnd(), next_window ? next_window : HWND_TOP, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE);
}
...
void HWNDMessageHandler::Show(ui::mojom::WindowShowState show_state, const gfx::Rect& pixel_restore_bounds)
{
    TRACE_EVENT0("views", "HWNDMessageHandler::Show");

    int native_show_state;
    if (show_state == ui::mojom::WindowShowState::kMaximized && !pixel_restore_bounds.IsEmpty()) {
...
void HWNDMessageHandler::Hide()
{
    if (IsWindow(hwnd())) {
        ...
        SetWindowPos(hwnd(), nullptr, 0, 0, 0, 0, SWP_HIDEWINDOW | SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOREPOSITION | SWP_NOSIZE | SWP_NOZORDER);
    }
}
...
{
    if (IsMinimized()) {
        base::AutoReset<bool> restoring_activate(&notify_restore_on_activate_, true);
...
{
    ::SetWindowPos(hwnd(), on_top ? HWND_TOPMOST : HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
}
...
bool HWNDMessageHandler::RunMoveLoop(const gfx::Vector2d& drag_offset, bool hide_on_escape)
{
    ReleaseCapture();
    MoveLoopMouseWatcher watcher(msg_handler_weak_factory_.GetWeakPtr(), hide_on_escape);
    // In Aura, we handle touch events asynchronously. So we need to allow nested
    // tasks while in windows move loop.
    base::CurrentThread::ScopedAllowApplicationTasksInNativeNestedLoop allow;

    SendMessage(hwnd(), WM_SYSCOMMAND, SC_MOVE | 0x0002, static_cast<LPARAM>(GetMessagePos()));
...
void HWNDMessageHandler::SendFrameChanged()
{
    SetWindowPos(hwnd(), nullptr, 0, 0, 0, 0,
        SWP_FRAMECHANGED | SWP_NOACTIVATE | SWP_NOCOPYBITS | SWP_NOMOVE | SWP_NOOWNERZORDER | SWP_NOREPOSITION | SWP_NOSENDCHANGING | SWP_NOSIZE
            | SWP_NOZORDER);
}
...
void HWNDMessageHandler::SetCapture()
{
    ...
    DCHECK(!HasCapture()) << " release capture error = " << logging::SystemErrorCodeToString(release_capture_errno_);
    ::SetCapture(hwnd());
}
...
void HWNDMessageHandler::SetVisibilityChangedAnimationsEnabled(bool enabled)
{
    ...
    DwmSetWindowAttribute(hwnd(), DWMWA_TRANSITIONS_FORCEDISABLED, &dwm_value, sizeof(dwm_value));
}
...
bool HWNDMessageHandler::SetTitle(const std::u16string& title)
{
    ...
    if (len_with_null == 1 && title.length() == 0)
        return false;
    if (len_with_null - 1 == title.length() && GetWindowText(hwnd(), base::WriteInto(&current_title, len_with_null), len_with_null)
...
void HWNDMessageHandler::SetCursor(scoped_refptr<ui::WinCursor> cursor)
{
    ...
    TRACE_EVENT1("ui,input", "HWNDMessageHandler::SetCursor", "cursor", static_cast<const void*>(cursor->hcursor()));
...
{
    if (!delegate_->HasNonClientView() || !delegate_->CanActivate() || !delegate_->HasFrame() || (delegate_->GetFrameMode() == FrameMode::CUSTOM_DRAWN)) {
        return;
    }

    DefWindowProcWithRedrawLock(WM_NCACTIVATE, delegate_->ShouldPaintAsActive(), 0);
}

void HWNDMessageHandler::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
    if (!window_icon.isNull()) {
        ...
        SendMessage(hwnd(), WM_SETICON, ICON_SMALL, reinterpret_cast<LPARAM>(window_icon_.get()));
    }
    if (!app_icon.isNull()) {
        base::win::ScopedHICON previous_icon = std::move(app_icon_);
        app_icon_ = IconUtil::CreateHICONFromSkBitmap(*app_icon.bitmap());
        SendMessage(hwnd(), WM_SETICON, ICON_BIG, reinterpret_cast<LPARAM>(app_icon_.get()));
    }
}

void HWNDMessageHandler::SetFullscreen(bool fullscreen, int64_t target_display_id)
{
    // Erase any prior reference to this window in the fullscreen window map.
    HMONITOR monitor = MonitorFromWindow(hwnd(), MONITOR_DEFAULTTOPRIMARY);
    FullscreenWindowMonitorMap::iterator iter = fullscreen_monitor_map_.Get().find(monitor);
...
void HWNDMessageHandler::SetAspectRatio(float aspect_ratio, const gfx::Size& excluded_margin)
{
    ...
    // Convert to pixels.
    excluded_margin_ = display::win::ScreenWin::DIPToScreenSize(hwnd(), excluded_margin);
...
bool HWNDMessageHandler::HasChildRenderingWindow()
{
    ...
    // software rendering.
    return gfx::RenderingWindowManager::GetInstance()->HasValidChildWindow(hwnd());
}
...
std::unique_ptr<aura::ScopedEnableUnadjustedMouseEvents> HWNDMessageHandler::RegisterUnadjustedMouseEvent()
{
    std::unique_ptr<ScopedEnableUnadjustedMouseEventsWin> scoped_enable = ScopedEnableUnadjustedMouseEventsWin::StartMonitor(this);
...
LRESULT HWNDMessageHandler::OnWndProc(UINT message, WPARAM w_param, LPARAM l_param)
{
    TRACE_EVENT("ui,toplevel", "HWNDMessageHandler::OnWndProc", [&](perfetto::EventContext ctx) {
        perfetto::protos::pbzero::ChromeWindowHandleEventInfo* args = ctx.event()->set_chrome_window_handle_event_info();
        args->set_message_id(message);
        args->set_hwnd_ptr(static_cast<uint64_t>(reinterpret_cast<uintptr_t>(hwnd())));
    });
    ...
    if (message == WM_ACTIVATE && IsTopLevelWindow(window)) {
        PostProcessActivateMessage(LOWORD(w_param), !!HIWORD(w_param), reinterpret_cast<HWND>(l_param));
    }
    return result;
}
...
void HWNDMessageHandler::OnCaretBoundsChanged(const ui::TextInputClient* client)
{
    if (!ax_system_caret_)
    ...
    gfx::Rect caret_bounds = display::win::ScreenWin::DIPToScreenRect(hwnd(), dip_caret_bounds);
    // Collapse any selection.
    caret_bounds.set_width(1);
    ax_system_caret_->MoveCaretTo(caret_bounds);
}

void HWNDMessageHandler::OnTextInputStateChanged(const ui::TextInputClient* client)
{
    if (!client || client->GetTextInputType() == ui::TEXT_INPUT_TYPE_NONE)
        OnCaretBoundsChanged(client);
}

void HWNDMessageHandler::OnInputMethodDestroyed(const ui::InputMethod* input_method)
{
    DestroyAXSystemCaret();
}

LRESULT HWNDMessageHandler::HandleMouseMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled)
{
...
LRESULT HWNDMessageHandler::HandleKeyboardMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled)
{
    base::WeakPtr<HWNDMessageHandler> ref(msg_handler_weak_factory_.GetWeakPtr());
    LRESULT ret = 0;
...
LRESULT HWNDMessageHandler::HandleTouchMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled)
{
    base::WeakPtr<HWNDMessageHandler> ref(msg_handler_weak_factory_.GetWeakPtr());
...
LRESULT HWNDMessageHandler::HandlePointerMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled)
{
    base::WeakPtr<HWNDMessageHandler> ref(msg_handler_weak_factory_.GetWeakPtr());
...
LRESULT HWNDMessageHandler::HandleInputMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled)
{
    base::WeakPtr<HWNDMessageHandler> ref(msg_handler_weak_factory_.GetWeakPtr());
...
LRESULT HWNDMessageHandler::HandleScrollMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled)
{
    base::WeakPtr<HWNDMessageHandler> ref(msg_handler_weak_factory_.GetWeakPtr());
...
LRESULT HWNDMessageHandler::HandleNcHitTestMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled)
{
    base::WeakPtr<HWNDMessageHandler> ref(msg_handler_weak_factory_.GetWeakPtr());
    LRESULT ret = OnNCHitTest(gfx::Point(CR_GET_X_LPARAM(l_param), CR_GET_Y_LPARAM(l_param)));
    *handled = !ref.get() || msg_handled_;
    return ret;
}
...
void HWNDMessageHandler::ApplyPinchZoomScale(float scale)
{
    ...
    ui::GestureEvent event(cursor_pos.x, cursor_pos.y, ui::EF_NONE, base::TimeTicks::Now(), event_details);
...
void HWNDMessageHandler::ApplyPanGestureEvent(int scroll_x, int scroll_y, ui::EventMomentumPhase momentum_phase, ui::ScrollEventPhase phase)
{
    ...
    ui::ScrollEvent event(
        ui::EventType::kScroll, cursor_location, ui::EventTimeForNow(), modifiers, scroll_x, scroll_y, scroll_x, scroll_y, 2, momentum_phase, phase);
    delegate_->HandleScrollEvent(&event);
}

void HWNDMessageHandler::ApplyPanGestureScroll(int scroll_x, int scroll_y)
{
    ApplyPanGestureEvent(scroll_x, scroll_y, ui::EventMomentumPhase::NONE, ui::ScrollEventPhase::kUpdate);
}

void HWNDMessageHandler::ApplyPanGestureFling(int scroll_x, int scroll_y)
{
    ApplyPanGestureEvent(scroll_x, scroll_y, ui::EventMomentumPhase::INERTIAL_UPDATE, ui::ScrollEventPhase::kNone);
}

void HWNDMessageHandler::ApplyPanGestureScrollBegin(int scroll_x, int scroll_y)
{
    // Phase information will be ingored in ApplyPanGestureEvent().
    ApplyPanGestureEvent(scroll_x, scroll_y, ui::EventMomentumPhase::NONE, ui::ScrollEventPhase::kBegan);
}

void HWNDMessageHandler::ApplyPanGestureScrollEnd(bool transitioning_to_pinch)
{
    ApplyPanGestureEvent(0, 0, transitioning_to_pinch ? ui::EventMomentumPhase::BLOCKED : ui::EventMomentumPhase::NONE, ui::ScrollEventPhase::kEnd);
}

void HWNDMessageHandler::ApplyPanGestureFlingBegin()
{
    ApplyPanGestureEvent(0, 0, ui::EventMomentumPhase::BEGAN, ui::ScrollEventPhase::kNone);
}

void HWNDMessageHandler::ApplyPanGestureFlingEnd()
{
    ApplyPanGestureEvent(0, 0, ui::EventMomentumPhase::END, ui::ScrollEventPhase::kNone);
}
...
void HWNDMessageHandler::InitExtras()
{
    if (!called_enable_non_client_dpi_scaling_ && delegate_->HasFrame()) {
        ...
        static const auto enable_child_window_dpi_message_func
            = reinterpret_cast<EnableChildWindowDpiMessagePtr>(base::win::GetUser32FunctionPointer("EnableChildWindowDpiMessage"));
        if (enable_child_window_dpi_message_func) {
            enable_child_window_dpi_message_func(hwnd(), TRUE);
        }
    }

    prop_window_target_ = std::make_unique<ui::ViewProp>(hwnd(), ui::WindowEventTarget::kWin32InputEventTarget, static_cast<ui::WindowEventTarget*>(this));
...
int HWNDMessageHandler::GetAppbarAutohideEdges(HMONITOR monitor)
{
    ...
    return ViewsDelegate::GetInstance()->GetAppbarAutohideEdges(
        monitor, base::BindOnce(&HWNDMessageHandler::OnAppbarAutohideEdgesChanged, autohide_factory_.GetWeakPtr()));
}
...
void HWNDMessageHandler::OnAppbarAutohideEdgesChanged()
{
    ...
    SetWindowPos(
        hwnd(), nullptr, client.left, client.top, client.right - client.left, client.bottom - client.top, SWP_FRAMECHANGED | SWP_NOZORDER | SWP_NOACTIVATE);
}
...
void HWNDMessageHandler::PostProcessActivateMessage(int activation_state, bool minimized, HWND window_gaining_or_losing_activation)
{
    ...
    if (::IsChild(hwnd(), window_gaining_or_losing_activation)
        && gfx::GetClassName(window_gaining_or_losing_activation) == std::wstring(ui::kLegacyRenderWidgetHostHwnd)) {
    ...
    if (!active) {
        if (IsFullscreen() && ::IsWindow(window_gaining_or_losing_activation)) {
            HMONITOR active_window_monitor = MonitorFromWindow(window_gaining_or_losing_activation, MONITOR_DEFAULTTOPRIMARY);
            HMONITOR fullscreen_window_monitor = MonitorFromWindow(hwnd(), MONITOR_DEFAULTTOPRIMARY);
        ...
        // this.
        CheckAndHandleBackgroundFullscreenOnMonitor(window_gaining_or_losing_activation);
    }
}
...
void HWNDMessageHandler::ClientAreaSizeChanged()
{
    ...
    // window bounds.
    if ((background_fullscreen_hack_ && !sent_window_size_changing_) || IsHeadless()) {
        return;
    }
    auto ref = msg_handler_weak_factory_.GetWeakPtr();
...
bool HWNDMessageHandler::GetClientAreaInsets(gfx::Insets* insets, HMONITOR monitor) const
{
...
void HWNDMessageHandler::ResetWindowRegion(bool force, bool redraw)
{
    ...
    if (!is_translucent_ && !custom_window_region_.is_valid() && (IsFrameSystemDrawn() || !delegate_->HasNonClientView())) {
        if (force)
    ...
    if (custom_window_region_.is_valid()) {
        ...
        CombineRgn(new_region.get(), custom_window_region_.get(), nullptr, RGN_COPY);
    } else if (IsMaximized()) {
        ...
        SkPath window_mask;
        delegate_->GetWindowMask(gfx::Size(window_rect.right - window_rect.left, window_rect.bottom - window_rect.top), &window_mask);
...
LRESULT HWNDMessageHandler::DefWindowProcWithRedrawLock(UINT message, WPARAM w_param, LPARAM l_param)
{
...
void HWNDMessageHandler::ForceRedrawWindow(int attempts)
{
    if (ui::IsWorkstationLocked()) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&HWNDMessageHandler::ForceRedrawWindow, msg_handler_weak_factory_.GetWeakPtr(), attempts), base::Milliseconds(500));
        return;
    }
...
bool HWNDMessageHandler::IsFrameSystemDrawn() const
{
    ...
    return frame_mode == FrameMode::SYSTEM_DRAWN || frame_mode == FrameMode::SYSTEM_DRAWN_NO_CONTROLS;
}
...
{
    if (delegate_->HasNonClientView() && !active && thread_id != GetCurrentThreadId()) {
...
BOOL HWNDMessageHandler::OnAppCommand(HWND window, int command, WORD device, WORD keystate)
{
...
void HWNDMessageHandler::OnCommand(UINT notification_code, int command, HWND window)
{
...
LRESULT HWNDMessageHandler::OnCreate(CREATESTRUCT* create_struct)
{
    ...
    session_change_observer_ = std::make_unique<ui::SessionChangeObserver>(base::BindRepeating(&HWNDMessageHandler::OnSessionChange, base::Unretained(this)));
...
void HWNDMessageHandler::OnDestroy()
{
    ...
    const auto i = base::ranges::find(map, this, &FullscreenWindowMonitorMap::value_type::second);
    if (i != map.end())
        map.erase(i);
...
void HWNDMessageHandler::OnDisplayChange(UINT bits_per_pixel, const gfx::Size& screen_size)
{
...
LRESULT HWNDMessageHandler::OnDpiChanged(UINT msg, WPARAM w_param, LPARAM l_param)
{
    if (LOWORD(w_param) != HIWORD(w_param))
        NOTIMPLEMENTED() << "Received non-square scaling factors";

    TRACE_EVENT("ui", "HWNDMessageHandler::OnDpiChanged", [&](perfetto::EventContext ctx) {
        perfetto::protos::pbzero::ChromeWindowHandleEventInfo* args = ctx.event()->set_chrome_window_handle_event_info();
...
{
    gfx::Insets insets;
    if (delegate_->GetDwmFrameInsetsInPixels(&insets) && !insets.IsEmpty() && needs_dwm_frame_clear_) {
...
void HWNDMessageHandler::OnGetMinMaxInfo(MINMAXINFO* minmax_info)
{
    ...
    if (delegate_->WidgetSizeIsClientSize()) {
        ...
        min_window_size.Enlarge(window_rect.right - window_rect.left, window_rect.bottom - window_rect.top);
...
LRESULT HWNDMessageHandler::OnGetObject(UINT message, WPARAM w_param, LPARAM l_param)
{
    ...
    const bool is_uia_active = is_uia_request && ::ui::AXPlatform::GetInstance().IsUiaProviderEnabled();
    ...
    if ((is_uia_request || is_msaa_request) && delegate_->GetNativeViewAccessible()) {
        ...
        if (is_uia_active) {
            ...
            reference_result = UiaReturnRawElementProvider(hwnd(), w_param, l_param, root.Get());
        } else if (is_msaa_request) {
            // Retrieve MSAA dispatch object for the root view.
            Microsoft::WRL::ComPtr<IAccessible> root(delegate_->GetNativeViewAccessible());
            reference_result = LresultFromObject(IID_IAccessible, w_param, root.Get());
        }
    } else if (::GetFocus() == hwnd() && ax_system_caret_ && static_cast<DWORD>(OBJID_CARET) == obj_id) {
        Microsoft::WRL::ComPtr<IAccessible> ax_system_caret_accessible = ax_system_caret_->GetCaret();
        reference_result = LresultFromObject(IID_IAccessible, w_param, ax_system_caret_accessible.Get());
    }

    return reference_result;
}

LRESULT HWNDMessageHandler::OnImeMessages(UINT message, WPARAM w_param, LPARAM l_param)
{
    LRESULT result = 0;
    base::WeakPtr<HWNDMessageHandler> ref(msg_handler_weak_factory_.GetWeakPtr());
...
void HWNDMessageHandler::OnInputLangChange(DWORD character_set, HKL input_language_id)
{
    delegate_->HandleInputLanguageChange(character_set, input_language_id);
}

LRESULT HWNDMessageHandler::OnKeyEvent(UINT message, WPARAM w_param, LPARAM l_param)
{
    CHROME_MSG msg = { hwnd(), message, w_param, l_param, static_cast<DWORD>(GetMessageTime()) };
    ui::KeyEvent key(msg);
    base::WeakPtr<HWNDMessageHandler> ref(msg_handler_weak_factory_.GetWeakPtr());
...
LRESULT HWNDMessageHandler::OnMouseActivate(UINT message, WPARAM w_param, LPARAM l_param)
{
...
LRESULT HWNDMessageHandler::OnMouseRange(UINT message, WPARAM w_param, LPARAM l_param)
{
...
LRESULT HWNDMessageHandler::OnPointerActivate(UINT message, WPARAM w_param, LPARAM l_param)
{
    POINTER_INPUT_TYPE pointer_type;
    if (::GetPointerType(GET_POINTERID_WPARAM(w_param), &pointer_type) && pointer_type == PT_TOUCHPAD) {
...
LRESULT HWNDMessageHandler::OnPointerEvent(UINT message, WPARAM w_param, LPARAM l_param)
{
    POINTER_INPUT_TYPE pointer_type;
    ...
    if (pointer_type == PT_PEN && (message == WM_NCPOINTERDOWN || message == WM_NCPOINTERUP || message == WM_NCPOINTERUPDATE)) {
...
LRESULT HWNDMessageHandler::OnInputEvent(UINT message, WPARAM w_param, LPARAM l_param)
{
    if (!using_wm_input_)
        return -1;
    ...
    result = ::GetRawInputData(input_handle, RID_INPUT, buffer.get(), &size, sizeof(RAWINPUTHEADER));
    if (result == static_cast<UINT>(-1)) {
        PLOG(ERROR) << "GetRawInputData() failed";
        return 0;
    }
    ...
    if (input->header.dwType == RIM_TYPEMOUSE && input->data.mouse.usButtonFlags != RI_MOUSE_WHEEL) {
        POINT cursor_pos = { 0 };
        ::GetCursorPos(&cursor_pos);
        ScreenToClient(hwnd(), &cursor_pos);
        ui::MouseEvent event(ui::EventType::kMouseMoved, gfx::PointF(cursor_pos.x, cursor_pos.y), gfx::PointF(cursor_pos.x, cursor_pos.y),
            ui::EventTimeForNow(), GetFlagsFromRawInputMessage(input), 0);
        if (!(input->data.mouse.usFlags & MOUSE_MOVE_ABSOLUTE)) {
            ui::MouseEvent::DispatcherApi(&event).set_movement(gfx::Vector2dF(input->data.mouse.lLastX, input->data.mouse.lLastY));
        }
...
LRESULT HWNDMessageHandler::OnNCActivate(UINT message, WPARAM w_param, LPARAM l_param)
{
    ...
    if (delegate_->GetFrameMode() == FrameMode::CUSTOM_DRAWN) {
        ...
        RedrawWindow(hwnd(), nullptr, nullptr, RDW_NOCHILDREN | RDW_INVALIDATE | RDW_UPDATENOW);
    ...
    // corruption.
    if (!delegate_->HasFrame() || delegate_->GetFrameMode() == FrameMode::CUSTOM_DRAWN) {
        SetMsgHandled(TRUE);
        return TRUE;
    }

    return DefWindowProcWithRedrawLock(WM_NCACTIVATE, paint_as_active || active, 0);
}
...
LRESULT HWNDMessageHandler::OnNCCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    if (delegate_->HasFrame()) {
        ...
        static const auto enable_non_client_dpi_scaling_func
            = reinterpret_cast<EnableNonClientDpiScalingPtr>(base::win::GetUser32FunctionPointer("EnableNonClientDpiScaling"));
        called_enable_non_client_dpi_scaling_ = !!(enable_non_client_dpi_scaling_func && enable_non_client_dpi_scaling_func(hwnd()));
    }
    return FALSE;
}
...
LRESULT HWNDMessageHandler::OnNCHitTest(const gfx::Point& point)
{
    ...
    if (HasSystemFrame() && delegate_->GetFrameMode() != FrameMode::SYSTEM_DRAWN_NO_CONTROLS) {
        LRESULT result;
        if (DwmDefWindowProc(hwnd(), WM_NCHITTEST, 0, MAKELPARAM(point.x(), point.y()), &result)) {
            return result;
        }
    }
    ...
    // us.
    LRESULT hit_test_code = DefWindowProc(hwnd(), WM_NCHITTEST, 0, MAKELPARAM(point.x(), point.y()));
...
void HWNDMessageHandler::OnNCPaint(HRGN rgn)
{
    ...
    if (gfx::Size(window_rect.right - window_rect.left, window_rect.bottom - window_rect.top) != root_view_size) {
...
LRESULT HWNDMessageHandler::OnNCUAHDrawCaption(UINT message, WPARAM w_param, LPARAM l_param)
{
...
LRESULT HWNDMessageHandler::OnNCUAHDrawFrame(UINT message, WPARAM w_param, LPARAM l_param)
{
    // See comment in widget_win.h at the definition of WM_NCUAHDRAWCAPTION for
    // an explanation about why we need to handle this message.
    SetMsgHandled(delegate_->GetFrameMode() == FrameMode::CUSTOM_DRAWN);
        ...
        if (HasChildRenderingWindow()) {
            ...
            if (GetClientRect(hwnd(), &cr)) {
                ...
                if (exposed_pixels_.height() > 0) {
                    RECT rect = { 0, client_area.height() - exposed_pixels_.height(), client_area.width(), client_area.height() };
                    FillRect(ps.hdc, &rect, brush);
                }
                if (exposed_pixels_.width() > 0) {
                    RECT rect = { client_area.width() - exposed_pixels_.width(), 0, client_area.width(), client_area.height() - exposed_pixels_.height() };
                    FillRect(ps.hdc, &rect, brush);
                }
            }
        }
...
LRESULT HWNDMessageHandler::OnReflectedMessage(UINT message, WPARAM w_param, LPARAM l_param)
{
    SetMsgHandled(FALSE);
    return 0;
}

LRESULT HWNDMessageHandler::OnScrollMessage(UINT message, WPARAM w_param, LPARAM l_param)
{
    CHROME_MSG msg = { hwnd(), message, w_param, l_param, static_cast<DWORD>(GetMessageTime()) };
...
LRESULT HWNDMessageHandler::OnSetCursor(UINT message, WPARAM w_param, LPARAM l_param)
{
...
LRESULT HWNDMessageHandler::OnSetIcon(UINT size_type, HICON new_icon)
{
    ...
    return DefWindowProcWithRedrawLock(WM_SETICON, size_type, reinterpret_cast<LPARAM>(new_icon));
}

LRESULT HWNDMessageHandler::OnSetText(const wchar_t* text)
{
    // Use a ScopedRedrawLock to avoid weird non-client painting.
    return DefWindowProcWithRedrawLock(WM_SETTEXT, NULL, reinterpret_cast<LPARAM>(text));
}
...
void HWNDMessageHandler::OnSettingChange(UINT flags, const wchar_t* section)
{
    if (!GetParent(hwnd()) && (flags == SPI_SETWORKAREA)) {
        ...
        ::SetWindowPos(hwnd(), nullptr, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOZORDER | SWP_NOREDRAW | SWP_NOACTIVATE | SWP_NOOWNERZORDER);
        SetMsgHandled(TRUE);
    } else {
...
void HWNDMessageHandler::OnSysCommand(UINT notification_code, const gfx::Point& point)
{
    ...
    if (IsFullscreen()
        && (((notification_code & sc_mask) == SC_SIZE) || ((notification_code & sc_mask) == SC_MOVE) || ((notification_code & sc_mask) == SC_MAXIMIZE)))
        return;

    const bool window_control_action
        = (notification_code & sc_mask) == SC_MINIMIZE || (notification_code & sc_mask) == SC_MAXIMIZE || (notification_code & sc_mask) == SC_RESTORE;
    const bool custom_controls_frame_mode
        = delegate_->GetFrameMode() == FrameMode::SYSTEM_DRAWN_NO_CONTROLS || delegate_->GetFrameMode() == FrameMode::CUSTOM_DRAWN;
    if (custom_controls_frame_mode && window_control_action)
        delegate_->ResetWindowControls();

    if (delegate_->GetFrameMode() == FrameMode::CUSTOM_DRAWN) {
        const bool window_bounds_change = (notification_code & sc_mask) == SC_MOVE || (notification_code & sc_mask) == SC_SIZE;
    ...
    if ((notification_code & sc_mask) == SC_KEYMENU && point.x() == 0) {
        ...
        ui::Accelerator accelerator(ui::KeyboardCodeForWindowsKeyCode(VK_MENU), modifiers);
    ...
    DefWindowProc(hwnd(), WM_SYSCOMMAND, notification_code, MAKELPARAM(point.x(), point.y()));
...
LRESULT HWNDMessageHandler::OnTouchEvent(UINT message, WPARAM w_param, LPARAM l_param)
{
...
void HWNDMessageHandler::OnWindowPosChanging(WINDOWPOS* window_pos)
{
    ...
    if (ignore_window_pos_changes_) {
        ...
        if (!(window_pos->flags & ((IsVisible() ? SWP_HIDEWINDOW : SWP_SHOWWINDOW) | SWP_FRAMECHANGED))
            ...
            window_pos->flags &= static_cast<unsigned int>(~(SWP_SHOWWINDOW | SWP_HIDEWINDOW));
        }
    } else if (!GetParent(hwnd())) {
        RECT window_rect;
        const bool have_new_window_rect = !(window_pos->flags & SWP_NOMOVE) && !(window_pos->flags & SWP_NOSIZE);
        ...
        if ((have_new_window_rect || GetWindowRect(hwnd(), &window_rect)) && GetMonitorAndRects(window_rect, &monitor, &monitor_rect, &work_area)) {
            ...
            if (IsMaximized()) {
                ...
                if (GetClientAreaInsets(&client_area_insets, monitor))
                    ...
                    expected_maximized_bounds.Inset(gfx::ScaleToCeiledInsets(client_area_insets, -1));
            }
            ...
            const bool incorrect_maximized_bounds = IsMaximized() && have_new_window_rect
                && (expected_maximized_bounds.x() != window_pos->x || expected_maximized_bounds.y() != window_pos->y
                    || expected_maximized_bounds.width() != window_pos->cx || expected_maximized_bounds.height() != window_pos->cy);

            // If the size of a background fullscreen window changes again, then we
            // should reset the |background_fullscreen_hack_| flag.
            if (background_fullscreen_hack_ && (!(window_pos->flags & SWP_NOSIZE) && (monitor_rect.height() - window_pos->cy != 1))) {
                background_fullscreen_hack_ = false;
            }
            const bool fullscreen_without_hack = IsFullscreen() && !background_fullscreen_hack_;

            // If the browser window is arranged by Snap, then we should not change
            // its position but let Windows do it.
            if (same_monitor && (incorrect_maximized_bounds || fullscreen_without_hack || work_area_changed) && !IsWindowArranged(hwnd())) {
                ...
                window_pos->flags &= static_cast<unsigned int>(~(SWP_NOSIZE | SWP_NOMOVE | SWP_NOREDRAW));
                ...
                base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
                    FROM_HERE, base::BindOnce(&HWNDMessageHandler::StopIgnoringPosChanges, msg_handler_weak_factory_.GetWeakPtr()));
            }
    ...
    if ((old_size != new_size && !(window_pos->flags & SWP_NOSIZE)) || window_pos->flags & SWP_FRAMECHANGED) {
        ...
        // in that case.
        PostMessage(hwnd(), WM_WINDOWSIZINGFINISHED, ++current_window_size_message_, 0);
...
LRESULT HWNDMessageHandler::OnWindowSizingFinished(UINT message, WPARAM w_param, LPARAM l_param)
{
...
void HWNDMessageHandler::OnSessionChange(WPARAM status_code, const bool* is_current_session)
{
...
LRESULT HWNDMessageHandler::HandleMouseEventInternal(UINT message, WPARAM w_param, LPARAM l_param, bool track_mouse)
{
    ...
    if (is_pen_active_in_client_area_) {
        ...
        static const auto get_current_input_message_source
            = reinterpret_cast<decltype(&::GetCurrentInputMessageSource)>(base::win::GetUser32FunctionPointer("GetCurrentInputMessageSource"));
        if (get_current_input_message_source && get_current_input_message_source(&input_message_source)
    ...
    if (message != WM_MOUSEWHEEL && message != WM_MOUSEHWHEEL && ui::IsMouseEventFromTouch(message)) {
        ...
        // they won't work.
        if (delegate_->GetFrameMode() == FrameMode::SYSTEM_DRAWN && (hittest == HTCLOSE || hittest == HTMINBUTTON || hittest == HTMAXBUTTON)) {
            SetMsgHandled(FALSE);
        }
    ...
    if (message == WM_RBUTTONUP && is_right_mouse_pressed_on_caption_) {
        ...
        w_param = static_cast<WPARAM>(SendMessage(hwnd(), WM_NCHITTEST, 0, MAKELPARAM(screen_point.x, screen_point.y)));
        if (w_param == HTCAPTION || w_param == HTSYSMENU) {
            ShowSystemMenuAtScreenPixelLocation(hwnd(), gfx::Point(screen_point));
            return 0;
        }
    } else if (message == WM_NCLBUTTONDOWN && delegate_->GetFrameMode() == FrameMode::CUSTOM_DRAWN) {
        switch (w_param) {
        case HTCLOSE:
        case HTMINBUTTON:
        case HTMAXBUTTON: {
    ...
    CHROME_MSG msg = { hwnd(), message, w_param, l_param, static_cast<DWORD>(message_time), { CR_GET_X_LPARAM(l_param), CR_GET_Y_LPARAM(l_param) } };
    ui::MouseEvent event(msg);
    if (IsSynthesizedMouseMessage(message, message_time, l_param))
        event.SetFlags(event.flags() | ui::EF_FROM_TOUCH);

    if (event.type() == ui::EventType::kMouseMoved && !HasCapture() && track_mouse) {
        ...
        return (ui::RerouteMouseWheel(hwnd(), w_param, l_param) || delegate_->HandleMouseEvent(&mouse_wheel_event)) ? 0 : 1;
    }

    // Suppress |EventType::kMouseMoved| and |EventType::kMouseDragged| events
    // from WM_MOUSE* messages when using WM_INPUT.
    if (using_wm_input_ && (event.type() == ui::EventType::kMouseMoved || event.type() == ui::EventType::kMouseDragged)) {
        return 0;
    }
    ...
    if (event.type() == ui::EventType::kMouseDragged) {
        ...
        if (point.x == 0 && point.y == 0 && num_drag_events_after_press_ <= kMaxDragEventsToIgnore00Move) {
            POINT cursor_pos;
            ::GetCursorPos(&cursor_pos);
            ...
            auto distance = sqrt(pow(static_cast<float>(abs(cursor_pos.x)), 2) + pow(static_cast<float>(abs(cursor_pos.y)), 2));
    ...
    if (!handled && message == WM_NCLBUTTONDOWN && w_param != HTSYSMENU && w_param != HTCAPTION && delegate_->GetFrameMode() == FrameMode::CUSTOM_DRAWN) {
...
LRESULT HWNDMessageHandler::HandlePointerEventTypeTouchOrNonClient(UINT message, WPARAM w_param, LPARAM l_param)
{
    UINT32 pointer_id = GET_POINTERID_WPARAM(w_param);
    using GetPointerTouchInfoFn = BOOL(WINAPI*)(UINT32, POINTER_TOUCH_INFO*);
    POINTER_TOUCH_INFO pointer_touch_info;
    static const auto get_pointer_touch_info = reinterpret_cast<GetPointerTouchInfoFn>(base::win::GetUser32FunctionPointer("GetPointerTouchInfo"));
    if (!get_pointer_touch_info || !get_pointer_touch_info(pointer_id, &pointer_touch_info)) {
        SetMsgHandled(FALSE);
        return -1;
    }
    ...
    if (message == WM_POINTERDOWN || message == WM_NCPOINTERDOWN) {
        touch_down_contexts_++;
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&HWNDMessageHandler::ResetTouchDownContext, msg_handler_weak_factory_.GetWeakPtr()), kTouchDownContextResetTimeout);
    }
    ...
    if ((message == WM_POINTERUPDATE) && !(pointer_flags & POINTER_FLAG_INCONTACT)) {
        SetMsgHandled(TRUE);
        return 0;
    }
    ...
    auto mapped_pointer_id = static_cast<ui::PointerId>(id_generator_.GetGeneratedID(pointer_id));
    ...
    float radius_x = (pointer_touch_info.rcContact.right - pointer_touch_info.rcContact.left) / 2.0;
    float radius_y = (pointer_touch_info.rcContact.bottom - pointer_touch_info.rcContact.top) / 2.0;
    auto rotation_angle = static_cast<int>(pointer_touch_info.orientation);
    rotation_angle %= 180;
    if (rotation_angle < 0)
        rotation_angle += 180;

    ui::TouchEvent event(event_type, touch_point, event_time,
        ui::PointerDetails(ui::EventPointerType::kTouch, mapped_pointer_id, radius_x, radius_y, pressure, rotation_angle), ui::GetModifiersFromKeyState());
    ui::ComputeEventLatencyOSFromPOINTER_INFO(event_type, pointer_info, event_time);
    event.latency()->AddLatencyNumberWithTimestamp(ui::INPUT_EVENT_LATENCY_ORIGINAL_COMPONENT, event_time);
    ...
    if (ref) {
        ...
        if (delegate_->GetFrameMode() == FrameMode::SYSTEM_DRAWN) {
            ...
            const bool on_titlebar = SendMessage(hwnd(), WM_NCHITTEST, 0, l_param) == HTCAPTION;
...
LRESULT HWNDMessageHandler::HandlePointerEventTypePen(UINT message, UINT32 pointer_id, POINTER_PEN_INFO pointer_pen_info)
{
    ...
    std::unique_ptr<ui::Event> event = pen_processor_.GenerateEvent(message, pointer_id, pointer_pen_info, point);
...
LRESULT HWNDMessageHandler::HandlePointerEventTypePenClient(UINT message, WPARAM w_param, LPARAM l_param)
{
    UINT32 pointer_id = GET_POINTERID_WPARAM(w_param);
...
bool HWNDMessageHandler::IsSynthesizedMouseMessage(unsigned int message, int message_time, LPARAM l_param)
{
    ...
    // touch message.
    if (last_touch_or_pen_message_time_ && message_time >= last_touch_or_pen_message_time_
        && ((message_time - last_touch_or_pen_message_time_) <= kSynthesizedMouseMessagesTimeDifference)) {
...
void HWNDMessageHandler::UpdateDwmFrame()
{
    ...
    if (delegate_->GetDwmFrameInsetsInPixels(&insets)) {
        MARGINS margins = { insets.left(), insets.right(), insets.top(), insets.bottom() };
        DwmExtendFrameIntoClientArea(hwnd(), &margins);
    }
}

void HWNDMessageHandler::GenerateTouchEvent(
    ui::EventType event_type, const gfx::Point& point, ui::PointerId id, base::TimeTicks time_stamp, TouchEvents* touch_events)
{
    ui::TouchEvent event(event_type, point, time_stamp, ui::PointerDetails(ui::EventPointerType::kTouch, id));

    event.SetFlags(ui::GetModifiersFromKeyState());

    event.latency()->AddLatencyNumberWithTimestamp(ui::INPUT_EVENT_LATENCY_ORIGINAL_COMPONENT, time_stamp);

    touch_events->push_back(event);
}

bool HWNDMessageHandler::HandleMouseInputForCaption(unsigned int message, WPARAM w_param, LPARAM l_param)
{
    ...
    case WM_NCMOUSEMOVE: {
        ...
        if (should_handle_pending_ncl_button_down) {
            l_param = MAKELPARAM(caption_left_button_click_pos_.x(), caption_left_button_click_pos_.y());
...
void HWNDMessageHandler::SetBoundsInternal(const gfx::Rect& bounds_in_pixels, bool force_size_changed)
{
    gfx::Size old_size = GetClientAreaBounds().size();

    SetWindowPos(
        hwnd(), nullptr, bounds_in_pixels.x(), bounds_in_pixels.y(), bounds_in_pixels.width(), bounds_in_pixels.height(), SWP_NOACTIVATE | SWP_NOZORDER);
    ...
    // changed.
    if (old_size == bounds_in_pixels.size() && force_size_changed && !background_fullscreen_hack_) {
        auto ref = msg_handler_weak_factory_.GetWeakPtr();
        delegate_->HandleClientSizeChanged(GetClientAreaBounds().size());
        if (!ref) {
            return;
...
void HWNDMessageHandler::CheckAndHandleBackgroundFullscreenOnMonitor(HWND window)
{
...
void HWNDMessageHandler::SizeWindowToAspectRatio(UINT param, gfx::Rect* window_rect)
{
    gfx::Size min_window_size;
    gfx::Size max_window_size;
    delegate_->GetMinMaxSize(&min_window_size, &max_window_size);
    ...
    gfx::SizeRectToAspectRatioWithExcludedMargin(
        GetWindowResizeEdge(param), aspect_ratio_.value(), min_window_size, max_size_param, excluded_margin_, *window_rect);
}
...
// static
bool HWNDMessageHandler::GetMonitorAndRects(const RECT& rect, HMONITOR* monitor, gfx::Rect* monitor_rect, gfx::Rect* work_area)
{
    DCHECK(monitor);
    DCHECK(monitor_rect);
    DCHECK(work_area);
