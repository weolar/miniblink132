...
public:
    ...
    static std::unique_ptr<HWNDMessageHandler> Create(HWNDMessageHandlerDelegate* delegate, const std::string& debugging_id, bool headless_mode);
    ...
    virtual void GetWindowPlacement(gfx::Rect* bounds, ui::mojom::WindowShowState* show_state) const;
    ...
    virtual void SetBounds(const gfx::Rect& bounds_in_pixels, bool force_size_changed);
    ...
    virtual void Show(ui::mojom::WindowShowState show_state, const gfx::Rect& pixel_restore_bounds);
    virtual void Hide();
    ...
    virtual bool RunMoveLoop(const gfx::Vector2d& drag_offset, bool hide_on_escape);
    ...
    virtual void SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon);
    ...
    virtual void SetAspectRatio(float aspect_ratio, const gfx::Size& excluded_mar);
    ...
    virtual std::unique_ptr<aura::ScopedEnableUnadjustedMouseEvents> RegisterUnadjustedMouseEvent();

    virtual void set_using_wm_input(bool using_wm_input);
    virtual bool using_wm_input() const;

protected:
    HWNDMessageHandler(HWNDMessageHandlerDelegate* delegate, const std::string& debugging_id);
    ...
    virtual void SetBoundsInternal(const gfx::Rect& bounds_in_pixels, bool force_size_changed);

    // These are shared with subclasses.
    static bool IsTopLevelWindow(HWND window);
    static bool GetMonitorAndRects(const RECT& rect, HMONITOR* monitor, gfx::Rect* monitor_rect, gfx::Rect* work_area);
...
private:
    ...
    LRESULT HandleMouseMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled) override;
    LRESULT HandleKeyboardMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled) override;
    LRESULT HandleTouchMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled) override;
    LRESULT HandlePointerMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled) override;
    LRESULT HandleInputMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled) override;
    LRESULT HandleScrollMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled) override;
    LRESULT HandleNcHitTestMessage(unsigned int message, WPARAM w_param, LPARAM l_param, bool* handled) override;
    ...
    void ApplyPanGestureEvent(int scroll_x, int scroll_y, ui::EventMomentumPhase momentum_phase, ui::ScrollEventPhase phase);
    ...
    // windows procedure.
    void PostProcessActivateMessage(int activation_state, bool minimized, HWND window_gaining_or_losing_activation);
    ...
    LRESULT DefWindowProcWithRedrawLock(UINT message, WPARAM w_param, LPARAM l_param);
    ...
    CR_MESSAGE_RANGE_HANDLER_EX(WM_NCMOUSEMOVE, WM_NCXBUTTONDBLCLK, OnMouseRange)
    ...
    LRESULT HandleMouseEventInternal(UINT message, WPARAM w_param, LPARAM l_param, bool track_mouse);
    ...
    // kinds.
    LRESULT HandlePointerEventTypeTouchOrNonClient(UINT message, WPARAM w_param, LPARAM l_param);

    // Helper to handle client area events of PT_PEN.
    LRESULT HandlePointerEventTypePenClient(UINT message, WPARAM w_param, LPARAM l_param);

    // Helper to handle client area events of PT_PEN.
    LRESULT HandlePointerEventTypePen(UINT message, UINT32 pointer_id, POINTER_PEN_INFO pointer_pen_info);
    ...
    bool IsSynthesizedMouseMessage(unsigned int message, int message_time, LPARAM l_param);
    ...
    void GenerateTouchEvent(ui::EventType event_type, const gfx::Point& point, ui::PointerId id, base::TimeTicks time_stamp, TouchEvents* touch_events);

    // Handles WM_NCLBUTTONDOWN and WM_NCMOUSEMOVE messages on the caption.
    // Returns true if the message was handled.
    bool HandleMouseInputForCaption(unsigned int message, WPARAM w_param, LPARAM l_param);
    ...
    static base::LazyInstance<FullscreenWindowMonitorMap>::DestructorAtExit fullscreen_monitor_map_;
    ...
    base::ScopedObservation<ui::InputMethod, ui::InputMethodObserver> observation_ { this };
