...
PenEventProcessor::PenEventProcessor(ui::SequentialIDGenerator* id_generator, bool direct_manipulation_enabled)
...
std::unique_ptr<ui::Event> PenEventProcessor::GenerateEvent(UINT message, UINT32 pointer_id, const POINTER_PEN_INFO& pointer_pen_info, const gfx::Point& point)
{
    ...
    } else if (eraser_pointer_id_ && *eraser_pointer_id_ == mapped_pointer_id && (message == WM_POINTERUP || message == WM_NCPOINTERUP)) {
        input_type = ui::EventPointerType::kEraser;
    ...
    ui::PointerDetails pointer_details(
        input_type, mapped_pointer_id, /* radius_x */ 0.0f, /* radius_y */ 0.0f, pressure, rotation_angle, tilt_x, tilt_y, /* tangential_pressure */ 0.0f);

    int32_t device_id = pen_id_handler_.TryGetPenUniqueId(pointer_id).value_or(ui::ED_UNKNOWN_DEVICE);

    // If the flag is disabled, we send mouse events for all pen inputs.
    if (!direct_manipulation_enabled_) {
        return GenerateMouseEvent(message, pointer_id, pointer_pen_info.pointerInfo, point, pointer_details, device_id);
    }
    bool is_pointer_event = message == WM_POINTERENTER || message == WM_POINTERLEAVE;
    ...
    bool send_touch = send_touch_for_pen_.count(pointer_id) == 0 ? false : send_touch_for_pen_[pointer_id];
        ...
        if (!pen_in_contact_[pointer_id]) {
            send_touch = send_touch_for_pen_[pointer_id]
                = (pointer_pen_info.pointerInfo.pointerFlags
                      & (POINTER_FLAG_SECONDBUTTON | POINTER_FLAG_THIRDBUTTON | POINTER_FLAG_FOURTHBUTTON | POINTER_FLAG_FIFTHBUTTON))
                == 0;
        }
    ...
    if (stylus_handwriting::win::IsStylusHandwritingWinEnabled() && message == WM_POINTERDOWN) {
        handwriting_properties = std::make_optional<ui::StylusHandwritingPropertiesWin>();
        handwriting_properties->handwriting_pointer_id = pointer_id;
        handwriting_properties->handwriting_stroke_id = ui::GetHandwritingStrokeId(pointer_id);
    }

    return GenerateTouchEvent(message, pointer_id, pointer_pen_info.pointerInfo, point, pointer_details, handwriting_properties, device_id);
}

std::unique_ptr<ui::Event> PenEventProcessor::GenerateMouseEvent(
    UINT message, UINT32 pointer_id, const POINTER_INFO& pointer_info, const gfx::Point& point, const ui::PointerDetails& pointer_details, int32_t device_id)
{
    ...
    case WM_NCPOINTERUP:
        ...
        click_count = 1;
        if (sent_mouse_down_.count(pointer_id) == 0 || !sent_mouse_down_[pointer_id])
            return nullptr;
    ...
    std::unique_ptr<ui::Event> event = std::make_unique<ui::MouseEvent>(
        event_type, point, point, ui::EventTimeForNow(), flag | ui::GetModifiersFromKeyState(), changed_flag, pointer_details);
    event->AsMouseEvent()->SetClickCount(click_count);
    event->set_source_device_id(device_id);
    return event;
}

std::unique_ptr<ui::Event> PenEventProcessor::GenerateTouchEvent(UINT message, UINT32 pointer_id, const POINTER_INFO& pointer_info, const gfx::Point& point,
    const ui::PointerDetails& pointer_details, const std::optional<ui::StylusHandwritingPropertiesWin>& handwriting_properties, int32_t device_id)
{
    ...
    case WM_NCPOINTERUP:
        ...
        id_generator_->ReleaseNumber(pointer_id);
        if (sent_touch_start_.count(pointer_id) == 0 || !sent_touch_start_[pointer_id])
            return nullptr;
    ...
    std::unique_ptr<ui::TouchEvent> event
        = std::make_unique<ui::TouchEvent>(event_type, point, event_time, pointer_details, flags | ui::GetModifiersFromKeyState());
