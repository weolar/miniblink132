...
HWNDMessageHandlerHeadless::HWNDMessageHandlerHeadless(HWNDMessageHandlerDelegate* delegate, const std::string& debugging_id)
...
void HWNDMessageHandlerHeadless::Init(HWND parent, const gfx::Rect& bounds)
{
    ...
    if (initial_bounds_valid_) {
        ...
        if (::GetWindowRect(hwnd(), &window_rect)) {
            ...
            SetHeadlessWindowBounds(ScaleWindowBoundsMaybe(hwnd(), kDefaultHeadlessWindowSize));
        }
    }
...
void HWNDMessageHandlerHeadless::GetWindowPlacement(gfx::Rect* bounds, ui::mojom::WindowShowState* show_state) const
{
    if (bounds) {
...
void HWNDMessageHandlerHeadless::Show(ui::mojom::WindowShowState show_state, const gfx::Rect& pixel_restore_bounds)
{
...
void HWNDMessageHandlerHeadless::SetFullscreen(bool fullscreen, int64_t target_display_id)
{
    if (fullscreen) {
...
void HWNDMessageHandlerHeadless::SetHeadlessWindowBounds(const gfx::Rect& bounds)
{
    if (bounds_ != bounds) {
        bounds_ = bounds;
...
void HWNDMessageHandlerHeadless::SetBoundsInternal(const gfx::Rect& bounds_in_pixels, bool force_size_changed)
{
