...
{
    Microsoft::WRL::ComPtr<FakeIPenDeviceStatics> pen_device_statics = FakeIPenDeviceStatics::GetInstance();
    views::PenIdHandler::ScopedPenIdStaticsForTesting scoper(&FakeIPenDeviceStatics::FakeIPenDeviceStaticsComPtr);
    ...
    const std::optional<int32_t> id = pen_id_handler.TryGetPenUniqueId(kPointerId1);

    const std::optional<int32_t> id2 = pen_id_handler.TryGetPenUniqueId(kPointerId2);
    EXPECT_NE(id, id2);

    const std::optional<int32_t> id3 = pen_id_handler.TryGetPenUniqueId(kPointerId3);
    EXPECT_NE(id2, id3);
    EXPECT_NE(id, id3);
...
{
    auto* null_lambda = static_cast<Microsoft::WRL::ComPtr<ABI::Windows::Devices::Input::IPenDeviceStatics> (*)()>(
        []() -> Microsoft::WRL::ComPtr<ABI::Windows::Devices::Input::IPenDeviceStatics> { return nullptr; });
