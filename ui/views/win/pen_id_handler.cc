...
public:
    PenIdStatics()
    {
        ...
        base::win::RoGetActivationFactory(base::win::HStringReference(RuntimeClass_Windows_Devices_Input_PenDevice).Get(), IID_PPV_ARGS(&pen_device_statics_));
        TRACE_EVENT_INSTANT0("event", "PenIdStatics::PenIdStatics", TRACE_EVENT_SCOPE_THREAD);
    }
...
bool PenDeviceApiSupported()
{
    ...
    return base::win::OSInfo::Kernel32Version() > base::win::Version::WIN10_21H2
        || (base::win::OSInfo::Kernel32Version() == base::win::Version::WIN10_21H2 && base::win::OSInfo::GetInstance()->version_number().patch >= 1503);
}

} // namespace

PenIdHandler::ScopedPenIdStaticsForTesting::ScopedPenIdStaticsForTesting(PenIdHandler::GetPenDeviceStatics pen_device_statics)
    : pen_device_resetter_(&get_pen_device_statics, pen_device_statics)
{
}

PenIdHandler::ScopedPenIdStaticsForTesting::~ScopedPenIdStaticsForTesting() = default;
...
std::optional<std::string> PenIdHandler::TryGetGuid(UINT32 pointer_id) const
{
    ...
    const Microsoft::WRL::ComPtr<IPenDeviceStatics> pen_device_statics
        = get_pen_device_statics ? (*get_pen_device_statics)() : PenIdStatics::GetInstance()->PenDeviceStatics();
    ...
    if (!pen_device_statics) {
        TRACE_EVENT_INSTANT0("event", "PenIdHandler::TryGetGuid no statics", TRACE_EVENT_SCOPE_THREAD);
        return std::nullopt;
    }
    ...
    if (FAILED(hr) || !pen_device) {
        TRACE_EVENT_INSTANT0("event", "PenIdHandler::TryGetGuid GetFromPointerId failed", TRACE_EVENT_SCOPE_THREAD);
        return std::nullopt;
    }
    ...
    if (FAILED(hr)) {
        TRACE_EVENT_INSTANT0("event", "PenIdHandler::TryGetGuid get_PenId failed", TRACE_EVENT_SCOPE_THREAD);
        return std::nullopt;
    }

    TRACE_EVENT_INSTANT0("event", "PenIdHandler::TryGetGuid successful", TRACE_EVENT_SCOPE_THREAD);
...
void PenIdHandler::InitPenIdStatics()
{
    ...
    // loaded.
    base::ThreadPool::PostTask(FROM_HERE, base::BindOnce(base::IgnoreResult(&PenIdStatics::GetInstance)));
}
