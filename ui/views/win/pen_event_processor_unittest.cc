...
{
    scoped_feature_list_.InitAndEnableFeature(stylus_handwriting::win::kStylusHandwritingWin);
}
...
TEST_F(PenProcessorTest, TypicalCaseDMDisabled)
{
    ...
    std::unique_ptr<ui::Event> event = processor.GenerateEvent(WM_POINTERENTER, 0, pen_info, point);
    ASSERT_TRUE(event);
    ASSERT_TRUE(event->IsMouseEvent());
    EXPECT_EQ(ui::EventType::kMouseEntered, event->AsMouseEvent()->type());

    pen_info.pointerInfo.pointerFlags = POINTER_FLAG_INCONTACT | POINTER_FLAG_FIRSTBUTTON;
    ...
    EXPECT_EQ(ui::EF_LEFT_MOUSE_BUTTON, event->AsMouseEvent()->changed_button_flags());
...
TEST_F(PenProcessorTest, UnpairedPointerDownTouchDMEnabled)
{
    ...
    std::unique_ptr<ui::Event> event = processor.GenerateEvent(WM_POINTERUP, 0, pen_info, point);
...
TEST_F(PenProcessorTest, TouchFlagDMEnabled)
{
    ...
    std::unique_ptr<ui::Event> event = processor.GenerateEvent(WM_POINTERDOWN, 0, pen_info, point);
    ASSERT_TRUE(event);
    ASSERT_TRUE(event->IsTouchEvent());
    EXPECT_EQ(ui::EventType::kTouchPressed, event->AsTouchEvent()->type());
...
TEST_F(PenProcessorTest, PenEraserFlagDMEnabled)
{
    ...
    ASSERT_TRUE(event->IsTouchEvent());
    EXPECT_EQ(ui::EventType::kTouchPressed, event->AsTouchEvent()->type());
    EXPECT_EQ(ui::EventPointerType::kEraser, event->AsTouchEvent()->pointer_details().pointer_type);
...
TEST_F(PenProcessorTest, MultiPenDMEnabled)
{
    ...
    for (size_t i = 0; i < pen_info.size(); i++) {
        pen_info[i].pointerInfo.pointerFlags = POINTER_FLAG_INCONTACT | POINTER_FLAG_FIRSTBUTTON;
        pen_info[i].pointerInfo.ButtonChangeType = POINTER_CHANGE_FIRSTBUTTON_DOWN;

        size_t pointer_id = i;
        std::unique_ptr<ui::Event> event = processor.GenerateEvent(WM_POINTERDOWN, pointer_id, pen_info[i], point);
        ASSERT_TRUE(event);
        ASSERT_TRUE(event->IsTouchEvent());
        EXPECT_EQ(ui::EventType::kTouchPressed, event->AsTouchEvent()->type());
    }
    ...
    for (size_t i = 0; i < pen_info.size(); i++) {
        ...
        size_t pointer_id = i;
        std::unique_ptr<ui::Event> event = processor.GenerateEvent(WM_POINTERUP, pointer_id, pen_info[i], point);
        ASSERT_TRUE(event);
        ASSERT_TRUE(event->IsTouchEvent());
        EXPECT_EQ(ui::EventType::kTouchReleased, event->AsTouchEvent()->type());
    }
...
TEST_F(PenProcessorTest, StylusHandwritingPropertiesDMEnabled)
{
    ...
    std::unique_ptr<ui::Event> event = processor.GenerateEvent(WM_POINTERDOWN, pointer_id, pen_info, point);
    ASSERT_TRUE(event);
    ASSERT_TRUE(event->IsTouchEvent());
    ASSERT_TRUE(event->AsTouchEvent()->properties());
    EXPECT_FALSE(event->AsTouchEvent()->properties()->empty());

    const std::optional<ui::StylusHandwritingPropertiesWin> properties = ui::GetStylusHandwritingProperties(*event);
    ...
    pen_info.pointerInfo.ButtonChangeType = POINTER_CHANGE_NONE;
    event = processor.GenerateEvent(WM_POINTERUPDATE, pointer_id, pen_info, point);
    ASSERT_TRUE(event);
    ASSERT_TRUE(event->IsMouseEvent());
