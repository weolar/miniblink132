...
DEFINE_OWNED_UI_CLASS_PROPERTY_KEY(base::debug::StackTrace, kViewStackTraceKey, nullptr)
...
void ViewDebugWrapperImpl::ForAllProperties(PropCallback callback)
{
    ...
    for (auto* member : *(view->GetClassMetaData())) {
        ...
        if (!!(flags & ui::metadata::PropertyFlags::kSerializable)) {
            callback.Run(member->member_name(), base::UTF16ToUTF8(member->GetValueAsString(view)));
        }
    }
}
...
{
    std::string debug_string = std::string("View: ") + view->GetClassName() + "\n";
    debug_string += std::string("Hierarchy:\n    ") + GetViewTreeAsString(view);

    debug_string += "\nView created here:\n";

    static bool has_stack_trace = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kViewStackTraces);

    if (has_stack_trace) {
        debug_string += view->GetProperty(kViewStackTraceKey)->ToString();
    } else {
        debug_string += std::string("  Run with --") + switches::kViewStackTraces + " to get a stack trace for when this View was created.";
    }
