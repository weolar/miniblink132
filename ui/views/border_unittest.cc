...
public:
    struct DrawRectCall {
        ...
        bool operator<(const DrawRectCall& other) const
        {
            return std::tie(rect.fLeft, rect.fTop, rect.fRight, rect.fBottom)
                < std::tie(other.rect.fLeft, other.rect.fTop, other.rect.fRight, other.rect.fBottom);
        }
    ...
    struct DrawRRectCall {
        ...
        bool operator<(const DrawRRectCall& other) const
        {
            ...
            return std::tie(rect.fLeft, rect.fTop, rect.fRight, rect.fBottom)
                < std::tie(other_rect.fLeft, other_rect.fTop, other_rect.fRight, other_rect.fBottom);
        }
    ...
    {
        return std::vector<DrawRectCall>(draw_rect_calls_.begin(), draw_rect_calls_.end());
    }

    // Return calls in sorted order.
    std::vector<DrawRRectCall> draw_rrect_calls()
    {
        return std::vector<DrawRRectCall>(draw_rrect_calls_.begin(), draw_rrect_calls_.end());
    }
    ...
    const SkRect& last_clip_bounds() const
    {
        return last_clip_bounds_;
    }
...
public:
    ...
    gfx::Canvas* given_canvas() const
    {
        return given_canvas_;
    }

    // Gets the size given to the last call to Paint().
    const gfx::Size& given_size() const
    {
        return given_size_;
    }
...
public:
    ...
    std::unique_ptr<MockCanvas> DrawIntoMockCanvas()
    {
        ...
        std::unique_ptr<MockCanvas> mock(new MockCanvas(kCanvasWidth, kCanvasHeight));
...
TEST_F(BorderTest, SolidBorder)
{
    ...
    std::vector<MockCanvas::DrawRectCall> draw_rect_calls = mock->draw_rect_calls();
...
{
    std::unique_ptr<Border> border(CreateRoundedRectBorder(3, LayoutProvider::Get()->GetCornerRadiusMetric(Emphasis::kLow), SK_ColorBLUE));
    EXPECT_EQ(gfx::Size(6, 6), border->GetMinimumSize());
    EXPECT_EQ(gfx::Insets(3), border->GetInsets());
    border->Paint(*view_, canvas_.get());
    ...
    std::vector<MockCanvas::DrawRRectCall> draw_rrect_calls = mock->draw_rrect_calls();
...
TEST_F(BorderTest, BorderPainter)
{
    ...
    std::unique_ptr<Border> border(CreateBorderPainter(std::move(painter), kInsets));
...
TEST_F(BorderTest, ExtraInsetsBorder)
{
    ...
    std::unique_ptr<Border> border = CreateSolidBorder(kOriginalInset, kBorderColor);
    ...
    std::unique_ptr<Border> extra_insets_border = CreatePaddedBorder(std::move(border), kExtraInsets);
