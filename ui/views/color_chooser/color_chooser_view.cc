...
protected:
    ...
    {
        if (event->type() == ui::EventType::kGestureTap || event->type() == ui::EventType::kGestureTapDown || event->IsScrollGestureEvent()) {
            ProcessEventAtLocation(event->location());
            event->SetHandled();
        }
    }
...
void DrawGradientRect(const gfx::Rect& rect, SkColor start_color, SkColor end_color, bool is_horizontal, gfx::Canvas* canvas)
{
    // TODO(crbug.com/40219248): Remove FromColor and make all SkColor4f.
    SkColor4f colors[2] = { SkColor4f::FromColor(start_color), SkColor4f::FromColor(end_color) };
    SkPoint points[2];
    points[0].iset(0, 0);
    if (is_horizontal)
    ...
    cc::PaintFlags flags;
    flags.setShader(cc::PaintShader::MakeLinearGradient(points, colors, nullptr, 2, SkTileMode::kClamp));
...
{
    level_ = std::max(kBorderWidth, std::min(height() - 1 - kBorderWidth, point.y()));
    int base_height = kSaturationValueSize - 1;
    changed_callback_.Run(360.f * (base_height - (level_ - kBorderWidth)) / base_height);
    SchedulePaint();
}

gfx::Size HueView::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
    // We put indicators on the both sides of the hue bar.
    return gfx::Size(kHueBarWidth + kHueIndicatorSize * 2 + kBorderWidth * 2, kSaturationValueSize + kBorderWidth * 2);
}
...
void HueView::OnPaint(gfx::Canvas* canvas)
{
    ...
    canvas->FillRect(gfx::Rect(kHueIndicatorSize, 0, kHueBarWidth + kBorderWidth, height() - 1), background_color_);
    ...
    for (int y = 0; y < kSaturationValueSize; ++y) {
        ...
        canvas->FillRect(gfx::Rect(base_left, y + kBorderWidth, kHueBarWidth, 1), SkHSVToColor(hsv));
    }

    // Put the triangular indicators besides.
    SkPath left_indicator_path;
    SkPath right_indicator_path;
    left_indicator_path.moveTo(SK_ScalarHalf, SkIntToScalar(level_ - kHueIndicatorSize));
    left_indicator_path.lineTo(kHueIndicatorSize, SkIntToScalar(level_));
    left_indicator_path.lineTo(SK_ScalarHalf, SkIntToScalar(level_ + kHueIndicatorSize));
    left_indicator_path.lineTo(SK_ScalarHalf, SkIntToScalar(level_ - kHueIndicatorSize));
    right_indicator_path.moveTo(SkIntToScalar(width()) - SK_ScalarHalf, SkIntToScalar(level_ - kHueIndicatorSize));
    right_indicator_path.lineTo(SkIntToScalar(width() - kHueIndicatorSize) - SK_ScalarHalf, SkIntToScalar(level_));
    right_indicator_path.lineTo(SkIntToScalar(width()) - SK_ScalarHalf, SkIntToScalar(level_ + kHueIndicatorSize));
    right_indicator_path.lineTo(SkIntToScalar(width()) - SK_ScalarHalf, SkIntToScalar(level_ - kHueIndicatorSize));
...
public:
    using SaturationValueChangedCallback = base::RepeatingCallback<void(SkScalar, SkScalar)>;
    explicit SaturationValueView(const SaturationValueChangedCallback& changed_callback);
...
SaturationValueView::SaturationValueView(const SaturationValueChangedCallback& changed_callback)
    : changed_callback_(changed_callback)
    ,
...
{
    LocatedEventHandlerView::OnThemeChanged();
    GetBorder()->set_color(GetColorProvider()->GetColor(ui::kColorFocusableBorderUnfocused));
    SchedulePaint();
}
...
void SaturationValueView::OnSaturationValueChanged(SkScalar saturation, SkScalar value)
{
...
gfx::Size SaturationValueView::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
    return gfx::Size(kSaturationValueSize + kBorderWidth * 2, kSaturationValueSize + kBorderWidth * 2);
}
...
void SaturationValueView::OnPaint(gfx::Canvas* canvas)
{
    ...
    DrawGradientRect(color_bounds, SkHSVToColor(255, left_hsv), SkHSVToColor(255, hsv), true /* is_horizontal */, canvas);

    // Overlays vertical gradient for value.
    SkScalar hsv_bottom[3] = { 0, SK_Scalar1, 0 };
    DrawGradientRect(color_bounds, SK_ColorTRANSPARENT, SkHSVToColor(255, hsv_bottom), false /* is_horizontal */, canvas);

    // Draw the crosshair marker.
    canvas->FillRect(
        gfx::Rect(marker_position_.x(), marker_position_.y() - kSaturationValueIndicatorSize, 1, kSaturationValueIndicatorSize * 2 + 1), marker_color_);
    canvas->FillRect(
        gfx::Rect(marker_position_.x() - kSaturationValueIndicatorSize, marker_position_.y(), kSaturationValueIndicatorSize * 2 + 1, 1), marker_color_);
...
{
    SetVisible(true);
    SetBorder(CreateThemedSolidBorder(kBorderWidth, ui::kColorFocusableBorderUnfocused));
}
...
std::unique_ptr<View> ColorChooser::BuildView()
{
    ...
    view->SetLayoutManager(std::make_unique<BoxLayout>(BoxLayout::Orientation::kVertical, gfx::Insets(kMarginWidth), kMarginWidth));

    auto container = std::make_unique<View>();
    container->SetLayoutManager(std::make_unique<BoxLayout>(BoxLayout::Orientation::kHorizontal, gfx::Insets(), kMarginWidth));
    saturation_value_ = container->AddChildView(
        std::make_unique<SaturationValueView>(base::BindRepeating(&ColorChooser::OnSaturationValueChosen, weak_ptr_factory_.GetWeakPtr())));
    hue_ = container->AddChildView(std::make_unique<HueView>(base::BindRepeating(&ColorChooser::OnHueChosen, weak_ptr_factory_.GetWeakPtr())));
    view->AddChildView(std::move(container));

    auto container2 = std::make_unique<View>();
    BoxLayout* layout = container2->SetLayoutManager(std::make_unique<views::BoxLayout>(BoxLayout::Orientation::kHorizontal, gfx::Insets(), kMarginWidth));
    auto textfield = std::make_unique<Textfield>();
    textfield->set_controller(this);
    textfield->SetDefaultWidthInChars(kTextfieldLengthInChars);
    textfield->GetViewAccessibility().SetName(l10n_util::GetStringUTF16(IDS_APP_ACCNAME_COLOR_CHOOSER_HEX_INPUT));
    textfield_ = container2->AddChildView(std::move(textfield));
    selected_color_patch_ = container2->AddChildView(std::make_unique<SelectedColorPatchView>());
...
void ColorChooser::OnSaturationValueChosen(SkScalar saturation, SkScalar value)
{
...
void ColorChooser::ContentsChanged(Textfield* sender, const std::u16string& new_contents)
{
...
bool ColorChooser::HandleKeyEvent(Textfield* sender, const ui::KeyEvent& key_event)
{
    DCHECK(IsViewAttached());

    if (key_event.type() != ui::EventType::kKeyPressed || (key_event.key_code() != ui::VKEY_RETURN && key_event.key_code() != ui::VKEY_ESCAPE)) {
        return false;
    }
...
std::unique_ptr<WidgetDelegate> ColorChooser::MakeWidgetDelegate()
{
    ...
    delegate->RegisterWindowClosingCallback(base::BindOnce(&ColorChooser::OnViewClosing, weak_ptr_factory_.GetWeakPtr()));
