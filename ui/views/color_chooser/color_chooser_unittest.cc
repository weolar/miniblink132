...
public:
    ...
    void SetUp() override
    {
        ...
        widget_ = CreateTestWidget(views::Widget::InitParams::CLIENT_OWNS_WIDGET, views::Widget::InitParams::TYPE_WINDOW);
        widget_->GetContentsView()->AddChildView(std::move(view));
        generator_ = std::make_unique<ui::test::EventGenerator>(views::GetRootWindow(widget_.get()), widget_->GetNativeWindow());
    }
    ...
    {
        return chooser_->selected_color_patch_for_testing()->background()->get_color();
    }
    ...
    SkColor GetTextualColor() const
    {
        ...
        uint32_t color;
        return base::HexStringToUInt(base::UTF16ToUTF8(text.substr(1)), &color) ? SkColorSetA(color, SK_AlphaOPAQUE) : SK_ColorTRANSPARENT;
    }
    ...
    void TypeColor(const std::string& color)
    {
        ...
        chooser_->ContentsChanged(chooser_->textfield_for_testing(), chooser_->textfield_for_testing()->GetText());
    }
...
#endif
        ui::MouseEvent press(ui::EventType::kMousePressed, gfx::Point(view->x() + p.x(), view->y() + p.y()), gfx::Point(0, 0), base::TimeTicks::Now(), 0, 0);
