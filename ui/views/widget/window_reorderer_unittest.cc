...
protected:
    ...
    {
        Widget::InitParams params = CreateParamsForTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_CONTROL);
...
TEST_F(WindowReordererTest, Basic)
{
    ...
    EXPECT_EQ("v w1 w2", ui::test::ChildLayerNamesAsString(*parent_window->layer()));
    ...
    EXPECT_EQ("v w2 w1", ui::test::ChildLayerNamesAsString(*parent_window->layer()));

    auto* host_view1 = contents_view->AddChildViewAt(std::make_unique<View>(), 0);
    w1->GetNativeView()->SetProperty(kHostViewKey, host_view1);
    EXPECT_EQ("w1 w2", ChildWindowNamesAsString(*parent_window));
    EXPECT_EQ("w1 v w2", ui::test::ChildLayerNamesAsString(*parent_window->layer()));
    ...
    EXPECT_EQ("w1 w2", ChildWindowNamesAsString(*parent_window));
    EXPECT_EQ("w1 w2 v", ui::test::ChildLayerNamesAsString(*parent_window->layer()));
...
TEST_F(WindowReordererTest, Association)
{
    ...
    aura::Window* w1 = aura::test::CreateTestWindowWithId(0, parent->GetNativeWindow());
    ...
    EXPECT_EQ("w2 w1", ChildWindowNamesAsString(*parent_window));
    EXPECT_EQ("w2 w1", ui::test::ChildLayerNamesAsString(*parent_window->layer()));
    ...
    EXPECT_EQ("w1 w2", ChildWindowNamesAsString(*parent_window));
    EXPECT_EQ("w1 w2", ui::test::ChildLayerNamesAsString(*parent_window->layer()));
...
TEST_F(WindowReordererTest, HostViewParentHasLayer)
{
    ...
    EXPECT_EQ("v11 w v13 v2", ui::test::ChildLayerNamesAsString(*parent_window->layer()));

    // |w|'s layer should be stacked above |v1|'s layer.
    v1->SetPaintToLayer();
    v1->layer()->SetName("v1");
    EXPECT_EQ("w", ChildWindowNamesAsString(*parent_window));
    EXPECT_EQ("v1 w v2", ui::test::ChildLayerNamesAsString(*parent_window->layer()));

    // Test moving the host view from one view with a layer to another.
    v2->AddChildView(v1->RemoveChildViewT(v12));
    EXPECT_EQ("w", ChildWindowNamesAsString(*parent_window));
    EXPECT_EQ("v1 v2 w", ui::test::ChildLayerNamesAsString(*parent_window->layer()));
}
...
TEST_F(WindowReordererTest, ViewWithLayerBeneath)
{
    ...
    View* view_with_layer_beneath = contents_view->AddChildView(std::make_unique<View>());
    ui::Layer layer_beneath;
    view_with_layer_beneath->AddLayerToRegion(&layer_beneath, LayerRegion::kBelow);
    ...
    EXPECT_EQ("beneath view", ui::test::ChildLayerNamesAsString(*parent_window->layer()));
    ...
    EXPECT_EQ("beneath view child_widget", ui::test::ChildLayerNamesAsString(*parent_window->layer()));
}
