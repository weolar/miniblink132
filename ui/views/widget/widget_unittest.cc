...
std::unique_ptr<ui::test::EventGenerator> CreateEventGenerator(gfx::NativeWindow root_window, gfx::NativeWindow target_window)
{
    auto generator = std::make_unique<ui::test::EventGenerator>(root_window, target_window);
    return generator;
}
...
{
    return ui::GestureEvent(x, y, 0, base::TimeTicks(), ui::GestureEventDetails(type));
}

ui::GestureEvent CreateTestGestureEvent(const ui::GestureEventDetails& details, int x, int y)
{
...
public:
    ...
    TestWidgetRemovalsObserver& operator=(const TestWidgetRemovalsObserver&) = delete;
...
public:
    ...
    void SetInitFunction(const InitFunction& init)
    {
        init_ = std::move(init);
    }
    Widget::InitParams CreateParams(Widget::InitParams::Ownership ownership, Widget::InitParams::Type type) override
    {
...
{
    SetInitFunction(base::BindLambdaForTesting([](Widget::InitParams* params) { params->name = "MyWidget"; }));
    std::unique_ptr<Widget> widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
...
TEST_F(WidgetWithCustomParamsTest, NamePropagatedFromDelegate)
{
    ...
    SetInitFunction(base::BindLambdaForTesting([&](Widget::InitParams* params) { params->delegate = &delegate; }));

    std::unique_ptr<Widget> widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);

    EXPECT_EQ(delegate.internal_name(), widget->native_widget_private()->GetName());
...
{
    SetInitFunction(base::BindLambdaForTesting([](Widget::InitParams* params) { params->autosize = true; }));
...
TEST_F(WidgetWithCustomParamsTest, NamePropagatedFromContentsViewClassName)
{
    ...
    EXPECT_EQ(contents->GetClassName(), widget->native_widget_private()->GetName());
...
public:
    ...
    void SetUp() override
    {
        ...
        ui::ColorProviderManager& manager = ui::ColorProviderManager::GetForTesting();
...
private:
    static void AddColor(ui::ColorProvider* provider, const ui::ColorProviderKey& key)
    {
        ui::ColorMixer& mixer = provider->AddMixer();
        mixer[ui::kColorSysPrimary] = { key.color_mode == ui::ColorProviderKey::ColorMode::kDark ? kDarkColor : kLightColor };
    }
};
...
TEST_F(WidgetColorModeTest, ColorModeOverride_NoOverride)
{
    ...
    std::unique_ptr<Widget> widget = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    ...
    EXPECT_EQ(kDarkColor, widget->GetColorProvider()->GetColor(ui::kColorSysPrimary));
}
...
TEST_F(WidgetColorModeTest, ColorModeOverride_LightOverride)
{
    ...
    // Verify that we resolve the light color even though the theme is dark.
    EXPECT_EQ(kLightColor, widget->GetColorProvider()->GetColor(ui::kColorSysPrimary));
}
...
TEST_F(WidgetColorModeTest, ChildInheritsColorMode_NoOverrides)
{
    ...
    std::unique_ptr<Widget> widget_child = base::WrapUnique(CreateChildPlatformWidget(widget->GetNativeView(), Widget::InitParams::CLIENT_OWNS_WIDGET));
    ...
    EXPECT_EQ(kDarkColor, widget->GetColorProvider()->GetColor(ui::kColorSysPrimary));
    EXPECT_EQ(kDarkColor, widget_child->GetColorProvider()->GetColor(ui::kColorSysPrimary));

    // Set the parent's native theme to light. The child should inherit the color
    // mode of the parent.
    test_theme.SetDarkMode(false);
    EXPECT_EQ(kLightColor, widget->GetColorProvider()->GetColor(ui::kColorSysPrimary));
    EXPECT_EQ(kLightColor, widget_child->GetColorProvider()->GetColor(ui::kColorSysPrimary));
}
...
TEST_F(WidgetTest, GetParent)
{
    ...
    std::unique_ptr<Widget> toplevel = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    std::unique_ptr<Widget> child = base::WrapUnique(CreateChildPlatformWidget(toplevel->GetNativeView(), Widget::InitParams::CLIENT_OWNS_WIDGET));
    std::unique_ptr<Widget> grandchild = base::WrapUnique(CreateChildPlatformWidget(child->GetNativeView(), Widget::InitParams::CLIENT_OWNS_WIDGET));
...
TEST_F(WidgetTest, ArrowKeyFocusTraversalOffByDefault)
{
    ...
    auto* const button1 = container->AddChildView(std::make_unique<LabelButton>());
    auto* const button2 = container->AddChildView(std::make_unique<LabelButton>());
    toplevel->Show();
    button1->RequestFocus();

    ui::KeyEvent right_arrow(ui::EventType::kKeyPressed, ui::VKEY_RIGHT, ui::EF_NONE);
    toplevel->OnKeyEvent(&right_arrow);
    EXPECT_TRUE(button1->HasFocus());
    EXPECT_FALSE(button2->HasFocus());

    ui::KeyEvent left_arrow(ui::EventType::kKeyPressed, ui::VKEY_LEFT, ui::EF_NONE);
    ...
    ui::KeyEvent down_arrow(ui::EventType::kKeyPressed, ui::VKEY_DOWN, ui::EF_NONE);
...
TEST_F(WidgetTest, ArrowKeyTraversalMovesFocusBetweenViews)
{
    ...
    auto* const button1 = container->AddChildView(std::make_unique<LabelButton>());
    auto* const button2 = container->AddChildView(std::make_unique<LabelButton>());
    auto* const button3 = container->AddChildView(std::make_unique<LabelButton>());
    toplevel->Show();
    button1->RequestFocus();
    ...
    ui::KeyEvent down_arrow2(ui::EventType::kKeyPressed, ui::VKEY_DOWN, ui::EF_NONE);
...
{
    std::unique_ptr<Widget> parent = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    std::unique_ptr<Widget> child = base::WrapUnique(CreateChildPlatformWidget(parent->GetNativeView(), Widget::InitParams::CLIENT_OWNS_WIDGET));
...
TEST_F(WidgetTest, GetTopLevelWidget_Native)
{
    ...
    std::unique_ptr<Widget> child = base::WrapUnique(CreateChildPlatformWidget(parent, Widget::InitParams::CLIENT_OWNS_WIDGET));
...
{
    std::unique_ptr<Widget> top1 = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    top1->Show();
    RunPendingMessages();

    std::unique_ptr<Widget> top2 = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    top2->Show();
    RunPendingMessages();
...
TEST_F(NativeWidgetOwnsWidgetTest, NonDesktopWidget_CloseNow)
{
    ...
    Widget::InitParams params = CreateParams(Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP);
    params.native_widget = CreatePlatformNativeWidgetImpl(widget, kStubCapture, &state()->native_widget_deleted);
...
TEST_F(NativeWidgetOwnsWidgetTest, NonDesktopWidget_Close)
{
    ...
    Widget* widget = new OwnershipTestWidget(state());
    Widget::InitParams params = CreateParams(Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP);
    params.native_widget = CreatePlatformNativeWidgetImpl(widget, kStubCapture, waiter.GetNativeWidgetDestroyedCallback());
    widget->Init(std::move(params));
...
TEST_F(NativeWidgetOwnsWidgetTest, DesktopWidget_Close)
{
    ...
    Widget* widget = new OwnershipTestWidget(state());
    Widget::InitParams params = CreateParams(Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP);
    params.native_widget = CreatePlatformDesktopNativeWidgetImpl(widget, kStubCapture, waiter.GetNativeWidgetDestroyedCallback());
    widget->Init(std::move(params));
...
TEST_F(WidgetOwnsNativeWidgetTest, Ownership)
{
    ...
    Widget::InitParams params = CreateParamsForTestWidget(Widget::InitParams::WIDGET_OWNS_NATIVE_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    params.native_widget = CreatePlatformNativeWidgetImpl(widget.get(), kStubCapture, &state()->native_widget_deleted);
    widget->Init(std::move(params));
...
{
    auto widget = std::make_unique<OwnershipTestWidget>(state());
    Widget::InitParams params = CreateParamsForTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    params.native_widget = CreatePlatformNativeWidgetImpl(widget.get(), kStubCapture, &state()->native_widget_deleted);
    widget->Init(std::move(params));
...
public:
    ...
    int widget_destroying_count() const
    {
        return widget_destroying_count_;
    }

    int widget_destroyed_count() const
    {
        return widget_destroyed_count_;
    }
...
class WidgetWithDestroyedNativeViewOrNativeWidgetTest
    : public ViewsTestBase,
      public testing::WithParamInterface<std::tuple<ViewsTestBase::NativeWidgetType, Widget::InitParams::Ownership>> {
public:
    WidgetWithDestroyedNativeViewOrNativeWidgetTest() = default;

    WidgetWithDestroyedNativeViewOrNativeWidgetTest(const WidgetWithDestroyedNativeViewOrNativeWidgetTest&) = delete;
    WidgetWithDestroyedNativeViewOrNativeWidgetTest& operator=(const WidgetWithDestroyedNativeViewOrNativeWidgetTest&) = delete;
    ...
    void SetUp() override
    {
        set_native_widget_type(std::get<ViewsTestBase::NativeWidgetType>(GetParam()));
        ViewsTestBase::SetUp();
        if (std::get<Widget::InitParams::Ownership>(GetParam()) == Widget::InitParams::CLIENT_OWNS_WIDGET) {
    ...
    static std::string PrintTestName(const ::testing::TestParamInfo<WidgetWithDestroyedNativeViewOrNativeWidgetTest::ParamType>& info)
    {
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, AddAndRemoveRemovalsObserver)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, CloseAllSecondaryWidgets)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, CreateNonClientFrameView)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, GetClientAreaBoundsInScreen)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, GetNativeWindowProperty)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, GetPrimaryWindowWidget)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, GetTopLevelWidgetForNativeView)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, GetWidgetForNativeView)
{
    Widget::GetWidgetForNativeView(widget()->GetNativeView());
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, GetWidgetForNativeWindow)
{
    Widget::GetWidgetForNativeWindow(widget()->GetNativeWindow());
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, GetWindowBoundsInScreen)
{
    widget()->GetWindowBoundsInScreen();
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, GetWorkAreaBoundsInScreen)
{
...
{
    Widget::InitParams params(std::get<Widget::InitParams::Ownership>(GetParam()));
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, IsNativeWidgetInitialized)
{
...
{
    std::unique_ptr<Widget> other_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, IsVisibleOnAllWorkspaces)
{
...
{
    ui::GestureEvent event = CreateTestGestureEvent(ui::EventType::kGestureScrollBegin, 5, 5);
...
{
    gfx::Point p(200, 200);
    ui::MouseEvent event(ui::EventType::kMouseMoved, p, p, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetActivationChanged)
{
    widget()->OnNativeWidgetActivationChanged(false);
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetAddedToCompositor)
{
    widget()->OnNativeWidgetAddedToCompositor();
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetBeginUserBoundsChange)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetDestroyed)
{
    widget()->OnNativeWidgetDestroyed();
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetDestroying)
{
    EXPECT_DCHECK_DEATH(widget()->OnNativeWidgetDestroying());
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetEndUserBoundsChange)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetPaint)
{
    ...
    widget()->OnNativeWidgetPaint(ui::PaintContext(display_list.get(), 1, gfx::Rect(), false));
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetParentChanged)
{
    widget()->OnNativeWidgetParentChanged(nullptr);
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetRemovingFromCompositor)
{
    widget()->OnNativeWidgetRemovingFromCompositor();
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetSizeChanged)
{
    widget()->OnNativeWidgetSizeChanged(gfx::Size());
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetVisibilityChanged)
{
    widget()->OnNativeWidgetVisibilityChanged(false);
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetWindowShowStateChanged)
{
    widget()->OnNativeWidgetWindowShowStateChanged();
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnNativeWidgetWorkspaceChanged)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnParentShouldPaintAsActiveChanged)
{
...
{
    ui::ScrollEvent scroll(ui::EventType::kScroll, gfx::Point(65, 5), ui::EventTimeForNow(), 0, 0, 20, 0, 20, 2);
    widget()->OnScrollEvent(&scroll);
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, OnSizeConstraintsChanged)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, LayoutRootViewIfNecessary)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, NotifyNativeViewHierarchyChanged)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, RegisterPaintAsActiveChangedCallback)
{
    auto subscription = widget()->RegisterPaintAsActiveChangedCallback(base::DoNothing());
}
...
{
    EXPECT_DCHECK_DEATH(Widget::ReparentNativeView(widget()->GetNativeView(), nullptr));
}
...
{
    widget()->RunMoveLoop(gfx::Vector2d(), Widget::MoveLoopSource::kMouse, Widget::MoveLoopEscapeBehavior::kHide);
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, RunShellDrag)
{
    std::unique_ptr<OSExchangeData> data(std::make_unique<OSExchangeData>());
    widget()->RunShellDrag(nullptr, std::move(data), gfx::Point(), 0, ui::mojom::DragEventSource::kMouse);
}
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, SetCanAppearInExistingFullscreenSpaces)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, SetFocusTraversableParent)
{
    std::unique_ptr<Widget> another_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
    widget()->SetFocusTraversableParent(another_widget->GetFocusTraversable());
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, SetFocusTraversableParentView)
{
    std::unique_ptr<Widget> another_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, SetNativeWindowProperty)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, SetVisibilityChangedAnimationsEnabled)
{
    widget()->SetVisibilityChangedAnimationsEnabled(false);
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, SetVisibilityAnimationDuration)
{
    widget()->SetVisibilityAnimationDuration(base::Seconds(1));
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, SetVisibilityAnimationTransition)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, SetVisibleOnAllWorkspaces)
{
    widget()->SetVisibleOnAllWorkspaces(true);
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, ShouldDescendIntoChildForEventHandling)
{
    widget()->ShouldDescendIntoChildForEventHandling(nullptr, gfx::NativeView(), nullptr, gfx::Point(0, 0));
}

TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, ShouldHandleNativeWidgetActivationChanged)
{
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, ShouldWindowContentsBeTransparent)
{
...
{
    std::unique_ptr<Widget> another_widget = CreateTestWidget(std::get<Widget::InitParams::Ownership>(GetParam()));
...
TEST_P(WidgetWithDestroyedNativeViewOrNativeWidgetTest, SynthesizeMouseMoveEvent)
{
...
{
    widget()->ViewHierarchyChanged(ViewHierarchyChangedDetails(true, nullptr, nullptr, nullptr));
}

INSTANTIATE_TEST_SUITE_P(PlatformWidgetWithDestroyedNativeViewOrNativeWidgetTest, WidgetWithDestroyedNativeViewOrNativeWidgetTest,
    ::testing::Combine(::testing::Values(ViewsTestBase::NativeWidgetType::kDefault, ViewsTestBase::NativeWidgetType::kDesktop),
        ::testing::Values(Widget::InitParams::WIDGET_OWNS_NATIVE_WIDGET, Widget::InitParams::CLIENT_OWNS_WIDGET)),
...
public:
    ...
    const Widget* widget_activated() const
    {
        return widget_activated_;
    }
    const std::string& widget_deactivated() const
    {
        return widget_deactivated_;
    }
    const std::string& widget_shown() const
    {
        return widget_shown_;
    }
    const std::string& widget_hidden() const
    {
        return widget_hidden_;
    }
...
public:
    WidgetActivationForwarder(Widget* current_active_widget, Widget* widget_to_activate)
    ...
    WidgetActivationForwarder& operator=(const WidgetActivationForwarder&) = delete;
...
public:
    explicit WidgetCloseCounter(Widget* widget)
...
TEST_F(WidgetObserverTest, DestroyBubble)
{
    ...
    ViewsDelegate::GetInstance()->set_native_widget_factory(ViewsDelegate::NativeWidgetFactory());

    std::unique_ptr<Widget> anchor = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    anchor->Show();

    auto bubble_delegate = std::make_unique<TestBubbleDialogDelegateView>(anchor->client_view());
    {
        std::unique_ptr<Widget> bubble_widget
            = base::WrapUnique(BubbleDialogDelegateView::CreateBubble(bubble_delegate.release(), Widget::InitParams::CLIENT_OWNS_WIDGET));
        bubble_widget->Show();
    }
...
TEST_F(WidgetObserverTest, WidgetBoundsChangedNative)
{
    ...
    Widget::InitParams params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
...
public:
    ...
    DesktopWidgetObserverTest& operator=(const DesktopWidgetObserverTest&) = delete;
...
TEST_F(DesktopWidgetTest, GetWindowPlacement)
{
    ...
    views::test::PropertyWaiter minimize_waiter(base::BindRepeating(&Widget::IsMinimized, base::Unretained(widget.get())), true);
    widget->Minimize();
    ...
    views::test::PropertyWaiter restore_waiter(base::BindRepeating(&Widget::IsMinimized, base::Unretained(widget.get())), false);
    widget->Restore();
...
TEST_F(DesktopWidgetTest, MinimumSizeConstraints)
{
    ...
    delegate.InitWidget(CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW));
    Widget* widget = delegate.GetWidget();
    ...
    EXPECT_EQ(delegate.initial_bounds().size(), widget->GetWindowBoundsInScreen().size());
    ...
    if (!widget->ShouldUseNativeFrame()) {
        ...
        minimum_size = widget->non_client_view()->GetWindowBoundsForClientBounds(gfx::Rect(minimum_size)).size();
    }
...
TEST_F(DesktopWidgetTest, GetPossiblyDestroyedParent)
{
    ...
    const auto create_widget = [](Widget* parent, bool is_desktop) {
        ...
        if (is_desktop) {
            init_params.native_widget = new test::TestPlatformNativeWidget<DesktopNativeWidgetAura>(widget, false, nullptr);
        } else {
            init_params.native_widget = new test::TestPlatformNativeWidget<NativeWidgetAura>(widget, false, nullptr);
        }
...
TEST_F(WidgetTest, GetWindowBoundsInScreen)
{
...

    {
        ...
        EXPECT_NE(kTestSize.ToString(), widget->GetWindowBoundsInScreen().size().ToString());
        widget->SetSize(kTestSize);
        EXPECT_EQ(kTestSize.ToString(), widget->GetWindowBoundsInScreen().size().ToString());

        EXPECT_NE(kTestBounds.ToString(), widget->GetWindowBoundsInScreen().ToString());
        widget->SetBounds(kTestBounds);
        EXPECT_EQ(kTestBounds.ToString(), widget->GetWindowBoundsInScreen().ToString());

        // Changing just the size should not change the origin.
        widget->SetSize(kTestSize);
        EXPECT_EQ(kTestBounds.origin().ToString(), widget->GetWindowBoundsInScreen().origin().ToString());
    }
    ...
    EXPECT_EQ(kTestBounds.ToString(), widget->GetClientAreaBoundsInScreen().ToString());
...
TEST_F(WidgetTest, KeyboardInputEvent)
{
    ...
    ui::KeyEvent backspace_p(ui::EventType::kKeyPressed, ui::VKEY_DELETE, ui::EF_NONE);
    toplevel->OnKeyEvent(&backspace_p);
    EXPECT_TRUE(backspace_p.stopped_propagation());
    ui::KeyEvent backspace_r(ui::EventType::kKeyReleased, ui::VKEY_DELETE, ui::EF_NONE);
...
{
    // Create a widget.
    std::unique_ptr<Widget> widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    NonClientView* non_client_view = widget->non_client_view();
    non_client_view->SetFrameView(std::make_unique<MinimumSizeFrameView>(widget.get()));
...
public:
    ...
    DesktopAuraTestValidPaintWidget(const DesktopAuraTestValidPaintWidget&) = delete;
    DesktopAuraTestValidPaintWidget& operator=(const DesktopAuraTestValidPaintWidget&) = delete;
...
public:
    std::unique_ptr<DesktopAuraTestValidPaintWidget> CreateDesktopAuraTestValidPaintWidget(
        Widget::InitParams::Type type = Widget::InitParams::TYPE_WINDOW_FRAMELESS)
    {
        auto widget = std::make_unique<DesktopAuraTestValidPaintWidget>(CreateParamsForTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, type));

        View* contents_view = widget->SetContentsView(std::make_unique<ContentsView>());
...
TEST_F(WidgetTest, WheelEventsFromScrollEventTarget)
{
    ...
    ui::ScrollEvent scroll2(ui::EventType::kScroll, gfx::Point(5, 5), ui::EventTimeForNow(), 0, 0, 20, 0, 20, 2);
...
TEST_F(WidgetTest, GestureScrollEventDispatching)
{
    ...
    {
        ui::GestureEvent begin = CreateTestGestureEvent(ui::EventType::kGestureScrollBegin, 5, 5);
        widget->OnGestureEvent(&begin);
        ui::GestureEvent update = CreateTestGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollUpdate, 20, 10), 25, 15);
        widget->OnGestureEvent(&update);
        ui::GestureEvent end = CreateTestGestureEvent(ui::EventType::kGestureScrollEnd, 25, 15);
        widget->OnGestureEvent(&end);

        EXPECT_EQ(1, noscroll_view->GetEventCount(ui::EventType::kGestureScrollBegin));
        EXPECT_EQ(0, noscroll_view->GetEventCount(ui::EventType::kGestureScrollUpdate));
        EXPECT_EQ(0, noscroll_view->GetEventCount(ui::EventType::kGestureScrollEnd));
    }

    {
        ui::GestureEvent begin = CreateTestGestureEvent(ui::EventType::kGestureScrollBegin, 65, 5);
        widget->OnGestureEvent(&begin);
        ui::GestureEvent update = CreateTestGestureEvent(ui::GestureEventDetails(ui::EventType::kGestureScrollUpdate, 20, 10), 85, 15);
        widget->OnGestureEvent(&update);
        ui::GestureEvent end = CreateTestGestureEvent(ui::EventType::kGestureScrollEnd, 85, 15);
        widget->OnGestureEvent(&end);

        EXPECT_EQ(1, scroll_view->GetEventCount(ui::EventType::kGestureScrollBegin));
        EXPECT_EQ(1, scroll_view->GetEventCount(ui::EventType::kGestureScrollUpdate));
...
TEST_F(WidgetTest, EventHandlersOnRootView)
{
    ...
    EventCountView* view = root_view->AddChildView(std::make_unique<EventCountView>());
    ...
    ui::ScrollEvent scroll(ui::EventType::kScroll, gfx::Point(5, 5), ui::EventTimeForNow(), 0, 0, 20, 0, 20, 2);
    widget->OnScrollEvent(&scroll);
    ...
    // the root view.
    ui::ScrollEvent fling(ui::EventType::kScrollFlingStart, gfx::Point(5, 5), ui::EventTimeForNow(), 0, 0, 20, 0, 20, 2);
    ...
    ui::GestureEvent tap_down = CreateTestGestureEvent(ui::EventType::kGestureTapDown, 5, 5);
    ...
    ui::GestureEvent tap_cancel = CreateTestGestureEvent(ui::EventType::kGestureTapCancel, 5, 5);
    ...
    ui::ScrollEvent consumed_scroll(ui::EventType::kScroll, gfx::Point(5, 5), ui::EventTimeForNow(), 0, 0, 20, 0, 20, 2);
...
TEST_F(WidgetTest, SynthesizeMouseMoveEvent)
{
    ...
    EventCountView* v1 = root_view->AddChildView(std::make_unique<EventCountView>());
    v1->SetBounds(5, 5, 10, 10);
    EventCountView* v2 = root_view->AddChildView(std::make_unique<EventCountView>());
    ...
    auto generator = CreateEventGenerator(GetContext(), widget->GetNativeWindow());
...
TEST_F(WidgetTest, MouseEventDispatchWhileTouchIsDown)
{
    ...
    EventCountView* event_count_view = widget->GetRootView()->AddChildView(std::make_unique<EventCountView>());
...
TEST_F(WidgetTest, MousePressCausesCapture)
{
    ...
    EXPECT_EQ(gfx::NativeView(), internal::NativeWidgetPrivate::GetGlobalCapture(widget->GetNativeView()));
    ...
    EXPECT_EQ(1, event_count_view->GetEventCount(ui::EventType::kMousePressed));
    EXPECT_EQ(widget->GetNativeView(), internal::NativeWidgetPrivate::GetGlobalCapture(widget->GetNativeView()));
...
private:
    ...
    void OnMouseEvent(ui::MouseEvent* event) override
    {
        if (event->type() == ui::EventType::kMousePressed) {
            ...
            widget_->SetCapture(widget_->GetRootView()->AddChildView(std::move(event_count_view)));
        }
    }
...
TEST_F(WidgetTest, CaptureDuringMousePressNotOverridden)
{
    ...
    event_count_view->AddPostTargetHandler(&consumer);
    auto generator = CreateEventGenerator(GetRootWindow(widget), widget->GetNativeWindow());
    generator->MoveMouseTo(widget->GetClientAreaBoundsInScreen().CenterPoint());
    ...
    EXPECT_EQ(1, event_count_view->GetEventCount(ui::EventType::kMousePressed));
    EXPECT_NE(widget->GetNativeView(), internal::NativeWidgetPrivate::GetGlobalCapture(widget->GetNativeView()));
    EXPECT_EQ(widget2->GetNativeView(), internal::NativeWidgetPrivate::GetGlobalCapture(widget->GetNativeView()));
...
public:
    explicit ClosingEventObserver(Widget* widget)
        : widget_(widget)
    {
    }
...
public:
    explicit ClosingView(Widget* widget)
        : widget_(widget)
    {
    }
...
TEST_F(WidgetTest, DestroyedWithCaptureViaEventMonitor)
{
    ...
    ClosingView* closing_view = widget->GetContentsView()->AddChildView(std::make_unique<ClosingView>(widget));
    widget->SetCapture(closing_view);

    ClosingEventObserver closing_event_observer(widget);
    auto monitor = EventMonitor::CreateApplicationMonitor(&closing_event_observer, widget->GetNativeWindow(), { ui::EventType::kMousePressed });
...
public:
    explicit PaintAsActiveCallbackCounter(Widget* widget)
    {
        ...
        paint_as_active_subscription_
            = widget->RegisterPaintAsActiveChangedCallback(base::BindRepeating(&PaintAsActiveCallbackCounter::Call, base::Unretained(this)));
    }
    void Call()
    {
        count_++;
...
TEST_F(WidgetTest, LockParentPaintAsActive)
{
    ...
    WidgetAutoclosePtr grandchild(CreateChildPlatformWidget(child->GetNativeView()));
...
TEST_F(DesktopWidgetTest, ClosingActiveChildDoesNotPrematurelyPaintParentInactive)
{
    // top_level_widget that owns the bubble widget.
    auto top_level_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
    top_level_widget->Show();

    // Create the child bubble widget.
    auto bubble_widget = std::make_unique<Widget>();
    Widget::InitParams init_params = CreateParamsForTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_BUBBLE);
...
TEST_F(DesktopWidgetTest, LockPaintAsActiveAndCloseParent)
{
    ...
    std::unique_ptr<Widget> parent = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
    parent->Show();
    ...
    delegate->InitWidget(std::move(params));
    delegate->RegisterDeleteDelegateCallback(base::DoNothingWithBoundArgs(delegate->GetWidget()->LockPaintAsActive()));
...
TEST_F(DesktopWidgetTest, WidgetDestroyedItselfDoesNotCrash)
{
    ...
    delegate.InitWidget(CreateParamsForTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS));
...
TEST_F(DesktopWidgetTest, CloseRequested_DisallowClose)
{
    ...
    EXPECT_EQ(Widget::ClosedReason::kUnspecified, delegate.GetWidget()->closed_reason());
    EXPECT_EQ(kReason, delegate.last_closed_reason());
...
protected:
    ...
    {
        auto widget = std::make_unique<Widget>();
        Widget::InitParams init_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);

        if (!desktop_native_widget) {
            init_params.native_widget = CreatePlatformNativeWidgetImpl(widget.get(), kStubCapture, nullptr);
        }
        widget->Init(std::move(init_params));
...
TEST_F(WidgetTest, WidgetDeleted_InOnMousePressed)
{
    ...
    widget->SetContentsView(std::make_unique<CloseWidgetView>(ui::EventType::kMousePressed));
...
TEST_F(WidgetTest, WidgetDeleted_InDispatchGestureEvent)
{
    ...
    widget->SetContentsView(std::make_unique<CloseWidgetView>(ui::EventType::kGestureTapDown));
...
public:
    ...
    GetNativeThemeFromDestructorView(const GetNativeThemeFromDestructorView&) = delete;
    GetNativeThemeFromDestructorView& operator=(const GetNativeThemeFromDestructorView&) = delete;

    ~GetNativeThemeFromDestructorView() override
    {
        VerifyNativeTheme();
    }
...
private:
    ...
    {
        ASSERT_TRUE(GetNativeTheme() != nullptr);
    }
};
...
bool RunGetNativeThemeFromDestructor(Widget::InitParams params, bool is_first_run)
{
    ...
    if (!is_first_run) {
        params.native_widget = CreatePlatformNativeWidgetImpl(widget.get(), kStubCapture, nullptr);
...
{
    if (RunGetNativeThemeFromDestructor(CreateParams(Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP), true)) {
        RunGetNativeThemeFromDestructor(CreateParams(Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP), false);
    }
}
...
TEST_F(DesktopWidgetTest, CloseDestroys)
{
    ...
    CloseDestroysWidget* widget = new CloseDestroysWidget(&destroyed, run_loop.QuitClosure());
    Widget::InitParams params = CreateParams(Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET, Widget::InitParams::TYPE_MENU);
...
TEST_F(WidgetTest, CloseWidgetWhileAnimating)
{
    ...
    gfx::Rect bounds(100, 100, 50, 50);
    {
        ...
        ui::ScopedAnimationDurationScaleMode animation_scale_mode(ui::ScopedAnimationDurationScaleMode::NON_ZERO_DURATION);
        ui::ScopedLayerAnimationSettings animation_settings(widget->GetLayer()->GetAnimator());
...
{
    CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP)->OnSizeConstraintsChanged();
}
...
#if BUILDFLAG(IS_WIN)
#define MAYBE_DisableTestRootViewHandlersWhenHidden DISABLED_TestRootViewHandlersWhenHidden
#else
...
TEST_F(WidgetTest, MAYBE_DisableTestRootViewHandlersWhenHidden)
{
    ...
    ui::MouseEvent press(
        ui::EventType::kMousePressed, click_location, click_location, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    ...
    ui::MouseEvent move(ui::EventType::kMouseMoved, move_location, move_location, ui::EventTimeForNow(), 0, 0);
    ...
    ui::GestureEvent tap_down = CreateTestGestureEvent(ui::EventType::kGestureTapDown, 15, 15);
    widget->OnGestureEvent(&tap_down);
...
TEST_F(WidgetTest, GestureEndEvents)
{
    ...
    EXPECT_EQ(nullptr, GetGestureHandler(root_view));
    ui::GestureEvent end = CreateTestGestureEvent(ui::EventType::kGestureEnd, 15, 15);
    widget->OnGestureEvent(&end);
    EXPECT_EQ(nullptr, GetGestureHandler(root_view));
    ...
    view->set_handle_mode(EventCountView::CONSUME_EVENTS);
    ui::GestureEvent tap = CreateTestGestureEvent(ui::EventType::kGestureTap, 15, 15);
    ...
    ui::GestureEvent end_second_touch_point = CreateTestGestureEvent(details, 15, 15);
...
TEST_F(WidgetTest, GestureEventsNotProcessed)
{
    ...
    ui::GestureEvent begin = CreateTestGestureEvent(ui::EventType::kGestureBegin, 5, 5);
    widget->OnGestureEvent(&begin);
    ...
    ui::GestureEvent end = CreateTestGestureEvent(ui::EventType::kGestureEnd, 5, 5);
    widget->OnGestureEvent(&end);
    ...
    ui::GestureEventDetails details(ui::EventType::kGestureEnd);
    details.set_touch_points(2);
    ui::GestureEvent end_second_touch_point = CreateTestGestureEvent(details, 5, 5);
    widget->OnGestureEvent(&end_second_touch_point);
    EXPECT_EQ(0, v1->GetEventCount(ui::EventType::kGestureEnd));
    EXPECT_EQ(0, v2->GetEventCount(ui::EventType::kGestureEnd));
    EXPECT_EQ(0, v3->GetEventCount(ui::EventType::kGestureEnd));
    ...
    ui::GestureEvent scroll_update = CreateTestGestureEvent(ui::EventType::kGestureScrollUpdate, 5, 5);
    ...
    // when there is no default gesture handler set, but they should be marked as
    // handled by OnEventProcessingStarted().
    ui::GestureEvent scroll_end = CreateTestGestureEvent(ui::EventType::kGestureScrollEnd, 5, 5);
    ...
    // when there is no default gesture handler set, but they should be marked as
    // handled by OnEventProcessingStarted().
    ui::GestureEvent scroll_fling_start = CreateTestGestureEvent(ui::EventType::kScrollFlingStart, 5, 5);
...
TEST_F(WidgetTest, GestureEventDispatch)
{
    ...
    ui::GestureEvent tap = CreateTestGestureEvent(ui::EventType::kGestureTap, 5, 5);
    EXPECT_EQ(nullptr, GetGestureHandler(root_view));
    widget->OnGestureEvent(&tap);
    ...
    widget->OnGestureEvent(&tap);
    EXPECT_TRUE(tap.handled());
    ui::GestureEvent show_press = CreateTestGestureEvent(ui::EventType::kGestureShowPress, 5, 5);
...
TEST_F(WidgetTest, ScrollGestureEventDispatch)
{
    ...
    ui::GestureEvent scroll_begin = CreateTestGestureEvent(ui::EventType::kGestureScrollBegin, 5, 5);
    ...
    ui::GestureEvent pinch_begin = CreateTestGestureEvent(ui::EventType::kGesturePinchBegin, 5, 5);
...
TEST_F(WidgetTest, GestureEventLocationWhileBubbling)
{
    ...
    ui::GestureEvent tap = CreateTestGestureEvent(ui::EventType::kGestureTap, location_in_root.x(), location_in_root.y());
...
public:
    DestroyedTrackingView(const std::string& name, std::vector<std::string>* add_to)
        : name_(name)
        , add_to_(add_to)
    {
    }
    ...
    ~DestroyedTrackingView() override
    {
...
public:
    ...
    WidgetChildDestructionTest& operator=(const WidgetChildDestructionTest&) = delete;
    ...
    void RunDestroyChildWidgetsTest(bool top_level_has_desktop_native_widget_aura, bool child_has_desktop_native_widget_aura)
    {
        ...
        if (!top_level_has_desktop_native_widget_aura) {
            params.native_widget = CreatePlatformNativeWidgetImpl(top_level, kStubCapture, nullptr);
        }
        top_level->Init(std::move(params));
        top_level->GetRootView()->AddChildView(new DestroyedTrackingView("parent", &destroyed));
        top_level->Show();
        ...
        if (!child_has_desktop_native_widget_aura) {
            child_params.native_widget = CreatePlatformNativeWidgetImpl(child, kStubCapture, nullptr);
        }
        child->Init(std::move(child_params));
        child->GetRootView()->AddChildView(new DestroyedTrackingView("child", &destroyed));
        child->Show();
...
TEST_F(WidgetChildDestructionTest, DestroyChildWidgetsInOrderWithDesktopNativeWidget)
{
...
// DesktopNativeWidgetAura.
TEST_F(WidgetChildDestructionTest, DestroyChildWidgetsInOrderWithDesktopNativeWidgetForBoth)
{
...
{
    std::unique_ptr<Widget> top_level_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
    top_level_widget->SetFullscreen(true);
    EXPECT_EQ(top_level_widget->IsVisible(), IsNativeWindowVisible(top_level_widget->GetNativeWindow()));
}
...
public:
    ...
    bool fullscreen_layout_called() const
    {
        return fullscreen_layout_called_;
    }
...
public:
    ...
    IsActiveFromDestroyObserver& operator=(const IsActiveFromDestroyObserver&) = delete;
...
public:
    Widget::InitParams CreateParams(Widget::InitParams::Ownership ownership, Widget::InitParams::Type type) override
    {
        Widget::InitParams params = DesktopWidgetTest::CreateParams(ownership, type);
        if (context_) {
...
TEST_F(ChildDesktopWidgetTest, IsActiveFromDestroy)
{
    ...
    std::unique_ptr<Widget> parent_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
    parent_widget->Show();

    std::unique_ptr<Widget> child_widget = CreateChildWidget(parent_widget->GetNativeWindow());
...
TEST_F(WidgetTest, MouseEventTypesViaGenerator)
{
    ...
    const gfx::Point out_of_bounds_point = view->GetBoundsInScreen().bottom_right() + gfx::Vector2d(10, 10);
...
private:
    bool Subclass()
    {
        old_proc_ = reinterpret_cast<WNDPROC>(::SetWindowLongPtr(window_, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(WndProc)));
        return old_proc_ != nullptr;
    }

    void Unsubclass()
    {
        ::SetWindowLongPtr(window_, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(old_proc_));
    }

    static LRESULT CALLBACK WndProc(HWND window, unsigned int message, WPARAM w_param, LPARAM l_param)
    {
        ...
        LRESULT ret = ::CallWindowProc(instance_->old_proc_, window, message, w_param, l_param);
...
TEST_F(DesktopWidgetTest, DISABLED_SysCommandMoveOnNCLButtonDownOnCaptionAndMoveTest)
{
    std::unique_ptr<Widget> widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    widget->Show();
...
public:
    ...
    void OnDeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor) override
    {
        ...
        View::OnDeviceScaleFactorChanged(old_device_scale_factor, new_device_scale_factor);
    }

    float last_scale_factor() const
    {
        return last_scale_factor_;
    }
...
TEST_F(WidgetTest, MouseWheelEvent)
{
    ...
    EventCountView* event_count_view = widget->client_view()->AddChildView(std::make_unique<EventCountView>());
    event_count_view->SetBounds(0, 0, 600, 600);
    widget->Show();

    auto event_generator = CreateEventGenerator(GetContext(), widget->GetNativeWindow());
...
public:
    ...
    TestSaveWindowPlacementWidgetDelegate(const TestSaveWindowPlacementWidgetDelegate&) = delete;
    TestSaveWindowPlacementWidgetDelegate operator=(const TestSaveWindowPlacementWidgetDelegate&) = delete;
    ...
    // ViewsDelegate:
    std::string GetWindowName() const final
    {
    ...
    void SaveWindowPlacement(const gfx::Rect& bounds, ui::mojom::WindowShowState show_state) override
    {
...
TEST_F(WidgetTest, ClientViewAccessibilityProperties)
{
    ...
    widget->client_view()->GetViewAccessibility().GetAccessibleNodeData(&node_data);
...
TEST_F(WidgetTest, NonClientViewAccessibilityProperties)
{
    ...
    non_client_view->frame_view()->GetViewAccessibility().GetAccessibleNodeData(&node_data);
    EXPECT_EQ(node_data.role, ax::mojom::Role::kClient);
}

class WidgetChildObserver : public WidgetObserver {
public:
    explicit WidgetChildObserver(Widget* widget)
    {
...
TEST_F(WidgetTest, ChildWidgetNotifiesObserverWhenInitializedAndDestroyed)
{
    ...
    std::unique_ptr<Widget> child_widget = base::WrapUnique(CreateChildPlatformWidget(widget->GetNativeView(), Widget::InitParams::CLIENT_OWNS_WIDGET));
...
TEST_F(WidgetTest, ChildWidgetNotifiesObserverWhenReparented)
{
    ...
    std::unique_ptr<Widget> widget_1 = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    WidgetChildObserver observer_1(widget_1.get());

    std::unique_ptr<Widget> widget_2 = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    WidgetChildObserver observer_2(widget_2.get());

    // Create a child widget of `widget_1`.
    std::unique_ptr<Widget> child_widget = base::WrapUnique(CreateChildPlatformWidget(widget_1->GetNativeView(), Widget::InitParams::CLIENT_OWNS_WIDGET));
    EXPECT_EQ(observer_1.child_widget(), child_widget.get());

    Widget::ReparentNativeView(child_widget->GetNativeView(), widget_2->GetNativeView());
...
class WidgetSetAspectRatioTest : public ViewsTestBase, public testing::WithParamInterface<gfx::Size /* margin */> {
public:
    ...
    void SetUp() override
    {
        ...
        ON_CALL(*native_widget(), CreateNonClientFrameView).WillByDefault([this]() { return std::make_unique<NonClientFrameViewWithFixedMargin>(margin()); });
    ...
    MockNativeWidget* native_widget()
    {
...
private:
    ...
    public:
        ...
        {
            gfx::Rect r = bounds();
            return gfx::Rect(r.x(), r.y(), r.width() - margin_.width(), r.height() - margin_.height());
        }
...
INSTANTIATE_TEST_SUITE_P(WidgetSetAspectRatioTestInstantiation, WidgetSetAspectRatioTest, ::testing::Values(gfx::Size(15, 20), gfx::Size(0, 0)));
...
public:
    ...
    Widget::InitParams CreateParams(Widget::InitParams::Ownership ownership, Widget::InitParams::Type type) override
    {
        Widget::InitParams params = WidgetTest::CreateParams(ownership, override_type_.value_or(type));
...
private:
    ...
    void InitControllers()
    {
        focus_controller_ = std::make_unique<wm::FocusController>(new TestFocusRules);
        shadow_controller_ = std::make_unique<wm::ShadowController>(focus_controller_.get(), nullptr);
    }
...
TEST_F(WidgetShadowTest, MAYBE_ShadowsInRootWindow)
{
    ...
    EXPECT_EQ(top_level_window_should_have_shadow, WidgetHasInProcessShadow(top_level));
    top_level->Show();
    EXPECT_EQ(top_level_window_should_have_shadow, WidgetHasInProcessShadow(top_level));
...
TEST_F(DesktopWidgetTest, WindowModalOwnerDestroyedEnabledTest)
{
    ...
    const auto create_params = [this](Widget* widget, gfx::NativeView parent) {
        Widget::InitParams init_params = CreateParamsForTestWidget(Widget::InitParams::TYPE_WINDOW);
    ...
    owner_dialog_widget.Init(create_params(&owner_dialog_widget, top_level_widget->GetNativeView()));
    ...
    owned_dialog_widget.Init(create_params(&owned_dialog_widget, owner_dialog_widget.GetNativeView()));
...
TEST_F(DesktopWidgetTest, StackAboveTest)
{
    ...
    Widget* grandchild_one = CreateChildNativeWidgetWithParent(child_one->AsWidget());
    Widget* grandchild_two = CreateChildNativeWidgetWithParent(child_two->AsWidget());
...
public:
    CompositingWidgetTest()
        : widget_types_ { Widget::InitParams::TYPE_WINDOW, Widget::InitParams::TYPE_WINDOW_FRAMELESS, Widget::InitParams::TYPE_CONTROL,
            Widget::InitParams::TYPE_POPUP, Widget::InitParams::TYPE_MENU, Widget::InitParams::TYPE_TOOLTIP, Widget::InitParams::TYPE_BUBBLE,
    ...
    void CheckAllWidgetsForOpacity(const Widget::InitParams::WindowOpacity opacity)
    {
        opacity_ = opacity;
        for (const auto& widget_type : widget_types_) {
...
#endif
            std::unique_ptr<Widget> widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET, widget_type);

            // Use NativeWidgetAura directly.
            if (widget_type == Widget::InitParams::TYPE_WINDOW_FRAMELESS || widget_type == Widget::InitParams::TYPE_CONTROL) {
                continue;
            }
            ...
            bool should_be_transparent = opacity_ == Widget::InitParams::WindowOpacity::kTranslucent;
#else
            bool should_be_transparent = widget->ShouldWindowContentsBeTransparent();
#endif

            EXPECT_EQ(IsNativeWindowTransparent(widget->GetNativeWindow()), should_be_transparent);
        }
    }

protected:
    const std::vector<Widget::InitParams::Type> widget_types_;
    Widget::InitParams::WindowOpacity opacity_ = Widget::InitParams::WindowOpacity::kInferred;
};
...
public:
    ...
    void SetUp() override
    {
        ...
        Widget::InitParams params = CreateParams(Widget::InitParams::TYPE_WINDOW);
        native_widget_ = std::make_unique<MockNativeWidget>(widget());
        ON_CALL(*native_widget(), SetAllowScreenshots(_)).WillByDefault([this](bool allowed) {
