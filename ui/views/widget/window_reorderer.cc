...
void GetViewsWithAssociatedWindow(const aura::Window& parent_window, std::map<views::View*, aura::Window*>* hosted_windows)
{
...
// |parent_layer|.
void GetOrderOfViewsWithLayers(views::View* view, ui::Layer* parent_layer, const std::map<views::View*, aura::Window*>& hosts, std::vector<views::View*>* order)
{
    DCHECK(view);
    SCOPED_CRASH_KEY_STRING64("GetOrderOfViewsWithLayers", "view_name", view->GetObjectName());
...
WindowReorderer::AssociationObserver::AssociationObserver(WindowReorderer* reorderer)
...
void WindowReorderer::AssociationObserver::StartObserving(aura::Window* window)
{
...
void WindowReorderer::AssociationObserver::OnWindowPropertyChanged(aura::Window* window, const void* key, intptr_t old)
{
    if (key == kHostViewKey)
        reorderer_->ReorderChildWindows();
}

void WindowReorderer::AssociationObserver::OnWindowDestroying(aura::Window* window)
{
...
{
    if (!parent_window_observation_.IsObserving() || !view_observation_.IsObserving()) {
        return;
    }
    ...
    GetOrderOfViewsWithLayers(root_view, parent_window.layer(), hosted_windows, &view_with_layer_order);
