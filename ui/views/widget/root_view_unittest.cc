...
public:
    explicit RootViewTestState(ViewsTestBase* delegate, RootViewTestStateInit init = {})
    {
        Widget::InitParams init_params = delegate->CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, init.type);
...
TEST_F(RootViewTest, DeleteViewDuringKeyEventDispatch)
{
    ...
    View* child = state.AddChildView(std::make_unique<DeleteOnKeyEventView>(&got_key_event));
    ...
    ui::KeyEvent key_event(ui::EventType::kKeyPressed, ui::VKEY_ESCAPE, ui::EF_NONE);
...
public:
    ...
    TestContextMenuController& operator=(const TestContextMenuController&) = delete;

    ~TestContextMenuController() override = default;

    int show_context_menu_calls() const
    {
        return show_context_menu_calls_;
    }
    View* menu_source_view() const
    {
        return menu_source_view_;
    }
    ...
    void ShowContextMenuForViewImpl(View* source, const gfx::Point& point, ui::mojom::MenuSourceType source_type) override
    {
...
private:
    ...
    ui::mojom::MenuSourceType menu_source_type_ = ui::mojom::MenuSourceType::kNone;
};
...
TEST_F(RootViewTest, ContextMenuFromKeyEvent)
{
    ...
    ui::KeyEvent nomenu_key_event = ui::KeyEvent::FromCharacter('a', ui::VKEY_A, ui::DomCode::NONE, ui::EF_NONE);
    ...
    ui::KeyEvent menu_key_event(ui::EventType::kKeyPressed, ui::VKEY_F10, ui::EF_SHIFT_DOWN);
    ...
    EXPECT_EQ(ui::mojom::MenuSourceType::kKeyboard, controller.menu_source_type());
    controller.Reset();

    // A context menu should be shown for a keypress of VKEY_APPS.
    ui::KeyEvent menu_key_event2(ui::EventType::kKeyPressed, ui::VKEY_APPS, ui::EF_NONE);
...
TEST_F(RootViewTest, GestureHandlerResetAfterMouseReleased)
{
    ...
    View* gesture_handler = state.AddChildView(std::make_unique<GestureHandlingView>());
    gesture_handler->SetBoundsRect(gfx::Rect(gfx::Size { 50, 50 }));

    // Create a child view to handle mouse events.
    View* mouse_handler = state.AddChildView(std::make_unique<MouseHandlingView>());
    mouse_handler->SetBoundsRect(gfx::Rect(gesture_handler->bounds().bottom_right(), gfx::Size { 50, 50 }));

    // Emulate to start gesture scroll on `child_view`.
    const gfx::Point gesture_handler_center_point = gesture_handler->GetBoundsInScreen().CenterPoint();
    ui::GestureEvent scroll_begin(gesture_handler_center_point.x(), gesture_handler_center_point.y(), ui::EF_NONE, base::TimeTicks(),
        ui::GestureEventDetails(ui::EventType::kGestureScrollBegin));
    root_view->OnEventFromSource(&scroll_begin);
    ui::GestureEvent scroll_update(gesture_handler_center_point.x(), gesture_handler_center_point.y(), ui::EF_NONE, base::TimeTicks(),
    ...
    const gfx::Point mouse_handler_center_point = mouse_handler->GetBoundsInScreen().CenterPoint();
    ui::MouseEvent pressed_event(ui::EventType::kMousePressed, mouse_handler_center_point, mouse_handler_center_point, ui::EventTimeForNow(), ui::EF_NONE,
        /*changed_button_flags=*/0);
    ui::MouseEvent released_event(ui::EventType::kMouseReleased, mouse_handler_center_point, mouse_handler_center_point, ui::EventTimeForNow(), ui::EF_NONE,
        /*changed_button_flags=*/0);
...
{
    RootViewTestState state(this, { .bounds = { 100, 100 }, .type = Widget::InitParams::TYPE_POPUP });
    internal::RootView* root_view = state.GetRootView();
    ...
    ui::GestureEvent long_press1(5, 5, 0, base::TimeTicks(), ui::GestureEventDetails(ui::EventType::kGestureLongPress));
    ui::EventDispatchDetails details = root_view->OnEventFromSource(&long_press1);

    ui::GestureEvent end1(5, 5, 0, base::TimeTicks(), ui::GestureEventDetails(ui::EventType::kGestureEnd));
    ...
    // |other_child_view|.
    ui::GestureEvent long_press2(25, 5, 0, base::TimeTicks(), ui::GestureEventDetails(ui::EventType::kGestureLongPress));
    details = root_view->OnEventFromSource(&long_press2);

    ui::GestureEvent end2(25, 5, 0, base::TimeTicks(), ui::GestureEventDetails(ui::EventType::kGestureEnd));
    ...
    // |parent_view| should show a context menu as a result of a long press on
    // itself.
    ui::GestureEvent long_press3(50, 50, 0, base::TimeTicks(), ui::GestureEventDetails(ui::EventType::kGestureLongPress));
    details = root_view->OnEventFromSource(&long_press3);

    ui::GestureEvent end3(25, 5, 0, base::TimeTicks(), ui::GestureEventDetails(ui::EventType::kGestureEnd));
...
public:
    NestedEventOnEvent(ui::EventType nested_event_type, View* root_view)
        : nested_event_type_(nested_event_type)
    ...
    void OnEvent(ui::Event* event) override
    {
        if (event->type() == nested_event_type_) {
            ui::MouseEvent exit_event(ui::EventType::kMouseExited, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
...
{
    RootViewTestState state(this, { .bounds = { 10, 10, 500, 500 }, .type = Widget::InitParams::TYPE_POPUP });
    internal::RootView* root_view = state.GetRootView();
    bool view_destroyed = false;

    View* child = state.AddChildView(std::make_unique<DeleteViewOnEvent>(ui::EventType::kMouseExited, &view_destroyed));
    ...
    ui::MouseEvent moved_event(ui::EventType::kMouseMoved, gfx::Point(15, 15), gfx::Point(15, 15), ui::EventTimeForNow(), 0, 0);
...
TEST_F(RootViewTest, DeleteViewOnMouseEnterDispatch)
{
    ...
    View* child = state.AddChildView(std::make_unique<DeleteViewOnEvent>(ui::EventType::kMouseEntered, &view_destroyed));
    ...
    ui::MouseEvent moved_event2(ui::EventType::kMouseMoved, gfx::Point(115, 115), gfx::Point(115, 115), ui::EventTimeForNow(), 0, 0);
...
TEST_F(RootViewTest, ClearMouseMoveHandlerOnMouseExitDispatch)
{
    ...
    View* child = state.AddChildView(std::make_unique<NestedEventOnEvent>(ui::EventType::kMouseExited, root_view));
    // Make |child| smaller than the containing Widget and RootView.
    child->SetBounds(100, 100, 100, 100);

    // Generate a mouse move event which ensures that |mouse_moved_handler_|
    // is set to the child view in the RootView class.
    ui::MouseEvent moved_event(ui::EventType::kMouseMoved, gfx::Point(110, 110), gfx::Point(110, 110), ui::EventTimeForNow(), 0, 0);
    root_view->OnMouseMoved(moved_event);
    ...
    ui::MouseEvent move_event2(ui::EventType::kMouseMoved, gfx::Point(15, 15), gfx::Point(15, 15), ui::EventTimeForNow(), 0, 0);
...
TEST_F(RootViewTest, ClearMouseMoveHandlerOnMouseExitDispatchWithContentViewDisabled)
{
    RootViewTestState state(this, { .bounds = { 10, 10, 500, 500 }, .type = Widget::InitParams::TYPE_POPUP });
    internal::RootView* root_view = state.GetRootView();
    ...
    // RootView::OnMouseMoved.
    ui::MouseEvent move_event2(ui::EventType::kMouseMoved, gfx::Point(200, 200), gfx::Point(200, 200), ui::EventTimeForNow(), 0, 0);
    root_view->OnMouseMoved(move_event2);
}
...
TEST_F(RootViewTest, ClearMouseMoveHandlerOnMouseEnterDispatch)
{
    ...
    View* child = state.AddChildView(std::make_unique<NestedEventOnEvent>(ui::EventType::kMouseEntered, root_view));
...
TEST_F(RootViewTest, DeleteWidgetOnMouseExitDispatch)
{
    ...
    auto* child = content->AddChildView(
        std::make_unique<DeleteWidgetOnMouseExit>(base::BindOnce([](std::unique_ptr<Widget>* widget) { widget->reset(); }, base::Unretained(&widget))));
    ...
    ui::MouseEvent moved_event(ui::EventType::kMouseMoved, gfx::Point(115, 115), gfx::Point(115, 115), ui::EventTimeForNow(), 0, 0);
    root_view->OnMouseMoved(moved_event);
...
public:
    ...
    RootViewTestDialogDelegate& operator=(const RootViewTestDialogDelegate&) = delete;
...
TEST_F(RootViewTest, SingleLayoutDuringInit)
{
    ...
    Widget* widget = DialogDelegate::CreateDialogWidget(delegate, GetContext(), nullptr);
...
TEST_F(RootViewTest, AnnounceTextAsTest)
{
    ...
    root_view->AnnounceTextAs(kAlertText, ui::AXPlatformNode::AnnouncementType::kAlert);
    ...
    EXPECT_EQ(kAlertText, node_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
    ...
    root_view->AnnounceTextAs(kPoliteText, ui::AXPlatformNode::AnnouncementType::kPolite);
    ...
    EXPECT_EQ(kPoliteText, node_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
    hidden_polite_view->GetViewAccessibility().GetAccessibleNodeData(&node_data);
    EXPECT_TRUE(node_data.HasStringAttribute(ax::mojom::StringAttribute::kContainerLiveStatus));
    const std::string& val = node_data.GetStringAttribute(ax::mojom::StringAttribute::kContainerLiveStatus);
    ...
    EXPECT_TRUE(node_data.GetBoolAttribute(ax::mojom::BoolAttribute::kLiveAtomic));
    EXPECT_EQ("polite", node_data.GetStringAttribute(ax::mojom::StringAttribute::kLiveStatus));
    EXPECT_EQ("additions text", node_data.GetStringAttribute(ax::mojom::StringAttribute::kLiveRelevant));
    EXPECT_EQ("additions text", node_data.GetStringAttribute(ax::mojom::StringAttribute::kContainerLiveRelevant));
}
...
TEST_F(RootViewTest, MouseEventDispatchedToClosestEnabledView)
{
    ...
    ui::MouseEvent pressed_event(ui::EventType::kMousePressed, gfx::Point(5, 5), gfx::Point(5, 5), ui::EventTimeForNow(), 0, 0);
    ui::MouseEvent released_event(ui::EventType::kMouseReleased, gfx::Point(5, 5), gfx::Point(5, 5), ui::EventTimeForNow(), 0, 0);
    root_view->OnMousePressed(pressed_event);
    root_view->OnMouseReleased(released_event);
