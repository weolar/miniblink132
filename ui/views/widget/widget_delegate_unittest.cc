...
TEST_F(WidgetDelegateTest, ClientViewFactoryCanReplaceClientView)
{
    ...
    auto delegate = std::make_unique<WidgetDelegate>();
    delegate->SetClientViewFactory(base::BindLambdaForTesting([&tracker](Widget* widget) {
        auto view = std::make_unique<ClientView>(widget, nullptr);
        tracker.SetView(view.get());
        return view;
    }));

    auto client = base::WrapUnique<ClientView>(delegate->CreateClientView(nullptr));
...
TEST_F(WidgetDelegateTest, AppIconCanDifferFromWindowIcon)
{
    ...
    EXPECT_TRUE(delegate->GetWindowIcon().Rasterize(nullptr).BackedBySameObjectAs(window_icon));
    EXPECT_TRUE(delegate->GetWindowAppIcon().Rasterize(nullptr).BackedBySameObjectAs(app_icon));
}
...
TEST_F(WidgetDelegateTest, AppIconFallsBackToWindowIcon)
{
    ...
    EXPECT_TRUE(delegate->GetWindowAppIcon().Rasterize(nullptr).BackedBySameObjectAs(window_icon));
}
...
public:
    ...
    void SetAccessiblePanes(const std::vector<raw_ptr<View, VectorExperimental>>& panes)
    {
...
TEST_F(WidgetDelegateTest, RotatePaneFocusFromView)
{
    ...
    auto* pane1 = widget->GetContentsView()->AddChildView(std::make_unique<AccessiblePaneView>());
    ...
    auto* pane2 = widget->GetContentsView()->AddChildView(std::make_unique<AccessiblePaneView>());
    ...
    auto get_focused_view = [focus_manager] { return focus_manager->GetFocusedView(); };
    ...
    // Rotate to the next.
    EXPECT_TRUE(delegate.RotatePaneFocusFromView(get_focused_view(), true, false));
    EXPECT_EQ(v3, get_focused_view());

    // Attempting to rotate again at the end should result in a non-rotation.
    EXPECT_FALSE(delegate.RotatePaneFocusFromView(get_focused_view(), true, false));
    ...
    EXPECT_FALSE(delegate.RotatePaneFocusFromView(get_focused_view(), false, false));
    EXPECT_EQ(v1, get_focused_view());

    // Now wrap around and rotate to the end again.
    EXPECT_TRUE(delegate.RotatePaneFocusFromView(get_focused_view(), false, true));
    EXPECT_EQ(v3, get_focused_view());

    EXPECT_TRUE(delegate.RotatePaneFocusFromView(get_focused_view(), false, false));
    EXPECT_EQ(v1, get_focused_view());
    ...
    EXPECT_FALSE(delegate.RotatePaneFocusFromView(get_focused_view(), true, true));
...
#if defined(USE_AURA)
        EXPECT_EQ((widget->GetNativeWindow()->GetProperty(aura::client::kResizeBehaviorKey) & aura::client::kResizeBehaviorCanFullscreen) > 0, expected);
#endif
    };
...
#if defined(USE_AURA)
        EXPECT_EQ((widget->GetNativeWindow()->GetProperty(aura::client::kResizeBehaviorKey) & aura::client::kResizeBehaviorCanResize) > 0, expected);
#endif
    };
...
#if defined(USE_AURA)
        EXPECT_EQ((widget->GetNativeWindow()->GetProperty(aura::client::kResizeBehaviorKey) & aura::client::kResizeBehaviorCanMaximize) > 0, expected);
#endif
    };
...
#if defined(USE_AURA)
        EXPECT_EQ((widget->GetNativeWindow()->GetProperty(aura::client::kResizeBehaviorKey) & aura::client::kResizeBehaviorCanMinimize) > 0, expected);
#endif
    };
