...
public:
    MouseEnterExitEvent(const ui::MouseEvent& event, ui::EventType type)
        : ui::MouseEvent(event, static_cast<View*>(nullptr), static_cast<View*>(nullptr))
    {
        DCHECK(type == ui::EventType::kMouseEntered || type == ui::EventType::kMouseExited);
        SetType(type);
    }
...
public:
    ...
    void AnnounceTextAs(const std::u16string& text, ui::AXPlatformNode::AnnouncementType announcement_type)
    {
    ...
    ax::mojom::Event AnnounceEventType(ui::AXPlatformNode::AnnouncementType announcement_type) const
    {
        CHECK(HasValidRole());
    ...
    void UpdateAccessibleAttributes(const std::u16string& announce_text, std::optional<ui::AXPlatformNode::AnnouncementType> announcement_type = std::nullopt)
    {
        ...
        ScopedAccessibilityEventBlocker scoped_event_blocker(GetViewAccessibility());
        ...
        if (announce_text.empty()) {
            GetViewAccessibility().SetName(announce_text, ax::mojom::NameFrom::kAttributeExplicitlyEmpty);
        } else {
            GetViewAccessibility().SetName(announce_text);
        }

        if (base::FeatureList::IsEnabled(features::kAnnounceTextAdditionalAttributes)) {
    ...
    {
        return GetViewAccessibility().GetCachedRole() == ax::mojom::Role::kStaticText || GetViewAccessibility().GetCachedRole() == ax::mojom::Role::kAlert
            || GetViewAccessibility().GetCachedRole() == ax::mojom::Role::kStatus;
    }

    bool IsValidEvent(ax::mojom::Event announce_event_type) const
    {
        return announce_event_type == ax::mojom::Event::kAlert || announce_event_type == ax::mojom::Event::kLiveRegionChanged;
    }
};
...
// macOS doesn't have keyboard-triggered context menus.
        ...
        if (v && v->GetEnabled() && ((event->key_code() == ui::VKEY_APPS) || (event->key_code() == ui::VKEY_F10 && event->IsShiftDown()))) {
...
private:
    ...
    void OnGestureEvent(ui::GestureEvent* event) override
    {
        ...
        if (touch_dnd_enabled_ && event->type() == ui::EventType::kGestureLongPress
            && (!target->drag_controller() || target->drag_controller()->CanStartDragForView(target, location, location))) {
            if (target->DoDrag(*event, location, ui::mojom::DragEventSource::kTouch)) {
                event->StopPropagation();
                return;
            }
        }
        ...
        if (target->context_menu_controller()
            && (event->type() == ui::EventType::kGestureLongPress || event->type() == ui::EventType::kGestureLongTap
            ...
            target->ShowContextMenu(screen_location, ui::mojom::MenuSourceType::kTouch);
            event->StopPropagation();
        }
    }
...
RootView::RootView(Widget* widget)
    : widget_(widget)
    , pre_dispatch_handler_(std::make_unique<internal::PreEventDispatchHandler>(this))
    , post_dispatch_handler_(std::make_unique<internal::PostEventDispatchHandler>())
{
    AddPostTargetHandler(post_dispatch_handler_.get());
    SetEventTargeter(std::unique_ptr<ViewTargeter>(new RootViewTargeter(this, this)));

    auto* widget_delegate = widget->widget_delegate();
    if (widget_delegate) {
        if (ax::mojom::Role::kUnknown == GetViewAccessibility().GetCachedRole()) {
            GetViewAccessibility().SetRole(widget_delegate->GetAccessibleWindowRole());
        }
    }
}
...
{
    DCHECK(contents_view && GetWidget()->native_widget()) << "Can't be called because the widget is not initialized or is "
                                                             "destroyed";
...
void RootView::DeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor)
{
    View::PropagateDeviceScaleFactorChanged(old_device_scale_factor, new_device_scale_factor);
}
...
void RootView::AnnounceTextAs(const std::u16string& text, ui::AXPlatformNode::AnnouncementType announcement_type)
{
    if (text.empty()) {
        return;
    }
...
void RootView::OnEventProcessingStarted(ui::Event* event)
{
    ...
    // already been set.
    if (gesture_event->type() == ui::EventType::kGestureEnd && (gesture_event->details().touch_points() > 1 || !gesture_handler_)) {
        event->SetHandled();
        return;
    }
    ...
    if (!gesture_handler_
        && (gesture_event->type() == ui::EventType::kGestureScrollUpdate || gesture_event->type() == ui::EventType::kGestureScrollEnd
...
bool RootView::OnMousePressed(const ui::MouseEvent& event)
{
    ...
    if (mouse_pressed_handler_) {
        ui::MouseEvent mouse_pressed_event(event, static_cast<View*>(this), mouse_pressed_handler_.get());
        drag_info_.Reset();
    ...
    // the mouse event.
    for (mouse_pressed_handler_ = GetEventHandlerForPoint(event.location()); mouse_pressed_handler_ && (mouse_pressed_handler_ != this);
         mouse_pressed_handler_ = mouse_pressed_handler_->parent()) {
        DVLOG(1) << "OnMousePressed testing " << mouse_pressed_handler_->GetClassName();
        ...
        if (mouse_pressed_event.handled()) {
            ...
            DVLOG(1) << "OnMousePressed handled by " << mouse_pressed_handler_->GetClassName();
            return true;
        }
    }
...
bool RootView::OnMouseDragged(const ui::MouseEvent& event)
{
    if (mouse_pressed_handler_) {
        ...
        ui::MouseEvent mouse_event(event, static_cast<View*>(this), mouse_pressed_handler_.get());
...
void RootView::OnMouseReleased(const ui::MouseEvent& event)
{
    ...
    if (mouse_pressed_handler_) {
        ui::MouseEvent mouse_released(event, static_cast<View*>(this), mouse_pressed_handler_.get());
...
void RootView::OnMouseCaptureLost()
{
    if (mouse_pressed_handler_ || gesture_handler_) {
        ...
        if (mouse_pressed_handler_) {
            ...
            ui::MouseEvent release_event(ui::EventType::kMouseReleased, last_point, last_point, ui::EventTimeForNow(), last_mouse_event_flags_, 0);
...
void RootView::OnMouseExited(const ui::MouseEvent& event)
{
    if (mouse_move_handler_ != nullptr) {
        ...
        if (!dispatch_details.target_destroyed && mouse_move_handler_) {
            dispatch_details = NotifyEnterExitOfDescendant(event, ui::EventType::kMouseExited, mouse_move_handler_, nullptr);
            if (dispatch_details.dispatcher_destroyed)
                return;
        }
        mouse_move_handler_ = nullptr;
...
{
    for (View* v = GetEventHandlerForPoint(event.location()); v && v != this && !event.handled(); v = v->parent()) {
...
{
#if defined(USE_AURA)
    ui::GestureRecognizer* gesture_recognizer = (gesture_handler_ && widget_ ? widget_->GetGestureRecognizer() : nullptr);
...
void RootView::GetAccessibleNodeData(ui::AXNodeData* node_data)
{
    ...
    if (node_data->GetStringAttribute(ax::mojom::StringAttribute::kName).empty()
        && static_cast<ax::mojom::NameFrom>(node_data->GetIntAttribute(ax::mojom::IntAttribute::kNameFrom)) != ax::mojom::NameFrom::kAttributeExplicitlyEmpty) {
...
void RootView::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details)
{
...
View::LayerOffsetData RootView::CalculateOffsetToAncestorWithLayer(ui::Layer** layer_parent)
{
...
void RootView::HandleMouseEnteredOrMoved(const ui::MouseEvent& event)
{
    ...
    if (mouse_move_handler_ && !mouse_move_handler_->GetEnabled() && v->Contains(mouse_move_handler_)) {
        v = mouse_move_handler_;
    }

    if (v && v != this) {
        if (v != mouse_move_handler_) {
            if (mouse_move_handler_ != nullptr && (!mouse_move_handler_->GetNotifyEnterExitOnChild() || !mouse_move_handler_->Contains(v))) {
                MouseEnterExitEvent exit(event, ui::EventType::kMouseExited);
                exit.ConvertLocationToTarget(static_cast<View*>(this), mouse_move_handler_.get());
                ...
                if (!dispatch_details.target_destroyed) {
                    ...
                    dispatch_details = NotifyEnterExitOfDescendant(event, ui::EventType::kMouseExited, mouse_move_handler_, v);
                    if (dispatch_details.dispatcher_destroyed) {
                        return;
                    }
                }
            ...
            if (!mouse_move_handler_->GetNotifyEnterExitOnChild() || !mouse_move_handler_->Contains(old_handler)) {
                MouseEnterExitEvent entered(event, ui::EventType::kMouseEntered);
                entered.ConvertLocationToTarget(static_cast<View*>(this), mouse_move_handler_.get());
                ...
                dispatch_details = NotifyEnterExitOfDescendant(event, ui::EventType::kMouseEntered, mouse_move_handler_, old_handler);
                if (dispatch_details.dispatcher_destroyed || dispatch_details.target_destroyed) {
                    return;
                }
            }
        ...
        if (event.type() == ui::EventType::kMouseMoved) {
            ui::MouseEvent moved_event(event, static_cast<View*>(this), mouse_move_handler_.get());
...
ui::EventDispatchDetails RootView::NotifyEnterExitOfDescendant(const ui::MouseEvent& event, ui::EventType type, View* view, View* sibling)
{
...
ui::EventDispatchDetails RootView::PreDispatchEvent(ui::EventTarget* target, ui::Event* event)
{
...
ui::EventDispatchDetails RootView::PostDispatchEvent(ui::EventTarget* target, const ui::Event& event)
{
