...
TEST_F(NativeWidgetAuraTest, NonActiveWindowRequestImeFocus)
{
    ...
    std::unique_ptr<wm::FocusController> focus_controller = std::make_unique<wm::FocusController>(test_focus_rules);
    wm::SetActivationClient(GetContext(), focus_controller.get());

    auto widget1 = std::make_unique<Widget>();
    Widget::InitParams params1(Widget::InitParams::Ownership::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    params1.context = GetContext();
    params1.native_widget = CreatePlatformNativeWidgetImpl(widget1.get(), kDefault, nullptr);
    widget1->Init(std::move(params1));
    auto* textfield1 = widget1->GetRootView()->AddChildView(std::make_unique<Textfield>());
    ...
    params2.native_widget = CreatePlatformNativeWidgetImpl(widget2.get(), kDefault, nullptr);
    widget2->Init(std::move(params2));
    auto* textfield2a = widget2->GetRootView()->AddChildView(std::make_unique<Textfield>());
    auto* textfield2b = widget2->GetRootView()->AddChildView(std::make_unique<Textfield>());
