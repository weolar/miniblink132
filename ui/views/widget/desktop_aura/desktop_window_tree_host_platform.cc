...
DEFINE_UI_CLASS_PROPERTY_KEY(DesktopWindowTreeHostPlatform*, kHostForRootWindow, nullptr)
...
{
    if (show_state != ui::mojom::WindowShowState::kDefault && show_state != ui::mojom::WindowShowState::kNormal
        && show_state != ui::mojom::WindowShowState::kInactive && show_state != ui::mojom::WindowShowState::kMaximized) {
...
ui::PlatformWindowOpacity GetPlatformWindowOpacity(Widget::InitParams::WindowOpacity opacity)
{
    switch (opacity) {
...
ui::PlatformWindowType GetPlatformWindowType(Widget::InitParams::Type window_type)
{
    switch (window_type) {
...
ui::PlatformWindowShadowType GetPlatformWindowShadowType(Widget::InitParams::ShadowType shadow_type)
{
    switch (shadow_type) {
...
ui::PlatformWindowInitProperties ConvertWidgetInitParamsToInitProperties(const Widget::InitParams& params)
{
    ui::PlatformWindowInitProperties properties;
    properties.type = GetPlatformWindowType(params.type);
    properties.accept_events = params.accept_events;
    properties.activatable = params.activatable == Widget::InitParams::Activatable::kYes;
    ...
    properties.is_security_surface = properties.z_order == ui::ZOrderLevel::kSecuritySurface;
    ...
    if (ui::OzonePlatform::GetInstance()->GetPlatformProperties().set_parent_for_non_top_level_windows) {
...
SkPath GetWindowMask(const Widget* widget)
{
    ...
    const_cast<NonClientView*>(widget->non_client_view())->GetWindowMask(widget->GetWindowBoundsInScreen().size(), &window_mask);
    return window_mask;
}
...
DesktopWindowTreeHostPlatform::DesktopWindowTreeHostPlatform(
    internal::NativeWidgetDelegate* native_widget_delegate, DesktopNativeWidgetAura* desktop_native_widget_aura)
...
// static
aura::Window* DesktopWindowTreeHostPlatform::GetContentWindowForWidget(gfx::AcceleratedWidget widget)
{
...
// static
DesktopWindowTreeHostPlatform* DesktopWindowTreeHostPlatform::GetHostForWidget(gfx::AcceleratedWidget widget)
{
...
std::vector<aura::Window*> DesktopWindowTreeHostPlatform::GetAllOpenWindows()
{
    ...
    base::ranges::transform(open_windows(), windows.begin(), DesktopWindowTreeHostPlatform::GetContentWindowForWidget);
    return windows;
}

// static
void DesktopWindowTreeHostPlatform::CleanUpWindowList(void (*func)(aura::Window* window))
{
    if (!open_windows_)
        return;
...
void DesktopWindowTreeHostPlatform::Init(const Widget::InitParams& params)
{
    ...
    ui::PlatformWindowInitProperties properties = ConvertWidgetInitParamsToInitProperties(params);
    ...
    if (properties.parent_widget) {
        window_parent_ = DesktopWindowTreeHostPlatform::GetHostForWidget(properties.parent_widget);
    ...
    CreateCompositor(params.force_software_compositing || params.type == Widget::InitParams::TYPE_TOOLTIP);
...
void DesktopWindowTreeHostPlatform::OnNativeWidgetCreated(const Widget::InitParams& params)
{
    ...
    platform_window()->SetUseNativeFrame(params.type == Widget::InitParams::TYPE_WINDOW && !params.remove_standard_frame);
...
void DesktopWindowTreeHostPlatform::OnWidgetInitDone()
{
    ...
    GetContentWindow()->SetFillsBoundsCompletely(GetWindowMaskForClipping().isEmpty());
}
...
void DesktopWindowTreeHostPlatform::OnActiveWindowChanged(bool active)
{
    ...
    if (base::FeatureList::IsEnabled(features::kOzoneBubblesUsePlatformWidgets) && active && !is_active_ && !window_children_.empty()) {
        Activate();
    }
#endif
}

std::unique_ptr<corewm::Tooltip> DesktopWindowTreeHostPlatform::CreateTooltip()
{
    return std::make_unique<corewm::TooltipAura>();
}

std::unique_ptr<aura::client::DragDropClient> DesktopWindowTreeHostPlatform::CreateDragDropClient()
{
...
void DesktopWindowTreeHostPlatform::Close()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&DesktopWindowTreeHostPlatform::CloseNow, close_widget_factory_.GetWeakPtr()));
}
...
void DesktopWindowTreeHostPlatform::CloseNow()
{
    ...
    // remove themselves.
    std::set<raw_ptr<DesktopWindowTreeHostPlatform, SetExperimental>> window_children_copy = window_children_;
...
void DesktopWindowTreeHostPlatform::Show(ui::mojom::WindowShowState show_state, const gfx::Rect& restore_bounds)
{
    ...
    if (native_widget_delegate_->CanActivate()) {
        if (show_state != ui::mojom::WindowShowState::kInactive && show_state != ui::mojom::WindowShowState::kMinimized) {
            Activate();
        }
        ...
        // example.
        native_widget_delegate_->SetInitialFocus(IsActive() ? show_state : ui::mojom::WindowShowState::kInactive);
    }
...
void DesktopWindowTreeHostPlatform::CenterWindow(const gfx::Size& size)
{
    ...
    if (wm::GetTransientParent(GetContentWindow())) {
        gfx::Rect transient_parent_rect = wm::GetTransientParent(GetContentWindow())->GetBoundsInScreen();
...
void DesktopWindowTreeHostPlatform::GetWindowPlacement(gfx::Rect* bounds, ui::mojom::WindowShowState* show_state) const
{
...
std::string DesktopWindowTreeHostPlatform::GetWorkspace() const
{
    ...
    return workspace_extension ? workspace_extension->GetWorkspace() : std::string();
}
...
void DesktopWindowTreeHostPlatform::SetShape(std::unique_ptr<Widget::ShapeRects> native_shape)
{
...
{
    return platform_window()->GetPlatformWindowState() == ui::PlatformWindowState::kMaximized;
}

bool DesktopWindowTreeHostPlatform::IsMinimized() const
{
    return platform_window()->GetPlatformWindowState() == ui::PlatformWindowState::kMinimized;
}
...
void DesktopWindowTreeHostPlatform::SetVisibleOnAllWorkspaces(bool always_visible)
{
    auto* workspace_extension = ui::GetWorkspaceExtension(*platform_window());
...
{
    auto* workspace_extension = ui::GetWorkspaceExtension(*platform_window());
    return workspace_extension ? workspace_extension->IsVisibleOnAllWorkspaces() : false;
}

bool DesktopWindowTreeHostPlatform::SetWindowTitle(const std::u16string& title)
{
...
void DesktopWindowTreeHostPlatform::ClearNativeFocus()
{
    ...
    if (GetContentWindow() && aura::client::GetFocusClient(GetContentWindow())
        && GetContentWindow()->Contains(aura::client::GetFocusClient(GetContentWindow())->GetFocusedWindow())) {
        aura::client::GetFocusClient(GetContentWindow())->FocusWindow(GetContentWindow());
    }
}
...
void DesktopWindowTreeHostPlatform::SetVisibilityChangedAnimationsEnabled(bool value)
{
    platform_window()->SetVisibilityChangedAnimationsEnabled(value);
    if (desktop_native_widget_aura_->widget_type() != Widget::InitParams::TYPE_WINDOW) {
        GetContentWindow()->SetProperty(aura::client::kAnimationsDisabledKey, !value);
    }
}

std::unique_ptr<NonClientFrameView> DesktopWindowTreeHostPlatform::CreateNonClientFrameView()
{
    return ShouldUseNativeFrame() ? std::make_unique<NativeFrameView>(GetWidget()) : nullptr;
}
...
{
    return platform_window()->ShouldWindowContentsBeTransparent() || !(GetWindowMaskForClipping().isEmpty());
}
...
void DesktopWindowTreeHostPlatform::FrameTypeChanged()
{
    ...
    platform_window()->SetUseNativeFrame(new_type == Widget::FrameType::kForceNative);
...
void DesktopWindowTreeHostPlatform::SetFullscreen(bool fullscreen, int64_t target_display_id)
{
...
{
    return ui::IsPlatformWindowStateFullscreen(platform_window()->GetPlatformWindowState());
}
...
void DesktopWindowTreeHostPlatform::SetAspectRatio(const gfx::SizeF& aspect_ratio, const gfx::Size& excluded_margin)
{
...
void DesktopWindowTreeHostPlatform::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
    platform_window()->SetWindowIcons(window_icon, app_icon);
}

void DesktopWindowTreeHostPlatform::InitModalType(ui::mojom::ModalType modal_type)
{
    NOTIMPLEMENTED_LOG_ONCE();
}
...
bool DesktopWindowTreeHostPlatform::ShouldUseDesktopNativeCursorManager() const
{
    return true;
}
...
void DesktopWindowTreeHostPlatform::UpdateWindowShapeIfNeeded(const ui::PaintContext& context)
{
...
gfx::Transform DesktopWindowTreeHostPlatform::GetRootTransform() const
{
    ...
    const float scale = root_window ? screen->GetPreferredScaleFactorForWindow(const_cast<aura::Window*>(root_window)).value_or(1.0f)
...
void DesktopWindowTreeHostPlatform::OnCompositorVisibilityChanging(ui::Compositor* compositor, bool visible)
{
...
void DesktopWindowTreeHostPlatform::OnCompositorVisibilityChanged(ui::Compositor* compositor, bool visible)
{
...
gfx::Insets DesktopWindowTreeHostPlatform::CalculateInsetsInDIP(ui::PlatformWindowState window_state) const
{
...
void DesktopWindowTreeHostPlatform::OnWindowStateChanged(ui::PlatformWindowState old_state, ui::PlatformWindowState new_state)
{
    ...
    // window is minimized.
    if (!aura::NativeWindowOcclusionTracker::IsNativeWindowOcclusionTrackingAlwaysEnabled(this) && is_minimized != was_minimized) {
        if (is_minimized) {
            SetVisible(false);
        } else {
            SetVisible(true);
...
void DesktopWindowTreeHostPlatform::OnAcceleratedWidgetAvailable(gfx::AcceleratedWidget widget)
{
...
bool DesktopWindowTreeHostPlatform::OnRotateFocus(ui::PlatformWindowDelegate::RotateDirection direction, bool reset)
{
    return DesktopWindowTreeHostPlatform::RotateFocusForWidget(*GetWidget(), direction, reset);
}
...
std::optional<gfx::Size> DesktopWindowTreeHostPlatform::GetMinimumSizeForWindow() const
{
    return native_widget_delegate_->GetMinimumSize();
}

std::optional<gfx::Size> DesktopWindowTreeHostPlatform::GetMaximumSizeForWindow() const
{
...
SkPath DesktopWindowTreeHostPlatform::GetWindowMaskForWindowShapeInPixels()
{
    ...
    if (!window_mask.isEmpty()) {
        window_mask.transform(gfx::TransformToFlattenedSkMatrix(GetRootTransform()));
    }
    return window_mask;
}

std::optional<ui::OwnedWindowAnchor> DesktopWindowTreeHostPlatform::GetOwnedWindowAnchorAndRectInDIP()
{
    const auto* anchor = GetContentWindow()->GetProperty(aura::client::kOwnedWindowAnchor);
    return anchor ? std::make_optional(*anchor) : std::nullopt;
}

gfx::Rect DesktopWindowTreeHostPlatform::ConvertRectToPixels(const gfx::Rect& rect_in_dip) const
{
    return ToPixelRect(rect_in_dip);
}

gfx::Rect DesktopWindowTreeHostPlatform::ConvertRectToDIP(const gfx::Rect& rect_in_pixels) const
{
    return ToDIPRect(rect_in_pixels);
}

gfx::PointF DesktopWindowTreeHostPlatform::ConvertScreenPointToLocalDIP(const gfx::Point& screen_in_pixels) const
{
...
gfx::Insets DesktopWindowTreeHostPlatform::ConvertInsetsToPixels(const gfx::Insets& insets_dip) const
{
...
gfx::Rect DesktopWindowTreeHostPlatform::ToDIPRect(const gfx::Rect& rect_in_pixels) const
{
    return GetRootTransform().InverseMapRect(rect_in_pixels).value_or(rect_in_pixels);
}

gfx::Rect DesktopWindowTreeHostPlatform::ToPixelRect(const gfx::Rect& rect_in_dip) const
{
    gfx::RectF rect_in_pixels_f = GetRootTransform().MapRect(gfx::RectF(rect_in_dip));
...
void DesktopWindowTreeHostPlatform::AddAdditionalInitProperties(const Widget::InitParams& params, ui::PlatformWindowInitProperties* properties)
{
}
...
display::Display DesktopWindowTreeHostPlatform::GetDisplayNearestRootWindow() const
{
    DCHECK(window());
    DCHECK(window()->IsRootWindow());
    // TODO(sky): GetDisplayNearestWindow() should take a const aura::Window*.
    return display::Screen::GetScreen()->GetDisplayNearestWindow(const_cast<aura::Window*>(window()));
}

bool DesktopWindowTreeHostPlatform::RotateFocusForWidget(Widget& widget, ui::PlatformWindowDelegate::RotateDirection direction, bool reset)
{
    if (reset) {
        widget.GetFocusManager()->ClearFocus();
    }
    auto focus_manager_direction
        = direction == RotateDirection::kForward ? views::FocusManager::Direction::kForward : views::FocusManager::Direction::kBackward;
    auto wrapping = reset ? views::FocusManager::FocusCycleWrapping::kEnabled : views::FocusManager::FocusCycleWrapping::kDisabled;
    return widget.GetFocusManager()->RotatePaneFocus(focus_manager_direction, wrapping);
}
...
// static
std::list<gfx::AcceleratedWidget>& DesktopWindowTreeHostPlatform::open_windows()
{
    if (!open_windows_)
