...
TEST_F(DesktopNativeWidgetAuraTest, DesktopAuraWindowSizeTest)
{
    ...
    Widget::InitParams init_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP);
#else
    ...
    EXPECT_EQ(bounds.ToString(), widget.GetNativeView()->GetRootWindow()->bounds().ToString());
    EXPECT_EQ(bounds.ToString(), widget.GetNativeView()->bounds().ToString());
    EXPECT_EQ(bounds.ToString(), widget.GetNativeView()->parent()->bounds().ToString());

    gfx::Rect new_bounds(0, 0, 200, 200);
    widget.SetBounds(new_bounds);
    EXPECT_EQ(new_bounds.ToString(), widget.GetNativeView()->GetRootWindow()->bounds().ToString());
    EXPECT_EQ(new_bounds.ToString(), widget.GetNativeView()->bounds().ToString());
    EXPECT_EQ(new_bounds.ToString(), widget.GetNativeView()->parent()->bounds().ToString());
}
...
TEST_F(DesktopNativeWidgetAuraTest, NativeViewNoActivate)
{
    ...
    EXPECT_EQ(nullptr, aura::client::GetFocusClient(widget.GetNativeWindow())->GetFocusedWindow());
}
...
TEST_F(DesktopNativeWidgetAuraTest, WidgetNotVisibleOnlyWindowTreeHostShown)
{
    ...
    Widget::InitParams init_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    widget.Init(std::move(init_params));
    ShowWindow(widget.GetNativeView()->GetHost()->GetAcceleratedWidget(), SW_SHOWNORMAL);
...
TEST_F(DesktopNativeWidgetAuraTest, MAYBE_GlobalCursorState)
{
    ...
    Widget widget_a;
    Widget::InitParams init_params_a = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    widget_a.Init(std::move(init_params_a));

    Widget widget_b;
    Widget::InitParams init_params_b = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    widget_b.Init(std::move(init_params_b));

    aura::client::CursorClient* cursor_client_a = aura::client::GetCursorClient(widget_a.GetNativeView()->GetHost()->window());
    aura::client::CursorClient* cursor_client_b = aura::client::GetCursorClient(widget_b.GetNativeView()->GetHost()->window());
    ...
    EXPECT_EQ(ui::mojom::CursorType::kPointer, cursor_client_a->GetCursor().type());
    EXPECT_EQ(ui::mojom::CursorType::kPointer, cursor_client_b->GetCursor().type());
...
std::unique_ptr<Widget> CreateAndShowControlWidget(aura::Window* parent)
{
    ...
    Widget::InitParams params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_CONTROL);
...
#if BUILDFLAG(IS_CHROMEOS)
// TODO(crbug.com/40607034): investigate fixing and enabling on Chrome OS.
#define MAYBE_ReorderDoesntRecomputeOcclusion DISABLED_ReorderDoesntRecomputeOcclusion
#else
...
TEST_F(DesktopNativeWidgetAuraTest, MAYBE_ReorderDoesntRecomputeOcclusion)
{
    ...
    aura::test::WindowOcclusionTrackerTestApi window_occlusion_tracker_test_api(aura::Env::GetInstance()->GetWindowOcclusionTracker());
    const int num_times_occlusion_recomputed = window_occlusion_tracker_test_api.GetNumTimesOcclusionRecomputed();
    contents_view->ReorderChildView(host_view3, 0);
    EXPECT_EQ(num_times_occlusion_recomputed + 1, window_occlusion_tracker_test_api.GetNumTimesOcclusionRecomputed());
}

void QuitNestedLoopAndCloseWidget(std::unique_ptr<Widget> widget, base::OnceClosure quit_runloop)
{
...
TEST_F(DesktopNativeWidgetAuraTest, WidgetCanBeDestroyedFromNestedLoop)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&QuitNestedLoopAndCloseWidget, std::move(widget), run_loop.QuitClosure()));
    run_loop.Run();
}
...
public:
    ...
    TestDesktopNativeWidgetAura& operator=(const TestDesktopNativeWidgetAura&) = delete;

    ~TestDesktopNativeWidgetAura() override = default;

    void CloseNow() override
    {
        DesktopNativeWidgetAura::CloseNow();
    }
};

// Series of tests that verifies a null NativeWidgetDelegate does not cause
// a crash.
class DesktopNativeWidgetAuraWithNoDelegateTest : public DesktopNativeWidgetAuraTest {
public:
    DesktopNativeWidgetAuraWithNoDelegateTest() = default;

    DesktopNativeWidgetAuraWithNoDelegateTest(const DesktopNativeWidgetAuraWithNoDelegateTest&) = delete;
    DesktopNativeWidgetAuraWithNoDelegateTest& operator=(const DesktopNativeWidgetAuraWithNoDelegateTest&) = delete;
...
{
    SkPath mask;
    static_cast<aura::WindowDelegate*>(desktop_native_widget_)->GetHitTestMask(&mask);
}
...
{
    static_cast<aura::WindowDelegate*>(desktop_native_widget_)->GetNonClientComponent(gfx::Point());
}

TEST_F(DesktopNativeWidgetAuraWithNoDelegateTest, GetWidgetTest)
{
    static_cast<internal::NativeWidgetPrivate*>(desktop_native_widget_)->GetWidget();
}
...
{
    ui::GestureEvent gesture(0, 0, 0, ui::EventTimeForNow(), ui::GestureEventDetails(ui::EventType::kGestureTapDown));
    static_cast<ui::EventHandler*>(desktop_native_widget_)->OnGestureEvent(&gesture);
}

TEST_F(DesktopNativeWidgetAuraWithNoDelegateTest, OnHostMovedInPixelsTest)
{
    static_cast<aura::WindowTreeHostObserver*>(desktop_native_widget_)->OnHostMovedInPixels(nullptr);
}

TEST_F(DesktopNativeWidgetAuraWithNoDelegateTest, OnHostResizedTest)
{
    static_cast<aura::WindowTreeHostObserver*>(desktop_native_widget_)->OnHostResized(nullptr);
}

TEST_F(DesktopNativeWidgetAuraWithNoDelegateTest, OnHostWorkspaceChangedTest)
{
    static_cast<aura::WindowTreeHostObserver*>(desktop_native_widget_)->OnHostWorkspaceChanged(nullptr);
}
...
{
    ui::MouseEvent move(ui::EventType::kMouseMoved, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
...
{
    static_cast<aura::WindowDelegate*>(desktop_native_widget_)->OnPaint(ui::PaintContext(nullptr, 0, gfx::Rect(), false));
}

TEST_F(DesktopNativeWidgetAuraWithNoDelegateTest, OnScrollEventTest)
{
    ui::ScrollEvent scroll(ui::EventType::kScroll, gfx::Point(), ui::EventTimeForNow(), 0, 0, 0, 0, 0, 0);
    static_cast<ui::EventHandler*>(desktop_native_widget_)->OnScrollEvent(&scroll);
}
...
TEST_F(DesktopNativeWidgetAuraWithNoDelegateTest, OnWindowActivatedTest)
{
    static_cast<wm::ActivationChangeObserver*>(desktop_native_widget_)
        ->OnWindowActivated(wm::ActivationChangeObserver::ActivationReason::ACTIVATION_CLIENT,
            static_cast<internal::NativeWidgetPrivate*>(desktop_native_widget_)->GetNativeView(), nullptr);
}

TEST_F(DesktopNativeWidgetAuraWithNoDelegateTest, OnWindowFocusedTest)
{
    static_cast<aura::client::FocusChangeObserver*>(desktop_native_widget_)->OnWindowFocused(nullptr, nullptr);
}

TEST_F(DesktopNativeWidgetAuraWithNoDelegateTest, ShouldActivateTest)
{
    static_cast<wm::ActivationDelegate*>(desktop_native_widget_)->ShouldActivate();
}

TEST_F(DesktopNativeWidgetAuraWithNoDelegateTest, ShouldDescendIntoChildForEventHandlingTest)
{
    static_cast<aura::WindowDelegate*>(desktop_native_widget_)->ShouldDescendIntoChildForEventHandling(nullptr, gfx::Point());
}

TEST_F(DesktopNativeWidgetAuraWithNoDelegateTest, UpdateVisualStateTest)
{
    static_cast<aura::WindowDelegate*>(desktop_native_widget_)->UpdateVisualState();
}
...
public:
    ...
    DesktopAuraTopLevelWindowTest& operator=(const DesktopAuraTopLevelWindowTest&) = delete;
    ...
    {
        Widget::InitParams init_params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
        ...
        if (fullscreen) {
            owned_window_->SetProperty(aura::client::kShowStateKey, ui::mojom::WindowShowState::kFullscreen);
        } else {
            owned_window_->SetType(aura::client::WINDOW_TYPE_MENU);
        }
        owned_window_->Init(ui::LAYER_TEXTURED);
        aura::client::ParentWindowWithContext(owned_window_, widget_.GetNativeView()->GetRootWindow(), gfx::Rect(0, 0, 1900, 1600), display::kInvalidDisplayId);
        owned_window_->Show();
...
private:
    ...
    base::ScopedMultiSourceObservation<aura::Window, aura::WindowObserver> window_observations_ { this };
...
TEST_F(DesktopNativeWidgetAuraTest, FullscreenWindowDestroyedBeforeOwnerTest)
{
    ...
    ASSERT_NO_FATAL_FAILURE(fullscreen_window.CreateTopLevelWindow(gfx::Rect(0, 0, 200, 200), true));
...
TEST_F(DesktopNativeWidgetAuraTest, TopLevelOwnedPopupTest)
{
    ...
    ASSERT_NO_FATAL_FAILURE(popup_window.CreateTopLevelWindow(gfx::Rect(0, 0, 200, 200), false));
...
TEST_F(DesktopNativeWidgetAuraTest, TopLevelOwnedPopupResizeTest)
{
    ...
    EXPECT_EQ(popup_window.top_level_widget()->GetNativeView()->bounds().size(), new_size.size());
...
TEST_F(DesktopNativeWidgetAuraTest, TopLevelOwnedPopupRepositionTest)
{
    ...
    popup_window.owned_window()->SetBoundsInScreen(new_pos, display::Screen::GetScreen()->GetDisplayNearestPoint(gfx::Point()));

    EXPECT_EQ(new_pos, popup_window.top_level_widget()->GetWindowBoundsInScreen());
...
void GenerateMouseEvents(Widget* widget, ui::EventType last_event_type)
{
    ...
    ui::MouseEvent move_event(ui::EventType::kMouseMoved, screen_bounds.CenterPoint(), screen_bounds.CenterPoint(), ui::EventTimeForNow(), 0, 0);
    ui::EventSink* sink = WidgetTest::GetEventSink(widget);
    ui::EventDispatchDetails details = sink->OnEventFromSource(&move_event);
    if (last_event_type == ui::EventType::kMouseEntered || details.dispatcher_destroyed) {
        return;
    }
    details = sink->OnEventFromSource(&move_event);
    if (last_event_type == ui::EventType::kMouseMoved || details.dispatcher_destroyed) {
        return;
    }

    ui::MouseEvent press_event(ui::EventType::kMousePressed, screen_bounds.CenterPoint(), screen_bounds.CenterPoint(), ui::EventTimeForNow(), 0, 0);
    details = sink->OnEventFromSource(&press_event);
    if (last_event_type == ui::EventType::kMousePressed || details.dispatcher_destroyed) {
        return;
    }

    gfx::Point end_point(screen_bounds.CenterPoint());
    end_point.Offset(1, 1);
    ui::MouseEvent drag_event(ui::EventType::kMouseDragged, end_point, end_point, ui::EventTimeForNow(), 0, 0);
    details = sink->OnEventFromSource(&drag_event);
    if (last_event_type == ui::EventType::kMouseDragged || details.dispatcher_destroyed) {
        return;
    }

    ui::MouseEvent release_event(ui::EventType::kMouseReleased, end_point, end_point, ui::EventTimeForNow(), 0, 0);
...
void RunCloseWidgetDuringDispatchTest(WidgetTest* test, ui::EventType last_event_type)
{
    // |widget| is deleted by CloseWidgetView.
    auto widget = std::make_unique<Widget>();
    Widget::InitParams params = test->CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP);
...
TEST_F(DesktopNativeWidgetAuraTest, MAYBE_WindowMouseModalityTest)
{
    ...
    ui::MouseEvent move_main(ui::EventType::kMouseMoved, cursor_location_main, cursor_location_main, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ui::EventDispatchDetails details = GetEventSink(&top_level_widget)->OnEventFromSource(&move_main);
    ...
    ui::MouseEvent mouse_down_dialog(
        ui::EventType::kMousePressed, cursor_location_dialog, cursor_location_dialog, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    details = GetEventSink(&top_level_widget)->OnEventFromSource(&mouse_down_dialog);
    ASSERT_FALSE(details.dispatcher_destroyed);
    ...
    ui::MouseEvent mouse_down_main(ui::EventType::kMouseMoved, cursor_location_main2, cursor_location_main2, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    details = GetEventSink(&top_level_widget)->OnEventFromSource(&mouse_down_main);
    ASSERT_FALSE(details.dispatcher_destroyed);
...
TEST_F(DesktopNativeWidgetAuraTest, WindowModalityActivationTest)
{
    ...
    widget_delegate.InitWidget(CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW));
    ...
    LRESULT activate_result = ::SendMessage(win32_window, WM_MOUSEACTIVATE, reinterpret_cast<WPARAM>(win32_window), MAKELPARAM(WM_LBUTTONDOWN, HTCLIENT));
...
// event
TEST_F(DesktopNativeWidgetAuraTest, CharMessagesAsKeyboardMessagesDoesNotCrash)
{
    Widget widget;
    Widget::InitParams params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    widget.Init(std::move(params));
    ...
    ui::WindowEventTarget* target = reinterpret_cast<ui::WindowEventTarget*>(
        ui::ViewProp::GetValue(widget.GetNativeWindow()->GetHost()->GetAcceleratedWidget(), ui::WindowEventTarget::kWin32InputEventTarget));
...
{
    Widget root, widget;
    Widget::InitParams root_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    Widget::InitParams widget_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
