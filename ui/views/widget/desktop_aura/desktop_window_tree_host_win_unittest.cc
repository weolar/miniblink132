...
TEST_F(DesktopWindowTreeHostWinTest, DebuggingId)
{
    ...
    DesktopWindowTreeHostWin* desktop_window_tree_host = static_cast<DesktopWindowTreeHostWin*>(widget.GetNativeWindow()->GetHost());
    EXPECT_EQ(std::string(kDebuggingName), DesktopWindowTreeHostWinTestApi(desktop_window_tree_host).GetHwndMessageHandler()->debugging_id());
}
...
class DesktopWindowTreeHostWinAccessibilityObjectTest : public DesktopWidgetTest {
public:
    DesktopWindowTreeHostWinAccessibilityObjectTest() = default;

    DesktopWindowTreeHostWinAccessibilityObjectTest(const DesktopWindowTreeHostWinAccessibilityObjectTest&) = delete;
    DesktopWindowTreeHostWinAccessibilityObjectTest& operator=(const DesktopWindowTreeHostWinAccessibilityObjectTest&) = delete;
...
protected:
    ...
    {
        DesktopWindowTreeHostWinTestApi host(static_cast<DesktopWindowTreeHostWin*>(widget.GetNativeWindow()->GetHost()));
    ...
    void CacheCaretNode(const Widget& widget)
    {
        ...
        host.GetAXSystemCaret()->GetCaret()->QueryInterface(IID_PPV_ARGS(&test_node_));
    }
...
{
    {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            EXPECT_HRESULT_SUCCEEDED(::AccessibleObjectFromWindow(hwnd, OBJID_CLIENT, IID_PPV_ARGS(&root_accessible)));
...
{
    {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            EXPECT_HRESULT_SUCCEEDED(::AccessibleObjectFromWindow(hwnd, OBJID_CARET, IID_PPV_ARGS(&caret_accessible)));
...
TEST_F(DesktopWindowTreeHostWinAccessibilityObjectTest, UiaRootDoesNotLeak)
{
...

    {
        ...
        ASSERT_HRESULT_SUCCEEDED(CoCreateInstance(CLSID_CUIAutomation, nullptr, CLSCTX_INPROC_SERVER, IID_IUIAutomation, &uia));
        HWND hwnd = widget.GetNativeWindow()->GetHost()->GetAcceleratedWidget();
        for (int i = 0; i < 10; i++) {
            ...
            // to it.
            ::UiaRaiseStructureChangedEvent(test_node_.Get(), StructureChangeType_ChildrenInvalidated, nullptr, 0);
        }
