...
DEFINE_EXPORTED_UI_CLASS_PROPERTY_TYPE(VIEWS_EXPORT, views::DesktopNativeWidgetAura*)

namespace views {

DEFINE_UI_CLASS_PROPERTY_KEY(DesktopNativeWidgetAura*, kDesktopNativeWidgetAuraKey, NULL)
...
public:
    ...
    static aura::Window* CreateParentWindow(
        aura::Window* child_window, aura::Window* context, const gfx::Rect& bounds, bool full_screen, bool is_menu, ui::ZOrderLevel root_z_order)
    {
        ...
        init_params.activatable = full_screen ? Widget::InitParams::Activatable::kYes : Widget::InitParams::Activatable::kNo;
        ...
        if (anchor) {
            init_params.init_properties_container.SetProperty(aura::client::kOwnedWindowAnchor, *anchor);
    ...
    DesktopNativeWidgetTopLevelHandler(const DesktopNativeWidgetTopLevelHandler&) = delete;
    DesktopNativeWidgetTopLevelHandler& operator=(const DesktopNativeWidgetTopLevelHandler&) = delete;
    ...
    void OnWindowBoundsChanged(aura::Window* window, const gfx::Rect& old_bounds, const gfx::Rect& new_bounds, ui::PropertyChangeReason reason) override
    {
...
class DesktopNativeWidgetAuraWindowParentingClient : public aura::client::WindowParentingClient {
public:
    explicit DesktopNativeWidgetAuraWindowParentingClient(aura::Window* root_window)
        : root_window_(root_window)
    {
        aura::client::SetWindowParentingClient(root_window_, this);
    }

    DesktopNativeWidgetAuraWindowParentingClient(const DesktopNativeWidgetAuraWindowParentingClient&) = delete;
    DesktopNativeWidgetAuraWindowParentingClient& operator=(const DesktopNativeWidgetAuraWindowParentingClient&) = delete;
    ...
    aura::Window* GetDefaultParent(aura::Window* window, const gfx::Rect& bounds, const int64_t display_id) override
    {
        // TODO(crbug.com/40192931): Re-enable this logic once Fuchsia's windowing
        // APIs provide the required functionality.
#if !BUILDFLAG(IS_FUCHSIA)
        bool is_fullscreen = window->GetProperty(aura::client::kShowStateKey) == ui::mojom::WindowShowState::kFullscreen;
        ...
        if (is_fullscreen || is_menu) {
            ...
            return DesktopNativeWidgetTopLevelHandler::CreateParentWindow(window, root_window_ /* context */, bounds, is_fullscreen, is_menu, root_z_order);
        }
...
public:
    ...
    RootWindowDestructionObserver& operator=(const RootWindowDestructionObserver&) = delete;
...
DesktopNativeCursorManager* DesktopNativeWidgetAura::native_cursor_manager_ = nullptr;
wm::CursorManager* DesktopNativeWidgetAura::cursor_manager_ = nullptr;

DesktopNativeWidgetAura::DesktopNativeWidgetAura(internal::NativeWidgetDelegate* delegate)
...
// static
DesktopNativeWidgetAura* DesktopNativeWidgetAura::ForWindow(aura::Window* window)
{
...
void DesktopNativeWidgetAura::OnHostClosed()
{
    ...
    // itself.
    bool should_delete_this = (ownership_ == Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET) || (ownership_ == Widget::InitParams::CLIENT_OWNS_WIDGET);
...
void DesktopNativeWidgetAura::OnDesktopWindowTreeHostDestroyed(aura::WindowTreeHost* host)
{
...
base::WeakPtr<internal::NativeWidgetPrivate> DesktopNativeWidgetAura::GetWeakPtr()
{
    return weak_ptr_factory_.GetWeakPtr();
}

void DesktopNativeWidgetAura::NotifyAccessibilityEvent(ax::mojom::Event event_type)
{
...
{
    if (!native_widget_delegate_ || !native_widget_delegate_->ShouldHandleNativeWidgetActivationChanged(active)) {
        return;
    }
    ...
    if (active) {
        ...
        if (GetWidget()->HasFocusManager()) {
            ...
            View* view_for_activation = focus_manager->GetFocusedView() ? focus_manager->GetFocusedView() : focus_manager->GetStoredFocusView();
            if (!view_for_activation || !view_for_activation->GetWidget()) {
                ...
                activation_client->ActivateWindow(view_for_activation->GetWidget()->GetNativeView());
...
void DesktopNativeWidgetAura::UpdateWindowTransparency()
{
    ...
    const bool transparent = desktop_window_tree_host_->ShouldWindowContentsBeTransparent();

    auto* window_tree_host = desktop_window_tree_host_->AsWindowTreeHost();
    window_tree_host->compositor()->SetBackgroundColor(transparent ? SK_ColorTRANSPARENT : SK_ColorWHITE);
...
void DesktopNativeWidgetAura::InitNativeWidget(Widget::InitParams params)
{
    ...
    if (ownership_ == Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET)
        owned_native_widget_delegate = base::WrapUnique(native_widget_delegate_.get());

    content_window_->AcquireAllPropertiesFrom(std::move(params.init_properties_container));
    ...
    if (!desktop_window_tree_host_) {
        if (params.desktop_window_tree_host) {
            ...
            desktop_window_tree_host_ = DesktopWindowTreeHost::Create(native_widget_delegate_.get(), this);
        }
    ...
    if (widget_type_ == Widget::InitParams::TYPE_WINDOW)
        window_modality_controller_ = std::make_unique<wm::WindowModalityController>(host_->window());
    ...
    use_desktop_native_cursor_manager_ = desktop_window_tree_host_->ShouldUseDesktopNativeCursorManager();
    if (use_desktop_native_cursor_manager_) {
        ...
        if (!native_cursor_manager_) {
            native_cursor_manager_ = desktop_window_tree_host_->GetSingletonDesktopNativeCursorManager();
        }

        native_cursor_manager_->AddHost(host());

        if (!cursor_manager_) {
            cursor_manager_ = new wm::CursorManager(std::unique_ptr<wm::NativeCursorManager>(native_cursor_manager_));
    ...
    window_parenting_client_ = std::make_unique<DesktopNativeWidgetAuraWindowParentingClient>(host_->window());
    ...
    if (params.type != Widget::InitParams::TYPE_TOOLTIP) {
        ...
        tooltip_controller_ = std::make_unique<corewm::TooltipController>(desktop_window_tree_host_->CreateTooltip(), wm::GetActivationClient(host_->window()));
        wm::SetTooltipClient(host_->window(), tooltip_controller_.get());
        host_->window()->AddPreTargetHandler(tooltip_controller_.get());
    }

    if (params.opacity == Widget::InitParams::WindowOpacity::kTranslucent && desktop_window_tree_host_->ShouldCreateVisibilityController()) {
        visibility_controller_ = std::make_unique<wm::VisibilityController>();
        aura::client::SetVisibilityClient(host_->window(), visibility_controller_.get());
        wm::SetChildWindowVisibilityChangesAnimated(host_->window());
    }

    if (params.type == Widget::InitParams::TYPE_WINDOW) {
        focus_manager_event_handler_ = std::make_unique<FocusManagerEventHandler>(GetWidget(), host_->window());
    }

    event_client_ = std::make_unique<DesktopEventClient>();
    aura::client::SetEventClient(host_->window(), event_client_.get());

    shadow_controller_ = std::make_unique<wm::ShadowController>(wm::GetActivationClient(host_->window()), nullptr);

    OnSizeConstraintsChanged();

    window_reorderer_ = std::make_unique<WindowReorderer>(content_window_, GetWidget()->GetRootView());
}
...
void DesktopNativeWidgetAura::ReparentNativeViewImpl(gfx::NativeView new_parent)
{
    desktop_window_tree_host_->SetParent(new_parent ? new_parent->GetHost()->GetAcceleratedWidget() : gfx::kNullAcceleratedWidget);
}

std::unique_ptr<NonClientFrameView> DesktopNativeWidgetAura::CreateNonClientFrameView()
{
...
{
    return native_widget_delegate_ ? native_widget_delegate_->AsWidget() : nullptr;
}
...
void DesktopNativeWidgetAura::SetNativeWindowProperty(const char* name, void* value)
{
    if (content_window_)
...
{
    return content_window_ ? content_window_->GetNativeWindowProperty(name) : nullptr;
}
...
void DesktopNativeWidgetAura::GetWindowPlacement(gfx::Rect* bounds, ui::mojom::WindowShowState* maximized) const
{
    if (desktop_window_tree_host_)
...
void DesktopNativeWidgetAura::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
    if (desktop_window_tree_host_)
        desktop_window_tree_host_->SetWindowIcons(window_icon, app_icon);

    if (content_window_)
        NativeWidgetAura::AssignIconToAuraWindow(content_window_, window_icon, app_icon);
}
...
{
    return desktop_window_tree_host_ ? desktop_window_tree_host_->GetWindowBoundsInScreen() : gfx::Rect();
}

gfx::Rect DesktopNativeWidgetAura::GetClientAreaBoundsInScreen() const
{
    return desktop_window_tree_host_ ? desktop_window_tree_host_->GetClientAreaBoundsInScreen() : gfx::Rect();
}

gfx::Rect DesktopNativeWidgetAura::GetRestoredBounds() const
{
    return desktop_window_tree_host_ ? desktop_window_tree_host_->GetRestoredBounds() : gfx::Rect();
}

std::string DesktopNativeWidgetAura::GetWorkspace() const
{
    return desktop_window_tree_host_ ? desktop_window_tree_host_->GetWorkspace() : std::string();
}
...
{
    return desktop_window_tree_host_ && desktop_window_tree_host_->IsStackedAbove(native_view);
}

void DesktopNativeWidgetAura::SetShape(std::unique_ptr<Widget::ShapeRects> shape)
{
    if (desktop_window_tree_host_)
...
void DesktopNativeWidgetAura::Show(ui::mojom::WindowShowState show_state, const gfx::Rect& restore_bounds)
{
    if (!desktop_window_tree_host_)
        return;
...
bool DesktopNativeWidgetAura::IsVisible() const
{
    ...
    return content_window_ && content_window_->TargetVisibility() && desktop_window_tree_host_ && desktop_window_tree_host_->IsVisible();
}
...
{
    return content_window_ && desktop_window_tree_host_ && desktop_window_tree_host_->IsActive() && wm::IsActiveWindow(content_window_);
}
...
{
    return desktop_window_tree_host_ && desktop_window_tree_host_->IsVisibleOnAllWorkspaces();
}
...
void DesktopNativeWidgetAura::Minimize()
{
    ...
    internal::RootView* root_view = static_cast<internal::RootView*>(GetWidget()->GetRootView());
...
void DesktopNativeWidgetAura::SetFullscreen(bool fullscreen, int64_t target_display_id)
{
    if (desktop_window_tree_host_)
...
void DesktopNativeWidgetAura::SetCanAppearInExistingFullscreenSpaces(bool can_appear_in_existing_fullscreen_spaces)
{
}
...
void DesktopNativeWidgetAura::SetAspectRatio(const gfx::SizeF& aspect_ratio, const gfx::Size& excluded_margin)
{
    if (desktop_window_tree_host_)
...
void DesktopNativeWidgetAura::RunShellDrag(
    std::unique_ptr<ui::OSExchangeData> data, const gfx::Point& location, int operation, ui::mojom::DragEventSource source)
{
    if (!content_window_)
        return;

    views::RunShellDrag(content_window_, std::move(data), location, operation, source);
}
...
{
    return desktop_window_tree_host_ ? desktop_window_tree_host_->IsMoveLoopSupported() : true;
}

Widget::MoveLoopResult DesktopNativeWidgetAura::RunMoveLoop(
    const gfx::Vector2d& drag_offset, Widget::MoveLoopSource source, Widget::MoveLoopEscapeBehavior escape_behavior)
{
    if (!desktop_window_tree_host_)
        return Widget::MoveLoopResult::kCanceled;
    return desktop_window_tree_host_->RunMoveLoop(drag_offset, source, escape_behavior);
}
...
void DesktopNativeWidgetAura::SetVisibilityChangedAnimationsEnabled(bool value)
{
    if (desktop_window_tree_host_)
        desktop_window_tree_host_->SetVisibilityChangedAnimationsEnabled(value);
}

void DesktopNativeWidgetAura::SetVisibilityAnimationDuration(const base::TimeDelta& duration)
{
    wm::SetWindowVisibilityAnimationDuration(content_window_, duration);
}

void DesktopNativeWidgetAura::SetVisibilityAnimationTransition(Widget::VisibilityTransition transition)
{
...
void DesktopNativeWidgetAura::OnSizeConstraintsChanged()
{
    if (!desktop_window_tree_host_)
        return;
    NativeWidgetAura::SetResizeBehaviorFromDelegate(GetWidget()->widget_delegate(), content_window_);
...
{
    return desktop_window_tree_host_ ? desktop_window_tree_host_->AreScreenshotsAllowed() : true;
}
...
{
    return native_widget_delegate_ ? native_widget_delegate_->GetMinimumSize() : gfx::Size();
}

gfx::Size DesktopNativeWidgetAura::GetMaximumSize() const
{
    return native_widget_delegate_ ? native_widget_delegate_->GetMaximumSize() : gfx::Size();
}
...
int DesktopNativeWidgetAura::GetNonClientComponent(const gfx::Point& point) const
{
    return native_widget_delegate_ ? native_widget_delegate_->GetNonClientComponent(point) : 0;
}

bool DesktopNativeWidgetAura::ShouldDescendIntoChildForEventHandling(aura::Window* child, const gfx::Point& location)
{
    return native_widget_delegate_ ? native_widget_delegate_->ShouldDescendIntoChildForEventHandling(content_window_->layer(), child, child->layer(), location)
                                   : false;
}
...
void DesktopNativeWidgetAura::OnDeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor)
{
    if (Widget* widget = GetWidget()) {
        widget->DeviceScaleFactorChanged(old_device_scale_factor, new_device_scale_factor);
    }
}
...
{
    return native_widget_delegate_ ? native_widget_delegate_->HasHitTestMask() : false;
}
...
{
    return (!should_activate_.has_value() || should_activate_.value()) && (native_widget_delegate_ && native_widget_delegate_->CanActivate());
}

////////////////////////////////////////////////////////////////////////////////
// DesktopNativeWidgetAura, wm::ActivationChangeObserver implementation:

void DesktopNativeWidgetAura::OnWindowActivated(wm::ActivationChangeObserver::ActivationReason reason, aura::Window* gained_active, aura::Window* lost_active)
{
    ...
    if (native_widget_delegate_ && !should_activate_.has_value() && (tree_host_active || !content_window_activated)) {
    ...
    desktop_window_tree_host_->OnActiveWindowChanged(content_window_->Contains(gained_active));
}

////////////////////////////////////////////////////////////////////////////////
// DesktopNativeWidgetAura, aura::client::FocusChangeObserver implementation:

void DesktopNativeWidgetAura::OnWindowFocused(aura::Window* gained_focus, aura::Window* lost_focus)
{
    if (!native_widget_delegate_)
        return;
...
{
    DCHECK(drop_helper_.get() != nullptr);
    last_drop_operation_ = drop_helper_->OnDragOver(event.data(), event.location(), event.source_operations());
}

aura::client::DragUpdateInfo DesktopNativeWidgetAura::OnDragUpdated(const ui::DropTargetEvent& event)
{
    DCHECK(drop_helper_.get() != nullptr);
    last_drop_operation_ = drop_helper_->OnDragOver(event.data(), event.location(), event.source_operations());

    return aura::client::DragUpdateInfo(last_drop_operation_, ui::DataTransferEndpoint(ui::EndpointType::kDefault));
}
...
aura::client::DragDropDelegate::DropCallback DesktopNativeWidgetAura::GetDropCallback(const ui::DropTargetEvent& event)
{
    DCHECK(drop_helper_);
    auto drop_helper_cb = drop_helper_->GetDropCallback(event.data(), event.location(), last_drop_operation_);
    return base::BindOnce(&DesktopNativeWidgetAura::PerformDrop, weak_ptr_factory_.GetWeakPtr(), std::move(drop_helper_cb));
}
...
void DesktopNativeWidgetAura::OnHostWorkspaceChanged(aura::WindowTreeHost* host)
{
    if (native_widget_delegate_)
...
void DesktopNativeWidgetAura::PerformDrop(views::DropHelper::DropCallback drop_cb, std::unique_ptr<ui::OSExchangeData> data,
    ui::mojom::DragOperation& output_drag_op, std::unique_ptr<ui::LayerTreeOwner> drag_image_layer_owner)
{
    if (ShouldActivate())
        Activate();

    if (drop_cb)
        std::move(drop_cb).Run(std::move(data), output_drag_op, std::move(drag_image_layer_owner));
}
