...
WindowEventFilterLinux::WindowEventFilterLinux(DesktopWindowTreeHostPlatform* desktop_window_tree_host, ui::WmMoveResizeHandler* handler)
    : desktop_window_tree_host_(desktop_window_tree_host)
    , handler_(handler)
{
...
void WindowEventFilterLinux::HandleLocatedEventWithHitTest(int hit_test, ui::LocatedEvent* event)
{
    if (event->type() != ui::EventType::kMousePressed) {
        return;
    }

    if (event->IsMouseEvent() && HandleMouseEventWithHitTest(hit_test, event->AsMouseEvent())) {
        return;
    }

    if (desktop_window_tree_host_->GetContentWindow()->GetProperty(aura::client::kResizeBehaviorKey) & aura::client::kResizeBehaviorCanResize) {
        MaybeDispatchHostWindowDragMovement(hit_test, event);
    }
}

bool WindowEventFilterLinux::HandleMouseEventWithHitTest(int hit_test, ui::MouseEvent* event)
{
...
void WindowEventFilterLinux::OnClickedCaption(ui::MouseEvent* event, int previous_click_component)
{
    ...
    } else if (event->IsLeftMouseButton() && event->flags() & ui::EF_IS_DOUBLE_CLICK) {
    ...
    ui::LinuxUi::WindowFrameAction action = linux_ui_theme ? linux_ui_theme->GetWindowFrameAction(action_type) : default_action;
    switch (action) {
    ...
    case ui::LinuxUi::WindowFrameAction::kMenu:
        ...
        // location in px.
        gfx::PointF location = desktop_window_tree_host_->GetRootTransform().InverseMapPoint(event->location_f()).value_or(event->location_f());
        gfx::Point location_in_screen = gfx::ToRoundedPoint(location);
        views::View::ConvertPointToScreen(view, &location_in_screen);
        view->ShowContextMenu(location_in_screen, ui::mojom::MenuSourceType::kMouse);
        event->SetHandled();
        break;
    }
}
...
void WindowEventFilterLinux::OnClickedMaximizeButton(ui::MouseEvent* event)
{
    ...
    gfx::Rect display_work_area = display::Screen::GetScreen()->GetDisplayNearestWindow(content_window).work_area();
...
{
    if (!(window->GetProperty(aura::client::kResizeBehaviorKey) & aura::client::kResizeBehaviorCanMaximize)) {
        return;
    }
...
void WindowEventFilterLinux::MaybeDispatchHostWindowDragMovement(int hittest, ui::LocatedEvent* event)
{
    ...
    auto bounds_in_px = desktop_window_tree_host_->AsWindowTreeHost()->GetBoundsInPixels();
...
void WindowEventFilterLinux::OnGestureEvent(ui::GestureEvent* event)
{
    ...
    int hit_test_code = window->delegate() ? window->delegate()->GetNonClientComponent(event->location()) : HTNOWHERE;
    ...
    if (event->type() == ui::EventType::kGestureTap) {
        ...
        if (click_component_ == HTCAPTION && click_component_ == previous_click_component && event->details().tap_count() == 2) {
