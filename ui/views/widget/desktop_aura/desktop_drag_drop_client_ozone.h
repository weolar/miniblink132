...
class VIEWS_EXPORT DesktopDragDropClientOzone : public aura::client::DragDropClient, public ui::WmDropHandler, public aura::WindowObserver {
public:
    DesktopDragDropClientOzone(aura::Window* root_window, ui::WmDragHandler* drag_handler);

    DesktopDragDropClientOzone(const DesktopDragDropClientOzone&) = delete;
    DesktopDragDropClientOzone& operator=(const DesktopDragDropClientOzone&) = delete;
...
protected:
    ...
    ui::mojom::DragOperation StartDragAndDrop(std::unique_ptr<ui::OSExchangeData> data, aura::Window* root_window, aura::Window* source_window,
        const gfx::Point& root_location, int allowed_operations, ui::mojom::DragEventSource source) override;
#if BUILDFLAG(IS_LINUX)
    ...
    // ui::WmDropHandler
    void OnDragEnter(const gfx::PointF& location, int operations, int modifiers) override;
    void OnDragDataAvailable(std::unique_ptr<ui::OSExchangeData> data) override;
    int OnDragMotion(const gfx::PointF& location, int operations, int modifiers) override;
...
private:
    ...
    base::ScopedObservation<aura::Window, aura::WindowObserver> entered_window_observation_ { this };
    ...
    ui::mojom::DragOperation selected_operation_ = ui::mojom::DragOperation::kNone;

    // Holds data about the ongoing outcoming drag session, if any.
    std::unique_ptr<DragContext> drag_context_;

    base::ObserverList<aura::client::DragDropClientObserver>::Unchecked observers_;
