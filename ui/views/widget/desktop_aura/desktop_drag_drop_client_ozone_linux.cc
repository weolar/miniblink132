...
DesktopDragDropClientOzoneLinux::DesktopDragDropClientOzoneLinux(aura::Window* root_window, ui::WmDragHandler* drag_handler)
...
ui::WmDragHandler::LocationDelegate* DesktopDragDropClientOzoneLinux::GetLocationDelegate()
{
    return this;
}

void DesktopDragDropClientOzoneLinux::OnDragLocationChanged(const gfx::Point& screen_point_px)
{
    DCHECK(drag_context());
    ...
    if (dispatch_mouse_event) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&DesktopDragDropClientOzoneLinux::UpdateDragWidgetLocation, weak_factory_.GetWeakPtr()));
    }
}

void DesktopDragDropClientOzoneLinux::OnDragOperationChanged(ui::mojom::DragOperation operation)
{
...
std::optional<gfx::AcceleratedWidget> DesktopDragDropClientOzoneLinux::GetDragWidget()
{
    DCHECK(drag_context());
    if (drag_context()->widget)
        return drag_context()->widget->GetNativeWindow()->GetHost()->GetAcceleratedWidget();
    return std::nullopt;
}
...
void DesktopDragDropClientOzoneLinux::UpdateDragWidgetLocation()
{
    ...
    float scale_factor = ui::GetScaleFactorForNativeView(drag_context()->widget->GetNativeWindow());
    gfx::Point scaled_point = gfx::ScaleToRoundedPoint(*drag_context()->last_screen_location_px, 1.f / scale_factor);
    drag_context()->widget->SetBounds(gfx::Rect(scaled_point - drag_context()->offset, drag_context()->size));
