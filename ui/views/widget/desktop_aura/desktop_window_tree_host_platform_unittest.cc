...
public:
    ...
    bool widget_destroying() const
    {
        return on_widget_destroying_;
    }
    bool visible() const
    {
        return visible_;
...
std::unique_ptr<Widget> CreateWidgetWithNativeWidgetWithParams(Widget::InitParams params)
{
...
public:
    ...
    DesktopWindowTreeHostPlatformTest(const DesktopWindowTreeHostPlatformTest&) = delete;
    DesktopWindowTreeHostPlatformTest& operator=(const DesktopWindowTreeHostPlatformTest&) = delete;
...
TEST_F(DesktopWindowTreeHostPlatformTest, CallOnNativeWidgetDestroying)
{
    ...
    observer.WaitForChange(TestWidgetObserver::Change::kDestroying, false /* old_value */);
...
TEST_F(DesktopWindowTreeHostPlatformTest, ToggleMinimizePropogateToContentWindow)
{
    std::unique_ptr<Widget> widget = CreateWidgetWithNativeWidget();
    widget->Show();

    auto* host_platform = DesktopWindowTreeHostPlatform::GetHostForWidget(widget->GetNativeWindow()->GetHost()->GetAcceleratedWidget());
    ASSERT_TRUE(host_platform);

    EXPECT_TRUE(widget->GetNativeWindow()->IsVisible());

    // Pretend a PlatformWindow enters the minimized state.
    host_platform->OnWindowStateChanged(ui::PlatformWindowState::kUnknown, ui::PlatformWindowState::kMinimized);

    EXPECT_FALSE(widget->GetNativeWindow()->IsVisible());

    // Pretend a PlatformWindow exits the minimized state.
    host_platform->OnWindowStateChanged(ui::PlatformWindowState::kMinimized, ui::PlatformWindowState::kNormal);
    EXPECT_TRUE(widget->GetNativeWindow()->IsVisible());
}
...
TEST_F(DesktopWindowTreeHostPlatformTest, UpdateWindowShapeFromWindowMask)
{
    ...
    auto* content_window = DesktopWindowTreeHostPlatform::GetContentWindowForWidget(widget->GetNativeWindow()->GetHost()->GetAcceleratedWidget());
...
// Calling show/hide/show triggers changing visibility of the native widget.
TEST_F(DesktopWindowTreeHostPlatformTest, OnAcceleratedWidgetMadeVisibleCalled)
{
    std::unique_ptr<Widget> widget = CreateWidgetWithNativeWidget();
...
TEST_F(DesktopWindowTreeHostPlatformTest, SetBoundsWithMinMax)
{
    ...
    EXPECT_EQ(gfx::Size(200, 100).ToString(), widget.GetWindowBoundsInScreen().size().ToString());
    widget.SetBounds(gfx::Rect(300, 200));
    EXPECT_EQ(gfx::Size(300, 200).ToString(), widget.GetWindowBoundsInScreen().size().ToString());

    widget.set_min_size(gfx::Size(100, 100));
    widget.SetBounds(gfx::Rect(50, 500));
    EXPECT_EQ(gfx::Size(100, 500).ToString(), widget.GetWindowBoundsInScreen().size().ToString());
}

class ResizeObserver : public aura::WindowTreeHostObserver {
public:
    explicit ResizeObserver(aura::WindowTreeHost* host)
        : host_(host)
    {
    ...
    int bounds_change_count() const
    {
        return bounds_change_count_;
    }
    int resize_count() const
    {
...
TEST_F(DesktopWindowTreeHostPlatformTest, SetBoundsWithUnchangedSize)
{
    ...
    auto* dwth_platform = DesktopWindowTreeHostPlatform::GetHostForWidget(widget->GetNativeWindow()->GetHost()->GetAcceleratedWidget());
...
TEST_F(DesktopWindowTreeHostPlatformTest, MakesParentChildRelationship)
{
    ...
    Widget::InitParams widget_2_params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_MENU);
    widget_2_params.bounds = gfx::Rect(110, 110, 100, 100);
    widget_2_params.parent = widget->GetNativeWindow();
    auto widget2 = CreateWidgetWithNativeWidgetWithParams(std::move(widget_2_params));
    widget2->Show();
    ...
    auto* host_platform2 = DesktopWindowTreeHostPlatform::GetHostForWidget(widget2->GetNativeWindow()->GetHost()->GetAcceleratedWidget());
    EXPECT_EQ(host_platform2->window_parent_, host_platform);
    EXPECT_EQ(*host_platform->window_children_.begin(), host_platform2);

    Widget::InitParams widget_3_params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_MENU);
    widget_3_params.bounds = gfx::Rect(120, 120, 50, 80);
    widget_3_params.parent = widget->GetNativeWindow();
    auto widget3 = CreateWidgetWithNativeWidgetWithParams(std::move(widget_3_params));
    widget3->Show();

    EXPECT_EQ(host_platform->window_parent_, nullptr);
    EXPECT_EQ(host_platform->window_children_.size(), 2u);

    auto* host_platform3 = DesktopWindowTreeHostPlatform::GetHostForWidget(widget3->GetNativeWindow()->GetHost()->GetAcceleratedWidget());
    EXPECT_EQ(host_platform3->window_parent_, host_platform);
    EXPECT_NE(host_platform->window_children_.find(host_platform3), host_platform->window_children_.end());

    Widget::InitParams widget_4_params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_TOOLTIP);
    widget_4_params.bounds = gfx::Rect(105, 105, 10, 10);
    widget_4_params.context = widget->GetNativeWindow();
    auto widget4 = CreateWidgetWithNativeWidgetWithParams(std::move(widget_4_params));
    widget4->Show();

    EXPECT_EQ(host_platform->window_parent_, nullptr);
    auto* host_platform4 = DesktopWindowTreeHostPlatform::GetHostForWidget(widget4->GetNativeWindow()->GetHost()->GetAcceleratedWidget());
    if (context_is_also_parent) {
        ...
        EXPECT_NE(host_platform->window_children_.find(host_platform4), host_platform->window_children_.end());
    } else {
...
public:
    ...
    void AddAccessiblePane(View* pane)
    {
        accessible_panes_.push_back(pane);
    }
...
TEST_F(DesktopWindowTreeHostPlatformTest, OnRotateFocus)
{
    ...
    EXPECT_TRUE(DesktopWindowTreeHostPlatform::RotateFocusForWidget(*widget, Direction::kForward, true));
    EXPECT_EQ(views[0], focus_manager->GetFocusedView());

    EXPECT_TRUE(DesktopWindowTreeHostPlatform::RotateFocusForWidget(*widget, Direction::kForward, false));
    EXPECT_EQ(views[1], focus_manager->GetFocusedView());

    EXPECT_TRUE(DesktopWindowTreeHostPlatform::RotateFocusForWidget(*widget, Direction::kBackward, false));
    EXPECT_EQ(views[0], focus_manager->GetFocusedView());

    // Attempting to rotate again without resetting should notify that we've
    // reached the end.
    EXPECT_FALSE(DesktopWindowTreeHostPlatform::RotateFocusForWidget(*widget, Direction::kBackward, false));
    EXPECT_EQ(views[0], focus_manager->GetFocusedView());

    // Restart rotating from back.
    EXPECT_TRUE(DesktopWindowTreeHostPlatform::RotateFocusForWidget(*widget, Direction::kBackward, true));
    EXPECT_EQ(views[1], focus_manager->GetFocusedView());
}
...
#if !BUILDFLAG(IS_FUCHSIA)
class ScopedPlatformWindowFactoryDelegate : public aura::WindowTreeHostPlatform::PlatformWindowFactoryDelegateForTesting {
public:
    ScopedPlatformWindowFactoryDelegate()
    {
        aura::WindowTreeHostPlatform::SetPlatformWindowFactoryDelegateForTesting(this);
    }
    ScopedPlatformWindowFactoryDelegate(const ScopedPlatformWindowFactoryDelegate&) = delete;
    ScopedPlatformWindowFactoryDelegate& operator=(const ScopedPlatformWindowFactoryDelegate&) = delete;
    ~ScopedPlatformWindowFactoryDelegate() override
    {
        aura::WindowTreeHostPlatform::SetPlatformWindowFactoryDelegateForTesting(nullptr);
    }

    std::unique_ptr<ui::PlatformWindow> Create(aura::WindowTreeHostPlatform* host) override
    {
...
TEST_F(DesktopWindowTreeHostPlatformTest, ShowInitiallyMinimizedWidget)
{
    ...
    std::unique_ptr<ScopedPlatformWindowFactoryDelegate> scoped_platform_window_factory_delegate(new ScopedPlatformWindowFactoryDelegate);
    std::unique_ptr<Widget> widget = CreateWidgetWithNativeWidgetWithParams(std::move(params));
...
TEST_F(DesktopWindowTreeHostPlatformTest, FocusParentWindowWillActivate)
{
    ...
    Widget::InitParams widget_2_params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_BUBBLE);
    widget_2_params.bounds = gfx::Rect(110, 110, 100, 100);
    widget_2_params.parent = widget->GetNativeWindow();
    auto widget2 = CreateWidgetWithNativeWidgetWithParams(std::move(widget_2_params));
    widget2->Show();
    ...
    std::unique_ptr<aura::Window> child_window = std::make_unique<aura::Window>(nullptr);
