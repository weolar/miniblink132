...
TEST_F(DesktopNativeWidgetAuraTest, WidgetsWithChildrenDeactivateCorrectly)
{
    ...
    Widget::InitParams params1(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    ...
    Widget::InitParams params_child(views::Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_BUBBLE);
    params_child.parent = widget1->GetNativeView();
    params_child.native_widget = CreatePlatformNativeWidgetImpl(widget1_child.get(), kDefault, nullptr);
    ...
    Widget::InitParams params2(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    params2.context = GetContext();
    params2.native_widget = new DesktopNativeWidgetAura(widget2.get());
    widget2->Init(std::move(params2));

    auto* activation_client1 = wm::GetActivationClient(widget1->GetNativeView()->GetRootWindow());
    auto* activation_client1_child = wm::GetActivationClient(widget1_child->GetNativeView()->GetRootWindow());
    auto* activation_client2 = wm::GetActivationClient(widget2->GetNativeView()->GetRootWindow());
    ...
    const auto show_widget = [&](Widget* target) {
        ...
        views::test::WaitForWidgetActive(widget1_child.get(), target == widget1_child.get());
    ...
    EXPECT_FALSE(widget2->IsActive());
    EXPECT_EQ(activation_client1->GetActiveWindow(), widget1_child->GetNativeView());
    EXPECT_EQ(activation_client2->GetActiveWindow(), nullptr);
...
// crbug.com/1294404).
TEST_F(DesktopNativeWidgetAuraTest, DesktopWidgetsRegainFocusWhenChildWidgetClosed)
{
    ...
    params_child.native_widget = CreatePlatformNativeWidgetImpl(widget_child.get(), kDefault, nullptr);
    widget_child->Init(std::move(params_child));
    widget_child->widget_delegate()->SetCanActivate(true);

    auto* activation_client = wm::GetActivationClient(widget->GetNativeView()->GetRootWindow());
    auto* activation_client_child = wm::GetActivationClient(widget_child->GetNativeView()->GetRootWindow());
    ...
    EXPECT_EQ(activation_client->GetActiveWindow(), widget_child->GetNativeView());
