...
void UpdateMouseLockRegion(aura::Window* window, bool locked)
{
    ...
    RECT window_rect = display::Screen::GetScreen()->DIPToScreenRectInWindow(window, window->GetBoundsInScreen()).ToRECT();
...
DEFINE_UI_CLASS_PROPERTY_KEY(DesktopWindowTreeHostWin*, kDesktopWindowTreeHostKey, NULL)
...
DesktopWindowTreeHostWin::DesktopWindowTreeHostWin(internal::NativeWidgetDelegate* native_widget_delegate, DesktopNativeWidgetAura* desktop_native_widget_aura)
...
void DesktopWindowTreeHostWin::Init(const Widget::InitParams& params)
{
    ...
    message_handler_ = HWNDMessageHandler::Create(this, native_widget_delegate_->AsWidget()->GetName(), params.ShouldInitAsHeadless());

    ConfigureWindowStyles(message_handler_.get(), params, GetWidget()->widget_delegate(), native_widget_delegate_.get());
    ...
    gfx::Rect pixel_bounds = display::win::ScreenWin::DIPToScreenRect(nullptr, params.bounds);
...
void DesktopWindowTreeHostWin::OnNativeWidgetCreated(const Widget::InitParams& params)
{
    ...
    should_animate_window_close_ = content_window()->GetType() != aura::client::WINDOW_TYPE_NORMAL && !wm::WindowAnimationsDisabled(content_window());
}
...
std::unique_ptr<aura::client::DragDropClient> DesktopWindowTreeHostWin::CreateDragDropClient()
{
    auto res = std::make_unique<DesktopDragDropClientWin>(window(), GetHWND(), this);
...
void DesktopWindowTreeHostWin::Show(ui::mojom::WindowShowState show_state, const gfx::Rect& restore_bounds)
{
    ...
    if (show_state == ui::mojom::WindowShowState::kMaximized) {
        ...
        // details.
        pixel_restore_bounds = display::win::ScreenWin::DIPToScreenRect(nullptr, restore_bounds);
    }
...
{
    gfx::Size size_in_pixels = display::win::ScreenWin::DIPToScreenSize(GetHWND(), size);
    gfx::Size expanded = GetExpandedWindowSize(message_handler_->is_translucent(), size_in_pixels);
    window_enlargement_ = gfx::Vector2d(expanded.width() - size_in_pixels.width(), expanded.height() - size_in_pixels.height());
...
void DesktopWindowTreeHostWin::CenterWindow(const gfx::Size& size)
{
    ...
    expanded_size = GetExpandedWindowSize(message_handler_->is_translucent(), size_in_pixels);
    window_enlargement_ = gfx::Vector2d(expanded_size.width() - size_in_pixels.width(), expanded_size.height() - size_in_pixels.height());
    message_handler_->CenterWindow(expanded_size);
}

void DesktopWindowTreeHostWin::GetWindowPlacement(gfx::Rect* bounds, ui::mojom::WindowShowState* show_state) const
{
...
gfx::Rect DesktopWindowTreeHostWin::GetWorkAreaBoundsInScreen() const
{
    ...
    GetMonitorInfo(MonitorFromWindow(message_handler_->hwnd(), MONITOR_DEFAULTTONEAREST), &monitor_info);
    gfx::Rect pixel_bounds = gfx::Rect(monitor_info.rcWork);
    return display::win::ScreenWin::ScreenToDIPRect(GetHWND(), pixel_bounds);
}

void DesktopWindowTreeHostWin::SetShape(std::unique_ptr<Widget::ShapeRects> native_shape)
{
    ...
    if (scale > 1.0) {
        ...
        for (const gfx::Rect& rect : *native_shape) {
            ...
            SkRect scaled_rect = SkRect::MakeLTRB(sk_rect.left() * scale, sk_rect.top() * scale, sk_rect.right() * scale, sk_rect.bottom() * scale);
...
ui::ZOrderLevel DesktopWindowTreeHostWin::GetZOrderLevel() const
{
    ...
    return window_always_on_top ? ui::ZOrderLevel::kFloatingWindow : ui::ZOrderLevel::kNormal;
}
...
{
    const bool hide_on_escape = escape_behavior == Widget::MoveLoopEscapeBehavior::kHide;
    return message_handler_->RunMoveLoop(drag_offset, hide_on_escape) ? Widget::MoveLoopResult::kSuccessful : Widget::MoveLoopResult::kCanceled;
}
...
void DesktopWindowTreeHostWin::SetVisibilityChangedAnimationsEnabled(bool value)
{
    message_handler_->SetVisibilityChangedAnimationsEnabled(value);
    if (desktop_native_widget_aura_->widget_type() != Widget::InitParams::TYPE_WINDOW || remove_standard_frame_) {
        content_window()->SetProperty(aura::client::kAnimationsDisabledKey, !value);
    }
}

std::unique_ptr<NonClientFrameView> DesktopWindowTreeHostWin::CreateNonClientFrameView()
{
    return ShouldUseNativeFrame() ? std::make_unique<NativeFrameView>(native_widget_delegate_->AsWidget()) : nullptr;
}
...
void DesktopWindowTreeHostWin::SetFullscreen(bool fullscreen, int64_t target_display_id)
{
...
void DesktopWindowTreeHostWin::SetAspectRatio(const gfx::SizeF& aspect_ratio, const gfx::Size& excluded_margin)
{
    DCHECK(!aspect_ratio.IsEmpty());
    message_handler_->SetAspectRatio(aspect_ratio.width() / aspect_ratio.height(), excluded_margin);
}

void DesktopWindowTreeHostWin::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
...
gfx::Rect DesktopWindowTreeHostWin::GetBoundsInPixels() const
{
    ...
    gfx::Rect without_expansion(bounds.x() + window_expansion_top_left_delta_.x(), bounds.y() + window_expansion_top_left_delta_.y(),
...
void DesktopWindowTreeHostWin::SetBoundsInPixels(const gfx::Rect& bounds)
{
    ...
    gfx::Rect expanded(bounds.x() - window_expansion_top_left_delta_.x(), bounds.y() - window_expansion_top_left_delta_.y(),
        bounds.width() + window_expansion_bottom_right_delta_.x(), bounds.height() + window_expansion_bottom_right_delta_.y());

    gfx::Rect new_expanded(expanded.origin(), GetExpandedWindowSize(message_handler_->is_translucent(), expanded.size()));
    window_enlargement_ = gfx::Vector2d(new_expanded.width() - expanded.width(), new_expanded.height() - expanded.height());
...
gfx::Rect DesktopWindowTreeHostWin::GetBoundsInAcceleratedWidgetPixelCoordinates()
{
    if (message_handler_->IsMinimized())
        return gfx::Rect();
    const gfx::Rect client_bounds = message_handler_->GetClientAreaBoundsInScreen();
...
bool DesktopWindowTreeHostWin::CaptureSystemKeyEventsImpl(std::optional<base::flat_set<ui::DomCode>> dom_codes)
{
    ...
    keyboard_hook_ = ui::KeyboardHook::CreateModifierKeyboardHook(
        std::move(dom_codes), GetAcceleratedWidget(), base::BindRepeating(&DesktopWindowTreeHostWin::HandleKeyEvent, base::Unretained(this)));
...
base::flat_map<std::string, std::string> DesktopWindowTreeHostWin::GetKeyboardLayoutMap()
{
...
{
    TRACE_EVENT1("ui,input", "DesktopWindowTreeHostWin::SetCursorNative", "cursor", cursor.type());

    message_handler_->SetCursor(ui::WinCursor::FromPlatformCursor(cursor.platform()));
}
...
void DesktopWindowTreeHostWin::MoveCursorToScreenLocationInPixels(const gfx::Point& location_in_pixels)
{
...
std::unique_ptr<aura::ScopedEnableUnadjustedMouseEvents> DesktopWindowTreeHostWin::RequestUnadjustedMovement()
{
...
void DesktopWindowTreeHostWin::SetHostTransitionOffsets(const gfx::Vector2d& top_left_delta, const gfx::Vector2d& bottom_right_delta)
{
...
FrameMode DesktopWindowTreeHostWin::GetFrameMode() const
{
    if (const Widget* widget = GetWidget()) {
        return widget->ShouldUseNativeFrame() ? FrameMode::SYSTEM_DRAWN : FrameMode::CUSTOM_DRAWN;
    }
...
bool DesktopWindowTreeHostWin::CanActivate() const
{
    if (IsModalWindowActive())
        return true;
    return native_widget_delegate_ ? native_widget_delegate_->CanActivate() : false;
}
...
int DesktopWindowTreeHostWin::GetNonClientComponent(const gfx::Point& point) const
{
    gfx::Point dip_position = display::win::ScreenWin::ClientToDIPPoint(GetHWND(), point);
    return native_widget_delegate_->GetNonClientComponent(dip_position);
}

void DesktopWindowTreeHostWin::GetWindowMask(const gfx::Size& size, SkPath* path)
{
    if (Widget* widget = GetWidget(); widget && widget->non_client_view()) {
        widget->non_client_view()->GetWindowMask(display::win::ScreenWin::ScreenToDIPSize(GetHWND(), size), path);
...
bool DesktopWindowTreeHostWin::GetClientAreaInsets(gfx::Insets* insets, HMONITOR monitor) const
{
    return false;
}

bool DesktopWindowTreeHostWin::GetDwmFrameInsetsInPixels(gfx::Insets* insets) const
{
    return false;
}

void DesktopWindowTreeHostWin::GetMinMaxSize(gfx::Size* min_size, gfx::Size* max_size) const
{
...
gfx::Size DesktopWindowTreeHostWin::DIPToScreenSize(const gfx::Size& dip_size) const
{
...
void DesktopWindowTreeHostWin::HandleAccelerator(const ui::Accelerator& accelerator)
{
    if (Widget* widget = GetWidget()) {
...
bool DesktopWindowTreeHostWin::HandleInitialFocus(ui::mojom::WindowShowState show_state)
{
    if (Widget* widget = GetWidget()) {
...
void DesktopWindowTreeHostWin::HandleClientSizeChanged(const gfx::Size& new_size)
{
    CheckForMonitorChange();
    if (dispatcher())
...
void DesktopWindowTreeHostWin::HandleKeyEvent(ui::KeyEvent* event)
{
    ...
    if ((event->type() == ui::EventType::kKeyPressed) && (event->key_code() == ui::VKEY_SPACE) && (event->flags() & ui::EF_ALT_DOWN)
...
void DesktopWindowTreeHostWin::HandleTouchEvent(ui::TouchEvent* event)
{
    ...
    if (host) {
        ...
        if (target && target->HasCapture() && target != this) {
            ...
            ui::TouchEvent target_event(*event, static_cast<View*>(nullptr), static_cast<View*>(nullptr));
...
bool DesktopWindowTreeHostWin::HandleIMEMessage(UINT message, WPARAM w_param, LPARAM l_param, LRESULT* result)
{
    ...
    if ((message == WM_SYSCHAR) && (w_param == VK_SPACE)) {
        if (Widget* widget = GetWidget(); widget && widget->non_client_view()) {
            ...
            ShowSystemMenuAtScreenPixelLocation(GetHWND(), frame->GetSystemMenuScreenPixelLocation());
            return true;
        }
    }
...
void DesktopWindowTreeHostWin::HandlePaintAccelerated(const gfx::Rect& invalid_rect)
{
    if (compositor())
...
bool DesktopWindowTreeHostWin::PreHandleMSG(UINT message, WPARAM w_param, LPARAM l_param, LRESULT* result)
{
    return false;
}

void DesktopWindowTreeHostWin::PostHandleMSG(UINT message, WPARAM w_param, LPARAM l_param)
{
}
...
void DesktopWindowTreeHostWin::HandleWindowScaleFactorChanged(float window_scale_factor)
{
    ...
    // using?
    if (compositor()) {
        compositor()->SetScaleAndSize(window_scale_factor, message_handler_->GetClientAreaBounds().size(), window()->GetLocalSurfaceId());
    }
}

void DesktopWindowTreeHostWin::HandleHeadlessWindowBoundsChanged(const gfx::Rect& bounds)
{
    window()->SetProperty(aura::client::kHeadlessBoundsKey, bounds);
}

DesktopNativeCursorManager* DesktopWindowTreeHostWin::GetSingletonDesktopNativeCursorManager()
{
...
void DesktopWindowTreeHostWin::SetBoundsInDIP(const gfx::Rect& bounds)
{
    ...
    const gfx::Rect bounds_in_pixels = display::Screen::GetScreen()->DIPToScreenRectInWindow(root, AdjustedContentBounds(bounds));
...
bool DesktopWindowTreeHostWin::IsModalWindowActive() const
{
    ...
    const auto is_active
        = [](const aura::Window* child) { return child->GetProperty(aura::client::kModalKey) != ui::mojom::ModalType::kNone && child->TargetVisibility(); };
    return base::ranges::any_of(window()->children(), is_active);
}

void DesktopWindowTreeHostWin::CheckForMonitorChange()
{
    HMONITOR monitor_from_window = ::MonitorFromWindow(GetHWND(), MONITOR_DEFAULTTOPRIMARY);
...
gfx::Rect DesktopWindowTreeHostWin::AdjustedContentBounds(const gfx::Rect& bounds)
{
...
void DesktopWindowTreeHostWin::UpdateAllowScreenshots()
{
    ...
    SetWindowDisplayAffinity(GetHWND(), allow_screenshots_ ? WDA_NONE : WDA_MONITOR);
}
...
{
    return new DesktopWindowTreeHostWin(native_widget_delegate, desktop_native_widget_aura);
}
