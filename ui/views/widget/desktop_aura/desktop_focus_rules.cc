...
bool DesktopFocusRules::CanActivateWindow(const aura::Window* window) const
{
    ...
    if (window && window->IsRootWindow())
        return false;
    if (window && IsToplevelWindow(window) && content_window_->GetRootWindow()->Contains(window)
        && wm::WindowStateIs(window->GetRootWindow(), ui::mojom::WindowShowState::kMinimized)) {
        return true;
    }
...
bool DesktopFocusRules::CanFocusWindow(const aura::Window* window, const ui::Event* event) const
{
    return BaseFocusRules::CanFocusWindow(window, event) || wm::WindowStateIs(window->GetRootWindow(), ui::mojom::WindowShowState::kMinimized);
}

bool DesktopFocusRules::SupportsChildActivation(const aura::Window* window) const
{
...
bool DesktopFocusRules::IsWindowConsideredVisibleForActivation(const aura::Window* window) const
{
...
const aura::Window* DesktopFocusRules::GetToplevelWindow(const aura::Window* window) const
{
    const aura::Window* top_level_window = wm::BaseFocusRules::GetToplevelWindow(window);
    // In Desktop-Aura, only the content_window or children of the RootWindow are
...
aura::Window* DesktopFocusRules::GetNextActivatableWindow(aura::Window* window) const
{
