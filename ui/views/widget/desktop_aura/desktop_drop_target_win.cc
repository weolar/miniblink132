...
DWORD DesktopDropTargetWin::OnDragEnter(IDataObject* data_object, DWORD key_state, POINT position, DWORD effect)
{
    ...
    return ui::DragDropTypes::DragOperationToDropEffect(ui::DragDropTypes::DRAG_NONE);
}

DWORD DesktopDropTargetWin::OnDragOver(IDataObject* data_object, DWORD key_state, POINT position, DWORD effect)
{
...
DWORD DesktopDropTargetWin::OnDrop(IDataObject* data_object, DWORD key_state, POINT position, DWORD effect)
{
    ...
    return ui::DragDropTypes::DragOperationToDropEffect(static_cast<int>(drag_operation));
}
...
void DesktopDropTargetWin::Translate(IDataObject* data_object, DWORD key_state, POINT position, DWORD effect, std::unique_ptr<OSExchangeData>* data,
    std::unique_ptr<ui::DropTargetEvent>* event, DragDropDelegate** delegate)
{
    ...
    const bool should_change_target_window = !target_window || !target_window_observation_.IsObservingSource(target_window);
    ...
    *delegate = target_window ? aura::client::GetDragDropDelegate(target_window) : nullptr;
    if (!*delegate) {
        return;
    }

    *data = std::make_unique<OSExchangeData>(std::make_unique<OSExchangeDataProviderWin>(data_object));
    location = root_location;
    aura::Window::ConvertPointToTarget(root_window_, target_window, &location);
    *event = std::make_unique<ui::DropTargetEvent>(
        *(data->get()), gfx::PointF(location), gfx::PointF(root_location), ui::DragDropTypes::DropEffectToDragOperation(effect));
...
void DesktopDropTargetWin::NotifyDragLeave()
{
    ...
    DragDropDelegate* delegate = aura::client::GetDragDropDelegate(target_window_observation_.GetSource());
    if (delegate)
