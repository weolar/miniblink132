...
void DispatchMouseMotionEventSync(const gfx::Point& point_in_screen)
{
    ...
    ui_controls::SendMouseMoveNotifyWhenDone(point_in_screen.x(), point_in_screen.y(), run_loop.QuitClosure());
    run_loop.Run();
}
...
public:
    ...
    gfx::Point pointer_location_in_px() const
    {
        return pointer_location_in_px_;
    }
    ...
    void DispatchHostWindowDragMovement(int hittest, const gfx::Point& pointer_location_in_px) override
    {
        hittest_ = hittest;
...
void SetExpectationBasedOnHittestValue(int hittest, const FakeWmMoveResizeHandler& handler, const gfx::Point& pointer_location_in_px)
{
    if (IsNonClientComponent(hittest)) {
        ...
        EXPECT_EQ(handler.pointer_location_in_px().ToString(), pointer_location_in_px.ToString());
...
public:
    ...
    HitTestNonClientFrameView& operator=(const HitTestNonClientFrameView&) = delete;

    ~HitTestNonClientFrameView() override = default;

    void set_hit_test_result(int component)
    {
        hit_test_result_ = component;
    }
...
class TestDesktopWindowTreeHostPlatformImpl : public DesktopWindowTreeHostPlatformImpl {
public:
    TestDesktopWindowTreeHostPlatformImpl(internal::NativeWidgetDelegate* native_widget_delegate, DesktopNativeWidgetAura* desktop_native_widget_aura)
        : DesktopWindowTreeHostPlatformImpl(native_widget_delegate, desktop_native_widget_aura)
    {
    }

    TestDesktopWindowTreeHostPlatformImpl(const TestDesktopWindowTreeHostPlatformImpl&) = delete;
    TestDesktopWindowTreeHostPlatformImpl& operator=(const TestDesktopWindowTreeHostPlatformImpl&) = delete;
    ...
    void ResetCalledMaximize()
    {
        called_maximize_ = false;
    }
    bool called_maximize() const
    {
        return called_maximize_;
    }
...
class DesktopWindowTreeHostPlatformImplTest : public test::DesktopWidgetTestInteractive, public views::WidgetObserver {
public:
    DesktopWindowTreeHostPlatformImplTest() = default;

    DesktopWindowTreeHostPlatformImplTest(const DesktopWindowTreeHostPlatformImplTest&) = delete;
    DesktopWindowTreeHostPlatformImplTest& operator=(const DesktopWindowTreeHostPlatformImplTest&) = delete;
...
protected:
    Widget* BuildTopLevelDesktopWidget(const gfx::Rect& bounds)
    {
        ...
        Widget::InitParams toplevel_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    ...
    void GenerateAndDispatchClickMouseEvent(const gfx::Point& click_location, int flags)
    {
        DCHECK(host_);
        DispatchEvent(GenerateMouseEvent(ui::EventType::kMousePressed, click_location, flags));
        DispatchEvent(GenerateMouseEvent(ui::EventType::kMouseReleased, click_location, flags));
    }

    ui::MouseEvent* GenerateMouseEvent(ui::EventType event_type, const gfx::Point& click_location, int flags)
    {
        int flag = 0;
        if (flags & ui::EF_LEFT_MOUSE_BUTTON) {
            flag = ui::EF_LEFT_MOUSE_BUTTON;
        } else {
            CHECK(flags & ui::EF_RIGHT_MOUSE_BUTTON) << "Other mouse clicks are not supported yet. Add the new one.";
            flag = ui::EF_RIGHT_MOUSE_BUTTON;
        }
        return new ui::MouseEvent(event_type, click_location, click_location, base::TimeTicks::Now(), flags, flag);
    }

    ui::GestureEvent* GenerateGestureEvent(const gfx::Point& gesture_location, const ui::GestureEventDetails& gesture_details)
    {
        return new ui::GestureEvent(gesture_location.x(), gesture_location.y(), 0, base::TimeTicks::Now(), gesture_details);
    }
...
class DesktopWindowTreeHostPlatformImplTestWithTouch : public DesktopWindowTreeHostPlatformImplTest, public testing::WithParamInterface<bool> {
public:
    bool use_touch_event() const
    {
        return GetParam();
    }
};
...
TEST_P(DesktopWindowTreeHostPlatformImplTestWithTouch, HitTest)
{
    ...
    auto handler = std::make_unique<FakeWmMoveResizeHandler>(host_->platform_window());
    host_->DestroyNonClientEventFilter();
    host_->non_client_window_event_filter_ = std::make_unique<WindowEventFilterLinux>(host_, handler.get());
    ...
    for (int hittest : hittest_values) {
        ...
        if (hittest == HTCAPTION) {
            ...
            bounds = gfx::Rect(gfx::Point(bounds.x() + 2, bounds.y() + 4), bounds.size());
            ...
            bounds = gfx::Rect(gfx::Point(bounds.origin()), gfx::Size(bounds.size().width() + 5, bounds.size().height() + 10));
            handler->set_bounds(bounds);
        }
        ...
        if (use_touch_event()) {
            ui::GestureEventDetails gesture_details(ui::EventType::kGestureScrollBegin);
            DispatchEvent(GenerateGestureEvent(pointer_location_in_px, gesture_details));
        } else {
            DispatchEvent(GenerateMouseEvent(ui::EventType::kMousePressed, pointer_location_in_px, ui::EF_LEFT_MOUSE_BUTTON));
        }
        ...
        SetExpectationBasedOnHittestValue(hittest, *handler.get(), expected_pointer_location_in_px);
        ...
        if (use_touch_event()) {
            ...
            DispatchEvent(GenerateMouseEvent(ui::EventType::kMouseReleased, pointer_location_in_px, ui::EF_LEFT_MOUSE_BUTTON));
        }
    }
...
TEST_P(DesktopWindowTreeHostPlatformImplTestWithTouch, DoubleClickHeaderMaximizes)
{
    ...
    aura::Window* window = widget->GetNativeWindow();
    window->SetProperty(aura::client::kResizeBehaviorKey, aura::client::kResizeBehaviorCanMaximize);
...
// second click.
TEST_P(DesktopWindowTreeHostPlatformImplTestWithTouch, DoubleClickTwoDifferentTargetsDoesntMaximizes)
{
    gfx::Rect bounds(0, 0, 100, 100);
    std::unique_ptr<Widget> widget(BuildTopLevelDesktopWidget(bounds));
    widget->Show();
...
TEST_F(DesktopWindowTreeHostPlatformImplTest, RightClickDuringDoubleClickDoesntMaximize)
{
    gfx::Rect bounds(0, 0, 100, 100);
    std::unique_ptr<Widget> widget(BuildTopLevelDesktopWidget(bounds));
    widget->Show();
...
TEST_F(DesktopWindowTreeHostPlatformImplTest, CaptureEventForwarding)
{
    ...
    EXPECT_EQ(point_in_screen.ToString(), aura::Env::GetInstance()->last_mouse_location().ToString());
...
INSTANTIATE_TEST_SUITE_P(, DesktopWindowTreeHostPlatformImplTestWithTouch, testing::Bool());
