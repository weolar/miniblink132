...
DesktopDragDropClientWin::DesktopDragDropClientWin(aura::Window* root_window, HWND window, DesktopWindowTreeHostWin* desktop_host)
...
ui::mojom::DragOperation DesktopDragDropClientWin::StartDragAndDrop(std::unique_ptr<ui::OSExchangeData> data, aura::Window* root_window,
    aura::Window* source_window, const gfx::Point& screen_location, int allowed_operations, ui::mojom::DragEventSource source)
{
    gfx::Point touch_screen_point;
    if (source == ui::mojom::DragEventSource::kTouch) {
        display::Screen* screen = display::Screen::GetScreen();
        CHECK(screen);
        aura::Window* window = screen->GetWindowAtScreenPoint(screen->GetCursorScreenPoint());
        touch_screen_point = screen_location + source_window->GetBoundsInScreen().OffsetFromOrigin();
        source_window->GetHost()->ConvertDIPToPixels(&touch_screen_point);
        bool touch_down = aura::Env::GetInstance()->is_touch_down();
        bool touch_over_other_window = !window || window->GetRootWindow() != root_window;
        bool touch_drag_cursor_sync = base::FeatureList::IsEnabled(features::kEnableTouchDragCursorSync);
        ...
        if (touch_drag_cursor_sync && (!touch_down || touch_over_other_window)) {
            return ui::PreferredDragOperation(ui::DragDropTypes::DropEffectToDragOperation(DROPEFFECT_NONE));
        }
    ...
    HRESULT result = ::DoDragDrop(ui::OSExchangeDataProviderWin::GetIDataObject(*data.get()), drag_source_.Get(),
        ui::DragDropTypes::DragOperationToDropEffect(allowed_operations), &effect);
    ...
    return ui::PreferredDragOperation(ui::DragDropTypes::DropEffectToDragOperation(effect));
}
...
void DesktopDragDropClientWin::AddObserver(aura::client::DragDropClientObserver* observer)
{
    NOTIMPLEMENTED();
}

void DesktopDragDropClientWin::RemoveObserver(aura::client::DragDropClientObserver* observer)
{
    NOTIMPLEMENTED();
}
