...
TEST_F(DesktopWindowTreeHostPlatformImplTest, ChildWindowDestructionDuringTearDown)
{
    Widget parent_widget;
    Widget::InitParams parent_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    ...
    ASSERT_NE(parent_widget.GetNativeWindow()->GetHost()->GetAcceleratedWidget(), child_widget.GetNativeWindow()->GetHost()->GetAcceleratedWidget());
...
TEST_F(DesktopWindowTreeHostPlatformImplTest, MouseNCEvents)
{
    ...
    auto* host_platform = static_cast<DesktopWindowTreeHostPlatform*>(widget->GetNativeWindow()->GetHost());
    ASSERT_TRUE(host_platform);

    ui::MouseEvent event(ui::EventType::kMousePressed, gfx::PointF(500, 500), gfx::PointF(500, 500), base::TimeTicks::Now(), 0, 0, {});
...
TEST_F(DesktopWindowTreeHostPlatformImplTest, ContentWindowVisibilityMatchesCompositorVisibility)
{
    std::unique_ptr<Widget> widget = CreateWidget(new ShapedWidgetDelegate());
    // Disable native occlusion tracking so it doesn't interfere with visibility
    // for this test.
    aura::NativeWindowOcclusionTracker::DisableNativeWindowOcclusionTracking(widget->GetNativeWindow()->GetHost());
    widget->Show();
    base::RunLoop().RunUntilIdle();
...
TEST_F(DesktopWindowTreeHostPlatformImplTest, SetZOrderCorrectlySetsZOrderOnPlatformWindows)
{
    ...
    ASSERT_EQ(ui::ZOrderLevel::kFloatingWindow, platform_window->GetZOrderLevel());
...
class DesktopWindowTreeHostPlatformImplHighDPITest : public DesktopWindowTreeHostPlatformImplTest {
public:
...
TEST_F(DesktopWindowTreeHostPlatformImplHighDPITest, MouseNCEvents)
{
    ...
    ui::MouseEvent event(ui::EventType::kMousePressed, gfx::PointF(1001, 1001), gfx::PointF(1001, 1001), base::TimeTicks::Now(), 0, 0, {});
    host_platform->DispatchEvent(&event);
