...
public:
    ...
    SwapWithNewSizeObserverHelper(ui::Compositor* compositor, const HelperCallback& callback)
        : callback_(callback)
    {
    ...
    SwapWithNewSizeObserverHelper& operator=(const SwapWithNewSizeObserverHelper&) = delete;
...
private:
    ...
    void OnCompositingCompleteSwapWithNewSize(ui::Compositor* compositor, const gfx::Size& size) override
    {
    ...
    base::ScopedObservation<ui::Compositor, ui::CompositorObserver> compositor_observation_ { this };
...
// static
const char DesktopWindowTreeHostLinux::kWindowKey[] = "DesktopWindowTreeHostLinux";
...
DesktopWindowTreeHostLinux::DesktopWindowTreeHostLinux(
    ...
    : DesktopWindowTreeHostPlatform(native_widget_delegate, desktop_native_widget_aura)
{
...
base::OnceClosure DesktopWindowTreeHostLinux::DisableEventListening()
{
    ...
    if (modal_dialog_counter_ == 1) {
        ...
        targeter_for_modal_ = std::make_unique<aura::ScopedWindowTargeter>(window(), std::make_unique<aura::NullWindowTargeter>());
    }

    return base::BindOnce(&DesktopWindowTreeHostLinux::EnableEventListening, weak_factory_.GetWeakPtr());
}
...
void DesktopWindowTreeHostLinux::UpdateFrameHints()
{
    if (!GetContentWindow()->targeter()) {
        ...
        GetContentWindow()->targeter()->GetHitTestRects(GetContentWindow(), &hit_test_rect_mouse_dp, &hit_test_rect_touch_dp);
        platform_window()->SetInputRegion(std::optional<std::vector<gfx::Rect>>({ ConvertRectToPixels(hit_test_rect_mouse_dp) }));
    }
}
...
void DesktopWindowTreeHostLinux::Init(const Widget::InitParams& params)
{
    ...
    if (GetX11Extension() && GetX11Extension()->IsSyncExtensionAvailable()) {
        compositor_observer_ = std::make_unique<SwapWithNewSizeObserverHelper>(
            compositor(), base::BindRepeating(&DesktopWindowTreeHostLinux::OnCompleteSwapWithNewSize, base::Unretained(this)));
    }
}

void DesktopWindowTreeHostLinux::OnNativeWidgetCreated(const Widget::InitParams& params)
{
    CreateNonClientEventFilter();
    DesktopWindowTreeHostPlatform::OnNativeWidgetCreated(params);
}

void DesktopWindowTreeHostLinux::InitModalType(ui::mojom::ModalType modal_type)
{
    switch (modal_type) {
    ...
    const gfx::Vector2d& drag_offset, Widget::MoveLoopSource source, Widget::MoveLoopEscapeBehavior escape_behavior)
{
    ...
    Widget::MoveLoopResult result = DesktopWindowTreeHostPlatform::RunMoveLoop(drag_offset, source, escape_behavior);
    if (weak_this.get())
...
void DesktopWindowTreeHostLinux::DispatchEvent(ui::Event* event)
{
    ...
    if (event->IsMouseEvent() || event->IsTouchEvent()) {
        ...
        if (GetContentWindow() && GetContentWindow()->delegate()) {
            ...
            gfx::PointF location_in_dip = GetRootTransform().InverseMapPoint(location).value_or(location);
            hit_test_code = GetContentWindow()->delegate()->GetNonClientComponent(gfx::ToRoundedPoint(location_in_dip));
        ...
        if (located_event->IsMouseEvent() && (located_event->AsMouseEvent()->IsAnyButton() || located_event->IsMouseWheelEvent()))
            FlashFrame(false);
    }
    ...
    // destroyed.
    if ((event->IsMouseEvent() || event->IsTouchEvent()) && non_client_window_event_filter_) {
        non_client_window_event_filter_->HandleLocatedEventWithHitTest(hit_test_code, event->AsLocatedEvent());
    }
...
{
    return platform_window() ? ui::GetX11Extension(*(platform_window())) : nullptr;
}
...
#if BUILDFLAG(USE_ATK)
bool DesktopWindowTreeHostLinux::OnAtkKeyEvent(AtkKeyEventStruct* atk_event, bool transient)
{
    if (!transient && !IsActive() && !HasCapture())
        return false;
    return ui::AtkUtilAuraLinux::HandleAtkKeyEvent(atk_event) == ui::DiscardAtkKeyEvent::Discard;
}
#endif
...
gfx::Rect DesktopWindowTreeHostLinux::GetGuessedFullScreenSizeInPx() const
{
    ...
    const display::Display display = screen->GetDisplayMatching(GetWindowBoundsInScreen());
    return gfx::Rect(gfx::ScaleToFlooredPoint(display.bounds().origin(), display.device_scale_factor()), display.GetSizeInPixel());
}

void DesktopWindowTreeHostLinux::AddAdditionalInitProperties(const Widget::InitParams& params, ui::PlatformWindowInitProperties* properties)
{
    ...
    properties->background_color = GetWidget()->GetColorProvider()->GetColor(target_color);
...
base::flat_map<std::string, std::string> DesktopWindowTreeHostLinux::GetKeyboardLayoutMap()
{
...
void DesktopWindowTreeHostLinux::OnCompleteSwapWithNewSize(const gfx::Size& size)
{
    if (GetX11Extension())
...
void DesktopWindowTreeHostLinux::CreateNonClientEventFilter()
{
    ...
    non_client_window_event_filter_ = std::make_unique<WindowEventFilterClass>(this, GetWmMoveResizeHandler(*platform_window()));
}
...
DesktopWindowTreeHost* DesktopWindowTreeHost::Create(
    internal::NativeWidgetDelegate* native_widget_delegate, DesktopNativeWidgetAura* desktop_native_widget_aura)
{
    return new DesktopWindowTreeHostLinux(native_widget_delegate, desktop_native_widget_aura);
}
