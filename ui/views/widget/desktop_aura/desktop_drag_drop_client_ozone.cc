...
std::unique_ptr<Widget> CreateDragWidget(const gfx::Point& root_location, const gfx::ImageSkia& image, const gfx::Vector2d& drag_widget_offset)
{
    auto widget = std::make_unique<Widget>();
    Widget::InitParams params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_DRAG);
...
void DropIfAllowed(const ui::OSExchangeData* drag_data, aura::client::DragUpdateInfo& drag_info, base::OnceClosure drop_cb)
{
    if (ui::DataTransferPolicyController::HasInstance()) {
        ui::DataTransferPolicyController::Get()->DropIfAllowed(
            (drag_data->GetSource() ? std::make_optional<ui::DataTransferEndpoint>(*drag_data->GetSource()) : std::nullopt), { drag_info.data_endpoint },
            drag_data->GetFilenames(), std::move(drop_cb));
    } else {
...
// closure will run.
void PerformDrop(aura::client::DragDropDelegate::DropCallback drop_cb, std::unique_ptr<ui::OSExchangeData> data_to_drop, base::ScopedClosureRunner drag_cancel)
{
    if (drop_cb) {
...
DesktopDragDropClientOzone::DesktopDragDropClientOzone(aura::Window* root_window, ui::WmDragHandler* drag_handler)
...
{
    ResetDragDropTarget();
    observers_.Notify(&aura::client::DragDropClientObserver::OnDragDropClientDestroying);
}

DragOperation DesktopDragDropClientOzone::StartDragAndDrop(std::unique_ptr<ui::OSExchangeData> data, aura::Window* root_window, aura::Window* source_window,
    const gfx::Point& root_location, int allowed_operations, ui::mojom::DragEventSource source)
{
    if (!drag_handler_) {
    ...
    if (!ui::OzonePlatform::GetInstance()->GetPlatformProperties().platform_shows_drag_image) {
        ...
        if (IsValidDragImage(drag_image)) {
            ...
            drag_context_->widget = CreateDragWidget(root_location, drag_image, drag_context_->offset);
        }
    }
    ...
    const bool drag_succeeded = drag_handler_->StartDrag(*data.get(), allowed_operations, source, cursor_client->GetCursor(), !source_window->HasCapture(),
        base::BindOnce(&DesktopDragDropClientOzone::OnDragStarted, weak_factory_.GetWeakPtr()),
        base::BindOnce(&DesktopDragDropClientOzone::OnDragFinished, weak_factory_.GetWeakPtr()), GetLocationDelegate());
...
void DesktopDragDropClientOzone::UpdateDragImage(const gfx::ImageSkia& image, const gfx::Vector2d& offset)
{
...
void DesktopDragDropClientOzone::AddObserver(aura::client::DragDropClientObserver* observer)
{
    observers_.AddObserver(observer);
}

void DesktopDragDropClientOzone::RemoveObserver(aura::client::DragDropClientObserver* observer)
{
    observers_.RemoveObserver(observer);
}

void DesktopDragDropClientOzone::OnDragEnter(const gfx::PointF& location, int operations, int modifiers)
{
...
void DesktopDragDropClientOzone::OnDragDataAvailable(std::unique_ptr<ui::OSExchangeData> data)
{
    DCHECK(data);
    data_to_drop_ = std::move(data);
    std::unique_ptr<ui::DropTargetEvent> event = UpdateTargetAndCreateDropEvent();
    if (event) {
        observers_.Notify(&aura::client::DragDropClientObserver::OnDragUpdated, *event);
    }
}

int DesktopDragDropClientOzone::OnDragMotion(const gfx::PointF& location, int operations, int modifiers)
{
    drag_location_ = location;
    available_operations_ = operations;
    modifiers_ = modifiers;
    ...
    if (data_to_drop_) {
        ...
        if (delegate_ && event) {
            if (auto drop_cb = delegate_->GetDropCallback(*event)) {
                base::ScopedClosureRunner drag_cancel(base::BindOnce(&DesktopDragDropClientOzone::DragCancel, weak_factory_.GetWeakPtr()));

                auto* data_to_drop_raw = data_to_drop_.get();
                DropIfAllowed(data_to_drop_raw, current_drag_update_info_,
                    base::BindOnce(&PerformDrop, std::move(drop_cb), std::move(data_to_drop_), std::move(drag_cancel)));

                observers_.Notify(&aura::client::DragDropClientObserver::OnDragCompleted, *event);
            }
        }
    }
...
ui::WmDragHandler::LocationDelegate* DesktopDragDropClientOzone::GetLocationDelegate()
{
    return nullptr;
}
...
void DesktopDragDropClientOzone::OnDragFinished(DragOperation operation)
{
    ...
    observers_.Notify(&aura::client::DragDropClientObserver::OnDropCompleted, operation);
}

std::unique_ptr<ui::DropTargetEvent> DesktopDragDropClientOzone::UpdateTargetAndCreateDropEvent()
{
    DCHECK(data_to_drop_);

    aura::Window* window = root_window_->GetEventHandlerForPoint(gfx::ToFlooredPoint(drag_location_));
    ...
    auto event = std::make_unique<ui::DropTargetEvent>(*data_to_drop_, target_location, gfx::PointF(drag_location_), available_operations_);
