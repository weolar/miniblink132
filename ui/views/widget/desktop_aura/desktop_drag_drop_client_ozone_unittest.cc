...
public:
    ...
    void SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon) override
    {
    }
    ...
    // ui::WmDragHandler
    bool StartDrag(const OSExchangeData& data, int operation, DragEventSource source, gfx::NativeCursor cursor, bool can_grab_pointer,
        base::OnceClosure drag_started_callback, WmDragHandler::DragFinishedCallback drag_finished_callback, WmDragHandler::LocationDelegate* delegate) override
    {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&FakePlatformWindow::ProcessDrag, base::Unretained(this), std::move(source_data_), operation));
    ...
    void OnDragEnter(const gfx::PointF& point, std::unique_ptr<OSExchangeData> data, int operation)
    {
...
public:
    ...
    int last_event_flags() const
    {
        return last_event_flags_;
    }
    ui::OSExchangeData* received_data() const
    {
        return received_data_.get();
    }
...
private:
    ...
    aura::client::DragUpdateInfo OnDragUpdated(const ui::DropTargetEvent& event) override
    {
        ++num_updates_;
        last_event_flags_ = event.flags();

        return aura::client::DragUpdateInfo(static_cast<int>(destination_operation_), ui::DataTransferEndpoint(ui::EndpointType::kDefault));
    }
    ...
    {
        last_event_flags_ = event.flags();
        return base::BindOnce(&FakeDragDropDelegate::PerformDrop, base::Unretained(this));
    }

    void PerformDrop(
        std::unique_ptr<ui::OSExchangeData> data, ui::mojom::DragOperation& output_drag_op, std::unique_ptr<ui::LayerTreeOwner> drag_image_layer_owner)
    {
        ++num_drops_;
...
public:
    ...
    DesktopDragDropClientOzoneTest(const DesktopDragDropClientOzoneTest&) = delete;
    DesktopDragDropClientOzoneTest& operator=(const DesktopDragDropClientOzoneTest&) = delete;
    ...
    DragOperation StartDragAndDrop(int allowed_operations)
    {
        ...
        return client_->StartDragAndDrop(std::move(data), widget_->GetNativeWindow()->GetRootWindow(), widget_->GetNativeWindow(), gfx::Point(),
            allowed_operations, ui::mojom::DragEventSource::kMouse);
    }
    ...
    void SetUp() override
    {
        ...
        client_ = std::make_unique<DesktopDragDropClientOzone>(window, drag_handler);
...
TEST_F(DesktopDragDropClientOzoneTest, StartDrag)
{
    ...
    DragOperation operation = StartDragAndDrop(ui::DragDropTypes::DRAG_COPY | ui::DragDropTypes::DRAG_MOVE);
...
TEST_F(DesktopDragDropClientOzoneTest, ReceiveDrag)
{
    ...
    std::unique_ptr<ui::OSExchangeData> data = std::make_unique<ui::OSExchangeData>();
    data->SetString(sample_data);

    // Simulate that the drag enter/motion/drop/leave events happen with the
    // |suggested_operation|.
    int suggested_operation = ui::DragDropTypes::DRAG_COPY | ui::DragDropTypes::DRAG_MOVE;
    platform_window_->OnDragEnter(kStartDragLocation, std::move(data), suggested_operation);
    int updated_operation = platform_window_->OnDragMotion(kStartDragLocation, suggested_operation);
    ...
    std::optional<std::u16string> string_data = dragdrop_delegate_->received_data()->GetString();
...
{
    const int suggested_operation = ui::DragDropTypes::DRAG_COPY | ui::DragDropTypes::DRAG_MOVE;
    ...
    aura::client::SetDragDropDelegate(another_window, another_dragdrop_delegate.get());
    ...
    ui::WmDragHandler* drag_handler = ui::GetWmDragHandler(*(another_platform_window));
    auto another_client = std::make_unique<DesktopDragDropClientOzone>(another_window, drag_handler);
    SetWmDropHandler(another_platform_window.get(), another_client.get());

    std::unique_ptr<ui::OSExchangeData> another_data = std::make_unique<ui::OSExchangeData>();
    another_data->SetString(sample_data);
    another_platform_window->OnDragEnter(gfx::PointF(), std::move(another_data), suggested_operation);
...
class MockDataTransferPolicyController : public ui::DataTransferPolicyController {
public:
    MOCK_METHOD3(IsClipboardReadAllowed,
        bool(base::optional_ref<const ui::DataTransferEndpoint> data_src, base::optional_ref<const ui::DataTransferEndpoint> data_dst,
            const std::optional<size_t> size));
    MOCK_METHOD5(PasteIfAllowed,
        void(base::optional_ref<const ui::DataTransferEndpoint> data_src, base::optional_ref<const ui::DataTransferEndpoint> data_dst,
...
TEST_F(DesktopDragDropClientOzoneTest, DataLeakPreventionAllowDrop)
{
    ...
    EXPECT_CALL(dtp_controller, DropIfAllowed(testing::_, testing::_, testing::_, testing::_))
        .WillOnce([&](std::optional<ui::DataTransferEndpoint> data_src, std::optional<ui::DataTransferEndpoint> data_dst,
                      std::optional<std::vector<ui::FileInfo>> filenames, base::OnceClosure drop_cb) { std::move(drop_cb).Run(); });
