...
NativeWidget* CreateNativeWidget(const Widget::InitParams& params, internal::NativeWidgetDelegate* delegate)
{
...
// static
Widget::DisableActivationChangeHandlingType Widget::g_disable_activation_change_handling_ = Widget::DisableActivationChangeHandlingType::kNone;
...
public:
    DefaultWidgetDelegate()
    {
        ...
        RegisterDeleteDelegateCallback(base::BindOnce(&DefaultWidgetDelegate::Destroy, base::Unretained(this)));
    }
...
bool Widget::InitParams::CanActivate() const
{
    ...
    return type != InitParams::TYPE_CONTROL && type != InitParams::TYPE_POPUP && type != InitParams::TYPE_MENU && type != InitParams::TYPE_TOOLTIP
...
Widget::~Widget()
{
    ...
    if (ownership_ == InitParams::WIDGET_OWNS_NATIVE_WIDGET) {
        ...
        CHECK(!native_widget_) << "Destroying a widget with a live native widget. "
...
// static
Widget* Widget::CreateWindowWithParent(WidgetDelegate* delegate, gfx::NativeView parent, const gfx::Rect& bounds)
{
...
Widget* Widget::CreateWindowWithParent(std::unique_ptr<WidgetDelegate> delegate, gfx::NativeView parent, const gfx::Rect& bounds)
{
    return CreateWindowWithParent(delegate.release(), parent, bounds);
}

// static
Widget* Widget::CreateWindowWithContext(WidgetDelegate* delegate, gfx::NativeWindow context, const gfx::Rect& bounds)
{
    Widget::InitParams params(Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET);
    params.delegate = delegate;
...
// static
Widget* Widget::CreateWindowWithContext(std::unique_ptr<WidgetDelegate> delegate, gfx::NativeWindow context, const gfx::Rect& bounds)
{
...
Widget* Widget::GetWidgetForNativeWindow(gfx::NativeWindow native_window)
{
    ...
    internal::NativeWidgetPrivate* native_widget = internal::NativeWidgetPrivate::GetNativeWidgetForNativeWindow(native_window);
    return native_widget ? native_widget->GetWidget() : nullptr;
}
...
// static
void Widget::GetAllChildWidgets(gfx::NativeView native_view, Widgets* children)
{
    if (!native_view)
        return;
...
// static
void Widget::ReparentNativeView(gfx::NativeView native_view, gfx::NativeView new_parent)
{
    DCHECK(native_view);
    ...
    Widget* parent_widget = new_parent ? GetWidgetForNativeView(new_parent) : nullptr;
    if (child_widget)
...
{
    return ui::GetLocalizedContentsWidthForFontList(col_resource_id, ui::ResourceBundle::GetSharedInstance().GetFontListWithDelta(ui::kMessageFontSizeDelta));
}

// static
int Widget::GetLocalizedContentsHeight(int row_resource_id)
{
    return ui::GetLocalizedContentsHeightForFontList(row_resource_id, ui::ResourceBundle::GetSharedInstance().GetFontListWithDelta(ui::kMessageFontSizeDelta));
}

// static
gfx::Size Widget::GetLocalizedContentsSize(int col_resource_id, int row_resource_id)
{
    return gfx::Size(GetLocalizedContentsWidth(col_resource_id), GetLocalizedContentsHeight(row_resource_id));
}
...
void Widget::Init(InitParams params)
{
    ...
    DCHECK(!native_widget_initialized_) << "This widget has already been initialized";
    ...
    if (parent_) {
        parent_paint_as_active_subscription_
            = parent_->RegisterPaintAsActiveChangedCallback(base::BindRepeating(&Widget::OnParentShouldPaintAsActiveChanged, base::Unretained(this)));
    }
    ...
    if (params.opacity == views::Widget::InitParams::WindowOpacity::kInferred && params.type != views::Widget::InitParams::TYPE_WINDOW) {
    ...
    if (params.delegate) {
        ...
        widget_delegate_ = default_delegate.release()->AttachWidgetAndGetHandle(this);
    }
    ...
    params.activatable = can_activate ? InitParams::Activatable::kYes : InitParams::Activatable::kNo;
    ...
    internal::NativeWidgetPrivate* native_widget_raw_ptr = CreateNativeWidget(params, this)->AsNativeWidgetPrivate();
    ...
    if (RequiresNonClientView(type)) {
        non_client_view_ = new NonClientView(widget_delegate_->CreateClientView(this));
    ...
    internal::AnyWidgetObserverSingleton::GetInstance()->OnAnyWidgetInitialized(this);
}
...
{
    return native_widget_ ? native_widget_->GetNativeWindow() : gfx::NativeWindow();
}
...
{
    removals_observers_.Notify(&WidgetRemovalsObserver::OnWillRemoveView, this, view);
}
...
{
    return native_widget_ ? native_widget_->GetWindowBoundsInScreen() : gfx::Rect();
}

gfx::Rect Widget::GetClientAreaBoundsInScreen() const
{
    return native_widget_ ? native_widget_->GetClientAreaBoundsInScreen() : gfx::Rect();
}
...
Widget::MoveLoopResult Widget::RunMoveLoop(const gfx::Vector2d& drag_offset, MoveLoopSource source, MoveLoopEscapeBehavior escape_behavior)
{
    if (!native_widget_)
...
void Widget::CloseWithReason(ClosedReason closed_reason)
{
    ...
    if (non_client_view_ && non_client_view_->OnWindowCloseRequested() == CloseRequestResult::kCannotClose) {
        return;
    }
    ...
    if (closed_reason == ClosedReason::kLostFocus
        && (g_disable_activation_change_handling_ == DisableActivationChangeHandlingType::kIgnore
            || g_disable_activation_change_handling_ == DisableActivationChangeHandlingType::kIgnoreDeactivationOnly)) {
        return;
    }
...
void Widget::Show()
{
    ...
    TRACE_EVENT1("views", "Widget::Show", "layer", layer ? layer->name() : "none");
    ui::mojom::WindowShowState preferred_show_state = CanActivate() ? ui::mojom::WindowShowState::kNormal : ui::mojom::WindowShowState::kInactive;
    if (non_client_view_) {
        ...
        if (saved_show_state_ == ui::mojom::WindowShowState::kMaximized && !initial_restored_bounds_.IsEmpty() && !IsFullscreen()) {
            native_widget_->Show(ui::mojom::WindowShowState::kMaximized, initial_restored_bounds_);
        } else {
...
void Widget::ShowInactive()
{
    ...
    if (saved_show_state_ == ui::mojom::WindowShowState::kMaximized && !initial_restored_bounds_.IsEmpty()) {
...
{
    return native_widget_ ? native_widget_->GetZOrderLevel() : ui::ZOrderLevel::kNormal;
}
...
void Widget::SetFullscreen(bool fullscreen, int64_t target_display_id)
{
    ...
    if (IsFullscreen() == fullscreen && target_display_id == display::kInvalidDisplayId) {
        return;
    }
...
void Widget::SetCanAppearInExistingFullscreenSpaces(bool can_appear_in_existing_fullscreen_spaces)
{
    if (native_widget_) {
        native_widget_->SetCanAppearInExistingFullscreenSpaces(can_appear_in_existing_fullscreen_spaces);
    }
}
...
void Widget::SetAspectRatio(const gfx::SizeF& aspect_ratio)
{
    ...
    if (non_client_view() && non_client_view()->frame_view()) {
        excluded_margin = non_client_view()->bounds().size() - non_client_view()->frame_view()->GetBoundsForClientView().size();
    }
...
const ui::ThemeProvider* Widget::GetThemeProvider() const
{
    ...
    return (root_widget && root_widget != this) ? root_widget->GetThemeProvider() : nullptr;
}
...
ui::InputMethod* Widget::GetInputMethod()
{
    if (is_top_level() && native_widget_) {
        ...
        return (toplevel && toplevel != this) ? toplevel->GetInputMethod() : nullptr;
    }
}
...
void Widget::RunShellDrag(View* view, std::unique_ptr<ui::OSExchangeData> data, const gfx::Point& location, int operation, ui::mojom::DragEventSource source)
{
    if (view) {
...
void Widget::OnRootViewLayoutInvalidated()
{
    ...
    if (is_autosized() && IsNativeWidgetInitialized() && GetContentsView() && widget_delegate_) {
        if (gfx::Rect desired_bounds = widget_delegate_->GetDesiredWidgetBounds(); !desired_bounds.IsEmpty() && desired_bounds != GetWindowBoundsInScreen()) {
...
{
    return native_widget_ ? native_widget_->GetNativeWindowProperty(name) : nullptr;
}
...
void Widget::UpdateWindowTitle()
{
    ...
    if (!title_changed) {
        ...
        bool title_visibility_changed = non_client_view()->IsWindowTitleVisible() != widget_delegate_->ShouldShowWindowTitle();
...
void Widget::UpdateWindowIcon()
{
    ...
    native_widget_->SetWindowIcons(
        widget_delegate_->GetWindowIcon().Rasterize(GetColorProvider()), widget_delegate_->GetWindowAppIcon().Rasterize(GetColorProvider()));
}
...
void Widget::DeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor)
{
    root_view_->DeviceScaleFactorChanged(old_device_scale_factor, new_device_scale_factor);
}
...
{
    return native_widget_ ? native_widget_->ShouldWindowContentsBeTransparent() : false;
}
...
{
    return native_widget_ ? native_widget_->GetWorkAreaBoundsInScreen() : gfx::Rect();
}
...
void Widget::SynthesizeMouseMoveEvent()
{
    ...
    ui::MouseEvent mouse_event(ui::EventType::kMouseMoved, mouse_location, mouse_location, ui::EventTimeForNow(), ui::EF_IS_SYNTHESIZED, 0);
...
base::CallbackListSubscription Widget::RegisterPaintAsActiveChangedCallback(PaintAsActiveCallbackList::CallbackType callback)
{
...
std::unique_ptr<Widget::PaintAsActiveLock> Widget::LockPaintAsActive()
{
    ...
    return std::make_unique<PaintAsActiveLockImpl>(weak_ptr_factory_.GetWeakPtr());
}
...
bool Widget::ShouldPaintAsActive() const
{
    ...
    return native_widget_active_ || paint_as_active_refcount_ || (parent() && parent()->ShouldPaintAsActive());
}
...
bool Widget::OnNativeWidgetActivationChanged(bool active)
{
    ...
    if (active) {
        base::AutoReset<bool> is_traversing_widget_tree(&is_traversing_widget_tree_, true);
        Widget* root = nullptr;
        for (Widget* widget = this; widget; widget = widget->parent()) {
            widget->observers_.Notify(&WidgetObserver::OnWidgetTreeActivated, widget, this);
            root = widget;
        }
#if BUILDFLAG(IS_WIN)
...
{
    return (g_disable_activation_change_handling_ != DisableActivationChangeHandlingType::kIgnore)
        && (g_disable_activation_change_handling_ != DisableActivationChangeHandlingType::kIgnoreDeactivationOnly || active);
}
...
void Widget::OnNativeWidgetMove()
{
    ...
    observers_.Notify(&WidgetObserver::OnWidgetBoundsChanged, this, GetWindowBoundsInScreen());
}
...
void Widget::OnKeyEvent(ui::KeyEvent* event)
{
    ...
    if (!event->handled() && GetFocusManager() && !GetFocusManager()->OnKeyEvent(*event)) {
...
void Widget::OnMouseEvent(ui::MouseEvent* event)
{
    ...
    case ui::EventType::kMousePressed: {
        ...
        gfx::NativeView current_capture = internal::NativeWidgetPrivate::GetGlobalCapture(native_widget_->GetNativeView());
        ...
        if (root_view && root_view->OnMousePressed(*event) && widget_deletion_observer.IsWidgetAlive() && IsVisible() && native_widget_->IsMouseButtonDown()
            && current_capture == internal::NativeWidgetPrivate::GetGlobalCapture(native_widget_->GetNativeView())) {
            is_mouse_button_pressed_ = true;
            if (!native_widget_->HasCapture())
                native_widget_->SetCapture();
            event->SetHandled();
    ...
    case ui::EventType::kMouseReleased:
        ...
        if ((event->flags() & ui::EF_IS_NON_CLIENT) == 0 &&
            ...
            // process it.
            (event->flags() & (ui::EF_LEFT_MOUSE_BUTTON | ui::EF_MIDDLE_MOUSE_BUTTON | ui::EF_RIGHT_MOUSE_BUTTON)) != 0) {
            event->SetHandled();
        }
        return;
...
bool Widget::SetInitialFocus(ui::mojom::WindowShowState show_state)
{
    ...
    if (!focus_on_creation_ || show_state == ui::mojom::WindowShowState::kInactive || show_state == ui::mojom::WindowShowState::kMinimized) {
...
bool Widget::ShouldDescendIntoChildForEventHandling(ui::Layer* root_layer, gfx::NativeView child, ui::Layer* child_layer, const gfx::Point& location)
{
    if (widget_delegate_ && !widget_delegate_->ShouldDescendIntoChildForEventHandling(child, location)) {
        return false;
    }
    ...
    auto child_layer_iter = base::ranges::find(root_layer->children(), child_layer);
...
void Widget::LayoutRootViewIfNecessary()
{
    if (root_view_ && root_view_->needs_layout()) {
        ...
        TRACE_EVENT1("ui", "Widget::LayoutRootViewIfNecessary", "widget name", GetName());
        root_view_->LayoutImmediately();
    }
}
...
void Widget::SetColorModeOverride(std::optional<ui::ColorProviderKey::ColorMode> color_mode)
{
...
ui::ColorProviderKey Widget::GetColorProviderKey() const
{
    ...
    ui::ColorProviderKey key = parent_ ? parent_->GetColorProviderKey() : GetNativeTheme()->GetColorProviderKey(GetCustomTheme());
...
{
    return ui::ColorProviderManager::Get().GetColorProviderFor(GetColorProviderKey());
}

ui::RendererColorMap Widget::GetRendererColorMap(ui::ColorProviderKey::ColorMode color_mode, ui::ColorProviderKey::ForcedColors forced_colors) const
{
    auto key = GetColorProviderKey();
    key.color_mode = color_mode;
    key.forced_colors = forced_colors;
    ui::ColorProvider* color_provider = ui::ColorProviderManager::Get().GetColorProviderFor(key);
...
void Widget::SaveWindowPlacement()
{
    ...
    // bail.
    if (!widget_delegate_ || !widget_delegate_->ShouldSaveWindowPlacement() || !native_widget_)
        return;
...
void Widget::SetParent(Widget* parent)
{
    ...
    if (parent) {
        if (has_lock_on_parent || native_widget_active_)
            parent_paint_as_active_lock_ = parent->LockPaintAsActive();
        parent_paint_as_active_subscription_
            = parent->RegisterPaintAsActiveChangedCallback(base::BindRepeating(&Widget::OnParentShouldPaintAsActiveChanged, base::Unretained(this)));
    }
...
bool Widget::GetSavedWindowPlacement(gfx::Rect* bounds, ui::mojom::WindowShowState* show_state)
{
...
DEFINE_ENUM_CONVERTERS(ui::ZOrderLevel, { ui::ZOrderLevel::kNormal, u"kNormal" }, { ui::ZOrderLevel::kFloatingWindow, u"kFloatingWindow" },
    { ui::ZOrderLevel::kFloatingUIElement, u"kFloatingUIElement" }, { ui::ZOrderLevel::kSecuritySurface, u"kSecuritySurface" })
