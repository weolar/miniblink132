...
public:
    ...
    internal::NativeWidgetPrivate* get() const
    {
        return native_widget_;
    }
...
public:
    ...
    internal::NativeWidgetPrivate* CreateNativeWidgetOfType(Widget::InitParams::Type type)
    {
...
{
    ScopedTestWidget widget(CreateNativeWidget());
    EXPECT_EQ(widget.get(), internal::NativeWidgetPrivate::GetNativeWidgetForNativeView(widget->GetWidget()->GetNativeView()));
}
...
{
    ScopedTestWidget widget(CreateNativeWidget());
    EXPECT_EQ(widget.get(), internal::NativeWidgetPrivate::GetTopLevelNativeWidget(widget->GetWidget()->GetNativeView()));
}
...
TEST_F(NativeWidgetTest, GetTopLevelNativeWidget2)
{
    internal::NativeWidgetPrivate* child_widget = CreateNativeSubWidget();
    {
        ...
        NativeViewHost* child_host = toplevel_widget->GetWidget()->SetContentsView(std::make_unique<NativeViewHost>());

        // |child_host| hosts |child_widget|'s NativeView.
        child_host->Attach(child_widget->GetWidget()->GetNativeView());

        EXPECT_EQ(toplevel_widget.get(), internal::NativeWidgetPrivate::GetTopLevelNativeWidget(child_widget->GetWidget()->GetNativeView()));
    }
...
TEST_F(NativeWidgetTest, GetTopLevelNativeWidget3)
{
    ...
    Widget::ReparentNativeView(grandchild_widget->GetNativeView(), child_widget->GetNativeView());
    Widget::ReparentNativeView(child_widget->GetNativeView(), root_widget->GetNativeView());

    EXPECT_EQ(child_widget.get(), internal::NativeWidgetPrivate::GetTopLevelNativeWidget(grandchild_widget->GetWidget()->GetNativeView()));
}
