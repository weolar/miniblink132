...
DEFINE_UI_CLASS_PROPERTY_KEY(internal::NativeWidgetPrivate*, kNativeWidgetPrivateKey, nullptr)
...
void SetIcon(aura::Window* window, const aura::WindowProperty<gfx::ImageSkia*>* key, const gfx::ImageSkia& value)
{
    if (value.isNull())
...
bool FindLayersInOrder(const std::vector<raw_ptr<ui::Layer, VectorExperimental>>& children, const ui::Layer** first, const ui::Layer** second)
{
...
void ReparentAuraWindow(aura::Window* window, aura::Window* parent)
{
    if (parent) {
        ...
        aura::client::ParentWindowWithContext(window, root_window, root_window->GetBoundsInScreen(), display::kInvalidDisplayId);
    }
}
...
// static
void NativeWidgetAura::RegisterNativeWidgetForWindow(internal::NativeWidgetPrivate* native_widget, aura::Window* window)
{
    window->SetProperty(kNativeWidgetPrivateKey, native_widget);
}

// static
void NativeWidgetAura::AssignIconToAuraWindow(aura::Window* window, const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
    if (window) {
...
// static
void NativeWidgetAura::SetShadowElevationFromInitParams(aura::Window* window, const Widget::InitParams& params)
{
    if (params.shadow_type == Widget::InitParams::ShadowType::kNone) {
        wm::SetShadowElevation(window, wm::kShadowElevationNone);
    } else if (params.shadow_type == Widget::InitParams::ShadowType::kDrop && params.shadow_elevation) {
        wm::SetShadowElevation(window, *params.shadow_elevation);
    }
}

// static
void NativeWidgetAura::SetResizeBehaviorFromDelegate(WidgetDelegate* delegate, aura::Window* window)
{
    if (!window)
        return;
...
void NativeWidgetAura::InitNativeWidget(Widget::InitParams params)
{
    ...
    window_->AcquireAllPropertiesFrom(std::move(params.init_properties_container));

    RegisterNativeWidgetForWindow(this, window_);
    window_->SetType(GetAuraWindowTypeForWidgetType(params.type));
    if (params.corner_radius) {
        window_->SetProperty(aura::client::kWindowCornerRadiusKey, *params.corner_radius);
    }
    ...
    if (params.visible_on_all_workspaces) {
        window_->SetProperty(aura::client::kWindowWorkspaceKey, aura::client::kWindowWorkspaceVisibleOnAllWorkspaces);
    } else if (const base::Uuid& desk_uuid = base::Uuid::ParseLowercase(params.workspace); desk_uuid.is_valid()) {
        window_->SetProperty(aura::client::kDeskUuidKey, desk_uuid.AsLowercaseString());
    ...
    window_->SetTransparent(params.opacity == Widget::InitParams::WindowOpacity::kTranslucent);
    ...
    if (!params.child) {
        ...
        if (parent && parent->GetType() != aura::client::WINDOW_TYPE_UNKNOWN) {
            ...
            if (params.type == Widget::InitParams::TYPE_BUBBLE) {
                wm::TransientWindowManager::GetOrCreate(window_)->set_parent_controls_visibility(true);
            }
        }
        ...
        if (parent_or_context && window_bounds == gfx::Rect()) {
            ...
            gfx::Rect bounds = display::Screen::GetScreen()->GetDisplayNearestWindow(parent_or_context).bounds();
    ...
    if (parent) {
        ...
        aura::client::ParentWindowWithContext(window_, context->GetRootWindow(), window_bounds, target_display.value_or(display::kInvalidDisplayId));
    }
    ...
    window_->SetEventTargetingPolicy(params.accept_events ? aura::EventTargetingPolicy::kTargetAndDescendants : aura::EventTargetingPolicy::kNone);
    ...
    if (params.type == Widget::InitParams::TYPE_WINDOW) {
        focus_manager_event_handler_ = std::make_unique<FocusManagerEventHandler>(GetWidget(), window_);
    }

    wm::SetActivationDelegate(window_, this);

    window_reorderer_ = std::make_unique<WindowReorderer>(window_, GetWidget()->GetRootView());
}
...
std::unique_ptr<NonClientFrameView> NativeWidgetAura::CreateNonClientFrameView()
{
    return nullptr;
}
...
void NativeWidgetAura::CenterWindow(const gfx::Size& size)
{
    ...
    gfx::Rect work_area = display::Screen::GetScreen()->GetDisplayNearestWindow(window_).work_area();
    ...
    if (screen_position_client) {
        ...
        screen_position_client->ConvertPointFromScreen(window_->GetRootWindow(), &origin);
    ...
    gfx::Rect window_bounds(parent_bounds.x() + (parent_bounds.width() - size.width()) / 2, parent_bounds.y() + (parent_bounds.height() - size.height()) / 2,
    ...
    aura::Window::ConvertPointToTarget(window_->GetRootWindow(), window_->parent(), &origin);
    window_bounds.set_origin(origin);
    window_->SetBounds(window_bounds);
}

void NativeWidgetAura::GetWindowPlacement(gfx::Rect* bounds, ui::mojom::WindowShowState* show_state) const
{
    // The interface specifies returning restored bounds, not current bounds.
    *bounds = GetRestoredBounds();
    *show_state = window_ ? window_->GetProperty(aura::client::kShowStateKey) : ui::mojom::WindowShowState::kDefault;
}
...
void NativeWidgetAura::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
...
gfx::Rect NativeWidgetAura::GetRestoredBounds() const
{
    ...
    auto* screen_position_client = aura::client::GetScreenPositionClient(window_->GetRootWindow());
    if (screen_position_client) {
        // |window_|'s bounds are in parent's coordinate system so use that when
        // converting.
        gfx::Rect bounds = window_->bounds();
        gfx::Point origin = bounds.origin();
        screen_position_client->ConvertPointToScreenIgnoringTransforms(window_->parent(), &origin);
...
std::string NativeWidgetAura::GetWorkspace() const
{
    ...
    if (desk_index == aura::client::kWindowWorkspaceUnassignedWorkspace || desk_index == aura::client::kWindowWorkspaceVisibleOnAllWorkspaces) {
        return std::string();
    }
...
void NativeWidgetAura::SetBoundsInternal(const gfx::Rect& bounds, std::optional<int64_t> display_id)
{
    display::Display dst_display;
    auto* screen = display::Screen::GetScreen();
    // TODO(crbug.com/40281188): Call SetBoundsInScreen directly.
    if (!display_id || !screen->GetDisplayWithDisplayId(display_id.value(), &dst_display)) {
...
void NativeWidgetAura::SetBoundsConstrained(const gfx::Rect& bounds)
{
    ...
    if (window_->parent()) {
        if (window_->parent()->GetProperty(wm::kUsesScreenCoordinatesKey)) {
            new_bounds = NativeWidgetPrivate::ConstrainBoundsToDisplayWorkArea(new_bounds);
        } else {
...
bool NativeWidgetAura::IsStackedAbove(gfx::NativeView native_view)
{
    ...
    if (GetNativeWindow()->GetRootWindow() != native_view->GetRootWindow()) {
        return GetTopLevelWidget()->IsStackedAbove(native_view->GetToplevelWindow());
    }

    const ui::Layer* first = native_view->layer(); // below
    const ui::Layer* second = GetWidget()->GetLayer(); // above
    return FindLayersInOrder(GetNativeWindow()->GetRootWindow()->layer()->children(), &first, &second);
}
...
void NativeWidgetAura::Close()
{
    ...
    DCHECK(window_ || ownership_ == Widget::InitParams::WIDGET_OWNS_NATIVE_WIDGET || ownership_ == Widget::InitParams::CLIENT_OWNS_WIDGET);
    if (window_) {
        Hide();
        window_->SetProperty(aura::client::kModalKey, ui::mojom::ModalType::kNone);
    }

    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&NativeWidgetAura::CloseNow, weak_factory.GetWeakPtr()));
}
...
void NativeWidgetAura::Show(ui::mojom::WindowShowState show_state, const gfx::Rect& restore_bounds)
{
    if (!window_)
        return;

    if ((show_state == ui::mojom::WindowShowState::kMaximized || show_state == ui::mojom::WindowShowState::kMinimized) && !restore_bounds.IsEmpty()) {
        SetRestoreBounds(window_, restore_bounds);
    }
    if (show_state == ui::mojom::WindowShowState::kMaximized || show_state == ui::mojom::WindowShowState::kFullscreen) {
    ...
    if (delegate_->CanActivate()) {
        ...
        SetInitialFocus(IsActive() ? show_state : ui::mojom::WindowShowState::kInactive);
    }
...
void NativeWidgetAura::SetVisibleOnAllWorkspaces(bool always_visible)
{
    window_->SetProperty(aura::client::kWindowWorkspaceKey,
        always_visible ? aura::client::kWindowWorkspaceVisibleOnAllWorkspaces : aura::client::kWindowWorkspaceUnassignedWorkspace);
}

bool NativeWidgetAura::IsVisibleOnAllWorkspaces() const
{
    return window_ && window_->GetProperty(aura::client::kWindowWorkspaceKey) == aura::client::kWindowWorkspaceVisibleOnAllWorkspaces;
}

void NativeWidgetAura::Maximize()
{
    if (window_)
        window_->SetProperty(aura::client::kShowStateKey, ui::mojom::WindowShowState::kMaximized);
}

void NativeWidgetAura::Minimize()
{
    if (window_)
        window_->SetProperty(aura::client::kShowStateKey, ui::mojom::WindowShowState::kMinimized);
}

bool NativeWidgetAura::IsMaximized() const
{
    return window_ && window_->GetProperty(aura::client::kShowStateKey) == ui::mojom::WindowShowState::kMaximized;
}

bool NativeWidgetAura::IsMinimized() const
{
    return window_ && window_->GetProperty(aura::client::kShowStateKey) == ui::mojom::WindowShowState::kMinimized;
}
...
void NativeWidgetAura::SetFullscreen(bool fullscreen, int64_t target_display_id)
{
    if (!window_) {
        return;
    }
...
{
    return window_ && window_->GetProperty(aura::client::kShowStateKey) == ui::mojom::WindowShowState::kFullscreen;
}

void NativeWidgetAura::SetCanAppearInExistingFullscreenSpaces(bool can_appear_in_existing_fullscreen_spaces)
{
}
...
void NativeWidgetAura::SetAspectRatio(const gfx::SizeF& aspect_ratio, const gfx::Size& excluded_margin)
{
...
void NativeWidgetAura::RunShellDrag(std::unique_ptr<ui::OSExchangeData> data, const gfx::Point& location, int operation, ui::mojom::DragEventSource source)
{
    if (window_)
...
gfx::Rect NativeWidgetAura::GetWorkAreaBoundsInScreen() const
{
    if (!window_)
        return gfx::Rect();
    return display::Screen::GetScreen()->GetDisplayNearestWindow(window_).work_area();
}
    ...
    const gfx::Vector2d& drag_offset, Widget::MoveLoopSource source, Widget::MoveLoopEscapeBehavior escape_behavior)
{
    ...
    wm::WindowMoveClient* move_client = wm::GetWindowMoveClient(window_->GetRootWindow());
    if (!move_client)
        return Widget::MoveLoopResult::kCanceled;

    SetCapture();
    wm::WindowMoveSource window_move_source = source == Widget::MoveLoopSource::kMouse ? wm::WINDOW_MOVE_SOURCE_MOUSE : wm::WINDOW_MOVE_SOURCE_TOUCH;
    if (move_client->RunMoveLoop(window_, drag_offset, window_move_source) == wm::MOVE_SUCCESSFUL) {
...
void NativeWidgetAura::SetVisibilityAnimationDuration(const base::TimeDelta& duration)
{
    wm::SetWindowVisibilityAnimationDuration(window_, duration);
}

void NativeWidgetAura::SetVisibilityAnimationTransition(Widget::VisibilityTransition transition)
{
...
void NativeWidgetAura::OnBoundsChanged(const gfx::Rect& old_bounds, const gfx::Rect& new_bounds)
{
    ...
    if (delegate_ && (old_bounds.origin() != new_bounds.origin() || (old_bounds == gfx::Rect(0, 0, 0, 0) && !new_bounds.IsEmpty()))) {
...
bool NativeWidgetAura::ShouldDescendIntoChildForEventHandling(aura::Window* child, const gfx::Point& location)
{
    return delegate_ ? delegate_->ShouldDescendIntoChildForEventHandling(window_->layer(), child, child->layer(), location) : false;
}
...
void NativeWidgetAura::OnDeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor)
{
...
void NativeWidgetAura::OnWindowPropertyChanged(aura::Window* window, const void* key, intptr_t old)
{
...
void NativeWidgetAura::OnWindowRemovingFromRootWindow(aura::Window* window, aura::Window* new_root)
{
    if (delegate_)
...
void NativeWidgetAura::OnWindowActivated(wm::ActivationChangeObserver::ActivationReason, aura::Window* gained_active, aura::Window* lost_active)
{
...
void NativeWidgetAura::OnWindowFocused(aura::Window* gained_focus, aura::Window* lost_focus)
{
...
aura::client::DragUpdateInfo NativeWidgetAura::OnDragUpdated(const ui::DropTargetEvent& event)
{
    DCHECK(drop_helper_.get() != nullptr);
    last_drop_operation_ = drop_helper_->OnDragOver(event.data(), event.location(), event.source_operations());
...
aura::client::DragDropDelegate::DropCallback NativeWidgetAura::GetDropCallback(const ui::DropTargetEvent& event)
{
    DCHECK(drop_helper_);
    return drop_helper_->GetDropCallback(event.data(), event.location(), last_drop_operation_);
}
...
// static
NativeWidgetPrivate* NativeWidgetPrivate::CreateNativeWidget(internal::NativeWidgetDelegate* delegate)
{
    return new NativeWidgetAura(delegate);
}

// static
NativeWidgetPrivate* NativeWidgetPrivate::GetNativeWidgetForNativeView(gfx::NativeView native_view)
{
    return native_view->GetProperty(kNativeWidgetPrivateKey);
}

// static
NativeWidgetPrivate* NativeWidgetPrivate::GetNativeWidgetForNativeWindow(gfx::NativeWindow native_window)
{
    return native_window->GetProperty(kNativeWidgetPrivateKey);
}

// static
NativeWidgetPrivate* NativeWidgetPrivate::GetTopLevelNativeWidget(gfx::NativeView native_view)
{
...
// static
void NativeWidgetPrivate::GetAllChildWidgets(gfx::NativeView native_view, Widget::Widgets* children)
{
    {
        // Code expects widget for |native_view| to be added to |children|.
        NativeWidgetPrivate* native_widget = static_cast<NativeWidgetPrivate*>(GetNativeWidgetForNativeView(native_view));
...
// static
void NativeWidgetPrivate::GetAllOwnedWidgets(gfx::NativeView native_view, Widget::Widgets* owned)
{
    // Add all owned widgets.
    for (aura::Window* transient_child : wm::GetTransientChildren(native_view)) {
        NativeWidgetPrivate* native_widget = static_cast<NativeWidgetPrivate*>(GetNativeWidgetForNativeView(transient_child));
        if (native_widget && native_widget->GetWidget())
...
// static
void NativeWidgetPrivate::ReparentNativeView(gfx::NativeView native_view, gfx::NativeView new_parent)
{
...
// static
gfx::NativeView NativeWidgetPrivate::GetGlobalCapture(gfx::NativeView native_view)
{
    aura::client::CaptureClient* capture_client = aura::client::GetCaptureClient(native_view->GetRootWindow());
    if (!capture_client)
        return nullptr;
