...
TEST_F(AnyWidgetObserverTest, ObservesInitialize)
{
    ...
    observer.set_initialized_callback(base::BindLambdaForTesting([&](Widget*) { initialized = true; }));

    EXPECT_FALSE(initialized);
    std::unique_ptr<Widget> w0 = base::WrapUnique(WidgetTest::CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
...
TEST_F(AnyWidgetObserverTest, ObservesClose)
{
    ...
    observer.set_closing_callback(base::BindLambdaForTesting([&](Widget*) { closing = true; }));
...
TEST_F(AnyWidgetObserverTest, ObservesShow)
{
    ...
    observer.set_shown_callback(base::BindLambdaForTesting([&](Widget*) { shown = true; }));
...
TEST_F(AnyWidgetObserverTest, ObservesHide)
{
    ...
    observer.set_hidden_callback(base::BindLambdaForTesting([&](Widget*) { hidden = true; }));
...
public:
    ...
    std::unique_ptr<views::Widget> CreateNamedWidget(const std::string& name)
    {
        ...
        params.native_widget = views::test::CreatePlatformNativeWidgetImpl(widget.get(), views::test::kStubCapture, nullptr);
        params.name = name;
...
{
    views::NamedWidgetShownWaiter waiter(views::test::AnyWidgetTestPasskey {}, "TestWidget");

    std::unique_ptr<Widget> w0 = CreateNamedWidget("TestWidget");
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce([](views::Widget* widget) { widget->Show(); }, base::Unretained(w0.get())));
