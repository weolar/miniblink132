...
public:
    using ClientViewFactory = base::OnceCallback<std::unique_ptr<ClientView>(Widget*)>;
    ...
    virtual void SaveWindowPlacement(const gfx::Rect& bounds, ui::mojom::WindowShowState show_state);

    // Retrieves the window's bounds and "show" states.
    // This behavior can be overridden to provide additional functionality.
    virtual bool GetSavedWindowPlacement(const Widget* widget, gfx::Rect* bounds, ui::mojom::WindowShowState* show_state) const;
    ...
    virtual std::unique_ptr<NonClientFrameView> CreateNonClientFrameView(Widget* widget);
    ...
    template <typename T> T* SetContentsView(std::unique_ptr<T> contents)
    {
    ...
    bool focus_traverses_out() const
    {
        return params_.focus_traverses_out;
    }
    ...
    // set to true.
    virtual bool RotatePaneFocusFromView(views::View* focused_view, bool forward, bool enable_wrapping);

    bool owned_by_widget() const
    {
        return owned_by_widget_;
    }

    void set_internal_name(std::string name)
    {
        params_.internal_name = name;
    }
    std::string internal_name() const
    {
        return params_.internal_name;
    }
    ...
    void set_desired_bounds_delegate(base::RepeatingCallback<gfx::Rect()> desired_bounds_delegate)
    {
