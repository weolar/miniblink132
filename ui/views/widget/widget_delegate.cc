...
std::unique_ptr<ClientView> CreateDefaultClientView(WidgetDelegate* delegate, Widget* widget)
{
    return std::make_unique<ClientView>(widget, delegate->TransferOwnershipOfContentsView());
}
...
WidgetDelegate::WidgetDelegate()
    ...
    , client_view_factory_(base::BindOnce(&CreateDefaultClientView, base::Unretained(this)))
...
WidgetDelegate::~WidgetDelegate()
{
    ...
    if (!contents_view_taken_ && default_contents_view_ && !default_contents_view_->parent()) {
...
{
    return params_.accessible_title.empty() ? GetWindowTitle() : params_.accessible_title;
}
...
bool WidgetDelegate::RotatePaneFocusFromView(View* focused_view, bool forward, bool enable_wrapping)
{
    ...
    if (focused_view) {
        const auto i = base::ranges::find_if(panes, [focused_view](const auto* pane) { return pane && pane->Contains(focused_view); });
        if (i != panes.cend()) {
...
void WidgetDelegate::SaveWindowPlacement(const gfx::Rect& bounds, ui::mojom::WindowShowState show_state)
{
    std::string window_name = GetWindowName();
    if (!window_name.empty()) {
        ViewsDelegate::GetInstance()->SaveWindowPlacement(GetWidget(), window_name, bounds, show_state);
    }
}
...
bool WidgetDelegate::GetSavedWindowPlacement(const Widget* widget, gfx::Rect* bounds, ui::mojom::WindowShowState* show_state) const
{
    std::string window_name = GetWindowName();
    if (window_name.empty() || !ViewsDelegate::GetInstance()->GetSavedWindowPlacement(widget, window_name, bounds, show_state))
        return false;
    // Try to find a display intersecting the saved bounds.
    const auto& display = display::Screen::GetScreen()->GetDisplayMatching(*bounds);
    return display.bounds().Intersects(*bounds);
}

base::WeakPtr<WidgetDelegate> WidgetDelegate::AttachWidgetAndGetHandle(Widget* widget)
{
...
void WidgetDelegate::DeleteDelegate()
{
    ...
    if (weak_this && !owned_by_widget && widget_ && widget_->ownership() == Widget::InitParams::CLIENT_OWNS_WIDGET) {
...
std::unique_ptr<NonClientFrameView> WidgetDelegate::CreateNonClientFrameView(Widget* widget)
{
    return nullptr;
}
...
bool WidgetDelegate::ShouldDescendIntoChildForEventHandling(gfx::NativeView child, const gfx::Point& location)
{
    return true;
}
...
{
    bool old_can_fullscreen = std::exchange(params_.can_fullscreen, can_fullscreen);
...
void WidgetDelegate::SetEnableArrowKeyTraversal(bool enable_arrow_key_traversal)
{
...
void WidgetDelegate::RegisterWidgetInitializedCallback(base::OnceClosure callback)
{
    DCHECK(widget_initialized_callbacks_);
    widget_initialized_callbacks_->emplace_back(std::move(callback));
}

void WidgetDelegate::RegisterWindowWillCloseCallback(base::OnceClosure callback)
{
...
void WidgetDelegate::RegisterDeleteDelegateCallback(base::OnceClosure callback)
{
...
gfx::Rect WidgetDelegate::GetDesiredWidgetBounds()
{
    ...
    return gfx::Rect(GetWidget()->GetWindowBoundsInScreen().origin(), GetWidget()->GetContentsView()->GetPreferredSize({}));
}
