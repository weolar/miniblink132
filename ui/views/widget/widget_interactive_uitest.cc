...
public:
    DragView(base::OnceClosure on_drag_enter, base::OnceClosure on_drag_exit, base::OnceClosure on_capture_lost, base::OnceClosure on_mouse_exit)
...
private:
    // DragController:
    bool CanStartDragForView(views::View* sender, const gfx::Point& press_pt, const gfx::Point& current_pt) override
    {
        EXPECT_EQ(sender, this);
        return true;
    }

    int GetDragOperationsForView(views::View* sender, const gfx::Point& press_pt) override
    {
        EXPECT_EQ(sender, this);
        return ui::DragDropTypes::DRAG_COPY;
    }

    void WriteDragDataForView(views::View* sender, const gfx::Point& press_pt, ui::OSExchangeData* data) override
    {
        data->provider().SetString(u"test");
    }

    // View:
    bool GetDropFormats(int* formats, std::set<ui::ClipboardFormatType>* format_types) override
    {
    ...
    base::OnceClosure on_drag_enter_, on_drag_exit_, on_capture_lost_, on_mouse_exit_;
};
...
ui::mojom::WindowShowState GetWidgetShowState(const Widget* widget)
{
    ...
    return widget->IsActive() ? ui::mojom::WindowShowState::kNormal : ui::mojom::WindowShowState::kInactive;
}
...
{
    ::SetActiveWindow(widget->GetNativeWindow()->GetHost()->GetAcceleratedWidget());
}
#endif
...
TEST_F(DesktopWidgetTestInteractive, DesktopNativeWidgetAuraActivationAndFocusTest)
{
    ...
    std::unique_ptr<Widget> widget1 = base::WrapUnique(CreateTopLevelNativeWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    ...
    wm::ActivationClient* activation_client1 = wm::GetActivationClient(root_window1);
    ...
    std::unique_ptr<Widget> widget2 = base::WrapUnique(CreateTopLevelNativeWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    ...
    wm::ActivationClient* activation_client2 = wm::GetActivationClient(root_window2);
    EXPECT_TRUE(activation_client2 != nullptr);
    EXPECT_EQ(activation_client2->GetActiveWindow(), widget2->GetNativeView());
    EXPECT_EQ(activation_client1->GetActiveWindow(), reinterpret_cast<aura::Window*>(NULL));

    // Now set focus back to widget 1 and expect the active window to be its
    // window.
    focusable_view1->RequestFocus();
    ActivatePlatformWindow(widget1.get());
    EXPECT_EQ(activation_client2->GetActiveWindow(), reinterpret_cast<aura::Window*>(NULL));
    EXPECT_EQ(activation_client1->GetActiveWindow(), widget1->GetNativeView());
}
...
{
    std::unique_ptr<Widget> widget = base::WrapUnique(CreateTopLevelNativeWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    View* focusable_view = widget->GetContentsView()->AddChildView(std::make_unique<View>());
    ...
    // Show a bubble.
    auto owned_bubble_delegate_view = std::make_unique<views::BubbleDialogDelegateView>(focusable_view, BubbleBorder::NONE);
    owned_bubble_delegate_view->SetFocusBehavior(View::FocusBehavior::ALWAYS);
    BubbleDialogDelegateView* bubble_delegate_view = owned_bubble_delegate_view.get();
    BubbleDialogDelegateView::CreateBubble(std::move(owned_bubble_delegate_view))->Show();
...
public:
    explicit TouchEventHandler(Widget* widget)
        : widget_(widget)
    {
    ...
    {
        widget_->GetNativeWindow()->GetHost()->window()->RemovePreTargetHandler(this);
    }
    ...
    static void __stdcall AsyncActivateMouse(HWND hwnd, UINT msg, ULONG_PTR data, LRESULT result)
    {
    ...
    {
        SendMessageCallback(
            widget_->GetNativeWindow()->GetHost()->GetAcceleratedWidget(), WM_MOUSEACTIVATE, 0, 0, AsyncActivateMouse, reinterpret_cast<ULONG_PTR>(this));
    }
...
TEST_F(DesktopWidgetTestInteractive, DISABLED_TouchNoActivateWindow)
{
...

    {
        ...
        ASSERT_TRUE(ui_controls::SendTouchEvents(ui_controls::kTouchPress, 1, 100, 100));
...
{
    std::unique_ptr<Widget> toplevel = base::WrapUnique(CreateTopLevelFramelessPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    ...
    ui::MouseEvent moved_out(ui::EventType::kMouseMoved, p1, p1, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ...
    gfx::Point p2(95, 95);
    ui::MouseEvent moved_over(ui::EventType::kMouseMoved, p2, p2, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ...
    gfx::Point p3(102, 95);
    ui::MouseEvent moved_resizer(ui::EventType::kMouseMoved, p3, p3, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
...
{
    std::unique_ptr<Widget> widget1 = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    View* view1 = widget1->GetContentsView()->AddChildView(std::make_unique<View>());
    view1->SetFocusBehavior(View::FocusBehavior::ALWAYS);

    std::unique_ptr<Widget> widget2 = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
...
{
    std::unique_ptr<Widget> w1 = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    std::unique_ptr<Widget> w2 = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    std::unique_ptr<Widget> w3 = base::WrapUnique(CreateTopLevelPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
...
TEST_F(WidgetTestInteractive, ChildWidgetStackAbove)
{
    ...
    auto children = std::to_array<Widget*>({ CreateChildPlatformWidget(toplevel->GetNativeView()), CreateChildPlatformWidget(toplevel->GetNativeView()),
    ...
    do {
        ...
        for (size_t i = 0; i < order.size(); i++) {
            for (size_t j = 0; j < order.size(); j++) {
                if (i < j) {
                    EXPECT_FALSE(IsWindowStackedAbove(children[order[i]], children[order[j]]));
                } else if (i > j) {
                    EXPECT_TRUE(IsWindowStackedAbove(children[order[i]], children[order[j]]));
                }
            }
        }
...
TEST_F(WidgetTestInteractive, ViewFocusOnHWNDEnabledChanges)
{
    ...
    for (View* view : widget->GetContentsView()->children()) {
        SCOPED_TRACE("Child view " + base::NumberToString(widget->GetContentsView()->GetIndexOf(view).value()));
...
TEST_F(WidgetTestInteractive, WidgetNotActivatedOnFakeActivationMessages)
{
    ...
    Widget::InitParams init_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    ...
    init_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
...
TEST_F(WidgetTestInteractive, FullscreenBoundsReducedOnActivationLoss)
{
    ...
    widget1->Activate();
    RunPendingMessages();
    EXPECT_EQ(::GetActiveWindow(), widget1->GetNativeWindow()->GetHost()->GetAcceleratedWidget());
    ...
    widget2->Activate();
    RunPendingMessages();
    EXPECT_EQ(::GetActiveWindow(), widget2->GetNativeWindow()->GetHost()->GetAcceleratedWidget());

    gfx::Rect fullscreen_bounds_after_activation_loss = widget1->GetWindowBoundsInScreen();

    // After deactivation loss the bounds of the fullscreen widget should be
    // reduced by 1px.
    EXPECT_EQ(fullscreen_bounds.height() - fullscreen_bounds_after_activation_loss.height(), 1);

    widget1->Activate();
    RunPendingMessages();
    EXPECT_EQ(::GetActiveWindow(), widget1->GetNativeWindow()->GetHost()->GetAcceleratedWidget());

    gfx::Rect fullscreen_bounds_after_activate = widget1->GetWindowBoundsInScreen();
...
TEST_F(DesktopWidgetTestInteractive, WindowModalWindowDestroyedActivationTest)
{
    ...
    Widget* modal_dialog_widget = views::DialogDelegate::CreateDialogWidget(dialog_delegate.release(), nullptr, top_level_widget->GetNativeView());
...
TEST_F(DesktopWidgetTestInteractive, TouchSelectionQuickMenuIsNotActivated)
{
    ...
    generator.GestureTapAt(textfield_ptr->GetBoundsInScreen().origin() + gfx::Vector2d(10, 10));
    ...
    static_cast<TouchSelectionControllerImpl*>(textfield_test_api.touch_selection_controller())->ShowQuickMenuImmediatelyForTesting();
...
TEST_F(WidgetTestInteractive, DisableViewDoesNotActivateWidget)
{
    ...
    auto widget1 = std::make_unique<Widget>();
    Widget::InitParams params1 = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP);
    ...
    auto widget2 = std::make_unique<Widget>();
    Widget::InitParams params2 = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP);
...
TEST_F(WidgetTestInteractive, ShowCreatesActiveWindow)
{
    ...
    ShowSync(widget.get());
    EXPECT_EQ(GetWidgetShowState(widget.get()), ui::mojom::WindowShowState::kNormal);
}
...
TEST_F(WidgetTestInteractive, ShowInactive)
{
    ...
    EXPECT_EQ(GetWidgetShowState(widget.get()), ui::mojom::WindowShowState::kInactive);
}
...
TEST_F(WidgetTestInteractive, WidgetShouldBeActiveWhenShow)
{
    ...
    std::unique_ptr<Widget> anchor_widget = base::WrapUnique(CreateTopLevelNativeWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
...
TEST_F(WidgetTestInteractive, InactiveWidgetDoesNotGrabActivation)
{
    ...
    EXPECT_EQ(GetWidgetShowState(widget2.get()), ui::mojom::WindowShowState::kInactive);
    EXPECT_EQ(GetWidgetShowState(widget.get()), ui::mojom::WindowShowState::kNormal);
}
...
TEST_F(WidgetTestInteractive, MAYBE_ExitFullscreenRestoreState)
{
    ...
    EXPECT_EQ(ui::mojom::WindowShowState::kNormal, GetWidgetShowState(toplevel.get()));

    toplevel->SetFullscreen(true);
    EXPECT_EQ(ui::mojom::WindowShowState::kFullscreen, GetWidgetShowState(toplevel.get()));
    toplevel->SetFullscreen(false);
    EXPECT_NE(ui::mojom::WindowShowState::kFullscreen, GetWidgetShowState(toplevel.get()));
    ...
    toplevel->Maximize();
    EXPECT_EQ(ui::mojom::WindowShowState::kMaximized, GetWidgetShowState(toplevel.get()));
...
TEST_F(DesktopWidgetTestInteractive, RestoreAfterMinimize)
{
    ...
    PropertyWaiter minimize_waiter(base::BindRepeating(&Widget::IsMinimized, base::Unretained(widget.get())), true);
    widget->Minimize();
    EXPECT_TRUE(minimize_waiter.Wait());

    PropertyWaiter restore_waiter(base::BindRepeating(&Widget::IsMinimized, base::Unretained(widget.get())), false);
    widget->Restore();
...
TEST_F(DesktopWidgetTestInteractive, ShowAfterMaximize)
{
    ...
    PropertyWaiter maximize_waiter(base::BindRepeating(&Widget::IsMaximized, base::Unretained(widget.get())), true);
    widget->Maximize();
...
TEST_F(DesktopWidgetTestInteractive, RestoreAndMinimizeVisibility)
{
    ...
    PropertyWaiter minimize_widget_waiter(base::BindRepeating(&Widget::IsMinimized, base::Unretained(widget.get())), true);
    widget->Minimize();
    EXPECT_TRUE(minimize_widget_waiter.Wait());
    EXPECT_TRUE(widget->IsVisible());
    EXPECT_FALSE(root_window->IsVisible());

    PropertyWaiter restore_widget_waiter(base::BindRepeating(&Widget::IsMinimized, base::Unretained(widget.get())), false);
    widget->Restore();
...
public:
    explicit SyntheticMouseMoveCounter(Widget* widget)
        : widget_(widget)
    {
    ...
    SyntheticMouseMoveCounter& operator=(const SyntheticMouseMoveCounter&) = delete;
    ...
    int num_synthetic_mouse_moves() const
    {
        return count_;
    }
...
{
    // Create a top-level widget.
    std::unique_ptr<Widget> widget_below = base::WrapUnique(CreateTopLevelPlatformDesktopWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    ...
    Widget::InitParams child_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    ...
    std::unique_ptr<Widget> widget_above = base::WrapUnique(CreateTopLevelPlatformDesktopWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    widget_above->SetBounds(gfx::Rect(300, 300));
    widget_above->Show();
    base::RunLoop().RunUntilIdle();

    EXPECT_TRUE(widget_above->AsWidget()->IsStackedAbove(widget_below->AsWidget()->GetNativeView()));
...
TEST_F(DesktopWidgetTestInteractive, DesktopNativeWidgetWithModalTransientChild)
{
    ...
    auto dialog_delegate = std::make_unique<DialogDelegateView>();
    dialog_delegate->SetModalType(ui::mojom::ModalType::kWindow);
    Widget* modal_dialog_widget = DialogDelegate::CreateDialogWidget(dialog_delegate.release(), nullptr, top_level->GetNativeView());
...
public:
    ...
    CaptureLostTrackingWidget& operator=(const CaptureLostTrackingWidget&) = delete;
...
public:
    ...
    {
        std::unique_ptr<Widget> widget1 = std::make_unique<CaptureLostTrackingWidget>(capture_state1_.get());
        InitPlatformWidget(widget1.get(), use_desktop_native_widget);
        widget1->Show();

        std::unique_ptr<Widget> widget2 = std::make_unique<CaptureLostTrackingWidget>(capture_state2_.get());
    ...
    void InitPlatformWidget(Widget* widget, bool use_desktop_native_widget)
    {
        ...
        params.native_widget = use_desktop_native_widget ? nullptr : CreatePlatformNativeWidgetImpl(widget, kDefault, nullptr);
        widget->Init(std::move(params));
    }
...
{
    auto widget = std::make_unique<Widget>();
    Widget::InitParams params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    params.bounds = gfx::Rect(400, 400);
    widget->Init(std::move(params));

    auto contents_view = std::make_unique<View>();
    MouseView* mouse_view1 = contents_view->AddChildView(std::make_unique<MouseView>());
    MouseView* mouse_view2 = contents_view->AddChildView(std::make_unique<MouseView>());
    ...
    widget->Show();
    ui::test::EventGenerator generator(GetRootWindow(widget.get()), widget->GetNativeWindow());
    generator.set_current_screen_location(widget->GetClientAreaBoundsInScreen().CenterPoint());
...
TEST_F(WidgetCaptureTest, CaptureAutoReset)
{
    ...
    ui::MouseEvent release(
        ui::EventType::kMouseReleased, click_location, click_location, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
...
TEST_F(WidgetCaptureTest, ResetCaptureOnGestureEnd)
{
    ...
    ui::GestureEvent tap_down(15, 15, 0, base::TimeTicks(), ui::GestureEventDetails(ui::EventType::kGestureTapDown));
    ui::GestureEvent end(15, 15, 0, base::TimeTicks(), ui::GestureEventDetails(ui::EventType::kGestureEnd));
...
TEST_F(WidgetCaptureTest, DisableCaptureWidgetFromMousePress)
{
    ...
    std::unique_ptr<Widget> first = base::WrapUnique(CreateTopLevelFramelessPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    std::unique_ptr<Widget> second = base::WrapUnique(CreateTopLevelFramelessPlatformWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    Widget* second_ptr = second.get();

    NestedLoopCaptureView* container = first->SetContentsView(std::make_unique<NestedLoopCaptureView>(std::move(second)));

    second_ptr->SetContentsView(std::make_unique<ExitLoopOnRelease>(container->GetQuitClosure()));
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE,
        base::BindOnce(&Widget::OnMouseEvent, base::Unretained(second_ptr),
            base::Owned(new ui::MouseEvent(
                ui::EventType::kMouseReleased, location, location, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON))));
    ui::MouseEvent press(ui::EventType::kMousePressed, location, location, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
...
{
    std::unique_ptr<Widget> top_level = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
    top_level->SetContentsView(std::make_unique<MouseView>());

    auto child1 = std::make_unique<Widget>();
    Widget::InitParams params1 = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_CONTROL);
    ...
    Widget::InitParams params2 = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_CONTROL);
    ...
    // Click on child1.
    ui::test::EventGenerator generator(GetRootWindow(top_level.get()), child1->GetNativeWindow());
    generator.set_current_screen_location(child1->GetClientAreaBoundsInScreen().CenterPoint());
    generator.PressLeftButton();
    ...
    generator.set_current_screen_location(child2->GetClientAreaBoundsInScreen().CenterPoint());
    generator.PressLeftButton();
    ...
    generator.set_current_screen_location(top_level->GetClientAreaBoundsInScreen().origin());
    generator.PressLeftButton();
...
// those.
#if BUILDFLAG(IS_MAC)
#define MAYBE_SystemModalWindowReleasesCapture DISABLED_SystemModalWindowReleasesCapture
...
TEST_F(WidgetCaptureTest, MAYBE_SystemModalWindowReleasesCapture)
{
    ...
    ASSERT_FALSE(focus_listener.focus_changes().empty());
    EXPECT_EQ(top_level_widget->GetNativeView(), focus_listener.focus_changes().back());
...
{
    auto widget1 = std::make_unique<Widget>();
    Widget::InitParams params1 = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    widget1->Init(std::move(params1));
    MouseView* mouse_view1 = widget1->SetContentsView(std::make_unique<MouseView>());
    widget1->Show();
    widget1->SetBounds(gfx::Rect(300, 300));

    auto widget2 = std::make_unique<Widget>();
    Widget::InitParams params2 = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    widget2->Init(std::move(params2));
    widget2->Show();
...
public:
    ...
    CaptureOnActivationObserver& operator=(const CaptureOnActivationObserver&) = delete;
    ...
    bool activation_observed() const
    {
        return activation_observed_;
    }
...
TEST_F(WidgetCaptureTest, SetCaptureToNonToplevel)
{
    ...
    Widget::InitParams toplevel_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    toplevel->Init(std::move(toplevel_params));
    toplevel->Show();

    auto child = std::make_unique<Widget>();
    Widget::InitParams child_params = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
...
TEST_F(WidgetCaptureTest, MouseEventDispatchedToRightWindow)
{
    ...
    Widget::InitParams params1 = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    ...
    Widget::InitParams params2 = CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    ...
    ui::MouseEvent mouse_event(ui::EventType::kMouseExited, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ui::EventDispatchDetails details = widget1->GetNativeWindow()->GetHost()->GetEventSink()->OnEventFromSource(&mouse_event);
...
public:
    ...
    WidgetInputMethodInteractiveTest(const WidgetInputMethodInteractiveTest&) = delete;
    WidgetInputMethodInteractiveTest& operator=(const WidgetInputMethodInteractiveTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        deactivate_widget_ = base::WrapUnique(CreateTopLevelNativeWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
...
TEST_F(WidgetInputMethodInteractiveTest, MAYBE_Activation)
{
    ...
    EXPECT_EQ(ui::TEXT_INPUT_TYPE_NONE, widget->GetInputMethod()->GetTextInputType());
}
...
{
    std::unique_ptr<Widget> widget = base::WrapUnique(CreateTopLevelNativeWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    Textfield* const textfield1 = widget->GetRootView()->AddChildView(CreateTextfield());
    Textfield* const textfield2 = widget->GetRootView()->AddChildView(CreateTextfield());
...
{
    std::unique_ptr<Widget> parent = base::WrapUnique(CreateTopLevelNativeWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    parent->SetBounds(gfx::Rect(100, 100, 100, 100));

    std::unique_ptr<Widget> child = base::WrapUnique(CreateChildNativeWidgetWithParent(parent.get(), Widget::InitParams::CLIENT_OWNS_WIDGET));
    child->SetBounds(gfx::Rect(0, 0, 50, 50));
    child->Show();

    Textfield* const textfield_parent = parent->GetRootView()->AddChildView(CreateTextfield());
    Textfield* const textfield_child = child->GetRootView()->AddChildView(CreateTextfield());
    ...
    EXPECT_EQ(ui::TEXT_INPUT_TYPE_PASSWORD, parent->GetInputMethod()->GetTextInputType());

    textfield_child->RequestFocus();
    EXPECT_EQ(ui::TEXT_INPUT_TYPE_TEXT, parent->GetInputMethod()->GetTextInputType());
...
// AuraTestHelper which sets the input method as DummyInputMethod.
    ...
    EXPECT_EQ(ui::TEXT_INPUT_TYPE_NONE, parent->GetInputMethod()->GetTextInputType());
...
{
    std::unique_ptr<Widget> widget = base::WrapUnique(CreateTopLevelNativeWidget(Widget::InitParams::CLIENT_OWNS_WIDGET));
    Textfield* const textfield = widget->GetRootView()->AddChildView(CreateTextfield());
    ShowSync(widget.get());
    EXPECT_EQ(ui::TEXT_INPUT_TYPE_NONE, widget->GetInputMethod()->GetTextInputType());
...
TEST_F(WidgetInputMethodInteractiveTest, AcceleratorInTextfield)
{
    ...
    ui::KeyEvent key_event(ui::EventType::kKeyPressed, ui::VKEY_F, ui::EF_ALT_DOWN);
    ui::Accelerator accelerator(key_event);
    widget->GetFocusManager()->RegisterAccelerator(accelerator, ui::AcceleratorManager::kNormalPriority, textfield);
...
class DesktopWidgetDragTestInteractive : public DesktopWidgetTestInteractive, public WidgetObserver {
public:
    DesktopWidgetDragTestInteractive() = default;

    DesktopWidgetDragTestInteractive(const DesktopWidgetDragTestInteractive&) = delete;
    DesktopWidgetDragTestInteractive& operator=(const DesktopWidgetDragTestInteractive&) = delete;
...
protected:
    ...
    void InitWidget(Widget* widget, base::OnceClosure on_drag_enter, base::OnceClosure on_drag_exit, base::OnceClosure on_capture_lost)
    {
        ...
        auto on_mouse_exit = base::BindLambdaForTesting([]() {
            ...
            EXPECT_TRUE(ui_controls::SendMouseMove(target_location.x(), target_location.y()));
        });

        widget->client_view()->AddChildView(
            std::make_unique<DragView>(std::move(on_drag_enter), std::move(on_drag_exit), std::move(on_capture_lost), std::move(on_mouse_exit)));
    ...
    void StartDrag()
    {
        ...
        EXPECT_TRUE(ui_controls::SendMouseMoveNotifyWhenDone(start_location.x(), start_location.y(), move_loop.QuitClosure()));
        move_loop.Run();
        base::RunLoop press_loop;
        EXPECT_TRUE(ui_controls::SendMouseEventsNotifyWhenDone(ui_controls::MouseButton::LEFT, ui_controls::MouseButtonState::DOWN, press_loop.QuitClosure()));
        press_loop.Run();
...
private:
    // WidgetObserver:
    void OnWidgetDragComplete(Widget* widget) override
    {
...
#define MAYBE_RunShellDragUpdatesMouseButtonState DISABLED_RunShellDragUpdatesMouseButtonState
#else
#define MAYBE_RunShellDragUpdatesMouseButtonState RunShellDragUpdatesMouseButtonState
#endif
TEST_F(DesktopWidgetDragTestInteractive, MAYBE_RunShellDragUpdatesMouseButtonState)
{
#if BUILDFLAG(IS_WIN)
    ...
    // drags.
    aura::test::EnvTestHelper(aura::Env::GetInstance()).SetInputStateLookup(aura::InputStateLookup::Create());
#endif
    ...
    auto on_enter = [&]() {
        ...
        EXPECT_TRUE(ui_controls::SendMouseEvents(ui_controls::MouseButton::LEFT, ui_controls::MouseButtonState::UP));
    };
    ...
    InitWidget(widget.get(), base::BindLambdaForTesting(on_enter), base::DoNothing(), std::move(on_capture_lost));
    ...
    base::RunLoop move_loop;
    EXPECT_TRUE(ui_controls::SendMouseMoveNotifyWhenDone(target_location.x(), target_location.y(), move_loop.QuitClosure()));
    move_loop.Run();
