...
TEST_F(DesktopScreenPositionClientTest, PositionDialog)
{
    ...
    Widget* dialog = DialogDelegate::CreateDialogWidget(dialog_delegate_view, nullptr, parent_widget.GetNativeView());
...
TEST_F(DesktopScreenPositionClientTest, PositionControlWithNonRootParent)
{
    ...
    gfx::Rect work_area = display::Screen::GetScreen()->GetDisplayNearestPoint(origin).work_area();
    ...
    params1.bounds = gfx::Rect(origin + work_area.OffsetFromOrigin(), gfx::Size(700, work_area.height() - origin.y() - work_area.y()));
    ...
    params2.child = true;
    params2.native_widget = test::CreatePlatformNativeWidgetImpl(&widget2, test::kStubCapture, nullptr);
    ...
    params3.native_widget = test::CreatePlatformNativeWidgetImpl(&widget3, test::kStubCapture, nullptr);
    widget3.Init(std::move(params3));

    // The origin of the 3rd window should be the sum of all parent origins.
    gfx::Point expected_origin(origin.x() * 3 + work_area.x(), origin.y() * 3 + work_area.y());
    gfx::Rect expected_bounds(expected_origin, gfx::Size(500, work_area.height() - 200));
...
TEST_F(DesktopScreenPositionClientTest, InitialBoundsConstrainedToDesktop)
{
    ...
    gfx::Rect work_area = display::Screen::GetScreen()->GetPrimaryDisplay().work_area();
    // Make the origin start at 75% of the width and height.
    gfx::Point origin = gfx::Point(work_area.width() * 3 / 4, work_area.height() * 3 / 4);
    ...
    Widget::InitParams params = CreateParams(Widget::InitParams::TYPE_WINDOW);
    params.bounds = gfx::Rect(origin, gfx::Size(work_area.width() / 2, work_area.height() / 2));
    params.ownership = Widget::InitParams::CLIENT_OWNS_WIDGET;
    widget.Init(std::move(params));

    // The bounds of the window should be fully on the primary display.
    gfx::Point expected_origin(work_area.right() - work_area.width() / 2, work_area.bottom() - work_area.height() / 2);
    gfx::Rect expected_bounds(expected_origin, gfx::Size(work_area.width() / 2, work_area.height() / 2));
...
TEST_F(DesktopScreenPositionClientTest, InitialBoundsConstrainedToParent)
{
    ...
    // Use the primary display for this test.
    gfx::Rect work_area = display::Screen::GetScreen()->GetPrimaryDisplay().work_area();
    gfx::Point origin = gfx::Point(work_area.x() + work_area.width() / 4, work_area.y() + work_area.height() / 4);
    ...
    Widget::InitParams params1 = CreateParams(Widget::InitParams::TYPE_WINDOW);
    params1.bounds = gfx::Rect(origin, gfx::Size(work_area.width() / 2, work_area.height() / 2));
    params1.ownership = Widget::InitParams::CLIENT_OWNS_WIDGET;
    widget1.Init(std::move(params1));
    ...
    Widget::InitParams params2 = CreateParams(Widget::InitParams::TYPE_WINDOW);
    params2.bounds = gfx::Rect(widget_bounds.width() * 3 / 4, widget_bounds.height() * 3 / 4, widget_bounds.width() / 2, widget_bounds.height() / 2);
    params2.parent = widget1.GetNativeView();
    params2.ownership = Widget::InitParams::CLIENT_OWNS_WIDGET;
    params2.child = true;
    params2.native_widget = test::CreatePlatformNativeWidgetImpl(&widget2, test::kStubCapture, nullptr);
    ...
    gfx::Point expected_origin(widget_bounds.right() - widget_bounds.width() / 2, widget_bounds.bottom() - widget_bounds.height() / 2);
    gfx::Rect expected_bounds(expected_origin, gfx::Size(widget_bounds.width() / 2, widget_bounds.height() / 2));
