...
{
    Widget::InitParams params(Widget::InitParams::Ownership::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_POPUP);
...
public:
    ...
    void SetUp() override
    {
        ...
        focus_controller_ = std::make_unique<wm::FocusController>(test_focus_rules_);
...
TEST_F(NativeWidgetAuraTest, CenterWindowLargeParent)
{
    ...
    EXPECT_EQ(gfx::Rect((640 - 100) / 2, (480 - 100) / 2, 100, 100), window->GetNativeWindow()->bounds());
}
...
TEST_F(NativeWidgetAuraTest, CenterWindowSmallParent)
{
    ...
    window->CenterWindow(gfx::Size(100, 100));
    EXPECT_EQ(gfx::Rect((480 - 100) / 2, (320 - 100) / 2, 100, 100), window->GetNativeWindow()->bounds());
}
...
public:
    ...
    {
        return handled_gestures_set_.find(type_to_query) != handled_gestures_set_.cend();
    }
    ...
    void OnGestureEvent(ui::GestureEvent* event) override
    {
        ...
        if (handled_gestures_set_.find(event_type) == handled_gestures_set_.cend()) {
...
TEST_F(NativeWidgetAuraTest, MouseClickInterruptsGestureScroll)
{
    ...
    auto* child_view = contents_view->AddChildView(std::make_unique<EventHandlingView>());
    child_view->SetBoundsRect(gfx::Rect(gfx::Size { 50, 50 }));

    auto scroll_callback = [](ui::test::EventGenerator* event_generator, int* step_count, ui::EventType event_type, const gfx::Vector2dF& offset) {
    ...
    generator.GestureScrollSequenceWithCallback(center_point, target_point,
        /*duration=*/base::Milliseconds(100), step_count, base::BindRepeating(scroll_callback, &generator, &step_count));
...
{
    Widget::InitParams params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
...
TEST_F(NativeWidgetAuraTest, GetWorkspace)
{
    ...
    widget->GetNativeWindow()->SetProperty(aura::client::kWindowWorkspaceKey, aura::client::kWindowWorkspaceUnassignedWorkspace);
    EXPECT_EQ("", widget->GetWorkspace());

    widget->GetNativeWindow()->SetProperty(aura::client::kWindowWorkspaceKey, aura::client::kWindowWorkspaceVisibleOnAllWorkspaces);
    EXPECT_EQ("", widget->GetWorkspace());

    const int desk_index = 1;
    widget->GetNativeWindow()->SetProperty(aura::client::kWindowWorkspaceKey, desk_index);
...
public:
    ...
    // aura::WindowObserver:
    void OnWindowPropertyChanged(aura::Window* window, const void* key, intptr_t old) override
    {
        if (key != aura::client::kShowStateKey)
            return;
        count_++;
        state_ = window_observation_.GetSource()->GetProperty(aura::client::kShowStateKey);
    }
...
TEST_F(NativeWidgetAuraTest, ToggleState)
{
    ...
    auto widget = std::make_unique<Widget>();
    widget->Init(std::move(params));
    auto observer = std::make_unique<TestWindowObserver>(widget->GetNativeWindow());
    widget->Show();
...
TEST_F(NativeWidgetAuraTest, ShowMaximizedDoesntBounceAround)
{
    ...
    widget->Init(std::move(params));
    EXPECT_FALSE(static_cast<TestWidget*>(widget.get())->did_size_change_more_than_once());
}
...
public:
    ...
    PropertyTestLayoutManager& operator=(const PropertyTestLayoutManager&) = delete;
...
private:
    ...
    void OnWindowAddedToLayout(aura::Window* child) override
    {
        EXPECT_EQ(aura::client::kResizeBehaviorCanResize | aura::client::kResizeBehaviorCanMaximize | aura::client::kResizeBehaviorCanMinimize
...
{
    root_window()->SetBounds(gfx::Rect(0, 0, 640, 480));
    PropertyTestLayoutManager* layout_manager = root_window()->SetLayoutManager(std::make_unique<PropertyTestLayoutManager>());
    auto widget = std::make_unique<TestWidget>();
    Widget::InitParams params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);

    auto delegate_owned = std::make_unique<WidgetDelegate>();
    params.delegate = delegate_owned.get();
    params.delegate->RegisterDeleteDelegateCallback(base::DoNothingWithBoundArgs(std::move(delegate_owned)));
...
TEST_F(NativeWidgetAuraTest, DontCaptureOnGesture)
{
    ...
    auto widget = std::make_unique<TestWidget>();
    Widget::InitParams params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    params.context = root_window();
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(41, 51), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
    ...
    // the press.
    ui::TouchEvent release(ui::EventType::kTouchReleased, gfx::Point(250, 251), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
...
TEST_F(NativeWidgetAuraTest, PreferViewLayersToChildWindows)
{
    ...
    Widget::InitParams parent_params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    ...
    // Point is over |child|.
    EXPECT_EQ(child->GetNativeWindow(), parent->GetNativeWindow()->GetEventHandlerForPoint(gfx::Point(50, 50)));
    ...
    EXPECT_EQ(child->GetNativeWindow(), parent->GetNativeWindow()->GetEventHandlerForPoint(gfx::Point(20, 20)));
    ...
    EXPECT_EQ(parent->GetNativeWindow(), parent->GetNativeWindow()->GetEventHandlerForPoint(gfx::Point(20, 20)));

    // Point is over |child|, it should get the event.
    EXPECT_EQ(child->GetNativeWindow(), parent->GetNativeWindow()->GetEventHandlerForPoint(gfx::Point(70, 70)));
...
TEST_F(NativeWidgetAuraTest, ShouldDescendIntoChildForEventHandlingChecksVisibleBounds)
{
    ...
    View* parent_root_view_child = parent_root_view->AddChildView(std::make_unique<View>());
    ...
    View* parent_root_view_child_child = parent_root_view_child->AddChildView(std::make_unique<View>());
    parent_root_view_child_child->SetBounds(0, 0, 100, 100);
    parent_root_view_child_child->SetPaintToLayer();
    parent_root_view_child_child->layer()->parent()->StackAtTop(parent_root_view_child_child->layer());
...
TEST_F(NativeWidgetAuraTest, ShouldDescendIntoChildForEventHandlingIgnoresViewsThatDoNotProcessEvents)
{
    // Create two widgets: `parent` and `child`. `child` is a child of `parent`.
    auto parent = std::make_unique<Widget>();
    Widget::InitParams parent_params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW_FRAMELESS);
    parent_params.context = root_window();
    parent->Init(std::move(parent_params));
    View* const parent_root_view = parent->SetContentsView(std::make_unique<View>());
    parent->SetBounds(gfx::Rect(0, 0, 400, 400));
    parent->Show();
    ...
    View* const view_overlapping_child = parent_root_view->AddChildView(std::make_unique<View>());
    view_overlapping_child->SetBoundsRect(gfx::Rect(0, 0, 200, 200));
    view_overlapping_child->SetPaintToLayer();
    view_overlapping_child->layer()->parent()->StackAtTop(view_overlapping_child->layer());
    ...
    EXPECT_EQ(parent->GetNativeWindow(), parent->GetNativeWindow()->GetEventHandlerForPoint(gfx::Point(50, 50)));
...
TEST_F(NativeWidgetAuraTest, OnWidgetMovedInvokedAfterAcquireLayer)
{
    ...
    UniqueWidgetPtr widget = base::WrapUnique(Widget::CreateWindowWithContext(delegate_ptr, root_window(), gfx::Rect(10, 10, 100, 200)));
    widget->Show();
...
TEST_F(NativeWidgetAuraTest, VisibilityOfChildBubbleWindow)
{
    ...
    auto parent = std::make_unique<Widget>();
    Widget::InitParams parent_params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    parent_params.context = root_window();
    parent->Init(std::move(parent_params));
    parent->SetBounds(gfx::Rect(0, 0, 480, 320));
    ...
    auto child = std::make_unique<Widget>();
    Widget::InitParams child_params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_BUBBLE);
    child_params.parent = parent->GetNativeWindow();
    child->Init(std::move(child_params));
    child->SetBounds(gfx::Rect(0, 0, 200, 200));
    child->Show();
    ...
    wm::TransientWindowManager* manager = wm::TransientWindowManager::GetOrCreate(child->GetNativeWindow());
...
TEST_F(NativeWidgetAuraTest, TransientChildModalWindowVisibility)
{
    ...
    auto child = std::make_unique<Widget>();
    Widget::InitParams child_params(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    child_params.parent = parent->GetNativeWindow();

    child_params.delegate = delegate_owned.get();
    child_params.delegate->RegisterDeleteDelegateCallback(base::DoNothingWithBoundArgs(std::move(delegate_owned)));
...
TEST_F(NativeWidgetAuraTest, MinimizedWidgetRestoreBounds)
{
    ...
    aura::Window* window = widget->GetNativeWindow();
    EXPECT_EQ(ui::mojom::WindowShowState::kMinimized, window->GetProperty(aura::client::kShowStateKey));
    EXPECT_EQ(restore_bounds, *window->GetProperty(aura::client::kRestoreBoundsKey));
...
TEST_F(NativeWidgetAuraTest, WorkspaceUuid)
{
    ...
    EXPECT_EQ(-1, widget->GetNativeWindow()->GetProperty(aura::client::kWindowWorkspaceKey));
    EXPECT_THAT(widget->GetNativeWindow()->GetProperty(aura::client::kDeskUuidKey), testing::Pointee(uuid));

    // If the `workspace` param is an int, `kWindowWorkspaceKey` should be set.
    Widget::InitParams params2(Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_WINDOW);
    ...
    EXPECT_EQ(2, widget2->GetNativeWindow()->GetProperty(aura::client::kWindowWorkspaceKey));
    EXPECT_FALSE(widget2->GetNativeWindow()->GetProperty(aura::client::kDeskUuidKey));
}
...
public:
    ...
    NativeWidgetAuraWithNoDelegateTest(const NativeWidgetAuraWithNoDelegateTest&) = delete;
...
TEST_F(NativeWidgetAuraWithNoDelegateTest, OnWindowTargetVisibilityChangedTest)
{
...
{
    native_widget_->OnWindowActivated(wm::ActivationChangeObserver::ActivationReason::ACTIVATION_CLIENT, native_widget_->GetNativeView(), nullptr);
}
...
TEST_F(NativeWidgetAuraWithNoDelegateTest, ShouldDescendIntoChildForEventHandlingTest)
{
