...
TooltipManagerAura::TooltipManagerAura(internal::NativeWidgetPrivate* native_widget)
...
{
    return ui::ResourceBundle::GetSharedInstance().GetFontList(ui::ResourceBundle::BaseFont);
}

// static
void TooltipManagerAura::UpdateTooltipManagerForCapture(internal::NativeWidgetPrivate* source)
{
    ...
    gfx::Point screen_loc(root_window->GetHost()->dispatcher()->GetLastMouseLocationInRoot());
    ...
    while (target) {
        internal::NativeWidgetPrivate* target_native_widget = internal::NativeWidgetPrivate::GetNativeWidgetForNativeView(target);
...
{
    return wm::GetTooltipClient(native_widget_->GetNativeView()->GetRootWindow())->GetMaxWidth(point);
}
...
void TooltipManagerAura::UpdateTooltipForFocus(View* view)
{
    ...
    if (wm::GetTooltipClient(root_window)) {
        ...
        auto root_bounds = View::ConvertRectToTarget(view, view->GetWidget()->GetRootView(), bounds);

        wm::GetTooltipClient(root_window)->UpdateTooltipFromKeyboard(root_bounds, GetWindow());
    }
}
...
{
    View* root_view = native_widget_->GetWidget() ? native_widget_->GetWidget()->GetRootView() : nullptr;
...
void TooltipManagerAura::UpdateTooltipForTarget(View* target, const gfx::Point& point, aura::Window* root_window)
{
    if (target) {
