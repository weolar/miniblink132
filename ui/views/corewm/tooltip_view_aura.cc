    ...
    : render_text_(gfx::RenderText::CreateRenderText())
{
    ...
    SetBackground(views::CreateThemedSolidBackground(ui::kColorTooltipBackground));
    SetBorder(views::CreatePaddedBorder(
        views::CreateThemedSolidBorder(kTooltipBorderThickness, ui::kColorTooltipForeground), kBorderInset - gfx::Insets(kTooltipBorderThickness)));
...
gfx::Size TooltipViewAura::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
...
void TooltipViewAura::OnThemeChanged()
{
    ...
    // opaque.
    render_text_->set_subpixel_rendering_suppressed(SkColorGetA(background()->get_color()) != SK_AlphaOPAQUE);
    render_text_->SetColor(GetColorProvider()->GetColor(ui::kColorTooltipForeground));
}
