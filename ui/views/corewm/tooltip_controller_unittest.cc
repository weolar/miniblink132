...
public:
    ...
    void SetUp() override
    {
        ...
        if (root_window) {
            ...
            controller_ = std::make_unique<TooltipController>(std::unique_ptr<views::corewm::Tooltip>(tooltip_),
        ...
        helper_ = std::make_unique<TooltipControllerTestHelper>(widget_->GetNativeWindow()->GetRootWindow());
...
protected:
    ...
    aura::Window* CreateNormalWindow(int id, aura::Window* parent, aura::WindowDelegate* delegate)
    {
        aura::Window* window = new aura::Window(delegate ? delegate : aura::test::TestWindowDelegate::CreateSelfDestroyingDelegate());
        window->SetId(id);
...
TEST_F(TooltipControllerTest, ViewTooltip)
{
    ...
    EXPECT_EQ(GetWindow(), GetRootWindow()->GetEventHandlerForPoint(generator_->current_screen_location()));
    ...
    EXPECT_EQ(helper_->state_manager()->tooltip_trigger(), TooltipTrigger::kCursor);
...
{
    std::u16string text = u"Really, really, really, really, really, really long tooltip that "
                          u"exceeds max width";
    ...
    EXPECT_TRUE(helper_->IsTooltipVisible());
    EXPECT_EQ(helper_->state_manager()->tooltip_trigger(), TooltipTrigger::kCursor);
    const gfx::RenderText* render_text = test::TooltipAuraTestApi(tooltip_).GetRenderText();
...
TEST_F(TooltipControllerTest, AccessibleNodeData)
{
    ...
    EXPECT_EQ(text, base::ASCIIToUTF16(node_data.GetStringAttribute(ax::mojom::StringAttribute::kName)));
}
...
TEST_F(TooltipControllerTest, TooltipBounds)
{
    ...
    gfx::Rect display_bounds(display::Screen::GetScreen()->GetDisplayNearestPoint(gfx::Point(0, 0)).bounds());
    ...
    // 1. The tooltip fits entirely in the window.
    {
        ...
        gfx::Rect bounds = test::TooltipAuraTestApi(tooltip_).GetTooltipBounds(tooltip_size, anchor_point, TooltipTrigger::kCursor);
        gfx::Point expected_position(anchor_point.x() + TooltipAura::kCursorOffsetX, a_expected_y);
        ...
        bounds = test::TooltipAuraTestApi(tooltip_).GetTooltipBounds(tooltip_size, anchor_point, TooltipTrigger::kKeyboard);
        expected_position = gfx::Point(anchor_point.x() - tooltip_size.width() / 2, b_expected_y);
        EXPECT_EQ(bounds, gfx::Rect(expected_position, tooltip_size));
    }
    ...
    // 3. The tooltip overflows on the right side of the window.
    {
        ...
        // bottom-right corner of the cursor.
        gfx::Rect bounds = test::TooltipAuraTestApi(tooltip_).GetTooltipBounds(tooltip_size, anchor_point, TooltipTrigger::kCursor);
        gfx::Point expected_position(display_bounds.right() - tooltip_size.width(), a_expected_y);
        EXPECT_EQ(bounds, gfx::Rect(expected_position, tooltip_size));

        // B. When not attached to the cursor, the tooltip should be horizontally
        // centered with the anchor point.
        bounds = test::TooltipAuraTestApi(tooltip_).GetTooltipBounds(tooltip_size, anchor_point, TooltipTrigger::kKeyboard);
        expected_position = gfx::Point(display_bounds.right() - tooltip_size.width(), b_expected_y);
        EXPECT_EQ(bounds, gfx::Rect(expected_position, tooltip_size));
    }
    // 4. The tooltip overflows on the bottom.
    {
        ...
        // bottom-right corner of the cursor.
        gfx::Rect bounds = test::TooltipAuraTestApi(tooltip_).GetTooltipBounds(tooltip_size, anchor_point, TooltipTrigger::kCursor);
        gfx::Point expected_position(anchor_point.x() + TooltipAura::kCursorOffsetX, anchor_point.y() - tooltip_size.height());
        EXPECT_EQ(bounds, gfx::Rect(expected_position, tooltip_size));

        // B. When not attached to the cursor, the tooltip should be horizontally
        // centered with the anchor point.
        bounds = test::TooltipAuraTestApi(tooltip_).GetTooltipBounds(tooltip_size, anchor_point, TooltipTrigger::kKeyboard);
        expected_position = gfx::Point(anchor_point.x() - tooltip_size.width() / 2, anchor_point.y() - tooltip_size.height());
        EXPECT_EQ(bounds, gfx::Rect(expected_position, tooltip_size));
    }
}
#endif
...
TEST_F(TooltipControllerTest, TooltipsInMultipleViews)
{
    ...
    for (int i = 0; i < 49; ++i) {
        ...
        EXPECT_TRUE(helper_->IsTooltipVisible());
        EXPECT_EQ(helper_->state_manager()->tooltip_trigger(), TooltipTrigger::kCursor);
        EXPECT_EQ(window, root_window->GetEventHandlerForPoint(generator_->current_screen_location()));
        std::u16string expected_tooltip = u"Tooltip Text";
...
TEST_F(TooltipControllerTest, TooltipHidesOnKeyPressAndStaysHiddenUntilChange)
{
    ...
    for (int i = 0; i < 49; i++) {
        ...
        EXPECT_FALSE(helper_->IsTooltipVisible());
        EXPECT_FALSE(helper_->IsWillHideTooltipTimerRunning());
        EXPECT_EQ(window, GetRootWindow()->GetEventHandlerForPoint(generator_->current_screen_location()));
...
TEST_F(TooltipControllerTest, ShowAndHideTooltipTriggeredFromKeyboard)
{
    ...
    helper_->controller()->UpdateTooltipFromKeyboard(view_->ConvertRectToWidget(view_->bounds()), GetWindow());
    ...
    EXPECT_TRUE(helper_->IsTooltipVisible());
    EXPECT_EQ(helper_->state_manager()->tooltip_trigger(), TooltipTrigger::kKeyboard);
...
TEST_F(TooltipControllerTest, KeyboardTriggeredTooltipStaysVisibleOnMouseExitedEvent)
{
    std::u16string expected_tooltip = u"Tooltip Text";
    ...
    // exited event.
    generator_->MoveMouseRelativeTo(widget_->GetNativeWindow(), view_->bounds().CenterPoint());
    EXPECT_TRUE(helper_->IsTooltipVisible());
...
TEST_F(TooltipControllerTest, MAYBE_Capture)
{
    ...
    widget2->Show();
    EXPECT_GE(IndexInParent(widget2->GetNativeWindow()), IndexInParent(widget_->GetNativeWindow()));
    ...
    widget_->SetNativeWindowProperty(TooltipManager::kGroupingPropertyKey, reinterpret_cast<void*>(grouping_key));
    widget2->SetNativeWindowProperty(TooltipManager::kGroupingPropertyKey, reinterpret_cast<void*>(grouping_key));
    ...
    Widget::ReparentNativeView(widget_child->GetNativeView(), widget2->GetNativeView());
    widget_child->Show();
...
public:
    ...
    const std::u16string& tooltip_text() const
    {
    ...
    void Update(aura::Window* window, const std::u16string& tooltip_text, const gfx::Point& position, const TooltipTrigger trigger) override
    {
...
public:
    ...
    void SetUp() override
    {
        ...
        controller_ = std::make_unique<TooltipController>(std::unique_ptr<corewm::Tooltip>(test_tooltip_),
            /* activation_client */ nullptr);
        root_window()->AddPreTargetHandler(controller_.get());
        SetTooltipClient(root_window(), controller_.get());
...
TEST_F(TooltipControllerTest2, VerifyLeadingTrailingWhitespaceStripped)
{
    ...
    std::unique_ptr<aura::Window> window(CreateNormalWindow(100, root_window(), &test_delegate));
...
public:
    ...
    void SetUp() override
    {
        ...
        controller_ = std::make_unique<TooltipController>(std::move(tooltip), /* activation_client */ nullptr);
        auto* tooltip_controller = static_cast<TooltipController*>(wm::GetTooltipClient(GetRootWindow()));
...
TEST_F(TooltipStateManagerTest, ShowAndHideTooltip)
{
    ...
    helper_->state_manager()->Show(GetRootWindow(), expected_text, gfx::Point(0, 0), TooltipTrigger::kCursor, helper_->GetShowTooltipDelay(), {});
...
TEST_F(TooltipStateManagerTest, UpdatePositionIfNeeded)
{
...

    {
        ...
        // update the position.
        helper_->state_manager()->Show(GetRootWindow(), expected_text, position, TooltipTrigger::kCursor, helper_->GetShowTooltipDelay(), {});
        EXPECT_EQ(GetRootWindow(), helper_->state_manager()->tooltip_parent_window());
        EXPECT_EQ(expected_text, helper_->state_manager()->tooltip_text());
        EXPECT_EQ(position, helper_->GetTooltipPosition());
        EXPECT_FALSE(helper_->IsTooltipVisible());
        ...
        helper_->state_manager()->UpdatePositionIfNeeded(new_position, TooltipTrigger::kKeyboard);
        EXPECT_EQ(position, helper_->GetTooltipPosition());

        // But it should be updated when the position's update is triggered by the
        // cursor.
        helper_->state_manager()->UpdatePositionIfNeeded(new_position, TooltipTrigger::kCursor);
...

    {
        ...
        // 1. When the |will_show_tooltip_timer_| is running, validate that we can
        // update the position.
        helper_->state_manager()->Show(GetRootWindow(), expected_text, position, TooltipTrigger::kKeyboard, helper_->GetShowTooltipDelay(), {});
        EXPECT_EQ(GetRootWindow(), helper_->state_manager()->tooltip_parent_window());
        EXPECT_EQ(expected_text, helper_->state_manager()->tooltip_text());
        EXPECT_EQ(position, helper_->GetTooltipPosition());
        EXPECT_FALSE(helper_->IsTooltipVisible());
