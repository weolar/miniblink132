...
TooltipControllerTestHelper::TooltipControllerTestHelper(aura::Window* root_window)
    : root_window_(root_window)
    , controller_(static_cast<TooltipController*>(wm::GetTooltipClient(root_window)))
{
    CHECK(root_window_);
...
void TooltipControllerTestHelper::MockWindowActivated(aura::Window* window, bool active)
{
    aura::Window* gained_active = active ? window : nullptr;
    aura::Window* lost_active = active ? nullptr : window;
    controller_->OnWindowActivated(wm::ActivationChangeObserver::ActivationReason::ACTIVATION_CLIENT, gained_active, lost_active);
}

void TooltipControllerTestHelper::OnWindowPropertyChanged(aura::Window* window, const void* key, intptr_t old)
{
