...
TooltipAura::TooltipAura()
    : TooltipAura(base::BindRepeating([]() { return std::make_unique<TooltipViewAura>(); }))
{
}

TooltipAura::TooltipAura(const TooltipAura::TooltipViewFactory& tooltip_view_factory)
...
{
    DCHECK(widget_);
    widget_->GetTooltipView()->GetViewAccessibility().GetAccessibleNodeData(node_data);
}

gfx::Rect TooltipAura::GetTooltipBounds(
    const gfx::Size& tooltip_size, const gfx::Point& anchor_point, const TooltipTrigger trigger, ui::OwnedWindowAnchor* anchor)
{
    ...
    case TooltipTrigger::kCursor: {
        const int x_offset = base::i18n::IsRTL() ? -tooltip_size.width() : kCursorOffsetX;
    ...
    anchor->anchor_position = trigger == TooltipTrigger::kCursor ? ui::OwnedWindowAnchorPosition::kBottomRight : ui::OwnedWindowAnchorPosition::kTop;
    anchor->constraint_adjustment = ui::OwnedWindowConstraintAdjustment::kAdjustmentSlideX | ui::OwnedWindowConstraintAdjustment::kAdjustmentSlideY
        | ui::OwnedWindowConstraintAdjustment::kAdjustmentFlipY;
    // TODO(msisov): handle RTL.
    anchor->anchor_rect = gfx::Rect(anchor_point, { kCursorOffsetX, kCursorOffsetY });

    display::Screen* screen = display::Screen::GetScreen();
    gfx::Rect display_bounds(screen->GetDisplayNearestPoint(anchor_point).bounds());
...
void TooltipAura::CreateTooltipWidget(const gfx::Rect& bounds, const ui::OwnedWindowAnchor& anchor)
{
    DCHECK(!widget_);
    DCHECK(tooltip_window_);
    widget_ = std::make_unique<TooltipWidget>();
    views::Widget::InitParams params(views::Widget::InitParams::CLIENT_OWNS_WIDGET, views::Widget::InitParams::TYPE_TOOLTIP);
    ...
    params.init_properties_container.SetProperty(aura::client::kOwnedWindowAnchor, anchor);
...
void TooltipAura::Update(aura::Window* window, const std::u16string& tooltip_text, const gfx::Point& position, const TooltipTrigger trigger)
{
    ...
    auto bounds = GetTooltipBounds(new_tooltip_view->GetPreferredSize({}), anchor_point, trigger, &anchor);
...
void TooltipAura::Show()
{
    if (widget_) {
        ...
        widget_->GetTooltipView()->NotifyAccessibilityEvent(ax::mojom::Event::kTooltipOpened, true);
        ...
        if (toplevel_window) {
            ...
            aura::Window::ConvertRectToTarget(tooltip_window_, toplevel_window, &bounds);
            observers_.Notify(&wm::TooltipObserver::OnTooltipShown, toplevel_window, widget_->GetTooltipView()->render_text()->text(), bounds);
        }
    }
}
...
void TooltipAura::Hide()
{
    if (widget_) {
        ...
        widget_->GetTooltipView()->NotifyAccessibilityEvent(ax::mojom::Event::kTooltipClosed, true);
