...
bool IsValidTarget(aura::Window* event_target, aura::Window* target)
{
    ...
    if (event_target->Contains(target) && event_target->GetBoundsInScreen().Contains(target->GetBoundsInScreen())) {
        return true;
    }

    void* event_target_grouping_id = event_target->GetNativeWindowProperty(TooltipManager::kGroupingPropertyKey);
    ...
    return event_target_grouping_id
        && (event_target_grouping_id == target->GetNativeWindowProperty(TooltipManager::kGroupingPropertyKey)
            || (toplevel_of_target && event_target_grouping_id == toplevel_of_target->GetNativeWindowProperty(TooltipManager::kGroupingPropertyKey)));
}
...
aura::Window* GetTooltipTarget(const ui::MouseEvent& event, gfx::Point* location)
{
    switch (event.type()) {
...
TooltipController::TooltipController(std::unique_ptr<Tooltip> tooltip, wm::ActivationClient* activation_client)
    : activation_client_(activation_client)
    , state_manager_(std::make_unique<TooltipStateManager>(std::move(tooltip)))
{
...
void TooltipController::UpdateTooltip(aura::Window* target)
{
    ...
    if (target && observed_window_ == target) {
        if (state_manager_->tooltip_parent_window() || IsTooltipTextUpdateNeeded()) {
...
void TooltipController::UpdateTooltipFromKeyboard(const gfx::Rect& bounds, aura::Window* target)
{
    UpdateTooltipFromKeyboardWithAnchorPoint(bounds.bottom_center(), target);
}

void TooltipController::UpdateTooltipFromKeyboardWithAnchorPoint(const gfx::Point& anchor_point, aura::Window* target)
{
    ...
    if (state_manager_->tooltip_parent_window()) {
        state_manager_->UpdatePositionIfNeeded(last_focus_loc_, TooltipTrigger::kKeyboard);
    }
...
{
    return target && target == state_manager_->tooltip_parent_window() && state_manager_->tooltip_trigger() == TooltipTrigger::kKeyboard;
}

void TooltipController::SetHideTooltipTimeout(aura::Window* target, base::TimeDelta timeout)
{
...
void TooltipController::OnMouseEvent(ui::MouseEvent* event)
{
    ...
    case ui::EventType::kMouseExited:
        ...
        if (state_manager_->tooltip_parent_window() && state_manager_->tooltip_trigger() == TooltipTrigger::kKeyboard) {
            return;
        }
        SetObservedWindow(nullptr);
        break;
    ...
    case ui::EventType::kMouseDragged: {
        ...
        state_manager_->UpdatePositionIfNeeded(last_mouse_loc_, TooltipTrigger::kCursor);
        SetObservedWindow(target);

        is_duplicate_pen_hover_event_ = IsDuplicatePenHoverEvent(event->pointer_details().pointer_type);

        if (state_manager_->IsVisible() || (observed_window_ && IsTooltipTextUpdateNeeded())) {
            UpdateIfRequired(TooltipTrigger::kCursor);
        }
        break;
    }
...
void TooltipController::OnCursorVisibilityChanged(bool is_visible)
{
    if (is_visible && !state_manager_->tooltip_parent_window()) {
        ...
        UpdateIfRequired(TooltipTrigger::kCursor);
    } else if (!is_visible && state_manager_->tooltip_parent_window() && state_manager_->tooltip_trigger() == TooltipTrigger::kCursor) {
...
void TooltipController::OnWindowPropertyChanged(aura::Window* window, const void* key, intptr_t old)
{
    if ((key == wm::kTooltipIdKey || key == wm::kTooltipTextKey) && wm::GetTooltipText(window) != std::u16string()
        && (IsTooltipTextUpdateNeeded() || IsTooltipIdUpdateNeeded())) {
        UpdateIfRequired(state_manager_->tooltip_trigger());
    }
}

void TooltipController::OnWindowActivated(ActivationReason reason, aura::Window* gained_active, aura::Window* lost_active)
{
...
void TooltipController::SetShowTooltipDelay(aura::Window* target, base::TimeDelta delay)
{
...
void TooltipController::UpdateIfRequired(TooltipTrigger trigger)
{
    ...
    if (!state_manager_->IsVisible() || IsTooltipTextUpdateNeeded() || IsTooltipIdUpdateNeeded()) {
        gfx::Point tooltip_point = ((trigger == TooltipTrigger::kCursor) ? last_mouse_loc_ : last_focus_loc_);
        state_manager_->Show(observed_window_, wm::GetTooltipText(observed_window_), tooltip_point, trigger, GetShowTooltipDelay(), GetHideTooltipDelay());
    }
}
...
bool TooltipController::IsDragDropInProgress() const
{
    if (!observed_window_)
        return false;
    aura::client::DragDropClient* client = aura::client::GetDragDropClient(observed_window_->GetRootWindow());
...
{
    std::map<aura::Window*, base::TimeDelta>::const_iterator it = show_tooltip_delay_map_.find(observed_window_);
    if (it == show_tooltip_delay_map_.end()) {
        return skip_show_delay_for_testing_ ? base::TimeDelta() : kDefaultShowTooltipDelay;
    }
    return it->second;
}
...
{
    std::map<aura::Window*, base::TimeDelta>::const_iterator it = hide_tooltip_timeout_map_.find(observed_window_);
...
void TooltipController::ResetWindowAtMousePressedIfNeeded(aura::Window* target, bool force_reset)
{
    ...
    if (force_reset
        || (tooltip_window_at_mouse_press() && target == tooltip_window_at_mouse_press() && wm::GetTooltipText(target) != tooltip_text_at_mouse_press_)) {
        tooltip_window_at_mouse_press_tracker_.RemoveAll();
    }
}
...
bool TooltipController::ShouldHideBecauseMouseWasOncePressed()
{
    ...
    return tooltip_window_at_mouse_press() && observed_window_ == tooltip_window_at_mouse_press()
        && wm::GetTooltipText(observed_window_) == tooltip_text_at_mouse_press_;
}

bool TooltipController::IsDuplicatePenHoverEvent(ui::EventPointerType pointer_type)
{
