...
void TooltipStateManager::Show(aura::Window* window, const std::u16string& tooltip_text, const gfx::Point& position, TooltipTrigger trigger,
    const base::TimeDelta show_delay, const base::TimeDelta hide_delay)
{
    ...
    std::u16string truncated_text = gfx::TruncateString(tooltip_text_, kMaxTooltipLength, gfx::WORD_BREAK);
...
void TooltipStateManager::UpdatePositionIfNeeded(const gfx::Point& position, TooltipTrigger trigger)
{
...
void TooltipStateManager::ShowNow(const std::u16string& trimmed_text, const base::TimeDelta hide_delay)
{
    ...
    tooltip_->Update(tooltip_parent_window_, trimmed_text, position_, tooltip_trigger_);
    tooltip_->Show();
    if (!hide_delay.is_zero()) {
        will_hide_tooltip_timer_.Start(FROM_HERE, hide_delay, this, &TooltipStateManager::HideAndReset);
    }
}

void TooltipStateManager::StartWillShowTooltipTimer(const std::u16string& trimmed_text, const base::TimeDelta show_delay, const base::TimeDelta hide_delay)
{
    if (!show_delay.is_zero()) {
        // Start will show tooltip timer is show_delay is non-zero.
        will_show_tooltip_timer_.Start(
            FROM_HERE, show_delay, base::BindOnce(&TooltipStateManager::ShowNow, weak_factory_.GetWeakPtr(), trimmed_text, hide_delay));
        return;
    } else {
