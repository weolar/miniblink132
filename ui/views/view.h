...
struct VIEWS_EXPORT ViewHierarchyChangedDetails {
    ViewHierarchyChangedDetails(bool is_add, View* parent, View* child, View* move_view)
        : is_add(is_add)
        , parent(parent)
        , child(child)
        , move_view(move_view)
...
public:
    ...
    public:
        explicit LayerOffsetData(float device_scale_factor = 1.f, const gfx::Vector2d& offset = gfx::Vector2d())
            : device_scale_factor_(device_scale_factor)
        {
            AddOffset(offset);
        }

        const gfx::Vector2d& offset() const
        {
            return offset_;
        }
    ...
    private:
        ...
        void AddOffset(const gfx::Vector2d& offset_to_parent)
        {
            ...
            gfx::Vector2dF fractional_pixel_offset(offset_to_parent.x() * device_scale_factor_, offset_to_parent.y() * device_scale_factor_);

            // Since pixels cannot be fractional, we need to round the offset to get
            // the correct physical pixel coordinate.
            gfx::Vector2d integral_pixel_offset = gfx::ToRoundedVector2d(fractional_pixel_offset);
    ...
    {
        owned_by_client_ = true;
    }
    bool owned_by_client() const
    {
        return owned_by_client_;
    }
    ...
    template <typename T> T* AddChildView(std::unique_ptr<T> view)
    {
        DCHECK(!view->owned_by_client()) << "This should only be called if the client is passing ownership of "
                                            "|view| to the parent View.";
        CHECK_CLASS_HAS_METADATA(T)
        return AddChildView<T>(view.release());
    }
    template <typename T> T* AddChildViewAt(std::unique_ptr<T> view, size_t index)
    {
        DCHECK(!view->owned_by_client()) << "This should only be called if the client is passing ownership of "
                                            "|view| to the parent View.";
        CHECK_CLASS_HAS_METADATA(T)
    ...
    // for new code.
    template <typename T> T* AddChildView(T* view)
    {
        CHECK_CLASS_HAS_METADATA(T)
        AddChildViewAtImpl(view, children_.size());
        return view;
    }
    template <typename T> T* AddChildViewAt(T* view, size_t index)
    {
        CHECK_CLASS_HAS_METADATA(T)
    ...
    template <typename T, base::RawPtrTraits Traits = base::RawPtrTraits::kEmpty> T* AddChildView(raw_ptr<T, Traits> view)
    {
        CHECK_CLASS_HAS_METADATA(T)
        AddChildViewAtImpl(view.get(), children_.size());
        return view;
    }
    template <typename T, base::RawPtrTraits Traits = base::RawPtrTraits::kEmpty> T* AddChildViewAt(raw_ptr<T, Traits> view, size_t index)
    {
        CHECK_CLASS_HAS_METADATA(T)
    ...
    template <typename T> std::unique_ptr<T> RemoveChildViewT(T* view)
    {
        DCHECK(!view->owned_by_client()) << "This should only be called if the client doesn't already have "
                                            "ownership of |view|.";
    ...
    template <typename T, base::RawPtrTraits Traits = base::RawPtrTraits::kEmpty> std::unique_ptr<T> RemoveChildViewT(raw_ptr<T, Traits> view)
    {
    ...
    [[nodiscard]] base::CallbackListSubscription AddVisibleChangedCallback(PropertyChangedCallback callback);
    ...
    [[nodiscard]] base::CallbackListSubscription AddEnabledChangedCallback(PropertyChangedCallback callback);
    ...
    std::vector<ui::Layer*> GetLayersInOrder(ViewLayer view_layer = ViewLayer::kInclude);
    ...
    void SetLayoutManagerUseConstrainedSpace(bool layout_manager_use_constrained_space);
    ...
    template <typename LayoutManager> LayoutManager* SetLayoutManager(std::unique_ptr<LayoutManager> layout_manager)
    {
    ...
    [[nodiscard]] base::CallbackListSubscription AddIDChangedCallback(PropertyChangedCallback callback);
    ...
    // will be invoked whenever the property changes.
    [[nodiscard]] base::CallbackListSubscription AddGroupChangedCallback(PropertyChangedCallback callback);
    ...
    [[nodiscard]] static gfx::Point ConvertPointToTarget(const View* source, const View* target, const gfx::Point& point);
    ...
    static void ConvertPointToTarget(const View* source, const View* target, gfx::Point* point);
    ...
    // the same view hierarchy.
    // Neither |source| nor |target| can be null.
    [[nodiscard]] static gfx::RectF ConvertRectToTarget(const View* source, const View* target, const gfx::RectF& rect);
    // The in-place version of this method is strongly discouraged, please use the
    // by-value version above for improved const-compatability and readability.
    static void ConvertRectToTarget(const View* source, const View* target, gfx::RectF* rect);
    ...
    // (0.00001f).
    static gfx::Rect ConvertRectToTarget(const View* source, const View* target, const gfx::Rect& rect);
    ...
    [[nodiscard]] static gfx::Point ConvertPointToScreen(const View* src, const gfx::Point& point);
    // The in-place version of this method is strongly discouraged, please use the
    // by-value version above for improved const-compatability and readability.
    ...
    // coordinate system.
    [[nodiscard]] static gfx::Point ConvertPointFromScreen(const View* src, const gfx::Point& point);
    // The in-place version of this method is strongly discouraged, please use the
    // by-value version above for improved const-compatability and readability.
    ...
    {
        return const_cast<ui::ColorProvider*>(std::as_const(*this).GetColorProvider());
    }
    ...
    [[nodiscard]] base::CallbackListSubscription AddFlipCanvasOnPaintForRTLUIChangedCallback(PropertyChangedCallback callback);
    ...
    // ViewTargeter.
    std::unique_ptr<ViewTargeter> SetEventTargeter(std::unique_ptr<ViewTargeter> targeter);
    ...
    void ConvertEventToTarget(const ui::EventTarget* target, ui::LocatedEvent* event) const override;
    ...
    // mac's native menu.
    virtual void ShowContextMenu(const gfx::Point& p, ui::mojom::MenuSourceType source_type);
    ...
    // support dropping.
    virtual bool GetDropFormats(int* formats, std::set<ui::ClipboardFormatType>* format_types);
    ...
    void SetAccessibleRole(const ax::mojom::Role role, const std::u16string& role_description);
    ...
    void SetAccessibleDescription(const std::u16string& description, ax::mojom::DescriptionFrom description_from);
    ...
    // problems.
    void NotifyAccessibilityEvent(ax::mojom::Event event_type, bool send_native_event);
    ...
    // to the new name.
    virtual void AdjustAccessibleName(std::u16string& new_name, ax::mojom::NameFrom& name_from)
    {
    }
    ...
    // view properties.
    base::CallbackListSubscription RegisterNotifyViewControllerCallback(base::RepeatingClosureList::CallbackType callback);
    ...
    LifeCycleState life_cycle_state() const
    {
        return life_cycle_state_;
    }
...
protected:
    ...
    // it exists.
    virtual gfx::Size CalculatePreferredSize(const SizeBounds& available_size) const;
    ...
    virtual LayerOffsetData CalculateOffsetToAncestorWithLayer(ui::Layer** layer_parent);
    ...
    // the right layer.
    void MoveLayerToParent(ui::Layer* parent_layer, const LayerOffsetData& offset_data);
    ...
    void OnLayerTransformed(const gfx::Transform& old_transform, ui::PropertyChangeReason reason) final;
    void OnLayerClipRectChanged(const gfx::Rect& old_rect, ui::PropertyChangeReason reason) override;
    ...
    template <typename Super, typename This>
        requires std::derived_from<Super, View> && std::derived_from<This, Super> && (!std::same_as<Super, This>)
    ...
    void OnPropertyChanged(ui::metadata::PropertyKey property, PropertyEffects property_effects);
...
private:
    ...
    void SetUpTransformRecorderForPainting(const gfx::Vector2d& offset_from_parent, ui::TransformRecorder* recorder) const;

    // Recursively calls the painting method |func| on all non-layered children,
    // in Z order.
    void RecursivePaintHelper(void (View::*func)(const PaintInfo&), const PaintInfo& info);
    ...
    void DoRemoveChildView(View* view, bool update_tool_tip, bool delete_removed_view, View* new_parent);
    ...
    // children.
    void PropagateRemoveNotifications(View* old_parent, View* new_parent, bool is_removed_from_widget);

    // Call ViewHierarchyChanged() for all children.
    // If is_added_to_widget is true, calls AddedToWidget for all children.
    void PropagateAddNotifications(const ViewHierarchyChangedDetails& details, bool is_added_to_widget);
    ...
    void SetLayerBounds(const gfx::Size& size_in_dip, const LayerOffsetData& layer_offset_data);
    ...
    void AddLayerToRegionImpl(ui::Layer* new_layer, std::vector<raw_ptr<ui::Layer, VectorExperimental>>& layer_vector);
    ...
    void PropagateDeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor);
    ...
    bool DoDrag(const ui::LocatedEvent& event, const gfx::Point& press_pt, ui::mojom::DragEventSource source);
    ...
    // The menu controller.
    raw_ptr<ContextMenuController, DanglingUntriaged> context_menu_controller_ = nullptr;
...
template <typename LayoutManager> BuilderT& SetLayoutManager(std::unique_ptr<LayoutManager> layout_manager) &
{
    auto setter = std::make_unique<::views::internal::PropertySetter<ViewClass_, std::unique_ptr<LayoutManager>,
        decltype((static_cast<LayoutManager* (ViewClass_::*)(std::unique_ptr<LayoutManager>)>(&ViewClass_::SetLayoutManager))), &ViewClass_::SetLayoutManager>>(
...
VIEW_BUILDER_OVERLOAD_METHOD(SetAccessibleName, std::u16string, ax::mojom::NameFrom)
...
VIEW_BUILDER_OVERLOAD_METHOD(SetAccessibleDescription, const std::u16string&, ax::mojom::DescriptionFrom)
VIEW_BUILDER_OVERLOAD_METHOD(SetAccessibleRole, ax::mojom::Role)
VIEW_BUILDER_OVERLOAD_METHOD(SetAccessibleRole, ax::mojom::Role, const std::u16string&)
