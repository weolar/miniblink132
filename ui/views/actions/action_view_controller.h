...
template <typename ViewT> class VIEWS_EXPORT ActionViewControllerTemplate : public ActionViewControllerBase {
public:
    ActionViewControllerTemplate() = default;
    ActionViewControllerTemplate(ViewT* view, base::WeakPtr<actions::ActionItem> action_item)
    {
        SetActionView(view);
        SetActionItem(action_item);
    }
    explicit ActionViewControllerTemplate(ViewT* view)
    {
        SetActionView(view);
    }
    ActionViewControllerTemplate(const ActionViewControllerTemplate&) = delete;
    ActionViewControllerTemplate& operator=(const ActionViewControllerTemplate&) = delete;
    ...
    void LinkActionItemAndView()
    {
        ...
        action_changed_subscription_
            = action_item->AddActionChangedCallback(base::BindRepeating(&ActionViewControllerTemplate<ViewT>::ActionItemChanged, base::Unretained(this)));
        ActionItemChanged();
    }
    ...
    void SetActionView(ViewT* action_view)
    {
        ...
        action_view->GetActionViewInterface()->LinkActionInvocationToView(
            base::BindRepeating(&ActionViewControllerTemplate::InvokeAction, base::Unretained(this)));
        view_changed_subscription_
            = action_view->RegisterNotifyViewControllerCallback(base::BindRepeating(&ActionViewControllerTemplate::OnViewChanged, base::Unretained(this)));
        LinkActionItemAndView();
    }
...
public:
    ...
    template <typename ViewT> void CreateActionViewRelationship(ViewT* view, base::WeakPtr<actions::ActionItem> action_item)
    {
        std::unique_ptr<ActionViewControllerTemplate<ViewT>> controller = std::make_unique<ActionViewControllerTemplate<ViewT>>(view, action_item);
        action_view_controller_templates_[view] = std::move(controller);
    }

    std::map<View*, std::unique_ptr<ActionViewControllerBase>> action_view_controller_templates_;
};
