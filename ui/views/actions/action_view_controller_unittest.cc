...
public:
    ...
    {
        action_item->InvokeAction(actions::ActionInvocationContext::Builder().SetProperty(kTestPropertyValueKey, true).Build());
    }
...
{
    return actions::ActionItem::Builder().SetText(kActionTextDisabled).SetActionId(kTestActionIdDisabled).SetVisible(true).SetEnabled(false).Build();
}

std::unique_ptr<actions::ActionItem> CreateEnabledActionItem()
{
    return actions::ActionItem::Builder().SetText(kActionTextEnabled).SetActionId(kTestActionIdEnabled).SetVisible(true).SetEnabled(true).Build();
}
...
{
    std::unique_ptr<actions::ActionItem> disabled_action_item = CreateDisabledActionItem();
    auto action_view = std::make_unique<MdTextButton>();
    auto action_view_controller = std::make_unique<ActionViewControllerTemplate<MdTextButton>>(action_view.get(), disabled_action_item->GetAsWeakPtr());
    EXPECT_EQ(action_view->GetText(), kActionTextDisabled);
    EXPECT_FALSE(action_view->GetEnabled());
    std::unique_ptr<actions::ActionItem> enabled_action_item = CreateEnabledActionItem();
...
TEST_F(ActionViewControllerTest, TestReassignActionView)
{
    ...
    auto action_view_controller = std::make_unique<ActionViewControllerTemplate<MdTextButton>>(first_action_view.get(), action_item->GetAsWeakPtr());
...
TEST_F(ActionViewControllerTest, TestActionViewDestroyed)
{
    std::unique_ptr<actions::ActionItem> action_item = CreateDisabledActionItem();
    auto action_view = std::make_unique<MdTextButton>();
    auto action_view_controller = std::make_unique<ActionViewControllerTemplate<MdTextButton>>(action_view.get(), action_item->GetAsWeakPtr());
    action_view.reset();
    action_item->SetEnabled(true);
}
...
{
    std::unique_ptr<Widget> test_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
    View* parent_view = test_widget->SetContentsView(std::make_unique<View>());
    MdTextButton* action_view = parent_view->AddChildView(std::make_unique<MdTextButton>());
    test_widget->Show();
    std::unique_ptr<actions::ActionItem> action_item = CreateEnabledActionItem();
    auto action_view_controller = std::make_unique<ActionViewControllerTemplate<MdTextButton>>(action_view, action_item->GetAsWeakPtr());
...
TEST_F(ActionViewControllerTest, TestCreateActionViewRelationship)
{
    auto first_action_view = std::make_unique<MdTextButton>();
    auto second_action_view = std::make_unique<MdTextButton>();
    std::unique_ptr<actions::ActionItem> first_action_item = CreateEnabledActionItem();
    std::unique_ptr<actions::ActionItem> second_action_item = CreateDisabledActionItem();
    ActionViewController action_view_controller = ActionViewController();
    action_view_controller.CreateActionViewRelationship(first_action_view.get(), first_action_item->GetAsWeakPtr());
    action_view_controller.CreateActionViewRelationship(second_action_view.get(), second_action_item->GetAsWeakPtr());
    ...
    action_view_controller.CreateActionViewRelationship(first_action_view.get(), second_action_item->GetAsWeakPtr());
...
{
    std::unique_ptr<actions::ActionItem> action_item = CreateEnabledActionItem();
    auto invoke_action_callback
        = [](actions::ActionItem* action_item, actions::ActionInvocationContext context) { EXPECT_EQ(context.GetProperty(kTestPropertyValueKey), true); };
    action_item->SetInvokeActionCallback(base::BindRepeating(invoke_action_callback));

    std::unique_ptr<Widget> test_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
    View* parent_view = test_widget->SetContentsView(std::make_unique<View>());
    TestActionButton* test_button = parent_view->AddChildView(std::make_unique<TestActionButton>());
    test_widget->Show();

    ActionViewController action_view_controller = ActionViewController();
    action_view_controller.CreateActionViewRelationship(test_button, action_item->GetAsWeakPtr());
    ui::MouseEvent e(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), 0, 0);
...
TEST_F(ActionViewControllerTest, TestOnViewChanged)
{
    std::unique_ptr<Widget> test_widget = CreateTestWidget(Widget::InitParams::CLIENT_OWNS_WIDGET);
    View* parent_view = test_widget->SetContentsView(std::make_unique<View>());
    TestActionButton* test_view = parent_view->AddChildView(std::make_unique<TestActionButton>());
    test_widget->Show();
    ...
    action_view_controller->CreateActionViewRelationship(test_view, action_item->GetAsWeakPtr());
