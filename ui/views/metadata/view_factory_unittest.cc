...
public:
    ...
    //                  if ever needed
    void ArbitraryMethod(int int_param, float float_param, views::PropertyEffects property_effects)
    {
    ...
    int get_int_param() const
    {
        return int_param_;
    }
    float get_float_param() const
    {
        return float_param_;
    }
...
TEST_F(ViewFactoryTest, TestViewBuilder)
{
    ...
    auto view = views::Builder<views::View>()
                    ...
                    .AddChildren(views::Builder<views::View>().SetEnabled(false).SetVisible(true).SetProperty(views::kMarginsKey, gfx::Insets(5)),
                        views::Builder<views::View>().SetGroup(5).SetID(1).SetFocusBehavior(views::View::FocusBehavior::NEVER),
                        views::Builder<views::LabelButton>().CopyAddressTo(&button).SetIsDefault(true).SetEnabled(true).SetText(u"Test"),
                        views::Builder<views::ScrollView>()
                            .CopyAddressTo(&scroll_view)
                            .SetContents(views::Builder<views::LabelButton>().CopyAddressTo(&scroll_button).SetText(u"ScrollTest"))
                            .SetHeader(views::Builder<views::View>().SetID(2)),
                        views::Builder<views::LabelButton>().CopyAddressTo(&view_with_layout_manager).SetLayoutManager(std::move(layout_manager)))
                    .Build();
    ...
    EXPECT_EQ(view->children()[1]->GetFocusBehavior(), views::View::FocusBehavior::NEVER);
...
TEST_F(ViewFactoryTest, TestViewBuilderOwnerships)
{
    ...
    view_builder.CopyAddressTo(&parent)
        ...
        .SetBackground(views::CreateSolidBackground(SK_ColorWHITE))
        .SetBorder(views::CreateEmptyBorder(0));
    view_builder.AddChild(views::Builder<views::View>().SetEnabled(false).SetVisible(true).SetProperty(views::kMarginsKey, gfx::Insets(5)));
    view_builder.AddChild(views::Builder<views::View>().SetGroup(5).SetID(1).SetFocusBehavior(views::View::FocusBehavior::NEVER));
    view_builder.AddChild(views::Builder<views::LabelButton>().CopyAddressTo(&button).SetIsDefault(true).SetEnabled(true).SetText(u"Test"));
...
{
    auto view = views::Builder<internal::TestView>().SetEnabled(false).ArbitraryMethod(10, 5.5, views::kPropertyEffectsLayout).Build();
...
{
    views::UniqueWidgetPtr widget = base::WrapUnique(CreateTopLevelPlatformWidget());
    auto* view = widget->GetContentsView()->AddChildView(views::Builder<internal::TestView>()
...
TEST_F(ViewFactoryTest, TestViewBuilderAddChildAtIndex)
{
    ...
    std::unique_ptr<views::View> view
        = views::Builder<views::View>()
              .CopyAddressTo(&parent)
              .AddChild(views::Builder<views::LabelButton>().CopyAddressTo(&cancel_button).SetIsDefault(false).SetEnabled(true).SetText(u"Cancel"))
              .AddChildAt(views::Builder<views::LabelButton>().CopyAddressTo(&ok_button).SetIsDefault(false).SetEnabled(true).SetText(u"OK"), 0)
              .Build();
...
TEST_F(ViewFactoryTest, TestOrderOfOperations)
{
...

    {
        ...
        EXPECT_CALL(custom_configure_callback, Run(testing::Pointer(view))).Times(2).WillRepeatedly(testing::Invoke([](views::View* view) {
        ...
        EXPECT_CALL(after_build_callback, Run(testing::Pointer(view))).Times(2).WillRepeatedly(testing::Invoke([](views::View* view) {
