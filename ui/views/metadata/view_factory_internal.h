...
template <typename T> class ViewClassTrait { };

template <typename T> class ViewClassTrait<Builder<T>> {
public:
    using ViewClass_ = T;
};
...
template <typename TClass, typename TValue, typename TSig, TSig Set, typename FType = typename std::remove_reference<TValue>::type>
...
template <typename TClass, typename TValue> class ClassPropertyValueSetter : public PropertySetterBase {
public:
    ClassPropertyValueSetter(const ui::ClassProperty<TValue>* property, TValue value)
        : property_(property)
        , value_(value)
    {
    }
...
template <typename TClass, typename TValue> class ClassPropertyMoveSetter : public PropertySetterBase {
public:
    ClassPropertyMoveSetter(const ui::ClassProperty<TValue*>* property, const TValue& value)
        : property_(property)
        , value_(value)
    {
    }
    ClassPropertyMoveSetter(const ui::ClassProperty<TValue*>* property, TValue&& value)
        : property_(property)
        , value_(std::move(value))
    {
    }
...
template <typename TClass, typename TValue> class ClassPropertyUniquePtrSetter : public PropertySetterBase {
public:
    ClassPropertyUniquePtrSetter(const ui::ClassProperty<TValue*>* property, std::unique_ptr<TValue> value)
        : property_(property)
        , value_(std::move(value))
    {
    }
    ClassPropertyUniquePtrSetter(const ClassPropertyUniquePtrSetter&) = delete;
    ClassPropertyUniquePtrSetter& operator=(const ClassPropertyUniquePtrSetter&) = delete;
...
template <typename TClass, typename TSig, TSig Set, typename... Args> class ClassMethodCaller : public PropertySetterBase {
public:
    ...
    void SetProperty(View* obj) override
    {
        std::apply(Set, std::tuple_cat(std::make_tuple(static_cast<TClass*>(obj)), std::move(args_)));
    }
...
protected:
    ...
    using ChildList = std::vector<std::pair<std::unique_ptr<ViewBuilderCore>, std::optional<size_t>>>;
...
template <typename TClass, typename TValue, typename TSig, TSig Set> class ViewBuilderSetter : public PropertySetterBase {
public:
