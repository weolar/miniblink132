...
template <typename Builder> class BaseViewBuilderT : public internal::ViewBuilderCore {
public:
    ...
    BaseViewBuilderT()
    {
        view_ = std::make_unique<ViewClass_>();
    }
    explicit BaseViewBuilderT(std::unique_ptr<ViewClass_> view)
    {
        view_ = std::move(view);
    }
    explicit BaseViewBuilderT(ViewClass_* root_view)
    ...
    Builder& AfterBuild(AfterBuildCallback after_build_callback) &
    {
        ...
        if (after_build_callback_) {
            after_build_callback_ = base::BindOnce(
                [](AfterBuildCallback previous_callback, AfterBuildCallback current_callback, ViewClass_* root_view) {
    ...
    template <typename ViewPtr> Builder& CopyAddressTo(ViewPtr* view_address) &
    {
    ...
    Builder& CustomConfigure(ConfigureCallback configure_callback) &
    {
        ...
        if (configure_callback_) {
            configure_callback_ = base::BindOnce(
                [](ConfigureCallback current_callback, ConfigureCallback previous_callback, ViewClass_* root_view) {
                    std::move(current_callback).Run(root_view);
                    std::move(previous_callback).Run(root_view);
                },
    ...
    template <typename Child> Builder& AddChild(Child&& child) &
    {
    ...
    template <typename Child> Builder& AddChildAt(Child&& child, size_t index) &
    {
    ...
    template <typename Child, typename... Types> Builder& AddChildren(Child&& child, Types&&... args) &
    {
    ...
    template <typename T> Builder& SetProperty(const ui::ClassProperty<T>* property, ui::metadata::ArgType<T> value) &
    {
        auto setter = std::make_unique<internal::ClassPropertyValueSetter<ViewClass_, T>>(property, value);
    ...
    template <typename T> Builder& SetProperty(const ui::ClassProperty<T*>* property, ui::metadata::ArgType<T> value) &
    {
        auto setter = std::make_unique<internal::ClassPropertyMoveSetter<ViewClass_, T>>(property, value);
        internal::ViewBuilderCore::AddPropertySetter(std::move(setter));
        return *static_cast<Builder*>(this);
    }
    ...
    template <typename T> Builder& SetProperty(const ui::ClassProperty<T*>* property, T&& value) &
    {
        auto setter = std::make_unique<internal::ClassPropertyMoveSetter<ViewClass_, T>>(property, std::move(value));
        internal::ViewBuilderCore::AddPropertySetter(std::move(setter));
        return *static_cast<Builder*>(this);
    }
    ...
    template <typename T> Builder& SetProperty(const ui::ClassProperty<T*>* property, std::unique_ptr<T> value) &
    {
        auto setter = std::make_unique<internal::ClassPropertyUniquePtrSetter<ViewClass_, T>>(property, std::move(value));
        internal::ViewBuilderCore::AddPropertySetter(std::move(setter));
        return *static_cast<Builder*>(this);
    }
...
protected:
    ...
    template <typename... Args> Builder& AddChildrenImpl(Args*... args) &
    {
    ...
    std::unique_ptr<View> DoBuild() override
    {
...
#define DECL_PARAMS(...) BASE_CONCAT(DECL_PARAM, NUM_ARGS(__VA_ARGS__))(__VA_ARGS__)
...
#define PASS_PARAMS(...) BASE_CONCAT(PASS_PARAM, NUM_ARGS(__VA_ARGS__))(__VA_ARGS__)
...
#define BEGIN_VIEW_BUILDER(export, view_class, ancestor)                                                                                                       \
    template <typename BuilderT> class export view_class##BuilderT : public ancestor##BuilderT<BuilderT>                                                       \
    {                                                                                                                                                          \
    private:                                                                                                                                                   \
    ...
    public:                                                                                                                                                    \
        ...
        explicit view_class##BuilderT(typename ::views::internal::ViewClassTrait<BuilderT>::ViewClass_* root_view)                                             \
            : ancestor##BuilderT<BuilderT>(root_view)                                                                                                          \
        {                                                                                                                                                      \
        }                                                                                                                                                      \
        explicit view_class##BuilderT(std::unique_ptr<typename ::views::internal::ViewClassTrait<BuilderT>::ViewClass_> view)                                  \
...
#define VIEW_BUILDER_PROPERTY2(property_type, property_name)                                                                                                   \
    ...
    {                                                                                                                                                          \
        auto setter = std::make_unique<                                                                                                                        \
            ::views::internal::PropertySetter<ViewClass_, property_type, decltype(&ViewClass_::Set##property_name), &ViewClass_::Set##property_name>>(         \
            std::move(value));                                                                                                                                 \
...
#define VIEW_BUILDER_PROPERTY3(property_type, property_name, field_type)                                                                                       \
    BuilderT& Set##property_name(::ui::metadata::ArgType<property_type> value)&                                                                                \
    {                                                                                                                                                          \
        auto setter = std::make_unique<::views::internal::PropertySetter<ViewClass_, property_type, decltype(&ViewClass_::Set##property_name),                 \
...
#define VIEW_BUILDER_OVERLOAD_METHOD_CLASS(class_name, method_name, ...)                                                                                       \
    ...
    {                                                                                                                                                          \
        auto caller = std::make_unique<::views::internal::ClassMethodCaller<ViewClass_,                                                                        \
            decltype((static_cast<void (class_name::*)(__VA_ARGS__)>(&ViewClass_::method_name))), &class_name::method_name, __VA_ARGS__>>(                     \
...
#define VIEW_BUILDER_OVERLOAD_METHOD(method_name, ...) VIEW_BUILDER_OVERLOAD_METHOD_CLASS(ViewClass_, method_name, __VA_ARGS__)

#define VIEW_BUILDER_METHOD(method_name, ...)                                                                                                                  \
    template <typename... Args> BuilderT& method_name(Args&&... args)&                                                                                         \
    {                                                                                                                                                          \
        auto caller                                                                                                                                            \
            = std::make_unique<::views::internal::ClassMethodCaller<ViewClass_, decltype(&ViewClass_::method_name), &ViewClass_::method_name, __VA_ARGS__>>(   \
...
#define VIEW_BUILDER_METHOD_ALIAS(builder_method, view_method, ...)                                                                                            \
    template <typename... Args> BuilderT& builder_method(Args&&... args)&                                                                                      \
    {                                                                                                                                                          \
        auto caller                                                                                                                                            \
            = std::make_unique<::views::internal::ClassMethodCaller<ViewClass_, decltype(&ViewClass_::view_method), &ViewClass_::view_method, __VA_ARGS__>>(   \
                std::forward<Args>(args)...);                                                                                                                  \
        ::views::internal::ViewBuilderCore::AddPropertySetter(std::move(caller));                                                                              \
        return *static_cast<BuilderT*>(this);                                                                                                                  \
    }                                                                                                                                                          \
...
#define VIEW_BUILDER_VIEW_TYPE_PROPERTY(property_type, property_name)                                                                                          \
    template <typename _View> BuilderT& Set##property_name(_View&& view)&                                                                                      \
    {                                                                                                                                                          \
        auto setter = std::make_unique<::views::internal::ViewBuilderSetter<ViewClass_, property_type,                                                         \
            decltype(&ViewClass_::Set##property_name<property_type>), &ViewClass_::Set##property_name<property_type>>>(view.Release());                        \
        ::views::internal::ViewBuilderCore::AddPropertySetter(std::move(setter));                                                                              \
        return *static_cast<BuilderT*>(this);                                                                                                                  \
    }                                                                                                                                                          \
    template <typename _View> BuilderT&& Set##property_name(_View&& view)&&                                                                                    \
...
#define VIEW_BUILDER_VIEW_PROPERTY(property_type, property_name)                                                                                               \
    ...
    {                                                                                                                                                          \
        auto setter = std::make_unique<                                                                                                                        \
            ::views::internal::ViewBuilderSetter<ViewClass_, property_type, decltype(&ViewClass_::Set##property_name), &ViewClass_::Set##property_name>>(      \
            view.Release());                                                                                                                                   \
        ::views::internal::ViewBuilderCore::AddPropertySetter(std::move(setter));                                                                              \
        return *static_cast<BuilderT*>(this);                                                                                                                  \
    }                                                                                                                                                          \
...
#define VIEW_BUILDER_PROPERTY_DEFAULT(property_type, property_name, default)                                                                                   \
    BuilderT& Set##property_name(::ui::metadata::ArgType<property_type> value = default)&                                                                      \
    {                                                                                                                                                          \
        auto setter = std::make_unique<                                                                                                                        \
            ::views::internal::PropertySetter<ViewClass_, property_type, decltype(&ViewClass_::Set##property_name), &ViewClass_::Set##property_name>>(         \
            std::move(value));                                                                                                                                 \
...
#define DEFINE_VIEW_BUILDER(export, view_class)                                                                                                                \
    namespace views {                                                                                                                                          \
    template <> class export Builder<view_class> : public view_class##BuilderT<Builder<view_class>>                                                            \
    {                                                                                                                                                          \
    private:                                                                                                                                                   \
        using ViewClass_ = view_class;                                                                                                                         \
                                                                                                                                                               \
    public:                                                                                                                                                    \
        ...
        ~Builder() = default;                                                                                                                                  \
        [[nodiscard]] std::unique_ptr<internal::ViewBuilderCore> Release() override                                                                            \
        {                                                                                                                                                      \
