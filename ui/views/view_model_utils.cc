...
{
    return base::ranges::all_of(model.entries(), [](const ViewModelBase::Entry& entry) { return entry.view->bounds() == entry.ideal_bounds; });
}

// static
size_t ViewModelUtils::DetermineMoveIndex(const ViewModelBase& model, View* view, bool is_horizontal, int x, int y)
{
    ...
    for (; iter->view != view; ++iter) {
        const int mid_point = primary_axis_coordinate(is_horizontal, iter->ideal_bounds.CenterPoint());
        if (value < mid_point)
    ...
    const int delta = primary_axis_coordinate(is_horizontal, std::next(iter)->ideal_bounds.origin() - iter->ideal_bounds.origin().OffsetFromOrigin());

    for (++iter; iter != entries.end(); ++iter) {
        const int mid_point = primary_axis_coordinate(is_horizontal, iter->ideal_bounds.CenterPoint()) - delta;
        if (value < mid_point)
