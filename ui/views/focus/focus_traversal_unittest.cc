...
public:
    explicit BorderView(std::unique_ptr<View> child)
...
protected:
    ...
    void AdvanceEntireFocusLoop(base::span<const int> traversal_ids, bool reverse)
    {
        for (size_t i = 0; i < 3; ++i) {
            for (size_t j = 0; j < traversal_ids.size(); j++) {
                SCOPED_TRACE(testing::Message() << "reverse:" << reverse << " i:" << i << " j:" << j);
                GetFocusManager()->AdvanceFocus(reverse);
                View* focused_view = GetFocusManager()->GetFocusedView();
                EXPECT_NE(nullptr, focused_view);
                if (focused_view)
                    EXPECT_EQ(traversal_ids[reverse ? traversal_ids.size() - j - 1 : j], focused_view->GetID());
            }
        }
    }
...
private:
    ...
    void ReverseChildrenFocusOrderImpl(View* parent, base::flat_set<View*> seen_views = {})
    {
        std::vector<raw_ptr<View, VectorExperimental>> children_views = parent->children();
...
void FocusTraversalTest::InitContentView()
{
    ...
    text_field_ptr->SetBounds(label_x + label_width + 5, y, text_field_width, label_height);
    ...
    auto button = std::make_unique<MdTextButton>(Button::PressedCallback(), u"Click me");
    ...
    auto* radio_button_ptr = right_container_->AddChildView(std::move(radio_button));
    ...
    inner_container->SetBackground(CreateSolidBackground(SkColorSetRGB(230, 230, 230)));
    inner_container->SetID(INNER_CONTAINER_ID);
    auto* inner_container_ptr = right_container_->AddChildView(std::move(inner_container));
    inner_container_ptr->SetBounds(100, 10, 150, 180);

    auto scroll_view = std::make_unique<ScrollView>();
    scroll_view->SetID(SCROLL_VIEW_ID);
    auto* scroll_view_ptr = inner_container_ptr->AddChildView(std::move(scroll_view));
    scroll_view_ptr->SetBounds(1, 1, 148, 178);

    auto scroll_content = std::make_unique<View>();
    scroll_content->SetBounds(0, 0, 200, 200);
    scroll_content->SetBackground(CreateSolidBackground(SkColorSetRGB(200, 200, 200)));
    auto* scroll_content_ptr = scroll_view_ptr->SetContents(std::move(scroll_content));

    constexpr auto kTitles = std::to_array<const char* const>({ "Rosetta", "Stupeur et tremblement", "The diner game", "Ridicule", "Le placard",
        "Les Visiteurs", "Amelie", "Joyeux Noel", "Camping", "Brice de Nice", "Taxi", "Asterix" });

    constexpr auto kIDs = std::to_array<int>({ ROSETTA_LINK_ID, STUPEUR_ET_TREMBLEMENT_LINK_ID, DINER_GAME_LINK_ID, RIDICULE_LINK_ID, CLOSET_LINK_ID,
        VISITING_LINK_ID, AMELIE_LINK_ID, JOYEUX_NOEL_LINK_ID, CAMPING_LINK_ID, BRICE_DE_NICE_LINK_ID, TAXI_LINK_ID, ASTERIX_LINK_ID });
    ...
    auto search_border_view = std::make_unique<BorderView>(std::move(border_contents));
    search_border_view->SetID(SEARCH_CONTAINER_ID);
    search_border_view_ = GetContentsView()->AddChildView(std::move(search_border_view));
    ...
    button = std::make_unique<MdTextButton>(Button::PressedCallback(), u"SuperStar");
    button->SetBounds(60, 5, 100, 30);
    button->SetID(THUMBNAIL_SUPER_STAR_ID);
    view_contents->AddChildView(std::move(button));

    auto* contents_ptr = GetContentsView()->AddChildView(std::move(view_contents));
...
TEST_F(FocusTraversalTest, NormalTraversal)
{
    constexpr auto kTraversalIDs = std::to_array<int>(
        { TOP_CHECKBOX_ID, APPLE_TEXTFIELD_ID, ORANGE_TEXTFIELD_ID, BANANA_TEXTFIELD_ID, KIWI_TEXTFIELD_ID, FRUIT_BUTTON_ID, FRUIT_CHECKBOX_ID, COMBOBOX_ID,
            BROCCOLI_BUTTON_ID, ROSETTA_LINK_ID, STUPEUR_ET_TREMBLEMENT_LINK_ID, DINER_GAME_LINK_ID, RIDICULE_LINK_ID, CLOSET_LINK_ID, VISITING_LINK_ID,
            AMELIE_LINK_ID, JOYEUX_NOEL_LINK_ID, CAMPING_LINK_ID, BRICE_DE_NICE_LINK_ID, TAXI_LINK_ID, ASTERIX_LINK_ID, OK_BUTTON_ID, CANCEL_BUTTON_ID,
            HELP_BUTTON_ID, STYLE_CONTAINER_ID, BOLD_CHECKBOX_ID, ITALIC_CHECKBOX_ID, UNDERLINED_CHECKBOX_ID, STYLE_HELP_LINK_ID, STYLE_TEXT_EDIT_ID,
            SEARCH_TEXTFIELD_ID, SEARCH_BUTTON_ID, HELP_LINK_ID, THUMBNAIL_CONTAINER_ID, THUMBNAIL_STAR_ID, THUMBNAIL_SUPER_STAR_ID });
...
TEST_F(FocusTraversalTest, NormalTraversalMac)
{
    ...
    const int kTraversalIDs[]
        = { APPLE_TEXTFIELD_ID, ORANGE_TEXTFIELD_ID, BANANA_TEXTFIELD_ID, KIWI_TEXTFIELD_ID, STYLE_TEXT_EDIT_ID, SEARCH_TEXTFIELD_ID, THUMBNAIL_CONTAINER_ID };
...
TEST_F(FocusTraversalTest, FullKeyboardToggle)
{
    ...
    // focus behavior.
    GetFocusManager()->SetKeyboardAccessible(false);
    EXPECT_EQ(THUMBNAIL_CONTAINER_ID, GetFocusManager()->GetFocusedView()->GetID());
...
{
    constexpr auto kDisabledIDs
        = std::to_array<int>({ BANANA_TEXTFIELD_ID, FRUIT_CHECKBOX_ID, COMBOBOX_ID, ASPARAGUS_BUTTON_ID, CAULIFLOWER_BUTTON_ID, CLOSET_LINK_ID,
            VISITING_LINK_ID, BRICE_DE_NICE_LINK_ID, TAXI_LINK_ID, ASTERIX_LINK_ID, HELP_BUTTON_ID, BOLD_CHECKBOX_ID, SEARCH_TEXTFIELD_ID, HELP_LINK_ID });

    constexpr auto kTraversalIDs = std::to_array<int>({ TOP_CHECKBOX_ID, APPLE_TEXTFIELD_ID, ORANGE_TEXTFIELD_ID, KIWI_TEXTFIELD_ID, FRUIT_BUTTON_ID,
        BROCCOLI_BUTTON_ID, ROSETTA_LINK_ID, STUPEUR_ET_TREMBLEMENT_LINK_ID, DINER_GAME_LINK_ID, RIDICULE_LINK_ID, AMELIE_LINK_ID, JOYEUX_NOEL_LINK_ID,
        CAMPING_LINK_ID, OK_BUTTON_ID, CANCEL_BUTTON_ID, STYLE_CONTAINER_ID, ITALIC_CHECKBOX_ID, UNDERLINED_CHECKBOX_ID, STYLE_HELP_LINK_ID, STYLE_TEXT_EDIT_ID,
        SEARCH_BUTTON_ID, THUMBNAIL_CONTAINER_ID, THUMBNAIL_STAR_ID, THUMBNAIL_SUPER_STAR_ID });
...
{
    const int kInvisibleIDs[] = { TOP_CHECKBOX_ID, OK_BUTTON_ID, THUMBNAIL_CONTAINER_ID };

    const int kTraversalIDs[] = { APPLE_TEXTFIELD_ID, ORANGE_TEXTFIELD_ID, BANANA_TEXTFIELD_ID, KIWI_TEXTFIELD_ID, FRUIT_BUTTON_ID, FRUIT_CHECKBOX_ID,
        COMBOBOX_ID, BROCCOLI_BUTTON_ID, ROSETTA_LINK_ID, STUPEUR_ET_TREMBLEMENT_LINK_ID, DINER_GAME_LINK_ID, RIDICULE_LINK_ID, CLOSET_LINK_ID,
        VISITING_LINK_ID, AMELIE_LINK_ID, JOYEUX_NOEL_LINK_ID, CAMPING_LINK_ID, BRICE_DE_NICE_LINK_ID, TAXI_LINK_ID, ASTERIX_LINK_ID, CANCEL_BUTTON_ID,
        HELP_BUTTON_ID, STYLE_CONTAINER_ID, BOLD_CHECKBOX_ID, ITALIC_CHECKBOX_ID, UNDERLINED_CHECKBOX_ID, STYLE_HELP_LINK_ID, STYLE_TEXT_EDIT_ID,
        SEARCH_TEXTFIELD_ID, SEARCH_BUTTON_ID, HELP_LINK_ID };
...
TEST_F(FocusTraversalTest, PaneTraversal)
{
    ...
    constexpr auto kLeftTraversalIDs = std::to_array<int>(
        { APPLE_TEXTFIELD_ID, ORANGE_TEXTFIELD_ID, BANANA_TEXTFIELD_ID, KIWI_TEXTFIELD_ID, FRUIT_BUTTON_ID, FRUIT_CHECKBOX_ID, COMBOBOX_ID });
    ...
    constexpr auto kRightTraversalIDs = std::to_array<int>({ BROCCOLI_BUTTON_ID, DINER_GAME_LINK_ID, RIDICULE_LINK_ID, CLOSET_LINK_ID, VISITING_LINK_ID,
        AMELIE_LINK_ID, JOYEUX_NOEL_LINK_ID, CAMPING_LINK_ID, BRICE_DE_NICE_LINK_ID, TAXI_LINK_ID, ASTERIX_LINK_ID });
    ...
    FindViewByID(STUPEUR_ET_TREMBLEMENT_LINK_ID)->SetFocusBehavior(View::FocusBehavior::NEVER);
    FindViewByID(DINER_GAME_LINK_ID)->SetFocusBehavior(View::FocusBehavior::ACCESSIBLE_ONLY);
...
TEST_F(FocusTraversalTest, TraversesFocusInFocusOrder)
{
    ...
    constexpr auto kTraversalIDs = std::to_array<int>({ THUMBNAIL_CONTAINER_ID, THUMBNAIL_SUPER_STAR_ID, THUMBNAIL_STAR_ID,
        ...
        // children.
        SEARCH_TEXTFIELD_ID, SEARCH_BUTTON_ID, HELP_LINK_ID, STYLE_TEXT_EDIT_ID, STYLE_HELP_LINK_ID, UNDERLINED_CHECKBOX_ID, ITALIC_CHECKBOX_ID,
        BOLD_CHECKBOX_ID, STYLE_CONTAINER_ID, HELP_BUTTON_ID, CANCEL_BUTTON_ID, OK_BUTTON_ID, ASTERIX_LINK_ID, TAXI_LINK_ID, BRICE_DE_NICE_LINK_ID,
        CAMPING_LINK_ID, JOYEUX_NOEL_LINK_ID, AMELIE_LINK_ID, VISITING_LINK_ID, CLOSET_LINK_ID, RIDICULE_LINK_ID, DINER_GAME_LINK_ID,
        STUPEUR_ET_TREMBLEMENT_LINK_ID, ROSETTA_LINK_ID, BROCCOLI_BUTTON_ID, COMBOBOX_ID, FRUIT_CHECKBOX_ID, FRUIT_BUTTON_ID, KIWI_TEXTFIELD_ID,
        BANANA_TEXTFIELD_ID, ORANGE_TEXTFIELD_ID, APPLE_TEXTFIELD_ID, TOP_CHECKBOX_ID });
...
public:
    FocusTraversalNonFocusableTest(const FocusTraversalNonFocusableTest&) = delete;
    FocusTraversalNonFocusableTest& operator=(const FocusTraversalNonFocusableTest&) = delete;
