...
FocusManager::FocusManager(Widget* widget, std::unique_ptr<FocusManagerDelegate> delegate)
    : widget_(widget)
...
bool FocusManager::OnKeyEvent(const ui::KeyEvent& event)
{
    ...
    if (event.type() == ui::EventType::kKeyPressed) {
        ...
        if (IsArrowKeyTraversalEnabledForWidget() && ProcessArrowKeyTraversal(event)) {
            return false;
        }
        ...
        if (focused_view_ && focused_view_->GetGroup() != -1 && (is_left || is_right)) {
            bool next = is_right;
            View::Views views;
            ...
            std::erase_if(views, [this](View* v) { return v != focused_view_ && !v->GetViewAccessibility().IsAccessibilityFocusable(); });
            ...
            SetFocusedViewWithReason(views[index], FocusChangeReason::kFocusTraversal);
            return false;
        }
    }
...
void FocusManager::AdvanceFocus(bool reverse)
{
    ...
    if (v) {
        ...
        // FocusManager.
        DCHECK(v->GetWidget());
        v->GetWidget()->GetFocusManager()->SetFocusedViewWithReason(v, FocusChangeReason::kFocusTraversal);
...
bool FocusManager::RotatePaneFocus(Direction direction, FocusCycleWrapping wrapping)
{
    return widget_->widget_delegate()->RotatePaneFocusFromView(GetFocusedView(), direction == Direction::kForward, wrapping == FocusCycleWrapping::kEnabled);
}

View* FocusManager::GetNextFocusableView(View* original_starting_view, Widget* starting_widget, bool reverse, bool dont_loop)
{
    DCHECK(!focused_view_ || ContainsView(focused_view_)) << " focus_view=" << focused_view_;
    ...
    while (parent_focus_traversable) {
        ...
        auto check_starting_view = reverse ? FocusSearch::StartingViewPolicy::kCheckStartingView : FocusSearch::StartingViewPolicy::kSkipStartingView;
        v = parent_focus_traversable->GetFocusSearch()->FindNextFocusableView(starting_view,
            reverse ? FocusSearch::SearchDirection::kBackwards : FocusSearch::SearchDirection::kForwards, FocusSearch::TraversalDirection::kUp,
            check_starting_view, FocusSearch::AnchoredDialogPolicy::kSkipAnchoredDialog, &new_focus_traversable, &new_starting_view);
    ...
    Widget* widget = starting_view ? starting_view->GetWidget() : original_starting_view->GetWidget();
...
void FocusManager::SetFocusedViewWithReason(View* view, FocusChangeReason reason)
{
    ...
    focus_change_listeners_.Notify(&FocusChangeListener::OnWillChangeFocus, focused_view_, view);

    View* old_focused_view = focused_view_;
    focused_view_ = view;
    base::AutoReset<int> entrance_count_resetter(&setting_focused_view_entrance_count_, setting_focused_view_entrance_count_ + 1);
    ...
    focus_change_listeners_.Notify(&FocusChangeListener::OnDidChangeFocus, old_focused_view, focused_view_);
}
...
bool FocusManager::RestoreFocusedView()
{
    ...
    if (view) {
        if (ContainsView(view)) {
            if (!view->IsFocusable() && view->GetViewAccessibility().IsAccessibilityFocusable()) {
...
View* FocusManager::FindFocusableView(FocusTraversable* focus_traversable, View* starting_view, bool reverse)
{
    FocusTraversable* new_focus_traversable = nullptr;
    View* new_starting_view = nullptr;
    const FocusSearch::AnchoredDialogPolicy can_go_into_anchored_dialog = FocusSearch::AnchoredDialogPolicy::kCanGoIntoAnchoredDialog;
    const FocusSearch::SearchDirection search_direction = reverse ? FocusSearch::SearchDirection::kBackwards : FocusSearch::SearchDirection::kForwards;
    View* v = nullptr;

    // Let's go down the FocusTraversable tree as much as we can.
    do {
        v = focus_traversable->GetFocusSearch()->FindNextFocusableView(starting_view, search_direction, FocusSearch::TraversalDirection::kDown,
            FocusSearch::StartingViewPolicy::kSkipStartingView, can_go_into_anchored_dialog, &new_focus_traversable, &new_starting_view);
...
void FocusManager::RegisterAccelerator(const ui::Accelerator& accelerator, ui::AcceleratorManager::HandlerPriority priority, ui::AcceleratorTarget* target)
{
    accelerator_manager_.Register({ accelerator }, priority, target);
}

void FocusManager::UnregisterAccelerator(const ui::Accelerator& accelerator, ui::AcceleratorTarget* target)
{
...
bool FocusManager::IsAcceleratorRegistered(const ui::Accelerator& accelerator) const
{
    return accelerator_manager_.IsRegistered(accelerator);
}

bool FocusManager::HasPriorityHandler(const ui::Accelerator& accelerator) const
{
...
{
    return key_event.key_code() == ui::VKEY_TAB && (!key_event.IsControlDown() && !key_event.IsAltDown());
}
...
bool FocusManager::ProcessArrowKeyTraversal(const ui::KeyEvent& event)
{
    ...
    if (key != ui::VKEY_UP && key != ui::VKEY_DOWN && key != ui::VKEY_LEFT && key != ui::VKEY_RIGHT) {
        return false;
    }

    const ui::KeyboardCode reverse = base::i18n::IsRTL() ? ui::VKEY_RIGHT : ui::VKEY_LEFT;
...
#if BUILDFLAG(IS_MAC)
    return keyboard_accessible_ ? view->GetViewAccessibility().IsAccessibilityFocusable() : view->IsFocusable();
#else
...
bool FocusManager::RedirectAcceleratorToBubbleAnchorWidget(const ui::Accelerator& accelerator)
{
    if (!widget_->widget_delegate())
        return false;

    views::BubbleDialogDelegate* widget_delegate = widget_->widget_delegate()->AsBubbleDialogDelegate();
    Widget* anchor_widget = widget_delegate ? widget_delegate->anchor_widget() : nullptr;
    if (!anchor_widget)
        return false;
    ...
    const bool close_widget_on_deactivate = widget_delegate->ShouldCloseOnDeactivate();
#endif

    // The parent view must be focused for it to process events.
    focus_manager->SetFocusedView(anchor_widget->GetRootView());
    const bool accelerator_processed = focus_manager->ProcessAccelerator(accelerator);
...
bool FocusManager::IsArrowKeyTraversalEnabledForWidget() const
{
    ...
    Widget* const widget = (focused_view_ && focused_view_->GetWidget()) ? focused_view_->GetWidget() : widget_.get();
    return widget && widget->widget_delegate() && widget->widget_delegate()->enable_arrow_key_traversal();
}
