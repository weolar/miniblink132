...
TEST_F(FocusManagerTest, ViewFocusCallbacks)
{
    ...
    EXPECT_EQ(FocusChangeReason::kDirectFocusChange, event_list->vec[0].focus_change_reason);
    ...
    EXPECT_EQ(FocusChangeReason::kDirectFocusChange, event_list->vec[0].focus_change_reason);
    EXPECT_EQ(FocusChangeReason::kDirectFocusChange, event_list->vec[1].focus_change_reason);
...
TEST_F(FocusManagerTest, CallsNormalAcceleratorTarget)
{
    ...
    // Register targets.
    focus_manager->RegisterAccelerator(return_accelerator, ui::AcceleratorManager::kNormalPriority, &return_target);
    focus_manager->RegisterAccelerator(escape_accelerator, ui::AcceleratorManager::kNormalPriority, &escape_target);
    ...
    focus_manager->RegisterAccelerator(return_accelerator, ui::AcceleratorManager::kNormalPriority, &return_target2);
    ...
    focus_manager->RegisterAccelerator(return_accelerator, ui::AcceleratorManager::kNormalPriority, &return_target3);
    // Hitting the return key.
...
TEST_F(FocusManagerTest, HighPriorityHandlers)
{
    ...
    focus_manager->RegisterAccelerator(escape_accelerator, ui::AcceleratorManager::kHighPriority, &escape_target_high);
    ...
    focus_manager->RegisterAccelerator(escape_accelerator, ui::AcceleratorManager::kNormalPriority, &escape_target_normal);
    ...
    focus_manager->UnregisterAccelerator(escape_accelerator, &escape_target_normal);
    EXPECT_TRUE(focus_manager->HasPriorityHandler(escape_accelerator));
...
TEST_F(FocusManagerTest, CallsEnabledAcceleratorTargetsOnly)
{
    ...
    focus_manager->RegisterAccelerator(return_accelerator, ui::AcceleratorManager::kNormalPriority, &return_target1);
    focus_manager->RegisterAccelerator(return_accelerator, ui::AcceleratorManager::kNormalPriority, &return_target2);
    EXPECT_TRUE(focus_manager->ProcessAccelerator(return_accelerator));
...
public:
    SelfUnregisteringAcceleratorTarget(const ui::Accelerator& accelerator, FocusManager* focus_manager)
        : accelerator_(accelerator)
        , focus_manager_(focus_manager)
    {
    }

    SelfUnregisteringAcceleratorTarget(const SelfUnregisteringAcceleratorTarget&) = delete;
    SelfUnregisteringAcceleratorTarget& operator=(const SelfUnregisteringAcceleratorTarget&) = delete;
...
TEST_F(FocusManagerTest, CallsSelfDeletingAcceleratorTarget)
{
    ...
    // Register the target.
    focus_manager->RegisterAccelerator(return_accelerator, ui::AcceleratorManager::kNormalPriority, &target);
...
{
    const ui::KeyEvent event(ui::EventType::kKeyPressed, ui::VKEY_RETURN, ui::EF_NONE);
    ui::Accelerator accelerator(event.key_code(), event.flags());
    ui::TestAcceleratorTarget target(true);
    FocusManager* focus_manager = GetFocusManager();
    focus_manager->RegisterAccelerator(accelerator, ui::AcceleratorManager::kNormalPriority, &target);
...
public:
    ...
    FocusInAboutToRequestFocusFromTabTraversalView(const FocusInAboutToRequestFocusFromTabTraversalView&) = delete;
    FocusInAboutToRequestFocusFromTabTraversalView& operator=(const FocusInAboutToRequestFocusFromTabTraversalView&) = delete;

    void set_view_to_focus(View* view)
    {
        view_to_focus_ = view;
    }
...
TEST_F(FocusManagerTest, RotateFocus)
{
    ...
    EXPECT_TRUE(focus_manager->RotatePaneFocus(Direction::kForward, FocusCycleWrapping::kEnabled));
...
class FocusManagerArrowKeyTraversalTest : public FocusManagerTest, public testing::WithParamInterface<bool> {
public:
    FocusManagerArrowKeyTraversalTest() = default;
    FocusManagerArrowKeyTraversalTest(const FocusManagerArrowKeyTraversalTest&) = delete;
    FocusManagerArrowKeyTraversalTest& operator=(const FocusManagerArrowKeyTraversalTest&) = delete;
    ...
    {
        FocusManager::set_arrow_key_traversal_enabled(previous_arrow_key_traversal_enabled_);
...
INSTANTIATE_TEST_SUITE_P(All, FocusManagerArrowKeyTraversalTest, testing::Bool());
...
{
    FocusManager* focus_manager = GetFocusManager();
    const ui::KeyEvent left_key(ui::EventType::kKeyPressed, ui::VKEY_LEFT, ui::EF_NONE);
    const ui::KeyEvent right_key(ui::EventType::kKeyPressed, ui::VKEY_RIGHT, ui::EF_NONE);
    const ui::KeyEvent up_key(ui::EventType::kKeyPressed, ui::VKEY_UP, ui::EF_NONE);
    const ui::KeyEvent down_key(ui::EventType::kKeyPressed, ui::VKEY_DOWN, ui::EF_NONE);
...
TEST_F(FocusManagerTest, SkipViewsInArrowKeyTraversal)
{
    ...
    for (size_t i = 0; i < 5; ++i) {
        auto* view = GetContentsView()->AddChildView(std::make_unique<views::View>());
        view->SetGroup(12345);
        // Testing both kind of focuses (Always focusable and only accessibility
        // focusable).
        view->SetFocusBehavior((i == 0 || i == 4) ? View::FocusBehavior::ALWAYS : View::FocusBehavior::ACCESSIBLE_ONLY);
        v.push_back(view);
    }
...
TEST_F(FocusManagerTest, StoreFocusedView)
{
    ...
    EXPECT_EQ(FocusChangeReason::kFocusRestore, event_list->vec[2].focus_change_reason);
    ...
    EXPECT_EQ(ON_FOCUS, event_list->vec[1].type);
    EXPECT_EQ(kView1ID, event_list->vec[1].view_id);
    EXPECT_EQ(FocusChangeReason::kFocusRestore, event_list->vec[1].focus_change_reason);
...
public:
    ...
    AdvanceFocusWidgetDelegate& operator=(const AdvanceFocusWidgetDelegate&) = delete;
...
public:
    ...
    {
        TestBubbleDialogDelegateView* bubble = new TestBubbleDialogDelegateView(anchor);
        Widget* bubble_widget = BubbleDialogDelegateView::CreateBubble(bubble);
        bubble_widget->SetFocusTraversableParent(bubble->anchor_widget()->GetFocusTraversable());
    ...
    void UseNativeWidgetAura()
    {
        use_native_widget_aura_ = true;
    }

    void OnBeforeBubbleWidgetInit(Widget::InitParams* params, Widget* widget) const override
    {
#if defined(USE_AURA)
        if (use_native_widget_aura_) {
            params->native_widget = new test::TestPlatformNativeWidget<NativeWidgetAura>(widget, false, nullptr);
        }
#endif // USE_AURA
    }
...
TEST_F(FocusManagerTest, AdvanceFocusStaysInWidget)
{
    ...
    std::unique_ptr<AdvanceFocusWidgetDelegate> delegate_owned = std::make_unique<AdvanceFocusWidgetDelegate>(child_widget.get());
...
TEST_F(FocusManagerTest, NavigateIntoAnchoredDialog)
{
    ...
    BubbleDialogDelegateView* bubble_delegate = TestBubbleDialogDelegateView::CreateAndShowBubble(parent3);
...
TEST_F(FocusManagerTest, AnchoredDialogInPane)
{
    ...
    AccessiblePaneView* pane = root_view->AddChildView(std::make_unique<AccessiblePaneView>());
    View* anchor = pane->AddChildView(std::make_unique<View>());
    anchor->SetFocusBehavior(View::FocusBehavior::ALWAYS);

    BubbleDialogDelegateView* bubble = TestBubbleDialogDelegateView::CreateAndShowBubble(anchor);
...
TEST_F(FocusManagerTest, FocusRing)
{
    ...
    View* view = GetContentsView()->AddChildView(std::make_unique<StaticSizedView>(gfx::Size(10, 10)));
...
public:
    ...
    DesktopWidgetFocusManagerTest& operator=(const DesktopWidgetFocusManagerTest&) = delete;
...
TEST_F(DesktopWidgetFocusManagerTest, AnchoredDialogInDesktopNativeWidgetAura)
{
    ...
    TestBubbleDialogDelegateView* bubble_delegate = TestBubbleDialogDelegateView::CreateAndShowBubble(parent2);
    Widget* bubble_widget = bubble_delegate->GetWidget();
    ...
    parent1->GetViewAccessibility().SetName(u"Parent 1", ax::mojom::NameFrom::kAttribute);
    parent2->GetViewAccessibility().SetName(u"", ax::mojom::NameFrom::kAttributeExplicitlyEmpty);
    child->GetViewAccessibility().SetName("uChild", ax::mojom::NameFrom::kAttribute);
    ...
    aura::client::FocusClient* focus_client = aura::client::GetFocusClient(widget->GetNativeView());
...
public:
    ...
    RedirectToParentFocusManagerTest(const RedirectToParentFocusManagerTest&) = delete;
    RedirectToParentFocusManagerTest& operator=(const RedirectToParentFocusManagerTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        View* anchor = GetWidget()->GetRootView()->AddChildView(std::make_unique<View>());
        anchor->SetFocusBehavior(View::FocusBehavior::ALWAYS);
...
TEST_F(RedirectToParentFocusManagerTest, ParentHandlesAcceleratorFromBubble)
{
    ...
    parent_focus_manager_->RegisterAccelerator(return_accelerator, ui::AcceleratorManager::kNormalPriority, &parent_return_target);

    EXPECT_TRUE(!bubble_focus_manager_->IsAcceleratorRegistered(return_accelerator));
...
TEST_F(RedirectToParentFocusManagerTest, BubbleHandlesRegisteredAccelerators)
{
    ...
    bubble_focus_manager_->RegisterAccelerator(return_accelerator, ui::AcceleratorManager::kNormalPriority, &bubble_return_target);
    parent_focus_manager_->RegisterAccelerator(return_accelerator, ui::AcceleratorManager::kNormalPriority, &parent_return_target);
...
TEST_F(RedirectToParentFocusManagerTest, NotProcessedAccelerator)
{
    ...
    EXPECT_TRUE(!bubble_focus_manager_->IsAcceleratorRegistered(return_accelerator));
    EXPECT_TRUE(!parent_focus_manager_->IsAcceleratorRegistered(return_accelerator));
