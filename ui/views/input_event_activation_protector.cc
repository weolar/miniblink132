...
void InputEventActivationProtector::MaybeUpdateViewProtectedTimeStamp(bool force)
{
...
bool InputEventActivationProtector::IsPossiblyUnintendedInteraction(const ui::Event& event)
{
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableInputEventActivationProtectionForTesting)) [[unlikely]] {
        return false;
    }
    ...
    const base::TimeDelta kShortInterval = base::Milliseconds(GetDoubleClickInterval());
    const bool short_event_after_last_event = event.time_stamp() < last_event_timestamp_ + kShortInterval;
