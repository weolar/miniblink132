...
public:
    ...
    MOCK_METHOD3(FullscreenControllerSetFrame, void(const gfx::Rect&, bool animate, base::OnceCallback<void()> animation_complete));
    ...
    MOCK_CONST_METHOD1(FullscreenControllerGetFrameForDisplay, gfx::Rect(int64_t display_id));
...
void FullscreenControllerSetFrameFake(const gfx::Rect&, bool, base::OnceCallback<void()> callback)
{
...
TEST_F(MacFullscreenControllerTest, SimpleUserInitiated)
{
    ...
    EXPECT_CALL(mock_client_, FullscreenControllerGetFrame()).Times(1).WillOnce(Return(kWindowRect));
    ...
    EXPECT_CALL(mock_client_, FullscreenControllerTransitionComplete(true)).Times(1);
    ...
    EXPECT_CALL(mock_client_, FullscreenControllerGetDisplayId()).WillOnce(Return(kDisplay0Id));
    ...
    EXPECT_CALL(mock_client_, FullscreenControllerTransitionStart(false)).Times(1);
    ...
    // Complete the transition.
    EXPECT_CALL(mock_client_, FullscreenControllerTransitionComplete(false)).Times(1);
...
TEST_F(MacFullscreenControllerTest, SimpleProgrammatic)
{
    ...
    EXPECT_CALL(mock_client_, FullscreenControllerToggleFullscreen())
        .WillOnce(Invoke(&controller_, &NativeWidgetNSWindowFullscreenController::OnWindowWillEnterFullscreen));
    task_environment_.RunUntilIdle();
    EXPECT_TRUE(controller_.IsInFullscreenTransition());
    ...
    EXPECT_CALL(mock_client_, FullscreenControllerToggleFullscreen())
        .WillOnce(Invoke(&controller_, &NativeWidgetNSWindowFullscreenController::OnWindowWillExitFullscreen));
    task_environment_.RunUntilIdle();
    EXPECT_TRUE(controller_.IsInFullscreenTransition());
...
TEST_F(MacFullscreenControllerTest, SimpleCrossScreen)
{
    ...
    EXPECT_CALL(mock_client_, FullscreenControllerGetFrameForDisplay(kDisplay1Id)).Times(1).WillOnce(Return(kDisplay1Frame));
    EXPECT_CALL(mock_client_, FullscreenControllerSetFrame(kDisplay1Frame, true, _)).WillOnce(FullscreenControllerSetFrameFake);
    EXPECT_CALL(mock_client_, FullscreenControllerToggleFullscreen())
        .WillOnce(Invoke(&controller_, &NativeWidgetNSWindowFullscreenController::OnWindowWillEnterFullscreen));
    task_environment_.RunUntilIdle();
    EXPECT_TRUE(controller_.IsInFullscreenTransition());
    ...
    EXPECT_CALL(mock_client_, FullscreenControllerGetDisplayId()).WillOnce(Return(kDisplay1Id));
    controller_.EnterFullscreen(kDisplay1Id);
    task_environment_.RunUntilIdle();
    EXPECT_FALSE(controller_.IsInFullscreenTransition());
    EXPECT_TRUE(controller_.GetTargetFullscreenState());
    ...
    EXPECT_CALL(mock_client_, FullscreenControllerSetFrame(kWindowRect, true, _)).WillOnce(FullscreenControllerSetFrameFake);
    EXPECT_CALL(mock_client_, FullscreenControllerTransitionComplete(false)).Times(1);
    task_environment_.RunUntilIdle();
    EXPECT_FALSE(controller_.IsInFullscreenTransition());
    EXPECT_FALSE(controller_.GetTargetFullscreenState());
...
TEST_F(MacFullscreenControllerTest, CrossScreenFromFullscreen)
{
    ...
    EXPECT_CALL(mock_client_, FullscreenControllerTransitionStart(true)).Times(1);
    EXPECT_CALL(mock_client_, FullscreenControllerGetDisplayId()).WillRepeatedly(Return(kDisplay0Id));
    controller_.EnterFullscreen(kDisplay1Id);
    EXPECT_TRUE(controller_.IsInFullscreenTransition());
    EXPECT_TRUE(controller_.GetTargetFullscreenState());
...
{
    // Enter fullscreen on display 1, from display 0.
    EXPECT_CALL(mock_client_, FullscreenControllerGetFrame()).WillOnce(Return(kWindowRect));
    EXPECT_CALL(mock_client_, FullscreenControllerTransitionStart(true)).Times(1);
    controller_.EnterFullscreen(kDisplay1Id);
    EXPECT_TRUE(controller_.IsInFullscreenTransition());
    EXPECT_TRUE(controller_.GetTargetFullscreenState());
    ...
    // though we are not actually moving displays. It will also then post a task
    // to ToggleFullscreen (because RunUntilIdle will pick up that posted task).
    EXPECT_CALL(mock_client_, FullscreenControllerGetFrameForDisplay(kDisplay1Id)).WillOnce(Return(kDisplay1Frame));
    EXPECT_CALL(mock_client_, FullscreenControllerSetFrame(kDisplay1Frame, true, _)).WillOnce(FullscreenControllerSetFrameFake);
    EXPECT_CALL(mock_client_, FullscreenControllerToggleFullscreen())
        .WillOnce(Invoke(&controller_, &NativeWidgetNSWindowFullscreenController::OnWindowWillEnterFullscreen));
    task_environment_.RunUntilIdle();
