...
class VIEWS_EXPORT NativeWidgetMacNSWindowHost : public remote_cocoa::NativeWidgetNSWindowHostHelper,
...
public:
    ...
    static NativeWidgetMacNSWindowHost* GetFromNativeWindow(gfx::NativeWindow window);
    ...
    static NativeWidgetMacNSWindowHost* GetFromId(uint64_t bridged_native_widget_id);
    uint64_t bridged_native_widget_id() const
    {
        return widget_id_;
    }
    ...
    NativeWidgetMacNSWindowHost& operator=(const NativeWidgetMacNSWindowHost&) = delete;
    ...
    NativeWidgetMacNSWindowHost* parent() const
    {
        return parent_;
    }
    ...
    TextInputHost* text_input_host() const
    {
    ...
    void CreateRemoteNSWindow(remote_cocoa::ApplicationHost* application_host, remote_cocoa::mojom::CreateWindowParamsPtr window_create_params);

    void InitWindow(const Widget::InitParams& params, const gfx::Rect& initial_bounds_in_screen);
    ...
    void SetFullscreen(bool fullscreen, int64_t target_display_id = display::kInvalidDisplayId);
    ...
    uint64_t GetRootViewNSViewId() const
    {
        return root_view_id_;
    }

    void set_immersive_mode_reveal_client(ImmersiveModeRevealClient* reveal_client)
    {
    ...
    const display::Display& GetCurrentDisplay() const
    {
        return display_;
    }
    ...
    {
        return is_miniaturized_;
    }
    bool IsWindowKey() const
    {
        return is_window_key_;
    }
    bool IsMouseCaptureActive() const
    {
        return is_mouse_capture_active_;
    }
    bool IsZoomed() const
    {
        return is_zoomed_;
    ...
    std::unique_ptr<NativeWidgetMacEventMonitor> AddEventMonitor(NativeWidgetMacEventMonitor::Client* client);
...
private:
    ...
    void GetAttachedNativeViewHostViewsRecursive(View* view, std::vector<NSView*>* attached_native_view_host_views_ordered) const;
    ...
    void GetWordAt(const gfx::Point& location_in_content, bool* found_word, gfx::DecoratedText* decorated_word, gfx::Point* baseline_point) override;
    ...
    void OnApplicationHostDestroying(remote_cocoa::ApplicationHost* host) override;
    ...
    bool DispatchKeyEventRemote(std::unique_ptr<ui::Event> event, bool* event_handled) override;
    bool DispatchKeyEventToMenuControllerRemote(std::unique_ptr<ui::Event> event, bool* event_swallowed, bool* event_handled) override;
    bool DispatchMonitorEvent(std::unique_ptr<ui::Event> event, bool* event_handled) override;
    bool GetHasMenuController(bool* has_menu_controller) override;
    bool GetIsDraggableBackgroundAt(const gfx::Point& location_in_content, bool* is_draggable_background) override;
    bool GetWidgetIsModal(bool* widget_is_modal) override;
    bool GetIsFocusedViewTextual(bool* is_textual) override;
    void OnWindowGeometryChanged(const gfx::Rect& window_bounds_in_screen_dips, const gfx::Rect& content_bounds_in_screen_dips) override;
    void OnWindowFullscreenTransitionStart(bool target_fullscreen_state) override;
    void OnWindowFullscreenTransitionComplete(bool target_fullscreen_state) override;
    ...
    void OnWindowKeyStatusChanged(bool is_key, bool is_content_first_responder, bool full_keyboard_access_enabled) override;
    void OnWindowStateRestorationDataChanged(const std::vector<uint8_t>& data) override;
    ...
    bool GetDialogButtonInfo(
        ui::mojom::DialogButton type, bool* button_exists, std::u16string* button_label, bool* is_button_enabled, bool* is_button_default) override;
    ...
    bool GetWindowFrameTitlebarHeight(bool* override_titlebar_height, float* titlebar_height) override;
    ...
    bool GetRootViewAccessibilityToken(base::ProcessId* pid, std::vector<uint8_t>* token) override;
    bool ValidateUserInterfaceItem(int32_t command, remote_cocoa::mojom::ValidateUserInterfaceItemResultPtr* out_result) override;
    bool WillExecuteCommand(int32_t command, WindowOpenDisposition window_open_disposition, bool is_before_first_responder, bool* will_execute) override;
    bool ExecuteCommand(int32_t command, WindowOpenDisposition window_open_disposition, bool is_before_first_responder, bool* was_executed) override;
    bool HandleAccelerator(const ui::Accelerator& accelerator, bool require_priority_handler, bool* was_handled) override;
    ...
    void DispatchKeyEventRemote(std::unique_ptr<ui::Event> event, DispatchKeyEventRemoteCallback callback) override;
    void DispatchKeyEventToMenuControllerRemote(std::unique_ptr<ui::Event> event, DispatchKeyEventToMenuControllerRemoteCallback callback) override;
    void DispatchMonitorEvent(std::unique_ptr<ui::Event> event, DispatchMonitorEventCallback callback) override;
    void GetHasMenuController(GetHasMenuControllerCallback callback) override;
    void GetIsDraggableBackgroundAt(const gfx::Point& location_in_content, GetIsDraggableBackgroundAtCallback callback) override;
    void GetTooltipTextAt(const gfx::Point& location_in_content, GetTooltipTextAtCallback callback) override;
    void GetWidgetIsModal(GetWidgetIsModalCallback callback) override;
    void GetIsFocusedViewTextual(GetIsFocusedViewTextualCallback callback) override;
    void GetDialogButtonInfo(ui::mojom::DialogButton button, GetDialogButtonInfoCallback callback) override;
    void GetDoDialogButtonsExist(GetDoDialogButtonsExistCallback callback) override;
    void GetShouldShowWindowTitle(GetShouldShowWindowTitleCallback callback) override;
    void GetCanWindowBecomeKey(GetCanWindowBecomeKeyCallback callback) override;
    void GetAlwaysRenderWindowAsKey(GetAlwaysRenderWindowAsKeyCallback callback) override;
    void OnWindowCloseRequested(OnWindowCloseRequestedCallback callback) override;
    void GetWindowFrameTitlebarHeight(GetWindowFrameTitlebarHeightCallback callback) override;
    void GetRootViewAccessibilityToken(GetRootViewAccessibilityTokenCallback callback) override;
    void ValidateUserInterfaceItem(int32_t command, ValidateUserInterfaceItemCallback callback) override;
    void WillExecuteCommand(
        int32_t command, WindowOpenDisposition window_open_disposition, bool is_before_first_responder, ExecuteCommandCallback callback) override;
    void ExecuteCommand(
        int32_t command, WindowOpenDisposition window_open_disposition, bool is_before_first_responder, ExecuteCommandCallback callback) override;
    void HandleAccelerator(const ui::Accelerator& accelerator, bool require_priority_handler, HandleAcceleratorCallback callback) override;
    ...
    const raw_ptr<views::NativeWidgetMac> native_widget_mac_; // Weak. Owns |this_|.

    // Structure used to look up this structure's interfaces from its
    // gfx::NativeWindow.
    std::unique_ptr<remote_cocoa::ScopedNativeWindowMapping> native_window_mapping_;
    ...
    // process.
    mojo::AssociatedRemote<remote_cocoa::mojom::NativeWidgetNSWindow> remote_ns_window_remote_;
    ...
    std::unique_ptr<remote_cocoa::NativeWidgetNSWindowBridge> in_process_ns_window_bridge_;
    ...
    std::unique_ptr<remote_cocoa::ScopedNSViewIdMapping> in_process_view_id_mapping_;
    ...
    mojo::AssociatedReceiver<remote_cocoa::mojom::NativeWidgetNSWindowHost> remote_ns_window_host_receiver_ { this };

    base::ScopedObservation<View, ViewObserver> root_view_observation_ { this };

    base::WeakPtrFactory<NativeWidgetMacNSWindowHost> weak_factory_for_vsync_update_ { this };
