...
public:
    ...
    TestSelectionControllerDelegate(const TestSelectionControllerDelegate&) = delete;
    TestSelectionControllerDelegate& operator=(const TestSelectionControllerDelegate&) = delete;
...
public:
    void SetUp() override
    {
        ...
        delegate_ = std::make_unique<TestSelectionControllerDelegate>(render_text_.get());
    ...
    {
        return base::UTF16ToASCII(render_text_->GetTextFromRange(render_text_->selection()));
    }
    ...
    void DragMouse(const gfx::Point& location)
    {
        ...
        controller_->OnMouseDragged(ui::MouseEvent(ui::EventType::kMouseDragged, location, location, last_event_time_, mouse_flags_, 0));
    }
...
private:
    void PressMouseButton(const gfx::Point& location, int button, bool focused)
    {
        ...
        controller_->OnMousePressed(ui::MouseEvent(ui::EventType::kMousePressed, location, location, last_event_time_, mouse_flags_, button), false,
            focused ? SelectionController::InitialFocusStateOnMousePress::kFocused : SelectionController::InitialFocusStateOnMousePress::kUnFocused);
    }
    ...
    void ReleaseMouseButton(int button)
    {
        DCHECK(mouse_flags_ & button);
        mouse_flags_ &= ~button;
        controller_->OnMouseReleased(ui::MouseEvent(ui::EventType::kMouseReleased, mouse_location_, mouse_location_, last_event_time_, mouse_flags_, button));
    }
