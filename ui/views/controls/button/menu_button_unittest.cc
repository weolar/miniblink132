...
public:
    TestMenuButton()
        : TestMenuButton(base::BindRepeating(&TestMenuButton::ButtonPressed, base::Unretained(this)))
    {
    }
    ...
    Button::ButtonState last_state() const
    {
        return last_state_;
    }
    ui::EventType last_event_type() const
    {
...
protected:
    ...
    {
        return static_cast<test::TestInkDrop*>(test::InkDropHostTestApi(InkDrop::Get(button())).ink_drop());
    }
    ...
    void ConfigureMenuButton(std::unique_ptr<TestMenuButton> test_button)
    {
        ...
        generator_ = std::make_unique<ui::test::EventGenerator>(GetRootWindow(widget()));
        ...
        test::InkDropHostTestApi(InkDrop::Get(button())).SetInkDrop(std::move(ink_drop));
...
public:
    explicit PressStateButton(bool release_lock)
        : TestMenuButton(base::BindRepeating(&PressStateButton::ButtonPressed, base::Unretained(this)))
...
public:
    ...
    void WriteDragDataForView(View* sender, const gfx::Point& press_pt, ui::OSExchangeData* data) override
    {
    }
    ...
    bool CanStartDragForView(View* sender, const gfx::Point& press_pt, const gfx::Point& p) override
    {
        return true;
    }
};
...
class TestDragDropClient : public aura::client::DragDropClient, public ui::EventHandler {
public:
    TestDragDropClient();
...
TEST_F(MenuButtonTest, ActivateOnKeyPress)
{
    ...
    button()->OnKeyPressed(ui::KeyEvent(ui::EventType::kKeyPressed, ui::KeyboardCode::VKEY_SPACE, ui::DomCode::SPACE, 0));
    EXPECT_TRUE(button()->clicked());

    button()->Reset();
    EXPECT_FALSE(button()->clicked());

    button()->OnKeyPressed(ui::KeyEvent(ui::EventType::kKeyPressed, ui::KeyboardCode::VKEY_RETURN, ui::DomCode::ENTER, 0));
...
TEST_F(MenuButtonTest, InkDropCenterSetFromClick)
{
    ...
    EXPECT_TRUE(button()->clicked());
    gfx::Point inkdrop_center_point = InkDrop::Get(button())->GetInkDropCenterBasedOnLastEvent();
...
#define MAYBE_InkDropCenterSetFromClickWithPressedLock DISABLED_InkDropCenterSetFromClickWithPressedLock
#else
#define MAYBE_InkDropCenterSetFromClickWithPressedLock InkDropCenterSetFromClickWithPressedLock
...
TEST_F(MenuButtonTest, MAYBE_InkDropCenterSetFromClickWithPressedLock)
{
    ...
    ui::MouseEvent click_event(ui::EventType::kMousePressed, click_point, click_point, base::TimeTicks(), 0, 0);
    MenuButtonController::PressedLock pressed_lock(button()->button_controller(), false, &click_event);

    EXPECT_EQ(Button::STATE_PRESSED, button()->GetState());
    EXPECT_EQ(click_point, InkDrop::Get(button())->GetInkDropCenterBasedOnLastEvent());
}

// Test that the MenuButton stays pressed while there are any PressedLocks.
// TODO(crbug.com/40903656): Test flaky on MSAN ChromeOS builders.
#if BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
#define MAYBE_ButtonStateForMenuButtonsWithPressedLocks DISABLED_ButtonStateForMenuButtonsWithPressedLocks
#else
#define MAYBE_ButtonStateForMenuButtonsWithPressedLocks ButtonStateForMenuButtonsWithPressedLocks
#endif // BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
TEST_F(MenuButtonTest, MAYBE_ButtonStateForMenuButtonsWithPressedLocks)
{
    ...
    auto pressed_lock1 = std::make_unique<MenuButtonController::PressedLock>(button()->button_controller());
    EXPECT_EQ(Button::STATE_PRESSED, button()->GetState());
    ...
    auto pressed_lock2 = std::make_unique<MenuButtonController::PressedLock>(button()->button_controller());
    EXPECT_EQ(Button::STATE_PRESSED, button()->GetState());
...
// TODO(crbug.com/40903656): Test flaky on MSAN ChromeOS builders.
#if BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
#define MAYBE_DraggableMenuButtonActivatesOnRelease DISABLED_DraggableMenuButtonActivatesOnRelease
#else
#define MAYBE_DraggableMenuButtonActivatesOnRelease DraggableMenuButtonActivatesOnRelease
#endif // BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
...
// TODO(crbug.com/40903656): Test flaky on MSAN ChromeOS builders.
#if BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
#define MAYBE_InkDropStateForMenuButtonActivationsWithoutCallback DISABLED_InkDropStateForMenuButtonActivationsWithoutCallback
#else
#define MAYBE_InkDropStateForMenuButtonActivationsWithoutCallback InkDropStateForMenuButtonActivationsWithoutCallback
#endif // BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
TEST_F(MenuButtonTest, MAYBE_InkDropStateForMenuButtonActivationsWithoutCallback)
{
    ConfigureMenuButton(std::make_unique<TestMenuButton>(Button::PressedCallback()));
...
#else
#define MAYBE_InkDropStateForMenuButtonActivationsWithCallbackThatDoesntAcquireALock InkDropStateForMenuButtonActivationsWithCallbackThatDoesntAcquireALock
#endif // BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
TEST_F(MenuButtonTest, MAYBE_InkDropStateForMenuButtonActivationsWithCallbackThatDoesntAcquireALock)
{
    ConfigureMenuButton(std::make_unique<TestMenuButton>());
    button()->Activate(nullptr);
...
TEST_F(MenuButtonTest, InkDropStateForMenuButtonActivationsWithCallbackThatReleasesAllLocks)
{
...
// TODO(crbug.com/40903656): Test flaky on MSAN ChromeOS builders.
#if BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
#define MAYBE_InkDropStateForMenuButtonsWithPressedLocks DISABLED_InkDropStateForMenuButtonsWithPressedLocks
#else
#define MAYBE_InkDropStateForMenuButtonsWithPressedLocks InkDropStateForMenuButtonsWithPressedLocks
#endif // BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
...
#if BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
#define MAYBE_OneInkDropAnimationForReentrantPressedLocks DISABLED_OneInkDropAnimationForReentrantPressedLocks
#else
#define MAYBE_OneInkDropAnimationForReentrantPressedLocks OneInkDropAnimationForReentrantPressedLocks
#endif // BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
...
// TODO(crbug.com/40903656): Test flaky on MSAN ChromeOS builders.
#if BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
#define MAYBE_InkDropStateForMenuButtonWithPressedLockBeforeActivation DISABLED_InkDropStateForMenuButtonWithPressedLockBeforeActivation
#else
#define MAYBE_InkDropStateForMenuButtonWithPressedLockBeforeActivation InkDropStateForMenuButtonWithPressedLockBeforeActivation
#endif // BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
TEST_F(MenuButtonTest, MAYBE_InkDropStateForMenuButtonWithPressedLockBeforeActivation)
{
    ConfigureMenuButton(std::make_unique<TestMenuButton>());
...
// TODO(crbug.com/40903656): Test flaky on MSAN ChromeOS builders.
#if BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
#define MAYBE_DraggableMenuButtonDoesNotActivateOnDrag DISABLED_DraggableMenuButtonDoesNotActivateOnDrag
#else
#define MAYBE_DraggableMenuButtonDoesNotActivateOnDrag DraggableMenuButtonDoesNotActivateOnDrag
#endif // BUILDFLAG(IS_CHROMEOS) && defined(MEMORY_SANITIZER)
TEST_F(MenuButtonTest, MAYBE_DraggableMenuButtonDoesNotActivateOnDrag)
{
    ...
    button()->AddPreTargetHandler(&drag_client, ui::EventTarget::Priority::kSystem);
...
TEST_F(MenuButtonTest, InkDropIsntHoveredAfterDismissingMenuWhenMouseOutsideButton)
{
    auto press_state_button = std::make_unique<PressStateButton>(false);
    auto* test_button = press_state_button.get();
    ConfigureMenuButton(std::move(press_state_button));
...
TEST_F(MenuButtonTest, DestroyButtonInGesture)
{
    std::unique_ptr<TestMenuButton> test_menu_button
        = std::make_unique<TestMenuButton>(base::BindRepeating([](std::unique_ptr<TestMenuButton>* button) { button->reset(); }, &test_menu_button));
    ConfigureMenuButton(std::move(test_menu_button));

    ui::GestureEvent gesture_event(0, 0, 0, base::TimeTicks::Now(), ui::GestureEventDetails(ui::EventType::kGestureTap));
