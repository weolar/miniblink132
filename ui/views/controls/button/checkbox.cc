...
class Checkbox::FocusRingHighlightPathGenerator : public views::HighlightPathGenerator {
public:
...
Checkbox::Checkbox(const std::u16string& label, PressedCallback callback, int button_context)
    : LabelButton(std::move(callback), label, button_context)
{
    ...
    InkDrop::Get(this)->SetCreateRippleCallback(base::BindRepeating(
        [](Checkbox* host) {
            ...
            return InkDrop::Get(host)->CreateSquareRipple(host->image_container_view()->GetMirroredContentsBounds().CenterPoint(), gfx::Size(21, 21));
        },
        this));
    ...
    InkDrop::Install(image_container_view(), std::make_unique<InkDropHost>(image_container_view()));
    ...
    InkDrop::Get(image_container_view())
        ->SetCreateHighlightCallback(base::BindRepeating(
            [](View* host) {
                int radius = InkDropHost::GetLargeSize(kCheckboxInkDropSize).width() / 2;
                return std::make_unique<views::InkDropHighlight>(gfx::PointF(host->GetContentsBounds().CenterPoint()),
                    std::make_unique<CircleLayerDelegate>(views::InkDrop::Get(host)->GetBaseColor(), radius));
            },
            image_container_view()));

    InkDrop::Get(image_container_view())
        ->SetCreateRippleCallback(base::BindRepeating(
            [](View* host) { return InkDrop::Get(host)->CreateSquareRipple(host->GetContentsBounds().CenterPoint(), kCheckboxInkDropSize); },
            image_container_view()));

    // Usually ink-drop ripples match the text color. Checkboxes use the
    // color of the unchecked, enabled icon.
    InkDrop::Get(image_container_view())->SetBaseColorId(ui::kColorCheckboxForegroundUnchecked);
...
base::CallbackListSubscription Checkbox::AddCheckedChangedCallback(PropertyChangedCallback callback)
{
...
gfx::ImageSkia Checkbox::GetImage(ButtonState for_state) const
{
    ...
    if (GetChecked()) {
        const gfx::ImageSkia check_icon = gfx::CreateVectorIcon(GetVectorIcon(), kCheckboxIconDipSize, GetIconCheckColor(icon_state));

        return gfx::ImageSkiaOperations::CreateImageWithRoundRectBackground(
            gfx::SizeF(kCheckboxIconDipSize, kCheckboxIconDipSize), kCheckboxIconCornerRadius, container_color, check_icon);
    }
    return gfx::CreateVectorIcon(GetVectorIcon(), kCheckboxIconDipSize, container_color);
}

std::unique_ptr<LabelButtonBorder> Checkbox::CreateDefaultBorder() const
{
    std::unique_ptr<LabelButtonBorder> border = LabelButton::CreateDefaultBorder();
    border->set_insets(LayoutProvider::Get()->GetInsetsMetric(INSETS_CHECKBOX_RADIO_BUTTON));
    return border;
}
...
SkColor Checkbox::GetIconImageColor(int icon_state) const
{
    if (icon_state & IconState::CHECKED) {
        return GetColorProvider()->GetColor((icon_state & IconState::ENABLED) ? ui::kColorCheckboxContainer : ui::kColorCheckboxContainerDisabled);
    }
    return GetColorProvider()->GetColor((icon_state & IconState::ENABLED) ? ui::kColorCheckboxOutline : ui::kColorCheckboxOutlineDisabled);
}
...
SkColor Checkbox::GetIconCheckColor(int icon_state) const
{
    ...
    return GetColorProvider()->GetColor((icon_state & IconState::ENABLED) ? ui::kColorCheckboxCheck : ui::kColorCheckboxCheckDisabled);
}
...
{
    GetViewAccessibility().SetCheckedState(GetChecked() ? ax::mojom::CheckedState::kTrue : ax::mojom::CheckedState::kFalse);
}
...
{
    SetDefaultActionVerb(checked_ ? ax::mojom::DefaultActionVerb::kUncheck : ax::mojom::DefaultActionVerb::kCheck);
...
void CheckboxActionViewInterface::ActionItemChangedImpl(actions::ActionItem* action_item)
{
    LabelButtonActionViewInterface::ActionItemChangedImpl(action_item);
    action_view_->SetChecked(action_item->GetChecked());
}

void CheckboxActionViewInterface::OnViewChangedImpl(actions::ActionItem* action_item)
{
