...
public:
    explicit TestLabelButton(const std::u16string& text = std::u16string(), int button_context = style::CONTEXT_BUTTON)
...
public:
    ...
    void SetUp() override
    {
        ...
        auto* container = test_widget_->client_view()->AddChildView(std::make_unique<View>());
        ...
        themed_normal_text_color_ = button->GetColorProvider()->GetColor(ui::kColorLabelForeground);
        ...
        styled_highlight_text_color_ = styled_normal_text_color_ = button->GetColorProvider()->GetColor(ui::kColorButtonForeground);
#else
...
protected:
    ...
    {
        return static_cast<TestLabelButton*>(test_widget_->GetContentsView()->GetViewByID(1));
    }
...
TEST_F(LabelButtonTest, Init)
{
    ...
    EXPECT_EQ(text, accessible_node_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
...
TEST_F(LabelButtonTest, Label)
{
    ...
    EXPECT_EQ(button()->GetPreferredSize({}), gfx::Size(long_text_width, font_list.GetHeight() * 2));
}
...
TEST_F(LabelButtonTest, LabelPreferredSizeWithMaxWidth)
{
    ...
    for (bool is_multiline : { false, true }) {
        ...
        for (bool set_image : { false, true }) {
            if (set_image)
                button()->SetImageModel(Button::STATE_NORMAL, ui::ImageModel::FromImageSkia(gfx::test::CreateImageSkia(/*size=*/16)));
...
TEST_F(LabelButtonTest, LabelShrinksDownImmediatelyIfAlreadySmallerThanPreferredSizeWithoutLabel)
{
    button()->SetBoundsRect(gfx::Rect(1, 1));
...
TEST_F(LabelButtonTest, AccessibleState)
{
    ...
    button()->GetViewAccessibility().GetAccessibleNodeData(&accessible_node_data);
    EXPECT_EQ(ax::mojom::Role::kButton, accessible_node_data.role);
    EXPECT_EQ(std::u16string(), accessible_node_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
    ...
    button()->GetViewAccessibility().GetAccessibleNodeData(&accessible_node_data);
    EXPECT_EQ(tooltip_text, accessible_node_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
    ...
    button()->GetViewAccessibility().GetAccessibleNodeData(&accessible_node_data);
    EXPECT_EQ(label_text, accessible_node_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
...
TEST_F(LabelButtonTest, Image)
{
    ...
    button()->SetImageModel(Button::STATE_NORMAL, ui::ImageModel::FromImageSkia(small_image));
    ...
    button()->SetImageModel(Button::STATE_NORMAL, ui::ImageModel::FromImageSkia(large_image));
...
{
    const std::u16string text(u"Some long text that would result in multiline label");
    button()->SetText(text);
    ...
    button()->SetImageModel(Button::STATE_NORMAL, ui::ImageModel::FromImageSkia(image));
...
TEST_F(LabelButtonTest, LabelAndImage)
{
    ...
    views::test::RunScheduledLayout(button());
    EXPECT_LT(button()->image_container_view()->bounds().right(), button()->label()->bounds().x());
    int left_align_label_midpoint = button()->label()->bounds().CenterPoint().x();
    button()->SetHorizontalAlignment(gfx::ALIGN_CENTER);
    views::test::RunScheduledLayout(button());
    EXPECT_LT(button()->image_container_view()->bounds().right(), button()->label()->bounds().x());
    int center_align_label_midpoint = button()->label()->bounds().CenterPoint().x();
    EXPECT_LT(left_align_label_midpoint, center_align_label_midpoint);
    button()->SetHorizontalAlignment(gfx::ALIGN_RIGHT);
    views::test::RunScheduledLayout(button());
    EXPECT_LT(button()->label()->bounds().right(), button()->image_container_view()->bounds().x());
    ...
    button()->SetText(text);
    button()->SetImageModel(Button::STATE_NORMAL, ui::ImageModel::FromImageSkia(image));
    button()->SetMinSize(gfx::Size((text_width + image_size) * 2, image_size * 2));
    EXPECT_EQ(button()->GetPreferredSize({}).width(), (text_width + image_size) * 2);
...
TEST_F(LabelButtonTest, LabelWrapAndImageAlignment)
{
    ...
    const int image_spacing = provider->GetDistanceMetric(DISTANCE_RELATED_LABEL_HORIZONTAL);
    ...
    const int unclamped_size_without_label = image.width() + image_spacing + button_insets.width();

    button()->SetImageModel(Button::STATE_NORMAL, ui::ImageModel::FromImageSkia(image));
    button()->SetImageCentered(false);
    button()->SetMaxSize(gfx::Size(unclamped_size_without_label + text_wrap_width, 0));

    gfx::Size preferred_size = button()->GetPreferredSize({});
    preferred_size.set_height(button()->GetHeightForWidth(preferred_size.width()));
    button()->SetSize(preferred_size);
    views::test::RunScheduledLayout(button());

    gfx::Size label_width = button()->label()->GetPreferredSize(views::SizeBounds(text_wrap_width, {}));
    EXPECT_EQ(preferred_size.width(), unclamped_size_without_label + label_width.width());
    EXPECT_EQ(preferred_size.height(), font_list.GetHeight() * 2 + button_insets.height());

    // The image should be centered on the first line of the multi-line label
    EXPECT_EQ(button()->image_container_view()->y(), (font_list.GetHeight() - button()->image_container_view()->height()) / 2 + button_insets.top());
}
...
TEST_F(LabelButtonTest, GetHeightForWidthConsistentWithGetPreferredSize)
{
    ...
    for (int image_size : { kTinyImageSize, kLargeImageSize }) {
        ...
        EXPECT_EQ(std::max(image_size, font_height) + button()->GetInsets().height(), preferred_button_size.height());

        // Make sure this preferred height is consistent with GetHeightForWidth().
        EXPECT_EQ(preferred_button_size.height(), button()->GetHeightForWidth(preferred_button_size.width()));
    }
}
...
TEST_F(LabelButtonTest, TextSizeFromContext)
{
    ...
    const auto get_delta
        = [](auto context) { return TypographyProvider().GetFont(context, style::STYLE_PRIMARY).GetFontSize() - gfx::FontList().GetFontSize(); };
    ...
    const std::u16string text(u"abcdefghijklm");
    button()->SetText(text);
    EXPECT_EQ(default_delta, button()->label()->font_list().GetFontSize() - gfx::FontList().GetFontSize());

    TestLabelButton* alternate_button = new TestLabelButton(text, kAlternateContext);
    button()->parent()->AddChildView(alternate_button);
    EXPECT_EQ(alternate_delta, alternate_button->label()->font_list().GetFontSize() - gfx::FontList().GetFontSize());

    // The button size increases when the font size is increased.
    EXPECT_LT(button()->GetPreferredSize({}).width(), alternate_button->GetPreferredSize({}).width());
    EXPECT_LT(button()->GetPreferredSize({}).height(), alternate_button->GetPreferredSize({}).height());
}
...
TEST_F(LabelButtonTest, ChangeLabelImageSpacing)
{
    ...
    button()->SetImageModel(Button::STATE_NORMAL, ui::ImageModel::FromImageSkia(gfx::test::CreateImageSkia(/*size=*/50)));
...
{
    ASSERT_NE(button()->GetNativeTheme()->GetPlatformHighContrastColorScheme(), ui::NativeTheme::PlatformHighContrastColorScheme::kDark);
...
TEST_F(LabelButtonTest, SetEnabledTextColorIds)
{
    ...
    UseDarkColors();
    EXPECT_EQ(button()->label()->GetEnabledColorId(), ui::kColorAccent);
    EXPECT_EQ(button()->label()->GetEnabledColor(), button()->GetColorProvider()->GetColor(ui::kColorAccent));
}
...
TEST_F(LabelButtonTest, ImageOrLabelGetClipped)
{
    ...
    const gfx::FontList font_list = button()->label()->font_list();
    const int image_size = font_list.GetHeight();
    button()->SetImageModel(Button::STATE_NORMAL, ui::ImageModel::FromImageSkia(gfx::test::CreateImageSkia(image_size)));

    button()->SetBoundsRect(gfx::Rect(button()->GetPreferredSize({})));
    // The border size + the content height is more than button's preferred size.
    button()->SetBorder(CreateEmptyBorder(gfx::Insets::TLBR(image_size / 2, 0, image_size / 2, 0)));
    views::test::RunScheduledLayout(button());
...
TEST_F(LabelButtonTest, UpdateImageAfterSettingImageModel)
{
    auto is_showing_image = [&](const gfx::ImageSkia& image) {
        const auto* image_view = AsViewClass<ImageView>(button()->image_container_view());
        CHECK(image_view);
        return image_view->GetImage().BackedBySameObjectAs(image);
    };

    auto normal_image = gfx::test::CreateImageSkia(/*size=*/16);
    button()->SetImageModel(Button::STATE_NORMAL, ui::ImageModel::FromImageSkia(normal_image));
    ...
    button()->SetImageModel(Button::STATE_DISABLED, ui::ImageModel::FromImageSkia(disabled_image));
    ...
    button()->SetImageModel(Button::STATE_DISABLED, ui::ImageModel::FromImageSkia(disabled_empty_image));
...
public:
    ...
    void SetUp() override
    {
        ...
        auto* button = widget_->SetContentsView(std::make_unique<LabelButton>(Button::PressedCallback(), std::u16string()));

        auto* test_ink_drop = new test::TestInkDrop();
        test::InkDropHostTestApi(InkDrop::Get(button)).SetInkDrop(base::WrapUnique(test_ink_drop));
    }
...
protected:
    ...
    {
        return static_cast<test::TestInkDrop*>(InkDrop::Get(button())->GetInkDrop());
    }
...
TEST_F(InkDropLabelButtonTest, HoverStateAfterMouseEnterAndExitEvents)
{
    ...
    const gfx::Point out_of_bounds_point(button()->GetBoundsInScreen().bottom_right() + gfx::Vector2d(1, 1));
...
{
    View* target_view = widget_->GetRootView()->GetEventHandlerForPoint(button()->bounds().CenterPoint());
...
public:
    ...
    LabelButtonVisualStateTest& operator=(const LabelButtonVisualStateTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        style_of_inactive_widget_ = PlatformStyle::kInactiveWidgetControlsAppearDisabled ? Button::STATE_DISABLED : Button::STATE_NORMAL;
    }
...
protected:
    ...
    {
        return widget->GetContentsView()->AddChildView(std::make_unique<TestLabelButton>());
    }
...
{
    std::unique_ptr<Widget> child_widget = CreateActivatableChildWidget(test_widget_);
...
TEST_F(LabelButtonActionViewInterfaceTest, TestActionChanged)
{
    ...
    std::unique_ptr<actions::ActionItem> action_item = actions::ActionItem::Builder().SetText(test_string).SetActionId(0).SetEnabled(false).Build();
    label_button->GetActionViewInterface()->ActionItemChangedImpl(action_item.get());
