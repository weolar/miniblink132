...
public:
    explicit TestCheckbox(const std::u16string& label = std::u16string(), int button_context = style::CONTEXT_BUTTON)
...
TEST_F(CheckboxTest, AccessibilityTest)
{
    ...
    EXPECT_EQ(ax_data.GetString16Attribute(ax::mojom::StringAttribute::kName), label_text);
    ...
    EXPECT_EQ(ax_data.GetIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds)[0], label_data.id);
...
TEST_F(CheckboxTest, TestCorrectCheckColor)
{
    ...
    SkColor expected = checkbox()->GetColorProvider()->GetColor(ui::kColorCheckboxCheck);
    EXPECT_EQ(actual, expected);

    // Disabled
    icon_state = checkbox()->GetIconState(Button::ButtonState::STATE_DISABLED);
    actual = checkbox()->GetIconCheckColor(icon_state);
    expected = checkbox()->GetColorProvider()->GetColor(ui::kColorCheckboxCheckDisabled);
    EXPECT_EQ(actual, expected);
}
...
TEST_F(CheckboxTest, TestCorrectContainerColor)
{
    ...
    SkColor expected = checkbox()->GetColorProvider()->GetColor(ui::kColorCheckboxContainer);
    EXPECT_EQ(actual, expected);

    // Disabled
    icon_state = checkbox()->GetIconState(Button::ButtonState::STATE_DISABLED);
    actual = checkbox()->GetIconImageColor(icon_state);
    expected = checkbox()->GetColorProvider()->GetColor(ui::kColorCheckboxContainerDisabled);
    EXPECT_EQ(actual, expected);
}
...
TEST_F(CheckboxTest, AccessibleDefaultActionVerb)
{
    ...
    checkbox()->SetChecked(true);
    checkbox()->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.GetDefaultActionVerb(), ax::mojom::DefaultActionVerb::kUncheck);
...
{
    std::unique_ptr<actions::ActionItem> action_item = actions::ActionItem::Builder().SetActionId(0).SetEnabled(false).SetChecked(true).Build();
    checkbox()->GetActionViewInterface()->ActionItemChangedImpl(action_item.get());
...
TEST_F(CheckboxActionViewControllerTest, TestCheckboxClicked)
{
    ...
    std::unique_ptr<actions::ActionItem> action_item = actions::ActionItem::Builder().SetActionId(0).SetEnabled(false).SetChecked(false).Build();
    EXPECT_FALSE(action_item->GetChecked());
    view_controller.CreateActionViewRelationship(checkbox(), action_item->GetAsWeakPtr());
