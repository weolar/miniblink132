...
void PaintHelper(
    LabelButtonAssetBorder* border, gfx::Canvas* canvas, ui::NativeTheme::State state, const gfx::Rect& rect, const ui::NativeTheme::ExtraParams& extra)
{
    const auto& button = absl::get<ui::NativeTheme::ButtonExtraParams>(extra);
    Painter* painter = border->GetPainter(button.is_focused, Button::GetButtonStateFrom(state));
...
bool LabelButtonBorder::PaintsButtonState(bool focused, Button::ButtonState state)
{
    return false;
}
...
LabelButtonAssetBorder::LabelButtonAssetBorder()
{
    ...
    SetPainter(false, Button::STATE_HOVERED, Painter::CreateImageGridPainter(kTextHoveredImages));
    SetPainter(false, Button::STATE_PRESSED, Painter::CreateImageGridPainter(kTextPressedImages));
}
...
{
    return LayoutProvider::Get()->GetInsetsMetric(InsetsMetric::INSETS_LABEL_BUTTON);
}

bool LabelButtonAssetBorder::PaintsButtonState(bool focused, Button::ButtonState state)
{
...
void LabelButtonAssetBorder::Paint(const View& view, gfx::Canvas* canvas)
{
    ...
    ui::NativeTheme::ExtraParams extra(absl::in_place_type<ui::NativeTheme::ButtonExtraParams>);
    ...
    if (animation && animation->is_animating()) {
        ...
        uint8_t fg_alpha = static_cast<uint8_t>(animation->CurrentValueBetween(0, 255));
...
Painter* LabelButtonAssetBorder::GetPainter(bool focused, Button::ButtonState state)
{
    return painters_[focused ? 1 : 0][state].get();
}

void LabelButtonAssetBorder::SetPainter(bool focused, Button::ButtonState state, std::unique_ptr<Painter> painter)
{
