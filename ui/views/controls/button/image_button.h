...
public:
    ...
    virtual void SetImageModel(ButtonState state, const ui::ImageModel& image_model);

    // Set the background details.  The background image uses the same alignment
    // as the image.
    void SetBackgroundImage(SkColor color, const gfx::ImageSkia* image, const gfx::ImageSkia* mask);
    ...
    // guidelines.
    static std::unique_ptr<ImageButton> CreateIconButton(PressedCallback callback, const gfx::VectorIcon& icon, const std::u16string& accessible_name,
        MaterialIconStyle icon_style = MaterialIconStyle::kLarge, std::optional<gfx::Insets> insets = std::nullopt);
...
VIEW_BUILDER_PROPERTY(ImageButton::HorizontalAlignment, ImageHorizontalAlignment)
...
public:
    ...
    void SetToggledImageModel(ButtonState state, const ui::ImageModel& image_model);
    ...
    Background* GetToggledBackground() const
    {
        return toggled_background_.get();
    }
    ...
    void SetImageModel(ButtonState state, const ui::ImageModel& image_model) override;
...
VIEW_BUILDER_METHOD(SetToggledImageModel, Button::ButtonState, const ui::ImageModel&)
END_VIEW_BUILDER
