...
public:
    explicit TestLabel(SkColor* last_color, std::optional<ui::ColorId>* last_color_id)
...
public:
    ...
    void SetUp() override
    {
        ...
        widget_->SetContentsView(std::make_unique<TestLabel>(&last_color_, &last_color_id_));
...
TEST_F(LabelButtonLabelTest, Colors)
{
    ...
    SkColor default_theme_enabled_color = label()->GetColorProvider()->GetColor(ui::kColorLabelForeground);
    EXPECT_EQ(default_theme_enabled_color, last_color_);

    label()->SetEnabled(false);
    SkColor default_theme_disabled_color = label()->GetColorProvider()->GetColor(ui::kColorLabelForegroundDisabled);
    EXPECT_EQ(default_theme_disabled_color, last_color_);

    SetUseDarkColors(true);

    SkColor dark_theme_disabled_color = label()->GetColorProvider()->GetColor(ui::kColorLabelForegroundDisabled);
    EXPECT_NE(default_theme_disabled_color, dark_theme_disabled_color);
    EXPECT_EQ(dark_theme_disabled_color, last_color_);

    label()->SetEnabled(true);
    SkColor dark_theme_enabled_color = label()->GetColorProvider()->GetColor(ui::kColorLabelForeground);
...
TEST_F(LabelButtonLabelTest, ColorIds)
{
    ...
    EXPECT_EQ(last_color_, label()->GetColorProvider()->GetColor(ui::kColorAccent));

    label()->SetEnabled(false);
    label()->SetDisabledColorId(ui::kColorBadgeBackground);
    EXPECT_EQ(last_color_id_.value(), ui::kColorBadgeBackground);
    EXPECT_EQ(last_color_, label()->GetColorProvider()->GetColor(ui::kColorBadgeBackground));
