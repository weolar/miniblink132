...
Button::DefaultButtonControllerDelegate::DefaultButtonControllerDelegate(Button* button)
    : ButtonControllerDelegate(button)
{
}

Button::DefaultButtonControllerDelegate::~DefaultButtonControllerDelegate() = default;
...
void Button::DefaultButtonControllerDelegate::NotifyClick(const ui::Event& event)
{
    button()->NotifyClick(event);
}

void Button::DefaultButtonControllerDelegate::OnClickCanceled(const ui::Event& event)
{
    button()->OnClickCanceled(event);
}

bool Button::DefaultButtonControllerDelegate::IsTriggerableEvent(const ui::Event& event)
{
    return button()->IsTriggerableEvent(event);
}

bool Button::DefaultButtonControllerDelegate::ShouldEnterPushedState(const ui::Event& event)
{
...
int Button::DefaultButtonControllerDelegate::GetDragOperations(const gfx::Point& press_pt)
{
...
Button::PressedCallback::PressedCallback(Button::PressedCallback::Callback callback)
...
Button::PressedCallback& Button::PressedCallback::operator=(PressedCallback&&) = default;

Button::PressedCallback::~PressedCallback() = default;

Button::PressedCallback::operator bool() const
{
    return absl::visit([](const auto& callback) { return !callback.is_null(); }, callback_);
}
...
Button::ScopedAnchorHighlight::ScopedAnchorHighlight(base::WeakPtr<Button> button)
...
Button::ScopedAnchorHighlight::ScopedAnchorHighlight(Button::ScopedAnchorHighlight&&) = default;
...
Button::ScopedAnchorHighlight& Button::ScopedAnchorHighlight::operator=(Button::ScopedAnchorHighlight&& other)
{
    if (button_) {
        button_->ReleaseAnchorHighlight();
    }
...
void Button::SetTooltipText(const std::u16string& tooltip_text)
{
    ...
    if (GetViewAccessibility().GetCachedName().empty() || GetViewAccessibility().GetCachedName() == tooltip_text_) {
...
void Button::AdjustAccessibleName(std::u16string& new_name, ax::mojom::NameFrom& name_from)
{
    if (new_name.empty()) {
...
void Button::SetHideInkDropWhenShowingContextMenu(bool value)
{
    ...
    OnPropertyChanged(&hide_ink_drop_when_showing_context_menu_, kPropertyEffectsNone);
}
...
void Button::SetHotTracked(bool is_hot_tracked)
{
    if (state_ != STATE_DISABLED) {
        ...
        if (show_ink_drop_when_hot_tracked_) {
            InkDrop::Get(ink_drop_view_)->AnimateToState(is_hot_tracked ? views::InkDropState::ACTIVATED : views::InkDropState::HIDDEN, nullptr);
        }
    }
...
void Button::SetHighlighted(bool highlighted)
{
    ...
    InkDropState state = highlighted ? views::InkDropState::ACTIVATED : views::InkDropState::DEACTIVATED;
    if (InkDrop::Get(ink_drop_view_)->GetInkDrop()->GetTargetInkDropState() == state) {
        return;
    }
...
base::CallbackListSubscription Button::AddStateChangedCallback(PropertyChangedCallback callback)
{
    return AddPropertyChangedCallback(&state_, std::move(callback));
}

base::CallbackListSubscription Button::AddAnchorCountChangedCallback(base::RepeatingCallback<void(size_t)> callback)
{
    return anchor_count_changed_callbacks_.Add(std::move(callback));
}

Button::KeyClickAction Button::GetKeyClickActionForEvent(const ui::KeyEvent& event)
{
    ...
    if (event.key_code() == ui::VKEY_RETURN && PlatformStyle::kReturnClicksFocusedControl)
        return KeyClickAction::kOnKeyPress;
    return KeyClickAction::kNone;
}

void Button::SetButtonController(std::unique_ptr<ButtonController> button_controller)
{
...
gfx::Point Button::GetMenuPosition() const
{
    ...
    DCHECK(GetWidget());
    const int max_x_coordinate = GetWidget()->GetWorkAreaBoundsInScreen().right() - 1;
...
bool Button::OnMouseDragged(const ui::MouseEvent& event)
{
    if (state_ != STATE_DISABLED) {
        ...
        const bool should_show_pending = should_enter_pushed && button_controller_->notify_action() == ButtonController::NotifyAction::kOnRelease && !InDrag();
        if (HitTestPoint(event.location())) {
            SetState(should_enter_pushed ? STATE_PRESSED : STATE_HOVERED);
            if (should_show_pending && InkDrop::Get(ink_drop_view_)->GetInkDrop()->GetTargetInkDropState() == views::InkDropState::HIDDEN) {
                InkDrop::Get(ink_drop_view_)->AnimateToState(views::InkDropState::ACTION_PENDING, &event);
            }
        } else {
            SetState(STATE_NORMAL);
            if (should_show_pending && InkDrop::Get(ink_drop_view_)->GetInkDrop()->GetTargetInkDropState() == views::InkDropState::ACTION_PENDING) {
                InkDrop::Get(ink_drop_view_)->AnimateToState(views::InkDropState::HIDDEN, &event);
            }
        }
    }
    return true;
...
void Button::OnMouseCaptureLost()
{
    ...
    InkDrop::Get(ink_drop_view_)->AnimateToState(views::InkDropState::HIDDEN, nullptr /* event */);
...
void Button::ShowContextMenu(const gfx::Point& p, ui::mojom::MenuSourceType source_type)
{
    ...
    if (hide_ink_drop_when_showing_context_menu_) {
        InkDrop::Get(ink_drop_view_)->GetInkDrop()->SetHovered(false);
        InkDrop::Get(ink_drop_view_)->AnimateToState(InkDropState::HIDDEN, nullptr /* event */);
    }
...
void Button::OnBlur()
{
    ...
    if (IsHotTracked() || state_ == STATE_PRESSED) {
        SetState(STATE_NORMAL);
        if (InkDrop::Get(ink_drop_view_)->GetInkDrop()->GetTargetInkDropState() != views::InkDropState::HIDDEN) {
            InkDrop::Get(ink_drop_view_)->AnimateToState(views::InkDropState::HIDDEN, nullptr /* event */);
        }
    ...
    , callback_(std::move(callback))
{
    ...
    button_controller_ = std::make_unique<ButtonController>(this, std::make_unique<DefaultButtonControllerDelegate>(this));
    InkDrop::Get(ink_drop_view_)
        ->SetCreateInkDropCallback(base::BindRepeating(
            [](Button* button) {
                std::unique_ptr<InkDrop> ink_drop = InkDrop::CreateInkDropForFloodFillRipple(InkDrop::Get(button));
...
void Button::NotifyClick(const ui::Event& event)
{
    if (has_ink_drop_action_on_click_) {
        InkDrop::Get(ink_drop_view_)->AnimateToState(InkDropState::ACTION_TRIGGERED, ui::LocatedEvent::FromIfValid(&event));
    }
    ...
    if (element_id) {
        views::ElementTrackerViews::GetInstance()->NotifyViewActivated(element_id, this);
    }
...
void Button::OnClickCanceled(const ui::Event& event)
{
    if (ShouldUpdateInkDropOnClickCanceled()) {
        if (InkDrop::Get(ink_drop_view_)->GetInkDrop()->GetTargetInkDropState() == views::InkDropState::ACTION_PENDING
            || InkDrop::Get(ink_drop_view_)->GetInkDrop()->GetTargetInkDropState() == views::InkDropState::ALTERNATE_ACTION_PENDING) {
            InkDrop::Get(ink_drop_view_)->AnimateToState(views::InkDropState::HIDDEN, ui::LocatedEvent::FromIfValid(&event));
        }
    }
}
...
void Button::OnEnabledChanged()
{
    ...
    if (GetEnabled()) {
        ...
        InkDrop::Get(ink_drop_view_)->GetInkDrop()->SetHovered(should_enter_hover_state);
    } else {
...
void ButtonActionViewInterface::ActionItemChangedImpl(actions::ActionItem* action_item)
{
...
void ButtonActionViewInterface::LinkActionInvocationToView(base::RepeatingClosure invoke_action_callback)
{
    if (!action_view_) {
        return;
    }
...
DEFINE_ENUM_CONVERTERS(views::Button::ButtonState, { views::Button::STATE_NORMAL, u"STATE_NORMAL" }, { views::Button::STATE_HOVERED, u"STATE_HOVERED" },
    { views::Button::STATE_PRESSED, u"STATE_PRESSED" }, { views::Button::STATE_DISABLED, u"STATE_DISABLED" })
