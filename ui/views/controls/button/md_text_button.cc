...
MdTextButton::MdTextButton(PressedCallback callback, const std::u16string& text, int button_context, bool use_text_color_for_icon,
    std::unique_ptr<LabelButtonImageContainer> image_container)
    : LabelButton(std::move(callback), text, button_context, std::move(image_container))
    , use_text_color_for_icon_(use_text_color_for_icon)
{
    ...
    InkDrop::Get(this)->SetBaseColorCallback(base::BindRepeating([](MdTextButton* host) { return host->GetHoverColor(host->GetStyle()); }, this));
    ...
    const int minimum_width = LayoutProvider::Get()->GetDistanceMetric(DISTANCE_DIALOG_BUTTON_MINIMUM_WIDTH);
...
void MdTextButton::SetStyle(ui::ButtonStyle button_style)
{
    ...
    style_ = button_style;
    SetProperty(kDrawFocusRingBackgroundOutline, button_style == ui::ButtonStyle::kProminent);
    UpdateColors();
}
...
void MdTextButton::SetBgColorIdOverride(const std::optional<ui::ColorId> color_id)
{
...
void MdTextButton::SetBgColorOverrideDeprecated(const std::optional<SkColor>& color)
{
...
void MdTextButton::SetStrokeColorIdOverride(const std::optional<ui::ColorId> color_id)
{
...
void MdTextButton::SetStrokeColorOverrideDeprecated(const std::optional<SkColor>& color)
{
...
{
    return corner_radius_.value_or(LayoutProvider::Get()->GetCornerRadiusMetric(ShapeContextTokens::kButtonRadius, size()));
}
...
void MdTextButton::SetImageModel(ButtonState for_state, const std::optional<ui::ImageModel>& image_model)
{
...
{
    SetStyle(style_ == ui::ButtonStyle::kProminent || GetIsDefault() ? ui::ButtonStyle::kProminent : ui::ButtonStyle::kDefault);
    UpdateColors();
...
void MdTextButton::UpdatePadding()
{
    ...
    SetBorder(CreateEmptyBorder(custom_padding_.value_or(CalculateDefaultPadding())));
}

gfx::Insets MdTextButton::CalculateDefaultPadding() const
{
    int target_height = LayoutProvider::GetControlHeightForFont(label()->GetTextContext(), style::STYLE_PRIMARY, label()->font_list());
    ...
    int right_padding = LayoutProvider::Get()->GetDistanceMetric(DISTANCE_BUTTON_HORIZONTAL_PADDING);
    int left_padding = right_padding;
    if (HasImage(GetVisualState())) {
        constexpr int kLeftPadding = 12;
        left_padding = kLeftPadding;
    }
    return gfx::Insets::TLBR(top_padding, left_padding, bottom_padding, right_padding);
}
...
void MdTextButton::UpdateTextColor()
{
    ...
    LabelButton::SetEnabledTextColorIds(typography_provider.GetColorId(label()->GetTextContext(), text_style));
    ...
    if (GetState() == STATE_DISABLED) {
        LabelButton::SetTextColorId(STATE_DISABLED, typography_provider.GetColorId(label()->GetTextContext(), style::STYLE_DISABLED));
    }
...
void MdTextButton::UpdateBackgroundColor()
{
    ...
    if (bg_color_id_override_) {
        ...
        bg_color = color_provider->GetColor(HasFocus() ? ui::kColorButtonBackgroundProminentFocused : ui::kColorButtonBackgroundProminent);
        if (is_disabled) {
            bg_color = color_provider->GetColor(ui::kColorButtonBackgroundProminentDisabled);
        }
    } else if (style_ == ui::ButtonStyle::kTonal) {
        bg_color = color_provider->GetColor(HasFocus() ? ui::kColorButtonBackgroundTonalFocused : ui::kColorButtonBackgroundTonal);
        if (is_disabled) {
            bg_color = color_provider->GetColor(ui::kColorButtonBackgroundTonalDisabled);
        }
    }
    ...
    SkColor stroke_color = color_provider->GetColor(is_disabled ? ui::kColorButtonBorderDisabled : ui::kColorButtonBorder);
    ...
    } else if (style_ == ui::ButtonStyle::kProminent || style_ == ui::ButtonStyle::kText || style_ == ui::ButtonStyle::kTonal) {
        stroke_color = SK_ColorTRANSPARENT;
    }

    SetBackground(CreateBackgroundFromPainter(Painter::CreateRoundRectWith1PxBorderPainter(
        bg_color, stroke_color, GetCornerRadiusValue(), SkBlendMode::kSrcOver, true /* antialias */, true /* should_border_scale */)));
}

void MdTextButton::UpdateIconColor()
{
    if (use_text_color_for_icon_ && HasImage(ButtonState::STATE_NORMAL)) {
        const std::optional<ui::ImageModel>& image_model = GetImageModel(ButtonState::STATE_NORMAL);
        if (image_model.has_value() && image_model->IsVectorIcon()) {
            LabelButton::SetImageModel(ButtonState::STATE_NORMAL,
                ui::ImageModel::FromVectorIcon(
                    *image_model->GetVectorIcon().vector_icon(), LabelButton::GetCurrentTextColor(), image_model->GetVectorIcon().icon_size()));
        }
    }
}
...
MdTextButtonActionViewInterface::MdTextButtonActionViewInterface(MdTextButton* action_view)
    : LabelButtonActionViewInterface(action_view)
    , action_view_(action_view)
{
}

void MdTextButtonActionViewInterface::ActionItemChangedImpl(actions::ActionItem* action_item)
{
