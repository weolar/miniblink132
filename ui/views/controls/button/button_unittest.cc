...
public:
    explicit TestButton(bool has_ink_drop_action_on_click)
        : Button(base::BindRepeating([](bool* pressed) { *pressed = true; }, &pressed_))
    {
...
public:
    ...
    {
        highlighted_changed_subscription_ = InkDrop::Get(button)->AddHighlightedChangedCallback(
            base::BindRepeating([](TestButtonObserver* obs) { obs->highlighted_changed_ = true; }, base::Unretained(this)));
        state_changed_subscription_
            = button->AddStateChangedCallback(base::BindRepeating([](TestButtonObserver* obs) { obs->state_changed_ = true; }, base::Unretained(this)));
    }
    ...
    bool highlighted_changed() const
    {
        return highlighted_changed_;
    }
...
TestInkDrop* AddTestInkDrop(TestButton* button)
{
    ...
    InkDropHostTestApi(InkDrop::Get(button)).SetInkDrop(std::move(owned_ink_drop));
    return ink_drop;
}
...
public:
    ...
    {
        widget()->SetContentsView(std::make_unique<TestButton>(has_ink_drop_action_on_click));
...
protected:
    ...
    TestButtonObserver* button_observer()
    {
        return button_observer_.get();
    }
...
TEST_F(ButtonTest, NotifyAction)
{
    ...
    button()->OnMousePressed(
        ui::MouseEvent(ui::EventType::kMousePressed, center, center, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON));
    EXPECT_EQ(Button::STATE_PRESSED, button()->GetState());
    EXPECT_FALSE(button()->pressed());

    button()->OnMouseReleased(
        ui::MouseEvent(ui::EventType::kMouseReleased, center, center, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON));
    EXPECT_EQ(Button::STATE_HOVERED, button()->GetState());
    EXPECT_TRUE(button()->pressed());

    // Set the notify action to happen on mouse press.
    button()->Reset();
    button()->button_controller()->set_notify_action(ButtonController::NotifyAction::kOnPress);
    button()->OnMousePressed(
        ui::MouseEvent(ui::EventType::kMousePressed, center, center, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON));
    EXPECT_EQ(Button::STATE_PRESSED, button()->GetState());
    EXPECT_TRUE(button()->pressed());
...
TEST_F(ButtonTest, NotifyActionNoClick)
{
    ...
    // By default the button should notify the callback on mouse release.
    button()->OnMousePressed(
        ui::MouseEvent(ui::EventType::kMousePressed, center, center, ui::EventTimeForNow(), ui::EF_RIGHT_MOUSE_BUTTON, ui::EF_RIGHT_MOUSE_BUTTON));
    EXPECT_FALSE(button()->canceled());

    button()->OnMouseReleased(
        ui::MouseEvent(ui::EventType::kMouseReleased, center, center, ui::EventTimeForNow(), ui::EF_RIGHT_MOUSE_BUTTON, ui::EF_RIGHT_MOUSE_BUTTON));
...
{
    button()->SetCallback(base::BindRepeating([](TestButton* button) { button->SetEnabled(false); }, button()));
...
TEST_F(ButtonTest, HideInkDropHighlightWhenRemoved)
{
    ...
    TestButton* button = contents_view->AddChildView(std::make_unique<TestButton>(false));
...
TEST_F(ButtonTest, InkDropShowHideOnMouseDraggedNotifyOnRelease)
{
    ...
    TestInkDrop* ink_drop = CreateButtonWithInkDrop(false);
    button()->button_controller()->set_notify_action(ButtonController::NotifyAction::kOnRelease);
    ...
    button()->OnMouseDragged(ui::MouseEvent(ui::EventType::kMousePressed, oob, oob, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON));
    ...
    button()->OnMouseReleased(
        ui::MouseEvent(ui::EventType::kMousePressed, oob, oob, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON));
...
{
    bool pressed = false;
    button()->SetCallback(base::BindRepeating([](bool* pressed) { *pressed = true; }, &pressed));
...
{
    VisibilityTestButton* button = widget()->SetContentsView(std::make_unique<VisibilityTestButton>());
...
TEST_F(ButtonTest, ActionOnSpace)
{
    ...
    ui::KeyEvent space_press(ui::EventType::kKeyPressed, ui::VKEY_SPACE, ui::EF_NONE);
    ...
    ui::KeyEvent space_release(ui::EventType::kKeyReleased, ui::VKEY_SPACE, ui::EF_NONE);
...
TEST_F(ButtonTest, ActionOnReturn)
{
    ...
    ui::KeyEvent return_release(ui::EventType::kKeyReleased, ui::VKEY_RETURN, ui::EF_NONE);
...
TEST_F(ButtonTest, CustomActionOnKeyPressedEvent)
{
    ...
    ui::KeyEvent control_press(ui::EventType::kKeyPressed, ui::VKEY_CONTROL, ui::EF_NONE);
    EXPECT_TRUE(button()->OnKeyPressed(control_press));
    EXPECT_EQ(Button::STATE_NORMAL, button()->GetState());
    EXPECT_TRUE(button()->pressed());

    ui::KeyEvent control_release(ui::EventType::kKeyReleased, ui::VKEY_CONTROL, ui::EF_NONE);
...
TEST_F(ButtonTest, AccessibleRole)
{
    ...
    EXPECT_EQ(button()->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kButton);
    ...
    EXPECT_EQ(button()->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kCheckBox);
}
...
TEST_F(ButtonTest, AnchorHighlightDestructionClearsHighlight)
{
    ...
    std::optional<Button::ScopedAnchorHighlight> highlight = button()->AddAnchorHighlight();
    EXPECT_EQ(InkDropState::ACTIVATED, ink_drop->GetTargetInkDropState());
...
TEST_F(ButtonTest, NestedAnchorHighlights)
{
    ...
    std::optional<Button::ScopedAnchorHighlight> highlight1 = button()->AddAnchorHighlight();
    std::optional<Button::ScopedAnchorHighlight> highlight2 = button()->AddAnchorHighlight();
...
TEST_F(ButtonActionViewInterfaceTest, TestActionChanged)
{
    ...
    std::unique_ptr<actions::ActionItem> action_item = actions::ActionItem::Builder().SetTooltipText(test_string).SetActionId(0).SetEnabled(false).Build();
...
TEST_F(ButtonActionViewInterfaceTest, TestActionTriggered)
{
    ...
    bool called = false;
    button()->GetActionViewInterface()->LinkActionInvocationToView(base::BindRepeating([](bool* called_bool) { *called_bool = true; }, &called));
