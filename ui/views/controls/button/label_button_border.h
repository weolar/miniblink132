...
public:
    ...
    void SetPainter(bool focused, Button::ButtonState state, std::unique_ptr<Painter> painter);

private:
    // The painters used for each unfocused or focused button state.
    std::array<std::array<std::unique_ptr<Painter>, Button::STATE_COUNT>, 2> painters_;
};
