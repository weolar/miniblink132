...
MdTextButtonWithDownArrow::MdTextButtonWithDownArrow(PressedCallback callback, const std::u16string& text)
    : MdTextButton(std::move(callback), text)
{
    SetHorizontalAlignment(gfx::ALIGN_RIGHT);
    SetImageLabelSpacing(LayoutProvider::Get()->GetDistanceMetric(DISTANCE_DROPDOWN_BUTTON_LABEL_ARROW_SPACING));
    SetDropArrowImage();

    // Reduce padding between the drop arrow and the right border.
    const gfx::Insets original_padding = GetInsets();
    SetCustomPadding(gfx::Insets::TLBR(original_padding.top(), LayoutProvider::Get()->GetDistanceMetric(DISTANCE_DROPDOWN_BUTTON_LEFT_MARGIN),
        original_padding.bottom(), LayoutProvider::Get()->GetDistanceMetric(DISTANCE_DROPDOWN_BUTTON_RIGHT_MARGIN)));
}
...
void MdTextButtonWithDownArrow::SetDropArrowImage()
{
    ...
    auto drop_arrow_image = ui::ImageModel::FromVectorIcon(kArrowDropDownIcon, drop_arrow_color, kDropdownArrowSize);
