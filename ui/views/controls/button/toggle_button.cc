...
std::optional<SkColor> GetSkColorFromVariant(const absl::variant<ui::ColorId, SkColor>& color_variant)
{
    return absl::holds_alternative<SkColor>(color_variant) ? std::make_optional(absl::get<SkColor>(color_variant)) : std::nullopt;
}

SkColor ConvertVariantToSkColor(const absl::variant<ui::ColorId, SkColor> color_variant, const ui::ColorProvider* color_provider)
{
    return absl::holds_alternative<SkColor>(color_variant) ? absl::get<SkColor>(color_variant)
                                                           : color_provider->GetColor(absl::get<ui::ColorId>(color_variant));
}

} // namespace

class ToggleButton::FocusRingHighlightPathGenerator : public views::HighlightPathGenerator {
public:
...
public:
    explicit ThumbView(bool has_shadow)
    ...
    void Update(const gfx::Rect& bounds, float color_ratio, float hover_ratio, bool is_on, bool is_hovered)
    {
    ...
    {
        return has_shadow_ ? gfx::Insets(-kShadowBlur) + gfx::Vector2d(kShadowOffsetX, kShadowOffsetY) : gfx::Insets();
    }
...
private:
    ...
    void OnPaint(gfx::Canvas* canvas) override
    {
        ...
        if (has_shadow_) {
            ...
            gfx::ShadowValue shadow(gfx::Vector2d(kShadowOffsetX, kShadowOffsetY), 2 * kShadowBlur, color_provider->GetColor(ui::kColorToggleButtonShadow));
            shadows.push_back(shadow.Scale(dsf));
            thumb_flags.setLooper(gfx::CreateShadowDrawLooper(shadows));
        }
        thumb_flags.setAntiAlias(true);
        const SkColor thumb_on_color = ConvertVariantToSkColor(thumb_on_color_, color_provider);
        const SkColor thumb_off_color = ConvertVariantToSkColor(thumb_off_color_, color_provider);
        SkColor thumb_color = color_utils::AlphaBlend(thumb_on_color, thumb_off_color, color_ratio_);
        if (is_hovered_ && is_on_) {
            ...
            thumb_color = color_utils::AlphaBlend(color_provider->GetColor(ui::kColorToggleButtonThumbOnHover), thumb_color, hover_ratio_);
        }
        ...
        canvas->DrawCircle(thumb_bounds.CenterPoint(), thumb_bounds.height() / 2.f, thumb_flags);
    }
    ...
    void OnEnabledStateChanged()
    {
        ...
        if (absl::holds_alternative<ui::ColorId>(thumb_on_color_)) {
            thumb_on_color_ = GetEnabled() ? ui::kColorToggleButtonThumbOn : ui::kColorToggleButtonThumbOnDisabled;
        }

        if (absl::holds_alternative<ui::ColorId>(thumb_off_color_)) {
            thumb_off_color_ = GetEnabled() ? ui::kColorToggleButtonThumbOff : ui::kColorToggleButtonThumbOffDisabled;
        }
    }
    ...
    absl::variant<ui::ColorId, SkColor> thumb_on_color_ = ui::kColorToggleButtonThumbOn;
    absl::variant<ui::ColorId, SkColor> thumb_off_color_ = ui::kColorToggleButtonThumbOff;
    ...
    base::CallbackListSubscription enabled_state_changed_subscription_ { AddEnabledChangedCallback(
        base::BindRepeating(&ThumbView::OnEnabledStateChanged, base::Unretained(this))) };
};
    ...
    : Button(std::move(callback))
{
    ...
    InkDrop::Get(this)->SetCreateRippleCallback(base::BindRepeating(
        [](ToggleButton* host, gfx::Insets insets) -> std::unique_ptr<InkDropRipple> {
            ...
            std::unique_ptr<SquareInkDropRipple> ripple
                = std::make_unique<SquareInkDropRipple>(InkDrop::Get(host), gfx::Size(kHoverDiameter, kHoverDiameter), kHoverDiameter / 2, rect.size(),
                    rect.height() / 2, rect.CenterPoint(), SkColorSetA(pressed_color, SK_AlphaOPAQUE), pressed_alpha / SK_AlphaOPAQUE);
            ripple->set_activated_shape(views::SquareInkDropRipple::ActivatedShape::kCircle);
            return ripple;
        },
        this, -thumb_view_->GetShadowOutsets()));
    InkDrop::Get(this)->SetBaseColorCallback(
        base::BindRepeating([](ToggleButton* host) { return host->GetTrackColor(host->GetIsOn() || host->HasFocus()); }, this));
    InkDrop::Get(this)->SetCreateHighlightCallback(base::BindRepeating(
        [](ToggleButton* host) {
            ...
            auto ink_drop_highlight = std::make_unique<InkDropHighlight>(
                thumb_size, thumb_size.height() / 2, gfx::PointF(thumb_bounds.CenterPoint()), SkColorSetA(hover_color, SK_AlphaOPAQUE));
    ...
    FocusRing::Get(this)->SetPathGenerator(std::make_unique<FocusRingHighlightPathGenerator>());
...
void ToggleButton::AnimateIsOn(bool is_on)
{
    ...
    GetViewAccessibility().SetCheckedState(GetIsOn() ? ax::mojom::CheckedState::kTrue : ax::mojom::CheckedState::kFalse);
...
void ToggleButton::AddLayerToRegion(ui::Layer* layer, views::LayerRegion region)
{
...
gfx::Size ToggleButton::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
...
gfx::Rect ToggleButton::GetThumbBounds() const
{
    ...
    thumb_bounds.set_x(thumb_bounds.x() + slide_animation_.GetCurrentValue() * (thumb_bounds.width() - thumb_bounds.height()));
...
{
    thumb_view_->Update(GetThumbBounds(), static_cast<float>(slide_animation_.GetCurrentValue()), static_cast<float>(hover_animation_.GetCurrentValue()),
        GetIsOn(), IsMouseHovered());
    if (IsMouseHovered()) {
        InkDrop::Get(this)->GetInkDrop()->SetHovered(!slide_animation_.is_animating());
    }
...
{
    return ConvertVariantToSkColor(is_on ? track_on_color_ : track_off_color_, GetColorProvider());
}
...
void ToggleButton::NotifyClick(const ui::Event& event)
{
    ...
    InkDrop::Get(this)->AnimateToState(InkDropState::ACTION_TRIGGERED, ui::LocatedEvent::FromIfValid(&event));
...
void ToggleButton::StateChanged(ButtonState old_state)
{
    ...
    if (absl::holds_alternative<ui::ColorId>(track_on_color_)) {
        track_on_color_ = enabled ? ui::kColorToggleButtonTrackOn : ui::kColorToggleButtonTrackOnDisabled;
    }

    if (absl::holds_alternative<ui::ColorId>(track_off_color_)) {
        track_off_color_ = enabled ? ui::kColorToggleButtonTrackOff : ui::kColorToggleButtonTrackOffDisabled;
    }
...
void ToggleButton::PaintButtonContents(gfx::Canvas* canvas)
{
    ...
    const float color_ratio = static_cast<float>(slide_animation_.GetCurrentValue());
    track_flags.setColor(color_utils::AlphaBlend(GetTrackColor(true), GetTrackColor(false), color_ratio));
    ...
    if (!GetIsOn() && inner_border_enabled_) {
        ...
        track_flags.setColor(GetColorProvider()->GetColor(ui::kColorToggleButtonShadow));
