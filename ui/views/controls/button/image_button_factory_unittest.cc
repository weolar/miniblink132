...
public:
    ...
    ImageButtonFactoryWidgetTest& operator=(const ImageButtonFactoryWidgetTest&) = delete;
...
TEST_F(ImageButtonFactoryWidgetTest, SetImageFromVectorIconWithColor)
{
    ...
    SetImageFromVectorIconWithColor(button(), vector_icons::kCloseRoundedIcon, SK_ColorRED, SK_ColorRED);
...
{
    AddImageButton(CreateVectorImageButtonWithNativeTheme(Button::PressedCallback(), vector_icons::kCloseRoundedIcon));
    EXPECT_EQ(button()->GetColorProvider()->GetColor(ui::kColorIcon), InkDrop::Get(button())->GetBaseColor());
}

TEST_F(ImageButtonFactoryWidgetTest, CreateVectorImageButtonWithNativeThemeWithSize)
{
    constexpr int kSize = 15;
    AddImageButton(CreateVectorImageButtonWithNativeTheme(Button::PressedCallback(), vector_icons::kEditIcon, kSize));
...
{
    auto toggle_image_button = CreateVectorToggleImageButton(Button::PressedCallback());
    ui::AXNodeData data;
    const ui::ImageModel& image = ui::ImageModel::FromVectorIcon(gfx::VectorIcon(), 23, 56);
...
TEST_F(ImageButtonFactoryWidgetTest, AccessibleName)
{
    ...
    std::unique_ptr<ToggleImageButton> button = CreateVectorToggleImageButton(Button::PressedCallback());
    ...
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), untoggled_text);

    button->SetToggled(true);
    data = ui::AXNodeData();
    button->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), toggled_accessible_name);

    button->SetToggledAccessibleName(toggled_accessible_empty_name);
    data = ui::AXNodeData();
    button->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), toggled_tooltip_text);
