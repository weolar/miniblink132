...
TEST_F(RadioButtonTest, Focus)
{
    ...
    ui::KeyEvent pressed_tab(ui::EventType::kKeyPressed, ui::VKEY_TAB, ui::EF_NONE);
    ...
    focus_manager->OnKeyEvent(ui::KeyEvent(ui::EventType::kKeyPressed, ui::VKEY_DOWN, ui::EF_NONE));
    EXPECT_EQ(button2, focus_manager->GetFocusedView());
    EXPECT_FALSE(button1->GetChecked());
    EXPECT_TRUE(button2->GetChecked());

    focus_manager->OnKeyEvent(ui::KeyEvent(ui::EventType::kKeyPressed, ui::VKEY_UP, ui::EF_NONE));
    EXPECT_EQ(button1, focus_manager->GetFocusedView());
    EXPECT_TRUE(button1->GetChecked());
    EXPECT_FALSE(button2->GetChecked());
}
...
TEST_F(RadioButtonTest, FocusOnClick)
{
    ...
    const ui::MouseEvent event(ui::EventType::kMousePressed, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
