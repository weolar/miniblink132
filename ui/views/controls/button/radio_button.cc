...
void RadioButton::SetChecked(bool checked)
{
    ...
    if (checked) {
        ...
        for (views::View* peer : other) {
            if (peer != this) {
                DCHECK(IsViewClass<RadioButton>(peer)) << "radio-button-nt has same group as non radio-button-nt views.";
...
{
    return gfx::CreateVectorIcon(GetVectorIcon(), kRadioButtonIconDipSizeCr2023, GetIconImageColor(GetIconState(for_state)));
}

SkPath RadioButton::GetFocusRingPath() const
{
    SkPath path;
    const gfx::Point center = image_container_view()->GetMirroredBounds().CenterPoint();
