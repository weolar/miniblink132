...
TEST_F(MdTextButtonTest, BackgroundColorChangesWithWidgetActivation)
{
    ...
    auto* button = widget->SetContentsView(std::make_unique<MdTextButton>(Button::PressedCallback(), u" "));
    ...
    auto background_color = [button](const SkBitmap& bitmap) { return bitmap.getColor(button->size().width() / 2., button->size().height() / 2.); };

    EXPECT_EQ(background_color(active_bitmap), color_provider->GetColor(ui::kColorButtonBackgroundProminent));
    ...
    EXPECT_EQ(background_color(inactive_bitmap), color_provider->GetColor(ui::kColorButtonBackgroundProminentDisabled));
}
...
TEST_F(MdTextButtonActionViewInterfaceTest, TestActionChanged)
{
    ...
    md_text_button->GetActionViewInterface()->ActionItemChangedImpl(action_item.get());
...
TEST_F(MdTextButtonActionViewInterfaceTest, DefaultCornerRadiusDependsOnButtonSize)
{
    auto md_text_button = std::make_unique<MdTextButton>();
    constexpr gfx::Size kSize1(100, 100);
    constexpr gfx::Size kSize2(50, 50);

    const int corner_radius_1 = LayoutProvider::Get()->GetCornerRadiusMetric(ShapeContextTokens::kButtonRadius, kSize1);
    const int corner_radius_2 = LayoutProvider::Get()->GetCornerRadiusMetric(ShapeContextTokens::kButtonRadius, kSize2);
...
TEST_F(MdTextButtonActionViewInterfaceTest, CustomCornerRadiusIsNotOverriddenOnButtonSizeChange)
{
    auto md_text_button = std::make_unique<MdTextButton>();
