...
namespace {
constexpr Button::ButtonState kEnabledStates[] = { Button::STATE_NORMAL, Button::STATE_HOVERED, Button::STATE_PRESSED };
} // namespace

LabelButton::LabelButton(PressedCallback callback, const std::u16string& text, int button_context, std::unique_ptr<LabelButtonImageContainer> image_container)
    : Button(std::move(callback))
    , image_container_(std::move(image_container))
    , cached_normal_font_list_(TypographyProvider::Get().GetFont(button_context, style::STYLE_PRIMARY))
    , cached_default_button_font_list_(TypographyProvider::Get().GetFont(button_context, style::STYLE_DIALOG_BUTTON_DEFAULT))
    , appear_disabled_in_inactive_widget_(PlatformStyle::kInactiveWidgetControlsAppearDisabled)
{
    ink_drop_container_ = AddChildView(Builder<InkDropContainerView>().SetAutoMatchParentBounds(true).Build());
    ink_drop_container_->SetVisible(false);
    ink_drop_container_->SetProperty(kViewIgnoredByLayoutKey, true);

    AddChildView(image_container_->CreateView());

    label_ = AddChildView(std::make_unique<internal::LabelButtonLabel>(text, button_context));
...
const std::optional<ui::ImageModel>& LabelButton::GetImageModel(ButtonState state) const
{
    return button_state_image_models_[state];
}

void LabelButton::SetImageModel(ButtonState state, const std::optional<ui::ImageModel>& image_model)
{
    ...
    if (state == old_image_state || state == ImageStateForState(GetVisualState())) {
        UpdateImage();
    }
}
...
{
    return button_state_image_models_[state].has_value() && !button_state_image_models_[state]->IsEmpty();
}
...
void LabelButton::SetFocusRingCornerRadius(float radius)
{
    ...
    views::InstallRoundRectHighlightPathGenerator(this, gfx::Insets(), focus_ring_corner_radius_);
...
gfx::Size LabelButton::CalculatePreferredSize(const SizeBounds& available_size) const
{
    ...
    if (!shrinking_down_label_) {
        SizeBound available_width = max_size_.width() > 0 ? available_size.width().min_of(max_size_.width()) : available_size.width();
        SizeBound label_available_width = std::max<SizeBound>(0, available_width - size.width());

        const gfx::Size preferred_label_size = label_->GetPreferredSize(SizeBounds(label_available_width, {}));
        size.Enlarge(preferred_label_size.width(), 0);
        size.SetToMax(gfx::Size(0, preferred_label_size.height() + GetInsets().height()));
    }
...
ProposedLayout LabelButton::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
    ...
    gfx::Size image_size = image_container_view()->GetPreferredSize(SizeBounds(image_area.size()));
    ...
    gfx::Size label_size(std::min(label_area.width(), label_->GetPreferredSize(SizeBounds(label_area.size())).width()), label_area.height());
    ...
    if (label_->GetMultiLine() && !image_centered_) {
        ...
        int first_line_y = label_area.y() + (label_area.height() - label_height) / 2;
        int image_origin_y = first_line_y + (label_->font_list().GetHeight() - image_size.height()) / 2;
    ...
    if (horizontal_alignment == gfx::ALIGN_CENTER) {
        const int spacing = (image_size.width() > 0 && label_size.width() > 0) ? GetImageLabelSpacing() : 0;
    ...
    layouts.child_layouts.emplace_back(const_cast<LabelButton*>(this)->image_container_view(),
        static_cast<DelegatingLayoutManager*>(GetLayoutManager())->CanBeVisible(image_container_view()), gfx::Rect(image_origin, image_size), SizeBounds());
    ...
    layouts.child_layouts.emplace_back(
        label_.get(), static_cast<DelegatingLayoutManager*>(GetLayoutManager())->CanBeVisible(label_.get()), label_bounds, SizeBounds());
    layouts.host_size = gfx::Size(size_bounds.width().value(), size_bounds.height().value());
...
ui::NativeTheme::State LabelButton::GetThemeState(ui::NativeTheme::ExtraParams* params) const
{
...
ui::NativeTheme::State LabelButton::GetBackgroundThemeState(ui::NativeTheme::ExtraParams* params) const
{
    GetExtraParams(params);
    return ui::NativeTheme::kNormal;
}

ui::NativeTheme::State LabelButton::GetForegroundThemeState(ui::NativeTheme::ExtraParams* params) const
{
    GetExtraParams(params);
    return ui::NativeTheme::kHovered;
}
...
void LabelButton::AddLayerToRegion(ui::Layer* new_layer, views::LayerRegion region)
{
#if !BUILDFLAG(IS_WIN)
    image_container_view()->SetPaintToLayer();
    image_container_view()->layer()->SetFillsBoundsOpaquely(false);
...
void LabelButton::GetExtraParams(ui::NativeTheme::ExtraParams* params) const
{
    ...
    button.is_focused = HasFocus() && GetViewAccessibility().IsAccessibilityFocusable();
...
PropertyEffects LabelButton::UpdateStyleToIndicateDefaultStatus()
{
    ...
    DCHECK_EQ(cached_normal_font_list_.GetFontSize(), label()->font_list().GetFontSize());
    ...
    label_->SetFontList(GetIsDefault() ? cached_default_button_font_list_ : cached_normal_font_list_);
...
void LabelButton::AddedToWidget()
{
    if (appear_disabled_in_inactive_widget_) {
        paint_as_active_subscription_
            = GetWidget()->RegisterPaintAsActiveChangedCallback(base::BindRepeating(&LabelButton::VisualStateChanged, base::Unretained(this)));
...
void LabelButton::SetTextInternal(const std::u16string& text)
{
    ...
    const auto effects = shrinking_down_label_ ? kPropertyEffectsPreferredSizeChanged : kPropertyEffectsNone;
...
void LabelButton::ClearTextIfShrunkDown()
{
    ...
    if (shrinking_down_label_ && width() <= preferred_size.width() && height() <= preferred_size.height()) {
...
Button::ButtonState LabelButton::GetVisualState() const
{
    ...
    if (!widget || !widget->ShouldViewsStyleFollowWidgetActivation() || !appear_disabled_in_inactive_widget_) {
        return GetState();
    }

    // Paint as inactive if neither this widget nor its parent should paint as
    // active.
    if (!widget->ShouldPaintAsActive() && !(widget->parent() && widget->parent()->ShouldPaintAsActive()))
...
void LabelButton::ResetColorsFromNativeTheme()
{
    ...
    constexpr std::array<ui::ColorId, STATE_COUNT> color_ids { { ui::kColorLabelForeground, ui::kColorLabelForeground, ui::kColorLabelForeground,
...
void LabelButton::ResetLabelEnabledColor()
{
    ...
    const absl::variant<SkColor, ui::ColorId>& color = button_state_colors_[GetState()];
    if (absl::holds_alternative<SkColor>(color) && label_->GetEnabledColor() != absl::get<SkColor>(color)) {
...
Button::ButtonState LabelButton::ImageStateForState(ButtonState for_state) const
{
    return button_state_image_models_[for_state].has_value() ? for_state : STATE_NORMAL;
}

void LabelButton::FlipCanvasOnPaintForRTLUIChanged()
{
    image_container_view()->SetFlipCanvasOnPaintForRTLUI(GetFlipCanvasOnPaintForRTLUI());
}

LabelButtonActionViewInterface::LabelButtonActionViewInterface(LabelButton* action_view)
    : ButtonActionViewInterface(action_view)
    , action_view_(action_view)
{
}

void LabelButtonActionViewInterface::ActionItemChangedImpl(actions::ActionItem* action_item)
{
    ButtonActionViewInterface::ActionItemChangedImpl(action_item);
    action_view_->SetText(action_item->GetText());
    action_view_->SetImageModel(action_view_->GetState(), action_item->GetImage());
}
