...
namespace {
ui::EventType NotifyActionToMouseEventType(ButtonController::NotifyAction notify_action)
{
...
MenuButtonController::PressedLock::PressedLock(MenuButtonController* menu_button_controller)
    : PressedLock(menu_button_controller, false, nullptr)
{
}

MenuButtonController::PressedLock::PressedLock(MenuButtonController* menu_button_controller, bool is_sibling_menu_show, const ui::LocatedEvent* event)
    : menu_button_controller_(menu_button_controller->weak_factory_.GetWeakPtr())
{
    menu_button_controller_->IncrementPressedLocked(is_sibling_menu_show, event);
}

std::unique_ptr<MenuButtonController::PressedLock> MenuButtonController::TakeLock()
{
    return TakeLock(false, nullptr);
}

std::unique_ptr<MenuButtonController::PressedLock> MenuButtonController::TakeLock(bool is_sibling_menu_show, const ui::LocatedEvent* event)
{
    return std::make_unique<MenuButtonController::PressedLock>(this, is_sibling_menu_show, event);
}
...
MenuButtonController::MenuButtonController(Button* button, Button::PressedCallback callback, std::unique_ptr<ButtonControllerDelegate> delegate)
...
bool MenuButtonController::OnMousePressed(const ui::MouseEvent& event)
{
    ...
    // different event.
    is_intentional_menu_trigger_ = (TimeTicks::Now() - menu_closed_time_) >= kMinimumTimeBetweenButtonClicks;

    if (button()->GetRequestFocusOnPress())
        button()->RequestFocus();
    if (button()->GetState() != Button::STATE_DISABLED && button()->HitTestPoint(event.location()) && IsTriggerableEvent(event)) {
...
{
    if (button()->GetState() != Button::STATE_DISABLED && delegate()->IsTriggerableEvent(event) && button()->HitTestPoint(event.location())
        && !delegate()->InDrag()) {
        Activate(&event);
    } else {
...
void MenuButtonController::UpdateButtonAccessibleDefaultActionVerb()
{
    if (button()->GetEnabled()) {
        button()->GetViewAccessibility().SetDefaultActionVerb(ax::mojom::DefaultActionVerb::kOpen);
    } else {
...
{
    return ButtonController::IsTriggerableEvent(event) && IsTriggerableEventType(event) && is_intentional_menu_trigger_;
}
...
void MenuButtonController::OnGestureEvent(ui::GestureEvent* event)
{
    if (button()->GetState() != Button::STATE_DISABLED) {
        ...
        if (event->type() == ui::EventType::kGestureTapDown) {
            ...
            && (event->type() == ui::EventType::kGestureTapCancel || event->type() == ui::EventType::kGestureEnd) && pressed_lock_count_ == 0) {
            button()->SetState(Button::STATE_NORMAL);
        }
    }
...
bool MenuButtonController::Activate(const ui::Event* event)
{
    if (callback_) {
        ...
        // to NULL.
        static_cast<internal::RootView*>(button()->GetWidget()->GetRootView())->SetMouseAndGestureHandler(nullptr);
        ...
        const ui::ElementIdentifier id = button()->GetProperty(views::kElementIdentifierKey);
        if (id) {
            views::ElementTrackerViews::GetInstance()->NotifyViewActivated(id, button());
        }
        ...
        ui::KeyEvent fake_event(ui::EventType::kKeyPressed, ui::VKEY_SPACE, ui::EF_IS_SYNTHESIZED);
        if (!event)
            event = &fake_event;
        ...
        if (!increment_pressed_lock_called && pressed_lock_count_ == 0) {
            InkDrop::Get(button())->AnimateToState(InkDropState::ACTION_TRIGGERED, ui::LocatedEvent::FromIfValid(event));
        }
    ...
    InkDrop::Get(button())->AnimateToState(InkDropState::HIDDEN, ui::LocatedEvent::FromIfValid(event));
    return true;
}
...
bool MenuButtonController::IsTriggerableEventType(const ui::Event& event)
{
    if (event.IsMouseEvent()) {
        ...
        // notify_action.
        ui::EventType active_on = delegate()->GetDragOperations(mouse_event->location()) == ui::DragDropTypes::DRAG_NONE
...
void MenuButtonController::IncrementPressedLocked(bool snap_ink_drop_to_activated, const ui::LocatedEvent* event)
{
    ++pressed_lock_count_;
    ...
    if (!state_changed_subscription_) {
        state_changed_subscription_
            = button()->AddStateChangedCallback(base::BindRepeating(&MenuButtonController::OnButtonStateChangedWhilePressedLocked, base::Unretained(this)));
    }
...
void MenuButtonController::DecrementPressedLocked()
{
    ...
    if (pressed_lock_count_ == 0) {
        ...
        } else if (button()->GetWidget() && !button()->GetWidget()->dragged_view() && delegate()->ShouldEnterHoveredState()) {
        ...
        if (button()->GetWidget() && button()->GetState() != Button::STATE_PRESSED)
            InkDrop::Get(button())->AnimateToState(InkDropState::DEACTIVATED, nullptr /* event */);
    }
}
