...
StyledLabel::RangeStyleInfo& StyledLabel::RangeStyleInfo::operator=(const RangeStyleInfo&) = default;
StyledLabel::RangeStyleInfo::~RangeStyleInfo() = default;

// static
StyledLabel::RangeStyleInfo StyledLabel::RangeStyleInfo::CreateForLink(base::RepeatingClosure callback)
{
    // Adapt this closure to a Link::ClickedCallback by discarding the extra arg.
    return CreateForLink(base::BindRepeating([](base::RepeatingClosure closure, const ui::Event&) { closure.Run(); }, std::move(callback)));
}

// static
StyledLabel::RangeStyleInfo StyledLabel::RangeStyleInfo::CreateForLink(Link::ClickedCallback callback)
{
...
StyledLabel::LayoutSizeInfo& StyledLabel::LayoutSizeInfo::operator=(const LayoutSizeInfo&) = default;
StyledLabel::LayoutSizeInfo::~LayoutSizeInfo() = default;

bool StyledLabel::StyleRange::operator<(const StyledLabel::StyleRange& other) const
{
...
{
    return style_info.custom_font.value_or(TypographyProvider::Get().GetFont(text_context_, style_info.text_style.value_or(default_text_style_)));
}

void StyledLabel::AddStyleRange(const gfx::Range& range, const RangeStyleInfo& style_info)
{
...
void StyledLabel::SetDefaultEnabledColorId(std::optional<ui::ColorId> enabled_color_id)
{
...
{
    return line_height_.value_or(TypographyProvider::Get().GetLineHeight(text_context_, default_text_style_));
}
...
const StyledLabel::LayoutSizeInfo& StyledLabel::GetLayoutSizeInfoForWidth(int w) const
{
    if (auto it = layout_size_info_cache_.Get(w); it != layout_size_info_cache_.end()) {
        return it->second;
    }
    CalculateLayout(w);
...
void StyledLabel::SizeToFit(int fixed_width)
{
    ...
    gfx::Size size = CalculatePreferredSize(SizeBounds(fixed_width_ == 0 ? SizeBound() : SizeBound(width()), {}));
    size.set_width(std::max(size.width(), fixed_width));
    SetSize(size);
}

base::CallbackListSubscription StyledLabel::AddTextChangedCallback(views::PropertyChangedCallback callback)
{
...
gfx::Size StyledLabel::GetMinimumSize() const
{
    ...
    // fail.
    return CalculatePreferredSize(SizeBounds(width() == 0 ? SizeBound() : SizeBound(width()), {}));
}

gfx::Size StyledLabel::CalculatePreferredSize(const SizeBounds& available_size) const
{
    int width = 0;
    if (fixed_width_) {
        width = fixed_width_;
    ...
    return GetLayoutSizeInfoForWidth(width == 0 ? std::numeric_limits<int>::max() : width).total_size;
}
...
int StyledLabel::StartX(int excess_space) const
{
    ...
    if (horizontal_alignment_ == (base::i18n::IsRTL() ? gfx::ALIGN_RIGHT : gfx::ALIGN_LEFT)) {
        return x;
    }
    return x + (horizontal_alignment_ == gfx::ALIGN_CENTER ? (excess_space / 2) : excess_space);
}
...
void StyledLabel::CalculateLayout(int width) const
{
    ...
    if (width >= layout_size_info_.total_size.width() && width <= layout_size_info_.max_valid_width)
        return;
    ...
    for (std::u16string remaining_string = text_; content_width > 0 && !remaining_string.empty();) {
        ...
        while (!remaining_string.empty()) {
            if (views.empty() && can_trim_leading_whitespace) {
                if (remaining_string.front() == '\n') {
                    ...
                    base::TrimWhitespace(remaining_string, base::TRIM_LEADING, &remaining_string);
                }
            }
            ...
            if (position != range.start() || (current_range != style_ranges_.end() && !current_range->style_info.custom_view)) {
                const gfx::Rect chunk_bounds(line_size.width(), 0, content_width - line_size.width(), line_height);
                ...
                gfx::FontList text_font_list = GetFontList((position >= range.start()) ? current_range->style_info : RangeStyleInfo());
                int elide_result
                    = gfx::ElideRectangleText(remaining_string, text_font_list, chunk_bounds.width(), chunk_bounds.height(), gfx::WRAP_LONG_WORDS, &substrings);
                ...
                bool truncated_chunk = line_size.width() != 0 && (elide_result & gfx::INSUFFICIENT_SPACE_FOR_FIRST_WORD) != 0;
                if (substrings[0].empty() || truncated_chunk) {
                    ...
                    if ((line_size.width() != 0) || (layout_views_->views_per_line.size() > 1))
                        break;
            ...
            if (position >= range.start()) {
                ...
                if (custom_view && position == range.start() && line_size.width() != 0) {
                    SizeBounds chunk_size(content_width - line_size.width(), {});
                    int custom_view_width = custom_view->GetPreferredSize(chunk_size).width();
                ...
                if (!custom_view) {
                    label = CreateLabel(chunk, style_info, range, &previous_link_fragment);
                } else {
                ...
                label = CreateLabel(chunk, default_style, range, &previous_link_fragment);
            }

            View* child_view = custom_view ? custom_view : label.get();
            const gfx::Size child_size = child_view->GetPreferredSize(SizeBounds(content_width - line_size.width(), {}));
            // A custom view could be wider than the available width.
            line_size.SetSize(std::min(line_size.width() + child_size.width(), content_width), std::max(line_size.height(), child_size.height()));
            ...
            if (!custom_view && substrings.size() > 1 && chunk.size() == substrings[0].size()) {
                break;
            }
        }
    ...
    layout_size_info_.total_size.SetSize(max_width + insets.width(), total_height + insets.height());
}

std::unique_ptr<Label> StyledLabel::CreateLabel(
    const std::u16string& text, const RangeStyleInfo& style_info, const gfx::Range& range, LinkFragment** previous_link_fragment) const
{
    std::unique_ptr<Label> result;
    if (style_info.text_style == style::STYLE_LINK || style_info.text_style == style::STYLE_LINK_3 || style_info.text_style == style::STYLE_LINK_5) {
        ...
        auto link = std::make_unique<LinkFragment>(text, text_context_, *style_info.text_style, *previous_link_fragment);
        *previous_link_fragment = link.get();
        ...
        result = std::make_unique<Label>(text, Label::CustomFont { style_info.custom_font.value() });
    } else {
        result = std::make_unique<Label>(text, text_context_, style_info.text_style.value_or(default_text_style_));
    }
    ...
    if (absl::holds_alternative<SkColor>(displayed_on_background_color_)) {
        result->SetBackgroundColor(absl::get<SkColor>(displayed_on_background_color_));
    } else if (absl::holds_alternative<ui::ColorId>(displayed_on_background_color_)) {
...
void StyledLabel::UpdateLabelBackgroundColor()
{
    for (View* child : children()) {
        if (!child->GetProperty(kStyledLabelCustomViewKey)) {
            ...
            static_cast<Label*>(child)->SetBackgroundColorId(absl::holds_alternative<ui::ColorId>(displayed_on_background_color_)
                    ? std::optional<ui::ColorId>(absl::get<ui::ColorId>(displayed_on_background_color_))
                    : std::nullopt);
            if (absl::holds_alternative<SkColor>(displayed_on_background_color_)) {
                static_cast<Label*>(child)->SetBackgroundColor(absl::get<SkColor>(displayed_on_background_color_));
            }
        }
    }
}
...
void StyledLabel::RecreateChildViews()
{
    ...
    if (layout_views_) {
        ...
        DCHECK_EQ(layout_size_info_.line_sizes.size(), layout_views_->views_per_line.size());
        ...
        for (size_t line = 0; line < layout_views_->views_per_line.size(); ++line) {
            ...
            for (views::View* view : layout_views_->views_per_line[line]) {
                ...
                if (view->GetProperty(kStyledLabelCustomViewKey)) {
                    auto custom_view = base::ranges::find(custom_views_, view, &std::unique_ptr<View>::get);
