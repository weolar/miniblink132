...
void MenuSeparator::OnPaint(gfx::Canvas* canvas)
{
    ...
    if (type_ == ui::SPACING_SEPARATOR || width() < menu_config.separator_horizontal_border_padding * 2) {
        return;
    }
    ...
    gfx::Rect paint_rect(
        menu_config.separator_horizontal_border_padding, y, width() - menu_config.separator_horizontal_border_padding * 2, separator_thickness);
    if (type_ == ui::PADDED_SEPARATOR) {
        paint_rect.Inset(gfx::Insets::TLBR(0, menu_config.padded_separator_start_padding, 0, 0));
    }
    ...
    menu_separator.color_id = MenuController::GetActiveInstance()->GetSeparatorColorId();
    menu_separator.type = type_;
    GetNativeTheme()->Paint(canvas->sk_canvas(), GetColorProvider(), ui::NativeTheme::kMenuPopupSeparator, ui::NativeTheme::kNormal, GetLocalBounds(),
        ui::NativeTheme::ExtraParams(menu_separator));
}

gfx::Size MenuSeparator::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
    const MenuConfig& menu_config = MenuConfig::instance();
