...
MenuPreTargetHandlerAura::MenuPreTargetHandlerAura(MenuController* controller, Widget* owner)
    : controller_(controller)
    , root_(GetOwnerRootWindow(owner))
{
    if (root_) {
    ...
    aura::Env::GetInstance()->AddPreTargetHandler(this, ui::EventTarget::Priority::kSystem);
}
...
void MenuPreTargetHandlerAura::OnWindowActivated(wm::ActivationChangeObserver::ActivationReason reason, aura::Window* gained_active, aura::Window* lost_active)
{
...
bool MenuPreTargetHandlerAura::ShouldCancelMenuForEvent(const ui::KeyEvent& event)
{
...
// static
std::unique_ptr<MenuPreTargetHandler> MenuPreTargetHandler::Create(MenuController* controller, Widget* owner)
{
