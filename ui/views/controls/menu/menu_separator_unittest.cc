...
TEST_F(MenuSeparatorTest, AccessibleRole)
{
    ...
    IgnoreMissingWidgetForTestingScopedSetter a11y_ignore_missing_widget_(separator->GetViewAccessibility());

    ui::AXNodeData data;
    separator->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.role, ax::mojom::Role::kSplitter);
    EXPECT_EQ(separator->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kSplitter);
}
