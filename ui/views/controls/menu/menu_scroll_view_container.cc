...
public:
    ...
    {
        return gfx::Size(MenuConfig::instance().scroll_arrow_height * 2 - 1, pref_height_);
    }
    ...
    void OnPaint(gfx::Canvas* canvas) override
    {
        ...
        GetNativeTheme()->Paint(canvas->sk_canvas(), color_provider, ui::NativeTheme::kMenuItemBackground, ui::NativeTheme::kNormal, GetLocalBounds(),
            ui::NativeTheme::ExtraParams(absl::in_place_type<ui::NativeTheme::MenuItemExtraParams>));
        ...
        path.lineTo(SkIntToScalar(x - config.scroll_arrow_height), SkIntToScalar(y_bottom));
        path.lineTo(SkIntToScalar(x + config.scroll_arrow_height), SkIntToScalar(y_bottom));
...
public:
    MenuScrollView(View* child, MenuScrollViewContainer* owner)
        : owner_(owner)
    {
        AddChildView(child);
    }
    ...
    void ScrollRectToVisible(const gfx::Rect& rect) override
    {
        ...
        int old_y = child->y();
        int y = -std::max(0, std::min(child->GetPreferredSize({}).height() - this->height(), dy - child->y()));
        child->SetY(y);
...
MenuScrollViewContainer::MenuScrollViewContainer(SubmenuView* content_view)
    ...
    , use_ash_system_ui_layout_(content_view->GetMenuItem()->GetMenuController()->use_ash_system_ui_layout())
{
    ...
    auto* layout = background_view_->SetLayoutManager(std::make_unique<views::FlexLayout>());
    layout->SetOrientation(views::LayoutOrientation::kVertical);

    scroll_up_button_ = background_view_->AddChildView(std::make_unique<MenuScrollButton>(content_view, true));

    scroll_view_ = background_view_->AddChildView(std::make_unique<MenuScrollView>(content_view_, this));
    ...
    scroll_down_button_ = background_view_->AddChildView(std::make_unique<MenuScrollButton>(content_view, false));

    arrow_ = BubbleBorderTypeFromAnchor(content_view_->GetMenuItem()->GetMenuController()->GetAnchorPosition());
...
{
    return arrow_ != BubbleBorder::NONE || (MenuConfig::instance().use_bubble_border && GetCornerRadius());
}

MenuItemView* MenuScrollViewContainer::GetFootnote() const
{
    MenuItemView* const footnote = content_view_->GetLastItem();
    return (footnote && footnote->GetType() == MenuItemView::Type::kHighlighted) ? footnote : nullptr;
}

int MenuScrollViewContainer::GetCornerRadius() const
{
    return MenuConfig::instance().CornerRadiusForMenu(content_view_->GetMenuItem()->GetMenuController());
}
...
gfx::RoundedCornersF MenuScrollViewContainer::GetRoundedCorners() const
{
    ...
    std::optional<gfx::RoundedCornersF> rounded_corners = menu_controller->rounded_corners();
...
gfx::Size MenuScrollViewContainer::CalculatePreferredSize(const SizeBounds& available_size) const
{
    gfx::Size prefsize = scroll_view_->GetContents()->GetPreferredSize(available_size);
...
void MenuScrollViewContainer::OnPaintBackground(gfx::Canvas* canvas)
{
    ...
    if (border_color_id_.has_value()) {
        ...
        canvas->DrawRoundRect(GetLocalBounds(), menu_background.corner_radius, flags);
        return;
    }
    GetNativeTheme()->Paint(canvas->sk_canvas(), color_provider, ui::NativeTheme::kMenuPopupBackground, ui::NativeTheme::kNormal, bounds,
...
void MenuScrollViewContainer::OnBoundsChanged(const gfx::Rect& previous_bounds)
{
    ...
    scroll_down_button_->SetVisible(content_view_->GetPreferredSize({}).height() > GetContentsBounds().height());

    const bool any_scroll_button_visible = scroll_up_button_->GetVisible() || scroll_down_button_->GetVisible();
...
void MenuScrollViewContainer::CreateDefaultBorder()
{
    ...
    const int vertical_inset
        = corner_radius_ ? menu_config.rounded_menu_vertical_border_size.value_or(corner_radius_) : menu_config.nonrounded_menu_vertical_border_size;
    const int horizontal_inset = menu_config.menu_horizontal_border_size;
    const bool has_footnote = !!GetFootnote();
    auto insets = gfx::Insets::TLBR(vertical_inset, horizontal_inset, has_footnote ? 0 : vertical_inset, horizontal_inset);
    ...
    if (border_color_id_.has_value()) {
        SetBorder(views::CreateThemedSolidSidedBorder(insets, border_color_id_.value()));
        return;
    }

    SetBackground(CreateThemedRoundedRectBackground(ui::kColorMenuBackground, corner_radius_, views::RoundRectPainter::kBorderWidth));

    const auto* const color_provider = GetColorProvider();
    SkColor color = color_provider ? color_provider->GetColor(ui::kColorMenuBorder) : gfx::kPlaceholderColor;
    if (has_footnote) {
        insets.set_bottom(views::RoundRectPainter::kBorderWidth);
    }
    SetBorder(views::CreateBorderPainter(std::make_unique<views::RoundRectPainter>(color, corner_radius_), insets));
}
...
void MenuScrollViewContainer::CreateBubbleBorder()
{
    ...
    bubble_border->set_md_shadow_elevation(
        content_view_->GetMenuItem()->GetParentMenuItem() ? menu_config.bubble_submenu_shadow_elevation : menu_config.bubble_menu_shadow_elevation);
    ...
    if (use_ash_system_ui_layout_ || border_radius) {
        if (const auto* const menu_controller = content_view_->GetMenuItem()->GetMenuController();
            use_ash_system_ui_layout_ && menu_controller && menu_controller->rounded_corners().has_value()) {
    ...
    const gfx::Insets border_thickness(menu_config.use_outer_border ? BubbleBorder::kBorderThicknessDip : 0);
    outside_border_insets_ = bubble_border->GetInsets() - border_thickness;
    additional_insets_ = gfx::Insets::VH(use_ash_system_ui_layout_ ? menu_config.vertical_touchable_menu_item_padding
                                                                   : menu_config.rounded_menu_vertical_border_size.value_or(border_radius),
    ...
    if (use_ash_system_ui_layout_) {
        ...
        scroll_view_->GetContents()->SetBorder(CreateEmptyBorder(std::exchange(additional_insets_, {})));

        background_view_->SetBackground(CreateThemedRoundedRectBackground(id, corner_radius_));
        background_view_->layer()->SetRoundedCornerRadius(GetRoundedCorners());

#if BUILDFLAG(IS_CHROMEOS)
        background_view_->SetBorder(std::make_unique<HighlightBorder>(GetRoundedCorners(), HighlightBorder::Type::kHighlightBorderOnShadow));
#endif
    } else {
...
BubbleBorder::Arrow MenuScrollViewContainer::BubbleBorderTypeFromAnchor(MenuAnchorPosition anchor)
{
    switch (anchor) {
