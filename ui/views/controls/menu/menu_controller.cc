...
DEFINE_OWNED_UI_CLASS_PROPERTY_KEY(std::vector<views::ViewTracker>, kOrderedMenuChildren, nullptr)
...
bool AcceleratorShouldCancelMenu(const ui::Accelerator& accelerator)
{
    ...
    if (accelerator.key_code() == ui::VKEY_CONTROL || accelerator.key_code() == ui::VKEY_MENU || // aka Alt
        accelerator.key_code() == ui::VKEY_COMMAND || accelerator.key_code() == ui::VKEY_SHIFT) {
        return false;
    }
    ...
    // this code path.
    return accelerator.IsCtrlDown() || accelerator.IsAltDown() || accelerator.IsCmdDown();
}
#endif
...
bool ShouldIgnoreScreenBoundsForMenus()
{
    ...
    return !ui::OzonePlatform::GetInstance()->GetPlatformProperties().supports_global_screen_coordinates;
#else
    return false;
#endif
}
...
constexpr base::TimeDelta kAlertAnimationThrobDuration = base::Milliseconds(1000);
...
MenuPartType GetScrollButtonAt(SubmenuView* source, const gfx::Point& location)
{
    MenuScrollViewContainer* scroll_view = source->GetScrollViewContainer();
    if (const View* child_under_mouse = scroll_view->GetEventHandlerForPoint(location); child_under_mouse && child_under_mouse->GetEnabled()) {
...
gfx::Point ConvertFromScreen(const SubmenuView& submenu, const gfx::Point& location)
{
    return View::ConvertPointFromScreen(submenu.GetWidget()->GetRootView(), location);
}
gfx::Point ConvertToScreen(const SubmenuView& submenu, const gfx::Point& location)
{
    return View::ConvertPointToScreen(submenu.GetWidget()->GetRootView(), location);
}

template <typename T> T ConvertLocatedEventForRootView(const SubmenuView& submenu, const MenuHostRootView& root_view, const T& event)
{
    T converted_event = event;
    if (submenu.GetWidget()->GetRootView() != &root_view) {
        converted_event.set_location(View::ConvertPointFromScreen(&root_view, ConvertToScreen(submenu, event.location())));
    }
...
gfx::Point GetLocationInRootMenu(const SubmenuView& submenu, const gfx::Point& location)
{
    // Because the root menu and `submenu` may not be in the same widget, we need
    // to round-trip through screen coordinates.
    return ConvertFromScreen(GetRootMenu(submenu), ConvertToScreen(submenu, location));
}

bool Contains(const SubmenuView& submenu, const gfx::Point& location)
{
    return submenu.GetWidget()->GetRootView()->GetLocalBounds().Contains(location);
}
...
// incorrect.
View* GetFirstFocusableViewForward(View* view, View::Views::const_iterator pos)
{
...
View* GetFirstFocusableViewBackward(View* view, View::Views::const_reverse_iterator pos)
{
    for (auto i = pos; i != view->children().crend(); ++i) {
        View* deepest = GetFirstFocusableViewBackward(*i, (*i)->children().crbegin());
        if (deepest)
            return deepest;
    }
    return view->IsFocusable() ? view : nullptr;
...
View* GetInitialFocusableView(View* start, bool forward)
{
    ...
    return forward ? GetFirstFocusableViewForward(start, children.cbegin()) : GetFirstFocusableViewBackward(start, children.crbegin());
}
...
View* GetNextFocusableView(View* ancestor, View* start_at, bool forward)
{
    ...
    do {
        ...
        View* next
            = forward ? GetFirstFocusableViewForward(new_parent, std::next(pos)) : GetFirstFocusableViewBackward(new_parent, std::make_reverse_iterator(pos));
        if (next)
            return next;
        parent = new_parent;
...
static void RepostEventImpl(const ui::LocatedEvent* event, const gfx::Point& screen_loc, gfx::NativeView native_view, gfx::NativeWindow window)
{
    ...
    gfx::Point screen_loc_pixels = display::win::ScreenWin::DIPToScreenPoint(screen_loc);
    ...
    if (event->IsMouseEvent()) {
        ...
        if (!target_window || !source_window || GetWindowThreadProcessId(source_window, nullptr) != GetWindowThreadProcessId(target_window, nullptr)) {
        ...
        WPARAM target = client_area ? event->native_event().wParam : static_cast<WPARAM>(nc_hit_result);
        LPARAM window_coords = MAKELPARAM(window_x, window_y);
        PostMessage(target_window, event->native_event().message, target, window_coords);
        return;
    }
    ...
    std::unique_ptr<ui::LocatedEvent> located_event(static_cast<ui::LocatedEvent*>(clone.release()));
...
public:
    ...
    void Update(const MenuController::MenuPart& part)
    {
        ...
        if (!scrolling_timer_.IsRunning()) {
            scrolling_timer_.Start(FROM_HERE, base::Hertz(60), this, &MenuScrollTask::Run);
        }
    }
    ...
    const SubmenuView* submenu() const
    {
        return submenu_;
    }
...
private:
    void Run()
    {
        ...
        const int delta_y = base::ClampRound((base::Time::Now() - start_scroll_time_).InSecondsF() * pixels_per_second_);
...
void MenuController::Run(Widget* parent, MenuButtonController* button_controller, MenuItemView* root, const gfx::Rect& anchor_bounds,
    MenuAnchorPosition position, bool context_menu, bool is_nested_drag, gfx::NativeView native_view_for_gestures)
{
        ...
        if (root_view) {
            ...
            if (event && event->type() == ui::EventType::kMousePressed) {
                menu_start_mouse_press_loc_
                    = View::ConvertPointToScreen(static_cast<View*>(event->target()), static_cast<const ui::MouseEvent*>(event)->location());
            }
        }
    }
...
#if BUILDFLAG(IS_MAC)
    menu_cocoa_watcher_ = std::make_unique<MenuCocoaWatcherMac>(base::BindOnce(&MenuController::Cancel, this->AsWeakPtr(), ExitType::kAll));
#endif
    ...
    if (button_controller) {
        pressed_lock_ = button_controller->TakeLock(false, ui::LocatedEvent::FromIfValid(event));
    }
...
void MenuController::Cancel(ExitType type)
{
    ...
    if (for_drop_) {
        ...
        DCHECK(selected);
        showing_ = false;
        delegate_->OnMenuClosed(internal::MenuControllerDelegate::NOTIFY_DELEGATE, selected->GetRootMenuItem(), accept_event_flags_);
...
void MenuController::AddNestedDelegate(internal::MenuControllerDelegate* delegate)
{
...
bool MenuController::OnMousePressed(SubmenuView* source, const ui::MouseEvent& event)
{
    ...
    // view of the target.
    MenuHostRootView* forward_to_root = current_mouse_pressed_state_ ? current_mouse_event_target_.get() : GetRootView(source, event.location());

    current_mouse_pressed_state_ |= event.changed_button_flags();

    if (forward_to_root) {
        // Reset hot-tracking if a different view is getting a mouse press.
        const ui::MouseEvent event_for_root = ConvertLocatedEventForRootView(*source, *forward_to_root, event);
...
bool MenuController::OnMouseDragged(SubmenuView* source, const ui::MouseEvent& event)
{
    if (current_mouse_event_target_) {
        return current_mouse_event_target_->ProcessMouseDragged(ConvertLocatedEventForRootView(*source, *current_mouse_event_target_, event));
    }
    ...
    if (part.type == MenuPartType::kMenuItem) {
        ...
        if (part.menu || !part.submenu) {
            ...
            SetSelection(part.menu ? part.menu.get() : state_.item.get(), SELECTION_OPEN_SUBMENU);
        }
        ...
        if (!ShowSiblingMenu(source, event.location())) {
            if (!part.is_scroll() && pending_state_.item && pending_state_.item->GetParentMenuItem() && !pending_state_.item->SubmenuIsShowing()) {
                SetSelection(pending_state_.item->GetParentMenuItem(), SELECTION_OPEN_SUBMENU);
            }
        }
    }
...
void MenuController::OnMouseReleased(SubmenuView* source, const ui::MouseEvent& event)
{
    current_mouse_pressed_state_ &= ~event.changed_button_flags();
    ...
    if (current_mouse_event_target_) {
        ...
        cached_event_target->ProcessMouseReleased(ConvertLocatedEventForRootView(*source, *cached_event_target, event));
        return;
    }
    ...
    if (event.IsRightMouseButton() && part.type == MenuPartType::kMenuItem) {
        ...
        if (!menu && part.submenu->children().size() == 1 && IsViewClass<EmptyMenuMenuItem>(part.submenu->children().front())) {
            menu = part.parent;
        }

        if (menu) {
            if (ShowContextMenu(menu, ConvertToScreen(*source, event.location()), ui::mojom::MenuSourceType::kMouse)) {
                return;
            }
        }
    }
    ...
    bool plain_left_click_with_children = part.should_submenu_show && part.menu && part.menu->HasSubmenu() && (event.flags() & ui::EF_LEFT_MOUSE_BUTTON)
    ...
    if (!part.is_scroll() && part.menu && !plain_left_click_with_children) {
        ...
        if (time_shown < menu_selection_hold_time) {
            ...
            const gfx::Vector2d moved = ConvertToScreen(*source, event.location()) - menu_start_mouse_press_loc_;
        ...
        if (part.menu->GetDelegate()->ShouldExecuteCommandWithoutClosingMenu(command, event)) {
            part.menu->GetDelegate()->ExecuteCommand(command, event.flags());
            return;
        }
        if ((!part.menu->NonIconChildViewsCount() || part.menu->GetTriggerActionWithNonIconChildViews())
        ...
        SetSelection(part.menu ? part.menu.get() : state_.item.get(), SELECTION_OPEN_SUBMENU | SELECTION_UPDATE_IMMEDIATELY);
    }
    SendMouseCaptureLostToActiveView();
    MaybeForwardToAnnotation(source, event);
}

void MenuController::OnMouseMoved(SubmenuView* source, const ui::MouseEvent& event)
{
    if (current_mouse_event_target_) {
        current_mouse_event_target_->ProcessMouseMoved(ConvertLocatedEventForRootView(*source, *current_mouse_event_target_, event));
        return;
    }
    ...
    if (menu_open_mouse_loc_ && *menu_open_mouse_loc_ == GetLocationInRootMenu(*source, event.location())) {
        return;
    }
    ...
    if (root_view) {
        ...
        View* view = root_view->GetEventHandlerForPoint(ConvertLocatedEventForRootView(*source, *root_view, event).location());
...
void MenuController::OnMouseEntered(SubmenuView* source, const ui::MouseEvent& event)
{
    // MouseEntered is always followed by a mouse moved, so don't need to
    // do anything here.
}

bool MenuController::OnMouseWheel(SubmenuView* source, const ui::MouseWheelEvent& event)
{
...
void MenuController::OnGestureEvent(SubmenuView* source, ui::GestureEvent* event)
{
    if (owner_ && send_gesture_events_to_owner()) {
#if defined(USE_AURA)
        gfx::NativeView target = native_view_for_gestures_ ? native_view_for_gestures_ : owner()->GetNativeWindow();
        event->ConvertLocationToTarget(source->GetWidget()->GetNativeWindow(), target);
    ...
    if (event->type() == ui::EventType::kGestureTapDown) {
        ...
        if (part.type == MenuPartType::kMenuItem && part.menu) {
            if (ShowContextMenu(part.menu, ConvertToScreen(*source, event->location()), ui::mojom::MenuSourceType::kTouch)) {
                event->StopPropagation();
            }
        }
    } else if (event->type() == ui::EventType::kGestureTap) {
        if (!part.is_scroll() && part.menu && !(part.should_submenu_show && part.menu->HasSubmenu())) {
            const int command = part.menu->GetCommand();
            if (part.menu->GetDelegate()->ShouldExecuteCommandWithoutClosingMenu(command, *event)) {
                item_selected_by_touch_ = true;
                part.menu->GetDelegate()->ExecuteCommand(command, 0);
            } else if (part.menu->GetDelegate()->IsTriggerableEvent(part.menu, *event)) {
                item_selected_by_touch_ = true;
                Accept(part.menu, event->flags());
            }
            event->StopPropagation();
    ...
    } else if (event->type() == ui::EventType::kGestureTapCancel && part.menu && part.type == MenuPartType::kMenuItem) {
        ...
        SetSelection(part.menu->GetParentMenuItem(), SELECTION_OPEN_SUBMENU | SELECTION_UPDATE_IMMEDIATELY);
        event->StopPropagation();
    }
...
View* MenuController::GetTooltipHandlerForPoint(SubmenuView* source, const gfx::Point& point)
{
    MenuHostRootView* root_view = GetRootView(source, point);
    return root_view ? root_view->ProcessGetTooltipHandlerForPoint(point) : nullptr;
}

void MenuController::ViewHierarchyChanged(SubmenuView* source, const ViewHierarchyChangedDetails& details)
{
    if (!details.is_add) {
...
bool MenuController::GetDropFormats(SubmenuView* source, int* formats, std::set<ui::ClipboardFormatType>* format_types)
{
    return source->GetMenuItem()->GetDelegate()->GetDropFormats(source->GetMenuItem(), formats, format_types);
}

bool MenuController::AreDropTypesRequired(SubmenuView* source)
{
    return source->GetMenuItem()->GetDelegate()->AreDropTypesRequired(source->GetMenuItem());
}

bool MenuController::CanDrop(SubmenuView* source, const OSExchangeData& data)
{
    return source->GetMenuItem()->GetDelegate()->CanDrop(source->GetMenuItem(), data);
}

void MenuController::OnDragEntered(SubmenuView* source, const ui::DropTargetEvent& event)
{
    valid_drop_coordinates_ = false;
}

int MenuController::OnDragUpdated(SubmenuView* source, const ui::DropTargetEvent& event)
{
    StopCancelAllTimer();

    const gfx::Point screen_loc = View::ConvertPointToScreen(source, event.location());
    ...
    if (!menu_item) {
        ...
        menu_item = AsViewClass<EmptyMenuMenuItem>(source->GetTooltipHandlerForPoint(event.location()));
        if (menu_item)
    ...
    if (menu_item) {
        ...
        if (!over_empty_menu) {
            ...
            if (menu_item->HasSubmenu() && (menu_item_loc.y() > kDropBetweenPixels && menu_item_loc.y() < (menu_item_height - kDropBetweenPixels))) {
                ...
                drop_position = (menu_item_loc.y() < menu_item_height / 2) ? MenuDelegate::DropPosition::kBefore : MenuDelegate::DropPosition::kAfter;
            }
        ...
        drop_operation = static_cast<int>(menu_item->GetDelegate()->GetDropOperation(query_menu_item, event, &drop_position));
        ...
        if (drop_position == MenuDelegate::DropPosition::kNone || drop_operation == ui::DragDropTypes::DRAG_NONE)
            menu_item = nullptr;
    } else {
...
views::View::DropCallback MenuController::GetDropCallback(SubmenuView* source, const ui::DropTargetEvent& event)
{
    DCHECK(drop_target_);
    ...
    if (for_drop_) {
        delegate_->OnMenuClosed(internal::MenuControllerDelegate::DONT_NOTIFY_DELEGATE, item->GetRootMenuItem(), accept_event_flags_);
    }

    // WARNING: the call to MenuClosed deletes us.

    return drop_target->GetDelegate()->GetDropCallback(drop_target, drop_position, event);
}

void MenuController::OnDragEnteredScrollButton(SubmenuView* source, bool is_up)
{
    MenuPart part;
...
ui::PostDispatchAction MenuController::OnWillDispatchKeyEvent(ui::KeyEvent* event)
{
    ...
    if (event->type() == ui::EventType::kKeyPressed) {
        ...
        if (!IsEditableCombobox() && !event->stopped_propagation()) {
            ...
            const int kKeyFlagsMask = ui::EF_CONTROL_DOWN | ui::EF_ALT_DOWN | ui::EF_COMMAND_DOWN;
    ...
    ViewsDelegate::ProcessMenuAcceleratorResult result = ViewsDelegate::GetInstance()->ProcessAcceleratorWhileMenuShowing(accelerator);
    ...
    if (IsEditableCombobox()) {
        const base::flat_set<ui::KeyboardCode> kKeysThatDontPropagate = { ui::VKEY_DOWN, ui::VKEY_UP, ui::VKEY_ESCAPE, ui::VKEY_F4, ui::VKEY_RETURN };
        if (kKeysThatDontPropagate.find(event->key_code()) == kKeysThatDontPropagate.end())
            return ui::POST_DISPATCH_PERFORM_DEFAULT;
    }
    event->SetSkipped();
    return ui::POST_DISPATCH_NONE;
...
void MenuController::UpdateSubmenuSelection(SubmenuView* submenu)
{
    if (submenu->IsShowing()) {
        HandleMouseLocation(submenu, ConvertFromScreen(GetRootMenu(*submenu), display::Screen::GetScreen()->GetCursorScreenPoint()));
    }
}
...
void MenuController::SetSelection(MenuItemView* menu_item, int selection_types)
{
    ...
    BuildPathsAndCalculateDiff(pending_state_.item, menu_item, &current_path, &new_path, &paths_differ_at);
    ...
    const bool pending_item_changed = pending_state_.item != menu_item || pending_state_.submenu_open != !!(selection_types & SELECTION_OPEN_SUBMENU);

    if (pending_item_changed && pending_state_.item)
        SetHotTrackedButton(nullptr);

    // Notify an accessibility focus event on all menu items except for the root.
    bool ensure_focus_within_popup = menu_item && pending_item_changed
        && (MenuDepth(menu_item) != 1 || menu_item->GetType() != MenuItemView::Type::kSubMenu
            || (menu_item->GetType() == MenuItemView::Type::kActionableSubMenu && (selection_types & SELECTION_OPEN_SUBMENU) == 0));
    ...
    MenuDelegate* current_delegate = current_path.empty() ? nullptr : current_path.front()->GetDelegate();
    for (size_t i = paths_differ_at; i < current_size; ++i) {
        if (current_delegate
            && (current_path[i]->GetType() == MenuItemView::Type::kSubMenu || current_path[i]->GetType() == MenuItemView::Type::kActionableSubMenu)) {
    ...
    for (size_t i = paths_differ_at; i < new_size; ++i) {
        ...
        if (new_path[i]->GetType() == MenuItemView::Type::kActionableSubMenu) {
            new_path[i]->SetSelectionOfActionableSubmenu((selection_types & SELECTION_OPEN_SUBMENU) != 0);
        }
    }
    if (menu_item && menu_item->GetType() == MenuItemView::Type::kActionableSubMenu) {
        menu_item->SetSelectionOfActionableSubmenu((selection_types & SELECTION_OPEN_SUBMENU) != 0);
    }
    ...
    if (ensure_focus_within_popup) {
        ...
        // submenu.
        if (menu_item->GetParentMenuItem() && menu_item->GetParentMenuItem()->GetSubmenu()) {
            menu_item->GetParentMenuItem()->GetSubmenu()->NotifyAccessibilityEvent(ax::mojom::Event::kSelectedChildrenChanged,
                /*send_native_event=*/true);
        }
    }
}

void MenuController::SetSelectionOnPointerDown(SubmenuView* source, const ui::LocatedEvent* event)
{
    if (for_drop_)
        return;
    ...
    if (part.type == MenuPartType::kNone
        || (part.type == MenuPartType::kMenuItem && part.menu && part.menu->GetRootMenuItem() != state_.item->GetRootMenuItem())) {
...
void MenuController::StartDrag(SubmenuView* source, const gfx::Point& location)
{
    MenuItemView* item = state_.item;
    DCHECK(item);
    ...
    const gfx::Point press_loc = View::ConvertPointFromScreen(item, ConvertToScreen(*source, location));
    ...
    gfx::ImageSkia image = gfx::ImageSkia::CreateFromBitmap(canvas.GetBitmap(), raster_scale);
    ...
    item->GetWidget()->RunShellDrag(nullptr, std::move(data), widget_loc, drag_ops, ui::mojom::DragEventSource::kMouse);
    if (!this_ref) {
        return;
    }
...
// On Mac, treat space the same as return.
    ...
    case ui::VKEY_SPACE:
        ...
        // same first word.
        if (key_code == ui::VKEY_SPACE && MenuConfig::instance().all_menus_use_prefix_selection && ShouldContinuePrefixSelection()) {
            break;
        }
        if (pending_state_.item) {
            if (pending_state_.item->HasSubmenu()) {
                if ((key_code == ui::VKEY_F4 || (key_code == ui::VKEY_RETURN && IsEditableCombobox())) && pending_state_.item->GetSubmenu()->IsShowing())
                    Cancel(ExitType::kAll);
                else
                    OpenSubmenuChangeSelectionIfCan();
            } else {
                handled_key_code = true;
                if (!SendAcceleratorToHotTrackedView(event.flags()) && pending_state_.item->GetEnabled()) {
                    const int command = pending_state_.item->GetCommand();
                    if (pending_state_.item->GetDelegate()->ShouldExecuteCommandWithoutClosingMenu(command, event)) {
    ...
    case ui::VKEY_ESCAPE:
        if (!state_.item->GetParentMenuItem() || (!state_.item->GetParentMenuItem()->GetParentMenuItem() && (!state_.item->SubmenuIsShowing()))) {
            ...
            // menu.
            Cancel(delegate_stack_.size() > 1 ? ExitType::kOutermost : ExitType::kAll);
            break;
        }
        CloseSubmenu();
        break;
    ...
    case ui::VKEY_APPS: {
        ...
        if (hot_view) {
            hot_view->ShowContextMenu(hot_view->GetKeyboardContextMenuLocation(), ui::mojom::MenuSourceType::kKeyboard);
        } else if (pending_state_.item->GetEnabled() && pending_state_.item->GetRootMenuItem() != pending_state_.item) {
            ...
            ShowContextMenu(pending_state_.item, pending_state_.item->GetKeyboardContextMenuLocation(), ui::mojom::MenuSourceType::kKeyboard);
        }
        break;
    }
#endif
...
MenuController::MenuController(bool for_drop, internal::MenuControllerDelegate* delegate)
    : for_drop_(for_drop)
    , result_(nullptr)
...
void MenuController::UpdateInitialLocation(const gfx::Rect& anchor_bounds, MenuAnchorPosition position, bool context_menu)
{
    ...
    const display::Display display = display::Screen::GetScreen()->GetDisplayNearestPoint(anchor_bounds.origin());
...
// static
MenuAnchorPosition MenuController::AdjustAnchorPositionForRtl(MenuAnchorPosition position)
{
    if (!base::i18n::IsRTL())
        return position;
...
void MenuController::Accept(MenuItemView* item, int event_flags)
{
    ...
    menu_closure_animation_ = std::make_unique<MenuClosureAnimationMac>(
        item, item->GetParentMenuItem()->GetSubmenu(), base::BindOnce(&MenuController::ReallyAccept, base::Unretained(this)));
...
void MenuController::ReallyAccept()
{
    ...
    if (result_ && !menu_stack_.empty() && !result_->GetDelegate()->ShouldCloseAllMenusOnExecute(result_->GetCommand())) {
...
bool MenuController::ShowSiblingMenu(SubmenuView* source, const gfx::Point& mouse_location)
{
    ...
    if (!owner_ || !display::Screen::GetScreen()->IsWindowUnderCursor(owner_->GetNativeWindow())) {
        return false;
    }
    ...
    MenuItemView* alt_menu
        = source->GetMenuItem()->GetDelegate()->GetSiblingMenu(source->GetMenuItem()->GetRootMenuItem(), screen_point, &anchor, &has_mnemonics, &button);
    ...
    UpdateInitialLocation(button->GetBoundsInScreen(), anchor, state_.context_menu);
    alt_menu->PrepareForRun(has_mnemonics, source->GetMenuItem()->GetRootMenuItem()->show_mnemonics_);
    alt_menu->controller_ = AsWeakPtr();
    SetSelection(alt_menu, SELECTION_OPEN_SUBMENU | SELECTION_UPDATE_IMMEDIATELY);
    return true;
}

bool MenuController::ShowContextMenu(MenuItemView* menu_item, const gfx::Point& screen_location, ui::mojom::MenuSourceType source_type)
{
    ...
    if (menu_item->GetDelegate()->ShowContextMenu(menu_item, menu_item->GetCommand(), screen_location, source_type)) {
        SendMouseCaptureLostToActiveView();
        return true;
    }
    return false;
...
void MenuController::CloseAllNestedMenus()
{
    for (auto& nested_menu : menu_stack_) {
        ...
        for (MenuItemView* item = last_item; item; item = item->GetParentMenuItem()) {
            CloseMenu(item);
            last_item = item;
        }
...
MenuItemView* MenuController::GetMenuItemAt(View* source, const gfx::Point& location)
{
    ...
    return (child_under_mouse && child_under_mouse->GetEnabled()) ? AsViewClass<MenuItemView>(child_under_mouse) : nullptr;
}

MenuController::MenuPart MenuController::GetMenuPart(SubmenuView* source, const gfx::Point& source_loc)
{
    return GetMenuPartByScreenCoordinateUsingMenu(state_.item, ConvertToScreen(*source, source_loc));
}

MenuController::MenuPart MenuController::GetMenuPartByScreenCoordinateUsingMenu(MenuItemView* item, const gfx::Point& screen_loc)
{
    MenuPart part;
    for (; item; item = item->GetParentMenuItem()) {
        if (item->SubmenuIsShowing() && GetMenuPartByScreenCoordinateImpl(item->GetSubmenu(), screen_loc, &part)) {
            return part;
        }
    }
    return part;
...
bool MenuController::GetMenuPartByScreenCoordinateImpl(SubmenuView* menu, const gfx::Point& screen_loc, MenuPart* part)
{
    ...
    if (menu->GetVisibleBounds().Contains(menu_loc)) {
        ...
        part->submenu = menu;
        part->should_submenu_show = part->submenu && part->menu
            && (part->menu->GetType() == MenuItemView::Type::kSubMenu || IsLocationOverSubmenuAreaOfActionableSubmenu(part->menu, screen_loc));
        if (!part->menu)
...
MenuHostRootView* MenuController::GetRootView(SubmenuView* submenu, const gfx::Point& source_loc)
{
    MenuPart part = GetMenuPart(submenu, source_loc);
    SubmenuView* view = part.submenu;
    return view && view->GetWidget() ? static_cast<MenuHostRootView*>(view->GetWidget()->GetRootView()) : nullptr;
}

bool MenuController::IsLocationOverSubmenuAreaOfActionableSubmenu(MenuItemView* item, const gfx::Point& screen_loc) const
{
...
void MenuController::CommitPendingSelection()
{
    ...
    std::vector<MenuItemView*> current_path;
    std::vector<MenuItemView*> new_path;
    BuildPathsAndCalculateDiff(state_.item, pending_state_.item, &current_path, &new_path, &paths_differ_at);
    ...
    if (scroll_task_.get() && scroll_task_->submenu()) {
        ...
        bool found = false;
        for (MenuItemView* item = state_.item; item && !found; item = item->GetParentMenuItem()) {
            found = (item->SubmenuIsShowing() && item->GetSubmenu() == scroll_task_->submenu());
        }
        if (!found)
            StopScrollingViaButton();
    }
...
void MenuController::OpenMenuImpl(MenuItemView* item, bool show)
{
    ...
    const MenuOpenDirection preferred_open_direction = GetChildMenuOpenDirectionAtDepth(menu_depth);
    ...
    bool calculate_as_bubble_menu = MenuItemView::IsBubble(state_.anchor) || (menu_config.use_bubble_border && menu_config.CornerRadiusForMenu(this));
    gfx::Rect bounds = calculate_as_bubble_menu ? CalculateBubbleMenuBounds(item, preferred_open_direction, &resulting_direction, &anchor)
                                                : CalculateMenuBounds(item, preferred_open_direction, &resulting_direction, &anchor);
    ...
    if (show) {
        ...
        if (item->GetSubmenu()->GetWidget()) {
            const gfx::Point mouse_pos = ConvertFromScreen(*item->submenu_, display::Screen::GetScreen()->GetCursorScreenPoint());
            MenuPart part_under_mouse = GetMenuPart(item->submenu_.get(), mouse_pos);
            if (part_under_mouse.type != MenuPartType::kNone) {
                menu_open_mouse_loc_ = GetLocationInRootMenu(*item->submenu_, mouse_pos);
            }
        }

        item->GetSubmenu()->GetWidget()->SetNativeWindowProperty(
            TooltipManager::kGroupingPropertyKey, reinterpret_cast<void*>(MenuConfig::kMenuControllerGroupingId));
...
void MenuController::BuildPathsAndCalculateDiff(
    MenuItemView* old_item, MenuItemView* new_item, std::vector<MenuItemView*>* old_path, std::vector<MenuItemView*>* new_path, size_t* first_diff_at)
{
    DCHECK(old_path && new_path && first_diff_at);
    BuildMenuItemPath(old_item, old_path);
    BuildMenuItemPath(new_item, new_path);

    *first_diff_at = static_cast<size_t>(std::distance(old_path->begin(), base::ranges::mismatch(*old_path, *new_path).first));
}

void MenuController::BuildMenuItemPath(MenuItemView* item, std::vector<MenuItemView*>* path)
{
    if (!item)
        return;
...
{
    show_timer_.Start(FROM_HERE, base::Milliseconds(MenuConfig::instance().show_delay), this, &MenuController::CommitPendingSelection);
}
...
{
    cancel_all_timer_.Start(FROM_HERE, base::Milliseconds(kCloseOnExitTime), base::BindOnce(&MenuController::Cancel, base::Unretained(this), ExitType::kAll));
}
...
gfx::Rect MenuController::CalculateMenuBounds(
    MenuItemView* item, MenuOpenDirection preferred_open_direction, MenuOpenDirection* resulting_direction, ui::OwnedWindowAnchor* anchor)
{
    DCHECK(item);
    DCHECK(anchor);
    ...
    gfx::Rect anchor_bounds = is_child_menu ? item->GetBoundsInScreen() : state_.initial_bounds;
    if (is_child_menu) {
    ...
    menu_bounds.set_width(std::min(menu_bounds.width(), item->GetDelegate()->GetMaxWidthForMenu(item)));
    if (!monitor_bounds.IsEmpty()) {
        menu_bounds.set_width(std::min(menu_bounds.width(), monitor_bounds.width()));
        menu_bounds.set_height(std::min(menu_bounds.height(), monitor_bounds.height()));
    }
    ...
    *resulting_direction = preferred_open_direction;
    ...
    if (is_child_menu) {
        ...
        const bool create_on_right
            = layout_is_rtl ? preferred_open_direction == MenuOpenDirection::kTrailing : preferred_open_direction == MenuOpenDirection::kLeading;

        const int left_of_parent = anchor_bounds.x() - menu_bounds.width() + menu_config.submenu_horizontal_overlap;
        const int right_of_parent = anchor_bounds.right() - menu_config.submenu_horizontal_overlap;

        menu_bounds.set_y(anchor_bounds.y() - scroll_view_container->GetInsets().top());
        ...
        if (menu_bounds.x() < monitor_bounds.x()) {
            *resulting_direction = layout_is_rtl ? MenuOpenDirection::kTrailing : MenuOpenDirection::kLeading;
            menu_bounds.set_x(right_of_parent);
        } else if (menu_bounds.right() > monitor_bounds.right()) {
            *resulting_direction = layout_is_rtl ? MenuOpenDirection::kLeading : MenuOpenDirection::kTrailing;
        ...
        if (state_.anchor == MenuAnchorPosition::kTopRight) {
            ...
            // it below.
            const int horizontally_centered = anchor_bounds.x() + (anchor_bounds.width() - menu_bounds.width()) / 2;
            menu_bounds.set_x(horizontally_centered);
            menu_bounds.set_y(above_anchor - kTouchYPadding);
            if (!ShouldIgnoreScreenBoundsForMenus() && menu_bounds.y() < monitor_bounds.y())
        ...
        if (item->actual_menu_position() == MenuPosition::kBelowBounds || item->actual_menu_position() == MenuPosition::kAboveBounds) {
    ...
    menu_bounds.set_x(std::clamp(menu_bounds.x(), monitor_bounds.x(), monitor_bounds.right() - menu_bounds.width()));
    menu_bounds.set_y(std::clamp(menu_bounds.y(), monitor_bounds.y(), monitor_bounds.bottom() - menu_bounds.height()));
    ...
    MenuItemView* item, MenuOpenDirection preferred_open_direction, MenuOpenDirection* resulting_direction, ui::OwnedWindowAnchor* anchor)
{
    ...
    *resulting_direction = preferred_open_direction;
    ...
    menu_size.set_width(std::min(menu_size.width(), item->GetDelegate()->GetMaxWidthForMenu(item)));

    // For comboboxes, ensure the menu is at least as wide as the anchor.
    const gfx::Insets border_insets = scroll_view_container->outside_border_insets();
    if (IsCombobox()) {
    ...
    if (!is_child_menu) {
        ...
        if (!monitor_bounds.IsEmpty()) {
            ...
            if (!state_.context_menu) {
                ...
                if (is_anchored_bubble || is_bubble_menu || item->actual_menu_position() == MenuPosition::kAboveBounds) {
                    // menu_size is expected to include not just the content size
                    // but also the (border and shadow) insets, which can go offscreen.
                    max_height = std::max(anchor_bounds.y() - monitor_bounds.y(), monitor_bounds.bottom() - anchor_bounds.bottom())
                        - (is_bubble_menu ? 0 : menu_config.touchable_anchor_offset) + border_insets.height();
                }
            }
        ...
        case MenuAnchorPosition::kBottomCenter:
            ...
            x_menu_on_left = anchor_bounds.right() - (state_.anchor == MenuAnchorPosition::kBottomCenter ? menu_size.width() / 2 : menu_size.width())
                + border_insets.right();
            ...
            y_menu_above = anchor_bounds.y() - menu_size.height() + border_insets.bottom() - (is_anchored_bubble ? menu_config.touchable_anchor_offset : 0);
            // Align the top of the menu with the bottom of the anchor.
            y_menu_below = anchor_bounds.bottom() - border_insets.top() + (is_anchored_bubble ? menu_config.touchable_anchor_offset : 0);
            break;
        case MenuAnchorPosition::kBubbleLeft:
        case MenuAnchorPosition::kBubbleRight:
            // Align the right edge of the menu with the left edge of the anchor.
            x_menu_on_left = anchor_bounds.x() - menu_size.width() + border_insets.right() - menu_config.touchable_anchor_offset;
            // Align the left edge of the menu with the right edge of the anchor.
            x_menu_on_right = anchor_bounds.right() - border_insets.left() + menu_config.touchable_anchor_offset;
            // Align the bottom of the menu with the bottom of the anchor.
            y_menu_above = anchor_bounds.bottom() - menu_size.height() + border_insets.bottom();
        ...
        case MenuAnchorPosition::kTopRight:
        case MenuAnchorPosition::kBottomCenter:
            x = x_menu_on_left + border_insets.left() >= monitor_bounds.x() ? x_menu_on_left : x_menu_on_right;
            break;
        case MenuAnchorPosition::kBubbleTopRight:
        case MenuAnchorPosition::kBubbleRight:
        case MenuAnchorPosition::kBubbleBottomRight:
        case MenuAnchorPosition::kTopLeft:
            x = x_menu_on_right + menu_size.width() - border_insets.right() <= monitor_bounds.right() ? x_menu_on_right : x_menu_on_left;
            break;
        }

        // Choose the most appropriate y coordinate.
        const bool able_to_show_menu_below = y_menu_below + menu_size.height() - border_insets.bottom() <= monitor_bounds.bottom();
        const bool able_to_show_menu_above = y_menu_above + border_insets.top() >= monitor_bounds.y();
        switch (state_.anchor) {
        case MenuAnchorPosition::kBubbleLeft:
        case MenuAnchorPosition::kBubbleRight:
        case MenuAnchorPosition::kBubbleBottomLeft:
        ...
        case MenuAnchorPosition::kBottomCenter:
            ...
            if (able_to_show_menu_below && (item->actual_menu_position() != MenuPosition::kAboveBounds || !able_to_show_menu_above)) {
                y = y_menu_below;
                item->set_actual_menu_position(MenuPosition::kBelowBounds);
                ...
                y = monitor_bounds.bottom() + border_insets.bottom() - menu_size.height();
        ...
        case MenuAnchorPosition::kBubbleTopRight:
            // Respect the actual menu position calculated earlier if possible, to
            // prevent changing positions during menu size updates.
            if (able_to_show_menu_above && (item->actual_menu_position() != MenuPosition::kBelowBounds || !able_to_show_menu_below)) {
                y = y_menu_above;
                item->set_actual_menu_position(MenuPosition::kAboveBounds);
        ...
        const int x_max = monitor_bounds.right() - menu_size.width() + border_insets.right();
        const int y_min = monitor_bounds.y() - border_insets.top();
        const int y_max = monitor_bounds.bottom() - menu_size.height() + border_insets.bottom();
        ...
        const bool create_on_right
            = base::i18n::IsRTL() ? preferred_open_direction == MenuOpenDirection::kTrailing : preferred_open_direction == MenuOpenDirection::kLeading;

        const int width_with_right_inset = menu_size.width() - border_insets.right();
        const int x_min = monitor_bounds.x() - border_insets.left();
        const int x_max = monitor_bounds.right() - width_with_right_inset;
        const int x_left = anchor_bounds.x() - width_with_right_inset + menu_config.submenu_horizontal_overlap;
        const int x_right = anchor_bounds.right() - border_insets.left() - menu_config.submenu_horizontal_overlap;
        if (create_on_right) {
            ...
            if (monitor_bounds.width() == 0 || x_left >= x_min) {
                ...
                *resulting_direction = preferred_open_direction == MenuOpenDirection::kLeading ? MenuOpenDirection::kTrailing : MenuOpenDirection::kLeading;
                x = x_right;
            } else {
        ...
        menu_size.set_height(std::min(menu_size.height(), monitor_bounds.height() + border_insets.height()));
        y = anchor_bounds.y() - border_insets.top()
            - (use_ash_system_ui_layout_ ? menu_config.vertical_touchable_menu_item_padding
                                         : menu_config.rounded_menu_vertical_border_size.value_or(corner_radius));
        auto y_min = monitor_bounds.y() - border_insets.top();
        auto y_max = monitor_bounds.bottom() + border_insets.bottom() - menu_size.height();
        y = std::clamp(y, y_min, y_max);
    }
...
void MenuController::IncrementSelection(SelectionIncrementDirectionType direction)
{
    ...
    if (!item->children().empty()) {
        ...
        View* to_make_hot = button ? GetNextFocusableView(item, button, direction_is_down) : GetInitialFocusableView(item, direction_is_down);
...
void MenuController::SetSelectionIndices(MenuItemView* parent)
{
    ...
    for (MenuItemView* item : submenu->GetMenuItems()) {
        ...
        if (!item->children().empty()) {
            for (View* child = GetInitialFocusableView(item, true); child; child = GetNextFocusableView(item, child, true)) {
    ...
    parent->SetProperty(kOrderedMenuChildren, std::make_unique<std::vector<ViewTracker>>(ordering.begin(), ordering.end()));
...
void MenuController::MoveSelectionToFirstOrLastItem(SelectionIncrementDirectionType direction)
{
    MenuItemView* item = pending_state_.item;
    DCHECK(item);
...
MenuItemView* MenuController::FindInitialSelectableMenuItem(MenuItemView* parent, SelectionIncrementDirectionType direction)
{
...
MenuController::SelectByCharDetails MenuController::FindChildForMnemonic(
    MenuItemView* parent, char16_t key, bool (*match_function)(MenuItemView* menu, char16_t mnemonic))
{
    ...
    for (size_t i = 0; i < menu_items.size(); ++i) {
        ...
        if (child->GetEnabled() && child->GetVisible()) {
            ...
            if (match_function(child, key)) {
                ...
                if (!details.next_match.has_value() && details.index_of_item.has_value() && i > details.index_of_item)
...
void MenuController::AcceptOrSelect(MenuItemView* parent, const SelectByCharDetails& details)
{
    ...
    if (!details.has_multiple) {
        ...
        if (submenu->GetMenuItemAt(details.first_match.value())->HasSubmenu()) {
            SetSelection(submenu->GetMenuItemAt(details.first_match.value()), SELECTION_OPEN_SUBMENU | SELECTION_UPDATE_IMMEDIATELY);
        } else {
        ...
        SetSelection(submenu->GetMenuItemAt(details.first_match.value()), SELECTION_DEFAULT);
    } else {
        SetSelection(submenu->GetMenuItemAt(details.next_match.value()), SELECTION_DEFAULT);
    }
}
...
void MenuController::SelectByChar(char16_t character)
{
    ...
    if (IsReadonlyCombobox() || MenuConfig::instance().all_menus_use_prefix_selection) {
        item->GetSubmenu()->GetPrefixSelector()->InsertText(char_array, ui::TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    } else {
...
void MenuController::RepostEventAndCancel(SubmenuView* source, const ui::LocatedEvent* event)
{
    ...
    if (event->IsMouseEvent() || event->IsTouchEvent()) {
        ...
        if (state_.item) {
            ...
            gfx::NativeWindow window = native_view ? display::Screen::GetScreen()->GetWindowAtScreenPoint(screen_loc) : nullptr;
    ...
    if (!menu_stack_.empty()) {
        ...
        MenuPart last_part = GetMenuPartByScreenCoordinateUsingMenu(menu_stack_.back().first.item, screen_loc);
    ...
    menu_closure_animation_ = std::make_unique<MenuClosureAnimationMac>(
        nullptr, state_.item->GetSubmenu(), base::BindOnce(&MenuController::Cancel, base::Unretained(this), exit_type));
    menu_closure_animation_->Start();
#else
    Cancel(exit_type);
#endif
...
void MenuController::SetDropMenuItem(MenuItemView* new_target, MenuDelegate::DropPosition new_position)
{
    if (new_target == drop_target_ && new_position == drop_position_)
        return;

    if (drop_target_) {
        drop_target_->GetParentMenuItem()->GetSubmenu()->SetDropMenuItem(nullptr, MenuDelegate::DropPosition::kNone);
    }
    drop_target_ = new_target;
    drop_position_ = new_position;
    if (drop_target_) {
        drop_target_->GetParentMenuItem()->GetSubmenu()->SetDropMenuItem(drop_target_, drop_position_);
    }
}
...
void MenuController::UpdateActiveMouseView(SubmenuView* event_source, const ui::MouseEvent& event, View* target_menu)
{
    View* target = nullptr;
    ...
    if (target != active_mouse_view) {
        ...
        if (active_mouse_view) {
            ...
            ui::MouseEvent mouse_entered_event(ui::EventType::kMouseEntered, target_point, target_point, ui::EventTimeForNow(), 0, 0);
            active_mouse_view->OnMouseEntered(mouse_entered_event);

            ui::MouseEvent mouse_pressed_event(
                ui::EventType::kMousePressed, target_point, target_point, ui::EventTimeForNow(), event.flags(), event.changed_button_flags());
    ...
    if (active_mouse_view) {
        ...
        ui::MouseEvent mouse_dragged_event(
            ui::EventType::kMouseDragged, target_point, target_point, ui::EventTimeForNow(), event.flags(), event.changed_button_flags());
        active_mouse_view->OnMouseDragged(mouse_dragged_event);
    }
}

void MenuController::SendMouseReleaseToActiveView(SubmenuView* event_source, const ui::MouseEvent& event)
{
    View* active_mouse_view = active_mouse_view_tracker_->view();
    if (!active_mouse_view)
        return;

    const gfx::Point target_loc = View::ConvertPointFromScreen(active_mouse_view, ConvertToScreen(*event_source, event.location()));
    ui::MouseEvent release_event(ui::EventType::kMouseReleased, target_loc, target_loc, ui::EventTimeForNow(), event.flags(), event.changed_button_flags());
...
void MenuController::ExitMenu()
{
    ...
    delegate->OnMenuClosed(internal::MenuControllerDelegate::NOTIFY_DELEGATE, result, accept_event_flags);
...
MenuItemView* MenuController::ExitTopMostMenu()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&ViewsDelegate::ReleaseRef, base::Unretained(ViewsDelegate::GetInstance())));
    ...
    if (item_selected_by_touch_) {
        ...
        if (::GetWindowThreadProcessId(window, nullptr) == ::GetCurrentThreadId()) {
            ::SetProp(window, ui::kIgnoreTouchMouseActivateForWindow, reinterpret_cast<HANDLE>(true));
        }
    }
#endif
...
void MenuController::HandleMouseLocation(SubmenuView* source, const gfx::Point& mouse_location)
{
    if (showing_submenu_)
        return;
    ...
    if (part.type == MenuPartType::kNone && ShowSiblingMenu(source, mouse_location)) {
        return;
    }

    if (part.type == MenuPartType::kMenuItem && part.menu) {
        SetSelection(part.menu, part.should_submenu_show ? SELECTION_OPEN_SUBMENU : SELECTION_DEFAULT);
    } else if (!part.is_scroll() && pending_state_.item && pending_state_.item->GetParentMenuItem() && !pending_state_.item->SubmenuIsShowing()) {
...
void MenuController::SetInitialHotTrackedView(MenuItemView* item, SelectionIncrementDirectionType direction)
{
    if (!item)
        return;
    SetSelection(item, SELECTION_DEFAULT);
    View* hot_view = GetInitialFocusableView(item, direction == INCREMENT_SELECTION_DOWN);
    SetHotTrackedButton(Button::AsButton(hot_view));
}

void MenuController::SetNextHotTrackedView(MenuItemView* item, SelectionIncrementDirectionType direction)
{
    ...
    const size_t delta = direction == INCREMENT_SELECTION_UP ? (num_menu_items - 1) : 1;
    while (true) {
...
void MenuController::SetAnchorParametersForItem(MenuItemView* item, const gfx::Point& item_loc, ui::OwnedWindowAnchor* anchor)
{
    if (item->GetParentMenuItem()) {
        ...
        if (item->GetParentMenuItem()->GetParentMenuItem()) {
            ...
            View::ConvertPointToScreen(item->GetParentMenuItem(), &parent_of_parent_item_loc);
        ...
        anchor->constraint_adjustment = ui::OwnedWindowConstraintAdjustment::kAdjustmentSlideY | ui::OwnedWindowConstraintAdjustment::kAdjustmentFlipX
            | ui::OwnedWindowConstraintAdjustment::kAdjustmentResizeX | ui::OwnedWindowConstraintAdjustment::kAdjustmentRezizeY;
    } else {
        if (state_.context_menu) {
            anchor->anchor_position = ui::OwnedWindowAnchorPosition::kBottomLeft;
            anchor->anchor_gravity = ui::OwnedWindowAnchorGravity::kBottomRight;
            anchor->constraint_adjustment = ui::OwnedWindowConstraintAdjustment::kAdjustmentSlideX | ui::OwnedWindowConstraintAdjustment::kAdjustmentFlipY
...
base::CallbackListSubscription MenuController::AddAnnotationCallback(AnnotationCallback callback)
{
    return annotation_callbacks_.Add(base::BindRepeating(
        [](AnnotationCallback callback, bool& result, const ui::LocatedEvent& event) {
            if (result) {
...
bool MenuController::MaybeForwardToAnnotation(SubmenuView* source, const ui::LocatedEvent& event)
{
    const std::unique_ptr<ui::Event> cloned = event.Clone();
    auto* located = static_cast<ui::LocatedEvent*>(cloned.get());
    const gfx::Point screen_loc = View::ConvertPointToScreen(source->GetScrollViewContainer(), event.location());
...
MenuController::MenuOpenDirection MenuController::GetChildMenuOpenDirectionAtDepth(size_t depth) const
{
    const size_t index = depth - 1;
    return index >= child_menu_open_direction_.size() ? MenuOpenDirection::kLeading : child_menu_open_direction_.at(index);
}

void MenuController::SetChildMenuOpenDirectionAtDepth(size_t depth, MenuOpenDirection direction)
{
    const size_t index = depth - 1;
...
void MenuController::SetMenuRoundedCorners(std::optional<gfx::RoundedCornersF> corners)
{
