...
MenuModelAdapter::MenuModelAdapter(ui::MenuModel* menu_model, base::RepeatingClosure on_menu_closed_callback)
...
std::optional<SkColor> MenuModelAdapter::GetLabelColor(int command_id) const
{
    ...
    return command_id == ui::MenuModel::kTitleId ? std::make_optional(menu_->GetSubmenu()->GetColorProvider()->GetColor(
               views::TypographyProvider::Get().GetColorId(views::style::CONTEXT_MENU, views::style::STYLE_PRIMARY)))
                                                 : std::nullopt;
}
...
// Static.
MenuItemView* MenuModelAdapter::AddMenuItemFromModelAt(ui::MenuModel* model, size_t model_index, MenuItemView* menu, size_t menu_index, int item_id)
{
    ...
    if (*type == MenuItemView::Type::kSeparator) {
        return menu->AddMenuItemAt(menu_index, item_id, std::u16string(), std::u16string(), std::u16string(), ui::ImageModel(), ui::ImageModel(), *type,
    ...
    auto* const menu_item_view = menu->AddMenuItemAt(menu_index, item_id, model->GetLabelAt(model_index), model->GetSecondaryLabelAt(model_index),
        model->GetMinorTextAt(model_index), minor_icon, icon, *type, ui::NORMAL_SEPARATOR, model->GetSubmenuBackgroundColorId(model_index),
        model->GetForegroundColorId(model_index), model->GetSelectedBackgroundColorId(model_index));

    if (model->IsAlertedAt(model_index)) {
        menu_item_view->SetAlerted();
    }
    menu_item_view->set_is_new(model->IsNewFeatureAt(model_index));
    menu_item_view->set_may_have_mnemonics(model->MayHaveMnemonicsAt(model_index));
    const ui::ElementIdentifier element_id = model->GetElementIdentifierAt(model_index);
    if (element_id) {
...
// Static.
MenuItemView* MenuModelAdapter::AppendMenuItemFromModel(ui::MenuModel* model, size_t model_index, MenuItemView* menu, int item_id)
{
    const size_t menu_index = menu->HasSubmenu() ? menu->GetSubmenu()->children().size() : size_t { 0 };
    return AddMenuItemFromModelAt(model, model_index, menu, menu_index, item_id);
}

MenuItemView* MenuModelAdapter::AppendMenuItem(MenuItemView* menu, ui::MenuModel* model, size_t index)
{
    return AppendMenuItemFromModel(model, index, menu, model->GetCommandIdAt(index));
}
...
bool MenuModelAdapter::IsTriggerableEvent(MenuItemView* source, const ui::Event& e)
{
    return e.type() == ui::EventType::kGestureTap || e.type() == ui::EventType::kGestureTapDown || (e.IsMouseEvent() && (triggerable_event_flags_ & e.flags()));
}
...
const gfx::FontList* MenuModelAdapter::GetLabelFontList(int id) const
{
    ...
    if (ui::MenuModel::GetModelAndIndexForCommandId(id, &model, &index)) {
        if (const gfx::FontList* const font_list = model->GetLabelFontListAt(index)) {
            return font_list;
        }
    }
...
void MenuModelAdapter::WillShowMenu(MenuItemView* menu)
{
    ...
    const std::map<MenuItemView*, raw_ptr<ui::MenuModel, CtnExperimental>>::const_iterator map_iterator = menu_map_.find(menu);
...
void MenuModelAdapter::BuildMenuImpl(MenuItemView* menu, ui::MenuModel* model)
{
    ...
    for (size_t i = 0; i < item_count; ++i) {
        ...
        const bool is_submenu = type == ui::MenuModel::TYPE_SUBMENU || type == ui::MenuModel::TYPE_ACTIONABLE_SUBMENU;
        if (!item) {
            CHECK(!is_submenu);
            continue;
        }
        ...
        item->SetEnabled(type != ui::MenuModel::TYPE_TITLE && model->IsEnabledAt(i));
        item->SetVisible(model->IsVisibleAt(i));

        if (is_submenu) {
            const auto item_type = item->GetType();
            CHECK(item_type == MenuItemView::Type::kSubMenu || item_type == MenuItemView::Type::kActionableSubMenu);
