...
public:
    ...
    {
        auto menu_item_view = std::make_unique<TestMenuItemView>(menu_delegate_.get());
    ...
    {
        menu_runner_ = std::make_unique<MenuRunner>(CreateMenuItemView(), run_types);
    }

    views::TestMenuItemView* menu_item_view()
    {
    ...
    void SetUp() override
    {
        ...
        // flaky).
        MenuCocoaWatcherMac::SetNotificationFilterForTesting(MacNotificationFilter::IgnoreWorkspaceNotifications);
#endif
...
#if BUILDFLAG(IS_MAC)
        MenuCocoaWatcherMac::SetNotificationFilterForTesting(MacNotificationFilter::DontIgnoreNotifications);
#endif
...
TEST_F(MenuRunnerTest, AsynchronousRun)
{
    ...
    runner->RunMenuAt(owner(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, ui::mojom::MenuSourceType::kNone, nullptr);
...
TEST_F(MenuRunnerTest, NonLatinMnemonic)
{
    ...
    ui::test::EventGenerator generator(GetContext(), owner()->GetNativeWindow());
    ui::KeyEvent key_press = ui::KeyEvent::FromCharacter(0x062f, ui::VKEY_N, ui::DomCode::NONE, 0);
...
TEST_F(MenuRunnerTest, MenuItemViewShowsMnemonics)
{
    ...
    menu_runner()->RunMenuAt(owner(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, ui::mojom::MenuSourceType::kNone, nullptr);
...
TEST_F(MenuRunnerTest, PrefixSelect)
{
    ...
    menu_item_view()->GetSubmenu()->GetPrefixSelector()->set_tick_clock_for_testing(&clock);
...
TEST_F(MenuRunnerTest, NestingDuringDrag)
{
    ...
    MenuRunner nested_runner(MenuRunner(std::make_unique<MenuItemView>(nested_delegate.get()), MenuRunner::IS_NESTED));
    nested_runner.RunMenuAt(owner(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, ui::mojom::MenuSourceType::kNone, nullptr);
...
private:
    ...
    void OnMouseEvent(ui::MouseEvent* event) override
    {
        if (event->type() == ui::EventType::kMousePressed) {
            runner_->RunMenuAt(owner_, nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, ui::mojom::MenuSourceType::kNone, nullptr);
            event->SetHandled();
        }
    }
...
public:
    ...
    {
        return static_cast<EventCountView*>(widget()->GetRootView()->GetViewByID(kEventCountViewID));
    }

    std::unique_ptr<ui::test::EventGenerator> EventGeneratorForWidget(Widget* widget)
    {
        return std::make_unique<ui::test::EventGenerator>(GetContext(), widget->GetNativeWindow());
    }

    void AddMenuLauncherEventHandler(Widget* widget)
    {
        consumer_ = std::make_unique<MenuLauncherEventHandler>(menu_runner(), widget);
...
TEST_F(MenuRunnerWidgetTest, ClearsMouseHandlerOnRun)
{
    ...
    generator->MoveMouseTo(second_event_count_view->GetBoundsInScreen().CenterPoint());
    generator->PressLeftButton();
    EXPECT_EQ(1, second_event_count_view->GetEventCount(ui::EventType::kMousePressed));
}
...
{
    internal::MenuRunnerImpl* menu_runner = new internal::MenuRunnerImpl(CreateMenuItemView());
    menu_runner->RunMenuAt(owner(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, 0, nullptr);
    ...
    internal::MenuRunnerImpl* menu_runner2 = new internal::MenuRunnerImpl(base::WrapUnique<MenuItemView>(menu_item_view2));
    menu_runner2->RunMenuAt(owner(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, MenuRunner::IS_NESTED, nullptr);
    ...
    menu_runner->OnMenuClosed(internal::MenuControllerDelegate::NOTIFY_DELEGATE, nullptr, 0);
...
TEST_F(MenuRunnerImplTest, MenuRunnerDestroyedWithNoActiveController)
{
    ...
    internal::MenuRunnerImpl* menu_runner2 = new internal::MenuRunnerImpl(base::WrapUnique<MenuItemView>(menu_item_view2));
    menu_runner2->RunMenuAt(owner(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, MenuRunner::FOR_DROP, nullptr);
...
public:
    ...
    MenuRunnerDestructionTest& operator=(const MenuRunnerDestructionTest&) = delete;

    ~MenuRunnerDestructionTest() override = default;

    base::WeakPtr<internal::MenuRunnerImpl> MenuRunnerAsWeakPtr(internal::MenuRunnerImpl* menu_runner);

    // ViewsTestBase:
    void SetUp() override;
};

base::WeakPtr<internal::MenuRunnerImpl> MenuRunnerDestructionTest::MenuRunnerAsWeakPtr(internal::MenuRunnerImpl* menu_runner)
{
...
TEST_F(MenuRunnerDestructionTest, MenuRunnerDestroyedDuringReleaseRef)
{
    ...
    static_cast<ReleaseRefTestViewsDelegate*>(test_views_delegate())->set_release_ref_callback(base::BindLambdaForTesting([&]() {
        run_loop.Quit();
        ResetMenuItemView();
        menu_runner->Release();
    }));
...
TEST_F(MenuRunnerImplTest, FocusOnMenuClose)
{
    ...
    LabelButton* button = owner()->GetRootView()->AddChildView(std::move(button_managed));
    ...
    ViewAccessibility::AccessibilityEventsCallback accessibility_events_callback = base::BindRepeating(
        [](bool* focus_after_menu_close_sent, const ui::AXPlatformNodeDelegate* delegate, const ax::mojom::Event event_type) {
            if (event_type == ax::mojom::Event::kFocusAfterMenuClose)
                *focus_after_menu_close_sent = true;
        },
        &focus_after_menu_close_sent);
    button->GetViewAccessibility().set_accessibility_events_callback(std::move(accessibility_events_callback));
    menu_runner->OnMenuClosed(internal::MenuControllerDelegate::NOTIFY_DELEGATE, nullptr, 0);
    ...
    button->GetViewAccessibility().set_accessibility_events_callback(base::DoNothing());
...
{
    // Create test button that has focus.
    LabelButton* button = new LabelButton(Button::PressedCallback(), std::u16string(), style::CONTEXT_BUTTON);
    button->SetID(1);
...
TEST_F(MenuRunnerTest, ShowMenuHostDurationMetricsDoesLog)
{
    ...
    std::string histogram_name = "Chrome.AppMenu.MenuHostInitToNextFramePresented";

    InitMenuRunner(0);
    MenuRunner* runner = menu_runner();
    runner->RunMenuAt(owner(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, ui::mojom::MenuSourceType::kNone, nullptr, std::nullopt, histogram_name);
...
TEST_F(MenuRunnerTest, ShowMenuHostDurationMetricsDoesNotLog)
{
    ...
    InitMenuRunner(0);
    MenuRunner* runner = menu_runner();
    runner->RunMenuAt(owner(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, ui::mojom::MenuSourceType::kNone, nullptr, std::nullopt);
