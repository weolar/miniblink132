...
std::u16string MenuDelegate::GetTooltipText(int id, const gfx::Point& screen_loc) const
{
    return std::u16string();
}
...
bool MenuDelegate::ShowContextMenu(MenuItemView* source, int id, const gfx::Point& p, ui::mojom::MenuSourceType source_type)
{
    return false;
}
...
bool MenuDelegate::ShouldExecuteCommandWithoutClosingMenu(int id, const ui::Event& e)
{
    return false;
}

bool MenuDelegate::IsTriggerableEvent(MenuItemView* source, const ui::Event& e)
{
    return e.type() == ui::EventType::kGestureTap || e.type() == ui::EventType::kGestureTapDown
        || (e.IsMouseEvent() && (e.flags() & (ui::EF_LEFT_MOUSE_BUTTON | ui::EF_RIGHT_MOUSE_BUTTON)));
}
...
bool MenuDelegate::GetDropFormats(MenuItemView* menu, int* formats, std::set<ui::ClipboardFormatType>* format_types)
{
    return false;
}
...
ui::mojom::DragOperation MenuDelegate::GetDropOperation(MenuItemView* item, const ui::DropTargetEvent& event, DropPosition* position)
{
    NOTREACHED() << "If you override CanDrop, you must override this too";
}

views::View::DropCallback MenuDelegate::GetDropCallback(MenuItemView* menu, DropPosition position, const ui::DropTargetEvent& event)
{
    NOTREACHED() << "If you override CanDrop, you must override this too";
}
...
MenuItemView* MenuDelegate::GetSiblingMenu(
    MenuItemView* menu, const gfx::Point& screen_point, MenuAnchorPosition* anchor, bool* has_mnemonics, MenuButton** button)
{
    return nullptr;
}
