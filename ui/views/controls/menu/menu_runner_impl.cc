...
void FireFocusAfterMenuClose(base::WeakPtr<Widget> widget)
{
    if (widget) {
        ...
        if (focus_manager && focus_manager->GetFocusedView()) {
            focus_manager->GetFocusedView()->GetViewAccessibility().FireFocusAfterMenuClose();
        }
    }
}
...
bool IsAltPressed()
{
    if (const auto* const platorm_menu_utils = ui::OzonePlatform::GetInstance()->GetPlatformMenuUtils()) {
        return (platorm_menu_utils->GetCurrentKeyModifiers() & ui::EF_ALT_DOWN) != 0;
    }
    return false;
}
...
#if !BUILDFLAG(IS_MAC)
MenuRunnerImplInterface* MenuRunnerImplInterface::Create(ui::MenuModel* menu_model, int32_t run_types, base::RepeatingClosure on_menu_closed_callback)
{
    return new MenuRunnerImplAdapter(menu_model, std::move(on_menu_closed_callback));
}
#endif
...
void MenuRunnerImpl::RunMenuAt(Widget* parent, MenuButtonController* button_controller, const gfx::Rect& bounds, MenuAnchorPosition anchor, int32_t run_types,
    gfx::NativeView native_view_for_gestures, std::optional<gfx::RoundedCornersF> corners, std::optional<std::string> show_menu_host_duration_histogram)
{
    ...
    DCHECK((run_types & MenuRunner::COMBOBOX) == 0 || (run_types & MenuRunner::EDITABLE_COMBOBOX) == 0);
    ...
    controller->set_send_gesture_events_to_owner((run_types & MenuRunner::SEND_GESTURE_EVENTS_TO_OWNER) != 0);
    controller->set_use_ash_system_ui_layout((run_types & MenuRunner::USE_ASH_SYS_UI_LAYOUT) != 0);
    controller_ = controller->AsWeakPtr();
    menu_->set_controller(controller_.get());
    menu_->PrepareForRun(has_mnemonics, !for_drop_ && ShouldShowMnemonics(run_types));
    if (show_menu_host_duration_histogram.has_value() && !show_menu_host_duration_histogram.value().empty()) {
        controller->SetShowMenuHostDurationHistogram(std::move(show_menu_host_duration_histogram));
    }

    controller->Run(parent, button_controller, menu_.get(), bounds, anchor, (run_types & MenuRunner::CONTEXT_MENU) != 0,
        (run_types & MenuRunner::NESTED_DRAG) != 0, native_view_for_gestures);
}
...
void MenuRunnerImpl::OnMenuClosed(NotifyType type, MenuItemView* menu, int mouse_event_flags)
{
    ...
    if (menu_->GetDelegate()) {
        ...
        if (menu && !for_drop_) {
            ...
            menu_->GetDelegate()->ExecuteCommand(menu->GetCommand(), mouse_event_flags);
        }
