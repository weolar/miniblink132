...
VerticalSeparator::VerticalSeparator()
{
    ...
    SetPreferredSize(gfx::Size(config.actionable_submenu_vertical_separator_width, config.actionable_submenu_vertical_separator_height));
...
MenuItemView::MenuItemView(MenuDelegate* delegate)
    : MenuItemView(/* parent */ nullptr,
        /* command */ 0, Type::kSubMenu, delegate)
{
}
...
void MenuItemView::UpdateAccessibleCheckedState()
{
    if (type_ == Type::kCheckbox || type_ == Type::kRadio) {
        bool is_checked = GetDelegate() && GetDelegate()->IsItemChecked(GetCommand());
        const ax::mojom::CheckedState checked_state = is_checked ? ax::mojom::CheckedState::kTrue : ax::mojom::CheckedState::kFalse;
        ...
        if (auto* scrollview_accessibility = GetScrollViewContainerViewAccessibility()) {
...
void MenuItemView::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details)
{
    // Whether the selection is painted may change based on the number of
    // children.
    if (details.parent == this && update_selection_based_state_in_view_herarchy_changed_) {
...
std::u16string MenuItemView::GetTooltipText(const gfx::Point& p) const
{
    ...
    if (!controller || controller->exit_type() != MenuController::ExitType::kNone) {
    ...
    return delegate ? delegate->GetTooltipText(command_, ConvertPointToScreen(this, p)) : std::u16string();
}
...
bool MenuItemView::HandleAccessibleAction(const ui::AXActionData& action_data)
{
    ...
    case ax::mojom::Action::kDoDefault: {
        ...
        ui::KeyEvent event(ui::EventType::kKeyPressed, ui::VKEY_RETURN, ui::DomCode::ENTER, ui::EF_NONE, ui::DomKey::ENTER, ui::EventTimeForNow());
...
// static
std::u16string MenuItemView::GetAccessibleNameForMenuItem(const std::u16string& item_text, const std::u16string& minor_text, bool is_new_feature)
{
    ...
    size_t index = 0;
    const char16_t amp = '&';
    while ((index = accessible_name.find(amp, index)) != std::u16string::npos && index + 1 < accessible_name.length()) {
        accessible_name.replace(index, accessible_name.length() - index, accessible_name.substr(index + 1));
...
MenuItemView* MenuItemView::AddMenuItemAt(size_t index, int item_id, const std::u16string& label, const std::u16string& secondary_label,
    const std::u16string& minor_text, const ui::ImageModel& minor_icon, const ui::ImageModel& icon, Type type, ui::MenuSeparatorType separator_style,
    std::optional<ui::ColorId> submenu_background_color, std::optional<ui::ColorId> foreground_color, std::optional<ui::ColorId> selected_color_id)
{
    ...
    if (!submenu_) {
        ...
        if (submenu_background_color.has_value()) {
            submenu_->SetBackground(views::CreateThemedSolidBackground(submenu_background_color.value()));
        }
    }
    DCHECK_LE(index, submenu_->children().size());
    if (type == Type::kSeparator) {
        submenu_->AddChildViewAt(std::make_unique<MenuSeparator>(separator_style), index);
        return nullptr;
    }
    MenuItemView* item = new MenuItemView(this, item_id, type);
    item->SetTitle(label.empty() && GetDelegate() ? GetDelegate()->GetLabel(item_id) : label);
...
void MenuItemView::RemoveAllMenuItems()
{
    ...
    removed_items_.insert(removed_items_.end(), submenu_->children().begin(), submenu_->children().end());

    submenu_->RemoveAllChildViewsWithoutDeleting();
}

MenuItemView* MenuItemView::AppendMenuItem(int item_id, const std::u16string& label, const ui::ImageModel& icon)
{
...
{
    return AppendMenuItemImpl(ui::MenuModel::kTitleId, label, ui::ImageModel(), Type::kTitle);
}

MenuItemView* MenuItemView::AppendSubMenu(int item_id, const std::u16string& label, const ui::ImageModel& icon)
{
...
MenuItemView* MenuItemView::AppendMenuItemImpl(int item_id, const std::u16string& label, const ui::ImageModel& icon, Type type)
{
    const size_t index = submenu_ ? submenu_->children().size() : size_t { 0 };
    return AddMenuItemAt(index, item_id, label, std::u16string(), std::u16string(), ui::ImageModel(), icon, type, ui::NORMAL_SEPARATOR);
}
...
SubmenuView* MenuItemView::CreateSubmenu()
{
    ...
    std::unique_ptr<AXVirtualView> virtual_child = std::make_unique<AXVirtualView>();
...
{
    CHECK(type_ == Type::kSubMenu || type_ == Type::kActionableSubMenu) << "SetSubmenuId called on menu item with type " << static_cast<int>(type_);
    submenu_id_ = id;
    if (submenu_) {
...
base::CallbackListSubscription MenuItemView::AddSelectedChangedCallback(PropertyChangedCallback callback)
{
    return AddPropertyChangedCallback(&selected_, std::move(callback));
}

void MenuItemView::SetSelectionOfActionableSubmenu(bool submenu_area_of_actionable_submenu_selected)
{
    DCHECK_EQ(Type::kActionableSubMenu, type_);
    if (submenu_area_of_actionable_submenu_selected_ == submenu_area_of_actionable_submenu_selected) {
        return;
    }

    submenu_area_of_actionable_submenu_selected_ = submenu_area_of_actionable_submenu_selected;
    SchedulePaint();
}
...
{
    {
        ...
        base::AutoReset setter(&update_selection_based_state_in_view_herarchy_changed_, false);
        if (icon_view_) {
...
gfx::Size MenuItemView::CalculatePreferredSize(const SizeBounds& available_size) const
{
    // If this is a container, we can just use the preferred size.
    if (IsContainer()) {
        const gfx::Insets margins = GetContainerMargins();
        gfx::Size content_size = children().front()->GetPreferredSize(available_size.Inset(margins));
...
gfx::Rect MenuItemView::GetSubmenuAreaOfActionableSubmenu() const
{
    DCHECK_EQ(Type::kActionableSubMenu, type_);
    const MenuConfig& config = MenuConfig::instance();
    return gfx::Rect(gfx::Point(vertical_separator_->bounds().right(), 0), gfx::Size(config.actionable_submenu_width, height()));
}
...
int MenuItemView::GetContentStart() const
{
    ...
    return GetItemHorizontalBorder()
        + ((controller && controller->use_ash_system_ui_layout()) ? config.touchable_item_horizontal_padding : config.item_horizontal_padding);
}
...
{
    if (!GetRootMenuItem()->has_mnemonics_ || !MenuConfig::instance().use_mnemonics || !may_have_mnemonics()) {
        return 0;
    }
...
ProposedLayout MenuItemView::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
    ProposedLayout layout;
    DCHECK(size_bounds.is_fully_bounded());
    layout.host_size = gfx::Size(size_bounds.width().value(), size_bounds.height().value());
    ...
    if (IsContainer()) {
        ...
        layout.child_layouts.emplace_back(children().front(), children().front()->GetVisible(), bounds);
    } else {
        ...
        int child_end = layout.host_size.width() - (children_use_full_width_ ? 0 : submenu->trailing_padding());
        for (View* child : base::Reversed(children())) {
            ...
            layout.child_layouts.emplace_back(child, child->GetVisible(), gfx::Rect(child_end - width, 0, width, layout.host_size.height()));
        ...
        if (radio_check_image_view_) {
            ...
            layout.child_layouts.emplace_back(
                radio_check_image_view_.get(), radio_check_image_view_->GetVisible(), gfx::Rect(icon_x, y, kMenuCheckSize, kMenuCheckSize));
        }
        if (icon_view_) {
            const gfx::Size preferred_size = icon_view_->GetPreferredSize({});
            int x = (config.icons_in_label ? submenu->label_start() : icon_x) + ((submenu->icon_area_width() - preferred_size.width()) / 2);
            ...
            layout.child_layouts.emplace_back(icon_view_.get(), icon_view_->GetVisible(), gfx::Rect(x, y, preferred_size.width(), preferred_size.height()));
        }

        if (submenu_arrow_image_view_) {
            const int x = layout.host_size.width() - GetItemHorizontalBorder()
                - (type_ == Type::kActionableSubMenu ? config.actionable_submenu_arrow_to_edge_padding : config.arrow_to_edge_padding) - config.arrow_size;
            const int y = (layout.host_size.height() - config.arrow_size) / 2;
            layout.child_layouts.emplace_back(
                submenu_arrow_image_view_.get(), submenu_arrow_image_view_->GetVisible(), gfx::Rect(x, y, config.arrow_size, config.arrow_size));
        }

        if (vertical_separator_) {
            const gfx::Size preferred_size = vertical_separator_->GetPreferredSize({});
            const int x = layout.host_size.width() - config.actionable_submenu_width - config.actionable_submenu_vertical_separator_width;
            const int y = (layout.host_size.height() - preferred_size.height()) / 2;
            layout.child_layouts.emplace_back(
                vertical_separator_.get(), vertical_separator_->GetVisible(), gfx::Rect(x, y, preferred_size.width(), preferred_size.height()));
        }
    }
...
{
    bool ignore_enabled = ui::AXPlatform::GetInstance().GetMode().has_mode(ui::AXMode::kNativeAPIs);
...
int MenuItemView::GetItemHorizontalBorder() const
{
    const auto* const controller = GetMenuController();
    const MenuConfig& config = MenuConfig::instance();
    return (controller && controller->use_ash_system_ui_layout()) ? config.ash_item_horizontal_border_padding : config.item_horizontal_border_padding;
}
...
MenuItemView::MenuItemView(MenuItemView* parent, int command, MenuItemView::Type type)
    : MenuItemView(parent, command, type, /* delegate */ nullptr)
{
}

MenuItemView::MenuItemView(MenuItemView* parent, int command, Type type, MenuDelegate* delegate)
    : delegate_(delegate)
    ...
    , command_(command)
{
    ...
    if (type_ == Type::kCheckbox || type_ == Type::kRadio) {
        ...
        bool show_check_radio_icon = type_ == Type::kRadio || (type_ == Type::kCheckbox && GetDelegate() && GetDelegate()->IsItemChecked(GetCommand()));
    ...
    visible_changed_callback_ = AddVisibleChangedCallback(base::BindRepeating(&MenuItemView::UpdateAccessibleSelection, base::Unretained(this)));
    enabled_changed_callback_ = AddEnabledChangedCallback(base::BindRepeating(&MenuItemView::UpdateAccessibleSelection, base::Unretained(this)));
...
{
    int flags = base::i18n::IsRTL() ? gfx::Canvas::TEXT_ALIGN_RIGHT : gfx::Canvas::TEXT_ALIGN_LEFT;

    if (GetRootMenuItem()->has_mnemonics_ && may_have_mnemonics()) {
        flags |= (MenuConfig::instance().show_mnemonics || GetRootMenuItem()->show_mnemonics_) ? gfx::Canvas::SHOW_PREFIX : gfx::Canvas::HIDE_PREFIX;
    }
    return flags;
}
...
const gfx::FontList MenuItemView::GetFontList() const
{
    if (const MenuDelegate* delegate = GetDelegate()) {
        if (const gfx::FontList* font_list = delegate->GetLabelFontList(GetCommand())) {
            return *font_list;
        }
    }
    auto* menu_controller = GetMenuController();
    if (menu_controller && menu_controller->use_ash_system_ui_layout()) {
        return TypographyProvider::Get().GetFont(style::CONTEXT_TOUCH_MENU, style::STYLE_PRIMARY);
    }
    return menu_controller && menu_controller->IsContextMenu() ? MenuConfig::instance().context_menu_font_list : MenuConfig::instance().font_list;
}
...
void MenuItemView::UpdateEmptyMenusAndMetrics()
{
    ...
    for (View* child : children) {
        if (IsViewClass<EmptyMenuMenuItem>(child)) {
            ...
            has_visible_menu_items |= IsViewClass<MenuItemView>(child) && child->GetVisible();
        }
    }
...
void MenuItemView::OnPaintImpl(gfx::Canvas* canvas, PaintMode mode)
{
    ...
    const int total_text_height = secondary_title().empty() ? text_height : text_height * 2;
    const int top_margin = vertical_margin + (available_height - total_text_height) / 2;
    ...
    int width = this->width() - submenu->trailing_padding() - accel_width - label_start;
    gfx::Rect text_bounds(label_start, top_margin, width, text_height);
    text_bounds.set_x(GetMirroredXForRect(text_bounds));
    int flags = GetDrawStringFlags();
    if (mode == PaintMode::kForDrag)
        flags |= gfx::Canvas::NO_SUBPIXEL_RENDERING;
    canvas->DrawStringRectWithFlags(title(), font_list, colors.fg_color, text_bounds, flags);
    ...
    if (!secondary_title().empty()) {
        ...
        canvas->DrawStringRectWithFlags(secondary_title(), font_list, colors.minor_fg_color, text_bounds, flags);
    }

    if (ShouldShowNewBadge()) {
        BadgePainter::PaintBadge(
            canvas, this, label_start + gfx::GetStringWidth(title(), font_list) + BadgePainter::kBadgeHorizontalMargin, top_margin, new_badge_text_, font_list);
    }

    PaintMinorIconAndText(canvas, colors.minor_fg_color);
}

void MenuItemView::PaintBackground(gfx::Canvas* canvas, PaintMode mode, bool paint_as_selected)
{
    if (menu_item_background_.has_value()) {
        ...
        flags.setColor(GetColorProvider()->GetColor(background_info.background_color_id));
        canvas->DrawRoundRect(bounds, background_info.corner_radius, flags);
    }
    const auto& config = MenuConfig::instance();
    if (type_ == Type::kHighlighted || is_alerted_ || (paint_as_selected && selected_color_id_.has_value())) {
        SkColor color = gfx::kPlaceholderColor;

        ui::ColorProvider* color_provider = GetColorProvider();
        if (type_ == Type::kHighlighted || selected_color_id_.has_value()) {
            const ui::ColorId color_id
                = selected_color_id_.value_or(paint_as_selected ? ui::kColorMenuItemBackgroundSelected : ui::kColorMenuItemBackgroundHighlighted);
            color = color_provider->GetColor(color_id);
        } else {
            const auto* animation = GetMenuController()->GetAlertAnimation();
            color = gfx::Tween::ColorValueBetween(animation->GetCurrentValue(), color_provider->GetColor(ui::kColorMenuItemBackgroundAlertedInitial),
        ...
        if (type_ == Type::kActionableSubMenu) {
            if (submenu_area_of_actionable_submenu_selected_) {
                ...
                item_bounds.set_width(item_bounds.width() - config.actionable_submenu_width - 1);
            }
        }
        ...
        GetNativeTheme()->Paint(canvas->sk_canvas(), GetColorProvider(), ui::NativeTheme::kMenuItemBackground, ui::NativeTheme::kHovered, item_bounds,
            ui::NativeTheme::ExtraParams(menu_item_extra_params));
    }
}
...
void MenuItemView::PaintMinorIconAndText(gfx::Canvas* canvas, SkColor color)
{
    ...
    const MenuConfig& config = MenuConfig::instance();
    const int vertical_margin = GetVerticalMargin();
    const int submenu_arrow_width = submenu_arrow_image_view_ ? submenu_arrow_image_view_->width() + config.item_horizontal_padding : 0;

    gfx::Rect minor_text_bounds(width() - submenu->trailing_padding() - max_minor_text_width, vertical_margin, max_minor_text_width - submenu_arrow_width,
        height() - vertical_margin * 2);
    minor_text_bounds.set_x(GetMirroredXForRect(minor_text_bounds));

    std::unique_ptr<gfx::RenderText> render_text = gfx::RenderText::CreateRenderText();
    if (!minor_text.empty()) {
        render_text->SetText(minor_text);
        render_text->SetFontList(GetFontList());
        render_text->SetColor(color);
        render_text->SetDisplayRect(minor_text_bounds);
        render_text->SetHorizontalAlignment(base::i18n::IsRTL() ? gfx::ALIGN_LEFT : gfx::ALIGN_RIGHT);
    ...
    if (!minor_icon_.IsEmpty()) {
        ...
        const int image_x = GetMirroredRect(minor_text_bounds).right() - render_text->GetContentWidth()
            - (minor_text.empty() ? 0 : config.item_horizontal_padding) - image.width();
        const int image_y = minor_text_bounds.y() + (minor_text_bounds.height() - image.height()) / 2;
        canvas->DrawImageInt(image, GetMirroredXWithWidthInView(image_x, image.width()), image_y);
    }
}
...
SkColor MenuItemView::GetTextColor(bool minor, bool paint_as_selected) const
{
    ...
    // default color.
    if (paint_as_selected && selected_color_id_.has_value() && foreground_color_id_.has_value()) {
        return GetColorProvider()->GetColor(foreground_color_id_.value());
    }

    style::TextContext context = GetMenuController() && GetMenuController()->use_ash_system_ui_layout() ? style::CONTEXT_TOUCH_MENU : style::CONTEXT_MENU;
    ...
    return GetColorProvider()->GetColor(TypographyProvider::Get().GetColorId(context, text_style));
}

MenuItemView::Colors MenuItemView::CalculateColors(bool paint_as_selected) const
{
    const std::optional<SkColor> label_color_from_delegate = GetMenuLabelColor();
    Colors colors;
    colors.fg_color = label_color_from_delegate ? *label_color_from_delegate : GetTextColor(/*minor=*/false, paint_as_selected);
...
std::u16string MenuItemView::CalculateAccessibleName() const
{
    ...
    if (IsContainer()) {
        ...
        item_text = child_node_data.GetString16Attribute(ax::mojom::StringAttribute::kName);
    } else {
        item_text = title_;
    }
    return GetAccessibleNameForMenuItem(item_text, GetMinorText(), ShouldShowNewBadge());
}
...
gfx::Size MenuItemView::GetChildPreferredSize() const
{
    ...
    const auto add_width = [this](int width, const View* child) {
        if (child == icon_view_ || child == radio_check_image_view_ || child == submenu_arrow_image_view_ || child == vertical_separator_)
            return width;
        if (width)
            width += kChildHorizontalPadding;
        return width + child->GetPreferredSize({}).width();
    };
    const int width = std::accumulate(children().cbegin(), children().cend(), 0, add_width);
...
MenuItemView::MenuItemDimensions MenuItemView::CalculateDimensions() const
{
    ...
    const bool use_ash_system_ui_layout = GetMenuController() && GetMenuController()->use_ash_system_ui_layout();

    MenuItemDimensions dimensions;
    dimensions.children_width = child_size.width();
    const MenuConfig& config = MenuConfig::instance();
    dimensions.height = use_ash_system_ui_layout ? config.touchable_menu_height : child_size.height();

    // In the container case, only the child size plus margins need to be
    // considered.
    if (IsContainer()) {
        const gfx::Insets margins = GetContainerMargins();
        dimensions.height = ApplyMinIconHeight(dimensions.height) + margins.height();
    ...
    dimensions.standard_width = GetLabelStartForThisItem() + title_width + GetContainingSubmenu()->trailing_padding();
    ...
    if (title_width && dimensions.children_width) {
        dimensions.standard_width += LayoutProvider::Get()->GetDistanceMetric(views::DISTANCE_RELATED_LABEL_HORIZONTAL);
    }
    if (ShouldShowNewBadge()) {
        dimensions.standard_width += BadgePainter::kBadgeHorizontalMargin + views::BadgePainter::GetBadgeSize(new_badge_text_, font_list).width();
    }
    ...
    if (use_ash_system_ui_layout) {
        ...
        dimensions.standard_width = std::clamp(dimensions.standard_width, config.touchable_menu_min_width, config.touchable_menu_max_width);

        if (icon_view_) {
            dimensions.height = std::max(dimensions.height, icon_view_->GetPreferredSize({}).height() + 2 * config.vertical_touchable_menu_item_padding);
        }
        return dimensions;
    }
    ...
    if (!config.reserve_dedicated_arrow_column && HasSubmenu()) {
        ...
        dimensions.minor_text_width
            += config.arrow_size + ((type_ == Type::kActionableSubMenu) ? config.actionable_submenu_arrow_to_edge_padding : config.arrow_to_edge_padding);
    }

    // Determine the height to use.
    int label_text_height = secondary_title().empty() ? font_list.GetHeight() : font_list.GetHeight() * 2;
    dimensions.height = std::max(dimensions.height, label_text_height + vertical_margins);
...
void MenuItemView::ApplyMinimumDimensions(MenuItemDimensions* dims) const
{
    ...
    if (const auto* const controller = GetMenuController(); !controller || controller->IsCombobox()) {
        return;
    }

    const MenuConfig& config = MenuConfig::instance();
    dims->height = std::max(dims->height, IsContainer() ? config.minimum_container_item_height : config.minimum_text_item_height);
}
...
int MenuItemView::ApplyMinIconHeight(int height) const
{
    ...
    return type_ == Type::kSeparator ? height : std::max(height, GetContainingSubmenu()->min_icon_height());
}
...
int MenuItemView::GetLabelStartForThisItem() const
{
    ...
    const SubmenuView* const submenu = GetContainingSubmenu();
    if (!icon_view_ || (!icons_in_label && type_ != Type::kCheckbox && type_ != Type::kRadio)) {
        return submenu->label_start();
    }

    // The icon will be drawn starting where the label normally starts, so indent
    // past it.
    const int icon_width = icons_in_label ? submenu->icon_area_width() : icon_view_->GetPreferredSize({}).width();
    return submenu->label_start() + icon_width + LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_LABEL_HORIZONTAL);
}

std::u16string MenuItemView::GetMinorText() const
{
    std::u16string accel_text;
    return MenuConfig::instance().ShouldShowAcceleratorText(this, &accel_text) ? accel_text : minor_text_;
}
...
gfx::Insets MenuItemView::GetContainerMargins() const
{
    ...
    // margins as minimums.
    const gfx::Insets* margins_prop = children().front()->GetProperty(views::kMarginsKey);
...
int MenuItemView::NonIconChildViewsCount() const
{
    ...
    // for details.
    return static_cast<int>(children().size()) - (icon_view_ ? 1 : 0) - (radio_check_image_view_ ? 1 : 0) - (submenu_arrow_image_view_ ? 1 : 0)
        - (vertical_separator_ ? 1 : 0);
}
...
void MenuItemView::UpdateSelectionBasedState(bool paint_as_selected)
{
    ...
    const Colors colors = CalculateColors(paint_as_selected);
    if (submenu_arrow_image_view_) {
        submenu_arrow_image_view_->SetImage(ui::ImageModel::FromVectorIcon(vector_icons::kSubmenuArrowChromeRefreshIcon, colors.icon_color));
    }
    MenuDelegate* delegate = GetDelegate();
    if (type_ == Type::kCheckbox && delegate && delegate->IsItemChecked(GetCommand())) {
        radio_check_image_view_->SetImage(ui::ImageModel::FromVectorIcon(kMenuCheckIcon, colors.icon_color));
    } else if (type_ == Type::kRadio) {
        const bool toggled = delegate && delegate->IsItemChecked(GetCommand());
        const gfx::VectorIcon& radio_icon = toggled ? kMenuRadioSelectedIcon : kMenuRadioEmptyIcon;
        const SkColor radio_icon_color
            = GetColorProvider()->GetColor(toggled ? ui::kColorRadioButtonForegroundChecked : ui::kColorRadioButtonForegroundUnchecked);
        radio_check_image_view_->SetImage(ui::ImageModel::FromVectorIcon(radio_icon, radio_icon_color, kMenuCheckSize));
    }
    ...
    if ((!GetEnabled() || foreground_color_id_.has_value()) && icon_view_) {
        ...
        if (!icon_model.IsEmpty() && icon_model.IsVectorIcon()) {
            ...
            const ui::ImageModel& image_model = ui::ImageModel::FromVectorIcon(*icon,
                GetEnabled() ? GetColorProvider()->GetColor(foreground_color_id_.value()) : GetColorProvider()->GetColor(ui::kColorMenuIconDisabled),
                model.icon_size());
...
{
    return forced_visual_selection_.value_or(mode == PaintMode::kNormal && IsSelected() && GetContainingSubmenu()->GetShowSelection(this)
        && (NonIconChildViewsCount() == 0 || highlight_when_selected_with_child_views_));
}

bool MenuItemView::IsScheduledForDeletion() const
{
    return parent_menu_item_ && (base::Contains(parent_menu_item_->removed_items_, this) || parent_menu_item_->IsScheduledForDeletion());
}
...
int MenuItemView::GetVerticalMargin() const
{
    ...
    const auto* const controller = GetMenuController();
    const MenuConfig& config = MenuConfig::instance();
    return (controller && controller->use_ash_system_ui_layout()) ? config.ash_item_vertical_margin : config.item_vertical_margin;
}
...
{
    return submenu_ && submenu_->GetScrollViewContainer() ? &submenu_->GetScrollViewContainer()->GetViewAccessibility() : nullptr;
}
...
void MenuItemView::UpdateAccessibleName()
{
    ...
    if (accessible_name.empty()) {
        GetViewAccessibility().SetName(std::string(), ax::mojom::NameFrom::kAttributeExplicitlyEmpty);
        if (auto* submenuview_accessibility = GetSubmenuViewAccessibility()) {
            submenuview_accessibility->SetName(std::string(), ax::mojom::NameFrom::kAttributeExplicitlyEmpty);
        }
        if (auto* scrollview_accessibility = GetScrollViewContainerViewAccessibility()) {
            scrollview_accessibility->SetName(std::string(), ax::mojom::NameFrom::kAttributeExplicitlyEmpty);
        }
    } else {
