...
MenuRunner::MenuRunner(ui::MenuModel* menu_model, int32_t run_types, base::RepeatingClosure on_menu_closed_callback)
    : run_types_(run_types)
    , impl_(internal::MenuRunnerImplInterface::Create(menu_model, run_types, std::move(on_menu_closed_callback)))
{
}
...
void MenuRunner::RunMenuAt(Widget* parent, MenuButtonController* button_controller, const gfx::Rect& bounds, MenuAnchorPosition anchor,
    ui::mojom::MenuSourceType source_type, gfx::NativeView native_view_for_gestures, std::optional<gfx::RoundedCornersF> corners,
    std::optional<std::string> show_menu_host_duration_histogram)
{
    ...
    if (ViewsDelegate::GetInstance() && ViewsDelegate::GetInstance()->IsShuttingDown()) {
        return;
    }
    ...
    if (runner_handler_.get()) {
        runner_handler_->RunMenuAt(parent, button_controller, bounds, anchor, source_type, run_types_);
        return;
    }
    ...
    impl_->RunMenuAt(parent, button_controller, bounds, anchor, run_types_, native_view_for_gestures, corners, std::move(show_menu_host_duration_histogram));
}
...
void MenuRunner::SetRunnerHandler(std::unique_ptr<MenuRunnerHandler> runner_handler)
{
