...
void NativeMenuWin::Rebuild(MenuInsertionDelegateWin* delegate)
{
    ...
    for (size_t model_index = 0; model_index < model_->GetItemCount(); ++model_index) {
...
void NativeMenuWin::UpdateStates()
{
    ...
    for (const auto& item : items_) {
        size_t menu_index = model_index + first_item_index_;
        SetMenuItemState(menu_index, model_->IsEnabledAt(model_index), model_->IsItemCheckedAt(model_index), false);
        if (model_->IsItemDynamicAt(model_index)) {
            // TODO(atwilson): Update the icon as well (http://crbug.com/66508).
            SetMenuItemLabel(menu_index, model_index, model_->GetLabelAt(model_index));
        }
...
void NativeMenuWin::AddMenuItemAt(size_t menu_index, size_t model_index)
{
    ...
    if (type == ui::MenuModel::TYPE_SUBMENU) {
        item_data->submenu = std::make_unique<NativeMenuWin>(model_->GetSubmenuModelAt(model_index), nullptr);
    ...
    items_.insert(items_.begin() + static_cast<ptrdiff_t>(model_index), std::move(item_data));
    UpdateMenuItemInfoForString(&mii, model_index, model_->GetLabelAt(model_index));
...
void NativeMenuWin::AddSeparatorItemAt(size_t menu_index, size_t model_index)
{
    ...
    items_.insert(items_.begin() + static_cast<ptrdiff_t>(model_index), std::make_unique<ItemData>());
    InsertMenuItem(menu_, static_cast<UINT>(menu_index), TRUE, &mii);
}

void NativeMenuWin::SetMenuItemState(size_t menu_index, bool enabled, bool checked, bool is_default)
{
...
void NativeMenuWin::SetMenuItemLabel(size_t menu_index, size_t model_index, const std::u16string& label)
{
    if (IsSeparatorItemAt(menu_index))
        return;
...
void NativeMenuWin::UpdateMenuItemInfoForString(MENUITEMINFO* mii, size_t model_index, const std::u16string& label)
{
