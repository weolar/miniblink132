...
class VIEWS_EXPORT MenuController final : public gfx::AnimationDelegate, public WidgetObserver {
public:
    ...
    using AnnotationCallback = base::RepeatingCallback<bool(const ui::LocatedEvent& event)>;
    ...
    void Run(Widget* parent, MenuButtonController* button_controller, MenuItemView* root, const gfx::Rect& anchor_bounds, MenuAnchorPosition position,
        bool context_menu, bool is_nested_drag, gfx::NativeView native_view_for_gestures = gfx::NativeView());
    ...
    bool in_nested_run() const
    {
        return !menu_stack_.empty();
    }
    ...
    views::MenuItemView* GetSelectedMenuItem()
    {
        return pending_state_.item;
    }
    ...
    MenuAnchorPosition GetAnchorPosition()
    {
    ...
    void ViewHierarchyChanged(SubmenuView* source, const ViewHierarchyChangedDetails& details);

    bool GetDropFormats(SubmenuView* source, int* formats, std::set<ui::ClipboardFormatType>* format_types);
    ...
    views::View::DropCallback GetDropCallback(SubmenuView* source, const ui::DropTargetEvent& event);
    ...
    bool use_ash_system_ui_layout() const
    {
        return use_ash_system_ui_layout_;
    }
    ...
    base::CallbackListSubscription AddAnnotationCallback(AnnotationCallback callback);
    ...
    {
        std::optional<std::string> value = std::move(show_menu_host_duration_histogram_);
...
private:
    ...
    void SetSelectionOnPointerDown(SubmenuView* source, const ui::LocatedEvent* event);
    ...
    void UpdateInitialLocation(const gfx::Rect& anchor_bounds, MenuAnchorPosition position, bool context_menu);
    ...
    bool ShowContextMenu(MenuItemView* menu_item, const gfx::Point& screen_location, ui::mojom::MenuSourceType source_type);
    ...
    // all its parents.
    MenuPart GetMenuPartByScreenCoordinateUsingMenu(MenuItemView* item, const gfx::Point& screen_loc);
    ...
    bool GetMenuPartByScreenCoordinateImpl(SubmenuView* menu, const gfx::Point& screen_loc, MenuPart* part);

    // Returns the RootView of the target for the mouse event, if there is a
    // target at |source_loc|.
    MenuHostRootView* GetRootView(SubmenuView* source, const gfx::Point& source_loc);

    // Returns whether the location is over the ACTIONABLE_SUBMENU's submenu area.
    bool IsLocationOverSubmenuAreaOfActionableSubmenu(MenuItemView* item, const gfx::Point& screen_loc) const;
    ...
    // two paths.
    void BuildPathsAndCalculateDiff(
    ...
    void MoveSelectionToFirstOrLastItem(SelectionIncrementDirectionType direction);
    ...
    MenuItemView* FindInitialSelectableMenuItem(MenuItemView* parent, SelectionIncrementDirectionType direction);
    ...
    SelectByCharDetails FindChildForMnemonic(MenuItemView* parent, char16_t key, bool (*match_function)(MenuItemView* menu, char16_t mnemonic));
    ...
    void SetDropMenuItem(MenuItemView* new_item, MenuDelegate::DropPosition position);
    ...
    // itself.
    void UpdateActiveMouseView(SubmenuView* event_source, const ui::MouseEvent& event, View* target_menu);

    // Sends a mouse release event to the current active mouse view and sets
    // it to null.
    void SendMouseReleaseToActiveView(SubmenuView* event_source, const ui::MouseEvent& event);
    ...
    void HandleMouseLocation(SubmenuView* source, const gfx::Point& mouse_location);

    // Sets hot-tracked state to the first focusable descendant view of |item|.
    void SetInitialHotTrackedView(MenuItemView* item, SelectionIncrementDirectionType direction);

    // Sets hot-tracked state to the next focusable element after |item| in
    // |direction|.
    void SetNextHotTrackedView(MenuItemView* item, SelectionIncrementDirectionType direction);
    ...
    void SetAnchorParametersForItem(MenuItemView* item, const gfx::Point& item_loc, ui::OwnedWindowAnchor* anchor);
    ...
    bool MaybeForwardToAnnotation(SubmenuView* source, const ui::LocatedEvent& event);
    ...
    void SetChildMenuOpenDirectionAtDepth(size_t depth, MenuOpenDirection direction);
    ...
    using NestedState = std::pair<State, std::unique_ptr<MenuButtonController::PressedLock>>;
    ...
    std::list<raw_ptr<internal::MenuControllerDelegate, CtnExperimental>> delegate_stack_;
    ...
    MenuDelegate::DropPosition drop_position_ = MenuDelegate::DropPosition::kUnknow;
    ...
    base::RepeatingCallbackList<void(bool&, const ui::LocatedEvent& event)> annotation_callbacks_;
