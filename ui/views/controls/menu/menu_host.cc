...
// of the menu.
class PreMenuEventDispatchHandler : public ui::EventHandler, aura::WindowObserver {
public:
    PreMenuEventDispatchHandler(MenuController* controller, SubmenuView* submenu, aura::Window* window)
        : menu_controller_(controller->AsWeakPtr())
        , submenu_(submenu)
    {
    ...
    PreMenuEventDispatchHandler& operator=(const PreMenuEventDispatchHandler&) = delete;
...
void TransferGesture(ui::GestureRecognizer* gesture_recognizer, gfx::NativeView source, gfx::NativeView target)
{
#if defined(USE_AURA)
    ...
    gesture_recognizer->TransferEventsTo(source, target, ui::TransferTouchesBehavior::kCancel);
#else
    NOTIMPLEMENTED();
...
void MenuHost::InitMenuHost(const InitParams& init_params)
{
    ...
    Widget::InitParams params(Widget::InitParams::NATIVE_WIDGET_OWNS_WIDGET, Widget::InitParams::TYPE_MENU);
    ...
    params.shadow_type = bubble_border ? Widget::InitParams::ShadowType::kNone : Widget::InitParams::ShadowType::kDrop;

    const int corner_radius = MenuConfig::instance().CornerRadiusForMenu(menu_controller);
    params.opacity = (bubble_border || corner_radius) ? Widget::InitParams::WindowOpacity::kTranslucent : Widget::InitParams::WindowOpacity::kOpaque;
    params.corner_radius = corner_radius;
    params.parent = init_params.parent ? init_params.parent->GetNativeView() : gfx::NativeView();
    params.context = init_params.context ? init_params.context->GetNativeWindow() : gfx::NativeWindow();
    params.bounds = init_params.bounds;

#if defined(USE_AURA)
    params.init_properties_container.SetProperty(aura::client::kOwnedWindowAnchor, init_params.owned_window_anchor);
#endif
    ...
    std::optional<std::string> show_menu_host_duration_histogram = menu_controller->TakeShowMenuHostDurationHistogram();
    ...
    if (show_menu_host_duration_histogram.has_value()) {
        ...
        GetCompositor()->RequestSuccessfulPresentationTimeForNextFrame(base::BindOnce(
            [](std::string histogram, base::TimeTicks menu_host_init_time, const viz::FrameTimingDetails& frame_timing_details) {
                base::TimeTicks presentation_time = frame_timing_details.presentation_feedback.timestamp;
                UMA_HISTOGRAM_TIMES(histogram, presentation_time - menu_host_init_time);
            },
            show_menu_host_duration_histogram.value(), base::TimeTicks::Now()));
    }

#if defined(USE_AURA)
    pre_dispatch_handler_ = std::make_unique<internal::PreMenuEventDispatchHandler>(menu_controller, submenu_, GetNativeView());
#endif
...
void MenuHost::ShowMenuHost(bool do_capture)
{
    ...
    if (do_capture) {
        ...
        if (menu_controller && menu_controller->send_gesture_events_to_owner()) {
            ...
            gfx::NativeView source_view = native_view_for_gestures_ ? native_view_for_gestures_ : GetOwner()->GetNativeView();
            internal::TransferGesture(GetGestureRecognizer(), source_view, GetNativeView());
        } else {
...
{
    MenuController* menu_controller = submenu_->GetMenuItem()->GetMenuController();
    if (GetOwner() && menu_controller && menu_controller->send_gesture_events_to_owner()) {
        gfx::NativeView target_view = native_view_for_gestures_ ? native_view_for_gestures_ : GetOwner()->GetNativeView();
        internal::TransferGesture(GetGestureRecognizer(), GetNativeView(), target_view);
    }
...
void MenuHost::SetMenuHostOwnedWindowAnchor(const ui::OwnedWindowAnchor& anchor)
{
#if defined(USE_AURA)
    native_widget_private()->GetNativeWindow()->SetProperty(aura::client::kOwnedWindowAnchor, anchor);
#endif
}
...
void MenuHost::OnDragComplete()
{
    ...
    bool should_close = menu_controller->exit_type() != MenuController::ExitType::kNone;
...
{
    return GetOwner() ? GetOwner()->GetPrimaryWindowWidget() : Widget::GetPrimaryWindowWidget();
}
