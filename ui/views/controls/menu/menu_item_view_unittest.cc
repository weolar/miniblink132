...
TEST_F(MenuItemViewUnitTest, TestMenuItemViewWithFlexibleWidthChild)
{
    ...
    // items.)
    EXPECT_GE(submenu->GetPreferredSize({}).height(), label_size.height() + flex_height);
}
...
TEST_F(MenuItemViewUnitTest, TestEmptyWhenAllItemsAreHidden)
{
    ...
    const auto* empty_item = AsViewClass<EmptyMenuMenuItem>(submenu->children().front());
    ASSERT_TRUE(empty_item);
    EXPECT_EQ(l10n_util::GetStringUTF16(IDS_APP_MENU_EMPTY_SUBMENU), empty_item->title());
}
...
TEST_F(MenuItemViewUnitTest, NotifiesSelectedChanged)
{
    ...
    auto subscription = menu_item_view->AddSelectedChangedCallback(base::BindLambdaForTesting([&]() { is_selected = menu_item_view->IsSelected(); }));
...
TEST_F(MenuItemViewUnitTest, AccessibleKeyShortcutsTest)
{
    ...
    if (MenuConfig::instance().use_mnemonics) {
        ...
        EXPECT_FALSE(data1.HasStringAttribute(ax::mojom::StringAttribute::kKeyShortcuts));
        EXPECT_FALSE(data2.HasStringAttribute(ax::mojom::StringAttribute::kKeyShortcuts));
        EXPECT_FALSE(data3.HasStringAttribute(ax::mojom::StringAttribute::kKeyShortcuts));
        EXPECT_FALSE(data4.HasStringAttribute(ax::mojom::StringAttribute::kKeyShortcuts));
        ...
        item3->GetViewAccessibility().GetAccessibleNodeData(&data3);
        submenu->GetViewAccessibility().GetAccessibleNodeData(&data4);
        EXPECT_EQ("i", data1.GetStringAttribute(ax::mojom::StringAttribute::kKeyShortcuts));
        EXPECT_EQ("e", data2.GetStringAttribute(ax::mojom::StringAttribute::kKeyShortcuts));
        EXPECT_EQ("m", data3.GetStringAttribute(ax::mojom::StringAttribute::kKeyShortcuts));
        EXPECT_EQ("m", data4.GetStringAttribute(ax::mojom::StringAttribute::kKeyShortcuts));
...
{
    views::TestMenuItemView root_menu;
    views::MenuItemView* item1 = root_menu.AppendMenuItemImpl(0, u"checkbox", ui::ImageModel(), MenuItemView::Type::kCheckbox);
    views::MenuItemView* item2 = root_menu.AppendMenuItemImpl(1, u"radio", ui::ImageModel(), MenuItemView::Type::kRadio);
    views::MenuItemView* item3 = root_menu.AppendMenuItemImpl(2, u"title", ui::ImageModel(), MenuItemView::Type::kTitle);
    views::MenuItemView* item4 = root_menu.AppendMenuItemImpl(3, u"highlighted", ui::ImageModel(), MenuItemView::Type::kHighlighted);
    ui::AXNodeData data1, data2, data3, data4;
...
public:
    ...
    void SetUp() override
    {
        ...
        auto menu_item_view_owning = std::make_unique<TestMenuItemView>(menu_delegate_.get());
        menu_item_view_ = menu_item_view_owning.get();
        menu_runner_ = std::make_unique<MenuRunner>(std::move(menu_item_view_owning), MenuRunner::USE_ASH_SYS_UI_LAYOUT);
        menu_runner_->RunMenuAt(widget_.get(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, ui::mojom::MenuSourceType::kKeyboard);
    }
...
TEST_F(TouchableMenuItemViewTest, MinAndMaxWidth)
{
    ...
    gfx::Size item2_size = AppendItemAndGetSize(2, u"Item2 bigger than min less than max");
    ...
    gfx::Size item3_size = AppendItemAndGetSize(3,
...
TEST_F(MenuItemViewLayoutTest, ContainerLayoutRespectsMarginsAndPreferredSize)
{
    ...
    const gfx::Insets actual_margins = test_item()->GetContentsBounds().InsetsFrom(child_bounds);
...
public:
    ...
    MenuItemViewPaintUnitTest& operator=(const MenuItemViewPaintUnitTest&) = delete;

    ~MenuItemViewPaintUnitTest() override = default;

    MenuItemView* menu_item_view()
    {
        return menu_item_view_;
    }
    ...
    void SetUp() override
    {
        ...
        auto menu_item_view_owning = std::make_unique<MenuItemView>(menu_delegate_.get());
        menu_item_view_ = menu_item_view_owning.get();
        ...
        menu_runner_ = std::make_unique<MenuRunner>(std::move(menu_item_view_owning), 0);
    }
    ...
    test::TestMenuDelegate* GetDelegate()
    {
        return menu_delegate_.get();
    }

    ax::mojom::CheckedState GetCheckedStatus(int command, views::MenuItemView::Type type)
    {
        if (type == views::MenuItemView::Type::kRadio || type == views::MenuItemView::Type::kCheckbox) {
            bool is_checked = GetDelegate() && GetDelegate()->IsItemChecked(command);
            return is_checked ? ax::mojom::CheckedState::kTrue : ax::mojom::CheckedState::kFalse;
        } else {
...
{
    auto AddItem = [this](auto label, auto secondary_label, auto minor_label, auto minor_icon) {
        menu_item_view()->AddMenuItemAt(
            0, 1000, label, secondary_label, minor_label, minor_icon, ui::ImageModel(), views::MenuItemView::Type::kNormal, ui::NORMAL_SEPARATOR);
    };
    AddItem(u"No secondary label, no minor content", std::u16string(), std::u16string(), ui::ImageModel());
    AddItem(u"No secondary label, minor text only", std::u16string(), u"minor text", ui::ImageModel());
    AddItem(u"No secondary label, minor icon only", std::u16string(), std::u16string(), ui::ImageModel::FromVectorIcon(views::kMenuCheckIcon));
    AddItem(u"No secondary label, minor text and icon", std::u16string(), u"minor text", ui::ImageModel::FromVectorIcon(views::kMenuCheckIcon));
    AddItem(u"Secondary label, no minor content", u"secondary label", std::u16string(), ui::ImageModel());
    AddItem(u"Secondary label, minor text only", u"secondary label", u"minor text", ui::ImageModel());
    AddItem(u"Secondary label, minor icon only", u"secondary label", std::u16string(), ui::ImageModel::FromVectorIcon(views::kMenuCheckIcon));
    AddItem(u"Secondary label, minor text and icon", u"secondary label", u"minor text", ui::ImageModel::FromVectorIcon(views::kMenuCheckIcon));

    menu_runner()->RunMenuAt(widget(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, ui::mojom::MenuSourceType::kKeyboard);
    ...
    menu_item_view()->GetSubmenu()->Paint(PaintInfo::CreateRootPaintInfo(canvas_painter.context(), size));
}
...
{
    auto AddItem = [this](auto label, auto submenu_background_color, auto foreground_color, auto selected_color) {
        menu_item_view()->AddMenuItemAt(0, 1000, label, std::u16string(), std::u16string(), ui::ImageModel(), ui::ImageModel(),
            views::MenuItemView::Type::kNormal, ui::NORMAL_SEPARATOR, submenu_background_color, foreground_color, selected_color);
    };
    ...
    AddItem(u"No selected color", background_color, foreground_color, std::nullopt);
    AddItem(u"No foreground color", background_color, std::nullopt, selected_color);
    AddItem(u"No background color", std::nullopt, foreground_color, selected_color);
    AddItem(u"No background or foreground", std::nullopt, std::nullopt, selected_color);
    AddItem(u"No background or selected", std::nullopt, foreground_color, std::nullopt);
    AddItem(u"No foreground or selected", background_color, std::nullopt, std::nullopt);
...
{
    MenuItemView* submenu_item = menu_item_view()->AppendSubMenu(1, u"My Submenu");
    submenu_item->AppendMenuItem(1, u"submenu item 1");

    menu_runner()->RunMenuAt(widget(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, ui::mojom::MenuSourceType::kKeyboard);

    ImageView* submenu_arrow_image_view = TestMenuItemView::submenu_arrow_image_view(submenu_item);
...
// state.
TEST_F(MenuItemViewPaintUnitTest, SelectionBasedStateNotUpdatedWhenScheduledForDeletion)
{
    MenuItemView* submenu_item = menu_item_view()->AppendSubMenu(1, u"submenu_item");
    MenuItemView* submenu_child = submenu_item->AppendMenuItem(1, u"submenu_child");
...
{
    MenuItemView* child_menu_item = menu_item_view()->AppendMenuItem(1, u"menu item");
...
{
    MenuItemView* submenu_item = menu_item_view()->AppendSubMenu(1, u"submenu_item");
    MenuItemView* submenu_child1 = submenu_item->AppendMenuItem(1, u"submenu_child");
    MenuItemView* submenu_child2 = submenu_item->AppendMenuItem(1, u"submenu_child");
...
TEST_F(MenuItemViewPaintUnitTest, AccessibleCheckedStateChange)
{
    ...
    auto AddItem = [this](auto command_, auto type_) {
        menu_item_view()->AddMenuItemAt(0, command_, u"No custom colors", std::u16string(), std::u16string(), ui::ImageModel(), ui::ImageModel(), type_,
            ui::NORMAL_SEPARATOR, std::nullopt, std::nullopt, std::nullopt);
    };

    type = views::MenuItemView::Type::kRadio;
    AddItem(command, type);
    menu_item_view()->GetMenuItemByID(command)->GetViewAccessibility().GetAccessibleNodeData(&data);
    ...
    AddItem(command, type);
    menu_item_view()->GetMenuItemByID(command)->GetSubmenu()->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.GetCheckedState(), GetCheckedStatus(command, type));
    EXPECT_EQ(data.GetCheckedState(), ax::mojom::CheckedState::kNone);
}
...
TEST_F(MenuItemViewPaintUnitTest, AccessibleHasPopup)
{
    ...
    type = views::MenuItemView::Type::kCheckbox;
    AddItem(command, type);
    menu_item_view()->GetMenuItemByID(command)->GetViewAccessibility().GetAccessibleNodeData(&data1);
    ...
    menu_item_view()->GetMenuItemByID(command)->GetSubmenu()->GetViewAccessibility().GetAccessibleNodeData(&data2);
...
{
    MenuItemView* submenu_item_view = menu_item_view()->AppendSubMenu(1, u"Submenu");
    menu_runner()->RunMenuAt(widget(), nullptr, gfx::Rect(), MenuAnchorPosition::kTopLeft, ui::mojom::MenuSourceType::kKeyboard);
