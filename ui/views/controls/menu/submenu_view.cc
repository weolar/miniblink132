...
template <typename MIV, typename V> std::vector<MIV*> GetMenuItemsFromChildren(const View::Views& children)
{
    std::vector<MIV*> menu_items;
    base::ranges::transform(children, std::back_inserter(menu_items), static_cast<MIV* (*)(V*)>(&AsViewClass<MenuItemView>));
    std::erase_if(menu_items, [](MIV* item) { return !item || IsViewClass<EmptyMenuMenuItem>(item); });
    return menu_items;
}
...
void SubmenuView::UpdateMenuPartSizes()
{
    ...
    const auto get_metrics = [&] { return std::tie(icon_area_width_, label_start_, trailing_padding_); };
    const auto old_metrics = get_metrics();

    trailing_padding_ = config.item_horizontal_padding + parent_menu_item_->GetItemHorizontalBorder();
    const auto& menu_items = GetMenuItems();
    if (config.reserve_dedicated_arrow_column && base::ranges::any_of(menu_items, &MenuItemView::HasSubmenu)) {
        trailing_padding_ += config.arrow_size
            + (base::Contains(menu_items, MenuItemView::Type::kActionableSubMenu, &MenuItemView::GetType) ? config.actionable_submenu_arrow_to_edge_padding
                                                                                                          : config.arrow_to_edge_padding);
    }

    const auto is_check_or_radio = [](const auto* item) {
        const auto type = item->GetType();
        return type == MenuItemView::Type::kCheckbox || type == MenuItemView::Type::kRadio;
    };
    icon_area_width_ = min_icon_height_ = (config.always_reserve_check_region || base::ranges::any_of(menu_items, is_check_or_radio)) ? kMenuCheckSize : 0;
    int max_icon_width = 0;
    if (!menu_items.empty()) {
        std::vector<int> widths(menu_items.size());
        base::ranges::transform(menu_items, widths.begin(), [&](const MenuItemView* item) {
            ...
            return (config.icons_in_label || !is_check_or_radio(item)) ? icon_size.width() : 0;
        });
    ...
    if (icon_area_width_) {
        ...
        label_start_ += (controller && controller->use_ash_system_ui_layout()) ? config.touchable_item_horizontal_padding : config.icon_label_spacing;
    }
...
void SubmenuView::ChildPreferredSizeChanged(View* child)
{
    ...
    if (controller) {
        ...
        gfx::Rect bounds = controller->CalculateMenuBounds(item, MenuController::MenuOpenDirection::kTrailing, &dir, &anchor);
...
void SubmenuView::Layout(PassKey)
{
    ...
    SetBounds(x(), (pref_height > parent()->height()) ? std::max(parent()->height() - pref_height, y()) : 0, parent()->width(), pref_height);

    const gfx::Insets insets = GetInsets();
    const int x = insets.left();
    int y = insets.top();
    const int menu_item_width = width() - insets.width();
    const int between_item_vertical_padding = MenuConfig::instance().between_item_vertical_padding;
    bool previous_child_was_lower_separator = false;
    for (View* child : children()) {
        if (child->GetVisible()) {
            const auto* separator = AsViewClass<MenuSeparator>(child);
            if (y != insets.top() && !previous_child_was_lower_separator && (!separator || separator->GetType() != ui::UPPER_SEPARATOR)) {
                y += between_item_vertical_padding;
            }
            child->SetBounds(x, y, menu_item_width, child->GetHeightForWidth(menu_item_width));
            y = child->bounds().bottom();
            previous_child_was_lower_separator = separator && separator->GetType() == ui::LOWER_SEPARATOR;
        }
    }
}

gfx::Size SubmenuView::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
    if (children().empty()) {
        return gfx::Size();
    }
    ...
    for (const View* child : children()) {
        ...
        if (const auto* const menu = AsViewClass<const MenuItemView>(child)) {
            ...
            max_complex_width = std::max(max_complex_width, dimensions.standard_width + dimensions.children_width);
            touchable_minimum_width = dimensions.standard_width;
        } else {
            max_complex_width = std::max(max_complex_width, child->GetPreferredSize({}).width());
        }
    }
    ...
    const gfx::Insets insets = GetInsets();
    int width = std::max(max_complex_width, std::max(max_simple_width + max_minor_text_width_ + insets.width(), minimum_preferred_width_ - 2 * insets.width()));

    if (parent_menu_item_->GetMenuController() && parent_menu_item_->GetMenuController()->use_ash_system_ui_layout()) {
    ...
    const auto get_height = [&](int height, const View* child) {
        ...
        const auto* separator = AsViewClass<MenuSeparator>(child);
        if (height && !previous_child_was_lower_separator && (!separator || separator->GetType() != ui::UPPER_SEPARATOR)) {
            height += config.between_item_vertical_padding;
        }
        previous_child_was_lower_separator = separator && separator->GetType() == ui::LOWER_SEPARATOR;
        return height + child->GetHeightForWidth(menu_item_width);
    };
    const int height = std::accumulate(children().cbegin(), children().cend(), 0, get_height);
...
void SubmenuView::PaintChildren(const PaintInfo& paint_info)
{
    ...
    if (paint_drop_indicator) {
        ...
        const SkColor drop_indicator_color = GetColorProvider()->GetColor(ui::kColorMenuDropmarker);
        recorder.canvas()->FillRect(bounds, drop_indicator_color);
    }
}

bool SubmenuView::GetDropFormats(int* formats, std::set<ui::ClipboardFormatType>* format_types)
{
    DCHECK(parent_menu_item_->GetMenuController());
    return parent_menu_item_->GetMenuController()->GetDropFormats(this, formats, format_types);
}
...
views::View::DropCallback SubmenuView::GetDropCallback(const ui::DropTargetEvent& event)
{
    DCHECK(parent_menu_item_->GetMenuController());
    drop_item_ = nullptr;
...
bool SubmenuView::OnMouseWheel(const ui::MouseWheelEvent& e)
{
    ...
    auto i = base::ranges::lower_bound(menu_items, vis_bounds.y(), {}, &MenuItemView::y);
    ...
    const auto scrolled_to_top = [&vis_bounds](const MenuItemView* item) { return item->y() == vis_bounds.y(); };
    ...
    for (bool scroll_up = (e.y_offset() > 0); delta != 0; --delta) {
        ...
        ScrollRectToVisible(gfx::Rect(gfx::Point(0, scroll_target), vis_bounds.size()));
...
std::optional<size_t> SubmenuView::GetSelectedRow()
{
    ...
    return (i == menu_items.cend()) ? std::nullopt : std::make_optional(static_cast<size_t>(std::distance(menu_items.cbegin(), i)));
}

void SubmenuView::SetSelectedRow(std::optional<size_t> row)
{
    parent_menu_item_->GetMenuController()->SetSelection(GetMenuItemAt(row.value()), MenuController::SELECTION_DEFAULT);
}

std::u16string SubmenuView::GetTextForRow(size_t row)
{
    return MenuItemView::GetAccessibleNameForMenuItem(GetMenuItemAt(row)->title(), std::u16string(), GetMenuItemAt(row)->ShouldShowNewBadge());
}
...
void SubmenuView::ShowAt(const MenuHost::InitParams& init_params)
{
    ...
    if (!GetMenuItem()->GetParentMenuItem()) {
        GetScrollViewContainer()->NotifyAccessibilityEvent(ax::mojom::Event::kMenuStart, true);
    }
    ...
    if (GetRowCount() == 0) {
        GetViewAccessibility().AnnouncePolitely(l10n_util::GetStringUTF16(IDS_APP_MENU_AX_ANNOUNCE_EMPTY_SUBMENU));
    }
}

void SubmenuView::Reposition(const gfx::Rect& bounds, const ui::OwnedWindowAnchor& anchor)
{
    if (host_) {
...
void SubmenuView::Hide()
{
    if (host_) {
        ...
        if (!GetMenuItem()->GetParentMenuItem()) {
            GetScrollViewContainer()->NotifyAccessibilityEvent(ax::mojom::Event::kMenuEnd, true);
...
void SubmenuView::SetDropMenuItem(MenuItemView* item, MenuDelegate::DropPosition position)
{
    ...
    if (!old_drop_item || !item) {
        ...
        // selected state.
        for (View* child : children()) {
            if (auto* child_menu_item = AsViewClass<MenuItemView>(child); child_menu_item && child_menu_item->GetVisible() && child_menu_item->IsSelected()) {
...
{
    return !drop_item_ || (drop_item_ == item && drop_position_ == MenuDelegate::DropPosition::kOn);
}
...
void SubmenuView::SchedulePaintForDropIndicator(MenuItemView* item, MenuDelegate::DropPosition position)
{
    if (item == nullptr)
        return;
...
gfx::Rect SubmenuView::CalculateDropIndicatorBounds(MenuItemView* item, MenuDelegate::DropPosition position)
{
