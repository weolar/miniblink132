...
public:
    ...
    // Item definition.
    struct Item {
        Item(ItemType item_type, const std::string& item_label, ui::MenuModel* item_submenu)
            : type(item_type)
        ...
        Item(ItemType item_type, const std::string& item_label, ui::MenuModel* item_submenu, bool enabled, bool visible)
            : type(item_type)
            , label(base::ASCIIToUTF16(item_label))
            , submenu(item_submenu)
            , enabled(enabled)
    ...
    const Item& GetItemDefinition(size_t index)
    {
        return items_[index];
    }

    // Access index argument to ActivatedAt().
    std::optional<size_t> last_activation() const
    {
...
public:
    ActionableSubmenuModel()
        : MenuModelBase(kActionableSubmenuIdBase)
    {
...
public:
        ...
        : MenuModelBase(kRootIdBase)
    {
        ...
        items_.emplace_back(TYPE_ACTIONABLE_SUBMENU, "actionable 5", actionable_submenu_model_.get());
    }
...
void CheckSubmenu(const RootModel& model, views::MenuItemView* menu, views::MenuModelAdapter* delegate, int submenu_id, size_t expected_children,
    size_t submenu_model_index, int id)
{
    ...
    MenuModelBase* submodel = static_cast<MenuModelBase*>(model.GetSubmenuModelAt(submenu_model_index));
    ...
    for (size_t i = 0; i < subitem_container->children().size(); ++i, ++id) {
        ...
        case ui::MenuModel::TYPE_ACTIONABLE_SUBMENU:
            EXPECT_EQ(views::MenuItemView::Type::kActionableSubMenu, item->GetType());
            break;
...
TEST_F(MenuModelAdapterTest, BasicTest)
{
    ...
    CheckSubmenu(model, menu, &delegate, kRootIdBase + submenu_index, 2, submenu_index, kSubmenuIdBase);

    // Check the actionable submenu.
    const int actionable_submenu_index = 5;
    CheckSubmenu(model, menu, &delegate, kRootIdBase + actionable_submenu_index, 2, actionable_submenu_index, kActionableSubmenuIdBase);
}
