...
class VIEWS_EXPORT MenuRunnerImplRemoteCocoa : public internal::MenuRunnerImplInterface, public remote_cocoa::mojom::MenuHost {
public:
    MenuRunnerImplRemoteCocoa(ui::MenuModel* menu_model, base::RepeatingClosure on_menu_closed_callback);

    MenuRunnerImplRemoteCocoa(const MenuRunnerImplRemoteCocoa&) = delete;
    MenuRunnerImplRemoteCocoa& operator=(const MenuRunnerImplRemoteCocoa&) = delete;
    ...
    // submenu).
    void RunMenu(Widget* widget, const gfx::Point& anchor, uint64_t target_view_id = 0);
    ...
    void UpdateMenuItem(int command_id, bool enabled, bool hidden, const std::u16string& title);

    // MenuRunnerImplInterface:
    bool IsRunning() const override;
    void Release() override;
    void RunMenuAt(Widget* parent, MenuButtonController* button_controller, const gfx::Rect& bounds, MenuAnchorPosition anchor, int32_t run_types,
        gfx::NativeView native_view_for_gestures, std::optional<gfx::RoundedCornersF> corners,
...
private:
    ...
    std::vector<remote_cocoa::mojom::MenuItemPtr> ModelToMojo(const ui::MenuModel& menu_model, std::set<int>& command_ids);
