...
public:
    ...
    static std::u16string GetAccessibleNameForMenuItem(const std::u16string& item_text, const std::u16string& accelerator_text, bool is_new_feature);
    ...
    MenuItemView* AddMenuItemAt(size_t index, int item_id, const std::u16string& label, const std::u16string& secondary_label, const std::u16string& minor_text,
        const ui::ImageModel& minor_icon, const ui::ImageModel& icon, Type type, ui::MenuSeparatorType separator_style,
        std::optional<ui::ColorId> submenu_background_color = std::nullopt, std::optional<ui::ColorId> foreground_color = std::nullopt,
        std::optional<ui::ColorId> selected_color_id = std::nullopt);

    void SetMenuItemBackground(std::optional<MenuItemBackground> menu_item_background)
    {
    ...
    std::optional<ui::ColorId> GetSelectedColorId()
    {
        return selected_color_id_;
    }

    void SetHighlightWhenSelectedWithChildViews(bool highlight_when_selected_with_child_views)
    {
        highlight_when_selected_with_child_views_ = highlight_when_selected_with_child_views;
    }
    ...
    // icon       The icon.
    MenuItemView* AppendMenuItem(int item_id, const std::u16string& label = std::u16string(), const ui::ImageModel& icon = ui::ImageModel());

    MenuItemView* AppendTitle(const std::u16string& label);

    // Append a submenu to this menu.
    // The returned pointer is owned by this menu.
    MenuItemView* AppendSubMenu(int item_id, const std::u16string& label, const ui::ImageModel& icon = ui::ImageModel());
    ...
    MenuItemView* AppendMenuItemImpl(int item_id, const std::u16string& label, const ui::ImageModel& icon, Type type);
    ...
    MenuItemView* GetParentMenuItem()
    {
        return parent_menu_item_;
    }
    const MenuItemView* GetParentMenuItem() const
    {
        return parent_menu_item_;
    }
    ...
    const std::u16string& secondary_title() const
    {
    ...
    [[nodiscard]] base::CallbackListSubscription AddSelectedChangedCallback(PropertyChangedCallback callback);

    // Sets whether the submenu area of an ACTIONABLE_SUBMENU is selected.
    void SetSelectionOfActionableSubmenu(bool submenu_area_of_actionable_submenu_selected);
    ...
    void set_delegate(MenuDelegate* delegate)
    {
        delegate_ = delegate;
    }
    ...
    // item.
    bool has_mnemonics() const
    {
        return has_mnemonics_;
    }
    ...
    void SetTriggerActionWithNonIconChildViews(bool trigger_action_with_non_icon_child_views)
    {
        trigger_action_with_non_icon_child_views_ = trigger_action_with_non_icon_child_views;
    }
...
private:
    ...
    MenuItemView(MenuItemView* parent, int command, Type type, MenuDelegate* delegate);
    ...
    // background.
    void PaintBackground(gfx::Canvas* canvas, PaintMode mode, bool paint_as_selected);
    ...
    // active menu.
    MenuPosition actual_menu_position() const
    {
        return actual_menu_position_;
    }
    ...
    const std::u16string new_badge_text_ = l10n_util::GetStringUTF16(IDS_NEW_BADGE);
