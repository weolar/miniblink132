...
remote_cocoa::mojom::MenuControllerParamsPtr MenuControllerParamsForWidget(Widget* widget)
{
    ...
    params->badge_font = BadgePainter::GetBadgeFont(MenuConfig::instance().font_list).GetPrimaryFont();
    params->badge_color = color_provider->GetColor(ui::kColorBadgeInCocoaMenuBackground);
    params->badge_text_color = color_provider->GetColor(ui::kColorBadgeInCocoaMenuForeground);
    ...
    params->badge_radius = base::checked_cast<uint32_t>(LayoutProvider::Get()->GetCornerRadiusMetric(ShapeContextTokens::kBadgeRadius));
    params->iph_dot_color = color_provider->GetColor(ui::kColorButtonBackgroundProminent);
    return params;
}
