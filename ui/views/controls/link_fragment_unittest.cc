...
protected:
    void CreateWidgetContent(View* container) override
    {
        ...
        fragments_[0] = container->AddChildView(std::make_unique<LinkFragment>(kLinkLabel));
        gfx::Rect current_rect = gfx::ScaleToEnclosedRect(container->GetLocalBounds(), 0.3f);
        ...
        fragments_[1] = container->AddChildView(std::make_unique<LinkFragment>(kLinkLabel));
        fragments_[1]->SetBoundsRect(current_rect);

        current_rect.Offset(width, 0);
        fragments_[2] = container->AddChildView(std::make_unique<LinkFragment>(kLinkLabel, style::CONTEXT_LABEL, style::STYLE_LINK, fragment(1)));
...
TEST_F(LinkFragmentTest, TestUnderlineOnHover)
{
    ...
    const gfx::Point point_outside = GetBoundsForFragment(2).bottom_right() + gfx::Vector2d(1, 1);
    event_generator()->MoveMouseTo(point_outside);
    EXPECT_FALSE(fragment(0)->IsMouseHovered());

    const auto is_underlined = [this](size_t index) { return !!(fragment(index)->font_list().GetFontStyle() & gfx::Font::UNDERLINE); };
