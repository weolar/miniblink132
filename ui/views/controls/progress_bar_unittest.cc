...
protected:
    ...
    void SetUp() override
    {
        ...
        auto* layout = container_view_->SetLayoutManager(std::make_unique<FlexLayout>());
...
TEST_F(ProgressBarTest, AccessibleNodeData)
{
    ...
    EXPECT_EQ(std::u16string(), node_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
    EXPECT_EQ(std::string("62%"), node_data.GetStringAttribute(ax::mojom::StringAttribute::kValue));
...
TEST_F(ProgressBarTest, OverrideDefaultColors)
{
    ...
    EXPECT_EQ(color_provider->GetColor(ui::kColorSysPrimary), bar()->GetForegroundColor());
    EXPECT_EQ(color_provider->GetColor(ui::kColorSysPrimaryContainer), bar()->GetBackgroundColor());
    EXPECT_EQ(ui::kColorSysPrimary, bar()->GetForegroundColorId().value());
    EXPECT_EQ(ui::kColorSysPrimaryContainer, bar()->GetBackgroundColorId().value());
...
TEST_F(ProgressBarTest, RoundCornerMax)
{
    ...
    views::test::RunScheduledLayout(container_view_);
    EXPECT_EQ(gfx::RoundedCornersF(12, 12, 12, 12), bar()->GetPreferredCornerRadii());
}
...
TEST_F(ProgressBarTest, RemoveValue)
{
    ...
    ui::AXNodeData node_data;
    bar()->GetViewAccessibility().GetAccessibleNodeData(&node_data);
    EXPECT_EQ(std::string(""), node_data.GetStringAttribute(ax::mojom::StringAttribute::kValue));
}
