...
public:
    ...
    void SetUp() override
    {
        ...
        textarea_ = PrepareTextfields(0, std::make_unique<Textarea>(), gfx::Rect(100, 100, 800, 100));
    }

protected:
    void RunMoveUpDownTest(int start_index, ui::KeyboardCode key_code, std::vector<int> expected)
    {
        DCHECK(key_code == ui::VKEY_UP || key_code == ui::VKEY_DOWN);
        textarea_->SetSelectedRange(gfx::Range(start_index));
        for (size_t i = 0; i < expected.size(); ++i) {
            SCOPED_TRACE(testing::Message() << (key_code == ui::VKEY_UP ? "MOVE UP " : "MOVE DOWN ") << i + 1 << " times from Range " << start_index);
    ...
    {
        return GetTextfieldTestApi().GetRenderText()->GetLineContainingCaret(textarea_->GetSelectionModel());
    }
...
TEST_F(TextareaTest, MovePageUpDownAndModifySelection)
{
    ...
    EXPECT_TRUE(textarea_->IsTextEditCommandEnabled(ui::TextEditCommand::MOVE_PAGE_UP));
    EXPECT_TRUE(textarea_->IsTextEditCommandEnabled(ui::TextEditCommand::MOVE_PAGE_DOWN));
    EXPECT_TRUE(textarea_->IsTextEditCommandEnabled(ui::TextEditCommand::MOVE_PAGE_UP_AND_MODIFY_SELECTION));
    EXPECT_TRUE(textarea_->IsTextEditCommandEnabled(ui::TextEditCommand::MOVE_PAGE_DOWN_AND_MODIFY_SELECTION));
...
TEST_F(TextareaTest, MoveRangeSelectionExtentExpandByWord)
{
    ...
    gfx::test::RenderTextTestApi(GetTextfieldTestApi().GetRenderText()).SetGlyphHeight(kGlyphHeight);
    ...
    textarea_->SelectBetweenCoordinates(gfx::Point(GetCursorPositionX(2), kFirstLineMiddleY), gfx::Point(GetCursorPositionX(3), kFirstLineMiddleY));

    // Expand the selection. The end of the selection should move to the nearest
    // word boundary.
    textarea_->MoveRangeSelectionExtent(gfx::Point(GetCursorPositionX(22), kSecondLineMiddleY));
    gfx::Range range;
    ...
    textarea_->MoveRangeSelectionExtent(gfx::Point(GetCursorPositionX(7), kFirstLineMiddleY));
    textarea_->MoveRangeSelectionExtent(gfx::Point(GetCursorPositionX(29), kSecondLineMiddleY));
    textarea_->GetEditableSelectionRange(&range);
...
TEST_F(TextareaTest, MoveRangeSelectionExtentShrinkByCharacter)
{
    ...
    const int kFirstLineMiddleY = GetCursorYForTesting() + kGlyphHeight / 2;
    const int kSecondLineMiddleY = kFirstLineMiddleY + kGlyphHeight;
    textarea_->SelectBetweenCoordinates(gfx::Point(GetCursorPositionX(2), kFirstLineMiddleY), gfx::Point(GetCursorPositionX(25), kSecondLineMiddleY));
