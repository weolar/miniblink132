...
void SeparatorTest::SetUp()
{
    ...
    expected_foreground_color_ = widget_->GetColorProvider()->GetColor(kForegroundColorId);
}
...
TEST_F(SeparatorTest, GetPreferredSize_VerticalOrientation)
{
    ...
    EXPECT_EQ(separator_->GetPreferredSize({}), gfx::Size(Separator::kThickness, kLength));
}
...
TEST_F(SeparatorTest, GetPreferredSize_HorizontalOrientation)
{
    ...
    separator_->SetPreferredLength(kLength);
    EXPECT_EQ(separator_->GetPreferredSize({}), gfx::Size(kLength, Separator::kThickness));
}
