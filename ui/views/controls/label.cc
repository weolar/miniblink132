...
DEFINE_OWNED_UI_CLASS_PROPERTY_KEY(CascadingProperty<SkColor>, kCascadingLabelEnabledColor, nullptr)
...
Label::Label(const std::u16string& text, int text_context, int text_style, gfx::DirectionalityMode directionality_mode)
    : text_context_(text_context)
    , text_style_(text_style)
    , context_menu_contents_(this)
{
    Init(text, TypographyProvider::Get().GetFont(text_context, text_style), directionality_mode);
}
...
{
    return TypographyProvider::Get().GetFont(style::CONTEXT_LABEL, style::STYLE_PRIMARY);
}
...
void Label::SetText(const std::u16string& new_text)
{
    ...
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&full_text_, kLabelText), kPropertyEffectsPreferredSizeChanged);
    ...
    if (GetViewAccessibility().GetCachedName().empty() || GetViewAccessibility().GetCachedName() == current_text) {
        if (new_text.empty()) {
            GetViewAccessibility().SetName(new_text, ax::mojom::NameFrom::kAttributeExplicitlyEmpty);
        } else {
...
void Label::AdjustAccessibleName(std::u16string& new_name, ax::mojom::NameFrom& name_from)
{
    if (new_name.empty()) {
...
void Label::SetTextContext(int text_context)
{
    if (text_context == text_context_)
        return;
    text_context_ = text_context;
    full_text_->SetFontList(TypographyProvider::Get().GetFont(text_context_, text_style_));
    full_text_->SetMinLineHeight(GetLineHeight());
    ClearDisplayText();
    if (GetWidget())
        UpdateColorsFromTheme();

    GetViewAccessibility().SetRole(text_context_ == style::CONTEXT_DIALOG_TITLE ? ax::mojom::Role::kTitleBar : ax::mojom::Role::kStaticText);
...
{
    if (style == text_style_ || !range.IsValid() || range.is_empty() || !gfx::Range(0, GetText().size()).Contains(range)) {
        return;
    }
    ...
    DCHECK_EQ(details.typeface, typography_provider.GetFontDetails(text_context_, text_style_).typeface);
    DCHECK_EQ(details.size_delta, typography_provider.GetFontDetails(text_context_, text_style_).size_delta);
...
void Label::SetAutoColorReadabilityEnabled(bool auto_color_readability_enabled)
{
...
void Label::SetBackgroundColorId(std::optional<ui::ColorId> background_color_id)
{
...
void Label::SetShadows(const gfx::ShadowValues& shadows)
{
    ...
    ClearDisplayText();
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&full_text_, kLabelShadows), kPropertyEffectsPreferredSizeChanged);
}
...
void Label::SetSkipSubpixelRenderingOpacityCheck(bool skip_subpixel_rendering_opacity_check)
{
    if (skip_subpixel_rendering_opacity_check_ == skip_subpixel_rendering_opacity_check) {
        return;
    }
    skip_subpixel_rendering_opacity_check_ = skip_subpixel_rendering_opacity_check;
    OnPropertyChanged(&skip_subpixel_rendering_opacity_check_, kPropertyEffectsNone);
}
...
void Label::SetHorizontalAlignment(gfx::HorizontalAlignment alignment)
{
    ...
    ClearDisplayText();
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&full_text_, kLabelHorizontalAlignment), kPropertyEffectsPaint);
#if BUILDFLAG(SUPPORTS_AX_TEXT_OFFSETS)
    MaybeRefreshAccessibleTextOffsets();
#endif // BUILDFLAG(SUPPORTS_AX_TEXT_OFFSETS)
}
...
void Label::SetVerticalAlignment(gfx::VerticalAlignment alignment)
{
    ...
    ClearDisplayText();
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&full_text_, kLabelVerticalAlignment), kPropertyEffectsPaint);
}
...
int Label::GetLineHeight() const
{
    ...
    return line_height_.value_or(std::max(TypographyProvider::Get().GetLineHeight(text_context_, text_style_), font_list().GetHeight()));
}
...
void Label::SetLineHeight(int line_height)
{
    ...
    ClearDisplayText();
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&full_text_, kLabelLineHeight), kPropertyEffectsPreferredSizeChanged);
}
...
void Label::SetObscured(bool obscured)
{
    ...
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&full_text_, kLabelObscured), kPropertyEffectsPreferredSizeChanged);
#if BUILDFLAG(SUPPORTS_AX_TEXT_OFFSETS)
...
bool Label::IsDisplayTextClipped() const
{
    ...
    return text_bounds.width() > GetContentsBounds().width() || text_bounds.height() > GetContentsBounds().height();
}
...
bool Label::IsDisplayTextTruncated() const
{
    ...
    return (display_text_ && display_text_->text() != display_text_->GetDisplayText()) || IsDisplayTextClipped();
}
...
void Label::SetAllowCharacterBreak(bool allow_character_break)
{
    ...
    ClearDisplayText();
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&full_text_, kLabelAllowCharacterBreak), kPropertyEffectsPreferredSizeChanged);
}
...
{
    DCHECK(!GetMultiLine() || (elide_behavior == gfx::ELIDE_TAIL || elide_behavior == gfx::NO_ELIDE));
...
void Label::SetMaximumWidthSingleLine(int max_width)
{
    ...
    UpdateFullTextElideBehavior();
    OnPropertyChanged(&max_width_single_line_, kPropertyEffectsPreferredSizeChanged);
}
...
void Label::SetCollapseWhenHidden(bool value)
{
    ...
    OnPropertyChanged(&collapse_when_hidden_, kPropertyEffectsPreferredSizeChanged);
}
...
{
    const gfx::RenderText* render_text = GetRenderTextForSelectionController();
    return render_text ? render_text->selection().length() == render_text->text().length() : false;
}
...
base::CallbackListSubscription Label::AddTextChangedCallback(views::PropertyChangedCallback callback)
{
    return AddPropertyChangedCallback(ui::metadata::MakeUniquePropertyKey(&full_text_, kLabelText), std::move(callback));
}

base::CallbackListSubscription Label::AddTextContextChangedCallback(views::PropertyChangedCallback callback)
{
...
gfx::Size Label::CalculatePreferredSize(const SizeBounds& available_size) const
{
...
gfx::Size Label::GetMinimumSize() const
{
    ...
    if (elide_behavior_ == gfx::ELIDE_HEAD || elide_behavior_ == gfx::ELIDE_MIDDLE || elide_behavior_ == gfx::ELIDE_TAIL
        || elide_behavior_ == gfx::ELIDE_EMAIL) {
        size.set_width(gfx::Canvas::GetStringWidth(std::u16string(gfx::kEllipsisUTF16), font_list()));
    }
...
int Label::GetLabelHeightForWidth(int w) const
{
    ...
    if (!GetMultiLine() || GetText().empty() || w < 0) {
        ...
        height = std::max(base::checked_cast<int>(GetMaxLines()), 1) * base_line_height;
    } else {
        ...
        height = GetMaxLines() > 0 ? std::min(base::checked_cast<int>(GetMaxLines()) * base_line_height, string_height) : string_height;
    }
...
std::unique_ptr<gfx::RenderText> Label::CreateRenderText() const
{
    ...
    gfx::ElideBehavior elide_behavior = GetMultiLine() && (elide_behavior_ != gfx::NO_ELIDE) ? gfx::ELIDE_TAIL : elide_behavior_;

    std::unique_ptr<gfx::RenderText> render_text = full_text_->CreateInstanceOfSameStyle(GetText());
...
gfx::Rect Label::GetTextBounds() const
{
    ...
    return gfx::Rect(gfx::Point() + display_text_->GetLineOffset(0), display_text_->GetStringSize());
}
...
int Label::GetFontListY() const
{
    ...
    return GetInsets().top() + display_text_->GetBaseline() - font_list().GetBaseline();
}
...
void Label::PaintText(gfx::Canvas* canvas)
{
    ...
    // DCHECK detects.
    if (!display_text_ || display_text_->subpixel_rendering_suppressed() || skip_subpixel_rendering_opacity_check_) {
        return;
    }
...
{
    return GetRenderTextForSelectionController() ? ui::mojom::CursorType::kIBeam : ui::Cursor();
}
...
bool Label::OnMousePressed(const ui::MouseEvent& event)
{
    ...
    if ((event.IsOnlyLeftMouseButton() || event.IsOnlyRightMouseButton()) && GetFocusManager() && !had_focus) {
...
bool Label::CanHandleAccelerators() const
{
    ...
    return HasFocus() && GetRenderTextForSelectionController() && View::CanHandleAccelerators();
}
...
void Label::OnAccessibilityInitializing(ui::AXNodeData* data)
{
    if (display_text_ && ::ui::AXPlatform::GetInstance().IsUiaProviderEnabled()) {
        ...
        data->AddIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets, ComputeTextOffsets(display_text_.get()));
...
{
    if (::ui::AXPlatform::GetInstance().IsUiaProviderEnabled() && GetViewAccessibility().is_initialized()
        && ax_name_used_to_compute_offsets_ != GetViewAccessibility().GetCachedName()) {
...
bool Label::RefreshAccessibleTextOffsetsIfNeeded() const
{
    ...
    GetViewAccessibility().SetCharacterOffsets(ComputeTextOffsets(display_text_.get()));
...
void Label::OnDeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor)
{
...
void Label::ShowContextMenuForViewImpl(View* source, const gfx::Point& point, ui::mojom::MenuSourceType source_type)
{
    if (!GetRenderTextForSelectionController())
        return;

    context_menu_runner_ = std::make_unique<MenuRunner>(&context_menu_contents_, MenuRunner::HAS_MNEMONICS | MenuRunner::CONTEXT_MENU);
    context_menu_runner_->RunMenuAt(GetWidget(), nullptr, gfx::Rect(point, gfx::Size()), MenuAnchorPosition::kTopLeft, source_type);
}

bool Label::GetWordLookupDataAtPoint(const gfx::Point& point, gfx::DecoratedText* decorated_word, gfx::Rect* rect)
{
    gfx::RenderText* render_text = GetRenderTextForSelectionController();
...
bool Label::GetWordLookupDataFromSelection(gfx::DecoratedText* decorated_text, gfx::Rect* rect)
{
    gfx::RenderText* render_text = GetRenderTextForSelectionController();
    return render_text ? render_text->GetLookupDataForRange(render_text->selection(), decorated_text, rect) : false;
}

gfx::RenderText* Label::GetRenderTextForSelectionController()
{
    return const_cast<gfx::RenderText*>(static_cast<const Label*>(this)->GetRenderTextForSelectionController());
}
...
bool Label::GetAcceleratorForCommandId(int command_id, ui::Accelerator* accelerator) const
{
    switch (command_id) {
    case MenuCommands::kCopy:
...
void Label::Init(const std::u16string& text, const gfx::FontList& font_list, gfx::DirectionalityMode directionality_mode)
{
...
gfx::Size Label::GetBoundedTextSize(const SizeBounds& available_size) const
{
    ...
    SizeBound w = std::max<SizeBound>(0, available_size.width() - GetInsets().width());
    if (GetText().empty() || (w == 0 && GetMultiLine())) {
        ...
        full_text_->SetDisplayRect(gfx::Rect(0, 0, max_width_single_line_ - GetInsets().width(), 0));
        size = full_text_->GetStringSize();

        if (base_line_height > 0) {
            size.set_height(base::checked_cast<int>(GetRequiredLines()) * base_line_height);
        }
    } else {
...
SkColor Label::GetForegroundColor(SkColor foreground, SkColor background) const
{
    return (auto_color_readability_enabled_ && IsOpaque(background)) ? color_utils::BlendForMinContrast(foreground, background).color : foreground;
}

void Label::RecalculateColors()
{
    actual_enabled_color_ = GetForegroundColor(requested_enabled_color_, background_color_);
    ...
    actual_selection_text_color_
        = GetForegroundColor(requested_selection_text_color_, color_utils::GetResultingPaintColor(selection_background_color_, background_color_));
...
void Label::ApplyTextColors() const
{
    ...
    display_text_->set_selection_background_focused_color(selection_background_color_);
    const bool subpixel_rendering_enabled = subpixel_rendering_enabled_ && IsOpaque(background_color_);
...
void Label::UpdateColorsFromTheme()
{
    ...
    if (enabled_color_id_.has_value()) {
        ...
        const std::optional<SkColor> cascading_color = GetCascadingProperty(this, kCascadingLabelEnabledColor);
        requested_enabled_color_ = cascading_color.value_or(GetColorProvider()->GetColor(TypographyProvider::Get().GetColorId(text_context_, text_style_)));
    }
    ...
    if (!selection_text_color_set_) {
        requested_selection_text_color_ = color_provider->GetColor(ui::kColorLabelSelectionForeground);
    }
    if (!selection_background_color_set_) {
        selection_background_color_ = color_provider->GetColor(ui::kColorLabelSelectionBackground);
    }
    RecalculateColors();
}
...
void Label::ClearDisplayText()
{
    ...
    if (HasSelection()) {
        stored_selection_range_ = GetRenderTextForSelectionController()->selection();
    }
...
{
    const gfx::RenderText* render_text = GetRenderTextForSelectionController();
    return render_text ? render_text->GetTextFromRange(render_text->selection()) : std::u16string();
}
...
void Label::CopyToClipboard()
{
    if (!HasSelection() || GetObscured())
        return;
    ui::ScopedClipboardWriter(ui::ClipboardBuffer::kCopyPaste).WriteText(GetSelectedText());
}

void Label::BuildContextMenuContents()
{
    context_menu_contents_.AddItemWithStringId(MenuCommands::kCopy, IDS_APP_COPY);
    context_menu_contents_.AddItemWithStringId(MenuCommands::kSelectAll, IDS_APP_SELECT_ALL);
}
...
void Label::UpdateFullTextElideBehavior()
{
    ...
    full_text_->SetElideBehavior(max_width_single_line_ > 0 ? elide_behavior_ : gfx::NO_ELIDE);
}
