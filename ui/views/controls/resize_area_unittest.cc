...
const int kDistancePerGestureScrollUpdate = kGestureScrollDistance / kGestureScrollSteps;
} // namespace
...
public:
    ...
    int resize_amount()
    {
        return resize_amount_;
    }
    bool done_resizing()
    {
        return done_resizing_;
    }
    bool on_resize_called()
    {
        return on_resize_called_;
    }
...
protected:
    ...
    int resize_amount()
    {
        return delegate_->resize_amount();
    }
    bool done_resizing()
    {
        return delegate_->done_resizing();
    }
    bool on_resize_called()
    {
        return delegate_->on_resize_called();
    }
    views::Widget* widget()
    {
        return widget_.get();
...
void ResizeAreaTest::ProcessGesture(ui::EventType type, const gfx::Vector2dF& delta)
{
    if (type == ui::EventType::kGestureScrollBegin) {
        ...
        EXPECT_EQ(kDistancePerGestureScrollUpdate * gesture_scroll_updates_seen_, resize_amount());
        EXPECT_FALSE(done_resizing());
...
TEST_F(ResizeAreaTest, SuccessfulGestureDrag)
{
    ...
    event_generator()->GestureScrollSequenceWithCallback(start, gfx::Point(start.x() + kGestureScrollDistance, start.y()), base::Milliseconds(200),
        kGestureScrollSteps, base::BindRepeating(&ResizeAreaTest::ProcessGesture, base::Unretained(this)));
}
...
TEST_F(ResizeAreaTest, AccessibleRole)
{
    ...
    EXPECT_EQ(resize_area->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kSplitter);

    data = ui::AXNodeData();
    resize_area->GetViewAccessibility().SetRole(ax::mojom::Role::kButton);
    resize_area->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.role, ax::mojom::Role::kButton);
    EXPECT_EQ(resize_area->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kButton);
}
