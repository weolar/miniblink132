...
{
    View::OnThemeChanged();
    if (image_model_.IsImageGenerator() || (image_model_.IsVectorIcon() && !image_model_.GetVectorIcon().has_color())) {
        scaled_image_ = gfx::ImageSkia();
        SchedulePaint();
    }
}
...
void ImageView::OnPaintImage(gfx::Canvas* canvas)
{
    ...
    if (image_bounds.size() != gfx::Size(image.width(), image.height())) {
        ...
        canvas->DrawImageInt(
            image, 0, 0, image.width(), image.height(), image_bounds.x(), image_bounds.y(), image_bounds.width(), image_bounds.height(), true, flags);
    } else {
...
gfx::ImageSkia ImageView::GetPaintImage(float scale)
{
    ...
    if (image_model_.IsImage() || image_model_.IsImageGenerator()) {
        ...
        gfx::Size scaled_size = gfx::ScaleToCeiledSize(rep.pixel_size(), scale / rep.scale());
        scaled_image_.AddRepresentation(gfx::ImageSkiaRep(
            skia::ImageOperations::Resize(rep.GetBitmap(), skia::ImageOperations::RESIZE_BEST, scaled_size.width(), scaled_size.height()), scale));
