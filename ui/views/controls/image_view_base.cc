...
void ImageViewBase::SetTooltipText(const std::u16string& tooltip)
{
    ...
    if (GetViewAccessibility().GetCachedName().empty() || GetViewAccessibility().GetCachedName() == current_tooltip) {
...
void ImageViewBase::AdjustAccessibleName(std::u16string& new_name, ax::mojom::NameFrom& name_from)
{
    if (new_name.empty()) {
...
gfx::Size ImageViewBase::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
...
void ImageViewBase::UpdateImageOrigin()
{
    ...
    if (base::i18n::IsRTL() && (horizontal_alignment_ != Alignment::kCenter)) {
        actual_horizontal_alignment = (horizontal_alignment_ == Alignment::kLeading) ? Alignment::kTrailing : Alignment::kLeading;
    }
...
DEFINE_ENUM_CONVERTERS(views::ImageViewBase::Alignment, { views::ImageViewBase::Alignment::kLeading, u"kLeading" },
    { views::ImageViewBase::Alignment::kCenter, u"kCenter" }, { views::ImageViewBase::Alignment::kTrailing, u"kTrailing" })
