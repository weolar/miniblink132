...
{
    return !path.isEmpty() && (path.isRect(nullptr) || path.isOval(nullptr) || path.isRRect(nullptr));
}
...
SkPath GetHighlightPathInternal(const View* view, float halo_thickness)
{
    ...
    return SkPath().addRRect(SkRRect::MakeRectXY(RectToSkRect(client_rect), corner_radius, corner_radius));
}
...
void FocusRing::SetPathGenerator(std::unique_ptr<HighlightPathGenerator> generator)
{
...
void FocusRing::Layout(PassKey)
{
    ...
    if (IsPathUsable(path)) {
        const gfx::Rect path_bounds = gfx::ToEnclosingRect(gfx::SkRectToRectF(path.getBounds()));
        const gfx::Rect expanded_bounds = gfx::UnionRects(focus_bounds, path_bounds);
        ...
        const int min_x_inset = std::min(expansion_insets.left(), expansion_insets.right());
...
void FocusRing::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details)
{
...
SkRRect FocusRing::GetRingRoundRect() const
{
    ...
    DCHECK_EQ(GetFlipCanvasOnPaintForRTLUI(), parent()->GetFlipCanvasOnPaintForRTLUI());
...
void FocusRing::RefreshLayer()
{
    ...
    // returns a new value.
    const bool should_paint = has_focus_predicate_ || (parent() && parent()->HasFocus());
...
bool FocusRing::ShouldSetOutsetFocusRing() const
{
    ...
    // with UX.
    return !outset_focus_ring_disabled_ && halo_inset_ == FocusRing::kDefaultHaloInset;
}
...
bool FocusRing::ShouldPaint()
{
    ...
    return has_focus_predicate_ ? has_focus_predicate_.Run(parent()) : parent()->HasFocus();
}

SkRRect FocusRing::RingRectFromPathRect(const SkRect& rect) const
{
    const double corner_radius = GetCornerRadius(halo_thickness_);
    return RingRectFromPathRect(SkRRect::MakeRectXY(rect, corner_radius, corner_radius));
}
...
SkRRect FocusRing::RingRectFromPathRect(const SkRRect& rrect) const
{
    ...
    SkRRect skr = rrect.makeOffset(r.x() - rrect.rect().x(), r.y() - rrect.rect().y());
