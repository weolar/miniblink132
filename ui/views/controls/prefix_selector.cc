...
void PrefixSelector::SetCompositionText(const ui::CompositionText& composition)
{
}
...
void PrefixSelector::InsertText(const std::u16string& text, InsertTextCursorBehavior cursor_behavior)
{
...
#if BUILDFLAG(IS_WIN)
std::optional<gfx::Rect> PrefixSelector::GetProximateCharacterBounds(const gfx::Range& range) const
{
    NOTIMPLEMENTED_LOG_ONCE();
    return std::nullopt;
}

std::optional<size_t> PrefixSelector::GetProximateCharacterIndexFromPoint(const gfx::Point& point, ui::IndexFromPointFlags flags) const
{
    NOTIMPLEMENTED_LOG_ONCE();
    return std::nullopt;
}
...
bool PrefixSelector::GetCompositionCharacterBounds(size_t index, gfx::Rect* rect) const
{
...
bool PrefixSelector::GetTextFromRange(const gfx::Range& range, std::u16string* text) const
{
    return false;
}
...
bool PrefixSelector::ChangeTextDirectionAndLayoutAlignment(base::i18n::TextDirection direction)
{
    return true;
}
...
bool PrefixSelector::IsTextEditCommandEnabled(ui::TextEditCommand command) const
{
    return false;
}

void PrefixSelector::SetTextEditCommandForNextKeyEvent(ui::TextEditCommand command)
{
}
...
bool PrefixSelector::SetCompositionFromExistingText(const gfx::Range& range, const std::vector<ui::ImeTextSpan>& ui_ime_text_spans)
{
...
#if BUILDFLAG(IS_WIN)
void PrefixSelector::SetActiveCompositionForAccessibility(const gfx::Range& range, const std::u16string& active_composition_text, bool is_composition_committed)
{
}
#endif

#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_CHROMEOS)
void PrefixSelector::GetActiveTextInputControlLayoutBounds(std::optional<gfx::Rect>* control_bounds, std::optional<gfx::Rect>* selection_bounds)
{
}
#endif
...
bool PrefixSelector::TextAtRowMatchesText(size_t row, const std::u16string& lower_text)
{
    const std::u16string model_text(base::i18n::ToLower(prefix_delegate_->GetTextForRow(row)));
