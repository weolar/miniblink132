...
ScrollBarButton::ScrollBarButton(PressedCallback callback, Type type, const base::TickClock* tick_clock)
    : Button(std::move(callback))
    , type_(type)
    , repeater_(base::BindRepeating(&ScrollBarButton::RepeaterNotifyClick, base::Unretained(this)), tick_clock)
{
...
gfx::Size ScrollBarButton::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
    if (!GetWidget())
        return gfx::Size();
...
void ScrollBarButton::PaintButtonContents(gfx::Canvas* canvas)
{
    ...
    GetNativeTheme()->Paint(canvas->sk_canvas(), GetColorProvider(), GetNativeThemePart(), GetNativeThemeState(), bounds, GetNativeThemeParams());
}
...
void ScrollBarButton::RepeaterNotifyClick()
{
    ...
    ui::MouseEvent event(ui::EventType::kMouseReleased, cursor_point, cursor_point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    Button::NotifyClick(event);
}
