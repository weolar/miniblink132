...
gfx::Size ScrollBarThumb::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
    if (!GetWidget())
        return gfx::Size();
    return GetNativeTheme()->GetPartSize(GetNativeThemePart(), GetNativeThemeState(), GetNativeThemeParams());
}
...
void ScrollBarThumb::OnPaint(gfx::Canvas* canvas)
{
    ...
    GetNativeTheme()->Paint(canvas->sk_canvas(), GetColorProvider(), GetNativeThemePart(), theme_state, local_bounds, extra_params);
    const ui::NativeTheme::Part gripper_part = scroll_bar_->GetOrientation() == ScrollBar::Orientation::kHorizontal
        ? ui::NativeTheme::kScrollbarHorizontalGripper
        : ui::NativeTheme::kScrollbarVerticalGripper;
    GetNativeTheme()->Paint(canvas->sk_canvas(), GetColorProvider(), gripper_part, theme_state, local_bounds, extra_params);
}
    ...
    : ScrollBar(orientation)
{
    ...
    const auto scroll_func = [](ScrollBarViews* scrollbar, ScrollAmount amount) { scrollbar->ScrollByAmount(amount); };
    using Type = ScrollBarButton::Type;
    prev_button_ = AddChildView(std::make_unique<ScrollBarButton>(base::BindRepeating(scroll_func, base::Unretained(this), ScrollAmount::kPrevLine),
        orientation == ScrollBar::Orientation::kHorizontal ? Type::kLeft : Type::kUp));
    ...
    GetThumb()->SetProperty(views::kFlexBehaviorKey, views::FlexSpecification(views::MinimumFlexSizeRule::kPreferred, views::MaximumFlexSizeRule::kUnbounded));

    next_button_ = AddChildView(std::make_unique<ScrollBarButton>(base::BindRepeating(scroll_func, base::Unretained(this), ScrollBar::ScrollAmount::kNextLine),
        orientation == ScrollBar::Orientation::kHorizontal ? Type::kRight : Type::kDown));
    next_button_->set_context_menu_controller(this);
    part_ = orientation == ScrollBar::Orientation::kHorizontal ? ui::NativeTheme::kScrollbarHorizontalTrack : ui::NativeTheme::kScrollbarVerticalTrack;
}
...
int ScrollBarViews::GetVerticalScrollBarWidth(const ui::NativeTheme* theme)
{
    ...
    gfx::Size button_size = theme->GetPartSize(ui::NativeTheme::kScrollbarUpArrow, ui::NativeTheme::kNormal, ui::NativeTheme::ExtraParams(scrollbar_arrow));

    ui::NativeTheme::ScrollbarThumbExtraParams scrollbar_thumb;
    scrollbar_thumb.is_hovering = false;
    gfx::Size track_size
        = theme->GetPartSize(ui::NativeTheme::kScrollbarVerticalThumb, ui::NativeTheme::kNormal, ui::NativeTheme::ExtraParams(scrollbar_thumb));
...
void ScrollBarViews::OnPaint(gfx::Canvas* canvas)
{
    ...
    if (!upper_bounds.IsEmpty()) {
        GetNativeTheme()->Paint(canvas->sk_canvas(), GetColorProvider(), part_, state_, upper_bounds, params);
    }

    scrollbar_track.is_upper = false;
    if (GetOrientation() == ScrollBar::Orientation::kHorizontal) {
        bounds.Inset(gfx::Insets::TLBR(0, thumb->bounds().right() - bounds.x(), 0, 0));
    } else {
        bounds.Inset(gfx::Insets::TLBR(thumb->bounds().bottom() - bounds.y(), 0, 0, 0));
    }
    if (!bounds.IsEmpty()) {
        GetNativeTheme()->Paint(canvas->sk_canvas(), GetColorProvider(), part_, state_, bounds, params);
    }
}
...
int ScrollBarViews::GetThickness() const
{
    ...
    return GetOrientation() == ScrollBar::Orientation::kHorizontal ? size.height() : size.width();
}
