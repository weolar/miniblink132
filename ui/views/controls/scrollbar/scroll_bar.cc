...
void ScrollBar::ScrollToThumbPosition(int thumb_position, bool scroll_to_middle)
{
    SetContentsScrollOffset(CalculateContentsOffset(static_cast<float>(thumb_position), scroll_to_middle));
    ScrollContentsToOffset();
    SchedulePaint();
}
...
void ScrollBar::OnGestureEvent(ui::GestureEvent* event)
{
    ...
    // fling).
    if (scroll_animator_ && scroll_animator_->is_scrolling() && (event->type() != ui::EventType::kGestureEnd || event->details().touch_points() > 1)) {
    ...
    // ends incorrectly.
    if (event->type() == ui::EventType::kGestureEnd && scroll_status_ != ScrollStatus::kScrollInEnding && scroll_status_ != ScrollStatus::kScrollEnded) {
        scroll_status_ = ScrollStatus::kScrollEnded;
        controller()->OnScrollEnded();
    }
    ...
    if (event->type() == ui::EventType::kScrollFlingStart) {
        ...
        GetOrCreateScrollAnimator()->Start(GetOrientation() == Orientation::kHorizontal ? event->details().velocity_x() : 0.f,
            GetOrientation() == Orientation::kHorizontal ? 0.f : event->details().velocity_y());
        event->SetHandled();
    }
}
...
{
    return ScrollByContentsOffset(GetOrientation() == Orientation::kHorizontal ? dx : dy);
}
...
void ScrollBar::ShowContextMenuForViewImpl(View* source, const gfx::Point& p, ui::mojom::MenuSourceType source_type)
{
    Widget* widget = GetWidget();
    gfx::Rect widget_bounds = widget->GetWindowBoundsInScreen();
    gfx::Point temp_pt(p.x() - widget_bounds.x(), p.y() - widget_bounds.y());
    View::ConvertPointFromWidget(this, &temp_pt);
    context_menu_mouse_position_ = GetOrientation() == Orientation::kHorizontal ? temp_pt.x() : temp_pt.y();

    if (!menu_model_) {
        menu_model_ = std::make_unique<ui::SimpleMenuModel>(this);
        menu_model_->AddItemWithStringId(ScrollBarContextMenuCommand_ScrollHere, IDS_APP_SCROLLBAR_CXMENU_SCROLLHERE);
        menu_model_->AddSeparator(ui::NORMAL_SEPARATOR);
        menu_model_->AddItemWithStringId(ScrollBarContextMenuCommand_ScrollStart,
            GetOrientation() == Orientation::kHorizontal ? IDS_APP_SCROLLBAR_CXMENU_SCROLLLEFTEDGE : IDS_APP_SCROLLBAR_CXMENU_SCROLLHOME);
        menu_model_->AddItemWithStringId(ScrollBarContextMenuCommand_ScrollEnd,
            GetOrientation() == Orientation::kHorizontal ? IDS_APP_SCROLLBAR_CXMENU_SCROLLRIGHTEDGE : IDS_APP_SCROLLBAR_CXMENU_SCROLLEND);
        menu_model_->AddSeparator(ui::NORMAL_SEPARATOR);
        menu_model_->AddItemWithStringId(ScrollBarContextMenuCommand_ScrollPageUp, IDS_APP_SCROLLBAR_CXMENU_SCROLLPAGEUP);
        menu_model_->AddItemWithStringId(ScrollBarContextMenuCommand_ScrollPageDown, IDS_APP_SCROLLBAR_CXMENU_SCROLLPAGEDOWN);
        menu_model_->AddSeparator(ui::NORMAL_SEPARATOR);
        menu_model_->AddItemWithStringId(ScrollBarContextMenuCommand_ScrollPrev,
            GetOrientation() == Orientation::kHorizontal ? IDS_APP_SCROLLBAR_CXMENU_SCROLLLEFT : IDS_APP_SCROLLBAR_CXMENU_SCROLLUP);
    ...
    menu_runner_ = std::make_unique<MenuRunner>(menu_model_.get(), MenuRunner::HAS_MNEMONICS | views::MenuRunner::CONTEXT_MENU);
    menu_runner_->RunMenuAt(GetWidget(), nullptr, gfx::Rect(p, gfx::Size()), MenuAnchorPosition::kTopLeft, source_type);
}
...
void ScrollBar::Update(int viewport_size, int content_size, int contents_scroll_offset)
{
    ...
    float ratio = std::min<float>(1.0, static_cast<float>(viewport_size) / contents_size_);
...
ScrollBar::ScrollBar(Orientation orientation)
    ...
    , repeater_(base::BindRepeating(&ScrollBar::TrackClicked, base::Unretained(this)))
{
...
#if !BUILDFLAG(IS_MAC)
// static
base::RetainingOneShotTimer* ScrollBar::GetHideTimerForTesting(ScrollBar* scroll_bar)
{
    return nullptr;
}
#endif
...
int ScrollBar::GetTrackSize() const
{
    ...
    return GetOrientation() == Orientation::kHorizontal ? track_bounds.width() : track_bounds.height();
}
...
int ScrollBar::CalculateContentsOffset(float thumb_position, bool scroll_to_middle) const
{
    ...
    float result = (thumb_position * (contents_size_ - viewport_size_)) / (track_size - thumb_size);
    return base::ClampRound(result);
}

void ScrollBar::SetContentsScrollOffset(int contents_scroll_offset)
{
    contents_scroll_offset_ = std::clamp(contents_scroll_offset, GetMinPosition(), GetMaxPosition());
}

ScrollBar::ScrollAmount ScrollBar::DetermineScrollAmountByKeyCode(const ui::KeyboardCode& keycode) const
{
    // Reject arrows that don't match the scrollbar orientation.
    if (GetOrientation() == Orientation::kHorizontal ? (keycode == ui::VKEY_UP || keycode == ui::VKEY_DOWN)
                                                     : (keycode == ui::VKEY_LEFT || keycode == ui::VKEY_RIGHT)) {
        return ScrollAmount::kNone;
    }

    static const base::NoDestructor<base::flat_map<ui::KeyboardCode, ScrollAmount>> kMap({
