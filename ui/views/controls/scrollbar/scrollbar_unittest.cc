...
public:
    ...
    void SetUp() override
    {
        ...
        widget_->Show();
        auto* container = widget_->SetContentsView(std::make_unique<TestScrollbarContainer>());
...
protected:
    ScrollBar* scrollbar()
    {
        return static_cast<ScrollBar*>(widget_->GetContentsView()->GetViewByID(kScrollBarID));
    }
...
TEST_F(ScrollBarViewsTest, ScrollBarFitsToBottom)
{
    ...
    EXPECT_EQ((scrollbar()->GetTrackBounds().width() - scrollbar()->GetThumbLengthForTesting()) / 2, scrollbar()->GetPosition());

    // Scroll to the end of the document.
    scrollbar()->Update(100, 1999, 1899);
    EXPECT_EQ(scrollbar()->GetTrackBounds().width() - scrollbar()->GetThumbLengthForTesting(), scrollbar()->GetPosition());
}
...
TEST_F(ScrollBarViewsTest, ThumbFullLengthOfTrack)
{
    ...
    EXPECT_EQ(scrollbar()->GetTrackBounds().width(), scrollbar()->GetThumbLengthForTesting());
...
TEST_F(ScrollBarViewsTest, AccessibleRole)
{
    ...
    EXPECT_EQ(scrollbar()->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kScrollBar);

    data = ui::AXNodeData();
    scrollbar()->GetViewAccessibility().SetRole(ax::mojom::Role::kButton);
    scrollbar()->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.role, ax::mojom::Role::kButton);
    EXPECT_EQ(scrollbar()->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kButton);
}
...
TEST_F(ScrollBarViewsTest, RightClickOpensMenu)
{
    ...
    scrollbar()->ShowContextMenu(scrollbar()->GetBoundsInScreen().CenterPoint(), ui::mojom::MenuSourceType::kMouse);
...
TEST_F(ScrollBarViewsTest, TestPageScrollingByPress)
{
    ...
    EXPECT_EQ(0, scrollbar()->GetPosition());
    generator.MoveMouseTo(scrollbar()->GetThumb()->GetBoundsInScreen().right_center() + gfx::Vector2d(4, 0));
...
TEST_F(ScrollBarViewsTest, DragThumbScrollsContent)
{
    ui::test::EventGenerator generator(GetRootWindow(widget_.get()));
    EXPECT_EQ(0, scrollbar()->GetPosition());
    generator.MoveMouseTo(scrollbar()->GetThumb()->GetBoundsInScreen().CenterPoint());
...
TEST_F(ScrollBarViewsTest, FlingGestureScrollsView)
{
    ...
    ui::test::EventGenerator generator(GetRootWindow(widget_.get()));
    EXPECT_EQ(0, scrollbar()->GetPosition());
    const gfx::Point start_pos = widget_->GetContentsView()->GetBoundsInScreen().CenterPoint();
    const gfx::Point end_pos = start_pos + gfx::Vector2d(-100, 0);
    generator.GestureScrollSequence(
        start_pos, end_pos, generator.CalculateScrollDurationForFlingVelocity(start_pos, end_pos, kScrollVelocity, kNumScrollSteps), kNumScrollSteps);
