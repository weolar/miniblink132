...
class ImageViewTest : public ViewsTestBase, public ::testing::WithParamInterface<Axis> {
public:
    ...
    void SetUp() override
    {
        ...
        BoxLayout::Orientation orientation = GetParam() == Axis::kHorizontal ? BoxLayout::Orientation::kHorizontal : BoxLayout::Orientation::kVertical;
...
TEST_P(ImageViewTest, CenterAlignment)
{
    ...
    image_view()->SetBorder(CreateEmptyBorder(gfx::Insets::TLBR(kLeadingInset, kLeadingInset, kTrailingInset, kTrailingInset)));
    views::test::RunScheduledLayout(image_view());
...
TEST_P(ImageViewTest, SetAccessibleNameNotifiesAccessibilityEvent)
{
    ...
    EXPECT_EQ(test_tooltip_text, image_view()->GetViewAccessibility().GetCachedName());
    ui::AXNodeData data;
...
TEST_P(ImageViewTest, AccessibleNameFromTooltipText)
{
    ...
    ui::AXNodeData data;
    image_view()->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), std::u16string());
    EXPECT_EQ(image_view()->GetViewAccessibility().GetCachedName(), std::u16string());
    ...
    image_view()->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), tooltip_text);
    ...
    image_view()->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), accessible_name);
    EXPECT_EQ(image_view()->GetViewAccessibility().GetCachedName(), accessible_name);
    EXPECT_EQ(image_view()->GetTooltipText(), tooltip_text);
    EXPECT_EQ(data.role, ax::mojom::Role::kImage);
    EXPECT_FALSE(image_view()->GetViewAccessibility().GetIsIgnored());
...
INSTANTIATE_TEST_SUITE_P(All, ImageViewTest, ::testing::Values(Axis::kHorizontal, Axis::kVertical));
