...
// this exact angle.
template <class T> T CombineScrollOffsets(T x, T y)
{
...
public:
    ...
    void OnPaint(gfx::Canvas* canvas) override
    {
        ...
        ui::NativeTheme::ExtraParams params(absl::in_place_type<ui::NativeTheme::ScrollbarExtraParams>);
#else
        ui::NativeTheme::ExtraParams params(absl::in_place_type<ui::NativeTheme::ScrollbarTrackExtraParams>);
#endif
        GetNativeTheme()->Paint(canvas->sk_canvas(), GetColorProvider(), ui::NativeTheme::kScrollbarCorner, ui::NativeTheme::kNormal, GetLocalBounds(), params);
    }
};
...
{
    return base::ranges::any_of(view->children(), [](View* child) { return child->layer() || DoesDescendantHaveLayer(child); });
}
...
void ConstrainScrollToBounds(View* viewport, View* view, bool scroll_with_layers_enabled)
{
    if (!view) {
        return;
    }
    ...
    const bool scrolls_with_layers = scroll_with_layers_enabled && viewport->layer() != nullptr;
    if (scrolls_with_layers) {
        DCHECK(view->layer());
        DCHECK_EQ(0, view->x());
        DCHECK_EQ(0, view->y());
    }
    gfx::PointF offset = scrolls_with_layers ? view->layer()->CurrentScrollOffset() : gfx::PointF(-view->x(), -view->y());
...
public:
    explicit Viewport(ScrollView* scroll_view)
    ...
    void ScrollRectToVisible(const gfx::Rect& rect) override
    {
        ...
        // so fall back to it.
        if (!scroll_view_->IsHorizontalScrollEnabled() && !scroll_view_->IsVerticalScrollEnabled()) {
    ...
    void OnChildLayerChanged(View* child) override
    {
        ...
        DCHECK(!scroll_view_->scroll_with_layers_enabled_ || child != scroll_view_->contents_)
...
private:
    void UpdateContentsViewportLayer()
    {
        ...
        const ui::LayerType new_layer_type = CalculateLayerTypeForContentsViewport();
    ...
    ui::LayerType CalculateLayerTypeForContentsViewport() const
    {
        ...
        if (scroll_view_->scroll_with_layers_enabled_) {
            return scroll_view_->layer_type_ == ui::LAYER_TEXTURED ? ui::LAYER_NOT_DRAWN : ui::LAYER_TEXTURED;
        }
        ...
        const View* contents = !this->children().empty() ? this->children()[0] : nullptr;

        auto has_textured_layer { [](const View* contents) { return contents->layer() && contents->layer()->type() == ui::LAYER_TEXTURED; } };
...
ScrollView::ScrollView()
    : ScrollView(base::FeatureList::IsEnabled(::features::kUiCompositorScrollWithLayers) ? ScrollWithLayers::kEnabled : ScrollWithLayers::kDisabled)
{
}

ScrollView::ScrollView(ScrollWithLayers scroll_with_layers)
    : horiz_sb_(AddChildView(PlatformStyle::CreateScrollBar(ScrollBar::Orientation::kHorizontal)))
    , vert_sb_(AddChildView(PlatformStyle::CreateScrollBar(ScrollBar::Orientation::kVertical)))
    , corner_view_(std::make_unique<ScrollCornerView>())
    , scroll_with_layers_enabled_(scroll_with_layers == ScrollWithLayers::kEnabled)
{
...
{
    View* grandparent = contents->parent() ? contents->parent()->parent() : nullptr;
...
void ScrollView::SetContentsImpl(std::unique_ptr<View> a_view)
{
    ...
    if (a_view && ScrollsWithLayers()) {
        ...
        a_view->layer()->SetDidScrollCallback(base::BindRepeating(&ScrollView::OnLayerScrolled, base::Unretained(this)));
        a_view->layer()->SetScrollable(contents_viewport_->bounds().size());
    }
    contents_ = ReplaceChildView(contents_viewport_, contents_.ExtractAsDangling(), std::move(a_view));
    UpdateBackground();
}
...
{
    header_ = ReplaceChildView(header_viewport_, header_.ExtractAsDangling(), std::move(a_header));
}
...
void ScrollView::SetViewportRoundedCornerRadius(const gfx::RoundedCornersF& radii)
{
    DCHECK(contents_viewport_->layer()) << "Please ensure you have enabled ScrollWithLayers.";
...
void ScrollView::SetBackgroundThemeColorId(const std::optional<ui::ColorId>& color_id)
{
...
gfx::Rect ScrollView::GetVisibleRect() const
{
    ...
    return gfx::Rect(offset.x(), offset.y(), contents_viewport_->width(), contents_viewport_->height());
}

void ScrollView::SetHorizontalScrollBarMode(ScrollBarMode horizontal_scroll_bar_mode)
{
...
void ScrollView::SetVerticalScrollBarMode(ScrollBarMode vertical_scroll_bar_mode)
{
    ...
    DCHECK(!treat_all_scroll_events_as_horizontal_ || vertical_scroll_bar_mode == ScrollBarMode::kDisabled);
...
void ScrollView::SetTreatAllScrollEventsAsHorizontal(bool treat_all_scroll_events_as_horizontal)
{
    if (treat_all_scroll_events_as_horizontal_ == treat_all_scroll_events_as_horizontal) {
        return;
    }
    treat_all_scroll_events_as_horizontal_ = treat_all_scroll_events_as_horizontal;
    OnPropertyChanged(&treat_all_scroll_events_as_horizontal_, kPropertyEffectsNone);
...
View* ScrollView::SetCustomOverflowIndicator(OverflowIndicatorAlignment side, std::unique_ptr<View> indicator, int thickness, bool fills_opaquely)
{
    if (thickness < 0) {
        thickness = 0;
    }
...
ScrollBar* ScrollView::SetHorizontalScrollBar(std::unique_ptr<ScrollBar> horiz_sb)
{
...
ScrollBar* ScrollView::SetVerticalScrollBar(std::unique_ptr<ScrollBar> vert_sb)
{
    DCHECK(vert_sb);
...
base::CallbackListSubscription ScrollView::AddContentsScrolledCallback(ScrollViewCallback callback)
{
    return on_contents_scrolled_.Add(std::move(callback));
}

base::CallbackListSubscription ScrollView::AddContentsScrollEndedCallback(ScrollViewCallback callback)
{
    return on_contents_scroll_ended_.Add(std::move(callback));
}

gfx::Size ScrollView::CalculatePreferredSize(const SizeBounds& available_size) const
{
    gfx::Insets insets = GetInsets();
    gfx::Size size = contents_ ? contents_->GetPreferredSize(available_size.Inset(insets)) : gfx::Size();
...
void ScrollView::Layout(PassKey)
{
    ...
    if (horizontal_scroll_bar_mode_ == ScrollBarMode::kEnabled && vertical_scroll_bar_mode_ == ScrollBarMode::kEnabled) {
#if BUILDFLAG(IS_MAC)
    ...
    const int header_height = std::min(viewport_bounds.height(), header_ ? header_->GetPreferredSize({}).height() : 0);
    ...
    if (contents_) {
        ...
        ComputeScrollBarsVisibility(viewport_size, content_size, &horiz_sb_required, &vert_sb_required);
    }
    // Overlay scrollbars don't need a corner view.
    bool corner_view_required = horiz_sb_required && vert_sb_required && !vert_sb_->OverlapsContent();
    // Take action.
    ...
    if (horiz_sb_required) {
        gfx::Rect horiz_sb_bounds(contents_x, viewport_bounds.bottom(), viewport_bounds.right() - contents_x, horiz_sb_layout_height);
        if (horiz_sb_->OverlapsContent()) {
            horiz_sb_bounds.Inset(gfx::Insets::TLBR(-horiz_sb_->GetThickness(), 0, 0, vert_sb_required ? vert_sb_->GetThickness() : 0));
        }

        horiz_sb_->SetBoundsRect(horiz_sb_bounds);
    }
    if (vert_sb_required) {
        gfx::Rect vert_sb_bounds(viewport_bounds.right(), contents_y, vert_sb_layout_width, viewport_bounds.bottom() - contents_y);
        if (vert_sb_->OverlapsContent()) {
            // In the overlay scrollbar case, the scrollbar only covers the viewport
            // (and not the header).
            vert_sb_bounds.Inset(gfx::Insets::TLBR(header_height, -vert_sb_->GetThickness(), horiz_sb_required ? horiz_sb_->GetThickness() : 0, 0));
        }
    ...
    if (corner_view_required) {
        ...
        corner_view_->SetBounds(vert_sb_->bounds().x(), horiz_sb_->bounds().y(), vert_sb_layout_width, horiz_sb_layout_height);
    }
    ...
    ConstrainScrollToBounds(header_viewport_, header_, scroll_with_layers_enabled_);
    ConstrainScrollToBounds(contents_viewport_, contents_, scroll_with_layers_enabled_);
    SchedulePaint();
...
bool ScrollView::OnMouseWheel(const ui::MouseWheelEvent& e)
{
    ...
    const ui::MouseWheelEvent to_propagate
        = treat_all_scroll_events_as_horizontal_ ? ui::MouseWheelEvent(e, CombineScrollOffsets(e.x_offset(), e.y_offset()), 0) : e;
...
void ScrollView::OnScrollEvent(ui::ScrollEvent* event)
{
    ...
    // option.
    ui::ScrollEvent e = treat_all_scroll_events_as_horizontal_
        ? ui::ScrollEvent(event->type(), event->location_f(), event->root_location_f(), event->time_stamp(), event->flags(),
            CombineScrollOffsets(event->x_offset(), event->y_offset()), 0.0f, CombineScrollOffsets(event->y_offset_ordinal(), event->x_offset_ordinal()), 0.0f,
            event->finger_count(), event->momentum_phase(), event->scroll_event_phase())
        : *event;

    ui::ScrollInputHandler* compositor_scroller = GetWidget()->GetCompositor()->scroll_input_handler();
...
void ScrollView::OnGestureEvent(ui::GestureEvent* event)
{
    ...
    // the scrollbars.
    bool scroll_event = event->type() == ui::EventType::kGestureScrollUpdate || event->type() == ui::EventType::kGestureScrollBegin
        || event->type() == ui::EventType::kGestureScrollEnd || event->type() == ui::EventType::kScrollFlingStart;
    ...
    // TODO(crbug.com/40471184): Use composited scrolling.
    if (IsVerticalScrollEnabled() && (scroll_event || (vert_sb_->GetVisible() && vert_sb_->bounds().Contains(event->location())))) {
        vert_sb_->OnGestureEvent(event);
    }
    if (!event->handled() && IsHorizontalScrollEnabled() && (scroll_event || (horiz_sb_->GetVisible() && horiz_sb_->bounds().Contains(event->location())))) {
...
void ScrollView::ScrollToPosition(ScrollBar* source, int position)
{
    ...
    if (source == horiz_sb_ && IsHorizontalScrollEnabled()) {
        position = AdjustPosition(offset.x(), position, contents_->width(), contents_viewport_->width());
        if (offset.x() == position) {
            return;
        }
        offset.set_x(position);
    } else if (source == vert_sb_ && IsVerticalScrollEnabled()) {
        position = AdjustPosition(offset.y(), position, contents_->height(), contents_viewport_->height());
...
int ScrollView::GetScrollIncrement(ScrollBar* source, bool is_page, bool is_positive)
{
    bool is_horizontal = source->GetOrientation() == ScrollBar::Orientation::kHorizontal;
    if (is_page) {
...
View* ScrollView::ReplaceChildView(View* parent, raw_ptr<View>::DanglingType old_view, std::unique_ptr<View> new_view)
{
    if (old_view) {
...
void ScrollView::ScrollContentsRegionToBeVisible(const gfx::Rect& rect)
{
    ...
    const int max_x = std::min(contents_max_x, x + std::min(contents_region.width(), contents_viewport_->width()));
    const int max_y = std::min(contents_max_y, y + std::min(contents_region.height(), contents_viewport_->height()));
...
void ScrollView::ComputeScrollBarsVisibility(const gfx::Size& vp_size, const gfx::Size& content_size, bool* horiz_is_shown, bool* vert_is_shown) const
{
    const bool horizontal_enabled = horizontal_scroll_bar_mode_ == ScrollBarMode::kEnabled;
    const bool vertical_enabled = vertical_scroll_bar_mode_ == ScrollBarMode::kEnabled;
    if (!horizontal_enabled) {
        *horiz_is_shown = false;
        *vert_is_shown = vertical_enabled && content_size.height() > vp_size.height();
        return;
    }
    if (!vertical_enabled) {
        *vert_is_shown = false;
    ...
    if (content_size.width() <= vp_size.width() && content_size.height() <= vp_size.height()) {
        *horiz_is_shown = false;
        *vert_is_shown = false;
    } else if (content_size.width() <= vp_size.width() - GetScrollBarLayoutWidth()) {
        *horiz_is_shown = false;
        *vert_is_shown = true;
    } else if (content_size.height() <= vp_size.height() - GetScrollBarLayoutHeight()) {
        *horiz_is_shown = true;
        *vert_is_shown = false;
    } else {
        *horiz_is_shown = true;
...
{
    return ScrollsWithLayers() ? contents_->layer()->CurrentScrollOffset() : gfx::PointF(-contents_->x(), -contents_->y());
}
...
void ScrollView::ScrollByOffset(const gfx::PointF& offset)
{
    ...
    ScrollToOffset(gfx::PointF(current_offset.x() + offset.x(), current_offset.y() + offset.y()));
}
...
bool ScrollView::IsHorizontalScrollEnabled() const
{
    return horizontal_scroll_bar_mode_ == ScrollBarMode::kHiddenButEnabled
        || (horizontal_scroll_bar_mode_ == ScrollBarMode::kEnabled && horiz_sb_->GetVisible());
}

bool ScrollView::IsVerticalScrollEnabled() const
{
    return vertical_scroll_bar_mode_ == ScrollBarMode::kHiddenButEnabled || (vertical_scroll_bar_mode_ == ScrollBarMode::kEnabled && vert_sb_->GetVisible());
}
...
void ScrollView::OnLayerScrolled(const gfx::PointF& current_offset, const cc::ElementId&)
{
...
void ScrollView::UpdateBorder()
{
    ...
    SetBorder(
        CreateSolidBorder(1, GetColorProvider()->GetColor(draw_focus_indicator_ ? ui::kColorFocusableBorderFocused : ui::kColorFocusableBorderUnfocused)));
}
...
void ScrollView::UpdateBackground()
{
    ...
    auto create_background = [background_color]() { return background_color ? CreateSolidBackground(background_color.value()) : nullptr; };
...
{
    return background_color_id_ ? GetColorProvider()->GetColor(background_color_id_.value()) : background_color_;
}
...
void ScrollView::PositionOverflowIndicators()
{
    ...
    more_content_left_->SetBoundsRect(gfx::Rect(x, y, more_content_left_thickness_, h));
    more_content_top_->SetBoundsRect(gfx::Rect(x, y, w, more_content_top_thickness_));
    more_content_right_->SetBoundsRect(gfx::Rect(contents_bounds.right() - more_content_right_thickness_, y, more_content_right_thickness_, h));
    more_content_bottom_->SetBoundsRect(gfx::Rect(x, contents_bounds.bottom() - more_content_bottom_thickness_, w, more_content_bottom_thickness_));
}

void ScrollView::UpdateOverflowIndicatorVisibility(const gfx::PointF& offset)
{
    SetControlVisibility(
        more_content_top_.get(), !draw_border_ && !header_ && IsVerticalScrollEnabled() && offset.y() > vert_sb_->GetMinPosition() && draw_overflow_indicator_);
    SetControlVisibility(more_content_bottom_.get(),
        !draw_border_ && IsVerticalScrollEnabled() && !horiz_sb_->GetVisible() && offset.y() < vert_sb_->GetMaxPosition() && draw_overflow_indicator_);

    SetControlVisibility(
        more_content_left_.get(), !draw_border_ && IsHorizontalScrollEnabled() && offset.x() > horiz_sb_->GetMinPosition() && draw_overflow_indicator_);
    SetControlVisibility(more_content_right_.get(),
        !draw_border_ && IsHorizontalScrollEnabled() && !vert_sb_->GetVisible() && offset.x() < horiz_sb_->GetMaxPosition() && draw_overflow_indicator_);
}
...
VariableRowHeightScrollHelper::VariableRowHeightScrollHelper(Controller* controller)
...
int VariableRowHeightScrollHelper::GetPageScrollIncrement(ScrollView* scroll_view, bool is_horizontal, bool is_positive)
{
    if (is_horizontal) {
        return 0;
    }
    ...
    if (is_positive) {
        ...
        int bottom = std::min(scroll_view->contents()->height() - 1, y + vis_height);
...
int VariableRowHeightScrollHelper::GetLineScrollIncrement(ScrollView* scroll_view, bool is_horizontal, bool is_positive)
{
    if (is_horizontal) {
        return 0;
    }
...
VariableRowHeightScrollHelper::RowInfo VariableRowHeightScrollHelper::GetRowInfo(int y)
{
...
FixedRowHeightScrollHelper::FixedRowHeightScrollHelper(int top_margin, int row_height)
...
VariableRowHeightScrollHelper::RowInfo FixedRowHeightScrollHelper::GetRowInfo(int y)
{
    if (y < top_margin_) {
        return RowInfo(0, top_margin_);
    }
    return RowInfo((y - top_margin_) / row_height_ * row_height_ + top_margin_, row_height_);
}
