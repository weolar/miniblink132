...
bool UnhandledKeyboardEventHandler::HandleKeyboardEvent(const input::NativeWebKeyboardEvent& event, FocusManager* focus_manager)
{
    CHECK(focus_manager);
    ...
    if (event.GetType() == blink::WebInputEvent::Type::kChar && ignore_next_char_event_) {
    ...
    if (event.GetType() == blink::WebInputEvent::Type::kKeyUp) {
        const ui::Accelerator accelerator = ui::GetAcceleratorFromNativeWebKeyboardEvent(event);
        if (focus_manager->ProcessAccelerator(accelerator)) {
            return true;
        }
    }
