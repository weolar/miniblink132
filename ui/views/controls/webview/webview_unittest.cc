...
public:
    ...
    WebViewTestWebContentsObserver(const WebViewTestWebContentsObserver&) = delete;
    WebViewTestWebContentsObserver& operator=(const WebViewTestWebContentsObserver&) = delete;
...
#if defined(USE_AURA)
            valid_root_while_shown_ = web_contents()->GetNativeView()->GetRootWindow() != nullptr;
#endif
            was_shown_ = true;
            ++shown_count_;
            break;
    ...
    bool valid_root_while_shown() const
    {
        return valid_root_while_shown_;
    }
...
public:
    ...
    WebViewTestWebContentsDelegate(const WebViewTestWebContentsDelegate&) = delete;
    WebViewTestWebContentsDelegate& operator=(const WebViewTestWebContentsDelegate&) = delete;

    ~WebViewTestWebContentsDelegate() override = default;

    void set_is_fullscreened(bool fs)
    {
        is_fullscreened_ = fs;
    }

    // content::WebContentsDelegate overrides.
    bool IsFullscreenForTabOrPending(const content::WebContents* ignored) override
    {
...
void SimulateRendererCrash(content::WebContents* contents, WebView* view)
{
    ...
    static_cast<content::WebContentsObserver*>(view)->RenderFrameDeleted(contents->GetPrimaryMainFrame());
}
...
public:
    ...
    WebViewUnitTest()
        : views::test::WidgetTest(std::unique_ptr<base::test::TaskEnvironment>(std::make_unique<content::BrowserTaskEnvironment>()))
    {
    }
    ...
    std::unique_ptr<content::WebContents> CreateWebContentsForWebView(content::BrowserContext* browser_context)
    {
        return content::WebContentsTester::CreateTestWebContents(browser_context, nullptr);
    }
    ...
    void SetUp() override
    {
        ...
        views::WebView::WebContentsCreator creator = base::BindRepeating(&WebViewUnitTest::CreateWebContentsForWebView, base::Unretained(this));
        scoped_web_contents_creator_ = std::make_unique<views::WebView::ScopedWebContentsCreatorForTesting>(creator);
        ...
        base::CommandLine::ForCurrentProcess()->AppendSwitch(switches::kDisableBackgroundingOccludedWindowsForTesting);
        ...
        View* const contents_view = top_level_widget_->SetContentsView(std::make_unique<View>());
    ...
    void TearDown() override
    {
        ...
        top_level_widget_.ExtractAsDangling()->Close(); // Deletes all children and itself.
        RunPendingMessages();
...
protected:
    ...
    WebView* web_view()
    {
        return static_cast<WebView*>(top_level_widget()->GetContentsView()->GetViewByID(kWebViewID));
    }
    NativeViewHost* holder()
    {
        return web_view()->holder_;
    }

    std::unique_ptr<content::WebContents> CreateWebContents() const
    {
        return content::WebContents::Create(content::WebContents::CreateParams(browser_context_.get()));
    }

    std::unique_ptr<content::WebContents> CreateTestWebContents() const
    {
        return content::WebContentsTester::CreateTestWebContents(browser_context_.get(), /*instance=*/nullptr);
    }
...
private:
    ...
    std::unique_ptr<views::WebView::ScopedWebContentsCreatorForTesting> scoped_web_contents_creator_;
...
TEST_F(WebViewUnitTest, TestWebViewAttachDetachWebContents)
{
    ...
    const std::unique_ptr<content::WebContents> web_contents1(CreateWebContents());
    ...
    // the new one.
    const std::unique_ptr<content::WebContents> web_contents2(CreateWebContents());
...
TEST_F(WebViewUnitTest, DetachedWebViewDestructor)
{
    ...
    auto* web_view = contents_view->AddChildView(std::make_unique<WebView>(web_contents->GetBrowserContext()));
...
{
    const std::unique_ptr<content::WebContents> web_contents = CreateTestWebContents();
...
TEST_F(WebViewUnitTest, ReparentingUpdatesParentAccessible)
{
    ...
    EXPECT_EQ(added_web_view->parent()->GetNativeViewAccessible(), added_web_view->holder()->GetParentAccessible());
    ...
    std::unique_ptr<WebView> removed_view = contents_view_1->RemoveChildViewT(added_web_view);
