...
void UpdateNativeViewHostAccessibleParent(NativeViewHost* holder, View* parent)
{
    if (!parent) {
        return;
    }
...
WebView::ScopedWebContentsCreatorForTesting::ScopedWebContentsCreatorForTesting(WebContentsCreator creator)
{
...
void WebView::SetWebContents(content::WebContents* replacement)
{
    ...
    if (replacement) {
        ...
        replacement->SetUserData(kIsWebViewContentsKey, std::make_unique<base::SupportsUserData::Data>());
    }
...
void WebView::LoadInitialURL(const GURL& url, HttpsUpgradePolicy https_upgrade_policy, base::Location invoke_location)
{
    content::NavigationController::LoadURLParams params(url);
    params.referrer = content::Referrer();
    params.transition_type = ui::PAGE_TRANSITION_AUTO_TOPLEVEL;
    params.force_no_https_upgrade = https_upgrade_policy == HttpsUpgradePolicy::kNoUpgrade;
...
void WebView::EnableSizingFromWebContents(const gfx::Size& min_size, const gfx::Size& max_size)
{
    DCHECK(!max_size.IsEmpty());
    min_size_ = min_size;
    max_size_ = max_size;
    if (web_contents() && web_contents()->GetPrimaryMainFrame()->IsRenderFrameLive()) {
...
base::CallbackListSubscription WebView::AddWebContentsAttachedCallback(WebContentsAttachedCallback callback)
{
...
void WebView::OnBoundsChanged(const gfx::Rect& previous_bounds)
{
    ...
    if (!web_contents() || !web_contents()->IsBeingCaptured() || web_contents()->GetPreferredSize().IsEmpty()
        || !(web_contents()->GetDelegate() && web_contents()->GetDelegate()->IsFullscreenForTabOrPending(web_contents()))) {
    ...
    if (y < x) {
        holder_bounds.ClampToCenteredSize(gfx::Size(holder_bounds.width(), static_cast<int>(y / capture_size.width())));
    } else {
        holder_bounds.ClampToCenteredSize(gfx::Size(static_cast<int>(x / capture_size.height()), holder_bounds.height()));
    }
...
gfx::NativeViewAccessible WebView::GetNativeViewAccessible()
{
    if (web_contents() && !web_contents()->IsCrashed()) {
        ...
        if (host_view) {
            gfx::NativeViewAccessible accessible = host_view->GetNativeViewAccessible();
            // |accessible| needs to know whether this is the primary WebContents.
            if (is_primary_web_contents_for_window_) {
                if (auto* ax_platform_node = ui::AXPlatformNode::FromNativeViewAccessible(accessible)) {
...
void WebView::RenderFrameHostChanged(content::RenderFrameHost* old_host, content::RenderFrameHost* new_host)
{
...
void WebView::DidToggleFullscreenModeForTab(bool entered_fullscreen, bool will_cause_resize)
{
...
void WebView::OnWebContentsFocused(content::RenderWidgetHost* render_widget_host)
{
    RequestFocus();
}
...
void WebView::ResizeDueToAutoResize(content::WebContents* source, const gfx::Size& new_size)
{
...
void WebView::AttachWebContentsNativeView()
{
    ...
    const auto* bg_color = WebContentsSetBackgroundColor::FromWebContents(web_contents());
    if (bg_color) {
...
void WebView::UpdateCrashedOverlayView()
{
    ...
    SetFocusBehavior(web_contents() ? FocusBehavior::ALWAYS : FocusBehavior::NEVER);
...
void WebView::NotifyAccessibilityWebContentsChanged()
{
    if (!lock_child_ax_tree_id_override_) {
        content::RenderFrameHost* rfh = web_contents() ? web_contents()->GetPrimaryMainFrame() : nullptr;
        GetViewAccessibility().SetChildTreeID(rfh ? rfh->GetAXTreeID() : ui::AXTreeIDUnknown());
    }
    NotifyAccessibilityEvent(ax::mojom::Event::kChildrenChanged, false);
}

std::unique_ptr<content::WebContents> WebView::CreateWebContents(content::BrowserContext* browser_context, base::Location creator_location)
{
    ...
    if (!contents) {
        content::WebContents::CreateParams create_params(browser_context, creator_location);
