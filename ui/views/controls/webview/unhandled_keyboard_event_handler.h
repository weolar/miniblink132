...
public:
    ...
    UnhandledKeyboardEventHandler& operator=(const UnhandledKeyboardEventHandler&) = delete;

    ~UnhandledKeyboardEventHandler();

    bool HandleKeyboardEvent(const input::NativeWebKeyboardEvent& event, FocusManager* focus_manager);

private:
    // Platform specific handling for unhandled keyboard events.
    static bool HandleNativeKeyboardEvent(const input::NativeWebKeyboardEvent& event, FocusManager* focus_manager);
