...
class TestWebDialogViewWebDialogDelegate : public ui::test::TestWebDialogDelegate {
public:
...
public:
    ...
    void SetUp() override
    {
        ...
        web_dialog_delegate_ = std::make_unique<TestWebDialogViewWebDialogDelegate>();
        web_contents_ = CreateWebContents();
        web_dialog_view_
            = new WebDialogView(web_contents_->GetBrowserContext(), web_dialog_delegate_.get(), std::make_unique<ui::test::TestWebContentsHandler>());
        ...
        widget_ = views::DialogDelegate::CreateDialogWidget(web_dialog_view_, GetContext(), nullptr);
        widget_->Show();
    ...
    bool widget_is_closed()
    {
        return widget_->IsClosed();
    }

    WebDialogView* web_dialog_view()
    {
        return web_dialog_view_;
    }
    ...
    void ResetWebDialogDelegate()
    {
        web_dialog_delegate_.reset();
    }

protected:
    std::unique_ptr<content::TestWebContents> CreateWebContents() const
    {
        return base::WrapUnique<content::TestWebContents>(content::TestWebContents::Create(content::WebContents::CreateParams(browser_context_.get())));
    }
...
TEST_F(WebDialogViewUnitTest, WebDialogViewClosedOnEscape)
{
    ...
    const ui::KeyEvent escape_event(ui::EventType::kKeyPressed, ui::VKEY_ESCAPE, ui::EF_NONE);
