...
ObservableWebView::ObservableWebView(content::BrowserContext* browser_context, WebDialogDelegate* delegate)
...
void ObservableWebView::DidFinishLoad(content::RenderFrameHost* render_frame_host, const GURL& validated_url)
{
...
WebDialogView::WebDialogView(
    content::BrowserContext* context, WebDialogDelegate* delegate, std::unique_ptr<WebContentsHandler> handler, content::WebContents* web_contents)
    ...
    , web_view_(new ObservableWebView(context, delegate))
{
    ...
    if (delegate_) {
        ...
        RegisterWindowWillCloseCallback(base::BindOnce(&WebDialogView::NotifyDialogWillClose, base::Unretained(this)));
    }

    if (web_contents) {
        web_view_->SetWebContents(web_contents);
    }
    web_view_->SetProperty(views::kElementIdentifierKey, delegate_ ? delegate_->web_view_element_id() : ui::ElementIdentifier());
}
...
{
    gfx::RoundedCornersF corner_radii(GetWebDialogFrameKind() == WebDialogDelegate::FrameKind::kDialog ? GetCornerRadius() : 0);

    SetWebViewCornersRadii(corner_radii);
}

gfx::Size WebDialogView::CalculatePreferredSize(const SizeBounds& available_size) const
{
    gfx::Size out;
    if (delegate_)
...
void WebDialogView::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details)
{
...
std::unique_ptr<NonClientFrameView> WebDialogView::CreateNonClientFrameView(Widget* widget)
{
    if (!delegate_)
...
void WebDialogView::GetWebUIMessageHandlers(std::vector<WebUIMessageHandler*>* handlers)
{
    if (delegate_)
...
void WebDialogView::OnCloseContents(WebContents* source, bool* out_close_dialog)
{
...
bool WebDialogView::HandleContextMenu(content::RenderFrameHost& render_frame_host, const content::ContextMenuParams& params)
{
    if (delegate_)
        return delegate_->HandleContextMenu(render_frame_host, params);
    return WebDialogWebContentsDelegate::HandleContextMenu(render_frame_host, params);
}
...
void WebDialogView::SetContentsBounds(WebContents* source, const gfx::Rect& bounds)
{
...
bool WebDialogView::HandleKeyboardEvent(content::WebContents* source, const input::NativeWebKeyboardEvent& event)
{
    if (!event.os_event) {
        return false;
    }

    return unhandled_keyboard_event_handler_.HandleKeyboardEvent(event, GetFocusManager());
}
...
content::WebContents* WebDialogView::OpenURLFromTab(
    content::WebContents* source, const content::OpenURLParams& params, base::OnceCallback<void(content::NavigationHandle&)> navigation_handle_callback)
{
    content::WebContents* new_contents = nullptr;
    auto split_navigation_handle_callback = base::SplitOnceCallback(std::move(navigation_handle_callback));
    if (delegate_ && delegate_->HandleOpenURLFromTab(source, params, std::move(split_navigation_handle_callback.first), &new_contents)) {
        return new_contents;
    }
    return WebDialogWebContentsDelegate::OpenURLFromTab(source, params, std::move(split_navigation_handle_callback.second));
}

content::WebContents* WebDialogView::AddNewContents(content::WebContents* source, std::unique_ptr<content::WebContents> new_contents, const GURL& target_url,
    WindowOpenDisposition disposition, const blink::mojom::WindowFeatures& window_features, bool user_gesture, bool* was_blocked)
{
    WebDialogWebContentsDelegate::AddNewContents(source, std::move(new_contents), target_url, disposition, window_features, user_gesture, was_blocked);
    return nullptr;
}

void WebDialogView::LoadingStateChanged(content::WebContents* source, bool should_show_loading_ui)
{
    if (delegate_)
        delegate_->OnLoadingStateChanged(source);
}

void WebDialogView::BeforeUnloadFired(content::WebContents* tab, bool proceed, bool* proceed_to_fire_unload)
{
    before_unload_fired_ = true;
    *proceed_to_fire_unload = proceed;
}

bool WebDialogView::IsWebContentsCreationOverridden(content::SiteInstance* source_site_instance, content::mojom::WindowContainerType window_container_type,
    const GURL& opener_url, const std::string& frame_name, const GURL& target_url)
{
    if (delegate_)
...
void WebDialogView::RequestMediaAccessPermission(
    content::WebContents* web_contents, const content::MediaStreamRequest& request, content::MediaResponseCallback callback)
{
    if (delegate_) {
        delegate_->RequestMediaAccessPermission(web_contents, request, std::move(callback));
    }
}

bool WebDialogView::CheckMediaAccessPermission(
    content::RenderFrameHost* render_frame_host, const url::Origin& security_origin, blink::mojom::MediaStreamType type)
{
    if (delegate_) {
        return delegate_->CheckMediaAccessPermission(render_frame_host, security_origin, type);
    }
    return false;
}
