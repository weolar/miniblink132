...
public:
    ...
    template <typename T> T* SetContents(std::unique_ptr<T> a_view)
    {
    ...
    template <typename T> T* SetHeader(std::unique_ptr<T> a_header)
    {
    ...
    void SetTreatAllScrollEventsAsHorizontal(bool treat_all_scroll_events_as_horizontal);

    // Gets/Sets whether the keyboard arrow keys attempt to scroll the view.
    bool GetAllowKeyboardScrolling() const
    {
        return allow_keyboard_scrolling_;
    }
    void SetAllowKeyboardScrolling(bool allow_keyboard_scrolling);

    bool GetDrawOverflowIndicator() const
    {
        return draw_overflow_indicator_;
    }
    void SetDrawOverflowIndicator(bool draw_overflow_indicator);

    View* SetCustomOverflowIndicator(OverflowIndicatorAlignment side, std::unique_ptr<View> indicator, int thickness, bool fills_opaquely);
    ...
    {
        return max_height_ >= 0 && min_height_ >= 0;
    }
    ...
    const ScrollBar* horizontal_scroll_bar() const
    {
        return horiz_sb_;
    }
    ...
    const ScrollBar* vertical_scroll_bar() const
    {
        return vert_sb_;
    }
    ...
    base::CallbackListSubscription AddContentsScrolledCallback(ScrollViewCallback callback);
    ...
    base::CallbackListSubscription AddContentsScrollEndedCallback(ScrollViewCallback callback);
    ...
    int GetScrollIncrement(ScrollBar* source, bool is_page, bool is_positive) override;
...
private:
    ...
    View* ReplaceChildView(View* parent, raw_ptr<View>::DanglingType old_view, std::unique_ptr<View> new_view);
    ...
    // and content sizes.
    void ComputeScrollBarsVisibility(const gfx::Size& viewport_size, const gfx::Size& content_size, bool* horiz_is_shown, bool* vert_is_shown) const;
...
public:
    ...
    VariableRowHeightScrollHelper& operator=(const VariableRowHeightScrollHelper&) = delete;
    ...
    // to.
    int GetPageScrollIncrement(ScrollView* scroll_view, bool is_horizontal, bool is_positive);
    int GetLineScrollIncrement(ScrollView* scroll_view, bool is_horizontal, bool is_positive);
...
public:
    ...
    FixedRowHeightScrollHelper& operator=(const FixedRowHeightScrollHelper&) = delete;
