...
class VIEWS_EXPORT TreeView : public View, public ui::TreeModelObserver, public TextfieldController, public FocusChangeListener, public PrefixDelegate {
...
public:
    ...
    static std::unique_ptr<ScrollView> CreateScrollViewWithTree(std::unique_ptr<TreeView> tree);
    ...
    {
        return const_cast<ui::TreeModelNode*>(const_cast<const TreeView*>(this)->GetSelectedNode());
    }
    ...
    {
        return const_cast<ui::TreeModelNode*>(const_cast<const TreeView*>(this)->GetActiveNode());
    }
    ...
    void ShowContextMenu(const gfx::Point& p, ui::mojom::MenuSourceType source_type) override;
    bool HandleAccessibleAction(const ui::AXActionData& action_data) override;

    // TreeModelObserver overrides:
    void TreeNodeAdded(ui::TreeModel* model, ui::TreeModelNode* parent, size_t index) override;
    void TreeNodeRemoved(ui::TreeModel* model, ui::TreeModelNode* parent, size_t index) override;
    void TreeNodeChanged(ui::TreeModel* model, ui::TreeModelNode* model_node) override;
...
private:
    ...
    void UpdateSelection(ui::TreeModelNode* model_node, SelectionType selection_type);
    ...
    public:
        ...
        AXVirtualView* accessibility_view() const
        {
            return accessibility_view_;
        }
        ...
        void set_loaded_children(bool value)
        {
            loaded_children_ = value;
        }
        bool loaded_children() const
        {
            return loaded_children_;
        }
    ...
    int root_row() const
    {
        return root_shown_ ? 0 : -1;
    }

    // Depth of the root node.
    int root_depth() const
    {
        return root_shown_ ? 0 : -1;
    }
    ...
    void UpdateAccessiblePositionalPropertiesForNodeAndChildren(InternalNode* node);
    ...
    // node.
    std::unique_ptr<AXVirtualView> CreateAndSetAccessibilityView(InternalNode* node);
    ...
    // of |*row|.
    void PaintRows(gfx::Canvas* canvas, int min_row, int max_row, InternalNode* node, int depth, int* row);
    ...
    void PaintExpandControl(gfx::Canvas* canvas, const gfx::Rect& node_bounds, bool expanded);

    // Paints the icon for the specified |node| in |bounds| to |canvas|.
    void PaintNodeIcon(gfx::Canvas* canvas, InternalNode* node, const gfx::Rect& bounds);

    // Returns the InternalNode for a model node. |create_type| indicates whether
    // this should load InternalNode or not.
    InternalNode* GetInternalNodeForModelNode(ui::TreeModelNode* model_node, CreateType create_type);
    ...
    gfx::Rect GetForegroundBoundsForNodeImpl(InternalNode* node, int row, int depth);
    ...
    InternalNode* GetNodeByRowImpl(InternalNode* node, int target_row, int current_depth, int* current_row, int* node_depth);
