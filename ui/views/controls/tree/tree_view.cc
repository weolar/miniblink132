...
void PaintRowIcon(gfx::Canvas* canvas, const gfx::ImageSkia& icon, int x, const gfx::Rect& rect)
{
    canvas->DrawImageInt(icon, rect.x() + x, rect.y() + (rect.height() - icon.height()) / 2);
}
...
{
    static const int kSpaceThicknessForFocusRing = FocusRing::kDefaultHaloThickness - FocusRing::kDefaultHaloInset;
    ...
    , drawing_provider_(std::make_unique<TreeViewDrawingProvider>())
{
    ...
    folder_icon_ = ui::ImageModel::FromVectorIcon(vector_icons::kFolderChromeRefreshIcon, ui::kColorIcon);

    text_offset_ = folder_icon_.Size().width() + kImagePadding + kImagePadding + kArrowRegionSize;
...
// static
std::unique_ptr<ScrollView> TreeView::CreateScrollViewWithTree(std::unique_ptr<TreeView> tree)
{
...
void TreeView::SetModel(TreeModel* model)
{
    ...
    if (model_) {
        ...
        std::unique_ptr<AXVirtualView> ax_root_view = CreateAndSetAccessibilityView(&root_);
...
void TreeView::StartEditing(TreeModelNode* node)
{
    ...
    editor_->GetViewAccessibility().SetName(selected_node_->model_node()->GetAccessibleTitle());
    LayoutEditor();
...
void TreeView::CommitEdit()
{
    ...
    editor_->GetViewAccessibility().SetName(GetSelectedNode()->GetAccessibleTitle());
...
void TreeView::Collapse(ui::TreeModelNode* model_node)
{
    ...
    if (model_node == root_.model_node() && !root_shown_)
        return;
    InternalNode* node = GetInternalNodeForModelNode(model_node, CreateType::kDontCreateIfNotLoaded);
    if (!node)
        return;
...
void TreeView::Expand(TreeModelNode* node)
{
    if (ExpandImpl(node)) {
        DrawnNodesChanged();
        InternalNode* internal_node = GetInternalNodeForModelNode(node, CreateType::kDontCreateIfNotLoaded);
        AXVirtualView* ax_view = internal_node ? internal_node->accessibility_view() : nullptr;
        if (ax_view) {
            ax_view->NotifyAccessibilityEvent(ax::mojom::Event::kExpandedChanged);
...
void TreeView::SetDrawingProvider(std::unique_ptr<TreeViewDrawingProvider> provider)
{
...
gfx::Size TreeView::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
...
void TreeView::ShowContextMenu(const gfx::Point& p, ui::mojom::MenuSourceType source_type)
{
    if (!model_)
        return;
...
bool TreeView::HandleAccessibleAction(const ui::AXActionData& action_data)
{
    ...
    InternalNode* node = ax_view ? GetInternalNodeForVirtualView(ax_view) : nullptr;
    if (!node) {
...
void TreeView::TreeNodeAdded(TreeModel* model, TreeModelNode* parent, size_t index)
{
    InternalNode* parent_node = GetInternalNodeForModelNode(parent, CreateType::kDontCreateIfNotLoaded);
...
void TreeView::TreeNodeRemoved(TreeModel* model, TreeModelNode* parent, size_t index)
{
    InternalNode* parent_node = GetInternalNodeForModelNode(parent, CreateType::kDontCreateIfNotLoaded);
    ...
    DCHECK(parent_node->accessibility_view()->Contains(child_removing->accessibility_view()));
    {
...
void TreeView::TreeNodeChanged(TreeModel* model, TreeModelNode* model_node)
{
    ...
    if (old_width != node->text_width() && ((node == &root_ && root_shown_) || (node != &root_ && IsExpanded(node->parent()->model_node())))) {
        node->accessibility_view()->NotifyAccessibilityEvent(ax::mojom::Event::kLocationChanged);
        DrawnNodesChanged();
    }
...
void TreeView::ContentsChanged(Textfield* sender, const std::u16string& new_contents)
{
}

bool TreeView::HandleKeyEvent(Textfield* sender, const ui::KeyEvent& key_event)
{
...
std::optional<size_t> TreeView::GetSelectedRow()
{
    ...
    return (row == -1) ? std::nullopt : std::make_optional(static_cast<size_t>(row));
}
...
void TreeView::SetSelectedRow(std::optional<size_t> row)
{
    ...
    SetSelectedNode(GetNodeForRow(row.has_value() ? static_cast<int>(row.value()) : -1));
}
...
bool TreeView::OnKeyPressed(const ui::KeyEvent& event)
{
    ...
    case ui::VKEY_F2:
        if (!editing_) {
            ...
            if (active_node && (!controller_ || controller_->CanEdit(this, active_node))) {
    ...
    case ui::VKEY_UP:
    case ui::VKEY_DOWN:
        IncrementSelection(event.key_code() == ui::VKEY_UP ? IncrementType::kPrevious : IncrementType::kNext);
        return true;
...
void TreeView::OnPaint(gfx::Canvas* canvas)
{
    ...
    int min_y, max_y;
    {
        ...
        if (canvas->sk_canvas()->getLocalClipBounds(&sk_clip_rect)) {
            ...
            gfx::Rect clip_rect = gfx::ToEnclosingRect(gfx::SkRectToRectF(sk_clip_rect));
            min_y = clip_rect.y();
...
void TreeView::UpdateSelection(TreeModelNode* model_node, SelectionType selection_type)
{
    CancelEdit();
    ...
    InternalNode* node = model_node ? GetInternalNodeForModelNode(model_node, CreateType::kCreateIfNotLoaded) : nullptr;
    ...
    bool selection_changed = (selection_type == SelectionType::kActiveAndSelected && (!selected_node_ || selected_node_ != node));
    ...
    if (active_changed) {
        ...
        GetViewAccessibility().OverrideFocus(node ? node->accessibility_view() : nullptr);
    }

    if (selection_changed) {
        AXVirtualView* ax_selected_view = node ? node->accessibility_view() : nullptr;
        if (ax_selected_view)
...
void TreeView::UpdateAccessiblePositionalProperties(InternalNode* node)
{
    ...
    if (IsRoot(node)) {
        ...
        if (depth >= 0) {
            row = 1;
            node_data.AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, int32_t { depth + 1 });
        ...
        size_t sibling_size = model_->GetChildren(node->parent()->model_node()).size();
        node_data.AddIntAttribute(ax::mojom::IntAttribute::kPosInSet, static_cast<int32_t>(pos_in_parent));
        node_data.AddIntAttribute(ax::mojom::IntAttribute::kSetSize, static_cast<int32_t>(sibling_size));
    }

    int ignored_depth;
    const bool is_visible_or_offscreen = row >= 0 && GetNodeByRow(row, &ignored_depth) == node;
    if (is_visible_or_offscreen) {
        ...
        node_data.AddState(node != &root_ || root_shown_ ? ax::mojom::State::kInvisible : ax::mojom::State::kIgnored);
    }
}

void TreeView::UpdateAccessiblePositionalPropertiesForNodeAndChildren(InternalNode* node)
{
...
void TreeView::ConfigureInternalNode(TreeModelNode* model_node, InternalNode* node)
{
...
void TreeView::UpdateNodeTextWidth(InternalNode* node)
{
    ...
    gfx::Canvas::SizeStringInt(node->model_node()->GetTitle(), font_list_, &width, &height, 0, gfx::Canvas::NO_ELLIPSIS);
    node->set_text_width(width);
}

std::unique_ptr<AXVirtualView> TreeView::CreateAndSetAccessibilityView(InternalNode* node)
{
    DCHECK(node);
...
void TreeView::SetAccessibleSelectionForNode(InternalNode* node, bool selected)
{
    if (!node) {
        return;
    }
...
void TreeView::UpdatePreferredSize()
{
    ...
    preferred_size_.SetSize(
        root_.GetMaxWidth(this, text_offset_, root_shown_ ? 1 : 0) + kTextHorizontalPadding * 2, row_height_ * base::checked_cast<int>(GetRowCount()));
    ...
    int vertical_space
        = std::max(0, (empty_editor_size_.height() - font_list_.GetHeight()) / 2 - kTextVerticalPadding) + GetSpaceThicknessForFocusRing() + row_height_;
...
void TreeView::LayoutEditor()
{
    ...
    row_bounds.Inset(gfx::Insets::VH(kTextVerticalPadding, kTextHorizontalPadding));
    row_bounds.Inset(gfx::Insets::VH(-(empty_editor_size_.height() - font_list_.GetHeight()) / 2, -empty_editor_size_.width() / 2));
    ...
    if (scroll_view) {
        ...
        row_bounds.set_size(gfx::Size(std::min(row_bounds.width(), content_bounds.width()), std::min(row_bounds.height(), content_bounds.height())));
    }
...
void TreeView::PaintRows(gfx::Canvas* canvas, int min_row, int max_row, InternalNode* node, int depth, int* row)
{
    if (*row >= max_row)
        return;
...
void TreeView::PaintRow(gfx::Canvas* canvas, InternalNode* node, int row, int depth)
{
    gfx::Rect bounds(GetForegroundBoundsForNodeImpl(node, row, depth));
    const SkColor selected_row_bg_color = drawing_provider()->GetBackgroundColorForNode(this, node->model_node());

    // Paint the row background.
    if (PlatformStyle::kTreeViewSelectionPaintsEntireRow && selected_node_ == node) {
    ...
    if (!PlatformStyle::kTreeViewSelectionPaintsEntireRow && node == selected_node_) {
        canvas->FillRect(text_bounds, selected_row_bg_color);
    }

    // Paint the auxiliary text.
    std::u16string aux_text = drawing_provider()->GetAuxiliaryTextForNode(this, node->model_node());
    if (!aux_text.empty()) {
        ...
        if (!aux_text_bounds.IsEmpty()) {
            int align = base::i18n::IsRTL() ? gfx::Canvas::TEXT_ALIGN_LEFT : gfx::Canvas::TEXT_ALIGN_RIGHT;
            canvas->DrawStringRectWithFlags(
                aux_text, font_list_, drawing_provider()->GetAuxiliaryTextColorForNode(this, node->model_node()), aux_text_bounds, align);
        }
    }

    // Paint the text.
    const gfx::Rect internal_bounds(text_bounds.x() + kTextHorizontalPadding, text_bounds.y() + kTextVerticalPadding,
        text_bounds.width() - kTextHorizontalPadding * 2, text_bounds.height() - kTextVerticalPadding * 2);
    canvas->DrawStringRect(node->model_node()->GetTitle(), font_list_, drawing_provider()->GetTextColorForNode(this, node->model_node()), internal_bounds);
}

void TreeView::PaintExpandControl(gfx::Canvas* canvas, const gfx::Rect& node_bounds, bool expanded)
{
    gfx::ImageSkia arrow
        = gfx::CreateVectorIcon(vector_icons::kSubmenuArrowIcon, color_utils::DeriveDefaultIconColor(drawing_provider()->GetTextColorForNode(this, nullptr)));
    if (expanded) {
        arrow = gfx::ImageSkiaOperations::CreateRotatedImage(
            arrow, base::i18n::IsRTL() ? SkBitmapOperations::ROTATION_270_CW : SkBitmapOperations::ROTATION_90_CW);
    }
    gfx::Rect arrow_bounds = node_bounds;
    arrow_bounds.Inset(gfx::Insets::VH((node_bounds.height() - arrow.height()) / 2, (kArrowRegionSize - arrow.width()) / 2));
    canvas->DrawImageInt(arrow, base::i18n::IsRTL() ? arrow_bounds.right() - arrow.width() : arrow_bounds.x(), arrow_bounds.y());
}

void TreeView::PaintNodeIcon(gfx::Canvas* canvas, InternalNode* node, const gfx::Rect& bounds)
{
    ...
    if (!icon_index.has_value()) {
        ...
        PaintRowIcon(canvas, folder_icon_.Rasterize(GetColorProvider()), icon_x, gfx::Rect(0, bounds.y(), bounds.width(), bounds.height()));
    } else {
        const gfx::ImageSkia& icon = icons_[icon_index.value()].Rasterize(GetColorProvider());
...
TreeView::InternalNode* TreeView::GetInternalNodeForModelNode(ui::TreeModelNode* model_node, CreateType create_type)
{
    ...
    size_t index = model_->GetIndexOf(parent_internal_node->model_node(), model_node).value();
    return parent_internal_node->children()[index].get();
}

TreeView::InternalNode* TreeView::GetInternalNodeForVirtualView(AXVirtualView* ax_view)
{
    if (ax_view == root_.accessibility_view())
        return &root_;
    DCHECK(ax_view);
    InternalNode* parent_internal_node = GetInternalNodeForVirtualView(ax_view->virtual_parent_view());
    if (!parent_internal_node)
        return nullptr;
...
{
    return PlatformStyle::kTreeViewSelectionPaintsEntireRow ? GetBoundsForNode(node) : GetForegroundBoundsForNode(node);
}
...
{
    gfx::Rect text_bounds = GetTextBoundsForNode(node);
    int width = base::i18n::IsRTL() ? text_bounds.x() - kTextHorizontalPadding - kAuxiliaryTextLineEndPadding
                                    : bounds().width() - text_bounds.right() - kTextHorizontalPadding - kAuxiliaryTextLineEndPadding;
    if (width < 0)
        return gfx::Rect();
    int x = base::i18n::IsRTL() ? kAuxiliaryTextLineEndPadding : bounds().right() - width - kAuxiliaryTextLineEndPadding;
    return gfx::Rect(x, text_bounds.y(), width, text_bounds.height());
}

gfx::Rect TreeView::GetForegroundBoundsForNodeImpl(InternalNode* node, int row, int depth)
{
    int width = drawing_provider()->ShouldDrawIconForNode(this, node->model_node()) ? text_offset_ + node->text_width() + kTextHorizontalPadding * 2
                                                                                    : kArrowRegionSize + node->text_width() + kTextHorizontalPadding * 2;

    gfx::Rect rect(depth * kIndent + kHorizontalInset, row * row_height_, width, row_height_);
    ...
    *depth = -1;
    ...
    while (tmp_node->parent()) {
        ...
        for (size_t i = 0; i < index_in_parent; ++i) {
            row += static_cast<int>(tmp_node->parent()->children()[i]->NumExpandedNodes());
        }
...
TreeView::InternalNode* TreeView::GetNodeByRowImpl(InternalNode* node, int target_row, int current_depth, int* current_row, int* node_depth)
{
    ...
    if (node->is_expanded()) {
        ...
        for (const auto& child : node->children()) {
            InternalNode* result = GetNodeByRowImpl(child.get(), target_row, current_depth, current_row, node_depth);
            if (result)
                return result;
        }
    }
...
void TreeView::IncrementSelection(IncrementType type)
{
    ...
    int new_row = std::clamp(row + delta, 0, base::checked_cast<int>(GetRowCount()) - 1);
    if (new_row == row)
...
bool TreeView::ExpandImpl(TreeModelNode* model_node)
{
    ...
    InternalNode* internal_node = GetInternalNodeForModelNode(model_node, CreateType::kCreateIfNotLoaded);
...
bool TreeView::IsPointInExpandControl(InternalNode* node, const gfx::Point& point)
{
    if (model_->GetChildren(node->model_node()).empty())
        return false;
    ...
    gfx::Rect arrow_bounds(arrow_dx, row * row_height_, kArrowRegionSize, row_height_);
    if (base::i18n::IsRTL())
...
{
    bool has_icon = tree->drawing_provider()->ShouldDrawIconForNode(tree, model_node());
    int max_width = (has_icon ? text_width_ : kArrowRegionSize) + indent * depth;
    if (!is_expanded_)
        return max_width;
    for (const auto& child : children()) {
        max_width = std::max(max_width, child->GetMaxWidth(tree, indent, depth + 1));
    }
    return max_width;
}
