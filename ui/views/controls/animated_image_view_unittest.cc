...
template <typename T> const T* FindPaintOp(const cc::PaintRecord& paint_record, cc::PaintOpType paint_op_type)
{
    for (const cc::PaintOp& op : paint_record) {
        ...
        if (op.GetType() == cc::PaintOpType::kDrawRecord) {
            const T* record_op_result = FindPaintOp<T>(static_cast<const cc::DrawRecordOp&>(op).record, paint_op_type);
            if (record_op_result)
...
protected:
    ...
    static constexpr gfx::Size kDefaultSize = gfx::Size(kDefaultWidthAndHeight, kDefaultWidthAndHeight);
    ...
    std::unique_ptr<lottie::Animation> CreateAnimationWithSize(const gfx::Size& size)
    {
        return std::make_unique<lottie::Animation>(cc::CreateSkottie(size, /*duration_secs=*/1));
    }
    ...
    cc::PaintRecord Paint(const gfx::Rect& invalidation_rect)
    {
        ...
        ui::PaintContext paint_context(display_list.get(),
            /*device_scale_factor=*/1.f, invalidation_rect, /*is_pixel_canvas=*/true);
        view()->Paint(PaintInfo::CreateRootPaintInfo(paint_context, invalidation_rect.size()));
        RunPendingMessages();
...
TEST_F(AnimatedImageViewTest, PaintsWithAdditionalTranslation)
{
    ...
    static constexpr float kExpectedDefaultOrigin = (kDefaultWidthAndHeight - 80) / 2;

    // Default should be no extra translation.
    cc::PaintRecord paint_record = Paint(view()->bounds());
    const cc::TranslateOp* translate_op = FindPaintOp<cc::TranslateOp>(paint_record, cc::PaintOpType::kTranslate);
    ...
    translate_op = FindPaintOp<cc::TranslateOp>(paint_record, cc::PaintOpType::kTranslate);
    ASSERT_THAT(translate_op, NotNull());
