...
Label* LabelAt(StyledLabel* styled, size_t index, std::string expected_classname = Label::kViewClassName)
{
...
protected:
    ...
    StyledLabel* InitStyledLabel(const std::string& ascii_text)
    {
        ...
        StyledLabel* styled = container->AddChildView(std::make_unique<StyledLabel>());
...
TEST_F(StyledLabelTest, NoWrapping)
{
    ...
    EXPECT_EQ(label_preferred_size.height(), StyledLabelContentHeightForWidth(styled, label_preferred_size.width() * 2));
}
...
TEST_F(StyledLabelTest, FirstLineNotEmptyWhenLeadingWhitespaceTooLong)
{
    ...
    EXPECT_EQ(label_preferred_size.height(), styled->GetHeightForWidth(label_preferred_size.width() / 2));
}
...
TEST_F(StyledLabelTest, BasicWrapping)
{
    ...
    gfx::Size label_preferred_size = label.GetPreferredSize({});
    EXPECT_EQ(label_preferred_size.height() * 2, StyledLabelContentHeightForWidth(styled, label_preferred_size.width()));

    // Also respect the border.
    styled->SetBorder(CreateEmptyBorder(3));
    styled->SetBounds(0, 0, styled->GetInsets().width() + label_preferred_size.width(), styled->GetInsets().height() + 2 * label_preferred_size.height());
    test::RunScheduledLayout(styled);
    ASSERT_EQ(2u, styled->children().size());
...
TEST_F(StyledLabelTest, AllowEmptyLines)
{
    ...
    ASSERT_EQ(2u, styled->children().size());
    EXPECT_EQ(styled->GetHeightForWidth(1000), styled->children()[1]->bounds().bottom());
}
...
TEST_F(StyledLabelTest, CreateLinks)
{
    ...
    styled->AddStyleRange(gfx::Range(0, 1), StyledLabel::RangeStyleInfo::CreateForLink(base::RepeatingClosure()));
    styled->AddStyleRange(gfx::Range(1, 2), StyledLabel::RangeStyleInfo::CreateForLink(base::RepeatingClosure()));
    styled->AddStyleRange(gfx::Range(10, 11), StyledLabel::RangeStyleInfo::CreateForLink(base::RepeatingClosure()));
    styled->AddStyleRange(gfx::Range(12, 13), StyledLabel::RangeStyleInfo::CreateForLink(base::RepeatingClosure()));
...
TEST_F(StyledLabelTest, StyledRangeCustomFontUnderlined)
{
    ...
    style_info.custom_font = styled->GetFontList().DeriveWithStyle(gfx::Font::UNDERLINE);
    styled->AddStyleRange(gfx::Range(text.size(), text.size() + underlined_text.size()), style_info);

    styled->SetBounds(0, 0, 1000, 1000);
    test::RunScheduledLayout(styled);

    ASSERT_EQ(2u, styled->children().size());
    EXPECT_EQ(gfx::Font::UNDERLINE, LabelAt(styled, 1)->font_list().GetFontStyle());
}

TEST_F(StyledLabelTest, StyledRangeTextStyleBold)
{
    test::TestLayoutProvider bold_provider;
    const std::string bold_text("This is a block of text whose style will be set to BOLD in the test");
    ...
    auto details = bold_provider.GetFontDetails(style::CONTEXT_LABEL, style::STYLE_DISABLED);
    details.weight = gfx::Font::Weight::BOLD;
    bold_provider.SetFontDetails(style::CONTEXT_LABEL, style::STYLE_DISABLED, details);
    ...
    label.SetFontList(label.font_list().DeriveWithWeight(gfx::Font::Weight::BOLD));
...
TEST_F(StyledLabelInWidgetTest, Color)
{
    ...
    StyledLabel::RangeStyleInfo style_info_link = StyledLabel::RangeStyleInfo::CreateForLink(base::RepeatingClosure());
    styled->AddStyleRange(gfx::Range(text_blue.size(), text_blue.size() + text_link.size()), style_info_link);
    ...
    Label* label = container->AddChildView(std::make_unique<Label>(ASCIIToUTF16(text)));
    const SkColor kDefaultTextColor = label->GetEnabledColor();

    // Obtain the default text color for a link.
    Link* link = container->AddChildView(std::make_unique<Link>(ASCIIToUTF16(text_link)));
    const SkColor kDefaultLinkColor = link->GetEnabledColor();

    ASSERT_EQ(3u, styled->children().size());
    EXPECT_EQ(SK_ColorBLUE, LabelAt(styled, 0)->GetEnabledColor());
    EXPECT_EQ(kDefaultLinkColor, LabelAt(styled, 1, LinkFragment::kViewClassName)->GetEnabledColor());
...
TEST_F(StyledLabelInWidgetTest, SetBackgroundColor)
{
    ...
    EXPECT_EQ(widget()->GetColorProvider()->GetColor(ui::kColorDialogBackground), LabelAt(styled, 0)->GetBackgroundColor());
    ...
    EXPECT_EQ(widget()->GetColorProvider()->GetColor(ui::kColorAlertHighSeverity), LabelAt(styled, 0)->GetBackgroundColor());
...
TEST_F(StyledLabelTest, StyledRangeWithTooltip)
{
    ...
    StyledLabel* styled = InitStyledLabel(text + tooltip_text + normal_text + link_text);
    StyledLabel::RangeStyleInfo tooltip_style;
    tooltip_style.tooltip = u"tooltip";
    styled->AddStyleRange(gfx::Range(tooltip_start, tooltip_start + tooltip_text.size()), tooltip_style);
    styled->AddStyleRange(gfx::Range(link_start, link_start + link_text.size()), StyledLabel::RangeStyleInfo::CreateForLink(base::RepeatingClosure()));

    // Break line inside the range with the tooltip.
    Label label(ASCIIToUTF16(text + tooltip_text.substr(0, tooltip_text.size() - 3)));
    gfx::Size label_preferred_size = label.GetPreferredSize({});
    int pref_height = styled->GetHeightForWidth(label_preferred_size.width());
    EXPECT_EQ(label_preferred_size.height() * 3, pref_height - styled->GetInsets().height());
    ...
    EXPECT_EQ(styled->children()[0]->bounds().right(), styled->children()[1]->x());
    EXPECT_EQ(styled->children()[2]->bounds().right(), styled->children()[3]->x());

    std::u16string tooltip = styled->children()[1]->GetTooltipText(gfx::Point(1, 1));
...
TEST_F(StyledLabelTest, SetTextContextAndDefaultStyle)
{
    ...
    Label label(ASCIIToUTF16(text), style::CONTEXT_DIALOG_TITLE, style::STYLE_DISABLED);
...
TEST_F(StyledLabelTest, LineHeightWithLink)
{
    ...
    styled->AddStyleRange(gfx::Range(0, 3), StyledLabel::RangeStyleInfo::CreateForLink(base::RepeatingClosure()));
    styled->AddStyleRange(gfx::Range(4, 7), StyledLabel::RangeStyleInfo::CreateForLink(base::RepeatingClosure()));
    styled->AddStyleRange(gfx::Range(8, 13), StyledLabel::RangeStyleInfo::CreateForLink(base::RepeatingClosure()));
    EXPECT_EQ(18 * 3, styled->GetHeightForWidth(100));
}
...
TEST_F(StyledLabelTest, CacheSize)
{
    ...
    const std::string text("This is a test block of text.");
    const std::u16string another_text(u"This is a test block of text. This text is much longer than previous");
    ...
    View* first_child_after_layout = styled->children().empty() ? nullptr : styled->children().front();
    ...
    test::RunScheduledLayout(styled);
    View* first_child_after_second_layout = styled->children().empty() ? nullptr : styled->children().front();
    ...
    View* first_child_after_text_update = styled->children().empty() ? nullptr : styled->children().front();
...
TEST_F(StyledLabelTest, Border)
{
    ...
    test::RunScheduledLayout(styled);
    EXPECT_EQ(label_preferred_size.height() + 5 /*top border*/ + 6 /*bottom border*/, styled->GetPreferredSize(SizeBounds(1000, {})).height());
    EXPECT_EQ(label_preferred_size.width() + 10 /*left border*/ + 20 /*right border*/, styled->GetPreferredSize(SizeBounds(1000, {})).width());
}
...
TEST_F(StyledLabelTest, LineHeightWithShorterCustomView)
{
    ...
    std::unique_ptr<View> custom_view = std::make_unique<StaticSizedView>(gfx::Size(20, default_height - less_height));
    StyledLabel::RangeStyleInfo style_info;
    style_info.custom_view = custom_view.get();
    styled = InitStyledLabel(text + custom_view_text);
    styled->AddStyleRange(gfx::Range(text.size(), text.size() + custom_view_text.size()), style_info);
...
TEST_F(StyledLabelTest, LineHeightWithTallerCustomView)
{
    ...
    std::unique_ptr<View> custom_view = std::make_unique<StaticSizedView>(gfx::Size(20, default_height + more_height));
    StyledLabel::RangeStyleInfo style_info;
    style_info.custom_view = custom_view.get();
    styled = InitStyledLabel(text + custom_view_text);
    styled->AddStyleRange(gfx::Range(text.size(), text.size() + custom_view_text.size()), style_info);
...
TEST_F(StyledLabelTest, LineWrapperWithCustomView)
{
    ...
    std::unique_ptr<View> custom_view = std::make_unique<StaticSizedView>(gfx::Size(200, custom_view_height));
    StyledLabel::RangeStyleInfo style_info;
    style_info.custom_view = custom_view.get();
    styled = InitStyledLabel(text_before + custom_view_text + text_after);
    styled->AddStyleRange(gfx::Range(text_before.size(), text_before.size() + custom_view_text.size()), style_info);
    styled->AddCustomView(std::move(custom_view));
    EXPECT_EQ(default_height * 2 + custom_view_height, styled->GetHeightForWidth(100));
}
...
TEST_F(StyledLabelTest, AlignmentInLTR)
{
    ...
    test::RunScheduledLayout(styled);
    Label label(ASCIIToUTF16(text));
    EXPECT_EQ((1000 - label.GetPreferredSize({}).width()) / 2, children.front()->bounds().x());
}
...
TEST_F(StyledLabelTest, AlignmentInRTL)
{
    ...
    base::CommandLine::ForCurrentProcess()->AppendSwitchASCII(switches::kForceUIDirection, switches::kForceDirectionRTL);
...
TEST_F(StyledLabelTest, ViewsCenteredWithLinkAndCustomView)
{
    ...
    styled->AddStyleRange(gfx::Range(text.size(), text.size() + link_text.size()), StyledLabel::RangeStyleInfo::CreateForLink(base::RepeatingClosure()));

    int custom_view_height = 25;
    std::unique_ptr<View> custom_view = std::make_unique<StaticSizedView>(gfx::Size(20, custom_view_height));
    StyledLabel::RangeStyleInfo style_info;
    style_info.custom_view = custom_view.get();
    styled->AddStyleRange(gfx::Range(text.size() + link_text.size(), text.size() + link_text.size() + custom_view_text.size()), style_info);
    styled->AddCustomView(std::move(custom_view));

    styled->SetBounds(0, 0, 1000, 500);
    test::RunScheduledLayout(styled);
    const int height = styled->GetPreferredSize(SizeBounds(styled->size())).height();
...
TEST_F(StyledLabelTest, ViewsCenteredForEvenAndOddSizes)
{
    ...
    for (int height : { 60, 61 }) {
        ...
        const auto view_heights = std::to_array<int>({ height, height / 2, height / 2 + 1 });
        for (uint32_t i = 0; i < view_heights.size(); ++i) {
            auto view = std::make_unique<StaticSizedView>(gfx::Size(kViewWidth, view_heights[i]));
            StyledLabel::RangeStyleInfo style_info;
...
TEST_F(StyledLabelTest, AccessibleNameAndRole)
{
    ...
    IgnoreMissingWidgetForTestingScopedSetter a11y_ignore_missing_widget_(styled->GetViewAccessibility());

    EXPECT_EQ(styled->GetViewAccessibility().GetCachedName(), base::UTF8ToUTF16(text));
    EXPECT_EQ(styled->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kStaticText);
    ...
    EXPECT_EQ(styled->GetViewAccessibility().GetCachedName(), base::UTF8ToUTF16(text));
    EXPECT_EQ(styled->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kTitleBar);
    ...
    EXPECT_EQ(styled->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kLink);
