...
public:
    ...
    explicit Link(const std::u16string& title = std::u16string(), int text_context = style::CONTEXT_LABEL, int text_style = style::STYLE_LINK);
    ...
    void SetCallback(base::RepeatingClosure callback)
    {
        ...
        callback_ = base::BindRepeating([](base::RepeatingClosure closure, const ui::Event& event) { closure.Run(); }, std::move(callback));
    }
