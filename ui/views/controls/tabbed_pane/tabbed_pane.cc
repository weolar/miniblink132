...
TabbedPaneTab::TabbedPaneTab(TabbedPane* tabbed_pane, const std::u16string& title, View* contents)
    : tabbed_pane_(tabbed_pane)
    , contents_(contents)
{
    // Calculate the size while the font list is bold.
    auto title_label = std::make_unique<Label>(title, style::CONTEXT_LABEL, style::STYLE_TAB_ACTIVE);
    ...
    if (tabbed_pane_->GetOrientation() == TabbedPane::Orientation::kVertical) {
        ...
        const bool is_highlight_style = tabbed_pane_->GetStyle() == TabbedPane::TabStripStyle::kHighlight;
        constexpr auto kTabPadding = gfx::Insets::VH(5, 10);
        constexpr auto kTabPaddingHighlight = gfx::Insets::TLBR(8, 32, 8, 0);
        SetBorder(CreateEmptyBorder(is_highlight_style ? kTabPaddingHighlight : kTabPadding));
    } else {
    ...
    title_text_changed_callback_ = title_->AddTextChangedCallback(base::BindRepeating(&TabbedPaneTab::UpdateAccessibleName, base::Unretained(this)));
}
...
#if BUILDFLAG(IS_MAC)
    SetFocusBehavior(selected ? FocusBehavior::ACCESSIBLE_ONLY : FocusBehavior::NEVER);
#else
...
gfx::Size TabbedPaneTab::CalculatePreferredSize(const SizeBounds& available_size) const
{
    int width = preferred_title_width_ + GetInsets().width();
    if (tabbed_pane_->GetStyle() == TabbedPane::TabStripStyle::kHighlight && tabbed_pane_->GetOrientation() == TabbedPane::Orientation::kVertical) {
        width = std::max(width, 192);
    }
    return gfx::Size(width, 32);
}

bool TabbedPaneTab::HandleAccessibleAction(const ui::AXActionData& action_data)
{
...
void TabbedPaneTab::OnFocus()
{
    ...
    if (tabbed_pane_->GetStyle() != TabbedPane::TabStripStyle::kHighlight) {
        ...
        SetBorder(CreatePaddedBorder(
            CreateSolidBorder(border_size, GetColorProvider()->GetColor(ui::kColorFocusableBorderFocused)), GetInsets() - gfx::Insets(border_size)));
    }
...
bool TabbedPaneTab::OnKeyPressed(const ui::KeyEvent& event)
{
    ...
    if (tabbed_pane_->GetOrientation() == TabbedPane::Orientation::kHorizontal) {
        ...
        return (key == ui::VKEY_LEFT || key == ui::VKEY_RIGHT) && tabbed_pane_->MoveSelectionBy(delta);
    }
    // Use up and down arrows to navigate tabs in vertical orientation.
    return (key == ui::VKEY_UP || key == ui::VKEY_DOWN) && tabbed_pane_->MoveSelectionBy(key == ui::VKEY_DOWN ? 1 : -1);
}
...
void TabbedPaneTab::OnStateChanged()
{
    ...
    if (tabbed_pane_->GetStyle() == TabbedPane::TabStripStyle::kHighlight) {
        ...
        gfx::Font::Weight font_weight = (state_ == State::kActive) ? font_weight = gfx::Font::Weight::BOLD : gfx::Font::Weight::MEDIUM;
        ui::ResourceBundle& rb = ui::ResourceBundle::GetSharedInstance();
        title_->SetFontList(rb.GetFontListForDetails(ui::ResourceBundle::FontDetails(std::string(), font_size_delta, font_weight)));
    } else {
...
void TabbedPaneTab::OnPaint(gfx::Canvas* canvas)
{
    ...
    if (!selected() || tabbed_pane_->GetOrientation() != TabbedPane::Orientation::kVertical
        || tabbed_pane_->GetStyle() != TabbedPane::TabStripStyle::kHighlight) {
        return;
    }
    constexpr SkScalar kRadius = SkIntToScalar(32);
    constexpr SkScalar kLTRRadii[8] = { 0, 0, kRadius, kRadius, kRadius, kRadius, 0, 0 };
    constexpr SkScalar kRTLRadii[8] = { kRadius, kRadius, 0, 0, 0, 0, kRadius, kRadius };
    SkPath path;
    path.addRoundRect(gfx::RectToSkRect(GetLocalBounds()), base::i18n::IsRTL() ? kRTLRadii : kLTRRadii);

    cc::PaintFlags fill_flags;
    fill_flags.setAntiAlias(true);
    fill_flags.setColor(GetColorProvider()->GetColor(HasFocus() ? ui::kColorTabBackgroundHighlightedFocused : ui::kColorTabBackgroundHighlighted));
...
void TabbedPaneTab::UpdatePreferredTitleWidth()
{
    ...
    SetState(State::kInactive);
    preferred_title_width_ = std::max(preferred_title_width_, title_->GetPreferredSize({}).width());
    SetState(old_state);
}
...
{
    DCHECK(GetWidget());
    const SkColor font_color = GetColorProvider()->GetColor(state_ == State::kActive ? ui::kColorTabForegroundSelected : ui::kColorTabForeground);
...
void TabbedPaneTab::UpdateAccessibleName()
{
    if (title_->GetText().empty()) {
        ...
        GetViewAccessibility().SetName(title_->GetText(), ax::mojom::NameFrom::kContents);
    }
...
TabbedPaneTabStrip::TabbedPaneTabStrip(TabbedPane::Orientation orientation, TabbedPane::TabStripStyle style)
    ...
    , style_(style)
{
    ...
    if (orientation == TabbedPane::Orientation::kHorizontal) {
        ...
        layout = std::make_unique<BoxLayout>(BoxLayout::Orientation::kVertical, kEdgePadding, kTabSpacing);
...
void TabbedPaneTabStrip::OnSelectedTabChanged(TabbedPaneTab* from_tab, TabbedPaneTab* to_tab, bool animate)
{
    ...
    if (GetOrientation() == TabbedPane::Orientation::kHorizontal) {
        animating_from_ = { from_tab->GetMirroredX(), from_tab->GetMirroredX() + from_tab->width() };
        animating_to_ = { to_tab->GetMirroredX(), to_tab->GetMirroredX() + to_tab->width() };
    } else {
        animating_from_ = { from_tab->bounds().y(), from_tab->bounds().y() + from_tab->height() };
        animating_to_ = { to_tab->bounds().y(), to_tab->bounds().y() + to_tab->height() };
    }
...
TabbedPaneTab* TabbedPaneTabStrip::GetTabAtDeltaFromSelected(int delta) const
{
    ...
    return GetTabAtIndex((selected_tab_index + static_cast<size_t>(delta)) % num_children);
}
...
void TabbedPaneTabStrip::OnPaintBorder(gfx::Canvas* canvas)
{
    ...
    const bool is_horizontal = GetOrientation() == TabbedPane::Orientation::kHorizontal;
    int max_cross_axis;
    gfx::Rect rect;
    constexpr int kUnselectedBorderThickness = 1;
    if (is_horizontal) {
        max_cross_axis = children().front()->bounds().bottom();
        rect = gfx::Rect(0, max_cross_axis - kUnselectedBorderThickness, width(), kUnselectedBorderThickness);
    } else {
        max_cross_axis = width();
        rect = gfx::Rect(max_cross_axis - kUnselectedBorderThickness, 0, kUnselectedBorderThickness, height());
    }
    canvas->FillRect(rect, GetColorProvider()->GetColor(ui::kColorTabContentSeparator));
    ...
    if (expand_animation_->is_animating()) {
        ...
        double anim_value = gfx::Tween::CalculateValue(gfx::Tween::FAST_OUT_LINEAR_IN, expand_animation_->GetCurrentValue());
        if (animating_leading) {
            min_main_axis = gfx::Tween::IntValueBetween(anim_value, animating_from_.start, animating_to_.start);
            max_main_axis = animating_from_.end;
        } else {
            min_main_axis = animating_from_.start;
            max_main_axis = gfx::Tween::IntValueBetween(anim_value, animating_from_.end, animating_to_.end);
        }
    } else if (contract_animation_->is_animating()) {
        bool animating_leading = animating_to_.start < animating_from_.start;
        double anim_value = gfx::Tween::CalculateValue(gfx::Tween::LINEAR_OUT_SLOW_IN, contract_animation_->GetCurrentValue());
        if (animating_leading) {
    ...
    rect = gfx::Rect(min_main_axis, max_cross_axis - kSelectedBorderThickness, max_main_axis - min_main_axis, kSelectedBorderThickness);
    if (!is_horizontal) {
        rect.Transpose();
    }
    canvas->FillRect(rect, GetColorProvider()->GetColor(ui::kColorTabBorderSelected));
}
...
TabbedPane::TabbedPane(TabbedPane::Orientation orientation, TabbedPane::TabStripStyle style, bool scrollable)
{
    DCHECK(orientation != TabbedPane::Orientation::kHorizontal || style != TabbedPane::TabStripStyle::kHighlight);
    if (orientation == TabbedPane::Orientation::kHorizontal)
        SetOrientation(views::LayoutOrientation::kVertical);

    auto tab_strip = std::make_unique<TabbedPaneTabStrip>(orientation, style);
    if (scrollable) {
        scroll_view_ = AddChildView(std::make_unique<ScrollView>(ScrollView::ScrollWithLayers::kEnabled));
    ...
    contents_->SetProperty(views::kFlexBehaviorKey, views::FlexSpecification(views::MinimumFlexSizeRule::kScaleToZero, views::MaximumFlexSizeRule::kUnbounded));
    ...
    AddAccelerator(ui::Accelerator(ui::VKEY_TAB, ui::EF_CONTROL_DOWN | ui::EF_SHIFT_DOWN));
...
void TabbedPane::AddTabInternal(size_t index, const std::u16string& title, std::unique_ptr<View> contents)
{
    DCHECK_LE(index, GetTabCount());
    contents->SetVisible(false);
    contents->GetViewAccessibility().SetRole(ax::mojom::Role::kTabPanel);
    if (!title.empty()) {
        contents->GetViewAccessibility().SetName(title, ax::mojom::NameFrom::kAttribute);
    }

    tab_strip_->AddChildViewAt(std::make_unique<TabbedPaneTab>(this, title, contents.get()), index);
...
void TabbedPane::SelectTab(TabbedPaneTab* new_selected_tab, bool animate)
{
    ...
    if (old_selected_tab) {
        ...
        tab_strip_->OnSelectedTabChanged(old_selected_tab, new_selected_tab, animate);
    ...
    if (focus_manager) {
        ...
        if (focused_view && contents_->Contains(focused_view) && !new_selected_tab->contents()->Contains(focused_view))
            focus_manager->SetFocusedView(new_selected_tab->contents());
    }

    if (listener()) {
        listener()->TabSelectedAt(base::checked_cast<int>(tab_strip_->GetIndexOf(new_selected_tab).value()));
    }
}
...
gfx::Size TabbedPane::CalculatePreferredSize(const SizeBounds& available_size) const
{
    ...
    const gfx::Size size = GetLayoutManager()->GetPreferredSize(this, available_size);
...
DEFINE_ENUM_CONVERTERS(
    views::TabbedPane::Orientation, { views::TabbedPane::Orientation::kHorizontal, u"HORIZONTAL" }, { views::TabbedPane::Orientation::kVertical, u"VERTICAL" })

DEFINE_ENUM_CONVERTERS(
    views::TabbedPane::TabStripStyle, { views::TabbedPane::TabStripStyle::kBorder, u"BORDER" }, { views::TabbedPane::TabStripStyle::kHighlight, u"HIGHLIGHT" })
