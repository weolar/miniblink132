...
TEST_F(TabbedPaneTest, HorizontalOrientationDefault)
{
    ...
    EXPECT_EQ(tabbed_pane->GetOrientation(), TabbedPane::Orientation::kHorizontal);
}

// Tests tab orientation.
TEST_F(TabbedPaneTest, VerticalOrientation)
{
    auto tabbed_pane = std::make_unique<TabbedPane>(TabbedPane::Orientation::kVertical, TabbedPane::TabStripStyle::kBorder);
...
{
    auto tabbed_pane = std::make_unique<TabbedPane>(TabbedPane::Orientation::kVertical, TabbedPane::TabStripStyle::kHighlight);
...
{
    auto tabbed_pane_vertical = std::make_unique<TabbedPane>(TabbedPane::Orientation::kVertical, TabbedPane::TabStripStyle::kBorder, true);
    ASSERT_NE(tabbed_pane_vertical->GetScrollView(), nullptr);
    EXPECT_THAT(tabbed_pane_vertical->GetScrollView(), testing::A<ScrollView*>());

    auto tabbed_pane_horizontal = std::make_unique<TabbedPane>(TabbedPane::Orientation::kHorizontal, TabbedPane::TabStripStyle::kBorder, true);
    ASSERT_NE(tabbed_pane_horizontal->GetScrollView(), nullptr);
    EXPECT_THAT(tabbed_pane_horizontal->GetScrollView(), testing::A<ScrollView*>());
}
...
{
    auto tabbed_pane = std::make_unique<TabbedPane>(TabbedPane::Orientation::kVertical, TabbedPane::TabStripStyle::kBorder);
    View* child1 = tabbed_pane->AddTab(u"tab1", std::make_unique<StaticSizedView>(gfx::Size(20, 10)));
    View* child2 = tabbed_pane->AddTab(u"tab2", std::make_unique<StaticSizedView>(gfx::Size(5, 5)));
...
protected:
    ...
    {
        return static_cast<TabbedPaneTab*>(tabbed_pane_->tab_strip_->children()[index]);
    }
    ...
    void SendKeyPressToSelectedTab(ui::KeyboardCode keyboard_code)
    {
        tabbed_pane_->GetSelectedTab()->OnKeyPressed(
            ui::KeyEvent(ui::EventType::kKeyPressed, keyboard_code, ui::UsLayoutKeyboardCodeToDomCode(keyboard_code), 0));
    }
...
{
    View* child1 = tabbed_pane_->AddTab(u"tab1", std::make_unique<StaticSizedView>(gfx::Size(20, 10)));
    View* child2 = tabbed_pane_->AddTab(u"tab2", std::make_unique<StaticSizedView>(gfx::Size(5, 5)));
    ...
    EXPECT_EQ(tabbed_pane_->GetPreferredSize({}).width(),
        tabbed_pane_->GetTabAt(0)->GetPreferredSize({}).width() + tabbed_pane_->GetTabAt(1)->GetPreferredSize({}).width());
    ...
    // child.
    View* child3 = tabbed_pane_->AddTab(u"tab3", std::make_unique<StaticSizedView>(gfx::Size(150, 5)));
...
TEST_F(TabbedPaneWithWidgetTest, AddAndSelect)
{
    ...
    View* tab0 = tabbed_pane_->AddTabAtIndex(0, u"tab0", std::make_unique<View>());
...
TEST_F(TabbedPaneWithWidgetTest, SelectTabWithAccessibleAction)
{
    ...
    for (size_t i = 0; i < kNumTabs; ++i) {
        ...
        EXPECT_EQ(DefaultTabTitle(), data.GetString16Attribute(ax::mojom::StringAttribute::kName));
        EXPECT_EQ(i == 0, data.GetBoolAttribute(ax::mojom::BoolAttribute::kSelected));
    }
...
TEST_F(TabbedPaneWithWidgetTest, AccessiblePaneContentsTitleTracksTabTitle)
{
    const std::u16string kFirstTitle = u"Tab1";
    const std::u16string kSecondTitle = u"Tab2";
    View* const tab1_contents = tabbed_pane_->AddTab(kFirstTitle, std::make_unique<View>());
    View* const tab2_contents = tabbed_pane_->AddTab(kSecondTitle, std::make_unique<View>());
...
TEST_F(TabbedPaneWithWidgetTest, AccessibleEvents)
{
    ...
    tabbed_pane_->SelectTabAt(1);
    EXPECT_EQ(1u, tabbed_pane_->GetSelectedTabIndex());
    EXPECT_EQ(1, counter.GetCount(ax::mojom::Event::kSelection, ax::mojom::Role::kTab));
    EXPECT_EQ(1, counter.GetCount(ax::mojom::Event::kSelectedChildrenChanged, ax::mojom::Role::kTabList));
    ...
    EXPECT_EQ(1, counter.GetCount(ax::mojom::Event::kFocus, ax::mojom::Role::kTab));
...
TEST_F(TabbedPaneWithWidgetTest, AccessibleNameTest)
{
    ...
    EXPECT_EQ(u"Tab1", data.GetString16Attribute(ax::mojom::StringAttribute::kName));
    EXPECT_EQ(ax::mojom::NameFrom::kContents, data.GetNameFrom());

    GetTabAt(0)->SetTitleText(u"Updated Tab1");
    data = ui::AXNodeData();
    GetTabAt(0)->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(u"Updated Tab1", data.GetString16Attribute(ax::mojom::StringAttribute::kName));
    EXPECT_EQ(ax::mojom::NameFrom::kContents, data.GetNameFrom());
...
TEST_F(TabbedPaneWithWidgetTest, AccessibleNameWithMultipleTabsTest)
{
    ...
    EXPECT_EQ(u"Tab1", tabbed_pane_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
    ...
    GetTabAt(0)->GetViewAccessibility().GetAccessibleNodeData(&tab_data);
    tabbed_pane_->GetViewAccessibility().GetAccessibleNodeData(&tabbed_pane_data);
    EXPECT_EQ(u"Updated Tab1", tabbed_pane_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
    ...
    tabbed_pane_->GetViewAccessibility().GetAccessibleNodeData(&tabbed_pane_data);
    EXPECT_EQ(u"Tab2", tabbed_pane_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
}
