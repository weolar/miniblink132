...
public:
    ...
    explicit TabbedPane(Orientation orientation = Orientation::kHorizontal, TabStripStyle style = TabStripStyle::kBorder, bool scrollable = false);
    ...
    // the TabbedPane.
    template <typename T> T* AddTab(const std::u16string& title, std::unique_ptr<T> contents)
    {
    ...
    template <typename T> T* AddTabAtIndex(size_t index, const std::u16string& title, std::unique_ptr<T> contents)
    {
...
private:
    ...
    // when the tab is selected and is owned by the TabbedPane. If the tabbed pane
    // is currently empty, the new tab is selected.
    void AddTabInternal(size_t index, const std::u16string& title, std::unique_ptr<View> contents);
...
public:
    TabbedPaneTab(TabbedPane* tabbed_pane, const std::u16string& title, View* contents);
...
public:
    ...
    TabbedPaneTabStrip(TabbedPane::Orientation orientation, TabbedPane::TabStripStyle style);
    ...
    // selected tab.
    void OnSelectedTabChanged(TabbedPaneTab* from_tab, TabbedPaneTab* to_tab, bool animate = true);
...
private:
    ...
    std::unique_ptr<gfx::LinearAnimation> expand_animation_ = std::make_unique<gfx::LinearAnimation>(this);
    std::unique_ptr<gfx::LinearAnimation> contract_animation_ = std::make_unique<gfx::LinearAnimation>(this);
...
VIEW_BUILDER_METHOD_ALIAS(AddTab, AddTab<View>, const std::u16string&, std::unique_ptr<View>)
END_VIEW_BUILDER
