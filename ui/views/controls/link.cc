    ...
    : Label(title, text_context, text_style)
{
    ...
    enabled_changed_subscription_ = AddEnabledChangedCallback(base::BindRepeating(&Link::RecalculateFont, base::Unretained(this)));
...
SkColor Link::GetColor() const
{
    ...
    if (GetTextContext() == style::CONTEXT_BUBBLE_FOOTER) {
        return color_provider->GetColor(pressed_ ? ui::kColorLinkForegroundPressedOnBubbleFooter : ui::kColorLinkForegroundOnBubbleFooter);
    }

    return color_provider->GetColor(pressed_ ? ui::kColorLinkForegroundPressed : ui::kColorLinkForeground);
}
...
{
    if (!GetEnabled() || (!event.IsLeftMouseButton() && !event.IsMiddleMouseButton()))
        return false;
    SetPressed(true);
    return true;
}
...
{
    SetPressed(GetEnabled() && (event.IsLeftMouseButton() || event.IsMiddleMouseButton()) && HitTestPoint(event.location()));
    return true;
}
...
void Link::OnMouseReleased(const ui::MouseEvent& event)
{
    ...
    OnMouseCaptureLost();
    if (GetEnabled() && (event.IsLeftMouseButton() || event.IsMiddleMouseButton()) && HitTestPoint(event.location()))
        OnClick(event);
}
...
{
    bool activate = (((event.key_code() == ui::VKEY_SPACE) && (event.flags() & ui::EF_ALT_DOWN) == 0)
        || (event.key_code() == ui::VKEY_RETURN && PlatformStyle::kReturnClicksFocusedControl));
    if (!activate)
        return false;
...
bool Link::SkipDefaultKeyEventProcessing(const ui::KeyEvent& event)
{
    ...
    // accelerator.
    return event.key_code() == ui::VKEY_SPACE || (event.key_code() == ui::VKEY_RETURN && PlatformStyle::kReturnClicksFocusedControl);
}
...
void Link::RecalculateFont()
{
    ...
    const int intended_style
        = ((GetEnabled() && (HasFocus() || IsMouseHovered())) || force_underline_) ? (style | gfx::Font::UNDERLINE) : (style & ~gfx::Font::UNDERLINE);
