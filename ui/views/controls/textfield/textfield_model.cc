...
{
    std::sort(ranges->begin(), ranges->end(), [](const auto& r1, const auto& r2) { return r1.GetMin() > r2.GetMin(); });
}
...
size_t adjust_position_for_removals(size_t position, std::vector<gfx::Range> ranges)
{
    size_t adjustment = 0;
...
public:
    ...
    void Undo(TextfieldModel* model)
    {
        ...
        model->ModifyText({ { static_cast<uint32_t>(new_text_start_), static_cast<uint32_t>(new_text_end()) } }, insertion_texts, insertion_text_starts,
    ...
    void Redo(TextfieldModel* model)
    {
        ...
        for (size_t i = 0; i < old_texts_.size(); ++i) {
            deletions.emplace_back(old_text_starts_[i], old_text_starts_[i] + old_texts_[i].length());
        }
        model->ModifyText(
            deletions, { new_text_ }, { new_text_start_ }, { static_cast<uint32_t>(new_cursor_pos_), static_cast<uint32_t>(new_cursor_pos_) }, {});
    }
...
private:
    ...
    Edit(Type type, MergeType merge_type, std::vector<std::u16string> old_texts, std::vector<size_t> old_text_starts, gfx::Range old_primary_selection,
        std::vector<gfx::Range> old_secondary_selections, bool delete_backward, size_t new_cursor_pos, const std::u16string& new_text, size_t new_text_start)
        : type_(type)
    ...
    {
        return merge_type_ == MergeType::kMergeable;
    }

    // Should this edit be forcibly merged with the previous edit?
    bool force_merge() const
    {
        return merge_type_ == MergeType::kForceMerge;
    }

    // Returns the end index of the |new_text_|.
    size_t new_text_end() const
    {
        return new_text_start_ + new_text_.length();
    }
...
public:
    InsertEdit(bool mergeable, const std::u16string& new_text, size_t at)
        : Edit(Type::kInsert, mergeable ? MergeType::kMergeable : MergeType::kDoNotMerge, {} /* old_texts */, {} /* old_text_starts */,
            { gfx::Range(at, at) } /* old_primary_selection */, {} /* old_secondary_selections */, false /* delete_backward */,
            at + new_text.length() /* new_cursor_pos */, new_text /* new_text */, at /* new_text_start */)
    {
    }
    ...
    bool DoMerge(const Edit* edit) override
    {
        ...
        // this insert ended.
        if (edit->type() != Type::kInsert || new_text_end() != edit->new_text_start_)
            return false;
        new_text_ += edit->new_text_;
        new_cursor_pos_ = edit->new_cursor_pos_;
        return true;
...
public:
    ReplaceEdit(MergeType merge_type, std::vector<std::u16string> old_texts, std::vector<size_t> old_text_starts, gfx::Range old_primary_selection,
        std::vector<gfx::Range> old_secondary_selections, bool backward, size_t new_cursor_pos, const std::u16string& new_text, size_t new_text_start)
        : Edit(Type::kReplace, merge_type, old_texts, old_text_starts, old_primary_selection, old_secondary_selections, backward, new_cursor_pos, new_text,
            new_text_start)
    {
    }
    ...
    bool DoMerge(const Edit* edit) override
    {
        ...
        if (edit->type() == Type::kDelete || edit->old_texts_.size() > 1 || new_text_end() != edit->new_text_start_
            || (!edit->old_text_starts_.empty() && new_text_end() != edit->old_text_starts_[0]))
            return false;
...
public:
    DeleteEdit(bool mergeable, std::vector<std::u16string> texts, std::vector<size_t> text_starts, gfx::Range old_primary_selection,
        std::vector<gfx::Range> old_secondary_selections, bool backward, size_t new_cursor_pos)
        : Edit(Type::kDelete, mergeable ? MergeType::kMergeable : MergeType::kDoNotMerge, texts, text_starts, old_primary_selection, old_secondary_selections,
            backward, new_cursor_pos, std::u16string() /* new_text */, 0 /* new_text_start */)
    {
    }
    ...
    bool DoMerge(const Edit* edit) override
    {
        ...
        if (delete_backward_) {
            ...
            if (!edit->delete_backward_ || old_text_starts_[0] != edit->old_text_starts_[0] + edit->old_texts_[0].length())
                return false;
            ...
            if (edit->delete_backward_ || old_text_starts_[0] != edit->old_text_starts_[0])
                return false;
            old_texts_[0] += edit->old_texts_[0];
        }
        return true;
...
void SelectRangeInCompositionText(gfx::RenderText* render_text, size_t cursor, const gfx::Range& range)
{
    DCHECK(render_text);
...
bool TextfieldModel::SetText(const std::u16string& new_text, size_t cursor_position)
{
    ...
    if (text() != new_text) {
        ...
        ExecuteAndRecordReplace(changed ? MergeType::kDoNotMerge : MergeType::kForceMerge, { gfx::Range(0, text().length()) }, cursor_position, new_text, 0U);
    }
    ClearSelection();
    return changed;
}
...
void TextfieldModel::Append(const std::u16string& new_text)
{
    ...
    MoveCursor(gfx::LINE_BREAK, render_text_->GetVisualDirectionOfLogicalEnd(), gfx::SELECTION_NONE);
    InsertText(new_text);
...
bool TextfieldModel::Delete(bool add_to_kill_buffer)
{
    ...
    if (cursor_position < text().length()) {
        size_t next_grapheme_index = render_text_->IndexOfAdjacentGrapheme(cursor_position, gfx::CURSOR_FORWARD);
...
bool TextfieldModel::Backspace(bool add_to_kill_buffer)
{
    ...
    if (cursor_position > 0) {
        gfx::Range range_to_delete(PlatformStyle::RangeToDeleteBackwards(text(), cursor_position));
        if (add_to_kill_buffer)
            SetKillBuffer(GetTextFromRange(range_to_delete));
        ExecuteAndRecordDelete({ range_to_delete }, true);
        return true;
...
void TextfieldModel::MoveCursor(gfx::BreakType break_type, gfx::VisualCursorDirection direction, gfx::SelectionBehavior selection_behavior)
{
    if (HasCompositionText())
        ConfirmCompositionText();
...
bool TextfieldModel::MoveCursorTo(const gfx::SelectionModel& cursor)
{
    if (HasCompositionText()) {
        ...
        return render_text_->SetSelection(gfx::SelectionModel(cursor.caret_pos(), cursor.caret_affinity()));
    }
...
{
    if (!HasCompositionText() && HasSelection(true) && !render_text_->obscured()) {
...
{
    std::u16string text;
    ui::Clipboard::GetForCurrentThread()->ReadText(ui::ClipboardBuffer::kCopyPaste, /* data_dst = */ nullptr, &text);
    if (text.empty())
        return false;
...
bool TextfieldModel::Transpose()
{
    ...
    size_t prev = render_text_->IndexOfAdjacentGrapheme(cur, gfx::CURSOR_BACKWARD);
    ...
    std::u16string transposed_text = text.substr(cur - prev) + text.substr(0, cur - prev);
...
bool TextfieldModel::HasSelection(bool primary_only) const
{
    ...
    return base::ranges::any_of(render_text_->GetAllSelections(), [](const auto& selection) { return !selection.is_empty(); });
}
...
void TextfieldModel::DeletePrimarySelectionAndInsertTextAt(const std::u16string& new_text, size_t position)
{
    using MergeType = internal::MergeType;
    if (HasCompositionText())
        CancelCompositionText();
    ...
    ExecuteAndRecordReplace(MergeType::kDoNotMerge, { render_text_->selection() }, position + new_text.length(), new_text, position);
}
...
void TextfieldModel::SetCompositionText(const ui::CompositionText& composition)
{
    if (HasCompositionText())
        CancelCompositionText();
    ...
    if (composition.ime_text_spans.size() > 0 && composition.ime_text_spans[0].thickness != ui::ImeTextSpan::Thickness::kNone)
    ...
    if (emphasized_range.IsValid()) {
        ...
        SelectRangeInCompositionText(render_text_.get(), cursor, composition.selection);
    } else {
...
{
    DCHECK(HasCompositionText());
    std::u16string composition = text().substr(composition_range_.start(), composition_range_.length());
    ...
    AddOrMergeEditHistory(std::make_unique<internal::InsertEdit>(false, composition, composition_range_.start()));
...
void TextfieldModel::InsertTextInternal(const std::u16string& new_text, bool mergeable)
{
    using MergeType = internal::MergeType;
    if (HasCompositionText()) {
        CancelCompositionText();
        ...
        ExecuteAndRecordReplaceSelection(mergeable ? MergeType::kMergeable : MergeType::kDoNotMerge, new_text);
    } else {
        ExecuteAndRecordInsert(new_text, mergeable);
    }
}

void TextfieldModel::ReplaceTextInternal(const std::u16string& new_text, bool mergeable)
{
    if (HasCompositionText()) {
        CancelCompositionText();
    } else if (!HasSelection()) {
...
void TextfieldModel::ExecuteAndRecordDelete(std::vector<gfx::Range> ranges, bool mergeable)
{
    ...
    auto edit = std::make_unique<internal::DeleteEdit>(
        mergeable, old_texts, old_text_starts, render_text_->selection(), render_text_->secondary_selections(), backward, cursor_pos);
    edit->Redo(this);
    AddOrMergeEditHistory(std::move(edit));
}

void TextfieldModel::ExecuteAndRecordReplaceSelection(internal::MergeType merge_type, const std::u16string& new_text)
{
    auto replacement_ranges = render_text_->GetAllSelections();
    size_t new_text_start = adjust_position_for_removals(GetCursorPosition(), replacement_ranges);
    size_t new_cursor_pos = new_text_start + new_text.length();

    ExecuteAndRecordReplace(merge_type, replacement_ranges, new_cursor_pos, new_text, new_text_start);
}

void TextfieldModel::ExecuteAndRecordReplace(
    internal::MergeType merge_type, std::vector<gfx::Range> replacement_ranges, size_t new_cursor_pos, const std::u16string& new_text, size_t new_text_start)
{
    // We need only check replacement_ranges[0] as |delete_backwards_| is
    // irrelevant for multi-range deletions which can't be merged anyways.
    ...
    auto edit = std::make_unique<internal::ReplaceEdit>(merge_type, old_texts, old_text_starts, render_text_->selection(), render_text_->secondary_selections(),
        backward, new_cursor_pos, new_text, new_text_start);
    edit->Redo(this);
    AddOrMergeEditHistory(std::move(edit));
}

void TextfieldModel::ExecuteAndRecordInsert(const std::u16string& new_text, bool mergeable)
{
    auto edit = std::make_unique<internal::InsertEdit>(mergeable, new_text, GetCursorPosition());
    edit->Redo(this);
    AddOrMergeEditHistory(std::move(edit));
}

void TextfieldModel::AddOrMergeEditHistory(std::unique_ptr<internal::Edit> edit)
{
    ClearRedoHistory();

    if (current_edit_ != edit_history_.end() && (*current_edit_)->Merge(edit.get())) {
...
void TextfieldModel::ModifyText(const std::vector<gfx::Range>& deletions, const std::vector<std::u16string>& insertion_texts,
    const std::vector<size_t>& insertion_positions, const gfx::Range& primary_selection, const std::vector<gfx::Range>& secondary_selections)
{
