...
public:
    ...
    // The current composition text will be confirmed.
    void MoveCursor(gfx::BreakType break_type, gfx::VisualCursorDirection direction, gfx::SelectionBehavior selection_behavior);
    ...
    void DeletePrimarySelectionAndInsertTextAt(const std::u16string& new_text, size_t position);
...
private:
    ...
    void ExecuteAndRecordReplaceSelection(internal::MergeType merge_type, const std::u16string& new_text);
    void ExecuteAndRecordReplace(internal::MergeType merge_type, std::vector<gfx::Range> replacement_range, size_t new_cursor_pos,
        const std::u16string& new_text, size_t new_text_start);
    ...
    // ordered decreasing.
    void ModifyText(const std::vector<gfx::Range>& deletions, const std::vector<std::u16string>& insertion_texts,
