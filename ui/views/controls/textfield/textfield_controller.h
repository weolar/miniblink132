...
public:
    ...
    virtual void ContentsChanged(Textfield* sender, const std::u16string& new_contents)
    {
    }
    ...
    // here.
    virtual bool HandleMouseEvent(Textfield* sender, const ui::MouseEvent& mouse_event);

    // Called to get notified about gesture events in the edit.
    // Returns true if the message was handled and should not be processed
    // further. Currently, only tap events are sent here.
    virtual bool HandleGestureEvent(Textfield* sender, const ui::GestureEvent& gesture_event);
    ...
    virtual void AppendDropFormats(int* formats, std::set<ui::ClipboardFormatType>* format_types)
    {
    }
    ...
    virtual views::View::DropCallback CreateDropCallback(const ui::DropTargetEvent& event);
