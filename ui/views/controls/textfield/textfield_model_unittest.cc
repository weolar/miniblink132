...
class TextfieldModelTest : public ViewsTestBase, public TextfieldModel::Delegate {
public:
...
protected:
    ...
    const std::vector<std::u16string> GetAllSelectionTexts(TextfieldModel* model) const
    {
    ...
    void VerifyAllSelectionTexts(TextfieldModel* model, std::vector<std::u16string> expected_selected_texts) const
    {
...
TEST_F(TextfieldModelTest, EditString)
{
    ...
    model.MoveCursor(gfx::CHARACTER_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_NONE);
    model.InsertChar('E');
...
TEST_F(TextfieldModelTest, EditString_ComplexScript)
{
    ...
    EXPECT_EQ(u"\x0915\x093f\x0915\x094d\x0915\x0915\x094d\x092e\x094d", model.text());
    ...
    EXPECT_EQ(2U, model.GetCursorPosition());
    model.InsertChar('a');
    EXPECT_EQ(u"\x0915\x093f\x0061\x0915\x094d\x0915\x0915\x094d\x092e\x094d", model.text());
...
TEST_F(TextfieldModelTest, EmptyString)
{
    ...
    model.MoveCursor(gfx::CHARACTER_BREAK, gfx::CURSOR_LEFT, gfx::SELECTION_RETAIN);
    EXPECT_EQ(0U, model.GetCursorPosition());
    model.MoveCursor(gfx::CHARACTER_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_RETAIN);
    EXPECT_EQ(0U, model.GetCursorPosition());
...
TEST_F(TextfieldModelTest, Selection_BidiWithNonSpacingMarks)
{
    ...
    // to test 2 characters belong to the same grapheme.
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
    model.Append(u"abc\x05E9\x05BC\x05C1\x05B8\x05E0\x05B8"
                 u"def");
    model.MoveCursor(gfx::CHARACTER_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_NONE);
    model.MoveCursor(gfx::CHARACTER_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_NONE);
    ...
    EXPECT_EQ(u"c\x05E9\x05BC\x05C1\x05B8\x05E0\x05B8"
              u"d",
    ...
    EXPECT_EQ(std::u16string(), model.GetSelectedText());
    model.SelectAll(false);
    EXPECT_EQ(u"abc\x05E9\x05BC\x05C1\x05B8\x05E0\x05B8"
              u"def",
        model.GetSelectedText());
#endif
...
TEST_F(TextfieldModelTest, Clipboard)
{
    ...
    ui::ScopedClipboardWriter(ui::ClipboardBuffer::kCopyPaste).WriteText(initial_clipboard_text);
    ...
    clipboard->ReadText(ui::ClipboardBuffer::kCopyPaste, /* data_dst = */ nullptr, &clipboard_text);
...
static void SelectWordTestVerifier(const TextfieldModel& model, const std::u16string& expected_selected_string, size_t expected_cursor_pos)
{
...
TEST_F(TextfieldModelTest, SelectWordTest_MixScripts)
{
    ...
    model.SetText(u"a\x05d0 \x05d1\x05d2 \x0915\x094d\x0915 "
    ...
    for (size_t i = 0; i < word_and_cursor.size(); ++i) {
        ...
        for (size_t j = 0; j < i; ++j)
            model.MoveCursor(gfx::CHARACTER_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_NONE);
        model.SelectWord();
        SelectWordTestVerifier(model, base::WideToUTF16(word_and_cursor[i].word), word_and_cursor[i].cursor);
    }
}
#endif
...
TEST_F(TextfieldModelTest, SelectSelectionModelTest)
{
    TextfieldModel model(nullptr);
    model.Append(u"HELLO WORLD");
    model.SelectSelectionModel(gfx::SelectionModel(gfx::Range(0, 6), gfx::CURSOR_BACKWARD));
    EXPECT_EQ(u"HELLO ", model.GetSelectedText());

    model.SelectSelectionModel(gfx::SelectionModel(gfx::Range(6, 1), gfx::CURSOR_FORWARD));
    EXPECT_EQ(u"ELLO ", model.GetSelectedText());

    model.SelectSelectionModel(gfx::SelectionModel(gfx::Range(2, 1000), gfx::CURSOR_BACKWARD));
    EXPECT_EQ(u"LLO WORLD", model.GetSelectedText());

    model.SelectSelectionModel(gfx::SelectionModel(gfx::Range(1000, 3), gfx::CURSOR_FORWARD));
    EXPECT_EQ(u"LO WORLD", model.GetSelectedText());
    ...
    model.SelectSelectionModel(gfx::SelectionModel(gfx::Range(1000, 100), gfx::CURSOR_FORWARD));
    EXPECT_TRUE(model.GetSelectedText().empty());

    model.SelectSelectionModel(gfx::SelectionModel(1000, gfx::CURSOR_BACKWARD));
    EXPECT_TRUE(model.GetSelectedText().empty());

    gfx::SelectionModel mutliselection_selection_model { { 2, 3 }, gfx::CURSOR_BACKWARD };
...
TEST_F(TextfieldModelTest, CompositionTextTest)
{
    ...
    composition.ime_text_spans.emplace_back(ui::ImeTextSpan::Type::kComposition, 0, 3, ui::ImeTextSpan::Thickness::kThin);
    ...
    composition.ime_text_spans.emplace_back(ui::ImeTextSpan::Type::kComposition, 0, 2, ui::ImeTextSpan::Thickness::kThick);
    composition.ime_text_spans.emplace_back(ui::ImeTextSpan::Type::kComposition, 2, 3, ui::ImeTextSpan::Thickness::kThin);
    model.SetCompositionText(composition);
    EXPECT_TRUE(model.HasCompositionText());
    EXPECT_TRUE(model.HasSelection());
    ...
    model.SetCompositionText(composition);
    gfx::SelectionModel sel(gfx::Range(model.render_text()->selection().start(), 0), gfx::CURSOR_FORWARD);
...
TEST_F(TextfieldModelTest, UndoRedo_CutCopyPasteTest)
{
    ...
    EXPECT_EQ(u"ABCDE", model.text());
    EXPECT_EQ(3U, model.GetCursorPosition());
    EXPECT_TRUE(model.render_text()->selection().EqualsIgnoringDirection(gfx::Range(1, 3)));
    EXPECT_TRUE(model.Undo()); //                                   |
    EXPECT_EQ(u"", model.text());
    EXPECT_EQ(0U, model.GetCursorPosition());
...
TEST_F(TextfieldModelTest, Clipboard_WhiteSpaceStringTest)
{
    ...
    // as-is.
    ui::ScopedClipboardWriter(ui::ClipboardBuffer::kCopyPaste).WriteText(u"FOO \t\t BAR");
    ...
    // space.
    ui::ScopedClipboardWriter(ui::ClipboardBuffer::kCopyPaste).WriteText(u"     \t\t");
    EXPECT_TRUE(model.Paste());
    EXPECT_EQ(u" ", model.text());
...
TEST_F(TextfieldModelTest, Transpose)
{
    ...
    const auto test_strings = std::to_array<std::u16string>({ ltr, rtl, surrogate_pairs });
    ...
    std::vector<TestCase> ltr_tests = { { gfx::Range(0), ltr, gfx::Range(0) }, { gfx::Range(1), ltr_transposed, gfx::Range(2) },
        { gfx::Range(2), ltr_transposed, gfx::Range(2) }, { gfx::Range(0, 2), ltr, gfx::Range(0, 2) } };

    std::vector<TestCase> rtl_tests = { { gfx::Range(0), rtl, gfx::Range(0) }, { gfx::Range(1), rtl_transposed, gfx::Range(2) },
        { gfx::Range(2), rtl_transposed, gfx::Range(2) }, { gfx::Range(0, 1), rtl, gfx::Range(0, 1) } };

    // Only test at valid grapheme boundaries.
    std::vector<TestCase> surrogate_pairs_test = { { gfx::Range(0), surrogate_pairs, gfx::Range(0) },
        { gfx::Range(2), std::u16string({ 'a', 0xD83D, 0xDE07, 0xD83D, 0xDE0E }), gfx::Range(3) },
        { gfx::Range(3), std::u16string({ 0xD83D, 0xDE07, 0xD83D, 0xDE0E, 'a' }), gfx::Range(5) },
        { gfx::Range(5), std::u16string({ 0xD83D, 0xDE07, 0xD83D, 0xDE0E, 'a' }), gfx::Range(5) }, { gfx::Range(3, 5), surrogate_pairs, gfx::Range(3, 5) } };

    std::vector<std::vector<TestCase>> all_tests = { ltr_tests, rtl_tests, surrogate_pairs_test };
    ...
    for (size_t i = 0; i < std::size(test_strings); i++) {
        for (size_t j = 0; j < all_tests[i].size(); j++) {
            SCOPED_TRACE(testing::Message() << "Testing case " << i << ", " << j << " with string " << test_strings[i]);
