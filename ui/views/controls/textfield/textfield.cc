...
ui::TextEditCommand GetTextEditCommandFromMenuCommand(int command_id, bool has_selection)
{
    switch (command_id) {
...
base::TimeDelta GetPasswordRevealDuration(const ui::KeyEvent& event)
{
    ...
    bool from_vk = properties && properties->find(ui::kPropertyFromVK) != properties->end();
    if (from_vk) {
        std::vector<uint8_t> fromVKPropertyArray = properties->find(ui::kPropertyFromVK)->second;
    ...
    , selection_controller_(this)
{
    ...
    views::InstallRoundRectHighlightPathGenerator(this, gfx::Insets(), GetCornerRadius());
    FocusRing::Install(this);
    FocusRing::Get(this)->SetOutsetFocusRingDisabled(true);
    InkDropHost* ink_drop_host = InkDrop::Install(this, std::make_unique<views::InkDropHost>(this));
    ...
    ink_drop_host->SetBaseColorCallback(base::BindRepeating(
        [](Textfield* host) { return host->HasFocus() ? SK_ColorTRANSPARENT : host->GetColorProvider()->GetColor(ui::kColorTextfieldHover); }, this));
...
void Textfield::SetTextWithoutCaretBoundsChangeNotification(const std::u16string& text, size_t cursor_position)
{
...
{
    return text_color_.value_or(GetColorProvider()->GetColor(TypographyProvider::Get().GetColorId(style::CONTEXT_TEXTFIELD, GetTextStyle())));
}
...
SkColor Textfield::GetBackgroundColor() const
{
    return background_color_.value_or(
        GetColorProvider()->GetColor(GetReadOnly() || !GetEnabled() ? ui::kColorTextfieldBackgroundDisabled : ui::kColorTextfieldBackground));
}
...
{
    return selection_text_color_.value_or(GetColorProvider()->GetColor(ui::kColorTextfieldSelectionForeground));
}
...
{
    return selection_background_color_.value_or(GetColorProvider()->GetColor(ui::kColorTextfieldSelectionBackground));
}
...
void Textfield::SetCursorEnabled(bool enabled)
{
    ...
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&model_, kTextfieldCursorEnabled), kPropertyEffectsPaint);
}
...
void Textfield::SetHorizontalAlignment(gfx::HorizontalAlignment alignment)
{
    ...
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&model_, kTextfieldHorizontalAlignment), kPropertyEffectsNone);
}
...
void Textfield::SetSelectedRange(const gfx::Range& range)
{
    ...
    UpdateAfterChange(TextChangeType::kNone, true);
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&model_, kTextfieldSelectedRange), kPropertyEffectsPaint);
}
...
void Textfield::SetColor(SkColor value)
{
    ...
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&model_, kTextfieldTextColor), kPropertyEffectsPaint);
}
...
void Textfield::ApplyStyle(gfx::TextStyle style, bool value, const gfx::Range& range)
{
...
gfx::Size Textfield::CalculatePreferredSize(const SizeBounds& available_size) const
{
    DCHECK_GE(default_width_in_chars_, minimum_width_in_chars_);
    return gfx::Size(CharsToDips(default_width_in_chars_), LayoutProvider::GetControlHeightForFont(style::CONTEXT_TEXTFIELD, GetTextStyle(), GetFontList()));
}
...
ui::Cursor Textfield::GetCursor(const ui::MouseEvent& event)
{
    ...
    bool text_cursor = !initiating_drag_ && (drag_event || !in_selection || !platform_arrow);
...
bool Textfield::OnMousePressed(const ui::MouseEvent& event)
{
    ...
    if (!handled && (event.IsOnlyLeftMouseButton() || event.IsOnlyRightMouseButton())) {
        if (!had_focus)
    ...
    return selection_controller_.OnMousePressed(event, handled,
        had_focus ? SelectionController::InitialFocusStateOnMousePress::kFocused : SelectionController::InitialFocusStateOnMousePress::kUnFocused);
}
...
bool Textfield::OnKeyPressed(const ui::KeyEvent& event)
{
    ...
    if (!handled && linux_ui && linux_ui->GetTextEditCommandsForEvent(event, ui::TEXT_INPUT_FLAG_NONE, &commands)) {
...
void Textfield::OnGestureEvent(ui::GestureEvent* event)
{
    ...
    case ui::EventType::kGestureTap: {
        ...
        const size_t tap_pos = GetRenderText()->FindCursorPosition(event->location()).caret_pos();
        const bool should_toggle_menu = event->details().tap_count() == 1 && GetSelectedRange() == gfx::Range(tap_pos);
...
{
    ui::KeyEvent event(accelerator.key_state() == ui::Accelerator::KeyState::PRESSED ? ui::EventType::kKeyPressed : ui::EventType::kKeyReleased,
...
bool Textfield::SkipDefaultKeyEventProcessing(const ui::KeyEvent& event)
{
    ...
    auto* linux_ui = ui::LinuxUi::instance();
    std::vector<ui::TextEditCommandAuraLinux> commands;
    if (linux_ui && linux_ui->GetTextEditCommandsForEvent(event, ui::TEXT_INPUT_FLAG_NONE, &commands)) {
        const auto is_enabled = [this](const auto& command) { return IsTextEditCommandEnabled(command.command()); };
...
bool Textfield::GetDropFormats(int* formats, std::set<ui::ClipboardFormatType>* format_types)
{
...
bool Textfield::CanDrop(const OSExchangeData& data)
{
    ...
    return GetEnabled() && !GetReadOnly() && data.HasAnyFormat(formats, format_types);
}
...
views::View::DropCallback Textfield::GetDropCallback(const ui::DropTargetEvent& event)
{
    DCHECK(CanDrop(event.data()));
    ...
    DCHECK(!initiating_drag_ || !GetRenderText()->IsPointInSelection(event.location()));

    return base::BindOnce(&Textfield::DropDraggedText, drop_weak_ptr_factory_.GetWeakPtr());
}
...
void Textfield::GetAccessibleNodeData(ui::AXNodeData* node_data)
{
    ...
    node_data->AddIntAttribute(ax::mojom::IntAttribute::kTextSelStart, base::checked_cast<int32_t>(range.start()));
    node_data->AddIntAttribute(ax::mojom::IntAttribute::kTextSelEnd, base::checked_cast<int32_t>(range.end()));
}
...
void Textfield::OnAccessibilityInitializing(ui::AXNodeData* data)
{
    ...
    if (::ui::AXPlatform::GetInstance().IsUiaProviderEnabled() && !GetRenderText()->multiline()) {
        ax_value_used_to_compute_offsets_ = GetRenderText()->GetDisplayText();

        data->AddIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets, ComputeTextOffsets(GetRenderText()));
        WordBoundaries boundaries = ComputeWordBoundaries(GetText());
        data->AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, boundaries.starts);
        data->AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, boundaries.ends);
    }
...
{
    bool should_update = ax_value_used_to_compute_offsets_ != GetRenderText()->GetDisplayText();
    if (::ui::AXPlatform::GetInstance().IsUiaProviderEnabled() && GetViewAccessibility().is_initialized() && should_update) {
...
void Textfield::RefreshAccessibleTextOffsets()
{
    ...
    GetViewAccessibility().SetCharacterOffsets(ComputeTextOffsets(GetRenderText()));
...
bool Textfield::HandleAccessibleAction(const ui::AXActionData& action_data)
{
    if (action_data.action == ax::mojom::Action::kSetSelection) {
        ...
        const gfx::Range range(static_cast<size_t>(action_data.anchor_offset), static_cast<size_t>(action_data.focus_offset));
...
void Textfield::OnFocus()
{
    ...
    if (text_input_type_ == ui::TEXT_INPUT_TYPE_PASSWORD)
        password_input_enabler_ = std::make_unique<ui::ScopedPasswordInputEnabler>();
...
#if BUILDFLAG(IS_CHROMEOS)
        wm::RestoreWindowBoundsOnClientFocusLost(GetNativeView()->GetToplevelWindow());
...
void Textfield::OnThemeChanged()
{
    ...
    render_text->set_selection_background_focused_color(GetSelectionBackgroundColor());
...
{
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&model_, kTextfieldText), kPropertyEffectsPaint);
...
void Textfield::ShowContextMenuForViewImpl(View* source, const gfx::Point& point, ui::mojom::MenuSourceType source_type)
{
    UpdateContextMenu();
    context_menu_runner_->RunMenuAt(GetWidget(), nullptr, gfx::Rect(point, gfx::Size()), MenuAnchorPosition::kTopLeft, source_type);
}

////////////////////////////////////////////////////////////////////////////////
// Textfield, DragController overrides:

void Textfield::WriteDragDataForView(View* sender, const gfx::Point& press_pt, OSExchangeData* data)
{
    ...
    display::Display display = display::Screen::GetScreen()->GetDisplayNearestView(native_view);
    ...
    SkColor color = views::Widget::IsWindowCompositingSupported() ? SK_ColorTRANSPARENT : GetBackgroundColor();
    label.Paint(PaintInfo::CreateRootPaintInfo(
        ui::CanvasPainter(&bitmap, label.size(), raster_scale, color, GetWidget()->GetCompositor()->is_pixel_canvas()).context(), label.size()));
...
int Textfield::GetDragOperationsForView(View* sender, const gfx::Point& p)
{
    ...
    if (!GetEnabled() || text_input_type_ == ui::TEXT_INPUT_TYPE_PASSWORD || !GetRenderText()->IsPointInSelection(p)) {
...
bool Textfield::CanStartDragForView(View* sender, const gfx::Point& press_pt, const gfx::Point& p)
{
...
bool Textfield::GetWordLookupDataAtPoint(const gfx::Point& point, gfx::DecoratedText* decorated_word, gfx::Rect* rect)
{
    return GetRenderText()->GetWordLookupDataAtPoint(point, decorated_word, rect);
}

bool Textfield::GetWordLookupDataFromSelection(gfx::DecoratedText* decorated_text, gfx::Rect* rect)
{
    if (GetRenderText()->obscured()) {
        return false;
    }
    return GetRenderText()->GetLookupDataForRange(GetRenderText()->selection(), decorated_text, rect);
}
...
void Textfield::MoveRangeSelectionExtent(const gfx::Point& extent)
{
    ...
    if (!::features::IsTouchTextEditingRedesignEnabled()) {
        gfx::SelectionModel base_caret = render_text->GetSelectionModelForSelectionStart();
        gfx::SelectionModel extent_caret = render_text->FindCursorPosition(extent);
        gfx::SelectionModel selection_model(gfx::Range(base_caret.caret_pos(), extent_caret.caret_pos()), extent_caret.caret_affinity());
    ...
    const gfx::SelectionModel cursor_position_at_old_extent = render_text->FindCursorPosition(selection_extent_);
    const gfx::SelectionModel cursor_position_at_new_extent = render_text->FindCursorPosition(extent);

    if (render_text->GetLineContainingCaret(cursor_position_at_old_extent) != render_text->GetLineContainingCaret(cursor_position_at_new_extent)) {
    ...
    const gfx::Point old_extent_with_offset = selection_extent_ + gfx::Vector2d(extent_offset_x_, 0);
    const size_t caret_pos_at_old_extent_with_offset = render_text->FindCursorPosition(old_extent_with_offset).caret_pos();
    gfx::Point new_extent_with_offset = extent + gfx::Vector2d(extent_offset_x_, 0);
    size_t caret_pos_at_new_extent_with_offset = render_text->FindCursorPosition(new_extent_with_offset).caret_pos();
    ...
    if (break_type_ == gfx::CHARACTER_BREAK) {
        ...
        const gfx::Range nearest_word_boundaries = render_text->ExpandRangeToWordBoundary(selection);
        const bool extent_moved_past_next_word_boundary = caret_pos_at_new_extent_with_offset <= nearest_word_boundaries.GetMin()
        ...
        const bool selection_shrinking = selection.is_reversed() ? caret_pos_at_new_extent_with_offset > caret_pos_at_old_extent_with_offset
                                                                 : caret_pos_at_new_extent_with_offset < caret_pos_at_old_extent_with_offset;
        if (selection_shrinking) {
            break_type_ = gfx::CHARACTER_BREAK;
            const gfx::Rect cursor_bounds = render_text->GetCursorBounds(GetSelectionModel(), true);
            extent_offset_x_ = cursor_bounds.CenterPoint().x() - selection_extent_.x();
        }
    }
    selection_extent_ = extent;
    new_extent_with_offset = extent + gfx::Vector2d(extent_offset_x_, 0);
    caret_pos_at_new_extent_with_offset = render_text->FindCursorPosition(new_extent_with_offset).caret_pos();
    ...
    if (break_type_ == gfx::WORD_BREAK) {
        ...
        const gfx::Range nearest_word_boundaries = render_text->ExpandRangeToWordBoundary(gfx::Range(end));
        DCHECK(end >= nearest_word_boundaries.start() && end <= nearest_word_boundaries.end());
        end = end - nearest_word_boundaries.start() < nearest_word_boundaries.end() - end ? nearest_word_boundaries.start() : nearest_word_boundaries.end();
    }
    ...
    const gfx::LogicalCursorDirection affinity = start > end ? gfx::CURSOR_FORWARD : gfx::CURSOR_BACKWARD;
    OnBeforeUserAction();
    SelectSelectionModel(gfx::SelectionModel(gfx::Range(start, end), affinity));
    OnAfterUserAction();
}

void Textfield::SelectBetweenCoordinates(const gfx::Point& base, const gfx::Point& extent)
{
    if (GetTextInputType() == ui::TEXT_INPUT_TYPE_NONE) {
        return;
    }
    ...
    gfx::SelectionModel extent_caret = GetRenderText()->FindCursorPosition(extent);
    gfx::SelectionModel selection(gfx::Range(base_caret.caret_pos(), extent_caret.caret_pos()), extent_caret.caret_affinity());
...
void Textfield::GetSelectionEndPoints(gfx::SelectionBound* anchor, gfx::SelectionBound* focus)
{
    gfx::RenderText* render_text = GetRenderText();
...
bool Textfield::IsSelectionDragging() const
{
    return selection_dragging_state_ == SelectionDraggingState::kDraggingCursor
        || selection_dragging_state_ == SelectionDraggingState::kDraggingSelectionExtent;
}
...
{
    if (text_services_context_menu_ && text_services_context_menu_->SupportsCommand(command_id)) {
...
bool Textfield::IsCommandIdEnabled(int command_id) const
{
    ...
    return IsTextEditCommandEnabled(GetTextEditCommandFromMenuCommand(command_id, HasSelection()));
}

bool Textfield::GetAcceleratorForCommandId(int command_id, ui::Accelerator* accelerator) const
{
    switch (command_id) {
    case kUndo:
    ...
    default:
        return text_services_context_menu_->GetAcceleratorForCommandId(command_id, accelerator);
    }
}
...
void Textfield::ExecuteCommand(int command_id, int event_flags)
{
    ...
    Textfield::ExecuteTextEditCommand(GetTextEditCommandFromMenuCommand(command_id, HasSelection()));

    if (::features::IsTouchTextEditingRedesignEnabled() && (event_flags & ui::EF_FROM_TOUCH)
        && (command_id == Textfield::kSelectAll || command_id == Textfield::kSelectWord)) {
        CreateTouchSelectionControllerAndNotifyIt();
    }
}
...
void Textfield::InsertText(const std::u16string& new_text, InsertTextCursorBehavior cursor_behavior)
{
    std::u16string filtered_new_text;
    base::ranges::copy_if(new_text, std::back_inserter(filtered_new_text), IsValidCharToInsert);

    if (GetTextInputType() == ui::TEXT_INPUT_TYPE_NONE || filtered_new_text.empty())
        return;
...
void Textfield::InsertChar(const ui::KeyEvent& event)
{
    ...
    const bool should_insert_char = IsValidCharToInsert(ch) && !ui::IsSystemKeyModifier(event.flags()) && !IsControlKeyModifier(event.flags());
...
#if BUILDFLAG(IS_WIN)
std::optional<gfx::Rect> Textfield::GetProximateCharacterBounds(const gfx::Range& range) const
{
    NOTIMPLEMENTED_LOG_ONCE();
    return std::nullopt;
}

std::optional<size_t> Textfield::GetProximateCharacterIndexFromPoint(const gfx::Point& point, ui::IndexFromPointFlags flags) const
{
    NOTIMPLEMENTED_LOG_ONCE();
    return std::nullopt;
}
...
bool Textfield::GetCompositionCharacterBounds(size_t index, gfx::Rect* rect) const
{
    DCHECK(rect);
    ...
    if (!render_text->IsValidCursorIndex(text_index)) {
        text_index = render_text->IndexOfAdjacentGrapheme(text_index, gfx::CURSOR_BACKWARD);
    }
...
bool Textfield::GetTextFromRange(const gfx::Range& range, std::u16string* range_text) const
{
    if (!ImeEditingAllowed() || !range.IsValid())
        return false;
...
bool Textfield::ChangeTextDirectionAndLayoutAlignment(base::i18n::TextDirection direction)
{
    ...
    const auto new_mode = default_rtl ? gfx::DIRECTIONALITY_FORCE_RTL : gfx::DIRECTIONALITY_FORCE_LTR;
    auto* render_text = GetRenderText();
    const bool modes_match = new_mode == render_text->directionality_mode();
    render_text->SetDirectionalityMode(modes_match ? gfx::DIRECTIONALITY_FROM_TEXT : new_mode);
    ...
    // been modified.
    bool dir_from_text = modes_match && GetHorizontalAlignment() == gfx::ALIGN_TO_HEAD;
...
bool Textfield::IsTextEditCommandEnabled(ui::TextEditCommand command) const
{
    ...
    case ui::TextEditCommand::MOVE_TO_BEGINNING_OF_DOCUMENT_AND_MODIFY_SELECTION:
    ...
    case ui::TextEditCommand::MOVE_TO_BEGINNING_OF_PARAGRAPH_AND_MODIFY_SELECTION:
    ...
    case ui::TextEditCommand::PASTE: {
        ui::DataTransferEndpoint data_dst(ui::EndpointType::kDefault, { .notify_if_restricted = show_rejection_ui_if_any_ });
        ui::Clipboard::GetForCurrentThread()->ReadText(ui::ClipboardBuffer::kCopyPaste, &data_dst, &result);
    }
        return editable && !result.empty();
    case ui::TextEditCommand::SELECT_ALL:
        return !GetText().empty() && GetSelectedRange().length() != GetText().length();
...
bool Textfield::SetCompositionFromExistingText(const gfx::Range& range, const std::vector<ui::ImeTextSpan>& ui_ime_text_spans)
{
...
{
    if (!range.is_empty()) {
        base::UmaHistogramEnumeration("InputMethod.Assistive.Autocorrect.Count", TextInputClient::SubClass::kTextField);
    }
    // TODO(b/316461955): Implement autocorrect UI for native fields.
    NOTIMPLEMENTED_LOG_ONCE();
    return false;
...
bool Textfield::AddGrammarFragments(const std::vector<ui::GrammarFragment>& fragments)
{
    if (!fragments.empty()) {
        base::UmaHistogramEnumeration("InputMethod.Assistive.Grammar.Count", TextInputClient::SubClass::kTextField);
    }
...
void Textfield::GetActiveTextInputControlLayoutBounds(std::optional<gfx::Rect>* control_bounds, std::optional<gfx::Rect>* selection_bounds)
{
...
void Textfield::SetActiveCompositionForAccessibility(const gfx::Range& range, const std::u16string& active_composition_text, bool is_composition_committed)
{
}
#endif
...
void Textfield::OnViewFocused(views::View* observed_view)
{
    ...
    observed_view->NotifyAccessibilityEvent(ax::mojom::Event::kTextSelectionChanged, true);
}
...
std::u16string Textfield::GetSelectionClipboardText() const
{
    ...
    ui::Clipboard::GetForCurrentThread()->ReadText(ui::ClipboardBuffer::kSelection, /* data_dst = */ nullptr, &selection_clipboard_text);
...
void Textfield::ExecuteTextEditCommand(ui::TextEditCommand command)
{
    ...
    if (cursor_changed && HasSelection())
        UpdateSelectionClipboard();
    UpdateAfterChange(text_changed ? TextChangeType::kUserTriggered : TextChangeType::kNone, cursor_changed);
    OnAfterUserAction();
}

Textfield::EditCommandResult Textfield::DoExecuteTextEditCommand(ui::TextEditCommand command)
{
    bool changed = false;
    ...
    case ui::TextEditCommand::MOVE_FORWARD_AND_MODIFY_SELECTION:
        ...
        break;
    case ui::TextEditCommand::MOVE_LEFT:
        model_->MoveCursor(gfx::CHARACTER_BREAK, gfx::CURSOR_LEFT, gfx::SELECTION_NONE);
        break;
    case ui::TextEditCommand::MOVE_LEFT_AND_MODIFY_SELECTION:
        model_->MoveCursor(gfx::CHARACTER_BREAK, gfx::CURSOR_LEFT, gfx::SELECTION_RETAIN);
        break;
    case ui::TextEditCommand::MOVE_RIGHT:
        model_->MoveCursor(gfx::CHARACTER_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_NONE);
        break;
    case ui::TextEditCommand::MOVE_RIGHT_AND_MODIFY_SELECTION:
        model_->MoveCursor(gfx::CHARACTER_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_RETAIN);
        break;
    case ui::TextEditCommand::MOVE_TO_BEGINNING_OF_LINE:
    ...
    case ui::TextEditCommand::MOVE_DOWN_AND_MODIFY_SELECTION:
        ...
        break;
    case ui::TextEditCommand::MOVE_PARAGRAPH_BACKWARD_AND_MODIFY_SELECTION:
        model_->MoveCursor(gfx::FIELD_BREAK, begin, kMoveParagraphSelectionBehavior);
        break;
    case ui::TextEditCommand::MOVE_PARAGRAPH_FORWARD_AND_MODIFY_SELECTION:
        model_->MoveCursor(gfx::FIELD_BREAK, end, kMoveParagraphSelectionBehavior);
        break;
    case ui::TextEditCommand::MOVE_WORD_BACKWARD:
    ...
    case ui::TextEditCommand::MOVE_WORD_FORWARD_AND_MODIFY_SELECTION:
        ...
        break;
    case ui::TextEditCommand::MOVE_WORD_LEFT:
        model_->MoveCursor(gfx::WORD_BREAK, gfx::CURSOR_LEFT, gfx::SELECTION_NONE);
        break;
    case ui::TextEditCommand::MOVE_WORD_LEFT_AND_MODIFY_SELECTION:
        model_->MoveCursor(gfx::WORD_BREAK, gfx::CURSOR_LEFT, kWordSelectionBehavior);
        break;
    case ui::TextEditCommand::MOVE_WORD_RIGHT:
        model_->MoveCursor(gfx::WORD_BREAK, gfx::CURSOR_RIGHT, gfx::SELECTION_NONE);
        break;
    case ui::TextEditCommand::MOVE_WORD_RIGHT_AND_MODIFY_SELECTION:
        model_->MoveCursor(gfx::WORD_BREAK, gfx::CURSOR_RIGHT, kWordSelectionBehavior);
        break;
    case ui::TextEditCommand::UNDO:
...
#if BUILDFLAG(IS_WIN)
    show_virtual_keyboard = details.primary_pointer_type() == ui::EventPointerType::kTouch || details.primary_pointer_type() == ui::EventPointerType::kPen;
#endif
...
base::CallbackListSubscription Textfield::AddTextChangedCallback(views::PropertyChangedCallback callback)
{
    return AddPropertyChangedCallback(ui::metadata::MakeUniquePropertyKey(&model_, kTextfieldText), std::move(callback));
}
...
ui::TextEditCommand Textfield::GetCommandForKeyEvent(const ui::KeyEvent& event)
{
    ...
    case ui::VKEY_Z:
        if (control && !shift && !alt)
            return ui::TextEditCommand::UNDO;
        return (control && shift && !alt) ? ui::TextEditCommand::REDO : ui::TextEditCommand::INVALID_COMMAND;
    case ui::VKEY_Y:
        return (control && !alt) ? ui::TextEditCommand::REDO : ui::TextEditCommand::INVALID_COMMAND;
    case ui::VKEY_A:
        return (control && !alt) ? ui::TextEditCommand::SELECT_ALL : ui::TextEditCommand::INVALID_COMMAND;
    case ui::VKEY_X:
        return (control && !alt) ? ui::TextEditCommand::CUT : ui::TextEditCommand::INVALID_COMMAND;
    case ui::VKEY_C:
        return (control && !alt) ? ui::TextEditCommand::COPY : ui::TextEditCommand::INVALID_COMMAND;
    case ui::VKEY_V:
        return (control && !alt) ? ui::TextEditCommand::PASTE : ui::TextEditCommand::INVALID_COMMAND;
    case ui::VKEY_RIGHT:
        // Ignore alt+right, which may be a browser navigation shortcut.
        if (alt)
            return ui::TextEditCommand::INVALID_COMMAND;
        if (!shift) {
            return control ? ui::TextEditCommand::MOVE_WORD_RIGHT : ui::TextEditCommand::MOVE_RIGHT;
        }
        return control ? ui::TextEditCommand::MOVE_WORD_RIGHT_AND_MODIFY_SELECTION : ui::TextEditCommand::MOVE_RIGHT_AND_MODIFY_SELECTION;
    case ui::VKEY_LEFT:
        // Ignore alt+left, which may be a browser navigation shortcut.
        if (alt)
            return ui::TextEditCommand::INVALID_COMMAND;
        if (!shift) {
            return control ? ui::TextEditCommand::MOVE_WORD_LEFT : ui::TextEditCommand::MOVE_LEFT;
        }
        return control ? ui::TextEditCommand::MOVE_WORD_LEFT_AND_MODIFY_SELECTION : ui::TextEditCommand::MOVE_LEFT_AND_MODIFY_SELECTION;
    case ui::VKEY_HOME:
        if (shift) {
            return ui::TextEditCommand::MOVE_TO_BEGINNING_OF_LINE_AND_MODIFY_SELECTION;
        }
#if BUILDFLAG(IS_MAC)
...
#if BUILDFLAG(IS_MAC)
        if (control && shift) {
            return ui::TextEditCommand::MOVE_PARAGRAPH_BACKWARD_AND_MODIFY_SELECTION;
        }
        if (command)
            return ui::TextEditCommand::MOVE_TO_BEGINNING_OF_DOCUMENT;
        return shift ? ui::TextEditCommand::MOVE_TO_BEGINNING_OF_LINE_AND_MODIFY_SELECTION : ui::TextEditCommand::MOVE_UP;
#else
        return shift ? ui::TextEditCommand::MOVE_TO_BEGINNING_OF_LINE_AND_MODIFY_SELECTION : ui::TextEditCommand::INVALID_COMMAND;
#endif
    case ui::VKEY_DOWN:
#if BUILDFLAG(IS_MAC)
        if (control && shift) {
        ...
        return shift ? ui::TextEditCommand::MOVE_TO_END_OF_LINE_AND_MODIFY_SELECTION : ui::TextEditCommand::MOVE_DOWN;
#else
        return shift ? ui::TextEditCommand::MOVE_TO_END_OF_LINE_AND_MODIFY_SELECTION : ui::TextEditCommand::INVALID_COMMAND;
#endif
    case ui::VKEY_BACK:
        if (!control) {
#if BUILDFLAG(IS_WIN)
    ...
    case ui::VKEY_DELETE:
        ...
        return shift ? ui::TextEditCommand::CUT : ui::TextEditCommand::DELETE_FORWARD;
    case ui::VKEY_INSERT:
        if (control && !shift)
            return ui::TextEditCommand::COPY;
        return (shift && !control) ? ui::TextEditCommand::PASTE : ui::TextEditCommand::INVALID_COMMAND;
    case ui::VKEY_PRIOR:
        return control ? ui::TextEditCommand::SCROLL_PAGE_UP : ui::TextEditCommand::INVALID_COMMAND;
    case ui::VKEY_NEXT:
        return control ? ui::TextEditCommand::SCROLL_PAGE_DOWN : ui::TextEditCommand::INVALID_COMMAND;
    default:
        return ui::TextEditCommand::INVALID_COMMAND;
    }
}
...
{
    if (ui::ScopedAnimationDurationScaleMode::duration_multiplier() == ui::ScopedAnimationDurationScaleMode::NON_ZERO_DURATION) {
...
void Textfield::OnAfterPointerAction(bool text_changed, bool selection_changed)
{
    OnAfterUserAction();
    const auto text_change_type = text_changed ? TextChangeType::kUserTriggered : TextChangeType::kNone;
...
void Textfield::UpdateSelectionClipboard()
{
    if (ui::Clipboard::IsSupportedClipboardBuffer(ui::ClipboardBuffer::kSelection)) {
        if (text_input_type_ != ui::TEXT_INPUT_TYPE_PASSWORD) {
            ui::ScopedClipboardWriter(ui::ClipboardBuffer::kSelection).WriteText(GetSelectedText());
            if (controller_)
...
void Textfield::UpdateBackgroundColor()
{
    ...
    SetBackground(CreateBackgroundFromPainter(Painter::CreateSolidRoundRectPainter(color, GetCornerRadius())));
    ...
    GetRenderText()->set_subpixel_rendering_suppressed(SkColorGetA(color) != SK_AlphaOPAQUE);
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&model_, kTextfieldBackgroundColor), kPropertyEffectsPaint);
}
...
void Textfield::UpdateDefaultBorder()
{
    ...
    border->SetInsets(gfx::Insets::TLBR(extra_insets_.top() + provider->GetDistanceMetric(DISTANCE_CONTROL_VERTICAL_TEXT_PADDING),
        extra_insets_.left() + provider->GetDistanceMetric(DISTANCE_TEXTFIELD_HORIZONTAL_TEXT_PADDING),
        extra_insets_.bottom() + provider->GetDistanceMetric(DISTANCE_CONTROL_VERTICAL_TEXT_PADDING),
        extra_insets_.right() + provider->GetDistanceMetric(DISTANCE_TEXTFIELD_HORIZONTAL_TEXT_PADDING)));
    if (invalid_) {
...
void Textfield::UpdateSelectionTextColor()
{
    ...
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&model_, kTextfieldSelectionTextColor), kPropertyEffectsPaint);
}
...
void Textfield::UpdateSelectionBackgroundColor()
{
    ...
    GetRenderText()->set_selection_background_focused_color(GetSelectionBackgroundColor());
    OnPropertyChanged(ui::metadata::MakeUniquePropertyKey(&model_, kTextfieldSelectionBackgroundColor), kPropertyEffectsPaint);
}

void Textfield::UpdateAfterChange(TextChangeType text_change_type, bool cursor_changed, std::optional<bool> notify_caret_bounds_changed)
{
    ...
    const bool anything_changed = (text_change_type != TextChangeType::kNone) || cursor_changed;
...
void Textfield::UpdateAccessibilityTextDirection()
{
    GetViewAccessibility().SetTextDirection(
        static_cast<int32_t>(GetTextDirection() == base::i18n::RIGHT_TO_LEFT ? ax::mojom::WritingDirection::kRtl : ax::mojom::WritingDirection::kLtr));
}

void Textfield::UpdateAccessibleValue()
{
    if (text_input_type_ == ui::TEXT_INPUT_TYPE_PASSWORD) {
        GetViewAccessibility().SetValue(std::u16string(GetText().size(), gfx::RenderText::kPasswordReplacementChar));
    } else {
...
void Textfield::UpdateCursorViewPosition()
{
    ...
    GetViewAccessibility().SetScrollX(GetRenderText()->GetUpdatedDisplayOffset().x());
}
...
void Textfield::PaintTextAndCursor(gfx::Canvas* canvas)
{
    ...
    if (ShouldShowPlaceholderText()) {
        ...
        canvas->DrawStringRectWithFlags(GetPlaceholderText(), placeholder_font_list_.value_or(GetFontList()),
            placeholder_text_color_.value_or(GetColorProvider()->GetColor(
                TypographyProvider::Get().GetColorId(style::CONTEXT_TEXTFIELD_PLACEHOLDER, GetInvalid() ? style::STYLE_INVALID : style::STYLE_PRIMARY))),
    ...
    if (drop_cursor_visible_ && IsDropCursorForInsertion()) {
        ...
        canvas->FillRect(render_text->GetCursorBounds(drop_cursor_position_, true), GetTextColor());
    }
...
bool Textfield::Cut()
{
    if (!GetReadOnly() && text_input_type_ != ui::TEXT_INPUT_TYPE_PASSWORD && model_->Cut()) {
        if (controller_)
...
void Textfield::UpdateContextMenu()
{
    ...
    text_services_context_menu_ = ViewsTextServicesContextMenu::Create(context_menu_contents_.get(), this);

    context_menu_runner_ = std::make_unique<MenuRunner>(context_menu_contents_.get(), MenuRunner::HAS_MNEMONICS | MenuRunner::CONTEXT_MENU);
}
...
void Textfield::RevealPasswordChar(std::optional<size_t> index, base::TimeDelta duration)
{
    ...
    if (index.has_value()) {
        password_reveal_timer_.Start(
            FROM_HERE, duration, base::BindOnce(&Textfield::RevealPasswordChar, weak_ptr_factory_.GetWeakPtr(), std::nullopt, duration));
    }
}
...
void Textfield::CreateTouchSelectionControllerAndNotifyIt()
{
    ...
    if (!touch_selection_controller_) {
#if defined(USE_AURA)
        touch_selection_controller_ = std::make_unique<TouchSelectionControllerImpl>(this);
#endif
    }
    if (touch_selection_controller_)
        touch_selection_controller_->SelectionChanged();
...
bool Textfield::ShouldShowCursor() const
{
    ...
    return HasFocus() && !HasSelection(true) && GetEnabled() && !GetReadOnly() && !drop_cursor_visible_ && GetRenderText()->cursor_enabled()
...
int Textfield::CharsToDips(int width_in_chars) const
{
    ...
    const int cursor_width = (!GetReadOnly() && GetRenderText()->cursor_enabled()) ? 1 : 0;
    return GetFontList().GetExpectedTextWidth(width_in_chars) + cursor_width + GetInsets().width();
}
...
void Textfield::StartBlinkingCursor()
{
    ...
    cursor_blink_timer_.Start(FROM_HERE, Textfield::GetCaretBlinkInterval(), this, &Textfield::OnCursorBlinkTimerFired);
}
...
void Textfield::OnCursorBlinkTimerFired()
{
    ...
    // can be removed.
    float new_opacity = cursor_view_->layer()->opacity() != kOpaque ? kOpaque : kAlmostTransparent;
...
void Textfield::DropDraggedText(
    const ui::DropTargetEvent& event, ui::mojom::DragOperation& output_drag_op, std::unique_ptr<ui::LayerTreeOwner> drag_image_layer_owner)
{
    DCHECK(CanDrop(event.data()));
...
{
    return LayoutProvider::Get()->GetCornerRadiusMetric(ShapeContextTokens::kTextfieldRadius, size());
}
...
{
    int new_display_offset = drag_start_display_offset_ + drag_location_x - drag_start_location_x_;
...
bool Textfield::HandleGestureForSelectionDragging(ui::GestureEvent* event)
{
    ...
    case ui::EventType::kGestureScrollBegin:
        ...
        if (event->details().touch_points() == 1 && StartSelectionDragging(*event)) {
            CreateTouchSelectionControllerAndNotifyIt();
    ...
    case ui::EventType::kGestureScrollUpdate:
        ...
        if (IsSelectionDragging() && event->details().touch_points() > 1) {
            ...
            OnGestureScrollBegin(event->location().x());
            return false;
        } else if (selection_dragging_state_ == SelectionDraggingState::kDraggingSelectionExtent) {
            MoveRangeSelectionExtent(event->location() + selection_dragging_offset_);
            event->SetHandled();
            return true;
        } else if (selection_dragging_state_ == SelectionDraggingState::kDraggingCursor) {
            MoveCursorTo(event->location(), false);
            event->SetHandled();
            return true;
        }
...
bool Textfield::StartSelectionDragging(const ui::GestureEvent& event)
{
    ...
    if (selection_dragging_state_ == SelectionDraggingState::kSelectedWord) {
        ...
        gfx::SelectionModel start_sel = render_text->GetSelectionModelForSelectionStart();
        const gfx::SelectionModel& sel = render_text->selection_model();
        gfx::Point selection_start = render_text->GetCursorBounds(start_sel, true).CenterPoint();
        gfx::Point selection_end = render_text->GetCursorBounds(sel, true).CenterPoint();
        ...
        if (std::fabs(delta_y) > std::fabs(delta_x)) {
            ...
            // movement.
            drag_direction = delta_x * (selection_end.x() - selection_start.x()) < 0 ? gfx::CURSOR_BACKWARD : gfx::CURSOR_FORWARD;
        }

        gfx::Point base = drag_direction == gfx::CURSOR_FORWARD ? selection_start : selection_end;
        gfx::Point extent = drag_direction == gfx::CURSOR_FORWARD ? selection_end : selection_start;
        SelectBetweenCoordinates(base, extent);

        selection_dragging_offset_ = extent - event.location();
        selection_dragging_state_ = SelectionDraggingState::kDraggingSelectionExtent;
        return true;
    } else if (selection_dragging_state_ == SelectionDraggingState::kNone && std::fabs(delta_x) >= std::fabs(delta_y)) {
...
{
    if (GetEnabled()) {
        GetViewAccessibility().SetDefaultActionVerb(ax::mojom::DefaultActionVerb::kActivate);
    } else {
