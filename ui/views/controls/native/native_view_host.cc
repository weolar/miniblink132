...
void NativeViewHost::SetBackgroundColorWhenClipped(std::optional<SkColor> color)
{
...
void NativeViewHost::Layout(PassKey)
{
    ...
    if (visible && !fast_resize_) {
        if (vis_bounds.size() != size()) {
            ...
            int x = vis_bounds.x();
            int y = vis_bounds.y();
            native_wrapper_->InstallClip(x, y, vis_bounds.width(), vis_bounds.height());
    ...
    if (visible) {
        ...
        gfx::Size native_view_size = native_view_size_.IsEmpty() ? local_bounds.size() : native_view_size_;
        native_wrapper_->ShowWidget(
            local_bounds.x(), local_bounds.y(), local_bounds.width(), local_bounds.height(), native_view_size.width(), native_view_size.height());
    } else {
        native_wrapper_->HideWidget();
    }
}
...
void NativeViewHost::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details)
{
