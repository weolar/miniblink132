...
public:
    ...
    bool CanFocus() override
    {
        ...
        // parents can focus.
        return !native_view_ || !native_view_->delegate() || native_view_->delegate()->CanFocus();
    }
...
void NativeViewHostAura::AttachNativeView()
{
    ...
    host_->native_view()->SetProperty(views::kHostViewKey, static_cast<View*>(host_));
...
void NativeViewHostAura::SetParentAccessible(gfx::NativeViewAccessible accessible)
{
    host_->native_view()->SetProperty(aura::client::kParentNativeViewAccessibleKey, accessible);
}

gfx::NativeViewAccessible NativeViewHostAura::GetParentAccessible()
{
    return host_->native_view()->GetProperty(aura::client::kParentNativeViewAccessibleKey);
}
...
bool NativeViewHostAura::SetCornerRadii(const gfx::RoundedCornersF& corner_radii)
{
...
{
    clip_rect_ = std::make_unique<gfx::Rect>(host_->ConvertRectToWidget(gfx::Rect(x, y, w, h)));
}
...
void NativeViewHostAura::ShowWidget(int x, int y, int w, int h, int native_w, int native_h)
{
    if (host_->fast_resize()) {
        ...
        if (w > 0 && h > 0 && native_w > 0 && native_h > 0) {
            transform.Scale(static_cast<SkScalar>(w) / native_w, static_cast<SkScalar>(h) / native_h);
        }
    ...
    host_->native_view()->SetBounds(gfx::Rect(x - clip_offset.x(), y - clip_offset.y(), native_w, native_h));
    host_->native_view()->Show();
...
// static
NativeViewHostWrapper* NativeViewHostWrapper::CreateWrapper(NativeViewHost* host)
{
...
void NativeViewHostAura::CreateClippingWindow()
{
    ...
    clipping_window_ = std::make_unique<aura::Window>(clipping_window_delegate_.get(), aura::client::WINDOW_TYPE_CONTROL);
...
void NativeViewHostAura::AddClippingWindow()
{
    ...
    host_->native_view()->SetProperty(aura::client::kHostWindowKey, host_->GetWidget()->GetNativeView());
    Widget::ReparentNativeView(host_->native_view(), clipping_window_.get());
    if (host_->GetWidget()->GetNativeView()) {
        Widget::ReparentNativeView(clipping_window_.get(), host_->GetWidget()->GetNativeView());
    }
}
...
void NativeViewHostAura::RemoveClippingWindow()
{
    ...
    if (host_->native_view()->parent() == clipping_window_.get()) {
        if (host_->GetWidget() && host_->GetWidget()->GetNativeView()) {
            Widget::ReparentNativeView(host_->native_view(), host_->GetWidget()->GetNativeView());
        } else {
            clipping_window_->RemoveChild(host_->native_view());
        }
    }
...
void NativeViewHostAura::UpdateInsets()
{
    ...
    if (top_inset_ == 0) {
        ...
        if (!clipping_window_->targeter()) {
            clipping_window_->SetEventTargeter(std::make_unique<aura::WindowTargeter>());
        }
        clipping_window_->targeter()->SetInsets(gfx::Insets::TLBR(top_inset_, 0, 0, 0));
    }
}
