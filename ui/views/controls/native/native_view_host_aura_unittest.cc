...
public:
    ...
    struct EventDetails {
        ...
        EventDetails(EventType event_type, aura::Window& window, const gfx::Rect& event_bounds)
            : type(event_type)
            , bounds(event_bounds)
        {
        ...
        {
            return type != rhs.type || window_id != rhs.window_id || bounds != rhs.bounds;
        }
    };
    ...
    NativeViewHostWindowObserver& operator=(const NativeViewHostWindowObserver&) = delete;

    ~NativeViewHostWindowObserver() override = default;

    const std::vector<EventDetails>& events() const
    {
        return events_;
    }
    ...
    {
        EventDetails event(visible ? EVENT_SHOWN : EVENT_HIDDEN, *window, window->GetBoundsInRootWindow());
    ...
    {
        EventDetails event(EVENT_BOUNDS_CHANGED, *window, window->GetBoundsInRootWindow());
        events_.push_back(event);
    }
...
public:
    ...
    void CreateHost()
    {
        CreateTopLevel();
        CreateTestingHost();
        child_ = CreateChildForHost(toplevel()->GetNativeView(), toplevel()->client_view(), new View, host());
    }
...
TEST_F(NativeViewHostAuraTest, HostViewPropertyKey)
{
    ...
    EXPECT_EQ(host()->GetWidget()->GetNativeView(), child_win->GetProperty(aura::client::kHostWindowKey));
...
TEST_F(NativeViewHostAuraTest, CursorForNativeView)
{
    ...
    ui::MouseEvent move_event(ui::EventType::kMouseMoved, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), 0, 0);
...
TEST_F(NativeViewHostAuraTest, FastResizePath)
{
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100).ToString(), host()->native_view()->bounds().ToString());
    EXPECT_EQ(gfx::Rect(5, 10, 100, 100).ToString(), clipping_window()->bounds().ToString());
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100).ToString(), host()->native_view()->bounds().ToString());
    EXPECT_EQ(gfx::Rect(10, 25, 50, 50).ToString(), clipping_window()->bounds().ToString());

    // Turning off fast resize should make the native view start resizing again.
    host()->set_fast_resize(false);
    native_host()->ShowWidget(10, 25, 50, 50, 50, 50);
    EXPECT_EQ(gfx::Rect(0, 0, 50, 50).ToString(), host()->native_view()->bounds().ToString());
    EXPECT_EQ(gfx::Rect(10, 25, 50, 50).ToString(), clipping_window()->bounds().ToString());
...
TEST_F(NativeViewHostAuraTest, BoundsWhileScaling)
{
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 200, 200).ToString(), host()->native_view()->bounds().ToString());
    EXPECT_EQ(gfx::Rect(5, 10, 100, 100).ToString(), clipping_window()->bounds().ToString());
...
TEST_F(NativeViewHostAuraTest, InstallClip)
{
    ...
    EXPECT_EQ(gfx::Rect(0, 0, 100, 100).ToString(), host()->native_view()->bounds().ToString());
    EXPECT_EQ(gfx::Rect(10, 20, 100, 100).ToString(), clipping_window()->bounds().ToString());

    // Clip to the bottom right quarter of the native view.
    native_host()->InstallClip(60 - client_bounds.x(), 70 - client_bounds.y(), 50, 50);
    native_host()->ShowWidget(10, 20, 100, 100, 100, 100);
    EXPECT_EQ(gfx::Rect(-50, -50, 100, 100).ToString(), host()->native_view()->bounds().ToString());
    EXPECT_EQ(gfx::Rect(60, 70, 50, 50).ToString(), clipping_window()->bounds().ToString());

    // Clip to the center of the native view.
    native_host()->InstallClip(35 - client_bounds.x(), 45 - client_bounds.y(), 50, 50);
    native_host()->ShowWidget(10, 20, 100, 100, 100, 100);
    EXPECT_EQ(gfx::Rect(-25, -25, 100, 100).ToString(), host()->native_view()->bounds().ToString());
    EXPECT_EQ(gfx::Rect(35, 45, 50, 50).ToString(), clipping_window()->bounds().ToString());
...
TEST_F(NativeViewHostAuraTest, ParentAfterDetach)
{
    ...
    EXPECT_EQ(0u, test_observer.events().size()) << (*test_observer.events().begin()).type;
    delete child_win;

    ASSERT_EQ(1u, test_observer.events().size());
    EXPECT_EQ(NativeViewHostWindowObserver::EVENT_DESTROYED, test_observer.events().back().type);
}
...
TEST_F(NativeViewHostAuraTest, RemoveClippingWindowOrder)
{
    ...
    EXPECT_EQ(NativeViewHostWindowObserver::EVENT_HIDDEN, test_observer.events()[0].type);
...
TEST_F(NativeViewHostAuraTest, Attach)
{
    ...
    EXPECT_EQ(NativeViewHostWindowObserver::EVENT_BOUNDS_CHANGED, test_observer.events()[0].type);
    EXPECT_EQ(child()->GetNativeView()->GetId(), test_observer.events()[0].window_id);
    EXPECT_EQ(expected_bounds.ToString(), test_observer.events()[0].bounds.ToString());
    EXPECT_EQ(NativeViewHostWindowObserver::EVENT_SHOWN, test_observer.events()[1].type);
    EXPECT_EQ(child()->GetNativeView()->GetId(), test_observer.events()[1].window_id);
    EXPECT_EQ(expected_bounds.ToString(), test_observer.events()[1].bounds.ToString());
    EXPECT_EQ(NativeViewHostWindowObserver::EVENT_SHOWN, test_observer.events()[2].type);
    EXPECT_EQ(clipping_window()->GetId(), test_observer.events()[2].window_id);
    EXPECT_EQ(expected_bounds.ToString(), test_observer.events()[2].bounds.ToString());
...
public:
    ...
    int did_change_focus_count() const
    {
        return did_change_focus_count_;
    }
...
TEST_F(NativeViewHostAuraTest, FocusManagerUpdatedDuringDestruction)
{
    ...
    std::unique_ptr<NativeViewHost> native_view_host = std::make_unique<NativeViewHost>();
    toplevel()->GetContentsView()->AddChildView(native_view_host.get());

    auto widget_delegate_view = std::make_unique<WidgetDelegateView>();
    Widget::InitParams params = CreateParams(views::Widget::InitParams::CLIENT_OWNS_WIDGET, Widget::InitParams::TYPE_CONTROL);
    ...
    View* view1 = child_widget->GetContentsView()->AddChildView(std::make_unique<View>()); // Owned by |child_widget|.
...
ui::EventTarget* GetTarget(aura::Window* window, const gfx::Point& location)
{
    ...
    aura::Window::ConvertPointToTarget(window, window->GetRootWindow(), &root_location);
    ui::MouseEvent event(ui::EventType::kMouseMoved, root_location, root_location, base::TimeTicks::Now(), 0, 0);
    return window->GetHost()->dispatcher()->event_targeter()->FindTargetForEvent(window->GetRootWindow(), &event);
}
...
TEST_F(NativeViewHostAuraTest, TopInsets)
{
    ...
    EXPECT_EQ(child_window, GetTarget(toplevel_window, gfx::Point(1, 1) + offset));
    EXPECT_EQ(child_window, GetTarget(toplevel_window, gfx::Point(1, 11) + offset));
...
TEST_F(NativeViewHostAuraTest, WindowHiddenWhenAttached)
{
    ...
    CreateTopLevel();
    NativeViewHost* host = toplevel()->GetRootView()->AddChildView(std::make_unique<NativeViewHost>());
    host->SetVisible(false);
...
class WidgetDelegateForShouldDescendIntoChildForEventHandling : public WidgetDelegate {
public:
    ...
    bool ShouldDescendIntoChildForEventHandling(gfx::NativeView child, const gfx::Point& location) override
    {
...
TEST_F(NativeViewHostAuraTest, ShouldDescendIntoChildForEventHandling)
{
    ...
    aura::Window::ConvertPointToTarget(toplevel()->GetNativeView(), window->GetRootWindow(), &press_location);
