...
public:
    ...
    NativeViewHierarchyChangedTestView(const NativeViewHierarchyChangedTestView&) = delete;
    NativeViewHierarchyChangedTestView& operator=(const NativeViewHierarchyChangedTestView&) = delete;
...
public:
    ...
    ViewHierarchyChangedTestHost& operator=(const ViewHierarchyChangedTestHost&) = delete;
    ...
    {
        gfx::NativeView parent_before = native_view() ? GetNativeParent(native_view()) : nullptr;
        NativeViewHost::ViewHierarchyChanged(details);
        gfx::NativeView parent_after = native_view() ? GetNativeParent(native_view()) : nullptr;
...
TEST_F(NativeViewHostTest, NativeViewHierarchyChanged)
{
    ...
    std::unique_ptr<Widget> child = CreateChildForHost(toplevel()->GetNativeView(), toplevel()->GetRootView(), test_view, host);
#if defined(USE_AURA)
...
#endif
    EXPECT_NE(toplevel()->GetNativeView(), GetNativeParent(child->GetNativeView()));
    test_view->ResetCount();
    ...
    EXPECT_EQ(2, test_view->notification_count());
    EXPECT_EQ(toplevel()->GetNativeView(), GetNativeParent(GetNativeParent(child->GetNativeView())));
#else
    EXPECT_EQ(1, test_view->notification_count());
    EXPECT_EQ(toplevel()->GetNativeView(), GetNativeParent(child->GetNativeView()));
#endif
}
...
TEST_F(NativeViewHostTest, ViewHierarchyChangedForHost)
{
    ...
    std::unique_ptr<Widget> child0 = CreateChildForHost(toplevel()->GetNativeView(), toplevel()->GetRootView(), new View, host0);
    NativeViewHost* host1 = new NativeViewHost;
    std::unique_ptr<Widget> child1 = CreateChildForHost(toplevel()->GetNativeView(), toplevel()->GetRootView(), new View, host1);
    ViewHierarchyChangedTestHost* test_host = new ViewHierarchyChangedTestHost;
    std::unique_ptr<Widget> test_child = CreateChildForHost(host0->native_view(), host0, new View, test_host);
...
TEST_F(NativeViewHostTest, ViewHierarchyChangedForHostParent)
{
    ...
    std::unique_ptr<Widget> child0 = CreateChildForHost(toplevel()->GetNativeView(), view0, new View, host0);
    ViewHierarchyChangedTestHost* host1 = new ViewHierarchyChangedTestHost;
    std::unique_ptr<Widget> child1 = CreateChildForHost(toplevel()->GetNativeView(), view1, new View, host1);
