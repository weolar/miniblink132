...
void TestSliderListener::SliderValueChanged(views::Slider* sender, float value, float old_value, views::SliderChangeReason reason)
{
    ++last_event_epoch_;
}
...
class SliderTest : public views::ViewsTestBase, public testing::WithParamInterface<TestSliderType> {
public:
    SliderTest() = default;
...
protected:
    Slider* slider()
    {
        return static_cast<Slider*>(widget_->GetContentsView());
    }
    ...
    const base::flat_set<float>& values() const
    {
        return values_;
    }
...
{
    gfx::Point point = slider()->GetBoundsInScreen().origin() + gfx::Vector2d(x, y);
...
void SliderTest::SetUp()
{
    ...
    views::Widget::InitParams init_params(CreateParams(Widget::InitParams::CLIENT_OWNS_WIDGET, views::Widget::InitParams::TYPE_WINDOW_FRAMELESS));
...
TEST_P(SliderTest, NukeAllowedValues)
{
    ...
    // calculations.
    EXPECT_FLOAT_EQ(
        static_cast<float>(position - test::SliderTestApi(slider()).initial_button_offset()) / (slider()->width() - kThumbWidth), slider()->GetValue());
}
...
TEST_P(SliderTest, AccessibleValue)
{
    ...
    slider()->GetViewAccessibility().GetAccessibleNodeData(&data);
    if (GetParam() == TestSliderType::kContinuousTest) {
        EXPECT_EQ(std::string(""), data.GetStringAttribute(ax::mojom::StringAttribute::kValue));
    } else if (GetParam() == TestSliderType::kDiscreteEnd2EndTest) {
        EXPECT_EQ(std::string(""), data.GetStringAttribute(ax::mojom::StringAttribute::kValue));
    } else {
        EXPECT_EQ(std::string("10%"), data.GetStringAttribute(ax::mojom::StringAttribute::kValue));
    }
    ...
    data = ui::AXNodeData();
    slider()->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(std::string("50%"), data.GetStringAttribute(ax::mojom::StringAttribute::kValue));
}
...
TEST_P(SliderTest, SliderValueForScrollGesture)
{
    ...
    slider()->SetValue(0.5);
    event_generator()->GestureScrollSequence(gfx::Point(0.5 * max_x(), 0.5 * max_y()), gfx::Point(0, 0), base::Milliseconds(10), 5 /* steps */);
    EXPECT_EQ(GetMinValue(), slider()->GetValue());

    // Scroll above the maximum.
    slider()->SetValue(0.5);
    event_generator()->GestureScrollSequence(gfx::Point(0.5 * max_x(), 0.5 * max_y()), gfx::Point(max_x(), max_y()), base::Milliseconds(10), 5 /* steps */);
    EXPECT_EQ(GetMaxValue(), slider()->GetValue());
    ...
    event_generator()->GestureScrollSequence(
        gfx::Point(0.25 * max_x(), 0.25 * max_y()), gfx::Point(0.78 * max_x(), 0.78 * max_y()), base::Milliseconds(10), 5 /* steps */);
    if (GetParam() == TestSliderType::kContinuousTest) {
        // Continuous slider.
...
TEST_P(SliderTest, SliderListenerEventsForScrollGesture)
{
    ...
    event_generator()->GestureScrollSequence(
        gfx::Point(0.25 * max_x(), 0.25 * max_y()), gfx::Point(0.75 * max_x(), 0.75 * max_y()), base::Milliseconds(0), 5 /* steps */);

    EXPECT_EQ(1, slider_listener.last_drag_started_epoch());
    EXPECT_GT(slider_listener.last_drag_ended_epoch(), slider_listener.last_drag_started_epoch());
    EXPECT_EQ(slider(), slider_listener.last_drag_started_sender());
    EXPECT_EQ(slider(), slider_listener.last_drag_ended_sender());
    test::SliderTestApi(slider()).SetListener(nullptr);
}
...
TEST_P(SliderTest, SliderListenerEventsForMultiFingerScrollGesture)
{
    ...
    gfx::Point points[] = { gfx::Point(0, 0.1 * max_y()), gfx::Point(0, 0.2 * max_y()) };
    event_generator()->GestureMultiFingerScroll(2 /* count */, points, 0 /* event_separation_time_ms */, 5 /* steps */, 2 /* move_x */, 0 /* move_y */);
...
INSTANTIATE_TEST_SUITE_P(
    All, SliderTest, ::testing::Values(TestSliderType::kContinuousTest, TestSliderType::kDiscreteEnd2EndTest, TestSliderType::kDiscreteInnerTest));
} // namespace views
