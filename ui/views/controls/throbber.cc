...
void Throbber::Start()
{
    ...
    timer_.Start(FROM_HERE, base::Milliseconds(GetFrameDelay(diameter_)), base::BindRepeating(&Throbber::SchedulePaint, base::Unretained(this)));
...
gfx::Size Throbber::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
...
{
    SkColor color = GetColorProvider()->GetColor(color_id_.value_or(ui::kColorThrobber));
    ...
    if (!IsRunning()) {
        if (checked_) {
            ...
            canvas->Translate(gfx::Vector2d((width() - kCheckmarkDipSize) / 2, (height() - kCheckmarkDipSize) / 2));
            gfx::PaintVectorIcon(canvas, vector_icons::kCheckCircleIcon, kCheckmarkDipSize, color);
        }
        return;
    }
...
void SmoothedThrobber::Start()
{
    ...
    if (!IsRunning() && !start_timer_.IsRunning()) {
        start_timer_.Start(FROM_HERE, start_delay_, this, &SmoothedThrobber::StartDelayOver);
    }
}
...
void SmoothedThrobber::Stop()
{
    ...
    stop_timer_.Stop();
    stop_timer_.Start(FROM_HERE, stop_delay_, this, &SmoothedThrobber::StopDelayOver);
}
