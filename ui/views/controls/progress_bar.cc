...
void AddPossiblyRoundRectToPath(const gfx::Rect& rectangle, const gfx::RoundedCornersF& preferred_corner_radii, SkPath* path)
{
    if (preferred_corner_radii.IsEmpty() || rectangle.height() == 0) {
        path->addRect(gfx::RectToSkRect(rectangle));
        return;
    }
    SkVector radii[4] = { { preferred_corner_radii.upper_left(), preferred_corner_radii.upper_left() },
        { preferred_corner_radii.upper_right(), preferred_corner_radii.upper_right() },
        { preferred_corner_radii.lower_right(), preferred_corner_radii.lower_right() },
        { preferred_corner_radii.lower_left(), preferred_corner_radii.lower_left() } };
...
gfx::Size ProgressBar::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
...
void ProgressBar::OnPaint(gfx::Canvas* canvas)
{
    ...
    // Draw slice.
    SkPath slice_path;
    const int slice_width = static_cast<int>(content_bounds.width() * std::min(current_value_, 1.0) + 0.5);
    if (slice_width < 1) {
        return;
    }
...
SkColor ProgressBar::GetForegroundColor() const
{
    ...
    return GetColorProvider()->GetColor(foreground_color_id_.value_or(GetPaused() ? ui::kColorProgressBarPaused : ui::kColorProgressBar));
}
...
SkColor ProgressBar::GetBackgroundColor() const
{
    ...
    return background_color_.value_or(GetColorProvider()->GetColor(ui::kColorProgressBarBackground));
}
...
gfx::RoundedCornersF ProgressBar::GetPreferredCornerRadii() const
{
    ...
    return gfx::RoundedCornersF(std::min(max_radius, preferred_corner_radii_->upper_left()), std::min(max_radius, preferred_corner_radii_->upper_right()),
        std::min(max_radius, preferred_corner_radii_->lower_right()), std::min(max_radius, preferred_corner_radii_->lower_left()));
}

void ProgressBar::SetPreferredCornerRadii(std::optional<gfx::RoundedCornersF> preferred_corner_radii)
{
...
void ProgressBar::OnPaintIndeterminate(gfx::Canvas* canvas)
{
    ...
    int bar1_end_x = std::round(content_bounds.width() * std::min(1.0, bar1_left + bar1_width));
    int bar2_start_x = std::round(content_bounds.width() * bar2_left);
    int bar2_end_x = std::round(content_bounds.width() * std::min(1.0, bar2_left + bar2_width));
...
void ProgressBar::MaybeNotifyAccessibilityValueChanged()
{
    ...
    if (!GetWidget() || !GetWidget()->IsVisible() || RoundToPercent(current_value_) == last_announced_percentage_) {
        return;
    }
    last_announced_percentage_ = RoundToPercent(current_value_);
    GetViewAccessibility().SetValue(base::FormatPercent(last_announced_percentage_));
}
