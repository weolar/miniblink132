...
public:
    EvilListener()
    {
        combobox()->SetCallback(base::BindRepeating(&EvilListener::OnPerformAction, base::Unretained(this)));
    }
...
private:
    ...
    std::unique_ptr<TestCombobox> combobox_ = std::make_unique<TestCombobox>(&model_);
};

class TestComboboxListener {
public:
    explicit TestComboboxListener(Combobox* combobox)
        : combobox_(combobox)
    {
    }
...
protected:
    ...
    void ReleaseKey(ui::KeyboardCode key_code, ui::EventFlags flags = ui::EF_NONE)
    {
    ...
    TestCombobox* combobox()
    {
        return widget_ ? static_cast<TestCombobox*>(widget_->GetContentsView()->GetViewByID(1)) : nullptr;
    }
...
TEST_F(ComboboxTest, GetTextForRowTest)
{
    ...
    for (size_t i = 0; i < combobox()->GetRowCount(); ++i) {
        if (separators.count(i) != 0) {
            ...
            EXPECT_EQ(ASCIIToUTF16(i % 2 == 0 ? "PEANUT BUTTER" : "JELLY"), combobox()->GetTextForRow(i)) << i;
        }
    }
}
...
TEST_F(ComboboxTest, ListenerHandlesDelete)
{
    ...
    ASSERT_NO_FATAL_FAILURE({
        ui::MenuModel* model = ComboboxTestApi(evil_listener->combobox()).menu_model();
        model->ActivatedAt(2);
    });
...
TEST_F(ComboboxTest, Click)
{
    ...
    combobox()->SetCallback(base::BindRepeating(&TestComboboxListener::OnPerformAction, base::Unretained(&listener)));
    views::test::RunScheduledLayout(combobox());

    // Click the left side. The menu is shown.
    EXPECT_EQ(0, menu_show_count_);
    PerformClick(gfx::Point(combobox()->x() + 1, combobox()->y() + combobox()->height() / 2));
...
TEST_F(ComboboxTest, NotifyOnClickWithReturnKey)
{
    ...
    EXPECT_EQ(PlatformStyle::kReturnClicksFocusedControl ? 1 : 0, menu_show_count_);
    EXPECT_FALSE(listener.on_perform_action_called());
}
...
TEST_F(ComboboxTest, ExpandedCollapsedAccessibleState)
{
    ...
    combobox()->OnKeyPressed(ui::KeyEvent(ui::EventType::kKeyPressed, ui::VKEY_SPACE, ui::EF_NONE));
...
TEST_F(ComboboxTest, AccessibleDefaultActionVerb)
{
    ...
    ui::AXNodeData node_data;
    combobox()->GetViewAccessibility().GetAccessibleNodeData(&node_data);
    EXPECT_EQ(ax::mojom::DefaultActionVerb::kOpen, node_data.GetDefaultActionVerb());

    node_data = ui::AXNodeData();
    combobox()->SetEnabled(false);
    combobox()->GetViewAccessibility().GetAccessibleNodeData(&node_data);
    EXPECT_FALSE(node_data.HasIntAttribute(ax::mojom::IntAttribute::kDefaultActionVerb));
...
TEST_F(ComboboxTest, AccessibleValue)
{
    ...
    auto simple_model = std::make_unique<ui::SimpleComboboxModel>(std::vector<ui::SimpleComboboxModel::Item>());
    ...
    EXPECT_EQ("", node_data.GetStringAttribute(ax::mojom::StringAttribute::kValue));

    // Non-empty model.
    simple_model->UpdateItemList({ ui::SimpleComboboxModel::Item(u"Peanut Butter"), ui::SimpleComboboxModel::Item(u"Yogurt") });
    node_data = ui::AXNodeData();
    combobox->GetViewAccessibility().GetAccessibleNodeData(&node_data);
    EXPECT_EQ(2u, combobox->GetModel()->GetItemCount());
    EXPECT_EQ(0u, combobox->GetSelectedIndex());
    EXPECT_EQ("Peanut Butter", node_data.GetStringAttribute(ax::mojom::StringAttribute::kValue));
    ...
    EXPECT_EQ(1u, combobox->GetSelectedIndex());
    combobox->GetViewAccessibility().GetAccessibleNodeData(&node_data);
    EXPECT_EQ("Yogurt", node_data.GetStringAttribute(ax::mojom::StringAttribute::kValue));
}
...
TEST_F(ComboboxTest, NotifyOnClickWithMouse)
{
    ...
    const gfx::Point right_point(combobox()->x() + combobox()->width() - 1, combobox()->y() + combobox()->height() / 2);
    ...
    const gfx::Point left_point(gfx::Point(combobox()->x() + 1, combobox()->y() + combobox()->height() / 2));
...
TEST_F(ComboboxTest, ConsumingPressKeyEvents)
{
    ...
    EXPECT_TRUE(combobox()->OnKeyPressed(ui::KeyEvent(ui::EventType::kKeyPressed, ui::VKEY_SPACE, ui::EF_NONE)));
    EXPECT_EQ(1, menu_show_count_);

    ui::KeyEvent return_press(ui::EventType::kKeyPressed, ui::VKEY_RETURN, ui::EF_NONE);
...
{
    const gfx::FontList& font_list = TypographyProvider::Get().GetFont(Combobox::kContext, Combobox::kStyle);
    InitCombobox(nullptr);
    combobox()->SetSizeToLargestLabel(false);
    ComboboxTestApi(combobox()).PerformActionAt(1);
    EXPECT_EQ(gfx::GetStringWidth(model_->GetItemAt(1), font_list), ComboboxTestApi(combobox()).content_size().width());
    combobox()->SetSelectedIndex(1);
    EXPECT_EQ(gfx::GetStringWidth(model_->GetItemAt(1), font_list), ComboboxTestApi(combobox()).content_size().width());
    ...
    EXPECT_EQ(gfx::GetStringWidth(model_->GetItemAt(0), font_list), ComboboxTestApi(combobox()).content_size().width());
    combobox()->SetSelectedIndex(0);
    EXPECT_EQ(gfx::GetStringWidth(model_->GetItemAt(0), font_list), ComboboxTestApi(combobox()).content_size().width());
}
...
TEST_F(ComboboxTest, TypingPrefixNotifiesListener)
{
    ...
    ui::KeyEvent key_event(ui::EventType::kKeyPressed, ui::VKEY_J, ui::DomCode::US_J, 0, ui::DomKey::FromCharacter('J'), ui::EventTimeForNow());
    ...
    key_event = ui::KeyEvent(ui::EventType::kKeyPressed, ui::VKEY_E, ui::DomCode::US_E, 0, ui::DomKey::FromCharacter('E'), ui::EventTimeForNow());
    input_client->InsertChar(key_event);
    EXPECT_EQ(1, listener.actions_performed());
    EXPECT_EQ(1u, listener.perform_action_index());
    ...
    key_event = ui::KeyEvent(ui::EventType::kKeyPressed, ui::VKEY_E, ui::DomCode::US_P, 0, ui::DomKey::FromCharacter('P'), ui::EventTimeForNow());
    input_client->InsertChar(key_event);
...
TEST_F(ComboboxTest, MenuModel)
{
    ...
    EXPECT_EQ(ui::MenuModel::TYPE_SEPARATOR, menu_model->GetTypeAt(kSeparatorIndex));
...
TEST_F(ComboboxTest, SetTooltipTextNotifiesAccessibilityEvent)
{
    ...
    EXPECT_EQ(1, counter.GetCount(ax::mojom::Event::kTextChanged, ax::mojom::Role::kButton));
    EXPECT_EQ(1, counter.GetCount(ax::mojom::Event::kTextChanged, ax::mojom::Role::kComboBoxSelect));
    EXPECT_EQ(test_tooltip_text, combobox()->GetViewAccessibility().GetCachedName());
    ui::AXNodeData data;
    ...
    EXPECT_EQ(u"PEANUT BUTTER", data.GetString16Attribute(ax::mojom::StringAttribute::kValue));
}
...
std::string GetComboboxA11yValue(Combobox* combobox)
{
    ...
    return selected_index ? base::UTF16ToUTF8(combobox->GetModel()->GetItemAt(selected_index.value())) : std::string();
}
...
public:
    ...
    void SetDefaultIndex(size_t default_index)
    {
        default_index_ = default_index;
    }
...
TEST_F(ComboboxDefaultTest, SetModel)
{
    ...
    std::unique_ptr<ConfigurableComboboxModel> model = std::make_unique<ConfigurableComboboxModel>(&destroyed);
...
TEST_F(ComboboxDefaultTest, SetOwnedModelOverwriteOwned)
{
    ...
    {
        auto combobox = std::make_unique<Combobox>();
        combobox->SetOwnedModel(std::make_unique<ConfigurableComboboxModel>(&destroyed_first));
        ASSERT_FALSE(destroyed_first);
        combobox->SetOwnedModel(std::make_unique<ConfigurableComboboxModel>(&destroyed_second));
...
TEST_F(ComboboxDefaultTest, InteractionWithEmptyModel)
{
    ...
    IgnoreMissingWidgetForTestingScopedSetter ignore_missing_widget(combobox->GetViewAccessibility());

    combobox->GetViewAccessibility().GetAccessibleNodeData(&node_data);
    EXPECT_EQ(0u, combobox->GetModel()->GetItemCount());
    EXPECT_EQ(std::nullopt, combobox->GetSelectedIndex());
    EXPECT_EQ(GetComboboxA11yValue(combobox.get()), node_data.GetStringAttribute(ax::mojom::StringAttribute::kValue));
