...
{
    return LayoutProvider::Get()->GetCornerRadiusMetric(ShapeContextTokens::kComboboxRadius);
}
...
SkColor GetTextColorForEnableState(const Combobox& combobox, bool enabled)
{
    ...
    return combobox.GetColorProvider()->GetColor(TypographyProvider::Get().GetColorId(style::CONTEXT_TEXTFIELD, style));
}
...
Combobox::Combobox(ui::ComboboxModel* model)
{
    ...
    arrow_button_ = AddChildView(std::make_unique<TransparentButton>(base::BindRepeating(&Combobox::ArrowButtonPressed, base::Unretained(this))));
    ...
    enabled_changed_subscription_ = AddEnabledChangedCallback(base::BindRepeating(
        [](Combobox* combobox) {
            combobox->SetBackgroundColorId(combobox->GetEnabled() ? ui::kColorComboboxBackground : ui::kColorComboboxBackgroundDisabled);
...
void Combobox::SetSelectedIndex(std::optional<size_t> index)
{
    ...
    if (selected_index_.has_value()) {
        GetViewAccessibility().SetPosInSet(base::checked_cast<int>(selected_index_.value()));
    }
...
base::CallbackListSubscription Combobox::AddSelectedIndexChangedCallback(views::PropertyChangedCallback callback)
{
...
void Combobox::SetTooltipTextAndAccessibleName(const std::u16string& tooltip_text)
{
...
void Combobox::SetEventHighlighting(bool should_highlight)
{
    ...
    AsViewClass<TransparentButton>(arrow_button_)->UpdateInkDrop(should_highlight);
}
...
{
    return GetModel()->IsItemSeparatorAt(row) ? std::u16string() : GetModel()->GetItemAt(row);
}

base::CallbackListSubscription Combobox::AddMenuWillShowCallback(MenuWillShowCallback callback)
{
...
gfx::Size Combobox::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
    ...
    int width = std::max(kMinComboboxWidth, content_size_.width()) + LayoutProvider::Get()->GetDistanceMetric(DISTANCE_TEXTFIELD_HORIZONTAL_TEXT_PADDING) * 2
        + GetInsets().width();
    ...
    return gfx::Size(width, LayoutProvider::GetControlHeightForFont(kContext, kStyle, GetForegroundFontList()));
}
...
bool Combobox::SkipDefaultKeyEventProcessing(const ui::KeyEvent& e)
{
    ...
    if (e.key_code() != ui::VKEY_ESCAPE || e.IsShiftDown() || e.IsControlDown() || e.IsAltDown() || e.IsAltGrDown()) {
        return false;
    }
    return !!menu_runner_;
}
...
bool Combobox::OnKeyPressed(const ui::KeyEvent& e)
{
...
#if BUILDFLAG(IS_MAC)
    if (e.key_code() != ui::VKEY_DOWN && e.key_code() != ui::VKEY_UP && e.key_code() != ui::VKEY_SPACE && e.key_code() != ui::VKEY_HOME
    ...
    return true;
#else
    const auto index_at_or_after = [](ui::ComboboxModel* model, size_t index) -> std::optional<size_t> {
    ...
    const auto index_before = [](ui::ComboboxModel* model, size_t index) -> std::optional<size_t> {
        for (; index > 0; --index) {
...
void Combobox::OnComboboxModelChanged(ui::ComboboxModel* model)
{
    ...
    // default index.
    if (!selected_index_.has_value() || selected_index_ >= model_->GetItemCount() || model_->GetItemCount() == 0
    ...
    GetViewAccessibility().SetSetSize(base::checked_cast<int>(model_->GetItemCount()));
}
...
void Combobox::UpdateBorder()
{
    ...
    SetBorder(CreateThemedRoundedRectBorder(
        kBorderThickness, GetCornerRadius(), invalid_ ? ui::kColorAlertHighSeverity : border_color_id_.value_or(ui::kColorComboboxContainerOutline)));
}
...
void Combobox::PaintIconAndText(gfx::Canvas* canvas)
{
    ...
    insets += gfx::Insets::VH(0, LayoutProvider::Get()->GetDistanceMetric(DISTANCE_TEXTFIELD_HORIZONTAL_TEXT_PADDING));
    ...
    if (!icon.IsEmpty()) {
        ...
        if (foreground_icon_color_id_ && icon.IsVectorIcon()) {
            icon = ui::ImageModel::FromVectorIcon(*icon.GetVectorIcon().vector_icon(), foreground_icon_color_id_.value(), icon.GetVectorIcon().icon_size());
        }
    ...
    // Draw the text.
    SkColor text_color = foreground_color_id_ ? GetColorProvider()->GetColor(*foreground_color_id_) : GetTextColorForEnableState(*this, GetEnabled());
    ...
    if (!text.empty() && !icon.IsEmpty()) {
        x += LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_LABEL_HORIZONTAL);
    }
    ...
    // ChromeRefresh2023.
    if (should_show_arrow_) {
        gfx::Rect arrow_bounds(width() - GetComboboxArrowContainerWidthAndMargins(), 0, GetComboboxArrowContainerWidth(), height());
...
void Combobox::ShowDropDownMenu(ui::mojom::MenuSourceType source_type)
{
    ...
    if (!menu_runner_ || IsMenuRunning()) {
        menu_runner_ = std::make_unique<MenuRunner>(
            menu_model_.get(), MenuRunner::COMBOBOX, base::BindRepeating(&Combobox::OnMenuClosed, base::Unretained(this), original_state));
    }
    if (should_highlight_) {
        InkDrop::Get(arrow_button_)->AnimateToState(InkDropState::ACTIVATED, nullptr);
    }
...
{
    if (should_highlight_) {
        InkDrop::Get(arrow_button_)->AnimateToState(InkDropState::DEACTIVATED, nullptr);
    ...
    for (size_t i = 0; i < GetModel()->GetItemCount(); ++i) {
        ...
        if (size_to_largest_label_ || i == selected_index_) {
            ...
            if (!icon.IsEmpty()) {
                ...
                if (!text.empty()) {
                    item_width += LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_LABEL_HORIZONTAL);
                }
            }
...
int Combobox::MaybeAdjustWidthForCheckmarks(int original_width) const
{
    return MenuConfig::instance().check_selected_combobox_item
        ? original_width + kMenuCheckSize + LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_BUTTON_HORIZONTAL)
        : original_width;
}
