...
int GetComboboxArrowContainerWidthAndMargins()
{
    ...
    // combobox.
    return GetComboboxArrowContainerWidth() + LayoutProvider::Get()->GetDistanceMetric(DISTANCE_TEXTFIELD_HORIZONTAL_TEXT_PADDING);
}
...
void PaintComboboxArrow(SkColor color, const gfx::Rect& bounds, gfx::Canvas* canvas)
{
...
void ConfigureComboboxButtonInkDrop(Button* host_view)
{
    ...
    views::InkDrop::Get(host_view)->SetBaseColorCallback(base::BindRepeating(
        [](Button* host) { return color_utils::DeriveDefaultIconColor(host->GetColorProvider()->GetColor(ui::kColorComboboxInkDropHovered)); }, host_view));
    InkDrop::Get(host_view)->SetCreateRippleCallback(base::BindRepeating(
        [](Button* host, float opacity) -> std::unique_ptr<views::InkDropRipple> {
            return std::make_unique<views::FloodFillInkDropRipple>(InkDrop::Get(host), host->size(), InkDrop::Get(host)->GetInkDropCenterBasedOnLastEvent(),
                host->GetColorProvider()->GetColor(ui::kColorComboboxInkDropRipple), opacity);
        },
        host_view, kOpaque));
}
