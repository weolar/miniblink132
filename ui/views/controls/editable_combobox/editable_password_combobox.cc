...
// visibility.
std::unique_ptr<ToggleImageButton> CreateEye(ImageButton::PressedCallback callback)
{
    ...
    SetImageFromVectorIconWithColorId(button.get(), kEyeIcon, ui::kColorIcon, ui::kColorIconDisabled);
    SetToggledImageFromVectorIconWithColorId(button.get(), kEyeCrossedIcon, ui::kColorIcon, ui::kColorIconDisabled);
...
class PasswordMenuDecorationStrategy : public EditableCombobox::MenuDecorationStrategy {
public:
    explicit PasswordMenuDecorationStrategy(const EditablePasswordCombobox* parent)
        : parent_(parent)
    {
        DCHECK(parent);
    }
    ...
    {
        return parent_->ArePasswordsRevealed() ? text : std::u16string(text.length(), gfx::RenderText::kPasswordReplacementChar);
    }
...
EditablePasswordCombobox::EditablePasswordCombobox(
    std::unique_ptr<ui::ComboboxModel> combobox_model, int text_context, int text_style, bool display_arrow, Button::PressedCallback eye_callback)
    : EditableCombobox(std::move(combobox_model),
        /*filter_on_edit=*/false,
        /*show_on_empty=*/true, text_context, text_style, display_arrow)
{
    // By default, clicking on the eye reveals/hides passwords.
    if (!eye_callback) {
        eye_callback
            = base::BindRepeating([](views::EditablePasswordCombobox* combobox_ptr) { combobox_ptr->RevealPasswords(!combobox_ptr->ArePasswordsRevealed()); },
    ...
    SetMenuDecorationStrategy(std::make_unique<PasswordMenuDecorationStrategy>(this));
}

EditablePasswordCombobox::~EditablePasswordCombobox() = default;

void EditablePasswordCombobox::SetPasswordIconTooltips(const std::u16string& tooltip_text, const std::u16string& toggled_tooltip_text)
{
...
void EditablePasswordCombobox::RevealPasswords(bool revealed)
{
    ...
    GetTextfield().SetTextInputType(revealed ? ui::TEXT_INPUT_TYPE_TEXT : ui::TEXT_INPUT_TYPE_PASSWORD);
