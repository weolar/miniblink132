...
public:
    ...
    EditablePasswordComboboxTest& operator=(const EditablePasswordComboboxTest&) = delete;
...
protected:
    ...
    {
        return static_cast<EditablePasswordCombobox*>(widget_->GetContentsView()->GetViewByID(kComboboxId));
    }
...
void EditablePasswordComboboxTest::SetUp()
{
    ...
    auto combobox = std::make_unique<EditablePasswordCombobox>(std::make_unique<ui::SimpleComboboxModel>(std::vector<ui::SimpleComboboxModel::Item> {
                                                                   ui::SimpleComboboxModel::Item(u"item0"), ui::SimpleComboboxModel::Item(u"item1") }),
        style::CONTEXT_BUTTON, style::STYLE_PRIMARY, /* display_arrow=*/true, Button::PressedCallback(eye_callback_.Get()));
    ...
    generator_ = std::make_unique<ui::test::EventGenerator>(GetRootWindow(widget_.get()));
...
{
    const std::u16string kObscuredPassword(5, gfx::RenderText::kPasswordReplacementChar);
...
{
    auto combobox = std::make_unique<EditablePasswordCombobox>(std::make_unique<ui::SimpleComboboxModel>(
        std::vector<ui::SimpleComboboxModel::Item> { ui::SimpleComboboxModel::Item(u"item0"), ui::SimpleComboboxModel::Item(u"item1") }));
