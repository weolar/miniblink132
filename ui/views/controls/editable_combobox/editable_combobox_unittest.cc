...
public:
    ...
    void InitEditableCombobox(int item_count = 8, bool filter_on_edit = false, bool show_on_empty = true);

    // Initializes the combobox with the given items.
    void InitEditableCombobox(const std::vector<std::u16string>& items, bool filter_on_edit, bool show_on_empty = true);

    void InitEditableCombobox(const std::vector<ui::SimpleComboboxModel::Item>& items, bool filter_on_edit, bool show_on_empty = true);
...
protected:
    ...
    void PerformMouseEvent(Widget* widget, const gfx::Point& point, ui::EventType type);
    void PerformClick(Widget* widget, const gfx::Point& point);
    void SendKeyEvent(ui::KeyboardCode key_code, bool alt = false, bool shift = false, bool ctrl_cmd = false);
...
// Initializes the combobox with the given number of items.
void EditableComboboxTest::InitEditableCombobox(const int item_count, const bool filter_on_edit, const bool show_on_empty)
{
...
void EditableComboboxTest::InitEditableCombobox(const std::vector<std::u16string>& strings, bool filter_on_edit, bool show_on_empty)
{
    std::vector<ui::SimpleComboboxModel::Item> items;
...
// Initializes the combobox with the given items.
void EditableComboboxTest::InitEditableCombobox(const std::vector<ui::SimpleComboboxModel::Item>& items, const bool filter_on_edit, const bool show_on_empty)
{
    InitWidget();
    ...
    combobox_ = parent_of_combobox_->AddChildView(
        std::make_unique<EditableCombobox>(std::make_unique<ui::SimpleComboboxModel>(items), filter_on_edit, show_on_empty));
    combobox_->SetCallback(base::BindRepeating(&EditableComboboxTest::OnContentChanged, base::Unretained(this)));
...
ui::ImageModel EditableComboboxTest::GetIconAt(size_t index, IconSource icon_source) const
{
    switch (icon_source) {
...
{
    ui::MouseEvent e(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), 0, 0);
...
void EditableComboboxTest::SetContextMenuController(ContextMenuController* controller)
{
...
{
    ui::MouseEvent drag(ui::EventType::kMouseDragged, location, location, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, 0);
...
void EditableComboboxTest::PerformMouseEvent(Widget* widget, const gfx::Point& point, const ui::EventType type)
{
    ui::MouseEvent event = ui::MouseEvent(type, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON | ui::EF_NUM_LOCK_ON, ui::EF_LEFT_MOUSE_BUTTON);
    widget->OnMouseEvent(&event);
}

void EditableComboboxTest::PerformClick(Widget* widget, const gfx::Point& point)
{
    PerformMouseEvent(widget, point, ui::EventType::kMousePressed);
    PerformMouseEvent(widget, point, ui::EventType::kMouseReleased);
}

void EditableComboboxTest::SendKeyEvent(ui::KeyboardCode key_code, const bool alt, const bool shift, const bool ctrl_cmd)
{
#if BUILDFLAG(IS_MAC)
...
TEST_F(EditableComboboxTest, ClickOutsideEditableComboboxWithoutLosingFocusClosesMenu)
{
    InitEditableCombobox();
    ClickArrow();
    EXPECT_TRUE(IsMenuOpen());
    ...
    const gfx::Point outside_point(combobox_->x() + combobox_->width() + 1, combobox_->y() + 1);
...
TEST_F(EditableComboboxTest, RemovingParentOfControlWhileMenuOpenClosesMenu)
{
    ...
    ClickArrow();
    EXPECT_TRUE(IsMenuOpen());
    auto parent = widget_->GetContentsView()->RemoveChildViewT(parent_of_combobox_);
    EXPECT_EQ(nullptr, GetMenuRunner());
    combobox_ = nullptr;
    parent_of_combobox_ = nullptr;
}
...
#if BUILDFLAG(IS_WIN)
// Flaky on Windows. https://crbug.com/965601
#define MAYBE_F4ClosesMenuWhileSelectingHighlightedMenuItem DISABLED_F4ClosesMenuWhileSelectingHighlightedMenuItem
#else
#define MAYBE_F4ClosesMenuWhileSelectingHighlightedMenuItem F4ClosesMenuWhileSelectingHighlightedMenuItem
#endif
TEST_F(EditableComboboxTest, MAYBE_F4ClosesMenuWhileSelectingHighlightedMenuItem)
{
    InitEditableCombobox();
    FocusTextfield();
    SendKeyEvent(ui::VKEY_A);
...
#define MAYBE_RefocusingReopensMenuBasedOnLatestContent DISABLED_RefocusingReopensMenuBasedOnLatestContent
#else
#define MAYBE_RefocusingReopensMenuBasedOnLatestContent RefocusingReopensMenuBasedOnLatestContent
#endif
...
{
    ui::SimpleComboboxModel::Item item1(u"abc", std::u16string(), ui::ImageModel::FromImage(gfx::test::CreateImage(16, 16)));

    ui::SimpleComboboxModel::Item item2(u"def", std::u16string(), ui::ImageModel::FromImage(gfx::test::CreateImage(20, 20)));
...
TEST_F(EditableComboboxTest, ShowContextMenuOnMouseRelease)
{
    ...
    ui::MouseEvent click_mouse_event(
        ui::EventType::kMousePressed, textfield_point, textfield_point, ui::EventTimeForNow(), ui::EF_RIGHT_MOUSE_BUTTON, ui::EF_RIGHT_MOUSE_BUTTON);
    widget_->OnMouseEvent(&click_mouse_event);
    EXPECT_FALSE(IsMenuOpen());
    ui::MouseEvent release_mouse_event(
        ui::EventType::kMouseReleased, textfield_point, textfield_point, ui::EventTimeForNow(), ui::EF_RIGHT_MOUSE_BUTTON, ui::EF_RIGHT_MOUSE_BUTTON);
...
TEST_F(EditableComboboxTest, AccessibleNameAndRole)
{
    ...
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), u"abc");
    EXPECT_EQ(combobox_->GetViewAccessibility().GetCachedName(), u"abc");

    data = ui::AXNodeData();
    combobox_->GetViewAccessibility().SetName(u"New name");
    combobox_->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), u"New name");
...
TEST_F(EditableComboboxTest, AccessibleValue)
{
    ...
    std::u16string val = data.GetString16Attribute(ax::mojom::StringAttribute::kValue);
...
public:
    ...
    ConfigurableComboboxModel& operator=(const ConfigurableComboboxModel&) = delete;
    ...
    void SetItemCount(size_t item_count)
    {
...
{
    std::unique_ptr<ConfigurableComboboxModel> model = std::make_unique<ConfigurableComboboxModel>();
...
TEST_F(EditableComboboxDefaultTest, SetModelOverwrite)
{
    ...
    {
        auto combobox = std::make_unique<EditableCombobox>();
        combobox->SetModel(std::make_unique<ConfigurableComboboxModel>(&destroyed_first));
        ASSERT_FALSE(destroyed_first);
        combobox->SetModel(std::make_unique<ConfigurableComboboxModel>(&destroyed_second));
