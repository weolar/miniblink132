...
public:
    ...
    explicit EditableCombobox(std::unique_ptr<ui::ComboboxModel> combobox_model, bool filter_on_edit = false, bool show_on_empty = true,
        int text_context = kDefaultTextContext, int text_style = kDefaultTextStyle, bool display_arrow = true);
...
protected:
    ...
    // menu.
    void SetMenuDecorationStrategy(std::unique_ptr<MenuDecorationStrategy> strategy);
    ...
    template <typename T> T* AddControlElement(std::unique_ptr<T> view)
    {
        T* raw_view = control_elements_container_->AddChildViewAt(std::move(view), 0);
    ...
    Button* GetArrowButtonForTesting()
    {
        return arrow_;
    }
...
private:
    ...
    void ShowDropDownMenu(ui::mojom::MenuSourceType source_type = ui::mojom::MenuSourceType::kNone);
    ...
    void OnLayoutIsAnimatingChanged(views::AnimatingLayoutManager* source, bool is_animating) override;
