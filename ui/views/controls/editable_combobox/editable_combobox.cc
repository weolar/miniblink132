...
public:
    explicit Arrow(PressedCallback callback)
        : Button(std::move(callback))
    {
        SetPreferredSize(gfx::Size(kEditableComboboxButtonSize, kEditableComboboxButtonSize));

        button_controller()->set_notify_action(ButtonController::NotifyAction::kOnPress);
...
private:
    void PaintButtonContents(gfx::Canvas* canvas) override
    {
        ...
        PaintComboboxArrow(GetColorProvider()->GetColor(
                               TypographyProvider::Get().GetColorId(style::CONTEXT_TEXTFIELD, GetEnabled() ? style::STYLE_PRIMARY : style::STYLE_DISABLED)),
            arrow_bounds, canvas);
    }
    ...
    {
        if (GetEnabled()) {
            GetViewAccessibility().SetDefaultActionVerb(ax::mojom::DefaultActionVerb::kOpen);
        } else {
    ...
    base::CallbackListSubscription enabled_changed_subscription_
        = AddEnabledChangedCallback(base::BindRepeating(&Arrow::UpdateAccessibleDefaultActionVerb, base::Unretained(this)));
};
...
std::u16string EditableCombobox::MenuDecorationStrategy::DecorateItemText(std::u16string text) const
{
    return text;
}
...
class EditableCombobox::EditableComboboxMenuModel final : public ui::MenuModel, public ui::ComboboxModelObserver {
public:
    EditableComboboxMenuModel(EditableCombobox* owner, std::unique_ptr<ui::ComboboxModel> combobox_model, const bool filter_on_edit, const bool show_on_empty)
    ...
    EditableComboboxMenuModel& operator=(const EditableComboboxMenuModel&) = delete;
    ...
    void UpdateItemsShown()
    {
        ...
        if (show_on_empty_ || !owner_->GetText().empty()) {
            for (size_t i = 0; i < combobox_model_->GetItemCount(); ++i) {
                if (!filter_on_edit_ || base::StartsWith(combobox_model_->GetItemAt(i), owner_->GetText(), base::CompareCase::INSENSITIVE_ASCII)) {
    ...
    void SetDecorationStrategy(std::unique_ptr<EditableCombobox::MenuDecorationStrategy> strategy)
    {
        DCHECK(strategy);
    ...
    void DisableUpdateItemsShown()
    {
        update_items_shown_enabled_ = false;
    }

    void EnableUpdateItemsShown()
    {
        update_items_shown_enabled_ = true;
    }
    ...
    {
        return items_shown_.size();
    }

    // ui::MenuModel:
    std::u16string GetLabelAt(size_t index) const override
    {
        std::u16string text = decoration_strategy_->DecorateItemText(GetItemTextAt(index));
...
private:
    ...
    bool GetAcceleratorAt(size_t index, ui::Accelerator* accelerator) const override
    {
        return false;
    }
    ...
    {
        return UseCheckmarks() && combobox_model_->GetItemAt(items_shown_[index].index) == owner_->GetText();
    }
    ...
    base::ScopedObservation<ui::ComboboxModel, ui::ComboboxModelObserver> observation_ { this };
...
class EditableCombobox::EditableComboboxPreTargetHandler : public ui::EventHandler {
public:
    ...
    EditableComboboxPreTargetHandler(const EditableComboboxPreTargetHandler&) = delete;
    EditableComboboxPreTargetHandler& operator=(const EditableComboboxPreTargetHandler&) = delete;

    ~EditableComboboxPreTargetHandler() override
    {
        StopObserving();
    }
    ...
    {
        if (event->type() == ui::EventType::kMousePressed && event->button_flags() == event->changed_button_flags()) {
...
EditableCombobox::EditableCombobox(std::unique_ptr<ui::ComboboxModel> combobox_model, const bool filter_on_edit, const bool show_on_empty,
    const int text_context, const int text_style, const bool display_arrow)
    ...
    , show_on_empty_(show_on_empty)
{
    ...
    control_elements_container_->SetInsideBorderInsets(
        gfx::Insets::TLBR(kEditableComboboxControlsContainerInsets, 0, kEditableComboboxControlsContainerInsets, kEditableComboboxControlsContainerInsets));
    control_elements_container_->SetBetweenChildSpacing(kComboboxArrowPaddingWidth);
    if (display_arrow) {
        arrow_ = AddControlElement(std::make_unique<Arrow>(base::BindRepeating(&EditableCombobox::ArrowButtonPressed, base::Unretained(this))));
...
{
    CloseMenu();
    menu_model_ = std::make_unique<EditableComboboxMenuModel>(this, std::move(model), filter_on_edit_, show_on_empty_);
}
...
void EditableCombobox::SetMenuDecorationStrategy(std::unique_ptr<MenuDecorationStrategy> strategy)
{
    DCHECK(menu_model_);
...
gfx::Size EditableCombobox::CalculatePreferredSize(const SizeBounds& available_size) const
{
...
void EditableCombobox::ContentsChanged(Textfield* sender, const std::u16string& new_contents)
{
    HandleNewContent(new_contents);
    ShowDropDownMenu(ui::mojom::MenuSourceType::kKeyboard);
}

bool EditableCombobox::HandleKeyEvent(Textfield* sender, const ui::KeyEvent& key_event)
{
    if (key_event.type() == ui::EventType::kKeyPressed && (key_event.key_code() == ui::VKEY_UP || key_event.key_code() == ui::VKEY_DOWN)) {
        ShowDropDownMenu(ui::mojom::MenuSourceType::kKeyboard);
        return true;
    }
    return false;
...
void EditableCombobox::OnLayoutIsAnimatingChanged(views::AnimatingLayoutManager* source, bool is_animating)
{
...
ProposedLayout EditableCombobox::CalculateProposedLayout(const SizeBounds& size_bounds) const
{
    ProposedLayout layout;
    layout.host_size = gfx::Size(size_bounds.width().value(), size_bounds.height().value());
    layout.child_layouts.emplace_back(textfield_.get(), textfield_->GetVisible(), gfx::Rect(0, 0, layout.host_size.width(), layout.host_size.height()));
    const int preferred_width = control_elements_container_->GetPreferredSize({}).width();
    layout.child_layouts.emplace_back(control_elements_container_.get(), control_elements_container_->GetVisible(),
        gfx::Rect(layout.host_size.width() - preferred_width, 0, preferred_width, layout.host_size.height()));
    return layout;
}
...
void EditableCombobox::ShowDropDownMenu(ui::mojom::MenuSourceType source_type)
{
    ...
    pre_target_handler_ = std::make_unique<EditableComboboxPreTargetHandler>(this, GetWidget()->GetRootView());
    ...
    menu_runner_ = std::make_unique<MenuRunner>(
        menu_model_.get(), MenuRunner::EDITABLE_COMBOBOX, base::BindRepeating(&EditableCombobox::CloseMenu, base::Unretained(this)));
    menu_runner_->RunMenuAt(GetWidget(), nullptr, bounds, MenuAnchorPosition::kTopLeft, source_type);
}

void EditableCombobox::UpdateTextfieldInsets()
{
    textfield_->SetExtraInsets(gfx::Insets::TLBR(0, 0, 0, std::max(control_elements_container_->GetPreferredSize({}).width() - kComboboxArrowPaddingWidth, 0)));
}
