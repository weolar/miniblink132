...
public:
    ...
    int schedule_paint_count() const
    {
        return schedule_paint_count_;
    }
    ...
    void SimulatePaint()
    {
        ...
        Paint(PaintInfo::CreateRootPaintInfo(ui::CanvasPainter(&bitmap, bounds().size(), 1.f, color, false).context(), bounds().size()));
    }
...
std::u16string GetClipboardText(ui::ClipboardBuffer clipboard_buffer)
{
    ...
    ui::Clipboard::GetForCurrentThread()->ReadText(clipboard_buffer, /* data_dst = */ nullptr, &clipboard_text);
...
public:
    ...
    static constexpr bool kExtends = gfx::RenderText::kDragToEndIfOutsideVerticalBounds;
    ...
    {
        LabelTest::SetUp();
        event_generator_ = std::make_unique<ui::test::EventGenerator>(GetRootWindow(widget()));
    }
...
protected:
    ...
    void PerformMousePress(const gfx::Point& point)
    {
        ui::MouseEvent pressed_event
            = ui::MouseEvent(ui::EventType::kMousePressed, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    ...
    void PerformMouseRelease(const gfx::Point& point)
    {
        ui::MouseEvent released_event
            = ui::MouseEvent(ui::EventType::kMouseReleased, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    ...
    {
        ui::MouseEvent drag(ui::EventType::kMouseDragged, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, 0);
    ...
    {
        SimulatePaint();
        gfx::RenderText* render_text = label()->GetRenderTextForSelectionController();
        ...
        if (!render_text->multiline()) {
            return render_text->GetCursorBounds(gfx::SelectionModel(index, gfx::CURSOR_FORWARD), true).left_center();
        }
        ...
        const bool rtl = render_text->GetDisplayTextDirection() == base::i18n::RIGHT_TO_LEFT;
        // Return Point corresponding to the leading edge of the character.
        return rtl ? bounds[0].right_center() + gfx::Vector2d(-1, 0) : bounds[0].left_center() + gfx::Vector2d(1, 0);
    }
...
{
    const auto color = label()->GetWidget()->GetColorProvider()->GetColor(ui::kColorPrimaryForeground);
    label()->SetAutoColorReadabilityEnabled(false);
    ...
    EXPECT_EQ(label()->GetWidget()->GetColorProvider()->GetColor(ui::kColorAccent), label()->GetEnabledColor());
}

TEST_F(LabelTest, BackgroundColor)
{
    // The correct default background color is set.
    EXPECT_EQ(widget()->GetColorProvider()->GetColor(ui::kColorDialogBackground), label()->GetBackgroundColor());
...
TEST_F(LabelTest, BackgroundColorId)
{
    ...
    EXPECT_EQ(widget()->GetColorProvider()->GetColor(ui::kColorAlertHighSeverity), label()->GetBackgroundColor());
...
TEST_F(LabelTest, AlignmentProperty)
{
    ...
    for (size_t i = 0; i < 2; ++i) {
        ...
        EXPECT_EQ(reverse_alignment ? gfx::ALIGN_LEFT : gfx::ALIGN_RIGHT, label()->GetHorizontalAlignment());
        label()->SetHorizontalAlignment(gfx::ALIGN_LEFT);
        EXPECT_EQ(reverse_alignment ? gfx::ALIGN_RIGHT : gfx::ALIGN_LEFT, label()->GetHorizontalAlignment());
...
TEST_F(LabelTest, ElideBehaviorMinimumWidth)
{
    ...
    EXPECT_EQ(gfx::Canvas::GetStringWidth(std::u16string(gfx::kEllipsisUTF16), label()->font_list()), size.width());
...
TEST_F(LabelTest, ObscuredProperty)
{
    ...
    EXPECT_EQ(std::u16string(test_text.size(), gfx::RenderText::kPasswordReplacementChar), label()->GetDisplayTextForTesting());
    EXPECT_EQ(test_text, label()->GetText());

    label()->SetText(test_text + test_text);
    label()->SizeToPreferredSize();
    EXPECT_EQ(std::u16string(test_text.size() * 2, gfx::RenderText::kPasswordReplacementChar), label()->GetDisplayTextForTesting());
...
TEST_F(LabelTest, ObscuredSurrogatePair)
{
    ...
    label()->SetObscured(true);
    label()->SizeToPreferredSize();
    EXPECT_EQ(std::u16string(1, gfx::RenderText::kPasswordReplacementChar), label()->GetDisplayTextForTesting());
...
TEST_F(LabelTest, MultilinePreferredSizeWithConstraintTest)
{
    ...
    const gfx::Size single_line_size = label()->GetPreferredSize({ /* Unbounded */ });

    // Test the preferred size when the label is not yet laid out.
    label()->SetMultiLine(true);
    const gfx::Size multi_line_size_unbounded = label()->GetPreferredSize({ /* Unbounded */ });
    EXPECT_EQ(single_line_size, multi_line_size_unbounded);

    const gfx::Size multi_line_size_bounded = label()->GetPreferredSize({ single_line_size.width() / 2, { /* Unbounded */ } });
    EXPECT_GT(multi_line_size_unbounded.width(), multi_line_size_bounded.width());
    EXPECT_LT(multi_line_size_unbounded.height(), multi_line_size_bounded.height());
    ...
    label()->SetBounds(0, 0, layout_width, label()->GetHeightForWidth(layout_width));
    const gfx::Size multi_line_size_unbounded2 = label()->GetPreferredSize({ /* Unbounded */ });
    const gfx::Size multi_line_size_bounded2 = label()->GetPreferredSize({ single_line_size.width() / 2, { /* Unbounded */ } });
...
TEST_F(LabelTest, TooltipProperty)
{
    ...
    label()->SetBounds(0, 0, 1000, label()->GetPreferredSize(SizeBounds(1000, {})).height() / 2);
    EXPECT_TRUE(label()->GetTooltipText(gfx::Point()).empty());
...
TEST_F(LabelTest, Accessibility)
{
    ...
    EXPECT_EQ(label()->GetText(), node_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
    EXPECT_FALSE(node_data.HasIntAttribute(ax::mojom::IntAttribute::kRestriction));
    ...
    label()->GetViewAccessibility().GetAccessibleNodeData(&node_data);
    EXPECT_EQ(accessible_name, node_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
    EXPECT_NE(label()->GetText(), node_data.GetString16Attribute(ax::mojom::StringAttribute::kName));
...
TEST_F(LabelTest, AccessibleNameAndRole)
{
    ...
    EXPECT_EQ(label()->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kStaticText);

    ui::AXNodeData data;
    label()->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), u"Text");
    EXPECT_EQ(data.role, ax::mojom::Role::kStaticText);

    label()->SetTextContext(style::CONTEXT_DIALOG_TITLE);

    EXPECT_EQ(label()->GetViewAccessibility().GetCachedName(), u"Text");
    EXPECT_EQ(label()->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kTitleBar);
    ...
    EXPECT_EQ(label()->GetViewAccessibility().GetCachedRole(), ax::mojom::Role::kLink);

    data = ui::AXNodeData();
    label()->GetViewAccessibility().GetAccessibleNodeData(&data);
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), u"New Text");
...
TEST_F(LabelTest, SingleLineSizing)
{
    ...
    const gfx::Size size_with_border = label()->GetPreferredSize(SizeBounds(label()->size()));
    EXPECT_EQ(size_with_border.height(), size.height() + border.height());
    EXPECT_EQ(size_with_border.width(), size.width() + border.width());
    EXPECT_EQ(size.height() + border.height(), label()->GetHeightForWidth(size_with_border.width()));
}
...
TEST_F(LabelTest, MultiLineSizing)
{
    ...
    EXPECT_EQ(height_for_constrained_width, label()->GetHeightForWidth(required_width - 1));
    ...
    EXPECT_EQ(required_size_with_border.height(), required_size.height() + border.height());
    EXPECT_EQ(required_size_with_border.width(), required_size.width() + border.width());
}
...
TEST_F(LabelTest, MultiLineSetMaxLines)
{
    ...
    label()->SetBorder(CreateEmptyBorder(border));
    EXPECT_EQ(two_line_size.height() + border.height(), label()->GetPreferredSize({}).height());
}
#endif
...
TEST_F(LabelTest, MultiLineSizingWithElide)
{
    ...
    EXPECT_EQ(required_size, label()->GetPreferredSize(SizeBounds(required_size)));
    EXPECT_EQ(text, label()->GetDisplayTextForTesting());

    gfx::Size narrow_size = label()->GetPreferredSize(SizeBounds(required_size.width() - 1, {}));
    ...
    EXPECT_EQ(narrow_size, label()->GetPreferredSize(SizeBounds(required_size.width() - 1, {})));
...
{
    Label bidi_text_force_url(ToRTL("0123456") + u".com", 0, style::STYLE_PRIMARY, gfx::DirectionalityMode::DIRECTIONALITY_AS_URL);
    EXPECT_EQ(base::i18n::TextDirection::LEFT_TO_RIGHT, bidi_text_force_url.GetTextDirectionForTesting());

    Label rtl_text_force_ltr(ToRTL("0123456"), 0, style::STYLE_PRIMARY, gfx::DirectionalityMode::DIRECTIONALITY_FORCE_LTR);
    EXPECT_EQ(base::i18n::TextDirection::LEFT_TO_RIGHT, rtl_text_force_ltr.GetTextDirectionForTesting());

    Label ltr_text_force_rtl(u"0123456", 0, style::STYLE_PRIMARY, gfx::DirectionalityMode::DIRECTIONALITY_FORCE_RTL);
    EXPECT_EQ(base::i18n::TextDirection::RIGHT_TO_LEFT, ltr_text_force_rtl.GetTextDirectionForTesting());
}

TEST_F(LabelTest, DefaultDirectionalityIsFromText)
{
    Label ltr(u"Foo");
    EXPECT_EQ(base::i18n::TextDirection::LEFT_TO_RIGHT, ltr.GetTextDirectionForTesting());

    Label rtl(ToRTL("0123456"));
    EXPECT_EQ(base::i18n::TextDirection::RIGHT_TO_LEFT, rtl.GetTextDirectionForTesting());
}
...
TEST_F(LabelTest, TextChangedCallback)
{
    ...
    auto subscription = label()->AddTextChangedCallback(base::BindRepeating([](bool* text_changed) { *text_changed = true; }, &text_changed));
...
TEST_F(LabelTest, GetSubstringBounds)
{
    ...
    auto substring_bounds_with_inset = label()->GetSubstringBounds(gfx::Range(0, 3));
    ...
    EXPECT_EQ(substring_bounds[0].width(), substring_bounds_with_inset[0].width());
    EXPECT_EQ(substring_bounds[0].height(), substring_bounds_with_inset[0].height());
}

// TODO(crbug.com/40725997): Enable on ChromeOS along with the DCHECK in Label.
#if BUILDFLAG(IS_CHROMEOS)
#define MAYBE_ChecksSubpixelRenderingOntoOpaqueSurface DISABLED_ChecksSubpixelRenderingOntoOpaqueSurface
#else
#define MAYBE_ChecksSubpixelRenderingOntoOpaqueSurface ChecksSubpixelRenderingOntoOpaqueSurface
#endif
...
TEST_F(LabelTest, AccessibleGraphemeOffsets)
{
    ...
    const auto cases = std::to_array<Case>({
        { std::u16string(), {} },
        // LTR.
        { u"This is a string", { 0, 6, 13, 15, 21, 24, 27, 32, 35, 41, 45, 50, 54, 58, 61, 68, 76 } },
        ...
        // should we for Views.
        { u"اللغة العربيي", { 67, 63, 59, 52, 46, 43, 40, 36, 29, 24, 20, 16, 6, 17 } },
    ...
    for (size_t i = 0; i < std::size(cases); i++) {
        ...
        ui::AXNodeData node_data;
        label()->GetViewAccessibility().GetAccessibleNodeData(&node_data);
        EXPECT_EQ(node_data.GetIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets), cases[i].expected_offsets);
    }
}
...
TEST_F(LabelTest, AccessibleGraphemeOffsetsObscured)
{
    ...
    label()->SizeToPreferredSize();
    label()->SetObscured(true);
    EXPECT_EQ(std::u16string(text.size(), gfx::RenderText::kPasswordReplacementChar), label()->GetDisplayTextForTesting());
    ui::AXNodeData node_data;
    label()->GetViewAccessibility().GetAccessibleNodeData(&node_data);
...
TEST_F(LabelTest, AccessibleGraphemeOffsetsElided)
{
    ...
    ui::AXNodeData node_data;
    label()->GetViewAccessibility().GetAccessibleNodeData(&node_data);
    std::vector<int32_t> expected_offsets = { 1, 7, 14, 16, 22, 25, 28, 38, 38, 38, 38, 38, 38, 38, 38, 38, 38 };
    EXPECT_EQ(node_data.GetIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets), expected_offsets);
}
