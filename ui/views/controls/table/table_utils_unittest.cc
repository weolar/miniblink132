...
TEST(TableUtilsTest, SetWidthHonored)
{
    ...
    std::vector<int> result(CalculateTableColumnSizes(100, 0, font_list, font_list, 0, 0, columns, &model));
    EXPECT_EQ("20,30", IntVectorToString(result));

    // Same with some padding, it should be ignored.
    result = CalculateTableColumnSizes(100, 0, font_list, font_list, 2, 0, columns, &model);
    EXPECT_EQ("20,30", IntVectorToString(result));

    // Same with not enough space, it shouldn't matter.
    result = CalculateTableColumnSizes(10, 0, font_list, font_list, 2, 0, columns, &model);
    EXPECT_EQ("20,30", IntVectorToString(result));
}
...
TEST(TableUtilsTest, LastColumnGetsAllSpace)
{
    ...
    columns.emplace_back();
    gfx::FontList font_list;
    std::vector<int> result(CalculateTableColumnSizes(500, 0, font_list, font_list, 0, 0, columns, &model));
    EXPECT_NE(0, result[0]);
    EXPECT_GE(result[1], WidthForContent(font_list, font_list, 0, 0, columns[1], &model));
...
TEST(TableUtilsTest, SingleResizableColumn)
{
    ...
    gfx::FontList font_list;
    std::vector<int> result(CalculateTableColumnSizes(500, 0, font_list, font_list, 0, 0, columns, &model));
    EXPECT_EQ(result[0], WidthForContent(font_list, font_list, 0, 0, columns[0], &model));
    EXPECT_EQ(result[1], WidthForContent(font_list, font_list, 0, 0, columns[1], &model));
    EXPECT_EQ(500 - result[0] - result[1], result[2]);

    // The same with a slightly larger width passed in.
    result = CalculateTableColumnSizes(1000, 0, font_list, font_list, 0, 0, columns, &model);
    EXPECT_EQ(result[0], WidthForContent(font_list, font_list, 0, 0, columns[0], &model));
    EXPECT_EQ(result[1], WidthForContent(font_list, font_list, 0, 0, columns[1], &model));
    EXPECT_EQ(1000 - result[0] - result[1], result[2]);

    // Verify padding for the first column is honored.
    result = CalculateTableColumnSizes(1000, 10, font_list, font_list, 0, 0, columns, &model);
    EXPECT_EQ(result[0], WidthForContent(font_list, font_list, 0, 0, columns[0], &model) + 10);
    EXPECT_EQ(result[1], WidthForContent(font_list, font_list, 0, 0, columns[1], &model));
    EXPECT_EQ(1000 - result[0] - result[1], result[2]);
    ...
    // to min size.
    result = CalculateTableColumnSizes(1000, 0, font_list, font_list, 0, 0, columns, &model);
    result = CalculateTableColumnSizes(result[0] + result[1], 0, font_list, font_list, 0, 0, columns, &model);
    EXPECT_EQ(result[0], WidthForContent(font_list, font_list, 0, 0, columns[0], &model));
    EXPECT_EQ(result[1], WidthForContent(font_list, font_list, 0, 0, columns[1], &model));
