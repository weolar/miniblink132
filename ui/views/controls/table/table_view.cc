...
void GetModelIndexToRangeStart(TableGrouper* grouper, size_t row_count, std::map<size_t, size_t>* model_index_to_range_start)
{
...
{
    return has_focus ? ui::kColorTableBackgroundSelectedFocused : ui::kColorTableBackgroundSelectedUnfocused;
}

// Returns the color id for text. |has_focus| indicates if the table has focus.
ui::ColorId selected_text_color_id(bool has_focus)
{
    return has_focus ? ui::kColorTableForegroundSelectedFocused : ui::kColorTableForegroundSelectedUnfocused;
}
    ...
    : weak_factory_(this)
{
    ...
    row_height_ = LayoutProvider::GetControlHeightForFont(kTextContext, kTextStyle, font_list_);

    // Always focusable, even on Mac (consistent with NSTableView).
    SetFocusBehavior(FocusBehavior::ALWAYS);
    set_suppress_default_focus_handling();
    views::HighlightPathGenerator::Install(this, std::make_unique<TableView::HighlightPathGenerator>());

    FocusRing::Install(this);
    views::FocusRing::Get(this)->SetHasFocusPredicate(base::BindRepeating([](const View* view) {
    ...
    GetViewAccessibility().SetName(std::u16string(), ax::mojom::NameFrom::kAttributeExplicitlyEmpty);
    ...
    GetViewAccessibility().SetTableColumnCount(static_cast<int32_t>(visible_columns_.size()));
}

TableView::TableView(ui::TableModel* model, const std::vector<ui::TableColumn>& columns, TableType table_type, bool single_selection)
    : TableView()
{
    GetViewAccessibility().SetRole(ax::mojom::Role::kListGrid);
...
// static
std::unique_ptr<ScrollView> TableView::CreateScrollViewWithTable(std::unique_ptr<TableView> table, bool has_border)
{
    auto scroll_view = has_border ? ScrollView::CreateScrollViewWithBorder() : std::make_unique<ScrollView>();
...
// static
Builder<ScrollView> TableView::CreateScrollViewBuilderWithTable(Builder<TableView>&& table)
{
    ...
    return Builder<ScrollView>(std::move(scroll_view))
        .SetContents(std::move(table).CustomConfigure(
            base::BindOnce([](ScrollView* scroll_view, TableView* table_view) { table_view->CreateHeaderIfNecessary(scroll_view); }, scroll_view_ptr)));
}

void TableView::Init(ui::TableModel* model, const std::vector<ui::TableColumn>& columns, TableType table_type, bool single_selection)
{
    SetColumns(columns);
...
void TableView::Select(std::optional<size_t> model_row)
{
    ...
    SelectByViewIndex(model_row.has_value() ? std::make_optional(ModelToView(model_row.value())) : std::nullopt);
}
...
{
    return selection_model_.empty() ? std::nullopt : std::make_optional(*selection_model_.selected_indices().begin());
}
...
void TableView::SetColumnVisibility(int id, bool is_visible)
{
    ...
    if (is_visible) {
        ...
        const auto i = base::ranges::find(visible_columns_, id, [](const auto& column) { return column.column.id; });
        if (i != visible_columns_.end()) {
            visible_columns_.erase(i);
            if (active_visible_column_index_.has_value() && active_visible_column_index_.value() >= visible_columns_.size())
                SetActiveVisibleColumnIndex(visible_columns_.empty() ? std::nullopt : std::make_optional(visible_columns_.size() - 1));
        }
    ...
    if (!sort.empty() && sort[0].column_id == column.id) {
        if (sort[0].ascending == column.initial_sort_is_ascending) {
            ...
            GetViewAccessibility().AnnounceText(l10n_util::GetStringFUTF16(
                sort[0].ascending ? IDS_APP_TABLE_COLUMN_SORTED_ASC_ACCNAME : IDS_APP_TABLE_COLUMN_SORTED_DESC_ACCNAME, column.title));
        } else {
            // Second toggle clears the sort.
            sort.clear();
            GetViewAccessibility().AnnounceText(l10n_util::GetStringFUTF16(IDS_APP_TABLE_COLUMN_NOT_SORTED_ACCNAME, column.title));
        }
    } else {
        ...
        GetViewAccessibility().AnnounceText(
            l10n_util::GetStringFUTF16(sort[0].ascending ? IDS_APP_TABLE_COLUMN_SORTED_ASC_ACCNAME : IDS_APP_TABLE_COLUMN_SORTED_DESC_ACCNAME, column.title));
    }
...
{
    return base::Contains(visible_columns_, id, [](const VisibleColumn& column) { return column.column.id; });
}
...
{
    return selection_model_.active().has_value() && active_visible_column_index_.has_value();
}
...
size_t TableView::ModelToView(size_t model_index) const
{
    if (!GetIsSorted())
        return model_index;
    CHECK_LT(model_index, model_to_view_.size()) << " out of bounds model_index " << model_index;
...
size_t TableView::ViewToModel(size_t view_index) const
{
    ...
    if (!GetIsSorted())
        return view_index;
    DCHECK_LT(view_index, view_to_model_.size()) << " out of bounds view_index " << view_index;
...
void TableView::Layout(PassKey)
{
    ...
    if (header_) {
        header_->SetBoundsRect(gfx::Rect(header_->bounds().origin(), gfx::Size(width, header_->GetPreferredSize({}).height())));
    }

    views::FocusRing::Get(this)->DeprecatedLayoutImmediately();
}

gfx::Size TableView::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
    int width = 50;
...
bool TableView::OnKeyPressed(const ui::KeyEvent& event)
{
    ...
    case ui::VKEY_LEFT:
        if (PlatformStyle::kTableViewSupportsKeyboardNavigationByCell) {
            const AdvanceDirection direction = base::i18n::IsRTL() ? AdvanceDirection::kIncrement : AdvanceDirection::kDecrement;
            if (IsCmdOrCtrl(event)) {
                if (active_visible_column_index_.has_value() && header_) {
                    header_->ResizeColumnViaKeyboard(active_visible_column_index_.value(), direction);
                    UpdateFocusRings();
                }
            } else {
    ...
    case ui::VKEY_RIGHT:
        ...
        // specified.
        if (PlatformStyle::kTableViewSupportsKeyboardNavigationByCell) {
            const AdvanceDirection direction = base::i18n::IsRTL() ? AdvanceDirection::kDecrement : AdvanceDirection::kIncrement;
            if (IsCmdOrCtrl(event)) {
                if (active_visible_column_index_.has_value() && header_) {
                    header_->ResizeColumnViaKeyboard(active_visible_column_index_.value(), direction);
                    UpdateFocusRings();
    ...
    case ui::VKEY_RETURN:
        if (PlatformStyle::kTableViewSupportsKeyboardNavigationByCell && active_visible_column_index_.has_value() && header_row_is_active_) {
    ...
    case ui::VKEY_SPACE:
        if (PlatformStyle::kTableViewSupportsKeyboardNavigationByCell && active_visible_column_index_.has_value()) {
            ToggleSortOrder(active_visible_column_index_.value());
            return true;
        }
        break;
...
std::u16string TableView::GetTooltipText(const gfx::Point& p) const
{
    ...
    if (row < 0 || static_cast<size_t>(row) >= GetRowCount() || visible_columns_.empty()) {
        return std::u16string();
    }

    const int x = GetMirroredXInView(p.x());
    const std::optional<size_t> column = GetClosestVisibleColumnIndex(*this, x);
    if (!column.has_value() || x < visible_columns_[column.value()].x || x > (visible_columns_[column.value()].x + visible_columns_[column.value()].width)) {
        return std::u16string();
    }
...
bool TableView::HandleAccessibleAction(const ui::AXActionData& action_data)
{
    ...
    bool focus_on_row = ax_view ? ax_view->GetData().role == ax::mojom::Role::kRow : false;
    ...
    case ax::mojom::Action::kDoDefault:
        ...
        if (focus_on_row) {
            ...
            DCHECK(ax_view);
            size_t row_index = base::checked_cast<size_t>(ax_view->GetData().GetIntAttribute(ax::mojom::IntAttribute::kTableRowIndex));
            SelectByViewIndex(row_index);
            GetViewAccessibility().AnnounceText(
                l10n_util::GetStringFUTF16(IDS_TABLE_VIEW_AX_ANNOUNCE_ROW_SELECTED, model_->GetText(ViewToModel(row_index), GetVisibleColumn(0).column.id)));
        } else {
    ...
    case ax::mojom::Action::kShowContextMenu:
        ShowContextMenu(GetBoundsInScreen().CenterPoint(), ui::mojom::MenuSourceType::kKeyboard);
        break;
...
void TableView::OnItemsAdded(size_t start, size_t length)
{
    ...
    for (size_t i = 0; i < length; ++i) {
        ...
        const size_t virtual_children_count = GetViewAccessibility().virtual_children().size();
        const size_t next_index = header_ ? virtual_children_count - 1 : virtual_children_count;
        GetViewAccessibility().AddVirtualChildView(CreateRowAccessibilityView(next_index));
    }
...
void TableView::OnItemsMoved(size_t old_start, size_t length, size_t new_start)
{
...
void TableView::OnItemsRemoved(size_t start, size_t length)
{
    ...
    const std::optional<size_t> previously_selected_model_index = GetFirstSelectedRow();
    std::optional<size_t> previously_selected_view_index = previously_selected_model_index;
    if (previously_selected_model_index.has_value() && GetIsSorted())
        previously_selected_view_index = model_to_view_[previously_selected_model_index.value()];
    for (size_t i = 0; i < length; ++i)
    ...
    // index.
    if (selection_model_.empty() && previously_selected_view_index.has_value() && GetRowCount() && select_on_remove_) {
        selection_model_.SetSelectedIndex(ViewToModel(std::min(GetRowCount() - 1, previously_selected_view_index.value())));
        // `ListSelectionModel::SetSelectedIndex` clears the selection and selects
        // only the specified index.
        ClearAccessibleSelection();
        SetAccessibleSelectionForIndex(std::min(GetRowCount() - 1, previously_selected_view_index.value()),
            /* selected */ true);
    }
    if (!selection_model_.empty()) {
        const size_t selected_model_index = *selection_model_.selected_indices().begin();
...
void TableView::OnPaintImpl(gfx::Canvas* canvas)
{
    ...
    const SkColor default_bg_color = color_provider->GetColor(ui::kColorTableBackground);
    ...
    const SkColor selected_bg_color = color_provider->GetColor(text_background_color_id(HasFocus()));
    const SkColor fg_color = color_provider->GetColor(ui::kColorTableForeground);
    const SkColor selected_fg_color = color_provider->GetColor(selected_text_color_id(HasFocus()));
    const SkColor alternate_bg_color = color_provider->GetColor(ui::kColorTableBackgroundAlternate);
    ...
    for (size_t i = region.min_row; i < region.max_row; ++i) {
        ...
        for (size_t j = region.min_column; j < region.max_column; ++j) {
            ...
            if (j == 0 && grouper_) {
                text_bounds.Inset(gfx::Insets().set_left(kGroupingIndicatorSize + cell_element_spacing));
            }

            // Always paint the icon in the first visible column.
            if (j == 0 && table_type_ == TableType::kIconAndText) {
                gfx::ImageSkia image = model_->GetIcon(model_index).Rasterize(GetColorProvider());
                if (!image.isNull()) {
                    // Need to check the area where the icon is paint. And it is necessary
                    // to consider the UI layout direction.
                    gfx::Rect dest_image_bounds = GetPaintIconDestBounds(cell_bounds, text_bounds.x());
                    // The area does not have a width drawing icon.
                    if (!dest_image_bounds.IsEmpty()) {
                        gfx::Rect src_image_bounds = GetPaintIconSrcBounds(image.size(), dest_image_bounds.width());
                        canvas->DrawImageInt(image, src_image_bounds.x(), src_image_bounds.y(), src_image_bounds.width(), src_image_bounds.height(),
                            dest_image_bounds.x(), dest_image_bounds.y(), dest_image_bounds.width(), dest_image_bounds.height(), true);
                    }
                }
                text_bounds.Inset(gfx::Insets().set_left(ui::TableModel::kIconSize + cell_element_spacing));
            }

            // Paint text if there is still room for it after all that insetting.
            if (!text_bounds.IsEmpty()) {
                canvas->DrawStringRectWithFlags(model_->GetText(model_index, visible_columns_[j].column.id), font_list_,
                    is_selected ? selected_fg_color : fg_color, GetMirroredRect(text_bounds),
                    TableColumnAlignmentToCanvasAlignment(GetMirroredTableColumnAlignment(visible_columns_[j].column.alignment)));
            }
        }
    }
    ...
    const SkColor grouping_color = color_provider->GetColor(ui::kColorTableGroupingIndicator);
    ...
    for (size_t i = region.min_row; i < region.max_row;) {
        ...
        canvas->DrawLine(gfx::PointF(group_indicator_x, start_cell_bounds.CenterPoint().y()),
            gfx::PointF(group_indicator_x, last_cell_bounds.CenterPoint().y()), grouping_flags);
        i = last + 1;
    }
}
...
{
    return LayoutProvider::Get()->GetDistanceMetric(DISTANCE_TABLE_CELL_HORIZONTAL_MARGIN);
}

int TableView::GetCellElementSpacing() const
{
    return LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_LABEL_HORIZONTAL);
}
...
void TableView::SortItemsAndUpdateMapping(bool schedule_paint)
{
    ...
    if (!GetIsSorted()) {
        ...
        if (grouper_) {
            ...
            GetModelIndexToRangeStart(grouper_, row_count, &sort_helper.model_index_to_range_start);
            std::stable_sort(view_to_model_.begin(), view_to_model_.end(), sort_helper);
        } else {
            std::stable_sort(view_to_model_.begin(), view_to_model_.end(), SortHelper(this));
        }
...
{
    const int sort_result = model_->CompareValues(model_row1, model_row2, sort_descriptors_[0].column_id);
    if (sort_result == 0 && sort_descriptors_.size() > 1) {
        // Try the secondary sort.
        return SwapCompareResult(model_->CompareValues(model_row1, model_row2, sort_descriptors_[1].column_id), sort_descriptors_[1].ascending);
    }
...
{
    return gfx::Rect(0, static_cast<int>(row) * row_height_, width(), row_height_);
}

gfx::Rect TableView::GetCellBounds(size_t row, size_t visible_column_index) const
{
    if (!header_)
        return GetRowBounds(row);
    const VisibleColumn& vis_col(visible_columns_[visible_column_index]);
    return gfx::Rect(vis_col.x, static_cast<int>(row) * row_height_, vis_col.width, row_height_);
}
...
gfx::Rect TableView::GetActiveCellBounds() const
{
    if (!selection_model_.active().has_value())
        return gfx::Rect();
    return GetCellBounds(ModelToView(selection_model_.active().value()), active_visible_column_index_.value());
}

void TableView::AdjustCellBoundsForText(size_t visible_column_index, gfx::Rect* bounds) const
{
    const int cell_margin = GetCellMargin();
    const int cell_element_spacing = GetCellElementSpacing();
...
void TableView::CreateHeaderIfNecessary(ScrollView* scroll_view)
{
    ...
    header_ = scroll_view->SetHeader(std::make_unique<TableHeader>(weak_factory_.GetWeakPtr()));

    // The header accessibility view should be the first row, to match the
    // original view accessibility construction.
    GetViewAccessibility().AddVirtualChildViewAt(CreateHeaderAccessibilityView(), 0);
}
...
void TableView::UpdateVisibleColumnSizes()
{
    ...
    std::vector<int> sizes = views::CalculateTableColumnSizes(layout_width_, first_column_padding, header_->font_list(), font_list_,
        std::max(cell_margin, header_->GetCellHorizontalPadding()) * 2, header_->GetSortIndicatorWidth(), columns, model_);
...
gfx::Rect TableView::GetPaintIconSrcBounds(const gfx::Size& image_size, int image_dest_width) const
{
    int src_image_x = 0;
    int src_image_width = image_size.width() * image_dest_width / ui::TableModel::kIconSize;
    if (GetMirrored()) {
...
gfx::Rect TableView::GetPaintIconDestBounds(const gfx::Rect& cell_bounds, int text_bounds_x) const
{
    int dest_image_x = GetMirroredXWithWidthInView(text_bounds_x, ui::TableModel::kIconSize);
    ...
    if (GetMirrored()) {
        if (dest_image_x < mirrored_cell_bounds.x()) {
            dest_image_width = dest_image_x + dest_image_width - mirrored_cell_bounds.x();
    ...
    return gfx::Rect(dest_image_x, cell_bounds.y() + (cell_bounds.height() - ui::TableModel::kIconSize) / 2, dest_image_width > 0 ? dest_image_width : 0,
        ui::TableModel::kIconSize);
}

TableView::PaintRegion TableView::GetPaintRegion(const gfx::Rect& bounds) const
{
    DCHECK(!visible_columns_.empty());
    DCHECK(GetRowCount());

    PaintRegion region;
    region.min_row = static_cast<size_t>(std::clamp(bounds.y() / row_height_, 0, base::saturated_cast<int>(GetRowCount() - 1)));
    ...
    for (size_t i = 0; i < visible_columns_.size(); ++i) {
        ...
        if (region.min_column == visible_columns_.size() && max_x >= paint_x)
            region.min_column = i;
        if (region.min_column != visible_columns_.size() && visible_columns_[i].x >= paint_max_x) {
            region.max_column = i;
            break;
        }
    }
...
void TableView::SchedulePaintForSelection()
{
    if (selection_model_.size() == 1) {
        ...
        if (selection_model_.active().has_value() && first_model_row != selection_model_.active().value()) {
            SchedulePaintInRect(GetRowBounds(ModelToView(selection_model_.active().value())));
        }
...
void TableView::AdvanceActiveVisibleColumn(AdvanceDirection direction)
{
    ...
    if (!active_visible_column_index_.has_value()) {
        if (!selection_model_.active().has_value() && !header_row_is_active_ && GetRowCount()) {
            SelectByViewIndex(size_t { 0 });
        }
    ...
    if (direction == AdvanceDirection::kDecrement) {
        SetActiveVisibleColumnIndex(std::max(size_t { 1 }, active_visible_column_index_.value()) - 1);
    } else {
        SetActiveVisibleColumnIndex(std::min(visible_columns_.size() - 1, active_visible_column_index_.value() + 1));
    }
}
...
void TableView::SetActiveVisibleColumnIndex(std::optional<size_t> index)
{
    ...
    if (active_visible_column_index_.has_value()) {
        if (selection_model_.active().has_value()) {
            ScrollRectToVisible(GetCellBounds(ModelToView(selection_model_.active().value()), active_visible_column_index_.value()));
        } else if (header_row_is_active()) {
            const TableView::VisibleColumn& column = GetVisibleColumn(active_visible_column_index_.value());
            ScrollRectToVisible(gfx::Rect(column.x, 0, column.width, height()));
        }
        UpdateAccessibleSelectionForColumnIndex(active_visible_column_index_.value());
    }
...
{
    if (!selection_model_.active().has_value()) {
        bool make_header_active = header_ && direction == AdvanceDirection::kDecrement;
    ...
    if (direction == AdvanceDirection::kDecrement) {
        ...
        header_row_is_active_ = make_header_active;
        SelectByViewIndex(make_header_active ? std::nullopt : std::make_optional(std::max(size_t { 1 }, view_range_start) - 1));
    } else {
        header_row_is_active_ = false;
        SelectByViewIndex(std::min(GetRowCount() - 1, view_range_start + range.length));
    }
}

void TableView::ConfigureSelectionModelForEvent(const ui::LocatedEvent& event, ui::ListSelectionModel* model) const
{
    ...
    if (!selection_model_.anchor().has_value() || single_selection_ || (!IsCmdOrCtrl(event) && !event.IsShiftDown())) {
    ...
    if ((IsCmdOrCtrl(event) && event.IsShiftDown()) || event.IsShiftDown()) {
        ...
        *model = selection_model_;
        model->set_anchor(ViewToModel(view_index));
        model->set_active(ViewToModel(view_index));
        SelectRowsInRangeFrom(view_index, !model->IsSelected(ViewToModel(view_index)), model);
    }
}

void TableView::SelectRowsInRangeFrom(size_t view_index, bool select, ui::ListSelectionModel* model) const
{
...
void TableView::RebuildVirtualAccessibilityChildren()
{
    ...
    for (size_t index = 0; index < GetRowCount(); ++index) {
        GetViewAccessibility().AddVirtualChildView(CreateRowAccessibilityView(index));
...
void TableView::UpdateAccessibleNameForIndex(size_t start_view_index, size_t length)
{
    if (start_view_index >= GetRowCount() || start_view_index + length > GetRowCount()) {
        return;
    }

    for (size_t view_index = start_view_index; view_index < start_view_index + length; ++view_index) {
        ...
        if (!PlatformStyle::kTableViewSupportsKeyboardNavigationByCell && visible_columns_.size()) {
            ax_row->GetCustomData().SetName(model_->GetText(model_index, GetVisibleColumn(0).column.id));
        }
        ...
        for (auto& ax_cell : ax_row->children()) {
            ...
            std::u16string current_name = base::UTF8ToUTF16(cell_data.GetStringAttribute(ax::mojom::StringAttribute::kName));
            std::u16string new_name = model()->GetText(model_index, GetVisibleColumn(column_index.value()).column.id);
...
void TableView::SetAccessibleSelectionForIndex(size_t view_index, bool selected) const
{
    ...
    ax_row->GetCustomData().AddBoolAttribute(ax::mojom::BoolAttribute::kSelected, selected);

    // Select/Unselect the cell.
    if (PlatformStyle::kTableViewSupportsKeyboardNavigationByCell) {
        for (size_t cell = 0; cell < ax_row->children().size(); cell++) {
            if (cell == GetActiveVisibleColumnIndex()) {
                ax_row->children()[cell]->GetCustomData().AddBoolAttribute(ax::mojom::BoolAttribute::kSelected, selected);
            }
        }
    }
}

void TableView::SetAccessibleSelectionForRange(size_t start_view_index, size_t end_view_index, bool selected) const
{
...
void TableView::UpdateAccessibleSelectionForColumnIndex(size_t visible_column_index) const
{
    if (!PlatformStyle::kTableViewSupportsKeyboardNavigationByCell || visible_column_index >= visible_columns_.size()) {
        return;
    }

    for (size_t row = 0; row < GetRowCount(); ++row) {
        if (selection_model().IsSelected(ViewToModel(row))) {
            GetVirtualAccessibilityCell(row, visible_column_index)->GetCustomData().AddBoolAttribute(ax::mojom::BoolAttribute::kSelected, true);
        }
    }
}

std::unique_ptr<AXVirtualView> TableView::CreateRowAccessibilityView(size_t row_index)
{
    ...
    row_data.AddIntAttribute(ax::mojom::IntAttribute::kTableRowIndex, static_cast<int32_t>(row_index));
    ...
    for (size_t visible_column_index = 0; visible_column_index < visible_columns_.size(); ++visible_column_index) {
        std::unique_ptr<AXVirtualView> ax_cell = CreateCellAccessibilityView(row_index, visible_column_index);
...
std::unique_ptr<AXVirtualView> TableView::CreateCellAccessibilityView(size_t row_index, size_t column_index)
{
    ...
    cell_data.AddIntAttribute(ax::mojom::IntAttribute::kTableCellRowIndex, static_cast<int32_t>(row_index));
    ...
    cell_data.AddIntAttribute(ax::mojom::IntAttribute::kTableCellColumnIndex, static_cast<int32_t>(column_index));
    ...
    const std::optional<int> primary_sorted_column_id = sort_descriptors().empty() ? std::nullopt : std::make_optional(sort_descriptors()[0].column_id);

    if (column.sortable && primary_sorted_column_id.has_value() && column.id == primary_sorted_column_id.value()) {
        sort_direction = GetFirstSortDescriptorDirection();
    }
    cell_data.AddIntAttribute(ax::mojom::IntAttribute::kSortDirection, static_cast<int32_t>(sort_direction));
...
std::unique_ptr<AXVirtualView> TableView::CreateHeaderAccessibilityView()
{
    ...
    for (size_t visible_column_index = 0; visible_column_index < visible_columns_.size(); ++visible_column_index) {
        ...
        cell_data.AddIntAttribute(ax::mojom::IntAttribute::kTableCellColumnIndex, static_cast<int32_t>(visible_column_index));
        cell_data.AddIntAttribute(ax::mojom::IntAttribute::kTableCellColumnSpan, 1);
        if (base::i18n::IsRTL()) {
            cell_data.SetTextDirection(ax::mojom::WritingDirection::kRtl);
        }
...
bool TableView::UpdateVirtualAccessibilityRowData(AXVirtualView* ax_row, int view_index, int model_index)
{
    DCHECK_GE(view_index, 0);

    ui::AXNodeData& row_data = ax_row->GetCustomData();

    int previous_view_index = row_data.GetIntAttribute(ax::mojom::IntAttribute::kTableRowIndex);
    if (previous_view_index == view_index)
        return false;

    row_data.AddIntAttribute(ax::mojom::IntAttribute::kTableRowIndex, static_cast<int32_t>(view_index));

    // Update the cell's in the current row to have the new data.
    for (const auto& ax_cell : ax_row->children()) {
        ui::AXNodeData& cell_data = ax_cell->GetCustomData();
        cell_data.AddIntAttribute(ax::mojom::IntAttribute::kTableCellRowIndex, static_cast<int32_t>(view_index));
    }
...
void TableView::UpdateVirtualAccessibilityChildrenBounds()
{
    ...
    if (header_) {
        ...
        row_data.relative_bounds.bounds = gfx::RectF(CalculateHeaderRowAccessibilityBounds());

        // Update the bounds for every child cell in this row.
        for (size_t visible_column_index = 0; visible_column_index < ax_row->children().size(); visible_column_index++) {
            ui::AXNodeData& cell_data = ax_row->children()[visible_column_index]->GetCustomData();
            DCHECK_EQ(cell_data.role, ax::mojom::Role::kColumnHeader);

            if (visible_column_index < visible_columns_.size()) {
                cell_data.relative_bounds.bounds = gfx::RectF(CalculateHeaderCellAccessibilityBounds(visible_column_index));
            } else {
    ...
    for (size_t row_index = 0; row_index < GetRowCount(); row_index++) {
        ...
        ui::AXNodeData& row_data = ax_row->GetCustomData();
        DCHECK_EQ(row_data.role, ax::mojom::Role::kRow);
        row_data.relative_bounds.bounds = gfx::RectF(CalculateTableRowAccessibilityBounds(row_index));
        ...
        for (size_t visible_column_index = 0; visible_column_index < ax_row->children().size(); visible_column_index++) {
            ...
            if (visible_column_index < visible_columns_.size()) {
                cell_data.relative_bounds.bounds = gfx::RectF(CalculateTableCellAccessibilityBounds(row_index, visible_column_index));
            } else {
                cell_data.relative_bounds.bounds = gfx::RectF();
            }
        }
...
gfx::Rect TableView::CalculateHeaderCellAccessibilityBounds(const size_t visible_column_index) const
{
    const gfx::Rect& header_bounds = CalculateHeaderRowAccessibilityBounds();
    const VisibleColumn& visible_column = visible_columns_[visible_column_index];
    gfx::Rect header_cell_bounds(visible_column.x, header_bounds.y(), visible_column.width, header_bounds.height());
    return header_cell_bounds;
}

gfx::Rect TableView::CalculateTableRowAccessibilityBounds(const size_t row_index) const
{
    gfx::Rect row_bounds = GetRowBounds(row_index);
    return row_bounds;
}

gfx::Rect TableView::CalculateTableCellAccessibilityBounds(const size_t row_index, const size_t visible_column_index) const
{
...
void TableView::ScheduleUpdateAccessibilityFocusIfNeeded()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&TableView::UpdateAccessibilityFocus, weak_factory_.GetWeakPtr(), UpdateAccessibilityFocusPassKey()));
}

void TableView::UpdateAccessibilityFocus(UpdateAccessibilityFocusPassKey pass_key)
{
    ...
    if (header_ && header_row_is_active_) {
        ...
        if (!PlatformStyle::kTableViewSupportsKeyboardNavigationByCell || !active_visible_column_index_.has_value()) {
            ...
            AXVirtualView* ax_header_cell = GetVirtualAccessibilityCellImpl(ax_header_row, active_visible_column_index_.value());
            if (ax_header_cell) {
    ...
    if (!selection_model_.active().has_value() || !active_visible_column_index_.has_value()) {
    ...
    if (!PlatformStyle::kTableViewSupportsKeyboardNavigationByCell) {
        ...
        AXVirtualView* ax_cell = GetVirtualAccessibilityCellImpl(ax_row, active_visible_column_index_.value());
        if (ax_cell) {
...
AXVirtualView* TableView::GetVirtualAccessibilityBodyRow(size_t row) const
{
    ...
    if (header_)
        ++row;
    CHECK_LT(row, GetViewAccessibility().virtual_children().size()) << "|row| not found. Did you forget to call "
                                                                       "RebuildVirtualAccessibilityChildren()?";
...
AXVirtualView* TableView::GetVirtualAccessibilityCell(size_t row, size_t visible_column_index) const
{
    return GetVirtualAccessibilityCellImpl(GetVirtualAccessibilityBodyRow(row), visible_column_index);
}
...
AXVirtualView* TableView::GetVirtualAccessibilityCellImpl(AXVirtualView* ax_row, size_t visible_column_index) const
{
    DCHECK(ax_row) << "|row| not found. Did you forget to call "
                      "RebuildVirtualAccessibilityChildren()?";
    const auto matches_index = [visible_column_index](const auto& ax_cell) {
        DCHECK(ax_cell);
        DCHECK(ax_cell->GetData().role == ax::mojom::Role::kColumnHeader || ax_cell->GetData().role == ax::mojom::Role::kGridCell);
        return base::checked_cast<size_t>(ax_cell->GetData().GetIntAttribute(ax::mojom::IntAttribute::kTableCellColumnIndex)) == visible_column_index;
    };
    const auto i = base::ranges::find_if(ax_row->children(), matches_index);
    DCHECK(i != ax_row->children().cend()) << "|visible_column_index| not found. Did you forget to call "
                                           << "RebuildVirtualAccessibilityChildren()?";
    return i->get();
}
...
DEFINE_ENUM_CONVERTERS(views::TableType, { views::TableType::kTextOnly, u"TEXT_ONLY" }, { views::TableType::kIconAndText, u"ICON_AND_TEXT" })
