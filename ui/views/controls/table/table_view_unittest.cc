...
public:
    ...
    std::string GetPaintRegion(const gfx::Rect& bounds)
    {
        ...
        return "rows=" + base::NumberToString(region.min_row) + " " + base::NumberToString(region.max_row) + " cols=" + base::NumberToString(region.min_column)
    ...
    AXVirtualView* GetVirtualAccessibilityHeaderCell(size_t visible_column_index)
    {
        return table_->GetVirtualAccessibilityCellImpl(GetVirtualAccessibilityHeaderRow(), visible_column_index);
    }

    AXVirtualView* GetVirtualAccessibilityCell(size_t row, size_t visible_column_index)
    {
    ...
    void SelectRowsInRangeFrom(size_t view_index, bool select, ui::ListSelectionModel* model)
    {
    ...
    std::vector<std::vector<gfx::Rect>> GenerateExpectedBounds()
    {
        ...
        gfx::Rect header_row_bounds = table_->CalculateHeaderRowAccessibilityBounds();
        View::ConvertRectToScreen(table_, &header_row_bounds);
        header_row.push_back(header_row_bounds);
        for (size_t column_index = 0; column_index < visible_col_count(); column_index++) {
        ...
        for (size_t row_index = 0; row_index < table_->GetRowCount(); row_index++) {
            ...
            for (size_t column_index = 0; column_index < visible_col_count(); column_index++) {
                gfx::Rect table_cell_bounds = table_->CalculateTableCellAccessibilityBounds(row_index, column_index);
    ...
    {
        return table_->GetCellMargin();
    }

    int GetCellElementSpacing() const
    {
        return table_->GetCellElementSpacing();
    }

    gfx::Rect GetPaintIconSrcBounds(const gfx::Size& image_size, int image_dest_width)
    {
        return table_->GetPaintIconSrcBounds(image_size, image_dest_width);
    }

    gfx::Rect GetPaintIconDestBounds(const gfx::Rect& cell_bounds, int text_bounds_x)
    {
...
{
    if (row <= rows_.size()) {
        rows_.erase(rows_.begin() + row, rows_.begin() + std::clamp(row + length, size_t { 0 }, rows_.size()));
    }
...
std::string GetRowsInViewOrderAsString(TableView* table)
{
    ...
    for (size_t i = 0; i < table->GetRowCount(); ++i) {
        ...
        for (size_t j = 0; j < table->visible_columns().size(); ++j) {
            ...
            result += base::UTF16ToUTF8(table->model()->GetText(table->ViewToModel(i), column.id));
        }
        result += "]";
    }
    return result;
...
std::string GetRowsInVirtualViewAsString(TableView* table)
{
    ...
    for (size_t row_index = 0; row_index < virtual_children.size(); row_index++) {
        ...
        for (size_t cell_index = 0; cell_index < row->children().size(); cell_index++) {
            if (cell_index != 0)
                result += ", "; // Comma between each value in the row.
...
std::string GetHeaderRowAsString(TableView* table)
{
    ...
    for (size_t col_index = 0; col_index < table->visible_columns().size(); ++col_index) {
        if (col_index != 0)
            result += ", "; // Comma between each column.

        result += base::UTF16ToUTF8(table->GetVisibleColumn(col_index).column.title);
    }
...
{
    const ui::MouseEvent pressed(ui::EventType::kMousePressed, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    return target->OnMousePressed(pressed);
}

void ReleaseLeftMouseAt(views::View* target, const gfx::Point& point)
{
    const ui::MouseEvent release(ui::EventType::kMouseReleased, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    target->OnMouseReleased(release);
}

bool DragLeftMouseTo(views::View* target, const gfx::Point& point)
{
    const ui::MouseEvent dragged(ui::EventType::kMouseDragged, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, 0);
...
class TableViewTest : public ViewsTestBase,
                      public ::testing::WithParamInterface<std::tuple</*use_default_construction=*/bool,
                          /*use_rtl=*/bool>> {
public:
    ...
    void SetUp() override
    {
        ...
        if (use_default_construction()) {
            ...
            table = std::make_unique<TableView>(model_.get(), columns, TableType::kTextOnly, false);
        }
        table_ = table.get();
        ...
        test::RunScheduledLayout(widget_->GetRootView()->AddChildView(std::move(scroll_view)));
        widget_->Show();
    }
    ...
    void VerifyTableViewAndAXOrder(std::string expected_view_order)
    {
        ...
        if (helper_->header()) {
            expected_view_order = GetHeaderRowAsString(table_) + ", " + expected_view_order;
        }
    ...
    void VerifyAXRowIndexes()
    {
        ...
        for (int index = first_row_index; index < virtual_row_count; index++) {
            ...
            const int stored_index = row_data.GetIntAttribute(ax::mojom::IntAttribute::kTableRowIndex);
    ...
    void VerifyTableAccChildrenBounds(const ViewAccessibility& view_accessibility, const std::vector<std::vector<gfx::Rect>>& expected_bounds)
    {
        ...
        for (size_t row_index = 0; row_index < virtual_children.size(); row_index++) {
            ...
            gfx::Rect row_custom_bounds = row->GetBoundsRect(ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kUnclipped, &offscreen_result);
            EXPECT_EQ(row_custom_bounds, expected_bounds[row_index][0]);
            if (table_->GetVisibleBounds().Intersects(expected_bounds[row_index][0])) {
            ...
            for (size_t cell_index = 0; cell_index < row->children().size(); cell_index++) {
                ...
                gfx::Rect cell_custom_bounds = cell->GetBoundsRect(ui::AXCoordinateSystem::kScreenDIPs, ui::AXClippingBehavior::kUnclipped, &offscreen_result);
                EXPECT_EQ(cell_custom_bounds, expected_bounds[row_index][expected_bounds_index]);
                if (table_->GetVisibleBounds().Intersects(expected_bounds[row_index][expected_bounds_index])) {
    ...
    bool use_default_construction() const
    {
...
protected:
    virtual WidgetDelegate* ConfigureWidgetDelegate()
    {
        return nullptr;
    }
...
INSTANTIATE_TEST_SUITE_P(All, TableViewTest,
...
TEST_P(TableViewTest, SelectedIndexWithNoRows)
{
    ...
    table_->OnKeyPressed(ui::KeyEvent(ui::EventType::kKeyPressed, ui::VKEY_DOWN, 0));
    EXPECT_TRUE(table_->selection_model().empty());
}
...
TEST_P(TableViewTest, GetPaintRegion)
{
    ...
    EXPECT_EQ("rows=0 4 cols=0 1", helper_->GetPaintRegion(gfx::Rect(0, 0, 1, table_->height())));
}
...
TEST_P(TableViewTest, RebuildVirtualAccessibilityChildren)
{
    ...
    EXPECT_EQ(table_->GetRowCount(), static_cast<size_t>(data.GetIntAttribute(ax::mojom::IntAttribute::kTableRowCount)));
    EXPECT_EQ(helper_->visible_col_count(), static_cast<size_t>(data.GetIntAttribute(ax::mojom::IntAttribute::kTableColumnCount)));

    // The header takes up another row.
    ASSERT_EQ(static_cast<size_t>(table_->GetRowCount() + 1), view_accessibility.virtual_children().size());
    ...
    size_t i = 0;
    for (auto child_iter = view_accessibility.virtual_children().begin() + 1; i < table_->GetRowCount(); ++child_iter, ++i) {
        const auto& row = *child_iter;
        ASSERT_TRUE(row);
        const ui::AXNodeData& row_data = row->GetData();
        EXPECT_EQ(ax::mojom::Role::kRow, row_data.role);
        EXPECT_EQ(i, static_cast<size_t>(row_data.GetIntAttribute(ax::mojom::IntAttribute::kTableRowIndex)));
        ...
        for (const auto& cell : row->children()) {
            ...
            const ui::AXNodeData& cell_data = cell->GetData();
            EXPECT_EQ(ax::mojom::Role::kGridCell, cell_data.role);
            EXPECT_EQ(i, static_cast<size_t>(cell_data.GetIntAttribute(ax::mojom::IntAttribute::kTableCellRowIndex)));
            EXPECT_EQ(j++, cell_data.GetIntAttribute(ax::mojom::IntAttribute::kTableCellColumnIndex));
...
TEST_P(TableViewTest, UpdateVirtualAccessibilityChildrenBoundsWithResize)
{
    ...
    VerifyTableAccChildrenBounds(table_->GetViewAccessibility(), expected_bounds_after_resize);
}
...
TEST_P(TableViewTest, UpdateVirtualAccessibilityChildrenBoundsHideColumn)
{
    ...
    VerifyTableAccChildrenBounds(table_->GetViewAccessibility(), expected_bounds_after_hiding);
}
...
TEST_P(TableViewTest, GetVirtualAccessibilityCell)
{
    for (size_t i = 0; i < table_->GetRowCount(); ++i) {
        for (size_t j = 0; j < helper_->visible_col_count(); ++j) {
            ...
            EXPECT_EQ(ax::mojom::Role::kGridCell, cell_data.role);
            EXPECT_EQ(i, static_cast<size_t>(cell_data.GetIntAttribute(ax::mojom::IntAttribute::kTableCellRowIndex)));
            EXPECT_EQ(j, static_cast<size_t>(cell_data.GetIntAttribute(ax::mojom::IntAttribute::kTableCellColumnIndex)));
        }
    }
}
...
TEST_P(TableViewTest, ChangingCellFiresAccessibilityEvent)
{
    ...
    table_->GetViewAccessibility().set_accessibility_events_callback(
        base::BindLambdaForTesting([&](const ui::AXPlatformNodeDelegate*, const ax::mojom::Event event) {
...
TEST_P(TableViewTest, ResizeViaGesture)
{
    ...
    EXPECT_NE(0, x);
    // Drag the mouse 1 pixel to the left.
    ui::GestureEvent scroll_begin(x, 0, 0, base::TimeTicks(), ui::GestureEventDetails(ui::EventType::kGestureScrollBegin));
    helper_->header()->OnGestureEvent(&scroll_begin);
    ui::GestureEvent scroll_update(x - 1, 0, 0, base::TimeTicks(), ui::GestureEventDetails(ui::EventType::kGestureScrollUpdate));
...
TEST_P(TableViewTest, ResizeHonorsMinimum)
{
    ...
    int title_width = gfx::GetStringWidth(table_->GetVisibleColumn(0).column.title, helper_->font_list());
...
TEST_P(TableViewTest, Sort)
{
    ...
    VerifyTableViewAndAXOrder("[-1, 0], [0, 1], [1, 1], [2, 2], [11, 11], [12, 12]");
    ...
    VerifyTableViewAndAXOrder("[-1, 20], [-1, 0], [0, 1], [1, 1], [2, 2], [11, 11], [12, 12]");
    ...
    VerifyTableViewAndAXOrder("[-1, 0], [1, 1], [0, 1], [2, 2], [11, 11], [12, 12], [-1, 20]");
    ...
    VerifyTableViewAndAXOrder("[-1, 20], [12, 12], [11, 11], [2, 2], [1, 1], [0, 1], [-1, 0]");
    ...
    VerifyTableViewAndAXOrder("[-1, 20], [12, 12], [11, 11], [2, 2], [1, 1], [-1, 0]");
...
TEST_P(TableViewTest, Tooltip)
{
    ...
    auto local_point_for_row = [&](int row) { return gfx::Point(5, (row + 0.5) * table_->GetRowHeight()); };
    auto expected = [](int row) { return u"Tooltip" + base::NumberToString16(row); };
...
public:
    ...
    void SetRanges(const std::vector<size_t>& ranges)
    {
        ranges_ = ranges;
    }
    ...
    for (size_t i = 0; i < table_->GetRowCount(); ++i) {
        ...
        for (size_t j = 0; j < helper_->visible_col_count(); ++j) {
            ...
            if (PlatformStyle::kTableViewSupportsKeyboardNavigationByCell && j == 0) {
                EXPECT_TRUE(cell_data.GetBoolAttribute(ax::mojom::BoolAttribute::kSelected));
            } else {
                EXPECT_FALSE(cell_data.HasBoolAttribute(ax::mojom::BoolAttribute::kSelected));
            }
        }
    }
}
...
TEST_P(TableViewTest, VirtualAccessibilitySetSelectionRowsInRange)
{
    ...
    for (size_t i = 0; i < table_->GetRowCount(); ++i) {
        ...
        if (i == 2 || i == 3) {
            ...
            EXPECT_FALSE(row_data.GetBoolAttribute(ax::mojom::BoolAttribute::kSelected));
        }
        for (size_t j = 0; j < helper_->visible_col_count(); ++j) {
            const AXVirtualView* cell = helper_->GetVirtualAccessibilityCell(i, j);
            ASSERT_TRUE(cell);
            const ui::AXNodeData& cell_data = cell->GetData();
            if (PlatformStyle::kTableViewSupportsKeyboardNavigationByCell && j == 1 && (i == 2 || i == 3)) {
                EXPECT_TRUE(cell_data.GetBoolAttribute(ax::mojom::BoolAttribute::kSelected));
            } else {
                EXPECT_FALSE(cell_data.HasBoolAttribute(ax::mojom::BoolAttribute::kSelected));
            }
    ...
    for (size_t i = 0; i < table_->GetRowCount(); ++i) {
        ...
        for (size_t j = 0; j < helper_->visible_col_count(); ++j) {
            ...
            ASSERT_TRUE(cell);
            const ui::AXNodeData& cell_data = cell->GetData();
            EXPECT_FALSE(cell_data.GetBoolAttribute(ax::mojom::BoolAttribute::kSelected));
        }
    }
}
...
TEST_P(TableViewTest, SelectAll)
{
    ...
    EXPECT_EQ(1, observer.GetChangedCountAndClear());
    EXPECT_EQ("active=<none> anchor=<none> selection=0 1 2 3", SelectionStateAsString());
...
TEST_P(TableViewTest, AddingRemovingMultipleRows)
{
    ...
    VerifyTableViewAndAXOrder("[10, 10], [11, 11], [12, 12], [0, 1], [1, 1], [2, 2], [3, 0]");
...
TEST_P(TableViewTest, KeyLeftRightScrollRectToVisibleInTableView)
{
    ...
    test::RunScheduledLayout(scroll_view_);
    EXPECT_EQ(1u, helper_->GetActiveVisibleColumnIndex());
    EXPECT_EQ(table_->GetVisibleBounds(), gfx::Rect(kColumn0Width, visible_bounds.y(), visible_bounds.width(), visible_bounds.height()));
...
TEST_P(TableViewTest, KeyLeftRightScrollRectToVisibleInTableHeader)
{
    ...
    test::RunScheduledLayout(scroll_view_);
    EXPECT_EQ(1u, helper_->GetActiveVisibleColumnIndex());
    EXPECT_EQ(table_->GetVisibleBounds(), gfx::Rect(table_->GetVisibleColumn(0).width, visible_bounds.y(), visible_bounds.width(), visible_bounds.height()));
...
TEST_P(TableViewTest, TableHeaderRowAccessibleViewFocusable)
{
    ...
    EXPECT_TRUE(table_->HasFocus());
    EXPECT_TRUE(table_->header_row_is_active());
    EXPECT_EQ(helper_->GetVirtualAccessibilityHeaderRow(), table_->GetViewAccessibility().FocusedVirtualChild());
    ...
    EXPECT_TRUE(table_->HasFocus());
    EXPECT_FALSE(table_->header_row_is_active());
    EXPECT_NE(helper_->GetVirtualAccessibilityHeaderRow(), table_->GetViewAccessibility().FocusedVirtualChild());
}
...
TEST_P(TableViewTest, TableHeaderColumnAccessibleViewsFocusable)
{
    ...
    EXPECT_TRUE(table_->HasFocus());
    EXPECT_TRUE(table_->header_row_is_active());
    EXPECT_EQ(helper_->GetVirtualAccessibilityHeaderRow(), view_accessibility.FocusedVirtualChild());
    ...
    RunPendingMessages();
    ASSERT_EQ(0u, helper_->GetActiveVisibleColumnIndex());
    EXPECT_EQ(helper_->GetVirtualAccessibilityHeaderCell(0), view_accessibility.FocusedVirtualChild());

    PressKey(ui::VKEY_RIGHT);
    RunPendingMessages();
    ASSERT_EQ(1u, helper_->GetActiveVisibleColumnIndex());
    EXPECT_EQ(helper_->GetVirtualAccessibilityHeaderCell(1), view_accessibility.FocusedVirtualChild());
...
protected:
    ...
    {
        delegate_.RegisterDeleteDelegateCallback(base::BindOnce(&TableViewFocusTest::OnDeleteDelegate, base::Unretained(this)));
        delegate_.RegisterWidgetInitializedCallback(base::BindOnce(&TableViewFocusTest::OnWidgetInitialized, base::Unretained(this)));
        return &delegate_;
    }

    void OnWidgetInitialized()
    {
        delegate_.GetWidget()->GetFocusManager()->AddFocusChangeListener(&listener_);
    }

    void OnDeleteDelegate()
    {
        delegate_.GetWidget()->GetFocusManager()->RemoveFocusChangeListener(&listener_);
    }
...
INSTANTIATE_TEST_SUITE_P(All, TableViewFocusTest,
    testing::Combine(/*use_default_construction=*/testing::Bool(),
        /*use_rtl=*/testing::Bool()));
...
public:
    ...
    TableViewDefaultConstructabilityTest(const TableViewDefaultConstructabilityTest&) = delete;
    TableViewDefaultConstructabilityTest& operator=(const TableViewDefaultConstructabilityTest&) = delete;
...
{
    auto scroll_view = TableView::CreateScrollViewWithTable(std::make_unique<TableView>());
    scroll_view->SetBounds(0, 0, 10000, 10000);
...
public:
    ...
    {
        return ui::ImageModel::FromImageSkia(gfx::ImageSkia::CreateFrom1xBitmap(icon_));
    }
...
public:
    ...
    TableViewPaintIconBoundsTest& operator=(const TableViewPaintIconBoundsTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        auto table = std::make_unique<TableView>(model_.get(), columns, TableType::kTextOnly, false);
        table_ = table.get();
        auto scroll_view = TableView::CreateScrollViewWithTable(std::move(table));
...
TEST_F(TableViewPaintIconBoundsTest, TestPaintIconBoundsForNormally)
{
    ...
    gfx::ImageSkia image = model_->GetIcon(0).Rasterize(table_->GetColorProvider());
    EXPECT_FALSE(image.isNull());
    int first_column_min_width = cell_margin + cell_element_spacing + kGroupingIndicatorSize;
    gfx::Rect cell_bounds = helper_->GetCellBounds(0, 0);
    // icon can be paint within the cell bounds.
    EXPECT_GT(cell_bounds.width(), first_column_min_width + ui::TableModel::kIconSize);
    ...
    {
        EXPECT_FALSE(base::i18n::IsRTL());
        gfx::Rect dest_image_bounds = helper_->GetPaintIconDestBounds(cell_bounds, text_bounds.x());
        ...
        EXPECT_EQ(dest_image_bounds,
            gfx::Rect(first_column_min_width, cell_bounds.y() + (cell_bounds.height() - ui::TableModel::kIconSize) / 2, ui::TableModel::kIconSize,
                ui::TableModel::kIconSize));
        gfx::Rect src_image_bounds = helper_->GetPaintIconSrcBounds(image.size(), dest_image_bounds.width());
    ...
    }
    {
        ...
        // ui::TableModel::kIconSize.
        EXPECT_EQ(dest_image_bounds,
            gfx::Rect(table_->width() - first_column_min_width - ui::TableModel::kIconSize,
                cell_bounds.y() + (cell_bounds.height() - ui::TableModel::kIconSize) / 2, ui::TableModel::kIconSize, ui::TableModel::kIconSize));
        gfx::Rect src_image_bounds = helper_->GetPaintIconSrcBounds(image.size(), dest_image_bounds.width());
        EXPECT_EQ(src_image_bounds, gfx::Rect(image.size()));
        base::i18n::SetRTLForTesting(false);
    }
...
TEST_F(TableViewPaintIconBoundsTest, TestPaintIconBoundsForClipped)
{
    ...
    text_bounds.Inset(gfx::Insets().set_left(kGroupingIndicatorSize + cell_element_spacing));
    {
        ...
        EXPECT_EQ(dest_image_bounds,
            gfx::Rect(
                first_column_min_width, cell_bounds.y() + (cell_bounds.height() - ui::TableModel::kIconSize) / 2, dest_image_width, ui::TableModel::kIconSize));
        ...
        EXPECT_EQ(src_image_bounds, gfx::Rect(0, 0, image.width() / 2, image.height()));
    }
    {
        ...
        EXPECT_EQ(dest_image_bounds,
            gfx::Rect(table_->width() - (first_column_min_width + dest_image_width), cell_bounds.y() + (cell_bounds.height() - ui::TableModel::kIconSize) / 2,
        ...
        EXPECT_EQ(src_image_bounds, gfx::Rect(image.width() - src_image_bounds.width(), 0, image.width() / 2, image.height()));
        base::i18n::SetRTLForTesting(false);
    }
}
