...
class TableHeader::HighlightPathGenerator : public views::HighlightPathGenerator {
public:
...
{
    HighlightPathGenerator::Install(this, std::make_unique<TableHeader::HighlightPathGenerator>());
...
{
    return table_->header_style().cell_vertical_padding.value_or(kCellVerticalPaddingDefault);
}

int TableHeader::GetCellHorizontalPadding() const
{
    return table_->header_style().cell_horizontal_padding.value_or(kCellHorizontalPaddingDefault);
}

int TableHeader::GetResizeBarVerticalPadding() const
{
    return table_->header_style().resize_bar_vertical_padding.value_or(kVerticalSeparatorPaddingDefault);
}

int TableHeader::GetSeparatorHorizontalPadding() const
{
    return table_->header_style().separator_horizontal_padding.value_or(kHorizontalSeparatorPaddingDefault);
}
...
void TableHeader::OnPaint(gfx::Canvas* canvas)
{
    ...
    const SkColor text_color = color_provider->GetColor(ui::kColorTableHeaderForeground);
    const SkColor separator_color = color_provider->GetColor(ui::kColorTableHeaderSeparator);
    ...
    SkColor border_color = color_provider->GetColor(ui::kColorFocusableBorderUnfocused);
    canvas->DrawSharpLine(
        gfx::PointF(separator_horizontal_padding, height() - 1), gfx::PointF(width() - separator_horizontal_padding, height() - 1), border_color);
    ...
    for (const auto& column : columns) {
        if (column.width >= 2) {
            ...
            canvas->DrawSharpLine(
                gfx::PointF(separator_x, resize_bar_vertical_padding), gfx::PointF(separator_x, height() - resize_bar_vertical_padding), separator_color);
        }
        ...
        const int title_width = gfx::GetStringWidth(column.column.title, font_list_);
        const bool paint_sort_indicator = (column.column.id == sorted_column_id && title_width + sort_indicator_width <= width);

        if (paint_sort_indicator)
            width -= sort_indicator_width;

        canvas->DrawStringRectWithFlags(column.column.title, font_list_, text_color,
            gfx::Rect(GetMirroredXWithWidthInView(x, width), vertical_padding, width, height() - vertical_padding * 2),
            TableColumnAlignmentToCanvasAlignment(GetMirroredTableColumnAlignment(column.column.alignment)));
        ...
        if (paint_sort_indicator) {
            ...
            if (table_->sort_descriptors()[0].ascending) {
                indicator_path.moveTo(SkIntToScalar(indicator_x), SkIntToScalar(indicator_y + kSortIndicatorSize));
                indicator_path.lineTo(SkIntToScalar(indicator_x + kSortIndicatorSize * scale), SkIntToScalar(indicator_y + kSortIndicatorSize));
                indicator_path.lineTo(SkIntToScalar(indicator_x + kSortIndicatorSize / 2 * scale), SkIntToScalar(indicator_y));
            } else {
                indicator_path.moveTo(SkIntToScalar(indicator_x), SkIntToScalar(indicator_y));
                indicator_path.lineTo(SkIntToScalar(indicator_x + kSortIndicatorSize * scale), SkIntToScalar(indicator_y));
                indicator_path.lineTo(SkIntToScalar(indicator_x + kSortIndicatorSize / 2 * scale), SkIntToScalar(indicator_y + kSortIndicatorSize));
            }
...
gfx::Size TableHeader::CalculatePreferredSize(const SizeBounds& /*available_size*/) const
{
...
{
    return GetResizeColumn(GetMirroredXInView(event.x())).has_value() ? ui::mojom::CursorType::kColumnResize : View::GetCursor(event);
}
...
{
    if (is_resizing()) {
        table_->SetVisibleColumnWidth(resize_details_->column_index, resize_details_->initial_width);
    }
    resize_details_.reset();
}
...
{
    View::OnThemeChanged();
    SetBackground(CreateSolidBackground(GetColorProvider()->GetColor(ui::kColorTableHeaderBackground)));
}

void TableHeader::ResizeColumnViaKeyboard(size_t index, TableView::AdvanceDirection direction)
{
    const TableView::VisibleColumn& column = table_->GetVisibleColumn(index);
    const int needed_for_title = gfx::GetStringWidth(column.column.title, font_list_) + 2 * GetCellHorizontalPadding();
    ...
    table_->SetVisibleColumnWidth(index, std::max({ kMinColumnWidth, needed_for_title, new_width }));
}
...
{
    const std::optional<size_t> active_index = table_->GetActiveVisibleColumnIndex();
    DCHECK(active_index.has_value());
    const TableView::VisibleColumn& column = table_->GetVisibleColumn(active_index.value());
...
bool TableHeader::StartResize(const ui::LocatedEvent& event)
{
    ...
    const std::optional<size_t> index = GetResizeColumn(GetMirroredXInView(event.x()));
    if (!index.has_value())
        return false;
    ...
    resize_details_->initial_width = table_->GetVisibleColumn(index.value()).width;
    return true;
}
...
void TableHeader::ContinueResize(const ui::LocatedEvent& event)
{
    ...
    const TableView::VisibleColumn& column = table_->GetVisibleColumn(resize_details_->column_index);
    const int needed_for_title = gfx::GetStringWidth(column.column.title, font_list_) + 2 * GetCellHorizontalPadding();
    table_->SetVisibleColumnWidth(resize_details_->column_index, std::max({ kMinColumnWidth, needed_for_title, resize_details_->initial_width + delta }));
}
...
void TableHeader::ToggleSortOrder(const ui::LocatedEvent& event)
{
    ...
    if (!index.has_value())
        return;
    const TableView::VisibleColumn& column(table_->GetVisibleColumn(index.value()));
...
std::optional<size_t> TableHeader::GetResizeColumn(int x) const
{
    ...
    const TableView::VisibleColumn& column(table_->GetVisibleColumn(index.value()));
    if (index.value() > 0 && x >= column.x - kResizePadding && x <= column.x + kResizePadding) {
        return index.value() - 1;
    }
    const int max_x = column.x + column.width;
    return (x >= max_x - kResizePadding && x <= max_x + kResizePadding) ? std::make_optional(index.value()) : std::nullopt;
}
