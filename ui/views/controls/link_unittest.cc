...
public:
    ...
    void SetUp() override
    {
        ...
        event_generator_ = std::make_unique<ui::test::EventGenerator>(GetContext(), widget()->GetNativeWindow());
    }
...
protected:
    void CreateWidgetContent(View* container) override
    {
        ...
        link_->SetBoundsRect(gfx::ScaleToEnclosedRect(container->GetLocalBounds(), 0.5f));
    }
...
TEST_F(LinkTest, TestLinkClick)
{
    ...
    link()->SetCallback(base::BindRepeating([](bool* link_clicked) { *link_clicked = true; }, &link_clicked));
...
TEST_F(LinkTest, TestLinkTap)
{
    ...
    link()->SizeToPreferredSize();
    gfx::Point point = link()->bounds().CenterPoint();
    ui::GestureEvent tap_event(point.x(), point.y(), 0, ui::EventTimeForNow(), ui::GestureEventDetails(ui::EventType::kGestureTap));
...
TEST_F(LinkTest, TestUnderlineOnHover)
{
    ...
    const auto link_underlined = [link = link()]() { return !!(link->font_list().GetFontStyle() & gfx::Font::UNDERLINE); };
...
TEST_F(LinkTest, AccessibleProperties)
{
    ...
    EXPECT_EQ(data.GetString16Attribute(ax::mojom::StringAttribute::kName), u"TestLink");
