...
float GetNearestAllowedValue(const base::flat_set<float>& allowed_values, float suggested_value)
{
    if (allowed_values.empty())
        return suggested_value;

    const base::flat_set<float>::const_iterator greater = allowed_values.upper_bound(suggested_value);
...
void Slider::SetAllowedValues(const base::flat_set<float>* allowed_values)
{
    ...
    const float new_value = (position == allowed_values_.end()) ? *allowed_values_.cbegin() : *position;
    if (new_value != value_)
        SetValue(new_value);
}
...
{
    return move_animation_ && move_animation_->is_animating() ? move_animation_->CurrentValueBetween(initial_animating_value_, value_) : value_;
}
...
void Slider::AnimationProgressed(const gfx::Animation* animation)
{
    if (animation == &highlight_animation_) {
        thumb_highlight_radius_ = animation->CurrentValueBetween(kThumbRadius, kThumbHighlightRadius);
    }
...
void Slider::PrepareForMove(const int new_x)
{
    ...
    if (candidate_x >= value_indicator_radius_ - kThumbRadius && candidate_x < value_indicator_radius_ + kThumbRadius)
...
void Slider::MoveButtonTo(const gfx::Point& point)
{
    ...
    // Calculate the value.
    int amount = base::i18n::IsRTL() ? width() - inset.left() - point.x() - initial_button_offset_ : point.x() - inset.left() - initial_button_offset_;
    SetValueInternal(static_cast<float>(amount) / (width() - inset.width() - 2 * value_indicator_radius_), SliderChangeReason::kByUser);
}
...
gfx::Size Slider::CalculatePreferredSize(const SizeBounds& available_size) const
{
    constexpr int kSizeMajor = 200;
    constexpr int kSizeMinor = 40;

    return gfx::Size(std::max(available_size.width().value_or(0), kSizeMajor), kSizeMinor);
}
...
bool Slider::OnKeyPressed(const ui::KeyEvent& event)
{
    ...
    if (allowed_values_.empty()) {
        SetValueInternal(value_ + direction * keyboard_increment_, SliderChangeReason::kByUser);
    } else {
        // discrete slider.
        if (direction > 0) {
            const base::flat_set<float>::const_iterator greater = allowed_values_.upper_bound(value_);
            SetValueInternal(greater == allowed_values_.cend() ? *allowed_values_.crend() : *greater, SliderChangeReason::kByUser);
        } else {
            const base::flat_set<float>::const_iterator lesser = allowed_values_.lower_bound(value_);
            // Current value must be in the list of allowed values.
            DCHECK(lesser != allowed_values_.cend());
            SetValueInternal(lesser == allowed_values_.cbegin() ? *allowed_values_.cbegin() : *std::prev(lesser), SliderChangeReason::kByUser);
        }
    }
    return true;
}
...
void Slider::OnPaint(gfx::Canvas* canvas)
{
    ...
    canvas->DrawRoundRect(gfx::Rect(content.x(), y, full - GetSliderExtraPadding(), kLineThickness), kSliderRoundedRadius, slider_flags);
    slider_flags.setColor(GetTroughColor());
    canvas->DrawRoundRect(
        gfx::Rect(x + kThumbRadius + GetSliderExtraPadding(), y, empty - GetSliderExtraPadding(), kLineThickness), kSliderRoundedRadius, slider_flags);

    gfx::Point thumb_center(x, content.height() / 2);

    // Paint the thumb highlight if it exists.
    const int thumb_highlight_radius = HasFocus() ? kThumbHighlightRadius : thumb_highlight_radius_;
    if (thumb_highlight_radius > kThumbRadius) {
        ...
        canvas->DrawCircle(thumb_center, thumb_highlight_radius, highlight_background);
...
{
    GetViewAccessibility().SetValue(base::UTF8ToUTF16(base::StringPrintf("%d%%", static_cast<int>(value_ * 100 + 0.5))));
}
