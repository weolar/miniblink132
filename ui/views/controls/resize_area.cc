...
{
    return GetEnabled() ? ui::Cursor(ui::mojom::CursorType::kEastWestResize) : ui::Cursor();
}
...
void ResizeArea::OnGestureEvent(ui::GestureEvent* event)
{
    if (event->type() == ui::EventType::kGestureTapDown) {
        ...
        event->SetHandled();
    } else if (event->type() == ui::EventType::kGestureScrollBegin || event->type() == ui::EventType::kGestureScrollUpdate) {
...
void ResizeArea::ReportResizeAmount(int resize_amount, bool last_update)
{
    ...
    delegate_->OnResize(base::i18n::IsRTL() ? -resize_amount : resize_amount, last_update);
}
