...
// "", "abc", "def", "", "", "hij", and "".
void SplitStringIntoParagraphs(const std::u16string& text, std::vector<std::u16string>* paragraphs)
{
    paragraphs->clear();
...
MessageBoxView::MessageBoxView(const std::u16string& message, bool detect_directionality)
    : inter_row_vertical_spacing_(LayoutProvider::Get()->GetDistanceMetric(DISTANCE_RELATED_CONTROL_VERTICAL))
    , message_width_(kDefaultMessageWidth)
{
    ...
    auto message_contents = Builder<BoxLayoutView>()
    ...
    auto add_label = [&message_contents, this](const std::u16string& text, gfx::HorizontalAlignment alignment) {
        message_contents.AddChild(Builder<Label>()
                                      .SetText(text)
                                      .SetTextContext(style::CONTEXT_DIALOG_BODY_TEXT)
                                      .SetMultiLine(!text.empty())
                                      .SetAllowCharacterBreak(true)
                                      .SetHorizontalAlignment(alignment)
                                      .CustomConfigure(base::BindOnce([](std::vector<raw_ptr<Label, VectorExperimental>>& message_labels,
                                                                          Label* message_label) { message_labels.push_back(message_label); },
    ...
    Builder<MessageBoxView>(this)
        .SetOrientation(BoxLayout::Orientation::kVertical)
        .AddChildren(Builder<ScrollView>()
                         .CopyAddressTo(&scroll_view_)
                         .ClipHeightTo(0, provider->GetDistanceMetric(DISTANCE_DIALOG_SCROLLABLE_AREA_MAX_HEIGHT))
            ...
            Builder<Textfield>()
                ...
                .SetVisible(false)
                .CustomConfigure(base::BindOnce([](Textfield* prompt_field) { prompt_field->GetViewAccessibility().SetIsIgnored(true); })),
            Builder<Checkbox>().CopyAddressTo(&checkbox_).SetProperty(kMarginsKey, horizontal_insets).SetVisible(false),
            Builder<Link>().CopyAddressTo(&link_).SetProperty(kMarginsKey, horizontal_insets).SetHorizontalAlignment(gfx::ALIGN_LEFT).SetVisible(false))
        .BuildChildren();
...
{
    return prompt_field_ && prompt_field_->GetVisible() ? prompt_field_.get() : nullptr;
}

std::u16string MessageBoxView::GetInputText()
{
    return prompt_field_ && prompt_field_->GetVisible() ? prompt_field_->GetText() : std::u16string();
}
...
void MessageBoxView::SetLink(const std::u16string& text, Link::ClickedCallback callback)
{
    DCHECK(!text.empty());
...
gfx::Size MessageBoxView::CalculatePreferredSize(const SizeBounds& available_size) const
{
    return BoxLayoutView::CalculatePreferredSize(SizeBounds(message_width_, available_size.height()));
}

void MessageBoxView::ViewHierarchyChanged(const ViewHierarchyChangedDetails& details)
{
...
bool MessageBoxView::AcceleratorPressed(const ui::Accelerator& accelerator)
{
    ...
    scw.WriteText(std::accumulate(
        message_labels_.cbegin(), message_labels_.cend(), std::u16string(), [](const std::u16string& left, Label* right) { return left + right->GetText(); }));
    return true;
}
...
void MessageBoxView::ResetLayoutManager()
{
    ...
    views::DialogContentType trailing_content_type = views::DialogContentType::kText;
    ...
    const LayoutProvider* provider = LayoutProvider::Get();
    gfx::Insets border_insets = provider->GetDialogInsetsForContentType(views::DialogContentType::kText, trailing_content_type);
...
gfx::Insets MessageBoxView::GetHorizontalInsets(const LayoutProvider* provider)
{
    gfx::Insets horizontal_insets = provider->GetInsetsMetric(views::INSETS_DIALOG);
