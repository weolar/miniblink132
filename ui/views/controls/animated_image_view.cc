...
bool AreAnimatedImagesEqual(const lottie::Animation& animation_1, const lottie::Animation& animation_2)
{
...
void AnimatedImageView::SetAnimatedImage(std::unique_ptr<lottie::Animation> animated_image)
{
    if (animated_image_ && AreAnimatedImagesEqual(*animated_image, *animated_image_)) {
        Stop();
        return;
    }
...
void AnimatedImageView::Play(std::optional<lottie::Animation::PlaybackConfig> playback_config)
{
    ...
    if (!playback_config) {
        playback_config = lottie::Animation::PlaybackConfig::CreateDefault(*animated_image_);
    }
    set_check_active_duration(playback_config->style != lottie::Animation::Style::kLoop);

    if (GetWidget()) {
        DoPlay(std::move(*playback_config));
    } else {
        // Playback will start in `AddedToWidget`.
        playback_config_ = std::make_unique<lottie::Animation::PlaybackConfig>(std::move(*playback_config));
    }
}
...
{
    return image_size_.value_or(animated_image_ ? animated_image_->GetOriginalSize() : gfx::Size());
}
...
{
    TRACE_EVENT1("views", "AnimatedImageView::OnAnimationStep", "timestamp", timestamp);
...
void AnimatedImageView::DoPlay(lottie::Animation::PlaybackConfig playback_config)
{
    SetCompositorFromWidget();
