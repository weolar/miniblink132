...
public:
    ...
    {
        ScrollBar* scroll_bar = orientation == VERTICAL ? scroll_view_->vertical_scroll_bar() : scroll_view_->horizontal_scroll_bar();
        return scroll_bar;
    }

    const base::OneShotTimer& GetScrollBarTimer(ScrollBarOrientation orientation)
    {
    ...
    base::RetainingOneShotTimer* GetScrollBarHideTimer(ScrollBarOrientation orientation)
    {
    ...
    View* corner_view()
    {
        return scroll_view_->corner_view_.get();
    }
...
public:
    ...
    const gfx::Point last_location() const
    {
...
void CheckScrollbarVisibility(const ScrollView* scroll_view, ScrollBarOrientation orientation, bool should_be_visible)
{
    const ScrollBar* scrollbar = orientation == HORIZONTAL ? scroll_view->horizontal_scroll_bar() : scroll_view->vertical_scroll_bar();
...
{
    return ui::MouseEvent(type, location, location, base::TimeTicks(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
}
...
protected:
    ...
    void SetOverlayScrollersEnabled(bool enabled)
    {
        ...
        scroller_style_ = std::make_unique<ui::test::ScopedPreferredScrollerStyle>(enabled);
    }
...
private:
    ...
    std::unique_ptr<ui::test::ScopedPreferredScrollerStyle> scroller_style_ = std::make_unique<ui::test::ScopedPreferredScrollerStyle>(false);
...
class WidgetScrollViewTest : public test::WidgetTest, public ui::CompositorObserver {
public:
    ...
    ScrollView* AddScrollViewWithContents(std::unique_ptr<View> contents, bool commit_layers = true)
    {
#if BUILDFLAG(IS_MAC)
        scroller_style_ = std::make_unique<ui::test::ScopedPreferredScrollerStyle>(use_overlay_scrollers_);
#endif
        ...
        ScrollView* scroll_view = widget_->SetContentsView(std::make_unique<ScrollView>());
    ...
    ScrollView* AddScrollViewWithContentSize(const gfx::Size& contents_size, bool commit_layers = true)
    {
    ...
    void WaitForCommit()
    {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, quit_closure_, TestTimeouts::action_timeout());
        run_loop.Run();
    ...
    void TestClickAt(const gfx::Point& location)
    {
        ...
        ui::MouseEvent release(TestLeftMouseAt(location, ui::EventType::kMouseReleased));
...
class WidgetScrollViewTestRTLAndLayers : public WidgetScrollViewTest, public ::testing::WithParamInterface<UiConfig> {
public:
    WidgetScrollViewTestRTLAndLayers()
        : locale_(IsTestingRtl() ? "he" : "en")
    {
        if (IsTestingLayers()) {
            layer_config_.InitAndEnableFeature(::features::kUiCompositorScrollWithLayers);
        } else {
            layer_config_.InitAndDisableFeature(::features::kUiCompositorScrollWithLayers);
        }
    }

    WidgetScrollViewTestRTLAndLayers(const WidgetScrollViewTestRTLAndLayers&) = delete;
    WidgetScrollViewTestRTLAndLayers& operator=(const WidgetScrollViewTestRTLAndLayers&) = delete;

    bool IsTestingRtl() const
    {
        return GetParam() == UiConfig::kRtl || GetParam() == UiConfig::kRtlWithLayers;
    }

    bool IsTestingLayers() const
    {
        return GetParam() == UiConfig::kLtrWithLayers || GetParam() == UiConfig::kRtlWithLayers;
    }
    ...
    {
        const gfx::Point test_mouse_point_in_root = IsTestingRtl() ? gfx::Point(kDefaultWidth - 1, 1) : gfx::Point(1, 1);
...
{
    const gfx::Rect default_outer_bounds(0, 0, 100, 100);
    scroll_view_->SetHorizontalScrollBarMode(ScrollView::ScrollBarMode::kHiddenButEnabled);
    scroll_view_->SetContents(std::make_unique<VerticalResizingView>());
    scroll_view_->SetBoundsRect(default_outer_bounds);
    views::test::RunScheduledLayout(scroll_view_.get());
    EXPECT_FALSE(scroll_view_->vertical_scroll_bar()->GetVisible());
...
{
    const gfx::Rect default_outer_bounds(0, 0, 100, 100);
    scroll_view_->SetVerticalScrollBarMode(ScrollView::ScrollBarMode::kHiddenButEnabled);
...
{
    const gfx::Rect default_outer_bounds(0, 0, 100, 100);
    scroll_view_->SetHorizontalScrollBarMode(ScrollView::ScrollBarMode::kDisabled);
    scroll_view_->SetContents(std::make_unique<VerticalResizingView>());
    scroll_view_->SetBoundsRect(default_outer_bounds);
    views::test::RunScheduledLayout(scroll_view_.get());
    EXPECT_FALSE(scroll_view_->vertical_scroll_bar()->GetVisible());
...
TEST_F(ScrollViewTest, ScrollBars)
{
    ...
    EXPECT_EQ(100 - scroll_view_->GetScrollBarLayoutWidth(), contents->parent()->width());
    ...
    EXPECT_TRUE(!scroll_view_->horizontal_scroll_bar() || !scroll_view_->horizontal_scroll_bar()->GetVisible());
    ...
    EXPECT_EQ(100 - scroll_view_->GetScrollBarLayoutHeight(), contents->parent()->height());
    ...
    scroll_view_->SetBorder(CreateEmptyBorder(gfx::Insets::TLBR(kTopPadding, kLeftPadding, kBottomPadding, kRightPadding)));
    contents->SetBounds(0, 0, 50, 400);
    InvalidateAndRunScheduledLayoutOnScrollView();
    EXPECT_EQ(100 - scroll_view_->GetScrollBarLayoutWidth() - kLeftPadding - kRightPadding, contents->parent()->width());
    ...
    EXPECT_EQ(100 - scroll_view_->GetScrollBarLayoutHeight() - kTopPadding - kBottomPadding, contents->parent()->height());
    ASSERT_TRUE(scroll_view_->horizontal_scroll_bar() != nullptr);
    EXPECT_TRUE(scroll_view_->horizontal_scroll_bar()->GetVisible());
    EXPECT_TRUE(!scroll_view_->vertical_scroll_bar() || !scroll_view_->vertical_scroll_bar()->GetVisible());
    ...
    EXPECT_EQ(100 - kRightPadding, bounds.right());
    EXPECT_EQ(kTopPadding, bounds.y());
    EXPECT_EQ(100 - kBottomPadding - HorizontalScrollBarHeight(), bounds.bottom());
}
...
TEST_F(ScrollViewTest, ScrollBarsWithHeader)
{
    ...
    EXPECT_EQ(100 - scroll_view_->GetScrollBarLayoutWidth(), header->parent()->width());
    ...
    const int expected_scrollbar_y = scroll_view_->vertical_scroll_bar()->OverlapsContent() ? header->bounds().bottom() : header->y();
    ...
    EXPECT_EQ(20, contents->parent()->y());
    EXPECT_EQ(100, contents->parent()->width());
    EXPECT_EQ(100 - scroll_view_->GetScrollBarLayoutHeight() - 20, contents->parent()->height());
    EXPECT_EQ(0, header->parent()->x());
    EXPECT_EQ(0, header->parent()->y());
...
TEST_F(ScrollViewTest, HorizontalScrollBarAccessibleScrollXProperties)
{
    ...
    scroll_view_->GetViewAccessibility().GetAccessibleNodeData(&scroll_view_node_data);
    EXPECT_TRUE(scroll_view_node_data.GetBoolAttribute(ax::mojom::BoolAttribute::kScrollable));
    EXPECT_EQ(scroll_bar->GetMinPosition(), scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollXMin));
    EXPECT_EQ(scroll_bar->GetMaxPosition(), scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollXMax));
    ...
    scroll_view_node_data = ui::AXNodeData();
    scroll_view_->GetViewAccessibility().GetAccessibleNodeData(&scroll_view_node_data);
    EXPECT_EQ(0, scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollXMin));
    EXPECT_EQ(400 - test_api.contents_viewport()->width(), scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollXMax));

    // Scroll the horizontal scrollbar after which kScrollX should be updated.
    EXPECT_EQ(0, scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollX));
    scroll_view_->ScrollToPosition(scroll_bar, 20);
    scroll_view_node_data = ui::AXNodeData();
    scroll_view_->GetViewAccessibility().GetAccessibleNodeData(&scroll_view_node_data);
    EXPECT_EQ(test_api.CurrentOffset().x(), scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollX));
}
...
TEST_F(ScrollViewTest, VerticalScrollBarAccessibleScrollYProperties)
{
    ...
    scroll_view_->GetViewAccessibility().GetAccessibleNodeData(&scroll_view_node_data);
    EXPECT_TRUE(scroll_view_node_data.GetBoolAttribute(ax::mojom::BoolAttribute::kScrollable));
    EXPECT_EQ(scroll_bar->GetMinPosition(), scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollYMin));
    EXPECT_EQ(scroll_bar->GetMaxPosition(), scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollYMax));
    ...
    scroll_view_node_data = ui::AXNodeData();
    scroll_view_->GetViewAccessibility().GetAccessibleNodeData(&scroll_view_node_data);
    EXPECT_EQ(0, scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollYMin));
    EXPECT_EQ(400 - test_api.contents_viewport()->height(), scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollYMax));

    // Scroll the vertical scrollbar after which kScrollY should be updated.
    EXPECT_EQ(0, scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollY));
    scroll_view_->ScrollToPosition(scroll_bar, 20);
    scroll_view_node_data = ui::AXNodeData();
    scroll_view_->GetViewAccessibility().GetAccessibleNodeData(&scroll_view_node_data);
    EXPECT_EQ(test_api.CurrentOffset().y(), scroll_view_node_data.GetIntAttribute(ax::mojom::IntAttribute::kScrollY));
}
...
TEST_F(ScrollViewTest, ScrollViewToVisibleOnContentsRootFocus)
{
    ...
    auto* outer_contents_ptr = scroll_view_->SetContents(std::move(outer_contents));

    auto inner_scroll_view = std::make_unique<ScrollView>();
    auto* inner_scroll_view_ptr = outer_contents_ptr->AddChildView(std::move(inner_scroll_view));
    ...
    auto* inner_contents_ptr = inner_scroll_view_ptr->SetContents(std::move(inner_contents));
    ...
    const int inner_viewport_height = inner_test_api.contents_viewport()->height();
    ...
    const int outer_viewport_height = outer_test_api.contents_viewport()->height();
...
TEST_F(ScrollViewTest, ClipHeightToNormalContentHeight)
{
    ...
    EXPECT_EQ(gfx::Size(kWidth, kNormalContentHeight), scroll_view_->GetPreferredSize({}));

    scroll_view_->SizeToPreferredSize();
    views::test::RunScheduledLayout(scroll_view_.get());

    EXPECT_EQ(gfx::Size(kWidth, kNormalContentHeight), scroll_view_->contents()->size());
...
TEST_F(ScrollViewTest, ClipHeightToShortContentHeight)
{
    ...
    View* contents = scroll_view_->SetContents(std::make_unique<views::StaticSizedView>(gfx::Size(kWidth, kShortContentHeight)));
    ...
    if (contents->layer()) {
        ...
        EXPECT_EQ(gfx::Size(kWidth, kShortContentHeight), scroll_view_->contents()->size());
    }
...
TEST_F(ScrollViewTest, ClipHeightToScrollbarUsesWidth)
{
    ...
    scroll_view_->SetContents(std::make_unique<views::ProportionallySizedView>(1000));
...
// layer.
TEST_F(ScrollViewTest, ContentsViewportLayerWhenContentsDoesNotHaveLayer_ScrollWithLayersDisabled)
{
    // Disabling scroll_with_layers feature explicitly.
    ScrollView scroll_view(ScrollView::ScrollWithLayers::kDisabled);
    ScrollViewTestApi test_api(&scroll_view);
...
TEST_F(ScrollViewTest, ContentsLayerCannotBeChangedAfterContentsAreSet_ScrollWithLayersEnabled)
{
...
TEST_F(ScrollViewTest, ContentsLayerCanBeChangedAfterContentsAreSet_ScrollWithLayersDisabled)
{
    ScrollView scroll_view(ScrollView::ScrollWithLayers::kDisabled);
    ScrollViewTestApi test_api(&scroll_view);
...
TEST_F(ScrollViewTest, ContentsViewportLayerUsedWhenScrollViewContentsAreChanged_ScrollWithLayersDisabled)
{
    // Disabling scroll_with_layers feature explicitly.
    ScrollView scroll_view(ScrollView::ScrollWithLayers::kDisabled);
    ScrollViewTestApi test_api(&scroll_view);
...
// enabled.
TEST_F(ScrollViewTest, ContentsViewportLayerUsedWhenNotDrawnUsedForContents_ScrollWithLayersEnabled)
{
    ScrollView scroll_view(ScrollView::ScrollWithLayers::kEnabled);
    ScrollViewTestApi test_api(&scroll_view);
...
TEST_F(ScrollViewTest, ContentsViewportLayerHasRoundedCorners_ScrollWithLayersEnabled)
{
    ScrollView scroll_view(ScrollView::ScrollWithLayers::kEnabled);
    ScrollViewTestApi test_api(&scroll_view);
    ASSERT_TRUE(test_api.contents_viewport()->layer());
    ...
    EXPECT_EQ(test_api.contents_viewport()->layer()->rounded_corner_radii(), corner_radii);
}
...
TEST_F(WidgetScrollViewTest, OverlayScrollBarsCannotProcessEventsWhenTransparent)
{
    // Allow expectations to distinguish between fade outs and immediate changes.
    ui::ScopedAnimationDurationScaleMode really_animate(ui::ScopedAnimationDurationScaleMode::NON_ZERO_DURATION);

    SetUseOverlayScrollers();

    ScrollView* scroll_view = AddScrollViewWithContentSize(gfx::Size(kDefaultWidth * 5, kDefaultHeight * 5));
    ScrollViewTestApi test_api(scroll_view);
    ...
    ui::test::EventGenerator generator(GetContext(), scroll_view->GetWidget()->GetNativeWindow());
...
TEST_F(WidgetScrollViewTest, ScrollersOnRest)
{
    ...
    ScrollView* scroll_view = AddScrollViewWithContentSize(gfx::Size(kDefaultWidth * 5, kDefaultHeight * 5));
    ScrollViewTestApi test_api(scroll_view);
    const auto bar = std::to_array<ScrollBar*>({ test_api.GetScrollBar(HORIZONTAL), test_api.GetScrollBar(VERTICAL) });
    const auto hide_timer
        = std::to_array<base::RetainingOneShotTimer*>({ test_api.GetScrollBarHideTimer(HORIZONTAL), test_api.GetScrollBarHideTimer(VERTICAL) });
    ...
    generator.ScrollSequence(generator.current_screen_location(), base::TimeDelta(), 0, y_offset, kSteps, kNnumFingers);
    ...
    generator.ScrollSequence(generator.current_screen_location(), base::TimeDelta(), x_offset, 0, kSteps, kNnumFingers);
    for (ScrollBarOrientation orientation : { HORIZONTAL, VERTICAL }) {
...
TEST_F(ScrollViewTest, VerticalOverflowIndicators)
{
    ...
    scroll_view_->SetSize(gfx::Size(kWidth + test_api.GetScrollBar(VERTICAL)->GetThickness(), kMaxHeight));
...
TEST_F(ScrollViewTest, HorizontalOverflowIndicators)
{
    ...
    scroll_view_->SetSize(gfx::Size(kWidth, kHeight + test_api.GetScrollBar(HORIZONTAL)->GetThickness()));
...
TEST_F(ScrollViewTest, VerticalWithHeaderOverflowIndicators)
{
    ...
    // Make sure the size is set such that no horizontal scrollbar gets shown.
    scroll_view_->SetSize(gfx::Size(kWidth + test_api.GetScrollBar(VERTICAL)->GetThickness(), kMaxHeight + header_ptr->height()));
...
TEST_F(ScrollViewTest, CustomOverflowIndicator)
{
    ...
    scroll_view_->SetHorizontalScrollBarMode(views::ScrollView::ScrollBarMode::kHiddenButEnabled);
    scroll_view_->SetVerticalScrollBarMode(views::ScrollView::ScrollBarMode::kHiddenButEnabled);
    ...
    View* left_indicator = scroll_view_->SetCustomOverflowIndicator(OverflowIndicatorAlignment::kLeft, std::make_unique<View>(), 1, true);
    ...
    View* top_indicator = scroll_view_->SetCustomOverflowIndicator(OverflowIndicatorAlignment::kTop, std::make_unique<View>(), 20, false);
    ...
    View* right_indicator = scroll_view_->SetCustomOverflowIndicator(OverflowIndicatorAlignment::kRight, std::make_unique<View>(), -1, true);
    EXPECT_EQ(gfx::Rect(100, 0, 0, 100), right_indicator->bounds());

    // Thicker than the scrollview is strange, but works as you'd expect.
    View* bottom_indicator = scroll_view_->SetCustomOverflowIndicator(OverflowIndicatorAlignment::kBottom, std::make_unique<View>(), 1000, true);
...
TEST_F(ScrollViewTest, IgnoreOverlapWithDisabledHorizontalScroll)
{
    ...
    scroll_view_->SetHorizontalScrollBar(std::make_unique<TestScrollBar>(ScrollBar::Orientation::kHorizontal,
        /*overlaps_content=*/true, kThickness));
    // Assume vertical scroll bar is custom and it we want it to not overlap.
    scroll_view_->SetVerticalScrollBar(std::make_unique<TestScrollBar>(ScrollBar::Orientation::kVertical,
...
TEST_F(WidgetScrollViewTest, ScrollTrackScrolling)
{
    ...
    ScrollView* scroll_view = AddScrollViewWithContentSize(gfx::Size(10, kDefaultHeight * 5));
    ScrollViewTestApi test_api(scroll_view);
...
TEST_F(WidgetScrollViewTest, EventLocation)
{
    ...
    // Scroll down a page.
    contents_ptr->ScrollRectToVisible(gfx::Rect(0, kDefaultHeight, 1, kDefaultHeight));
    TestClickAt(location_in_widget);
    EXPECT_EQ(gfx::Point(10, 10 + kDefaultHeight), contents_ptr->last_location());

    // Scroll right a page (and back up).
    contents_ptr->ScrollRectToVisible(gfx::Rect(kDefaultWidth, 0, kDefaultWidth, 1));
    TestClickAt(location_in_widget);
    EXPECT_EQ(gfx::Point(10 + kDefaultWidth, 10), contents_ptr->last_location());

    // Scroll both directions.
    contents_ptr->ScrollRectToVisible(gfx::Rect(kDefaultWidth, kDefaultHeight, kDefaultWidth, kDefaultHeight));
    TestClickAt(location_in_widget);
    EXPECT_EQ(gfx::Point(10 + kDefaultWidth, 10 + kDefaultHeight), contents_ptr->last_location());
}
...
TEST_P(WidgetScrollViewTestRTLAndLayers, ScrollOffsetWithoutLayers)
{
    ...
    constexpr gfx::Size kContentSize(kCellWidth * kNesting, kCellHeight * kNesting);
    ...
    if (IsTestingRtl()) {
        ...
        EXPECT_EQ(gfx::Point(kDefaultWidth - 1, 1), HitTestInCorner(scroll_view->GetWidget()->GetRootView(), false));
        EXPECT_EQ(gfx::Point(kContentSize.width() - 1, 1), HitTestInCorner(scroll_view->contents(), false));
    } else {
        EXPECT_EQ(gfx::Point(1, 1), HitTestInCorner(scroll_view->GetWidget()->GetRootView(), false));
        EXPECT_EQ(gfx::Point(1, 1), HitTestInCorner(scroll_view->contents(), false));
    }
    ...
    EXPECT_EQ(gfx::Point(1, kCellHeight * 2 + 1), HitTestInCorner(scroll_view->contents()));

    // Test horizontal scrolling.
    offset.set_x(kCellWidth * 2);
    scroll_view->contents()->ScrollRectToVisible(offset);
    EXPECT_EQ(gfx::PointF(offset.x(), offset.y()), test_api.CurrentOffset());
    EXPECT_EQ(gfx::Point(kCellWidth * 2 + 1, kCellHeight * 2 + 1), HitTestInCorner(scroll_view->contents()));
    ...
    constexpr gfx::Rect kCellRect(kCellWidth, kCellHeight, kContentSize.width(), kContentSize.height());
    for (int i = 1; i < kNesting; ++i) {
        ...
        EXPECT_EQ(gfx::PointF(kCellWidth * i - x_offset_in_cell, kCellHeight * i - y_offset_in_cell), test_api.CurrentOffset());

        // Now scroll the rest.
        deepest_view->ScrollViewToVisible();
        EXPECT_EQ(gfx::PointF(kCellWidth * i, kCellHeight * i), test_api.CurrentOffset());
    ...
    EXPECT_EQ(gfx::PointF(kCellWidth * (kNesting - 1), kCellHeight * (kNesting - 1)), test_api.CurrentOffset());
}
...
TEST_P(WidgetScrollViewTestRTLAndLayers, ScrollOffsetUsingLayers)
{
    ...
    ScrollView* scroll_view = AddScrollViewWithContentSize(gfx::Size(kDefaultWidth * 5, kDefaultHeight * 5), false);
    ScrollViewTestApi test_api(scroll_view);
    ...
    cc::ElementId element_id = container->layer()->cc_layer_for_testing()->element_id();
    ...
    EXPECT_TRUE(compositor->ScrollLayerTo(element_id, gfx::PointF(0, offset.y())));
    EXPECT_EQ(gfx::PointF(0, offset.y()), test_api.CurrentOffset());
...
static void ApplyScrollEvent(const ScrollViewTestApi& test_api, ScrollView* scroll_view, ui::ScrollEvent& scroll_event)
{
...
TEST_F(WidgetScrollViewTest, CompositedScrollEvents)
{
    ...
    ui::ScrollEvent scroll(ui::EventType::kScroll, gfx::Point(), base::TimeTicks::Now(), 0, 0, -10, 0, -10, 3);
...
{
    // Set up with a vertical scroll bar.
    ScrollView* scroll_view = AddScrollViewWithContentSize(gfx::Size(kDefaultHeight * 5, 10));
...
TEST_F(WidgetScrollViewTest, DISABLED_CompositedTransposedScrollEventsHorizontalComponentIsLarger)
{
    // Set up with a vertical scroll bar.
    ScrollView* scroll_view = AddScrollViewWithContentSize(gfx::Size(kDefaultHeight * 5, 10));
    scroll_view->SetTreatAllScrollEventsAsHorizontal(true);
    ...
    ui::ScrollEvent scroll(ui::EventType::kScroll, gfx::Point(), base::TimeTicks::Now(), 0, -10, 7, -10, 7, 3);
    ApplyScrollEvent(test_api, scroll_view, scroll);
...
TEST_F(WidgetScrollViewTest, CompositedTransposedScrollEventsVerticalComponentIsLarger)
{
    // Set up with a vertical scroll bar.
    ScrollView* scroll_view = AddScrollViewWithContentSize(gfx::Size(kDefaultHeight * 5, 10));
    scroll_view->SetTreatAllScrollEventsAsHorizontal(true);
    ...
    // component is greater.
    ui::ScrollEvent scroll(ui::EventType::kScroll, gfx::Point(), base::TimeTicks::Now(), 0, 7, -10, 7, -10, 3);
    ApplyScrollEvent(test_api, scroll_view, scroll);
...
TEST_F(WidgetScrollViewTest, UnboundedScrollViewUsesContentPreferredSize)
{
    ...
    ScrollView* scroll_view = AddScrollViewWithContents(std::move(contents), true);
...
INSTANTIATE_TEST_SUITE_P(All, WidgetScrollViewTestRTLAndLayers,
    ::testing::Values(UiConfig::kLtr, UiConfig::kRtl, UiConfig::kLtrWithLayers, UiConfig::kRtlWithLayers), &UiConfigToString);
