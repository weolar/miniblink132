...
LinkFragment::LinkFragment(const std::u16string& title, int text_context, int text_style, LinkFragment* other_fragment)
...
{
    return GetEnabled() && (HasFocus() || IsMouseHovered() || GetForceUnderline());
}

void LinkFragment::RecalculateFont()
{
    // Check whether any link fragment should be underlined.
    const bool should_be_underlined = InvokeOnFragments(&LinkFragment::IsUnderlined, this);

    // If the style differs from the current one, update.
    if ((font_list().GetFontStyle() & gfx::Font::UNDERLINE) != should_be_underlined) {
        InvokeOnFragments(
            [should_be_underlined](LinkFragment* fragment) {
                const int style = fragment->font_list().GetFontStyle();
                const int intended_style = should_be_underlined ? (style | gfx::Font::UNDERLINE) : (style & ~gfx::Font::UNDERLINE);
                fragment->Label::SetFontList(fragment->font_list().DeriveWithStyle(intended_style));
