...
public:
    BaseRoundedRectBackground(const gfx::RoundedCornersF& radii, int for_border_thickness)
        : radii_(radii)
    ...
    BaseRoundedRectBackground& operator=(const BaseRoundedRectBackground&) = delete;
    ...
    void Paint(gfx::Canvas* canvas, View* view) const override
    {
        ...
        SkPath path;
        SkScalar radii[8] = { radii_.upper_left(), radii_.upper_left(), radii_.upper_right(), radii_.upper_right(), radii_.lower_right(), radii_.lower_right(),
...
public:
    RoundedRectBackground(SkColor color, const gfx::RoundedCornersF& radii, int for_border_thickness)
...
public:
    ...
    ThemedVectorIconBackground& operator=(const ThemedVectorIconBackground&) = delete;
...
public:
    ThemedRoundedRectBackground(ui::ColorId color_id, const gfx::RoundedCornersF& radii, int for_border_thickness)
        : BaseRoundedRectBackground(radii, for_border_thickness)
        , color_id_(color_id)
    {
    }
    ...
    ThemedRoundedRectBackground& operator=(const ThemedRoundedRectBackground&) = delete;
...
std::unique_ptr<Background> CreateRoundedRectBackground(SkColor color, float radius, int for_border_thickness)
{
    return CreateRoundedRectBackground(color, gfx::RoundedCornersF { radius }, for_border_thickness);
}

std::unique_ptr<Background> CreateRoundedRectBackground(SkColor color, const gfx::RoundedCornersF& radii, int for_border_thickness)
{
    ...
    return std::make_unique<RoundedRectBackground>(color, radii, for_border_thickness);
}

std::unique_ptr<Background> CreateThemedRoundedRectBackground(ui::ColorId color_id, float radius, int for_border_thickness)
{
    return CreateThemedRoundedRectBackground(color_id, gfx::RoundedCornersF { radius }, for_border_thickness);
}

std::unique_ptr<Background> CreateThemedRoundedRectBackground(ui::ColorId color_id, float top_radius, float bottom_radius, int for_border_thickness)
{
    return CreateThemedRoundedRectBackground(color_id, gfx::RoundedCornersF { top_radius, top_radius, bottom_radius, bottom_radius }, for_border_thickness);
}

std::unique_ptr<Background> CreateThemedRoundedRectBackground(ui::ColorId color_id, const gfx::RoundedCornersF& radii, int for_border_thickness)
{
    // If the radii is not set, fallback to SolidBackground since it results in
    // more efficient tiling by cc. See crbug.com/1464128.
    if (radii.IsEmpty() && for_border_thickness == 0) {
    ...
    return std::make_unique<ThemedRoundedRectBackground>(color_id, radii, for_border_thickness);
}

std::unique_ptr<Background> CreateThemedVectorIconBackground(const ui::ThemedVectorIcon& icon)
{
...
std::unique_ptr<Background> CreateBackgroundFromPainter(std::unique_ptr<Painter> painter)
{
