...
namespace views {
namespace {
void PrintViewHierarchyImp(const View* view, size_t indent, std::ostringstream* out)
{
...
void PrintFocusHierarchyImp(const View* view, size_t indent, std::ostringstream* out)
{
    *out << std::string(indent, ' ');
    *out << view->GetClassName();
    *out << ' ';
...
std::string PrintViewGraphImpl(const View* view)
{
    ...
    result.append(base::StringPrintf("\\n bounds: (%d, %d), (%dx%d)", view->bounds().x(), view->bounds().y(), view->bounds().width(), view->bounds().height()));
    ...
    if (!view->GetTransform().IsIdentity()) {
        ...
        result.append(base::StringPrintf("\\n translation: (%f, %f)", translation.x(), translation.y()));
...
void PrintWidgetInformation(const Widget& widget, bool detailed, std::ostringstream* out)
{
