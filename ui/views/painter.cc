...
public:
    SolidRoundRectPainter(SkColor bg_color, SkColor stroke_color, gfx::RoundedCornersF radii, const gfx::Insets& insets, SkBlendMode blend_mode, bool antialias,
...
SolidRoundRectPainter::SolidRoundRectPainter(SkColor bg_color, SkColor stroke_color, gfx::RoundedCornersF radii, const gfx::Insets& insets,
    SkBlendMode blend_mode, bool antialias, bool should_border_scale)
    : bg_color_(bg_color)
...
void SolidRoundRectPainter::Paint(gfx::Canvas* canvas, const gfx::Size& size)
{
    ...
    SkPath fill_path;
    const std::array<SkScalar, 8> scaled_radii = { { radii_.upper_left() * scale, radii_.upper_left() * scale, radii_.upper_right() * scale,
        radii_.upper_right() * scale, radii_.lower_right() * scale, radii_.lower_right() * scale, radii_.lower_left() * scale, radii_.lower_left() * scale } };

    UNSAFE_TODO(fill_path.addRoundRect(gfx::RectFToSkRect(fill_rect), scaled_radii.data()));
    ...
    if (stroke_color_ != SK_ColorTRANSPARENT) {
        ...
        UNSAFE_TODO(stroke_path.addRoundRect(gfx::RectFToSkRect(stroke_rect), stroke_radii.data()));
...
SolidFocusPainter::SolidFocusPainter(SkColor color, int thickness, const gfx::InsetsF& insets)
    : color_(color)
...
ImagePainter::ImagePainter(const gfx::ImageSkia& image, const gfx::Insets& insets)
...
// static
void Painter::PaintPainterAt(gfx::Canvas* canvas, Painter* painter, const gfx::Rect& rect)
{
    DCHECK(canvas);
    DCHECK(painter);
    canvas->Save();
...
// static
void Painter::PaintFocusPainter(View* view, gfx::Canvas* canvas, Painter* focus_painter)
{
...
// static
std::unique_ptr<Painter> Painter::CreateSolidRoundRectPainter(SkColor color, float radius, const gfx::Insets& insets, SkBlendMode blend_mode, bool antialias)
{
    return std::make_unique<SolidRoundRectPainter>(color, SK_ColorTRANSPARENT, gfx::RoundedCornersF(radius), insets, blend_mode, antialias, false);
}

// static
std::unique_ptr<Painter> Painter::CreateSolidRoundRectPainterWithVariableRadius(
    SkColor color, gfx::RoundedCornersF radii, const gfx::Insets& insets, SkBlendMode blend_mode, bool antialias)
{
    return std::make_unique<SolidRoundRectPainter>(color, SK_ColorTRANSPARENT, radii, insets, blend_mode, antialias, false);
}

// static
std::unique_ptr<Painter> Painter::CreateRoundRectWith1PxBorderPainter(
    SkColor bg_color, SkColor stroke_color, float radius, SkBlendMode blend_mode, bool antialias, bool should_border_scale)
{
    return std::make_unique<SolidRoundRectPainter>(
        bg_color, stroke_color, gfx::RoundedCornersF(radius), gfx::Insets(), blend_mode, antialias, should_border_scale);
}

// static
std::unique_ptr<Painter> Painter::CreateImagePainter(const gfx::ImageSkia& image, const gfx::Insets& insets)
{
    return std::make_unique<ImagePainter>(image, insets);
}

// static
std::unique_ptr<Painter> Painter::CreateImageGridPainter(const int image_ids[])
{
...
// static
std::unique_ptr<Painter> Painter::CreateSolidFocusPainter(SkColor color, int thickness, const gfx::InsetsF& insets)
{
    return std::make_unique<SolidFocusPainter>(color, thickness, insets);
}

// static
std::unique_ptr<ui::LayerOwner> Painter::CreatePaintedLayer(std::unique_ptr<Painter> painter)
{
