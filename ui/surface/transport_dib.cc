...
// static
std::unique_ptr<TransportDIB> TransportDIB::Map(base::UnsafeSharedMemoryRegion region)
{
...
// static
std::unique_ptr<TransportDIB> TransportDIB::CreateWithHandle(base::UnsafeSharedMemoryRegion region)
{
    return base::WrapUnique(new TransportDIB(std::move(region)));
}

std::unique_ptr<SkCanvas> TransportDIB::GetPlatformCanvas(int w, int h, bool opaque)
{
    ...
    // are too large.
    std::unique_ptr<SkCanvas> canvas = skia::CreatePlatformCanvasWithSharedSection(w, h, opaque, shm_region_.GetPlatformHandle(), skia::RETURN_NULL_ON_FAILURE);
    ...
    if ((!memory() && !Map()) || !VerifyCanvasSize(w, h))
        return nullptr;
    return skia::CreatePlatformCanvasWithPixels(w, h, opaque, static_cast<uint8_t*>(memory()), skia::RETURN_NULL_ON_FAILURE);
#endif
}
