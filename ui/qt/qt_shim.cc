...
SkColor GradientColor(const QGradient& gradient)
{
    ...
    for (int i = 0; i < stops.size(); i++) {
        ...
        float left_interval = i == 0 ? stops[i].first : (stops[i].first - stops[i - 1].first) / 2;
        float right_interval = i == stops.size() - 1 ? 1 - stops[i].first : (stops[i + 1].first - stops[i].first) / 2;
...
float GetScreenScale(const QScreen* screen)
{
    ...
    double scale = screen->devicePixelRatio() * screen->logicalDotsPerInch() / kDefaultPixelDpi;
...
QtShim::QtShim(QtInterface::Delegate* delegate, int* argc, char** argv)
    ...
    , app_(*argc, argv)
{
    connect(&app_, SIGNAL(fontChanged(const QFont&)), this, SLOT(FontChanged(const QFont&)));
    connect(&app_, SIGNAL(paletteChanged(const QPalette&)), this, SLOT(PaletteChanged(const QPalette&)));
    connect(&app_, SIGNAL(screenAdded(QScreen*)), this, SLOT(ScreenAdded(QScreen*)));
    connect(&app_, SIGNAL(screenRemoved(QScreen*)), this, SLOT(ScreenRemoved(QScreen*)));
...
Image QtShim::GetIconForContentType(const String& content_type, int size) const
{
    QMimeDatabase db;
    for (const char* mime : { content_type.c_str(), "application/octet-stream" }) {
        ...
        for (const auto& name : { mt.iconName(), mt.genericIconName() }) {
            ...
            if (auto bytes = image.sizeInBytes()) {
                return { image.width(), image.height(), static_cast<float>(image.devicePixelRatio()), Buffer(image.bits(), bytes) };
            }
        }
    }
    return {};
...
{
    return BrushColor(app_.palette().brush(ColorStateToColorGroup(state), ColorTypeToColorRole(role)));
}

SkColor QtShim::GetFrameColor(ColorState state, bool use_custom_frame) const
{
    constexpr int kSampleSize = 32;
    return TextureColor(DrawHeaderImpl(kSampleSize, kSampleSize, GetColor(ColorType::kWindowBg, state), state, use_custom_frame));
}
...
{
    connect(screen, SIGNAL(logicalDotsPerInchChanged(qreal)), this, SLOT(LogicalDotsPerInchChanged(qreal)));
    connect(screen, SIGNAL(physicalDotsPerInchChanged(qreal)), this, SLOT(PhysicalDotsPerInchChanged(qreal)));
...
Image QtShim::DrawHeader(int width, int height, SkColor default_color, ColorState state, bool use_custom_frame) const
{
    QImage image = DrawHeaderImpl(width, height, default_color, state, use_custom_frame);
    return { width, height, 1.0f, Buffer(image.bits(), image.sizeInBytes()) };
}

QImage QtShim::DrawHeaderImpl(int width, int height, SkColor default_color, ColorState state, bool use_custom_frame) const
{
    ...
    if (use_custom_frame) {
        ...
        opt.rect = QRect(-kBorderWidth, -kBorderWidth, width + 2 * kBorderWidth, height + 2 * kBorderWidth);
        if (state == ColorState::kNormal) {
            opt.titleBarState = QStyle::State_Active;
        }
        app_.style()->drawComplexControl(QStyle::CC_TitleBar, &opt, &painter, nullptr);
    } else {
        painter.fillRect(0, 0, width, height, app_.palette().brush(ColorStateToColorGroup(state), QPalette::Window));
    }
    return image;
}
...
qt::QtInterface* CreateQtInterface(qt::QtInterface::Delegate* delegate, int* argc, char** argv)
{
