...
int Qt5WeightToCssWeight(int weight)
{
    ...
    for (size_t i = 0; i < std::size(kMapping) - 1; i++) {
        ...
        if (weight <= hi.qt_weight) {
            return (weight - lo.qt_weight) * (hi.css_weight - lo.css_weight) / (hi.qt_weight - lo.qt_weight) + lo.css_weight;
        }
    }
    NOTREACHED();
}

gfx::FontRenderParams::Hinting QtHintingToGfxHinting(qt::FontHinting hinting, gfx::FontRenderParams::Hinting default_hinting)
{
    switch (hinting) {
...
public:
    explicit QtNativeTheme(QtInterface* shim)
        : ui::NativeThemeAura(/*use_overlay_scrollbars=*/false,
            /*should_only_use_dark_colors=*/false, ui::SystemTheme::kQt)
        , shim_(shim)
    {
    }
    ...
    void PaintFrameTopArea(
        cc::PaintCanvas* canvas, State state, const gfx::Rect& rect, const FrameTopAreaExtraParams& frame_top_area, ColorScheme color_scheme) const override
    {
        auto image = shim_->DrawHeader(rect.width(), rect.height(), frame_top_area.default_background_color,
            frame_top_area.is_active ? ColorState::kNormal : ColorState::kInactive, frame_top_area.use_custom_frame);
        SkImageInfo image_info = SkImageInfo::Make(image.width, image.height, kBGRA_8888_SkColorType, kPremul_SkAlphaType);
        SkBitmap bitmap;
        bitmap.installPixels(
            image_info, image.data_argb.Take(), image_info.minRowBytes(), [](void* data, void*) { free(data); }, nullptr);
        bitmap.setImmutable();
        canvas->drawImage(cc::PaintImage::CreateFromBitmap(std::move(bitmap)), rect.x(), rect.y());
    }
...
std::unique_ptr<ui::LinuxInputMethodContext> QtUi::CreateInputMethodContext(ui::LinuxInputMethodContextDelegate* delegate) const
{
    return fallback_linux_ui_ ? fallback_linux_ui_->CreateInputMethodContext(delegate) : nullptr;
}
...
ui::SelectFileDialog* QtUi::CreateSelectFileDialog(void* listener, std::unique_ptr<ui::SelectFilePolicy> policy) const
{
    return fallback_linux_ui_ ? fallback_linux_ui_->CreateSelectFileDialog(listener, std::move(policy)) : nullptr;
}
...
bool QtUi::Initialize()
{
    ...
    PreferQt6() ? load_libqt_shim(6) || load_libqt_shim(5) : load_libqt_shim(5) || load_libqt_shim(6);
    if (!libqt_shim) {
        return false;
    }
    ...
    shim_.reset((reinterpret_cast<decltype(&CreateQtInterface)>(create_qt_interface)(this, &cmd_line_.argc, cmd_line_.argv.data())));
    native_theme_ = std::make_unique<QtNativeTheme>(shim_.get());
    ui::ColorProviderManager::Get().AppendColorProviderInitializer(base::BindRepeating(&QtUi::AddNativeColorMixer, base::Unretained(this)));
...
void QtUi::InitializeFontSettings()
{
    ...
    int weight = qt_version_ == 5 ? Qt5WeightToCssWeight(desc.weight) : desc.weight;
...
DISABLE_CFI_VCALL
gfx::Image QtUi::GetIconForContentType(const std::string& content_type, int size, float scale) const
{
    Image image = shim_->GetIconForContentType(String(content_type.c_str()), size * scale);
    ...
    bitmap.installPixels(
        image_info, image.data_argb.Take(), image_info.minRowBytes(), [](void* data, void*) { free(data); }, nullptr);
    gfx::ImageSkia image_skia = gfx::ImageSkia::CreateFromBitmap(bitmap, image.scale);
    image_skia.MakeThreadSafe();
    return gfx::Image(image_skia);
}

QtUi::WindowFrameAction QtUi::GetWindowFrameAction(WindowFrameActionSource source)
{
...
{
    return color_utils::IsDark(shim_->GetColor(ColorType::kWindowBg, ColorState::kNormal));
}
...
void QtUi::AddWindowButtonOrderObserver(ui::WindowButtonOrderObserver* observer)
{
...
void QtUi::RemoveWindowButtonOrderObserver(ui::WindowButtonOrderObserver* observer)
{
    if (fallback_linux_ui_) {
...
ui::WindowFrameProvider* QtUi::GetWindowFrameProvider(bool solid_frame, bool tiled)
{
    // QT prefers server-side decorations.
    return nullptr;
}
...
{
    return fallback_linux_ui_ ? fallback_linux_ui_->GetKeyboardLayoutMap() : base::flat_map<std::string, std::string> {};
}
...
bool QtUi::GetTextEditCommandsForEvent(const ui::Event& event, int text_flags, std::vector<ui::TextEditCommandAuraLinux>* commands)
{
...
printing::PrintDialogLinuxInterface* QtUi::CreatePrintDialog(printing::PrintingContextLinux* context)
{
    return fallback_linux_ui_ ? fallback_linux_ui_->CreatePrintDialog(context) : nullptr;
}

gfx::Size QtUi::GetPdfPaperSize(printing::PrintingContextLinux* context)
{
    return fallback_linux_ui_ ? fallback_linux_ui_->GetPdfPaperSize(context) : gfx::Size();
}
#endif
...
void QtUi::ScaleFactorMaybeChanged()
{
    ...
    if (!scale_factor_task_active_) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&QtUi::ScaleFactorMaybeChangedImpl, weak_factory_.GetWeakPtr()));
    }
}

DISABLE_CFI_VCALL
void QtUi::AddNativeColorMixer(ui::ColorProvider* provider, const ui::ColorProviderKey& key)
{
    ...
    struct {
        ...
        // Core colors
        { ui::kColorDisabledForeground, ColorType::kWindowFg, ColorState::kDisabled },
        ...
        { ui::kColorSecondaryForeground, ColorType::kWindowFg, ColorState::kDisabled },
        ...
        { ui::kColorNativeHeaderButtonBorderInactive, ColorType::kMidground, ColorState::kInactive },
        { ui::kColorNativeHeaderSeparatorBorderActive, ColorType::kMidground },
        { ui::kColorNativeHeaderSeparatorBorderInactive, ColorType::kMidground, ColorState::kInactive },
        { ui::kColorNativeLabelForeground, ColorType::kWindowFg },
        { ui::kColorNativeTextfieldBorderUnfocused, ColorType::kMidground, ColorState::kInactive },
    ...
    const SkColor accent = accent_color_.value_or(shim_->GetColor(ColorType::kHighlightBg, ColorState::kNormal));
    for (ui::ColorId accent_id : kAccentIds) {
        mixer[accent_id] = { accent };
    }

    const bool use_custom_frame = key.frame_type == ui::ColorProviderKey::FrameType::kChromium;
    mixer[ui::kColorFrameActive] = { shim_->GetFrameColor(ColorState::kNormal, use_custom_frame) };
    mixer[ui::kColorFrameInactive] = { shim_->GetFrameColor(ColorState::kInactive, use_custom_frame) };

    const SkColor button_fg = shim_->GetColor(ColorType::kButtonFg, ColorState::kNormal);
    mixer[ui::kColorNativeTabForegroundInactiveFrameActive] = ui::BlendForMinContrast({ button_fg }, { ui::kColorFrameActive });
    mixer[ui::kColorNativeTabForegroundInactiveFrameInactive] = ui::BlendForMinContrast({ button_fg }, { ui::kColorFrameInactive });
}
...
std::optional<SkColor> QtUi::GetColor(int id, bool use_custom_frame) const
{
    ...
    case ThemeProperties::COLOR_TAB_FOREGROUND_INACTIVE_FRAME_ACTIVE:
        return color_utils::BlendForMinContrast(
            shim_->GetColor(ColorType::kButtonBg, ColorState::kNormal), shim_->GetFrameColor(ColorState::kNormal, use_custom_frame))
            .color;
    case ThemeProperties::COLOR_TAB_FOREGROUND_INACTIVE_FRAME_INACTIVE:
        return color_utils::BlendForMinContrast(
            shim_->GetColor(ColorType::kButtonBg, ColorState::kInactive), shim_->GetFrameColor(ColorState::kInactive, use_custom_frame))
            .color;
    case ThemeProperties::COLOR_TAB_STROKE_FRAME_ACTIVE:
        return color_utils::BlendForMinContrast(
            shim_->GetColor(ColorType::kButtonBg, ColorState::kNormal), shim_->GetColor(ColorType::kButtonBg, ColorState::kNormal), SK_ColorBLACK, 2.0)
            .color;
    case ThemeProperties::COLOR_TAB_STROKE_FRAME_INACTIVE:
        return color_utils::BlendForMinContrast(
            shim_->GetColor(ColorType::kButtonBg, ColorState::kInactive), shim_->GetColor(ColorType::kButtonBg, ColorState::kInactive), SK_ColorBLACK, 2.0)
            .color;
    default:
        return std::nullopt;
    }
...
void QtUi::ScaleFactorMaybeChangedImpl()
{
    ...
    size_t n_monitors = shim_->GetMonitorConfig(&qt_monitors, &new_config.primary_scale);
    ...
    for (size_t i = 0; i < n_monitors; i++) {
        ...
        ui_monitors.push_back(display::DisplayGeometry { { monitor.x_px, monitor.y_px, monitor.width_px, monitor.height_px }, monitor.scale });
    }
