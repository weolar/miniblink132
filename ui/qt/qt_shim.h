...
public:
    ...
    // QtInterface:
    size_t GetMonitorConfig(MonitorScale** monitors, float* primary_scale) override;
    FontRenderParams GetFontRenderParams() const override;
    FontDescription GetFontDescription() const override;
    Image GetIconForContentType(const String& content_type, int size) const override;
    ...
    Image DrawHeader(int width, int height, SkColor default_color, ColorState state, bool use_custom_frame) const override;
...
private:
    QImage DrawHeaderImpl(int width, int height, SkColor default_color, ColorState state, bool use_custom_frame) const;
