...
void ContentRendererClient::RenderFrameCreated(content::RenderFrame* render_frame)
{
    if (!render_frame->IsMainFrame()) {
        return;
    }

    std::unique_ptr<RenderFrameObserver> observer = std::make_unique<RenderFrameObserver>(render_frame);
...
{
    blink::WebCustomElement::AddEmbedderCustomElementName(blink::WebString("webview"));
}
