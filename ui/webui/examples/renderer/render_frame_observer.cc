...
public:
    ...
    bool IsRenderFrameAvailable()
    {
        return render_frame() != nullptr;
    }
...
void AllowCustomElementNameRegistration(const v8::FunctionCallbackInfo<v8::Value>& args)
{
...
void RegisterWebView(const blink::LocalFrameToken& frame_token, const v8::FunctionCallbackInfo<v8::Value>& args)
{
    CHECK(args.Length() == 1);
    ...
    guest_view->ViewCreated(view_instance_id, "BrowserWebView", std::move(receiver));
...
content::RenderFrame* GetRenderFrame(v8::Local<v8::Value> value)
{
    ...
    if (!v8::Local<v8::Object>::Cast(value)->GetCreationContext().ToLocal(&context)) {
        if (context.IsEmpty()) {
            return nullptr;
        }
    }
...
void AttachIframeGuest(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    std::unique_ptr<base::Value> attach_params = content::V8ValueConverter::Create()->FromV8Value(args[2], context);
    CHECK(attach_params);
    ...
    content::RenderFrame* embedder_parent_frame = content::RenderFrame::FromWebFrame(parent_frame->ToWebLocalFrame());

    auto* guest_view_container = guest_view::GuestViewContainer::FromID(container_id);
    ...
    guest_view_container = new guest_view::GuestViewContainer(embedder_parent_frame, container_id);

    std::unique_ptr<guest_view::GuestViewAttachRequest> request = std::make_unique<guest_view::GuestViewAttachRequest>(
        guest_view_container, render_frame, guest_instance_id, std::move(*attach_params).TakeDict(), v8::Local<v8::Function>(), args.GetIsolate());
...
public:
    using V8Callback = base::RepeatingCallback<void(const v8::FunctionCallbackInfo<v8::Value>&)>;

    explicit V8BinderContext(content::RenderFrame* render_frame)
        : isolate_(render_frame->GetWebFrame()->GetAgentGroupScheduler()->Isolate())
    ...
    void CreateWebshellObject()
    {
        ...
        context_->Global()->CreateDataProperty(context_, CreateV8String("webshell"), object_).FromJust();
    }
    ...
    void AddCallbackToWebshellObject(const char* name, V8Callback callback)
    {
        ...
        std::unique_ptr<V8Callback> callback_container = std::make_unique<V8Callback>(std::move(callback));
        SetPrivateData(callback_holder, kCallback, v8::External::New(isolate_, callback_container.get()));
        global_callback_holder.SetWeak(callback_container.release(), CleanupV8Callback, v8::WeakCallbackType::kParameter);
        object_->CreateDataProperty(context_, CreateV8String(name), v8::Function::New(context_, CallCallback, callback_holder).ToLocalChecked()).FromJust();
    }

private:
    static constexpr char kCallback[] = "callback";

    void SetPrivateData(v8::Local<v8::Object> object, const char* key, v8::Local<v8::Value> value)
    {
        object->SetPrivate(context_, v8::Private::ForApi(isolate_, CreateV8String(key)), value);
    }

    static bool GetPrivateData(v8::Local<v8::Context> context, v8::Local<v8::Object> object, const char* key, v8::Local<v8::Value>* value)
    {
        v8::Isolate* isolate = context->GetIsolate();
        return object->GetPrivate(context, v8::Private::ForApi(isolate, CreateV8String(isolate, key))).ToLocal(value);
    }
    ...
    // static
    static v8::Local<v8::String> CreateV8String(v8::Isolate* isolate, const char* str)
    {
    ...
    static void CallCallback(const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        ...
        if (!GetPrivateData(context, callback_holder, kCallback, &callback_function_value)) {
            return;
        }

        CHECK(callback_function_value->IsExternal());
        V8Callback* callback = static_cast<V8Callback*>(callback_function_value.As<v8::External>()->Value());
        callback->Run(args);
    }
...
void RenderFrameObserver::SelfOwn(std::unique_ptr<RenderFrameObserver> this_instance)
{
...
void RenderFrameObserver::DidStartNavigation(const GURL& url, std::optional<blink::WebNavigationType> navigation_type)
{
...
void RenderFrameObserver::ReadyToCommitNavigation(blink::WebDocumentLoader* document_loader)
{
    V8BinderContext binder_context(render_frame());
    binder_context.CreateWebshellObject();
    binder_context.AddCallbackToWebshellObject("allowWebviewElementRegistration", base::BindRepeating(&AllowCustomElementNameRegistration));
    binder_context.AddCallbackToWebshellObject("getNextId", base::BindRepeating(&GetNextId));
    binder_context.AddCallbackToWebshellObject("registerWebView", base::BindRepeating(&RegisterWebView, render_frame()->GetWebFrame()->GetLocalFrameToken()));
    binder_context.AddCallbackToWebshellObject("attachIframeGuest", base::BindRepeating(&AttachIframeGuest));
}
