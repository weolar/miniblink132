...
private:
    ...
    void SetDisplay(const display::Display& display, wm::NativeCursorManagerDelegate* delegate) override
    {
    ...
    void SetCursor(gfx::NativeCursor cursor, wm::NativeCursorManagerDelegate* delegate) override
    {
    ...
    void SetVisibility(bool visible, wm::NativeCursorManagerDelegate* delegate) override
    {
    ...
    void SetCursorSize(ui::CursorSize cursor_size, wm::NativeCursorManagerDelegate* delegate) override
    {
        NOTIMPLEMENTED();
    }

    void SetMouseEventsEnabled(bool enabled, wm::NativeCursorManagerDelegate* delegate) override
    {
...
AuraContext::ContextualizedWindowTreeHost::ContextualizedWindowTreeHost(
    base::PassKey<AuraContext>, AuraContext* context, std::unique_ptr<aura::WindowTreeHost> window_tree_host)
    : context_(context)
    ...
    : screen_(aura::TestScreen::Create(gfx::Size(1024, 768)))
{
    ...
    cursor_manager_ = std::make_unique<wm::CursorManager>(std::move(native_cursor_manager));
}

AuraContext::~AuraContext() = default;

std::unique_ptr<AuraContext::ContextualizedWindowTreeHost> AuraContext::CreateWindowTreeHost()
{
    ui::PlatformWindowInitProperties properties;
    properties.bounds = gfx::Rect(gfx::Size(1024, 768));
    auto host = aura::WindowTreeHost::Create(std::move(properties));
    return std::make_unique<ContextualizedWindowTreeHost>(base::PassKey<AuraContext>(), this, std::move(host));
}
