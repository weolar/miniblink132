...
ContentWindow::ContentWindow(AuraContext* aura_context, std::unique_ptr<content::WebContents> web_contents)
    : web_contents_(std::move(web_contents))
{
    ...
    window_tree_host->window()->GetRootWindow()->AddPreTargetHandler(root_window_event_filter_.get());
}
...
void ContentWindow::SetTitle(const std::u16string& title)
{
    ...
    aura::WindowTreeHostPlatform::GetHostForWindow(window_tree_host->window())->platform_window()->SetTitle(title);
}
