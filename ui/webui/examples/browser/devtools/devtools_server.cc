...
private:
    ...
    std::unique_ptr<net::ServerSocket> CreateForHttpServer() override
    {
        ...
        std::unique_ptr<net::ServerSocket> socket = std::make_unique<net::TCPServerSocket>(nullptr, net::NetLogSource());
...
{
    content::DevToolsAgentHost::StartRemoteDebuggingServer(TCPServerSocketFactory::Create(), browser_context->GetPath(), base::FilePath());

    const base::CommandLine& command_line = *base::CommandLine::ForCurrentProcess();
    if (command_line.HasSwitch(switches::kRemoteDebuggingPipe)) {
        content::DevToolsAgentHost::StartRemoteDebuggingPipeHandler(base::OnceClosure());
    }
}
