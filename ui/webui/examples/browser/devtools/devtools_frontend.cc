...
{
    return GURL(base::StringPrintf("http://127.0.0.1:%d/devtools/devtools_app.html", devtools::GetHttpHandlerPort()));
}
...
class DevToolsFrontend::AgentHostClient : public content::WebContentsObserver, public content::DevToolsAgentHostClient {
public:
    AgentHostClient(content::WebContents* devtools_contents, content::WebContents* inspected_contents)
    ...
    {
        std::string_view str_message(reinterpret_cast<const char*>(message.data()), message.size());
        if (str_message.length() < kMaxMessageChunkSize) {
            CallClientFunction("DevToolsAPI", "dispatchMessage", base::Value(std::string(str_message)));
        } else {
            size_t total_size = str_message.length();
            for (size_t pos = 0; pos < str_message.length(); pos += kMaxMessageChunkSize) {
                std::string_view str_message_chunk = str_message.substr(pos, kMaxMessageChunkSize);

                CallClientFunction("DevToolsAPI", "dispatchMessageChunk", base::Value(std::string(str_message_chunk)),
    ...
    void CallClientFunction(const std::string& object_name, const std::string& method_name, base::Value arg1 = {}, base::Value arg2 = {}, base::Value arg3 = {},
        base::OnceCallback<void(base::Value)> cb = base::NullCallback())
    {
        ...
        host->ExecuteJavaScriptMethod(base::ASCIIToUTF16(object_name), base::ASCIIToUTF16(method_name), std::move(arguments), std::move(cb));
    }
...
private:
    ...
    void ReadyToCommitNavigation(content::NavigationHandle* navigation_handle) override
    {
        ...
        if (navigation_handle->IsInPrimaryMainFrame()) {
            frontend_host_ = content::DevToolsFrontendHost::Create(
                frame, base::BindRepeating(&AgentHostClient::HandleMessageFromDevToolsFrontend, base::Unretained(this)));
            return;
        }
        ...
        if (it == extensions_api_.end())
            return;
        std::string script = base::StringPrintf("%s(\"%s\")", it->second.c_str(), base::Uuid::GenerateRandomV4().AsLowercaseString().c_str());
    ...
    void HandleMessageFromDevToolsFrontend(base::Value::Dict message)
    {
        ...
        if (*method == "dispatchProtocolMessage" && params.size() == 1) {
            ...
            if (!agent_host_ || !protocol_message)
                return;
            agent_host_->DispatchProtocolMessage(this, base::as_bytes(base::make_span(*protocol_message)));
            ...
            CallClientFunction("DevToolsAPI", "fileSystemsLoaded", base::Value(base::Value::Type::LIST));
    ...
    {
        CallClientFunction("DevToolsAPI", "embedderMessageAck", base::Value(request_id), std::move(arg));
    }
...
// static
DevToolsFrontend* DevToolsFrontend::CreateAndGet(content::WebContents* inspected_contents)
{
    return DevToolsFrontend::Pointer::Create(inspected_contents);
}

void DevToolsFrontend::SetDevtoolsWebContents(content::WebContents* devtools_contents)
{
    devtools_contents_ = devtools_contents;
    agent_host_client_ = std::make_unique<AgentHostClient>(devtools_contents_, inspected_contents_);
