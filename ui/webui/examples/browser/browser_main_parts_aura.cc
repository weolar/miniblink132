...
private:
    ...
    void CreateAndShowWindowForWebContents(std::unique_ptr<content::WebContents> web_contents, const std::u16string& title) override
    {
        auto content_window = std::make_unique<ContentWindow>(aura_context_.get(), std::move(web_contents));
        ContentWindow* content_window_ptr = content_window.get();
        content_window_ptr->SetTitle(title);
        content_window_ptr->Show();
        content_window_ptr->SetCloseCallback(base::BindOnce(&BrowserMainPartsAura::OnWindowClosed, weak_factory_.GetWeakPtr(), std::move(content_window)));
    }
    ...
    void OnWindowClosed(std::unique_ptr<ContentWindow> content_window)
    {
        ...
        // callback.
        content::GetUIThreadTaskRunner({})->DeleteSoon(FROM_HERE, std::move(content_window));
...
{
    return std::make_unique<BrowserMainPartsAura>(BrowserMainPartsAura::PassKey());
}
