...
void TestLookup(const char* name, KeyboardLayoutEngine* engine)
{
    static const struct {
        ...
        { DomCode::LAUNCH_ASSISTANT, EF_NONE, DomKey::LAUNCH_ASSISTANT, VKEY_ASSISTANT, 0 },
    };

    for (const auto& t : kTestCases) {
        DomKey dom_key;
        KeyboardCode keycode;
        SCOPED_TRACE(base::StringPrintf("%s(%s, 0x%X)", name, KeycodeConverter::DomCodeToCodeString(t.input_dom_code).c_str(), t.input_flags));
        EXPECT_TRUE(engine->Lookup(t.input_dom_code, t.input_flags, &dom_key, &keycode));
        EXPECT_EQ(t.output_dom_key, dom_key);
        EXPECT_EQ(t.output_keycode, keycode);
        KeyEvent key_event(EventType::kKeyPressed, keycode, t.input_dom_code, t.input_flags, dom_key, EventTimeForNow());
...
TEST(LayoutEngineTest, Lookup)
{
...

    {
        ...
        auto xkb_engine = std::make_unique<XkbKeyboardLayoutEngine>(xkb_evdev_code_converter);
        xkb_engine->SetCurrentLayoutFromBuffer(kUsLayoutXkbKeymap, strlen(kUsLayoutXkbKeymap));
