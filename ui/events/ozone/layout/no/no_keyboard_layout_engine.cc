...
void NoKeyboardLayoutEngine::SetCurrentLayoutByName(const std::string& layout_name, base::OnceCallback<void(bool)> callback)
{
    std::move(callback).Run(false);
}

bool NoKeyboardLayoutEngine::SetCurrentLayoutFromBuffer(const char* keymap_str, size_t size)
{
    return false;
}
...
bool NoKeyboardLayoutEngine::Lookup(DomCode dom_code, int flags, DomKey* dom_key, KeyboardCode* key_code) const
{
    return false;
}
