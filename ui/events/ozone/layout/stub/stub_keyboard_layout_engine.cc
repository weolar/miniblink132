...
void StubKeyboardLayoutEngine::SetCurrentLayoutByName(const std::string& layout_name, base::OnceCallback<void(bool)> callback)
{
    std::move(callback).Run(false);
}

bool StubKeyboardLayoutEngine::SetCurrentLayoutFromBuffer(const char* keymap_str, size_t size)
{
    return false;
}
...
bool StubKeyboardLayoutEngine::Lookup(DomCode dom_code, int flags, DomKey* out_dom_key, KeyboardCode* out_key_code) const
{
...
void StubKeyboardLayoutEngine::SetInitCallbackForTest(base::OnceClosure closure)
{
    std::move(closure).Run();
}

void StubKeyboardLayoutEngine::SetCustomLookupTableForTesting(const std::vector<CustomLookupEntry>& table)
{
