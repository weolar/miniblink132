...
public:
    ...
    xkb_keysym_t CharacterToKeySym(char16_t c) const
    {
        return 0x01000000u + c;
    }

    KeyboardCode GetKeyboardCode(DomCode dom_code, int flags, char16_t character) const
    {
        KeyboardCode key_code
            = DifficultKeyboardCode(dom_code, flags, key_code_converter_->DomCodeToXkbKeyCode(dom_code), flags, CharacterToKeySym(character), character);
        if (key_code == VKEY_UNKNOWN) {
            DomKey dummy_dom_key;
            // If this fails, key_code remains VKEY_UNKNOWN.
            std::ignore = DomCodeToUsLayoutDomKey(dom_code, EF_NONE, &dummy_dom_key, &key_code);
        }
        return key_code;
    }

    // XkbKeyboardLayoutEngine overrides:
    bool XkbLookup(xkb_keycode_t xkb_keycode, xkb_mod_mask_t xkb_flags, xkb_keysym_t* xkb_keysym, uint32_t* character) const override
    {
        switch (entry_type_) {
        case EntryType::NONE:
            break;
...
public:
    XkbLayoutEngineVkTest()
        : layout_engine_(std::make_unique<VkTestXkbKeyboardLayoutEngine>(keycode_converter_))
    {
    }
...
TEST_F(XkbLayoutEngineVkTest, KeyboardCodeForPrintable)
{
    ...
    for (size_t i = 0; i < std::size(kVkeyTestCase); ++i) {
        ...
        KeyboardCode key_code = layout_engine_->GetKeyboardCode(e.test.dom_code, EF_NONE, e.test.plain_character);
        EXPECT_EQ(e.key_code, key_code);

        if (e.test.shift_character) {
            // Test with predetermined shifted character.
            key_code = layout_engine_->GetKeyboardCode(e.test.dom_code, EF_SHIFT_DOWN, e.test.shift_character);
            EXPECT_EQ(e.key_code, key_code);
        }

        if (e.test.altgr_character) {
            // Test with predetermined AltGr character.
            key_code = layout_engine_->GetKeyboardCode(e.test.dom_code, EF_ALTGR_DOWN, e.test.altgr_character);
            EXPECT_EQ(e.key_code, key_code);
        }
...
TEST_F(XkbLayoutEngineVkTest, KeyboardCodeForNonPrintable)
{
    ...
    for (const auto& e : kVkeyTestCase) {
        ...
        EXPECT_TRUE(layout_engine_->Lookup(e.test.dom_code, e.test.flags, &dom_key, &key_code));
        EXPECT_EQ(e.key_code, key_code);
    }
}
...
{
    static const VkTestXkbKeyboardLayoutEngine::RuleNames kVkeyTestCase[] = { /* 0 */ { "us", "us", "" },
        /* 1 */ { "jp", "jp", "" },
        /* 2 */ { "us(intl)", "us", "intl" },
    ...
    for (size_t i = 0; i < std::size(kVkeyTestCase); ++i) {
        ...
        XkbKeyboardLayoutEngine::ParseLayoutName(e->layout_name, &layout_id, &layout_variant);
...
TEST_F(XkbLayoutEngineVkTest, GetDomCodeByKeysym)
{
    // Set up US keyboard layout.
    {
        ...
        std::unique_ptr<xkb_keymap, ui::XkbKeymapDeleter> xkb_keymap(xkb_keymap_new_from_names(xkb_context.get(), &names, XKB_KEYMAP_COMPILE_NO_FLAGS));
        std::unique_ptr<char, base::FreeDeleter> layout(xkb_keymap_get_as_string(xkb_keymap.get(), XKB_KEYMAP_FORMAT_TEXT_V1));
        layout_engine_->SetCurrentLayoutFromBuffer(layout.get(), std::strlen(layout.get()));
    }
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_EQ(test_case.expected_dom_code, layout_engine_->GetDomCodeByKeysym(test_case.keysym, modifiers))
