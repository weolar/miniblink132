...
class COMPONENT_EXPORT(EVENTS_OZONE_LAYOUT) XkbKeyboardLayoutEngine : public KeyboardLayoutEngine {
public:
    ...
    bool SetCurrentLayoutFromBuffer(const char* keymap_string, size_t size) override;

    bool UsesISOLevel5Shift() const override;
    bool UsesAltGr() const override;

    bool Lookup(DomCode dom_code, int flags, DomKey* dom_key, KeyboardCode* key_code) const override;

    void SetInitCallbackForTest(base::OnceClosure closure) override;

    int UpdateModifiers(uint32_t depressed, uint32_t latched, uint32_t locked, uint32_t group);
    ...
    DomCode GetDomCodeByKeysym(uint32_t keysym, const std::optional<std::vector<std::string_view>>& modifiers) const;

    static void ParseLayoutName(const std::string& layout_name, std::string* layout_id, std::string* layout_variant);
...
private:
    ...
    virtual bool XkbLookup(xkb_keycode_t xkb_keycode, xkb_mod_mask_t xkb_flags, xkb_keysym_t* xkb_keysym, uint32_t* character) const;
    ...
    char16_t XkbSubCharacter(xkb_keycode_t xkb_keycode, xkb_mod_mask_t base_flags, char16_t base_character, xkb_mod_mask_t flags) const;

    // Callback when keymap file is loaded complete.
    void OnKeymapLoaded(base::OnceCallback<void(bool)> callback, const std::string& layout_name, std::unique_ptr<char, base::FreeDeleter> keymap_str);
