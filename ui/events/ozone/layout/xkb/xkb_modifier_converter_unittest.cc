...
TEST(XkbModifierConverterTest, Basic)
{
    ...
    // Conversion from ui::EventFlags to xkb_mod_mask_t.
    {
        ...
        EXPECT_EQ(3u, converter.MaskFromUiFlags(ui::EF_SHIFT_DOWN | ui::EF_CAPS_LOCK_ON));

        // Unknown ui::EventFlags are just ignored.
        EXPECT_EQ(0u, converter.MaskFromUiFlags(ui::EF_LEFT_MOUSE_BUTTON));
        EXPECT_EQ(1u, converter.MaskFromUiFlags(ui::EF_SHIFT_DOWN | ui::EF_LEFT_MOUSE_BUTTON));
    }
    ...
    // Conversion from xkb_mod_mask_t to ui::EventFlags.
    {
        ...
        EXPECT_EQ(ui::EF_SHIFT_DOWN | ui::EF_CAPS_LOCK_ON, converter.UiFlagsFromMask(3));
    ...
    EXPECT_EQ(3u, converter.MaskFromNames({ XKB_MOD_NAME_SHIFT, XKB_MOD_NAME_CAPS }));
...
TEST(XkbModifierConverterTest, FromKeymap)
{
    ...
    XkbModifierConverter converter = XkbModifierConverter::CreateFromKeymap(xkb_keymap.get());
