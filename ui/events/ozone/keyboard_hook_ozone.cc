...
public:
    explicit KeyboardHookOzone(std::unique_ptr<PlatformKeyboardHook> platform_hook)
...
std::unique_ptr<KeyboardHook> KeyboardHook::CreateModifierKeyboardHook(
    std::optional<base::flat_set<DomCode>> dom_codes, gfx::AcceleratedWidget accelerated_widget, KeyEventCallback callback)
{
    if (auto platform_hook = OzonePlatform::GetInstance()->CreateKeyboardHook(
            PlatformKeyboardHookTypes::kModifier, std::move(callback), std::move(dom_codes), accelerated_widget)) {
