...
void TransformCursorMove(display::Display::Rotation rotation, float scale, gfx::Vector2dF* delta)
{
    float dx;
    float dy;
...
void CursorController::AddCursorObserver(CursorObserver* observer)
{
    ...
    CHECK(!base::Contains(cursor_observers_, observer), base::NotFatalUntil::M126);
...
void CursorController::SetCursorConfigForWindow(gfx::AcceleratedWidget widget, display::Display::Rotation rotation, float scale)
{
...
void CursorController::ClearCursorConfigForWindow(gfx::AcceleratedWidget widget)
{
    base::AutoLock lock(window_to_cursor_configuration_map_lock_);
    window_to_cursor_configuration_map_.erase(widget);
}

void CursorController::ApplyCursorConfigForWindow(gfx::AcceleratedWidget widget, gfx::Vector2dF* delta) const
{
    base::AutoLock lock(window_to_cursor_configuration_map_lock_);
