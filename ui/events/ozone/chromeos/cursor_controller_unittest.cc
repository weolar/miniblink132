...
public:
    ...
    {
        ui::CursorController::GetInstance()->ClearCursorConfigForWindow(kTestWindow);
    }
};
...
TEST_F(CursorControllerTest, ClearedIdentity)
{
    ...
    cursor_controller->SetCursorConfigForWindow(kTestWindow, display::Display::ROTATE_180, 3.2f);
...
TEST_F(CursorControllerTest, RotatedHighDpi)
{
    ...
    cursor_controller->SetCursorConfigForWindow(kTestWindow, display::Display::ROTATE_90, 2.f);
    gfx::Vector2dF delta(3.f, 5.f);
    cursor_controller->ApplyCursorConfigForWindow(kTestWindow, &delta);
