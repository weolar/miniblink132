...
public:
    ...
    void SetCursorConfigForWindow(gfx::AcceleratedWidget widget, display::Display::Rotation rotation, float scale);
    ...
    void ApplyCursorConfigForWindow(gfx::AcceleratedWidget widget, gfx::Vector2dF* delta) const;
...
private:
    ...
    typedef std::map<gfx::AcceleratedWidget, PerWindowCursorConfiguration> WindowToCursorConfigurationMap;

    mutable base::Lock window_to_cursor_configuration_map_lock_;
    WindowToCursorConfigurationMap window_to_cursor_configuration_map_ GUARDED_BY(window_to_cursor_configuration_map_lock_);

    mutable base::Lock cursor_observers_lock_;
    std::vector<CursorObserver*> cursor_observers_ GUARDED_BY(cursor_observers_lock_);
};
