...
void EventAutoRepeatHandler::SetAutoRepeatRate(const base::TimeDelta& delay, const base::TimeDelta& interval)
{
    repeat_delay_ = delay;
    repeat_interval_ = interval;
}

void EventAutoRepeatHandler::GetAutoRepeatRate(base::TimeDelta* delay, base::TimeDelta* interval)
{
    *delay = repeat_delay_;
    *interval = repeat_interval_;
}

void EventAutoRepeatHandler::UpdateKeyRepeat(
    unsigned int key, unsigned int scan_code, bool down, bool suppress_auto_repeat, int device_id, base::TimeTicks base_timestamp)
{
...
void EventAutoRepeatHandler::StartKeyRepeat(unsigned int key, unsigned int scan_code, int device_id, base::TimeTicks base_timestamp)
{
    repeat_key_ = key;
    repeat_scan_code_ = scan_code;
    repeat_device_id_ = device_id;
    rebase_timestamp_ = { .base_timestamp = base_timestamp, .start_timestamp = EventTimeForNow() };
    repeat_sequence_++;
...
void EventAutoRepeatHandler::ScheduleKeyRepeat(const base::TimeDelta& delay)
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&EventAutoRepeatHandler::OnRepeatTimeout, weak_ptr_factory_.GetWeakPtr(), repeat_sequence_), delay);
}
...
void EventAutoRepeatHandler::OnRepeatTimeout(unsigned int sequence)
{
    ...
    base::OnceClosure commit = base::BindOnce(&EventAutoRepeatHandler::OnRepeatCommit, weak_ptr_factory_.GetWeakPtr(), repeat_sequence_);
...
void EventAutoRepeatHandler::OnRepeatCommit(unsigned int sequence)
{
    ...
    delegate_->DispatchKey(repeat_key_, repeat_scan_code_, /*down =*/true,
        /*repeat=*/true, timestamp, repeat_device_id_, ui::EF_NONE);
