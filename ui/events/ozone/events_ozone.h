...
EVENTS_EXPORT EventResult DispatchEventFromNativeUiEvent(const PlatformEvent& native_event, base::OnceCallback<void(ui::Event*)> callback);
...
constexpr uint8_t kPropertyKeyboardImeHandledFlag = 1 << (24 - kPropertyKeyboardImeFlagOffset);
// Ignored is the 25-th bit.
constexpr uint8_t kPropertyKeyboardImeIgnoredFlag = 1 << (25 - kPropertyKeyboardImeFlagOffset);

// Sets the `flags` as keyboard ime flag property to given `properties`.
EVENTS_EXPORT void SetKeyboardImeFlagProperty(KeyEvent::Properties* properties, uint8_t flags);
