...
TEST(InputControllerEvdevTest, AccelerationSuspension)
{
    ...
    EXPECT_TRUE(controller.GetInputDeviceSettings().GetMouseSettings().acceleration_enabled);
    EXPECT_TRUE(controller.GetInputDeviceSettings().GetPointingStickSettings().acceleration_enabled);
