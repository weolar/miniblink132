...
class TestMicrophoneMuteObserver : public ui::MicrophoneMuteSwitchMonitor::Observer {
public:
    ...
    TestMicrophoneMuteObserver& operator=(const TestMicrophoneMuteObserver&) = delete;
...
public:
    ...
    void SetUp() override
    {
        ...
        event_factory_ = ui::CreateEventFactoryEvdevForTest(nullptr, device_manager_.get(), keyboard_layout_engine_.get(),
            base::BindRepeating([](ui::Event* event) { ADD_FAILURE() << "Unexpected event dispatch " << event->ToString(); }));
    ...
    std::unique_ptr<ui::MicrophoneMuteSwitchEventConverterEvdev> CreateDevice(const ui::DeviceCapabilities& caps)
    {
        int evdev_io[2];
        if (pipe(evdev_io))
        ...
        return std::make_unique<ui::MicrophoneMuteSwitchEventConverterEvdev>(
            std::move(events_in), base::FilePath(kTestDevicePath), 1, devinfo, dispatcher_.get());
    }
...
TEST_F(MicrophoneMuteSwitchEventConverterEvdevTest, MuteChangeEvents)
{
    ...
    std::unique_ptr<ui::MicrophoneMuteSwitchEventConverterEvdev> dev = CreateDevice(ui::kPuffMicrophoneMuteSwitch);

    EXPECT_FALSE(ui::MicrophoneMuteSwitchMonitor::Get()->microphone_mute_switch_on());
    ...
    EXPECT_TRUE(ui::MicrophoneMuteSwitchMonitor::Get()->microphone_mute_switch_on());
    ...
    EXPECT_EQ(std::vector<bool> { false }, test_observer.GetAndResetObservedValues());
}
