...
void TouchEventLogEvdev::DumpLog(const char* filename)
{
    ...
    for (size_t i = 0; i < axes_.size(); ++i) {
        std::string absinfo = base::StringPrintf("# absinfo: %d %d %d %d %d %d\n", axes_[i].code, axes_[i].info.maximum, axes_[i].info.maximum,
            axes_[i].info.fuzz, axes_[i].info.flat, axes_[i].info.resolution);
    ...
    for (int i = 0; i < kDebugBufferSize; ++i) {
        ...
        std::string event_string = base::StringPrintf(
            "E: %ld.%06ld %04x %04x %d %d\n", te->ev.input_event_sec, te->ev.input_event_usec, te->ev.type, te->ev.code, te->ev.value, te->slot);
        report_content += event_string;
    }
    file.Write(0, base::as_byte_span(report_content));
}

TouchEventLogEvdev::AbsAxisData::AbsAxisData(int code, const input_absinfo& info)
    : code(code)
    , info(info)
{
}
