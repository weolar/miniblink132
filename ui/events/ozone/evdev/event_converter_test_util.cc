...
public:
    ...
    {
        event_factory_evdev_->DispatchKeyboardDevicesUpdated(devices, key_bits_mapping);
    }
    ...
    {
        event_factory_evdev_->DispatchTouchpadDevicesUpdated(devices, has_haptic_touchpad);
    }
    ...
    {
        event_factory_evdev_->DispatchGamepadDevicesUpdated(devices, key_bits_mapping);
    }
...
public:
    TestEventFactoryEvdev(
        CursorDelegateEvdev* cursor, DeviceManager* device_manager, KeyboardLayoutEngine* keyboard_layout_engine, const EventDispatchCallback& callback)
...
std::unique_ptr<DeviceEventDispatcherEvdev> CreateDeviceEventDispatcherEvdevForTest(EventFactoryEvdev* event_factory)
{
...
std::unique_ptr<EventFactoryEvdev> CreateEventFactoryEvdevForTest(
    CursorDelegateEvdev* cursor, DeviceManager* device_manager, KeyboardLayoutEngine* keyboard_layout_engine, const EventDispatchCallback& callback)
{
    return std::make_unique<TestEventFactoryEvdev>(cursor, device_manager, keyboard_layout_engine, callback);
}
