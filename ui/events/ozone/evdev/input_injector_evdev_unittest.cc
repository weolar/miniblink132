...
public:
    ...
    {
        DispatchEventFromNativeUiEvent(event, base::BindOnce(&EventObserver::OnEvent, base::Unretained(this)));
    }
...
{
    if (arg->type() != type) {
        *result_listener << "Expected type: " << base::to_underlying(type) << " actual: " << base::to_underlying(arg->type()) << " (" << arg->GetName() << ")";
        return false;
    }
    ...
    if (arg->x() != x || arg->y() != y) {
        *result_listener << "Expected location: (" << x << ", " << y << ") actual: (" << arg->x() << ", " << arg->y() << ")";
        return false;
    }
    return true;
}
...
InputInjectorEvdevTest::InputInjectorEvdevTest()
    : dispatch_callback_(base::BindRepeating(&EventObserver::EventDispatchCallback, base::Unretained(&event_observer_)))
    , device_manager_(CreateDeviceManagerForTest())
    , keyboard_layout_engine_(std::make_unique<ui::StubKeyboardLayoutEngine>())
    , event_factory_(CreateEventFactoryEvdevForTest(&cursor_, device_manager_.get(), keyboard_layout_engine_.get(), dispatch_callback_))
    , injector_(CreateDeviceEventDispatcherEvdevForTest(event_factory_.get()), &cursor_)
{
}

void InputInjectorEvdevTest::SimulateMouseClick(int x, int y, EventFlags button, int count)
{
...
{
    InSequence dummy;
    EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(EventType::kMouseMoved, EF_NONE, x, y)));

    for (int i = 0; i < count; i++) {
        EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(EventType::kMousePressed, button, x, y)));
        EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(EventType::kMouseReleased, button, x, y)));
    }
}
...
{
    InSequence dummy;
    EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(EventType::kMousePressed, EF_LEFT_MOUSE_BUTTON, 0, 0)));
    EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(EventType::kMouseDragged, EF_LEFT_MOUSE_BUTTON, 1, 1)));
    EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(EventType::kMouseDragged, EF_LEFT_MOUSE_BUTTON, 2, 3)));
    EXPECT_CALL(event_observer_, OnMouseEvent(MatchesMouseEvent(EventType::kMouseReleased, EF_LEFT_MOUSE_BUTTON, 2, 3)));
...
TEST_F(InputInjectorEvdevTest, MouseWheel)
{
    ...
    EXPECT_CALL(event_observer_,
        OnMouseWheelEvent(
            AllOf(MatchesMouseEvent(EventType::kMousewheel, 0, 10, 20), Property(&MouseWheelEvent::x_offset, 0), Property(&MouseWheelEvent::y_offset, 100))));
    EXPECT_CALL(event_observer_,
        OnMouseWheelEvent(
            AllOf(MatchesMouseEvent(EventType::kMousewheel, 0, 10, 20), Property(&MouseWheelEvent::x_offset, 100), Property(&MouseWheelEvent::y_offset, 0))));
