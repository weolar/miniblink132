...
struct DeviceId {
    ...
    {
        return vendor == other.vendor ? product_id < other.product_id : vendor < other.vendor;
    }
};
...
bool IsForceLibinput(const EventDeviceInfo& devinfo)
{
    for (auto entry : kForceLibinputlist) {
        if (devinfo.vendor_id() == entry.vendor && devinfo.product_id() == entry.product_id) {
            return true;
        }
    }
...
bool GetEventBits(int fd, const base::FilePath& path, unsigned int type, void* buf, unsigned int size)
{
    if (ioctl(fd, EVIOCGBIT(type, size), buf) < 0) {
        PLOG(ERROR) << "Failed EVIOCGBIT (path=" << path.value() << " type=" << type << " size=" << size << ")";
        return false;
    }
...
bool GetPropBits(int fd, const base::FilePath& path, void* buf, unsigned int size)
{
...
bool GetAbsInfo(int fd, const base::FilePath& path, int code, struct input_absinfo* absinfo)
{
...
void GetSlotValues(int fd, const base::FilePath& path, int32_t* request, unsigned int size)
{
    size_t data_size = size * sizeof(*request);
    if (ioctl(fd, EVIOCGMTSLOTS(data_size), request) < 0) {
        PLOG(ERROR) << "Failed EVIOCGMTSLOTS (code=" << request[0] << " path=" << path.value() << ")";
    }
}

void AssignBitset(const unsigned long* src, size_t src_len, unsigned long* dst, size_t dst_len)
{
...
bool IsDenylistedAbsoluteMouseDevice(const input_id& id)
{
    ...
    for (size_t i = 0; i < std::size(kUSBLegacyDenyListedDevices); ++i) {
        if (id.vendor == kUSBLegacyDenyListedDevices[i].vid && id.product == kUSBLegacyDenyListedDevices[i].pid) {
            return true;
        }
    }
...
void EventDeviceInfo::SetAbsInfo(unsigned int code, const input_absinfo& abs_info)
{
    if (code > ABS_MAX)
        return;
...
void EventDeviceInfo::SetAbsMtSlots(unsigned int code, const std::vector<int32_t>& values)
{
...
void EventDeviceInfo::SetAbsMtSlot(unsigned int code, unsigned int slot, uint32_t value)
{
    int index = code - EVDEV_ABS_MT_FIRST;
    if (index < 0 || index >= EVDEV_ABS_MT_COUNT)
        return;
...
bool EventDeviceInfo::SupportsHeatmap() const
{
    for (const auto& device_id : kHeatmapSupportedDevices) {
        if (input_id_.vendor == device_id.vendor && input_id_.product == device_id.product_id) {
            return true;
        }
    }
    return false;
...
int32_t EventDeviceInfo::GetAbsMtSlotValue(unsigned int code, unsigned int slot) const
{
...
int32_t EventDeviceInfo::GetAbsMtSlotValueWithDefault(unsigned int code, unsigned int slot, int32_t default_value) const
{
    if (!HasAbsEvent(code))
...
bool EventDeviceInfo::UseLibinput() const
{
    ...
    if (HasTouchpad()) {
        auto overridden_state = base::FeatureList::GetStateIfOverridden(ui::kLibinputHandleTouchpad);
        if (overridden_state.has_value()) {
            useLibinput = overridden_state.value();
        } else {
            useLibinput = !HasMultitouch() || !HasValidMTAbsXY() || IsSemiMultitouch() || IsForceLibinput(*this);
        }
    }
...
void RecordBlocklistedKeyboardMetric(input_id input_id_)
{
    ...
    auto [_, inserted] = logged_devices->insert({ input_id_.vendor, input_id_.product });
    if (inserted) {
        base::UmaHistogramEnumeration("ChromeOS.Inputs.ComboDeviceClassification", ComboDeviceClassification::kKnownKeyboardImposter);
    }
}
...
bool EventDeviceInfo::HasKeyboard() const
{
    ...
    return type == KeyboardType::VALID_KEYBOARD || type == KeyboardType::IN_ALLOWLIST;
}
...
bool EventDeviceInfo::HasMouse() const
{
    ...
    if (input_id_.vendor == kSteelSeriesBluetoothVendorId
        && (input_id_.product == kSteelSeriesStratusDuoBluetoothProductId || input_id_.product == kSteelSeriesStratusPlusBluetoothProductId)) {
        return false;
    }
    ...
    if (input_id_.vendor == kFlossVirtualSuspendVendorId && input_id_.product == kFlossVirtualSuspendProductId && name_ == kFlossVirtualSuspendName) {
        return false;
    }
...
{
    return HasSwEvent(SW_PEN_INSERTED) && (device_type_ == INPUT_DEVICE_UNKNOWN || device_type_ == INPUT_DEVICE_INTERNAL);
}
...
bool EventDeviceInfo::HasNumberpad() const
{
    ...
    for (int key : { KEY_KP0, KEY_KP1, KEY_KP2, KEY_KP3, KEY_KP4, KEY_KP5, KEY_KP6, KEY_KP7, KEY_KP8, KEY_KP9 }) {
        if (!HasKeyEvent(key))
            return false;
    }
    return true;
...
ui::InputDeviceType EventDeviceInfo::GetInputDeviceTypeFromId(input_id id)
{
    ...
    case BUS_USB:
        return IsInternalUSB(id) ? ui::InputDeviceType::INPUT_DEVICE_INTERNAL : ui::InputDeviceType::INPUT_DEVICE_USB;
    case BUS_BLUETOOTH:
...
bool EventDeviceInfo::IsInternalUSB(input_id id)
{
    ...
    if (ash::switches::IsRevenBranding()) {
        usb_internal_ids.insert(usb_internal_ids.end(),
            {
...
EventDeviceInfo::LegacyAbsoluteDeviceType EventDeviceInfo::ProbeLegacyAbsoluteDevice() const
{
    if (!HasAbsXY())
    ...
    if (HasKeyEvent(BTN_LEFT) && !HasAbsEvent(ABS_Z) && !IsDenylistedAbsoluteMouseDevice(input_id_))
        return LegacyAbsoluteDeviceType::TOUCHSCREEN;
...
std::ostream& operator<<(std::ostream& os, const EventDeviceType value)
{
    ...
    return os << "ui::EventDeviceType::unknown_value(" << static_cast<unsigned int>(value) << ")";
}
...
std::ostream& operator<<(std::ostream& os, const KeyboardType value)
{
    ...
    return os << "ui::KeyboardType::unknown_value(" << static_cast<unsigned int>(value) << ")";
}
