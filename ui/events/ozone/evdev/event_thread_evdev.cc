...
public:
    EvdevThread(std::unique_ptr<DeviceEventDispatcherEvdev> dispatcher, CursorDelegateEvdev* cursor, EventThreadStartCallback callback,
    ...
    void Init() override
    {
        ...
        input_device_factory_
            = std::make_unique<InputDeviceFactoryEvdev>(std::move(dispatcher_), cursor_, std::make_unique<InputDeviceOpenerEvdev>(), input_controller_);

        auto proxy = std::make_unique<InputDeviceFactoryEvdevProxy>(base::SingleThreadTaskRunner::GetCurrentDefault(), input_device_factory_->GetWeakPtr());

        if (cursor_)
            cursor_->InitializeOnEvdev();

        init_runner_->PostTask(FROM_HERE, base::BindOnce(std::move(init_callback_), std::move(proxy)));
    }
...
void EventThreadEvdev::Start(std::unique_ptr<DeviceEventDispatcherEvdev> dispatcher, CursorDelegateEvdev* cursor, EventThreadStartCallback callback,
    InputControllerEvdev* input_controller)
{
    TRACE_EVENT0("evdev", "EventThreadEvdev::Start");
    thread_ = std::make_unique<EvdevThread>(std::move(dispatcher), cursor, std::move(callback), input_controller);
