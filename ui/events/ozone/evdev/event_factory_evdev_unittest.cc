...
protected:
    EventFactoryEvdevTest()
        : event_factory_(nullptr, &device_manager_, nullptr)
    {
        scoped_feature_list_.InitAndEnableFeature(kEnableOrdinalMotion);
        event_factory_.SetUserInputTaskRunner(base::SingleThreadTaskRunner::GetCurrentDefault());
        event_factory_.Init();
...
{
    EXPECT_CALL(event_observer_, WillProcessEvent).WillOnce([](const PlatformEvent& platform_event) {
    ...
    event_factory_.DispatchMouseMoveEvent(MouseMoveEventParams { 0, 0, {}, &ordinal_movement, PointerDetails(EventPointerType::kMouse), EventTimeForNow() });
}
...
TEST_F(EventFactoryEvdevTest, NoOrdinalImpliesNoFlag)
{
    ...
    event_factory_.DispatchMouseMoveEvent(MouseMoveEventParams { 0, 0, {},
        /*ordinal_movement=*/nullptr, PointerDetails(EventPointerType::kMouse), EventTimeForNow() });
}
...
TEST_F(EventFactoryEvdevTest, MouseSideButtonRemapsToBack)
{
    ...
    event_factory_.DispatchMouseButtonEvent(MouseButtonEventParams {
        0, 0, gfx::PointF {}, BTN_SIDE, true, MouseButtonMapType::kMouse, PointerDetails(EventPointerType::kMouse), EventTimeForNow() });
}
...
TEST_F(EventFactoryEvdevTest, MouseBackButtonRemapsToBack)
{
    ...
    event_factory_.DispatchMouseButtonEvent(MouseButtonEventParams {
        0, 0, gfx::PointF {}, BTN_BACK, true, MouseButtonMapType::kMouse, PointerDetails(EventPointerType::kMouse), EventTimeForNow() });
}
...
TEST_F(EventFactoryEvdevTest, MouseExtraButtonRemapsToForward)
{
    EXPECT_CALL(event_observer_, WillProcessEvent).WillOnce([](const PlatformEvent& platform_event) {
        MouseEvent mouse_event(platform_event);
        EXPECT_TRUE(mouse_event.changed_button_flags() & EF_FORWARD_MOUSE_BUTTON);
        auto button = GetForwardBackMouseButtonProperty(mouse_event);
        ASSERT_TRUE(button);
    ...
    event_factory_.DispatchMouseButtonEvent(MouseButtonEventParams {
        0, 0, gfx::PointF {}, BTN_EXTRA, true, MouseButtonMapType::kMouse, PointerDetails(EventPointerType::kMouse), EventTimeForNow() });
}
...
TEST_F(EventFactoryEvdevTest, MouseForwardButtonRemapsToForward)
{
    ...
    event_factory_.DispatchMouseButtonEvent(MouseButtonEventParams {
        0, 0, gfx::PointF {}, BTN_FORWARD, true, MouseButtonMapType::kMouse, PointerDetails(EventPointerType::kMouse), EventTimeForNow() });
}
