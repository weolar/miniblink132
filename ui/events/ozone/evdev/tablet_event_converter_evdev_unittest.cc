...
public:
    MockTabletEventConverterEvdev(
        base::ScopedFD fd, base::FilePath path, CursorDelegateEvdev* cursor, const EventDeviceInfo& devinfo, DeviceEventDispatcherEvdev* dispatcher);

    MockTabletEventConverterEvdev(const MockTabletEventConverterEvdev&) = delete;
    MockTabletEventConverterEvdev& operator=(const MockTabletEventConverterEvdev&) = delete;
...
public:
    MockTabletCursorEvdev()
    {
        cursor_confined_bounds_ = gfx::Rect(1024, 768);
    }
...
MockTabletEventConverterEvdev::MockTabletEventConverterEvdev(
    base::ScopedFD fd, base::FilePath path, CursorDelegateEvdev* cursor, const EventDeviceInfo& devinfo, DeviceEventDispatcherEvdev* dispatcher)
    : TabletEventConverterEvdev(std::move(fd), path, 1, cursor, devinfo, dispatcher)
{
    int fds[2];
...
void MockTabletEventConverterEvdev::ConfigureReadMock(struct input_event* queue, long read_this_many, long queue_index)
{
...
public:
    TabletEventConverterEvdevTest()
        ...
        , event_factory_(ui::CreateEventFactoryEvdevForTest(cursor_.get(), device_manager_.get(), keyboard_layout_engine_.get(),
              base::BindRepeating(&TabletEventConverterEvdevTest::DispatchEventForTest, base::Unretained(this))))
    ...
    TabletEventConverterEvdevTest& operator=(const TabletEventConverterEvdevTest&) = delete;

    std::unique_ptr<ui::MockTabletEventConverterEvdev> CreateDevice(const ui::EventDeviceInfo& devinfo)
    {
        ...
        return std::make_unique<ui::MockTabletEventConverterEvdev>(
            std::move(events_in), base::FilePath(kTestDevicePath), cursor_.get(), devinfo, dispatcher_.get());
    }

    std::unique_ptr<ui::MockTabletEventConverterEvdev> CreateDevice(const ui::DeviceCapabilities& caps)
    {
    ...
    void CheckEvents(struct ExpectedEvent expected_events[], unsigned num_events)
    {
        ASSERT_EQ(num_events, size());
        for (unsigned i = 0; i < num_events; ++i) {
            ui::MouseEvent* event = dispatched_event(i);
            EXPECT_EQ(event->pointer_details().pointer_type, expected_events[i].pointer_type) << " at event " << i;
            EXPECT_EQ(event->type(), expected_events[i].type) << " at event " << i;
            EXPECT_EQ(event->flags() & ui::EF_MOUSE_BUTTON, expected_events[i].button_flags) << " at event " << i;
        }
    }
...
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(kWacomIntuos5SPen);
...
TEST_F(TabletEventConverterEvdevTest, MoveTopRight)
{
    ...
    EXPECT_GT(cursor()->GetLocation().x(), cursor()->GetCursorConfinedBounds().width() - EPSILON);
    EXPECT_LT(cursor()->GetLocation().y(), EPSILON);
}
...
TEST_F(TabletEventConverterEvdevTest, MoveBottomLeft)
{
    ...
    EXPECT_LT(cursor()->GetLocation().x(), EPSILON);
    EXPECT_GT(cursor()->GetLocation().y(), cursor()->GetCursorConfinedBounds().height() - EPSILON);
}
...
TEST_F(TabletEventConverterEvdevTest, MoveBottomRight)
{
    ...
    EXPECT_GT(cursor()->GetLocation().x(), cursor()->GetCursorConfinedBounds().height() - EPSILON);
    EXPECT_GT(cursor()->GetLocation().y(), cursor()->GetCursorConfinedBounds().height() - EPSILON);
}

TEST_F(TabletEventConverterEvdevTest, ShouldDisableMouseWarpingToOtherDisplays)
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(kWacomIntuos5SPen);
...
TEST_F(TabletEventConverterEvdevTest, TabletButtonPress)
{
    ...
    feature_list.InitWithFeatures({ ash::features::kInputDeviceSettingsSplit, ash::features::kPeripheralCustomization }, {});
...
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(EpsonBrightLink1430);
...
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(WacomOnePenTabletMediumCTC6110WL);
    ...
    ExpectedEvent expected_events[] = {
        ...
        { PointerType::kPen, ui::EventType::kMousePressed, ui::EF_LEFT_MOUSE_BUTTON },
        { PointerType::kPen, ui::EventType::kMouseDragged, ui::EF_LEFT_MOUSE_BUTTON },
        { PointerType::kPen, ui::EventType::kMouseReleased, ui::EF_LEFT_MOUSE_BUTTON },
        { PointerType::kPen, ui::EventType::kMouseMoved, ui::EF_NONE },
    };
...
TEST_F(TabletEventConverterEvdevTest, SideEraserAlwaysPressedKernel5)
{
    ...
    ExpectedEvent expected_events[] = {
        ...
        { PointerType::kEraser, ui::EventType::kMousePressed, ui::EF_LEFT_MOUSE_BUTTON },
        { PointerType::kEraser, ui::EventType::kMouseDragged, ui::EF_LEFT_MOUSE_BUTTON },
        { PointerType::kEraser, ui::EventType::kMouseReleased, ui::EF_LEFT_MOUSE_BUTTON },
        { PointerType::kEraser, ui::EventType::kMouseMoved, ui::EF_NONE },
    };
...
TEST_F(TabletEventConverterEvdevTest, SideEraserReleasedWhileTouchingKernel5)
{
    ...
    struct input_event mock_kernel_queue[] = {
        { { 0, 0 }, EV_KEY, BTN_TOOL_RUBBER, 1 }, { { 0, 0 }, EV_ABS, ABS_X, 10326 }, { { 0, 0 }, EV_ABS, ABS_Y, 6075 }, { { 0, 0 }, EV_ABS, ABS_TILT_X, 1900 },
        { { 0, 0 }, EV_ABS, ABS_TILT_Y, 1400 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, // Event 0

        { { 0, 0 }, EV_MSC, MSC_SCAN, 0xd0045 }, { { 0, 0 }, EV_KEY, BTN_TOUCH, 1 }, // Event 1
        { { 0, 0 }, EV_KEY, BTN_TOOL_PEN, 1 }, { { 0, 0 }, EV_ABS, ABS_X, 10274 }, { { 0, 0 }, EV_ABS, ABS_Y, 6397 }, { { 0, 0 }, EV_ABS, ABS_PRESSURE, 1 },
        { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, // Event 2

        { { 0, 0 }, EV_MSC, MSC_SCAN, 0xd0045 }, { { 0, 0 }, EV_KEY, BTN_TOUCH, 0 }, // Event 3
        { { 0, 0 }, EV_ABS, ABS_PRESSURE, 0 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, // Event 4

        { { 0, 0 }, EV_KEY, BTN_TOOL_PEN, 0 }, { { 0, 0 }, EV_KEY, BTN_TOOL_RUBBER, 0 }, { { 0, 0 }, EV_ABS, ABS_TILT_X, 0 },
        { { 0, 0 }, EV_ABS, ABS_TILT_Y, 0 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 },

        { { 0, 0 }, EV_KEY, BTN_TOOL_PEN, 1 }, { { 0, 0 }, EV_ABS, ABS_X, 10297 }, { { 0, 0 }, EV_ABS, ABS_Y, 6947 }, { { 0, 0 }, EV_ABS, ABS_TILT_X, 2200 },
        { { 0, 0 }, EV_ABS, ABS_TILT_Y, 1800 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, // Event 5
    };
...
TEST_F(TabletEventConverterEvdevTest, SideEraserReleasedWhileTouchingKernel6)
{
    ...
    struct input_event mock_kernel_queue[] = {
        { { 0, 0 }, EV_KEY, BTN_TOOL_RUBBER, 1 }, { { 0, 0 }, EV_ABS, ABS_X, 12465 }, { { 0, 0 }, EV_ABS, ABS_Y, 7531 }, { { 0, 0 }, EV_ABS, ABS_TILT_X, 1200 },
        { { 0, 0 }, EV_ABS, ABS_TILT_Y, 700 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, // Event 0

        { { 0, 0 }, EV_MSC, MSC_SCAN, 0xd0045 }, { { 0, 0 }, EV_KEY, BTN_TOUCH, 1 }, // Event 1
        { { 0, 0 }, EV_ABS, ABS_PRESSURE, 1 }, { { 0, 0 }, EV_ABS, ABS_X, 12804 }, { { 0, 0 }, EV_ABS, ABS_Y, 8026 },
        { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, // Event 2
        ...
        { { 0, 0 }, EV_KEY, BTN_TOOL_PEN, 1 }, { { 0, 0 }, EV_ABS, ABS_Y, 8255 }, { { 0, 0 }, EV_ABS, ABS_TILT_X, 1100 }, { { 0, 0 }, EV_ABS, ABS_TILT_Y, 500 },
...
TEST_F(TabletEventConverterEvdevTest, SideEraserPressedWhileTouchingKernel5And6)
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(WacomOnePenTabletMediumCTC6110WL);
...
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(WacomIntuousProMPenPTH660);
...
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(XPPenStarG640);
    ...
    ExpectedEvent expected_events[] = {
        { PointerType::kPen, ui::EventType::kMousePressed, ui::EF_RIGHT_MOUSE_BUTTON },
        { PointerType::kPen, ui::EventType::kMouseDragged, ui::EF_RIGHT_MOUSE_BUTTON },
        { PointerType::kPen, ui::EventType::kMousePressed, ui::EF_LEFT_MOUSE_BUTTON | ui::EF_RIGHT_MOUSE_BUTTON },
        { PointerType::kPen, ui::EventType::kMouseDragged, ui::EF_LEFT_MOUSE_BUTTON | ui::EF_RIGHT_MOUSE_BUTTON },
        { PointerType::kPen, ui::EventType::kMouseReleased, ui::EF_LEFT_MOUSE_BUTTON | ui::EF_RIGHT_MOUSE_BUTTON },
        { PointerType::kPen, ui::EventType::kMouseDragged, ui::EF_RIGHT_MOUSE_BUTTON },
        { PointerType::kPen, ui::EventType::kMouseReleased, ui::EF_RIGHT_MOUSE_BUTTON },
    };
...
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(WacomOneByWacomSPenCTC472);
...
TEST_F(TabletEventConverterEvdevTest, Button1AlwaysPressedWithTailEraserKernel5And6)
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(WacomIntuousProMPenPTH660);
    ...
    ExpectedEvent expected_events[] = {
        { PointerType::kEraser, ui::EventType::kMouseMoved, ui::EF_NONE },
        { PointerType::kEraser, ui::EventType::kMousePressed, ui::EF_MIDDLE_MOUSE_BUTTON },
        { PointerType::kEraser, ui::EventType::kMousePressed, ui::EF_LEFT_MOUSE_BUTTON | ui::EF_MIDDLE_MOUSE_BUTTON },
        { PointerType::kEraser, ui::EventType::kMouseReleased, ui::EF_LEFT_MOUSE_BUTTON | ui::EF_MIDDLE_MOUSE_BUTTON },
        { PointerType::kEraser, ui::EventType::kMouseReleased, ui::EF_MIDDLE_MOUSE_BUTTON },
    };
...
TEST_F(TabletEventConverterEvdevTest, Button1ReleasedWhileTouchingWithTailEraserKernel5And6)
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(WacomIntuousProMPenPTH660);
...
TEST_F(TabletEventConverterEvdevTest, Button1PressedWhileTouchingWithTailEraserKernel5And6)
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(WacomIntuousProMPenPTH660);
...
TEST_F(TabletEventConverterEvdevTest, Button2AlwaysPressedWithTailEraserKernel5And6)
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(WacomIntuousProMPenPTH660);
    ...
    ExpectedEvent expected_events[] = {
        { PointerType::kEraser, ui::EventType::kMouseMoved, ui::EF_NONE },
        { PointerType::kEraser, ui::EventType::kMousePressed, ui::EF_RIGHT_MOUSE_BUTTON },
        { PointerType::kEraser, ui::EventType::kMousePressed, ui::EF_LEFT_MOUSE_BUTTON | ui::EF_RIGHT_MOUSE_BUTTON },
        { PointerType::kEraser, ui::EventType::kMouseReleased, ui::EF_LEFT_MOUSE_BUTTON | ui::EF_RIGHT_MOUSE_BUTTON },
        { PointerType::kEraser, ui::EventType::kMouseReleased, ui::EF_RIGHT_MOUSE_BUTTON },
    };
...
TEST_F(TabletEventConverterEvdevTest, Button2ReleasedWhileTouchingWithTailEraserKernel5And6)
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(WacomIntuousProMPenPTH660);
...
TEST_F(TabletEventConverterEvdevTest, Button2PressedWhileTouchingWithTailEraserKernel5And6)
{
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(WacomIntuousProMPenPTH660);
...
TEST_F(TabletEventConverterEvdevTest, Basic)
{
    ...
    std::unique_ptr<ui::MockTabletEventConverterEvdev> dev = CreateDevice(devinfo);
...
TEST_F(TabletEventConverterEvdevTest, LogXYAbs)
{
    ...
    input_absinfo absinfo_y = { .minimum = 3000, .maximum = 4000, .resolution = 9000 };
...
TEST_F(TabletEventConverterEvdevTest, LogXYTilt)
{
    ...
    input_absinfo absinfo_tilt_x = { .minimum = -120, .maximum = 40, .resolution = 12 };
    input_absinfo absinfo_tilt_y = { .minimum = 300, .maximum = 400, .resolution = 900 };
