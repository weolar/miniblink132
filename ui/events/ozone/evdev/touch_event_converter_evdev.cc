...
ui::HeatmapPalmDetector::ModelId GetHidrawModelId(const ui::EventDeviceInfo& info)
{
    ...
    std::vector<SupportedHidrawDevice> supported_hidraw_devices = GetSupportedHidrawDevices();
    for (const SupportedHidrawDevice& device : GetSupportedHidrawDevices()) {
        if (info.name() == device.name && info.vendor_id() == device.vendor_id && info.product_id() == device.product_id) {
...
{
    return base::FileEnumerator(root_path, false, base::FileEnumerator::DIRECTORIES).Next();
}

std::optional<ui::HeatmapPalmDetector::CropHeatmap> GetCropHeatmap(const ui::EventDeviceInfo& info)
{
...
BASE_FEATURE(kEnableSingleCancelTouch, "EnableSingleTouchCancel", base::FEATURE_DISABLED_BY_DEFAULT);

TouchEventConverterEvdev::TouchEventConverterEvdev(base::ScopedFD fd, base::FilePath path, int id, const EventDeviceInfo& devinfo,
    SharedPalmDetectionFilterState* shared_palm_state, DeviceEventDispatcherEvdev* dispatcher)
    : EventConverterEvdev(
        fd.get(), path, id, devinfo.device_type(), devinfo.name(), devinfo.phys(), devinfo.vendor_id(), devinfo.product_id(), devinfo.version())
    , input_device_fd_(std::move(fd))
    , dispatcher_(dispatcher)
    , palm_detection_filter_(CreatePalmDetectionFilter(devinfo, shared_palm_state))
    , palm_on_touch_major_max_(base::FeatureList::IsEnabled(kEnablePalmOnMaxTouchMajor))
    , palm_on_tool_type_palm_(base::FeatureList::IsEnabled(kEnablePalmOnToolTypePalm))
    , shared_palm_state_(shared_palm_state)
{
    if (base::FeatureList::IsEnabled(kEnableNeuralPalmDetectionFilter)
        && NeuralStylusPalmDetectionFilter::CompatibleWithNeuralStylusPalmDetectionFilter(devinfo)) {
...
// static
std::unique_ptr<TouchEventConverterEvdev> TouchEventConverterEvdev::Create(base::ScopedFD fd, base::FilePath path, int id, const EventDeviceInfo& devinfo,
    SharedPalmDetectionFilterState* shared_palm_state, DeviceEventDispatcherEvdev* dispatcher)
{
    auto converter = std::make_unique<TouchEventConverterEvdev>(std::move(fd), std::move(path), id, devinfo, shared_palm_state, dispatcher);
...
void TouchEventConverterEvdev::Initialize(const EventDeviceInfo& info)
{
    ...
    int32_t touch_major_res = info.GetAbsInfoByCode(ABS_MT_TOUCH_MAJOR).resolution;
    int32_t touch_minor_res = info.GetAbsInfoByCode(ABS_MT_TOUCH_MINOR).resolution;
    ...
    if (has_mt_) {
        ...
        tool_x_num_tuxels_ = info.GetAbsMaximum(ABS_MT_TOOL_X) - tool_x_min_tuxels_ + 1;
        tool_x_res_ = info.GetAbsInfoByCode(ABS_MT_TOOL_X).resolution;
        tool_y_min_tuxels_ = info.GetAbsMinimum(ABS_MT_TOOL_Y);
        tool_y_num_tuxels_ = info.GetAbsMaximum(ABS_MT_TOOL_Y) - tool_y_min_tuxels_ + 1;
    ...
    if (info.HasAbsEvent(ABS_MT_TOOL_X)) {
        DCHECK_EQ(tool_x_res_, x_res_) << "Expected tool X resolution to be identical as position X.";
    }
    if (info.HasAbsEvent(ABS_MT_TOOL_Y)) {
        DCHECK_EQ(tool_y_res_, y_res_) << "Expected tool Y resolution to be identical as position Y.";
    }
    ...
    if (has_mt_) {
        for (size_t i = 0; i < events_.size(); ++i) {
            ...
            events_[i].tracking_id = info.GetAbsMtSlotValueWithDefault(ABS_MT_TRACKING_ID, i, kTrackingIdForUnusedSlot);
            ...
            // Optional bits.
            int touch_major = info.GetAbsMtSlotValueWithDefault(ABS_MT_TOUCH_MAJOR, i, 0);
            int touch_minor = info.GetAbsMtSlotValueWithDefault(ABS_MT_TOUCH_MINOR, i, 0);
            events_[i].orientation = info.GetAbsMtSlotValueWithDefault(ABS_MT_ORIENTATION, i, 0);
            events_[i].pressure = ScalePressure(info.GetAbsMtSlotValueWithDefault(ABS_MT_PRESSURE, i, 0));
            int tool_type = info.GetAbsMtSlotValueWithDefault(ABS_MT_TOOL_TYPE, i, MT_TOOL_FINGER);
    ...
    if (base::FeatureList::IsEnabled(kEnableHeatmapPalmDetection)) {
        ...
        if (hidraw_model_id != HeatmapPalmDetector::ModelId::kNotSupported) {
            ...
            auto hidraw_device_dir = base::FilePath("/sys/class/input").Append(path_.BaseName()).Append("device/device/hidraw");
            auto hidraw_device_path = base::FilePath("/dev").Append(GetHidrawPath(hidraw_device_dir).BaseName());
            auto* palm_detector = HeatmapPalmDetector::GetInstance();
            if (palm_detector) {
                palm_detector->Start(hidraw_model_id, hidraw_device_path.value(), GetCropHeatmap(info));
            }
        }
    }
}
...
void TouchEventConverterEvdev::Reinitialize()
{
    ...
    if (!info.Initialize(fd_, path_)) {
        LOG(ERROR) << "Failed to synchronize state for touch device: " << path_.value();
        Stop();
        return;
    }
    Initialize(info);
...
{
    TRACE_EVENT1("evdev", "TouchEventConverterEvdev::OnFileCanReadWithoutBlocking", "fd", fd);
...
void TouchEventConverterEvdev::SetPalmSuppressionCallback(const base::RepeatingCallback<void(bool)>& callback)
{
    enable_palm_suppression_callback_ = callback;
}

void TouchEventConverterEvdev::SetReportStylusStateCallback(const ReportStylusStateCallback& callback)
{
    report_stylus_state_callback_ = callback;
}

void TouchEventConverterEvdev::SetGetLatestStylusStateCallback(const GetLatestStylusStateCallback& callback)
{
    get_latest_stylus_state_callback_ = callback;
}

void TouchEventConverterEvdev::ProcessMultitouchEvent(const input_event& input)
{
    ...
    if (input.type == EV_SYN) {
        ...
        if (events_.size() <= current_slot_) {
            LOG(ERROR) << "current_slot_ (" << current_slot_ << ") >= events_.size() (" << events_.size() << ")";
        } else {
            ProcessAbs(input);
        }
...
void TouchEventConverterEvdev::EmulateMultitouchEvent(const input_event& event)
{
    ...
    if (event.type == EV_ABS) {
        ...
        if (event.code == BTN_TOUCH || event.code == BTN_0 || (quirk_left_mouse_button_ && event.code == BTN_LEFT)) {
...
EventType TouchEventConverterEvdev::GetEventTypeForTouch(const InProgressTouchEvdev& touch)
{
    bool touch_is_alive = touch.touching && !touch.delayed && !touch.cancelled;
    bool touch_was_alive = touch.was_touching && !touch.was_delayed && !touch.was_cancelled;
...
void TouchEventConverterEvdev::ReportTouchEvent(const InProgressTouchEvdev& event, EventType event_type, base::TimeTicks timestamp)
{
    ui::PointerDetails details(event.reported_tool_type, /* pointer_id*/ 0, event.radius_x, event.radius_y, event.pressure,
        /* twist */ 0, event.tilt_x, event.tilt_y);
    int flags = event.stylus_button ? ui::EF_LEFT_MOUSE_BUTTON : 0;
    dispatcher_->DispatchTouchEvent(TouchEventParams(input_device_.id, event.slot, event_type, gfx::PointF(event.x, event.y), details, timestamp, flags));
}
...
bool TouchEventConverterEvdev::MaybeCancelAllTouches()
{
    ...
    // cancelled touches.
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableCancelAllTouches) || base::FeatureList::IsEnabled(kEnableSingleCancelTouch)) {
        return false;
    }
    for (size_t i = 0; i < events_.size(); i++) {
...
bool TouchEventConverterEvdev::IsPalm(const InProgressTouchEvdev& touch)
{
    ...
    if (palm_on_tool_type_palm_ && touch.tool_type == MT_TOOL_PALM)
        return true;
    else if (palm_on_touch_major_max_ && major_max_ > 0 && touch.major == major_max_)
        return true;
    return false;
}
...
void TouchEventConverterEvdev::ReportEvents(base::TimeTicks timestamp)
{
    ...
    for (size_t i = 0; i < events_.size(); i++) {
        ...
        if ((event->was_cancelled || event->cancelled) && !held_events_[i].empty()) {
            ...
            auto empty_q = std::queue<std::pair<InProgressTouchEvdev, base::TimeTicks>>();
            held_events_[i].swap(empty_q);
            UMA_HISTOGRAM_COUNTS_100(kHoldCountAtCancelEventName, empty_q.size());
        }

        if (!held_events_[i].empty()) {
            UMA_HISTOGRAM_COUNTS_100(kHoldCountAtReleaseEventName, held_events_[i].size());
...
void TouchEventConverterEvdev::UpdateSharedPalmState(base::TimeTicks timestamp)
{
...
{
    if (!(type() == InputDeviceType::INPUT_DEVICE_INTERNAL && IsEnabled()) || has_pen_ || x_res_ <= 0 || y_res_ <= 0) {
        return;
    }
    ...
    for (auto& event : events_) {
        ...
        for (auto it = cancelled_touches_.begin(); it != cancelled_touches_.end();) {
            auto& touch = *it;
            if ((timestamp - touch.cancel_timestamp).InSeconds() > kMaxRepeatedTouchGapInSeconds) {
                it = cancelled_touches_.erase(it);
                continue;
            }
            float dist_sqr = pow(float(event.x - touch.start_x) / x_res_, 2) + pow(float(event.y - touch.start_y) / y_res_, 2);
    ...
    for (const auto& event : events_) {
        if (event.touching && event.cancelled && (!event.was_cancelled || !event.was_touching)) {
            // It is newly canceled touch.
            cancelled_touches_.insert(CancelledTouch(timestamp, event.start_x, event.start_y));
        }
    }
}
...
void TouchEventConverterEvdev::RecordMetrics(base::TimeTicks timestamp)
{
    ...
    record_session_timer_.Start(FROM_HERE, base::Seconds(kMaxTouchSessionGapInSeconds),
        base::BindOnce(&TouchEventConverterEvdev::RecordSession, weak_factory_.GetWeakPtr(), timestamp - session_start_time_.value()));

    if (has_pen_) {
        if (events_[0].touching && !events_[0].was_touching) {
            base::TimeDelta gap = timestamp - shared_palm_state_->latest_finger_touch_time;
            UMA_HISTOGRAM_TIMES(kTouchGapBeforeStylusEventName, gap);
            UMA_HISTOGRAM_ENUMERATION(kTouchTypeBeforeStylusEventName,
                gap.InMilliseconds() >= kMaxTouchStylusGapInMs ? TouchType::kNone : (shared_palm_state_->has_palm ? TouchType::kPalm : TouchType::kFinger));

            if (shared_palm_state_->need_to_record_after_stylus_metrics) {
                shared_palm_state_->need_to_record_after_stylus_metrics = false;
                UMA_HISTOGRAM_TIMES(kTouchGapAfterStylusEventName, base::Seconds(kMaxTouchStylusGapInSeconds));
                UMA_HISTOGRAM_ENUMERATION(kTouchTypeAfterStylusEventName, TouchType::kNone);
            }
        }
    } else {
        ...
        if (shared_palm_state_->need_to_record_after_stylus_metrics) {
            shared_palm_state_->need_to_record_after_stylus_metrics = false;
            base::TimeDelta gap = timestamp - shared_palm_state_->latest_stylus_touch_time;
            UMA_HISTOGRAM_TIMES(kTouchGapAfterStylusEventName, gap);
            UMA_HISTOGRAM_ENUMERATION(kTouchTypeAfterStylusEventName, shared_palm_state_->has_palm ? TouchType::kPalm : TouchType::kFinger);
        }
    }
}
...
void TouchEventConverterEvdev::ProcessTouchEvent(InProgressTouchEvdev* event, base::TimeTicks timestamp)
{
    if (enable_palm_suppression_callback_) {
...
void TouchEventConverterEvdev::UpdateRadiusFromTouchWithOrientation(InProgressTouchEvdev* event) const
{
...
{
    os << "class=ui::TouchEventConverterEvdev id=" << input_device_.id << std::endl
...
const char TouchEventConverterEvdev::kHoldCountAtReleaseEventName[] = "Ozone.TouchEventConverterEvdev.HoldCountAtRelease";
const char TouchEventConverterEvdev::kHoldCountAtCancelEventName[] = "Ozone.TouchEventConverterEvdev.HoldCountAtCancel";
const char TouchEventConverterEvdev::kPalmFilterTimerEventName[] = "Ozone.TouchEventConverterEvdev.PalmDetectionFilterTime";
const char TouchEventConverterEvdev::kPalmTouchCountEventName[] = "Ozone.TouchEventConverterEvdev.PalmTouchCount";
const char TouchEventConverterEvdev::kRepeatedTouchCountEventName[] = "Ozone.TouchEventConverterEvdev.RepeatedTouchCount";
const char TouchEventConverterEvdev::kTouchGapAfterStylusEventName[] = "Ozone.TouchEventConverterEvdev.TouchGapAfterStylus";
const char TouchEventConverterEvdev::kTouchGapBeforeStylusEventName[] = "Ozone.TouchEventConverterEvdev.TouchGapBeforeStylus";
const char TouchEventConverterEvdev::kTouchTypeAfterStylusEventName[] = "Ozone.TouchEventConverterEvdev.TouchTypeAfterStylus";
const char TouchEventConverterEvdev::kTouchTypeBeforeStylusEventName[] = "Ozone.TouchEventConverterEvdev.TouchTypeBeforeStylus";
const char TouchEventConverterEvdev::kTouchSessionCountEventName[] = "Ozone.TouchEventConverterEvdev.TouchSessionCount";
const char TouchEventConverterEvdev::kTouchSessionLengthEventName[] = "Ozone.TouchEventConverterEvdev.TouchSessionLength";
const char TouchEventConverterEvdev::kStylusSessionCountEventName[] = "Ozone.TouchEventConverterEvdev.StylusSessionCount";
const char TouchEventConverterEvdev::kStylusSessionLengthEventName[] = "Ozone.TouchEventConverterEvdev.StylusSessionLength";
} // namespace ui
