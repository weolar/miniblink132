...
GamepadEventConverterEvdev::GamepadEventConverterEvdev(
    base::ScopedFD fd, base::FilePath path, int id, const EventDeviceInfo& devinfo, DeviceEventDispatcherEvdev* dispatcher)
    : EventConverterEvdev(
...
void GamepadEventConverterEvdev::PlayVibrationEffect(uint8_t amplitude, uint16_t duration_millis)
{
    ...
    uint16_t magnitude_scaled = static_cast<uint16_t>(amplitude_ratio * kRumbleMagnitudeMax);

    effect_id_ = StoreRumbleEffect(input_device_fd_, effect_id_, duration_millis, 0, magnitude_scaled, magnitude_scaled);
...
{
    if (!supports_rumble_) {
        LOG(ERROR) << "Device doesn't support rumble, but SetZeroVibration is called.";
        return;
    }
    if (effect_id_ == kInvalidEffectId) {
...
int GamepadEventConverterEvdev::StoreRumbleEffect(
    const base::ScopedFD& fd, int effect_id, uint16_t duration, uint16_t start_delay, uint16_t strong_magnitude, uint16_t weak_magnitude)
{
...
void GamepadEventConverterEvdev::StartOrStopEffect(const base::ScopedFD& fd, int effect_id, bool do_start)
{
...
void GamepadEventConverterEvdev::DestroyEffect(const base::ScopedFD& fd, int effect_id)
{
...
ssize_t GamepadEventConverterEvdev::WriteEvent(const base::ScopedFD& fd, const struct input_event& event)
{
    return HANDLE_EINTR(write(fd.get(), static_cast<const void*>(&event), sizeof(event)));
}

int GamepadEventConverterEvdev::UploadFfEffect(const base::ScopedFD& fd, struct ff_effect* effect)
{
...
{
    TRACE_EVENT1("evdev", "GamepadEventConverterEvdev::OnFileCanReadWithoutBlocking", "fd", fd);
    while (true) {
        input_event input;
...
std::vector<ui::GamepadDevice::Axis> GamepadEventConverterEvdev::GetGamepadAxes() const
{
    return axes_;
}
...
void GamepadEventConverterEvdev::ProcessEvdevKey(uint16_t code, int value, const base::TimeTicks& timestamp)
{
    if (value)
        pressed_buttons_.insert(code);
    else
        pressed_buttons_.erase(code);
    GamepadEvent event(input_device_.id, GamepadEventType::BUTTON, code, value, timestamp);
    dispatcher_->DispatchGamepadEvent(event);
    will_send_frame_ = true;
}

void GamepadEventConverterEvdev::ProcessEvdevAbs(uint16_t code, int value, const base::TimeTicks& timestamp)
{
    GamepadEvent event(input_device_.id, GamepadEventType::AXIS, code, value, timestamp);
    dispatcher_->DispatchGamepadEvent(event);
    will_send_frame_ = true;
}
...
void GamepadEventConverterEvdev::ResyncGamepad()
{
    ...
    if (!info.Initialize(fd_, path_)) {
        LOG(ERROR) << "Failed to synchronize state for gamepad device: " << path_.value();
        Stop();
        return;
    }
    for (int code = 0; code < ABS_CNT; ++code) {
...
{
    if (will_send_frame_) {
        GamepadEvent event(input_device_.id, GamepadEventType::FRAME, 0, 0, timestamp);
        dispatcher_->DispatchGamepadEvent(event);
        will_send_frame_ = false;
    }
}

std::ostream& GamepadEventConverterEvdev::DescribeForLog(std::ostream& os) const
{
    os << "class=ui::GamepadEventConverterEvdev id=" << input_device_.id << std::endl << " supports_rumble=" << supports_rumble_ << std::endl << "base ";
