...
class COMPONENT_EXPORT(EVDEV) StylusButtonEventConverterEvdev : public EventConverterEvdev {
public:
    StylusButtonEventConverterEvdev(base::ScopedFD fd, base::FilePath path, int id, const EventDeviceInfo& devinfo, DeviceEventDispatcherEvdev* dispatcher);

    StylusButtonEventConverterEvdev(const StylusButtonEventConverterEvdev&) = delete;
    StylusButtonEventConverterEvdev& operator=(const StylusButtonEventConverterEvdev&) = delete;
