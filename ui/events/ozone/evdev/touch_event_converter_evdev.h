...
class COMPONENT_EXPORT(EVDEV) TouchEventConverterEvdev : public EventConverterEvdev {
public:
    TouchEventConverterEvdev(base::ScopedFD fd, base::FilePath path, int id, const EventDeviceInfo& devinfo, SharedPalmDetectionFilterState* shared_palm_state,
    ...
    static std::unique_ptr<TouchEventConverterEvdev> Create(base::ScopedFD fd, base::FilePath path, int id, const EventDeviceInfo& devinfo,
    ...
    void SetPalmSuppressionCallback(const base::RepeatingCallback<void(bool)>& callback) override;

    // Sets callback to report the latest stylus state.
    void SetReportStylusStateCallback(const ReportStylusStateCallback& callback) override;

    // Sets callback to get the latest stylus state.
    void SetGetLatestStylusStateCallback(const GetLatestStylusStateCallback& callback) override;
...
private:
    struct CancelledTouch {
        ...
        CancelledTouch(base::TimeTicks cancel_timestamp, float start_x, float start_y)
        ...
        {
            return cancel_timestamp == other.cancel_timestamp && start_x == other.start_x && start_y == other.start_y;
        }

        bool operator<(const CancelledTouch& other) const
        {
            return std::tie(cancel_timestamp, start_x, start_y) < std::tie(other.cancel_timestamp, other.start_x, other.start_y);
        }
    };
    ...
    void ReportTouchEvent(const InProgressTouchEvdev& event, EventType event_type, base::TimeTicks timestamp);
    void ReportEvents(base::TimeTicks timestamp);

    void ProcessTouchEvent(InProgressTouchEvdev* event, base::TimeTicks timestamp);
    ...
    // were held at.
    std::vector<std::queue<std::pair<InProgressTouchEvdev, base::TimeTicks>>> held_events_;
