...
EventConverterEvdevImpl::EventConverterEvdevImpl(
    base::ScopedFD fd, base::FilePath path, int id, const EventDeviceInfo& devinfo, CursorDelegateEvdev* cursor, DeviceEventDispatcherEvdev* dispatcher)
    : EventConverterEvdev(
...
{
    TRACE_EVENT1("evdev", "EventConverterEvdevImpl::OnFileCanReadWithoutBlocking", "fd", fd);
...
void EventConverterEvdevImpl::SetKeyFilter(bool enable_filter, std::vector<DomCode> allowed_keys)
{
    if (!enable_filter) {
        blocked_keys_.reset();
        return;
...
ui::StylusState EventConverterEvdevImpl::GetStylusSwitchState()
{
    ...
    return EvdevBitIsSet(array, kSwitchStylusInserted) ? ui::StylusState::INSERTED : ui::StylusState::REMOVED;
}

void EventConverterEvdevImpl::ProcessEvents(const input_event* inputs, int count)
{
    for (int i = 0; i < count; ++i) {
        ...
        case EV_SW:
            if (input.code == kSwitchStylusInserted) {
                dispatcher_->DispatchStylusStateChanged(input.value ? ui::StylusState::INSERTED : ui::StylusState::REMOVED);
            }
            break;
        }
    }
...
void EventConverterEvdevImpl::ConvertKeyEvent(const input_event& input)
{
    ...
    // Keyboard processing.
    OnKeyChange(input.code, input.value != kKeyReleaseValue, TimeTicksFromInputEvent(input));
}
...
void EventConverterEvdevImpl::OnKeyChange(unsigned int key, bool down, const base::TimeTicks& timestamp)
{
    if (key > KEY_MAX)
        return;
    ...
    dispatcher_->DispatchKeyEvent(KeyEventParams(input_device_.id, ui::EF_NONE, key, last_scan_code_, down, false /* suppress_auto_repeat */, timestamp));
}
...
void EventConverterEvdevImpl::OnButtonChange(int code, bool down, base::TimeTicks timestamp)
{
    ...
    dispatcher_->DispatchMouseButtonEvent(MouseButtonEventParams(
        input_device_.id, EF_NONE, cursor_->GetLocation(), code, down, MouseButtonMapType::kMouse, PointerDetails(EventPointerType::kMouse), timestamp));
}

void EventConverterEvdevImpl::SetReceivedValidInputCallback(ReceivedValidInputCallback callback)
{
...
void EventConverterEvdevImpl::FlushEvents(const input_event& input)
{
    ...
    dispatcher_->DispatchMouseMoveEvent(MouseMoveEventParams(input_device_.id, EF_NONE, cursor_->GetLocation(), nullptr /* ordinal_delta*/,
        PointerDetails(EventPointerType::kMouse), TimeTicksFromInputEvent(input)));
