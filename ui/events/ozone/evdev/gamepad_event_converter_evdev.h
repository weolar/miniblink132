...
class COMPONENT_EXPORT(EVDEV) GamepadEventConverterEvdev : public EventConverterEvdev {
public:
    GamepadEventConverterEvdev(base::ScopedFD fd, base::FilePath path, int id, const EventDeviceInfo& info, DeviceEventDispatcherEvdev* dispatcher);

    GamepadEventConverterEvdev(const GamepadEventConverterEvdev&) = delete;
    GamepadEventConverterEvdev& operator=(const GamepadEventConverterEvdev&) = delete;
    ...
    void PlayVibrationEffect(uint8_t amplitude, uint16_t duration_millis) override;
...
private:
    ...
    void ProcessEvdevKey(uint16_t code, int value, const base::TimeTicks& timestamp);

    // This function processes EV_ABS event from gamepad device.
    void ProcessEvdevAbs(uint16_t code, int value, const base::TimeTicks& timestamp);
    ...
    int StoreRumbleEffect(const base::ScopedFD& fd, int effect_id, uint16_t duration, uint16_t start_delay, uint16_t strong_magnitude, uint16_t weak_magnitude);

    // This function controls the playback of the effect on the gamepad device.
    void StartOrStopEffect(const base::ScopedFD& fd, int effect_id, bool do_start);
    ...
    // of the write.
    virtual ssize_t WriteEvent(const base::ScopedFD& fd, const struct input_event& input);

    // This function uploads the ff_effect to the gamepad device and returns the
    // unique id assigned by the driver.
    virtual int UploadFfEffect(const base::ScopedFD& fd, struct ff_effect* effect);
