...
std::optional<KeyboardCode> RemapButtonsToKeyboardCodes(unsigned int key)
{
    ...
    if (key >= BTN_0 && key <= BTN_9) {
        return static_cast<KeyboardCode>(static_cast<int>(VKEY_BUTTON_0) + (key - BTN_0));
    }

    // BTN_A through BTN_Z is only 6 buttons as it only includes {A, B, C, X, Y,
    // Z}.
    if (key >= BTN_A && key <= BTN_Z) {
        return static_cast<KeyboardCode>(static_cast<int>(VKEY_BUTTON_A) + (key - BTN_A));
    }
#endif
...
KeyboardEvdev::KeyboardEvdev(EventModifiers* modifiers, KeyboardLayoutEngine* keyboard_layout_engine, const EventDispatchCallback& callback,
...
void KeyboardEvdev::OnKeyChange(
    unsigned int key, unsigned int scan_code, bool down, bool suppress_auto_repeat, base::TimeTicks timestamp, int device_id, int flags)
{
    if (key > KEY_MAX)
        return;
    ...
    auto_repeat_handler_.UpdateKeyRepeat(key, scan_code, down, suppress_auto_repeat, device_id, timestamp);
...
void KeyboardEvdev::SetAutoRepeatRate(const base::TimeDelta& delay, const base::TimeDelta& interval)
{
    auto_repeat_handler_.SetAutoRepeatRate(delay, interval);
}

void KeyboardEvdev::GetAutoRepeatRate(base::TimeDelta* delay, base::TimeDelta* interval)
{
    auto_repeat_handler_.GetAutoRepeatRate(delay, interval);
}

void KeyboardEvdev::SetCurrentLayoutByName(const std::string& layout_name, base::OnceCallback<void(bool)> callback)
{
    keyboard_layout_engine_->SetCurrentLayoutByName(layout_name, std::move(callback));
    RefreshModifiers();
}
...
void KeyboardEvdev::DispatchKey(unsigned int key, unsigned int scan_code, bool down, bool repeat, base::TimeTicks timestamp, int device_id, int flags)
{
    DomCode dom_code = KeycodeConverter::EvdevCodeToDomCode(key);
    DomKey dom_key;
    KeyboardCode key_code;

    if (auto button_key_code = RemapButtonsToKeyboardCodes(key); button_key_code.has_value()) {
        ...
        if (!keyboard_layout_engine_->Lookup(dom_code, modifier_flags, &dom_key, &key_code)) {
            return;
        }
        if (!repeat) {
            int flag = ModifierDomKeyToEventFlag(dom_key);
    ...
    KeyEvent event(down ? EventType::kKeyPressed : EventType::kKeyReleased, key_code, dom_code, flags | modifiers_->GetModifierFlags(), dom_key, timestamp);
