...
class MockStylusButtonEventConverterEvdev : public StylusButtonEventConverterEvdev {
public:
    MockStylusButtonEventConverterEvdev(base::ScopedFD fd, base::FilePath path, const EventDeviceInfo& devinfo, DeviceEventDispatcherEvdev* dispatcher);

    MockStylusButtonEventConverterEvdev(const MockStylusButtonEventConverterEvdev&) = delete;
    MockStylusButtonEventConverterEvdev& operator=(const MockStylusButtonEventConverterEvdev&) = delete;
...
MockStylusButtonEventConverterEvdev::MockStylusButtonEventConverterEvdev(
    base::ScopedFD fd, base::FilePath path, const EventDeviceInfo& devinfo, DeviceEventDispatcherEvdev* dispatcher)
    : StylusButtonEventConverterEvdev(std::move(fd), path, 1, devinfo, dispatcher)
{
    int fds[2];
...
void MockStylusButtonEventConverterEvdev::ConfigureReadMock(struct input_event* queue, long read_this_many, long queue_index)
{
...
public:
    ...
    StylusButtonEventConverterEvdevTest(const StylusButtonEventConverterEvdevTest&) = delete;
    StylusButtonEventConverterEvdevTest& operator=(const StylusButtonEventConverterEvdevTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        event_factory_ = ui::CreateEventFactoryEvdevForTest(nullptr, device_manager_.get(), keyboard_layout_engine_.get(),
            base::BindRepeating(&StylusButtonEventConverterEvdevTest::DispatchEventForTest, base::Unretained(this)));
        dispatcher_ = ui::CreateDeviceEventDispatcherEvdevForTest(event_factory_.get());
    }

    ui::MockStylusButtonEventConverterEvdev* CreateDevice(const ui::DeviceCapabilities& caps)
    {
        int evdev_io[2];
        if (pipe(evdev_io))
            PLOG(FATAL) << "failed pipe";
        ...
        return new ui::MockStylusButtonEventConverterEvdev(std::move(events_in), base::FilePath(kTestDevicePath), devinfo, dispatcher_.get());
    }
...
{
    std::unique_ptr<ui::MockStylusButtonEventConverterEvdev> dev = base::WrapUnique(CreateDevice(ui::kDellActivePenButton));
