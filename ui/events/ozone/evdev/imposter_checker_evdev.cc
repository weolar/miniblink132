...
std::vector<int> ImposterCheckerEvdev::GetIdsOnSamePhys(const std::string& phys_path)
{
...
bool ImposterCheckerEvdev::IsSuspectedKeyboardImposter(EventConverterEvdev* converter, bool shared_phys)
{
...
#endif
    if (!converter->HasKeyboard() || (!converter->HasMouse() && !shared_phys) || converter->type() == InputDeviceType::INPUT_DEVICE_INTERNAL) {
        return false;
    }
#if BUILDFLAG(IS_CHROMEOS)
...
bool ImposterCheckerEvdev::IsSuspectedMouseImposter(EventConverterEvdev* converter, bool shared_phys)
{
    if (!base::FeatureList::IsEnabled(kEnableFakeMouseHeuristic)) {
        return false;
    }

    if (!converter->HasMouse() || (!converter->HasKeyboard() && !shared_phys) || converter->type() == InputDeviceType::INPUT_DEVICE_INTERNAL) {
        return false;
    }
    return true;
}

std::vector<int> ImposterCheckerEvdev::OnDeviceAdded(EventConverterEvdev* converter)
{
...
bool ImposterCheckerEvdev::FlagSuspectedImposter(EventConverterEvdev* converter)
{
    bool shared_phys = devices_on_phys_path_.count(StandardizedPhys(converter->input_device().phys)) > 1;

    bool is_keyboard_imposter = IsSuspectedKeyboardImposter(converter, shared_phys);
    ...
    if (is_keyboard_imposter) {
        LOG(WARNING) << "Device Name: " << converter->input_device().name << " Vendor ID: " << base::StringPrintf("0x%04x", converter->input_device().vendor_id)
                     << " Product ID: " << base::StringPrintf("0x%04x", converter->input_device().product_id)
...
std::vector<int> ImposterCheckerEvdev::OnDeviceRemoved(EventConverterEvdev* converter)
{
    std::erase_if(devices_on_phys_path_, [&](const auto& devices_on_phys_path_entry) { return devices_on_phys_path_entry.second == converter->id(); });
    return GetIdsOnSamePhys(StandardizedPhys(converter->input_device().phys));
}

ImposterCheckerEvdev::ImposterCheckerEvdev()
    : imposter_checker_evdev_state_(std::make_unique<ImposterCheckerEvdevState>())
{
}
