...
InputInjectorEvdev::InputInjectorEvdev(std::unique_ptr<DeviceEventDispatcherEvdev> dispatcher, CursorDelegateEvdev* cursor)
    : cursor_(cursor)
...
void InputInjectorEvdev::InjectMouseButton(EventFlags button, bool down)
{
    ...
    dispatcher_->DispatchMouseButtonEvent(MouseButtonEventParams(
        device_id_, EF_NONE, cursor_->GetLocation(), code, down, MouseButtonMapType::kNone, PointerDetails(EventPointerType::kMouse), EventTimeForNow()));
}

void InputInjectorEvdev::InjectMouseWheel(int delta_x, int delta_y)
{
    dispatcher_->DispatchMouseWheelEvent(MouseWheelEventParams(device_id_, cursor_->GetLocation(), gfx::Vector2d(delta_x, delta_y), EventTimeForNow()));
}
...
void InputInjectorEvdev::MoveCursorTo(const gfx::PointF& location)
{
    ...
    dispatcher_->DispatchMouseMoveEvent(MouseMoveEventParams(
        device_id_, event_flags, cursor_->GetLocation(), nullptr /* ordinal_delta */, PointerDetails(EventPointerType::kMouse), EventTimeForNow()));
}

void InputInjectorEvdev::InjectKeyEvent(DomCode physical_key, bool down, bool suppress_auto_repeat)
{
    if (physical_key == DomCode::NONE)
        return;

    int evdev_code = KeycodeConverter::DomCodeToEvdevCode(physical_key);
    dispatcher_->DispatchKeyEvent(KeyEventParams(device_id_, EF_NONE, evdev_code, 0 /*scan_code*/, down, suppress_auto_repeat, EventTimeForNow()));
}
