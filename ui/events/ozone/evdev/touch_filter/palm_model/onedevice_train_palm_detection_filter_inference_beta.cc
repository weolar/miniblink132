...
#if USE_EIGEN
template <typename Scalar> using ConstMatrixMap = typename Eigen::Map<const Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>>;
template <typename Scalar> using ConstRowVectorMap = typename Eigen::Map<const Eigen::Matrix<Scalar, Eigen::Dynamic, 1>>;
template <typename Scalar> using RowVectorMap = typename Eigen::Map<Eigen::Matrix<Scalar, Eigen::Dynamic, 1>>;
template <typename Scalar> using MatrixMap = typename Eigen::Map<Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>>;
template <typename Scalar, typename StorageIndex> using SparseMatrix = Eigen::SparseMatrix<Scalar, Eigen::RowMajor, StorageIndex>;
#endif
...
public:
    ...
    void WriteTimingsToInfoLog()
    {
        ...
        for (auto& entry : time_per_op_) {
            total += entry.second;
            absl::StrAppend(&message, entry.first, ": ", absl::LegacyPrecision(absl::ToDoubleMilliseconds(entry.second)), " ms\n");
        }
...
public:
    ...
    ~BenchmarkTimer()
    {
        ...
        VLOG(1) << "Time for " << name_ << ": " << absl::ToDoubleMilliseconds(elapsed) << " ms";
    }
...
inline std::tuple<int, int> GetConvOutputSizeVALID(const int32_t* input_shape, const int32_t* kernel_shape, int32_t stride_y, int32_t stride_x)
{
    return std::make_tuple((input_shape[1] + stride_y - kernel_shape[0]) / stride_y, (input_shape[2] + stride_x - kernel_shape[1]) / stride_x);
}

// For convolutional operations, calculates the output size with SAME padding.
// Returns (height, width).
inline std::tuple<int, int> GetConvOutputSizeSAME(const int32_t* input_shape, int32_t stride_y, int32_t stride_x)
{
    return std::make_tuple((input_shape[1] + stride_y - 1) / stride_y, (input_shape[2] + stride_x - 1) / stride_x);
}

// Helper to compute the size of the inner loop for an op that uses indices to
// specify which axes are reduced.
template <typename Tidx>
int32_t GetReduceInnerSize(int32_t input_tensor_rank, const int32_t* __restrict input_shape, int32_t index_tensor_rank, const int32_t* __restrict index_shape,
...
template <typename T>
void ConcatV2Args2(int32_t arg0_rank, const int32_t* __restrict arg0_shape, const T* __restrict arg0_values, int32_t arg1_rank,
    const int32_t* __restrict arg1_shape, const T* __restrict arg1_values, const int32_t* __restrict axis_value, T* __restrict output_values)
{
...
template <typename T>
void MatMul(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t* __restrict weight_shape, const T* __restrict weight_values,
    T* __restrict output_values)
{
    BENCHMARK_TIMER("MatMul");
#if USE_EIGEN
    const auto in = ConstMatrixMap<T>(input_values, input_shape[1], input_shape[0]);
    const auto weight = ConstMatrixMap<T>(weight_values, weight_shape[1], weight_shape[0]);
    ...
    for (int32_t batch = 0; batch < batch_size; ++batch) {
        for (int32_t out_i = 0; out_i < num_outputs; ++out_i) {
            ...
            for (int32_t in_i = 0; in_i < num_inputs; ++in_i) {
                value += input_values[batch * num_inputs + in_i] * weight_values[in_i * num_outputs + out_i];
            }
            *output_values++ = value;
        }
    }
...
template <typename T>
void DepthwiseConv2dNative(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t* __restrict kernel_shape,
    const T* __restrict kernel_values, int32_t stride_y, int32_t stride_x, int32_t out_height, int32_t out_width, T* __restrict output_values)
{
    ...
    const int32_t pad_height = ((out_height - 1) * stride_y + kernel_height - in_height) / 2;
    const int32_t pad_width = ((out_width - 1) * stride_x + kernel_width - in_width) / 2;
    ...
    for (int32_t batch = 0; batch < batch_size; ++batch) {
        for (int32_t out_y = 0; out_y < out_height; ++out_y) {
            for (int32_t out_x = 0; out_x < out_width; ++out_x) {
                ...
                const int32_t kernel_y_start = std::max(static_cast<int32_t>(0), -in_y_origin);
                const int32_t kernel_y_end = std::min(kernel_height, in_height - in_y_origin);
                const int32_t kernel_x_start = std::max(static_cast<int32_t>(0), -in_x_origin);
                const int32_t kernel_x_end = std::min(kernel_width, in_width - in_x_origin);
                    ...
                    for (int32_t mul_c = 0; mul_c < depth_mul; ++mul_c, ++out_write_ptr) {
                        ...
                        for (int32_t k_y = kernel_y_start; k_y < kernel_y_end; ++k_y) {
                            ...
                            for (int32_t k_x = kernel_x_start; k_x < kernel_x_end; ++k_x) {
                                ...
                                const T input_value = input_values[batch * in_strides[0] + // batch
                                    in_y * in_strides[1] + // y
                                    in_x * in_strides[2] + // x
                                    in_c]; // in chan
                                const T kernel_value = kernel_values[k_y * kernel_strides[0] + // y
...
template <typename T>
void DepthwiseConv2dNativeVALID(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t* __restrict kernel_shape,
    const T* __restrict kernel_values, int32_t stride_y, int32_t stride_x, T* __restrict output_values)
{
    const auto out_size = GetConvOutputSizeVALID(input_shape, kernel_shape, stride_y, stride_x);
    DepthwiseConv2dNative<T>(
        input_shape, input_values, kernel_shape, kernel_values, stride_y, stride_x, std::get<0>(out_size), std::get<1>(out_size), output_values);
}

template <typename T>
void DepthwiseConv2dNativeSAME(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t* __restrict kernel_shape,
    const T* __restrict kernel_values, int32_t stride_y, int32_t stride_x, T* __restrict output_values)
{
...
template <typename T>
void FullyConnected(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t* __restrict weight_shape,
    const T* __restrict weight_values, const int32_t* __restrict bias_shape, const T* __restrict bias_values, T* __restrict output_values)
{
...
void SpaceToBatchNDRank4(const int32_t* __restrict input_shape, const T* __restrict input_values, const TBlocks* __restrict block_shape_values,
    const TPaddings* __restrict padding_values, T* __restrict output_values)
{
    ...
    const int32_t output_height = (padding_top + padding_bottom + input_height) / block_shape_height;
    const int32_t output_width = (padding_left + padding_right + input_width) / block_shape_width;
    ...
    for (int32_t out_b = 0; out_b < output_batch_size; ++out_b) {
        ...
        for (int32_t out_h = 0; out_h < output_height; ++out_h) {
            for (int32_t out_w = 0; out_w < output_width; ++out_w) {
                T* out = output_values + (((out_b * output_height + out_h) * output_width + out_w) * output_depth + 0);
                // Check if padding cell are being handled.
                if (out_h * block_shape_height + shift_h < padding_top || out_h * block_shape_height + shift_h >= padding_top + input_height
                    || out_w * block_shape_width + shift_w < padding_left || out_w * block_shape_width + shift_w >= padding_left + input_width) {
// This may not execute correctly when pad_value != 0 and T != uint8.
                    ...
                    const int32_t i1 = (out_h * block_shape_height + shift_h) - padding_top;
                    const int32_t i2 = (out_w * block_shape_width + shift_w) - padding_left;
                    const T* in = input_values + (((i0 * input_height + i1) * input_width + i2) * input_depth + 0);
#if USE_TYPED_MEMSETMEMCPY
...
void BatchToSpaceNDRank4(const int32_t* __restrict input_shape, const T* __restrict input_values, const TBlocks* __restrict block_shape_values,
    const TCrops* __restrict crops_values, T* __restrict output_values)
{
    ...
    const int32_t output_depth = input_depth;
    const int32_t output_batch_size = input_batch_size / (block_shape_width * block_shape_height);
    const int32_t output_height = input_height * block_shape_height - crops_top - crops_bottom;
    const int32_t output_width = input_width * block_shape_width - crops_left - crops_right;
    ...
    for (int32_t in_batch = 0; in_batch < input_batch_size; ++in_batch) {
        ...
        for (int32_t in_h = 0; in_h < input_height; ++in_h) {
            const int32_t out_h = in_h * block_shape_height + spatial_offset / block_shape_width - crops_top;
            if (out_h < 0 || out_h >= output_height) {
                continue;
            }
            for (int32_t in_w = 0; in_w < input_width; ++in_w) {
                const int32_t out_w = in_w * block_shape_width + spatial_offset % block_shape_width - crops_left;

                if (out_w < 0 || out_w >= output_width) {
                    continue;
                }
                T* out = output_values + (((out_batch * output_height + out_h) * output_width + out_w) * output_depth + 0);
                const T* in = input_values + (((in_batch * input_height + in_h) * input_width + in_w) * input_depth + 0);
#if USE_TYPED_MEMSETMEMCPY
                std::copy(in, in + input_depth, out);
#else
                std::memcpy(out, in, input_depth * sizeof(T));
...
void SparseDenseMatMulCSR(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t num_rows, const int32_t* __restrict nnz_shape,
    const T* __restrict nnz_values, const Tidx* __restrict outer_index, const Tidx* __restrict cols, T* __restrict output_values)
{
    BENCHMARK_TIMER("SparseDenseMatMulCSR");
    const int32_t num_cols = input_shape[1];
    const auto in = ConstMatrixMap<T>(input_values, input_shape[1], input_shape[0]);
    const Eigen::Map<const SparseMatrix<T, Tidx>> weight(num_rows, num_cols, nnz_shape[0], outer_index, cols, nnz_values);
    auto result = MatrixMap<T>(output_values, num_rows, input_shape[0]);
    result.noalias() = weight * in;
}

template <typename T, typename Tidx>
void SparseFullyConnectedCSR(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t num_rows,
    const int32_t* __restrict nnz_shape, const T* __restrict nnz_values, const Tidx* __restrict outer_index, const Tidx* __restrict cols,
    const int32_t* __restrict bias_shape, const T* __restrict bias_values, T* __restrict output_values)
{
...
void Gather(int32_t params_rank, const int32_t* __restrict params_shape, const T* __restrict params_values, int32_t indices_rank,
    const int32_t* __restrict indices_shape, const TIndex* __restrict indices_values, T* __restrict output_values)
{
    ...
    for (int32_t i = 0; i < num_indices; ++i) {
        ...
        if (index < 0 || index >= num_params) {
            ...
            std::copy(params_values + index * slice_size, params_values + index * slice_size + slice_size, output_values);
        }
...
template <typename T>
void Im2Row(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t* __restrict kernel_shape, int32_t stride_y, int32_t stride_x,
    int32_t out_height, int32_t out_width, T* __restrict output_values)
{
            ...
            for (int32_t out_x = 0; out_x < out_width; ++out_x) {
                ...
                for (int32_t k_y = kernel_y_start; k_y < kernel_y_end; ++k_y) {
                    ...
                    // Valid values.
                    {
                        ...
                        const int32_t num_coeffs = (kernel_x_end - kernel_x_start) * in_depth;
#if USE_TYPED_MEMSETMEMCPY
                        const int32_t offset = batch * batch_stride + in_y * y_stride + in_x * x_stride;
                        std::copy(input_values + offset, input_values + offset + num_coeffs, output_values);
#else
...
template <typename T>
void Im2RowVALID(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t* __restrict kernel_shape, int32_t stride_y,
    int32_t stride_x, T* __restrict output_values)
{
    const auto out_size = GetConvOutputSizeVALID(input_shape, kernel_shape, stride_y, stride_x);
    Im2Row<T>(input_shape, input_values, kernel_shape, stride_y, stride_x, std::get<0>(out_size), std::get<1>(out_size), output_values);
}

template <typename T>
void Im2RowSAME(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t* __restrict kernel_shape, int32_t stride_y,
    int32_t stride_x, T* __restrict output_values)
{
    const auto out_size = GetConvOutputSizeSAME(input_shape, stride_y, stride_x);
    Im2Row<T>(input_shape, input_values, kernel_shape, stride_y, stride_x, std::get<0>(out_size), std::get<1>(out_size), output_values);
...
#define POOL_OP(OP_NAME, DEFAULT_VALUE, UPDATE_EXPR, RESULT_EXPR)                                                                                              \
    template <typename T>                                                                                                                                      \
    void OP_NAME##Pool(const int32_t* __restrict input_shape, const T* __restrict input_values, int32_t stride_y, int32_t stride_x, int32_t kernel_height,     \
        int32_t kernel_width, int32_t out_height, int32_t out_width, T* __restrict output_values)                                                              \
    {                                                                                                                                                          \
        ...
        for (int32_t batch = 0; batch < batch_size; ++batch) {                                                                                                 \
            for (int32_t out_y = 0; out_y < out_height; ++out_y) {                                                                                             \
                for (int32_t out_x = 0; out_x < out_width; ++out_x) {                                                                                          \
                    ...
                    const int32_t kernel_x_start = std::max(static_cast<int32_t>(0), -in_x_origin);                                                            \
                    const int32_t kernel_x_end = std::min(kernel_width, in_width - in_x_origin);                                                               \
                    const int32_t count = (kernel_y_end - kernel_y_start) * (kernel_x_end - kernel_x_start);                                                   \
                    (void)sizeof(count);                                                                                                                       \
                                                                                                                                                               \
                    for (int32_t chan = 0; chan < depth; ++chan, ++out_write_ptr) {                                                                            \
                        ...
                        for (int32_t k_y = kernel_y_start; k_y < kernel_y_end; ++k_y) {                                                                        \
                            ...
                            for (int32_t k_x = kernel_x_start; k_x < kernel_x_end; ++k_x) {                                                                    \
                                const int32_t in_x = in_x_origin + k_x;                                                                                        \
                                assert(in_x >= 0 && in_x < in_width);                                                                                          \
                                const T next = input_values[batch * in_strides[0] + in_y * in_strides[1] + in_x * in_strides[2] + chan];                       \
                                value = UPDATE_EXPR;                                                                                                           \
                            }                                                                                                                                  \
                        }                                                                                                                                      \
                        *out_write_ptr = RESULT_EXPR;                                                                                                          \
                                                                                                                                                               ...
                                                                                                                                                               \
    template <typename T>                                                                                                                                      \
    void OP_NAME##PoolVALID(const int32_t* __restrict input_shape, const T* __restrict input_values, int32_t stride_y, int32_t stride_x,                       \
        int32_t kernel_height, int32_t kernel_width, T* __restrict output_values)                                                                              \
    {                                                                                                                                                          \
        const int32_t kernel_shape[4] = { kernel_height, kernel_width, 1, 1 };                                                                                 \
        const auto out_size = GetConvOutputSizeVALID(input_shape, kernel_shape, stride_y, stride_x);                                                           \
        OP_NAME##Pool<T>(                                                                                                                                      \
            input_shape, input_values, stride_y, stride_x, kernel_height, kernel_width, std::get<0>(out_size), std::get<1>(out_size), output_values);          \
    }                                                                                                                                                          \
                                                                                                                                                               \
    template <typename T>                                                                                                                                      \
    void OP_NAME##PoolSAME(const int32_t* __restrict input_shape, const T* __restrict input_values, int32_t stride_y, int32_t stride_x, int32_t kernel_height, \
...
template <typename T> void Memcpy(const int32_t rank, const int32_t* __restrict input_shape, const T* __restrict input_values, T* __restrict output_values)
{
...
template <typename T>
void Softmax(const int32_t rank, const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t reduce_dim, T* __restrict output_values,
...
template <typename T>
int32_t StridedSliceBegin(int32_t range_mask, const T* __restrict range_values, const T* __restrict strides, const int32_t* __restrict input_shape, int32_t dim)
{
...
template <typename T>
int32_t StridedSliceEnd(int32_t range_mask, const T* __restrict range_values, const T* __restrict strides, const int32_t* __restrict input_shape, int32_t dim)
{
    const bool is_explicit = 0 == (range_mask & (1 << dim));
    if (is_explicit) {
        const T range_value = range_values[dim];
...
template <typename T, typename TIdx>
void StridedSlice(const int32_t input_rank, const int32_t* __restrict input_shape, const T* __restrict input_values, const TIdx* __restrict begin,
    const TIdx* __restrict end, const TIdx* __restrict strides, int32_t begin_mask, int32_t end_mask, T* __restrict output_values)
{
    ...
    for (int32_t dim = 0; dim < input_rank; ++dim) {
        ...
        dim_begin[dim] = StridedSliceBegin(begin_mask, begin, strides, input_shape, dim);
        dim_end[dim] = StridedSliceEnd(end_mask, end, strides, input_shape, dim);
        dim_is_full_range[dim] = dim_begin[dim] == 0 && dim_end[dim] == input_shape[dim] && stride == 1;
    ...
    for (int32_t dim = input_rank - 1; dim >= 0 && dim_is_full_range[dim]; --dim) {
...
#if USE_TYPED_MEMSETMEMCPY
        std::copy(input_values + read_offset, input_values + read_offset + block_size, output_values);
#else
        std::memcpy(output_values, input_values + read_offset, block_size * sizeof(T));
#endif
...
template <typename T>
void TransposeRank3(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t* __restrict perm, T* __restrict output_values)
{
    ...
    const int32_t out_strides[3] = { in_strides[perm[0]], in_strides[perm[1]], in_strides[perm[2]] };
    const int32_t out_shape[3] = { input_shape[perm[0]], input_shape[perm[1]], input_shape[perm[2]] };
...
template <typename T>
void TransposeRank4(const int32_t* __restrict input_shape, const T* __restrict input_values, const int32_t* __restrict perm, T* __restrict output_values)
{
    ...
    const int32_t out_strides[4] = { in_strides[perm[0]], in_strides[perm[1]], in_strides[perm[2]], in_strides[perm[3]] };
    const int32_t out_shape[4] = { input_shape[perm[0]], input_shape[perm[1]], input_shape[perm[2]], input_shape[perm[3]] };
...
void OneHot(const int32_t input_rank, const int32_t* __restrict input_shape, const TIdx* __restrict input_values, const TDepth* __restrict depth,
    const T* __restrict on_value, const T* __restrict off_value, const int32_t axis, T* __restrict output_values)
{
    ...
    for (int32_t i = 0; i < prefix_dim_size; ++i) {
        ...
        for (TDepth d = 0; d < *depth; ++d) {
            for (int32_t j = 0; j < suffix_dim_size; ++j, ++write_offset) {
                ...
                output_values[write_offset] = (input_values[read_offset] == d) ? *on_value : *off_value;
            }
        }
    }
}
...
template <typename T, typename TIdx, typename TDepth>
void OneHotLastDim(const int32_t input_rank, const int32_t* __restrict input_shape, const TIdx* __restrict input_values, const TDepth* __restrict depth,
    const T* __restrict on_value, const T* __restrict off_value, T* __restrict output_values)
{
    ...
    for (int32_t i = 0; i < num_elements; ++i) {
        for (TDepth d = 0; d < *depth; ++d, ++write_offset) {
            output_values[write_offset] = (input_values[i] == d) ? *on_value : *off_value;
        }
    }
}
...
#define SIMPLE_UNARY_OP(OP_NAME, _, EXPR_EIGEN)                                                                                                                \
    template <typename T>                                                                                                                                      \
    void OP_NAME(const int32_t rank, const int32_t* __restrict input_shape, const T* __restrict input_values, T* __restrict output_values)                     \
    {                                                                                                                                                          \
...
SIMPLE_UNARY_OP(Elu, value < 0 ? std::expm1(value) : value,
    // Use branchless version of Elu: min(ReLU, e^x - 1)
    values.max(0).min(values.exp() - 1))
SIMPLE_UNARY_OP(HardSigmoid, std::min(std::max((static_cast<T>(0.2) * value + static_cast<T>(0.5)), static_cast<T>(0)), static_cast<T>(1)),
...
SIMPLE_UNARY_OP(Relu6, std::min(std::max(value, static_cast<T>(0)), static_cast<T>(6)), values.max(0).min(6))
SIMPLE_UNARY_OP(Rsqrt, static_cast<T>(1) / std::sqrt(value), values.rsqrt())
SIMPLE_UNARY_OP(Sigmoid, static_cast<T>(1) / (1 + std::exp(-value)), ((-values).exp() + 1).cwiseInverse())
...
void OpNoBroadcast(const int32_t left_rank, const int32_t* __restrict left_shape, const T* __restrict left_values, const int32_t right_rank,
    const int32_t* __restrict right_shape, const T* __restrict right_values, T* __restrict output_values, OP op)
{
...
template <typename T, typename OP>
void OpInnerBroadcast(int32_t left_rank, const int32_t* __restrict left_shape, const T* __restrict left_values, int32_t right_rank,
    const int32_t* __restrict right_shape, const T* __restrict right_values, T* __restrict output_values, OP op)
{
...
// 0].
inline bool IncrementIndices(int32_t rank, const int32_t* shape, int32_t* indices)
{
    int32_t i = rank - 1;
...
inline int32_t Offset(int32_t rank, const int32_t* shape, const int32_t* indices)
{
    int32_t offset = 0;
    int32_t mul = 1;
...
inline int32_t BroadcastOffset(int32_t input_rank, const int32_t* input_shape, int32_t indices_rank, const int32_t* indices)
{
    int32_t offset = 0;
    int32_t mul = 1;
    for (int32_t i = input_rank - 1; i >= 0; --i) {
        int32_t index = input_shape[i] == 1 ? 0 : indices[i + indices_rank - input_rank];
        offset += mul * index;
        mul *= input_shape[i];
    }
    return offset;
...
template <typename T, typename OP>
void OpGenericBroadcast(int32_t left_rank, const int32_t* __restrict left_shape, const T* __restrict left_values, int32_t right_rank,
    const int32_t* __restrict right_shape, const T* __restrict right_values, T* __restrict output_values, OP op)
{
    ...
    for (int32_t i = 0; i < output_rank; ++i) {
        ...
        output_shape[i] = std::max(left_i >= 0 ? left_shape[left_i] : 0, right_i >= 0 ? right_shape[right_i] : 0);
    }

    int32_t output_indices[kMaxRank] {};
    do {
        output_values[Offset(output_rank, output_shape, output_indices)] = op(left_values[BroadcastOffset(left_rank, left_shape, output_rank, output_indices)],
            right_values[BroadcastOffset(right_rank, right_shape, output_rank, output_indices)]);
    } while (IncrementIndices(output_rank, output_shape, output_indices));
}

#define BROADCAST_BINARY_OP(OP_NAME, EXPR, EXPR_EIGEN)                                                                                                         \
    template <typename T> struct Op##OP_NAME {                                                                                                                 \
        const char* name = #OP_NAME;                                                                                                                           \
        T operator()(const T lhs, const T rhs)                                                                                                                 \
        {                                                                                                                                                      \
            return EXPR;                                                                                                                                       \
        }                                                                                                                                                      \
        template <typename X, typename Y, typename Z> void apply(const X& lhs, const Y& rhs, Z out)                                                            \
        {                                                                                                                                                      \
            out = EXPR_EIGEN;                                                                                                                                  \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
    template <typename T>                                                                                                                                      \
    void OP_NAME##NoBroadcast(const int32_t left_rank, const int32_t* __restrict left_shape, const T* __restrict left_values, const int32_t right_rank,        \
        const int32_t* __restrict right_shape, const T* __restrict right_values, T* __restrict output_values)                                                  \
    {                                                                                                                                                          \
        OpNoBroadcast(left_rank, left_shape, left_values, right_rank, right_shape, right_values, output_values, Op##OP_NAME<T>());                             \
    }                                                                                                                                                          \
    template <typename T>                                                                                                                                      \
    void OP_NAME##InnerBroadcast(const int32_t left_rank, const int32_t* __restrict left_shape, const T* __restrict left_values, const int32_t right_rank,     \
        const int32_t* __restrict right_shape, const T* __restrict right_values, T* __restrict output_values)                                                  \
    {                                                                                                                                                          \
        OpInnerBroadcast(left_rank, left_shape, left_values, right_rank, right_shape, right_values, output_values, Op##OP_NAME<T>());                          \
    }                                                                                                                                                          \
    template <typename T>                                                                                                                                      \
    void OP_NAME(const int32_t left_rank, const int32_t* __restrict left_shape, const T* __restrict left_values, const int32_t right_rank,                     \
        const int32_t* __restrict right_shape, const T* __restrict right_values, T* __restrict output_values)                                                  \
    {                                                                                                                                                          \
        OpGenericBroadcast(left_rank, left_shape, left_values, right_rank, right_shape, right_values, output_values, Op##OP_NAME<T>());                        \
    }
...
BROADCAST_BINARY_OP(SquaredDifference, (lhs - rhs) * (lhs - rhs), (lhs - rhs).square())
...
#define REDUCE_OP(OP_NAME, DEFAULT_VALUE, UPDATE_EXPR, RESULT_EXPR)                                                                                            \
    template <typename T, typename Tidx>                                                                                                                       \
    void OP_NAME##InnerReduce(int32_t input_rank, const int32_t* __restrict input_shape, const T* __restrict input_values, int32_t index_tensor_rank,          \
        const int32_t* __restrict index_shape, const Tidx* __restrict index_values, T* __restrict output_values)                                               \
    {                                                                                                                                                          \
        BENCHMARK_TIMER(#OP_NAME, "InnerReduce");                                                                                                              \
        const int32_t inner_size = GetReduceInnerSize(input_rank, input_shape, index_tensor_rank, index_shape, index_values);                                  \
    ...
    template <typename T, typename Tidx>                                                                                                                       \
    void OP_NAME##GenericReduceRank2(int32_t input_rank, const int32_t* __restrict input_shape, const T* __restrict input_values, int32_t index_tensor_rank,   \
        const int32_t* __restrict index_shape, const Tidx* __restrict index_values, T* __restrict output_values)                                               \
    {                                                                                                                                                          \
        assert(input_rank == 2);                                                                                                                               \
        assert(index_tensor_rank <= 1);                                                                                                                        \
    ...
    template <typename T, typename Tidx>                                                                                                                       \
    void OP_NAME##GenericReduceRank3(int32_t input_rank, const int32_t* __restrict input_shape, const T* __restrict input_values, int32_t index_tensor_rank,   \
        const int32_t* __restrict index_shape, const Tidx* __restrict index_values, T* __restrict output_values)                                               \
    {                                                                                                                                                          \
        assert(input_rank == 3);                                                                                                                               \
        assert(index_tensor_rank <= 1);                                                                                                                        \
        ...
        for (int32_t dim0 = 0; dim0 < input_shape[0]; ++dim0) {                                                                                                \
            for (int32_t dim1 = 0; dim1 < input_shape[1]; ++dim1) {                                                                                            \
                for (int32_t dim2 = 0; dim2 < input_shape[2]; ++dim2, ++input_values) {                                                                        \
                    T* out_ptr = output_values + out_strides[0] * dim0 + out_strides[1] * dim1 + out_strides[2] * dim2;                                        \
                    const T prev = *out_ptr;                                                                                                                   \
                    const T next = *input_values;                                                                                                              \
                    *out_ptr = UPDATE_EXPR;                                                                                                                    \
                }                                                                                                                                              \
        ...
        const T count = (reduce_mask[0] ? input_shape[0] : 1) * (reduce_mask[1] ? input_shape[1] : 1) * (reduce_mask[2] ? input_shape[2] : 1);                 \
        /* Used by mean reduce. */                                                                                                                             \
        (void)sizeof(count);                                                                                                                                   \
        for (int32_t i = 0; i < output_size; ++i) {                                                                                                            \
            const T value = output_values[i];                                                                                                                  \
    ...
    template <typename T, typename Tidx>                                                                                                                       \
    void OP_NAME##GenericReduceRank4(int32_t input_rank, const int32_t* __restrict input_shape, const T* __restrict input_values, int32_t index_tensor_rank,   \
        const int32_t* __restrict index_shape, const Tidx* __restrict index_values, T* __restrict output_values)                                               \
    {                                                                                                                                                          \
        assert(input_rank == 4);                                                                                                                               \
        assert(index_tensor_rank <= 1);                                                                                                                        \
        BENCHMARK_TIMER(#OP_NAME, "GenericReduceRank4");                                                                                                       \
        int32_t out_shape[4] = { input_shape[0], input_shape[1], input_shape[2], input_shape[3] };                                                             \
        ...
        for (int32_t dim0 = 0; dim0 < input_shape[0]; ++dim0) {                                                                                                \
            for (int32_t dim1 = 0; dim1 < input_shape[1]; ++dim1) {                                                                                            \
                for (int32_t dim2 = 0; dim2 < input_shape[2]; ++dim2) {                                                                                        \
                    for (int32_t dim3 = 0; dim3 < input_shape[3]; ++dim3, ++input_values) {                                                                    \
                        T* out_ptr = output_values + out_strides[0] * dim0 + out_strides[1] * dim1 + out_strides[2] * dim2 + out_strides[3] * dim3;            \
                        const T prev = *out_ptr;                                                                                                               \
                        const T next = *input_values;                                                                                                          \
                        *out_ptr = UPDATE_EXPR;                                                                                                                \
                    }                                                                                                                                          \
    ...
    template <typename T, typename Tidx>                                                                                                                       \
    void OP_NAME##GenericReduceRank5(int32_t input_rank, const int32_t* __restrict input_shape, const T* __restrict input_values, int32_t index_tensor_rank,   \
        const int32_t* __restrict index_shape, const Tidx* __restrict index_values, T* __restrict output_values)                                               \
    {                                                                                                                                                          \
        assert(input_rank == 5);                                                                                                                               \
        assert(index_tensor_rank <= 1);                                                                                                                        \
        BENCHMARK_TIMER(#OP_NAME, "GenericReduceRank5");                                                                                                       \
        int32_t out_shape[5] = { input_shape[0], input_shape[1], input_shape[2], input_shape[3], input_shape[4] };                                             \
        ...
        const int32_t out_strides[5] = {                                                                                                                       \
            reduce_mask[0] ? 0 : out_shape[1] * out_shape[2] * out_shape[3] * out_shape[4],                                                                    \
            ...
            for (int32_t dim1 = 0; dim1 < input_shape[1]; ++dim1) {                                                                                            \
                for (int32_t dim2 = 0; dim2 < input_shape[2]; ++dim2) {                                                                                        \
                    for (int32_t dim3 = 0; dim3 < input_shape[3]; ++dim3) {                                                                                    \
                        for (int32_t dim4 = 0; dim4 < input_shape[4]; ++dim4, ++input_values) {                                                                \
                            T* out_ptr = output_values + out_strides[0] * dim0 + out_strides[1] * dim1 + out_strides[2] * dim2 + out_strides[3] * dim3         \
                                + out_strides[4] * dim4;                                                                                                       \
                            const T prev = *out_ptr;                                                                                                           \
                            const T next = *input_values;                                                                                                      \
        ...
        const T count = (reduce_mask[0] ? input_shape[0] : 1) * (reduce_mask[1] ? input_shape[1] : 1) * (reduce_mask[2] ? input_shape[2] : 1)                  \
            * (reduce_mask[3] ? input_shape[3] : 1) * (reduce_mask[4] ? input_shape[4] : 1);                                                                   \
        /* Used by mean reduce. */                                                                                                                             \
        (void)sizeof(count);                                                                                                                                   \
        for (int32_t i = 0; i < output_size; ++i) {                                                                                                            \
            const T value = output_values[i];                                                                                                                  \
...
template <typename T>
void DequantizeMinCombined(const int32_t rank, const int32_t* __restrict input_shape, const T* __restrict input_values, const float* __restrict min_range,
    const float* __restrict max_range, float* __restrict output_values)
{
    BENCHMARK_TIMER("DequantizeMinCombined");
    const int32_t size = ShapeSize(rank, input_shape);
    const float offset = std::is_signed<T>::value ? (static_cast<float>(std::numeric_limits<T>::max()) - std::numeric_limits<T>::min() + 1) / 2.0f : 0.0f;
    const float range_scale = (max_range[0] - min_range[0]) / (static_cast<float>(std::numeric_limits<T>::max()) - std::numeric_limits<T>::min());
    for (int32_t i = 0; i < size; ++i) {
        output_values[i] = ((static_cast<int32_t>(input_values[i]) + offset) * range_scale) + min_range[0];
    }
}

template <typename T>
void DequantizeMinFirst(const int32_t rank, const int32_t* __restrict input_shape, const T* __restrict input_values, const float* __restrict min_range,
    const float* __restrict max_range, float* __restrict output_values)
{
    BENCHMARK_TIMER("DequantizeMinFirst");
    const int32_t size = ShapeSize(rank, input_shape);
    const float range_scale = (max_range[0] - min_range[0]) / (static_cast<float>(std::numeric_limits<T>::max()) - std::numeric_limits<T>::min());
    const float range_min_rounded = (max_range[0] == min_range[0] ? min_range[0] : std::round(min_range[0] / range_scale) * range_scale);
    for (int32_t i = 0; i < size; ++i) {
        output_values[i] = ((static_cast<int32_t>(input_values[i]) - std::numeric_limits<T>::min()) * range_scale) + range_min_rounded;
    }
}
...
template <typename T>
void AddN(const int32_t rank, const int32_t* __restrict shape, std::initializer_list<const T* __restrict> input_values, T* __restrict output_values)
{
...
int32_t input_from_feature_columns_input_layer_concat_concat0Shape[2] = { 1, 325 };
int32_t logits_MatMul_merged_with_dnn_logits_BiasAdd0Shape[2] = { 1, 1 };

void Inference(const float* __restrict input_from_feature_columns_input_layer_concat_concat0 /* shape: 1,325 */
    ,
...
{
    const int32_t input_from_feature_columns_input_layer_concat_concat0_shape[] = { 1, 325 };
    ...
    FullyConnected<float>(input_from_feature_columns_input_layer_concat_concat0_shape, input_from_feature_columns_input_layer_concat_concat0,
        dnn_hiddenlayer_0_kernel__1__cf__1_shape, dnn_hiddenlayer_0_kernel__1__cf__1.values, dnn_hiddenlayer_0_bias__0__cf__0_shape,
    ...
    FullyConnected<float>(fixed->shape0, fixed->alloc0, dnn_logits_kernel__3__cf__3_shape, dnn_logits_kernel__3__cf__3.values, dnn_logits_bias__2__cf__2_shape,
        dnn_logits_bias__2__cf__2.values, logits_MatMul_merged_with_dnn_logits_BiasAdd0);
