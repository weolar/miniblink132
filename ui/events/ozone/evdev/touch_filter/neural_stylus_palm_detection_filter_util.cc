...
namespace {
float ScaledRadius(float radius, const NeuralStylusPalmDetectionFilterModelConfig& model_config)
{
    ...
    for (uint32_t i = 0; i < model_config.radius_polynomial_resize.size(); ++i) {
        ...
        return_value += model_config.radius_polynomial_resize[i] * powf(radius, power);
    }
    return return_value;
}
 ...
 */
PalmFilterSample GetSampleAtTime(base::TimeTicks time, const PalmFilterSample& before, const PalmFilterSample& after)
{
    ...
    const float proportion = static_cast<float>((time - before.time).InNanoseconds()) / (after.time - before.time).InNanoseconds();
    result.edge = interpolate(before.edge, after.edge, proportion);
    result.point.set_x(interpolate(before.point.x(), after.point.x(), proportion));
    result.point.set_y(interpolate(before.point.y(), after.point.y(), proportion));
    result.time = time;
    return result;
}
} // namespace

PalmFilterSample CreatePalmFilterSample(const InProgressTouchEvdev& touch, const base::TimeTicks& time,
    const NeuralStylusPalmDetectionFilterModelConfig& model_config, const PalmFilterDeviceInfo& dev_info)
{
    ...
    sample.major_radius = ScaledRadius(std::max(touch.major, touch.minor) / dev_info.major_radius_res, model_config);
    if (dev_info.minor_radius_supported) {
        sample.minor_radius = ScaledRadius(std::min(touch.major, touch.minor) / dev_info.minor_radius_res, model_config);
    } else {
    ...
    sample.point = gfx::PointF(touch.x / dev_info.x_res, touch.y / dev_info.y_res);
...
PalmFilterStroke::PalmFilterStroke(const NeuralStylusPalmDetectionFilterModelConfig& model_config, int tracking_id)
...
void PalmFilterStroke::ProcessSample(const PalmFilterSample& sample)
{
    ...
    if (resample_period_.has_value()) {
        // Prune based on time
        const base::TimeDelta max_duration = (*resample_period_) * (max_sample_count_ - 1);
        while (samples_.size() > 2 && samples_.back().time - samples_[1].time >= max_duration) {
...
void PalmFilterStroke::AddToUnscaledCentroid(const gfx::Vector2dF point)
{
    ...
    const gfx::PointF new_unscaled_centroid = unscaled_centroid_ + corrected_point;
    unscaled_centroid_sum_error_ = (new_unscaled_centroid - unscaled_centroid_) - corrected_point;
...
PalmFilterSample PalmFilterStroke::GetSampleAt(base::TimeTicks time) const
{
    ...
    if (i == 0 || i == samples_.size()) {
        LOG(DFATAL) << "Invalid index: " << i << ", can't interpolate for time: " << time;
        return {};
    }
