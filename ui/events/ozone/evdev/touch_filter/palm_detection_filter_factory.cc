...
{
    std::vector<std::string> split_radii = base::SplitString(radius_string, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    for (const std::string& unparsed : split_radii) {
        ...
        if (!base::StringToDouble(unparsed, &item) && !std::isnan(item)) {
            LOG(DFATAL) << "Unable to parse " << unparsed << " to a floating point; Returning empty.";
            return {};
        }
...
std::string FetchNeuralPalmRadiusPolynomial(const EventDeviceInfo& devinfo, const std::string param_string)
{
    ...
    std::optional<base::Value> ozone_switch_value = base::JSONReader::Read(base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(kOzoneNNPalmSwitchName));
    if (ozone_switch_value.has_value() && ozone_switch_value->is_dict()) {
        std::string* switch_string_value = ozone_switch_value->GetDict().FindString(kOzoneNNPalmRadiusPolynomialProperty);
...
std::string FetchNeuralPalmModelVersion(const EventDeviceInfo& devinfo, const std::string param_string)
{
    if (!param_string.empty()) {
        return param_string;
    }
    ...
    std::optional<base::Value> ozone_switch_value = base::JSONReader::Read(base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(kOzoneNNPalmSwitchName));
    if (ozone_switch_value.has_value() && ozone_switch_value->is_dict()) {
        std::string* switch_string_value = ozone_switch_value->GetDict().FindString(kOzoneNNPalmModelVersionProperty);
        if (switch_string_value != nullptr) {
            return *switch_string_value;
        }
    }
...
std::unique_ptr<PalmDetectionFilter> CreatePalmDetectionFilter(const EventDeviceInfo& devinfo, SharedPalmDetectionFilterState* shared_palm_state)
{
    if (base::FeatureList::IsEnabled(kEnableNeuralPalmDetectionFilter)
        ...
        std::string polynomial_string = internal::FetchNeuralPalmRadiusPolynomial(devinfo, kNeuralPalmRadiusPolynomial.Get());
        VLOG(1) << "Will attempt to use radius polynomial: " << polynomial_string;
        std::vector<float> radius_polynomial = internal::ParseRadiusPolynomial(polynomial_string);
        std::string model_version = internal::FetchNeuralPalmModelVersion(devinfo, kNeuralPalmModelVersion.Get());
        // There's only one model right now.
        std::unique_ptr<NeuralStylusPalmDetectionFilterModel> model
            = std::make_unique<OneDeviceTrainNeuralStylusPalmDetectionFilterModel>(model_version, radius_polynomial);
    ...
    if (base::FeatureList::IsEnabled(kEnableHeuristicPalmDetectionFilter)
        && HeuristicStylusPalmDetectionFilter::CompatibleWithHeuristicStylusPalmDetectionFilter(devinfo)) {
        const base::TimeDelta hold_time = base::Seconds(kHeuristicHoldThresholdSeconds.Get());
        const base::TimeDelta cancel_time = base::Seconds(kHeuristicCancelThresholdSeconds.Get());
        const int stroke_count = kHeuristicStrokeCount.Get();
        return std::make_unique<HeuristicStylusPalmDetectionFilter>(shared_palm_state, stroke_count, hold_time, cancel_time);
    }
