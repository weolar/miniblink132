...
public:
    ...
    OpenPalmDetectionFilterTest& operator=(const OpenPalmDetectionFilterTest&) = delete;

    void SetUp() override
    {
        shared_palm_state = std::make_unique<SharedPalmDetectionFilterState>();
        palm_detection_filter_ = std::make_unique<OpenPalmDetectionFilter>(shared_palm_state.get());
    }
...
TEST_F(OpenPalmDetectionFilterTest, TestSetsToZero)
{
    ...
    palm_detection_filter_->Filter(inputs, base::TimeTicks::Now(), &hold, &suppress);
