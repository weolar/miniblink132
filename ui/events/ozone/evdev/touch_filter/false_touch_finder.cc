...
std::unique_ptr<FalseTouchFinder> FalseTouchFinder::Create(gfx::Size touchscreen_size)
{
    bool edge_filtering = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEdgeTouchFiltering);
    if (edge_filtering) {
        return base::WrapUnique(new FalseTouchFinder(edge_filtering, touchscreen_size));
    }
    return nullptr;
}

void FalseTouchFinder::HandleTouches(const std::vector<InProgressTouchEvdev>& touches, base::TimeTicks time)
{
...
FalseTouchFinder::FalseTouchFinder(bool edge_filtering, gfx::Size touchscreen_size)
