...
bool IsEarlyStageSample(const PalmFilterStroke& stroke, const NeuralStylusPalmDetectionFilterModelConfig& config)
{
    if (!config.resample_period) {
        return config.early_stage_sample_counts.find(stroke.samples_seen()) != config.early_stage_sample_counts.end();
    }
    ...
    if (stroke.samples().size() == 1) {
        return config.early_stage_sample_counts.find(1) != config.early_stage_sample_counts.end();
    }
...
bool HasDecidedStroke(const PalmFilterStroke& stroke, const NeuralStylusPalmDetectionFilterModelConfig& config)
{
    if (!config.resample_period) {
        return stroke.samples_seen() >= config.max_sample_count;
    }
    const base::TimeDelta max_duration = config.GetEquivalentDuration(config.max_sample_count);
    return stroke.Duration() >= max_duration;
}

bool IsVeryShortStroke(const PalmFilterStroke& stroke, const NeuralStylusPalmDetectionFilterModelConfig& config)
{
    if (!config.resample_period) {
        return stroke.samples_seen() < config.min_sample_count;
    }
    return stroke.Duration() < config.GetEquivalentDuration(config.min_sample_count);
}
...
bool HasInsufficientDataAsNeighbor(
    const PalmFilterStroke& neighbor_stroke, size_t neighbor_min_sample_count, const NeuralStylusPalmDetectionFilterModelConfig& config)
{
    if (!config.resample_period) {
        return neighbor_stroke.samples().size() < neighbor_min_sample_count;
    }
    return neighbor_stroke.Duration() < config.GetEquivalentDuration(neighbor_min_sample_count);
}

} // namespace

NeuralStylusPalmDetectionFilter::NeuralStylusPalmDetectionFilter(
    const EventDeviceInfo& devinfo, std::unique_ptr<NeuralStylusPalmDetectionFilterModel> palm_model, SharedPalmDetectionFilterState* shared_palm_state)
...
{
    DCHECK(CompatibleWithNeuralStylusPalmDetectionFilter(devinfo)) << "One should run compatible check before instantiation.";
}
...
void NeuralStylusPalmDetectionFilter::FindBiggestNeighborsWithin(int neighbor_count, unsigned long neighbor_min_sample_count, float max_distance,
    const PalmFilterStroke& stroke, std::vector<std::pair<float, int>>* biggest_strokes) const
{
    if (neighbor_count <= 0) {
        return;
    }
    ...
    for (const auto& lookup : strokes_) {
        ...
        if (HasInsufficientDataAsNeighbor(neighbor, neighbor_min_sample_count, model_->config())) {
            continue;
        }
        float distance = EuclideanDistance(neighbor.GetCentroid(), stroke.GetCentroid());
        if (distance > max_distance) {
            continue;
        }
        float size = neighbor.BiggestSize();
        biggest_strokes_queue.push(std::make_tuple(size, distance, neighbor.tracking_id()));
    }
    for (int i = 0; i < neighbor_count && !biggest_strokes_queue.empty(); ++i) {
        ...
        biggest_strokes->emplace_back(std::get<1>(big_stroke), std::get<2>(big_stroke));
    }
}

void NeuralStylusPalmDetectionFilter::FindNearestNeighborsWithin(int neighbor_count, unsigned long neighbor_min_sample_count, float max_distance,
    const PalmFilterStroke& stroke, std::vector<std::pair<float, int>>* nearest_strokes) const
{
    using StrokeId = int;
    using Distance = float;
    using DistanceWithStrokeId = std::pair<Distance, StrokeId>;
    std::priority_queue<DistanceWithStrokeId, std::vector<DistanceWithStrokeId>, std::greater<DistanceWithStrokeId>> queue;
    if (neighbor_count <= 0) {
        return;
    }
    for (const auto& lookup : strokes_) {
...
void NeuralStylusPalmDetectionFilter::Filter(const std::vector<InProgressTouchEvdev>& touches, base::TimeTicks time,
    std::bitset<kNumTouchEvdevSlots>* slots_to_hold, std::bitset<kNumTouchEvdevSlots>* slots_to_suppress)
{
    ...
    for (const auto& touch : touches) {
        ...
        if (!touch.was_touching) {
            ...
            DCHECK(strokes_.count(tracking_id) == 0) << " Tracking id " << tracking_id;

            strokes_.emplace(tracking_id, PalmFilterStroke(model_->config(), tracking_id));
        ...
        stroke.ProcessSample(CreatePalmFilterSample(touch, time, model_->config(), palm_filter_dev_info_));
        ...
        if (config.heuristic_delay_start_if_palm && !end_of_stroke && !HasDecidedStroke(stroke, config) && IsHeuristicPalmStroke(stroke)) {
        ...
        if (!is_delay_.test(slot) && config.nn_delay_start_if_palm && IsEarlyStageSample(stroke, config)) {
            VLOG(1) << "About to run a early_stage prediction.";
            if (DetectSpuriousStroke(ExtractFeatures(tracking_id), model_->config().output_threshold)) {
    ...
    for (const int slot : slots_to_decide) {
        ...
        is_palm_.set(slot, DetectSpuriousStroke(ExtractFeatures(tracking_id), model_->config().output_threshold));
    }
...
bool NeuralStylusPalmDetectionFilter::ShouldDecideStroke(const PalmFilterStroke& stroke) const
{
    const NeuralStylusPalmDetectionFilterModelConfig& config = model_->config();
    ...
    return stroke.LastSampleCrossed(config.GetEquivalentDuration(config.max_sample_count));
}

bool NeuralStylusPalmDetectionFilter::IsHeuristicPalmStroke(const PalmFilterStroke& stroke) const
{
    const auto& config = model_->config();
    if (config.resample_period) {
        if (stroke.Duration() > config.GetEquivalentDuration(config.max_sample_count)) {
            LOG(DFATAL) << "Should not call this method on long strokes. Got duration = " << stroke.Duration();
            return false;
        }
    } else {
    ...
    if (config.heuristic_palm_area_limit > 0.0) {
        ...
        FindBiggestNeighborsWithin(1 /* neighbors */, 1 /* neighbor min sample count */, config.max_neighbor_distance_in_mm, stroke, &biggest_strokes);
        if (!biggest_strokes.empty() && strokes_.find(biggest_strokes[0].second)->second.BiggestSize() >= config.heuristic_palm_area_limit) {
...
bool NeuralStylusPalmDetectionFilter::DetectSpuriousStroke(const std::vector<float>& features, float threshold) const
{
...
std::vector<float> NeuralStylusPalmDetectionFilter::ExtractFeatures(int tracking_id) const
{
    ...
    const NeuralStylusPalmDetectionFilterModelConfig& config = model_->config();
    FindNearestNeighborsWithin(config.nearest_neighbor_count, config.neighbor_min_sample_count, config.max_neighbor_distance_in_mm, stroke, &nearest_strokes);
    FindBiggestNeighborsWithin(
        config.biggest_near_neighbor_count, config.neighbor_min_sample_count, config.max_neighbor_distance_in_mm, stroke, &biggest_strokes);
    for (uint32_t i = 0; i < config.nearest_neighbor_count; ++i) {
        if (i < nearest_strokes.size()) {
            const auto& nearest_stroke = nearest_strokes[i];
            AppendFeaturesAsNeighbor(strokes_.at(nearest_stroke.second), nearest_stroke.first, &features);
        } else {
            features.resize(features.size() + features_per_stroke + kExtraFeaturesForNeighbor, 0);
        }
    }
    ...
    for (uint32_t i = 0; i < config.biggest_near_neighbor_count; ++i) {
        if (i < biggest_strokes.size()) {
            ...
            AppendFeaturesAsNeighbor(strokes_.at(biggest_stroke.second), biggest_stroke.first, &features);
        } else {
            features.resize(features.size() + features_per_stroke + kExtraFeaturesForNeighbor, 0);
        }
    }
...
void NeuralStylusPalmDetectionFilter::AppendFeatures(const PalmFilterStroke& stroke, std::vector<float>* features) const
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        features->push_back(sample.minor_radius <= 0.0 ? sample.major_radius : sample.minor_radius);
        float distance = 0;
        if (i != 0) {
    ...
    // "fill proportion."
    features->push_back(static_cast<float>(size) / model_->config().max_sample_count);
    features->push_back(EuclideanDistance(stroke.samples().front().point, stroke.samples().back().point));
 ...
 * samples.
 */
void NeuralStylusPalmDetectionFilter::AppendResampledFeatures(const PalmFilterStroke& stroke, std::vector<float>* features) const
{
    ...
    const base::TimeDelta max_duration = model_->config().GetEquivalentDuration(model_->config().max_sample_count);
    for (auto time = first_time; (time - first_time) <= max_duration && time <= stroke.samples().back().time; time += resample_period) {
        sample_count++;
        ...
        float distance = 0;
        if (time != first_time) {
            distance = EuclideanDistance(stroke.GetSampleAt(time - resample_period).point, sample.point);
        }
        features->push_back(distance);
        features->push_back(sample.edge);
        features->push_back(1.0); // existence.
    ...
    // "fill proportion."
    features->push_back(static_cast<float>(sample_count) / model_->config().max_sample_count);
    features->push_back(EuclideanDistance(stroke.samples().front().point, stroke.samples().back().point));

    // Start sequence number. 0 is min.
    uint32_t samples_seen = (stroke.Duration() / (*model_->config().resample_period)) + 1;
    if (samples_seen < model_->config().max_sample_count) {
        features->push_back(0);
    } else {
        features->push_back(samples_seen - model_->config().max_sample_count);
...
void NeuralStylusPalmDetectionFilter::AppendFeaturesAsNeighbor(const PalmFilterStroke& stroke, float distance, std::vector<float>* features) const
{
...
const char NeuralStylusPalmDetectionFilter::kFilterName[] = "NeuralStylusPalmDetectionFilter";
std::string NeuralStylusPalmDetectionFilter::FilterNameForTesting() const
{
    return kFilterName;
}

bool NeuralStylusPalmDetectionFilter::CompatibleWithNeuralStylusPalmDetectionFilter(const EventDeviceInfo& devinfo)
{
    return NeuralStylusPalmDetectionFilter::CompatibleWithNeuralStylusPalmDetectionFilter(
        devinfo, base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(kOzoneNNPalmSwitchName));
}

bool NeuralStylusPalmDetectionFilter::CompatibleWithNeuralStylusPalmDetectionFilter(
    const EventDeviceInfo& devinfo, const std::string& ozone_params_switch_string)
{
    ...
    static constexpr int kRequiredAbsMtCodes[] = { ABS_MT_POSITION_X, ABS_MT_POSITION_Y, ABS_MT_TOUCH_MAJOR };
    ...
    // is present.
    if (devinfo.HasAbsEvent(ABS_MT_TOUCH_MINOR) && !code_check(ABS_MT_TOUCH_MINOR)) {
        return false;
    }
    ...
    std::optional<base::Value> value = base::JSONReader::Read(ozone_params_switch_string);
    if (value != std::nullopt && !ozone_params_switch_string.empty()) {
        ...
        std::string* touch_string_val = value_dict->FindString(kOzoneNNPalmTouchCompatibleProperty);
...
void NeuralStylusPalmDetectionFilter::EraseOldStrokes(base::TimeTicks time)
{
    ...
    for (auto it = strokes_.begin(); it != strokes_.end();) {
        ...
        const base::TimeTicks most_recent_sample_time = it->second.samples().back().time;
