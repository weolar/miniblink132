...
void HeuristicStylusPalmDetectionFilter::Filter(const std::vector<InProgressTouchEvdev>& touches, base::TimeTicks time,
    std::bitset<kNumTouchEvdevSlots>* slots_to_hold, std::bitset<kNumTouchEvdevSlots>* slots_to_suppress)
{
    slots_to_hold->reset();
    slots_to_suppress->reset();
    const size_t events_size = std::min(touches.size(), static_cast<size_t>(kNumTouchEvdevSlots));
    DCHECK_LE(touches.size(), static_cast<size_t>(kNumTouchEvdevSlots)) << "heuristic filtering only expected to work on devices with "
    ...
    for (size_t i = 0; i < events_size; ++i) {
        ...
        stroke_length_[i]++;
        base::TimeDelta time_since_stylus_for_touch_start = touch_started_time_[i] - shared_palm_state_->latest_stylus_touch_time;
        if (time_since_stylus_for_touch_start < time_after_stylus_to_cancel_) {
            slots_to_suppress->set(i, true);
        } else if (time_since_stylus_for_touch_start < time_after_stylus_to_hold_ && stroke_length_[i] <= hold_stroke_count_) {
...
HeuristicStylusPalmDetectionFilter::HeuristicStylusPalmDetectionFilter(
    SharedPalmDetectionFilterState* shared_palm_state, int hold_stroke_count, base::TimeDelta hold, base::TimeDelta cancel)
...
const char HeuristicStylusPalmDetectionFilter::kFilterName[] = "HeuristicStylusPalmDetectionFilter";
...
bool HeuristicStylusPalmDetectionFilter::CompatibleWithHeuristicStylusPalmDetectionFilter(const EventDeviceInfo& device_info)
{
