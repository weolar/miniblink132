...
public:
    ...
    HeuristicStylusPalmDetectionFilterTest(const HeuristicStylusPalmDetectionFilterTest&) = delete;
    HeuristicStylusPalmDetectionFilterTest& operator=(const HeuristicStylusPalmDetectionFilterTest&) = delete;

    void SetUp() override
    {
        shared_palm_state = std::make_unique<SharedPalmDetectionFilterState>();
        palm_detection_filter_ = std::make_unique<HeuristicStylusPalmDetectionFilter>(shared_palm_state.get(), hold_sample_count, hold_time, suppress_time);
...
class HeuristicStylusPalmDetectionFilterDeathTest : public HeuristicStylusPalmDetectionFilterTest { };
...
TEST_F(HeuristicStylusPalmDetectionFilterDeathTest, TestDCheck)
{
    ...
    EXPECT_DCHECK_DEATH(palm_detection_filter_
        = std::make_unique<HeuristicStylusPalmDetectionFilter>(shared_palm_state.get(), hold_sample_count, hold_time, hold_time + base::Milliseconds(0.1)));
}
...
TEST_F(HeuristicStylusPalmDetectionFilterTest, TestCancelAfterStylus)
{
    ...
    for (; start_time < test_start_time_ + base::Milliseconds(1000); start_time += sample_interval) {
        palm_detection_filter_->Filter(touches_, start_time, &hold, &suppress);
        EXPECT_TRUE(hold.none());
        EXPECT_TRUE(suppress.test(0));
        EXPECT_TRUE(suppress.test(1));
...
TEST_F(HeuristicStylusPalmDetectionFilterTest, TestHoldAfterStylus)
{
    ...
    touches_[0].tool_code = 0;
    touches_[1].touching = true;
    base::TimeTicks start_time = test_start_time_ + hold_time - (hold_sample_count - 2) * sample_interval;
    palm_detection_filter_->Filter(touches_, start_time, &hold, &suppress);
    EXPECT_TRUE(suppress.none());
...
TEST_F(HeuristicStylusPalmDetectionFilterTest, TestNothingLongAfterStylus)
{
    ...
    touches_[0].tool_code = 0;
    touches_[1].touching = true;
    base::TimeTicks start_time = test_start_time_ + hold_time + base::Milliseconds(1e-2);
    palm_detection_filter_->Filter(touches_, start_time, &hold, &suppress);
    EXPECT_TRUE(hold.none());
    EXPECT_TRUE(suppress.none());
}
