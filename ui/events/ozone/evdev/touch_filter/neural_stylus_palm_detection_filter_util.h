...
struct COMPONENT_EXPORT(EVDEV) PalmFilterSample {
    ...
    {
        return major_radius == other.major_radius && minor_radius == other.minor_radius && pressure == other.pressure && edge == other.edge
            && tracking_id == other.tracking_id && point == other.point && time == other.time;
    }
};
...
public:
    explicit PalmFilterStroke(const NeuralStylusPalmDetectionFilterModelConfig& model_config, int tracking_id);
...
private:
   ...
   */
    ...
    gfx::Vector2dF unscaled_centroid_sum_error_ = gfx::PointF(0., 0.).OffsetFromOrigin();
};
