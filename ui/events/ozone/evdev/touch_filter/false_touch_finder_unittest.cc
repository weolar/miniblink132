    ...
    bool FilterAndCheck(const TouchEntry entries[], size_t count)
    {
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            if (i == count - 1 || entry.time_ms != entries[i + 1].time_ms) {
                false_touch_finder_->HandleTouches(touches, base::TimeTicks() + base::Milliseconds(entry.time_ms));
                ...
                for (size_t j = 0; j < touches.size(); ++j) {
                    ...
                    if (false_touch_finder_->SlotShouldDelay(slot) != expect_delay) {
                        LOG(ERROR) << base::StringPrintf("Incorrect filtering at %dms for slot %zu", entry.time_ms, slot);
                        return false;
                    }
                }
...
TEST_F(FalseTouchFinderTest, EdgeTap)
{
    ...
    const TouchEntry kTestData[] = { { 10, 1, true, gfx::PointF(0, 100), 0.35, true }, { 20, 1, true, gfx::PointF(0, 100), 0.35, true },
        { 30, 1, false, gfx::PointF(0, 100), 0.35, true }, { 40, 2, true, gfx::PointF(touchscreen_width - 1, 100), 0.35, true },
        { 50, 2, true, gfx::PointF(touchscreen_width - 1, 100), 0.35, true }, { 60, 2, false, gfx::PointF(touchscreen_width - 1, 100), 0.35, true },
        { 70, 3, true, gfx::PointF(100, 0), 0.35, true }, { 80, 3, true, gfx::PointF(100, 0), 0.35, true }, { 90, 3, false, gfx::PointF(100, 0), 0.35, true },
        { 100, 4, true, gfx::PointF(100, touchscreen_height - 1), 0.35, true }, { 110, 4, true, gfx::PointF(100, touchscreen_height - 1), 0.35, true },
...
TEST_F(FalseTouchFinderTest, MoveFromEdge)
{
    const TouchEntry kTestData[] = { { 10, 1, true, gfx::PointF(0, 100), 0.35, true }, { 20, 1, true, gfx::PointF(0, 100), 0.35, true },
        ...
        { 60, 1, true, gfx::PointF(0, 100), 0.35, true }, { 70, 1, true, gfx::PointF(0, 101), 0.35, false },
...
{
    const TouchEntry kTestData[] = { { 10, 1, true, gfx::PointF(100, 100), 0.35, false }, { 20, 1, true, gfx::PointF(100, 100), 0.35, false },
        { 30, 1, true, gfx::PointF(0, 100), 0.35, false }, { 40, 1, false, gfx::PointF(0, 100), 0.35, false } };
    EXPECT_TRUE(FilterAndCheck(kTestData, std::size(kTestData)));
}
...
{
    const TouchEntry kTestData[] = { { 10, 1, true, gfx::PointF(10, 10), 0.243137, false }, { 20, 1, true, gfx::PointF(10, 10), 0.231373, false },
        { 30, 1, true, gfx::PointF(10, 10), 0.215686, false }, { 40, 1, true, gfx::PointF(10, 10), 0.211765, false },
