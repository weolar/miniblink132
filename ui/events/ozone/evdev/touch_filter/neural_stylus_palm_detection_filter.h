...
class COMPONENT_EXPORT(EVDEV) NeuralStylusPalmDetectionFilter : public PalmDetectionFilter {
public:
    ...
    NeuralStylusPalmDetectionFilter(const NeuralStylusPalmDetectionFilter&) = delete;
    NeuralStylusPalmDetectionFilter& operator=(const NeuralStylusPalmDetectionFilter&) = delete;

    ~NeuralStylusPalmDetectionFilter() override;
    void Filter(const std::vector<InProgressTouchEvdev>& touches, base::TimeTicks time, std::bitset<kNumTouchEvdevSlots>* slots_to_hold,
        std::bitset<kNumTouchEvdevSlots>* slots_to_suppress) override;

    static bool CompatibleWithNeuralStylusPalmDetectionFilter(const EventDeviceInfo& devinfo);

    static bool CompatibleWithNeuralStylusPalmDetectionFilter(const EventDeviceInfo& devinfo, const std::string& ozone_params_switch_string);
...
private:
    void FindNearestNeighborsWithin(int neighbor_count, unsigned long neighbor_min_sample_count, float max_distance, const PalmFilterStroke& stroke,
        std::vector<std::pair<float, int>>* nearest_strokes) const;
    void FindBiggestNeighborsWithin(int neighbor_count, unsigned long neighbor_min_sample_count, float max_distance, const PalmFilterStroke& stroke,
        std::vector<std::pair<float, int>>* biggest_strokes) const;

    bool DetectSpuriousStroke(const std::vector<float>& features, float threshold) const;
    // Extracts the feature vector for the specified stroke.
    std::vector<float> ExtractFeatures(int tracking_id) const;
    void AppendFeatures(const PalmFilterStroke& stroke, std::vector<float>* features) const;
    void AppendResampledFeatures(const PalmFilterStroke& stroke, std::vector<float>* features) const;
    void AppendFeaturesAsNeighbor(const PalmFilterStroke& stroke, float distance, std::vector<float>* features) const;
