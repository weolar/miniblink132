...
class NeuralStylusPalmDetectionFilterUtilTest : public testing::TestWithParam<bool> {
public:
    NeuralStylusPalmDetectionFilterUtilTest() = default;

    NeuralStylusPalmDetectionFilterUtilTest(const NeuralStylusPalmDetectionFilterUtilTest&) = delete;
    NeuralStylusPalmDetectionFilterUtilTest& operator=(const NeuralStylusPalmDetectionFilterUtilTest&) = delete;
...
INSTANTIATE_TEST_SUITE_P(ParametricUtilTest, NeuralStylusPalmDetectionFilterUtilTest, ::testing::Bool(),
    [](const auto& paramInfo) { return paramInfo.param ? "ResamplingEnabled" : "ResamplingDisabled"; });

TEST_P(NeuralStylusPalmDetectionFilterUtilTest, DistilledNocturneTest)
{
    const PalmFilterDeviceInfo nocturne_distilled = CreatePalmFilterDeviceInfo(nocturne_touchscreen_);
    EXPECT_FLOAT_EQ(nocturne_distilled.max_x, nocturne_touchscreen_.GetAbsMaximum(ABS_MT_POSITION_X));
    EXPECT_FLOAT_EQ(nocturne_distilled.max_y, nocturne_touchscreen_.GetAbsMaximum(ABS_MT_POSITION_Y));
    EXPECT_FLOAT_EQ(nocturne_distilled.x_res, nocturne_touchscreen_.GetAbsResolution(ABS_MT_POSITION_X));
    EXPECT_FLOAT_EQ(nocturne_distilled.y_res, nocturne_touchscreen_.GetAbsResolution(ABS_MT_POSITION_Y));
    EXPECT_FLOAT_EQ(nocturne_distilled.major_radius_res, nocturne_touchscreen_.GetAbsResolution(ABS_MT_TOUCH_MAJOR));
    EXPECT_TRUE(nocturne_distilled.minor_radius_supported);
    EXPECT_FLOAT_EQ(nocturne_distilled.minor_radius_res, nocturne_touchscreen_.GetAbsResolution(ABS_MT_TOUCH_MINOR));
}
...
TEST_P(NeuralStylusPalmDetectionFilterUtilTest, DistillerKohakuTest)
{
    ...
    ASSERT_TRUE(CapabilitiesToDeviceInfo(kKohakuTouchscreen, &kohaku_touchscreen));
    const PalmFilterDeviceInfo kohaku_distilled = CreatePalmFilterDeviceInfo(kohaku_touchscreen);
...
TEST_P(NeuralStylusPalmDetectionFilterUtilTest, DistilledLinkTest)
{
    ...
    const PalmFilterDeviceInfo link_distilled = CreatePalmFilterDeviceInfo(link_touchscreen);
    EXPECT_FALSE(link_distilled.minor_radius_supported);
    EXPECT_FLOAT_EQ(1.f, link_distilled.major_radius_res);
    EXPECT_FLOAT_EQ(link_distilled.major_radius_res, link_distilled.minor_radius_res);
}
...
TEST_P(NeuralStylusPalmDetectionFilterUtilTest, PalmFilterSampleTest)
{
    ...
    const PalmFilterDeviceInfo nocturne_distilled = CreatePalmFilterDeviceInfo(nocturne_touchscreen_);
    const PalmFilterSample sample = CreatePalmFilterSample(touch_, time, model_config_, nocturne_distilled);
...
TEST_P(NeuralStylusPalmDetectionFilterUtilTest, LinkTouchscreenSampleTest)
{
    ...
    model_config_.radius_polynomial_resize = { link_touchscreen.GetAbsResolution(ABS_MT_POSITION_X) / 40.0f, 0.0 };
    const PalmFilterSample sample = CreatePalmFilterSample(touch_, time, model_config_, link_distilled);
...
TEST_P(NeuralStylusPalmDetectionFilterUtilTest, PalmFilterStrokeTest)
{
    ...
    for (int i = 0; i < 500000; ++i) {
        touch_.x = 15 + i;
        PalmFilterSample sample = CreatePalmFilterSample(touch_, time, model_config_, nocturne_distilled);
        time += base::Milliseconds(8);
        stroke.ProcessSample(std::move(sample));
        EXPECT_EQ(touch_.tracking_id, stroke.tracking_id());
        if (i < 3) {
            if (i == 0) {
                EXPECT_FLOAT_EQ(gfx::PointF(15 / 40.f, 0.5).x(), stroke.GetCentroid().x());
            } else if (i == 1) {
                EXPECT_FLOAT_EQ(gfx::PointF((30 + 1) / (2 * 40.f), 0.5).x(), stroke.GetCentroid().x());
            } else if (i == 2) {
                EXPECT_FLOAT_EQ(gfx::PointF((45 + 1 + 2) / (3 * 40.f), 0.5).x(), stroke.GetCentroid().x());
            }
            continue;
        }
        float expected_x = (45 + 3 * i - 3) / (3 * 40.f);
        gfx::PointF expected_centroid = gfx::PointF(expected_x, 0.5);
        ASSERT_FLOAT_EQ(expected_centroid.x(), stroke.GetCentroid().x()) << "failed at i " << i;
    }
}

TEST_P(NeuralStylusPalmDetectionFilterUtilTest, PalmFilterStrokeBiggestSizeTest)
{
    ...
    for (int i = 0; i < 500; ++i) {
        ...
        PalmFilterSample second_sample = CreatePalmFilterSample(touch_, time, model_config_, nocturne_distilled);
...
TEST_P(NeuralStylusPalmDetectionFilterUtilTest, UnscaledMajorMinorResolution)
{
    ...
    PalmFilterSample sample = CreatePalmFilterSample(touch_, time, model_config_, device_info);
...
TEST_P(NeuralStylusPalmDetectionFilterUtilTest, SampleRadiusConversion)
{
    ...
    sample = CreatePalmFilterSample(touch_, time, model_config_, nocturne_distilled);
...
TEST(PalmFilterStrokeTest, NumberOfResampledValues)
{
    ...
    InProgressTouchEvdev touch_;
    touch_.tracking_id = stroke.tracking_id();
    PalmFilterSample sample = CreatePalmFilterSample(touch_, down_time, model_config_, device_info);
    stroke.ProcessSample(sample);
    ...
    stroke.ProcessSample(sample);
    ASSERT_THAT(stroke.samples(), ElementsAre(SampleTime(down_time), SampleTime(time)));
    ...
    stroke.ProcessSample(sample);
    ASSERT_THAT(
        stroke.samples(), ElementsAre(SampleTime(down_time), SampleTime(down_time + base::Milliseconds(4)), SampleTime(down_time + base::Milliseconds(10))));
...
TEST(PalmFilterStrokeTest, ResamplingTest)
{
    ...
    touch_.minor = 3;
    base::TimeTicks down_time = base::TimeTicks::UnixEpoch() + base::Seconds(30);
    PalmFilterSample sample1 = CreatePalmFilterSample(touch_, down_time, model_config_, device_info);
    ...
    touch_.major = 12;
    touch_.minor = 11;
    PalmFilterSample sample2 = CreatePalmFilterSample(touch_, time, model_config_, device_info);
    ...
    touch_.major = 14;
    touch_.minor = 13;
    PalmFilterSample sample3 = CreatePalmFilterSample(touch_, time, model_config_, device_info);
...
TEST(PalmFilterStrokeTest, MultipleResampledValues)
{
    ...
    PalmFilterSample resampled_sample = stroke.GetSampleAt(down_time + base::Milliseconds(8));
