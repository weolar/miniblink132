...
GesturesProp::GesturesProp(const std::string& name, const PropertyType type, const size_t count)
    : name_(name)
    , type_(type)
    , count_(count)
{
...
void GesturesProp::SetHandlers(GesturesPropGetHandler get, GesturesPropSetHandler set, void* data)
{
    get_ = get;
    set_ = set;
    handler_data_ = data;
...
template <typename T> class TypedGesturesProp : public GesturesProp {
public:
    TypedGesturesProp(const std::string& name, const PropertyType type, const size_t count, T* value)
...
protected:
    ...
    template <typename U> std::vector<U> GetNumericalPropertyValue() const
    {
    ...
    template <typename U> bool SetNumericalPropertyValue(const std::vector<U>& value)
    {
    ...
    void InitializeNumericalProperty(const T* init, const GesturesProp* default_property)
    {
...
private:
    ...
    template <typename U> std::vector<U> GetNumericalValue() const
    {
    ...
    template <typename U> bool SetNumericalValue(const std::vector<U>& value)
    {
        for (size_t i = 0; i < count(); ++i)
...
public:
    GesturesIntProp(const std::string& name, const size_t count, int* value, const int* init, const GesturesProp* default_property)
...
public:
    GesturesShortProp(const std::string& name, const size_t count, short* value, const short* init, const GesturesProp* default_property)
...
public:
    GesturesBoolProp(const std::string& name, const size_t count, GesturesPropBool* value, const GesturesPropBool* init, const GesturesProp* default_property)
        : TypedGesturesProp<GesturesPropBool>(name, PropertyType::PT_BOOL, count, value)
    {
        InitializeNumericalProperty(init, default_property);
    }
...
public:
    GesturesDoubleProp(const std::string& name, const size_t count, double* value, const double* init, const GesturesProp* default_property)
...
public:
    ...
    // it always allocates.
    GesturesStringProp(const std::string& name, const char** value, const char* init, const GesturesProp* default_property)
...
private:
    // Initialize the object.
    void InitializeStringProperty(const char** value, const char* init, const GesturesProp* default_property)
    {
    ...
    const char** GetStringWritebackPtr() const override
    {
        return write_back_;
    }
...
bool IsDeviceOfType(const ui::GesturePropertyProvider::DevicePtr device, const ui::EventDeviceType type, const GesturesProp* device_mouse_property,
...
std::string GetDeviceNodePath(const ui::GesturePropertyProvider::DevicePtr device)
{
    std::string proc_symlink = "/proc/self/fd/" + base::NumberToString(device->fd);
    base::FilePath path;
...
bool IsMatchTypeSupported(const std::string& match_type)
{
    ...
    constexpr auto kSupportedMatchTypes = base::MakeFixedFlatSet<std::string_view>(
        { "MatchProduct", "MatchDevicePath", "MatchUSBID", "MatchDMIProduct", "MatchIsPointer", "MatchIsTouchpad", "MatchIsTouchscreen" });
    constexpr auto kUnsupportedMatchTypes = base::MakeFixedFlatSet<std::string_view>(
        { "MatchVendor", "MatchOS", "MatchPnPID", "MatchDriver", "MatchTag", "MatchLayout", "MatchIsKeyboard", "MatchIsJoystick", "MatchIsTablet" });
...
template <typename T> void LogArrayProperty(std::ostream& os, const std::vector<T>& value)
{
    os << "(";
...
std::ostream& operator<<(std::ostream& out, const ui::GesturePropertyProvider::PropertyType type)
{
    std::string s;
...
std::ostream& operator<<(std::ostream& os, const GesturesProp& prop)
{
    ...
    os << "\"" << property->name() << "\", " << property->type() << ", " << property->count() << ", (" << property->IsAllocated() << ", "
       << property->IsReadOnly() << "), ";
...
public:
    ...
    explicit MatchDmiProduct(const std::string& dmi_product_name, const std::string& arg, bool load_error = false);
...
MatchCriteria::MatchCriteria(const std::string& arg)
{
    ...
    args_ = base::SplitString(arg, "|", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    if (args_.empty()) {
    ...
    : MatchCriteria(arg)
{
    ...
    for (size_t i = 0; i < args_.size(); ++i) {
        ...
        std::vector<std::string> tokens = base::SplitString(args_[i], ":", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
MatchDmiProduct::MatchDmiProduct(const std::string& dmi_product_name, const std::string& arg, bool load_error)
    : MatchCriteria(arg)
    ...
    , is_valid_(false)
{
    ...
    if (!is_valid_) {
        LOG(ERROR) << "No valid device class boolean keyword found, will be ignored: \"" << arg << "\"";
    }
}
...
bool MatchIsPointer::Match(const DevicePtr device)
{
    if (!is_valid_)
        return true;
    return (value_
        == (device->info.evdev_class == EvdevClassMouse || device->info.evdev_class == EvdevClassPointingStick
...
bool GesturePropertyProvider::GetDeviceIdsByType(const EventDeviceType type, std::vector<DeviceId>* device_ids)
{
    bool exists = false;
...
bool GesturePropertyProvider::IsDeviceIdOfType(const DeviceId device_id, const EventDeviceType type)
{
    auto it = device_map_.find(device_id);
    if (it == device_map_.end())
        return false;
    return IsDeviceOfType(it->second, type, GetProperty(device_id, "Device Mouse"), GetProperty(device_id, "Device Touchpad"));
}

GesturesProp* GesturePropertyProvider::GetProperty(const DeviceId device_id, const std::string& name)
{
    return FindProperty(device_id, name);
}

std::vector<std::string> GesturePropertyProvider::GetPropertyNamesById(const DeviceId device_id)
{
...
std::string GesturePropertyProvider::GetDeviceNameById(const DeviceId device_id)
{
    auto it = device_map_.find(device_id);
    if (it == device_map_.end())
        return std::string();
...
void GesturePropertyProvider::RegisterDevice(const DeviceId id, const DevicePtr device)
{
    ...
    device_data_map_[id] = std::make_unique<internal::GestureDevicePropertyData>();
...
void GesturePropertyProvider::AddProperty(const DeviceId device_id, const std::string& name, std::unique_ptr<GesturesProp> property)
{
...
void GesturePropertyProvider::DeleteProperty(const DeviceId device_id, const std::string& name)
{
    auto it = device_data_map_.find(device_id);
    if (it != device_data_map_.end())
...
GesturesProp* GesturePropertyProvider::FindProperty(const DeviceId device_id, const std::string& name)
{
    auto it = device_data_map_.find(device_id);
    if (it == device_data_map_.end())
        return nullptr;
...
GesturesProp* GesturePropertyProvider::GetDefaultProperty(const DeviceId device_id, const std::string& name)
{
    auto it = device_data_map_.find(device_id);
    if (it == device_data_map_.end())
        return nullptr;
...
void GesturePropertyProvider::LoadDeviceConfigurations()
{
    ...
    base::FileEnumerator file_enum(base::FilePath(kConfigurationFilePath), false, base::FileEnumerator::FILES, "*.conf");
    for (base::FilePath path = file_enum.Next(); !path.empty(); path = file_enum.Next()) {
        files.insert(path);
    }
    ...
    for (auto file_iter = files.begin(); file_iter != files.end(); ++file_iter) {
        ...
        if (!base::ReadFileToString(*file_iter, &content)) {
            LOG(ERROR) << "Can't loading gestures conf file: " << (*file_iter).value();
            continue;
        }
...
void GesturePropertyProvider::ParseXorgConfFile(const std::string& content)
{
    ...
    for (const std::string& section : base::SplitStringUsingSubstr(content, "EndSection", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
        // Create a new configuration section.
        configurations_.push_back(std::make_unique<internal::ConfigurationSection>());
        ...
        while (is_input_class_section && lines.GetNext()) {
            ...
            while (pieces.GetNext()) {
                ...
                if (is_parsing) {
                    ...
                    // the entry type.
                    std::string arg;
                    base::TrimWhitespaceASCII(piece.substr(1, piece.size() - 2), base::TRIM_ALL, &arg);
                    if (next_is_section_type) {
                        ...
                        std::unique_ptr<GesturesProp> property = CreateDefaultProperty(option_name, arg);
                        if (property)
                        ...
                        if (IsMatchTypeSupported(match_type)) {
                            std::unique_ptr<internal::MatchCriteria> criteria = CreateMatchCriteria(match_type, arg);
                            if (criteria)
            ...
            if (!has_error && (next_is_option_value || next_is_match_criteria)) {
                if (next_is_option_value) {
                    std::unique_ptr<GesturesProp> property = CreateDefaultProperty(option_name, "on");
                    if (property)
                        config->properties.push_back(std::move(property));
                } else if (IsMatchTypeSupported(match_type) && IsMatchDeviceType(match_type)) {
                    std::unique_ptr<internal::MatchCriteria> criteria = CreateMatchCriteria(match_type, "on");
                    if (criteria)
                        config->criterias.push_back(std::move(criteria));
                }
            }
...
std::unique_ptr<internal::MatchCriteria> GesturePropertyProvider::CreateMatchCriteria(const std::string& match_type, const std::string& arg)
{
...
bool GesturePropertyProvider::LoadDmiProductName()
{
    ...
    base::TrimWhitespaceASCII(dmi_product_name_, base::TRIM_ALL, &dmi_product_name_);
    dmi_product_name_loaded_ = true;
    return true;
}

std::unique_ptr<GesturesProp> GesturePropertyProvider::CreateDefaultProperty(const std::string& name, const std::string& value)
{
    // Our parsing rule:
    ...
    if (is_all_numeric && numbers.size()) {
        property.reset(new GesturesDoubleProp(name, numbers.size(), NULL, numbers.data(), NULL));
    } else {
...
void GesturePropertyProvider::SetupDefaultProperties(const DeviceId device_id, const DevicePtr device)
{
    DVLOG(2) << "Setting up default properties for (" << device << ", " << device_id << ", " << device->info.name << ")";
    ...
    for (const auto& configuration : configurations_) {
        if (configuration->Match(device)) {
            DVLOG(2) << "Conf section \"" << configuration->identifier << "\" is matched";
...
GesturesProp* GesturesPropFunctionsWrapper::CreateInt(void* device_data, const char* name, int* value, size_t count, const int* init)
{
    return CreateProperty<int, GesturesIntProp>(device_data, name, value, count, init);
}

GesturesProp* GesturesPropFunctionsWrapper::CreateShort(void* device_data, const char* name, short* value, size_t count, const short* init)
{
    return CreateProperty<short, GesturesShortProp>(device_data, name, value, count, init);
}

GesturesProp* GesturesPropFunctionsWrapper::CreateBool(void* device_data, const char* name, GesturesPropBool* value, size_t count, const GesturesPropBool* init)
{
    return CreateProperty<GesturesPropBool, GesturesBoolProp>(device_data, name, value, count, init);
}

GesturesProp* GesturesPropFunctionsWrapper::CreateReal(void* device_data, const char* name, double* value, size_t count, const double* init)
{
    return CreateProperty<double, GesturesDoubleProp>(device_data, name, value, count, init);
}

GesturesProp* GesturesPropFunctionsWrapper::CreateString(void* device_data, const char* name, const char** value, const char* init)
{
...
void GesturesPropFunctionsWrapper::RegisterHandlers(
    void* device_data, GesturesProp* property, void* handler_data, GesturesPropGetHandler get, GesturesPropSetHandler set)
{
    // Sanity checks
...
void GesturesPropFunctionsWrapper::Free(void* device_data, GesturesProp* property)
{
    if (!property)
        return;
...
bool GesturesPropFunctionsWrapper::InitializeDeviceProperties(void* device_data, GestureDeviceProperties* properties)
{
    if (!device_data)
        return false;
    ...
    CreateString(device_data, "Device Node", NULL, GetDeviceNodePath(device).c_str());
    ...
    CreateIntSingle(device_data, "Active Area Left", &properties->area_left, Event_Get_Left(device));
    CreateIntSingle(device_data, "Active Area Right", &properties->area_right, Event_Get_Right(device));
    CreateIntSingle(device_data, "Active Area Top", &properties->area_top, Event_Get_Top(device));
    CreateIntSingle(device_data, "Active Area Bottom", &properties->area_bottom, Event_Get_Bottom(device));

    // Trackpad resolution (pixels/mm). If not configured in .conf file,
    // use x/y resolution as reported by kernel driver.
    CreateIntSingle(device_data, "Vertical Resolution", &properties->res_y, Event_Get_Res_Y(device));
    CreateIntSingle(device_data, "Horizontal Resolution", &properties->res_x, Event_Get_Res_X(device));

    // Trackpad orientation minimum/maximum. If not configured in .conf file,
    // use min/max as reported by kernel driver.
    CreateIntSingle(device_data, "Orientation Minimum", &properties->orientation_minimum, Event_Get_Orientation_Minimum(device));
    CreateIntSingle(device_data, "Orientation Maximum", &properties->orientation_maximum, Event_Get_Orientation_Maximum(device));

    // Log dump property. Will call Event_Dump_Debug_Log when its value is being
    // set.
    GesturesProp* dump_debug_log_prop = CreateBoolSingle(device_data, "Dump Debug Log", &properties->dump_debug_log, false);
    RegisterHandlers(device_data, dump_debug_log_prop, device, NULL, DumpTouchEvdevDebugLog);

    // Whether to do the gesture recognition or just passing the multi-touch data
    // to upper layers.
    CreateBoolSingle(device_data, "Raw Touch Passthrough", &properties->raw_passthrough, false);
    return true;
}
...
GesturesProp* GesturesPropFunctionsWrapper::CreateProperty(void* device_data, const char* name, T* value, size_t count, const T* init)
{
...
bool GesturesPropFunctionsWrapper::PreCreateProperty(void* device_data, const char* name, GesturesProp** default_property)
{
    GesturePropertyProvider* provider = GetPropertyProvider(device_data);
    ...
    if (property) {
        LOG(WARNING) << "Gesture property \"" << name << "\" re-created. This shouldn't happen at the normal usage.";
...
void GesturesPropFunctionsWrapper::PostCreateProperty(void* device_data, const char* name, std::unique_ptr<GesturesProp> property)
{
    // Log the creation.
...
GesturesProp* GesturesPropFunctionsWrapper::CreateIntSingle(void* device_data, const char* name, int* value, int init)
{
    return CreateInt(device_data, name, value, 1, &init);
}

GesturesProp* GesturesPropFunctionsWrapper::CreateBoolSingle(void* device_data, const char* name, GesturesPropBool* value, GesturesPropBool init)
{
    return CreateBool(device_data, name, value, 1, &init);
}

GesturePropertyProvider* GesturesPropFunctionsWrapper::GetPropertyProvider(void* device_data)
{
    return static_cast<GestureInterpreterLibevdevCros*>(device_data)->property_provider();
}

GesturePropertyProvider::DevicePtr GesturesPropFunctionsWrapper::GetDevicePointer(void* device_data)
{
    return static_cast<GestureInterpreterLibevdevCros*>(device_data)->evdev();
}

GesturePropertyProvider::DeviceId GesturesPropFunctionsWrapper::GetDeviceId(void* device_data)
{
 ...
 * */
const GesturesPropProvider kGesturePropProvider = { GesturesPropFunctionsWrapper::CreateInt, GesturesPropFunctionsWrapper::CreateShort,
    GesturesPropFunctionsWrapper::CreateBool, GesturesPropFunctionsWrapper::CreateString, GesturesPropFunctionsWrapper::CreateReal,
    GesturesPropFunctionsWrapper::RegisterHandlers, GesturesPropFunctionsWrapper::Free };
