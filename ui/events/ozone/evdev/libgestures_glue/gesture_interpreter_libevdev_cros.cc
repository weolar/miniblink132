...
HardwareProperties GestureHardwareProperties(Evdev* evdev, const GestureDeviceProperties* props)
{
    ...
    hwprops.is_haptic_pad = EvdevBitIsSet(evdev->info.prop_bitmask, INPUT_PROP_HAPTICPAD);
    hwprops.has_wheel = EvdevBitIsSet(evdev->info.rel_bitmask, REL_WHEEL) || EvdevBitIsSet(evdev->info.rel_bitmask, REL_HWHEEL);
    hwprops.wheel_is_hi_res = EvdevBitIsSet(evdev->info.rel_bitmask, REL_WHEEL_HI_RES);
    hwprops.reports_pressure = EvdevBitIsSet(evdev->info.abs_bitmask, ABS_MT_PRESSURE) || EvdevBitIsSet(evdev->info.abs_bitmask, ABS_PRESSURE);
...
static constexpr unsigned int kModifierEvdevCodes[]
    = { KEY_LEFTALT, KEY_RIGHTALT, KEY_LEFTMETA, KEY_RIGHTMETA, KEY_LEFTCTRL, KEY_RIGHTCTRL, KEY_LEFTSHIFT, KEY_RIGHTSHIFT };

} // namespace

void GestureInterpreterLibevdevCros::RecordClickMetric(stime_t duration, float movement)
{
    int time_bucket;
    ...
    base::UmaHistogramExactLinear(ClickDurationMetricNames[time_bucket], move_bucket, num_move_buckets);
}

GestureInterpreterLibevdevCros::GestureInterpreterLibevdevCros(
    int id, CursorDelegateEvdev* cursor, GesturePropertyProvider* property_provider, DeviceEventDispatcherEvdev* dispatcher)
    : id_(id)
    , cursor_(cursor)
...
void GestureInterpreterLibevdevCros::OnLibEvdevCrosOpen(Evdev* evdev, EventStateRec* evstate)
{
    DCHECK(evdev->info.is_monotonic) << "libevdev must use monotonic timestamps";

    // Set device pointer and initialize properties.
    evdev_ = evdev;
    GesturesPropFunctionsWrapper::InitializeDeviceProperties(this, device_properties_.get());
    ...
    is_pointing_stick_ = property_provider_->IsDeviceIdOfType(id_, DT_POINTING_STICK);

    // Create & initialize GestureInterpreter.
    DCHECK(!interpreter_);
    interpreter_ = NewGestureInterpreter();
    GestureInterpreterSetPropProvider(interpreter_, const_cast<GesturesPropProvider*>(&kGesturePropProvider), this);
    GestureInterpreterInitialize(interpreter_, devclass);
    GestureInterpreterSetHardwareProperties(interpreter_, &hwprops);
    GestureInterpreterSetTimerProvider(interpreter_, const_cast<GesturesTimerProvider*>(&kGestureTimerProvider), this);
    ...
    if (base::FeatureList::IsEnabled(kEnableFastTouchpadClick)) {
        GesturesProp* property = property_provider_->GetProperty(id_, "Wiggle Button Down Timeout");
        if (property) {
...
void GestureInterpreterLibevdevCros::OnLibEvdevCrosEvent(Evdev* evdev, EventStateRec* evstate, const timeval& time)
{
    ...
    if (EvdevBitIsSet(evdev->info.msc_bitmask, MSC_TIMESTAMP)) {
        hwstate.msc_timestamp = static_cast<stime_t>(Event_Get_Timestamp(evdev)) / base::Time::kMicrosecondsPerSecond;
    } else {
...
void GestureInterpreterLibevdevCros::OnLibEvdevCrosStopped(Evdev* evdev, EventStateRec* state)
{
...
void GestureInterpreterLibevdevCros::SetupHapticButtonGeneration(const base::RepeatingCallback<void(bool)>& callback)
{
    click_callback_ = callback;

    GesturesProp* property = property_provider_->GetProperty(id_, "Enable Haptic Button Generation");
...
void GestureInterpreterLibevdevCros::OnGestureReady(const Gesture* gesture)
{
    ...
    case kGestureTypeFourFingerSwipeLift:
        OnGestureFourFingerSwipeLift(gesture, &gesture->details.four_finger_swipe_lift);
        break;
    ...
    case kGestureTypeMetrics:
        ...
        break;
    default:
        LOG(WARNING) << base::StringPrintf("Unrecognized gesture type (%u)", gesture->type);
        break;
    }
}

void GestureInterpreterLibevdevCros::OnGestureMove(const Gesture* gesture, const GestureMove* move)
{
    DVLOG(3) << base::StringPrintf("Gesture Move: (%f, %f) [%f, %f]", move->dx, move->dy, move->ordinal_dx, move->ordinal_dy);
    if (!cursor_)
        return; // No cursor!
    ...
    dispatcher_->DispatchMouseMoveEvent(MouseMoveEventParams(
        id_, EF_NONE, cursor_->GetLocation(), &ordinal_delta, PointerDetails(EventPointerType::kMouse), StimeToTimeTicks(gesture->end_time)));
}

void GestureInterpreterLibevdevCros::OnGestureScroll(const Gesture* gesture, const GestureScroll* scroll)
{
    DVLOG(3) << base::StringPrintf("Gesture Scroll: (%f, %f) [%f, %f]", scroll->dx, scroll->dy, scroll->ordinal_dx, scroll->ordinal_dy);
    if (!cursor_)
        return; // No cursor!

    if (is_mouse_) {
        // Traditional mice don't emit scroll events, but multitouch mice still do.
        dispatcher_->DispatchMouseWheelEvent(MouseWheelEventParams(id_, cursor_->GetLocation(), gfx::Vector2d(scroll->dx, scroll->dy),
            gfx::Vector2d(scroll->dx / kMultitouchMousePixelsPerTick * 120, scroll->dy / kMultitouchMousePixelsPerTick * 120),
            StimeToTimeTicks(gesture->end_time)));
    } else {
        dispatcher_->DispatchScrollEvent(ScrollEventParams(id_, EventType::kScroll, cursor_->GetLocation(), gfx::Vector2dF(scroll->dx, scroll->dy),
            gfx::Vector2dF(scroll->ordinal_dx, scroll->ordinal_dy), kGestureScrollFingerCount, StimeToTimeTicks(gesture->end_time)));
    }
}

void GestureInterpreterLibevdevCros::OnGestureMouseWheel(const Gesture* gesture, const GestureMouseWheel* wheel)
{
    DVLOG(3) << base::StringPrintf("Gesture Mouse Wheel: (%f, %f) [%d, %d]", wheel->dx, wheel->dy, wheel->tick_120ths_dx, wheel->tick_120ths_dy);
    if (!cursor_)
        return; // No cursor!

    dispatcher_->DispatchMouseWheelEvent(MouseWheelEventParams(id_, cursor_->GetLocation(), gfx::Vector2d(wheel->dx, wheel->dy),
        gfx::Vector2d(wheel->tick_120ths_dx, wheel->tick_120ths_dy), StimeToTimeTicks(gesture->end_time)));
}

void GestureInterpreterLibevdevCros::OnGestureButtonsChange(const Gesture* gesture, const GestureButtonsChange* buttons)
{
    DVLOG(3) << base::StringPrintf("Gesture Button Change: down=0x%02x up=0x%02x", buttons->down, buttons->up);
...
void GestureInterpreterLibevdevCros::OnGestureContactInitiated(const Gesture* gesture)
{
    // TODO(spang): handle contact initiated.
}

void GestureInterpreterLibevdevCros::OnGestureFling(const Gesture* gesture, const GestureFling* fling)
{
    DVLOG(3) << base::StringPrintf(
        "Gesture Fling: (%f, %f) [%f, %f] fling_state=%d", fling->vx, fling->vy, fling->ordinal_vx, fling->ordinal_vy, fling->fling_state);

    if (!cursor_)
        return; // No cursor!

    EventType type = (fling->fling_state == GESTURES_FLING_START ? EventType::kScrollFlingStart : EventType::kScrollFlingCancel);

    // Fling is like 2-finger scrolling but with velocity instead of displacement.
    dispatcher_->DispatchScrollEvent(ScrollEventParams(id_, type, cursor_->GetLocation(), gfx::Vector2dF(fling->vx, fling->vy),
        gfx::Vector2dF(fling->ordinal_vx, fling->ordinal_vy), kGestureScrollFingerCount, StimeToTimeTicks(gesture->end_time)));
}

void GestureInterpreterLibevdevCros::OnGestureSwipe(const Gesture* gesture, const GestureSwipe* swipe)
{
    DVLOG(3) << base::StringPrintf("Gesture Swipe: (%f, %f) [%f, %f]", swipe->dx, swipe->dy, swipe->ordinal_dx, swipe->ordinal_dy);

    if (!cursor_)
        return; // No cursor!

    // Swipe is 3-finger scrolling.
    dispatcher_->DispatchScrollEvent(ScrollEventParams(id_, EventType::kScroll, cursor_->GetLocation(), gfx::Vector2dF(swipe->dx, swipe->dy),
        gfx::Vector2dF(swipe->ordinal_dx, swipe->ordinal_dy), kGestureSwipeFingerCount, StimeToTimeTicks(gesture->end_time)));
}

void GestureInterpreterLibevdevCros::OnGestureSwipeLift(const Gesture* gesture, const GestureSwipeLift* swipelift)
{
    ...
    dispatcher_->DispatchScrollEvent(ScrollEventParams(id_, EventType::kScrollFlingStart, cursor_->GetLocation(), gfx::Vector2dF() /* delta */,
        gfx::Vector2dF() /* ordinal_delta */, kGestureScrollFingerCount, StimeToTimeTicks(gesture->end_time)));
}

void GestureInterpreterLibevdevCros::OnGestureFourFingerSwipe(const Gesture* gesture, const GestureFourFingerSwipe* swipe)
{
    DVLOG(3) << base::StringPrintf("Gesture Four Finger Swipe: (%f, %f) [%f, %f]", swipe->dx, swipe->dy, swipe->ordinal_dx, swipe->ordinal_dy);
...
void GestureInterpreterLibevdevCros::OnGestureFourFingerSwipeLift(const Gesture* gesture, const GestureFourFingerSwipeLift* swipe)
{
    ...
    dispatcher_->DispatchScrollEvent(ScrollEventParams(id_, EventType::kScrollFlingStart, cursor_->GetLocation(),
        /*delta=*/gfx::Vector2dF(), /*ordinal_delta=*/gfx::Vector2dF(),
        /*finger_count=*/4, StimeToTimeTicks(gesture->end_time)));
}

void GestureInterpreterLibevdevCros::OnGesturePinch(const Gesture* gesture, const GesturePinch* pinch)
{
    DVLOG(3) << base::StringPrintf("Gesture Pinch: dz=%f [%f] zoom_state=%u", pinch->dz, pinch->ordinal_dz, pinch->zoom_state);
    ...
    case GESTURES_ZOOM_END:
        ...
        break;
    default:
        LOG(WARNING) << base::StringPrintf("Unrecognized pinch zoom state (%u)", pinch->zoom_state);
        return;
    }
    dispatcher_->DispatchPinchEvent(PinchEventParams(id_, type, cursor_->GetLocation(), pinch->dz, StimeToTimeTicks(gesture->end_time)));
}

void GestureInterpreterLibevdevCros::OnGestureMetrics(const Gesture* gesture, const GestureMetrics* metrics)
{
    DVLOG(3) << base::StringPrintf("Gesture Metrics: [%f, %f] type=%d", metrics->data[0], metrics->data[1], metrics->type);

    // TODO(spang): Hook up metrics.
}

void GestureInterpreterLibevdevCros::DispatchChangedMouseButtons(unsigned int changed_buttons, bool down, stime_t time)
{
...
void GestureInterpreterLibevdevCros::DispatchMouseButton(unsigned int button, bool down, stime_t time)
{
    ...
    dispatcher_->DispatchMouseButtonEvent(
        MouseButtonEventParams(id_, EF_NONE, cursor_->GetLocation(), button, down, map_type, PointerDetails(EventPointerType::kMouse), StimeToTimeTicks(time)));
}

void GestureInterpreterLibevdevCros::SetReceivedValidKeyboardInputCallback(base::RepeatingCallback<void(uint64_t)> callback)
{
    received_keyboard_input_ = std::move(callback);
}

void GestureInterpreterLibevdevCros::SetReceivedValidMouseInputCallback(base::RepeatingCallback<void(int)> callback)
{
    received_mouse_input_ = std::move(callback);
}

void GestureInterpreterLibevdevCros::DispatchChangedKeys(unsigned long* new_key_state, stime_t timestamp)
{
    ...
    for (unsigned long key = 0; key < KEY_CNT; ++key) {
        if (EvdevBitIsSet(key_state_diff, key)) {
            ...
            dispatcher_->DispatchKeyEvent(
                KeyEventParams(id_, ui::EF_NONE, key, 0 /* scan_code */, value, false /* suppress_auto_repeat */, StimeToTimeTicks(timestamp)));
        }
    }
...
bool GestureInterpreterLibevdevCros::SetMouseButtonState(unsigned int button, bool down)
{
...
void GestureInterpreterLibevdevCros::SetBlockModifiers(bool block_modifiers)
{
    ...
    // blocking modifiers.
    const bool should_release_held_modifiers = block_modifiers && !block_modifiers_;
    ...
    if (should_release_held_modifiers) {
        ...
        DispatchChangedKeys(copy_key_state, ui::EventTimeStampToSeconds(ui::EventTimeForNow()));
    }
}
