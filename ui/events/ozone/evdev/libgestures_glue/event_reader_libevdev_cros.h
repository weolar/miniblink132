...
public:
    ...
    public:
        ...
        virtual void OnLibEvdevCrosEvent(Evdev* evdev, EventStateRec* state, const timeval& time) = 0;
        ...
        virtual void SetupHapticButtonGeneration(const base::RepeatingCallback<void(bool)>& callback) = 0;

        // For keyboard/pointer combo devices. Sets a callback that will be called
        // whenever the device registers valid keyboard input.
        virtual void SetReceivedValidKeyboardInputCallback(base::RepeatingCallback<void(uint64_t)> callback) = 0;

        // For keyboard/pointer combo devices. Sets a callback that will be called
        // whenever the device registers valid mouse input.
        virtual void SetReceivedValidMouseInputCallback(base::RepeatingCallback<void(int)> callback) = 0;
    ...
    EventReaderLibevdevCros(base::ScopedFD fd, const base::FilePath& path, int id, const EventDeviceInfo& devinfo, std::unique_ptr<Delegate> delegate);
    ...
    void SetReceivedValidInputCallback(ReceivedValidInputCallback callback) override;
    void SetBlockModifiers(bool block_modifiers) override;

    std::ostream& DescribeForLog(std::ostream& os) const override;

private:
    static void OnSynReport(void* data, EventStateRec* evstate, struct timeval* tv);
