...
bool PropertyTypeMatchesValues(ui::GesturePropertyProvider::PropertyType type, GesturePropValue::Tag values_tag)
{
    switch (type) {
...
bool TrySetPropertyValues(GesturesProp* property, ozone::mojom::GesturePropValuePtr values)
{
...
GesturePropertiesService::GesturePropertiesService(GesturePropertyProvider* provider, mojo::PendingReceiver<ozone::mojom::GesturePropertiesService> receiver)
    : prop_provider_(provider)
...
void GesturePropertiesService::ListProperties(int device_id, ListPropertiesCallback reply)
{
    std::vector<std::string> response = prop_provider_->GetPropertyNamesById(device_id);
    std::move(reply).Run(response);
}

void GesturePropertiesService::GetProperty(int device_id, const std::string& name, GetPropertyCallback reply)
{
    bool is_read_only = true;
    GesturesProp* property = prop_provider_->GetProperty(device_id, name);
    ozone::mojom::GesturePropValuePtr prop_value = GesturePropValueFromProp(property);
    if (property != nullptr) {
...
void GesturePropertiesService::SetProperty(int device_id, const std::string& name, ozone::mojom::GesturePropValuePtr values, SetPropertyCallback reply)
{
    GesturesProp* property = prop_provider_->GetProperty(device_id, name);
    if (property == NULL) {
    ...
    std::move(reply).Run(did_set ? ozone::mojom::SetGesturePropErrorCode::SUCCESS : ozone::mojom::SetGesturePropErrorCode::UNKNOWN_ERROR);
}
