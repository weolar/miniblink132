...
template <typename T> std::string DumpArrayProperty(const std::vector<T>& value, const char* format)
{
    std::string ret;
...
void CompressDumpedLog(std::unique_ptr<std::vector<std::string>> log_paths)
{
    for (size_t i = 0; i < log_paths->size(); ++i) {
        ...
        base::Move(base::FilePath((*log_paths)[i] + ".gz"), base::FilePath((*log_paths)[i]));
    }
}
...
std::string GenerateEventLogName(const base::FilePath& out_dir, const std::string& prefix, const std::string& now, int id)
{
    return out_dir.value() + "/" + prefix + now + "." + base::NumberToString(id);
}
// Name event logs in a way that is compatible with existing toolchain.
std::string GenerateEventLogName(GesturePropertyProvider* provider, const base::FilePath& out_dir, const std::string& prefix, const std::string& now, int id)
{
    return out_dir.value() + "/" + prefix + now + "." + base::NumberToString(id) + "." + GetCanonicalDeviceName(provider->GetDeviceNameById(id));
}

// Set the logging properties to dump event logs.
void StartToDumpEventLog(GesturePropertyProvider* provider, const int device_id)
{
    // Dump gesture log.
...
void DumpTouchDeviceStatus(GesturePropertyProvider* provider, std::string* status)
{
    ...
    for (size_t i = 0; i < ids.size(); ++i) {
        ...
        status->append(base::StringPrintf("Device \'%s\':\n", provider->GetDeviceNameById(ids[i]).c_str()));
...
void DumpTouchEventLog(const std::map<base::FilePath, std::unique_ptr<EventConverterEvdev>>& converters, GesturePropertyProvider* provider,
    const base::FilePath& out_dir, InputController::GetTouchEventLogReply reply)
{
    ...
    std::unique_ptr<std::vector<std::string>> log_paths_to_be_compressed(new std::vector<std::string>);
    for (size_t i = 0; i < ids.size(); ++i) {
        ...
        std::string gesture_log_filename = GenerateEventLogName(provider, out_dir, "touchpad_activity_", now, ids[i]);
        base::Move(base::FilePath(kTouchpadGestureLogPath), base::FilePath(gesture_log_filename));
        std::string evdev_log_filename = GenerateEventLogName(provider, out_dir, "cmt_input_events_", now, ids[i]);
        base::Move(base::FilePath(kTouchpadEvdevLogPath), base::FilePath(evdev_log_filename));
    ...
    for (const auto& converter_pair : converters) {
        ...
        if (converter->HasTouchscreen()) {
            std::string touch_evdev_log_filename = GenerateEventLogName(out_dir, "evdev_input_events_", now, converter->id());
#if BUILDFLAG(IS_CHROMEOS)
            converter->DumpTouchEventLog(touch_evdev_log_filename.c_str());
#else
            converter->DumpTouchEventLog(kInputEventsLogFile);
            base::Move(base::FilePath(kInputEventsLogFile), base::FilePath(touch_evdev_log_filename));
    ...
    base::ThreadPool::PostTaskAndReply(FROM_HERE, { base::MayBlock(), base::TaskPriority::BEST_EFFORT, base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN },
        base::BindOnce(&CompressDumpedLog, std::move(log_paths_to_be_compressed)), base::BindOnce(std::move(reply), log_paths));
}
