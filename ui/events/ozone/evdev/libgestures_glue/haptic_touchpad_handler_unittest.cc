...
public:
    explicit TestHapticTouchpadHandler(int fd)
        : HapticTouchpadHandler(fd)
    {
    }
    ...
    bool TakeControlOfClickEffects() override
    {
        return true;
    }
...
public:
    ...
    HapticTouchpadHandlerTest& operator=(const HapticTouchpadHandlerTest&) = delete;

    std::unique_ptr<ui::TestHapticTouchpadHandler> CreateHandler(const ui::DeviceCapabilities& caps)
    {
...
TEST_F(HapticTouchpadHandlerTest, BasicUiEffects)
{
    ...
    handler->PlayEffect(HapticTouchpadEffect::kSnap, HapticTouchpadEffectStrength::kLow);
    handler->PlayEffect(HapticTouchpadEffect::kSnap, HapticTouchpadEffectStrength::kMedium);
    handler->PlayEffect(HapticTouchpadEffect::kSnap, HapticTouchpadEffectStrength::kLow);
...
TEST_F(HapticTouchpadHandlerTest, SetReleaseEffectWhenButtonNotPressed)
{
    ...
    handler->OnGestureClick(true);
    handler->OnGestureClick(false);
    handler->SetEffectForNextButtonRelease(HapticTouchpadEffect::kKnock, HapticTouchpadEffectStrength::kHigh);
    handler->OnGestureClick(true);
    handler->OnGestureClick(false);
