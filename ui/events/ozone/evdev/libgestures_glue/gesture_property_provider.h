...
public:
    ...
    // won't be deleted.
    bool GetDeviceIdsByType(const EventDeviceType type, std::vector<DeviceId>* device_ids);
...
private:
    ...
    void AddProperty(const DeviceId device_id, const std::string& name, std::unique_ptr<GesturesProp> property);
    ...
    GesturesProp* GetDefaultProperty(const DeviceId device_id, const std::string& name);
    ...
    std::unique_ptr<internal::MatchCriteria> CreateMatchCriteria(const std::string& match_type, const std::string& arg);
    ...
    std::unique_ptr<GesturesProp> CreateDefaultProperty(const std::string& name, const std::string& value);
    ...
    std::unordered_map<DeviceId, std::unique_ptr<internal::GestureDevicePropertyData>> device_data_map_;
...
public:
    ...
    // lib.
    static GesturesProp* CreateInt(void* device_data, const char* name, int* value, size_t count, const int* init);
    static GesturesProp* CreateShort(void* device_data, const char* name, short* value, size_t count, const short* init);
    static GesturesProp* CreateBool(void* device_data, const char* name, GesturesPropBool* value, size_t count, const GesturesPropBool* init);
    ...
    static GesturesProp* CreateString(void* device_data, const char* name, const char** value, const char* init);

    static GesturesProp* CreateReal(void* device_data, const char* name, double* value, size_t count, const double* init);

    // Set the handlers to call when a property is accessed.
    static void RegisterHandlers(void* device_data, GesturesProp* property, void* handler_data, GesturesPropGetHandler get, GesturesPropSetHandler set);
    ...
    // gesture lib.
    static bool InitializeDeviceProperties(void* device_data, GestureDeviceProperties* properties);
...
private:
    ...
    template <typename T, class PROPTYPE> static GesturesProp* CreateProperty(void* device_data, const char* name, T* value, size_t count, const T* init);

    // Do things that should happen BEFORE we create the property.
    static bool PreCreateProperty(void* device_data, const char* name, GesturesProp** default_property);

    // Do things that should happen AFTER we create the property.
    static void PostCreateProperty(void* device_data, const char* name, std::unique_ptr<GesturesProp> property);

    // Some other utility functions used in InitializeDeviceProperties.
    static GesturesProp* CreateIntSingle(void* device_data, const char* name, int* value, int init);
    static GesturesProp* CreateBoolSingle(void* device_data, const char* name, GesturesPropBool* value, GesturesPropBool init);
...
public:
    ...
    GesturesProp(const std::string& name, const PropertyType type, const size_t count);
    ...
    void SetHandlers(GesturesPropGetHandler get, GesturesPropSetHandler set, void* data);
...
private:
    // For logging purpose.
    friend std::ostream& operator<<(std::ostream& os, const GesturesProp& property);
