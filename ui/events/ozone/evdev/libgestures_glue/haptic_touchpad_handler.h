...
public:
    ...
    // haptic features.
    static std::unique_ptr<HapticTouchpadHandler> Create(int fd, const EventDeviceInfo& devinfo);
    ...
    void SetEffectForNextButtonRelease(HapticTouchpadEffect effect_type, HapticTouchpadEffectStrength strength);
    ...
    void PlayEffect(HapticTouchpadEffect effect, HapticTouchpadEffectStrength strength);
...
private:
    ...
    std::map<HapticTouchpadEffect, std::map<HapticTouchpadEffectStrength, int>> ff_effect_id_;
