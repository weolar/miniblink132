...
EventReaderLibevdevCros::EventReaderLibevdevCros(
    base::ScopedFD fd, const base::FilePath& path, int id, const EventDeviceInfo& devinfo, std::unique_ptr<Delegate> delegate)
    : EventConverterEvdev(
    ...
    , delegate_(std::move(delegate))
{
    ...
    if (haptic_touchpad_handler_) {
        delegate_->SetupHapticButtonGeneration(base::BindRepeating(&HapticTouchpadHandler::OnGestureClick, base::Unretained(haptic_touchpad_handler_.get())));
    }
}
...
{
    TRACE_EVENT1("evdev", "EventReaderLibevdevCros::OnFileCanReadWithoutBlocking", "fd", fd);
...
{
    return haptic_touchpad_handler_ && haptic_feedback_enabled_ && touch_count_ > 0;
}

void EventReaderLibevdevCros::PlayHapticTouchpadEffect(HapticTouchpadEffect effect, HapticTouchpadEffectStrength strength)
{
    if (CanHandleHapticFeedback())
        haptic_touchpad_handler_->PlayEffect(effect, strength);
}

void EventReaderLibevdevCros::SetHapticTouchpadEffectForNextButtonRelease(HapticTouchpadEffect effect, HapticTouchpadEffectStrength strength)
{
    if (CanHandleHapticFeedback())
        haptic_touchpad_handler_->SetEffectForNextButtonRelease(effect, strength);
}

void EventReaderLibevdevCros::ApplyDeviceSettings(const InputDeviceSettingsEvdev& settings)
{
    const auto& touchpad_settings = settings.GetTouchpadSettings(id());
    if (haptic_touchpad_handler_) {
        haptic_touchpad_handler_->SetClickStrength(static_cast<HapticTouchpadEffectStrength>(touchpad_settings.haptic_click_sensitivity));
    }
...
void EventReaderLibevdevCros::SetReceivedValidInputCallback(ReceivedValidInputCallback callback)
{
    delegate_->SetReceivedValidKeyboardInputCallback(base::BindRepeating(&EventReaderLibevdevCros::ReceivedKeyboardInput, base::Unretained(this)));
    delegate_->SetReceivedValidMouseInputCallback(base::BindRepeating(&EventReaderLibevdevCros::ReceivedMouseInput, base::Unretained(this)));
...
// static
void EventReaderLibevdevCros::OnSynReport(void* data, EventStateRec* evstate, struct timeval* tv)
{
...
// static
void EventReaderLibevdevCros::OnLogMessage(void* data, int level, const char* fmt, ...)
{
    va_list args;
    va_start(args, fmt);
