...
class COMPONENT_EXPORT(EVDEV) GestureInterpreterLibevdevCros : public EventReaderLibevdevCros::Delegate {
public:
    GestureInterpreterLibevdevCros(int id, CursorDelegateEvdev* cursor, GesturePropertyProvider* property_provider, DeviceEventDispatcherEvdev* dispatcher);

    GestureInterpreterLibevdevCros(const GestureInterpreterLibevdevCros&) = delete;
    GestureInterpreterLibevdevCros& operator=(const GestureInterpreterLibevdevCros&) = delete;
    ...
    void OnLibEvdevCrosEvent(Evdev* evdev, EventStateRec* evstate, const timeval& time) override;
    void OnLibEvdevCrosStopped(Evdev* evdev, EventStateRec* state) override;
    void SetupHapticButtonGeneration(const base::RepeatingCallback<void(bool)>& callback) override;
    void SetReceivedValidKeyboardInputCallback(base::RepeatingCallback<void(uint64_t)> callback) override;
    void SetReceivedValidMouseInputCallback(base::RepeatingCallback<void(int)> callback) override;
...
private:
    ...
    void OnGestureMouseWheel(const Gesture* gesture, const GestureMouseWheel* wheel);
    void OnGestureButtonsChange(const Gesture* gesture, const GestureButtonsChange* move);
    ...
    void OnGestureSwipeLift(const Gesture* gesture, const GestureSwipeLift* swipelift);
    void OnGestureFourFingerSwipe(const Gesture* gesture, const GestureFourFingerSwipe* swipe);
    void OnGestureFourFingerSwipeLift(const Gesture* gesture, const GestureFourFingerSwipeLift* swipe);
    ...
    void DispatchChangedMouseButtons(unsigned int changed_buttons, bool down, stime_t time);
    void DispatchMouseButton(unsigned int button, bool down, stime_t time);
