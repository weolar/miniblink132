...
#if defined(USE_EVDEV_GESTURES)
void SetGestureIntProperty(GesturePropertyProvider* provider, int id, const std::string& name, int value)
{
...
void SetGestureBoolProperty(GesturePropertyProvider* provider, int id, const std::string& name, bool value)
{
    GesturesProp* property = provider->GetProperty(id, name);
    if (property) {
...
{
    return !converter.HasTouchscreen() && !converter.HasKeyboard() && !converter.HasMouse() && !converter.HasPointingStick() && !converter.HasTouchpad()
        && !converter.HasGamepad() && !converter.HasGraphicsTablet();
}

} // namespace

InputDeviceFactoryEvdev::InputDeviceFactoryEvdev(std::unique_ptr<DeviceEventDispatcherEvdev> dispatcher, CursorDelegateEvdev* cursor,
    std::unique_ptr<InputDeviceOpener> input_device_opener, InputControllerEvdev* input_controller)
...
void InputDeviceFactoryEvdev::AddInputDevice(int id, const base::FilePath& path)
{
    ...
    std::unique_ptr<EventConverterEvdev> converter = input_device_opener_->OpenInputDevice(params);

    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&InputDeviceFactoryEvdev::AttachInputDevice, weak_ptr_factory_.GetWeakPtr(), std::move(converter)));
...
void InputDeviceFactoryEvdev::AttachInputDevice(std::unique_ptr<EventConverterEvdev> converter)
{
    if (converter.get()) {
        ...
        if (converter->type() == InputDeviceType::INPUT_DEVICE_INTERNAL && converter->HasPen() && base::FeatureList::IsEnabled(kEnablePalmSuppression)) {
            converter->SetPalmSuppressionCallback(base::BindRepeating(&InputDeviceFactoryEvdev::EnablePalmSuppression, base::Unretained(this)));
        }

        if (converter->type() == InputDeviceType::INPUT_DEVICE_INTERNAL && converter->HasPen()) {
            converter->SetReportStylusStateCallback(base::BindRepeating(&InputDeviceFactoryEvdev::SetLatestStylusState, base::Unretained(this)));
        }

        if (converter->type() == InputDeviceType::INPUT_DEVICE_INTERNAL && converter->HasTouchscreen() && !converter->HasPen()) {
            converter->SetGetLatestStylusStateCallback(base::BindRepeating(&InputDeviceFactoryEvdev::GetLatestStylusState, base::Unretained(this)));
        }

        if ((converter->type() == InputDeviceType::INPUT_DEVICE_USB || converter->type() == InputDeviceType::INPUT_DEVICE_BLUETOOTH)
            && (converter->HasKeyboard() || converter->HasMouse())) {
            converter->SetReceivedValidInputCallback(base::BindRepeating(&InputDeviceFactoryEvdev::UpdateDevicesOnImposterOverride, base::Unretained(this)));
        }
        ...
        // physical port.
        std::vector<int> ids_to_check = imposter_checker_->OnDeviceAdded(converters_[path].get());
        // Check for imposters on all devices that share the same physical port.
        for (const auto& it : converters_) {
            if (base::Contains(ids_to_check, it.second->id()) && imposter_checker_->FlagSuspectedImposter(it.second.get())) {
...
void InputDeviceFactoryEvdev::DetachInputDevice(const base::FilePath& path)
{
    ...
    if (converter) {
        ...
        // physical port.
        std::vector<int> ids_to_check = imposter_checker_->OnDeviceRemoved(converter.get());
        // Check for imposters on all devices that share the same physical port.
...
void InputDeviceFactoryEvdev::GetStylusSwitchState(InputController::GetStylusSwitchStateReply reply)
{
    for (const auto& it : converters_) {
...
void InputDeviceFactoryEvdev::UpdateInputDeviceSettings(const InputDeviceSettingsEvdev& settings)
{
    input_device_settings_ = settings;
    ApplyInputDeviceSettings();
}

void InputDeviceFactoryEvdev::GetTouchDeviceStatus(InputController::GetTouchDeviceStatusReply reply)
{
    std::string status;
#if defined(USE_EVDEV_GESTURES)
...
void InputDeviceFactoryEvdev::GetTouchEventLog(const base::FilePath& out_dir, InputController::GetTouchEventLogReply reply)
{
#if defined(USE_EVDEV_GESTURES)
    DumpTouchEventLog(converters_, gesture_property_provider_.get(), out_dir, std::move(reply));
#else
    std::move(reply).Run(std::vector<base::FilePath>());
#endif
}

void InputDeviceFactoryEvdev::DescribeForLog(InputController::DescribeForLogReply reply) const
{
...
void InputDeviceFactoryEvdev::GetGesturePropertiesService(mojo::PendingReceiver<ozone::mojom::GesturePropertiesService> receiver)
{
#if defined(USE_EVDEV_GESTURES)
    gesture_properties_service_ = std::make_unique<GesturePropertiesService>(gesture_property_provider_.get(), std::move(receiver));
#endif
}
...
void InputDeviceFactoryEvdev::SetMousePropertiesPerDevice()
{
    ...
    for (const int id : ids) {
        ...
        SetIntPropertyForOneDevice(id, "Pointer Sensitivity", mouse_settings.sensitivity);
        SetBoolPropertyForOneDevice(id, "Pointer Acceleration", input_device_settings_.suspend_acceleration ? false : mouse_settings.acceleration_enabled);
        SetIntPropertyForOneDevice(id, "Mouse Scroll Sensitivity", mouse_settings.scroll_sensitivity);
        SetBoolPropertyForOneDevice(id, "Mouse Scroll Acceleration", mouse_settings.scroll_acceleration_enabled);
        SetBoolPropertyForOneDevice(id, "Mouse Reverse Scrolling", mouse_settings.reverse_scroll_enabled);
        ...
        if (gesture_property_provider_->IsDeviceIdOfType(id, DT_MULTITOUCH)) {
            ...
            if (input_device_settings_.enable_per_device_settings) {
                SetBoolPropertyForOneDevice(id, "Australian Scrolling", mouse_settings.reverse_scroll_enabled);
            } else {
                SetBoolPropertyForOneDevice(id, "Australian Scrolling", input_device_settings_.GetTouchpadSettings(id).natural_scroll_enabled);
            }
        }
...
void InputDeviceFactoryEvdev::SetTouchpadPropertiesPerDevice()
{
    ...
    for (const int id : ids) {
        const auto& touchpad_settings = input_device_settings_.GetTouchpadSettings(id);
        SetIntPropertyForOneDevice(id, "Haptic Button Sensitivity", touchpad_settings.haptic_click_sensitivity);
        SetIntPropertyForOneDevice(id, "Pointer Sensitivity", touchpad_settings.sensitivity);
        SetIntPropertyForOneDevice(id, "Scroll Sensitivity", touchpad_settings.scroll_sensitivity);
        SetBoolPropertyForOneDevice(id, "Pointer Acceleration", touchpad_settings.acceleration_enabled);
        SetBoolPropertyForOneDevice(id, "Scroll Acceleration", touchpad_settings.scroll_acceleration_enabled);
        SetBoolPropertyForOneDevice(id, "Australian Scrolling", touchpad_settings.natural_scroll_enabled);
        SetBoolPropertyForOneDevice(id, "Tap Enable", touchpad_settings.tap_to_click_enabled);
        SetBoolPropertyForOneDevice(id, "Tap Drag Enable", touchpad_settings.tap_dragging_enabled);
    }
#endif
}
...
void InputDeviceFactoryEvdev::SetPointingStickPropertiesPerDevice()
{
    ...
    for (const int id : ids) {
        const auto& pointing_stick_settings = input_device_settings_.GetPointingStickSettings(id);
        SetIntPropertyForOneDevice(id, "Pointer Sensitivity", pointing_stick_settings.sensitivity);
        SetBoolPropertyForOneDevice(
            id, "Pointer Acceleration", input_device_settings_.suspend_acceleration ? false : pointing_stick_settings.acceleration_enabled);
        SetBoolPropertyForOneDevice(id, "Mouse High Resolution Scrolling", true);
        SetBoolPropertyForOneDevice(id, "Output Mouse Wheel Gestures", true);
    }
#endif
...
void InputDeviceFactoryEvdev::ApplyInputDeviceSettings()
{
    ...
    SetBoolPropertyForOneType(DT_TOUCHPAD, "T5R2 Three Finger Click Enable", input_device_settings_.three_finger_click_enabled);
    SetBoolPropertyForOneType(DT_ALL, "Event Logging Enable", base::FeatureList::IsEnabled(ui::kEnableInputEventLogging));
    ...
    for (const auto& it : converters_) {
        ...
        if (converter->type() == InputDeviceType::INPUT_DEVICE_INTERNAL && converter->HasKeyboard()) {
            converter->SetKeyFilter(input_device_settings_.enable_internal_keyboard_filter, input_device_settings_.internal_keyboard_allowed_keys);
        }

        // Block modifiers on the current converter if the device id exists in
        // `input_device_settings_.blocked_modifiers_devices`
        converter->SetBlockModifiers(base::Contains(input_device_settings_.blocked_modifiers_devices, converter->id()));

        converter->ApplyDeviceSettings(input_device_settings_);

        converter->SetTouchEventLoggingEnabled(input_device_settings_.touch_event_logging_enabled);
    }
...
void InputDeviceFactoryEvdev::PlayVibrationEffect(int id, uint8_t amplitude, uint16_t duration_millis)
{
    for (const auto& it : converters_) {
        if (it.second->id() == id) {
...
void InputDeviceFactoryEvdev::PlayHapticTouchpadEffect(ui::HapticTouchpadEffect effect, ui::HapticTouchpadEffectStrength strength)
{
    for (const auto& it : converters_) {
...
void InputDeviceFactoryEvdev::SetHapticTouchpadEffectForNextButtonRelease(ui::HapticTouchpadEffect effect, ui::HapticTouchpadEffectStrength strength)
{
    for (const auto& it : converters_) {
        if (it.second->HasHapticTouchpad()) {
...
bool InputDeviceFactoryEvdev::IsDeviceEnabled(const EventConverterEvdev* converter)
{
    ...
    if (palm_suppression_enabled_ && converter->type() == InputDeviceType::INPUT_DEVICE_INTERNAL && converter->HasTouchscreen() && !converter->HasPen())
        return false;

    return input_device_settings_.enable_devices;
}

void InputDeviceFactoryEvdev::UpdateDirtyFlags(const EventConverterEvdev* converter)
{
...
void InputDeviceFactoryEvdev::NotifyTouchscreensUpdated()
{
    ...
    for (const auto& it : converters_) {
        if (it.second->HasTouchscreen()) {
            touchscreens.emplace_back(it.second->input_device(), it.second->GetTouchscreenSize(), it.second->GetTouchPoints(), it.second->HasPen(),
                it.second->type() == ui::InputDeviceType::INPUT_DEVICE_INTERNAL && has_stylus_switch);
        }
    }
...
void InputDeviceFactoryEvdev::NotifyKeyboardsUpdated()
{
    ...
    for (auto& converter : converters_) {
        if (converter.second->HasKeyboard()) {
            keyboards.emplace_back(converter.second->input_device(), converter.second->HasAssistantKey(), converter.second->HasFunctionKey());
            key_bits_mapping[converter.second->id()] = converter.second->GetKeyboardKeyBits();
        }
    }
    dispatcher_->DispatchKeyboardDevicesUpdated(keyboards, std::move(key_bits_mapping));
}
...
void InputDeviceFactoryEvdev::NotifyMouseDevicesUpdated()
{
    ...
    for (auto& converter : converters_) {
        if (converter.second->HasMouse()) {
            ...
            if (converter.second->type() != ui::InputDeviceType::INPUT_DEVICE_INTERNAL) {
                has_mouse = true;
            }
        }
    }
...
void InputDeviceFactoryEvdev::NotifyTouchpadDevicesUpdated()
{
    ...
    for (const auto& it : converters_) {
        if (it.second->HasTouchpad()) {
            ...
            touchpads.emplace_back(it.second->input_device(), it.second->HasHapticTouchpad());
...
void InputDeviceFactoryEvdev::NotifyGamepadDevicesUpdated()
{
    ...
    for (auto it = converters_.begin(); it != converters_.end(); ++it) {
        if (it->second->HasGamepad()) {
            gamepads.emplace_back(it->second->input_device(), it->second->GetGamepadAxes(), it->second->GetGamepadRumbleCapability());
...
void InputDeviceFactoryEvdev::UpdateDevicesOnImposterOverride(const EventConverterEvdev* converter)
{
    UpdateDirtyFlags(converter);
    NotifyDevicesUpdated();
}

void InputDeviceFactoryEvdev::SetBoolPropertyForOneDevice(int device_id, const std::string& name, bool value)
{
#if defined(USE_EVDEV_GESTURES)
    SetGestureBoolProperty(gesture_property_provider_.get(), device_id, name, value);
#endif
}

void InputDeviceFactoryEvdev::SetIntPropertyForOneDevice(int device_id, const std::string& name, int value)
{
#if defined(USE_EVDEV_GESTURES)
    SetGestureIntProperty(gesture_property_provider_.get(), device_id, name, value);
#endif
}

void InputDeviceFactoryEvdev::SetIntPropertyForOneType(const EventDeviceType type, const std::string& name, int value)
{
#if defined(USE_EVDEV_GESTURES)
    std::vector<int> ids;
    gesture_property_provider_->GetDeviceIdsByType(type, &ids);
    for (size_t i = 0; i < ids.size(); ++i) {
        SetGestureIntProperty(gesture_property_provider_.get(), ids[i], name, value);
    }
#endif
...
void InputDeviceFactoryEvdev::SetBoolPropertyForOneType(const EventDeviceType type, const std::string& name, bool value)
{
#if defined(USE_EVDEV_GESTURES)
    std::vector<int> ids;
    gesture_property_provider_->GetDeviceIdsByType(type, &ids);
    for (size_t i = 0; i < ids.size(); ++i) {
        SetGestureBoolProperty(gesture_property_provider_.get(), ids[i], name, value);
    }
#endif
}
...
void InputDeviceFactoryEvdev::EnablePalmSuppression(bool enabled)
{
    ...
    // inline here.
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&InputDeviceFactoryEvdev::EnableDevices, weak_ptr_factory_.GetWeakPtr()));
}
...
void InputDeviceFactoryEvdev::SetLatestStylusState(
    const InProgressTouchEvdev& event, const int32_t x_res, const int32_t y_res, const base::TimeTicks& timestamp)
{
...
void InputDeviceFactoryEvdev::GetLatestStylusState(const InProgressStylusState** stylus_state) const
{
