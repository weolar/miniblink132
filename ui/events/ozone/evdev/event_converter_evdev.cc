...
EventConverterEvdev::EventConverterEvdev(int fd, const base::FilePath& path, int id, InputDeviceType type, const std::string& name, const std::string& phys,
    uint16_t vendor_id, uint16_t product_id, uint16_t version)
    : fd_(fd)
    , path_(path)
    , input_device_(id, type, name, phys, GetInputPathInSys(path), vendor_id, product_id, version)
...
{
    return (key >= KEY_1 && key <= KEY_EQUAL) || (key >= KEY_Q && key <= KEY_RIGHTBRACE) || (key >= KEY_A && key <= KEY_APOSTROPHE)
        || (key >= KEY_BACKSLASH && key <= KEY_SLASH);
}

void EventConverterEvdev::ApplyDeviceSettings(const InputDeviceSettingsEvdev& settings)
{
}

void EventConverterEvdev::Start()
{
    base::CurrentUIThread::Get()->WatchFileDescriptor(fd_, true, base::MessagePumpForUI::WATCH_READ, &controller_, this);
    watching_ = true;
}
...
void EventConverterEvdev::SetEnabled(bool enabled)
{
    if (enabled == input_device_.enabled)
        return;
    if (enabled) {
        TRACE_EVENT1("evdev", "EventConverterEvdev::OnEnabled", "path", path_.value());
        OnEnabled();
    } else {
        TRACE_EVENT1("evdev", "EventConverterEvdev::OnDisabled", "path", path_.value());
        OnDisabled();
    }
...
std::vector<ui::GamepadDevice::Axis> EventConverterEvdev::GetGamepadAxes() const
{
    NOTREACHED();
}
...
void EventConverterEvdev::PlayVibrationEffect(uint8_t amplitude, uint16_t duration_millis)
{
    NOTREACHED();
}
...
void EventConverterEvdev::PlayHapticTouchpadEffect(HapticTouchpadEffect effect, HapticTouchpadEffectStrength strength)
{
    NOTREACHED();
}

void EventConverterEvdev::SetHapticTouchpadEffectForNextButtonRelease(HapticTouchpadEffect effect, HapticTouchpadEffectStrength strength)
{
    NOTREACHED();
}
...
void EventConverterEvdev::SetKeyFilter(bool enable_filter, std::vector<DomCode> allowed_keys)
{
    NOTREACHED();
}
...
void EventConverterEvdev::SetPalmSuppressionCallback(const base::RepeatingCallback<void(bool)>& callback)
{
}

void EventConverterEvdev::SetReportStylusStateCallback(const ReportStylusStateCallback& callback)
{
}

void EventConverterEvdev::SetGetLatestStylusStateCallback(const GetLatestStylusStateCallback& callback)
{
}

void EventConverterEvdev::SetReceivedValidInputCallback(ReceivedValidInputCallback callback)
{
}
...
base::TimeTicks EventConverterEvdev::TimeTicksFromInputEvent(const input_event& event)
{
    base::TimeTicks timestamp = ui::EventTimeStampFromSeconds(event.input_event_sec) + base::Microseconds(event.input_event_usec);
...
{
    os << "class=ui::EventConverterEvdev id=" << input_device_.id << std::endl << " path=\"" << path_.value() << "\"" << std::endl << "member ";
