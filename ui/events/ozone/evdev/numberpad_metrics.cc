...
{
    bool result = (input_device.vendor_id == kDynamicNumberpadVendor && input_device.product_id == kDynamicNumberpadProduct);
    return result;
}
...
{
    if (IsNumpadKey(keycode) || IsNumpadEnterKey(keycode) || IsNumpadOtherKey(keycode) || IsNumlockKey(keycode))
        return true;
...
{
    bool result = (input_device.type == INPUT_DEVICE_INTERNAL || input_device.type == INPUT_DEVICE_UNKNOWN);
    return result;
}
...
NumberpadMetricsDelegate::NumberpadMetricsDelegate(const std::string& feature_name)
...
void NumberpadMetricsRecorder::RemoveDevice(const ui::InputDevice& input_device)
{
    if (IsInternalDevice(input_device)) {
        if (IsDeviceDynamicNumberpad(input_device)) {
            dynamic_numpad_present_--;
...
void NumberpadMetricsRecorder::UpdateDeviceState()
{
    ...
    dynamic_enter_keystrokes_metrics_delegate_.SetState(dynamic_numpad_present_ > 0);
    dynamic_non_enter_keystrokes_metrics_delegate_.SetState(dynamic_numpad_present_ > 0);

    internal_enter_keystrokes_metrics_delegate_.SetState(internal_numpad_present_ > 0);
    internal_non_enter_keystrokes_metrics_delegate_.SetState(internal_numpad_present_ > 0);

    external_enter_keystrokes_metrics_delegate_.SetState(true, external_numpad_present_ > 0);
    external_non_enter_keystrokes_metrics_delegate_.SetState(true, external_numpad_present_ > 0);
}
...
void NumberpadMetricsRecorder::SetDynamicNumlockStateOn()
{
    if (dynamic_numlock_state_known_) {
        ...
        if (dynamic_numlock_state_ == false) {
            LOG(ERROR) << "Dynamic numberpad in an unexpected state; numlock is on now.";
...
void NumberpadMetricsRecorder::ProcessKey(unsigned int key, bool down, const InputDevice& input_device)
{
