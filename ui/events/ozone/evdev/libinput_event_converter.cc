...
double GetAxisValue(libinput_event_pointer* const event, const libinput_pointer_axis axis)
{
...
template <typename Value> void LogConfigStatus(const libinput_config_status status, const char* key, const Value value)
{
    switch (status) {
    ...
    case LIBINPUT_CONFIG_STATUS_INVALID:
        LOG(ERROR) << "libinput config: invalid parameter for " << key << ": " << value;
        break;
    }
}
...
LibInputEventConverter::LibInputEvent::LibInputEvent(libinput_event* const event)
    : event_(event)
{
}
...
libinput_event_pointer* LibInputEventConverter::LibInputEvent::PointerEvent() const
{
...
LibInputEventConverter::LibInputDevice::LibInputDevice(int id, libinput_device* const device)
    : device_id_(id)
    , device_(device)
{
    DCHECK(device);
    ...
    DVLOG(2) << "device: \"" << libinput_device_get_name(device_) << "\", capabilities: " << GetCapabilitiesString();
}
...
void LibInputEventConverter::LibInputDevice::ApplySettings(const InputDeviceSettingsEvdev& settings) const
{
...
std::string LibInputEventConverter::LibInputDevice::GetCapabilitiesString()
{
    ...
    std::vector<std::pair<libinput_device_capability, std::string>> caps = { { LIBINPUT_DEVICE_CAP_KEYBOARD, "keyboard" },
        { LIBINPUT_DEVICE_CAP_POINTER, "pointer" }, { LIBINPUT_DEVICE_CAP_TOUCH, "touch" }, { LIBINPUT_DEVICE_CAP_TABLET_TOOL, "tablet-tool" },
        { LIBINPUT_DEVICE_CAP_TABLET_PAD, "tablet-pad" }, { LIBINPUT_DEVICE_CAP_GESTURE, "gesture" }, { LIBINPUT_DEVICE_CAP_SWITCH, "switch" } };
...
void LibInputEventConverter::LibInputDevice::SetNaturalScrollEnabled(const bool enabled) const
{
    const auto status = libinput_device_config_scroll_set_natural_scroll_enabled(device_, enabled);
    LogConfigStatus(status, "natural-scroll", enabled);
}

void LibInputEventConverter::LibInputDevice::SetSensitivity(const int sensitivity) const
{
...
void LibInputEventConverter::LibInputDevice::SetTapToClickEnabled(const bool enabled) const
{
    const auto arg = (enabled ? LIBINPUT_CONFIG_TAP_ENABLED : LIBINPUT_CONFIG_TAP_DISABLED);

    const auto status = libinput_device_config_tap_set_enabled(device_, arg);
    LogConfigStatus(status, "tap-to-click", arg);
}

LibInputEventConverter::LibInputContext::LibInputContext(LibInputContext&& other)
    : li_(other.li_)
{
    other.li_ = nullptr;
}
...
std::optional<LibInputEventConverter::LibInputContext> LibInputEventConverter::LibInputContext::Create()
{
...
std::optional<LibInputEventConverter::LibInputDevice> LibInputEventConverter::LibInputContext::AddDevice(int id, const base::FilePath& path) const
{
...
std::optional<LibInputEventConverter::LibInputEvent> LibInputEventConverter::LibInputContext::NextEvent() const
{
...
int LibInputEventConverter::LibInputContext::OpenRestricted(const char* path, int flags, void* user_data)
{
...
void LibInputEventConverter::LibInputContext::CloseRestricted(int fd, void* user_data)
{
    close(fd);
}

void LibInputEventConverter::LibInputContext::LogHandler(libinput* libinput, enum libinput_log_priority priority, const char* format, va_list args)
{
    switch (priority) {
...
constexpr libinput_interface LibInputEventConverter::LibInputContext::interface_;

std::unique_ptr<LibInputEventConverter> LibInputEventConverter::Create(
    const base::FilePath& path, int id, const EventDeviceInfo& devinfo, CursorDelegateEvdev* cursor, DeviceEventDispatcherEvdev* dispatcher)
{
    ...
    return std::make_unique<LibInputEventConverter>(std::move(context.value()), path, id, devinfo, cursor, dispatcher);
}

LibInputEventConverter::LibInputEventConverter(LibInputEventConverter::LibInputContext&& ctx, const base::FilePath& path, int id,
    const EventDeviceInfo& devinfo, CursorDelegateEvdev* cursor, DeviceEventDispatcherEvdev* dispatcher)
    : EventConverterEvdev(
        ctx.Fd(), path, id, devinfo.device_type(), devinfo.name(), devinfo.phys(), devinfo.vendor_id(), devinfo.product_id(), devinfo.version())
...
void LibInputEventConverter::ApplyDeviceSettings(const InputDeviceSettingsEvdev& settings)
{
    if (device_) {
        device_->ApplySettings(settings);
    } else {
        LOG(ERROR) << "Unable to apply settings due to libinput_path_add_device failure";
    }
}
...
void LibInputEventConverter::HandlePointerMotion(const LibInputEvent& evt)
{
    ...
    dispatcher_->DispatchMouseMoveEvent(
        { input_device_.id, flags, cursor_->GetLocation(), nullptr /*delta*/, PointerDetails(EventPointerType::kMouse), Timestamp(evt) });
}
...
void LibInputEventConverter::HandlePointerButton(const LibInputEvent& evt)
{
    ...
    const bool down = libinput_event_pointer_get_button_state(event) == LIBINPUT_BUTTON_STATE_PRESSED;
    ...
    dispatcher_->DispatchMouseButtonEvent(
        { input_device_.id, flags, cursor_->GetLocation(), button, down, allow_remap, PointerDetails(EventPointerType::kMouse), Timestamp(evt) });
}
...
void LibInputEventConverter::HandlePointerAxis(const LibInputEvent& evt)
{
    ...
    dispatcher_->DispatchScrollEvent({ input_device_.id, EventType::kScroll, cursor_->GetLocation(), delta, delta, 2, Timestamp(evt) });
}
