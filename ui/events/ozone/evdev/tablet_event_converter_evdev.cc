...
TabletEventConverterEvdev::TabletEventConverterEvdev(
    base::ScopedFD fd, base::FilePath path, int id, CursorDelegateEvdev* cursor, const EventDeviceInfo& info, DeviceEventDispatcherEvdev* dispatcher)
    : EventConverterEvdev(fd.get(), path, id, info.device_type(), info.name(), info.phys(), info.vendor_id(), info.product_id(), info.version())
...
{
    TRACE_EVENT1("evdev", "TabletEventConverterEvdev::OnFileCanReadWithoutBlocking", "fd", fd);
...
std::ostream& TabletEventConverterEvdev::DescribeForLog(std::ostream& os) const
{
    os << "class=ui::TabletEventConverterEvdev id=" << input_device_.id << std::endl
...
void TabletEventConverterEvdev::ProcessEvents(const input_event* inputs, int count)
{
...
void TabletEventConverterEvdev::ConvertKeyEvent(const input_event& input)
{
    ...
    if (input.code >= BTN_TOOL_PEN && input.code <= BTN_TOOL_LENS) {
        ...
        if (input.value == 1)
            ...
            LOG(WARNING) << "Unexpected value: " << input.value << " for code: " << input.code;
    }
    ...
    if ((input.code >= BTN_0 && input.code <= BTN_9) || (input.code >= BTN_A && input.code <= BTN_Z)) {
        dispatcher_->DispatchKeyEvent(KeyEventParams { input_device_.id, EF_NONE, input.code, input.code, static_cast<bool>(input.value),
            /*suppress_auto_repeat=*/false, TimeTicksFromInputEvent(input) });
        return;
    }
#endif
}
...
void TabletEventConverterEvdev::DispatchMouseButton(const input_event& input)
{
    ...
    dispatcher_->DispatchMouseButtonEvent(MouseButtonEventParams(input_device_.id, EF_NONE, cursor_->GetLocation(), button, down, MouseButtonMapType::kNone,
...
void TabletEventConverterEvdev::FlushEvents(const input_event& input)
{
    ...
    dispatcher_->DispatchMouseMoveEvent(MouseMoveEventParams(input_device_.id, event_flags, cursor_->GetLocation(),
