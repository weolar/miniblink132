...
class COMPONENT_EXPORT(EVDEV) EventConverterEvdevImpl : public EventConverterEvdev {
public:
    EventConverterEvdevImpl(
        base::ScopedFD fd, base::FilePath path, int id, const EventDeviceInfo& info, CursorDelegateEvdev* cursor, DeviceEventDispatcherEvdev* dispatcher);
...
private:
    ...
    void OnKeyChange(unsigned int key, bool down, const base::TimeTicks& timestamp);
    void ReleaseKeys();
