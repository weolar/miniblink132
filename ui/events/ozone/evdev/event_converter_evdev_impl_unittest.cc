...
public:
    MockEventConverterEvdevImpl(base::ScopedFD fd, const ui::EventDeviceInfo& info, CursorDelegateEvdev* cursor, DeviceEventDispatcherEvdev* dispatcher)
        : EventConverterEvdevImpl(std::move(fd), base::FilePath(kTestDevicePath), 1, info, cursor, dispatcher)
    {
        SetEnabled(true);
    }

    MockEventConverterEvdevImpl(const MockEventConverterEvdevImpl&) = delete;
    MockEventConverterEvdevImpl& operator=(const MockEventConverterEvdevImpl&) = delete;
...
public:
    ...
    EventConverterEvdevImplTest& operator=(const EventConverterEvdevImplTest&) = delete;
    ...
    void SetUpDevice(const ui::EventDeviceInfo& info)
    {
        ...
        keyboard_layout_engine_->SetCustomLookupTableForTesting({
            { ui::DomCode::MICROPHONE_MUTE_TOGGLE, ui::DomKey::MICROPHONE_TOGGLE, ui::DomKey::MICROPHONE_TOGGLE, ui::VKEY_MICROPHONE_MUTE_TOGGLE },
        });
        ...
        event_factory_ = ui::CreateEventFactoryEvdevForTest(cursor_.get(), device_manager_.get(), keyboard_layout_engine_.get(),
            base::BindRepeating(&EventConverterEvdevImplTest::DispatchEventForTest, base::Unretained(this)));
        dispatcher_ = ui::CreateDeviceEventDispatcherEvdevForTest(event_factory_.get());
        device_ = std::make_unique<ui::MockEventConverterEvdevImpl>(std::move(events_in), info, cursor_.get(), dispatcher_.get());
        test_clock_ = std::make_unique<ui::test::ScopedEventTestTickClock>();
    }

    ui::FakeCursorDelegateEvdev* cursor()
    {
        return cursor_.get();
    }
    ui::MockEventConverterEvdevImpl* device()
    {
        return device_.get();
    }
...
public:
    ...
    EventConverterEvdevImplLogTest(const EventConverterEvdevImplLogTest&) = delete;
    EventConverterEvdevImplLogTest& operator=(const EventConverterEvdevImplLogTest&) = delete;

    void SetUp() override
    {
    } // Do not SetUpDevice by default

    void AdjustId(ui::EventDeviceInfo& devinfo, uint16_t input_id::*field, uint16_t value)
    {
        input_id id = { .bustype = devinfo.bustype(), .vendor = devinfo.vendor_id(), .product = devinfo.product_id(), .version = devinfo.version() };
        id.*field = value;
        devinfo.SetId(id);
    }

    void AdjustAxis(ui::EventDeviceInfo& devinfo, unsigned int code, int32_t input_absinfo::*field, int32_t value)
    {
...
class DeferDeviceSetUpEventConverterEvdevImplTest : public EventConverterEvdevImplTest {
public:
    DeferDeviceSetUpEventConverterEvdevImplTest() = default;

    DeferDeviceSetUpEventConverterEvdevImplTest(const DeferDeviceSetUpEventConverterEvdevImplTest&) = delete;
    DeferDeviceSetUpEventConverterEvdevImplTest& operator=(const DeferDeviceSetUpEventConverterEvdevImplTest&) = delete;
...
TEST_F(EventConverterEvdevImplTest, ShouldSwapMouseButtonsFromUserPreference)
{
    ...
    ClearDispatchedEvents();
    GetInputController()->SetPrimaryButtonRight(device()->input_device().id, false);
    dev->ProcessEvents(mock_kernel_queue, std::size(mock_kernel_queue));
    EXPECT_EQ(4u, size());
    ...
    ClearDispatchedEvents();
    GetInputController()->SetPrimaryButtonRight(device()->input_device().id, true);
...
TEST_F(DeferDeviceSetUpEventConverterEvdevImplTest, DisableBlockTelephonyDevicePhoneMute)
{
    // By default Block Phone Mic Mute is enabled. We disable it and validate that
    // events are processed.
    scoped_feature_list_->InitAndDisableFeature(ui::kBlockTelephonyDevicePhoneMute);
    SetUpDevice(ui::EventDeviceInfo());
    ui::MockEventConverterEvdevImpl* dev = device();
...
TEST_F(DeferDeviceSetUpEventConverterEvdevImplTest, EnableBlockTelephonyDevicePhoneMute)
{
    // We enable the flag it and validate that events are not processed.
    scoped_feature_list_->InitAndEnableFeature(ui::kBlockTelephonyDevicePhoneMute);
    SetUpDevice(ui::EventDeviceInfo());
    ui::MockEventConverterEvdevImpl* dev = device();
...
TEST_F(EventConverterEvdevImplLogTest, ChangeProduct)
{
    ...
    std::string log = LogSubst(kDefaultDeviceLogDescription, "product_id", "0105");
    Check(devinfo, log);
}
...
TEST_F(EventConverterEvdevImplLogTest, ChangeName)
{
    ...
    std::string log = LogSubst(kDefaultDeviceLogDescription, "name", "\"changed the name\"");
    Check(devinfo, log);
}
...
TEST_F(EventConverterEvdevImplLogTest, ChangeInputDeviceType)
{
    ...
    std::string log = LogSubst(kDefaultDeviceLogDescription, "input_device_type", "ui::InputDeviceType::INPUT_DEVICE_USB");
    Check(devinfo, log);
}
...
TEST_F(EventConverterEvdevImplLogTest, ChangeKeyboardImposter)
{
    ...
    std::stringstream output;
    device()->DescribeForLog(output);
    std::string log = LogSubst(kDefaultDeviceLogDescription, "suspected_keyboard_imposter", "1");
    EXPECT_EQ(output.str(), log);
}
...
TEST_F(EventConverterEvdevImplLogTest, ChangeMouseImposter)
{
    ...
    std::stringstream output;
    device()->DescribeForLog(output);
    std::string log = LogSubst(kDefaultDeviceLogDescription, "suspected_mouse_imposter", "1");
    EXPECT_EQ(output.str(), log);
}
...
TEST_F(EventConverterEvdevImplLogTest, ChangeKeyboardType)
{
    ...
    std::string log = LogSubst(kDefaultDeviceLogDescription, "keyboard_type", "ui::KeyboardType::VALID_KEYBOARD");
    Check(devinfo, log);
}
...
TEST_F(EventConverterEvdevImplLogTest, ChangeCapslockLED)
{
    ...
    std::string log = LogSubst(kDefaultDeviceLogDescription, "has_caps_lock_led", "1");
    Check(devinfo, log);
}
