...
std::string LogSubst(std::string description, std::string key, std::string replacement)
{
...
public:
    FakeHeatmapPalmDetector()
    {
        palm_tracking_ids_.clear();
    }

    void Start(ModelId model_id, std::string_view hidraw_path, std::optional<CropHeatmap> crop_heatmap) override
    {
    }
    ...
    void AddTouchRecord(base::Time timestamp, const std::vector<int>& tracking_ids) override
    {
    }
    ...
    void SetPalm(int tracking_id)
    {
        palm_tracking_ids_.insert(tracking_id);
    }
...
public:
    MockTouchEventConverterEvdev(
        ...
        base::ScopedFD fd, base::FilePath path, const EventDeviceInfo& devinfo, SharedPalmDetectionFilterState* shared_palm_state,
        DeviceEventDispatcherEvdev* dispatcher);

    MockTouchEventConverterEvdev(const MockTouchEventConverterEvdev&) = delete;
    MockTouchEventConverterEvdev& operator=(const MockTouchEventConverterEvdev&) = delete;

    ~MockTouchEventConverterEvdev() override;

    void ConfigureReadMock(struct input_event* queue, long read_this_many, long queue_index);
    ...
    FalseTouchFinder* false_touch_finder()
    {
        return false_touch_finder_.get();
    }
    InProgressTouchEvdev& event(int slot)
    {
        return events_[slot];
    }
...
public:
    MockDeviceEventDispatcherEvdev(const base::RepeatingCallback<void(const GenericEventParams& params)>& callback)
        : callback_(callback)
    {
    }
    ...
    void DispatchKeyboardDevicesUpdated(const std::vector<KeyboardDevice>& devices, base::flat_map<int, std::vector<uint64_t>> key_bits_mapping) override
    {
    }
    void DispatchTouchscreenDevicesUpdated(const std::vector<TouchscreenDevice>& devices) override
    {
    }
    void DispatchMouseDevicesUpdated(const std::vector<InputDevice>& devices, bool has_mouse) override
    {
    }
    void DispatchPointingStickDevicesUpdated(const std::vector<InputDevice>& devices) override
    {
    }
    void DispatchTouchpadDevicesUpdated(const std::vector<TouchpadDevice>& devices, bool has_haptic_touchpad) override
    {
    }
    void DispatchGraphicsTabletDevicesUpdated(const std::vector<InputDevice>& devices) override
    {
    }
    void DispatchUncategorizedDevicesUpdated(const std::vector<InputDevice>& devices) override
    {
    }
    ...
    void DispatchGamepadDevicesUpdated(const std::vector<GamepadDevice>& devices, base::flat_map<int, std::vector<uint64_t>> key_bits_mapping) override
    {
    }
...
MockTouchEventConverterEvdev::MockTouchEventConverterEvdev(base::ScopedFD fd, base::FilePath path, const EventDeviceInfo& devinfo,
    SharedPalmDetectionFilterState* shared_palm_state, DeviceEventDispatcherEvdev* dispatcher)
    : TouchEventConverterEvdev(std::move(fd), path, 1, devinfo, shared_palm_state, dispatcher)
{
    int fds[2];
    ...
    EXPECT_TRUE(base::SetNonBlocking(fds[0]) || base::SetNonBlocking(fds[1])) << "failed to set non-blocking: " << strerror(errno);
...
void MockTouchEventConverterEvdev::ConfigureReadMock(struct input_event* queue, long read_this_many, long queue_index)
{
    int nwrite = HANDLE_EINTR(write(write_pipe_, queue + queue_index, sizeof(struct input_event) * read_this_many));
    DPCHECK(nwrite == static_cast<int>(sizeof(struct input_event) * read_this_many)) << "write() failed";
}
...
public:
    ...
    TouchEventConverterEvdevTest& operator=(const TouchEventConverterEvdevTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        scoped_feature_list_->InitWithFeatures({ kEnablePalmOnMaxTouchMajor, kEnablePalmOnToolTypePalm }, { kEnableSingleCancelTouch });
        SetUpDevice();
    }
    ...
    void SetUpDevice()
    {
        ...
        dispatcher_ = std::make_unique<ui::MockDeviceEventDispatcherEvdev>(
            base::BindRepeating(&TouchEventConverterEvdevTest::DispatchCallback, base::Unretained(this)));
        device_ = std::make_unique<ui::MockTouchEventConverterEvdev>(
            std::move(events_in), base::FilePath(kTestDevicePath), devinfo, shared_palm_state_.get(), dispatcher_.get());
    ...
    ui::MockTouchEventConverterEvdev* device()
    {
        return device_.get();
    }
    ...
    ui::MockDeviceEventDispatcherEvdev* dispatcher()
    {
        return dispatcher_.get();
    }
    unsigned size()
    {
        return dispatched_events_.size();
    }
    ...
    {
        DCHECK_GT(dispatched_events_.size(), index);
        EXPECT_EQ(GenericEventParamsType::MOUSE_MOVE, dispatched_events_[index].type);
        return dispatched_events_[index].mouse_move;
    }
    const ui::MouseButtonEventParams& dispatched_mouse_button_event(unsigned index)
    {
        DCHECK_GT(dispatched_events_.size(), index);
        EXPECT_EQ(GenericEventParamsType::MOUSE_BUTTON, dispatched_events_[index].type);
    ...
    {
        base::TimeTicks ticks = base::TimeTicks() + base::Seconds(time.tv_sec) + base::Microseconds(time.tv_usec);
...
TEST_F(TouchEventConverterEvdevTest, TouchMove)
{
    ...
    // Press.
    dev->ConfigureReadMock(mock_kernel_queue_press, std::size(mock_kernel_queue_press), 0);
    dev->ReadNow();
    EXPECT_EQ(1u, size());
    ...
    // Move.
    dev->ConfigureReadMock(mock_kernel_queue_move, std::size(mock_kernel_queue_move), 0);
    dev->ReadNow();
    EXPECT_EQ(2u, size());
    ...
    // Release.
    dev->ConfigureReadMock(mock_kernel_queue_release, std::size(mock_kernel_queue_release), 0);
    dev->ReadNow();
    EXPECT_EQ(3u, size());
...
TEST_F(TouchEventConverterEvdevTest, TwoFingerGesture)
{
    ...
    struct input_event mock_kernel_queue_press0[]
        = { { { 0, 0 }, EV_ABS, ABS_MT_TRACKING_ID, 684 }, { { 0, 0 }, EV_ABS, ABS_MT_TOUCH_MAJOR, 3 }, { { 0, 0 }, EV_ABS, ABS_MT_PRESSURE, 45 },
              { { 0, 0 }, EV_ABS, ABS_MT_POSITION_X, 42 }, { { 0, 0 }, EV_ABS, ABS_MT_POSITION_Y, 51 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 } };
    // Setup and discard a press.
    dev->ConfigureReadMock(mock_kernel_queue_press0, 6, 0);
    dev->ReadNow();
    EXPECT_EQ(1u, size());

    struct input_event mock_kernel_queue_move0[] = { { { 0, 0 }, EV_ABS, ABS_MT_POSITION_X, 40 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 } };
    ...
    struct input_event mock_kernel_queue_move0press1[]
        = { { { 0, 0 }, EV_ABS, ABS_MT_POSITION_X, 40 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, { { 0, 0 }, EV_ABS, ABS_MT_SLOT, 1 },
              { { 0, 0 }, EV_ABS, ABS_MT_TRACKING_ID, 686 }, { { 0, 0 }, EV_ABS, ABS_MT_TOUCH_MAJOR, 3 }, { { 0, 0 }, EV_ABS, ABS_MT_PRESSURE, 45 },
              { { 0, 0 }, EV_ABS, ABS_MT_POSITION_X, 101 }, { { 0, 0 }, EV_ABS, ABS_MT_POSITION_Y, 102 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 } };
    ...
    struct input_event mock_kernel_queue_stationary0_move1[] = { { { 0, 0 }, EV_ABS, ABS_MT_POSITION_X, 40 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 } };
    ...
    struct input_event mock_kernel_queue_move0_stationary1[]
        = { { { 0, 0 }, EV_ABS, ABS_MT_SLOT, 0 }, { { 0, 0 }, EV_ABS, ABS_MT_POSITION_X, 39 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 } };
    ...
    // Release 0, move 1.
    struct input_event mock_kernel_queue_release0_move1[] = { { { 0, 0 }, EV_ABS, ABS_MT_TRACKING_ID, -1 }, { { 0, 0 }, EV_ABS, ABS_MT_SLOT, 1 },
...
TEST_F(TouchEventConverterEvdevTest, Unsync)
{
    ...
    struct input_event mock_kernel_queue_move0[]
        = { { { 0, 0 }, EV_SYN, SYN_DROPPED, 0 }, { { 0, 0 }, EV_ABS, ABS_MT_POSITION_X, 40 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 } };
    ...
    struct input_event mock_kernel_queue_move1[] = { { { 0, 0 }, EV_ABS, ABS_MT_POSITION_X, 40 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 } };
...
TEST_F(TouchEventConverterEvdevTest, ShouldResumeExistingContactsOnStart)
{
    ...
    dev->ConfigureReadMock(mock_kernel_queue_empty_report, std::size(mock_kernel_queue_empty_report), 0);
    dev->ReadNow();
    EXPECT_EQ(1u, size());
...
TEST_F(TouchEventConverterEvdevTest, ToolTypePalmNotCancelTouch)
{
    ...
    dev->ConfigureReadMock(mock_kernel_queue_max_major, std::size(mock_kernel_queue_max_major), 0);
    dev->ReadNow();
    EXPECT_EQ(6u, size());
...
TEST_F(TouchEventConverterEvdevTest, PalmShouldCancelTouch)
{
    ...
    dev->ConfigureReadMock(mock_kernel_queue_tool_palm, std::size(mock_kernel_queue_tool_palm), 0);
    dev->ReadNow();
    EXPECT_EQ(8u, size());
...
TEST_F(TouchEventConverterEvdevTest, TrackingIdShouldNotResetCancelByPalm)
{
    ...
    dev->ConfigureReadMock(mock_kernel_new_touch_without_new_major, std::size(mock_kernel_new_touch_without_new_major), 0);
    dev->ReadNow();
    EXPECT_EQ(4u, size());
...
TEST_F(TouchEventConverterEvdevTest, TrackingIdShouldNotResetCancelByPalmSingleCancel)
{
    // Flip field to true.
    scoped_feature_list_ = std::make_unique<base::test::ScopedFeatureList>();
...
// crbug.com/407386
TEST_F(TouchEventConverterEvdevTest, DontChangeMultitouchPositionFromLegacyAxes)
{
    ui::MockTouchEventConverterEvdev* dev = device();
...
public:
    ...
    EventTypeTouchNoiseFilter& operator=(const EventTypeTouchNoiseFilter&) = delete;
    ...
    // TouchFilter:
    void Filter(const std::vector<InProgressTouchEvdev>& touches, base::TimeTicks time, std::bitset<kNumTouchEvdevSlots>* slots_with_noise) override
    {
...
private:
    ...
    {
        if (touch.touching)
            return touch.was_touching ? EventType::kTouchMoved : EventType::kTouchPressed;
...
class TouchEventConverterEvdevTouchNoiseTest : public TouchEventConverterEvdevTest {
public:
    TouchEventConverterEvdevTouchNoiseTest()
    {
    }

    TouchEventConverterEvdevTouchNoiseTest(const TouchEventConverterEvdevTouchNoiseTest&) = delete;
    TouchEventConverterEvdevTouchNoiseTest& operator=(const TouchEventConverterEvdevTouchNoiseTest&) = delete;
    ...
    void SetUp() override
    {
        base::CommandLine::ForCurrentProcess()->AppendSwitch(switches::kEdgeTouchFiltering);
...
TEST_F(TouchEventConverterEvdevTest, ActiveStylusTouchAndRelease)
{
    ...
    base::MockCallback<EventConverterEvdev::ReportStylusStateCallback> mock_report_callback;
    // The 1st and 2nd SYN_REPORTs share the same x and y.
    EXPECT_CALL(mock_report_callback, Run(VeirfyInProgressTouchEvdev(9170, 3658), 100, 100, base::TimeTicks())).Times(2);
    // The 3rd and 4th SYN_REPORTs share the same x and y.
    EXPECT_CALL(mock_report_callback, Run(VeirfyInProgressTouchEvdev(9173, 3906), 100, 100, base::TimeTicks())).Times(2);
    dev->SetReportStylusStateCallback(mock_report_callback.Get());

    struct input_event mock_kernel_queue[] {
        { { 0, 0 }, EV_KEY, BTN_TOOL_PEN, 1 }, { { 0, 0 }, EV_ABS, ABS_X, 9170 }, { { 0, 0 }, EV_ABS, ABS_Y, 3658 }, { { 0, 0 }, EV_ABS, ABS_TILT_X, 0 },
            { { 0, 0 }, EV_ABS, ABS_TILT_Y, 0 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, { { 0, 0 }, EV_KEY, BTN_TOUCH, 1 },
            { { 0, 0 }, EV_ABS, ABS_PRESSURE, 60 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, { { 0, 0 }, EV_KEY, BTN_TOUCH, 0 }, { { 0, 0 }, EV_ABS, ABS_X, 9173 },
            { { 0, 0 }, EV_ABS, ABS_Y, 3906 }, { { 0, 0 }, EV_ABS, ABS_TILT_X, 30 }, { { 0, 0 }, EV_ABS, ABS_TILT_Y, 50 },
            { { 0, 0 }, EV_ABS, ABS_PRESSURE, 0 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, { { 0, 0 }, EV_KEY, BTN_TOOL_PEN, 0 },
            { { 0, 0 }, EV_SYN, SYN_REPORT, 0 },
    };
...
TEST_F(TouchEventConverterEvdevTest, ActiveStylusMotion)
{
    ...
    struct input_event mock_kernel_queue[] {
        { { 0, 0 }, EV_KEY, BTN_TOOL_PEN, 1 }, { { 0, 0 }, EV_ABS, ABS_X, 8921 }, { { 0, 0 }, EV_ABS, ABS_Y, 1072 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 },
            { { 0, 0 }, EV_KEY, BTN_TOUCH, 1 }, { { 0, 0 }, EV_ABS, ABS_PRESSURE, 35 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, { { 0, 0 }, EV_ABS, ABS_X, 8934 },
            { { 0, 0 }, EV_ABS, ABS_Y, 981 }, { { 0, 0 }, EV_ABS, ABS_PRESSURE, 184 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, { { 0, 0 }, EV_ABS, ABS_X, 8930 },
            { { 0, 0 }, EV_ABS, ABS_Y, 980 }, { { 0, 0 }, EV_ABS, ABS_PRESSURE, 348 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, { { 0, 0 }, EV_KEY, BTN_TOUCH, 0 },
            { { 0, 0 }, EV_ABS, ABS_PRESSURE, 0 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 }, { { 0, 0 }, EV_KEY, BTN_TOOL_PEN, 0 },
            { { 0, 0 }, EV_SYN, SYN_REPORT, 0 },
    };
...
TEST_F(TouchEventConverterEvdevTest, ActiveStylusBarrelButtonWhileHovering)
{
    ...
    struct input_event mock_kernel_queue[] {
        // Hover
        { { 0, 0 }, EV_KEY, BTN_TOOL_PEN, 1 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 },

            // Button pressed
            { { 0, 0 }, EV_KEY, BTN_STYLUS, 1 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 },
            ...
            // Releasing button
            { { 0, 0 }, EV_KEY, BTN_STYLUS, 0 }, { { 0, 0 }, EV_SYN, SYN_REPORT, 0 },
...
TEST_F(TouchEventConverterEvdevTest, ActiveStylusBarrelButton)
{
    ...
    EXPECT_EQ(EventPointerType::kPen, button_down_event.pointer_details.pointer_type);
    ...
    EXPECT_EQ(EventPointerType::kPen, button_up_event.pointer_details.pointer_type);
...
TEST_F(TouchEventConverterEvdevTest, HeldEventNotSent)
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        SetTestNowTime(time);
        device()->ConfigureReadMock(mock_kernel_queue, std::size(mock_kernel_queue), 0);
        device()->ReadNow();
    }
    SetTestNowTime(time);
    ...
    device()->ReadNow();
    EXPECT_EQ(4u, size());
    const base::TimeTicks base_ticks = base::TimeTicks() + base::Seconds(time.tv_sec);
    ...
    EXPECT_THAT(histogram_tester_.GetAllSamples(TouchEventConverterEvdev::kHoldCountAtReleaseEventName), testing::ElementsAre(base::Bucket(3, 1)));
    EXPECT_THAT(histogram_tester_.GetAllSamples(TouchEventConverterEvdev::kHoldCountAtCancelEventName), testing::ElementsAre());
    EXPECT_THAT(histogram_tester_.GetAllSamples(TouchEventConverterEvdev::kPalmFilterTimerEventName), testing::SizeIs(1));
}
...
TEST_F(TouchEventConverterEvdevTest, HeldThenEnd)
{
    ...
    // Press.
    device()->ConfigureReadMock(mock_kernel_queue_release, std::size(mock_kernel_queue_release), 0);
    device()->ReadNow();
    EXPECT_EQ(4u, size());
...
TEST_F(TouchEventConverterEvdevTest, SentHeldThenPalm)
{
    ...
    EXPECT_THAT(histogram_tester_.GetAllSamples(TouchEventConverterEvdev::kHoldCountAtReleaseEventName), testing::ElementsAre());
    EXPECT_THAT(histogram_tester_.GetAllSamples(TouchEventConverterEvdev::kHoldCountAtCancelEventName), testing::ElementsAre(base::Bucket(5, 1)));
}
...
TEST_F(TouchEventConverterEvdevTest, HeldThenPalm)
{
    ...
    EXPECT_FALSE(device()->event(0).held);
    EXPECT_THAT(histogram_tester_.GetAllSamples(TouchEventConverterEvdev::kHoldCountAtReleaseEventName), testing::ElementsAre());
    EXPECT_THAT(histogram_tester_.GetAllSamples(TouchEventConverterEvdev::kHoldCountAtCancelEventName), testing::ElementsAre(base::Bucket(4, 1)));
}
...
TEST_F(TouchEventConverterEvdevTest, ScalePressure)
{
    ...
    struct input_event mock_kernel_queue[] = {
        ...
        { time, EV_ABS, ABS_MT_POSITION_X, 1795 },
        { time, EV_ABS, ABS_MT_POSITION_Y, 5559 },
        { time, EV_ABS, ABS_MT_PRESSURE, devinfo.GetAbsMaximum(ABS_MT_PRESSURE) * 2 },
        { time, EV_ABS, ABS_MT_TOUCH_MAJOR, 14 },
        { time, EV_ABS, ABS_MT_TOUCH_MINOR, 14 },
        { time, EV_KEY, BTN_TOUCH, 1 },
        { time, EV_ABS, ABS_X, 1795 },
...
TEST_F(TouchEventConverterEvdevTest, FingerSizeWithResolution)
{
    ...
    EXPECT_EQ(0, event.slot);
    EXPECT_FLOAT_EQ(217.0 / devinfo.GetAbsMaximum(ABS_MT_PRESSURE), event.pointer_details.force);
    EXPECT_EQ(EventPointerType::kTouch, event.pointer_details.pointer_type);
...
TEST_F(TouchEventConverterEvdevTest, AbsTiltXY)
{
    ...
    input_absinfo absinfo_x = { .minimum = -100, .maximum = 100, .resolution = 123 };
...
TEST_F(TouchEventConverterEvdevTest, AbsPositionXY)
{
    ...
    input_absinfo absinfo_x = { .minimum = -200, .maximum = 400, .resolution = 1230 };
    input_absinfo absinfo_y = { .minimum = 100, .maximum = 390, .resolution = 2560 };
...
TEST_F(TouchEventConverterEvdevTest, AbsMtPositionXY)
{
    ...
    input_absinfo absinfo_x = { .minimum = -250, .maximum = 450, .resolution = 1000 };
    input_absinfo absinfo_y = { .minimum = 1000, .maximum = 3900, .resolution = 250 };
...
TEST_F(TouchEventConverterEvdevTest, ChangePalmOnTouchMajorMax)
{
    TearDownDevice();
    scoped_feature_list_ = std::make_unique<base::test::ScopedFeatureList>();
    scoped_feature_list_->InitWithFeatures({ kEnablePalmOnToolTypePalm }, { kEnableSingleCancelTouch, kEnablePalmOnMaxTouchMajor });
    SetUpDevice();
...
TEST_F(TouchEventConverterEvdevTest, ChangePalmOnTool)
{
    TearDownDevice();
    scoped_feature_list_ = std::make_unique<base::test::ScopedFeatureList>();
    scoped_feature_list_->InitWithFeatures({ kEnablePalmOnMaxTouchMajor }, { kEnableSingleCancelTouch, kEnablePalmOnToolTypePalm });
    SetUpDevice();
...
TEST_F(TouchEventConverterEvdevTest, HeatmapPalmRejection)
{
    ...
    scoped_feature_list_->InitWithFeatures(
        { kEnableHeatmapPalmDetection }, { kEnablePalmOnMaxTouchMajor, kEnablePalmOnToolTypePalm, kEnableSingleCancelTouch });
    SetUpDevice();
...
TEST_F(TouchEventConverterEvdevTest, RecordFingerSessionMetrics)
{
    ...
    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kTouchSessionCountEventName, 0);
    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kTouchSessionLengthEventName, 0);

    task_environment_.FastForwardBy(base::Milliseconds(5100));

    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kTouchSessionCountEventName, 1);
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchSessionLengthEventName, 2000 /*ms*/, 1);
    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kStylusSessionCountEventName, 0);
    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kStylusSessionLengthEventName, 0);
}
...
TEST_F(TouchEventConverterEvdevTest, RecordStylusSessionMetrics)
{
    ...
    SetTestNowTime(time0);
    dev->ConfigureReadMock(mock_kernel_queue_stylus_down, std::size(mock_kernel_queue_stylus_down), 0);
    dev->ReadNow();
    ...
    SetTestNowTime(time2);
    dev->ConfigureReadMock(mock_kernel_queue_stylus_up, std::size(mock_kernel_queue_stylus_up), 0);
    dev->ReadNow();

    SetTestNowTime(time3);
    dev->ConfigureReadMock(mock_kernel_queue_stylus_down_again, std::size(mock_kernel_queue_stylus_down_again), 0);
    dev->ReadNow();

    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kStylusSessionCountEventName, 0);
    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kStylusSessionLengthEventName, 0);
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchGapBeforeStylusEventName, 10000 /*ms*/, 2);
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchTypeBeforeStylusEventName, 0 /*kNone*/, 2);
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchGapAfterStylusEventName, 10000 /*ms*/, 1);
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchTypeAfterStylusEventName, 0 /*kNone*/, 1);

    task_environment_.FastForwardBy(base::Milliseconds(5100));

    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kStylusSessionCountEventName, 1);
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kStylusSessionLengthEventName, 3000 /*ms*/, 1);
    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kTouchSessionCountEventName, 0);
    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kTouchSessionLengthEventName, 0);
}
...
TEST_F(TouchEventConverterEvdevTest, RecordPalmTouchCountMetrics)
{
    ...
    SetTestNowTime(time0);
    dev->ConfigureReadMock(mock_kernel_queue_finger, std::size(mock_kernel_queue_finger), 0);
    dev->ReadNow();

    SetTestNowTime(time1);
    dev->ConfigureReadMock(mock_kernel_queue_palm1, std::size(mock_kernel_queue_palm1), 0);
    dev->ReadNow();

    SetTestNowTime(time2);
    dev->ConfigureReadMock(mock_kernel_queue_palm2, std::size(mock_kernel_queue_palm2), 0);
    dev->ReadNow();
    ...
    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kPalmTouchCountEventName, 1);

    task_environment_.FastForwardBy(base::Milliseconds(5100));

    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kTouchSessionCountEventName, 1);
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchSessionLengthEventName, 200 /*ms*/, 1);
}
...
TEST_F(TouchEventConverterEvdevTest, RecordRepeatedTouchCountMetrics)
{
    ...
    SetTestNowTime(time2);
    dev->ConfigureReadMock(mock_kernel_queue_palm, std::size(mock_kernel_queue_palm), 0);
    dev->ReadNow();
    ...
    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kPalmTouchCountEventName, 1);
    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kRepeatedTouchCountEventName, 0);

    SetTestNowTime(time4);
    dev->ConfigureReadMock(mock_kernel_queue_repeated_touch, std::size(mock_kernel_queue_repeated_touch), 0);
    dev->ReadNow();

    histogram_tester_.ExpectTotalCount(TouchEventConverterEvdev::kRepeatedTouchCountEventName, 1);
}
...
TEST_F(TouchEventConverterEvdevTest, RecordFingerBeforeStylus)
{
    ...
    std::unique_ptr<ui::MockTouchEventConverterEvdev> stylus
        = std::make_unique<ui::MockTouchEventConverterEvdev>(std::move(events_in), base::FilePath(kTestDevicePath), devinfo, shared_palm_state(), dispatcher());
    ...
    SetTestNowTime(time0);
    touch_screen->ConfigureReadMock(mock_kernel_queue_finger_press, std::size(mock_kernel_queue_finger_press), 0);
    touch_screen->ReadNow();

    SetTestNowTime(time1);
    stylus->ConfigureReadMock(mock_kernel_queue_stylus_down, std::size(mock_kernel_queue_stylus_down), 0);
    stylus->ReadNow();

    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchGapBeforeStylusEventName, 100 /*ms*/, 1);
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchTypeBeforeStylusEventName, 1 /*kFinger*/, 1);
}
...
TEST_F(TouchEventConverterEvdevTest, RecordPalmBeforeStylus)
{
    ...
    SetTestNowTime(time0);
    touch_screen->ConfigureReadMock(mock_kernel_queue_palm_press, std::size(mock_kernel_queue_palm_press), 0);
    touch_screen->ReadNow();
    ...
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchGapBeforeStylusEventName, 200 /*ms*/, 1);
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchTypeBeforeStylusEventName, 2 /*kPalm*/, 1);
}
...
TEST_F(TouchEventConverterEvdevTest, RecordFingerAfterStylus)
{
    ...
    SetTestNowTime(time1);
    stylus->ConfigureReadMock(mock_kernel_queue_stylus_up, std::size(mock_kernel_queue_stylus_up), 0);
    stylus->ReadNow();

    SetTestNowTime(time2);
    touch_screen->ConfigureReadMock(mock_kernel_queue_finger, std::size(mock_kernel_queue_finger), 0);
    touch_screen->ReadNow();

    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchGapAfterStylusEventName, 400 /*ms*/, 1);
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchTypeAfterStylusEventName, 1 /*kFinger*/, 1);
}
...
TEST_F(TouchEventConverterEvdevTest, RecordPalmAfterStylus)
{
    ...
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchGapAfterStylusEventName, 400 /*ms*/, 1);
    histogram_tester_.ExpectUniqueSample(TouchEventConverterEvdev::kTouchTypeAfterStylusEventName, 2 /*kPalm*/, 1);
}
} // namespace ui
