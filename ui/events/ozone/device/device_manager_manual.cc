...
{
    base::FileEnumerator file_enum(base::FilePath(kDevInput), false, base::FileEnumerator::FILES, FILE_PATH_LITERAL("event*[0-9]"));
...
DeviceManagerManual::DeviceManagerManual()
    ...
    , watcher_(new base::FilePathWatcher, base::OnTaskRunnerDeleter(blocking_task_runner_))
{
}
...
void DeviceManagerManual::StartWatching()
{
    blocking_task_runner_->PostTaskAndReplyWithResult(FROM_HERE,
        base::BindOnce(&base::FilePathWatcher::Watch, base::Unretained(watcher_.get()), base::FilePath(kDevInput), base::FilePathWatcher::Type::kNonRecursive,
            base::BindRepeating(
                &DeviceManagerManual::OnWatcherEventOnUiSequence, base::SequencedTaskRunner::GetCurrentDefault(), weak_ptr_factory_.GetWeakPtr())),
...
void DeviceManagerManual::InitiateScanDevices()
{
    ...
    base::ThreadPool::PostTaskAndReply(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN },
        base::BindOnce(&ScanDevicesOnWorkerThread, result),
        base::BindOnce(&DeviceManagerManual::OnDevicesScanned, weak_ptr_factory_.GetWeakPtr(), base::Owned(result)));
}

void DeviceManagerManual::OnDevicesScanned(std::vector<base::FilePath>* result)
{
...
void DeviceManagerManual::OnWatcherEvent(const base::FilePath& path, bool error)
{
...
void DeviceManagerManual::OnWatcherEventOnUiSequence(
    scoped_refptr<base::TaskRunner> ui_thread_runner, base::WeakPtr<DeviceManagerManual> device_manager, const base::FilePath& path, bool error)
{
    ui_thread_runner->PostTask(FROM_HERE, BindOnce(&DeviceManagerManual::OnWatcherEvent, device_manager, path, error));
}
