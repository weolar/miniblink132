...
device::ScopedUdevMonitorPtr UdevCreateMonitor(struct udev* udev)
{
    ...
    if (monitor) {
        for (size_t i = 0; i < std::size(kSubsystems); ++i)
            device::udev_monitor_filter_add_match_subsystem_devtype(monitor, kSubsystems[i], nullptr);
...
void DeviceManagerUdev::CreateMonitor()
{
    ...
    if (monitor_) {
        ...
        CHECK_GT(fd, 0);
        base::CurrentUIThread::Get()->WatchFileDescriptor(fd, true, base::MessagePumpForUI::WATCH_READ, &controller_, this);
    }
}
...
void DeviceManagerUdev::ScanDevices(DeviceEventObserver* observer)
{
    ...
    {
        device::ScopedUdevDevicePtr device(device::udev_device_new_from_syspath(udev_.get(), device::udev_list_entry_get_name(entry)));
        if (!device)
            continue;
...
void DeviceManagerUdev::OnFileCanReadWithoutBlocking(int fd)
{
    ...
    device::ScopedUdevDevicePtr device(device::udev_monitor_receive_device(monitor_.get()));
    if (!device)
        return;
...
std::unique_ptr<DeviceEvent> DeviceManagerUdev::ProcessMessage(udev_device* device)
{
    ...
    udev_list_entry* property_list = device::udev_device_get_properties_list_entry(device);
    udev_list_entry* entry;
    udev_list_entry_foreach(entry, property_list)
    {
        const std::string key(device::udev_list_entry_get_name(entry));
        const std::string value(device::udev_device_get_property_value(device, key.c_str()));
        property_map.insert({ key, value });
    }

    return std::make_unique<DeviceEvent>(device_type, action_type, base::FilePath(path), property_map);
}
