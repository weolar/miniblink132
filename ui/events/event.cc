...
constexpr int kChangedButtonFlagMask = EF_LEFT_MOUSE_BUTTON | EF_MIDDLE_MOUSE_BUTTON | EF_RIGHT_MOUSE_BUTTON | EF_BACK_MOUSE_BUTTON | EF_FORWARD_MOUSE_BUTTON;
...
bool IsRepeatedClickTimes(const base::TimeTicks& time_stamp1, const base::TimeTicks& time_stamp2)
{
...
{
    return base::StrCat({ GetName(), " time_stamp=", base::NumberToString(time_stamp_.since_origin().InSecondsF()),
...
Event::Event(const Event& copy)
    ...
    , flags_(copy.flags_)
    , native_event_(ShouldCopyPlatformEvents() ? copy.native_event_ : CreateInvalidPlatformEvent())
    , source_device_id_(copy.source_device_id_)
    , properties_(copy.properties_ ? std::make_unique<Properties>(*copy.properties_) : nullptr)
{
}
...
Event& Event::operator=(const Event& rhs)
{
    if (this != &rhs) {
        ...
        flags_ = rhs.flags_;
        native_event_ = ShouldCopyPlatformEvents() ? rhs.native_event_ : CreateInvalidPlatformEvent();
...
LocatedEvent::LocatedEvent(const PlatformEvent& native_event)
    : Event(native_event, EventTypeFromNative(native_event), EventFlagsFromNative(native_event))
    , location_(EventLocationFromNative(native_event))
    , root_location_(location_)
{
}

LocatedEvent::LocatedEvent(EventType type, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, int flags)
...
void LocatedEvent::UpdateForRootTransform(const gfx::Transform& reversed_root_transform, const gfx::Transform& reversed_local_transform)
{
    if (target()) {
...
{
    return base::StrCat({ Event::ToString(), " location=", location_.ToString(), " root_location=", root_location_.ToString() });
}
...
{
    latency()->AddLatencyNumberWithTimestamp(INPUT_EVENT_LATENCY_ORIGINAL_COMPONENT, time_stamp());
    latency()->AddLatencyNumber(INPUT_EVENT_LATENCY_UI_COMPONENT);
    InitializeNative();
}

MouseEvent::MouseEvent(EventType type, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, int flags,
    int changed_button_flags, const PointerDetails& pointer_details)
    ...
    , pointer_details_(pointer_details)
{
    ...
    DCHECK_EQ(changed_button_flags_, changed_button_flags_ & kChangedButtonFlagMask);
    latency()->AddLatencyNumber(INPUT_EVENT_LATENCY_UI_COMPONENT);
...
MouseEvent::MouseEvent(EventType type, const gfx::Point& location, const gfx::Point& root_location, base::TimeTicks time_stamp, int flags,
    int changed_button_flags, const PointerDetails& pointer_details)
    : MouseEvent(type, gfx::PointF(location), gfx::PointF(root_location), time_stamp, flags, changed_button_flags, pointer_details)
{
}
...
// static
bool MouseEvent::IsRepeatedClickEvent(const MouseEvent& event1, const MouseEvent& event2)
{
    // These values match the Windows defaults.
    static const int kDoubleClickWidth = 4;
    static const int kDoubleClickHeight = 4;

    if (event1.type() != EventType::kMousePressed || event2.type() != EventType::kMousePressed) {
        return false;
    }
...
MouseWheelEvent::MouseWheelEvent(const ScrollEvent& scroll_event)
    ...
    , offset_(base::ClampRound(scroll_event.x_offset()), base::ClampRound(scroll_event.y_offset()))
{
    SetType(EventType::kMousewheel);
}

MouseWheelEvent::MouseWheelEvent(const MouseEvent& mouse_event, int x_offset, int y_offset)
...
MouseWheelEvent::MouseWheelEvent(const gfx::Vector2d& offset, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp,
    int flags, int changed_button_flags, const std::optional<gfx::Vector2d> tick_120ths)
    : MouseEvent(EventType::kUnknown, location, root_location, time_stamp, flags, changed_button_flags)
    , offset_(offset)
{
    ...
    if (!tick_120ths) {
        ...
        tick_120ths_ = gfx::Vector2d(offset_.x() / MouseWheelEvent::kWheelDelta * 120, offset_.y() / MouseWheelEvent::kWheelDelta * 120);
    } else {
...
MouseWheelEvent::MouseWheelEvent(
    const gfx::Vector2d& offset, const gfx::Point& location, const gfx::Point& root_location, base::TimeTicks time_stamp, int flags, int changed_button_flags)
    : MouseWheelEvent(offset, gfx::PointF(location), gfx::PointF(root_location), time_stamp, flags, changed_button_flags)
{
}
...
TouchEvent::TouchEvent(
    EventType type, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, const PointerDetails& pointer_details, int flags)
    : LocatedEvent(type, location, root_location, time_stamp, flags)
    , unique_event_id_(ui::GetNextTouchEventId())
    , pointer_details_(pointer_details)
{
...
TouchEvent::TouchEvent(EventType type, const gfx::Point& location, base::TimeTicks time_stamp, const PointerDetails& pointer_details, int flags)
    : TouchEvent(type, gfx::PointF(location), gfx::PointF(location), time_stamp, pointer_details, flags)
{
}
...
void TouchEvent::UpdateForRootTransform(const gfx::Transform& inverted_root_transform, const gfx::Transform& inverted_local_transform)
{
    LocatedEvent::UpdateForRootTransform(inverted_root_transform, inverted_local_transform);
    ...
    // matrix here.
    const double new_x = fabs(pointer_details_.radius_x * inverted_root_transform.rc(0, 0) + pointer_details_.radius_y * inverted_root_transform.rc(0, 1));
    const double new_y = fabs(pointer_details_.radius_x * inverted_root_transform.rc(1, 0) + pointer_details_.radius_y * inverted_root_transform.rc(1, 1));
    ...
    // section.
    if (!IsNearZero(pointer_details_.tilt_x) || !IsNearZero(pointer_details_.tilt_y)) {
        if (IsNearZero(inverted_root_transform.rc(0, 1)) && IsNearZero(inverted_root_transform.rc(1, 0))) {
            pointer_details_.tilt_x *= std::copysign(1, inverted_root_transform.rc(0, 0));
            pointer_details_.tilt_y *= std::copysign(1, inverted_root_transform.rc(1, 1));
        } else if (IsNearZero(inverted_root_transform.rc(0, 0)) && IsNearZero(inverted_root_transform.rc(1, 1))) {
            double new_tilt_x = pointer_details_.tilt_y * std::copysign(1, inverted_root_transform.rc(0, 1));
            double new_tilt_y = pointer_details_.tilt_x * std::copysign(1, inverted_root_transform.rc(1, 0));
...
void TouchEvent::DisableSynchronousHandling()
{
    ...
    dispatcher_api.set_result(static_cast<EventResult>(result() | ER_DISABLE_SYNC_HANDLING));
}

void TouchEvent::ForceProcessGesture()
{
    DispatcherApi dispatcher_api(this);
    dispatcher_api.set_result(static_cast<EventResult>(result() | ER_FORCE_PROCESS_GESTURE));
}

void TouchEvent::SetPointerDetailsForTest(const PointerDetails& pointer_details)
{
...
KeyEvent::KeyEvent(EventType type, KeyboardCode key_code, int flags, base::TimeTicks time_stamp)
    : Event(type, time_stamp, flags)
    , key_code_(key_code)
...
KeyEvent::KeyEvent(EventType type, KeyboardCode key_code, DomCode code, int flags)
...
KeyEvent::KeyEvent(EventType type, KeyboardCode key_code, DomCode code, int flags, DomKey key, base::TimeTicks time_stamp, bool is_char)
    : Event(type, time_stamp, flags)
    , key_code_(key_code)
    , code_(code)
    , is_char_(is_char)
...
KeyEvent::KeyEvent(EventType type, KeyboardCode key_code, DomCode code, int flags, base::TimeTicks time_stamp)
    : Event(type, time_stamp, flags)
    , key_code_(key_code)
    , code_(code)
{
...
KeyEvent KeyEvent::FromCharacter(char16_t character, KeyboardCode key_code, DomCode code, int flags, base::TimeTicks time_stamp)
{
    return KeyEvent(EventType::kKeyPressed, key_code, code, flags, DomKey::FromCharacter(character), time_stamp, true);
}
...
void KeyEvent::ApplyLayout() const
{
...
#if BUILDFLAG(IS_OZONE)
    if (KeyboardLayoutEngineManager::GetKeyboardLayoutEngine()->Lookup(code, flags(), &key_, &dummy_key_code)) {
        return;
    }
#endif
    ...
    if (IsPlatformEventValid(native_event())) {
        DCHECK(EventTypeFromNative(native_event()) == EventType::kKeyPressed || EventTypeFromNative(native_event()) == EventType::kKeyReleased);
    }
#endif
...
#endif
    if (!is_repeat) {
        if (key_code() == last->key_code() && (flags() & ~EF_MOUSE_BUTTON) == (last->flags() & ~EF_IS_REPEAT & ~EF_MOUSE_BUTTON)
            && (time_stamp() - last->time_stamp()).InMilliseconds() < kMaxAutoRepeatTimeMs) {
            is_repeat = true;
        }
    }
...
KeyEvent** KeyEvent::GetLastKeyEvent()
{
    ...
    return properties() && properties()->contains(kPropertyKeyboardImeFlag) ? &last_ibus_key_event_ : &last_key_event_;
#else
...
bool KeyEvent::IsUnicodeKeyCode() const
{
    ...
    return (!(flags() & EF_IS_EXTENDED_KEY)
        && (key == VKEY_INSERT || key == VKEY_END || key == VKEY_DOWN || key == VKEY_NEXT || key == VKEY_LEFT || key == VKEY_CLEAR || key == VKEY_RIGHT
            || key == VKEY_HOME || key == VKEY_UP || key == VKEY_PRIOR));
#else
    return false;
#endif
}
...
std::string KeyEvent::ToString() const
{
    ...
    return base::StrCat({
        Event::ToString(), " code=", KeycodeConverter::DomCodeToCodeString(code()), base::StringPrintf("(0x%04x)", static_cast<uint32_t>(code_)),
            " key=", KeycodeConverter::DomKeyToKeyString(dom_key), base::StringPrintf("(0x%04x)", static_cast<uint32_t>(dom_key)),
            " keycode=", base::StringPrintf("(0x%04x)", key_code_),
#if BUILDFLAG(IS_OZONE)
            " scan_code=", base::StringPrintf("(0x%04x)", scan_code_),
#endif // BUILDFLAG(IS_OZONE)
            " flags=", base::JoinString(KeyEventFlagsNames(flags()), "|"), base::StringPrintf("(0x%04x)", flags()),
    });
}
    ...
    , scroll_event_phase_(ScrollEventPhase::kNone)
{
    ...
    if (type() == EventType::kScroll) {
        GetScrollOffsets(native_event, &x_offset_, &y_offset_, &x_offset_ordinal_, &y_offset_ordinal_, &finger_count_, &momentum_phase_);
    } else if (type() == EventType::kScrollFlingStart || type() == EventType::kScrollFlingCancel) {
        GetFlingData(native_event, &x_offset_, &y_offset_, &x_offset_ordinal_, &y_offset_ordinal_, nullptr);
    } else {
        NOTREACHED() << "Unexpected event type " << base::to_underlying(type()) << " when constructing a ScrollEvent.";
    }
}

ScrollEvent::ScrollEvent(EventType type, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, int flags, float x_offset,
    float y_offset, float x_offset_ordinal, float y_offset_ordinal, int finger_count, EventMomentumPhase momentum_phase, ScrollEventPhase scroll_event_phase)
...
ScrollEvent::ScrollEvent(EventType type, const gfx::Point& location, base::TimeTicks time_stamp, int flags, float x_offset, float y_offset,
    float x_offset_ordinal, float y_offset_ordinal, int finger_count, EventMomentumPhase momentum_phase, ScrollEventPhase scroll_event_phase)
    : ScrollEvent(type, gfx::PointF(location), gfx::PointF(location), time_stamp, flags, x_offset, y_offset, x_offset_ordinal, y_offset_ordinal, finger_count,
        momentum_phase, scroll_event_phase)
{
}
...
std::string ScrollEvent::ToString() const
{
    return base::StrCat({
        ...
        base::StringPrintf(" offset_ordinal=%g,%g", x_offset_ordinal_, y_offset_ordinal_),
        " momentum_phase=",
...
GestureEvent::GestureEvent(float x, float y, int flags, base::TimeTicks time_stamp, const GestureEventDetails& details, uint32_t unique_touch_event_id)
    : LocatedEvent(details.type(), gfx::PointF(x, y), gfx::PointF(x, y), time_stamp, flags | EF_FROM_TOUCH)
    , details_(details)
