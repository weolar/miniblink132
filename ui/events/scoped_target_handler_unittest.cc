...
public:
    ...
    void SetHandler(std::unique_ptr<EventHandler> target_handler, std::unique_ptr<EventHandler> delegate)
    {
...
public:
    ...
    TargetDestroyingEventHandler& operator=(const TargetDestroyingEventHandler&) = delete;
...
public:
    ...
    EventCountingEventHandler& operator=(const EventCountingEventHandler&) = delete;
...
public:
    ...
    EventStopPropagationHandler& operator=(const EventStopPropagationHandler&) = delete;
...
TEST(ScopedTargetHandlerTest, HandlerInvoked)
{
    ...
    std::unique_ptr<NestedEventHandler> target_handler(new NestedEventHandler(target.get(), 1));
    std::unique_ptr<EventCountingEventHandler> delegate(new EventCountingEventHandler(target.get(), &count));
    target->SetHandler(std::move(target_handler), std::move(delegate));
    MouseEvent event(EventType::kMousePressed, gfx::Point(), gfx::Point(), EventTimeForNow(), EF_LEFT_MOUSE_BUTTON, EF_LEFT_MOUSE_BUTTON);
...
TEST(ScopedTargetHandlerTest, HandlerInvokedOnceThenEventStopsPropagating)
{
    int count = 0;
    std::unique_ptr<TestEventTarget> target = std::make_unique<TestEventTarget>();
    std::unique_ptr<NestedEventHandler> target_handler(new NestedEventHandler(target.get(), 3));
    std::unique_ptr<EventStopPropagationHandler> delegate(new EventStopPropagationHandler(target.get(), &count));
    target->SetHandler(std::move(target_handler), std::move(delegate));
    MouseEvent event(EventType::kMousePressed, gfx::Point(), gfx::Point(), EventTimeForNow(), EF_LEFT_MOUSE_BUTTON, EF_LEFT_MOUSE_BUTTON);
    target->DispatchEvent(&event);
    EXPECT_EQ(1, count);
...
TEST(ScopedTargetHandlerTest, HandlerInvokedNested)
{
    int count = 0;
    std::unique_ptr<TestEventTarget> target = std::make_unique<TestEventTarget>();
    std::unique_ptr<NestedEventHandler> target_handler(new NestedEventHandler(target.get(), 2));
    std::unique_ptr<EventCountingEventHandler> delegate(new EventCountingEventHandler(target.get(), &count));
    target->SetHandler(std::move(target_handler), std::move(delegate));
    MouseEvent event(EventType::kMousePressed, gfx::Point(), gfx::Point(), EventTimeForNow(), EF_LEFT_MOUSE_BUTTON, EF_LEFT_MOUSE_BUTTON);
    target->DispatchEvent(&event);
...
TEST(ScopedTargetHandlerTest, SafeToDestroy)
{
    ...
    std::unique_ptr<TargetDestroyingEventHandler> target_handler(new TargetDestroyingEventHandler(target, 1));
    std::unique_ptr<EventCountingEventHandler> delegate(new EventCountingEventHandler(target, &count));
    target->SetHandler(std::move(target_handler), std::move(delegate));
    MouseEvent event(EventType::kMousePressed, gfx::Point(), gfx::Point(), EventTimeForNow(), EF_LEFT_MOUSE_BUTTON, EF_LEFT_MOUSE_BUTTON);
    target->DispatchEvent(&event);
    EXPECT_EQ(0, count);
...
TEST(ScopedTargetHandlerTest, SafeToDestroyNested)
{
    int count = 0;
    TestEventTarget* target = new TestEventTarget;
    std::unique_ptr<TargetDestroyingEventHandler> target_handler(new TargetDestroyingEventHandler(target, 2));
    std::unique_ptr<EventCountingEventHandler> delegate(new EventCountingEventHandler(target, &count));
    target->SetHandler(std::move(target_handler), std::move(delegate));
    MouseEvent event(EventType::kMousePressed, gfx::Point(), gfx::Point(), EventTimeForNow(), EF_LEFT_MOUSE_BUTTON, EF_LEFT_MOUSE_BUTTON);
    target->DispatchEvent(&event);
