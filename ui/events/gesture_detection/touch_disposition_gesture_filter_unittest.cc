...
class TouchDispositionGestureFilterTest : public testing::Test, public TouchDispositionGestureFilterClient {
public:
    ...
    {
        EXPECT_EQ(GestureDeviceType::DEVICE_TOUCHSCREEN, event.details.device_type());
        ++sent_gesture_count_;
...
protected:
    ...
    ::testing::AssertionResult GesturesMatch(const GestureList& expected, const GestureList& actual)
    {
        if (expected.size() != actual.size()) {
            return ::testing::AssertionFailure() << "actual.size(" << actual.size() << ") != expected.size(" << expected.size() << ")";
        }

        for (size_t i = 0; i < expected.size(); ++i) {
            if (expected[i] != actual[i]) {
                return ::testing::AssertionFailure() << "actual[" << i << "] (" << base::to_underlying(actual[i]) << ") != expected[" << i << "] ("
    ...
    GestureList Gestures(EventType type0, EventType type1, EventType type2)
    {
    ...
    GestureList Gestures(EventType type0, EventType type1, EventType type2, EventType type3)
    {
    ...
    TouchDispositionGestureFilter::PacketResult SendTouchGesturesWithResult(const MotionEvent& touch, const GestureEventDataPacket& packet)
    {
    ...
    uint32_t SendTouchGestures(const MotionEvent& touch, const GestureEventDataPacket& packet)
    {
    ...
    TouchDispositionGestureFilter::PacketResult SendTimeoutGesture(EventType type)
    {
        return queue_->OnGesturePacket(GestureEventDataPacket::FromTouchTimeout(CreateGesture(type)));
    }

    TouchDispositionGestureFilter::PacketResult SendGesturePacket(const GestureEventDataPacket& packet)
    {
        return queue_->OnGesturePacket(packet);
    }

    void SendTouchEventAck(uint32_t touch_event_id, bool event_consumed, bool is_source_touch_event_set_blocking)
    {
        queue_->OnTouchEventAck(touch_event_id, event_consumed, is_source_touch_event_set_blocking);
    }

    void SendTouchConsumedAck(uint32_t touch_event_id)
    {
        SendTouchEventAck(touch_event_id, true /* event_consumed */, false /* is_source_touch_event_set_blocking */);
    }

    void SendTouchNotConsumedAck(uint32_t touch_event_id)
    {
        SendTouchEventAck(touch_event_id, false /* event_consumed */, false /* is_source_touch_event_set_blocking */);
    }

    void SendTouchConsumedAckForLastTouch()
    {
        SendTouchEventAck(last_sent_touch_event_id_, true /* event_consumed */, false /* is_source_touch_event_set_blocking */);
    }

    void SendTouchNotConsumedAckForLastTouch()
    {
        SendTouchEventAck(last_sent_touch_event_id_, false /* event_consumed */, false /* is_source_touch_event_set_blocking */);
    }
    ...
    uint32_t SendPacket(const MockMotionEvent& touch_event, GestureList gesture_list)
    {
        ...
        EXPECT_EQ(TouchDispositionGestureFilter::SUCCESS, SendTouchGesturesWithResult(touch_event, gesture_packet));
    ...
    GestureEventData CreateGesture(EventType type, float x, float y, float diameter)
    {
        GestureEventDetails details(type);
        details.set_device_type(GestureDeviceType::DEVICE_TOUCHSCREEN);
        return GestureEventData(details, 0, MotionEvent::ToolType::FINGER, base::TimeTicks(), x, y, 0, 0, 1,
            gfx::RectF(x - diameter / 2, y - diameter / 2, diameter, diameter), kDefaultEventFlags, 0U);
    }
...
{
    GestureEventDataPacket packet = GestureEventDataPacket::FromTouch(MockMotionEvent(MotionEvent::Action::CANCEL, base::TimeTicks(), 0, 0));
    EXPECT_EQ(TouchDispositionGestureFilter::PacketResult::EMPTY_GESTURE_SEQUENCE, SendGesturePacket(packet));
}

TEST_F(TouchDispositionGestureFilterTest, BasicGestures)
{
    GestureList press_gestures = Gestures(EventType::kGestureBegin, EventType::kGestureScrollBegin);
    ...
    GestureList release_gestures = Gestures(EventType::kScrollFlingStart, EventType::kScrollFlingCancel, EventType::kGestureEnd);
...
TEST_F(TouchDispositionGestureFilterTest, BasicGesturesConsumed)
{
    ...
    scoped_feature_list.InitAndEnableFeature(features::kEnableGestureBeginEndTypes);

    // A consumed touch's gesture should only be sent on Android.
    SendPacket(PressTouchPoint(), Gestures(EventType::kGestureBegin, EventType::kGestureScrollBegin));
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureBegin), GetAndResetSentGestures()));

    SendPacket(MoveTouchPoint(), Gestures(EventType::kGestureScrollUpdate));
    SendTouchConsumedAckForLastTouch();
    EXPECT_FALSE(GesturesSent());

    SendPacket(ReleaseTouchPoint(), Gestures(EventType::kScrollFlingStart, EventType::kScrollFlingCancel, EventType::kGestureEnd));
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureEnd), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, NotConsumedThenConsumed)
{
    ...
    SendPacket(PressTouchPoint(), Gestures(EventType::kGestureScrollBegin));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureScrollBegin), GetAndResetSentGestures()));
    ...
    SendPacket(MoveTouchPoint(), Gestures(EventType::kGestureScrollUpdate, EventType::kGesturePinchUpdate));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureScrollUpdate), GetAndResetSentGestures()));
    ...
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureScrollEnd), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, NotConsumedThenNoConsumer)
{
    ...
    SendPacket(PressTouchPoint(), Gestures(EventType::kGesturePinchBegin));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGesturePinchBegin), GetAndResetSentGestures()));

    // End events should be dispatched when their start events were, independent
    // of the ack state.
    SendPacket(ReleaseTouchPoint(), Gestures(EventType::kGesturePinchEnd));
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGesturePinchEnd), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, EndingEventsSent)
{
    ...
    SendPacket(ReleaseTouchPoint(), Gestures(EventType::kGestureScrollUpdate, EventType::kGestureScrollEnd));
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureScrollEnd), GetAndResetSentGestures()));
    ...
    SendPacket(ReleaseTouchPoint(), Gestures(EventType::kScrollFlingStart));
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kScrollFlingStart), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, UpdateEventsSuppressedPerEvent)
{
    ...
    SendPacket(MoveTouchPoint(), Gestures(EventType::kGesturePinchUpdate));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGesturePinchUpdate), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, MultipleTouchSequences)
{
    ...
    uint32_t touch_press_event_id1 = SendPacket(PressTouchPoint(), Gestures(EventType::kGestureTapDown));
    uint32_t touch_release_event_id1 = SendPacket(ReleaseTouchPoint(), Gestures(EventType::kGestureTap));
    uint32_t touch_press_event_id2 = SendPacket(PressTouchPoint(), Gestures(EventType::kGestureScrollBegin));
    uint32_t touch_release_event_id2 = SendPacket(ReleaseTouchPoint(), Gestures(EventType::kGestureScrollEnd));
    ...
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureScrollBegin, EventType::kGestureScrollEnd), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, FlingCancelledOnNewTouchSequence)
{
    ...
    // Simulate a fling.
    SendPacket(PressTouchPoint(), Gestures(EventType::kGestureTapDown, EventType::kGestureScrollBegin));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapDown, EventType::kGestureTapCancel, EventType::kGestureScrollBegin), GetAndResetSentGestures()));
    SendPacket(ReleaseTouchPoint(), Gestures(EventType::kScrollFlingStart));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kScrollFlingStart), GetAndResetSentGestures()));
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kScrollFlingCancel), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, ScrollEndedOnTouchReleaseIfNoFling)
{
    ...
    SendPacket(PressTouchPoint(2, 3), Gestures(EventType::kGestureTapDown, EventType::kGestureScrollBegin));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapDown, EventType::kGestureTapCancel, EventType::kGestureScrollBegin), GetAndResetSentGestures()));
    SendPacket(ReleaseTouchPoint(), NoGestures());
    SendTouchNotConsumedAckForLastTouch();
...
TEST_F(TouchDispositionGestureFilterTest, FlingCancelledOnScrollBegin)
{
    ...
    SendPacket(PressTouchPoint(), Gestures(EventType::kGestureTapDown, EventType::kGestureScrollBegin, EventType::kScrollFlingStart));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapDown, EventType::kGestureTapCancel, EventType::kGestureScrollBegin, EventType::kScrollFlingStart),
        GetAndResetSentGestures()));

    // The new fling should cancel the preceding one.
    SendPacket(ReleaseTouchPoint(), Gestures(EventType::kGestureScrollBegin, EventType::kScrollFlingStart));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(
        GesturesMatch(Gestures(EventType::kScrollFlingCancel, EventType::kGestureScrollBegin, EventType::kScrollFlingStart), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, FlingNotCancelledIfGFCEventReceived)
{
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(
        GesturesMatch(Gestures(EventType::kGestureScrollBegin, EventType::kScrollFlingStart, EventType::kScrollFlingCancel), GetAndResetSentGestures()));
    EXPECT_EQ(LastSentGestureLocation(), gfx::PointF(2, 3));
...
TEST_F(TouchDispositionGestureFilterTest, TapCancelledWhenScrollBegins)
{
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapDown), GetAndResetSentGestures()));
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapCancel, EventType::kGestureScrollBegin), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, TapNotCancelledIfTapEndingEventReceived)
{
    SendPacket(PressTouchPoint(), Gestures(EventType::kGestureTapDown));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapDown), GetAndResetSentGestures()));
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureShowPress, EventType::kGestureTap), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, TimeoutGestures)
{
    ...
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureShowPress), GetAndResetSentGestures()));

    SendTimeoutGesture(EventType::kGestureLongPress);
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureLongPress), GetAndResetSentGestures()));

    SendPacket(ReleaseTouchPoint(), Gestures(EventType::kGestureLongTap));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapCancel, EventType::kGestureLongTap), GetAndResetSentGestures()));
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapDown, EventType::kGestureLongPress), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, SpuriousAcksIgnored)
{
    ...
    uint32_t touch_press_event_id = SendPacket(PressTouchPoint(), Gestures(EventType::kGestureScrollBegin));
    uint32_t touch_move_event_id = SendPacket(MoveTouchPoint(), Gestures(EventType::kGestureScrollUpdate));
    SendTouchNotConsumedAck(touch_press_event_id);
    SendTouchNotConsumedAck(touch_move_event_id);
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureScrollBegin, EventType::kGestureScrollUpdate), GetAndResetSentGestures()));
...
{
    GestureEventDataPacket packet;
    EXPECT_EQ(TouchDispositionGestureFilter::INVALID_PACKET_TYPE, SendGesturePacket(packet));
    EXPECT_TRUE(IsEmpty());
}

TEST_F(TouchDispositionGestureFilterTest, PacketsWithInvalidOrderIgnored)
{
    EXPECT_EQ(TouchDispositionGestureFilter::INVALID_PACKET_ORDER, SendTimeoutGesture(EventType::kGestureShowPress));
    EXPECT_TRUE(IsEmpty());
}
...
TEST_F(TouchDispositionGestureFilterTest, ConsumedTouchCancel)
{
    ...
    SendPacket(CancelTouchPoint(), Gestures(EventType::kGestureTapCancel, EventType::kGestureScrollEnd));
    EXPECT_FALSE(GesturesSent());
    SendTouchConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapCancel, EventType::kGestureScrollEnd), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, TimeoutEventAfterRelease)
{
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_FALSE(GesturesSent());
    SendPacket(ReleaseTouchPoint(), Gestures(EventType::kGestureTapDown, EventType::kGestureTapUnconfirmed));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapDown, EventType::kGestureTapUnconfirmed), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, ShowPressInsertedBeforeTap)
{
    ...
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapUnconfirmed), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, ShowPressNotInsertedIfAlreadySent)
{
    ...
    SendPacket(ReleaseTouchPoint(), Gestures(EventType::kGestureTap));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTap), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, TapAndScrollCancelledOnTouchCancel)
{
    ...
    SendPacket(CancelTouchPoint(), NoGestures());
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapCancel), GetAndResetSentGestures()));
    EXPECT_EQ(CurrentTouchTime(), LastSentGestureTime());
    EXPECT_EQ(LastSentGestureLocation(), gfx::PointF(1, 1));
    EXPECT_EQ(LastSentGestureRawLocation(), gfx::PointF(1, 1) + raw_offset);
...
TEST_F(TouchDispositionGestureFilterTest, ConsumedScrollUpdateMakesFlingScrollEnd)
{
    // A consumed touch's gesture should not be sent.
    SendPacket(PressTouchPoint(), Gestures(EventType::kGestureBegin, EventType::kGestureScrollBegin));
    SendTouchNotConsumedAckForLastTouch();

    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureBegin, EventType::kGestureScrollBegin), GetAndResetSentGestures()));
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureScrollEnd, EventType::kGestureEnd), GetAndResetSentGestures()));
    EXPECT_EQ(LastSentGestureLocation(), gfx::PointF(2, 3));
...
TEST_F(TouchDispositionGestureFilterTest, TestCancelMidGesture)
{
    ...
    SendPacket(PressTouchPoint(1, 1), Gestures(EventType::kGestureTapDown));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapDown, EventType::kGestureTapCancel), GetAndResetSentGestures()));
    EXPECT_EQ(LastSentGestureLocation(), gfx::PointF(1, 1));
}
...
TEST_F(TouchDispositionGestureFilterTest, TestAllowedMultiFingerSwipe)
{
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureSwipe), GetAndResetSentGestures()));
}
...
{
    SendPacket(PressTouchPoint(), Gestures(EventType::kGestureBegin, EventType::kGestureTapDown));
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureBegin, EventType::kGestureTapDown), GetAndResetSentGestures()));
    SendTimeoutGesture(EventType::kGestureShowPress);
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureShowPress), GetAndResetSentGestures()));
    ...
    SendTouchNotConsumedAckForLastTouch();
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureTapCancel, EventType::kGestureEnd), GetAndResetSentGestures()));
}
...
TEST_F(TouchDispositionGestureFilterTest, AckQueueBack)
{
    ...
    // Pending touch move.
    touch_event_id = SendPacket(MoveTouchPoint(), Gestures(EventType::kGestureScrollUpdate));
    EXPECT_FALSE(GesturesSent());
    ...
    EXPECT_TRUE(
        GesturesMatch(Gestures(EventType::kGestureScrollUpdate, EventType::kGestureScrollUpdate, EventType::kGestureScrollUpdate), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, AckQueueGestureAtBack)
{
    ...
    uint32_t touch_press_event_id1 = SendPacket(PressTouchPoint(), Gestures(EventType::kGestureBegin));
    uint32_t touch_release_event_id1 = SendPacket(ReleaseTouchPoint(), Gestures(EventType::kGestureEnd));
    ...
    EXPECT_TRUE(
        GesturesMatch(Gestures(EventType::kGestureBegin, EventType::kGestureEnd, EventType::kGestureBegin, EventType::kGestureEnd), GetAndResetSentGestures()));
    EXPECT_EQ(gfx::PointF(2, 3), LastSentGestureLocation());
}

TEST_F(TouchDispositionGestureFilterTest, SyncAcksOnlyTriggerAppropriateGestures)
{
    // Queue a touch press.
    uint32_t touch_press_event_id = SendPacket(PressTouchPoint(), Gestures(EventType::kGestureBegin));
    ...
    uint32_t touch_release_event_id = SendPacket(ReleaseTouchPoint(), Gestures(EventType::kGestureEnd));

    EXPECT_FALSE(GesturesSent());

    // Ack the touch press. All events but the release should be acked.
    SendTouchNotConsumedAck(touch_press_event_id);
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureBegin, EventType::kGestureScrollUpdate, EventType::kGestureScrollUpdate), GetAndResetSentGestures()));
...
TEST_F(TouchDispositionGestureFilterTest, GestureBeginEndWhenTouchStartConsumed)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndEnableFeature(features::kEnableGestureBeginEndTypes);
...
TEST_F(TouchDispositionGestureFilterTest, GestureBeginEndWhenTouchEndConsumed)
{
    ...
    SendPacket(ReleaseTouchPoint(), Gestures(EventType::kGestureTap, EventType::kGestureEnd));
    SendTouchConsumedAckForLastTouch();
...
TEST_F(TouchDispositionGestureFilterTest, GestureBeginEndWhenTouchNotConsumed)
{
    ...
    EXPECT_TRUE(GesturesMatch(Gestures(EventType::kGestureShowPress, EventType::kGestureTap, EventType::kGestureEnd), GetAndResetSentGestures()));
}
