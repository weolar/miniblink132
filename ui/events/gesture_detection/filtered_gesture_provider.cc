...
FilteredGestureProvider::FilteredGestureProvider(const GestureProvider::Config& config, GestureProviderClient* client)
    : client_(client)
...
void FilteredGestureProvider::UpdateConfig(const GestureProvider::Config& config)
{
    gesture_provider_ = std::make_unique<ui::GestureProvider>(config, this);
}

FilteredGestureProvider::TouchHandlingResult FilteredGestureProvider::OnTouchEvent(const MotionEvent& event)
{
    ...
    if (filter_result != TouchDispositionGestureFilter::SUCCESS) {
        DCHECK_EQ(filter_result, TouchDispositionGestureFilter::EMPTY_GESTURE_SEQUENCE) << "Invalid touch gesture sequence detected.";
        return TouchHandlingResult();
    }
...
void FilteredGestureProvider::OnTouchEventAck(
    uint32_t unique_event_id, bool event_consumed, bool is_source_touch_event_set_blocking, const std::optional<EventLatencyMetadata>& event_latency_metadata)
{
    gesture_filter_.OnTouchEventAck(unique_event_id, event_consumed, is_source_touch_event_set_blocking, event_latency_metadata);
}
...
void FilteredGestureProvider::SetMultiTouchZoomSupportEnabled(bool enabled)
{
    gesture_provider_->SetMultiTouchZoomSupportEnabled(enabled);
}

void FilteredGestureProvider::SetDoubleTapSupportForPlatformEnabled(bool enabled)
{
...
void FilteredGestureProvider::OnGestureEvent(const GestureEventData& event)
{
    ...
    gesture_filter_.OnGesturePacket(GestureEventDataPacket::FromTouchTimeout(event));
}
...
void FilteredGestureProvider::ForwardGestureEvent(const GestureEventData& event)
{
