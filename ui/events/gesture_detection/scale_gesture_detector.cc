...
ScaleGestureDetector::ScaleGestureDetector(const Config& config, ScaleGestureListener* listener)
    : listener_(listener)
...
bool ScaleGestureDetector::OnTouchEvent(const MotionEvent& event)
{
    ...
    const bool is_stylus_button_down = (event.GetButtonState() & MotionEvent::BUTTON_STYLUS_PRIMARY) != 0;

    const bool anchored_scale_cancelled = anchored_scale_mode_ == ANCHORED_SCALE_MODE_STYLUS && !is_stylus_button_down;

    const bool stream_complete = action == MotionEvent::Action::UP || action == MotionEvent::Action::CANCEL || anchored_scale_cancelled
    ...
    if (!in_progress_ && stylus_scale_enabled_ && !InAnchoredScaleMode() && !stream_complete && is_stylus_button_down) {
    ...
    const bool config_changed = action == MotionEvent::Action::DOWN || action == MotionEvent::Action::POINTER_UP || action == MotionEvent::Action::POINTER_DOWN
    ...
    bool span_exceeds_min_span = span >= min_span + kSlopEpsilon || initial_span_ >= min_span + kSlopEpsilon;
    if (!in_progress_ && span_exceeds_min_span && (was_in_progress || std::abs(span - initial_span_) > span_slop_ + kSlopEpsilon)) {
    ...
    if (!InAnchoredScaleMode() && in_progress_ && span < min_span_ + kSlopEpsilon) {
        listener_->OnScaleEnd(*this, event);
        ResetScaleWithSpan(span);
    }
...
float ScaleGestureDetector::GetScaleFactor() const
{
    ...
    if (InAnchoredScaleMode()) {
        ...
        // larger the scale.
        const bool scale_up = (event_before_or_above_starting_gesture_event_ && (curr_span < prev_span_))
            || (!event_before_or_above_starting_gesture_event_ && (curr_span > prev_span_));
        const float span_diff = (std::abs(1.f - (curr_span / prev_span_)) * kScaleFactor);
        return prev_span_ <= 0 ? 1.f : (scale_up ? (1.f + span_diff) : (1.f - span_diff));
    }
...
float ScaleGestureDetector::CalculateAngle(const MotionEvent& event, int action_index, float focus_x, float focus_y) const
{
