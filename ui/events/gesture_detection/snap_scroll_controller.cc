...
float CalculateChannelDistance(float snap_bound, const gfx::SizeF& display_size)
{
    ...
    float screen_size = std::abs(hypot(static_cast<float>(display_size.width()), static_cast<float>(display_size.height())));

    float snap_channel_distance = screen_size * kSnapChannelDipsPerScreenDip;
    return std::max(kMinChannelDistance, std::min(kMaxChannelDistance, snap_channel_distance));
}
...
void SnapScrollController::SetSnapScrollMode(const MotionEvent& event, bool is_scale_gesture_detection_in_progress, float slop_distance)
{
...
void SnapScrollController::UpdateSnapScrollMode(float distance_x, float distance_y, float slop_distance)
{
    if (!IsSnappingScrolls())
        return;

    float channel_distance = CalculateChannelDistance(slop_distance, display_size_);
