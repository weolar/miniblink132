...
GestureDetector::Config BuildGestureDetectorConfig(const GestureConfiguration& gesture_config, scoped_refptr<base::SequencedTaskRunner> task_runner)
{
    GestureDetector::Config config;
    config.longpress_timeout = base::Milliseconds(gesture_config.long_press_time_in_ms());
    config.shortpress_timeout = gesture_config.short_press_time();
    config.showpress_timeout = base::Milliseconds(gesture_config.show_press_delay_in_ms());
    config.double_tap_timeout = base::Milliseconds(gesture_config.double_tap_timeout_in_ms());
    ...
    config.two_finger_tap_timeout = base::Milliseconds(gesture_config.max_touch_down_duration_for_click_in_ms());
...
ScaleGestureDetector::Config BuildScaleGestureDetectorConfig(const GestureConfiguration& gesture_config)
{
...
GestureProvider::Config BuildGestureProviderConfig(const GestureConfiguration& gesture_config, scoped_refptr<base::SequencedTaskRunner> task_runner)
{
    GestureProvider::Config config;
    config.gesture_detector_config = BuildGestureDetectorConfig(gesture_config, task_runner);
...
GestureProvider::Config GetGestureProviderConfig(GestureProviderConfigType type, scoped_refptr<base::SequencedTaskRunner> task_runner)
{
    GestureProvider::Config config;
    switch (type) {
    case GestureProviderConfigType::CURRENT_PLATFORM:
        config = BuildGestureProviderConfig(*GestureConfiguration::GetInstance(), task_runner);
        break;
    case GestureProviderConfigType::GENERIC_DESKTOP:
        config = BuildGestureProviderConfig(GenericDesktopGestureConfiguration(), task_runner);
        break;
