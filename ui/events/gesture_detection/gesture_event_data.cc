...
GestureEventData::GestureEventData(const GestureEventDetails& details, int motion_event_id, MotionEvent::ToolType primary_tool_type, base::TimeTicks time,
    float x, float y, float raw_x, float raw_y, size_t touch_point_count, const gfx::RectF& bounding_box, int flags, uint32_t unique_touch_event_id)
    : details(details)
    ...
    , unique_touch_event_id(unique_touch_event_id)
{
    ...
    this->details.set_primary_unique_touch_event_id(details.primary_unique_touch_event_id());
    this->details.set_touch_points(static_cast<int>(touch_point_count));
    this->details.set_bounding_box(bounding_box);
}

GestureEventData::GestureEventData(EventType type, const GestureEventData& other)
    ...
    , unique_touch_event_id(other.unique_touch_event_id)
{
    ...
    details.set_primary_unique_touch_event_id(other.details.primary_unique_touch_event_id());
...
GestureEventData& GestureEventData::operator=(const GestureEventData& other) = default;
