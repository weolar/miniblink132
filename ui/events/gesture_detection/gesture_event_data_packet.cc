...
GestureEventDataPacket::GestureSource ToGestureSource(const ui::MotionEvent& event)
{
...
GestureEventDataPacket::GestureEventDataPacket(
    base::TimeTicks timestamp, GestureSource source, const gfx::PointF& touch_location, const gfx::PointF& raw_touch_location, uint32_t unique_touch_event_id)
...
GestureEventDataPacket::GestureEventDataPacket(const GestureEventDataPacket& other)
...
GestureEventDataPacket& GestureEventDataPacket::operator=(const GestureEventDataPacket& other)
{
    timestamp_ = other.timestamp_;
    gesture_source_ = other.gesture_source_;
    touch_location_ = other.touch_location_;
...
GestureEventDataPacket GestureEventDataPacket::FromTouch(const ui::MotionEvent& touch)
{
    return GestureEventDataPacket(touch.GetEventTime(), ToGestureSource(touch), gfx::PointF(touch.GetX(), touch.GetY()),
        gfx::PointF(touch.GetRawX(), touch.GetRawY()), touch.GetUniqueEventId());
}

GestureEventDataPacket GestureEventDataPacket::FromTouchTimeout(const GestureEventData& gesture)
{
    GestureEventDataPacket packet(
        gesture.time, TOUCH_TIMEOUT, gfx::PointF(gesture.x, gesture.y), gfx::PointF(gesture.raw_x, gesture.raw_y), gesture.unique_touch_event_id);
    packet.Push(gesture);
    return packet;
}

void GestureEventDataPacket::Ack(bool event_consumed, bool is_source_touch_event_set_blocking)
{
    ...
    for (auto& gesture : gestures_) {
        gesture.details.set_is_source_touch_event_set_blocking(is_source_touch_event_set_blocking);
    }
}
    ...
    const EventLatencyMetadata& event_latency_metadata, const base::RepeatingCallback<bool(const ui::GestureEventData&)>& filter)
{
    for (auto& gesture : gestures_) {
        if (filter.Run(gesture)) {
            gesture.details.GetModifiableEventLatencyMetadata() = event_latency_metadata;
        }
    }
}
