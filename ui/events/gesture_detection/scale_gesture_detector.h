...
private:
    enum AnchoredScaleMode { ANCHORED_SCALE_MODE_NONE, ANCHORED_SCALE_MODE_DOUBLE_TAP, ANCHORED_SCALE_MODE_STYLUS };

    void ResetScaleWithSpan(float span);
    float CalculateAngle(const MotionEvent& event, int action_index, float focus_x, float focus_y) const;
