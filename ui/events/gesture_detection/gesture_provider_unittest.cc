...
public:
    ...
    static MockMotionEvent ObtainMotionEvent(base::TimeTicks event_time, MotionEvent::Action action, float x, float y)
    {
        return MockMotionEvent(action, event_time, x, y);
    }

    static MockMotionEvent ObtainMotionEvent(base::TimeTicks event_time, MotionEvent::Action action, float x0, float y0, float x1, float y1)
    {
        return MockMotionEvent(action, event_time, x0, y0, x1, y1);
    }

    static MockMotionEvent ObtainMotionEvent(base::TimeTicks event_time, MotionEvent::Action action, float x0, float y0, float x1, float y1, float x2, float y2)
    {
        return MockMotionEvent(action, event_time, x0, y0, x1, y1, x2, y2);
    }

    static MockMotionEvent ObtainMotionEvent(base::TimeTicks event_time, MotionEvent::Action action, const std::vector<gfx::PointF>& positions)
    {
        switch (positions.size()) {
        case 1:
            return MockMotionEvent(action, event_time, positions[0].x(), positions[0].y());
        case 2:
            return MockMotionEvent(action, event_time, positions[0].x(), positions[0].y(), positions[1].x(), positions[1].y());
        case 3:
            return MockMotionEvent(
                action, event_time, positions[0].x(), positions[0].y(), positions[1].x(), positions[1].y(), positions[2].x(), positions[2].y());
        default:
    ...
    static MockMotionEvent ObtainMotionEvent(base::TimeTicks event_time, MotionEvent::Action action)
    {
    ...
    {
        EXPECT_EQ(GestureDeviceType::DEVICE_TOUCHSCREEN, gesture.details.device_type());
    ...
    bool CancelActiveTouchSequence()
    {
        if (!gesture_provider_->current_down_event())
            return false;
        return gesture_provider_->OnTouchEvent(*gesture_provider_->current_down_event()->Cancel());
    }
    ...
    size_t GetReceivedGestureCount() const
    {
    ...
    {
        return active_scroll_begin_event_ ? active_scroll_begin_event_.get() : nullptr;
    }
    ...
    float GetTapSlop(MotionEvent::ToolType tool_type = MotionEvent::ToolType::FINGER) const
    {
        return tool_type == MotionEvent::ToolType::FINGER ? GetDefaultConfig().gesture_detector_config.touch_slop
    ...
    void EnableTwoFingerTap(float max_distance_for_two_finger_tap, base::TimeDelta two_finger_tap_timeout)
    {
        GestureProvider::Config config = GetDefaultConfig();
    ...
    void SetMinMaxGestureBoundsLength(float min_gesture_bound_length, float max_gesture_bound_length)
    {
        GestureProvider::Config config = GetDefaultConfig();
    ...
    void SetShowPressAndLongPressTimeout(base::TimeDelta showpress_timeout, base::TimeDelta shortpress_timeout, base::TimeDelta longpress_timeout)
    {
        GestureProvider::Config config = GetDefaultConfig();
    ...
    {
        GestureProvider::Config config = GetDefaultConfig();
        config.gesture_detector_config.stylus_button_accelerated_longpress_enabled = enabled;
        SetUpWithConfig(config);
    }
    ...
    {
        GestureProvider::Config config = GetDefaultConfig();
        config.gesture_detector_config.deep_press_accelerated_longpress_enabled = enabled;
        SetUpWithConfig(config);
    }
...
protected:
    void CheckScrollEventSequenceForEndActionType(MotionEvent::Action end_action_type)
    {
        ...
        MockMotionEvent event = ObtainMotionEvent(event_time, MotionEvent::Action::DOWN);
        ...
        EXPECT_EQ(GetMostRecentGestureEvent().unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
        uint32_t primary_unique_touch_event_id = GetMostRecentGestureEvent().unique_touch_event_id;

        event = ObtainMotionEvent(event_time + kOneSecond, MotionEvent::Action::MOVE, scroll_to_x, scroll_to_y);
        ...
        EXPECT_EQ(motion_event_flags, GetMostRecentGestureEvent().flags);
        EXPECT_EQ(event.GetToolType(0), GetMostRecentGestureEvent().primary_tool_type);
        EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
        EXPECT_EQ(EventType::kGestureScrollUpdate, GetMostRecentGestureEventType());
        EXPECT_EQ(BoundsForSingleMockTouchAtLocation(scroll_to_x, scroll_to_y), GetMostRecentGestureEvent().details.bounding_box_f());
        ...
        EXPECT_EQ(primary_unique_touch_event_id, GetReceivedGesture(1).details.primary_unique_touch_event_id());
        EXPECT_EQ(event_time + kOneSecond, GetReceivedGesture(1).time) << "ScrollBegin should have the time of the Action::MOVE";

        event = ObtainMotionEvent(event_time + kOneSecond, end_action_type, scroll_to_x, scroll_to_y);
        event.SetToolType(0, MotionEvent::ToolType::FINGER);
        event.SetPrimaryPointerId(motion_event_id);
    ...
    void CheckNoScrollWithinTouchSlop(MotionEvent::ToolType tool_type)
    {
        ...
        event = ObtainMotionEvent(event_time += kOneMicrosecond, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY + tap_slop);
        event.SetToolType(0, tool_type);
        EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
        EXPECT_FALSE(HasReceivedGesture(EventType::kGestureScrollBegin));

        event = ObtainMotionEvent(event_time += kOneMicrosecond, MotionEvent::Action::MOVE, kFakeCoordX + tap_slop, kFakeCoordY);
        event.SetToolType(0, tool_type);
        EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
        EXPECT_FALSE(HasReceivedGesture(EventType::kGestureScrollBegin));
        ...
        event = ObtainMotionEvent(event_time += kOneMicrosecond, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY + tap_slop + 1.f);
        event.SetToolType(0, tool_type);
        EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
        EXPECT_TRUE(HasReceivedGesture(EventType::kGestureScrollBegin));
    }
    ...
    void ThreeFingerSwipe(float vx0, float vy0, float vx1, float vy1, float vx2, float vy2)
    {
        std::vector<gfx::Vector2dF> velocities;
        velocities.push_back(gfx::Vector2dF(vx0, vy0));
        velocities.push_back(gfx::Vector2dF(vx1, vy1));
    ...
    void MultiFingerSwipe(std::vector<gfx::Vector2dF> velocities)
    {
        ...
        for (size_t i = 0; i < positions.size(); ++i) {
            ...
            event_positions.assign(positions.begin(), positions.begin() + pointer_count);
            MockMotionEvent event
                = ObtainMotionEvent(event_time, pointer_count > 1 ? MotionEvent::Action::POINTER_DOWN : MotionEvent::Action::DOWN, event_positions);
            EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
        }

        for (size_t i = 0; i < positions.size(); ++i)
            positions[i] += gfx::ScaleVector2d(velocities[i], dt);
        MockMotionEvent event = ObtainMotionEvent(event_time + kDeltaTimeForFlingSequences, MotionEvent::Action::MOVE, positions);
        EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

        for (size_t i = 0; i < positions.size(); ++i)
            positions[i] += gfx::ScaleVector2d(velocities[i], dt);
        event = ObtainMotionEvent(event_time + 2 * kDeltaTimeForFlingSequences, MotionEvent::Action::MOVE, positions);
        EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

        event = ObtainMotionEvent(event_time + 2 * kDeltaTimeForFlingSequences, MotionEvent::Action::POINTER_UP, positions);
        EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    }
    ...
    static void RunTasksAndWait(base::TimeDelta delay)
    {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), delay);
        run_loop.Run();
    }
...
TEST_F(GestureProviderTest, GestureTap)
{
    ...
    EXPECT_EQ(event.GetToolType(0), GetMostRecentGestureEvent().primary_tool_type);
    EXPECT_EQ(motion_event_flags, GetMostRecentGestureEvent().flags);
    EXPECT_EQ(BoundsForSingleMockTouchAtLocation(kFakeCoordX, kFakeCoordY), GetMostRecentGestureEvent().details.bounding_box_f());
    EXPECT_EQ(GetMostRecentGestureEvent().unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
    uint32_t primary_unique_touch_event_id = GetMostRecentGestureEvent().unique_touch_event_id;

    event = ObtainMotionEvent(event_time + kOneMicrosecond, MotionEvent::Action::UP);
    event.SetToolType(0, MotionEvent::ToolType::FINGER);
    event.SetPrimaryPointerId(motion_event_id);
    event.set_flags(motion_event_flags);
...
TEST_F(GestureProviderTest, GestureFlingAndCancelLongPress)
{
    ...
    event = ObtainMotionEvent(event_time + delta_time, MotionEvent::Action::MOVE, kFakeCoordX * 10, kFakeCoordY * 10);
    event.SetPrimaryPointerId(motion_event_id);
    event.set_flags(motion_event_flags);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());

    event = ObtainMotionEvent(event_time + delta_time * 2, MotionEvent::Action::UP, kFakeCoordX * 10, kFakeCoordY * 10);
    event.SetPrimaryPointerId(motion_event_id);
    event.set_flags(motion_event_flags);
    ...
    EXPECT_EQ(BoundsForSingleMockTouchAtLocation(kFakeCoordX * 10, kFakeCoordY * 10), GetMostRecentGestureEvent().details.bounding_box_f());
}
...
TEST_F(GestureProviderTest, FlingEventSequence)
{
    ...
    event = ObtainMotionEvent(event_time + delta_time, MotionEvent::Action::MOVE, kFakeCoordX * 5, kFakeCoordY * 5);
    event.SetPrimaryPointerId(motion_event_id);
    ...
    EXPECT_TRUE(hint_x > 0 && hint_y > 0 && hint_x > hint_y) << "ScrollBegin hint should be in positive X axis";
    ...
    EXPECT_EQ(event_time + delta_time * 2, GetMostRecentGestureEvent().time) << "FlingStart should have the time of the Action::UP";
}
...
TEST_F(GestureProviderTest, GestureCancelledOnCancelEvent)
{
    ...
    EXPECT_EQ(primary_unique_touch_event_id, GetNthMostRecentGestureEvent(1).details.primary_unique_touch_event_id());
    EXPECT_EQ(EventType::kGestureLongPress, GetNthMostRecentGestureEventType(0));
    EXPECT_EQ(primary_unique_touch_event_id, GetNthMostRecentGestureEvent(0).details.primary_unique_touch_event_id());
    ...
    event = ObtainMotionEvent(event_time + kOneMicrosecond * 2, MotionEvent::Action::UP);
...
TEST_F(GestureProviderTest, GestureCancelledOnDetectionReset)
{
    ...
    EXPECT_EQ(EventType::kGestureShowPress, GetNthMostRecentGestureEventType(2));
    EXPECT_EQ(primary_unique_touch_event_id, GetNthMostRecentGestureEvent(2).details.primary_unique_touch_event_id());
    EXPECT_EQ(EventType::kGestureShortPress, GetNthMostRecentGestureEventType(1));
    EXPECT_EQ(primary_unique_touch_event_id, GetNthMostRecentGestureEvent(1).details.primary_unique_touch_event_id());
    EXPECT_EQ(EventType::kGestureLongPress, GetNthMostRecentGestureEventType(0));
    EXPECT_EQ(primary_unique_touch_event_id, GetNthMostRecentGestureEvent(0).details.primary_unique_touch_event_id());
...
TEST_F(GestureProviderTest, TapPendingConfirmationCancelledOnCancelEvent)
{
    ...
    event = ObtainMotionEvent(event_time + kOneMicrosecond * 2, MotionEvent::Action::CANCEL);
    gesture_provider_->OnTouchEvent(event);
...
TEST_F(GestureProviderTest, NoTapAfterScrollBegins)
{
    ...
    EXPECT_EQ(GetMostRecentGestureEvent().unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
    uint32_t primary_unique_touch_event_id = GetMostRecentGestureEvent().unique_touch_event_id;
    event = ObtainMotionEvent(event_time + kOneMicrosecond, MotionEvent::Action::MOVE, kFakeCoordX + 50, kFakeCoordY + 50);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureScrollUpdate, GetMostRecentGestureEventType());
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());

    event = ObtainMotionEvent(event_time + kOneSecond, MotionEvent::Action::UP, kFakeCoordX + 50, kFakeCoordY + 50);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureScrollEnd, GetMostRecentGestureEventType());
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
...
TEST_F(GestureProviderTest, DoubleTap)
{
    ...
    event = ObtainMotionEvent(event_time + kOneMicrosecond, MotionEvent::Action::UP, kFakeCoordX, kFakeCoordY);
    gesture_provider_->OnTouchEvent(event);
    EXPECT_EQ(EventType::kGestureTapUnconfirmed, GetMostRecentGestureEventType());
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.touch_points());
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::DOWN, kFakeCoordX, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureTapDown, GetMostRecentGestureEventType());
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.touch_points());
    EXPECT_EQ(GetMostRecentGestureEvent().unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
    primary_unique_touch_event_id = GetMostRecentGestureEvent().unique_touch_event_id;

    // Moving a very small amount of distance should not trigger the double tap
    // drag zoom mode.
    event = ObtainMotionEvent(event_time + kOneMicrosecond, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY + 1);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureTapDown, GetMostRecentGestureEventType());
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.touch_points());
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());

    event = ObtainMotionEvent(event_time + kOneMicrosecond * 2, MotionEvent::Action::UP, kFakeCoordX, kFakeCoordY + 1);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ...
    EXPECT_EQ(primary_unique_touch_event_id, double_tap.details.primary_unique_touch_event_id());
}
...
TEST_F(GestureProviderTest, DoubleTapDragZoomBasic)
{
    ...
    MockMotionEvent event = ObtainMotionEvent(down_time_1, MotionEvent::Action::DOWN);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(GetMostRecentGestureEvent().unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
    uint32_t primary_unique_touch_event_id = GetMostRecentGestureEvent().unique_touch_event_id;

    event = ObtainMotionEvent(down_time_1 + kOneMicrosecond, MotionEvent::Action::UP, kFakeCoordX, kFakeCoordY);
    gesture_provider_->OnTouchEvent(event);
    EXPECT_EQ(EventType::kGestureTapUnconfirmed, GetMostRecentGestureEventType());
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.touch_points());
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());

    event = ObtainMotionEvent(down_time_2, MotionEvent::Action::DOWN, kFakeCoordX, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureTapDown, GetMostRecentGestureEventType());
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.touch_points());
    EXPECT_EQ(GetMostRecentGestureEvent().unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
    ...
    event = ObtainMotionEvent(down_time_2 + kOneMicrosecond, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY + 100);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ...
    EXPECT_EQ(BoundsForSingleMockTouchAtLocation(kFakeCoordX, kFakeCoordY + 100), GetMostRecentGestureEvent().details.bounding_box_f());

    event = ObtainMotionEvent(down_time_2 + kOneMicrosecond * 2, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY + 200);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ASSERT_EQ(EventType::kGesturePinchUpdate, GetMostRecentGestureEventType());
    EXPECT_LT(1.f, GetMostRecentGestureEvent().details.scale());
    EXPECT_EQ(BoundsForSingleMockTouchAtLocation(kFakeCoordX, kFakeCoordY + 200), GetMostRecentGestureEvent().details.bounding_box_f());
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());

    event = ObtainMotionEvent(down_time_2 + kOneMicrosecond * 3, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY + 100);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ASSERT_EQ(EventType::kGesturePinchUpdate, GetMostRecentGestureEventType());
    ...
    event = ObtainMotionEvent(down_time_2 + kOneMicrosecond * 4, MotionEvent::Action::UP, kFakeCoordX, kFakeCoordY - 200);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, ScrollUpdateValues)
{
    ...
    event = ObtainMotionEvent(event_time + kOneMicrosecond, MotionEvent::Action::MOVE, kFakeCoordX - delta_x / 2, kFakeCoordY - delta_y / 2);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());

    event = ObtainMotionEvent(event_time + kOneMicrosecond * 2, MotionEvent::Action::MOVE, kFakeCoordX - delta_x, kFakeCoordY - delta_y);
    ...
    EXPECT_EQ(primary_unique_touch_event_id, gesture.details.primary_unique_touch_event_id());
}
...
TEST_F(GestureProviderTest, FractionalScroll)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY + 100);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
    ...
    // horizontal motion.
    for (int i = 1; i <= 10; i++) {
        event = ObtainMotionEvent(event_time + kOneMicrosecond * i, MotionEvent::Action::MOVE, kFakeCoordX + delta_x * i, kFakeCoordY + delta_y * i);
        EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, ScrollBeginValues)
{
    ...
    event = ObtainMotionEvent(event_time + kOneMicrosecond, MotionEvent::Action::MOVE, kFakeCoordX + 2, kFakeCoordY + 1);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ...
    EXPECT_EQ(primary_unique_touch_event_id, scroll_begin_gesture->details.primary_unique_touch_event_id());
}
...
TEST_F(GestureProviderTest, SlopRegionCheckOnOneFingerScroll)
{
    ...
    MockMotionEvent event = ObtainMotionEvent(event_time, MotionEvent::Action::DOWN, 0, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(GetMostRecentGestureEvent().unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
    uint32_t primary_unique_touch_event_id = GetMostRecentGestureEvent().unique_touch_event_id;

    // Move within slop region.
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, 0, scaled_tap_slop / 2);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    // Exceed slop region.
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, 0, 2 * scaled_tap_slop);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    event = ObtainMotionEvent(event_time, MotionEvent::Action::UP, 0, 2 * scaled_tap_slop);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ...
    EXPECT_EQ(primary_unique_touch_event_id, GetReceivedGesture(2).details.primary_unique_touch_event_id());
    EXPECT_EQ(EventType::kGestureScrollEnd, GetReceivedGesture(3).type());
    EXPECT_EQ(primary_unique_touch_event_id, GetReceivedGesture(3).details.primary_unique_touch_event_id());
...
TEST_F(GestureProviderTest, SlopRegionCheckOnTwoFingerScroll)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, 0, 0, kMaxTwoFingerTapSeparation / 2, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());

    // Move within slop region: two-finger tap happens.
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, 0, scaled_tap_slop / 2, kMaxTwoFingerTapSeparation / 2, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_UP, 0, scaled_tap_slop / 2, kMaxTwoFingerTapSeparation / 2, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());

    // Exceed slop region: scroll.
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, 0, scaled_tap_slop / 2, kMaxTwoFingerTapSeparation / 2, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::UP, 0, scaled_tap_slop / 2);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(primary_unique_touch_event_id, GetMostRecentGestureEvent().details.primary_unique_touch_event_id());
...
TEST_F(GestureProviderTest, NoSlopRegionCheckOnThreeFingerScroll)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, 0, 0, kMaxTwoFingerTapSeparation / 2, 0, 2 * kMaxTwoFingerTapSeparation, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    // Move within slop region, three-finger scroll always happens.
    event = ObtainMotionEvent(
        event_time, MotionEvent::Action::MOVE, 0, scaled_tap_slop / 2, kMaxTwoFingerTapSeparation / 2, 0, 2 * kMaxTwoFingerTapSeparation, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, NoFlingBeforeExeedingSlopRegion)
{
    base::TimeTicks event_time = base::TimeTicks::Now();
    base::TimeDelta delta_time = kDeltaTimeForFlingSequences;
    MockMotionEvent event = ObtainMotionEvent(event_time + delta_time, MotionEvent::Action::DOWN, 0, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    event = ObtainMotionEvent(event_time + 2 * delta_time, MotionEvent::Action::POINTER_DOWN, 0, 0, 10, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    // Fast pointer movements within touch slop region.
    event = ObtainMotionEvent(event_time + 3 * delta_time, MotionEvent::Action::MOVE, 1, 0, 11, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    event = ObtainMotionEvent(event_time + 4 * delta_time, MotionEvent::Action::MOVE, 2, 0, 12, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    event = ObtainMotionEvent(event_time + 5 * delta_time, MotionEvent::Action::MOVE, 3, 0, 13, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, LongPressAndTapCancelledWhenScrollBegins)
{
    ...
    MockMotionEvent event = ObtainMotionEvent(event_time, MotionEvent::Action::DOWN);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    event = ObtainMotionEvent(event_time + kOneMicrosecond, MotionEvent::Action::MOVE, kFakeCoordX * 5, kFakeCoordY * 5);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    event = ObtainMotionEvent(event_time + kOneMicrosecond * 2, MotionEvent::Action::MOVE, kFakeCoordX * 10, kFakeCoordY * 10);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, GestureLongTap)
{
    ...
    EXPECT_EQ(BoundsForSingleMockTouchAtLocation(kFakeCoordX, kFakeCoordY), GetNthMostRecentGestureEvent(1).details.bounding_box_f());
    EXPECT_EQ(EventType::kGestureLongPress, GetNthMostRecentGestureEventType(0));
    EXPECT_EQ(1, GetNthMostRecentGestureEvent(0).details.touch_points());
    EXPECT_EQ(BoundsForSingleMockTouchAtLocation(kFakeCoordX, kFakeCoordY), GetNthMostRecentGestureEvent(0).details.bounding_box_f());
...
TEST_F(GestureProviderTest, GestureLongPressDoesNotPreventScrolling)
{
    ...
    event = ObtainMotionEvent(event_time + long_press_timeout, MotionEvent::Action::MOVE, kFakeCoordX + 100, kFakeCoordY + 100);
    gesture_provider_->OnTouchEvent(event);
    ...
    event = ObtainMotionEvent(event_time + long_press_timeout, MotionEvent::Action::UP);
    gesture_provider_->OnTouchEvent(event);
    EXPECT_FALSE(HasReceivedGesture(EventType::kGestureLongTap));
}
...
TEST_F(GestureProviderTest, DeepPressAcceleratedLongPress)
{
    ...
    event = ObtainMotionEvent(event_time + kOneMicrosecond, MotionEvent::Action::MOVE);
    event.SetToolType(0, MotionEvent::ToolType::FINGER);
...
TEST_F(GestureProviderTest, NoGestureLongPressDuringDoubleTap)
{
    ...
    MockMotionEvent event = ObtainMotionEvent(event_time, MotionEvent::Action::DOWN, kFakeCoordX, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ...
    event = ObtainMotionEvent(event_time + long_press_timeout, MotionEvent::Action::MOVE, kFakeCoordX + 20, kFakeCoordY + 20);
    event.SetPrimaryPointerId(motion_event_id);
    ...
    event = ObtainMotionEvent(event_time + long_press_timeout + kOneMicrosecond, MotionEvent::Action::UP, kFakeCoordX, kFakeCoordY + 1);
    event.SetPrimaryPointerId(motion_event_id);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGesturePinchEnd, GetMostRecentGestureEventType());
    EXPECT_EQ(motion_event_id, GetMostRecentGestureEvent().motion_event_id);
...
TEST_F(GestureProviderTest, TouchSlopRemovedFromScroll)
{
    ...
    event = ObtainMotionEvent(event_time + kOneMicrosecond * 2, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY + tap_slop + scroll_delta);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, NoDoubleTapDragZoomWhenDisabledOnPlatform)
{
    ...
    EXPECT_EQ(EventType::kGestureScrollUpdate, GetMostRecentGestureEventType());
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.touch_points());
    EXPECT_EQ(down_time_2 + kOneMicrosecond * 2, GetMostRecentGestureEvent().time);
    EXPECT_FALSE(HasReceivedGesture(EventType::kGesturePinchUpdate));

    event = ObtainMotionEvent(down_time_2 + kOneMicrosecond * 3, MotionEvent::Action::UP, kFakeCoordX, kFakeCoordY + 200);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, PinchZoom)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, kFakeCoordX, kFakeCoordY, secondary_coord_x, secondary_coord_y);
    event.SetPrimaryPointerId(motion_event_id);
    event.SetRawOffset(raw_offset_x, raw_offset_y);
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY, secondary_coord_x, secondary_coord_y);
    event.SetPrimaryPointerId(motion_event_id);
    event.SetRawOffset(raw_offset_x, raw_offset_y);
    ...
    EXPECT_EQ((kFakeCoordX + secondary_coord_x) / 2 + raw_offset_x, GetReceivedGesture(3).raw_x);
    EXPECT_EQ((kFakeCoordY + secondary_coord_y) / 2 + raw_offset_y, GetReceivedGesture(3).raw_y);

    EXPECT_EQ(gfx::RectF(kFakeCoordX - kMockTouchRadius, kFakeCoordY - kMockTouchRadius, secondary_coord_x - kFakeCoordX + kMockTouchRadius * 2,
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_UP, kFakeCoordX, kFakeCoordY, secondary_coord_x, secondary_coord_y);
    event.SetPrimaryPointerId(motion_event_id);
    ...
    EXPECT_EQ(EventType::kGestureScrollEnd, GetMostRecentGestureEventType());
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.touch_points());
    EXPECT_EQ(gfx::RectF(kFakeCoordX - kMockTouchRadius, kFakeCoordY - kMockTouchRadius, kMockTouchRadius * 2, kMockTouchRadius * 2),
        GetMostRecentGestureEvent().details.bounding_box_f());
}
...
TEST_F(GestureProviderTest, NoPinchZoomWithFatFinger)
{
    ...
    event = ObtainMotionEvent(event_time + kOneSecond * 2, MotionEvent::Action::MOVE, kFakeCoordX + 1.f, kFakeCoordY);
    event.SetTouchMajor(1.f);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(1U, GetReceivedGestureCount());

    event = ObtainMotionEvent(event_time + kOneSecond * 3, MotionEvent::Action::MOVE);
    event.SetTouchMajor(kFatFingerSize * 3.5f);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(1U, GetReceivedGestureCount());

    event = ObtainMotionEvent(event_time + kOneSecond * 4, MotionEvent::Action::MOVE);
    event.SetTouchMajor(kFatFingerSize * 5.f);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(1U, GetReceivedGestureCount());

    event = ObtainMotionEvent(event_time + kOneSecond * 4, MotionEvent::Action::MOVE, kFakeCoordX + 50.f, kFakeCoordY - 25.f);
    event.SetTouchMajor(kFatFingerSize * 10.f);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_FALSE(HasReceivedGesture(EventType::kGesturePinchBegin));

    event = ObtainMotionEvent(event_time + kOneSecond * 4, MotionEvent::Action::MOVE, kFakeCoordX + 100.f, kFakeCoordY - 50.f);
    event.SetTouchMajor(kFatFingerSize * 5.f);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_FALSE(HasReceivedGesture(EventType::kGesturePinchBegin));
}
...
TEST_F(GestureProviderTest, MultiFingerSwipe)
{
    ...
    TwoFingerSwipe(min_swipe_velocity * 2, min_swipe_velocity * 2, min_swipe_velocity * 2, min_swipe_velocity * 2);
    EXPECT_FALSE(HasReceivedGesture(EventType::kGestureSwipe));
    ResetGestureDetection();

    // Swipe down with non-zero velocities on both axes and dominant direction.
    TwoFingerSwipe(-min_swipe_velocity, min_swipe_velocity * 4, -min_swipe_velocity, min_swipe_velocity * 4);
    EXPECT_TRUE(HasReceivedGesture(EventType::kGestureSwipe));
    EXPECT_TRUE(GetMostRecentGestureEvent().details.swipe_down());
    ...
    TwoFingerSwipe(min_swipe_velocity / 2, min_swipe_velocity / 2, 0, min_swipe_velocity * 2);
    EXPECT_TRUE(HasReceivedGesture(EventType::kGestureSwipe));
    EXPECT_TRUE(GetMostRecentGestureEvent().details.swipe_down());
    EXPECT_FALSE(GetMostRecentGestureEvent().details.swipe_right());
    EXPECT_EQ(2, GetMostRecentGestureEvent().details.touch_points());
    ...
    ThreeFingerSwipe(min_swipe_velocity * 2, 0, min_swipe_velocity * 3, 0, min_swipe_velocity * 4, 0);
    EXPECT_TRUE(HasReceivedGesture(EventType::kGestureSwipe));
    EXPECT_TRUE(GetMostRecentGestureEvent().details.swipe_right());
    ...
    ThreeFingerSwipe(min_swipe_velocity * 2, 0, 0, min_swipe_velocity * 3, min_swipe_velocity * 4, 0);
    EXPECT_FALSE(HasReceivedGesture(EventType::kGestureSwipe));
}
...
TEST_F(GestureProviderTest, CancelActiveTouchSequence)
{
    ...
    event = ObtainMotionEvent(event_time + kOneMicrosecond * 3, MotionEvent::Action::DOWN);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureTapDown, GetMostRecentGestureEventType());
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.touch_points());
}
...
TEST_F(GestureProviderTest, DoubleTapDragZoomCancelledOnSecondaryPointerDown)
{
    ...
    event = ObtainMotionEvent(down_time_1 + kOneMicrosecond, MotionEvent::Action::UP);
    gesture_provider_->OnTouchEvent(event);
    EXPECT_EQ(EventType::kGestureTapUnconfirmed, GetMostRecentGestureEventType());
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.touch_points());
    ...
    event = ObtainMotionEvent(down_time_2 + kOneMicrosecond, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY - 30);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGesturePinchBegin, GetNthMostRecentGestureEventType(1));
    EXPECT_EQ(EventType::kGesturePinchUpdate, GetMostRecentGestureEventType());
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.touch_points());
    ...
    event = ObtainMotionEvent(
        down_time_2 + kOneMicrosecond * 2, MotionEvent::Action::POINTER_DOWN, kFakeCoordX, kFakeCoordY - 30, kFakeCoordX + 50, kFakeCoordY + 50);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGesturePinchEnd, GetMostRecentGestureEventType());
    EXPECT_EQ(2, GetMostRecentGestureEvent().details.touch_points());
    ...
    event = ObtainMotionEvent(
        down_time_2 + kOneMicrosecond * 3, MotionEvent::Action::POINTER_UP, kFakeCoordX, kFakeCoordY - 30, kFakeCoordX + 50, kFakeCoordY + 50);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, GestureBeginAndEnd)
{
    ...
    EXPECT_EQ(0U, GetReceivedGestureCount());
    MockMotionEvent event = ObtainMotionEvent(event_time, MotionEvent::Action::DOWN, 1, 1);
    event.SetRawOffset(raw_offset_x, raw_offset_y);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ...
    EXPECT_EQ(gfx::RectF(1 - kMockTouchRadius, 1 - kMockTouchRadius, kMockTouchRadius * 2, kMockTouchRadius * 2),
        GetMostRecentGestureEvent().details.bounding_box_f());

    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, 1, 1, 2, 2);
    event.SetRawOffset(raw_offset_x, raw_offset_y);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, 1, 1, 2, 2, 3, 3);
    event.SetRawOffset(raw_offset_x, raw_offset_y);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureBegin, GetMostRecentGestureEventType());
    EXPECT_EQ(4U, GetReceivedGestureCount());
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_UP, 1, 1, 2, 2, 3, 3);
    event.SetRawOffset(raw_offset_x, raw_offset_y);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, 2, 2, 3, 3, 4, 4);
    event.SetRawOffset(raw_offset_x, raw_offset_y);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureBegin, GetMostRecentGestureEventType());
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_UP, 2, 2, 3, 3, 4, 4);
    event.SetRawOffset(raw_offset_x, raw_offset_y);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureEnd, GetMostRecentGestureEventType());
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_UP, 3, 3, 4, 4);
    event.SetRawOffset(raw_offset_x, raw_offset_y);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureEnd, GetMostRecentGestureEventType());
...
TEST_F(GestureProviderTest, GestureBeginAndEndOnCancel)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::CANCEL, 1, 1, 2, 2, 3, 3);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(5U, GetReceivedGestureCount());
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::CANCEL, 1, 1, 3, 3);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(6U, GetReceivedGestureCount());
    EXPECT_EQ(2, GetMostRecentGestureEvent().details.touch_points());
...
TEST_F(GestureProviderTest, TwoFingerTap)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, 0, -scaled_tap_slop / 2, kMaxTwoFingerTapSeparation / 2 + scaled_tap_slop / 2, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_UP, 0, 0, kMaxTwoFingerTapSeparation, 0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    EXPECT_EQ(EventType::kGestureTapDown, GetReceivedGesture(0).type());
    EXPECT_EQ(EventType::kGestureTwoFingerTap, GetReceivedGesture(1).type());
    EXPECT_EQ(2U, GetReceivedGestureCount());
    EXPECT_EQ(kMockTouchRadius * 2, GetReceivedGesture(1).details.first_finger_width());
    EXPECT_EQ(kMockTouchRadius * 2, GetReceivedGesture(1).details.first_finger_height());
}
...
TEST_F(GestureProviderTest, TwoFingerTapCancelledByFingerMovement)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, kFakeCoordX, kFakeCoordY, kFakeCoordX, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY, kFakeCoordX + 2 * scaled_tap_slop + 2, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_UP, kFakeCoordX, kFakeCoordY, kFakeCoordX, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    ...
    EXPECT_FLOAT_EQ(scaled_tap_slop / 2 + 1, GetReceivedGesture(2).details.scroll_x());
...
TEST_F(GestureProviderTest, TwoFingerTapCancelledByDelay)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY);

    event
        = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, kFakeCoordX, kFakeCoordY, kFakeCoordX + kMaxTwoFingerTapSeparation / 2, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    event = ObtainMotionEvent(event_time + kOneSecond + kOneMicrosecond, MotionEvent::Action::POINTER_UP, kFakeCoordX, kFakeCoordY,
...
TEST_F(GestureProviderTest, TwoFingerTapCancelledByDistanceBetweenPointers)
{
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, kFakeCoordX, kFakeCoordY, kFakeCoordX + kMaxTwoFingerTapSeparation, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_UP, kFakeCoordX, kFakeCoordY, kFakeCoordX + kMaxTwoFingerTapSeparation, kFakeCoordY);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
...
TEST_F(GestureProviderTest, PinchExceedingSlopCausesUpdate)
{
    ...
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(6U, GetReceivedGestureCount());
    EXPECT_EQ(EventType::kGestureScrollUpdate, GetNthMostRecentGestureEventType(2));
    EXPECT_EQ(EventType::kGesturePinchBegin, GetNthMostRecentGestureEventType(1));
    EXPECT_EQ(EventType::kGesturePinchUpdate, GetNthMostRecentGestureEventType(0));
    EXPECT_LT(1.f, GetMostRecentGestureEvent().details.scale());
}
...
TEST_F(GestureProviderTest, PinchBelowMinSpanCausesUpdate)
{
    ...
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(5U, GetReceivedGestureCount());
    EXPECT_EQ(EventType::kGestureScrollBegin, GetNthMostRecentGestureEventType(3));
    EXPECT_EQ(EventType::kGestureScrollUpdate, GetNthMostRecentGestureEventType(2));
    EXPECT_EQ(EventType::kGesturePinchBegin, GetNthMostRecentGestureEventType(1));
    EXPECT_EQ(EventType::kGesturePinchUpdate, GetNthMostRecentGestureEventType(0));
    ...
    EXPECT_EQ(8U, GetReceivedGestureCount());
    EXPECT_EQ(EventType::kGestureScrollUpdate, GetNthMostRecentGestureEventType(2));
    EXPECT_EQ(EventType::kGesturePinchUpdate, GetNthMostRecentGestureEventType(1));
...
TEST_F(GestureProviderTest, PinchZoomWithThreshold)
{
    ...
    gestures_.clear();
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY, secondary_coord_x + kMinPinchUpdateDistance, secondary_coord_y);
    ...
    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, kFakeCoordX, kFakeCoordY,
        secondary_coord_x + kMinPinchUpdateDistance + kOvershootMinPinchUpdateDistance, secondary_coord_y);
...
TEST_F(GestureProviderTest, MinGestureBoundsLength)
{
    ...
    EXPECT_EQ(EventType::kGestureTapDown, GetMostRecentGestureEventType());
    EXPECT_EQ(kMinGestureBoundsLength, GetMostRecentGestureEvent().details.bounding_box_f().width());
    EXPECT_EQ(kMinGestureBoundsLength, GetMostRecentGestureEvent().details.bounding_box_f().height());
...
TEST_F(GestureProviderTest, MaxGestureBoundsLength)
{
    ...
    EXPECT_EQ(EventType::kGestureTapDown, GetMostRecentGestureEventType());
    EXPECT_EQ(kMaxGestureBoundsLength, GetMostRecentGestureEvent().details.bounding_box_f().width());
    EXPECT_EQ(kMaxGestureBoundsLength, GetMostRecentGestureEvent().details.bounding_box_f().height());
...
{
    base::TimeTicks event_time = base::TimeTicks::Now();
    MockMotionEvent event = ObtainMotionEvent(event_time, MotionEvent::Action::DOWN, 10, 20);
    event.SetTouchMajor(0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(gfx::RectF(10, 20, 0, 0), GetMostRecentGestureEvent().details.bounding_box_f());

    event = ObtainMotionEvent(event_time, MotionEvent::Action::POINTER_DOWN, 10, 20, 110, 120);
    event.SetTouchMajor(0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    event = ObtainMotionEvent(event_time, MotionEvent::Action::MOVE, 10, 20, 110, 150);
    event.SetTouchMajor(0);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));

    EXPECT_EQ(gfx::RectF(10, 20, 100, 130), GetMostRecentGestureEvent().details.bounding_box_f());
}
...
TEST_F(GestureProviderTest, NoMinOrMaxGestureBoundsLengthWithStylusOrMouse)
{
    ...
    SetMinMaxGestureBoundsLength(kMinGestureBoundsLength, kMaxGestureBoundsLength);
    gesture_provider_->SetDoubleTapSupportForPlatformEnabled(false);
    ...
    EXPECT_EQ(EventType::kGestureTapDown, GetMostRecentGestureEventType());
    EXPECT_EQ(MotionEvent::ToolType::MOUSE, GetMostRecentGestureEvent().primary_tool_type);
    ...
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureTap, GetMostRecentGestureEventType());
    EXPECT_EQ(MotionEvent::ToolType::STYLUS, GetMostRecentGestureEvent().primary_tool_type);
    ...
    EXPECT_EQ(MotionEvent::ToolType::MOUSE, GetMostRecentGestureEvent().primary_tool_type);
    EXPECT_EQ(EventType::kGestureTapDown, GetMostRecentGestureEventType());
    EXPECT_EQ(2.f * kMaxGestureBoundsLength, GetMostRecentGestureEvent().details.bounding_box_f().width());
    EXPECT_EQ(2.f * kMaxGestureBoundsLength, GetMostRecentGestureEvent().details.bounding_box_f().height());
    ...
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureTap, GetMostRecentGestureEventType());
    EXPECT_EQ(MotionEvent::ToolType::ERASER, GetMostRecentGestureEvent().primary_tool_type);
    EXPECT_EQ(2.f * kMaxGestureBoundsLength, GetMostRecentGestureEvent().details.bounding_box_f().width());
    EXPECT_EQ(2.f * kMaxGestureBoundsLength, GetMostRecentGestureEvent().details.bounding_box_f().height());
}
...
#define MAYBE_BoundingBoxForShowPressAndTapGesture DISABLED_BoundingBoxForShowPressAndTapGesture
#else
#define MAYBE_BoundingBoxForShowPressAndTapGesture BoundingBoxForShowPressAndTapGesture
#endif
...
TEST_F(GestureProviderTest, MAYBE_BoundingBoxForShowPressAndTapGesture)
{
    ...
    SetShowPressAndLongPressTimeout(showpress_timeout, shortpress_timeout, longpress_timeout);

    MockMotionEvent event = ObtainMotionEvent(event_time, MotionEvent::Action::DOWN, 10, 10);
    ...
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.tap_down_count());
    EXPECT_EQ(1, GetMostRecentGestureEvent().details.touch_points());
    EXPECT_EQ(gfx::RectF(5, 5, 10, 10), GetMostRecentGestureEvent().details.bounding_box_f());

    event = ObtainMotionEvent(event_time + kOneMicrosecond, MotionEvent::Action::MOVE, 11, 9);
    event.SetTouchMajor(20);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    event = ObtainMotionEvent(event_time + kOneMicrosecond, MotionEvent::Action::MOVE, 8, 11);
    event.SetTouchMajor(10);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    RunTasksAndWait(showpress_timeout + kOneMicrosecond);
    EXPECT_EQ(EventType::kGestureShowPress, GetMostRecentGestureEventType());
    EXPECT_EQ(gfx::RectF(0, 0, 20, 20), GetMostRecentGestureEvent().details.bounding_box_f());
...
TEST_F(GestureProviderTest, SingleTapRepeat)
{
    ...
    // the tap count.
    event_time += GetValidDoubleTapDelay();
    event = ObtainMotionEvent(event_time, MotionEvent::Action::DOWN, kFakeCoordX, kFakeCoordY + GetTapSlop() / 2);
    EXPECT_TRUE(gesture_provider_->OnTouchEvent(event));
    EXPECT_EQ(EventType::kGestureTapDown, GetMostRecentGestureEventType());
