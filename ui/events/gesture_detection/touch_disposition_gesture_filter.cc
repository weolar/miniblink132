...
static_assert(base::to_underlying(EventType::kGestureTypeEnd) - base::to_underlying(EventType::kGestureTypeStart) < 32, "gesture type count too large");

GestureEventData CreateGesture(EventType type, int motion_event_id, MotionEvent::ToolType primary_tool_type, const GestureEventDataPacket& packet)
{
    ...
    return GestureEventData(details, motion_event_id, primary_tool_type, packet.timestamp(), packet.touch_location().x(), packet.touch_location().y(),
        packet.raw_touch_location().x(), packet.raw_touch_location().y(), 1, gfx::RectF(packet.touch_location(), gfx::SizeF()), flags,
...
struct DispositionHandlingInfo {
    ...
    DispositionHandlingInfo(int required_touches, EventType antecedent_event_type)
        : required_touches(required_touches)
...
DispositionHandlingInfo Info(int required_touches, EventType antecedent_event_type)
{
...
int GetGestureTypeIndex(EventType type)
{
    ...
    return base::to_underlying(type) - base::to_underlying(EventType::kGestureTypeStart);
}
...
{
    return gesture_data.type() == EventType::kGestureScrollUpdate || gesture_data.type() == EventType::kGestureScrollBegin;
}
...
TouchDispositionGestureFilter::TouchDispositionGestureFilter(TouchDispositionGestureFilterClient* client)
    : client_(client)
...
TouchDispositionGestureFilter::PacketResult TouchDispositionGestureFilter::OnGesturePacket(const GestureEventDataPacket& packet)
{
    ...
    if (!Tail().empty()) {
        DCHECK((packet.gesture_source() == GestureEventDataPacket::TOUCH_TIMEOUT) || (packet.unique_touch_event_id() != Tail().back().unique_touch_event_id()));
    }
    if (!Head().empty()) {
        DCHECK((packet.gesture_source() == GestureEventDataPacket::TOUCH_TIMEOUT) || packet.unique_touch_event_id() != Head().front().unique_touch_event_id());
    }
...
void TouchDispositionGestureFilter::OnTouchEventAck(uint32_t unique_touch_event_id, bool event_consumed, bool is_source_touch_event_set_blocking,
    const std::optional<EventLatencyMetadata>& event_latency_metadata)
{
    ...
    if (!Tail().empty() && Tail().back().unique_touch_event_id() == unique_touch_event_id
...
void TouchDispositionGestureFilter::SendAckedEvents(const std::optional<EventLatencyMetadata>& event_latency_metadata)
{
    ...
    while (!IsEmpty() && (!Head().empty() || sequences_.size() != 1)) {
        ...
        DCHECK_NE(sequence.front().gesture_source(), GestureEventDataPacket::UNDEFINED);
        DCHECK_NE(sequence.front().gesture_source(), GestureEventDataPacket::INVALID);
        ...
        if (source != GestureEventDataPacket::TOUCH_TIMEOUT) {
            ...
            state_.OnTouchEventAck(ack_state == GestureEventDataPacket::AckState::CONSUMED, IsTouchStartEvent(source));
        }
        ...
        if (source == GestureEventDataPacket::TOUCH_MOVE && event_latency_metadata.has_value()) {
            EventLatencyMetadata gesture_event_latency_metadata;
            gesture_event_latency_metadata.scrolls_blocking_touch_dispatched_to_renderer = event_latency_metadata->dispatched_to_renderer;
            packet.AddEventLatencyMetadataToGestures(std::move(gesture_event_latency_metadata), base::BindRepeating(DoAddInputTimestampsToGesture));
        }
...
void TouchDispositionGestureFilter::FilterAndSendPacket(const GestureEventDataPacket& packet)
{
    TRACE_EVENT("input", "TouchDispositionGestureFilter::FilterAndSendPacket", [&](perfetto::EventContext ctx) {
    ...
    for (size_t i = 0; i < packet.gesture_count(); ++i) {
        ...
        if (state_.Filter(gesture.details.type())) {
            ...
            if (base::FeatureList::IsEnabled(features::kEnableGestureBeginEndTypes)
                && (gesture.details.type() == EventType::kGestureBegin || gesture.details.type() == EventType::kGestureEnd)) {
...
void TouchDispositionGestureFilter::SendGesture(const GestureEventData& event, const GestureEventDataPacket& packet_being_sent)
{
    TRACE_EVENT("input", "TouchDispositionGestureFilter::SendGesture");
    DCHECK(event.unique_touch_event_id == packet_being_sent.unique_touch_event_id());
    ...
    case EventType::kGestureTap:
        DCHECK(needs_tap_ending_event_);
        if (needs_show_press_event_) {
            SendGesture(GestureEventData(EventType::kGestureShowPress, event), packet_being_sent);
...
void TouchDispositionGestureFilter::CancelTapIfNecessary(const GestureEventDataPacket& packet_being_sent)
{
    if (!needs_tap_ending_event_)
        return;

    SendGesture(
        CreateGesture(EventType::kGestureTapCancel, ending_event_motion_event_id_, ending_event_primary_tool_type_, packet_being_sent), packet_being_sent);
    DCHECK(!needs_tap_ending_event_);
}

void TouchDispositionGestureFilter::CancelFlingIfNecessary(const GestureEventDataPacket& packet_being_sent)
{
    if (!needs_fling_ending_event_)
        return;

    SendGesture(
        CreateGesture(EventType::kScrollFlingCancel, ending_event_motion_event_id_, ending_event_primary_tool_type_, packet_being_sent), packet_being_sent);
    DCHECK(!needs_fling_ending_event_);
}

void TouchDispositionGestureFilter::EndScrollIfNecessary(const GestureEventDataPacket& packet_being_sent)
{
    if (!needs_scroll_ending_event_)
        return;

    SendGesture(
        CreateGesture(EventType::kGestureScrollEnd, ending_event_motion_event_id_, ending_event_primary_tool_type_, packet_being_sent), packet_being_sent);
...
TouchDispositionGestureFilter::GestureSequence& TouchDispositionGestureFilter::Head()
{
    DCHECK(!sequences_.empty());
    return sequences_.front();
}

TouchDispositionGestureFilter::GestureSequence& TouchDispositionGestureFilter::Tail()
{
    DCHECK(!sequences_.empty());
...
void TouchDispositionGestureFilter::GestureHandlingState::OnTouchEventAck(bool event_consumed, bool is_touch_start_event)
{
...
bool TouchDispositionGestureFilter::GestureHandlingState::Filter(EventType gesture_type)
{
    ...
    if ((required_touches & RT_START && start_touch_consumed_) || (required_touches & RT_CURRENT && current_touch_consumed_)
        || (antecedent_event_type != EventType::kUnknown && last_gesture_of_type_dropped_.has_bit(GetGestureTypeIndex(antecedent_event_type)))) {
...
bool TouchDispositionGestureFilter::GestureHandlingState::HasFilteredGestureType(EventType gesture_type) const
{
    return any_gesture_of_type_dropped_.has_bit(GetGestureTypeIndex(gesture_type));
}
