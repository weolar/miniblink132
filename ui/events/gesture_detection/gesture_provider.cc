...
gfx::RectF ClampBoundingBox(const gfx::RectF& bounds, float min_length, float max_length)
{
    ...
    return gfx::RectF(center.x() - width / 2.f, center.y() - height / 2.f, width, height);
}

float EffectiveSlopDistance(const MotionEvent& event, const GestureProvider::Config& config)
{
    return (base::FeatureList::IsEnabled(features::kStylusSpecificTapSlop) && event.GetToolType() == MotionEvent::ToolType::STYLUS)
...
class GestureProvider::GestureListenerImpl : public ScaleGestureListener, public GestureListener, public DoubleTapListener {
public:
    GestureListenerImpl(const GestureProvider::Config& config, GestureProviderClient* client, GestureProvider* gesture_provider)
        : config_(config)
        , client_(client)
    ...
    void OnTouchEvent(const MotionEvent& event)
    {
        ...
        snap_scroll_controller_.SetSnapScrollMode(event, in_scale_gesture, EffectiveSlopDistance(event, config_));
        if (in_scale_gesture) {
        ...
        if (action == MotionEvent::Action::DOWN) {
            ...
            current_down_action_unique_touch_event_id_ = gesture_provider_->current_down_event()->GetUniqueEventId();
        ...
        gesture_detector_.OnTouchEvent(event, client_->RequiresDoubleTapGestureEvents());
        scale_gesture_detector_.OnTouchEvent(event);

        if (action == MotionEvent::Action::UP || action == MotionEvent::Action::CANCEL) {
            ...
            if (scroll_event_sent_)
                Send(CreateGesture(CreateTouchGestureDetails(EventType::kGestureScrollEnd), event));
            ...
            if (action != MotionEvent::Action::CANCEL || !GestureConfiguration::GetInstance()->single_pointer_cancel_enabled() || event.GetPointerCount() == 1)
            ...
            if (!show_press_event_sent_ && !scroll_event_sent_) {
                max_diameter_before_show_press_ = std::max(max_diameter_before_show_press_, event.GetTouchMajor());
            }
        }
    }
    ...
    void SendImpl(GestureEventData gesture, bool should_update)
    {
        ...
        DCHECK(!current_down_action_event_time_.is_null() || gesture.type() == EventType::kGestureTap || gesture.type() == EventType::kGestureShowPress
            || gesture.type() == EventType::kGestureTapCancel || gesture.type() == EventType::kGestureBegin || gesture.type() == EventType::kGestureEnd);

        if (gesture.primary_tool_type == MotionEvent::ToolType::UNKNOWN || gesture.primary_tool_type == MotionEvent::ToolType::FINGER) {
            gesture.details.set_bounding_box(
                ClampBoundingBox(gesture.details.bounding_box_f(), config_.min_gesture_bounds_length, config_.max_gesture_bounds_length));
        }
        ...
        case EventType::kGestureScrollEnd:
            ...
            if (pinch_event_sent_)
                SendImpl(GestureEventData(EventType::kGesturePinchEnd, gesture), should_update);
            break;
        case EventType::kGesturePinchBegin:
            DCHECK(!pinch_event_sent_);
            if (!scroll_event_sent_ && !scale_gesture_detector_.InAnchoredScaleMode()) {
                SendImpl(GestureEventData(EventType::kGestureScrollBegin, gesture), should_update);
            }
            break;
        default:
            break;
    ...
    {
        MotionEventGeneric generic_cancel_event(MotionEvent::Action::CANCEL, base::TimeTicks::Now(), PointerProperties());
        ...
        if (scroll_event_sent_) {
            SendImpl(CreateGesture(CreateTouchGestureDetails(EventType::kGestureScrollEnd), generic_cancel_event),
                /*should_update=*/false);
        }
        SendImpl(CreateGesture(CreateTouchGestureDetails(EventType::kGestureEnd), generic_cancel_event),
            /*should_update=*/false);
    }

    // ScaleGestureListener implementation.
    bool OnScaleBegin(const ScaleGestureDetector& detector, const MotionEvent& e) override
    {
    ...
    void OnScaleEnd(const ScaleGestureDetector& detector, const MotionEvent& e) override
    {
        if (!pinch_event_sent_)
            return;
        Send(CreateGesture(CreateTouchGestureDetails(EventType::kGesturePinchEnd), e));
    }

    bool OnScale(const ScaleGestureDetector& detector, const MotionEvent& e) override
    {
        if (ignore_multitouch_zoom_events_ && !detector.InAnchoredScaleMode())
            return false;
        ...
        if (!pinch_event_sent_) {
            first_scale = true;
            GestureEventDetails details = CreateTouchGestureDetails(EventType::kGesturePinchBegin);
            Send(CreateGesture(details, e.GetPointerId(), e.GetToolType(), detector.GetEventTime(), detector.GetFocusX(), detector.GetFocusY(),
                detector.GetFocusX() + e.GetRawOffsetX(), detector.GetFocusY() + e.GetRawOffsetY(), e.GetPointerCount(),
                GetBoundingBox(e, EventType::kGesturePinchBegin), e.GetFlags()));
        }

        if (std::abs(detector.GetCurrentSpan() - detector.GetPreviousSpan()) < config_.scale_gesture_detector_config.min_pinch_update_span_delta) {
            return false;
        }
        ...
        if (detector.InAnchoredScaleMode()) {
            ...
            float prev_y = first_scale ? config_.gesture_detector_config.touch_slop * 2 : detector.GetPreviousSpanY();
            float dy = (detector.GetCurrentSpanY() - prev_y) * 0.5f;
            scale = std::pow(scale > 1 ? 1.0f + kDoubleTapDragZoomSpeed : 1.0f - kDoubleTapDragZoomSpeed, std::abs(dy));
        }
        GestureEventDetails pinch_details = CreateTouchGestureDetails(EventType::kGesturePinchUpdate);
        pinch_details.set_scale(scale);
        pinch_details.set_pinch_angle(angle);
        Send(CreateGesture(pinch_details, e.GetPointerId(), e.GetToolType(), detector.GetEventTime(), detector.GetFocusX(), detector.GetFocusY(),
            detector.GetFocusX() + e.GetRawOffsetX(), detector.GetFocusY() + e.GetRawOffsetY(), e.GetPointerCount(), GetBoundingBox(e, pinch_details.type()),
            e.GetFlags()));
        return true;
    }
    ...
    bool OnDown(const MotionEvent& e, int tap_down_count) override
    {
        ...
        GestureEventDetails tap_details = CreateTouchGestureDetails(EventType::kGestureTapDown);
    ...
    bool OnScroll(const MotionEvent& e1, const MotionEvent& e2, const MotionEvent& secondary_pointer_down, float raw_distance_x, float raw_distance_y) override
    {
        ...
        if (!scroll_event_sent_ && e2.GetPointerCount() < 3) {
            ...
            gfx::Vector2dF delta = ComputeFirstScrollDelta(e1, e2, secondary_pointer_down);
            distance_x = delta.x();
            distance_y = delta.y();
        }

        snap_scroll_controller_.UpdateSnapScrollMode(distance_x, distance_y, EffectiveSlopDistance(e2, config_));
        ...
        if (!scroll_event_sent_) {
            ...
            GestureEventDetails scroll_details = CreateTouchGestureDetails(EventType::kGestureScrollBegin, -distance_x, -distance_y);
            ...
            Send(CreateGesture(scroll_details, e2.GetPointerId(), e2.GetToolType(), e2.GetEventTime(), e1.GetX(), e1.GetY(), e1.GetRawX(), e1.GetRawY(),
                e2.GetPointerCount(), GetBoundingBox(e2, scroll_details.type()), e2.GetFlags()));
            DCHECK(scroll_event_sent_);
        }
        scroll_focus_point_.SetPoint(scroll_focus_point_.x() - raw_distance_x, scroll_focus_point_.y() - raw_distance_y);

        GestureEventDetails scroll_details = CreateTouchGestureDetails(EventType::kGestureScrollUpdate, -distance_x, -distance_y);
        const gfx::RectF bounding_box = GetBoundingBox(e2, scroll_details.type());
        const gfx::PointF raw_center = scroll_focus_point_ + gfx::Vector2dF(e2.GetRawOffsetX(), e2.GetRawOffsetY());
        Send(CreateGesture(scroll_details, e2.GetPointerId(), e2.GetToolType(), e2.GetEventTime(), scroll_focus_point_.x(), scroll_focus_point_.y(),
            raw_center.x(), raw_center.y(), e2.GetPointerCount(), bounding_box, e2.GetFlags()));

        return true;
    }

    bool OnFling(const MotionEvent& e1, const MotionEvent& e2, float velocity_x, float velocity_y) override
    {
        if (snap_scroll_controller_.IsSnappingScrolls()) {
            if (snap_scroll_controller_.IsSnapHorizontal()) {
                velocity_y = 0;
        ...
        if (!scroll_event_sent_) {
            ...
            GestureEventDetails scroll_details = CreateTouchGestureDetails(EventType::kGestureScrollBegin, velocity_x, velocity_y);
            Send(CreateGesture(scroll_details, e2));
        }

        GestureEventDetails fling_details = CreateTouchGestureDetails(EventType::kScrollFlingStart, velocity_x, velocity_y);
        Send(CreateGesture(fling_details, e2));
        return true;
    }

    bool OnSwipe(const MotionEvent& e1, const MotionEvent& e2, float velocity_x, float velocity_y) override
    {
        GestureEventDetails swipe_details = CreateTouchGestureDetails(EventType::kGestureSwipe, velocity_x, velocity_y);
    ...
    bool OnTwoFingerTap(const MotionEvent& e1, const MotionEvent& e2) override
    {
        ...
        // primary pointer.
        GestureEventDetails two_finger_tap_details = CreateTouchGestureDetails(EventType::kGestureTwoFingerTap, e1.GetTouchMajor(), e1.GetTouchMajor());
        Send(CreateGesture(two_finger_tap_details, e2.GetPointerId(), e2.GetToolType(), e2.GetEventTime(), e1.GetX(), e1.GetY(), e1.GetRawX(), e1.GetRawY(),
            e2.GetPointerCount(), GetBoundingBox(e2, two_finger_tap_details.type()), e2.GetFlags()));
        return true;
    }

    void OnTapCancel(const MotionEvent& e) override
    {
        GestureEventDetails tap_cancel_details = CreateTouchGestureDetails(EventType::kGestureTapCancel);
        Send(CreateGesture(tap_cancel_details, e));
    }

    void OnShowPress(const MotionEvent& e) override
    {
        GestureEventDetails show_press_details = CreateTouchGestureDetails(EventType::kGestureShowPress);
    ...
    bool OnSingleTapUp(const MotionEvent& e, int tap_count) override
    {
        ...
        if (!ignore_single_tap_) {
            if (e.GetEventTime() - current_down_action_event_time_ > config_.gesture_detector_config.double_tap_timeout) {
        ...
        if (e.GetAction() == MotionEvent::Action::UP && !current_longpress_time_.is_null() && !IsScaleGestureDetectionInProgress()) {
            GestureEventDetails long_tap_details = CreateTouchGestureDetails(EventType::kGestureLongTap);
    ...
    void OnShortPress(const MotionEvent& e) override
    {
        ...
        GestureEventDetails short_press_details = CreateTouchGestureDetails(EventType::kGestureShortPress);
    ...
    void OnLongPress(const MotionEvent& e) override
    {
        DCHECK(!IsDoubleTapInProgress());
        SetIgnoreSingleTap(true);
        GestureEventDetails long_press_details = CreateTouchGestureDetails(EventType::kGestureLongPress);
    ...
    GestureEventDetails CreateTouchGestureDetails(EventType type) const
    {
        ...
        details.set_primary_unique_touch_event_id(current_down_action_unique_touch_event_id_);
        return details;
    }

    GestureEventDetails CreateTouchGestureDetails(EventType type, float delta_x, float delta_y) const
    {
    ...
    GestureEventData CreateGesture(const GestureEventDetails& details, int motion_event_id, MotionEvent::ToolType primary_tool_type, base::TimeTicks time,
        float x, float y, float raw_x, float raw_y, size_t touch_point_count, const gfx::RectF& bounding_box, int flags) const
    {
        return GestureEventData(details, motion_event_id, primary_tool_type, time, x, y, raw_x, raw_y, touch_point_count, bounding_box, flags, 0U);
    }

    GestureEventData CreateGesture(const GestureEventDetails& details, const MotionEvent& event) const
    {
        return GestureEventData(details, event.GetPointerId(), event.GetToolType(), event.GetEventTime(), event.GetX(), event.GetY(), event.GetRawX(),
            event.GetRawY(), event.GetPointerCount(), GetBoundingBox(event, details.type()), event.GetFlags(), event.GetUniqueEventId());
    }

    GestureEventData CreateTapGesture(EventType type, const MotionEvent& event, int tap_count) const
    {
    ...
    gfx::RectF GetBoundingBox(const MotionEvent& event, EventType type) const
    {
        ...
        for (size_t i = 0; i < event.GetPointerCount(); ++i) {
            ...
            if (type == EventType::kGestureShowPress || type == EventType::kGestureTap || type == EventType::kGestureTapUnconfirmed) {
                DCHECK_EQ(0U, i);
    ...
    {
        return gesture_detector_.is_double_tapping() || (IsScaleGestureDetectionInProgress() && InAnchoredScaleMode());
    }

    bool IsScrollInProgress() const
    {
        return scroll_event_sent_;
    }

    bool IsPinchInProgress() const
    {
        return pinch_event_sent_;
    }
...
private:
    ...
    {
        return gesture_detector_.has_doubletap_listener() && client_->RequiresDoubleTapGestureEvents();
    }

    void SetIgnoreSingleTap(bool value)
    {
        ignore_single_tap_ = value;
    }
    ...
    gfx::Vector2dF SubtractSlopRegion(const float dx, const float dy)
    {
        ...
        float epsilon = 1e-3f;
        if (distance > epsilon) {
            float ratio = std::max(0.f, distance - config_.gesture_detector_config.touch_slop) / distance;
    ...
    gfx::Vector2dF ComputeFirstScrollDelta(const MotionEvent& ev1, const MotionEvent& ev2, const MotionEvent& secondary_pointer_down)
    {
        ...
        for (size_t i = 0; i < ev2.GetPointerCount(); i++) {
            ...
            const MotionEvent* source_pointer_down_event = gesture_detector_.GetSourcePointerDownEvent(ev1, &secondary_pointer_down, pointer_id);

            if (!source_pointer_down_event)
                continue;
            int source_index = source_pointer_down_event->FindPointerIndexOfId(pointer_id);
...
GestureProvider::GestureProvider(const Config& config, GestureProviderClient* client)
    : double_tap_support_for_page_(true)
    , double_tap_support_for_platform_(config.double_tap_support_for_platform_enabled)
    , gesture_begin_end_types_enabled_(config.gesture_begin_end_types_enabled)
{
    DCHECK(client);
    DCHECK(!config.min_gesture_bounds_length || !config.max_gesture_bounds_length || config.min_gesture_bounds_length <= config.max_gesture_bounds_length);
    TRACE_EVENT0("input", "GestureProvider::InitGestureDetectors");
    gesture_listener_ = std::make_unique<GestureListenerImpl>(config, client, this);
...
{
    TRACE_EVENT1("input", "GestureProvider::OnTouchEvent", "action", GetMotionEventActionName(event.GetAction()));
...
bool GestureProvider::CanHandle(const MotionEvent& event) const
{
    ...
    // events.
    return current_down_event_ || event.GetAction() == MotionEvent::Action::DOWN || event.GetAction() == MotionEvent::Action::CANCEL;
}
...
void GestureProvider::OnTouchEventHandlingBegin(const MotionEvent& event)
{
    ...
    case MotionEvent::Action::DOWN:
        ...
        if (gesture_begin_end_types_enabled_) {
            ...
            gesture_listener_->Send(gesture_listener_->CreateGesture(details, event));
        }
        break;
    case MotionEvent::Action::POINTER_DOWN:
        if (gesture_begin_end_types_enabled_) {
            ...
            details.set_device_type(GestureDeviceType::DEVICE_TOUCHSCREEN);
            details.set_primary_unique_touch_event_id(event.GetUniqueEventId());
            gesture_listener_->Send(gesture_listener_->CreateGesture(details, event.GetPointerId(), event.GetToolType(), event.GetEventTime(),
                event.GetX(action_index), event.GetY(action_index), event.GetRawX(action_index), event.GetRawY(action_index), event.GetPointerCount(),
                gesture_listener_->GetBoundingBox(event, EventType::kGestureBegin), event.GetFlags()));
        }
        break;
    case MotionEvent::Action::POINTER_UP:
    case MotionEvent::Action::UP:
    ...
    case MotionEvent::Action::CANCEL: {
        if (gesture_begin_end_types_enabled_) {
            ...
            if (current_down_event_) {
                details.set_primary_unique_touch_event_id(current_down_event_->GetUniqueEventId());
            }
            gesture_listener_->Send(gesture_listener_->CreateGesture(details, event));
        }

        if (event.GetAction() != MotionEvent::Action::CANCEL || !GestureConfiguration::GetInstance()->single_pointer_cancel_enabled()
...
void GestureProvider::UpdateDoubleTapDetectionSupport()
{
    ...
    const bool double_tap_enabled = double_tap_support_for_page_ && double_tap_support_for_platform_;
