...
void GestureTouchUMAHistogram::RecordGestureEvent(const GestureEventData& gesture)
{
    UMA_HISTOGRAM_ENUMERATION("Event.GestureCreated", UMAEventTypeFromEvent(gesture), UMA_ET_COUNT);
}

#define RECORD_MAX_DRAG_DISTANCE(HISTOGRAM_NAME, DIST_SQUARED) UMA_HISTOGRAM_CUSTOM_COUNTS(HISTOGRAM_NAME, static_cast<int>(sqrt(DIST_SQUARED)), 1, 1500, 50)
...
void GestureTouchUMAHistogram::RecordTouchEvent(const MotionEvent& event)
{
    ...
    case MotionEvent::Action::MOVE:
        if (is_single_finger_) {
            float cur_dist = (start_touch_position_ - gfx::Point(event.GetX(), event.GetY())).LengthSquared();
    ...
    case MotionEvent::Action::UP:
        if (is_single_finger_) {
            if (tool_type_ == MotionEvent::ToolType::FINGER) {
                RECORD_MAX_DRAG_DISTANCE("Event.MaxDragDistance.FINGER", max_distance_from_start_squared_);
            } else if (tool_type_ == MotionEvent::ToolType::STYLUS) {
                RECORD_MAX_DRAG_DISTANCE("Event.MaxDragDistance.STYLUS", max_distance_from_start_squared_);
            } else if (tool_type_ == MotionEvent::ToolType::ERASER) {
                RECORD_MAX_DRAG_DISTANCE("Event.MaxDragDistance.ERASER", max_distance_from_start_squared_);
            }
...
UMAEventType GestureTouchUMAHistogram::UMAEventTypeFromEvent(const GestureEventData& gesture)
{
    switch (gesture.type()) {
