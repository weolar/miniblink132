...
enum TimeoutEvent { SHOW_PRESS = 0, SHORT_PRESS, LONG_PRESS, TAP, TIMEOUT_EVENT_COUNT };
...
public:
        ...
        : gesture_detector_(gesture_detector)
    {
        ...
        timeout_delays_[SHORT_PRESS] = config.shortpress_timeout + config.showpress_timeout;

        timeout_callbacks_[LONG_PRESS] = &GestureDetector::OnLongPressTimeout;
        timeout_delays_[LONG_PRESS] = config.longpress_timeout + config.showpress_timeout;
    ...
    {
        timeout_timers_[event].Start(FROM_HERE, timeout_delays_[event], gesture_detector_.get(), timeout_callbacks_[event]);
    }
...
GestureDetector::GestureDetector(const Config& config, GestureListener* listener, DoubleTapListener* optional_double_tap_listener)
...
bool GestureDetector::OnTouchEvent(const MotionEvent& ev, bool should_process_double_tap)
{
    ...
    case MotionEvent::Action::POINTER_DOWN: {
        ...
        CancelTaps();
        maximum_pointer_count_ = std::max(maximum_pointer_count_, static_cast<int>(ev.GetPointerCount()));
        ...
        if (maximum_pointer_count_ > 2 || dx * dx + dy * dy >= two_finger_tap_distance_square_)
    ...
    case MotionEvent::Action::POINTER_UP: {
        ...
        if (two_finger_tap_allowed_for_gesture_ && ev.GetPointerCount() == 2 && secondary_pointer_down_event_
            && (ev.GetEventTime() - secondary_pointer_down_event_->GetEventTime() <= two_finger_tap_timeout_)) {
    ...
    case MotionEvent::Action::DOWN: {
        bool is_repeated_tap
            = current_down_event_ && previous_up_event_ && IsRepeatedTap(*current_down_event_, *previous_up_event_, ev, should_process_double_tap);
        ...
        int previous_tap_count = is_down_candidate_for_repeated_single_tap_ ? (1 + current_single_tap_repeat_count_) % single_tap_repeat_interval_ : 0;
    ...
    case MotionEvent::Action::MOVE: {
        ...
        if (is_double_tapping_) {
            ...
            if (!IsWithinSlopForTap(ev)) {
                handled = listener_->OnScroll(*current_down_event_, ev,
                    (maximum_pointer_count_ > 1 && secondary_pointer_down_event_) ? *secondary_pointer_down_event_ : ev, scroll_x, scroll_y);
        ...
        } else if (std::abs(scroll_x) > kScrollEpsilon || std::abs(scroll_y) > kScrollEpsilon) {
            handled = listener_->OnScroll(*current_down_event_, ev,
                (maximum_pointer_count_ > 1 && secondary_pointer_down_event_) ? *secondary_pointer_down_event_ : ev, scroll_x, scroll_y);
            last_focus_x_ = focus_x;
            last_focus_y_ = focus_y;
        ...
        if (ev.GetPointerCount() == 1 && timeout_handler_->HasTimeout(LONG_PRESS)) {
            if (ev.GetToolType(0) == MotionEvent::ToolType::STYLUS && stylus_button_accelerated_longpress_enabled_
            ...
            } else if (ev.GetToolType(0) == MotionEvent::ToolType::FINGER && deep_press_accelerated_longpress_enabled_
                && ev.GetClassification() == MotionEvent::Classification::DEEP_PRESS) {
    ...
    case MotionEvent::Action::UP:
        still_down_ = false;
        {
            if (is_double_tapping_ && should_process_double_tap) {
                ...
                DCHECK(double_tap_listener_);
                handled |= double_tap_listener_->OnDoubleTapEvent(ev);
            } else if (all_pointers_within_slop_regions_ && maximum_pointer_count_ == 1) {
                if (is_down_candidate_for_repeated_single_tap_) {
                    current_single_tap_repeat_count_ = (1 + current_single_tap_repeat_count_) % single_tap_repeat_interval_;
                } else {
                    current_single_tap_repeat_count_ = 0;
                }
                handled = listener_->OnSingleTapUp(ev, 1 + current_single_tap_repeat_count_);
                if (defer_confirm_single_tap_ && should_process_double_tap && double_tap_listener_) {
                ...
                if ((std::abs(velocity_y) > min_fling_velocity_) || (std::abs(velocity_x) > min_fling_velocity_)) {
                    handled = listener_->OnFling(*current_down_event_, ev, velocity_x, velocity_y);
                }
...
void GestureDetector::SetDoubleTapListener(DoubleTapListener* double_tap_listener)
{
...
void GestureDetector::Init(const Config& config)
{
    ...
    const float maximum_swipe_deviation_angle = std::clamp(config.maximum_swipe_deviation_angle, 0.001f, 45.0f);
    min_swipe_direction_component_ratio_ = 1.f / tan(base::DegToRad(maximum_swipe_deviation_angle));

    two_finger_tap_enabled_ = config.two_finger_tap_enabled;
    two_finger_tap_distance_square_ = config.two_finger_tap_max_separation * config.two_finger_tap_max_separation;
    ...
    stylus_button_accelerated_longpress_enabled_ = config.stylus_button_accelerated_longpress_enabled;
    deep_press_accelerated_longpress_enabled_ = config.deep_press_accelerated_longpress_enabled;
}
...
bool GestureDetector::IsRepeatedTap(
    const MotionEvent& first_down, const MotionEvent& first_up, const MotionEvent& second_down, bool should_process_double_tap) const
{
    if (!always_in_bigger_tap_region_)
        return false;

    const base::TimeDelta delta_time = second_down.GetEventTime() - first_up.GetEventTime();
    ...
    if (should_process_double_tap && double_tap_listener_ && delta_time < double_tap_min_time_) {
        return false;
    }
...
bool GestureDetector::HandleSwipeIfNeeded(const MotionEvent& up, float vx, float vy)
{
    ...
    float ratio = vx_abs > vy_abs ? vx_abs / std::max(vy_abs, 0.001f) : vy_abs / std::max(vx_abs, 0.001f);
...
bool GestureDetector::IsWithinSlopForTap(const MotionEvent& ev)
{
    ...
    for (size_t i = 0; i < ev.GetPointerCount(); i++) {
        ...
        const MotionEvent* source_pointer_down_event = GetSourcePointerDownEvent(*current_down_event_.get(), secondary_pointer_down_event_.get(), pointer_id);
        ...
        bool is_stylus_slop_effective = base::FeatureList::IsEnabled(features::kStylusSpecificTapSlop) && ev.GetToolType(i) == MotionEvent::ToolType::STYLUS;
        float slop_square = is_stylus_slop_effective ? stylus_slop_square_ : touch_slop_square_;
...
const MotionEvent* GestureDetector::GetSourcePointerDownEvent(
    const MotionEvent& current_down_event, const MotionEvent* secondary_pointer_down_event, const int pointer_id)
{
