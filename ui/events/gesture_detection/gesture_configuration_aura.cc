...
private:
        ...
        : GestureConfiguration()
    {
        ...
        if (command_line->HasSwitch(switches::kTouchSlopDistance)
            && base::StringToDouble(command_line->GetSwitchValueASCII(switches::kTouchSlopDistance), &touch_slop_distance)) {
        ...
        set_min_pinch_update_span_delta(base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kCompensateForUnstablePinchZoom) ? 5 : 0);
