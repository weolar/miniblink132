...
class TestTarget : public EventTarget, public EventHandler {
public:
    TestTarget()
        : parent_(nullptr)
        , valid_(true)
    ...
    void set_parent(TestTarget* parent)
    {
        parent_ = parent;
    }
    ...
    const std::vector<int>& handler_list() const
    {
...
public:
    explicit TestEventHandler(int id)
        : id_(id)
    {
    }
    ...
    void set_pre_target(EventTarget* pre_target)
    {
        pre_target_ = pre_target;
    }
    ...
    void set_event_result(EventResult result)
    {
        event_result_ = result;
    }

    void set_expect_pre_target(bool expect)
    {
        expect_pre_target_ = expect;
    }
    void set_expect_post_target(bool expect)
    {
        expect_post_target_ = expect;
    }
...
public:
    EventHandlerDestroyDispatcherDelegate(std::unique_ptr<EventDispatcherDelegate> delegate, int id)
        : TestEventHandler(id)
        , dispatcher_delegate_(std::move(delegate))
    {
    }

    EventHandlerDestroyDispatcherDelegate(const EventHandlerDestroyDispatcherDelegate&) = delete;
    EventHandlerDestroyDispatcherDelegate& operator=(const EventHandlerDestroyDispatcherDelegate&) = delete;
...
public:
    explicit InvalidateTargetEventHandler(int id)
        : TestEventHandler(id)
    {
    }

    InvalidateTargetEventHandler(const InvalidateTargetEventHandler&) = delete;
    InvalidateTargetEventHandler& operator=(const InvalidateTargetEventHandler&) = delete;
...
public:
    ...
    void set_dispatcher_delegate(std::unique_ptr<EventDispatcherDelegate> dispatcher_delegate)
    {
...
TEST(EventDispatcherTest, EventDispatchOrder)
{
    ...
    MouseEvent mouse(ui::EventType::kMouseMoved, gfx::Point(3, 4), gfx::Point(3, 4), ui::EventTimeForNow(), 0, 0);
...

    {
        ...
        EXPECT_EQ(std::vector<int>(expected, expected + sizeof(expected) / sizeof(int)), child.handler_list());
    }
    ...
    EXPECT_TRUE(mouse.handled());
    EXPECT_EQ(std::vector<int>(nexpected, nexpected + sizeof(nexpected) / sizeof(int)), child.handler_list());
    ...
    EXPECT_TRUE(mouse.stopped_propagation());
    EXPECT_TRUE(mouse.handled());
    EXPECT_EQ(std::vector<int>(exp, exp + sizeof(exp) / sizeof(int)), child.handler_list());
...
TEST(EventDispatcherTest, EventDispatchPhase)
{
    ...
    int handlers[] = { 11, 11 };
    EXPECT_EQ(std::vector<int>(handlers, handlers + sizeof(handlers) / sizeof(int)), target.handler_list());
...
TEST(EventDispatcherTest, EventDispatcherDestroyedDuringDispatch)
{
    // Test for pre-target first.
    {
        ...
        EventHandlerDestroyDispatcherDelegate handler(std::move(owned_dispatcher), 5);
