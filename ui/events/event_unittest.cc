...
TEST(EventTest, GetCharacter)
{
    ...
    KeyEvent keyev5 = ui::KeyEvent::FromCharacter(0x200C, VKEY_UNKNOWN, ui::DomCode::NONE, EF_CONTROL_DOWN | EF_SHIFT_DOWN);
...
TEST(EventTest, ClickCount)
{
    ...
    MouseEvent mouseev(EventType::kMousePressed, origin, origin, EventTimeForNow(), 0, 0);
    for (int i = 1; i <= 3; ++i) {
...
{
    const gfx::Point origin(0, 0);
    MouseEvent event1(EventType::kMousePressed, origin, origin, EventTimeForNow(), 0, 0);
    MouseEvent event2(EventType::kMousePressed, origin, origin, EventTimeForNow(), 0, 0);
...
TEST(EventTest, NoRepeatedKeyEvent)
{
    ...
    absl::Cleanup scoped_restore_settings = [old_value = KeyEvent::IsSynthesizeKeyRepeatEnabled()] { KeyEvent::SetSynthesizeKeyRepeatEnabled(old_value); };
...
TEST(EventTest, SingleClickRightLeft)
{
    ...
    MouseEvent event(EventType::kMousePressed, point, point, time1, ui::EF_RIGHT_MOUSE_BUTTON, ui::EF_RIGHT_MOUSE_BUTTON);
    EXPECT_EQ(1, MouseEvent::GetRepeatCount(event));
    event = MouseEvent(EventType::kMousePressed, point, point, time2, ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    EXPECT_EQ(1, MouseEvent::GetRepeatCount(event));
    event = MouseEvent(EventType::kMouseReleased, point, point, time2, ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    EXPECT_EQ(1, MouseEvent::GetRepeatCount(event));
    event = MouseEvent(EventType::kMousePressed, point, point, time3, ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    EXPECT_EQ(2, MouseEvent::GetRepeatCount(event));
    MouseEvent::ResetLastClickForTest();
}
...
TEST(EventTest, KeyEvent)
{
    ...
    for (size_t i = 0; i < std::size(kTestData); ++i) {
        KeyEvent key(EventType::kKeyPressed, kTestData[i].key_code, kTestData[i].flags);
        EXPECT_EQ(kTestData[i].character, key.GetCharacter()) << " Index:" << i << " key_code:" << kTestData[i].key_code;
    }
}

TEST(EventTest, KeyEventDirectUnicode)
{
    KeyEvent key = ui::KeyEvent::FromCharacter(0x1234U, ui::VKEY_UNKNOWN, ui::DomCode::NONE, ui::EF_NONE);
...
TEST(EventTest, KeyEventCode)
{
    ...
    ASSERT_EQ(kDomCodeForSpace, ui::KeycodeConverter::CodeStringToDomCode(kCodeForSpace));
    const int kNativeCodeSpace = ui::KeycodeConverter::DomCodeToNativeKeycode(kDomCodeForSpace);
    ASSERT_NE(ui::KeycodeConverter::InvalidNativeKeycode(), kNativeCodeSpace);
    ASSERT_EQ(kNativeCodeSpace, ui::KeycodeConverter::DomCodeToNativeKeycode(kDomCodeForSpace));
    ...
    }
    {
        ...
        // the specified value.
        KeyEvent key(EventType::kKeyPressed, VKEY_RETURN, kDomCodeForSpace, EF_NONE);
        EXPECT_EQ(kCodeForSpace, key.GetCodeString());
    }
    {
...
{
    ui::TouchEvent touch_event_plain(EventType::kTouchPressed, gfx::Point(0, 0), ui::EventTimeForNow(), PointerDetails(ui::EventPointerType::kTouch, 0));

    EXPECT_EQ(EventPointerType::kTouch, touch_event_plain.pointer_details().pointer_type);
    ...
    ui::TouchEvent touch_event_with_details(EventType::kTouchPressed, gfx::Point(0, 0), ui::EventTimeForNow(),
        PointerDetails(ui::EventPointerType::kTouch,
            /* pointer_id*/ 0,
            /* radius_x */ 10.0f,
            /* radius_y */ 5.0f,
    ...
    EXPECT_EQ(EventPointerType::kTouch, touch_event_with_details.pointer_details().pointer_type);
    ...
    EXPECT_EQ(touch_event_with_details.pointer_details(), touch_event_copy.pointer_details());
}

TEST(EventTest, PointerDetailsMouse)
{
    ui::MouseEvent mouse_event(EventType::kMousePressed, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), 0, 0);

    EXPECT_EQ(EventPointerType::kMouse, mouse_event.pointer_details().pointer_type);
...
TEST(EventTest, PointerDetailsStylus)
{
    ...
    ui::MouseEvent stylus_event(EventType::kMousePressed, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), 0, 0, pointer_details);
    EXPECT_EQ(EventPointerType::kPen, stylus_event.pointer_details().pointer_type);
    ...
    EXPECT_EQ(stylus_event.pointer_details(), stylus_event_copy.pointer_details());
}

TEST(EventTest, PointerDetailsCustomTouch)
{
    ui::TouchEvent touch_event(EventType::kTouchPressed, gfx::Point(0, 0), ui::EventTimeForNow(), PointerDetails(ui::EventPointerType::kTouch, 0));

    EXPECT_EQ(EventPointerType::kTouch, touch_event.pointer_details().pointer_type);
...
TEST(EventTest, MouseEventLatencyUIComponentExists)
{
    const gfx::Point origin(0, 0);
    MouseEvent mouseev(EventType::kMousePressed, origin, origin, EventTimeForNow(), 0, 0);
    EXPECT_TRUE(mouseev.latency()->FindLatency(ui::INPUT_EVENT_LATENCY_UI_COMPONENT, nullptr));
}

TEST(EventTest, MouseWheelEventLatencyUIComponentExists)
{
    const gfx::Point origin(0, 0);
    MouseWheelEvent mouseWheelev(gfx::Vector2d(), origin, origin, EventTimeForNow(), 0, 0);
    EXPECT_TRUE(mouseWheelev.latency()->FindLatency(ui::INPUT_EVENT_LATENCY_UI_COMPONENT, nullptr));
}

TEST(EventTest, MouseWheelEventLinearTickCalculation)
{
    const gfx::Point origin;
    MouseWheelEvent mouse_wheel_ev(gfx::Vector2d(-2 * MouseWheelEvent::kWheelDelta, MouseWheelEvent::kWheelDelta), origin, origin, EventTimeForNow(), 0, 0);
...
TEST(EventTest, OrdinalMotionConversion)
{
    ...
    struct OrdinalMotionConversionModel {
        static void ConvertPointToTarget(const OrdinalMotionConversionModel*, const OrdinalMotionConversionModel*, gfx::Point*)
        {
            // Do nothing.
        }
    } src, dst;

    MouseEvent mouseev1(EventType::kMousePressed, origin, origin, EventTimeForNow(), 0, 0);
...
TEST(EventTest, UpdateForRootTransformation)
{
    ...
    ui::MouseEvent untargeted(EventType::kMousePressed, location, root_location, EventTimeForNow(), 0, 0);
    ...
    {
        PointerDetails pointer_details(EventPointerType::kTouch, 0 /* pointer id */, 3, 4, 50, 0 /* twist */, 0, 0);
        ui::TouchEvent targeted(EventType::kTouchPressed, f_location, f_root_location, EventTimeForNow(), pointer_details);
    ...
    // A touch event should scale the same way as others.
    {
        ...
        PointerDetails pointer_details(EventPointerType::kTouch, 0 /* pointer id */, 3, 4, 50, 0 /* twist */, 17.2 /* tilt_x */, -28.3 /* tilt_y */);
        ...
        EXPECT_EQ(updated_pointer_details, targeted.pointer_details())
            << " orig: " << pointer_details.ToString() << " vs " << targeted.pointer_details().ToString();
    }
    ...
    // A touch event should rotate appropriately.
    {
        ...
        gfx::Transform transform3x;
        transform3x.Scale(3, 3);
        PointerDetails pointer_details(EventPointerType::kTouch, 0 /* pointer id */, 3, 4, 50, 0 /* twist */, -17.4 /* tilt_x */, 31.2 /* tilt_y */);
        ...
        EXPECT_EQ(updated_pointer_details, targeted.pointer_details())
            << " orig: " << updated_pointer_details.ToString() << " vs " << targeted.pointer_details().ToString();
    }
    ...
    {
        ui::MouseEvent targeted(EventType::kMousePressed, location, root_location, EventTimeForNow(), 0, 0);
        Event::DispatcherApi(&targeted).set_target(&target);
        targeted.UpdateForRootTransform(identity_transform, identity_transform);
        EXPECT_EQ(location, targeted.location());
...
{
    MouseEvent m1(EventType::kMousePressed, gfx::Point(1, 2), gfx::Point(2, 3), EventTimeForNow(), EF_LEFT_MOUSE_BUTTON, EF_RIGHT_MOUSE_BUTTON);
    base::flat_map<std::string, std::vector<uint8_t>> properties;
    properties["a"] = { 1u };
    m1.SetProperties(properties);
    EXPECT_EQ(properties, *(m1.properties()));
    MouseEvent m2(EventType::kMouseReleased, gfx::Point(11, 21), gfx::Point(2, 2), EventTimeForNow(), EF_RIGHT_MOUSE_BUTTON, EF_LEFT_MOUSE_BUTTON);
    m2 = m1;
...
{
    MouseEvent mouse_event(EventType::kMousePressed, gfx::Point(1, 2), gfx::Point(2, 3), EventTimeForNow(), EF_LEFT_MOUSE_BUTTON, EF_RIGHT_MOUSE_BUTTON);
    EXPECT_FALSE(mouse_event.ToString().empty());

    ScrollEvent scroll_event(EventType::kScroll, gfx::Point(1, 2), EventTimeForNow(), EF_NONE, 1.f, 2.f, 3.f, 4.f, 1);
...
const struct AltGraphEventTestCase {
    ...
    { VKEY_C, KEYBOARD_LAYOUT_ENGLISH_US, { VKEY_RMENU, VKEY_LCONTROL, VKEY_MENU, VKEY_CONTROL }, EF_ALT_DOWN | EF_CONTROL_DOWN },
    { VKEY_E, KEYBOARD_LAYOUT_ENGLISH_US, { VKEY_RMENU, VKEY_LCONTROL, VKEY_MENU, VKEY_CONTROL }, EF_ALT_DOWN | EF_CONTROL_DOWN },

    // French -> Always expect AltGraph if VKEY_RMENU is pressed.
    { VKEY_C, KEYBOARD_LAYOUT_FRENCH, { VKEY_RMENU, VKEY_LCONTROL, VKEY_MENU, VKEY_CONTROL }, EF_ALTGR_DOWN },
    { VKEY_E, KEYBOARD_LAYOUT_FRENCH, { VKEY_RMENU, VKEY_LCONTROL, VKEY_MENU, VKEY_CONTROL }, EF_ALTGR_DOWN },

    // French -> Expect Control+Alt is AltGraph on AltGraph-shifted keys.
    { VKEY_C, KEYBOARD_LAYOUT_FRENCH, { VKEY_LMENU, VKEY_LCONTROL, VKEY_MENU, VKEY_CONTROL }, EF_ALT_DOWN | EF_CONTROL_DOWN },
    { VKEY_E, KEYBOARD_LAYOUT_FRENCH, { VKEY_LMENU, VKEY_LCONTROL, VKEY_MENU, VKEY_CONTROL }, EF_ALTGR_DOWN },
};

class AltGraphEventTest : public testing::TestWithParam<std::tuple<UINT, AltGraphEventTestCase>> {
public:
    AltGraphEventTest()
        : msg_({ nullptr, message_type(), static_cast<WPARAM>(test_case().key_code) })
    {
        ...
        // modifier keys.
        CHECK(ActivateKeyboardLayout(GetPlatformKeyboardLayout(test_case().layout), 0));
...
TEST_P(AltGraphEventTest, KeyEventAltGraphModifer)
{
    ...
    if (message_type() == WM_CHAR) {
        ...
        EXPECT_EQ(event.flags() & (EF_CONTROL_DOWN | EF_ALT_DOWN | EF_ALTGR_DOWN), EF_ALTGR_DOWN);
    } else {
        EXPECT_EQ(event.flags() & (EF_CONTROL_DOWN | EF_ALT_DOWN | EF_ALTGR_DOWN), test_case().expected_flags);
    }
}

INSTANTIATE_TEST_SUITE_P(WM_KEY, AltGraphEventTest, ::testing::Combine(::testing::Values(WM_KEYDOWN, WM_KEYUP), ::testing::ValuesIn(kAltGraphEventTestCases)));
...
public:
    ...
    ~EventLatencyTest() override
    {
        SetEventLatencyTickClockForTesting(nullptr);
    }
...
TEST_F(EventLatencyTest, ComputeEventLatencyOSFromTickCount)
{
    ...
    constexpr DWORD timestamp_msec = std::numeric_limits<DWORD>::max() -
        // Remove any portion that's not kTickInterval aligned.
        (std::numeric_limits<DWORD>::max() % kTickInterval.InMilliseconds()) - 4 * kTickInterval.InMilliseconds();
    ...
    {
        base::HistogramTester histogram_tester;
        ComputeEventLatencyOSFromTOUCHINPUT(EventType::kTouchPressed, touch_input, base::TimeTicks::Now());
        ComputeEventLatencyOSFromPOINTER_INFO(EventType::kTouchPressed, pointer_info, base::TimeTicks::Now());
        histogram_tester.ExpectUniqueTimeSample("Event.Latency.OS2.TOUCH_PRESSED", base::Milliseconds(0), 2);
    }
    ...
    UpdateTickClock(timestamp_msec + 2 * kTickInterval.InMilliseconds());
    {
        ...
        ComputeEventLatencyOSFromTOUCHINPUT(EventType::kTouchPressed, touch_input, base::TimeTicks::Now());
        ComputeEventLatencyOSFromPOINTER_INFO(EventType::kTouchPressed, pointer_info, base::TimeTicks::Now());
        histogram_tester.ExpectUniqueTimeSample("Event.Latency.OS2.TOUCH_PRESSED", base::Milliseconds(16), 2);
    }
    ...
    constexpr DWORD wrapped_timestamp_msec = timestamp_msec + 5 * static_cast<DWORD>(kTickInterval.InMilliseconds());
    static_assert(wrapped_timestamp_msec == 0, "timestamp should have wrapped around");
    UpdateTickClock(wrapped_timestamp_msec);
    {
        base::HistogramTester histogram_tester;
        ComputeEventLatencyOSFromTOUCHINPUT(EventType::kTouchPressed, touch_input, base::TimeTicks::Now());
        ComputeEventLatencyOSFromPOINTER_INFO(EventType::kTouchPressed, pointer_info, base::TimeTicks::Now());
        histogram_tester.ExpectUniqueTimeSample("Event.Latency.OS2.TOUCH_PRESSED", 4 * kTickInterval, 2);
    }
    ...
    constexpr DWORD wrapped_timestamp_msec2 = timestamp_msec + 15 * static_cast<DWORD>(kTickInterval.InMilliseconds());
    static_assert(wrapped_timestamp_msec2 == 10 * kTickInterval.InMilliseconds(), "timestamp should have wrapped around");
    UpdateTickClock(wrapped_timestamp_msec2);
    {
        base::HistogramTester histogram_tester;
        ComputeEventLatencyOSFromTOUCHINPUT(EventType::kTouchPressed, touch_input, base::TimeTicks::Now());
        ComputeEventLatencyOSFromPOINTER_INFO(EventType::kTouchPressed, pointer_info, base::TimeTicks::Now());
        histogram_tester.ExpectUniqueTimeSample("Event.Latency.OS2.TOUCH_PRESSED", 14 * kTickInterval, 2);
    }
    ...
    UpdateTickClock(timestamp_msec - kTickInterval.InMilliseconds());
    {
        ...
        ComputeEventLatencyOSFromTOUCHINPUT(EventType::kTouchPressed, touch_input, base::TimeTicks::Now());
        ComputeEventLatencyOSFromPOINTER_INFO(EventType::kTouchPressed, pointer_info, base::TimeTicks::Now());
        histogram_tester.ExpectUniqueTimeSample("Event.Latency.OS2.TOUCH_PRESSED", base::TimeDelta(), 2);
    }
...
TEST_F(EventLatencyTest, ComputeEventLatencyOSFromPerformanceCounter)
{
    ...
    if (!::QueryPerformanceFrequency(&ticks_per_sec) || ticks_per_sec.QuadPart <= 0 || !base::TimeTicks::IsHighResolution()) {
    ...
    UINT64 current_timestamp = base::TimeTicks::Now().since_origin().InSecondsF() * ticks_per_second;
    ...
    // Event created shortly before now.
    {
        ...
        base::HistogramTester histogram_tester;
        ComputeEventLatencyOSFromPOINTER_INFO(EventType::kTouchPressed, pointer_info, base::TimeTicks::Now());
        histogram_tester.ExpectUniqueTimeSample("Event.Latency.OS2.TOUCH_PRESSED", base::Seconds(1), 1);
    }
    ...
    // false). The delta should be recorded as 0.
    {
        ...
        base::HistogramTester histogram_tester;
        ComputeEventLatencyOSFromPOINTER_INFO(EventType::kTouchPressed, pointer_info, base::TimeTicks::Now());
        histogram_tester.ExpectUniqueTimeSample("Event.Latency.OS2.TOUCH_PRESSED", base::TimeDelta(), 1);
    }
...
{
    KeyEvent key_event = ui::KeyEvent::FromCharacter(0x5A, VKEY_Z, ui::DomCode::NONE, EF_NONE);
