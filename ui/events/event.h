...
public:
    ...
    public:
        ...
        void set_phase(EventPhase phase)
        {
            event_->phase_ = phase;
        }
        ...
        void set_time_stamp(base::TimeTicks time)
        {
            event_->time_stamp_ = time;
        }
    ...
    const PlatformEvent& native_event() const
    {
    ...
    const base::TimeTicks time_stamp() const
    {
        return time_stamp_;
    }
    int flags() const
    ...
    const LatencyInfo* latency() const
    {
        return &latency_;
    }
    void set_latency(const LatencyInfo& latency)
    {
        latency_ = latency;
    }
    ...
    void set_source_device_id(int id)
    {
        source_device_id_ = id;
    }
    ...
    {
        return (flags_ & EF_COMMAND_DOWN) != 0;
    }
    bool IsAltGrDown() const
    {
        return (flags_ & EF_ALTGR_DOWN) != 0;
    }
    bool IsCapsLockOn() const
    {
        return (flags_ & EF_CAPS_LOCK_ON) != 0;
    }

    bool IsSynthesized() const
    {
        return (flags_ & EF_IS_SYNTHESIZED) != 0;
    }

    bool IsCancelModeEvent() const
    {
        return type_ == EventType::kCancelMode;
    }
    ...
    {
        return type_ == EventType::kMousePressed || type_ == EventType::kMouseDragged || type_ == EventType::kMouseReleased || type_ == EventType::kMouseMoved
            || type_ == EventType::kMouseEntered || type_ == EventType::kMouseExited || type_ == EventType::kMousewheel
            || type_ == EventType::kMouseCaptureChanged;
    }

    bool IsTouchEvent() const
    {
        return type_ == EventType::kTouchReleased || type_ == EventType::kTouchPressed || type_ == EventType::kTouchMoved
    ...
    bool IsScrollEvent() const
    {
        ...
        // Gesture or Scroll events.
        return type_ == EventType::kScroll || ((type_ == EventType::kScrollFlingStart || type_ == EventType::kScrollFlingCancel) && !(flags() & EF_FROM_TOUCH));
    }

    bool IsPinchEvent() const
    {
        return type_ == EventType::kGesturePinchBegin || type_ == EventType::kGesturePinchUpdate || type_ == EventType::kGesturePinchEnd;
    }

    bool IsScrollGestureEvent() const
    {
        return type_ == EventType::kGestureScrollBegin || type_ == EventType::kGestureScrollUpdate || type_ == EventType::kGestureScrollEnd;
    }

    bool IsFlingScrollEvent() const
    {
        return type_ == EventType::kScrollFlingCancel || type_ == EventType::kScrollFlingStart;
    }

    bool IsMouseWheelEvent() const
    {
        return type_ == EventType::kMousewheel;
    }

    bool IsLocatedEvent() const
    {
        return IsMouseEvent() || IsScrollEvent() || IsTouchEvent() || IsGestureEvent() || type_ == EventType::kDropTargetEvent;
    }
...
public:
    ...
    const gfx::PointF& location_f() const
    {
        return location_;
    }
    ...
    virtual void UpdateForRootTransform(const gfx::Transform& inverted_root_transform, const gfx::Transform& inverted_local_transform);

    template <class T> void ConvertLocationToTarget(const T* source, const T* target)
    {
...
protected:
    ...
    LocatedEvent(EventType type, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, int flags);
...
public:
    ...
    template <class T>
    MouseEvent(const MouseEvent& model, T* source, T* target, EventType type, int flags)
        : LocatedEvent(model, source, target)
        , changed_button_flags_(model.changed_button_flags_)
        , movement_(model.movement_)
        , pointer_details_(model.pointer_details_)
    ...
    MouseEvent(EventType type, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, int flags, int changed_button_flags,
        const PointerDetails& pointer_details = PointerDetails(EventPointerType::kMouse, kPointerIdMouse));

    // DEPRECATED: Prefer constructor that takes gfx::PointF.
    MouseEvent(EventType type, const gfx::Point& location, const gfx::Point& root_location, base::TimeTicks time_stamp, int flags, int changed_button_flags,
        const PointerDetails& pointer_details = PointerDetails(EventPointerType::kMouse, kPointerIdMouse));
    ...
    class DispatcherApi {
    public:
        explicit DispatcherApi(MouseEvent* event)
            : event_(event)
        {
        }
    ...
    static bool IsRepeatedClickEvent(const MouseEvent& event1, const MouseEvent& event2);
    ...
    int changed_button_flags() const
    {
        return changed_button_flags_;
    }

    // Updates the button that changed.
    void set_changed_button_flags(int flags)
    {
        changed_button_flags_ = flags;
    }

    const gfx::Vector2dF& movement() const
    {
        return movement_;
    }

    const PointerDetails& pointer_details() const
    {
        return pointer_details_;
    }
...
public:
    ...
    MouseWheelEvent(const gfx::Vector2d& offset, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, int flags,
        int changed_button_flags, const std::optional<gfx::Vector2d> tick_120ths = std::nullopt);

    // DEPRECATED: Prefer the constructor that takes gfx::PointF.
    MouseWheelEvent(const gfx::Vector2d& offset, const gfx::Point& location, const gfx::Point& root_location, base::TimeTicks time_stamp, int flags,
    ...
    const gfx::Vector2d& tick_120ths() const
    {
        return tick_120ths_;
    }
...
public:
    ...
    TouchEvent(EventType type, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, const PointerDetails& pointer_details,
        int flags = 0);

    // DEPRECATED: Prefer the constructor that takes gfx::PointF.
    TouchEvent(EventType type, const gfx::Point& location, base::TimeTicks time_stamp, const PointerDetails& pointer_details, int flags = 0);
    ...
    uint32_t unique_event_id() const
    {
        return unique_event_id_;
    }

    void set_may_cause_scrolling(bool causes)
    {
        may_cause_scrolling_ = causes;
    }
    bool may_cause_scrolling() const
    {
        return may_cause_scrolling_;
    }
    ...
    void UpdateForRootTransform(const gfx::Transform& inverted_root_transform, const gfx::Transform& inverted_local_transform) override;
...
public:
    ...
    KeyEvent(EventType type, KeyboardCode key_code, int flags, base::TimeTicks time_stamp = base::TimeTicks());

    // Create a fully defined keystroke event.
    KeyEvent(EventType type, KeyboardCode key_code, DomCode code, int flags, DomKey key, base::TimeTicks time_stamp, bool is_char = false);

    // Create an event with event type.
    KeyEvent(EventType type, KeyboardCode key_code, DomCode code, int flags, base::TimeTicks time_stamp);
    ...
    static ui::KeyEvent FromCharacter(char16_t character, KeyboardCode key_code, DomCode code, int flags, base::TimeTicks time_stamp = base::TimeTicks());
    ...
    // This is only intended to be used externally by classes that are modifying
    // events in an EventRewriter.
    void set_key_code(KeyboardCode key_code)
    {
        key_code_ = key_code;
    }
    ...
    void set_scan_code(uint32_t scan_code)
    {
...
public:
    ...
    ScrollEvent(EventType type, const gfx::PointF& location, const gfx::PointF& root_location, base::TimeTicks time_stamp, int flags, float x_offset,
        float y_offset, float x_offset_ordinal, float y_offset_ordinal, int finger_count, EventMomentumPhase momentum_phase = EventMomentumPhase::NONE,
        ScrollEventPhase phase = ScrollEventPhase::kNone);

    // DEPRECATED: Prefer the constructor that takes gfx::PointF.
    ScrollEvent(EventType type, const gfx::Point& location, base::TimeTicks time_stamp, int flags, float x_offset, float y_offset, float x_offset_ordinal,
        float y_offset_ordinal, int finger_count, EventMomentumPhase momentum_phase = EventMomentumPhase::NONE,
        ScrollEventPhase phase = ScrollEventPhase::kNone);
    ...
    {
        return x_offset_ordinal_;
    }
    float y_offset_ordinal() const
    {
        return y_offset_ordinal_;
    }
    int finger_count() const
    {
        return finger_count_;
    }
    EventMomentumPhase momentum_phase() const
    {
        return momentum_phase_;
    }
    ScrollEventPhase scroll_event_phase() const
    {
        return scroll_event_phase_;
    }
...
public:
    ...
    GestureEvent(float x, float y, int flags, base::TimeTicks time_stamp, const GestureEventDetails& details, uint32_t unique_touch_event_id = 0);
    ...
    const GestureEventDetails& details() const
    {
        return details_;
    }
