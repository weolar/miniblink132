...
MockMotionEvent::MockMotionEvent(Action action, TimeTicks time, float x0, float y0)
    : MotionEventGeneric(action, time, CreatePointer(x0, y0, 0))
{
}

MockMotionEvent::MockMotionEvent(Action action, TimeTicks time, float x0, float y0, float x1, float y1)
    : MotionEventGeneric(action, time, CreatePointer(x0, y0, 0))
{
    PushPointer(x1, y1);
...
MockMotionEvent::MockMotionEvent(Action action, TimeTicks time, float x0, float y0, float x1, float y1, float x2, float y2)
    : MotionEventGeneric(action, time, CreatePointer(x0, y0, 0))
{
    PushPointer(x1, y1);
    PushPointer(x2, y2);
...
MockMotionEvent::MockMotionEvent(Action action, base::TimeTicks time, const std::vector<gfx::PointF>& positions)
{
    set_action(action);
    set_event_time(time);
...
MockMotionEvent& MockMotionEvent::SetRawOffset(float raw_offset_x, float raw_offset_y)
{
    for (size_t i = 0; i < GetPointerCount(); ++i) {
...
MockMotionEvent& MockMotionEvent::SetToolType(size_t pointer_index, ToolType tool_type)
{
...
{
    MotionEventGeneric::PushPointer(CreatePointer(x, y, static_cast<int>(GetPointerCount())));
}
...
std::string ToString(const MotionEvent& event)
{
    ...
    if (event.GetAction() == MotionEvent::Action::POINTER_DOWN || event.GetAction() == MotionEvent::Action::POINTER_UP)
    ...
    for (size_t i = 0; i < pointer_count; ++i) {
        ...
        while (!pointer_ids.is_empty()) {
            ...
            ss << "{"
               ...
               << "\n  Size: (" << event.GetTouchMajor(pi) << ", " << event.GetTouchMinor(pi) << ")"
            ...
            if (history_size) {
                ...
                for (size_t h = 0; h < history_size; ++h) {
                    ss << "\n   { " << event.GetHistoricalX(pi, h) << ", " << event.GetHistoricalY(pi, h) << ", " << event.GetHistoricalTouchMajor(pi, h)
