...
// static
XEventWaiter* XEventWaiter::Create(x11::Window window, base::OnceClosure callback)
{
    ...
    x11::Connection::Get()->SendEvent(marker_event, window, x11::EventMask::NoEvent);
...
void XEventWaiter::OnEvent(const x11::Event& xev)
{
    ...
    if (client && client->type == MarkerEventAtom()) {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, std::move(success_callback_));
        delete this;
    }
}
