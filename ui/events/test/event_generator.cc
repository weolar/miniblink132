...
ui::TouchEvent CreateTestTouchEvent(ui::EventType type, const gfx::Point& root_location, int touch_id, int flags, base::TimeTicks timestamp)
{
...
EventGenerator::EventGenerator(gfx::NativeWindow root_window, const gfx::Point& point)
    : current_screen_location_(point)
{
    Init(root_window, gfx::NativeWindow());
}

EventGenerator::EventGenerator(gfx::NativeWindow root_window, gfx::NativeWindow target_window)
{
...
void EventGenerator::PressButton(int flag)
{
    if (!(flags_ & flag)) {
        ...
        ui::MouseEvent mouseev(ui::EventType::kMousePressed, location, location, ui::EventTimeForNow(), flags_, flag);
...
void EventGenerator::ReleaseButton(int flag)
{
    if (flags_ & flag) {
        gfx::Point location = GetLocationInCurrentRoot();
        ui::MouseEvent mouseev(ui::EventType::kMouseReleased, location, location, ui::EventTimeForNow(), flags_, flag);
        mouseev.set_source_device_id(mouse_source_device_id_);
        Dispatch(&mouseev);
        flags_ ^= flag;
    }
...
{
    gfx::Point location = GetLocationInCurrentRoot();
    ui::MouseWheelEvent wheelev(gfx::Vector2d(delta_x, delta_y), location, location, ui::EventTimeForNow(), flags_, 0);
...
void EventGenerator::SendMouseEnter()
{
    ...
    ui::MouseEvent mouseev(ui::EventType::kMouseEntered, enter_location, enter_location, ui::EventTimeForNow(), flags_, 0);
    mouseev.set_source_device_id(mouse_source_device_id_);
    Dispatch(&mouseev);
}
...
void EventGenerator::SendMouseExit()
{
    ...
    ui::MouseEvent mouseev(ui::EventType::kMouseExited, exit_location, exit_location, ui::EventTimeForNow(), flags_, 0);
    mouseev.set_source_device_id(mouse_source_device_id_);
    Dispatch(&mouseev);
}

#if BUILDFLAG(IS_CHROMEOS)
void EventGenerator::MoveMouseToWithNative(const gfx::Point& point_in_host, const gfx::Point& point_for_native)
{
    ...
    std::unique_ptr<ui::MouseEvent> native_event(
        new ui::MouseEvent(ui::EventType::kMouseMoved, point_in_host, point_in_host, ui::EventTimeForNow(), flags_, 0));
...
{
    const ui::EventType event_type = (flags_ & ui::EF_LEFT_MOUSE_BUTTON) ? ui::EventType::kMouseDragged : ui::EventType::kMouseMoved;
    ui::MouseEvent mouseev(event_type, point_in_host, point_in_host, ui::EventTimeForNow(), flags_, 0);
    mouseev.set_source_device_id(mouse_source_device_id_);
    Dispatch(&mouseev);
...
void EventGenerator::MoveMouseTo(const gfx::Point& point_in_screen, int count)
{
    ...
    for (float i = 1; i <= count; i++) {
        ...
        step.Scale(i / count);
        gfx::Point move_point = current_screen_location_ + gfx::ToRoundedVector2d(step);
        if (!grab_)
            UpdateCurrentDispatcher(move_point);
        delegate()->ConvertPointToTarget(current_target_, &move_point);
        ui::MouseEvent mouseev(event_type, move_point, move_point, ui::EventTimeForNow(), flags_, 0);
        mouseev.set_source_device_id(mouse_source_device_id_);
        Dispatch(&mouseev);
    }
...
void EventGenerator::MoveMouseRelativeTo(const EventTarget* window, const gfx::Point& point_in_parent)
{
...
void EventGenerator::PressTouch(const std::optional<gfx::Point>& touch_location_in_screen)
{
    PressTouchId(0, touch_location_in_screen);
}

void EventGenerator::PressTouchId(int touch_id, const std::optional<gfx::Point>& touch_location_in_screen)
{
    if (touch_location_in_screen.has_value())
        SetCurrentScreenLocation(*touch_location_in_screen);
    ui::TouchEvent touchev = CreateTestTouchEvent(ui::EventType::kTouchPressed, GetLocationInCurrentRoot(), touch_id, flags_, ui::EventTimeForNow());
    Dispatch(&touchev);
}
...
void EventGenerator::MoveTouchId(const gfx::Point& point, int touch_id)
{
    ...
    ui::TouchEvent touchev = CreateTestTouchEvent(ui::EventType::kTouchMoved, GetLocationInCurrentRoot(), touch_id, flags_, ui::EventTimeForNow());
    Dispatch(&touchev);
...
{
    ui::TouchEvent touchev = CreateTestTouchEvent(ui::EventType::kTouchReleased, GetLocationInCurrentRoot(), touch_id, flags_, ui::EventTimeForNow());
    Dispatch(&touchev);
}
...
{
    ui::TouchEvent touchev = CreateTestTouchEvent(ui::EventType::kTouchCancelled, GetLocationInCurrentRoot(), touch_id, flags_, ui::EventTimeForNow());
    Dispatch(&touchev);
}
...
void EventGenerator::GestureTapAt(const gfx::Point& location)
{
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, converted_location, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    Dispatch(&press);

    ui::TouchEvent release(ui::EventType::kTouchReleased, converted_location, press.time_stamp() + base::Milliseconds(50),
...
void EventGenerator::GestureTapDownAndUp(const gfx::Point& location)
{
    ...
    ui::TouchEvent release(ui::EventType::kTouchReleased, converted_location, press.time_stamp() + base::Milliseconds(1000),
        ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    Dispatch(&release);
}

base::TimeDelta EventGenerator::CalculateScrollDurationForFlingVelocity(const gfx::Point& start, const gfx::Point& end, float velocity, int steps)
{
...
void EventGenerator::GestureScrollSequence(const gfx::Point& start, const gfx::Point& end, const base::TimeDelta& step_delay, int steps)
{
    GestureScrollSequenceWithCallback(start, end, step_delay, steps, base::BindRepeating(&DummyCallback));
}

void EventGenerator::GestureScrollSequenceWithCallback(
    const gfx::Point& start, const gfx::Point& end, const base::TimeDelta& step_delay, int steps, const ScrollStepCallback& callback)
{
...
void EventGenerator::GestureMultiFingerScrollWithDelays(int count, const gfx::Point start[], const gfx::Vector2d delta[], const int delay_adding_finger_ms[],
    const int delay_releasing_finger_ms[], int event_separation_time_ms, int steps)
{
    ...
    for (int i = 0; i < count; ++i) {
        pressed[i] = false;
        press_time[i] = press_time_first + base::Milliseconds(delay_adding_finger_ms[i]);
        release_time[i] = press_time_first + base::Milliseconds(delay_releasing_finger_ms[i]);
        DCHECK_LE(press_time[i], release_time[i]);
    }

    for (int step = 0; step < steps; ++step) {
        base::TimeTicks move_time = press_time_first + base::Milliseconds(event_separation_time_ms * step);

        for (int i = 0; i < count; ++i) {
            if (!pressed[i] && move_time >= press_time[i]) {
                ui::TouchEvent press(ui::EventType::kTouchPressed, points[i], press_time[i], ui::PointerDetails(ui::EventPointerType::kTouch, i));
                Dispatch(&press);
                pressed[i] = true;
            }
        }
        ...
        for (int i = 0; i < count && event_separation_time_ms > 0; ++i) {
            if (pressed[i] && move_time >= release_time[i]) {
                ui::TouchEvent release(ui::EventType::kTouchReleased, points[i], release_time[i], ui::PointerDetails(ui::EventPointerType::kTouch, i));
                Dispatch(&release);
                pressed[i] = false;
            }
        }
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (pressed[i]) {
                ui::TouchEvent move(ui::EventType::kTouchMoved, points[i], move_time, ui::PointerDetails(ui::EventPointerType::kTouch, i));
                Dispatch(&move);
            }
        }
    }

    base::TimeTicks default_release_time = press_time_first + base::Milliseconds(event_separation_time_ms * steps);
    // Ensures that all pressed fingers are released in the end.
    for (int i = 0; i < count; ++i) {
        if (pressed[i]) {
            ui::TouchEvent release(ui::EventType::kTouchReleased, points[i], default_release_time, ui::PointerDetails(ui::EventPointerType::kTouch, i));
            Dispatch(&release);
            pressed[i] = false;
        }
    }
    ...
    int count, const gfx::Point start[], const int delay_adding_finger_ms[], int event_separation_time_ms, int steps, int move_x, int move_y)
{
    ...
    GestureMultiFingerScrollWithDelays(count, start, delta, delay_adding_finger_ms, delay_releasing_finger_ms, event_separation_time_ms, steps);
}

void EventGenerator::GestureMultiFingerScroll(int count, const gfx::Point start[], int event_separation_time_ms, int steps, int move_x, int move_y)
{
    const int kMaxTouchPoints = 10;
    int delays[kMaxTouchPoints] = { 0 };
    GestureMultiFingerScrollWithDelays(count, start, delays, event_separation_time_ms, steps, move_x, move_y);
}

void EventGenerator::ScrollSequence(
    const gfx::Point& start, const base::TimeDelta& step_delay, float x_offset, float y_offset, int steps, int num_fingers, ScrollSequenceType end_state)
{
    UpdateCurrentDispatcher(start);

    base::TimeTicks timestamp = ui::EventTimeForNow();
    ui::ScrollEvent fling_cancel(ui::EventType::kScrollFlingCancel, start, timestamp, 0, 0, 0, 0, 0, num_fingers);
    ...
    for (int i = 0; i < steps; ++i) {
        timestamp += step_delay;
        ui::ScrollEvent move(ui::EventType::kScroll, start, timestamp, 0, dx, dy, dx, dy, num_fingers);
        Dispatch(&move);
    }
    ...
    ui::ScrollEvent fling_start(ui::EventType::kScrollFlingStart, start, timestamp, 0, x_offset, y_offset, x_offset, y_offset, num_fingers);
...
{
    int num_fingers = 2;
    ui::ScrollEvent scroll(ui::EventType::kScroll, current_screen_location_, ui::EventTimeForNow(), 0, 0, 0, 0, 0, num_fingers, EventMomentumPhase::MAY_BEGIN);
    Dispatch(&scroll);
}
...
{
    int num_fingers = 2;
    ui::ScrollEvent scroll(ui::EventType::kScroll, current_screen_location_, ui::EventTimeForNow(), 0, 0, 0, 0, 0, num_fingers, EventMomentumPhase::END);
    Dispatch(&scroll);
}

void EventGenerator::PressKey(ui::KeyboardCode key_code, int flags, int source_device_id)
{
    DispatchKeyEvent(true, key_code, flags, source_device_id);
}

void EventGenerator::ReleaseKey(ui::KeyboardCode key_code, int flags, int source_device_id)
{
    DispatchKeyEvent(false, key_code, flags, source_device_id);
}

void EventGenerator::PressAndReleaseKey(KeyboardCode key_code, int flags, int source_device_id)
{
...
void EventGenerator::PressKeyAndModifierKeys(KeyboardCode key_code, int flags, int source_device_id)
{
    PressModifierKeys(flags, source_device_id);
    PressKey(key_code, flags, source_device_id);
}

void EventGenerator::ReleaseKeyAndModifierKeys(KeyboardCode key_code, int flags, int source_device_id)
{
    ReleaseKey(key_code, flags, source_device_id);
    ReleaseModifierKeys(flags, source_device_id);
}

void EventGenerator::PressAndReleaseKeyAndModifierKeys(KeyboardCode key_code, int flags, int source_device_id)
{
...
void EventGenerator::Init(gfx::NativeWindow root_window, gfx::NativeWindow target_window)
{
    CHECK(g_event_generator_allowed) << "EventGenerator is not allowed in this test suite. Please use "
    ...
    if (!delegate_) {
        ...
        delegate_ = g_event_generator_delegate_factory.Run(this, root_window, target_window);
    }
    if (target_window)
        SetCurrentScreenLocation(delegate()->CenterOfWindow(target_window));
    else if (root_window)
...
void EventGenerator::DispatchKeyEvent(bool is_press, ui::KeyboardCode key_code, int flags, int source_device_id)
{
#if BUILDFLAG(IS_WIN)
    UINT key_press = WM_KEYDOWN;
    uint16_t character = ui::DomCodeToUsLayoutCharacter(ui::UsLayoutKeyboardCodeToDomCode(key_code), flags);
    if (is_press && character) {
        CHROME_MSG native_event = { NULL, WM_KEYDOWN, static_cast<UINT>(key_code), 0 };
        native_event.time = (ui::EventTimeForNow() - base::TimeTicks()).InMilliseconds() & UINT32_MAX;
    ...
    CHROME_MSG native_event = { NULL, (is_press ? key_press : WM_KEYUP), static_cast<UINT>(key_code), 0 };
    native_event.time = (ui::EventTimeForNow() - base::TimeTicks()).InMilliseconds() & UINT32_MAX;
    ui::KeyEvent keyev(native_event, flags);
#else
    ui::EventType type = is_press ? ui::EventType::kKeyPressed : ui::EventType::kKeyReleased;
...
void EmulateFullKeyPressReleaseSequence(test::EventGenerator* generator, KeyboardCode key, bool control, bool shift, bool alt, bool command)
{
