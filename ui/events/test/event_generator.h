...
public:
    ...
    using FactoryFunction = base::RepeatingCallback<std::unique_ptr<EventGeneratorDelegate>(
        EventGenerator* owner, gfx::NativeWindow root_window, gfx::NativeWindow target_window)>;
...
public:
    ...
    void set_mouse_source_device_id(int source_device_id = ui::ED_UNKNOWN_DEVICE)
    {
    ...
    void SetTouchForce(float force)
    {
        touch_pointer_details_.force = force;
    }
    ...
    void PressTouch(const std::optional<gfx::Point>& touch_location_in_screen = std::nullopt);
    ...
    void PressTouchId(int touch_id, const std::optional<gfx::Point>& touch_location_in_screen = std::nullopt);
    ...
    void GestureScrollSequenceWithCallback(
        const gfx::Point& start, const gfx::Point& end, const base::TimeDelta& duration, int steps, const ScrollStepCallback& callback);
    ...
    void PressKeyAndModifierKeys(KeyboardCode key, int flags, int source_device_id = ED_UNKNOWN_DEVICE);
    ...
    // the given `key`.
    void ReleaseKeyAndModifierKeys(KeyboardCode key, int flags, int source_device_id = ED_UNKNOWN_DEVICE);

    // Convenient alias to call PressKeyAndModifierKeys() followed by
    // ReleaseKeyAndModifierKeys() to emulate user key event sequence.
    void PressAndReleaseKeyAndModifierKeys(KeyboardCode key, int flags, int source_device_id = ED_UNKNOWN_DEVICE);
