...
NSEvent* MouseEventAtPoint(NSPoint point, NSEventType type, NSUInteger modifiers);
NSEvent* MouseEventWithType(NSEventType type, NSUInteger modifiers);
NSEvent* MouseEventAtPointInWindow(NSPoint point, NSEventType type, NSWindow* window, NSUInteger clickCount);
...
NSEvent* TestScrollEvent(
    NSPoint location, NSWindow* window, CGFloat delta_x, CGFloat delta_y, bool has_precise_deltas, NSEventPhase event_phase, NSEventPhase momentum_phase);
...
NSEvent* KeyEventWithKeyCode(unsigned short key_code, unichar c, NSEventType event_type, NSUInteger modifiers);
...
NSEvent* KeyEventWithModifierOnly(unsigned short key_code, NSUInteger modifiers);
...
NSEvent* SynthesizeKeyEvent(NSWindow* window, bool keyDown, ui::KeyboardCode keycode, NSUInteger flags, ui::DomKey dom_key = ui::DomKey::NONE);
