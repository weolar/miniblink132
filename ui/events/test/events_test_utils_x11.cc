...
x11::KeyButMask XEventState(int flags)
{
    ...
    return ((flags & ui::EF_SHIFT_DOWN) ? x11::KeyButMask::Shift : kNoMask) | ((flags & ui::EF_CAPS_LOCK_ON) ? x11::KeyButMask::Lock : kNoMask)
        | ((flags & ui::EF_CONTROL_DOWN) ? x11::KeyButMask::Control : kNoMask) | ((flags & ui::EF_ALT_DOWN) ? x11::KeyButMask::Mod1 : kNoMask)
        | ((flags & ui::EF_NUM_LOCK_ON) ? x11::KeyButMask::Mod2 : kNoMask) | ((flags & ui::EF_MOD3_DOWN) ? x11::KeyButMask::Mod3 : kNoMask)
        | ((flags & ui::EF_COMMAND_DOWN) ? x11::KeyButMask::Mod4 : kNoMask) | ((flags & ui::EF_ALTGR_DOWN) ? x11::KeyButMask::Mod5 : kNoMask)
        | ((flags & ui::EF_LEFT_MOUSE_BUTTON) ? x11::KeyButMask::Button1 : kNoMask)
        | ((flags & ui::EF_MIDDLE_MOUSE_BUTTON) ? x11::KeyButMask::Button2 : kNoMask)
        | ((flags & ui::EF_RIGHT_MOUSE_BUTTON) ? x11::KeyButMask::Button3 : kNoMask);
}
...
template <typename T> x11::Input::Fp1616 ToFp1616(T x)
{
    return static_cast<x11::Input::Fp1616>(x * (1 << 16));
}

x11::Event CreateXInput2Event(int deviceid, int evtype, int tracking_id, const gfx::Point& location)
{
    ...
    event.event_x = ToFp1616(location.x()), event.event_y = ToFp1616(location.y()), event.event = x11::Connection::Get()->default_root();
...
void ScopedXI2Event::InitKeyEvent(EventType type, KeyboardCode key_code, int flags)
{
    x11::KeyEvent key_event {
        .opcode = XKeyEventType(type),
        .detail = static_cast<x11::KeyCode>(XKeyCodeForWindowsKeyCode(key_code, flags, x11::Connection::Get())),
...
void ScopedXI2Event::InitMotionEvent(const gfx::Point& location, const gfx::Point& root_location, int flags)
{
...
void ScopedXI2Event::InitButtonEvent(EventType type, const gfx::Point& location, int flags)
{
    x11::ButtonEvent button_event {
        .opcode = type == ui::EventType::kMousePressed ? x11::ButtonEvent::Press : x11::ButtonEvent::Release,
...
void ScopedXI2Event::InitGenericKeyEvent(int deviceid, int sourceid, EventType type, KeyboardCode key_code, int flags)
{
    ...
    dev_event->detail = XKeyCodeForWindowsKeyCode(key_code, flags, x11::Connection::Get());
    dev_event->sourceid = static_cast<x11::Input::DeviceId>(sourceid);
}

void ScopedXI2Event::InitGenericButtonEvent(int deviceid, EventType type, const gfx::Point& location, int flags)
{
    event_ = CreateXInput2Event(deviceid, XIButtonEventType(type), 0, gfx::Point());

    auto* dev_event = event_.As<x11::Input::DeviceEvent>();
    dev_event->mods.effective = static_cast<uint32_t>(XEventState(flags));
    dev_event->detail = XButtonEventButton(type, flags);
    dev_event->event_x = ToFp1616(location.x()), dev_event->event_y = ToFp1616(location.y()),
...
void ScopedXI2Event::InitGenericMouseWheelEvent(int deviceid, int wheel_delta, int flags)
{
    InitGenericButtonEvent(deviceid, ui::EventType::kMousewheel, gfx::Point(), flags);
    event_.As<x11::Input::DeviceEvent>()->detail = wheel_delta > 0 ? 4 : 5;
}

void ScopedXI2Event::InitScrollEvent(int deviceid, int x_offset, int y_offset, int x_offset_ordinal, int y_offset_ordinal, int finger_count)
{
    event_ = CreateXInput2Event(deviceid, x11::Input::DeviceEvent::Motion, 0, gfx::Point());

    Valuator valuators[] = { Valuator(DeviceDataManagerX11::DT_CMT_SCROLL_X, x_offset), Valuator(DeviceDataManagerX11::DT_CMT_SCROLL_Y, y_offset),
        Valuator(DeviceDataManagerX11::DT_CMT_ORDINAL_X, x_offset_ordinal), Valuator(DeviceDataManagerX11::DT_CMT_ORDINAL_Y, y_offset_ordinal),
        Valuator(DeviceDataManagerX11::DT_CMT_FINGER_COUNT, finger_count) };
    SetUpValuators(std::vector<Valuator>(valuators, valuators + std::size(valuators)));
}

void ScopedXI2Event::InitFlingScrollEvent(int deviceid, int x_velocity, int y_velocity, int x_velocity_ordinal, int y_velocity_ordinal, bool is_cancel)
{
    event_ = CreateXInput2Event(deviceid, x11::Input::DeviceEvent::Motion, deviceid, gfx::Point());

    Valuator valuators[] = { Valuator(DeviceDataManagerX11::DT_CMT_FLING_STATE, is_cancel ? 1 : 0), Valuator(DeviceDataManagerX11::DT_CMT_FLING_Y, y_velocity),
        Valuator(DeviceDataManagerX11::DT_CMT_ORDINAL_Y, y_velocity_ordinal), Valuator(DeviceDataManagerX11::DT_CMT_FLING_X, x_velocity),
        Valuator(DeviceDataManagerX11::DT_CMT_ORDINAL_X, x_velocity_ordinal) };

    SetUpValuators(std::vector<Valuator>(valuators, valuators + std::size(valuators)));
}

void ScopedXI2Event::InitTouchEvent(int deviceid, int evtype, int tracking_id, const gfx::Point& location, const std::vector<Valuator>& valuators)
{
    ...
    for (size_t i = 0; i < valuators.size(); ++i) {
        if (valuators[i].data_type == DeviceDataManagerX11::DT_TOUCH_RAW_TIMESTAMP) {
    ...
    valuators_with_time.emplace_back(DeviceDataManagerX11::DT_TOUCH_RAW_TIMESTAMP, (ui::EventTimeForNow() - base::TimeTicks()).InMicroseconds());
...
void SetUpTouchPadForTest(int deviceid)
{
    ...
    ui::DeviceDataManagerX11* manager = ui::DeviceDataManagerX11::GetInstance();
    manager->SetDeviceListForTest(std::vector<int>(), device_list, std::vector<int>());
}
...
void SetUpTouchDevicesForTest(const std::vector<int>& devices)
{
    ...
    ui::DeviceDataManagerX11* manager = ui::DeviceDataManagerX11::GetInstance();
    manager->SetDeviceListForTest(devices, std::vector<int>(), std::vector<int>());
}
...
void SetUpPointerDevicesForTest(const std::vector<int>& devices)
{
    ...
    ui::DeviceDataManagerX11* manager = ui::DeviceDataManagerX11::GetInstance();
    manager->SetDeviceListForTest(std::vector<int>(), std::vector<int>(), devices);
}
