...
bool IsLocatedEventWithDifferentLocations(const Event& event)
{
    ...
    return located_event->target() && located_event->location_f() != located_event->root_location_f();
}

} // namespace

class EventSource::EventRewriterContinuationImpl : public EventRewriterContinuation {
public:
    ...
    static void Create(EventSource* const source, EventRewriter* rewriter)
    {
        ...
        source->rewriter_list_.push_back(std::make_unique<EventRewriterContinuationImpl>(source, rewriter));
    ...
    EventRewriterContinuationImpl(EventSource* const source, EventRewriter* rewriter)
        : source_(source)
        , rewriter_(rewriter)
    ...
    EventRewriterContinuationImpl& operator=(const EventRewriterContinuationImpl&) = delete;

    ~EventRewriterContinuationImpl() override
    {
    }

    EventRewriter* rewriter() const
    {
        return rewriter_;
    }
...
EventDispatchDetails EventSource::SendEventToSinkFromRewriter(const Event* event, const EventRewriter* rewriter)
{
    ...
    if (IsLocatedEventWithDifferentLocations(*event) && !next->rewriter()->SupportsNonRootLocation()) {
        event_clone = event->Clone();
        event_clone->AsLocatedEvent()->set_location_f(event_clone->AsLocatedEvent()->root_location_f());
        event_for_rewriting = event_clone.get();
    }
    return next->rewriter()->RewriteEvent(*event_for_rewriting, next->GetWeakPtr());
}

EventSource::EventRewriterList::iterator EventSource::FindContinuation(const EventRewriter* rewriter)
{
    auto it = find_if(rewriter_list_.begin(), rewriter_list_.end(),
        [rewriter](const std::unique_ptr<EventRewriterContinuationImpl>& p) -> bool { return p->rewriter() == rewriter; });
    return it;
}
