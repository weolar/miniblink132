...
base::TimeTicks EventLatencyTimeFromNative(const CHROME_MSG& native_event, base::TimeTicks current_time)
{
...
PointerDetails GetMousePointerDetailsFromNative(const CHROME_MSG& native_event)
{
...
{
    return !(event.hwnd == 0 && event.message == 0 && event.wParam == 0 && event.lParam == 0 && event.time == 0 && event.pt.x == 0 && event.pt.y == 0);
}

PointerDetails GetTouchPointerDetailsFromNative(const CHROME_MSG& native_event)
{
    NOTIMPLEMENTED();
...
bool GetScrollOffsets(const CHROME_MSG& native_event, float* x_offset, float* y_offset, float* x_offset_ordinal, float* y_offset_ordinal, int* finger_count,
    EventMomentumPhase* momentum_phase)
{
    return GetScrollOffsetsFromMSG(native_event);
}

bool GetFlingData(const CHROME_MSG& native_event, float* vx, float* vy, float* vx_ordinal, float* vy_ordinal, bool* is_cancel)
{
