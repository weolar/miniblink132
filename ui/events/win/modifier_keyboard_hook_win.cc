...
public:
    ModifierKeyboardHookWinImpl(std::optional<base::flat_set<DomCode>> dom_codes, KeyEventCallback callback, bool enable_hook_registration);

    ModifierKeyboardHookWinImpl(const ModifierKeyboardHookWinImpl&) = delete;
    ModifierKeyboardHookWinImpl& operator=(const ModifierKeyboardHookWinImpl&) = delete;
...
ModifierKeyboardHookWinImpl::ModifierKeyboardHookWinImpl(
    std::optional<base::flat_set<DomCode>> dom_codes, KeyEventCallback callback, bool enable_hook_registration)
    : KeyboardHookWinBase(std::move(dom_codes), std::move(callback), enable_hook_registration)
{
}
...
bool ModifierKeyboardHookWinImpl::Register()
{
    ...
    return KeyboardHookWinBase::Register(reinterpret_cast<HOOKPROC>(&ModifierKeyboardHookWinImpl::ProcessKeyEvent));
}
...
bool ModifierKeyboardHookWinImpl::ProcessKeyEventMessage(WPARAM w_param, DWORD vk, DWORD scan_code, DWORD time_stamp)
{
    ...
    DWORD non_located_vk = IsWindowsKey(vk) ? vk : LocatedToNonLocatedKeyboardCode(static_cast<KeyboardCode>(vk));

    bool is_repeat = false;
    CHROME_MSG msg = { nullptr, static_cast<UINT>(w_param), non_located_vk, GetLParamFromScanCode(scan_code), time_stamp };
...
void ModifierKeyboardHookWinImpl::UpdateModifierState(DWORD vk, bool is_key_down)
{
    BYTE keyboard_state[kKeyboardStateArraySize] = { 0 };
    if (!GetKeyboardState(keyboard_state)) {
        PLOG(ERROR) << "GetKeyboardState() failed: ";
    ...
    keyboard_state[VK_CONTROL] = (keyboard_state[VK_LCONTROL] == kKeyDown || keyboard_state[VK_RCONTROL] == kKeyDown) ? kKeyDown : kKeyUp;
    keyboard_state[VK_MENU] = (keyboard_state[VK_LMENU] == kKeyDown || keyboard_state[VK_RMENU] == kKeyDown) ? kKeyDown : kKeyUp;
...
// static
LRESULT CALLBACK ModifierKeyboardHookWinImpl::ProcessKeyEvent(int code, WPARAM w_param, LPARAM l_param)
{
...
{
    std::unique_ptr<ModifierKeyboardHookWinImpl> keyboard_hook = std::make_unique<ModifierKeyboardHookWinImpl>(std::move(dom_codes), std::move(callback),
...
// static
std::unique_ptr<KeyboardHookWinBase> KeyboardHookWinBase::CreateModifierKeyboardHookForTesting(
    std::optional<base::flat_set<DomCode>> dom_codes, KeyEventCallback callback)
{
    return std::make_unique<ModifierKeyboardHookWinImpl>(std::move(dom_codes), std::move(callback),
