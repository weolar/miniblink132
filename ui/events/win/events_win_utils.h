...
// calibration.
EVENTS_EXPORT base::TimeTicks EventLatencyTimeFromTickClock(DWORD event_time, base::TimeTicks current_time);
...
// false.
EVENTS_EXPORT base::TimeTicks EventLatencyTimeFromPerformanceCounter(UINT64 event_time);
...
EVENTS_EXPORT gfx::Point EventSystemLocationFromMSG(const CHROME_MSG& native_event);
...
EVENTS_EXPORT int GetChangedMouseButtonFlagsFromMSG(const CHROME_MSG& native_event);

// Returns the detailed pointer information for mouse events.
EVENTS_EXPORT PointerDetails GetMousePointerDetailsFromMSG(const CHROME_MSG& native_event);

// Gets the mouse wheel offsets from a native event.
EVENTS_EXPORT gfx::Vector2d GetMouseWheelOffsetFromMSG(const CHROME_MSG& native_event);

// Returns the detailed pointer information for touch events.
EVENTS_EXPORT PointerDetails GetTouchPointerDetailsFromMSG(const CHROME_MSG& native_event);
...
EVENTS_EXPORT bool GetFlingData(const CHROME_MSG& native_event, float* vx, float* vy, float* vx_ordinal, float* vy_ordinal, bool* is_cancel);
...
EVENTS_EXPORT void SetEventLatencyTickClockForTesting(const base::TickClock* clock);
