...
{
    return TouchEvent { EventType::kTouchPressed, gfx::PointF(), gfx::PointF(), base::TimeTicks(), PointerDetails() };
}
...
TEST(StylusHandwritingPropertiesWinTest, SetGetProperties)
{
    ...
    const std::optional<StylusHandwritingPropertiesWin> result_properties = GetStylusHandwritingProperties(event);
...
{
    TouchEvent event = GetTouchEvent();
    const std::optional<StylusHandwritingPropertiesWin> properties = GetStylusHandwritingProperties(event);
...
{
    TouchEvent event = GetTouchEvent();
    event.SetProperty(GetPropertyHandwritingPointerIdKeyForTesting(), ConvertToEventPropertyValue(kInvalidPointerId));
    EXPECT_CHECK_DEATH(GetStylusHandwritingProperties(event));

    event = GetTouchEvent();
    event.SetProperty(GetPropertyHandwritingStrokeIdKeyForTesting(), ConvertToEventPropertyValue(kInvalidStrokeId));
    EXPECT_CHECK_DEATH(GetStylusHandwritingProperties(event));
}
