...
public:
    ...
    BOOL ProcessWindowMessage(HWND window, UINT message, WPARAM w_param, LPARAM l_param, LRESULT& result, DWORD msg_map_id = 0) override
    {
...
TEST(EventWinTest, EventSystemLocationFromNative)
{
    ...
    test_window.Init(nullptr, gfx::Rect(x_window_offset, y_window_offset, 100, 100));
    EXPECT_TRUE(test_window.hwnd() != nullptr);

    {
        CHROME_MSG event = CreateEvent(WM_MOUSEWHEEL, x_coord, y_coord, test_window.hwnd());
        // Mouse wheel events already have screen coordinates so they should not be
        // converted.
        EXPECT_EQ(gfx::Point(x_coord, y_coord), EventSystemLocationFromNative(event));
    }

    CHROME_MSG event = CreateEvent(WM_LBUTTONDOWN, x_coord, y_coord, test_window.hwnd());
    EXPECT_EQ(gfx::Point(x_coord + x_window_offset, y_coord + y_window_offset), EventSystemLocationFromNative(event));
}
