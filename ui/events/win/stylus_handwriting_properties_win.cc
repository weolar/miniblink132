...
GetHandwritingStrokeIdForPointerFunc GetHandwritingStrokeIdForPointerFuncFromModule()
{
    HMODULE module = nullptr;
    if (GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_PIN, L"msctf.dll", &module)) {
        return reinterpret_cast<GetHandwritingStrokeIdForPointerFunc>(GetProcAddress(module, "GetHandwritingStrokeIdForPointer"));
    }
...
void SetStylusHandwritingProperties(Event& event, const StylusHandwritingPropertiesWin& properties)
{
    event.SetProperty(kPropertyHandwritingPointerId, ConvertToEventPropertyValue(properties.handwriting_pointer_id));
    event.SetProperty(kPropertyHandwritingStrokeId, ConvertToEventPropertyValue(properties.handwriting_stroke_id));
}

std::optional<StylusHandwritingPropertiesWin> GetStylusHandwritingProperties(const Event& event)
{
    ...
    if (const Event::Properties* event_properties = event.properties()) {
        ...
        if (it != event_properties->end()) {
            ...
            std::memcpy(&handwriting_pointer_id, it->second.data(), it->second.size());
            handwriting_properties = std::make_optional<StylusHandwritingPropertiesWin>();
...
{
    static const GetHandwritingStrokeIdForPointerFunc kGetHandwritingStrokeIdForPointerFunc = GetHandwritingStrokeIdForPointerFuncFromModule();

    if (!kGetHandwritingStrokeIdForPointerFunc) [[unlikely]] {
        TRACE_EVENT1("ime", "GetHandwritingStrokeIdForPointer", "func_pointer", "nullptr");
        // TODO(crbug.com/355578906): Add telemetry.
        return 0;
    }

    uint64_t stroke_id;
    const HRESULT hr = kGetHandwritingStrokeIdForPointerFunc(pointer_id, &stroke_id);
