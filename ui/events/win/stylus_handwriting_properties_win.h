...
struct StylusHandwritingPropertiesWin {
    ...
    StylusHandwritingPropertiesWin(uint32_t handwriting_pointer_id, uint64_t handwriting_stroke_id)
...
// event.
EVENTS_EXPORT void SetStylusHandwritingProperties(Event& event, const StylusHandwritingPropertiesWin& properties);

// Gets stylus handwriting properties (stroke and pointer ids) if such exist
// from the provided event.
EVENTS_EXPORT std::optional<StylusHandwritingPropertiesWin> GetStylusHandwritingProperties(const Event& event);
