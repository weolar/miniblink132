...
public:
    ...
    MediaKeyboardHookWinInteractiveTest(const MediaKeyboardHookWinInteractiveTest&) = delete;
    MediaKeyboardHookWinInteractiveTest& operator=(const MediaKeyboardHookWinInteractiveTest&) = delete;

protected:
    void SetUp() override
    {
        keyboard_hook_
            = KeyboardHook::CreateMediaKeyboardHook(base::BindRepeating(&MediaKeyboardHookWinInteractiveTest::HandleKeyEvent, base::Unretained(this)));
...
private:
    void HandleKeyEvent(KeyEvent* key_event)
    {
        ...
        if (key_event_wait_loop_.running() && key_events_.size() >= num_key_events_to_wait_for_) {
...
TEST_F(MediaKeyboardHookWinInteractiveTest, CallbackReceivesProperEvents)
{
    ...
    SendKeyDown(ui::VKEY_MEDIA_PLAY_PAUSE);
    WaitForKeyEvents(1);
    ExpectReceivedEvent(/*index=*/0, ui::VKEY_MEDIA_PLAY_PAUSE, EventType::kKeyPressed);

    // Send a key up event and validate it when received through the hook.
    SendKeyUp(ui::VKEY_MEDIA_PLAY_PAUSE);
    WaitForKeyEvents(2);
    ExpectReceivedEvent(/*index=*/1, ui::VKEY_MEDIA_PLAY_PAUSE, EventType::kKeyReleased);
}
