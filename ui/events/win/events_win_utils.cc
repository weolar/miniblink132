...
{
    return message == WM_MOUSELEAVE || message == WM_MOUSEHOVER || (message >= WM_MOUSEFIRST && message <= WM_MOUSELAST);
}
...
{
    return message == WM_NCMOUSELEAVE || message == WM_NCMOUSEHOVER || (message >= WM_NCMOUSEMOVE && message <= WM_NCXBUTTONDBLCLK);
}
...
{
    return native_event.message == WM_KEYDOWN || native_event.message == WM_SYSKEYDOWN || native_event.message == WM_CHAR || native_event.message == WM_SYSCHAR
        || native_event.message == WM_KEYUP || native_event.message == WM_SYSKEYUP;
}
...
EventType EventTypeFromMSG(const CHROME_MSG& native_event)
{
    ...
    case WM_MOUSEMOVE:
        return IsButtonDown(native_event) ? EventType::kMouseDragged : EventType::kMouseMoved;
    case WM_NCMOUSEMOVE:
...
base::TimeTicks EventLatencyTimeFromTickClock(DWORD event_time, base::TimeTicks current_time)
{
    ...
    base::TimeTicks event_tick_count = base::TimeTicks() + base::Milliseconds(event_time);
    ...
    if (current_tick_count < event_tick_count) {
        ...
        constexpr auto kAdditionalMillisecondsAfterWrap = static_cast<int64_t>(std::numeric_limits<DWORD>::max()) + 1;
    ...
    tick_delta = std::max(base::Microseconds(0), tick_delta - kMaxTickCountInterval);
...
{
    POINT global_point = { GET_X_LPARAM(native_event.lParam), GET_Y_LPARAM(native_event.lParam) };
...
bool GetFlingDataFromMSG(const CHROME_MSG& native_event, float* vx, float* vy, float* vx_ordinal, float* vy_ordinal, bool* is_cancel)
{
...
{
    return IsMouseMessage(message) && (GetMessageExtraInfo() & MOUSEEVENTF_FROMTOUCH) == MOUSEEVENTF_FROMTOUCH;
}
...
KeyEvent KeyEventFromMSG(const CHROME_MSG& msg)
{
    ...
    if (IsCharFromMSG(msg)) {
        ...
        return KeyEvent::FromCharacter(msg.wParam, key_code, code, flags, time_stamp);
    } else {
...
MouseEvent MouseEventFromMSG(const CHROME_MSG& msg)
{
    ...
    return MouseEvent(type, location, root_location, time_stamp, flags, changed_button_flags, pointer_details);
}
...
MouseWheelEvent MouseWheelEventFromMSG(const CHROME_MSG& msg)
{
    ...
    return MouseWheelEvent(offset, location, root_location, time_stamp, flags, changed_button_flags);
}
