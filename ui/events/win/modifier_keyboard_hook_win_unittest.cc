...
public:
    ...
    ModifierKeyboardHookWinTest& operator=(const ModifierKeyboardHookWinTest&) = delete;
...
protected:
    KeyboardHookWinBase* keyboard_hook()
    {
        return keyboard_hook_.get();
    }
    ...
    void SendModifierKeyDownEvent(KeyboardCode key_code, DomCode dom_code, int repeat_count = 1);
...
void ModifierKeyboardHookWinTest::SetUp()
{
    keyboard_hook_ = KeyboardHookWinBase::CreateModifierKeyboardHookForTesting(
        std::optional<base::flat_set<DomCode>>(), base::BindRepeating(&ModifierKeyboardHookWinTest::HandleKeyPress, base::Unretained(this)));

    keyboard_layout_ = std::make_unique<ScopedKeyboardLayout>(KeyboardLayout::KEYBOARD_LAYOUT_ENGLISH_US);
}
...
void ModifierKeyboardHookWinTest::SendModifierKeyDownEvent(KeyboardCode key_code, DomCode dom_code, int repeat_count /*=1*/)
{
    ...
    for (int i = 0; i < repeat_count; i++) {
        ASSERT_TRUE(keyboard_hook()->ProcessKeyEventMessage(WM_KEYDOWN, key_code, KeycodeConverter::DomCodeToNativeKeycode(dom_code), next_time_stamp()));
    }
}

void ModifierKeyboardHookWinTest::SendModifierKeyUpEvent(KeyboardCode key_code, DomCode dom_code)
{
    // Ensure we have a valid repeat count and the modifer passed in contains
    // location information.
    DCHECK_NE(key_code, KeyboardCode::VKEY_CONTROL);
    ...
    ASSERT_TRUE(keyboard_hook()->ProcessKeyEventMessage(WM_KEYUP, key_code, KeycodeConverter::DomCodeToNativeKeycode(dom_code), next_time_stamp()));
}

namespace {
void VerifyKeyEvent(KeyEvent* key_event, KeyboardCode non_located_key_code, DomCode dom_code, bool key_down, bool is_repeat)
{
    if (key_down) {
...
TEST_F(ModifierKeyboardHookWinTest, SimpleLeftControlKeypressTest)
{
    ...
    VerifyKeyEvent(&down_event, KeyboardCode::VKEY_CONTROL, dom_code, true, false);
...
TEST_F(ModifierKeyboardHookWinTest, RepeatingLeftControlKeypressTest)
{
    ...
    for (int i = 0; i < repeat_count; i++) {
        ...
        VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, dom_code, true, should_repeat);
...
TEST_F(ModifierKeyboardHookWinTest, SimpleLifoControlSequenceTest)
{
    ...
    VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, left_dom_code, true, false);
    ASSERT_TRUE(event.IsControlDown());
    ...
    VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, right_dom_code, true, true);
    ASSERT_TRUE(event.IsControlDown());
    ...
    VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, right_dom_code, false, false);
    ASSERT_TRUE(event.IsControlDown());

    // First key up.
    event = key_events()->at(5);
    VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, left_dom_code, false, false);
...
TEST_F(ModifierKeyboardHookWinTest, SimpleFifoControlSequenceTest)
{
    ...
    // First key down, no repeat.
    KeyEvent event = key_events()->at(0);
    VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, right_dom_code, true, false);
    ASSERT_TRUE(event.IsControlDown());
...
TEST_F(ModifierKeyboardHookWinTest, RepeatingLeftAltKeypressTest)
{
    ...
    for (int i = 0; i < repeat_count; i++) {
        KeyEvent event = key_events()->at(i);
        VerifyKeyEvent(&event, KeyboardCode::VKEY_MENU, dom_code, true, should_repeat);
        ASSERT_FALSE(event.IsControlDown());
...
TEST_F(ModifierKeyboardHookWinTest, RepeatingLeftWinKeypressTest)
{
    ...
    for (int i = 0; i < repeat_count; i++) {
        KeyEvent event = key_events()->at(i);
        // VKEY_LWIN is the 'non-located' version of the Windows key.
        VerifyKeyEvent(&event, KeyboardCode::VKEY_LWIN, dom_code, true, should_repeat);
        ASSERT_FALSE(event.IsControlDown());
        ASSERT_FALSE(event.IsAltDown());
...
TEST_F(ModifierKeyboardHookWinTest, RepeatingRightWinKeypressTest)
{
    ...
    for (int i = 0; i < repeat_count; i++) {
        KeyEvent event = key_events()->at(i);
        // VKEY_LWIN is the 'non-located' version of the Windows key.
        VerifyKeyEvent(&event, KeyboardCode::VKEY_RWIN, dom_code, true, should_repeat);
        ASSERT_FALSE(event.IsControlDown());
        ASSERT_FALSE(event.IsAltDown());
        ASSERT_TRUE(event.IsCommandDown());
        should_repeat = true;
...
TEST_F(ModifierKeyboardHookWinTest, CombinedModifierLifoSequenceKeypressTest)
{
    ...
    // First key down, no repeat.
    KeyEvent event = key_events()->at(0);
    VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, first_dom_code, true, false);
    ASSERT_TRUE(event.IsControlDown());
    ASSERT_FALSE(event.IsAltDown());
    ASSERT_FALSE(event.IsCommandDown());
    ...
    // First key still down, repeat.
    event = key_events()->at(1);
    VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, first_dom_code, true, true);
    ASSERT_TRUE(event.IsControlDown());
    ASSERT_FALSE(event.IsAltDown());
    ASSERT_FALSE(event.IsCommandDown());
    ...
    VerifyKeyEvent(&event, KeyboardCode::VKEY_RWIN, second_dom_code, false, false);
    ASSERT_TRUE(event.IsControlDown());
    ASSERT_FALSE(event.IsAltDown());
    ASSERT_FALSE(event.IsCommandDown());
    ...
    VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, first_dom_code, false, false);
    ASSERT_FALSE(event.IsControlDown());
    ASSERT_FALSE(event.IsAltDown());
    ASSERT_FALSE(event.IsCommandDown());
}
...
TEST_F(ModifierKeyboardHookWinTest, CombinedModifierFifoSequenceKeypressTest)
{
    ...
    // Second key up.
    event = key_events()->at(7);
    VerifyKeyEvent(&event, KeyboardCode::VKEY_LWIN, second_dom_code, false, false);
    ASSERT_FALSE(event.IsControlDown());
    ASSERT_TRUE(event.IsAltDown());
    ASSERT_FALSE(event.IsCommandDown());
...
TEST_F(ModifierKeyboardHookWinTest, SimpleAltGrKeyPressTest)
{
    ...
    const DWORD control_scan_code = KeycodeConverter::DomCodeToNativeKeycode(control_dom_code);
    const DWORD injected_control_scan_code = control_scan_code | 0x0200;

    ASSERT_TRUE(keyboard_hook()->ProcessKeyEventMessage(WM_KEYDOWN, KeyboardCode::VKEY_LCONTROL, injected_control_scan_code, next_time_stamp()));
    SendModifierKeyDownEvent(altgr_key_code, altgr_dom_code);

    ASSERT_TRUE(keyboard_hook()->ProcessKeyEventMessage(
        WM_KEYUP, KeyboardCode::VKEY_LCONTROL, KeycodeConverter::DomCodeToNativeKeycode(control_dom_code), next_time_stamp()));
    SendModifierKeyUpEvent(altgr_key_code, altgr_dom_code);
    ASSERT_EQ(key_events()->size(), 4ULL);

    // Injected control key down, no repeat.
    KeyEvent event = key_events()->at(0);
    VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, control_dom_code, true, false);
    ASSERT_TRUE(event.IsControlDown());
    ASSERT_FALSE(event.IsAltDown());
    ...
    event = key_events()->at(2);
    VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, control_dom_code, false, false);
    ASSERT_FALSE(event.IsControlDown());
    ASSERT_FALSE(event.IsAltDown());
    ASSERT_TRUE(event.IsAltGrDown());
    ASSERT_FALSE(event.IsCommandDown());
...
TEST_F(ModifierKeyboardHookWinTest, RepeatingAltGrKeyPressTest)
{
    ...
    event = key_events()->at(2);
    VerifyKeyEvent(&event, KeyboardCode::VKEY_CONTROL, control_dom_code, true, true);
    ASSERT_FALSE(event.IsControlDown());
    ASSERT_FALSE(event.IsAltDown());
    ASSERT_TRUE(event.IsAltGrDown());
    ASSERT_FALSE(event.IsCommandDown());
...
TEST_F(ModifierKeyboardHookWinTest, NonInterceptedKeysTest)
{
    ...
    ASSERT_FALSE(keyboard_hook()->ProcessKeyEventMessage(
        WM_KEYDOWN, KeyboardCode::VKEY_RSHIFT, KeycodeConverter::DomCodeToNativeKeycode(DomCode::SHIFT_RIGHT), next_time_stamp()));
    ASSERT_FALSE(keyboard_hook()->ProcessKeyEventMessage(
        WM_KEYUP, KeyboardCode::VKEY_RSHIFT, KeycodeConverter::DomCodeToNativeKeycode(DomCode::SHIFT_RIGHT), next_time_stamp()));

    ASSERT_FALSE(keyboard_hook()->ProcessKeyEventMessage(
        WM_KEYDOWN, KeyboardCode::VKEY_MEDIA_PLAY_PAUSE, KeycodeConverter::DomCodeToNativeKeycode(DomCode::MEDIA_PLAY_PAUSE), next_time_stamp()));
    ASSERT_FALSE(keyboard_hook()->ProcessKeyEventMessage(
        WM_KEYUP, KeyboardCode::VKEY_MEDIA_PLAY_PAUSE, KeycodeConverter::DomCodeToNativeKeycode(DomCode::MEDIA_PLAY_PAUSE), next_time_stamp()));

    ASSERT_FALSE(
        keyboard_hook()->ProcessKeyEventMessage(WM_KEYDOWN, KeyboardCode::VKEY_A, KeycodeConverter::DomCodeToNativeKeycode(DomCode::US_A), next_time_stamp()));
    ASSERT_FALSE(
        keyboard_hook()->ProcessKeyEventMessage(WM_KEYUP, KeyboardCode::VKEY_A, KeycodeConverter::DomCodeToNativeKeycode(DomCode::US_A), next_time_stamp()));
}
