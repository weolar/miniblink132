...
void EventTarget::ConvertEventToTarget(const EventTarget* target, LocatedEvent* event) const
{
}

gfx::PointF EventTarget::GetScreenLocationF(const ui::LocatedEvent& event) const
{
    NOTREACHED();
}
...
void EventTarget::AddPreTargetHandler(EventHandler* handler, Priority priority)
{
    CHECK(handler);
...
void EventTarget::RemovePreTargetHandler(EventHandler* handler)
{
    ...
    for (it = pre_target_list_.begin(), end = pre_target_list_.end(); it != end; ++it) {
...
void EventTarget::GetPreTargetHandlers(EventHandlerList* list)
{
    ...
    while (target) {
        ...
        temp.insert(temp.begin(), target->pre_target_list_.begin(), target->pre_target_list_.end());
...
void EventTarget::GetPostTargetHandlers(EventHandlerList* list)
{
    EventTarget* target = this;
    while (target) {
        list->insert(list->end(), target->post_target_list_.begin(), target->post_target_list_.end());
        target = target->GetParentTarget();
    }
}
