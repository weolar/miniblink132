...
public:
    ...
    virtual void AddMovement(const base::TimeTicks& event_time, BitSet32 id_bits, const Position* positions) = 0;
...
public:
    ...
    LeastSquaresVelocityTrackerStrategy(uint32_t degree, Weighting weighting, Restriction restriction);
    ~LeastSquaresVelocityTrackerStrategy() override;

    void Clear() override;
    void ClearPointers(BitSet32 id_bits) override;
    void AddMovement(const TimeTicks& event_time, BitSet32 id_bits, const Position* positions) override;
...
private:
    ...
    void InitState(State& state, const TimeTicks& event_time, float xpos, float ypos) const;
    void UpdateState(State& state, const TimeTicks& event_time, float xpos, float ypos) const;
...
VelocityTrackerStrategy* CreateStrategy(VelocityTracker::Strategy strategy)
{
    ...
    case VelocityTracker::LSQ2_RESTRICTED:
        return new LeastSquaresVelocityTrackerStrategy(
            2, LeastSquaresVelocityTrackerStrategy::WEIGHTING_NONE, LeastSquaresVelocityTrackerStrategy::RESTRICTION_ALIGNED_DIRECTIONS);
    ...
    case VelocityTracker::WLSQ2_DELTA:
        return new LeastSquaresVelocityTrackerStrategy(2, LeastSquaresVelocityTrackerStrategy::WEIGHTING_DELTA, no_restriction);
    case VelocityTracker::WLSQ2_CENTRAL:
        return new LeastSquaresVelocityTrackerStrategy(2, LeastSquaresVelocityTrackerStrategy::WEIGHTING_CENTRAL, no_restriction);
    case VelocityTracker::WLSQ2_RECENT:
        return new LeastSquaresVelocityTrackerStrategy(2, LeastSquaresVelocityTrackerStrategy::WEIGHTING_RECENT, no_restriction);
...
void VelocityTracker::ClearPointers(BitSet32 id_bits)
{
    ...
    if (active_pointer_id_ >= 0 && id_bits.has_bit(active_pointer_id_)) {
        active_pointer_id_ = !remaining_id_bits.is_empty() ? remaining_id_bits.first_marked_bit() : -1;
    }

    strategy_->ClearPointers(id_bits);
}

void VelocityTracker::AddMovement(const TimeTicks& event_time, BitSet32 id_bits, const Position* positions)
{
    while (id_bits.count() > MAX_POINTERS)
        id_bits.clear_last_marked_bit();

    if ((current_pointer_id_bits_.value & id_bits.value) && (event_time - last_event_time_) >= base::Milliseconds(kAssumePointerMoveStoppedTimeMs)) {
...
void VelocityTracker::AddMovement(const MotionEvent& event)
{
    ...
    case MotionEvent::Action::POINTER_UP:
        ...
        if ((event.GetEventTime() - last_event_time_) >= base::Milliseconds(kAssumePointerUpStoppedTimeMs))
            strategy_->Clear();
        return;
    default:
...
bool VelocityTracker::GetVelocity(uint32_t id, float* out_vx, float* out_vy) const
{
    Estimator estimator;
...
void LeastSquaresVelocityTrackerStrategy::AddMovement(const TimeTicks& event_time, BitSet32 id_bits, const Position* positions)
{
...
bool VelocityTracker::GetEstimator(uint32_t id, Estimator* out_estimator) const
{
...
LeastSquaresVelocityTrackerStrategy::LeastSquaresVelocityTrackerStrategy(uint32_t degree, Weighting weighting, Restriction restriction)
    : degree_(degree)
 ...
 */
static bool SolveLeastSquares(const float* x, const float* y, const float* w, uint32_t m, uint32_t n, float* out_b, float* out_det)
{
    constexpr uint32_t M_ARRAY_LENGTH = LeastSquaresVelocityTrackerStrategy::kHistorySize;
...
bool LeastSquaresVelocityTrackerStrategy::GetEstimator(uint32_t id, Estimator* out_estimator) const
{
    out_estimator->Clear();
...
void IntegratingVelocityTrackerStrategy::AddMovement(const TimeTicks& event_time, BitSet32 id_bits, const Position* positions)
{
    uint32_t index = 0;
...
bool IntegratingVelocityTrackerStrategy::GetEstimator(uint32_t id, Estimator* out_estimator) const
{
    out_estimator->Clear();
...
void IntegratingVelocityTrackerStrategy::InitState(State& state, const TimeTicks& event_time, float xpos, float ypos) const
{
...
void IntegratingVelocityTrackerStrategy::UpdateState(State& state, const TimeTicks& event_time, float xpos, float ypos) const
{
...
void IntegratingVelocityTrackerStrategy::PopulateEstimator(const State& state, Estimator* out_estimator) const
{
