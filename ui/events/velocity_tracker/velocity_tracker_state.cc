...
void VelocityTrackerState::ComputeCurrentVelocity(int32_t units, float max_velocity)
{
...
void VelocityTrackerState::GetVelocity(int32_t id, float* out_vx, float* out_vy) const
{
    ...
    float vx, vy;
    if (id >= 0 && id <= MotionEvent::MAX_POINTER_ID && calculated_id_bits_.has_bit(id)) {
