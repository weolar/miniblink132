...
struct BitSet32 {
    ...
    inline void mark_bit(uint32_t n)
    {
        value |= value_for_bit(n);
    }

    // Clears the specified bit.
    inline void clear_bit(uint32_t n)
    {
        value &= ~value_for_bit(n);
    }
    ...
    inline uint32_t first_marked_bit() const
    {
        return clz(value);
    }
    ...
    inline uint32_t first_unmarked_bit() const
    {
        return clz(~value);
    }
    ...
    // Result is undefined if all bits are unmarked.
    inline uint32_t last_marked_bit() const
    {
        return 31 - ctz(value);
    }
