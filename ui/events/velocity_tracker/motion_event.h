...
public:
    ...
    enum class Classification { NONE, AMBIGUOUS_GESTURE, DEEP_PRESS, LAST = DEEP_PRESS };
    ...
    virtual float GetHistoricalTouchMajor(size_t pointer_index, size_t historical_index) const;
    virtual float GetHistoricalX(size_t pointer_index, size_t historical_index) const;
    virtual float GetHistoricalY(size_t pointer_index, size_t historical_index) const;
    ...
    float GetRawOffsetX() const
    {
        return GetRawX() - GetX();
    }
    float GetRawOffsetY() const
    {
        return GetRawY() - GetY();
    }
    ...
    {
        return GetTangentialPressure(0);
    }

    ToolType GetToolType() const
    {
        return GetToolType(0);
    }
...
COMPONENT_EXPORT(VELOCITY_TRACKER) std::ostream& operator<<(std::ostream& stream, const MotionEvent::Action action);
COMPONENT_EXPORT(VELOCITY_TRACKER) std::ostream& operator<<(std::ostream& stream, const MotionEvent::ToolType tool_type);
