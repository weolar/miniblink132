...
protected:
    static MockMotionEvent Sample(MotionEvent::Action action, const gfx::PointF& p0, TimeTicks t0, const gfx::Vector2dF& v, base::TimeDelta dt)
    {
        const gfx::PointF p = p0 + ScaleVector2d(v, dt.InSecondsF());
        return MockMotionEvent(action, t0 + dt, p.x(), p.y());
    }

    static void ApplyMovementSequence(
        VelocityTrackerState* state, const gfx::PointF& p0, const gfx::Vector2dF& v, TimeTicks t0, base::TimeDelta t, size_t samples)
    {
        EXPECT_TRUE(samples);
        if (!samples)
            return;
    ...
    static void ApplyMovement(VelocityTrackerState* state, const gfx::PointF& p0, const gfx::Vector2dF& v, TimeTicks t0, base::TimeDelta t, size_t samples)
    {
        EXPECT_TRUE(samples);
        if (!samples)
            return;
...
TEST_F(VelocityTrackerTest, Basic)
{
    ...
    for (int i = 0; i <= VelocityTracker::STRATEGY_MAX; ++i) {
        VelocityTracker::Strategy strategy = static_cast<VelocityTracker::Strategy>(i);
...
TEST_F(VelocityTrackerTest, VaryingVelocity)
{
    ...
    for (int i = 0; i <= VelocityTracker::STRATEGY_MAX; ++i) {
        ...
        state.AddMovement(Sample(MotionEvent::Action::DOWN, p0, t0, vFast, base::TimeDelta()));
...
TEST_F(VelocityTrackerTest, DelayedActionUp)
{
    ...
    VelocityTrackerState state(VelocityTracker::Strategy::LSQ2);
    state.AddMovement(Sample(MotionEvent::Action::DOWN, p0, t0, v, base::TimeDelta()));
    ...
    state.AddMovement(Sample(MotionEvent::Action::UP, p1, t1, v, base::TimeDelta()));
