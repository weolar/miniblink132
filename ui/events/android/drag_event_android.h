...
public:
    DragEventAndroid(JNIEnv* env, int action, const gfx::PointF& location, const gfx::PointF& screen_location, const std::vector<std::u16string>& mime_types,
        jstring content, jobjectArray filenames, jstring text, jstring html, jstring url);
    ...
    // its location.
    std::unique_ptr<DragEventAndroid> CreateFor(const gfx::PointF& new_location) const;
