...
public:
    GestureEventAndroid(int type, const gfx::PointF& location, const gfx::PointF& screen_location, long time_ms, float scale, float delta_x, float delta_y,
        float velocity_x, float velocity_y, bool target_viewport, bool synthetic_scroll, bool prevent_boosting);
    ...
    const gfx::PointF& screen_location() const
    {
    ...
    bool synthetic_scroll() const
    {
        return synthetic_scroll_;
    }
    bool prevent_boosting() const
    {
        return prevent_boosting_;
    }

    // Creates a new GestureEventAndroid instance different from |this| only by
    // its location.
    std::unique_ptr<GestureEventAndroid> CreateFor(const gfx::PointF& new_location) const;
