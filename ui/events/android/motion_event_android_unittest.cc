...
TEST(MotionEventAndroidTest, Constructor)
{
    ...
    base::TimeTicks latest_event_time = base::TimeTicks() + base::Nanoseconds(kLatestEventTimeNS);
    base::TimeTicks oldest_event_time = base::TimeTicks() + base::Nanoseconds(kOldestEventTimeNS);

    ui::test::ScopedEventTestTickClock clock;
    clock.SetNowTicks(latest_event_time);

    MotionEventAndroid::Pointer p0(1, 13.7f, -7.13f, 5.3f, 1.2f, 0.1f, 0.2f, kAndroidToolTypeFinger);
    MotionEventAndroid::Pointer p1(2, -13.7f, 7.13f, 3.5f, 12.1f, -0.1f, 0.4f, kAndroidToolTypeFinger);
    float raw_offset = -3.f;
    int pointer_count = 2;
    int history_size = 0;
    int action_index = -1;
    MotionEventAndroidJava event(base::android::AttachCurrentThread(), nullptr, kPixToDip, 0.f, 0.f, 0.f, oldest_event_time, latest_event_time,
        kAndroidActionDown, pointer_count, history_size, action_index, kAndroidActionButton, 0, kAndroidButtonPrimary, kAndroidAltKeyDown, 0, raw_offset,
        -raw_offset, false, &p0, &p1);
    ...
    EXPECT_NEAR(p0.tilt_rad, ConvertToTiltRad(event.GetTiltX(0), event.GetTiltY(0)), float_error);
    EXPECT_NEAR(p1.tilt_rad, ConvertToTiltRad(event.GetTiltX(1), event.GetTiltY(1)), float_error);
...
TEST(MotionEventAndroidTest, Clone)
{
    ...
    MotionEventAndroid::Pointer p0(1, 13.7f, -7.13f, 5.3f, 1.2f, 0.1f, 0.2f, kAndroidToolTypeFinger);
    MotionEventAndroidJava event(base::android::AttachCurrentThread(), nullptr, kPixToDip, 0, 0, 0, base::TimeTicks(), kAndroidActionDown, pointer_count, 0, 0,
        0, 0, 0, 0, 0, 0, 0, false, &p0, nullptr);
...
TEST(MotionEventAndroidTest, Cancel)
{
    ...
    base::TimeTicks event_time = base::TimeTicks() + base::Nanoseconds(kEventTimeNS);
    ui::test::ScopedEventTestTickClock clock;
    clock.SetNowTicks(event_time);

    const int pointer_count = 1;
    MotionEventAndroid::Pointer p0(1, 13.7f, -7.13f, 5.3f, 1.2f, 0.1f, 0.2f, kAndroidToolTypeFinger);
    MotionEventAndroidJava event(base::android::AttachCurrentThread(), nullptr, kPixToDip, 0, 0, 0, base::TimeTicks() + base::Nanoseconds(kEventTimeNS),
        kAndroidActionDown, pointer_count, 0, 0, 0, 0, 0, 0, 0, 0, 0, false, &p0, nullptr);
    ...
    EXPECT_EQ(static_cast<size_t>(pointer_count), cancel_event->GetPointerCount());
...
TEST(MotionEventAndroidTest, NonEmptyHistoryForNonMoveEventsSanitized)
{
    ...
    MotionEventAndroidJava event(base::android::AttachCurrentThread(), nullptr, kPixToDip, 0, 0, 0, base::TimeTicks(), base::TimeTicks(), kAndroidActionDown,
        pointer_count, history_size, 0, 0, 0, 0, 0, 0, 0, 0, false, &p0, nullptr);
...
TEST(MotionEventAndroidTest, ActionIndexForPointerDown)
{
    ...
    int history_size = 0;
    int action_index = 1;
    MotionEventAndroidJava event(base::android::AttachCurrentThread(), nullptr, kPixToDip, 0, 0, 0, base::TimeTicks(), kAndroidActionPointerDown, pointer_count,
        history_size, action_index, 0, 0, 0, 0, 0, 0, 0, false, &p0, &p1);
...
TEST(MotionEventAndroidTest, NativeBackedConstructor)
{
    if (base::android::BuildInfo::GetInstance()->sdk_int() < base::android::SDK_VERSION_S) {
        GTEST_SKIP() << "AMotionEvent_fromJava used in test is only available on S+";
    }
    ...
    base::android::ScopedJavaLocalRef<jobject> java_motion_event
        = JNI_MotionEvent::Java_MotionEvent_obtain(env, down_time, event_time, kAndroidActionDown, x, y, kAndroidAltKeyDown);
    ...
    std::unique_ptr<MotionEventAndroid> event = MotionEventAndroidNative::Create(native_event, kPixToDip,
        /* y_offset_pix= */ 0);
