...
public:
    ...
    static std::unique_ptr<MotionEventAndroid> Create(const AInputEvent* event, float pix_to_dip, int y_offset_pix);

private:
    MotionEventAndroidNative(const AInputEvent* event, float pix_to_dip, float ticks_x, float ticks_y, float tick_multiplier, base::TimeTicks oldest_event_time,
        ...
        bool for_touch_handle, const Pointer* const pointer0, const Pointer* const pointer1, int y_offset);
