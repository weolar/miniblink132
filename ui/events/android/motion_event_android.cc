...
MotionEventAndroid::Pointer::Pointer(jint id, jfloat pos_x_pixels, jfloat pos_y_pixels, jfloat touch_major_pixels, jfloat touch_minor_pixels,
    jfloat orientation_rad, jfloat tilt_rad, jint tool_type)
    : id(id)
...
MotionEventAndroid::MotionEventAndroid(float pix_to_dip, float ticks_x, float ticks_y, float tick_multiplier, base::TimeTicks oldest_event_time,
    base::TimeTicks latest_event_time, int android_action, int pointer_count, int history_size, int action_index, int android_action_button,
    int android_gesture_classification, int android_button_state, int android_meta_state, int source, float raw_offset_x_pixels, float raw_offset_y_pixels,
    bool for_touch_handle, const Pointer* const pointer0, const Pointer* const pointer1)
    ...
    , cached_raw_position_offset_(ToDips(raw_offset_x_pixels), ToDips(raw_offset_y_pixels))
...
MotionEventAndroid::MotionEventAndroid(const MotionEventAndroid& e, const gfx::PointF& point)
...
{
    if (cached_pointer_count_ > 1) {
        gfx::Vector2dF diff = e.cached_pointers_[1].position - e.cached_pointers_[0].position;
        cached_pointers_[1] = CreateCachedPointer(e.cached_pointers_[1], point + diff);
    }
...
std::unique_ptr<MotionEventAndroid> MotionEventAndroid::CreateFor(const gfx::PointF& point) const
{
...
{
    return static_cast<MotionEvent::Classification>(cached_gesture_classification_);
}
...
{
    DCHECK(cached_action_ == MotionEvent::Action::POINTER_UP || cached_action_ == MotionEvent::Action::POINTER_DOWN)
...
MotionEventAndroid::ToolType MotionEventAndroid::FromAndroidToolType(int android_tool_type)
{
    ...
    default:
        NOTREACHED() << "Invalid Android MotionEvent tool type: " << android_tool_type;
    }
}
...
// in [-90, 90].
void MotionEventAndroid::ConvertTiltOrientationToTiltXY(float tilt_rad, float orientation_rad, float* tilt_x, float* tilt_y)
{
...
MotionEventAndroid::CachedPointer MotionEventAndroid::FromAndroidPointer(const Pointer& pointer) const
{
    ...
    ConvertTiltOrientationToTiltXY(tilt_rad, result.orientation, &result.tilt_x, &result.tilt_y);
    result.tool_type = FromAndroidToolType(pointer.tool_type);
    return result;
}

MotionEventAndroid::CachedPointer MotionEventAndroid::CreateCachedPointer(const CachedPointer& pointer, const gfx::PointF& point) const
{
    CachedPointer result;
    result.id = pointer.id;
