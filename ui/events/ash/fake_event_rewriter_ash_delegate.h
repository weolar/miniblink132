...
public:
    ...
    FakeEventRewriterAshDelegate& operator=(const FakeEventRewriterAshDelegate&) = delete;
    ~FakeEventRewriterAshDelegate() override;

    // Configures the modifier remapping as specified.
    void SetModifierRemapping(std::string_view pref_name, ui::mojom::ModifierKey value);
    ...
    std::optional<ui::mojom::ModifierKey> GetKeyboardRemappedModifierValue(
        int device_id, ui::mojom::ModifierKey modifier_key, const std::string& pref_name) const override;
    bool TopRowKeysAreFunctionKeys(int device_id) const override;
    bool IsExtensionCommandRegistered(KeyboardCode key_code, int flags) const override;
    ...
    void RecordSixPackEventRewrite(KeyboardCode key_code, bool alt_based) override;
    std::optional<ui::mojom::SimulateRightClickModifier> GetRemapRightClickModifier(int device_id) override;
    std::optional<ui::mojom::SixPackShortcutModifier> GetShortcutModifierForSixPackKey(int device_id, ui::KeyboardCode key_code) override;
    void NotifyRightClickRewriteBlockedBySetting(
        ui::mojom::SimulateRightClickModifier blocked_modifier, ui::mojom::SimulateRightClickModifier active_modifier) override;
    void NotifySixPackRewriteBlockedBySetting(ui::KeyboardCode key_code, ui::mojom::SixPackShortcutModifier blocked_modifier,
        ui::mojom::SixPackShortcutModifier active_modifier, int device_id) override;
    void NotifySixPackRewriteBlockedByFnKey(ui::KeyboardCode key_code, ui::mojom::SixPackShortcutModifier modifier) override;
    void NotifyTopRowRewriteBlockedByFnKey() override;

    std::optional<ui::mojom::ExtendedFkeysModifier> GetExtendedFkeySetting(int device_id, ui::KeyboardCode key_code) override;
