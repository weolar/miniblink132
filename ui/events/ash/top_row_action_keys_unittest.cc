...
constexpr auto kTopRowActionKeyToName = base::MakeFixedFlatMap<TopRowActionKey, const char*>({
#define TOP_ROW_ACTION_KEYS_ENTRY(action) { TopRowActionKey::k##action, #action },
#define TOP_ROW_ACTION_KEYS_LAST_ENTRY(action) { TopRowActionKey::k##action, #action },
    TOP_ROW_ACTION_KEYS
...
{
    const auto enums = base::ReadEnumFromEnumsXml("KeyboardTopRowActionKeys", "chromeos");
    ASSERT_TRUE(enums);
    ...
    for (const auto& entry : *enums) {
        ...
        EXPECT_EQ(entry.second, kTopRowActionKeyToName.at(static_cast<TopRowActionKey>(entry.first)))
            << "Enum entry name: " << entry.second
            << " in enums.xml is different from enum entry name: " << kTopRowActionKeyToName.at(static_cast<TopRowActionKey>(entry.first)) << " in C++ file";
    }
}
