...
constexpr auto kModifierKeyUsageMappings = base::MakeFixedFlatMap<DomCode, ModifierKeyUsageMetric>({
...
std::optional<std::string> GetModifierNameFromModifierKeyUsage(ModifierKeyUsageMetric val)
{
    switch (val) {
...
std::string_view GetDeviceNameForHistogram(KeyboardCapability::DeviceType device_type)
{
...
void RecordKeyUsageMetric(const KeyboardCapability& keyboard_capability, KeyboardCapability::DeviceType device_type, int device_id, DomCode dom_code,
    DomCode original_dom_code, ui::ModifierKeyUsageMetric modifier_key)
{
    ...
    const bool rewritten_to_right_alt = dom_codes_match && modifier_key == ModifierKeyUsageMetric::kRightAlt && !keyboard_capability.HasRightAltKey(device_id);
    const bool rewritten_to_assistant
        = dom_codes_match && modifier_key == ModifierKeyUsageMetric::kAssistant && !keyboard_capability.HasAssistantKey(device_id);
    const bool modifier_was_rewritten = !dom_codes_match || rewritten_to_assistant || rewritten_to_right_alt;
    base::UmaHistogramEnumeration(base::StrCat({ "ChromeOS.Inputs.KeyUsage.Internal.", *modifier_name }),
        modifier_was_rewritten ? KeyUsageCategory::kVirtuallyPressed : KeyUsageCategory::kPhysicallyPressed);
}

} // namespace

void RecordModifierKeyPressedBeforeRemapping(const KeyboardCapability& keyboard_capability, int device_id, DomCode dom_code)
{
    ...
    auto device_name = GetDeviceNameForHistogram(keyboard_capability.GetDeviceType(device_id));
    ...
    if (modifier_key == ModifierKeyUsageMetric::kAssistant && keyboard_capability.HasRightAltKey(device_id)) {
        modifier_key = ModifierKeyUsageMetric::kRightAlt;
    }

    if ((modifier_key == ModifierKeyUsageMetric::kFunction || modifier_key == ModifierKeyUsageMetric::kRightAlt)
        && !keyboard_capability.HasRightAltKey(device_id)) {
        return;
    }

    base::UmaHistogramEnumeration(base::StrCat({ "ChromeOS.Inputs.Keyboard.ModifierPressed.", device_name }), modifier_key);
}

void RecordModifierKeyPressedAfterRemapping(
    const KeyboardCapability& keyboard_capability, int device_id, DomCode dom_code, DomCode original_dom_code, bool is_right_alt_key)
{
    auto it = kModifierKeyUsageMappings.find(dom_code);
    if (it == kModifierKeyUsageMappings.end()) {
        return;
    ...
    base::UmaHistogramEnumeration(base::StrCat({ "ChromeOS.Inputs.Keyboard.RemappedModifierPressed.", device_name }), modifier_key);
    RecordKeyUsageMetric(keyboard_capability, device_type, device_id, dom_code, original_dom_code, modifier_key);
}
