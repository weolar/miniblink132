...
inline constexpr auto kLayout1TopRowKeyToFKeyMap = base::MakeFixedFlatMap<KeyboardCode, KeyboardCode>({
...
inline constexpr auto kLayout2TopRowKeyToFKeyMap = base::MakeFixedFlatMap<KeyboardCode, KeyboardCode>({
    { KeyboardCode::VKEY_BROWSER_BACK, KeyboardCode::VKEY_F1 },
...
inline constexpr auto kLayoutWilcoDrallionTopRowKeyToFKeyMap = base::MakeFixedFlatMap<KeyboardCode, KeyboardCode>({
    { KeyboardCode::VKEY_BROWSER_BACK, KeyboardCode::VKEY_F1 },
    { KeyboardCode::VKEY_BROWSER_REFRESH, KeyboardCode::VKEY_F2 },
    { KeyboardCode::VKEY_ZOOM, KeyboardCode::VKEY_F3 },
    { KeyboardCode::VKEY_MEDIA_LAUNCH_APP1, KeyboardCode::VKEY_F4 },
...
inline constexpr auto kSixPackKeyToSearchSystemKeyMap = base::MakeFixedFlatMap<KeyboardCode, KeyboardCode>({
...
inline constexpr auto kSixPackKeyToFnKeyMap = base::MakeFixedFlatMap<KeyboardCode, KeyboardCode>({
    { KeyboardCode::VKEY_DELETE, KeyboardCode::VKEY_BACK },
    { KeyboardCode::VKEY_HOME, KeyboardCode::VKEY_LEFT },
    { KeyboardCode::VKEY_PRIOR, KeyboardCode::VKEY_UP },
    { KeyboardCode::VKEY_END, KeyboardCode::VKEY_RIGHT },
...
inline constexpr auto kSixPackKeyToAltSystemKeyMap = base::MakeFixedFlatMap<KeyboardCode, KeyboardCode>({
    { KeyboardCode::VKEY_DELETE, KeyboardCode::VKEY_BACK },
...
public:
    using ScanCodeToEvdevKeyConverter = base::RepeatingCallback<std::optional<uint32_t>(const base::ScopedFD& fd, uint32_t scancode)>;
    ...
    struct KeyboardInfo {
        ...
        KeyboardTopRowLayout top_row_layout = KeyboardTopRowLayout::kKbdTopRowLayoutDefault;
    ...
    static std::unique_ptr<EventDeviceInfo> CreateEventDeviceInfoFromInputDevice(const KeyboardDevice& keyboard);

    // Converts from the given `key_code` to the corresponding meaning in
    // `TopRowActionKey` enum.
    static std::optional<TopRowActionKey> ConvertToTopRowActionKey(ui::KeyboardCode key_code);

    // Converts the given `action_key` to the corresponding `KeyboardCode` VKEY.
    static std::optional<KeyboardCode> ConvertToKeyboardCode(TopRowActionKey action_key);
    ...
    std::optional<KeyboardCode> GetMappedFKeyIfExists(const KeyboardCode& key_code, const KeyboardDevice& keyboard) const;
    ...
    std::vector<mojom::ModifierKey> GetModifierKeys(const KeyboardDevice& keyboard) const;
    ...
    const std::vector<uint32_t>* GetTopRowScanCodes(const KeyboardDevice& keyboard) const;
    ...
    bool HasKeyEvent(const KeyboardCode& key_code, const KeyboardDevice& keyboard) const;
    ...
    bool HasTopRowActionKey(const KeyboardDevice& keyboard, TopRowActionKey action_key) const;
    ...
    // given `keyboard`.
    std::optional<KeyboardCode> GetCorrespondingFunctionKey(const KeyboardDevice& keyboard, TopRowActionKey action_key) const;
    ...
    std::optional<TopRowActionKey> GetCorrespondingActionKeyForFKey(const KeyboardDevice& keyboard, KeyboardCode key_code) const;

    const std::vector<TopRowActionKey>* GetTopRowActionKeys(const KeyboardDevice& keyboard) const;
