...
KeyboardDeviceIdEventRewriter::KeyboardDeviceIdEventRewriter(KeyboardCapability* keyboard_capability)
...
EventDispatchDetails KeyboardDeviceIdEventRewriter::RewriteEvent(const Event& event, const Continuation continuation)
{
...
// static
int KeyboardDeviceIdEventRewriter::GetKeyboardDeviceId(int keyboard_device_id, int last_keyboard_device_id, KeyboardCapability* keyboard_capability)
{
    ...
    if (keyboard_capability->GetDeviceType(keyboard_device_id) == KeyboardCapability::DeviceType::kDeviceVirtualCoreKeyboard) {
...
std::optional<int> KeyboardDeviceIdEventRewriter::GetKeyboardDeviceIdInternal(const Event& event) const
{
    switch (event.type()) {
    case EventType::kKeyPressed:
    case EventType::kKeyReleased:
        return GetKeyboardDeviceId(event.source_device_id(), last_keyboard_device_id_, keyboard_capability_);
