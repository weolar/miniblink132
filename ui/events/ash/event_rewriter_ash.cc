...
enum class AutoRepeatUsageModifierFlag : uint32_t { kMeta = 0, kControl = 1, kAlt = 2, kShift = 3, kCapsLock = 4, kNumModifierFlags };
...
constexpr int kAutoRepeatUsageAmountToShiftModifierFlags = ((sizeof(uint32_t) * 8) - static_cast<int>(AutoRepeatUsageModifierFlag::kNumModifierFlags));
...
static_assert(kAutoRepeatUsageAmountToShiftModifierFlags >= kAutoRepeatUsageNumBitsReservedForKeyboardCode);
...
void RecordAutoRepeatUsageMetric(const KeyEvent& key_event, const std::unique_ptr<Event>& rewritten_event)
{
    ...
    if (!(auto_repeat_event->flags() & EF_IS_REPEAT) || auto_repeat_event->type() != EventType::kKeyPressed) {
        return;
    }

    // Apply remapped auto repeat event flags.
    uint32_t auto_repeat_usage_modifier_flags = 0;
    for (const auto& [event_flag, auto_repeat_flag] : kEventFlagToAutoRepeatMetricFlag) {
        if (auto_repeat_event->flags() & event_flag) {
            auto_repeat_usage_modifier_flags |= (1u << static_cast<uint32_t>(auto_repeat_flag));
        }
    }

    UMA_HISTOGRAM_SPARSE(
        "ChromeOS.Inputs.AutoRepeatUsage", ((auto_repeat_usage_modifier_flags << kAutoRepeatUsageAmountToShiftModifierFlags) + auto_repeat_event->key_code()));
}
...
} kModifierRemappings[] = { { EF_CONTROL_DOWN, ui::mojom::ModifierKey::kControl, prefs::kLanguageRemapControlKeyTo,
                                { EF_CONTROL_DOWN, DomCode::CONTROL_LEFT, DomKey::CONTROL, VKEY_CONTROL } },
    { EF_MOD3_DOWN | EF_ALTGR_DOWN, mojom::ModifierKey::kIsoLevel5ShiftMod3, nullptr,
        { EF_MOD3_DOWN | EF_ALTGR_DOWN, DomCode::CAPS_LOCK, DomKey::ALT_GRAPH, VKEY_ALTGR } },
    { EF_COMMAND_DOWN, ui::mojom::ModifierKey::kMeta, prefs::kLanguageRemapSearchKeyTo, { EF_COMMAND_DOWN, DomCode::META_LEFT, DomKey::META, VKEY_LWIN } },
    { EF_ALT_DOWN, ui::mojom::ModifierKey::kAlt, prefs::kLanguageRemapAltKeyTo, { EF_ALT_DOWN, DomCode::ALT_LEFT, DomKey::ALT, VKEY_MENU } },
    { EF_NONE, ui::mojom::ModifierKey::kVoid, nullptr, { EF_NONE, DomCode::NONE, DomKey::NONE, VKEY_UNKNOWN } },
    { EF_MOD3_DOWN, ui::mojom::ModifierKey::kCapsLock, prefs::kLanguageRemapCapsLockKeyTo,
        { EF_MOD3_DOWN, DomCode::CAPS_LOCK, DomKey::CAPS_LOCK, VKEY_CAPITAL } },
    { EF_NONE, ui::mojom::ModifierKey::kEscape, prefs::kLanguageRemapEscapeKeyTo, { EF_NONE, DomCode::ESCAPE, DomKey::ESCAPE, VKEY_ESCAPE } },
    { EF_NONE, ui::mojom::ModifierKey::kBackspace, prefs::kLanguageRemapBackspaceKeyTo, { EF_NONE, DomCode::BACKSPACE, DomKey::BACKSPACE, VKEY_BACK } },
    { EF_NONE, ui::mojom::ModifierKey::kAssistant, prefs::kLanguageRemapAssistantKeyTo,
        { EF_NONE, DomCode::LAUNCH_ASSISTANT, DomKey::LAUNCH_ASSISTANT, VKEY_ASSISTANT } },
    { EF_NONE, ui::mojom::ModifierKey::kRightAlt, nullptr, { EF_NONE, DomCode::LAUNCH_ASSISTANT, DomKey::LAUNCH_ASSISTANT, VKEY_RIGHT_ALT } },
    { EF_FUNCTION_DOWN, ui::mojom::ModifierKey::kFunction, nullptr, { EF_FUNCTION_DOWN, DomCode::FN, DomKey::FN, VKEY_FUNCTION } } };
...
constexpr const ModifierRemapping* kModifierRemappingIsoLevel5ShiftMod3 = GetModifierRemappingNeoMod3();
...
const ModifierRemapping* GetRemappedKey(int device_id, mojom::ModifierKey modifier_key, const std::string& pref_name, EventRewriterAsh::Delegate* delegate)
{
    if (!delegate) {
        return nullptr;
    }

    const auto remapped_modifier_key = delegate->GetKeyboardRemappedModifierValue(device_id, modifier_key, pref_name);
...
const ModifierRemapping* GetSearchRemappedKey(EventRewriterAsh::Delegate* delegate, int device_id, const KeyboardCapability& keyboard_capability)
{
    ...
    case KeyboardCapability::DeviceType::kDeviceExternalNullTopRowChromeOsKeyboard:
    ...
    return GetRemappedKey(device_id, mojom::ModifierKey::kMeta, pref_name, delegate);
}

bool ShouldRewriteMetaTopRowKeyComboEvents(EventRewriterAsh::Delegate* delegate, int device_id)
{
...
bool IsFirstPartyKoreanIME()
{
    ...
    auto current_input_method = manager->GetActiveIMEState()->GetCurrentInputMethod();
...
const KeyboardRemapping kMergedSixPackRemappings[]
    = { { // Search+Shift+BackSpace -> Insert
            { EF_COMMAND_DOWN | EF_SHIFT_DOWN, VKEY_BACK }, { EF_NONE, DomCode::INSERT, DomKey::INSERT, VKEY_INSERT } },
          { // Search+BackSpace -> Delete
              { EF_COMMAND_DOWN, VKEY_BACK }, { EF_NONE, DomCode::DEL, DomKey::DEL, VKEY_DELETE } },
          { // Alt+BackSpace -> Delete
              { EF_ALT_DOWN, VKEY_BACK }, { EF_NONE, DomCode::DEL, DomKey::DEL, VKEY_DELETE } },
          { // Search+Left -> Home
              { EF_COMMAND_DOWN, VKEY_LEFT }, { EF_NONE, DomCode::HOME, DomKey::HOME, VKEY_HOME } },
          { // Control+Alt+Up -> Home
              { EF_ALT_DOWN | EF_CONTROL_DOWN, VKEY_UP }, { EF_NONE, DomCode::HOME, DomKey::HOME, VKEY_HOME } },
          { // Search+Up -> Prior (aka PageUp)
              { EF_COMMAND_DOWN, VKEY_UP }, { EF_NONE, DomCode::PAGE_UP, DomKey::PAGE_UP, VKEY_PRIOR } },
          { // Alt+Up -> Prior (aka PageUp)
              { EF_ALT_DOWN, VKEY_UP }, { EF_NONE, DomCode::PAGE_UP, DomKey::PAGE_UP, VKEY_PRIOR } },
          { // Search+Right -> End
              { EF_COMMAND_DOWN, VKEY_RIGHT }, { EF_NONE, DomCode::END, DomKey::END, VKEY_END } },
          { // Control+Alt+Down -> End
              { EF_ALT_DOWN | EF_CONTROL_DOWN, VKEY_DOWN }, { EF_NONE, DomCode::END, DomKey::END, VKEY_END } },
          { // Search+Down -> Next (aka PageDown)
              { EF_COMMAND_DOWN, VKEY_DOWN }, { EF_NONE, DomCode::PAGE_DOWN, DomKey::PAGE_DOWN, VKEY_NEXT } },
          { // Alt+Down -> Next (aka PageDown)
              { EF_ALT_DOWN, VKEY_DOWN }, { EF_NONE, DomCode::PAGE_DOWN, DomKey::PAGE_DOWN, VKEY_NEXT } } };
...
bool MatchKeyboardRemapping(const EventRewriterAsh::MutableKeyState& suspect, const KeyboardRemapping::Condition& test, bool strict = false)
{
    // Reset non modifier key event related flags for strict mode.
    constexpr int kKeyEventModifiersMask = EF_SHIFT_DOWN | EF_CONTROL_DOWN | EF_ALT_DOWN | EF_COMMAND_DOWN | EF_ALTGR_DOWN | EF_MOD3_DOWN;

    const int suspect_flags_for_strict = suspect.flags & kKeyEventModifiersMask;
    const bool flag_matched = strict ? suspect_flags_for_strict == test.flags : ((suspect.flags & test.flags) == test.flags);
    return flag_matched && ((test.key_code == VKEY_UNKNOWN) || (test.key_code == suspect.key_code));
}

void ApplyRemapping(const EventRewriterAsh::MutableKeyState& changes, EventRewriterAsh::MutableKeyState* state)
{
...
bool RewriteWithKeyboardRemappings(const KeyboardRemapping* mappings, size_t num_mappings, const EventRewriterAsh::MutableKeyState& input_state,
    EventRewriterAsh::MutableKeyState* remapped_state, bool strict = false)
{
...
ui::KeyboardCode MatchedDeprecatedRemapping(const KeyboardRemapping* mappings, size_t num_mappings, const EventRewriterAsh::MutableKeyState& input_state)
{
    for (size_t i = 0; i < num_mappings; ++i) {
        const KeyboardRemapping& map = mappings[i];
...
void SetMeaningForLayout(EventType type, EventRewriterAsh::MutableKeyState* state)
{
...
KeyboardCode RelocateKeyboardCode(KeyboardCode key_code, DomKeyLocation location)
{
...
{
    for (const InputDevice& touchpad : DeviceDataManager::GetInstance()->GetTouchpadDevices()) {
...
bool IsKeyCodeInMappings(KeyboardCode key_code, const KeyboardRemapping* mappings, size_t num_mappings)
{
    for (size_t i = 0; i < num_mappings; ++i) {
        const KeyboardRemapping& map = mappings[i];
...
// generate an F-key.
void RecordSearchPlusDigitFKeyRewrite(ui::EventType event_type, ui::KeyboardCode key_code)
{
...
void RecordSixPackEventRewrites(EventRewriterAsh::Delegate* delegate, ui::EventType event_type, ui::KeyboardCode key_code, bool legacy_variant)
{
    if (event_type != EventType::kKeyPressed) {
        return;
    }
    ...
    if (!legacy_variant) {
        switch (key_code) {
        case ui::VKEY_DELETE:
            base::RecordAction(base::UserMetricsAction("SearchBasedKeyRewrite_Delete"));
            break;
        case ui::VKEY_INSERT:
            base::RecordAction(base::UserMetricsAction("SearchBasedKeyRewrite_Insert_ViaSearchShiftBackspace"));
            break;
        case ui::VKEY_HOME:
            base::RecordAction(base::UserMetricsAction("SearchBasedKeyRewrite_Home"));
            break;
        case ui::VKEY_END:
            base::RecordAction(base::UserMetricsAction("SearchBasedKeyRewrite_End"));
            break;
        case ui::VKEY_PRIOR:
            base::RecordAction(base::UserMetricsAction("SearchBasedKeyRewrite_PageUp"));
            break;
        case ui::VKEY_NEXT:
            base::RecordAction(base::UserMetricsAction("SearchBasedKeyRewrite_PageDown"));
            break;
        default:
            NOTREACHED();
        }
        ...
        switch (key_code) {
        case ui::VKEY_DELETE:
            base::RecordAction(base::UserMetricsAction("AltBasedKeyRewrite_Delete"));
            break;
        case ui::VKEY_INSERT:
            base::RecordAction(base::UserMetricsAction("SearchBasedKeyRewrite_Insert"));
            break;
        case ui::VKEY_HOME:
        ...
        case ui::VKEY_END:
            ...
            break;
        case ui::VKEY_PRIOR:
            base::RecordAction(base::UserMetricsAction("AltBasedKeyRewrite_PageUp"));
            break;
        case ui::VKEY_NEXT:
            base::RecordAction(base::UserMetricsAction("AltBasedKeyRewrite_PageDown"));
            break;
        default:
            NOTREACHED();
        }
...
void RecordFunctionKeyFromKeyCode(ui::KeyboardCode key_code, ui::InputKeyEventToFunctionKey event_enum)
{
    ...
    switch (key_code) {
    case VKEY_F1:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F1Pressed", event_enum);
        break;
    case VKEY_F2:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F2Pressed", event_enum);
        break;
    case VKEY_F3:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F3Pressed", event_enum);
        break;
    case VKEY_F4:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F4Pressed", event_enum);
        break;
    case VKEY_F5:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F5Pressed", event_enum);
        break;
    case VKEY_F6:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F6Pressed", event_enum);
        break;
    case VKEY_F7:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F7Pressed", event_enum);
        break;
    case VKEY_F8:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F8Pressed", event_enum);
        break;
    case VKEY_F9:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F9Pressed", event_enum);
        break;
    case VKEY_F10:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F10Pressed", event_enum);
        break;
    case VKEY_F11:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F11Pressed", event_enum);
        break;
    case VKEY_F12:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F12Pressed", event_enum);
        break;
    case VKEY_F13:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F13Pressed", event_enum);
        break;
    case VKEY_F14:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F14Pressed", event_enum);
        break;
    case VKEY_F15:
        UMA_HISTOGRAM_ENUMERATION("ChromeOS.Inputs.Keyboard.F15Pressed", event_enum);
        break;
    default:
        NOTREACHED();
    }
...
void RecordRewritingToFunctionKeys(const KeyEvent& key_event, const EventRewriterAsh::MutableKeyState* rewritten_state)
{
    ...
    if (rewritten_state->key_code < VKEY_F1 || rewritten_state->key_code > VKEY_F15) {
        return;
    }

    const bool search_is_pressed = (key_event.flags() & EF_COMMAND_DOWN) != 0;
    const bool search_in_rewritten_event = (rewritten_state->flags & EF_COMMAND_DOWN) != 0;
    if (search_is_pressed == search_in_rewritten_event) {
        if (key_event.key_code() >= VKEY_F1 && key_event.key_code() <= VKEY_F15) {
            ...
            RecordFunctionKeyFromKeyCode(rewritten_state->key_code, InputKeyEventToFunctionKey::kDirectlyFromKeyboard);
        } else {
            // Case 2: When the keyboard sends Top row key and we remap it to
            // Function key.
            RecordFunctionKeyFromKeyCode(rewritten_state->key_code, InputKeyEventToFunctionKey::kTopRowAutoTranslated);
        }
    } else if (search_is_pressed && !search_in_rewritten_event) {
        if (key_event.key_code() >= VKEY_F1 && key_event.key_code() <= VKEY_F15) {
            // Case 3: When Function key + Search and get Function key with no
            // search.
            RecordFunctionKeyFromKeyCode(rewritten_state->key_code, InputKeyEventToFunctionKey::kDirectlyWithSearch);
        } else if (key_event.key_code() >= VKEY_0 && key_event.key_code() <= VKEY_9) {
            // Case 4: When search + digit results in Function key.
            RecordFunctionKeyFromKeyCode(rewritten_state->key_code, InputKeyEventToFunctionKey::kSearchDigitTranslated);
        } else {
            // Case 5: When the keyboard sends Top row key + search and we remap it
            // to Function key.
            RecordFunctionKeyFromKeyCode(rewritten_state->key_code, InputKeyEventToFunctionKey::kSearchTopRowTranslated);
        }
    }
}

bool SkipSearchKeyRemapping(EventRewriterAsh::Delegate* delegate, ui::mojom::SixPackShortcutModifier modifier)
{
    return delegate && delegate->IsSearchKeyAcceleratorReserved() && modifier == ui::mojom::SixPackShortcutModifier::kSearch;
}

bool ShouldBlockSixPackEventRewrite(
    EventRewriterAsh::Delegate* delegate, std::optional<ui::mojom::SixPackShortcutModifier> modifier, int flags, ui::KeyboardCode key_code, int device_id)
{
    if (!modifier.has_value()) {
        return true;
    }

    const auto matched_remapping_modifier = flags & EF_COMMAND_DOWN ? ui::mojom::SixPackShortcutModifier::kSearch : ui::mojom::SixPackShortcutModifier::kAlt;

    if (*modifier == ui::mojom::SixPackShortcutModifier::kNone) {
        delegate->NotifySixPackRewriteBlockedBySetting(key_code, matched_remapping_modifier, *modifier, device_id);
        return true;
    }

    const auto flag_mask = *modifier == ui::mojom::SixPackShortcutModifier::kSearch ? EF_COMMAND_DOWN : EF_ALT_DOWN;
...
bool MaybeRewriteSearchBasedShortcutToSixPackKeyAction(
    EventRewriterAsh::Delegate* delegate, const KeyEvent& key_event, EventRewriterAsh::MutableKeyState* state)
{
    EventRewriterAsh::MutableKeyState incoming = *state;
    CHECK(incoming.flags & EF_COMMAND_DOWN);
    bool strict = false;
    bool skip_search_key_remapping = delegate && delegate->IsSearchKeyAcceleratorReserved();
    ...
    static const KeyboardRemapping kOldInsertRemapping[] = {
        { // Search+Period -> Insert
            { EF_COMMAND_DOWN, VKEY_OEM_PERIOD }, { EF_NONE, DomCode::INSERT, DomKey::INSERT, VKEY_INSERT } },
    };
    ...
    if (::features::IsImprovedKeyboardShortcutsEnabled()) {
        ...
        if (!skip_search_key_remapping && RewriteWithKeyboardRemappings(kNewInsertRemapping, std::size(kNewInsertRemapping), incoming, state, strict)) {
            RecordSixPackEventRewrites(/*delegate=*/nullptr, key_event.type(), state->key_code,
                /*legacy_variant=*/false);
            return true;
        }

        // Test for the deprecated insert rewrite in order to show a notification.
        const ui::KeyboardCode deprecated_key = MatchedDeprecatedRemapping(kOldInsertRemapping, std::size(kOldInsertRemapping), incoming);
        ...
        if (!skip_search_key_remapping && RewriteWithKeyboardRemappings(kOldInsertRemapping, std::size(kOldInsertRemapping), incoming, state, strict)) {
    ...
    static const KeyboardRemapping kSixPackRemappings[] = { { // Search+BackSpace -> Delete
                                                                { EF_COMMAND_DOWN, VKEY_BACK }, { EF_NONE, DomCode::DEL, DomKey::DEL, VKEY_DELETE } },
        { // Search+Left -> Home
            { EF_COMMAND_DOWN, VKEY_LEFT }, { EF_NONE, DomCode::HOME, DomKey::HOME, VKEY_HOME } },
        { // Search+Up -> Prior (aka PageUp)
    ...
    if (!skip_search_key_remapping && RewriteWithKeyboardRemappings(kSixPackRemappings, std::size(kSixPackRemappings), incoming, state, strict)) {
        RecordSixPackEventRewrites(delegate, key_event.type(), state->key_code,
            /*legacy_variant=*/false);
        return true;
    }
...
// Six Pack (PageUp, PageDown, Home, End, Insert, Delete) key rewrites.
bool MaybeRewriteAltBasedShortcutToSixPackKeyAction(EventRewriterAsh::Delegate* delegate, const KeyEvent& key_event, EventRewriterAsh::MutableKeyState* state)
{
    EventRewriterAsh::MutableKeyState incoming = *state;
    CHECK(incoming.flags & EF_ALT_DOWN);
    static const KeyboardRemapping kLegacySixPackRemappings[] = { { // Alt+BackSpace -> Delete
                                                                      { EF_ALT_DOWN, VKEY_BACK }, { EF_NONE, DomCode::DEL, DomKey::DEL, VKEY_DELETE } },
        { // Control+Alt+Up -> Home
            { EF_ALT_DOWN | EF_CONTROL_DOWN, VKEY_UP }, { EF_NONE, DomCode::HOME, DomKey::HOME, VKEY_HOME } },
        { // Alt+Up -> Prior (aka PageUp)
        ...
        { // Alt+Down -> Next (aka PageDown)
            { EF_ALT_DOWN, VKEY_DOWN }, { EF_NONE, DomCode::PAGE_DOWN, DomKey::PAGE_DOWN, VKEY_NEXT } } };
    if (!::features::IsImprovedKeyboardShortcutsEnabled()) {
        if (RewriteWithKeyboardRemappings(kLegacySixPackRemappings, std::size(kLegacySixPackRemappings), incoming, state)) {
            RecordSixPackEventRewrites(delegate, key_event.type(), state->key_code,
                /*legacy_variant=*/true);
            return true;
        }
    } else {
        const ui::KeyboardCode deprecated_key = MatchedDeprecatedRemapping(kLegacySixPackRemappings, std::size(kLegacySixPackRemappings), incoming);
        if (deprecated_key != VKEY_UNKNOWN) {
            // If the key would have matched prior to being deprecated then notify
            // the delegate to show a notification.
            delegate->NotifyDeprecatedSixPackKeyRewrite(deprecated_key);
...
void MaybeRewriteKeyEventToSixPackKeyAction(
    EventRewriterAsh::Delegate* delegate, const KeyEvent& key_event, EventRewriterAsh::MutableKeyState* state, int device_id)
{
    EventRewriterAsh::MutableKeyState incoming = *state;
    ...
    for (const auto& map : kMergedSixPackRemappings) {
        ...
        const auto modifier_flag = delegate->GetShortcutModifierForSixPackKey(key_event.source_device_id(), map.result.key_code);

        if (ShouldBlockSixPackEventRewrite(delegate, modifier_flag, map.condition.flags, map.result.key_code, device_id)) {
            continue;
        }
        ...
        RecordSixPackEventRewrites(delegate, key_event.type(), state->key_code,
            /*legacy_variant=*/*modifier_flag == ui::mojom::SixPackShortcutModifier::kAlt);
        return;
    }
}

void NotifySixPackRewriteBlockedByFnKey(EventRewriterAsh::Delegate* delegate, const KeyEvent& key_event, EventRewriterAsh::MutableKeyState* state)
{
    for (const auto& map : kMergedSixPackRemappings) {
        if (MatchKeyboardRemapping(*state, map.condition)) {
            delegate->NotifySixPackRewriteBlockedByFnKey(map.result.key_code,
                (map.condition.flags & ui::EF_COMMAND_DOWN) ? mojom::SixPackShortcutModifier::kSearch : mojom::SixPackShortcutModifier::kAlt);
            return;
        }
    }
}
...
bool MaybeNotifyTopRowKeyBlockedByFnKey(
    EventRewriterAsh::Delegate* delegate, const KeyboardCapability* keyboard_capability, const ui::KeyEvent& key_event, int device_id)
{
    ...
    const auto* scan_code_vector_ptr = keyboard_capability->GetTopRowScanCodes(device_id);
    ...
    const auto& key_iter = base::ranges::find(scan_code_vector, key_event.scan_code());
...
void MaybeRewriteFunctionBasedShortcutToSixPackKeyAction(const KeyEvent& key_event, EventRewriterAsh::MutableKeyState* state)
{
    EventRewriterAsh::MutableKeyState incoming = *state;
    static const KeyboardRemapping kFunctionSixPackRemappings[] = { { // Fn+BackSpace -> Delete
                                                                        { EF_FUNCTION_DOWN, VKEY_BACK }, { EF_NONE, DomCode::DEL, DomKey::DEL, VKEY_DELETE } },
        { // Fn+Left -> Home
            { EF_FUNCTION_DOWN, VKEY_LEFT }, { EF_NONE, DomCode::HOME, DomKey::HOME, VKEY_HOME } },
        { // Fn+Up -> Prior (aka PageUp)
            { EF_FUNCTION_DOWN, VKEY_UP }, { EF_NONE, DomCode::PAGE_UP, DomKey::PAGE_UP, VKEY_PRIOR } },
        { // Fn+Right -> End
            { EF_FUNCTION_DOWN, VKEY_RIGHT }, { EF_NONE, DomCode::END, DomKey::END, VKEY_END } },
        { // Fn+Down -> Next (aka PageDown)
            { EF_FUNCTION_DOWN, VKEY_DOWN }, { EF_NONE, DomCode::PAGE_DOWN, DomKey::PAGE_DOWN, VKEY_NEXT } } };
...
bool ExtendedFkeyModifiersMatch(int flags, ui::mojom::ExtendedFkeysModifier modifier_flag)
{
    ...
    case ui::mojom::ExtendedFkeysModifier::kCtrlShift:
        return (flags & (EF_SHIFT_DOWN | EF_CONTROL_DOWN)) == (EF_SHIFT_DOWN | EF_CONTROL_DOWN);
    }
}

void RewriteExtendedFunctionKeys(EventRewriterAsh::Delegate* delegate, const KeyEvent& event, int device_id, EventRewriterAsh::MutableKeyState* state)
{
    EventRewriterAsh::MutableKeyState incoming = *state;
    static const KeyboardRemapping kExtendedFkeysRemappings[] = {
        { // Shift+F1 -> F11.
            { EF_SHIFT_DOWN, VKEY_F1 }, { EF_NONE, DomCode::F11, DomKey::F11, VKEY_F11 } },
        { // Alt+F1 -> F11.
            { EF_ALT_DOWN, VKEY_F1 }, { EF_NONE, DomCode::F11, DomKey::F11, VKEY_F11 } },
        { // Ctrl+Shift+F1 -> F11.
            { EF_SHIFT_DOWN | EF_CONTROL_DOWN, VKEY_F1 }, { EF_NONE, DomCode::F11, DomKey::F11, VKEY_F11 } },
        { // Shift+F2 -> F11.
            { EF_SHIFT_DOWN, VKEY_F2 }, { EF_NONE, DomCode::F12, DomKey::F12, VKEY_F12 } },
        { // Alt+F2 -> F12.
            { EF_ALT_DOWN, VKEY_F2 }, { EF_NONE, DomCode::F12, DomKey::F12, VKEY_F12 } },
        { // Ctrl+Shift+F2 -> F12.
            { EF_SHIFT_DOWN | EF_CONTROL_DOWN, VKEY_F2 }, { EF_NONE, DomCode::F12, DomKey::F12, VKEY_F12 } },
    };
    ...
    for (const auto& remapping : kExtendedFkeysRemappings) {
        ...
        const auto shortcut = delegate->GetExtendedFkeySetting(device_id, remapping.result.key_code);
        if (!shortcut || !ExtendedFkeyModifiersMatch(remapping.condition.flags, shortcut.value())) {
            continue;
        }
...
EventRewriterAsh::MutableKeyState::MutableKeyState(const KeyEvent* key_event)
    : MutableKeyState(key_event->flags(), key_event->code(), key_event->GetDomKey(), key_event->key_code())
{
}

///////////////////////////////////////////////////////////////////////////////

EventRewriterAsh::EventRewriterAsh(
    Delegate* delegate, KeyboardCapability* keyboard_capability, EventRewriter* sticky_keys_controller, bool privacy_screen_supported)
    : EventRewriterAsh(
        delegate, keyboard_capability, sticky_keys_controller, privacy_screen_supported, ash::input_method::InputMethodManager::Get()->GetImeKeyboard())
{
}

EventRewriterAsh::EventRewriterAsh(Delegate* delegate, KeyboardCapability* keyboard_capability, EventRewriter* sticky_keys_controller,
    bool privacy_screen_supported, ash::input_method::ImeKeyboard* ime_keyboard)
...
void EventRewriterAsh::ResetStateForTesting()
{
    ...
    pressed_modifier_latches_ = latched_modifier_latches_ = used_modifier_latches_ = EF_NONE;
}

void EventRewriterAsh::RewriteMouseButtonEventForTesting(const MouseEvent& event, const Continuation continuation)
{
    RewriteMouseButtonEvent(event, continuation);
}

EventDispatchDetails EventRewriterAsh::RewriteEvent(const Event& event, const Continuation continuation)
{
    if ((event.type() == EventType::kKeyPressed) || (event.type() == EventType::kKeyReleased)) {
        std::unique_ptr<Event> rewritten_event;
        const base::Time key_rewrite_start_time = base::Time::Now();
        DCHECK((&event)->AsKeyEvent());
        const EventRewriteStatus status = RewriteKeyEvent(*((&event)->AsKeyEvent()), &rewritten_event);
        RecordAutoRepeatUsageMetric(*((&event)->AsKeyEvent()), rewritten_event);
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
            "ChromeOS.Inputs.EventRewriter.KeyRewriteLatency", base::Time::Now() - key_rewrite_start_time, base::Microseconds(1), base::Milliseconds(100), 100);
        return RewriteKeyEventInContext(*((&event)->AsKeyEvent()), std::move(rewritten_event), status, continuation);
    }
    if ((event.type() == EventType::kMousePressed) || (event.type() == EventType::kMouseReleased)) {
        return RewriteMouseButtonEvent(static_cast<const MouseEvent&>(event), continuation);
    }
    if (event.type() == EventType::kMousewheel) {
        return RewriteMouseWheelEvent(static_cast<const MouseWheelEvent&>(event), continuation);
    }
    if ((event.type() == EventType::kTouchPressed) || (event.type() == EventType::kTouchReleased)) {
        return RewriteTouchEvent(static_cast<const TouchEvent&>(event), continuation);
    }
    if (event.IsScrollEvent()) {
        return RewriteScrollEvent(static_cast<const ScrollEvent&>(event), continuation);
    }

    return SendEvent(continuation, &event);
}

void EventRewriterAsh::BuildRewrittenKeyEvent(const KeyEvent& key_event, const MutableKeyState& state, std::unique_ptr<Event>* rewritten_event)
{
    auto key_event_ptr = std::make_unique<KeyEvent>(key_event.type(), state.key_code, state.code, state.flags, state.key, key_event.time_stamp());
...
// static
bool EventRewriterAsh::HasAssistantKeyOnKeyboard(const KeyboardDevice& keyboard_device, bool* has_assistant_key)
{
    std::unique_ptr<EventDeviceInfo> devinfo = KeyboardCapability::CreateEventDeviceInfoFromInputDevice(keyboard_device);
    if (!devinfo) {
        return false;
    }
...
bool EventRewriterAsh::RewriteModifierKeys(const KeyEvent& key_event, int device_id, MutableKeyState* state)
{
    DCHECK(key_event.type() == EventType::kKeyPressed || key_event.type() == EventType::kKeyReleased);
    ...
    case DomKey::ALT_GRAPH:
        ...
        if (IsISOLevel5ShiftUsedByCurrentInputMethod()) {
            if (incoming.code == DomCode::CAPS_LOCK) {
                ...
                remapped_key = GetRemappedKey(device_id, mojom::ModifierKey::kCapsLock, prefs::kLanguageRemapCapsLockKeyTo, delegate_);
            } else {
                characteristic_flag = EF_ALTGR_DOWN;
                remapped_key = GetSearchRemappedKey(delegate_, device_id, *keyboard_capability_);
            }
        }
    ...
    case DomCode::CONTROL_RIGHT:
        ...
        remapped_key = GetRemappedKey(device_id, mojom::ModifierKey::kControl, prefs::kLanguageRemapControlKeyTo, delegate_);
        break;
    case DomCode::ALT_RIGHT:
        ...
        if (key_event.GetDomKey() == DomKey::HANGUL_MODE && IsFirstPartyKoreanIME()) {
            break;
        }
        [[fallthrough]];
    case DomCode::ALT_LEFT:
        ...
        remapped_key = GetRemappedKey(device_id, mojom::ModifierKey::kAlt, prefs::kLanguageRemapAltKeyTo, delegate_);
        break;
    case DomCode::ESCAPE:
        remapped_key = GetRemappedKey(device_id, mojom::ModifierKey::kEscape, prefs::kLanguageRemapEscapeKeyTo, delegate_);
        break;
    case DomCode::BACKSPACE:
        remapped_key = GetRemappedKey(device_id, mojom::ModifierKey::kBackspace, prefs::kLanguageRemapBackspaceKeyTo, delegate_);
        break;
    case DomCode::LAUNCH_ASSISTANT:
        if (keyboard_capability_->HasRightAltKey(device_id)) {
            remapped_key = GetRemappedKey(device_id, mojom::ModifierKey::kRightAlt, "", delegate_);
            break;
        }
        remapped_key = GetRemappedKey(device_id, mojom::ModifierKey::kAssistant, prefs::kLanguageRemapAssistantKeyTo, delegate_);
        break;
    case DomCode::FN:
        characteristic_flag = EF_FUNCTION_DOWN;
        remapped_key = GetRemappedKey(device_id, mojom::ModifierKey::kFunction, "", delegate_);
        break;
    default:
        break;
    }
    ...
    bool non_modifier_to_modifier = !KeycodeConverter::IsDomKeyForModifier(incoming.key) && KeycodeConverter::IsDomKeyForModifier(state->key);
    if (key_event.type() == EventType::kKeyPressed) {
        ...
        if (non_modifier_to_modifier) {
            ...
            if (previous_non_modifier_latches_.contains(incoming.code)) {
                pressed_modifier_latches_ &= ~previous_non_modifier_latches_[incoming.code];
            }
    ...
    // crbug.com/775743).
    if (!ash::features::IsModifierSplitEnabled() && key_event.type() == EventType::kKeyPressed && state->key_code == VKEY_CAPITAL) {
        ...
        bool is_repeat = base::ranges::find(pressed_key_states_, std::tuple(key_event.code(), key_event.GetDomKey(), key_event.key_code()), [](auto entry) {
            return std::tuple(entry.first.code, entry.first.key, entry.first.key_code);
        }) != pressed_key_states_.end();
        if (!is_repeat) {
            ime_keyboard_->SetCapsLockEnabled(!ime_keyboard_->IsCapsLockEnabled());
        }
    }
    state->flags = (state->flags & ~EF_CAPS_LOCK_ON) | (ime_keyboard_->IsCapsLockEnabled() ? EF_CAPS_LOCK_ON : 0);
...
{
    return keyboard_capability_->GetDeviceType(device_id) == KeyboardCapability::DeviceType::kDeviceHotrodRemote;
}

int EventRewriterAsh::GetRemappedModifierMasks(int device_id, int original_flags) const
{
    int unmodified_flags = original_flags;
    int rewritten_flags = pressed_modifier_latches_ | latched_modifier_latches_;
    for (size_t i = 0; unmodified_flags && (i < std::size(kModifierRemappings)); ++i) {
        const ModifierRemapping* remapped_key = nullptr;
        ...
        case EF_MOD3_DOWN | EF_ALTGR_DOWN:
            if ((original_flags & EF_ALTGR_DOWN) && IsISOLevel5ShiftUsedByCurrentInputMethod()) {
                remapped_key = kModifierRemappingIsoLevel5ShiftMod3;
            }
            break;
        default:
        ...
        // here.
        if (!remapped_key && &kModifierRemappings[i] != kModifierRemappingIsoLevel5ShiftMod3) {
            const std::string pref_name = kModifierRemappings[i].pref_name ? kModifierRemappings[i].pref_name : "";
            remapped_key = GetRemappedKey(device_id, kModifierRemappings[i].remap_to, pref_name, delegate_);
        }
        if (remapped_key) {
...
bool EventRewriterAsh::ShouldRemapToRightClick(const MouseEvent& mouse_event, int flags, int* matched_mask, bool* matched_alt_deprecation) const
{
    *matched_mask = 0;
    *matched_alt_deprecation = false;
    ...
    const bool release_without_modifier = AreFlagsSet(flags, EF_LEFT_MOUSE_BUTTON) && pressed_as_right_button_device_ids_.count(mouse_event.source_device_id())
    ...
    bool use_search_key = base::FeatureList::IsEnabled(::ash::features::kUseSearchClickForRightClick) || ::features::IsDeprecateAltClickEnabled();
    ...
    if (ash::features::IsAltClickAndSixPackCustomizationEnabled()) {
        std::optional<ui::mojom::SimulateRightClickModifier> modifier = delegate_->GetRemapRightClickModifier(mouse_event.source_device_id());
        if (!modifier.has_value()) {
            return false;
        }
        ...
        if (search_click_down && use_alt_key) {
            delegate_->NotifyRightClickRewriteBlockedBySetting(ui::mojom::SimulateRightClickModifier::kSearch, *modifier);
            return false;
        } else if (alt_click_down && use_search_key) {
            delegate_->NotifyRightClickRewriteBlockedBySetting(ui::mojom::SimulateRightClickModifier::kAlt, *modifier);
            return false;
        } else if ((search_click_down || alt_click_down) && *modifier == ui::mojom::SimulateRightClickModifier::kNone) {
            delegate_->NotifyRightClickRewriteBlockedBySetting(
                search_click_down ? ui::mojom::SimulateRightClickModifier::kSearch : ui::mojom::SimulateRightClickModifier::kAlt, *modifier);
            return false;
        }
    }
    ...
    if (use_search_key) {
        if (search_click_down) {
            ...
            *matched_alt_deprecation
                = ((mouse_event.type() == EventType::kMousePressed) || pressed_as_right_button_device_ids_.count(mouse_event.source_device_id()))
...
EventRewriteStatus EventRewriterAsh::RewriteKeyEvent(const KeyEvent& key_event, std::unique_ptr<Event>* rewritten_event)
{
    int device_id = KeyboardDeviceIdEventRewriter::GetKeyboardDeviceId(key_event.source_device_id(), last_keyboard_device_id_, keyboard_capability_);
    if (device_id != ED_UNKNOWN_DEVICE) {
        last_keyboard_device_id_ = device_id;
    }

    // Drop repeated keys from Hotrod remote.
    if ((key_event.flags() & EF_IS_REPEAT) && (key_event.type() == EventType::kKeyPressed) && IsHotrodRemote(device_id) && key_event.key_code() != VKEY_BACK) {
    ...
    const bool should_record_modifier_key_press_metrics
        = !(key_event.flags() & EF_IS_REPEAT) && key_event.type() == EventType::kKeyPressed && !ash::features::IsKeyboardRewriterFixEnabled();
    if (should_record_modifier_key_press_metrics) {
        RecordModifierKeyPressedBeforeRemapping(*keyboard_capability_, device_id, key_event.code());
    }
    ...
    if (!(key_event.flags() & EF_FINAL)) {
        if (!ash::features::IsKeyboardRewriterFixEnabled()) {
            ...
            if (RewriteModifierKeys(key_event, device_id, &state)) {
                if (should_record_modifier_key_press_metrics) {
                    RecordModifierKeyPressedAfterRemapping(*keyboard_capability_, device_id, state.code, key_event.code(), state.key_code == VKEY_RIGHT_ALT);
                }
    ...
    if (delegate_ && delegate_->IsExtensionCommandRegistered(state.key_code, state.flags)) {
    ...
    if (sticky_keys_controller_) {
        ...
        if (status == EVENT_REWRITE_REWRITTEN || status == EVENT_REWRITE_DISPATCH_ANOTHER) {
            state.flags = output_event->flags();
        }
        if (status == EVENT_REWRITE_DISCARD) {
            return EVENT_REWRITE_DISCARD;
        }
        is_sticky_key_extension_command = delegate_ && delegate_->IsExtensionCommandRegistered(state.key_code, state.flags);
    }
    ...
    if (!is_sticky_key_extension_command && !(key_event.flags() & EF_FINAL)) {
        ...
        if (features::AreF11AndF12ShortcutsEnabled() && keyboard_capability_->IsChromeOSKeyboard(last_keyboard_device_id_)) {
            RewriteExtendedFunctionKeys(delegate_, key_event, last_keyboard_device_id_, &state);
        }

        RecordRewritingToFunctionKeys(key_event, &state);
    }

    if ((key_event.flags() == state.flags) && (key_event.key_code() == state.key_code) && (status == EVENT_REWRITE_CONTINUE)) {
        return EVENT_REWRITE_CONTINUE;
    }
...
// new API.
EventDispatchDetails EventRewriterAsh::RewriteMouseButtonEvent(const MouseEvent& mouse_event, const Continuation continuation)
{
    ...
    if ((mouse_event.type() == EventType::kMousePressed) || (mouse_event.type() == EventType::kMouseReleased)) {
    ...
    if (changed_button != EF_NONE) {
        static_cast<MouseEvent*>(rewritten_event.get())->set_changed_button_flags(changed_button);
    }

    EventDispatchDetails details = SendEventFinally(continuation, rewritten_event.get());
    if (status == EVENT_REWRITE_DISPATCH_ANOTHER && !details.dispatcher_destroyed) {
        // Here, we know that another event is a modifier key release event from
        // StickyKeysController.
        return SendStickyKeysReleaseEvents(std::move(rewritten_event), continuation);
    }
    return details;
}

EventDispatchDetails EventRewriterAsh::RewriteMouseWheelEvent(const MouseWheelEvent& wheel_event, const Continuation continuation)
{
    if (!sticky_keys_controller_) {
...
EventDispatchDetails EventRewriterAsh::RewriteTouchEvent(const TouchEvent& touch_event, const Continuation continuation)
{
...
EventDispatchDetails EventRewriterAsh::RewriteScrollEvent(const ScrollEvent& scroll_event, const Continuation continuation)
{
    if (!sticky_keys_controller_) {
...
void EventRewriterAsh::RewriteNumPadKeys(const KeyEvent& key_event, MutableKeyState* state)
{
    DCHECK(key_event.type() == EventType::kKeyPressed || key_event.type() == EventType::kKeyReleased);
    static const struct NumPadRemapping {
        KeyboardCode input_key_code;
        MutableKeyState result;
    } kNumPadRemappings[] = { { VKEY_DELETE, { EF_NONE, DomCode::NONE, DomKey::FromCharacter('.'), VKEY_DECIMAL } },
        { VKEY_INSERT, { EF_NONE, DomCode::NONE, DomKey::FromCharacter('0'), VKEY_NUMPAD0 } },
        { VKEY_END, { EF_NONE, DomCode::NONE, DomKey::FromCharacter('1'), VKEY_NUMPAD1 } },
        { VKEY_DOWN, { EF_NONE, DomCode::NONE, DomKey::FromCharacter('2'), VKEY_NUMPAD2 } },
        { VKEY_NEXT, { EF_NONE, DomCode::NONE, DomKey::FromCharacter('3'), VKEY_NUMPAD3 } },
        { VKEY_LEFT, { EF_NONE, DomCode::NONE, DomKey::FromCharacter('4'), VKEY_NUMPAD4 } },
        { VKEY_CLEAR, { EF_NONE, DomCode::NONE, DomKey::FromCharacter('5'), VKEY_NUMPAD5 } },
        { VKEY_RIGHT, { EF_NONE, DomCode::NONE, DomKey::FromCharacter('6'), VKEY_NUMPAD6 } },
        { VKEY_HOME, { EF_NONE, DomCode::NONE, DomKey::FromCharacter('7'), VKEY_NUMPAD7 } },
        { VKEY_UP, { EF_NONE, DomCode::NONE, DomKey::FromCharacter('8'), VKEY_NUMPAD8 } },
        { VKEY_PRIOR, { EF_NONE, DomCode::NONE, DomKey::FromCharacter('9'), VKEY_NUMPAD9 } } };
    for (const auto& map : kNumPadRemappings) {
        if (state->key_code == map.input_key_code) {
            if (KeycodeConverter::DomCodeToLocation(state->code) == DomKeyLocation::NUMPAD) {
                ApplyRemapping(map.result, state);
            }
            return;
        }
...
void EventRewriterAsh::RewriteExtendedKeys(const KeyEvent& key_event, MutableKeyState* state)
{
    DCHECK(key_event.type() == EventType::kKeyPressed || key_event.type() == EventType::kKeyReleased);
    MutableKeyState incoming = *state;
    ...
    // are deprecated.
    if ((!::features::IsImprovedKeyboardShortcutsEnabled()) && ((incoming.flags & (EF_COMMAND_DOWN | EF_ALT_DOWN)) == (EF_COMMAND_DOWN | EF_ALT_DOWN))) {
        // Allow Search to avoid rewriting extended keys.
        // For these, we only remove the EF_COMMAND_DOWN flag.
        static const KeyboardRemapping::Condition kAvoidRemappings[] = { { // Alt+Backspace
    ...
    if (ash::features::IsAltClickAndSixPackCustomizationEnabled() && incoming.flags & (EF_COMMAND_DOWN | EF_ALT_DOWN)) {
        MaybeRewriteKeyEventToSixPackKeyAction(delegate_, key_event, state, last_keyboard_device_id_);
        return;
    }

    // TODO(b/279503977): Remove block once `kAltClickAndSixPackCustomization`
    // flag is enabled by default.
    if (incoming.flags & EF_COMMAND_DOWN && MaybeRewriteSearchBasedShortcutToSixPackKeyAction(delegate_, key_event, state)) {
        return;
    }

    // TODO(b/279503977): Remove block once `kAltClickAndSixPackCustomization`
    // flag is enabled by default.
    if (incoming.flags & EF_ALT_DOWN && is_alt_down_remapping_enabled_ && MaybeRewriteAltBasedShortcutToSixPackKeyAction(delegate_, key_event, state)) {
        return;
    }
}

void EventRewriterAsh::RewriteFunctionKeys(const KeyEvent& key_event, int device_id, MutableKeyState* state)
{
    CHECK(key_event.type() == EventType::kKeyPressed || key_event.type() == EventType::kKeyReleased);
    ...
    if (MaybeNotifyTopRowKeyBlockedByFnKey(delegate_, keyboard_capability_, key_event, device_id)) {
        return;
    }

    KeyboardCapability::KeyboardTopRowLayout layout = keyboard_capability_->GetTopRowLayout(device_id);

    // The modifier to base rewriting on depends on if the keyboard uses an FN key
    // or not.
    const EventFlags flip_rewrite_modifier = keyboard_capability_->HasFunctionKey(device_id) ? EF_FUNCTION_DOWN : EF_COMMAND_DOWN;
    const bool flip_modifier_is_pressed = (state->flags & flip_rewrite_modifier) != 0;
    const bool should_flip_top_row_mapping = ShouldRewriteMetaTopRowKeyComboEvents(delegate_, device_id) && flip_modifier_is_pressed;
    if (layout == KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutCustom) {
        if (RewriteTopRowKeysForCustomLayout(key_event, device_id, should_flip_top_row_mapping, flip_rewrite_modifier, state)) {
            return;
        }
    } else if (layout == KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutWilco
        || layout == KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutDrallion) {
        if (RewriteTopRowKeysForLayoutWilco(key_event, device_id, should_flip_top_row_mapping, flip_rewrite_modifier, state, layout)) {
            return;
        }
    } else {
        if (RewriteTopRowKeysForStandardLayouts(
                key_event, device_id, should_flip_top_row_mapping, flip_rewrite_modifier, flip_modifier_is_pressed, state, layout)) {
            return;
        }
    }
    ...
    if (state->flags & EF_COMMAND_DOWN) {
        ...
        if (!::features::IsImprovedKeyboardShortcutsEnabled()) {
            static const SearchToFunctionMap kNumberKeysToFkeys[] = { { DomCode::DIGIT1, { EF_NONE, DomCode::F1, DomKey::F1, VKEY_F1 } },
                { DomCode::DIGIT2, { EF_NONE, DomCode::F2, DomKey::F2, VKEY_F2 } }, { DomCode::DIGIT3, { EF_NONE, DomCode::F3, DomKey::F3, VKEY_F3 } },
                { DomCode::DIGIT4, { EF_NONE, DomCode::F4, DomKey::F4, VKEY_F4 } }, { DomCode::DIGIT5, { EF_NONE, DomCode::F5, DomKey::F5, VKEY_F5 } },
                { DomCode::DIGIT6, { EF_NONE, DomCode::F6, DomKey::F6, VKEY_F6 } }, { DomCode::DIGIT7, { EF_NONE, DomCode::F7, DomKey::F7, VKEY_F7 } },
                { DomCode::DIGIT8, { EF_NONE, DomCode::F8, DomKey::F8, VKEY_F8 } }, { DomCode::DIGIT9, { EF_NONE, DomCode::F9, DomKey::F9, VKEY_F9 } },
                { DomCode::DIGIT0, { EF_NONE, DomCode::F10, DomKey::F10, VKEY_F10 } }, { DomCode::MINUS, { EF_NONE, DomCode::F11, DomKey::F11, VKEY_F11 } },
...
int EventRewriterAsh::RewriteModifierClick(const MouseEvent& mouse_event, int* flags)
{
    ...
    if (ShouldRemapToRightClick(mouse_event, *flags, &matched_mask, &matched_alt_deprecation)) {
        ...
        *flags |= EF_RIGHT_MOUSE_BUTTON;
        if (mouse_event.type() == EventType::kMousePressed) {
            pressed_as_right_button_device_ids_.insert(mouse_event.source_device_id());
            if (matched_mask == kSearchLeftButton) {
                base::RecordAction(base::UserMetricsAction("SearchClickMappedToRightClick"));
...
EventDispatchDetails EventRewriterAsh::RewriteKeyEventInContext(
    const KeyEvent& key_event, std::unique_ptr<Event> rewritten_event, EventRewriteStatus status, const Continuation continuation)
{
    ...
    if (ash::features::IsKeyboardRewriterFixEnabled()) {
        ...
        MutableKeyState key_state(rewritten_event ? rewritten_event->AsKeyEvent() : &key_event);
        auto it = pressed_physical_keys_.find(key);
        bool is_rewritten_differently = it != pressed_physical_keys_.end()
            && (it->second.code != key_state.code || it->second.key != key_state.key || it->second.key_code != key_state.key_code);
        ...
        if (key_event.type() == EventType::kKeyPressed) {
            ...
            if (is_rewritten_differently) {
                auto dispatched_event = std::make_unique<KeyEvent>(ui::EventType::kKeyReleased, it->second.key_code, it->second.code,
                    key_event.flags() & ~it->second.flags, it->second.key, key_event.time_stamp());
            ...
            if (is_rewritten_differently) {
                ...
                auto rewritten_key_event = std::make_unique<KeyEvent>(ui::EventType::kKeyReleased, it->second.key_code, it->second.code,
                    key_event.flags() & ~key_state.flags, it->second.key, key_event.time_stamp());
        ...
        if (status == EventRewriteStatus::EVENT_REWRITE_DISPATCH_ANOTHER && !details.dispatcher_destroyed) {
            return SendStickyKeysReleaseEvents(std::move(rewritten_event), continuation);
        }
        return details;
    }
    ...
    auto key_state_comparator = [&current_key_state](const std::pair<MutableKeyState, MutableKeyState>& key_state) {
        return (current_key_state.code == key_state.first.code) && (current_key_state.key == key_state.first.key)
    ...
    if (key_event.type() == EventType::kKeyPressed) {
        current_key_state = MutableKeyState(rewritten_event ? static_cast<const KeyEvent*>(rewritten_event.get()) : &key_event);
    ...
    if (mapped_flag != EF_NONE) {
        ...
        int event_flags = rewritten_event ? rewritten_event->flags() : key_event.flags();
        Event::Properties properties = (rewritten_event && rewritten_event->properties()) ? *rewritten_event->properties() : Event::Properties();
        ...
        while (key_state_iter != pressed_key_states_.end() && !details.dispatcher_destroyed) {
            const bool is_rewritten = (key_state_iter->first.key != key_state_iter->second.key);
            const bool flag_affected = key_state_iter->second.flags & mapped_flag;
            const bool should_release = key_state_iter->second.key == current_key || (flag_affected && is_rewritten);

            if (should_release) {
                // If the key should be released, create a key event for it.
                auto dispatched_event = std::make_unique<KeyEvent>(key_event.type(), key_state_iter->first.key_code, key_state_iter->first.code, event_flags,
...
bool EventRewriterAsh::RewriteTopRowKeysForCustomLayout(
    const KeyEvent& key_event, int device_id, bool should_flip_top_row_mapping, EventFlags flip_rewrite_modifier, EventRewriterAsh::MutableKeyState* state)
{
    ...
    const auto* scan_code_vector_ptr = keyboard_capability_->GetTopRowScanCodes(device_id);
    if (!scan_code_vector_ptr || scan_code_vector_ptr->empty()) {
        LOG(WARNING) << "Found no top row key mapping for device " << device_id;
        return false;
    }
    ...
    if (is_action_key) {
        if (should_flip_top_row_mapping != ForceTopRowAsFunctionKeys(device_id)) {
            ApplyRemapping(kCustomTopRowLayoutFKeys[std::distance(scan_code_vector.begin(), key_iter)], state);
        }
...
bool EventRewriterAsh::RewriteTopRowKeysForLayoutWilco(const KeyEvent& key_event, int device_id, bool should_flip_top_row_mapping,
    EventFlags flip_rewrite_modifier, MutableKeyState* state, KeyboardCapability::KeyboardTopRowLayout layout)
{
    ...
    static const KeyboardRemapping kFnkeysToActionKeys[] = {
        { { EF_NONE, VKEY_F1 }, { EF_NONE, DomCode::BROWSER_BACK, DomKey::BROWSER_BACK, VKEY_BROWSER_BACK } },
        { { EF_NONE, VKEY_F2 }, { EF_NONE, DomCode::BROWSER_REFRESH, DomKey::BROWSER_REFRESH, VKEY_BROWSER_REFRESH } },
        // Map F3 to VKEY_ZOOM + EF_NONE == toggle full screen:
        { { EF_NONE, VKEY_F3 }, { EF_NONE, DomCode::ZOOM_TOGGLE, DomKey::ZOOM_TOGGLE, VKEY_ZOOM } },
        // Map F4 to VKEY_MEDIA_LAUNCH_APP1 + EF_NONE == overview:
        { { EF_NONE, VKEY_F4 }, { EF_NONE, DomCode::F4, DomKey::F4, VKEY_MEDIA_LAUNCH_APP1 } },
        { { EF_NONE, VKEY_F5 }, { EF_NONE, DomCode::BRIGHTNESS_DOWN, DomKey::BRIGHTNESS_DOWN, VKEY_BRIGHTNESS_DOWN } },
        { { EF_NONE, VKEY_F6 }, { EF_NONE, DomCode::BRIGHTNESS_UP, DomKey::BRIGHTNESS_UP, VKEY_BRIGHTNESS_UP } },
        { { EF_NONE, VKEY_F7 }, { EF_NONE, DomCode::VOLUME_MUTE, DomKey::AUDIO_VOLUME_MUTE, VKEY_VOLUME_MUTE } },
        { { EF_NONE, VKEY_F8 }, { EF_NONE, DomCode::VOLUME_DOWN, DomKey::AUDIO_VOLUME_DOWN, VKEY_VOLUME_DOWN } },
        { { EF_NONE, VKEY_F9 }, { EF_NONE, DomCode::VOLUME_UP, DomKey::AUDIO_VOLUME_UP, VKEY_VOLUME_UP } },
    ...
    static const KeyboardRemapping kActionToFnKeys[] = {
        { { EF_NONE, VKEY_BROWSER_BACK }, { EF_NONE, DomCode::F1, DomKey::F1, VKEY_F1 } },
        { { EF_NONE, VKEY_BROWSER_REFRESH }, { EF_NONE, DomCode::F2, DomKey::F2, VKEY_F2 } },
        { { EF_NONE, VKEY_MEDIA_LAUNCH_APP1 }, { EF_NONE, DomCode::F4, DomKey::F4, VKEY_F4 } },
        { { EF_NONE, VKEY_BRIGHTNESS_DOWN }, { EF_NONE, DomCode::F5, DomKey::F5, VKEY_F5 } },
        { { EF_NONE, VKEY_BRIGHTNESS_UP }, { EF_NONE, DomCode::F6, DomKey::F6, VKEY_F6 } },
        { { EF_NONE, VKEY_VOLUME_MUTE }, { EF_NONE, DomCode::F7, DomKey::F7, VKEY_F7 } },
        { { EF_NONE, VKEY_VOLUME_DOWN }, { EF_NONE, DomCode::F8, DomKey::F8, VKEY_F8 } },
        ...
        { { EF_CONTROL_DOWN, VKEY_ZOOM }, { EF_NONE, DomCode::F12, DomKey::F12, VKEY_F12 } },
        { { EF_NONE, VKEY_ZOOM }, { EF_NONE, DomCode::F3, DomKey::F3, VKEY_F3 } },
        // VKEY_PRIVACY_SCREEN_TOGGLE shares a key with F12 on Drallion.
        { { EF_NONE, VKEY_PRIVACY_SCREEN_TOGGLE }, { EF_NONE, DomCode::F12, DomKey::F12, VKEY_F12 } },
    };
    MutableKeyState incoming_with_modifier_removed_if_neccessary = *state;
    if (should_flip_top_row_mapping) {
        incoming_with_modifier_removed_if_neccessary.flags &= ~flip_rewrite_modifier;
    }
    ...
    if ((state->key_code >= VKEY_F1) && (state->key_code <= VKEY_F12)) {
        ...
        if (should_flip_top_row_mapping) {
            ...
            if (layout == KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutDrallion && state->key_code == VKEY_F12) {
            ...
            return RewriteWithKeyboardRemappings(kFnkeysToActionKeys, std::size(kFnkeysToActionKeys), incoming_with_modifier_removed_if_neccessary, state);
        }
        return true;
    } else if (IsKeyCodeInMappings(state->key_code, kActionToFnKeys, std::size(kActionToFnKeys))) {
        ...
        if (should_flip_top_row_mapping != ForceTopRowAsFunctionKeys(device_id)) {
            ...
            if (layout == KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutDrallion
            ...
            return RewriteWithKeyboardRemappings(kActionToFnKeys, std::size(kActionToFnKeys), incoming_with_modifier_removed_if_neccessary, state);
        }
        // Remap Privacy Screen Toggle to F12 on Drallion devices that do not have
        // privacy screens.
        if (layout == KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutDrallion && !privacy_screen_supported_
...
bool EventRewriterAsh::RewriteTopRowKeysForStandardLayouts(const KeyEvent& key_event, int device_id, bool should_flip_top_row_mapping,
    EventFlags flip_rewrite_modifier, bool rewrite_modifier_is_pressed, MutableKeyState* state, KeyboardCapability::KeyboardTopRowLayout layout)
{
    ...
    static const KeyboardRemapping kFkeysToSystemKeys1[] = {
        { { EF_NONE, VKEY_F1 }, { EF_NONE, DomCode::BROWSER_BACK, DomKey::BROWSER_BACK, VKEY_BROWSER_BACK } },
        { { EF_NONE, VKEY_F2 }, { EF_NONE, DomCode::BROWSER_FORWARD, DomKey::BROWSER_FORWARD, VKEY_BROWSER_FORWARD } },
        { { EF_NONE, VKEY_F3 }, { EF_NONE, DomCode::BROWSER_REFRESH, DomKey::BROWSER_REFRESH, VKEY_BROWSER_REFRESH } },
        { { EF_NONE, VKEY_F4 }, { EF_NONE, DomCode::ZOOM_TOGGLE, DomKey::ZOOM_TOGGLE, VKEY_ZOOM } },
        { { EF_NONE, VKEY_F5 }, { EF_NONE, DomCode::SELECT_TASK, DomKey::LAUNCH_MY_COMPUTER, VKEY_MEDIA_LAUNCH_APP1 } },
        { { EF_NONE, VKEY_F6 }, { EF_NONE, DomCode::BRIGHTNESS_DOWN, DomKey::BRIGHTNESS_DOWN, VKEY_BRIGHTNESS_DOWN } },
        { { EF_NONE, VKEY_F7 }, { EF_NONE, DomCode::BRIGHTNESS_UP, DomKey::BRIGHTNESS_UP, VKEY_BRIGHTNESS_UP } },
        { { EF_NONE, VKEY_F8 }, { EF_NONE, DomCode::VOLUME_MUTE, DomKey::AUDIO_VOLUME_MUTE, VKEY_VOLUME_MUTE } },
        { { EF_NONE, VKEY_F9 }, { EF_NONE, DomCode::VOLUME_DOWN, DomKey::AUDIO_VOLUME_DOWN, VKEY_VOLUME_DOWN } },
        { { EF_NONE, VKEY_F10 }, { EF_NONE, DomCode::VOLUME_UP, DomKey::AUDIO_VOLUME_UP, VKEY_VOLUME_UP } },
    };
    ...
    static const KeyboardRemapping kFkeysToSystemKeys2[] = {
        ...
        { { EF_NONE, VKEY_F2 }, { EF_NONE, DomCode::BROWSER_REFRESH, DomKey::BROWSER_REFRESH, VKEY_BROWSER_REFRESH } },
        { { EF_NONE, VKEY_F3 }, { EF_NONE, DomCode::ZOOM_TOGGLE, DomKey::ZOOM_TOGGLE, VKEY_ZOOM } },
        { { EF_NONE, VKEY_F4 }, { EF_NONE, DomCode::SELECT_TASK, DomKey::LAUNCH_MY_COMPUTER, VKEY_MEDIA_LAUNCH_APP1 } },
        { { EF_NONE, VKEY_F5 }, { EF_NONE, DomCode::BRIGHTNESS_DOWN, DomKey::BRIGHTNESS_DOWN, VKEY_BRIGHTNESS_DOWN } },
        { { EF_NONE, VKEY_F6 }, { EF_NONE, DomCode::BRIGHTNESS_UP, DomKey::BRIGHTNESS_UP, VKEY_BRIGHTNESS_UP } },
        { { EF_NONE, VKEY_F7 }, { EF_NONE, DomCode::MEDIA_PLAY_PAUSE, DomKey::MEDIA_PLAY_PAUSE, VKEY_MEDIA_PLAY_PAUSE } },
        { { EF_NONE, VKEY_F8 }, { EF_NONE, DomCode::VOLUME_MUTE, DomKey::AUDIO_VOLUME_MUTE, VKEY_VOLUME_MUTE } },
        { { EF_NONE, VKEY_F9 }, { EF_NONE, DomCode::VOLUME_DOWN, DomKey::AUDIO_VOLUME_DOWN, VKEY_VOLUME_DOWN } },
        { { EF_NONE, VKEY_F10 }, { EF_NONE, DomCode::VOLUME_UP, DomKey::AUDIO_VOLUME_UP, VKEY_VOLUME_UP } },
    };
    ...
    const bool should_rewrite_to_action_keys = (ForceTopRowAsFunctionKeys(device_id) == should_flip_top_row_mapping);
    if (should_rewrite_to_action_keys) {
        ...
        if (RewriteWithKeyboardRemappings(mapping, mappingSize, incoming_without_flip_modifier, state)) {
...
EventDispatchDetails EventRewriterAsh::SendStickyKeysReleaseEvents(std::unique_ptr<Event> rewritten_event, const Continuation continuation)
{
    EventDispatchDetails details;
    ...
    while (sticky_keys_controller_ && !details.dispatcher_destroyed) {
        ...
        EventRewriteStatus status = sticky_keys_controller_->NextDispatchEvent(*last_sent_event, &new_event);
