...
constexpr char kKbdDefaultCustomTopRowLayout[] = "01 02 03 04 05 06 07 08 09 0a 0b 0c 0d 0e 0f";
...
KeyboardDevice KeyboardDeviceFromCapabilities(int device_id, const DeviceCapabilities& capabilities)
{
    EventDeviceInfo device_info = {};
    CapabilitiesToDeviceInfo(capabilities, &device_info);
    return KeyboardDevice { InputDevice(device_id, device_info.device_type(), device_info.name(), device_info.phys(), base::FilePath(capabilities.path),
...
std::optional<uint32_t> GetEvdevKeyCodeForScanCode(const base::ScopedFD& fd, uint32_t scancode)
{
    switch (scancode) {
...
public:
    ...
    // fake udev.
    void AddFakeKeyboard(const KeyboardDevice& fake_keyboard, const std::string& layout, bool has_custom_top_row = false)
    {
        ...
        fake_udev_.AddFakeDevice(fake_keyboard.name, fake_keyboard.sys_path.value(),
            ...
            /*devtype=*/std::nullopt, std::move(sysfs_attributes), std::move(sysfs_properties));
    }
...
public:
    ...
    void SetUp() override
    {
        keyboard_capability_ = std::make_unique<KeyboardCapability>(base::BindRepeating(&GetEvdevKeyCodeForScanCode));
    ...
    KeyboardDevice AddFakeKeyboardInfoToKeyboardCapability(
        int device_id, DeviceCapabilities capabilities, KeyboardCapability::DeviceType device_type, KeyboardCapability::KeyboardTopRowLayout top_row_layout)
    {
        ...
        KeyboardDevice fake_keyboard = KeyboardDeviceFromCapabilities(device_id, capabilities);

        keyboard_capability_->SetKeyboardInfoForTesting(fake_keyboard, std::move(keyboard_info));
        fake_keyboard_devices_.push_back(fake_keyboard);
        DeviceDataManagerTestApi().SetKeyboardDevices(fake_keyboard_devices_);
...
class KeyboardCapabilityTest : public KeyboardCapabilityTestBase, public testing::WithParamInterface<bool> {
public:
    void SetUp() override
    {
        modifier_split_feature_list_ = std::make_unique<base::test::ScopedFeatureList>();
        if (GetParam()) {
            modifier_split_feature_list_->InitAndEnableFeature(ash::features::kModifierSplit);
        } else {
            modifier_split_feature_list_->InitAndDisableFeature(ash::features::kModifierSplit);
        }
...
TEST_P(KeyboardCapabilityTest, TestGetMappedFKeyIfExists)
{
    ...
    for (const auto& [key_code, f_key] : kLayout1TopRowKeyToFKeyMap) {
        EXPECT_EQ(f_key, keyboard_capability_->GetMappedFKeyIfExists(key_code, fake_keyboard).value());
    }
    // VKEY_MEDIA_PLAY_PAUSE key is not a top row key for layout1.
    EXPECT_FALSE(keyboard_capability_->GetMappedFKeyIfExists(KeyboardCode::VKEY_MEDIA_PLAY_PAUSE, fake_keyboard).has_value());
    ...
    EXPECT_FALSE(keyboard_capability_->GetMappedFKeyIfExists(KeyboardCode::VKEY_BROWSER_FORWARD, fake_keyboard).has_value());

    // Replace by a fake wilco keyboard.
    fake_keyboard_manager_->RemoveAllDevices();
    fake_keyboard_manager_->AddFakeKeyboard(fake_keyboard, kKbdTopRowLayoutWilcoTag);
    ...
    fake_keyboard_manager_->RemoveAllDevices();
    fake_keyboard_manager_->AddFakeKeyboard(fake_keyboard, kKbdTopRowLayoutDrallionTag);
    for (const auto& [key_code, f_key] : kLayoutWilcoDrallionTopRowKeyToFKeyMap) {
        EXPECT_EQ(f_key, keyboard_capability_->GetMappedFKeyIfExists(key_code, fake_keyboard).value());
    }
...
TEST_P(KeyboardCapabilityTest, TestGetMetaKey)
{
    ...
    // Provide specific keyboard. Launcher button depends on if the keyboard is
    // layout2 type.
    EXPECT_EQ(mojom::MetaKey::kSearch, keyboard_capability_->GetMetaKey(fake_keyboard1));
    // Do not provide specific keyboard. Launcher button depends on if any one
    // of the keyboards is layout2 type.
    EXPECT_EQ(mojom::MetaKey::kSearch, keyboard_capability_->GetMetaKeyToDisplay());
    ...
    EXPECT_EQ(mojom::MetaKey::kSearch, keyboard_capability_->GetMetaKey(fake_keyboard1));
    EXPECT_EQ(mojom::MetaKey::kLauncher, keyboard_capability_->GetMetaKey(fake_keyboard2));
    EXPECT_EQ(mojom::MetaKey::kLauncher, keyboard_capability_->GetMetaKeyToDisplay());
}
...
TEST_P(KeyboardCapabilityTest, TestGetMetaKey_ExternalChromeOS)
{
    ...
    fake_keyboard1.sys_path = base::FilePath("path1");
    fake_keyboard_manager_->AddFakeKeyboard(fake_keyboard1, kKbdTopRowLayout1Tag);
    EXPECT_EQ(mojom::MetaKey::kLauncher, keyboard_capability_->GetMetaKey(fake_keyboard1));
    EXPECT_EQ(mojom::MetaKey::kLauncher, keyboard_capability_->GetMetaKeyToDisplay());
...
TEST_P(KeyboardCapabilityTest, TestGetMetaKey_ExternalNonChromeOS)
{
    KeyboardDevice fake_keyboard1(
        ...
        /*name=*/"Keyboard1");
    fake_keyboard1.sys_path = base::FilePath("path1");
    fake_keyboard_manager_->AddFakeKeyboard(fake_keyboard1, kKbdTopRowLayoutUnspecified);
    EXPECT_EQ(mojom::MetaKey::kExternalMeta, keyboard_capability_->GetMetaKey(fake_keyboard1));
    EXPECT_EQ(
        ash::features::IsModifierSplitEnabled() ? mojom::MetaKey::kLauncherRefresh : mojom::MetaKey::kLauncher, keyboard_capability_->GetMetaKeyToDisplay());
    ...
    KeyboardDevice internal_keyboard(
        ...
        /*name=*/"Keyboard2");
    fake_keyboard1.sys_path = base::FilePath("path2");
    fake_keyboard_manager_->AddFakeKeyboard(internal_keyboard, kKbdTopRowLayout2Tag);
    EXPECT_EQ(mojom::MetaKey::kLauncher, keyboard_capability_->GetMetaKey(internal_keyboard));
    EXPECT_EQ(mojom::MetaKey::kLauncher, keyboard_capability_->GetMetaKeyToDisplay());
}
...
TEST_P(KeyboardCapabilityTest, TestGetMetaKey_SplitModifierKeyboard)
{
    if (!ash::features::IsModifierSplitEnabled()) {
        GTEST_SKIP() << "This test is only applicable with split modifier feature enabled.";
    }

    const KeyboardDevice split_modifier_keyboard = AddFakeKeyboardInfoToKeyboardCapability(kDeviceId1, kSplitModifierKeyboard,
        KeyboardCapability::DeviceType::kDeviceInternalKeyboard, KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutCustom);
    EXPECT_EQ(mojom::MetaKey::kLauncherRefresh, keyboard_capability_->GetMetaKey(split_modifier_keyboard));
    EXPECT_EQ(mojom::MetaKey::kLauncherRefresh, keyboard_capability_->GetMetaKeyToDisplay());
}
...
TEST_P(KeyboardCapabilityTest, TestRemoveDevicesFromList)
{
    const KeyboardDevice input_device1 = AddFakeKeyboardInfoToKeyboardCapability(
        kDeviceId1, kEveKeyboard, KeyboardCapability::DeviceType::kDeviceInternalKeyboard, KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayout2);
    const KeyboardDevice input_device2 = AddFakeKeyboardInfoToKeyboardCapability(kDeviceId2, kHpUsbKeyboard,
        KeyboardCapability::DeviceType::kDeviceExternalGenericKeyboard, KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayout1);
...
{
    base::CommandLine::ForCurrentProcess()->AppendSwitch(ash::switches::kRevenBranding);

    KeyboardDevice internal_keyboard(
        /*id=*/2, /*type=*/InputDeviceType::INPUT_DEVICE_INTERNAL,
        /*name=*/"internal keyboard");
    internal_keyboard.sys_path = base::FilePath("path1");
    fake_keyboard_manager_->AddFakeKeyboard(internal_keyboard, kKbdTopRowLayoutUnspecified);

    EXPECT_EQ(KeyboardCapability::DeviceType::kDeviceInternalRevenKeyboard, keyboard_capability_->GetDeviceType(internal_keyboard));
}
...
TEST_P(KeyboardCapabilityTest, TestHasGlobeKey)
{
    ...
    fake_keyboard_manager_->AddFakeKeyboard(bluetooth_keyboard, kKbdTopRowLayoutUnspecified);
    ...
    fake_keyboard_manager_->AddFakeKeyboard(internal_keyboard_layout, kKbdTopRowLayout1Tag);
    ...
    fake_keyboard_manager_->AddFakeKeyboard(bluetooth_keyboard_layout1, kKbdTopRowLayout1Tag);
    ...
    fake_keyboard_manager_->AddFakeKeyboard(bluetooth_keyboard_layout2, kKbdTopRowLayout2Tag);
    ...
    fake_keyboard_manager_->AddFakeKeyboard(bluetooth_keyboard_layout_custom, kKbdDefaultCustomTopRowLayout,
        /*has_custom_top_row=*/true);
    EXPECT_TRUE(keyboard_capability_->HasGlobeKey(bluetooth_keyboard_layout_custom));
    ...
    fake_keyboard_manager_->AddFakeKeyboard(bluetooth_keyboard_wilco, kKbdTopRowLayoutWilcoTag);
    ...
    fake_keyboard_manager_->AddFakeKeyboard(bluetooth_keyboard_drallion, kKbdTopRowLayoutDrallionTag);
...
TEST_P(KeyboardCapabilityTest, TestHasCalculatorKey)
{
    KeyboardDevice internal_keyboard(
        ...
        /*name=*/"Keyboard1");
    internal_keyboard.sys_path = base::FilePath("path1");
    fake_keyboard_manager_->AddFakeKeyboard(internal_keyboard, kKbdTopRowLayout1Tag);
    ...
    fake_keyboard_manager_->AddFakeKeyboard(external_keyboard, kKbdTopRowLayoutUnspecified);
...
class ModifierKeyTest
    : public KeyboardCapabilityTestBase,
      public testing::WithParamInterface<
          std::tuple<DeviceCapabilities, KeyboardCapability::DeviceType, KeyboardCapability::KeyboardTopRowLayout, std::vector<mojom::ModifierKey>>> { };

// Tests that the given `DeviceCapabilities` and
// `KeyboardCapability::DeviceType` combo generates the given set of
// modifier keys.
INSTANTIATE_TEST_SUITE_P(All, ModifierKeyTest,
    testing::ValuesIn(
        std::vector<std::tuple<DeviceCapabilities, KeyboardCapability::DeviceType, KeyboardCapability::KeyboardTopRowLayout, std::vector<mojom::ModifierKey>>> {
            { kDrobitKeyboard, KeyboardCapability::DeviceType::kDeviceInternalKeyboard, KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutCustom,
                { mojom::ModifierKey::kBackspace, mojom::ModifierKey::kControl, mojom::ModifierKey::kMeta, mojom::ModifierKey::kEscape,
                    mojom::ModifierKey::kAlt } },
            { kLogitechKeyboardK120, KeyboardCapability::DeviceType::kDeviceExternalGenericKeyboard,
                KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayout1,
                { mojom::ModifierKey::kBackspace, mojom::ModifierKey::kControl, mojom::ModifierKey::kMeta, mojom::ModifierKey::kEscape,
                    mojom::ModifierKey::kAlt, mojom::ModifierKey::kCapsLock } },
            { kHpUsbKeyboard, KeyboardCapability::DeviceType::kDeviceExternalGenericKeyboard, KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayout1,
                { mojom::ModifierKey::kBackspace, mojom::ModifierKey::kControl, mojom::ModifierKey::kMeta, mojom::ModifierKey::kEscape,
                    mojom::ModifierKey::kAlt, mojom::ModifierKey::kCapsLock } },
            // Tests that an external chromeos keyboard correctly omits capslock.
            { kHpUsbKeyboard, KeyboardCapability::DeviceType::kDeviceExternalChromeOsKeyboard, KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutCustom,
                { mojom::ModifierKey::kBackspace, mojom::ModifierKey::kControl, mojom::ModifierKey::kMeta, mojom::ModifierKey::kEscape,
                    mojom::ModifierKey::kAlt } } }));

TEST_P(ModifierKeyTest, TestGetModifierKeys)
{
    auto [capabilities, device_type, top_row_layout, expected_modifier_keys] = GetParam();

    const KeyboardDevice test_keyboard = AddFakeKeyboardInfoToKeyboardCapability(kDeviceId1, capabilities, device_type, top_row_layout);
...
TEST_P(KeyboardCapabilityTest, TestGetModifierKeysForSplitModifierKeyboard)
{
    ...
    const KeyboardDevice test_keyboard = AddFakeKeyboardInfoToKeyboardCapability(kDeviceId1, kSplitModifierKeyboard,
        KeyboardCapability::DeviceType::kDeviceInternalKeyboard, KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutCustom);
    auto modifier_keys = keyboard_capability_->GetModifierKeys(test_keyboard);

    std::vector<mojom::ModifierKey> expected_modifier_keys = { mojom::ModifierKey::kBackspace, mojom::ModifierKey::kControl, mojom::ModifierKey::kMeta,
        mojom::ModifierKey::kEscape, mojom::ModifierKey::kAlt, mojom::ModifierKey::kFunction, mojom::ModifierKey::kRightAlt };
    base::ranges::sort(expected_modifier_keys);
    base::ranges::sort(modifier_keys);
    EXPECT_EQ(expected_modifier_keys, modifier_keys);
}
...
TEST_P(KeyboardCapabilityTest, TestGetModifierKeysForEveKeyboard)
{
    ...
    std::vector<mojom::ModifierKey> expected_modifier_keys = { mojom::ModifierKey::kBackspace, mojom::ModifierKey::kControl, mojom::ModifierKey::kMeta,
        mojom::ModifierKey::kEscape, mojom::ModifierKey::kAlt, mojom::ModifierKey::kAssistant };
    base::ranges::sort(expected_modifier_keys);
    base::ranges::sort(modifier_keys);
    EXPECT_EQ(expected_modifier_keys, modifier_keys);
}

class KeyEventTest : public KeyboardCapabilityTestBase, public testing::WithParamInterface<std::tuple<bool, KeyEventTestData>> {
public:
    void SetUp() override
    {
        modifier_split_feature_list_ = std::make_unique<base::test::ScopedFeatureList>();
...
INSTANTIATE_TEST_SUITE_P(All, KeyEventTest,
    testing::Combine(testing::Bool(),
        testing::ValuesIn(std::vector<KeyEventTestData> {
            // Testing top row keys.
            { { INTERNAL }, { kKbdTopRowLayout1Tag }, VKEY_BROWSER_FORWARD, { true }, true },
            { { EXTERNAL_BLUETOOTH }, { kKbdTopRowLayout1Tag }, VKEY_ZOOM, { true }, true },
            { { EXTERNAL_USB }, { kKbdTopRowLayout1Tag }, VKEY_MEDIA_PLAY_PAUSE, { false }, false },
            { { INTERNAL }, { kKbdTopRowLayout2Tag }, VKEY_BROWSER_FORWARD, { false }, false },
            { { EXTERNAL_UNKNOWN }, { kKbdTopRowLayout2Tag }, VKEY_MEDIA_PLAY_PAUSE, { true }, true },
            { { INTERNAL }, { kKbdTopRowLayoutWilcoTag }, VKEY_ZOOM, { true }, true },
            { { EXTERNAL_BLUETOOTH }, { kKbdTopRowLayoutDrallionTag }, VKEY_BRIGHTNESS_UP,

                { true }, true },
            { { INTERNAL, EXTERNAL_BLUETOOTH }, { kKbdTopRowLayout1Tag, kKbdTopRowLayout2Tag }, VKEY_BROWSER_FORWARD, { true, false }, true },
            { { INTERNAL, EXTERNAL_BLUETOOTH }, { kKbdTopRowLayout2Tag, kKbdTopRowLayout2Tag }, VKEY_BROWSER_FORWARD, { false, false }, false },
            { { INTERNAL, EXTERNAL_USB, EXTERNAL_BLUETOOTH }, { kKbdTopRowLayout1Tag, kKbdTopRowLayout2Tag, kKbdTopRowLayoutWilcoTag }, VKEY_VOLUME_UP,
                { true, true, true }, true },
            ...
            { { INTERNAL, EXTERNAL_BLUETOOTH }, { kKbdTopRowLayout1Tag, kKbdTopRowLayoutWilcoTag }, VKEY_HOME, { false, true }, true },

            // Testing other keys.
            { { INTERNAL }, { kKbdTopRowLayout1Tag }, VKEY_LEFT, { true }, true },
            { { EXTERNAL_BLUETOOTH }, { kKbdTopRowLayout2Tag }, VKEY_ESCAPE, { true }, true },
...
{
    auto [keyboard_connection_types, keyboard_layout_types, key_code, expected_has_key_event, expected_has_key_event_on_any_keyboard] = std::get<1>(GetParam());
...
TEST_P(KeyboardCapabilityTest, TestHasAssistantKey)
{
    ...
    const KeyboardDevice test_keyboard_1 = AddFakeKeyboardInfoToKeyboardCapability(
        kDeviceId1, kEveKeyboard, KeyboardCapability::DeviceType::kDeviceInternalKeyboard, KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayout2);
    ...
    EXPECT_EQ(!GetParam(), keyboard_capability_->HasAssistantKey(test_keyboard_1));
    ...
    // assistant key.
    const KeyboardDevice test_keyboard_2 = AddFakeKeyboardInfoToKeyboardCapability(kDeviceId1, kDrallionKeyboard,
        KeyboardCapability::DeviceType::kDeviceInternalKeyboard, KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutDrallion);
...
{
    KeyboardDevice input_device(kDeviceId1, INPUT_DEVICE_INTERNAL, "Internal Keyboard");
    fake_keyboard_manager_->AddFakeKeyboard(input_device, kKbdTopRowLayoutUnspecified);

    EXPECT_EQ(KeyboardCapability::DeviceType::kDeviceInternalKeyboard, keyboard_capability_->GetDeviceType(input_device));
    EXPECT_EQ(KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutDefault, keyboard_capability_->GetTopRowLayout(input_device));
...
{
    KeyboardDevice input_device(kDeviceId1, INPUT_DEVICE_INTERNAL, "Internal Keyboard");
    fake_keyboard_manager_->AddFakeKeyboard(input_device, kKbdTopRowLayoutInvalidTag);

    EXPECT_EQ(KeyboardCapability::DeviceType::kDeviceUnknown, keyboard_capability_->GetDeviceType(input_device));
    EXPECT_EQ(KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutDefault, keyboard_capability_->GetTopRowLayout(input_device));
    EXPECT_TRUE(!keyboard_capability_->GetTopRowScanCodes(input_device) || keyboard_capability_->GetTopRowScanCodes(input_device)->empty());
}

TEST_P(KeyboardCapabilityTest, IdentifyKeyboardInvalidCustomLayout)
{
    KeyboardDevice input_device(kDeviceId1, INPUT_DEVICE_INTERNAL, "Internal Keyboard");
    fake_keyboard_manager_->AddFakeKeyboard(input_device, kKbdInvalidCustomTopRowLayout, /*has_custom_top_row=*/true);
...
{
    KeyboardDevice input_device(kDeviceId1, INPUT_DEVICE_UNKNOWN, "External Chrome Keyboard");
    fake_keyboard_manager_->AddFakeKeyboard(input_device, kKbdTopRowLayout1Tag,
        /*has_custom_top_row=*/false);

    EXPECT_EQ(KeyboardCapability::DeviceType::kDeviceExternalChromeOsKeyboard, keyboard_capability_->GetDeviceType(input_device));
    EXPECT_EQ(KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayout1, keyboard_capability_->GetTopRowLayout(input_device));
    EXPECT_EQ(0u, keyboard_capability_->GetTopRowScanCodes(input_device)->size());
}
...
TEST_P(KeyboardCapabilityTest, IdentifyKeyboardLayout2External)
{
    ...
    EXPECT_EQ(KeyboardCapability::DeviceType::kDeviceExternalChromeOsKeyboard, keyboard_capability_->GetDeviceType(input_device));
    EXPECT_EQ(KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayout2, keyboard_capability_->GetTopRowLayout(input_device));
    EXPECT_EQ(0u, keyboard_capability_->GetTopRowScanCodes(input_device)->size());
}

TEST_P(KeyboardCapabilityTest, IdentifyKeyboardCustomLayout)
{
    KeyboardDevice input_device(kDeviceId1, INPUT_DEVICE_INTERNAL, "Internal Custom Layout Keyboard");
    fake_keyboard_manager_->AddFakeKeyboard(input_device, kKbdDefaultCustomTopRowLayout,
        /*has_custom_top_row=*/true);

    EXPECT_EQ(KeyboardCapability::DeviceType::kDeviceInternalKeyboard, keyboard_capability_->GetDeviceType(input_device));
    EXPECT_EQ(KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutCustom, keyboard_capability_->GetTopRowLayout(input_device));

    const auto* top_row_scan_codes_ptr = keyboard_capability_->GetTopRowScanCodes(input_device);
...
{
    KeyboardDevice input_device(kDeviceId1, INPUT_DEVICE_INTERNAL, "Internal Keyboard");
    fake_keyboard_manager_->AddFakeKeyboard(input_device, kKbdTopRowLayoutWilcoTag,
        /*has_custom_top_row=*/false);

    EXPECT_EQ(KeyboardCapability::DeviceType::kDeviceInternalKeyboard, keyboard_capability_->GetDeviceType(input_device));
    EXPECT_EQ(KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutWilco, keyboard_capability_->GetTopRowLayout(input_device));
    EXPECT_EQ(0u, keyboard_capability_->GetTopRowScanCodes(input_device)->size());
}
...
{
    KeyboardDevice input_device(kDeviceId1, INPUT_DEVICE_INTERNAL, "Internal Keyboard");
    fake_keyboard_manager_->AddFakeKeyboard(input_device, kKbdTopRowLayoutDrallionTag,
        /*has_custom_top_row=*/false);

    EXPECT_EQ(KeyboardCapability::DeviceType::kDeviceInternalKeyboard, keyboard_capability_->GetDeviceType(input_device));
    EXPECT_EQ(KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutDrallion, keyboard_capability_->GetTopRowLayout(input_device));
    EXPECT_EQ(0u, keyboard_capability_->GetTopRowScanCodes(input_device)->size());
}
...
TEST_P(KeyboardCapabilityTest, TopRowLayout1)
{
    ...
    for (TopRowActionKey action_key = TopRowActionKey::kNone; action_key <= TopRowActionKey::kMaxValue;
         action_key = static_cast<TopRowActionKey>(static_cast<int>(action_key) + 1)) {
        EXPECT_EQ(base::Contains(kLayout1TopRowActionKeys, action_key), keyboard_capability_->HasTopRowActionKey(input_device, action_key))
    ...
    for (const auto action_key : kLayout1TopRowActionKeys) {
        EXPECT_EQ(expected_fkey, keyboard_capability_->GetCorrespondingFunctionKey(input_device, action_key));
        EXPECT_EQ(action_key, keyboard_capability_->GetCorrespondingActionKeyForFKey(input_device, expected_fkey));
        expected_fkey = static_cast<KeyboardCode>(static_cast<int>(expected_fkey) + 1);
    }
}
...
TEST_P(KeyboardCapabilityTest, TopRowLayout2)
{
    ...
    for (TopRowActionKey action_key = TopRowActionKey::kNone; action_key <= TopRowActionKey::kMaxValue;
         action_key = static_cast<TopRowActionKey>(static_cast<int>(action_key) + 1)) {
        EXPECT_EQ(base::Contains(kLayout2TopRowActionKeys, action_key), keyboard_capability_->HasTopRowActionKey(input_device, action_key))
            << "Action Key: " << static_cast<int>(action_key);
    }
...
{
    KeyboardDevice wilco_device(kDeviceId1, INPUT_DEVICE_INTERNAL, "Internal Keyboard");
    fake_keyboard_manager_->AddFakeKeyboard(wilco_device, kKbdTopRowLayoutWilcoTag,
        /*has_custom_top_row=*/false);
    KeyboardDevice drallion_device(kDeviceId2, INPUT_DEVICE_INTERNAL, "Internal Keyboard");
    fake_keyboard_manager_->AddFakeKeyboard(drallion_device, kKbdTopRowLayoutDrallionTag,
        /*has_custom_top_row=*/false);

    for (TopRowActionKey action_key = TopRowActionKey::kNone; action_key <= TopRowActionKey::kMaxValue;
         action_key = static_cast<TopRowActionKey>(static_cast<int>(action_key) + 1)) {
        EXPECT_EQ(base::Contains(kLayoutWilcoDrallionTopRowActionKeys, action_key), keyboard_capability_->HasTopRowActionKey(wilco_device, action_key))
            << "Action Key: " << static_cast<int>(action_key);
        EXPECT_EQ(base::Contains(kLayoutWilcoDrallionTopRowActionKeys, action_key), keyboard_capability_->HasTopRowActionKey(drallion_device, action_key))
            << "Action Key: " << static_cast<int>(action_key);
    }

    KeyboardCode expected_fkey = VKEY_F1;
    for (const auto action_key : kLayoutWilcoDrallionTopRowActionKeys) {
        EXPECT_EQ(expected_fkey, keyboard_capability_->GetCorrespondingFunctionKey(wilco_device, action_key));
        EXPECT_EQ(expected_fkey, keyboard_capability_->GetCorrespondingFunctionKey(drallion_device, action_key));
        EXPECT_EQ(action_key, keyboard_capability_->GetCorrespondingActionKeyForFKey(wilco_device, expected_fkey));
        EXPECT_EQ(action_key, keyboard_capability_->GetCorrespondingActionKeyForFKey(drallion_device, expected_fkey));
        expected_fkey = static_cast<KeyboardCode>(static_cast<int>(expected_fkey) + 1);
    }
}
...
TEST_P(KeyboardCapabilityTest, NullTopRowDescriptor)
{
    ...
    KeyboardDevice input_device(kDeviceId1, INPUT_DEVICE_BLUETOOTH, "External Keyboard");
    fake_keyboard_manager_->AddFakeKeyboard(input_device, "C0000 C0000 C0000 C0000",
        /*has_custom_top_row=*/true);
    EXPECT_EQ(KeyboardCapability::DeviceType::kDeviceExternalNullTopRowChromeOsKeyboard, keyboard_capability_->GetDeviceType(input_device));
...
class TopRowLayoutCustomTest : public KeyboardCapabilityTestBase, public testing::WithParamInterface<std::vector<TopRowActionKey>> {
public:
    void SetUp() override
    {
        KeyboardCapabilityTestBase::SetUp();
        ...
        for (const auto& action_key : top_row_action_keys_) {
            ...
            custom_scan_codes.push_back(base::ToLowerASCII(base::HexEncode(&scan_code, 1)));
        }
...
INSTANTIATE_TEST_SUITE_P(All, TopRowLayoutCustomTest,
    testing::ValuesIn(std::vector<std::vector<TopRowActionKey>> { // Test with full 15 key set.
        {
...
{
    KeyboardDevice keyboard(kDeviceId1, INPUT_DEVICE_INTERNAL, "Internal Keyboard");
    fake_keyboard_manager_->AddFakeKeyboard(keyboard, custom_layout_string_,
        /*has_custom_top_row=*/true);
    for (TopRowActionKey action_key = TopRowActionKey::kNone; action_key <= TopRowActionKey::kMaxValue;
         action_key = static_cast<TopRowActionKey>(static_cast<int>(action_key) + 1)) {
        EXPECT_EQ(base::Contains(top_row_action_keys_, action_key), keyboard_capability_->HasTopRowActionKey(keyboard, action_key))
            << "Action Key: " << static_cast<int>(action_key);
    }

    KeyboardCode expected_fkey = VKEY_F1;
    for (const auto action_key : top_row_action_keys_) {
        EXPECT_EQ(expected_fkey, keyboard_capability_->GetCorrespondingFunctionKey(keyboard, action_key));
        EXPECT_EQ(action_key, keyboard_capability_->GetCorrespondingActionKeyForFKey(keyboard, expected_fkey));
        expected_fkey = static_cast<KeyboardCode>(static_cast<int>(expected_fkey) + 1);
    }
}
