...
CapsLockEventRewriter::CapsLockEventRewriter(
    KeyboardLayoutEngine* keyboard_layout_engine, KeyboardCapability* keyboard_capability, ash::input_method::ImeKeyboard* ime_keyboard)
...
EventDispatchDetails CapsLockEventRewriter::RewriteEvent(const Event& event, const Continuation continuation)
{
    ...
    return SendEvent(continuation, rewritten_event ? rewritten_event.get() : &event);
}

std::unique_ptr<KeyEvent> CapsLockEventRewriter::RewritePressKeyEvent(const KeyEvent& key_event)
{
    internal::PhysicalKey physical_key { key_event.code(), GetKeyboardDeviceIdProperty(key_event) };
    ...
    RemappedKey remapped_key = { key_event.code(), key_event.GetDomKey(), key_event.key_code() };

    const bool is_right_alt_key = key_event.code() == DomCode::LAUNCH_ASSISTANT && HasRightAltProperty(key_event);
    ...
    if (is_right_alt_key && is_function_down) {
        ...
        if (!keyboard_layout_engine_->Lookup(remapped_dom_code, key_event.flags(), &dom_key, &key_code)) {
    ...
    if (remapped_key.key == DomKey::CAPS_LOCK) {
        if (pressed_modifier_keys_.insert_or_assign(physical_key, EF_MOD3_DOWN).second) {
...
std::unique_ptr<KeyEvent> CapsLockEventRewriter::RewriteReleaseKeyEvent(const KeyEvent& event)
{
    ...
    return BuildRewrittenEvent(event, remapped.value_or(RemappedKey { event.code(), event.GetDomKey(), event.key_code() }));
}

std::unique_ptr<KeyEvent> CapsLockEventRewriter::BuildRewrittenEvent(const KeyEvent& event, const RemappedKey& remapped)
{
    // Get rewritten flags removing the function modifier if it should be removed.
    const EventFlags flags = RewriteModifierFlags(event.flags() & ~(remapped.should_remove_function_modifier ? EF_FUNCTION_DOWN : EF_NONE));
    if (remapped.code == event.code() && remapped.key == event.GetDomKey() && remapped.key_code == event.key_code() && flags == event.flags()) {
        // Nothing is rewritten.
        return nullptr;
    }

    auto rewritten_event = std::make_unique<KeyEvent>(event.type(), remapped.key_code, remapped.code, flags, remapped.key, event.time_stamp());
