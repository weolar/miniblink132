...
EventDispatchDetails DiscardKeyEventRewriter::RewriteEvent(const Event& event, const Continuation continuation)
{
    ...
    case EventType::kKeyReleased: {
        ...
        if (event.flags() != rewritten_flags) {
            std::unique_ptr<KeyEvent> rewritten_key_event = std::make_unique<KeyEvent>(
                key_event.type(), key_event.key_code(), key_event.code(), rewritten_flags, key_event.GetDomKey(), key_event.time_stamp(), key_event.is_char());
