...
struct VendorProductId {
    ...
    {
        return vendor_id == other.vendor_id ? product_id < other.product_id : vendor_id < other.vendor_id;
    }
};
...
constexpr auto kRightAltBlocklist = base::MakeFixedFlatSet<std::string_view>({ "eve", "nocturne", "atlas" });
...
constexpr size_t kNumCustomTopRowFKeys = (kMaxCustomTopRowLayoutFKeyCode - VKEY_F1) + 1;

// Map used to convert VKEY -> TopRowActionKey and vice versa.
constexpr auto kVKeyToTopRowActionKeyMap = base::MakeFixedFlatMap<ui::KeyboardCode, TopRowActionKey>({
...
constexpr auto kChromeOSKeyboardsWithCapsLock = base::MakeFixedFlatSet<VendorProductId>({
    { 0x046d, 0xb370 } // Logitech Signature K650
});

std::optional<KeyboardDevice> FindKeyboardWithId(int device_id)
{
    const auto& keyboards = DeviceDataManager::GetInstance()->GetKeyboardDevices();
...
bool GetDeviceProperty(const base::FilePath& device_path, const char* key, std::string& value)
{
    ...
    device::ScopedUdevDevicePtr device(device::udev_device_new_from_syspath(udev.get(), device_path.value().c_str()));
    if (!device.get()) {
        return false;
    }
...
std::vector<uint32_t> ParseCustomTopRowLayoutScancodes(const std::string& layout)
{
    std::vector<uint32_t> scancode_vector;

    const std::vector<std::string> scan_code_strings = base::SplitString(layout, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    if (scan_code_strings.size() == 0 || scan_code_strings.size() > kNumCustomTopRowFKeys) {
        return {};
    }
...
bool GetDeviceAttributeRecursive(const base::FilePath& device_path, const char* key, std::string& value)
{
    device::ScopedUdevPtr udev(device::udev_new());
    if (!udev.get()) {
        return false;
...
base::ScopedFD GetEventDeviceNameFd(const KeyboardDevice& keyboard)
{
    ...
    std::string dev_name;
    if (!GetDeviceProperty(keyboard.sys_path, kDevNameProperty, dev_name) || dev_name.empty()) {
...
std::optional<uint32_t> ConvertScanCodeToEvdevKey(const base::ScopedFD& fd, uint32_t scancode)
{
    if (fd.get() < 0) {
        return std::nullopt;
    }
...
bool GetCustomTopRowLayoutAttribute(const KeyboardDevice& keyboard, std::string& out_prop)
{
    bool result = GetDeviceAttributeRecursive(keyboard.sys_path, kCustomTopRowLayoutAttribute, out_prop);

    if (result && out_prop.size() > 0) {
        VLOG(1) << "Identified custom top row keyboard layout: sys_path=" << keyboard.sys_path << " layout=" << out_prop;
        return true;
    }
...
bool GetCustomTopRowLayout(const KeyboardDevice& keyboard, std::string& out_prop)
{
    if (GetCustomTopRowLayoutAttribute(keyboard, out_prop)) {
        return true;
    }
    return GetDeviceProperty(keyboard.sys_path, kCustomTopRowLayoutProperty, out_prop);
}
...
bool GetTopRowLayoutProperty(const KeyboardDevice& keyboard_device, std::string& out_prop)
{
...
bool ParseKeyboardTopRowLayout(const std::string& layout_string, KeyboardTopRowLayout& out_layout)
{
    ...
    if (!base::StringToInt(layout_string, &layout_id)) {
        LOG(WARNING) << "Failed to parse layout " << kLayoutProperty << " value '" << layout_string << "'";
        return false;
    }

    if (layout_id < static_cast<int>(KeyboardTopRowLayout::kKbdTopRowLayoutMin) || layout_id > static_cast<int>(KeyboardTopRowLayout::kKbdTopRowLayoutMax)) {
...
KeyboardCapability::DeviceType IdentifyKeyboardType(const KeyboardDevice& keyboard_device, bool has_chromeos_top_row, bool has_null_top_row)
{
    if (keyboard_device.vendor_id == kHotrodRemoteVendorId && keyboard_device.product_id == kHotrodRemoteProductId) {
        VLOG(1) << "Hotrod remote '" << keyboard_device.name << "' connected: id=" << keyboard_device.id;
        return KeyboardCapability::DeviceType::kDeviceHotrodRemote;
    }

    if (base::EqualsCaseInsensitiveASCII(keyboard_device.name, "virtual core keyboard")) {
        VLOG(1) << "Xorg virtual '" << keyboard_device.name << "' connected: id=" << keyboard_device.id;
        return KeyboardCapability::DeviceType::kDeviceVirtualCoreKeyboard;
    }

    if (keyboard_device.type == INPUT_DEVICE_INTERNAL) {
        VLOG(1) << "Internal keyboard '" << keyboard_device.name << "' connected: id=" << keyboard_device.id;
        return ash::switches::IsRevenBranding() ? KeyboardCapability::DeviceType::kDeviceInternalRevenKeyboard
                                                : KeyboardCapability::DeviceType::kDeviceInternalKeyboard;
    }

    if (has_null_top_row) {
        VLOG(1) << "External Null Top Row keyboard '" << keyboard_device.name << "' connected: id=" << keyboard_device.id;
        return KeyboardCapability::DeviceType::kDeviceExternalNullTopRowChromeOsKeyboard;
    }

    if (has_chromeos_top_row) {
        // If the device was tagged as having Chrome OS top row layout it must be
        // a Chrome OS keyboard.
        VLOG(1) << "External Chrome OS keyboard '" << keyboard_device.name << "' connected: id=" << keyboard_device.id;
        return KeyboardCapability::DeviceType::kDeviceExternalChromeOsKeyboard;
    }

    const std::vector<std::string> tokens = base::SplitString(keyboard_device.name, " .", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    for (const auto& token : tokens) {
        ...
        if (!found_keyboard && base::EqualsCaseInsensitiveASCII(token, "keyboard")) {
            found_keyboard = true;
        }
    }
    if (found_apple) {
        ...
        if (found_keyboard) {
            VLOG(1) << "Apple keyboard '" << keyboard_device.name << "' connected: id=" << keyboard_device.id;
            return KeyboardCapability::DeviceType::kDeviceExternalAppleKeyboard;
        } else {
            VLOG(1) << "Apple device '" << keyboard_device.name << "' connected: id=" << keyboard_device.id;
            return KeyboardCapability::DeviceType::kDeviceExternalUnknown;
        }
    } else if (found_keyboard) {
        VLOG(1) << "External keyboard '" << keyboard_device.name << "' connected: id=" << keyboard_device.id;
        return KeyboardCapability::DeviceType::kDeviceExternalGenericKeyboard;
    } else {
        VLOG(1) << "External device '" << keyboard_device.name << "' connected: id=" << keyboard_device.id;
        return KeyboardCapability::DeviceType::kDeviceExternalUnknown;
    }
}

std::tuple<DeviceType, KeyboardTopRowLayout, std::vector<uint32_t>> IdentifyKeyboardInfo(const KeyboardDevice& keyboard)
{
    ...
    if (!top_row_scan_codes.empty()) {
        null_top_row = base::ranges::all_of(top_row_scan_codes, [](const uint32_t scancode) { return scancode == kCustomNullScanCode; });
        if (base::FeatureList::IsEnabled(ash::features::kNullTopRowFix) && null_top_row) {
    ...
    } else if (!GetTopRowLayoutProperty(keyboard, layout_string) || !ParseKeyboardTopRowLayout(layout_string, layout)) {
        return { KeyboardCapability::DeviceType::kDeviceUnknown, KeyboardTopRowLayout::kKbdTopRowLayoutDefault, {} };
    }

    return { IdentifyKeyboardType(keyboard, !top_row_scan_codes.empty() || !layout_string.empty(), null_top_row), layout, std::move(top_row_scan_codes) };
}

std::vector<TopRowActionKey> IdentifyCustomTopRowActionKeys(const KeyboardCapability::ScanCodeToEvdevKeyConverter& scan_code_to_evdev_key_converter,
    const KeyboardDevice& keyboard, const std::vector<uint32_t>& top_row_scan_codes)
{
    ...
    for (const auto& scancode : top_row_scan_codes) {
        ...
        const DomCode dom_code = KeycodeConverter::EvdevCodeToDomCode(*evdev_key_code);
...
std::vector<TopRowActionKey> IdentifyTopRowActionKeys(const KeyboardCapability::ScanCodeToEvdevKeyConverter& scan_code_to_evdev_key_converter,
    const KeyboardDevice& keyboard, DeviceType device_type, KeyboardTopRowLayout layout, const std::vector<uint32_t>& top_row_scan_codes)
{
    switch (layout) {
    case KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayout1:
        return std::vector<TopRowActionKey>(std::begin(kLayout1TopRowActionKeys), std::end(kLayout1TopRowActionKeys));
    case KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayout2:
        return std::vector<TopRowActionKey>(std::begin(kLayout2TopRowActionKeys), std::end(kLayout2TopRowActionKeys));
    ...
    case KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutDrallion:
        return std::vector<TopRowActionKey>(std::begin(kLayoutWilcoDrallionTopRowActionKeys), std::end(kLayoutWilcoDrallionTopRowActionKeys));
    case KeyboardCapability::KeyboardTopRowLayout::kKbdTopRowLayoutCustom:
        return IdentifyCustomTopRowActionKeys(scan_code_to_evdev_key_converter, keyboard, top_row_scan_codes);
    }
}
...
{
    for (const ui::KeyboardDevice& keyboard : ui::DeviceDataManager::GetInstance()->GetKeyboardDevices()) {
        if (!keyboard.suspected_keyboard_imposter && !IsInternalKeyboard(keyboard)) {
            return true;
        }
    }
    return false;
...
KeyboardCapability::KeyboardCapability()
    : scan_code_to_evdev_key_converter_(base::BindRepeating(&ConvertScanCodeToEvdevKey))
    , board_name_(base::ToLowerASCII(base::SysInfo::HardwareModelName()))
{
    DeviceDataManager::GetInstance()->AddObserver(this);
}

KeyboardCapability::KeyboardCapability(ScanCodeToEvdevKeyConverter scan_code_to_evdev_key_converter)
    : scan_code_to_evdev_key_converter_(std::move(scan_code_to_evdev_key_converter))
{
    DeviceDataManager::GetInstance()->AddObserver(this);
}
...
KeyboardCapability::KeyboardInfo& KeyboardCapability::KeyboardInfo::operator=(KeyboardInfo&&) = default;
KeyboardCapability::KeyboardInfo::~KeyboardInfo() = default;

// static
std::unique_ptr<KeyboardCapability> KeyboardCapability::CreateStubKeyboardCapability()
{
    return std::make_unique<KeyboardCapability>();
}

// static
std::unique_ptr<EventDeviceInfo> KeyboardCapability::CreateEventDeviceInfoFromInputDevice(const KeyboardDevice& keyboard)
{
    base::ScopedFD fd = GetEventDeviceNameFd(keyboard);
    if (fd.get() < 0) {
        return nullptr;
    ...
    std::unique_ptr<EventDeviceInfo> event_device_info = std::make_unique<EventDeviceInfo>();
    if (!event_device_info->Initialize(fd.get(), keyboard.sys_path)) {
        LOG(ERROR) << "Failed to get device information for " << keyboard.sys_path.value();
        return nullptr;
    }
...
// static
std::optional<TopRowActionKey> KeyboardCapability::ConvertToTopRowActionKey(ui::KeyboardCode key_code)
{
    const auto action_key = kVKeyToTopRowActionKeyMap.find(key_code);
    return (action_key != kVKeyToTopRowActionKeyMap.end()) ? std::make_optional<TopRowActionKey>(action_key->second) : std::nullopt;
}

// static
std::optional<KeyboardCode> KeyboardCapability::ConvertToKeyboardCode(TopRowActionKey action_key)
{
...
std::optional<KeyboardCode> KeyboardCapability::GetMappedFKeyIfExists(const KeyboardCode& key_code, const KeyboardDevice& keyboard) const
{
...
std::optional<KeyboardCode> KeyboardCapability::GetCorrespondingFunctionKey(const KeyboardDevice& keyboard, TopRowActionKey action_key) const
{
    ...
    auto iter = base::ranges::find(keyboard_info->top_row_action_keys, action_key);
    if (iter == keyboard_info->top_row_action_keys.end()) {
        return std::nullopt;
    }

    return kFunctionKeys[std::distance(keyboard_info->top_row_action_keys.begin(), iter)];
}

std::optional<TopRowActionKey> KeyboardCapability::GetCorrespondingActionKeyForFKey(const KeyboardDevice& keyboard, KeyboardCode key_code) const
{
    auto* keyboard_info = GetKeyboardInfo(keyboard);
    if (!keyboard_info) {
        return std::nullopt;
...
bool KeyboardCapability::HasLauncherButton(const KeyboardDevice& keyboard) const
{
...
{
    return ui::KeyboardCode::VKEY_F1 <= code && code <= ui::KeyboardCode::VKEY_F24;
}

// static
bool KeyboardCapability::IsF11OrF12(ui::KeyboardCode code)
{
    return code == ui::KeyboardCode::VKEY_F11 || code == ui::KeyboardCode::VKEY_F12;
}

std::vector<mojom::ModifierKey> KeyboardCapability::GetModifierKeys(const KeyboardDevice& keyboard) const
{
...
std::vector<mojom::ModifierKey> KeyboardCapability::GetModifierKeys(int device_id) const
{
...
DeviceType KeyboardCapability::GetDeviceType(const KeyboardDevice& keyboard) const
{
...
KeyboardTopRowLayout KeyboardCapability::GetTopRowLayout(const KeyboardDevice& keyboard) const
{
    const auto* keyboard_info = GetKeyboardInfo(keyboard);
    if (!keyboard_info) {
...
void KeyboardCapability::SetKeyboardInfoForTesting(const KeyboardDevice& keyboard, KeyboardInfo keyboard_info)
{
...
const KeyboardCapability::KeyboardInfo* KeyboardCapability::GetKeyboardInfo(const KeyboardDevice& keyboard) const
{
    ...
    std::tie(keyboard_info.device_type, keyboard_info.top_row_layout, keyboard_info.top_row_scan_codes) = IdentifyKeyboardInfo(keyboard);
    keyboard_info.top_row_action_keys = IdentifyTopRowActionKeys(
        scan_code_to_evdev_key_converter_, keyboard, keyboard_info.device_type, keyboard_info.top_row_layout, keyboard_info.top_row_scan_codes);
...
const std::vector<uint32_t>* KeyboardCapability::GetTopRowScanCodes(const KeyboardDevice& keyboard) const
{
...
const std::vector<uint32_t>* KeyboardCapability::GetTopRowScanCodes(int device_id) const
{
    auto keyboard = FindKeyboardWithId(device_id);
...
bool KeyboardCapability::HasGlobeKey(const KeyboardDevice& keyboard) const
{
    ...
    return !IsInternalKeyboard(keyboard) || keyboard_info->top_row_layout == KeyboardTopRowLayout::kKbdTopRowLayoutDrallion
        || keyboard_info->top_row_layout == KeyboardTopRowLayout::kKbdTopRowLayoutWilco;
}
...
bool KeyboardCapability::HasCalculatorKey(const KeyboardDevice& keyboard) const
{
...
bool KeyboardCapability::HasBrowserSearchKey(const KeyboardDevice& keyboard) const
{
    // TODO(dpad): Many external keyboards do not have this key, but currently we
    // do not have a good way to detect these situations.
    return !IsInternalKeyboard(keyboard);
...
const std::vector<TopRowActionKey>* KeyboardCapability::GetTopRowActionKeys(const KeyboardDevice& keyboard) const
{
    const auto* keyboard_info = GetKeyboardInfo(keyboard);
    if (!keyboard_info) {
        return nullptr;
...
const std::vector<TopRowActionKey>* KeyboardCapability::GetTopRowActionKeys(int device_id) const
{
    auto keyboard = FindKeyboardWithId(device_id);
    if (!keyboard) {
        return nullptr;
...
{
    return !IsChromeOSKeyboard(keyboard.id) || kChromeOSKeyboardsWithCapsLock.contains({ keyboard.vendor_id, keyboard.product_id });
}
...
bool KeyboardCapability::HasFunctionKey(const KeyboardDevice& keyboard) const
{
    ...
    return ash::features::IsModifierSplitEnabled() && keyboard.type == InputDeviceType::INPUT_DEVICE_INTERNAL && keyboard.has_function_key;
}
...
bool KeyboardCapability::HasRightAltKey(const KeyboardDevice& keyboard) const
{
    ...
    return keyboard.type == InputDeviceType::INPUT_DEVICE_INTERNAL && keyboard.has_assistant_key;
}
...
bool KeyboardCapability::HasRightAltKeyForOobe(const KeyboardDevice& keyboard) const
{
    if (ash::features::IsModifierSplitEnabled()) {
        return false;
    }
...
bool KeyboardCapability::IsSplitModifierKeyboardForOverride(const KeyboardDevice& keyboard) const
{
    if (kRightAltBlocklist.contains(board_name_)) {
        return false;
    }
...
ui::mojom::MetaKey KeyboardCapability::GetMetaKey(const KeyboardDevice& keyboard) const
{
    ...
    case ui::KeyboardCapability::DeviceType::kDeviceExternalNullTopRowChromeOsKeyboard:
    ...
    switch (top_row_layout) {
    case KeyboardTopRowLayout::kKbdTopRowLayout1:
        return IsInternalKeyboard(keyboard) ? mojom::MetaKey::kSearch : mojom::MetaKey::kLauncher;
    case KeyboardTopRowLayout::kKbdTopRowLayout2:
    case KeyboardTopRowLayout::kKbdTopRowLayoutWilco:
    case KeyboardTopRowLayout::kKbdTopRowLayoutDrallion:
    case KeyboardTopRowLayout::kKbdTopRowLayoutCustom:
...
ui::mojom::MetaKey KeyboardCapability::GetMetaKeyToDisplay() const
{
    ...
    if (current_best != mojom::MetaKey::kExternalMeta && current_best != mojom::MetaKey::kCommand) {
        return current_best;
    }
...
void KeyboardCapability::OnInputDeviceConfigurationChanged(uint8_t input_device_types)
{
...
void KeyboardCapability::TrimKeyboardInfoMap()
{
    ...
    auto sorted_keyboards = DeviceDataManager::GetInstance()->GetKeyboardDevices();
    base::ranges::sort(sorted_keyboards, [](const ui::KeyboardDevice& device1, const ui::KeyboardDevice& device2) { return device1.id < device2.id; });
    ...
    base::ranges::transform(keyboard_info_map_, std::back_inserter(cached_keyboard_info_ids), [](const auto& pair) { return pair.first; });
    ...
    base::ranges::set_difference(cached_keyboard_info_ids, sorted_keyboards, std::back_inserter(keyboard_ids_to_remove),
...
bool KeyboardCapability::HasKeyEvent(const KeyboardCode& key_code, const KeyboardDevice& keyboard) const
{
    // Handle top row keys.
    std::optional<TopRowActionKey> top_row_action_key = ConvertToTopRowActionKey(key_code);
...
bool KeyboardCapability::HasKeyEventOnAnyKeyboard(const KeyboardCode& key_code) const
{
    for (const ui::KeyboardDevice& keyboard : ui::DeviceDataManager::GetInstance()->GetKeyboardDevices()) {
...
bool KeyboardCapability::HasTopRowActionKey(const KeyboardDevice& keyboard, TopRowActionKey action_key) const
{
    const auto* keyboard_info = GetKeyboardInfo(keyboard);
    if (!keyboard_info) {
...
bool KeyboardCapability::HasTopRowActionKeyOnAnyKeyboard(TopRowActionKey action_key) const
{
    for (const ui::KeyboardDevice& keyboard : ui::DeviceDataManager::GetInstance()->GetKeyboardDevices()) {
...
bool KeyboardCapability::IsSplitModifierKeyboard(const KeyboardDevice& keyboard) const
{
...
bool KeyboardCapability::IsChromeOSKeyboard(int device_id) const
{
    ...
    return device_type == DeviceType::kDeviceInternalKeyboard || device_type == DeviceType::kDeviceExternalChromeOsKeyboard;
}
