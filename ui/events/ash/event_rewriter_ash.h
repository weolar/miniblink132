...
public:
    ...
    struct MutableKeyState {
        ...
        constexpr MutableKeyState(int input_flags, DomCode input_code, DomKey::Base input_key, KeyboardCode input_key_code)
            : flags(input_flags)
            , code(input_code)
            , key(input_key)
        ...
        friend bool operator==(const MutableKeyState& lhs, const MutableKeyState& rhs)
        {
            return lhs.flags == rhs.flags && lhs.code == rhs.code && lhs.key == rhs.key && lhs.key_code == rhs.key_code;
        }
    ...
    public:
        ...
        virtual std::optional<mojom::ModifierKey> GetKeyboardRemappedModifierValue(
            int device_id, mojom::ModifierKey modifier_key, const std::string& pref_name) const
            = 0;
        ...
        virtual bool IsExtensionCommandRegistered(KeyboardCode key_code, int flags) const = 0;
        ...
        virtual void RecordEventRemappedToRightClick(bool alt_based_right_click) = 0;
        ...
        // "six pack" key.
        virtual void RecordSixPackEventRewrite(KeyboardCode key_code, bool alt_based) = 0;
        ...
        virtual std::optional<ui::mojom::SimulateRightClickModifier> GetRemapRightClickModifier(int device_id) = 0;
        ...
        virtual std::optional<ui::mojom::SixPackShortcutModifier> GetShortcutModifierForSixPackKey(int device_id, ui::KeyboardCode key_code) = 0;
        ...
        virtual void NotifyRightClickRewriteBlockedBySetting(
            ui::mojom::SimulateRightClickModifier blocked_modifier, ui::mojom::SimulateRightClickModifier active_modifier)
            = 0;
        ...
        virtual void NotifySixPackRewriteBlockedBySetting(
            ui::KeyboardCode key_code, ui::mojom::SixPackShortcutModifier blocked_modifier, ui::mojom::SixPackShortcutModifier active_modifier, int device_id)
            = 0;
        ...
        virtual std::optional<ui::mojom::ExtendedFkeysModifier> GetExtendedFkeySetting(int device_id, ui::KeyboardCode key_code) = 0;
        ...
        virtual void NotifySixPackRewriteBlockedByFnKey(ui::KeyboardCode key_code, ui::mojom::SixPackShortcutModifier modifier) = 0;
    ...
    // don't happen.
    EventRewriterAsh(Delegate* delegate, KeyboardCapability* keyboard_capability, EventRewriter* sticky_keys_controller, bool privacy_screen_supported);
    ...
    void RewriteMouseButtonEventForTesting(const MouseEvent& event, const Continuation continuation);
    ...
    EventDispatchDetails RewriteEvent(const Event& event, const Continuation continuation) override;

    // Generate a new key event from an original key event and the replacement
    // state determined by a key rewriter.
    static void BuildRewrittenKeyEvent(const KeyEvent& key_event, const MutableKeyState& state, std::unique_ptr<Event>* rewritten_event);
    ...
    // |has_assistant_key|.
    static bool HasAssistantKeyOnKeyboard(const KeyboardDevice& keyboard_device, bool* has_assistant_key);
...
private:
    ...
    bool ShouldRemapToRightClick(const MouseEvent& mouse_event, int flags, int* matched_mask, bool* matched_alt_deprecation) const;

    // Rewrite a particular kind of event.
    EventRewriteStatus RewriteKeyEvent(const KeyEvent& key_event, std::unique_ptr<Event>* rewritten_event);
    EventDispatchDetails RewriteMouseButtonEvent(const MouseEvent& mouse_event, const Continuation continuation);
    EventDispatchDetails RewriteMouseWheelEvent(const MouseWheelEvent& mouse_event, const Continuation continuation);
    EventDispatchDetails RewriteTouchEvent(const TouchEvent& touch_event, const Continuation continuation);
    EventDispatchDetails RewriteScrollEvent(const ScrollEvent& scroll_event, const Continuation continuation);
    ...
    bool RewriteModifierKeys(const KeyEvent& event, int device_id, MutableKeyState* state);
    void RewriteNumPadKeys(const KeyEvent& event, MutableKeyState* state);
    void RewriteFunctionKeys(const KeyEvent& event, int device_id, MutableKeyState* state);
    ...
    bool RewriteTopRowKeysForCustomLayout(
        const ui::KeyEvent& key_event, int device_id, bool flip_remapping, EventFlags flip_remapping_flag, MutableKeyState* state);

    // Handle Fn/Action key remapping for Wilco keyboard layout.
    bool RewriteTopRowKeysForLayoutWilco(const KeyEvent& key_event, int device_id, bool flip_remapping, EventFlags flip_remapping_flag, MutableKeyState* state,
        KeyboardCapability::KeyboardTopRowLayout layout);

    bool RewriteTopRowKeysForStandardLayouts(const KeyEvent& key_event, int device_id, bool flip_remapping, EventFlags flip_remapping_flag,
        bool rewrite_modifier_is_pressed, MutableKeyState* state, KeyboardCapability::KeyboardTopRowLayout layout);
    ...
    EventDispatchDetails RewriteKeyEventInContext(
        const KeyEvent& event, std::unique_ptr<Event> rewritten_event, EventRewriteStatus status, const Continuation continuation);

    EventDispatchDetails SendStickyKeysReleaseEvents(std::unique_ptr<Event> rewritten_event, const Continuation continuation);
