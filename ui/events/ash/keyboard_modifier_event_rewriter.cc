...
{
    if (const UnmappedCode* unmapped_code = std::get_if<UnmappedCode>(&physical_code)) {
...
KeyboardModifierEventRewriter::KeyboardModifierEventRewriter(std::unique_ptr<Delegate> delegate, KeyboardLayoutEngine* keyboard_layout_engine,
    KeyboardCapability* keyboard_capability, ash::input_method::ImeKeyboard* ime_keyboard)
...
EventDispatchDetails KeyboardModifierEventRewriter::RewriteEvent(const Event& event, const Continuation continuation)
{
    ...
    case EventType::kKeyPressed: {
        ...
        if (should_record_metrics) {
            RecordModifierKeyPressedBeforeRemapping(*keyboard_capability_, GetKeyboardDeviceIdProperty(event), event.AsKeyEvent()->code());
        }

        rewritten_event = RewritePressKeyEvent(*event.AsKeyEvent());

        if (should_record_metrics) {
            const KeyEvent* event_for_record = rewritten_event ? rewritten_event->AsKeyEvent() : event.AsKeyEvent();
            RecordModifierKeyPressedAfterRemapping(*keyboard_capability_, GetKeyboardDeviceIdProperty(*event_for_record), event_for_record->code(),
                event.AsKeyEvent()->code(), HasRightAltProperty(*event_for_record));
        }
        break;
    }
    ...
    return continuation->SendEvent(rewritten_event ? rewritten_event.get() : &event);
}

std::unique_ptr<Event> KeyboardModifierEventRewriter::RewritePressKeyEvent(const KeyEvent& event)
{
    internal::PhysicalKey physical_key { event.code(), GetKeyboardDeviceIdProperty(event) };

    // Remap key based on user preferences.
    RemappedKey remapped = RemapPressKey(event).value_or(RemappedKey { event.code(), event.GetDomKey(), event.key_code() });
    ...
    // release event.
    if (GetDomCodeFromPhysicalCode(remapped.code) != event.code() || remapped.key != event.GetDomKey() || remapped.key_code != event.key_code()
    ...
    // Update modifier flags.
    {
        ...
        if (keyboard_layout_engine_->GetLayoutName() == "de(neo)" && remapped.key == DomKey::ALT_GRAPH) {
            modifier_flag |= EF_MOD3_DOWN;
        }
        if (pressed_modifier_keys_.insert_or_assign(physical_key, modifier_flag).second) {
            ...
            // CapsLock.
            if (!ash::features::IsModifierSplitEnabled() && remapped.key == DomKey::CAPS_LOCK) {
    ...
    if (!KeycodeConverter::IsDomKeyForModifier((rewritten_event ? *rewritten_event : event).GetDomKey())) {
        altgr_latch_ = false;
    }
...
std::optional<KeyboardModifierEventRewriter::RemappedKey> KeyboardModifierEventRewriter::RemapPressKey(const KeyEvent& event)
{
    ...
    const PhysicalCode remapped_code = GetRemappedPhysicalCode(event.code(), GetKeyboardDeviceIdProperty(event)).value_or(event.code());
    ...
    if (!keyboard_layout_engine_->Lookup(remapped_dom_code, RewriteModifierFlags(event.flags()), &dom_key, &keycode)) {
...
std::unique_ptr<Event> KeyboardModifierEventRewriter::RewriteReleaseKeyEvent(const KeyEvent& event)
{
    ...
    return BuildRewrittenEvent(event,
        remapped.value_or(RemappedKey {
            event.code(),
            event.GetDomKey(),
            event.key_code(),
        }));
...
std::unique_ptr<KeyEvent> KeyboardModifierEventRewriter::BuildRewrittenEvent(const KeyEvent& event, const RemappedKey& remapped)
{
    // Events with unmapped codes must always be rewritten.
    EventFlags flags = RewriteModifierFlags(event.flags());
    if (GetDomCodeFromPhysicalCode(remapped.code) == event.code() && remapped.key == event.GetDomKey() && remapped.key_code == event.key_code()
        && flags == event.flags() && !std::holds_alternative<UnmappedCode>(remapped.code)) {
        // Nothing is rewritten.
        return nullptr;
    }

    auto rewritten_event
        = std::make_unique<KeyEvent>(event.type(), remapped.key_code, GetDomCodeFromPhysicalCode(remapped.code), flags, remapped.key, event.time_stamp());
    ...
    if (const UnmappedCode* unmapped_code = std::get_if<UnmappedCode>(&remapped.code)) {
        if (*unmapped_code == KeyboardModifierEventRewriter::UnmappedCode::kRightAlt) {
...
EventFlags KeyboardModifierEventRewriter::RewriteModifierFlags(EventFlags flags) const
{
    // Bit mask of modifier flags to be rewritten.
    constexpr EventFlags kTargetModifierFlags = EF_CONTROL_DOWN | EF_ALT_DOWN | EF_COMMAND_DOWN | EF_ALTGR_DOWN | EF_MOD3_DOWN | EF_FUNCTION_DOWN;
...
std::optional<PhysicalCode> KeyboardModifierEventRewriter::GetRemappedPhysicalCode(DomCode code, int device_id) const
{
    bool is_left = true;
    ...
    case DomCode::META_LEFT:
        ...
        switch (static_cast<KeyboardCapability::DeviceType>(keyboard_capability_->GetDeviceType(device_id))) {
    ...
    CHECK(!pref_name.empty() || ash::features::IsInputDeviceSettingsSplitEnabled());

    auto modifier_value = delegate_->GetKeyboardRemappedModifierValue(device_id, modifier_key, std::string(pref_name));
