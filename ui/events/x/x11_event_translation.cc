...
public:
    TouchEventX11(EventType type, gfx::Point location, base::TimeTicks timestamp, const PointerDetails& pointer_details)
...
Event::Properties GetEventPropertiesFromXEvent(EventType type, const x11::Event& x11_event)
{
    ...
    if (type == EventType::kKeyPressed || type == EventType::kKeyReleased) {
        ...
        uint8_t ime_flags = (state >> kPropertyKeyboardImeFlagOffset) & kPropertyKeyboardImeFlagMask;
        if (ime_flags) {
        ...
        bool crossing_intermediate_window = crossing->detail == x11::NotifyDetail::Virtual;
        if (crossing_intermediate_window) {
            properties.emplace(kPropertyMouseCrossedIntermediateWindow, crossing_intermediate_window);
        }
    }
    return properties;
}

std::unique_ptr<KeyEvent> CreateKeyEvent(EventType event_type, const x11::Event& x11_event)
{
    ...
    std::unique_ptr<KeyEvent> event =
        ...
        std::make_unique<KeyEvent>(event_type, key_code, event_flags, EventTimeFromXEvent(x11_event));
#else
        std::make_unique<KeyEvent>(
            event_type, key_code, CodeFromXEvent(x11_event), event_flags, GetDomKeyFromXEvent(x11_event), EventTimeFromXEvent(x11_event));
#endif
...
std::unique_ptr<MouseEvent> CreateMouseEvent(EventType type, const x11::Event& x11_event)
{
    if (auto* crossing = x11_event.As<x11::CrossingEvent>()) {
        ...
        if (crossing->detail == x11::NotifyDetail::Ancestor && crossing->mode == x11::NotifyMode::Grab && crossing->opcode == x11::CrossingEvent::LeaveNotify) {
            return nullptr;
        }
    }

    PointerDetails details = GetStylusPointerDetailsFromXEvent(x11_event);
    auto event = std::make_unique<MouseEvent>(type, EventLocationFromXEvent(x11_event), EventSystemLocationFromXEvent(x11_event),
        EventTimeFromXEvent(x11_event), EventFlagsFromXEvent(x11_event), GetChangedMouseButtonFlagsFromXEvent(x11_event), details);
...
std::unique_ptr<MouseWheelEvent> CreateMouseWheelEvent(const x11::Event& x11_event)
{
    int button_flags = x11_event.As<x11::Input::DeviceEvent>() ? GetChangedMouseButtonFlagsFromXEvent(x11_event) : 0;
    auto event = std::make_unique<MouseWheelEvent>(GetMouseWheelOffsetFromXEvent(x11_event), EventLocationFromXEvent(x11_event),
        EventSystemLocationFromXEvent(x11_event), EventTimeFromXEvent(x11_event), EventFlagsFromXEvent(x11_event), button_flags);
...
std::unique_ptr<TouchEvent> CreateTouchEvent(EventType type, const x11::Event& xev)
{
    auto event = std::make_unique<TouchEventX11>(type, EventLocationFromXEvent(xev), EventTimeFromXEvent(xev), GetTouchPointerDetailsFromXEvent(xev));
...
std::unique_ptr<ScrollEvent> CreateScrollEvent(EventType type, const x11::Event& xev)
{
    float x_offset, y_offset, x_offset_ordinal, y_offset_ordinal;
    int finger_count = 0;

    if (type == EventType::kScroll) {
        GetScrollOffsetsFromXEvent(xev, &x_offset, &y_offset, &x_offset_ordinal, &y_offset_ordinal, &finger_count);
    } else {
        GetFlingDataFromXEvent(xev, &x_offset, &y_offset, &x_offset_ordinal, &y_offset_ordinal, nullptr);
    }
    ...
    auto event = std::make_unique<ScrollEvent>(type, EventLocationFromXEvent(xev), EventTimeFromXEvent(xev), EventFlagsFromXEvent(xev), x_offset, y_offset,
        x_offset_ordinal, y_offset_ordinal, finger_count);
    ...
    return (event->x_offset() != 0.0 || event->y_offset() != 0.0 || event->type() == EventType::kScrollFlingStart) ? std::move(event) : nullptr;
}

// Translates XI2 XEvent into a ui::Event.
std::unique_ptr<ui::Event> TranslateFromXI2Event(const x11::Event& xev, EventType event_type)
{
    switch (event_type) {
...
std::unique_ptr<MouseWheelEvent> BuildMouseWheelEventFromXEvent(const x11::Event& xev)
{
    auto event = BuildEventFromXEvent(xev);
