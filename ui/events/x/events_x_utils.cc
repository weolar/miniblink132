...
bool TouchEventIsGeneratedHack(const x11::Event& x11_event)
{
    ...
    CHECK(event);
    CHECK(event->opcode == x11::Input::DeviceEvent::TouchBegin || event->opcode == x11::Input::DeviceEvent::TouchUpdate
    ...
    if (!ui::DeviceDataManagerX11::GetInstance()->GetDataRange(deviceid, ui::DeviceDataManagerX11::DT_TOUCH_MAJOR, &min, &max)) {
        return false;
    }
...
int GetEventFlagsFromXGenericEvent(const x11::Event& x11_event)
{
    ...
    DCHECK(xievent);
    DCHECK(xievent->opcode == x11::Input::DeviceEvent::KeyPress || xievent->opcode == x11::Input::DeviceEvent::KeyRelease);
    return GetEventFlagsFromXState(xievent->mods.effective) | (x11_event.send_event() ? ui::EF_FINAL : 0);
}
...
int GetButtonMaskForX2Event(const x11::Input::DeviceEvent& xievent)
{
    ...
    for (size_t i = 0; i < 32 * xievent.button_mask.size(); i++) {
        if (ui::IsXinputMaskSet(xievent.button_mask.data(), i)) {
            int button = (xievent.sourceid == xievent.deviceid) ? ui::DeviceDataManagerX11::GetInstance()->GetMappedButton(i) : i;
...
ui::EventType GetTouchEventType(const x11::Event& x11_event)
{
    ...
    case x11::Input::DeviceEvent::TouchBegin:
        return TouchEventIsGeneratedHack(x11_event) ? ui::EventType::kUnknown : ui::EventType::kTouchPressed;
    case x11::Input::DeviceEvent::TouchUpdate:
        return TouchEventIsGeneratedHack(x11_event) ? ui::EventType::kUnknown : ui::EventType::kTouchMoved;
    case x11::Input::DeviceEvent::TouchEnd:
        return TouchEventIsGeneratedHack(x11_event) ? ui::EventType::kTouchCancelled : ui::EventType::kTouchReleased;
    default:;
    }
    ...
    case x11::Input::DeviceEvent::Motion:
        ...
        // touch event.
        if (!static_cast<bool>(event->flags & x11::Input::KeyEventFlags::KeyRepeat) && GetButtonMaskForX2Event(*event))
...
double GetParamFromXEvent(const x11::Event& xev, ui::DeviceDataManagerX11::DataType val, double default_value)
{
    ui::DeviceDataManagerX11::GetInstance()->GetEventData(xev, val, &default_value);
...
void ScaleTouchRadius(const x11::Event& x11_event, double* radius)
{
    ...
    DCHECK(xiev);
    ui::DeviceDataManagerX11::GetInstance()->ApplyTouchRadiusScale(static_cast<uint16_t>(xiev->sourceid), radius);
}
...
bool GetGestureTimes(const x11::Event& xev, double* start_time, double* end_time)
{
    ...
    ui::DeviceDataManagerX11::GetInstance()->GetGestureTimes(xev, start_time, end_time);
    return true;
}
...
base::TimeTicks TimeTicksFromXEventTime(x11::Time timestamp)
{
    ...
    bool had_recent_rollover = !g_last_seen_timestamp_ms || g_last_seen_timestamp_ms - timestamp64 > (UINT32_MAX >> 1);
    ...
    DCHECK(timestamp64 <= UINT32_MAX) << "X11 Time does not roll over 32 bit, the below logic is likely wrong";
...
base::TimeTicks TimeTicksFromXEvent(const x11::Event& xev)
{
    ...
    if (auto* device = xev.As<x11::Input::DeviceEvent>()) {
        ...
        } else if (ui::DeviceDataManagerX11::GetInstance()->GetEventData(xev, ui::DeviceDataManagerX11::DT_TOUCH_RAW_TIMESTAMP, &touch_timestamp)) {
...
EventType EventTypeFromXEvent(const x11::Event& xev)
{
    ...
    // manager.
    if (DeviceDataManager::HasInstance() && DeviceDataManagerX11::GetInstance()->IsEventBlocked(xev)) {
        return EventType::kUnknown;
    }

    if (auto* key = xev.As<x11::KeyEvent>()) {
        return key->opcode == x11::KeyEvent::Press ? EventType::kKeyPressed : EventType::kKeyReleased;
    }
    ...
    if (auto* motion = xev.As<x11::MotionNotifyEvent>()) {
        bool primary_button = static_cast<bool>(motion->state & (x11::KeyButMask::Button1 | x11::KeyButMask::Button2 | x11::KeyButMask::Button3));
    ...
    if (auto* xievent = xev.As<x11::Input::DeviceEvent>()) {
        ...
        case x11::Input::DeviceEvent::Motion: {
            ...
            if (GetFlingDataFromXEvent(xev, nullptr, nullptr, nullptr, nullptr, &is_cancel))
                return is_cancel ? EventType::kScrollFlingCancel : EventType::kScrollFlingStart;
            if (devices->IsScrollEvent(xev)) {
                return devices->IsTouchpadXInputEvent(xev) ? EventType::kScroll : EventType::kMousewheel;
            }
            ...
            if (DeviceDataManagerX11::GetInstance()->HasEventData(xev, DeviceDataManagerX11::DT_CMT_SCROLL_X)
                || DeviceDataManagerX11::GetInstance()->HasEventData(xev, DeviceDataManagerX11::DT_CMT_SCROLL_Y)) {
...
int GetEventFlagsFromXKeyEvent(const x11::KeyEvent& key, bool send_event)
{
    ...
    const auto shift_lock_mask = static_cast<int>(x11::KeyButMask::Shift | x11::KeyButMask::Lock);
    ...
    return GetEventFlagsFromXState(state) | (send_event ? ui::EF_FINAL : 0) | ime_fabricated_flag;
}
    ...
    if (auto* xievent = xev.As<x11::Input::DeviceEvent>()) {
        ...
        case x11::Input::DeviceEvent::TouchEnd:
            return GetButtonMaskForX2Event(*xievent) | GetEventFlagsFromXState(xievent->mods.effective)
                | GetEventFlagsFromXState(XModifierStateWatcher::GetInstance()->state());
        case x11::Input::DeviceEvent::ButtonPress:
        case x11::Input::DeviceEvent::ButtonRelease: {
            ...
            if (touch) {
                flags |= GetEventFlagsFromXState(XModifierStateWatcher::GetInstance()->state());
            }

            const EventType type = EventTypeFromXEvent(xev);
            int button = EventButtonFromXEvent(xev);
            if ((type == EventType::kMousePressed || type == EventType::kMouseReleased) && !touch) {
...
gfx::Point EventLocationFromXEvent(const x11::Event& xev)
{
    ...
    if (auto* xievent = xev.As<x11::Input::DeviceEvent>()) {
        ...
        case x11::Input::DeviceEvent::TouchUpdate:
        case x11::Input::DeviceEvent::TouchEnd:
            ui::DeviceDataManagerX11::GetInstance()->ApplyTouchTransformer(static_cast<uint16_t>(xievent->deviceid), &x, &y);
            break;
        default:
            break;
        }
...
gfx::Point EventSystemLocationFromXEvent(const x11::Event& xev)
{
    ...
    if (auto* crossing = xev.As<x11::Input::CrossingEvent>()) {
        return gfx::Point(Fp1616ToDouble(crossing->root_x), Fp1616ToDouble(crossing->root_y));
    }
    if (auto* xievent = xev.As<x11::Input::DeviceEvent>()) {
        return gfx::Point(Fp1616ToDouble(xievent->root_x), Fp1616ToDouble(xievent->root_y));
    }
    return gfx::Point();
}
...
int EventButtonFromXEvent(const x11::Event& xev)
{
    ...
    return (xievent->sourceid == xievent->deviceid) ? DeviceDataManagerX11::GetInstance()->GetMappedButton(button) : button;
}
...
int GetChangedMouseButtonFlagsFromXEvent(const x11::Event& xev)
{
    ...
    if (device && (device->opcode == x11::Input::DeviceEvent::ButtonPress || device->opcode == x11::Input::DeviceEvent::ButtonRelease)) {
...
gfx::Vector2d GetMouseWheelOffsetFromXEvent(const x11::Event& xev)
{
    ...
    if (GetScrollOffsetsFromXEvent(xev, &x_offset, &y_offset, nullptr, nullptr, nullptr)) {
        return gfx::Vector2d(static_cast<int>(x_offset), static_cast<int>(y_offset));
    }

    auto* device = xev.As<x11::Input::DeviceEvent>();
    int button = device ? EventButtonFromXEvent(xev) : static_cast<int>(xev.As<x11::ButtonEvent>()->detail);
    ...
    int scroll_class_type = DeviceDataManagerX11::GetInstance()->GetScrollClassDeviceDetail(xev);
...
float GetStylusForceFromXEvent(const x11::Event& x11_event)
{
    ...
    if (event->opcode == x11::Input::DeviceEvent::ButtonRelease)
        return 0.0;
    double force = GetParamFromXEvent(x11_event, ui::DeviceDataManagerX11::DT_STYLUS_PRESSURE, 0.0);
    auto deviceid = event->sourceid;
    // Force is normalized to fall into [0, 1]
    if (!ui::DeviceDataManagerX11::GetInstance()->NormalizeData(deviceid, ui::DeviceDataManagerX11::DT_STYLUS_PRESSURE, &force)) {
        force = 0.0;
    }
    return force;
}
...
{
    double tilt = GetParamFromXEvent(x11_event, ui::DeviceDataManagerX11::DT_STYLUS_TILT_X, 0.0);
    return std::clamp<float>(tilt, -90, 90);
}

float GetStylusTiltYFromXEvent(const x11::Event& x11_event)
{
    double tilt = GetParamFromXEvent(x11_event, ui::DeviceDataManagerX11::DT_STYLUS_TILT_Y, 0.0);
    return std::clamp<float>(tilt, -90, 90);
}

PointerDetails GetStylusPointerDetailsFromXEvent(const x11::Event& xev)
{
    if (!ui::DeviceDataManagerX11::HasInstance() || !ui::DeviceDataManagerX11::GetInstance()->IsStylusXInputEvent(xev)) {
...
int GetTouchIdFromXEvent(const x11::Event& xev)
{
    ...
    double tracking_id;
    if (!manager->GetEventData(xev, ui::DeviceDataManagerX11::DT_TOUCH_TRACKING_ID, &tracking_id)) {
...
{
    double radius = GetParamFromXEvent(xev, ui::DeviceDataManagerX11::DT_TOUCH_MAJOR, 0.0) / 2.0;
...
{
    double radius = GetParamFromXEvent(xev, ui::DeviceDataManagerX11::DT_TOUCH_MINOR, 0.0) / 2.0;
    ScaleTouchRadius(xev, &radius);
    return radius;
}
...
{
    return GetParamFromXEvent(xev, ui::DeviceDataManagerX11::DT_TOUCH_ORIENTATION, 0.0) / 2.0;
}
...
float GetTouchForceFromXEvent(const x11::Event& x11_event)
{
    ...
    if (event->opcode == x11::Input::DeviceEvent::TouchEnd)
        return 0.0;
    double force = 0.0;
    force = GetParamFromXEvent(x11_event, ui::DeviceDataManagerX11::DT_TOUCH_PRESSURE, 0.0);
    auto deviceid = event->sourceid;
    // Force is normalized to fall into [0, 1]
    if (!ui::DeviceDataManagerX11::GetInstance()->NormalizeData(deviceid, ui::DeviceDataManagerX11::DT_TOUCH_PRESSURE, &force))
        force = 0.0;
    return force;
}
...
PointerDetails GetTouchPointerDetailsFromXEvent(const x11::Event& xev)
{
    ...
    EventPointerType pointer_type = event ? ui::TouchFactory::GetInstance()->GetTouchDevicePointerType(event->sourceid) : EventPointerType::kTouch;
    return PointerDetails(pointer_type, GetTouchIdFromXEvent(xev), GetTouchRadiusXFromXEvent(xev), GetTouchRadiusYFromXEvent(xev), GetTouchForceFromXEvent(xev),
        GetTouchAngleFromXEvent(xev));
}

bool GetScrollOffsetsFromXEvent(const x11::Event& xev, float* x_offset, float* y_offset, float* x_offset_ordinal, float* y_offset_ordinal, int* finger_count)
{
    ...
    if (DeviceDataManagerX11::GetInstance()->IsScrollEvent(xev)) {
        DeviceDataManagerX11::GetInstance()->GetScrollOffsets(xev, x_offset, y_offset, x_offset_ordinal, y_offset_ordinal, finger_count);
        return true;
    }

    if (DeviceDataManagerX11::GetInstance()->GetScrollClassEventDetail(xev) != SCROLL_TYPE_NO_SCROLL) {
        double x_scroll_offset_dbl, y_scroll_offset_dbl;
        DeviceDataManagerX11::GetInstance()->GetScrollClassOffsets(xev, &x_scroll_offset_dbl, &y_scroll_offset_dbl);
...
bool GetFlingDataFromXEvent(const x11::Event& xev, float* vx, float* vy, float* vx_ordinal, float* vy_ordinal, bool* is_cancel)
{
    ...
    DeviceDataManagerX11::GetInstance()->GetFlingData(xev, vx, vy, vx_ordinal, vy_ordinal, is_cancel);
    return true;
}

bool IsAltPressed()
{
    return XModifierStateWatcher::GetInstance()->state() & static_cast<int>(x11::KeyButMask::Mod1);
}
