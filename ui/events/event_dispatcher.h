...
public:
    ...
    // dispatched.
    [[nodiscard]] EventDispatchDetails DispatchEvent(EventTarget* target, Event* event);
...
protected:
    ...
    [[nodiscard]] virtual EventDispatchDetails PreDispatchEvent(EventTarget* target, Event* event);

    // This is called right after the event dispatch is completed.
    // |target| is NULL if the target was deleted during dispatch.
    [[nodiscard]] virtual EventDispatchDetails PostDispatchEvent(EventTarget* target, const Event& event);

private:
    // Dispatches the event to the target.
    [[nodiscard]] EventDispatchDetails DispatchEventToTarget(EventTarget* target, Event* event);
