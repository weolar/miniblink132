...
void ExpectTouchEventsEqual(const TouchEvent& expected, const TouchEvent& actual)
{
...
void ExpectLocatedEventsEqual(const LocatedEvent& expected, const LocatedEvent& actual)
{
    EXPECT_EQ(expected.location_f(), actual.location_f());
    EXPECT_EQ(expected.root_location_f(), actual.root_location_f());
}

void ExpectMouseEventsEqual(const MouseEvent& expected, const MouseEvent& actual)
{
    EXPECT_EQ(expected.pointer_details(), actual.pointer_details());
    EXPECT_EQ(expected.changed_button_flags(), actual.changed_button_flags());
}

void ExpectMouseWheelEventsEqual(const MouseWheelEvent& expected, const MouseWheelEvent& actual)
{
...
void ExpectKeyEventsEqual(const KeyEvent& expected, const KeyEvent& actual)
{
    ...
    EXPECT_EQ(expected.GetConflatedWindowsKeyCode(), actual.GetConflatedWindowsKeyCode());
...
void ExpectEventsEqual(const Event& expected, const Event& actual)
{
    ...
    if (expected.IsLocatedEvent()) {
        ...
        ExpectLocatedEventsEqual(*expected.AsLocatedEvent(), *actual.AsLocatedEvent());
    }
    ...
    if (expected.IsMouseWheelEvent()) {
        ...
        ExpectMouseWheelEventsEqual(*expected.AsMouseWheelEvent(), *actual.AsMouseWheelEvent());
    }
...
TEST(StructTraitsTest, KeyEvent)
{
    ...
    const KeyEvent kTestData[] = {
        ...
        { EventType::kKeyPressed, VKEY_B, DomCode::US_B, EF_CONTROL_DOWN | EF_ALT_DOWN },
        ui::KeyEvent::FromCharacter('\x12', VKEY_2, DomCode::NONE, EF_CONTROL_DOWN),
        ui::KeyEvent::FromCharacter('Z', VKEY_Z, DomCode::NONE, EF_CAPS_LOCK_ON),
        ui::KeyEvent::FromCharacter('z', VKEY_Z, DomCode::NONE, EF_NONE),
        { EventType::kKeyPressed, VKEY_Z, EF_NONE, base::TimeTicks() + base::Microseconds(101) },
        ui::KeyEvent::FromCharacter('Z', VKEY_Z, DomCode::NONE, EF_NONE, base::TimeTicks() + base::Microseconds(102)),
    };

    for (size_t i = 0; i < std::size(kTestData); i++) {
        std::unique_ptr<Event> expected_copy = kTestData[i].Clone();
        std::unique_ptr<Event> output;
        ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::Event>(expected_copy, output));
...
TEST(StructTraitsTest, MouseEvent)
{
    const MouseEvent kTestData[] = {
        { EventType::kMousePressed, gfx::Point(10, 10), gfx::Point(20, 30), base::TimeTicks() + base::Microseconds(201), EF_NONE, 0,
            PointerDetails(EventPointerType::kMouse, kPointerIdMouse) },
        { EventType::kMouseDragged, gfx::Point(1, 5), gfx::Point(5, 1), base::TimeTicks() + base::Microseconds(202), EF_LEFT_MOUSE_BUTTON, EF_LEFT_MOUSE_BUTTON,
            PointerDetails(EventPointerType::kMouse, kPointerIdMouse) },
        { EventType::kMouseReleased, gfx::Point(411, 130), gfx::Point(20, 30), base::TimeTicks() + base::Microseconds(203),
            EF_MIDDLE_MOUSE_BUTTON | EF_RIGHT_MOUSE_BUTTON, EF_RIGHT_MOUSE_BUTTON, PointerDetails(EventPointerType::kMouse, kPointerIdMouse) },
        { EventType::kMouseMoved, gfx::Point(0, 1), gfx::Point(2, 3), base::TimeTicks() + base::Microseconds(204), EF_ALT_DOWN, 0,
            PointerDetails(EventPointerType::kMouse, kPointerIdMouse) },
        { EventType::kMouseEntered, gfx::Point(6, 7), gfx::Point(8, 9), base::TimeTicks() + base::Microseconds(205), EF_NONE, 0,
            PointerDetails(EventPointerType::kMouse, kPointerIdMouse) },
        { EventType::kMouseExited, gfx::Point(10, 10), gfx::Point(20, 30), base::TimeTicks() + base::Microseconds(206), EF_BACK_MOUSE_BUTTON, 0,
            PointerDetails(EventPointerType::kMouse, kPointerIdMouse) },
        { EventType::kMouseCaptureChanged, gfx::Point(99, 99), gfx::Point(99, 99), base::TimeTicks() + base::Microseconds(207), EF_CONTROL_DOWN, 0,
            PointerDetails(EventPointerType::kMouse, kPointerIdMouse) },
    };
...
TEST(StructTraitsTest, MouseWheelEvent)
{
    const MouseWheelEvent kTestData[] = {
        { gfx::Vector2d(11, 15), gfx::PointF(3, 4), gfx::PointF(40, 30), base::TimeTicks() + base::Microseconds(301), EF_LEFT_MOUSE_BUTTON,
            EF_LEFT_MOUSE_BUTTON, gfx::Vector2d(1320, 1800) },
        { gfx::Vector2d(-5, 3), gfx::PointF(40, 3), gfx::PointF(4, 0), base::TimeTicks() + base::Microseconds(302),
            EF_MIDDLE_MOUSE_BUTTON | EF_RIGHT_MOUSE_BUTTON, EF_MIDDLE_MOUSE_BUTTON | EF_RIGHT_MOUSE_BUTTON, gfx::Vector2d(-600, 360) },
        { gfx::Vector2d(1, 0), gfx::PointF(3, 4), gfx::PointF(40, 30), base::TimeTicks() + base::Microseconds(303), EF_NONE, EF_NONE, gfx::Vector2d(120, -15) },
    };
...
TEST(StructTraitsTest, FloatingPointLocations)
{
    ...
    MouseEvent mouse_event(EventType::kMousePressed, location, root_location, time_stamp, EF_NONE, EF_NONE);
    MouseWheelEvent wheel_event(gfx::Vector2d(1, 0), location, root_location, time_stamp, EF_NONE, EF_NONE);
    ScrollEvent scroll_event(EventType::kScroll, location, root_location, time_stamp, EF_NONE, 1, 2, 3, 4, 5);
    TouchEvent touch_event(EventType::kTouchPressed, location, root_location, time_stamp, {}, EF_NONE);
    Event* test_data[] = { &mouse_event, &wheel_event, &scroll_event, &touch_event };
    ...
    for (Event* event : test_data) {
        ...
        std::unique_ptr<Event> output;
        ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::Event>(event_copy, output));
...
TEST(StructTraitsTest, KeyEventPropertiesSerialized)
{
    ...
    ASSERT_TRUE(mojom::Event::Deserialize(mojom::Event::Serialize(&event_ptr), &deserialized));
...
TEST(StructTraitsTest, GestureEvent)
{
    ...
    swipe_top_left_details.set_device_type(ui::GestureDeviceType::DEVICE_TOUCHPAD);
    ...
    const GestureEvent kTestData[] = {
        { 10, 20, EF_NONE, base::TimeTicks() + base::Microseconds(401), GestureEventDetails(EventType::kScrollFlingStart) },
        { 10, 20, EF_NONE, base::TimeTicks() + base::Microseconds(401), GestureEventDetails(EventType::kGestureTap) },
        { 10, 20, EF_NONE, base::TimeTicks() + base::Microseconds(401), pinch_begin_details },
        { 10, 20, EF_NONE, base::TimeTicks() + base::Microseconds(401), pinch_end_details },
        { 10, 20, EF_NONE, base::TimeTicks() + base::Microseconds(401), pinch_update_details },
        { 10, 20, EF_NONE, base::TimeTicks() + base::Microseconds(401), swipe_top_left_details },
        { 10, 20, EF_NONE, base::TimeTicks() + base::Microseconds(401), swipe_right_details },
    };
    ...
    for (size_t i = 0; i < std::size(kTestData); i++) {
        ...
        EXPECT_EQ(kTestData[i].unique_touch_event_id(), output_ptr_event->unique_touch_event_id());
    }
}
...
TEST(StructTraitsTest, ScrollEvent)
{
    const ScrollEvent kTestData[] = {
        { EventType::kScroll, gfx::Point(10, 20), base::TimeTicks() + base::Microseconds(501), EF_NONE, 1, 2, 3, 4, 5, EventMomentumPhase::NONE,
            ScrollEventPhase::kNone },
        { EventType::kScroll, gfx::Point(10, 20), base::TimeTicks() + base::Microseconds(501), EF_NONE, 1, 2, 3, 4, 5, EventMomentumPhase::NONE,
            ScrollEventPhase::kUpdate },
        { EventType::kScroll, gfx::Point(10, 20), base::TimeTicks() + base::Microseconds(501), EF_NONE, 1, 2, 3, 4, 5, EventMomentumPhase::NONE,
            ScrollEventPhase::kBegan },
        { EventType::kScroll, gfx::Point(10, 20), base::TimeTicks() + base::Microseconds(501), EF_NONE, 1, 2, 3, 4, 5, EventMomentumPhase::NONE,
            ScrollEventPhase::kEnd },
        { EventType::kScroll, gfx::Point(10, 20), base::TimeTicks() + base::Microseconds(501), EF_NONE, 1, 2, 3, 4, 5, EventMomentumPhase::BEGAN,
            ScrollEventPhase::kNone },
        { EventType::kScroll, gfx::Point(10, 20), base::TimeTicks() + base::Microseconds(501), EF_NONE, 1, 2, 3, 4, 5, EventMomentumPhase::INERTIAL_UPDATE,
            ScrollEventPhase::kNone },
        { EventType::kScroll, gfx::Point(10, 20), base::TimeTicks() + base::Microseconds(501), EF_NONE, 1, 2, 3, 4, 5, EventMomentumPhase::END,
            ScrollEventPhase::kNone },
        { EventType::kScrollFlingStart, gfx::Point(10, 20), base::TimeTicks() + base::Microseconds(502), EF_NONE, 1, 2, 3, 4, 5, EventMomentumPhase::MAY_BEGIN,
            ScrollEventPhase::kNone },
        { EventType::kScrollFlingCancel, gfx::Point(10, 20), base::TimeTicks() + base::Microseconds(502), EF_NONE, 1, 2, 3, 4, 5, EventMomentumPhase::END,
            ScrollEventPhase::kNone },
    };
    ...
    for (size_t i = 0; i < std::size(kTestData); i++) {
        ...
        EXPECT_EQ(kTestData[i].x_offset_ordinal(), output_ptr_event->x_offset_ordinal());
        EXPECT_EQ(kTestData[i].y_offset_ordinal(), output_ptr_event->y_offset_ordinal());
        EXPECT_EQ(kTestData[i].finger_count(), output_ptr_event->finger_count());
        EXPECT_EQ(kTestData[i].momentum_phase(), output_ptr_event->momentum_phase());
    }
}
...
TEST(StructTraitsTest, PointerDetails)
{
    ...
    for (size_t i = 0; i < std::size(kTestData); i++) {
        ...
        ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::PointerDetails>(input, output));
...
TEST(StructTraitsTest, TouchEvent)
{
    const TouchEvent kTestData[] = {
        { EventType::kTouchReleased, { 1, 2 }, base::TimeTicks::Now(), { EventPointerType::kUnknown, 1, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f }, EF_SHIFT_DOWN },
        { EventType::kTouchPressed, { 1, 2 }, base::TimeTicks::Now(), {}, EF_CONTROL_DOWN },
    ...
    std::unique_ptr<TouchEvent> touch_event = std::make_unique<TouchEvent>(EventType::kTouchCancelled, gfx::Point(), base::TimeTicks::Now(), PointerDetails());
    touch_event->set_may_cause_scrolling(true);
    touch_event->set_hovering(true);
    std::unique_ptr<Event> expected = std::move(touch_event);
    std::unique_ptr<Event> output;
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::Event>(expected, output));
...
TEST(StructTraitsTest, UnserializedTouchEventFields)
{
    ...
    ASSERT_TRUE(mojo::test::SerializeAndDeserialize<mojom::Event>(expected, output));
    ExpectEventsEqual(*expected, *output);
    EXPECT_NE(expected->AsTouchEvent()->unique_event_id(), output->AsTouchEvent()->unique_event_id());
}
...
public:
    ...
    FixedKeyboardLayoutEngine& operator=(const FixedKeyboardLayoutEngine&) = delete;

    ~FixedKeyboardLayoutEngine() override = default;

    // StubKeyboardLayoutEngine:
    bool Lookup(DomCode dom_code, int flags, DomKey* out_dom_key, KeyboardCode* out_key_code) const override
    {
...
TEST(StructTraitsTest, DifferentKeyboardLayout)
{
    ...
    ScopedKeyboardLayoutEngine scoped_keyboard_layout_engine(std::make_unique<FixedKeyboardLayoutEngine>());
    std::unique_ptr<KeyEvent> key_event
        = std::make_unique<KeyEvent>(EventType::kKeyPressed, VKEY_S, DomCode::US_S, EF_NONE, DomKey::FromCharacter('s'), base::TimeTicks::Now());
