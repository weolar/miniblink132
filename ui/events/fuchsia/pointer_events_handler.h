...
struct InteractionLess {
    bool operator()(const fuchsia_ui_pointer::TouchInteractionId& interaction_id, const fuchsia_ui_pointer::TouchInteractionId& other_interaction_id) const
    {
        return (std::hash<uint32_t>()(interaction_id.device_id()) ^ std::hash<uint32_t>()(interaction_id.pointer_id())
                   ^ std::hash<uint32_t>()(interaction_id.interaction_id()))
            < (std::hash<uint32_t>()(other_interaction_id.device_id()) ^ std::hash<uint32_t>()(other_interaction_id.pointer_id())
...
public:
    PointerEventsHandler(fidl::ClientEnd<fuchsia_ui_pointer::TouchSource> touch_source, fidl::ClientEnd<fuchsia_ui_pointer::MouseSource> mouse_source);
...
private:
    ...
    void OnTouchSourceWatchResult(fidl::Result<fuchsia_ui_pointer::TouchSource::Watch>& result);
    void OnMouseSourceWatchResult(fidl::Result<fuchsia_ui_pointer::MouseSource::Watch>& result);
    ...
    fit::function<void(std::vector<fuchsia_ui_pointer::TouchEvent>)> touch_responder_;
    ...
    // interaction.
    base::flat_map<fuchsia_ui_pointer::TouchInteractionId, std::vector<TouchEvent>, InteractionLess> touch_buffer_;
    ...
    fit::function<void(std::vector<fuchsia_ui_pointer::MouseEvent>)> mouse_responder_;
    ...
    base::flat_map<MouseDeviceId, fuchsia_ui_pointer::MouseDeviceInfo> mouse_device_info_;
