...
template <typename T, class... Args> std::vector<T> MakeVector(Args&&... args)
{
    std::vector<T> output;
...
protected:
        ...
        , fake_mouse_source_binding_(&fake_mouse_source_)
    {
        pointer_handler_ = std::make_unique<PointerEventsHandler>(
            fidl::HLCPPToNatural(fake_touch_source_binding_.NewBinding()), fidl::HLCPPToNatural(fake_mouse_source_binding_.NewBinding()));
    }

    ~PointerEventsHandlerTest() override
    {
...
TEST_F(PointerEventsHandlerTest, Watch_EventCallbacksAreIndependent)
{
    ...
    pointer_handler_->StartWatching(base::BindLambdaForTesting([&events](Event* event) { events.push_back(event->Clone()); }));
    RunLoopUntilIdle(); // Server gets watch call.

    std::vector touch_events = MakeVector<fup::TouchEvent>(TouchEventBuilder().SetTouchInteractionStatus(fup::TouchInteractionStatus::kGranted).Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(touch_events)));
    RunLoopUntilIdle();

    ASSERT_EQ(events.size(), 1u);
    EXPECT_TRUE(events[0]->IsTouchEvent());
    EXPECT_EQ(events[0]->AsTouchEvent()->pointer_details().pointer_type, EventPointerType::kTouch);

    std::vector mouse_events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetPressedButtons({ 0 }).Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(mouse_events)));
    RunLoopUntilIdle();

    ASSERT_EQ(events.size(), 2u);
    EXPECT_TRUE(events[1]->IsMouseEvent());
    EXPECT_EQ(events[1]->AsMouseEvent()->pointer_details().pointer_type, EventPointerType::kMouse);
}

TEST_F(PointerEventsHandlerTest, Data_FuchsiaTimeVersusChromeTime)
{
    std::vector<TouchEvent> touch_events;
    pointer_handler_->StartWatching(base::BindLambdaForTesting([&touch_events](Event* event) { touch_events.push_back(*event->AsTouchEvent()); }));
    RunLoopUntilIdle(); // Server gets watch call.

    std::vector events = MakeVector<fup::TouchEvent>(
        TouchEventBuilder().SetTime(zx::time { 1111783u }).SetTouchInteractionStatus(fup::TouchInteractionStatus::kGranted).Build());
...
TEST_F(PointerEventsHandlerTest, Phase_ChromeMouseEventTypesAreSynthesized)
{
    ...
    pointer_handler_->StartWatching(base::BindLambdaForTesting([&mouse_events](Event* event) { mouse_events.push_back(*event->AsMouseEvent()); }));
    RunLoopUntilIdle(); // Server gets watch call.

    // Fuchsia button press -> Chrome EventType::kMousePressed and
    // EF_RIGHT_MOUSE_BUTTON
    std::vector events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetPressedButtons({ 0 }).SetButtons({ 2, 0, 1 }).Build());
    ...
    // EF_RIGHT_MOUSE_BUTTON
    events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetPressedButtons({ 0 }).WithoutViewParameters().WithoutDeviceInfo().Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
    ...
    events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetPressedButtons({}).WithoutViewParameters().WithoutDeviceInfo().Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, Phase_ChromeMouseEventFlagsAreSynthesized)
{
    ...
    events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetPressedButtons({ 2 }).WithoutViewParameters().WithoutDeviceInfo().Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, Phase_ChromeMouseEventFlagCombo)
{
    ...
    std::vector events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetPressedButtons({ 0, 1 }).Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, ChromeMouseEventDoubleClick)
{
    ...
    std::vector events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetPressedButtons({ 0 }).IncrementTime().Build(),
        MouseEventBuilder().SetPressedButtons({}).IncrementTime().Build(), MouseEventBuilder().SetPressedButtons({ 0 }).IncrementTime().Build(),
...
TEST_F(PointerEventsHandlerTest, MouseMultiButtonDrag)
{
    ...
    std::vector events = MakeVector<fup::MouseEvent>(
        ...
        MouseEventBuilder().SetPosition({ 10.f, 10.f }).SetPressedButtons({ 0, 1 }).IncrementTime().Build(),
        // drag with left, right button pressing.
        MouseEventBuilder().SetPosition({ 11.f, 10.f }).SetPressedButtons({ 0, 1 }).IncrementTime().Build(),
        // right button up.
        MouseEventBuilder().SetPosition({ 11.f, 10.f }).SetPressedButtons({ 0 }).IncrementTime().Build(),
        // drag with left button pressing.
        MouseEventBuilder().SetPosition({ 11.f, 11.f }).SetPressedButtons({ 0 }).IncrementTime().Build(),
        // left button up.
    ...
    EXPECT_EQ(mouse_events[2].flags(), EF_LEFT_MOUSE_BUTTON | EF_RIGHT_MOUSE_BUTTON);
    EXPECT_EQ(mouse_events[3].type(), EventType::kMouseReleased);
...
TEST_F(PointerEventsHandlerTest, MouseWheelEvent)
{
    ...
    pointer_handler_->StartWatching(base::BindLambdaForTesting([&mouse_events](Event* event) {
    ...
    std::vector events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetScroll({ 0, 1 }).Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
    ...
    events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetScroll({ 1, 0 }).Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, MouseWheelEventDeltaInPhysicalPixel)
{
    ...
    // receive a vertical scroll
    std::vector events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetScroll({ 0, 1 }).SetScrollInPhysicalPixel({ 0, 100 }).Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
    ...
    // receive a horizontal scroll
    events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetScroll({ 1, 0 }).SetScrollInPhysicalPixel({ 100, 0 }).Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, ScrollEventDeltaInPhysicalPixel)
{
    ...
    // receive a vertical scroll
    std::vector events
        = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetScroll({ 0, 1 }).SetScrollInPhysicalPixel({ 0, 100 }).SetIsPrecisionScroll(true).Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
    ...
    // receive a horizontal scroll
    events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetScroll({ 1, 0 }).SetScrollInPhysicalPixel({ 100, 0 }).SetIsPrecisionScroll(true).Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, ScrollEventDeltaInPhysicalPixelNoTickDelta)
{
    ...
    // receive a vertical scroll
    std::vector events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetScrollInPhysicalPixel({ 0, 100 }).SetIsPrecisionScroll(true).Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
    ...
    // receive a horizontal scroll
    events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetScrollInPhysicalPixel({ 100, 0 }).SetIsPrecisionScroll(true).Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, MouseWheelEventWithButtonPressed)
{
    ...
    std::vector events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetPressedButtons({ 0 }).IncrementTime().Build(),
        ...
        MouseEventBuilder().SetPressedButtons({ 0 }).SetScroll({ 0, 1 }).IncrementTime().Build());
    fake_mouse_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
...
TEST_F(PointerEventsHandlerTest, MouseWheelEventWithButtonDownBundled)
{
    ...
    std::vector events = MakeVector<fup::MouseEvent>(MouseEventBuilder().SetPressedButtons({ 0 }).SetScroll({ 0, 1 }).Build());
...
TEST_F(PointerEventsHandlerTest, Phase_ChromeTouchEventTypesAreSynthesized)
{
    ...
    std::vector events = MakeVector<fup::TouchEvent>(
        TouchEventBuilder().SetTime(zx::time { 1111000u }).SetTouchInteractionStatus(fup::TouchInteractionStatus::kGranted).Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
    ...
    events = MakeVector<fup::TouchEvent>(TouchEventBuilder().SetTime(zx::time { 2222000u }).SetPhase(fup::EventPhase::kChange).Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
    ...
    events = MakeVector<fup::TouchEvent>(TouchEventBuilder().SetTime(zx::time { 3333000u }).SetPhase(fup::EventPhase::kRemove).Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, Phase_FuchsiaCancelBecomesChromeCancel)
{
    ...
    events = MakeVector<fup::TouchEvent>(TouchEventBuilder().SetTime(zx::time { 2222000u }).SetPhase(fup::EventPhase::kCancel).Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, Coordinates_CorrectMapping)
{
    ...
    std::vector events = MakeVector<fup::TouchEvent>(TouchEventBuilder()
...
TEST_F(PointerEventsHandlerTest, Coordinates_PressedEventClampedToView)
{
    ...
    std::vector events = MakeVector<fup::TouchEvent>(
        TouchEventBuilder().SetPosition({ 10.f, kSmallDiscrepancy }).SetTouchInteractionStatus(fup::TouchInteractionStatus::kGranted).Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
{
    bool called = false;
    pointer_handler_->StartWatching(base::BindLambdaForTesting([&called](Event*) { called = true; }));
    RunLoopUntilIdle(); // Server gets Watch call.
...
TEST_F(PointerEventsHandlerTest, Protocol_ResponseMatchesEarlierEvents)
{
    ...
    std::vector events = MakeVector<fup::TouchEvent>(
        ...
        TouchEventBuilder().SetId({ { .device_id = 0u, .pointer_id = 1u, .interaction_id = 3u } }).SetPosition({ 10.f, 10.f }).Build(),

        // Fuchsia ptr 2 ADD sample. Yes response.
        TouchEventBuilder().SetId({ { .device_id = 0u, .pointer_id = 2u, .interaction_id = 3u } }).SetPosition({ 5.f, 5.f }).Build(),

        // Fuchsia ptr 3 ADD sample. Yes response.
        TouchEventBuilder().SetId({ { .device_id = 0u, .pointer_id = 3u, .interaction_id = 3u } }).SetPosition({ 1.f, 1.f }).Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();

    auto hlcpp_responses = fake_touch_source_.UploadedResponses();
    ASSERT_TRUE(hlcpp_responses.has_value());
    const std::vector<fuchsia_ui_pointer::TouchResponse> responses = fidl::HLCPPToNatural(hlcpp_responses.value());
...
TEST_F(PointerEventsHandlerTest, Protocol_LateGrant)
{
    ...
    std::vector events = MakeVector<fup::TouchEvent>(TouchEventBuilder().SetTime(zx::time { 1111000u }).Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
    ...
    events = MakeVector<fup::TouchEvent>(
        TouchEventBuilder().SetTime(zx::time { 3333000u }).SetTouchInteractionStatus(fup::TouchInteractionStatus::kGranted).WithoutSample().Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
    ...
    events = MakeVector<fup::TouchEvent>(TouchEventBuilder().SetTime(zx::time { 4444000u }).SetPhase(fup::EventPhase::kChange).Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, Protocol_LateGrantCombo)
{
    ...
    // Fuchsia ADD, no grant result - buffer it.
    std::vector events = MakeVector<fup::TouchEvent>(TouchEventBuilder().SetTime(zx::time { 1111000u }).SetPhase(fup::EventPhase::kAdd).Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, Protocol_LateDeny)
{
    ...
    events = MakeVector<fup::TouchEvent>(
        TouchEventBuilder().SetTime(zx::time { 3333000u }).SetTouchInteractionStatus(fup::TouchInteractionStatus::kDenied).WithoutSample().Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
...
TEST_F(PointerEventsHandlerTest, Protocol_PointersAreIndependent)
{
    ...
    const fup::TouchInteractionId kIxnTwo = { { .device_id = 1u, .pointer_id = 2u, .interaction_id = 2u } };

    // Fuchsia ptr1 ADD and ptr2 ADD, no grant result for either - buffer them.
    std::vector events = MakeVector<fup::TouchEvent>(TouchEventBuilder().SetTime(zx::time { 1111000u }).SetPhase(fup::EventPhase::kAdd).Build(),
        TouchEventBuilder().SetTime(zx::time { 1111000u }).SetId(kIxnTwo).SetPhase(fup::EventPhase::kAdd).SetPosition({ 15.f, 15.f }).Build());
    fake_touch_source_.ScheduleCallback(fidl::NaturalToHLCPP(std::move(events)));
    RunLoopUntilIdle();
