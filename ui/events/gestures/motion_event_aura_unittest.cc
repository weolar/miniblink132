...
{
    return ui::TouchEvent(type, gfx::Point(0, 0), base::TimeTicks(), ui::PointerDetails(ui::EventPointerType::kTouch, id));
}

ui::TouchEvent TouchWithPosition(ui::EventType type, int id, float x, float y, float raw_x, float raw_y)
{
    ui::TouchEvent event(type, gfx::Point(), base::TimeTicks(), ui::PointerDetails(ui::EventPointerType::kTouch, id));
    event.set_location_f(gfx::PointF(x, y));
    event.set_root_location_f(gfx::PointF(raw_x, raw_y));
    return event;
}

ui::TouchEvent TouchWithTapParams(ui::EventType type, int id, float radius_x, float radius_y, float rotation_angle, float pressure)
{
    ui::TouchEvent event(
        type, gfx::Point(1, 1), base::TimeTicks(), ui::PointerDetails(ui::EventPointerType::kTouch, id, radius_x, radius_y, pressure, rotation_angle), 0);
    return event;
}
...
{
    return ui::TouchEvent(type, gfx::Point(0, 0), MsToTicks(ms), ui::PointerDetails(ui::EventPointerType::kTouch, id));
}
...
TEST(MotionEventAuraTest, PointerLocations)
{
    ...
    raw_x = x + kRawOffsetX;
    raw_y = y + kRawOffsetY;
    TouchEvent press0 = TouchWithPosition(EventType::kTouchPressed, ids[0], x, y, raw_x, raw_y);
    EXPECT_TRUE(event.OnTouch(press0));
    ...
    raw_x = x + kRawOffsetX;
    raw_y = y + kRawOffsetY;
    TouchEvent press1 = TouchWithPosition(EventType::kTouchPressed, ids[1], x, y, raw_x, raw_y);
    EXPECT_TRUE(event.OnTouch(press1));
    ...
    raw_x = x + kRawOffsetX;
    raw_y = y + kRawOffsetY;
    TouchEvent move1 = TouchWithPosition(EventType::kTouchMoved, ids[1], x, y, raw_x, raw_y);
    ...
    raw_x = x + kRawOffsetX;
    raw_y = y + kRawOffsetY;
    TouchEvent move0 = TouchWithPosition(EventType::kTouchMoved, ids[0], x, y, raw_x, raw_y);
...
TEST(MotionEventAuraTest, TapParams)
{
    ...
    rotation_angle = 23.f;
    pressure = 0.123f;
    TouchEvent press0 = TouchWithTapParams(EventType::kTouchPressed, ids[0], radius_x, radius_y, rotation_angle, pressure);
    EXPECT_TRUE(event.OnTouch(press0));
    ...
    rotation_angle = 46.f;
    pressure = 0.456f;
    TouchEvent press1 = TouchWithTapParams(EventType::kTouchPressed, ids[1], radius_x, radius_y, rotation_angle, pressure);
    EXPECT_TRUE(event.OnTouch(press1));
    ...
    rotation_angle = 64.f;
    pressure = 0.654f;
    TouchEvent move1 = TouchWithTapParams(EventType::kTouchMoved, ids[1], radius_x, radius_y, rotation_angle, pressure);
    ...
    rotation_angle = 92.f;
    pressure = 0.789f;
    TouchEvent press2 = TouchWithTapParams(EventType::kTouchPressed, ids[2], radius_x, radius_y, rotation_angle, pressure);
    EXPECT_TRUE(event.OnTouch(press2));
    ...
    pressure = 0.012f;
    TouchEvent press3 = TouchWithTapParams(EventType::kTouchPressed, ids[3], radius_x, radius_y, rotation_angle, pressure);
    ...
    EXPECT_FLOAT_EQ(rotation_angle, base::RadToDeg(event.GetOrientation(3)) + 180);
...
TEST(MotionEventAuraTest, Timestamps)
{
    ...
    TouchEvent press0 = TouchWithTime(ui::EventType::kTouchPressed, ids[0], times_in_ms[0]);
    EXPECT_TRUE(event.OnTouch(press0));
    EXPECT_EQ(MsToTicks(times_in_ms[0]), event.GetEventTime());

    TouchEvent press1 = TouchWithTime(ui::EventType::kTouchPressed, ids[1], times_in_ms[1]);
    EXPECT_TRUE(event.OnTouch(press1));
    EXPECT_EQ(MsToTicks(times_in_ms[1]), event.GetEventTime());

    TouchEvent move0 = TouchWithTime(ui::EventType::kTouchMoved, ids[0], times_in_ms[2]);
...
TEST(MotionEventAuraTest, IgnoresRedundantPresses)
{
    ...
    MotionEventAura event;
    TouchEvent press1 = TouchWithPosition(EventType::kTouchPressed, id, position_1, position_1, position_1, position_1);
    EXPECT_TRUE(event.OnTouch(press1));
    TouchEvent press2 = TouchWithPosition(EventType::kTouchPressed, id, position_2, position_2, position_2, position_2);
...
TEST(MotionEventAuraTest, IgnoresStationaryMoves)
{
    ...
    TouchEvent move0 = TouchWithPosition(EventType::kTouchMoved, id, 10, 20, 10, 20);
    EXPECT_TRUE(event.OnTouch(move0));

    TouchEvent move1 = TouchWithPosition(EventType::kTouchMoved, id, 11, 21, 11, 21);
    EXPECT_TRUE(event.OnTouch(move1));
...
{
    MotionEventAura event;
    TouchEvent touch_event = ui::TouchEvent(EventType::kTouchPressed, gfx::Point(0, 0), base::TimeTicks(), ui::PointerDetails(ui::EventPointerType::kPen, 7));
    EXPECT_TRUE(event.OnTouch(touch_event));
    EXPECT_EQ(MotionEvent::ToolType::STYLUS, event.GetToolType(0));
