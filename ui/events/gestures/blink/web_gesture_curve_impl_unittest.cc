...
TEST(WebGestureCurveImplTest, Basic)
{
    ...
    base::TimeTicks time;
    auto curve = WebGestureCurveImpl::CreateFromUICurveForTesting(std::unique_ptr<ui::GestureCurve>(new ui::FlingCurve(velocity, time)), offset);
    ...
    EXPECT_TRUE(curve->Advance(0.45, current_velocity,
