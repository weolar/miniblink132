...
namespace aura::test {
FORWARD_DECLARE_TEST(GestureRecognizerTest, DestroyGestureProviderAuraBeforeAck);
FORWARD_DECLARE_TEST(GestureRecognizerTest, ResetGestureRecognizerWithGestureProvider);
} // namespace aura::test
...
// significantly.
class EVENTS_EXPORT GestureRecognizerImpl : public GestureRecognizer, public GestureProviderAuraClient {
public:
    typedef std::map<int, raw_ptr<GestureConsumer, CtnExperimental>> TouchIdToConsumerMap;
    ...
    std::vector<std::unique_ptr<TouchEvent>> GetEventPerPointForConsumer(GestureConsumer* consumer, EventType type);

    // Overridden from GestureRecognizer
    GestureConsumer* GetTouchLockedTarget(const TouchEvent& event) override;
    GestureConsumer* GetTargetForLocation(const gfx::PointF& location, int source_device_id) override;
    void CancelActiveTouchesExcept(GestureConsumer* not_cancelled) override;
    void CancelActiveTouchesOn(const std::vector<GestureConsumer*>& consumers) override;
    void TransferEventsTo(GestureConsumer* current_consumer, GestureConsumer* new_consumer, TransferTouchesBehavior transfer_touches_behavior) override;
    bool GetLastTouchPointForTarget(GestureConsumer* consumer, gfx::PointF* point) override;
    bool CancelActiveTouches(GestureConsumer* consumer) override;

protected:
    virtual GestureProviderAura* GetGestureProviderForConsumer(GestureConsumer* c);

    // Overridden from GestureRecognizer
    bool ProcessTouchEventPreDispatch(TouchEvent* event, GestureConsumer* consumer) override;

private:
    FRIEND_TEST_ALL_PREFIXES(aura::test::GestureRecognizerTest, DestroyGestureProviderAuraBeforeAck);
    FRIEND_TEST_ALL_PREFIXES(aura::test::GestureRecognizerTest, ResetGestureRecognizerWithGestureProvider);

    // Sets up the target consumer for gestures based on the touch-event.
    void SetupTargets(const TouchEvent& event, GestureConsumer* consumer);

    void DispatchGestureEvent(GestureConsumer* raw_input_consumer, GestureEvent* event);

    Gestures AckTouchEvent(uint32_t unique_event_id, ui::EventResult result, bool is_source_touch_event_set_blocking, GestureConsumer* consumer) override;
    ...
    void OnGestureProviderAuraWillBeDestroyed(GestureProviderAura* gesture_provider) override;
    ...
    std::map<uint32_t, raw_ptr<GestureProviderAura, CtnExperimental>> event_to_gesture_provider_;
