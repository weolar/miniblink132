...
public:
    ...
    virtual void OnGestureEvent(GestureConsumer* consumer, GestureEvent* event) = 0;

    // Called when `gesture_provider` will be destroyed.
    virtual void OnGestureProviderAuraWillBeDestroyed(GestureProviderAura* gesture_provider)
    {
    }
};
...
public:
    GestureProviderAura(GestureConsumer* consumer, GestureProviderAuraClient* client);
    ...
    void OnTouchEventAck(uint32_t unique_touch_event_id, bool event_consumed, bool is_source_touch_event_set_blocking);
