...
TEST(PhysicsBasedFlingCurveTest, BasicFlingTestVelocityY)
{
    ...
    const gfx::Vector2dF pixels_per_inch(kDefaultPixelsPerInch, kDefaultPixelsPerInch);
    const gfx::Size viewport(1920, 1080);

    PhysicsBasedFlingCurve curve(fling_velocity, now, pixels_per_inch, kBoostMultiplierUnboosted, viewport);
    ...
    EXPECT_TRUE(curve.ComputeScrollOffset(now - base::Milliseconds(20), &offset, &velocity));
    ...
    EXPECT_TRUE(curve.ComputeScrollOffset(now + base::Milliseconds(250), &offset, &velocity));
    delta = offset - cumulative_scroll;
    EXPECT_EQ(0, delta.x());
    EXPECT_NEAR(delta.y(), 923, 1);
    cumulative_scroll = offset;

    EXPECT_FALSE(curve.ComputeScrollOffset(now + base::Seconds(curve.curve_duration()), &offset, &velocity));
    delta = offset - cumulative_scroll;
    EXPECT_EQ(0, delta.x());
    EXPECT_NEAR(delta.y(), 2218, 1);
    cumulative_scroll = offset;

    EXPECT_FALSE(curve.ComputeScrollOffset(now + base::Seconds(10), &offset, &velocity));
    delta = offset - cumulative_scroll;
...
TEST(PhysicsBasedFlingCurveTest, BasicFlingTestVelocityX)
{
    ...
    EXPECT_FALSE(curve.ComputeScrollOffset(now + base::Seconds(20), &offset, &velocity));
    delta = offset - cumulative_scroll;
    EXPECT_TRUE(delta.IsZero());
}
...
TEST(PhysicsBasedFlingCurveTest, ControlPointsWithSlopeLessThan1)
{
    ...
    PhysicsBasedFlingCurve curve(velocity, now, pixels_per_inch, kBoostMultiplierUnboosted, viewport);
