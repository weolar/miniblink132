...
public:
    ...
    virtual bool ProcessTouchEventPreDispatch(TouchEvent* event, GestureConsumer* consumer) = 0;

    // Returns a list of zero or more GestureEvents. Acks the gesture packet in
    // the queue which matches with unique_event_id.
    virtual Gestures AckTouchEvent(uint32_t unique_event_id, ui::EventResult result, bool is_source_touch_event_set_blocking, GestureConsumer* consumer) = 0;
    ...
    virtual GestureConsumer* GetTargetForLocation(const gfx::PointF& location, int source_device_id) = 0;
    ...
    virtual void CancelActiveTouchesOn(const std::vector<GestureConsumer*>& consumers) = 0;
    ...
    virtual void TransferEventsTo(GestureConsumer* current_consumer, GestureConsumer* new_consumer, TransferTouchesBehavior transfer_touches_behavior) = 0;
    ...
    virtual bool GetLastTouchPointForTarget(GestureConsumer* consumer, gfx::PointF* point) = 0;
