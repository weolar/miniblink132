...
MotionEvent::ToolType EventPointerTypeToMotionEventToolType(EventPointerType type)
{
    switch (type) {
...
PointerProperties GetPointerPropertiesFromTouchEvent(const TouchEvent& touch)
{
    ...
    pointer_properties.tangential_pressure = touch.pointer_details().tangential_pressure;

    pointer_properties.SetAxesAndOrientation(touch.pointer_details().radius_x, touch.pointer_details().radius_y, touch.ComputeRotationAngle());
    if (!pointer_properties.touch_major) {
        ...
        case EventPointerType::kEraser:
            ...
            break;
        default:
            default_size = 2.f * GestureConfiguration::GetInstance()->default_radius();
            break;
        }
        pointer_properties.touch_major = pointer_properties.touch_minor = default_size;
        pointer_properties.orientation = 0;
    }

    pointer_properties.tool_type = EventPointerTypeToMotionEventToolType(touch.pointer_details().pointer_type);
...
bool MotionEventAura::OnTouch(const TouchEvent& touch)
{
    ...
    if (touch.type() == EventType::kTouchPressed && pointer_id_is_active) {
        ...
        return false;
    } else if (touch.type() != EventType::kTouchPressed && !pointer_id_is_active) {
    ...
    if (touch.type() == EventType::kTouchMoved && touch.x() == GetX(index) && touch.y() == GetY(index)) {
        return false;
    }
...
{
    if (event.type() != EventType::kTouchReleased && event.type() != EventType::kTouchCancelled) {
        return;
    }
...
{
    pointer(GetIndexFromId(touch.pointer_details().id)) = GetPointerPropertiesFromTouchEvent(touch);
}
