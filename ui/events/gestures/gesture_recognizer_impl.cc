...
void TransferConsumer(GestureConsumer* current_consumer, GestureConsumer* new_consumer, std::set<raw_ptr<GestureConsumer, SetExperimental>>& consumers)
{
...
template <class Key, class T, class Value> bool RemoveValueFromMap(std::map<Key, T>* map, const Value& value)
{
    bool removed = false;
...
GestureConsumer* GestureRecognizerImpl::GetTouchLockedTarget(const TouchEvent& event)
{
    return touch_id_target_[event.pointer_details().id];
}

GestureConsumer* GestureRecognizerImpl::GetTargetForLocation(const gfx::PointF& location, int source_device_id)
{
    const float max_distance = GestureConfiguration::GetInstance()->max_separation_for_gesture_touches_in_pixels();
...
void GestureRecognizerImpl::CancelActiveTouchesExcept(GestureConsumer* not_cancelled)
{
    CancelActiveTouchesExceptImpl(not_cancelled);
}

void GestureRecognizerImpl::CancelActiveTouchesOn(const std::vector<GestureConsumer*>& consumers)
{
...
void GestureRecognizerImpl::TransferEventsTo(
    GestureConsumer* current_consumer, GestureConsumer* new_consumer, TransferTouchesBehavior transfer_touches_behavior)
{
    ...
    std::vector<std::unique_ptr<TouchEvent>> cancelling_touches = GetEventPerPointForConsumer(current_consumer, EventType::kTouchCancelled);
    ...
    if (transfer_touches_behavior == TransferTouchesBehavior::kCancel && helper) {
        GestureProviderAura* gesture_provider = GetGestureProviderForConsumer(current_consumer);

        for (std::unique_ptr<TouchEvent>& event : cancelling_touches) {
            gesture_provider->OnTouchEnter(event->pointer_details().id, event->x(), event->y());
    ...
    GestureProviderAura* provider = new_consumer ? new_consumer->provider() : nullptr;
    if (provider) {
...
bool GestureRecognizerImpl::GetLastTouchPointForTarget(GestureConsumer* consumer, gfx::PointF* point)
{
    GestureProviderAura* provider = consumer->provider();
    if (!provider) {
        return false;
...
std::vector<std::unique_ptr<TouchEvent>> GestureRecognizerImpl::GetEventPerPointForConsumer(GestureConsumer* consumer, EventType type)
{
    ...
    for (size_t i = 0; i < pointer_state.GetPointerCount(); ++i) {
        auto touch_event = std::make_unique<TouchEvent>(
            type, gfx::Point(), EventTimeForNow(), PointerDetails(ui::EventPointerType::kTouch, pointer_state.GetPointerId(i)), EF_IS_SYNTHESIZED);
...
GestureProviderAura* GestureRecognizerImpl::GetGestureProviderForConsumer(GestureConsumer* consumer)
{
    GestureProviderAura* provider = consumer->provider();
    if (!provider) {
        consumers_.insert(consumer);
        consumer->set_gesture_provider(std::make_unique<GestureProviderAura>(consumer, this));
    }
    return consumer->provider();
}

bool GestureRecognizerImpl::ProcessTouchEventPreDispatch(TouchEvent* event, GestureConsumer* consumer)
{
...
void GestureRecognizerImpl::SetupTargets(const TouchEvent& event, GestureConsumer* target)
{
    event_to_gesture_provider_[event.unique_event_id()] = GetGestureProviderForConsumer(target);
    if (event.type() == ui::EventType::kTouchReleased || event.type() == ui::EventType::kTouchCancelled) {
...
void GestureRecognizerImpl::DispatchGestureEvent(GestureConsumer* raw_input_consumer, GestureEvent* event)
{
...
GestureRecognizer::Gestures GestureRecognizerImpl::AckTouchEvent(
    uint32_t unique_event_id, ui::EventResult result, bool is_source_touch_event_set_blocking, GestureConsumer* consumer)
{
    ...
    auto event_to_gesture_provider_iterator = event_to_gesture_provider_.find(unique_event_id);
    ...
    gesture_provider->OnTouchEventAck(unique_event_id, result != ER_UNHANDLED, is_source_touch_event_set_blocking);
    return gesture_provider->GetAndResetPendingGestures();
}

void GestureRecognizerImpl::CancelActiveTouchesExceptImpl(GestureConsumer* not_cancelled)
{
...
bool GestureRecognizerImpl::CancelActiveTouchesImpl(GestureConsumer* consumer)
{
    ...
    std::vector<std::unique_ptr<TouchEvent>> cancelling_touches = GetEventPerPointForConsumer(consumer, EventType::kTouchCancelled);
...
void GestureRecognizerImpl::RemoveGestureEventHelper(GestureEventHelper* helper)
{
...
bool GestureRecognizerImpl::DoesConsumerHaveActiveTouch(GestureConsumer* consumer) const
{
...
void GestureRecognizerImpl::SendSynthesizedEndEvents(GestureConsumer* consumer)
{
    GetGestureProviderForConsumer(consumer)->SendSynthesizedEndEvents();
}

void GestureRecognizerImpl::OnGestureEvent(GestureConsumer* raw_input_consumer, GestureEvent* event)
{
    DispatchGestureEvent(raw_input_consumer, event);
}

void GestureRecognizerImpl::OnGestureProviderAuraWillBeDestroyed(GestureProviderAura* gesture_provider)
{
    // Clean `event_to_gesture_provider_` by removing invalid raw pointers.
    for (auto iter = event_to_gesture_provider_.begin(); iter != event_to_gesture_provider_.end();) {
...
GestureEventHelper* GestureRecognizerImpl::FindDispatchHelperForConsumer(GestureConsumer* consumer)
{
