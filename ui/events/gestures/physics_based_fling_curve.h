...
public:
    PhysicsBasedFlingCurve(const gfx::Vector2dF& velocity, base::TimeTicks start_timestamp, const gfx::Vector2dF& pixels_per_inch,
    ...
    bool ComputeScrollOffset(base::TimeTicks time, gfx::Vector2dF* offset, gfx::Vector2dF* velocity) override;

    // TODO(crbug.com/40660860): Use base::TimeDelta for curve_duration()
    // once crrev.com/c/1865928 is merged.
    float curve_duration() const
    {
        return curve_duration_.InSecondsF();
    }
    const gfx::PointF& p1_for_testing() const
    {
        return p1_;
    }
    const gfx::PointF& p2_for_testing() const
    {
        return p2_;
    }
...
private:
    ...
    base::TimeDelta CalculateDurationAndConfigureControlPoints(const gfx::Vector2dF& velocity);
