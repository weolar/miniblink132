...
GestureEventDetails::GestureEventDetails(ui::EventType type, float delta_x, float delta_y, ui::ScrollGranularity units)
    : type_(type)
    , device_type_(GestureDeviceType::DEVICE_UNKNOWN)
    , touch_points_(1)
{
    ...
    default:
        NOTREACHED() << "Invalid event type for constructor: " << base::to_underlying(type);
    }
}

GestureEventDetails::GestureEventDetails(ui::EventType type, const GestureEventDetails& other)
    : type_(type)
    , data_(other.data_)
    ...
    , bounding_box_(other.bounding_box_)
{
    ...
    case ui::EventType::kGesturePinchUpdate:
        DCHECK_EQ(type, other.type()) << " - Invalid gesture conversion from " << base::to_underlying(other.type()) << " to " << base::to_underlying(type);
        break;
    default:
        break;
    }
