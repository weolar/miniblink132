...
protected:
    ...
    EventDispatchDetails SendEventToSinkFromRewriter(const Event* event, const EventRewriter* rewriter);
...
private:
    ...
    typedef std::list<std::unique_ptr<EventRewriterContinuationImpl>> EventRewriterList;
...
template <> struct ScopedObservationTraits<ui::EventSource, ui::EventRewriter> {
    static void AddObserver(ui::EventSource* source, ui::EventRewriter* observer)
    {
        source->AddEventRewriter(observer);
    }
    static void RemoveObserver(ui::EventSource* source, ui::EventRewriter* observer)
    {
