...
static constexpr PointerId kPointerIdMouse = std::numeric_limits<int32_t>::max();
...
public:
    PointerDetails();
    explicit PointerDetails(EventPointerType pointer_type, PointerId pointer_id = kPointerIdUnknown);
    PointerDetails(EventPointerType pointer_type, PointerId pointer_id, float radius_x, float radius_y, float force, float twist = 0.0f, double tilt_x = 0.0f,
        double tilt_y = 0.0f, float tangential_pressure = 0.0f);
