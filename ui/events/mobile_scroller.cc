...
struct SplineConstants {
    ...
    void CalculateCoefficients(float t, float* distance_coef, float* velocity_coef)
    {
        *distance_coef = 1.f;
        *velocity_coef = 0.f;
...
template <typename T> T Clamped(T t, T a, T b)
{
...
MobileScroller::MobileScroller(const Config& config)
    ...
    , tuning_coeff_(ComputeDeceleration(config.chromecast_optimized ? 0.9f : 0.84f))
{
}
...
bool MobileScroller::ComputeScrollOffset(base::TimeTicks time, gfx::Vector2dF* offset, gfx::Vector2dF* velocity)
{
    DCHECK(offset);
    DCHECK(velocity);
...
void MobileScroller::StartScroll(float start_x, float start_y, float dx, float dy, base::TimeTicks start_time)
{
    StartScroll(start_x, start_y, dx, dy, start_time, base::Milliseconds(kDefaultDurationMs));
}

void MobileScroller::StartScroll(float start_x, float start_y, float dx, float dy, base::TimeTicks start_time, base::TimeDelta duration)
{
...
void MobileScroller::Fling(
    float start_x, float start_y, float velocity_x, float velocity_y, float min_x, float max_x, float min_y, float max_y, base::TimeTicks start_time)
{
...
bool MobileScroller::ComputeScrollOffsetInternal(base::TimeTicks time)
{
    ...
    case FLING_MODE: {
        ...
        g_spline_constants.Get().CalculateCoefficients(u, &distance_coef, &velocity_coef);
