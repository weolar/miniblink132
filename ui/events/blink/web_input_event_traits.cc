...
void ApppendEventDetails(const WebKeyboardEvent& event, std::string* result)
{
    StringAppendF(result,
        "{\n WinCode: %d\n NativeCode: %d\n IsSystem: %d\n"
        " Text: %s\n UnmodifiedText: %s\n}",
        event.windows_key_code, event.native_key_code, event.is_system_key, reinterpret_cast<const char*>(event.text.data()),
...
void ApppendEventDetails(const WebMouseEvent& event, std::string* result)
{
    StringAppendF(result,
        "{\n Button: %d\n Pos: (%f, %f)\n"
        " GlobalPos: (%f, %f)\n Movement: (%d, %d)\n Clicks: %d\n}",
        static_cast<int>(event.button), event.PositionInWidget().x(), event.PositionInWidget().y(), event.PositionInScreen().x(), event.PositionInScreen().y(),
        event.movement_x, event.movement_y, event.click_count);
}
...
void ApppendEventDetails(const WebMouseWheelEvent& event, std::string* result)
{
    StringAppendF(result,
        "{\n Delta: (%f, %f)\n WheelTicks: (%f, %f)\n Accel: (%f, %f)\n"
        " DeltaUnits: %d\n Phase: (%d, %d)",
        event.delta_x, event.delta_y, event.wheel_ticks_x, event.wheel_ticks_y, event.acceleration_ratio_x, event.acceleration_ratio_y,
        static_cast<int>(event.delta_units), event.phase, event.momentum_phase);
}
...
void ApppendEventDetails(const WebGestureEvent& event, std::string* result)
{
    StringAppendF(result,
        "{\n Pos: (%f, %f)\n GlobalPos: (%f, %f)\n SourceDevice: %d\n"
        " RawData: (%f, %f)\n}",
        event.PositionInWidget().x(), event.PositionInWidget().y(), event.PositionInScreen().x(), event.PositionInScreen().y(),
        static_cast<int>(event.SourceDevice()), event.data.scroll_update.delta_x, event.data.scroll_update.delta_y);
}
...
void ApppendTouchPointDetails(const WebTouchPoint& point, std::string* result)
{
    StringAppendF(result,
        ...
        point.id, static_cast<int>(point.state), point.PositionInScreen().x(), point.PositionInScreen().y(), point.PositionInWidget().x(),
        point.PositionInWidget().y(), point.radius_x, point.radius_y, point.rotation_angle, point.force, point.tilt_x, point.tilt_y, point.twist,
...
void ApppendEventDetails(const WebTouchEvent& event, std::string* result)
{
    StringAppendF(result,
        ...
        event.touches_length, static_cast<int>(event.dispatch_type), event.moved_beyond_slop_region, event.hovering, event.unique_touch_event_id);
...
void ApppendEventDetails(const WebPointerEvent& event, std::string* result)
{
    StringAppendF(result,
        ...
        "(%f, %f)\n}",
        event.id, static_cast<int>(event.button), event.PositionInWidget().x(), event.PositionInWidget().y(), event.PositionInScreen().x(),
        event.PositionInScreen().y(), event.movement_x, event.movement_y, event.width, event.height, event.force, event.tangential_pressure,
        event.rotation_angle, event.tilt_x, event.tilt_y);
}

template <typename EventType> std::string Execute(const WebInputEvent& event)
{
    std::string result = base::StringPrintf(
        "%s (Time: %lf, Modifiers: %d)\n", WebInputEvent::GetName(event.GetType()), event.TimeStamp().since_origin().InSecondsF(), event.GetModifiers());
...
bool WebInputEventTraits::ShouldBlockEventStream(const WebInputEvent& event)
{
    ...
    case WebInputEvent::Type::kTouchScrollStarted:
        DCHECK_NE(WebInputEvent::DispatchType::kBlocking, static_cast<const WebTouchEvent&>(event).dispatch_type);
        return false;
    ...
    case WebInputEvent::Type::kTouchEnd:
        return static_cast<const WebTouchEvent&>(event).dispatch_type == WebInputEvent::DispatchType::kBlocking;
    ...
    case WebInputEvent::Type::kMouseWheel:
        return static_cast<const WebMouseWheelEvent&>(event).dispatch_type == WebInputEvent::DispatchType::kBlocking;
...
uint32_t WebInputEventTraits::GetUniqueTouchEventId(const WebInputEvent& event)
{
