...
WebMouseEvent WebMouseEventBuilder::Build(
    HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam, base::TimeTicks time_stamp, blink::WebPointerProperties::PointerType pointer_type)
{
    ...
    // set modifiers:
    int modifiers = ui::EventFlagsToWebEventModifiers(ui::GetModifiersFromKeyState());
    ...
    gfx::PointF dip_global_point = display::win::ScreenWin::ScreenToDIPPoint(gfx::PointF(global_point.x, global_point.y));
    ...
    bool cancel_previous_click = (abs(last_click_position_x - result.PositionInWidget().x()) > (::GetSystemMetrics(SM_CXDOUBLECLK) / 2))
        || (abs(last_click_position_y - result.PositionInWidget().y()) > (::GetSystemMetrics(SM_CYDOUBLECLK) / 2))
        || ((current_time - g_last_click_time).InMilliseconds() > ::GetDoubleClickTime());
    ...
    } else if (result.GetType() == WebInputEvent::Type::kMouseMove || result.GetType() == WebInputEvent::Type::kMouseLeave) {
...
WebMouseWheelEvent WebMouseWheelEventBuilder::Build(
    HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam, base::TimeTicks time_stamp, blink::WebPointerProperties::PointerType pointer_type)
{
    WebMouseWheelEvent result(WebInputEvent::Type::kMouseWheel, ui::EventFlagsToWebEventModifiers(ui::GetModifiersFromKeyState()), time_stamp);
    ...
    if (horizontal_scroll) {
        ...
        TRACE_EVENT1("input", "WebMouseWheelEventBuilder::Build", "scroll_chars", scroll_chars);
        scroll_delta *= static_cast<float>(scroll_chars);
    } else {
        unsigned long scroll_lines = kDefaultScrollLinesPerWheelDelta;
        SystemParametersInfo(SPI_GETWHEELSCROLLLINES, 0, &scroll_lines, 0);
        TRACE_EVENT1("input", "WebMouseWheelEventBuilder::Build", "scroll_lines", scroll_lines);
