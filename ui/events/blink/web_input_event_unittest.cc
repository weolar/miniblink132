...
TEST(WebInputEventTest, TestMakeWebKeyboardEvent)
{
    ...
    {
        // Press left Ctrl.
        KeyEvent event(EventType::kKeyPressed, VKEY_CONTROL, DomCode::CONTROL_LEFT, EF_CONTROL_DOWN);
        ...
        EXPECT_EQ(blink::WebInputEvent::kControlKey | blink::WebInputEvent::kIsLeft, webkit_event.GetModifiers());
        EXPECT_EQ(static_cast<int>(DomCode::CONTROL_LEFT), webkit_event.dom_code);
        EXPECT_EQ(DomKey::CONTROL, webkit_event.dom_key);
    }
    {
        // Release left Ctrl.
        KeyEvent event(EventType::kKeyReleased, VKEY_CONTROL, DomCode::CONTROL_LEFT, EF_NONE);
        blink::WebKeyboardEvent webkit_event = MakeWebKeyboardEvent(event);
    ...
    {
        // Press right Ctrl.
        KeyEvent event(EventType::kKeyPressed, VKEY_CONTROL, DomCode::CONTROL_RIGHT, EF_CONTROL_DOWN);
        blink::WebKeyboardEvent webkit_event = MakeWebKeyboardEvent(event);
        // However, modifier bit for Control in |webkit_event| should be set.
        EXPECT_EQ(blink::WebInputEvent::kControlKey | blink::WebInputEvent::kIsRight, webkit_event.GetModifiers());
        EXPECT_EQ(static_cast<int>(DomCode::CONTROL_RIGHT), webkit_event.dom_code);
    }
    {
        // Release right Ctrl.
        KeyEvent event(EventType::kKeyReleased, VKEY_CONTROL, DomCode::CONTROL_RIGHT, EF_NONE);
        blink::WebKeyboardEvent webkit_event = MakeWebKeyboardEvent(event);
        // However, modifier bit for Control in |webkit_event| shouldn't be set.
...
TEST(WebInputEventTest, TestMakeWebKeyboardEventWindowsKeyCode)
{
    ...
    {
        // Press left Ctrl.
        KeyEvent event(EventType::kKeyPressed, VKEY_CONTROL, DomCode::CONTROL_LEFT, EF_CONTROL_DOWN, DomKey::CONTROL, EventTimeForNow());
        blink::WebKeyboardEvent webkit_event = MakeWebKeyboardEvent(event);
        EXPECT_EQ(VKEY_CONTROL, webkit_event.windows_key_code);
    }
    {
        // Press right Ctrl.
        KeyEvent event(EventType::kKeyPressed, VKEY_CONTROL, DomCode::CONTROL_RIGHT, EF_CONTROL_DOWN, DomKey::CONTROL, EventTimeForNow());
        blink::WebKeyboardEvent webkit_event = MakeWebKeyboardEvent(event);
        EXPECT_EQ(VKEY_CONTROL, webkit_event.windows_key_code);
    }
#if BUILDFLAG(IS_WIN)
...
TEST(WebInputEventTest, TestMakeWebKeyboardEventKeyPadKeyCode)
{
    ...
    for (const auto& test_case : kTesCases) {
        KeyEvent event(EventType::kKeyPressed, test_case.ui_keycode, test_case.dom_code, EF_NONE);
        blink::WebKeyboardEvent webkit_event = MakeWebKeyboardEvent(event);
        EXPECT_EQ(test_case.expected_result, (webkit_event.GetModifiers() & blink::WebInputEvent::kIsKeyPad) != 0)
            << "Failed in "
            << "{dom_code:" << KeycodeConverter::DomCodeToCodeString(test_case.dom_code) << ", ui_keycode:" << test_case.ui_keycode
...
{
    {
        ...
        MouseEvent ui_event(EventType::kMousePressed, gfx::Point(123, 321), gfx::Point(123, 321), timestamp, EF_LEFT_MOUSE_BUTTON, EF_LEFT_MOUSE_BUTTON);
        blink::WebMouseEvent webkit_event = MakeWebMouseEvent(ui_event);
        EXPECT_EQ(EventFlagsToWebEventModifiers(ui_event.flags()), webkit_event.GetModifiers());
    ...
    }
    {
        // Left released.
        base::TimeTicks timestamp = EventTimeForNow();
        MouseEvent ui_event(EventType::kMouseReleased, gfx::Point(123, 321), gfx::Point(123, 321), timestamp, 0, EF_LEFT_MOUSE_BUTTON);
        blink::WebMouseEvent webkit_event = MakeWebMouseEvent(ui_event);
        EXPECT_EQ(EventFlagsToWebEventModifiers(ui_event.flags()), webkit_event.GetModifiers());
        EXPECT_EQ(timestamp, webkit_event.TimeStamp());
        EXPECT_EQ(blink::WebMouseEvent::Button::kLeft, webkit_event.button);
    ...
    }
    {
        // Middle pressed.
        base::TimeTicks timestamp = EventTimeForNow();
        MouseEvent ui_event(EventType::kMousePressed, gfx::Point(123, 321), gfx::Point(123, 321), timestamp, EF_MIDDLE_MOUSE_BUTTON, EF_MIDDLE_MOUSE_BUTTON);
        blink::WebMouseEvent webkit_event = MakeWebMouseEvent(ui_event);
        EXPECT_EQ(EventFlagsToWebEventModifiers(ui_event.flags()), webkit_event.GetModifiers());
        EXPECT_EQ(timestamp, webkit_event.TimeStamp());
    ...
    }
    {
        // Middle released.
        base::TimeTicks timestamp = EventTimeForNow();
        MouseEvent ui_event(EventType::kMouseReleased, gfx::Point(123, 321), gfx::Point(123, 321), timestamp, 0, EF_MIDDLE_MOUSE_BUTTON);
        blink::WebMouseEvent webkit_event = MakeWebMouseEvent(ui_event);
        EXPECT_EQ(EventFlagsToWebEventModifiers(ui_event.flags()), webkit_event.GetModifiers());
        EXPECT_EQ(timestamp, webkit_event.TimeStamp());
        EXPECT_EQ(blink::WebMouseEvent::Button::kMiddle, webkit_event.button);
    ...
    }
    {
        // Right pressed.
        base::TimeTicks timestamp = EventTimeForNow();
        MouseEvent ui_event(EventType::kMousePressed, gfx::Point(123, 321), gfx::Point(123, 321), timestamp, EF_RIGHT_MOUSE_BUTTON, EF_RIGHT_MOUSE_BUTTON);
        blink::WebMouseEvent webkit_event = MakeWebMouseEvent(ui_event);
        EXPECT_EQ(EventFlagsToWebEventModifiers(ui_event.flags()), webkit_event.GetModifiers());
        EXPECT_EQ(timestamp, webkit_event.TimeStamp());
    ...
    }
    {
        // Right released.
        base::TimeTicks timestamp = EventTimeForNow();
        MouseEvent ui_event(EventType::kMouseReleased, gfx::Point(123, 321), gfx::Point(123, 321), timestamp, 0, EF_RIGHT_MOUSE_BUTTON);
        blink::WebMouseEvent webkit_event = MakeWebMouseEvent(ui_event);
        EXPECT_EQ(EventFlagsToWebEventModifiers(ui_event.flags()), webkit_event.GetModifiers());
        EXPECT_EQ(timestamp, webkit_event.TimeStamp());
        EXPECT_EQ(blink::WebMouseEvent::Button::kRight, webkit_event.button);
    ...
    }
    {
        // Moved
        base::TimeTicks timestamp = EventTimeForNow();
        MouseEvent ui_event(EventType::kMouseMoved, gfx::Point(123, 321), gfx::Point(123, 321), timestamp, 0, 0);
        blink::WebMouseEvent webkit_event = MakeWebMouseEvent(ui_event);
        EXPECT_EQ(EventFlagsToWebEventModifiers(ui_event.flags()), webkit_event.GetModifiers());
        EXPECT_EQ(timestamp, webkit_event.TimeStamp());
    ...
    }
    {
        ...
        base::TimeTicks timestamp = EventTimeForNow();
        MouseEvent ui_event(EventType::kMouseMoved, gfx::Point(123, 321), gfx::Point(123, 321), timestamp, EF_LEFT_MOUSE_BUTTON, 0);
        blink::WebMouseEvent webkit_event = MakeWebMouseEvent(ui_event);
        EXPECT_EQ(EventFlagsToWebEventModifiers(ui_event.flags()), webkit_event.GetModifiers());
        EXPECT_EQ(timestamp, webkit_event.TimeStamp());
        EXPECT_EQ(blink::WebMouseEvent::Button::kLeft, webkit_event.button);
    ...
    }
    {
        ...
        base::TimeTicks timestamp = EventTimeForNow();
        MouseEvent ui_event(
            EventType::kMousePressed, gfx::Point(123, 321), gfx::Point(123, 321), timestamp, EF_LEFT_MOUSE_BUTTON | EF_SHIFT_DOWN, EF_LEFT_MOUSE_BUTTON);
        blink::WebMouseEvent webkit_event = MakeWebMouseEvent(ui_event);
        EXPECT_EQ(EventFlagsToWebEventModifiers(ui_event.flags()), webkit_event.GetModifiers());
        EXPECT_EQ(timestamp, webkit_event.TimeStamp());
        EXPECT_EQ(blink::WebMouseEvent::Button::kLeft, webkit_event.button);
    ...
    }
    {
        ...
        EXPECT_EQ(blink::WebPointerProperties::PointerType::kMouse, webkit_event.pointer_type);
    ...
    }
    {
        ...
        MouseEvent ui_event(
            EventType::kMousePressed, gfx::Point(123, 321), gfx::Point(123, 321), timestamp, EF_LEFT_MOUSE_BUTTON, EF_LEFT_MOUSE_BUTTON, pointer_details);
        blink::WebMouseEvent webkit_event = MakeWebMouseEvent(ui_event);

        EXPECT_EQ(blink::WebPointerProperties::PointerType::kPen, webkit_event.pointer_type);
...
{
    {
        ...
        MouseWheelEvent ui_event(
            gfx::Vector2d(MouseWheelEvent::kWheelDelta * 2, -MouseWheelEvent::kWheelDelta * 2), gfx::Point(123, 321), gfx::Point(123, 321), timestamp, 0, 0);
        ...
        EXPECT_FLOAT_EQ(ui_event.x_offset() / MouseWheelEvent::kWheelDelta, webkit_event.wheel_ticks_x);
        EXPECT_FLOAT_EQ(ui_event.y_offset() / MouseWheelEvent::kWheelDelta, webkit_event.wheel_ticks_y);
        EXPECT_EQ(blink::WebPointerProperties::PointerType::kMouse, webkit_event.pointer_type);
        EXPECT_EQ(0, webkit_event.tilt_x);
        EXPECT_EQ(0, webkit_event.tilt_y);
        EXPECT_TRUE(std::isnan(webkit_event.force));
...
TEST(WebInputEventTest, TestPercentMouseWheelScroll)
{
    ...
    scoped_feature_list.InitAndEnableFeature(features::kWindowsScrollingPersonality);

    base::TimeTicks timestamp = EventTimeForNow();
    MouseWheelEvent ui_event(gfx::Vector2d(0, -MouseWheelEvent::kWheelDelta), gfx::Point(123, 321), gfx::Point(123, 321), timestamp, 0, 0);
    blink::WebMouseWheelEvent webkit_event = MakeWebMouseWheelEvent(ui_event);

    EXPECT_EQ(ui::ScrollGranularity::kScrollByPercentage, webkit_event.delta_units);
...
TEST(WebInputEventTest, KeyEvent)
{
    ...
    struct {
        ...
        int web_modifiers;
    } tests[] = { { ui::KeyEvent(ui::EventType::kKeyPressed, ui::VKEY_A, ui::EF_NONE), blink::WebInputEvent::Type::kRawKeyDown, 0x0 },
        { ui::KeyEvent::FromCharacter(L'B', ui::VKEY_B, ui::DomCode::NONE, ui::EF_CONTROL_DOWN | ui::EF_SHIFT_DOWN), blink::WebInputEvent::Type::kChar,
            blink::WebInputEvent::kShiftKey | blink::WebInputEvent::kControlKey },
        { ui::KeyEvent(ui::EventType::kKeyReleased, ui::VKEY_C, ui::EF_ALT_DOWN), blink::WebInputEvent::Type::kKeyUp, blink::WebInputEvent::kAltKey } };
    ...
    for (size_t i = 0; i < std::size(tests); i++) {
        ...
        ASSERT_EQ(static_cast<int>(tests[i].event.GetLocatedWindowsKeyboardCode()), web_event.windows_key_code);
    }
}
...
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndDisableFeature(features::kWindowsScrollingPersonality);
    const int kDeltaX = 14;
    const int kDeltaY = -3;
    ui::MouseWheelEvent ui_event(ui::MouseEvent(ui::EventType::kUnknown, gfx::Point(), gfx::Point(), base::TimeTicks(), 0, 0), kDeltaX, kDeltaY);
...
TEST(WebInputEventTest, MousePointerEvent)
{
    struct {
        ...
        { ui::EventType::kMousePressed, blink::WebInputEvent::Type::kMouseDown, 0x0, 0x0, gfx::Point(3, 5), gfx::Point(113, 125) },
        { ui::EventType::kMouseReleased, blink::WebInputEvent::Type::kMouseUp, ui::EF_LEFT_MOUSE_BUTTON, blink::WebInputEvent::kLeftButtonDown,
            gfx::Point(100, 1), gfx::Point(50, 1) },
        { ui::EventType::kMouseMoved, blink::WebInputEvent::Type::kMouseMove, ui::EF_MIDDLE_MOUSE_BUTTON | ui::EF_RIGHT_MOUSE_BUTTON,
            blink::WebInputEvent::kMiddleButtonDown | blink::WebInputEvent::kRightButtonDown, gfx::Point(13, 3), gfx::Point(53, 3) },
    };

    for (size_t i = 0; i < std::size(tests); i++) {
        ui::MouseEvent ui_event(tests[i].ui_type, tests[i].location, tests[i].screen_location, base::TimeTicks(), tests[i].ui_modifiers, 0);
...
TEST(WebInputEventTest, MouseMoveUnadjustedMovement)
{
    ...
    ui::MouseEvent event(EventType::kMouseMoved, cursor_pos, cursor_pos, base::TimeTicks(), 0, 0);
