...
public:
    ...
    WebGestureEvent CreateFlingStart(const gfx::Vector2dF& velocity, int modifiers = 0, WebGestureDevice source_device = WebGestureDevice::kTouchscreen)
    {
        WebGestureEvent fling_start(WebInputEvent::Type::kGestureFlingStart, modifiers, event_time_, source_device);
        fling_start.data.fling_start.velocity_x = velocity.x();
        fling_start.data.fling_start.velocity_y = velocity.y();
        return fling_start;
    }

    WebGestureEvent CreateFlingCancel(WebGestureDevice source_device = WebGestureDevice::kTouchscreen)
    {
        WebGestureEvent fling_cancel(WebInputEvent::Type::kGestureFlingCancel, 0, event_time_, source_device);
        return fling_cancel;
    }

    WebGestureEvent CreateScrollBegin(gfx::Vector2dF delta, WebGestureDevice source_device = WebGestureDevice::kTouchscreen)
    {
        WebGestureEvent scroll_begin(WebInputEvent::Type::kGestureScrollBegin, 0, event_time_, source_device);
        scroll_begin.data.scroll_begin.delta_x_hint = delta.x();
        scroll_begin.data.scroll_begin.delta_y_hint = delta.y();
        scroll_begin.data.scroll_begin.delta_hint_units = ui::ScrollGranularity::kScrollByPrecisePixel;
        return scroll_begin;
    }

    WebGestureEvent CreateScrollUpdate(gfx::Vector2dF delta, WebGestureDevice source_device = WebGestureDevice::kTouchscreen)
    {
        WebGestureEvent scroll_update(WebInputEvent::Type::kGestureScrollUpdate, 0, event_time_, source_device);
        scroll_update.data.scroll_update.delta_x = delta.x();
        scroll_update.data.scroll_update.delta_y = delta.y();
        scroll_update.data.scroll_update.delta_units = ui::ScrollGranularity::kScrollByPrecisePixel;
        return scroll_update;
    }

    WebGestureEvent CreateScrollEnd(WebGestureDevice source_device = WebGestureDevice::kTouchscreen)
    {
        return WebGestureEvent(WebInputEvent::Type::kGestureScrollEnd, 0, event_time_, source_device);
    }
    ...
    void SimulateBoostingScroll()
    {
        ...
        event_time_ += kEventDelta;
        fling_booster_.ObserveGestureEvent(CreateScrollUpdate(DeltaFromVelocity(Vector2dF(0, kMinBoostScrollSpeed), kEventDelta)));
    }

    void ProgressFling(const Vector2dF& current_velocity, const Vector2dF& delta)
    {
...
TEST_F(FlingBoosterTest, FlingBoostBasic)
{
    ...
    EXPECT_EQ(Vector2dF(0, 1000), fling_velocity) << "First fling shouldn't be boosted";

    SimulateBoostingScroll();

    fling_velocity = SendFlingStart(CreateFlingStart(Vector2dF(0, 2000)));
    EXPECT_EQ(Vector2dF(0, 3000), fling_velocity) << "FlingStart with ongoing fling should be boosted";
}
...
TEST_F(FlingBoosterTest, NoFlingBoostIfScrollDelayed)
{
    ...
    fling_velocity = SendFlingStart(CreateFlingStart(Vector2dF(0, 2000)));
    EXPECT_EQ(Vector2dF(0, 2000), fling_velocity) << "ScrollUpdate delayed longer than boosting timeout; fling shouldn't "
                                                     "be boosted.";
}
...
TEST_F(FlingBoosterTest, NoFlingBoostIfBoostTooSlow)
{
    ...
    EXPECT_EQ(new_velocity, fling_velocity) << "Boosting FlingStart too slow; fling shouldn't be boosted.";
}
...
TEST_F(FlingBoosterTest, NoFlingBoostIfPreviousStartVelocityTooSlow)
{
    ...
    fling_velocity = SendFlingStart(CreateFlingStart(Vector2dF(0, kMinBoostFlingSpeed - 1)));

    SimulateBoostingScroll();
    fling_velocity = SendFlingStart(CreateFlingStart(Vector2dF(0, 2000)));
    EXPECT_EQ(Vector2dF(0, 2000), fling_velocity) << "Previous fling too slow and shouldn't be boosted.";
}
...
TEST_F(FlingBoosterTest, NoFlingBoostIfCurrentVelocityTooSlow)
{
    ...
    EXPECT_EQ(Vector2dF(0, 1100), fling_velocity) << "Existing fling too slow and shouldn't be boosted.";
}
...
TEST_F(FlingBoosterTest, NoFlingBoostIfFlingInDifferentDirection)
{
    ...
    EXPECT_EQ(Vector2dF(1000, 0), fling_velocity) << "Fling isn't in same direction, shouldn't boost.";
}
...
TEST_F(FlingBoosterTest, NoFlingBoostIfScrollInDifferentDirection)
{
    ...
    fling_velocity = SendFlingStart(CreateFlingStart(Vector2dF(0, 2000)));
    EXPECT_EQ(Vector2dF(0, 2000), fling_velocity) << "Scrolling in an orthogonal direction should prevent boosting, even "
...
TEST_F(FlingBoosterTest, NoFlingBoostIfPreventBoostingFlagIsSet)
{
    ...
    fling_velocity = SendFlingStart(CreateFlingStart(Vector2dF(0, 2000)));
    EXPECT_EQ(Vector2dF(0, 2000), fling_velocity) << "prevent_boosting on FlingCancel should avoid boosting a subsequent "
                                                     "FlingStart";
}
...
TEST_F(FlingBoosterTest, NoFlingBoostIfDifferentFlingModifiers)
{
    ...
    fling_velocity = SendFlingStart(CreateFlingStart(Vector2dF(0, 2000), MODIFIER_SHIFT));
    EXPECT_EQ(Vector2dF(0, 2000), fling_velocity) << "Changed modifier keys should prevent boost.";
}
...
TEST_F(FlingBoosterTest, NoFlingBoostIfDifferentFlingSourceDevices)
{
    ...
    fling_velocity = SendFlingStart(CreateFlingStart(Vector2dF(0, 1000), 0, WebGestureDevice::kTouchpad));
    EXPECT_EQ(Vector2dF(0, 1000), fling_velocity) << "Changed modifier keys should prevent boost.";
}
...
TEST_F(FlingBoosterTest, NoFlingBoostIfScrollBeginPastCutoffTime)
{
    ...
    fling_velocity = SendFlingStart(CreateFlingStart(Vector2dF(0, 2000)));
    EXPECT_EQ(Vector2dF(0, 2000), fling_velocity) << "Scroll must not be boosted since it occured after the boost timeout "
                                                     "delay.";
}
