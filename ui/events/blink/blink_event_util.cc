...
WebTouchPoint::State ToWebTouchPointState(const MotionEvent& event, size_t pointer_index)
{
    ...
    case MotionEvent::Action::POINTER_DOWN:
        return static_cast<int>(pointer_index) == event.GetActionIndex() ? WebTouchPoint::State::kStatePressed : WebTouchPoint::State::kStateStationary;
    case MotionEvent::Action::POINTER_UP:
        return static_cast<int>(pointer_index) == event.GetActionIndex() ? WebTouchPoint::State::kStateReleased : WebTouchPoint::State::kStateStationary;
    case MotionEvent::Action::NONE:
    case MotionEvent::Action::HOVER_ENTER:
    case MotionEvent::Action::HOVER_EXIT:
    case MotionEvent::Action::HOVER_MOVE:
...
WebPointerProperties::PointerType ToWebPointerType(MotionEvent::ToolType tool_type)
{
    switch (tool_type) {
...
WebPointerProperties::PointerType ToWebPointerType(EventPointerType event_pointer_type)
{
    ...
    case EventPointerType::kEraser:
        return WebPointerProperties::PointerType::kEraser;
    default:
        NOTREACHED() << "Invalid EventPointerType = " << static_cast<int>(event_pointer_type);
    }
}
...
WebTouchPoint CreateWebTouchPoint(const MotionEvent& event, size_t pointer_index)
{
    WebTouchPoint touch;

    SetWebPointerPropertiesFromMotionEventData(touch, event.GetPointerId(pointer_index), event.GetPressure(pointer_index), event.GetOrientation(pointer_index),
        event.GetTiltX(pointer_index), event.GetTiltY(pointer_index), event.GetTwist(pointer_index), event.GetTangentialPressure(pointer_index),
        0 /* no button changed */, event.GetToolType(pointer_index));

    touch.state = ToWebTouchPointState(event, pointer_index);
    touch.SetPositionInWidget(event.GetX(pointer_index), event.GetY(pointer_index));
    touch.SetPositionInScreen(event.GetRawX(pointer_index), event.GetRawY(pointer_index));
...
blink::WebTouchEvent CreateWebTouchEventFromMotionEvent(const MotionEvent& event, bool moved_beyond_slop_region, bool hovering)
{
    static_assert(static_cast<int>(MotionEvent::MAX_TOUCH_POINT_COUNT) == static_cast<int>(blink::WebTouchEvent::kTouchesLengthCap),
        "inconsistent maximum number of active touch points");

    blink::WebTouchEvent result(ToWebTouchEventType(event.GetAction()), EventFlagsToWebEventModifiers(event.GetFlags()), event.GetEventTime());
    result.dispatch_type
        = result.GetType() == WebInputEvent::Type::kTouchCancel ? WebInputEvent::DispatchType::kEventNonBlocking : WebInputEvent::DispatchType::kBlocking;
    ...
    result.touches_length = std::min(static_cast<unsigned>(event.GetPointerCount()), static_cast<unsigned>(WebTouchEvent::kTouchesLengthCap));
...
WebGestureEvent CreateWebGestureEvent(const GestureEventDetails& details, base::TimeTicks timestamp, const gfx::PointF& location,
    const gfx::PointF& raw_location, int flags, uint32_t unique_touch_event_id)
{
    ...
    WebGestureEvent gesture(WebInputEvent::Type::kUndefined, EventFlagsToWebEventModifiers(flags), timestamp, source_device);

    gesture.SetPositionInWidget(location);
    gesture.SetPositionInScreen(raw_location);

    gesture.is_source_touch_event_set_blocking = details.is_source_touch_event_set_blocking();
    gesture.primary_pointer_type = ToWebPointerType(details.primary_pointer_type());
    gesture.primary_unique_touch_event_id = details.primary_unique_touch_event_id();
    gesture.unique_touch_event_id = unique_touch_event_id;
    gesture.GetModifiableEventLatencyMetadata() = details.GetEventLatencyMetadata();

    switch (details.type()) {
    case EventType::kGestureShowPress:
        gesture.SetType(WebInputEvent::Type::kGestureShowPress);
        gesture.data.show_press.width = IfNanUseMaxFloat(details.bounding_box_f().width());
        gesture.data.show_press.height = IfNanUseMaxFloat(details.bounding_box_f().height());
        break;
    case EventType::kGestureDoubleTap:
        ...
        gesture.data.tap.width = IfNanUseMaxFloat(details.bounding_box_f().width());
        gesture.data.tap.height = IfNanUseMaxFloat(details.bounding_box_f().height());
    ...
    case EventType::kGestureShortPress:
        ...
        gesture.data.long_press.width = IfNanUseMaxFloat(details.bounding_box_f().width());
        gesture.data.long_press.height = IfNanUseMaxFloat(details.bounding_box_f().height());
        break;
    ...
    case EventType::kGestureTwoFingerTap:
        ...
        gesture.data.two_finger_tap.first_finger_width = IfNanUseMaxFloat(details.first_finger_width());
        gesture.data.two_finger_tap.first_finger_height = IfNanUseMaxFloat(details.first_finger_height());
        break;
    case EventType::kGestureScrollBegin:
        ...
        gesture.data.scroll_begin.delta_x_hint = IfNanUseMaxFloat(details.scroll_x_hint());
        gesture.data.scroll_begin.delta_y_hint = IfNanUseMaxFloat(details.scroll_y_hint());
        gesture.data.scroll_begin.delta_hint_units = details.scroll_begin_units();
        gesture.data.scroll_begin.inertial_phase = WebGestureEvent::InertialPhaseState::kNonMomentum;
        break;
    case EventType::kGestureScrollUpdate:
        ...
        gesture.data.scroll_update.inertial_phase = WebGestureEvent::InertialPhaseState::kNonMomentum;
        break;
    case EventType::kGestureScrollEnd:
        gesture.SetType(WebInputEvent::Type::kGestureScrollEnd);
        gesture.data.scroll_end.inertial_phase = WebGestureEvent::InertialPhaseState::kNonMomentum;
        break;
    case EventType::kScrollFlingStart:
        gesture.SetType(WebInputEvent::Type::kGestureFlingStart);
        gesture.data.fling_start.velocity_x = IfNanUseMaxFloat(details.velocity_x());
        gesture.data.fling_start.velocity_y = IfNanUseMaxFloat(details.velocity_y());
        break;
    ...
    case EventType::kGestureTapDown:
        ...
        gesture.data.tap_down.width = IfNanUseMaxFloat(details.bounding_box_f().width());
        gesture.data.tap_down.height = IfNanUseMaxFloat(details.bounding_box_f().height());
        break;
    ...
    case EventType::kGestureSwipe:
        ...
        break;
    default:
        NOTREACHED() << "EventType provided wasn't a valid gesture event: " << base::to_underlying(details.type());
    }
...
WebGestureEvent CreateWebGestureEventFromGestureEventData(const GestureEventData& data)
{
    return CreateWebGestureEvent(
        data.details, data.time, gfx::PointF(data.x, data.y), gfx::PointF(data.raw_x, data.raw_y), data.flags, data.unique_touch_event_id);
}

std::unique_ptr<blink::WebInputEvent> ScaleWebInputEvent(const blink::WebInputEvent& event, float scale, std::optional<int64_t> trace_id)
{
    return TranslateAndScaleWebInputEvent(event, gfx::Vector2dF(0, 0), scale, trace_id);
}

std::unique_ptr<blink::WebInputEvent> TranslateAndScaleWebInputEvent(
    const blink::WebInputEvent& event, const gfx::Vector2dF& delta, float scale, std::optional<int64_t> trace_id)
{
    ...
    if (event.GetType() == blink::WebMouseEvent::Type::kMouseWheel) {
        ...
        *wheel_event = static_cast<const blink::WebMouseWheelEvent&>(event);
        wheel_event->SetPositionInWidget(gfx::ScalePoint(wheel_event->PositionInWidget() + delta, scale));
        ...
        *mouse_event = static_cast<const blink::WebMouseEvent&>(event);
        mouse_event->SetPositionInWidget(gfx::ScalePoint(mouse_event->PositionInWidget() + delta, scale));
        ...
        *touch_event = static_cast<const blink::WebTouchEvent&>(event);
        for (unsigned i = 0; i < touch_event->touches_length; i++) {
            touch_event->touches[i].SetPositionInWidget(gfx::ScalePoint(touch_event->touches[i].PositionInWidget() + delta, scale));
        ...
        *gesture_event = static_cast<const blink::WebGestureEvent&>(event);
        gesture_event->SetPositionInWidget(gfx::ScalePoint(gesture_event->PositionInWidget() + delta, scale));
        switch (gesture_event->GetType()) {
        case blink::WebInputEvent::Type::kGestureScrollUpdate:
            if (gesture_event->data.scroll_update.delta_units == ui::ScrollGranularity::kScrollByPixel
                || gesture_event->data.scroll_update.delta_units == ui::ScrollGranularity::kScrollByPrecisePixel) {
                gesture_event->data.scroll_update.delta_x *= scale;
                gesture_event->data.scroll_update.delta_y *= scale;
            }
            break;
        case blink::WebInputEvent::Type::kGestureScrollBegin:
            if (gesture_event->data.scroll_begin.delta_hint_units == ui::ScrollGranularity::kScrollByPixel
                || gesture_event->data.scroll_begin.delta_hint_units == ui::ScrollGranularity::kScrollByPrecisePixel) {
        ...
        if (gesture_event->GetType() == blink::WebInputEvent::Type::kGestureScrollUpdate && trace_id.has_value()) {
            TRACE_EVENT("input,input.scrolling", "TranslateAndScaleWebInputEvent",
                [trace_id_value = *trace_id, delta_x = gesture_event->data.scroll_update.delta_x, delta_y = gesture_event->data.scroll_update.delta_y](
...
void SetWebPointerPropertiesFromMotionEventData(WebPointerProperties& webPointerProperties, int pointer_id, float pressure, float orientation_rad, float tilt_x,
    float tilt_y, float twist, float tangential_pressure, int android_buttons_changed, MotionEvent::ToolType tool_type)
{
...
EventPointerType WebPointerTypeToEventPointerType(WebPointerProperties::PointerType type)
{
    switch (type) {
...
blink::WebGestureEvent ScrollBeginFromScrollUpdate(const blink::WebGestureEvent& gesture_update)
{
    ...
    scroll_begin.data.scroll_begin.delta_x_hint = gesture_update.data.scroll_update.delta_x;
    scroll_begin.data.scroll_begin.delta_y_hint = gesture_update.data.scroll_update.delta_y;
    scroll_begin.data.scroll_begin.delta_hint_units = gesture_update.data.scroll_update.delta_units;
    scroll_begin.data.scroll_begin.target_viewport = false;
    scroll_begin.data.scroll_begin.inertial_phase = gesture_update.data.scroll_update.inertial_phase;
...
#if BUILDFLAG(IS_ANDROID)
std::unique_ptr<WebGestureEvent> CreateWebGestureEventFromGestureEventAndroid(const GestureEventAndroid& event)
{
    ...
    auto web_event = std::make_unique<WebGestureEvent>(event_type, WebInputEvent::kNoModifiers, base::TimeTicks() + base::Milliseconds(event.time()));
