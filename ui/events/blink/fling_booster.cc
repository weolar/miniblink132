...
gfx::Vector2dF FlingBooster::GetVelocityForFlingStart(const blink::WebGestureEvent& fling_start)
{
    DCHECK_EQ(blink::WebInputEvent::Type::kGestureFlingStart, fling_start.GetType());
    gfx::Vector2dF velocity(fling_start.data.fling_start.velocity_x, fling_start.data.fling_start.velocity_y);

    TRACE_EVENT2("input", "FlingBooster::GetVelocityForFlingStart", "vx", velocity.x(), "vy", velocity.y());

    if (ShouldBoostFling(fling_start)) {
        velocity += previous_fling_starting_velocity_;
        TRACE_EVENT_INSTANT2("input", "Boosted", TRACE_EVENT_SCOPE_THREAD, "vx", velocity.x(), "vy", velocity.y());
    }
...
{
    TRACE_EVENT1("input", "FlingBooster::ObserveGestureEvent", "type", WebInputEvent::GetName(gesture_event.GetType()));
    ...
    if (!cutoff_time_for_boost_.is_null() && gesture_event.TimeStamp() > cutoff_time_for_boost_) {
    ...
    case WebInputEvent::Type::kGestureScrollBegin: {
        cutoff_time_for_boost_ = gesture_event.TimeStamp() + kFlingBoostTimeoutDelay;
        break;
    }
    case WebInputEvent::Type::kGestureScrollUpdate: {
        ...
        // boost.
        gfx::Vector2dF delta(gesture_event.data.scroll_update.delta_x, gesture_event.data.scroll_update.delta_y);
        ...
        if (!previous_boosting_scroll_timestamp_.is_null()) {
            const double time_since_last_boost_event = (gesture_event.TimeStamp() - previous_boosting_scroll_timestamp_).InSecondsF();
            if (time_since_last_boost_event >= 0.001) {
                const gfx::Vector2dF scroll_velocity = gfx::ScaleVector2d(delta, 1. / time_since_last_boost_event);
    ...
    case WebInputEvent::Type::kGestureFlingCancel: {
        if (gesture_event.data.fling_cancel.prevent_boosting) {
            TRACE_EVENT_INSTANT0("input", "GFC PreventBoosting", TRACE_EVENT_SCOPE_THREAD);
            Reset();
            return;
        }
...
void FlingBooster::ObserveProgressFling(const gfx::Vector2dF& current_velocity)
{
    TRACE_EVENT2("input", "FlingBooster::ObserveProgressFling", "vx", current_velocity.x(), "vy", current_velocity.y());
    if (previous_fling_starting_velocity_.IsZero())
        return;
...
{
    DCHECK_EQ(WebInputEvent::Type::kGestureFlingStart, fling_start_event.GetType());
    if (previous_fling_starting_velocity_.IsZero()) {
        TRACE_EVENT_INSTANT0("input", "No Boost - NoActiveFling", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }

    if (source_device_ != fling_start_event.SourceDevice()) {
        TRACE_EVENT_INSTANT0("input", "No Boost - SourceDevice", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }

    if (modifiers_ != fling_start_event.GetModifiers()) {
        TRACE_EVENT_INSTANT0("input", "No Boost - Modifiers", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }

    if (cutoff_time_for_boost_.is_null()) {
        TRACE_EVENT_INSTANT0("input", "No Boost - CutoffTimeUnset", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }

    if (fling_start_event.TimeStamp() > cutoff_time_for_boost_) {
        TRACE_EVENT_INSTANT0("input", "No Boost - Timeout", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }

    gfx::Vector2dF new_fling_velocity(fling_start_event.data.fling_start.velocity_x, fling_start_event.data.fling_start.velocity_y);

    if (gfx::DotProduct(previous_fling_starting_velocity_, new_fling_velocity) <= 0) {
        TRACE_EVENT_INSTANT0("input", "No Boost - Direction", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }

    if (current_fling_velocity_.LengthSquared() < kMinBoostFlingSpeedSquare) {
        TRACE_EVENT_INSTANT0("input", "No Boost - CurrentVelocity", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }

    if (new_fling_velocity.LengthSquared() < kMinBoostFlingSpeedSquare) {
        TRACE_EVENT_INSTANT0("input", "No Boost - NewVelocity", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }
