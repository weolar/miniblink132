...
TEST(WebInputEventBuilderTest, TestMouseEventScale)
{
    ...
    WebMouseEvent mouse_move = ui::WebMouseEventBuilder::Build(
        ::GetDesktopWindow(), WM_MOUSEMOVE, 0, MAKELPARAM(300, 200), base::TimeTicks() + base::Seconds(100), blink::WebPointerProperties::PointerType::kMouse);
    ...
    EXPECT_EQ(blink::WebPointerProperties::PointerType::kMouse, mouse_move.pointer_type);
...
TEST(WebInputEventBuilderTest, TestPercentMouseWheelScroll)
{
    ...
    WebMouseWheelEvent mouse_wheel = WebMouseWheelEventBuilder::Build(::GetDesktopWindow(), WM_MOUSEWHEEL, MAKEWPARAM(0, -WHEEL_DELTA), MAKELPARAM(0, 0),
        base::TimeTicks() + base::Seconds(100), blink::WebPointerProperties::PointerType::kMouse);
    EXPECT_EQ(ui::ScrollGranularity::kScrollByPercentage, mouse_wheel.delta_units);
    EXPECT_FLOAT_EQ(0.f, mouse_wheel.delta_x);
    EXPECT_FLOAT_EQ(-0.05f, mouse_wheel.delta_y / static_cast<float>(system_scroll_lines));
    ...
    mouse_wheel = WebMouseWheelEventBuilder::Build(::GetDesktopWindow(), WM_MOUSEHWHEEL, MAKEWPARAM(0, -WHEEL_DELTA), MAKELPARAM(0, 0),
        base::TimeTicks() + base::Seconds(100), blink::WebPointerProperties::PointerType::kMouse);
    EXPECT_EQ(ui::ScrollGranularity::kScrollByPercentage, mouse_wheel.delta_units);
    EXPECT_FLOAT_EQ(0.05f, mouse_wheel.delta_x / static_cast<float>(system_scroll_chars));
...
void VerifyWebMouseWheelEventBuilderHistograms(UINT message, blink::WebPointerProperties::PointerType type, const char* histogram,
    std::vector<int>& event_timestamps_in_ms, std::map<int, int>& histogram_expectations)
{
    base::HistogramTester histogram_tester;

    EXPECT_TRUE(event_timestamps_in_ms.size() > 0 && histogram_expectations.size() > 0);
    for (int event_timestamp : event_timestamps_in_ms) {
        WebMouseWheelEventBuilder::Build(
            ::GetDesktopWindow(), message, MAKEWPARAM(0, -WHEEL_DELTA), MAKELPARAM(0, 0), base::TimeTicks() + base::Milliseconds(event_timestamp), type);
    }

    for (std::map<int, int>::iterator it = histogram_expectations.begin(); it != histogram_expectations.end(); ++it) {
        // Key is the (unique) velocity bucket.
        // Value is the count of data points for that bucket.
        EXPECT_EQ(histogram_tester.GetBucketCount(histogram, it->first), it->second);
    }
}
