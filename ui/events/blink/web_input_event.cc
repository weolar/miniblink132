...
{
    return event.target() ? event.target()->GetScreenLocationF(event) : event.root_location_f();
}
...
blink::WebGestureEvent MakeWebGestureEventFromUIEvent(const GestureEvent& event)
{
    return CreateWebGestureEvent(
        event.details(), event.time_stamp(), event.location_f(), event.root_location_f(), event.flags(), event.unique_touch_event_id());
}
...
blink::WebMouseEvent MakeUntranslatedWebMouseEventFromNativeEvent(
    const PlatformEvent& native_event, const base::TimeTicks& time_stamp, blink::WebPointerProperties::PointerType pointer_type)
{
    return WebMouseEventBuilder::Build(native_event.hwnd, native_event.message, native_event.wParam, native_event.lParam, time_stamp, pointer_type);
}

blink::WebMouseWheelEvent MakeUntranslatedWebMouseWheelEventFromNativeEvent(
    const PlatformEvent& native_event, const base::TimeTicks& time_stamp, blink::WebPointerProperties::PointerType pointer_type)
{
    return WebMouseWheelEventBuilder::Build(native_event.hwnd, native_event.message, native_event.wParam, native_event.lParam, time_stamp, pointer_type);
}
...
blink::WebKeyboardEvent MakeWebKeyboardEventFromUiEvent(const KeyEvent& event)
{
    ...
    case EventType::kKeyPressed:
        type = event.is_char() ? blink::WebInputEvent::Type::kChar : blink::WebInputEvent::Type::kRawKeyDown;
        break;
    ...
    blink::WebKeyboardEvent webkit_event(
        type, EventFlagsToWebEventModifiers(event.flags()) | DomCodeToWebInputEventModifiers(event.code()), event.time_stamp());
    ...
    webkit_event.native_key_code = KeycodeConverter::DomCodeToNativeKeycode(event.code());
...
blink::WebMouseWheelEvent MakeWebMouseWheelEventFromUiEvent(const ScrollEvent& event)
{
    blink::WebMouseWheelEvent webkit_event(blink::WebInputEvent::Type::kMouseWheel, EventFlagsToWebEventModifiers(event.flags()), event.time_stamp());
    ...
    webkit_event.wheel_ticks_x = webkit_event.delta_x / MouseWheelEvent::kWheelDelta;
    webkit_event.wheel_ticks_y = webkit_event.delta_y / MouseWheelEvent::kWheelDelta;
...
blink::WebGestureEvent MakeWebGestureEventFromUiEvent(const ScrollEvent& event)
{
    blink::WebInputEvent::Type type = blink::WebInputEvent::Type::kUndefined;
    switch (event.type()) {
    ...
    case EventType::kScroll:
        NOTREACHED() << "Invalid gesture type: " << base::to_underlying(event.type());
    default:
        NOTREACHED() << "Unknown gesture type: " << base::to_underlying(event.type());
    }

    blink::WebGestureEvent webkit_event(type, EventFlagsToWebEventModifiers(event.flags()), event.time_stamp(), blink::WebGestureDevice::kTouchpad);
...
blink::WebMouseWheelEvent MakeWebMouseWheelEventFromUiEvent(const MouseWheelEvent& event);
...
blink::WebMouseEvent MakeWebMouseEvent(const MouseEvent& event)
{
    ...
    blink::WebMouseEvent webkit_event =
        ...
        ? MakeUntranslatedWebMouseEventFromNativeEvent(event.native_event(), event.time_stamp(), event.pointer_details().pointer_type)
...
blink::WebMouseWheelEvent MakeWebMouseWheelEvent(const MouseWheelEvent& event)
{
    ...
    blink::WebMouseWheelEvent webkit_event = event.native_event().message
        ? MakeUntranslatedWebMouseWheelEventFromNativeEvent(event.native_event(), event.time_stamp(), event.pointer_details().pointer_type)
        : MakeWebMouseWheelEventFromUiEvent(event);
#else
    blink::WebMouseWheelEvent webkit_event = MakeWebMouseWheelEventFromUiEvent(event);
#endif
...
blink::WebKeyboardEvent MakeWebKeyboardEvent(const KeyEvent& event)
{
    ...
    if (event.HasNativeEvent()) {
        ...
        webkit_event.is_system_key = native_event.message == WM_SYSCHAR || native_event.message == WM_SYSKEYDOWN || native_event.message == WM_SYSKEYUP;
...
blink::WebGestureEvent MakeWebGestureEventFlingCancel(const blink::WebMouseWheelEvent& wheel_event)
{
    blink::WebGestureEvent gesture_event(
        blink::WebInputEvent::Type::kGestureFlingCancel, blink::WebInputEvent::kNoModifiers, wheel_event.TimeStamp(), blink::WebGestureDevice::kTouchpad);
...
blink::WebMouseEvent MakeWebMouseEventFromUiEvent(const MouseEvent& event)
{
    ...
    case EventType::kMouseExited: {
        ...
        if (props && props->contains(kPropertyMouseCrossedIntermediateWindow)) {
            ...
            static bool s_send_leave = base::FeatureList::IsEnabled(features::kSendMouseLeaveEvents);
            type = s_send_leave ? blink::WebInputEvent::Type::kMouseLeave : blink::WebInputEvent::Type::kMouseMove;
        }
        break;
    }
    ...
    case EventType::kMouseDragged:
        ...
        break;
    default:
        NOTIMPLEMENTED() << "Received unexpected event: " << base::to_underlying(event.type());
        break;
    }

    blink::WebMouseEvent webkit_event(type, EventFlagsToWebEventModifiers(event.flags()), event.time_stamp(), event.pointer_details().id);
    webkit_event.button = blink::WebMouseEvent::Button::kNoButton;
    ...
    webkit_event.tangential_pressure = event.pointer_details().tangential_pressure;
...
blink::WebMouseWheelEvent MakeWebMouseWheelEventFromUiEvent(const MouseWheelEvent& event)
{
    ...
    DCHECK(!(event.flags() & ui::EF_PRECISION_SCROLLING_DELTA && event.flags() & ui::EF_SCROLL_BY_PAGE));
    ...
    if (features::IsPercentBasedScrollingEnabled() && webkit_event.delta_units != ui::ScrollGranularity::kScrollByPage
        && webkit_event.delta_units != ui::ScrollGranularity::kScrollByPrecisePixel) {
        webkit_event.delta_units = ui::ScrollGranularity::kScrollByPercentage;
        webkit_event.delta_y *= (kScrollPercentPerLineOrChar / MouseWheelEvent::kWheelDelta);
        webkit_event.delta_x *= (kScrollPercentPerLineOrChar / MouseWheelEvent::kWheelDelta);
    }
