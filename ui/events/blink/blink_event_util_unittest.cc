...
TEST(BlinkEventUtilTest, NoScalingWith1DSF)
{
    ...
    auto event = CreateWebGestureEvent(details, base::TimeTicks(), gfx::PointF(1.f, 1.f), gfx::PointF(1.f, 1.f), 0, 0U);
...
void RunTest(ui::ScrollGranularity granularity)
{
    ...
    std::unique_ptr<blink::WebInputEvent> webEvent = ScaleWebInputEvent(event, 2.f);
    EXPECT_TRUE(webEvent);
    blink::WebMouseWheelEvent* mouseWheelEvent = static_cast<blink::WebMouseWheelEvent*>(webEvent.get());
...
TEST(BlinkEventUtilTest, NonPaginatedScrollBeginEvent)
{
    ...
    std::unique_ptr<blink::WebInputEvent> webEvent = ScaleWebInputEvent(event, 2.f);
    EXPECT_TRUE(webEvent);
    blink::WebGestureEvent* gestureEvent = static_cast<blink::WebGestureEvent*>(webEvent.get());
...
{
    ui::GestureEventDetails details(ui::EventType::kGestureScrollBegin, 1, 1, ui::ScrollGranularity::kScrollByPage);
    details.set_device_type(ui::GestureDeviceType::DEVICE_TOUCHSCREEN);
    auto event = CreateWebGestureEvent(details, base::TimeTicks(), gfx::PointF(1.f, 1.f), gfx::PointF(1.f, 1.f), 0, 0U);
    std::unique_ptr<blink::WebInputEvent> webEvent = ScaleWebInputEvent(event, 2.f);
    EXPECT_TRUE(webEvent);
...
TEST(BlinkEventUtilTest, EnsureFlingVelocityNotNaN)
{
    ...
    ui::GestureEventDetails details(ui::EventType::kScrollFlingStart, nan_number, 1.f);
    details.set_device_type(ui::GestureDeviceType::DEVICE_TOUCHSCREEN);
    auto event = CreateWebGestureEvent(details, base::TimeTicks(), gfx::PointF(1.f, 1.f), gfx::PointF(1.f, 1.f), 0, 0U);
    EXPECT_EQ(std::numeric_limits<float>::max(), event.data.fling_start.velocity_x);
...
{
    ui::GestureEventDetails details(ui::EventType::kGestureScrollUpdate, 1, 1, ui::ScrollGranularity::kScrollByPage);
    details.set_device_type(ui::GestureDeviceType::DEVICE_TOUCHSCREEN);
    auto event = CreateWebGestureEvent(details, base::TimeTicks(), gfx::PointF(1.f, 1.f), gfx::PointF(1.f, 1.f), 0, 0U);
    std::unique_ptr<blink::WebInputEvent> webEvent = ScaleWebInputEvent(event, 2.f);
    EXPECT_TRUE(webEvent);
...
TEST(BlinkEventUtilTest, WebEventModifersAndEventFlags)
{
    ...
    constexpr int kWebEventModifiersToTest[] = { WebInputEvent::kShiftKey, WebInputEvent::kControlKey, WebInputEvent::kAltKey, WebInputEvent::kAltGrKey,
        WebInputEvent::kMetaKey, WebInputEvent::kCapsLockOn, WebInputEvent::kNumLockOn, WebInputEvent::kScrollLockOn, WebInputEvent::kLeftButtonDown,
        WebInputEvent::kMiddleButtonDown, WebInputEvent::kRightButtonDown, WebInputEvent::kBackButtonDown, WebInputEvent::kForwardButtonDown,
