...
public:
    class DispatcherApi {
    public:
        explicit DispatcherApi(EventTarget* target)
            : target_(target)
        {
        }
    ...
    virtual void ConvertEventToTarget(const EventTarget* target, LocatedEvent* event) const;
    ...
    bool HasTargetHandler() const
    {
        return target_handler_ != nullptr;
    }

protected:
    EventHandler* target_handler()
    {
        return target_handler_;
    }
...
template <> struct ScopedObservationTraits<ui::EventTarget, ui::EventHandler> {
    static void AddObserver(ui::EventTarget* source, ui::EventHandler* observer)
    {
        source->AddPreTargetHandler(observer);
    }
    static void RemoveObserver(ui::EventTarget* source, ui::EventHandler* observer)
    {
