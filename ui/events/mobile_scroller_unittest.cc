...
TEST_F(MobileScrollerTest, Scroll)
{
    ...
    scroller.StartScroll(kDefaultStartX, kDefaultStartY, kDefaultDeltaX, kDefaultDeltaY, start_time);
    ...
    EXPECT_TRUE(scroller.ComputeScrollOffset(start_time + scroll_duration / 2, &offset, &velocity));
    ...
    // Advance to the end.
    EXPECT_FALSE(scroller.ComputeScrollOffset(start_time + scroll_duration, &offset, &velocity));
    ...
    EXPECT_FALSE(scroller.ComputeScrollOffset(start_time + scroll_duration * 2, &offset, &velocity));
    EXPECT_EQ(scroller.GetFinalX(), offset.x());
    EXPECT_EQ(scroller.GetFinalY(), offset.y());
    EXPECT_TRUE(scroller.IsFinished());
    EXPECT_EQ(scroll_duration, scroller.GetTimePassed());
...
TEST_F(MobileScrollerTest, Fling)
{
    ...
    scroller.Fling(kDefaultStartX, kDefaultStartY, kDefaultVelocityX, kDefaultVelocityY, INT_MIN, static_cast<float>(INT_MAX), INT_MIN,
        static_cast<float>(INT_MAX), start_time);
    ...
    const base::TimeDelta scroll_duration = scroller.GetDuration();
    gfx::Vector2dF offset, velocity;
    scroller.ComputeScrollOffset(start_time + scroll_duration / 2, &offset, &velocity);
    ...
    EXPECT_TRUE(scroller.IsScrollingInDirection(kDefaultVelocityX, kDefaultVelocityY));
