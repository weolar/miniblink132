...
protected:
    ...
    void SetUp() override
    {
        ...
        processor_.Reset();
        root()->SetEventTargeter(std::make_unique<TestEventTargeter>(root(), false));
    }
    ...
    {
        static_cast<TestEventTargeter*>(root()->GetEventTargeter())->set_target(target);
    }
...
TEST_F(EventProcessorTest, Basic)
{
    ...
    child->SetEventTargeter(std::make_unique<TestEventTargeter>(child.get(), false));
    SetTarget(child.get());
    root()->AddChild(std::move(child));

    MouseEvent mouse(EventType::kMouseMoved, gfx::Point(10, 10), gfx::Point(10, 10), EventTimeForNow(), EF_NONE, EF_NONE);
    DispatchEvent(&mouse);
...
TEST_F(EventProcessorTest, NestedEventProcessing)
{
    ...
    second_root->SetEventTargeter(std::make_unique<TestEventTargeter>(second_child.get(), false));
    ...
    auto target_handler = std::make_unique<ReDispatchEventHandler>(second_processor.get(), root()->child_at(0));
    EventHandler* old_handler = root()->child_at(0)->SetTargetHandler(target_handler.get());
    ...
    EXPECT_TRUE(second_root->child_at(0)->DidReceiveEvent(EventType::kMouseMoved));
    ...
    MouseEvent mouse2(EventType::kMouseMoved, gfx::Point(10, 10), gfx::Point(10, 10), EventTimeForNow(), EF_NONE, EF_NONE);
...
TEST_F(EventProcessorTest, DispatchToNextBestTarget)
{
    ...
    root()->SetEventTargeter(std::make_unique<TestEventTargeter>(grandchild.get(), true));
...
TEST_F(EventProcessorTest, HandlerSequence)
{
    ...
    std::string expected[] = { "PreR", "PreC", "PreG", "G", "PostG", "PostC", "PostR", "PreR", "PreC", "C", "PostC", "PostR", "PreR", "R", "PostR" };
    EXPECT_EQ(std::vector<std::string>(expected, expected + std::size(expected)), recorder);
...
public:
    ...
    DestroyDuringDispatchEventProcessor(const DestroyDuringDispatchEventProcessor&) = delete;
    DestroyDuringDispatchEventProcessor& operator=(const DestroyDuringDispatchEventProcessor&) = delete;
    ~DestroyDuringDispatchEventProcessor() override = default;

protected:
    EventDispatchDetails PostDispatchEvent(EventTarget* target, const Event& event) override;
};
...
public:
    ...
    DestroyDuringDispatchEventTarget(const DestroyDuringDispatchEventTarget&) = delete;
    DestroyDuringDispatchEventTarget& operator=(const DestroyDuringDispatchEventTarget&) = delete;
...
EventDispatchDetails DestroyDuringDispatchEventProcessor::PostDispatchEvent(EventTarget* target, const Event& event)
{
...
TEST(EventProcessorCrashTest, DestroyDuringDispatch)
{
    for (auto destroy_target : { kProcessor, kTargeter }) {
        ...
        auto target = std::make_unique<DestroyDuringDispatchEventTarget>(destroy_target);
        root->SetEventTargeter(std::make_unique<TestEventTargeter>(target.get(), false));
...
public:
    DestroyDuringFindTargetEventTargeter(std::unique_ptr<TestEventTarget> root, DestroyTarget target)
    ...
    DestroyDuringFindTargetEventTargeter(const DestroyDuringFindTargetEventTargeter&) = delete;
    DestroyDuringFindTargetEventTargeter& operator=(const DestroyDuringFindTargetEventTargeter&) = delete;
...
TEST(EventProcessorCrashTest, DestroyDuringFindTarget)
{
    for (auto destroy_target : { kProcessor, kTargeter }) {
        ...
        auto event_targeter = std::make_unique<DestroyDuringFindTargetEventTargeter>(std::move(root), destroy_target);
