...
constexpr char kPropertyMouseCrossedIntermediateWindow[] = "_mouseevent_cros_window_";

// Returns a ui::Event wrapping a native event. Ownership of the returned value
// is transferred to the caller.
EVENTS_EXPORT std::unique_ptr<Event> EventFromNative(const PlatformEvent& native_event);
...
EVENTS_EXPORT base::TimeTicks EventTimeFromNative(const PlatformEvent& native_event);
...
EVENTS_EXPORT base::TimeTicks EventLatencyTimeFromNative(const PlatformEvent& native_event, base::TimeTicks current_time);
...
EVENTS_EXPORT gfx::PointF EventLocationFromNative(const PlatformEvent& native_event);

// Gets the location in native system coordinate space.
EVENTS_EXPORT gfx::Point EventSystemLocationFromNative(const PlatformEvent& native_event);

// Returns the KeyboardCode from a native event.
EVENTS_EXPORT KeyboardCode KeyboardCodeFromNative(const PlatformEvent& native_event);
...
EVENTS_EXPORT int GetChangedMouseButtonFlagsFromNative(const PlatformEvent& native_event);

// Returns the detailed pointer information for mouse events.
EVENTS_EXPORT PointerDetails GetMousePointerDetailsFromNative(const PlatformEvent& native_event);

// Returns the movement vector associated with this mouse movement event.
EVENTS_EXPORT const gfx::Vector2dF& GetMouseMovementFromNative(const PlatformEvent& native_event);

// Gets the mouse wheel offsets from a native event.
EVENTS_EXPORT gfx::Vector2d GetMouseWheelOffset(const PlatformEvent& native_event);

// Gets the mouse wheel tick counts from a native event, with a value of 120
// representing a whole tick.
EVENTS_EXPORT gfx::Vector2d GetMouseWheelTick120ths(const PlatformEvent& native_event);
...
EVENTS_EXPORT PointerDetails GetTouchPointerDetailsFromNative(const PlatformEvent& native_event);
...
EVENTS_EXPORT bool GetFlingData(const PlatformEvent& native_event, float* vx, float* vy, float* vx_ordinal, float* vy_ordinal, bool* is_cancel);

// Returns whether this is a scroll event and optionally gets the amount to be
// scrolled. |x_offset|, |y_offset| and |finger_count| can be NULL.
EVENTS_EXPORT bool GetScrollOffsets(const PlatformEvent& native_event, float* x_offset, float* y_offset, float* x_offset_ordinal, float* y_offset_ordinal,
    int* finger_count, EventMomentumPhase* momentum_phase);
...
EVENTS_EXPORT void ComputeEventLatencyOS(ui::EventType type, base::TimeTicks time_stamp, base::TimeTicks current_time);

#if BUILDFLAG(IS_WIN)
// Like ComputeEventLatencyOS, but for events whose timestamp comes from a
// TOUCHINPUT structure instead of PlatformEvent.
EVENTS_EXPORT void ComputeEventLatencyOSFromTOUCHINPUT(ui::EventType event_type, TOUCHINPUT touch_input, base::TimeTicks current_time);

// Like ComputeEventLatencyOS, but for events whose timestamp comes from a
// POINTER_INFO structure instead of PlatformEvent.
EVENTS_EXPORT void ComputeEventLatencyOSFromPOINTER_INFO(ui::EventType event_type, POINTER_INFO pointer_info, base::TimeTicks current_time);
...
EVENTS_EXPORT CHROME_MSG MSGFromKeyEvent(KeyEvent* key_event, HWND hwnd = nullptr);
...
EVENTS_EXPORT void ConvertEventLocationToTargetWindowLocation(
    const gfx::Point& target_window_origin, const gfx::Point& current_window_origin, ui::LocatedEvent* located_event);

// Converts a value of an unsigned integer type to an Event::PropertyValue.
template <base::bits::UnsignedInteger T> Event::PropertyValue ConvertToEventPropertyValue(const T& value)
{
...
EVENTS_EXPORT std::vector<std::string_view> MouseEventFlagsNames(int event_flags);
} // namespace ui
