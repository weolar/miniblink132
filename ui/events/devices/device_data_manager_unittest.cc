...
TEST_F(DeviceDataManagerTest, DisplayIdUpdated)
{
    ...
    static_cast<DeviceHotplugEventObserver*>(device_data_manager)->OnTouchscreenDevicesUpdated(touchscreen_devices);
    ASSERT_EQ(1u, device_data_manager->GetTouchscreenDevices().size());
    EXPECT_EQ(display::kInvalidDisplayId, device_data_manager->GetTouchscreenDevices()[0].target_display_id);
    ...
    ASSERT_EQ(1u, device_data_manager->GetTouchscreenDevices().size());
    EXPECT_EQ(kDisplayId, device_data_manager->GetTouchscreenDevices()[0].target_display_id);
}
...
public:
    ...
    TestInputDeviceEventObserver& operator=(const TestInputDeviceEventObserver&) = delete;
...
TEST_F(DeviceDataManagerTest, AreTouchscreenTargetDisplaysValid)
{
    ...
    base::ScopedObservation<DeviceDataManager, InputDeviceEventObserver> scoped_obaservation(&observer);
