...
struct EVENTS_DEVICES_EXPORT TouchpadDevice : public InputDevice {
    ...
    TouchpadDevice(int id, InputDeviceType type, const std::string& name, bool is_haptic = false);
    TouchpadDevice(int id, InputDeviceType type, const std::string& name, const std::string& phys, const base::FilePath& sys_path, uint16_t vendor,
        uint16_t product, uint16_t version, bool is_haptic = false);
    explicit TouchpadDevice(const InputDevice& input_device, bool is_haptic = false);
