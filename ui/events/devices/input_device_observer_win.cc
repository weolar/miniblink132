...
#define WIN_NOTIFY_OBSERVERS(method_decl, input_device_types)                                                                                                  \
    ...
    {                                                                                                                                                          \
        observers_.Notify(&InputDeviceEventObserver::OnInputDeviceConfigurationChanged, InputDeviceEventObserver::input_device_types);                         \
    }
...
InputDeviceObserverWin::InputDeviceObserverWin()
    : registry_key_(HKEY_LOCAL_MACHINE, L"System\\CurrentControlSet\\Control\\PriorityControl", KEY_NOTIFY | KEY_READ)
{
    if (registry_key_.Valid()) {
        slate_mode_enabled_ = IsSlateModeEnabled();
        // Start watching the registry for changes.
        registry_key_.StartWatching(base::BindOnce(&InputDeviceObserverWin::OnRegistryKeyChanged, base::Unretained(this)));
    }
}

InputDeviceObserverWin* InputDeviceObserverWin::GetInstance()
{
    return base::Singleton<InputDeviceObserverWin, base::LeakySingletonTraits<InputDeviceObserverWin>>::get();
}
...
bool InputDeviceObserverWin::IsSlateModeEnabled()
{
    ...
    return registry_key_.ReadValueDW(L"ConvertibleSlateMode", &slate_enabled) == ERROR_SUCCESS && slate_enabled == 1;
}
...
void InputDeviceObserverWin::RemoveObserver(InputDeviceEventObserver* observer)
{
    observers_.RemoveObserver(observer);
}

WIN_NOTIFY_OBSERVERS(NotifyObserversKeyboardDeviceConfigurationChanged(), kKeyboard)

WIN_NOTIFY_OBSERVERS(NotifyObserversTouchpadDeviceConfigurationChanged(), kTouchpad)
