...
{
    return base::MakeAbsoluteFilePath(base::FilePath("/sys/class/input").Append(path.BaseName()));
}
...
InputDeviceType GetInputDeviceTypeFromPath(const base::FilePath& path)
{
    ...
    for (base::FilePath ancestor_path = sysfs_path; ancestor_path != base::FilePath("/"); ancestor_path = ancestor_path.DirName()) {
        // Bluetooth LE devices are virtual "uhid" devices.
        if (ancestor_path == base::FilePath(FILE_PATH_LITERAL("/sys/devices/virtual/misc/uhid"))) {
            return InputDeviceType::INPUT_DEVICE_BLUETOOTH;
        }

        std::string subsystem_path = base::MakeAbsoluteFilePath(ancestor_path.Append(FILE_PATH_LITERAL("subsystem"))).value();
