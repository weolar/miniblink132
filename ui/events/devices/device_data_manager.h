...
class EVENTS_DEVICES_EXPORT DeviceDataManager : public DeviceHotplugEventObserver {
public:
    ...
    void ConfigureTouchDevices(const std::vector<ui::TouchDeviceTransform>& transforms);
...
protected:
    ...
    void OnTouchscreenDevicesUpdated(const std::vector<TouchscreenDevice>& devices) override;
    void OnKeyboardDevicesUpdated(const std::vector<KeyboardDevice>& devices) override;
    void OnMouseDevicesUpdated(const std::vector<InputDevice>& devices) override;
    void OnPointingStickDevicesUpdated(const std::vector<InputDevice>& devices) override;
    void OnTouchpadDevicesUpdated(const std::vector<TouchpadDevice>& devices) override;
    void OnGraphicsTabletDevicesUpdated(const std::vector<InputDevice>& devices) override;
    void OnUncategorizedDevicesUpdated(const std::vector<InputDevice>& devices) override;
...
private:
    ...
    void UpdateTouchInfoFromTransform(const ui::TouchDeviceTransform& touch_device_transform);
    void UpdateTouchMap();
