...
public:
    ...
    virtual void OnTouchscreenDevicesUpdated(const std::vector<TouchscreenDevice>& devices) = 0;
    ...
    virtual void OnKeyboardDevicesUpdated(const std::vector<KeyboardDevice>& devices) = 0;

    // On a hotplug event this is called with the list of available mice. The set
    // of mice may not have changed.
    virtual void OnMouseDevicesUpdated(const std::vector<InputDevice>& devices) = 0;
    ...
    virtual void OnPointingStickDevicesUpdated(const std::vector<InputDevice>& devices) = 0;

    // On a hotplug event this is called with the list of available touchpads. The
    // set of touchpads may not have changed.
    virtual void OnTouchpadDevicesUpdated(const std::vector<TouchpadDevice>& devices) = 0;
    ...
    virtual void OnGraphicsTabletDevicesUpdated(const std::vector<InputDevice>& devices) = 0;
    ...
    // and touchpads.
    virtual void OnUncategorizedDevicesUpdated(const std::vector<InputDevice>& devices) = 0;
