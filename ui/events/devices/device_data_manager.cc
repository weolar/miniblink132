...
{
    return a.id == b.id && a.enabled == b.enabled && a.suspected_keyboard_imposter == b.suspected_keyboard_imposter
...
void DeviceDataManager::ConfigureTouchDevices(const std::vector<ui::TouchDeviceTransform>& transforms)
{
...
void DeviceDataManager::UpdateTouchInfoFromTransform(const ui::TouchDeviceTransform& touch_device_transform)
{
...
void DeviceDataManager::UpdateTouchMap()
{
    ...
    // connected.
    base::EraseIf(touch_map_, [this](const std::pair<int, TouchDeviceTransform>& map_entry) {
        // Remove the device identified by |map_entry| from |touch_map_| if it
        // is not present in the list of currently connected devices.
        return !base::Contains(touchscreen_devices_, map_entry.second.device_id, &TouchscreenDevice::id);
    });
}

void DeviceDataManager::ApplyTouchRadiusScale(int touch_device_id, double* radius)
{
...
void DeviceDataManager::ApplyTouchTransformer(int touch_device_id, float* x, float* y)
{
    auto iter = touch_map_.find(touch_device_id);
    if (iter != touch_map_.end()) {
...
const std::vector<TouchscreenDevice>& DeviceDataManager::GetTouchscreenDevices() const
{
    return touchscreen_devices_;
}

const std::vector<KeyboardDevice>& DeviceDataManager::GetKeyboardDevices() const
{
...
const std::vector<InputDevice>& DeviceDataManager::GetPointingStickDevices() const
{
    return pointing_stick_devices_;
}

const std::vector<TouchpadDevice>& DeviceDataManager::GetTouchpadDevices() const
{
    return touchpad_devices_;
}

const std::vector<InputDevice>& DeviceDataManager::GetGraphicsTabletDevices() const
{
    return graphics_tablet_devices_;
}

const std::vector<InputDevice>& DeviceDataManager::GetUncategorizedDevices() const
{
...
int64_t DeviceDataManager::GetTargetDisplayForTouchDevice(int touch_device_id) const
{
    auto iter = touch_map_.find(touch_device_id);
    if (iter != touch_map_.end())
...
void DeviceDataManager::OnTouchscreenDevicesUpdated(const std::vector<TouchscreenDevice>& devices)
{
    ...
    for (TouchscreenDevice& touchscreen_device : touchscreen_devices_) {
        touchscreen_device.target_display_id = GetTargetDisplayForTouchDevice(touchscreen_device.id);
    }
    UpdateTouchMap();
    NotifyObserversTouchscreenDeviceConfigurationChanged();
}

void DeviceDataManager::OnKeyboardDevicesUpdated(const std::vector<KeyboardDevice>& devices)
{
...
void DeviceDataManager::OnMouseDevicesUpdated(const std::vector<InputDevice>& devices)
{
...
void DeviceDataManager::OnPointingStickDevicesUpdated(const std::vector<InputDevice>& devices)
{
    if (base::ranges::equal(devices, pointing_stick_devices_, InputDeviceEquals)) {
        return;
    }
...
void DeviceDataManager::OnTouchpadDevicesUpdated(const std::vector<TouchpadDevice>& devices)
{
...
void DeviceDataManager::OnGraphicsTabletDevicesUpdated(const std::vector<InputDevice>& devices)
{
    if (base::ranges::equal(devices, graphics_tablet_devices_, InputDeviceEquals)) {
        return;
    }
...
void DeviceDataManager::OnUncategorizedDevicesUpdated(const std::vector<InputDevice>& devices)
{
...
NOTIFY_OBSERVERS(NotifyObserversKeyboardDeviceConfigurationChanged(), OnInputDeviceConfigurationChanged(InputDeviceEventObserver::kKeyboard))

NOTIFY_OBSERVERS(NotifyObserversMouseDeviceConfigurationChanged(), OnInputDeviceConfigurationChanged(InputDeviceEventObserver::kMouse))

NOTIFY_OBSERVERS(NotifyObserversPointingStickDeviceConfigurationChanged(), OnInputDeviceConfigurationChanged(InputDeviceEventObserver::kPointingStick))

NOTIFY_OBSERVERS(NotifyObserversTouchpadDeviceConfigurationChanged(), OnInputDeviceConfigurationChanged(InputDeviceEventObserver::kTouchpad))

NOTIFY_OBSERVERS(NotifyObserversGraphicsTabletDeviceConfigurationChanged(), OnInputDeviceConfigurationChanged(InputDeviceEventObserver::kGraphicsTablet))

NOTIFY_OBSERVERS(NotifyObserversUncategorizedDeviceConfigurationChanged(), OnInputDeviceConfigurationChanged(InputDeviceEventObserver::kUncategorized))

NOTIFY_OBSERVERS(NotifyObserversTouchscreenDeviceConfigurationChanged(), OnInputDeviceConfigurationChanged(InputDeviceEventObserver::kTouchscreen))

NOTIFY_OBSERVERS(NotifyObserversDeviceListsComplete(), OnDeviceListsComplete())

NOTIFY_OBSERVERS(NotifyObserversStylusStateChanged(StylusState state), OnStylusStateChanged(state))
