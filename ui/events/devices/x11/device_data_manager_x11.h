...
class EVENTS_DEVICES_X11_EXPORT DeviceDataManagerX11 : public DeviceDataManager {
public:
    ...
    void GetScrollOffsets(const x11::Event& xev, float* x_offset, float* y_offset, float* x_offset_ordinal, float* y_offset_ordinal, int* finger_count);
    ...
    // Pointers shouldn't be NULL.
    void GetScrollClassOffsets(const x11::Event& xev, double* x_offset, double* y_offset);
    ...
    void GetFlingData(const x11::Event& xev, float* vx, float* vy, float* vx_ordinal, float* vy_ordinal, bool* is_cancel);
    ...
    void GetMetricsData(const x11::Event& xev, GestureMetricsType* type, float* data1, float* data2);
    ...
    bool NormalizeData(x11::Input::DeviceId deviceid, const DataType type, double* value);
    ...
    bool GetDataRange(x11::Input::DeviceId deviceid, const DataType type, double* min, double* max);
    ...
    void SetDeviceListForTest(const std::vector<int>& touchscreen, const std::vector<int>& cmt_devices, const std::vector<int>& other_devices);

    void SetValuatorDataForTest(x11::Input::DeviceEvent* devev, DataType type, double value);

    // Sets the keys which are still allowed on a disabled keyboard device.
    void SetDisabledKeyboardAllowedKeys(std::unique_ptr<std::set<KeyboardCode>> excepted_keys);
...
protected:
    ...
    void OnKeyboardDevicesUpdated(const std::vector<KeyboardDevice>& devices) override;
...
private:
    ...
    void InitializeValuatorsForTest(int deviceid, int start_valuator, int end_valuator, double min_value, double max_value);

    // Updates a device based on a Valuator class info.
    DataType UpdateValuatorClassDevice(const x11::Input::DeviceClass::Valuator& valuator_class_info, x11::Atom* atoms, uint16_t deviceid);

    // Updates a device based on a Scroll class info.
    void UpdateScrollClassDevice(const x11::Input::DeviceClass::Scroll& scroll_class_info, uint16_t deviceid);
    ...
    // Returns true and sets the normalized value in |value| if normalization is
    // successful. Returns false and |value| is unchanged otherwise.
    double ExtractAndUpdateScrollOffset(DeviceDataManagerX11::ScrollInfo::AxisInfo* axis, double valuator) const;
