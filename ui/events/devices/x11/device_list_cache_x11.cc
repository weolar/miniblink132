...
const XDeviceList& DeviceListCacheX11::GetXDeviceList(x11::Connection* connection)
{
    if (!updated_)
...
const XIDeviceList& DeviceListCacheX11::GetXI2DeviceList(x11::Connection* connection)
{
    if (!updated_)
        UpdateDeviceList(connection);
    return xi_dev_list_;
