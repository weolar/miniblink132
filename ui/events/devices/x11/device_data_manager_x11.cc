...
static_assert(std::size(kCachedAtoms) == ui::DeviceDataManagerX11::DT_LAST_ENTRY, "kCachedAtoms count / enum mismatch");
...
template <typename Iterator> Iterator FindDeviceWithId(Iterator begin, Iterator end, x11::Input::DeviceId id)
{
...
const char kDisableHighPrecisionScrolling[] = "disable-high-precision-scrolling";

bool IsHighPrecisionScrollingDisabled()
{
    return base::CommandLine::ForCurrentProcess()->HasSwitch(kDisableHighPrecisionScrolling);
}
...
{
    return static_cast<double>(x.integral) + static_cast<double>(x.frac) / (1ULL << 32);
}
...
void DeviceDataManagerX11::CreateInstance()
{
    ...
    base::AtExitManager::RegisterTask(base::BindOnce(DeviceDataManager::DeleteInstance));
}
...
{
    return x11::Connection::Get()->xinput_version() >= std::pair<uint32_t, uint32_t> { 2, 2 };
}
...
void DeviceDataManagerX11::UpdateDeviceList(x11::Connection* connection)
{
    ...
    const XDeviceList& dev_list = ui::DeviceListCacheX11::GetInstance()->GetXDeviceList(connection);
    ...
    const XIDeviceList& info_list = ui::DeviceListCacheX11::GetInstance()->GetXI2DeviceList(connection);
    ...
    for (const auto& info : info_list) {
        ...
        if (info.type != x11::Input::DeviceType::SlavePointer && info.type != x11::Input::DeviceType::FloatingSlave) {
            continue;
        }
        ...
        data_type_lookup_[deviceid].resize(valuator_count_[deviceid], DT_LAST_ENTRY);
        for (int j = 0; j < kMaxSlotNum; j++)
            last_seen_valuator_[deviceid][j].resize(DT_LAST_ENTRY, 0);
        for (const auto& device_class : info.classes) {
            if (device_class.valuator.has_value()) {
                auto dt = UpdateValuatorClassDevice(*device_class.valuator, atoms, deviceid);
...
bool DeviceDataManagerX11::GetSlotNumber(const x11::Input::DeviceEvent& xiev, int* slot)
{
...
void DeviceDataManagerX11::GetEventRawData(const x11::Event& x11_event, EventData* data)
{
...
bool DeviceDataManagerX11::GetEventData(const x11::Event& x11_event, const DataType type, double* value)
{
    auto* xiev = x11_event.As<x11::Input::DeviceEvent>();
    if (!xiev)
        return false;
    ...
    if (type == DT_TOUCH_TRACKING_ID) {
        ...
        // events.
        if (xiev->opcode == x11::Input::DeviceEvent::TouchBegin || xiev->opcode == x11::Input::DeviceEvent::TouchEnd
...
bool DeviceDataManagerX11::IsStylusXInputEvent(const x11::Event& x11_event) const
{
    uint16_t source;
...
bool DeviceDataManagerX11::IsTouchpadXInputEvent(const x11::Event& x11_event) const
{
    uint16_t source;
    if (!GetSourceId(x11_event, &source))
        return false;
...
int DeviceDataManagerX11::GetScrollClassEventDetail(const x11::Event& x11_event) const
{
    ...
    return (horizontal_id != -1 && IsXinputMaskSet(xievent->valuator_mask.data(), horizontal_id) ? SCROLL_TYPE_HORIZONTAL : 0)
        | (vertical_id != -1 && IsXinputMaskSet(xievent->valuator_mask.data(), vertical_id) ? SCROLL_TYPE_VERTICAL : 0);
}

int DeviceDataManagerX11::GetScrollClassDeviceDetail(const x11::Event& x11_event) const
{
    auto* xiev = x11_event.As<x11::Input::DeviceEvent>();
    if (!xiev)
        return SCROLL_TYPE_NO_SCROLL;
...
bool DeviceDataManagerX11::HasEventData(const x11::Event& xev, const DataType type) const
{
...
bool DeviceDataManagerX11::IsScrollEvent(const x11::Event& x11_event) const
{
    ...
    return (HasEventData(x11_event, DT_CMT_SCROLL_X) || HasEventData(x11_event, DT_CMT_SCROLL_Y));
}
...
bool DeviceDataManagerX11::IsFlingEvent(const x11::Event& x11_event) const
{
    ...
    return (HasEventData(x11_event, DT_CMT_FLING_X) && HasEventData(x11_event, DT_CMT_FLING_Y) && HasEventData(x11_event, DT_CMT_FLING_STATE));
}

bool DeviceDataManagerX11::IsCMTMetricsEvent(const x11::Event& x11_event) const
{
    if (!IsCMTDeviceEvent(x11_event))
        return false;

    return (HasEventData(x11_event, DT_CMT_METRICS_TYPE) && HasEventData(x11_event, DT_CMT_METRICS_DATA1) && HasEventData(x11_event, DT_CMT_METRICS_DATA2));
}
...
bool DeviceDataManagerX11::HasGestureTimes(const x11::Event& x11_event) const
{
    ...
    return (HasEventData(x11_event, DT_CMT_START_TIME) && HasEventData(x11_event, DT_CMT_END_TIME));
}

void DeviceDataManagerX11::GetScrollOffsets(
    const x11::Event& xev, float* x_offset, float* y_offset, float* x_offset_ordinal, float* y_offset_ordinal, int* finger_count)
{
    *x_offset = 0;
    *y_offset = 0;
    *x_offset_ordinal = 0;
...
void DeviceDataManagerX11::GetScrollClassOffsets(const x11::Event& x11_event, double* x_offset, double* y_offset)
{
...
void DeviceDataManagerX11::InvalidateScrollClasses(x11::Input::DeviceId device_id)
{
...
void DeviceDataManagerX11::GetFlingData(const x11::Event& xev, float* vx, float* vy, float* vx_ordinal, float* vy_ordinal, bool* is_cancel)
{
    *vx = 0;
    *vy = 0;
    *vx_ordinal = 0;
...
void DeviceDataManagerX11::GetMetricsData(const x11::Event& xev, GestureMetricsType* type, float* data1, float* data2)
{
...
{
    return button > 0 && static_cast<unsigned int>(button) <= button_map_.size() ? button_map_[button - 1] : button;
}
...
void DeviceDataManagerX11::GetGestureTimes(const x11::Event& xev, double* start_time, double* end_time)
{
    *start_time = 0;
    *end_time = 0;
...
bool DeviceDataManagerX11::NormalizeData(x11::Input::DeviceId deviceid, const DataType type, double* value)
{
    double max_value;
    double min_value;
...
bool DeviceDataManagerX11::GetDataRange(x11::Input::DeviceId deviceid, const DataType type, double* min, double* max)
{
...
void DeviceDataManagerX11::SetDeviceListForTest(const std::vector<int>& touchscreen, const std::vector<int>& cmt_devices, const std::vector<int>& other_devices)
{
    for (int i = 0; i < kMaxDeviceNum; ++i) {
        valuator_count_[i] = 0;
        valuator_lookup_[i].clear();
    ...
    for (auto deviceid : touchscreen) {
        InitializeValuatorsForTest(deviceid, kTouchDataTypeStart, kTouchDataTypeEnd, 0, 1000);
    }
    ...
    for (auto deviceid : cmt_devices) {
        ...
        InitializeValuatorsForTest(deviceid, kCMTDataTypeStart, kCMTDataTypeEnd, -1000, 1000);
    }
...
void DeviceDataManagerX11::SetValuatorDataForTest(x11::Input::DeviceEvent* devev, DataType type, double value)
{
    ...
    for (int i = DT_LAST_ENTRY - 1; i > valuators - devev->axisvalues.data(); --i) {
...
void DeviceDataManagerX11::InitializeValuatorsForTest(int deviceid, int start_valuator, int end_valuator, double min_value, double max_value)
{
    valuator_lookup_[deviceid].resize(DT_LAST_ENTRY);
...
DeviceDataManagerX11::DataType DeviceDataManagerX11::UpdateValuatorClassDevice(
    const x11::Input::DeviceClass::Valuator& valuator_class_info, x11::Atom* atoms, uint16_t deviceid)
{
    DCHECK_LT(deviceid, kMaxDeviceNum);
    x11::Atom* label = std::find(atoms, atoms + DT_LAST_ENTRY, valuator_class_info.label);
...
void DeviceDataManagerX11::UpdateScrollClassDevice(const x11::Input::DeviceClass::Scroll& scroll_class_info, uint16_t deviceid)
{
...
double DeviceDataManagerX11::ExtractAndUpdateScrollOffset(ScrollInfo::AxisInfo* axis, double valuator) const
{
    double offset = 0;
    if (axis->seen)
...
void DeviceDataManagerX11::SetDisabledKeyboardAllowedKeys(std::unique_ptr<std::set<KeyboardCode>> excepted_keys)
{
...
bool DeviceDataManagerX11::IsDeviceEnabled(x11::Input::DeviceId device_id) const
{
...
bool DeviceDataManagerX11::IsEventBlocked(const x11::Event& x11_event)
{
    ...
    if (blocked_keyboard_allowed_keys_ && (xievent->opcode == x11::Input::DeviceEvent::KeyPress || xievent->opcode == x11::Input::DeviceEvent::KeyRelease)
        && blocked_keyboard_allowed_keys_->find(KeyboardCodeFromXKeyEvent(x11_event)) != blocked_keyboard_allowed_keys_->end()) {
        return false;
    }
...
void DeviceDataManagerX11::OnKeyboardDevicesUpdated(const std::vector<KeyboardDevice>& devices)
{
    std::vector<KeyboardDevice> keyboards(devices);
    for (auto blocked_iter = blocked_keyboard_devices_.begin(); blocked_iter != blocked_keyboard_devices_.end();) {
