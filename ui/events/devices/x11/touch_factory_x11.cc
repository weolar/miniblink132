...
void AddPointerDevicesFromString(const std::string& pointer_devices, EventPointerType type, std::vector<std::pair<int, EventPointerType>>* devices)
{
    for (std::string_view dev : base::SplitStringPiece(pointer_devices, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
        int devid;
...
void TouchFactory::SetTouchDeviceListFromCommandLine()
{
    ...
    AddPointerDevicesFromString(command_line->GetSwitchValueASCII(switches::kTouchDevices), EventPointerType::kTouch, &devices);
    AddPointerDevicesFromString(command_line->GetSwitchValueASCII(switches::kPenDevices), EventPointerType::kPen, &devices);
    if (!devices.empty())
...
void TouchFactory::UpdateDeviceList(x11::Connection* connection)
{
    ...
    const XIDeviceList& xi_dev_list = DeviceListCacheX11::GetInstance()->GetXI2DeviceList(connection);
    for (const auto& devinfo : xi_dev_list) {
        if (devinfo.type == x11::Input::DeviceType::FloatingSlave || devinfo.type == x11::Input::DeviceType::MasterPointer
            ...
            for (const auto& xiclassinfo : devinfo.classes) {
                ...
                auto master_id = devinfo.type == x11::Input::DeviceType::SlavePointer ? devinfo.attachment : devinfo.deviceid;
                ...
                if (devinfo.type == x11::Input::DeviceType::MasterPointer) {
                    ...
                    touch_device_list_[devinfo.deviceid] = { false, EventPointerType::kTouch };
                }
            }
            pointer_device_lookup_[static_cast<uint16_t>(devinfo.deviceid)] = (devinfo.type != x11::Input::DeviceType::SlavePointer);
...
bool TouchFactory::ShouldProcessDeviceEvent(const x11::Input::DeviceEvent& xiev)
{
    const bool is_touch_disabled = !touch_screens_enabled_;

    if (xiev.opcode == x11::Input::DeviceEvent::TouchBegin || xiev.opcode == x11::Input::DeviceEvent::TouchUpdate
        ...
        bool is_from_slave_device = !is_from_master_or_float && xiev.sourceid == xiev.deviceid;
    ...
    if (xiev.opcode == x11::Input::DeviceEvent::KeyPress || xiev.opcode == x11::Input::DeviceEvent::KeyRelease) {
        return !virtual_core_keyboard_device_.has_value() || *virtual_core_keyboard_device_ == xiev.deviceid;
    }

    return ShouldProcessEventForDevice(xiev.deviceid);
}

bool TouchFactory::ShouldProcessCrossingEvent(const x11::Input::CrossingEvent& xiev)
{
    ...
    if (xiev.opcode != x11::Input::CrossingEvent::Enter && xiev.opcode != x11::Input::CrossingEvent::Leave) {
        return true;
    }
...
void TouchFactory::SetTouchDeviceList(const std::vector<std::pair<int, EventPointerType>>& devices)
{
    touch_device_lookup_.reset();
    touch_device_list_.clear();
    for (auto& device : devices) {
        ...
        if (device_master_id_list_.find(device_id) != device_master_id_list_.end()) {
            ...
            touch_device_lookup_[static_cast<uint16_t>(device_master_id_list_[device_id])] = true;
...
{
    return IsValidDevice(static_cast<uint16_t>(deviceid)) && touch_device_lookup_[static_cast<uint16_t>(deviceid)];
}
...
EventPointerType TouchFactory::GetTouchDevicePointerType(x11::Input::DeviceId deviceid) const
{
...
void TouchFactory::CacheTouchscreenIds(x11::Input::DeviceId device_id)
{
    if (!DeviceDataManager::HasInstance())
        return;
    std::vector<TouchscreenDevice> touchscreens = DeviceDataManager::GetInstance()->GetTouchscreenDevices();
    const auto it = base::ranges::find(touchscreens, static_cast<int>(device_id), &TouchscreenDevice::id);
...
bool TouchFactory::ShouldProcessEventForDevice(x11::Input::DeviceId device_id) const
{
