...
std::ostream& operator<<(std::ostream& os, const InputDeviceType value)
{
    ...
    return os << "ui::InputDeviceType::unknown_value(" << static_cast<unsigned int>(value) << ")";
}
...
InputDevice::InputDevice(int id, InputDeviceType type, const std::string& name, const std::string& phys, const base::FilePath& sys_path, uint16_t vendor,
...
std::ostream& InputDevice::DescribeForLog(std::ostream& os) const
{
    return os << "class=ui::InputDevice id=" << id << std::endl
              ...
              << " suspected_keyboard_imposter=" << suspected_keyboard_imposter << std::endl
              << " suspected_mouse_imposter=" << suspected_mouse_imposter << std::endl
              << " sys_path=\"" << sys_path.AsUTF8Unsafe() << "\"" << std::endl
              << " vendor_id=" << base::StringPrintf("%04X", vendor_id) << std::endl
              << " product_id=" << base::StringPrintf("%04X", product_id) << std::endl
