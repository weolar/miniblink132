...
struct EVENTS_DEVICES_EXPORT TouchscreenDevice : public InputDevice {
    ...
    TouchscreenDevice(int id, InputDeviceType type, const std::string& name, const gfx::Size& size, int touch_points, bool has_stylus = false,
        bool has_stylus_garage_switch = false);

    TouchscreenDevice(const InputDevice& input_device, const gfx::Size& size, int touch_points, bool has_stylus = false, bool has_stylus_garage_switch = false);
