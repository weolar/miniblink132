...
struct EVENTS_DEVICES_EXPORT KeyboardDevice : public InputDevice {
    ...
    KeyboardDevice(int id, InputDeviceType type, const std::string& name, bool has_assistant_key = false, bool has_function_key = false);
    KeyboardDevice(int id, InputDeviceType type, const std::string& name, const std::string& phys, const base::FilePath& sys_path, uint16_t vendor,
        uint16_t product, uint16_t version, bool has_assistant_key = false, bool has_function_key = false);
    explicit KeyboardDevice(InputDevice input_device, bool has_assistant_key = false, bool has_function_key = false);
