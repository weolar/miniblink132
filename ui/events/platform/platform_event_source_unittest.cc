...
void RemoveDispatchers(PlatformEventDispatcher* first, PlatformEventDispatcher* second)
{
...
public:
    ...
    uint32_t Dispatch(const PlatformEvent& event)
    {
...
public:
    ...
    TestPlatformEventDispatcher& operator=(const TestPlatformEventDispatcher&) = delete;
...
public:
    ...
    TestPlatformEventObserver& operator=(const TestPlatformEventObserver&) = delete;
...
public:
    ...
    TestPlatformEventSource* source()
    {
        return source_.get();
    }
...
TEST_F(PlatformEventTest, DispatcherOrder)
{
    ...
    for (auto id : sequence) {
        dispatchers.push_back(std::make_unique<TestPlatformEventDispatcher>(id, &list_dispatcher));
    }
    std::optional<PlatformEvent> event(CreateInvalidPlatformEvent());
    source()->Dispatch(*event);
    ASSERT_EQ(std::size(sequence), list_dispatcher.size());
    EXPECT_EQ(std::vector<int>(sequence, sequence + std::size(sequence)), list_dispatcher);
}
...
TEST_F(PlatformEventTest, ObserverOrder)
{
    ...
    for (auto id : sequence) {
        observers.push_back(std::make_unique<TestPlatformEventObserver>(id, &list_observer));
    }
    std::optional<PlatformEvent> event(CreateInvalidPlatformEvent());
    source()->Dispatch(*event);
    ASSERT_EQ(std::size(sequence), list_observer.size());
    EXPECT_EQ(std::vector<int>(sequence, sequence + std::size(sequence)), list_observer);
}
...
TEST_F(PlatformEventTest, OverriddenDispatcherBasic)
{
    ...
    std::unique_ptr<ScopedEventDispatcher> handle = source()->OverrideDispatcher(&overriding_dispatcher);
    source()->Dispatch(*event);
    ASSERT_EQ(2u, list.size());
    EXPECT_EQ(15, list[0]);
    EXPECT_EQ(20, list[1]);
...
TEST_F(PlatformEventTest, OverriddenDispatcherInvokeDefaultDispatcher)
{
    ...
    std::unique_ptr<ScopedEventDispatcher> second_override_handle = source()->OverrideDispatcher(&second_overriding);
    source()->Dispatch(*event);
    ASSERT_EQ(2u, list.size());
    EXPECT_EQ(15, list[0]);
    EXPECT_EQ(50, list[1]);
...
public:
    ...
    RunCallbackDuringDispatch& operator=(const RunCallbackDuringDispatch&) = delete;
...
TEST_F(PlatformEventTest, DispatcherRemovesNextDispatcherDuringDispatch)
{
    ...
    second.set_callback(base::BindOnce(&RemoveDispatcher, base::Unretained(&third)));
...
TEST_F(PlatformEventTest, DispatcherRemovesSelfDuringDispatch)
{
    ...
    second.set_callback(base::BindOnce(&RemoveDispatcher, base::Unretained(&second)));
...
TEST_F(PlatformEventTest, DispatcherRemovesPrevDispatcherDuringDispatch)
{
    ...
    second.set_callback(base::BindOnce(&RemoveDispatcher, base::Unretained(&first)));
...
TEST_F(PlatformEventTest, DispatcherRemovesPrevDispatchersDuringDispatch)
{
    ...
    third.set_callback(base::BindOnce(&RemoveDispatchers, base::Unretained(&first), base::Unretained(&second)));
...
TEST_F(PlatformEventTest, DispatcherAddedDuringDispatchReceivesEvent)
{
    ...
    second.set_callback(base::BindOnce(&AddDispatcher, base::Unretained(&fourth)));
    list.clear();
    source()->Dispatch(*event);
    ASSERT_EQ(4u, list.size());
    EXPECT_EQ(10, list[0]);
...
public:
    ...
    PlatformEventTestWithMessageLoop(const PlatformEventTestWithMessageLoop&) = delete;
    PlatformEventTestWithMessageLoop& operator=(const PlatformEventTestWithMessageLoop&) = delete;
    ...
    void Run()
    {
        task_environment_.GetMainThreadTaskRunner()->PostTask(
            FROM_HERE, base::BindOnce(&PlatformEventTestWithMessageLoop::RunTestImpl, base::Unretained(this)));
...
// destroyed.
class ScopedDispatcherRestoresAfterDestroy : public PlatformEventTestWithMessageLoop {
public:
    // PlatformEventTestWithMessageLoop:
    void RunTestImpl() override
    {
        ...
        std::unique_ptr<ScopedEventDispatcher> first_override_handle = source()->OverrideDispatcher(&first_overriding);
...
public:
    ...
    DestroyScopedHandleDispatcher& operator=(const DestroyScopedHandleDispatcher&) = delete;
...
// function correctly.
class ConsecutiveOverriddenDispatcherInTheSameMessageLoopIteration : public PlatformEventTestWithMessageLoop {
public:
    void NestedTask(std::unique_ptr<ScopedEventDispatcher> dispatch_handle, std::vector<int>* list)
    {
        std::optional<PlatformEvent> event(CreateInvalidPlatformEvent());
        source()->Dispatch(*event);
        ASSERT_EQ(2u, list->size());
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(base::IgnoreResult(&TestPlatformEventSource::Dispatch), base::Unretained(source()), *event));
        run_loop.Run();
        ASSERT_EQ(2u, list->size());
        EXPECT_EQ(15, (*list)[0]);
        EXPECT_EQ(70, (*list)[1]);
    ...
    void RunTestImpl() override
    {
        ...
        std::unique_ptr<ScopedEventDispatcher> override_handle = source()->OverrideDispatcher(&overriding);
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE,
            base::BindOnce(&ConsecutiveOverriddenDispatcherInTheSameMessageLoopIteration::NestedTask, base::Unretained(this), std::move(override_handle),
...
RUN_TEST_IN_MESSAGE_LOOP(ConsecutiveOverriddenDispatcherInTheSameMessageLoopIteration)
