...
enum DeviceType { DEVICE_TYPE_KEYBOARD, DEVICE_TYPE_MOUSE, DEVICE_TYPE_TOUCHPAD, DEVICE_TYPE_TOUCHSCREEN, DEVICE_TYPE_OTHER };

using KeyboardDeviceCallback = base::OnceCallback<void(const std::vector<KeyboardDevice>&)>;

using TouchpadDeviceCallback = base::OnceCallback<void(const std::vector<TouchpadDevice>&)>;

using TouchscreenDeviceCallback = base::OnceCallback<void(const std::vector<TouchscreenDevice>&)>;

using InputDeviceCallback = base::OnceCallback<void(const std::vector<InputDevice>&)>;
...
struct DeviceInfo {
    DeviceInfo(const x11::Input::XIDeviceInfo& device, DeviceType type, const base::FilePath& path)
        : id(device.deviceid)
        , name(device.name)
        , use(device.type)
        , type(type)
...
bool IsKnownInvalidKeyboardDevice(const std::string& name)
{
    ...
    constexpr auto kSet
        = base::MakeFixedFlatSet<std::string_view>({ "Power Button", "Sleep Button", "Video Bus", "gpio-keys.5", "gpio-keys.12", "ROCKCHIP-I2S Headset Jack" });
...
base::FilePath GetDevicePath(x11::Connection* connection, const x11::Input::XIDeviceInfo& device)
{
    // Skip the main pointer and keyboard since XOpenDevice() generates a
    // BadDevice error when passed these devices.
    if (device.type == x11::Input::DeviceType::MasterPointer || device.type == x11::Input::DeviceType::MasterKeyboard)
    ...
    std::string path(reinterpret_cast<char*>(reply->data8->data()), reply->data8->size());
...
void HandleKeyboardDevicesInWorker(const std::vector<DeviceInfo>& device_infos, scoped_refptr<base::TaskRunner> reply_runner, KeyboardDeviceCallback callback)
{
    ...
    for (const DeviceInfo& device_info : device_infos) {
        ...
        devices.emplace_back(static_cast<uint16_t>(device_info.id), type, device_info.name);
    }

    reply_runner->PostTask(FROM_HERE, base::BindOnce(std::move(callback), devices));
}

// Helper used to parse mouse information. When it is done it uses
// |reply_runner| and |callback| to update the state on the UI thread.
void HandleMouseDevicesInWorker(const std::vector<DeviceInfo>& device_infos, scoped_refptr<base::TaskRunner> reply_runner, InputDeviceCallback callback)
{
    std::vector<InputDevice> devices;
    for (const DeviceInfo& device_info : device_infos) {
        if (device_info.type != DEVICE_TYPE_MOUSE || device_info.use != x11::Input::DeviceType::SlavePointer) {
            continue;
        }
...
// |reply_runner| and |callback| to update the state on the UI thread.
void HandleTouchpadDevicesInWorker(const std::vector<DeviceInfo>& device_infos, scoped_refptr<base::TaskRunner> reply_runner, TouchpadDeviceCallback callback)
{
    std::vector<TouchpadDevice> devices;
    for (const DeviceInfo& device_info : device_infos) {
        if (device_info.type != DEVICE_TYPE_TOUCHPAD || device_info.use != x11::Input::DeviceType::SlavePointer) {
            continue;
        }
...
// |reply_runner| and |callback| to update the state on the UI thread.
void HandleTouchscreenDevicesInWorker(const std::vector<DeviceInfo>& device_infos, const DisplayState& display_state,
    scoped_refptr<base::TaskRunner> reply_runner, TouchscreenDeviceCallback callback)
{
    std::vector<TouchscreenDevice> devices;
    if (display_state.mt_position_x == x11::Atom::None || display_state.mt_position_y == x11::Atom::None)
        return;

    for (const DeviceInfo& device_info : device_infos) {
        if (device_info.type != DEVICE_TYPE_TOUCHSCREEN
            || (device_info.use != x11::Input::DeviceType::FloatingSlave && device_info.use != x11::Input::DeviceType::SlavePointer)) {
            continue;
        }
        ...
        for (const auto& valuator : device_info.valuator_class_infos) {
            if (display_state.mt_position_x == valuator.label) {
                ...
                if (valuator.number == 0 && valuator.mode == x11::Input::ValuatorMode::Absolute && Fp3232ToDouble(valuator.min) == 0.0) {
                    max_x = Fp3232ToDouble(valuator.max);
                }
            } else if (display_state.mt_position_y == valuator.label) {
                // Ignore Y axis valuator with unexpected properties
                if (valuator.number == 1 && valuator.mode == x11::Input::ValuatorMode::Absolute && Fp3232ToDouble(valuator.min) == 0.0) {
        ...
        if (max_x > 0.0 && max_y > 0.0) {
            ...
            // the size.
            devices.emplace_back(static_cast<uint16_t>(device_info.id), type, device_info.name, gfx::Size(max_x + 1, max_y + 1),
...
void HandleHotplugEventInWorker(
    const std::vector<DeviceInfo>& devices, const DisplayState& display_state, scoped_refptr<base::TaskRunner> reply_runner, UiCallbacks callbacks)
{
    HandleTouchscreenDevicesInWorker(devices, display_state, reply_runner, std::move(callbacks.touchscreen_callback));
    HandleKeyboardDevicesInWorker(devices, reply_runner, std::move(callbacks.keyboard_callback));
    HandleMouseDevicesInWorker(devices, reply_runner, std::move(callbacks.mouse_callback));
    HandleTouchpadDevicesInWorker(devices, reply_runner, std::move(callbacks.touchpad_callback));
    reply_runner->PostTask(FROM_HERE, std::move(callbacks.hotplug_finished_callback));
}
...
void X11HotplugEventHandler::OnHotplugEvent()
{
    ...
    const XDeviceList& device_list_xi = DeviceListCacheX11::GetInstance()->GetXDeviceList(connection);
    const XIDeviceList& device_list_xi2 = DeviceListCacheX11::GetInstance()->GetXI2DeviceList(connection);
    ...
    for (const auto& device : device_list_xi) {
        ...
        if (type == x11::GetAtom("KEYBOARD") || type == x11::GetAtom("xwayland-keyboard")) {
            device_types[id] = DEVICE_TYPE_KEYBOARD;
        } else if (type == x11::GetAtom("MOUSE") || type == x11::GetAtom("xwayland-pointer")) {
            device_types[id] = DEVICE_TYPE_MOUSE;
        } else if (type == x11::GetAtom("TOUCHPAD")) {
            device_types[id] = DEVICE_TYPE_TOUCHPAD;
        } else if (type == x11::GetAtom("TOUCHSCREEN") || type == x11::GetAtom("xwayland-touch")) {
    ...
    for (const auto& device : device_list_xi2) {
        ...
        auto deviceid = static_cast<uint16_t>(device.deviceid);
        DeviceType device_type = deviceid < kMaxDeviceNum ? device_types[deviceid] : DEVICE_TYPE_OTHER;
        device_infos.emplace_back(device, device_type, GetDevicePath(connection, device));
    }
    ...
    base::ThreadPool::PostTask(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::CONTINUE_ON_SHUTDOWN },
        base::BindOnce(&HandleHotplugEventInWorker, device_infos, display_state, base::SingleThreadTaskRunner::GetCurrentDefault(), std::move(callbacks)));
}
