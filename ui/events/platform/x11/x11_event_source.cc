...
void InitializeXkb(x11::Connection* connection)
{
    ...
    xkb.PerClientFlags({
        ...
        .OnResponse(base::BindOnce([](x11::Xkb::PerClientFlagsResponse response) {
            if (!response || !static_cast<bool>(response->supported & x11::Xkb::PerClientFlag::DetectableAutoRepeat)) {
...
std::optional<gfx::Point> GetRootCursorLocationFromEvent(const x11::Event& event)
{
    ...
    if (event.As<x11::ButtonEvent>() || event.As<x11::MotionNotifyEvent>() || event.As<x11::CrossingEvent>()) {
        is_valid_event = true;
    } else if (device
        && (device->opcode == x11::Input::DeviceEvent::ButtonPress || device->opcode == x11::Input::DeviceEvent::ButtonRelease
            || device->opcode == x11::Input::DeviceEvent::Motion)) {
        is_valid_event = touch_factory->ShouldProcessDeviceEvent(*device);
    } else if (crossing && (crossing->opcode == x11::Input::CrossingEvent::Enter || crossing->opcode == x11::Input::CrossingEvent::Leave)) {
...
x11::Time X11EventSource::GetCurrentServerTime()
{
    ...
    if (!dummy_initialized_) {
        ...
        dummy_window_events_ = connection_->ScopedSelectEvent(dummy_window_, x11::EventMask::PropertyChange);
...
void X11EventSource::OnEvent(const x11::Event& x11_event)
{
    ...
    if (x11_event.As<x11::Input::HierarchyEvent>()) {
        ...
        if (device->reason == x11::Input::ChangeReason::DeviceChange) {
            ...
            ui::DeviceDataManagerX11::GetInstance()->InvalidateScrollClasses(device->sourceid);
        }
    }
    ...
    if (crossing && crossing->opcode == x11::CrossingEvent::EnterNotify && crossing->detail != x11::NotifyDetail::Inferior
        && crossing->mode != x11::NotifyMode::Ungrab) {
        // Clear stored scroll data
        ui::DeviceDataManagerX11::GetInstance()->InvalidateScrollClasses(DeviceDataManagerX11::kAllDevices);
    }
    ...
    if (ui::PlatformEventSource::ShouldIgnoreNativePlatformEvents() && translated_event && translated_event->IsMouseEvent()) {
        return;
    }
    if (translated_event && translated_event->type() != EventType::kUnknown) {
#if BUILDFLAG(IS_CHROMEOS)
        if (translated_event->IsLocatedEvent()) {
            ui::CursorController::GetInstance()->SetCursorLocation(translated_event->AsLocatedEvent()->location_f());
        }
#endif
