...
void X11EventWatcherFdWatch::StartWatching()
{
    ...
    base::CurrentUIThread::Get()->WatchFileDescriptor(fd, true, base::MessagePumpForUI::WATCH_READ, &connection_watcher_, this);

    PCHECK(pipe2(pipe_, O_CLOEXEC | O_NONBLOCK) != -1);
    base::CurrentUIThread::Get()->WatchFileDescriptor(pipe_[0], true, base::MessagePumpForUI::WATCH_READ, &pipe_watcher_, this);
    started_ = true;
}
