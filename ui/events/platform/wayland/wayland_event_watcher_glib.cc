...
gboolean WatchSourcePrepare(GSource* source, gint* timeout_ms)
{
    ...
    *timeout_ms = -1;
    ...
    auto* event_watcher_glib = static_cast<GLibWaylandSource*>(source)->event_watcher.get();
...
gboolean WatchSourceDispatch(GSource* source, GSourceFunc unused_func, gpointer data)
{
    auto* event_watcher_glib = static_cast<GLibWaylandSource*>(source)->event_watcher.get();
...
WaylandEventWatcherGlib::WaylandEventWatcherGlib(wl_display* display, wl_event_queue* event_queue, bool use_threaded_polling)
...
bool WaylandEventWatcherGlib::StartWatchingFD(int fd)
{
    ...
    GLibWaylandSource* glib_wayland_source = static_cast<GLibWaylandSource*>(g_source_new(&WatchSourceFuncs, sizeof(GLibWaylandSource)));
...
// static
std::unique_ptr<WaylandEventWatcher> WaylandEventWatcher::CreateWaylandEventWatcher(wl_display* display, wl_event_queue* event_queue, bool use_threaded_polling)
{
    return std::make_unique<WaylandEventWatcherGlib>(display, event_queue, use_threaded_polling);
}
