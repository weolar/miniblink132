...
WaylandEventWatcherFdWatch::WaylandEventWatcherFdWatch(wl_display* display, wl_event_queue* event_queue, bool use_threaded_polling)
...
bool WaylandEventWatcherFdWatch::StartWatchingFD(int fd)
{
    ...
    return base::CurrentUIThread::Get()->WatchFileDescriptor(fd, true, base::MessagePumpForUI::WATCH_READ, &controller_, this);
}

void WaylandEventWatcherFdWatch::StopWatchingFD()
{
    DCHECK(base::CurrentUIThread::IsSet());
    CHECK(controller_.StopWatchingFileDescriptor()) << "Unable to stop watching the Wayland display fd.";
}
...
{
    return std::make_unique<WaylandEventWatcherFdWatch>(display, event_queue, use_threaded_polling);
}
