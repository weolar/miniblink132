...
std::string GetWaylandProtocolError(int err, wl_display* display)
{
    ...
    if (err == EPROTO) {
        ...
        if (intf) {
            error_string = base::StringPrintf("Fatal Wayland protocol error %u on interface %s (object %u). "
                                              "Shutting down..",
                ec, intf->name, id);
        } else {
            error_string = base::StringPrintf("Fatal Wayland protocol error %u. Shutting down..", ec);
        }
    } else {
        error_string = base::StringPrintf("Fatal Wayland communication error: %s.", std::strerror(err));
    }
...
void RecordCrashKeys(const std::string& error_string)
{
    ...
    base::Environment::Create()->GetVar(base::nix::kXdgCurrentDesktopEnvVar, &compositor_name);
...
public:
    explicit WaylandEventWatcherThread(base::OnceClosure start_processing_events_cb)
    ...
    ~WaylandEventWatcherThread() override
    {
        Stop();
    }
...
WaylandEventWatcher::WaylandEventWatcher(wl_display* display, wl_event_queue* event_queue, bool use_threaded_polling)
    : display_(display)
...
void WaylandEventWatcher::SetShutdownCb(base::OnceCallback<void()> shutdown_cb)
{
...
void WaylandEventWatcher::StartProcessingEvents()
{
    ...
    if (use_threaded_polling_) {
        if (!ui_thread_task_runner_) {
            ui_thread_task_runner_ = base::SingleThreadTaskRunner::GetCurrentDefault();
        ...
        if (!thread_) {
            ...
            thread_ = std::make_unique<WaylandEventWatcherThread>(base::BindOnce(&WaylandEventWatcher::StartProcessingEventsThread, base::Unretained(this)));
            ...
            watching_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&WaylandEventWatcher::StartProcessingEventsInternal, base::Unretained(this)));
        }
    } else {
...
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    watching_thread_task_runner_ = base::SingleThreadTaskRunner::GetCurrentDefault();
...
void WaylandEventWatcher::StopProcessingEvents()
{
    ...
    if (use_threaded_polling_) {
        watching_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&WaylandEventWatcher::StopProcessingEventsInternal, base::Unretained(this)));
    } else {
...
{
    if (use_threaded_polling_ && watching_thread_task_runner_->BelongsToCurrentThread()) {
        base::WaitableEvent event;
        auto cb = base::BindOnce(&WaylandEventWatcher::WlDisplayDispatchPendingQueueInternal, weak_this_, &event);
...
void WaylandEventWatcher::WlDisplayDispatchPendingQueueInternal(base::WaitableEvent* event)
{
