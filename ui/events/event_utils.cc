...
#define UMA_HISTOGRAM_EVENT_LATENCY_TIMES(name, sample) UMA_HISTOGRAM_CUSTOM_TIMES(name, sample, base::Milliseconds(1), base::Minutes(1), 50)
...
void RecordEventLatencyTrace(perfetto::StaticString name, base::TimeTicks time_stamp, base::TimeTicks current_time)
{
...
{
    return GetInternalDisplayTouchSupport() == display::Display::TouchSupport::AVAILABLE;
}
...
void ComputeEventLatencyOS(const PlatformEvent& native_event)
{
    ...
    base::TimeTicks time_stamp = EventLatencyTimeFromNative(native_event, current_time);
    EventType type = EventTypeFromNative(native_event);
    ComputeEventLatencyOS(type, time_stamp, current_time);
}

void ComputeEventLatencyOS(EventType type, base::TimeTicks time_stamp, base::TimeTicks current_time)
{
    static constexpr char kKeyPressedEventName[] = "Event.Latency.OS2.KEY_PRESSED";
    static constexpr char kMousePressedEventName[] = "Event.Latency.OS2.MOUSE_PRESSED";
    static constexpr char kMouseWheelEventName[] = "Event.Latency.OS2.MOUSE_WHEEL";
    static constexpr char kTouchCancelledEventName[] = "Event.Latency.OS2.TOUCH_CANCELLED";
    static constexpr char kTouchMovedEventName[] = "Event.Latency.OS2.TOUCH_MOVED";
    static constexpr char kTouchPressedEventName[] = "Event.Latency.OS2.TOUCH_PRESSED";
    static constexpr char kTouchReleasedEventName[] = "Event.Latency.OS2.TOUCH_RELEASED";
    ...
    case EventType::kTouchReleased:
        ...
        RecordEventLatencyTrace(kTouchReleasedEventName, time_stamp, current_time);
        return;
    case EventType::kTouchCancelled:
        UMA_HISTOGRAM_EVENT_LATENCY_TIMES(kTouchCancelledEventName, delta);
        RecordEventLatencyTrace(kTouchCancelledEventName, time_stamp, current_time);
        return;
    case EventType::kKeyPressed:
...
void ComputeEventLatencyOSFromTOUCHINPUT(EventType event_type, TOUCHINPUT touch_input, base::TimeTicks current_time)
{
    base::TimeTicks time_stamp = EventLatencyTimeFromTickClock(touch_input.dwTime, current_time);
    ComputeEventLatencyOS(event_type, time_stamp, current_time);
}

void ComputeEventLatencyOSFromPOINTER_INFO(EventType event_type, POINTER_INFO pointer_info, base::TimeTicks current_time)
{
    ...
    if (pointer_info.PerformanceCount) {
        ...
        time_stamp = EventLatencyTimeFromPerformanceCounter(pointer_info.PerformanceCount);
    } else if (pointer_info.dwTime) {
        time_stamp = EventLatencyTimeFromTickClock(pointer_info.dwTime, current_time);
    } else {
