...
#define EXPECT_CODEPOINT_MAPS(codepoint)                                                                                                                       \
    EXPECT_EQ(ui::DomKey::FromCharacter(codepoint), ui::DomKeyFromFuchsiaKeyMeaning(KeyMeaning::WithCodepoint(codepoint))) << " for codepoint " << codepoint;
...
{
    // Yen sign.
    EXPECT_EQ(ui::DomKey::FromCharacter(0x00a5), ui::DomKeyFromFuchsiaKeyMeaning(KeyMeaning::WithCodepoint(165)));

    // Right brace.
    EXPECT_EQ(ui::DomKey::FromCharacter(0x007d), ui::DomKeyFromFuchsiaKeyMeaning(KeyMeaning::WithCodepoint(125)));

    // Lower case c with circumflex.
    EXPECT_EQ(ui::DomKey::FromCharacter(0x0109), ui::DomKeyFromFuchsiaKeyMeaning(KeyMeaning::WithCodepoint(265)));

    // Check that NonPrintableKeys are converted correctly.
    EXPECT_EQ(ui::DomKey::ENTER, ui::DomKeyFromFuchsiaKeyMeaning(KeyMeaning::WithNonPrintableKey(NonPrintableKey::kEnter)));
}
