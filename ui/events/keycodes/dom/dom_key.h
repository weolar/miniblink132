...
private:
    ...
    static_assert(TYPE_UNICODE != 0 && TYPE_NON_UNICODE != 0 && TYPE_DEAD != 0, "suspicious representation change");
...
#undef DOM_KEY_MAP_DECLARATION_END
    ...
    {
        return (value_ & TYPE_MASK) == TYPE_UNICODE;
    }

    // True if the value is a dead key.
    bool IsDeadKey() const
    {
        return (value_ & TYPE_MASK) == TYPE_DEAD;
    }

    // True if the value is the same as the value of DomKey::COMPOSE.
    bool IsComposeKey() const
    {
        return *this == DomKey::COMPOSE;
    }
    ...
    constexpr static DomKey DeadKeyFromCombiningCharacter(uint32_t combining_character)
    {
...
private:
    static bool IsValidValue(Base value)
    {
