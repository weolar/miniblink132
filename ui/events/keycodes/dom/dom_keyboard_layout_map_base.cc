...
base::flat_map<std::string, std::string> DomKeyboardLayoutMapBase::Generate()
{
    ...
    std::unique_ptr<ui::DomKeyboardLayoutManager> keyboard_layout_manager = std::make_unique<ui::DomKeyboardLayoutManager>();

    for (size_t i = 0; i < keyboard_layout_count; i++) {
        DomKeyboardLayout* const dom_keyboard_layout = keyboard_layout_manager->GetLayout(i);
...
void DomKeyboardLayoutMapBase::PopulateLayout(uint32_t keyboard_layout_index, ui::DomKeyboardLayout* layout)
{
    DCHECK(layout);

    for (const ui::DomCode dom_code : ui::kWritingSystemKeyDomCodes) {
        ui::DomKey dom_key = GetDomKeyFromDomCodeForLayout(dom_code, keyboard_layout_index);
