...
private:
    ...
    ui::DomKey GetDomKeyFromDomCodeForLayout(ui::DomCode dom_code, uint32_t keyboard_layout_index) override;
...
uint32_t DomKeyboardLayoutMapWin::GetKeyboardLayoutCount()
{
    ...
    const size_t copy_count = ::GetKeyboardLayoutList(keyboard_layout_handles_.size(), keyboard_layout_handles_.data());
    if (!copy_count) {
        DPLOG(ERROR) << "GetKeyboardLayoutList failed: ";
        return false;
    }
    ...
    // evaluated first.
    auto iter = base::ranges::find(keyboard_layout_handles_, GetKeyboardLayout(0));
    if (iter != keyboard_layout_handles_.begin() && iter != keyboard_layout_handles_.end())
        std::iter_swap(keyboard_layout_handles_.begin(), iter);

    return keyboard_layout_handles_.size();
}

ui::DomKey DomKeyboardLayoutMapWin::GetDomKeyFromDomCodeForLayout(ui::DomCode dom_code, uint32_t keyboard_layout_index)
{
    ...
    uint32_t virtual_key_code = MapVirtualKeyEx(scan_code, MAPVK_VSC_TO_VK_EX, keyboard_layout);
    if (!virtual_key_code) {
        if (GetLastError() != 0)
    ...
    int key_type = ::ToUnicodeEx(virtual_key_code, scan_code, keyboard_state, char_buffer, std::size(char_buffer), /*wFlags=*/0, keyboard_layout);
    ...
    if (key_type == 1)
        ...
        ::ToUnicodeEx(0x0020, 0x0039, keyboard_state, char_buffer, std::size(char_buffer), /*wFlags=*/0, keyboard_layout);
    }
    return key;
}
