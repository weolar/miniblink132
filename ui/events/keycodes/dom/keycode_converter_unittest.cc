...
#define DOM_CODE_DECLARATION const KeycodeConverterData kKeycodeConverterData[] =
    ...
    for (const auto& it : kKeycodeConverterData) {
        ...
        for (size_t i = 0; i < kNativeColumns; ++i) {
            ...
            EXPECT_EQ(0U, usb_to_native[i].count(it.usb_keycode))
                << " duplicate of USB code 0x" << std::hex << std::setfill('0') << std::setw(6) << it.usb_keycode << " to native 0x" << std::setw(4)
                << it.native_keycode[i] << " (previous was 0x" << std::setw(4) << usb_to_native[i][it.usb_keycode] << ")";
            usb_to_native[i][it.usb_keycode] = it.native_keycode[i];
            EXPECT_EQ(0U, native_to_usb[i].count(it.native_keycode[i]))
                << " duplicate of native code 0x" << std::hex << std::setfill('0') << std::setw(4) << it.native_keycode[i] << " to USB 0x" << std::setw(6)
                << it.usb_keycode << " (previous was 0x" << std::setw(6) << native_to_usb[i][it.native_keycode[i]] << ")";
...
TEST(UsbKeycodeMap, Basic)
{
    ...
    EXPECT_EQ(ui::KeycodeConverter::InvalidUsbKeycode(), keycode_map[0].usb_keycode);
    EXPECT_EQ(ui::KeycodeConverter::InvalidNativeKeycode(), keycode_map[0].native_keycode);
    EXPECT_EQ(ui::KeycodeConverter::InvalidNativeKeycode(), ui::KeycodeConverter::DomCodeToNativeKeycode(ui::DomCode::NONE));
    ...
    for (size_t i = 0; i < numEntries; ++i) {
        ...
        EXPECT_EQ(entry->native_keycode, ui::KeycodeConverter::UsbKeycodeToNativeKeycode(entry->usb_keycode));
        ...
        if (entry->code && *entry->code) {
            ...
            EXPECT_EQ(entry->native_keycode, ui::KeycodeConverter::DomCodeToNativeKeycode(dom_code));
        }
    }
}
...
TEST(UsbKeycodeMap, NonExistent)
{
    ...
    EXPECT_EQ(ui::KeycodeConverter::InvalidNativeKeycode(), ui::KeycodeConverter::UsbKeycodeToNativeKeycode(kUsbNonExistentKeycode));
}
...
TEST(UsbKeycodeMap, UsBackslashIsNonUsHash)
{
    ...
    EXPECT_EQ(ui::KeycodeConverter::UsbKeycodeToNativeKeycode(kUsbUsBackslash), ui::KeycodeConverter::UsbKeycodeToNativeKeycode(kUsbNonUsHash));
}
...
TEST(KeycodeConverter, DomCode)
{
    ...
    for (size_t i = 0; i < numEntries; ++i) {
        ...
        if (entry->code) {
            ...
            EXPECT_STREQ(entry->code, ui::KeycodeConverter::DomCodeToCodeString(code).c_str());
        }
        ui::DomCode code = ui::KeycodeConverter::NativeKeycodeToDomCode(entry->native_keycode);
        EXPECT_EQ(entry->native_keycode, ui::KeycodeConverter::DomCodeToNativeKeycode(code));
    }
}
...
TEST(KeycodeConverter, DomKey)
{
    const struct {
        ...
        { ui::DomKey::FromCharacter(0x1F648), true, false, true, "\xF0\x9F\x99\x88", false },
        ...
        // 'Dead' key.
        { ui::DomKey::DeadKeyFromCombiningCharacter(0xFFFF), false, true, true, "Dead", true },
    ...
    for (size_t i = 0; (s = ui::KeycodeConverter::DomKeyStringForTest(i)) != nullptr; ++i) {
        SCOPED_TRACE(i);
