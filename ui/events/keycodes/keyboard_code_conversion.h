...
EVENTS_BASE_EXPORT char16_t DomCodeToUsLayoutCharacter(DomCode dom_code, int flags);
[[nodiscard]] EVENTS_BASE_EXPORT bool DomCodeToUsLayoutDomKey(DomCode dom_code, int flags, DomKey* dom_key, KeyboardCode* key_code);
...
// or action, keys.
[[nodiscard]] EVENTS_BASE_EXPORT bool DomCodeToNonPrintableDomKey(DomCode dom_code, DomKey* dom_key, KeyboardCode* key_code);
...
// are untouched.
[[nodiscard]] EVENTS_BASE_EXPORT bool DomCodeToControlCharacter(DomCode dom_code, int flags, DomKey* dom_key, KeyboardCode* key_code);

// Returns a Windows-based VKEY for a non-printable DOM Level 3 |key|.
// The returned VKEY is non-located (e.g. VKEY_SHIFT).
EVENTS_BASE_EXPORT KeyboardCode NonPrintableDomKeyToKeyboardCode(DomKey dom_key);
...
// non-located value.
EVENTS_BASE_EXPORT KeyboardCode LocatedToNonLocatedKeyboardCode(KeyboardCode key_code);

// Determine the located VKEY corresponding to a non-located VKEY.
EVENTS_BASE_EXPORT KeyboardCode NonLocatedToLocatedKeyboardCode(KeyboardCode key_code, DomCode dom_code);
...
// (e.g. VKEY_SHIFT).
EVENTS_BASE_EXPORT KeyboardCode DomCodeToUsLayoutNonLocatedKeyboardCode(DomCode dom_code);
