...
const struct {
    ...
    // Control characters
    { XK_BackSpace, 0x0008 }, { XK_Tab, 0x0009 }, { XK_Linefeed, 0x000a }, { XK_Clear, 0x000b }, { XK_Return, 0x000d }, { XK_Escape, 0x001b },
    { XK_Delete, 0x007f },

    // Numeric keypad
    { XK_KP_Space, 0x0020 }, { XK_KP_Tab, 0x0009 }, { XK_KP_Enter, 0x000d }, { XK_KP_Equal, 0x003d }, { XK_KP_Multiply, 0x002a }, { XK_KP_Add, 0x002b },
    { XK_KP_Separator, 0x002c }, { XK_KP_Subtract, 0x002d }, { XK_KP_Decimal, 0x002e }, { XK_KP_Divide, 0x002f }, { XK_KP_0, 0x0030 }, { XK_KP_1, 0x0031 },
    { XK_KP_2, 0x0032 }, { XK_KP_3, 0x0033 }, { XK_KP_4, 0x0034 }, { XK_KP_5, 0x0035 }, { XK_KP_6, 0x0036 }, { XK_KP_7, 0x0037 }, { XK_KP_8, 0x0038 },
    { XK_KP_9, 0x0039 },
...
public:
        ...
        : keysym_to_unicode_map_(std::size(g_keysym_to_unicode_table))
    {
        for (size_t i = 0; i < std::size(g_keysym_to_unicode_table); ++i) {
            keysym_to_unicode_map_[g_keysym_to_unicode_table[i].keysym] = g_keysym_to_unicode_table[i].unicode;
        }
    }
    ...
    uint16_t UnicodeFromKeySym(uint32_t keysym) const
    {
        ...
        if ((0x0020 <= keysym && keysym <= 0x007e) || (0x00a0 <= keysym && keysym <= 0x00ff))
...
static base::LazyInstance<KeySymToUnicode>::Leaky g_keysym_to_unicode = LAZY_INSTANCE_INITIALIZER;

uint16_t GetUnicodeCharacterFromXKeySym(unsigned long keysym)
{
    return g_keysym_to_unicode.Get().UnicodeFromKeySym(static_cast<uint32_t>(keysym));
}
