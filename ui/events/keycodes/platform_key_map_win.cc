...
void SetModifierState(BYTE* keyboard_state, int flags)
{
    ...
    DCHECK_EQ(flags & ~(EF_SHIFT_DOWN | EF_CONTROL_DOWN | EF_ALT_DOWN | EF_CAPS_LOCK_ON), 0);
}
...
{
    return (flags & EF_ALTGR_DOWN) ? ((flags & ~EF_ALTGR_DOWN) | kControlAndAltFlags) : flags;
}
...
DomKey LanguageSpecificOemKeyboardCodeToDomKey(KeyboardCode key_code, HKL layout)
{
...
DomKey NonPrintableKeyboardCodeToDomKey(KeyboardCode key_code, HKL layout)
{
    ...
    const NonPrintableKeyEntry* result = std::lower_bound(std::begin(kNonPrintableKeyMap), std::end(kNonPrintableKeyMap), key_code,
        [](const NonPrintableKeyEntry& entry, KeyboardCode needle) { return entry.key_code < needle; });
...
PlatformKeyMap* PlatformKeyMap::GetThreadLocalPlatformKeyMap()
{
    ...
    // in the same process.
    static base::LazyInstance<base::ThreadLocalOwnedPointer<PlatformKeyMap>>::DestructorAtExit platform_key_map_tls_instance = LAZY_INSTANCE_INITIALIZER;
...
DomKey PlatformKeyMap::DomKeyFromKeyboardCodeImpl(KeyboardCode key_code, int* flags) const
{
    ...
    const int flags_to_try[] = {
        ...
        lookup_flags,
        lookup_flags & (EF_SHIFT_DOWN | EF_CAPS_LOCK_ON),
        EF_NONE,
    };

    for (auto try_flags : flags_to_try) {
        const auto& it = printable_keycode_to_key_.find(std::make_pair(static_cast<int>(key_code), try_flags));
...
// static
DomKey PlatformKeyMap::DomKeyFromKeyboardCode(KeyboardCode key_code, int* flags)
{
...
void PlatformKeyMap::UpdateLayout(HKL layout)
{
    ...
    for (int modifier_combination = 0; modifier_combination <= kModifierFlagsCombinations; ++modifier_combination) {
        ...
        for (int key_code = 0; key_code <= 0xFF; ++key_code) {
            ...
            int rv = ::ToUnicodeEx(key_code, 0, keyboard_state, translated_chars, std::size(translated_chars), 0, keyboard_layout_);

            if (rv == -1) {
                // Dead key, injecting VK_SPACE to get character representation.
                BYTE empty_state[256];
                memset(empty_state, 0, sizeof(empty_state));
                rv = ::ToUnicodeEx(VK_SPACE, 0, empty_state, translated_chars, std::size(translated_chars), 0, keyboard_layout_);
                // Expecting a dead key character (not followed by a space).
                if (rv == 1) {
                    printable_keycode_to_key_[std::make_pair(static_cast<int>(key_code), flags)] = DomKey::DeadKeyFromCombiningCharacter(translated_chars[0]);
                } else {
                    // TODO(input-dev): Check if this will actually happen.
                }
            } else if (rv == 1) {
                if (translated_chars[0] >= 0x20) {
                    printable_keycode_to_key_[std::make_pair(static_cast<int>(key_code), flags)] = DomKey::FromCharacter(translated_chars[0]);
