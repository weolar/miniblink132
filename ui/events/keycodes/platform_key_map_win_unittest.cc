...
public:
    ...
    void CheckKeyboardCodeToKeyString(const char* label, const PlatformKeyMap& keymap, const TestKey& test_case)
    {
        KeyboardCode key_code = test_case.key_code;
        EXPECT_STREQ(test_case.normal, KeycodeConverter::DomKeyToKeyString(DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_NONE)).c_str()) << label;
        EXPECT_STREQ(test_case.shift, KeycodeConverter::DomKeyToKeyString(DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_SHIFT_DOWN)).c_str()) << label;
        EXPECT_STREQ(test_case.capslock, KeycodeConverter::DomKeyToKeyString(DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_CAPS_LOCK_ON)).c_str()) << label;
        EXPECT_STREQ(test_case.altgr, KeycodeConverter::DomKeyToKeyString(DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_ALTGR_DOWN)).c_str()) << label;
        EXPECT_STREQ(test_case.shift_capslock,
            KeycodeConverter::DomKeyToKeyString(DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_SHIFT_DOWN | EF_CAPS_LOCK_ON)).c_str())
            << label;
        EXPECT_STREQ(
            test_case.shift_altgr, KeycodeConverter::DomKeyToKeyString(DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_SHIFT_DOWN | EF_ALTGR_DOWN)).c_str())
            << label;
        EXPECT_STREQ(test_case.altgr_capslock,
            KeycodeConverter::DomKeyToKeyString(DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_ALTGR_DOWN | EF_CAPS_LOCK_ON)).c_str())
            << label;
    }

    // Need this helper function to access private methods of |PlatformKeyMap|.
    DomKey DomKeyFromKeyboardCodeImpl(const PlatformKeyMap& keymap, KeyboardCode key_code, int flags)
    {
    ...
    DomKeyAndFlags DomKeyAndFlagsFromKeyboardCode(const PlatformKeyMap& keymap, KeyboardCode key_code, int flags)
    {
...
TEST_F(PlatformKeyMapTest, NumPad)
{
    ...
    for (const auto& test_case : kNumPadTestCases) {
        ...
        EXPECT_EQ(test_case.key, DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_NONE)) << key_code;
        EXPECT_EQ(test_case.key, DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_ALTGR_DOWN)) << key_code;
        EXPECT_EQ(test_case.key, DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_CONTROL_DOWN)) << key_code;
        EXPECT_EQ(test_case.key, DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_ALTGR_DOWN | EF_CONTROL_DOWN)) << key_code;
    }
}
...
TEST_F(PlatformKeyMapTest, NonPrintableKey)
{
    ...
    for (const auto& test_case : kNonPrintableCodeMap) {
        ...
        if (test_case.dom_code == DomCode::PAUSE || test_case.dom_code == DomCode::LANG2 || test_case.dom_code == DomCode::NON_CONVERT)
            continue;

        int scan_code = ui::KeycodeConverter::DomCodeToNativeKeycode(test_case.dom_code);
        // TODO(input-dev): Some |scan_code| should map to different |key_code|
        // based on modifiers.
        KeyboardCode key_code = static_cast<KeyboardCode>(::MapVirtualKeyEx(scan_code, MAPVK_VSC_TO_VK, layout));

        if (key_code == VKEY_UNKNOWN)
            continue;

        EXPECT_EQ(test_case.dom_key, DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_NONE)) << key_code << ", " << scan_code;
        EXPECT_EQ(test_case.dom_key, DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_ALTGR_DOWN)) << key_code << ", " << scan_code;
        EXPECT_EQ(test_case.dom_key, DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_CONTROL_DOWN)) << key_code << ", " << scan_code;
        EXPECT_EQ(test_case.dom_key, DomKeyFromKeyboardCodeImpl(keymap, key_code, EF_ALTGR_DOWN | EF_CONTROL_DOWN)) << key_code << ", " << scan_code;
    }
}
...
TEST_F(PlatformKeyMapTest, KoreanSpecificKeys)
{
    ...
    PlatformKeyMap us_keymap(GetPlatformKeyboardLayout(KEYBOARD_LAYOUT_ENGLISH_US));
    PlatformKeyMap kr_keymap(GetPlatformKeyboardLayout(KEYBOARD_LAYOUT_KOREAN));
    for (const auto& test_case : kKoreanTestCases) {
        EXPECT_EQ(test_case.us_key, DomKeyFromKeyboardCodeImpl(us_keymap, test_case.key_code, EF_NONE)) << test_case.key_code;
        EXPECT_EQ(test_case.kr_key, DomKeyFromKeyboardCodeImpl(kr_keymap, test_case.key_code, EF_NONE)) << test_case.key_code;
    }
}
...
TEST_F(PlatformKeyMapTest, JapaneseSpecificKeys)
{
    ...
    for (const auto& test_case : kJapaneseTestCases) {
        EXPECT_EQ(test_case.us_key, DomKeyFromKeyboardCodeImpl(us_keymap, test_case.key_code, EF_NONE)) << test_case.key_code;
        EXPECT_EQ(test_case.jp_key, DomKeyFromKeyboardCodeImpl(jp_keymap, test_case.key_code, EF_NONE)) << test_case.key_code;
    }
}
...
{
    PlatformKeyMap us_keymap(GetPlatformKeyboardLayout(KEYBOARD_LAYOUT_ENGLISH_US));
    EXPECT_EQ(DomKey::ALT, DomKeyFromKeyboardCodeImpl(us_keymap, VKEY_MENU, EF_ALTGR_DOWN));
    EXPECT_EQ(DomKey::ALT, DomKeyFromKeyboardCodeImpl(us_keymap, VKEY_MENU, EF_ALTGR_DOWN | EF_IS_EXTENDED_KEY));

    PlatformKeyMap fr_keymap(GetPlatformKeyboardLayout(KEYBOARD_LAYOUT_FRENCH));
    EXPECT_EQ(DomKey::ALT, DomKeyFromKeyboardCodeImpl(fr_keymap, VKEY_MENU, EF_ALTGR_DOWN));
    EXPECT_EQ(DomKey::ALT_GRAPH, DomKeyFromKeyboardCodeImpl(fr_keymap, VKEY_MENU, EF_ALTGR_DOWN | EF_IS_EXTENDED_KEY));
}
...
class AltGraphModifierTest : public PlatformKeyMapTest, public testing::WithParamInterface<KeyboardLayout> {
public:
    AltGraphModifierTest()
        : keymap_(GetPlatformKeyboardLayout(GetParam()))
    {
    }
...
TEST_P(AltGraphModifierTest, AltGraphModifierBehaviour)
{
    ...
    for (const auto& test_case : kAltGraphModifierTestCases) {
        DomKeyAndFlags result = DomKeyAndFlagsFromKeyboardCode(keymap_, test_case.key_code, test_case.flags);
        if (GetParam() == KEYBOARD_LAYOUT_FRENCH && test_case.expect_alt_graph) {
            EXPECT_EQ(EF_ALTGR_DOWN, result.flags) << " for key_code=" << test_case.key_code << " flags=" << test_case.flags;
        } else {
            EXPECT_EQ(test_case.flags, result.flags) << " for key_code=" << test_case.key_code << " flags=" << test_case.flags;
        }
    }
}

INSTANTIATE_TEST_SUITE_P(VerifyAltGraph, AltGraphModifierTest, ::testing::Values(KEYBOARD_LAYOUT_ENGLISH_US, KEYBOARD_LAYOUT_FRENCH));
