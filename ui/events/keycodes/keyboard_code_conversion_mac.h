...
EVENTS_BASE_EXPORT int MacKeyCodeForWindowsKeyCode(KeyboardCode keycode, NSUInteger flags, unichar* us_keyboard_shifted_character, unichar* keyboard_character);

// Returns the WindowsKeyCode from the Mac key code.
EVENTS_BASE_EXPORT KeyboardCode KeyboardCodeFromKeyCode(unsigned short key_code);
...
EVENTS_BASE_EXPORT UniChar TranslatedUnicodeCharFromKeyCode(
    TISInputSourceRef input_source, UInt16 key_code, UInt16 key_action, UInt32 modifier_key_state, UInt32 keyboard_type, UInt32* dead_key_state);
