...
public:
    ...
    ~TestEventRewriteSink() override
    {
        CheckAllReceived();
    }

    void AddExpectedEvent(EventType type)
    {
        expected_events_.push_back(type);
    }
...
std::unique_ptr<Event> CreateEventForType(EventType type)
{
    ...
    case EventType::kMouseReleased:
        return std::make_unique<MouseEvent>(type, gfx::Point(), gfx::Point(), base::TimeTicks::Now(), 0, 0);
    ...
    case EventType::kScrollFlingStart:
        return std::make_unique<ScrollEvent>(type, gfx::Point(), base::TimeTicks::Now(), 0, 0, 0, 0, 0, 0);
    default:
...
public:
    ...
    EventRewriteStatus RewriteEvent(const Event& event, std::unique_ptr<Event>* rewritten_event) override
    {
        if (status_ == EVENT_REWRITE_REWRITTEN)
            *rewritten_event = CreateEventForType(type_);
        return status_;
    }
    EventRewriteStatus NextDispatchEvent(const Event& last_event, std::unique_ptr<Event>* new_event) override
    {
        NOTREACHED();
    }
...
public:
    ...
    void AddRule(int from_state, EventType from_type, int to_state, EventType to_type, EventRewriteStatus to_status)
    {
    ...
    EventRewriteStatus RewriteEvent(const Event& event, std::unique_ptr<Event>* rewritten_event) override
    {
        ...
        if ((find->second.status == EVENT_REWRITE_REWRITTEN) || (find->second.status == EVENT_REWRITE_DISPATCH_ANOTHER)) {
...
public:
    explicit TestConstantEventRewriter(EventType type)
        : type_(type)
    {
    }
    EventDispatchDetails RewriteEvent(const Event& event, const Continuation continuation) override
...
public:
    ...
    TestStateMachineEventRewriter()
        : state_(0)
    {
    }
    void AddRule(int from_state, EventType from_type, int to_state, EventType to_type, RewriteAction rewrite_action, StateAction state_action)
    {
    ...
    {
        for (;;) {
            RewriteRules::iterator find = rules_.find(RewriteCase(state_, event.type()));
            if (find == rules_.end())
                return SendEvent(continuation, &event);
            state_ = find->second.state;
...
TEST(EventRewriterTest, EventRewritingOld)
{
    ...
    TestConstantEventRewriterOld r0(EVENT_REWRITE_REWRITTEN, EventType::kCancelMode);
    ...
    TestConstantEventRewriterOld r3(EVENT_REWRITE_REWRITTEN, EventType::kCancelMode);
    ...
    // clang-format on
    r2.AddRule(0, EventType::kScrollFlingStart, 0, EventType::kScrollFlingCancel, EVENT_REWRITE_REWRITTEN);
    // clang-format on
...
TEST(EventRewriterTest, EventRewriting)
{
    ...
    s.RemoveEventRewriter(&r0);
    r2.AddRule(
        0, EventType::kScrollFlingStart, 0, EventType::kScrollFlingCancel, TestStateMachineEventRewriter::REPLACE, TestStateMachineEventRewriter::RETURN);
    p.AddExpectedEvent(EventType::kScrollFlingCancel);
    s.Send(EventType::kScrollFlingStart);
    p.CheckAllReceived();
    s.RemoveEventRewriter(&r3);
    ...
    // (that happens to be analogous to sticky keys).
    r2.AddRule(0, EventType::kKeyPressed, 1, EventType::kUnknown, TestStateMachineEventRewriter::ACCEPT, TestStateMachineEventRewriter::RETURN);
    r2.AddRule(1, EventType::kMousePressed, 0, EventType::kUnknown, TestStateMachineEventRewriter::ACCEPT, TestStateMachineEventRewriter::RETURN);
    r2.AddRule(1, EventType::kKeyReleased, 2, EventType::kUnknown, TestStateMachineEventRewriter::DISCARD, TestStateMachineEventRewriter::RETURN);
    r2.AddRule(2, EventType::kMouseReleased, 3, EventType::kMouseReleased, TestStateMachineEventRewriter::REPLACE, TestStateMachineEventRewriter::PROCEED);
    r2.AddRule(3, EventType::kMouseReleased, 0, EventType::kKeyReleased, TestStateMachineEventRewriter::REPLACE, TestStateMachineEventRewriter::RETURN);
    p.AddExpectedEvent(EventType::kKeyPressed);
    s.Send(EventType::kKeyPressed); // state 0 EventType::kKeyPressed -> 1 ACCEPT
        // EventType::kKeyPressed
    s.Send(EventType::kKeyReleased); // state 1 EventType::kKeyReleased -> 2 DISCARD
    p.AddExpectedEvent(EventType::kMousePressed);
