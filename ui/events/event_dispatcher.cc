...
EventDispatchDetails EventDispatcherDelegate::DispatchEvent(EventTarget* target, Event* event)
{
    CHECK(target);
    ...
    if (!event->handled() && !details.dispatcher_destroyed && !details.target_destroyed) {
        details = DispatchEventToTarget(target, event);
    }
    bool target_destroyed_during_dispatch = details.target_destroyed;
    if (!details.dispatcher_destroyed) {
        details = PostDispatchEvent(target_destroyed_during_dispatch ? NULL : target, *event);
    }
...
EventDispatchDetails EventDispatcherDelegate::PreDispatchEvent(EventTarget* target, Event* event)
{
    return EventDispatchDetails();
}

EventDispatchDetails EventDispatcherDelegate::PostDispatchEvent(EventTarget* target, const Event& event)
{
    return EventDispatchDetails();
}

EventDispatchDetails EventDispatcherDelegate::DispatchEventToTarget(EventTarget* target, Event* event)
{
    ...
    details.target_destroyed = (!details.dispatcher_destroyed && !CanDispatchToTarget(target));
    return details;
}
...
void EventDispatcher::ProcessEvent(EventTarget* target, Event* event)
{
    ...
    // abstraction.
    if (delegate_ && delegate_->CanDispatchToTarget(target) && target->target_handler()) {
...
void EventDispatcher::DispatchEventToEventHandlers(EventHandlerList* list, Event* event)
{
