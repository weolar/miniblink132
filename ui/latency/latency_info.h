...
public:
    ...
    static bool Verify(const std::vector<LatencyInfo>& latency_info, const char* referring_msg);
    ...
    void AddLatencyNumberWithTraceName(LatencyComponentType component, const char* trace_name_str, base::TimeTicks now);

    // Modifies the current sequence number and adds a certain number of events
    // for a specific component.
    void AddLatencyNumberWithTimestamp(LatencyComponentType component, base::TimeTicks time);
    ...
    const LatencyMap& latency_components() const
    {
        return latency_components_;
    }
    ...
    void set_gesture_scroll_id(int64_t id)
    {
        gesture_scroll_id_ = id;
    }
    int64_t touch_trace_id() const
    {
        return touch_trace_id_;
    }
    void set_touch_trace_id(int64_t id)
    {
        touch_trace_id_ = id;
    }

private:
    void AddLatencyNumberWithTimestampImpl(LatencyComponentType component, base::TimeTicks time, const char* trace_name_str);
    ...
    friend struct mojo::StructTraits<ui::mojom::LatencyInfoDataView, ui::LatencyInfo>;
#endif
};
