...
{
    task_runner_
        = base::ThreadPool::CreateSequencedTaskRunner({ base::MayBlock(), base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN, base::TaskPriority::BEST_EFFORT });
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&JankyDurationTracker::InitializeOnThreadPool, base::Unretained(this)));
}
...
void JankyDurationTracker::InitializeOnThreadPool()
{
    ...
    file_path_ = command_line->GetSwitchValuePath(switches::kWatchDirForScrollJankReport);
    if (file_path_.empty()) {
        return;
    }
    watcher_ = std::make_unique<base::FilePathWatcher>();
    if (!watcher_->Watch(
            file_path_, base::FilePathWatcher::Type::kNonRecursive, base::BindRepeating(&JankyDurationTracker::OnChanged, base::Unretained(this)))) {
...
void JankyDurationTracker::ReportTotalsAndReset()
{
    ...
    uint64_t non_janky_total = non_janky_total_.exchange(0, std::memory_order_relaxed);
    VLOG(0) << "JankyDurationTrackerCSV:" << janky_total << "," << non_janky_total << "," << std::setprecision(2)
