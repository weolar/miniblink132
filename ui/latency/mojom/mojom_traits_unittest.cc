...
private:
    // TraitsTestService:
    void EchoLatencyInfo(const LatencyInfo& info, EchoLatencyInfoCallback callback) override
    {
...
TEST_F(StructTraitsTest, LatencyInfo)
{
    ...
    EXPECT_TRUE(output.FindLatency(INPUT_EVENT_LATENCY_ORIGINAL_COMPONENT, nullptr));
}
