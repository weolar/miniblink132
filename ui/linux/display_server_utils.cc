...
bool InspectWaylandDisplay(base::Environment& env)
{
    ...
    const bool has_wayland_display = env.GetVar("WAYLAND_DISPLAY", &wayland_display) && !wayland_display.empty();
    if (has_wayland_display) {
        return true;
    }

    std::string xdg_runtime_dir;
    const bool has_xdg_runtime_dir = env.GetVar("XDG_RUNTIME_DIR", &xdg_runtime_dir) && !xdg_runtime_dir.empty();
    if (has_xdg_runtime_dir) {
        constexpr char kDefaultWaylandSocketName[] = "wayland-0";
        const auto wayland_socket_path = base::FilePath(xdg_runtime_dir).Append(kDefaultWaylandSocketName);
...
std::string MaybeFixPlatformName(const std::string& platform_hint)
{
    ...
    if (platform_hint == kPlatformWayland || platform_hint == "auto") {
        ...
        const bool has_xdg_session_type = env->GetVar(base::nix::kXdgSessionTypeEnvVar, &xdg_session_type) && !xdg_session_type.empty();

        if ((has_xdg_session_type && xdg_session_type == "wayland") || (platform_hint == kPlatformWayland && HasWaylandDisplay(*env))) {
...
{
#if BUILDFLAG(IS_OZONE_WAYLAND)
    const auto ozone_platform_hint = command_line.GetSwitchValueASCII(switches::kOzonePlatformHint);
    if (!ozone_platform_hint.empty() || !base::FeatureList::IsEnabled(features::kOverrideDefaultOzonePlatformHintToAuto)) {
        return;
    }
    ...
    if (success && system_info.gpus[system_info.activeGPUIndex].vendorId != angle::kVendorID_NVIDIA) {
