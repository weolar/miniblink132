...
{
    static base::NoDestructor<std::vector<raw_ptr<LinuxUiTheme, VectorExperimental>>> themes;
    return *themes;
}
...
LinuxUiAndTheme* GetDefaultLinuxUiAndTheme()
{
    ...
    std::string ui_toolkit = base::ToLowerASCII(cmd_line->GetSwitchValueASCII(switches::kUiToolkitFlag));
    if (ui_toolkit == "gtk") {
...
const std::vector<raw_ptr<LinuxUiTheme, VectorExperimental>>& GetLinuxUiThemes()
{
