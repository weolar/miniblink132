...
void OSExchangeData::SetFilenames(const std::vector<FileInfo>& filenames)
{
    provider_->SetFilenames(filenames);
}

void OSExchangeData::SetPickledData(const ClipboardFormatType& format, const base::Pickle& data)
{
...
std::optional<OSExchangeData::UrlInfo> OSExchangeData::GetURLAndTitle(FilenameToURLPolicy policy) const
{
    return provider_->GetURLAndTitle(policy);
}

std::optional<std::vector<GURL>> OSExchangeData::GetURLs(FilenameToURLPolicy policy) const
{
...
std::optional<base::Pickle> OSExchangeData::GetPickledData(const ClipboardFormatType& format) const
{
...
bool OSExchangeData::HasAnyFormat(int formats, const std::set<ClipboardFormatType>& format_types) const
{
...
void OSExchangeData::SetFileContents(const base::FilePath& filename, const std::string& file_contents)
{
    provider_->SetFileContents(filename, file_contents);
}

std::optional<OSExchangeData::FileContentsInfo> OSExchangeData::GetFileContents() const
{
...
std::optional<std::vector<FileInfo>> OSExchangeData::GetVirtualFilenames() const
{
    return provider_->GetVirtualFilenames();
}

void OSExchangeData::GetVirtualFilesAsTempFiles(base::OnceCallback<void(const std::vector<std::pair<base::FilePath, base::FilePath>>&)> callback) const
{
...
void OSExchangeData::SetSource(std::unique_ptr<DataTransferEndpoint> data_source)
{
