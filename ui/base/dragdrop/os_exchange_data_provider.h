...
public:
    ...
    virtual void SetPickledData(const ClipboardFormatType& format, const base::Pickle& data) = 0;
    ...
    virtual std::optional<UrlInfo> GetURLAndTitle(FilenameToURLPolicy policy) const = 0;
    virtual std::optional<std::vector<GURL>> GetURLs(FilenameToURLPolicy policy) const = 0;
    virtual std::optional<std::vector<FileInfo>> GetFilenames() const = 0;
    virtual std::optional<base::Pickle> GetPickledData(const ClipboardFormatType& format) const = 0;
    ...
    virtual void SetFileContents(const base::FilePath& filename, const std::string& file_contents) = 0;
    ...
    virtual void GetVirtualFilesAsTempFiles(base::OnceCallback<void(const std::vector<std::pair</*temp path*/ base::FilePath,
                /*display name*/ base::FilePath>>&)>
            callback) const
        = 0;
    virtual void SetVirtualFileContentsForTesting(const std::vector<std::pair<base::FilePath, std::string>>& filenames_and_contents, DWORD tymed) = 0;
...
#if defined(USE_AURA) || BUILDFLAG(IS_APPLE)
    virtual void SetDragImage(const gfx::ImageSkia& image, const gfx::Vector2d& cursor_offset) = 0;
