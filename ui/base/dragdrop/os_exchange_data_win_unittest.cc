...
public:
    ...
    MOCK_METHOD3_WITH_CALLTYPE(STDMETHODCALLTYPE, Read, HRESULT(void* pv, ULONG cb, ULONG* pcbRead));

    MOCK_METHOD3_WITH_CALLTYPE(STDMETHODCALLTYPE, Write, HRESULT(void const* pv, ULONG cb, ULONG* pcbW));

    // Overridden from IStream:
    MOCK_METHOD1_WITH_CALLTYPE(STDMETHODCALLTYPE, SetSize, HRESULT(ULARGE_INTEGER));

    MOCK_METHOD4_WITH_CALLTYPE(STDMETHODCALLTYPE, CopyTo, HRESULT(IStream*, ULARGE_INTEGER, ULARGE_INTEGER*, ULARGE_INTEGER*));
    ...
    MOCK_METHOD3_WITH_CALLTYPE(STDMETHODCALLTYPE, LockRegion, HRESULT(ULARGE_INTEGER, ULARGE_INTEGER, DWORD));

    MOCK_METHOD3_WITH_CALLTYPE(STDMETHODCALLTYPE, UnlockRegion, HRESULT(ULARGE_INTEGER, ULARGE_INTEGER, DWORD));

    MOCK_METHOD1_WITH_CALLTYPE(STDMETHODCALLTYPE, Clone, HRESULT(IStream**));

    MOCK_METHOD3_WITH_CALLTYPE(STDMETHODCALLTYPE, Seek, HRESULT(LARGE_INTEGER liDistanceToMove, DWORD dwOrigin, ULARGE_INTEGER* lpNewFilePointer));
...
public:
    ...
    void OnGotVirtualFilesAsTempFiles(const std::vector<std::pair<base::FilePath, base::FilePath>>& filepaths_and_names)
    {
        ...
        for (const auto& filepath_and_name : filepaths_and_names) {
            retrieved_virtual_files_.push_back(FileInfo(filepath_and_name.first, filepath_and_name.second));
        }
    }
...
TEST_F(OSExchangeDataWinTest, StringDataAccessViaCOM)
{
    ...
    Microsoft::WRL::ComPtr<IDataObject> com_data(OSExchangeDataProviderWin::GetIDataObject(data));
    ...
    std::wstring output = base::win::ScopedHGlobal<wchar_t*>(medium.hGlobal).data();
...
TEST_F(OSExchangeDataWinTest, StringDataWritingViaCOM)
{
    ...
    std::optional<OSExchangeData::UrlInfo> url_info = data2.GetURLAndTitle(FilenameToURLPolicy::CONVERT_FILENAMES);
...
TEST_F(OSExchangeDataWinTest, MultipleFormatsViaCOM)
{
    ...
    std::wstring output_url = base::win::ScopedHGlobal<wchar_t*>(medium.hGlobal).data();
    ...
    std::wstring output_text = base::win::ScopedHGlobal<wchar_t*>(medium.hGlobal).data();
...
TEST_F(OSExchangeDataWinTest, TestURLExchangeFormatsViaCOM)
{
    ...
    Microsoft::WRL::ComPtr<IDataObject> com_data(OSExchangeDataProviderWin::GetIDataObject(data));
    {
        ...
        FORMATETC format_etc = { cfstr_file_contents, nullptr, DVASPECT_CONTENT, 0, TYMED_HGLOBAL };
        EXPECT_EQ(S_OK, com_data->QueryGetData(&format_etc));
...
TEST_F(OSExchangeDataWinTest, FileContents)
{
    ...
    std::optional<OSExchangeData::FileContentsInfo> file_contents_info = copy.GetFileContents();
...
TEST_F(OSExchangeDataWinTest, VirtualFiles)
{
    ...
    const std::vector<std::pair<base::FilePath, std::string>> kTestFilenamesAndContents = {
        { base::FilePath(FILE_PATH_LITERAL("filename.txt")), std::string("just some data") },
        { base::FilePath(FILE_PATH_LITERAL("another filename.txt")), std::string("just some data\0with\0nulls", 25) },
        { base::FilePath(FILE_PATH_LITERAL("and another filename.txt")), std::string("just some more data") },
    };

    for (const auto& tymed : kStorageMediaTypesForVirtualFiles) {
        OSExchangeData data;
        data.provider().SetVirtualFileContentsForTesting(kTestFilenamesAndContents, tymed);

        OSExchangeData copy(data.provider().Clone());
        std::optional<std::vector<FileInfo>> file_infos = copy.GetVirtualFilenames();
        ...
        for (size_t i = 0; i < file_infos.value().size(); i++) {
            EXPECT_EQ(kTestFilenamesAndContents[i].first, file_infos.value()[i].display_name);
        ...
        auto callback = base::BindOnce(&OSExchangeDataWinTest::OnGotVirtualFilesAsTempFiles, base::Unretained(this));
        ...
        EXPECT_EQ(kTestFilenamesAndContents.size(), retrieved_virtual_files_.size());
        for (size_t i = 0; i < retrieved_virtual_files_.size(); i++) {
            EXPECT_EQ(kTestFilenamesAndContents[i].first, retrieved_virtual_files_[i].display_name);
            ...
            EXPECT_EQ(base::MakeLongFilePath(temp_dir), base::MakeLongFilePath(retrieved_virtual_files_[i].path.DirName()));
            EXPECT_EQ(kTestFilenamesAndContents[i].first.Extension(), retrieved_virtual_files_[i].path.Extension());
            std::string read_contents;
            EXPECT_TRUE(base::ReadFileToString(retrieved_virtual_files_[i].path, &read_contents));
            if (tymed != TYMED_ISTORAGE) {
                ...
                EXPECT_TRUE(base::Contains(read_contents, kTestFilenamesAndContents[i].second));
            }
        }
    }
}
...
TEST_F(OSExchangeDataWinTest, VirtualFilesRealFilesPreferred)
{
    ...
    const std::vector<FileInfo> kTestFilenames = {
        { base::FilePath(FILE_PATH_LITERAL("C:\\tmp\\test_file1")), base::FilePath() },
        { base::FilePath(FILE_PATH_LITERAL("C:\\tmp\\test_file2")), base::FilePath() },
    };
...
{
    const std::vector<std::pair<base::FilePath, std::string>> kTestFilenamesAndContents = {
        { base::FilePath(FILE_PATH_LITERAL("A (1) (2).txt")), std::string("just some data") },
        { base::FilePath(FILE_PATH_LITERAL("A.txt")), std::string("just some more data") },
        { base::FilePath(FILE_PATH_LITERAL("A (1).txt")), std::string("just some more more data") },
        { base::FilePath(FILE_PATH_LITERAL("A.txt")), std::string("just some more more more data") },
    };
    ...
    for (const auto& tymed : kStorageMediaTypesForVirtualFiles) {
        ...
        for (size_t i = 0; i < file_infos.value().size(); i++) {
            ...
            for (size_t j = 0; j < i; j++) {
                EXPECT_FALSE(base::FilePath::CompareEqualIgnoreCase(file_infos.value()[j].display_name.value(), file_infos.value()[i].display_name.value()));
            }
        }
        ...
        for (size_t i = 0; i < retrieved_virtual_files_.size(); i++) {
            ...
            for (size_t j = 0; j < i; j++) {
                EXPECT_FALSE(
                    base::FilePath::CompareEqualIgnoreCase(retrieved_virtual_files_[j].display_name.value(), retrieved_virtual_files_[i].display_name.value()));
            }

            // Check that temp file path is unique.
            for (size_t j = 0; j < i; j++) {
                EXPECT_FALSE(base::FilePath::CompareEqualIgnoreCase(retrieved_virtual_files_[j].path.value(), retrieved_virtual_files_[i].path.value()));
            }
...
{
    const std::vector<std::pair<base::FilePath, std::string>> kTestFilenamesAndContents = {
        { base::FilePath(FILE_PATH_LITERAL("a.txt")), std::string("just some data") },
        { base::FilePath(FILE_PATH_LITERAL("B.txt")), std::string("just some more data") },
        { base::FilePath(FILE_PATH_LITERAL("A.txt")), std::string("just some more more data") },
    };
...
{
    const std::wstring kInvalidFileNameCharacters(FILE_PATH_LITERAL("\\/:*?\"<>|"));
    const std::wstring kInvalidFilePathCharacters(FILE_PATH_LITERAL("/*?\"<>|"));
    const base::FilePath kPathWithInvalidFileNameCharacters = base::FilePath(kInvalidFileNameCharacters).AddExtension(FILE_PATH_LITERAL("txt"));
    const base::FilePath kEmptyDisplayName(FILE_PATH_LITERAL(""));
    const base::FilePath kMaxPathDisplayName = base::FilePath(std::wstring(MAX_PATH - 5, L'a')).AddExtension(FILE_PATH_LITERAL("txt"));

    const std::vector<std::pair<base::FilePath, std::string>> kTestFilenamesAndContents = {
        { kPathWithInvalidFileNameCharacters, std::string("just some data") },
        { kPathWithInvalidFileNameCharacters, std::string("just some data\0with\0nulls", 25) },
        { // Test that still get a unique name if a previous uniquified
            // name is a duplicate of this one.
            kPathWithInvalidFileNameCharacters.InsertBeforeExtension(FILE_PATH_LITERAL(" (1)")), std::string("just some more data") },
        // Expect a default display name to be generated ("download" if it
        // matters).
        { kEmptyDisplayName, std::string("data for an empty display name") },
        { kEmptyDisplayName, std::string("data for another empty display name") },
        // Expect good behavior if the display name length exceeds MAX_PATH.
        { kMaxPathDisplayName, std::string("data for a >MAX_PATH display name") },
        { kMaxPathDisplayName, std::string("data for another >MAX_PATH display name") },
    };
    ...
    for (const auto& tymed : kStorageMediaTypesForVirtualFiles) {
        ...
        for (size_t i = 0; i < file_infos.value().size(); i++) {
            ...
            EXPECT_EQ(std::wstring::npos, file_infos.value()[i].display_name.value().find_first_of(kInvalidFileNameCharacters));
            // Check that display name is unique.
            for (size_t j = 0; j < i; j++) {
                EXPECT_FALSE(base::FilePath::CompareEqualIgnoreCase(file_infos.value()[j].display_name.value(), file_infos.value()[i].display_name.value()));
            }
        ...
        for (size_t i = 0; i < retrieved_virtual_files_.size(); i++) {
            // Check that display name does not contain invalid characters.
            EXPECT_EQ(std::wstring::npos, retrieved_virtual_files_[i].display_name.value().find_first_of(kInvalidFileNameCharacters));
            // Check that display name is unique.
            for (size_t j = 0; j < i; j++) {
                EXPECT_FALSE(
                    base::FilePath::CompareEqualIgnoreCase(retrieved_virtual_files_[j].display_name.value(), retrieved_virtual_files_[i].display_name.value()));
            ...
            // for separator).
            EXPECT_EQ(std::wstring::npos, retrieved_virtual_files_[i].path.value().find_first_of(kInvalidFilePathCharacters));
            // Check that temp file path is unique.
            for (size_t j = 0; j < i; j++) {
                EXPECT_FALSE(base::FilePath::CompareEqualIgnoreCase(retrieved_virtual_files_[j].path.value(), retrieved_virtual_files_[i].path.value()));
            }
...
{
    const std::vector<std::pair<base::FilePath, std::string>> kTestFilenamesAndContents = {
        { base::FilePath(FILE_PATH_LITERAL("file_with_no_contents.txt")), std::string() },
    };
...
TEST_F(OSExchangeDataWinTest, CFHtml)
{
    ...
    std::string expected_cf_html("Version:0.9\r\nStartHTML:0000000139\r\nEndHTML:0000000288\r\n"
...
TEST_F(OSExchangeDataWinTest, OnDownloadCompleted)
{
    ...
    DownloadFileInfo file_info(base::FilePath(FILE_PATH_LITERAL("file_with_no_contents.txt")), std::move(download_file_provider));
    provider.SetDownloadFileInfo(&file_info);

    OSExchangeDataProviderWin::GetDataObjectImpl(data)->OnDownloadCompleted(base::FilePath());
...
TEST_F(OSExchangeDataWinTest, SetDataRelease)
{
    ...
    ASSERT_EQ(stream.AddRef(), 1u);
    {
        ...
        EXPECT_EQ(S_OK, data_object->SetData(&format_etc, &medium, /* fRelease= */ TRUE));
...
TEST_F(OSExchangeDataWinTest, SetDataNoRelease)
{
    ...
    ASSERT_EQ(stream.GetRefCount(), 0u);
    {
        ...
        EXPECT_EQ(S_OK,
            data_object->SetData(&format_etc, &medium,
                /* fRelease= */ FALSE));
        ASSERT_EQ(stream.GetRefCount(), 1u);
    }
