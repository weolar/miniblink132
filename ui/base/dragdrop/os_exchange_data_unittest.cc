...
TEST_F(OSExchangeDataTest, StringDataGetAndSet)
{
    ...
    std::optional<OSExchangeData::UrlInfo> url_info = copy.GetURLAndTitle(FilenameToURLPolicy::DO_NOT_CONVERT_FILENAMES);
...
TEST_F(OSExchangeDataTest, URLStringFileContents)
{
    ...
    std::optional<OSExchangeData::FileContentsInfo> file_contents = copy.GetFileContents();
...
TEST_F(OSExchangeDataTest, TestFileToURLConversion)
{
...

    {
        ...
        std::optional<OSExchangeData::UrlInfo> no_converted_filenames_url_info = copy.GetURLAndTitle(FilenameToURLPolicy::DO_NOT_CONVERT_FILENAMES);
        EXPECT_FALSE(no_converted_filenames_url_info.has_value());
    }

    {
        EXPECT_TRUE(copy.HasURL(FilenameToURLPolicy::CONVERT_FILENAMES));
        std::optional<OSExchangeData::UrlInfo> converted_url_info = copy.GetURLAndTitle(FilenameToURLPolicy::CONVERT_FILENAMES);
...
{
    const ClipboardFormatType kTestFormat = ClipboardFormatType::GetType("application/vnd.chromium.test");
    ...
    std::optional<base::Pickle> restored_pickle = copy.GetPickledData(kTestFormat);
...
TEST_F(OSExchangeDataTest, TestHTML)
{
    ...
    const std::u16string html = u"<HTML>\n<BODY>\n"
...
{
    const url::Origin tuple_origin = url::Origin::Create(GURL("https://www.google.com/"));
