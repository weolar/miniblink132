...
const base::FilePath::CharType kFileContentsFileName[] = FILE_PATH_LITERAL("file.jpg");
...
TEST(OSExchangeDataProviderNonBackedTest, CloneTest)
{
    ...
    original.SetPickledData(ClipboardFormatType::PlainTextType(), original_pickle);
    original.SetFileContents(base::FilePath(kFileContentsFileName), std::string(kFileContents));
    ...
    std::optional<OSExchangeDataProvider::UrlInfo> url_info = copy->GetURLAndTitle(FilenameToURLPolicy::DO_NOT_CONVERT_FILENAMES);
    EXPECT_TRUE(url_info.has_value());
    EXPECT_EQ(GURL(kUrl), url_info->url);
    EXPECT_EQ(kUrlTitle, url_info->title);

    std::optional<base::Pickle> copy_pickle = copy->GetPickledData(ClipboardFormatType::PlainTextType());
    ...
    std::optional<OSExchangeDataProvider::FileContentsInfo> copy_file_contents = copy->GetFileContents();
    ASSERT_TRUE(copy_file_contents.has_value());
    EXPECT_EQ(base::FilePath(kFileContentsFileName), copy_file_contents->filename);
    EXPECT_EQ(std::string(kFileContents), copy_file_contents->file_contents);

    std::optional<OSExchangeDataProvider::HtmlInfo> html_content = copy->GetHtml();
