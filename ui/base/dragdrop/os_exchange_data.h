...
public:
    ...
    const OSExchangeDataProvider& provider() const
    {
        return *provider_;
    }
    ...
    void SetFilenames(const std::vector<FileInfo>& file_names);
    // Adds pickled data of the specified format.
    void SetPickledData(const ClipboardFormatType& format, const base::Pickle& data);
    ...
    std::optional<base::Pickle> GetPickledData(const ClipboardFormatType& format) const;
    ...
    bool HasAnyFormat(int formats, const std::set<ClipboardFormatType>& types) const;

    // Adds the bytes of a file (CFSTR_FILECONTENTS and CFSTR_FILEDESCRIPTOR on
    // Windows).
    void SetFileContents(const base::FilePath& filename, const std::string& file_contents);
    ...
    // requested
    void GetVirtualFilesAsTempFiles(base::OnceCallback<void(const std::vector</*temp path*/ std::pair<base::FilePath,
