...
std::unique_ptr<OSExchangeDataProvider> OSExchangeDataProviderNonBacked::Clone() const
{
...
void OSExchangeDataProviderNonBacked::MarkRendererTaintedFromOrigin(const url::Origin& origin)
{
...
std::optional<url::Origin> OSExchangeDataProviderNonBacked::GetRendererTaintedOrigin() const
{
...
void OSExchangeDataProviderNonBacked::SetURL(const GURL& url, const std::u16string& title)
{
    url_ = url;
    title_ = title;
...
void OSExchangeDataProviderNonBacked::SetFilenames(const std::vector<FileInfo>& filenames)
{
    filenames_ = filenames;
    formats_ |= OSExchangeData::FILE_NAME;
}

void OSExchangeDataProviderNonBacked::SetPickledData(const ClipboardFormatType& format, const base::Pickle& data)
{
    pickle_data_[format] = data;
    formats_ |= OSExchangeData::PICKLED_DATA;
}

std::optional<std::u16string> OSExchangeDataProviderNonBacked::GetString() const
{
...
std::optional<OSExchangeDataProvider::UrlInfo> OSExchangeDataProviderNonBacked::GetURLAndTitle(FilenameToURLPolicy policy) const
{
    if ((formats_ & OSExchangeData::URL) == 0) {
        GURL url;
        if (GetPlainTextURL(&url) || (policy == FilenameToURLPolicy::CONVERT_FILENAMES && GetFileURL(&url))) {
            DCHECK(url.is_valid());
...
std::optional<std::vector<GURL>> OSExchangeDataProviderNonBacked::GetURLs(FilenameToURLPolicy policy) const
{
...
std::optional<std::vector<FileInfo>> OSExchangeDataProviderNonBacked::GetFilenames() const
{
...
std::optional<base::Pickle> OSExchangeDataProviderNonBacked::GetPickledData(const ClipboardFormatType& format) const
{
...
bool OSExchangeDataProviderNonBacked::HasURL(FilenameToURLPolicy policy) const
{
    ...
    return GetPlainTextURL(nullptr) || (policy == FilenameToURLPolicy::CONVERT_FILENAMES && GetFileURL(nullptr));
}
...
bool OSExchangeDataProviderNonBacked::HasCustomFormat(const ClipboardFormatType& format) const
{
    return base::Contains(pickle_data_, format);
}

void OSExchangeDataProviderNonBacked::SetFileContents(const base::FilePath& filename, const std::string& file_contents)
{
    file_contents_filename_ = filename;
    file_contents_ = file_contents;
}

std::optional<OSExchangeDataProvider::FileContentsInfo> OSExchangeDataProviderNonBacked::GetFileContents() const
{
    if (file_contents_filename_.empty()) {
        return std::nullopt;
    }
    return FileContentsInfo { .filename = file_contents_filename_, .file_contents = file_contents_ };
}
...
void OSExchangeDataProviderNonBacked::SetHtml(const std::u16string& html, const GURL& base_url)
{
...
std::optional<OSExchangeData::HtmlInfo> OSExchangeDataProviderNonBacked::GetHtml() const
{
    if (!HasHtml()) {
        return std::nullopt;
    }
...
void OSExchangeDataProviderNonBacked::SetDragImage(const gfx::ImageSkia& image, const gfx::Vector2d& cursor_offset)
{
    drag_image_ = image;
...
void OSExchangeDataProviderNonBacked::SetSource(std::unique_ptr<DataTransferEndpoint> data_source)
{
...
void OSExchangeDataProviderNonBacked::CopyData(OSExchangeDataProviderNonBacked* provider) const
{
    DCHECK(provider);
    ...
    provider->source_ = source_ ? std::make_unique<DataTransferEndpoint>(*source_.get()) : nullptr;
