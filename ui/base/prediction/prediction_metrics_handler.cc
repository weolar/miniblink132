...
namespace ui {
namespace {
base::HistogramBase* GetHistogram(std::string_view name, std::string_view suffix)
{
    return base::Histogram::FactoryGet(base::StrCat({ name, ".", suffix }), 1, 1000, 50, base::HistogramBase::kUmaTargetedHistogramFlag);
}
} // namespace

PredictionMetricsHandler::PredictionMetricsHandler(std::string histogram_name)
    : histogram_name_(std::move(histogram_name))
//     , over_prediction_histogram_(*GetHistogram(histogram_name_, "OverPrediction"))
//     , under_prediction_histogram_(*GetHistogram(histogram_name_, "UnderPrediction"))
//     , prediction_score_histogram_(*GetHistogram(histogram_name_, "PredictionScore"))
//     , frame_over_prediction_histogram_(*GetHistogram(histogram_name_, "FrameOverPrediction"))
//     , frame_under_prediction_histogram_(*GetHistogram(histogram_name_, "FrameUnderPrediction"))
//     , frame_prediction_score_histogram_(*GetHistogram(histogram_name_, "FramePredictionScore"))
//     , prediction_jitter_histogram_(*GetHistogram(histogram_name_, "PredictionJitter"))
...
void PredictionMetricsHandler::AddRealEvent(const gfx::PointF& pos, const base::TimeTicks& time_stamp, const base::TimeTicks& frame_time, bool scrolling)
{
...
void PredictionMetricsHandler::AddPredictedEvent(const gfx::PointF& pos, const base::TimeTicks& time_stamp, const base::TimeTicks& frame_time, bool scrolling)
{
    ...
    if (!predicted_events_queue_.empty() && time_stamp < predicted_events_queue_.back().time_stamp)
        needs_sorting = true;
    ...
    if (needs_sorting) {
        std::sort(
            predicted_events_queue_.begin(), predicted_events_queue_.end(), [](const EventData& a, const EventData& b) { return a.time_stamp < b.time_stamp; });
    }
}
...
void PredictionMetricsHandler::EvaluatePrediction()
{
    while (!predicted_events_queue_.empty()) {
        ...
        if (events_queue_.size() < 2 || events_queue_.back().time_stamp <= predicted_events_queue_.front().time_stamp
            || events_queue_.back().time_stamp <= predicted_events_queue_.front().frame_time) {
            return;
        }
...
int PredictionMetricsHandler::GetInterpolatedEventForPredictedEvent(const base::TimeTicks& interpolation_timestamp, gfx::PointF* interpolated)
{
    size_t idx = 0;
    while (idx < events_queue_.size() && interpolation_timestamp >= events_queue_[idx].time_stamp)
        idx++;

    if (idx == 0 || idx == events_queue_.size())
        return -1;

    const float alpha = (interpolation_timestamp - events_queue_[idx - 1].time_stamp) / (events_queue_[idx].time_stamp - events_queue_[idx - 1].time_stamp);
    *interpolated = events_queue_[idx - 1].pos + ScaleVector2d(events_queue_[idx].pos - events_queue_[idx - 1].pos, alpha);
    return idx - 1;
}
...
void PredictionMetricsHandler::ComputeMetrics()
{
    ...
    int low_idx_interpolated = GetInterpolatedEventForPredictedEvent(predicted_events_queue_.front().time_stamp, &interpolated_);
    int low_idx_frame_interpolated = GetInterpolatedEventForPredictedEvent(predicted_events_queue_.front().frame_time, &frame_interpolated_);

    next_real_ = events_queue_[low_idx_interpolated + 1].pos;
    next_real_point_after_frame_ = events_queue_[low_idx_frame_interpolated + 1].pos;

    int first_needed_event = std::min(low_idx_interpolated, low_idx_frame_interpolated);
...
double PredictionMetricsHandler::ComputeOverUnderPredictionMetric() const
{
    ...
    gfx::Vector2dF relative_direction = predicted_events_queue_.front().pos - interpolated_;
...
{
    gfx::Vector2dF real_direction = next_real_point_after_frame_ - frame_interpolated_;
    gfx::Vector2dF relative_direction = predicted_events_queue_.front().pos - frame_interpolated_;
    if (gfx::DotProduct(real_direction, relative_direction) >= 0)
        return relative_direction.Length();
    else
        return -relative_direction.Length();
...
{
    gfx::Vector2dF delta = frame_interpolated_ - predicted_events_queue_.front().pos;
    gfx::Vector2dF last_delta = last_frame_interpolated_ - last_predicted_.value();
    return (delta - last_delta).Length();
}
