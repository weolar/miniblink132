...
void KalmanPredictor::Update(const InputData& cur_input)
{
    ...
    while (last_points_.back().time_stamp - last_points_.front().time_stamp > kMaxTimeInQueue) {
...
std::unique_ptr<InputPredictor::InputData> KalmanPredictor::GeneratePrediction(base::TimeTicks predict_time, base::TimeDelta frame_interval)
{
    if (!HasPrediction())
        return nullptr;

    DCHECK(last_points_.size());
    float pred_dt = (predict_time - last_points_.back().time_stamp).InMillisecondsF();

    gfx::PointF position(last_points_.back().pos.x(), last_points_.back().pos.y());
    ...
    if (prediction_options_ & kDirectionCutOffEnabled) {
        gfx::Vector2dF future_velocity = velocity + ScaleVector2d(acceleration, pred_dt);
    ...
    if (prediction_options_ & kHeuristicsEnabled) {
        ...
        for (size_t i = 2; i < last_points_.size(); i++) {
            gfx::Vector2dF first_dir = last_points_[i - 1].pos - last_points_[i - 2].pos;
            gfx::Vector2dF second_dir = last_points_[i].pos - last_points_[i - 1].pos;
            if (first_dir.Length() && second_dir.Length()) {
                points_angle += atan2(first_dir.x(), first_dir.y()) - atan2(second_dir.x(), second_dir.y());
            }
        }
        if (base::RadToDeg(fabsf(points_angle)) > 15) {
            position += ScaleVector2d(acceleration, kAccelerationInfluence * pred_dt * pred_dt);
        }
    } else {
        position += ScaleVector2d(acceleration, kAccelerationInfluence * pred_dt * pred_dt);
    }
...
{
    return time_filter_.GetPosition() ? std::max(kMinTimeInterval, base::Milliseconds(time_filter_.GetPosition())) : kTimeInterval;
}
...
{
    return gfx::Vector2dF(x_predictor_.GetAcceleration(), y_predictor_.GetAcceleration());
}
