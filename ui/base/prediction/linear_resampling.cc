...
inline gfx::PointF lerp(const InputPredictor::InputData& a, const InputPredictor::InputData& b, base::TimeTicks sample_time)
{
    const float alpha = (sample_time - a.time_stamp) / (a.time_stamp - b.time_stamp);
...
void LinearResampling::Update(const InputData& new_input)
{
    ...
    // is a new trajectory
    if (!events_queue_.empty() && new_input.time_stamp - events_queue_.front().time_stamp > kMaxTimeDelta) {
        Reset();
    }
...
{
    return events_queue_.size() == kNumEventsForResampling && events_dt_ >= kResampleMinDelta;
}

std::unique_ptr<InputPredictor::InputData> LinearResampling::GeneratePrediction(base::TimeTicks frame_time, base::TimeDelta frame_interval)
{
    if (!HasPrediction())
        return nullptr;

    base::TimeDelta resample_latency = latency_calculator_.GetResampleLatency(frame_interval);
    ...
    if (!base::FeatureList::IsEnabled(::features::kResamplingScrollEventsExperimentalPrediction)) {
        base::TimeDelta max_prediction = std::min(kResampleMaxPrediction, events_dt_ / 2.0);

        sample_time = std::min(sample_time, events_queue_[0].time_stamp + max_prediction);
    }

    return std::make_unique<InputData>(lerp(events_queue_[0], events_queue_[1], sample_time), sample_time);
}
...
base::TimeDelta LinearResampling::LatencyCalculator::GetResampleLatency(base::TimeDelta frame_interval)
{
...
{
    std::string prediction_type = GetFieldTrialParamValueByFeature(::features::kResamplingScrollEventsExperimentalPrediction, "mode");

    if (prediction_type != ::features::kPredictionTypeTimeBased && prediction_type != ::features::kPredictionTypeFramesBased)
        return kResampleLatency;

    std::string latency_value = GetFieldTrialParamValueByFeature(::features::kResamplingScrollEventsExperimentalPrediction, "latency");
    double latency;
    if (base::StringToDouble(latency_value, &latency)) {
        return prediction_type == ::features::kPredictionTypeTimeBased ? base::Milliseconds(latency) : latency * frame_interval_ + kResampleLatency;
    }

    return prediction_type == ::features::kPredictionTypeTimeBased ? kResampleLatencyExperimental : 0.5 * frame_interval_ + kResampleLatency;
}
