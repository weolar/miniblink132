...
{
    return equation_order_ == EquationOrder::kFirstOrder ? features::kPredictorNameLinearFirst : features::kPredictorNameLinearSecond;
}
...
void LinearPredictor::Update(const InputData& new_input)
{
    ...
    // is a new trajectory
    if (!events_queue_.empty() && new_input.time_stamp - events_queue_.back().time_stamp > kMaxTimeDelta) {
        Reset();
    }
    ...
    if (events_queue_.size() >= static_cast<size_t>(EquationOrder::kFirstOrder)) {
        ...
        events_dt_ = (events_queue_.at(events_queue_.size() - 1).time_stamp - events_queue_.at(events_queue_.size() - 2).time_stamp).InMillisecondsF();

        // Get delta pos between the last 2 events
        gfx::Vector2dF delta_pos = events_queue_.at(events_queue_.size() - 1).pos - events_queue_.at(events_queue_.size() - 2).pos;
...
bool LinearPredictor::HasPrediction() const
{
    ...
    // first order
    return events_queue_.size() >= static_cast<size_t>(EquationOrder::kFirstOrder);
}

std::unique_ptr<InputPredictor::InputData> LinearPredictor::GeneratePrediction(base::TimeTicks predict_time, base::TimeDelta frame_interval)
{
    if (!HasPrediction())
        return nullptr;

    float pred_dt = (predict_time - events_queue_.back().time_stamp).InMillisecondsF();

    // Compute the prediction
    // Note : a first order prediction is computed when only 2 events are
    // available in the second order predictor
    if (equation_order_ == EquationOrder::kSecondOrder && events_dt_ > 0 && events_queue_.size() == static_cast<size_t>(EquationOrder::kSecondOrder)) {
        // Add the acceleration term to the current result
        return std::make_unique<InputData>(GeneratePredictionSecondOrder(pred_dt), predict_time);
    }
    return std::make_unique<InputData>(GeneratePredictionFirstOrder(pred_dt), predict_time);
}
...
gfx::PointF LinearPredictor::GeneratePredictionSecondOrder(float pred_dt) const
{
    DCHECK(equation_order_ == EquationOrder::kSecondOrder);

    gfx::Vector2dF acceleration = ScaleVector2d(cur_velocity_ - last_velocity_, 1.0 / events_dt_);
    return events_queue_.back().pos + ScaleVector2d(cur_velocity_, pred_dt) + ScaleVector2d(acceleration, 0.5 * pred_dt * pred_dt);
}

base::TimeDelta LinearPredictor::TimeInterval() const
{
    if (events_queue_.size() > 1) {
        return std::max(kMinTimeInterval, (events_queue_.back().time_stamp - events_queue_.front().time_stamp) / (events_queue_.size() - 1));
    }
