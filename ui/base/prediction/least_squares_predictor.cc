...
// Solve XB = y.
static bool SolveLeastSquares(const gfx::Matrix3F& x, const std::deque<double>& y, gfx::Vector3dF& result)
{
    ...
    result = gfx::MatrixProduct(gfx::MatrixProduct(temp, x_transpose), gfx::Vector3dF(y[0], y[1], y[2]));
    return true;
}
...
std::unique_ptr<InputPredictor::InputData> LeastSquaresPredictor::GeneratePrediction(base::TimeTicks predict_time, base::TimeDelta frame_interval)
{
    if (!HasPrediction())
        return nullptr;
    ...
    if (SolveLeastSquares(time_matrix, x_queue_, b1) && SolveLeastSquares(time_matrix, y_queue_, b2)) {
        gfx::Vector3dF prediction_time(1, pred_dt, pred_dt * pred_dt);

        return std::make_unique<InputData>(gfx::PointF(gfx::DotProduct(prediction_time, b1), gfx::DotProduct(prediction_time, b2)), predict_time);
    }
    return nullptr;
}
...
{
    if (time_.size() > 1) {
        return std::max(kMinTimeInterval, (time_.back() - time_.front()) / (time_.size() - 1));
    }
