...
{
    std::unique_ptr<KalmanFilter> kalman_filter = std::make_unique<KalmanFilter>();
    ...
    for (size_t i = 0; i < data.observation.size(); i++) {
        ...
        EXPECT_NEAR(data.acceleration[i], kalman_filter->GetAcceleration(), kEpsilon);
    }
}
...
public:
    ...
    void SetUp() override
    {
        predictor_ = std::make_unique<KalmanPredictor>(KalmanPredictor::PredictionOptions::kNone);
    }
};
...
TEST_F(KalmanPredictorTest, ShouldHasPrediction)
{
    for (uint32_t i = 0; i < kExpectedStableIterNum; i++) {
        ...
        InputPredictor::InputData data = { gfx::PointF(1, 1), FromMilliseconds(8 * i) };
...
TEST_F(KalmanPredictorTest, PredictLinearValue)
{
    ...
    for (size_t i = 0; i < t_stabilizer.size(); i++) {
        InputPredictor::InputData data = { gfx::PointF(x_stabilizer[i], y_stabilizer[i]), FromMilliseconds(t_stabilizer[i]) };
        predictor_->Update(data);
    }
    ...
    for (size_t i = 0; i < t.size(); i++) {
        ...
        InputPredictor::InputData data = { gfx::PointF(x[i], y[i]), FromMilliseconds(t[i]) };
        predictor_->Update(data);
    }
}
...
TEST_F(KalmanPredictorTest, TimeInterval)
{
    ...
    EXPECT_EQ(predictor_->TimeInterval().InMillisecondsF(), base::Milliseconds(7).InMillisecondsF());
}

// Test the benefit from the heuristic approach on noisy data.
TEST_F(KalmanPredictorTest, HeuristicApproach)
{
    std::unique_ptr<InputPredictor> heuristic_predictor = std::make_unique<KalmanPredictor>(KalmanPredictor::PredictionOptions::kHeuristicsEnabled);
    ...
    for (size_t i = 0; i < t.size(); i++) {
        ...
        if (heuristic_predictor->HasPrediction() && predictor_->HasPrediction()) {
            auto heuristic_result = heuristic_predictor->GeneratePrediction(FromMilliseconds(t[i]));
            auto result = predictor_->GeneratePrediction(FromMilliseconds(t[i]));
            EXPECT_TRUE(heuristic_result);
            EXPECT_TRUE(result);
            EXPECT_LE((heuristic_result->pos - point).Length(), (result->pos - point).Length());
        }
...
{
    predictor_ = std::make_unique<KalmanPredictor>(KalmanPredictor::PredictionOptions::kDirectionCutOffEnabled);
