...
OneEuroFilter::OneEuroFilter(double mincutoff, double beta)
{
    ...
    CHECK_GE(beta, 0);
    x_filter_ = std::make_unique<::OneEuroFilter>(kDefaultFrequency, mincutoff, beta, kDefaultDcutoff);
    y_filter_ = std::make_unique<::OneEuroFilter>(kDefaultFrequency, mincutoff, beta, kDefaultDcutoff);
}
...
bool OneEuroFilter::Filter(const base::TimeTicks& timestamp, gfx::PointF* position) const
{
    if (!position)
        return false;
