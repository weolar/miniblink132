...
public:
    ...
    void SetUp() override
    {
        predictor_ = std::make_unique<LinearResampling>();
    }

    void ValidatePredictorFrameBased(const std::vector<double>& events_x, const std::vector<double>& events_y, const std::vector<double>& events_time_ms,
        const std::vector<double>& prediction_time_ms, const std::vector<double>& predicted_x, const std::vector<double>& predicted_y,
        const double vsync_frequency)
    {
        ...
        for (size_t i = 0; i < events_time_ms.size(); i++) {
            InputPredictor::InputData data = { gfx::PointF(events_x[i], events_y[i]), FromMilliseconds(events_time_ms[i]) };
            predictor_->Update(data);

            if (predictor_->HasPrediction()) {
                auto result = predictor_->GeneratePrediction(FromMilliseconds(prediction_time_ms[current_prediction_index]), frame_interval);
                EXPECT_TRUE(result);
...
TEST_F(LinearResamplingTest, ShouldHavePrediction)
{
    ...
    // 1st event.
    predictor.Update(InputPredictor::InputData({ gfx::PointF(0, 0), FromMilliseconds(0) }));
    EXPECT_FALSE(predictor.HasPrediction());

    // 2nd event.
    predictor.Update(InputPredictor::InputData({ gfx::PointF(0, 0), FromMilliseconds(8) }));
...
TEST_F(LinearResamplingTest, ResampleMinDelta)
{
    ...
    predictor_->Update(InputPredictor::InputData({ gfx::PointF(0, 0), FromMilliseconds(0) }));
    predictor_->Update(InputPredictor::InputData({ gfx::PointF(0, 0), FromMilliseconds(1) }));
    // No prediction when last_dt < kResampleMinDelta.
    EXPECT_FALSE(predictor_->HasPrediction());

    // Has prediction when last_dt >= kResampleMinDelta.
    predictor_->Update(InputPredictor::InputData({ gfx::PointF(0, 0), FromMilliseconds(3) }));
    EXPECT_TRUE(predictor_->HasPrediction());

    predictor_->Update(InputPredictor::InputData({ gfx::PointF(0, 0), FromMilliseconds(15) }));
    EXPECT_TRUE(predictor_->HasPrediction());

    // Predictor is reset when dt > kMaxTimeDelta.
    predictor_->Update(InputPredictor::InputData({ gfx::PointF(0, 0), FromMilliseconds(36) }));
    EXPECT_FALSE(predictor_->HasPrediction());
}
...
TEST_F(LinearResamplingTest, ResamplingValueWithExperimentalLatencyTimeBased)
{
    ...
    feature_list.Reset();
    feature_list.InitAndEnableFeatureWithParameters(features::kResamplingScrollEventsExperimentalPrediction, params);
...
// of the default -5ms.
TEST_F(LinearResamplingTest, ResamplingValueWithExperimentalLatencyTimeBasedSwitch)
{
    base::FieldTrialParams params;
    params["mode"] = ::features::kPredictionTypeTimeBased;
    params["latency"] = "1.0";
...
TEST_F(LinearResamplingTest, ResamplingValueWithExperimentalLatencyFrameBasedSwitch)
{
    base::FieldTrialParams params;
    params["mode"] = ::features::kPredictionTypeFramesBased;
    params["latency"] = "1.0";
