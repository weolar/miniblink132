...
bool IsMouseOrTouchpadPresent()
{
    ...
    for (const ui::InputDevice& device : device_data_manager->GetTouchpadDevices()) {
        if (device.enabled)
            return true;
    }
    ...
    // connected.
    for (const ui::InputDevice& device : device_data_manager->GetPointingStickDevices()) {
        if (device.enabled) {
            return true;
        }
    }
...
{
    int max_touch = 0;
    const std::vector<ui::TouchscreenDevice>& touchscreen_devices = ui::DeviceDataManager::GetInstance()->GetTouchscreenDevices();
