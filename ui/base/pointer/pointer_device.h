...
enum HoverType { HOVER_TYPE_NONE = 1 << 0, HOVER_TYPE_FIRST = HOVER_TYPE_NONE, HOVER_TYPE_HOVER = 1 << 1, HOVER_TYPE_LAST = HOVER_TYPE_HOVER };
...
COMPONENT_EXPORT(UI_BASE)
void SetAvailablePointerAndHoverTypesForTesting(int available_pointer_types, int available_hover_types);
COMPONENT_EXPORT(UI_BASE)
...
inline constexpr bool operator==(const PointerDevice& left, const PointerDevice& right)
{
    return left.key == right.key;
}

inline constexpr bool operator==(const PointerDevice& left, PointerDevice::Key right)
{
    return left.key == right;
}

inline constexpr bool operator==(PointerDevice::Key left, const PointerDevice& right)
{
