...
PointerDigitizerType ToPointerDigitizerType(POINTER_DEVICE_TYPE pointer_device_type)
{
...
{
    return { .key = device.device, .digitizer = ToPointerDigitizerType(device.pointerDeviceType), .max_active_contacts = device.maxActiveContacts };
}
...
std::optional<PointerDevice> GetPointerDevice(PointerDevice::Key key)
{
    ...
    return base::win::GetPointerDevice(key, device) ? std::make_optional(ToPointerDevice(device)) : std::nullopt;
}

std::vector<PointerDevice> GetPointerDevices()
{
    std::vector<PointerDevice> result;
    if (std::optional<std::vector<POINTER_DEVICE_INFO>> pointer_devices = base::win::GetPointerDevices()) {
        result.reserve(pointer_devices->size());
        std::transform(pointer_devices->cbegin(), pointer_devices->cend(), std::back_inserter(result), &ToPointerDevice);
    }
    return result;
}
