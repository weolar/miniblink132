...
public:
    explicit TestObserver(TouchUiController* controller)
        : subscription_(controller->RegisterCallback(base::BindLambdaForTesting([this]() { ++touch_ui_changes_; })))
    {
    }
    ~TestObserver() = default;

    int touch_ui_changes() const
    {
        return touch_ui_changes_;
    }
...
TEST_F(TouchUiControllerTest, DetectPointerDevices)
{
    ...
    constexpr const char kOnDisconnectedHistogram[] = "Input.Digitizer.OnDisconnected";
    constexpr const char kMaxTouchPointsDirectPenHistogram[] = "Input.Digitizer.MaxTouchPoints.DirectPen";
    constexpr const char kMaxTouchPointsIndirectPenHistogram[] = "Input.Digitizer.MaxTouchPoints.IndirectPen";
    constexpr const char kMaxTouchPointsTouchHistogram[] = "Input.Digitizer.MaxTouchPoints.Touch";
    constexpr const char kMaxTouchPointsTouchPadHistogram[] = "Input.Digitizer.MaxTouchPoints.TouchPad";
    constexpr const char kMaxTouchPointsSupportedBySystemAtStartupHistogram[] = "Input.Digitizer.MaxTouchPointsSupportedBySystemAtStartup";

    static const PointerDevice kDirectPenDevice = { .key = PointerDevice::Key(0), .digitizer = PointerDigitizerType::kDirectPen, .max_active_contacts = 11 };
    static const PointerDevice kIndirectPenDevice
        = { .key = PointerDevice::Key(1), .digitizer = PointerDigitizerType::kIndirectPen, .max_active_contacts = 20 };
    static const PointerDevice kTouchKeyDevice = { .key = PointerDevice::Key(2), .digitizer = PointerDigitizerType::kTouch, .max_active_contacts = 1 };
    static const PointerDevice kTouchPadDevice = { .key = PointerDevice::Key(3), .digitizer = PointerDigitizerType::kTouchPad, .max_active_contacts = 5 };
    ...
    MockTouchUiController controller(TouchUiState::kAuto);
    controller.SetMockConnectedPointerDevices((std::vector<PointerDevice> { kTouchPadDevice, kDirectPenDevice, kTouchKeyDevice }));
    RunUntilIdle();
    histogram_tester.ExpectTotalCount(kOnStartupHistogram, 3);
    histogram_tester.ExpectBucketCount(kOnStartupHistogram, PointerDigitizerType::kDirectPen, 1);
    histogram_tester.ExpectBucketCount(kOnStartupHistogram, PointerDigitizerType::kTouch, 1);
    histogram_tester.ExpectBucketCount(kOnStartupHistogram, PointerDigitizerType::kTouchPad, 1);
    ...
    histogram_tester.ExpectTotalCount(kMaxTouchPointsSupportedBySystemAtStartupHistogram, 1);
    histogram_tester.ExpectBucketCount(kMaxTouchPointsSupportedBySystemAtStartupHistogram, 11, 1);
    ...
    controller.SetMockConnectedPointerDevices({ kTouchPadDevice, kIndirectPenDevice });
    ...
    EXPECT_EQ(controller.GetLastKnownPointerDevicesForTesting(), (std::vector<PointerDevice> { kTouchPadDevice, kIndirectPenDevice }));

    histogram_tester.ExpectTotalCount(kOnConnectedHistogram, 1);
    histogram_tester.ExpectBucketCount(kOnConnectedHistogram, PointerDigitizerType::kIndirectPen, 1);
    histogram_tester.ExpectTotalCount(kOnDisconnectedHistogram, 2);
    histogram_tester.ExpectBucketCount(kOnDisconnectedHistogram, PointerDigitizerType::kTouch, 1);
    histogram_tester.ExpectBucketCount(kOnDisconnectedHistogram, PointerDigitizerType::kDirectPen, 1);
    histogram_tester.ExpectTotalCount(kMaxTouchPointsIndirectPenHistogram, 1);
    histogram_tester.ExpectBucketCount(kMaxTouchPointsIndirectPenHistogram, 20, 1);
