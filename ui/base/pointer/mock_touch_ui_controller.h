...
public:
    explicit MockTouchUiController(TouchUiController::TouchUiState touch_ui_state = TouchUiController::TouchUiState::kAuto);
    ...
    void SetMockConnectedPointerDevices(const std::vector<PointerDevice>& devices);
...
protected:
    ...
    std::optional<PointerDevice> GetPointerDevice(PointerDevice::Key key) const final;
