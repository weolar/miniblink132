...
std::ostream& ui::mojom::operator<<(std::ostream& a, ::ui::mojom::CursorType b)
{
    *(int*)1 = 1;
    return a << (int)(b);
}
...
CursorData::CursorData(std::vector<SkBitmap> bitmaps, gfx::Point hotspot, float scale_factor)
...
// static
Cursor Cursor::NewCustom(SkBitmap bitmap, gfx::Point hotspot, float image_scale_factor)
{
    ...
    , image_scale_factor_(image_scale_factor)
{
    ...
    if (!custom_bitmap_.empty()) {
        custom_hotspot_ = gfx::Point(std::clamp(hotspot.x(), 0, custom_bitmap_.width() - 1), std::clamp(hotspot.y(), 0, custom_bitmap_.height() - 1));
    }
}
...
bool Cursor::operator==(const Cursor& cursor) const
{
    return type_ == cursor.type_ && platform_cursor_ == cursor.platform_cursor_
        && (type_ != CursorType::kCustom
            || (image_scale_factor_ == cursor.image_scale_factor_ && custom_hotspot_ == cursor.custom_hotspot_
                && gfx::BitmapsAreEqual(custom_bitmap_, cursor.custom_bitmap_)));
}

// static
bool Cursor::AreDimensionsValidForWeb(const gfx::Size& size, float scale_factor)
{
    if (scale_factor == 0) {
        return false;
    }
    ...
    if (size_in_dip.width() > kMaximumCursorDIPSize || size_in_dip.height() > kMaximumCursorDIPSize) {
        return false;
    }
