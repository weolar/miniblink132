...
TEST(CursorTest, CustomType)
{
    ...
    EXPECT_EQ(kBitmap.getGenerationID(), cursor.custom_bitmap().getGenerationID());
    ...
    Cursor copy(cursor);
    EXPECT_EQ(cursor.custom_bitmap().getGenerationID(), copy.custom_bitmap().getGenerationID());
    EXPECT_TRUE(gfx::BitmapsAreEqual(cursor.custom_bitmap(), copy.custom_bitmap()));
    EXPECT_EQ(cursor, copy);
}

TEST(CursorTest, CustomTypeComparesBitmapPixels)
{
    const Cursor kCursor1 = Cursor::NewCustom(gfx::test::CreateBitmap(/*size=*/10), gfx::Point());
    const Cursor kCursor2 = Cursor::NewCustom(gfx::test::CreateBitmap(/*size=*/10), gfx::Point());

    EXPECT_NE(kCursor1.custom_bitmap().getGenerationID(), kCursor2.custom_bitmap().getGenerationID());
    EXPECT_TRUE(gfx::BitmapsAreEqual(kCursor1.custom_bitmap(), kCursor2.custom_bitmap()));
...
TEST(CursorTest, ClampHotspot)
{
    ...
    const Cursor cursor = Cursor::NewCustom(gfx::test::CreateBitmap(5, 7), gfx::Point(100, 100));
