...
{
    return mojom::Cursor::DeserializeFromMessage(mojom::Cursor::SerializeAsMessage(&in), out);
}
...
{
    for (int i = base::checked_cast<int>(CursorType::kMinValue); i < base::checked_cast<int>(CursorType::kMaxValue); ++i) {
...
TEST_F(CursorStructTraitsTest, TestBitmapCursor)
{
    ...
    EXPECT_TRUE(gfx::BitmapsAreEqual(input.custom_bitmap(), output.custom_bitmap()));
    EXPECT_NE(input.custom_bitmap().getGenerationID(), output.custom_bitmap().getGenerationID());

    // Make a copy of output; the bitmap generation ids should be the same.
    const Cursor copy = output;
    EXPECT_EQ(output.custom_bitmap().getGenerationID(), copy.custom_bitmap().getGenerationID());
...
TEST_F(CursorStructTraitsTest, TestEmptyCursor)
{
    ...
    const float kScale = 2.0f;
    const Cursor input = Cursor::NewCustom(SkBitmap(), std::move(hotspot), kScale);
    Cursor output;
...
TEST_F(CursorStructTraitsTest, TestCustomCursorConstraints)
{
    ...
    input = ui::Cursor::NewCustom(gfx::test::CreateBitmap(kMaximumCursorDIPSize + 1, 3), kHotspot, 1.0f);
    EXPECT_FALSE(EchoCursor(input, &output));

    // Expect a failure when the 1x scaled image height is too large.
    input = ui::Cursor::NewCustom(gfx::test::CreateBitmap(3, kMaximumCursorDIPSize + 1), kHotspot, 1.0f);
    EXPECT_FALSE(EchoCursor(input, &output));

    // Expect a failure when the scaled image width is too large.
    input = ui::Cursor::NewCustom(gfx::test::CreateBitmap(kMaximumCursorDIPSize / 2, 5), kHotspot, 0.02f);
    EXPECT_FALSE(EchoCursor(input, &output));

    // Expect a failure when the scaled image height is too large.
    input = ui::Cursor::NewCustom(gfx::test::CreateBitmap(5, kMaximumCursorDIPSize / 4), kHotspot, 0.1f);
    EXPECT_FALSE(EchoCursor(input, &output));

    // Expect a failure if calculating the image size overflows.
    input = ui::Cursor::NewCustom(gfx::test::CreateBitmap(10, 10), kHotspot, std::numeric_limits<float>::epsilon());
    EXPECT_FALSE(EchoCursor(input, &output));
}
