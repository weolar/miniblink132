...
public:
    CursorData();
    CursorData(std::vector<SkBitmap> bitmaps, gfx::Point hotspot, float scale_factor = 1.0f);
...
public:
    ...
    static Cursor NewCustom(SkBitmap bitmap, gfx::Point hotspot, float image_scale_factor = 1.0f);
    Cursor();
    ...
    scoped_refptr<PlatformCursor> platform() const
    {
    ...
    bool operator==(mojom::CursorType type) const
    {
        return type_ == type;
    }
    bool operator!=(mojom::CursorType type) const
    ...
    static bool AreDimensionsValidForWeb(const gfx::Size& size, float scale_factor);
