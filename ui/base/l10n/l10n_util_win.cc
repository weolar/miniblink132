...
base::LazyInstance<OverrideLocaleHolder>::DestructorAtExit override_locale_holder = LAZY_INSTANCE_INITIALIZER;
...
void AdjustUiFont(gfx::win::FontAdjustment& font_adjustment)
{
    ...
    if (!ui_font_family.empty()) {
        int scaler;
        if (base::StringToInt(l10n_util::GetStringUTF16(IDS_UI_FONT_SIZE_SCALER), &scaler)) {
            if (const bool is_custom = ui_font_family != L"default"; is_custom || scaler != 100) {
                if (is_custom) {
...
void OverrideLocaleWithUILanguageList()
{
    ...
    base::ranges::transform(ui_languages, std::back_inserter(ascii_languages), &base::WideToASCII);
