...
{
    static const char* const kDuplicateNames[] = { "ar_001", "en", "en_001", "en_150",
        "pt", // pt-BR and pt-PT are used.
        "zh", "zh_hans_cn", "zh_hant_hk", "zh_hant_mo", "zh_hans_sg", "zh_hant_tw" };

    // Skip all the es_Foo other than es_419 for now.
    if (base::StartsWith(locale_name, "es_", base::CompareCase::INSENSITIVE_ASCII)) {
...
// more information.
bool HasStringsForLocale(const std::string& locale, const bool perform_io = true)
{
    if (!perform_io) {
        return std::binary_search(std::begin(kPlatformLocales), std::end(kPlatformLocales), locale);
    }
...
struct AvailableLocalesTraits : base::internal::DestructorAtExitLazyInstanceTraits<std::vector<std::string>> {
    static std::vector<std::string>* New(void* instance)
    {
        std::vector<std::string>* locales = base::internal::DestructorAtExitLazyInstanceTraits<std::vector<std::string>>::New(instance);
...
base::LazyInstance<std::vector<std::string>, AvailableLocalesTraits> g_available_locales = LAZY_INSTANCE_INITIALIZER;
...
std::string GetCountry(std::string_view locale)
{
    ...
    return (hyphen_pos == std::string::npos) ? std::string() : std::string(locale).substr(hyphen_pos + 1);
}

// TODO(jshin): revamp this function completely to use a more systematic
// and generic locale fallback based on ICU/CLDR.
bool CheckAndResolveLocale(const std::string& locale, std::string* resolved_locale, const bool perform_io)
{
    ...
    if (lang.size() < locale.size()) {
        ...
        // Spanish locale).
        if (base::EqualsCaseInsensitiveASCII(lang, "es") && !base::EqualsCaseInsensitiveASCII(region, "es")) {
#if BUILDFLAG(IS_IOS)
...
#endif
        } else if (base::EqualsCaseInsensitiveASCII(lang, "pt") && !base::EqualsCaseInsensitiveASCII(region, "br")) {
            ...
            if (base::EqualsCaseInsensitiveASCII(region, "hk") || base::EqualsCaseInsensitiveASCII(region, "mo")) { // Macao
            ...
            if (base::EqualsCaseInsensitiveASCII(region, "lr") || base::EqualsCaseInsensitiveASCII(region, "ph")) {
...
std::string GetApplicationLocaleInternalNonMac(const std::string& pref_locale)
{
    ...
    if (!languages.empty()) {
        ...
        base::ranges::transform(languages, std::back_inserter(candidates), &base::i18n::GetCanonicalLocale);
    } else {
...
std::string GetApplicationLocale(const std::string& pref_locale, bool set_icu_locale)
{
...
bool IsLocaleNameTranslated(std::string_view locale, std::string_view display_locale)
{
    std::u16string display_name = l10n_util::GetDisplayNameForLocale(locale, display_locale, false);
...
std::u16string GetDisplayNameForLocaleWithoutCountry(std::string_view locale, std::string_view display_locale, bool is_for_ui, bool disallow_default)
{
    return GetDisplayNameForLocale(GetLanguage(locale), display_locale, is_for_ui, disallow_default);
}

std::u16string GetDisplayNameForLocale(std::string_view locale, std::string_view display_locale, bool is_for_ui, bool disallow_default)
{
...
#endif // BUILDFLAG(IS_ANDROID)
    {
        ...
        if (locale_code[0] == '-' || locale_code[0] == '_') {
            actual_size = uloc_getDisplayCountry(
                locale_code.c_str(), display_locale_code.c_str(), base::WriteInto(&display_name, kBufferSize), kBufferSize - 1, &error);
        } else {
            actual_size
                = uloc_getDisplayName(locale_code.c_str(), display_locale_code.c_str(), base::WriteInto(&display_name, kBufferSize), kBufferSize - 1, &error);
        }
...
std::u16string GetDisplayNameForCountry(const std::string& country_code, const std::string& display_locale)
{
...
void GetParentLocales(const std::string& current_locale, std::vector<std::string>* parent_locales)
{
...
bool IsValidLocaleSyntax(const std::string& locale)
{
    ...
    if (split_point != std::string::npos) {
        ...
        if (equals_loc == 0 || equals_loc == std::string::npos || equals_loc > keywords.size() - 2) {
            return false;
        }
    }
...
std::u16string FormatString(const std::u16string& format_string, const std::vector<std::u16string>& replacements, std::vector<size_t>* offsets)
{
#if DCHECK_IS_ON()
    ...
    for (size_t i = 0; i < 9; ++i) {
        ...
        if (placeholder_should_exist) {
            DCHECK_NE(std::string::npos, pos) << " Didn't find a " << placeholder << " placeholder in " << format_string;
        } else {
            DCHECK_EQ(std::string::npos, pos) << " Unexpectedly found a " << placeholder << " placeholder in " << format_string;
        }
    }
#endif
...
std::u16string GetStringFUTF16(int message_id, const std::vector<std::u16string>& replacements, std::vector<size_t>* offsets)
{
...
std::string GetStringFUTF8(int message_id, const std::u16string& a, const std::u16string& b)
{
    return base::UTF16ToUTF8(GetStringFUTF16(message_id, a, b));
}

std::string GetStringFUTF8(int message_id, const std::u16string& a, const std::u16string& b, const std::u16string& c)
{
    return base::UTF16ToUTF8(GetStringFUTF16(message_id, a, b, c));
}

std::string GetStringFUTF8(int message_id, const std::u16string& a, const std::u16string& b, const std::u16string& c, const std::u16string& d)
{
...
std::u16string GetStringFUTF16(int message_id, const std::u16string& a, const std::u16string& b)
{
    return GetStringFUTF16(message_id, a, b, nullptr);
}

std::u16string GetStringFUTF16(int message_id, const std::u16string& a, const std::u16string& b, const std::u16string& c)
{
    std::vector<std::u16string> replacements = { a, b, c };
    return GetStringFUTF16(message_id, replacements, nullptr);
}

std::u16string GetStringFUTF16(int message_id, const std::u16string& a, const std::u16string& b, const std::u16string& c, const std::u16string& d)
{
    std::vector<std::u16string> replacements = { a, b, c, d };
    return GetStringFUTF16(message_id, replacements, nullptr);
}

std::u16string GetStringFUTF16(
    int message_id, const std::u16string& a, const std::u16string& b, const std::u16string& c, const std::u16string& d, const std::u16string& e)
{
    std::vector<std::u16string> replacements = { a, b, c, d, e };
    return GetStringFUTF16(message_id, replacements, nullptr);
}

std::u16string GetStringFUTF16(int message_id, const std::u16string& a, size_t* offset)
{
    DCHECK(offset);
...
std::u16string GetStringFUTF16(int message_id, const std::u16string& a, const std::u16string& b, std::vector<size_t>* offsets)
{
...
{
    return base::i18n::MessageFormatter::FormatWithNumberedArgs(GetStringUTF16(message_id), number);
}
...
{
    return base::i18n::MessageFormatter::FormatWithNumberedArgs(GetStringUTF16(message_id), is_multiple ? "multiple" : "single");
}

void SortStrings16(const std::string& locale, std::vector<std::u16string>* strings)
{
...
void GetAcceptLanguagesForLocale(const std::string& display_locale, std::vector<std::string>* locale_codes)
{
    for (std::string_view accept_language : kAcceptLanguageList) {
...
bool IsAcceptLanguageDisplayable(const std::string& display_locale, const std::string& locale)
{
    return IsPossibleAcceptLanguage(locale) && l10n_util::IsLocaleNameTranslated(locale.c_str(), display_locale);
}

std::vector<std::string> KeepAcceptedLanguages(base::span<const std::string> languages)
{
    std::vector<std::string> filtered_languages;
    base::ranges::copy_if(languages, std::back_inserter(filtered_languages), IsPossibleAcceptLanguage);
...
{
    return std::vector<std::string_view>(kAcceptLanguageList.begin(), kAcceptLanguageList.end());
}
