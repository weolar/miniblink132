...
template <class T, class Method> class StringMethodComparatorWithCollator {
public:
    ...
    bool operator()(const std::unique_ptr<T>& lhs_t, const std::unique_ptr<T>& rhs_t)
    {
        return base::i18n::CompareString16WithCollator(*collator_, (lhs_t.get()->*method_)(), (rhs_t.get()->*method_)()) == UCOL_LESS;
    }
...
template <class T, class Method> class StringMethodComparator {
public:
...
template <class T, class Method> void SortStringsUsingMethod(const std::string& locale, std::vector<std::unique_ptr<T>>* elements, Method method)
{
    UErrorCode error = U_ZERO_ERROR;
    icu::Locale loc(locale.c_str());
    std::unique_ptr<icu::Collator> collator(icu::Collator::createInstance(loc, error));
    if (U_FAILURE(error)) {
        sort(elements->begin(), elements->end(), StringMethodComparator<T, Method>(method));
        return;
    }

    std::sort(elements->begin(), elements->end(), StringMethodComparatorWithCollator<T, Method>(collator.get(), method));
}
...
template <class Element> class StringComparator {
public:
    ...
    bool operator()(const Element& lhs, const Element& rhs) const
    {
        ...
        return StringComparator<std::u16string>(collator_)(lhs_string_key, rhs_string_key);
    }
...
template <> COMPONENT_EXPORT(UI_BASE) inline bool StringComparator<std::u16string>::operator()(const std::u16string& lhs, const std::u16string& rhs) const
{
...
void SortVectorWithStringKey(const std::string& locale, std::vector<Element>* elements, size_t begin_index, size_t end_index, bool needs_stable_sort)
{
...
template <class Element> void SortVectorWithStringKey(const std::string& locale, std::vector<Element>* elements, bool needs_stable_sort)
{
    SortVectorWithStringKey<Element>(locale, elements, 0, elements->size(), needs_stable_sort);
}
