...
class StringWrapper {
public:
    explicit StringWrapper(const std::u16string& string)
        : string_(string)
    {
    }
...
TEST_F(L10nUtilTest, GetAppLocale)
{
    ...
    for (size_t i = 0; i < std::size(filenames); ++i) {
        base::FilePath filename = new_locale_dir.AppendASCII(filenames[i] + ".pak");
...
TEST_F(L10nUtilTest, SortStringsUsingFunction)
{
    ...
    l10n_util::SortStringsUsingMethod("en-US", &strings, &StringWrapper::string);
 ...
 */
void CheckUiDisplayNameForLocale(const std::string& locale, const std::string& display_locale, bool is_rtl)
{
    EXPECT_EQ(true, base::i18n::IsRTL());
    std::u16string result = l10n_util::GetDisplayNameForLocale(locale, display_locale,
        /* is_for_ui */ true);
...
{
    ASSERT_EQ(u"English", l10n_util::GetDisplayNameForLocaleWithoutCountry("en-US", "en", false));
    ASSERT_EQ(u"English", l10n_util::GetDisplayNameForLocaleWithoutCountry("en-GB", "en", false));
    ASSERT_EQ(u"English", l10n_util::GetDisplayNameForLocaleWithoutCountry("en-AU", "en", false));
    ASSERT_EQ(u"English", l10n_util::GetDisplayNameForLocaleWithoutCountry("en", "en", false));
    EXPECT_EQ(u"Spanish", l10n_util::GetDisplayNameForLocaleWithoutCountry("es-419", "en", false));
    EXPECT_EQ(u"Chinese", l10n_util::GetDisplayNameForLocaleWithoutCountry("zh-CH", "en", false));
    EXPECT_EQ(u"Chinese", l10n_util::GetDisplayNameForLocaleWithoutCountry("zh-TW", "en", false));
}
...
TEST_F(L10nUtilTest, GetDisplayNameForLocale)
{
    ...
    // on other platforms.
    std::u16string result = l10n_util::GetDisplayNameForLocale("zh-CN", "en", false);
    EXPECT_TRUE(base::MatchPattern(base::UTF16ToUTF8(result), "Chinese*Simplified*"));
    ...
    EXPECT_TRUE(base::MatchPattern(base::UTF16ToUTF8(result), "Chinese*Traditional*"));
...
TEST_F(L10nUtilTest, IsValidLocaleSyntax)
{
    ...
    EXPECT_TRUE(l10n_util::IsValidLocaleSyntax("fr@collation=phonebook;calendar=islamic-civil"));
    EXPECT_TRUE(l10n_util::IsValidLocaleSyntax("sr_Latn_RS_REVISED@currency=USD"));
...
TEST_F(L10nUtilTest, TimeDurationFormatAllLocales)
{
    ...
    for (const std::string& locale : l10n_util::GetAvailableICULocales()) {
        ...
        std::u16string str;
        const bool result = base::TimeDurationFormat(kDelta, base::DURATION_WIDTH_NUMERIC, &str);
...
TEST_F(L10nUtilTest, GetUserFacingUILocaleList)
{
    ...
    const base::flat_set<std::string> locales = l10n_util::GetUserFacingUILocaleList();
...
TEST_F(L10nUtilTest, PlatformLocalesIsSorted)
{
    ...
    for (size_t i = 1; i < locales_size; i++) {
        ...
        EXPECT_LT(strcmp(last_locale, cur_locale), 0) << "Incorrect ordering in kPlatformLocales: " << last_locale << " >= " << cur_locale;
...
{
    // All valid languages.
    EXPECT_EQ(l10n_util::KeepAcceptedLanguages({ "en", "es", "fr" }), std::vector<std::string>({ "en", "es", "fr" }));
    // Some invalid languages.
    EXPECT_EQ(l10n_util::KeepAcceptedLanguages({ "en", "es", "iw" }), std::vector<std::string>({ "en", "es" }));
    // All invalid languages.
    EXPECT_EQ(l10n_util::KeepAcceptedLanguages({ "iw", "ch_ZN" }), std::vector<std::string> {});
    // Empty input.
    EXPECT_EQ(l10n_util::KeepAcceptedLanguages({}), std::vector<std::string> {});
    // Maintain languages order.
    EXPECT_EQ(l10n_util::KeepAcceptedLanguages({ "en", "aa", "es", "iw", "fr", "xx" }), std::vector<std::string>({ "en", "es", "fr" }));
}
...
TEST_F(L10nUtilTest, FormatStringComputeCorrectOffsetInRTL)
{
    ...
    std::u16string formatted_string = l10n_util::FormatString(kFormatString, { u"Replacement" }, &offsets);
