...
public:
    ...
    Formatter(const Pluralities& sec_pluralities, const Pluralities& min_pluralities, const Pluralities& hour_pluralities, const Pluralities& day_pluralities,
        const Pluralities& month_pluralities, const Pluralities& year_pluralities);

    Formatter(const Pluralities& sec_pluralities, const Pluralities& min_pluralities, const Pluralities& hour_pluralities, const Pluralities& day_pluralities,
        const Pluralities& month_pluralities, const Pluralities& year_pluralities, const Pluralities& min_sec_pluralities1,
    ...
    void Format(TwoUnits units, int value_1, int value_2, icu::UnicodeString* formatted_string) const;
...
private:
    ...
    std::unique_ptr<icu::MessageFormat> CreateFallbackFormat(const icu::PluralRules& rules, const Pluralities& pluralities) const;

    std::unique_ptr<icu::MessageFormat> InitFormat(const Pluralities& pluralities);
...
public:
    ...
    const Formatter* Get(TimeFormat::Format format, TimeFormat::Length length) const;
...
private:
    ...
    std::unique_ptr<Formatter> formatter_[TimeFormat::FORMAT_COUNT][TimeFormat::LENGTH_COUNT];
};
...
extern COMPONENT_EXPORT(UI_BASE) base::LazyInstance<FormatterContainer>::Leaky g_container;
