...
base::LazyInstance<FormatterContainer>::Leaky g_container = LAZY_INSTANCE_INITIALIZER;

// static
std::u16string TimeFormat::Simple(TimeFormat::Format format, TimeFormat::Length length, const base::TimeDelta& delta)
{
    return Detailed(format, length, 0, delta);
}

std::u16string TimeFormat::SimpleWithMonthAndYear(TimeFormat::Format format, TimeFormat::Length length, const base::TimeDelta& delta, bool with_month_and_year)
{
    return DetailedWithMonthAndYear(format, length, 0, delta, with_month_and_year);
}

// static
std::u16string TimeFormat::Detailed(TimeFormat::Format format, TimeFormat::Length length, int cutoff, const base::TimeDelta& delta)
{
    return DetailedWithMonthAndYear(format, length, cutoff, delta, false);
}

std::u16string TimeFormat::DetailedWithMonthAndYear(
    TimeFormat::Format format, TimeFormat::Length length, int cutoff, const base::TimeDelta& delta, bool with_month_and_year)
{
    ...
    if (delta < kMinute - kHalfSecond) {
    ...
    } else if (delta < kHour - (cutoff < base::Time::kMinutesPerHour ? kHalfMinute : kHalfSecond)) {
        ...
        if (delta >= cutoff * kMinute - kHalfSecond) {
            ...
            const int seconds = base::ClampRound(delta.InSecondsF()) % base::Time::kSecondsPerMinute;
            formatter->Format(Formatter::TWO_UNITS_MIN_SEC, minutes, seconds, &time_string);
        }
    } else if (delta < kDay - (cutoff < base::Time::kHoursPerDay ? kHalfHour : kHalfMinute)) {
        ...
        if (delta >= cutoff * kHour - kHalfMinute) {
            ...
            const int minutes = (delta + kHalfMinute).InMinutes() % base::Time::kMinutesPerHour;
            formatter->Format(Formatter::TWO_UNITS_HOUR_MIN, hours, minutes, &time_string);
        }
        ...
        if (delta >= cutoff * kDay - kHalfHour) {
            ...
            const int hours = (delta + kHalfHour).InHours() % base::Time::kHoursPerDay;
            formatter->Format(Formatter::TWO_UNITS_DAY_HOUR, days, hours, &time_string);
        }
    } else if (delta < kYear) {
    ...
    time_string.extract(base::WriteInto(&result, static_cast<size_t>(capacity)), capacity, error);
    DCHECK(U_SUCCESS(error));
    return result;
}

// static
std::u16string TimeFormat::RelativeDate(const base::Time& time, const base::Time* optional_midnight_today)
{
    const base::Time midnight_today = optional_midnight_today ? *optional_midnight_today : base::Time::Now().LocalMidnight();
    constexpr base::TimeDelta kDay = base::Days(1);
    ...
    return (time >= yesterday) ? l10n_util::GetStringUTF16(IDS_PAST_TIME_YESTERDAY) : std::u16string();
}
