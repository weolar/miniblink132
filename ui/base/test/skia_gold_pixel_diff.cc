...
const char* kGoldOutputTriageFormat = "Untriaged or negative image: https://chrome-gold.skia.org";
...
void AppendArgsJustAfterProgram(base::CommandLine& cmd, base::CommandLine::StringVector args)
{
    base::CommandLine::StringVector& argv = const_cast<base::CommandLine::StringVector&>(cmd.argv());
...
void FillInSystemEnvironment(TestEnvironmentMap& test_environment)
{
    ...
    auto CheckInsertDefaultKey = [&test_environment](TestEnvironmentKey key, std::string value) {
...
bool WriteTestEnvironmentToFile(const TestEnvironmentMap& test_environment, const base::FilePath& keys_file)
{
    base::Value::Dict ds;
    ...
    base::File file(keys_file, base::File::Flags::FLAG_CREATE_ALWAYS | base::File::Flags::FLAG_WRITE);
...
using SessionCache = base::LRUCache<SessionCacheKey, std::unique_ptr<SkiaGoldPixelDiff>>;
...
int LaunchProcess(const base::CommandLine& cmdline)
{
    ...
    if (triage_location_start != std::string::npos) {
        ...
        LOG(WARNING) << "For committers not using @google.com account, triage "
                     ...
                     << output.substr(triage_location_start + strlen(kGoldOutputTriageFormat),
                            triage_location_end - triage_location_start - strlen(kGoldOutputTriageFormat));
    }
    return exit_code;
}
...
SkiaGoldPixelDiff::ScopedSessionCacheForTesting::ScopedSessionCacheForTesting()
{
    g_sessions.Clear();
}
...
// static
SkiaGoldPixelDiff* SkiaGoldPixelDiff::GetSession(const std::optional<std::string>& corpus, TestEnvironmentMap test_environment)
{
    ...
    if (it == g_sessions.end()) {
        std::unique_ptr<SkiaGoldPixelDiff> pixel_diff = std::unique_ptr<SkiaGoldPixelDiff>(new SkiaGoldPixelDiff());
...
// static
base::AutoReset<SkiaGoldPixelDiff::LaunchProcessCallback> SkiaGoldPixelDiff::OverrideLaunchProcessForTesting(
    SkiaGoldPixelDiff::LaunchProcessCallback custom_launch_process)
{
    base::AutoReset auto_reset(&g_custom_launch_process, std::move(custom_launch_process));
    return auto_reset;
}
...
{
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(kBypassSkiaGoldFunctionality)) {
    ...
    base::FilePath json_temp_file = working_dir_.Append(FILE_PATH_LITERAL("keys_file.txt"));
    ASSERT_TRUE(WriteTestEnvironmentToFile(test_environment_, json_temp_file));
    base::FilePath failure_temp_file = working_dir_.Append(FILE_PATH_LITERAL("failure.log"));
...
void SkiaGoldPixelDiff::Init(const std::string& corpus, TestEnvironmentMap test_environment)
{
    ...
    ASSERT_TRUE(cmd_line->HasSwitch(kBuildRevisionKey) || is_dry_run_) << "Missing switch " << kBuildRevisionKey;

    // Use the dummy revision code for dry run.
    build_revision_ = is_dry_run_ ? kDummyBuildRevision : cmd_line->GetSwitchValueASCII(kBuildRevisionKey);

    ASSERT_TRUE(cmd_line->HasSwitch(kIssueKey) && cmd_line->HasSwitch(kPatchSetKey) && cmd_line->HasSwitch(kJobIdKey)
        || !cmd_line->HasSwitch(kIssueKey) && !cmd_line->HasSwitch(kPatchSetKey) && !cmd_line->HasSwitch(kJobIdKey))
        << "Missing switch. If it's running for tryjob, you should pass --" << kIssueKey << " --" << kPatchSetKey << " --" << kJobIdKey
    ...
    base::ScopedAllowBlockingForTesting allow_blocking;
    base::CreateNewTempDirectory(FILE_PATH_LITERAL("SkiaGoldTemp"), &working_dir_);

    InitSkiaGold();
}

bool SkiaGoldPixelDiff::UploadToSkiaGoldServer(
    const base::FilePath& local_file_path, const std::string& remote_golden_image_name, const SkiaGoldMatchingAlgorithm* algorithm) const
{
    // Copy the png file to another place for local debugging.
    base::CommandLine* process_command_line = base::CommandLine::ForCurrentProcess();
    if (process_command_line->HasSwitch(kPngFilePathDebugging)) {
        base::FilePath path = process_command_line->GetSwitchValuePath(kPngFilePathDebugging);
        if (!base::PathExists(path)) {
            base::CreateDirectory(path);
        }
        base::FilePath filepath;
        if (remote_golden_image_name.length() <= 4
            || (remote_golden_image_name.length() > 4 && remote_golden_image_name.substr(remote_golden_image_name.length() - 4) != ".png")) {
...
// static
std::string SkiaGoldPixelDiff::GetGoldenImageName(const std::string& test_suite_name, const std::string& test_name, const std::optional<std::string>& suffix)
{
    std::vector<std::string_view> parts;

    // Test suites can have "/" in their names from a parameterization
    // instantiation, which isn't allowed in file names.
    auto test_suite_parts = base::SplitStringPiece(test_suite_name, "/", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    auto test_name_parts = base::SplitStringPiece(test_name, "/", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    const char* separator = GetPlatform() == std::string("ash") ? kAshSeparator : kNonAshSeparator;
    return base::JoinString(parts, separator);
}

// static
std::string SkiaGoldPixelDiff::GetGoldenImageName(const ::testing::TestInfo* test_info, const std::optional<std::string>& suffix)
{
    return GetGoldenImageName(test_info->test_suite_name(), test_info->name(), suffix);
}

bool SkiaGoldPixelDiff::CompareScreenshot(const std::string& golden_image_name, const SkBitmap& bitmap, const SkiaGoldMatchingAlgorithm* algorithm) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK(initialized_) << "Initialize the class before using this method.";
    std::optional<std::vector<uint8_t>> output = gfx::PNGCodec::EncodeBGRASkBitmap(bitmap, /*discard_transparency=*/true);
    if (!output) {
    ...
    CHECK_EQ(golden_image_name.find_first_of(" /"), std::string::npos)
        << " a golden image name should not contain any space or back slash: " << golden_image_name;

    base::ScopedAllowBlockingForTesting allow_blocking;
    base::FilePath temporary_path = working_dir_.Append(base::FilePath::FromUTF8Unsafe(golden_image_name + ".png"));
    base::File file(temporary_path, base::File::Flags::FLAG_CREATE_ALWAYS | base::File::Flags::FLAG_WRITE);
    ...
    bool success = UploadToSkiaGoldServer(temporary_path, golden_image_name, algorithm);
...
void SkiaGoldPixelDiff::GenerateLocalDiff(const std::string& remote_golden_image_name, const base::FilePath& test_output_path) const
{
    base::CommandLine* process_command_line = base::CommandLine::ForCurrentProcess();
    if (!process_command_line->HasSwitch(kPngFilePathDebugging)) {
        LOG(WARNING) << "Please use --" << kPngFilePathDebugging << " to generate local diff images for this screenshot mismatch.";
        return;
    }
    base::FilePath path = process_command_line->GetSwitchValuePath(kPngFilePathDebugging);
    if (!base::PathExists(path)) {
    ...
    base::FilePath temp_work_dir = base::FileEnumerator(temp_dir.GetPath(), false, base::FileEnumerator::DIRECTORIES, working_dir_.BaseName().value()).Next();
    ...
    auto AssignIfNewer = [](std::optional<DiffLink>& image, const base::FilePath& png_path, const base::Time& mtime) {
    ...
    DiffLinks results;
    base::FileEnumerator e(output_dir, false, base::FileEnumerator::FILES, FILE_PATH_LITERAL("*.png"));
    ...
    auto FormatPathForTerminalOutput = [](std::optional<DiffLink>& path) -> std::optional<std::string> {
        if (path.has_value()) {
            base::FilePath path_absolute = path.value().png_path;
            if (!path_absolute.IsAbsolute()) {
                path_absolute = base::PathService::CheckedGet(base::DIR_CURRENT).Append(path_absolute);
            }
            ...
            return { std::string("file:///") + path_normalized.NormalizePathSeparatorsTo(FILE_PATH_LITERAL('/')).MaybeAsASCII() };
        } else {
            return std::nullopt;
        }
    };
    ...
    LOG(INFO) << "\n  Generated image: " << FormatPathForTerminalOutput(results.given_image).value_or(failure_message)
              << "\n  Closest image: " << FormatPathForTerminalOutput(results.closest_image).value_or(failure_message)
              << "\n  Diff image: " << FormatPathForTerminalOutput(results.diff_image).value_or(failure_message) << "\n";
}
