...
public:
    using MockLaunchProcess = testing::MockFunction<SkiaGoldPixelDiff::LaunchProcessCallback::RunType>;
    ...
    {
        SkImageInfo info = SkImageInfo::Make(10, 10, SkColorType::kBGRA_8888_SkColorType, SkAlphaType::kPremul_SkAlphaType);
...
protected:
    void SetUp() override
    {
        ...
        auto_reset_custom_launch_process_.emplace(
            SkiaGoldPixelDiff::OverrideLaunchProcessForTesting(base::BindLambdaForTesting(mock_launch_process_->AsStdFunction())));
    }
...
private:
    ...
    std::optional<base::AutoReset<SkiaGoldPixelDiff::LaunchProcessCallback>> auto_reset_custom_launch_process_;
};
...
{
    base::CommandLine::ForCurrentProcess()->AppendSwitch("bypass-skia-gold-functionality");
...
TEST_F(SkiaGoldPixelDiffTest, LuciAuthSwitch)
{
    ...
    EXPECT_CALL(mock_launch_process(), Call(AllOf(Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--luci")))))).Times(1);
    auto* mock_pixel = SkiaGoldPixelDiff::GetSession();
    bool ret = mock_pixel->CompareScreenshot("test", GetTestBitmap());
    EXPECT_TRUE(ret);
}
...
TEST_F(SkiaGoldPixelDiffTest, NoLuciAuthSwitch)
{
    ...
    EXPECT_CALL(mock_launch_process(), Call(_)).Times(AnyNumber());
    EXPECT_CALL(mock_launch_process(), Call(AllOf(Property(&base::CommandLine::GetCommandLineString, Not(HasSubstr(FILE_PATH_LITERAL("--luci"))))))).Times(3);
    auto* mock_pixel = SkiaGoldPixelDiff::GetSession();
    bool ret = mock_pixel->CompareScreenshot("test", GetTestBitmap());
    EXPECT_TRUE(ret);
}
...
TEST_F(SkiaGoldPixelDiffTest, LocalNoLuciAuth)
{
    ...
    base::ScopedEnvironmentVariableOverride env_override("CHROMIUM_TEST_LAUNCHER_BOT_MODE");
...
TEST_F(SkiaGoldPixelDiffTest, FuzzyMatching)
{
    ...
    EXPECT_CALL(mock_launch_process(),
        Call(AllOf(Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--add-test-optional-key=image_matching_algorithm:fuzzy"))),
            Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--add-test-optional-key=fuzzy_max_different_pixels:1"))),
            Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--add-test-optional-key=fuzzy_pixel_delta_threshold:2"))))))
        .Times(1);
    auto* mock_pixel = SkiaGoldPixelDiff::GetSession();
...
TEST_F(SkiaGoldPixelDiffTest, SobelMatching)
{
    EXPECT_CALL(mock_launch_process(), Call(_)).Times(AnyNumber());
    EXPECT_CALL(mock_launch_process(),
        Call(AllOf(Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--add-test-optional-key=image_matching_algorithm:sobel"))),
            Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--add-test-optional-key=fuzzy_max_different_pixels:1"))),
            Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--add-test-optional-key=fuzzy_pixel_delta_threshold:2"))),
            Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--add-test-optional-key=sobel_edge_threshold:3"))),
            Property(&base::CommandLine::GetCommandLineString,
                HasSubstr(FILE_PATH_LITERAL("--add-test-optional-key=fuzzy_"
...
TEST_F(SkiaGoldPixelDiffTest, DefaultCorpus)
{
    EXPECT_CALL(mock_launch_process(), Call(_)).Times(AnyNumber());
    EXPECT_CALL(
        mock_launch_process(), Call(AllOf(Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--corpus=gtest-pixeltests"))))))
        .Times(1);
    auto* mock_pixel = SkiaGoldPixelDiff::GetSession();
    bool ret = mock_pixel->CompareScreenshot("test", GetTestBitmap());
    EXPECT_TRUE(ret);
...
{
    EXPECT_CALL(mock_launch_process(), Call(_)).Times(AnyNumber());
    EXPECT_CALL(mock_launch_process(), Call(AllOf(Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--corpus=corpus"))))))
        .Times(1);
...
TEST_F(SkiaGoldPixelDiffTest, DefaultCodeReviewSystem)
{
    ...
    EXPECT_CALL(mock_launch_process(), Call(_)).Times(AnyNumber());
    EXPECT_CALL(mock_launch_process(), Call(AllOf(Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--crs=gerrit")))))).Times(1);
    auto* mock_pixel = SkiaGoldPixelDiff::GetSession();
    bool ret = mock_pixel->CompareScreenshot("test", GetTestBitmap());
    EXPECT_TRUE(ret);
}
...
TEST_F(SkiaGoldPixelDiffTest, ExplicitCodeReviewSystem)
{
    ...
    EXPECT_CALL(mock_launch_process(), Call(_)).Times(AnyNumber());
    EXPECT_CALL(mock_launch_process(),
        Call(AllOf(Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--crs=new-crs"))),
            Property(&base::CommandLine::GetCommandLineString, Not(HasSubstr(FILE_PATH_LITERAL("gerrit")))))))
        .Times(1);
    auto* mock_pixel = SkiaGoldPixelDiff::GetSession();
    bool ret = mock_pixel->CompareScreenshot("test", GetTestBitmap());
    EXPECT_TRUE(ret);
...
TEST_F(SkiaGoldPixelDiffTest, DryRunLocally)
{
    ...
    EXPECT_CALL(mock_launch_process(), Call(_)).Times(AnyNumber());
    EXPECT_CALL(mock_launch_process(), Call(AllOf(Property(&base::CommandLine::GetCommandLineString, HasSubstr(FILE_PATH_LITERAL("--dryrun")))))).Times(1);
    auto* mock_pixel = SkiaGoldPixelDiff::GetSession();
    bool ret = mock_pixel->CompareScreenshot("test", GetTestBitmap());
    EXPECT_TRUE(ret);
}
...
TEST_F(SkiaGoldPixelDiffTest, NotDryRunOnBots)
{
    ...
    EXPECT_CALL(mock_launch_process(), Call(_)).Times(AnyNumber());
    EXPECT_CALL(mock_launch_process(), Call(AllOf(Property(&base::CommandLine::GetCommandLineString, Not(HasSubstr(FILE_PATH_LITERAL("--dryrun"))))))).Times(3);
    auto* mock_pixel = SkiaGoldPixelDiff::GetSession();
    bool ret = mock_pixel->CompareScreenshot("test", GetTestBitmap());
    EXPECT_TRUE(ret);
}
...
TEST_F(SkiaGoldPixelDiffTest, ReuseSession)
{
    ...
    const TestEnvironmentMap test_environment = { { TestEnvironmentKey::kSystemVersion, "value" } };
    ...
    auto* instance_different_corpus = SkiaGoldPixelDiff::GetSession("different-corpus-for-testing", test_environment);
    EXPECT_NE(instance, instance_different_corpus);

    auto* instance_different_map
        = SkiaGoldPixelDiff::GetSession(corpus, TestEnvironmentMap { { TestEnvironmentKey::kSystemVersion, "different-value-for-testing" } });
    EXPECT_NE(instance, instance_different_map);

    // Check that after some modifications to the session cache, we can still get
    // cache hits.
    EXPECT_EQ(instance, SkiaGoldPixelDiff::GetSession(corpus, test_environment));
    EXPECT_EQ(instance_different_corpus, SkiaGoldPixelDiff::GetSession("different-corpus-for-testing", test_environment));
...
{
    {
        const std::string name = SkiaGoldPixelDiff::GetGoldenImageName("Prefix", "TestName");
        EXPECT_TRUE(name.find("Prefix") != std::string::npos);
        EXPECT_TRUE(name.find("TestName") != std::string::npos);
    }

    {
        const std::string name = SkiaGoldPixelDiff::GetGoldenImageName("Prefix", "TestName", { "Suffix" });
        EXPECT_TRUE(name.find("Prefix") != std::string::npos);
        EXPECT_TRUE(name.find("TestName") != std::string::npos);
...
{
    {
        const std::string name = SkiaGoldPixelDiff::GetGoldenImageName("TestSuiteName", "TestName/ParamValue");
    ...
    {
        const std::string name = SkiaGoldPixelDiff::GetGoldenImageName("ParamInstantiation/TestSuiteName", "TestName/ParamValue");
    ...
    {
        const std::string name = SkiaGoldPixelDiff::GetGoldenImageName("ParamInstantiation/TestSuiteName", "TestName/ParamValue", { "Suffix" });
        EXPECT_TRUE(name.find("ParamInstantiation") != std::string::npos);
        EXPECT_TRUE(name.find("TestSuiteName") != std::string::npos);
        EXPECT_TRUE(name.find("TestName") != std::string::npos);
        EXPECT_TRUE(name.find("ParamValue") != std::string::npos);
...
TEST_F(SkiaGoldPixelDiffTest, MustBeUsedSequentially)
{
    ...
    thread.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
        ...
        EXPECT_DCHECK_DEATH_WITH(mock_pixel2->CompareScreenshot("test", GetTestBitmap()), "CalledOnValidSequence");
    }));
...
TEST_F(SkiaGoldPixelDiffTest, GoldenImageNameCannotHaveIllegalCharacters)
{
    ...
    EXPECT_DCHECK_DEATH_WITH(SkiaGoldPixelDiff::GetSession()->CompareScreenshot("image_name_with space", GetTestBitmap()),
        "a golden image name should not contain any space or back slash");

    EXPECT_DCHECK_DEATH_WITH(SkiaGoldPixelDiff::GetSession()->CompareScreenshot("image_name_with/slash", GetTestBitmap()),
        "a golden image name should not contain any space or back slash");
}
#endif
