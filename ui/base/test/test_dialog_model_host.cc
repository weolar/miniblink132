...
DialogModel::Button* GetButton(DialogModel* dialog_model, TestDialogModelHost::ButtonId button_id, base::PassKey<DialogModelHost> pass_key)
{
    switch (button_id) {
...
TestDialogModelHost::TestDialogModelHost(std::unique_ptr<DialogModel> dialog_model)
...
void TestDialogModelHost::DestroyWithoutAction(std::unique_ptr<TestDialogModelHost> host)
{
...
{
    dialog_model_->extra_button(DialogModelHost::GetPassKey())->OnPressed(DialogModelHost::GetPassKey(), event);
}
...
{
    FindSingleTextfield()->OnTextChanged(DialogModelFieldHost::GetPassKey(), std::move(text));
}

// Bypasses PassKey() requirement for accessing accelerators().
const base::flat_set<Accelerator>& TestDialogModelHost::GetAccelerators(ButtonId button_id)
{
    return GetButton(dialog_model_.get(), button_id, DialogModelHost::GetPassKey())->accelerators();
}

const std::u16string& TestDialogModelHost::GetLabel(ButtonId button_id)
{
    return GetButton(dialog_model_.get(), button_id, DialogModelHost::GetPassKey())->label();
}

ElementIdentifier TestDialogModelHost::GetId(ButtonId button_id)
{
    return GetButton(dialog_model_.get(), button_id, DialogModelHost::GetPassKey())->id();
}
