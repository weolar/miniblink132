...
public:
    ...
    virtual bool SendKeyEvents(gfx::NativeWindow window, ui::KeyboardCode key, int key_event_types, int accerelator_state) = 0;
    virtual bool SendKeyEventsNotifyWhenDone(
        gfx::NativeWindow window, ui::KeyboardCode key, int key_event_types, base::OnceClosure task, int accelerator_state, KeyEventType wait_for)
        = 0;

    // Simulate a mouse move. (x,y) are absolute screen coordinates.
    virtual bool SendMouseMove(int x, int y) = 0;
    virtual bool SendMouseMoveNotifyWhenDone(int x, int y, base::OnceClosure task) = 0;
    ...
    virtual bool SendMouseEvents(MouseButton type, int button_state, int accelerator_state) = 0;
    virtual bool SendMouseEventsNotifyWhenDone(MouseButton type, int button_state, base::OnceClosure task, int accelerator_state) = 0;
    ...
    virtual bool SendTouchEventsNotifyWhenDone(int action, int id, int x, int y, base::OnceClosure task) = 0;
#endif
};
