...
template <typename T> class ScopedGObject {
public:
    ...
    ScopedGObject(ScopedGObject<T>&& other)
        : obj_(other.obj_)
    {
        other.obj_ = nullptr;
    }
...
private:
    template <typename U> friend ScopedGObject<U> TakeGObject(U* obj);
    template <typename U> friend ScopedGObject<U> WrapGObject(U* obj);

    explicit ScopedGObject(T* obj)
        : obj_(obj)
    {
    }
...
template <typename T> ScopedGObject<T> TakeGObject(T* obj)
{
...
template <typename T> ScopedGObject<T> WrapGObject(T* obj)
{
    ScopedGObject<T> scoped(obj);
    scoped.Ref();
    return scoped;
