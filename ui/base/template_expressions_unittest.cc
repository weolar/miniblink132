...
TEST(TemplateExpressionsTest, ReplaceTemplateExpressionsPieces)
{
    ...
    EXPECT_EQ("multiple: word, number.", ReplaceTemplateExpressions("multiple: $i18n{test}, $i18n{5}.", substitutions));
}

TEST(TemplateExpressionsTest, ReplaceTemplateExpressionsConsecutiveDollarSignsPieces)
{
    TemplateReplacements substitutions;
    substitutions["a"] = "x";
    EXPECT_EQ("$ $$ $$$", ReplaceTemplateExpressions("$ $$ $$$", substitutions));
...
TEST(TemplateExpressionsTest, ReplaceTemplateExpressionsEscaping)
{
    ...
    EXPECT_EQ("a&quot;b&#39;c&lt;d&gt;e&amp;f", ReplaceTemplateExpressions("$i18n{punctuationSample}", substitutions));
    EXPECT_EQ("&lt;div&gt;hello&lt;/div&gt;", ReplaceTemplateExpressions("$i18n{htmlSample}", substitutions));
    EXPECT_EQ("multiple: &lt;div&gt;hello&lt;/div&gt;, a&quot;b&#39;c&lt;d&gt;e&amp;f.",
        ReplaceTemplateExpressions("multiple: $i18n{htmlSample}, $i18n{punctuationSample}.", substitutions));
}
...
TEST(TemplateExpressionsTest, ReplaceTemplateExpressionsRaw)
{
    ...
    EXPECT_EQ("<a href=\"example.com\">hello</a>", ReplaceTemplateExpressions("$i18nRaw{rawSample}", substitutions));
}
...
TEST(TemplateExpressionsTest, ReplaceTemplateExpressionsSkipPlaceholderCheck)
{
    ...
    EXPECT_DCHECK_DEATH(ReplaceTemplateExpressions("$i18nRaw{rawSample}", substitutions,
...
TEST(TemplateExpressionsTest, ReplaceTemplateExpressionsPolymerQuoting)
{
    ...
    EXPECT_EQ("<div>[[Call('don\\'t do it')]]", ReplaceTemplateExpressions("<div>[[Call('$i18nPolymer{singleSample}')]]", substitutions));
    // This resolves |Call('\"moo\" said the cow')| to Polymer, which is
    // presented as |"moo" said the cow| to the user.
    EXPECT_EQ("<div>[[Call('&quot;moo&quot; said the cow')]]", ReplaceTemplateExpressions("<div>[[Call('$i18nPolymer{doubleSample}')]]", substitutions));
}
...
TEST(TemplateExpressionsTest, ReplaceTemplateExpressionsPolymerMixed)
{
    ...
    substitutions["htmlSample"] = "<div>hello</div>";
    EXPECT_EQ(R"(a&quot;b\'c<d>e&f\,g)", ReplaceTemplateExpressions("$i18nPolymer{punctuationSample}", substitutions));
    EXPECT_EQ("<div>hello</div>", ReplaceTemplateExpressions("$i18nPolymer{htmlSample}", substitutions));
...
TEST(TemplateExpressionsTest, JSNoReplacementOutsideTemplate)
{
    ...
    const TestCase kTestCases[] = { // No substitutions should occur in normal JS code.
    ...
    for (const TestCase test_case : kTestCases) {
        ASSERT_TRUE(ReplaceTemplateExpressionsInJS(test_case.js_in, substitutions, &formatted));
...
{
    TemplateReplacements substitutions;
    substitutions["backtickSample"] = "`, attached: function() { alert(1); },_template: html`";
    substitutions["dollarSignSample"] = "5$";
    substitutions["punctuationSample"] = "a\"b'c<d>e&f";
    substitutions["htmlSample"] = "<div>hello</div>";

    const TestCase kTestCases[] = { // Substitution with a backtick in the replacement.
        { "Polymer({\n"
          "  _template: html`\n"
          "<!--_html_template_start_-->\n"
          "    <span>\n"
...
TEST(TemplateExpressionsTest, JSReplacementsError)
{
    ...
    std::string formatted;
    for (const TestCase test_case : kTestCases) {
        ASSERT_FALSE(ReplaceTemplateExpressionsInJS(test_case.js_in, substitutions, &formatted));
        formatted.clear();
    }
}
...
TEST(TemplateExpressionsTest, JSMultipleTemplates)
{
    ...
    const TestCase kTestCases[] = { // Only the second template has substitutions
        { "Polymer({\n"
          "  _template: html`<!--_html_template_start_-->\n"
          "    <div>Hello</div>\n"
          "<!--_html_template_end_-->`,\n"
