...
template <typename T> struct ClassProperty {
    T default_value;
    const char* name;
...
public:
    ...
    // version below.
    template <typename T> void SetProperty(const ClassProperty<T>* property, T value);
    ...
    template <typename T> void SetProperty(const ClassProperty<T*>* property, const T& value);
    ...
    template <typename T> T* SetProperty(const ClassProperty<T*>* property, std::unique_ptr<T> value);
    ...
    // contents.
    template <typename T> T GetProperty(const ClassProperty<T>* property) const;

    // Sets the |property| to its default value. Useful for avoiding a cast when
    // setting to NULL.
    template <typename T> void ClearProperty(const ClassProperty<T>* property);
...
protected:
    ...
    int64_t SetPropertyInternal(const void* key, const char* name, PropertyDeallocator deallocator, int64_t value, int64_t default_value);
    ...
    // notification.
    int64_t GetPropertyInternal(const void* key, int64_t default_value, bool search_parent) const;
...
template <typename T> class ClassPropertyCaster {
public:
    static int64_t ToInt64(T x)
    {
        return static_cast<int64_t>(x);
    }
    static T FromInt64(int64_t x)
    {
        return static_cast<T>(x);
    }
};
template <typename T> class ClassPropertyCaster<T*> {
public:
    static int64_t ToInt64(T* x)
    {
...
template <> class ClassPropertyCaster<base::TimeDelta> {
public:
    static int64_t ToInt64(base::TimeDelta x)
    {
        return x.InMicroseconds();
    }
    static base::TimeDelta FromInt64(int64_t x)
    {
        return base::Microseconds(x);
    }
};
template <> class ClassPropertyCaster<float> {
public:
    static int64_t ToInt64(float x)
    {
        static_assert(sizeof(float) <= sizeof(int64_t), "expected float size <= 8 bytes");
        int64_t ret = 0;
...
public:
    template <typename T> static void Set(::ui::PropertyHandler* handler, const ::ui::ClassProperty<T>* property, T value)
    {
        int64_t old = handler->SetPropertyInternal(property, property->name, value == property->default_value ? nullptr : property->deallocator,
            ClassPropertyCaster<T>::ToInt64(value), ClassPropertyCaster<T>::ToInt64(property->default_value));
        if (property->deallocator && old != ClassPropertyCaster<T>::ToInt64(property->default_value)) {
            (*property->deallocator)(old);
        }
    }
    template <typename T> static T Get(const ::ui::PropertyHandler* handler, const ::ui::ClassProperty<T>* property, bool allow_cascade)
    {
        return ClassPropertyCaster<T>::FromInt64(
            handler->GetPropertyInternal(property, ClassPropertyCaster<T>::ToInt64(property->default_value), property->cascading && allow_cascade));
    }
    template <typename T> static void Clear(::ui::PropertyHandler* handler, const ::ui::ClassProperty<T>* property)
    {
...
template <typename T> void PropertyHandler::SetProperty(const ClassProperty<T*>* property, const T& value)
{
...
template <typename T> void PropertyHandler::SetProperty(const ClassProperty<T*>* property, T&& value)
{
    // Prevent additional heap allocation if possible.
    T* const old = subtle::PropertyHelper::Get<T*>(this, property, false);
    if (old) {
...
template <typename T> T* PropertyHandler::SetProperty(const ClassProperty<T*>* property, std::unique_ptr<T> value)
{
...
#define DECLARE_EXPORTED_UI_CLASS_PROPERTY_TYPE(EXPORT, T)                                                                                                     \
    namespace ui {                                                                                                                                             \
    template <> EXPORT void PropertyHandler::SetProperty(const ClassProperty<T>* property, T value);                                                           \
    template <> EXPORT T PropertyHandler::GetProperty(const ClassProperty<T>* property) const;                                                                 \
    template <> EXPORT void PropertyHandler::ClearProperty(const ClassProperty<T>* property);                                                                  \
    } // namespace ui
...
#define DEFINE_UI_CLASS_PROPERTY_TYPE(T) DEFINE_EXPORTED_UI_CLASS_PROPERTY_TYPE(, T)

#define DEFINE_UI_CLASS_PROPERTY_KEY(TYPE, NAME, DEFAULT)                                                                                                      \
    static_assert(sizeof(TYPE) <= sizeof(int64_t), "property type too large");                                                                                 \
    const ::ui::ClassProperty<TYPE> NAME##_Value = { DEFAULT, #NAME, false, nullptr };                                                                         \
...
#define DEFINE_OWNED_UI_CLASS_PROPERTY_KEY(TYPE, NAME, DEFAULT)                                                                                                \
    ...
    const ::ui::ClassProperty<TYPE*> NAME##_Value = { DEFAULT, #NAME, false, &Deallocator##NAME };                                                             \
    } /* namespace */                                                                                                                                          \
    const ::ui::ClassProperty<TYPE*>* const NAME = &NAME##_Value;

#define DEFINE_CASCADING_UI_CLASS_PROPERTY_KEY(TYPE, NAME, DEFAULT)                                                                                            \
    static_assert(sizeof(TYPE) <= sizeof(int64_t), "property type too large");                                                                                 \
    const ::ui::ClassProperty<TYPE> NAME##_Value = { DEFAULT, #NAME, true, nullptr };                                                                          \
    const ::ui::ClassProperty<TYPE>* const NAME = &NAME##_Value;
...
#define DEFINE_CASCADING_OWNED_UI_CLASS_PROPERTY_KEY(TYPE, NAME, DEFAULT)                                                                                      \
    ...
    const ::ui::ClassProperty<TYPE*> NAME##_Value = { DEFAULT, #NAME, true, &Deallocator##NAME };                                                              \
    } /* namespace */                                                                                                                                          \
    const ::ui::ClassProperty<TYPE*>* const NAME = &NAME##_Value;
