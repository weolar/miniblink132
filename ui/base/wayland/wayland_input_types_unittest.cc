...
TEST(WaylandInputTypesTest, TextInputType)
{
    ...
    for (int i = 0; i <= TEXT_INPUT_TYPE_MAX; ++i) {
        ...
        EXPECT_EQ(input_type, ConvertToTextInputType(ConvertFromTextInputType(input_type)));
    }

    // Passing the invalid value returns nullopt.
    auto kInvalidValue = static_cast<zcr_extended_text_input_v1_input_type>(0xFFFFFFFF);
...
TEST(WaylandInputTypesTest, TextInputMode)
{
    ...
    for (int i = 0; i <= TEXT_INPUT_MODE_MAX; ++i) {
        ...
        EXPECT_EQ(input_mode, ConvertToTextInputMode(ConvertFromTextInputMode(input_mode)));
    }

    // Passing the invalid value returns nullopt.
    auto kInvalidValue = static_cast<zcr_extended_text_input_v1_input_mode>(0xFFFFFFFF);
