...
#define MAP_TYPES(macro)                                                                                                                                       \
    macro(NONE) macro(TEXT) macro(PASSWORD) macro(SEARCH) macro(EMAIL) macro(NUMBER) macro(TELEPHONE) macro(URL) macro(DATE) macro(DATE_TIME)                  \
        macro(DATE_TIME_LOCAL) macro(MONTH) macro(TIME) macro(WEEK) macro(TEXT_AREA) macro(CONTENT_EDITABLE) macro(DATE_TIME_FIELD) macro(NULL)

// Similar to MAP_TYPES, list all TextInputMode. See above how to update.
#define MAP_MODES(macro) macro(DEFAULT) macro(NONE) macro(TEXT) macro(TEL) macro(URL) macro(EMAIL) macro(NUMERIC) macro(DECIMAL) macro(SEARCH)

// Similar to MAP_TYPES, list all TextInputFlags. See above how to update.
#define MAP_FLAGS(macro)                                                                                                                                       \
    macro(NONE) macro(AUTOCOMPLETE_ON) macro(AUTOCOMPLETE_OFF) macro(AUTOCORRECT_ON) macro(AUTOCORRECT_OFF) macro(SPELLCHECK_ON) macro(SPELLCHECK_OFF)         \
        macro(AUTOCAPITALIZE_NONE) macro(AUTOCAPITALIZE_CHARACTERS) macro(AUTOCAPITALIZE_WORDS) macro(AUTOCAPITALIZE_SENTENCES) macro(HAS_BEEN_PASSWORD)       \
            macro(VERTICAL)
