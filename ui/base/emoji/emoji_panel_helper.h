...
using EmojiKeyboardCallback = base::RepeatingCallback<void(EmojiPickerCategory, EmojiPickerFocusBehavior, const std::string& initial_query)>;

// Show the emoji picker pre scrolled to a specific category
COMPONENT_EXPORT(UI_BASE_EMOJI)
void ShowEmojiPanelInSpecificMode(EmojiPickerCategory category, EmojiPickerFocusBehavior focus_behavior, const std::string& initial_query);
