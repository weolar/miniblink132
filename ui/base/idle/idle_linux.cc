...
private:
    ...
    void TryCurrentService()
    {
        ...
        if (proxy_) {
            task_runner_->PostTask(FROM_HERE, base::BindOnce(&dbus::ObjectProxy::Detach, base::Unretained(proxy_)));
            proxy_ = nullptr;
        }
        ...
        if (current_service_ >= kServiceCount) {
            if (current_service_ == kServiceCount) {
                ...
                LOG(WARNING) << "None of the known D-Bus ScreenSaver services could be used.";
                ++current_service_;
            }
            return;
        }
        ...
        dbus_proxy->CallMethod(&name_has_owner_call, dbus::ObjectProxy::TIMEOUT_USE_DEFAULT,
            base::BindOnce(&DBusScreenSaverWatcher::OnServiceHasOwner, weak_factory_.GetWeakPtr()));
    }
    ...
    void OnServiceHasOwner(dbus::Response* response)
    {
        ...
        if (!response || !reader.PopBool(&owned) || !owned) {
            VLOG(1) << kServices[current_service_].service_name << " D-Bus service does not exist";
            ++current_service_;
            return TryCurrentService();
        }

        // Now connect the ActiveChanged signal.
        proxy_ = bus_->GetObjectProxy(kServices[current_service_].service_name, dbus::ObjectPath(kServices[current_service_].object_path));
        proxy_->ConnectToSignal(kServices[current_service_].interface, kSignalName,
            base::BindRepeating(&DBusScreenSaverWatcher::OnActiveChanged, weak_factory_.GetWeakPtr()),
            base::BindOnce(&DBusScreenSaverWatcher::OnActiveChangedSignalConnected, weak_factory_.GetWeakPtr()));
    }

    void OnActiveChangedSignalConnected(const std::string& interface, const std::string& signal, bool succeeded)
    {
        DCHECK_LT(current_service_, kServiceCount);
        ...
        if (!succeeded) {
            VLOG(1) << "Cannot connect to " << kSignalName << " signal of " << interface << " D-Bus service";
            ++current_service_;
            return TryCurrentService();
        }
        ...
        dbus::MethodCall method_call(kServices[current_service_].interface, kMethodName);
        proxy_->CallMethod(
            &method_call, dbus::ObjectProxy::TIMEOUT_USE_DEFAULT, base::BindOnce(&DBusScreenSaverWatcher::OnGetActive, weak_factory_.GetWeakPtr()));
    }
    ...
    void OnGetActive(dbus::Response* response)
    {
        ...
        if (!response || !UpdateLockState(response)) {
            VLOG(1) << "Call to " << kMethodName << " method of " << kServices[current_service_].interface << " D-Bus service failed";
            ++current_service_;
            return TryCurrentService();
        }
    }

    void OnActiveChanged(dbus::Signal* signal)
    {
        UpdateLockState(signal);
    }
