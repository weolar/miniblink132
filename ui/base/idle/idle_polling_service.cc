...
void IdlePollingService::AddObserver(Observer* observer)
{
    ...
    if (observers_.empty()) {
        DCHECK(!timer_.IsRunning());
        PollIdleState();
        timer_.Start(FROM_HERE, poll_interval_, base::BindRepeating(&IdlePollingService::PollIdleState, base::Unretained(this)));
    }
...
void IdlePollingService::SetProviderForTest(std::unique_ptr<IdleTimeProvider> provider)
{
...
void IdlePollingService::SetTaskRunnerForTest(scoped_refptr<base::SequencedTaskRunner> task_runner)
{
