...
public:
    ...
    bool float_property_changed() const
    {
        return float_property_changed_;
    }
    void OnFloatPropertyChanged()
    {
        float_property_changed_ = true;
    }

protected:
    template <typename T> UM::MemberMetaDataBase* GetMemberMetaData(T* obj, const std::string& member_name)
    {
        UM::ClassMetaData* meta_data = obj->GetClassMetaData();
        if (meta_data == nullptr)
            return nullptr;

        UM::MemberMetaDataBase* member_data = meta_data->FindMemberData(member_name);
        return member_data;
    }
...
class MetadataTestBaseClass : public ui::metadata::MetaDataProvider, public ui::PropertyHandler {
public:
    ...
    [[nodiscard]] base::CallbackListSubscription AddIntPropertyChangedCallback(ui::metadata::PropertyChangedCallback callback)
    {
...
public:
    ...
    [[nodiscard]] base::CallbackListSubscription AddFloatPropertyChangedCallback(ui::metadata::PropertyChangedCallback callback)
    {
...
TEST_F(MetadataTest, TestFloatMetadataPropertyAccess)
{
    ...
    UM::MemberMetaDataBase* member_data = GetMemberMetaData(&test_obj, "FloatProperty");
...
TEST_F(MetadataTest, TestFloatPropertyChangedCallback)
{
    ...
    base::CallbackListSubscription callback
        = test_obj.AddFloatPropertyChangedCallback(base::BindRepeating(&MetadataTest::OnFloatPropertyChanged, base::Unretained(this)));
...
TEST_F(MetadataTest, TestMetaDataParentClassTracking)
{
    ...
    EXPECT_EQ(base_class_meta_data, derived_class_meta_data->parent_class_meta_data());
}
...
TEST_F(MetadataTest, TestMetaDataFindParentClassMember)
{
    ...
    UM::MemberMetaDataBase* member_data = derived_class_meta_data->FindMemberData("IntProperty");
...
TEST_F(MetadataTest, TestClassPropertyMetaData)
{
    ...
    std::map<std::string, std::u16string> expected_kv = { { "kIntKey", u"-1" }, { "kOwnedInsetsKey1", u"(not assigned)" },
        { "kOwnedInsetsKey2", u"(not assigned)" }, { "kInsetsKey1", u"(not assigned)" }, { "kInsetsKey2", u"(not assigned)" } };
    ...
    expected_kv = { { "kIntKey", u"1" }, { "kOwnedInsetsKey1", u"8,8,8,8" }, { "kOwnedInsetsKey2", u"(assigned)" }, { "kInsetsKey1", u"8,8,8,8" },
