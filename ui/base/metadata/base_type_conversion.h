...
template <typename T>
using ArgType = typename std::conditional<std::is_fundamental<T>::value || std::is_enum<T>::value || std::is_pointer<T>::value
        || (std::is_move_assignable<T>::value && std::is_move_constructible<T>::value && !std::is_copy_assignable<T>::value
...
template <bool serializable, bool read_only = false, const char* name_prefix = kNoPrefix> struct BaseTypeConverter {
    ...
    static const char* PropertyNamePrefix()
    {
        return name_prefix;
    }
};

template <typename T> struct TypeConverter : BaseTypeConverter<std::is_enum<T>::value> {
...
template <typename T, typename K> struct Uniquifier {
    using type = T;
    using tag = K;
};
...
#define MAKE_TYPE_UNIQUE(type_name)                                                                                                                            \
    ...
    using type_name##Unique = ::ui::metadata::Uniquifier<type_name, type_name##Tag>
...
template <typename T> struct EnumStringsMap;
...
#define EXPORT_ENUM_CONVERTERS(T, EXPORT)                                                                                                                      \
    template <> EXPORT std::u16string ui::metadata::TypeConverter<T>::ToString(ui::metadata::ArgType<T> source_value);                                         \
                                                                                                                                                               \
    template <> EXPORT std::optional<T> ui::metadata::TypeConverter<T>::FromString(const std::u16string& str);                                                 \
                                                                                                                                                               \
    template <> EXPORT ui::metadata::ValidStrings ui::metadata::TypeConverter<T>::GetValidStrings();
...
#define DEFINE_ENUM_CONVERTERS(T, ...)                                                                                                                         \
    template <> struct ui::metadata::EnumStringsMap<T> {                                                                                                       \
        static_assert(std::is_enum<T>::value, "Error: " #T " is not an enum.");                                                                                \
                                                                                                                                                               \
        static const auto& Get()                                                                                                                               \
        {                                                                                                                                                      \
            static constexpr auto kMap = base::MakeFixedFlatMap<T, std::u16string_view>({ __VA_ARGS__ });                                                      \
            return kMap;                                                                                                                                       \
        }                                                                                                                                                      \
    };                                                                                                                                                         \
                                                                                                                                                               \
    template <> std::u16string ui::metadata::TypeConverter<T>::ToString(ui::metadata::ArgType<T> source_value)                                                 \
    {                                                                                                                                                          \
        ...
        return it != map.end() ? std::u16string(it->second) : std::u16string();                                                                                \
                                                                                                                                                               ...
                                                                                                                                                               \
    template <> std::optional<T> ui::metadata::TypeConverter<T>::FromString(const std::u16string& str)                                                         \
    {                                                                                                                                                          \
        const auto& map = EnumStringsMap<T>::Get();                                                                                                            \
        ...
        return it != map.end() ? std::make_optional(it->first) : std::nullopt;                                                                                 \
                                                                                                                                                               ...
                                                                                                                                                               \
    template <> ui::metadata::ValidStrings ui::metadata::TypeConverter<T>::GetValidStrings()                                                                   \
    {                                                                                                                                                          \
        ValidStrings string_values;                                                                                                                            \
        base::ranges::transform(EnumStringsMap<T>::Get(), std::back_inserter(string_values), [](const auto& pair) { return std::u16string(pair.second); });    \
...
#define DECLARE_CONVERSIONS(T)                                                                                                                                 \
    template <> struct COMPONENT_EXPORT(UI_BASE_METADATA) TypeConverter<T> : BaseTypeConverter<true> {                                                         \
        static std::u16string ToString(ArgType<T> source_value);                                                                                               \
        static std::optional<T> FromString(const std::u16string& source_value);                                                                                \
        static ValidStrings GetValidStrings()                                                                                                                  \
        {                                                                                                                                                      \
...
template <> struct COMPONENT_EXPORT(UI_BASE_METADATA) TypeConverter<bool> : BaseTypeConverter<true> {
...
template <typename T> struct TypeConverter<std::optional<T>> : BaseTypeConverter<TypeConverter<T>::is_serializable> {
    ...
    static std::optional<std::optional<T>> FromString(const std::u16string& source_value)
    {
...
template <typename T> struct TypeConverter<std::unique_ptr<T>> : BaseTypeConverter<false, true> {
    ...
    static std::optional<std::unique_ptr<T>> FromString(const std::u16string& source_value)
    {
...
template <typename T> struct TypeConverter<T*> : BaseTypeConverter<false, true> {
    static std::u16string ToString(ArgType<T*> source_value)
    {
        return PointerToString(source_value);
    }
...
template <typename T> struct TypeConverter<std::vector<T>> : BaseTypeConverter<TypeConverter<T>::is_serializable> {
    static std::u16string ToString(ArgType<std::vector<T>> source_value)
    {
        std::vector<std::u16string> serialized;
        base::ranges::transform(source_value, std::back_inserter(serialized), &TypeConverter<T>::ToString);
        return u"{" + base::JoinString(serialized, u",") + u"}";
    }
    static std::optional<std::vector<T>> FromString(const std::u16string& source_value)
    {
        if (source_value.empty() || source_value.front() != u'{' || source_value.back() != u'}')
            return std::nullopt;
        const auto values = base::SplitString(source_value.substr(1, source_value.length() - 2), u",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
        std::vector<T> output;
...
template <> struct COMPONENT_EXPORT(UI_BASE_METADATA) TypeConverter<UNIQUE_TYPE_NAME(SkColor)> : BaseTypeConverter<true, false, kSkColorPrefix> {
    ...
    static bool GetNextColor(std::u16string::const_iterator start, std::u16string::const_iterator end, std::u16string& color);
    ...
    static std::optional<SkColor> GetNextColor(std::u16string::const_iterator start, std::u16string::const_iterator end);
    ...
    static std::optional<SkColor> RgbaPiecesToSkColor(const std::vector<std::u16string_view>& pieces, size_t start_piece);

private:
    static std::optional<SkColor> ParseHexString(const std::u16string& hex_string);
    static std::optional<SkColor> ParseHslString(const std::u16string& hsl_string);
    static std::optional<SkColor> ParseRgbString(const std::u16string& rgb_string);
};
...
EXPORT_ENUM_CONVERTERS(gfx::HorizontalAlignment, COMPONENT_EXPORT(UI_BASE_METADATA))
