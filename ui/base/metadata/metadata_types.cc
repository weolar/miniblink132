...
{
    return static_cast<PropertyFlags>(static_cast<uint32_t>(op1) | static_cast<uint32_t>(op2));
}
...
base::CallbackListSubscription MetaDataProvider::AddPropertyChangedCallback(PropertyKey property, PropertyChangedCallback callback)
{
    auto entry = property_changed_vectors_.find(property);
    if (entry == property_changed_vectors_.end()) {
        entry = property_changed_vectors_.emplace(property, std::make_unique<PropertyChangedCallbacks>()).first;
    }
...
void ClassMetaData::AddMemberData(std::unique_ptr<MemberMetaDataBase> member_data)
{
    members_.push_back(member_data.release());
}

MemberMetaDataBase* ClassMetaData::FindMemberData(const std::string& member_name)
{
...
/** Member Iterator */
ClassMetaData::ClassMemberIterator::ClassMemberIterator(const ClassMetaData::ClassMemberIterator& other)
{
...
ClassMetaData::ClassMemberIterator::ClassMemberIterator(ClassMetaData* starting_container)
{
...
bool ClassMetaData::ClassMemberIterator::operator==(const ClassMemberIterator& rhs) const
{
    return current_vector_index_ == rhs.current_vector_index_ && current_collection_ == rhs.current_collection_;
}

ClassMetaData::ClassMemberIterator& ClassMetaData::ClassMemberIterator::operator++()
{
    IncrementHelper();
    return *this;
}

ClassMetaData::ClassMemberIterator ClassMetaData::ClassMemberIterator::operator++(int)
{
...
std::string ClassMetaData::ClassMemberIterator::GetCurrentCollectionName() const
{
...
void MemberMetaDataBase::SetValueAsString(void* obj, const std::u16string& new_value)
{
    NOTREACHED();
}
