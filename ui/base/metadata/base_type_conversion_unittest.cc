...
{
    int from_int = 5;
    std::u16string to_string = ui::metadata::TypeConverter<int>::ToString(from_int);
...
TEST_F(TypeConversionTest, TestConversion_BogusStringToInt)
{
    ...
    EXPECT_EQ(ui::metadata::TypeConverter<int>::FromString(from_string), std::nullopt);
}
...
TEST_F(TypeConversionTest, TestConversion_OptionalIntToString)
{
    ...
    std::u16string to_string = ui::metadata::TypeConverter<std::optional<int>>::ToString(src);
...
TEST_F(TypeConversionTest, TestConversion_StringToOptionalInt)
{
    ...
    EXPECT_EQ(ui::metadata::TypeConverter<std::optional<int>>::FromString(ui::metadata::GetNullOptStr()), std::make_optional(ret));

    EXPECT_EQ(ui::metadata::TypeConverter<std::optional<int>>::FromString(u"10"), 10);

    EXPECT_EQ(ui::metadata::TypeConverter<std::optional<int>>::FromString(u"ab0"), std::nullopt);
}

TEST_F(TypeConversionTest, TestConversion_ShadowValuesToString)
{
    gfx::ShadowValues shadow_values;
    shadow_values.emplace_back(gfx::Vector2d(1, 2), .3, SkColorSetARGB(128, 255, 0, 0));

    EXPECT_EQ(ui::metadata::TypeConverter<gfx::ShadowValues>::ToString(shadow_values), u"[ (1,2),0.30,rgba(255,0,0,128) ]");

    shadow_values.emplace_back(gfx::Vector2d(9, 8), .76, SkColorSetARGB(20, 0, 64, 255));

    EXPECT_EQ(ui::metadata::TypeConverter<gfx::ShadowValues>::ToString(shadow_values), u"[ (1,2),0.30,rgba(255,0,0,128); (9,8),0.76,rgba(0,64,255,20) ]");
}

TEST_F(TypeConversionTest, TestConversion_StringToShadowValues)
{
    std::optional<gfx::ShadowValues> opt_result
        = ui::metadata::TypeConverter<gfx::ShadowValues>::FromString(u"[ (6,4),0.53,rgba(23,44,0,1); (93,83),4.33,rgba(10,20,0,0.059) ]");
...
TEST_F(TypeConversionTest, TestConversion_SkColorConversions)
{
    ...
    std::optional<SkColor> result = ui::metadata::SkColorConverter::FromString(u"0x112233");
    EXPECT_TRUE(result);
    ...
    result = ui::metadata::SkColorConverter::FromString(u"rgba(0, 128, 192, 0.5)");
    EXPECT_TRUE(result);
    ...
    result = ui::metadata::SkColorConverter::FromString(u"hsl(195, 100%, 50%, 0.5)");
    EXPECT_TRUE(result);
...
TEST_F(TypeConversionTest, TestConversion_ColorParserTest)
{
    ...
    std::u16string color;
    const std::u16string source = u"rgb(0, 128, 192), hsl(90, 100%, 30%), rgba(128, 128, 128, 0.5), "
                                  u"hsla(240, 100%, 50%, 0.5)";
    auto start_pos = source.cbegin();
    EXPECT_TRUE(converter::GetNextColor(start_pos, source.cend(), color, start_pos));
...
TEST_F(TypeConversionTest, TestConversion_InsetsToString)
{
    ...
    std::u16string to_string = ui::metadata::TypeConverter<gfx::Insets>::ToString(kInsets);
...
TEST_F(TypeConversionTest, TestConversion_StringToInsets)
{
    ...
    EXPECT_EQ(ui::metadata::TypeConverter<gfx::Insets>::FromString(from_string), gfx::Insets::TLBR(2, 3, 4, 5));
}
...
TEST_F(TypeConversionTest, TestConversion_VectorToString)
{
    ...
    std::u16string to_string = ui::metadata::TypeConverter<std::vector<int>>::ToString(kVector);
...
{
    std::u16string from_string = u"{2,3,4,5}";
    EXPECT_EQ(ui::metadata::TypeConverter<std::vector<int>>::FromString(from_string), std::vector<int>({ 2, 3, 4, 5 }));
}
...
TEST_F(TypeConversionTest, CheckIsSerializable)
{
    ...
    EXPECT_TRUE(ui::metadata::TypeConverter<std::optional<const char*>>::IsSerializable());
    EXPECT_TRUE(ui::metadata::TypeConverter<std::optional<int>>::IsSerializable());
}
