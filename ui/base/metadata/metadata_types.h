...
static inline PropertyKey MakeUniquePropertyKey(PropertyKey base, uintptr_t offset)
{
    return reinterpret_cast<PropertyKey>(reinterpret_cast<uintptr_t>(base) + offset);
}
...
protected:
    [[nodiscard]] base::CallbackListSubscription AddPropertyChangedCallback(PropertyKey property, PropertyChangedCallback callback);
    void TriggerChangedCallback(PropertyKey property);

private:
    using PropertyChangedVectors = std::map<PropertyKey, std::unique_ptr<PropertyChangedCallbacks>>;
...
public:
    ...
    {
        static_assert(std::is_same<decltype(type_name_), std::string_view>::value,
    ...
    const std::vector<raw_ptr<MemberMetaDataBase, VectorExperimental>>& members() const
    {
        return members_;
    }
    ...
    public:
        ...
        {
            if (current_collection_ == nullptr || current_vector_index_ >= current_collection_->members().size())
                return nullptr;
...
public:
    ...
    MemberMetaDataBase(const std::string& member_name, const std::string& member_type)
