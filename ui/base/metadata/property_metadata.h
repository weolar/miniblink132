...
template <typename TKey, typename TValue> struct ClassPropertyMetaDataTypeHelper;

template <typename TKValue_, typename TValue_>
    requires(std::same_as<TKValue_, TValue_> || std::same_as<TKValue_, TValue_*>)
struct ClassPropertyMetaDataTypeHelper<const ui::ClassProperty<TKValue_>* const, TValue_> {
...
template <typename TClass> TClass* AsClass(void* obj)
{
    return static_cast<TClass*>(reinterpret_cast<TClass*>(obj)->ReinterpretToBaseClass(obj));
}
...
template <typename TClass, typename TValue, typename TRet, TRet (TClass::*Get)() const, typename TConverter = ui::metadata::TypeConverter<TValue>>
class ObjectPropertyReadOnlyMetaData : public ui::metadata::MemberMetaDataBase {
public:
    using MemberMetaDataBase::MemberMetaDataBase;
    ObjectPropertyReadOnlyMetaData(const ObjectPropertyReadOnlyMetaData&) = delete;
    ObjectPropertyReadOnlyMetaData& operator=(const ObjectPropertyReadOnlyMetaData&) = delete;
    ...
    ui::metadata::PropertyFlags GetPropertyFlags() const override
    {
        if constexpr (kTypeIsSerializable) {
            return ui::metadata::PropertyFlags::kReadOnly | ui::metadata::PropertyFlags::kSerializable;
        }
...
template <typename TClass, typename TValue, typename TSig, TSig Set, typename TRet, TRet (TClass::*Get)() const,
    typename TConverter = ui::metadata::TypeConverter<TValue>>
class ObjectPropertyMetaData : public ObjectPropertyReadOnlyMetaData<TClass, TValue, TRet, Get, TConverter> {
public:
    using ObjectPropertyReadOnlyMetaData<TClass, TValue, TRet, Get, TConverter>::ObjectPropertyReadOnlyMetaData;
    ...
    ui::metadata::MemberMetaDataBase::ValueStrings GetValidValues() const override
    {
        if constexpr (kTypeIsSerializable) {
...
template <typename TClass, typename TKey, typename TValue, typename TConverter = ui::metadata::TypeConverter<TValue>>
...
public:
    ...
    ClassPropertyMetaData(TKey key, const std::string& property_type)
        : MemberMetaDataBase(key->name, property_type)
        , key_(key)
    {
    }
    ...
    std::u16string GetValueAsString(void* obj) const override
    {
        typename TypeHelper::TKValue value = internal::AsClass<TClass>(obj)->GetProperty(key_);
