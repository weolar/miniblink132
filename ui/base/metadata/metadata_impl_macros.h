...
#define BEGIN_METADATA_BASE(class_name)                                                                                                                        \
    METADATA_REINTERPRET_BASE_CLASS_INTERNAL(class_name, METADATA_CLASS_NAME_INTERNAL(class_name))                                                             \
    BEGIN_METADATA_INTERNAL_BASE(class_name, METADATA_CLASS_NAME_INTERNAL(class_name), class_name)

#define _BEGIN_NESTED_METADATA(outer_class, class_name)                                                                                                        \
    BEGIN_METADATA_INTERNAL(outer_class::class_name, METADATA_CLASS_NAME_INTERNAL(class_name), class_name::kAncestorClass)                                     \
    METADATA_PARENT_CLASS_INTERNAL(class_name::kAncestorClass)

#define _BEGIN_METADATA_SIMPLE(class_name)                                                                                                                     \
    BEGIN_METADATA_INTERNAL(class_name, METADATA_CLASS_NAME_INTERNAL(class_name), class_name::kAncestorClass)                                                  \
    METADATA_PARENT_CLASS_INTERNAL(class_name::kAncestorClass)
...
#define BEGIN_METADATA(class_name, ...)                                                                                                                        \
    _GET_MD_MACRO_NAME(class_name, ##__VA_ARGS__, _BEGIN_NESTED_METADATA, _BEGIN_METADATA_SIMPLE)                                                              \
...
// templated versions.
#define BEGIN_TEMPLATE_METADATA(class_name_alias, template_name) BEGIN_TEMPLATE_METADATA_INTERNAL(class_name_alias, METADATA_CLASS_NAME_INTERNAL(template_name))
...
#define ADD_PROPERTY_METADATA(property_type, property_name, ...)                                                                                               \
    auto property_name##_prop                                                                                                                                  \
        = std::make_unique<METADATA_PROPERTY_TYPE_INTERNAL(property_type, property_name, ##__VA_ARGS__)>(#property_name, #property_type);                      \
...
#define ADD_READONLY_PROPERTY_METADATA(property_type, property_name, ...)                                                                                      \
    auto property_name##_prop                                                                                                                                  \
        = std::make_unique<METADATA_READONLY_PROPERTY_TYPE_INTERNAL(property_type, property_name, ##__VA_ARGS__)>(#property_name, #property_type);             \
    AddMemberData(std::move(property_name##_prop));
...
#define ADD_CLASS_PROPERTY_METADATA(property_type, property_key, ...)                                                                                          \
    auto property_key##_prop                                                                                                                                   \
        = std::make_unique<METADATA_CLASS_PROPERTY_TYPE_INTERNAL(property_type, property_key, ##__VA_ARGS__)>(property_key, #property_type);                   \
