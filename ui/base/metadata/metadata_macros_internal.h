...
#define METADATA_ACCESSORS_INTERNAL_TEMPLATE(class_name)                                                                                                       \
    ...
    {                                                                                                                                                          \
        if (!METADATA_CLASS_NAME_INTERNAL(class_name)::meta_data_) {                                                                                           \
            METADATA_CLASS_NAME_INTERNAL(class_name)::meta_data_ = ui::metadata::MakeAndRegisterClassInfo<METADATA_CLASS_NAME_INTERNAL(class_name)>();         \
        }                                                                                                                                                      \
...
#define METADATA_CLASS_INTERNAL(class_name, file, line)                                                                                                        \
    ...
    public:                                                                                                                                                    \
        ...
        METADATA_CLASS_NAME_INTERNAL(class_name) & operator=(const METADATA_CLASS_NAME_INTERNAL(class_name) &) = delete;                                       \
                                                                                                                                                               \
    private:                                                                                                                                                   \
...
#define METADATA_PROPERTY_TYPE_INTERNAL(property_type, property_name, ...)                                                                                     \
    ui::metadata::ObjectPropertyMetaData<TheClass, property_type, decltype(&TheClass::Set##property_name), &TheClass::Set##property_name,                      \
        decltype(std::declval<TheClass>().Get##property_name()), &TheClass::Get##property_name, ##__VA_ARGS__>

#define METADATA_READONLY_PROPERTY_TYPE_INTERNAL(property_type, property_name, ...)                                                                            \
    ui::metadata::ObjectPropertyReadOnlyMetaData<TheClass, property_type, decltype(std::declval<TheClass>().Get##property_name()),                             \
        &TheClass::Get##property_name, ##__VA_ARGS__>

#define METADATA_CLASS_PROPERTY_TYPE_INTERNAL(property_type, property_key, ...)                                                                                \
    ui::metadata::ClassPropertyMetaData<TheClass, decltype(property_key), property_type, ##__VA_ARGS__>

#define BEGIN_METADATA_INTERNAL(qualified_class_name, metadata_class_name, parent_class_name)                                                                  \
    ui::metadata::ClassMetaData* qualified_class_name::metadata_class_name::meta_data_ = nullptr;                                                              \
                                                                                                                                                               \
    ui::metadata::ClassMetaData* qualified_class_name::MetaData()                                                                                              \
    {                                                                                                                                                          \
        static_assert(std::is_base_of_v<parent_class_name, qualified_class_name>, "class not child of parent");                                                \
        if (!qualified_class_name::metadata_class_name::meta_data_) {                                                                                          \
            qualified_class_name::metadata_class_name::meta_data_ = ui::metadata::MakeAndRegisterClassInfo<qualified_class_name::metadata_class_name>();       \
        }                                                                                                                                                      \
        return qualified_class_name::metadata_class_name::meta_data_;                                                                                          \
    }                                                                                                                                                          \
                                                                                                                                                               \
    const ui::metadata::ClassMetaData* qualified_class_name::GetClassMetaData() const                                                                          \
    {                                                                                                                                                          \
        return MetaData();                                                                                                                                     \
    }                                                                                                                                                          \
                                                                                                                                                               \
...
#define BEGIN_TEMPLATE_METADATA_INTERNAL(qualified_class_name, metadata_class_name)                                                                            \
    template <> ui::metadata::ClassMetaData* qualified_class_name::metadata_class_name::meta_data_ = nullptr;                                                  \
                                                                                                                                                               \
    template <> const char qualified_class_name::kViewClassName[] = #qualified_class_name;                                                                     \
                                                                                                                                                               \
    template <> void qualified_class_name::metadata_class_name::BuildMetaData()                                                                                \
    {                                                                                                                                                          \
        SetTypeName(#qualified_class_name);                                                                                                                    \
        SetParentClassMetaData(kAncestorClass::MetaData());

#define BEGIN_METADATA_INTERNAL_BASE(qualified_class_name, metadata_class_name, parent_class_name)                                                             \
    const char* qualified_class_name::GetClassName() const                                                                                                     \
    {                                                                                                                                                          \
        return GetClassMetaData()->type_name();                                                                                                                \
    }                                                                                                                                                          \
                                                                                                                                                               \
    BEGIN_METADATA_INTERNAL(qualified_class_name, metadata_class_name, parent_class_name)
...
#define METADATA_REINTERPRET_BASE_CLASS_INTERNAL(qualified_class_name, metadata_class_name)                                                                    \
    qualified_class_name* qualified_class_name::ReinterpretToBaseClass(void* obj)                                                                              \
    {                                                                                                                                                          \
        return reinterpret_cast<qualified_class_name*>(obj);                                                                                                   \
    }

#define METADATA_PARENT_CLASS_INTERNAL(parent_class_name) SetParentClassMetaData(parent_class_name::MetaData());

#define DECLARE_TEMPLATE_METADATA_INTERNAL(qualified_class_name, metadata_class_name)                                                                          \
    template <> ui::metadata::ClassMetaData* qualified_class_name::metadata_class_name::meta_data_
