...
base::Value::List GetDesktopEnvironmentInfo()
{
    ...
    if (env->GetVar(base::nix::kXdgCurrentDesktopEnvVar, &value)) {
        result.Append(display::BuildGpuInfoEntry(base::nix::kXdgCurrentDesktopEnvVar, value));
    }
    if (env->GetVar(base::nix::kXdgSessionTypeEnvVar, &value)) {
        result.Append(display::BuildGpuInfoEntry(base::nix::kXdgSessionTypeEnvVar, value));
    }
