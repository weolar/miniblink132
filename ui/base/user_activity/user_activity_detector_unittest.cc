...
TEST_F(UserActivityDetectorTest, Basic)
{
    ...
    base::TimeDelta advance_delta = base::Milliseconds(UserActivityDetector::kNotifyIntervalMs);
    AdvanceTime(advance_delta);
    ui::MouseEvent mouse_event(ui::EventType::kMouseMoved, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    OnEvent(&mouse_event);
    ...
    const base::TimeDelta kIgnoreMouseTime = base::Milliseconds(UserActivityDetector::kDisplayPowerChangeIgnoreMouseMs);
    ...
    AdvanceTime(advance_delta);
    ui::TouchEvent touch_event(ui::EventType::kTouchPressed, gfx::Point(), base::TimeTicks(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    OnEvent(&touch_event);
    ...
    AdvanceTime(advance_delta);
    ui::GestureEvent gesture_event(0, 0, ui::EF_NONE, base::TimeTicks::Now(), ui::GestureEventDetails(ui::EventType::kGestureTap));
...
TEST_F(UserActivityDetectorTest, RateLimitNotifications)
{
    ...
    AdvanceTime(base::Milliseconds(UserActivityDetector::kNotifyIntervalMs - 100));
    OnEvent(&event);
    EXPECT_FALSE(event.handled());
    EXPECT_EQ(0, observer_->num_invocations());
    observer_->reset_stats();
...
{
    ui::MouseEvent mouse_event(ui::EventType::kMouseMoved, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), ui::EF_IS_SYNTHESIZED, ui::EF_NONE);
    OnEvent(&mouse_event);
    EXPECT_FALSE(mouse_event.handled());
...
TEST_F(UserActivityDetectorTest, HandleExternalUserActivity)
{
    ...
    base::TimeDelta half_advance_delta = base::Milliseconds(UserActivityDetector::kNotifyIntervalMs / 2);
