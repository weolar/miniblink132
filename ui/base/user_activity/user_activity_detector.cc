...
{
    std::string details = base::StringPrintf("type=%d name=%s flags=%d time=%" PRId64, base::to_underlying(event->type()), event->GetName(), event->flags(),
        (event->time_stamp() - base::TimeTicks()).InMilliseconds());

    if (event->IsKeyEvent()) {
        details += base::StringPrintf(" key_code=%d", static_cast<const ui::KeyEvent*>(event)->key_code());
    } else if (event->IsMouseEvent() || event->IsTouchEvent() || event->IsGestureEvent()) {
        details += base::StringPrintf(" location=%s", static_cast<const ui::LocatedEvent*>(event)->location().ToString().c_str());
    }
...
bool UserActivityDetector::HasObserver(const UserActivityObserver* observer) const
{
...
{
    honor_mouse_events_time_ = GetCurrentTime() + base::Milliseconds(kDisplayPowerChangeIgnoreMouseMs);
}
...
void UserActivityDetector::DidProcessEvent(const PlatformEvent& platform_event)
{
...
void UserActivityDetector::ProcessReceivedEvent(const ui::Event* event)
{
    ...
    if (event->IsMouseEvent() || event->IsMouseWheelEvent()) {
        if (event->flags() & ui::EF_IS_SYNTHESIZED)
            return;
        if (!honor_mouse_events_time_.is_null() && GetCurrentTime() < honor_mouse_events_time_)
            return;
    }
