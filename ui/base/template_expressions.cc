...
HtmlTemplate FindHtmlTemplate(std::string_view source)
{
    ...
    std::string_view::size_type found_end = source.find(kHtmlTemplateEnd, out.start);
    ...
    if (source.substr(out.start, out.length).find(kHtmlTemplateStart) != std::string_view::npos) {
        out.type = INVALID;
        return out;
    }
...
std::string PolymerParameterEscape(const std::string& in_string, bool is_javascript)
{
    std::string out;
...
bool EscapeForJS(const std::string& in_string, std::optional<char> in_previous, std::string* out_string)
{
...
bool HasUnexpectedPlaceholder(const std::string& key, const std::string& replacement)
{
...
bool ReplaceTemplateExpressionsInternal(std::string_view source, const ui::TemplateReplacements& replacements, bool is_javascript, std::string* formatted,
    bool skip_unexpected_placeholder_check = false)
{
    ...
    while (true) {
        ...
        if (next_pos == std::string::npos) {
            formatted->append(source.data() + current_pos, source.size() - current_pos);
            break;
        }
        ...
        CHECK(value != replacements.end()) << "$i18n replacement key \"" << key << "\" not found";

        std::string replacement = value->second;
        if (is_javascript) {
            // Run JS escaping first.
            std::optional<char> last = formatted->empty() ? std::nullopt : std::make_optional(formatted->back());
        ...
        if (!skip_unexpected_placeholder_check && context != "Polymer") {
            DCHECK(!HasUnexpectedPlaceholder(key, replacement)) << "Dangling placeholder found in " << key;
        }
#endif
...
void TemplateReplacementsFromDictionaryValue(const base::Value::Dict& dictionary, TemplateReplacements* replacements)
{
...
bool ReplaceTemplateExpressionsInJS(std::string_view source, const TemplateReplacements& replacements, std::string* formatted)
{
    ...
    while (true) {
        ...
        std::string_view html_template = remaining.substr(current_template.start, current_template.length);

        // Perform replacements with JS escaping.
        std::string formatted_html;
        if (!ReplaceTemplateExpressionsInternal(html_template, replacements, true, &formatted_html)) {
            return false;
        }
        ...
        remaining = remaining.substr(current_template.start + current_template.length);
    }
}

std::string ReplaceTemplateExpressions(std::string_view source, const TemplateReplacements& replacements, bool skip_unexpected_placeholder_check)
{
    std::string formatted;
    ReplaceTemplateExpressionsInternal(source, replacements, false, &formatted, skip_unexpected_placeholder_check);
    return formatted;
}
} // namespace ui
