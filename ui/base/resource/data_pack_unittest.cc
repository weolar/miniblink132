...
class DataPackTest : public testing::TestWithParam<DataPack::TextEncodingType> {
public:
    DataPackTest()
    {
    }
...
TEST(DataPackTest, LoadFromPath)
{
    ...
    base::FilePath data_path = dir.GetPath().Append(FILE_PATH_LITERAL("sample.pak"));
    ...
    ASSERT_EQ(pack.GetStringView(4), std::make_optional(std::string_view { "this is id 4" }));
    ASSERT_TRUE(pack.HasResource(6));
    ASSERT_EQ(pack.GetStringView(6), std::make_optional(std::string_view { "this is id 6" }));
...
TEST(DataPackTest, LoadFromPathCompressed)
{
    base::ScopedTempDir dir;
    ASSERT_TRUE(dir.CreateUniqueTempDir());
    base::FilePath data_path = dir.GetPath().Append(FILE_PATH_LITERAL("sample.pak.gz"));

    // Dump contents into a compressed pak file.
    std::string compressed;
    ASSERT_TRUE(compression::GzipCompress({ kSamplePakContentsV4, kSamplePakSizeV4 }, &compressed));
...
TEST(DataPackTest, LoadFromFileRegion)
{
    ...
    ASSERT_TRUE(base::AppendToFile(data_path, { kSamplePakContentsV4, kSamplePakSizeV4 }));
...
TEST(DataPackTest, LoadFromBufferV5)
{
    ...
    ASSERT_TRUE(pack.LoadFromBuffer({ kSampleCompressPakContentsV5, kSampleCompressPakSizeV5 }));
    ...
    ASSERT_EQ(pack.GetStringView(10), std::make_optional(std::string_view { "this is id 4" }));
...
INSTANTIATE_TEST_SUITE_P(WriteBINARY, DataPackTest, ::testing::Values(DataPack::BINARY));
INSTANTIATE_TEST_SUITE_P(WriteUTF8, DataPackTest, ::testing::Values(DataPack::UTF8));
INSTANTIATE_TEST_SUITE_P(WriteUTF16, DataPackTest, ::testing::Values(DataPack::UTF16));
...
TEST_P(DataPackTest, Write)
{
    ...
    ASSERT_EQ(pack.GetStringView(15), std::make_optional(std::string_view { fifteen }));
...
TEST_P(DataPackTest, WriteWithAliases)
{
    ...
    ASSERT_EQ(pack.GetStringView(11), std::make_optional(std::string_view { three }));
...
TEST(DataPackTest, ModifiedWhileUsed)
{
    ...
    ASSERT_TRUE(base::WriteFile(data_path, { kSampleCorruptPakContents, kSampleCorruptPakSize }));
...
TEST(DataPackTest, Misordered)
{
    ...
    ASSERT_FALSE(pack.LoadFromBuffer({ kSampleMisorderedPakContents, kSampleMisorderedPakSize }));
}
