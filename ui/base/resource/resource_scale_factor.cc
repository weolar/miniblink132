...
static_assert(NUM_SCALE_FACTORS == std::size(kResourceScaleFactorScales), "kResourceScaleFactorScales has incorrect size");
...
void SetSupportedResourceScaleFactors(const std::vector<ResourceScaleFactor>& scale_factors)
{
    ...
    g_supported_resource_scale_factors = new std::vector<ResourceScaleFactor>(scale_factors);
    std::sort(g_supported_resource_scale_factors->begin(), g_supported_resource_scale_factors->end(),
        [](ResourceScaleFactor lhs, ResourceScaleFactor rhs) { return kResourceScaleFactorScales[lhs] < kResourceScaleFactorScales[rhs]; });
}

const std::vector<ResourceScaleFactor>& GetSupportedResourceScaleFactors()
{
    CHECK_NE(g_supported_resource_scale_factors, nullptr) << "ResourceBundle needs to be initialized.";
...
ResourceScaleFactor GetSupportedResourceScaleFactor(float scale)
{
    ...
    for (const auto scale_factor : *g_supported_resource_scale_factors) {
        const float diff = std::abs(kResourceScaleFactorScales[scale_factor] - scale);
...
ResourceScaleFactor GetSupportedResourceScaleFactorForRescale(float scale)
{
    ...
    for (auto supported_scale : *g_supported_resource_scale_factors) {
        if (kResourceScaleFactorScales[supported_scale] + kFallbackToSmallerScaleDiff >= scale) {
...
ScopedSetSupportedResourceScaleFactors::ScopedSetSupportedResourceScaleFactors(const std::vector<ResourceScaleFactor>& new_scale_factors)
{
    if (g_supported_resource_scale_factors) {
        original_scale_factors_ = std::make_unique<std::vector<ResourceScaleFactor>>(*g_supported_resource_scale_factors);
    }
