...
public:
    ...
    struct COMPONENT_EXPORT(UI_BASE) FontDetails {
        explicit FontDetails(std::string typeface = std::string(), int size_delta = 0, gfx::Font::Weight weight = gfx::Font::Weight::NORMAL);
    ...
    enum LoadResources { LOAD_COMMON_RESOURCES, DO_NOT_LOAD_COMMON_RESOURCES };
    ...
    public:
        ...
        virtual base::FilePath GetPathForResourcePack(const base::FilePath& pack_path, ResourceScaleFactor scale_factor) = 0;

        // Called before a locale pack file is loaded. Return the full path for
        // the pack file to continue loading or an empty value to cancel loading.
        // |pack_path| will contain the complete default path for the pack file if
        // known or just the pack file name otherwise.
        virtual base::FilePath GetPathForLocalePack(const base::FilePath& pack_path, const std::string& locale) = 0;
        ...
        // default resource.
        virtual base::RefCountedMemory* LoadDataResourceBytes(int resource_id, ResourceScaleFactor scale_factor) = 0;
        ...
        virtual std::optional<std::string> LoadDataResourceString(int resource_id) = 0;

        // Retrieve a raw data resource. Return true if a resource was provided or
        // false to attempt retrieval of the default resource.
        virtual bool GetRawDataResource(int resource_id, ResourceScaleFactor scale_factor, std::string_view* value) const = 0;

        // Retrieve a localized string. Return true if a string was provided or
        // false to attempt retrieval of the default string.
        virtual bool GetLocalizedString(int message_id, std::u16string* value) const = 0;
    ...
    static std::string InitSharedInstanceWithLocale(const std::string& pref_locale, Delegate* delegate, LoadResources load_resources);
    ...
    static void InitSharedInstanceWithPakFileRegion(base::File pak_file, const base::MemoryMappedFile::Region& region);
    ...
    static void InitSharedInstanceWithBuffer(base::span<const uint8_t> buffer, ResourceScaleFactor scale_factor);
    ...
    void LoadSecondaryLocaleDataWithPakFileRegion(base::File pak_file, const base::MemoryMappedFile::Region& region);
    ...
    void AddDataPackFromPath(const base::FilePath& path, ResourceScaleFactor scale_factor);

    // Same as above but using only a region (offset + size) of the file.
    void AddDataPackFromFileRegion(base::File file, const base::MemoryMappedFile::Region& region, ResourceScaleFactor scale_factor);

    // Same as above but using contents of the given buffer.
    void AddDataPackFromBuffer(base::span<const uint8_t> buffer, ResourceScaleFactor scale_factor);

    // Same as AddDataPackFromPath but does not log an error if the pack fails to
    // load.
    void AddOptionalDataPackFromPath(const base::FilePath& path, ResourceScaleFactor scale_factor);
    ...
    base::RefCountedMemory* LoadDataResourceBytesForScale(int resource_id, ResourceScaleFactor scale_factor) const;
    ...
    // (such as wallpaper).
    std::string_view GetRawDataResourceForScale(int resource_id, ResourceScaleFactor scale_factor, ResourceScaleFactor* loaded_scale_factor = nullptr) const;
    ...
    std::string LoadDataResourceStringForScale(int resource_id, ResourceScaleFactor scaling_factor) const;
    ...
    // is not thread safe!
    void OverrideLocaleStringResource(int resource_id, const std::u16string& string);
    ...
    std::string GetLoadedLocaleForTesting()
    {
        return loaded_locale_;
    }
#if DCHECK_IS_ON()
...
private:
    ...
    void AddDataPackFromPathInternal(const base::FilePath& path, ResourceScaleFactor scale_factor, bool optional);
    ...
    std::string LoadLocaleResources(const std::string& pref_locale, bool crash_on_failure);

    // Load test resources in given paths. If either path is empty an empty
    // resource pack is loaded.
    void LoadTestResources(const base::FilePath& path, const base::FilePath& locale_path);
    ...
    bool LoadBitmap(const ResourceHandle& data_handle, int resource_id, SkBitmap* bitmap, bool* fell_back_to_1x) const;
    ...
    bool LoadBitmap(int resource_id, ResourceScaleFactor* scale_factor, SkBitmap* bitmap, bool* fell_back_to_1x) const;
    ...
    static bool DecodePNG(base::span<const uint8_t> buf, SkBitmap* bitmap, bool* fell_back_to_1x);
