...
ResourceBundle::LottieImageParseFunction g_parse_lottie_as_still_image_ = nullptr;
ResourceBundle::LottieThemedImageParseFunction g_parse_lottie_as_themed_still_image_ = nullptr;
#endif
...
bool HasGzipHeader(std::string_view data)
{
    ...
    net::GZipHeader::Status header_status = header.ReadMore(data.data(), data.length(), &header_end);
...
bool HasBrotliHeader(std::string_view data)
{
    ...
    static_assert(std::size(ResourceBundle::kBrotliConst) == 2, "Magic number should be 2 bytes long");
    return data.size() >= ResourceBundle::kBrotliHeaderSize && *data_bytes == ResourceBundle::kBrotliConst[0]
...
size_t GetBrotliDecompressSize(std::string_view input)
{
    ...
    int bytes_size = static_cast<int>(ResourceBundle::kBrotliHeaderSize - std::size(ResourceBundle::kBrotliConst));
...
bool BrotliDecompress(std::string_view input, OutputBufferType output)
{
    ...
    return BrotliDecoderDecompress(
               input.size() - ResourceBundle::kBrotliHeaderSize, raw_input, &decompress_size, GetBufferForWriting(output, decompress_size).data())
...
void DecompressIfNeeded(std::string_view data, OutputBufferType output)
{
    if (!data.empty() && HasGzipHeader(data)) {
        ...
        bool success = compression::GzipUncompress(base::as_bytes(base::make_span(data)), GetBufferForWriting(output, uncompressed_size));
        DCHECK(success);
...
public:
    ...
    gfx::ImageSkiaRep GetImageForScale(float scale) override
    {
        ...
        bool found = rb_->LoadBitmap(resource_id_, &scale_factor, &image, &fell_back_to_1x);
        if (!found) {
#if BUILDFLAG(IS_ANDROID)
            // TODO(oshima): Android unit_tests runs at DSF=3 with 100P assets.
            return gfx::ImageSkiaRep();
#else
            DUMP_WILL_BE_NOTREACHED() << "Unable to load bitmap image with id " << resource_id_ << ", scale=" << scale;
        ...
        if (fell_back_to_1x) {
            ...
            image = skia::ImageOperations::Resize(
                image, skia::ImageOperations::RESIZE_LANCZOS3, base::ClampCeil(image.width() * scale), base::ClampCeil(image.height() * scale));
        } else {
...
ResourceBundle::FontDetails::FontDetails(std::string typeface, int size_delta, gfx::Font::Weight weight)
    : typeface(typeface)
...
{
    return std::tie(typeface, size_delta, weight) == std::tie(rhs.typeface, rhs.size_delta, rhs.weight);
}
...
// static
std::string ResourceBundle::InitSharedInstanceWithLocale(const std::string& pref_locale, Delegate* delegate, LoadResources load_resources)
{
...
// static
void ResourceBundle::InitSharedInstanceWithBuffer(base::span<const uint8_t> buffer, ResourceScaleFactor scale_factor)
{
...
// static
void ResourceBundle::InitSharedInstanceWithPakFileRegion(base::File pak_file, const base::MemoryMappedFile::Region& region)
{
    InitSharedInstance(nullptr);
    auto data_pack = std::make_unique<DataPack>(k100Percent);
    CHECK(data_pack->LoadFromFileRegion(std::move(pak_file), region)) << "failed to load pak file";
    g_shared_instance_->locale_resources_data_ = std::move(data_pack);
    g_shared_instance_->InitDefaultFontList();
}
...
// static
ResourceBundle* ResourceBundle::SwapSharedInstanceForTesting(ResourceBundle* instance)
{
...
void ResourceBundle::SetLottieParsingFunctions(
    LottieImageParseFunction parse_lottie_as_still_image, LottieThemedImageParseFunction parse_lottie_as_themed_still_image)
{
...
void ResourceBundle::LoadSecondaryLocaleDataWithPakFileRegion(base::File pak_file, const base::MemoryMappedFile::Region& region)
{
    auto data_pack = std::make_unique<DataPack>(k100Percent);
    CHECK(data_pack->LoadFromFileRegion(std::move(pak_file), region)) << "failed to load secondary pak file";
...
void ResourceBundle::AddDataPackFromPath(const base::FilePath& path, ResourceScaleFactor scale_factor)
{
    AddDataPackFromPathInternal(path, scale_factor, false);
}

void ResourceBundle::AddOptionalDataPackFromPath(const base::FilePath& path, ResourceScaleFactor scale_factor)
{
    AddDataPackFromPathInternal(path, scale_factor, true);
}

void ResourceBundle::AddDataPackFromBuffer(base::span<const uint8_t> buffer, ResourceScaleFactor scale_factor)
{
...
void ResourceBundle::AddDataPackFromFileRegion(base::File file, const base::MemoryMappedFile::Region& region, ResourceScaleFactor scale_factor)
{
    auto data_pack = std::make_unique<DataPack>(scale_factor);
...
base::FilePath ResourceBundle::GetLocaleFilePath(const std::string& app_locale)
{
    ...
    if (base::PathService::Get(ui::DIR_LOCALES, &locale_file_path)) {
        locale_file_path = locale_file_path.AppendASCII(app_locale + kPakFileExtension);
    }
    ...
    if (HasSharedInstance() && GetSharedInstance().delegate_) {
        locale_file_path = GetSharedInstance().delegate_->GetPathForLocalePack(locale_file_path, app_locale);
    }
...
std::string ResourceBundle::LoadLocaleResources(const std::string& pref_locale, bool crash_on_failure)
{
    ...
    if (!data_pack->LoadFromPath(locale_file_path) && crash_on_failure) {
        ...
        base::wcslcpy(path_copy, locale_file_path.value().c_str(), std::size(path_copy));
...
void ResourceBundle::LoadTestResources(const base::FilePath& path, const base::FilePath& locale_path)
{
    is_test_resources_ = true;
    // Use the given resource pak for both common and localized resources.

    if (!path.empty()) {
        const ResourceScaleFactor scale_factor = ui::GetSupportedResourceScaleFactors()[0];
        auto data_pack = std::make_unique<DataPack>(scale_factor);
...
void ResourceBundle::OverrideLocaleStringResource(int resource_id, const std::u16string& string)
{
...
std::u16string ResourceBundle::MaybeMangleLocalizedString(const std::u16string& str) const
{
    ...
    return base::JoinString({ start_marker, dashes, str, dashes, end_marker }, u" ");
}

std::string ResourceBundle::ReloadLocaleResources(const std::string& pref_locale)
{
...
gfx::Image& ResourceBundle::GetImageNamed(int resource_id)
{
    ...
    if (image.IsEmpty()) {
        ...
        CHECK(!image_skia.isNull()) << "Unable to load image with id " << resource_id;
...
std::optional<ResourceBundle::LottieData> ResourceBundle::GetLottieData(int resource_id) const
{
    ...
    if (potential_lottie.substr(0u, std::size(kLottiePrefix)) != std::string_view(kLottiePrefix, std::size(kLottiePrefix))) {
        return std::nullopt;
    }

    LottieData result;
    DecompressIfNeeded(potential_lottie.substr(std::size(kLottiePrefix)), &result);
    return result;
}

#if BUILDFLAG(IS_CHROMEOS)
const ui::ImageModel& ResourceBundle::GetThemedLottieImageNamed(int resource_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    // resulting image.
    auto inserted = image_models_.emplace(resource_id, (*g_parse_lottie_as_themed_still_image_)(std::move(*data)));
    DCHECK(inserted.second);
    return inserted.first->second;
}
#endif
...
base::RefCountedMemory* ResourceBundle::LoadDataResourceBytes(int resource_id) const
{
    return LoadDataResourceBytesForScale(resource_id, ui::kScaleFactorNone);
}

base::RefCountedMemory* ResourceBundle::LoadDataResourceBytesForScale(int resource_id, ResourceScaleFactor scale_factor) const
{
    TRACE_EVENT("ui", "ResourceBundle::LoadDataResourceBytesForScale", [&](perfetto::EventContext ctx) {
    ...
    if (delegate_) {
        base::RefCountedMemory* bytes = delegate_->LoadDataResourceBytes(resource_id, scale_factor);
        if (bytes)
            return bytes;
    }
...
std::string_view ResourceBundle::GetRawDataResourceForScale(int resource_id, ResourceScaleFactor scale_factor, ResourceScaleFactor* loaded_scale_factor) const
{
    if (delegate_) {
    ...
    if (scale_factor != ui::k100Percent) {
        for (const auto& resource_handle : resource_handles_) {
            if (resource_handle->GetResourceScaleFactor() == scale_factor) {
                if (auto data = resource_handle->GetStringView(static_cast<uint16_t>(resource_id)); data.has_value()) {
                    if (loaded_scale_factor) {
                        *loaded_scale_factor = scale_factor;
                    }
                    return data.value();
    ...
    for (const auto& resource_handle : resource_handles_) {
        if ((resource_handle->GetResourceScaleFactor() == ui::k100Percent || resource_handle->GetResourceScaleFactor() == ui::k200Percent
                || resource_handle->GetResourceScaleFactor() == ui::k300Percent || resource_handle->GetResourceScaleFactor() == ui::kScaleFactorNone)) {
            if (auto data = resource_handle->GetStringView(static_cast<uint16_t>(resource_id)); data.has_value()) {
                if (loaded_scale_factor) {
...
{
    if (delegate_) {
        std::optional<std::string> data = delegate_->LoadDataResourceString(resource_id);
        if (data)
            return data.value();
    }
...
std::string ResourceBundle::LoadDataResourceStringForScale(int resource_id, ResourceScaleFactor scaling_factor) const
{
    std::string output;
    DecompressIfNeeded(GetRawDataResourceForScale(resource_id, scaling_factor), &output);
    return output;
}
...
std::string ResourceBundle::LoadLocalizedResourceString(int resource_id) const
{
    ...
    if (locale_resources_data_.get()) {
        data = locale_resources_data_->GetStringView(static_cast<uint16_t>(resource_id)).value_or(std::string_view());
    }
    if (data.empty() && secondary_locale_resources_data_.get()) {
        data = secondary_locale_resources_data_->GetStringView(static_cast<uint16_t>(resource_id)).value_or(std::string_view());
    }
    if (data.empty()) {
...
#endif
    DCHECK(!IsGzipped(resource_id) && !IsBrotli(resource_id)) << "Compressed string encountered, perhaps use "
                                                                 "ResourceBundle::LoadLocalizedResourceString instead";
    return GetLocalizedStringImpl(resource_id);
}

base::RefCountedMemory* ResourceBundle::LoadLocalizedResourceBytes(int resource_id) const
{
    {
        base::AutoLock lock_scope(*locale_resources_data_lock_);

        if (locale_resources_data_.get()) {
            if (auto data = locale_resources_data_->GetStringView(static_cast<uint16_t>(resource_id)); data.has_value() && !data->empty()) {
                return new base::RefCountedStaticMemory(base::as_byte_span(*data));
            }
        }

        if (secondary_locale_resources_data_.get()) {
            if (auto data = secondary_locale_resources_data_->GetStringView(static_cast<uint16_t>(resource_id)); data.has_value() && !data->empty()) {
                return new base::RefCountedStaticMemory(base::as_byte_span(*data));
            }
        }
    }
...
const gfx::FontList& ResourceBundle::GetFontListForDetails(const FontDetails& details)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    gfx::FontList base_font_list = details.typeface.empty()
        ? default_font_list
        : gfx::FontList({ details.typeface }, default_font_list.GetFontStyle(), default_font_list.GetFontSize(), default_font_list.GetFontWeight());
    ...
    styled.first->second = sized.first->second.Derive(0, sized.first->second.GetFontStyle(), details.weight);
...
{
    mangle_localized_strings_ = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kMangleLocalizedStrings);
}
...
void ResourceBundle::LoadChromeResources()
{
    ...
    if (IsScaleFactorSupported(k100Percent)) {
        AddDataPackFromPath(GetResourcesPakFilePath("chrome_100_percent.pak"), k100Percent);
    }

    if (IsScaleFactorSupported(k200Percent)) {
        AddOptionalDataPackFromPath(GetResourcesPakFilePath("chrome_200_percent.pak"), k200Percent);
    }
}

void ResourceBundle::AddDataPackFromPathInternal(const base::FilePath& path, ResourceScaleFactor scale_factor, bool optional)
{
...
void ResourceBundle::AddResourceHandle(std::unique_ptr<ResourceHandle> resource_handle)
{
#if DCHECK_IS_ON()
    resource_handle->CheckForDuplicateResources(resource_handles_);
#endif

    if (GetScaleForResourceScaleFactor(resource_handle->GetResourceScaleFactor()) > GetScaleForResourceScaleFactor(max_scale_factor_))
...
void ResourceBundle::InitDefaultFontList()
{
    ...
    std::string font_family = base::UTF16ToUTF8(GetLocalizedStringImpl(IDS_UI_FONT_FAMILY_CROS));
...
gfx::ImageSkia ResourceBundle::CreateImageSkia(int resource_id)
{
    ...
    const ResourceScaleFactor scale_factor_to_load = display::win::GetDPIScale() > 1.25 ? GetMaxResourceScaleFactor() : ui::k100Percent;
#else
    ...
    return gfx::ImageSkia(std::make_unique<BitmapImageSource>(this, resource_id), GetScaleForResourceScaleFactor(scale_factor_to_load));
}

bool ResourceBundle::LoadBitmap(const ResourceHandle& data_handle, int resource_id, SkBitmap* bitmap, bool* fell_back_to_1x) const
{
    DCHECK(fell_back_to_1x);
    scoped_refptr<base::RefCountedMemory> memory(data_handle.GetStaticMemory(static_cast<uint16_t>(resource_id)));
    if (!memory.get())
        return false;
...
bool ResourceBundle::LoadBitmap(int resource_id, ResourceScaleFactor* scale_factor, SkBitmap* bitmap, bool* fell_back_to_1x) const
{
    DCHECK(fell_back_to_1x);
    for (const auto& pack : resource_handles_) {
        if (pack->GetResourceScaleFactor() == ui::kScaleFactorNone && LoadBitmap(*pack, resource_id, bitmap, fell_back_to_1x)) {
            DCHECK(!*fell_back_to_1x);
            *scale_factor = ui::kScaleFactorNone;
            return true;
        }

        if (pack->GetResourceScaleFactor() == *scale_factor && LoadBitmap(*pack, resource_id, bitmap, fell_back_to_1x)) {
            return true;
        }
    }
    ...
    if (is_test_resources_ && *scale_factor != ui::k100Percent) {
        for (const auto& pack : resource_handles_) {
            if (pack->GetResourceScaleFactor() == ui::k100Percent && LoadBitmap(*pack, resource_id, bitmap, fell_back_to_1x)) {
...
std::u16string ResourceBundle::GetLocalizedStringImpl(int resource_id) const
{
    ...
    IdToStringMap::const_iterator it = overridden_locale_strings_.find(resource_id);
    ...
    if (!(data = locale_resources_data_->GetStringView(static_cast<uint16_t>(resource_id))).has_value()) {
        if (secondary_locale_resources_data_.get()
            && (data = secondary_locale_resources_data_->GetStringView(static_cast<uint16_t>(resource_id))).has_value()) {
    ...
    DCHECK(encoding == ResourceHandle::UTF16 || encoding == ResourceHandle::UTF8) << "requested localized string from binary pack file";

    // Data pack encodes strings as either UTF8 or UTF16.
    std::u16string msg;
    if (encoding == ResourceHandle::UTF16) {
        msg.assign(reinterpret_cast<const char16_t*>(data->data()), data->length() / 2);
...
{
    if (buf.size() < std::size(kPngMagic) || buf.first(std::size(kPngMagic)) != kPngMagic) {
    ...
    for (;;) {
        ...
        if (length == 0u) {
            auto scale_chunk = buf.subspan(sizeof(uint32_t), std::size(kPngScaleChunkType));
            if (scale_chunk == kPngScaleChunkType) {
                return true;
            }
        }
        auto data_chunk = buf.subspan(sizeof(uint32_t), std::size(kPngDataChunkType));
...
// static
bool ResourceBundle::DecodePNG(base::span<const uint8_t> buf, SkBitmap* bitmap, bool* fell_back_to_1x)
{
