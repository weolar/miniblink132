...
const unsigned char kPngScaleChunk[12] = { 0x00, 0x00, 0x00, 0x00, 'c', 's', 'C', 'l', 0xc1, 0x30, 0x60, 0x4d };
...
gfx::ImageSkia ParseLottieAsStillImageForTesting(std::vector<uint8_t> data)
{
    ...
    return gfx::ImageSkia(gfx::ImageSkiaRep(gfx::Size(kDimension, kDimension), 0.f));
}
#endif

// Returns |bitmap_data| with |custom_chunk| inserted after the IHDR chunk.
void AddCustomChunk(std::string_view custom_chunk, std::vector<unsigned char>* bitmap_data)
{
    size_t chunk_offset = 0u;

    // Expect the magic signature first.
    auto magic_span = base::as_byte_span(*bitmap_data).first<std::size(kPngMagic)>();
    ...
    uint32_t ihdr_chunk_length = base::numerics::U32FromBigEndian(ihdr_chunk.first<sizeof(uint32_t)>());
    auto ihdr_type = ihdr_chunk.subspan<sizeof(uint32_t), std::size(kPngIHDRChunkType)>();
    ...
    bitmap_data->insert(bitmap_data->begin() + chunk_offset, custom_chunk.begin(), custom_chunk.end());
}
...
void CreateDataPackWithSingleBitmap(const base::FilePath& path, int edge_size, std::string_view custom_chunk)
{
    SkBitmap bitmap;
    bitmap.allocN32Pixels(edge_size, edge_size);
    bitmap.eraseColor(SK_ColorWHITE);
    std::optional<std::vector<uint8_t>> bitmap_data = gfx::PNGCodec::EncodeBGRASkBitmap(bitmap, /*discard_transparency=*/false);
...
TEST_F(ResourceBundleTest, DelegateGetPathForResourcePack)
{
    ...
    EXPECT_CALL(delegate_, GetPathForResourcePack(Property(&base::FilePath::value, pack_path.value()), pack_scale_factor)).Times(1).WillOnce(Return(pack_path));
...
{
    ResourceBundle* orig_instance = ResourceBundle::SwapSharedInstanceForTesting(nullptr);
    ResourceBundle::InitSharedInstance(&delegate_);

    std::string locale = "en-US";

    // Cancel the load.
    EXPECT_CALL(delegate_, GetPathForLocalePack(_, _)).WillRepeatedly(Return(base::FilePath())).RetiresOnSaturation();

    EXPECT_FALSE(ResourceBundle::LocaleDataPakExists(locale));
    EXPECT_EQ("", ResourceBundle::GetSharedInstance().LoadLocaleResources(locale, /*crash_on_failure=*/false));

    // Allow the load to proceed.
    EXPECT_CALL(delegate_, GetPathForLocalePack(_, _)).WillRepeatedly(ReturnArg<0>());

    EXPECT_TRUE(ResourceBundle::LocaleDataPakExists(locale));
    EXPECT_EQ(locale, ResourceBundle::GetSharedInstance().LoadLocaleResources(locale, /*crash_on_failure=*/false));
...
TEST_F(ResourceBundleTest, DelegateGetImageNamed)
{
    ...
    EXPECT_CALL(delegate_, GetImageNamed(resource_id)).Times(1).WillOnce(Return(empty_image));
...
TEST_F(ResourceBundleTest, DelegateGetNativeImageNamed)
{
    ...
    EXPECT_CALL(delegate_, GetImageNamed(resource_id)).Times(Between(0, 1)).WillOnce(Return(empty_image));
    EXPECT_CALL(delegate_, GetNativeImageNamed(resource_id)).Times(Between(0, 1)).WillOnce(Return(empty_image));
...
TEST_F(ResourceBundleTest, DelegateHasDataResource)
{
    ...
    EXPECT_CALL(delegate_, HasDataResource(resource_id)).Times(1).WillOnce(Return(true));
...
TEST_F(ResourceBundleTest, DelegateLoadDataResourceBytes)
{
    ...
    scoped_refptr<base::RefCountedStaticMemory> static_memory(new base::RefCountedStaticMemory(data));

    int resource_id = 5;
    ResourceScaleFactor scale_factor = ui::kScaleFactorNone;

    EXPECT_CALL(delegate_, LoadDataResourceBytes(resource_id, scale_factor)).Times(1).WillOnce(Return(static_memory.get()));

    scoped_refptr<base::RefCountedMemory> result = resource_bundle->LoadDataResourceBytesForScale(resource_id, scale_factor);
...
TEST_F(ResourceBundleTest, DelegateGetRawDataResource)
{
    ...
    EXPECT_CALL(delegate_, GetRawDataResource(resource_id, ui::kScaleFactorNone, _)).Times(1).WillOnce(DoAll(SetArgPointee<2>(string_piece), Return(true)));
...
TEST_F(ResourceBundleTest, IsGzipped)
{
    ...
    base::FilePath data_path = temp_dir_.GetPath().Append(FILE_PATH_LITERAL("sample.pak"));
    // Dump contents into a pak file and load it.
    ASSERT_TRUE(base::WriteFile(data_path, { kSampleCompressPakContentsV5, kSampleCompressPakSizeV5 }));
...
TEST_F(ResourceBundleTest, DelegateGetLocalizedString)
{
    ...
    EXPECT_CALL(delegate_, GetLocalizedString(resource_id, _)).Times(1).WillOnce(DoAll(SetArgPointee<1>(data), Return(true)));
...
TEST_F(ResourceBundleTest, CanOverrideStringResources)
{
    ...
    EXPECT_TRUE(resource_bundle->get_can_override_locale_string_resources_for_test());
    resource_bundle->GetLocalizedString(resource_id);
    EXPECT_FALSE(resource_bundle->get_can_override_locale_string_resources_for_test());
}
#endif
...
TEST_F(ResourceBundleTest, DelegateGetLocalizedStringWithOverride)
{
    ...
    EXPECT_CALL(delegate_, GetLocalizedString(resource_id, _)).Times(1).WillOnce(DoAll(SetArgPointee<1>(delegate_data), Return(true)));
...
public:
    ...
    ResourceBundle* CreateResourceBundleWithEmptyLocalePak()
    {
        ...
        const base::FilePath& locale_path = dir_path().Append(FILE_PATH_LITERAL("locale.pak"));
        EXPECT_TRUE(base::WriteFile(locale_path, { kEmptyPakContents, kEmptyPakSize }));
...
TEST_F(ResourceBundleImageTest, LoadDataResourceBytes)
{
    ...
    scoped_refptr<base::RefCountedMemory> resource = resource_bundle->LoadDataResourceBytes(4);
    EXPECT_EQ("this is id 4", base::as_string_view(*resource));

    // Test the brotli data.
    scoped_refptr<base::RefCountedMemory> brotli_resource = resource_bundle->LoadDataResourceBytes(6);
    EXPECT_EQ("this is id 6", base::as_string_view(*brotli_resource));

    // Test the gzipped data.
    scoped_refptr<base::RefCountedMemory> gzip_resource = resource_bundle->LoadDataResourceBytes(8);
...
TEST_F(ResourceBundleImageTest, LoadDataResourceBytesNotFound)
{
    ...
    EXPECT_EQ(nullptr, resource_bundle->LoadDataResourceBytes(kUnfoundResourceId));

    // Give a .pak file that doesn't exist so we will fail to load it.
    resource_bundle->AddDataPackFromPath(base::FilePath(FILE_PATH_LITERAL("non-existant-file.pak")), ui::kScaleFactorNone);
    EXPECT_EQ(nullptr, resource_bundle->LoadDataResourceBytes(kUnfoundResourceId));
}
...
TEST_F(ResourceBundleImageTest, LoadDataResourceStringForScale)
{
    ...
    ASSERT_TRUE(base::WriteFile(data_path, { kSampleCompressPakContentsV5, kSampleCompressPakSizeV5 }));
    ASSERT_TRUE(base::WriteFile(data_2x_path, { kSampleCompressScaledPakContents, kSampleCompressScaledPakSize }));
    ...
    EXPECT_EQ("this is id 6", resource_bundle->LoadDataResourceStringForScale(6, k100Percent));
    EXPECT_EQ("this is id 6 x2", resource_bundle->LoadDataResourceStringForScale(6, k200Percent));
}
...
TEST_F(ResourceBundleImageTest, GetRawDataResource)
{
    ...
    ASSERT_TRUE(base::WriteFile(data_2x_path, { kSamplePakContents2x, kSamplePakSize2x }));
    ...
    EXPECT_EQ("this is id 4", resource_bundle->GetRawDataResourceForScale(4, k100Percent));
    EXPECT_EQ("this is id 4 2x", resource_bundle->GetRawDataResourceForScale(4, k200Percent));

    // Resource ID 6 only exists in the 1x pak so we expect the same resource
    // for both scale factor requests.
    EXPECT_EQ("this is id 6", resource_bundle->GetRawDataResourceForScale(6, k100Percent));
    EXPECT_EQ("this is id 6", resource_bundle->GetRawDataResourceForScale(6, k200Percent));
}
...
TEST_F(ResourceBundleImageTest, GetImageNamed)
{
...
#endif
    test::ScopedSetSupportedResourceScaleFactors scoped_supported({ k100Percent, k200Percent });
    ...
    EXPECT_EQ(ui::k200Percent, GetSupportedResourceScaleFactor(image_skia->image_reps()[0].scale()));
#else
    EXPECT_EQ(ui::k100Percent, GetSupportedResourceScaleFactor(image_skia->image_reps()[0].scale()));
#endif

    // Resource ID 3 exists in both 1x and 2x paks. Image reps should be
    // available for both scale factors in |image_skia|.
    gfx::ImageSkiaRep image_rep = image_skia->GetRepresentation(GetScaleForResourceScaleFactor(ui::k100Percent));
    EXPECT_EQ(ui::k100Percent, GetSupportedResourceScaleFactor(image_rep.scale()));
    image_rep = image_skia->GetRepresentation(GetScaleForResourceScaleFactor(ui::k200Percent));
    EXPECT_EQ(ui::k200Percent, GetSupportedResourceScaleFactor(image_rep.scale()));

    // Requesting the 1.4x resource should return either the 1x or the 2x
    // resource.
    image_rep = image_skia->GetRepresentation(1.4f);
    ResourceScaleFactor scale_factor = GetSupportedResourceScaleFactor(image_rep.scale());
    EXPECT_TRUE(scale_factor == ui::k100Percent || scale_factor == ui::k200Percent);
...
TEST_F(ResourceBundleImageTest, GetImageNamedFallback1x)
{
    ...
    CreateDataPackWithSingleBitmap(data_2x_path, 10, std::string_view(reinterpret_cast<const char*>(kPngScaleChunk), std::size(kPngScaleChunk)));
    ...
    // proper 2x size.
    gfx::ImageSkiaRep image_rep = image_skia->GetRepresentation(GetScaleForResourceScaleFactor(ui::k200Percent));
    EXPECT_EQ(ui::k200Percent, GetSupportedResourceScaleFactor(image_rep.scale()));
...
TEST_F(ResourceBundleImageTest, FallbackToNone)
{
    ...
    test::ScopedSetSupportedResourceScaleFactors scoped_supported({ k100Percent, k200Percent, k300Percent });
...
{
    // Create the pak files.
    const base::FilePath data_unscaled_path = dir_path().AppendASCII("sample.pak");
    const std::map<uint16_t, std::string_view> resources = { std::make_pair(3u, kLottieData) };
...
#if BUILDFLAG(IS_CHROMEOS)
    ui::ResourceBundle::SetLottieParsingFunctions(&ParseLottieAsStillImageForTesting,
