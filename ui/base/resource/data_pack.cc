...
static const size_t kHeaderLengthV5 = sizeof(uint32_t) + sizeof(uint8_t) * 4 + sizeof(uint16_t) * 2;
...
void MaybePrintResourceId(uint16_t resource_id)
{
    static const bool print_resource_ids = [] {
        ...
        return base::CommandLine::InitializedForCurrentProcess() && base::CommandLine::ForCurrentProcess()->HasSwitch("print-resource-ids");
    }();
...
bool MmapHasGzipHeader(const base::MemoryMappedFile* mmap)
{
    ...
    net::GZipHeader::Status header_status = header.ReadMore(reinterpret_cast<const char*>(mmap->data()), mmap->length(), &header_end);
...
void DataPack::Iterator::UpdateResourceData()
{
    ...
    resource_data_ = new ResourceData(entry_->resource_id, GetStringViewFromOffset(entry_->file_offset, next_entry->file_offset, data_source_));
}
...
// static
std::unique_ptr<DataPack::DataSource> DataPack::LoadFromPathInternal(const base::FilePath& path)
{
    std::unique_ptr<base::MemoryMappedFile> mmap = std::make_unique<base::MemoryMappedFile>();
    ...
    base::File data_file(path, base::File::FLAG_OPEN | base::File::FLAG_READ | base::File::FLAG_WIN_EXCLUSIVE_WRITE | base::File::FLAG_WIN_SHARE_DELETE);
    if (!data_file.IsValid()) {
        DLOG(ERROR) << "Failed to open datapack with base::File::Error " << data_file.error_details();
        return nullptr;
    }
    ...
    if (MmapHasGzipHeader(mmap.get())) {
        std::string_view compressed(reinterpret_cast<char*>(mmap->data()), mmap->length());
        std::string data;
...
{
    return LoadFromFileRegion(std::move(file), base::MemoryMappedFile::Region::kWholeFile);
}

bool DataPack::LoadFromFileRegion(base::File file, const base::MemoryMappedFile::Region& region)
{
    std::unique_ptr<base::MemoryMappedFile> mmap = std::make_unique<base::MemoryMappedFile>();
...
bool DataPack::SanityCheckFileAndRegisterResources(size_t margin_to_skip, const uint8_t* data, size_t data_length)
{
    ...
    if (margin_to_skip + resource_table_size + alias_table_size > data_length) {
        ...
        LOG(ERROR) << "Data pack file corruption: "
                   ...
                   << "data length is " << data_length << " bytes, expected longer than " << margin_to_skip + resource_table_size + alias_table_size
                   << " bytes.";
        return false;
    }

    resource_table_ = reinterpret_cast<const Entry*>(&data[margin_to_skip]);
    alias_table_ = reinterpret_cast<const Alias*>(&data[margin_to_skip + resource_table_size]);
...
bool DataPack::LoadImpl(std::unique_ptr<DataPack::DataSource> data_source)
{
    ...
    size_t header_length = version == kFileFormatV4 ? kHeaderLengthV4 : kHeaderLengthV5;
    ...
    if (version == kFileFormatV4) {
        ...
        LOG(ERROR) << "Bad data pack version: got " << version << ", expected " << kFileFormatV4 << " or " << kFileFormatV5;
        return false;
    }

    if (text_encoding_type_ != UTF8 && text_encoding_type_ != UTF16 && text_encoding_type_ != BINARY) {
        LOG(ERROR) << "Bad data pack text encoding: got " << text_encoding_type_ << ", expected between " << BINARY << " and " << UTF16;
        return false;
    }
...
const DataPack::Entry* DataPack::LookupEntryById(uint16_t resource_id) const
{
    ...
    const Entry* ret = reinterpret_cast<const Entry*>(bsearch(&resource_id, resource_table_, resource_count_, sizeof(Entry), Entry::CompareById));
    if (ret == nullptr) {
        // Search the alias table for the ~10% of entries which are aliases.
        const Alias* alias = reinterpret_cast<const Alias*>(bsearch(&resource_id, alias_table_, alias_count_, sizeof(Alias), Alias::CompareById));
        if (alias != nullptr) {
...
// static
std::string_view DataPack::GetStringViewFromOffset(uint32_t target_offset, uint32_t next_offset, const uint8_t* data_source)
{
    size_t length = next_offset - target_offset;
    return { reinterpret_cast<const char*>(data_source + target_offset), length };
}

std::optional<std::string_view> DataPack::GetStringView(uint16_t resource_id) const
{
    ...
    size_t entry_offset = reinterpret_cast<const uint8_t*>(next_entry) - data_source_->GetData();
    ...
    if (target->file_offset > next_entry->file_offset) {
        ...
        LOG(ERROR) << "Entry #" << next_index << " in data pack is before Entry #" << entry_index << ". This should have been caught when loading. "
                   << "Was the file modified?";
        return std::nullopt;
    }

    MaybePrintResourceId(resource_id);
    return GetStringViewFromOffset(target->file_offset, next_entry->file_offset, data_source_->GetData());
}

base::RefCountedStaticMemory* DataPack::GetStaticMemory(uint16_t resource_id) const
{
...
#if DCHECK_IS_ON()
void DataPack::CheckForDuplicateResources(const std::vector<std::unique_ptr<ResourceHandle>>& packs)
{
    for (size_t i = 0; i < resource_count_ + 1; ++i) {
        ...
        for (const auto& handle : packs) {
            if (GetScaleForResourceScaleFactor(handle->GetResourceScaleFactor()) != resource_scale)
                continue;
            DCHECK(!handle->HasResource(resource_id)) << "Duplicate resource " << resource_id << " with scale " << resource_scale;
        }
    }
}
#endif // DCHECK_IS_ON()
...
// static
bool DataPack::WritePack(const base::FilePath& path, const std::map<uint16_t, std::string_view>& resources, TextEncodingType text_encoding_type)
{
    if (text_encoding_type != UTF8 && text_encoding_type != UTF16 && text_encoding_type != BINARY) {
        LOG(ERROR) << "Invalid text encoding type, got " << text_encoding_type << ", expected between " << BINARY << " and " << UTF16;
        return false;
    }
    ...
    DCHECK_EQ(static_cast<size_t>(entry_count) + static_cast<size_t>(alias_count), resources_count);
