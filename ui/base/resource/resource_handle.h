...
public:
    ...
    enum TextEncodingType { BINARY, UTF8, UTF16 };
    ...
    virtual std::optional<std::string_view> GetStringView(uint16_t resource_id) const = 0;

    // Like GetStringView(), but returns a reference to memory.
    // Caller owns the returned object.
    virtual base::RefCountedStaticMemory* GetStaticMemory(uint16_t resource_id) const = 0;
    ...
    // of resources.
    virtual void CheckForDuplicateResources(const std::vector<std::unique_ptr<ResourceHandle>>& packs) = 0;
#endif
};
