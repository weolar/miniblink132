...
bool LoadFromApkOrFile(const char* apk_path, const base::FilePath* disk_path, int* out_fd, base::MemoryMappedFile::Region* out_region)
{
    ...
    if (*out_fd < 0 && disk_path != nullptr) {
        auto flags = static_cast<uint32_t>(base::File::FLAG_OPEN | base::File::FLAG_READ);
...
int LoadLocalePakFromApk(const std::string& app_locale, bool in_split, base::MemoryMappedFile::Region* out_region)
{
    bool log_error = true;
    std::string locale_path_within_apk = GetPathForAndroidLocalePakWithinApk(app_locale, in_split, log_error);
...
std::unique_ptr<DataPack> LoadDataPackFromLocalePak(int locale_pack_fd, const base::MemoryMappedFile::Region& region)
{
    auto data_pack = std::make_unique<DataPack>(k100Percent);
    CHECK(data_pack->LoadFromFileRegion(base::File(locale_pack_fd), region)) << "failed to load locale.pak";
    return data_pack;
}
...
void ResourceBundle::LoadCommonResources()
{
    ...
    bool success = LoadFromApkOrFile("assets/chrome_100_percent.pak", &disk_path, &g_chrome_100_percent_fd, &g_chrome_100_percent_region);
    DCHECK(success);

    AddDataPackFromFileRegion(base::File(g_chrome_100_percent_fd), g_chrome_100_percent_region, k100Percent);
}
...
bool ResourceBundle::LocaleDataPakExists(const std::string& locale)
{
    ...
    if (!in_split) {
        return !GetPathForAndroidLocalePakWithinApk(locale, in_split, log_error).empty();
    }
...
{
    DCHECK(!locale_resources_data_.get() && !secondary_locale_resources_data_.get()) << "locale.pak already loaded";
    ...
    if (g_locale_paks_in_apk) {
        g_locale_pack_fd = LoadLocalePakFromApk(app_locale, false, &g_locale_pack_region);
    } else {
        // Support overridden pak path for testing.
        base::FilePath locale_file_path = GetOverriddenPakPath();
        if (locale_file_path.empty()) {
            // Try to find the uncompressed split-specific asset file.
            g_locale_pack_fd = LoadLocalePakFromApk(app_locale, true, &g_locale_pack_region);
        }
    ...
    locale_resources_data_ = LoadDataPackFromLocalePak(g_locale_pack_fd, g_locale_pack_region);
    ...
    if (g_load_secondary_locale_paks) {
        g_secondary_locale_pack_fd = LoadLocalePakFromApk(app_locale, false, &g_secondary_locale_pack_region);

        secondary_locale_resources_data_ = LoadDataPackFromLocalePak(g_secondary_locale_pack_fd, g_secondary_locale_pack_region);
...
void LoadMainAndroidPackFile(const char* path_within_apk, const base::FilePath& disk_file_path)
{
    if (LoadFromApkOrFile(path_within_apk, &disk_file_path, &g_resources_pack_fd, &g_resources_pack_region)) {
        ResourceBundle::GetSharedInstance().AddDataPackFromFileRegion(base::File(g_resources_pack_fd), g_resources_pack_region, kScaleFactorNone);
    }
}

void LoadPackFileFromApk(const std::string& path, const std::string& split_name)
{
    base::MemoryMappedFile::Region region;
    int fd = base::android::OpenApkAsset(path, split_name, &region);
    CHECK_GE(fd, 0) << "Could not find " << path << " in APK.";
    ui::ResourceBundle::GetSharedInstance().AddDataPackFromFileRegion(base::File(fd), region, ui::kScaleFactorNone);
}
...
std::string GetPathForAndroidLocalePakWithinApk(const std::string& locale, bool in_bundle, bool log_error)
{
    JNIEnv* env = base::android::AttachCurrentThread();
    base::android::ScopedJavaLocalRef<jstring> ret
        = Java_ResourceBundle_getLocalePakResourcePath(env, base::android::ConvertUTF8ToJavaString(env, locale), in_bundle, log_error);
    if (ret.obj() == nullptr) {
        return std::string();
    }
...
{
    Java_ResourceBundle_setNoAvailableLocalePaks(base::android::AttachCurrentThread());
}
