...
#pragma pack(pop)
    ...
    static std::unique_ptr<DataSource> LoadFromPathInternal(const base::FilePath& path);
    ...
    // |data_source_|.
    bool LoadFromFileRegion(base::File file, const base::MemoryMappedFile::Region& region);
    ...
    // indicate BINARY.
    static bool WritePack(const base::FilePath& path, const std::map<uint16_t, std::string_view>& resources, TextEncodingType textEncodingType);

    // ResourceHandle implementation:
    bool HasResource(uint16_t resource_id) const override;
    std::optional<std::string_view> GetStringView(uint16_t resource_id) const override;
    base::RefCountedStaticMemory* GetStaticMemory(uint16_t resource_id) const override;
    ...
    // of resources.
    void CheckForDuplicateResources(const std::vector<std::unique_ptr<ResourceHandle>>& packs) override;
#endif
    ...
    size_t GetAliasTableSize() const
    {
        return alias_count_;
    }
    ...
    size_t GetResourceTableSizeForTesting() const
    {
...
private:
    ...
    bool SanityCheckFileAndRegisterResources(size_t margin_to_skip, const uint8_t* data, size_t data_length);

    // Returns the string between `target_offset` and `next_offset` in data pack.
    static std::string_view GetStringViewFromOffset(uint32_t target_offset, uint32_t next_offset, const uint8_t* data_source);
