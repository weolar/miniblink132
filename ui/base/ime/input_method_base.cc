...
InputMethodBase::InputMethodBase(ImeKeyEventDispatcher* ime_key_event_dispatcher)
    : InputMethodBase(ime_key_event_dispatcher, nullptr)
{
}

InputMethodBase::InputMethodBase(ImeKeyEventDispatcher* ime_key_event_dispatcher, std::unique_ptr<VirtualKeyboardController> keyboard_controller)
...
void InputMethodBase::SetImeKeyEventDispatcher(ImeKeyEventDispatcher* ime_key_event_dispatcher)
{
    ime_key_event_dispatcher_ = ime_key_event_dispatcher;
}
...
TextInputType InputMethodBase::GetTextInputType() const
{
    ...
    return client ? (client->GetTextInputFlags() & TEXT_INPUT_FLAG_HAS_BEEN_PASSWORD ? TEXT_INPUT_TYPE_PASSWORD : client->GetTextInputType())
                  : TEXT_INPUT_TYPE_NONE;
}

void InputMethodBase::SetVirtualKeyboardVisibilityIfEnabled(bool should_show)
{
    observer_list_.Notify(&InputMethodObserver::OnVirtualKeyboardVisibilityChangedIfEnabled, should_show);
...
void InputMethodBase::SetVirtualKeyboardControllerForTesting(std::unique_ptr<VirtualKeyboardController> controller)
{
...
ui::EventDispatchDetails InputMethodBase::DispatchKeyEventPostIME(ui::KeyEvent* event) const
{
    if (text_input_client_) {
    ...
    return ime_key_event_dispatcher_ ? ime_key_event_dispatcher_->DispatchKeyEventPostIME(event) : ui::EventDispatchDetails();
}

void InputMethodBase::NotifyTextInputStateChanged(const TextInputClient* client)
{
    observer_list_.Notify(&InputMethodObserver::OnTextInputStateChanged, client);
}

void InputMethodBase::NotifyTextInputCaretBoundsChanged(const TextInputClient* client)
{
    observer_list_.Notify(&InputMethodObserver::OnCaretBoundsChanged, client);
}

void InputMethodBase::SetFocusedTextInputClientInternal(TextInputClient* client)
{
