...
VirtualKeyboardControllerFuchsia::VirtualKeyboardControllerFuchsia(fuchsia_ui_views::ViewRef view_ref, ui::InputMethodBase* input_method)
    : input_method_(input_method)
{
    DCHECK(input_method_);

    auto controller_creator_client_end = base::fuchsia_component::Connect<fuchsia_input_virtualkeyboard::ControllerCreator>();
    if (!controller_creator_client_end.is_ok()) {
        LOG(ERROR) << base::FidlConnectionErrorMessage(controller_creator_client_end);
        return;
    }
    fidl::Client controller_creator(std::move(controller_creator_client_end.value()), async_get_default_dispatcher());

    auto controller_endpoints = fidl::CreateEndpoints<fuchsia_input_virtualkeyboard::Controller>();
    ...
    controller_client_.Bind(std::move(controller_endpoints->client), async_get_default_dispatcher(), &controller_error_logger_);
...
bool VirtualKeyboardControllerFuchsia::DisplayVirtualKeyboard()
{
    ...
    if (result.is_error()) {
        ZX_DLOG(ERROR, result.error_value().status()) << "Error calling RequestShow()";
        return false;
    }
...
void VirtualKeyboardControllerFuchsia::DismissVirtualKeyboard()
{
    ...
    if (result.is_error()) {
        ZX_DLOG(ERROR, result.error_value().status()) << "Error calling RequestHide()";
    }
}

void VirtualKeyboardControllerFuchsia::AddObserver(VirtualKeyboardControllerObserver* observer)
{
}

void VirtualKeyboardControllerFuchsia::RemoveObserver(VirtualKeyboardControllerObserver* observer)
{
}
...
void VirtualKeyboardControllerFuchsia::WatchVisibility()
{
    ...
    controller_client_->WatchVisibility().Then(fit::bind_member(this, &VirtualKeyboardControllerFuchsia::OnVisibilityChange));
}

void VirtualKeyboardControllerFuchsia::OnVisibilityChange(const fidl::Result<fuchsia_input_virtualkeyboard::Controller::WatchVisibility>& result)
{
    if (result.is_error()) {
...
fuchsia_input_virtualkeyboard::TextType VirtualKeyboardControllerFuchsia::GetFocusedTextType() const
{
...
void VirtualKeyboardControllerFuchsia::UpdateTextType()
{
    ...
    DVLOG(1) << "UpdateTextType() called (current: " << requested_type_ << ", new: " << new_type << ")";
    if (new_type != requested_type_) {
        auto result = controller_client_->SetTextType(new_type);
        if (result.is_error()) {
            ZX_DLOG(ERROR, result.error_value().status()) << "Error calling SetTextType()";
        }
