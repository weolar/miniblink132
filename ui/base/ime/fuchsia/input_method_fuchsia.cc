...
InputMethodFuchsia::InputMethodFuchsia(bool enable_virtual_keyboard, ImeKeyEventDispatcher* ime_key_event_dispatcher, fuchsia_ui_views::ViewRef view_ref)
...
{
    return virtual_keyboard_controller_ ? &virtual_keyboard_controller_.value() : nullptr;
}

ui::EventDispatchDetails InputMethodFuchsia::DispatchKeyEvent(ui::KeyEvent* event)
{
    ...
    if (!event->stopped_propagation() && !dispatch_details.dispatcher_destroyed && event->type() == EventType::kKeyPressed && GetTextInputClient()) {
...
void InputMethodFuchsia::OnTextInputTypeChanged(TextInputClient* client)
{
    ...
    if (client->GetTextInputType() == TEXT_INPUT_TYPE_NONE || client->GetTextInputMode() == TEXT_INPUT_MODE_NONE) {
        virtual_keyboard_controller_->DismissVirtualKeyboard();
    } else {
