...
class COMPONENT_EXPORT(UI_BASE_IME_FUCHSIA) KeyboardClient : public fidl::Server<fuchsia_ui_input3::KeyboardListener> {
public:
    // `keyboard_service` and `event_sink` must outlive `this`.
    KeyboardClient(fidl::Client<fuchsia_ui_input3::Keyboard>& keyboard_fidl_client, fuchsia_ui_views::ViewRef view_ref, InputEventSink* event_sink);
    ...
    void OnKeyEvent(OnKeyEventRequest& request, OnKeyEventCompleter::Sync& completer) final;
...
private:
    ...
    std::optional<fidl::ServerBinding<fuchsia_ui_input3::KeyboardListener>> binding_;
