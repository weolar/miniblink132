...
MockInputMethod::MockInputMethod(ImeKeyEventDispatcher* ime_key_event_dispatcher)
...
void MockInputMethod::SetImeKeyEventDispatcher(ImeKeyEventDispatcher* ime_key_event_dispatcher)
{
    ime_key_event_dispatcher_ = ime_key_event_dispatcher;
}
...
ui::EventDispatchDetails MockInputMethod::DispatchKeyEvent(ui::KeyEvent* event)
{
...
#if BUILDFLAG(IS_WIN)
bool MockInputMethod::OnUntranslatedIMEMessage(const CHROME_MSG event, NativeEventResult* result)
{
    if (result)
...
void MockInputMethod::SetVirtualKeyboardControllerForTesting(std::unique_ptr<VirtualKeyboardController> controller)
{
}
