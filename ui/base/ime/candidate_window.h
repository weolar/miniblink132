...
public:
    ...
    void GetInfolistEntries(std::vector<InfolistEntry>* entries, bool* has_highlighted) const;

    // Returns the number of candidates in one page.
    uint32_t page_size() const
    {
        return property_->page_size;
    }
    void set_page_size(uint32_t page_size)
    {
        property_->page_size = page_size;
    }

    // Returns the cursor index of the currently selected candidate.
    uint32_t cursor_position() const
    {
        return property_->cursor_position;
    }
    ...
    {
        return property_->is_cursor_visible;
    }
    ...
    const int& total_candidates() const
    {
        return property_->total_candidates;
    }
    ...
    bool is_user_selecting() const
    {
        return property_->is_user_selecting;
    }
