...
public:
    ...
    ImeKeyEventDispatcherForTesting(const ImeKeyEventDispatcherForTesting&) = delete;
    ImeKeyEventDispatcherForTesting& operator=(const ImeKeyEventDispatcherForTesting&) = delete;
...
protected:
    ...
    void SetUp() override
    {
        ...
        input_method_minimal_ = std::make_unique<InputMethodMinimal>(dispatcher_.get());
...
TEST_F(InputMethodMinimalTest, StopPropagationTest)
{
    ...
    std::unique_ptr<DummyTextInputClient> client = std::make_unique<DummyTextInputClient>();
    ...
    std::unique_ptr<ImeKeyEventDispatcherForTesting> dispatcher_no_propagation = std::make_unique<ImeKeyEventDispatcherForTesting>(false);
    input_method_minimal_->SetImeKeyEventDispatcher(dispatcher_no_propagation.get());
    input_method_minimal_->DispatchKeyEvent(&key);
