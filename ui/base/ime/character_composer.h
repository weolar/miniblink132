...
public:
    ...
    explicit CharacterComposer(PreeditStringMode mode = PreeditStringMode::kHexModeOnly);
    ...
    const std::u16string& preedit_string() const
    {
        return preedit_string_;
    }
...
public:
    ...
    virtual CheckSequenceResult CheckSequence(const ui::CharacterComposer::ComposeBuffer& sequence, uint32_t* composed_character) const = 0;
};
...
public:
    ...
    explicit TreeComposeChecker(const CompositionData& data)
        : data_(data)
    {
    }
    CheckSequenceResult CheckSequence(const ui::CharacterComposer::ComposeBuffer& sequence, uint32_t* composed_character) const override;
