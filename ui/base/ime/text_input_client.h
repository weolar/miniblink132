...
public:
    ...
    virtual void InsertText(const std::u16string& text, InsertTextCursorBehavior cursor_behavior) = 0;
    ...
    virtual std::optional<gfx::Rect> GetProximateCharacterBounds(const gfx::Range& range) const = 0;
    ...
    // mechanism is to mitigate performance costs (CPU and memory) when processing
    // very large documents.
    virtual std::optional<size_t> GetProximateCharacterIndexFromPoint(const gfx::Point& point, IndexFromPointFlags flags) const = 0;
#endif // BUILDFLAG(IS_WIN)
    ...
    virtual bool GetCompositionCharacterBounds(size_t index, gfx::Rect* rect) const = 0;
    ...
    virtual bool GetTextFromRange(const gfx::Range& range, std::u16string* text) const = 0;
    ...
    // Windows.
    // Returns false if the operation is not supported.
    virtual bool ChangeTextDirectionAndLayoutAlignment(base::i18n::TextDirection direction) = 0;
    ...
    virtual void ExtendSelectionAndReplace(size_t length_before_selection, size_t length_after_selection, std::u16string_view replacement_string);
#endif
    ...
    virtual bool SetCompositionFromExistingText(const gfx::Range& range, const std::vector<ui::ImeTextSpan>& ui_ime_text_spans) = 0;
#endif
    ...
    // is successful.
    virtual bool AddGrammarFragments(const std::vector<GrammarFragment>& fragments);
    ...
    // ChromeOS.
    virtual void GetActiveTextInputControlLayoutBounds(std::optional<gfx::Rect>* control_bounds, std::optional<gfx::Rect>* selection_bounds) = 0;
#endif
    ...
    virtual void SetActiveCompositionForAccessibility(const gfx::Range& range, const std::u16string& active_composition_text, bool is_composition_committed)
        = 0;
#endif
...
#if BUILDFLAG(IS_WIN)
inline constexpr ui::IndexFromPointFlags operator&(ui::IndexFromPointFlags a, ui::IndexFromPointFlags b)
{
    using T = std::underlying_type_t<ui::IndexFromPointFlags>;
    return static_cast<ui::IndexFromPointFlags>(static_cast<T>(a) & static_cast<T>(b));
}
