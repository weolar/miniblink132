...
public:
    ...
    MOCK_METHOD2(InsertText, void(const std::u16string&, ui::TextInputClient::InsertTextCursorBehavior cursor_behavior));
    ...
    MOCK_CONST_METHOD1(GetProximateCharacterBounds, std::optional<gfx::Rect>(const gfx::Range&));
    MOCK_CONST_METHOD2(GetProximateCharacterIndexFromPoint, std::optional<size_t>(const gfx::Point&, IndexFromPointFlags));
    ...
    MOCK_CONST_METHOD2(GetTextFromRange, bool(const gfx::Range&, std::u16string*));
    MOCK_METHOD0(OnInputMethodChanged, void());
    MOCK_METHOD1(ChangeTextDirectionAndLayoutAlignment, bool(base::i18n::TextDirection));
    ...
    MOCK_METHOD2(SetCompositionFromExistingText, bool(const gfx::Range&, const std::vector<ui::ImeTextSpan>&));
    MOCK_METHOD3(SetActiveCompositionForAccessibility, void(const gfx::Range&, const std::u16string&, bool));
    MOCK_METHOD2(GetActiveTextInputControlLayoutBounds, void(std::optional<gfx::Rect>* control_bounds, std::optional<gfx::Rect>* selection_bounds));
...
public:
    ...
    // ITextStoreACPSink
    MOCK_METHOD2_WITH_CALLTYPE(STDMETHODCALLTYPE, OnTextChange, HRESULT(DWORD, const TS_TEXTCHANGE*));
    MOCK_METHOD0_WITH_CALLTYPE(STDMETHODCALLTYPE, OnSelectionChange, HRESULT());
    MOCK_METHOD2_WITH_CALLTYPE(STDMETHODCALLTYPE, OnLayoutChange, HRESULT(TsLayoutCode, TsViewCookie));
    MOCK_METHOD1_WITH_CALLTYPE(STDMETHODCALLTYPE, OnStatusChange, HRESULT(DWORD));
    MOCK_METHOD4_WITH_CALLTYPE(STDMETHODCALLTYPE, OnAttrsChange, HRESULT(LONG, LONG, ULONG, const TS_ATTRID*));
    MOCK_METHOD1_WITH_CALLTYPE(STDMETHODCALLTYPE, OnLockGranted, HRESULT(DWORD));
    MOCK_METHOD0_WITH_CALLTYPE(STDMETHODCALLTYPE, OnStartEditTransaction, HRESULT());
    MOCK_METHOD0_WITH_CALLTYPE(STDMETHODCALLTYPE, OnEndEditTransaction, HRESULT());
...
protected:
    void SetUp() override
    {
        ...
        EXPECT_EQ(S_OK, text_store_->AdviseSink(IID_ITextStoreACPSink, sink_.get(), TS_AS_ALL_SINKS));
    ...
    {
        return &text_store_->composition_start_;
    }
...
protected:
    ...
    bool* edit_flag()
    {
        return &text_store_->edit_flag_;
    }
    bool* new_text_inserted()
    {
        return &text_store_->new_text_inserted_;
    }
    std::u16string* string_buffer()
    {
        return &text_store_->string_buffer_document_;
    ...
    gfx::Range* selection()
    {
        return &text_store_->selection_;
    }
    ImeTextSpans* text_spans()
    {
        return &text_store_->text_spans_;
    }
    gfx::Range* composition_range()
    {
        return &text_store_->composition_range_;
    }
    bool* has_composition_range()
    {
        return &text_store_->has_composition_range_;
    }

    void SetInternalState(const std::u16string& new_string_buffer, LONG new_composition_start, LONG new_selection_start, LONG new_selection_end)
    {
    ...
    {
        return text_store_->HasReadLock();
    }
    bool HasReadWriteLock() const
    {
        return text_store_->HasReadWriteLock();
    }
    ...
    void SetTextTest(LONG acp_start, LONG acp_end, const std::wstring& text, HRESULT error_code)
    {
        TS_TEXTCHANGE change = {};
        ASSERT_EQ(error_code, text_store_->SetText(0, acp_start, acp_end, text.c_str(), text.size(), &change));
        if (error_code == S_OK) {
    ...
    void GetTextTest(LONG acp_start, LONG acp_end, const std::wstring& expected_string, LONG expected_next_acp)
    {
        wchar_t buffer[1024] = {};
        ...
        LONG next_acp = 0;
        ASSERT_EQ(S_OK, text_store_->GetText(acp_start, acp_end, buffer, 1024, &text_buffer_copied, &run_info, 1, &run_info_buffer_copied, &next_acp));
        ASSERT_EQ(expected_string.size(), text_buffer_copied);
        EXPECT_EQ(expected_string, std::wstring(buffer, buffer + text_buffer_copied));
    ...
    void GetTextErrorTest(LONG acp_start, LONG acp_end, HRESULT error_code)
    {
        ...
        ULONG run_info_buffer_copied = 0;
        LONG next_acp = 0;
        EXPECT_EQ(error_code, text_store_->GetText(acp_start, acp_end, buffer, 1024, &text_buffer_copied, &run_info, 1, &run_info_buffer_copied, &next_acp));
    }

    void InsertTextAtSelectionTest(const wchar_t* buffer, ULONG buffer_size, LONG expected_start, LONG expected_end, LONG expected_change_start,
        LONG expected_change_old_end, LONG expected_change_new_end)
    {
        LONG start = 0;
        LONG end = 0;
        TS_TEXTCHANGE change = {};
        EXPECT_EQ(S_OK, text_store_->InsertTextAtSelection(0, buffer, buffer_size, &start, &end, &change));
    ...
    void InsertTextAtSelectionQueryOnlyTest(const wchar_t* buffer, ULONG buffer_size, LONG expected_start, LONG expected_end)
    {
        LONG start = 0;
        LONG end = 0;
        EXPECT_EQ(S_OK, text_store_->InsertTextAtSelection(TS_IAS_QUERYONLY, buffer, buffer_size, &start, &end, nullptr));
        EXPECT_EQ(expected_start, start);
        EXPECT_EQ(expected_end, end);
    }

    void GetTextExtTest(
        TsViewCookie view_cookie, LONG acp_start, LONG acp_end, LONG expected_left, LONG expected_top, LONG expected_right, LONG expected_bottom)
    {
        RECT rect = {};
        BOOL clipped = FALSE;
        EXPECT_EQ(S_OK, text_store_->GetTextExt(view_cookie, acp_start, acp_end, &rect, &clipped));
    ...
    void GetTextExtNoLayoutTest(TsViewCookie view_cookie, LONG acp_start, LONG acp_end)
    {
        RECT rect = {};
        BOOL clipped = FALSE;
        EXPECT_EQ(TS_E_NOLAYOUT, text_store_->GetTextExt(view_cookie, acp_start, acp_end, &rect, &clipped));
    }
    ...
    void ResetCompositionStateTest()
    {
        ...
        EXPECT_EQ(gfx::Range::InvalidRange(), text_store_->previous_composition_selection_range_);
        ...
        EXPECT_EQ(text_store_->composition_from_client_.end(), text_store_->selection_.start());
        EXPECT_EQ(text_store_->composition_from_client_.end(), text_store_->selection_.end());
...
TEST_F(TSFTextStoreTest, GetStatusTest)
{
    ...
    EXPECT_EQ((ULONG)(TS_SS_TRANSITORY | TS_SS_NOHIDDENTEXT), status.dwStaticFlags);

    text_store_->UseEmptyTextStore(true);
    status = {};
    EXPECT_CALL(text_input_client_, GetTextInputType()).WillRepeatedly(Return(TEXT_INPUT_TYPE_NONE));
    EXPECT_EQ(S_OK, text_store_->GetStatus(&status));
    EXPECT_EQ((ULONG)TS_SD_READONLY, status.dwDynamicFlags & TS_SD_READONLY);

    status = {};
    text_store_->UseEmptyTextStore(false);
    EXPECT_CALL(text_input_client_, GetTextInputType()).WillRepeatedly(Return(TEXT_INPUT_TYPE_TEXT));
    EXPECT_EQ(S_OK, text_store_->GetStatus(&status));
...
TEST_F(TSFTextStoreTest, DummyLockTest)
{
    ...
    text_store_->UseEmptyTextStore(false);
    EXPECT_CALL(text_input_client_, GetTextInputType()).WillRepeatedly(Return(TEXT_INPUT_TYPE_TEXT));
    EXPECT_EQ(S_OK, text_store_->RequestLock(TS_LF_READWRITE | TS_LF_SYNC, &result));

    text_store_->UseEmptyTextStore(true);
    EXPECT_EQ(E_FAIL, text_store_->RequestLock(TS_LF_READWRITE | TS_LF_SYNC, &result));
    EXPECT_CALL(text_input_client_, GetTextInputType()).WillRepeatedly(Return(TEXT_INPUT_TYPE_NONE));
    EXPECT_EQ(E_FAIL, text_store_->RequestLock(TS_LF_READWRITE | TS_LF_SYNC, &result));
}
...
TEST_F(TSFTextStoreTest, QueryInsertTest)
{
    ...
    *composition_start() = 0;
    EXPECT_EQ(E_INVALIDARG, text_store_->QueryInsert(0, 0, 0, nullptr, &result_end));
    EXPECT_EQ(E_INVALIDARG, text_store_->QueryInsert(0, 0, 0, &result_start, nullptr));
    EXPECT_EQ(S_OK, text_store_->QueryInsert(0, 0, 0, &result_start, &result_end));
    EXPECT_EQ(0, result_start);
    EXPECT_EQ(0, result_end);
    *string_buffer() = u"1234";
    *composition_start() = 1;
    EXPECT_EQ(S_OK, text_store_->QueryInsert(0, 1, 0, &result_start, &result_end));
    EXPECT_EQ(1, result_start);
    EXPECT_EQ(1, result_end);
    EXPECT_EQ(E_INVALIDARG, text_store_->QueryInsert(1, 0, 0, &result_start, &result_end));
    EXPECT_EQ(S_OK, text_store_->QueryInsert(2, 2, 0, &result_start, &result_end));
    EXPECT_EQ(2, result_start);
    EXPECT_EQ(2, result_end);
    EXPECT_EQ(S_OK, text_store_->QueryInsert(2, 3, 0, &result_start, &result_end));
    EXPECT_EQ(2, result_start);
    EXPECT_EQ(3, result_end);
    EXPECT_EQ(E_INVALIDARG, text_store_->QueryInsert(3, 2, 0, &result_start, &result_end));
    EXPECT_EQ(S_OK, text_store_->QueryInsert(3, 4, 0, &result_start, &result_end));
    EXPECT_EQ(3, result_start);
    EXPECT_EQ(4, result_end);
    EXPECT_EQ(S_OK, text_store_->QueryInsert(3, 5, 0, &result_start, &result_end));
    EXPECT_EQ(3, result_start);
    EXPECT_EQ(4, result_end);

    *string_buffer() = u"";
    *composition_start() = 2;
    EXPECT_EQ(S_OK, text_store_->QueryInsert(0, 2, 5, &result_start, &result_end));
    EXPECT_EQ(0, result_start);
...
public:
    ...
    SyncRequestLockTestCallback& operator=(const SyncRequestLockTestCallback&) = delete;
...
public:
    ...
    AsyncRequestLockTestCallback& operator=(const AsyncRequestLockTestCallback&) = delete;
...
public:
    ...
    RequestLockTextChangeTestCallback(const RequestLockTextChangeTestCallback&) = delete;
    RequestLockTextChangeTestCallback& operator=(const RequestLockTextChangeTestCallback&) = delete;
    ...
    void InsertText(const std::u16string& text, ui::TextInputClient::InsertTextCursorBehavior cursor_behavior)
    {
        EXPECT_EQ(2, state_);
...
TEST_F(TSFTextStoreTest, RequestLockOnTextChangeTest)
{
    ...
    EXPECT_CALL(*sink_, OnLockGranted(_))
        .WillOnce(Invoke(&callback, &RequestLockTextChangeTestCallback::LockGranted1))
        .WillOnce(Invoke(&callback, &RequestLockTextChangeTestCallback::LockGranted2));

    EXPECT_CALL(*sink_, OnSelectionChange()).WillOnce(Invoke(&callback, &RequestLockTextChangeTestCallback::OnSelectionChange));
    EXPECT_CALL(text_input_client_, InsertText(_, _)).WillOnce(Invoke(&callback, &RequestLockTextChangeTestCallback::InsertText));
    EXPECT_CALL(text_input_client_, GetEditableSelectionRange(_)).WillOnce(Invoke(&callback, &RequestLockTextChangeTestCallback::GetEditableSelectionRange));
    EXPECT_CALL(text_input_client_, GetTextFromRange(_, _)).WillOnce(Invoke(&callback, &RequestLockTextChangeTestCallback::GetTextFromRange));
    EXPECT_CALL(text_input_client_, GetTextRange(_)).WillOnce(Invoke(&callback, &RequestLockTextChangeTestCallback::GetTextRange));

    ON_CALL(text_input_client_, GetCompositionTextRange(_)).WillByDefault(Invoke(&callback, &TSFTextStoreTestCallback::GetCompositionTextRange));
...
TEST_F(TSFTextStoreTest, SetGetSelectionTest)
{
    ...
    EXPECT_CALL(*sink_, OnLockGranted(_))
        ...
        .WillOnce(Invoke(&callback, &SelectionTestCallback::ReadWriteLockGranted));

    TS_SELECTION_ACP selection_buffer = {};
    ULONG fetched_count = 0;
    EXPECT_EQ(TS_E_NOLOCK, text_store_->GetSelection(0, 1, &selection_buffer, &fetched_count));
...
TEST_F(TSFTextStoreTest, SetGetTextTest)
{
    ...
    EXPECT_CALL(*sink_, OnLockGranted(_))
        ...
        .WillOnce(Invoke(&callback, &SetGetTextTestCallback::ReadWriteLockGranted));
    ...
    ULONG run_info_buffer_copied = 0;
    LONG next_acp = 0;
    EXPECT_EQ(TF_E_NOLOCK, text_store_->GetText(0, -1, buffer, 1024, &text_buffer_copied, &run_info, 1, &run_info_buffer_copied, &next_acp));
    TS_TEXTCHANGE change = {};
...
public:
    ...
    InsertTextAtSelectionTestCallback(const InsertTextAtSelectionTestCallback&) = delete;
    InsertTextAtSelectionTestCallback& operator=(const InsertTextAtSelectionTestCallback&) = delete;
    ...
    HRESULT ReadLockGranted(DWORD flags)
    {
        ...
        LONG end = 0;
        TS_TEXTCHANGE change = {};
        EXPECT_EQ(TS_E_NOLOCK, text_store_->InsertTextAtSelection(0, kBuffer, 10, &start, &end, &change));
        return S_OK;
    }
...
TEST_F(TSFTextStoreTest, InsertTextAtSelectionTest)
{
    ...
    EXPECT_CALL(*sink_, OnLockGranted(_))
        .WillOnce(Invoke(&callback, &InsertTextAtSelectionTestCallback::ReadLockGranted))
        .WillOnce(Invoke(&callback, &InsertTextAtSelectionTestCallback::ReadWriteLockGranted));
...
public:
    ...
    void SetCompositionText1(const ui::CompositionText& composition)
    {
        ...
        EXPECT_EQ(SK_ColorTRANSPARENT, composition.ime_text_spans[0].background_color);
        EXPECT_EQ(0u, composition.ime_text_spans[0].start_offset);
        EXPECT_EQ(5u, composition.ime_text_spans[0].end_offset);
        EXPECT_EQ(ImeTextSpan::Thickness::kThin, composition.ime_text_spans[0].thickness);
    ...
    void InsertText2(const std::u16string& text, ui::TextInputClient::InsertTextCursorBehavior cursor_behavior)
    {
    ...
    void InsertText3(const std::u16string& text, ui::TextInputClient::InsertTextCursorBehavior cursor_behavior)
    {
...
TEST_F(TSFTextStoreTest, ScenarioTest)
{
    ...
    EXPECT_CALL(text_input_client_, HasCompositionText()).WillRepeatedly(Invoke(&callback, &ScenarioTestCallback::ClientHasCompositionText));
...
TEST_F(TSFTextStoreTest, GetTextExtTest)
{
    ...
    EXPECT_CALL(text_input_client_, GetCaretBounds()).WillRepeatedly(Invoke(&callback, &GetTextExtTestCallback::GetCaretBounds));

    EXPECT_CALL(text_input_client_, GetCompositionCharacterBounds(_, _))
        .WillRepeatedly(Invoke(&callback, &GetTextExtTestCallback::GetCompositionCharacterBounds));

    EXPECT_CALL(text_input_client_, HasCompositionText()).WillRepeatedly(Invoke(&callback, &GetTextExtTestCallback::ClientHasCompositionText));

    EXPECT_CALL(*sink_, OnLockGranted(_)).WillOnce(Invoke(&callback, &GetTextExtTestCallback::LockGranted));
...
TEST_F(TSFTextStoreTest, RequestSupportedAttrs)
{
    ...
    EXPECT_CALL(text_input_client_, GetTextInputType()).WillRepeatedly(Return(TEXT_INPUT_TYPE_TEXT));
    EXPECT_CALL(text_input_client_, GetTextInputMode()).WillRepeatedly(Return(TEXT_INPUT_MODE_DEFAULT));
    EXPECT_CALL(text_input_client_, GetTextEditingContext()).WillOnce(Return(ui::TextInputClient::EditingContext())).WillOnce(Return(expected_editing_context));

    EXPECT_HRESULT_FAILED(text_store_->RequestSupportedAttrs(0, 1, nullptr));

    const TS_ATTRID kUnknownAttributes[] = { GUID_NULL };
    EXPECT_HRESULT_SUCCEEDED(text_store_->RequestSupportedAttrs(0, std::size(kUnknownAttributes), kUnknownAttributes)) << "Mustn't fail for unknown attributes";

    const TS_ATTRID kAttributes[] = { GUID_NULL, GUID_PROP_INPUTSCOPE, GUID_NULL };
    EXPECT_EQ(S_OK, text_store_->RequestSupportedAttrs(0, std::size(kAttributes), kAttributes)) << "InputScope must be supported";
    const TS_ATTRID urlAttributes[] = { GUID_PROP_URL };
    ui::TextInputClient::EditingContext actual_editing_context = text_input_client_.GetTextEditingContext();
    EXPECT_TRUE(actual_editing_context.page_url.is_empty());
    EXPECT_EQ(S_OK, text_store_->RequestSupportedAttrs(0, std::size(urlAttributes), urlAttributes)) << "Should return S_OK even if URL not supported";

    actual_editing_context = text_input_client_.GetTextEditingContext();
    EXPECT_TRUE(!actual_editing_context.page_url.is_empty());
    EXPECT_TRUE(actual_editing_context.page_url.spec().compare(expected_editing_context.page_url.spec()) == 0);
    EXPECT_EQ(S_OK, text_store_->RequestSupportedAttrs(0, std::size(urlAttributes), urlAttributes)) << "Expect URL to be supported";

    {
        ...
        EXPECT_HRESULT_FAILED(text_store_->RequestSupportedAttrs(0, std::size(kAttributes), kAttributes));
    }
}
...
TEST_F(TSFTextStoreTest, RetrieveRequestedAttrs)
{
    ...
    EXPECT_CALL(text_input_client_, GetTextInputType()).WillRepeatedly(Return(TEXT_INPUT_TYPE_TEXT));
    EXPECT_CALL(text_input_client_, GetTextInputMode()).WillRepeatedly(Return(TEXT_INPUT_MODE_DEFAULT));
    EXPECT_CALL(text_input_client_, GetTextEditingContext()).WillRepeatedly(Return(expected_editing_context));

    ULONG num_copied = 0xfffffff;
    EXPECT_HRESULT_FAILED(text_store_->RetrieveRequestedAttrs(1, nullptr, &num_copied));

    {
        ...
        ASSERT_EQ(S_OK, text_store_->RequestSupportedAttrs(0, std::size(kAttributes), kAttributes));
        ASSERT_EQ(S_OK, text_store_->RetrieveRequestedAttrs(std::size(buffer), buffer, &num_copied));
        ...
        for (size_t i = 0; i < num_copied; ++i) {
            ...
            if (IsEqualGUID(buffer[i].idAttr, GUID_PROP_INPUTSCOPE)) {
                ...
                EXPECT_HRESULT_SUCCEEDED(variant.AsInput()->punkVal->QueryInterface(IID_PPV_ARGS(&input_scope)));
    ...
    }
    {
        ...
        ASSERT_EQ(S_OK, text_store_->RequestSupportedAttrs(0, std::size(urlAttributes), urlAttributes));
        ASSERT_EQ(S_OK, text_store_->RetrieveRequestedAttrs(std::size(buffer), buffer, &num_copied));
        ...
        std::string url_string = base::WideToUTF8(std::wstring(variant.ptr()->bstrVal, SysStringLen(variant.ptr()->bstrVal)));
        EXPECT_EQ(expected_editing_context.page_url.spec(), url_string) << "Expected url strings to match";
    }
    {
        SCOPED_TRACE("Verify URL and InputScope support");
        TS_ATTRVAL buffer[2] = {};
        num_copied = 0xfffffff;
        const TS_ATTRID inputScopeAndUrlAttributes[] = { GUID_PROP_INPUTSCOPE, GUID_PROP_URL };

        // This call should have a valid URL so the URL property should be returned
        // and is expected to match the test_url value set above.
        ASSERT_EQ(S_OK, text_store_->RequestSupportedAttrs(0, std::size(inputScopeAndUrlAttributes), inputScopeAndUrlAttributes));
        ASSERT_EQ(S_OK, text_store_->RetrieveRequestedAttrs(std::size(buffer), buffer, &num_copied));
        EXPECT_EQ(num_copied, 2U) << "Expect both URL & InputScope properties to be supported";
        for (size_t i = 0; i < num_copied; ++i) {
            base::win::ScopedVariant variant;
            // Move ownership from |buffer[i].varValue| to |variant|.
            std::swap(*variant.Receive(), buffer[i].varValue);
...

    {
        ...
        ASSERT_EQ(S_OK, text_store_->RequestSupportedAttrs(0, std::size(attributes), attributes));
        ...
        EXPECT_CALL(text_input_client_, GetTextInputFlags()).WillOnce(Return(ui::TEXT_INPUT_FLAG_VERTICAL));
        ASSERT_EQ(S_OK, text_store_->RetrieveRequestedAttrs(std::size(buffer), buffer, &num_copied));
        EXPECT_EQ(num_copied, 1U);
        EXPECT_TRUE(IsEqualGUID(buffer[0].idAttr, TSATTRID_Text_VerticalWriting));
        EXPECT_EQ(VT_BOOL, buffer[0].varValue.vt);
...

    {
        ...
        num_copied = 0xfffffff;
        TS_ATTRVAL buffer[2] = {};
        EXPECT_HRESULT_FAILED(text_store_->RetrieveRequestedAttrs(std::size(buffer), buffer, &num_copied));
    }
}
...
public:
    ...
    void SetCompositionText2(const ui::CompositionText& composition)
    {
        ...
        EXPECT_EQ(0u, composition.ime_text_spans[0].start_offset);
        EXPECT_EQ(1u, composition.ime_text_spans[0].end_offset);
        EXPECT_EQ(ImeTextSpan::Thickness::kThick, composition.ime_text_spans[0].thickness);
        SetHasCompositionText(true);
    }
...
TEST_F(TSFTextStoreTest, KeyEventTest)
{
    ...
    EXPECT_CALL(ime_key_event_dispatcher_, DispatchKeyEventPostIME(_))
        .WillOnce(Invoke(&callback, &KeyEventTestCallback::DispatchKeyEventPostIME1))
        .WillOnce(Invoke(&callback, &KeyEventTestCallback::DispatchKeyEventPostIME2))
        .WillOnce(Invoke(&callback, &KeyEventTestCallback::DispatchKeyEventPostIME3a))
        .WillOnce(Invoke(&callback, &KeyEventTestCallback::DispatchKeyEventPostIME3b));
    ...
    ON_CALL(text_input_client_, HasCompositionText()).WillByDefault(Invoke(&callback, &TSFTextStoreTestCallback::HasCompositionText));
...
public:
    ...
    AccessibilityEventTestCallback(const AccessibilityEventTestCallback&) = delete;
    AccessibilityEventTestCallback& operator=(const AccessibilityEventTestCallback&) = delete;
    ...
    void SetActiveCompositionForAccessibility1(const gfx::Range& range, const std::u16string& active_composition_text, bool committed_composition)
    {
...
TEST_F(TSFTextStoreTest, AccessibilityEventTest)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_)).WillOnce(Invoke(&callback, &AccessibilityEventTestCallback::SetCompositionText1));

    EXPECT_CALL(text_input_client_, SetActiveCompositionForAccessibility(_, _, _))
        .WillOnce(Invoke(&callback, &AccessibilityEventTestCallback::SetActiveCompositionForAccessibility1));

    EXPECT_CALL(*sink_, OnLockGranted(_)).WillOnce(Invoke(&callback, &AccessibilityEventTestCallback::LockGranted1));
...
public:
    ...
    DiffingAlgorithmTestCallback& operator=(const DiffingAlgorithmTestCallback&) = delete;
...
TEST_F(TSFTextStoreTest, DiffingAlgorithmTest)
{
    ...
    EXPECT_CALL(*sink_, OnTextChange(_, _))
        ...
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnTextChange10))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnTextChange11))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnTextChange12))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnTextChange13))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnTextChange14));

    EXPECT_CALL(*sink_, OnSelectionChange())
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange1))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange3))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange4))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange5))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange6))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange7))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange8))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange9))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange10))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange11))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange12))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange13))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::OnSelectionChange14));

    EXPECT_CALL(text_input_client_, InsertText(_, _)).WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::InsertText2));
    ...
    EXPECT_CALL(*sink_, OnLockGranted(_))
        ...
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted10a))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted10b))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted11))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted11a))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted11b))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted12))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted12a))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted12b))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted13))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted13a))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted13b))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted14))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted14a))
        .WillOnce(Invoke(&callback, &DiffingAlgorithmTestCallback::LockGranted14b));

    ON_CALL(text_input_client_, GetCompositionTextRange(_)).WillByDefault(Invoke(&callback, &TSFTextStoreTestCallback::GetCompositionTextRange));

    ON_CALL(text_input_client_, GetTextRange(_)).WillByDefault(Invoke(&callback, &TSFTextStoreTestCallback::GetTextRange));

    ON_CALL(text_input_client_, GetTextFromRange(_, _)).WillByDefault(Invoke(&callback, &TSFTextStoreTestCallback::GetTextFromRange));

    ON_CALL(text_input_client_, GetEditableSelectionRange(_)).WillByDefault(Invoke(&callback, &TSFTextStoreTestCallback::GetEditableSelectionRange));
...
public:
    ...
    void InsertText4(const std::u16string& text, ui::TextInputClient::InsertTextCursorBehavior cursor_behavior)
    {
        EXPECT_EQ(u"c", text);
    }
    ...
    void InsertText6(const std::u16string& text, ui::TextInputClient::InsertTextCursorBehavior cursor_behavior)
    {
        EXPECT_EQ(u"e", text);
        SetHasCompositionText(false);
    }
...
TEST_F(TSFTextStoreTest, RegressionTest)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_))
        ...
        .WillOnce(Invoke(&callback, &RegressionTestCallback::SetCompositionText5));

    EXPECT_CALL(ime_key_event_dispatcher_, DispatchKeyEventPostIME(_))
        .WillOnce(Invoke(&callback, &RegressionTestCallback::DispatchKeyEventPostIME1))
        .WillOnce(Invoke(&callback, &RegressionTestCallback::DispatchKeyEventPostIME2a))
        .WillOnce(Invoke(&callback, &RegressionTestCallback::DispatchKeyEventPostIME2b))
        .WillOnce(Invoke(&callback, &RegressionTestCallback::DispatchKeyEventPostIME4))
        .WillOnce(Invoke(&callback, &RegressionTestCallback::DispatchKeyEventPostIME5));
...
TEST_F(TSFTextStoreTest, RegressionTest2)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_)).WillOnce(Invoke(&callback, &RegressionTest2Callback::SetCompositionText2));

    EXPECT_CALL(text_input_client_, InsertText(_, _)).WillOnce(Invoke(&callback, &RegressionTest2Callback::InsertText3));
...
TEST_F(TSFTextStoreTest, RegressionTest3)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_))
        .WillOnce(Invoke(&callback, &RegressionTest3Callback::SetCompositionText1))
        .WillOnce(Invoke(&callback, &RegressionTest3Callback::SetCompositionText2));
...
TEST_F(TSFTextStoreTest, RegressionTest4)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_)).WillOnce(Invoke(&callback, &RegressionTest4Callback::SetCompositionText1));

    EXPECT_CALL(text_input_client_, InsertText(_, _)).WillOnce(Invoke(&callback, &RegressionTest4Callback::InsertText2));
...
public:
    ...
    void SetCompositionText1(const ui::CompositionText& composition)
    {
        ...
        EXPECT_EQ(ImeTextSpan::Thickness::kThick, composition.ime_text_spans[0].thickness);
        EXPECT_EQ(ImeTextSpan::Thickness::kThin, composition.ime_text_spans[1].thickness);
        SetHasCompositionText(true);
    }
    ...
    void SetCompositionText2(const ui::CompositionText& composition)
    {
        ...
        ASSERT_EQ(2u, composition.ime_text_spans.size());
        EXPECT_EQ(ImeTextSpan::Thickness::kThin, composition.ime_text_spans[0].thickness);
        EXPECT_EQ(ImeTextSpan::Thickness::kThick, composition.ime_text_spans[1].thickness);
        SetHasCompositionText(true);
    }
...
TEST_F(TSFTextStoreTest, RegressionTest5)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_))
        .WillOnce(Invoke(&callback, &RegressionTest5Callback::SetCompositionText1))
        .WillOnce(Invoke(&callback, &RegressionTest5Callback::SetCompositionText2));

    EXPECT_CALL(text_input_client_, InsertText(_, _)).WillOnce(Invoke(&callback, &RegressionTest5Callback::InsertText3));
...
TEST_F(TSFTextStoreTest, RegressionTest6)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_)).WillOnce(Invoke(&callback, &RegressionTest6Callback::SetCompositionText1));

    EXPECT_CALL(text_input_client_, InsertText(_, _)).WillOnce(Invoke(&callback, &RegressionTest6Callback::InsertText2));
...
public:
    ...
    UnderlineStyleTestCallback& operator=(const UnderlineStyleTestCallback&) = delete;
    ...
    void SetCompositionText1(const ui::CompositionText& composition)
    {
        ...
        EXPECT_EQ(1u, composition.ime_text_spans[0].end_offset);
        EXPECT_EQ(ImeTextSpan::Thickness::kThin, composition.ime_text_spans[0].thickness);
        EXPECT_EQ(ImeTextSpan::UnderlineStyle::kSolid, composition.ime_text_spans[0].underline_style);
        SetHasCompositionText(true);
    }
};
...
TEST_F(TSFTextStoreTest, UnderlineStyleTest)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_)).WillOnce(Invoke(&callback, &UnderlineStyleTestCallback::SetCompositionText1));

    EXPECT_CALL(*sink_, OnLockGranted(_)).WillOnce(Invoke(&callback, &UnderlineStyleTestCallback::LockGranted1));
...
public:
    ...
    bool SetCompositionFromExistingText2(const gfx::Range& range, const std::vector<ui::ImeTextSpan>& text_spans)
    {
        EXPECT_EQ(0u, range.start());
        EXPECT_EQ(1u, range.end());
...
TEST_F(TSFTextStoreTest, RegressionTest7)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionFromExistingText(_, _))
        .WillOnce(Invoke(&callback, &RegressionTest7Callback::SetCompositionFromExistingText2));

    EXPECT_CALL(text_input_client_, InsertText(_, _)).WillOnce(Invoke(&callback, &RegressionTest7Callback::InsertText3));
...
public:
    ...
    void ClearCompositionText2()
    {
        EXPECT_EQ(false, *has_composition_range());
    }
};
...
TEST_F(TSFTextStoreTest, RegressionTest8)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_)).WillOnce(Invoke(&callback, &RegressionTest8Callback::SetCompositionText1));

    EXPECT_CALL(text_input_client_, ClearCompositionText()).WillOnce(Invoke(&callback, &RegressionTest8Callback::ClearCompositionText2));
...
TEST_F(TSFTextStoreTest, RegressionTest9)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_))
        .WillOnce(Invoke(&callback, &RegressionTest9Callback::SetCompositionText2))
        .WillOnce(Invoke(&callback, &RegressionTest9Callback::SetCompositionText3));

    EXPECT_CALL(text_input_client_, InsertText(_, _)).WillOnce(Invoke(&callback, &RegressionTest9Callback::InsertText4));
...
TEST_F(TSFTextStoreTest, RegressionTest10)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_))
        .WillOnce(Invoke(&callback, &RegressionTest10Callback::SetCompositionText1))
        .WillOnce(Invoke(&callback, &RegressionTest10Callback::SetCompositionText2));
...
TEST_F(TSFTextStoreTest, RegressionTest11)
{
    ...
    EXPECT_CALL(text_input_client_, SetCompositionText(_)).WillOnce(Invoke(&callback, &RegressionTest11Callback::SetCompositionText));
...
TEST_F(TSFTextStoreTest, RegressionTest13)
{
    ...
    EXPECT_CALL(text_input_client_, ExtendSelectionAndDelete(_, _)).Times(1);
    EXPECT_CALL(text_input_client_, InsertText(_, _)).Times(0);
    EXPECT_CALL(text_input_client_, SetCompositionText(_)).WillOnce(Invoke(&callback, &RegressionTest13Callback::SetCompositionText1));
...
TEST_F(TSFTextStoreTest, RegressionTest14)
{
    ...
    EXPECT_CALL(text_input_client_, InsertText(_, _)).Times(0);
    EXPECT_CALL(text_input_client_, SetCompositionText(_)).WillOnce(Invoke(&callback, &RegressionTest14Callback::SetCompositionText1));
    EXPECT_CALL(text_input_client_, SetEditableSelectionRange(_)).WillOnce(Invoke(&callback, &RegressionTest14Callback::SetEditableSelectionRange2));
...
public:
    ...
    TextInputClientReentrancyTestCallback(const TextInputClientReentrancyTestCallback&) = delete;
    TextInputClientReentrancyTestCallback& operator=(const TextInputClientReentrancyTestCallback&) = delete;
...
TEST_F(TSFTextStoreTest, TextInputClientReentrancTest)
{
    ...
    EXPECT_CALL(*sink_, OnLockGranted(_))
        .WillOnce(Invoke(&callback, &TextInputClientReentrancyTestCallback::LockGranted1))
        .WillOnce(Invoke(&callback, &TextInputClientReentrancyTestCallback::LockGranted2))
        .WillOnce(Invoke(&callback, &TextInputClientReentrancyTestCallback::LockGranted3))
        .WillOnce(Invoke(&callback, &TextInputClientReentrancyTestCallback::LockGranted4));

    EXPECT_CALL(text_input_client_, InsertText(_, _)).WillOnce(Invoke(&callback, &TextInputClientReentrancyTestCallback::InsertText2));

    EXPECT_CALL(text_input_client_, SetCompositionText(_)).WillOnce(Invoke(&callback, &TextInputClientReentrancyTestCallback::SetCompositionText1));
