...
class ATL_NO_VTABLE TSFEventRouter::Delegate : public ATL::CComObjectRootEx<CComSingleThreadModel>, public ITfUIElementSink, public ITfTextEditSink {
public:
    ...
    // ITfTextEditSink:
    IFACEMETHODIMP OnEndEdit(ITfContext* context, TfEditCookie read_only_cookie, ITfEditRecord* edit_record) override;
...
HRESULT TSFEventRouter::Delegate::OnEndEdit(ITfContext* context, TfEditCookie read_only_cookie, ITfEditRecord* edit_record)
{
    ...
    if (FAILED(edit_record->GetTextAndPropertyUpdates(TF_GTP_INCL_TEXT, nullptr, 0, &ranges)))
...
HRESULT TSFEventRouter::Delegate::BeginUIElement(DWORD element_id, BOOL* is_show)
{
    if (is_show)
    ...
    std::pair<std::set<DWORD>::iterator, bool> insert_result = open_candidate_window_ids_.insert(element_id);
...
void TSFEventRouter::Delegate::SetManager(ITfThreadMgr* thread_manager)
{
    ...
    if (FAILED(thread_manager->GetFocus(&document_manager)) || !document_manager.Get() || FAILED(document_manager->GetBase(&context_))
        || FAILED(context_.As(&context_source_)))
        return;
    context_source_->AdviseSink(IID_ITfTextEditSink, static_cast<ITfTextEditSink*>(this), &context_source_cookie_);

    if (FAILED(thread_manager->QueryInterface(IID_PPV_ARGS(&ui_element_manager_))) || FAILED(ui_element_manager_.As(&ui_source_)))
        return;
    ui_source_->AdviseSink(IID_ITfUIElementSink, static_cast<ITfUIElementSink*>(this), &ui_source_cookie_);
}
