...
public:
    ...
    IFACEMETHODIMP GetInputScopes(InputScope** input_scopes, UINT* count) override
    {
        if (!count || !input_scopes)
            return E_INVALIDARG;
        *input_scopes = static_cast<InputScope*>(CoTaskMemAlloc(sizeof(InputScope) * input_scopes_.size()));
        if (!input_scopes) {
            *count = 0;
    ...
    IFACEMETHODIMP GetSRGS(BSTR* srgs) override
    {
        return E_NOTIMPL;
    }

    IFACEMETHODIMP GetXML(BSTR* xml) override
    {
        return E_NOTIMPL;
    }
...
std::vector<InputScope> GetInputScopes(TextInputType text_input_type, TextInputMode text_input_mode)
{
    std::vector<InputScope> input_scopes;

    AppendNonTrivialInputScope(&input_scopes, ConvertTextInputTypeToInputScope(text_input_type));
    AppendNonTrivialInputScope(&input_scopes, ConvertTextInputModeToInputScope(text_input_mode));
...
ITfInputScope* CreateInputScope(TextInputType text_input_type, TextInputMode text_input_mode, bool should_do_learning)
{
    std::vector<InputScope> input_scopes;
...
typedef HRESULT(WINAPI* SetInputScopeFunc)(HWND window_handle, InputScope input_scope);
...
void SetInputScope(HWND window_handle, InputScope input_scope)
{
    ...
    if (!g_get_set_input_scope_done) {
        ...
        if (GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_PIN, L"msctf.dll", &module)) {
            g_set_input_scope = reinterpret_cast<SetInputScopeFunc>(GetProcAddress(module, "SetInputScope"));
        }
    }
    ...
    if (g_set_input_scope) {
        ...
        if (hr != S_OK) {
            TRACE_EVENT2("ime", "SetInputScope", "input_scope", input_scope, "hr", hr);
        }
    }
}
