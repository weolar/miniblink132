...
class COMPONENT_EXPORT(UI_BASE_IME_WIN) InputMethodWinBase : public InputMethodBase {
public:
    InputMethodWinBase(ImeKeyEventDispatcher* ime_key_event_dispatcher, HWND attached_window_handle);
...
protected:
    ...
    LRESULT OnChar(HWND window_handle, UINT message, WPARAM wparam, LPARAM lparam, const CHROME_MSG& event, BOOL* handled);
    ...
    LRESULT OnImeRequest(UINT message, WPARAM wparam, LPARAM lparam, BOOL* handled);
    ...
    void ProcessKeyEventDone(ui::KeyEvent* event, const std::vector<CHROME_MSG>* char_msgs, bool is_handled);

    ui::EventDispatchDetails ProcessUnhandledKeyEvent(ui::KeyEvent* event, const std::vector<CHROME_MSG>* char_msgs);
...
private:
    ...
    bool HandlePeekMessage(HWND hwnd, UINT msg_filter_min, UINT msg_filter_max, std::vector<CHROME_MSG>* char_msgs);
};
