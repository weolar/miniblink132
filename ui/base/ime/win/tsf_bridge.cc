...
public:
    ...
    void SetImeKeyEventDispatcher(ImeKeyEventDispatcher* ime_key_event_dispatcher) override;
    void RemoveImeKeyEventDispatcher(ImeKeyEventDispatcher* ime_key_event_dispatcher) override;
...
private:
    ...
    // not necessary.
    HRESULT CreateDocumentManager(TSFTextStore* text_store, ITfDocumentMgr** document_manager, ITfContext** context, DWORD* source_cookie,
        DWORD* key_trace_sink_cookie, DWORD* language_profile_cookie);
...
TSFBridgeImpl::~TSFBridgeImpl()
{
    ...
    for (auto& pair : tsf_document_map_) {
        ...
        if (document.source_cookie != TF_INVALID_COOKIE && SUCCEEDED(document.document_manager->GetBase(&context)) && SUCCEEDED(context.As(&source))) {
            source->UnadviseSink(document.source_cookie);
        }
        if (thread_manager_ != nullptr) {
            Microsoft::WRL::ComPtr<ITfSource> key_trace_sink_source;
            if (document.key_trace_sink_cookie != TF_INVALID_COOKIE && SUCCEEDED(thread_manager_.As(&key_trace_sink_source))) {
                key_trace_sink_source->UnadviseSink(document.key_trace_sink_cookie);
            }
            Microsoft::WRL::ComPtr<ITfSource> language_profile_source;
            if (document.language_profile_cookie != TF_INVALID_COOKIE && SUCCEEDED(input_processor_profiles_.As(&language_profile_source))) {
...
HRESULT TSFBridgeImpl::Initialize()
{
    ...
    HRESULT hr = ::CoCreateInstance(CLSID_TF_InputProcessorProfiles, nullptr, CLSCTX_ALL, IID_PPV_ARGS(&input_processor_profiles_));
    if (FAILED(hr)) {
        DVLOG(1) << "Failed to create InputProcessorProfiles instance.";
        return hr;
    }

    hr = ::CoCreateInstance(CLSID_TF_ThreadMgr, nullptr, CLSCTX_ALL, IID_PPV_ARGS(&thread_manager_));
    if (FAILED(hr)) {
    ...
    hr = thread_compartment_manager->GetCompartment(GUID_COMPARTMENT_KEYBOARD_INPUTMODE_SENTENCE, &sentence_compartment);
    if (FAILED(hr)) {
...
void TSFBridgeImpl::OnTextInputTypeChanged(const TextInputClient* client)
{
    ...
    if (input_type_ != TEXT_INPUT_TYPE_NONE && input_type_ == client_->GetTextInputType()) {
        if (empty_tsf_support_) {
            ...
            TSFDocumentMap::iterator it = tsf_document_map_.find(TEXT_INPUT_TYPE_NONE);
...
void TSFBridgeImpl::SetFocusedClient(HWND focused_window, TextInputClient* client)
{
    DCHECK(base::CurrentUIThread::IsSet());
    DCHECK(client);
    DCHECK(IsInitialized());
    ...
    for (TSFDocumentMap::iterator it = tsf_document_map_.begin(); it != tsf_document_map_.end(); ++it) {
...
void TSFBridgeImpl::SetImeKeyEventDispatcher(ImeKeyEventDispatcher* ime_key_event_dispatcher)
{
    DCHECK(base::CurrentUIThread::IsSet());
...
void TSFBridgeImpl::RemoveImeKeyEventDispatcher(ImeKeyEventDispatcher* ime_key_event_dispatcher)
{
    DCHECK(base::CurrentUIThread::IsSet());
    DCHECK(IsInitialized());
    ...
    for (TSFDocumentMap::iterator it = tsf_document_map_.begin(); it != tsf_document_map_.end(); ++it) {
        if (it->second.text_store.get() == nullptr)
            continue;
        it->second.text_store->RemoveImeKeyEventDispatcher(ime_key_event_dispatcher);
    }
}
...
bool TSFBridgeImpl::IsInputLanguageCJK()
{
    ...
    return lang_locale == LANG_CHINESE || lang_locale == LANG_JAPANESE || lang_locale == LANG_KOREAN;
}
...
HRESULT TSFBridgeImpl::CreateDocumentManager(TSFTextStore* text_store, ITfDocumentMgr** document_manager, ITfContext** context, DWORD* source_cookie,
    DWORD* key_trace_sink_cookie, DWORD* language_profile_cookie)
{
    ...
    if (!text_store || !source_cookie || !key_trace_sink_cookie || !language_profile_cookie) {
        return S_OK;
    }

    DWORD edit_cookie = TF_INVALID_EDIT_COOKIE;
    hr = (*document_manager)->CreateContext(client_id_, 0, static_cast<ITextStoreACP*>(text_store), context, &edit_cookie);
    if (FAILED(hr)) {
    ...
    hr = source->AdviseSink(IID_ITfTextEditSink, static_cast<ITfTextEditSink*>(text_store), source_cookie);
    if (FAILED(hr)) {
    ...
    hr = source_ITfThreadMgr->AdviseSink(IID_ITfKeyTraceEventSink, static_cast<ITfKeyTraceEventSink*>(text_store), key_trace_sink_cookie);
    if (FAILED(hr)) {
    ...
    hr = input_processor_profiles_->QueryInterface(IID_PPV_ARGS(&language_source));
    if (FAILED(hr)) {
        DVLOG(1) << "Failed to get source_ITfInputProcessorProfiles.";
        return hr;
    }

    hr = language_source->AdviseSink(IID_ITfLanguageProfileNotifySink, static_cast<ITfTextEditSink*>(text_store), language_profile_cookie);
    if (FAILED(hr)) {
...
HRESULT TSFBridgeImpl::InitializeDocumentMapInternal()
{
    ...
    HRESULT res = thread_manager_->QueryInterface(GUID_COMPARTMENT_EMPTYCONTEXT, &flag_empty_context);
    if (SUCCEEDED(res)) {
    ...
    for (size_t i = 0; i < std::size(kTextInputTypes); ++i) {
        ...
        const bool use_null_text_store = (input_type == TEXT_INPUT_TYPE_NONE && !empty_tsf_support_);
        DWORD* source_cookie_ptr = use_null_text_store ? nullptr : &source_cookie;
        DWORD* key_trace_sink_cookie_ptr = use_null_text_store ? nullptr : &key_trace_sink_cookie;
        DWORD* language_profile_cookie_ptr = use_null_text_store ? nullptr : &language_profile_cookie;
        scoped_refptr<TSFTextStore> text_store = use_null_text_store ? nullptr : new TSFTextStore();
        ...
        if (FAILED(hr))
            return hr;
        if (input_type == TEXT_INPUT_TYPE_PASSWORD || (empty_tsf_support_ && input_type == TEXT_INPUT_TYPE_NONE)) {
        ...
        tsf_document_map_[input_type].language_profile_cookie = language_profile_cookie;
        if (text_store)
...
HRESULT TSFBridgeImpl::InitializeDisabledContext(ITfContext* context)
{
    ...
    hr = compartment_mgr->GetCompartment(GUID_COMPARTMENT_EMPTYCONTEXT, &empty_context);
    if (FAILED(hr)) {
...
void TSFBridgeImpl::UpdateAssociateFocus()
{
    ...
    thread_manager_->AssociateFocus(attached_window_handle_, document->document_manager.Get(), &previous_focus);
}
...
void TSFBridgeImpl::ClearAssociateFocus()
{
    ...
    if (attached_window_handle_ == nullptr)
        return;
    Microsoft::WRL::ComPtr<ITfDocumentMgr> previous_focus;
    thread_manager_->AssociateFocus(attached_window_handle_, nullptr, &previous_focus);
}
...
{
    static base::NoDestructor<base::ThreadLocalOwnedPointer<TSFBridge>> tsf_bridge;
    return *tsf_bridge;
}
...
// static
void TSFBridge::ReplaceThreadLocalTSFBridge(std::unique_ptr<TSFBridge> new_instance)
{
    if (!base::CurrentUIThread::IsSet()) {
        return;
    }
...
{
    return base::CurrentUIThread::IsSet() ? GetThreadLocalTSFBridge().Get() : nullptr;
}
