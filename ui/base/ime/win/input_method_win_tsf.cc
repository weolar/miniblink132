...
public:
    ...
    // etc.) is open.
    bool IsCandidatePopupOpen() const
    {
...
InputMethodWinTSF::InputMethodWinTSF(ImeKeyEventDispatcher* ime_key_event_dispatcher, HWND attached_window_handle)
...
InputMethodWinTSF::~InputMethodWinTSF()
{
    ...
    if (ui::TSFBridge::GetInstance()) {
        ui::TSFBridge::GetInstance()->RemoveImeKeyEventDispatcher(InputMethodBase::ime_key_event_dispatcher());
    }
}
...
void InputMethodWinTSF::OnFocus()
{
    ...
    tsf_event_router_->SetManager(ui::TSFBridge::GetInstance()->GetThreadManager().Get());
    ui::TSFBridge::GetInstance()->SetImeKeyEventDispatcher(InputMethodBase::ime_key_event_dispatcher());
}
...
bool InputMethodWinTSF::OnUntranslatedIMEMessage(const CHROME_MSG event, InputMethod::NativeEventResult* result)
{
    ...
    case WM_IME_REQUEST:
        ...
        original_result = OnImeRequest(event.message, event.wParam, event.lParam, &handled);
        break;
    case WM_CHAR:
    case WM_SYSCHAR:
        ...
        original_result = OnChar(event.hwnd, event.message, event.wParam, event.lParam, event, &handled);
        break;
    }
...
void InputMethodWinTSF::OnTextInputTypeChanged(TextInputClient* client)
{
    ...
    if (!ui::TSFBridge::GetInstance() || !IsTextInputClientFocused(client) || !IsWindowFocused(client)) {
        return;
    }
...
void InputMethodWinTSF::OnWillChangeFocusedClient(TextInputClient* focused_before, TextInputClient* focused)
{
...
void InputMethodWinTSF::OnDidChangeFocusedClient(TextInputClient* focused_before, TextInputClient* focused)
{
    if (ui::TSFBridge::GetInstance() && IsWindowFocused(focused) && IsTextInputClientFocused(focused)) {
        ui::TSFBridge::GetInstance()->SetFocusedClient(attached_window_handle_, focused);
