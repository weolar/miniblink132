...
bool GetWindowClientRect(HWND window_handle, POINT* left_top, POINT* right_bottom)
{
    RECT client_rect = {};
...
{
    HRESULT hr = ::CoCreateInstance(CLSID_TF_CategoryMgr, nullptr, CLSCTX_ALL, IID_PPV_ARGS(&category_manager_));
    if (FAILED(hr)) {
        DVLOG(1) << "Failed to initialize CategoryMgr.";
        return hr;
    }

    hr = ::CoCreateInstance(CLSID_TF_DisplayAttributeMgr, nullptr, CLSCTX_ALL, IID_PPV_ARGS(&display_attribute_manager_));
    if (FAILED(hr)) {
        DVLOG(1) << "Failed to initialize DisplayAttributeMgr.";
        return hr;
    }

    hr = ::CoCreateInstance(CLSID_TF_InputProcessorProfiles, nullptr, CLSCTX_INPROC_SERVER, IID_PPV_ARGS(&input_processor_profile_mgr_));
    if (FAILED(hr)) {
...
HRESULT TSFTextStore::FindNextAttrTransition(LONG acp_start, LONG acp_halt, ULONG num_filter_attributes, const TS_ATTRID* filter_attributes, DWORD flags,
    LONG* acp_next, BOOL* found, LONG* found_offset)
{
...
HRESULT TSFTextStore::GetACPFromPoint(TsViewCookie view_cookie, const POINT* point, DWORD flags, LONG* acp)
{
    ...
    if (stylus_handwriting::win::IsStylusHandwritingWinEnabled()) {
        ...
        const std::optional<size_t> index = text_input_client_->GetProximateCharacterIndexFromPoint(gfx::Point(*point), index_flags);
        if (!index.has_value()) {
...
HRESULT TSFTextStore::GetEmbedded(LONG acp_pos, REFGUID service, REFIID iid, IUnknown** unknown)
{
...
HRESULT TSFTextStore::GetFormattedText(LONG acp_start, LONG acp_end, IDataObject** data_object)
{
    NOTIMPLEMENTED();
    return E_NOTIMPL;
}
...
HRESULT TSFTextStore::GetScreenExt(TsViewCookie view_cookie, RECT* rect)
{
    ...
    text_input_client_->GetActiveTextInputControlLayoutBounds(&result_rect, &tmp_rect);
    if (result_rect) {
        // This conversion is required for high dpi monitors.
        *rect = display::win::ScreenWin::DIPToScreenRect(window_handle_, result_rect.value()).ToRECT();
    } else {
    ...
    TRACE_EVENT1("ime", "TSFTextStore::GetScreenExt", "control_bounding_rect", gfx::Rect(*rect).ToString());
    return S_OK;
}

HRESULT TSFTextStore::GetSelection(ULONG selection_index, ULONG selection_buffer_size, TS_SELECTION_ACP* selection_buffer, ULONG* fetched_count)
{
    if (!selection_buffer)
        return E_INVALIDARG;
    if (!fetched_count)
    ...
    if (!HasReadLock())
        return TS_E_NOLOCK;
    *fetched_count = 0;
    if ((selection_buffer_size > 0) && ((selection_index == 0) || (selection_index == TS_DEFAULT_SELECTION))) {
...
HRESULT TSFTextStore::GetText(LONG acp_start, LONG acp_end, wchar_t* text_buffer, ULONG text_buffer_size, ULONG* text_buffer_copied,
    TS_RUNINFO* run_info_buffer, ULONG run_info_buffer_size, ULONG* run_info_buffer_copied, LONG* next_acp)
{
    ...
    if (!((0 <= acp_start) && (acp_start <= acp_end) && (acp_end <= string_buffer_size))) {
        return TF_E_INVALIDPOS;
    }
    acp_end = std::min(acp_end, acp_start + static_cast<LONG>(text_buffer_size));
    *text_buffer_copied = acp_end - acp_start;

    const std::u16string& result = string_buffer_document_.substr(acp_start, *text_buffer_copied);
...
HRESULT TSFTextStore::GetTextExt(TsViewCookie view_cookie, LONG acp_start, LONG acp_end, RECT* rect, BOOL* clipped)
{
    if (!rect || !clipped)
        return E_INVALIDARG;
    if (!text_input_client_)
        return E_UNEXPECTED;
    const bool is_stylus_handwriting_win_enabled = stylus_handwriting::win::IsStylusHandwritingWinEnabled();
    ...
    if (view_cookie == kViewCookie
        && !(
            (static_cast<LONG>(composition_start_) <= acp_start) && (acp_start <= acp_end) && (acp_end <= static_cast<LONG>(string_buffer_document_.size())))) {
        return TS_E_INVALIDPOS;
    }

    TRACE_EVENT1("ime", "TSFTextStore::GetTextExt", "start, end", base::NumberToString(acp_start) + ", " + base::NumberToString(acp_end));
    ...
    if (view_cookie == kViewCookie && start_pos == end_pos) {
        if (text_input_client_->HasCompositionText()) {
            ...
            } else if (text_input_client_->GetCompositionCharacterBounds(start_pos - 1, &tmp_rect)) {
        ...
        if (text_input_client_->HasCompositionText()) {
            if (text_input_client_->GetCompositionCharacterBounds(start_pos, &tmp_rect)) {
                result_rect = gfx::Rect(tmp_rect);
                if (text_input_client_->GetCompositionCharacterBounds(end_pos - 1, &tmp_rect)) {
                    result_rect->set_width(tmp_rect.x() - result_rect->x() + tmp_rect.width());
                    result_rect->set_height(tmp_rect.y() - result_rect->y() + tmp_rect.height());
                } else {
        ...
        result_rect = text_input_client_->GetProximateCharacterBounds(gfx::Range(acp_start, acp_end));
        if (!result_rect.has_value()) {
            return TS_E_NOLAYOUT;
        }
    }
    TRACE_EVENT1("ime", "TSFTextStore::GetTextExt", "DIP rect", result_rect->ToString());

    *rect = display::win::ScreenWin::DIPToScreenRect(window_handle_, result_rect.value()).ToRECT();
    *clipped = FALSE;
    TRACE_EVENT1("ime", "TSFTextStore::GetTextExt", "screen rect", gfx::Rect(*rect).ToString());
    return S_OK;
}
...
HRESULT TSFTextStore::InsertEmbedded(DWORD flags, LONG acp_start, LONG acp_end, IDataObject* data_object, TS_TEXTCHANGE* change)
{
    // We don't support any embedded objects.
    NOTIMPLEMENTED();
    return E_NOTIMPL;
...
HRESULT TSFTextStore::InsertEmbeddedAtSelection(DWORD flags, IDataObject* data_object, LONG* acp_start, LONG* acp_end, TS_TEXTCHANGE* change)
{
    // We don't support any embedded objects.
    NOTIMPLEMENTED();
    return E_NOTIMPL;
...
HRESULT TSFTextStore::InsertTextAtSelection(
    DWORD flags, const wchar_t* text_buffer, ULONG text_buffer_size, LONG* acp_start, LONG* acp_end, TS_TEXTCHANGE* text_change)
{
    ...
    if (text_buffer_size >= 0) {
        if (!new_text_inserted_) {
            ...
            LONG old_delta = (LONG)replace_text_range_.start() - (LONG)replace_text_range_.end() + replace_text_size_;
            LONG new_delta = start_pos - end_pos + text_buffer_size;
            replace_text_range_.set_start(std::min(static_cast<size_t>(start_pos), replace_text_range_.start()));
            // New replacement text ends after previous replacement text. We need to
            // use the new end after adjusting with previous delta.
            if ((uint32_t)end_pos >= replace_text_range_.start() + replace_text_size_) {
    ...
    string_buffer_document_
        = string_buffer_document_.substr(0, start_pos) + std::u16string(text_buffer, text_buffer + text_buffer_size) + string_buffer_document_.substr(end_pos);

    // reconstruct string that needs to be inserted.
    string_pending_insertion_ = string_buffer_document_.substr(start_pos, text_buffer_size);
...
HRESULT TSFTextStore::QueryInsert(LONG acp_test_start, LONG acp_test_end, ULONG text_size, LONG* acp_result_start, LONG* acp_result_end)
{
    ...
    *acp_result_start = std::min(std::max(acp_test_start, composition_start), buffer_size);
    *acp_result_end = std::min(std::max(acp_test_end, composition_start), buffer_size) + text_size;
    return S_OK;
}

HRESULT TSFTextStore::QueryInsertEmbedded(const GUID* service, const FORMATETC* format, BOOL* insertable)
{
    if (!format)
        return E_INVALIDARG;
    // We don't support any embedded objects.
...
HRESULT TSFTextStore::RequestAttrsAtPosition(LONG acp_pos, ULONG attribute_buffer_size, const TS_ATTRID* attribute_buffer, DWORD flags)
{
...
HRESULT TSFTextStore::RequestAttrsTransitioningAtPosition(LONG acp_pos, ULONG attribute_buffer_size, const TS_ATTRID* attribute_buffer, DWORD flags)
{
    // We don't support any document attributes.
    // This method just returns S_OK, and the subsequently called
    // RetrieveRequestedAttrs() returns 0 as the number of supported attributes.
...
HRESULT TSFTextStore::RequestLock(DWORD lock_flags, HRESULT* result)
{
    ...
    const size_t last_composition_start = text_input_client_->HasCompositionText() ? composition_start_ : selection_.start();
    ...
    *result = text_store_acp_sink_->OnLockGranted(current_lock_type_);
    ...
    if (string_pending_insertion_.empty()) {
        if (!text_input_client_->HasCompositionText()) {
            ...
            if (new_text_inserted_ && !replace_text_range_.is_empty() && !replace_text_size_) {
            ...
            if (has_composition_range_ && on_start_composition_called_) {
                is_tic_write_in_progress_ = true;
                string_pending_insertion_ = string_buffer_document_.substr(composition_range_.GetMin(), composition_range_.length());
                StartCompositionOnExistingText();
                is_tic_write_in_progress_ = false;
            } else {
                composition_start_ = selection_.start();
                if (!selection_.EqualsIgnoringDirection(selection_from_client_) && !IsInputIME()) {
    ...
    if ((has_composition_range_ || on_start_composition_called_) && wparam_keydown_cached_ != 0 && lparam_keydown_cached_ != 0) {
        DispatchKeyEvent(ui::EventType::kKeyPressed, wparam_keydown_cached_, lparam_keydown_cached_);
    }
    ...
    size_t new_composition_start = !has_composition_range_ && new_text_inserted_ ? replace_text_range_.start() + replace_text_size_ : composition_start_;
    ...
    if (((new_composition_start > last_composition_start && text_input_client_->HasCompositionText()) || !has_composition_range_) && text_input_client_) {
        is_tic_write_in_progress_ = true;
        CommitTextAndEndCompositionIfAny(last_composition_start, new_composition_start);
        is_tic_write_in_progress_ = false;
    }

    const std::u16string& composition_string = string_buffer_document_.substr(composition_range_.GetMin(), composition_range_.length());
    ...
    if ((has_composition_range_
            && (previous_composition_start_ != composition_range_.start() || previous_composition_string_ != composition_string
                || !previous_composition_selection_range_.EqualsIgnoringDirection(selection_) || previous_text_spans_ != text_spans_))
        || ((wparam_keydown_fired_ != 0) && text_input_client_->HasCompositionText() && composition_string.empty())) {
        ...
        // there are any.
        is_tic_write_in_progress_ = true;
        if (new_text_inserted_ && !replace_text_range_.is_empty() && !text_input_client_->HasCompositionText()
            && last_composition_start > replace_text_range_.start()) {
            text_input_client_->ExtendSelectionAndDelete(last_composition_start - replace_text_range_.start(), 0);
        }
...
HRESULT TSFTextStore::RequestSupportedAttrs(DWORD /* flags */, // Seems that we should ignore this.
    ULONG attribute_buffer_size, const TS_ATTRID* attribute_buffer)
{
    if (!attribute_buffer)
        return E_INVALIDARG;
    if (!text_input_client_)
    ...
    for (size_t i = 0; i < attribute_buffer_size; ++i) {
        ...
        if (IsEqualGUID(GUID_PROP_INPUTSCOPE, attribute) || IsEqualGUID(GUID_PROP_URL, attribute) || IsEqualGUID(TSATTRID_Text_VerticalWriting, attribute)) {
...
HRESULT TSFTextStore::RetrieveRequestedAttrs(ULONG attribute_buffer_size, TS_ATTRVAL* attribute_buffer, ULONG* attribute_buffer_copied)
{
    ...
    *attribute_buffer_copied = std::min(attribute_buffer_size, static_cast<ULONG>(supported_attrs_.size()));
    ...
    for (size_t i = 0; i < *attribute_buffer_copied; ++i) {
        ...
        if (IsEqualGUID(GUID_PROP_INPUTSCOPE, supported_attrs_[i])) {
            ...
            attribute_buffer[i].varValue.punkVal = tsf_inputscope::CreateInputScope(
                text_input_client_->GetTextInputType(), text_input_client_->GetTextInputMode(), text_input_client_->ShouldDoLearning());
            attribute_buffer[i].varValue.punkVal->AddRef();
        } else if (IsEqualGUID(GUID_PROP_URL, supported_attrs_[i])) {
            const ui::TextInputClient::EditingContext editing_context = text_input_client_->GetTextEditingContext();
            ...
            attribute_buffer[i].varValue.bstrVal = SysAllocStringLen(wide_url.c_str(), wide_url.length());
        } else if (IsEqualGUID(TSATTRID_Text_VerticalWriting, supported_attrs_[i])) {
            attribute_buffer[i].varValue.vt = VT_BOOL;
            attribute_buffer[i].varValue.boolVal = !!(text_input_client_->GetTextInputFlags() & ui::TEXT_INPUT_FLAG_VERTICAL);
        }
    }
    return S_OK;
}

HRESULT TSFTextStore::SetSelection(ULONG selection_buffer_size, const TS_SELECTION_ACP* selection_buffer)
{
    if (!HasReadWriteLock())
        return TF_E_NOLOCK;
    if (selection_buffer_size > 0) {
        ...
        if (!((start_pos <= end_pos) && (end_pos <= static_cast<LONG>(string_buffer_document_.size())))) {
            return TF_E_INVALIDPOS;
        }
        selection_.set_start(start_pos);
...
HRESULT TSFTextStore::SetText(DWORD flags, LONG acp_start, LONG acp_end, const wchar_t* text_buffer, ULONG text_buffer_size, TS_TEXTCHANGE* text_change)
{
    ...
    ret = InsertTextAtSelection(0, text_buffer, text_buffer_size, &acp_start, &acp_end, &change);
    if (ret != S_OK)
        return ret;
...
HRESULT TSFTextStore::OnStartComposition(ITfCompositionView* composition_view, BOOL* ok)
{
...
HRESULT TSFTextStore::OnUpdateComposition(ITfCompositionView* composition_view, ITfRange* range)
{
    return S_OK;
}
...
void TSFTextStore::DispatchKeyEvent(ui::EventType type, WPARAM wparam, LPARAM lparam)
{
    if (!text_input_client_)
        return;
    ...
    const CHROME_MSG key_event_MSG = { window_handle_, message, VK_PROCESSKEY, lparam };
...
HRESULT TSFTextStore::OnEndEdit(ITfContext* context, TfEditCookie read_only_edit_cookie, ITfEditRecord* edit_record)
{
    ...
    ImeTextSpans spans;
    if (!GetCompositionStatus(context, read_only_edit_cookie, &committed_size, &spans)) {
        return S_OK;
    }
    text_spans_ = spans;
    edit_flag_ = true;
    ...
    if (enum_composition_view->Next(1, &composition_view, nullptr) == S_OK && SUCCEEDED(composition_view->GetRange(&range))
...
bool TSFTextStore::GetDisplayAttribute(TfGuidAtom guid_atom, TF_DISPLAYATTRIBUTE* attribute)
{
    ...
    if (FAILED(display_attribute_manager_->GetDisplayAttributeInfo(guid, &display_attribute_info, nullptr))) {
        return false;
    }
...
bool TSFTextStore::GetCompositionStatus(ITfContext* context, const TfEditCookie read_only_edit_cookie, size_t* committed_size, ImeTextSpans* spans)
{
    DCHECK(context);
    ...
    if (FAILED(context->TrackProperties(rgGuids, 2, nullptr, 0, &track_property))) {
        return false;
    }
    ...
    *committed_size = 0;
    ...
    if (FAILED(context->GetEnd(read_only_edit_cookie, &end_range)))
        return false;
    if (FAILED(start_to_end_range->ShiftEndToRange(read_only_edit_cookie, end_range.Get(), TF_ANCHOR_END))) {
        return false;
    }

    Microsoft::WRL::ComPtr<IEnumTfRanges> ranges;
    if (FAILED(track_property->EnumRanges(read_only_edit_cookie, &ranges, start_to_end_range.Get()))) {
        return false;
    }
    ...
    while (true) {
        ...
        if (FAILED(track_property->GetValue(read_only_edit_cookie, range.Get(), value.Receive()))) {
            return false;
        }
        if (FAILED(value.AsInput()->punkVal->QueryInterface(IID_PPV_ARGS(&enum_prop_value))))
            return false;
        ...
        while (enum_prop_value->Next(1, &property_value, nullptr) == S_OK) {
            if (IsEqualGUID(property_value.guidId, GUID_PROP_COMPOSING)) {
                ...
                TfGuidAtom guid_atom = static_cast<TfGuidAtom>(property_value.varValue.lVal);
        ...
        if (!is_composition) {
            ...
            if (selection_.EqualsIgnoringDirection(gfx::Range(span.start_offset, span.end_offset))) {
...
void TSFTextStore::ResetCompositionState()
{
    ...
    selection_ = gfx::Range(composition_from_client_.end(), composition_from_client_.end());
...
void TSFTextStore::CalculateTextandSelectionDiffAndNotifyIfNeeded()
{
    ...
    if (!text_input_client_ || is_notification_in_progress_ || is_tic_write_in_progress_)
        return;

    // TODO(snianu) Perhaps we can do the diff at the TextInputManager layer and
    // only report the diffs?
    TRACE_EVENT0("ime", "TSFTextStore::CalculateTextandSelectionDiffAndNotifyIfNeeded");
    ...
    if (text_input_client_->GetTextRange(&latest_buffer_range_from_client)
        && text_input_client_->GetTextFromRange(latest_buffer_range_from_client, &latest_buffer_from_client)
        && text_input_client_->GetEditableSelectionRange(&latest_selection_from_client)
        && latest_selection_from_client.IsBoundedBy(latest_buffer_range_from_client)) {
        gfx::Range latest_composition_from_client;
        if (text_input_client_->HasCompositionText() && text_input_client_->GetCompositionTextRange(&latest_composition_from_client))
        ...
        if (!buffer_from_client_.compare(latest_buffer_from_client) && selection_from_client_.EqualsIgnoringDirection(latest_selection_from_client)) {
            return;
        }
        ...
        bool notify_text_change = (text_store_acp_sink_mask_ & TS_AS_TEXT_CHANGE) != 0;
        bool notify_selection_change = (text_store_acp_sink_mask_ & TS_AS_SEL_CHANGE) != 0;
        ...
        if (latest_buffer_from_client.compare(string_buffer_document_)) {
            ...
            if (notify_text_change) {
                ...
                for (; acp_start < std::min(latest_buffer_from_client.size(), string_buffer_document_.size()); acp_start++) {
                    if (latest_buffer_from_client.at(acp_start) != string_buffer_document_.at(acp_start)) {
                        break;
                    }
                }
                ...
                while (acp_old_end > 0 && acp_new_end > 0) {
                    ...
                    if (acp_old_end >= acp_start && acp_new_end >= acp_start) {
                        if (latest_buffer_from_client.at(acp_new_end) != string_buffer_document_.at(acp_old_end)) {
                            acp_old_end++;
                            acp_new_end++;
                            break;
                        }
        ...
        if (notify_text_change && text_changed) {
            TRACE_EVENT2("ime", "TSFTextStore::CalculateTextandSelectionDiffAndNotifyIfNeeded", "text_change_start", base::NumberToString(text_change.acpStart),
        ...
        if (notify_selection_change && selection_changed) {
            TRACE_EVENT1("ime", "TSFTextStore::CalculateTextandSelectionDiffAndNotifyIfNeeded", "new_selection", selection_.ToString());
...
void TSFTextStore::SetFocusedTextInputClient(HWND focused_window, TextInputClient* text_input_client)
{
    window_handle_ = focused_window;
    text_input_client_ = text_input_client;
}

void TSFTextStore::RemoveFocusedTextInputClient(TextInputClient* text_input_client)
{
...
void TSFTextStore::SetImeKeyEventDispatcher(ImeKeyEventDispatcher* ime_key_event_dispatcher)
{
    ime_key_event_dispatcher_ = ime_key_event_dispatcher;
}

void TSFTextStore::RemoveImeKeyEventDispatcher(ImeKeyEventDispatcher* ime_key_event_dispatcher)
{
...
void TSFTextStore::StartCompositionOnExistingText() const
{
    ...
    text_input_client_->SetCompositionFromExistingText(composition_range_, text_spans);
}

void TSFTextStore::CommitTextAndEndCompositionIfAny(size_t old_size, size_t new_size) const
{
    ...
    if (new_text_inserted_ && !text_input_client_->HasCompositionText()) {
        ...
        new_committed_string_offset = std::min(static_cast<ULONG>(replace_text_range_.start()), static_cast<ULONG>(string_buffer_document_.size()));
        new_committed_string_size
            = (new_text_size == 0 && selection_.end() > new_committed_string_offset) ? selection_.end() - new_committed_string_offset : new_text_size;
        ...
        if (old_size > replace_text_range_.start()) {
            text_input_client_->ExtendSelectionAndDelete(old_size - replace_text_range_.start(), 0);
        }
    } else {
        ...
        if (new_text_inserted_ && (old_size > replace_text_range_.start()) && !replace_text_range_.is_empty()) {
            // Delete text that has already been inserted into blink.
            text_input_client_->ExtendSelectionAndDelete(replace_text_range_.end() - replace_text_range_.start(), 0);
        ...
        // Also clamp the offsets if they are out of bounds of the buffer
        new_committed_string_offset = std::min(static_cast<ULONG>(new_committed_string_offset), static_cast<ULONG>(string_buffer_document_.size()));
        new_committed_string_size = (new_committed_string_size == 0 && selection_.end() > new_committed_string_offset)
            ? selection_.end() - new_committed_string_offset
            : new_committed_string_size;
    }

    // Construct string to be committed.
    const std::u16string& new_committed_string = string_buffer_document_.substr(new_committed_string_offset, new_committed_string_size);
    ...
    if (!new_committed_string.empty()) {
        ...
        if (!text_input_client_->HasCompositionText() && on_start_composition_called_) {
            ImeTextSpans spans;
            ImeTextSpan span;
            span.start_offset = 0;
        ...
        text_input_client_->InsertText(new_committed_string, ui::TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    } else {
        text_input_client_->ClearCompositionText();
    }

    if (!selection_.is_empty() && !IsInputIME() && selection_.GetMax() <= string_buffer_document_.size()) {
        text_input_client_->SetEditableSelectionRange(selection_);
    }

    // Notify accessibility about this committed composition
    text_input_client_->SetActiveCompositionForAccessibility(replace_text_range_, new_committed_string,
        /*is_composition_committed*/ true);
}

void TSFTextStore::StartCompositionOnNewText(size_t start_offset, const std::u16string& composition_string)
{
    CompositionText composition_text;
    ...
    if (text_input_client_) {
        ...
        if (!composition_string.empty()) {
            text_input_client_->SetActiveCompositionForAccessibility(composition_range_, composition_string,
                /*is_composition_committed*/ false);
        } else {
            // User wants to commit the current composition
            const std::u16string& committed_string = string_buffer_document_.substr(composition_range_.GetMin(), composition_range_.length());
            text_input_client_->SetActiveCompositionForAccessibility(composition_range_, committed_string,
                /*is_composition_committed*/ true);
        }
    }
}

void TSFTextStore::GetStyle(const TF_DISPLAYATTRIBUTE& attribute, ImeTextSpan* span)
{
    ...
    // any style or not.
    span->thickness = attribute.fBoldLine ? ImeTextSpan::Thickness::kThick : ImeTextSpan::Thickness::kThin;
    ...
    if (attribute.crText.type != TF_CT_NONE) {
        span->text_color = SkColorSetRGB(GetRValue(attribute.crText.cr), GetGValue(attribute.crText.cr), GetBValue(attribute.crText.cr));
    }
    if (attribute.crLine.type != TF_CT_NONE) {
        span->underline_color = SkColorSetRGB(GetRValue(attribute.crLine.cr), GetGValue(attribute.crLine.cr), GetBValue(attribute.crLine.cr));
    }
}
...
bool TSFTextStore::IsInputIME() const
{
    ...
    if (SUCCEEDED(input_processor_profile_mgr_->GetActiveProfile(GUID_TFCAT_TIP_KEYBOARD, &profile))) {
        return profile.hkl == NULL && profile.dwProfileType == TF_PROFILETYPE_INPUTPROCESSOR;
    }
    return false;
}
