...
std::unique_ptr<VirtualKeyboardController> CreateKeyboardController(HWND attached_window_handle)
{
    if (base::win::GetVersion() >= base::win::Version::WIN10_RS4) {
        return std::make_unique<OnScreenKeyboardDisplayManagerInputPane>(attached_window_handle);
    } else {
        return std::make_unique<OnScreenKeyboardDisplayManagerTabTip>(attached_window_handle);
    }
}
...
bool IsRTLKeyboardLayoutInstalled()
{
    ...
    for (int i = 0; i < size; ++i) {
        if (IsRTLPrimaryLangID(PRIMARYLANGID(reinterpret_cast<uintptr_t>(layouts[i])))) {
...
InputMethodWinBase::InputMethodWinBase(ImeKeyEventDispatcher* ime_key_event_dispatcher, HWND attached_window_handle)
    : InputMethodBase(ime_key_event_dispatcher, CreateKeyboardController(attached_window_handle))
...
void InputMethodWinBase::OnDidChangeFocusedClient(TextInputClient* focused_before, TextInputClient* focused)
{
    if (focused_before != focused)
        accept_carriage_return_ = false;
}

ui::EventDispatchDetails InputMethodWinBase::DispatchKeyEvent(ui::KeyEvent* event)
{
    CHROME_MSG native_key_event = MSGFromKeyEvent(event);
    if (native_key_event.message == WM_CHAR) {
        auto ref = weak_ptr_factory_.GetWeakPtr();
        BOOL handled = FALSE;
        OnChar(native_key_event.hwnd, native_key_event.message, native_key_event.wParam, native_key_event.lParam, native_key_event, &handled);
        if (!ref)
    ...
    if (!HandlePeekMessage(native_key_event.hwnd, WM_CHAR, WM_DEADCHAR, &char_msgs)) {
        return DispatcherDestroyedDetails();
    }
    if (!HandlePeekMessage(native_key_event.hwnd, WM_SYSCHAR, WM_SYSDEADCHAR, &char_msgs)) {
        return DispatcherDestroyedDetails();
    }
    ...
    if (IsRTLKeyboardLayoutInstalled() && !IsTextInputTypeNone()) {
        ...
        } else if (event->type() == ui::EventType::kKeyReleased && (code == ui::VKEY_SHIFT || code == ui::VKEY_CONTROL)
            && pending_requested_direction_ != base::i18n::UNKNOWN_DIRECTION) {
            GetTextInputClient()->ChangeTextDirectionAndLayoutAlignment(pending_requested_direction_);
            pending_requested_direction_ = base::i18n::UNKNOWN_DIRECTION;
        }
    }
...
bool InputMethodWinBase::HandlePeekMessage(HWND hwnd, UINT msg_filter_min, UINT msg_filter_max, std::vector<CHROME_MSG>* char_msgs)
{
    auto ref = weak_ptr_factory_.GetWeakPtr();
    while (true) {
        CHROME_MSG msg_found;
        const bool result = !!::PeekMessage(ChromeToWindowsType(&msg_found), hwnd, msg_filter_min, msg_filter_max, PM_REMOVE);
...
bool InputMethodWinBase::IsWindowFocused(const TextInputClient* client) const
{
    ...
    return attached_window_handle_ && (GetActiveWindow() == attached_window_handle_ || GetFocus() == attached_window_handle_);
}

LRESULT InputMethodWinBase::OnChar(HWND window_handle, UINT message, WPARAM wparam, LPARAM lparam, const CHROME_MSG& event, BOOL* handled)
{
    *handled = TRUE;
...
LRESULT InputMethodWinBase::OnImeRequest(UINT message, WPARAM wparam, LPARAM lparam, BOOL* handled)
{
    *handled = FALSE;
...
LRESULT InputMethodWinBase::OnDocumentFeed(RECONVERTSTRING* reconv)
{
    ...
    if (!result || target_range.is_empty()) {
        if (!client->GetEditableSelectionRange(&target_range) || !target_range.IsValid()) {
            return 0;
        }
    }
    ...
    memcpy((char*)reconv + sizeof(RECONVERTSTRING), text.c_str(), len * sizeof(WCHAR));
...
LRESULT InputMethodWinBase::OnReconvertString(RECONVERTSTRING* reconv)
{
    ...
    if (!client->GetEditableSelectionRange(&selection_range) || selection_range.is_empty()) {
        return 0;
    }
    ...
    memcpy(reinterpret_cast<char*>(reconv) + sizeof(RECONVERTSTRING), text.c_str(), len * sizeof(WCHAR));
...
LRESULT InputMethodWinBase::OnQueryCharPosition(IMECHARPOSITION* char_positon)
{
    ...
    gfx::Rect dip_rect;
    if (client->HasCompositionText()) {
        if (!client->GetCompositionCharacterBounds(char_positon->dwCharPos, &dip_rect)) {
            return 0;
        }
    } else {
    ...
    const gfx::Rect rect = display::win::ScreenWin::DIPToScreenRect(attached_window_handle_, dip_rect);
...
void InputMethodWinBase::ProcessKeyEventDone(ui::KeyEvent* event, const std::vector<CHROME_MSG>* char_msgs, bool is_handled)
{
    if (is_handled)
        return;
...
ui::EventDispatchDetails InputMethodWinBase::ProcessUnhandledKeyEvent(ui::KeyEvent* event, const std::vector<CHROME_MSG>* char_msgs)
{
    DCHECK(event);
