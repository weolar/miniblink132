...
constexpr base::TimeDelta kDismissKeyboardRetryTimeout = base::Milliseconds(100);
constexpr int kDismissKeyboardMaxRetries = 5;

constexpr wchar_t kOSKClassName[] = L"IPTip_Main_Window";

constexpr wchar_t kWindows8OSKRegPath[] = L"Software\\Classes\\CLSID\\{054AAE20-4BEA-4347-8A35-64A533254A9D}"
                                          L"\\LocalServer32";
...
public:
    OnScreenKeyboardDetector(OnScreenKeyboardDisplayManagerTabTip* display_manager);
...
private:
    ...
    base::WeakPtrFactory<OnScreenKeyboardDetector> keyboard_detector_factory_ { this };
};

// OnScreenKeyboardDetector member definitions.
OnScreenKeyboardDetector::OnScreenKeyboardDetector(OnScreenKeyboardDisplayManagerTabTip* display_manager)
...
void OnScreenKeyboardDetector::DetectKeyboard(HWND main_window)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&OnScreenKeyboardDetector::CheckIfKeyboardVisible, keyboard_detector_factory_.GetWeakPtr()), kCheckOSKDelay);
}
    ...
    if (keyboard_detect_requested_) {
        if (keyboard_dismiss_retry_count_ < kDismissKeyboardMaxRetries) {
            ...
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE,
                base::BindOnce(base::IgnoreResult(&OnScreenKeyboardDetector::DismissKeyboard), keyboard_detector_factory_.GetWeakPtr()),
...
gfx::Rect OnScreenKeyboardDetector::GetOccludedRect()
{
    ...
    if (!::GetWindowRect(osk, &osk_rect) || !::GetWindowRect(main_window_, &main_window_rect)) {
        return occluded_rect;
    }
...
void OnScreenKeyboardDetector::HideIfNecessary()
{
    ...
    if (!::IsWindowEnabled(osk)) {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&OnScreenKeyboardDetector::HideIfNecessary, keyboard_detector_factory_.GetWeakPtr()), kCheckOSKDelay);
    }
}

void OnScreenKeyboardDetector::HandleKeyboardVisible(const gfx::Rect& occluded_rect)
{
...
OnScreenKeyboardDisplayManagerTabTip::OnScreenKeyboardDisplayManagerTabTip(HWND hwnd)
    : hwnd_(hwnd)
{
}
...
bool OnScreenKeyboardDisplayManagerTabTip::DisplayVirtualKeyboard()
{
    ...
    HINSTANCE ret = ::ShellExecuteW(nullptr, L"", osk_path_.c_str(), nullptr, nullptr, SW_SHOW);
...
void OnScreenKeyboardDisplayManagerTabTip::AddObserver(VirtualKeyboardControllerObserver* observer)
{
    observers_.AddObserver(observer);
}

void OnScreenKeyboardDisplayManagerTabTip::RemoveObserver(VirtualKeyboardControllerObserver* observer)
{
...
bool OnScreenKeyboardDisplayManagerTabTip::GetOSKPath(std::wstring* osk_path)
{
    ...
    base::win::RegKey key(HKEY_LOCAL_MACHINE, kWindows8OSKRegPath, KEY_READ | KEY_WOW64_64KEY);
    DWORD osk_path_length = 1024;
    if (key.ReadValue(nullptr, base::WriteInto(osk_path, osk_path_length), &osk_path_length, nullptr) != ERROR_SUCCESS) {
        return false;
    }
    ...
    *osk_path = base::ToLowerASCII(*osk_path);
    ...
    if (common_program_files_offset != std::wstring::npos) {
        ...
        osk_path->erase(common_program_files_offset, wcslen(L"%commonprogramfiles%"));
        ...
        DWORD buffer_size = GetEnvironmentVariable(L"CommonProgramW6432", nullptr, 0);
        if (buffer_size) {
            GetEnvironmentVariable(L"CommonProgramW6432", base::WriteInto(&common_program_files_path, buffer_size), buffer_size);
            DCHECK(!common_program_files_path.empty());
        } else {
            base::win::ScopedCoMem<wchar_t> common_program_files;
            if (FAILED(SHGetKnownFolderPath(FOLDERID_ProgramFilesCommon, 0, nullptr, &common_program_files))) {
                return false;
            }
...
void OnScreenKeyboardDisplayManagerTabTip::NotifyKeyboardVisible(const gfx::Rect& occluded_rect)
{
    observers_.Notify(&VirtualKeyboardControllerObserver::OnKeyboardVisible, occluded_rect);
}
