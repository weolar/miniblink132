...
class COMPONENT_EXPORT(UI_BASE_IME_WIN) OnScreenKeyboardDisplayManagerInputPane final : public VirtualKeyboardController {
public:
    explicit OnScreenKeyboardDisplayManagerInputPane(HWND hwnd);

    OnScreenKeyboardDisplayManagerInputPane(const OnScreenKeyboardDisplayManagerInputPane&) = delete;
    OnScreenKeyboardDisplayManagerInputPane& operator=(const OnScreenKeyboardDisplayManagerInputPane&) = delete;
    ...
    void SetInputPaneForTesting(Microsoft::WRL::ComPtr<ABI::Windows::UI::ViewManagement::IInputPane> pane);
    // Returns whether show/hide VK API is called from
    // VirtualKeyboardController or not.
    mojom::VirtualKeyboardVisibilityRequest GetLastVirtualKeyboardVisibilityRequest() const
    {
...
private:
    ...
    const HWND hwnd_;
    base::ObserverList<VirtualKeyboardControllerObserver, false>::Unchecked observers_;
    ...
    mojom::VirtualKeyboardVisibilityRequest last_vk_visibility_request_ = mojom::VirtualKeyboardVisibilityRequest::NONE;
    std::unique_ptr<VirtualKeyboardDebounceTimer> debouncer_;
    base::WeakPtrFactory<OnScreenKeyboardDisplayManagerInputPane> weak_factory_ { this };
};
