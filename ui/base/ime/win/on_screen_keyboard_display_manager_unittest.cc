...
class MockVirtualKeyboardControllerObserver : public VirtualKeyboardControllerObserver {
public:
    MockVirtualKeyboardControllerObserver() = default;

    MockVirtualKeyboardControllerObserver(const MockVirtualKeyboardControllerObserver&) = delete;
    MockVirtualKeyboardControllerObserver& operator=(const MockVirtualKeyboardControllerObserver&) = delete;
...
class MockInputPane : public Microsoft::WRL::RuntimeClass<Microsoft::WRL::RuntimeClassFlags<Microsoft::WRL::WinRt | Microsoft::WRL::InhibitRoOriginateError>,
                          ABI::Windows::UI::ViewManagement::IInputPane2, ABI::Windows::UI::ViewManagement::IInputPane> {
public:
    ...
    IFACEMETHODIMP
    add_Showing(InputPaneEventHandler* handler, EventRegistrationToken* token) override
    {
    ...
    IFACEMETHODIMP add_Hiding(InputPaneEventHandler* handler, EventRegistrationToken* token) override
    {
    ...
    {
        return show_handler_ && hide_handler_;
    }
...
protected:
    ...
    void WaitForEventsWithTimeDelay(int64_t time_delta_ms = 10)
    {
        ...
        task_environment_.GetMainThreadTaskRunner()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), base::Milliseconds(time_delta_ms));
        run_loop.Run();
    }
...
{
    std::unique_ptr<OnScreenKeyboardDisplayManagerTabTip> keyboard_display_manager(CreateTabTip());
...
TEST_F(OnScreenKeyboardTest, InputPane)
{
    ...
    if (base::win::GetVersion() < base::win::Version::WIN10_RS1)
        return;
    std::unique_ptr<OnScreenKeyboardDisplayManagerInputPane> keyboard_display_manager = CreateInputPane();

    std::unique_ptr<MockVirtualKeyboardControllerObserver> observer = std::make_unique<MockVirtualKeyboardControllerObserver>();

    Microsoft::WRL::ComPtr<MockInputPane> input_pane = Microsoft::WRL::Make<MockInputPane>();
