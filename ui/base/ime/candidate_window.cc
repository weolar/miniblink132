...
{
    if (page_size() != cw.page_size() || cursor_position() != cw.cursor_position() || is_cursor_visible() != cw.is_cursor_visible()
        || orientation() != cw.orientation() || show_window_at_composition() != cw.show_window_at_composition()
        || is_auxiliary_text_visible() != cw.is_auxiliary_text_visible() || auxiliary_text() != cw.auxiliary_text()
        || candidates_.size() != cw.candidates_.size() || is_user_selecting() != cw.is_user_selecting()) {
        return false;
    }
    ...
    for (size_t i = 0; i < candidates_.size(); ++i) {
        ...
        if (left.value != right.value || left.label != right.label || left.annotation != right.annotation || left.description_title != right.description_title
...
void CandidateWindow::GetInfolistEntries(std::vector<ui::InfolistEntry>* infolist_entries, bool* has_highlighted) const
{
    ...
    *has_highlighted = false;
    ...
    for (size_t i = 0; i < candidates().size(); ++i) {
        ...
        InfolistEntry entry(candidate_entry.description_title, candidate_entry.description_body);
