...
TEST(UtfOffsetTest, Utf16OffsetFromUtf8Offset)
{
    ...
    for (const auto& test_case : kTestCases) {
        EXPECT_EQ(test_case.expect, Utf16OffsetFromUtf8Offset(test_case.str, test_case.offset)) << " at " << test_case.str << "[" << test_case.offset << "]";
    }
}
...
TEST(UtfOffsetTest, Utf8OffsetFromUtf16Offset)
{
    ...
    for (const auto& test_case : kTestCases) {
        EXPECT_EQ(test_case.expect, Utf8OffsetFromUtf16Offset(test_case.str, test_case.offset)) << " at " << test_case.str << "[" << test_case.offset << "]";
    }
}
