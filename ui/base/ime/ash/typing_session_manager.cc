...
TypingSessionManager::TypingSessionManager(const TypingSessionManager& typing_session_manager)
{
}
...
void TypingSessionManager::Heartbeat()
{
    ...
    // start a new one.
    if (is_active_) {
        if (last_user_action_time_ + base::Milliseconds(INACTIVITY_TIMEOUT_FOR_SESSION_IN_MS) < current_time) {
            EndAndRecordSession();
        }
    }
...
void TypingSessionManager::EndAndRecordSession()
{
    ...
    if (characters_committed_this_session_count_ >= MIN_CHARACTERS_COMMITTED && time_taken >= MIN_SESSION_DURATION_IN_MS) {
