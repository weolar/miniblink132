...
public:
    ...
    public:
        ...
        virtual void InputMethodChanged(InputMethodManager* manager, Profile* profile, bool show_message) = 0;
        ...
        virtual void OnExtraInputEnabledStateChange(
            bool is_extra_input_options_enabled, bool is_emoji_enabled, bool is_handwriting_enabled, bool is_voice_enabled)
        {
        }
        ...
        virtual void OnInputMethodExtensionRemoved(const std::string& extension_id)
        {
        }
    };
    ...
    public:
        ...
        // is called.
        virtual void ImeMenuItemsChanged(const std::string& engine_id, const std::vector<MenuItem>& items) = 0;
    };
    ...
    public:
        ...
        // |instance|.
        virtual void AddInputMethodExtension(const std::string& extension_id, const InputMethodDescriptors& descriptors, TextInputMethod* instance) = 0;
        ...
        // input method list.
        virtual void ChangeInputMethod(const std::string& input_method_id, bool show_message) = 0;
        ...
        virtual bool EnableInputMethod(const std::string& new_enabled_input_method_id) = 0;
        ...
        virtual void EnableLoginLayouts(const std::string& language_code, const std::vector<std::string>& initial_layouts) = 0;
        ...
        virtual InputMethodDescriptors GetEnabledInputMethodsSortedByLocalizedDisplayNames() const = 0;
        ...
        // Although presented as a list, the result is NOT sorted in any specific
        // order; the ordering is arbitrary and undefined.
        virtual const std::vector<std::string>& GetEnabledInputMethodIds() const = 0;
        ...
        virtual const InputMethodDescriptor* GetInputMethodFromId(const std::string& input_method_id) const = 0;
        ...
        virtual void SetInputMethodLoginDefaultFromVPD(const std::string& locale, const std::string& layout) = 0;
        ...
        virtual bool ReplaceEnabledInputMethods(const std::vector<std::string>& new_enabled_input_method_ids) = 0;
        ...
        // non-empty.
        virtual bool SetAllowedInputMethods(const std::vector<std::string>& allowed_input_method_ids) = 0;
        ...
        // are allowed.
        virtual const std::vector<std::string>& GetAllowedInputMethodIds() const = 0;
    ...
    // TODO(crbug/1279743): This is a stateful global. Make it into true global
    // singleton first, then use dependency injection instead in the next step.
    static COMPONENT_EXPORT(UI_BASE_IME_ASH) void Initialize(InputMethodManager* instance);
    ...
    virtual void AddCandidateWindowObserver(CandidateWindowObserver* observer) = 0;
    virtual void AddImeMenuObserver(ImeMenuObserver* observer) = 0;
    virtual void RemoveObserver(Observer* observer) = 0;
    virtual void RemoveCandidateWindowObserver(CandidateWindowObserver* observer) = 0;
    ...
    virtual void ConnectInputEngineManager(mojo::PendingReceiver<ime::mojom::InputEngineManager> receiver) = 0;

    // Connects a receiver to the InputMethodUserDataService instance.
    virtual void BindInputMethodUserDataService(mojo::PendingReceiver<ime::mojom::InputMethodUserDataService> receiver) = 0;
    ...
    virtual std::string GetMigratedInputMethodID(const std::string& input_method_id) = 0;
    ...
    virtual bool GetMigratedInputMethodIDs(std::vector<std::string>* input_method_ids) = 0;
    ...
    virtual void NotifyImeMenuItemsChanged(const std::string& engine_id, const std::vector<MenuItem>& items) = 0;
    ...
    virtual void SetImeMenuFeatureEnabled(ImeMenuFeature feature, bool enabled) = 0;
    ...
    virtual void NotifyInputMethodExtensionAdded(const std::string& extension_id) = 0;
    virtual void NotifyInputMethodExtensionRemoved(const std::string& extension_id) = 0;
};
