...
const int kArcIMEPrefixLength = sizeof(kArcIMEPrefix) / sizeof(kArcIMEPrefix[0]) - 1;
...
const char kBrailleImeEngineId[] = "_comp_ime_jddehjeebkoimngcbdkaahpobgicbffpbraille";

const char kArcImeLanguage[] = "_arc_ime_language_";

std::string GetInputMethodID(const std::string& extension_id, const std::string& engine_id)
{
...
std::string GetComponentInputMethodID(const std::string& extension_id, const std::string& engine_id)
{
    DCHECK(!extension_id.empty());
    DCHECK(!engine_id.empty());
...
std::string GetArcInputMethodID(const std::string& extension_id, const std::string& engine_id)
{
    DCHECK(!extension_id.empty());
    DCHECK(!engine_id.empty());
...
std::string GetExtensionIDFromInputMethodID(const std::string& input_method_id)
{
...
{
    if (engine_id.starts_with(kComponentExtensionIMEPrefix) || engine_id.starts_with(kExtensionIMEPrefix) || engine_id.starts_with(kArcIMEPrefix)) {
        return engine_id;
    }
    ...
    if (engine_id.starts_with("zh-") && engine_id.find("pinyin") != std::string::npos) {
        return GetComponentInputMethodID(kChinesePinyinExtensionId, engine_id);
    }
    if (engine_id.starts_with("zh-") && engine_id.find("zhuyin") != std::string::npos) {
        return GetComponentInputMethodID(kChineseZhuyinExtensionId, engine_id);
    }
    if (engine_id.starts_with("zh-") && engine_id.find("cangjie") != std::string::npos) {
...
{
    return input_method_id.starts_with(kExtensionIMEPrefix) && input_method_id.size() > kExtensionIMEPrefixLength + kExtensionIdLength;
}

bool IsComponentExtensionIME(const std::string& input_method_id)
{
    return input_method_id.starts_with(kComponentExtensionIMEPrefix) && input_method_id.size() > kComponentExtensionIMEPrefixLength + kExtensionIdLength;
}

bool IsArcIME(const std::string& input_method_id)
{
    return input_method_id.starts_with(kArcIMEPrefix) && input_method_id.size() > kArcIMEPrefixLength + kExtensionIdLength;
}
...
#if BUILDFLAG(GOOGLE_CHROME_BRANDING)
    return input_method_id == base::StrCat({ kComponentExtensionIMEPrefix, kXkbExtensionId, "ko-t-i0-und" });
#else
    return false;
#endif
}
