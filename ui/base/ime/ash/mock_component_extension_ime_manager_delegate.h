...
class COMPONENT_EXPORT(UI_BASE_IME_ASH) MockComponentExtensionIMEManagerDelegate : public ComponentExtensionIMEManagerDelegate {
public:
    MockComponentExtensionIMEManagerDelegate();
    ~MockComponentExtensionIMEManagerDelegate() override;

    MockComponentExtensionIMEManagerDelegate(const MockComponentExtensionIMEManagerDelegate&) = delete;
    MockComponentExtensionIMEManagerDelegate& operator=(const MockComponentExtensionIMEManagerDelegate&) = delete;

    std::vector<ComponentExtensionIME> ListIME() override;
    void Load(content::BrowserContext*, const std::string& extension_id, const std::string& manifest, const base::FilePath& path) override;
    ...
    int load_call_count() const
    {
        return load_call_count_;
    }
