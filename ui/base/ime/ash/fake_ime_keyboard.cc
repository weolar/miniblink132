...
void FakeImeKeyboard::SetCurrentKeyboardLayoutByName(const std::string& layout_name, base::OnceCallback<void(bool)> callback)
{
    ++set_current_keyboard_layout_by_name_count_;
    std::move(callback).Run(ImeKeyboard::SetCurrentKeyboardLayoutByNameImpl(layout_name));
}
