...
void MockIMEInputContextHandler::CommitText(const std::u16string& text, ui::TextInputClient::InsertTextCursorBehavior cursor_behavior)
{
...
void MockIMEInputContextHandler::UpdateCompositionText(const ui::CompositionText& text, uint32_t cursor_pos, bool visible)
{
...
bool MockIMEInputContextHandler::SetCompositionRange(uint32_t before, uint32_t after, const std::vector<ui::ImeTextSpan>& text_spans)
{
...
bool MockIMEInputContextHandler::SetComposingRange(uint32_t start, uint32_t end, const std::vector<ui::ImeTextSpan>& text_spans)
{
...
void MockIMEInputContextHandler::SetAutocorrectRange(const gfx::Range& range, SetAutocorrectRangeDoneCallback callback)
{
...
std::optional<ui::GrammarFragment> MockIMEInputContextHandler::GetGrammarFragmentAtCursor()
{
...
bool MockIMEInputContextHandler::ClearGrammarFragments(const gfx::Range& range)
{
...
bool MockIMEInputContextHandler::AddGrammarFragments(const std::vector<ui::GrammarFragment>& fragments)
{
    grammar_fragments_.insert(grammar_fragments_.end(), fragments.begin(), fragments.end());
    return true;
}

void MockIMEInputContextHandler::DeleteSurroundingText(uint32_t num_char16s_before_cursor, uint32_t num_char16s_after_cursor)
{
    ++delete_surrounding_text_call_count_;
    last_delete_surrounding_text_arg_.num_char16s_before_cursor = num_char16s_before_cursor;
    last_delete_surrounding_text_arg_.num_char16s_after_cursor = num_char16s_after_cursor;
}

void MockIMEInputContextHandler::ReplaceSurroundingText(
    uint32_t length_before_selection, uint32_t length_after_selection, const std::u16string_view replacement_text)
{
    last_replace_surrounding_text_arg_.length_before_selection = length_before_selection;
    last_replace_surrounding_text_arg_.length_after_selection = length_after_selection;
    last_replace_surrounding_text_arg_.replacement_text = std::u16string(replacement_text);
}
...
void MockIMEInputContextHandler::ConfirmComposition(bool reset_engine)
{
    ...
    CommitText(last_update_composition_arg_.composition_text.text, ui::TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
