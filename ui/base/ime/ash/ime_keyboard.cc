...
void ImeKeyboard::SetCurrentKeyboardLayoutByName(const std::string& layout_name, base::OnceCallback<void(bool)> callback)
{
    std::move(callback).Run(SetCurrentKeyboardLayoutByNameImpl(layout_name));
}

bool ImeKeyboard::SetCurrentKeyboardLayoutByNameImpl(const std::string& layout_name)
{
...
void ImeKeyboard::SetCapsLockEnabled(bool enable_caps_lock)
{
    ...
    if (old_state != enable_caps_lock) {
        ...
        observers_.Notify(&ImeKeyboard::Observer::OnCapsLockChanged, enable_caps_lock);
    }
}
