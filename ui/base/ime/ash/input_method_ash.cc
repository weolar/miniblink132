...
template <typename T> T ConvertTextInputFlagToEnum(int flags, int flag_on_value, int flag_off_value)
{
...
{
    return url.spec().starts_with("chrome-untrusted://terminal") || url.spec().starts_with("chrome-untrusted://crosh");
}
...
InputMethodAsh::InputMethodAsh(ui::ImeKeyEventDispatcher* ime_key_event_dispatcher)
...
InputMethodAsh::PendingSetCompositionRange::PendingSetCompositionRange(const gfx::Range& range, const std::vector<ui::ImeTextSpan>& text_spans)
    : range(range)
    , text_spans(text_spans)
{
}

InputMethodAsh::PendingSetCompositionRange::PendingSetCompositionRange(const PendingSetCompositionRange& other) = default;

InputMethodAsh::PendingSetCompositionRange::~PendingSetCompositionRange() = default;

InputMethodAsh::PendingAutocorrectRange::PendingAutocorrectRange(const gfx::Range& range, TextInputTarget::SetAutocorrectRangeDoneCallback callback)
    : range(range)
...
ui::EventDispatchDetails InputMethodAsh::DispatchKeyEvent(ui::KeyEvent* event)
{
    ...
    if (manager) {
        ...
        if (keyboard && event->type() == ui::EventType::kKeyPressed && event->key_code() != ui::VKEY_CAPITAL
        ...
        input_method::InputMethodManager::State* state = manager->GetActiveIMEState().get();
    ...
    // are not supported.
    if (base::FeatureList::IsEnabled(features::kInputMethodDeadKeyFixForNoInputField) && GetTextInputClient() == nullptr) {
        return DispatchKeyEventPostIME(event);
    }
    ...
    if (IsPasswordOrNoneInputFieldFocused() || !GetEngine()) {
        if (event->type() == ui::EventType::kKeyPressed) {
            if (ExecuteCharacterComposer(*event)) {
                ...
                return ProcessKeyEventPostIME(event, ui::ime::KeyEventHandledState::kHandledByIME,
    ...
    GetEngine()->ProcessKeyEvent(*event,
        base::BindOnce(&InputMethodAsh::ProcessKeyEventDone, weak_ptr_factory_.GetWeakPtr(),
            // Pass the ownership of the new copied event.
            base::Owned(new ui::KeyEvent(*event))));
    return dispatch_details_.value_or(ui::EventDispatchDetails());
}

void InputMethodAsh::ProcessKeyEventDone(ui::KeyEvent* event, ui::ime::KeyEventHandledState handled_state)
{
    DCHECK(event);
    ...
    if (event->type() == ui::EventType::kKeyPressed) {
        if (handled_state != ui::ime::KeyEventHandledState::kNotHandled) {
            ...
            if (!is_handled_by_char_composer && !ui::KeycodeConverter::IsDomKeyForModifier(event->GetDomKey())) {
    ...
    if (event->type() == ui::EventType::kKeyPressed || event->type() == ui::EventType::kKeyReleased) {
        ui::ime::KeyEventHandledState handled_state_to_process = is_handled_by_char_composer ? ui::ime::KeyEventHandledState::kHandledByIME : handled_state;
...
void InputMethodAsh::OnCaretBoundsChanged(const TextInputClient* client)
{
    ...
    IMECandidateWindowHandlerInterface* candidate_window = IMEBridge::Get()->GetCandidateWindowHandler();
    IMEAssistiveWindowHandlerInterface* assistive_window = IMEBridge::Get()->GetAssistiveWindowHandler();
    ...
    if (candidate_window) {
        candidate_window->SetCursorAndCompositionBounds(caret_rect, composition_bounds);
    }
    ...
    if (!client->GetTextRange(&text_range) || !client->GetTextFromRange(text_range, &surrounding_text)
    ...
    if (GetEngine()) {
        ...
        const gfx::Range relative_selection_range(selection_range.start() - offset, selection_range.end() - offset);
        GetEngine()->SetSurroundingText(surrounding_text, relative_selection_range, offset);
    }
}
...
void InputMethodAsh::OnWillChangeFocusedClient(TextInputClient* focused_before, TextInputClient* focused)
{
    ConfirmComposition(/* reset_engine */ true);
...
void InputMethodAsh::OnDidChangeFocusedClient(TextInputClient* focused_before, TextInputClient* focused)
{
...
bool InputMethodAsh::SetCompositionRange(uint32_t before, uint32_t after, const std::vector<ui::ImeTextSpan>& text_spans)
{
    ...
    const gfx::Range composition_range(range.start() >= before ? range.start() - before : 0, range.end() + after);
    ...
    return SetComposingRange(composition_range.start(), composition_range.end(), text_spans);
}

bool InputMethodAsh::SetComposingRange(uint32_t start, uint32_t end, const std::vector<ui::ImeTextSpan>& text_spans)
{
    TextInputClient* client = GetTextInputClient();
    ...
    auto non_empty_text_spans = !text_spans.empty() ? text_spans
                                                    : std::vector<ui::ImeTextSpan> { ui::ImeTextSpan(ui::ImeTextSpan::Type::kComposition,
    ...
    if (handling_key_event_) {
        ...
        pending_composition_range_ = PendingSetCompositionRange { composition_range, non_empty_text_spans };
        return true;
    } else {
        composing_text_ = true;
        return client->SetCompositionFromExistingText(composition_range, non_empty_text_spans);
    }
}
...
void InputMethodAsh::SetAutocorrectRange(const gfx::Range& range, SetAutocorrectRangeDoneCallback callback)
{
    if (IsTextInputTypeNone()) {
    ...
    if (handling_key_event_) {
        ...
        pending_autocorrect_range_ = std::make_unique<InputMethodAsh::PendingAutocorrectRange>(range, std::move(callback));
    } else {
        std::move(callback).Run(GetTextInputClient()->SetAutocorrectRange(range));
    }
}

std::optional<ui::GrammarFragment> InputMethodAsh::GetGrammarFragmentAtCursor()
{
    if (IsTextInputTypeNone()) {
        return std::nullopt;
    }
...
bool InputMethodAsh::AddGrammarFragments(const std::vector<ui::GrammarFragment>& fragments)
{
    if (IsTextInputTypeNone()) {
        return false;
    }
...
void InputMethodAsh::ConfirmComposition(bool reset_engine)
{
    ...
    if (client && (client->HasCompositionText() || client->SupportsAlwaysConfirmComposition())) {
        const size_t characters_committed = client->ConfirmCompositionText(/*keep_selection*/ true);
...
void InputMethodAsh::UpdateContextFocusState()
{
    ...
    TextInputClient* client = GetTextInputClient();
    focused_url_ = client && !IsPasswordOrNoneInputFieldFocused() ? client->GetTextEditingContext().page_url : GURL();
}

ui::EventDispatchDetails InputMethodAsh::ProcessKeyEventPostIME(ui::KeyEvent* event, ui::ime::KeyEventHandledState handled_state, bool stopped_propagation)
{
    bool handled
        = handled_state == ui::ime::KeyEventHandledState::kHandledByIME || handled_state == ui::ime::KeyEventHandledState::kHandledByAssistiveSuggester;

    auto properties = event->properties() ? *event->properties() : ui::Event::Properties();
    // Mark whether the key is handled by IME or not.
    ui::SetKeyboardImeFlagProperty(&properties, handled ? ui::kPropertyKeyboardImeHandledFlag : ui::kPropertyKeyboardImeIgnoredFlag);
    // Mark whether autorepeat needs to be suppressed.
    if (handled_state == ui::ime::KeyEventHandledState::kNotHandledSuppressAutoRepeat) {
    ...
    if (event->type() == ui::EventType::kKeyPressed && handled) {
        bool only_dispatch_vkey_processkey = (handled_state == ui::ime::KeyEventHandledState::kHandledByAssistiveSuggester);
        ui::EventDispatchDetails dispatch_details = ProcessFilteredKeyPressEvent(event, only_dispatch_vkey_processkey);
...
ui::EventDispatchDetails InputMethodAsh::ProcessFilteredKeyPressEvent(ui::KeyEvent* event, bool only_dispatch_vkey_processkey)
{
    if (!only_dispatch_vkey_processkey) {
        ...
        // method.
        if ((base::FeatureList::IsEnabled(features::kInputMethodDeadKeyFix) || (focused_url_.is_valid() && IsTerminalOrCrosh(focused_url_)))
            && event->GetDomKey().IsDeadKey()) {
            return DispatchKeyEventPostIME(event);
        }
    }

    ui::KeyEvent fabricated_event(ui::EventType::kKeyPressed, ui::VKEY_PROCESSKEY, event->code(), event->flags(), ui::DomKey::PROCESS, event->time_stamp());
    if (const auto* properties = event->properties()) {
        fabricated_event.SetProperties(*properties);
    }
    ui::EventDispatchDetails dispatch_details = DispatchKeyEventPostIME(&fabricated_event);
...
ui::EventDispatchDetails InputMethodAsh::ProcessUnfilteredKeyPressEvent(ui::KeyEvent* event)
{
...
void InputMethodAsh::MaybeProcessPendingInputMethodResult(ui::KeyEvent* event, bool handled)
{
    TextInputClient* client = GetTextInputClient();
    DCHECK(client);
    ...
    if (pending_commit_) {
        if (handled && NeedInsertChar()) {
            for (const auto& ch : pending_commit_->text) {
                ui::KeyEvent ch_event(ui::EventType::kKeyPressed, ui::VKEY_UNKNOWN, ui::EF_NONE);
            ...
            client->InsertText(u"", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
            composing_text_ = false;
        } else {
            // Split the commit into two separate commits, one for the substring
            // before the cursor and one for the substring after.
            const std::u16string before_cursor = pending_commit_->text.substr(0, pending_commit_->cursor);
            if (!before_cursor.empty()) {
                client->InsertText(before_cursor, TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
            }
            const std::u16string after_cursor = pending_commit_->text.substr(pending_commit_->cursor);
            if (!after_cursor.empty()) {
                client->InsertText(after_cursor, TextInputClient::InsertTextCursorBehavior::kMoveCursorBeforeText);
            }
            composing_text_ = false;
        }
    ...
    if (composition_changed_ && !IsTextInputTypeNone()) {
        if (pending_composition_range_) {
            client->SetCompositionFromExistingText(pending_composition_range_->range, pending_composition_range_->text_spans);
        }
    ...
    if (pending_autocorrect_range_) {
        std::move(pending_autocorrect_range_->callback).Run(client->SetAutocorrectRange(pending_autocorrect_range_->range));
...
bool InputMethodAsh::NeedInsertChar() const
{
    return GetTextInputClient()
        && (IsTextInputTypeNone() || (!composing_text_ && pending_commit_ && pending_commit_->text.length() == 1 && pending_commit_->cursor == 1));
}
...
void InputMethodAsh::CommitText(const std::u16string& text, TextInputClient::InsertTextCursorBehavior cursor_behavior)
{
    ...
    if (cursor_behavior == TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText) {
...
void InputMethodAsh::UpdateCompositionText(const CompositionText& text, uint32_t cursor_pos, bool visible)
{
    if (IsTextInputTypeNone()) {
        return;
    }
...
TextInputMethod::InputContext InputMethodAsh::GetInputContext() const
{
    ...
    TextInputMethod::InputContext input_context(flags & ui::TEXT_INPUT_FLAG_HAS_BEEN_PASSWORD ? ui::TEXT_INPUT_TYPE_PASSWORD : client->GetTextInputType());
    input_context.mode = client->GetTextInputMode();
    input_context.autocompletion_mode
        = ConvertTextInputFlagToEnum<AutocompletionMode>(flags, ui::TEXT_INPUT_FLAG_AUTOCOMPLETE_ON, ui::TEXT_INPUT_FLAG_AUTOCOMPLETE_OFF);
    input_context.autocorrection_mode
        = ConvertTextInputFlagToEnum<AutocorrectionMode>(flags, ui::TEXT_INPUT_FLAG_AUTOCORRECT_ON, ui::TEXT_INPUT_FLAG_AUTOCORRECT_OFF);
    input_context.autocapitalization_mode = ConvertAutocapitalizationMode(flags);
    input_context.spellcheck_mode = ConvertTextInputFlagToEnum<SpellcheckMode>(flags, ui::TEXT_INPUT_FLAG_SPELLCHECK_ON, ui::TEXT_INPUT_FLAG_SPELLCHECK_OFF);
    input_context.focus_reason = client->GetFocusReason();
    input_context.personalization_mode = client->ShouldDoLearning() ? PersonalizationMode::kEnabled : PersonalizationMode::kDisabled;
...
SurroundingTextInfo InputMethodAsh::GetSurroundingTextInfo()
{
    ...
    TextInputClient* client = GetTextInputClient();
    if (!client || !client->GetTextRange(&text_range) || !client->GetTextFromRange(text_range, &info.surrounding_text)
    ...
    info.selection_range.set_start(info.selection_range.start() - text_range.start());
    info.selection_range.set_end(info.selection_range.end() - text_range.start());
    info.offset = text_range.start();
    return info;
}

void InputMethodAsh::DeleteSurroundingText(uint32_t num_char16s_before_cursor, uint32_t num_char16s_after_cursor)
{
    if (!GetTextInputClient()) {
        return;
    }
    ...
    GetTextInputClient()->ExtendSelectionAndDelete(num_char16s_before_cursor, num_char16s_after_cursor);
}

void InputMethodAsh::ReplaceSurroundingText(uint32_t length_before_selection, uint32_t length_after_selection, std::u16string_view replacement_text)
{
    if (!GetTextInputClient()) {
        return;
    }

    GetTextInputClient()->ExtendSelectionAndReplace(length_before_selection, length_after_selection, replacement_text);
}
...
bool InputMethodAsh::ExecuteCharacterComposer(const ui::KeyEvent& event)
{
    ...
    if (!commit_text.empty()) {
        CommitText(commit_text, TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    }
    return true;
}

CompositionText InputMethodAsh::ExtractCompositionText(const CompositionText& text, uint32_t cursor_position) const
{
    ...
    for (base::i18n::UTF16CharIterator char_iterator(composition.text); !char_iterator.end(); char_iterator.Advance()) {
    ...
    if (!text_ime_text_spans.empty()) {
        for (const auto& text_ime_text_span : text_ime_text_spans) {
            ...
            ui::ImeTextSpan ime_text_span(ui::ImeTextSpan::Type::kComposition, char16_offsets[start], char16_offsets[end], text_ime_text_span.thickness,
                ui::ImeTextSpan::UnderlineStyle::kSolid, text_ime_text_span.background_color);
    ...
    if (text.selection.start() < text.selection.end()) {
        const size_t start = std::min(text.selection.start(), static_cast<size_t>(char_length));
        const size_t end = std::min(text.selection.end(), static_cast<size_t>(char_length));
        ui::ImeTextSpan ime_text_span(ui::ImeTextSpan::Type::kComposition, char16_offsets[start], char16_offsets[end], ui::ImeTextSpan::Thickness::kThick,
    ...
    if (composition.ime_text_spans.empty()) {
        composition.ime_text_spans.push_back(ui::ImeTextSpan(
            ui::ImeTextSpan::Type::kComposition, 0, length, ui::ImeTextSpan::Thickness::kThin, ui::ImeTextSpan::UnderlineStyle::kSolid, SK_ColorTRANSPARENT));
    }
...
bool InputMethodAsh::IsPasswordOrNoneInputFieldFocused()
{
    ...
    return type == ui::TEXT_INPUT_TYPE_NONE || type == ui::TEXT_INPUT_TYPE_PASSWORD;
}
...
{
    ui::KeyEvent evt(
        pressed ? ui::EventType::kKeyPressed : ui::EventType::kKeyReleased, pressed ? ui::VKEY_PROCESSKEY : ui::VKEY_UNKNOWN, ui::EF_IME_FABRICATED_KEY);
