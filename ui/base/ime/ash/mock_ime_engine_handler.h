...
class COMPONENT_EXPORT(UI_BASE_IME_ASH) MockIMEEngineHandler : public TextInputMethod {
public:
    ...
    void Reset() override;
    void ProcessKeyEvent(const ui::KeyEvent& key_event, KeyEventDoneCallback callback) override;
    ...
    void SetSurroundingText(const std::u16string& text, gfx::Range selection_range, uint32_t offset_pos) override;
    bool IsReadyForTesting() override;

    const std::string& GetActiveComponentId() const;

    int focus_in_call_count() const
    {
        return focus_in_call_count_;
    }
    int focus_out_call_count() const
    {
        return focus_out_call_count_;
    }
    int reset_call_count() const
    {
