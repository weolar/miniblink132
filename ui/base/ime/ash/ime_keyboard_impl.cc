...
void ImeKeyboardImpl::SetCurrentKeyboardLayoutByName(const std::string& layout_name, base::OnceCallback<void(bool)> callback)
{
    const bool result = ImeKeyboard::SetCurrentKeyboardLayoutByNameImpl(layout_name);
    if (!result) {
...
{
    input_controller_->SetAutoRepeatRate(rate.initial_delay, rate.repeat_interval);
    return true;
}
