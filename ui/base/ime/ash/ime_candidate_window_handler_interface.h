...
public:
    ...
    // showing.
    virtual void UpdateLookupTable(const ui::CandidateWindow& candidate_window) = 0;

    // Called when the IME updates the preedit text. The |text| is given in
    // UTF-16 encoding.
    virtual void UpdatePreeditText(const std::u16string& text, uint32_t cursor_pos, bool visible) = 0;

    // Called when the application changes its caret or composition bounds.
    virtual void SetCursorAndCompositionBounds(const gfx::Rect& cursor_bounds, const gfx::Rect& composition_bounds) = 0;
