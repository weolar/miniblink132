...
public:
    InputMethodUtilTest()
        : util_(&delegate_)
    {
        delegate_.set_get_localized_string_callback(base::BindRepeating(&l10n_util::GetStringUTF16));

        xkb_input_method_descriptors_ = {
            GetDesc(Id("xkb:us::eng"), "", "us", { "en", "en-US", "en-AU", "en-NZ" }, "US", true),
        ...
        non_xkb_input_method_descriptors_ = {
            GetDesc(Id(pinyin_ime_id), "Pinyin input for testing", "us", { "zh-CN" }, "CN", false),
            GetDesc(Id(zhuyin_ime_id), "Zhuyin input for testing", "us", { "zh-TW" }, "TW", false),
        };
    }
    ...
    static InputMethodDescriptor GetDesc(const std::string& id, const std::string& description, const std::string& layout,
        const std::vector<std::string>& language_codes, const std::string& indicator, bool is_login_keyboard)
    {
...
TEST_F(InputMethodUtilTest, GetInputMethodLongNameTest)
{
    ...
    {
        InputMethodDescriptor desc = GetDesc("xkb:jp::jpn", "", "jp", { "ja" }, "", true);
        EXPECT_EQ(u"Japanese", util_.GetInputMethodLongName(desc));
    }
    {
        InputMethodDescriptor desc = GetDesc("xkb:us:dvorak:eng", "", "us(dvorak)", { "en-US" }, "", true);
        EXPECT_EQ(u"US Dvorak", util_.GetInputMethodLongName(desc));
    }
    {
        InputMethodDescriptor desc = GetDesc("xkb:gb:dvorak:eng", "", "gb(dvorak)", { "en-US" }, "", true);
        EXPECT_EQ(u"UK Dvorak", util_.GetInputMethodLongName(desc));
    }
    {
        InputMethodDescriptor desc = GetDesc("invalid-id", "", "us", { "xx" }, "", true);
...
TEST_F(InputMethodUtilTest, TestGetInputMethodDescriptorFromId)
{
    ...
    const InputMethodDescriptor* descriptor = util_.GetInputMethodDescriptorFromId(Id(pinyin_ime_id));
...
TEST_F(InputMethodUtilTest, TestGetInputMethodIdsForLanguageCode)
{
    ...
    EXPECT_TRUE(util_.GetInputMethodIdsFromLanguageCodeInternal(language_code_to_ids_map, "ja", kAllInputMethods, &result));
    EXPECT_EQ(3U, result.size());
    EXPECT_TRUE(util_.GetInputMethodIdsFromLanguageCodeInternal(language_code_to_ids_map, "ja", kKeyboardLayoutsOnly, &result));
    ASSERT_EQ(1U, result.size());
    EXPECT_EQ("xkb:jp:jpn", result[0]);

    EXPECT_TRUE(util_.GetInputMethodIdsFromLanguageCodeInternal(language_code_to_ids_map, "fr", kAllInputMethods, &result));
    ASSERT_EQ(1U, result.size());
    EXPECT_EQ("xkb:fr:fra", result[0]);
    EXPECT_TRUE(util_.GetInputMethodIdsFromLanguageCodeInternal(language_code_to_ids_map, "fr", kKeyboardLayoutsOnly, &result));
    ASSERT_EQ(1U, result.size());
    EXPECT_EQ("xkb:fr:fra", result[0]);

    EXPECT_FALSE(util_.GetInputMethodIdsFromLanguageCodeInternal(language_code_to_ids_map, "invalid_lang", kAllInputMethods, &result));
    EXPECT_FALSE(util_.GetInputMethodIdsFromLanguageCodeInternal(language_code_to_ids_map, "invalid_lang", kKeyboardLayoutsOnly, &result));
}

InputMethodDescriptor DescWithIdAndHandwritingLanguage(const std::string& id, std::optional<std::string> handwriting_language)
{
    const std::string input_method_id = extension_ime_util::GetInputMethodIDByEngineID(id);
    return InputMethodDescriptor(input_method_id, /*name=*/"", /*indicator=*/"",
        /*keyboard_layout=*/"",
...
TEST_F(InputMethodUtilTest, TestGetInputMethodIdsForHandwritingLanguage)
{
    ...
    std::vector<InputMethodDescriptor> descriptors = { DescWithIdAndHandwritingLanguage("xkb:us::eng", "en"),
        DescWithIdAndHandwritingLanguage("xkb:gb:extd:eng", "en"), DescWithIdAndHandwritingLanguage("xkb:fr::fra", "fr") };
    util_.InitXkbInputMethodsForTesting(descriptors);
    util_.AppendInputMethods(non_xkb_input_method_descriptors_);

    EXPECT_THAT(util_.GetInputMethodIdsFromHandwritingLanguage("en"),
        UnorderedElementsAre(extension_ime_util::GetInputMethodIDByEngineID("xkb:us::eng"), extension_ime_util::GetInputMethodIDByEngineID("xkb:gb:extd:eng")));
    EXPECT_THAT(util_.GetInputMethodIdsFromHandwritingLanguage("fr"), UnorderedElementsAre(extension_ime_util::GetInputMethodIDByEngineID("xkb:fr::fra")));
    EXPECT_THAT(util_.GetInputMethodIdsFromHandwritingLanguage("zh"), IsEmpty());
    EXPECT_THAT(util_.GetInputMethodIdsFromHandwritingLanguage("zh-CN"), IsEmpty());
    EXPECT_THAT(util_.GetInputMethodIdsFromHandwritingLanguage("zh-TW"), IsEmpty());
}

// US keyboard + English US UI = US keyboard only.
TEST_F(InputMethodUtilTest, TestGetFirstLoginInputMethodIds_Us_And_EnUs)
{
    const InputMethodDescriptor* descriptor = util_.GetInputMethodDescriptorFromId(Id("xkb:us::eng")); // US keyboard.
    ASSERT_TRUE(nullptr != descriptor); // ASSERT_NE doesn't compile.
...
{
    const InputMethodDescriptor* descriptor = util_.GetInputMethodDescriptorFromId(Id("xkb:jp::jpn")); // JP keyboard.
    ASSERT_TRUE(nullptr != descriptor); // ASSERT_NE doesn't compile.
    std::vector<std::string> input_method_ids;
    util_.GetFirstLoginInputMethodIds("ja", *descriptor, &input_method_ids);
    ASSERT_EQ(2U, input_method_ids.size());
...
TEST_F(InputMethodUtilTest, TestInputMethodIDMigration)
{
    ...
    for (size_t i = 0; i < std::size(migration_cases); ++i) {
        EXPECT_EQ(extension_ime_util::GetInputMethodIDByEngineID(migration_cases[i][1]), input_method_ids[i]);
    }
}
...
TEST_F(InputMethodUtilTest, TestHardwareInputMethodIDs)
{
    ...
    std::vector<std::string> login_input_method_ids = util_.GetHardwareLoginInputMethodIds();

    EXPECT_EQ(2U, input_method_ids.size());
    EXPECT_EQ(1U, login_input_method_ids.size());

    EXPECT_EQ("xkb:us::eng", extension_ime_util::GetComponentIDByInputMethodID(input_method_ids[0]));
    EXPECT_EQ("xkb:ru::rus", extension_ime_util::GetComponentIDByInputMethodID(input_method_ids[1]));
    EXPECT_EQ("xkb:us::eng", extension_ime_util::GetComponentIDByInputMethodID(login_input_method_ids[0]));
}
