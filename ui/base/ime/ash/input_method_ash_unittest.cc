...
uint32_t GetOffsetInUTF16(const std::u16string& utf16_string, uint32_t utf8_offset)
{
...
public:
    explicit TestableInputMethodAsh(ui::ImeKeyEventDispatcher* ime_key_event_dispatcher)
    ...
    struct ProcessKeyEventPostIMEArgs {
        ProcessKeyEventPostIMEArgs()
            : event(ui::EventType::kUnknown, ui::VKEY_UNKNOWN, ui::DomCode::NONE, ui::EF_NONE)
    ...
    ui::EventDispatchDetails ProcessKeyEventPostIME(ui::KeyEvent* key_event, ui::ime::KeyEventHandledState handled_state, bool stopped_propagation) override
    {
        ui::EventDispatchDetails details = InputMethodAsh::ProcessKeyEventPostIME(key_event, handled_state, stopped_propagation);
    ...
    void CommitText(const std::u16string& text, TextInputClient::InsertTextCursorBehavior cursor_behavior) override
    {
    ...
    const std::u16string& text_committed() const
    {
        return text_committed_;
    }
...
public:
    SetSurroundingTextVerifier(const std::string& expected_surrounding_text, uint32_t expected_cursor_position, uint32_t expected_anchor_position)
    ...
    SetSurroundingTextVerifier& operator=(const SetSurroundingTextVerifier&) = delete;

    void Verify(const std::string& text, uint32_t cursor_pos, uint32_t anchor_pos)
    {
...
class TestInputMethodManager : public input_method::MockInputMethodManager {
    ...
    public:
        ...
        bool is_jp_kbd() const
        {
            return is_jp_kbd_;
        }
        bool is_jp_ime() const
        {
            return is_jp_ime_;
        }
...
public:
    ...
    MOCK_METHOD3(SetSurroundingText, void(const std::u16string&, gfx::Range, uint32_t));
};

class InputMethodAshTest : public ui::ImeKeyEventDispatcher, public testing::Test, public ui::DummyTextInputClient {
public:
    InputMethodAshTest()
        : dispatched_key_event_(ui::EventType::kUnknown, ui::VKEY_UNKNOWN, ui::EF_NONE)
    ...
    void SetUp() override
    {
        ...
        mock_ime_candidate_window_handler_ = std::make_unique<MockIMECandidateWindowHandler>();
        IMEBridge::Get()->SetCandidateWindowHandler(mock_ime_candidate_window_handler_.get());
    ...
    void InsertText(const std::u16string& text, TextInputClient::InsertTextCursorBehavior cursor_behavior) override
    {
        inserted_text_ = text;
    }
    ...
    bool GetTextFromRange(const gfx::Range& range, std::u16string* text) const override
    {
    ...
    bool SetCompositionFromExistingText(const gfx::Range& range, const std::vector<ui::ImeTextSpan>& ui_ime_text_spans) override
    {
        composition_text_ = CompositionText();
    ...
    void ResetFlags()
    {
        dispatched_key_event_ = ui::KeyEvent(ui::EventType::kUnknown, ui::VKEY_UNKNOWN, ui::EF_NONE);
...
protected:
    ...
    std::unique_ptr<MockIMECandidateWindowHandler> mock_ime_candidate_window_handler_;
...
TEST_F(InputMethodAshTest, HasBeenPasswordShouldTriggerPassowrd)
{
    ...
    EXPECT_EQ(mock_ime_engine_handler_->last_text_input_context().type, ui::TEXT_INPUT_TYPE_PASSWORD);
...
TEST_F(InputMethodAshTest, GetInputTextType_WithoutFocusedClient)
{
    ...
    input_method_ash_->SetFocusedTextInputClient(this);
    input_method_ash_->OnTextInputTypeChanged(this);
    EXPECT_EQ(ui::TEXT_INPUT_TYPE_PASSWORD, input_method_ash_->GetTextInputType());
}
...
TEST_F(InputMethodAshTest, OnWillChangeFocusedClientClearAutocorrectRange)
{
    ...
    input_method_ash_->SetFocusedTextInputClient(this);
    input_method_ash_->CommitText(u"hello", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
...
TEST_F(InputMethodAshTest, Focus_Text)
{
    ...
    EXPECT_EQ(1, mock_ime_candidate_window_handler_->set_cursor_and_composition_bounds_call_count());
...
TEST_F(InputMethodAshTest, Focus_Scenario)
{
    ...
    EXPECT_EQ(0, mock_ime_engine_handler_->focus_out_call_count());
    EXPECT_EQ(ui::TEXT_INPUT_TYPE_NONE, mock_ime_engine_handler_->last_text_input_context().type);
    EXPECT_EQ(ui::TEXT_INPUT_MODE_DEFAULT, mock_ime_engine_handler_->last_text_input_context().mode);
    ...
    EXPECT_EQ(1, mock_ime_engine_handler_->focus_in_call_count());
    EXPECT_EQ(0, mock_ime_engine_handler_->focus_out_call_count());
    EXPECT_EQ(ui::TEXT_INPUT_TYPE_TEXT, mock_ime_engine_handler_->last_text_input_context().type);
    EXPECT_EQ(ui::TEXT_INPUT_MODE_TEXT, mock_ime_engine_handler_->last_text_input_context().mode);
    ...
    EXPECT_EQ(1, mock_ime_engine_handler_->focus_out_call_count());
    EXPECT_EQ(ui::TEXT_INPUT_TYPE_TEXT, mock_ime_engine_handler_->last_text_input_context().type);
    EXPECT_EQ(ui::TEXT_INPUT_MODE_SEARCH, mock_ime_engine_handler_->last_text_input_context().mode);
    ...
    EXPECT_EQ(ui::TEXT_INPUT_TYPE_URL, mock_ime_engine_handler_->last_text_input_context().type);
    EXPECT_EQ(ui::TEXT_INPUT_MODE_SEARCH, mock_ime_engine_handler_->last_text_input_context().mode);
...
TEST_F(InputMethodAshTest, OnCaretBoundsChanged)
{
    ...
    EXPECT_EQ(2, mock_ime_candidate_window_handler_->set_cursor_and_composition_bounds_call_count());
    caret_bounds_ = gfx::Rect(0, 2, 3, 4);
    input_method_ash_->OnCaretBoundsChanged(this);
    EXPECT_EQ(3, mock_ime_candidate_window_handler_->set_cursor_and_composition_bounds_call_count());
    ...
    EXPECT_EQ(4, mock_ime_candidate_window_handler_->set_cursor_and_composition_bounds_call_count());
}
...
TEST_F(InputMethodAshTest, ExtractCompositionTextTest_NoAttribute)
{
    ...
    CompositionText composition_text = input_method_ash_->ExtractCompositionText(ash_composition_text, kCursorPos);
    ...
    EXPECT_EQ(kSampleAsciiText.size(), composition_text.ime_text_spans[0].end_offset);
    EXPECT_EQ(ui::ImeTextSpan::Thickness::kThin, composition_text.ime_text_spans[0].thickness);
}
...
TEST_F(InputMethodAshTest, ExtractCompositionTextTest_SingleUnderline)
{
    ...
    ui::ImeTextSpan underline(
        ui::ImeTextSpan::Type::kComposition, 1UL, 4UL, ui::ImeTextSpan::Thickness::kThin, ui::ImeTextSpan::UnderlineStyle::kSolid, SK_ColorTRANSPARENT);
    composition_text.ime_text_spans.push_back(underline);

    CompositionText composition_text2 = input_method_ash_->ExtractCompositionText(composition_text, kCursorPos);
    ...
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, underline.start_offset), composition_text2.ime_text_spans[0].start_offset);
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, underline.end_offset), composition_text2.ime_text_spans[0].end_offset);
    // Single underline represents as thin line with text color.
    EXPECT_EQ(SK_ColorTRANSPARENT, composition_text2.ime_text_spans[0].underline_color);
    EXPECT_EQ(ui::ImeTextSpan::Thickness::kThin, composition_text2.ime_text_spans[0].thickness);
    EXPECT_EQ(static_cast<SkColor>(SK_ColorTRANSPARENT), composition_text2.ime_text_spans[0].background_color);
}
...
TEST_F(InputMethodAshTest, ExtractCompositionTextTest_DoubleUnderline)
{
    ...
    composition_text.text = kSampleText;
    ui::ImeTextSpan underline(
        ui::ImeTextSpan::Type::kComposition, 1UL, 4UL, ui::ImeTextSpan::Thickness::kThick, ui::ImeTextSpan::UnderlineStyle::kSolid, SK_ColorTRANSPARENT);
    composition_text.ime_text_spans.push_back(underline);
    ...
    EXPECT_EQ(SK_ColorTRANSPARENT, composition_text2.ime_text_spans[0].underline_color);
    EXPECT_EQ(ui::ImeTextSpan::Thickness::kThick, composition_text2.ime_text_spans[0].thickness);
    EXPECT_EQ(static_cast<SkColor>(SK_ColorTRANSPARENT), composition_text2.ime_text_spans[0].background_color);
}
...
TEST_F(InputMethodAshTest, ExtractCompositionTextTest_Selection)
{
    ...
    EXPECT_EQ(kCursorPos, composition_text2.selection.end());
    ASSERT_EQ(1UL, composition_text2.ime_text_spans.size());
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, composition_text.selection.start()), composition_text2.ime_text_spans[0].start_offset);
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, composition_text.selection.end()), composition_text2.ime_text_spans[0].end_offset);
    EXPECT_EQ(SK_ColorTRANSPARENT, composition_text2.ime_text_spans[0].underline_color);
    EXPECT_EQ(ui::ImeTextSpan::Thickness::kThick, composition_text2.ime_text_spans[0].thickness);
    EXPECT_EQ(static_cast<SkColor>(SK_ColorTRANSPARENT), composition_text2.ime_text_spans[0].background_color);
}

TEST_F(InputMethodAshTest, ExtractCompositionTextTest_SelectionStartWithCursor)
{
    ...
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, composition_text.selection.end()), composition_text2.selection.start());
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, kCursorPos), composition_text2.selection.end());
    ASSERT_EQ(1UL, composition_text2.ime_text_spans.size());
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, composition_text.selection.start()), composition_text2.ime_text_spans[0].start_offset);
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, composition_text.selection.end()), composition_text2.ime_text_spans[0].end_offset);
    EXPECT_EQ(SK_ColorTRANSPARENT, composition_text2.ime_text_spans[0].underline_color);
...
TEST_F(InputMethodAshTest, ExtractCompositionTextTest_SelectionEndWithCursor)
{
    ...
    // If the cursor position is same as selection bounds, selection start
    // position become opposit side of selection from cursor.
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, composition_text.selection.start()), composition_text2.selection.start());
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, kCursorPos), composition_text2.selection.end());
    ASSERT_EQ(1UL, composition_text2.ime_text_spans.size());
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, composition_text.selection.start()), composition_text2.ime_text_spans[0].start_offset);
    EXPECT_EQ(GetOffsetInUTF16(kSampleText, composition_text.selection.end()), composition_text2.ime_text_spans[0].end_offset);
...
TEST_F(InputMethodAshTest, SurroundingText_NoSelectionTest)
{
    ...
    EXPECT_EQ(surrounding_text_, mock_ime_engine_handler_->last_set_surrounding_text());
    EXPECT_EQ(gfx::Range(3), mock_ime_engine_handler_->last_set_selection_range());
}
...
TEST_F(InputMethodAshTest, SurroundingText_SelectionTest)
{
    ...
    EXPECT_EQ(1, mock_ime_engine_handler_->set_surrounding_text_call_count());
    EXPECT_EQ(surrounding_text_, mock_ime_engine_handler_->last_set_surrounding_text());
    EXPECT_EQ(gfx::Range(2, 5), mock_ime_engine_handler_->last_set_selection_range());
}
...
TEST_F(InputMethodAshTest, SurroundingText_PartialText)
{
    ...
    EXPECT_EQ(gfx::Range(2, 4), mock_ime_engine_handler_->last_set_selection_range());
}
...
TEST_F(InputMethodAshTest, SetCompositionRangeWithSelectedTextAccountsForSelection)
{
    FakeTextInputClient fake_text_input_client(ui::TEXT_INPUT_TYPE_TEXT);
    ...
    EXPECT_THAT(fake_text_input_client.ime_text_spans(),
        testing::ElementsAre(ui::ImeTextSpan(ui::ImeTextSpan::Type::kComposition,
...
TEST_F(InputMethodAshTest, SetAutocorrectRange_SuccessfulSet)
{
    ...
    InsertText(u"a", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
...
public:
    ...
    InputMethodAshKeyEventTest& operator=(const InputMethodAshKeyEventTest&) = delete;
...
TEST_F(InputMethodAshKeyEventTest, KeyEventDelayResponseTest)
{
    ...
    static_cast<TextInputTarget*>(input_method_ash_.get())->CommitText(u"A", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);

    EXPECT_EQ(0, inserted_char_);

    // Do callback.
    std::move(mock_ime_engine_handler_->last_passed_callback()).Run(ui::ime::KeyEventHandledState::kHandledByIME);

    // Check the results
    EXPECT_EQ(1, input_method_ash_->process_key_event_post_ime_call_count());
    const ui::KeyEvent stored_event = input_method_ash_->process_key_event_post_ime_args().event;
    EXPECT_EQ(ui::VKEY_A, stored_event.key_code());
    EXPECT_EQ(kFlags, stored_event.flags());
    EXPECT_EQ(input_method_ash_->process_key_event_post_ime_args().handled_state, ui::ime::KeyEventHandledState::kHandledByIME);
...
TEST_F(InputMethodAshKeyEventTest, MultiKeyEventDelayResponseTest)
{
    ...
    comp.text = u"B";
    (static_cast<TextInputTarget*>(input_method_ash_.get()))->UpdateCompositionText(comp, comp.text.length(), true);
    ...
    EXPECT_EQ(1, input_method_ash_->process_key_event_post_ime_call_count());
    ui::KeyEvent stored_event = input_method_ash_->process_key_event_post_ime_args().event;
    ...
    mock_ime_engine_handler_->last_passed_callback().Run(ui::ime::KeyEventHandledState::kNotHandled);
    ...
    EXPECT_EQ(kFlags, stored_event.flags());
    EXPECT_EQ(input_method_ash_->process_key_event_post_ime_args().handled_state, ui::ime::KeyEventHandledState::kNotHandled);
...
TEST_F(InputMethodAshKeyEventTest, DeadKeyPressTest)
{
    ...
    ui::KeyEvent eventA(ui::EventType::kKeyPressed,
        ui::VKEY_OEM_4, // '['
        ui::DomCode::BRACKET_LEFT, 0, ui::DomKey::DeadKeyFromCombiningCharacter('^'), ui::EventTimeForNow());
    input_method_ash_->ProcessKeyEventPostIME(&eventA, ui::ime::KeyEventHandledState::kHandledByIME, true);
...
TEST_F(InputMethodAshTest, UnhandledDeadKeyForNonTerminalSendsDeadKeys)
{
    ...
    for (const GURL& url : {
        ...
        std::move(mock_ime_engine_handler_->last_passed_callback()).Run(ui::ime::KeyEventHandledState::kNotHandled);
        ...
        EXPECT_EQ(dispatched_key_press.GetDomKey(), ui::DomKey::DeadKeyFromCombiningCharacter('^'));
        ...
        EXPECT_EQ(dispatched_key_release.GetDomKey(), ui::DomKey::DeadKeyFromCombiningCharacter('^'));
    }
}
...
TEST_F(InputMethodAshTest, DeadKeyHandledByAssistiveSendsProcessKey)
{
    ...
    ui::KeyEvent key_press(ui::EventType::kKeyPressed,
        ...
        ui::DomCode::BRACKET_LEFT, 0, ui::DomKey::DeadKeyFromCombiningCharacter('^'), ui::EventTimeForNow());
    ime.DispatchKeyEvent(&key_press);
    std::move(mock_ime_engine_handler_->last_passed_callback()).Run(ui::ime::KeyEventHandledState::kHandledByAssistiveSuggester);
    const ui::KeyEvent dispatched_key_press = dispatched_key_event_;
...
TEST_F(InputMethodAshKeyEventTest, KeyboardImeFlags)
{
    ...
    {
        ui::KeyEvent eventA(ui::EventType::kKeyPressed, ui::VKEY_A, ui::DomCode::US_A, 0, ui::DomKey::FromCharacter('a'), ui::EventTimeForNow());
        input_method_ash_->ProcessKeyEventPostIME(&eventA, ui::ime::KeyEventHandledState::kHandledByIME, true);

        const ui::KeyEvent& key_event = dispatched_key_event_;
        EXPECT_EQ(ui::kPropertyKeyboardImeHandledFlag, ui::GetKeyboardImeFlags(key_event));
    }

    {
        ui::KeyEvent eventA(ui::EventType::kKeyPressed, ui::VKEY_A, ui::DomCode::US_A, 0, ui::DomKey::FromCharacter('a'), ui::EventTimeForNow());
        input_method_ash_->ProcessKeyEventPostIME(&eventA, ui::ime::KeyEventHandledState::kNotHandled, true);

        const ui::KeyEvent& key_event = dispatched_key_event_;
        EXPECT_EQ(ui::kPropertyKeyboardImeIgnoredFlag, ui::GetKeyboardImeFlags(key_event));
    }
}
...
TEST_F(InputMethodAshKeyEventTest, HandledKeyEventDoesNotSuppressAutoRepeat)
{
    ...
    {
        ui::KeyEvent eventA(ui::EventType::kKeyPressed, ui::VKEY_A, ui::DomCode::US_A, 0, ui::DomKey::FromCharacter('a'), ui::EventTimeForNow());
        input_method_ash_->ProcessKeyEventPostIME(&eventA, ui::ime::KeyEventHandledState::kHandledByIME,
            /*stopped_propagation=*/true);

        EXPECT_FALSE(ui::HasKeyEventSuppressAutoRepeat(*dispatched_key_event_.properties()));
    }

    {
        ui::KeyEvent eventA(ui::EventType::kKeyPressed, ui::VKEY_A, ui::DomCode::US_A, 0, ui::DomKey::FromCharacter('a'), ui::EventTimeForNow());
        input_method_ash_->ProcessKeyEventPostIME(&eventA, ui::ime::KeyEventHandledState::kHandledByAssistiveSuggester,
            /*stopped_propagation=*/true);
...
TEST_F(InputMethodAshKeyEventTest, NotHandledKeyEventDoesNotSuppressAutoRepeat)
{
    // Preparation.
    input_type_ = ui::TEXT_INPUT_TYPE_TEXT;
    input_method_ash_->OnTextInputTypeChanged(this);
    ...
    ui::KeyEvent eventA(ui::EventType::kKeyPressed, ui::VKEY_A, ui::DomCode::US_A, 0, ui::DomKey::FromCharacter('a'), ui::EventTimeForNow());
    input_method_ash_->ProcessKeyEventPostIME(&eventA, ui::ime::KeyEventHandledState::kNotHandled,
        /*stopped_propagation=*/false);

    EXPECT_FALSE(ui::HasKeyEventSuppressAutoRepeat(*dispatched_key_event_.properties()));
}

TEST_F(InputMethodAshKeyEventTest, NotHandledSuppressKeyEventSuppressesAutoRepeat)
{
    // Preparation.
    input_type_ = ui::TEXT_INPUT_TYPE_TEXT;
    input_method_ash_->OnTextInputTypeChanged(this);
    ...
    ui::KeyEvent eventA(ui::EventType::kKeyPressed, ui::VKEY_A, ui::DomCode::US_A, 0, ui::DomKey::FromCharacter('a'), ui::EventTimeForNow());
    input_method_ash_->ProcessKeyEventPostIME(&eventA, ui::ime::KeyEventHandledState::kNotHandledSuppressAutoRepeat,
        /*stopped_propagation=*/false);

    EXPECT_TRUE(ui::HasKeyEventSuppressAutoRepeat(*dispatched_key_event_.properties()));
}

TEST_F(InputMethodAshKeyEventTest, SingleCharAssistiveSuggesterKeyEventDispatchesProcessKey)
{
    ...
    input_method_ash_->OnTextInputTypeChanged(this);
    input_method_ash_->DispatchKeyEvent(&event);
    static_cast<TextInputTarget*>(input_method_ash_.get())->CommitText(u"b", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    std::move(mock_ime_engine_handler_->last_passed_callback()).Run(ui::ime::KeyEventHandledState::kHandledByAssistiveSuggester);
...
{
    ui::KeyEvent eventConvert(ui::EventType::kKeyPressed, ui::VKEY_CONVERT, ui::EF_NONE);
    ...
    ui::KeyEvent eventNonConvert(ui::EventType::kKeyPressed, ui::VKEY_NONCONVERT, ui::EF_NONE);
    ...
    ui::KeyEvent eventDbeSbc(ui::EventType::kKeyPressed, ui::VKEY_DBE_SBCSCHAR, ui::EF_NONE);
    input_method_ash_->DispatchKeyEvent(&eventDbeSbc);
    EXPECT_FALSE(input_method_manager_->state()->is_jp_kbd());
    EXPECT_TRUE(input_method_manager_->state()->is_jp_ime());

    ui::KeyEvent eventDbeDbc(ui::EventType::kKeyPressed, ui::VKEY_DBE_DBCSCHAR, ui::EF_NONE);
...
TEST_F(InputMethodAshKeyEventTest, SetAutocorrectRangeRunsAfterKeyEvent)
{
    input_type_ = ui::TEXT_INPUT_TYPE_TEXT;
    input_method_ash_->OnTextInputTypeChanged(this);
    input_method_ash_->CommitText(u"a", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);

    ui::KeyEvent event(ui::EventType::kKeyPressed, ui::VKEY_A, ui::DomCode::US_A, ui::EF_NONE, ui::DomKey::FromCharacter('a'), ui::EventTimeForNow());
    input_method_ash_->DispatchKeyEvent(&event);
...
TEST_F(InputMethodAshKeyEventTest, SetAutocorrectRangeRunsAfterCommitText)
{
    ...
    bool callback_called = false;
    input_method_ash_->SetAutocorrectRange(gfx::Range(0, 1), base::BindOnce([](bool* called, bool result) { *called = true; }, (&callback_called)));
    EXPECT_FALSE(callback_called);
...
TEST_F(InputMethodAshKeyEventTest, SetAutocorrectRangeCallsCallbackOnFailureAfterKeyEvent)
{
    input_type_ = ui::TEXT_INPUT_TYPE_TEXT;
    input_method_ash_->OnTextInputTypeChanged(this);
    input_method_ash_->CommitText(u"a", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
...
TEST_F(InputMethodAshKeyEventTest, LatestSetAutocorrectRangeOverridesPreviousRequestsWhileHandlingKeyEvent)
{
    input_type_ = ui::TEXT_INPUT_TYPE_TEXT;
    input_method_ash_->OnTextInputTypeChanged(this);
    input_method_ash_->CommitText(u"a", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    ...
    input_method_ash_->SetAutocorrectRange(gfx::Range(0, 1), base::BindOnce(set_range_callback, &first_set_callback_called, &first_set_callback_result));
    EXPECT_FALSE(first_set_callback_called);

    // Override first call.
    input_method_ash_->SetAutocorrectRange(gfx::Range(0, 1), base::BindOnce(set_range_callback, &second_set_callback_called, &second_set_callback_result));
...
TEST_F(InputMethodAshKeyEventTest, MultipleCommitTextsWhileHandlingKeyEventCoalescesIntoOne)
{
    FakeTextInputClient fake_text_input_client(ui::TEXT_INPUT_TYPE_TEXT);
    InputMethodAsh ime(this);
    ime.SetFocusedTextInputClient(&fake_text_input_client);
    ...
    ime.CommitText(u"a", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    ime.CommitText(u"b", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    ime.CommitText(u"cde", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    std::move(mock_ime_engine_handler_->last_passed_callback()).Run(ui::ime::KeyEventHandledState::kHandledByIME);
...
TEST_F(InputMethodAshKeyEventTest, MultipleCommitTextsWhileHandlingKeyEventCoalescesByCaretBehavior)
{
    FakeTextInputClient fake_text_input_client(ui::TEXT_INPUT_TYPE_TEXT);
    InputMethodAsh ime(this);
    ime.SetFocusedTextInputClient(&fake_text_input_client);
    ...
    ui::KeyEvent event(ui::EventType::kKeyPressed, ui::VKEY_A, ui::DomCode::US_A, ui::EF_NONE, ui::DomKey::FromCharacter('a'), ui::EventTimeForNow());
    ime.DispatchKeyEvent(&event);
    ime.CommitText(u"a", TextInputClient::InsertTextCursorBehavior::kMoveCursorBeforeText);
    ime.CommitText(u"b", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    ime.CommitText(u"c", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    ime.CommitText(u"d", TextInputClient::InsertTextCursorBehavior::kMoveCursorBeforeText);
    ime.CommitText(u"e", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    std::move(mock_ime_engine_handler_->last_passed_callback()).Run(ui::ime::KeyEventHandledState::kHandledByIME);
...
TEST_F(InputMethodAshKeyEventTest, CommitTextEmptyRunsAfterKeyEvent)
{
    ...
    ui::KeyEvent event(ui::EventType::kKeyPressed, ui::VKEY_A, ui::DomCode::US_A, ui::EF_NONE, ui::DomKey::FromCharacter('a'), ui::EventTimeForNow());
    ime.DispatchKeyEvent(&event);
    ime.CommitText(u"", TextInputClient::InsertTextCursorBehavior::kMoveCursorBeforeText);
    std::move(mock_ime_engine_handler_->last_passed_callback()).Run(ui::ime::KeyEventHandledState::kHandledByIME);
...
TEST_F(InputMethodAshTest, CommitTextReplacesSelection)
{
    ...
    ime.CommitText(u"", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
...
TEST_F(InputMethodAshTest, DoesNotResetEngineWithNoComposition)
{
    ...
    ime.CommitText(u"hello", TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
    ime.CancelComposition(&fake_text_input_client);
