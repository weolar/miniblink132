...
InputMethodDescriptor::InputMethodDescriptor(const std::string& id, const std::string& name, const std::string& indicator, const std::string& keyboard_layout,
    const std::vector<std::string>& language_codes, bool is_login_keyboard, const GURL& options_page_url, const GURL& input_view_url,
...
InputMethodDescriptor::InputMethodDescriptor(const InputMethodDescriptor& other) = default;
...
std::u16string InputMethodDescriptor::GetIndicator() const
{
    ...
    if (indicator_.empty()) {
        if (extension_ime_util::IsKeyboardLayoutExtension(id_)) {
            return base::UTF8ToUTF16(base::ToUpperASCII(keyboard_layout_.substr(0, 2)));
        }
        DCHECK(language_codes_.size() > 0);
        return base::UTF8ToUTF16(base::ToUpperASCII(language_codes_[0].substr(0, 2)));
    }
