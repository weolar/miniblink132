...
public:
    ...
    ComponentExtensionIMEManagerTest(const ComponentExtensionIMEManagerTest&) = delete;
    ComponentExtensionIMEManagerTest& operator=(const ComponentExtensionIMEManagerTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        component_ext_mgr_ = std::make_unique<ComponentExtensionIMEManager>(std::move(delegate));
    }

protected:
    raw_ptr<MockComponentExtensionIMEManagerDelegate, DanglingUntriaged> mock_delegate_;
...
TEST_F(ComponentExtensionIMEManagerTest, LoadComponentExtensionIMETest)
{
    for (size_t i = 0; i < ime_list_.size(); ++i) {
        for (size_t j = 0; j < ime_list_[i].engines.size(); ++j) {
            const std::string input_method_id = extension_ime_util::GetComponentInputMethodID(ime_list_[i].id, ime_list_[i].engines[j].engine_id);
            component_ext_mgr_->LoadComponentExtensionIME(nullptr /* profile */, input_method_id);
...
{
    EXPECT_TRUE(component_ext_mgr_->IsAllowlisted(extension_ime_util::GetComponentInputMethodID(ime_list_[0].id, ime_list_[0].engines[0].engine_id)));
    EXPECT_FALSE(component_ext_mgr_->IsAllowlisted(extension_ime_util::GetInputMethodID(ime_list_[0].id, ime_list_[0].engines[0].engine_id)));
    EXPECT_FALSE(component_ext_mgr_->IsAllowlisted("mozc"));
    EXPECT_FALSE(component_ext_mgr_->IsAllowlisted(extension_ime_util::GetInputMethodID("AAAA", "012345")));
    EXPECT_FALSE(component_ext_mgr_->IsAllowlisted(extension_ime_util::GetComponentInputMethodID("AAAA", "012345")));
}
...
TEST_F(ComponentExtensionIMEManagerTest, GetAllIMEAsInputMethodDescriptorHandwriting)
{
    ime_list_.clear();
    ...
    InputMethodDescriptors descriptors = component_ext_mgr_->GetAllIMEAsInputMethodDescriptor();

    EXPECT_THAT(descriptors,
        ElementsAre(Property(&InputMethodDescriptor::handwriting_language, Optional(Eq("en"))),
            Property(&InputMethodDescriptor::handwriting_language, Optional(Eq(""))), Property(&InputMethodDescriptor::handwriting_language, std::nullopt)));
}
