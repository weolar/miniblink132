...
std::vector<ComponentExtensionIME> MockComponentExtensionIMEManagerDelegate::ListIME()
{
    return ime_list_;
}

void MockComponentExtensionIMEManagerDelegate::Load(
    content::BrowserContext* profile, const std::string& extension_id, const std::string& manifest, const base::FilePath& path)
{
    load_call_count_++;
    last_loaded_extension_id_ = extension_id;
}

bool MockComponentExtensionIMEManagerDelegate::IsInLoginLayoutAllowlist(const std::string& layout)
{
