...
public:
    ...
    MockInputMethodManagerImpl& operator=(const MockInputMethodManagerImpl&) = delete;
    ...
    void SetComponentExtensionIMEManager(std::unique_ptr<ComponentExtensionIMEManager> comp_ime_manager);

    // Set the value returned by IsISOLevel5ShiftUsedByCurrentInputMethod
    void set_mod3_used(bool value)
    {
        mod3_used_ = value;
    }

    int add_observer_count() const
    {
        return add_observer_count_;
    }
    int remove_observer_count() const
    {
        return remove_observer_count_;
    }

    int add_menu_observer_count() const
    {
        return add_menu_observer_count_;
    }
    int remove_menu_observer_count() const
    {
        return remove_menu_observer_count_;
    }
