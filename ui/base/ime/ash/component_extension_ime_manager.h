...
public:
    explicit ComponentExtensionIMEManager(std::unique_ptr<ComponentExtensionIMEManagerDelegate> delegate);

    ComponentExtensionIMEManager(const ComponentExtensionIMEManager&) = delete;
    ComponentExtensionIMEManager& operator=(const ComponentExtensionIMEManager&) = delete;
    ...
    // |input_method_id|.
    bool LoadComponentExtensionIME(content::BrowserContext* profile, const std::string& input_method_id, std::set<std::string>* extension_loaded = nullptr);
    ...
    virtual input_method::InputMethodDescriptors GetXkbIMEAsInputMethodDescriptor();
...
private:
    ...
    bool FindEngineEntry(const std::string& input_method_id, ComponentExtensionIME* out_extension);
