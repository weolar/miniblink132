...
bool CheckCharacterComposeTable(const ui::CharacterComposer::ComposeBuffer& compose_sequence, uint32_t* composed_character)
{
    const ui::TreeComposeChecker kTreeComposeChecker(kCompositions);
    return kTreeComposeChecker.CheckSequence(compose_sequence, composed_character) != ui::ComposeChecker::CheckSequenceResult::NO_MATCH;
}
...
void CharacterComposer::UpdatePreeditStringSequenceMode()
{
    ...
    for (auto key : compose_buffer_) {
        if (key.IsCharacter()) {
            ...
            if (std::optional<char16_t> non_combining_character = DeadKeyToNonCombiningCharacter(key)) {
                base::WriteUnicodeCharacter(*non_combining_character, &preedit_string_);
            }
        } else if (key.IsComposeKey() && (compose_buffer_.size() == 1)) {
            base::WriteUnicodeCharacter(kPreeditStringComposeKeySymbol, &preedit_string_);
        }
    }
}
...
ComposeChecker::CheckSequenceResult TreeComposeChecker::CheckSequence(const ui::CharacterComposer::ComposeBuffer& sequence, uint32_t* composed_character) const
{
    *composed_character = 0;
    ...
    for (const auto& keystroke : sequence) {
        ...
        if (keystroke.IsDeadKey() || keystroke.IsComposeKey()) {
            ...
            // key.
            character = keystroke.IsComposeKey() ? 0 : keystroke.ToDeadKeyCombiningCharacter();
...
bool TreeComposeChecker::Find(uint16_t index, uint16_t size, uint16_t key, uint16_t* value) const
{
    struct TableEntry {
        uint16_t key;
        uint16_t value;
