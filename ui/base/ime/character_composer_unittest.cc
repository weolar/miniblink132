...
protected:
    ...
    KeyEvent* DeadKeyPress(char16_t combining_character) const
    {
        KeyEvent* event = new KeyEvent(
            EventType::kKeyPressed, VKEY_UNKNOWN, DomCode::NONE, EF_NONE, DomKey::DeadKeyFromCombiningCharacter(combining_character), EventTimeForNow());
        return event;
    }
    ...
    void ExpectDeadKeyComposed(char16_t combining_character, const std::u16string& expected_character)
    {
        std::unique_ptr<KeyEvent> event(DeadKeyPress(combining_character));
        EXPECT_TRUE(character_composer_->FilterKeyPress(*event));
    ...
    std::unique_ptr<KeyEvent> ComposeKeyPress() const
    {
        ...
        return std::make_unique<KeyEvent>(EventType::kKeyPressed, KeyboardCode::VKEY_COMPOSE, DomCode::ALT_RIGHT, EF_NONE, DomKey::COMPOSE, EventTimeForNow());
    }
    ...
    std::unique_ptr<KeyEvent> UnicodeKeyPress(KeyboardCode vkey, DomCode code, int flags, char16_t character) const
    {
        return std::make_unique<KeyEvent>(EventType::kKeyPressed, vkey, code, flags, DomKey::FromCharacter(character), EventTimeForNow());
    }

    // Expects key is not filtered and no character is composed.
    void ExpectUnicodeKeyNotFiltered(KeyboardCode vkey, DomCode code, int flags, char16_t character)
    {
    ...
    // Expects key is filtered and no character is composed.
    void ExpectUnicodeKeyFiltered(KeyboardCode vkey, DomCode code, int flags, char16_t character)
    {
        auto event = UnicodeKeyPress(vkey, code, flags, character);
        EXPECT_TRUE(character_composer_->FilterKeyPress(*event));
        EXPECT_TRUE(character_composer_->composed_character().empty());
    ...
    // Expects key is filtered and the given character is composed.
    void ExpectUnicodeKeyComposed(KeyboardCode vkey, DomCode code, int flags, char16_t character, const std::u16string& expected_character)
    {
        auto event = UnicodeKeyPress(vkey, code, flags, character);
        EXPECT_TRUE(character_composer_->FilterKeyPress(*event));
        EXPECT_EQ(expected_character, character_composer_->composed_character());
...
TEST_F(CharacterComposerTest, FullyMatchingSequences)
{
    ...
    ExpectDeadKeyFiltered(kCombiningAcute);
    ExpectUnicodeKeyComposed(VKEY_A, DomCode::US_A, EF_NONE, 'a', std::u16string(1, 0x00E1));
    ...
    ExpectDeadKeyFiltered(kCombiningAcute);
    ExpectUnicodeKeyComposed(VKEY_A, DomCode::US_A, EF_NONE, 'A', std::u16string(1, 0x00C1));
    ...
    ExpectComposeKeyFiltered();
    ExpectUnicodeKeyFiltered(VKEY_OEM_7, DomCode::QUOTE, 0, '\'');
    ExpectUnicodeKeyComposed(VKEY_OEM_7, DomCode::QUOTE, 0, '\'', std::u16string(1, 0x00B4));
    // LATIN SMALL LETTER A WITH CIRCUMFLEX AND ACUTE
    ExpectDeadKeyFiltered(kCombiningAcute);
    ExpectDeadKeyFiltered(kCombiningCircumflex);
    ExpectUnicodeKeyComposed(VKEY_A, DomCode::US_A, EF_NONE, 'a', std::u16string(1, 0x1EA5));
    ...
    ExpectUnicodeKeyComposed(VKEY_U, DomCode::US_U, EF_NONE, 'U', std::u16string(1, 0x1EEA));
    ...
    ExpectDeadKeyFiltered(kCombiningAcute);
    ExpectUnicodeKeyComposed(VKEY_C, DomCode::US_C, EF_NONE, 'C', std::u16string(1, 0x0106));
    ...
    ExpectDeadKeyFiltered(kCombiningAcute);
    ExpectUnicodeKeyComposed(VKEY_C, DomCode::US_C, EF_NONE, 'c', std::u16string(1, 0x0107));
    ...
    ExpectDeadKeyFiltered(kCombiningAcute);
    ExpectUnicodeKeyComposed(VKEY_E, DomCode::US_E, EF_NONE, 0x03B5, std::u16string(1, 0x03AD));
    ...
    ExpectDeadKeyFiltered('\'');
    ExpectUnicodeKeyComposed(VKEY_C, DomCode::US_C, EF_NONE, 'c', std::u16string(1, 0x00E7));
    // APOSTROPHE
    ExpectDeadKeyFiltered('\'');
    ExpectUnicodeKeyComposed(VKEY_SPACE, DomCode::SPACE, EF_NONE, ' ', std::u16string(1, '\''));
    // Unmatched composition with printable character.
    static constexpr char16_t kApostropheS[] = { '\'', 's' };
    ExpectDeadKeyFiltered('\'');
    ExpectUnicodeKeyComposed(VKEY_S, DomCode::US_S, EF_NONE, 's', std::u16string(kApostropheS, 2));
...
TEST_F(CharacterComposerTest, KeySequenceCompositionPreeditEnabled)
{
    ...
    character_composer_.reset(new CharacterComposer(CharacterComposer::PreeditStringMode::kAlwaysEnabled));
    ...
    // LATIN SMALL LETTER A WITH ACUTE (via Compose key)
    ExpectComposeKeyFiltered();
    EXPECT_EQ(character_composer_->preedit_string(), std::u16string(1, CharacterComposer::kPreeditStringComposeKeySymbol));
    ExpectUnicodeKeyFiltered(VKEY_OEM_7, DomCode::QUOTE, 0, '\'');
...
TEST_F(CharacterComposerTest, HexadecimalComposition)
{
    ...
    ExpectUnicodeKeyFiltered(VKEY_U, DomCode::US_U, EF_SHIFT_DOWN | EF_CONTROL_DOWN, 'U');
    ...
    ExpectUnicodeKeyComposed(VKEY_SPACE, DomCode::SPACE, EF_NONE, ' ', std::u16string(1, 0x3042));
    ...
    ExpectUnicodeKeyComposed(VKEY_RETURN, DomCode::ENTER, EF_NONE, '\r', std::u16string(kMusicalKeyboard, kMusicalKeyboard + std::size(kMusicalKeyboard)));
}
...
TEST_F(CharacterComposerTest, HexadecimalCompositionPreedit)
{
    ...
    EXPECT_EQ(u"u304", character_composer_->preedit_string());
    ExpectUnicodeKeyFiltered(VKEY_2, DomCode::DIGIT2, EF_NONE, '2');
    ExpectUnicodeKeyComposed(VKEY_RETURN, DomCode::ENTER, EF_NONE, '\r', std::u16string(1, 0x3042));
...
TEST_F(CharacterComposerTest, HexadecimalCompositionWithNonHexKey)
{
    ...
    ExpectUnicodeKeyFiltered(VKEY_U, DomCode::US_U, EF_SHIFT_DOWN | EF_CONTROL_DOWN, 0x15);
    ExpectUnicodeKeyFiltered(VKEY_X, DomCode::US_X, 0, 'x');
...
TEST_F(CharacterComposerTest, HexadecimalCompositionWithAdditionalModifiers)
{
    // Ctrl+Shift+Alt+U
    // HIRAGANA LETTER A (U+3042)
    ExpectUnicodeKeyFiltered(VKEY_U, DomCode::US_U, EF_SHIFT_DOWN | EF_CONTROL_DOWN | EF_ALT_DOWN, 0x15);
    ExpectUnicodeKeyFiltered(VKEY_3, DomCode::DIGIT3, EF_NONE, '3');
    ExpectUnicodeKeyFiltered(VKEY_0, DomCode::DIGIT0, EF_NONE, '0');
    ExpectUnicodeKeyFiltered(VKEY_4, DomCode::DIGIT4, EF_NONE, '4');
    ExpectUnicodeKeyFiltered(VKEY_2, DomCode::DIGIT2, EF_NONE, '2');
    ...
    ExpectUnicodeKeyNotFiltered(VKEY_U, DomCode::US_U, EF_SHIFT_DOWN | EF_CONTROL_DOWN | EF_CAPS_LOCK_ON, 'u');
}
...
TEST_F(CharacterComposerTest, HexadecimalCompositionPreeditWithModifierPressed)
{
    ...
    EXPECT_EQ(u"u304a", character_composer_->preedit_string());
    ExpectUnicodeKeyFiltered(ui::VKEY_BACK, DomCode::BACKSPACE, kControlShift, '\b');
    EXPECT_EQ(u"u304", character_composer_->preedit_string());
    ExpectUnicodeKeyFiltered(ui::VKEY_2, DomCode::DIGIT2, kControlShift, 0);
    EXPECT_EQ(u"u3042", character_composer_->preedit_string());
    ExpectUnicodeKeyComposed(VKEY_RETURN, DomCode::ENTER, kControlShift, '\r', std::u16string(1, 0x3042));
    EXPECT_EQ(u"", character_composer_->preedit_string());
    ...
    ExpectUnicodeKeyFiltered(ui::VKEY_2, DomCode::DIGIT2, kControlShift, 0);
    EXPECT_EQ(u"u3042", character_composer_->preedit_string());
    ExpectUnicodeKeyFiltered(ui::VKEY_ESCAPE, DomCode::ESCAPE, kControlShift, 0x1B);
    EXPECT_EQ(u"", character_composer_->preedit_string());
}
