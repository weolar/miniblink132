...
public:
    ...
    DummyTextInputClient(TextInputType text_input_type, TextInputMode text_input_mode);
    ...
    std::optional<size_t> GetProximateCharacterIndexFromPoint(const gfx::Point& point, IndexFromPointFlags flags) const override;
    ...
    bool AddGrammarFragments(const std::vector<GrammarFragment>& fragments) override;
#endif
    ...
    int insert_char_count() const
    {
        return insert_char_count_;
    }
    char16_t last_insert_char() const
    {
        return last_insert_char_;
    }
