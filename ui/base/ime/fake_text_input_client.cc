...
FakeTextInputClient::FakeTextInputClient(InputMethod* input_method, Options options)
...
void FakeTextInputClient::SetTextAndSelection(const std::u16string& text, gfx::Range selection)
{
...
void FakeTextInputClient::SetCompositionText(const CompositionText& composition)
{
...
void FakeTextInputClient::InsertText(const std::u16string& text, TextInputClient::InsertTextCursorBehavior cursor_behavior)
{
    const gfx::Range replacement_range = composition_range_.is_empty() ? selection_ : composition_range_;
    ...
    if (cursor_behavior == TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText) {
        selection_ = gfx::Range(replacement_range.start() + text.length(), replacement_range.start() + text.length());
    }
...
#if BUILDFLAG(IS_WIN)
std::optional<gfx::Rect> FakeTextInputClient::GetProximateCharacterBounds(const gfx::Range& range) const
{
    return std::nullopt;
}

std::optional<size_t> FakeTextInputClient::GetProximateCharacterIndexFromPoint(const gfx::Point& point, IndexFromPointFlags flags) const
{
    return std::nullopt;
}
#endif // BUILDFLAG(IS_WIN)

bool FakeTextInputClient::GetCompositionCharacterBounds(size_t index, gfx::Rect* rect) const
{
    return false;
}
...
bool FakeTextInputClient::GetTextFromRange(const gfx::Range& range, std::u16string* text) const
{
...
bool FakeTextInputClient::ChangeTextDirectionAndLayoutAlignment(base::i18n::TextDirection direction)
{
    return false;
}

void FakeTextInputClient::ExtendSelectionAndDelete(size_t before, size_t after)
{
}
...
bool FakeTextInputClient::IsTextEditCommandEnabled(TextEditCommand command) const
{
    return false;
}

void FakeTextInputClient::SetTextEditCommandForNextKeyEvent(TextEditCommand command)
{
}
...
bool FakeTextInputClient::SetCompositionFromExistingText(const gfx::Range& range, const std::vector<ui::ImeTextSpan>& ui_ime_text_spans)
{
...
void FakeTextInputClient::GetActiveTextInputControlLayoutBounds(std::optional<gfx::Rect>* control_bounds, std::optional<gfx::Rect>* selection_bounds)
{
}

ui::TextInputClient::EditingContext FakeTextInputClient::GetTextEditingContext()
{
...
void FakeTextInputClient::SetActiveCompositionForAccessibility(
    const gfx::Range& range, const std::u16string& active_composition_text, bool is_composition_committed)
{
}
#endif
