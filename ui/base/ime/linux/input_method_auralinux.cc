...
bool IsEventFromVK(const ui::KeyEvent& event)
{
    ...
    return properties && properties->find(ui::kPropertyFromVK) != properties->end();
}
...
bool IsSameKeyEvent(const ui::KeyEvent& lhs, const ui::KeyEvent& rhs)
{
    ...
    return lhs.type() == rhs.type() && lhs.code() == rhs.code() && (lhs.flags() & ~ui::EF_IS_REPEAT) == (rhs.flags() & ~ui::EF_IS_REPEAT);
}
...
InputMethodAuraLinux::InputMethodAuraLinux(ImeKeyEventDispatcher* ime_key_event_dispatcher)
...
ui::EventDispatchDetails InputMethodAuraLinux::DispatchKeyEvent(ui::KeyEvent* event)
{
    DCHECK(event->type() == EventType::kKeyPressed || event->type() == EventType::kKeyReleased);
    ...
    if (ime_filtered_key_event_.has_value() && !IsSameKeyEvent(*ime_filtered_key_event_, *event) && ime_filtered_key_event_->GetDomKey().IsDeadKey()) {
    ...
    if (!GetTextInputClient()) {
        ...
        if (event->type() == EventType::kKeyPressed && context_->IsPeekKeyEvent(*event)) {
    ...
    if (IsEventFromVK(*event)) {
        ...
        if ((event->is_char() || event->GetDomKey().IsCharacter()) && event->type() == ui::EventType::kKeyPressed) {
    ...
    if (event->type() == ui::EventType::kKeyPressed && filtered) {
        ...
        if (details.target_destroyed || details.dispatcher_destroyed || event->stopped_propagation()) {
            return details;
        }
    }
    ...
    should_stop_propagation |= UpdateCompositionIfChanged(commit_result == CommitResult::kSuccess);
...
ui::EventDispatchDetails InputMethodAuraLinux::DispatchImeFilteredKeyPressEvent(ui::KeyEvent* event)
{
    ...
    if (event->key_code() == VKEY_RETURN)
        ...
        details = NeedInsertChar(result_text_) ? DispatchKeyEventPostIME(event) : SendFakeProcessKeyEvent(event);
    }
    if (details.dispatcher_destroyed)
        return details;
...
InputMethodAuraLinux::CommitResult InputMethodAuraLinux::MaybeCommitResult(bool filtered, const KeyEvent& event)
{
    ...
    if (filtered && NeedInsertChar(result_text)) {
        ...
        // keydown event once.
        client->InsertText(result_text, ui::TextInputClient::InsertTextCursorBehavior::kMoveCursorAfterText);
        // If the client changes we assume that the original target has been
        // destroyed.
        if (client != GetTextInputClient())
            return CommitResult::kTargetDestroyed;
...
bool InputMethodAuraLinux::UpdateCompositionIfTextSelected()
{
    ...
    if (!client->HasCompositionText() && composition_.text.empty() && selection_range_.IsValid() && !selection_range_.is_empty()) {
...
{
    TextInputClient* client = GetTextInputClient();
    bool update_composition = client && composition_changed_ && !IsTextInputTypeNone();
...
void InputMethodAuraLinux::UpdateContextFocusState()
{
    ...
    if (client) {
        ...
        reason = text_input_type_ == TEXT_INPUT_TYPE_NONE ? TextInputClient::FocusReason::FOCUS_REASON_NONE : TextInputClient::FocusReason::FOCUS_REASON_OTHER;
    }
...
void InputMethodAuraLinux::OnCaretBoundsChanged(const TextInputClient* client)
{
    ...
    std::u16string text;
    if (client->GetTextRange(&text_range) && client->GetTextFromRange(text_range, &text) && client->GetEditableSelectionRange(&selection_range)) {
        ...
        if (surrounding_text_ != text || text_range_ != text_range || selection_range_ != selection_range) {
            surrounding_text_ = text;
            text_range_ = text_range;
            selection_range_ = selection_range;
            context_->SetSurroundingText(text, text_range, composition_range, selection_range);
        }
    }
}
...
void InputMethodAuraLinux::ResetContext()
{
    ...
    if (!composition_.text.empty()) {
        ...
        suppress_non_key_input_until_ = base::TimeTicks::Now() + kIgnoreCommitsDuration;
    }
...
{
    return !is_sync_mode_ && base::TimeTicks::Now() < suppress_non_key_input_until_;
}
...
VirtualKeyboardController* InputMethodAuraLinux::GetVirtualKeyboardController()
{
...
void InputMethodAuraLinux::OnCommit(const std::u16string& text)
{
    ...
    if (!is_sync_mode_ && !IsTextInputTypeNone()) {
        ui::KeyEvent event = ui::KeyEvent(ui::EventType::kKeyPressed, ui::VKEY_PROCESSKEY, 0);
...
void InputMethodAuraLinux::OnDeleteSurroundingText(size_t before, size_t after)
{
    auto* client = GetTextInputClient();
...
void InputMethodAuraLinux::OnPreeditChanged(const CompositionText& composition_text)
{
...
{
    TextInputClient* client = GetTextInputClient();
    OnPreeditUpdate(CompositionText(), !is_sync_mode_ && client && client->HasCompositionText());
}

void InputMethodAuraLinux::OnSetPreeditRegion(const gfx::Range& range, const std::vector<ImeTextSpan>& spans)
{
...
void InputMethodAuraLinux::OnSetVirtualKeyboardOccludedBounds(const gfx::Rect& screen_bounds)
{
...
void InputMethodAuraLinux::OnWillChangeFocusedClient(TextInputClient* focused_before, TextInputClient* focused)
{
    ResetContext();
    context_->WillUpdateFocus(focused_before, focused);
}

void InputMethodAuraLinux::OnDidChangeFocusedClient(TextInputClient* focused_before, TextInputClient* focused)
{
    UpdateContextFocusState();
...
void InputMethodAuraLinux::OnPreeditUpdate(const ui::CompositionText& composition_text, bool force_update_client)
{
...
bool InputMethodAuraLinux::NeedInsertChar(const std::optional<std::u16string>& result_text) const
{
    return IsTextInputTypeNone() || (!composition_changed_ && composition_.text.empty() && result_text && result_text->length() == 1);
}

ui::EventDispatchDetails InputMethodAuraLinux::SendFakeProcessKeyEvent(ui::KeyEvent* event) const
{
    ui::KeyEvent key_event(ui::EventType::kKeyPressed, ui::VKEY_PROCESSKEY, event->code(), event->flags(), ui::DomKey::PROCESS, event->time_stamp());
