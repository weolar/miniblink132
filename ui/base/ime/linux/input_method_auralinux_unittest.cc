...
public:
    explicit LinuxInputMethodContextForTesting(LinuxInputMethodContextDelegate* delegate)
        : delegate_(delegate)
        , is_sync_mode_(false)
        , eat_key_(false)
    {
    ...
    LinuxInputMethodContextForTesting(const LinuxInputMethodContextForTesting&) = delete;
    LinuxInputMethodContextForTesting& operator=(const LinuxInputMethodContextForTesting&) = delete;

    void SetSyncMode(bool is_sync_mode)
    {
        is_sync_mode_ = is_sync_mode;
    }
    void SetEatKey(bool eat_key)
    {
        eat_key_ = eat_key;
    }
    ...
    {
        actions_.push_back(u"S");
    }

    void AddCompositionEndAction()
    {
        actions_.push_back(u"E");
    }
    ...
    TextInputType input_type() const
    {
        return attributes_.input_type;
    }
    TextInputMode input_mode() const
    {
        return attributes_.input_mode;
    }
    uint32_t input_flags() const
    {
        return attributes_.flags;
    }
    bool should_do_learning() const
    {
        return attributes_.should_do_learning;
    }
    bool can_compose_inline() const
    {
        return attributes_.can_compose_inline;
    }
    TextInputClient* old_client()
    {
        return old_client_;
    }
...
protected:
    bool DispatchKeyEvent(const ui::KeyEvent& key_event) override
    {
        ...
        for (const auto& action : actions_) {
            std::vector<std::u16string> parts = base::SplitString(action, std::u16string(1, ':'), base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    void WillUpdateFocus(TextInputClient* old_client, TextInputClient* new_client) override
    {
    ...
    void UpdateFocus(
        bool has_client, TextInputType old_type, const TextInputClientAttributes& new_client_attributes, ui::TextInputClient::FocusReason reason) override
    {
    ...
    void SetSurroundingText(
        const std::u16string& text, const gfx::Range& text_range, const gfx::Range& composition_range, const gfx::Range& selection_range) override
    {
        TestResult::GetInstance()->RecordAction(u"surroundingtext:" + text);
        TestResult::GetInstance()->RecordAction(base::ASCIIToUTF16(base::StringPrintf("textrangestart:%zu", text_range.start())));
        TestResult::GetInstance()->RecordAction(base::ASCIIToUTF16(base::StringPrintf("textrangeend:%zu", text_range.end())));
        TestResult::GetInstance()->RecordAction(base::ASCIIToUTF16(base::StringPrintf("compositionrangestart:%zu", composition_range.start())));
        TestResult::GetInstance()->RecordAction(base::ASCIIToUTF16(base::StringPrintf("compositionrangeend:%zu", composition_range.end())));
        TestResult::GetInstance()->RecordAction(base::ASCIIToUTF16(base::StringPrintf("selectionrangestart:%zu", selection_range.start())));
        TestResult::GetInstance()->RecordAction(base::ASCIIToUTF16(base::StringPrintf("selectionrangeend:%zu", selection_range.end())));
    }
...
public:
    ...
    InputMethodDelegateForTesting& operator=(const InputMethodDelegateForTesting&) = delete;

    ~InputMethodDelegateForTesting() override
    {
    }

    ui::EventDispatchDetails DispatchKeyEventPostIME(ui::KeyEvent* key_event) override
    {
        std::string action;
...
protected:
    ...
    void SetCompositionText(const CompositionText& composition) override
    {
        ...
        TestResult::GetInstance()->RecordAction(u"compositionupdate:" + composition.text);
    }
    ...
    {
        TestResult::GetInstance()->RecordAction(u"keypress:" + base::ASCIIToUTF16(base::NumberToString(static_cast<uint16_t>(event.GetCharacter()))));
    }
...
protected:
    ...
    {
        GetInputMethodContextFactoryForTest() = base::BindRepeating([](LinuxInputMethodContextDelegate* delegate) -> std::unique_ptr<LinuxInputMethodContext> {
    ...
    void SetUp() override
    {
        ...
        input_method_auralinux_ = std::make_unique<InputMethodAuraLinux>(delegate_.get());
        input_method_auralinux_->OnFocus();
        context_ = static_cast<LinuxInputMethodContextForTesting*>(input_method_auralinux_->GetContextForTesting());
    }
...
TEST_F(InputMethodAuraLinuxTest, BasicSyncModeTest)
{
    ...
    auto client1 = std::make_unique<TextInputClientForTesting>(TEXT_INPUT_TYPE_TEXT);
    ...
    auto client2 = std::make_unique<TextInputClientForTesting>(TEXT_INPUT_TYPE_PASSWORD);
    context_->SetEatKey(false);
...
TEST_F(InputMethodAuraLinuxTest, IBusUSTest)
{
    ...
    auto client = std::make_unique<TextInputClientForTesting>(TEXT_INPUT_TYPE_TEXT);
...
// crbug.com/463491
void DeadKeyTest(
    TextInputType text_input_type, InputMethodAuraLinux* input_method_auralinux, LinuxInputMethodContextForTesting* context, TestResult* test_result)
{
    ...
    {
        KeyEvent dead_key(EventType::kKeyPressed, VKEY_OEM_4, ui::DomCode::BRACKET_LEFT,
            /* flags= */ 0, DomKey::DeadKeyFromCombiningCharacter(kCombiningGraveAccent), base::TimeTicks());
    ...
    {
        KeyEvent dead_key(EventType::kKeyReleased, VKEY_OEM_4, ui::DomCode::BRACKET_LEFT,
            /* flags= */ 0, DomKey::DeadKeyFromCombiningCharacter(kCombiningGraveAccent), base::TimeTicks());
        input_method_auralinux->DispatchKeyEvent(&dead_key);
    }
...
{
    DeadKeyTest(TEXT_INPUT_TYPE_TEXT, input_method_auralinux_.get(), context_, test_result_);
}

TEST_F(InputMethodAuraLinuxTest, DeadKeyTestTypeNone)
{
    DeadKeyTest(TEXT_INPUT_TYPE_NONE, input_method_auralinux_.get(), context_, test_result_);
}
...
TEST_F(InputMethodAuraLinuxTest, SetPreeditRegionSingleCharTest)
{
    ...
    input_method_auralinux_->OnCaretBoundsChanged(client.get());
    input_method_auralinux_->OnSetPreeditRegion(client->text_range, std::vector<ImeTextSpan>());
...
TEST_F(InputMethodAuraLinuxTest, OnInsertImage)
{
    ...
    auto client = std::make_unique<TextInputClientForTesting>(TEXT_INPUT_TYPE_CONTENT_EDITABLE);
    InstallFirstClient(client.get());
...
{
    EXPECT_EQ(input_method_auralinux_->GetVirtualKeyboardController(), context_->GetVirtualKeyboardController());
}

TEST_F(InputMethodAuraLinuxTest, SetContentTypeWithUpdateFocus)
{
    auto client1 = std::make_unique<TextInputClientForTesting>(TEXT_INPUT_TYPE_TEXT);
    auto client2 = std::make_unique<TextInputClientForTesting>(TEXT_INPUT_TYPE_URL);
...
{
    auto client1 = std::make_unique<TextInputClientForTesting>(TEXT_INPUT_TYPE_TEXT);
    auto client2 = std::make_unique<TextInputClientForTesting>(TEXT_INPUT_TYPE_TEXT);
