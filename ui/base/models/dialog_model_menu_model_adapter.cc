...
DialogModelMenuModelAdapter::DialogModelMenuModelAdapter(std::unique_ptr<DialogModel> model)
...
MenuSeparatorType DialogModelMenuModelAdapter::GetSeparatorTypeAt(size_t index) const
{
    CHECK_EQ(GetField(index)->type(), DialogModelField::kSeparator, base::NotFatalUntil::M123);
...
bool DialogModelMenuModelAdapter::GetAcceleratorAt(size_t index, ui::Accelerator* accelerator) const
{
...
ButtonMenuItemModel* DialogModelMenuModelAdapter::GetButtonMenuItemAt(size_t index) const
{
    NOTREACHED();
}
...
bool DialogModelMenuModelAdapter::IsEnabledAt(size_t index) const
{
    ...
    return field->type() != DialogModelField::kSeparator && field->AsMenuItem()->is_enabled();
}

ui::ElementIdentifier DialogModelMenuModelAdapter::GetElementIdentifierAt(size_t index) const
{
    CHECK_LT(index, GetItemCount(), base::NotFatalUntil::M123);
...
const DialogModelField* DialogModelMenuModelAdapter::GetField(size_t index) const
{
    CHECK_LT(index, GetItemCount(), base::NotFatalUntil::M123);
...
{
    return const_cast<DialogModelField*>(const_cast<const DialogModelMenuModelAdapter*>(this)->GetField(index));
}
