...
// }
template <class NodeType> class TreeNodeIterator {
public:
        ...
        : prune_(prune)
    {
        ...
        const auto i = base::ranges::find_if(node->children(), [prune](const auto& child) { return prune.is_null() || !prune.Run(child.get()); });
    ...
    NodeType* Next()
    {
        ...
        NodeType* result = positions_.top().node->children()[positions_.top().index].get();
        ...
        while (!positions_.empty()) {
            ...
            else if (!prune_.is_null() && prune_.Run(top.node->children()[top.index].get()))
...
private:
    template <class PositionNodeType> struct Position {
        Position(PositionNodeType* node, size_t index)
            : node(node)
            , index(index)
        {
        }
