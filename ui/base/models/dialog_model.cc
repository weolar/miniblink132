...
DialogModel::Button::Params& DialogModel::Button::Params::SetId(ElementIdentifier id)
{
...
DialogModel::Button::Params& DialogModel::Button::Params::SetLabel(std::u16string label)
{
...
DialogModel::Button::Params& DialogModel::Button::Params::SetStyle(std::optional<ButtonStyle> style)
{
...
DialogModel::Button::Params& DialogModel::Button::Params::SetEnabled(bool is_enabled)
{
    is_enabled_ = is_enabled;
    return *this;
}

DialogModel::Button::Params& DialogModel::Button::Params::AddAccelerator(Accelerator accelerator)
{
    accelerators_.insert(std::move(accelerator));
    return *this;
}

DialogModel::Button::Button(base::RepeatingCallback<void(const Event&)> callback, const DialogModel::Button::Params& params)
...
void DialogModel::Button::OnPressed(base::PassKey<DialogModelHost>, const Event& event)
{
    callback_.Run(event);
}

DialogModel::Builder::Builder(std::unique_ptr<DialogModelDelegate> delegate)
    : model_(std::make_unique<DialogModel>(base::PassKey<Builder>(), std::move(delegate)))
{
}
...
DialogModel::Builder& DialogModel::Builder::AddOkButton(ButtonCallbackVariant callback, const DialogModel::Button::Params& params)
{
    return AddButtonInternal(std::move(callback), params, model_->ok_button_, model_->accept_action_callback_);
}

DialogModel::Builder& DialogModel::Builder::AddCancelButton(ButtonCallbackVariant callback, const DialogModel::Button::Params& params)
{
    return AddButtonInternal(std::move(callback), params, model_->cancel_button_, model_->cancel_action_callback_);
}

DialogModel::Builder& DialogModel::Builder::AddButtonInternal(ButtonCallbackVariant callback, const DialogModel::Button::Params& params,
    std::optional<ui::DialogModel::Button>& model_button, ButtonCallbackVariant& model_callback)
{
    ...
    model_button.emplace(base::BindRepeating([](const Event&) { NOTREACHED(); }), params);

    return *this;
}

DialogModel::Builder& DialogModel::Builder::AddExtraButton(base::RepeatingCallback<void(const Event&)> callback, const DialogModel::Button::Params& params)
{
    CHECK(params.is_visible_);
...
DialogModel::Builder& DialogModel::Builder::AddExtraLink(DialogModelLabel::TextReplacement link)
{
    CHECK(!model_->extra_button_, base::NotFatalUntil::M123);
    CHECK(!model_->extra_link_, base::NotFatalUntil::M123);
...
DialogModel::Builder& DialogModel::Builder::OverrideDefaultButton(mojom::DialogButton button)
{
...
DialogModel::Builder& DialogModel::Builder::SetInitiallyFocusedField(ElementIdentifier id)
{
...
DialogModel::DialogModel(base::PassKey<Builder>, std::unique_ptr<DialogModelDelegate> delegate)
...
bool DialogModel::HasField(ElementIdentifier id) const
{
    return base::ranges::any_of(contents_.fields(),
        ...
        || (ok_button_ && ok_button_->id_ == id) || (cancel_button_ && cancel_button_->id_ == id) || (extra_button_ && extra_button_->id_ == id);
}
...
DialogModel::Button* DialogModel::MaybeGetButtonByUniqueId(ElementIdentifier id)
{
...
bool DialogModel::RunButtonCallback(ButtonCallbackVariant& callback_variant)
{
    return absl::visit(base::Overloaded {
                           ...
                           [](base::RepeatingCallback<bool()>& callback) { return callback.Run(); },
...
void DialogModel::SetButtonLabel(DialogModel::Button* button, const std::u16string& label)
{
    CHECK(button);
    button->label_ = label;
