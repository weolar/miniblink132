...
// static
bool MenuModel::GetModelAndIndexForCommandId(int command_id, MenuModel** model, size_t* index)
{
    const size_t item_count = (*model)->GetItemCount();
    for (size_t i = 0; i < item_count; ++i) {
        const size_t candidate_index = i;
        // Actionable submenus have commands.
        if ((*model)->GetTypeAt(candidate_index) == TYPE_ACTIONABLE_SUBMENU && (*model)->GetCommandIdAt(candidate_index) == command_id) {
            *index = candidate_index;
            return true;
        }
        if ((*model)->GetTypeAt(candidate_index) == TYPE_SUBMENU || (*model)->GetTypeAt(candidate_index) == TYPE_ACTIONABLE_SUBMENU) {
...
{
    return (GetTypeAt(index) == ui::MenuModel::TYPE_TITLE) ? &ui::ResourceBundle::GetSharedInstance().GetFontList(ui::ResourceBundle::BoldFont) : nullptr;
}
...
std::optional<ui::ColorId> MenuModel::GetSubmenuBackgroundColorId(size_t index)
{
    return std::nullopt;
}

std::optional<ui::ColorId> MenuModel::GetSelectedBackgroundColorId(size_t index)
{
    return std::nullopt;
}
