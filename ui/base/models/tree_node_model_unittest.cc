...
protected:
    ...
    {
        std::string result(base::StringPrintf("added=%d removed=%d changed=%d", added_count_, removed_count_, changed_count_));
...
private:
    ...
    void TreeNodeAdded(TreeModel* model, TreeModelNode* parent, size_t index) override
    {
        added_count_++;
    }
    void TreeNodeRemoved(TreeModel* model, TreeModelNode* parent, size_t index) override
    {
        removed_count_++;
    }
...
TEST_F(TreeNodeModelTest, SortChildren)
{
    ...
    root.SortChildren([](const std::unique_ptr<TestNode>& lhs, const std::unique_ptr<TestNode>& rhs) { return lhs->value < rhs->value; });
