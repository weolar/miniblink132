...
VectorIconModel::VectorIconModel(const gfx::VectorIcon& vector_icon, ColorId color_id, int icon_size, const gfx::VectorIcon* badge_icon)
...
VectorIconModel::VectorIconModel(const gfx::VectorIcon& vector_icon, SkColor color, int icon_size, const gfx::VectorIcon* badge_icon)
    : vector_icon_(&vector_icon)
    , icon_size_(icon_size)
    , color_(color)
    , badge_icon_(badge_icon)
...
{
    return std::tie(vector_icon_, icon_size_, color_, badge_icon_) == std::tie(other.vector_icon_, other.icon_size_, other.color_, other.badge_icon_);
}
...
// static
ImageModel ImageModel::FromVectorIcon(const gfx::VectorIcon& vector_icon, ColorId color_id, int icon_size, const gfx::VectorIcon* badge_icon)
{
    if (!icon_size)
        icon_size = gfx::GetDefaultSizeOfVectorIcon(vector_icon);
    return ImageModel(VectorIconModel(vector_icon, color_id, icon_size, badge_icon));
}

// static
ImageModel ImageModel::FromVectorIcon(const gfx::VectorIcon& vector_icon, SkColor color, int icon_size, const gfx::VectorIcon* badge_icon)
{
    if (!icon_size)
        icon_size = gfx::GetDefaultSizeOfVectorIcon(vector_icon);
...
{
    return ImageModel::FromImage(ResourceBundle::GetSharedInstance().GetImageNamed(resource_id));
}

// static
ImageModel ImageModel::FromImageGenerator(ImageGenerator generator, gfx::Size size)
{
...
{
    return absl::holds_alternative<VectorIconModel>(icon_) && !absl::get<VectorIconModel>(icon_).is_empty();
}

bool ImageModel::IsImage() const
{
    return absl::holds_alternative<gfx::Image>(icon_) && !absl::get<gfx::Image>(icon_).IsEmpty();
}

bool ImageModel::IsImageGenerator() const
{
    return absl::holds_alternative<ImageGeneratorAndSize>(icon_) && !absl::get<ImageGeneratorAndSize>(icon_).size.IsEmpty();
}
...
gfx::Size ImageModel::Size() const
{
    ...
    return IsImageGenerator() ? absl::get<ImageGeneratorAndSize>(icon_).size : gfx::Size();
}
...
gfx::ImageSkia ImageModel::Rasterize(const ui::ColorProvider* color_provider) const
{
...
ImageModel::ImageGeneratorAndSize::ImageGeneratorAndSize(ImageGenerator generator, gfx::Size size)
    : generator(std::move(generator))
    , size(std::move(size))
{
}

ImageModel::ImageGeneratorAndSize::ImageGeneratorAndSize(const ImageGeneratorAndSize&) = default;

ImageModel::ImageGeneratorAndSize& ImageModel::ImageGeneratorAndSize::operator=(const ImageGeneratorAndSize&) = default;

ImageModel::ImageGeneratorAndSize::~ImageGeneratorAndSize() = default;

bool ImageModel::ImageGeneratorAndSize::operator==(const ImageGeneratorAndSize& other) const
{
