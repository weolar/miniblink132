...
{
    return (original_index >= insert_position) ? original_index + 1 : original_index;
}

// Determines the new index for `original_index` after inserting an item at
// `insert_position`. This is called for the active and anchor indexes.
std::optional<size_t> GetIndexAfterInsertion(size_t insert_position, std::optional<size_t> original_index)
{
    return original_index.has_value() ? std::optional<size_t>(GetIndexAfterInsertion(insert_position, original_index.value())) : std::nullopt;
}
...
std::optional<size_t> GetIndexAfterRemoval(size_t remove_position, size_t original_index)
{
...
// std::nullopt if `original_index` should be erased from its container.
std::optional<size_t> GetIndexAfterRemoval(size_t remove_position, std::optional<size_t> original_index)
{
    return original_index.has_value() ? std::optional<size_t>(GetIndexAfterRemoval(remove_position, original_index.value())) : std::nullopt;
}
...
std::optional<size_t> GetIndexAfterMove(size_t source_position, size_t destination_position, size_t range_size, std::optional<size_t> original_index)
{
    ...
    if (destination_position <= original_index_val && original_index_val < source_position + range_size) {
...
ListSelectionModel& ListSelectionModel::operator=(const ListSelectionModel&) = default;
ListSelectionModel& ListSelectionModel::operator=(ListSelectionModel&&) = default;

bool ListSelectionModel::operator==(const ListSelectionModel& other) const
{
    return std::tie(active_, anchor_, selected_indices_) == std::tie(other.active_, other.anchor_, other.selected_indices_);
}
...
void ListSelectionModel::AddIndexRangeToSelection(size_t index_start, size_t index_end)
{
...
void ListSelectionModel::SetSelectionFromAnchorTo(size_t index)
{
    if (!anchor_.has_value()) {
        ...
        for (size_t min = std::min(index, anchor_.value()), delta = std::max(index, anchor_.value()) - min, i = min; i <= min + delta; ++i) {
...
void ListSelectionModel::AddSelectionFromAnchorTo(size_t index)
{
    if (!anchor_.has_value()) {
        ...
        for (size_t i = std::min(index, anchor_.value()), end = std::max(index, anchor_.value()); i <= end; ++i) {
            selected_indices_.insert(i);
        }
        set_active(index);
    }
...
void ListSelectionModel::Move(size_t old_index, size_t new_index, size_t length)
{
    ...
    auto low = std::lower_bound(selected_indices_.begin(), selected_indices_.end(), new_index);
    auto high = std::lower_bound(low, selected_indices_.end(), old_index + length);
...
{
    const auto optional_to_string = [](const auto& opt) { return opt.has_value() ? base::NumberToString(opt.value()) : std::string("<none>"); };
    std::vector<std::string> index_strings;
    base::ranges::transform(selected_indices_, std::back_inserter(index_strings), [](const auto& index) { return base::NumberToString(index); });
    return "active=" + optional_to_string(active_) + " anchor=" + optional_to_string(anchor_) + " selection=" + base::JoinString(index_strings, " ");
}
