...
TEST_F(DialogModelButtonTest, UsesParamsUniqueId)
{
    ...
    auto host
        = std::make_unique<TestDialogModelHost>(DialogModel::Builder().AddOkButton(base::DoNothing(), DialogModel::Button::Params().SetId(kUniqueId)).Build());
...
TEST_F(DialogModelButtonTest, UsesParamsAccelerators)
{
    ...
    auto host = std::make_unique<TestDialogModelHost>(
        DialogModel::Builder()
            .AddOkButton(base::DoNothing(), DialogModel::Button::Params().AddAccelerator(accelerator_1).AddAccelerator(accelerator_2))
            .Build());
    EXPECT_THAT(host->GetAccelerators(TestDialogModelHost::ButtonId::kOk), testing::UnorderedElementsAre(accelerator_1, accelerator_2));
}
...
TEST_F(DialogModelButtonTest, UsesCallback)
{
    ...
    auto host = std::make_unique<TestDialogModelHost>(DialogModel::Builder()
...
public:
    void DialogButtonUsesArguments(TestDialogModelHost::ButtonId button_id)
    {
        ...
        base::OnceClosure callback = base::BindRepeating([](bool* callback_called) { *callback_called = true; }, &callback_called);
        ...
        case TestDialogModelHost::ButtonId::kExtra:
            ...
            builder.AddExtraButton(base::BindRepeating([](base::OnceClosure* callback, const Event& event) { std::move(*callback).Run(); }, &callback), params);
            break;
        ...
        EXPECT_EQ(label, host->GetLabel(button_id)) << "The label parameter wasn't used.";

        EXPECT_THAT(host->GetAccelerators(button_id), testing::UnorderedElementsAre(accelerator)) << "The params parameter wasn't used.";
        ...
        case TestDialogModelHost::ButtonId::kCancel:
            ...
            break;
        case TestDialogModelHost::ButtonId::kExtra:
            host->TriggerExtraButton(KeyEvent(EventType::kKeyPressed, VKEY_RETURN, EF_NONE));
            break;
        case TestDialogModelHost::ButtonId::kOk:
