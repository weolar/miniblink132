...
template <class NodeType> class TreeNode : public TreeModelNode {
public:
    ...
    NodeType* Add(std::unique_ptr<NodeType> node, size_t index)
    {
        ...
        children_.insert(children_.begin() + static_cast<ptrdiff_t>(index), std::move(node));
        return node_ptr;
    }
    ...
    const TreeNodes& children() const
    {
        return children_;
    }
    ...
    {
        DCHECK(node);
        const auto i = base::ranges::find(children_, node, &std::unique_ptr<NodeType>::get);
        return i != children_.end() ? std::make_optional(static_cast<size_t>(i - children_.begin())) : std::nullopt;
    }
    ...
    void SetPlaceholderAccessibleTitle(std::u16string placeholder_accessible_title)
    {
    ...
    void ReorderChildren(const std::vector<size_t>& new_order)
    {
        ...
        DCHECK_EQ(children_count, std::set(new_order.begin(), new_order.end()).size());
    ...
    template <typename Compare> void SortChildren(Compare comp)
    {
...
//  using MyModel = ui::TreeNodeModel<MyNode>;
template <class ValueType> class TreeNodeWithValue : public TreeNode<TreeNodeWithValue<ValueType>> {
public:
    TreeNodeWithValue()
    {
    }
...
template <class NodeType> class TreeNodeModel : public TreeModel {
public:
    ...
    NodeType* Add(NodeType* parent, std::unique_ptr<NodeType> node, size_t index)
    {
        DCHECK(parent);
        DCHECK(node);
    ...
    {
        observer_list_.Notify(&TreeModelObserver::TreeNodeAdded, this, parent, index);
    }

    void NotifyObserverTreeNodeRemoved(NodeType* parent, size_t index)
    {
        observer_list_.Notify(&TreeModelObserver::TreeNodeRemoved, this, parent, index);
    }
    ...
    Nodes GetChildren(const TreeModelNode* parent) const override
    {
        ...
        base::ranges::transform(children, std::back_inserter(nodes), &TreeNode<NodeType>::TreeNodes::value_type::get);
        return nodes;
    }

    std::optional<size_t> GetIndexOf(TreeModelNode* parent, TreeModelNode* child) const override
    {
        DCHECK(parent);
