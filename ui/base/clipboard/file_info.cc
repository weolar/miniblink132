...
{
    return path.length() > 2 && base::IsAsciiAlpha(path[0]) && path[1] == ':' && base::FilePath::IsSeparator(path[2]);
}
...
FileInfo::FileInfo(const base::FilePath& path, const base::FilePath& display_name)
    : path(path)
...
std::vector<FileInfo> URIListToFileInfos(std::string_view uri_list)
{
    ...
    std::vector<std::string_view> lines = base::SplitStringPiece(uri_list, kURIListSeparator, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
std::string FilePathToFileURL(const base::FilePath& file_path)
{
    ...
    if (path.size() > 1 && base::FilePath::IsSeparator(path[0]) && base::FilePath::IsSeparator(path[1])) {
