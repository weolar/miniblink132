...
public:
    ...
    void ReadHTML(std::u16string* markup, std::string* src_url, uint32_t* fragment_start, uint32_t* fragment_end) const
    {
        markup->clear();
        if (src_url)
            src_url->clear();
    ...
    void ReadPng(Clipboard::ReadPngCallback callback) const
    {
        ...
        DCHECK(maybe_bitmap.has_value()) << "We should not be showing that PNG format is on the "
                                            "clipboard if neither a PNG or bitmap is available.";

        // Creates a new entry if one doesn't exist.
        auto& callbacks_for_image = callbacks_awaiting_image_encoding_[sequence_number()];
        ...
        png_encoding_runner_->PostTaskAndReplyWithResult(FROM_HERE, base::BindOnce(&clipboard_util::EncodeBitmapToPng, std::move(maybe_bitmap.value())),
            base::BindOnce(&ClipboardInternal::DidEncodePng, weak_factory_.GetMutableWeakPtr(), sequence_number()));
    }

    // Reads data of type |type| from the ClipboardData.
    void ReadDataTransferCustomData(const std::u16string& type, std::u16string* result) const
    {
        result->clear();
        const ClipboardData* data = GetData();
        ...
        std::optional<std::u16string> maybe_result = ReadCustomDataForType(base::as_byte_span(data->GetDataTransferCustomData()), type);
        if (maybe_result) {
    ...
    std::unique_ptr<ClipboardData> WriteData(std::unique_ptr<ClipboardData> data)
    {
        DCHECK(data);
    ...
    bool IsReadAllowed(const DataTransferEndpoint* data_dst, std::optional<ClipboardInternalFormat> format,
        const std::optional<ClipboardFormatType>& custom_data_format = std::nullopt) const
    {
        DataTransferPolicyController* policy_controller = DataTransferPolicyController::Get();
        auto* data = GetData();
        if (!policy_controller || !data)
            return true;
        return policy_controller->IsClipboardReadAllowed(data->source(), data_dst, data->CalculateSize(format, custom_data_format));
    }
...
private:
    ...
    void DidEncodePng(ClipboardSequenceNumberToken token, std::vector<uint8_t> png_data)
    {
    ...
    // encoding.
    mutable std::map<ClipboardSequenceNumberToken, std::vector<Clipboard::ReadPngCallback>> callbacks_awaiting_image_encoding_;
    ...
    scoped_refptr<base::SequencedTaskRunner> png_encoding_runner_ = base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_VISIBLE });
...
public:
    ...
    static void CommitToClipboard(ClipboardInternal& clipboard, std::optional<DataTransferEndpoint> data_src)
    {
    ...
    static void WriteHTML(std::string_view markup, std::optional<std::string_view> source_url)
    {
        ClipboardData* data = GetCurrentData();
    ...
    static void WriteData(const ClipboardFormatType& format, base::span<const uint8_t> data)
    {
        ClipboardData* clipboard_data = GetCurrentData();
        clipboard_data->SetCustomData(format, std::string(reinterpret_cast<const char*>(data.data()), data.size()));
    }
...
const ClipboardData* ClipboardNonBacked::GetClipboardData(DataTransferEndpoint* data_dst, ClipboardBuffer buffer) const
{
    DCHECK(CalledOnValidThread());
...
std::unique_ptr<ClipboardData> ClipboardNonBacked::WriteClipboardData(std::unique_ptr<ClipboardData> data, ClipboardBuffer buffer)
{
    DCHECK(CalledOnValidThread());
...
std::optional<DataTransferEndpoint> ClipboardNonBacked::GetSource(ClipboardBuffer buffer) const
{
    const ClipboardData* data = GetInternalClipboard(buffer).GetData();
    return data ? data->source() : std::nullopt;
}

const ClipboardSequenceNumberToken& ClipboardNonBacked::GetSequenceNumber(ClipboardBuffer buffer) const
{
    DCHECK(CalledOnValidThread());
    return GetInternalClipboard(buffer).sequence_number();
}

int ClipboardNonBacked::NumImagesEncodedForTesting(ClipboardBuffer buffer) const
{
    DCHECK(CalledOnValidThread());
    const ClipboardInternal& clipboard_internal = GetInternalClipboard(buffer);
...
namespace {
bool IsReadAllowedAndAvailableFormat(const ClipboardInternal& clipboard_internal, const DataTransferEndpoint* data_dst, ClipboardInternalFormat format)
{
    return clipboard_internal.IsReadAllowed(data_dst, format) && clipboard_internal.IsFormatAvailable(format);
}
} // namespace

bool ClipboardNonBacked::IsFormatAvailable(const ClipboardFormatType& format, ClipboardBuffer buffer, const DataTransferEndpoint* data_dst) const
{
    DCHECK(CalledOnValidThread());
    ...
    if (format == ClipboardFormatType::PlainTextType() || format == ClipboardFormatType::UrlType()) {
        return IsReadAllowedAndAvailableFormat(clipboard_internal, data_dst, ClipboardInternalFormat::kText);
    }

    if (format == ClipboardFormatType::HtmlType()) {
        return IsReadAllowedAndAvailableFormat(clipboard_internal, data_dst, ClipboardInternalFormat::kHtml);
    }

    if (format == ClipboardFormatType::SvgType()) {
        return IsReadAllowedAndAvailableFormat(clipboard_internal, data_dst, ClipboardInternalFormat::kSvg);
    }

    if (format == ClipboardFormatType::RtfType()) {
        return IsReadAllowedAndAvailableFormat(clipboard_internal, data_dst, ClipboardInternalFormat::kRtf);
    }

    if (format == ClipboardFormatType::PngType() || format == ClipboardFormatType::BitmapType()) {
        return IsReadAllowedAndAvailableFormat(clipboard_internal, data_dst, ClipboardInternalFormat::kPng);
    }

    if (format == ClipboardFormatType::WebKitSmartPasteType()) {
        return IsReadAllowedAndAvailableFormat(clipboard_internal, data_dst, ClipboardInternalFormat::kWeb);
    }

    if (format == ClipboardFormatType::FilenamesType()) {
        return IsReadAllowedAndAvailableFormat(clipboard_internal, data_dst, ClipboardInternalFormat::kFilenames);
    }
...
std::vector<std::u16string> ClipboardNonBacked::GetStandardFormats(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst) const
{
...
void ClipboardNonBacked::ReadAvailableTypes(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::vector<std::u16string>* types) const
{
    DCHECK(CalledOnValidThread());
    DCHECK(types);
    ...
    *types = GetStandardFormats(buffer, data_dst);
    ...
    if (clipboard_internal.IsFormatAvailable(ClipboardInternalFormat::kCustom) && clipboard_internal.GetData()) {
        ReadCustomDataTypes(base::as_byte_span(clipboard_internal.GetData()->GetDataTransferCustomData()), types);
    }
}

void ClipboardNonBacked::ReadText(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* result) const
{
    DCHECK(CalledOnValidThread());

    const ClipboardInternal& clipboard_internal = GetInternalClipboard(buffer);

    if (!clipboard_internal.IsReadAllowed(data_dst, ClipboardInternalFormat::kText)) {
        return;
    }
...
void ClipboardNonBacked::ReadAsciiText(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::string* result) const
{
    DCHECK(CalledOnValidThread());
...
void ClipboardNonBacked::ReadHTML(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* markup, std::string* src_url,
    uint32_t* fragment_start, uint32_t* fragment_end) const
{
    ...
    if (!clipboard_internal.IsReadAllowed(data_dst, ClipboardInternalFormat::kHtml)) {
        return;
    }
...
void ClipboardNonBacked::ReadSvg(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* result) const
{
    DCHECK(CalledOnValidThread());

    const ClipboardInternal& clipboard_internal = GetInternalClipboard(buffer);

    if (!clipboard_internal.IsReadAllowed(data_dst, ClipboardInternalFormat::kSvg)) {
        return;
    }
...
void ClipboardNonBacked::ReadRTF(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::string* result) const
{
    DCHECK(CalledOnValidThread());

    const ClipboardInternal& clipboard_internal = GetInternalClipboard(buffer);

    if (!clipboard_internal.IsReadAllowed(data_dst, ClipboardInternalFormat::kRtf)) {
        return;
    }
...
void ClipboardNonBacked::ReadPng(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, ReadPngCallback callback) const
{
    DCHECK(CalledOnValidThread());

    const ClipboardInternal& clipboard_internal = GetInternalClipboard(buffer);

    if (!clipboard_internal.IsReadAllowed(data_dst, ClipboardInternalFormat::kPng)) {
        std::move(callback).Run(std::vector<uint8_t>());
        return;
    }
    ...
    ClipboardBuffer buffer, const std::u16string& type, const DataTransferEndpoint* data_dst, std::u16string* result) const
{
    ...
    if (!clipboard_internal.IsReadAllowed(data_dst, ClipboardInternalFormat::kCustom, ClipboardFormatType::DataTransferCustomType())) {
        return;
    }
...
void ClipboardNonBacked::ReadFilenames(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::vector<ui::FileInfo>* result) const
{
    DCHECK(CalledOnValidThread());

    const ClipboardInternal& clipboard_internal = GetInternalClipboard(buffer);

    if (!clipboard_internal.IsReadAllowed(data_dst, ClipboardInternalFormat::kFilenames)) {
        return;
    }
...
void ClipboardNonBacked::ReadBookmark(const DataTransferEndpoint* data_dst, std::u16string* title, std::string* url) const
{
    DCHECK(CalledOnValidThread());

    const ClipboardInternal& clipboard_internal = GetInternalClipboard(ClipboardBuffer::kCopyPaste);

    if (!clipboard_internal.IsReadAllowed(data_dst, ClipboardInternalFormat::kBookmark)) {
        return;
    }
...
void ClipboardNonBacked::ReadData(const ClipboardFormatType& format, const DataTransferEndpoint* data_dst, std::string* result) const
{
    DCHECK(CalledOnValidThread());

    const ClipboardInternal& clipboard_internal = GetInternalClipboard(ClipboardBuffer::kCopyPaste);

    if (!clipboard_internal.IsReadAllowed(data_dst, ClipboardInternalFormat::kCustom, format)) {
        return;
    }
...
void ClipboardNonBacked::WritePortableAndPlatformRepresentations(ClipboardBuffer buffer, const ObjectMap& objects,
    std::vector<Clipboard::PlatformRepresentation> platform_representations, std::unique_ptr<DataTransferEndpoint> data_src, uint32_t privacy_types)
{
    ...
    ClipboardDataBuilder::CommitToClipboard(clipboard_internal, base::OptionalFromPtr(data_src.get()));
}
...
void ClipboardNonBacked::WriteHTML(std::string_view markup, std::optional<std::string_view> source_url)
{
...
void ClipboardNonBacked::WriteBookmark(std::string_view title, std::string_view url)
{
...
void ClipboardNonBacked::WriteData(const ClipboardFormatType& format, base::span<const uint8_t> data)
{
...
const ClipboardInternal& ClipboardNonBacked::GetInternalClipboard(ClipboardBuffer buffer) const
{
    return *internal_clipboards_.at(buffer);
}

ClipboardInternal& ClipboardNonBacked::GetInternalClipboard(ClipboardBuffer buffer)
{
    return *internal_clipboards_.at(buffer);
}
