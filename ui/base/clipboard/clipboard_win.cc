...
bool ClipboardOwnerWndProc(UINT message, WPARAM wparam, LPARAM lparam, LRESULT* result)
{
    switch (message) {
...
template <typename charT> HGLOBAL CreateGlobalData(const std::basic_string<charT>& str)
{
...
void MakeBitmapOpaque(SkPixmap* pixmap)
{
    for (int x = 0; x < pixmap->width(); ++x) {
        for (int y = 0; y < pixmap->height(); ++y) {
            *pixmap->writable_addr32(x, y) = SkColorSetA(*pixmap->addr32(x, y), SK_AlphaOPAQUE);
        }
    }
}

template <typename StringType> void TrimAfterNull(StringType* result)
{
...
{
    return ::IsClipboardFormatAvailable(ClipboardFormatType::CFHDropType().ToFormatEtc().cfFormat)
        || ::IsClipboardFormatAvailable(ClipboardFormatType::FilenameType().ToFormatEtc().cfFormat)
        || ::IsClipboardFormatAvailable(ClipboardFormatType::FilenameAType().ToFormatEtc().cfFormat);
}
...
std::optional<DataTransferEndpoint> ClipboardWin::GetSource(ClipboardBuffer buffer) const
{
    ...
    HANDLE data = ::GetClipboardData(ClipboardFormatType::InternalSourceUrlType().ToFormatEtc().cfFormat);
    if (!data) {
        return std::nullopt;
    }

    std::string source_string;
    source_string.assign(static_cast<const char*>(::GlobalLock(data)), ::GlobalSize(data));
    ::GlobalUnlock(data);
...
const ClipboardSequenceNumberToken& ClipboardWin::GetSequenceNumber(ClipboardBuffer buffer) const
{
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
...
// platforms.
bool ClipboardWin::IsFormatAvailable(const ClipboardFormatType& format, ClipboardBuffer buffer, const DataTransferEndpoint* data_dst) const
{
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
    ...
    if (format == ClipboardFormatType::PngType() || format == ClipboardFormatType::BitmapType()) {
        return ::IsClipboardFormatAvailable(ClipboardFormatType::PngType().ToFormatEtc().cfFormat) != FALSE
            || ::IsClipboardFormatAvailable(ClipboardFormatType::BitmapType().ToFormatEtc().cfFormat) != FALSE;
    }
...
std::vector<std::u16string> ClipboardWin::GetStandardFormats(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst) const
{
    std::vector<std::u16string> types;
    if (::IsClipboardFormatAvailable(ClipboardFormatType::PlainTextAType().ToFormatEtc().cfFormat))
        types.push_back(base::UTF8ToUTF16(kMimeTypeText));
    if (::IsClipboardFormatAvailable(ClipboardFormatType::HtmlType().ToFormatEtc().cfFormat))
        types.push_back(base::UTF8ToUTF16(kMimeTypeHTML));
    if (::IsClipboardFormatAvailable(ClipboardFormatType::SvgType().ToFormatEtc().cfFormat))
        types.push_back(base::UTF8ToUTF16(kMimeTypeSvg));
    if (::IsClipboardFormatAvailable(ClipboardFormatType::RtfType().ToFormatEtc().cfFormat))
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardWin::ReadAvailableTypes(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::vector<std::u16string>* types) const
{
    DCHECK(types);
    ...
    *types = GetStandardFormats(buffer, data_dst);
    ...
    HANDLE hdata = ::GetClipboardData(ClipboardFormatType::DataTransferCustomType().ToFormatEtc().cfFormat);
    if (!hdata)
        return;

    base::win::ScopedHGlobal<const uint8_t*> locked_data(hdata);
    ReadCustomDataTypes(base::span<const uint8_t>(locked_data.data(), locked_data.size()), types);
}

// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardWin::ReadText(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* result) const
{
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
    ...
    result->assign(static_cast<const char16_t*>(::GlobalLock(data)), ::GlobalSize(data) / sizeof(char16_t));
    ::GlobalUnlock(data);
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardWin::ReadAsciiText(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::string* result) const
{
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
    RecordRead(ClipboardFormatMetric::kText);
    CHECK(result);
    ...
    result->assign(static_cast<const char*>(::GlobalLock(data)), ::GlobalSize(data));
    ::GlobalUnlock(data);
    TrimAfterNull(result);
}
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardWin::ReadHTML(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* markup, std::string* src_url,
    uint32_t* fragment_start, uint32_t* fragment_end) const
{
    ...
    *fragment_end = 0;
    ...
    HANDLE data = ::GetClipboardData(ClipboardFormatType::HtmlType().ToFormatEtc().cfFormat);
    if (!data)
        return;

    std::string cf_html(static_cast<const char*>(::GlobalLock(data)), ::GlobalSize(data));
    ::GlobalUnlock(data);
    ...
    clipboard_util::CFHtmlExtractMetadata(cf_html, src_url, &html_start, &start_index, &end_index);
    ...
    std::vector<size_t> offsets = { start_index - html_start, end_index - html_start };
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardWin::ReadSvg(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* result) const
{
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
    ...
    if (true/*base::FeatureList::IsEnabled(features::kUseUtf8EncodingForSvgImage)*/) {
        *result = base::UTF8ToUTF16(data);
    } else {
        result->assign(reinterpret_cast<const char16_t*>(data.data()), data.size() / sizeof(char16_t));
    }
    TrimAfterNull(result);
}
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardWin::ReadRTF(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::string* result) const
{
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardWin::ReadPng(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, ReadPngCallback callback) const
{
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(
        FROM_HERE, { base::MayBlock(), base::TaskPriority::USER_BLOCKING }, base::BindOnce(&clipboard_util::EncodeBitmapToPng, bitmap), std::move(callback));
}
    ...
    ClipboardBuffer buffer, const std::u16string& type, const DataTransferEndpoint* data_dst, std::u16string* result) const
{
    ...
    base::win::ScopedHGlobal<const uint8_t*> locked_data(hdata);
    if (std::optional<std::u16string> maybe_result = ReadCustomDataForType(base::span<const uint8_t>(locked_data.data(), locked_data.size()), type); maybe_result) {
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardWin::ReadFilenames(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::vector<ui::FileInfo>* result) const
{
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
    DCHECK(result);
    ...
    HANDLE data = ::GetClipboardData(ClipboardFormatType::CFHDropType().ToFormatEtc().cfFormat);
    if (data) {
        {
    ...
    data = ::GetClipboardData(ClipboardFormatType::FilenameType().ToFormatEtc().cfFormat);
    if (data) {
        {
    ...
    data = ::GetClipboardData(ClipboardFormatType::FilenameAType().ToFormatEtc().cfFormat);
    if (data) {
        {
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardWin::ReadBookmark(const DataTransferEndpoint* data_dst, std::u16string* title, std::string* url) const
{
    ...
    HANDLE data = ::GetClipboardData(ClipboardFormatType::UrlType().ToFormatEtc().cfFormat);
    if (!data)
        return;

    std::u16string bookmark(static_cast<const char16_t*>(::GlobalLock(data)), ::GlobalSize(data) / sizeof(char16_t));
    ::GlobalUnlock(data);
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardWin::ReadData(const ClipboardFormatType& format, const DataTransferEndpoint* data_dst, std::string* result) const
{
...
void ClipboardWin::WritePortableAndPlatformRepresentations(ClipboardBuffer buffer, const ObjectMap& objects,
...
void ClipboardWin::WriteHTML(std::string_view markup, std::optional<std::string_view> source_url)
{
    // Add Windows specific headers to the HTML payload before writing to the
    // clipboard.
    std::string html_fragment = clipboard_util::HtmlToCFHtml(markup, source_url.value_or(""));
...
{
    //WriteData(ClipboardFormatType::RtfType(), base::as_bytes(base::make_span(rtf)));
    WriteData(ClipboardFormatType::RtfType(), base::span<const uint8_t>((const uint8_t*)rtf.data(), rtf.size()));
}
...
void ClipboardWin::WriteWebSmartPaste()
{
    ...
    ::SetClipboardData(ClipboardFormatType::WebKitSmartPasteType().ToFormatEtc().cfFormat, nullptr);
}
...
void ClipboardWin::WriteBitmap(const SkBitmap& bitmap)
{
    ...
    // background sequence.
    std::vector<uint8_t> png_encoded_bitmap = clipboard_util::EncodeBitmapToPngAcceptJank(bitmap);
...
void ClipboardWin::WriteData(const ClipboardFormatType& format, base::span<const uint8_t> data)
{
...
void ClipboardWin::WriteClipboardHistory()
{
    ...
    // is not available.
    DWORD value = 0;
    WriteData(ClipboardFormatType::ClipboardHistoryType(), base::make_span(reinterpret_cast<const uint8_t*>(&value), sizeof(value)));
}
...
void ClipboardWin::WriteUploadCloudClipboard()
{
    ...
    // is not available.
    DWORD value = 0;
    WriteData(ClipboardFormatType::UploadCloudClipboardType(), base::make_span(reinterpret_cast<const uint8_t*>(&value), sizeof(value)));
}
...
std::vector<uint8_t> ClipboardWin::ReadPngInternal(ClipboardBuffer buffer) const
{
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
    ...
    HANDLE data = ::GetClipboardData(ClipboardFormatType::PngType().ToFormatEtc().cfFormat);

    if (!data)
        return std::vector<uint8_t>();

    std::string result(static_cast<const char*>(::GlobalLock(data)), ::GlobalSize(data));
    ::GlobalUnlock(data);
...
SkBitmap ClipboardWin::ReadBitmapInternal(ClipboardBuffer buffer) const
{
    ...
    if (!base::CheckMul(bitmap->bmiHeader.biWidth, base::CheckMul(bitmap->bmiHeader.biHeight, bytes_per_pixel)).AssignIfValid(&image_size_bytes))
        return SkBitmap();
    ...
    case 4:
    case 8:
        color_table_length = bitmap->bmiHeader.biClrUsed ? bitmap->bmiHeader.biClrUsed : 1 << bitmap->bmiHeader.biBitCount;
        break;
    case 16:
    case 32:
    ...
    const void* bitmap_bits = reinterpret_cast<const char*>(bitmap) + bitmap->bmiHeader.biSize + color_table_length * sizeof(RGBQUAD);

    void* dst_bits;
    // dst_hbitmap is freed by the release_proc in skia_bitmap (below)
    base::win::ScopedBitmap dst_hbitmap = skia::CreateHBitmapXRGB8888(bitmap->bmiHeader.biWidth, bitmap->bmiHeader.biHeight, 0, &dst_bits);

    {
        base::win::ScopedCreateDC hdc(CreateCompatibleDC(nullptr));
        HBITMAP old_hbitmap = static_cast<HBITMAP>(SelectObject(hdc.Get(), dst_hbitmap.get()));
        ::SetDIBitsToDevice(
            hdc.Get(), 0, 0, bitmap->bmiHeader.biWidth, bitmap->bmiHeader.biHeight, 0, 0, 0, bitmap->bmiHeader.biHeight, bitmap_bits, bitmap, DIB_RGB_COLORS);
    ...
    SkPixmap device_pixels(SkImageInfo::MakeN32Premul(bitmap->bmiHeader.biWidth, bitmap->bmiHeader.biHeight), dst_bits, bitmap->bmiHeader.biWidth * 4);
    ...
    skia_bitmap.installPixels(
        device_pixels.info(), device_pixels.writable_addr(), device_pixels.rowBytes(),
        [](void* pixels, void* hbitmap) { DeleteObject(static_cast<HBITMAP>(hbitmap)); }, dst_hbitmap.release());
    return skia_bitmap;
}
...
void ClipboardWin::WriteToClipboard(ClipboardFormatType format, HANDLE handle)
{
    ...
    if (handle && !::SetClipboardData(cf_format, handle)) {
        DCHECK_NE(GetLastError(), static_cast<unsigned long>(ERROR_CLIPBOARD_NOT_OPEN));
        ::GlobalFree(handle);
    }
}
