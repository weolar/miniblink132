...
// goes away.
class COMPONENT_EXPORT(UI_BASE_CLIPBOARD) ClipboardNonBacked : public Clipboard {
public:
    ...
    const ClipboardData* GetClipboardData(DataTransferEndpoint* data_dst, ClipboardBuffer buffer = ClipboardBuffer::kCopyPaste) const;

    // Writes the current ClipboardData and returns the previous data.
    // The data source is expected to be set in `data`.
    std::unique_ptr<ClipboardData> WriteClipboardData(std::unique_ptr<ClipboardData> data, ClipboardBuffer buffer = ClipboardBuffer::kCopyPaste);
    ...
    int NumImagesEncodedForTesting(ClipboardBuffer buffer = ClipboardBuffer::kCopyPaste) const;
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(ClipboardNonBackedMockTimeTest, RecordsTimeIntervalBetweenCommitAndRead);
    ClipboardNonBacked();
    ...
    void ReadHTML(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* markup, std::string* src_url, uint32_t* fragment_start,
    ...
    void ReadDataTransferCustomData(
        ClipboardBuffer buffer, const std::u16string& type, const DataTransferEndpoint* data_dst, std::u16string* result) const override;
    ...
    void WritePortableAndPlatformRepresentations(ClipboardBuffer buffer, const ObjectMap& objects,
        std::vector<Clipboard::PlatformRepresentation> platform_representations, std::unique_ptr<DataTransferEndpoint> data_src,
    ...
    base::flat_map<ClipboardBuffer, std::unique_ptr<ClipboardInternal>> internal_clipboards_;
};
