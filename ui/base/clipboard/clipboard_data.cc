...
bool ClipboardData::operator==(const ClipboardData& that) const
{
    ...
    bool equal_except_images = format_ == that.format() && text_ == that.text() && markup_data_ == that.markup_data() && url_ == that.url()
        && rtf_data_ == that.rtf_data() && bookmark_title_ == that.bookmark_title() && bookmark_url_ == that.bookmark_url() && custom_data_ == that.custom_data_
        && web_smart_paste_ == that.web_smart_paste() && svg_data_ == that.svg_data() && filenames_ == that.filenames()
        && (src_ ? (that.source().has_value() && *src_ == *that.source()) : !that.source().has_value());
    ...
    return !maybe_bitmap_.has_value() || gfx::BitmapsAreEqual(maybe_bitmap_.value(), that.maybe_bitmap_.value());
}
...
std::optional<size_t> ClipboardData::CalculateSize(
    const std::optional<ClipboardInternalFormat>& format, const std::optional<ClipboardFormatType>& custom_data_format) const
{
    size_t total_size = 0;
    ...
    if (format.has_value() || (format_ & static_cast<int>(ClipboardInternalFormat::kFilenames))) {
        return std::nullopt;
    }
...
bool ClipboardData::HasCustomDataFormat(const ClipboardFormatType& format) const
{
    return custom_data_.find(format) != custom_data_.end();
}

std::string ClipboardData::GetCustomData(const ClipboardFormatType& format) const
{
...
void ClipboardData::SetCustomData(const ClipboardFormatType& format, const std::string& data)
{
