...
// 'file:///path'.
std::string /*COMPONENT_EXPORT(UI_BASE_FILE_INFO)*/ FilePathToFileURL(const base::FilePath& file_path);
...
std::vector<FileInfo> /*COMPONENT_EXPORT(UI_BASE_FILE_INFO)*/ URIListToFileInfos(std::string_view uri_list);

// Returns UTF8 text/uri-list CRLF-separated file:// URLs from filenames.
std::string /*COMPONENT_EXPORT(UI_BASE_FILE_INFO)*/ FileInfosToURIList(const std::vector<FileInfo>& filenames);
