...
std::vector<uint8_t> GetPngData(const base::android::ScopedJavaGlobalRef<jobject>& clipboard_manager)
{
    JNIEnv* env = AttachCurrentThread();
    ScopedJavaLocalRef<jbyteArray> jimage_data = Java_Clipboard_getPng(env, clipboard_manager);
...
// entry.
void JNI_Clipboard_AddMapEntry(
    JNIEnv* env, std::map<ClipboardFormatType, std::string>* map, const ClipboardFormatType& format, const ScopedJavaLocalRef<jstring>& jstr)
{
    if (jstr.is_null()) {
        map->erase(format);
        return;
...
{
    JNIEnv* env = AttachCurrentThread();
    Java_Clipboard_setNativePtr(env, clipboard_manager_, reinterpret_cast<intptr_t>(clipboard));
}
...
void ClipboardMap::GetPng(ReadPngCallback callback)
{
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock(), base::TaskPriority::USER_BLOCKING },
        base::BindOnce(&GetPngData, clipboard_manager_), base::BindOnce(&ClipboardMap::DidGetPng, base::Unretained(this), std::move(callback)));
}

void ClipboardMap::DidGetPng(ReadPngCallback callback, std::vector<uint8_t> result)
{
...
bool ClipboardMap::HasFormat(const ClipboardFormatType& format)
{
    ...
    if (map_state_ == MapState::kUpToDate) {
        ...
        if (format == ClipboardFormatType::PngType() || format == ClipboardFormatType::BitmapType()) {
            return base::Contains(map_, ClipboardFormatType::PngType()) || base::Contains(map_, ClipboardFormatType::BitmapType());
        }
    ...
    } else if (format == ClipboardFormatType::PngType() || format == ClipboardFormatType::BitmapType()) {
...
{
    base::Time timestamp = base::Time::FromMillisecondsSinceUnixEpoch(timestamp_ms);
...
void ClipboardMap::Set(const ClipboardFormatType& format, std::string_view data)
{
    base::AutoLock lock(lock_);
    map_[format] = data;
...
void ClipboardMap::CommitToAndroidClipboard()
{
    JNIEnv* env = AttachCurrentThread();
    base::AutoLock lock(lock_);
    if (mark_password_data_ && base::Contains(map_, ClipboardFormatType::PlainTextType())) {
        ScopedJavaLocalRef<jstring> str = ConvertUTF8ToJavaString(env, map_[ClipboardFormatType::PlainTextType()]);
        DCHECK(str.obj());
        ...
        ScopedJavaLocalRef<jstring> html = ConvertUTF8ToJavaString(env, map_[ClipboardFormatType::HtmlType()]);
        ScopedJavaLocalRef<jstring> text = ConvertUTF8ToJavaString(env, map_[ClipboardFormatType::PlainTextType()]);
        ...
        ScopedJavaLocalRef<jbyteArray> image_data = ToJavaByteArray(env, map_[ClipboardFormatType::PngType()]);
        ScopedJavaLocalRef<jstring> image_extension = ConvertUTF8ToJavaString(env, kPngExtension);
        DCHECK(image_data.obj());
        // TODO(crbug.com/40187527) In unit tests, `jimageuri` is empty.
        Java_Clipboard_setImage(env, clipboard_manager_, image_data, image_extension);
        ScopedJavaLocalRef<jstring> jimageuri = Java_Clipboard_getImageUriString(env, clipboard_manager_);
        JNI_Clipboard_AddMapEntry(env, &map_, ClipboardFormatType::BitmapType(), jimageuri);
        ...
        ScopedJavaLocalRef<jobjectArray> arr = base::android::ToJavaArrayOfStrings(env, paths);
        DCHECK(arr.obj());
...
void ClipboardMap::UpdateFromAndroidClipboard()
{
    ...
    ScopedJavaLocalRef<jstring> jtext = Java_Clipboard_getCoercedText(env, clipboard_manager_);
    ScopedJavaLocalRef<jstring> jhtml = Java_Clipboard_getHTMLText(env, clipboard_manager_);
    ScopedJavaLocalRef<jstring> jurl = Java_Clipboard_getUrl(env, clipboard_manager_);
    ScopedJavaLocalRef<jstring> jimageuri = Java_Clipboard_getImageUriString(env, clipboard_manager_);
    if (base::FeatureList::IsEnabled(features::kClipboardFiles)) {
        filenames_.clear();
        std::vector<std::vector<std::string>> filenames;
        base::android::Java2dStringArrayTo2dStringVector(env, Java_Clipboard_getFilenames(env, clipboard_manager_), &filenames);
    ...
    JNI_Clipboard_AddMapEntry(env, &map_, ClipboardFormatType::PlainTextType(), jtext);
...
void ClipboardAndroid::OnPrimaryClipChanged(JNIEnv* env, const base::android::JavaParamRef<jobject>& obj)
{
    g_map.Get().OnPrimaryClipboardChanged();
}

void ClipboardAndroid::OnPrimaryClipTimestampInvalidated(JNIEnv* env, const base::android::JavaParamRef<jobject>& obj, const jlong j_timestamp_ms)
{
...
void ClipboardAndroid::SetLastModifiedTimeWithoutRunningCallback(base::Time time)
{
...
std::optional<DataTransferEndpoint> ClipboardAndroid::GetSource(ClipboardBuffer buffer) const
{
    DCHECK(CalledOnValidThread());
...
const ClipboardSequenceNumberToken& ClipboardAndroid::GetSequenceNumber(ClipboardBuffer /* buffer */) const
{
    DCHECK(CalledOnValidThread());
...
// platforms.
bool ClipboardAndroid::IsFormatAvailable(const ClipboardFormatType& format, ClipboardBuffer buffer, const DataTransferEndpoint* data_dst) const
{
    DCHECK(CalledOnValidThread());
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
...
std::vector<std::u16string> ClipboardAndroid::GetStandardFormats(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst) const
{
    ...
    if (IsFormatAvailable(ClipboardFormatType::BitmapType(), buffer, data_dst) || IsFormatAvailable(ClipboardFormatType::PngType(), buffer, data_dst)) {
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardAndroid::ReadAvailableTypes(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::vector<std::u16string>* types) const
{
    DCHECK(CalledOnValidThread());
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
    DCHECK(types);
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardAndroid::ReadText(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* result) const
{
    DCHECK(CalledOnValidThread());
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
    std::string utf8;
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardAndroid::ReadAsciiText(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::string* result) const
{
    DCHECK(CalledOnValidThread());
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardAndroid::ReadHTML(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* markup, std::string* src_url,
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardAndroid::ReadSvg(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* result) const
{
    DCHECK(CalledOnValidThread());
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardAndroid::ReadRTF(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::string* result) const
{
    DCHECK(CalledOnValidThread());
    NOTIMPLEMENTED();
}
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardAndroid::ReadPng(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, ReadPngCallback callback) const
{
    DCHECK(CalledOnValidThread());
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardAndroid::ReadFilenames(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::vector<ui::FileInfo>* result) const
{
    DCHECK(CalledOnValidThread());
    DCHECK_EQ(buffer, ClipboardBuffer::kCopyPaste);
...
// other platforms.
void ClipboardAndroid::ReadBookmark(const DataTransferEndpoint* data_dst, std::u16string* title, std::string* url) const
{
    DCHECK(CalledOnValidThread());
...
// |data_dst| is not used. It's only passed to be consistent with other
// platforms.
void ClipboardAndroid::ReadData(const ClipboardFormatType& format, const DataTransferEndpoint* data_dst, std::string* result) const
{
    DCHECK(CalledOnValidThread());
...
// platforms.
void ClipboardAndroid::WritePortableAndPlatformRepresentations(ClipboardBuffer buffer, const ObjectMap& objects,
...
void ClipboardAndroid::WriteHTML(std::string_view markup, std::optional<std::string_view> /* source_url */)
{
...
// URL spec.
void ClipboardAndroid::WriteBookmark(std::string_view title, std::string_view url)
{
...
void ClipboardAndroid::WriteBitmap(const SkBitmap& sk_bitmap)
{
    ...
    // background sequence.
    scoped_refptr<base::RefCountedMemory> image_memory = gfx::Image::CreateFrom1xBitmap(sk_bitmap).As1xPNGBytes();
    g_map.Get().Set(ClipboardFormatType::PngType(), std::string(base::as_string_view(*image_memory)));
}

void ClipboardAndroid::WriteData(const ClipboardFormatType& format, base::span<const uint8_t> data)
{
    g_map.Get().Set(format, std::string(reinterpret_cast<const char*>(data.data()), data.size()));
}
