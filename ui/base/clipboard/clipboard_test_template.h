...
template <typename ClipboardTraits> class ClipboardTest : public PlatformTest {
public:
#if BUILDFLAG(IS_ANDROID)
    ClipboardTest()
    {
...
public:
    ...
    MOCK_METHOD3(IsClipboardReadAllowed,
        bool(base::optional_ref<const DataTransferEndpoint> data_src, base::optional_ref<const DataTransferEndpoint> data_dst,
            const std::optional<size_t> size));
    MOCK_METHOD5(PasteIfAllowed,
        void(base::optional_ref<const DataTransferEndpoint> data_src, base::optional_ref<const DataTransferEndpoint> data_dst,
            absl::variant<size_t, std::vector<base::FilePath>> pasted_content, content::RenderFrameHost* rfh, base::OnceCallback<void(bool)> callback));
    MOCK_METHOD4(DropIfAllowed,
        void(std::optional<ui::DataTransferEndpoint> data_src, std::optional<ui::DataTransferEndpoint> data_dst,
            std::optional<std::vector<ui::FileInfo>> filenames, base::OnceClosure drop_cb));
};
...
TYPED_TEST(ClipboardTest, ClearTest)
{
    ...
    EXPECT_FALSE(this->clipboard().IsFormatAvailable(ClipboardFormatType::PlainTextType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
#if BUILDFLAG(IS_WIN)
    EXPECT_FALSE(this->clipboard().IsFormatAvailable(ClipboardFormatType::PlainTextAType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
#endif
}
...
TYPED_TEST(ClipboardTest, TextTest)
{
    ...
    EXPECT_THAT(this->GetAvailableTypes(ClipboardBuffer::kCopyPaste), Contains(ASCIIToUTF16(kMimeTypeText)));
    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::PlainTextType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
#if BUILDFLAG(IS_WIN)
    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::PlainTextAType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
#endif
...
TYPED_TEST(ClipboardTest, HTMLTest)
{
    ...
    EXPECT_THAT(this->GetAvailableTypes(ClipboardBuffer::kCopyPaste), Contains(ASCIIToUTF16(kMimeTypeHTML)));
    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::HtmlType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
    uint32_t fragment_start;
    uint32_t fragment_end;
    this->clipboard().ReadHTML(ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr, &markup_result, &url_result, &fragment_start, &fragment_end);
    EXPECT_LE(markup.size(), fragment_end - fragment_start);
    EXPECT_EQ(markup, markup_result.substr(fragment_end - markup.size(), markup.size()));
#if BUILDFLAG(IS_WIN)
...
TYPED_TEST(ClipboardTest, SvgTest)
{
    ...
    EXPECT_THAT(this->GetAvailableTypes(ClipboardBuffer::kCopyPaste), Contains(ASCIIToUTF16(kMimeTypeSvg)));
    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::SvgType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
...
TYPED_TEST(ClipboardTest, RTFTest)
{
    ...
    EXPECT_THAT(this->GetAvailableTypes(ClipboardBuffer::kCopyPaste), Contains(ASCIIToUTF16(kMimeTypeRTF)));
    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::RtfType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
    std::string result;
...
TYPED_TEST(ClipboardTest, MultipleBufferTest)
{
    ...
    EXPECT_THAT(this->GetAvailableTypes(ClipboardBuffer::kCopyPaste), Contains(ASCIIToUTF16(kMimeTypeText)));
    EXPECT_THAT(this->GetAvailableTypes(ClipboardBuffer::kSelection), Contains(ASCIIToUTF16(kMimeTypeHTML)));

    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::PlainTextType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
    EXPECT_FALSE(this->clipboard().IsFormatAvailable(ClipboardFormatType::PlainTextType(), ClipboardBuffer::kSelection,
        /* data_dst = */ nullptr));

    EXPECT_FALSE(this->clipboard().IsFormatAvailable(ClipboardFormatType::HtmlType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::HtmlType(), ClipboardBuffer::kSelection,
        /* data_dst = */ nullptr));
...
TYPED_TEST(ClipboardTest, BookmarkTest)
{
    ...
    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::UrlType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
    this->clipboard().ReadBookmark(/* data_dst = */ nullptr, &title_result, &url_result);
#if !BUILDFLAG(IS_WIN)
...
TYPED_TEST(ClipboardTest, FilenamesTest)
{
    ...
    {
        ScopedClipboardWriter clipboard_writer(ClipboardBuffer::kCopyPaste);
        clipboard_writer.WriteFilenames(ui::FileInfosToURIList({ ui::FileInfo(file, base::FilePath()) }));
    }

    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::FilenamesType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
...
#if BUILDFLAG(IS_POSIX) && !BUILDFLAG(IS_APPLE) && !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_CHROMEOS) && !BUILDFLAG(IS_OZONE)
    ascii_text.clear();
...
TYPED_TEST(ClipboardTest, ChromiumCustomFormatTest)
{
    ...
    EXPECT_THAT(this->GetAvailableTypes(ClipboardBuffer::kCopyPaste), Contains(ASCIIToUTF16(kMimeTypeHTML)));
    EXPECT_THAT(this->GetAvailableTypes(ClipboardBuffer::kCopyPaste), testing::Not(Contains(ASCIIToUTF16(kMimeTypeDataTransferCustomData))));
    {
        ScopedClipboardWriter clipboard_writer(ClipboardBuffer::kCopyPaste);
        ...
        clipboard_writer.WritePickledData(pickle, ClipboardFormatType::DataTransferCustomType());
    }
    EXPECT_THAT(this->GetAvailableTypes(ClipboardBuffer::kCopyPaste), testing::Not(Contains(ASCIIToUTF16(kMimeTypeHTML))));
    EXPECT_THAT(this->GetAvailableTypes(ClipboardBuffer::kCopyPaste), Contains(ASCIIToUTF16(kMimeTypeDataTransferCustomData)));
}
#endif // BUILDFLAG(IS_WIN)
...
void WriteBitmap(Clipboard* clipboard, const SkImageInfo& info, const void* bitmap_data)
{
    {
        ScopedClipboardWriter clipboard_writer(ClipboardBuffer::kCopyPaste, std::make_unique<DataTransferEndpoint>(GURL("https://google.com/")));
        SkBitmap bitmap;
...
void AssertBitmapMatchesExpected(const SkBitmap& image, const SkImageInfo& info, const U8x4* expect_data)
{
    ASSERT_EQ(image.info().colorType(), kN32_SkColorType);
    ASSERT_NE(image.info().alphaType(), kUnpremul_SkAlphaType);
    EXPECT_EQ(gfx::Size(info.width(), info.height()), gfx::Size(image.width(), image.height()));
    for (int y = 0; y < image.height(); ++y) {
        const U8x4* actual_row = reinterpret_cast<const U8x4*>(image.getAddr32(0, y));
...
template <typename T> static void TestBitmapWriteAndPngRead(Clipboard* clipboard, const SkImageInfo& info, const T* bitmap_data, const U8x4* expect_data)
{
    ...
    EXPECT_TRUE(clipboard->IsFormatAvailable(ClipboardFormatType::BitmapType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
    EXPECT_TRUE(clipboard->IsFormatAvailable(ClipboardFormatType::PngType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
...
TYPED_TEST(ClipboardTest, BitmapWriteAndPngRead_F16_Premul)
{
    ...
    EXPECT_DEATH_IF_SUPPORTED(
        TestBitmapWriteAndPngRead(&this->clipboard(), SkImageInfo::Make(1, 1, kRGBA_F16_SkColorType, kPremul_SkAlphaType), &kRGBAF16Premul, &kRGBAPremul), "");
}
#endif // !BUILDFLAG(IS_ANDROID)
...
TYPED_TEST(ClipboardTest, BitmapWriteAndPngRead_N32_Premul)
{
    ...
    TestBitmapWriteAndPngRead(&this->clipboard(), SkImageInfo::MakeN32Premul(4, 3), b, b);
}
...
TYPED_TEST(ClipboardTest, BitmapWriteAndPngRead_N32_Premul_2x7)
{
    ...
    TestBitmapWriteAndPngRead(&this->clipboard(), SkImageInfo::MakeN32Premul(2, 7), b, b);
}
...
{
    const ClipboardFormatType kFormat = ClipboardFormatType::GetType("chromium/x-test-format");
    ...
    ASSERT_TRUE(this->clipboard().IsFormatAvailable(kFormat, ClipboardBuffer::kCopyPaste, /* data_dst = */ nullptr));
    std::string output;
...
{
    const ClipboardFormatType kFormat1 = ClipboardFormatType::GetType("chromium/x-test-format1");
    std::string payload1("test string1");
    base::Pickle write_pickle1;
    write_pickle1.WriteString(payload1);

    const ClipboardFormatType kFormat2 = ClipboardFormatType::GetType("chromium/x-test-format2");
    ...
    ASSERT_FALSE(this->clipboard().IsFormatAvailable(kFormat1, ClipboardBuffer::kCopyPaste, /* data_dst = */ nullptr));
    ASSERT_TRUE(this->clipboard().IsFormatAvailable(kFormat2, ClipboardBuffer::kCopyPaste, /* data_dst = */ nullptr));
    ...
    base::Pickle read_pickle2 = base::Pickle::WithData(base::as_byte_span(output2));
    ...
    ASSERT_TRUE(this->clipboard().IsFormatAvailable(kFormat1, ClipboardBuffer::kCopyPaste, /* data_dst = */ nullptr));
    ASSERT_FALSE(this->clipboard().IsFormatAvailable(kFormat2, ClipboardBuffer::kCopyPaste, /* data_dst = */ nullptr));
    ...
    base::Pickle read_pickle1 = base::Pickle::WithData(base::as_byte_span(output1));
...
TYPED_TEST(ClipboardTest, DataTest)
{
    ...
    base::span<const uint8_t> payload_span(reinterpret_cast<const uint8_t*>(payload.data()), payload.size());

    {
        ScopedClipboardWriter clipboard_writer(ClipboardBuffer::kCopyPaste);
        clipboard_writer.WriteData(kFormatString16, mojo_base::BigBuffer(payload_span));
    }

    std::map<std::string, std::string> custom_format_names = this->clipboard().ExtractCustomPlatformNames(ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr);
    EXPECT_TRUE(custom_format_names.find(kFormatString) != custom_format_names.end());
    std::string output;
    this->clipboard().ReadData(ClipboardFormatType::CustomPlatformType(custom_format_names[kFormatString]),
...
TYPED_TEST(ClipboardTest, MultipleDataTest)
{
    ...
    base::span<const uint8_t> payload_span1(reinterpret_cast<const uint8_t*>(payload1.data()), payload1.size());
    ...
    base::span<const uint8_t> payload_span2(reinterpret_cast<const uint8_t*>(payload2.data()), payload2.size());

    {
        ScopedClipboardWriter clipboard_writer(ClipboardBuffer::kCopyPaste);
        // Both payloads should write successfully and not overwrite one another.
        clipboard_writer.WriteData(kFormatString116, mojo_base::BigBuffer(payload_span1));
        clipboard_writer.WriteData(kFormatString216, mojo_base::BigBuffer(payload_span2));
    }

    // Check format 1.
    EXPECT_THAT(this->clipboard().ReadAvailableStandardAndCustomFormatNames(ClipboardBuffer::kCopyPaste, /* data_dst = */ nullptr),
    ...
    std::map<std::string, std::string> custom_format_names = this->clipboard().ExtractCustomPlatformNames(ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr);
    EXPECT_TRUE(custom_format_names.find(kFormatString1) != custom_format_names.end());
    std::string output1;
    this->clipboard().ReadData(ClipboardFormatType::CustomPlatformType(custom_format_names[kFormatString1]),
    ...
    EXPECT_TRUE(custom_format_names.find(kFormatString2) != custom_format_names.end());
    std::string output2;
    this->clipboard().ReadData(ClipboardFormatType::CustomPlatformType(custom_format_names[kFormatString2]),
...
TYPED_TEST(ClipboardTest, DataAndPortableFormatTest)
{
    ...
    // Check format 2.
    EXPECT_THAT(
        this->clipboard().ReadAvailableStandardAndCustomFormatNames(ClipboardBuffer::kCopyPaste, /* data_dst = */ nullptr), Contains(u"web text/plain"));
    EXPECT_TRUE(custom_format_names.find(kFormatString2) != custom_format_names.end());
    std::string output2;
    this->clipboard().ReadData(ClipboardFormatType::CustomPlatformType(custom_format_names[kFormatString2]),
        /* data_dst = */ nullptr, &output2);
...
#endif
    base::span<const uint8_t> text_span(reinterpret_cast<const uint8_t*>(kPlatformSpecificText.data()), kPlatformSpecificText.size());
    {
        ScopedClipboardWriter clipboard_writer(ClipboardBuffer::kCopyPaste);
        clipboard_writer.WriteData(kFormatString16, mojo_base::BigBuffer(text_span));
    }
...
TYPED_TEST(ClipboardTest, HyperlinkTest)
{
    ...
    const std::u16string kExpectedHtml(u"<a href=\"http://www.example.com?x=3&amp;lt=3#&quot;&#39;&lt;&gt;\">"
    ...
    uint32_t fragment_end;
    this->clipboard().ReadHTML(ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr, &html_result, &url_result, &fragment_start, &fragment_end);
    EXPECT_EQ(kExpectedHtml, html_result.substr(fragment_end - kExpectedHtml.size(), kExpectedHtml.size()));
}
#endif
...
TYPED_TEST(ClipboardTest, WebSmartPasteTest)
{
    ...
    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::WebKitSmartPasteType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
}

#if BUILDFLAG(IS_WIN) // Windows only tests.
void HtmlTestHelper(const std::string& cf_html, const std::string& expected_html)
{
    std::string html;
...
TYPED_TEST(ClipboardTest, PrivacyMetadataTest)
{
    ...
    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::ClipboardHistoryType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
    EXPECT_TRUE(this->clipboard().IsFormatAvailable(ClipboardFormatType::UploadCloudClipboardType(), ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr));
    std::string result;
...
{
    const ClipboardSequenceNumberToken first_sequence_number = this->clipboard().GetSequenceNumber(ClipboardBuffer::kCopyPaste);
    ...
    const ClipboardSequenceNumberToken second_sequence_number = this->clipboard().GetSequenceNumber(ClipboardBuffer::kCopyPaste);
...
TYPED_TEST(ClipboardTest, PolicyAllowDataRead)
{
    ...
    {
        ScopedClipboardWriter writer(ClipboardBuffer::kCopyPaste, std::make_unique<DataTransferEndpoint>(GURL("https://www.google.com")));
        writer.WriteText(kTestText);
    }
    EXPECT_CALL(*policy_controller, IsClipboardReadAllowed).WillRepeatedly(testing::Return(true));
...
TYPED_TEST(ClipboardTest, PolicyDisallow_ReadText)
{
    ...
    const std::u16string kTestText(u"World");
    {
        ScopedClipboardWriter writer(ClipboardBuffer::kCopyPaste, std::make_unique<DataTransferEndpoint>(GURL("https://google.com/")));
        writer.WriteText(kTestText);
    }
    EXPECT_CALL(*policy_controller, IsClipboardReadAllowed).WillRepeatedly(testing::Return(false));
    std::u16string read_result;
    this->clipboard().ReadText(ClipboardBuffer::kCopyPaste,
        /* data_dst = */ nullptr, &read_result);
    ::testing::Mock::VerifyAndClearExpectations(policy_controller.get());
...
TYPED_TEST(ClipboardTest, PolicyDisallow_ReadPng)
{
    ...
    WriteBitmap(&this->clipboard(), SkImageInfo::MakeN32Premul(4, 3), reinterpret_cast<const void*>(kBitMapData));
    EXPECT_CALL(*policy_controller, IsClipboardReadAllowed).WillRepeatedly(testing::Return(false));
