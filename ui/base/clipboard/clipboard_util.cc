...
std::vector<uint8_t> EncodeBitmapToPngImpl(const SkBitmap& bitmap)
{
    ...
    // compression ratio.
    std::optional<std::vector<uint8_t>> data = gfx::PNGCodec::FastEncodeBGRASkBitmap(bitmap,
...
{
    return url.empty() || (/*!base::FeatureList::IsEnabled(features::kWriteBookmarkWithoutTitle) &&*/ title.empty());
}
