...
{
    const std::string result_cfhtml = clipboard_util::HtmlToCFHtml(std::string(), "www.example.com");
...
{
    const std::string well_formed_html = "<html><head><style>p {color:blue}</style></head><body><p>Hello "
                                         "World</p></body></html>";
    const std::string url = "www.example.com";
    const std::string expected_cfhtml = "Version:0.9\r\n"
    ...
    const std::string actual_cfhtml = clipboard_util::HtmlToCFHtml(well_formed_html, url);
