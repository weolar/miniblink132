...
namespace {
bool IsReadAllowed(base::optional_ref<const DataTransferEndpoint> src, base::optional_ref<const DataTransferEndpoint> dst)
{
...
TestClipboard* TestClipboard::CreateForCurrentThread()
{
    ...
    (*Clipboard::ClipboardMapPtr())[base::PlatformThread::CurrentId()] = base::WrapUnique(clipboard);
    return clipboard;
}
...
std::optional<DataTransferEndpoint> TestClipboard::GetSource(ClipboardBuffer buffer) const
{
    return GetStore(buffer).GetDataSource();
}

const ClipboardSequenceNumberToken& TestClipboard::GetSequenceNumber(ClipboardBuffer buffer) const
{
    return GetStore(buffer).sequence_number;
}

bool TestClipboard::IsFormatAvailable(const ClipboardFormatType& format, ClipboardBuffer buffer, const ui::DataTransferEndpoint* data_dst) const
{
    ...
    if (format == ClipboardFormatType::UrlType())
        return IsFormatAvailable(ClipboardFormatType::PlainTextType(), buffer, data_dst);
    ...
    if (format == ClipboardFormatType::PngType() || format == ClipboardFormatType::BitmapType()) {
        return base::Contains(store.data, ClipboardFormatType::PngType()) || base::Contains(store.data, ClipboardFormatType::BitmapType());
    }
...
std::vector<std::u16string> TestClipboard::GetStandardFormats(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst) const
{
    ...
    if (IsFormatAvailable(ClipboardFormatType::PngType(), buffer, data_dst) || IsFormatAvailable(ClipboardFormatType::BitmapType(), buffer, data_dst))
...
void TestClipboard::ReadAvailableTypes(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::vector<std::u16string>* types) const
{
    DCHECK(types);
    types->clear();
    if (!IsReadAllowed(GetStore(buffer).data_src, data_dst)) {
...
void TestClipboard::ReadText(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* result) const
{
    if (!IsReadAllowed(GetStore(buffer).data_src, data_dst)) {
        return;
    }
...
void TestClipboard::ReadAsciiText(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::string* result) const
{
    const DataStore& store = GetStore(buffer);
    if (!IsReadAllowed(store.data_src, data_dst)) {
        return;
...
void TestClipboard::ReadHTML(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* markup, std::string* src_url,
...
void TestClipboard::ReadSvg(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::u16string* result) const
{
    const DataStore& store = GetStore(buffer);
    if (!IsReadAllowed(store.data_src, data_dst)) {
        return;
...
void TestClipboard::ReadRTF(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::string* result) const
{
    const DataStore& store = GetStore(buffer);
    if (!IsReadAllowed(store.data_src, data_dst)) {
        return;
...
void TestClipboard::ReadPng(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, ReadPngCallback callback) const
{
    const DataStore& store = GetStore(buffer);
    if (!IsReadAllowed(store.data_src, data_dst)) {
    ...
    ClipboardBuffer buffer, const std::u16string& type, const DataTransferEndpoint* data_dst, std::u16string* result) const
{
    ...
    if (it != store.data.end()) {
        result->clear();
        *result = ReadCustomDataForType(base::as_byte_span(it->second), type).value_or(u"");
    }
}

void TestClipboard::ReadFilenames(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, std::vector<ui::FileInfo>* result) const
{
    const DataStore& store = GetStore(buffer);
    if (!IsReadAllowed(store.data_src, data_dst)) {
        return;
...
// TODO(crbug.com/40704509): |data_dst| should be supported.
void TestClipboard::ReadBookmark(const DataTransferEndpoint* data_dst, std::u16string* title, std::string* url) const
{
...
void TestClipboard::ReadData(const ClipboardFormatType& format, const DataTransferEndpoint* data_dst, std::string* result) const
{
    const DataStore& store = GetDefaultStore();
    if (!IsReadAllowed(store.data_src, data_dst)) {
        return;
...
void TestClipboard::WritePortableAndPlatformRepresentations(ClipboardBuffer buffer, const ObjectMap& objects,
...
void TestClipboard::WriteText(std::string_view text)
{
    ...
    if (IsSupportedClipboardBuffer(ClipboardBuffer::kSelection))
        GetStore(ClipboardBuffer::kSelection).data[ClipboardFormatType::PlainTextType()] = text;
    ClipboardMonitor::GetInstance()->NotifyClipboardDataChanged();
}

void TestClipboard::WriteHTML(std::string_view markup, std::optional<std::string_view> source_url)
{
...
void TestClipboard::WriteBookmark(std::string_view title, std::string_view url)
{
...
void TestClipboard::WriteBitmap(const SkBitmap& bitmap)
{
    ...
    GetDefaultStore().png = gfx::PNGCodec::EncodeBGRASkBitmap(bitmap, /*discard_transparency=*/false).value();
    ClipboardMonitor::GetInstance()->NotifyClipboardDataChanged();
}

void TestClipboard::WriteData(const ClipboardFormatType& format, base::span<const uint8_t> data)
{
    GetDefaultStore().data[format] = std::string(reinterpret_cast<const char*>(data.data()), data.size());
    ClipboardMonitor::GetInstance()->NotifyClipboardDataChanged();
}
...
TestClipboard::DataStore& TestClipboard::DataStore::operator=(const DataStore& other)
{
    sequence_number = other.sequence_number;
    data = other.data;
    url_title = other.url_title;
...
void TestClipboard::DataStore::SetDataSource(std::optional<DataTransferEndpoint> new_data_src)
{
    data_src = std::move(new_data_src);
}

std::optional<DataTransferEndpoint> TestClipboard::DataStore::GetDataSource() const
{
    return data_src;
}

const TestClipboard::DataStore& TestClipboard::GetStore(ClipboardBuffer buffer) const
{
