...
TEST(FileInfoTest, Roundtrip)
{
    ...
    for (const TestCase& test : tests) {
        ...
        for (const auto& path : test.paths) {
            expected.push_back(base::FilePath(path).NormalizePathSeparators().value());
        }
...
TEST(FileInfoTest, Backslashes)
{
    ...
    const TestCase tests[] = {
        ...
        { FPL("\\\\host\\path"), "file:///%5C%5Chost%5Cpath", FPL("/\\\\host\\path") },
#endif
    };
    for (const TestCase& test : tests) {
        ...
        EXPECT_EQ(test.path_roundtrip.value_or(test.path), filenames[0].path.value());
    }
}
