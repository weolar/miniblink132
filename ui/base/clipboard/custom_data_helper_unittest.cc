...
{
    std::unordered_map<std::u16string, std::u16string> data = { { u"abc", std::u16string() }, { u"de", u"1" }, { u"f", u"23" } };
    WriteCustomDataToPickle(data, pickle);
}
...
TEST(CustomDataHelperTest, EmptyReadMap)
{
    ...
    EXPECT_EQ((std::unordered_map<std::u16string, std::u16string>()), ReadCustomDataIntoMap(pickle));
}
...
TEST(CustomDataHelperTest, ReadMap)
{
    ...
    const std::unordered_map<std::u16string, std::u16string> expected = { { u"abc", std::u16string() }, { u"de", u"1" }, { u"f", u"23" } };
