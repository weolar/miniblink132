...
public:
    ...
    int data_changed_count() const
    {
        return data_changed_count_;
    }
...
TEST_F(ClipboardWinTest, NoDataChangedNotificationOnRead)
{
    ...
    clipboard->ReadAsciiText(ClipboardBuffer::kCopyPaste, nullptr, &ascii_text_result);
    ASSERT_EQ(data_changed_count(), 0);
    ...
    uint32_t start;
    uint32_t end;
    clipboard->ReadHTML(ClipboardBuffer::kCopyPaste, nullptr, &html, &src_url, &start, &end);
    ASSERT_EQ(data_changed_count(), 0);
    ...
    clipboard->ReadPng(ClipboardBuffer::kCopyPaste, nullptr, png_future.GetCallback());
    ASSERT_TRUE(png_future.Wait());
    ASSERT_EQ(data_changed_count(), 0);

    std::u16string custom_data_result;
    clipboard->ReadDataTransferCustomData(ClipboardBuffer::kCopyPaste, u"text/plain", nullptr, &custom_data_result);
    ASSERT_EQ(data_changed_count(), 0);
    ...
    clipboard->ReadData(ClipboardFormatType::PlainTextType(), nullptr, &data_result);
    ASSERT_EQ(data_changed_count(), 0);
}
