...
ScopedClipboardWriter::ScopedClipboardWriter(ClipboardBuffer buffer, std::unique_ptr<DataTransferEndpoint> data_src)
    : buffer_(buffer)
...
ScopedClipboardWriter::~ScopedClipboardWriter()
{
    ...
    if (main_frame_url_.is_valid() || frame_url_.is_valid()) {
        auto text_iter = objects_.find(base::VariantIndexOfType<Clipboard::Data, Clipboard::TextData>());
        if (text_iter != objects_.end()) {
            const auto& text_data = absl::get<Clipboard::TextData>(text_iter->second.data);
            Clipboard::GetForCurrentThread()->NotifyCopyWithUrl(text_data.data, frame_url_, main_frame_url_);
        }
    }

    if (!objects_.empty() || !platform_representations_.empty()) {
        Clipboard::GetForCurrentThread()->WritePortableAndPlatformRepresentations(
            buffer_, objects_, std::move(platform_representations_), std::move(data_src_), privacy_types_);
    }
}

void ScopedClipboardWriter::SetDataSource(std::unique_ptr<DataTransferEndpoint> data_src)
{
    data_src_ = std::move(data_src);
}

void ScopedClipboardWriter::SetDataSourceURL(const GURL& main_frame, const GURL& frame_url)
{
...
void ScopedClipboardWriter::WriteHTML(const std::u16string& markup, const std::string& source_url)
{
...
void ScopedClipboardWriter::WriteSvg(const std::u16string& markup)
{
    ...
    Clipboard::Data data = Clipboard::SvgData { .markup = base::UTF16ToUTF8(markup) };
    const size_t index = data.index();
    objects_[index] = Clipboard::ObjectMapParams(std::move(data));
}
...
void ScopedClipboardWriter::WriteBookmark(const std::u16string& bookmark_title, const std::string& url)
{
    ...
    Clipboard::Data data = Clipboard::BookmarkData { .title = base::UTF16ToUTF8(bookmark_title), .url = url };
    const size_t index = data.index();
    objects_[index] = Clipboard::ObjectMapParams(std::move(data));
}

void ScopedClipboardWriter::WriteHyperlink(const std::u16string& anchor_text, const std::string& url)
{
...
void ScopedClipboardWriter::WritePickledData(const base::Pickle& pickle, const ClipboardFormatType& format)
{
    RecordWrite(ClipboardFormatMetric::kCustomData);
    Clipboard::RawData raw_data;
    raw_data.format = format;
    raw_data.data = std::vector<uint8_t>(reinterpret_cast<const uint8_t*>(pickle.data()), reinterpret_cast<const uint8_t*>(pickle.data()) + pickle.size());
    Clipboard::Data data = std::move(raw_data);
    const size_t index = data.index();
    objects_[index] = Clipboard::ObjectMapParams(std::move(data));
}

void ScopedClipboardWriter::WriteData(const std::u16string& format, mojo_base::BigBuffer data)
{
    ...
    if (registered_formats_.find(format_in_ascii) == registered_formats_.end()) {
        std::string web_custom_format_string = ClipboardFormatType::WebCustomFormatName(counter_);
        registered_formats_[format_in_ascii] = web_custom_format_string;
        counter_++;
        platform_representations_.push_back({ web_custom_format_string, std::move(data) });
    }
}
