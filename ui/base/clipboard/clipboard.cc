...
// static
void Clipboard::SetAllowedThreads(const std::vector<base::PlatformThreadId>& allowed_threads)
{
...
// static
void Clipboard::SetClipboardForCurrentThread(std::unique_ptr<Clipboard> platform_clipboard)
{
    base::AutoLock lock(ClipboardMapLock());
...
std::map<std::string, std::string> Clipboard::ExtractCustomPlatformNames(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst) const
{
    // Read the JSON metadata payload.
    std::map<std::string, std::string> custom_format_names;
    if (IsFormatAvailable(ui::ClipboardFormatType::WebCustomFormatMap(), buffer, data_dst)) {
        std::string custom_format_json;
        // Read the custom format map.
        ReadData(ui::ClipboardFormatType::WebCustomFormatMap(), data_dst, &custom_format_json);
        if (!custom_format_json.empty()) {
            std::optional<base::Value> json_val = base::JSONReader::Read(custom_format_json);
            if (json_val.has_value() && json_val->is_dict()) {
                for (const auto it : json_val->GetDict()) {
                    ...
                    if (custom_format_name) {
                        ...
                        if (net::ParseMimeTypeWithoutParameter(web_format, &web_top_level_mime_type, &web_mime_sub_type)) {
                            std::string web_custom_format_string = base::StrCat({ kWebClipboardFormatPrefix, web_top_level_mime_type, "/", web_mime_sub_type });
                            custom_format_names.emplace(std::move(web_custom_format_string), *custom_format_name);
                        }
                    }
                }
            }
...
std::vector<std::u16string> Clipboard::ReadAvailableStandardAndCustomFormatNames(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst) const
{
    ...
    std::map<std::string, std::string> custom_format_names = ExtractCustomPlatformNames(buffer, data_dst);
...
void Clipboard::DispatchPortableRepresentation(const ObjectMapParams& params)
{
    ...
    absl::visit(base::Overloaded {
                    ...
                    [&](const BookmarkData& data) {
                        if (ui::clipboard_util::ShouldSkipBookmark(base::UTF8ToUTF16(data.title), data.url)) {
                            return;
                        }
                    ...
                    [&](const RawData& data) {
                        ...
                        base::span<const uint8_t> byte_data(data.data.data(), data.data.size());
                        //WriteData(data.format, base::as_bytes(base::make_span(data.data)));
                        WriteData(data.format, byte_data);
                    },
                    [&](const SvgData& data) {
                        if (data.markup.empty()) {
                            return;
                    ...
                    [&](const WebCustomFormatMapData& data) {
                        ...
                        //WriteData(ClipboardFormatType::WebCustomFormatMap(), base::as_bytes(base::make_span(data.data)));
                        base::span<const uint8_t> byte_data((const uint8_t*)data.data.data(), data.data.size());
                        WriteData(ClipboardFormatType::WebCustomFormatMap(), byte_data);
                    },
                },
        params.data);
}
...
Clipboard::ObjectMapParams::ObjectMapParams(const ObjectMapParams& other) = default;
Clipboard::ObjectMapParams& Clipboard::ObjectMapParams::operator=(const ObjectMapParams& other) = default;

Clipboard::ObjectMapParams::ObjectMapParams(ObjectMapParams&& other) = default;
Clipboard::ObjectMapParams& Clipboard::ObjectMapParams::operator=(ObjectMapParams&& other) = default;

Clipboard::ObjectMapParams::~ObjectMapParams() = default;

void Clipboard::DispatchPlatformRepresentations(std::vector<Clipboard::PlatformRepresentation> platform_representations)
{
    for (const auto& representation : platform_representations) {
        base::span<const uint8_t> data(representation.data.data(), representation.data.size());
        //WriteData(ClipboardFormatType::CustomPlatformType(representation.format), base::as_bytes(base::make_span(representation.data)));
        WriteData(ClipboardFormatType::CustomPlatformType(representation.format), data);
    }
}
...
void Clipboard::NotifyCopyWithUrl(std::string_view text, const GURL& frame, const GURL& main_frame)
{
    GURL text_url(text);
    if (text_url.is_valid()) {
        write_observers_.Notify(&ClipboardWriteObserver::OnCopyURL, text_url, frame, main_frame);
    }
}
...
{
    static base::NoDestructor<std::vector<base::PlatformThreadId>> allowed_threads;
...
void Clipboard::ReadAvailableTypes(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, ReadAvailableTypesCallback callback) const
{
...
void Clipboard::ReadText(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, ReadTextCallback callback) const
{
...
void Clipboard::ReadAsciiText(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, ReadAsciiTextCallback callback) const
{
    std::string result;
...
void Clipboard::ReadHTML(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, ReadHtmlCallback callback) const
{
    ...
    std::move(callback).Run(std::move(markup), GURL(src_url), fragment_start, fragment_end);
}

void Clipboard::ReadSvg(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, ReadSvgCallback callback) const
{
    std::u16string result;
...
void Clipboard::ReadRTF(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, ReadRTFCallback callback) const
{
    std::string result;
...
void Clipboard::ReadFilenames(ClipboardBuffer buffer, const DataTransferEndpoint* data_dst, ReadFilenamesCallback callback) const
{
...
void Clipboard::ReadBookmark(const DataTransferEndpoint* data_dst, ReadBookmarkCallback callback) const
{
    std::u16string title;
    std::string url;
...
void Clipboard::ReadData(const ClipboardFormatType& format, const DataTransferEndpoint* data_dst, ReadDataCallback callback) const
{
    std::string result;
