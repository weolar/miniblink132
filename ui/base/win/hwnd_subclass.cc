...
{
    ui::HWNDSubclass* wrapped_wnd_proc = reinterpret_cast<ui::HWNDSubclass*>(ui::ViewProp::GetValue(hwnd, kHWNDSubclassKey));
    return wrapped_wnd_proc ? wrapped_wnd_proc->OnWndProc(hwnd, message, w_param, l_param) : DefWindowProc(hwnd, message, w_param, l_param);
}
...
public:
    ...
    {
        return base::Singleton<HWNDSubclassFactory, base::LeakySingletonTraits<HWNDSubclassFactory>>::get();
    }
    ...
    {
        DCHECK(target);
        HWNDSubclass* subclass = reinterpret_cast<HWNDSubclass*>(ui::ViewProp::GetValue(target, kHWNDSubclassKey));
        if (!subclass) {
...
{
    HWNDSubclassFactory::GetInstance()->GetHwndSubclassForTarget(target)->AddFilter(filter);
}
...
{
    std::vector<raw_ptr<HWNDMessageFilter, VectorExperimental>>::iterator it = base::ranges::find(filters_, filter);
    if (it != filters_.end())
        filters_.erase(it);
}
...
LRESULT HWNDSubclass::OnWndProc(HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param)
{
    ...
    if (message == WM_TOUCH) {
        ...
        if (ui::GetTouchInputInfoWrapper(reinterpret_cast<HTOUCHINPUT>(l_param), 1, &point, sizeof(TOUCHINPUT))) {
            POINT touch_location = { TOUCH_COORD_TO_PIXEL(point.x), TOUCH_COORD_TO_PIXEL(point.y) };
    ...
    for (std::vector<raw_ptr<HWNDMessageFilter, VectorExperimental>>::iterator it = filters_.begin(); it != filters_.end(); ++it) {
        LRESULT l_result = 0;
