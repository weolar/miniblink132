...
bool SendMouseEvent(const gfx::Point& point, int flags)
{
    ...
    input.mi.dx = static_cast<LONG>(std::max(1.0, std::ceil(screen_x * (kNormalizedScreenSize / screen_width))));
    input.mi.dy = static_cast<LONG>(std::max(1.0, std::ceil(screen_y * (kNormalizedScreenSize / screen_height))));
