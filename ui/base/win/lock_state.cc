...
bool IsSessionLocked()
{
    ...
    if (::WTSQuerySessionInformation(WTS_CURRENT_SERVER, WTS_CURRENT_SESSION, WTSSessionInfoEx, &buffer, &buffer_length)
        && buffer_length >= sizeof(WTSINFOEXW)) {
        auto* info = reinterpret_cast<WTSINFOEXW*>(buffer);
        is_locked = info->Data.WTSInfoExLevel1.SessionFlags == WTS_SESSIONSTATE_LOCK;
    }
    if (buffer)
...
public:
    SessionLockedObserver()
        : session_change_observer_(base::BindRepeating(&SessionLockedObserver::OnSessionChange, base::Unretained(this)))
