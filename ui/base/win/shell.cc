...
bool InvokeShellExecute(const std::wstring& path, const std::wstring& working_directory, const std::wstring& args, const std::wstring& verb,
    const std::wstring& class_name, DWORD mask)
{
...
bool OpenFileViaShell(const base::FilePath& full_path)
{
    ...
    return InvokeShellExecute(full_path.value(), full_path.DirName().value(), std::wstring(), std::wstring(), std::wstring(), kDefaultShellExecuteFlags);
}
...
bool OpenFolderViaShell(const base::FilePath& full_path)
{
    ...
    return InvokeShellExecute(full_path.value(), full_path.value(), std::wstring(), L"explore", L"folder", kDefaultShellExecuteFlags);
}
...
bool PreventWindowFromPinning(HWND hwnd)
{
    ...
    return base::win::SetBooleanValueForPropertyStore(pps.Get(), PKEY_AppUserModel_PreventPinning, true);
}
...
void SetAppDetailsForWindow(const std::wstring& app_id, const base::FilePath& app_icon_path, int app_icon_index, const std::wstring& relaunch_command,
    const std::wstring& relaunch_display_name, HWND hwnd)
{
    DCHECK(hwnd);
    ...
    if (!app_icon_path.empty()) {
        ...
        base::win::SetStringValueForPropertyStore(
            pps.Get(), PKEY_AppUserModel_RelaunchIconResource, base::StrCat({ app_icon_path.value(), L",", base::NumberToWString(app_icon_index) }).c_str());
    }
    if (!relaunch_command.empty()) {
        base::win::SetStringValueForPropertyStore(pps.Get(), PKEY_AppUserModel_RelaunchCommand, relaunch_command.c_str());
    }
    if (!relaunch_display_name.empty()) {
        base::win::SetStringValueForPropertyStore(pps.Get(), PKEY_AppUserModel_RelaunchDisplayNameResource, relaunch_display_name.c_str());
    }
}

void SetAppIdForWindow(const std::wstring& app_id, HWND hwnd)
{
    SetAppDetailsForWindow(app_id, base::FilePath(), 0, std::wstring(), std::wstring(), hwnd);
}

void SetAppIconForWindow(const base::FilePath& app_icon_path, int app_icon_index, HWND hwnd)
{
    SetAppDetailsForWindow(std::wstring(), app_icon_path, app_icon_index, std::wstring(), std::wstring(), hwnd);
}

void SetRelaunchDetailsForWindow(const std::wstring& relaunch_command, const std::wstring& display_name, HWND hwnd)
{
    SetAppDetailsForWindow(std::wstring(), base::FilePath(), 0, relaunch_command, display_name, hwnd);
}
