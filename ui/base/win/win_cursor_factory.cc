...
scoped_refptr<PlatformCursor> WinCursorFactory::GetDefaultCursor(mojom::CursorType type)
{
...
std::optional<CursorData> WinCursorFactory::GetCursorData(mojom::CursorType type)
{
    ...
    return ui::CursorData({ std::move(bitmap) }, IconUtil::GetHotSpotFromHICON(hcursor));
}

scoped_refptr<PlatformCursor> WinCursorFactory::CreateImageCursor(mojom::CursorType type, const SkBitmap& bitmap, const gfx::Point& hotspot, float scale)
{
    return base::MakeRefCounted<WinCursor>(IconUtil::CreateCursorFromSkBitmap(bitmap, hotspot).release(),
        /*should_destroy=*/true);
}
