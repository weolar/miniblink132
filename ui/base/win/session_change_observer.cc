...
public:
    ...
    WtsRegistrationNotificationManager()
    {
        ...
        singleton_hwnd_observer_
            = std::make_unique<gfx::SingletonHwndObserver>(base::BindRepeating(&WtsRegistrationNotificationManager::OnWndProc, base::Unretained(this)));

        base::OnceClosure wts_register
            = base::BindOnce(base::IgnoreResult(&WTSRegisterSessionNotification), gfx::SingletonHwnd::GetInstance()->hwnd(), DWORD { NOTIFY_FOR_THIS_SESSION });

        base::ThreadPool::CreateCOMSTATaskRunner({})->PostTask(FROM_HERE, std::move(wts_register));
    }

    WtsRegistrationNotificationManager(const WtsRegistrationNotificationManager&) = delete;
    WtsRegistrationNotificationManager& operator=(const WtsRegistrationNotificationManager&) = delete;
...
private:
    friend struct base::DefaultSingletonTraits<WtsRegistrationNotificationManager>;
        ...
        case WM_WTSSESSION_CHANGE:
            if (wparam == WTS_SESSION_LOCK || wparam == WTS_SESSION_UNLOCK) {
                ...
                if (!::ProcessIdToSessionId(::GetCurrentProcessId(), &current_session_id)) {
                    ...
                    is_current_session = (static_cast<DWORD>(lparam) == current_session_id);
                }
                observer_list_.Notify(&SessionChangeObserver::OnSessionChange, wparam, is_current_session_ptr);
            }
            break;
        case WM_DESTROY:
            RemoveSingletonHwndObserver();
...
void SessionChangeObserver::OnSessionChange(WPARAM wparam, const bool* is_current_session)
{
