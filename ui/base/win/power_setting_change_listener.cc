...
PowerSettingChangeObserver::PowerSettingChangeObserver()
    : singleton_hwnd_observer_(new gfx::SingletonHwndObserver(base::BindRepeating(&PowerSettingChangeObserver::OnWndProc, base::Unretained(this))))
...
void PowerSettingChangeObserver::AddListener(PowerSettingChangeListener* listener)
{
    listeners_.AddObserver(listener);
}

void PowerSettingChangeObserver::RemoveListener(PowerSettingChangeListener* listener)
{
    listeners_.RemoveObserver(listener);
}

void PowerSettingChangeObserver::OnWndProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    if (message == WM_POWERBROADCAST) {
        ...
        case PBT_POWERSETTINGCHANGE: {
            ...
            if (setting && IsEqualGUID(setting->PowerSetting, GUID_SESSION_DISPLAY_STATUS) && setting->DataLength == sizeof(DWORD)) {
                OnDisplayStateChanged(PowerMonitorOff != static_cast<MONITOR_DISPLAY_STATE>(setting->Data[0]));
            }
        } break;
...
{
    listeners_.Notify(&PowerSettingChangeListener::OnDisplayStateChanged, display_on);
}

HPOWERNOTIFY PowerSettingChangeObserver::RegisterNotification(LPCGUID power_setting)
{
    return RegisterPowerSettingNotification(gfx::SingletonHwnd::GetInstance()->hwnd(), power_setting, DEVICE_NOTIFY_WINDOW_HANDLE);
}
...
ScopedPowerSettingChangeListener::ScopedPowerSettingChangeListener(PowerSettingChangeListener* listener)
    : listener_(listener)
{
