...
const x11::ModMask kModifiersMasks[] = { {}, // No additional modifier.
    x11::ModMask::c_2, // Num lock
    x11::ModMask::Lock, // Caps lock
    x11::ModMask::c_5, // Scroll lock
    x11::ModMask::c_2 | x11::ModMask::Lock, x11::ModMask::c_2 | x11::ModMask::c_5, x11::ModMask::Lock | x11::ModMask::c_5,
    x11::ModMask::c_2 | x11::ModMask::Lock | x11::ModMask::c_5 };

x11::ModMask GetNativeModifiers(bool is_alt_down, bool is_ctrl_down, bool is_shift_down)
{
    constexpr auto kNoMods = x11::ModMask {};
    return (is_shift_down ? x11::ModMask::Shift : kNoMods) | (is_ctrl_down ? x11::ModMask::Control : kNoMods) | (is_alt_down ? x11::ModMask::c_1 : kNoMods);
}
...
bool XGlobalShortcutListener::RegisterAccelerator(KeyboardCode key_code, bool is_alt_down, bool is_ctrl_down, bool is_shift_down)
{
    ...
    for (size_t i = 0; i < std::size(kModifiersMasks); i++) {
        grab_requests[i] = connection_->GrabKey({ false, x_root_window_, modifiers | kModifiersMasks[i], keycode, x11::GrabMode::Async, x11::GrabMode::Async });
    }
    connection_->Flush();
    ...
    registered_combinations_.insert(Accelerator(key_code, is_alt_down, is_ctrl_down, is_shift_down));

    return true;
}

void XGlobalShortcutListener::UnregisterAccelerator(KeyboardCode key_code, bool is_alt_down, bool is_ctrl_down, bool is_shift_down)
{
    auto modifiers = GetNativeModifiers(is_alt_down, is_ctrl_down, is_shift_down);
    auto keysym = XKeysymForWindowsKeyCode(key_code, false);
    auto keycode = connection_->KeysymToKeycode(keysym);
    ...
    registered_combinations_.erase(Accelerator(key_code, is_alt_down, is_ctrl_down, is_shift_down));
}
...
void XGlobalShortcutListener::OnKeyPressEvent(const KeyEvent& event)
{
    ...
    if (!base::Contains(registered_combinations_, Accelerator(key_code, is_alt_down, is_ctrl_down, is_shift_down))) {
        return;
    }
