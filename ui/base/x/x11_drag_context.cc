...
XDragContext::XDragContext(x11::Window local_window, const x11::ClientMessageEvent& event, const SelectionFormatMap& data)
    : local_window_(local_window)
    , source_window_(static_cast<x11::Window>(event.data.data32[0]))
{
    XDragDropClient* source_client = XDragDropClient::GetForWindow(source_window_);
    if (!source_client) {
        bool get_types_from_property = ((event.data.data32[1] & 1) != 0);

        if (get_types_from_property) {
            if (!x11::Connection::Get()->GetArrayProperty(source_window_, x11::GetAtom(kXdndTypeList), &unfetched_targets_)) {
                return;
            }
        } else {
            ...
            for (size_t i = 2; i < 5; ++i) {
                if (event.data.data32[i]) {
                    unfetched_targets_.push_back(static_cast<x11::Atom>(event.data.data32[i]));
                }
            }
        }
...
void XDragContext::OnXdndPositionMessage(
    XDragDropClient* client, x11::Atom suggested_action, x11::Window source_window, x11::Time time_stamp, const gfx::Point& screen_point)
{
...
void XDragContext::RequestNextTarget()
{
    ...
    x11::Connection::Get()->ConvertSelection(
        { local_window_, x11::GetAtom(kXdndSelection), target, x11::GetAtom(kChromiumDragReciever), position_time_stamp_ });
}
...
void XDragContext::OnSelectionNotify(const x11::SelectionNotifyEvent& event)
{
    ...
    if (event.property != x11::Atom::None) {
        ...
        LOG(ERROR) << "XConvertSelection failed for source-advertised target " << static_cast<uint32_t>(event.target);
    }
...
void XDragContext::ReadActions()
{
    ...
    if (!source_client) {
        ...
        if (!x11::Connection::Get()->GetArrayProperty(source_window_, x11::GetAtom(kXdndActionList), &atom_array)) {
            actions_.clear();
        } else {
...
void XDragContext::MaskOperation(x11::Atom xdnd_operation, int* drag_operation) const
{
...
bool XDragContext::DispatchPropertyNotifyEvent(const x11::PropertyNotifyEvent& prop)
{
