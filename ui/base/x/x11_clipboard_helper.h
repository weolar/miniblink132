...
public:
    using SelectionChangeCallback = base::RepeatingCallback<void(ClipboardBuffer)>;
    ...
    void InsertMapping(const std::string& key, const scoped_refptr<base::RefCountedMemory>& memory);
    ...
    SelectionData Read(ClipboardBuffer buffer, const std::vector<x11::Atom>& types);
    ...
    bool IsFormatAvailable(ClipboardBuffer buffer, const ClipboardFormatType& format);
