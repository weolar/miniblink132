...
{
    return { x11::GetAtom(kMimeTypeLinuxUtf8String), x11::GetAtom(kMimeTypeLinuxString), x11::GetAtom(kMimeTypeLinuxText), x11::GetAtom(kMimeTypeText),
        x11::GetAtom(kMimeTypeTextUtf8) };
}
...
void GetAtomIntersection(const std::vector<x11::Atom>& desired, const std::vector<x11::Atom>& offered, std::vector<x11::Atom>* output)
{
...
void AddString16ToVector(const std::u16string& str, std::vector<unsigned char>* bytes)
{
...
std::string RefCountedMemoryToString(const scoped_refptr<base::RefCountedMemory>& memory)
{
    CHECK(memory.get());
...
std::u16string RefCountedMemoryToString16(const scoped_refptr<base::RefCountedMemory>& memory)
{
    CHECK(memory.get());
...
SelectionFormatMap::SelectionFormatMap(const SelectionFormatMap& other) = default;

SelectionFormatMap::~SelectionFormatMap() = default;

void SelectionFormatMap::Insert(x11::Atom atom, const scoped_refptr<base::RefCountedMemory>& item)
{
    data_.erase(atom);
    data_.emplace(atom, item);
}

ui::SelectionData SelectionFormatMap::GetFirstOf(const std::vector<x11::Atom>& requested_types) const
{
...
SelectionData::SelectionData(x11::Atom type, const scoped_refptr<base::RefCountedMemory>& memory)
    : type_(type)
    , memory_(memory)
{
}
...
{
    if (type_ == x11::GetAtom(kMimeTypeLinuxUtf8String) || type_ == x11::GetAtom(kMimeTypeLinuxText) || type_ == x11::GetAtom(kMimeTypeTextUtf8)) {
        return RefCountedMemoryToString(memory_);
    } else {
        // BTW, I looked at COMPOUND_TEXT, and there's no way we're going to
        // support that. Yuck.
        CHECK(type_ == x11::GetAtom(kMimeTypeLinuxString) || type_ == x11::GetAtom(kMimeTypeText));
        std::string result;
        base::ConvertToUtf8AndNormalize(RefCountedMemoryToString(memory_), base::kCodepageLatin1, &result);
        return result;
    }
}
