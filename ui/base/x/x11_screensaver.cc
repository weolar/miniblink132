...
public:
    ScreensaverStatusWatcher()
    {
        ...
        x_screensaver_status_
            = std::make_unique<x11::PropertyCache>(connection, connection->default_root(), std::vector<x11::Atom> { x11::GetAtom("_SCREENSAVER_STATUS") });

        connection->AddEventObserver(this);
        connection->screensaver().SelectInput(connection->default_root(), x11::ScreenSaver::Event::NotifyMask);

        auto reply = connection->screensaver().QueryInfo(connection->default_root()).Sync();
        if (reply) {
            mit_screensaver_active_ = IsMitScreensaverActive(static_cast<x11::ScreenSaver::State>(reply->state));
        }
    }
    ...
    bool ScreensaverActive()
    {
        ...
        auto* status = x_screensaver_status_->GetAs<x11::Atom>(x11::GetAtom("_SCREENSAVER_STATUS"));
