...
void XrandrIntervalOnlyVSyncProvider::GetVSyncParameters(UpdateVSyncCallback callback)
{
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), base::TimeTicks(), XRandrHelper::GetInterval()));
}

bool XrandrIntervalOnlyVSyncProvider::GetVSyncParametersIfAvailable(base::TimeTicks* timebase, base::TimeDelta* interval)
{
    *interval = XRandrHelper::GetInterval();
    return true;
}

bool XrandrIntervalOnlyVSyncProvider::SupportGetVSyncParametersIfAvailable() const
{
    return true;
}
