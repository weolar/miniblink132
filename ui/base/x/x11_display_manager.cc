...
void XDisplayManager::Init()
{
    if (IsXrandrAvailable()) {
        ...
        randr.SelectInput({ x_root_window_, x11::RandR::NotifyMask::ScreenChange | x11::RandR::NotifyMask::OutputChange | x11::RandR::NotifyMask::CrtcChange });
    }
    FetchDisplayList();
}
...
void XDisplayManager::OnEvent(const x11::Event& xev)
{
    ...
    if (xev.As<x11::RandR::NotifyEvent>() || (prop && prop->atom == x11::GetAtom("_NET_WORKAREA"))) {
        DispatchDelayedDisplayListUpdate();
    }
}

void XDisplayManager::SetDisplayList(std::vector<display::Display> displays, size_t primary_display_index)
{
...
void XDisplayManager::FetchDisplayList()
{
    ...
    display::DisplayConfig empty_display_config { display::Display::HasForceDeviceScaleFactor() ? display::Display::GetForcedDeviceScaleFactor() : 1.0f };
    ...
    if (IsXrandrAvailable()) {
        displays = BuildDisplaysFromXRandRInfo(*display_config, &primary_display_index);
    } else {
        displays = GetFallbackDisplayList(display_config->primary_scale, &primary_display_index);
    }
...
void XDisplayManager::OnCurrentWorkspaceChanged(const std::string& new_workspace)
{
...
{
    update_task_.Reset(base::BindOnce(&XDisplayManager::UpdateDisplayList, base::Unretained(this)));
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, update_task_.callback());
}
