...
X11CursorFactory::X11CursorFactory()
    : cursor_loader_(std::make_unique<XCursorLoader>(x11::Connection::Get(),
        base::BindRepeating(&X11CursorFactory::ClearThemeCursors,
...
scoped_refptr<PlatformCursor> X11CursorFactory::CreateImageCursor(mojom::CursorType type, const SkBitmap& bitmap, const gfx::Point& hotspot, float scale)
{
...
void X11CursorFactory::OnCursorThemeNameChanged(const std::string& cursor_theme_name)
{
    ClearThemeCursors();
}
...
scoped_refptr<PlatformCursor> X11CursorFactory::GetDefaultCursor(mojom::CursorType type)
{
    if (!default_cursors_.count(type)) {
        // Try to load a predefined X11 cursor.
        default_cursors_[type]
            = type == mojom::CursorType::kNone ? CreateInvisibleCursor(cursor_loader_.get()) : cursor_loader_->LoadCursor(CursorNamesFromType(type));
    }
