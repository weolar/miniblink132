...
public:
    explicit SelectionRequestorTest()
    ...
    void SendSelectionNotify(x11::Atom selection, x11::Atom target, const std::string& value)
    {
        connection_->SetStringProperty(x_window_, requestor_->x_property_, x11::Atom::STRING, value);
...
protected:
    void SetUp() override
    {
        ...
        helper_ = std::make_unique<XClipboardHelper>(base::BindRepeating([](ClipboardBuffer buffer) {}));
...
void PerformBlockingConvertSelection(SelectionRequestor* requestor, x11::Atom selection, x11::Atom target, const std::string& expected_data)
{
    std::vector<uint8_t> out_data;
    x11::Atom out_type = x11::Atom::None;
    EXPECT_TRUE(requestor->PerformBlockingConvertSelection(selection, target, &out_data, &out_type));
    EXPECT_EQ(expected_data.size(), out_data.size());
    EXPECT_EQ(expected_data, ui::RefCountedMemoryToString(base::MakeRefCounted<base::RefCountedBytes>(std::move(out_data))));
...
TEST_F(SelectionRequestorTest, DISABLED_NestedRequests)
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&PerformBlockingConvertSelection, base::Unretained(requestor_), selection, target2, "Data2"));
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&SelectionRequestorTest::SendSelectionNotify, base::Unretained(this), selection, target1, "Data1"));
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&SelectionRequestorTest::SendSelectionNotify, base::Unretained(this), selection, target2, "Data2"));
