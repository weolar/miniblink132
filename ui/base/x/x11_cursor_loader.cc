...
std::string CursorPathFromLibXcursor()
{
    ...
    if (auto* sym = reinterpret_cast<decltype(&XcursorLibraryPath)>(dlsym(lib.get(), "XcursorLibraryPath"))) {
...
{
    constexpr const char kDefaultPath[] = "~/.local/share/icons:~/.icons:/usr/share/icons:/usr/share/pixmaps:"
...
x11::Render::PictFormat GetRenderARGBFormat(const x11::Render::QueryPictFormatsReply& formats)
{
    for (const auto& format : formats.formats) {
        if (format.type == x11::Render::PictType::Direct && format.depth == 32 && format.direct.alpha_shift == 24 && format.direct.alpha_mask == 0xff
            && format.direct.red_shift == 16 && format.direct.red_mask == 0xff && format.direct.green_shift == 8 && format.direct.green_mask == 0xff
...
std::vector<std::string> GetBaseThemes(const base::FilePath& abspath)
{
    ...
    for (const auto& pair : pairs) {
        if (base::TrimWhitespaceASCII(pair.first, base::TRIM_ALL) == kKeyInherits) {
            return base::SplitString(pair.second, ",;", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        }
    }
    return {};
}
...
scoped_refptr<base::RefCountedMemory> ReadCursorFromTheme(const std::string& theme, const std::string& name)
{
    constexpr const char kCursorDir[] = "cursors";
    constexpr const char kThemeInfo[] = "index.theme";
    std::vector<std::string> base_themes;

    auto paths = base::SplitString(CursorPath(), ":", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
scoped_refptr<base::RefCountedMemory> ReadCursorFile(const std::string& name, const std::string& rm_xcursor_theme)
{
    constexpr const char kDefaultTheme[] = "default";
    std::string themes[] = {
#if BUILDFLAG(IS_LINUX)
        // The toolkit theme has the highest priority.
        LinuxUi::instance() ? LinuxUi::instance()->GetCursorThemeName() : std::string(),
#endif
...
std::vector<XCursorLoader::Image> ReadCursorImages(const std::vector<std::string>& names, const std::string& rm_xcursor_theme, uint32_t preferred_size)
{
...
XCursorLoader::XCursorLoader(x11::Connection* connection, base::RepeatingClosure on_cursor_config_changed)
    : connection_(connection)
    , on_cursor_config_changed_(std::move(on_cursor_config_changed))
    , rm_cache_(connection, connection->default_root(), { x11::Atom::RESOURCE_MANAGER },
          base::BindRepeating(&XCursorLoader::OnPropertyChanged, base::Unretained(this)))
{
...
scoped_refptr<X11Cursor> XCursorLoader::LoadCursor(const std::vector<std::string>& names)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    auto cursor = base::MakeRefCounted<X11Cursor>();
    if (SupportsCreateCursor()) {
        base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock(), base::TaskShutdownBehavior::SKIP_ON_SHUTDOWN },
            base::BindOnce(ReadCursorImages, names, rm_xcursor_theme_, GetPreferredCursorSize()),
            base::BindOnce(&XCursorLoader::LoadCursorImpl, weak_factory_.GetWeakPtr(), cursor, names));
    } else {
...
scoped_refptr<X11Cursor> XCursorLoader::CreateCursor(const std::vector<Image>& images)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    for (const Image& image : images) {
        ...
        elements.push_back(x11::Render::AnimationCursorElement { cursor->xcursor_, static_cast<uint32_t>(image.frame_delay.InMilliseconds()) });
    }
...
scoped_refptr<X11Cursor> XCursorLoader::CreateCursor(const SkBitmap& bitmap, const gfx::Point& hotspot)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    connection_->CreatePixmap({ 32, pixmap, connection_->default_root(), width, height });
    ...
    auto cursor = connection_->GenerateId<x11::Cursor>();
    connection_->render().CreateCursor({ cursor, pic, static_cast<uint16_t>(hotspot.x()), static_cast<uint16_t>(hotspot.y()) });
...
void XCursorLoader::LoadCursorImpl(scoped_refptr<X11Cursor> cursor, const std::vector<std::string>& names, const std::vector<XCursorLoader::Image>& images)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (!images.empty()) {
        ...
        connection_->CreateGlyphCursor({ xcursor, cursor_font_, cursor_font_, static_cast<uint16_t>(2 * core_char), static_cast<uint16_t>(2 * core_char + 1),
            kFontCursorFgColor, kFontCursorFgColor, kFontCursorFgColor, kFontCursorBgColor, kFontCursorBgColor, kFontCursorBgColor });
    }
...
uint32_t XCursorLoader::GetPreferredCursorSize() const
{
    ...
    return std::min(screen.width_in_pixels, screen.height_in_pixels) / kScreenCursorRatio;
}
...
uint16_t XCursorLoader::CursorNamesToChar(const std::vector<std::string>& names) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void XCursorLoader::OnPropertyChanged(x11::Atom property, const x11::GetPropertyResponse& value)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    size_t size = 0;
    if (const char* resource_manager = x11::PropertyCache::GetAs<char>(value, &size)) {
...
std::vector<XCursorLoader::Image> ParseCursorFile(scoped_refptr<base::RefCountedMemory> file, uint32_t preferred_size)
{
    ...
    auto ReadBytes = [&](base::span<uint8_t> dest) {
        ...
        if (auto end = base::CheckAdd(offset, dest.size()); !end.IsValid() || end.ValueOrDie() > src.size()) {
            return false;
        }
    ...
    auto ReadU32 = [&](uint32_t& dest) {
        ...
        if (auto end = base::CheckAdd(offset, sizeof(dest)); !end.IsValid() || end.ValueOrDie() > src.size()) {
            return false;
        }
        dest = base::numerics::U32FromLittleEndian(src.subspan(offset).first<sizeof(dest)>());
        offset += sizeof(dest);
        return true;
    };
    ...
    for (const auto& entry : toc) {
        auto delta = [](uint32_t x, uint32_t y) { return std::max(x, y) - std::min(x, y); };
    ...
    for (const auto& entry : toc) {
        ...
        if (!ReadU32(chunk_header.header) || //
            ...
            chunk_header.type != entry.type || chunk_header.subtype != entry.subtype) {
            continue;
        }
        ...
        base::span<uint8_t> pixels =
            ...
            // API.
            UNSAFE_TODO(base::span(static_cast<uint8_t*>(bitmap.getPixels()), bitmap.computeByteSize()));
        if (!ReadBytes(pixels)) {
            continue;
        }
        images.push_back(XCursorLoader::Image { bitmap, gfx::Point(image.xhot, image.yhot), base::Milliseconds(image.delay) });
    }
    return images;
}
