...
XUserInputMonitor::XUserInputMonitor(const scoped_refptr<base::SingleThreadTaskRunner>& io_task_runner)
...
void XUserInputMonitor::OnEvent(const x11::Event& event)
{
    ...
    if (!raw || (raw->opcode != x11::Input::RawDeviceEvent::RawKeyPress && raw->opcode != x11::Input::RawDeviceEvent::RawKeyRelease)) {
        return;
    }

    EventType type = raw->opcode == x11::Input::RawDeviceEvent::RawKeyPress ? EventType::kKeyPressed : EventType::kKeyReleased;

    auto key_sym = connection_->KeycodeToKeysym(static_cast<x11::KeyCode>(raw->detail), 0);
    ...
    if (key_press_count_mapping_) {
        write_key_press_callback_.Run(*key_press_count_mapping_, GetKeyPressCount());
    }
}
...
void XUserInputMonitor::StartMonitor(WriteKeyPressCallback& callback)
{
    ...
    connection_->xinput().XISelectEvents({ connection_->default_root(), { { x11::Input::DeviceId::AllMaster, { mask } } } });
    connection_->Flush();

    // Register OnConnectionData() to be called every time there is something to
    // read from |connection_|.
    watch_controller_
        = base::FileDescriptorWatcher::WatchReadable(connection_->GetFd(), base::BindRepeating(&XUserInputMonitor::OnConnectionData, base::Unretained(this)));
...
void XUserInputMonitor::StartMonitorWithMapping(WriteKeyPressCallback& callback, base::WritableSharedMemoryMapping mapping)
{
