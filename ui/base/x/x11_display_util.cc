...
std::map<x11::RandR::Output, size_t> GetMonitors(const x11::Response<x11::RandR::GetMonitorsReply>& reply)
{
...
x11::Future<x11::GetPropertyReply> GetWorkAreaFuture(x11::Connection* connection)
{
...
x11::Future<x11::GetPropertyReply> GetIccProfileFuture(x11::Connection* connection, size_t monitor)
{
    std::string atom_name = monitor == 0 ? "_ICC_PROFILE" : base::StringPrintf("_ICC_PROFILE_%zu", monitor);
...
gfx::ICCProfile GetIccProfileSync(x11::Future<x11::GetPropertyReply> future)
{
    ...
    return gfx::ICCProfile::FromData(response->value->bytes(), response->value_len * response->format / 8u);
}

x11::Future<x11::RandR::GetOutputPropertyReply> GetEdidFuture(x11::Connection* connection, x11::RandR::Output output)
{
...
void ClipWorkArea(std::vector<display::Display>* displays, size_t primary_display_index, const gfx::Rect& net_workarea)
{
    ...
    auto get_work_area = [&](const display::Display& display) {
        float scale = display::Display::HasForceDeviceScaleFactor() ? display::Display::GetForcedDeviceScaleFactor() : display.device_scale_factor();
    ...
    if (base::ranges::count_if(*displays, [&](const display::Display& display) { return get_work_area(display).Contains(display.bounds()); }) == 1) {
        return;
    }
    ...
    // display.
    const auto found = base::ranges::find_if(*displays, [&](const display::Display& display) { return display.bounds().Contains(get_work_area(display)); });

    // If the work area spans multiple displays, intersect the work area with the
    // primary display, like GTK does.
    display::Display& primary = found == displays->end() ? (*displays)[primary_display_index] : *found;
...
float GetRefreshRateFromXRRModeInfo(const std::vector<x11::RandR::ModeInfo>& modes, x11::RandR::Mode current_mode_id)
{
    for (const auto& mode_info : modes) {
        ...
        return mode_info.dot_clock / static_cast<float>(mode_info.htotal * mode_info.vtotal);
    }
    return 0;
}
...
int DefaultBitsPerComponent()
{
    ...
    if (visual.c_class == x11::VisualClass::DirectColor || visual.c_class == x11::VisualClass::TrueColor) {
        ...
        auto bits = [](auto mask) { return std::bitset<sizeof(mask) * 8> { mask }.count(); };
...
std::vector<uint8_t> GetEdidProperty(x11::Response<x11::RandR::GetOutputPropertyReply> response)
{
...
float GetDisplayScale(const gfx::Rect& bounds, const display::DisplayConfig& display_config)
{
    ...
    for (const auto& geometry : display_config.display_geometries) {
        const auto dist_scale = std::make_pair(RectDistance(geometry.bounds_px, bounds), geometry.scale);
        min_dist_scale = std::min(min_dist_scale, dist_scale);
    }
    return min_dist_scale.second;
}

gfx::PointF DisplayOriginPxToDip(const display::Display& parent, const display::Display& child, const gfx::PointF& parent_origin_dip)
{
    ...
    auto map_coordinate = [&](int parent_l_px, int parent_r_px, int child_l_px, int child_r_px, float parent_l_dip) {
    ...
    const float x = map_coordinate(parent_px.x(), parent_px.right(), child_px.x(), child_px.right(), parent_origin_dip.x());
    const float y = map_coordinate(parent_px.y(), parent_px.bottom(), child_px.y(), child_px.bottom(), parent_origin_dip.y());
    return { x, y };
}

} // namespace

std::vector<display::Display> GetFallbackDisplayList(float scale, size_t* primary_display_index_out)
{
    auto* connection = x11::Connection::Get();
    const auto& screen = connection->default_screen();
    gfx::Size physical_size(screen.width_in_millimeters, screen.height_in_millimeters);
    ...
    if (!display::Display::HasForceDeviceScaleFactor() && display::IsDisplaySizeValid(physical_size)) {
    ...
    *primary_display_index_out = 0;
    ...
    ClipWorkArea(&displays, *primary_display_index_out, GetWorkAreaSync(GetWorkAreaFuture(connection)));

    return displays;
}

std::vector<display::Display> BuildDisplaysFromXRandRInfo(const display::DisplayConfig& display_config, size_t* primary_display_index_out)
{
    ...
    const size_t n_iccs = monitors_reply ? std::max<size_t>(1, monitors_reply->monitors.size()) : 1;
    ...
    for (auto crtc : resources->crtcs) {
        crtc_futures.push_back(randr.GetCrtcInfo({ crtc, resources->config_timestamp }));
    }
    connection->Flush();
    ...
    for (auto output : resources->outputs) {
        output_futures.push_back(randr.GetOutputInfo({ output, resources->config_timestamp }));
    }
    connection->Flush();
    ...
    for (size_t i = 0; i < resources->outputs.size(); i++) {
        ...
        int64_t display_id = output_32 > 0xff ? 0 : edid_parser.GetIndexBasedDisplayId(output_32);
        ...
        auto process_type = command_line->GetSwitchValueASCII("type");
        ...
        auto monitor_iter = output_to_monitor.find(static_cast<x11::RandR::Output>(output_id));
        if (monitor_iter != output_to_monitor.end() && monitor_iter->second == 0) {
            monitor_order_primary_display_index = display_index;
        }

        if (!display::HasForceDisplayColorProfile()) {
            const size_t monitor = monitor_iter == output_to_monitor.end() ? 0 : monitor_iter->second;
            ...
            display.SetColorSpaces(gfx::DisplayColorSpaces(color_space, gfx::BufferFormat::BGRA_8888));
        }
        ...
        float refresh_rate = GetRefreshRateFromXRRModeInfo(resources->modes, crtc->mode);
    ...
    if (!display::Display::HasForceDeviceScaleFactor()) {
        for (auto& display : displays) {
            display.set_device_scale_factor(GetDisplayScale(display.bounds(), display_config));
        }

        ConvertDisplayBoundsToDips(&displays, *primary_display_index_out);
    }

    ClipWorkArea(&displays, *primary_display_index_out, GetWorkAreaSync(std::move(work_area_future)));
    return displays;
}
...
base::TimeDelta GetPrimaryDisplayRefreshIntervalFromXrandr()
{
    ...
    auto displays = BuildDisplaysFromXRandRInfo(display::DisplayConfig(), &primary_display_index);
...
void ConvertDisplayBoundsToDips(std::vector<display::Display>* displays, size_t primary_display_index)
{
    ...
    origins_dip[primary_display_index] = gfx::ScalePoint(gfx::PointF(primary_display.bounds().origin()), 1.0f / primary_display.device_scale_factor());
    ...
    std::priority_queue<EdgeDistance, std::vector<EdgeDistance>, std::greater<>> queue;
    ...
    auto remove_from_fringe = [&](size_t parent) {
        ...
        for (size_t child : fringe) {
            const auto dist = RectDistance(displays->at(parent).bounds(), displays->at(child).bounds());
    ...
    while (!queue.empty()) {
        ...
        if (fringe.contains(child)) {
            origins_dip[child] = DisplayOriginPxToDip(displays->at(parent), displays->at(child), origins_dip[parent]);
    ...
    for (size_t i = 0; i < displays->size(); i++) {
        ...
        gfx::SizeF size_dip = gfx::ScaleSize(gfx::SizeF(display.size()), 1.0f / display.device_scale_factor());
        gfx::Rect bounds_dip = gfx::ToEnclosingRect(gfx::RectF(origins_dip[i], size_dip));
