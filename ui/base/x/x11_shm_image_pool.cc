...
constexpr float kShmResizeShrinkThreshold = 1.0f / (kShmResizeThreshold * kShmResizeThreshold);
...
bool ShouldUseMitShm(x11::Connection* connection)
{
    ...
    if (env->GetVar("J2D_USE_MITSHM", &j2d_use_mitshm) && (j2d_use_mitshm == "0" || base::EqualsCaseInsensitiveASCII(j2d_use_mitshm, "false"))) {
        return false;
    }
...
XShmImagePool::XShmImagePool(
    x11::Connection* connection, x11::Drawable drawable, x11::VisualId visual, int depth, std::size_t frames_pending, bool enable_multibuffering)
...
bool XShmImagePool::Resize(const gfx::Size& pixel_size)
{
    ...
    std::unique_ptr<XShmImagePool, decltype(cleanup_fn)> cleanup { this, cleanup_fn };
    ...
    if (pixel_size.width() <= 0 || pixel_size.height() <= 0 || pixel_size.GetArea() <= kMinImageAreaForShmem) {
        return false;
    }
    ...
    SkImageInfo image_info = SkImageInfo::Make(pixel_size.width(), pixel_size.height(), color_type, kPremul_SkAlphaType);
    std::size_t needed_frame_bytes = image_info.computeMinByteSize();

    if (needed_frame_bytes > frame_bytes_ || needed_frame_bytes < frame_bytes_ * kShmResizeShrinkThreshold) {
        // Resize.
        Cleanup();
        ...
        for (FrameState& state : frame_states_) {
            state.shmid = shmget(IPC_PRIVATE, frame_bytes_, IPC_CREAT | SHM_R | SHM_W | (SHM_R >> 6) | (SHM_W >> 6));
            if (state.shmid < 0)
                return false;
...
void XShmImagePool::SwapBuffers(base::OnceCallback<void(const gfx::Size&)> callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void XShmImagePool::DispatchShmCompletionEvent(x11::Shm::CompletionEvent event)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
