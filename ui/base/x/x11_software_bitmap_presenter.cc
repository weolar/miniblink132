...
// static
bool X11SoftwareBitmapPresenter::CompositeBitmap(
    x11::Connection* connection, x11::Drawable widget, int x, int y, int width, int height, int depth, x11::GraphicsContext gc, const void* data)
{
    int16_t x_i16 = x;
    int16_t y_i16 = y;
    ...
    constexpr auto kAllPlanes = std::numeric_limits<decltype(x11::GetImageRequest::plane_mask)>::max();

    scoped_refptr<x11::UnsizedRefCountedMemory> bg;
    auto req = connection->GetImage({ x11::ImageFormat::ZPixmap, widget, x_i16, y_i16, w_u16, h_u16, kAllPlanes });
    if (auto reply = req.Sync()) {
        ...
        connection->ChangeGC(x11::ChangeGCRequest { .gc = gc, .subwindow_mode = x11::SubwindowMode::IncludeInferiors });
        connection->CopyArea({ widget, pixmap_id, gc, x_i16, y_i16, 0, 0, w_u16, h_u16 });
        connection->ChangeGC(x11::ChangeGCRequest { .gc = gc, .subwindow_mode = x11::SubwindowMode::ClipByChildren });

        auto pix_req = connection->GetImage({ x11::ImageFormat::ZPixmap, pixmap_id, 0, 0, w_u16, h_u16, kAllPlanes });
    ...
    SkBitmap bg_bitmap;
    SkImageInfo image_info = SkImageInfo::Make(w_u16, h_u16, kBGRA_8888_SkColorType, kPremul_SkAlphaType);
    if (!bg_bitmap.installPixels(image_info, bg->bytes(), image_info.minRowBytes())) {
        return false;
    }
    SkCanvas canvas(bg_bitmap);

    SkBitmap fg_bitmap;
    image_info = SkImageInfo::Make(width, height, kBGRA_8888_SkColorType, kPremul_SkAlphaType);
    ...
    connection->PutImage(
        { x11::ImageFormat::ZPixmap, widget, gc, w_u16, h_u16, x_i16, y_i16, 0, d_u8, x11::SizedRefCountedMemory::From(bg, size_t { w_u16 } * h_u16) });

    return true;
}

X11SoftwareBitmapPresenter::X11SoftwareBitmapPresenter(x11::Connection& connection, gfx::AcceleratedWidget widget, bool enable_multibuffering)
    ...
    , enable_multibuffering_(enable_multibuffering)
{
    ...
    shm_pool_ = std::make_unique<ui::XShmImagePool>(&connection_.get(), widget_, visual_, depth_, MaxFramesPending(), enable_multibuffering_);

    // TODO(thomasanderson): Avoid going through the X11 server to plumb this
    // property in.
    connection_->GetPropertyAs(widget_, x11::GetAtom("CHROMIUM_COMPOSITE_WINDOW"), &composite_);
}
...
void X11SoftwareBitmapPresenter::Resize(const gfx::Size& pixel_size)
{
    ...
    if (!composite_ && shm_pool_ && shm_pool_->Resize(pixel_size)) {
        ...
        SkImageInfo info = SkImageInfo::Make(viewport_pixel_size_.width(), viewport_pixel_size_.height(), color_type, kOpaque_SkAlphaType);
...
void X11SoftwareBitmapPresenter::EndPaint(const gfx::Rect& damage_rect)
{
    ...
    if (ShmPoolReady()) {
        ...
        x11::Shm::PutImageRequest put_image_request {
            .drawable = widget_,
            .gc = gc_,
            .total_width = static_cast<uint16_t>(shm_pool_->CurrentBitmap().width()),
            .total_height = static_cast<uint16_t>(shm_pool_->CurrentBitmap().height()),
    ...
    if (composite_ && CompositeBitmap(&connection_.get(), widget_, rect.x(), rect.y(), rect.width(), rect.height(), depth_, gc_, skia_pixmap.addr())) {
        // Flush now to ensure the X server gets the request as early as
        // possible to reduce frame-to-frame latency.
    ...
    DrawPixmap(connection, visual_, widget_, gc_, skia_pixmap, rect.x(), rect.y(), rect.x(), rect.y(), rect.width(), rect.height());
...
void X11SoftwareBitmapPresenter::OnSwapBuffers(SwapBuffersCallback swap_ack_callback)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
