...
template <typename T> void PostEventToWindowTreeHost(gfx::AcceleratedWidget widget, T* xevent)
{
...
X11UIControlsTestHelper::X11UIControlsTestHelper()
    ...
    , x_window_(connection_->CreateDummyWindow("Chromium X11UIControlsTestHelper Window"))
{
}
...
void X11UIControlsTestHelper::SendKeyEvents(
    gfx::AcceleratedWidget widget, ui::KeyboardCode key, int key_event_types, int accelerator_state, base::OnceClosure closure)
{
    ...
    if (press) {
        ...
        if (control) {
            SetKeycodeAndSendThenMask(widget, &xevent, XK_Control_L, x11::KeyButMask::Control);
        }
        if (shift) {
            SetKeycodeAndSendThenMask(widget, &xevent, XK_Shift_L, x11::KeyButMask::Shift);
        }
        if (alt) {
            SetKeycodeAndSendThenMask(widget, &xevent, XK_Alt_L, x11::KeyButMask::Mod1);
        }
        xevent.detail = x11::Connection::Get()->KeysymToKeycode(ui::XKeysymForWindowsKeyCode(key, shift));
    ...
    if (release) {
        ...
        PostEventToWindowTreeHost(widget, &xevent);
        if (alt) {
            UnmaskAndSetKeycodeThenSend(widget, &xevent, x11::KeyButMask::Mod1, XK_Alt_L);
        }
        if (shift) {
            UnmaskAndSetKeycodeThenSend(widget, &xevent, x11::KeyButMask::Shift, XK_Shift_L);
        }
        if (control) {
            UnmaskAndSetKeycodeThenSend(widget, &xevent, x11::KeyButMask::Control, XK_Control_L);
        }
    }
...
void X11UIControlsTestHelper::SendMouseMotionNotifyEvent(
    gfx::AcceleratedWidget widget, const gfx::Point& mouse_loc, const gfx::Point& mouse_loc_in_connection_px, base::OnceClosure closure)
{
...
void X11UIControlsTestHelper::SendMouseEvent(gfx::AcceleratedWidget widget, MouseButton type, int button_state, int accelerator_state,
    const gfx::Point& mouse_loc, const gfx::Point& mouse_loc_in_connection_px, base::OnceClosure closure)
{
...
void X11UIControlsTestHelper::RunClosureAfterAllPendingUIEvents(base::OnceClosure closure)
{
    if (closure.is_null())
        return;
...
void X11UIControlsTestHelper::SetKeycodeAndSendThenMask(gfx::AcceleratedWidget widget, x11::KeyEvent* xevent, uint32_t keysym, x11::KeyButMask mask)
{
...
void X11UIControlsTestHelper::UnmaskAndSetKeycodeThenSend(gfx::AcceleratedWidget widget, x11::KeyEvent* xevent, x11::KeyButMask mask, uint32_t keysym)
{
    xevent->state = static_cast<x11::KeyButMask>(static_cast<uint32_t>(xevent->state) ^ static_cast<uint32_t>(mask));
    xevent->detail = x11::Connection::Get()->KeysymToKeycode(keysym);
    PostEventToWindowTreeHost(widget, xevent);
}
