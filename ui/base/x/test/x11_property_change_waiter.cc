...
X11PropertyChangeWaiter::X11PropertyChangeWaiter(x11::Window window, const char* property)
...
void X11PropertyChangeWaiter::OnEvent(const x11::Event& x11_event)
{
    ...
    if (!wait_ || !prop || prop->window != x_window_ || prop->atom != x11::GetAtom(property_) || ShouldKeepOnWaiting()) {
        return;
    }
