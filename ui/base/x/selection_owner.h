...
public:
    SelectionOwner(x11::Connection& connection, x11::Window xwindow, x11::Atom selection_name);
    ...
    const SelectionFormatMap& selection_format_map()
    {
        return format_map_;
    }
...
private:
    ...
    struct IncrementalTransfer {
        IncrementalTransfer(x11::Window window, x11::Atom target, x11::Atom property, x11::ScopedEventSelector event_selector,
    ...
    // of |requestor|.
    bool ProcessTarget(x11::Atom target, x11::Window requestor, x11::Atom property);
    ...
    void CompleteIncrementalTransfer(std::vector<IncrementalTransfer>::iterator it);

    // Returns the incremental data transfer, if any, which was waiting for
    // |event|.
    std::vector<IncrementalTransfer>::iterator FindIncrementalTransferForEvent(const x11::PropertyNotifyEvent& event);
