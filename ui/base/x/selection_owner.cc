...
static_assert(KSelectionOwnerTimerPeriodMs <= kIncrementalTransferTimeoutMs, "timer period must be <= transfer timeout");
...
bool GetAtomPairArrayProperty(x11::Connection& connection, x11::Window window, x11::Atom property, std::vector<std::pair<x11::Atom, x11::Atom>>* value)
{
    ...
    if (!connection.GetArrayProperty(window, property, &atoms) || atoms.size() % 2 != 0) {
        return false;
    }
...
void SetSelectionOwner(x11::Window window, x11::Atom selection, x11::Time time = x11::Time::CurrentTime)
{
...
SelectionOwner::SelectionOwner(x11::Connection& connection, x11::Window x_window, x11::Atom selection_name)
...
void SelectionOwner::OnSelectionRequest(const x11::SelectionRequestEvent& request)
{
    ...
    if (requested_target == x11::GetAtom(kMultiple)) {
        ...
        if (GetAtomPairArrayProperty(connection_.get(), requestor, requested_property, &conversions)) {
            std::vector<x11::Atom> conversion_results;
            for (const std::pair<x11::Atom, x11::Atom>& conversion : conversions) {
                bool conversion_successful = ProcessTarget(conversion.first, requestor, conversion.second);
                conversion_results.push_back(conversion.first);
                conversion_results.push_back(conversion_successful ? conversion.second : x11::Atom::None);
            }

            // Set the property to indicate which conversions succeeded. This matches
            // what GTK does.
            connection_->SetArrayProperty(requestor, requested_property, x11::GetAtom(kAtomPair), conversion_results);
...
bool SelectionOwner::CanDispatchPropertyEvent(const x11::PropertyNotifyEvent& event)
{
    return event.state == x11::Property::Delete && FindIncrementalTransferForEvent(event) != incremental_transfers_.end();
}
...
bool SelectionOwner::ProcessTarget(x11::Atom target, x11::Window requestor, x11::Atom property)
{
    ...
    if (target == timestamp_atom) {
        connection_->SetProperty(requestor, property, x11::Atom::INTEGER, acquired_selection_timestamp_);
        return true;
    }
    ...
    if (target == targets_atom) {
        ...
        // types we support.
        std::vector<x11::Atom> targets = { timestamp_atom, targets_atom, save_targets_atom, multiple_atom };
        RetrieveTargets(&targets);

        connection_->SetArrayProperty(requestor, property, x11::Atom::ATOM, targets);
        return true;
    }
    ...
    if (it != format_map_.end()) {
        if (it->second->size() > GetMaxIncrementalTransferSize()) {
            ...
            connection_->SetProperty(requestor, property, x11::GetAtom(kIncr), length);
            ...
            base::TimeTicks timeout = base::TimeTicks::Now() + base::Milliseconds(kIncrementalTransferTimeoutMs);
            incremental_transfers_.emplace_back(
                requestor, target, property, connection_->ScopedSelectEvent(requestor, x11::EventMask::PropertyChange), it->second, 0, timeout);
            ...
            if (!incremental_transfer_abort_timer_.IsRunning()) {
                incremental_transfer_abort_timer_.Start(
                    FROM_HERE, base::Milliseconds(KSelectionOwnerTimerPeriodMs), this, &SelectionOwner::AbortStaleIncrementalTransfers);
            }
        } else {
            auto& mem = it->second;
...
void SelectionOwner::ProcessIncrementalTransfer(IncrementalTransfer* transfer)
{
    ...
    connection_->SetArrayProperty(transfer->window, transfer->property, transfer->target, buf);
    transfer->offset += chunk_length;
    transfer->timeout = base::TimeTicks::Now() + base::Milliseconds(kIncrementalTransferTimeoutMs);
...
void SelectionOwner::AbortStaleIncrementalTransfers()
{
    ...
    for (int i = static_cast<int>(incremental_transfers_.size()) - 1; i >= 0; --i) {
...
void SelectionOwner::CompleteIncrementalTransfer(std::vector<IncrementalTransfer>::iterator it)
{
...
std::vector<SelectionOwner::IncrementalTransfer>::iterator SelectionOwner::FindIncrementalTransferForEvent(const x11::PropertyNotifyEvent& prop)
{
    for (auto it = incremental_transfers_.begin(); it != incremental_transfers_.end(); ++it) {
...
SelectionOwner::IncrementalTransfer::IncrementalTransfer(x11::Window window, x11::Atom target, x11::Atom property, x11::ScopedEventSelector event_selector,
    const scoped_refptr<base::RefCountedMemory>& data, int offset, base::TimeTicks timeout)
    : window(window)
    , target(target)
    , property(property)
...
SelectionOwner::IncrementalTransfer::IncrementalTransfer(IncrementalTransfer&& other) = default;
