...
uint32_t X11WholeScreenMoveLoop::DispatchEvent(const ui::PlatformEvent& event)
{
    ...
    case ui::EventType::kMouseDragged: {
        ...
        if ((current_xevent.As<x11::MotionNotifyEvent>() || current_xevent.As<x11::Input::DeviceEvent>())
        ...
        delegate_->OnMouseMovement(mouse_event->root_location(), mouse_event->flags(), mouse_event->time_stamp());
...
bool X11WholeScreenMoveLoop::RunMoveLoop(
    bool can_grab_pointer, scoped_refptr<ui::X11Cursor> old_cursor, scoped_refptr<ui::X11Cursor> new_cursor, base::OnceClosure started_callback)
{
    ...
    std::unique_ptr<ui::ScopedEventDispatcher> old_dispatcher = std::move(nested_dispatcher_);
...
bool X11WholeScreenMoveLoop::GrabPointer(scoped_refptr<X11Cursor> cursor)
{
    ...
    if (ret != x11::GrabStatus::Success) {
        DLOG(ERROR) << "Grabbing pointer for dragging failed: " << GrabStatusToString(ret);
    }
    connection->Flush();
...
void X11WholeScreenMoveLoop::GrabEscKey()
{
    ...
    auto esc_keycode = connection->KeysymToKeycode(XK_Escape);
    for (auto mask : kModifiersMasks) {
        connection->GrabKey({ false, grab_input_window_, mask, esc_keycode, x11::GrabMode::Async, x11::GrabMode::Async });
    }
}

void X11WholeScreenMoveLoop::CreateDragInputWindow(x11::Connection* connection)
{
    ...
    auto event_mask = x11::EventMask::ButtonPress | x11::EventMask::ButtonRelease | x11::EventMask::PointerMotion | x11::EventMask::KeyPress
        | x11::EventMask::KeyRelease | x11::EventMask::StructureNotify;
    grab_input_window_events_ = connection->ScopedSelectEvent(grab_input_window_, event_mask);
