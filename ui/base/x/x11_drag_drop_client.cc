...
static base::LazyInstance<std::map<x11::Window, XDragDropClient*>>::Leaky g_live_client_map = LAZY_INSTANCE_INITIALIZER;
...
int XGetMaskAsEventFlags()
{
    ...
    if (auto reply = connection->QueryPointer({ connection->default_root() }).Sync()) {
        mask = reply->mask;
    }
...
{
    std::map<x11::Window, XDragDropClient*>::const_iterator it = g_live_client_map.Get().find(window);
...
XDragDropClient::XDragDropClient(XDragDropClient::Delegate* delegate, x11::Window xwindow)
    : delegate_(delegate)
    , xwindow_(xwindow)
{
    DCHECK(delegate_);
    ...
    x11::Connection::Get()->SetProperty(xwindow_, x11::GetAtom(kXdndAware), x11::Atom::ATOM, xdnd_version);
...
void XDragDropClient::CompleteXdndPosition(x11::Window source_window, const gfx::Point& screen_point)
{
    DragOperation drag_operation = PreferredDragOperation(delegate_->UpdateDrag(screen_point));
    ...
    xev.data.data32[1] = (drag_operation != DragOperation::kNone) ? (kWantFurtherPosEvents | kWillAcceptDrop) : 0;
    xev.data.data32[4] = static_cast<uint32_t>(DragOperationToAtom(drag_operation));
    SendXClientEvent(source_window, xev);
}

void XDragDropClient::ProcessMouseMove(const gfx::Point& screen_point, unsigned long event_time)
{
    ...
    if (target_current_window_ != x11::Window::None) {
        if (waiting_on_status_) {
            next_position_message_ = std::make_unique<std::pair<gfx::Point, unsigned long>>(screen_point, event_time);
        } else {
...
{
    DVLOG(1) << "OnXdndEnter, version " << ((event.data.data32[1] & 0xff000000) >> 24);
    ...
    ResetDragContext();
    auto* source_client = GetForWindow(static_cast<x11::Window>(event.data.data32[0]));
    DCHECK(!source_client || source_client->source_provider_);
    target_current_context_
        = std::make_unique<XDragContext>(xwindow_, event, (source_client ? source_client->source_provider_->GetFormatMap() : SelectionFormatMap()));
    ...
    if (!source_client) {
        ...
        delegate_->OnBeginForeignDrag(static_cast<x11::Window>(event.data.data32[0]));
    }
...
void XDragDropClient::OnXdndPosition(const x11::ClientMessageEvent& event)
{
    ...
    target_current_context()->OnXdndPositionMessage(this, suggested_action, source_window, time_stamp, gfx::Point(x_root_window, y_root_window));
}
...
void XDragDropClient::OnXdndStatus(const x11::ClientMessageEvent& event)
{
    ...
    if (source_state_ != SourceState::kPendingDrop && source_state_ != SourceState::kOther) {
        return;
    }
...
void XDragDropClient::OnXdndDrop(const x11::ClientMessageEvent& event)
{
    ...
    xev.data.data32[2] = static_cast<uint32_t>(DragOperationToAtom(drag_operation));
    SendXClientEvent(source_window, xev);
}
...
void XDragDropClient::OnSelectionNotify(const x11::SelectionNotifyEvent& xselection)
{
...
void XDragDropClient::InitDrag(int allowed_operations, const OSExchangeData* data)
{
    target_current_window_ = x11::Window::None;
    ...
    source_provider_ = static_cast<const XOSExchangeDataProvider*>(&data->provider());
    ...
    if (!source_provider_->file_contents_name().empty()) {
        ...
        x11::Connection::Get()->SetStringProperty(
            xwindow_, x11::GetAtom(kXdndDirectSave0), x11::GetAtom(kMimeTypeText), source_provider_->file_contents_name().AsUTF8Unsafe());
    }
    x11::Connection::Get()->SetArrayProperty(xwindow_, x11::GetAtom(kXdndActionList), x11::Atom::ATOM, actions);
}

void XDragDropClient::CleanupDrag()
{
    source_provider_ = nullptr;
    x11::Connection::Get()->DeleteProperty(xwindow_, x11::GetAtom(kXdndActionList));
    x11::Connection::Get()->DeleteProperty(xwindow_, x11::GetAtom(kXdndDirectSave0));
}

void XDragDropClient::UpdateModifierState(int flags)
{
    const int kModifiers
        = EF_SHIFT_DOWN | EF_CONTROL_DOWN | EF_ALT_DOWN | EF_COMMAND_DOWN | EF_LEFT_MOUSE_BUTTON | EF_MIDDLE_MOUSE_BUTTON | EF_RIGHT_MOUSE_BUTTON;
...
void XDragDropClient::ResetDragContext()
{
    ...
    XDragDropClient* source_client = GetForWindow(target_current_context()->source_window());
    if (!source_client) {
...
{
    end_move_loop_timer_.Start(FROM_HERE, base::Milliseconds(1000), this, &XDragDropClient::EndMoveLoop);
}
...
void XDragDropClient::HandleMouseMovement(const gfx::Point& screen_point, int flags, base::TimeTicks event_time)
{
    UpdateModifierState(flags);
    StopRepeatMouseMoveTimer();
    ProcessMouseMove(screen_point, (event_time - base::TimeTicks()).InMilliseconds());
}
...
x11::ClientMessageEvent XDragDropClient::PrepareXdndClientMessage(const char* message, x11::Window recipient) const
{
...
x11::Window XDragDropClient::FindWindowFor(const gfx::Point& screen_point)
{
    ...
    x11::Connection::Get()->GetPropertyAs(target, x11::GetAtom(kXdndProxy), &target);

    uint32_t version;
    if (x11::Connection::Get()->GetPropertyAs(target, x11::GetAtom(kXdndAware), &version) && version >= kMaxXdndVersion) {
        return target;
    }
    return x11::Window::None;
}

void XDragDropClient::SendXClientEvent(x11::Window window, const x11::ClientMessageEvent& xev)
{
...
void XDragDropClient::SendXdndEnter(x11::Window dest_window, const std::vector<x11::Atom>& targets)
{
    ...
    if (targets.size() > 3) {
        xev.data.data32[1] |= 1;
        x11::Connection::Get()->SetArrayProperty(xwindow(), x11::GetAtom(kXdndTypeList), x11::Atom::ATOM, targets);
    } else {
...
void XDragDropClient::SendXdndPosition(x11::Window dest_window, const gfx::Point& screen_point, unsigned long event_time)
{
    ...
    xev.data.data32[4] = static_cast<uint32_t>(DragOperationToAtom(PreferredDragOperation(allowed_operations_)));
    SendXClientEvent(dest_window, xev);
    ...
    repeat_mouse_move_timer_.Start(
        FROM_HERE, base::Milliseconds(350), base::BindOnce(&XDragDropClient::ProcessMouseMove, base::Unretained(this), screen_point, event_time));
}
