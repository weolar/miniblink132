...
void GetPlatformExtraDisplayAttribs(EGLenum platform_type, std::vector<EGLAttrib>* attributes)
{
    // ANGLE_NULL and SwiftShader backends don't use the visual,
    // and may run without X11 where we can't get it anyway.
    if ((platform_type != EGL_PLATFORM_ANGLE_TYPE_NULL_ANGLE) && !base::Contains(*attributes, EGL_PLATFORM_ANGLE_DEVICE_TYPE_SWIFTSHADER_ANGLE)) {
        x11::VisualId visual_id;
        x11::Connection::Get()->GetOrCreateVisualManager().ChooseVisualForWindow(true, &visual_id, nullptr, nullptr, nullptr);
...
void ChoosePlatformCustomAlphaAndBufferSize(EGLint* alpha_size, EGLint* buffer_size)
{
    // If we're using ANGLE_NULL, we may not have a display, in which case we
    // can't use XVisualManager.
    if (gl::GLSurfaceEGL::GetGLDisplayEGL()->GetNativeDisplay().GetDisplay() != EGL_DEFAULT_DISPLAY) {
        uint8_t depth;
        x11::Connection::Get()->GetOrCreateVisualManager().ChooseVisualForWindow(true, nullptr, &depth, nullptr, nullptr);
        *buffer_size = depth;
