...
XOSExchangeDataProvider::XOSExchangeDataProvider(x11::Window x_window, x11::Window source_window, const SelectionFormatMap& selection)
    ...
    , selection_owner_(connection_.get(), x_window_, x11::GetAtom(kDndSelection))
{
}
...
void XOSExchangeDataProvider::RetrieveTargets(std::vector<x11::Atom>* targets) const
{
...
void XOSExchangeDataProvider::MarkRendererTaintedFromOrigin(const url::Origin& origin)
{
    format_map_.Insert(x11::GetAtom(kRendererTaint), base::MakeRefCounted<base::RefCountedString>(origin.opaque() ? std::string() : origin.Serialize()));
}
...
std::optional<url::Origin> XOSExchangeDataProvider::GetRendererTaintedOrigin() const
{
...
{
    format_map_.Insert(x11::GetAtom(kFromPrivileged), scoped_refptr<base::RefCountedMemory>(base::MakeRefCounted<base::RefCountedString>(std::string())));
}
...
void XOSExchangeDataProvider::SetString(const std::u16string& text_data)
{
    ...
    scoped_refptr<base::RefCountedMemory> mem(base::MakeRefCounted<base::RefCountedString>(base::UTF16ToUTF8(text_data)));
...
void XOSExchangeDataProvider::SetURL(const GURL& url, const std::u16string& title)
{
    ...
    if (url.is_valid()) {
        ...
        format_map_.Insert(
            x11::GetAtom(kNetscapeURL), scoped_refptr<base::RefCountedMemory>(base::MakeRefCounted<base::RefCountedString>(std::move(netscape_url))));
    }
}
...
void XOSExchangeDataProvider::SetFilenames(const std::vector<FileInfo>& filenames)
{
    ...
    scoped_refptr<base::RefCountedMemory> mem(base::MakeRefCounted<base::RefCountedString>(base::JoinString(paths, "\n")));
    format_map_.Insert(x11::GetAtom(kMimeTypeURIList), mem);
}

void XOSExchangeDataProvider::SetPickledData(const ClipboardFormatType& format, const base::Pickle& pickle)
{
...
std::optional<OSExchangeDataProvider::UrlInfo> XOSExchangeDataProvider::GetURLAndTitle(FilenameToURLPolicy policy) const
{
    ...
    if (data.IsValid()) {
        ...
        if (data.GetType() == x11::GetAtom(kMimeTypeMozillaURL)) {
            ...
            std::vector<std::u16string> tokens = base::SplitString(unparsed, u"\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
            if (tokens.size() > 0) {
                GURL url = GURL(tokens[0]);
                if (!url.is_valid()) {
                    return std::nullopt;
                }
                return UrlInfo { std::move(url), tokens.size() > 1 ? std::move(tokens[1]) : std::u16string() };
            }
            ...
            for (const std::string& token : tokens) {
                ...
                if (!test_url.SchemeIsFile() || policy == FilenameToURLPolicy::CONVERT_FILENAMES) {
...
std::optional<std::vector<GURL>> XOSExchangeDataProvider::GetURLs(FilenameToURLPolicy policy) const
{
...
std::optional<std::vector<FileInfo>> XOSExchangeDataProvider::GetFilenames() const
{
...
std::optional<base::Pickle> XOSExchangeDataProvider::GetPickledData(const ClipboardFormatType& format) const
{
    ui::SelectionData data = format_map_.Get(x11::GetAtom(format.GetName().c_str()));
    if (!data.IsValid()) {
        return std::nullopt;
    }
...
bool XOSExchangeDataProvider::HasURL(FilenameToURLPolicy policy) const
{
    ...
    if (data.IsValid()) {
        if (data.GetType() == x11::GetAtom(kMimeTypeMozillaURL)) {
            ...
            std::vector<std::string> tokens = ui::ParseURIList(data);
            for (const std::string& token : tokens) {
                if (!GURL(token).SchemeIsFile() || policy == FilenameToURLPolicy::CONVERT_FILENAMES) {
                    return true;
                }
            }
...
bool XOSExchangeDataProvider::HasCustomFormat(const ClipboardFormatType& format) const
{
...
void XOSExchangeDataProvider::SetFileContents(const base::FilePath& filename, const std::string& file_contents)
{
    ...
    //   the work.
    InsertData(x11::GetAtom(kXdndDirectSave0), scoped_refptr<base::RefCountedMemory>(base::MakeRefCounted<base::RefCountedString>(std::string("F"))));
    InsertData(x11::GetAtom(kMimeTypeOctetStream), scoped_refptr<base::RefCountedMemory>(base::MakeRefCounted<base::RefCountedString>(file_contents)));
}

std::optional<OSExchangeDataProvider::FileContentsInfo> XOSExchangeDataProvider::GetFileContents() const
{
    std::vector<char> str;
    if (!connection_->GetArrayProperty(source_window_, x11::GetAtom(kXdndDirectSave0), &str)) {
        return std::nullopt;
    }

    base::FilePath filename = base::FilePath(base::FilePath::StringPieceType(str.data(), str.size()));
    if (filename.empty()) {
        return std::nullopt;
    }
    ...
    if (data.IsValid()) {
        ...
        return FileContentsInfo { .filename = filename, .file_contents = std::move(file_contents) };
    }
    return std::nullopt;
}
...
void XOSExchangeDataProvider::SetHtml(const std::u16string& html, const GURL& base_url)
{
    std::vector<unsigned char> bytes;
...
std::optional<OSExchangeDataProvider::HtmlInfo> XOSExchangeDataProvider::GetHtml() const
{
    std::vector<x11::Atom> url_atoms;
...
void XOSExchangeDataProvider::SetDragImage(const gfx::ImageSkia& image, const gfx::Vector2d& cursor_offset)
{
    drag_image_ = image;
...
void XOSExchangeDataProvider::InsertData(x11::Atom format, const scoped_refptr<base::RefCountedMemory>& data)
{
    format_map_.Insert(format, data);
}

void XOSExchangeDataProvider::SetSource(std::unique_ptr<DataTransferEndpoint> data_source)
{
}
