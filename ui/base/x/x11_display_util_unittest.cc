...
void ConvertDisplayBoundsToDipsHelper(std::vector<DisplayGeometryTestData> test_data, size_t primary_display_index)
{
...
TEST(X11DisplayUtilTest, RectDistance)
{
    ...
    EXPECT_EQ(RectDistance({ 10, 10, 10, 10 }, { 20, 20, 10, 10 }), std::make_pair(0, 0));

    // Adjacent rects have a distance of {0, x}.
    EXPECT_EQ(RectDistance({ 10, 10, 10, 10 }, { 20, 10, 10, 10 }), std::make_pair(0, -10));
    EXPECT_EQ(RectDistance({ 20, 10, 10, 10 }, { 10, 10, 10, 10 }), std::make_pair(0, -10));
    EXPECT_EQ(RectDistance({ 10, 10, 10, 10 }, { 10, 20, 10, 10 }), std::make_pair(0, -10));
    EXPECT_EQ(RectDistance({ 10, 20, 10, 10 }, { 10, 10, 10, 10 }), std::make_pair(0, -10));

    // Identical rects have a distance of {-min(w, h), -max(w, h)}.
    EXPECT_EQ(RectDistance({ 0, 0, 10, 20 }, { 0, 0, 10, 20 }), std::make_pair(-10, -20));
    EXPECT_EQ(RectDistance({ 0, 0, 20, 10 }, { 0, 0, 20, 10 }), std::make_pair(-10, -20));

    // Separated rects have a positive distance.
    EXPECT_EQ(RectDistance({ 10, 10, 10, 10 }, { 30, 10, 10, 10 }), std::make_pair(10, -10));
    EXPECT_EQ(RectDistance({ 30, 10, 10, 10 }, { 10, 10, 10, 10 }), std::make_pair(10, -10));
}

TEST(X11DisplayUtilTest, ConvertDisplayBoundsToDips)
{
    // Single display
    ConvertDisplayBoundsToDipsHelper({ { { 0, 0, 1000, 1000 }, 1.0f, { 0, 0, 1000, 1000 } } }, 0);
    ConvertDisplayBoundsToDipsHelper({ { { 0, 0, 1000, 1000 }, 2.0f, { 0, 0, 500, 500 } } }, 0);
    ConvertDisplayBoundsToDipsHelper({ { { 0, 0, 1000, 1000 }, 0.5f, { 0, 0, 2000, 2000 } } }, 0);
    // Displays should be positioned relative to the primary display.
    ConvertDisplayBoundsToDipsHelper({ { { 1000, 1000, 1000, 1000 }, 1.0f, { 1000, 1000, 1000, 1000 } } }, 0);
    ConvertDisplayBoundsToDipsHelper({ { { 1000, 1000, 1000, 1000 }, 2.0f, { 500, 500, 500, 500 } } }, 0);
