...
bool IsX11ScreenSaverAvailable()
{
    ...
    return !base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kHeadless)
        && x11::Connection::Get()->screensaver_version() >= std::pair<uint32_t, uint32_t> { 1, 1 };
}

// Returns true if the event has event_x and event_y fields.
bool EventHasCoordinates(const x11::Event& event)
{
    return event.As<x11::KeyEvent>() || event.As<x11::ButtonEvent>() || event.As<x11::MotionNotifyEvent>() || event.As<x11::CrossingEvent>()
        || event.As<x11::Input::LegacyDeviceEvent>() || event.As<x11::Input::DeviceEvent>() || event.As<x11::Input::CrossingEvent>();
}
...
void DrawPixmap(x11::Connection* connection, x11::VisualId visual, x11::Drawable drawable, x11::GraphicsContext gc, const SkPixmap& skia_pixmap, int src_x,
    int src_y, int dst_x, int dst_y, int width, int height)
{
    ...
    int rows_per_request = (connection->MaxRequestSizeInBytes() - kPutImageExtraSize) / row_bytes;
    ...
    for (int row = 0; row < height; row += rows_per_request) {
        ...
        auto data = base::MakeRefCounted<base::RefCountedStaticMemory>(base::span(vec).subspan(row * row_bytes, n_rows * row_bytes));
        connection->PutImage({
...
{
    return x11::Connection::Get()->shm_version() > std::pair<uint32_t, uint32_t> { 0, 0 };
}

int CoalescePendingMotionEvents(const x11::Event& x11_event, x11::Event* last_event)
{
    ...
    DCHECK(!device || device->opcode == x11::Input::DeviceEvent::Motion || device->opcode == x11::Input::DeviceEvent::TouchUpdate);
    ...
    for (auto& event : conn->events()) {
        ...
        if (motion) {
            ...
            if (next_motion && next_motion->event == motion->event && next_motion->child == motion->child && next_motion->state == motion->state) {
            ...
            if (!ui::TouchFactory::GetInstance()->ShouldProcessDeviceEvent(*next_device)) {
                event = x11::Event();
                continue;
            }
            ...
            // have changed.
            if (next_device->opcode == device->opcode && !ddmx11->IsCMTGestureEvent(event) && ddmx11->GetScrollClassEventDetail(event) == SCROLL_TYPE_NO_SCROLL
                && device->event == next_device->event && device->child == next_device->child && device->detail == next_device->detail
                && device->button_mask == next_device->button_mask && device->mods.base == next_device->mods.base
                && device->mods.latched == next_device->mods.latched && device->mods.locked == next_device->mods.locked
...
{
    return x11::Connection::Get()->GetPropertyAs(GetX11RootWindow(), x11::GetAtom("_NET_CURRENT_DESKTOP"), desktop);
}

void SetHideTitlebarWhenMaximizedProperty(x11::Window window, HideTitlebarWhenMaximized property)
{
    x11::Connection::Get()->SetProperty(window, x11::GetAtom("_GTK_HIDE_TITLEBAR_WHEN_MAXIMIZED"), x11::Atom::CARDINAL, static_cast<uint32_t>(property));
}

bool GetRawBytesOfProperty(x11::Window window, x11::Atom property, scoped_refptr<base::RefCountedMemory>* out_data, x11::Atom* out_type)
{
    ...
    *out_data = UNSAFE_BUFFERS(x11::SizedRefCountedMemory::From(response->value, response->value_len * response->format / 8u));
    if (out_type) {
...
void SetWindowClassHint(x11::Connection* connection, x11::Window window, const std::string& res_name, const std::string& res_class)
{
    auto str = base::StringPrintf("%s%c%s", res_name.c_str(), '\0', res_class.c_str());
    std::vector<char> data(str.data(), str.data() + str.size() + 1);
    x11::Connection::Get()->SetArrayProperty(window, x11::Atom::WM_CLASS, x11::Atom::STRING, data);
}
...
void SetWindowRole(x11::Window window, const std::string& role)
{
    ...
    if (role.empty()) {
        ...
        x11::Connection::Get()->SetStringProperty(window, prop, x11::Atom::STRING, role);
    }
}

void SetWMSpecState(x11::Window window, bool enabled, x11::Atom state1, x11::Atom state2)
{
    SendClientMessage(window, GetX11RootWindow(), x11::GetAtom("_NET_WM_STATE"),
        { static_cast<uint32_t>(enabled ? kNetWMStateAdd : kNetWMStateRemove), static_cast<uint32_t>(state1), static_cast<uint32_t>(state2), 1, 0 });
}

void DoWMMoveResize(x11::Connection* connection, x11::Window root_window, x11::Window window, const gfx::Point& location_px, int direction)
{
    ...
    SendClientMessage(window, root_window, x11::GetAtom("_NET_WM_MOVERESIZE"),
        { static_cast<uint32_t>(location_px.x()), static_cast<uint32_t>(location_px.y()), static_cast<uint32_t>(direction), 0, 0 });
}

bool HasWMSpecProperty(const base::flat_set<x11::Atom>& properties, x11::Atom atom)
{
...
bool GetCustomFramePrefDefault()
{
    ...
    if (!x11::Connection::Get()->WmSupportsHint(x11::GetAtom("_NET_WM_MOVERESIZE"))) {
        return false;
    }
...
{
    return x11::Connection::Get()->GetPropertyAs(window, x11::GetAtom("_NET_WM_DESKTOP"), desktop);
}
...
bool IsX11WindowFullScreen(x11::Window window)
{
    ...
    if (x11::Connection::Get()->WmSupportsHint(fullscreen_atom)) {
        ...
        if (x11::Connection::Get()->GetArrayProperty(window, x11::GetAtom("_NET_WM_STATE"), &atom_properties)) {
    ...
    if (auto geometry = connection->GetGeometry(window).Sync()) {
        window_rect = gfx::Rect(geometry->x, geometry->y, geometry->width, geometry->height);
    } else {
        return false;
    }
...
SkColorType ColorTypeForVisual(x11::VisualId visual)
{
    ...
    if (!vis->visual_type->red_mask || !vis->visual_type->green_mask || !vis->visual_type->blue_mask) {
        return kUnknown_SkColorType;
    }
    for (const auto& color_info : color_infos) {
        if (vis->visual_type->red_mask == color_info.red_mask && vis->visual_type->green_mask == color_info.green_mask
            && vis->visual_type->blue_mask == color_info.blue_mask && vis->format->bits_per_pixel == color_info.bpp) {
            return color_info.color_type;
        }
    }
    LOG(ERROR) << "Unsupported visual with rgb mask 0x" << std::hex << vis->visual_type->red_mask << ", 0x" << vis->visual_type->green_mask << ", 0x"
               << vis->visual_type->blue_mask << ".  Please report this to https://crbug.com/1025266";
    return kUnknown_SkColorType;
}
...
gfx::ImageSkia GetNativeWindowIcon(intptr_t target_window_id)
{
    ...
    if (!x11::Connection::Get()->GetArrayProperty(static_cast<x11::Window>(target_window_id), x11::GetAtom("_NET_WM_ICON"), &data)) {
    ...
    size_t i = 0;
    while (i + 1 < data.size()) {
        if ((static_cast<int>(data[i] * data[i + 1]) > width * height) && (i + 1 + data[i] * data[i + 1] < data.size())) {
    ...
    for (long y = 0; y < height; ++y) {
        for (long x = 0; x < width; ++x) {
            pixels_data[result.rowBytesAsPixels() * y + x] = static_cast<uint32_t>(data[start + width * y + x]);
        }
    }
