...
GlobalMediaKeysListenerWin::GlobalMediaKeysListenerWin(MediaKeysListener::Delegate* delegate)
    : delegate_(delegate)
{
    DCHECK(delegate_);
...
bool GlobalMediaKeysListenerWin::StartWatchingMediaKey(KeyboardCode key_code)
{
    ...
    std::unique_ptr<gfx::SingletonHwndHotKeyObserver> observer = gfx::SingletonHwndHotKeyObserver::Create(
        base::BindRepeating(&GlobalMediaKeysListenerWin::OnWndProc, base::Unretained(this)), key_code, /*modifiers=*/0);
...
void GlobalMediaKeysListenerWin::OnWndProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
