...
template <typename V> class COMPONENT_EXPORT(UI_BASE) AcceleratorMap {
public:
    ...
    V* Find(const Accelerator& accelerator)
    {
        ...
        return const_cast<V*>(const_cast<const AcceleratorMap*>(this)->Find(accelerator));
    }
    ...
    {
        // Call the const implementation to avoid duplicating code.
        return const_cast<V&>(const_cast<const AcceleratorMap*>(this)->Get(accelerator));
    }
...
private:
    ...
    Accelerator RemapAcceleratorForLookup(const Accelerator& accelerator) const
    {
        ...
        if (use_positional_lookup_) {
            ...
            KeyboardCode remapped_key_code = KeycodeConverter::MapPositionalDomCodeToUSShortcutKey(accelerator.code(), accelerator.key_code());
            lookup_key_code = remapped_key_code != VKEY_UNKNOWN ? remapped_key_code : lookup_key_code;
        }

        return Accelerator(lookup_key_code, DomCode::NONE, accelerator.modifiers(), accelerator.key_state());
    }
    ...
    {
        return const_cast<V*>(const_cast<const AcceleratorMap*>(this)->FindImpl(accelerator));
    }
};
