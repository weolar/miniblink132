...
const int kModifierMask = EF_SHIFT_DOWN | EF_CONTROL_DOWN | EF_ALT_DOWN | EF_COMMAND_DOWN | EF_FUNCTION_DOWN | EF_ALTGR_DOWN;

const int kInterestingFlagsMask = kModifierMask | EF_IS_SYNTHESIZED | EF_IS_REPEAT;

std::u16string ApplyModifierToAcceleratorString(const std::u16string& accelerator, int modifier_message_id)
{
    return l10n_util::GetStringFUTF16(IDS_APP_ACCELERATOR_WITH_MODIFIER, l10n_util::GetStringUTF16(modifier_message_id), accelerator);
}

} // namespace

Accelerator::Accelerator()
    : Accelerator(VKEY_UNKNOWN, EF_NONE)
{
}

Accelerator::Accelerator(KeyboardCode key_code, int modifiers, KeyState key_state, base::TimeTicks time_stamp)
    : key_code_(key_code)
...
#if BUILDFLAG(IS_CHROMEOS)
Accelerator::Accelerator(KeyboardCode key_code, DomCode code, int modifiers, KeyState key_state, base::TimeTicks time_stamp)
    : key_code_(key_code)
    , code_(code)
    , key_state_(key_state)
    , modifiers_(modifiers & kInterestingFlagsMask)
...
Accelerator::Accelerator(const KeyEvent& key_event)
    ...
    , key_state_(key_event.type() == EventType::kKeyPressed ? KeyState::PRESSED : KeyState::RELEASED)
    ,
    ...
    , source_device_id_(key_event.source_device_id())
{
    ...
    if (key_event.key_code() == VKEY_ASSISTANT && HasRightAltProperty(key_event)) {
...
{
    return KeyEvent(key_state() == Accelerator::KeyState::PRESSED ? EventType::kKeyPressed : EventType::kKeyReleased, key_code(),
#if BUILDFLAG(IS_CHROMEOS)
        code(),
#endif
...
bool Accelerator::operator<(const Accelerator& rhs) const
{
    ...
    return std::tie(key_code_, key_state_, modifiers_with_mask) < std::tie(rhs.key_code_, rhs.key_state_, rhs_modifiers_with_mask);
}

bool Accelerator::operator==(const Accelerator& rhs) const
{
    return (key_code_ == rhs.key_code_) && (key_state_ == rhs.key_state_) && (MaskOutKeyEventFlags(modifiers_) == MaskOutKeyEventFlags(rhs.modifiers_))
...
std::u16string Accelerator::GetShortcutText() const
{
    ...
    if (shortcut.empty()) {
        ...
        if (c != 0)
            shortcut += static_cast<std::u16string::value_type>(base::ToUpperASCII(c));
#endif
    }
...
std::u16string Accelerator::ApplyLongFormModifiers(const std::u16string& shortcut) const
{
...
std::u16string Accelerator::ApplyShortFormModifiers(const std::u16string& shortcut) const
{
