...
const int kAcceleratorModifiers[] = { EF_SHIFT_DOWN, EF_CONTROL_DOWN, EF_ALT_DOWN, EF_COMMAND_DOWN };
...
TEST_F(AcceleratorManagerTest, Register)
{
    ...
    manager_.Register({ accelerator_a, accelerator_b, accelerator_c, accelerator_d }, AcceleratorManager::kNormalPriority, &target);
...
TEST_F(AcceleratorManagerTest, RegisterMultipleTarget)
{
    ...
    manager_.Register({ accelerator_a }, AcceleratorManager::kNormalPriority, &target1);
    TestAcceleratorTarget target2;
    manager_.Register({ accelerator_a }, AcceleratorManager::kNormalPriority, &target2);
...
TEST_F(AcceleratorManagerTest, Unregister)
{
    ...
    TestAcceleratorTarget target;
    const Accelerator accelerator_b(VKEY_B, EF_NONE);
    manager_.Register({ accelerator_a, accelerator_b }, AcceleratorManager::kNormalPriority, &target);
...
TEST_F(AcceleratorManagerTest, UnregisterAll)
{
    ...
    TestAcceleratorTarget target1;
    const Accelerator accelerator_b(VKEY_B, EF_NONE);
    manager_.Register({ accelerator_a, accelerator_b }, AcceleratorManager::kNormalPriority, &target1);

    const Accelerator accelerator_c(VKEY_C, EF_NONE);
    TestAcceleratorTarget target2;
    manager_.Register({ accelerator_c }, AcceleratorManager::kNormalPriority, &target2);
...
TEST_F(AcceleratorManagerTest, Process)
{
    ...
    for (size_t i = 0; i < (1 << std::size(kAcceleratorModifiers)); ++i) {
        ...
        manager_.Register({ accelerator }, AcceleratorManager::kNormalPriority, &target);
        ...
        EXPECT_FALSE(manager_.Process(GetAccelerator(VKEY_UNKNOWN, modifiers))) << i; // different vkey
        EXPECT_FALSE(manager_.Process(GetAccelerator(VKEY_B, modifiers))) << i; // different vkey
        EXPECT_FALSE(manager_.Process(GetAccelerator(VKEY_SHIFT, modifiers))) << i; // different vkey

        for (size_t test_i = 0; test_i < (1 << std::size(kAcceleratorModifiers)); ++test_i) {
            if (test_i == i)
                continue;
            const int test_modifiers = BuildAcceleratorModifier(test_i);
            const Accelerator test_accelerator(GetAccelerator(VKEY_A, test_modifiers));
            EXPECT_FALSE(manager_.Process(test_accelerator)) << " i=" << i << " test_i=" << test_i;
        }
...
TEST_F(AcceleratorManagerTest, PositionalShortcuts_AllEqual)
{
    ...
    scoped_feature_list.InitAndEnableFeature(features::kImprovedKeyboardShortcuts);
    ...
    KeyEvent event(ui::EventType::kKeyPressed, VKEY_OEM_6, ui::DomCode::BRACKET_RIGHT, ui::EF_CONTROL_DOWN, ui::DomKey::FromCharacter(']'), base::TimeTicks());
...
TEST_F(AcceleratorManagerTest, PositionalShortcuts_MatchingDomCode)
{
    ...
    manager.Register({ accelerator }, AcceleratorManager::kNormalPriority, &target);
    KeyEvent event(
        ui::EventType::kKeyPressed, VKEY_OEM_PLUS, ui::DomCode::BRACKET_RIGHT, ui::EF_CONTROL_DOWN, ui::DomKey::FromCharacter(']'), base::TimeTicks());
    const Accelerator trigger(event);
    EXPECT_TRUE(manager.IsRegistered(trigger));
    EXPECT_TRUE(manager.Process(trigger));
}
...
TEST_F(AcceleratorManagerTest, PositionalShortcuts_NotMatchingDomCode)
{
    ...
    const Accelerator accelerator(VKEY_OEM_6, EF_CONTROL_DOWN);
    manager.Register({ accelerator }, AcceleratorManager::kNormalPriority, &target);
    KeyEvent event(ui::EventType::kKeyPressed, VKEY_OEM_6, ui::DomCode::BRACKET_LEFT, ui::EF_CONTROL_DOWN, ui::DomKey::FromCharacter(']'), base::TimeTicks());
    const Accelerator trigger(event);
...
TEST_F(AcceleratorManagerTest, PositionalShortcuts_NonPositionalMatch)
{
    ...
    manager.Register({ accelerator }, AcceleratorManager::kNormalPriority, &target);
    KeyEvent event(ui::EventType::kKeyPressed, VKEY_Z, ui::DomCode::US_Y, ui::EF_CONTROL_DOWN, ui::DomKey::FromCharacter(']'), base::TimeTicks());
    const Accelerator trigger(event);
    EXPECT_TRUE(manager.IsRegistered(trigger));
    EXPECT_TRUE(manager.Process(trigger));
}
...
TEST_F(AcceleratorManagerTest, PositionalShortcuts_NonPositionalNonMatch)
{
    ...
    const Accelerator accelerator(VKEY_Z, EF_CONTROL_DOWN);
    manager.Register({ accelerator }, AcceleratorManager::kNormalPriority, &target);
    KeyEvent event(ui::EventType::kKeyPressed, VKEY_Y, ui::DomCode::US_Z, ui::EF_CONTROL_DOWN, ui::DomKey::FromCharacter(']'), base::TimeTicks());
    const Accelerator trigger(event);
    EXPECT_FALSE(manager.IsRegistered(trigger));
    EXPECT_FALSE(manager.Process(trigger));
}
