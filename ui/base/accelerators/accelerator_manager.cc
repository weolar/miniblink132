...
void AcceleratorManager::Register(const std::vector<ui::Accelerator>& accelerators, HandlerPriority priority, AcceleratorTarget* target)
{
    DCHECK(target);

    for (const ui::Accelerator& accelerator : accelerators) {
        accelerators_.GetOrInsertDefault(accelerator).RegisterWithPriority(target, priority);
    }
}

void AcceleratorManager::Unregister(const Accelerator& accelerator, AcceleratorTarget* target)
{
    DCHECK(target);
...
{
    for (auto map_iter = accelerators_.begin(); map_iter != accelerators_.end();) {
...
bool AcceleratorManager::HasPriorityHandler(const Accelerator& accelerator) const
{
    const AcceleratorTargetInfo* target_info = accelerators_.Find(accelerator);
...
AcceleratorManager::AcceleratorTargetInfo::AcceleratorTargetInfo(const AcceleratorManager::AcceleratorTargetInfo& other) = default;

AcceleratorManager::AcceleratorTargetInfo& AcceleratorManager::AcceleratorTargetInfo::operator=(const AcceleratorManager::AcceleratorTargetInfo& other)
    = default;

AcceleratorManager::AcceleratorTargetInfo::~AcceleratorTargetInfo() = default;

void AcceleratorManager::AcceleratorTargetInfo::RegisterWithPriority(AcceleratorTarget* target, HandlerPriority priority)
{
    ...
    if (priority == kHighPriority) {
        DCHECK(!has_priority_handler_) << "Only one high-priority handler can be registered";
...
bool AcceleratorManager::AcceleratorTargetInfo::Unregister(AcceleratorTarget* target)
{
    DCHECK(!targets_.empty());
...
bool AcceleratorManager::AcceleratorTargetInfo::TryProcess(const Accelerator& accelerator)
{
    DCHECK(!targets_.empty());

    for (AcceleratorTarget* target : targets_) {
        if (target->CanHandleAccelerators() && target->AcceleratorPressed(accelerator)) {
            return true;
        }
    }
...
bool AcceleratorManager::AcceleratorTargetInfo::Contains(AcceleratorTarget* target) const
{
    DCHECK(target);
