...
template <> struct EnumTraits<ui::mojom::AcceleratorKeyState, ui::Accelerator::KeyState> {
    static ui::mojom::AcceleratorKeyState ToMojom(ui::Accelerator::KeyState input)
    {
        switch (input) {
    ...
    static bool FromMojom(ui::mojom::AcceleratorKeyState input, ui::Accelerator::KeyState* out)
    {
        switch (input) {
...
template <> struct StructTraits<ui::mojom::AcceleratorDataView, ui::Accelerator> {
    ...
    static int32_t modifiers(const ui::Accelerator& p)
    {
        return p.modifiers();
    }
    ...
    static bool Read(ui::mojom::AcceleratorDataView data, ui::Accelerator* out)
    {
        ...
        if (!data.ReadTimeStamp(&time_stamp))
            return false;
        *out = ui::Accelerator(static_cast<ui::KeyboardCode>(data.key_code()), data.modifiers(), key_state, time_stamp);
        return true;
    }
};
