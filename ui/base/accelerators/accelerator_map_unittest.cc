...
bool IsValidMatch(AcceleratorMap<int>* map, const Accelerator& pressed, int expected)
{
    return nullptr != map->Find(pressed) && expected == *map->Find(pressed) && expected == map->Get(pressed);
}
...
TEST(AcceleratorMapTest, DomCodesStrippedWhenInserted)
{
    ...
    Accelerator accelerator(ui::VKEY_F, DomCode::US_F, ui::EF_ALT_DOWN | ui::EF_CONTROL_DOWN);
