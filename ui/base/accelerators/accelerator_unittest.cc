...
TEST(AcceleratorTest, MAYBE_GetShortcutText)
{
    ...
    for (const auto& key : keys) {
        std::u16string text = Accelerator(key.code, key.modifiers).GetShortcutText();
#if BUILDFLAG(IS_MAC)
...
{
    const Accelerator accelerator(VKEY_Z, EF_COMMAND_DOWN, Accelerator::KeyState::RELEASED, base::TimeTicks());
...
{
    ui::KeyEvent key_event(ui::EventType::kKeyPressed, ui::VKEY_F, ui::EF_ALT_DOWN | ui::EF_CONTROL_DOWN | ui::EF_FUNCTION_DOWN);
    Accelerator accelerator(key_event);

    EXPECT_EQ(accelerator.key_code(), ui::VKEY_F);
    EXPECT_EQ(accelerator.modifiers(), ui::EF_ALT_DOWN | ui::EF_CONTROL_DOWN | ui::EF_FUNCTION_DOWN);
}
...
public:
    ...
    {
        ui::KeyEvent key_event(ui::EventType::kKeyPressed, ui::VKEY_F, modifier_flags);
        Accelerator accelerator(key_event);
...
TEST_F(AcceleratorTestMac, ModifierFlagsShortFormRepresentation)
{
    ...
    while (modifier_flag) {
        ...
        if (Accelerator::MaskOutKeyEventFlags(modifier_flag) && modifier_flag != EF_ALTGR_DOWN) {
...
TEST(AcceleratorTest, ConversionFromKeyEvent_Ash)
{
    ...
    ui::KeyEvent key_event(ui::EventType::kKeyPressed, ui::VKEY_F, ui::EF_ALT_DOWN | ui::EF_CONTROL_DOWN);
    Accelerator accelerator(key_event);
