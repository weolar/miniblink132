...
public:
    ...
    FrameworkSpecificImplementation(const FrameworkSpecificImplementation&) = delete;
    ...
    {
        return GetInstanceFrameworkIdentifier() == T::GetFrameworkIdentifier() ? static_cast<T*>(this) : nullptr;
    }
    ...
    {
        return GetInstanceFrameworkIdentifier() == T::GetFrameworkIdentifier() ? static_cast<const T*>(this) : nullptr;
    }
...
#define DEFINE_FRAMEWORK_SPECIFIC_METADATA(ClassName)                                                                                                          \
    ...
    ui::FrameworkSpecificImplementation::FrameworkIdentifier ClassName::GetFrameworkIdentifier()                                                               \
    {                                                                                                                                                          \
        DEFINE_LOCAL_ELEMENT_IDENTIFIER_VALUE(k##ClassName##Identifier);                                                                                       \
        return k##ClassName##Identifier;                                                                                                                       \
    }                                                                                                                                                          \
    ui::FrameworkSpecificImplementation::FrameworkIdentifier ClassName::GetInstanceFrameworkIdentifier() const                                                 \
    {                                                                                                                                                          \
...
template <class BaseClass> class FrameworkSpecificRegistrationList {
public:
    ...
    private:
        ...
        explicit Iterator(typename FrameworkSpecificRegistrationList<T>::ListType::iterator it)
            : it_(it)
        {
        }
    ...
    iterator begin()
    {
        return Iterator<BaseClass>(instances_.begin());
    }
    iterator end()
    {
        return Iterator<BaseClass>(instances_.end());
    }
    BaseClass& operator[](size_type index)
    {
    ...
    template <class DerivedClass, typename... Args> void MaybeRegister(Args&&... args)
    {
        for (const auto& instance : instances_) {
            if (instance->template IsA<DerivedClass>())
                return;
        }
        instances_.push_back(std::make_unique<DerivedClass>(std::forward<Args>(args)...));
    }
...
extern void PrintTo(const FrameworkSpecificImplementation& impl, std::ostream* os);

COMPONENT_EXPORT(UI_BASE)
extern std::ostream& operator<<(std::ostream& os, const FrameworkSpecificImplementation& impl);
