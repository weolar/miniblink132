...
template <typename T> class PollingStateObserver : public StateObserver<T> {
public:
    using PollCallback = base::RepeatingCallback<T()>;

    static constexpr base::TimeDelta kDefaultPollingInterval = base::Milliseconds(100);
    ...
    // value.
    template <typename C>
    explicit PollingStateObserver(C&& poll_callback, base::TimeDelta polling_interval = kDefaultPollingInterval)
        : poll_callback_(internal::MaybeBindRepeating(std::forward<C>(poll_callback)))
        , timer_(FROM_HERE, polling_interval, base::BindRepeating(&PollingStateObserver<T>::OnPoll, base::Unretained(this)))
    {
        timer_.Reset();
    }
    ...
    // StateObserver:
    T GetStateObserverInitialState() const final
    {
        return poll_callback_.Run();
    }
...
template <typename T> class PollingElementStateObserver : public PollingStateObserver<std::optional<T>> {
public:
    ...
    // value.
    template <typename C>
    PollingElementStateObserver(ElementIdentifier identifier, std::optional<ElementContext> context, C&& poll_element_callback,
        base::TimeDelta polling_interval = PollingStateObserver<std::optional<T>>::kDefaultPollingInterval)
        : PollingStateObserver<std::optional<T>>(base::BindRepeating(
                                                     [](ElementIdentifier id, std::optional<ElementContext> ctx, PollElementCallback cb) {
                                                         auto* const el = ctx.has_value()
                                                             ? ElementTracker::GetElementTracker()->GetFirstMatchingElement(id, *ctx)
                                                             : ElementTracker::GetElementTracker()->GetElementInAnyContext(id);
                                                         return el ? std::make_optional(cb.Run(el)) : std::nullopt;
                                                     },
                                                     identifier, context, internal::MaybeBindRepeating(std::forward<C>(poll_element_callback))),
            polling_interval)
    {
    }
