...
template <typename T> class StateObserver {
public:
    ...
    // `T()` by default.
    virtual T GetStateObserverInitialState() const
    {
        return T();
    }
...
template <typename T, typename Source, typename Observer> class ObservationStateObserver : public StateObserver<T>, public Observer {
public:
...
template <typename ObserverType> class StateIdentifier final {
public:
    ...
    constexpr ElementIdentifier identifier() const
    {
        return identifier_;
    }
...
template <typename T> extern void PrintTo(StateIdentifier<T> state_identifier, std::ostream* os)
{
    *os << "StateIdentifier " << state_identifier.identifier().GetRawValue() << " [" << state_identifier.identifier().GetName() << "]";
}

template <typename T> extern std::ostream& operator<<(std::ostream& os, StateIdentifier<T> state_identifier)
{
