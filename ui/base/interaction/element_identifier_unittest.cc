...
protected:
    static intptr_t GetRawValue(ElementIdentifier id)
    {
        return id.GetRawValue();
    }
};

DEFINE_CLASS_ELEMENT_IDENTIFIER_VALUE(ElementIdentifierTest, kTestClassElementIdentifier);

TEST_F(ElementIdentifierTest, Constexpr)
{
    EXPECT_TRUE(IdEqual(kTestElementIdentifier, kTestElementIdentifier));
    EXPECT_FALSE(IdEqual(kTestClassElementIdentifier, kTestLocalElementIdentifier));
...
TEST_F(ElementIdentifierTest, FromName)
{
    ...
    EXPECT_EQ(kTestElementIdentifier, ElementIdentifier::FromName(kTestElementIdentifierName));
}
