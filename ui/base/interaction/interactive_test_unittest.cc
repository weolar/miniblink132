...
using ActionRecord = std::tuple<ActionType, ElementIdentifier, ElementContext, InteractionTestUtil::InputType>;
...
constexpr char kSetOnIncompatibleActionMessage[] = "Explicitly testing incompatibility-handling.";
...
public:
    ...
    void set_result(ActionResult result)
    {
        result_ = result;
    }

    ActionResult PressButton(TrackedElement* element, InputType input_type) override
    {
        DoAction(ActionType::kPressButton, element, input_type);
        return result_;
    }

    ActionResult SelectMenuItem(TrackedElement* element, InputType input_type) override
    {
        DoAction(ActionType::kSelectMenuItem, element, input_type);
        return result_;
    }

    ActionResult DoDefaultAction(TrackedElement* element, InputType input_type) override
    {
        DoAction(ActionType::kDoDefaultAction, element, input_type);
        return result_;
    }

    ActionResult SelectTab(TrackedElement* tab_collection, size_t index, InputType input_type) override
    {
        DoAction(ActionType::kSelectTab, tab_collection, input_type);
        return result_;
    }

    ActionResult SelectDropdownItem(TrackedElement* collection, size_t item, InputType input_type) override
    {
        DoAction(ActionType::kSelectDropdownItem, collection, input_type);
        return result_;
    }

    ActionResult EnterText(TrackedElement* element, std::u16string text, TextEntryMode mode) override
    {
...
#if !BUILDFLAG(IS_IOS)
    ActionResult SendAccelerator(TrackedElement* element, Accelerator accel) override
    {
    ...
    const std::vector<ActionRecord>& records() const
    {
        return records_;
    }

private:
    void DoAction(ActionType action_type, TrackedElement* element, InputType input_type)
    {
        records_.emplace_back(action_type, element->identifier(), element->context(), input_type);
...
public:
    InteractiveTestTest()
    {
        ...
        internal::InteractiveTestPrivate::set_interactive_test_verbs_allowed(base::PassKey<InteractiveTestTest>());
    }
...
protected:
    ...
    template <typename... C> void QueueActions(C&&... actions)
    {
        (queued_actions_.emplace_back(internal::MaybeBind(std::forward<C>(actions))), ...);
...
private:
    void MaybePostQueuedAction()
    {
        ...
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, base::BindOnce(&InteractiveTestTest::RunQueuedAction, weak_ptr_factory_.GetWeakPtr()), base::Milliseconds(100));
    }
    ...
    base::test::SingleThreadTaskEnvironment task_environment_ { base::test::SingleThreadTaskEnvironment::MainThreadType::UI };
    base::WeakPtrFactory<InteractiveTestTest> weak_ptr_factory_ { this };
};

TEST_F(InteractiveTestTest, StepsConstructsMultiStep)
{
    auto result = Steps(StepBuilder(), Steps(StepBuilder(), StepBuilder()), StepBuilder());
...
TEST_F(InteractiveTestTest, WaitInAnyContext)
{
    ...
    QueueActions([&]() { e1.Show(); }, [&]() { e2.Show(); }, [&]() { e1.Activate(); }, [&]() { e2.SendCustomEvent(kTestEvent1); }, [&]() { e1.Hide(); });

    RunTestSequenceInContext(kTestContext1,
        InAnyContext(
            Steps(WaitForShow(kTestId1), WaitForShow(kTestId2), WaitForActivate(kTestId1), WaitForEvent(kTestId2, kTestEvent1), WaitForHide(kTestId1))));
}
...
TEST_F(InteractiveTestTest, FlushInAnyContext)
{
    ...
    RunTestSequenceInContext(kTestContext1, InAnyContext(Steps(WaitForShow(kTestId1), WaitForShow(kTestId2))));
}
...
TEST_F(InteractiveTestTest, InteractionVerbs)
{
    ...
    e3.Show();
    e4.Show();
    RunTestSequenceInContext(kTestContext1, PressButton(kTestId1, InputType::kDontCare), SelectMenuItem(kTestId2, InputType::kKeyboard),
        DoDefaultAction(kTestId3, InputType::kMouse), SelectTab(kTestId4, 3U, InputType::kTouch), SelectDropdownItem(kTestId1, 2U, InputType::kDontCare),
        EnterText(kTestId2, u"The quick brown fox.", TextEntryMode::kAppend), ActivateSurface(kTestId3),
#if !BUILDFLAG(IS_IOS)
    ...
    EXPECT_THAT(simulator()->records(),
        testing::ElementsAre(ActionRecord { ActionType::kPressButton, kTestId1, kTestContext1, InputType::kDontCare },
            ActionRecord { ActionType::kSelectMenuItem, kTestId2, kTestContext1, InputType::kKeyboard },
            ActionRecord { ActionType::kDoDefaultAction, kTestId3, kTestContext1, InputType::kMouse },
            ActionRecord { ActionType::kSelectTab, kTestId4, kTestContext1, InputType::kTouch },
            ActionRecord { ActionType::kSelectDropdownItem, kTestId1, kTestContext1, InputType::kDontCare },
            ActionRecord { ActionType::kEnterText, kTestId2, kTestContext1, InputType::kKeyboard },
            ActionRecord { ActionType::kActivateSurface, kTestId3, kTestContext1, InputType::kMouse },
#if !BUILDFLAG(IS_IOS)
            ActionRecord { ActionType::kSendAccelerator, kTestId4, kTestContext1, InputType::kKeyboard },
#endif
            ActionRecord { ActionType::kConfirm, kTestId1, kTestContext1, InputType::kDontCare }));
}
...
TEST_F(InteractiveTestTest, InteractionVerbsInAnyContext)
{
    ...
    e3.Show();
    e4.Show();
    RunTestSequenceInContext(kTestContext2, InAnyContext(PressButton(kTestId1, InputType::kDontCare)),
        InAnyContext(SelectMenuItem(kTestId2, InputType::kKeyboard)), InAnyContext(DoDefaultAction(kTestId3, InputType::kMouse)),
        InAnyContext(Steps(SelectTab(kTestId4, 3U, InputType::kTouch), SelectDropdownItem(kTestId1, 2U, InputType::kDontCare),
            EnterText(kTestId2, u"The quick brown fox."), ActivateSurface(kTestId3),
#if !BUILDFLAG(IS_IOS)
            SendAccelerator(kTestId4, Accelerator()),
#endif
            Confirm(kTestId1))));
...
TEST_F(InteractiveTestTest, InteractionVerbsInSameContext)
{
    ...
    e4.Show();
    RunTestSequenceInContext(kTestContext2, InAnyContext(PressButton(kTestId1, InputType::kDontCare)),
        InSameContext(SelectMenuItem(kTestId2, InputType::kKeyboard)), InSameContext(DoDefaultAction(kTestId3, InputType::kMouse)),
        InSameContext(Steps(SelectTab(kTestId4, 3U, InputType::kTouch), SelectDropdownItem(kTestId1, 2U, InputType::kDontCare),
            EnterText(kTestId2, u"The quick brown fox."), ActivateSurface(kTestId3),
#if !BUILDFLAG(IS_IOS)
            SendAccelerator(kTestId4, Accelerator()),
#endif
...
TEST_F(InteractiveTestTest, Do)
{
    ...
    EXPECT_CALL_IN_SCOPE(f, Run, RunTestSequenceInContext(kTestContext1, Do(f.Get())));
}
...
TEST_F(InteractiveTestTest, CheckFails)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run, { EXPECT_FALSE(RunTestSequenceInContext(kTestContext1, Check(check.Get()))); });
}
...
TEST_F(InteractiveTestTest, CheckResult)
{
    ...
    RunTestSequenceInContext(kTestContext1, CheckResult(f.Get(), 2), CheckResult(f.Get(), testing::Gt(2)), CheckResult(f2.Get(), kString));
}
...
TEST_F(InteractiveTestTest, CheckResultFails)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run, { EXPECT_FALSE(RunTestSequenceInContext(kTestContext1, CheckResult(f.Get(), 3))); });
}
...
TEST_F(InteractiveTestTest, CheckVariable)
{
    ...
    RunTestSequenceInContext(kTestContext1, CheckVariable(y, 0), Do([&]() {
        ...
        CheckVariable(x, testing::Gt(0)), CheckVariable(text, kNewValue));
}
...
TEST_F(InteractiveTestTest, CheckVariableFails)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run, { EXPECT_FALSE(RunTestSequenceInContext(kTestContext1, Do([&]() { x = 1; }), CheckVariable(x, 0))); });
}
...
TEST_F(InteractiveTestTest, CheckElement)
{
    ...
    UNCALLED_MOCK_CALLBACK(base::RepeatingCallback<bool(TrackedElement * el)>, cb1);
    EXPECT_CALL(cb1, Run).WillRepeatedly([&e1](TrackedElement* el) { return el == &e1; });

    UNCALLED_MOCK_CALLBACK(base::OnceCallback<bool(TrackedElement * el)>, cb2);
    EXPECT_CALL(cb2, Run).WillOnce([&e2](TrackedElement* el) { return el == &e2; });

    e1.Show();
    e2.Show();

    RunTestSequenceInContext(kTestContext1, CheckElement(kTestId1, cb1.Get()), CheckElement(kTestId2, cb2.Get()), CheckElement(kTestId1, cb1.Get()));
}
...
TEST_F(InteractiveTestTest, CheckElementFails)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run, { EXPECT_FALSE(RunTestSequenceInContext(kTestContext1, CheckElement(kTestId1, cb1.Get()))); });
}
...
TEST_F(InteractiveTestTest, CheckElementWithMatcher)
{
    ...
    UNCALLED_MOCK_CALLBACK(base::RepeatingCallback<TrackedElement*(TrackedElement * el)>, cb1);
    ...
    EXPECT_CALL(cb2, Run).WillOnce([&e1](TrackedElement* el) { return el == &e1 ? 1 : 2; });
    ...
    RunTestSequenceInContext(kTestContext1,
        ...
        CheckElement(kTestId2, cb2.Get(), testing::Gt(1)), CheckElement(kTestId2, cb1.Get(), testing::Ne(&e1)));
}
...
TEST_F(InteractiveTestTest, CheckElementWithMatcherFails)
{
    ...
    UNCALLED_MOCK_CALLBACK(base::RepeatingCallback<int(TrackedElement * el)>, cb1);
    EXPECT_CALL(cb1, Run).WillRepeatedly([&e1](TrackedElement* el) { return el == &e1 ? 1 : 2; });
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run, { EXPECT_FALSE(RunTestSequenceInContext(kTestContext1, CheckElement(kTestId1, cb1.Get(), 2))); });
}
...
TEST_F(InteractiveTestTest, After)
{
    ...
    QueueActions([&]() { el.Show(); }, [&]() { el.Activate(); }, [&]() { el.SendCustomEvent(kTestEvent1); }, [&]() { el.SendCustomEvent(kTestEvent2); },
        [&]() { el.Hide(); });

    RunTestSequenceInContext(kTestContext1, AfterShow(kTestId1, cb1.Get()), AfterActivate(kTestId1, cb2.Get()), AfterEvent(kTestId1, kTestEvent2, cb3.Get()),
...
TEST_F(InteractiveTestTest, WaitFor)
{
    ...
    RunTestSequenceInContext(kTestContext1, WaitForShow(kTestId1), WaitForShow(kTestId2, /* transition_only_on_event =*/true), WaitForActivate(kTestId1),
        WaitForEvent(kTestId2, kTestEvent1), WaitForHide(kTestId1), WaitForHide(kTestId2, /* transition_only_on_event =*/true));
}
...
TEST_F(InteractiveTestTest, PresentOrNotPresentInAnyContext)
{
    ...
    RunTestSequenceInContext(kTestContext1, EnsurePresent(kTestId1),
        ...
        EnsureNotPresent(kTestId2), InAnyContext(EnsureNotPresent(kTestId3)), InAnyContext(EnsurePresent(kTestId2)));
}
...
TEST_F(InteractiveTestTest, WithElementFails)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run, { EXPECT_FALSE(RunTestSequenceInContext(kTestContext1, WithElement(kTestId1, callback.Get()))); });
}
...
TEST_F(InteractiveTestTest, EnsureNotPresentFails)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run, { EXPECT_FALSE(RunTestSequenceInContext(kTestContext1, EnsureNotPresent(kTestId1))); });
}
...
TEST_F(InteractiveTestTest, EnsureNotPresentInAnyContextFails)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run, { EXPECT_FALSE(RunTestSequenceInContext(kTestContext2, InAnyContext(EnsureNotPresent(kTestId1)))); });
}
...
TEST_F(InteractiveTestTest, EnsurePresentFails)
{
    UNCALLED_MOCK_CALLBACK(InteractionSequence::AbortedCallback, aborted);
    private_test_impl().set_aborted_callback_for_testing(aborted.Get());
    EXPECT_CALL_IN_SCOPE(aborted, Run, { EXPECT_FALSE(RunTestSequenceInContext(kTestContext2, InAnyContext(EnsurePresent(kTestId2)))); });
}
...
TEST_F(InteractiveTestTest, NameElementWithPointer)
{
    ...
    EXPECT_CALL_IN_SCOPE(cb, Run(&el), RunTestSequenceInContext(kTestContext1, NameElement(kName, &el), WithElement(kName, cb.Get())));
}
...
TEST_F(InteractiveTestTest, NameElementWithReference)
{
    ...
    EXPECT_CALL_IN_SCOPE(
        cb, Run(&el), RunTestSequenceInContext(kTestContext1, Do([&]() { ptr = &el; }), NameElement(kName, std::ref(ptr)), WithElement(kName, cb.Get())));
}
...
TEST_F(InteractiveTestTest, NameElementWithCallback)
{
    ...
    EXPECT_CALL_IN_SCOPE(cb, Run(&el),
        RunTestSequenceInContext(
            kTestContext1, NameElement(kName, base::BindLambdaForTesting([&]() -> TrackedElement* { return &el; })), WithElement(kName, cb.Get())));
}
...
TEST_F(InteractiveTestTest, NameElementWithContext)
{
    ...
    EXPECT_CALL_IN_SCOPE(cb, Run(&el),
        RunTestSequenceInContext(kTestContext1, InContext(kTestContext2, NameElement(kName, base::BindLambdaForTesting([&](ElementContext context) {
            return ElementTracker::GetElementTracker()->GetUniqueElement(kTestId1, context);
        }))),
...
TEST_F(InteractiveTestTest, SimulatorSucceeds_SkipOnUnsupported)
{
    ...
    private_test_impl().set_aborted_callback_for_testing(aborted.Get());
    RunTestSequenceInContext(kTestContext1, SetOnIncompatibleAction(OnIncompatibleAction::kSkipTest, kSetOnIncompatibleActionMessage), PressButton(kTestId1));
...
TEST_F(InteractiveTestTest, SimulatorSucceeds_IgnoreOnUnsupported)
{
    ...
    private_test_impl().set_aborted_callback_for_testing(aborted.Get());
    RunTestSequenceInContext(
        kTestContext1, SetOnIncompatibleAction(OnIncompatibleAction::kIgnoreAndContinue, kSetOnIncompatibleActionMessage), PressButton(kTestId1));
    EXPECT_FALSE(private_test_impl().sequence_skipped());
}
...
TEST_F(InteractiveTestTest, SimulatorFailureFails)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run, RunTestSequenceInContext(kTestContext1, PressButton(kTestId1)));
    EXPECT_FALSE(private_test_impl().sequence_skipped());
}
...
TEST_F(InteractiveTestTest, SimulatorFailureFails_SkipOnUnsupported)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run,
        RunTestSequenceInContext(
            kTestContext1, SetOnIncompatibleAction(OnIncompatibleAction::kSkipTest, kSetOnIncompatibleActionMessage), PressButton(kTestId1)));
    EXPECT_FALSE(private_test_impl().sequence_skipped());
}
...
TEST_F(InteractiveTestTest, SimulatorNotSupportedContinuesOnUnsupported)
{
    ...
    private_test_impl().set_aborted_callback_for_testing(aborted.Get());
    simulator_->set_result(ActionResult::kKnownIncompatible);
    RunTestSequenceInContext(kTestContext1, SetOnIncompatibleAction(OnIncompatibleAction::kIgnoreAndContinue, kSetOnIncompatibleActionMessage),
...
TEST_F(InteractiveTestTest, CanChangeOnIncompatibleAction)
{
    ...
    EXPECT_CALL_IN_SCOPE(aborted, Run,
        RunTestSequenceInContext(kTestContext1,
            ...
            SetOnIncompatibleAction(OnIncompatibleAction::kIgnoreAndContinue, kSetOnIncompatibleActionMessage), PressButton(kTestId1),
            // By changing the incompatible mode, the step after this one should
            // fail.
            SetOnIncompatibleAction(OnIncompatibleAction::kFailTest, ""), PressButton(kTestId1)));
}
...
TEST_F(InteractiveTestTest, SimulatorNotSupportedHaltAndSucceedOnUnsupported)
{
    ...
    simulator_->set_result(ActionResult::kKnownIncompatible);
    RunTestSequenceInContext(kTestContext1, SetOnIncompatibleAction(OnIncompatibleAction::kHaltTest, kSetOnIncompatibleActionMessage), PressButton(kTestId1),
        Do([&result]() { result = true; }));
    EXPECT_FALSE(result);
}
...
TEST_F(InteractiveTestTest, IfMatcherTrue)
{
    ...
    EXPECT_CALL(step, Run);
    RunTestSequenceInContext(e1.context(), IfMatches(condition.Get(), testing::Eq(1), Do(step.Get())));
}
...
TEST_F(InteractiveTestTest, IfImplicitMatcherTrue)
{
    ...
    EXPECT_CALL(condition, Run).WillOnce(testing::Return(1));
    EXPECT_CALL(step, Run);
    RunTestSequenceInContext(e1.context(), IfMatches(condition.Get(), 1, Do(step.Get())));
}
...
TEST_F(InteractiveTestTest, IfWithMultiStep)
{
    ...
    RunTestSequenceInContext(e1.context(), If(condition.Get(), Steps(Do(step1.Get()), Do(step2.Get()))));
}

TEST_F(InteractiveTestTest, IfElementTrue)
{
    UNCALLED_MOCK_CALLBACK(base::OnceCallback<bool(const TrackedElement*)>, condition);
    UNCALLED_MOCK_CALLBACK(base::OnceClosure, step);
    ...
    EXPECT_CALL(step, Run);
    RunTestSequenceInContext(e1.context(), IfElement(e1.identifier(), condition.Get(), Do(step.Get())));
}
...
{
    UNCALLED_MOCK_CALLBACK(base::OnceCallback<std::string(const TrackedElement*)>, condition);
    UNCALLED_MOCK_CALLBACK(base::OnceClosure, step);

    TestElement e1(kTestId1, kTestContext1);
    e1.Show();

    EXPECT_CALL(condition, Run(&e1)).WillOnce(testing::Return(std::string("foo")));
    EXPECT_CALL(step, Run);
    RunTestSequenceInContext(e1.context(), IfElementMatches(e1.identifier(), condition.Get(), "foo", Do(step.Get())));
}
...
TEST_F(InteractiveTestTest, IfElementMatchesFalse)
{
    ...
    EXPECT_CALL(condition, Run(&e1)).WillOnce(testing::Return(std::string("bar")));
    RunTestSequenceInContext(e1.context(), IfElementMatches(e1.identifier(), condition.Get(), "foo", Do(step.Get())));
}
...
TEST_F(InteractiveTestTest, IfElementWithMultiStep)
{
    ...
    EXPECT_CALL(step1, Run);
    EXPECT_CALL(step2, Run);
    RunTestSequenceInContext(e1.context(), IfElement(e1.identifier(), condition.Get(), Steps(Do(step1.Get()), Do(step2.Get()))));
}
...
TEST_F(InteractiveTestTest, IfFails)
{
    ...
    RunTestSequenceInContext(e1.context(), If(condition.Get(), Check(base::BindOnce([]() { return false; }))));
}
...
TEST_F(InteractiveTestTest, IfThenElse_OnlyRunsThen)
{
    ...
    EXPECT_CALL(condition, Run).WillOnce(testing::Return(true));
    EXPECT_CALL(a, Run);
    RunTestSequenceInContext(kTestContext1, If(condition.Get(), Do(a.Get()), Do(b.Get())));
}
...
TEST_F(InteractiveTestTest, IfThenElse_ThenFails)
{
    ...
    EXPECT_CALL(condition, Run).WillOnce(testing::Return(true));
    EXPECT_CALL(aborted, Run);
    RunTestSequenceInContext(kTestContext1, If(condition.Get(), Check(base::BindOnce([]() { return false; })), Do(base::BindOnce([]() {}))));
}
...
TEST_F(InteractiveTestTest, IfThenElse_ElseFails)
{
    ...
    EXPECT_CALL(condition, Run).WillOnce(testing::Return(false));
    EXPECT_CALL(aborted, Run);
    RunTestSequenceInContext(kTestContext1, If(condition.Get(), Do(base::BindOnce([]() {}))), Check(base::BindOnce([]() { return false; })));
}
...
TEST_F(InteractiveTestTest, InParallel)
{
    ...
    RunTestSequenceInContext(kTestContext1, InParallel(Do(seq1.Get()), Do(seq2.Get())));
}
...
TEST_F(InteractiveTestTest, InParallelMultiStep)
{
    ...
    RunTestSequenceInContext(kTestContext1, InParallel(Steps(Do(seq11.Get()), Do(seq12.Get())), Steps(Do(seq21.Get()), Do(seq22.Get()))));
}
...
TEST_F(InteractiveTestTest, InParallelAsync)
{
    ...
    RunTestSequenceInContext(kTestContext1, InParallel(AfterShow(e1.identifier(), seq1.Get()), AfterShow(e2.identifier(), seq2.Get())));
}
...
TEST_F(InteractiveTestTest, InParallelPingPong)
{
    ...
    QueueActions([&e1]() { e1.Show(); });
    EXPECT_CALL(seq1, Run(&e1)).WillOnce([&e2](TrackedElement*) { e2.Show(); });
    EXPECT_CALL(seq2, Run(&e2)).WillOnce([&e1](TrackedElement*) { e1.SendCustomEvent(kTestEvent1); });
    EXPECT_CALL(seq3, Run(&e1));
    RunTestSequenceInContext(kTestContext1,
        InParallel(
            Steps(AfterShow(e1.identifier(), seq1.Get()), AfterEvent(e1.identifier(), kTestEvent1, seq3.Get())), AfterShow(e2.identifier(), seq2.Get())));
}
...
TEST_F(InteractiveTestTest, InParallelFails)
{
    ...
    EXPECT_CALL(aborted, Run);
    RunTestSequenceInContext(e1.context(), InParallel(Do(base::DoNothing()), Check(base::BindOnce([]() { return false; }))));
}
...
TEST_F(InteractiveTestTest, AnyOf)
{
    ...
    RunTestSequenceInContext(kTestContext1, AnyOf(Do(seq1.Get()), Do(seq1.Get())));
}
...
TEST_F(InteractiveTestTest, AnyOfOneFailsOneSucceeds)
{
    ...
    EXPECT_CALL(seq1, Run).Times(1);
    RunTestSequenceInContext(kTestContext1, AnyOf(Check(base::BindOnce([]() { return false; })), Do(seq1.Get()), Do(seq1.Get())));
}
...
TEST_F(InteractiveTestTest, AnyOfAllFail)
{
    ...
    EXPECT_CALL(aborted, Run);
    RunTestSequenceInContext(e1.context(), InParallel(Check(base::BindOnce([]() { return false; })), Check(base::BindOnce([]() { return false; }))));
}
...
{
    TestElement el(kTestId1, kTestContext1);
    QueueActions([&el]() { el.Show(); }, [&el]() { el.SendCustomEvent(kTestEvent1); });

    RunTestSequenceInContext(kTestContext1,
        If([]() { return true; },
            AnyOf(std::move(WaitForEvent(kTestId1, kTestEvent1).SetMustBeVisibleAtStart(false)),
                Steps(WaitForShow(kTestId1), WaitForEvent(kTestId1, kTestEvent2)))));
}
...
TEST_F(InteractiveTestTest, Log)
{
    ...
    RunTestSequenceInContext(e1.context(), Do([&]() {
        y = 2;
        ...
        Log(
            ...
            "\nOnceCallback - should be 3: ", base::BindOnce([](int x, int* y) { return x + *y; }, x, base::Unretained(&y)),
            "\nRepeatingCallback - should be 4: ", base::BindRepeating([](int x, int* y) { return x + *y + 1; }, x, base::Unretained(&y)),
...
TEST_F(InteractiveTestTest, DumpElementsInContext)
{
    ...
    RunTestSequenceInContext(e1.context(), DumpElementsInContext(), InContext(e5.context(), DumpElementsInContext()));
}
...
TEST_F(InteractiveTestTest, ActionBindingMethods)
{
    ...
    auto repeating_callback = base::BindRepeating([]() { LOG(INFO) << "Repeating callback."; });
    int x = 1;
    int y = 2;
    RunTestSequenceInContext(e1.context(),

        // Check all of the various ways to bind methods with Do().
        Do(base::DoNothing()), Do(&DoFunction), Do(lambda), Do(std::move(once_callback)), Do(repeating_callback),
        Do([x, &y]() { LOG(INFO) << "Bound args " << x << ", " << y; }), Do(empty_callable),

        // Check various ways to verify a return value.
        Check(base::BindOnce([]() { return true; })), Check([]() { return true; }), CheckResult([x, &y]() { return x + y; }, 3), Check(callable),
        CheckResult(std::move(mutable_callable), false),
        CheckElement(
            e1.identifier(), [](TrackedElement* el) { return el; }, &e1),

        // Verify that argument list reduction works with bare callbacks.
        AfterShow(e1.identifier(), [&e1](TrackedElement* el) { EXPECT_EQ(el, &e1); }), WithElement(e1.identifier(), []() {}),
        WithElement(e1.identifier(), [](InteractionSequence*, TrackedElement*) {}));
}
...
TEST_F(InteractiveTestTest, ConditionalBindingMethods)
{
    ...
    RunTestSequenceInContext(e1.context(), If([]() { return true; }, Do(correct.Get()), Do(incorrect.Get())),
        IfMatches([x, &y]() { return x + y; }, 2, Do(incorrect.Get()), Do(correct.Get())),
        IfElement(
            e1.identifier(), [&e1](const TrackedElement* el) { return el == &e1; }, Do(correct.Get()), Do(incorrect.Get())),
        IfElementMatches(kTestId2, &CheckElementFunction, testing::Ne(nullptr), Do(incorrect.Get()), Do(correct.Get())));
}

namespace {

template <typename T> class TestObservable;

template <typename T> class TestObserver : public base::CheckedObserver {
public:
    TestObserver() = default;
    ~TestObserver() override = default;

    virtual void OnObservableValueChanged(TestObservable<T>* observable, T value)
    {
    }
...
template <typename T> class TestObservable {
public:
    explicit TestObservable(T value)
        : value_(value)
    {
    }
    ~TestObservable()
...
template <typename T> class TestStateObserver : public ObservationStateObserver<T, TestObservable<T>, TestObserver<T>> {
public:
    ...
    // TestObserver:
    void OnObservableValueChanged(TestObservable<T>* observable, T value) override
    {
...
DEFINE_LOCAL_STATE_IDENTIFIER_VALUE(TestStateObserver<std::string>, kStringTestState);
DEFINE_LOCAL_STATE_IDENTIFIER_VALUE(TestStateObserver<std::u16string>, kWStringTestState);
DEFINE_LOCAL_STATE_IDENTIFIER_VALUE(TestStateObserver<MyStruct>, kStructTestState);
...
TEST_F(InteractiveTestTest, ObserveStateFromUniquePtr)
{
    ...
    QueueActions([&observable]() { observable.SetValue(0); }, [&observable]() { observable.SetValue(3); }, [&observable]() { observable.SetValue(1); });

    RunTestSequenceInContext(kTestContext1, ObserveState(kIntTestState, std::make_unique<TestStateObserver<int>>(&observable)), WaitForState(kIntTestState, 3),
        CheckResult([&observable]() { return observable.value(); }, 3), WaitForState(kIntTestState, testing::Ne(3)),
...
{
    TestObservable<int> observable(2);
    RunTestSequenceInContext(kTestContext1, ObserveState(kIntTestState, &observable), WaitForState(kIntTestState, 2));
}
...
TEST_F(InteractiveTestTest, ObserveStateFromArgsWithReferences)
{
    ...
    QueueActions([&observable]() { observable.SetValue(0); }, [&observable]() { observable.SetValue(3); });
    RunTestSequenceInContext(kTestContext1, Do([&]() {
        target = 3;
        obs_ptr = &observable;
    }),
        ObserveState(kIntTestState, std::ref(obs_ptr)), WaitForState(kIntTestState, std::ref(target)),
...
TEST_F(InteractiveTestTest, ObserveStateFromArgsWithFunctions)
{
    TestObservable<int> observable(2);
    QueueActions([&observable]() { observable.SetValue(0); }, [&observable]() { observable.SetValue(3); });
    RunTestSequenceInContext(kTestContext1, ObserveState(kIntTestState, [&]() { return &observable; }),
        WaitForState(kIntTestState, base::BindRepeating([]() { return 3; })), CheckResult([&observable]() { return observable.value(); }, 3));
}

TEST_F(InteractiveTestTest, ObserveStateResetsOnDestruction)
{
    auto observable = std::make_unique<TestObservable<int>>(2);
    QueueActions([&observable]() { observable->SetValue(0); }, [&observable]() { observable->SetValue(3); }, [&observable]() { observable.reset(); });
    RunTestSequenceInContext(
        kTestContext1, ObserveState(kIntTestState, observable.get()), WaitForState(kIntTestState, 3), WaitForState(kIntTestState, testing::Ne(3)));
}
...
TEST_F(InteractiveTestTest, ObserveStateWithString)
{
    ...
    QueueActions([&]() { observable.SetValue(kBar); }, [&]() { observable.SetValue(kBaz); });
    RunTestSequenceInContext(
        kTestContext1, ObserveState(kStringTestState, &observable), WaitForState(kStringTestState, kBar), WaitForState(kStringTestState, testing::Eq(kBaz)));
}
...
TEST_F(InteractiveTestTest, ObserveStateWithWideString)
{
    ...
    QueueActions([&]() { observable.SetValue(kBar); }, [&]() { observable.SetValue(kBaz); });
    RunTestSequenceInContext(
        kTestContext1, ObserveState(kWStringTestState, &observable), WaitForState(kWStringTestState, kBar), WaitForState(kWStringTestState, testing::Eq(kBaz)));
}
...
TEST_F(InteractiveTestTest, ObserveStateWithStruct)
{
    ...
    RunTestSequenceInContext(
        kTestContext1, ObserveState(kStructTestState, &observable), WaitForState(kStructTestState, testing::Field(&MyStruct::my_int, 123)));
}

TEST_F(InteractiveTestTest, StopObservingState)
{
    TestObservable<int> observable(1);
    RunTestSequenceInContext(kTestContext1, ObserveState(kIntTestState, std::make_unique<TestStateObserver<int>>(&observable)), WaitForState(kIntTestState, 1),
        StopObservingState(kIntTestState), EnsureNotPresent(kIntTestState.identifier()), Check([this]() { return state_observers().empty(); }));
}

DEFINE_LOCAL_STATE_IDENTIFIER_VALUE(PollingStateObserver<int>, kPollingTestState);

TEST_F(InteractiveTestTest, PollingStateObserver)
{
    UNCALLED_MOCK_CALLBACK(PollingStateObserver<int>::PollCallback, poll_cb);
    EXPECT_CALL(poll_cb, Run).WillOnce(testing::Return(0)).WillOnce(testing::Return(1));
    RunTestSequenceInContext(kTestContext1, PollState(kPollingTestState, poll_cb.Get(), base::Milliseconds(50)), WaitForState(kPollingTestState, 1));
}
...
TEST_F(InteractiveTestTest, PollingStateCalledOnce)
{
    ...
    RunTestSequenceInContext(kTestContext1, PollState(kPollingTestState, callback.Get()), WaitForState(kPollingTestState, 1));
    EXPECT_CALL(callback, Run).Times(0);
}

DEFINE_LOCAL_STATE_IDENTIFIER_VALUE(PollingElementStateObserver<std::string>, kPollingElementTestState);

TEST_F(InteractiveTestTest, PollingElementStateObserver)
{
    UNCALLED_MOCK_CALLBACK(PollingElementStateObserver<std::string>::PollElementCallback, poll_cb);
    TestElement el(kTestId1, kTestContext1);
    ...
    RunTestSequenceInContext(kTestContext1, PollElement(kPollingElementTestState, el.identifier(), poll_cb.Get(), base::Milliseconds(50)),
...
TEST_F(InteractiveTestTest, SubsequenceHidesElement)
{
    ...
    RunTestSequenceInContext(kTestContext1, WaitForShow(el1.identifier()), InParallel(Do([&el1]() { el1.Hide(); }), WaitForShow(el2.identifier())));
}
