...
public:
    ...
    explicit InteractiveTestPrivate(std::unique_ptr<InteractionTestUtil> test_util);
    ...
    bool sequence_skipped() const
    {
        return sequence_skipped_;
    }
    ...
    // simulation.
    void HandleActionResult(InteractionSequence* seq, const TrackedElement* el, const std::string& operation_name, ActionResult result);
    ...
    bool AddStateObserver(ElementIdentifier id, ElementContext context, std::unique_ptr<Observer> state_observer);
    ...
    void set_aborted_callback_for_testing(InteractionSequence::AbortedCallback aborted_callback_for_testing)
    {
    ...
    // verbs" flag.
    template <typename T>
        requires std::same_as<T, ui::test::InteractiveTestTest> || std::same_as<T, ChromeOSTestLauncherDelegate> || std::same_as<T, InteractiveUITestSuite>
...
protected:
    ...
    // known.
    virtual DebugTreeNode DebugDumpElements(ui::ElementContext current_context) const;

    // Dumps the contents of a particular context.
    virtual DebugTreeNode DebugDumpContext(const ui::ElementContext context) const;
...
private:
    ...
    OnIncompatibleAction on_incompatible_action_ = OnIncompatibleAction::kFailTest;
...
template <typename T> class StateObserverElementT : public StateObserverElement {
public:
    // A lookup table is provided per value of `T`.
    using LookupTable = std::map<std::pair<ElementIdentifier, ElementContext>, StateObserverElementT<T>*>;
    ...
    StateObserverElementT(ElementIdentifier id, ElementContext context, std::unique_ptr<StateObserver<T>> observer)
        : StateObserverElement(id, context)
        , current_value_(observer->GetStateObserverInitialState())
        , observer_(std::move(observer))
    {
        auto& table = GetLookupTable();
        CHECK(!base::Contains(table, std::make_pair(id, context))) << "Duplicate ID + context for StateObserver not allowed: " << id << ", " << context;
        table.emplace(std::make_pair(id, context), this);
        observer_->SetStateObserverStateChangedCallback(base::BindRepeating(&StateObserverElementT::OnStateChanged, base::Unretained(this)));
    ...
    static StateObserverElementT<T>* LookupElement(ElementIdentifier id, ElementContext context, bool search_all_contexts)
    {
...
template <typename T, typename V = std::decay_t<T>> bool MatchAndExplain(std::string_view test_name, const testing::Matcher<V>& matcher, const T& value)
{
...
bool InteractiveTestPrivate::AddStateObserver(ElementIdentifier id, ElementContext context, std::unique_ptr<Observer> state_observer)
{
    CHECK(id);
    CHECK(context);
    ...
    state_observer_elements_.emplace_back(std::make_unique<StateObserverElementT<V>>(id, context, std::move(state_observer)));
    return true;
}
...
template <typename T> inline constexpr bool IsFunctionPointerValue = false;

template <typename R, typename... Args> inline constexpr bool IsFunctionPointerValue<R (*)(Args...)> = true;
...
// base::OnceCallback.
template <typename F> auto MaybeBind(F&& function)
{
    ...
    } else if constexpr ((IsCallable<F> && !HasState<F>) || IsFunctionPointer<F>) {
...
template <typename F> struct MaybeBindTypeHelper {
...
template <> struct MaybeBindTypeHelper<decltype(base::DoNothing())> {
    using ReturnType = void;
};

// Optionally converts `function` to something that is compatible with a
// base::RepeatingCallback, or returns it as-is if it's already a callback.
template <typename F> base::RepeatingCallback<typename MaybeBindTypeHelper<F>::Signature> MaybeBindRepeating(F&& function)
{
    if constexpr (IsCallable<F> && !HasState<F> && std::copy_constructible<std::decay_t<F>>) {
...
template <typename T> struct ArgsExtractor;

template <typename R, typename... Args> struct ArgsExtractor<R(Args...)> {
    using Holder = std::tuple<Args...>;
};

template <typename F> using ReturnTypeOf = MaybeBindTypeHelper<F>::ReturnType;

template <size_t N, typename F> using NthArgumentOf = std::tuple_element_t<N, typename ArgsExtractor<typename MaybeBindTypeHelper<F>::Signature>::Holder>;

// Requires that `F` resolves to some kind of callable object with call
// signature `S`.
template <typename F, typename S>
concept HasSignature = std::same_as<typename MaybeBindTypeHelper<F>::Signature, S> || std::same_as<F, decltype(base::DoNothing())>;

// Helper for `HasCompatibleSignature`; see recursive implementation below.
template <typename F, typename S> inline constexpr bool HasCompatibleSignatureValue = false;
...
template <typename F, typename R, typename A, typename... Args>
    requires HasSignature<F, R(A, Args...)> || HasCompatibleSignature<F, R(Args...)>
...
template <typename T> struct MatcherTypeHelper {
    using ActualType = T;
};
...
template <typename C>
    requires(std::same_as<std::remove_const_t<C>, char> || std::same_as<std::remove_const_t<C>, char16_t>)
...
template <typename T> using MatcherTypeFor = MatcherTypeHelper<std::decay_t<T>>::ActualType;
...
template <typename T>
concept IsMatcher = IsGtestMatcher<T> || HasMatchAndExplain<T> || base::is_instantiation<testing::PolymorphicMatcher, T>;

// Accepts any function-like object that is compatible with
// `InteractionSequence::StepCallback`.
template <typename F>
concept IsStepCallback = internal::HasCompatibleSignature<F, void(InteractionSequence*, TrackedElement*)>;

// Accepts any function-like object that can be used with `Check()` and
// `CheckResult()`.
template <typename F, typename R>
concept IsCheckCallback = internal::HasCompatibleSignature<F, R(const InteractionSequence*, const TrackedElement*)>;

// Converts an ElementSpecifier to an element ID or name and sets it onto
// `builder`.
void SpecifyElement(ui::InteractionSequence::StepBuilder& builder, ElementSpecifier element);

std::string DescribeElement(ElementSpecifier spec);

InteractionSequence::Builder BuildSubsequence(InteractiveTestPrivate::MultiStep steps);
...
template <typename Arg> auto UnwrapArgument(Arg arg)
{
