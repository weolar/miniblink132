...
public:
    ...
    // proceed.
    using SubsequenceCondition = base::OnceCallback<bool(const InteractionSequence* seq, const TrackedElement* el)>;
    ...
    using StepStartCallback = base::OnceCallback<void(InteractionSequence* sequence, TrackedElement* element)>;
    ...
    private:
        ...
        std::unique_ptr<InteractionSequence> BuildSubsequence(const Configuration* owner_config, const Step* owning_step);
    ...
    public:
        ...
        StepBuilder& SetType(StepType step_type, CustomElementEventType event_type = CustomElementEventType());
        ...
        StepBuilder& AddSubsequence(Builder subsequence, SubsequenceCondition condition = AlwaysRun());
        ...
        StepBuilder& SetStartCallback(base::OnceCallback<void(TrackedElement*)> start_callback);
    ...
    static std::unique_ptr<Step> WithInitialElement(
        TrackedElement* element, StepStartCallback start_callback = StepStartCallback(), StepEndCallback end_callback = StepEndCallback());
...
private:
    ...
    explicit InteractionSequence(std::unique_ptr<Configuration> configuration, const Step* reference_step);
    ...
    bool MatchesNameIfSpecified(const TrackedElement* element, const std::string& name) const;
    ...
    void OnSubsequenceAborted(SubsequenceHandle subsequence, const AbortedData& aborted_data);
...
COMPONENT_EXPORT(UI_BASE)
extern void PrintTo(InteractionSequence::AbortedReason reason, std::ostream* os);

COMPONENT_EXPORT(UI_BASE)
extern void PrintTo(InteractionSequence::SubsequenceMode mode, std::ostream* os);

COMPONENT_EXPORT(UI_BASE)
extern void PrintTo(InteractionSequence::StepStartMode mode, std::ostream* os);

COMPONENT_EXPORT(UI_BASE)
extern void PrintTo(const InteractionSequence::AbortedData& aborted_data, std::ostream* os);
...
COMPONENT_EXPORT(UI_BASE)
extern std::ostream& operator<<(std::ostream& os, const InteractionSequence::AbortedData& aborted_data);
