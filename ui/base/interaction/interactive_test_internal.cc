...
StateObserverElement::StateObserverElement(ElementIdentifier id, ElementContext context)
...
InteractiveTestPrivate::InteractiveTestPrivate(std::unique_ptr<InteractionTestUtil> test_util)
...
void InteractiveTestPrivate::Init(ElementContext initial_context)
{
    ...
    for (ElementContext context : ElementTracker::GetElementTracker()->GetAllContextsForTesting()) {
        MaybeAddPivotElement(context);
    }
    context_subscription_ = ElementTracker::GetElementTracker()->AddAnyElementShownCallbackForTesting(
        base::BindRepeating(&InteractiveTestPrivate::OnElementAdded, base::Unretained(this)));
}
...
void InteractiveTestPrivate::MaybeAddPivotElement(ElementContext context)
{
    if (!base::Contains(pivot_elements_, context)) {
        auto pivot = std::make_unique<TestElement>(kInteractiveTestPivotElementId, context);
...
void InteractiveTestPrivate::HandleActionResult(InteractionSequence* seq, const TrackedElement* el, const std::string& operation_name, ActionResult result)
{
    switch (result) {
    ...
    case ActionResult::kKnownIncompatible:
        LOG(WARNING) << operation_name << " failed because it is unsupported on this platform for " << *el;
        if (!on_incompatible_action_reason_.empty()) {
            LOG(WARNING) << "Unsupported action was expected: " << on_incompatible_action_reason_;
        } else {
...
TrackedElement* InteractiveTestPrivate::GetPivotElement(ElementContext context) const
{
    const auto it = pivot_elements_.find(context);
    CHECK(it != pivot_elements_.end()) << "Tried to reference non-existent context.";
    return it->second.get();
}

bool InteractiveTestPrivate::RemoveStateObserver(ElementIdentifier id, ElementContext context)
{
    using It = decltype(state_observer_elements_.begin());
    It found = state_observer_elements_.end();
    for (It it = state_observer_elements_.begin(); it != state_observer_elements_.end(); ++it) {
        auto& entry = **it;
        if (entry.identifier() == id && (!context || entry.context() == context)) {
            CHECK(found == state_observer_elements_.end()) << "RemoveStateObserver: Duplicate entries found for " << id;
            found = it;
        }
    }
...
void InteractiveTestPrivate::OnSequenceAborted(const InteractionSequence::AbortedData& data)
{
    ...
    if (sequence_skipped_) {
        ...
        if (data.aborted_reason == InteractionSequence::AbortedReason::kElementHiddenBetweenTriggerAndStepStart) {
        ...
        GTEST_FAIL() << "Interactive test failed " << data << additional_message.str();
    }
}
...
InteractiveTestPrivate::DebugTreeNode::DebugTreeNode(DebugTreeNode&&) noexcept = default;
InteractiveTestPrivate::DebugTreeNode& InteractiveTestPrivate::DebugTreeNode::operator=(DebugTreeNode&&) noexcept = default;
InteractiveTestPrivate::DebugTreeNode::~DebugTreeNode() = default;

namespace {
void PrintDebugTree(std::ostream& stream, const InteractiveTestPrivate::DebugTreeNode& node, std::string prefix, bool last)
{
    stream << prefix;
    if (prefix.empty()) {
        stream << "\n";
...
void InteractiveTestPrivate::DebugTreeNode::PrintTo(std::ostream& stream) const
{
    PrintDebugTree(stream, *this, "", true);
}

InteractiveTestPrivate::DebugTreeNode InteractiveTestPrivate::DebugDumpElements(ui::ElementContext current_context) const
{
...
InteractiveTestPrivate::DebugTreeNode InteractiveTestPrivate::DebugDumpContext(ui::ElementContext context) const
{
...
InteractiveTestPrivate::DebugTreeNode InteractiveTestPrivate::DebugDumpElement(const ui::TrackedElement* el) const
{
    if (el->identifier() == kInteractiveTestPivotElementId) {
        return DebugTreeNode("Pivot element (part of test automation)");
    }
    return DebugTreeNode(
        base::StringPrintf("%s - %s at %s", el->GetImplementationName(), el->identifier().GetName().c_str(), DebugDumpBounds(el->GetScreenBounds())));
}

std::string InteractiveTestPrivate::DebugDescribeContext(ui::ElementContext context) const
{
...
std::string InteractiveTestPrivate::DebugDumpBounds(const gfx::Rect& bounds) const
{
    return base::StringPrintf("x:%d-%d y:%d-%d (%dx%d)", bounds.x(), bounds.right(), bounds.y(), bounds.bottom(), bounds.width(), bounds.height());
}

void SpecifyElement(ui::InteractionSequence::StepBuilder& builder, ElementSpecifier element)
{
    std::visit(base::Overloaded {
                   [&builder](ElementIdentifier id) { builder.SetElementID(id); }, [&builder](std::string_view name) { builder.SetElementName(name); } },
        element);
}
...
{
    return std::visit(
        base::Overloaded { [](ElementIdentifier id) { return id.GetName(); }, [](std::string_view name) { return base::StringPrintf("\"%s\"", name.data()); } },
        element);
}
