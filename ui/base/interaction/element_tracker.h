...
// implementations.
class COMPONENT_EXPORT(UI_BASE) TrackedElement : public FrameworkSpecificImplementation {
public:
...
public:
    ...
    virtual void NotifyCustomEvent(TrackedElement* element, CustomElementEventType event_type) = 0;
};
...
class COMPONENT_EXPORT(UI_BASE) ElementTracker : ElementTrackerFrameworkDelegate {
public:
    ...
    // given context.
    TrackedElement* GetUniqueElement(ElementIdentifier id, ElementContext context);
    ...
    TrackedElement* GetFirstMatchingElement(ElementIdentifier id, ElementContext context);
    ...
    ElementList GetAllMatchingElements(ElementIdentifier id, ElementContext context);
    ...
    Subscription AddElementShownCallback(ElementIdentifier id, ElementContext context, Callback callback);

    // Adds a callback that will be called whenever an element with identifier
    // `id` becomes visible in any context.
    Subscription AddElementShownInAnyContextCallback(ElementIdentifier id, Callback callback);

    // Adds a callback that will be called whenever an element with identifier
    // `id` in `context` is activated by the user.
    Subscription AddElementActivatedCallback(ElementIdentifier id, ElementContext context, Callback callback);

    // Adds a callback that will be called whenever an element with identifier
    // `id` is activated in any context.
    Subscription AddElementActivatedInAnyContextCallback(ElementIdentifier id, Callback callback);
    ...
    Subscription AddElementHiddenCallback(ElementIdentifier id, ElementContext context, Callback callback);
    ...
    // valid after the call, even if the same element object in its UI framework
    // is re-shown (a new TrackedElement may be generated).
    Subscription AddElementHiddenInAnyContextCallback(ElementIdentifier id, Callback callback);

    // Adds a callback that will be called whenever an event of `event_type` is
    // generated within `context` by any element.
    Subscription AddCustomEventCallback(CustomElementEventType event_type, ElementContext context, Callback callback);

    // Adds a callback that will be called whenever an event of `event_type` is
    // generated within any context.
    Subscription AddCustomEventInAnyContextCallback(CustomElementEventType event_type, Callback callback);
    ...
    // returned.
    ElementList GetAllElementsForTesting(std::optional<ElementContext> in_context = std::nullopt);
...
private:
    ...
    void NotifyCustomEvent(TrackedElement* element, CustomElementEventType event_type) override;

    ElementData* GetOrAddElementData(ElementIdentifier id, ElementContext context);
    ...
    base::RepeatingCallbackList<void(TrackedElement*)> any_element_shown_callbacks_;
...
#define DECLARE_CUSTOM_ELEMENT_EVENT_TYPE(EventName) DECLARE_ELEMENT_IDENTIFIER_VALUE(EventName)
#define DEFINE_CUSTOM_ELEMENT_EVENT_TYPE(EventName) DEFINE_ELEMENT_IDENTIFIER_VALUE(EventName)
...
#define DECLARE_EXPORTED_CUSTOM_ELEMENT_EVENT_TYPE(ExportName, EventName) DECLARE_EXPORTED_ELEMENT_IDENTIFIER_VALUE(ExportName, EventName)
#define DEFINE_EXPORTED_CUSTOM_ELEMENT_EVENT_TYPE(EventName) DEFINE_EXPORTED_ELEMENT_IDENTIFIER_VALUE(EventName)
...
// .cc file.
#define DECLARE_CLASS_CUSTOM_ELEMENT_EVENT_TYPE(EventName) DECLARE_CLASS_ELEMENT_IDENTIFIER_VALUE(EventName)
#define DEFINE_CLASS_CUSTOM_ELEMENT_EVENT_TYPE(ClassName, EventName) DEFINE_CLASS_ELEMENT_IDENTIFIER_VALUE(ClassName, EventName)
...
#define DEFINE_MACRO_CUSTOM_ELEMENT_EVENT_TYPE(File, Line, EventName) DEFINE_MACRO_ELEMENT_IDENTIFIER_VALUE(File, Line, EventName)
...
#define DEFINE_LOCAL_CUSTOM_ELEMENT_EVENT_TYPE(EventName) DEFINE_MACRO_ELEMENT_IDENTIFIER_VALUE(__FILE__, __LINE__, EventName)
