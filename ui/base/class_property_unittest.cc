...
public:
    ...
    AssignableTestProperty(int value)
        : value_(value)
    {
    }
    ...
    bool was_move_assigned() const
    {
        return was_move_assigned_;
    }
...
DEFINE_OWNED_UI_CLASS_PROPERTY_KEY(AssignableTestProperty, kAssignableKey, nullptr)
DEFINE_CASCADING_OWNED_UI_CLASS_PROPERTY_KEY(TestCascadingProperty, kCascadingOwnedKey, nullptr)
...
public:
    ...
    explicit TestPropertyHandler(TestPropertyHandler* parent)
        : parent_(parent)
    {
    }
...
protected:
    ...
    PropertyHandler* GetParentHandler() const override
    {
        return parent_;
    }
...
TEST(PropertyTest, OwnedProperty)
{
    TestProperty* p3;
    {
        ...
        TestProperty* p1 = h.SetProperty(kOwnedKey, std::make_unique<TestProperty>());
        EXPECT_EQ(p1, h.GetProperty(kOwnedKey));
        EXPECT_EQ(last_deleted, TestProperty::last_deleted());

        TestProperty* p2 = h.SetProperty(kOwnedKey, std::make_unique<TestProperty>());
...
TEST(PropertyTest, AcquireAllPropertiesFrom)
{
    ...
    TestProperty* p1 = src->SetProperty(kOwnedKey, std::make_unique<TestProperty>());
...
TEST(PropertyTest, CascadingProperties)
{
    ...
    h.SetProperty(kCascadingOwnedKey, std::make_unique<TestCascadingProperty>(&h));
