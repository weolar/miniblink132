...
{
    TRACE_EVENT2("ui", "GetBitmapDataUrl", "width", bitmap.width(), "height", bitmap.height());
    std::optional<std::vector<uint8_t>> output = gfx::PNGCodec::EncodeBGRASkBitmap(bitmap, /*discard_transparency=*/false);
...
WindowOpenDisposition GetDispositionFromClick(const base::Value::List& list, size_t start_index)
{
    ...
    return ui::DispositionFromClick(button == 1.0, alt_key, ctrl_key, meta_key, shift_key);
}
...
bool ParseFrameIndex(std::string_view identifier, int* frame_index)
{
    *frame_index = -1;
    ...
    unsigned frame = 0;
    if (!base::StringToUint(identifier.substr(0, identifier.length() - 1), &frame)) {
        DLOG(WARNING) << "Invalid frame index format: " << identifier;
        return false;
    }
...
void ParsePathAndImageSpec(const GURL& url, std::string* path, float* scale_factor, int* frame_index)
{
    *path = base::UnescapeBinaryURLComponent(url.path_piece().substr(1));
    ...
    if (pos != std::string::npos) {
        ...
        if (ParseScaleFactor(stripped_path.substr(pos + 1, stripped_path.length() - pos - 1), &factor)) {
    ...
    if (pos != std::string::npos) {
        ...
        if (ParseFrameIndex(stripped_path.substr(pos + 1, stripped_path.length() - pos - 1), &index)) {
...
void SetLoadTimeDataDefaults(const std::string& app_locale, base::Value::Dict* localized_strings)
{
...
std::string GetWebUiCssTextDefaults()
{
    ...
    return GetWebUiCssTextDefaults(resource_bundle.LoadDataResourceString(IDR_WEBUI_CSS_TEXT_DEFAULTS_CSS));
}
...
#if BUILDFLAG(IS_LINUX)
    std::string font_name = ui::ResourceBundle::GetSharedInstance().GetFont(ui::ResourceBundle::BaseFont).GetFontName();
...
std::string GetLocalizedHtml(std::string_view html_template, const base::Value::Dict& strings)
{
    // Populate $i18n{...} placeholders.
    ui::TemplateReplacements replacements;
    ui::TemplateReplacementsFromDictionaryValue(strings, &replacements);
    std::string output = ui::ReplaceTemplateExpressions(html_template, replacements);
