...
void AppendLoadTimeData(std::string* output)
{
    ...
    std::string load_time_data_src = ui::ResourceBundle::GetSharedInstance().LoadDataResourceString(IDR_WEBUI_JS_LOAD_TIME_DATA_DEPRECATED_JS);
...
std::string GetI18nTemplateHtml(std::string_view html_template, const base::Value::Dict& json)
{
...
void AppendJsonJS(const base::Value::Dict& json, std::string* output, bool from_js_module)
{
    if (from_js_module) {
