...
public:
    ...
    static std::unique_ptr<I18nSourceStream> Create(
        std::unique_ptr<SourceStream> previous, SourceStream::SourceType type, const ui::TemplateReplacements* replacements);

private:
    I18nSourceStream(std::unique_ptr<SourceStream> previous, SourceStream::SourceType type, const TemplateReplacements* replacements);

    // SourceStream implementation.
    std::string GetTypeAsString() const override;
    base::expected<size_t, net::Error> FilterData(net::IOBuffer* output_buffer, size_t output_buffer_size, net::IOBuffer* input_buffer,
        size_t input_buffer_size, size_t* consumed_bytes, bool upstream_end_reached) override;
