...
bool PageTransitionCoreTypeIs(PageTransition lhs, PageTransition rhs)
{
    ...
    return PageTransitionTypeIncludingQualifiersIs(PageTransitionStripQualifier(lhs), rhs_core);
}

bool PageTransitionTypeIncludingQualifiersIs(PageTransition lhs, PageTransition rhs)
{
...
{
    CHECK(IsValidPageTransitionType(type)) << "Invalid transition type: " << type
...
{
    return !PageTransitionCoreTypeIs(type, PAGE_TRANSITION_AUTO_SUBFRAME) && !PageTransitionCoreTypeIs(type, PAGE_TRANSITION_MANUAL_SUBFRAME);
}
