...
struct /*COMPONENT_EXPORT(UI_BASE_DATA_TRANSFER_POLICY)*/ DataTransferEndpointOptions {
...
public:
    ...
    explicit DataTransferEndpoint(const GURL& url, DataTransferEndpointOptions options = DataTransferEndpointOptions());
    // This constructor shouldn't be used if |type| == EndpointType::kUrl.
    explicit DataTransferEndpoint(EndpointType type, DataTransferEndpointOptions options = DataTransferEndpointOptions());
    ...
    bool IsUrlType() const
    {
        return type_ == EndpointType::kUrl;
    }
