...
class COMPONENT_EXPORT(UI_BASE_DATA_TRANSFER_POLICY) DataTransferPolicyController {
public:
    ...
    virtual bool IsClipboardReadAllowed(
        base::optional_ref<const DataTransferEndpoint> data_src, base::optional_ref<const DataTransferEndpoint> data_dst, std::optional<size_t> size)
        = 0;
    ...
    // data (text, image, etc...).
    virtual void PasteIfAllowed(base::optional_ref<const DataTransferEndpoint> data_src, base::optional_ref<const DataTransferEndpoint> data_dst,
        absl::variant<size_t, std::vector<base::FilePath>> pasted_content, content::RenderFrameHost* rfh, base::OnceCallback<void(bool)> paste_cb)
        = 0;
    ...
    // drop the data.
    virtual void DropIfAllowed(std::optional<DataTransferEndpoint> data_src, std::optional<DataTransferEndpoint> data_dst,
        std::optional<std::vector<FileInfo>> filenames, base::OnceClosure drop_cb)
        = 0;
