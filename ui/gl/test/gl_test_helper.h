...
public:
    ...
    static std::pair<scoped_refptr<GLSurface>, scoped_refptr<GLContext>> CreateOffscreenGLSurfaceAndContext();

#if BUILDFLAG(IS_WIN)
    // Check that |location| is inside the bounds of |bitmap| and return the color
    // at that pixel.
    static SkColor GetColorAtPoint(const SkBitmap& bitmap, const gfx::Point& location);
