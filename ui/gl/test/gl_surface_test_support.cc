...
GLDisplay* InitializeOneOffHelper(bool init_extensions)
{
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kUseGpuInTests)) {
    ...
    DCHECK(!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kUseGL)) << "kUseGL has not effect in tests";

    bool disable_gl_drawing = true;

    CHECK(gl::init::InitializeStaticGLBindingsImplementation(impl));
    GLDisplay* display = gl::init::InitializeGLOneOffPlatformImplementation(disable_gl_drawing, init_extensions, gl::GpuPreference::kDefault);
    CHECK(display);
    return display;
}
} // namespace
...
// static
GLDisplay* GLSurfaceTestSupport::InitializeOneOffImplementation(GLImplementationParts impl)
{
    DCHECK(!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kUseGL)) << "kUseGL has not effect in tests";
...
GLDisplay* GLSurfaceTestSupport::InitializeOneOffWithMockBindings()
{
    ...
    return InitializeOneOffImplementation(GLImplementationParts(kGLImplementationMockGL));
}
...
GLDisplay* GLSurfaceTestSupport::InitializeOneOffWithStubBindings()
{
    ...
    ui::OzonePlatform::InitializeForGPU(params);
#endif
    return InitializeOneOffImplementation(GLImplementationParts(kGLImplementationStubGL));
}
...
GLDisplay* GLSurfaceTestSupport::InitializeOneOffWithNullAngleBindings()
{
    ...
    ui::OzonePlatform::InitializeForGPU(params);
#endif
    auto* display = InitializeOneOffImplementation(GLImplementationParts(gl::ANGLEImplementation::kNull));
