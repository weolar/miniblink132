...
template <typename T> void rgb_to_yuv(uint8_t r, uint8_t g, uint8_t b, T* y, T* u, T* v)
{
...
UNSAFE_BUFFER_USAGE base::span<uint8_t> ToSpan_uint8(uint8_t* data, size_t start, size_t size)
{
    return UNSAFE_BUFFERS(base::span<uint8_t>(data + start, size));
}

UNSAFE_BUFFER_USAGE base::span<uint16_t> ToSpan_uint16(uint8_t* data, size_t start, size_t size)
{
...
UNSAFE_BUFFER_USAGE base::span<uint32_t> ToSpan_uint32(uint8_t* data, size_t start, size_t size)
{
...
// static
GLDisplay* GLTestSupport::InitializeGL(std::optional<GLImplementationParts> prefered_impl)
{
#if BUILDFLAG(IS_OZONE)
    ...
    std::vector<GLImplementationParts> allowed_impls = init::GetAllowedGLImplementations();
    DCHECK(!allowed_impls.empty());

    GLImplementationParts impl = prefered_impl ? *prefered_impl : allowed_impls[0];
    DCHECK(impl.IsAllowed(allowed_impls));

    GLDisplay* display = GLSurfaceTestSupport::InitializeOneOffImplementation(impl);
#if BUILDFLAG(IS_OZONE)
...
// static
void GLTestSupport::SetBufferDataToColor(
    int width, int height, int stride, int plane, gfx::BufferFormat format, base::span<const uint8_t, 4> color, uint8_t* data)
{
    switch (format) {
    ...
    case gfx::BufferFormat::BGR_565:
        ...
        for (int y = 0; y < height; ++y) {
            auto row = UNSAFE_TODO(ToSpan_uint16(data, y * stride, width));
            for (int x = 0; x < width; ++x) {
                row[x] = ((color[0] >> 3) << 11) | ((color[1] >> 2) << 5) | (color[2] >> 3);
            }
        }
        return;
