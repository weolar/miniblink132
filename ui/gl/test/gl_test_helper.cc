...
GLuint GLTestHelper::SetupFramebuffer(int width, int height)
{
    ...
    glFramebufferTexture2DEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, color_buffer_texture, 0);
    if (glCheckFramebufferStatusEXT(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        EXPECT_EQ(static_cast<GLenum>(GL_FRAMEBUFFER_COMPLETE), glCheckFramebufferStatusEXT(GL_FRAMEBUFFER)) << "Error setting up framebuffer";
...
std::pair<scoped_refptr<GLSurface>, scoped_refptr<GLContext>> GLTestHelper::CreateOffscreenGLSurfaceAndContext()
{
    scoped_refptr<GLSurface> gl_surface = init::CreateOffscreenGLSurface(gl::GLSurfaceEGL::GetGLDisplayEGL(), gfx::Size());

    scoped_refptr<GLContext> context = gl::init::CreateGLContext(nullptr, gl_surface.get(), GLContextAttribs());
...
// static
SkColor GLTestHelper::GetColorAtPoint(const SkBitmap& bitmap, const gfx::Point& location)
{
...
{
    {
        ...
        Microsoft::WRL::ComPtr<IDCompositionDevice2> dcomp_device = GetDirectCompositionDevice();
        if (dcomp_device) {
    ...
    void* bits = nullptr;
    base::win::ScopedBitmap bitmap(::CreateDIBSection(mem_hdc.Get(), reinterpret_cast<BITMAPINFO*>(&hdr), DIB_RGB_COLORS, &bits, nullptr, 0));
    ...
    CHECK(sk_bitmap.tryAllocPixels(SkImageInfo::Make(
        SkISize::Make(size.width(), size.height()), SkColorInfo(SkColorType::kBGRA_8888_SkColorType, SkAlphaType::kPremul_SkAlphaType, nullptr))));
...
// static
SkColor GLTestHelper::ReadBackWindowPixel(HWND window, const gfx::Point& point)
{
