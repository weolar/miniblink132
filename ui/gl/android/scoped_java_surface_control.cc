...
ScopedJavaSurfaceControl::ScopedJavaSurfaceControl(const base::android::JavaRef<jobject>& j_surface_control, bool release_on_destroy)
...
ScopedJavaSurfaceControl::ScopedJavaSurfaceControl(ScopedJavaSurfaceControl&& other)
...
ScopedJavaSurfaceControl& ScopedJavaSurfaceControl::operator=(ScopedJavaSurfaceControl&& other)
{
    if (this != &other) {
        DestroyIfNeeded();
        j_surface_control_ = std::move(other.j_surface_control_);
...
base::android::ScopedJavaGlobalRef<jobject> ScopedJavaSurfaceControl::TakeJavaSurfaceControl(bool& release_on_destroy)
{
...
scoped_refptr<gfx::SurfaceControl::Surface> ScopedJavaSurfaceControl::MakeSurface()
{
    if (!j_surface_control_)
        return nullptr;
    JNIEnv* env = base::android::AttachCurrentThread();
    return base::MakeRefCounted<gfx::SurfaceControl::Surface>(env, j_surface_control_);
}

void ScopedJavaSurfaceControl::DestroyIfNeeded()
{
    if (release_on_destroy_ && j_surface_control_) {
        Java_ScopedJavaSurfaceControl_releaseSurfaceControl(base::android::AttachCurrentThread(), j_surface_control_);
    }
