...
scoped_refptr<SurfaceTexture> SurfaceTexture::Create(int texture_id)
{
    ...
    if (texture_id != 0 && gl::g_current_gl_driver->ext.b_GL_ANGLE_texture_external_update) {
        GLint prev_texture = 0;
        glGetIntegerv(GL_TEXTURE_BINDING_EXTERNAL_OES, &prev_texture);
        glBindTexture(GL_TEXTURE_EXTERNAL_OES, texture_id);
        glGetTexParameteriv(GL_TEXTURE_EXTERNAL_OES, GL_TEXTURE_NATIVE_ID_ANGLE, &native_id);
        glBindTexture(GL_TEXTURE_EXTERNAL_OES, prev_texture);
    }

    JNIEnv* env = base::android::AttachCurrentThread();
    return new SurfaceTexture(Java_ChromeSurfaceTexture_Constructor(env, native_id));
}

SurfaceTexture::SurfaceTexture(const base::android::ScopedJavaLocalRef<jobject>& j_surface_texture)
{
...
void SurfaceTexture::SetFrameAvailableCallback(base::RepeatingClosure callback)
{
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_ChromeSurfaceTexture_setNativeListener(env, j_surface_texture_, reinterpret_cast<intptr_t>(new SurfaceTextureListener(std::move(callback), false)));
}

void SurfaceTexture::SetFrameAvailableCallbackOnAnyThread(base::RepeatingClosure callback)
{
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_ChromeSurfaceTexture_setNativeListener(env, j_surface_texture_, reinterpret_cast<intptr_t>(new SurfaceTextureListener(std::move(callback), true)));
}

void SurfaceTexture::UpdateTexImage()
{
    static auto* kCrashKey = base::debug::AllocateCrashKeyString("inside_surface_texture_update_tex_image", base::debug::CrashKeySize::Size256);
...
void SurfaceTexture::GetTransformMatrix(base::span<float, 16> mtx)
{
    ...
    base::android::ScopedJavaLocalRef<jfloatArray> jmatrix(env, env->NewFloatArray(16));
    Java_ChromeSurfaceTexture_getTransformMatrix(env, j_surface_texture_, jmatrix);
...
void SurfaceTexture::AttachToGLContext()
{
    ...
    int texture_id = 0;
    if (gl::g_current_gl_driver->ext.b_GL_ANGLE_texture_external_update) {
        glGetTexParameteriv(GL_TEXTURE_EXTERNAL_OES, GL_TEXTURE_NATIVE_ID_ANGLE, &texture_id);
    } else {
    ...
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_ChromeSurfaceTexture_attachToGLContext(env, j_surface_texture_, texture_id);
...
{
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_ChromeSurfaceTexture_setDefaultBufferSize(env, j_surface_texture_, width, height);
}
