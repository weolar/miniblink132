...
private:
    static constexpr ANGLEImplementation MakeANGLEImplementation(const GLImplementation gl_impl, const ANGLEImplementation angle_impl)
    {
...
GL_EXPORT std::string FilterGLExtensionList(const char* extension_list, const std::vector<std::string>& disabled_extensions);
...
GL_EXPORT void SetGLImplementationParts(const GLImplementationParts& implementation);
...
GL_EXPORT void SetSoftwareWebGLCommandLineSwitches(base::CommandLine* command_line);

// Return requested GL implementation by checking commandline. If there isn't
// gl related argument, nullopt is returned.
GL_EXPORT std::optional<GLImplementationParts> GetRequestedGLImplementationFromCommandLine(const base::CommandLine* command_line);
...
GL_EXPORT GLImplementationParts GetNamedGLImplementation(const std::string& gl_name, const std::string& angle_name);

// Get the name of a GL implementation.
GL_EXPORT const char* GetGLImplementationGLName(GLImplementationParts implementation);
GL_EXPORT const char* GetGLImplementationANGLEName(GLImplementationParts implementation);
...
GL_EXPORT gfx::ExtensionSet GetRequestableGLExtensionsFromCurrentContext(GLApi* api);

// Helpers to load a library and log error on failure.
GL_EXPORT base::NativeLibrary LoadLibraryAndPrintError(const base::FilePath::CharType* filename);
GL_EXPORT base::NativeLibrary LoadLibraryAndPrintError(const base::FilePath& filename);
