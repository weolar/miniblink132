...
std::pair<base::TimeTicks, base::TimeDelta> TryGetVSyncParamsFromDwmCompInfo()
{
    ...
    if (result == S_OK && timing_info.qpcVBlank <= LLONG_MAX && timing_info.qpcRefreshPeriod <= LLONG_MAX) {
        ...
        if (timing_info.rateRefresh.uiDenominator > 0 && timing_info.rateRefresh.uiNumerator > 0) {
            ...
            rate_interval
                = base::Microseconds(timing_info.rateRefresh.uiDenominator * base::Time::kMicrosecondsPerSecond / timing_info.rateRefresh.uiNumerator);
        }
        ...
        if (base::TimeTicks::IsHighResolution()) {
            ...
            timebase = base::TimeTicks::FromQPCValue(base::checked_cast<LONGLONG>(timing_info.qpcVBlank));
            interval = base::TimeDelta::FromQPCValue(base::checked_cast<LONGLONG>(timing_info.qpcRefreshPeriod));
...
base::TimeDelta TryGetVsyncIntervalFromDisplayConfig(gfx::AcceleratedWidget window)
{
    base::TimeDelta interval;

    HMONITOR monitor = window ? ::MonitorFromWindow(window, MONITOR_DEFAULTTONEAREST) : ::MonitorFromWindow(nullptr, MONITOR_DEFAULTTOPRIMARY);

    if (auto path_info = display::win::GetDisplayConfigPathInfo(monitor); path_info) {
        ...
        if (refresh_rate.Denominator != 0 && refresh_rate.Numerator != 0) {
            double micro_seconds = base::ClampDiv(
                base::ClampMul(base::Time::kMicrosecondsPerSecond, static_cast<double>(refresh_rate.Denominator)), static_cast<double>(refresh_rate.Numerator));
...
base::TimeDelta TryGetVSyncIntervalFromDisplaySettings(gfx::AcceleratedWidget window)
{
    base::TimeDelta interval;
    ...
    if (::GetMonitorInfo(monitor, &monitor_info)) {
        ...
        if (::EnumDisplaySettings(monitor_info.szDevice, ENUM_CURRENT_SETTINGS, &display_info) && display_info.dmDisplayFrequency > 1) {
            interval = base::Microseconds((1.0 / base::checked_cast<double>(display_info.dmDisplayFrequency)) * base::Time::kMicrosecondsPerSecond);
        }
    }
...
bool VSyncProviderWin::GetVSyncParametersIfAvailable(base::TimeTicks* out_timebase, base::TimeDelta* out_interval)
{
...
bool VSyncProviderWin::GetVSyncIntervalIfAvailable(base::TimeDelta* out_interval)
{
