...
{
    base::WaitableEvent done(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED);
    task_runner->PostTask(FROM_HERE, BindOnce(&base::WaitableEvent::Signal, Unretained(&done)));
    done.Wait();
}

void DestroyPresenter(scoped_refptr<DCompPresenter> presenter)
{
    scoped_refptr<base::TaskRunner> task_runner = presenter->GetWindowTaskRunnerForTesting();
...
Microsoft::WRL::ComPtr<ID3D11Texture2D> CreateNV12Texture(const Microsoft::WRL::ComPtr<ID3D11Device>& d3d11_device, const gfx::Size& size)
{
...
Microsoft::WRL::ComPtr<ID3D11Texture2D> CreateP010Texture(const Microsoft::WRL::ComPtr<ID3D11Device>& d3d11_device, const gfx::Size& size)
{
    D3D11_TEXTURE2D_DESC desc = {};
    desc.Width = size.width();
    desc.Height = size.height();
    ...
    // Y, U, and V should all be 160. Output color should be pink.
    std::vector<uint16_t> image_data(size.width() * size.height() * 3 / 2, 0xA000);
    D3D11_SUBRESOURCE_DATA data = {};
    data.pSysMem = (const void*)&image_data[0];
...
void ClearRect(IDCompositionSurface* surface, const gfx::Rect& update_rect, SkColor4f update_color)
{
    Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device = GetDirectCompositionD3D11Device();
    ...
    hr = d3d11_device->CreateRenderTargetView(update_texture.Get(), nullptr, &rtv);
    CHECK_EQ(S_OK, hr);

    immediate_context->ClearRenderTargetView(rtv.Get(), update_color.premul().vec());
...
DCLayerOverlayImage CreateDCompSurface(
    const gfx::Size& surface_size, SkColor4f initial_color, std::vector<std::pair<gfx::Rect, SkColor4f>> rectangles_back_to_front = {})
{
    HRESULT hr = S_OK;

    Microsoft::WRL::ComPtr<IDCompositionDevice2> dcomp_device = gl::GetDirectCompositionDevice();

    Microsoft::WRL::ComPtr<IDCompositionSurface> surface;
    hr = dcomp_device->CreateSurface(surface_size.width(), surface_size.height(), DXGI_FORMAT_B8G8R8A8_UNORM,
        initial_color.isOpaque() ? DXGI_ALPHA_MODE_IGNORE : DXGI_ALPHA_MODE_PREMULTIPLIED, &surface);
    CHECK_EQ(S_OK, hr);

    // Add a rect that initializes the whole surface to |initial_color|.
    rectangles_back_to_front.insert(rectangles_back_to_front.begin(), { gfx::Rect(surface_size), initial_color });
...
std::unique_ptr<DCLayerOverlayParams> CreateParamsFromImage(DCLayerOverlayImage image, std::optional<gfx::RectF> content_rect_override = {})
{
    auto params = std::make_unique<DCLayerOverlayParams>();
    params->content_rect = content_rect_override.value_or(gfx::RectF(image.size()));
...
public:
    DCompPresenterTestBase()
        : parent_window_(ui::GetHiddenWindow())
    {
    }
...
protected:
    void SetUp() override
    {
        enabled_features_.InitWithFeatures(enabled_features_list_, disabled_features_list_);
        display_ = gl::init::InitializeGLNoExtensionsOneOff(
            /*init_bindings=*/true, /*gpu_preference=*/gl::GpuPreference::kDefault);
        std::tie(gl_surface_, context_) = GLTestHelper::CreateOffscreenGLSurfaceAndContext();

        // These tests are assumed to run on battery.
        fake_power_monitor_source_.SetBatteryPowerStatus(base::PowerStateObserver::BatteryPowerStatus::kBatteryPower);
    ...
    scoped_refptr<DCompPresenter> CreateDCompPresenter()
    {
        ...
        scoped_refptr<DCompPresenter> presenter = base::MakeRefCounted<DCompPresenter>(settings);
    ...
    // and color.
    void InitializeRootAndScheduleRootSurface(const gfx::Size& window_size, SkColor4f initial_color)
    {
        // Schedule the root surface as a normal overlay
        auto params = CreateParamsFromImage(CreateDCompSurface(window_size, initial_color));
        params->z_order = 0;
    ...
    void PresentAndCheckSwapResult(gfx::SwapResult expected_swap_result)
    {
        ...
        presenter_->Present(base::BindOnce(
                                [](base::RepeatingClosure quit_closure, gfx::SwapResult expected_swap_result, gfx::SwapCompletionResult result) {
...
class DCompPresenterTest : public DCompPresenterTestBase, public testing::WithParamInterface<bool> {
public:
    void SetUp() override
    {
        if (GetParam()) {
...
TEST_P(DCompPresenterTest, NoPresentTwice)
{
    ...
    Microsoft::WRL::ComPtr<ID3D11Texture2D> texture = CreateNV12Texture(d3d11_device, texture_size);
    ASSERT_NE(texture, nullptr);

    {
        auto params = CreateParamsFromImage(DCLayerOverlayImage(texture_size, texture));
        params->quad_rect = gfx::Rect(100, 100);
        params->video_params.color_space = gfx::ColorSpace::CreateREC709();
        presenter_->ScheduleDCLayer(std::move(params));
    }

    Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain = presenter_->GetLayerSwapChainForTesting(0);
    ...
    EXPECT_HRESULT_SUCCEEDED(swap_chain->GetLastPresentCount(&last_present_count));
    ...
    Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain2 = presenter_->GetLayerSwapChainForTesting(0);
    ...
    Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain3 = presenter_->GetLayerSwapChainForTesting(0);
    EXPECT_HRESULT_SUCCEEDED(swap_chain3->GetLastPresentCount(&last_present_count));
...
TEST_P(DCompPresenterTest, ProtectedVideos)
{
    ...
    // Software protected video
    {
        ...
        params->quad_rect = gfx::Rect(window_size);
        params->video_params.color_space = gfx::ColorSpace::CreateREC709();
        params->video_params.protected_video_type = gfx::ProtectedVideoType::kSoftwareProtected;
...
TEST_P(DCompPresenterTest, NoBackgroundColorSurfaceForNonColorOverlays)
{
    ...
    auto root_surface = CreateParamsFromImage(CreateDCompSurface(window_size, SkColors::kBlack));
...
TEST_P(DCompPresenterTest, BackgroundColorSurfaceTrim)
{
    ...
    for (size_t num_buffers = 1; num_buffers <= kMaxSolidColorBuffers + 1; num_buffers++) {
        ...
        {
            PresentAndCheckSwapResult(gfx::SwapResult::SWAP_ACK);
            EXPECT_EQ(std::min(num_buffers, kMaxSolidColorBuffers), layer_tree->GetNumSurfacesInPoolForTesting());
        }
    }
}
...
TEST_P(DCompPresenterTest, DelegatedInkVisualAddedWithRootSurfaceVisualNull)
{
    std::unique_ptr<gfx::DelegatedInkMetadata> metadata
        = std::make_unique<gfx::DelegatedInkMetadata>(gfx::PointF(12, 12), /*diameter=*/3, SK_ColorBLACK, base::TimeTicks::Now(), gfx::RectF(10, 10, 90, 90),
            /*hovering=*/false);
...
TEST_P(DCompPresenterTest, VisualsReused)
{
    ...
    Microsoft::WRL::ComPtr<IDCompositionVisual2> visual0 = dcLayerTree->GetContentVisualForTesting(0);
    Microsoft::WRL::ComPtr<IDCompositionVisual2> visual1 = dcLayerTree->GetContentVisualForTesting(1);
...
void ScheduleDCLayer(scoped_refptr<gl::Presenter> presenter, Microsoft::WRL::ComPtr<IDXGISwapChain1> swap_chain, const gfx::Size& swap_chain_size, int z_order)
{
    auto params = std::make_unique<DCLayerOverlayParams>();
...
void CreateSwapChain(
    IDXGIFactory2* dxgi_factory, ID3D11Device* d3d11_device, const DXGI_SWAP_CHAIN_DESC1& desc, Microsoft::WRL::ComPtr<IDXGISwapChain1>& swap_chain)
{
    ASSERT_HRESULT_SUCCEEDED(dxgi_factory->CreateSwapChainForComposition(d3d11_device, &desc, nullptr, &swap_chain));
    ASSERT_TRUE(swap_chain);
}

TEST_P(DCompPresenterTest, MatchedAndUnmatchedVisualsReused)
{
    if (context_ && context_->GetVersionInfo() && context_->GetVersionInfo()->driver_vendor.find("AMD") != std::string::npos) {
    ...
    Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device = QueryD3D11DeviceObjectFromANGLE();
    ...
    ASSERT_HRESULT_SUCCEEDED(dxgi_adapter->GetParent(IID_PPV_ARGS(&dxgi_factory)));
    ...
    Microsoft::WRL::ComPtr<IDCompositionVisual2> visualRS = dc_layer_tree->GetContentVisualForTesting(0);
    EXPECT_NE(visualRS, nullptr);
    Microsoft::WRL::ComPtr<IDCompositionVisual2> visualA = dc_layer_tree->GetContentVisualForTesting(1);
    EXPECT_NE(visualA, nullptr);
    Microsoft::WRL::ComPtr<IDCompositionVisual2> visualB = dc_layer_tree->GetContentVisualForTesting(2);
    EXPECT_NE(visualB, nullptr);
    Microsoft::WRL::ComPtr<IDCompositionVisual2> visualC = dc_layer_tree->GetContentVisualForTesting(3);
    EXPECT_NE(visualC, nullptr);
    Microsoft::WRL::ComPtr<IDCompositionVisual2> visualD = dc_layer_tree->GetContentVisualForTesting(4);
    EXPECT_NE(visualD, nullptr);
    Microsoft::WRL::ComPtr<IDCompositionVisual2> visualE = dc_layer_tree->GetContentVisualForTesting(5);
    EXPECT_NE(visualE, nullptr);
    Microsoft::WRL::ComPtr<IDCompositionVisual2> visualF = dc_layer_tree->GetContentVisualForTesting(6);
    ...
    EXPECT_EQ(visualRS.Get(), dc_layer_tree->GetContentVisualForTesting(0) /*RS*/);
...
INSTANTIATE_TEST_SUITE_P(DCompPresenterTest, DCompPresenterTest, testing::Bool());
...
protected:
    ...
    void InitializeForPixelTest(
        const gfx::Size& window_size, const gfx::Size& texture_size, const gfx::Rect& content_rect, const gfx::Rect& quad_rect, const bool is_p010)
    {
        EXPECT_TRUE(presenter_->Resize(window_size, 1.0, gfx::ColorSpace(), true));
        ...
        Microsoft::WRL::ComPtr<ID3D11Texture2D> texture
            = is_p010 ? CreateP010Texture(d3d11_device, texture_size) : CreateNV12Texture(d3d11_device, texture_size);
        ASSERT_NE(texture, nullptr);
    ...
    void RunNearestNeighborTest(bool scale_via_buffer)
    {
        ...
        auto dc_layer_params = CreateParamsFromImage(CreateDCompSurface(gfx::Size(2, 2), SkColors::kBlack,
            { { gfx::Rect(0, 0, 1, 1), SkColors::kRed }, { gfx::Rect(1, 0, 1, 1), SkColors::kGreen }, { gfx::Rect(0, 1, 1, 1), SkColors::kBlue },
        ...
        if (scale_via_buffer) {
            ...
            // scaled up
            dc_layer_params->quad_rect = gfx::ToNearestRect(dc_layer_params->content_rect);
            dc_layer_params->transform = gfx::Transform::MakeScale(
                window_size.width() / dc_layer_params->quad_rect.width(), window_size.height() / dc_layer_params->quad_rect.height());
        }
        ...
        EXPECT_SKCOLOR_EQ(SK_ColorRED, GLTestHelper::GetColorAtPoint(pixels, gfx::Point(49, 49)));
        EXPECT_SKCOLOR_EQ(SK_ColorGREEN, GLTestHelper::GetColorAtPoint(pixels, gfx::Point(51, 49)));
        EXPECT_SKCOLOR_EQ(SK_ColorBLUE, GLTestHelper::GetColorAtPoint(pixels, gfx::Point(49, 51)));
        EXPECT_SKCOLOR_EQ(SK_ColorBLACK, GLTestHelper::GetColorAtPoint(pixels, gfx::Point(51, 51)));
    }
    ...
    void CheckOverlayExactlyFillsHole(
        const gfx::Size& window_size, const gfx::Rect& root_surface_hole, std::unique_ptr<DCLayerOverlayParams> fit_in_hole_overlay)
    {
        ...
        auto root_surface
            = CreateParamsFromImage(CreateDCompSurface(window_size, kRootSurfaceInitialColor, { { root_surface_hole, kRootSurfaceHiddenColor } }));
        root_surface->quad_rect = gfx::Rect(window_size);
        ...
        for (int y = 0; y < window_size.height(); y++) {
            for (int x = 0; x < window_size.width(); x++) {
                ...
                SkColor expected_color = (in_hole ? kOverlayExpectedColor : kRootSurfaceInitialColor).toSkColor();
                if (actual_color != expected_color) {
                    ADD_FAILURE() << "Unexpected pixel at " << location.ToString() << " (in_hole=" << in_hole << ")\n"
                                  << "Expected:\n  " << std::hex << "0x" << expected_color << " ("
                                  << CheckOverlayExactlyFillsHoleColorToString(SkColor4f::FromColor(expected_color)) << ")\n"
                                  << "But got:\n  "
                                  << "0x" << actual_color << " (" << CheckOverlayExactlyFillsHoleColorToString(SkColor4f::FromColor(actual_color)) << ")";
                    return;
                }
            }
        }
    ...
    void InitializeSwapChainForTest(
        gfx::Size swap_chain_size, Microsoft::WRL::ComPtr<IDXGISwapChain1>& swap_chain, Microsoft::WRL::ComPtr<ID3D11RenderTargetView>& rtv)
    {
        Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device = GetDirectCompositionD3D11Device();
        ASSERT_TRUE(d3d11_device);
        Microsoft::WRL::ComPtr<IDXGIDevice> dxgi_device;
        ...
        ASSERT_HRESULT_SUCCEEDED(dxgi_factory->CreateSwapChainForComposition(d3d11_device.Get(), &desc, nullptr, &swap_chain));
        ASSERT_TRUE(swap_chain);
        Microsoft::WRL::ComPtr<ID3D11Texture2D> front_buffer_texture;
        ASSERT_HRESULT_SUCCEEDED(swap_chain->GetBuffer(1u, IID_PPV_ARGS(&front_buffer_texture)));
        ASSERT_TRUE(front_buffer_texture);
        Microsoft::WRL::ComPtr<ID3D11Texture2D> back_buffer_texture;
        ASSERT_TRUE(SUCCEEDED(swap_chain->GetBuffer(0u, IID_PPV_ARGS(&back_buffer_texture))));
        ASSERT_TRUE(back_buffer_texture);

        d3d11_device->CreateRenderTargetView(back_buffer_texture.Get(), nullptr, &rtv);
    }

    [[nodiscard]] HRESULT ClearRenderTargetViewAndPresent(const SkColor4f& clear_color, IDXGISwapChain1* swap_chain, ID3D11RenderTargetView* rtv)
    {
...
class DCompPresenterPixelTest : public DCompPresenterPixelTestBase, public testing::WithParamInterface<bool> {
protected:
    void SetUp() override
    {
        if (GetParam()) {
...
class DCompPresenterVideoPixelTest : public DCompPresenterPixelTestBase, public testing::WithParamInterface<bool> {
protected:
    void SetUp() override
    {
        if (GetParam()) {
    ...
    void TestVideo(const gfx::ColorSpace& color_space, SkColor expected_color, bool check_color)
    {
        if (!presenter_) {
            return;
        }
        ...
        if (check_color) {
            EXPECT_SKCOLOR_CLOSE(expected_color, GLTestHelper::ReadBackWindowPixel(window_.hwnd(), gfx::Point(75, 75)), kMaxColorChannelDeviation);
        }
    }
};

INSTANTIATE_TEST_SUITE_P(DCompPresenterVideoPixelTest, DCompPresenterVideoPixelTest, testing::Bool());

TEST_P(DCompPresenterVideoPixelTest, BT601)
{
    TestVideo(gfx::ColorSpace::CreateREC601(), SkColorSetRGB(0xdb, 0x81, 0xe8), true);
}

TEST_P(DCompPresenterVideoPixelTest, BT709)
{
    TestVideo(gfx::ColorSpace::CreateREC709(), SkColorSetRGB(0xe1, 0x90, 0xeb), true);
}
...
INSTANTIATE_TEST_SUITE_P(DCompPresenterPixelTest, DCompPresenterPixelTest, testing::Bool());
...
TEST_P(DCompPresenterPixelTest, SoftwareVideoSwapchain)
{
    ...
    auto params = CreateParamsFromImage(DCLayerOverlayImage(y_size, nv12_pixmap.data(), stride));
    params->quad_rect = gfx::Rect(window_size);
    params->video_params.color_space = gfx::ColorSpace::CreateREC709();
    presenter_->ScheduleDCLayer(std::move(params));
...
TEST_P(DCompPresenterPixelTest, VideoHandleSwapchain)
{
    ...
    InitializeForPixelTest(window_size, texture_size, content_rect, quad_rect, false);
...
{
    if (context_ && context_->GetVersionInfo() && context_->GetVersionInfo()->driver_vendor.find("AMD") != std::string::npos) {
        GTEST_SKIP() << "CreateSwapChainForCompositionSurfaceHandle fails with YUY2 format "
...
TEST_P(DCompPresenterPixelTest, P010SwapChain)
{
    ...
    gfx::Rect content_rect(0, 0, 49, 49);
    gfx::Rect quad_rect(window_size);
    InitializeForPixelTest(window_size, texture_size, content_rect, quad_rect, true);
...
TEST_P(DCompPresenterPixelTest, NonZeroBoundsOffset)
{
    ...
    for (const auto& test_case : test_cases) {
        ...
        EXPECT_SKCOLOR_CLOSE(expected_color, GLTestHelper::GetColorAtPoint(pixels, point), kMaxColorChannelDeviation) << " at " << point.ToString();
    }
}
...
TEST_P(DCompPresenterPixelTest, ResizeVideoLayer)
{
    ...
    gfx::Rect on_screen_rect = gfx::Rect(0, 0, monitor_size.width() - 2, monitor_size.height() - 2);
    {
        auto params = CreateParamsFromImage(DCLayerOverlayImage(texture_size, texture));
    ...
    gfx::Point offset;
    gfx::Rect clip_rect;
    presenter_->GetSwapChainVisualInfoForTesting(0, &transform, &offset, &clip_rect);
    ...
    SetDirectCompositionScaledOverlaysSupportedForTesting(true);
    on_screen_rect = gfx::Rect(0, 0, monitor_size.width() + 2, monitor_size.height() + 2);
    {
        auto params = CreateParamsFromImage(DCLayerOverlayImage(texture_size, texture));
        params->quad_rect = on_screen_rect;
        params->video_params.color_space = gfx::ColorSpace::CreateREC709();
...
TEST_P(DCompPresenterPixelTest, SwapChainImage)
{
    ...
    {
        ASSERT_HRESULT_SUCCEEDED(ClearRenderTargetViewAndPresent(SkColors::kRed, swap_chain.Get(), rtv.Get()));

        auto dc_layer_params = CreateParamsFromImage(DCLayerOverlayImage(swap_chain_size, swap_chain));
        dc_layer_params->quad_rect = gfx::Rect(window_size);
        dc_layer_params->z_order = 1;
    ...
    {
        ASSERT_HRESULT_SUCCEEDED(ClearRenderTargetViewAndPresent(SkColors::kGreen, swap_chain.Get(), rtv.Get()));
...
TEST_P(DCompPresenterPixelTest, QuadOffsetAppliedAfterTransform)
{
    ...
    const gfx::Transform quad_to_root_transform(gfx::AxisTransform2d(0.5, gfx::Vector2dF()));
    ...
    auto dc_layer_params = CreateParamsFromImage(CreateDCompSurface(quad_rect.size(), SkColors::kRed));
    ...
    EXPECT_SKCOLOR_CLOSE(SK_ColorBLACK, GLTestHelper::GetColorAtPoint(pixels, gfx::Point(0, mapped_quad_rect.y() - 1)), kMaxColorChannelDeviation);
    EXPECT_SKCOLOR_CLOSE(SK_ColorRED, GLTestHelper::ReadBackWindowPixel(window_.hwnd(), gfx::Point(0, mapped_quad_rect.y())), kMaxColorChannelDeviation);

    // Check the bottom edge of the scaled overlay
    EXPECT_SKCOLOR_CLOSE(SK_ColorRED, GLTestHelper::GetColorAtPoint(pixels, gfx::Point(0, mapped_quad_rect.bottom() - 1)), kMaxColorChannelDeviation);
    EXPECT_SKCOLOR_CLOSE(

        SK_ColorBLACK, GLTestHelper::GetColorAtPoint(pixels, gfx::Point(0, mapped_quad_rect.bottom())), kMaxColorChannelDeviation);
}
...
TEST_P(DCompPresenterPixelTest, ContentRectScalesUpBuffer)
{
    ...
    auto overlay = CreateParamsFromImage(CreateDCompSurface(gfx::Size(1, 1), kOverlayExpectedColor));
    overlay->quad_rect = root_surface_hole;
    overlay->z_order = 1;
    CheckOverlayExactlyFillsHole(window_size, root_surface_hole, std::move(overlay));
}
...
TEST_P(DCompPresenterPixelTest, ContentRectScalesDownBuffer)
{
    ...
    auto overlay = CreateParamsFromImage(CreateDCompSurface(gfx::Size(75, 100), kOverlayExpectedColor));
    overlay->quad_rect = root_surface_hole;
    overlay->z_order = 1;
    CheckOverlayExactlyFillsHole(window_size, root_surface_hole, std::move(overlay));
}
...
TEST_P(DCompPresenterPixelTest, ContentRectClipsBuffer)
{
    ...
    const gfx::Rect root_surface_hole = gfx::Rect(gfx::Point(20, 25), tex_coord.size());
    ...
    auto overlay = CreateParamsFromImage(CreateDCompSurface(window_size, kOverlayImageHiddenColor, { { tex_coord, kOverlayExpectedColor } }),
...
TEST_P(DCompPresenterPixelTest, ContentRectClipsAndScalesBuffer)
{
    ...
    const gfx::Rect root_surface_hole = gfx::Rect(gfx::Point(20, 25), gfx::Size(50, 60));
...
TEST_P(DCompPresenterPixelTest, BackgroundColorSurfaceReuse)
{
    ...
    for (const SkColor4f& color : colors) {
        ...
        EXPECT_SKCOLOR_EQ(color.toSkColor(), GLTestHelper::ReadBackWindowPixel(window_.hwnd(), gfx::Point(0, 0)));
        ...
        if (background_color_surface == nullptr) {
            background_color_surface = layer_tree->GetBackgroundColorSurfaceForTesting(0);
        }
        EXPECT_NE(background_color_surface, nullptr);
        EXPECT_EQ(background_color_surface, layer_tree->GetBackgroundColorSurfaceForTesting(0))
...
protected:
    void SetUp() override
    {
        ...
        const ui::test::TestEnvironmentMap test_environment = {
            { ui::test::TestEnvironmentKey::kSystemVersion, base::win::OSInfo::GetInstance()->release_id() },
            { ui::test::TestEnvironmentKey::kGpuDriverVendor, context_->GetVersionInfo()->driver_vendor },
            { ui::test::TestEnvironmentKey::kGpuDriverVersion, context_->GetVersionInfo()->driver_version },
            { ui::test::TestEnvironmentKey::kGlRenderer, context_->GetGLRenderer() },

        };

        pixel_diff_ = ui::test::SkiaGoldPixelDiff::GetSession(kSkiaGoldPixelDiffCorpus, test_environment);
    }
    ...
    {
        ASSERT_FALSE(test_initialized_) << "InitializeTest should only be called once per test";
    ...
    void PresentAndCheckScreenshot(std::string capture_name = std::string(), const base::Location& caller_location = FROM_HERE)
    {
        ...
        if (capture_names_in_test_.contains(capture_name)) {
            ADD_FAILURE_AT(caller_location.file_name(), caller_location.line_number())
                << "Capture names must be unique in a test. Capture name \"" << capture_name << "\" is already used.";
            return;
        }
        ...
        SkBitmap window_readback = GLTestHelper::ReadBackWindow(window_.hwnd(), window_size_);
        CHECK(pixel_diff_);

        std::string test_name = GetGoldenTestName(::testing::UnitTest::GetInstance()->current_test_info()->test_suite_name());
        std::string test_case_name = GetGoldenTestCaseName(::testing::UnitTest::GetInstance()->current_test_info()->name());

        if (!pixel_diff_->CompareScreenshot(ui::test::SkiaGoldPixelDiff::GetGoldenImageName(
                                                test_name, test_case_name, capture_name.empty() ? std::nullopt : std::make_optional(capture_name)),
                window_readback, matching_algorithm_.get())) {
            ADD_FAILURE_AT(caller_location.file_name(), caller_location.line_number())
                << "Screenshot mismatch for " << (capture_name.empty() ? "(unnamed capture)" : capture_name);
        }
    }

    const gfx::Size& current_window_size() const
    {
        return window_size_;
    }

    void AddOverlaysForOpacityTest(base::RepeatingCallback<std::unique_ptr<DCLayerOverlayParams>(const gfx::Rect&, float)> get_overlay_for_opacity)
    {
        const int kOverlayCount = 10;
        for (int i = 0; i < kOverlayCount; i++) {
            const int width = current_window_size().width() / kOverlayCount;
            const gfx::Rect quad_rect = gfx::Rect(i * width, 0, width, current_window_size().height());
            const float opacity = static_cast<float>(i) / static_cast<float>(kOverlayCount);
...
INSTANTIATE_TEST_SUITE_P(DCompPresenterSkiaGoldTest, DCompPresenterSkiaGoldTest, testing::Bool());
...
TEST_P(DCompPresenterSkiaGoldTest, TransformTranslate)
{
    ...
    auto overlay = CreateParamsFromImage(CreateDCompSurface(gfx::Size(50, 50), SkColors::kWhite));
...
TEST_P(DCompPresenterSkiaGoldTest, SolidColorSimpleOpaque)
{
    ...
    InitializeRootAndScheduleRootSurface(current_window_size(), root_surface_color);
...
TEST_P(DCompPresenterSkiaGoldTest, OpacityFromOverlay)
{
    ...
    AddOverlaysForOpacityTest(base::BindRepeating([](const gfx::Rect& quad_rect, float opacity) {
        auto overlay = CreateParamsFromImage(CreateDCompSurface(quad_rect.size(), SkColors::kWhite));
...
TEST_P(DCompPresenterSkiaGoldTest, OpacityFromImage)
{
    ...
    AddOverlaysForOpacityTest(base::BindRepeating([](const gfx::Rect& quad_rect, float opacity) {
        ...
        auto overlay = CreateParamsFromImage(CreateDCompSurface(quad_rect.size(), overlay_color));
        overlay->quad_rect = quad_rect;
        return overlay;
    }));
...
TEST_P(DCompPresenterSkiaGoldTest, SurfaceSerialForcesCommit)
{
    ...
    const std::vector<SkColor4f> colors = { SkColors::kRed, SkColors::kGreen, SkColors::kBlue, SkColors::kWhite };

    Microsoft::WRL::ComPtr<IDCompositionDevice2> dcomp_device = gl::GetDirectCompositionDevice();

    Microsoft::WRL::ComPtr<IDCompositionSurface> surface;
    ASSERT_HRESULT_SUCCEEDED(dcomp_device->CreateSurface(
        current_window_size().width(), current_window_size().height(), DXGI_FORMAT_B8G8R8A8_UNORM, DXGI_ALPHA_MODE_IGNORE, &surface));
    ...
    for (size_t i = 0; i < colors.size(); i++) {
        ...
        auto overlay = CreateParamsFromImage(DCLayerOverlayImage(current_window_size(), surface, surface_serial));
...
TEST_P(DCompPresenterSkiaGoldTest, RoundedCornerSimple)
{
    ...
    auto overlay = CreateParamsFromImage(CreateDCompSurface(current_window_size(), SkColors::kWhite));
    overlay->quad_rect = gfx::Rect(current_window_size());
    overlay->quad_rect.Inset(kPaddingFromEdgeForAntiAliasedOutput);
    overlay->z_order = 1;
    overlay->rounded_corner_bounds = gfx::RRectF(gfx::RectF(overlay->quad_rect), 25.f);
    presenter_->ScheduleDCLayer(std::move(overlay));
...
TEST_P(DCompPresenterSkiaGoldTest, RoundedCornerNonUniformRadii)
{
    ...
    bounds.SetCornerRadii(gfx::RRectF::Corner::kUpperRight, gfx::Vector2dF(15, 30));
    bounds.SetCornerRadii(gfx::RRectF::Corner::kLowerRight, gfx::Vector2dF(25, 20));
    bounds.SetCornerRadii(gfx::RRectF::Corner::kLowerLeft, gfx::Vector2dF(35, 10));
...
TEST_P(DCompPresenterSkiaGoldTest, NoSeamsBetweenAdjacentSolidColorsWithSharedRoundedCorner)
{
...
TEST_P(DCompPresenterSkiaGoldTest, SoftBordersFromNonIntegralTranslation)
{
    ...
    auto overlay = CreateParamsFromImage(CreateDCompSurface(gfx::Size(20, 20), SkColors::kWhite));
    overlay->quad_rect = gfx::Rect(overlay->overlay_image->size());
    overlay->transform.Translate(kPaddingFromEdgeForAntiAliasedOutput, kPaddingFromEdgeForAntiAliasedOutput);
...
TEST_P(DCompPresenterSkiaGoldTest, SoftBordersFromNonIntegralScaling)
{
    ...
    overlay->quad_rect = gfx::Rect(overlay->overlay_image->size());
    overlay->transform.Translate(kPaddingFromEdgeForAntiAliasedOutput, kPaddingFromEdgeForAntiAliasedOutput);
    overlay->transform.Scale((static_cast<float>(overlay->quad_rect.width()) + 0.5) / static_cast<float>(overlay->quad_rect.width()), 1);
    overlay->z_order = 1;
    presenter_->ScheduleDCLayer(std::move(overlay));
...
TEST_P(DCompPresenterSkiaGoldTest, SoftBordersFromNonIntegralRoundedCornerBounds)
{
    InitializeTest(gfx::Size(100, 100));

    InitializeRootAndScheduleRootSurface(current_window_size(), SkColors::kBlack);

    auto overlay = CreateParamsFromImage(CreateDCompSurface(gfx::Size(21, 20), SkColors::kWhite));
    overlay->quad_rect = gfx::Rect(overlay->overlay_image->size());

    // DComp seems to not actually use soft borders unless there's a non-zero
    // radius.
    const double kForceDCompRoundedCornerSoftBorder = std::numeric_limits<float>::epsilon();

    overlay->rounded_corner_bounds = gfx::RRectF(gfx::RectF(0, 0, 20.5, 20), kForceDCompRoundedCornerSoftBorder);
    overlay->rounded_corner_bounds.Offset(kPaddingFromEdgeForAntiAliasedOutput, kPaddingFromEdgeForAntiAliasedOutput);
    overlay->transform.Translate(kPaddingFromEdgeForAntiAliasedOutput, kPaddingFromEdgeForAntiAliasedOutput);
    overlay->z_order = 1;
    presenter_->ScheduleDCLayer(std::move(overlay));
...
TEST_P(DCompPresenterSkiaGoldTest, OverlaysAreSortedByZOrder)
{
    ...
    for (const auto& [color, z_order] : color_and_z_order) {
        ...
        auto overlay = CreateParamsFromImage(CreateDCompSurface(quad_rect.size(), color));
        overlay->quad_rect = quad_rect;
...

    {
        ...
        // our output opaque.
        auto overlay = CreateParamsFromImage(CreateDCompSurface(current_window_size(), SkColors::kBlack));
        overlay->quad_rect = gfx::Rect(current_window_size());
...
TEST_P(DCompPresenterSkiaGoldTest, ImageWithBackgroundColor)
{
    ...
    auto overlay = CreateParamsFromImage(CreateDCompSurface(gfx::Size(100, 50), SkColors::kTransparent,
        {
            { gfx::Rect(5, 5, 20, 20), SkColor4f::FromColor(SkColorSetA(SK_ColorRED, 0x80)) },
            { gfx::Rect(15, 15, 20, 20), SkColor4f::FromColor(SkColorSetA(SK_ColorBLUE, 0x80)) },
        }));
...
TEST_P(DCompPresenterSkiaGoldTest, NonIntegralContentRectHalfCoverage)
{
    ...
    auto overlay = CreateParamsFromImage(CreateDCompSurface(image_size, SkColors::kGreen, { { image_inner_rect, SkColors::kBlue } }));
    overlay->content_rect.Inset(0.5);
    overlay->quad_rect = gfx::Rect(gfx::Point(20, 20), gfx::Size(overlay->content_rect.width(), overlay->content_rect.height()));
    overlay->z_order = 1;
    presenter_->ScheduleDCLayer(std::move(overlay));
...
void RunSeamsWithComplexTransformTest(DCompPresenter* presenter, base::RepeatingCallback<void(int x, int y, DCLayerOverlayParams&)> update_overlay)
{
    ...
    for (int y = 0; y < 4; y++) {
        for (int x = 0; x < 4; x++) {
            auto overlay = std::make_unique<DCLayerOverlayParams>();
            overlay->quad_rect = gfx::Rect(x * tile_size.width(), y * tile_size.height(), tile_size.width(), tile_size.height());
...
TEST_P(DCompPresenterSkiaGoldTest, EdgeAANoSeamsOnSameLayerComplexTransform)
{
    ...
    RunSeamsWithComplexTransformTest(presenter_.get(), base::BindRepeating([](int x, int y, DCLayerOverlayParams& overlay) {
...
class DCompPresenterDelegatedInkSkiaGoldTest : public DCompPresenterSkiaGoldTest {
protected:
    void SetUp() override
    {
    ...
    void ScheduleInkTrail(
        const gfx::RectF& presentation_area, const SkColor& color, const float diameter, base::span<gfx::DelegatedInkPoint const> const points)
    {
        DCLayerTree* layer_tree = presenter_->GetLayerTreeForTesting();
        // Metadata timestamp and point should match the first DelegatedInkPoint.
        gfx::DelegatedInkMetadata metadata(points[0].point(), diameter, color, points[0].timestamp(), presentation_area,
            /*hovering=*/false);
        for (auto point : points) {
            layer_tree->GetInkRendererForTesting()->StoreDelegatedInkPoint(point);
        }
        layer_tree->SetDelegatedInkTrailStartPoint(std::make_unique<gfx::DelegatedInkMetadata>(metadata));
    }
};

constexpr base::TimeTicks kEarliestTimestamp = base::TimeTicks();
constexpr base::TimeDelta kMicrosecondsBetweenEachPoint = base::Microseconds(10);

INSTANTIATE_TEST_SUITE_P(All, DCompPresenterDelegatedInkSkiaGoldTest, testing::Bool());
...
TEST_P(DCompPresenterDelegatedInkSkiaGoldTest, NonIntegerPresentationArea)
{
    ...
    gfx::DelegatedInkPoint points[] = { { gfx::PointF(10.7, 10.7), kEarliestTimestamp, kPointerId },
        { gfx::PointF(10.7, 30), kEarliestTimestamp + kMicrosecondsBetweenEachPoint, kPointerId } };
    gfx::RectF presentation_area(10.4, 10.4, 100, 100);
    ScheduleInkTrail(presentation_area, SK_ColorRED, /* diameter= */ 10.0, points);
...
TEST_P(DCompPresenterDelegatedInkSkiaGoldTest, TrailSyncedToSwapChainPresent)
{
    ...
    gfx::DelegatedInkPoint points[]
        = { { gfx::PointF(12, 12), kEarliestTimestamp, kPointerId }, { gfx::PointF(12, 30), kEarliestTimestamp + kMicrosecondsBetweenEachPoint, kPointerId } };
    ...
    ASSERT_HRESULT_SUCCEEDED(ClearRenderTargetViewAndPresent(SkColors::kBlue, swap_chain.Get(), rtv.Get()));
    dc_layer_params = CreateParamsFromImage(DCLayerOverlayImage(swap_chain_size, swap_chain));
    dc_layer_params->quad_rect = gfx::Rect(monitor_size);
    presenter_->ScheduleDCLayer(std::move(dc_layer_params));
...
TEST_P(DCompPresenterDelegatedInkSkiaGoldTest, RootSurfaceIsDCompSurface)
{
    ...
    const int kPointerId = 2;
    gfx::DelegatedInkPoint points[]
        = { { gfx::PointF(11, 11), kEarliestTimestamp, kPointerId }, { gfx::PointF(30, 30), kEarliestTimestamp + kMicrosecondsBetweenEachPoint, kPointerId },
              { gfx::PointF(49, 11), kEarliestTimestamp + 2 * kMicrosecondsBetweenEachPoint, kPointerId } };
    gfx::RectF presentation_area(10, 10, 100, 100);
    ScheduleInkTrail(presentation_area, SK_ColorRED, /* diameter= */ 10.0, points);
    ...
    // frame.
    auto overlay = CreateParamsFromImage(CreateDCompSurface(window_size, SkColors::kWhite));
...
TEST_P(DCompPresenterDelegatedInkSkiaGoldTest, MetadataChangesProperties)
{
    ...
    gfx::DelegatedInkPoint blue_points[] = { { gfx::PointF(11, 11), kEarliestTimestamp + 3 * kMicrosecondsBetweenEachPoint, kPointerId2 },
        { gfx::PointF(30, 30), kEarliestTimestamp + 4 * kMicrosecondsBetweenEachPoint, kPointerId2 },
        { gfx::PointF(49, 11), kEarliestTimestamp + 5 * kMicrosecondsBetweenEachPoint, kPointerId2 } };
    ScheduleInkTrail(presentation_area, SK_ColorBLUE, /* diameter= */ 5.0, blue_points);
...
TEST_P(DCompPresenterDelegatedInkSkiaGoldTest, MetadataOutsidePresentationArea)
{
    gfx::Size window_size(200, 200);
    InitializeTest(window_size);
    ...
    const int kPointerId = 2;
    gfx::DelegatedInkPoint points[] = { { gfx::PointF(10.1, 10.1), kEarliestTimestamp, kPointerId },
        { gfx::PointF(10.6, 30), kEarliestTimestamp + kMicrosecondsBetweenEachPoint, kPointerId },
        { gfx::PointF(10.6, 60), kEarliestTimestamp + 2 * kMicrosecondsBetweenEachPoint, kPointerId } };
    gfx::RectF presentation_area(10.4, 10.4, 100, 100);
    ScheduleInkTrail(presentation_area, SK_ColorRED, /* diameter= */ 10.0, points);
...
TEST_P(DCompPresenterDelegatedInkSkiaGoldTest, InkTrailPortionClipped)
{
    ...
    const int kPointerId = 2;
    gfx::DelegatedInkPoint points[] = { // Inside presentation area.
        { gfx::PointF(55, 55), kEarliestTimestamp, kPointerId },
        // Outside presentation area.
        { gfx::PointF(75, 120), kEarliestTimestamp + kMicrosecondsBetweenEachPoint, kPointerId },
        // Inside presentation area.
        { gfx::PointF(90, 55), kEarliestTimestamp + 2 * kMicrosecondsBetweenEachPoint, kPointerId }
    };
...
TEST_P(DCompPresenterDelegatedInkSkiaGoldTest, InkTrailClippedDueToThickness)
{
    ...
    gfx::DelegatedInkPoint points[]
        = { { gfx::PointF(55, 55), kEarliestTimestamp, kPointerId }, { gfx::PointF(55, 85), kEarliestTimestamp + kMicrosecondsBetweenEachPoint, kPointerId },
              { gfx::PointF(55, 100), kEarliestTimestamp + 2 * kMicrosecondsBetweenEachPoint, kPointerId } };
    gfx::RectF presentation_area(50, 50, 100, 100);

    ScheduleInkTrail(presentation_area, SK_ColorBLUE, /* diameter= */ 40.0, points);
...
class DCompPresenterBufferCountTest : public DCompPresenterTestBase, public testing::WithParamInterface<std::tuple<bool, bool>> {
public:
    static std::string GetParamName(const testing::TestParamInfo<ParamType>& info)
    {
        return base::StringPrintf("%s_%s", std::get<0>(info.param) ? "DCompTripleBufferVideoSwapChain" : "DcompTripleBufferVideoSwapChain_default",
...
protected:
    void SetUp() override
    {
        if (std::get<0>(GetParam())) {
            DCompPresenterTestBase::EnableFeature(features::kDCompTripleBufferVideoSwapChain);
        } else {
            DCompPresenterTestBase::DisableFeature(features::kDCompTripleBufferVideoSwapChain);
        }
...
INSTANTIATE_TEST_SUITE_P(All, DCompPresenterBufferCountTest, testing::Combine(testing::Bool(), testing::Bool()), &DCompPresenterBufferCountTest::GetParamName);
...
class DCompPresenterLetterboxingTest : public DCompPresenterTestBase, public testing::WithParamInterface<std::tuple<LetterboxingTestParams, bool>> {
protected:
    void SetUp() override
    {
    ...
    virtual void SetupScopedFeatureList()
    {
        ...
        if (std::get<0>(GetParam()).use_letterbox_video_optimization) {
            DCompPresenterTestBase::EnableFeature(features::kDirectCompositionLetterboxVideoOptimization);
        } else {
            DCompPresenterTestBase::DisableFeature(features::kDirectCompositionLetterboxVideoOptimization);
        }
...
INSTANTIATE_TEST_SUITE_P(None, DCompPresenterLetterboxingTest, testing::Combine(::testing::Values(LetterboxingTestParams(false)), testing::Bool()));

INSTANTIATE_TEST_SUITE_P(LetterBoxOpt, DCompPresenterLetterboxingTest, testing::Combine(::testing::Values(LetterboxingTestParams(true)), testing::Bool()));
...
TEST_P(DCompPresenterLetterboxingTest, FullScreenLetterboxingResizeVideoLayer)
{
    ...
    const int letterboxing_height = (monitor_size.height() - texture_size.height()) / 2;
    const gfx::Rect quad_rect = gfx::Rect(0, 0, texture_size.width(), texture_size.height());
    gfx::Rect clip_rect = gfx::Rect(0, letterboxing_height, texture_size.width(), texture_size.height());
    gfx::Transform quad_to_root_transform(gfx::AxisTransform2d(1, gfx::Vector2dF(0, letterboxing_height)));
    {
        auto dc_layer_params = CreateParamsFromImage(DCLayerOverlayImage(texture_size, texture));
        dc_layer_params->quad_rect = quad_rect;
        dc_layer_params->transform = quad_to_root_transform;
    ...
    presenter_->GetSwapChainVisualInfoForTesting(0, &visual_transform, &visual_offset, &visual_clip_rect);
    ...
    texture = CreateNV12Texture(d3d11_device, texture_size);
    clip_rect = gfx::Rect(0, letterboxing_height - 2, texture_size.width(), texture_size.height());
    quad_to_root_transform = gfx::Transform(gfx::AxisTransform2d(1, gfx::Vector2dF(0, letterboxing_height - 2)));
    {
        auto dc_layer_params = CreateParamsFromImage(DCLayerOverlayImage(texture_size, texture));
        dc_layer_params->quad_rect = quad_rect;
        dc_layer_params->transform = quad_to_root_transform;
    ...
    presenter_->GetSwapChainVisualInfoForTesting(0, &visual_transform2, &visual_offset2, &visual_clip_rect2);
    ...
    presenter_->GetSwapChainVisualInfoForTesting(0, &visual_transform3, &visual_offset3, &visual_clip_rect3);
...
TEST_P(DCompPresenterLetterboxingTest, FullScreenLetterboxingWithDesktopPlaneRemoval)
{
    // Define 1920x1200 monitor size.
    const gfx::Size monitor_size(1920, 1200);
    SetDirectCompositionScaledOverlaysSupportedForTesting(true);
    ...
    const int letterboxing_height = (monitor_size.height() - texture_size.height()) / 2;
    const gfx::Rect quad_rect = gfx::Rect(0, 0, texture_size.width(), texture_size.height());
    const gfx::Rect clip_rect = gfx::Rect(0, letterboxing_height, texture_size.width(), texture_size.height());
    const gfx::Transform quad_to_root_transform(gfx::AxisTransform2d(1, gfx::Vector2dF(0, letterboxing_height)));
    {
        auto dc_layer_params = CreateParamsFromImage(DCLayerOverlayImage(texture_size, texture));
        dc_layer_params->quad_rect = quad_rect;
        dc_layer_params->transform = quad_to_root_transform;
    ...
    if (std::get<0>(GetParam()).use_letterbox_video_optimization) {
        ...
        EXPECT_HRESULT_SUCCEEDED(swap_chain->QueryInterface(IID_PPV_ARGS(&decode_swap_chain)));
        // The dest size has been set to monitor size.
        uint32_t dest_width, dest_height;
        EXPECT_HRESULT_SUCCEEDED(decode_swap_chain->GetDestSize(&dest_width, &dest_height));
...
TEST_P(DCompPresenterLetterboxingTest, FullScreenLetterboxingKeepVisualInfo)
{
    ...
    presenter_->GetSwapChainVisualInfoForTesting(0, &visual_transform1, &visual_offset1, &visual_clip_rect1);
...
TEST_P(DCompPresenterLetterboxingTest, FullScreenPillarboxingResizeVideoLayer)
{
    ...
    // First test if swap chain and its visual info is adjusted to fit the
    // monitor when letterboxing is generated for full screen presentation.
    const int letterboxing_width = (monitor_size.width() - texture_size.width()) / 2;
    const gfx::Rect quad_rect = gfx::Rect(0, 0, texture_size.width(), texture_size.height());
    gfx::Rect clip_rect = gfx::Rect(letterboxing_width, 0, texture_size.width(), texture_size.height());
    gfx::Transform quad_to_root_transform(gfx::AxisTransform2d(1, gfx::Vector2dF(letterboxing_width, 0)));
    {
        auto dc_layer_params = CreateParamsFromImage(DCLayerOverlayImage(texture_size, texture));
        dc_layer_params->quad_rect = quad_rect;
        dc_layer_params->transform = quad_to_root_transform;
    ...
    // some negative offset from typical letterboxing positioning.
    texture = CreateNV12Texture(d3d11_device, texture_size);
    clip_rect = gfx::Rect(letterboxing_width - 2, 0, texture_size.width(), texture_size.height());
    quad_to_root_transform = gfx::Transform(gfx::AxisTransform2d(1, gfx::Vector2dF(letterboxing_width - 2, 0)));
    {
        auto dc_layer_params = CreateParamsFromImage(DCLayerOverlayImage(texture_size, texture));
        dc_layer_params->quad_rect = quad_rect;
        dc_layer_params->transform = quad_to_root_transform;
...
TEST_P(DCompPresenterLetterboxingTest, FullScreenPillarboxingWithDesktopPlaneRemoval)
{
    // Define 1920x1200 monitor size.
    const gfx::Size monitor_size(1920, 1200);
    SetDirectCompositionScaledOverlaysSupportedForTesting(true);
    ...
    const int letterboxing_width = (monitor_size.width() - texture_size.width()) / 2;
    const gfx::Rect quad_rect = gfx::Rect(0, 0, texture_size.width(), texture_size.height());
    const gfx::Rect clip_rect = gfx::Rect(letterboxing_width, 0, texture_size.width(), texture_size.height());
    const gfx::Transform quad_to_root_transform(gfx::AxisTransform2d(1, gfx::Vector2dF(letterboxing_width, 0)));
    {
        auto dc_layer_params = CreateParamsFromImage(DCLayerOverlayImage(texture_size, texture));
        dc_layer_params->quad_rect = quad_rect;
        dc_layer_params->transform = quad_to_root_transform;
...
TEST_F(DCompPresenterFullscreenRoundingTest, FullScreenRoundingWithHalfPixelTranslation)
{
    ...
    const gfx::Transform quad_to_root_transform(gfx::AxisTransform2d(1, gfx::Vector2dF(0.5, 0.5)));
    {
        auto dc_layer_params = CreateParamsFromImage(DCLayerOverlayImage(texture_size, texture));
        dc_layer_params->quad_rect = quad_rect;
        dc_layer_params->transform = quad_to_root_transform;
...
TEST_F(DCompPresenterFullscreenRoundingTest, FullScreenContentWithClipping)
{
    ...
    const gfx::Transform quad_to_root_transform(gfx::AxisTransform2d(1.5, gfx::Vector2dF(0, 0)));
    {
        auto dc_layer_params = CreateParamsFromImage(DCLayerOverlayImage(texture_size, texture));
        dc_layer_params->quad_rect = quad_rect;
        dc_layer_params->transform = quad_to_root_transform;
