...
enum class MaximumGLESVersion { kGLES2_0 = 0, kGLES3_0 = 1, kGLES3_1 = 2, kGLES3_2 = 3, kMaxValue = kGLES3_2 };
#endif
...
GLContextAttribs& GLContextAttribs::operator=(const GLContextAttribs& other) = default;
GLContextAttribs& GLContextAttribs::operator=(GLContextAttribs&& other) = default;
...
GLContext::~GLContext()
{
    ...
    base::subtle::Atomic32 after_value = base::subtle::NoBarrier_AtomicIncrement(&total_gl_contexts_, -1);
...
{
    return static_cast<int32_t>(base::subtle::NoBarrier_Load(&total_gl_contexts_));
}
...
bool GLContext::Initialize(GLSurface* compatible_surface, const GLContextAttribs& attribs)
{
...
{
    return other_context && (this == other_context || share_group_ == other_context->share_group());
}
...
std::string GLContext::GetGLVersion()
{
    ...
    const char* version = reinterpret_cast<const char*>(gl_api_wrapper_->api()->glGetStringFn(GL_VERSION));
...
std::string GLContext::GetGLRenderer()
{
    DCHECK(IsCurrent(nullptr));
    DCHECK(gl_api_wrapper_);
    const char* renderer = reinterpret_cast<const char*>(gl_api_wrapper_->api()->glGetStringFn(GL_RENDERER));
...
CurrentGL* GLContext::GetCurrentGL()
{
    if (!static_bindings_initialized_) {
        ...
        gl_api_wrapper_ = std::make_unique<GL_IMPL_WRAPPER_TYPE(GL)>(std::move(gl_api));
...
void GLContext::AddMetalSharedEventsForBackpressure(std::vector<std::unique_ptr<gpu::BackpressureMetalSharedEvent>> events)
{
    for (auto& e : events) {
...
uint64_t GLContext::BackpressureFenceCreate()
{
    ...
    std::vector<std::unique_ptr<gpu::BackpressureMetalSharedEvent>> backpressure_events = std::move(next_backpressure_events_);
    ...
    if (gl::GetANGLEImplementation() == gl::ANGLEImplementation::kMetal) {
        ...
        backpressure_fences_[++next_backpressure_fence_] = { nullptr, std::move(backpressure_events) };
        ...
        glFlush();
        backpressure_fences_[++next_backpressure_fence_] = { GLFence::Create(), std::move(backpressure_events) };
        return next_backpressure_fence_;
    } else {
        glFinish();
...
void GLContext::BackpressureFenceWait(uint64_t fence_id)
{
    ...
    // remove them.
    while (!backpressure_fences_.empty() && backpressure_fences_.begin()->first < fence_id) {
...
void GLContext::FlushForDriverCrashWorkaround()
{
    ...
    static const bool needs_flush = base::mac::GetCPUType() == base::mac::CPUType::kIntel;
...
{
    return std::make_unique<GLVersionInfo>(GetGLVersion().c_str(), GetGLRenderer().c_str(), GetExtensions());
}
...
void GLContext::SetCurrent(GLSurface* surface)
{
    ...
    if (!surface && GetGLImplementation() != kGLImplementationMockGL && GetGLImplementation() != kGLImplementationStubGL) {
        SetThreadLocalCurrentGL(nullptr);
    }
}

void GLContext::SetDisabledGLExtensions(const std::string& disabled_extensions)
{
    DCHECK(!real_gl_api_);
...
bool GLContext::MakeVirtuallyCurrent(GLContext* virtual_context, GLSurface* surface)
{
    ...
    if (switched_real_contexts || !surface->IsCurrent()) {
        ...
        // an FBO.
        if (switched_real_contexts || !current_surface || !virtual_context->IsCurrent(surface)) {
    ...
    if (switched_real_contexts || virtual_context != current_virtual_context_) {
        ...
        // the new context.
        DCHECK(error == GL_NO_ERROR || error == GL_CONTEXT_LOST_KHR) << "GL error was: " << error;
#endif
        ...
        if (virtual_context->GetGLStateRestorer()->IsInitialized()) {
            ...
            GLStateRestorer* current_state = current_virtual_context_ ? current_virtual_context_->GetGLStateRestorer() : nullptr;
            if (current_state)
                current_state->PauseQueries();
            virtual_state->ResumeQueries();

            virtual_state->RestoreState((current_state && !switched_real_contexts) ? current_state : NULL);
        }
...
scoped_refptr<GLContext> InitializeGLContext(scoped_refptr<GLContext> context, GLSurface* compatible_surface, const GLContextAttribs& attribs)
{
