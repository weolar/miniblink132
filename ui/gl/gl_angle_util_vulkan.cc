...
{
    const char* const* extensions_ptr = reinterpret_cast<const char* const*>(extensions);
    gfx::ExtensionSet extension_set;
    // SAFETY: required from OpenGL C style API.
    for (const char* const* p = extensions_ptr; *p != nullptr; UNSAFE_BUFFERS(++p)) {
...
VkInstance QueryVkInstanceFromANGLE()
{
    ...
    intptr_t instance = 0;
    if (!eglQueryDeviceAttribEXT(egl_device, EGL_VULKAN_INSTANCE_ANGLE, &instance)) {
...
GL_EXPORT uint32_t QueryVkVersionFromANGLE()
{
    ...
    intptr_t version = 0;
    if (!eglQueryDeviceAttribEXT(egl_device, EGL_VULKAN_VERSION_ANGLE, &version)) {
...
VkPhysicalDevice QueryVkPhysicalDeviceFromANGLE()
{
    ...
    if (!eglQueryDeviceAttribEXT(egl_device, EGL_VULKAN_PHYSICAL_DEVICE_ANGLE, &physical_device)) {
...
int QueryVkQueueFramiliyIndexFromANGLE()
{
    ...
    intptr_t index = 0;
    if (!eglQueryDeviceAttribEXT(egl_device, EGL_VULKAN_QUEUE_FAMILIY_INDEX_ANGLE, &index)) {
...
gfx::ExtensionSet QueryVkDeviceExtensionsFromANGLE()
{
    ...
    if (!eglQueryDeviceAttribEXT(egl_device, EGL_VULKAN_DEVICE_EXTENSIONS_ANGLE, &extensions)) {
...
gfx::ExtensionSet QueryVkInstanceExtensionsFromANGLE()
{
    ...
    intptr_t extensions = 0;
    if (!eglQueryDeviceAttribEXT(egl_device, EGL_VULKAN_INSTANCE_EXTENSIONS_ANGLE, &extensions)) {
...
const VkPhysicalDeviceFeatures2KHR* QueryVkEnabledDeviceFeaturesFromANGLE()
{
    ...
    intptr_t features = 0;
    if (!eglQueryDeviceAttribEXT(egl_device, EGL_VULKAN_FEATURES_ANGLE, &features)) {
...
PFN_vkGetInstanceProcAddr QueryVkGetInstanceProcAddrFromANGLE()
{
    ...
    intptr_t proc = 0;
    if (!eglQueryDeviceAttribEXT(egl_device, EGL_VULKAN_GET_INSTANCE_PROC_ADDR, &proc)) {
