...
BASE_FEATURE(kFallbackBT709VideoToBT601, "FallbackBT709VideoToBT601", base::FEATURE_DISABLED_BY_DEFAULT);

BASE_FEATURE(kDisableVPBLTUpscale, "DisableVPBLTUpscale", base::FEATURE_DISABLED_BY_DEFAULT);
...
gfx::ColorSpace GetOutputColorSpace(const gfx::ColorSpace& input_color_space, bool is_yuv_swapchain)
{
    gfx::ColorSpace output_color_space = is_yuv_swapchain ? input_color_space : gfx::ColorSpace::CreateSRGB();
    if (base::FeatureList::IsEnabled(kFallbackBT709VideoToBT601) && (output_color_space == gfx::ColorSpace::CreateREC709())) {
...
{
    using PFN_DCOMPOSITION_CREATE_SURFACE_HANDLE = HRESULT(WINAPI*)(DWORD, SECURITY_ATTRIBUTES*, HANDLE*);
    static PFN_DCOMPOSITION_CREATE_SURFACE_HANDLE create_surface_handle_function = nullptr;
    ...
    if (!create_surface_handle_function) {
        ...
        create_surface_handle_function = reinterpret_cast<PFN_DCOMPOSITION_CREATE_SURFACE_HANDLE>(::GetProcAddress(dcomp, "DCompositionCreateSurfaceHandle"));
        if (!create_surface_handle_function) {
            DLOG(ERROR) << "Failed to get address for DCompositionCreateSurfaceHandle";
            return false;
        }
    }

    HRESULT hr = create_surface_handle_function(COMPOSITIONOBJECT_ALL_ACCESS, nullptr, handle);
    if (FAILED(hr)) {
        DLOG(ERROR) << "DCompositionCreateSurfaceHandle failed with error 0x" << std::hex << hr;
        return false;
    }
...
{
    return force_triple_buffer || base::FeatureList::IsEnabled(features::kDCompTripleBufferVideoSwapChain) ? 3u : 2u;
}
...
void UpdateSwapChainTransform(const gfx::Size& quad_size, const gfx::SizeF& swap_chain_size, gfx::Transform* visual_transform)
{
    float swap_chain_scale_x = quad_size.width() * 1.0f / swap_chain_size.width();
    float swap_chain_scale_y = quad_size.height() * 1.0f / swap_chain_size.height();
    visual_transform->Scale(swap_chain_scale_x, swap_chain_scale_y);
}

const GUID GUID_INTEL_VPE_INTERFACE = { 0xedd1d4b9, 0x8659, 0x4cbc, { 0xa4, 0xd6, 0x98, 0x31, 0xa2, 0x16, 0x3a, 0xc3 } };
...
HRESULT ToggleIntelVpSuperResolution(ID3D11VideoContext* video_context, ID3D11VideoProcessor* video_processor, bool enable)
{
    ...
    HRESULT hr = video_context->VideoProcessorSetOutputExtension(video_processor, &GUID_INTEL_VPE_INTERFACE, sizeof(ext), &ext);
    if (FAILED(hr)) {
        DLOG(ERROR) << "VideoProcessorSetOutputExtension failed with error 0x" << std::hex << hr;
        return hr;
    }

    ext.function = kIntelVpeFnMode;
    param = enable ? kIntelVpeModePreproc : kIntelVpeModeNone;
    hr = video_context->VideoProcessorSetOutputExtension(video_processor, &GUID_INTEL_VPE_INTERFACE, sizeof(ext), &ext);
    if (FAILED(hr)) {
        DLOG(ERROR) << "VideoProcessorSetOutputExtension failed with error 0x" << std::hex << hr;
        return hr;
    }
    ...
    hr = video_context->VideoProcessorSetStreamExtension(video_processor, 0, &GUID_INTEL_VPE_INTERFACE, sizeof(ext), &ext);
    if (FAILED(hr)) {
        DLOG(ERROR) << "VideoProcessorSetStreamExtension failed with error 0x" << std::hex << hr;
    }

    return hr;
}

HRESULT ToggleNvidiaVpSuperResolution(ID3D11VideoContext* video_context, ID3D11VideoProcessor* video_processor, bool enable)
{
    ...
    struct {
        ...
        UINT method;
        UINT enable;
    } stream_extension_info = { kStreamExtensionVersionV1, kStreamExtensionMethodSuperResolution, enable ? 1u : 0u };

    HRESULT hr
        = video_context->VideoProcessorSetStreamExtension(video_processor, 0, &kNvidiaPPEInterfaceGUID, sizeof(stream_extension_info), &stream_extension_info);

    base::UmaHistogramSparse(enable ? "GPU.NvidiaVpSuperResolution.On.SetStreamExt" : "GPU.NvidiaVpSuperResolution.Off.SetStreamExt", hr);
    if (FAILED(hr)) {
        DLOG(ERROR) << "VideoProcessorSetStreamExtension failed with error 0x" << std::hex << hr;
    }
...
HRESULT ToggleVpSuperResolution(UINT gpu_vendor_id, ID3D11VideoContext* video_context, ID3D11VideoProcessor* video_processor, bool enable)
{
    if (gpu_vendor_id == 0x8086 && base::FeatureList::IsEnabled(features::kIntelVpSuperResolution)) {
        return ToggleIntelVpSuperResolution(video_context, video_processor, enable);
    }

    if (gpu_vendor_id == 0x10de && base::FeatureList::IsEnabled(features::kNvidiaVpSuperResolution)) {
        return ToggleNvidiaVpSuperResolution(video_context, video_processor, enable);
    }
...
bool NvidiaDriverSupportsTrueHDR(ID3D11VideoContext* video_context, ID3D11VideoProcessor* video_processor)
{
    UINT driver_supports_true_hdr = 0;
    HRESULT hr = video_context->VideoProcessorGetStreamExtension(
        video_processor, 0, &kNvidiaTrueHDRInterfaceGUID, sizeof(driver_supports_true_hdr), &driver_supports_true_hdr);

    // The runtime never fails the GetStreamExtension hr unless a bad memory size
    // is provided.
    if (FAILED(hr)) {
        DLOG(ERROR) << "VideoProcessorGetStreamExtension failed with error 0x" << std::hex << hr;
        return false;
    }
...
bool GpuDriverSupportsVpAutoHDR(UINT gpu_vendor_id, ID3D11VideoContext* video_context, ID3D11VideoProcessor* video_processor)
{
...
HRESULT ToggleNvidiaVpTrueHDR(bool driver_supports_vp_auto_hdr, ID3D11VideoContext* video_context, ID3D11VideoProcessor* video_processor, bool enable)
{
    ...
    struct {
        ...
        UINT enable : 1;
        UINT reserved : 31;
    } stream_extension_info = { kStreamExtensionVersionV4, kStreamExtensionMethodTrueHDR, enable ? 1u : 0u, 0u };

    HRESULT hr = video_context->VideoProcessorSetStreamExtension(
        video_processor, 0, &kNvidiaTrueHDRInterfaceGUID, sizeof(stream_extension_info), &stream_extension_info);

    base::UmaHistogramSparse(enable ? "GPU.NvidiaVpTrueHDR.On.SetStreamExt" : "GPU.NvidiaVpTrueHDR.Off.SetStreamExt", hr);
    if (FAILED(hr)) {
        DLOG(ERROR) << "VideoProcessorSetStreamExtension failed with error 0x" << std::hex << hr;
    }
...
HRESULT ToggleVpAutoHDR(
    UINT gpu_vendor_id, bool driver_supports_vp_auto_hdr, ID3D11VideoContext* video_context, ID3D11VideoProcessor* video_processor, bool enable)
{
    if (gpu_vendor_id == 0x10de) {
        return ToggleNvidiaVpTrueHDR(driver_supports_vp_auto_hdr, video_context, video_processor, enable);
    }
...
{
    if (gpu_vendor_id == 0x10de && base::FeatureList::IsEnabled(features::kNvidiaVpTrueHDR)) {
        return true;
    }
...
bool TryDisableDesktopPlane(IDXGIDecodeSwapChain* decode_swap_chain, const gfx::Size& dest_size, const gfx::Rect& target_rect)
{
    // Get the original dest size in case of restoring.
    uint32_t original_dest_width, original_dest_height;
    HRESULT hr = decode_swap_chain->GetDestSize(&original_dest_width, &original_dest_height);
    if (FAILED(hr)) {
    ...
    if (dest_size.width() != (int)original_dest_width || dest_size.height() != (int)original_dest_height) {
...
{
    return ((hdr_metadata.smpte_st_2086 && hdr_metadata.smpte_st_2086->IsValid()) || (hdr_metadata.cta_861_3 && hdr_metadata.cta_861_3->IsValid()));
}
...
void SwapChainPresenter::PresentationHistory::AddSample(DXGI_FRAME_PRESENTATION_MODE mode)
{
...
SwapChainPresenter::SwapChainPresenter(
    DCLayerTree* layer_tree, Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device, Microsoft::WRL::ComPtr<IDCompositionDevice2> dcomp_device)
    : layer_tree_(layer_tree)
    , swap_chain_buffer_count_(BufferCount(layer_tree->force_dcomp_triple_buffer_video_swap_chain()))
    , switched_to_BGRA8888_time_tick_(base::TimeTicks::Now())
    , d3d11_device_(d3d11_device)
    , dcomp_device_(dcomp_device)
    , is_on_battery_power_(base::PowerMonitor::GetInstance()->AddPowerStateObserverAndReturnBatteryPowerStatus(this)
...
DXGI_FORMAT SwapChainPresenter::GetSwapChainFormat(gfx::ProtectedVideoType protected_video_type, bool use_hdr_swap_chain, bool use_p010_for_sdr_swap_chain)
{
    ...
    // P010 displayable.
    if (base::FeatureList::IsEnabled(kP010MPOForSDR) && use_p010_for_sdr_swap_chain) {
    ...
    if (failed_to_create_yuv_swapchain_ || !DirectCompositionHardwareOverlaysSupported()) {
    ...
    if (swap_chain_format_ == yuv_overlay_format) {
        ...
        // flickers.
        base::TimeDelta time_delta = base::TimeTicks::Now() - switched_to_BGRA8888_time_tick_;
...
Microsoft::WRL::ComPtr<ID3D11Texture2D> SwapChainPresenter::UploadVideoImage(
    const gfx::Size& texture_size, const uint8_t* shm_video_pixmap, size_t pixmap_stride)
{
    if (!shm_video_pixmap) {
    ...
    TRACE_EVENT1("gpu", "SwapChainPresenter::UploadVideoImage", "size", texture_size.ToString());
    ...
    if (!staging_texture_ || (staging_texture_size_ != texture_size)) {
        ...
        copy_texture_.Reset();
        HRESULT hr = d3d11_device_->CreateTexture2D(&desc, nullptr, &staging_texture_);
        if (FAILED(hr)) {
            DLOG(ERROR) << "Creating D3D11 video staging texture failed: 0x" << std::hex << hr;
    ...
    D3D11_MAP map_type = use_dynamic_texture ? D3D11_MAP_WRITE_DISCARD : D3D11_MAP_WRITE;
    D3D11_MAPPED_SUBRESOURCE mapped_resource;
    HRESULT hr = context->Map(staging_texture_.Get(), 0, map_type, 0, &mapped_resource);
    if (FAILED(hr)) {
        DLOG(ERROR) << "Mapping D3D11 video staging texture failed: 0x" << std::hex << hr;
        return nullptr;
    }
    ...
    if (texture_size.height() / 2 > 0) {
        ...
        // the stride.
        src_size += pixmap_stride * (texture_size.height() / 2 - 1) + texture_size.width();
        dest_size += dest_stride * (texture_size.height() / 2 - 1) + texture_size.width();
    }
    base::span<const uint8_t> src = UNSAFE_TODO(base::span(shm_video_pixmap, src_size));
    // SAFETY: required from Map() call result.
    base::span<uint8_t> dest = UNSAFE_BUFFERS(base::span(reinterpret_cast<uint8_t*>(mapped_resource.pData), dest_size));
    ...
    if (!copy_texture_) {
        ...
        if (FAILED(hr)) {
            DLOG(ERROR) << "Creating D3D11 video upload texture failed: 0x" << std::hex << hr;
            DisableDirectCompositionOverlays();
            return nullptr;
        }
...
gfx::Size SwapChainPresenter::GetMonitorSize() const
{
    if (GetDirectCompositionNumMonitors() == 1) {
        ...
        if (GetMonitorInfo(MonitorFromWindow(layer_tree_->window(), MONITOR_DEFAULTTONEAREST), &monitor_info)) {
...
void SwapChainPresenter::SetTargetToFullScreen(gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect, const std::optional<gfx::Rect>& target_rect)
{
    if (base::FeatureList::IsEnabled(kDisableVPBLTUpscale) && (std::abs(visual_transform->rc(0, 0)) > 1.0f) && (std::abs(visual_transform->rc(1, 1)) > 1.0f)
        ...
        visual_transform->set_rc(0, 3, visual_transform->rc(0, 3) - target_rect.value().x() * visual_transform->rc(0, 0));
        visual_transform->set_rc(1, 3, visual_transform->rc(1, 3) - target_rect.value().y() * visual_transform->rc(1, 1));
    } else {
        // Reset the horizontal/vertical shift according to the visual clip and
        // original transform, since DWM will do the positioning in case of overlay.
        visual_transform->set_rc(0, 3, visual_clip_rect->x() - visual_transform->rc(0, 3) * visual_transform->rc(0, 0));
        visual_transform->set_rc(1, 3, visual_clip_rect->y() - visual_transform->rc(1, 3) * visual_transform->rc(1, 1));
    }
...
void SwapChainPresenter::AdjustTargetToOptimalSizeIfNeeded(const DCLayerOverlayParams& params, const gfx::RectF& overlay_onscreen_rect,
    gfx::SizeF* swap_chain_size, gfx::Transform* visual_transform, gfx::RectF* visual_clip_rect, std::optional<gfx::SizeF>* dest_size,
    std::optional<gfx::RectF>* target_rect) const
{
    ...
    bool size_adjusted
        = AdjustTargetToFullScreenSizeIfNeeded(monitor_size_float, params, overlay_onscreen_rect, swap_chain_size, visual_transform, visual_clip_rect);

    // Adjustment for the full screen letterboxing scenario.
    if (!size_adjusted && params.video_params.possible_video_fullscreen_letterboxing) {
        AdjustTargetForFullScreenLetterboxing(
            monitor_size_float, params, overlay_onscreen_rect, swap_chain_size, visual_transform, visual_clip_rect, dest_size, target_rect);
    }
}

bool SwapChainPresenter::AdjustTargetToFullScreenSizeIfNeeded(const gfx::SizeF& monitor_size, const DCLayerOverlayParams& params,
    const gfx::RectF& overlay_onscreen_rect, gfx::SizeF* swap_chain_size, gfx::Transform* visual_transform, gfx::RectF* visual_clip_rect) const
{
    ...
    if (clipped_onscreen_rect == gfx::RectF(monitor_size) && overlay_onscreen_rect == gfx::RectF(monitor_size)) {
        return true;
    }
    ...
    if (!IsWithinMargin(clipped_onscreen_rect.x(), 0.0) || !IsWithinMargin(clipped_onscreen_rect.y(), 0.0)) {
        // Not fullscreen mode.
        return false;
    }
    ...
    // If no, do nothing.
    if (!IsWithinMargin(clipped_onscreen_rect.width(), monitor_size.width()) || !IsWithinMargin(clipped_onscreen_rect.height(), monitor_size.height())) {
        // Not fullscreen mode.
        return false;
    }
    ...
    if (params.clip_rect.has_value()) {
        if (!IsWithinMargin(overlay_onscreen_rect.width(), monitor_size.width()) || !IsWithinMargin(overlay_onscreen_rect.height(), monitor_size.height())) {
            return false;
        }
    }
    ...
    gfx::RectF unmapped_rect = gfx::RectF(params.quad_rect.x(), params.quad_rect.y(), swap_chain_size->width(), swap_chain_size->height());
    ...
    bool sufficiently_equal = new_rect.ApproximatelyEqual(gfx::RectF(monitor_size), kTolerance, kTolerance);
    DCHECK(sufficiently_equal) << ", params.quad_rect: " << params.quad_rect.ToString() << ", params.content_rect: " << params.content_rect.ToString()
                               << ", clipped_onscreen_rect: " << clipped_onscreen_rect.ToString()
                               << ", overlay_onscreen_rect: " << overlay_onscreen_rect.ToString() << ", params.transform: " << params.transform.ToString()
...
void SwapChainPresenter::AdjustTargetForFullScreenLetterboxing(const gfx::SizeF& monitor_size, const DCLayerOverlayParams& params,
    const gfx::RectF& overlay_onscreen_rect, gfx::SizeF* swap_chain_size, gfx::Transform* visual_transform, gfx::RectF* visual_clip_rect,
    std::optional<gfx::SizeF>* dest_size, std::optional<gfx::RectF>* target_rect) const
{
    if (!base::FeatureList::IsEnabled(features::kDirectCompositionLetterboxVideoOptimization)) {
        return;
    }
    ...
    bool is_onscreen_rect_x_near_0 = IsWithinMargin(clipped_onscreen_rect.x(), 0.0);
    bool is_onscreen_rect_y_near_0 = IsWithinMargin(clipped_onscreen_rect.y(), 0.0);
    ...
    if (is_onscreen_rect_x_near_0 && !IsWithinMargin(clipped_onscreen_rect.y() * 2.0 + clipped_onscreen_rect.height(), monitor_size.height())) {
        // Not fullscreen letterboxing mode.
        return;
    }

    if (is_onscreen_rect_y_near_0 && !IsWithinMargin(clipped_onscreen_rect.x() * 2.0 + clipped_onscreen_rect.width(), monitor_size.width())) {
        // Not fullscreen letterboxing mode.
        return;
    }

    if (params.clip_rect.has_value()) {
        if (is_onscreen_rect_x_near_0 && !IsWithinMargin(overlay_onscreen_rect.width(), monitor_size.width())) {
            // Not fullscreen letterboxing mode.
            return;
        }
        if (is_onscreen_rect_y_near_0 && !IsWithinMargin(overlay_onscreen_rect.height(), monitor_size.height())) {
            // Not fullscreen letterboxing mode.
            return;
        }
    }
    ...
    if (is_onscreen_rect_x_near_0) {
        ...
        if (new_y < new_onscreen_rect.y()) {
            ...
            new_onscreen_rect.set_height(new_onscreen_rect.height() + new_onscreen_rect.y() - new_y);
            ...
            new_onscreen_rect.set_height(monitor_size.height() - new_onscreen_rect.y() * 2);
        }
    ...
    if (is_onscreen_rect_y_near_0) {
        ...
        if (new_x < new_onscreen_rect.x()) {
            ...
            new_onscreen_rect.set_width(new_onscreen_rect.width() + new_onscreen_rect.x() - new_x);
            ...
            // the original point of the video onscreen rect. Meanwhile, increase its
            // size to make it symmetrical around the monitor center.
            new_onscreen_rect.set_width(monitor_size.width() - new_onscreen_rect.x() * 2);
        }
    ...
    if (new_onscreen_rect_float != clipped_onscreen_rect) {
        ...
        float scale_y;
        if (*swap_chain_size == overlay_onscreen_rect.size()) {
            scale_x = swap_chain_size->width() * 1.0f / new_onscreen_rect_float.width();
            scale_y = swap_chain_size->height() * 1.0f / new_onscreen_rect_float.height();
            visual_transform->Scale(scale_x, scale_y);
        ...
        scale_x = new_onscreen_rect_float.width() * 1.0f / overlay_onscreen_rect.width();
        scale_y = new_onscreen_rect_float.height() * 1.0f / overlay_onscreen_rect.height();
        visual_transform->Scale(scale_x, scale_y);

        // Update the origin.
        gfx::RectF unmapped_rect = gfx::RectF(gfx::PointF(params.quad_rect.origin()), gfx::SizeF(*swap_chain_size));
        gfx::RectF mapped_rect = visual_transform->MapRect(unmapped_rect);

        auto offset = new_onscreen_rect_float.OffsetFromOrigin() - mapped_rect.OffsetFromOrigin();
    ...
    if (params.z_order > 0) {
        if (base::FeatureList::IsEnabled(kDisableVPBLTUpscale) && (std::abs(visual_transform->rc(0, 0)) > 1.0f)
            ...
            *dest_size = gfx::ScaleSize(monitor_size, inverse_scale_x, inverse_scale_y);
            *target_rect = gfx::ScaleRect(*visual_clip_rect, inverse_scale_x, inverse_scale_y);
        } else {
...
#if DCHECK_IS_ON()
    {
        ...
        gfx::RectF new_swap_chain_rect = gfx::RectF(gfx::PointF(params.quad_rect.origin()), gfx::SizeF(*swap_chain_size));
        ...
        static auto* new_swap_chain_rect_key = base::debug::AllocateCrashKeyString("new-swap-chain-rect", base::debug::CrashKeySize::Size256);
        base::debug::ScopedCrashKeyString scoped_crash_key_1(new_swap_chain_rect_key, new_swap_chain_rect.ToString());
        static auto* visual_transform_key = base::debug::AllocateCrashKeyString("visual-transform", base::debug::CrashKeySize::Size256);
        base::debug::ScopedCrashKeyString scoped_crash_key_2(visual_transform_key, visual_transform->ToString());
        static auto* result_rect_key = base::debug::AllocateCrashKeyString("result-rect", base::debug::CrashKeySize::Size256);
        base::debug::ScopedCrashKeyString scoped_crash_key_3(result_rect_key, result_rect.ToString());
        ...
        if (is_onscreen_rect_x_near_0) {
            ...
            DCHECK_LE(std::abs(result_rect.width() - monitor_size.width()), kTolerance);
        }

        if (is_onscreen_rect_y_near_0) {
            DCHECK_LE(std::abs(result_rect.y()), kTolerance);
            DCHECK_LE(std::abs(result_rect.height() - monitor_size.height()), kTolerance);
        }
    }
#endif
}

gfx::Size SwapChainPresenter::CalculateSwapChainSize(const DCLayerOverlayParams& params, gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect,
    std::optional<gfx::Size>* dest_size, std::optional<gfx::Rect>* target_rect) const
{
    ...
    bool can_disable_vp_upscaling_blt = base::FeatureList::IsEnabled(kDisableVPBLTUpscale) && is_on_battery_power_ && std::abs(params.transform.rc(0, 0)) > 1.0f
        && std::abs(params.transform.rc(1, 1)) > 1.0f;

    if (visual_transform->IsScaleOrTranslation() && !can_disable_vp_upscaling_blt) {
    ...
    // Adjust the transform matrix.
    UpdateSwapChainTransform(params.quad_rect.size(), swap_chain_size, visual_transform);
    ...
    if (visual_transform->IsScaleOrTranslation()) {
        AdjustTargetToOptimalSizeIfNeeded(
            params, overlay_onscreen_rect, &swap_chain_size, visual_transform, &visual_clip_rect_float, &dest_size_float, &target_rect_float);
...
bool SwapChainPresenter::TryPresentToDecodeSwapChain(Microsoft::WRL::ComPtr<ID3D11Texture2D> texture, unsigned array_slice, const gfx::ColorSpace& color_space,
    const gfx::Rect& content_rect, const gfx::Size& swap_chain_size, DXGI_FORMAT swap_chain_format, const gfx::Transform& transform_to_root,
    const std::optional<gfx::Size> dest_size, const std::optional<gfx::Rect> target_rect)
{
    if (ShouldUseVideoProcessorScaling())
        return false;

    bool nv12_supported = (swap_chain_format == DXGI_FORMAT_NV12) && (DXGI_FORMAT_NV12 == GetDirectCompositionSDROverlayFormat());
    ...
    if (texture && nv12_supported && !failed_to_present_decode_swapchain_) {
        ...
        bool is_decoder_texture = (texture_desc.Format == DXGI_FORMAT_NV12) && (texture_desc.BindFlags & D3D11_BIND_DECODER);

        // Decode swap chains do not support shared resources.
        // TODO(sunnyps): Find a workaround for when the decoder moves to its own
        // thread and D3D device.  See https://crbug.com/911847
        bool is_shared_texture
            = texture_desc.MiscFlags & (D3D11_RESOURCE_MISC_SHARED | D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX | D3D11_RESOURCE_MISC_SHARED_NTHANDLE);
        ...
        bool compatible_transform = transform_to_root.IsPositiveScaleOrTranslation();
        ...
        // overlay.
        float swap_chain_scale_x = swap_chain_size.width() * 1.0f / content_rect.width();
        float swap_chain_scale_y = swap_chain_size.height() * 1.0f / content_rect.height();

        if (layer_tree_->no_downscaled_overlay_promotion()) {
            compatible_transform = compatible_transform && (swap_chain_scale_x >= 1.0f) && (swap_chain_scale_y >= 1.0f);
        }
        if (!DirectCompositionScaledOverlaysSupported()) {
            compatible_transform = compatible_transform && (swap_chain_scale_x == 1.0f) && (swap_chain_scale_y == 1.0f);
        }

        if (is_decoder_texture && !is_shared_texture && !is_unitary_texture_array && compatible_transform) {
            if (PresentToDecodeSwapChain(texture, array_slice, color_space, content_rect, swap_chain_size, dest_size, target_rect)) {
                return true;
            }
            ReleaseSwapChainResources();
            failed_to_present_decode_swapchain_ = true;
            DLOG(ERROR) << "Present to decode swap chain failed - falling back to blit";
        }
    }
    return false;
}

bool SwapChainPresenter::PresentToDecodeSwapChain(Microsoft::WRL::ComPtr<ID3D11Texture2D> texture, unsigned array_slice, const gfx::ColorSpace& color_space,
    const gfx::Rect& content_rect, const gfx::Size& swap_chain_size, const std::optional<gfx::Size> dest_size, const std::optional<gfx::Rect> target_rect)
{
    DCHECK(!swap_chain_size.IsEmpty());

    TRACE_EVENT2("gpu", "SwapChainPresenter::PresentToDecodeSwapChain", "content_rect", content_rect.ToString(), "swap_chain_size", swap_chain_size.ToString());
    ...
    if (!decode_swap_chain_ || decode_resource_ != decode_resource) {
        TRACE_EVENT0("gpu", "SwapChainPresenter::PresentToDecodeSwapChain::CreateDecodeSwapChain");
        ReleaseSwapChainResources();
        ...
        hr = media_factory->CreateDecodeSwapChainForCompositionSurfaceHandle(
            d3d11_device_.Get(), swap_chain_handle_.Get(), &desc, decode_resource_.Get(), nullptr, &decode_swap_chain_);
        if (FAILED(hr)) {
        ...
        hr = desktop_device->CreateSurfaceFromHandle(swap_chain_handle_.Get(), &decode_surface_);
        if (FAILED(hr)) {
            DLOG(ERROR) << "CreateSurfaceFromHandle failed with error 0x" << std::hex << hr;
            return false;
        }
    ...
    gfx::Size swap_chain_dest_size = dest_size.has_value() ? dest_size.value() : swap_chain_size;
    hr = decode_swap_chain_->SetDestSize(swap_chain_dest_size.width(), swap_chain_dest_size.height());
    if (FAILED(hr)) {
        DLOG(ERROR) << "SetDestSize failed with error 0x" << std::hex << hr;
        return false;
    }

    RECT swap_chain_target_rect = target_rect.has_value() ? (target_rect.value()).ToRECT() : gfx::Rect(swap_chain_size).ToRECT();
    ...
    if (color_space == gfx::ColorSpace::CreateREC709() || color_space == gfx::ColorSpace::CreateREC601() || !color_space.IsValid()) {
    ...
    hr = decode_swap_chain_->SetColorSpace(static_cast<DXGI_MULTIPLANE_OVERLAY_YCbCr_FLAGS>(color_space_flags));
    if (FAILED(hr)) {
...
bool SwapChainPresenter::PresentToSwapChain(DCLayerOverlayParams& params, gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect)
{
    DCHECK(params.overlay_image);
    DCHECK_NE(params.overlay_image->type(), DCLayerOverlayType::kDCompVisualContent);
    ...
    *visual_clip_rect = params.clip_rect.value_or(gfx::Rect());
    ...
    gfx::Size swap_chain_size = CalculateSwapChainSize(params, visual_transform, visual_clip_rect, &dest_size, &target_rect);

    if (overlay_type == DCLayerOverlayType::kD3D11Texture && !params.overlay_image->d3d11_video_texture()) {
    ...
    bool use_vp_auto_hdr = !content_is_hdr && DirectCompositionMonitorHDREnabled(layer_tree_->window()) && enable_vp_auto_hdr_ && !is_on_battery_power_;
    ...
    bool content_is_pq10
        = (input_color_space.GetPrimaryID() == gfx::ColorSpace::PrimaryID::BT2020) && (input_color_space.GetTransferID() == gfx::ColorSpace::TransferID::PQ);

    bool use_hdr_swap_chain = DirectCompositionMonitorHDREnabled(layer_tree_->window()) && (content_is_pq10 || use_vp_auto_hdr);
    ...
    bool use_p010_for_sdr_swap_chain
        = !DirectCompositionMonitorHDREnabled(layer_tree_->window()) && CheckDisplayableSupportForP010() && params.video_params.is_p010_content;

    DXGI_FORMAT swap_chain_format = GetSwapChainFormat(params.video_params.protected_video_type, use_hdr_swap_chain, use_p010_for_sdr_swap_chain);

    bool swap_chain_format_changed = swap_chain_format != swap_chain_format_;
    bool toggle_protected_video = swap_chain_protected_video_type_ != params.video_params.protected_video_type;

    bool contents_changed = last_overlay_image_ != params.overlay_image;

    if (swap_chain_ && !swap_chain_resized && !swap_chain_format_changed && !toggle_protected_video && !contents_changed) {
    ...
    Microsoft::WRL::ComPtr<ID3D11Texture2D> input_texture = params.overlay_image->d3d11_video_texture();
    unsigned input_level = params.overlay_image->texture_array_slice();

    if (TryPresentToDecodeSwapChain(input_texture, input_level, input_color_space, gfx::ToNearestRect(params.content_rect), swap_chain_size, swap_chain_format,
        ...
        if (dest_size.has_value() && target_rect.has_value() && params.z_order > 0) {
            SetTargetToFullScreen(visual_transform, visual_clip_rect, target_rect);
        } else {
    ...
    if (!swap_chain_ || swap_chain_resized || swap_chain_format_changed || toggle_protected_video) {
        if (!ReallocateSwapChain(swap_chain_size, swap_chain_format, params.video_params.protected_video_type)) {
            ReleaseSwapChainResources();
            return false;
        }
    ...
    if (input_texture) {
        ...
        // crbug.com/338686911.
        input_texture
            = UNSAFE_TODO(UploadVideoImage(params.overlay_image->size(), params.overlay_image->shm_video_pixmap(), params.overlay_image->pixmap_stride()));
        input_level = 0;
    }
    ...
    if (content_is_pq10) {
        ...
        if (!IsCompatibleHDRMetadata(hdr_metadata)) {
            hdr_metadata = gfx::HDRMetadata::PopulateUnspecifiedWithDefaults(std::make_optional(params.video_params.hdr_metadata));
        }
        stream_metadata = HDRMetadataHelperWin::HDRMetadataToDXGI(hdr_metadata);
    }

    if (!VideoProcessorBlt(
            std::move(input_texture), input_level, gfx::ToNearestRect(params.content_rect), input_color_space, stream_metadata, use_vp_auto_hdr)) {
        return false;
    }
    ...
    if (first_present_) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            static auto* hr_enqueue_set_event_key = base::debug::AllocateCrashKeyString("hr-EnqueueSetEvent", base::debug::CrashKeySize::Size64);
            base::debug::ScopedCrashKeyString scoped_crash_key_1(hr_enqueue_set_event_key, base::StringPrintf("0x%x", hr));
            static auto* hr_device_removed_reason_key = base::debug::AllocateCrashKeyString("hr-DeviceRemovedReason", base::debug::CrashKeySize::Size64);
            base::debug::ScopedCrashKeyString scoped_crash_key_2(hr_device_removed_reason_key, base::StringPrintf("0x%x", device_removed_reason));
    ...
    if (DirectCompositionSwapChainTearingEnabled()) {
        ...
        interval = 0;
    } else if (base::FeatureList::IsEnabled(features::kDXGISwapChainPresentInterval0)) {
        interval = 0;
    }
    ...
    if (IsYUVSwapChainFormat(swap_chain_format_) && dest_size.has_value() && target_rect.has_value()) {
        ...
        if (SUCCEEDED(hr)) {
            succeeded = TryDisableDesktopPlane(decode_swap_chain.Get(), *dest_size, *target_rect);
        } else {
            DLOG(ERROR) << "QueryInterface for IDXGIDecodeSwapChain failed with error 0x" << std::hex << hr;
        }
...
{
    base::UmaHistogramSparse("GPU.DirectComposition.SwapChainFormat3", swap_chain_format_);
    ...
    UMA_HISTOGRAM_ENUMERATION("GPU.DirectComposition.VideoPresentationMode", presentation_mode);

    TRACE_EVENT_INSTANT2(TRACE_DISABLED_BY_DEFAULT("gpu.service"), "SwapChain::Present", TRACE_EVENT_SCOPE_THREAD, "PixelFormat",
        DxgiFormatToString(swap_chain_format_), "ZeroCopy", !!decode_swap_chain_);
    Microsoft::WRL::ComPtr<IDXGISwapChainMedia> swap_chain_media = GetSwapChainMedia();
    if (swap_chain_media) {
        ...
        int mode = -1;
        if (SUCCEEDED(hr)) {
            base::UmaHistogramSparse("GPU.DirectComposition.CompositionMode2.VideoOrCanvas", stats.CompositionMode);
        ...
        TRACE_EVENT_INSTANT1(TRACE_DISABLED_BY_DEFAULT("gpu.service"), "GetFrameStatisticsMedia", TRACE_EVENT_SCOPE_THREAD, "CompositionMode", mode);
    }
}

bool SwapChainPresenter::PresentDCOMPSurface(DCLayerOverlayParams& params, gfx::Transform* visual_transform, gfx::Rect* visual_clip_rect)
{
    ...
    // representation.
    gfx::RectF overlay_onscreen_rect = visual_transform->MapRect(gfx::RectF(params.quad_rect));
    ...
    if (visual_transform->IsScaleOrTranslation()) {
        AdjustTargetToOptimalSizeIfNeeded(
            params, overlay_onscreen_rect, &on_screen_size_float, visual_transform, &visual_clip_rect_float, &dest_size, &target_rect);
    }
    ...
    dcomp_surface_proxy->SetProtectedVideoType(params.video_params.protected_video_type);
    ...
    if (content_size == gfx::Size(1, 1)) {
        ...
        ReleaseDCOMPSurfaceResourcesIfNeeded();
        DVLOG(2) << __func__ << " this=" << this << " dcomp_surface_proxy size (1x1) path.";
        return true;
    }
...
#if DCHECK_IS_ON()
    TRACE_EVENT2("gpu", "PresentDCOMPSurface", "finalized transform", visual_transform->ToString(), "finalized mapped rect", mapped_rect.ToString());
#endif // DCHECK_IS_ON()
...
bool SwapChainPresenter::VideoProcessorBlt(Microsoft::WRL::ComPtr<ID3D11Texture2D> input_texture, UINT input_level, const gfx::Rect& content_rect,
    const gfx::ColorSpace& src_color_space, std::optional<DXGI_HDR_METADATA_HDR10> stream_hdr_metadata, bool use_vp_auto_hdr)
{
    TRACE_EVENT2("gpu", "SwapChainPresenter::VideoProcessorBlt", "content_rect", content_rect.ToString(), "swap_chain_size", swap_chain_size_.ToString());
    ...
    gfx::ColorSpace output_color_space = GetOutputColorSpace(src_color_space, is_yuv_swapchain);
    bool video_processor_recreated = false;
    VideoProcessorWrapper* video_processor_wrapper
        = layer_tree_->InitializeVideoProcessor(content_rect.size(), swap_chain_size_, output_color_space.IsHDR(), video_processor_recreated);
    if (!video_processor_wrapper)
        return false;

    Microsoft::WRL::ComPtr<ID3D11VideoContext> video_context = video_processor_wrapper->video_context;
    Microsoft::WRL::ComPtr<ID3D11VideoProcessor> video_processor = video_processor_wrapper->video_processor;

    if (video_processor_recreated) {
        bool supports_vp_auto_hdr = GpuDriverSupportsVpAutoHDR(gpu_vendor_id_, video_context.Get(), video_processor.Get());
        video_processor_wrapper->SetDriverSupportsVpAutoHdr(supports_vp_auto_hdr);
    }
    bool driver_supports_vp_auto_hdr = video_processor_wrapper->GetDriverSupportsVpAutoHdr();

    Microsoft::WRL::ComPtr<IDXGISwapChain3> swap_chain3;
    Microsoft::WRL::ComPtr<ID3D11VideoContext1> context1;
    if (SUCCEEDED(swap_chain_.As(&swap_chain3)) && SUCCEEDED(video_context.As(&context1))) {
        DCHECK(swap_chain3);
        DCHECK(context1);
        // Set input color space.
        context1->VideoProcessorSetStreamColorSpace1(video_processor.Get(), 0, gfx::ColorSpaceWin::GetDXGIColorSpace(src_color_space));
        // Set output color space.
        DXGI_COLOR_SPACE_TYPE output_dxgi_color_space = gfx::ColorSpaceWin::GetDXGIColorSpace(output_color_space,
            /*force_yuv=*/is_yuv_swapchain);
        DXGI_COLOR_SPACE_TYPE swap_dxgi_color_space
            = use_vp_auto_hdr ? gfx::ColorSpaceWin::GetDXGIColorSpace(gfx::ColorSpace::CreateHDR10()) : output_dxgi_color_space;
        ...
        context1->VideoProcessorSetOutputColorSpace1(video_processor.Get(), output_dxgi_color_space);
    } else {
        ...
        video_context->VideoProcessorSetStreamColorSpace(video_processor.Get(), 0, &src_d3d11_color_space);
        D3D11_VIDEO_PROCESSOR_COLOR_SPACE output_d3d11_color_space = gfx::ColorSpaceWin::GetD3D11ColorSpace(output_color_space);
        video_context->VideoProcessorSetOutputColorSpace(video_processor.Get(), &output_d3d11_color_space);
    }

    Microsoft::WRL::ComPtr<ID3D11VideoContext2> context2;
    std::optional<DXGI_HDR_METADATA_HDR10> display_metadata = layer_tree_->GetHDRMetadataHelper()->GetDisplayMetadata(layer_tree_->window());
    if (display_metadata.has_value() && SUCCEEDED(video_context.As(&context2))) {
        if (stream_hdr_metadata.has_value()) {
            context2->VideoProcessorSetStreamHDRMetaData(
                video_processor.Get(), 0, DXGI_HDR_METADATA_TYPE_HDR10, sizeof(DXGI_HDR_METADATA_HDR10), &(*stream_hdr_metadata));
        }

        context2->VideoProcessorSetOutputHDRMetaData(
            video_processor.Get(), DXGI_HDR_METADATA_TYPE_HDR10, sizeof(DXGI_HDR_METADATA_HDR10), &(*display_metadata));
    }

    {
        Microsoft::WRL::ComPtr<ID3D11VideoDevice> video_device = video_processor_wrapper->video_device;
        Microsoft::WRL::ComPtr<ID3D11VideoProcessorEnumerator> video_processor_enumerator = video_processor_wrapper->video_processor_enumerator;
        ...
        HRESULT hr = video_device->CreateVideoProcessorInputView(input_texture.Get(), video_processor_enumerator.Get(), &input_desc, &input_view);
        if (FAILED(hr)) {
            DLOG(ERROR) << "CreateVideoProcessorInputView failed with error 0x" << std::hex << hr;
            return false;
        }
        ...
        video_context->VideoProcessorSetOutputTargetRect(video_processor.Get(), TRUE, &dest_rect);
        ...
        if (!output_view_) {
            ...
            hr = video_device->CreateVideoProcessorOutputView(swap_chain_buffer.Get(), video_processor_enumerator.Get(), &output_desc, &output_view_);
            if (FAILED(hr)) {
                DLOG(ERROR) << "CreateVideoProcessorOutputView failed with error 0x" << std::hex << hr;
                return false;
            }
            DCHECK(output_view_);
        }
        ...
        if (enable_vp_auto_hdr_) {
            hr = ToggleVpAutoHDR(gpu_vendor_id_, driver_supports_vp_auto_hdr, video_context.Get(), video_processor.Get(), use_vp_auto_hdr);
            if (FAILED(hr)) {
                enable_vp_auto_hdr_ = false;

                if (use_vp_auto_hdr) {
                    if (!RevertSwapChainToSDR(video_device, video_processor, video_processor_enumerator, swap_chain3, context1, src_color_space)) {
                        return false;
                    }
        ...
        bool use_vp_super_resolution = enable_vp_super_resolution_ && !is_on_battery_power_;
        if (enable_vp_super_resolution_) {
            hr = ToggleVpSuperResolution(gpu_vendor_id_, video_context.Get(), video_processor.Get(), use_vp_super_resolution);
            if (FAILED(hr)) {
...

        {
            ...
            hr = video_context->VideoProcessorBlt(video_processor.Get(), output_view_.Get(), 0, 1, &stream);
        }
        base::UmaHistogramSparse((use_vp_auto_hdr ? "GPU.VideoProcessorBlt.VpAutoHDR.On" : "GPU.VideoProcessorBlt.VpAutoHDR.Off"), hr);
        base::UmaHistogramSparse((use_vp_super_resolution ? "GPU.VideoProcessorBlt.VpSuperResolution.On" : "GPU.VideoProcessorBlt.VpSuperResolution.Off"), hr);
        ...
        if (FAILED(hr) && use_vp_super_resolution) {
            ...
            ToggleVpSuperResolution(gpu_vendor_id_, video_context.Get(), video_processor.Get(), false);
            {
                TRACE_EVENT0("gpu", "ID3D11VideoContext::VideoProcessorBlt");
                hr = video_context->VideoProcessorBlt(video_processor.Get(), output_view_.Get(), 0, 1, &stream);
            }
            base::UmaHistogramSparse("GPU.VideoProcessorBlt.VpSuperResolution.RetryOffAfterError", hr);
        ...
        if (FAILED(hr) && use_vp_auto_hdr) {
            ...
            ToggleVpAutoHDR(gpu_vendor_id_, driver_supports_vp_auto_hdr, video_context.Get(), video_processor.Get(), false);
            ...
            base::UmaHistogramSparse("GPU.VideoProcessorBlt.VpAutoHDR.RetryOffAfterError", hr);
...
bool SwapChainPresenter::ReallocateSwapChain(const gfx::Size& swap_chain_size, DXGI_FORMAT swap_chain_format, gfx::ProtectedVideoType protected_video_type)
{
    bool use_yuv_swap_chain = IsYUVSwapChainFormat(swap_chain_format);

    TRACE_EVENT2("gpu", "SwapChainPresenter::ReallocateSwapChain", "size", swap_chain_size.ToString(), "yuv", use_yuv_swap_chain);
    ...
    desc.Flags = DXGI_SWAP_CHAIN_FLAG_YUV_VIDEO | DXGI_SWAP_CHAIN_FLAG_FULLSCREEN_VIDEO;
    if (DirectCompositionSwapChainTearingEnabled()) {
    ...
    const std::string kSwapChainCreationResultByVideoTypeUmaPrefix = "GPU.DirectComposition.SwapChainCreationResult3.";
    const std::string protected_video_type_string = ProtectedVideoTypeToString(protected_video_type);

    if (use_yuv_swap_chain) {
        TRACE_EVENT1("gpu", "SwapChainPresenter::ReallocateSwapChain::YUV", "format", DxgiFormatToString(swap_chain_format));
        HRESULT hr = media_factory->CreateSwapChainForCompositionSurfaceHandle(d3d11_device_.Get(), swap_chain_handle_.Get(), &desc, nullptr, &swap_chain_);
        failed_to_create_yuv_swapchain_ = FAILED(hr);

        base::UmaHistogramSparse(kSwapChainCreationResultByVideoTypeUmaPrefix + protected_video_type_string, hr);

        if (failed_to_create_yuv_swapchain_) {
            DLOG(ERROR) << "Failed to create " << DxgiFormatToString(swap_chain_format) << " swap chain of size " << swap_chain_size.ToString()
    ...
    if (!use_yuv_swap_chain) {
        TRACE_EVENT1("gpu", "SwapChainPresenter::ReallocateSwapChain::RGB", "format", DxgiFormatToString(swap_chain_format));
        ...
        if (FAILED(hr)) {
            ...
            DisableDirectCompositionOverlays();
            DLOG(ERROR) << "Failed to create " << DxgiFormatToString(swap_chain_format) << " swap chain of size " << swap_chain_size.ToString()
                        << " with error 0x" << std::hex << hr << ". Disable overlay swap chains";
            return false;
        }
    }
    ...
    if (DXGIWaitableSwapChainEnabled()) {
        ...
        if (SUCCEEDED(swap_chain_.As(&swap_chain3))) {
            HRESULT hr = swap_chain3->SetMaximumFrameLatency(GetDXGIWaitableSwapChainMaxQueuedFrames());
            DCHECK(SUCCEEDED(hr)) << "SetMaximumFrameLatency failed with error " << logging::SystemErrorCodeToString(hr);
        }
    }
    ...
    enable_vp_auto_hdr_ = !layer_tree_->disable_vp_auto_hdr() && IsVpAutoHDREnabled(gpu_vendor_id_);
    enable_vp_super_resolution_ = !layer_tree_->disable_vp_super_resolution();

    return true;
}

void SwapChainPresenter::OnBatteryPowerStatusChange(base::PowerStateObserver::BatteryPowerStatus battery_power_status)
{
    is_on_battery_power_ = (battery_power_status == base::PowerStateObserver::BatteryPowerStatus::kBatteryPower);
}
...
void SwapChainPresenter::SetSwapChainPresentDuration()
{
    ...
    if (swap_chain_media) {
        ...
        if (duration_100ns > 0) {
            ...
            HRESULT hr = swap_chain_media->CheckPresentDurationSupport(duration_100ns, &smaller_duration, &larger_duration);
            if (FAILED(hr)) {
                DLOG(ERROR) << "CheckPresentDurationSupport failed with error 0x" << std::hex << hr;
                return;
            }
            ...
            // estimation errors.
            if (larger_duration > 0 && larger_duration - duration_100ns < kDurationThreshold) {
        ...
        if (FAILED(hr)) {
            DLOG(ERROR) << "SetPresentDuration failed with error 0x" << std::hex << hr;
        }
    }
}

Microsoft::WRL::ComPtr<IDXGISwapChainMedia> SwapChainPresenter::GetSwapChainMedia() const
{
...
bool SwapChainPresenter::RevertSwapChainToSDR(Microsoft::WRL::ComPtr<ID3D11VideoDevice> video_device,
    Microsoft::WRL::ComPtr<ID3D11VideoProcessor> video_processor, Microsoft::WRL::ComPtr<ID3D11VideoProcessorEnumerator> video_processor_enumerator,
    Microsoft::WRL::ComPtr<IDXGISwapChain3> swap_chain3, Microsoft::WRL::ComPtr<ID3D11VideoContext1> context1, const gfx::ColorSpace& input_color_space)
{
    if (!video_device || !video_processor || !video_processor_enumerator || !swap_chain3 || !context1) {
        return false;
    }

    // Restore the SDR swap chain and output view
    if (!ReallocateSwapChain(gfx::Size(swap_chain_size_),
    ...
    output_desc.ViewDimension = D3D11_VPOV_DIMENSION_TEXTURE2D;
    output_desc.Texture2D.MipSlice = 0;
    HRESULT hr = video_device->CreateVideoProcessorOutputView(swap_chain_buffer.Get(), video_processor_enumerator.Get(), &output_desc, &output_view_);
    if (FAILED(hr)) {
        DLOG(ERROR) << "CreateVideoProcessorOutputView failed with error 0x" << std::hex << hr;
        return false;
    }
    ...
    bool is_yuv_swapchain = IsYUVSwapChainFormat(swap_chain_format_);
    gfx::ColorSpace output_color_space = GetOutputColorSpace(input_color_space, is_yuv_swapchain);
    DXGI_COLOR_SPACE_TYPE output_dxgi_color_space = gfx::ColorSpaceWin::GetDXGIColorSpace(output_color_space, is_yuv_swapchain);
    context1->VideoProcessorSetOutputColorSpace1(video_processor.Get(), output_dxgi_color_space);
