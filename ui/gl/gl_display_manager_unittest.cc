...
protected:
    ...
    public:
        ...
        ScopedGLDisplayManagerEGL& operator=(const ScopedGLDisplayManagerEGL&) = delete;

        GLDisplayManagerEGL* operator->()
        {
            return &manager_;
        }
...
TEST_F(GLDisplayManagerEGLTest, SingleGPU)
{
    ...
    GLDisplayEGL* display_low_power = manager->GetDisplay(GpuPreference::kLowPower);
    EXPECT_EQ(display, display_low_power);

    // Query the high performance display. It should be the same as the default.
    GLDisplayEGL* display_high_performance = manager->GetDisplay(GpuPreference::kHighPerformance);
...
TEST_F(GLDisplayManagerEGLTest, DualGPUs)
{
    ...
    // Query again should return the same display.
    GLDisplayEGL* display_low_power_2 = manager->GetDisplay(GpuPreference::kLowPower);
