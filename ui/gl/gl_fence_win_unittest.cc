...
public:
    void SetUp() override
    {
        ...
        ON_CALL(d3d11_device_, QueryInterface(IID_ID3D11Device5, _)).WillByDefault(media::SetComPointeeAndReturnOk<1>(&d3d11_device_));
        ON_CALL(d3d11_device_, GetImmediateContext(_)).WillByDefault(media::SetComPointee<0>(&d3d11_device_context_));

        ON_CALL(d3d11_device_context_, QueryInterface(IID_ID3D11DeviceContext4, _)).WillByDefault(media::SetComPointeeAndReturnOk<1>(&d3d11_device_context_));

        ON_CALL(d3d11_fence_, GetDevice(_)).WillByDefault(media::SetComPointee<0>(&d3d11_device_));
    }
...
{
    EXPECT_CALL(d3d11_device_, QueryInterface(IID_ID3D11Device5, _)).WillOnce(Return(E_NOINTERFACE));

    std::unique_ptr<GLFenceWin> gl_fence_win = GLFenceWin::CreateForGpuFence(&d3d11_device_);
...
TEST_F(GLFenceWinTest, CreateForGpuFence)
{
    ...
    EXPECT_CALL(d3d11_device_, CreateFence(0, D3D11_FENCE_FLAG_SHARED, IID_ID3D11Fence, _)).WillOnce(media::SetComPointeeAndReturnOk<3>(&d3d11_fence_));
    ...
    EXPECT_CALL(d3d11_fence_, CreateSharedHandle(nullptr, DXGI_SHARED_RESOURCE_READ | DXGI_SHARED_RESOURCE_WRITE, nullptr, _))
        .WillOnce(DoAll(SetArgPointee<3>(mock_handle), Return(S_OK)));

    // Ensure we signal the fence with 1 to match the wait in ServerWait.
    EXPECT_CALL(d3d11_device_context_, Signal(&d3d11_fence_, 1)).WillOnce(Return(S_OK));
...
TEST_F(GLFenceWinTest, CreateFromGpuFence)
{
    ...
    EXPECT_CALL(d3d11_device_, OpenSharedFence(_, IID_ID3D11Fence, _)).WillOnce(media::SetComPointeeAndReturnOk<2>(&d3d11_fence_));

    std::unique_ptr<GLFenceWin> gl_fence_win = GLFenceWin::CreateFromGpuFence(&d3d11_device_, gpu_fence);
    EXPECT_NE(gl_fence_win.get(), nullptr);
    ...
    EXPECT_CALL(d3d11_device_context_, Wait(&d3d11_fence_, 1)).WillOnce(Return(S_OK));
