...
ScopedEGLImage MakeScopedEGLImage(EGLContext context, EGLenum target, EGLClientBuffer buffer, const EGLint* attrs)
{
    EGLImageKHR egl_image = eglCreateImageKHR(GLSurfaceEGL::GetGLDisplayEGL()->GetDisplay(), context, target, buffer, attrs);
...
void DeleteEGLImageTraits::Free(EGLImageKHR image)
{
    ...
    const EGLBoolean result = eglDestroyImageKHR(GLSurfaceEGL::GetGLDisplayEGL()->GetDisplay(), image);
    if (result == EGL_FALSE)
