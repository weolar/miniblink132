...
NativeViewGLSurfaceEGLX11::NativeViewGLSurfaceEGLX11(GLDisplayEGL* display, x11::Window window)
...
bool NativeViewGLSurfaceEGLX11::Initialize(GLSurfaceFormat format)
{
    ...
    if (auto reply = connection->QueryTree({ static_cast<x11::Window>(window_) }).Sync()) {
        children_ = std::move(reply->children);
    }
    for (auto child : children_) {
        connection->ChangeWindowAttributes({ .window = child, .event_mask = x11::EventMask::Exposure });
    }
...
gfx::SwapResult NativeViewGLSurfaceEGLX11::SwapBuffers(PresentationCallback callback, gfx::FrameData data)
{
...
EGLint NativeViewGLSurfaceEGLX11::GetNativeVisualID() const
{
    ...
    GetXNativeConnection()->GetOrCreateVisualManager().ChooseVisualForWindow(true, &visual_id, nullptr, nullptr, nullptr);
...
std::unique_ptr<gfx::VSyncProvider> NativeViewGLSurfaceEGLX11::CreateVsyncProviderInternal()
{
...
void NativeViewGLSurfaceEGLX11::OnEvent(const x11::Event& x11_event)
{
    ...
    x11::Connection::Get()->SendEvent(expose_copy, window, x11::EventMask::Exposure);
