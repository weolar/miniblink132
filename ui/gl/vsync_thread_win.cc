...
Microsoft::WRL::ComPtr<IDXGIAdapter> FindDXGIAdapterOnNewFactory(const LUID luid)
{
    HRESULT hr = S_OK;
    ...
    for (uint32_t i = 0;; i++) {
        ...
        if (FAILED(hr)) {
            DLOG(ERROR) << "EnumAdapters1 failed: " << logging::SystemErrorCodeToString(hr);
            return nullptr;
        }
        ...
        if (new_adapter_desc.AdapterLuid.HighPart == luid.HighPart && new_adapter_desc.AdapterLuid.LowPart == luid.LowPart) {
            return new_adapter;
        }
    }
...
Microsoft::WRL::ComPtr<IDXGIOutput> DXGIOutputFromMonitor(HMONITOR monitor, IDXGIAdapter* dxgi_adapter)
{
    CHECK(dxgi_adapter);
    ...
    for (uint32_t i = 0;; i++) {
        ...
        if (FAILED(hr)) {
            DLOG(ERROR) << "EnumOutputs failed: " << logging::SystemErrorCodeToString(hr);
            return nullptr;
        }
...
public:
    AutoVSyncThreadLock(VSyncThreadWin* vsync_thread) EXCLUSIVE_LOCK_FUNCTION(vsync_thread->lock_)
    {
...
{
    is_suspended_ = base::PowerMonitor::GetInstance()->AddPowerSuspendObserverAndReturnSuspendedState(this);
    vsync_thread_.StartWithOptions(base::Thread::Options(base::ThreadType::kDisplayCritical));
}
...
void VSyncThreadWin::PostTaskIfNeeded()
{
    ...
    if (!is_vsync_task_posted_ && !observers_.empty() && !is_suspended_) {
        vsync_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&VSyncThreadWin::WaitForVSync, base::Unretained(this)));
...
base::TimeDelta VSyncThreadWin::GetVsyncInterval()
{
    ...
    static bool use_sv3_workaround
        = base::win::GetVersion() > base::win::Version::WIN11_22H2 && base::FeatureList::IsEnabled(features::kUsePrimaryMonitorVSyncIntervalOnSV3);

    const bool get_vsync_params_succeeded = use_sv3_workaround ? vsync_provider_.GetVSyncIntervalIfAvailable(&vsync_interval)
                                                               : vsync_provider_.GetVSyncParametersIfAvailable(&vsync_timebase, &vsync_interval);
...
void VSyncThreadWin::WaitForVSync()
{
    ...
    if (primary_output_ && !DXGIOutputIsOnMonitor(primary_output_.Get(), monitor)) {
    ...
    const bool wait_for_vblank_succeeded = primary_output_ && SUCCEEDED(primary_output_->WaitForVBlank());
    ...
    const base::TimeDelta wait_for_vblank_elapsed_time = base::TimeTicks::Now() - wait_for_vblank_start_time;
    if (!wait_for_vblank_succeeded || wait_for_vblank_elapsed_time < kVBlankIntervalThreshold) {
        TRACE_EVENT2("gpu", "WaitForVSync Sleep", "has adapter", !!dxgi_adapter_, "has output", !!primary_output_);
