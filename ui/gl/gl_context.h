...
// user WebGL content.
enum ContextPriority { ContextPriorityLow, ContextPriorityMedium, ContextPriorityHigh };
...
enum class AngleContextVirtualizationGroup { kDefault = -1, kDrDc = 1, kGLImageProcessor = 2, kWebViewRenderThread = 3 };
...
struct GL_EXPORT GLContextAttribs {
    ...
    AngleContextVirtualizationGroup angle_context_virtualization_group_number = AngleContextVirtualizationGroup::kDefault;
...
public:
    ...
    // needed for EGL.
    bool Initialize(GLSurface* compatible_surface, const GLContextAttribs& attribs);
...
#if BUILDFLAG(IS_APPLE)
    virtual void AddMetalSharedEventsForBackpressure(std::vector<std::unique_ptr<gpu::BackpressureMetalSharedEvent>> events);
    ...
    gl::GLSurface* default_surface() const
    {
        return default_surface_.get();
    }
...
protected:
    ...
    virtual bool InitializeImpl(GLSurface* compatible_surface, const GLContextAttribs& attribs) = 0;

    GLApi* gl_api()
    {
        return gl_api_wrapper_->api();
    }
...
#if BUILDFLAG(IS_APPLE)
    using GLFenceAndMetalSharedEvents = std::pair<std::unique_ptr<GLFence>, std::vector<std::unique_ptr<gpu::BackpressureMetalSharedEvent>>>;

    std::vector<std::unique_ptr<gpu::BackpressureMetalSharedEvent>> next_backpressure_events_;
...
GL_EXPORT scoped_refptr<GLContext> InitializeGLContext(scoped_refptr<GLContext> context, GLSurface* compatible_surface, const GLContextAttribs& attribs);
