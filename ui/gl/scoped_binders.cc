...
ScopedFramebufferBinder::ScopedFramebufferBinder(unsigned int fbo)
    : state_restorer_(!GLContext::GetCurrent() ? nullptr : GLContext::GetCurrent()->GetGLStateRestorer())
    , old_fbo_(-1)
{
    if (!state_restorer_)
...
ScopedVertexAttribArray::ScopedVertexAttribArray(unsigned int index, int size, unsigned int type, char normalized, int stride, const void* pointer)
    : state_restorer_(!GLContext::GetCurrent() ? nullptr : GLContext::GetCurrent()->GetGLStateRestorer())
    , buffer_(0)
    , enabled_(GL_FALSE)
    , index_(index)
