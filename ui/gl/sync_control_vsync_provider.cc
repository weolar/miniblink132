...
void SyncControlVSyncProvider::GetVSyncParameters(UpdateVSyncCallback callback)
{
...
bool SyncControlVSyncProvider::GetVSyncParametersIfAvailable(base::TimeTicks* timebase_out, base::TimeDelta* interval_out)
{
    ...
    int64_t real_time_in_microseconds = base::TimeDelta::FromTimeSpec(real_time).InMicroseconds();
    int64_t monotonic_time_in_microseconds = monotonic_time.since_origin().InMicroseconds();
    ...
    const base::TimeTicks timebase = base::TimeTicks() + base::Microseconds(system_time);
    ...
    if (last_computed_intervals_.size() == 2) {
        ...
        if (relative_change < kRelativeIntervalDifferenceThreshold) {
            if (new_interval.InMicroseconds() < kMinVsyncIntervalUs || new_interval.InMicroseconds() > kMaxVsyncIntervalUs) {
                ...
                LOG(ERROR) << "Calculated bogus refresh interval=" << new_interval << ", last_timebase_=" << last_timebase_ << ", timebase=" << timebase
                           << ", last_media_stream_counter_=" << last_media_stream_counter_ << ", media_stream_counter=" << media_stream_counter;
            } else {
