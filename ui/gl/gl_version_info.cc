...
GLVersionInfo::GLVersionInfo(const char* version_str, const char* renderer_str, const gfx::ExtensionSet& extensions)
{
    Initialize(version_str, renderer_str, extensions);
}

void GLVersionInfo::Initialize(const char* version_str, const char* renderer_str, const gfx::ExtensionSet& extensions)
{
    if (version_str) {
    ...
    if (renderer_str) {
        ...
        if (is_angle) {
            is_angle_swiftshader = renderer_string.find("SwiftShader Device") != std::string::npos;
...
void GLVersionInfo::ParseVersionString(const char* version_str)
{
    ...
    if (!lstr.starts_with(kESPrefix)) {
        LOG(FATAL) << "Chrome runs only on top of OpenGL ES "
                   << "through either ANGLE or native: "
                   << "VERSION = " << version_str;
    }
    lstr.remove_prefix(kESPrefix.size());
    std::vector<std::string_view> pieces = base::SplitStringPiece(lstr, " -()@", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    if (pieces.size() == 0) {
...
void GLVersionInfo::ParseDriverInfo(const char* version_str)
{
    ...
    // the driver version.
    const std::map<std::string_view, std::string_view> kVendors = { { "ANGLE", "ANGLE" }, { "Mesa", "Mesa" }, { "INTEL", "INTEL" }, { "NVIDIA", "NVIDIA" },
        { "ATI", "ATI" }, { "FireGL", "FireGL" }, { "Chromium", "Chromium" }, { "APPLE", "APPLE" }, { "AMD", "AMD" }, { "Metal", "Apple" } };
    ...
    if (pieces[1].starts_with(kMaliPrefix)) {
        ...
        std::vector<std::string_view> numbers = base::SplitStringPiece(pieces[1], "p", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        if (numbers.size() != 2)
            return;
        std::vector<std::string_view> parts = base::SplitStringPiece(pieces[2], ".", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        if (parts.size() != 2)
            return;
        driver_vendor = "ARM";
...
void GLVersionInfo::ExtractDriverVendorANGLE(const char* renderer_str)
{
    ...
    std::vector<std::string_view> gl_strings = base::SplitStringPiece(rstr, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    if (rstr.starts_with("NVIDIA ")) {
        ...
        std::vector<std::string_view> pieces = base::SplitStringPiece(rstr, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
GLVersionInfo::VersionStrings GLVersionInfo::GetFakeVersionStrings(unsigned major, unsigned minor) const
{
