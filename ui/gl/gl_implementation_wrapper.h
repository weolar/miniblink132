...
#define GL_IMPL_WRAPPER_TYPE(name) GLImplementationWrapper<name##Api, Trace##name##Api, Log##name##Api>

namespace gl {

// Wraps a GLApi with its tracing and logging variants when the corresponding
// command line flags are passed.
template <class GLImplApi, class GLTraceImplApi, class GLLogImplApi> class GLImplementationWrapper {
public:
        ...
        : real_gl_(std::move(real_gl))
    {
        ...
        static bool enable_tracing = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableGPUServiceTracing);
        if (enable_tracing) {
            trace_gl_ = std::make_unique<GLTraceImplApi>(gl_api_);
            gl_api_ = trace_gl_.get();
        }

        static bool enable_logging = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableGPUServiceLogging);
        if (enable_logging) {
