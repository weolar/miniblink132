...
public:
    ...
    virtual void GetPlatformExtraDisplayAttribs(EGLenum platform_type, std::vector<EGLAttrib>* attributes) = 0;

    // Sets custom alpha and buffer size for a given platform. By default, the
    // values are not modified.
    virtual void ChoosePlatformCustomAlphaAndBufferSize(EGLint* alpha_size, EGLint* buffer_size) = 0;

    // X11 specific; returns scoped unset display env variable if vulkan surface
    // is not supported.
    virtual std::optional<base::ScopedEnvironmentVariableOverride> MaybeGetScopedDisplayUnsetForVulkan() = 0;
