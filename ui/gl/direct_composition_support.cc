...
{
    return base::FeatureList::IsEnabled(features::kDirectCompositionSoftwareOverlays) && !g_disable_sw_overlays;
}
...
bool CheckOverlayColorSpaceSupport(DXGI_FORMAT dxgi_format, DXGI_COLOR_SPACE_TYPE dxgi_color_space, Microsoft::WRL::ComPtr<IDXGIOutput> output,
    Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device)
{
    UINT color_space_support_flags = 0;
    Microsoft::WRL::ComPtr<IDXGIOutput4> output4;
    if (FAILED(output.As(&output4))
        || FAILED(output4->CheckOverlayColorSpaceSupport(dxgi_format, dxgi_color_space, d3d11_device.Get(), &color_space_support_flags)))
        return false;
    return (color_space_support_flags & DXGI_OVERLAY_COLOR_SPACE_SUPPORT_FLAG_PRESENT);
}
...
void SetOverlaySupportFlagsForFormats(UINT nv12_flags, UINT yuy2_flags, UINT bgra8_flags, UINT rgb10a2_flags, UINT p010_flags)
{
    base::AutoLock auto_lock(GetOverlayLock());
...
{
    return (flags & (DXGI_OVERLAY_SUPPORT_FLAG_DIRECT | DXGI_OVERLAY_SUPPORT_FLAG_SCALING));
}

void GetGpuDriverOverlayInfo(bool* supports_overlays, bool* supports_hardware_overlays, DXGI_FORMAT* overlay_format_used, DXGI_FORMAT* overlay_format_used_hdr,
    UINT* nv12_overlay_support_flags, UINT* yuy2_overlay_support_flags, UINT* bgra8_overlay_support_flags, UINT* rgb10a2_overlay_support_flags,
    ...
    *p010_overlay_support_flags = 0;
    ...
    while (true) {
        ...
        if (FAILED(output.As(&output3)))
            continue;
        DCHECK(output3);
        output3->CheckOverlaySupport(DXGI_FORMAT_NV12, d3d11_device.Get(), nv12_overlay_support_flags);
        output3->CheckOverlaySupport(DXGI_FORMAT_YUY2, d3d11_device.Get(), yuy2_overlay_support_flags);
        output3->CheckOverlaySupport(DXGI_FORMAT_B8G8R8A8_UNORM, d3d11_device.Get(), bgra8_overlay_support_flags);
        // Today it still returns false, which blocks Chrome from using HDR
        // overlays.
        output3->CheckOverlaySupport(DXGI_FORMAT_R10G10B10A2_UNORM, d3d11_device.Get(), rgb10a2_overlay_support_flags);
        output3->CheckOverlaySupport(DXGI_FORMAT_P010, d3d11_device.Get(), p010_overlay_support_flags);
        if (FlagsSupportsOverlays(*nv12_overlay_support_flags)) {
            ...
            *supports_hardware_overlays = true;
            ...
            if (g_check_ycbcr_studio_g22_left_p709_for_nv12_support
                && !CheckOverlayColorSpaceSupport(DXGI_FORMAT_NV12, DXGI_COLOR_SPACE_YCBCR_STUDIO_G22_LEFT_P709, output, d3d11_device)) {
        ...
        if (!*supports_hardware_overlays && FlagsSupportsOverlays(*yuy2_overlay_support_flags)) {
        ...
        if (FlagsSupportsOverlays(*rgb10a2_overlay_support_flags)) {
            if (!CheckOverlayColorSpaceSupport(DXGI_FORMAT_R10G10B10A2_UNORM, DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020, output, d3d11_device))
                *rgb10a2_overlay_support_flags = 0;
        }
...
void UpdateOverlaySupport()
{
    ...
    GetGpuDriverOverlayInfo(&supports_overlays, &supports_hardware_overlays, &overlay_format_used, &overlay_format_used_hdr, &nv12_overlay_support_flags,
        &yuy2_overlay_support_flags, &bgra8_overlay_support_flags, &rgb10a2_overlay_support_flags, &p010_overlay_support_flags);
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDirectCompositionVideoSwapChainFormat)) {
        std::string override_format = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kDirectCompositionVideoSwapChainFormat);
        if (override_format == kSwapChainFormatNV12) {
            ...
            LOG(ERROR) << "Invalid value for switch " << switches::kDirectCompositionVideoSwapChainFormat;
        }
    }

    // Record histograms.
    if (supports_overlays) {
        base::UmaHistogramSparse("GPU.DirectComposition.OverlayFormatUsed3", overlay_format_used);
    }
    base::UmaHistogramBoolean("GPU.DirectComposition.OverlaysSupported", supports_overlays);
    base::UmaHistogramBoolean("GPU.DirectComposition.HardwareOverlaysSupported", supports_hardware_overlays);
    ...
    SetOverlaySupportFlagsForFormats(
        nv12_overlay_support_flags, yuy2_overlay_support_flags, bgra8_overlay_support_flags, rgb10a2_overlay_support_flags, p010_overlay_support_flags);
...
std::vector<DXGI_OUTPUT_DESC1> GetDirectCompositionOutputDescs()
{
    ...
    if (FAILED(hr)) {
        LOG(ERROR) << "CreateDXGIFactory1 failed: " << logging::SystemErrorCodeToString(hr);
        return output_descs;
    }
    ...
    for (UINT adapter_index = 0;; ++adapter_index) {
        ...
        if (FAILED(hr)) {
            LOG(ERROR) << "Unexpected error creating DXGI adapter, EnumAdapters failed: " << logging::SystemErrorCodeToString(hr);
            break;
        }
        ...
        for (UINT output_index = 0;; ++output_index) {
            ...
            if (FAILED(hr)) {
                LOG(ERROR) << "Unexpected error creating DXGI adapter, EnumOutputs failed: " << logging::SystemErrorCodeToString(hr);
                break;
            }
            ...
            if (FAILED(hr)) {
                LOG(ERROR) << "Unexpected error getting output descriptor: " << logging::SystemErrorCodeToString(hr);
                continue;
            }
...
void UpdateMonitorInfo()
{
    ...
    if (GetMonitorInfo(MonitorFromWindow(nullptr, MONITOR_DEFAULTTOPRIMARY), &monitor_info)) {
...
void UpdateVideoProcessorAutoHDRSupport()
{
    ...
    hr = d3d11_video_device->CreateVideoProcessorEnumerator(&desc, &d3d11_video_enumerator);
    if (FAILED(hr)) {
        LOG(ERROR) << "CreateVideoProcessorEnumerator failed: " << logging::SystemErrorCodeToString(hr);
        SetSupportsVideoProcessorAutoHDR(false);
        return;
    }

    Microsoft::WRL::ComPtr<ID3D11VideoProcessor> d3d11_video_processor;
    hr = d3d11_video_device->CreateVideoProcessor(d3d11_video_enumerator.Get(), 0, &d3d11_video_processor);
    if (FAILED(hr)) {
        LOG(ERROR) << "CreateVideoProcessor failed: " << logging::SystemErrorCodeToString(hr);
        SetSupportsVideoProcessorAutoHDR(false);
        return;
    }
    ...
    hr = d3d11_video_context->VideoProcessorGetStreamExtension(
        d3d11_video_processor.Get(), 0, &kNvidiaTrueHDRInterfaceGUID, sizeof(driver_supports_true_hdr), &driver_supports_true_hdr);
    if (FAILED(hr)) {
        LOG(ERROR) << "VideoProcessorGetStreamExtension failed: " << logging::SystemErrorCodeToString(hr);
        SetSupportsVideoProcessorAutoHDR(false);
        return;
    }
...
void InitializeDirectComposition(Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device)
{
    ...
    using PFN_DCOMPOSITION_CREATE_DEVICE3 = HRESULT(WINAPI*)(IUnknown * renderingDevice, REFIID iid, void** dcompositionDevice);
    PFN_DCOMPOSITION_CREATE_DEVICE3 create_device3_function
        = reinterpret_cast<PFN_DCOMPOSITION_CREATE_DEVICE3>(::GetProcAddress(dcomp_module, "DCompositionCreateDevice3"));
    ...
    HRESULT hr = create_device3_function(dxgi_device.Get(), IID_PPV_ARGS(&desktop_device));
    if (FAILED(hr)) {
        LOG(ERROR) << "DCompositionCreateDevice3 failed: " << logging::SystemErrorCodeToString(hr);
        return;
    }
    ...
    if (FAILED(hr)) {
        LOG(ERROR) << "Failed to retrieve IDCompositionDevice3: " << logging::SystemErrorCodeToString(hr);
        return;
    }
...
void DisableDirectCompositionOverlays()
{
    ...
    DirectCompositionOverlayCapsMonitor::GetInstance()->NotifyOverlayCapsChanged();
}
...
bool DirectCompositionScaledOverlaysSupported()
{
    ...
    if (g_overlay_format_used == DXGI_FORMAT_NV12) {
        return (g_nv12_overlay_support_flags & DXGI_OVERLAY_SUPPORT_FLAG_SCALING) || (SupportsOverlays() && SupportsSoftwareOverlays());
...
bool CheckVideoProcessorFormatSupport(DXGI_FORMAT dxgi_format)
{
    ...
    if (FAILED(hr)) {
        LOG(ERROR) << "CheckFormatSupport failed: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
    ...
    if (FAILED(hr)) {
        LOG(ERROR) << "CheckVideoProcessorFormat failed: " << logging::SystemErrorCodeToString(hr);
        video_enumerator.Reset();
        return false;
    }

    video_enumerator.Reset();
    return (enumerator & D3D11_VIDEO_PROCESSOR_FORMAT_SUPPORT_OUTPUT) && (device & D3D11_FORMAT_SUPPORT_VIDEO_PROCESSOR_OUTPUT);
}
...
bool CheckDisplayableSupportForP010()
{
    static const bool p010_displayable = [] {
        ...
        if (!SUCCEEDED(d3d11_device->CheckFeatureSupport(D3D11_FEATURE_FORMAT_SUPPORT2, &supported_format, sizeof(supported_format)))) {
...
bool DirectCompositionMonitorHDREnabled(HWND window)
{
    ...
    return GetHDRMonitors()->find(MonitorFromWindow(window, MONITOR_DEFAULTTONEAREST)) != GetHDRMonitors()->end();
}
...
{
    DCHECK(format == DXGI_FORMAT_NV12 || format == DXGI_FORMAT_YUY2 || format == DXGI_FORMAT_B8G8R8A8_UNORM || format == DXGI_FORMAT_P010);
    UpdateOverlaySupport();
...
gfx::mojom::DXGIInfoPtr GetDirectCompositionHDRMonitorDXGIInfo()
{
    ...
    for (const auto& desc : GetDirectCompositionOutputDescs()) {
        ...
        result_output->hdr_enabled = desc.ColorSpace == DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020;
...
bool DXGISwapChainTearingSupported()
{
    static const bool supported = [] {
        ...
        DCHECK(dxgi_factory);
        if (FAILED(dxgi_factory->CheckFeatureSupport(DXGI_FEATURE_PRESENT_ALLOW_TEARING, &present_allow_tearing, sizeof(present_allow_tearing)))) {
            LOG(ERROR) << "Not using swap chain tearing because CheckFeatureSupport failed";
            return false;
        }
...
{
    return static_cast<UINT>(features::kDXGIWaitableSwapChainMaxQueuedFrames.Get());
}

void SetDirectCompositionOverlayWorkarounds(const DirectCompositionOverlayWorkarounds& workarounds)
{
    ...
    g_enable_bgra8_overlays_with_yuv_overlay_support = workarounds.enable_bgra8_overlays_with_yuv_overlay_support;
    ...
    g_check_ycbcr_studio_g22_left_p709_for_nv12_support = workarounds.check_ycbcr_studio_g22_left_p709_for_nv12_support;
}
...
void SetDirectCompositionMonitorInfoForTesting(int num_monitors, const gfx::Size& primary_monitor_size)
{
...
bool DirectCompositionTextureSupported()
{
    ...
    if (FAILED(hr)) {
        ...
        LOG(ERROR) << "QueryInterface to IDCompositionDevice4 failed: " << logging::SystemErrorCodeToString(hr);
        return false;
    }

    BOOL supports_composition_textures = FALSE;
    hr = dcomp_device4->CheckCompositionTextureSupport(d3d11_device.Get(), &supports_composition_textures);
    if (FAILED(hr)) {
        LOG(ERROR) << "CheckCompositionTextureSupport failed: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
...
DirectCompositionOverlayCapsMonitor::DirectCompositionOverlayCapsMonitor()
    : observer_list_(new base::ObserverListThreadSafe<DirectCompositionOverlayCapsObserver>())
{
...
// static
DirectCompositionOverlayCapsMonitor* DirectCompositionOverlayCapsMonitor::GetInstance()
{
    static base::NoDestructor<DirectCompositionOverlayCapsMonitor> direct_compoisition_overlay_cap_monitor;
    return direct_compoisition_overlay_cap_monitor.get();
}

void DirectCompositionOverlayCapsMonitor::AddObserver(DirectCompositionOverlayCapsObserver* observer)
{
    observer_list_->AddObserver(observer);
}

void DirectCompositionOverlayCapsMonitor::RemoveObserver(DirectCompositionOverlayCapsObserver* observer)
{
...
{
    observer_list_->Notify(FROM_HERE, &DirectCompositionOverlayCapsObserver::OnOverlayCapsChanged);
}

// Called from GpuSwitchingObserver on the GPU main thread.
void DirectCompositionOverlayCapsMonitor::OnGpuSwitched(gl::GpuPreference active_gpu_heuristic)
{
}
