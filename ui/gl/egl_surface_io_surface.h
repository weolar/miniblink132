...
public:
    ...
    static std::unique_ptr<ScopedEGLSurfaceIOSurface> Create(
        EGLDisplay display, unsigned gl_target, IOSurfaceRef io_surface, uint32_t plane, gfx::BufferFormat format);
...
private:
    ...
    bool CreatePBuffer(IOSurfaceRef io_surface, uint32_t plane, gfx::BufferFormat format);
    void DestroyPBuffer();
