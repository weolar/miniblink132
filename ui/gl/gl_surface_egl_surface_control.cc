...
{
    return base::StrCat({ base::android::BuildInfo::GetInstance()->package_name(), "/", suffix });
}
...
GLSurfaceEGLSurfaceControl::GLSurfaceEGLSurfaceControl(gl::ScopedANativeWindow window, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : GLSurfaceEGLSurfaceControl(
        base::MakeRefCounted<gfx::SurfaceControl::Surface>(window.a_native_window(), BuildSurfaceName(kRootSurfaceName).c_str()), std::move(task_runner))
{
}

GLSurfaceEGLSurfaceControl::GLSurfaceEGLSurfaceControl(
    gl::ScopedJavaSurfaceControl scoped_java_surface_control, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : GLSurfaceEGLSurfaceControl(scoped_java_surface_control.MakeSurface(), std::move(task_runner))
{
}

GLSurfaceEGLSurfaceControl::GLSurfaceEGLSurfaceControl(
    scoped_refptr<gfx::SurfaceControl::Surface> root_surface, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    ...
    , using_on_commit_callback_(!use_target_deadline_ && gfx::SurfaceControl::SupportsOnCommit())
{
}
...
{
    TRACE_EVENT_INSTANT0("gpu", "GLSurfaceEGLSurfaceControl::PreserveChildSurfaceControls", TRACE_EVENT_SCOPE_THREAD);
...
bool GLSurfaceEGLSurfaceControl::Resize(const gfx::Size& size, float scale_factor, const gfx::ColorSpace& color_space, bool has_alpha)
{
...
void GLSurfaceEGLSurfaceControl::Present(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, gfx::FrameData data)
{
    CommitPendingTransaction(std::move(completion_callback), std::move(presentation_callback));
}

void GLSurfaceEGLSurfaceControl::CommitPendingTransaction(SwapCompletionCallback completion_callback, PresentationCallback present_callback)
{
    ...
    if (!pending_transaction_ || surface_lost_) {
        ...
        surface_lost_ = true;
        std::move(completion_callback).Run(gfx::SwapCompletionResult(gfx::SwapResult::SWAP_FAILED));
    ...
    for (size_t i = 0; i < pending_surfaces_count_; ++i) {
        ...
        pending_transaction_->SetDamageRect(*surface_state.surface, gfx::Rect(GetBufferSize(surface_state.hardware_buffer)));
    }
    ...
    for (size_t i = pending_surfaces_count_; i < surface_list_.size(); ++i) {
        ...
        if (surface_state.hardware_buffer) {
            pending_transaction_->SetBuffer(*surface_state.surface, nullptr, base::ScopedFD());
    ...
    OnTransactionAckArgs::SequenceId ack_id = pending_transaction_ack_id_generator_.GenerateNextId();
    pending_transaction_acks_.emplace_back(
        ack_id, std::move(completion_callback), std::move(present_callback), std::move(resources_to_release), std::move(primary_plane_fences_));
    gfx::SurfaceControl::Transaction::OnCompleteCb complete_cb
        = base::BindOnce(&GLSurfaceEGLSurfaceControl::OnTransactionAckOnGpuThread, weak_factory_.GetWeakPtr(), ack_id);
    primary_plane_fences_.reset();
    pending_transaction_->SetOnCompleteCb(std::move(complete_cb), gpu_task_runner_);

    if (use_target_deadline_) {
        DCHECK(!!choreographer_vsync_id_for_next_frame_);
        DCHECK(gfx::SurfaceControl::SupportsSetFrameTimeline());
        pending_transaction_->SetFrameTimelineId(choreographer_vsync_id_for_next_frame_.value());
        choreographer_vsync_id_for_next_frame_.reset();
    }

    if (using_on_commit_callback_) {
        gfx::SurfaceControl::Transaction::OnCommitCb commit_cb
            = base::BindOnce(&GLSurfaceEGLSurfaceControl::OnTransactionCommittedOnGpuThread, weak_factory_.GetWeakPtr());
...
bool GLSurfaceEGLSurfaceControl::ScheduleOverlayPlane(
    OverlayImage image, std::unique_ptr<gfx::GpuFence> gpu_fence, const gfx::OverlayPlaneData& overlay_plane_data)
{
    if (surface_lost_) {
    ...
    if (uninitialized || surface_state.z_order != overlay_plane_data.z_order) {
        ...
        pending_transaction_->SetZOrder(*surface_state.surface, overlay_plane_data.z_order);
    }
    ...
    if (scoped_hardware_buffer) {
        ...
        if (is_primary_plane) {
            ...
            primary_plane_fences_->available_fence = scoped_hardware_buffer->TakeAvailableFence();
        }
    ...
    if (uninitialized || surface_state.hardware_buffer != hardware_buffer || gpu_fence) {
        ...
        if (is_primary_plane) {
            primary_plane_fences_->ready_fence = base::ScopedFD(HANDLE_EINTR(dup(fence_fd.get())));
        }

        pending_transaction_->SetBuffer(*surface_state.surface, surface_state.hardware_buffer, std::move(fence_fd));
    }

    if (hardware_buffer) {
        gfx::Size buffer_size = GetBufferSize(hardware_buffer);
        gfx::RectF scaled_rect = gfx::ScaleRect(overlay_plane_data.crop_rect, buffer_size.width(), buffer_size.height());
        ...
        auto transform = absl::get<gfx::OverlayTransform>(overlay_plane_data.plane_transform);
        if (uninitialized || surface_state.src != src || surface_state.dst != dst || surface_state.transform != transform) {
            surface_state.src = src;
            surface_state.dst = dst;
            surface_state.transform = transform;
            pending_transaction_->SetGeometry(*surface_state.surface, src, dst, transform);
        }
    }
    ...
    if (!gfx::SurfaceControl::SupportsColorSpace(image_color_space)) {
        LOG(DFATAL) << "Not supported color space used with overlay : " << image_color_space.ToString();
    }

    if (uninitialized || surface_state.color_space != image_color_space || surface_state.hdr_metadata != overlay_plane_data.hdr_metadata) {
        surface_state.color_space = image_color_space;
        surface_state.hdr_metadata = overlay_plane_data.hdr_metadata;
        pending_transaction_->SetColorSpace(*surface_state.surface, image_color_space, surface_state.hdr_metadata);
    }
...
void GLSurfaceEGLSurfaceControl::OnTransactionAckOnGpuThread(OnTransactionAckArgs::SequenceId id, gfx::SurfaceControl::TransactionStats transaction_stats)
{
    ...
    while (!pending_transaction_acks_.empty()) {
        ...
        OrderedOnTransactionAckOnGpuThread(std::move(args.completion_callback), std::move(args.presentation_callback), std::move(args.released_resources),
            std::move(args.primary_plane_fences), std::move(args.transaction_stats.value()));
        pending_transaction_acks_.pop_front();
    }
}

void GLSurfaceEGLSurfaceControl::OrderedOnTransactionAckOnGpuThread(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback,
    ResourceRefs released_resources, std::optional<PrimaryPlaneFences> primary_plane_fences, gfx::SurfaceControl::TransactionStats transaction_stats)
{
    TRACE_EVENT0("gpu", "GLSurfaceEGLSurfaceControl::OnTransactionAckOnGpuThread");
    ...
    std::move(completion_callback).Run(gfx::SwapCompletionResult(gfx::SwapResult::SWAP_ACK));

    PendingPresentationCallback pending_cb;
    if (primary_plane_fences) {
        pending_cb.available_time = GetSignalTime(primary_plane_fences->available_fence);
...
{
    TRACE_EVENT0("gpu", "GLSurfaceEGLSurfaceControl::OnTransactionCommittedOnGpuThread");
...
{
    TRACE_EVENT0("gpu", "GLSurfaceEGLSurfaceControl::CheckPendingPresentationCallbacks");
    ...
    while (!pending_presentation_callback_queue_.empty()) {
        ...
        base::TimeTicks signal_time;
        auto status
            = pending_cb.present_fence.is_valid() ? gfx::GpuFence::GetStatusChangeTime(pending_cb.present_fence.get(), &signal_time) : gfx::GpuFence::kInvalid;
        if (status == gfx::GpuFence::kNotSignaled)
            break;

        auto flags = gfx::PresentationFeedback::kHWCompletion | gfx::PresentationFeedback::kVSync;
    ...
    if (!pending_presentation_callback_queue_.empty() && pending_transaction_queue_.empty() && !num_transaction_commit_or_ack_pending_) {
        check_pending_presentation_callback_queue_task_.Reset(
            base::BindOnce(&GLSurfaceEGLSurfaceControl::CheckPendingPresentationCallbacks, weak_factory_.GetWeakPtr()));
        gpu_task_runner_->PostDelayedTask(FROM_HERE, check_pending_presentation_callback_queue_task_.callback(), base::Seconds(1) / 60);
    }
}
...
void GLSurfaceEGLSurfaceControl::SetChoreographerVsyncIdForNextFrame(std::optional<int64_t> choreographer_vsync_id)
{
    choreographer_vsync_id_for_next_frame_ = choreographer_vsync_id;
}

GLSurfaceEGLSurfaceControl::SurfaceState::SurfaceState(const gfx::SurfaceControl::Surface& parent, const std::string& name)
    : surface(new gfx::SurfaceControl::Surface(parent, name.c_str()))
{
}

GLSurfaceEGLSurfaceControl::SurfaceState::SurfaceState() = default;
GLSurfaceEGLSurfaceControl::SurfaceState::SurfaceState(SurfaceState&& other) = default;
GLSurfaceEGLSurfaceControl::SurfaceState& GLSurfaceEGLSurfaceControl::SurfaceState::operator=(SurfaceState&& other) = default;

GLSurfaceEGLSurfaceControl::SurfaceState::~SurfaceState() = default;

GLSurfaceEGLSurfaceControl::ResourceRef::ResourceRef() = default;
GLSurfaceEGLSurfaceControl::ResourceRef::~ResourceRef() = default;
GLSurfaceEGLSurfaceControl::ResourceRef::ResourceRef(ResourceRef&& other) = default;
GLSurfaceEGLSurfaceControl::ResourceRef& GLSurfaceEGLSurfaceControl::ResourceRef::operator=(ResourceRef&& other) = default;

GLSurfaceEGLSurfaceControl::PendingPresentationCallback::PendingPresentationCallback() = default;
GLSurfaceEGLSurfaceControl::PendingPresentationCallback::~PendingPresentationCallback() = default;
GLSurfaceEGLSurfaceControl::PendingPresentationCallback::PendingPresentationCallback(PendingPresentationCallback&& other) = default;
GLSurfaceEGLSurfaceControl::PendingPresentationCallback& GLSurfaceEGLSurfaceControl::PendingPresentationCallback::operator=(PendingPresentationCallback&& other)
    = default;

GLSurfaceEGLSurfaceControl::PrimaryPlaneFences::PrimaryPlaneFences() = default;
GLSurfaceEGLSurfaceControl::PrimaryPlaneFences::~PrimaryPlaneFences() = default;
GLSurfaceEGLSurfaceControl::PrimaryPlaneFences::PrimaryPlaneFences(PrimaryPlaneFences&& other) = default;
GLSurfaceEGLSurfaceControl::PrimaryPlaneFences& GLSurfaceEGLSurfaceControl::PrimaryPlaneFences::operator=(PrimaryPlaneFences&& other) = default;

GLSurfaceEGLSurfaceControl::TransactionAckTimeoutManager::TransactionAckTimeoutManager(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    : gpu_task_runner_(std::move(task_runner))
{
}
GLSurfaceEGLSurfaceControl::TransactionAckTimeoutManager::~TransactionAckTimeoutManager() = default;

void GLSurfaceEGLSurfaceControl::TransactionAckTimeoutManager::ScheduleHangDetection()
{
    DCHECK(gpu_task_runner_->BelongsToCurrentThread());
    ...
    hang_detection_cb_.Reset(
        base::BindOnce(&GLSurfaceEGLSurfaceControl::TransactionAckTimeoutManager::OnTransactionTimeout, base::Unretained(this), current_transaction_id_));
    gpu_task_runner_->PostDelayedTask(FROM_HERE, hang_detection_cb_.callback(), base::Seconds(kIdleDelaySeconds));
}

void GLSurfaceEGLSurfaceControl::TransactionAckTimeoutManager::OnTransactionAck()
{
...
void GLSurfaceEGLSurfaceControl::TransactionAckTimeoutManager::OnTransactionTimeout(TransactionId transaction_id)
{
...
GLSurfaceEGLSurfaceControl::OnTransactionAckArgs::OnTransactionAckArgs(SequenceId id, SwapCompletionCallback completion_callback,
    PresentationCallback presentation_callback, ResourceRefs released_resources, std::optional<PrimaryPlaneFences> primary_plane_fences)
    : id(id)
...
GLSurfaceEGLSurfaceControl::OnTransactionAckArgs::OnTransactionAckArgs(OnTransactionAckArgs&& other) = default;
GLSurfaceEGLSurfaceControl::OnTransactionAckArgs& GLSurfaceEGLSurfaceControl::OnTransactionAckArgs::operator=(
    GLSurfaceEGLSurfaceControl::OnTransactionAckArgs&& other)
    = default;
GLSurfaceEGLSurfaceControl::OnTransactionAckArgs::~OnTransactionAckArgs() = default;
