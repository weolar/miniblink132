...
public:
    explicit HDRMetadataHelperWin(Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device);
    ...
    static DXGI_HDR_METADATA_HDR10 HDRMetadataToDXGI(const gfx::HDRMetadata& hdr_metadata);

    // Convert |desc1| to DXGI's metadata format.
    static DXGI_HDR_METADATA_HDR10 OutputDESC1ToDXGI(const DXGI_OUTPUT_DESC1& desc1);
