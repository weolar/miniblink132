...
bool GLFence::IsSupported()
{
    ...
    return g_current_gl_version->is_es3 || (display && display->ext->b_EGL_KHR_fence_sync) || g_current_gl_driver->ext.b_GL_NV_fence;
}
...
std::unique_ptr<GLFence> GLFence::Create()
{
    ...
    GLDisplayEGL* display = GLDisplayEGL::GetDisplayForCurrentContext();
    if (display && display->ext->b_EGL_KHR_fence_sync && display->ext->b_EGL_KHR_wait_sync) {
...
bool GLFence::IsGpuFenceSupported()
{
    ...
    return gl::GLSurfaceEGL::GetGLDisplayEGL()->IsAndroidNativeFenceSyncSupported();
...
// static
std::unique_ptr<GLFence> GLFence::CreateFromGpuFence(const gfx::GpuFence& gpu_fence)
{
