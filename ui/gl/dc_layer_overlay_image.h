...
public:
    DCLayerOverlayImage(const gfx::Size& size, Microsoft::WRL::ComPtr<ID3D11Texture2D> d3d11_video_texture, size_t array_slice = 0u);
    DCLayerOverlayImage(const gfx::Size& size, const uint8_t* shm_video_pixmap, size_t stride);
    DCLayerOverlayImage(const gfx::Size& size, Microsoft::WRL::ComPtr<IUnknown> dcomp_visual_content, uint64_t dcomp_surface_serial = 0u);
    DCLayerOverlayImage(const gfx::Size& size, scoped_refptr<gl::DCOMPSurfaceProxy> dcomp_surface_proxy);
    DCLayerOverlayImage(DCLayerOverlayImage&&);
    DCLayerOverlayImage& operator=(DCLayerOverlayImage&&);
    ~DCLayerOverlayImage();

    DCLayerOverlayType type() const
    {
        return type_;
    }
    ...
    size_t texture_array_slice() const
    {
        return texture_array_slice_;
    }

    const uint8_t* shm_video_pixmap() const
    {
        return shm_video_pixmap_;
    }
    size_t pixmap_stride() const
    {
        return pixmap_stride_;
    }
    ...
    uint64_t dcomp_surface_serial() const
    {
        return dcomp_surface_serial_;
    }
    ...
    {
        return std::tie(type_, size_, d3d11_video_texture_, texture_array_slice_, shm_video_pixmap_, pixmap_stride_, dcomp_visual_content_,
                   dcomp_surface_serial_, dcomp_surface_proxy_)
            == std::tie(other.type_, other.size_, other.d3d11_video_texture_, other.texture_array_slice_, other.shm_video_pixmap_, other.pixmap_stride_,
                other.dcomp_visual_content_, other.dcomp_surface_serial_, other.dcomp_surface_proxy_);
    }
