...
// otherwise.
bool ChangeContextAttributes(std::vector<EGLint>& context_attributes, EGLint attribute, EGLint value)
{
...
bool IsARMSwiftShaderPlatform()
{
    ...
    base::win::OSInfo::WindowsArchitecture windows_architecture = base::win::OSInfo::GetInstance()->GetArchitecture();
    base::win::OSInfo* os_info = base::win::OSInfo::GetInstance();
    return windows_architecture == base::win::OSInfo::ARM64_ARCHITECTURE || os_info->IsWowX86OnARM64() || os_info->IsWowAMD64OnARM64();
#else
...
bool GLContextEGL::InitializeImpl(GLSurface* compatible_surface, const GLContextAttribs& attribs)
{
    ...
    if (!gl_display_->ext->b_EGL_KHR_no_config_context) {
        ...
        if (!config_) {
            LOG(ERROR) << "Failed to get config for surface " << compatible_surface->GetHandle();
            return false;
        }
        EGLint config_renderable_type = 0;
        if (!eglGetConfigAttrib(gl_display_->GetDisplay(), config_, EGL_RENDERABLE_TYPE, &config_renderable_type)) {
            LOG(ERROR) << "eglGetConfigAttrib failed with error " << GetLastEGLErrorString();
            return false;
        }

        // If the requested context is ES3 but the config cannot support ES3,
        // request ES2 instead.
        if ((config_renderable_type & EGL_OPENGL_ES3_BIT) == 0 && context_client_major_version >= 3) {
    ...
    if (attribs.can_skip_validation && GetGLImplementation() == kGLImplementationEGLANGLE) {
    ...
    if (attribs.webgl_compatibility_context && is_swangle && IsARMSwiftShaderPlatform()
        && !features::IsSwiftShaderAllowedByCommandLine(base::CommandLine::ForCurrentProcess())) {
        ...
        DVLOG(1) << __FUNCTION__ << ": Software WebGL contexts are not supported on ARM CPUs.";
        return false;
    }
    ...
    if (gl_display_->ext->b_EGL_EXT_create_context_robustness || is_swangle) {
        ...
        context_attributes.push_back((attribs.robust_buffer_access || is_swangle) ? EGL_TRUE : EGL_FALSE);
        if (attribs.lose_context_on_reset) {
            ...
            if (gl_display_->ext->b_EGL_NV_robustness_video_memory_purge) {
                context_attributes.push_back(EGL_GENERATE_RESET_ON_VIDEO_MEMORY_PURGE_NV);
                context_attributes.push_back(EGL_TRUE);
            }
        }
    } else {
    ...
    if (gl_display_->ext->b_EGL_CHROMIUM_create_context_bind_generates_resource) {
        ...
        context_attributes.push_back(attribs.bind_generates_resource ? EGL_TRUE : EGL_FALSE);
    } else {
    ...
    if (gl_display_->ext->b_EGL_ANGLE_create_context_webgl_compatibility) {
        ...
        context_attributes.push_back(attribs.webgl_compatibility_context ? EGL_TRUE : EGL_FALSE);
    } else {
    ...
    if (gl_display_->ext->b_EGL_ANGLE_display_texture_share_group) {
        ...
        context_attributes.push_back(global_texture_share_group_ ? EGL_TRUE : EGL_FALSE);
    } else {
    ...
    if (gl_display_->ext->b_EGL_ANGLE_display_semaphore_share_group) {
        ...
        context_attributes.push_back(attribs.global_semaphore_share_group ? EGL_TRUE : EGL_FALSE);
    } else {
    ...
    if (gl_display_->ext->b_EGL_ANGLE_create_context_client_arrays) {
        ...
        context_attributes.push_back(attribs.allow_client_arrays ? EGL_TRUE : EGL_FALSE);
    } else {
    ...
    if (gl_display_->ext->b_EGL_ANGLE_robust_resource_initialization || is_swangle) {
        context_attributes.push_back(EGL_ROBUST_RESOURCE_INITIALIZATION_ANGLE);
        context_attributes.push_back((attribs.robust_resource_initialization || is_swangle) ? EGL_TRUE : EGL_FALSE);
    } else {
    ...
    angle_context_virtualization_group_number_ = attribs.angle_context_virtualization_group_number;
    if (gl_display_->ext->b_EGL_ANGLE_context_virtualization) {
        context_attributes.push_back(EGL_CONTEXT_VIRTUALIZATION_GROUP_ANGLE);
        context_attributes.push_back(static_cast<EGLint>(angle_context_virtualization_group_number_));
    }
    ...
    context_ = eglCreateContext(gl_display_->GetDisplay(), config_, share_group() ? share_group()->GetHandle() : nullptr, context_attributes.data());
    if (context_) {
        return true;
    }
    ...
    if (gl_display_->ext->b_EGL_KHR_no_config_context && (error == EGL_BAD_MATCH || error == EGL_BAD_ATTRIBUTE)) {
        // Set up the list of versions to try: 3.1 -> 3.0 -> 2.0
        std::vector<std::pair<EGLint, EGLint>> candidate_versions;
        if (context_client_major_version == 3 && context_client_minor_version == 1) {
            candidate_versions.emplace_back(3, 0);
            candidate_versions.emplace_back(2, 0);
        } else if (context_client_major_version == 3 && context_client_minor_version == 0) {
            candidate_versions.emplace_back(2, 0);
        }

        for (const auto& version : candidate_versions) {
            if (!ChangeContextAttributes(context_attributes, EGL_CONTEXT_MAJOR_VERSION, version.first)
                || !ChangeContextAttributes(context_attributes, EGL_CONTEXT_MINOR_VERSION, version.second)) {
                break;
            }
    ...
    LOG(ERROR) << "eglCreateContext failed with error " << GetEGLErrorString(error);
    return false;
}
...
void GLContextEGL::Destroy()
{
    ...
    if (context_) {
        if (!eglDestroyContext(gl_display_->GetDisplay(), context_)) {
            LOG(ERROR) << "eglDestroyContext failed with error " << GetLastEGLErrorString();
        }
...
bool GLContextEGL::CanShareTexturesWithContext(GLContext* other_context)
{
    ...
    // number.
    return global_texture_share_group_ && other_egl_context->global_texture_share_group_
        && angle_context_virtualization_group_number_ == other_egl_context->angle_context_virtualization_group_number_
...
void GLContextEGL::ReleaseBackpressureFences()
{
    ...
    if (has_backpressure_fences) {
        ...
        if (context_ != current_egl_context) {
            ...
            if (!eglMakeCurrent(gl_display_->GetDisplay(), EGL_NO_SURFACE, EGL_NO_SURFACE, context_)) {
                LOG(ERROR) << "eglMakeCurrent failed with error " << GetLastEGLErrorString();
            }
        }
        ...
        if (prev_current_context != this) {
            SetThreadLocalCurrentGL(prev_current_context ? prev_current_context->GetCurrentGL() : nullptr);
        }

        if (context_ != current_egl_context) {
            if (!eglMakeCurrent(gl_display_->GetDisplay(), current_draw_surface, current_read_surface, current_egl_context)) {
                LOG(ERROR) << "eglMakeCurrent failed with error " << GetLastEGLErrorString();
            }
        }
    }
...
bool GLContextEGL::MakeCurrentImpl(GLSurface* surface)
{
    ...
    TRACE_EVENT2("gpu", "GLContextEGL::MakeCurrent", "context", static_cast<void*>(context_), "surface", static_cast<void*>(surface));

    if (unbind_fbo_on_makecurrent_ && GetCurrent()) {
        glBindFramebufferEXT(GL_FRAMEBUFFER, 0);
    }

    if (!eglMakeCurrent(gl_display_->GetDisplay(), surface->GetHandle(), surface->GetHandle(), context_)) {
        LOG(ERROR) << "eglMakeCurrent failed with error " << GetLastEGLErrorString();
        return false;
    }
...
void GLContextEGL::ReleaseCurrent(GLSurface* surface)
{
    ...
    SetCurrent(nullptr);
    if (!eglMakeCurrent(gl_display_->GetDisplay(), EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT)) {
        LOG(ERROR) << "eglMakeCurrent failed to release current with error " << GetLastEGLErrorString();
        lost_ = true;
    }
...
unsigned int GLContextEGL::CheckStickyGraphicsResetStatusImpl()
{
    ...
    if ((graphics_reset_status_ == GL_NO_ERROR) && gl_display_->ext->b_EGL_EXT_create_context_robustness
