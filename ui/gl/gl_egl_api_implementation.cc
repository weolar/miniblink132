...
void RealEGLApi::SetDisabledExtensions(const std::string& disabled_extensions)
{
    ...
    if (!disabled_extensions.empty()) {
        std::vector<std::string> candidates = base::SplitString(disabled_extensions, ", ;", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
const char* RealEGLApi::eglQueryStringFn(EGLDisplay dpy, EGLint name)
{
    if (name == EGL_EXTENSIONS) {
        ...
        if (it == filtered_exts_.end()) {
            it = filtered_exts_.emplace(dpy, FilterGLExtensionList(EGLApiBase::eglQueryStringFn(dpy, name), disabled_exts_)).first;
        }
...
void TraceEGLApi::SetDisabledExtensions(const std::string& disabled_extensions)
{
    if (egl_api_) {
        egl_api_->SetDisabledExtensions(disabled_extensions);
    }
