...
HDRMetadataHelperWin::HDRMetadataHelperWin(Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device)
...
std::optional<DXGI_HDR_METADATA_HDR10> HDRMetadataHelperWin::GetDisplayMetadata(HWND window)
{
    auto it = hdr_metadatas_.find(MonitorFromWindow(window, MONITOR_DEFAULTTONEAREST));
    if (it == hdr_metadatas_.end()) {
        return std::nullopt;
    }
    return it->second;
...
void HDRMetadataHelperWin::UpdateDisplayMetadata()
{
    ...
    for (unsigned int i = 0; dxgi_factory->EnumAdapters(i, &adapter) != DXGI_ERROR_NOT_FOUND; i++) {
        Microsoft::WRL::ComPtr<IDXGIOutput> output;
        for (unsigned int u = 0; adapter->EnumOutputs(u, &output) != DXGI_ERROR_NOT_FOUND; u++) {
...
// static
DXGI_HDR_METADATA_HDR10 HDRMetadataHelperWin::HDRMetadataToDXGI(const gfx::HDRMetadata& hdr_metadata)
{
    DXGI_HDR_METADATA_HDR10 metadata {};

    const auto smpte_st_2086 = hdr_metadata.smpte_st_2086.value_or(gfx::HdrMetadataSmpteSt2086());
    ...
    UNSAFE_BUFFERS(metadata.GreenPrimary[1]) = primaries.fGY * kPrimariesFixedPoint;
    metadata.BluePrimary[0] = primaries.fBX * kPrimariesFixedPoint;
    UNSAFE_BUFFERS(metadata.BluePrimary[1]) = primaries.fBY * kPrimariesFixedPoint;
    ...
    metadata.MinMasteringLuminance = smpte_st_2086.luminance_min * kMinLuminanceFixedPoint;

    const auto cta_861_3 = hdr_metadata.cta_861_3.value_or(gfx::HdrMetadataCta861_3());
...
DXGI_HDR_METADATA_HDR10 HDRMetadataHelperWin::OutputDESC1ToDXGI(const DXGI_OUTPUT_DESC1& desc1)
{
    DXGI_HDR_METADATA_HDR10 metadata {};
    ...
    // SAFETY: required from Windows API.
    UNSAFE_BUFFERS(metadata.RedPrimary[1]) = UNSAFE_BUFFERS(primary_r[1]) * kPrimariesFixedPoint;
    auto& primary_g = desc1.GreenPrimary;
    metadata.GreenPrimary[0] = primary_g[0] * kPrimariesFixedPoint;
    UNSAFE_BUFFERS(metadata.GreenPrimary[1]) = UNSAFE_BUFFERS(primary_g[1]) * kPrimariesFixedPoint;
    auto& primary_b = desc1.BluePrimary;
    metadata.BluePrimary[0] = primary_b[0] * kPrimariesFixedPoint;
    UNSAFE_BUFFERS(metadata.BluePrimary[1]) = UNSAFE_BUFFERS(primary_b[1]) * kPrimariesFixedPoint;
    auto& white_point = desc1.WhitePoint;
    metadata.WhitePoint[0] = white_point[0] * kPrimariesFixedPoint;
    UNSAFE_BUFFERS(metadata.WhitePoint[1]) = UNSAFE_BUFFERS(white_point[1]) * kPrimariesFixedPoint;
