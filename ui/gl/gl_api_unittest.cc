...
public:
    static GLFunctionPointerType GL_BINDING_CALL FakeGLGetProcAddress(const char* proc)
    {
    ...
    void SetUp() override
    {
        ...
        SetGLGetProcAddressProc(static_cast<GLGetProcAddressProc>(&FakeGLGetProcAddress));
    }
    ...
    void InitializeAPI(const char* disabled_extensions)
    {
        ...
        std::string extensions_string = GetGLExtensionsFromCurrentContext(api_.get());
        gfx::ExtensionSet extension_set = gfx::MakeExtensionSet(extensions_string);

        auto version = std::make_unique<GLVersionInfo>(
            reinterpret_cast<const char*>(api_->glGetStringFn(GL_VERSION)), reinterpret_cast<const char*>(api_->glGetStringFn(GL_RENDERER)), extension_set);
    ...
    {
        std::vector<std::string> extensions = base::SplitString(fake_extension_string_, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        *params = static_cast<GLint>(extensions.size());
    }

    static const GLubyte* GL_BINDING_CALL FakeGetStringi(GLenum name, GLuint index)
    {
        // |extensions| needs to be static so we can return c_str() from
        // its elements.
        static std::vector<std::string> extensions;
        extensions = base::SplitString(fake_extension_string_, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        GLuint count = static_cast<GLuint>(extensions.size());
        return (index < count) ? reinterpret_cast<const GLubyte*>(extensions[index].c_str()) : nullptr;
    }
    ...
    {
        return reinterpret_cast<const char*>(api_->glGetStringiFn(GL_EXTENSIONS, index));
    }
...
{
    static constexpr std::string_view kFakeExtensions = "GL_EXT_1 GL_EXT_2 GL_EXT_3 GL_EXT_4";
    static constexpr std::string_view kFakeDisabledExtensions = "GL_EXT_1,GL_EXT_2,GL_FAKE";
...
{
    static constexpr std::string_view kFakeExtensions = "GL_EXT_disjoint_timer_query";
    static constexpr std::string_view kFakeDisabledExtensions = "GL_EXT_disjoint_timer_query";
...
{
    static constexpr std::string_view kFakeExtensions = "GL_EXT_1 GL_EXT_2 GL_EXT_3 GL_EXT_4";
    static constexpr std::array<std::string_view, 4> kFakeExtensionList = { { "GL_EXT_1", "GL_EXT_2", "GL_EXT_3", "GL_EXT_4" } };
    static constexpr std::string_view kFakeDisabledExtensions = "GL_EXT_1,GL_EXT_2,GL_FAKE";
    static constexpr std::array<std::string_view, 2> kFilteredExtensions = { { "GL_EXT_3", "GL_EXT_4" } };
