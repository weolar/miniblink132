...
public:
    void SetUp() override
    {
        ...
        ON_CALL(*mock_dxgi_factory_.Get(), EnumAdapters(_, _)).WillByDefault(Return(DXGI_ERROR_NOT_FOUND));
        ...
        ON_CALL(*mock_d3d11_device_.Get(), QueryInterface(IID_IDXGIDevice, _)).WillByDefault(media::SetComPointeeAndReturnOk<1>(mock_dxgi_device_.Get()));
    }

    std::unique_ptr<HDRMetadataHelperWin> CreateHelper()
    {
        // Set the D3D11 device's adapter to the first one, somewhat arbitrarily.
        ON_CALL(*mock_dxgi_device_.Get(), GetAdapter(_)).WillByDefault(media::SetComPointeeAndReturnOk<0>(mock_dxgi_adapters_[0].Get()));
    ...
    void AddAdapter()
    {
        Microsoft::WRL::ComPtr<media::DXGIAdapterMock> dxgi_adapter = media::MakeComPtr<NiceMock<media::DXGIAdapterMock>>();
        ON_CALL(*dxgi_adapter.Get(), GetParent(_, _)).WillByDefault(media::SetComPointeeAndReturnOk<1>(mock_dxgi_factory_.Get()));

        // By default, the adapter has no outputs.
        ON_CALL(*dxgi_adapter.Get(), EnumOutputs(_, _)).WillByDefault(Return(DXGI_ERROR_NOT_FOUND));

        // Make the factory enumerate this adapter.
        ON_CALL(*mock_dxgi_factory_.Get(), EnumAdapters(mock_dxgi_adapters_.size(), _)).WillByDefault(media::SetComPointeeAndReturnOk<1>(dxgi_adapter.Get()));
    ...
    void AddOutput(const DXGI_OUTPUT_DESC1& desc1)
    {
        ...
        Microsoft::WRL::ComPtr<media::DXGIOutput6Mock> output6 = media::MakeComPtr<media::DXGIOutput6Mock>();
        mock_dxgi_output6s_.push_back(output6);
        ON_CALL(*output6.Get(), GetDesc1(_)).WillByDefault(DoAll(SetArgPointee<0>(desc1), Return(S_OK)));
        ...
        ON_CALL(*dxgi_adapter.Get(), EnumOutputs(0, _)).WillByDefault(media::SetComPointeeAndReturnOk<1>(output6.Get()));
    }
    ...
    std::vector<Microsoft::WRL::ComPtr<media::DXGIAdapterMock>> mock_dxgi_adapters_;
    std::vector<Microsoft::WRL::ComPtr<media::DXGIOutput6Mock>> mock_dxgi_output6s_;
...
TEST_F(HDRMetadataHelperWinTest, CachesMetadataIfAvailable)
{
    ...
    EXPECT_EQ(result->RedPrimary[0], static_cast<int>(desc.RedPrimary[0] * kPrimariesFixedPoint));
    // SAFETY: required from Windows API.
    EXPECT_EQ(UNSAFE_BUFFERS(result->RedPrimary[1]), static_cast<int>(UNSAFE_BUFFERS(desc.RedPrimary[1]) * kPrimariesFixedPoint));
    EXPECT_EQ(result->GreenPrimary[0], static_cast<int>(desc.GreenPrimary[0] * kPrimariesFixedPoint));
    EXPECT_EQ(UNSAFE_BUFFERS(result->GreenPrimary[1]), static_cast<int>(UNSAFE_BUFFERS(desc.GreenPrimary[1]) * kPrimariesFixedPoint));
    EXPECT_EQ(result->BluePrimary[0], static_cast<int>(desc.BluePrimary[0] * kPrimariesFixedPoint));
    EXPECT_EQ(UNSAFE_BUFFERS(result->BluePrimary[1]), static_cast<int>(UNSAFE_BUFFERS(desc.BluePrimary[1]) * kPrimariesFixedPoint));
    EXPECT_EQ(result->WhitePoint[0], static_cast<int>(desc.WhitePoint[0] * kPrimariesFixedPoint));
    EXPECT_EQ(UNSAFE_BUFFERS(result->WhitePoint[1]), static_cast<int>(UNSAFE_BUFFERS(desc.WhitePoint[1]) * kPrimariesFixedPoint));
    EXPECT_EQ(result->MaxMasteringLuminance, static_cast<unsigned>(desc.MaxLuminance));
    EXPECT_EQ(result->MinMasteringLuminance, static_cast<unsigned>(desc.MinLuminance * kMinLuminanceFixedPoint));
