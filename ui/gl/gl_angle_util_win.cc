...
void* QueryDeviceObjectFromANGLE(EGLDisplay egl_display, int object_type)
{
    ...
    intptr_t device = 0;
    if (!eglQueryDeviceAttribEXT(reinterpret_cast<EGLDeviceEXT>(egl_device), object_type, &device)) {
...
Microsoft::WRL::ComPtr<ID3D11Device> QueryD3D11DeviceObjectFromANGLE()
{
    ...
    if (!display
        || (display->GetDisplayType() != ANGLE_D3D11 && display->GetDisplayType() != ANGLE_D3D11_NULL && display->GetDisplayType() != ANGLE_D3D11on12)) {
        return nullptr;
    }
    Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device;
    d3d11_device = reinterpret_cast<ID3D11Device*>(QueryDeviceObjectFromANGLE(display->GetDisplay(), EGL_D3D11_DEVICE_ANGLE));
    return d3d11_device;
}
...
Microsoft::WRL::ComPtr<IDirect3DDevice9> QueryD3D9DeviceObjectFromANGLE()
{
    ...
    d3d9_device = reinterpret_cast<IDirect3DDevice9*>(QueryDeviceObjectFromANGLE(display->GetDisplay(), EGL_D3D9_DEVICE_ANGLE));
    return d3d9_device;
}
