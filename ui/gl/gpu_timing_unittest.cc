...
public:
    ...
    scoped_refptr<GPUTimingClient> CreateGPUTimingClient()
    {
        ...
        if (!cpu_time_bounded_) {
            client->SetCpuTimeForTesting(base::BindRepeating(&GPUTimingFake::GetFakeCPUTime));
...
TEST_F(GPUTimingTest, QueryTimestampUsingElapsedTest)
{
    ...
    EXPECT_CALL(*gl_, GetQueryiv(GL_TIMESTAMP, GL_QUERY_COUNTER_BITS, NotNull())).Times(Exactly(1)).WillRepeatedly(DoAll(SetArgPointee<2>(0), Return()));
