...
bool PassthroughCommandDecoderSupported()
{
    ...
    return display->ext->b_EGL_CHROMIUM_create_context_bind_generates_resource && display->ext->b_EGL_ANGLE_create_context_webgl_compatibility
        && display->ext->b_EGL_ANGLE_robust_resource_initialization && display->ext->b_EGL_ANGLE_display_texture_share_group
...
{
    return base::FeatureList::IsEnabled(features::kDCompTripleBufferRootSwapChain) ? 3u : 2u;
}
...
void LabelSwapChainBuffers(IDXGISwapChain* swap_chain, const char* name_prefix)
{
    ...
    if (FAILED(hr)) {
        DLOG(ERROR) << "Failed to GetDesc from swap chain: " << logging::SystemErrorCodeToString(hr);
        return;
    }
    for (unsigned int i = 0; i < desc.BufferCount; i++) {
        ...
        if (FAILED(hr)) {
            DLOG(ERROR) << "GetBuffer on swap chain buffer " << i << "failed: " << logging::SystemErrorCodeToString(hr);
            return;
        }
        const std::string buffer_name = base::StringPrintf("%s_Buffer_%d", name_prefix, i);
        hr = SetDebugName(swap_chain_buffer.Get(), buffer_name.c_str());
        if (FAILED(hr)) {
            DLOG(ERROR) << "Failed to label swap chain buffer " << i << ": " << logging::SystemErrorCodeToString(hr);
        }
    }
}
...
// operations
void LabelSwapChainAndBuffers(IDXGISwapChain* swap_chain, const char* name_prefix)
{
...
GL_EXPORT GLDisplay* GetDisplay(GpuPreference gpu_preference, gl::DisplayKey display_key)
{
    // TODO(344606399): Consider making callers directly create the EGL display.
    return GLDisplayManagerEGL::GetInstance()->GetDisplay(gpu_preference, display_key);
}
...
{
    GLDisplayManagerEGL::GetInstance()->SetGpuPreference(preference, system_device_id);
}
...
ScopedEnableTextureRectangleInShaderCompiler::ScopedEnableTextureRectangleInShaderCompiler(gl::GLApi* gl_api)
{
    if (gl_api) {
