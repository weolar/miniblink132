...
public:
    ...
    GLFenceAndroidNativeFenceSync& operator=(const GLFenceAndroidNativeFenceSync&) = delete;

    ~GLFenceAndroidNativeFenceSync() override;

    static std::unique_ptr<GLFenceAndroidNativeFenceSync> CreateForGpuFence();

    static std::unique_ptr<GLFenceAndroidNativeFenceSync> CreateFromGpuFence(const gfx::GpuFence&);
...
private:
    ...
    static std::unique_ptr<GLFenceAndroidNativeFenceSync> CreateInternal(EGLenum type, EGLint* attribs);
};
