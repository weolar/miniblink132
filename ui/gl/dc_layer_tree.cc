...
{
    return overlay->overlay_image && overlay->overlay_image->type() != DCLayerOverlayType::kDCompVisualContent;
}
...
Microsoft::WRL::ComPtr<IDCompositionVisual3> CheckedCastToVisual3(const Microsoft::WRL::ComPtr<IDCompositionVisual2>& visual2)
{
...
D2D_MATRIX_3X2_F TransformToD2D_MATRIX_3X2_F(const gfx::Transform& transform)
{
    ...
    return D2D1::Matrix3x2F(transform.rc(0, 0), transform.rc(1, 0), transform.rc(0, 1), transform.rc(1, 1), transform.rc(0, 3), transform.rc(1, 3));
}
...
D2D_MATRIX_4X4_F TransformToD2D_MATRIX_4X4_F(const gfx::Transform& transform)
{
    ...
    return D2D1::Matrix4x4F(t.rc(0, 0), t.rc(1, 0), t.rc(2, 0), t.rc(3, 0), t.rc(0, 1), t.rc(1, 1), t.rc(2, 1), t.rc(3, 1), t.rc(0, 2), t.rc(1, 2), t.rc(2, 2),
        t.rc(3, 2), t.rc(0, 3), t.rc(1, 3), t.rc(2, 3), t.rc(3, 3));
}
...
#if DCHECK_IS_ON()
bool VisualTreeValid(std::vector<std::optional<size_t>>& subtree_index_to_overlay, const std::vector<bool>& prev_subtree_is_attached_to_root)
{
...
void VideoProcessorWrapper::SizeSmoother::SizeSmoother::PutSize(gfx::Size size)
{
...
public:
    ...
    IDCompositionSurface* surface() const
    {
        return surface_.Get();
    }
...
private:
    ...
    explicit SolidColorSurface(Microsoft::WRL::ComPtr<IDCompositionSurface> surface)
    ...
    {
        HRESULT hr = S_OK;
        RECT update_rect = D2D1::Rect(0, 0, kSolidColorSurfaceSize.width(), kSolidColorSurfaceSize.height());
        Microsoft::WRL::ComPtr<ID3D11Texture2D> draw_texture;
        POINT update_offset;
        hr = surface_->BeginDraw(&update_rect, IID_PPV_ARGS(&draw_texture), &update_offset);
        if (FAILED(hr)) {
        ...
        if (FAILED(hr)) {
            LOG(ERROR) << "CreateRenderTargetView failed: " << logging::SystemErrorCodeToString(hr);
            return false;
        }
...
SolidColorSurfacePool::SolidColorSurfacePool(Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device, Microsoft::WRL::ComPtr<IDCompositionDevice3> dcomp_device)
...
IDCompositionSurface* SolidColorSurfacePool::GetSolidColorSurface(const SkColor4f& color)
{
    stats_since_last_trim_.num_surfaces_requested += 1;

    HRESULT hr = S_OK;

    auto first_unused_surface_it = std::next(tracked_surfaces_.begin(), num_used_this_frame_);

    if (auto found_color_it = base::ranges::find(tracked_surfaces_, color, &SolidColorSurface::color_); found_color_it != tracked_surfaces_.end()) {
    ...
    if (surface_to_fill_it == tracked_surfaces_.end()) {
        ...
        hr = dcomp_device_->CreateSurface(kSolidColorSurfaceSize.width(), kSolidColorSurfaceSize.height(),
            gfx::ColorSpaceWin::GetDXGIFormat(gfx::ColorSpace::CreateSRGB()), DXGI_ALPHA_MODE_IGNORE, &dcomp_surface);
        if (FAILED(hr)) {
            LOG(ERROR) << "CreateSurface failed: " << logging::SystemErrorCodeToString(hr);
            return nullptr;
        }

        surface_to_fill_it = tracked_surfaces_.insert(first_unused_surface_it, SolidColorSurface(std::move(dcomp_surface)));
    }
...
void SolidColorSurfacePool::TrimAfterCommit()
{
    ...
    size_t trim_target_size = std::max(num_used_this_frame_, kMaxSolidColorSurfacesToRetain);
    ...
    DVLOG(3) << "SolidColorSurfacePool stats before trim: "
             << "requested=" << stats_since_last_trim_.num_surfaces_requested << ", "
             << "recolored=" << stats_since_last_trim_.num_surfaces_recolored << ", "
             << "in-use/total=" << num_used_this_frame_ << "/" << tracked_surfaces_.size()
             << (num_used_this_frame_ > kMaxSolidColorSurfacesToRetain ? " (in-use exceeds kMaxSolidColorSurfacesToRetain)" : "") << ", will trim to "
             << trim_target_size;

    auto first_surface_to_remove = std::next(tracked_surfaces_.begin(), trim_target_size);
...
DCLayerTree::DCLayerTree(bool disable_nv12_dynamic_textures, bool disable_vp_auto_hdr, bool disable_vp_scaling, bool disable_vp_super_resolution,
    bool force_dcomp_triple_buffer_video_swap_chain, bool no_downscaled_overlay_promotion)
    ...
    , force_dcomp_triple_buffer_video_swap_chain_(force_dcomp_triple_buffer_video_swap_chain)
...
void DCLayerTree::Initialize(HWND window, Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device)
{
    window_ = window;
    DCHECK(window_);
    ...
    solid_color_surface_pool_ = std::make_unique<SolidColorSurfacePool>(d3d11_device_, dcomp_device_);

    Microsoft::WRL::ComPtr<IDCompositionDesktopDevice> desktop_device;
    dcomp_device_.As(&desktop_device);
    DCHECK(desktop_device);

    HRESULT hr = desktop_device->CreateTargetForHwnd(window_, TRUE, &dcomp_target_);
    ...
    dcomp_root_visual_->SetBitmapInterpolationMode(DCOMPOSITION_BITMAP_INTERPOLATION_MODE_LINEAR);
...
VideoProcessorWrapper* DCLayerTree::InitializeVideoProcessor(
    const gfx::Size& input_size, const gfx::Size& output_size, bool is_hdr_output, bool& video_processor_recreated)
{
    video_processor_recreated = false;
    auto& video_processor_wrapper = is_hdr_output ? video_processor_wrapper_hdr_ : video_processor_wrapper_sdr_;
    ...
    gfx::Size effective_input_size = video_processor_wrapper.input_size_smoother.GetSize();
    gfx::Size effective_output_size = video_processor_wrapper.output_size_smoother.GetSize();
    ...
    if (video_processor_wrapper.video_processor && video_processor_wrapper.video_input_size == effective_input_size
        && video_processor_wrapper.video_output_size == effective_output_size) {
        return &video_processor_wrapper;
    }

    TRACE_EVENT2("gpu", "DCLayerTree::InitializeVideoProcessor", "input_size", input_size.ToString(), "output_size", output_size.ToString());
    ...
    HRESULT hr = video_processor_wrapper.video_device->CreateVideoProcessorEnumerator(&desc, &video_processor_wrapper.video_processor_enumerator);
    if (FAILED(hr)) {
        DLOG(ERROR) << "CreateVideoProcessorEnumerator failed with error 0x" << std::hex << hr;
    ...
    hr = video_processor_wrapper.video_device->CreateVideoProcessor(
        video_processor_wrapper.video_processor_enumerator.Get(), 0, &video_processor_wrapper.video_processor);
    if (FAILED(hr)) {
        DLOG(ERROR) << "CreateVideoProcessor failed with error 0x" << std::hex << hr;
        // It might fail again next time. Disable overlay support so
        // overlay processor will stop sending down overlay frames.
        DisableDirectCompositionOverlays();
        return nullptr;
    ...
    video_processor_wrapper.video_context->VideoProcessorSetStreamAutoProcessingMode(video_processor_wrapper.video_processor.Get(), 0, FALSE);

    video_processor_recreated = true;
    return &video_processor_wrapper;
}

Microsoft::WRL::ComPtr<IDXGISwapChain1> DCLayerTree::GetLayerSwapChainForTesting(size_t index) const
{
    CHECK_IS_TEST();
...
void DCLayerTree::GetSwapChainVisualInfoForTesting(size_t index, gfx::Transform* transform, gfx::Point* offset, gfx::Rect* clip_rect) const
{
    CHECK_IS_TEST();
    if (visual_tree_) {
...
bool DCLayerTree::VisualTree::VisualSubtree::Update(IDCompositionDevice3* dcomp_device, Microsoft::WRL::ComPtr<IUnknown> dcomp_visual_content,
    uint64_t dcomp_surface_serial, const gfx::Size& image_size, const gfx::RectF& content_rect,
    Microsoft::WRL::ComPtr<IDCompositionSurface> background_color_surface, const SkColor4f& background_color, const gfx::Rect& quad_rect,
    bool nearest_neighbor_filter, const gfx::Transform& quad_to_root_transform, const gfx::RRectF& rounded_corner_bounds, float opacity,
    const std::optional<gfx::Rect>& clip_rect_in_root, bool allow_antialiasing)
{
    ...
    const bool dcomp_visual_content_changed = SetField(dcomp_visual_content_, dcomp_visual_content);
    const bool dcomp_surface_serial_changed = SetField(dcomp_surface_serial_, dcomp_surface_serial);
    ...
    const bool background_color_surface_changed = SetField(background_color_surface_, background_color_surface);
    const bool background_color_changed = SetField(background_color_, background_color);
    const bool quad_rect_changed = SetField(quad_rect_, quad_rect);
    const bool nearest_neighbor_filter_changed = SetField(nearest_neighbor_filter_, nearest_neighbor_filter);
    const bool quad_to_root_transform_changed = SetField(quad_to_root_transform_, quad_to_root_transform);
    const bool rounded_corner_bounds_changed = SetField(rounded_corner_bounds_, rounded_corner_bounds);
    const bool opacity_changed = SetField(opacity_, opacity);
    const bool clip_rect_in_root_changed = SetField(clip_rect_in_root_, clip_rect_in_root);
    const bool allow_antialiasing_changed = SetField(allow_antialiasing_, allow_antialiasing);
    ...
    if (!clip_visual_) {
        ...
        CHECK_EQ(hr, S_OK);
        hr = rounded_corners_visual_->AddVisual(transform_visual_.Get(), FALSE, nullptr);
        CHECK_EQ(hr, S_OK);
        hr = transform_visual_->AddVisual(background_color_visual_.Get(), FALSE, nullptr);
        CHECK_EQ(hr, S_OK);
    ...
    if (clip_rect_in_root_changed) {
        if (clip_rect_in_root_.has_value()) {
            ...
            hr = clip_visual_->SetClip(D2D1::RectF(clip_rect.x(), clip_rect.y(), clip_rect.right(), clip_rect.bottom()));
            CHECK_EQ(hr, S_OK);
        } else {
    ...
    if (rounded_corner_bounds_changed) {
        if (!rounded_corner_bounds_.IsEmpty()) {
            ...
            const gfx::Vector2dF top_left = rounded_corner_bounds_.GetCornerRadii(gfx::RRectF::Corner::kUpperLeft);
            hr = clip->SetTopLeftRadiusX(top_left.x());
            CHECK_EQ(hr, S_OK);
            hr = clip->SetTopLeftRadiusY(top_left.y());
            CHECK_EQ(hr, S_OK);

            const gfx::Vector2dF top_right = rounded_corner_bounds_.GetCornerRadii(gfx::RRectF::Corner::kUpperRight);
            hr = clip->SetTopRightRadiusX(top_right.x());
            CHECK_EQ(hr, S_OK);
            hr = clip->SetTopRightRadiusY(top_right.y());
            CHECK_EQ(hr, S_OK);

            const gfx::Vector2dF bottom_left = rounded_corner_bounds_.GetCornerRadii(gfx::RRectF::Corner::kLowerLeft);
            hr = clip->SetBottomLeftRadiusX(bottom_left.x());
            CHECK_EQ(hr, S_OK);
            hr = clip->SetBottomLeftRadiusY(bottom_left.y());
            CHECK_EQ(hr, S_OK);

            const gfx::Vector2dF bottom_right = rounded_corner_bounds_.GetCornerRadii(gfx::RRectF::Corner::kLowerRight);
            ...
            hr = rounded_corners_visual_->SetBorderMode(DCOMPOSITION_BORDER_MODE_SOFT);
            CHECK_EQ(hr, S_OK);
        } else {
            hr = rounded_corners_visual_->SetClip(nullptr);
            CHECK_EQ(hr, S_OK);
            hr = rounded_corners_visual_->SetBorderMode(DCOMPOSITION_BORDER_MODE_INHERIT);
            CHECK_EQ(hr, S_OK);
        }
    }
    ...
    if (quad_to_root_transform_changed) {
        if (quad_to_root_transform_.Is2dTransform()) {
            const D2D_MATRIX_3X2_F matrix = TransformToD2D_MATRIX_3X2_F(quad_to_root_transform_);
            hr = Microsoft::WRL::ComPtr<IDCompositionVisual>(transform_visual_)->SetTransform(matrix);
            CHECK_EQ(hr, S_OK);
        } else {
            const D2D_MATRIX_4X4_F matrix = TransformToD2D_MATRIX_4X4_F(quad_to_root_transform_);
    ...
    if (nearest_neighbor_filter_changed) {
        hr = transform_visual_->SetBitmapInterpolationMode(
            nearest_neighbor_filter_ ? DCOMPOSITION_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR : DCOMPOSITION_BITMAP_INTERPOLATION_MODE_LINEAR);
        CHECK_EQ(hr, S_OK);
    }
    ...
    if (image_size_changed || content_rect_changed || quad_rect_changed) {
        if (content_rect_.Contains(gfx::RectF(image_size_))) {
            ...
            const auto content_clip = D2D1::RectF(content_rect_.x(), content_rect_.y(), content_rect_.right(), content_rect_.bottom());
        ...
        const bool needs_scale
            = static_cast<float>(quad_rect_.width()) != content_rect_.width() || static_cast<float>(quad_rect_.height()) != content_rect_.height();
        if (needs_offset || needs_scale) {
            const float scale_x = static_cast<float>(quad_rect_.width()) / content_rect_.width();
            const float scale_y = static_cast<float>(quad_rect_.height()) / content_rect_.height();
            const D2D_MATRIX_3X2_F matrix = D2D1::Matrix3x2F::Translation(-content_rect_.x(), -content_rect_.y()) * D2D1::Matrix3x2F::Scale(scale_x, scale_y);
            hr = Microsoft::WRL::ComPtr<IDCompositionVisual>(content_visual_)->SetTransform(matrix);
            CHECK_EQ(hr, S_OK);
        } else {
    ...
    if (quad_rect_changed || background_color_surface_changed || background_color_changed) {
        if (!background_color_surface_ || background_color.fA == 0.0) {
            ...
            const D2D_MATRIX_3X2_F matrix = TransformToD2D_MATRIX_3X2_F(gfx::TransformBetweenRects(gfx::RectF(kSolidColorSurfaceSize), gfx::RectF(quad_rect_)));
            hr = Microsoft::WRL::ComPtr<IDCompositionVisual>(background_color_visual_)->SetTransform(matrix);
            CHECK_EQ(hr, S_OK);

            hr = background_color_visual_->SetContent(background_color_surface_.Get());
            CHECK_EQ(hr, S_OK);

            hr = CheckedCastToVisual3(background_color_visual_)->SetOpacity(background_color.fA);
            CHECK_EQ(hr, S_OK);
        }
    }

    if (quad_to_root_transform_changed || quad_rect_changed || allow_antialiasing_changed) {
        const float kNeedsSoftBorderTolerance = 0.001;
        const bool content_soft_borders = allow_antialiasing_
            && (!quad_to_root_transform_.Preserves2dAxisAlignment()
                || !gfx::IsNearestRectWithinDistance(quad_to_root_transform_.MapRect(gfx::RectF(quad_rect_)), kNeedsSoftBorderTolerance));
        ...
        hr = transform_visual_->SetBorderMode(content_soft_borders ? DCOMPOSITION_BORDER_MODE_SOFT : DCOMPOSITION_BORDER_MODE_HARD);
        CHECK_EQ(hr, S_OK);
    }
...
void DCLayerTree::VisualTree::VisualSubtree::GetSwapChainVisualInfoForTesting(gfx::Transform* transform, gfx::Point* offset, gfx::Rect* clip_rect) const
{
    CHECK_IS_TEST();
...
bool DCLayerTree::VisualTree::BuildTree(const std::vector<std::unique_ptr<DCLayerOverlayParams>>& overlays)
{
    ...
    // meaning not reused.
    std::vector<std::optional<size_t>> overlay_index_to_reused_subtree(overlays.size(), std::nullopt);
    ...
    std::vector<std::optional<size_t>> subtree_index_to_overlay(visual_subtrees_.size(), std::nullopt);
    ...
    // overlays.
    VisualSubtreeMap subtree_map = BuildMapAndAssignMatchingSubtrees(overlays, visual_subtrees, overlay_index_to_reused_subtree, subtree_index_to_overlay);
    ...
    std::vector<bool> prev_subtree_is_attached_to_root(visual_subtrees_.size(), true);

    bool needs_commit = DetachUnusedSubtreesFromRoot(first_prev_frame_subtree_unused_index, prev_subtree_is_attached_to_root);
    ...
    needs_commit |= DetachReusedSubtreesThatNeedRepositioningFromRoot(
        visual_subtrees, overlay_index_to_reused_subtree, subtree_index_to_overlay, prev_subtree_is_attached_to_root);
    ...
    for (size_t i = 1; i < overlays.size(); i++) {
        ...
        if (overlays[i]->aggregated_layer_id == overlays[i - 1]->aggregated_layer_id) {
    ...
    for (unsigned int i = 0; i < overlays.size(); i++) {
        ...
        if (visual_subtrees[i]) {
            ...
            subtree_attached_to_root = prev_subtree_is_attached_to_root[overlay_index_to_reused_subtree[i].value()];
        } else {
        ...
        const uint64_t dcomp_surface_serial = overlays[i]->overlay_image.has_value() ? overlays[i]->overlay_image->dcomp_surface_serial() : 0;
        const gfx::Size image_size = overlays[i]->overlay_image.has_value() ? overlays[i]->overlay_image->size() : gfx::Size();

        // Only get a background color surface if we have a non-transparent
        // background color.
        IDCompositionSurface* background_color_surface = nullptr;
        if (overlays[i]->background_color && overlays[i]->background_color->fA != 0.0) {
            background_color_surface = dc_layer_tree_->solid_color_surface_pool_->GetSolidColorSurface(overlays[i]->background_color.value());
        ...
        IUnknown* dcomp_visual_content = overlays[i]->overlay_image ? overlays[i]->overlay_image->dcomp_visual_content() : nullptr;
        ...
        const bool allow_antialiasing = !layers_with_multiple_overlays.contains(overlays[i]->aggregated_layer_id);

        needs_commit |= visual_subtrees[i]->Update(dc_layer_tree_->dcomp_device_.Get(), dcomp_visual_content, dcomp_surface_serial, image_size,
            overlays[i]->content_rect, background_color_surface, overlays[i]->background_color.value_or(SkColors::kTransparent), overlays[i]->quad_rect,
            overlays[i]->nearest_neighbor_filter, overlays[i]->transform, overlays[i]->rounded_corner_bounds, overlays[i]->opacity, overlays[i]->clip_rect,
            allow_antialiasing);

        if (!subtree_attached_to_root) {
            HRESULT hr = dc_layer_tree_->dcomp_root_visual_.Get()->AddVisual(visual_subtree->container_visual(), TRUE, left_sibling_visual);
            CHECK_EQ(hr, S_OK);
            needs_commit = true;
        }
...
DCLayerTree::VisualTree::VisualSubtreeMap DCLayerTree::VisualTree::BuildMapAndAssignMatchingSubtrees(
    const std::vector<std::unique_ptr<DCLayerOverlayParams>>& overlays, std::vector<std::unique_ptr<VisualSubtree>>& new_visual_subtrees,
    std::vector<std::optional<size_t>>& overlay_index_to_reused_subtree, std::vector<std::optional<size_t>>& subtree_index_to_overlay)
{
    ...
    for (size_t i = 0; i < overlays.size(); i++) {
        ...
        IUnknown* dcomp_visual_content = overlays[i]->overlay_image->dcomp_visual_content();
...
size_t DCLayerTree::VisualTree::ReuseUnmatchedSubtrees(std::vector<std::unique_ptr<VisualSubtree>>& new_visual_subtrees,
    std::vector<std::optional<size_t>>& overlay_index_to_reused_subtree, std::vector<std::optional<size_t>>& subtree_index_to_overlay)
{
    ...
    // a match.
    for (size_t i = 0; i < new_visual_subtrees.size() && prev_frame_subtree_index < visual_subtrees_.size(); i++) {
        ...
        for (; prev_frame_subtree_index < visual_subtrees_.size(); prev_frame_subtree_index++) {
            ...
            // Move visual subtree from the old subtrees to new subtrees.
            new_visual_subtrees[i] = std::move(visual_subtrees_[prev_frame_subtree_index]);
...
bool DCLayerTree::VisualTree::DetachUnusedSubtreesFromRoot(size_t first_prev_frame_subtree_unused_index, std::vector<bool>& prev_subtree_is_attached_to_root)
{
    CHECK_EQ(prev_subtree_is_attached_to_root.size(), visual_subtrees_.size());
    bool needs_commit = false;
    // Detach the remaining unused subtrees from the root.
    for (size_t i = first_prev_frame_subtree_unused_index; i < visual_subtrees_.size(); i++) {
...
bool DCLayerTree::VisualTree::DetachReusedSubtreesThatNeedRepositioningFromRoot(const std::vector<std::unique_ptr<VisualSubtree>>& new_visual_subtrees,
    const std::vector<std::optional<size_t>>& overlay_index_to_reused_subtree, const std::vector<std::optional<size_t>>& subtree_index_to_overlay,
    std::vector<bool>& prev_subtree_is_attached_to_root)
{
    ...
    for (size_t i = 0; i < overlay_index_to_reused_subtree.size(); i++) {
        ...
        for (; prev_frame_subtree_index < reused_subtree_index; prev_frame_subtree_index++) {
            if (!prev_subtree_is_attached_to_root[prev_frame_subtree_index]) {
                continue;
            }
            VisualSubtree* subtree = new_visual_subtrees[subtree_index_to_overlay[prev_frame_subtree_index].value()].get();
...
#if DCHECK_IS_ON()
        new_visual_subtrees[i]->attached_to_root_from_previous_frame_ = prev_subtree_is_attached_to_root[reused_subtree_index];
#endif // DCHECK_IS_ON()
    }
    return needs_commit;
}
...
{
    HRESULT hr = dc_layer_tree_->dcomp_root_visual_.Get()->RemoveVisual(subtree->container_visual());
    CHECK_EQ(hr, S_OK);
}

void DCLayerTree::VisualTree::GetSwapChainVisualInfoForTesting(size_t index, gfx::Transform* transform, gfx::Point* offset, gfx::Rect* clip_rect) const
{
    CHECK_IS_TEST();
...
bool DCLayerTree::CommitAndClearPendingOverlays(std::vector<std::unique_ptr<DCLayerOverlayParams>> overlays)
{
    TRACE_EVENT1("gpu", "DCLayerTree::CommitAndClearPendingOverlays", "num_overlays", overlays.size());
    ...
    if (it != overlays.end() && (*it)->overlay_image) {
        Microsoft::WRL::ComPtr<IUnknown> root_visual_content = (*it)->overlay_image->dcomp_visual_content();
    ...
    if (pending_delegated_ink_metadata_) {
        if (auto ink_layer = ink_renderer_->MakeDelegatedInkOverlay(dcomp_device_.Get(), root_swap_chain.Get(), std::move(pending_delegated_ink_metadata_))) {
    ...
    const size_t num_swap_chain_presenters
        = std::count_if(overlays.begin(), overlays.end(), [](const auto& overlay) { return NeedSwapChainPresenter(overlay.get()); });
    // Grow or shrink list of swap chain presenters to match pending overlays.
    ...
    std::sort(overlays.begin(), overlays.end(), [](const auto& a, const auto& b) -> bool { return a->z_order < b->z_order; });
    ...
    for (auto& overlay : overlays) {
        if (NeedSwapChainPresenter(overlay.get())) {
            ...
            if (!video_swap_chain) {
                ...
                video_swap_chain = std::make_unique<SwapChainPresenter>(this, d3d11_device_, dcomp_device_);
                if (frame_rate_ > 0) {
            ...
            gfx::Rect clip_rect;
            if (!video_swap_chain->PresentToSwapChain(*overlay, &transform, &clip_rect)) {
            ...
            overlay->overlay_image = DCLayerOverlayImage(video_swap_chain->content_size(), video_swap_chain->content());
...
{
    CHECK_IS_TEST();
    return solid_color_surface_pool_->GetNumSurfacesInPoolForTesting(); // IN-TEST
}

#if DCHECK_IS_ON()
bool DCLayerTree::GetAttachedToRootFromPreviousFrameForTesting(size_t index) const
{
    CHECK_IS_TEST();
    return visual_tree_ ? visual_tree_->GetAttachedToRootFromPreviousFrameForTesting( // IN-TEST
               index)
                        : false;
}
#endif // DCHECK_IS_ON()
...
void DCLayerTree::SetDelegatedInkTrailStartPoint(std::unique_ptr<gfx::DelegatedInkMetadata> metadata)
{
    DCHECK(SupportsDelegatedInk());
    pending_delegated_ink_metadata_ = std::move(metadata);
}

void DCLayerTree::InitDelegatedInkPointRendererReceiver(mojo::PendingReceiver<gfx::mojom::DelegatedInkPointRenderer> pending_receiver)
{
    DCHECK(SupportsDelegatedInk());
