...
GL_EXPORT void InitializeDirectComposition(Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device);
...
GL_EXPORT void SetDirectCompositionOverlayWorkarounds(const DirectCompositionOverlayWorkarounds& workarounds);
...
// Testing helpers.
GL_EXPORT void SetDirectCompositionScaledOverlaysSupportedForTesting(bool value);
GL_EXPORT void SetDirectCompositionOverlayFormatUsedForTesting(DXGI_FORMAT format);
GL_EXPORT void SetDirectCompositionMonitorInfoForTesting(int num_monitors, const gfx::Size& primary_monitor_size);

class GL_EXPORT DirectCompositionOverlayCapsObserver : public base::CheckedObserver {
public:
...
// overlay cap change.
class GL_EXPORT DirectCompositionOverlayCapsMonitor : public ui::GpuSwitchingObserver {
public:
    DirectCompositionOverlayCapsMonitor(const DirectCompositionOverlayCapsMonitor&) = delete;
    DirectCompositionOverlayCapsMonitor& operator=(const DirectCompositionOverlayCapsMonitor&) = delete;
...
private:
    ...
    scoped_refptr<base::ObserverListThreadSafe<DirectCompositionOverlayCapsObserver>> observer_list_;
};
} // namespace gl
