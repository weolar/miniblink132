...
void InitializeWindowClass()
{
    ...
    base::win::InitializeWindowClass(L"Intermediate D3D Window", &base::win::WrappedWindowProc<::DefWindowProc>, CS_OWNDC, 0, 0, nullptr,
...
public:
    ...
    static void Destroy(HWND window)
    {
        ...
        gfx::WindowImpl* window_data = reinterpret_cast<gfx::WindowImpl*>(gfx::GetWindowUserData(window));
...
// This runs on the window owner thread.
void CreateWindowsOnThread(base::WaitableEvent* event, HWND* child_window, HWND* parent_window)
{
    ...
    *parent_window = HiddenPopupWindow::Create();
    ...
    const HWND window = CreateWindowEx(WS_EX_NOPARENTNOTIFY | WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_NOREDIRECTIONBITMAP,
        reinterpret_cast<wchar_t*>(g_window_class), L"", WS_CHILDWINDOW | WS_DISABLED | WS_VISIBLE, 0, 0, /*width*/ 1,
...
class ChildWindowWin::ChildWindowThread : public base::RefCounted<ChildWindowThread> {
public:
...
void ChildWindowWin::Initialize()
{
    ...
    thread_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&CreateWindowsOnThread, &event, &window_, &initial_parent_window_));
    event.Wait();
}
...
ChildWindowWin::~ChildWindowWin()
{
    if (thread_) {
        ...
        task_runner->PostTaskAndReply(
            FROM_HERE, base::BindOnce(&DestroyWindowsOnThread, window_, initial_parent_window_), base::DoNothingWithBoundArgs(std::move(thread_)));
    }
}
...
void ChildWindowWin::Resize(const gfx::Size& size)
{
    ...
    constexpr UINT kFlags = SWP_NOACTIVATE | SWP_NOCOPYBITS | SWP_NOMOVE | SWP_NOOWNERZORDER | SWP_NOREDRAW | SWP_NOSENDCHANGING | SWP_NOZORDER;
    ...
    // window's demise.
    if (!::SetWindowPos(window_, nullptr, 0, 0, size.width(), size.height(), kFlags)) {
