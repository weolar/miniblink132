...
public:
    ...
    {
        return force_time_elapsed_query_;
    }

    GPUTiming::TimerType GetTimerType() const
    {
        return timer_type_;
    }
    ...
    {
        return cpu_time_for_testing_.is_null() ? (base::TimeTicks::Now() - base::TimeTicks()).InMicroseconds() : cpu_time_for_testing_.Run();
    }
    ...
    uint32_t GetElapsedQueryCount()
    {
        return elapsed_query_count_;
    }
    void IncElapsedQueryCount()
    {
        elapsed_query_count_++;
    }
    void DecElapsedQueryCount()
    {
        elapsed_query_count_--;
    }
...
public:
    ...
    {
        return end_value_ - start_value_;
    }
...
public:
    ...
    void EndQuery(GPUTimingImpl* gpu_timing, scoped_refptr<QueryResult> result)
    {
    ...
    void UpdateQueryResults(GPUTimingImpl* gpu_timing) override
    {
        ...
        const int64_t start_time = std::max(first_top_level_query_ ? query_begin_cpu_time_ : 0, std::max(prev_query_end_time_, gpu_timing->GetMaxTimeStamp()));
...
public:
    explicit TimeStampTimerQuery(uint32_t next_id)
        : TimerQuery(next_id)
    {
        glGenQueries(1, &gl_query_id_);
    }
...
scoped_refptr<QueryResult> GPUTimingImpl::BeginElapsedTimeQuery()
{
    ...
    return static_cast<TimeElapsedTimerQuery*>(queries_.back().get())->StartQuery(this);
}
...
void GPUTimingImpl::EndElapsedTimeQuery(scoped_refptr<QueryResult> result)
{
    ...
    if (GetElapsedQueryCount() > 1) {
        ...
        queries_.push_back(new TimeElapsedTimerQuery(this, next_timer_query_id_++));
        static_cast<TimeElapsedTimerQuery*>(queries_.back().get())->EndQuery(this, result);
    } else {
        // Simply end the query and reset the current offset
...
void GPUTimingImpl::SetLastElapsedQuery(scoped_refptr<TimeElapsedTimerQuery> query)
{
...
GPUTimer::GPUTimer(scoped_refptr<GPUTimingClient> gpu_timing_client, bool use_elapsed_timer)
...
std::unique_ptr<GPUTimer> GPUTimingClient::CreateGPUTimer(bool prefer_elapsed_time)
{
    if (gpu_timing_)
...
void GPUTimingClient::SetCpuTimeForTesting(base::RepeatingCallback<int64_t(void)> cpu_time)
{
    DCHECK(gpu_timing_);
