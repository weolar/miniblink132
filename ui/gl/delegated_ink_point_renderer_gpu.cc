...
bool TraceEventOnFailure(HRESULT hr, const char* name)
{
    ...
    TRACE_EVENT_INSTANT1("delegated_ink_trails", name, TRACE_EVENT_SCOPE_THREAD, "hr", hr);
    return true;
}
...
void DelegatedInkPointRendererGpu::InitMessagePipeline(mojo::PendingReceiver<gfx::mojom::DelegatedInkPointRenderer> pending_receiver)
{
...
bool DelegatedInkPointRendererGpu::Initialize(IDCompositionDevice2* dcomp_device2, IDXGISwapChain1* root_swap_chain)
{
    ...
    if (!ink_trail_device_
        && TraceEventOnFailure(dcomp_device2->QueryInterface(IID_PPV_ARGS(&ink_trail_device_)),
            "DelegatedInkPointRendererGpu::Initialize - "
            "DCompDevice2 as InkTrailDevice failed")) {
        return false;
    }

    if (TraceEventOnFailure(ink_trail_device_->CreateDelegatedInkTrailForSwapChain(root_swap_chain, &delegated_ink_trail_),
...
bool DelegatedInkPointRendererGpu::DelegatedInkIsSupported(const Microsoft::WRL::ComPtr<IDCompositionDevice2>& dcomp_device) const
{
...
uint64_t DelegatedInkPointRendererGpu::GetMaximumNumberOfPointerIdsForTesting() const
{
    CHECK_IS_TEST();
...
void DelegatedInkPointRendererGpu::ReportPointsDrawn()
{
    ...
    if (metadata_paint_time_.has_value()) {
        base::UmaHistogramCustomTimes(
            "Renderer.DelegatedInkTrail.OS.TimeFromDelegatedInkToApiPaint", now - metadata_paint_time_.value(), base::Milliseconds(1), base::Seconds(1), 50);
    ...
    for (const auto& point : points_to_be_drawn_) {
        UMA_HISTOGRAM_TIMES("Renderer.DelegatedInkTrail.OS.TimeToDrawPointsMillis", now - point.timestamp());
        ...
        // paint time.
        DelegatedInkPointRendererGpu::DelegatedInkPointTokenMap& token_map = delegated_ink_points_[point.pointer_id()];
    ...
    base::UmaHistogramTimes("Renderer.DelegatedInkTrail.LatencyImprovement.OS.WithoutPrediction", most_recent_timestamp - metadata_->timestamp());
    base::UmaHistogramCounts100("Renderer.DelegatedInkTrail.OS.OutstandingPointsToDraw", points_to_be_drawn_.size());

    points_to_be_drawn_.clear();
}

void DelegatedInkPointRendererGpu::SetDelegatedInkTrailStartPoint(std::unique_ptr<gfx::DelegatedInkMetadata> metadata)
{
    TRACE_EVENT_WITH_FLOW1("delegated_ink_trails", "DelegatedInkPointRendererGpu::SetDelegatedInkTrailStartPoint", TRACE_ID_GLOBAL(metadata->trace_id()),
        TRACE_EVENT_FLAG_FLOW_IN, "metadata", metadata->ToString());
    ...
    //     re-initialized.
    // (continued below)
    if (!force_new_ink_trail_ && metadata_ && metadata->color() == metadata_->color() && pointer_id_) {
        DelegatedInkPointRendererGpu::DelegatedInkPointTokenMap& token_map = delegated_ink_points_[pointer_id_.value()];
        auto point_matching_metadata_it = token_map.find(gfx::DelegatedInkPoint(metadata->point(), metadata->timestamp()));
        ...
        if (point_matching_metadata_it != token_map.end()) {
            gfx::DelegatedInkPoint point_matching_metadata = point_matching_metadata_it->first;
            ...
            // altogether.
            if (point_matching_metadata.MatchesDelegatedInkMetadata(metadata.get()) && token) {
                metadata_paint_time_ = point_matching_metadata.paint_timestamp();
                bool remove_trail_points_failed = TraceEventOnFailure(delegated_ink_trail_->RemoveTrailPoints(token.value()),
                ...
                if (!remove_trail_points_failed) {
                    ...
                    token_map.erase(token_map.begin(), std::next(point_matching_metadata_it));
                    // Ensure that points that are being removed from the trail are not
                    // being reported as painted in `ReportPointsDrawn()`.
                    points_to_be_drawn_.erase(std::remove_if(points_to_be_drawn_.begin(), points_to_be_drawn_.end(),
                                                  [&](const gfx::DelegatedInkPoint& x) { return metadata->timestamp() > x.timestamp(); }),
    ...
    if (TraceEventOnFailure(delegated_ink_trail_->StartNewTrail(d2d1_color),
            "DelegatedInkPointRendererGpu::SetDelegatedInkTrailStartPoint - "
...
void DelegatedInkPointRendererGpu::StoreDelegatedInkPoint(const gfx::DelegatedInkPoint& point)
{
    TRACE_EVENT_WITH_FLOW1("delegated_ink_trails", "DelegatedInkPointRendererGpu::StoreDelegatedInkPoint", TRACE_ID_GLOBAL(point.trace_id()),
        TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "point", point.ToString());

    const int32_t pointer_id = point.pointer_id();

    DCHECK(delegated_ink_points_.find(pointer_id) == delegated_ink_points_.end() || delegated_ink_points_[pointer_id].empty()
        || point.timestamp() > delegated_ink_points_[pointer_id].rbegin()->first.timestamp());

    if (metadata_ && point.timestamp() < metadata_->timestamp()) {
        return;
    }

    DelegatedInkPointRendererGpu::DelegatedInkPointTokenMap& token_map = delegated_ink_points_[pointer_id];
...
bool DelegatedInkPointRendererGpu::CheckForPointerIdForTesting(int32_t pointer_id) const
{
    CHECK_IS_TEST();
...
void DelegatedInkPointRendererGpu::EraseExcessPointerIds()
{
    ...
    while (token_map_it != delegated_ink_points_.end()) {
        const DelegatedInkPointRendererGpu::DelegatedInkPointTokenMap& token_map = token_map_it->second;
        if (token_map.empty()) {
    ...
    if (delegated_ink_points_.size() > kMaximumNumberOfPointerIds) {
        std::vector<std::pair<base::TimeTicks, int32_t>> earliest_time_and_pointer_id;
        for (const auto& it : delegated_ink_points_) {
            earliest_time_and_pointer_id.emplace_back(it.second.rbegin()->first.timestamp(), it.first);
        }

        std::sort(earliest_time_and_pointer_id.begin(), earliest_time_and_pointer_id.end());

        uint64_t pointer_id_to_remove = 0;
        while (pointer_id_to_remove < earliest_time_and_pointer_id.size() && pointer_id_
            && earliest_time_and_pointer_id[pointer_id_to_remove].second == pointer_id_.value()) {
            pointer_id_to_remove++;
        }

        CHECK_LT(pointer_id_to_remove, earliest_time_and_pointer_id.size());
        delegated_ink_points_.erase(earliest_time_and_pointer_id[pointer_id_to_remove].second);
    }
...
{
    if (pointer_id_ && delegated_ink_points_.find(pointer_id_.value()) != delegated_ink_points_.end()) {
        ...
        const gfx::DelegatedInkPoint& point_matching_metadata = delegated_ink_points_[pointer_id_.value()].begin()->first;
    ...
    for (auto token_map_it = delegated_ink_points_.begin(); token_map_it != delegated_ink_points_.end() && !pointer_id_; ++token_map_it) {
...
void DelegatedInkPointRendererGpu::DrawSavedTrailPoints()
{
    ...
    for (auto& it : delegated_ink_points_) {
        DelegatedInkPointRendererGpu::DelegatedInkPointTokenMap& token_map = it.second;
        token_map.erase(token_map.begin(), token_map.lower_bound(gfx::DelegatedInkPoint(metadata_->point(), metadata_->timestamp())));
    }
    ...
    if (pointer_id && delegated_ink_points_.find(pointer_id.value()) != delegated_ink_points_.end()) {
        DelegatedInkPointRendererGpu::DelegatedInkPointTokenMap& token_map = delegated_ink_points_[pointer_id.value()];
        if (!token_map.empty() && token_map.begin()->first.MatchesDelegatedInkMetadata(metadata_.get())) {
        ...
        TRACE_EVENT_INSTANT0("delegated_ink_trails", "DrawSavedTrailPoints failed - no pointer id", TRACE_EVENT_SCOPE_THREAD);
    }
}

std::unique_ptr<DCLayerOverlayParams> DelegatedInkPointRendererGpu::MakeDelegatedInkOverlay(
    IDCompositionDevice2* dcomp_device2, IDXGISwapChain1* root_swap_chain, std::unique_ptr<gfx::DelegatedInkMetadata> metadata)
{
    ...
    const gfx::Rect presentation_rect = gfx::ToEnclosedRect(metadata->presentation_area());
    const gfx::Size presentation_area_enclosed_size = gfx::Size(presentation_rect.width(), presentation_rect.height());
    ...
    ink_layer->clip_rect = std::make_optional<gfx::Rect>(0, 0, presentation_rect.right(), presentation_rect.bottom());
    ink_layer->transform = gfx::Transform::MakeTranslation(metadata->presentation_area().OffsetFromOrigin());
    ink_layer->overlay_image = DCLayerOverlayImage(presentation_area_enclosed_size, delegated_ink_trail_);
    SetDelegatedInkTrailStartPoint(std::move(metadata));
    return ink_layer;
}

bool DelegatedInkPointRendererGpu::DrawDelegatedInkPoint(const gfx::DelegatedInkPoint& point)
{
    ...
    // a time.
    if (TraceEventOnFailure(delegated_ink_trail_->AddTrailPoints(&ink_point,
    ...
    TRACE_EVENT_WITH_FLOW1("delegated_ink_trails",
        ...
        TRACE_ID_GLOBAL(point.trace_id()), TRACE_EVENT_FLAG_FLOW_IN, "point", point.ToString());

    if (point.timestamp().IsHighResolution() && point.timestamp().IsConsistentAcrossProcesses()) {
...
void DelegatedInkPointRendererGpu::InitializeForTesting(IDCompositionDevice2* dcomp_device2)
{
    CHECK_IS_TEST();
