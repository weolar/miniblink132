...
NativeViewGLSurfaceEGLX11GLES2::NativeViewGLSurfaceEGLX11GLES2(gl::GLDisplayEGL* display, x11::Window window)
...
bool NativeViewGLSurfaceEGLX11GLES2::InitializeNativeWindow()
{
    ...
    if (!geometry) {
        LOG(ERROR) << "GetGeometry failed for window " << static_cast<uint32_t>(parent_window_) << ".";
        return false;
    }
...
EGLConfig NativeViewGLSurfaceEGLX11GLES2::GetConfig()
{
    if (!config_) {
        ...
        std::array<EGLint, 15> config_attribs { { EGL_BUFFER_SIZE, ~0, EGL_ALPHA_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_RED_SIZE, 8,
            EGL_RENDERABLE_TYPE, EGL_OPENGL_ES2_BIT, EGL_SURFACE_TYPE, EGL_WINDOW_BIT | EGL_PBUFFER_BIT, EGL_NONE } };
        config_attribs[kBufferSizeOffset] = geometry->depth;

        EGLDisplay display = GLSurfaceEGL::GetGLDisplayEGL()->GetDisplay();
        x11::VisualId visual_id;
        connection->GetOrCreateVisualManager().ChooseVisualForWindow(true, &visual_id, nullptr, nullptr, nullptr);
        EGLint num_configs;
        if (!eglChooseConfig(display, config_attribs.data(), nullptr, 0, &num_configs)) {
        ...
        if (num_configs) {
            if (!eglChooseConfig(display, config_attribs.data(), &configs.front(), num_configs, &num_configs)) {
                LOG(ERROR) << "eglChooseConfig failed with error " << GetLastEGLErrorString();
                return nullptr;
            }
            for (EGLConfig config : configs) {
                ...
                if (!eglGetConfigAttrib(display, config, EGL_NATIVE_VISUAL_ID, &config_visual_id)) {
                    LOG(ERROR) << "eglGetConfigAttrib failed with error " << GetLastEGLErrorString();
                    return nullptr;
                }
                if (config_depth == geometry->depth && config_visual_id == static_cast<EGLint>(visual_id)) {
                    config_ = config;
                    return config_;
                }
            }
        ...
        if (!eglChooseConfig(display, config_attribs.data(), &config_, 1, &num_configs)) {
            LOG(ERROR) << "eglChooseConfig failed with error " << GetLastEGLErrorString();
            return nullptr;
        }
...
bool NativeViewGLSurfaceEGLX11GLES2::Resize(const gfx::Size& size, float scale_factor, const gfx::ColorSpace& color_space, bool has_alpha)
{
    if (size == GetSize())
        return true;
...
void NativeViewGLSurfaceEGLX11GLES2::OnEvent(const x11::Event& x11_event)
{
    ...
    x11::Connection::Get()->SendEvent(expose_copy, parent_window_, x11::EventMask::Exposure);
