...
public:
    ...
    void RemoveGpuPreference(GpuPreference preference)
    {
        ...
        for (auto iter = gpu_preference_map_.begin(); iter != gpu_preference_map_.end();
            /* no increment */) {
        ...
        if (!gpu_preference_map_.empty()) {
            ...
            if (iter == gpu_preference_map_.end()) {
                gpu_preference_map_[GpuPreference::kDefault] = gpu_preference_map_.begin()->second;
            }
        }
    ...
    uint64_t GetSystemDeviceId(GpuPreference preference)
    {
        ...
        auto iter = gpu_preference_map_.find(preference);
        if (!SupportsEGLDualGPURendering() || (iter == gpu_preference_map_.end() && preference != GpuPreference::kDefault)) {
    ...
    {
        return features::SupportsEGLDualGPURendering() || override_egl_dual_gpu_rendering_support_for_tests_;
    }

    GLDisplayPlatform* GetDisplay(GpuPreference preference, gl::DisplayKey display_key)
    {
...
private:
    ...
    GLDisplayPlatform* GetDisplay(uint64_t system_device_id, gl::DisplayKey display_key)
    {
        base::AutoLock auto_lock(lock_);
        for (const auto& display : displays_) {
            if (display->system_device_id() == system_device_id && display->display_key() == display_key) {
                return display.get();
            }
        }

        std::unique_ptr<GLDisplayPlatform> display(new GLDisplayPlatform(system_device_id, display_key));
...
extern template class EXPORT_TEMPLATE_DECLARE(GL_EXPORT) GLDisplayManager<GLDisplayEGL>;
