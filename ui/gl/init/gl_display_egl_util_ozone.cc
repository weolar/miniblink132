...
void GLDisplayEglUtilOzone::GetPlatformExtraDisplayAttribs(EGLenum platform_type, std::vector<EGLAttrib>* attributes)
{
...
void GLDisplayEglUtilOzone::ChoosePlatformCustomAlphaAndBufferSize(EGLint* alpha_size, EGLint* buffer_size)
{
    auto* utility = ui::OzonePlatform::GetInstance()->GetPlatformGLEGLUtility();
    if (utility)
...
std::optional<base::ScopedEnvironmentVariableOverride> GLDisplayEglUtilOzone::MaybeGetScopedDisplayUnsetForVulkan()
{
    auto* utility = ui::OzonePlatform::GetInstance()->GetPlatformGLEGLUtility();
    if (utility)
