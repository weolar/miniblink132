...
{
    return HasGLOzone() ? GetGLOzone()->GetGLWindowSystemBindingInfo(gl_info, info) : false;
}
...
scoped_refptr<GLContext> CreateGLContext(GLShareGroup* share_group, GLSurface* compatible_surface, const GLContextAttribs& attribs)
{
    ...
    if (HasGLOzone()) {
        return GetGLOzone()->CreateGLContext(share_group, compatible_surface, attribs);
    }
    ...
    case kGLImplementationStubGL: {
        scoped_refptr<GLContextStub> stub_context = base::MakeRefCounted<GLContextStub>(share_group);
...
scoped_refptr<Presenter> CreateSurfacelessViewGLSurface(GLDisplay* display, gfx::AcceleratedWidget window)
{
    TRACE_EVENT0("gpu", "gl::init::CreateSurfacelessViewGLSurface");
    return HasGLOzone() ? GetGLOzone()->CreateSurfacelessViewGLSurface(display, window) : nullptr;
}
