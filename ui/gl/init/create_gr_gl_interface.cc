...
BASE_FEATURE(kAddDelayToGLProgramLink, "AddDelayToGLProgramLink", base::FEATURE_DISABLED_BY_DEFAULT);
constexpr base::FeatureParam<int> kGLProgramLinkDelayMicroseconds { &kAddDelayToGLProgramLink, /*name=*/"GLProgramLinkDelayMicroseconds",
    /*default_value=*/1000 };
...
GLsync glFenceSyncEmulateEGL(GLenum condition, GLbitfield flags)
{
    ...
    const EGLenum syncType = display && display->ext->b_EGL_ANGLE_global_fence_sync ? EGL_SYNC_GLOBAL_FENCE_ANGLE : EGL_SYNC_FENCE_KHR;
...
GLenum glClientWaitSyncEmulateEGL(GLsync sync, GLbitfield flags, GLuint64 timeout)
{
    ...
    EGLint result = eglClientWaitSyncKHR(data->display, data->sync, egl_flags, timeout);
...
template <typename R, typename... Args> GrGLFunction<R GR_GL_FUNCTION_TYPE(Args...)> bind_with_api(R (gl::GLApi::*func)(Args...), gl::GLApi* api)
{
...
struct FlushHelper {
    FlushHelper()
    {
        TRACE_EVENT0("gpu", "CreateGrGLInterface - bind_with_flush_on_mac - beforefunc");
        glFlush();
    }
    ~FlushHelper()
    {
        TRACE_EVENT0("gpu", "CreateGrGLInterface - bind_with_flush_on_mac - afterfunc");
        glFlush();
    }
};

template <bool droppable, bool slow, bool need_flush, typename R, typename... Args>
GrGLFunction<R GR_GL_FUNCTION_TYPE(Args...)> bind_impl(R(GL_BINDING_CALL* func)(Args...), gl::ProgressReporter* progress_reporter)
{
...
GrGLFunction<R GR_GL_FUNCTION_TYPE(GLuint, Args...)> bind_timed_compile_function(
    R(GL_BINDING_CALL* func)(GLuint shader, Args...), glGetShaderivProc get_shader_iv, gl::ProgressReporter* progress_reporter)
{
    // Don't wrap missing functions.
    if (!func) {
        return nullptr;
    ...
    return [func, get_shader_iv, progress_reporter](GLuint shader, Args... args) -> R {
...
GrGLFunction<R GR_GL_FUNCTION_TYPE(GLuint, Args...)> bind_timed_link_function(
    R(GL_BINDING_CALL* func)(GLuint program, Args...), glGetProgramivProc get_program_iv, gl::ProgressReporter* progress_reporter)
{
    // Don't wrap missing functions.
    if (!func) {
        return nullptr;
    ...
    return [func, get_program_iv, progress_reporter](GLuint program, Args... args) -> R {
        gl::ScopedProgressReporter scoped_reporter(progress_reporter);
        SCOPED_UMA_HISTOGRAM_TIMER_MICROS("Gpu.GrLinkProgramUs");

        if (base::FeatureList::IsEnabled(kAddDelayToGLProgramLink)) {
            base::PlatformThread::Sleep(base::Microseconds(kGLProgramLinkDelayMicroseconds.Get()));
        }
...
template <typename... Traits> struct BindWithTraits {
    template <typename R, typename... Args>
    static GrGLFunction<R GR_GL_FUNCTION_TYPE(Args...)> bind(R(GL_BINDING_CALL* func)(Args...), gl::ProgressReporter* progress_reporter, bool is_angle)
    {
        constexpr bool droppable = base::trait_helpers::HasTrait<Droppable, Traits...>();
        constexpr bool slow = base::trait_helpers::HasTrait<Slow, Traits...>();
        constexpr bool need_flush = base::trait_helpers::HasTrait<NeedFlush, Traits...>();

#if BUILDFLAG(IS_MAC)
        // If running on Apple silicon, regardless of the architecture, don't
        // perform this workaround. See https://crbug.com/1131312.
        if (need_flush && base::mac::GetCPUType() == base::mac::CPUType::kIntel && !is_angle) {
            return bind_impl<droppable, slow, /*need_flush=*/true>(func, progress_reporter);
        } else {
            return bind_impl<droppable, slow, /*need_flush=*/false>(func, progress_reporter);
        }
#else
...
const GLubyte* GetStringHook(const char* gl_version_string, const char* glsl_version_string, GLenum name)
{
    switch (name) {
    case GL_VERSION:
...
sk_sp<GrGLInterface> CreateGrGLInterface(const gl::GLVersionInfo& version_info, gl::ProgressReporter* progress_reporter)
{
    ...
    if (apply_version_override) {
        ...
        get_string = [version](GLenum name) { return GetStringHook(version.gl_version, version.glsl_version, name); };
    } else {
    ...
    auto timed_compile_shader = bind_timed_compile_function(gl->glCompileShaderFn, gl->glGetShaderivFn, progress_reporter);

    auto timed_link_program = bind_timed_link_function(gl->glLinkProgramFn, gl->glGetProgramivFn, progress_reporter);
...
#define BIND_EXTENSION(skia_name, chrome_name, ...)                                                                                                            \
    functions->f##skia_name = BindWithTraits<__VA_ARGS__>::bind(gl->gl##chrome_name##Fn, progress_reporter, version_info.is_angle)
#define BIND(fname, ...) BIND_EXTENSION(fname, fname, __VA_ARGS__)
    ...
    BIND_EXTENSION(DrawArraysInstanced, DrawArraysInstancedANGLE, Droppable, SlowOnMac);
    BIND_EXTENSION(DrawArraysInstancedBaseInstance, DrawArraysInstancedBaseInstanceANGLE, Droppable, SlowOnMac);
    BIND_EXTENSION(MultiDrawArraysInstancedBaseInstance, MultiDrawArraysInstancedBaseInstanceANGLE, Droppable, SlowOnMac);
    BIND_EXTENSION(DrawElementsInstanced, DrawElementsInstancedANGLE, Droppable, SlowOnMac);
    BIND_EXTENSION(DrawElementsInstancedBaseVertexBaseInstance, DrawElementsInstancedBaseVertexBaseInstanceANGLE, Droppable, SlowOnMac);
    BIND_EXTENSION(MultiDrawElementsInstancedBaseVertexBaseInstance, MultiDrawElementsInstancedBaseVertexBaseInstanceANGLE, Droppable, SlowOnMac);
    ...
    BIND_EXTENSION(GetFramebufferAttachmentParameteriv, GetFramebufferAttachmentParameterivEXT);
    ...
    BIND_EXTENSION(DeleteFramebuffers, DeleteFramebuffersEXT, Slow, NeedFlushOnMac);
    ...
    BIND_EXTENSION(FramebufferTexture2DMultisample, FramebufferTexture2DMultisampleEXT);
    BIND_EXTENSION(RenderbufferStorageMultisampleES2EXT, RenderbufferStorageMultisampleEXT, NeedFlushOnMac);
    ...
    if (gl->glDebugMessageControlFn && gl->glDebugMessageInsertFn && gl->glDebugMessageCallbackFn && gl->glGetDebugMessageLogFn && gl->glPushDebugGroupFn
        && gl->glPopDebugGroupFn && gl->glObjectLabelFn) {
