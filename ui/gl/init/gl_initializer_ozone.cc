...
GLDisplay* InitializeGLOneOffPlatform(gl::GpuPreference gpu_preference)
{
    if (HasGLOzone()) {
        ...
        return GetGLOzone()->InitializeGLOneOffPlatform(supports_angle, init_displays, gpu_preference);
    }
...
bool InitializeStaticGLBindings(GLImplementationParts implementation)
{
    ...
    if (HasGLOzone(implementation)) {
        return GetGLOzone(implementation)->InitializeStaticGLBindings(implementation);
    }
