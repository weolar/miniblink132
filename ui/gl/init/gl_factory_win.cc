...
scoped_refptr<GLSurface> CreateViewGLSurface(GLDisplay* display, gfx::AcceleratedWidget window)
{
    ...
    case kGLImplementationEGLANGLE: {
        ...
        return InitializeGLSurface(
            base::MakeRefCounted<NativeViewGLSurfaceEGL>(display->GetAs<gl::GLDisplayEGL>(), window, std::make_unique<VSyncProviderWin>(window)));
    }
    case kGLImplementationMockGL:
    case kGLImplementationStubGL:
...
bool InitializeExtensionSettingsOneOffPlatform(GLDisplay* display)
{
    ...
    switch (implementation) {
    case kGLImplementationEGLANGLE:
        return InitializeExtensionSettingsOneOffEGL(display->GetAs<GLDisplayEGL>());
    case kGLImplementationMockGL:
    case kGLImplementationStubGL:
        return true;
    default:
