...
bool LoadD3DXLibrary(const base::FilePath& module_path, const base::FilePath::StringType& name)
{
    base::NativeLibrary library = base::LoadNativeLibrary(module_path.Append(name), nullptr);
    if (!library) {
...
bool InitializeStaticEGLInternalFromLibrary(GLImplementation implementation)
{
    ...
    base::NativeLibrary gles_library = base::LoadNativeLibrary(gles_path.Append(L"libglesv2.dll"), nullptr);
    if (!gles_library) {
    ...
    base::NativeLibrary egl_library = base::LoadNativeLibrary(gles_path.Append(L"libegl.dll"), nullptr);
    if (!egl_library) {
