...
public:
    ...
    // Implement GLContext.
    bool InitializeImpl(GLSurface* compatible_surface, const GLContextAttribs& attribs) override;
...
bool GLNonOwnedContext::InitializeImpl(GLSurface* compatible_surface, const GLContextAttribs& attribs)
{
...
bool GetGLWindowSystemBindingInfo(const GLVersionInfo& gl_info, GLWindowSystemBindingInfo* info)
{
...
scoped_refptr<GLContext> CreateGLContext(GLShareGroup* share_group, GLSurface* compatible_surface, const GLContextAttribs& attribs)
{
    ...
    case kGLImplementationStubGL: {
        scoped_refptr<GLContextStub> stub_context = MakeRefCounted<GLContextStub>(share_group);
    ...
    case kGLImplementationDisabled:
        NOTREACHED();
    default:
        if (compatible_surface->GetHandle() || compatible_surface->IsSurfaceless()) {
            return InitializeGLContext(new GLContextEGL(share_group), compatible_surface, attribs);
        } else {
            return InitializeGLContext(new GLNonOwnedContext(share_group), compatible_surface, attribs);
        }
    }
}

scoped_refptr<GLSurface> CreateViewGLSurface(GLDisplay* display, gfx::AcceleratedWidget window)
{
    ...
    case kGLImplementationEGLANGLE:
        if (window != gfx::kNullAcceleratedWidget) {
            return InitializeGLSurface(new NativeViewGLSurfaceEGL(display->GetAs<gl::GLDisplayEGL>(), ScopedANativeWindow::Wrap(window), nullptr));
        } else {
...
bool InitializeExtensionSettingsOneOffPlatform(GLDisplay* display)
{
    ...
    case kGLImplementationEGLGLES2:
    case kGLImplementationEGLANGLE:
        return InitializeExtensionSettingsOneOffEGL(static_cast<GLDisplayEGL*>(display));
    case kGLImplementationMockGL:
    case kGLImplementationStubGL:
        return true;
    default:
