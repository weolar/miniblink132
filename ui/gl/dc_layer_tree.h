...
struct VideoProcessorWrapper {
    ...
    bool GetDriverSupportsVpAutoHdr()
    {
        return driver_supports_vp_auto_hdr;
    }
...
public:
    SolidColorSurfacePool(Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device, Microsoft::WRL::ComPtr<IDCompositionDevice3> dcomp_device);
...
public:
    ...
    DCLayerTree(bool disable_nv12_dynamic_textures, bool disable_vp_auto_hdr, bool disable_vp_scaling, bool disable_vp_super_resolution,
    ...
    void Initialize(HWND window, Microsoft::WRL::ComPtr<ID3D11Device> d3d11_device);
    ...
    bool CommitAndClearPendingOverlays(std::vector<std::unique_ptr<DCLayerOverlayParams>> overlays);
    ...
    bool disable_vp_auto_hdr() const
    {
        return disable_vp_auto_hdr_;
    }

    bool disable_vp_scaling() const
    {
        return disable_vp_scaling_;
    }
    ...
    Microsoft::WRL::ComPtr<IDXGISwapChain1> GetLayerSwapChainForTesting(size_t index) const;

    void GetSwapChainVisualInfoForTesting(size_t index, gfx::Transform* transform, gfx::Point* offset, gfx::Rect* clip_rect) const;
    ...
    {
        CHECK_IS_TEST();
        return visual_tree_ ? visual_tree_->GetContentVisualForTesting(index) : nullptr;
    }
    IDCompositionSurface* GetBackgroundColorSurfaceForTesting(size_t index) const
    {
        CHECK_IS_TEST();
        return visual_tree_ ? visual_tree_->GetBackgroundColorSurfaceForTesting(index) : nullptr;
    }
    size_t GetNumSurfacesInPoolForTesting() const;
#if DCHECK_IS_ON()
    ...
    void SetDelegatedInkTrailStartPoint(std::unique_ptr<gfx::DelegatedInkMetadata>);
    ...
    public:
        ...
        bool BuildTree(const std::vector<std::unique_ptr<DCLayerOverlayParams>>& overlays);
        ...
        {
            CHECK_IS_TEST();
            return visual_subtrees_[index]->GetAttachedToRootFromPreviousFrameForTesting();
        }
#endif // DCHECK_IS_ON()
        ...
        public:
            ...
            bool Update(IDCompositionDevice3* dcomp_device, Microsoft::WRL::ComPtr<IUnknown> dcomp_visual_content, uint64_t dcomp_surface_serial,
                const gfx::Size& image_size, const gfx::RectF& content_rect, Microsoft::WRL::ComPtr<IDCompositionSurface> background_color_surface,
                const SkColor4f& background_color, const gfx::Rect& quad_rect, bool nearest_neighbor_filter, const gfx::Transform& quad_to_root_transform,
                const gfx::RRectF& rounded_corner_bounds, float opacity, const std::optional<gfx::Rect>& clip_rect_in_root, bool allow_antialiasing);
            ...
            void GetSwapChainVisualInfoForTesting(gfx::Transform* transform, gfx::Point* offset, gfx::Rect* clip_rect) const;
#if DCHECK_IS_ON()
    ...
    private:
        ...
        VisualSubtreeMap BuildMapAndAssignMatchingSubtrees(const std::vector<std::unique_ptr<DCLayerOverlayParams>>& overlays,
            std::vector<std::unique_ptr<VisualSubtree>>& visual_subtrees, std::vector<std::optional<size_t>>& overlay_index_to_reused_subtree,
        ...
        size_t ReuseUnmatchedSubtrees(std::vector<std::unique_ptr<VisualSubtree>>& new_visual_subtrees,
        ...
        bool DetachUnusedSubtreesFromRoot(size_t first_prev_frame_subtree_unused_index, std::vector<bool>& prev_subtree_is_attached_to_root);
        ...
        // Updates |prev_subtree_is_attached_to_root| accordingly.
        // Returns true if commit is needed.
        bool DetachReusedSubtreesThatNeedRepositioningFromRoot(const std::vector<std::unique_ptr<VisualSubtree>>& new_visual_subtrees,
