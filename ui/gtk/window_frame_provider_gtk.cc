...
{
    auto context = WindowContext(solid_frame, tiled, focused);
    context = AppendCssNodeToStyleContext(context, "headerbar.header-bar.titlebar");
    if (!focused) {
        gtk_style_context_set_state(context, GTK_STATE_FLAG_BACKDROP);
    }
...
SkBitmap PaintBitmap(const gfx::Size& bitmap_size, const gfx::RectF& render_bounds, GtkCssContext context, float scale)
{
    SkBitmap bitmap;
    ...
    gtk_render_background(context, cr, render_bounds.x(), render_bounds.y(), render_bounds.width(), render_bounds.height());
    gtk_render_frame(context, cr, render_bounds.x(), render_bounds.y(), render_bounds.width(), render_bounds.height());
...
SkBitmap PaintHeaderbar(const gfx::Size& size, GtkCssContext context, float scale)
{
    gfx::RectF tabstrip_bounds_dip(0, 0, size.width() / scale, size.height() / scale);
...
int ComputeTopCornerRadius()
{
    ...
    auto context = GtkCheckVersion(4) ? DecorationContext(false, false, false) : HeaderContext(false, false, false);
    ...
    auto bitmap = GtkCheckVersion(4) ? PaintBitmap(size_dip, { { 0, 0 }, gfx::SizeF(size_dip) }, context, 1) : PaintHeaderbar(size_dip, context, 1);
    DCHECK_EQ(bitmap.width(), bitmap.height());
    for (int i = 0; i < bitmap.width(); ++i) {
        if (SkColorGetA(bitmap.getColor(0, i)) == 255 && SkColorGetA(bitmap.getColor(i, 0)) == 255) {
            return i;
        }
    }
    return bitmap.width();
...
WindowFrameProviderGtk::Asset::Asset(const WindowFrameProviderGtk::Asset& src) = default;

WindowFrameProviderGtk::Asset& WindowFrameProviderGtk::Asset::operator=(const WindowFrameProviderGtk::Asset& src) = default;
    ...
    , tiled_(tiled)
{
    ...
    auto callback = base::BindRepeating(&WindowFrameProviderGtk::OnThemeChanged, base::Unretained(this));
    theme_name_signal_ = ScopedGSignal(settings, "notify::gtk-theme-name", callback, G_CONNECT_AFTER);
    prefer_dark_signal_ = ScopedGSignal(settings, "notify::gtk-application-prefer-dark-theme", callback, G_CONNECT_AFTER);
}
...
gfx::Insets WindowFrameProviderGtk::GetFrameThicknessDip()
{
    if (!frame_thickness_dip_.has_value()) {
        ...
        frame_thickness_dip_ = gfx::Insets::TLBR(get_inset([&](int i) { return asset.focused_bitmap.getColor(2 * asset.frame_size_px, i); }),
            get_inset([&](int i) { return asset.focused_bitmap.getColor(i, 2 * asset.frame_size_px); }),
            get_inset([&](int i) { return asset.focused_bitmap.getColor(2 * asset.frame_size_px, BitmapSizePx(asset) - i - 1); }),
            get_inset([&](int i) { return asset.focused_bitmap.getColor(BitmapSizePx(asset) - i - 1, 2 * asset.frame_size_px); }));
    }
    return *frame_thickness_dip_;
}

void WindowFrameProviderGtk::PaintWindowFrame(
    gfx::Canvas* canvas, const gfx::Rect& rect_dip, int top_area_height_dip, bool focused, const gfx::Insets& input_insets)
{
    ...
    auto effective_frame_thickness_px = gfx::ScaleToRoundedInsets(GetFrameThicknessDip(), scale);
    ...
    src_rect.Inset(gfx::Insets(asset.frame_size_px) - effective_frame_thickness_px);
    ...
    auto corner_insets = effective_frame_thickness_px + gfx::Insets::VH(corner_h, corner_w);

    auto image = gfx::ImageSkia::CreateFrom1xBitmap(focused ? asset.focused_bitmap : asset.unfocused_bitmap);

    auto draw_image = [&](int src_x, int src_y, int src_w, int src_h, int dst_x, int dst_y, int dst_w, int dst_h) {
        if (src_w <= 0 || src_h <= 0 || dst_w <= 0 || dst_h <= 0) {
            return;
        }
        canvas->DrawImageInt(image, src_x, src_y, src_w, src_h, dst_x, dst_y, dst_w, dst_h, false);
    };

    // Top left corner
    draw_image(src_rect.x(), src_rect.y(), corner_insets.left(), corner_insets.top(), 0, 0, corner_insets.left(), corner_insets.top());
    // Top right corner
    draw_image(BitmapSizePx(asset) - asset.frame_size_px - corner_w, src_rect.y(), corner_insets.right(), corner_insets.top(),
        client_bounds_px.right() - corner_w, 0, corner_insets.right(), corner_insets.top());
    // Bottom left corner
    draw_image(src_rect.x(), BitmapSizePx(asset) - asset.frame_size_px - corner_h, corner_insets.left(), corner_insets.bottom(), 0,
        client_bounds_px.bottom() - corner_h, corner_insets.left(), corner_insets.bottom());
    // Bottom right corner
    draw_image(BitmapSizePx(asset) - asset.frame_size_px - corner_w, BitmapSizePx(asset) - asset.frame_size_px - corner_h, corner_insets.right(),
        corner_insets.bottom(), client_bounds_px.right() - corner_w, client_bounds_px.bottom() - corner_h, corner_insets.right(), corner_insets.bottom());
    // Top edge
    draw_image(
        2 * asset.frame_size_px, src_rect.y(), 1, effective_frame_thickness_px.top(), corner_insets.left(), 0, edge_w, effective_frame_thickness_px.top());
    // Left edge
    draw_image(
        src_rect.x(), 2 * asset.frame_size_px, effective_frame_thickness_px.left(), 1, 0, corner_insets.top(), effective_frame_thickness_px.left(), edge_h);
    // Bottom edge
    draw_image(2 * asset.frame_size_px, BitmapSizePx(asset) - asset.frame_size_px, 1, effective_frame_thickness_px.bottom(), corner_insets.left(),
        client_bounds_px.bottom(), edge_w, effective_frame_thickness_px.bottom());
    // Right edge
    draw_image(BitmapSizePx(asset) - asset.frame_size_px, 2 * asset.frame_size_px, effective_frame_thickness_px.right(), 1, client_bounds_px.right(),
    ...
    auto header = PaintHeaderbar({ client_bounds_px.width(), top_area_height_px }, HeaderContext(solid_frame_, tiled_, focused), scale);
    image = gfx::ImageSkia::CreateFrom1xBitmap(header);
    // In GTK4, the headerbar gets clipped by the window.
    if (GtkCheckVersion(4)) {
        gfx::RectF bounds_px = gfx::RectF(client_bounds_px.x(), client_bounds_px.y(), header.width(), header.height());
    ...
    draw_image(0, 0, header.width(), header.height(), client_bounds_px.x(), client_bounds_px.y(), header.width(), header.height());
}

WindowFrameProviderGtk::Asset& WindowFrameProviderGtk::GetOrCreateAsset(float scale)
{
    ...
    gfx::Rect frame_bounds_dip(kMaxFrameSizeDip, kMaxFrameSizeDip, 2 * kMaxFrameSizeDip, 2 * kMaxFrameSizeDip);
    auto focused_context = DecorationContext(solid_frame_, tiled_, true);
    frame_bounds_dip.Inset(-GtkStyleContextGetPadding(focused_context));
    frame_bounds_dip.Inset(-GtkStyleContextGetBorder(focused_context));
    gfx::Size bitmap_size(BitmapSizePx(asset), BitmapSizePx(asset));
    asset.focused_bitmap = PaintBitmap(bitmap_size, gfx::RectF(frame_bounds_dip), focused_context, scale);
    asset.unfocused_bitmap = PaintBitmap(bitmap_size, gfx::RectF(frame_bounds_dip), DecorationContext(solid_frame_, tiled_, false), scale);
...
void WindowFrameProviderGtk::OnThemeChanged(GtkSettings* settings, GtkParamSpec* param)
{
    assets_.clear();
