...
x11::KeyButMask BuildXkbStateFromGdkEvent(unsigned int state, unsigned char group)
{
...
x11::Event ConvertGdkEventToKeyEvent(GdkEvent* gdk_event)
{
    if (!gtk::GtkCheckVersion(4)) {
        ...
        x11::KeyEvent key_event {
            .opcode = key->type == GdkKeyPress() ? x11::KeyEvent::Press : x11::KeyEvent::Release,
    ...
    gint n_entries = 0;
    gdk_display_map_keycode(gdk_display_get_default(), gdk_key_event_get_keycode(gdk_event), &keys, &keyvals, &n_entries);
    ...
    x11::KeyEvent key_event {
        .opcode = gtk::GdkEventGetEventType(gdk_event) == GdkKeyPress() ? x11::KeyEvent::Press : x11::KeyEvent::Release,
        .detail = static_cast<x11::KeyCode>(keymap_key.keycode),
        .time = static_cast<x11::Time>(gtk::GdkEventGetTime(gdk_event)),
        .root = ui::GetX11RootWindow(),
        .event = static_cast<x11::Window>(gdk_x11_surface_get_xid(gdk_event_get_surface(gdk_event))),
        .state = BuildXkbStateFromGdkEvent(gdk_event_get_modifier_state(gdk_event), keymap_key.group),
        .same_screen = true,
    };
...
void ProcessGdkEvent(GdkEvent* gdk_event)
{
    ...
    auto event_type = gtk::GtkCheckVersion(4) ? gtk::GdkEventGetEventType(gdk_event) : *reinterpret_cast<GdkEventType*>(gdk_event);
...
GtkEventLoopX11::GtkEventLoopX11(GtkWidget* widget)
{
    if (gtk::GtkCheckVersion(4)) {
        ...
        signal_ = ScopedGSignal(surface_, "event", base::BindRepeating(&GtkEventLoopX11::OnEvent, base::Unretained(this)));
    } else {
...
{
    if (!gtk::GtkCheckVersion(4)) {
        gdk_event_handler_set(reinterpret_cast<GdkEventFunc>(gtk_main_do_event), nullptr, nullptr);
    }
}
