...
GtkCssContext AppendCssNodeToStyleContextImpl(
    GtkCssContext context, const std::string& name, const std::string& object_name, const std::vector<std::string>& classes, GtkStateFlags state, float scale)
{
    if (GtkCheckVersion(4)) {
        ...
        //    on destruction.
        auto* widget_object = object_name.empty() ? g_object_new(GTK_TYPE_BOX, nullptr) : g_object_new(GTK_TYPE_BOX, "css-name", object_name.c_str(), nullptr);
        ...
        GtkWidgetPath* path = context ? gtk_widget_path_copy(gtk_style_context_get_path(context)) : gtk_widget_path_new();
...
{
    return reinterpret_cast<aura::Window*>(g_object_get_data(G_OBJECT(dialog), kAuraTransientParent));
}
...
base::OnceClosure DisableHostInputHandling(GtkWidget* dialog, aura::Window* parent)
{
    if (!parent) {
        return {};
    }
    auto* host = static_cast<views::DesktopWindowTreeHostLinux*>(parent->GetHost());
    if (!host) {
        return {};
    }
...
void ParseButtonLayout(const std::string& button_string, std::vector<views::FrameButton>* leading_buttons, std::vector<views::FrameButton>* trailing_buttons)
{
    ...
    while (tokenizer.GetNext()) {
        if (tokenizer.token_is_delim()) {
            ...
            if (token == "minimize") {
                (left_side ? leading_buttons : trailing_buttons)->push_back(views::FrameButton::kMinimize);
            } else if (token == "maximize") {
                (left_side ? leading_buttons : trailing_buttons)->push_back(views::FrameButton::kMaximize);
            } else if (token == "close") {
                (left_side ? leading_buttons : trailing_buttons)->push_back(views::FrameButton::kClose);
            }
        }
    }
}
...
CairoSurface::CairoSurface(SkBitmap& bitmap)
    : surface_(cairo_image_surface_create_for_data(static_cast<unsigned char*>(bitmap.getAddr(0, 0)), CAIRO_FORMAT_ARGB32, bitmap.width(), bitmap.height(),
...
CairoSurface::CairoSurface(const gfx::Size& size)
    : surface_(cairo_image_surface_create(CAIRO_FORMAT_ARGB32, size.width(), size.height()))
    , cairo_(cairo_create(surface_))
{
...
SkColor CairoSurface::GetAveragePixelValue(bool frame)
{
    ...
    SkColor* data = reinterpret_cast<SkColor*>(cairo_image_surface_get_data(surface_));
    ...
    return SkColorSetARGB(frame ? max_alpha : a / (width * height), r * 255 / a, g * 255 / a, b * 255 / a);
}

GtkCssContext::GtkCssContext(GtkWidget* widget, GtkWidget* root)
    : widget_(widget)
    , root_(WrapGObject(root))
{
    DCHECK(GtkCheckVersion(4));
}
...
{
    if (GtkCheckVersion(4)) {
        return GtkCssContext(WrapGObject(gtk_widget_get_parent(widget_)), root_ == widget_ ? ScopedGObject<GtkWidget>() : root_);
    }
...
GtkCssContext AppendCssNodeToStyleContext(GtkCssContext context, const std::string& css_node)
{
    enum {
        CSS_NAME,
        CSS_OBJECT_NAME,
    ...
    return AppendCssNodeToStyleContextImpl(context, name, object_name, classes, state, scale);
}
...
GtkCssContext GetStyleContextFromCss(const std::string& css_selector)
{
    ...
    for (const auto& widget_type : base::SplitString(css_selector, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY)) {
...
void RenderBackground(const gfx::Size& size, cairo_t* cr, GtkCssContext context)
{
    if (!context) {
        return;
    }
...
bool ContextHasClass(GtkCssContext context, const std::string& style_class)
{
    ...
    if (!GtkCheckVersion(4)) {
        has_class |= gtk_widget_path_iter_has_class(gtk_style_context_get_path(context), -1, style_class.c_str());
    }
    return has_class;
}
...
std::string GetGtkSettingsStringProperty(GtkSettings* settings, const gchar* prop_name)
{
...
int GetKeyEventProperty(const ui::KeyEvent& key_event, const char* property_key)
{
...
{
    DCHECK(!GtkCheckVersion(4));
    GdkEventType event_type = key_event.type() == ui::EventType::kKeyPressed ? GdkKeyPress() : GdkKeyRelease();
    ...
    gdk_keymap_translate_keyboard_state(keymap, hw_code, state, group, &keyval, nullptr, nullptr, &consumed);
...
{
    return GtkCheckVersion(4) ? gtk_icon_theme_get_for_display(gdk_display_get_default()) : gtk_icon_theme_get_default();
}
...
{
    auto widget = TakeGObject(gtk_separator_new(horizontal ? GTK_ORIENTATION_HORIZONTAL : GTK_ORIENTATION_VERTICAL));
