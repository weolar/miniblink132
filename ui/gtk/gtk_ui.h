...
public:
    ...
    void SetWindowButtonOrdering(const std::vector<views::FrameButton>& leading_buttons, const std::vector<views::FrameButton>& trailing_buttons);
    void SetWindowFrameAction(WindowFrameActionSource source, WindowFrameAction action);

    // ui::LinuxUi:
    bool Initialize() override;
    void InitializeFontSettings() override;
    base::TimeDelta GetCursorBlinkInterval() const override;
    gfx::Image GetIconForContentType(const std::string& content_type, int size, float scale) const override;
    base::flat_map<std::string, std::string> GetKeyboardLayoutMap() override;
#if BUILDFLAG(ENABLE_PRINTING)
    printing::PrintDialogLinuxInterface* CreatePrintDialog(printing::PrintingContextLinux* context) override;
    gfx::Size GetPdfPaperSize(printing::PrintingContextLinux* context) override;
#endif
    ui::SelectFileDialog* CreateSelectFileDialog(void* listener, std::unique_ptr<ui::SelectFilePolicy> policy) const override;
    std::string GetCursorThemeName() override;
    int GetCursorThemeSize() override;
    std::unique_ptr<ui::LinuxInputMethodContext> CreateInputMethodContext(ui::LinuxInputMethodContextDelegate* delegate) const override;
    ...
    void AddWindowButtonOrderObserver(ui::WindowButtonOrderObserver* observer) override;
    void RemoveWindowButtonOrderObserver(ui::WindowButtonOrderObserver* observer) override;
    WindowFrameAction GetWindowFrameAction(WindowFrameActionSource source) override;
    ...
    ui::WindowFrameProvider* GetWindowFrameProvider(bool solid_frame, bool tiled) override;
...
private:
    ...
    void OnMonitorsChanged(GListModel* list, guint position, guint removed, guint added);
    ...
    void AddGtkNativeColorMixer(ui::ColorProvider* provider, const ui::ColorProviderKey& key);
    ...
    base::flat_map<WindowFrameActionSource, WindowFrameAction> window_frame_actions_;
    ...
    base::ObserverList<ui::WindowButtonOrderObserver>::Unchecked window_button_order_observer_list_;

    std::vector<ScopedGSignal> signals_;
    // Two signals are registered for each monitor, so keep them in a pair.
    std::unordered_map<GdkMonitor*, std::pair<ScopedGSignal, ScopedGSignal>> monitor_signals_;
};
