...
template <typename T> SkColor GdkRgbaToSkColor(const T& color)
{
...
template <typename T> auto DlCast(void* symbol)
{
...
bool LoadGtkImpl()
{
    ...
    if (!base::StringToUint(cmd->GetSwitchValueASCII(kGtkVersionFlag), &gtk_version)) {
        gtk_version = 0;
    }
...
{
    return gfx::Insets::TLBR(border.top, border.left, border.bottom, border.right);
}
...
{
    static base::NoDestructor<base::Version> gtk_version(std::vector<uint32_t> { gtk_get_major_version(), gtk_get_minor_version(), gtk_get_micro_version() });
    return *gtk_version;
}
...
{
    static void* get_padding = DlSym(GetLibGtk(), "gtk_style_context_get_padding");
    GtkBorder padding;
    if (GtkCheckVersion(4)) {
        DlCast<void(GtkStyleContext*, GtkBorder*)>(get_padding)(context, &padding);
    } else {
        DlCast<void(GtkStyleContext*, GtkStateFlags, GtkBorder*)>(get_padding)(context, gtk_style_context_get_state(context), &padding);
    }
...
gfx::Insets GtkStyleContextGetBorder(GtkStyleContext* context)
{
    ...
    if (GtkCheckVersion(4)) {
        ...
        DlCast<void(GtkStyleContext*, GtkStateFlags, GtkBorder*)>(get_border)(context, gtk_style_context_get_state(context), &border);
    }
...
gfx::Insets GtkStyleContextGetMargin(GtkStyleContext* context)
{
    ...
    if (GtkCheckVersion(4)) {
        ...
        DlCast<void(GtkStyleContext*, GtkStateFlags, GtkBorder*)>(get_margin)(context, gtk_style_context_get_state(context), &margin);
    }
...
SkColor GtkStyleContextGetColor(GtkStyleContext* context)
{
    ...
    Gdk3Rgba color;
    DlCast<void(GtkStyleContext*, GtkStateFlags, Gdk3Rgba*)>(get_color)(context, gtk_style_context_get_state(context), &color);
    return GdkRgbaToSkColor(color);
}
...
SkColor GtkStyleContextGetBackgroundColor(GtkStyleContext* context)
{
    ...
    static void* get_bg_color = DlSym(GetLibGtk(), "gtk_style_context_get_background_color");
    Gdk3Rgba color;
    DlCast<void(GtkStyleContext*, GtkStateFlags, Gdk3Rgba*)>(get_bg_color)(context, gtk_style_context_get_state(context), &color);
    return GdkRgbaToSkColor(color);
}

DISABLE_CFI_DLSYM
std::optional<SkColor> GtkStyleContextLookupColor(GtkStyleContext* context, const gchar* color_name)
{
    DCHECK(!GtkCheckVersion(4));
    static void* lookup_color = DlSym(GetLibGtk(), "gtk_style_context_lookup_color");
    Gdk3Rgba color;
    if (DlCast<gboolean(GtkStyleContext*, const gchar*, Gdk3Rgba*)>(lookup_color)(context, color_name, &color)) {
        return GdkRgbaToSkColor(color);
    }
    return std::nullopt;
}
...
bool GtkImContextFilterKeypress(GtkIMContext* context, GdkEventKey* event)
{
    ...
    if (GtkCheckVersion(4)) {
        return DlCast<bool(GtkIMContext*, GdkEvent*)>(filter)(context, reinterpret_cast<GdkEvent*>(event));
    }
    return DlCast<bool(GtkIMContext*, GdkEventKey*)>(filter)(context, event);
}

DISABLE_CFI_DLSYM
bool GtkFileChooserSetCurrentFolder(GtkFileChooser* dialog, const base::FilePath& path)
{
    ...
    if (GtkCheckVersion(4)) {
        ...
        return DlCast<bool(GtkFileChooser*, GFile*, GError**)>(set)(dialog, file, nullptr);
    }
    return DlCast<bool(GtkFileChooser*, const gchar*)>(set)(dialog, path.value().c_str());
}

DISABLE_CFI_DLSYM
void GtkRenderIcon(GtkStyleContext* context, cairo_t* cr, GdkPixbuf* pixbuf, GdkTexture* texture, double x, double y)
{
    static void* render = DlSym(GetLibGtk(), "gtk_render_icon");
    if (texture) {
        DCHECK(GtkCheckVersion(4));
        DlCast<void(GtkStyleContext*, cairo_t*, GdkTexture*, double, double)>(render)(context, cr, texture, x, y);
    } else if (pixbuf) {
        DCHECK(!GtkCheckVersion(4));
        DlCast<void(GtkStyleContext*, cairo_t*, GdkPixbuf*, double, double)>(render)(context, cr, pixbuf, x, y);
    }
}
...
DISABLE_CFI_DLSYM
void GtkCssProviderLoadFromData(GtkCssProvider* css_provider, const char* data, gssize length)
{
    static void* load = DlSym(GetLibGtk(), "gtk_css_provider_load_from_data");
    if (GtkCheckVersion(4)) {
        DlCast<void(GtkCssProvider*, const char*, gssize)>(load)(css_provider, data, length);
    } else {
        DlCast<gboolean(GtkCssProvider*, const char*, gssize, GError**)>(load)(css_provider, data, length, nullptr);
    }
}
...
void GtkStyleContextGet(GtkStyleContext* context, ...)
{
    ...
    gtk_style_context_get_valist(context, gtk_style_context_get_state(context), args);
    va_end(args);
}
...
DISABLE_CFI_DLSYM
ScopedGObject<GtkIconInfo> Gtk3IconThemeLookupByGiconForScale(GtkIconTheme* theme, GIcon* icon, int size, int scale, GtkIconLookupFlags flags)
{
    DCHECK(!GtkCheckVersion(4));
    static void* lookup = DlSym(GetLibGtk(), "gtk_icon_theme_lookup_by_gicon_for_scale");
    return TakeGObject(DlCast<GtkIconInfo*(GtkIconTheme*, GIcon*, int, int, GtkIconLookupFlags)>(lookup)(theme, icon, size, scale, flags));
}

DISABLE_CFI_DLSYM
ScopedGObject<GtkIconPaintable> Gtk4IconThemeLookupIcon(
    GtkIconTheme* theme, const char* icon_name, const char* fallbacks[], int size, int scale, GtkTextDirection direction, GtkIconLookupFlags flags)
{
    static void* lookup = DlSym(GetLibGtk(), "gtk_icon_theme_lookup_icon");
    return TakeGObject(DlCast<GtkIconPaintable*(GtkIconTheme*, const char*, const char*[], int, int, GtkTextDirection, GtkIconLookupFlags)>(lookup)(
        theme, icon_name, fallbacks, size, scale, direction, flags));
}

DISABLE_CFI_DLSYM
ScopedGObject<GtkIconPaintable> Gtk4IconThemeLookupByGicon(
    GtkIconTheme* theme, GIcon* icon, int size, int scale, GtkTextDirection direction, GtkIconLookupFlags flags)
{
    static void* lookup = DlSym(GetLibGtk(), "gtk_icon_theme_lookup_by_gicon");
    DCHECK(GtkCheckVersion(4));
    return TakeGObject(
        DlCast<GtkIconPaintable*(GtkIconTheme*, GIcon*, int, int, GtkTextDirection, GtkIconLookupFlags)>(lookup)(theme, icon, size, scale, direction, flags));
}

DISABLE_CFI_DLSYM
GtkWidget* GtkFileChooserDialogNew(const gchar* title, GtkWindow* parent, GtkFileChooserAction action, const gchar* first_button_text,
    GtkResponseType first_response, const gchar* second_button_text, GtkResponseType second_response)
{
    static void* create = DlSym(GetLibGtk(), "gtk_file_chooser_dialog_new");
    return DlCast<GtkWidget*(const gchar*, GtkWindow*, GtkFileChooserAction, const gchar*, ...)>(create)(
        title, parent, action, first_button_text, first_response, second_button_text, second_response, nullptr);
}
