...
public:
    ...
    // non-nullptr.
    bool MatchEvent(const ui::Event& event, std::vector<ui::TextEditCommandAuraLinux>* commands);
...
private:
    ...
    struct HandlerClass {
        ...
        gboolean (*extend_selection)(
            GtkTextView* text_view, GtkTextExtendSelection granularity, const GtkTextIter* location, GtkTextIter* start, GtkTextIter* end);
    ...
    void EditCommandMatched(ui::TextEditCommand command, const std::string& value);
    ...
    static void DeleteFromCursor(GtkTextView* text_view, GtkDeleteType type, gint count);
    ...
    static void MoveCursor(GtkTextView* text_view, GtkMovementStep step, gint count, gboolean extend_selection);

    // Handler of "move-viewport" signal.
    static void MoveViewport(GtkTextView* text_view, GtkScrollStep step, gint count);
