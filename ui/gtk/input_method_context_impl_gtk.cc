...
InputMethodContextImplGtk::InputMethodContextImplGtk(ui::LinuxInputMethodContextDelegate* delegate)
    : delegate_(delegate)
{
    CHECK(delegate_);
    ...
    const gchar* context_id = gtk_im_multicontext_get_context_id(GTK_IM_MULTICONTEXT(gtk_context_.get()));
    // switch to allow wayland IM module if it is picked.
    if (context_id) {
        if (!base::CommandLine::ForCurrentProcess()->HasSwitch(kAllowGtkWaylandIm) && (std::string(context_id) == kContextIdWayland)) {
            ...
            gtk_im_multicontext_set_context_id(GTK_IM_MULTICONTEXT(gtk_context_.get()), kContextIdIbus);
        } else {
    ...
    auto connect = [&](const char* detailed_signal, auto receiver) {
        for (auto context : { gtk_context_, gtk_simple_context_ }) {
            ...
            // ScopedGSignal.
            signals_.emplace_back(context, detailed_signal, base::BindRepeating(receiver, base::Unretained(this)));
        }
    };
...
bool InputMethodContextImplGtk::DispatchKeyEvent(const ui::KeyEvent& key_event)
{
    ...
    caret_bounds = ScaleToRoundedRect(caret_bounds, GetDeviceScaleFactor() / gtk_widget_get_scale_factor(GetDummyWindow()));
    GdkRectangle gdk_rect = { caret_bounds.x(), caret_bounds.y(), caret_bounds.width(), caret_bounds.height() };
    gtk_im_context_set_cursor_location(gtk_context, &gdk_rect);

    if (!GtkCheckVersion(4)) {
        const bool handled = GtkImContextFilterKeypress(gtk_context, GdkEventToKey(event));
        gdk_event_free(event);
        return handled;
    }
    ...
    auto* surface = gtk_native_get_surface(gtk_widget_get_native(GetDummyWindow()));
    auto* device = gdk_seat_get_keyboard(gdk_display_get_default_seat(gdk_display_get_default()));
    ...
    return gtk_im_context_filter_key(gtk_context, press, surface, device, time, keycode, state, group);
}
...
void InputMethodContextImplGtk::UpdateFocus(
    bool has_client, ui::TextInputType old_type, const TextInputClientAttributes& new_client_attributes, ui::TextInputClient::FocusReason reason)
{
    ...
    if (new_client_attributes.flags & ui::TEXT_INPUT_FLAG_VERTICAL) {
        g_object_set(gtk_context_, "input-hints", GTK_INPUT_HINT_VERTICAL_WRITING, nullptr);
        g_object_set(gtk_simple_context_, "input-hints", GTK_INPUT_HINT_VERTICAL_WRITING, nullptr);
    }
}
...
void InputMethodContextImplGtk::OnPreeditChanged(GtkIMContext* context)
{
    ...
    ui::ExtractCompositionTextFromGtkPreedit(str, attrs, cursor_pos, &composition_text);
    g_free(str);
...
void InputMethodContextImplGtk::SetContextClientWindow(GdkWindow* window, GtkIMContext* gtk_context)
{
    gpointer& gdk_last_set_client_window = gtk_context == gtk_simple_context_ ? gdk_last_set_client_window_for_simple_ : gdk_last_set_client_window_;
...
ui::VirtualKeyboardController* InputMethodContextImplGtk::GetVirtualKeyboardController()
{
    return nullptr;
}
