...
{
    static base::NoDestructor<std::string> gettext_package("gtk" + base::NumberToString(GtkVersion().components()[0]) + "0");
...
void GtkFileChooserSetFilename(GtkFileChooser* dialog, const base::FilePath& path)
{
    if (GtkCheckVersion(4)) {
...
{
    GtkFileFilter* selected_filter = gtk_file_chooser_get_filter(GTK_FILE_CHOOSER(dialog));
    int idx = -1;
    if (GtkCheckVersion(4)) {
        auto filters = TakeGObject(gtk_file_chooser_get_filters(GTK_FILE_CHOOSER(dialog)));
...
std::string GtkFileChooserGetFilename(GtkWidget* dialog)
{
    ...
    if (GtkCheckVersion(4)) {
        if (auto file = TakeGObject(gtk_file_chooser_get_file(GTK_FILE_CHOOSER(dialog)))) {
            filename = g_file_peek_path(file);
        }
    } else {
        gchar_filename = std::unique_ptr<gchar, GFreeDeleter>(gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog)));
...
SelectFileDialogLinuxGtk::DialogState::DialogState(std::vector<ScopedGSignal> signals, aura::Window* parent, base::OnceClosure reenable_parent_events)
...
SelectFileDialogLinuxGtk::DialogState::DialogState(DialogState&& other) = default;

SelectFileDialogLinuxGtk::DialogState& SelectFileDialogLinuxGtk::DialogState::operator=(DialogState&& other) = default;

SelectFileDialogLinuxGtk::DialogState::~DialogState() = default;

SelectFileDialogLinuxGtk::SelectFileDialogLinuxGtk(Listener* listener, std::unique_ptr<ui::SelectFilePolicy> policy)
...
bool SelectFileDialogLinuxGtk::IsRunning(gfx::NativeWindow parent_window) const
{
    return std::any_of(dialogs_.begin(), dialogs_.end(), [=](const std::pair<GtkWidget*, DialogState>& pair) { return pair.second.parent == parent_window; });
}
...
void SelectFileDialogLinuxGtk::SelectFileImpl(Type type, const std::u16string& title, const base::FilePath& default_path, const FileTypeInfo* file_types,
    int file_type_index, const base::FilePath::StringType& default_extension, gfx::NativeWindow owning_window, const GURL* caller)
{
    ...
    auto connect = [&](const char* detailed_signal, auto receiver) {
        ...
        // ScopedGSignal.
        signals.emplace_back(dialog, detailed_signal, base::BindRepeating(receiver, base::Unretained(this)), G_CONNECT_AFTER);
    };
    switch (type) {
    case SELECT_FOLDER:
    case SELECT_UPLOAD_FOLDER:
    case SELECT_EXISTING_FOLDER:
        dialog = CreateSelectFolderDialog(type, title_string, default_path, owning_window);
        connect("response", &SelectFileDialogLinuxGtk::OnSelectSingleFolderDialogResponse);
        break;
    case SELECT_OPEN_FILE:
        dialog = CreateFileOpenDialog(title_string, default_path, owning_window);
        connect("response", &SelectFileDialogLinuxGtk::OnSelectSingleFileDialogResponse);
        break;
    case SELECT_OPEN_MULTI_FILE:
        dialog = CreateMultiFileOpenDialog(title_string, default_path, owning_window);
        connect("response", &SelectFileDialogLinuxGtk::OnSelectMultiFileDialogResponse);
        break;
    ...
    if (GtkCheckVersion(4)) {
        ...
        signals.emplace_back(dialog, "delete-event", base::BindRepeating(gtk_widget_hide_on_delete));
    }
    ...
    base::OnceClosure reenable_input_events = DisableHostInputHandling(dialog, owning_window);

    dialogs_[dialog] = DialogState(std::move(signals), owning_window, std::move(reenable_input_events));
...
void SelectFileDialogLinuxGtk::AddFilters(GtkFileChooser* chooser)
{
    for (size_t i = 0; i < file_types().extensions.size(); ++i) {
        ...
        for (const auto& current_extension : file_types().extensions[i]) {
            if (!current_extension.empty()) {
                ...
                for (std::string pattern : { "*." + base::ToLowerASCII(current_extension), "*." + base::ToUpperASCII(current_extension) }) {
        ...
        if (i < file_types().extension_description_overrides.size()) {
            gtk_file_filter_set_name(filter, base::UTF16ToUTF8(file_types().extension_description_overrides[i]).c_str());
        } else {
            // There is no system default filter description so we use
            // the extensions themselves if the description is blank.
            std::vector<std::string> fallback_labels_vector(fallback_labels.begin(), fallback_labels.end());
            std::string fallback_label = base::JoinString(fallback_labels_vector, ",");
    ...
    if (file_types().include_all_files && !file_types().extensions.empty()) {
        ...
        gtk_file_filter_set_name(filter, l10n_util::GetStringUTF8(IDS_SAVEAS_ALL_FILES).c_str());
        gtk_file_chooser_add_filter(chooser, filter);
    }
}

void SelectFileDialogLinuxGtk::FileSelected(GtkWidget* dialog, const base::FilePath& path)
{
    if (type() == SELECT_SAVEAS_FILE) {
        set_last_saved_path(path.DirName());
    } else if (type() == SELECT_OPEN_FILE || type() == SELECT_FOLDER || type() == SELECT_UPLOAD_FOLDER || type() == SELECT_EXISTING_FOLDER) {
    ...
    if (listener_) {
        listener_->FileSelected(ui::SelectedFileInfo(path), GtkDialogSelectedFilterIndex(dialog) + 1);
    }
    GtkWindowDestroy(dialog);
}

void SelectFileDialogLinuxGtk::MultiFilesSelected(GtkWidget* dialog, const std::vector<base::FilePath>& files)
{
...
GtkWidget* SelectFileDialogLinuxGtk::CreateFileOpenHelper(const std::string& title, const base::FilePath& default_path, gfx::NativeWindow parent)
{
    GtkWidget* dialog = GtkFileChooserDialogNew(
        title.c_str(), nullptr, GTK_FILE_CHOOSER_ACTION_OPEN, GetCancelLabel(), GTK_RESPONSE_CANCEL, GetOpenLabel(), GTK_RESPONSE_ACCEPT);
    ...
    if (!default_path.empty()) {
        ...
        GtkFileChooserSetCurrentFolder(GTK_FILE_CHOOSER(dialog), *last_opened_path());
    }
    return dialog;
}

GtkWidget* SelectFileDialogLinuxGtk::CreateSelectFolderDialog(Type type, const std::string& title, const base::FilePath& default_path, gfx::NativeWindow parent)
{
    std::string title_string = title;
    if (title_string.empty()) {
        title_string = (type == SELECT_UPLOAD_FOLDER) ? l10n_util::GetStringUTF8(IDS_SELECT_UPLOAD_FOLDER_DIALOG_TITLE)
                                                      : l10n_util::GetStringUTF8(IDS_SELECT_FOLDER_DIALOG_TITLE);
    }
    std::string accept_button_label = (type == SELECT_UPLOAD_FOLDER) ? l10n_util::GetStringUTF8(IDS_SELECT_UPLOAD_FOLDER_DIALOG_UPLOAD_BUTTON) : GetOpenLabel();

    GtkWidget* dialog = GtkFileChooserDialogNew(title_string.c_str(), nullptr, GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER, GetCancelLabel(), GTK_RESPONSE_CANCEL,
        accept_button_label.c_str(), GTK_RESPONSE_ACCEPT);
    SetGtkTransientForAura(dialog, parent);
    ...
    gtk_file_filter_set_name(only_folders, l10n_util::GetStringUTF8(IDS_SELECT_FOLDER_DIALOG_TITLE).c_str());
...
GtkWidget* SelectFileDialogLinuxGtk::CreateFileOpenDialog(const std::string& title, const base::FilePath& default_path, gfx::NativeWindow parent)
{
    std::string title_string = !title.empty() ? title : l10n_util::GetStringUTF8(IDS_OPEN_FILE_DIALOG_TITLE);
...
GtkWidget* SelectFileDialogLinuxGtk::CreateMultiFileOpenDialog(const std::string& title, const base::FilePath& default_path, gfx::NativeWindow parent)
{
    std::string title_string = !title.empty() ? title : l10n_util::GetStringUTF8(IDS_OPEN_FILES_DIALOG_TITLE);
...
GtkWidget* SelectFileDialogLinuxGtk::CreateSaveAsDialog(const std::string& title, const base::FilePath& default_path, gfx::NativeWindow parent)
{
    std::string title_string = !title.empty() ? title : l10n_util::GetStringUTF8(IDS_SAVE_AS_DIALOG_TITLE);

    GtkWidget* dialog = GtkFileChooserDialogNew(
        title_string.c_str(), nullptr, GTK_FILE_CHOOSER_ACTION_SAVE, GetCancelLabel(), GTK_RESPONSE_CANCEL, GetSaveLabel(), GTK_RESPONSE_ACCEPT);
    SetGtkTransientForAura(dialog, parent);
    ...
    if (!default_path.empty()) {
        if (CallDirectoryExistsOnUIThread(default_path)) {
            ...
            GtkFileChooserSetCurrentFolder(GTK_FILE_CHOOSER(dialog), default_path.DirName());
            gtk_file_chooser_set_current_name(GTK_FILE_CHOOSER(dialog), default_path.BaseName().value().c_str());
        }
    } else if (!last_saved_path()->empty()) {
        GtkFileChooserSetCurrentFolder(GTK_FILE_CHOOSER(dialog), *last_saved_path());
    }
    gtk_file_chooser_set_select_multiple(GTK_FILE_CHOOSER(dialog), FALSE);
...
{
    bool is_cancel = response_id == GTK_RESPONSE_CANCEL || response_id == GTK_RESPONSE_DELETE_EVENT;
    if (is_cancel)
        return true;
...
void SelectFileDialogLinuxGtk::SelectSingleFileHelper(GtkWidget* dialog, gint response_id, bool allow_folder)
{
...
void SelectFileDialogLinuxGtk::OnSelectSingleFileDialogResponse(GtkWidget* dialog, int response_id)
{
    SelectSingleFileHelper(dialog, response_id, false);
}

void SelectFileDialogLinuxGtk::OnSelectSingleFolderDialogResponse(GtkWidget* dialog, int response_id)
{
    SelectSingleFileHelper(dialog, response_id, true);
}

void SelectFileDialogLinuxGtk::OnSelectMultiFileDialogResponse(GtkWidget* dialog, int response_id)
{
    if (IsCancelResponse(response_id)) {
        FileNotSelected(dialog);
        return;
    ...
    std::erase_if(filenames, [this](const base::FilePath& path) { return CallDirectoryExistsOnUIThread(path); });
    if (filenames.empty()) {
        FileNotSelected(dialog);
        return;
    }
...
void SelectFileDialogLinuxGtk::OnUpdatePreview(GtkWidget* chooser)
{
    ...
    GdkPixbuf* pixbuf = gdk_pixbuf_new_from_file_at_size(filename, kPreviewWidth, kPreviewHeight, nullptr);
    g_free(filename);
    if (pixbuf) {
        gtk_image_set_from_pixbuf(GTK_IMAGE(preview_.get()), pixbuf);
        g_object_unref(pixbuf);
    }
    gtk_file_chooser_set_preview_widget_active(GTK_FILE_CHOOSER(chooser), pixbuf ? TRUE : FALSE);
}
