...
gfx::FontRenderParams GetGtkFontRenderParams()
{
    ...
    gchar* rgba = nullptr;
    g_object_get(gtk_settings, "gtk-xft-antialias", &antialias, "gtk-xft-hinting", &hinting, "gtk-xft-hintstyle", &hint_style, "gtk-xft-rgba", &rgba, nullptr);
...
bool GtkUi::Initialize()
{
    ...
    ui::ColorProviderManager::Get().AppendColorProviderInitializer(base::BindRepeating(&GtkUi::AddGtkNativeColorMixer, base::Unretained(this)));
    ...
    window_frame_actions_ = { { ActionSource::kDoubleClick, Action::kToggleMaximize }, { ActionSource::kMiddleClick, Action::kNone },
    ...
    auto connect = [&](auto* sender, const char* detailed_signal, auto receiver) {
        ...
        signals_.emplace_back(sender, detailed_signal, base::BindRepeating(receiver, base::Unretained(this)), G_CONNECT_AFTER);
    };
    ...
    connect(settings, "notify::gtk-application-prefer-dark-theme", &GtkUi::OnThemeChanged);
    connect(settings, "notify::gtk-cursor-theme-name", &GtkUi::OnCursorThemeNameChanged);
    connect(settings, "notify::gtk-cursor-theme-size", &GtkUi::OnCursorThemeSizeChanged);
    connect(settings, "notify::gtk-enable-animations", &GtkUi::OnEnableAnimationsChanged);
...
void GtkUi::InitializeFontSettings()
{
    ...
    query.families = base::SplitString(pango_font_description_get_family(desc), ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    if (pango_font_description_get_size_is_absolute(desc)) {
        ...
        const double size_points = pango_font_description_get_size(desc) / kPangoScale;
    ...
    query.weight = static_cast<gfx::Font::Weight>(pango_font_description_get_weight(desc));
    ...
    default_font_render_params_ = gfx::GetFontRenderParams(query, &default_font_family);
...
{
    for (const ColorMap& color_map : { colors_, use_custom_frame ? custom_frame_colors_ : native_frame_colors_ }) {
...
base::TimeDelta GtkUi::GetCursorBlinkInterval() const
{
    ...
    g_object_get(gtk_settings_get_default(), "gtk-cursor-blink-time", &cursor_blink_time, "gtk-cursor-blink", &cursor_blink, nullptr);
    return cursor_blink ? base::Seconds(cursor_blink_time / kGtkCursorBlinkCycleFactor) : base::TimeDelta();
}

gfx::Image GtkUi::GetIconForContentType(const std::string& content_type, int dip_size, float scale) const
{
    ...
    for (size_t i = 0; i < std::size(content_types); ++i) {
        ...
        SkBitmap bitmap;
        if (GtkCheckVersion(4)) {
            auto icon_paintable = Gtk4IconThemeLookupByGicon(theme, icon.get(), size, scale_int, GTK_TEXT_DIR_NONE, static_cast<GtkIconLookupFlags>(0));
            if (!icon_paintable) {
                continue;
            }

            auto* paintable = GlibCast<GdkPaintable>(icon_paintable.get(), gdk_paintable_get_type());
            ...
            bitmap.allocN32Pixels(gdk_texture_get_width(texture), gdk_texture_get_height(texture));
            gdk_texture_download(texture, static_cast<guchar*>(bitmap.getAddr(0, 0)), bitmap.rowBytes());

            gsk_render_node_unref(node);
        } else {
            auto icon_info
                = Gtk3IconThemeLookupByGiconForScale(theme, icon.get(), size, scale_int, static_cast<GtkIconLookupFlags>(GTK_ICON_LOOKUP_FORCE_SIZE));
            if (!icon_info) {
                continue;
            }
            auto* surface = gtk_icon_info_load_surface(icon_info.get(), nullptr, nullptr);
            if (!surface) {
                continue;
            }
            ...
            SkImageInfo image_info = SkImageInfo::Make(
                cairo_image_surface_get_width(surface), cairo_image_surface_get_height(surface), kBGRA_8888_SkColorType, kUnpremul_SkAlphaType);
            if (!bitmap.installPixels(
                    image_info, cairo_image_surface_get_data(surface), image_info.minRowBytes(),
                    [](void*, void* surface) { cairo_surface_destroy(reinterpret_cast<cairo_surface_t*>(surface)); }, surface)) {
                continue;
            }
        }
        gfx::ImageSkia image_skia = gfx::ImageSkia::CreateFromBitmap(bitmap, scale_int);
...
void GtkUi::SetWindowButtonOrdering(const std::vector<views::FrameButton>& leading_buttons, const std::vector<views::FrameButton>& trailing_buttons)
{
    views::WindowButtonOrderProvider::GetInstance()->SetWindowButtonOrder(leading_buttons, trailing_buttons);

    window_button_order_observer_list_.Notify(&ui::WindowButtonOrderObserver::OnWindowButtonOrderingChange);
}

void GtkUi::SetWindowFrameAction(WindowFrameActionSource source, WindowFrameAction action)
{
    window_frame_actions_[source] = action;
}

std::unique_ptr<ui::LinuxInputMethodContext> GtkUi::CreateInputMethodContext(ui::LinuxInputMethodContextDelegate* delegate) const
{
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableGtkIme)) {
        return nullptr;
    }
...
ui::SelectFileDialog* GtkUi::CreateSelectFileDialog(void* listener, std::unique_ptr<ui::SelectFilePolicy> policy) const
{
    return new SelectFileDialogLinuxGtk(static_cast<ui::SelectFileDialog::Listener*>(listener), std::move(policy));
}

ui::LinuxUi::WindowFrameAction GtkUi::GetWindowFrameAction(WindowFrameActionSource source)
{
...
{
    gboolean dark = false;
    g_object_get(gtk_settings_get_default(), "gtk-application-prefer-dark-theme", &dark, nullptr);
    return dark;
}
...
bool GtkUi::AnimationsEnabled() const
{
    ...
    g_object_get(gtk_settings_get_default(), "gtk-enable-animations", &animations_enabled, nullptr);
    return animations_enabled;
}

void GtkUi::AddWindowButtonOrderObserver(ui::WindowButtonOrderObserver* observer)
{
    window_button_order_observer_list_.AddObserver(observer);
}

void GtkUi::RemoveWindowButtonOrderObserver(ui::WindowButtonOrderObserver* observer)
{
...
ui::WindowFrameProvider* GtkUi::GetWindowFrameProvider(bool solid_frame, bool tiled)
{
...
base::flat_map<std::string, std::string> GtkUi::GetKeyboardLayoutMap()
{
    ...
    for (const ui::DomCode domcode : ui::kWritingSystemKeyDomCodes) {
        ...
        // order.
        const bool success = GtkCheckVersion(4) ? gdk_display_map_keycode(display, keycode, &keys, &keyvals, &n_entries)
...
{
    gint size = 0;
    g_object_get(gtk_settings_get_default(), "gtk-cursor-theme-size", &size, nullptr);
    return size;
}

bool GtkUi::GetTextEditCommandsForEvent(const ui::Event& event, int text_flags, std::vector<ui::TextEditCommandAuraLinux>* commands)
{
    ...
    if (text_flags & ui::TEXT_INPUT_FLAG_VERTICAL) {
        ...
        if (code == ui::VKEY_LEFT || code == ui::VKEY_RIGHT || code == ui::VKEY_UP || code == ui::VKEY_DOWN) {
            return false;
        }
    }
...
printing::PrintDialogLinuxInterface* GtkUi::CreatePrintDialog(printing::PrintingContextLinux* context)
{
...
void GtkUi::OnCursorThemeNameChanged(GtkSettings* settings, GtkParamSpec* param)
{
    std::string cursor_theme_name = GetCursorThemeName();
    if (cursor_theme_name.empty()) {
        return;
    }
    cursor_theme_observers().Notify(&ui::CursorThemeManagerObserver::OnCursorThemeNameChanged, cursor_theme_name);
}

void GtkUi::OnCursorThemeSizeChanged(GtkSettings* settings, GtkParamSpec* param)
{
    int cursor_theme_size = GetCursorThemeSize();
    if (!cursor_theme_size) {
        return;
    }
    cursor_theme_observers().Notify(&ui::CursorThemeManagerObserver::OnCursorThemeSizeChanged, cursor_theme_size);
}

void GtkUi::OnEnableAnimationsChanged(GtkSettings* settings, GtkParamSpec* param)
{
...
void GtkUi::OnMonitorsChanged(GListModel* list, guint position, guint removed, guint added)
{
    ...
    std::erase_if(monitor_signals_, [&](const auto& pair) { return !base::Contains(monitors, pair.first); });
    UpdateDeviceScaleFactor();
}
...
void GtkUi::UpdateColors()
{
    ...
    key.color_mode = (color_scheme == ui::NativeTheme::ColorScheme::kDark) ? ui::ColorProviderKey::ColorMode::kDark : ui::ColorProviderKey::ColorMode::kLight;
    key.contrast_mode = (color_scheme == ui::NativeTheme::ColorScheme::kPlatformHighContrast) ? ui::ColorProviderKey::ContrastMode::kHigh
                                                                                              : ui::ColorProviderKey::ContrastMode::kNormal;
    key.forced_colors = (color_scheme == ui::NativeTheme::ColorScheme::kPlatformHighContrast) ? ui::ColorProviderKey::ForcedColors::kActive
    ...
    const auto* color_provider = ui::ColorProviderManager::Get().GetColorProviderFor(key);
    ...
    inactive_selection_bg_color_ = GetSelectionBgColor("textview.view:backdrop "
                                                       "text:backdrop selection:backdrop");
    inactive_selection_fg_color_ = GetFgColor("textview.view:backdrop "
                                              "text:backdrop selection:backdrop");
    ...
    colors_[ThemeProperties::COLOR_NTP_BACKGROUND] = color_provider->GetColor(ui::kColorTextfieldBackground);
    colors_[ThemeProperties::COLOR_NTP_TEXT] = color_provider->GetColor(ui::kColorTextfieldForeground);
    ...
    colors_[ThemeProperties::COLOR_NTP_LINK] = color_provider->GetColor(ui::kColorTextfieldSelectionBackground);

    // Generate the colors that we pass to Blink.
    focus_ring_color_ = color_provider->GetColor(ui::kColorFocusableBorderFocused);
    ...
    active_selection_bg_color_ = color_provider->GetColor(ui::kColorTextfieldSelectionBackground);
    active_selection_fg_color_ = color_provider->GetColor(ui::kColorTextfieldSelectionForeground);
    ...
    for (bool custom_frame : { false, true }) {
        ColorMap& color_map = custom_frame ? custom_frame_colors_ : native_frame_colors_;
        const std::string header_selector = custom_frame ? "headerbar.header-bar.titlebar" : "menubar";
        const std::string header_selector_inactive = header_selector + ":backdrop";
        const SkColor frame_color = SkColorSetA(GetBgColor(header_selector), SK_AlphaOPAQUE);
        const SkColor frame_color_inactive = SkColorSetA(GetBgColor(header_selector_inactive), SK_AlphaOPAQUE);
        ...
        color_map[ThemeProperties::COLOR_FRAME_INACTIVE_INCOGNITO] = frame_color_inactive;

        // Compose the window color on the frame color to ensure the resulting tab
        // color is opaque.
        SkColor tab_color = color_utils::GetResultingPaintColor(GetBgColor(""), frame_color);

        color_map[ThemeProperties::COLOR_TOOLBAR] = tab_color;
        color_map[ThemeProperties::COLOR_TAB_BACKGROUND_ACTIVE_FRAME_ACTIVE] = tab_color;
        color_map[ThemeProperties::COLOR_TAB_BACKGROUND_ACTIVE_FRAME_INACTIVE] = tab_color;

        const SkColor background_tab_text_color = GetFgColor(header_selector + " label.title");
        const SkColor background_tab_text_color_inactive = GetFgColor(header_selector_inactive + " label.title");

        color_map[ThemeProperties::COLOR_TAB_FOREGROUND_INACTIVE_FRAME_ACTIVE] = background_tab_text_color;
        color_map[ThemeProperties::COLOR_TAB_FOREGROUND_INACTIVE_FRAME_ACTIVE_INCOGNITO] = background_tab_text_color;
        color_map[ThemeProperties::COLOR_TAB_FOREGROUND_INACTIVE_FRAME_INACTIVE] = background_tab_text_color_inactive;
        color_map[ThemeProperties::COLOR_TAB_FOREGROUND_INACTIVE_FRAME_INACTIVE_INCOGNITO] = background_tab_text_color_inactive;

        // These colors represent the border drawn around tabs and between
        // the tabstrip and toolbar.
        SkColor toolbar_top_separator = GetBorderColor(header_selector + " separator.vertical.titlebutton");
        SkColor toolbar_top_separator_inactive = GetBorderColor(header_selector + ":backdrop separator.vertical.titlebutton");
        ...
        auto toolbar_top_separator_has_good_contrast = [&]() {
            ...
            SkColor active = color_utils::GetResultingPaintColor(toolbar_top_separator, frame_color);
            SkColor inactive = color_utils::GetResultingPaintColor(toolbar_top_separator_inactive, frame_color_inactive);
            return color_utils::GetContrastRatio(frame_color, active) >= kMinContrastRatio
                && color_utils::GetContrastRatio(frame_color_inactive, inactive) >= kMinContrastRatio;
        };

        if (!toolbar_top_separator_has_good_contrast()) {
            toolbar_top_separator = GetBorderColor(header_selector + " button");
            toolbar_top_separator_inactive = GetBorderColor(header_selector + ":backdrop button");
        }

        // If we can't get a contrasting stroke from the theme, have ThemeService
        // provide a stroke color for us.
        if (toolbar_top_separator_has_good_contrast()) {
            color_map[ThemeProperties::COLOR_TAB_STROKE_FRAME_ACTIVE] = toolbar_top_separator;
            color_map[ThemeProperties::COLOR_TAB_STROKE_FRAME_INACTIVE] = toolbar_top_separator_inactive;
            color_map[ThemeProperties::COLOR_TOOLBAR_TOP_SEPARATOR_FRAME_ACTIVE] = toolbar_top_separator;
            color_map[ThemeProperties::COLOR_TOOLBAR_TOP_SEPARATOR_FRAME_INACTIVE] = toolbar_top_separator_inactive;
        }
    }
}
...
void GtkUi::TrackMonitor(GdkMonitor* monitor)
{
    auto connect = [&](const char* detailed_signal) {
        // Unretained() is safe since GtkUi will own the ScopedGSignal.
        return ScopedGSignal(monitor, detailed_signal, base::BindRepeating(&GtkUi::OnMonitorChanged, base::Unretained(this)), G_CONNECT_AFTER);
    };
    monitor_signals_[monitor] = { connect("notify::geometry"), connect("notify::scale-factor") };
}
...
display::DisplayConfig GtkUi::GetDisplayConfig() const
{
    ...
    const double font_scale = platform_->IncludeFontScaleInDeviceScale() ? FontScale() : 1.0;
    ...
    if (GtkCheckVersion(4)) {
        ...
        for (unsigned int i = 0; i < n_monitors; ++i) {
            monitors.push_back(static_cast<GdkMonitor*>(g_list_model_get_item(list, i)));
        }
    } else {
        const int n_monitors = gdk_display_get_n_monitors(display);
        monitors.reserve(n_monitors);
    ...
    config.primary_scale = std::max(1, gdk_monitor_get_scale_factor(primary)) * font_scale;
    ...
    for (GdkMonitor* monitor : monitors) {
        ...
        config.display_geometries.emplace_back(
            gfx::Rect(monitor_scale * geometry.x, monitor_scale * geometry.y, monitor_scale * geometry.width, monitor_scale * geometry.height),
            monitor_scale * font_scale);
    }
    return config;
}

void GtkUi::AddGtkNativeColorMixer(ui::ColorProvider* provider, const ui::ColorProviderKey& key)
{
...
void GtkUi::UpdateDeviceScaleFactor()
{
    ...
    if (display_config() != new_config) {
        ...
        device_scale_factor_observer_list().Notify(&ui::DeviceScaleFactorObserver::OnDeviceScaleFactorChanged);
    }
