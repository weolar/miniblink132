...
SkBitmap GetWidgetBitmap(const gfx::Size& size, GtkCssContext context, BackgroundRenderMode bg_mode, bool render_frame)
{
...
void PaintWidget(cc::PaintCanvas* canvas, const gfx::Rect& rect, GtkCssContext context, BackgroundRenderMode bg_mode, bool render_frame)
{
    canvas->drawImage(cc::PaintImage::CreateFromBitmap(GetWidgetBitmap(rect.size(), context, bg_mode, render_frame)), rect.x(), rect.y());
}
...
NativeThemeGtk::NativeThemeGtk()
    : NativeThemeBase(/*should_only_use_dark_colors=*/false, ui::SystemTheme::kGtk)
{
...
void NativeThemeGtk::SetThemeCssOverride(ScopedCssProvider provider)
{
    if (theme_css_override_) {
        if (GtkCheckVersion(4)) {
            gtk_style_context_remove_provider_for_display(gdk_display_get_default(), GTK_STYLE_PROVIDER(theme_css_override_.get()));
        } else {
            gtk_style_context_remove_provider_for_screen(gdk_screen_get_default(), GTK_STYLE_PROVIDER(theme_css_override_.get()));
        }
    }
    ...
    if (theme_css_override_) {
        if (GtkCheckVersion(4)) {
            gtk_style_context_add_provider_for_display(
                gdk_display_get_default(), GTK_STYLE_PROVIDER(theme_css_override_.get()), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
        } else {
            gtk_style_context_add_provider_for_screen(
                gdk_screen_get_default(), GTK_STYLE_PROVIDER(theme_css_override_.get()), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
        }
    }
}
...
void NativeThemeGtk::NotifyOnNativeThemeUpdated()
{
    ...
    if (auto* native_theme_aura = ui::NativeTheme::GetInstanceForNativeUi(); native_theme_aura->UpdateContrastRelatedStates(*this)) {
...
void NativeThemeGtk::OnThemeChanged(GtkSettings* settings, GtkParamSpec* param)
{
    SetThemeCssOverride(ScopedCssProvider());

    std::string theme_name = GetGtkSettingsStringProperty(settings, "gtk-theme-name");
    ...
    set_use_dark_colors(IsForcedDarkMode() || color_utils::IsDark(window_bg_color));
    ...
    bool high_contrast = theme_name.find("high") != std::string::npos && theme_name.find("contrast") != std::string::npos;
...
void NativeThemeGtk::PaintMenuPopupBackground(cc::PaintCanvas* canvas, const ui::ColorProvider* color_provider, const gfx::Size& size,
...
void NativeThemeGtk::PaintMenuItemBackground(cc::PaintCanvas* canvas, const ui::ColorProvider* color_provider, State state, const gfx::Rect& rect,
    const MenuItemExtraParams& menu_item, ColorScheme color_scheme) const
{
    auto context = GetStyleContextFromCss(StrCat({ GtkCssMenu(), " ", GtkCssMenuItem() }));
    ...
    cc::PaintCanvas* canvas, const ui::ColorProvider* color_provider, State state, const gfx::Rect& rect, const MenuSeparatorExtraParams& menu_separator) const
{
    ...
    auto context = GetStyleContextFromCss(StrCat({ GtkCssMenu(), " separator.horizontal" }));
    int min_height = 1;
    ...
    int h = std::max(min_height + padding.top() + padding.bottom() + border.top() + border.bottom(), 1);
    int x = margin.left();
...
{
    auto context = GetStyleContextFromCss(frame_top_area.use_custom_frame ? "headerbar.header-bar.titlebar" : "menubar");
    ApplyCssToContext(context, "* { border-radius: 0px; border-style: none; }");
    gtk_style_context_set_state(context, frame_top_area.is_active ? GTK_STATE_FLAG_NORMAL : GTK_STATE_FLAG_BACKDROP);

    SkBitmap bitmap = GetWidgetBitmap(rect.size(), context, frame_top_area.use_custom_frame ? BG_RENDER_NORMAL : BG_RENDER_RECURSIVE, false);
    bitmap.setImmutable();
