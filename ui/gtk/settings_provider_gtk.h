...
private:
    ...
    public:
        FrameActionSettingWatcher(SettingsProviderGtk* settings_provider, const std::string& setting_name, ui::LinuxUi::WindowFrameActionSource action_type,
            ui::LinuxUi::WindowFrameAction default_action);

        FrameActionSettingWatcher(const FrameActionSettingWatcher&) = delete;
        FrameActionSettingWatcher& operator=(const FrameActionSettingWatcher&) = delete;
    ...
    void OnDecorationButtonLayoutChanged(GtkSettings* settings, GParamSpec* param);

    raw_ptr<GtkUi> delegate_;

    ScopedGSignal signal_;

    std::vector<std::unique_ptr<FrameActionSettingWatcher>> frame_action_setting_watchers_;
};
