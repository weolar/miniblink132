...
WindowPositionInRootMonitor::WindowPositionInRootMonitor(aura::Window* window, base::RepeatingClosure callback)
...
void WindowPositionInRootMonitor::OnWindowParentChanged(aura::Window* window, aura::Window* parent)
{
...
{
    if (old_bounds.origin() != new_bounds.origin() && ancestors_.back()->GetRootWindow()) {
        callback_.Run();
    }
}
