...
BOOL CALLBACK EnumMonitorForProfilePathCallback(HMONITOR monitor, HDC input_hdc, LPRECT rect, LPARAM data)
{
    ...
    int64_t display_id = internal::DisplayInfo::DisplayIdFromMonitorInfo(monitor_info);
    std::map<int64_t, std::wstring>* display_id_to_path_map = reinterpret_cast<std::map<int64_t, std::wstring>*>(data);
...
void ColorProfileReader::UpdateIfNeeded()
{
    ...
    // during startup.
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock(), base::TaskPriority::USER_VISIBLE },
        base::BindOnce(&ColorProfileReader::BuildDisplayIdToPathMapOnBackgroundThread),
        base::BindOnce(&ColorProfileReader::BuildDisplayIdToPathMapCompleted, weak_factory_.GetWeakPtr()));
}

// static
ColorProfileReader::DisplayIdToPathMap ColorProfileReader::BuildDisplayIdToPathMapOnBackgroundThread()
{
    SCOPED_MAY_LOAD_LIBRARY_AT_BACKGROUND_PRIORITY();
    DisplayIdToPathMap display_id_to_path_map;
    EnumDisplayMonitors(nullptr, nullptr, EnumMonitorForProfilePathCallback, reinterpret_cast<LPARAM>(&display_id_to_path_map));
    return display_id_to_path_map;
}

void ColorProfileReader::BuildDisplayIdToPathMapCompleted(DisplayIdToPathMap new_display_id_to_path_map)
{
    ...
    base::ThreadPool::PostTaskAndReplyWithResult(FROM_HERE, { base::MayBlock(), base::TaskPriority::USER_VISIBLE },
        base::BindOnce(&ColorProfileReader::ReadProfilesOnBackgroundThread, new_display_id_to_path_map),
        base::BindOnce(&ColorProfileReader::ReadProfilesCompleted, weak_factory_.GetWeakPtr()));
}

// static
ColorProfileReader::DisplayIdToDataMap ColorProfileReader::ReadProfilesOnBackgroundThread(DisplayIdToPathMap new_display_id_to_path_map)
{
...
void ColorProfileReader::ReadProfilesCompleted(DisplayIdToDataMap display_id_to_data_map)
{
    DCHECK(update_in_flight_);
    update_in_flight_ = false;
    ...
    for (const auto& [display_id, profile_data] : display_id_to_data_map) {
        if (!profile_data.empty()) {
            display_id_to_profile_map_[display_id] = gfx::ICCProfile::FromData(profile_data.data(), profile_data.size());
        }
    }
...
gfx::ColorSpace ColorProfileReader::GetDisplayColorSpace(int64_t display_id) const
{
    ...
    return icc_profile.IsValid() ? icc_profile.GetPrimariesOnlyColorSpace() : gfx::ColorSpace::CreateSRGB();
}
