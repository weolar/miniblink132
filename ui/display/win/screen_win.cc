...
BASE_FEATURE(kSkipEmptyDisplayHotplugEvent, "SkipEmptyDisplayHotplugEvent", base::FEATURE_ENABLED_BY_DEFAULT);
...
float GetScaleFactorForDPI(int dpi, bool include_accessibility)
{
    ...
    return include_accessibility ? (scale * UwpTextScaleFactor::Instance()->GetTextScaleFactor()) : scale;
}
...
float GetMonitorScaleFactor(HMONITOR monitor, bool include_accessibility = true)
{
    DCHECK(monitor);
    ...
    return dpi ? GetScaleFactorForDPI(dpi.value(), include_accessibility) : GetDPIScale();
}
...
std::string GetFriendlyDeviceName(const std::optional<DISPLAYCONFIG_PATH_INFO>& path)
{
    if (!path)
        return std::string();
...
DISPLAYCONFIG_VIDEO_OUTPUT_TECHNOLOGY GetOutputTechnology(const std::optional<DISPLAYCONFIG_PATH_INFO>& path)
{
    if (path)
...
DisplaySettings GetDisplaySettingsForDevice(const wchar_t* device_name)
{
    ...
    return { OrientationToRotation(mode.dmDisplayOrientation), static_cast<int>(mode.dmDisplayFrequency) };
}

std::vector<internal::DisplayInfo> FindAndRemoveTouchingDisplayInfos(
    const internal::DisplayInfo& parent_info, std::vector<internal::DisplayInfo>* display_infos)
{
    const auto first_touching_it
        = std::partition(display_infos->begin(), display_infos->end(), [&](const auto& info) { return !DisplayInfosTouch(parent_info, info); });
    std::vector<internal::DisplayInfo> touching_display_infos(first_touching_it, display_infos->end());
...
gfx::DisplayColorSpaces CreateDisplayColorSpaces(const gfx::ColorSpace& color_space, float sdr_white_level)
{
    gfx::DisplayColorSpaces display_color_spaces(color_space);
    display_color_spaces.SetOutputBufferFormats(gfx::BufferFormat::BGRA_8888, gfx::BufferFormat::BGRA_8888);
...
gfx::DisplayColorSpaces GetDisplayColorSpacesForHdr(float sdr_white_level, const gfx::mojom::DXGIOutputDesc* dxgi_output_desc)
{
    auto color_spaces = CreateDisplayColorSpaces(gfx::ColorSpace::CreateSRGB(), sdr_white_level);
    ...
    if (dxgi_output_desc) {
        if (dxgi_output_desc->hdr_enabled) {
            hdr_max_luminance_relative = dxgi_output_desc->max_luminance / sdr_white_level;
        }
        color_spaces.SetPrimaries(dxgi_output_desc->primaries);
    }
    hdr_max_luminance_relative = std::max(hdr_max_luminance_relative, kMinHDRCapableMaxLuminanceRelative);
    ...
    for (const auto& usage : { gfx::ContentColorUsage::kWideColorGamut, gfx::ContentColorUsage::kHDR }) {
        ...
        if (base::win::GetVersion() > base::win::Version::WIN10_RS3) {
            color_spaces.SetOutputColorSpaceAndBufferFormat(usage, !kNeedsAlpha, scrgb_linear, gfx::BufferFormat::RGBA_F16);
        } else {
            color_spaces.SetOutputColorSpaceAndBufferFormat(usage, !kNeedsAlpha, hdr10, gfx::BufferFormat::RGBA_1010102);
        }
...
gfx::DisplayColorSpaces GetForcedDisplayColorSpaces()
{
    ...
    auto display_color_spaces = CreateDisplayColorSpaces(color_space, gfx::ColorSpace::kDefaultSDRWhiteLevel);
    ...
    if (color_space.GetTransferID() == gfx::ColorSpace::TransferID::PQ) {
        display_color_spaces.SetOutputBufferFormats(gfx::BufferFormat::RGBA_1010102, gfx::BufferFormat::RGBA_1010102);
    } else if (color_space.IsHDR()) {
        display_color_spaces.SetOutputBufferFormats(gfx::BufferFormat::RGBA_F16, gfx::BufferFormat::RGBA_F16);
    }
    return display_color_spaces;
}

Display CreateDisplayFromDisplayInfo(const internal::DisplayInfo& display_info, const ColorProfileReader* color_profile_reader,
    const gfx::mojom::DXGIOutputDesc* dxgi_output_desc, bool hdr_enabled)
{
    const float scale_factor = display_info.device_scale_factor();
    const gfx::Rect bounds = gfx::ScaleToEnclosingRect(display_info.screen_rect(), 1.0f / scale_factor);
    Display display(display_info.id(), bounds);
    display.set_device_scale_factor(scale_factor);
    display.set_work_area(gfx::ScaleToEnclosingRect(display_info.screen_work_rect(), 1.0f / scale_factor));
    ...
    if (HasForceDisplayColorProfile()) {
        ...
        color_spaces = GetDisplayColorSpacesForHdr(display_info.sdr_white_level(), dxgi_output_desc);
    } else {
        //color_spaces = CreateDisplayColorSpaces(color_profile_reader->GetDisplayColorSpace(display.id()), gfx::ColorSpace::kDefaultSDRWhiteLevel);
    }
...
std::vector<ScreenWinDisplay> DisplayInfosToScreenWinDisplays(
    const std::vector<internal::DisplayInfo>& display_infos, ColorProfileReader* color_profile_reader, gfx::mojom::DXGIInfo* dxgi_info)
{
    ...
    auto primary_display_iter = base::ranges::find_if(display_infos_remaining, [](const internal::DisplayInfo& display_info) {
    ...
    std::vector<internal::DisplayInfo> available_parents = { *primary_display_iter };
    ...
    while (!available_parents.empty()) {
        ...
        for (const auto& child : FindAndRemoveTouchingDisplayInfos(parent, &display_infos_remaining)) {
    ...
    if (dxgi_info) {
        for (const auto& dxgi_output_desc : dxgi_info->output_descs) {
            auto display_info_iter = base::ranges::find_if(
                display_infos, [&](const internal::DisplayInfo& display_info) { return display_info.device_name() == dxgi_output_desc->device_name; });
    ...
    for (const auto& display_info : display_infos) {
        displays.push_back(
            CreateDisplayFromDisplayInfo(display_info, color_profile_reader, dxgi_output_descs[display_info.id()], hdr_enabled[display_info.id()]));
    }
...
std::vector<Display> ScreenWinDisplaysToDisplays(const std::vector<ScreenWinDisplay>& screen_win_displays)
{
    std::vector<Display> displays;
...
std::optional<gfx::Vector2dF> GetPixelsPerInchForPointerDevice(HANDLE source_device)
{
    static const auto get_pointer_device_rects
        = reinterpret_cast<decltype(&::GetPointerDeviceRects)>(base::win::GetUser32FunctionPointer("GetPointerDeviceRects"));
    RECT device_rect, screen_rect;
    if (!get_pointer_device_rects || !get_pointer_device_rects(source_device, &device_rect, &screen_rect))
        return std::nullopt;
    ...
    return gfx::Vector2dF(kHimetricPerInch / himetric_per_pixel_x, kHimetricPerInch / himetric_per_pixel_y);
}
...
{
    if (const std::optional<std::vector<POINTER_DEVICE_INFO>> pointer_devices = base::win::GetPointerDevices()) {
        for (const auto& device : *pointer_devices) {
            if (device.pointerDeviceType == POINTER_DEVICE_TYPE_TOUCH && device.monitor == monitor) {
...
BOOL CALLBACK EnumDisplayMonitorsCallback(HMONITOR monitor, HDC hdc, LPRECT rect, LPARAM data)
{
...
std::vector<internal::DisplayInfo> GetDisplayInfosFromSystem()
{
    ...
    EnumDisplayMonitors(nullptr, nullptr, EnumDisplayMonitorsCallback, reinterpret_cast<LPARAM>(&monitors));

    std::vector<internal::DisplayInfo> display_infos;
    base::flat_set<int64_t> hashed_ids;
    base::flat_set<int64_t> hashed_keys;
    for (HMONITOR monitor : monitors) {
        const std::optional<MONITORINFOEX> monitor_info = MonitorInfoFromHMONITOR(monitor);
        if (!monitor_info) {
            DLOG(WARNING) << "Failed to get MONITORINFOEX for " << monitor;
            continue;
        }

        const auto display_settings = GetDisplaySettingsForDevice(monitor_info->szDevice);
        const gfx::Vector2dF pixels_per_inch = GetMonitorPixelsPerInch(monitor).value_or(GetDefaultMonitorPhysicalPixelsPerInch());
        const auto path_info = GetDisplayConfigPathInfo(monitor);
        display_infos.emplace_back(*monitor_info, GetMonitorScaleFactor(monitor), GetSDRWhiteLevel(path_info), display_settings.rotation,
            display_settings.frequency, pixels_per_inch, GetOutputTechnology(path_info), GetFriendlyDeviceName(path_info));
        ...
        if (!EnumDisplayDevices(monitor_info->szDevice, 0, &device, 0)) {
            ...
            if (base::WideToUTF8(device.DeviceID).empty()) {
                ...
                const int64_t hashed_id = static_cast<int64_t>(base::PersistentHash(base::as_byte_span(device.DeviceID)));
            ...
            if (base::WideToUTF8(device.DeviceKey).empty()) {
                ...
                int64_t hashed_key = static_cast<int64_t>(base::PersistentHash(base::as_byte_span(device.DeviceKey)));
...
gfx::PointF ScalePointRelative(const gfx::PointF& point, const gfx::Point& from_origin, const gfx::Point& to_origin, const float scale_factor)
{
    const gfx::PointF relative_point = point - from_origin.OffsetFromOrigin();
    const gfx::PointF scaled_relative_point = gfx::ScalePoint(relative_point, scale_factor);
    return scaled_relative_point + to_origin.OffsetFromOrigin();
}

gfx::PointF ScreenToDIPPoint(const gfx::PointF& screen_point, const ScreenWinDisplay& screen_win_display)
{
    const Display display = screen_win_display.display();
    return ScalePointRelative(screen_point, screen_win_display.pixel_bounds().origin(), display.bounds().origin(), 1.0f / display.device_scale_factor());
}

gfx::Point DIPToScreenPoint(const gfx::Point& dip_point, const ScreenWinDisplay& screen_win_display)
{
    const Display display = screen_win_display.display();
    return gfx::ToFlooredPoint(
        ScalePointRelative(gfx::PointF(dip_point), display.bounds().origin(), screen_win_display.pixel_bounds().origin(), display.device_scale_factor()));
}
...
ScreenWinDisplay CreateFallbackPrimaryScreenDisplay()
{
    ...
    internal::DisplayInfo display_info(
        monitor_info, 1.0f, 1.0f, Display::ROTATE_0, 60.0f, gfx::Vector2dF(), DISPLAYCONFIG_OUTPUT_TECHNOLOGY_OTHER, std::string());
...
{
    const ScreenWinDisplay screen_win_display = GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestScreenPoint, gfx::ToFlooredPoint(pixel_point));
...
{
    const ScreenWinDisplay screen_win_display = GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestDIPPoint, dip_point);
    return display::win::DIPToScreenPoint(dip_point, screen_win_display);
}

// static
gfx::Point ScreenWin::ClientToDIPPoint(HWND hwnd, const gfx::Point& client_point)
{
...
{
    const ScreenWinDisplay screen_win_display = hwnd ? GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestHWND, hwnd)
                                                     : GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestScreenRect, pixel_bounds);
    const gfx::Point origin = gfx::ToFlooredPoint(display::win::ScreenToDIPPoint(gfx::PointF(pixel_bounds.origin()), screen_win_display));
    const float scale_factor = 1.0f / screen_win_display.display().device_scale_factor();
...
gfx::Rect ScreenWin::DIPToScreenRect(HWND hwnd, const gfx::Rect& dip_bounds)
{
    ...
    const ScreenWinDisplay screen_win_display = hwnd ? GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestHWND, hwnd)
                                                     : GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestDIPRect, dip_bounds);
    const gfx::Point origin = display::win::DIPToScreenPoint(dip_bounds.origin(), screen_win_display);
...
// static
gfx::Size ScreenWin::ScreenToDIPSize(HWND hwnd, const gfx::Size& size_in_pixels)
{
...
int ScreenWin::GetSystemMetricsForMonitor(HMONITOR monitor, int metric)
{
    ...
    // ourselves.
    const bool include_accessibility = (metric != SM_CXSIZEFRAME) && (metric != SM_CYSIZEFRAME) && (metric != SM_CXPADDEDBORDER);

    // We'll then pull up the system metrics scaled by the appropriate amount.
    return g_instance->GetSystemMetricsForScaleFactor(GetMonitorScaleFactor(monitor, include_accessibility), metric);
}

// static
int ScreenWin::GetSystemMetricsInDIP(int metric)
{
    return g_instance ? g_instance->GetSystemMetricsForScaleFactor(1.0f, metric) : ::GetSystemMetrics(metric);
}
...
float ScreenWin::GetScaleFactorForHWND(HWND hwnd)
{
    ...
    const ScreenWinDisplay screen_win_display = GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestHWND, root_hwnd);
...
{
    const ScreenWinDisplay screen_win_display = GetScreenWinDisplayVia(&ScreenWin::GetScreenWinDisplayNearestDIPPoint, gfx::ToFlooredPoint(point));
...
int ScreenWin::GetDPIForHWND(HWND hwnd)
{
    ...
    return GetPerMonitorDPI(monitor).value_or(display::win::internal::GetDefaultSystemDPI());
}
...
// static
void ScreenWin::SetRequestHDRStatusCallback(RequestHDRStatusCallback request_hdr_status_callback)
{
    if (g_instance) {
        g_instance->request_hdr_status_callback_ = std::move(request_hdr_status_callback);
...
ScreenWinDisplay ScreenWin::GetScreenWinDisplayWithDisplayId(int64_t id)
{
    ...
    const auto it = base::ranges::find(g_instance->screen_win_displays_, id, [](const auto& display) { return display.display().id(); });
    ...
    // default display.
    return (it == g_instance->screen_win_displays_.cend()) ? g_instance->GetPrimaryScreenWinDisplay() : *it;
}
...
std::optional<bool> ScreenWin::IsWindowOnCurrentVirtualDesktop(gfx::NativeWindow window) const
{
    NOTREACHED();
}
...
{
    POINT cursor_loc;
    return ::GetCursorPos(&cursor_loc) && (GetNativeWindowFromHWND(::WindowFromPoint(cursor_loc)) == window);
}
...
gfx::NativeWindow ScreenWin::GetLocalProcessWindowAtPoint(const gfx::Point& point, const std::set<gfx::NativeWindow>& ignore)
{
    ...
    return LocalProcessWindowFinder::GetProcessWindowAtPoint(point, hwnd_set, this);
}
...
Display ScreenWin::GetDisplayNearestWindow(gfx::NativeWindow window) const
{
    ...
    return window_hwnd ? GetScreenWinDisplayNearestHWND(window_hwnd).display() : GetPrimaryDisplay();
}
...
gfx::Rect ScreenWin::ScreenToDIPRectInWindow(gfx::NativeWindow window, const gfx::Rect& screen_rect) const
{
    const HWND hwnd = window ? GetHWNDFromNativeWindow(window) : nullptr;
    return ScreenToDIPRect(hwnd, screen_rect);
}

gfx::Rect ScreenWin::DIPToScreenRectInWindow(gfx::NativeWindow window, const gfx::Rect& dip_rect) const
{
    const HWND hwnd = window ? GetHWNDFromNativeWindow(window) : nullptr;
    return DIPToScreenRect(hwnd, dip_rect);
}

void ScreenWin::UpdateFromDisplayInfos(const std::vector<internal::DisplayInfo>& display_infos)
{
    auto new_screen_win_displays = DisplayInfosToScreenWinDisplays(display_infos, /*color_profile_reader_.get()*/nullptr, dxgi_info_.get());
    ...
    const std::optional<MONITORINFOEX> primary_monitor_info = MonitorInfoFromHMONITOR(primary_monitor_);
    // Primary monitor, if it exists, has 0,0 origin. Guard the CHECK with kill switch
    // in case this caused the problem in the field.
    if (primary_monitor_info && base::FeatureList::IsEnabled(features::kSkipEmptyDisplayHotplugEvent)) {
        CHECK(gfx::Rect(primary_monitor_info->rcMonitor).origin().IsOrigin());
    }

    screen_win_displays_ = std::move(new_screen_win_displays);

    std::vector<Display> displays = ScreenWinDisplaysToDisplays(screen_win_displays_);
...
void ScreenWin::Initialize()
{
    ...
    singleton_hwnd_observer_ = std::make_unique<gfx::SingletonHwndObserver>(base::BindRepeating(&ScreenWin::OnWndProc, base::Unretained(this)));
...
std::optional<MONITORINFOEX> ScreenWin::MonitorInfoFromScreenPoint(const gfx::Point& screen_point) const
{
    return MonitorInfoFromHMONITOR(::MonitorFromPoint(screen_point.ToPOINT(), MONITOR_DEFAULTTONEAREST));
}

std::optional<MONITORINFOEX> ScreenWin::MonitorInfoFromScreenRect(const gfx::Rect& screen_rect) const
{
    const RECT win_rect = screen_rect.ToRECT();
    return MonitorInfoFromHMONITOR(::MonitorFromRect(&win_rect, MONITOR_DEFAULTTONEAREST));
}

std::optional<MONITORINFOEX> ScreenWin::MonitorInfoFromWindow(HWND hwnd, DWORD default_options) const
{
...
void ScreenWin::OnWndProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    if (message != WM_DISPLAYCHANGE && (message != WM_ACTIVATEAPP || wparam != TRUE) && (message != WM_SETTINGCHANGE || wparam != SPI_SETWORKAREA))
        return;
...
//             return display.GetColorSpaces().GetRasterColorSpace() != color_profile_reader_->GetDisplayColorSpace(display.id());
//         })) {
//         UpdateAllDisplaysAndNotify();
//     }
}
...
{
    return GetScreenWinDisplay(MonitorInfoFromWindow(hwnd, MONITOR_DEFAULTTONEAREST));
}

ScreenWinDisplay ScreenWin::GetScreenWinDisplayNearestScreenRect(const gfx::Rect& screen_rect) const
{
    return GetScreenWinDisplay(MonitorInfoFromScreenRect(screen_rect));
}

ScreenWinDisplay ScreenWin::GetScreenWinDisplayNearestScreenPoint(const gfx::Point& screen_point) const
{
    return GetScreenWinDisplay(MonitorInfoFromScreenPoint(screen_point));
}

ScreenWinDisplay ScreenWin::GetScreenWinDisplayNearestDIPPoint(const gfx::Point& dip_point) const
{
...
ScreenWinDisplay ScreenWin::GetScreenWinDisplayNearestDIPRect(const gfx::Rect& dip_rect) const
{
    const auto first_closer = [dip_rect](const auto& display1, const auto& display2) {
        return SquaredDistanceBetweenRects(dip_rect, display1.display().bounds()) < SquaredDistanceBetweenRects(dip_rect, display2.display().bounds());
    };
    const auto it = std::min_element(screen_win_displays_.cbegin(), screen_win_displays_.cend(), first_closer);
    return (it == screen_win_displays_.cend()) ? GetPrimaryScreenWinDisplay() : *it;
}

ScreenWinDisplay ScreenWin::GetPrimaryScreenWinDisplay() const
{
    const auto it = base::ranges::find_if(screen_win_displays_, [](const auto& display) { return IsPrimaryScreenWinDisplay(display); });
    if (it == screen_win_displays_.end()) {
        static base::NoDestructor<ScreenWinDisplay> fallback_primary_screen_win_display(CreateFallbackPrimaryScreenDisplay());
        LOG(WARNING) << "Unable to find a primary display. Fallback to fake display:" << fallback_primary_screen_win_display->display().ToString();
...
ScreenWinDisplay ScreenWin::GetScreenWinDisplay(std::optional<MONITORINFOEX> monitor_info) const
{
    if (monitor_info) {
        const int64_t id = internal::DisplayInfo::DisplayIdFromMonitorInfo(*monitor_info);
        const auto it = base::ranges::find(screen_win_displays_, id, [](const auto& display) { return display.display().id(); });
        // There is 1:1 correspondence between MONITORINFOEX and ScreenWinDisplay.
...
// static
template <typename Getter, typename GetterType> ScreenWinDisplay ScreenWin::GetScreenWinDisplayVia(Getter getter, GetterType value)
{
    return g_instance ? (g_instance->*getter)(value) : ScreenWinDisplay();
}

int ScreenWin::GetSystemMetricsForScaleFactor(float scale_factor, int metric) const
{
    if (!PerProcessDPIAwarenessDisabledForTesting()) {
        static const auto get_system_metrics_for_dpi
            = reinterpret_cast<decltype(&::GetSystemMetricsForDpi)>(base::win::GetUser32FunctionPointer("GetSystemMetricsForDpi"));
        if (get_system_metrics_for_dpi) {
            return get_system_metrics_for_dpi(metric, GetDPIFromScalingFactor(scale_factor));
        }
    }

    // Versions < WIN10_RS1 don't support GetSystemMetricsForDpi, but do support
    // per-process dpi awareness.
    return base::ClampRound(GetSystemMetrics(metric) * scale_factor / GetPrimaryDisplay().device_scale_factor());
}
...
// void UwpTextScaleFactor::Observer::OnUwpTextScaleFactorCleanup(UwpTextScaleFactor* source)
// {
//     *(int*)1 = 1;
// }
// 
// void UwpTextScaleFactor::RemoveObserver(UwpTextScaleFactor::Observer*)
// {
//     *(int*)1 = 1;
// }
// 
// UwpTextScaleFactor* UwpTextScaleFactor::Instance(void)
// {
//     *(int*)1 = 1;
//     return nullptr;
// }
