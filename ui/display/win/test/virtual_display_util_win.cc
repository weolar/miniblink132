...
static const auto kConfigMap = base::MakeFixedFlatMap<display::test::DisplayParams, std::reference_wrapper<const display::test::MonitorConfig>>(
    { { display::test::VirtualDisplayUtil::k1024x768, display::test::MonitorConfig::k1024x768 },
        { display::test::VirtualDisplayUtil::k1920x1080, display::test::MonitorConfig::k1920x1080 } });
...
bool IsHeadless()
{
    ...
    for (DWORD i = 0; EnumDisplayDevices(nullptr, i, &adapter, EDD_GET_DEVICE_INTERFACE_NAME); i++) {
...
bool SetDisplayResolution(const gfx::Size& size)
{
    ...
    LOG_IF(ERROR, settings_result != DISP_CHANGE_SUCCESSFUL) << "ChangeDisplaySettings failed with result: " << settings_result;
...
VirtualDisplayUtilWin::~VirtualDisplayUtilWin()
{
    ...
    if (is_headless_) {
        ...
        // crbug.com/341931537).
        CHECK_EQ(current_displays.size(), initial_displays_.size()) << "# of displays mismatch after driver was closed.";
        ...
        CHECK(initial_sizes == current_sizes) << "Display resolution does not match initial config. Initial: " << JoinDisplayStrings(initial_displays_)
...
{
    auto it = std::find_if(
        virtual_displays_.begin(), virtual_displays_.end(), [&display_id](const std::pair<unsigned short, int64_t>& obj) { return obj.second == display_id; });
    if (it == virtual_displays_.end()) {
        LOG(WARNING) << "Display ID " << display_id << " is not a virtual display.";
        return;
    }
    std::vector<MonitorConfig> monitors = current_config_.requested_configs();
    std::erase_if(monitors, [&it](MonitorConfig& c) { return c.product_code() == it->first; });
    if (SetDriverProperties(DriverProperties(monitors))) {
        StartWaiting();
    }
}
...
void VirtualDisplayUtilWin::ResetDisplays()
{
    ...
    if (current_config_.requested_configs().size() != prev_config.requested_configs().size()) {
        StartWaiting();
    }
}

void VirtualDisplayUtilWin::OnDisplayAdded(const display::Display& new_display)
{
    std::vector<MonitorConfig> requested = current_config_.requested_configs();
    HMONITOR monitor = ::MonitorFromPoint(win::ScreenWin::DIPToScreenPoint(new_display.work_area().CenterPoint()).ToPOINT(), MONITOR_DEFAULTTONEAREST);
    std::optional<DISPLAYCONFIG_PATH_INFO> path_info = ::display::win::GetDisplayConfigPathInfo(monitor);
    if (::display::win::GetDisplayManufacturerId(path_info) == kDriverMonitorManufacturer) {
        ...
        CHECK(it == virtual_displays_.end()) << "Detected duplicate virtual display product code:" << product_code;
        virtual_displays_[product_code] = new_display.id();
    }
    OnDisplayAddedOrRemoved(new_display.id());
}

void VirtualDisplayUtilWin::OnDisplaysRemoved(const display::Displays& removed_displays)
{
    for (const auto& display : removed_displays) {
        base::EraseIf(virtual_displays_, [&display](const std::pair<unsigned short, int64_t>& obj) { return obj.second == display.id(); });
...
void VirtualDisplayUtilWin::StartWaiting()
{
    ...
    if (virtual_displays_.size() != current_config_.requested_configs().size()) {
        ...
        ensure_extended_timer_.Start(FROM_HERE, base::Seconds(1), this, &VirtualDisplayUtilWin::EnsureExtendMode);
        run_loop_->Run();
    }
    run_loop_.reset();
}
...
void VirtualDisplayUtilWin::EnsureExtendMode()
{
    ...
    GetDisplayConfigBufferSizes(QDC_ALL_PATHS, &path_array_size, &mode_array_size);
    ...
    LONG ret = QueryDisplayConfig(QDC_DATABASE_CURRENT, &path_array_size, paths.data(), &mode_array_size, modes.data(), &topology);
    ...
    ret = SetDisplayConfig(0, nullptr, 0, nullptr, SDC_TOPOLOGY_EXTEND | SDC_APPLY);
    if (ret == ERROR_SUCCESS) {
        ensure_extended_timer_.Stop();
        return;
    }
...
uint8_t VirtualDisplayUtilWin::SynthesizeInternalDisplayId()
{
    ...
    CHECK_LT(synthesized_display_id, std::numeric_limits<uint8_t>::max()) << "All synthesized display IDs in use.";
    return synthesized_display_id++;
}

// static
std::unique_ptr<VirtualDisplayUtil> VirtualDisplayUtil::TryCreate(Screen* screen)
{
...
DisplayConfigWaiter::DisplayConfigWaiter(Screen* screen)
    : screen_(screen)
{
    screen_->AddObserver(this);
}
...
void DisplayConfigWaiter::OnDisplaysRemoved(const display::Displays& removed_displays)
{
    if (IsWaitConditionMet()) {
        run_loop_->Quit();
    }
