...
private:
    ...
    static ColorProfileReader::DisplayIdToPathMap BuildDisplayIdToPathMapOnBackgroundThread();

    // Called on the main thread when the device paths have been retrieved
    void BuildDisplayIdToPathMapCompleted(DisplayIdToPathMap new_display_id_to_path_map);

    // Do the actual reading from the filesystem. This needs to be run off of the
    // main thread.
    static DisplayIdToDataMap ReadProfilesOnBackgroundThread(DisplayIdToPathMap new_display_id_to_path_map);
