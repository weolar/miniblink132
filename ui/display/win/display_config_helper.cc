...
{
    for (LONG result = ERROR_INSUFFICIENT_BUFFER; result == ERROR_INSUFFICIENT_BUFFER;) {
        ...
        result = ::QueryDisplayConfig(QDC_ONLY_ACTIVE_PATHS, &path_elements, path_infos.data(), &mode_elements, mode_infos.data(), nullptr);
...
bool GetTargetDeviceName(const DISPLAYCONFIG_PATH_INFO& path, DISPLAYCONFIG_TARGET_DEVICE_NAME* device_name)
{
...
std::optional<DISPLAYCONFIG_PATH_INFO> GetDisplayConfigPathInfo(HMONITOR monitor)
{
...
DISPLAY_EXPORT std::optional<DISPLAYCONFIG_PATH_INFO> GetDisplayConfigPathInfo(MONITORINFOEX monitor_info)
{
    ...
    for (const auto& info : GetDisplayConfigPathInfos()) {
        ...
        if ((::DisplayConfigGetDeviceInfo(&device_name.header) == ERROR_SUCCESS) && (wcscmp(monitor_info.szDevice, device_name.viewGdiDeviceName) == 0)) {
            return info;
        }
    }
    return std::nullopt;
...
UINT16 GetDisplayManufacturerId(const std::optional<DISPLAYCONFIG_PATH_INFO>& path)
{
...
UINT16 GetDisplayProductCode(const std::optional<DISPLAYCONFIG_PATH_INFO>& path)
{
    DISPLAYCONFIG_TARGET_DEVICE_NAME targetName = {};
    if (path && GetTargetDeviceName(*path, &targetName)) {
