...
// static
gfx::NativeWindow LocalProcessWindowFinder::GetProcessWindowAtPoint(const gfx::Point& screen_loc, const std::set<HWND>& ignore, ScreenWin* screen_win)
{
    LocalProcessWindowFinder finder(screen_loc, screen_win, ignore);
    return finder.result_ ? screen_win->GetNativeWindowFromHWND(finder.result_) : nullptr;
}
...
bool LocalProcessWindowFinder::ShouldStopIterating(HWND hwnd)
{
    ...
    if (native_win) {
        on_current_workspace = screen_win_->IsWindowOnCurrentVirtualDesktop(native_win);
    }
    ...
    RECT r;
    if (!IsWindowVisible(hwnd) || gfx::IsWindowCloaked(hwnd) || !GetWindowRect(hwnd, &r) || !PtInRect(&r, screen_loc_.ToPOINT())) {
...
LocalProcessWindowFinder::LocalProcessWindowFinder(const gfx::Point& screen_loc, ScreenWin* screen_win, const std::set<HWND>& ignore)
