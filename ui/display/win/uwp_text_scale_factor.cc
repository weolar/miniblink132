...
typedef ITypedEventHandler<UISettings*, IInspectable*> TextScaleChangedEventHandler;
...
bool CreateUiSettingsComObject(ComPtr<IUISettings2>& ptr)
{
    ...
    auto hstring = base::win::ScopedHString::Create(RuntimeClass_Windows_UI_ViewManagement_UISettings);
    if (!hstring.is_valid()) {
        return false;
    }
    ...
    if (FAILED(hr)) {
        VLOG(2) << "RoActivateInstance failed: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
    ...
    if (FAILED(hr)) {
        VLOG(2) << "As IUISettings2 failed: " << logging::SystemErrorCodeToString(hr);
        return false;
    }
...
public:
        ...
        : text_scale_factor_changed_token_(kInvalidEventRegistrationToken)
    {
        ...
        auto text_scale_changed_handler = Callback<TextScaleChangedEventHandler>(this, &UwpTextScaleFactorImpl::OnTextScaleFactorChanged);
        if (text_scale_changed_handler) {
            HRESULT hr = ui_settings_com_object_->add_TextScaleFactorChanged(text_scale_changed_handler.Get(), &text_scale_factor_changed_token_);
            if (FAILED(hr)) {
                VLOG(2) << "Register text scale changed callback failed: " << logging::SystemErrorCodeToString(hr);
            }
        }
    }
    ...
    ~UwpTextScaleFactorImpl() override
    {
        ...
        if (ui_settings_com_object_) {
            if (text_scale_factor_changed_token_.value) {
                HRESULT hr = ui_settings_com_object_->remove_TextScaleFactorChanged(text_scale_factor_changed_token_);
                if (FAILED(hr)) {
                    VLOG(2) << "Failed to remove TextScaleFactorChanged listener: " << logging::SystemErrorCodeToString(hr);
                }
            }
    ...
    float GetTextScaleFactor() const override
    {
        ...
        if (ui_settings_com_object_) {
            ...
            if (FAILED(hr)) {
                VLOG(2) << "IUISettings2::TextScaleFactor failed: " << logging::SystemErrorCodeToString(hr);
...
void UwpTextScaleFactor::SetImplementationForTesting(UwpTextScaleFactor* mock_impl)
{
...
UwpTextScaleFactor* UwpTextScaleFactor::Instance()
{
    ...
    DCHECK(!g_default_instance_cleaned_up) << "Attempting to access UwpScreenMetrics after AtExit cleanup!";

    return g_implementation_for_testing ? g_implementation_for_testing : &instance.Get();
}
