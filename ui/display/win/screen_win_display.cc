...
Display CreateDisplayFromDisplayInfo(const internal::DisplayInfo& display_info)
{
    ...
    display.set_bounds(gfx::ScaleToEnclosingRect(display_info.screen_rect(), 1.0f / scale_factor));
...
ScreenWinDisplay::ScreenWinDisplay(const internal::DisplayInfo& display_info)
    : ScreenWinDisplay(CreateDisplayFromDisplayInfo(display_info), display_info)
{
}

ScreenWinDisplay::ScreenWinDisplay(const Display& display, const internal::DisplayInfo& display_info)
    : display_(display)
