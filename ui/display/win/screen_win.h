...
class DISPLAY_EXPORT ScreenWin : public Screen, public ColorProfileReader::Client, public UwpTextScaleFactor::Observer {
public:
    ScreenWin();
    ...
    static void SetRequestHDRStatusCallback(RequestHDRStatusCallback request_hdr_status_callback);
    ...
    // host.
    virtual std::optional<bool> IsWindowOnCurrentVirtualDesktop(gfx::NativeWindow window) const;

protected:
    FRIEND_TEST_ALL_PREFIXES(ScreenWinTestSingleDisplay1x, DisconnectPrimaryDisplay);
    ...
    gfx::Rect ScreenToDIPRectInWindow(gfx::NativeWindow window, const gfx::Rect& screen_rect) const override;
    gfx::Rect DIPToScreenRectInWindow(gfx::NativeWindow window, const gfx::Rect& dip_rect) const override;

    // ColorProfileReader::Client:
    void OnColorProfilesChanged() override;

    void UpdateFromDisplayInfos(const std::vector<internal::DisplayInfo>& display_infos);

    // Virtual to support mocking by unit tests.
    virtual std::optional<MONITORINFOEX> MonitorInfoFromScreenPoint(const gfx::Point& screen_point) const;
    virtual std::optional<MONITORINFOEX> MonitorInfoFromScreenRect(const gfx::Rect& screen_rect) const;
    virtual std::optional<MONITORINFOEX> MonitorInfoFromWindow(HWND hwnd, DWORD default_options) const;
...
private:
    ...
    ScreenWinDisplay GetScreenWinDisplayNearestScreenRect(const gfx::Rect& screen_rect) const;

    // Returns the ScreenWinDisplay closest to or enclosing |screen_point|.
    ScreenWinDisplay GetScreenWinDisplayNearestScreenPoint(const gfx::Point& screen_point) const;

    // Returns the ScreenWinDisplay closest to or enclosing |dip_point|.
    ScreenWinDisplay GetScreenWinDisplayNearestDIPPoint(const gfx::Point& dip_point) const;

    // Returns the ScreenWinDisplay closest to or enclosing |dip_rect|.
    ScreenWinDisplay GetScreenWinDisplayNearestDIPRect(const gfx::Rect& dip_rect) const;

    // Returns the ScreenWinDisplay corresponding to the primary monitor.
    ScreenWinDisplay GetPrimaryScreenWinDisplay() const;

    ScreenWinDisplay GetScreenWinDisplay(std::optional<MONITORINFOEX> monitor_info) const;
    ...
    template <typename Getter, typename GetterType> static ScreenWinDisplay GetScreenWinDisplayVia(Getter getter, GetterType value);
    ...
    std::unique_ptr<ColorProfileReader> color_profile_reader_ = std::make_unique<ColorProfileReader>(this);
    ...
    base::ScopedObservation<UwpTextScaleFactor, UwpTextScaleFactor::Observer> scale_factor_observation_ { this };
