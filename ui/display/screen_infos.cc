...
ScreenInfos::ScreenInfos(const ScreenInfo& screen_info)
    : screen_infos { screen_info }
    , current_display_id(screen_info.display_id)
{
}
...
{
    return screen_infos == other.screen_infos && current_display_id == other.current_display_id && system_cursor_size == other.system_cursor_size;
}
...
{
    return const_cast<ScreenInfo&>(const_cast<const ScreenInfos*>(this)->current());
}

const ScreenInfo& ScreenInfos::current() const
{
    const auto& current_screen_info = base::ranges::find(screen_infos, current_display_id, &ScreenInfo::display_id);
