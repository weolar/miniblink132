...
DisplayList::Displays::const_iterator DisplayList::FindDisplayById(int64_t id) const
{
    return base::ranges::find(displays_, id, &Display::id);
}

DisplayList::Displays::const_iterator DisplayList::GetPrimaryDisplayIterator() const
{
...
{
    return UpdateDisplay(display, display.id() == primary_id_ ? Type::PRIMARY : Type::NOT_PRIMARY);
}
...
uint32_t DisplayList::UpdateDisplay(const Display& display, Type type)
{
    ...
    if (local_display->GetColorSpaces() != display.GetColorSpaces() || local_display->depth_per_component() != display.depth_per_component()
...
DisplayList::Displays::iterator DisplayList::FindDisplayByIdInternal(int64_t id)
{
    return base::ranges::find(displays_, id, &Display::id);
}
