...
// static
void DisplayUtil::DisplayToScreenInfo(ScreenInfo* screen_info, const Display& display)
{
    ...
    screen_info->is_primary = screen && (screen->GetPrimaryDisplay().id() == display.id());
...
// static
void DisplayUtil::GetNativeViewScreenInfo(ScreenInfo* screen_info, gfx::NativeView native_view)
{
    // Some tests are run with no Screen initialized.
    ...
    Display display = native_view ? screen->GetDisplayNearestView(native_view) : screen->GetPrimaryDisplay();
    DisplayToScreenInfo(screen_info, display);
}

// static
mojom::ScreenOrientation DisplayUtil::GetOrientationTypeForMobile(const Display& display)
{
    ...
    switch (angle) {
    case 0:
        return natural_portrait ? mojom::ScreenOrientation::kPortraitPrimary : mojom::ScreenOrientation::kLandscapePrimary;
    case 90:
        return natural_portrait ? mojom::ScreenOrientation::kLandscapePrimary : mojom::ScreenOrientation::kPortraitSecondary;
    case 180:
        return natural_portrait ? mojom::ScreenOrientation::kPortraitSecondary : mojom::ScreenOrientation::kLandscapeSecondary;
    case 270:
        return natural_portrait ? mojom::ScreenOrientation::kLandscapeSecondary : mojom::ScreenOrientation::kPortraitPrimary;
    default:
        NOTREACHED();
    }
}
...
// static
mojom::ScreenOrientation DisplayUtil::GetOrientationTypeForDesktop(const Display& display)
{
    ...
    if (is_portrait) {
        return primary_portrait_angle == angle ? mojom::ScreenOrientation::kPortraitPrimary : mojom::ScreenOrientation::kPortraitSecondary;
    }

    return primary_landscape_angle == angle ? mojom::ScreenOrientation::kLandscapePrimary : mojom::ScreenOrientation::kLandscapeSecondary;
}
