...
void CheckDisplayLayoutsEqual(const DisplayLayout& input, const DisplayLayout& output)
{
    EXPECT_NE(&input, &output); // Make sure they aren't the same object.
...
void CheckDisplayModesEqual(const DisplayMode* input, const DisplayMode* output)
{
...
void CheckDisplaySnapShotMojoEqual(const DisplaySnapshot& input, const DisplaySnapshot& output)
{
    ...
    EXPECT_EQ(input.is_aspect_preserving_scaling(), output.is_aspect_preserving_scaling());
    EXPECT_EQ(input.has_overscan(), output.has_overscan());
    EXPECT_EQ(input.privacy_screen_state(), output.privacy_screen_state());
    EXPECT_EQ(input.has_color_correction_matrix(), output.has_color_correction_matrix());
    ...
    EXPECT_EQ(input.color_info().edid_primaries, output.color_info().edid_primaries);
    EXPECT_EQ(input.color_info().edid_gamma, output.color_info().edid_gamma);
    EXPECT_EQ(input.color_info().hdr_static_metadata, output.color_info().hdr_static_metadata);
    EXPECT_EQ(input.color_info().supports_color_temperature_adjustment, output.color_info().supports_color_temperature_adjustment);
    EXPECT_EQ(input.color_info().bits_per_channel, output.color_info().bits_per_channel);

    EXPECT_EQ(input.bits_per_channel(), output.bits_per_channel());
    EXPECT_EQ(input.hdr_static_metadata(), output.hdr_static_metadata());
    EXPECT_EQ(input.variable_refresh_rate_state(), output.variable_refresh_rate_state());
}
...
template <class MojomType, class Type> void SerializeAndDeserialize(const Type& input, Type* output)
{
...
template <class MojomType, class Type> void SerializeAndDeserialize(Type&& input, Type* output)
{
    MojomType::Deserialize(MojomType::Serialize(&input), output);
}
...
TEST(DisplayStructTraitsTest, ColorCalibrationRoundtrip)
{
    ...
    EXPECT_EQ(0, memcmp(&input.srgb_to_device_matrix, &output.srgb_to_device_matrix, sizeof(skcms_Matrix3x3)));
...
TEST(DisplayStructTraitsTest, ColorTemperatureAdjustmentRoundtrip)
{
    ...
    EXPECT_EQ(0, memcmp(&input.srgb_matrix, &output.srgb_matrix, sizeof(skcms_Matrix3x3)));
}
...
TEST(DisplayStructTraitsTest, DisplaySnapshotCurrentAndNativeModesNull)
{
    ...
    color_info.hdr_static_metadata.emplace(100.0, 80.0, 0.0, gfx::HDRStaticMetadata::EotfMask({ gfx::HDRStaticMetadata::Eotf::kGammaSdrRange }));
    ...
    const VariableRefreshRateState variable_refresh_rate_state = VariableRefreshRateState::kVrrEnabled;
    ...
    drm_formats_and_modifiers.emplace(DRM_FORMAT_ARGB8888, std::vector<uint64_t>({ DRM_FORMAT_MOD_INVALID }));

    std::unique_ptr<DisplaySnapshot> input = std::make_unique<DisplaySnapshot>(port_display_id, port_display_id, edid_display_id, connector_index, origin,
        physical_size, type, base_connector_id, path_topology, is_aspect_preserving_scaling, has_overscan, privacy_screen_state, has_content_protection_key,
        color_info, display_name, sys_path, std::move(modes), PanelOrientation::kNormal, edid, current_mode, native_mode, product_code, year_of_manufacture,
        maximum_cursor_size, variable_refresh_rate_state, std::move(drm_formats_and_modifiers));
...
TEST(DisplayStructTraitsTest, DisplaySnapshotExternal)
{
    ...
    const VariableRefreshRateState variable_refresh_rate_state = VariableRefreshRateState::kVrrDisabled;
    ...
    std::unique_ptr<DisplaySnapshot> input = std::make_unique<DisplaySnapshot>(port_display_id, port_display_id, edid_display_id, connector_index, origin,
        physical_size, type, base_connector_id, path_topology, is_aspect_preserving_scaling, has_overscan, privacy_screen_state, has_content_protection_key,
        color_info, display_name, sys_path, std::move(modes), PanelOrientation::kLeftUp, edid, current_mode, native_mode, product_code, year_of_manufacture,
        maximum_cursor_size, variable_refresh_rate_state, std::move(drm_formats_and_modifiers));
...
TEST(DisplayStructTraitsTest, DisplaySnapshotInternal)
{
    ...
    color_info.hdr_static_metadata.emplace(200.0, 100.0, 0.0,
    ...
    const int64_t product_code = 139;
    const int32_t year_of_manufacture = 2018;
    const VariableRefreshRateState variable_refresh_rate_state = VariableRefreshRateState::kVrrNotCapable;
    ...
    std::unique_ptr<DisplaySnapshot> input = std::make_unique<DisplaySnapshot>(port_display_id, port_display_id, edid_display_id, connector_index, origin,
        physical_size, type, base_connector_id, path_topology, is_aspect_preserving_scaling, has_overscan, privacy_screen_state, has_content_protection_key,
        color_info, display_name, sys_path, std::move(modes), PanelOrientation::kRightUp, edid, current_mode, native_mode, product_code, year_of_manufacture,
