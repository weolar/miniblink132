...
template <> struct COMPONENT_EXPORT(DISPLAY_SHARED_MOJOM_TRAITS) StructTraits<display::mojom::ScreenInfosDataView, display::ScreenInfos> {
    static const std::vector<display::ScreenInfo>& screen_infos(const display::ScreenInfos& r)
    {
    ...
    static bool Read(display::mojom::ScreenInfosDataView r, display::ScreenInfos* out);
};
