...
// static
bool StructTraits<display::mojom::ScreenInfosDataView, display::ScreenInfos>::Read(display::mojom::ScreenInfosDataView data, display::ScreenInfos* out)
{
    if (!data.ReadScreenInfos(&out->screen_infos) || !data.ReadSystemCursorSize(&out->system_cursor_size))
        return false;
