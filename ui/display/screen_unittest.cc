...
public:
    ScreenTest()
    {
        ...
        display.set_bounds(gfx::Rect(0, 0, DEFAULT_DISPLAY_WIDTH, DEFAULT_DISPLAY_HEIGHT));
        ...
        test_screen_.display_list().AddDisplay(Display(DISPLAY_2_ID), DisplayList::Type::NOT_PRIMARY);
...
public:
    ...
    ~ZeroDisplayScreenTest() override
    {
        Screen::SetScreenInstance(nullptr);
    }
...
{
    Display display;
    EXPECT_TRUE(Screen::GetScreen()->GetDisplayWithDisplayId(DEFAULT_DISPLAY_ID, &display));
...
TEST_F(ScreenTest, GetDisplayForNewWindows)
{
    ...
    EXPECT_EQ(screen->GetPrimaryDisplay().id(), screen->GetDisplayForNewWindows().id());
}
...
TEST_F(ScreenTest, GetScreenInfosNearestDisplay)
{
    ...
    // Nearest default
    {
        ScreenInfos screen_infos = screen->GetScreenInfosNearestDisplay(DEFAULT_DISPLAY_ID);
        ...
        EXPECT_EQ(screen->GetPrimaryDisplay().id(), screen_infos.current().display_id);
        EXPECT_TRUE(screen_infos.current().is_primary);
    }

    // Nearest display 2
    {
        ScreenInfos screen_infos = screen->GetScreenInfosNearestDisplay(DISPLAY_2_ID);
        ASSERT_EQ(screen_infos.screen_infos.size(), 2u);

        // Display 2 is current, but not the primary display.
        EXPECT_EQ(screen_infos.current().display_id, DISPLAY_2_ID);
        EXPECT_NE(screen->GetPrimaryDisplay().id(), screen_infos.current().display_id);
...
{
    Screen* screen = Screen::GetScreen();
    ScreenInfos screen_infos = screen->GetScreenInfosNearestDisplay(kInvalidDisplayId);
