...
int GetDisplayTreeDepth(int64_t display_id, int64_t primary_id, const std::map<int64_t, int64_t>& display_to_parent_ids_map)
{
...
bool AreDisplaysTouching(const Display& child_display, const Display& parent_display, DisplayPlacement::Position child_position)
{
    const gfx::Rect& a_bounds = child_display.bounds();
    const gfx::Rect& b_bounds = parent_display.bounds();

    if (child_position == DisplayPlacement::TOP || child_position == DisplayPlacement::BOTTOM) {
...
// IDs.
void UpdatePlacementList(Displays* display_list, std::vector<DisplayPlacement>* placement_list)
{
    std::sort(placement_list->begin(), placement_list->end(), ComparePlacements);

    for (DisplayPlacement& placement : *placement_list) {
        const Display* child_display = FindDisplayById(display_list, placement.display_id);
        const Display* parent_display = FindDisplayById(display_list, placement.parent_display_id);
        ...
        if (placement.position == DisplayPlacement::TOP || placement.position == DisplayPlacement::BOTTOM) {
...
// display.
void MaybeReparentTargetDisplay(int last_offset_x, int last_offset_y, const Display* last_intersecting_source_display, const Display* target_display,
    std::map<int64_t, int64_t>* display_to_parent_ids_map, Displays* display_list, std::vector<DisplayPlacement>* placement_list)
{
    ...
    if (iter != display_to_parent_ids_map->end()) {
        ...
        auto target_display_placement_itr = base::ranges::find(*placement_list, target_display->id(), &DisplayPlacement::display_id);
        CHECK(target_display_placement_itr != placement_list->end(), base::NotFatalUntil::M130);
        target_display_placement = &(*target_display_placement_itr);
        if (AreDisplaysTouching(*target_display, *parent_display, target_display_placement->position)) {
            return;
        }
    } else {
    ...
    // is called.
    target_display_placement->parent_display_id = last_intersecting_source_display->id();
    // Update the map.
    (*display_to_parent_ids_map)[target_display->id()] = last_intersecting_source_display->id();

    if (last_offset_x) {
        target_display_placement->position = last_offset_x > 0 ? DisplayPlacement::RIGHT : DisplayPlacement::LEFT;
    } else {
        target_display_placement->position = last_offset_y > 0 ? DisplayPlacement::BOTTOM : DisplayPlacement::TOP;
    }
}
...
int CalculateOffsetX(const gfx::Rect& source_bounds, const gfx::Rect& target_bounds)
{
...
int CalculateOffsetY(const gfx::Rect& source_bounds, const gfx::Rect& target_bounds)
{
...
void DeIntersectDisplays(int64_t primary_id, Displays* display_list, std::vector<DisplayPlacement>* placement_list, std::set<int64_t>* updated_displays)
{
    std::map<int64_t, int64_t> display_to_parent_ids_map;
    for (const DisplayPlacement& placement : *placement_list) {
        display_to_parent_ids_map.insert(std::make_pair(placement.display_id, placement.parent_display_id));
    }
    ...
    std::sort(sorted_displays.begin(), sorted_displays.end(), [&](Display* d1, Display* d2) {
        const int d1_depth = GetDisplayTreeDepth(d1->id(), primary_id, display_to_parent_ids_map);
        const int d2_depth = GetDisplayTreeDepth(d2->id(), primary_id, display_to_parent_ids_map);
    ...
    for (size_t i = 1; i < sorted_displays.size(); ++i) {
        ...
        MaybeReparentTargetDisplay(
            last_offset_x, last_offset_y, last_intersecting_source_display, target_display, &display_to_parent_ids_map, display_list, placement_list);
    }
...
// the vector.
bool IsPointOverRadiusVector(const gfx::Point& point, const gfx::Point& vector)
{
    ...
    // x * b >= y * a.
    return static_cast<int64_t>(point.x()) * static_cast<int64_t>(vector.y()) >= static_cast<int64_t>(point.y()) * static_cast<int64_t>(vector.x());
}
...
DisplayPlacement::DisplayPlacement()
    : DisplayPlacement(kInvalidDisplayId, kInvalidDisplayId, DisplayPlacement::RIGHT, 0, DisplayPlacement::TOP_LEFT)
{
}

DisplayPlacement::DisplayPlacement(Position position, int offset)
    : DisplayPlacement(kInvalidDisplayId, kInvalidDisplayId, position, offset, DisplayPlacement::TOP_LEFT)
{
}

DisplayPlacement::DisplayPlacement(Position position, int offset, OffsetReference offset_reference)
    : DisplayPlacement(kInvalidDisplayId, kInvalidDisplayId, position, offset, offset_reference)
{
}

DisplayPlacement::DisplayPlacement(int64_t display_id, int64_t parent_display_id, Position position, int offset, OffsetReference offset_reference)
...
DisplayPlacement& DisplayPlacement::operator=(const DisplayPlacement&) = default;

bool DisplayPlacement::operator==(const DisplayPlacement& other) const
{
    return display_id == other.display_id && parent_display_id == other.parent_display_id && position == other.position && offset == other.offset
...
// static
bool DisplayPlacement::StringToPosition(std::string_view string, Position* position)
{
    if (string == kTop) {
        *position = TOP;
        return true;
...
void DisplayLayout::ApplyToDisplayList(Displays* display_list, std::vector<int64_t>* updated_ids, int minimum_offset_overlap)
{
    if (placement_list.empty())
        return;

    if (!DisplayLayout::Validate(DisplayListToDisplayIdList(*display_list), *this)) {
    ...
    while (parents.size()) {
        ...
        for (const DisplayPlacement& placement : placement_list) {
            if (placement.parent_display_id == parent_id) {
                if (ApplyDisplayPlacement(placement, display_list, minimum_offset_overlap)) {
    ...
    DeIntersectDisplays(primary_id, display_list, &placement_list, &updated_displays);

    if (updated_ids) {
        updated_ids->insert(updated_ids->begin(), updated_displays.begin(), updated_displays.end());
    }
}
...
// static
bool DisplayLayout::Validate(const DisplayIdList& list, const DisplayLayout& layout)
{
    // The primary display should be in the list.
    if (!base::Contains(list, layout.primary_id)) {
        //DISPLAY_LOG(ERROR) << "The primary id: " << layout.primary_id << " is not in the id list.";
        return false;
    }
    ...
    for (const auto& placement : layout.placement_list) {
        ...
        if (prev_id >= (placement.display_id & 0xFF)) {
            //DISPLAY_LOG(ERROR) << "PlacementList must be sorted by first 8 bits of" << " display_id ";
            return false;
        }
        ...
        if (!base::Contains(list, placement.display_id)) {
            //DISPLAY_LOG(ERROR) << "display_id is not in the id list:" << placement.ToString();
            return false;
        }

        if (!base::Contains(list, placement.parent_display_id)) {
            //DISPLAY_LOG(ERROR) << "parent_display_id is not in the id list:" << placement.ToString();
            return false;
        }
        has_primary_as_parent |= layout.primary_id == placement.parent_display_id;
    }
//     if (!has_primary_as_parent)
//         DISPLAY_LOG(ERROR) << "At least, one placement must have the primary as a parent.";
...
{
    std::erase_if(placement_list, [&list](const DisplayPlacement& placement) { return base::Contains(list, placement.display_id); });
    for (DisplayPlacement& placement : placement_list) {
        if (base::Contains(list, placement.parent_display_id))
...
{
    const auto iter = base::ranges::find(placement_list, display_id, &DisplayPlacement::display_id);
    return (iter == placement_list.end()) ? DisplayPlacement() : DisplayPlacement(*iter);
}
...
// static
DisplayPlacement DisplayLayout::CreatePlacementForRectangles(const gfx::Rect& reference, const gfx::Rect& rectangle)
{
    ...
    // point).
    gfx::Point center(2 * (rectangle.x() - reference.x()) + rectangle.width(), 2 * (rectangle.y() - reference.y()) + rectangle.height());
    ...
    if (is_top_right) {
        position = is_bottom_right ? DisplayPlacement::RIGHT : DisplayPlacement::TOP;
    } else {
        position = is_bottom_right ? DisplayPlacement::BOTTOM : DisplayPlacement::LEFT;
    }
    ...
    int offset = (position == DisplayPlacement::LEFT || position == DisplayPlacement::RIGHT) ? rectangle.y() : rectangle.x();
    return DisplayPlacement(position, offset);
}

// static
bool DisplayLayout::ApplyDisplayPlacement(const DisplayPlacement& placement, Displays* display_list, int minimum_offset_overlap)
{
    const Display& parent_display = *FindDisplayById(display_list, placement.parent_display_id);
    ...
    if (position == DisplayPlacement::TOP || position == DisplayPlacement::BOTTOM) {
    ...
    target_display->set_bounds(gfx::Rect(new_target_origin, target_bounds.size()));
