...
const Display* FindDisplayNearestPoint(const Displays& displays, const gfx::Point& point)
{
...
const Display* FindDisplayWithBiggestIntersection(const Displays& displays, const gfx::Rect& rect)
{
    DCHECK(!displays.empty());
    int max_area = 0;
...
Displays::const_iterator FindDisplayContainingPoint(const Displays& displays, const gfx::Point& point_in_screen)
{
    return base::ranges::find_if(displays, [point_in_screen](const Display& display) { return display.bounds().Contains(point_in_screen); });
}
