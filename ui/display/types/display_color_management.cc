...
float EvaluateLut(float x, const std::vector<GammaRampRGBEntry>& lut, size_t channel)
{
...
GammaCurve::GammaCurve(GammaCurve&& other)
    ...
    , pre_curve_(std::move(other.pre_curve_))
{
}
...
void GammaCurve::Evaluate(float x, uint16_t& out_r, uint16_t& out_g, uint16_t& out_b) const
{
...
std::string GammaCurve::ToString() const
{
    ...
    for (size_t i = 0; i < lut_.size(); ++i) {
        str += base::StringPrintf("[%04x,%04x,%04x],", lut_[i].r, lut_[i].g, lut_[i].b);
    }
    if (pre_curve_) {
        str += ",after(";
...
std::string GammaCurve::ToActionString(const std::string& name) const
{
    ...
    std::string str = ",";
    for (size_t i = 0; i < lut_.size(); ++i) {
        str += base::StringPrintf("%s[%u]=%04x%04x%04x", name.c_str(), static_cast<uint32_t>(i), lut_[i].r, lut_[i].g, lut_[i].b);
        if (i != lut_.size() - 1) {
            str += ",";
        }
    }
