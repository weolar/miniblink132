...
using GetDisplaysCallback = base::OnceCallback<void(const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>&)>;
using ConfigureCallback = base::OnceCallback<void(const std::vector<DisplayConfigurationParams>& request_results, bool status)>;
using SetHdcpKeyPropCallback = base::OnceCallback<void(bool)>;
using GetHDCPStateCallback = base::OnceCallback<void(bool, HDCPState, ContentProtectionMethod)>;
...
public:
    ...
    virtual void SetHdcpKeyProp(int64_t display_id, const std::string& key, SetHdcpKeyPropCallback callback) = 0;

    // Gets HDCP state of output.
    virtual void GetHDCPState(const DisplaySnapshot& output, GetHDCPStateCallback callback) = 0;

    // Sets HDCP state of output.
    virtual void SetHDCPState(const DisplaySnapshot& output, HDCPState state, ContentProtectionMethod protection_method, SetHDCPStateCallback callback) = 0;

    // Sets the color temperature adjustment (e.g, for night light) for the
    // specified display.
    virtual void SetColorTemperatureAdjustment(int64_t display_id, const ColorTemperatureAdjustment& cta) = 0;

    // Sets the color calibration for the specified display.
    virtual void SetColorCalibration(int64_t display_id, const ColorCalibration& calibration) = 0;

    // Sets the display profile space gamma adjustment for the specified display.
    virtual void SetGammaAdjustment(int64_t display_id, const GammaAdjustment& gamma) = 0;

    // Sets the privacy screen state on the display with |display_id|.
    virtual void SetPrivacyScreen(int64_t display_id, bool enabled, SetPrivacyScreenCallback callback) = 0;

    // Get a description of supported seamless refresh rates for the display with
    // |display_id|.
    virtual void GetSeamlessRefreshRates(int64_t display_id, GetSeamlessRefreshRatesCallback callback) const = 0;
