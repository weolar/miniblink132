...
std::string ModeListString(const std::vector<std::unique_ptr<const DisplayMode>>& modes)
{
...
DisplaySnapshot::DisplaySnapshot(int64_t display_id, int64_t port_display_id, int64_t edid_display_id, uint16_t connector_index, const gfx::Point& origin,
    const gfx::Size& physical_size, DisplayConnectionType type, uint64_t base_connector_id, const std::vector<uint64_t>& path_topology,
    bool is_aspect_preserving_scaling, bool has_overscan, PrivacyScreenState privacy_screen_state, bool has_content_protection_key, const ColorInfo& color_info,
    std::string display_name, const base::FilePath& sys_path, DisplayModeList modes, PanelOrientation panel_orientation, const std::vector<uint8_t>& edid,
    const DisplayMode* current_mode, const DisplayMode* native_mode, int64_t product_code, int32_t year_of_manufacture, const gfx::Size& maximum_cursor_size,
    ...
    , drm_formats_and_modifiers_(drm_formats_and_modifiers)
{
    ...
    const size_t end = std::min(kSerialNumberBeginningByte + kSerialNumberLengthInBytes, edid_.size());
...
std::unique_ptr<DisplaySnapshot> DisplaySnapshot::Clone() const
{
    ...
    auto clone = std::make_unique<DisplaySnapshot>(display_id_, port_display_id_, edid_display_id_, connector_index_, origin_, physical_size_, type_,
        base_connector_id_, path_topology_, is_aspect_preserving_scaling_, has_overscan_, privacy_screen_state_, has_content_protection_key_, color_info_,
        display_name_, sys_path_, std::move(clone_modes), panel_orientation_, edid_, cloned_current_mode, cloned_native_mode, product_code_,
        year_of_manufacture_, maximum_cursor_size_, variable_refresh_rate_state_, drm_formats_and_modifiers_);
...
std::string DisplaySnapshot::ToString() const
{
    ...
    return base::StringPrintf("id=%" PRId64 " current_mode=%s native_mode=%s origin=%s"
                              " panel_orientation=%d"
                              " physical_size=%s, type=%s sharing_base_connector=%s name=\"%s\" "
                              "(year:%d) modes=(%s)",
        display_id(), current_mode_ ? current_mode_->ToString().c_str() : "nullptr", native_mode_ ? native_mode_->ToString().c_str() : "nullptr",
        origin_.ToString().c_str(), panel_orientation_, physical_size_.ToString().c_str(), DisplayConnectionTypeString(type_).c_str(),
        sharing_connector.c_str(), display_name_.c_str(), year_of_manufacture_, ModeListString(modes_).c_str());
}
...
{
    return variable_refresh_rate_state_ != VariableRefreshRateState::kVrrNotCapable;
}
...
{
    if (current_mode_ == mode || (current_mode_ && mode && *current_mode_ == *mode)) {
        return;
    }
    ...
    VLOG(3) << "Encountered mode which does not natively belong to display: " << mode->ToString();
