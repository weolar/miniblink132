...
float GetForcedDeviceScaleFactorImpl()
{
    ...
    if (HasForceDeviceScaleFactorImpl()) {
        std::string value = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kForceDeviceScaleFactor);
...
void Display::SetForceDeviceScaleFactor(double dsf)
{
    ...
    base::CommandLine::ForCurrentProcess()->AppendSwitchASCII(switches::kForceDeviceScaleFactor, base::StringPrintf("%.2f", dsf));
}
...
gfx::ColorSpace Display::GetForcedRasterColorProfile()
{
    ...
    std::string value = base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kForceRasterColorProfile);
...
{
    return base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kForceRasterColorProfile);
}

// static
bool Display::HasEnsureForcedColorProfile()
{
    static bool has_ensure_forced_color_profile = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnsureForcedColorProfile);
...
gfx::Insets Display::GetWorkAreaInsets() const
{
    return gfx::Insets::TLBR(
        work_area_.y() - bounds_.y(), work_area_.x() - bounds_.x(), bounds_.bottom() - work_area_.bottom(), bounds_.right() - work_area_.right());
}
...
{
    return base::StringPrintf("Display[%lld] bounds=[%s], workarea=[%s], scale=%g, rotation=%s, "
                              "panel_rotation=%s %s %s",
        static_cast<long long int>(id_), bounds_.ToString().c_str(), work_area_.ToString().c_str(), device_scale_factor_, ToRotationString(rotation_),
        ToRotationString(panel_rotation()), IsInternal() ? "internal" : "external", detected() ? "detected" : "not-detected");
}
...
{
    return EqualExceptForHdrHeadroom(*this, rhs) && (color_spaces_ == rhs.color_spaces_ || GetColorSpaces() == rhs.GetColorSpaces());
}

// static
bool Display::EqualExceptForHdrHeadroom(const Display& lhs, const Display& rhs)
{
    return lhs.id_ == rhs.id_ && lhs.bounds_ == rhs.bounds_ && lhs.size_in_pixels_ == rhs.size_in_pixels_ && lhs.native_origin_ == rhs.native_origin_
        && lhs.detected_ == rhs.detected_ && lhs.work_area_ == rhs.work_area_ && lhs.device_scale_factor_ == rhs.device_scale_factor_
        && lhs.rotation_ == rhs.rotation_ && lhs.touch_support_ == rhs.touch_support_ && lhs.accelerometer_support_ == rhs.accelerometer_support_
        && lhs.maximum_cursor_size_ == rhs.maximum_cursor_size_
        && (lhs.color_spaces_ == rhs.color_spaces_ || gfx::DisplayColorSpaces::EqualExceptForHdrHeadroom(lhs.GetColorSpaces(), rhs.GetColorSpaces()))
        && lhs.color_depth_ == rhs.color_depth_ && lhs.depth_per_component_ == rhs.depth_per_component_ && lhs.is_monochrome_ == rhs.is_monochrome_
        && lhs.display_frequency_ == rhs.display_frequency_ && lhs.label_ == rhs.label_;
}

void Display::SetDisplayColorSpacesRef(scoped_refptr<const DisplayColorSpacesRef> color_spaces)
{
...
scoped_refptr<const Display::DisplayColorSpacesRef> Display::GetDefaultDisplayColorSpacesRef()
{
    ...
    // GPU process.
    static const base::NoDestructor<scoped_refptr<const DisplayColorSpacesRef>> default_color_spaces_ref([] {
