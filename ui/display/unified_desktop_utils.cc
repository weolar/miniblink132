...
bool ValidateDisplayGraph(const DisplayChildToParentMap& child_to_parent, int64_t primary_id)
{
...
UnifiedDesktopLayoutMatrix BuildDisplayMatrix(const DisplayLayout& layout)
{
    ...
    for (const auto& placement : layout.placement_list) {
        ...
        while (displays_cells.count(current_display_id) == 0) {
            auto placement_iter = base::ranges::find(layout.placement_list, current_display_id, &DisplayPlacement::display_id);
            CHECK(placement_iter != layout.placement_list.end(), base::NotFatalUntil::M130);
        ...
        while (!unhandled_displays.empty()) {
            ...
            const Cell& parent_cell = displays_cells.at(current_placement.parent_display_id);
            ...
            case DisplayPlacement::TOP:
                ...
                new_cell_itr = displays_cells.emplace(current_placement.display_id, Cell(parent_cell.row - 1, parent_cell.column)).first;
                break;

            case DisplayPlacement::RIGHT:
                // Right of its parent. Go right a column (column + 1).
                new_cell_itr = displays_cells.emplace(current_placement.display_id, Cell(parent_cell.row, parent_cell.column + 1)).first;
                break;
...
bool BuildUnifiedDesktopMatrix(const DisplayIdList& ids_list, const DisplayLayout& layout, UnifiedDesktopLayoutMatrix* out_matrix)
{
    // The primary display should be in the IDs list.
    if (!base::Contains(ids_list, layout.primary_id)) {
        LOG(ERROR) << "The primary ID: " << layout.primary_id << " is not in the IDs list.";
        return false;
    }
    ...
    for (const auto& id : ids_list) {
        if (id == layout.primary_id)
            continue;
        if (!base::Contains(layout.placement_list, id, &DisplayPlacement::display_id)) {
    ...
    for (const auto& placement : layout.placement_list) {
        ...
        if (!base::Contains(ids_list, placement.display_id)) {
            LOG(ERROR) << "display_id: " << placement.display_id << " is not in the id list: " << placement.ToString();
            return false;
        }

        if (!base::Contains(ids_list, placement.parent_display_id)) {
            LOG(ERROR) << "parent_display_id: " << placement.parent_display_id << " is not in the id list: " << placement.ToString();
            return false;
        }

        if (!displays_filled_sides[placement.parent_display_id].emplace(placement.position).second) {
            LOG(ERROR) << "Parent display with ID: " << placement.parent_display_id << " has more than one display on the same side: " << placement.position;
            return false;
        }

        if (!child_to_parent.emplace(placement.display_id, placement.parent_display_id).second) {
