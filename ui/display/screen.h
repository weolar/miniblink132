...
public:
    ...
    static Screen* SetScreenInstance(Screen* instance, const base::Location& location = FROM_HERE);
    ...
    virtual gfx::NativeWindow GetLocalProcessWindowAtPoint(const gfx::Point& point, const std::set<gfx::NativeWindow>& ignore) = 0;
    ...
    // context.
    virtual gfx::Rect ScreenToDIPRectInWindow(gfx::NativeWindow window, const gfx::Rect& screen_rect) const;

    // Converts |dip_rect| to screen coordinates in the context of |window|
    // clamping to the enclosing rect if the coordinates do not fall on pixel
    // boundaries. If |window| is null, the primary display is used as the
    // context.
    virtual gfx::Rect DIPToScreenRectInWindow(gfx::NativeWindow window, const gfx::Rect& dip_rect) const;
    ...
    virtual void SetPanelRotationForTesting(int64_t display_id, Display::Rotation rotation);
    ...
    virtual std::optional<float> GetPreferredScaleFactorForWindow(gfx::NativeWindow window) const;
    virtual std::optional<float> GetPreferredScaleFactorForView(gfx::NativeView view) const;
