...
constexpr unsigned char kBadDisplayName[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x22\xf0\x6c\x28\x01\x01\x01\x01"
...
constexpr unsigned char kNormalDisplay[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x22\xf0\x6c\x28\x01\x01\x01\x01"
                                           "\x02\x16\x01\x04\xb5\x40\x28\x78\xe2\x8d\x85\xad\x4f\x35\xb1\x25"
                                           "\x0e\x50\x54\x00\x00\x00\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01"
                                           "\x01\x01\x01\x01\x01\x01\xe2\x68\x00\xa0\xa0\x40\x2e\x60\x30\x20"
                                           "\x36\x00\x81\x90\x21\x00\x00\x1a\xbc\x1b\x00\xa0\x50\x20\x17\x30"
...
constexpr unsigned char kNoMaxImageSizeDisplay[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x22\xf0\x6c\x28\x01\x01\x01\x01"
                                                   ...
                                                   "\x50\x20\x5a\x52\x33\x30\x77\x0a\x20\x20\x20\x20\x00\x00\x00\xff"
                                                   "\x00\x43\x4e\x34\x32\x30\x32\x31\x33\x37\x51\x0a\x20\x20\x00\x71";
constexpr size_t kNoMaxImageSizeDisplayLength = std::size(kNoMaxImageSizeDisplay);
...
constexpr unsigned char kBlockZeroSerialNumberOnlyDisplay[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x22\xf0\x6c\x28\x01\x01\x01\x01"
                                                              "\x02\x16\x01\x04\xb5\x40\x28\x78\xe2\x8d\x85\xad\x4f\x35\xb1\x25"
                                                              "\x0e\x50\x54\x00\x00\x00\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01"
                                                              "\x01\x01\x01\x01\x01\x01\xe2\x68\x00\xa0\xa0\x40\x2e\x60\x30\x20"
                                                              "\x36\x00\x81\x90\x21\x00\x00\x1a\xbc\x1b\x00\xa0\x50\x20\x17\x30"
...
constexpr size_t kBlockZeroSerialNumberOnlyDisplayLength = std::size(kBlockZeroSerialNumberOnlyDisplay);
...
// (0x00 at byte 16).
constexpr unsigned char kNoWeekOfManufactureDisplay[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x22\xf0\x6c\x28\x01\x01\x01\x01"
                                                        ...
                                                        "\x50\x20\x5a\x52\x33\x30\x77\x0a\x20\x20\x20\x20\x00\x00\x00\xff"
                                                        "\x00\x43\x4e\x34\x32\x30\x32\x31\x33\x37\x51\x0a\x20\x20\x00\x71";
constexpr size_t kNoWeekOfManufactureDisplayLength = std::size(kNoWeekOfManufactureDisplay);

// Week of manufacture can be used to signal that year of manufacture is the
// model year by setting byte 16 to 0xff.
constexpr unsigned char kModelYearDisplay[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x22\xf0\x6c\x28\x01\x01\x01\x01"
...
constexpr unsigned char kOverscanDisplay[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x4c\x2d\xfe\x08\x00\x00\x00\x00"
...
constexpr unsigned char kMisdetectedDisplay[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x10\xac\x64\x40\x4c\x30\x30\x32"
...
constexpr unsigned char kLP2565A[] = "\x00\xFF\xFF\xFF\xFF\xFF\xFF\x00\x22\xF0\x76\x26\x01\x01\x01\x01"
...
constexpr unsigned char kLP2565B[] = "\x00\xFF\xFF\xFF\xFF\xFF\xFF\x00\x22\xF0\x75\x26\x01\x01\x01\x01"
...
constexpr unsigned char kSamus[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x30\xe4\x2e\x04\x00\x00\x00\x00"
...
constexpr unsigned char kEve[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x4d\x10\x8a\x14\x00\x00\x00\x00"
...
constexpr unsigned char kHDRMetadata[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x4c\x2d\xf6\x0d\x00\x0e\x00\x01"
...
// 16843009 == 0x01010101
const std::string kGenericBlockZeroHashedSerialNumber = base::MD5String(std::string("16843009"));
const std::string kNormalDisplayHashedDescriptorBlockSerialNumber = base::MD5String(std::string("CN4202137Q"));

// Primaries coordinates ({RX, RY, GX, GY, BX, BY, WX, WY}) calculated by hand
// and rounded to 4 decimal places.
constexpr SkColorSpacePrimaries kNormalDisplayPrimaries = { 0.6777f, 0.3086f, 0.2100f, 0.6924f, 0.1465f, 0.0547f, 0.3135f, 0.3291f };
constexpr SkColorSpacePrimaries kInternalDisplayPrimaries = { 0.5850f, 0.3604f, 0.3750f, 0.5654f, 0.1553f, 0.0996f, 0.3135f, 0.3291f };
constexpr SkColorSpacePrimaries kOverscanDisplayPrimaries = { 0.6396f, 0.3301f, 0.2998f, 0.5996f, 0.1504f, 0.0596f, 0.3125f, 0.3291f };
constexpr SkColorSpacePrimaries kMisdetectedDisplayPrimaries = { 0.6777f, 0.3086f, 0.2100f, 0.6924f, 0.1465f, 0.0547f, 0.3135f, 0.3291f };
constexpr SkColorSpacePrimaries kLP2565APrimaries = { 0.6396f, 0.3301f, 0.2998f, 0.6084f, 0.1504f, 0.0596f, 0.3135f, 0.3291f };
constexpr SkColorSpacePrimaries kLP2565BPrimaries = { 0.6396f, 0.3301f, 0.2998f, 0.6084f, 0.1504f, 0.0596f, 0.3135f, 0.3291f };
constexpr SkColorSpacePrimaries kHPz32xPrimaries = { 0.6738f, 0.3164f, 0.1982f, 0.7197f, 0.1484f, 0.0439f, 0.3135f, 0.3291f };
constexpr SkColorSpacePrimaries kSamusPrimaries = { 0.6338f, 0.3477f, 0.3232f, 0.5771f, 0.1514f, 0.0908f, 0.3135f, 0.3291f };
constexpr SkColorSpacePrimaries kEvePrimaries = { 0.6396f, 0.3291f, 0.2998f, 0.5996f, 0.1494f, 0.0596f, 0.3125f, 0.3281f };
constexpr SkColorSpacePrimaries kHDRPrimaries = { 0.6406f, 0.3300f, 0.3007f, 0.6005f, 0.1503f, 0.0605f, 0.2802f, 0.2900f };
constexpr SkColorSpacePrimaries kDellTiledPrimaries = { 0.6807f, 0.3193f, 0.2002f, 0.7285f, 0.1494f, 0.0508f, 0.3134f, 0.3291f };
...
::testing::AssertionResult SkColorSpacePrimariesEquals(
    const char* lhs_expr, const char* rhs_expr, const SkColorSpacePrimaries& lhs, const SkColorSpacePrimaries& rhs)
{
...
struct TestParams {
    ...
    base::flat_set<EdidParser::PrimaryMatrixPair> supported_color_primary_matrix_ids;
    ...
    { .test_name = "BadDisplayName",
        ...
        .descriptor_block_serial_number_hash = kNormalDisplayHashedDescriptorBlockSerialNumber,
    ...
    { .test_name = "MisdetectedDisplay",
        ...
        .product_id = 0x6440u,
        .block_zero_serial_number_hash = base::MD5String("842018892"), // == LSB of 0x4c, 0x30, 0x30, 0x32
        ...
        .supported_color_primary_matrix_ids = { { gfx::ColorSpace::PrimaryID::BT709, gfx::ColorSpace::MatrixID::BT709 },
    ...
    { .test_name = "HDRMetadata",
        ...
        .product_id = 62989u,
        .block_zero_serial_number_hash = base::MD5String("16780800"), // == LSB of 0x00, 0x0e, 0x00 0x01
        .descriptor_block_serial_number_hash = kNoSerialNumber,
        ...
        .supported_color_transfer_ids = { gfx::ColorSpace::TransferID::BT709, gfx::ColorSpace::TransferID::PQ, gfx::ColorSpace::TransferID::HLG },
        .hdr_static_metadata = std::make_optional<gfx::HDRStaticMetadata>(603.666, 530.095, 0.00454,
...
public:
    EDIDParserTest()
        : parser_(std::vector<uint8_t>(GetParam().edid_blob, GetParam().edid_blob + GetParam().edid_blob_length))
    {
    }
...
{
    std::vector<uint8_t> expected_edid(GetParam().edid_blob, GetParam().edid_blob + GetParam().edid_blob_length);
    ...
    EXPECT_EQ(parser_.block_zero_serial_number_hash(), GetParam().block_zero_serial_number_hash);
    EXPECT_EQ(parser_.descriptor_block_serial_number_hash(), GetParam().descriptor_block_serial_number_hash);
    ...
    EXPECT_PRED_FORMAT2(SkColorSpacePrimariesEquals, parser_.primaries(), GetParam().primaries);

    EXPECT_EQ(parser_.GetProductCode(), GetParam().product_code);
    EXPECT_EQ(parser_.GetIndexBasedDisplayId(0 /* product_index */), GetParam().index_based_display_id_zero);
    EXPECT_EQ(parser_.GetEdidBasedDisplayId(), GetParam().edid_based_display_id);

    EXPECT_EQ(EdidParser::ManufacturerIdToString(parser_.manufacturer_id()), GetParam().manufacturer_id_string);
    EXPECT_EQ(EdidParser::ProductIdToString(parser_.product_id()), GetParam().product_id_string);

    EXPECT_EQ(GetParam().supported_color_primary_matrix_ids, parser_.supported_color_primary_matrix_ids());
    EXPECT_EQ(GetParam().supported_color_transfer_ids, parser_.supported_color_transfer_ids());

    const std::optional<gfx::HDRStaticMetadata> hdr_static_metadata = parser_.hdr_static_metadata();
    EXPECT_EQ(GetParam().hdr_static_metadata.has_value(), hdr_static_metadata.has_value());
    if (GetParam().hdr_static_metadata.has_value() && hdr_static_metadata.has_value()) {
        constexpr double epsilon = 0.001;
        EXPECT_NEAR(GetParam().hdr_static_metadata->max, hdr_static_metadata->max, epsilon);
        EXPECT_NEAR(GetParam().hdr_static_metadata->max_avg, hdr_static_metadata->max_avg, epsilon);
        EXPECT_NEAR(GetParam().hdr_static_metadata->min, hdr_static_metadata->min, epsilon);
    }
...
INSTANTIATE_TEST_SUITE_P(
    All, EDIDParserTest, ValuesIn(kTestCases), [](const testing::TestParamInfo<EDIDParserTest::ParamType>& info) { return info.param.test_name; });
