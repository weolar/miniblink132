...
// HP z32x monitor.
const unsigned char kHPz32x[] = "\x00\xFF\xFF\xFF\xFF\xFF\xFF\x00\x22\xF0\x75\x32\x01\x01\x01\x01"
...
const unsigned char kSamus[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x30\xe4\x2e\x04\x00\x00\x00\x00"
...
const unsigned char kEve[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x4d\x10\x8a\x14\x00\x00\x00\x00"
...
const unsigned char kInvalidEdid[] = "\x00\xFF\xFF\xFF\xFF\xFF\xFF\x00\x22\xF0\x76\x26\x01\x01\x01\x01"
                                     "\x02\x12\x01\x03\x80\x34\x21";

// Partially valid EDID: gamma information is marked as non existent.
const unsigned char kEdidWithNoGamma[] = "\x00\xFF\xFF\xFF\xFF\xFF\xFF\x00\x22\xF0\x76\x26\x01\x01\x01\x01"
...
const unsigned char kScreeboP3[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x09\xe5\x1b\x0c\x00\x00\x00\x00"
...
const unsigned char kCDBP3[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x06\xb3\xda\x16\x5e\x42\x02\x00"
...
constexpr unsigned char kHDR[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x4c\x83\x42\x41\x00\x00\x00\x00"
...
const unsigned char kSST210[] = "\xff\x00\xff\xff\xff\xff\x00\xff\x2d\x4c\x42\x52\x32\x31\x50\x43"
...
const unsigned char kSST210Corrected[] = "\xff\x00\xff\xff\xff\xff\x00\xff\x2d\x4c\x42\x52\x32\x31\x50\x43"
...
const unsigned char kBrokenBluePrimaries[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x4c\x83\x4d\x83\x00\x00\x00\x00"
...
const unsigned char kDTSAudio[] = "\x00\xff\xff\xff\xff\xff\xff\x00\x59\x3a\x39\x10\x01\x01\x01\x01"
...
TEST(DisplayUtilTest, GetColorSpaceFromEdid)
{
    ...
    constexpr SkColorSpacePrimaries expected_hpz32x_primaries
        = { .fRX = 0.673828f, .fRY = 0.316406f, .fGX = 0.198242f, .fGY = 0.719727f, .fBX = 0.148438f, .fBY = 0.043945f, .fWX = 0.313477f, .fWY = 0.329102f };
    ...
    const gfx::ColorSpace expected_hpz32x_color_space
        = gfx::ColorSpace::CreateCustom(expected_hpz32x_toXYZ50_matrix, skcms_TransferFunction({ 2.2, 1, 0, 0, 0, 0, 0 }));
    EXPECT_EQ(expected_hpz32x_color_space.ToString(), GetColorSpaceFromEdid(display::EdidParser(std::move(hpz32x_edid))).ToString());
    histogram_tester.ExpectBucketCount(
        "DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", static_cast<base::HistogramBase::Sample>(EdidColorSpaceChecksOutcome::kSuccess), 1);

    // Test with Chromebook Samus internal display.
    constexpr SkColorSpacePrimaries expected_samus_primaries
        = { .fRX = 0.633789f, .fRY = 0.347656f, .fGX = 0.323242f, .fGY = 0.577148f, .fBX = 0.151367f, .fBY = 0.090820f, .fWX = 0.313477f, .fWY = 0.329102f };
    ...
    const gfx::ColorSpace expected_samus_color_space
        = gfx::ColorSpace::CreateCustom(expected_samus_toXYZ50_matrix, skcms_TransferFunction({ 2.5, 1, 0, 0, 0, 0, 0 }));
    EXPECT_EQ(expected_samus_color_space.ToString(), GetColorSpaceFromEdid(display::EdidParser(std::move(samus_edid))).ToString());
    histogram_tester.ExpectBucketCount(
        "DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", static_cast<base::HistogramBase::Sample>(EdidColorSpaceChecksOutcome::kSuccess), 2);
    ...
    const gfx::ColorSpace expected_eve_color_space(gfx::ColorSpace::PrimaryID::BT709, gfx::ColorSpace::TransferID::CUSTOM, gfx::ColorSpace::MatrixID::RGB,
        gfx::ColorSpace::RangeID::FULL,
        /*custom_primary_matrix=*/nullptr, &eve_transfer);
    std::vector<uint8_t> eve_edid(kEve, kEve + std::size(kEve) - 1);
    EXPECT_EQ(expected_eve_color_space.ToString(), GetColorSpaceFromEdid(display::EdidParser(std::move(eve_edid))).ToString());
    histogram_tester.ExpectBucketCount(
        "DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", static_cast<base::HistogramBase::Sample>(EdidColorSpaceChecksOutcome::kSuccess), 3);

    // Test with a display that supports HDR: Chromebook Samsung Galaxy (kohaku).
    constexpr SkColorSpacePrimaries expected_hdr_primaries
        = { .fRX = 0.67970f, .fRY = 0.31930f, .fGX = 0.23240f, .fGY = 0.71870f, .fBX = 0.13965f, .fBY = 0.04390f, .fWX = 0.31250f, .fWY = 0.32910f };
    ...
    const gfx::ColorSpace expected_hdr_color_space = gfx::ColorSpace::CreateCustom(expected_hdr_toXYZ50_matrix, gfx::ColorSpace::TransferID::PQ);
    EXPECT_TRUE(expected_hdr_color_space.IsHDR());
    EXPECT_EQ(expected_hdr_color_space.ToString(), GetColorSpaceFromEdid(display::EdidParser(std::move(hdr_edid))).ToString());
    histogram_tester.ExpectBucketCount(
        "DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", static_cast<base::HistogramBase::Sample>(EdidColorSpaceChecksOutcome::kSuccess), 4);

    // Test with gamma marked as non-existent.
    std::vector<uint8_t> no_gamma_edid(kEdidWithNoGamma, kEdidWithNoGamma + std::size(kEdidWithNoGamma) - 1);
    const gfx::ColorSpace no_gamma_color_space = GetColorSpaceFromEdid(display::EdidParser(std::move(no_gamma_edid)));
    EXPECT_FALSE(no_gamma_color_space.IsValid());
    histogram_tester.ExpectBucketCount(
        "DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", static_cast<base::HistogramBase::Sample>(EdidColorSpaceChecksOutcome::kErrorBadGamma), 1);
    histogram_tester.ExpectTotalCount("DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", 5);

    // Test with a display with primaries near P3, but {BT2020,RGB} in its CDB.
    constexpr SkColorSpacePrimaries expected_p3_primaries
        = { .fRX = 0.680f, .fRY = 0.320f, .fGX = 0.265f, .fGY = 0.690f, .fBX = 0.150f, .fBY = 0.060f, .fWX = 0.3127, .fWY = 0.3290 };
    skcms_Matrix3x3 expected_p3_to_XYZ50_matrix;
    expected_p3_primaries.toXYZD50(&expected_p3_to_XYZ50_matrix);
    std::vector<uint8_t> p3_edid(kScreeboP3, kScreeboP3 + std::size(kScreeboP3) - 1);
    const gfx::ColorSpace expected_p3_color_space = gfx::ColorSpace::CreateCustom(expected_p3_to_XYZ50_matrix, gfx::ColorSpace::TransferID::PQ);
    EXPECT_TRUE(expected_p3_color_space.IsWide());
    // Expect P3 primaries and PQ transfer function.
    EXPECT_EQ(expected_p3_color_space.ToString(), GetColorSpaceFromEdid(display::EdidParser(std::move(p3_edid))).ToString());
    ...
    EXPECT_EQ(gfx::ColorSpace::CreateDisplayP3D65().ToString(), GetColorSpaceFromEdid(display::EdidParser(std::move(p3_cdb_edid), true)).ToString());
}
...
TEST(DisplayUtilTest, GetInvalidColorSpaceFromEdid)
{
    ...
    EXPECT_EQ(gfx::ColorSpace(), GetColorSpaceFromEdid(display::EdidParser(std::move(empty_edid))));
    histogram_tester.ExpectBucketCount(
        "DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", static_cast<base::HistogramBase::Sample>(EdidColorSpaceChecksOutcome::kErrorPrimariesAreaTooSmall), 1);

    std::vector<uint8_t> invalid_edid(kInvalidEdid, kInvalidEdid + std::size(kInvalidEdid) - 1);
    const gfx::ColorSpace invalid_color_space = GetColorSpaceFromEdid(display::EdidParser(std::move(invalid_edid)));
    EXPECT_FALSE(invalid_color_space.IsValid());
    histogram_tester.ExpectBucketCount(
        "DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", static_cast<base::HistogramBase::Sample>(EdidColorSpaceChecksOutcome::kErrorPrimariesAreaTooSmall), 2);

    std::vector<uint8_t> sst210_edid(kSST210, kSST210 + std::size(kSST210) - 1);
    const gfx::ColorSpace sst210_color_space = GetColorSpaceFromEdid(display::EdidParser(std::move(sst210_edid)));
    EXPECT_FALSE(sst210_color_space.IsValid()) << sst210_color_space.ToString();
    histogram_tester.ExpectBucketCount(
        "DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", static_cast<base::HistogramBase::Sample>(EdidColorSpaceChecksOutcome::kErrorBadCoordinates), 1);

    std::vector<uint8_t> sst210_edid_2(kSST210Corrected, kSST210Corrected + std::size(kSST210Corrected) - 1);
    const gfx::ColorSpace sst210_color_space_2 = GetColorSpaceFromEdid(display::EdidParser(std::move(sst210_edid_2)));
    EXPECT_FALSE(sst210_color_space_2.IsValid()) << sst210_color_space_2.ToString();
    histogram_tester.ExpectBucketCount(
        "DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", static_cast<base::HistogramBase::Sample>(EdidColorSpaceChecksOutcome::kErrorPrimariesAreaTooSmall), 3);

    std::vector<uint8_t> broken_blue_edid(kBrokenBluePrimaries, kBrokenBluePrimaries + std::size(kBrokenBluePrimaries) - 1);
    const gfx::ColorSpace broken_blue_color_space = GetColorSpaceFromEdid(display::EdidParser(std::move(broken_blue_edid)));
    EXPECT_FALSE(broken_blue_color_space.IsValid()) << broken_blue_color_space.ToString();
    histogram_tester.ExpectBucketCount(
        "DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", static_cast<base::HistogramBase::Sample>(EdidColorSpaceChecksOutcome::kErrorBluePrimaryIsBroken), 1);
    histogram_tester.ExpectTotalCount("DrmUtil.GetColorSpaceFromEdid.ChecksOutcome", 5);
}

TEST(DisplayUtilTest, GetAudioPassthroughFromEdid)
{
    std::vector<uint8_t> audio_edid(kDTSAudio, kDTSAudio + std::size(kDTSAudio) - 1);
    EXPECT_EQ(display::EdidParser(std::move(audio_edid)).audio_formats(),
        display::EdidParser::kAudioBitstreamPcmLinear | display::EdidParser::kAudioBitstreamDts | display::EdidParser::kAudioBitstreamDtsHd);
}
...
TEST(DisplayUtilTest, MultipleInternalDisplayIds)
{
    ...
    const base::flat_set<int64_t> kTestIdsList[] = {
        ...
        base::flat_set<int64_t>({ 7, 10, 100, std::numeric_limits<int64_t>::max() }),
    };
