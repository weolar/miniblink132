...
// value (like 0).
DISPLAY_UTIL_EXPORT int64_t GenerateDisplayID(uint16_t manufacturer_id, uint32_t product_code_hash, uint8_t output_index);
...
DISPLAY_UTIL_EXPORT gfx::ColorSpace GetColorSpaceFromEdid(const display::EdidParser& edid_parser);
...
DISPLAY_UTIL_EXPORT void SetInternalDisplayIds(base::flat_set<int64_t> display_ids);

// Converts the color string name into a gfx::ColorSpace profile.
DISPLAY_UTIL_EXPORT gfx::ColorSpace ForcedColorProfileStringToColorSpace(const std::string& value);
...
DISPLAY_UTIL_EXPORT uint16_t ConnectorIndex16(uint8_t device_index, uint8_t display_index);
