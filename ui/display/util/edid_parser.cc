...
constexpr char kParseExternalDisplayEdidOptionalsMetric[] = "Display.External.ParseEdidOptionals";
constexpr char kBlockZeroSerialNumberTypeMetric[] = "Display.External.BlockZeroSerialNumberType";
constexpr char kNumOfSerialNumbersProvidedByExternalDisplay[] = "Display.External.NumOfSerialNumbersProvided";
...
BlockZeroSerialNumberType GetSerialNumberType(base::span<const uint8_t, 4u> serial_number)
{
...
{
    return ((static_cast<uint32_t>(manufacturer_id_) << 16) | (static_cast<uint32_t>(product_id_)));
}
...
int64_t EdidParser::GetIndexBasedDisplayId(uint8_t output_index) const
{
    ...
    const uint32_t product_code_hash = display_name_.empty() ? 0 : base::Hash(display_name_);
...
{
    const std::string string_to_hash = base::NumberToString(manufacturer_id_) + base::NumberToString(product_id_) + display_name_
        + base::NumberToString(week_of_manufacture()) + base::NumberToString(year_of_manufacture_) + max_image_size().ToString()
...
// static
void EdidParser::SplitProductCodeInManufacturerIdAndProductId(int64_t product_code, uint16_t* manufacturer_id, uint16_t* product_id)
{
...
void EdidParser::ParseEdid(const std::vector<uint8_t>& edid)
{
    ...
    if (edid.size() < kManufacturerOffset + kManufacturerLength) {
        base::UmaHistogramEnumeration(kParseEdidFailureMetric, ParseEdidFailure::kManufacturerId);
    ...
    manufacturer_id_ = base::numerics::U16FromBigEndian(base::span(edid).subspan<kManufacturerOffset, kManufacturerLength>());

    if (edid.size() < kProductIdOffset + kProductIdLength) {
        base::UmaHistogramEnumeration(kParseEdidFailureMetric, ParseEdidFailure::kProductId);
        return; // Any other fields below are beyond this edid offset.
    }
    ...
    product_id_ = base::numerics::U16FromBigEndian(base::span(edid).subspan<kProductIdOffset, kProductIdLength>());
    ...
    if (edid.size() < kSerialNumberOffset + kSerialNumberLength) {
        base::UmaHistogramEnumeration(kParseEdidFailureMetric, ParseEdidFailure::kSerialNumber);
        return; // Any other fields below are beyond this edid offset.
    }

    auto serial_number_bytes = base::span(edid).subspan<kSerialNumberOffset, kSerialNumberLength>();
    ...
    if (is_external_display_) {
        base::UmaHistogramEnumeration(kBlockZeroSerialNumberTypeMetric, GetSerialNumberType(serial_number_bytes));
    }

    const uint32_t serial_number = base::numerics::U32FromLittleEndian(serial_number_bytes);
    if (serial_number) {
        block_zero_serial_number_hash_ = base::MD5String(base::NumberToString(serial_number));
    }
    ...
    if (edid.size() < kWeekOfManufactureOffset + 1) {
        base::UmaHistogramEnumeration(kParseEdidFailureMetric, ParseEdidFailure::kWeekOfManufacture);
        return; // Any other fields below are beyond this edid offset.
    }
    {
        const uint8_t byte_data = edid[kWeekOfManufactureOffset];
        // Store the value if it's within the range of 1-54 or equals to 0xFF.
        if ((byte_data > 0x00 && byte_data <= kValidWeekValueUpperBound) || byte_data == kModelYearMarker) {
    ...
    if (edid.size() < kYearOfManufactureOffset + 1) {
        base::UmaHistogramEnumeration(kParseEdidFailureMetric, ParseEdidFailure::kYearOfManufacture);
        return; // Any other fields below are beyond this edid offset.
    }
    {
    ...
    if (edid.size() < kVideoInputDefinitionOffset + 1) {
        base::UmaHistogramEnumeration(kParseEdidFailureMetric, ParseEdidFailure::kBitsPerChannel);
        return; // Any other fields below are beyond this edid offset.
    }
    if (edid[kEDIDRevisionNumberOffset] >= kEDIDRevision4Value && (edid[kVideoInputDefinitionOffset] & kDigitalInfoMask)) {
    ...
    if (edid.size() < kEDIDMaxVerticalImageSizeOffset + 1) {
        base::UmaHistogramEnumeration(kParseEdidFailureMetric, ParseEdidFailure::kPhysicalSize);
        return; // Any other fields below are beyond this edid offset.
    }
    const gfx::Size max_image_size(edid[kEDIDMaxHorizontalImageSizeOffset], edid[kEDIDMaxVerticalImageSizeOffset]);
    ...
    if (edid.size() < kGammaOffset + 1) {
        base::UmaHistogramEnumeration(kParseEdidFailureMetric, ParseEdidFailure::kGamma);
        return; // Any other fields below are beyond this edid offset.
    }
    ...
    static_assert(kChromaticityOffset + kChromaticityLength == kWhiteyMsbOffset + 1, "EDID Parameter section length error");

    if (edid.size() < kChromaticityOffset + kChromaticityLength) {
        base::UmaHistogramEnumeration(kParseEdidFailureMetric, ParseEdidFailure::kChromaticityCoordinates);
        return; // Any other fields below are beyond this edid offset.
    }
    ...
    primaries_.fRX = ((edid[kRedxMsbOffset] << 2) + ((red_green_lsbs >> kRedxLsbPosition) & kLsbMask)) / 1024.0f;
    primaries_.fRY = ((edid[kRedyMsbOffset] << 2) + ((red_green_lsbs >> kRedyLsbPosition) & kLsbMask)) / 1024.0f;
    primaries_.fGX = ((edid[kGreenxMsbOffset] << 2) + ((red_green_lsbs >> kGreenxLsbPosition) & kLsbMask)) / 1024.0f;
    primaries_.fGY = ((edid[kGreenyMsbOffset] << 2) + ((red_green_lsbs >> kGreenyLsbPosition) & kLsbMask)) / 1024.0f;
    primaries_.fBX = ((edid[kBluexMsbOffset] << 2) + ((blue_white_lsbs >> kBluexLsbPosition) & kLsbMask)) / 1024.0f;
    primaries_.fBY = ((edid[kBlueyMsbOffset] << 2) + ((blue_white_lsbs >> kBlueyLsbPosition) & kLsbMask)) / 1024.0f;
    primaries_.fWX = ((edid[kWhitexMsbOffset] << 2) + ((blue_white_lsbs >> kWhitexLsbPosition) & kLsbMask)) / 1024.0f;
    primaries_.fWY = ((edid[kWhiteyMsbOffset] << 2) + ((blue_white_lsbs >> kWhiteyLsbPosition) & kLsbMask)) / 1024.0f;
    ...
    for (size_t i = 0; i < kNumDescriptors; ++i) {
        ...
        if (edid[offset] == 0 && edid[offset + 1] == 0 && edid[offset + 2] == 0 && edid[offset + 3] == kMonitorNameDescriptor && edid[offset + 4] == 0) {
            std::string name(reinterpret_cast<const char*>(&edid[offset + 5]), kDescriptorLength - 5);
        ...
        if (edid[offset] == 0 && edid[offset + 1] == 0 && edid[offset + 2] == 0 && edid[offset + 3] == kDisplayRangeLimitsDescriptor) {
            // byte 4: Offsets for display range limits
            ...
            if (edid[offset + 5] == 0 || edid[offset + 6] == 0 || edid[offset + 7] == 0 || edid[offset + 8] == 0) {
                continue;
            }
        ...
        // we should check bytes 0-2 and 4, since it may have other values in
        // case that the descriptor contains other type of data.
        if (edid[offset] == 0 && edid[offset + 1] == 0 && edid[offset + 2] == 0 && edid[offset + 3] == kMonitorSerialNumberDescriptor
            && edid[offset + 4] == 0) {
            std::string serial_number_str(reinterpret_cast<const char*>(&edid[offset + 5]), kDescriptorLength - 5);
            base::TrimWhitespaceASCII(serial_number_str, base::TRIM_TRAILING, &serial_number_str);
            if (!serial_number_str.empty()) {
                descriptor_block_serial_number_hash_ = base::MD5String(serial_number_str);
            }
            continue;
        }
    }
    ...
    std::replace_if(
        display_name_.begin(), display_name_.end(), [](unsigned char c) { return !absl::ascii_isascii(c) || !absl::ascii_isprint(c); }, ' ');
    ...
    constexpr std::pair<gfx::ColorSpace::PrimaryID, gfx::ColorSpace::MatrixID> kPrimaryMatrixIDMap[]
        = { // xvYCC601. Standard Definition Colorimetry based on IEC 61966-2-4.
              { gfx::ColorSpace::PrimaryID::SMPTE170M, gfx::ColorSpace::MatrixID::SMPTE170M },
              ...
              // BT2020YCC. Colorimetry based on ITU-R BT.2020 Y’C’BC’R.
              { gfx::ColorSpace::PrimaryID::BT2020, gfx::ColorSpace::MatrixID::BT2020_NCL },
    ...
    if (edid.size() < kNumExtensionsOffset + 1) {
        base::UmaHistogramEnumeration(kParseEdidFailureMetric, ParseEdidFailure::kExtensions);
        return; // Any other fields below are beyond this edid offset.
    }
    ...
    for (size_t i = 0; i < num_extensions; ++i) {
        ...
        if (extention_tag != kCEAExtensionTag || revision != kExpectedExtensionRevision) {
            continue;
        }

        const uint8_t timing_descriptors_start = std::min(edid[extension_offset + 2], static_cast<unsigned char>(kExtensionSize));

        for (size_t data_offset = extension_offset + kDataBlockOffset; data_offset < extension_offset + timing_descriptors_start;) {
            ...
            if (tag == kAudioTag) {
                ...
                for (int sad_index = 0; sad_index + kCEAShortAudioDescriptorLength <= payload_length; sad_index += kCEAShortAudioDescriptorLength) {
            ...
            case kExtendedVideoCapabilityTag:
                ...
                overscan_flag_ = (edid[data_offset + 2] & (1 << kPTOverscanFlagPosition)) || (edid[data_offset + 2] & (1 << kITOverscanFlagPosition))
            ...
            case kColorimetryDataBlockCapabilityTag: {
                ...
                const std::bitset<kMaxNumColorimetryEntries> supported_primaries_bitfield(cdb_bits);
                static_assert(kMaxNumColorimetryEntries == std::size(kPrimaryMatrixIDMap), "kPrimaryIDMap should describe all possible colorimetry entries");
                for (size_t entry = 0; entry < kMaxNumColorimetryEntries; ++entry) {
                    if (supported_primaries_bitfield[entry] && std::get<0>(kPrimaryMatrixIDMap[entry]) != gfx::ColorSpace::PrimaryID::INVALID
                        && std::get<1>(kPrimaryMatrixIDMap[entry]) != gfx::ColorSpace::MatrixID::INVALID) {
                        supported_color_primary_matrix_ids_.insert(kPrimaryMatrixIDMap[entry]);
                    }
                }
                break;
            }
            ...
            case kHDRStaticMetadataCapabilityTag: {
                ...
                const std::bitset<kMaxNumHDRStaticMetadataEntries> supported_eotfs_bitfield(edid[data_offset + 2]);
                static_assert(kMaxNumHDRStaticMetadataEntries == std::size(kTransferIDMap), "kTransferIDMap should describe all possible transfer entries");
                for (size_t entry = 0; entry < kMaxNumHDRStaticMetadataEntries; ++entry) {
                ...
                hdr_static_metadata_->supported_eotf_mask = base::checked_cast<uint8_t>(supported_eotfs_bitfield.to_ulong());

                // See CEA 861.3-2015, Sec.7.5.13, "HDR Static Metadata Data Block"
                // for details on the following calculations.
                const uint8_t length_of_data_block = edid[data_offset] & kHDRStaticMetadataDataBlockLengthMask;
                if (length_of_data_block <= 3) {
                    break;
                }
                const uint8_t desired_content_max_luminance = edid[data_offset + 4];
                hdr_static_metadata_->max = 50.0 * pow(2, desired_content_max_luminance / 32.0);

                if (length_of_data_block <= 4) {
                    break;
                }
                const uint8_t desired_content_max_frame_average_luminance = edid[data_offset + 5];
                hdr_static_metadata_->max_avg = 50.0 * pow(2, desired_content_max_frame_average_luminance / 32.0);

                if (length_of_data_block <= 5) {
                    break;
                }
                const uint8_t desired_content_min_luminance = edid[data_offset + 6];
                hdr_static_metadata_->min = hdr_static_metadata_->max * pow(desired_content_min_luminance / 255.0, 2) / 100.0;
                break;
            }
            default:
                break;
    ...
    base::UmaHistogramEnumeration(kParseEdidFailureMetric, ParseEdidFailure::kNoError);
...
void EdidParser::ParseDisplayIdExtension(const std::vector<uint8_t>& edid, size_t extension_offset)
{
    ...
    const uint8_t num_bytes_in_section_data_blocks = edid[displayid_extension_offset + 1];
    ...
    const size_t max_offset = std::min(edid.size(), displayid_data_block_base + num_bytes_in_section_data_blocks);
    while (current_data_block_offset + kDataBlockNumPayloadBytesOffset < max_offset
        ...
        current_data_block_offset += edid[current_data_block_offset + kDataBlockNumPayloadBytesOffset] + kDataBlockNonPayloadBytes;
    }
}
...
// parsing more fields.
void EdidParser::ParseTiledDisplayBlock(const std::vector<uint8_t>& edid, size_t block_offset)
{
    // See:
    ...
    tile_can_scale_to_fit_ = (edid[block_offset + kTileCapabilitiesOffset] & kSingleTileBehaviorBitmask) == kSingleTileStretchToFit;
}
...
void EdidParser::ReportEdidOptionalsForExternalDisplay() const
{
    ...
    if (!week_of_manufacture_.has_value()) {
        ...
        base::UmaHistogramEnumeration(kParseExternalDisplayEdidOptionalsMetric, ParseEdidOptionals::kWeekOfManufacture);
    }
    if (!max_image_size_.has_value()) {
        all_optionals_available = false;
        base::UmaHistogramEnumeration(kParseExternalDisplayEdidOptionalsMetric, ParseEdidOptionals::kPhysicalSize);
    }
    uint8_t serial_number_count = kMaxSerialNumberCount;
    if (!block_zero_serial_number_hash_.has_value()) {
        all_optionals_available = false;
        serial_number_count--;
        base::UmaHistogramEnumeration(kParseExternalDisplayEdidOptionalsMetric, ParseEdidOptionals::kBlockZeroSerialNumber);
    }
    if (!descriptor_block_serial_number_hash_.has_value()) {
        all_optionals_available = false;
        serial_number_count--;
        base::UmaHistogramEnumeration(kParseExternalDisplayEdidOptionalsMetric, ParseEdidOptionals::kDescriptorBlockSerialNumber);
    }
    base::UmaHistogramExactLinear(kNumOfSerialNumbersProvidedByExternalDisplay, serial_number_count, kMaxSerialNumberCount);

    if (all_optionals_available) {
        base::UmaHistogramEnumeration(kParseExternalDisplayEdidOptionalsMetric, ParseEdidOptionals::kAllAvailable);
    }
}
