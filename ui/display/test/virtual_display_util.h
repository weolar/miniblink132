...
bool constexpr operator<(const display::test::DisplayParams& a, const display::test::DisplayParams& b)
{
    return std::tuple(a.resolution.width(), a.resolution.height(), a.dpi.x(), a.dpi.y(), a.description)
        < std::tuple(b.resolution.width(), b.resolution.height(), b.dpi.x(), b.dpi.y(), b.description);
}

bool constexpr operator==(const display::test::DisplayParams& a, const display::test::DisplayParams& b)
{
    return a.resolution == b.resolution && a.dpi == b.dpi && a.description == b.description;
}
