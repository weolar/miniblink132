...
DisplayInfoList CreateDisplayInfoListFromString(const std::string& specs, DisplayManager* display_manager, bool generate_new_ids)
{
    Displays list = display_manager->IsInUnifiedMode() ? display_manager->software_mirroring_display_list() : display_manager->active_display_list();
...
bool GetDisplayModeForResolution(const ManagedDisplayInfo& info, const gfx::Size& resolution, ManagedDisplayMode* mode)
{
    ...
    if (iter == modes.end()) {
        DLOG(WARNING) << "Unsupported resolution was requested:" << resolution.ToString();
        return false;
    }
    *mode = *iter;
    return true;
...
size_t DisplayManagerTestApi::maximum_support_display_ = kDefaultMaxSupportDisplayTest;
...
int64_t DisplayManagerTestApi::AddDisplay(const DisplayParams& display_params)
{
    ...
    for (const Display& display : current_displays) {
        ManagedDisplayInfo display_info = GetInternalManagedDisplayInfo(display.id());
...
void DisplayManagerTestApi::RemoveDisplay(int64_t display_id)
{
    ...
    for (const Display& display : active_displays) {
        ...
        desired_display_infos.push_back(GetInternalManagedDisplayInfo(display.id()));
    }
...
void DisplayManagerTestApi::UpdateDisplay(const std::string& display_specs, bool from_native_platform, bool generate_new_ids)
{
    DisplayInfoList display_info_list = CreateDisplayInfoListFromString(display_specs, display_manager_, generate_new_ids);
    UpdateDisplayWithDisplayInfoList(display_info_list, from_native_platform);
}

void DisplayManagerTestApi::UpdateDisplayWithDisplayInfoList(const std::vector<ManagedDisplayInfo>& display_info_list, bool from_native_platform)
{
    ...
    for (auto& info : display_list_copy) {
        ...
        if (!from_native_platform) {
            display_manager_->RegisterDisplayProperty(info.id(), info.GetRotation(Display::RotationSource::USER),
                /*overscan_insets=*/nullptr,
                /*resolution_in_pixels=*/gfx::Size(), info.device_scale_factor(), info.zoom_factor(), info.zoom_factor_map(), info.refresh_rate(),
                info.is_interlaced(), info.variable_refresh_rate_state(), info.vsync_rate_min());
        }
    }
...
const ManagedDisplayInfo& DisplayManagerTestApi::GetInternalManagedDisplayInfo(int64_t display_id)
{
    return display_manager_->display_info_[display_id];
}

void DisplayManagerTestApi::SetTouchSupport(int64_t display_id, Display::TouchSupport touch_support)
{
    display_manager_->FindDisplayForId(display_id)->set_touch_support(touch_support);
}
...
const Display& DisplayManagerTestApi::GetSecondaryDisplay() const
{
    ...
    const int64_t primary_display_id = Screen::GetScreen()->GetPrimaryDisplay().id();

    auto primary_display_iter = base::ranges::find(display_manager_->active_display_list_, primary_display_id, &Display::id);
...
ScopedSetInternalDisplayId::ScopedSetInternalDisplayId(DisplayManager* display_manager, int64_t id)
{
...
bool SetDisplayResolution(DisplayManager* display_manager, int64_t display_id, const gfx::Size& resolution)
{
...
std::unique_ptr<DisplayLayout> CreateDisplayLayout(DisplayManager* display_manager, DisplayPlacement::Position position, int offset)
{
    DisplayLayoutBuilder builder(Screen::GetScreen()->GetPrimaryDisplay().id());
    builder.SetSecondaryPlacement(DisplayManagerTestApi(display_manager).GetSecondaryDisplay().id(), position, offset);
