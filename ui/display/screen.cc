...
Screen::Screen()
    : display_id_for_new_windows_(kInvalidDisplayId)
{
}
...
// static
Screen* Screen::SetScreenInstance(Screen* instance, const base::Location& location)
{
    ...
    DCHECK(!g_screen || !instance || (instance && instance->shutdown_)) << "fail=" << location.ToString();
...
Display Screen::GetDisplayForNewWindows() const
{
    ...
    if (scoped_display_id_for_new_windows_ != kInvalidDisplayId && GetDisplayWithDisplayId(scoped_display_id_for_new_windows_, &display)) {
        return display;
    }
...
gfx::Rect Screen::ScreenToDIPRectInWindow(gfx::NativeWindow window, const gfx::Rect& screen_rect) const
{
    float scale = GetDisplayNearestWindow(window).device_scale_factor();
    return ScaleToEnclosingRect(screen_rect, 1.0f / scale);
}

gfx::Rect Screen::DIPToScreenRectInWindow(gfx::NativeWindow window, const gfx::Rect& dip_rect) const
{
    float scale = GetDisplayNearestWindow(window).device_scale_factor();
    return ScaleToEnclosingRect(dip_rect, scale);
}

bool Screen::GetDisplayWithDisplayId(int64_t display_id, Display* display) const
{
...
void Screen::SetPanelRotationForTesting(int64_t display_id, Display::Rotation rotation)
{
    // Not implemented.
    DCHECK(false);
}
...
base::Value::List Screen::GetGpuExtraInfo(const gfx::GpuExtraInfo& gpu_extra_info)
{
...
std::optional<float> Screen::GetPreferredScaleFactorForWindow(gfx::NativeWindow window) const
{
...
std::optional<float> Screen::GetPreferredScaleFactorForView(gfx::NativeView view) const
{
...
bool Screen::InTabletMode() const
{
    ...
    return state == TabletState::kInTabletMode || state == TabletState::kEnteringTabletMode;
}
#endif
...
void Screen::SetScopedDisplayForNewWindows(int64_t display_id)
{
    ...
    DCHECK(display_id == kInvalidDisplayId ^ scoped_display_id_for_new_windows_ == kInvalidDisplayId)
        << "display_id=" << display_id << ", scoped_display_id_for_new_windows_=" << scoped_display_id_for_new_windows_;
