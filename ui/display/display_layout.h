...
struct DISPLAY_EXPORT DisplayPlacement {
    ...
    DisplayPlacement(Position position, int offset, OffsetReference offset_reference);
    DisplayPlacement(int64_t display_id, int64_t parent_display_id, Position position, int offset, OffsetReference offset_reference);
...
public:
    ...
    void ApplyToDisplayList(Displays* display_list, std::vector<int64_t>* updated_ids, int minimum_offset_overlap);
    ...
    // |reference| rectangle.
    static DisplayPlacement CreatePlacementForRectangles(const gfx::Rect& reference, const gfx::Rect& rectangle);

private:
    // Apply the display placement to |display_list|.
    // Returns true if the display bounds were updated.
    static bool ApplyDisplayPlacement(const DisplayPlacement& placement, Displays* display_list, int minimum_offset_overlap);
};
