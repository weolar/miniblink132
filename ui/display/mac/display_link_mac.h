...
private:
    ...
    explicit VSyncCallbackMac(UnregisterCallback unregister_callback, Callback callback, bool do_callback_on_ctor_thread);
...
public:
    ...
    virtual std::unique_ptr<VSyncCallbackMac> RegisterCallback(VSyncCallbackMac::Callback callback) = 0;

    // Get the panel/monitor refresh rate
    virtual double GetRefreshRate() const = 0;
    virtual void GetRefreshIntervalRange(base::TimeDelta& min_interval, base::TimeDelta& max_interval, base::TimeDelta& granularity) const = 0;

    virtual void SetPreferredInterval(base::TimeDelta interval) = 0;
    virtual void SetPreferredIntervalRange(base::TimeDelta min_interval, base::TimeDelta max_interval, base::TimeDelta preferred_interval) = 0;
