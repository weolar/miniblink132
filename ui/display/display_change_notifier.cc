...
void DisplayChangeNotifier::NotifyDisplaysChanged(const std::vector<Display>& old_displays, const std::vector<Display>& new_displays)
{
    std::vector<Display> removed_displays;
    // Display present in old_displays but not in new_displays has been removed.
    for (auto old_it = old_displays.begin(); old_it != old_displays.end(); ++old_it) {
    ...
    if (!removed_displays.empty()) {
        observer_list_.Notify(&DisplayObserver::OnDisplaysRemoved, removed_displays);
    }

    // Display present in new_displays but not in old_displays has been added.
    // Display present in both might have been modified.
    for (auto new_it = new_displays.begin(); new_it != new_displays.end(); ++new_it) {
        ...
        if (metrics != DisplayObserver::DISPLAY_METRIC_NONE) {
            observer_list_.Notify(&DisplayObserver::OnDisplayMetricsChanged, *new_it, metrics);
        }
    }
}

void DisplayChangeNotifier::NotifyCurrentWorkspaceChanged(const std::string& workspace)
{
