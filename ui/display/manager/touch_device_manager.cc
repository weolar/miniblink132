...
bool IsDeviceConnectedViaUsb(const base::FilePath& path)
{
    for (const auto& component : path.GetComponents()) {
        if (base::StartsWith(component, "usb", base::CompareCase::INSENSITIVE_ASCII)) {
            return true;
        }
...
int GetUsbAssociationScore(const ManagedDisplayInfo* display, const ui::TouchscreenDevice& device)
{
    ...
    std::vector<base::FilePath::StringType> display_components = display->sys_path().GetComponents();
    std::vector<base::FilePath::StringType> device_components = device.sys_path.GetComponents();

    std::size_t largest_idx = 0;
    while (largest_idx < display_components.size() && largest_idx < device_components.size()
...
DeviceList::const_iterator GuessBestUsbDevice(const ManagedDisplayInfo* display, const DeviceList& devices)
{
    int best_score = 0;
...
// |display_id|.
void ClearCalibrationDataInMap(TouchDeviceManager::AssociationInfoMap& info_map, int64_t display_id)
{
...
ManagedDisplayInfo* GetBestMatchForDevice(
    const TouchDeviceManager::TouchAssociationMap& touch_associations, const TouchDeviceIdentifier& identifier, ManagedDisplayInfoList* displays)
{
    ...
    const TouchDeviceManager::AssociationInfoMap& info_map = touch_associations.at(identifier);
    ...
    for (auto* display : *displays) {
        ...
        if (!base::Contains(info_map, display->id()))
            continue;
        const TouchDeviceManager::TouchAssociationInfo& info = info_map.at(display->id());
...
std::set<TouchDeviceIdentifier, TouchDeviceIdentifier::WeakComp> GetCollisionSet(const DeviceList& devices)
{
    std::set<TouchDeviceIdentifier, TouchDeviceIdentifier::WeakComp> collision_set;
...
// static
const TouchDeviceIdentifier& TouchDeviceIdentifier::GetFallbackTouchDeviceIdentifier()
{
    static const TouchDeviceIdentifier kFallTouchDeviceIdentifier(
        GenerateIdentifier(kFallbackTouchDeviceName, 0, 0), base::PersistentHash(kFallbackTouchDevicePhys));
    return kFallTouchDeviceIdentifier;
}

// static
uint32_t TouchDeviceIdentifier::GenerateIdentifier(std::string name, uint16_t vendor_id, uint16_t product_id)
{
    std::string hash_str = name + "-" + base::NumberToString(vendor_id) + "-" + base::NumberToString(product_id);
    return base::PersistentHash(hash_str);
}

// static
TouchDeviceIdentifier TouchDeviceIdentifier::FromDevice(const ui::TouchscreenDevice& touch_device)
{
    if (!touch_device.id)
        return GetFallbackTouchDeviceIdentifier();
    return TouchDeviceIdentifier(
        GenerateIdentifier(touch_device.name, touch_device.vendor_id, touch_device.product_id), base::PersistentHash(touch_device.phys));
}
...
TouchDeviceIdentifier::TouchDeviceIdentifier(uint32_t identifier, uint32_t secondary_id)
    : id_(identifier)
...
TouchDeviceIdentifier& TouchDeviceIdentifier::operator=(TouchDeviceIdentifier other)
{
    id_ = other.id_;
    secondary_id_ = other.secondary_id_;
    return *this;
...
// static
bool TouchCalibrationData::CalibrationPointPairCompare(const CalibrationPointPair& pair_1, const CalibrationPointPair& pair_2)
{
...
TouchCalibrationData::TouchCalibrationData(const TouchCalibrationData::CalibrationPointPairQuad& point_pairs, const gfx::Size& bounds)
    : point_pairs(point_pairs)
    , bounds(bounds)
{
}

TouchCalibrationData::TouchCalibrationData(const TouchCalibrationData& calibration_data) = default;

TouchCalibrationData& TouchCalibrationData::operator=(const TouchCalibrationData& calibration_data) = default;
...
void TouchDeviceManager::AssociateTouchscreens(std::vector<ManagedDisplayInfo>* all_displays, const std::vector<ui::TouchscreenDevice>& all_devices)
{
    ...
    if (VLOG_IS_ON(2)) {
        for (const ManagedDisplayInfo* display : displays) {
            VLOG(2) << "Received display " << display->name() << " (size: " << display->GetNativeModeSize().ToString() << ", "
                    << "sys_path: " << display->sys_path().LossyDisplayName() << ")";
        }
        for (const ui::TouchscreenDevice& device : devices) {
            VLOG(2) << "Received device " << device.name << " (size: " << device.size.ToString() << ", sys_path: " << device.sys_path.LossyDisplayName() << ")";
        }
    }
...
void TouchDeviceManager::AssociateInternalDevices(ManagedDisplayInfoList* displays, DeviceList* devices)
{
    VLOG(2) << "Trying to match internal devices (" << displays->size() << " displays and " << devices->size() << " devices to match)";
    ...
    for (auto device_it = devices->begin(); device_it != devices->end();) {
        ...
        if (internal_display) {
            VLOG(2) << "=> Matched device " << internal_device.name << " to display " << internal_display->name();
    ...
    if (!matched && internal_display) {
        VLOG(2) << "=> No device found to match with internal display " << internal_display->name();
    }
}

void TouchDeviceManager::AssociateDevicesWithCollision(ManagedDisplayInfoList* displays, DeviceList* devices)
{
    ...
    std::set<TouchDeviceIdentifier, TouchDeviceIdentifier::WeakComp> collision_set = GetCollisionSet(*devices);
    if (collision_set.empty())
        return;

    VLOG(2) << "Trying to match " << devices->size() << " devices "
            << "and " << displays->size() << " displays where there is/are " << collision_set.size() << " collisions with the touch device ids";
    ...
    for (auto device_it = devices->begin(); device_it != devices->end();) {
        ...
        if (!base::Contains(collision_set, identifier) || !base::Contains(port_associations_, identifier)) {
            device_it++;
            continue;
        }
        ...
        ManagedDisplayInfoList::iterator display_it = base::ranges::find(*displays, display_id, &ManagedDisplayInfo::id);

        if (display_it != displays->end()) {
            VLOG(2) << "=> Matched device " << (*device_it).name << " to display " << (*display_it)->name();
...
void TouchDeviceManager::AssociateFromHistoricalData(ManagedDisplayInfoList* displays, DeviceList* devices)
{
    if (!devices->size() || !displays->size())
        return;
    ...
    for (auto device_it = devices->begin(); device_it != devices->end();) {
        auto* matched_display_info = GetBestMatchForDevice(touch_associations_, TouchDeviceIdentifier::FromDevice(*device_it), displays);
        if (matched_display_info) {
            VLOG(2) << "=> Matched device " << (*device_it).name << " to display " << matched_display_info->name();
...
void TouchDeviceManager::AssociateUsbDevices(ManagedDisplayInfoList* displays, DeviceList* devices)
{
    VLOG(2) << "Trying to match usb devices (" << displays->size() << " displays and " << devices->size() << " devices to match)";

    for (auto display_it = displays->begin(); display_it != displays->end(); display_it++) {
        ...
        if (device_it != devices->end()) {
            ...
            VLOG(2) << "=> Matched device " << device.name << " to display " << display->name() << " (score=" << GetUsbAssociationScore(display, device) << ")";
...
void TouchDeviceManager::AssociateSameSizeDevices(ManagedDisplayInfoList* displays, DeviceList* devices)
{
    // Associate screens/displays with the same size.
    VLOG(2) << "Trying to match same-size devices (" << displays->size() << " displays and " << devices->size() << " devices to match)";
    ...
    for (auto display_it = displays->begin(); display_it != displays->end(); display_it++) {
        ...
        DeviceList::iterator device_it = base::ranges::find_if(*devices, [&native_size](const ui::TouchscreenDevice& device) {
            ...
            // configuration.
            return std::abs(native_size.width() - device.size.width()) <= 1 && std::abs(native_size.height() - device.size.height()) <= 1;
        });

        if (device_it != devices->end()) {
            const ui::TouchscreenDevice& device = *device_it;
            VLOG(2) << "=> Matched device " << device.name << " to display " << display->name() << " (display_size: " << native_size.ToString()
...
void TouchDeviceManager::AssociateToSingleDisplay(ManagedDisplayInfoList* displays, DeviceList* devices)
{
    // If there is only one display left, then we should associate all input
    // devices with it.
    VLOG(2) << "Trying to match to single display (" << displays->size() << " displays and " << devices->size() << " devices to match)";
    ...
    for (const ui::TouchscreenDevice& device : *devices) {
        VLOG(2) << "=> Matched device " << device.name << " to display " << display->name();
        Associate(display, device);
    }
    devices->clear();
}

void TouchDeviceManager::AssociateAnyRemainingDevices(ManagedDisplayInfoList* displays, DeviceList* devices)
{
    if (!displays->size() || !devices->size())
        return;
    VLOG(2) << "Trying to match remaining " << devices->size() << " devices to a display.";
    ...
    VLOG(2) << "Matching " << devices->size() << " touch devices to " << display->name() << "[" << display->id() << "]";
...
void TouchDeviceManager::Associate(ManagedDisplayInfo* display, const ui::TouchscreenDevice& device)
{
    display->set_touch_support(Display::TouchSupport::AVAILABLE);
    active_touch_associations_[TouchDeviceIdentifier::FromDevice(device)] = display->id();
}
...
void TouchDeviceManager::AddTouchCalibrationData(const ui::TouchscreenDevice& device, int64_t display_id, const TouchCalibrationData& data)
{
    AddTouchCalibrationDataImpl(device, display_id, &data);
}

void TouchDeviceManager::AddTouchAssociation(const ui::TouchscreenDevice& device, int64_t display_id)
{
    AddTouchCalibrationDataImpl(device, display_id, /*data=*/nullptr);
}

void TouchDeviceManager::AddTouchCalibrationDataImpl(const ui::TouchscreenDevice& device, int64_t display_id, const TouchCalibrationData* data)
{
    const TouchDeviceIdentifier identifier = TouchDeviceIdentifier::FromDevice(device);
...
void TouchDeviceManager::ClearTouchCalibrationData(const ui::TouchscreenDevice& device, int64_t display_id)
{
    const TouchDeviceIdentifier identifier = TouchDeviceIdentifier::FromDevice(device);
...
TouchCalibrationData TouchDeviceManager::GetCalibrationData(const ui::TouchscreenDevice& touchscreen, int64_t display_id) const
{
    TouchDeviceIdentifier identifier = TouchDeviceIdentifier::FromDevice(touchscreen);
    ...
    TouchDeviceIdentifier fallback_identifier(TouchDeviceIdentifier::GetFallbackTouchDeviceIdentifier());
    if (base::Contains(touch_associations_, fallback_identifier)) {
        const AssociationInfoMap& info_map = touch_associations_.at(fallback_identifier);
        if (info_map.find(display_id) != info_map.end())
            return info_map.at(display_id).calibration_data;
    }
...
bool TouchDeviceManager::DisplayHasTouchDevice(int64_t display_id, const ui::TouchscreenDevice& device) const
{
    const TouchDeviceIdentifier identifier = TouchDeviceIdentifier::FromDevice(device);
    return base::Contains(active_touch_associations_, identifier) && active_touch_associations_.at(identifier) == display_id;
}

int64_t TouchDeviceManager::GetAssociatedDisplay(const ui::TouchscreenDevice& device) const
{
    const TouchDeviceIdentifier identifier = TouchDeviceIdentifier::FromDevice(device);
    if (base::Contains(active_touch_associations_, identifier))
...
std::vector<ui::TouchscreenDevice> TouchDeviceManager::GetAssociatedTouchDevicesForDisplay(int64_t display_id) const
{
    std::vector<ui::TouchscreenDevice> result;
    for (const auto& device : ui::DeviceDataManager::GetInstance()->GetTouchscreenDevices()) {
        const TouchDeviceIdentifier identifier = TouchDeviceIdentifier::FromDevice(device);
...
void TouchDeviceManager::RegisterTouchAssociations(const TouchAssociationMap& touch_associations, const PortAssociationMap& port_associations)
{
    touch_associations_ = touch_associations;
    port_associations_ = port_associations;
}

std::ostream& operator<<(std::ostream& os, const TouchDeviceIdentifier& identifier)
{
...
{
    for (const auto& device : ui::DeviceDataManager::GetInstance()->GetTouchscreenDevices()) {
        if (device.type == ui::InputDeviceType::INPUT_DEVICE_USB || device.type == ui::InputDeviceType::INPUT_DEVICE_BLUETOOTH) {
            return true;
        }
    }
    return false;
