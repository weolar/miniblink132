...
const std::string kFakeKey285 = "abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuv"
    ...
    void SetProvisionedKeyRequest(bool use_valid_key)
    {
        if (use_valid_key) {
            key_manager_.set_provisioned_key_request(
                base::BindRepeating([](base::OnceCallback<void(const std::string&)> cb) { std::move(cb).Run(kFakeKey285); }));
        } else {
            key_manager_.set_provisioned_key_request(base::BindRepeating([](base::OnceCallback<void(const std::string&)> cb) { std::move(cb).Run(""); }));
        }
    }

    void SetKeyIfRequiredForDisplay(auto on_key_set)
    {
        key_manager_.SetKeyIfRequired(std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>> { display_.get() }, kDisplayId, std::move(on_key_set));
    }
...
private:
    void SetUp() override
    {
        scoped_feature_list_.InitAndEnableFeature(features::kRequireHdcpKeyProvisioning);
...
TEST_F(ContentProtectionKeyManagerTest, TestIfKeySetWhenServerKeyIsValid)
{
    ...
    EXPECT_EQ(GetSetHdcpKeyPropAction(kDisplayId, true), log_.GetActionsAndClear());
}
...
TEST_F(ContentProtectionKeyManagerTest, TestIfKeyNotSetWhenDisplayIdMismatch)
{
    ...
    key_manager_.SetKeyIfRequired(std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>> { display_.get() }, kDisplayId + 1, std::move(on_key_set));
}

TEST_F(ContentProtectionKeyManagerTest, TestIfKeySetWithMultipleDisplaysOneIsValid)
{
    ...
    auto displays = std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>> { display_.get(), other_display.get() };
...
TEST_F(ContentProtectionKeyManagerTest, TestIfKeyNotSetWhenDisplayHasNoContentProtectionKeyProp)
{
    display_ = FakeDisplaySnapshot::Builder()
                   .SetId(kDisplayId)
                   .SetType(DISPLAY_CONNECTION_TYPE_DISPLAYPORT)
...
TEST_F(ContentProtectionKeyManagerTest, TestThatKeyIsSetForMultipleDisplays)
{
    ...
    key_manager_.SetKeyIfRequired(std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>> { other_display.get() }, kDisplayId + 1, std::move(on_key_set));

    EXPECT_EQ(GetSetHdcpKeyPropAction(kDisplayId, true) + "," + GetSetHdcpKeyPropAction(kDisplayId + 1, true), log_.GetActionsAndClear());
}

TEST_F(ContentProtectionKeyManagerTest, TestThatKeyIsFetchedOnlyOnceIfKeyValid)
{
    key_manager_.set_provisioned_key_request(base::BindRepeating([](base::OnceCallback<void(const std::string&)> cb) {
