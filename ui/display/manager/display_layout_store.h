...
public:
    ...
    void RegisterLayoutForDisplayIdList(const DisplayIdList& list, std::unique_ptr<DisplayLayout> layout);
...
private:
    friend void DisplayManager::UpdateDisplaysWith(const std::vector<ManagedDisplayInfo>& updated_display_info_list);
    ...
    const DisplayLayout& GetOrCreateRegisteredDisplayLayout(const DisplayIdList& display_id_list);
    ...
    const DisplayLayout& GetOrCreateRegisteredDisplayLayoutInternal(const DisplayIdList& list, bool create_if_not_exist);
