...
public:
    using ProvisionedKeyRequest = base::RepeatingCallback<void(base::OnceCallback<void(const std::string&)>)>;
    ...
    ContentProtectionKeyManager& operator=(const ContentProtectionKeyManager&) = delete;
    ...
    void SetKeyIfRequired(const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays_states, int64_t display_id, KeySetCallback on_key_set);
