...
std::unique_ptr<DisplaySnapshot> CreateDisplaySnapshot(int64_t id, DisplayConnectionType type)
{
    return FakeDisplaySnapshot::Builder().SetId(id).SetNativeMode(gfx::Size(1024, 768)).SetType(type).Build();
}
...
public:
    ...
    QueryContentProtectionTaskTest(const QueryContentProtectionTaskTest&) = delete;
    QueryContentProtectionTaskTest& operator=(const QueryContentProtectionTaskTest&) = delete;

    ~QueryContentProtectionTaskTest() override = default;

    void ResponseCallback(Status status, uint32_t connection_mask, uint32_t protection_mask)
    {
...
TEST_F(QueryContentProtectionTaskTest, QueryInternalDisplay)
{
    ...
    displays.push_back(CreateDisplaySnapshot(1, DISPLAY_CONNECTION_TYPE_INTERNAL));
    display_delegate_.SetOutputs(std::move(displays));
    TestDisplayLayoutManager layout_manager(display_delegate_.GetOutputs(), MULTIPLE_DISPLAY_STATE_SINGLE);

    QueryContentProtectionTask task(
        &layout_manager, &display_delegate_, 1, base::BindOnce(&QueryContentProtectionTaskTest::ResponseCallback, base::Unretained(this)));
    task.Run();
...
TEST_F(QueryContentProtectionTaskTest, QueryDisplayWithHdcpType0Enabled)
{
    ...
    display_delegate_.set_content_protection_method(CONTENT_PROTECTION_METHOD_HDCP_TYPE_0);
...
TEST_F(QueryContentProtectionTaskTest, QueryDisplayWithHdcpType1Enabled)
{
    ...
    TestDisplayLayoutManager layout_manager(display_delegate_.GetOutputs(), MULTIPLE_DISPLAY_STATE_SINGLE);
    display_delegate_.set_hdcp_state(HDCP_STATE_ENABLED);
    display_delegate_.set_content_protection_method(CONTENT_PROTECTION_METHOD_HDCP_TYPE_1);
...
TEST_F(QueryContentProtectionTaskTest, QueryInMultiDisplayMode)
{
    ...
    display_delegate_.SetOutputs(std::move(displays));
    TestDisplayLayoutManager layout_manager(display_delegate_.GetOutputs(), MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED);
...
TEST_F(QueryContentProtectionTaskTest, QueryInMirroringMode)
{
    ...
    displays.push_back(CreateDisplaySnapshot(2, DISPLAY_CONNECTION_TYPE_DVI));
    display_delegate_.SetOutputs(std::move(displays));
    TestDisplayLayoutManager layout_manager(display_delegate_.GetOutputs(), MULTIPLE_DISPLAY_STATE_MULTI_MIRROR);
    ...
    ASSERT_TRUE(response_);
    EXPECT_EQ(Status::SUCCESS, response_->status);
    EXPECT_EQ(static_cast<uint32_t>(DISPLAY_CONNECTION_TYPE_HDMI | DISPLAY_CONNECTION_TYPE_DVI), response_->connection_mask);
    EXPECT_EQ(CONTENT_PROTECTION_METHOD_NONE, response_->protection_mask);
}
...
TEST_F(QueryContentProtectionTaskTest, QueryAnalogDisplayMirror)
{
    ...
    ASSERT_TRUE(response_);
    EXPECT_EQ(Status::SUCCESS, response_->status);
    EXPECT_EQ(static_cast<uint32_t>(DISPLAY_CONNECTION_TYPE_HDMI | DISPLAY_CONNECTION_TYPE_VGA), response_->connection_mask);
    EXPECT_EQ(CONTENT_PROTECTION_METHOD_NONE, response_->protection_mask);

    response_.reset();

    QueryContentProtectionTask task2(
        &layout_manager, &display_delegate_, 2, base::BindOnce(&QueryContentProtectionTaskTest::ResponseCallback, base::Unretained(this)));
    task2.Run();
