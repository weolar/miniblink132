...
void MoveInternalDisplaysToTheFront(std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays)
{
...
bool ResolveOverrides(const DisplayConfigurator::RefreshRateOverrideMap& refresh_rate_overrides, std::vector<DisplayConfigureRequest>& requests)
{
    for (auto& request : requests) {
        ...
        auto override_it = refresh_rate_overrides.find(request.display->display_id());
        if (override_it == refresh_rate_overrides.end()) {
            continue;
        }

        request.mode
            = std::make_unique<const DisplayMode>(request.mode->size(), request.mode->is_interlaced(), override_it->second, /*vsync_rate_min=*/std::nullopt);
    }
    return true;
}
...
UpdateDisplayConfigurationTask::UpdateDisplayConfigurationTask(NativeDisplayDelegate* delegate, DisplayLayoutManager* layout_manager,
    MultipleDisplayState new_display_state, chromeos::DisplayPowerState new_power_state, int power_flags, const base::flat_set<int64_t>& new_vrr_state,
    const DisplayConfigurator::RefreshRateOverrideMap& refresh_rate_overrides, bool force_configure, ConfigurationType configuration_type,
...
void UpdateDisplayConfigurationTask::Run()
{
    ...
    delegate_->GetDisplays(base::BindOnce(&UpdateDisplayConfigurationTask::OnDisplaysUpdated, weak_ptr_factory_.GetWeakPtr()));
}
...
void UpdateDisplayConfigurationTask::OnDisplaysUpdated(const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays)
{
    ...
    VLOG(1) << "OnDisplaysUpdated: new_display_state=" << MultipleDisplayStateToString(new_display_state_)
            << " new_power_state=" << DisplayPowerStateToString(new_power_state_) << " flags=" << power_flags_
            << " new_vrr_state=" << VrrStateToString(new_vrr_state_) << " refresh_rate_overrides=" << RefreshRateOverrideToString(refresh_rate_overrides_)
            << " force_configure=" << force_configure_ << " display_count=" << cached_displays_.size();
    if (ShouldConfigure()) {
        EnterState(base::BindOnce(&UpdateDisplayConfigurationTask::OnStateEntered, weak_ptr_factory_.GetWeakPtr()));
    } else {
...
void UpdateDisplayConfigurationTask::EnterState(ConfigureDisplaysTask::ResponseCallback callback)
{
    VLOG(2) << "EnterState";
    std::vector<DisplayConfigureRequest> requests;
    if (!layout_manager_->GetDisplayLayout(cached_displays_, new_display_state_, new_power_state_, new_vrr_state_, &requests)) {
    ...
    if (!requests.empty()) {
        configure_task_ = std::make_unique<ConfigureDisplaysTask>(delegate_, requests, std::move(callback), configuration_type_);
        configure_task_->Run();
    } else {
        VLOG(2) << "No displays";
...
void UpdateDisplayConfigurationTask::OnStateEntered(ConfigureDisplaysTask::Status status)
{
    bool success = status != ConfigureDisplaysTask::ERROR;
    if (new_display_state_ == MULTIPLE_DISPLAY_STATE_MULTI_MIRROR && status == ConfigureDisplaysTask::PARTIAL_SUCCESS)
        success = false;
    ...
    if (layout_manager_->GetSoftwareMirroringController()) {
        ...
        if (!success && new_display_state_ == MULTIPLE_DISPLAY_STATE_MULTI_MIRROR) {
            if (layout_manager_->GetDisplayState() != MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED || layout_manager_->GetPowerState() != new_power_state_
                || force_configure_) {
                new_display_state_ = MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED;
                EnterState(base::BindOnce(&UpdateDisplayConfigurationTask::OnEnableSoftwareMirroring, weak_ptr_factory_.GetWeakPtr()));
                return;
            }

            success = layout_manager_->GetDisplayState() == MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED;
            enable_software_mirroring = success;
            if (success)
                new_display_state_ = MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED;
        }

        layout_manager_->GetSoftwareMirroringController()->SetSoftwareMirroring(enable_software_mirroring);
    }

    FinishConfiguration(success);
}

void UpdateDisplayConfigurationTask::OnEnableSoftwareMirroring(ConfigureDisplaysTask::Status status)
{
    bool success = status != ConfigureDisplaysTask::ERROR;
    layout_manager_->GetSoftwareMirroringController()->SetSoftwareMirroring(success);
    FinishConfiguration(success);
}

void UpdateDisplayConfigurationTask::FinishConfiguration(bool success)
{
    base::UmaHistogramBoolean("DisplayManager.UpdateDisplayConfigurationTask.Success", success);

    std::move(callback_).Run(success, cached_displays_, cached_unassociated_displays_, new_display_state_, new_power_state_);
}

bool UpdateDisplayConfigurationTask::ShouldForceDpms() const
{
    return new_power_state_ != chromeos::DISPLAY_POWER_ALL_OFF
        && (layout_manager_->GetPowerState() != new_power_state_ || (power_flags_ & DisplayConfigurator::kSetDisplayPowerForceProbe));
}
...
MultipleDisplayState UpdateDisplayConfigurationTask::ChooseDisplayState() const
{
...
bool UpdateDisplayConfigurationTask::ShouldConfigureVrr() const
{
    for (const DisplaySnapshot* display : cached_displays_) {
        ...
        if (new_vrr_state_.contains(display->display_id()) != display->IsVrrEnabled()) {
            return true;
        }
    }
