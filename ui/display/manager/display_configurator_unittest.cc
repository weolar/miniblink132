...
std::unique_ptr<DisplayMode> MakeDisplayMode(int width, int height, bool is_interlaced, float refresh_rate)
{
    return std::make_unique<DisplayMode>(gfx::Size { width, height }, is_interlaced, refresh_rate);
}
...
// display mode.
constexpr base::TimeDelta kLongDelay = base::Milliseconds(DisplayConfigurator::kResumeConfigureMultiDisplayDelayMs);
...
public:
    ...
    int num_power_state_changes() const
    {
        return num_power_state_changes_;
    }
    ...
    void OnDisplayConfigurationChanged(const DisplayConfigurator::DisplayStateList& outputs) override
    {
        num_changes_++;
        latest_outputs_ = outputs;
    }

    void OnDisplayConfigurationChangeFailed(const DisplayConfigurator::DisplayStateList& outputs, MultipleDisplayState failed_new_state) override
    {
        num_failures_++;
...
public:
    TestStateController()
        : state_(MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED)
    {
    }
    ...
    void set_state(MultipleDisplayState state)
    {
        state_ = state;
    }

    // DisplayConfigurator::StateController overrides:
    MultipleDisplayState GetStateForDisplayIds(const DisplayConfigurator::DisplayStateList& outputs) override
    {
        return state_;
    }
    bool GetSelectedModeForDisplayId(int64_t display_id, ManagedDisplayMode* out_mode) const override
    {
        return false;
    }
...
class TestMirroringController : public DisplayConfigurator::SoftwareMirroringController {
public:
...
public:
    ...
    {
        return base::BindOnce(&ConfigurationWaiter::OnConfigured, base::Unretained(this));
    }

    CallbackResult callback_result() const
    {
...
public:
    ...
    void SetUp() override
    {
        ...
        scoped_feature_list_.InitAndEnableFeature(features::kEnableHardwareMirrorMode);
        ...
        configurator_.SetDelegateForTesting(std::unique_ptr<NativeDisplayDelegate>(native_display_delegate_));
        ...
        outputs_[1] = FakeDisplaySnapshot::Builder()
                          ...
                          .SetIsAspectPreservingScaling(true)
                          .SetVariableRefreshRateState(VariableRefreshRateState::kVrrNotCapable)
                          .Build();
...
protected:
    ...
    template <typename... Modes> void InitWithOutputs(Modes... modes)
    {
        ...
        EXPECT_EQ(actions.empty() ? kInit
                                  : JoinActions(kInit, kTestModesetStr, GetCrtcActions(DisplayConfig::kStack, modes...).c_str(), kModesetOutcomeSuccess,
                                      kCommitModesetStr, GetCrtcActions(DisplayConfig::kStack, modes...).c_str(), kModesetOutcomeSuccess, nullptr),
            log_->GetActionsAndClear());
    }

    template <typename... Modes> std::string GetCrtcActions(Modes... modes)
    {
        return GetCrtcActions(DisplayConfig::kStack, modes...);
    }

    template <typename... Modes> std::string GetCrtcActions(DisplayConfig config, Modes... modes)
    {
...
private:
    template <size_t I> std::string JoinCrtcActions(DisplayConfig, const gfx::Point&)
    {
        return {};
    }

    template <size_t I, typename... Modes> std::string JoinCrtcActions(DisplayConfig config, gfx::Point origin, const DisplayMode* mode, Modes... modes)
    {
        static_assert(I < kNumOutputs, "More expected modes than outputs");

        std::string action = GetCrtcAction({ outputs_[I]->display_id(), origin, config == DisplayConfig::kOff ? nullptr : mode });
        ...
        return rest.empty() ? action : JoinActions(action.c_str(), rest.c_str(), nullptr);
    }
...
{
    std::unique_ptr<DisplaySnapshot> output = FakeDisplaySnapshot::Builder()
                                                  .SetId(kDisplayIds[0])
    ...
    const std::vector<std::unique_ptr<const DisplayMode>>& modes = output->modes();

    // Should pick native over highest refresh rate.
    EXPECT_EQ(modes[1].get(), DisplayConfigurator::FindDisplayModeMatchingSize(*output, gfx::Size(1920, 1200)));

    // Should pick highest refresh rate.
    EXPECT_EQ(modes[3].get(), DisplayConfigurator::FindDisplayModeMatchingSize(*output, gfx::Size(1920, 1080)));

    // Should pick non-interlaced mode.
    EXPECT_EQ(modes[7].get(), DisplayConfigurator::FindDisplayModeMatchingSize(*output, gfx::Size(1280, 720)));

    // Interlaced only. Should pick one with the highest refresh rate in
    // interlaced mode.
    EXPECT_EQ(modes[10].get(), DisplayConfigurator::FindDisplayModeMatchingSize(*output, gfx::Size(1024, 768)));

    // Mixed: Should pick one with the highest refresh rate in
    // interlaced mode.
    EXPECT_EQ(modes[13].get(), DisplayConfigurator::FindDisplayModeMatchingSize(*output, gfx::Size(1024, 600)));

    // Just one interlaced mode.
    EXPECT_EQ(modes[14].get(), DisplayConfigurator::FindDisplayModeMatchingSize(*output, gfx::Size(640, 480)));

    // Refresh rate not available.
    EXPECT_EQ(modes[15].get(), DisplayConfigurator::FindDisplayModeMatchingSize(*output, gfx::Size(320, 200)));

    // No mode found.
    EXPECT_EQ(nullptr, DisplayConfigurator::FindDisplayModeMatchingSize(*output, gfx::Size(1440, 900)));
}
...
TEST_F(DisplayConfiguratorTest, ConnectSecondOutput)
{
    ...
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(&small_mode_, &big_mode_).c_str(), kModesetOutcomeSuccess, kCommitModesetStr,
                  GetCrtcActions(&small_mode_, &big_mode_).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    ...
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(DisplayConfig::kMirror, &small_mode_, &small_mode_).c_str(), kModesetOutcomeSuccess,
                  kCommitModesetStr, GetCrtcActions(DisplayConfig::kMirror, &small_mode_, &small_mode_).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    EXPECT_FALSE(mirroring_controller_.SoftwareMirroringEnabled());
    EXPECT_EQ(1, observer_.num_changes());
    ...
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(&small_mode_).c_str(), kModesetOutcomeSuccess, kCommitModesetStr,
                  GetCrtcActions(&small_mode_).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    EXPECT_FALSE(mirroring_controller_.SoftwareMirroringEnabled());
    EXPECT_EQ(1, observer_.num_changes());
    ...
    configurator_.SetMultipleDisplayState(MULTIPLE_DISPLAY_STATE_MULTI_MIRROR);
    EXPECT_EQ(kNoActions, log_->GetActionsAndClear());
    EXPECT_EQ(MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED, configurator_.display_state());
...
TEST_F(DisplayConfiguratorTest, SetDisplayPower)
{
    ...
    configurator_.SetDisplayPower(
        chromeos::DISPLAY_POWER_INTERNAL_OFF_EXTERNAL_ON, DisplayConfigurator::kSetDisplayPowerNoFlags, config_waiter_.on_configuration_callback());
    EXPECT_EQ(kNoDelay, config_waiter_.Wait());
    EXPECT_EQ(CALLBACK_SUCCESS, config_waiter_.callback_result());
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(nullptr, &big_mode_).c_str(), kModesetOutcomeSuccess, kCommitModesetStr,
                  GetCrtcActions(nullptr, &big_mode_).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    ...
    observer_.Reset();
    config_waiter_.Reset();
    configurator_.SetDisplayPower(chromeos::DISPLAY_POWER_ALL_OFF, DisplayConfigurator::kSetDisplayPowerNoFlags, config_waiter_.on_configuration_callback());
    EXPECT_EQ(kNoDelay, config_waiter_.Wait());
    EXPECT_EQ(CALLBACK_SUCCESS, config_waiter_.callback_result());
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(DisplayConfig::kOff, nullptr, nullptr).c_str(), kModesetOutcomeSuccess, kCommitModesetStr,
                  GetCrtcActions(DisplayConfig::kOff, nullptr, nullptr).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    ...
    observer_.Reset();
    config_waiter_.Reset();
    configurator_.SetDisplayPower(chromeos::DISPLAY_POWER_ALL_ON, DisplayConfigurator::kSetDisplayPowerNoFlags, config_waiter_.on_configuration_callback());
    EXPECT_EQ(kNoDelay, config_waiter_.Wait());
    EXPECT_EQ(CALLBACK_SUCCESS, config_waiter_.callback_result());
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(DisplayConfig::kMirror, &small_mode_, &small_mode_).c_str(), kModesetOutcomeSuccess,
                  kCommitModesetStr, GetCrtcActions(DisplayConfig::kMirror, &small_mode_, &small_mode_).c_str(), kModesetOutcomeSuccess, nullptr),
    ...
    EXPECT_EQ(kNoDelay, config_waiter_.Wait());
    EXPECT_EQ(CALLBACK_SUCCESS, config_waiter_.callback_result());
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(DisplayConfig::kOff, &small_mode_, &big_mode_).c_str(), kModesetOutcomeSuccess, kCommitModesetStr,
                  GetCrtcActions(DisplayConfig::kOff, &small_mode_, &big_mode_).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    EXPECT_EQ(MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED, configurator_.display_state());
    EXPECT_TRUE(mirroring_controller_.SoftwareMirroringEnabled());
    EXPECT_EQ(1, observer_.num_changes());
...
TEST_F(DisplayConfiguratorTest, SuspendAndResume)
{
    ...
    EXPECT_EQ(kNoDelay, config_waiter_.Wait());
    EXPECT_EQ(CALLBACK_SUCCESS, config_waiter_.callback_result());
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(nullptr).c_str(), kModesetOutcomeSuccess, kCommitModesetStr, GetCrtcActions(nullptr).c_str(),
    ...
    EXPECT_EQ(kNoDelay, config_waiter_.Wait());
    EXPECT_EQ(CALLBACK_SUCCESS, config_waiter_.callback_result());
    EXPECT_EQ(chromeos::DISPLAY_POWER_ALL_OFF, configurator_.current_power_state());
    EXPECT_EQ(MULTIPLE_DISPLAY_STATE_MULTI_MIRROR, configurator_.display_state());
    EXPECT_EQ(kNoActions, log_->GetActionsAndClear());
...
TEST_F(DisplayConfiguratorTest, Headless)
{
    ...
    UpdateOutputs(1, true);
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(&big_mode_).c_str(), kModesetOutcomeSuccess, kCommitModesetStr, GetCrtcActions(&big_mode_).c_str(),
                  kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
...
TEST_F(DisplayConfiguratorTest, StartWithTwoOutputs)
{
    ...
    state_controller_.set_state(MULTIPLE_DISPLAY_STATE_MULTI_MIRROR);
    configurator_.ForceInitialConfigure();
    EXPECT_EQ(JoinActions(kInit, kTestModesetStr, GetCrtcActions(DisplayConfig::kMirror, &small_mode_, &small_mode_).c_str(), kModesetOutcomeSuccess,
                  kCommitModesetStr, GetCrtcActions(DisplayConfig::kMirror, &small_mode_, &small_mode_).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
}
...
TEST_F(DisplayConfiguratorTest, GetMultipleDisplayStateForSWMirroredDisplays)
{
    ...
    scoped_feature_list_.InitAndDisableFeature(features::kEnableHardwareMirrorMode);
    UpdateOutputs(2, false);
    Init(false);
    state_controller_.set_state(MULTIPLE_DISPLAY_STATE_MULTI_MIRROR);
    configurator_.ForceInitialConfigure();
...
TEST_F(DisplayConfiguratorTest, VerifyInternalPanelIsAtTheTopOfTheList)
{
    ...
    // the display list.
    SetOutput(0, FakeDisplaySnapshot::Builder().SetId(1L).SetType(DISPLAY_CONNECTION_TYPE_DISPLAYPORT).SetNativeMode(big_mode_.Clone()).Build());
    SetOutput(1, FakeDisplaySnapshot::Builder().SetId(2L).SetType(DISPLAY_CONNECTION_TYPE_INTERNAL).SetNativeMode(small_mode_.Clone()).Build());
    SetOutput(2, FakeDisplaySnapshot::Builder().SetId(3L).SetType(DISPLAY_CONNECTION_TYPE_HDMI).SetNativeMode(big_mode_.Clone()).Build());

    native_display_delegate_->set_max_configurable_pixels(big_mode_.size().GetArea());
...
TEST_F(DisplayConfiguratorTest, HandleConfigureCrtcFailure)
{
    ...
    native_display_delegate_->set_max_configurable_pixels(modes[2]->size().GetArea());
    state_controller_.set_state(MULTIPLE_DISPLAY_STATE_SINGLE);
    UpdateOutputs(1, true);

    EXPECT_EQ(JoinActions(
                  // Initial attempt fails.
                  kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), GetOutput(0)->native_mode() }).c_str(), kModesetOutcomeFailure,
    ...
    EXPECT_EQ(JoinActions(
                  // Initial attempt fails.
                  kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get() }).c_str(), kModesetOutcomeFailure,
                  // Initiate retry logic.
                  kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get() }).c_str(), kModesetOutcomeFailure,
                  // Retry attempts trying all available modes.
                  kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[3].get() }).c_str(), kModesetOutcomeFailure,
                  kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[4].get() }).c_str(), kModesetOutcomeFailure,
                  // Test-modeset passes for this mode.
                  kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[2].get() }).c_str(), kModesetOutcomeSuccess,
                  kCommitModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[2].get() }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    ...
    native_display_delegate_->set_max_configurable_pixels(modes[1]->size().GetArea());
    state_controller_.set_state(MULTIPLE_DISPLAY_STATE_MULTI_MIRROR);
    UpdateOutputs(2, true);
    ...
    EXPECT_EQ(
        JoinActions(
            ...
            kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), GetOutput(0)->native_mode() }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, 0), modes[0].get() }).c_str(), kModesetOutcomeFailure,
            // We first test-modeset the internal display with all other displays
            // disabled, which will fail.
            kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), GetOutput(0)->native_mode() }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, 0), nullptr }).c_str(), kModesetOutcomeFailure,
            ...
            kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, 0), modes[0].get() }).c_str(), kModesetOutcomeFailure, kTestModesetStr,
            GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, 0), modes[3].get() }).c_str(), kModesetOutcomeFailure, kTestModesetStr,
            GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, 0), modes[4].get() }).c_str(), kModesetOutcomeFailure, kTestModesetStr,
            GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, 0), modes[2].get() }).c_str(), kModesetOutcomeFailure,
            // This configuration still passes intermediate test-modeset.
            kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, 0), modes[1].get() }).c_str(), kModesetOutcomeSuccess, kCommitModesetStr,
            GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, 0), modes[1].get() }).c_str(), kModesetOutcomeSuccess,
            // Since mirror mode configuration failed it should now attempt to
            // configure in extended mode. However, initial attempt fails.
            // Initiate retry logic.
            kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), GetOutput(0)->native_mode() }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, modes[0]->size().height() + DisplayConfigurator::kVerticalGap), modes[0].get() }).c_str(),
            kModesetOutcomeFailure,
            // We first test-modeset the internal display with all other displays
            // disabled, which will fail.
            kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), GetOutput(0)->native_mode() }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, modes[0]->size().height() + DisplayConfigurator::kVerticalGap), nullptr }).c_str(),
            kModesetOutcomeFailure,
            // The configuration fails completely but still attempts to modeset
            // the external display.
            kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, modes[0]->size().height() + DisplayConfigurator::kVerticalGap), modes[0].get() }).c_str(),
            kModesetOutcomeFailure, kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, modes[0]->size().height() + DisplayConfigurator::kVerticalGap), modes[3].get() }).c_str(),
            kModesetOutcomeFailure, kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, modes[0]->size().height() + DisplayConfigurator::kVerticalGap), modes[4].get() }).c_str(),
            kModesetOutcomeFailure, kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, modes[0]->size().height() + DisplayConfigurator::kVerticalGap), modes[2].get() }).c_str(),
            kModesetOutcomeFailure,
            // This configuration passes test-modeset.
            kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, modes[0]->size().height() + DisplayConfigurator::kVerticalGap), modes[1].get() }).c_str(),
            kModesetOutcomeSuccess, kCommitModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, modes[0]->size().height() + DisplayConfigurator::kVerticalGap), modes[1].get() }).c_str(),
            kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
}
...
TEST_F(DisplayConfiguratorTest, DontRestoreStalePowerStateAfterResume)
{
    ...
    EXPECT_EQ(1, observer_.num_changes());
    EXPECT_EQ(0, observer_.num_failures());
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(DisplayConfig::kMirror, nullptr, &big_mode_).c_str(), kModesetOutcomeSuccess, kCommitModesetStr,
                  GetCrtcActions(DisplayConfig::kMirror, nullptr, &big_mode_).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    ...
    configurator_.ResumeDisplays();
    EXPECT_EQ(kLongDelay, test_api_.GetConfigureDelay());
    EXPECT_EQ(chromeos::DISPLAY_POWER_ALL_ON, configurator_.current_power_state());
    EXPECT_EQ(kNoActions, log_->GetActionsAndClear());
...
TEST_F(DisplayConfiguratorTest, ExternalControl)
{
    ...
    configurator_.RelinquishControl(base::BindOnce(&DisplayConfiguratorTest::OnDisplayControlUpdated, base::Unretained(this)));
    EXPECT_EQ(CALLBACK_SUCCESS, PopDisplayControlResult());
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(nullptr).c_str(), kModesetOutcomeSuccess, kCommitModesetStr, GetCrtcActions(nullptr).c_str(),
                  kModesetOutcomeSuccess, kRelinquishDisplayControl, nullptr),
        log_->GetActionsAndClear());
    configurator_.TakeControl(base::BindOnce(&DisplayConfiguratorTest::OnDisplayControlUpdated, base::Unretained(this)));
    EXPECT_EQ(CALLBACK_SUCCESS, PopDisplayControlResult());
    EXPECT_EQ(JoinActions(kTakeDisplayControl, kTestModesetStr, GetCrtcActions(&small_mode_).c_str(), kModesetOutcomeSuccess, kCommitModesetStr,
                  GetCrtcActions(&small_mode_).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
}

TEST_F(DisplayConfiguratorTest, SetDisplayPowerWhilePendingConfigurationTaskRunning)
{
    // Start out with two displays in extended mode.
    state_controller_.set_state(MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED);
    Init(false);
    ...
    config_waiter_.Reset();
    EXPECT_EQ(base::Milliseconds(DisplayConfigurator::kConfigureDelayMs), config_waiter_.Wait());
    EXPECT_EQ(CALLBACK_NOT_CALLED, config_waiter_.callback_result());
    EXPECT_EQ(kNoDelay, config_waiter_.Wait());
    EXPECT_EQ(CALLBACK_SUCCESS, config_waiter_.callback_result());
    EXPECT_EQ(2, observer_.num_changes());
...
TEST_F(DisplayConfiguratorTest, SetDisplayPowerAfterFailedDisplayConfiguration)
{
    // Start out with two displays in extended mode.
    state_controller_.set_state(MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED);
    Init(false);
    ...
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(DisplayConfig::kOff, &small_mode_, &big_mode_).c_str(), kModesetOutcomeFailure, kTestModesetStr,
                  GetCrtcActions(DisplayConfig::kOff, &small_mode_, &big_mode_).c_str(), kModesetOutcomeFailure, kTestModesetStr,
                  GetCrtcActions(DisplayConfig::kOff, &small_mode_, &big_mode_).c_str(), kModesetOutcomeFailure, nullptr),
        log_->GetActionsAndClear());
    ...
    EXPECT_EQ(2, observer_.num_failures());
    EXPECT_EQ(
        JoinActions(kTestModesetStr, GetCrtcActions(&small_mode_, &big_mode_).c_str(), kModesetOutcomeFailure,
            ...
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, small_mode_.size().height() + DisplayConfigurator::kVerticalGap), nullptr }).c_str(),
            kModesetOutcomeFailure,
            // Since internal displays are restricted to their preferred mode,
            ...
            kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, small_mode_.size().height() + DisplayConfigurator::kVerticalGap), &big_mode_ }).c_str(),
            kModesetOutcomeFailure, kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), nullptr }).c_str(),
            GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, small_mode_.size().height() + DisplayConfigurator::kVerticalGap), &small_mode_ }).c_str(),
...
TEST_F(DisplayConfiguratorTest, TestWithThreeDisplays)
{
    ...
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(&small_mode_, &big_mode_, &small_mode_).c_str(), kModesetOutcomeSuccess, kCommitModesetStr,
                  GetCrtcActions(&small_mode_, &big_mode_, &small_mode_).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    ...
    EXPECT_EQ(kNoDelay, config_waiter_.Wait());
    EXPECT_EQ(CALLBACK_SUCCESS, config_waiter_.callback_result());
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcActions(DisplayConfig::kOff, &small_mode_, &big_mode_, &small_mode_).c_str(), kModesetOutcomeSuccess,
                  kCommitModesetStr, GetCrtcActions(DisplayConfig::kOff, &small_mode_, &big_mode_, &small_mode_).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
...
TEST_F(DisplayConfiguratorTest, RefreshRateThrottle_SingleDisplay)
{
    ...
    configurator_.SetRefreshRateOverrides({ std::make_pair(GetOutput(0)->display_id(), 60.f) });
    EXPECT_EQ(60.0f, GetOutput(0)->current_mode()->refresh_rate());
    EXPECT_EQ(1, observer_.num_changes());
    EXPECT_EQ(JoinActions(kTestModesetStr, kSeamlessModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[1].get(), false }).c_str(),
                  kModesetOutcomeSuccess, kCommitModesetStr, kSeamlessModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[1].get(), false }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    observer_.Reset();
    ...
    EXPECT_EQ(120.0f, GetOutput(0)->current_mode()->refresh_rate());
    EXPECT_EQ(1, observer_.num_changes());
    EXPECT_EQ(JoinActions(kTestModesetStr, kSeamlessModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get() }).c_str(),
                  kModesetOutcomeSuccess, kCommitModesetStr, kSeamlessModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get() }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
}
...
TEST_F(DisplayConfiguratorTest, RefreshRateThrottle_OverrideWithNative)
{
    ...
    configurator_.SetRefreshRateOverrides({ std::make_pair(GetOutput(0)->display_id(), 120.f) });
    EXPECT_EQ(120.0f, GetOutput(0)->current_mode()->refresh_rate());
    EXPECT_EQ(1, observer_.num_changes());

    // No override should be set.
    EXPECT_EQ(JoinActions(kTestModesetStr, kSeamlessModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get(), false }).c_str(),
                  kModesetOutcomeSuccess, kCommitModesetStr, kSeamlessModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get(), false }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    observer_.Reset();
}
...
TEST_F(DisplayConfiguratorTest, RefreshRateThrottle_WrongDisplayId)
{
    ...
    configurator_.SetRefreshRateOverrides({ std::make_pair(GetOutput(0)->display_id() + 100, 60.f) });
    EXPECT_EQ(0, observer_.num_changes());
    EXPECT_EQ(kNoActions, log_->GetActionsAndClear());
}
...
TEST_F(DisplayConfiguratorTest, RefreshRateThrottle_MultipleDisplays)
{
    ...
    EXPECT_EQ(120.0f, GetOutput(1)->current_mode()->refresh_rate());
    EXPECT_EQ(1, observer_.num_changes());
    int vertical_offset = GetOutput(0)->native_mode()->size().height() + DisplayConfigurator::kVerticalGap;
    EXPECT_EQ(JoinActions(kTestModesetStr, kSeamlessModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[1].get() }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), modes[0].get() }).c_str(), kModesetOutcomeSuccess,
                  kCommitModesetStr, kSeamlessModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[1].get() }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), modes[0].get() }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    observer_.Reset();
    observer_.Reset();
    ...
    EXPECT_EQ(JoinActions(kTestModesetStr, kSeamlessModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get() }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), modes[0].get() }).c_str(), kModesetOutcomeSuccess,
                  kCommitModesetStr, kSeamlessModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get() }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), modes[0].get() }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
}
...
TEST_F(DisplayConfiguratorTest, RefreshRateThrottle_RaceWithDockMode)
{
    ...
    test_api_.GetDisplayLayoutManager()->GetDisplayLayout(native_display_delegate_->GetOutputs(), MULTIPLE_DISPLAY_STATE_SINGLE,
...
TEST_F(DisplayConfiguratorTest, RefreshRateThrottle_StaysThrottledForSeamlessConfig)
{
    InitWithOutputs(&small_mode_);
    ...
    EXPECT_EQ(JoinActions(kTestModesetStr, kSeamlessModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[1].get() }).c_str(),
                  kModesetOutcomeSuccess, kCommitModesetStr, kSeamlessModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[1].get() }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    observer_.Reset();
    ...
    EXPECT_EQ(
        JoinActions(kTestModesetStr, kSeamlessModesetStr,
            GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[1].get(), /*enable_vrr=*/true }).c_str(), kModesetOutcomeSuccess,
            kCommitModesetStr, kSeamlessModesetStr,
            GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[1].get(), /*enable_vrr=*/true }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    observer_.Reset();
    ...
    EXPECT_EQ(
        JoinActions(kTestModesetStr, kSeamlessModesetStr,
            GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get(), /*enable_vrr=*/true }).c_str(), kModesetOutcomeSuccess,
            kCommitModesetStr, kSeamlessModesetStr,
            GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get(), /*enable_vrr=*/true }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
    observer_.Reset();
}

TEST_F(DisplayConfiguratorTest, RefreshRateThrottle_ResetThrottleForFullConfig)
{
    InitWithOutputs(&small_mode_);
    ...
    int vertical_offset = GetOutput(0)->native_mode()->size().height() + DisplayConfigurator::kVerticalGap;
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get() }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), &big_mode_ }).c_str(), kModesetOutcomeSuccess, kCommitModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get() }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), &big_mode_ }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_->GetActionsAndClear());
}
...
TEST_F(DisplayConfiguratorTest, SetVrrEnabled)
{
    ...
    // Set VRR enabled.
    configurator_.SetVrrEnabled({ GetOutput(0)->display_id(), GetOutput(1)->display_id() });
    EXPECT_EQ(1, observer_.num_changes());
    EXPECT_TRUE(GetOutput(0)->IsVrrEnabled());
    EXPECT_FALSE(GetOutput(1)->IsVrrEnabled());
    int vertical_offset = GetOutput(0)->native_mode()->size().height() + DisplayConfigurator::kVerticalGap;
    EXPECT_EQ(JoinActions(kTestModesetStr, kSeamlessModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), GetOutput(0)->native_mode(), /*enable_vrr=*/true }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), GetOutput(1)->native_mode(), /*enable_vrr=*/false }).c_str(),
                  kModesetOutcomeSuccess, kCommitModesetStr, kSeamlessModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), GetOutput(0)->native_mode(), /*enable_vrr=*/true }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), GetOutput(1)->native_mode(), /*enable_vrr=*/false }).c_str(),
                  kModesetOutcomeSuccess, nullptr),
    ...
    EXPECT_FALSE(GetOutput(1)->IsVrrEnabled());
    EXPECT_EQ(JoinActions(kTestModesetStr, kSeamlessModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), GetOutput(0)->native_mode(), /*enable_vrr=*/false }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), GetOutput(1)->native_mode(), /*enable_vrr=*/false }).c_str(),
                  kModesetOutcomeSuccess, kCommitModesetStr, kSeamlessModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), GetOutput(0)->native_mode(), /*enable_vrr=*/false }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), GetOutput(1)->native_mode(), /*enable_vrr=*/false }).c_str(),
...
TEST_F(DisplayConfiguratorTest, SetVrrEnabled_NotCapable)
{
    ...
    output->set_variable_refresh_rate_state(VariableRefreshRateState::kVrrNotCapable);
...
TEST_F(DisplayConfiguratorTest, RefreshRateThrottle_VrrEnabledOnExternalDisplay)
{
    InitWithOutputs(&small_mode_, &big_mode_);
    // Set up each display with HRR native mode and eligible throttle candidate
    // mode.
    ...
    // result in seamless modesets.
    EXPECT_EQ(JoinActions(kTestModesetStr, kSeamlessModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[1].get(), /*enable_vrr=*/false }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), &big_mode_,
                                    /*enable_vrr=*/true })
                      .c_str(),
                  kModesetOutcomeSuccess, kCommitModesetStr, kSeamlessModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[1].get(), /*enable_vrr=*/false }).c_str(),
    ...
    // still result in seamless modesets.
    EXPECT_EQ(JoinActions(kTestModesetStr, kSeamlessModesetStr,
                  GetCrtcAction({ GetOutput(0)->display_id(), gfx::Point(0, 0), modes[0].get(), /*enable_vrr=*/false }).c_str(),
                  GetCrtcAction({ GetOutput(1)->display_id(), gfx::Point(0, vertical_offset), &big_mode_,
                                    /*enable_vrr=*/true })
                      .c_str(),
                  kModesetOutcomeSuccess, kCommitModesetStr, kSeamlessModesetStr,
...
public:
    ...
    DisplayConfiguratorMultiMirroringTest(const DisplayConfiguratorMultiMirroringTest&) = delete;
    DisplayConfiguratorMultiMirroringTest& operator=(const DisplayConfiguratorMultiMirroringTest&) = delete;
    ...
    void TestHardwareMirrorModeExist(std::unique_ptr<DisplayMode> expected_mirror_mode)
    {
        UpdateOutputs(3, true);
        log_->GetActionsAndClear();
        observer_.Reset();
        ...
        EXPECT_EQ(JoinActions(kTestModesetStr,
                      GetCrtcActions(DisplayConfig::kMirror, expected_mirror_mode.get(), expected_mirror_mode.get(), expected_mirror_mode.get()).c_str(),
...
TEST_F(DisplayConfiguratorMultiMirroringTest, FindMirrorModeWithInternalDisplay)
{
...
TEST_F(DisplayConfiguratorMultiMirroringTest, FindMirrorModeWithoutInternalDisplay)
{
