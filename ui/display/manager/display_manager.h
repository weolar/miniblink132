...
class DISPLAY_MANAGER_EXPORT DisplayManager : public DisplayConfigurator::SoftwareMirroringController {
public:
    ...
    public:
        ...
        virtual void UpdateDisplayMetrics(const Display& display, uint32_t metrics) = 0;

        // Create or updates the mirroring window with |display_info_list|.
        virtual void CreateOrUpdateMirroringDisplay(const DisplayInfoList& display_info_list) = 0;
    ...
    int64_t first_display_id() const
    {
    ...
    // physical display).
    bool IsConnectedDisplayIdListInSyncWithCurrentState(const DisplayIdList& display_id_list) const;
    ...
    const Display& FindDisplayContainingPoint(const gfx::Point& point_in_screen) const;
    ...
    // also become active.
    void SetDisplayRotation(int64_t display_id, Display::Rotation rotation, Display::RotationSource source);
    ...
    bool SetDisplayMode(int64_t display_id, const ManagedDisplayMode& display_mode);
    ...
    // associated data.
    void RegisterDisplayProperty(int64_t display_id, Display::Rotation rotation, const gfx::Insets* overscan_insets, const gfx::Size& resolution_in_pixels,
        float device_scale_factor, float display_zoom_factor, const DisplaySizeToZoomFactorMap& display_zoom_factor_map, float refresh_rate, bool is_interlaced,
        VariableRefreshRateState variable_refresh_rate_state, const std::optional<float>& vsync_rate_min);

    // Register stored rotation properties for the internal display.
    void RegisterDisplayRotationProperties(bool rotation_lock, Display::Rotation rotation);
    ...
    bool GetActiveModeForDisplayId(int64_t display_id, ManagedDisplayMode* mode) const;

    // Returns true and fills in the display's selected |mode| if found, or false.
    bool GetSelectedModeForDisplayId(int64_t display_id, ManagedDisplayMode* mode) const;
    ...
    void SetSelectedModeForDisplayId(int64_t display_id, const ManagedDisplayMode& display_mode);
    ...
    void OnNativeDisplaysChanged(const std::vector<ManagedDisplayInfo>& display_info_list);
    ...
    {
        return is_updating_display_list_ ? active_only_display_list_ : active_display_list();
    }

    const Displays& active_display_list() const
    {
    ...
    int64_t mirroring_source_id() const
    {
        return mirroring_source_id_;
    }
    ...
    void set_external_display_mirror_info(const std::set<int64_t>& external_display_mirror_info)
    {
    ...
    void set_mixed_mirror_mode_params(std::optional<MixedMirrorModeParams> mixed_params)
    {
    ...
    void inc_screen_capture_active_counter()
    {
        ++screen_capture_active_counter_;
    }
    ...
    bool unified_desktop_enabled() const
    {
        return unified_desktop_enabled_;
    }
    ...
    // not in Unified mode.
    Display GetMirroringDisplayForUnifiedDesktop(DisplayPositionInUnifiedMatrix cell_position) const;
    ...
    bool ShouldSetMirrorModeOn(const DisplayIdList& id_list, bool should_check_hardware_mirroring);
    ...
    // extended.
    void SetMirrorMode(MirrorMode mode, const std::optional<MixedMirrorModeParams>& mixed_params);
    ...
    void SetTouchCalibrationData(int64_t display_id, const TouchCalibrationData::CalibrationPointPairQuad& point_pair_quad, const gfx::Size& display_bounds,
        const ui::TouchscreenDevice& touchdevice, bool apply_spatial_calibration);
    void ClearTouchCalibrationData(int64_t display_id, std::optional<ui::TouchscreenDevice> touchdevice);
    ...
    void NotifyDidProcessDisplayChanges(const DisplayManagerObserver::DisplayConfigurationChange& config_change);
...
private:
    ...
    public:
        explicit BeginEndNotifier(DisplayManager* display_manager, bool notify_on_pending_change_only = false);
    ...
    private:
        ...
        DisplayManagerObserver::DisplayConfigurationChange CreateConfigChange() const;
    ...
    void UpdateDisplaysWith(const std::vector<ManagedDisplayInfo>& updated_display_info_list);
    ...
    // offset using |origin|.
    Display CreateMirroringDisplayFromDisplayInfoById(int64_t display_id, const gfx::Point& origin, float scale);
    ...
    void UpdateNonPrimaryDisplayBoundsForLayout(Displays* display_list, std::vector<size_t>* updated_indices);
    ...
    void ApplyDisplayLayout(DisplayLayout* layout, Displays* display_list, std::vector<int64_t>* updated_ids);
...
template <> struct ScopedObservationTraits<display::DisplayManager, display::DisplayManagerObserver> {
    static void AddObserver(display::DisplayManager* source, display::DisplayManagerObserver* observer)
    {
        source->AddDisplayManagerObserver(observer);
    }
    static void RemoveObserver(display::DisplayManager* source, display::DisplayManagerObserver* observer)
    {
        source->RemoveDisplayManagerObserver(observer);
    }
};

template <> struct ScopedObservationTraits<display::DisplayManager, display::DisplayObserver> {
    static void AddObserver(display::DisplayManager* source, display::DisplayObserver* observer)
    {
        source->AddDisplayObserver(observer);
    }
    static void RemoveObserver(display::DisplayManager* source, display::DisplayObserver* observer)
    {
