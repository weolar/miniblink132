...
bool AddLegacyValuesFromValue(const base::Value::Dict& dict, DisplayLayout* layout)
{
...
bool UpdateFromDict(const base::Value::Dict& dict, const std::string& field_name, bool* output)
{
...
//     The key is missing - output is left unchanged
//     The key matches the type - output is updated to the dict.
bool UpdateFromDict(const base::Value::Dict& dict, const std::string& field_name, int* output)
{
    const base::Value* field = dict.Find(field_name);
    if (!field) {
        LOG(WARNING) << "Missing field: " << field_name;
...
//     The key is missing - output is left unchanged
//     The key matches the type - output is updated to the dict.
bool UpdateFromDict(const base::Value::Dict& dict, const std::string& field_name, DisplayPlacement::Position* output)
{
    const base::Value* field = dict.Find(field_name);
    if (!field) {
        LOG(WARNING) << "Missing field: " << field_name;
    ...
    return field_value->empty() ? true : DisplayPlacement::StringToPosition(*field_value, output);
}

// Returns true if
//     The key is missing - output is left unchanged
//     The key matches the type - output is updated to the dict.
bool UpdateFromDict(const base::Value::Dict& dict, const std::string& field_name, int64_t* output)
{
    const base::Value* field = dict.Find(field_name);
    if (!field) {
        LOG(WARNING) << "Missing field: " << field_name;
    ...
    return field_value->empty() ? true : base::StringToInt64(*field_value, output);
}

// Returns true if
//     The key is missing - output is left unchanged
//     The key matches the type - output is updated to the dict.
bool UpdateFromDict(const base::Value::Dict& dict, const std::string& field_name, std::vector<DisplayPlacement>* output)
{
    const base::Value* field = dict.Find(field_name);
    if (!field) {
        LOG(WARNING) << "Missing field: " << field_name;
    ...
    for (const base::Value& list_item : list) {
        ...
        if (!UpdateFromDict(item_dict, kOffsetKey, &item.offset) || !UpdateFromDict(item_dict, kPositionKey, &item.position)
            || !UpdateFromDict(item_dict, kDisplayPlacementDisplayIdKey, &item.display_id)
            || !UpdateFromDict(item_dict, kDisplayPlacementParentDisplayIdKey, &item.parent_display_id)) {
            return false;
        }
...
bool JsonToDisplayLayout(const base::Value::Dict& dict, DisplayLayout* layout)
{
    ...
    if (!UpdateFromDict(dict, kDefaultUnifiedKey, &layout->default_unified) || !UpdateFromDict(dict, kPrimaryIdKey, &layout->primary_id)) {
        return false;
    }
...
void DisplayLayoutToJson(const DisplayLayout& layout, base::Value::Dict& dict)
{
    ...
    for (const auto& placement : layout.placement_list) {
        ...
        placement_value.Set(kPositionKey, DisplayPlacement::PositionToString(placement.position));
        placement_value.Set(kOffsetKey, placement.offset);
        placement_value.Set(kDisplayPlacementDisplayIdKey, base::NumberToString(placement.display_id));
        placement_value.Set(kDisplayPlacementParentDisplayIdKey, base::NumberToString(placement.parent_display_id));
