...
class DISPLAY_MANAGER_EXPORT ApplyContentProtectionTask : public ContentProtectionManager::Task {
public:
    ...
    ApplyContentProtectionTask(DisplayLayoutManager* layout_manager, NativeDisplayDelegate* native_display_delegate,
    ...
    ApplyContentProtectionTask& operator=(const ApplyContentProtectionTask&) = delete;
...
private:
    void OnGetHDCPState(int64_t display_id, bool success, HDCPState state, ContentProtectionMethod protection_method);
    ...
    struct HdcpRequest {
        HdcpRequest(int64_t display_id_in, HDCPState state_in, ContentProtectionMethod protection_method_in)
