    ...
    : default_display_placement_(DisplayPlacement::RIGHT, 0)
{
    ...
    if (command_line->HasSwitch(switches::kSecondaryDisplayLayout)) {
        std::string value = command_line->GetSwitchValueASCII(switches::kSecondaryDisplayLayout);
        char layout;
        int offset = 0;
...
void DisplayLayoutStore::SetDefaultDisplayPlacement(const DisplayPlacement& placement)
{
    base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
    if (!command_line->HasSwitch(switches::kSecondaryDisplayLayout))
...
void DisplayLayoutStore::RegisterLayoutForDisplayIdList(const DisplayIdList& list, std::unique_ptr<DisplayLayout> layout)
{
    ...
    if (layout->placement_list.size() >= 1 && layout->placement_list[0].display_id == kInvalidDisplayId) {
    ...
    if (!DisplayLayout::Validate(list, *layout.get())) {
        NOTREACHED() << "Attempting to register an invalid layout: ids=" << DisplayIdListToString(list) << ", layout=" << layout->ToString();
    }

    layouts_[list] = std::move(layout);
}

const DisplayLayout& DisplayLayoutStore::GetRegisteredDisplayLayout(const DisplayIdList& list)
{
    return GetOrCreateRegisteredDisplayLayoutInternal(list, /*create=*/false);
}

void DisplayLayoutStore::UpdateDefaultUnified(const DisplayIdList& list, bool default_unified)
{
...
const DisplayLayout& DisplayLayoutStore::GetOrCreateRegisteredDisplayLayout(const DisplayIdList& list)
{
    return GetOrCreateRegisteredDisplayLayoutInternal(list, /*create=*/true);
}

DisplayLayout* DisplayLayoutStore::CreateDefaultDisplayLayout(const DisplayIdList& list)
{
...
const DisplayLayout& DisplayLayoutStore::GetOrCreateRegisteredDisplayLayoutInternal(const DisplayIdList& list, bool create_if_not_exist)
{
    ...
    const DisplayLayout* layout = iter != layouts_.end() ? iter->second.get() : CreateDefaultDisplayLayout(list);
