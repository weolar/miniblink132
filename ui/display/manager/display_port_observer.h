...
class DISPLAY_MANAGER_EXPORT DisplayPortObserver : public DisplayConfigurator::Observer {
public:
    explicit DisplayPortObserver(DisplayConfigurator* configurator, base::RepeatingCallback<void(const std::vector<uint32_t>&)> on_port_change_callback);
...
private:
    ...
    const base::RepeatingCallback<void(const std::vector<uint32_t>&)> on_port_change_callback_;
