...
class TestSoftwareMirroringController : public DisplayConfigurator::SoftwareMirroringController {
public:
    ...
    TestSoftwareMirroringController(const TestSoftwareMirroringController&) = delete;
    TestSoftwareMirroringController& operator=(const TestSoftwareMirroringController&) = delete;
...
public:
    ...
    void set_should_mirror(bool should_mirror)
    {
        should_mirror_ = should_mirror;
    }

    void set_display_state(MultipleDisplayState state)
    {
    ...
    void set_software_mirroring_controller(std::unique_ptr<DisplayConfigurator::SoftwareMirroringController> software_mirroring_controller)
    {
        software_mirroring_controller_ = std::move(software_mirroring_controller);
    }

    // DisplayConfigurator::DisplayLayoutManager:
    DisplayConfigurator::SoftwareMirroringController* GetSoftwareMirroringController() const override
    {
    ...
    bool GetDisplayLayout(const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays, MultipleDisplayState new_display_state,
        chromeos::DisplayPowerState new_power_state, const base::flat_set<int64_t>& new_vrr_state,
        std::vector<DisplayConfigureRequest>* requests) const override
    {
        ...
        for (DisplaySnapshot* display : displays) {
            ...
            const DisplayMode* request_mode = new_power_state == chromeos::DISPLAY_POWER_ALL_ON ? mode : nullptr;
            bool request_vrr_state = new_vrr_state.contains(display->display_id()) && display->IsVrrCapable();
            requests->push_back(DisplayConfigureRequest(display, request_mode, origin, request_vrr_state));
...
private:
    const DisplayMode* FindMirrorMode(const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays) const
    {
    ...
    std::unique_ptr<DisplayConfigurator::SoftwareMirroringController> software_mirroring_controller_;
};
...
public:
    ...
    UpdateDisplayConfigurationTaskTest(const UpdateDisplayConfigurationTaskTest&) = delete;
    UpdateDisplayConfigurationTaskTest& operator=(const UpdateDisplayConfigurationTaskTest&) = delete;
    ...
    void ResponseCallback(bool success, const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays,
...
{
    {
        UpdateDisplayConfigurationTask task(&delegate_, &layout_manager_, MULTIPLE_DISPLAY_STATE_HEADLESS, chromeos::DISPLAY_POWER_ALL_ON,
            DisplayConfigurator::kSetDisplayPowerNoFlags,
            /*new_vrr_state=*/ {}, /*refresh_rate_overrides=*/ {},
            /*force_configure=*/false, kConfigurationTypeFull, base::BindOnce(&UpdateDisplayConfigurationTaskTest::ResponseCallback, base::Unretained(this)));
        task.Run();
    }
...
TEST_F(UpdateDisplayConfigurationTaskTest, SingleConfiguration)
{
    ...
    {
        UpdateDisplayConfigurationTask task(&delegate_, &layout_manager_, MULTIPLE_DISPLAY_STATE_SINGLE, chromeos::DISPLAY_POWER_ALL_ON,
            DisplayConfigurator::kSetDisplayPowerNoFlags,
            /*new_vrr_state=*/ {}, /*refresh_rate_overrides=*/ {},
            /*force_configure=*/false, kConfigurationTypeFull, base::BindOnce(&UpdateDisplayConfigurationTaskTest::ResponseCallback, base::Unretained(this)));
        task.Run();
    ...
    EXPECT_EQ(chromeos::DISPLAY_POWER_ALL_ON, power_state_);
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), &small_mode_ }).c_str(), kModesetOutcomeSuccess,
                  kCommitModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), &small_mode_ }).c_str(), kModesetOutcomeSuccess, nullptr),
...
TEST_F(UpdateDisplayConfigurationTaskTest, ExtendedConfiguration)
{
    ...
    {
        UpdateDisplayConfigurationTask task(&delegate_, &layout_manager_, MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED, chromeos::DISPLAY_POWER_ALL_ON,
            DisplayConfigurator::kSetDisplayPowerNoFlags,
            /*new_vrr_state=*/ {}, /*refresh_rate_overrides=*/ {},
            /*force_configure=*/false, kConfigurationTypeFull, base::BindOnce(&UpdateDisplayConfigurationTaskTest::ResponseCallback, base::Unretained(this)));
        task.Run();
    ...
    EXPECT_EQ(chromeos::DISPLAY_POWER_ALL_ON, power_state_);
    EXPECT_EQ(
        JoinActions(kTestModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), &small_mode_ }).c_str(),
            GetCrtcAction({ displays_[1]->display_id(), gfx::Point(0, small_mode_.size().height()), &big_mode_ }).c_str(), kModesetOutcomeSuccess,
            kCommitModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), &small_mode_ }).c_str(),
            GetCrtcAction({ displays_[1]->display_id(), gfx::Point(0, small_mode_.size().height()), &big_mode_ }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_.GetActionsAndClear());
}
...
TEST_F(UpdateDisplayConfigurationTaskTest, MirrorConfiguration)
{
    ...
    {
        UpdateDisplayConfigurationTask task(&delegate_, &layout_manager_, MULTIPLE_DISPLAY_STATE_MULTI_MIRROR, chromeos::DISPLAY_POWER_ALL_ON,
            DisplayConfigurator::kSetDisplayPowerNoFlags,
            /*new_vrr_state=*/ {}, /*refresh_rate_overrides=*/ {},
            /*force_configure=*/false, kConfigurationTypeFull, base::BindOnce(&UpdateDisplayConfigurationTaskTest::ResponseCallback, base::Unretained(this)));
        task.Run();
    ...
    EXPECT_EQ(chromeos::DISPLAY_POWER_ALL_ON, power_state_);
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), &small_mode_ }).c_str(),
                  GetCrtcAction({ displays_[1]->display_id(), gfx::Point(), &small_mode_ }).c_str(), kModesetOutcomeSuccess, kCommitModesetStr,
                  GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), &small_mode_ }).c_str(),
                  GetCrtcAction({ displays_[1]->display_id(), gfx::Point(), &small_mode_ }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_.GetActionsAndClear());
}
...
TEST_F(UpdateDisplayConfigurationTaskTest, FailExtendedConfiguration)
{
    ...
    EXPECT_EQ(
        JoinActions(
            ...
            kTestModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), &small_mode_ }).c_str(),
            GetCrtcAction({ displays_[1]->display_id(), gfx::Point(0, small_mode_.size().height()), &big_mode_ }).c_str(), kModesetOutcomeFailure,
            // We first attempt to modeset the internal display with all
            // other displays disabled, which will fail.
            kTestModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), &small_mode_ }).c_str(),
            GetCrtcAction({ displays_[1]->display_id(), gfx::Point(0, small_mode_.size().height()), nullptr }).c_str(), kModesetOutcomeFailure,
            ...
            kTestModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), nullptr }).c_str(),
            GetCrtcAction({ displays_[1]->display_id(), gfx::Point(0, small_mode_.size().height()), &big_mode_ }).c_str(), kModesetOutcomeFailure,
            kTestModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), nullptr }).c_str(),
            GetCrtcAction({ displays_[1]->display_id(), gfx::Point(0, small_mode_.size().height()), &small_mode_ }).c_str(), kModesetOutcomeFailure, nullptr),
        log_.GetActionsAndClear());
}
...
TEST_F(UpdateDisplayConfigurationTaskTest, SingleChangePowerConfiguration)
{
    ...
    // Turn power off
    {
        UpdateDisplayConfigurationTask task(&delegate_, &layout_manager_, MULTIPLE_DISPLAY_STATE_SINGLE, chromeos::DISPLAY_POWER_ALL_OFF,
            DisplayConfigurator::kSetDisplayPowerNoFlags,
            /*new_vrr_state=*/ {}, /*refresh_rate_overrides=*/ {},
            /*force_configure=*/false, kConfigurationTypeFull, base::BindOnce(&UpdateDisplayConfigurationTaskTest::ResponseCallback, base::Unretained(this)));
        task.Run();
    ...
    EXPECT_EQ(JoinActions(kTestModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), nullptr }).c_str(), kModesetOutcomeSuccess,
                  kCommitModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), nullptr }).c_str(), kModesetOutcomeSuccess, nullptr),
        log_.GetActionsAndClear());
}
...
{
    layout_manager_.set_should_mirror(false);
    layout_manager_.set_software_mirroring_controller(std::make_unique<TestSoftwareMirroringController>());
    UpdateDisplays(2);
    ...
    EXPECT_TRUE(configuration_status_);
    EXPECT_EQ(MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED, display_state_);
    EXPECT_TRUE(layout_manager_.GetSoftwareMirroringController()->SoftwareMirroringEnabled());
    EXPECT_EQ(kNoActions, log_.GetActionsAndClear());
}

TEST_F(UpdateDisplayConfigurationTaskTest, ForceConfigurationWhileGoingToSoftwareMirror)
{
    layout_manager_.set_should_mirror(false);
    layout_manager_.set_software_mirroring_controller(std::make_unique<TestSoftwareMirroringController>());
    UpdateDisplays(2);
...

    {
        UpdateDisplayConfigurationTask task(&delegate_, &layout_manager_, MULTIPLE_DISPLAY_STATE_MULTI_MIRROR, chromeos::DISPLAY_POWER_ALL_ON,
            DisplayConfigurator::kSetDisplayPowerNoFlags,
            /*new_vrr_state=*/ {}, /*refresh_rate_overrides=*/ {},
            /*force_configure=*/true, kConfigurationTypeFull, base::BindOnce(&UpdateDisplayConfigurationTaskTest::ResponseCallback, base::Unretained(this)));
        task.Run();
    }
...
TEST_F(UpdateDisplayConfigurationTaskTest, VrrConfiguration)
{
    ...
    EXPECT_EQ(chromeos::DISPLAY_POWER_ALL_ON, power_state_);
    EXPECT_EQ(
        JoinActions(kTestModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), &small_mode_, /*enable_vrr=*/true }).c_str(),
            GetCrtcAction({ displays_[1]->display_id(), gfx::Point(0, small_mode_.size().height()), &big_mode_, /*enable_vrr=*/false }).c_str(),
            kModesetOutcomeSuccess, kCommitModesetStr, GetCrtcAction({ displays_[0]->display_id(), gfx::Point(), &small_mode_, /*enable_vrr=*/true }).c_str(),
            GetCrtcAction({ displays_[1]->display_id(), gfx::Point(0, small_mode_.size().height()), &big_mode_, /*enable_vrr=*/false }).c_str(),
