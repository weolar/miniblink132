...
bool AreModesEqual(const display::DisplayMode& lhs, const display::DisplayMode& rhs)
{
...
int64_t FakeDisplayDelegate::AddDisplay(const gfx::Size& display_size)
{
    ...
    int64_t id = GenerateDisplayID(kReservedManufacturerID, kProductCodeHash, ++next_display_id_);
...
void FakeDisplayDelegate::RelinquishDisplayControl(DisplayControlCallback callback)
{
    std::move(callback).Run(false);
}
    ...
    const std::vector<display::DisplayConfigurationParams>& config_requests, ConfigureCallback callback, display::ModesetFlags modeset_flags)
{
    ...
    for (const auto& config : config_requests) {
        ...
        if (config.mode) {
            ...
            auto snapshot = find_if(
                displays_.begin(), displays_.end(), [&config](std::unique_ptr<DisplaySnapshot>& snapshot) { return snapshot->display_id() == config.id; });
    ...
    configure_callbacks_.push(base::BindOnce(std::move(callback), config_requests, config_success));
    ...
    if (!configure_timer_.IsRunning()) {
        configure_timer_.Start(FROM_HERE, kConfigureDisplayDelay, this, &FakeDisplayDelegate::ConfigureDone);
    }
}

void FakeDisplayDelegate::SetHdcpKeyProp(int64_t display_id, const std::string& key, SetHdcpKeyPropCallback callback)
{
    std::move(callback).Run(false);
}

void FakeDisplayDelegate::GetHDCPState(const DisplaySnapshot& output, GetHDCPStateCallback callback)
{
    std::move(callback).Run(false, HDCP_STATE_UNDESIRED, CONTENT_PROTECTION_METHOD_NONE);
}

void FakeDisplayDelegate::SetHDCPState(const DisplaySnapshot& output, HDCPState state, ContentProtectionMethod protection_method, SetHDCPStateCallback callback)
{
    std::move(callback).Run(false);
}

void FakeDisplayDelegate::SetColorTemperatureAdjustment(int64_t display_id, const ColorTemperatureAdjustment& cta)
{
}

void FakeDisplayDelegate::SetColorCalibration(int64_t display_id, const ColorCalibration& calibration)
{
}

void FakeDisplayDelegate::SetGammaAdjustment(int64_t display_id, const GammaAdjustment& gamma)
{
}

void FakeDisplayDelegate::SetPrivacyScreen(int64_t display_id, bool enabled, SetPrivacyScreenCallback callback)
{
    std::move(callback).Run(false);
}

void FakeDisplayDelegate::GetSeamlessRefreshRates(int64_t display_id, GetSeamlessRefreshRatesCallback callback) const
{
...
void FakeDisplayDelegate::CreateDisplaysFromSpecString(const std::string& str)
{
    ...
    for (const std::string& part : base::SplitString(str, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
        int64_t id = GenerateDisplayID(kReservedManufacturerID, kProductCodeHash, next_display_id_);
        std::unique_ptr<DisplaySnapshot> snapshot = FakeDisplaySnapshot::CreateFromSpec(id, part);
        if (snapshot) {
