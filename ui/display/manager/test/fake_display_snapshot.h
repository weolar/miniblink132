...
public:
    ...
    public:
        ...
        Builder& SetHDRStaticMetadata(const gfx::HDRStaticMetadata& hdr_static_metadata);
        Builder& SetVariableRefreshRateState(VariableRefreshRateState variable_refresh_rate_state);
    ...
    private:
        ...
        VariableRefreshRateState variable_refresh_rate_state_ = VariableRefreshRateState::kVrrNotCapable;
    };

    FakeDisplaySnapshot(int64_t display_id, int64_t port_display_id, int64_t edid_display_id, uint16_t connector_index, const gfx::Point& origin,
        ...
        bool is_aspect_preserving_scaling, bool has_overscan, PrivacyScreenState privacy_screen_state, bool has_content_protection_key_,
        std::string display_name, const base::FilePath& sys_path, DisplayModeList modes, const DisplayMode* current_mode, const DisplayMode* native_mode,
        int64_t product_code, const gfx::Size& maximum_cursor_size, const DisplaySnapshot::ColorInfo& color_info,
    ...
    // description.
    static std::unique_ptr<DisplaySnapshot> CreateFromSpec(int64_t id, const std::string& spec);
};
