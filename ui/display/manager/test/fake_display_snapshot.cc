...
std::string_view ExtractSuffix(std::string_view* str, std::string_view delimiter)
{
    std::vector<std::string_view> parts = base::SplitStringPiece(*str, delimiter, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
std::unique_ptr<DisplayMode> ParseDisplayMode(const std::string& str)
{
    ...
    if (!RE2::FullMatch(str, "(\\d+)x(\\d+)(?:%(\\d+\\.?\\d*))?", &width, &height, &refresh_rate_str)) {
    ...
    if (!refresh_rate_str.empty() && !base::StringToDouble(refresh_rate_str, &refresh_rate)) {
        LOG(ERROR) << "Unable to parse display mode \"" << str << "\"";
        return nullptr;
    }

    return std::make_unique<DisplayMode>(gfx::Size { width, height }, false, static_cast<float>(refresh_rate));
}
...
bool HandleModes(FakeDisplaySnapshot::Builder* builder, std::string_view resolutions)
{
    for (const std::string& mode_str : base::SplitString(resolutions, ":", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY)) {
...
bool HandleOptions(FakeDisplaySnapshot::Builder* builder, std::string_view options)
{
...
std::unique_ptr<FakeDisplaySnapshot> Builder::Build()
{
    ...
    if (name_.empty())
        name_ = base::StringPrintf("Fake Display with port_id=%" PRId64 ", edid_id=%" PRId64, port_display_id_, edid_display_id_);
    ...
    gfx::Size physical_size = gfx::ScaleToRoundedSize(native_mode_->size(), PixelPitchMmFromDPI(dpi_));

    return std::make_unique<FakeDisplaySnapshot>(id_, port_display_id_, edid_display_id_, connector_index_, origin_, physical_size, type_, base_connector_id_,
        path_topology_, is_aspect_preserving_scaling_, has_overscan_, privacy_screen_state_, has_content_protection_key_, name_, sys_path_, std::move(modes_),
        current_mode_, native_mode_, product_code_, maximum_cursor_size_, color_info_, variable_refresh_rate_state_, DrmFormatsAndModifiers());
}
...
Builder& Builder::SetHDRStaticMetadata(const gfx::HDRStaticMetadata& hdr_static_metadata)
{
    color_info_.hdr_static_metadata = hdr_static_metadata;
    return *this;
}

Builder& Builder::SetVariableRefreshRateState(VariableRefreshRateState variable_refresh_rate_state)
{
...
const DisplayMode* Builder::AddOrFindDisplayMode(std::unique_ptr<DisplayMode> mode)
{
    for (auto& existing : modes_) {
        if (mode->size() == existing->size() && mode->is_interlaced() == existing->is_interlaced() && mode->refresh_rate() == existing->refresh_rate()) {
            return existing.get();
        }
    }
...
FakeDisplaySnapshot::FakeDisplaySnapshot(int64_t display_id, int64_t port_display_id, int64_t edid_display_id, uint16_t connector_index,
    const gfx::Point& origin, const gfx::Size& physical_size, DisplayConnectionType type, uint64_t base_connector_id,
    const std::vector<uint64_t>& path_topology, bool is_aspect_preserving_scaling, bool has_overscan, PrivacyScreenState privacy_screen_state,
    bool has_content_protection_key, std::string display_name, const base::FilePath& sys_path, DisplayModeList modes, const DisplayMode* current_mode,
    const DisplayMode* native_mode, int64_t product_code, const gfx::Size& maximum_cursor_size, const DisplaySnapshot::ColorInfo& color_info,
    VariableRefreshRateState variable_refresh_rate_state, const DrmFormatsAndModifiers& drm_formats_and_modifiers)
    : DisplaySnapshot(display_id, port_display_id, edid_display_id, connector_index, origin, physical_size, type, base_connector_id, path_topology,
        is_aspect_preserving_scaling, has_overscan, privacy_screen_state, has_content_protection_key, color_info, display_name, sys_path, std::move(modes),
        display::PanelOrientation::kNormal, std::vector<uint8_t>(), current_mode, native_mode, product_code, 2018 /*year_of_manufacture */, maximum_cursor_size,
...
// static
std::unique_ptr<DisplaySnapshot> FakeDisplaySnapshot::CreateFromSpec(int64_t id, const std::string& spec)
{
    ...
    std::unique_ptr<DisplayMode> native_mode = ParseDisplayMode(std::string(leftover));
    ...
    if (!HandleModes(&builder, resolutions) || !HandleDPI(&builder, dpi) || !HandleOptions(&builder, options)) {
        return nullptr;
    }
