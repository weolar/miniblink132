...
// is_interlaced.
ManagedDisplayInfo::ManagedDisplayModeList GetModeListWithAllRefreshRates(const DisplaySnapshot& output)
{
    ManagedDisplayInfo::ManagedDisplayModeList display_mode_list;
    for (const auto& mode_info : output.modes()) {
        display_mode_list.emplace_back(mode_info->size(), mode_info->refresh_rate(), mode_info->is_interlaced(), output.native_mode() == mode_info.get(), 1.0);
    }

    std::sort(display_mode_list.begin(), display_mode_list.end(), [](const ManagedDisplayMode& lhs, const ManagedDisplayMode& rhs) {
        return std::forward_as_tuple(lhs.size().width(), lhs.size().height(), lhs.refresh_rate(), lhs.is_interlaced())
            < std::forward_as_tuple(rhs.size().width(), rhs.size().height(), rhs.refresh_rate(), rhs.is_interlaced());
    });
...
{
    if (!base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisplayProperties)) {
        return std::nullopt;
    }

    std::optional<base::Value> display_switch_value
        = base::JSONReader::Read(base::CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kDisplayProperties));
...
// static
ManagedDisplayInfo::ManagedDisplayModeList DisplayChangeObserver::GetInternalManagedDisplayModeList(
    const ManagedDisplayInfo& display_info, const DisplaySnapshot& output)
{
    const DisplayMode* ui_native_mode = output.native_mode();
    ManagedDisplayMode native_mode(
        ui_native_mode->size(), ui_native_mode->refresh_rate(), ui_native_mode->is_interlaced(), true, display_info.device_scale_factor());
    return CreateInternalManagedDisplayModeList(native_mode);
}

// static
ManagedDisplayInfo::ManagedDisplayModeList DisplayChangeObserver::GetExternalManagedDisplayModeList(const DisplaySnapshot& output)
{
    ...
    struct SizeComparator {
        ...
        {
            return std::forward_as_tuple(lhs.width(), lhs.height()) < std::forward_as_tuple(rhs.width(), rhs.height());
        }
    };

    using DisplayModeMap = std::map<gfx::Size, ManagedDisplayMode, SizeComparator>;
    ...
    for (const auto& mode_info : output.modes()) {
        ...
        ManagedDisplayMode display_mode(mode_info->size(), mode_info->refresh_rate(), mode_info->is_interlaced(), output.native_mode() == mode_info.get(), 1.0);
        ...
        } else if (display_mode_it->second.is_interlaced() && !display_mode.is_interlaced()) {
            display_mode_it->second = std::move(display_mode);
        } else if (!display_mode.is_interlaced() && display_mode_it->second.refresh_rate() < display_mode.refresh_rate()) {
            display_mode_it->second = std::move(display_mode);
        }
    }
    ...
    if (output.native_mode()) {
        ...
        CHECK(it != display_mode_map.end(), base::NotFatalUntil::M130) << "Native mode must be part of the mode list.";
...
MultipleDisplayState DisplayChangeObserver::GetStateForDisplayIds(const DisplayConfigurator::DisplayStateList& display_states)
{
    UpdateInternalDisplay(display_states);
    if (display_states.size() == 1)
        return MULTIPLE_DISPLAY_STATE_SINGLE;
    DisplayIdList list = GenerateDisplayIdList(display_states, &DisplaySnapshot::display_id);
    return display_manager_->ShouldSetMirrorModeOn(list, /*should_check_hardware_mirroring=*/true) ? MULTIPLE_DISPLAY_STATE_MULTI_MIRROR
                                                                                                   : MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED;
}

bool DisplayChangeObserver::GetSelectedModeForDisplayId(int64_t display_id, ManagedDisplayMode* out_mode) const
{
    return display_manager_->GetSelectedModeForDisplayId(display_id, out_mode);
}

void DisplayChangeObserver::OnDisplayConfigurationChanged(const DisplayConfigurator::DisplayStateList& display_states)
{
    UpdateInternalDisplay(display_states);
    ...
    display_manager_->touch_device_manager()->AssociateTouchscreens(&displays, ui::DeviceDataManager::GetInstance()->GetTouchscreenDevices());
...
void DisplayChangeObserver::OnDisplayConfigurationChangeFailed(const DisplayConfigurator::DisplayStateList& displays, MultipleDisplayState failed_new_state)
{
...
void DisplayChangeObserver::OnInputDeviceConfigurationChanged(uint8_t input_device_types)
{
    if (input_device_types & ui::InputDeviceEventObserver::kTouchscreen) {
        ...
        const auto& cached_displays = display_manager_->configurator()->cached_displays();
...
// static
float DisplayChangeObserver::FindDeviceScaleFactor(float dpi, const gfx::Size& size_in_pixels)
{
    ...
    if (display::features::IsOledScaleFactorEnabled() && size_in_pixels == k12DisplaySizeHackXol) {
        return 1.2f;
    }
...
// static
ManagedDisplayInfo DisplayChangeObserver::CreateManagedDisplayInfo(const DisplaySnapshot* snapshot, const DisplayMode* mode_info, bool native,
    float device_scale_factor, float dpi, const std::string& name, const gfx::RoundedCornersF& panel_radii)
{
    ...
    if (snapshot->product_code() != DisplaySnapshot::kInvalidProductCode) {
        ...
        EdidParser::SplitProductCodeInManufacturerIdAndProductId(snapshot->product_code(), &manufacturer_id, &product_id);
        new_info.set_manufacturer_id(EdidParser::ManufacturerIdToString(manufacturer_id));
    ...
    new_info.set_is_aspect_preserving_scaling(snapshot->is_aspect_preserving_scaling());
    if (dpi)
        new_info.set_device_dpi(dpi);

    // TODO(crbug.com/40652358): Remove kEnableUseHDRTransferFunction usage when
    // HDR is fully supported on ChromeOS.
    const bool allow_high_bit_depth = base::FeatureList::IsEnabled(features::kUseHDRTransferFunction);
    new_info.set_display_color_spaces(CreateDisplayColorSpaces(snapshot->color_space(), allow_high_bit_depth, snapshot->hdr_static_metadata()));
    new_info.SetSnapshotColorSpace(snapshot->color_space());
    constexpr int32_t kNormalBitDepth = 8;
    new_info.set_bits_per_channel(allow_high_bit_depth ? snapshot->bits_per_channel() : kNormalBitDepth);
    ...
    new_info.set_variable_refresh_rate_state(snapshot->variable_refresh_rate_state());
    new_info.set_connection_type(snapshot->type());
    new_info.set_physical_size(snapshot->physical_size());

    ManagedDisplayInfo::ManagedDisplayModeList display_modes = (snapshot->type() == DISPLAY_CONNECTION_TYPE_INTERNAL)
...
void DisplayChangeObserver::UpdateInternalDisplay(const DisplayConfigurator::DisplayStateList& display_states)
{
    ...
    for (display::DisplaySnapshot* state : display_states) {
        if (state->type() == DISPLAY_CONNECTION_TYPE_INTERNAL
            || (force_first_display_internal && (!HasInternalDisplay() || IsInternalDisplayId(state->display_id())))) {
            if (HasInternalDisplay()) {
                DCHECK_EQ(Display::InternalDisplayId(), state->display_id());
            }
            SetInternalDisplayIds({ state->display_id() });

            if (state->native_mode() && (!display_manager_->IsDisplayIdValid(state->display_id()) || !state->current_mode())) {
                ...
                ManagedDisplayInfo new_info = CreateManagedDisplayInfoInternal(state, state->native_mode());
...
ManagedDisplayInfo DisplayChangeObserver::CreateManagedDisplayInfoInternal(const DisplaySnapshot* snapshot, const DisplayMode* mode_info)
{
    bool native = false;
    float device_scale_factor = 1.0f;
    // Sets dpi only if the screen size is valid.
    const float dpi = IsDisplaySizeValid(snapshot->physical_size()) ? kInchInMm * mode_info->size().width() / snapshot->physical_size().width() : 0;
    ...
    std::string name = (snapshot->type() == DISPLAY_CONNECTION_TYPE_INTERNAL) ? l10n_util::GetStringUTF8(IDS_DISPLAY_NAME_INTERNAL) : snapshot->display_name();
    ...
    if (display::features::IsRoundedDisplayEnabled() && snapshot->type() == display::DISPLAY_CONNECTION_TYPE_INTERNAL) {
        panel_radii = internal_panel_radii_.value_or(gfx::RoundedCornersF());
    }

    return CreateManagedDisplayInfo(snapshot, mode_info, native, device_scale_factor, dpi, name, panel_radii);
}
