...
class DISPLAY_MANAGER_EXPORT DefaultTouchTransformSetter : public TouchTransformSetter {
public:
    DefaultTouchTransformSetter();

    DefaultTouchTransformSetter(const DefaultTouchTransformSetter&) = delete;
    DefaultTouchTransformSetter& operator=(const DefaultTouchTransformSetter&) = delete;

    ~DefaultTouchTransformSetter() override;

    // TouchTransformSetter:
    void ConfigureTouchDevices(const std::vector<ui::TouchDeviceTransform>& transforms) override;
};
