...
public:
    ...
    // connected to.
    struct WeakComp {
        bool operator()(const TouchDeviceIdentifier& lhs, const TouchDeviceIdentifier& rhs) const
        {
    ...
    static TouchDeviceIdentifier FromDevice(const ui::TouchscreenDevice& touch_device);
...
private:
    static uint32_t GenerateIdentifier(std::string name, uint16_t vendor_id, uint16_t product_id);
    uint32_t id_;
...
struct DISPLAY_MANAGER_EXPORT TouchCalibrationData {
    ...
    static bool CalibrationPointPairCompare(const CalibrationPointPair& pair_1, const CalibrationPointPair& pair_2);

    TouchCalibrationData();
    TouchCalibrationData(const CalibrationPointPairQuad& point_pairs, const gfx::Size& bounds);
...
public:
    ...
    using TouchAssociationMap = std::map<TouchDeviceIdentifier, AssociationInfoMap, TouchDeviceIdentifier::WeakComp>;
    ...
    void AssociateTouchscreens(std::vector<ManagedDisplayInfo>* all_displays, const std::vector<ui::TouchscreenDevice>& all_devices);
    ...
    void AddTouchCalibrationData(const ui::TouchscreenDevice& device, int64_t display_id, const TouchCalibrationData& data);
    ...
    // |active_touch_associations_|.
    void AddTouchAssociation(const ui::TouchscreenDevice& device, int64_t display_id);
    ...
    // data.
    void ClearTouchCalibrationData(const ui::TouchscreenDevice& device, int64_t display_id);
    ...
    TouchCalibrationData GetCalibrationData(const ui::TouchscreenDevice& touchscreen, int64_t display_id = kInvalidDisplayId) const;

    // Returns true of the display identified by |display_id| is associated with
    // the touch device identified by |device|.
    bool DisplayHasTouchDevice(int64_t display_id, const ui::TouchscreenDevice& device) const;
    ...
    // this display.
    std::vector<ui::TouchscreenDevice> GetAssociatedTouchDevicesForDisplay(int64_t display_id) const;
    ...
    void RegisterTouchAssociations(const TouchAssociationMap& touch_associations, const PortAssociationMap& port_associations);
...
private:
    ...
    void AssociateInternalDevices(std::vector<ManagedDisplayInfo*>* displays, std::vector<ui::TouchscreenDevice>* devices);

    void AssociateDevicesWithCollision(std::vector<ManagedDisplayInfo*>* displays, std::vector<ui::TouchscreenDevice>* devices);

    void AssociateFromHistoricalData(std::vector<ManagedDisplayInfo*>* displays, std::vector<ui::TouchscreenDevice>* devices);

    void AssociateUsbDevices(std::vector<ManagedDisplayInfo*>* displays, std::vector<ui::TouchscreenDevice>* devices);

    void AssociateSameSizeDevices(std::vector<ManagedDisplayInfo*>* displays, std::vector<ui::TouchscreenDevice>* devices);

    void AssociateToSingleDisplay(std::vector<ManagedDisplayInfo*>* displays, std::vector<ui::TouchscreenDevice>* devices);

    void AssociateAnyRemainingDevices(std::vector<ManagedDisplayInfo*>* displays, std::vector<ui::TouchscreenDevice>* devices);

    void Associate(ManagedDisplayInfo* display, const ui::TouchscreenDevice& device);

    void AddTouchCalibrationDataImpl(const ui::TouchscreenDevice& device, int64_t display_id, const TouchCalibrationData* data);
...
DISPLAY_MANAGER_EXPORT std::ostream& operator<<(std::ostream& os, const TouchDeviceIdentifier& identifier);
