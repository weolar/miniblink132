...
// software.
const char kMirroringImplementationHistogram[] = "DisplayManager.MirroringImplementation";

// The UMA histogram that logs the zoom percentage level of the internal
// display.
constexpr char kInternalDisplayZoomPercentageHistogram[] = "DisplayManager.InternalDisplayZoomPercentage";
...
ManagedDisplayInfo::ManagedDisplayModeList::const_iterator FindDisplayMode(const ManagedDisplayInfo& info, const ManagedDisplayMode& target_mode)
{
    const ManagedDisplayInfo::ManagedDisplayModeList& modes = info.display_modes();
    return base::ranges::find_if(modes, [target_mode](const ManagedDisplayMode& mode) { return target_mode.IsEquivalent(mode); });
}

void SetInternalManagedDisplayModeList(ManagedDisplayInfo* info)
{
    ManagedDisplayMode native_mode(
        info->bounds_in_native().size(), 0.0 /* refresh_rate */, false /* interlaced */, true /* native_mode */, info->device_scale_factor());
    info->SetManagedDisplayModes(CreateInternalManagedDisplayModeList(native_mode));
}
...
bool ContainsDisplayWithId(const std::vector<Display>& displays, int64_t display_id)
{
...
const ManagedDisplayMode* FindNextMode(const ManagedDisplayInfo::ManagedDisplayModeList& modes, size_t index, bool up)
{
...
bool GetDisplayModeForNextResolution(const ManagedDisplayInfo& info, bool up, ManagedDisplayMode* mode)
{
    DCHECK(!IsInternalDisplayId(info.id()));

    const ManagedDisplayInfo::ManagedDisplayModeList& modes = info.display_modes();
    ManagedDisplayMode tmp(info.size_in_pixel(), 0.0, false, false, info.device_scale_factor());
    const gfx::Size resolution = tmp.GetSizeInDIP();

    auto iter = base::ranges::find(modes, resolution, &ManagedDisplayMode::GetSizeInDIP);
    if (iter == modes.end()) {
        return false;
    }
...
const ManagedDisplayInfo* FindInfoById(const DisplayInfoList& display_info_list, int64_t id)
{
    const auto iter = base::ranges::find(display_info_list, id, &ManagedDisplayInfo::id);
...
bool ValidateMatrixForDisplayInfoList(const DisplayInfoList& display_info_list, const UnifiedDesktopLayoutMatrix& matrix)
{
...
void OnInternalDisplayZoomChanged(float zoom_factor)
{
    ...
    base::LinearHistogram::FactoryGet(
        kInternalDisplayZoomPercentageHistogram, kBucketSize, kMaxValue, kNumBuckets, base::HistogramBase::kUmaTargetedHistogramFlag)
...
float GetSdrLumForScreenBrightness(float percent, float hdr_max_lum)
{
    ...
    if (percent < brightness_pivot) {
        ...
        sdr_lum = ((percent - 100.f) * (hdr_max_lum - sdr_avg)) / (100.f - brightness_pivot);
        sdr_lum += hdr_max_lum;
    }
...
gfx::DisplayColorSpaces UpdateMaxLuminanceValue(const gfx::DisplayColorSpaces display_color_spaces, float brightness)
{
    ...
    // brightness level.
    auto hdr_space = display_color_spaces.GetOutputColorSpace(gfx::ContentColorUsage::kHDR, false);
    if (!hdr_space.IsAffectedBySDRWhiteLevel()) {
        return display_color_spaces;
    }

    float hdr_max = display_color_spaces.GetHDRMaxLuminanceRelative() * display_color_spaces.GetSDRMaxLuminanceNits();
...
DisplayManager::BeginEndNotifier::BeginEndNotifier(DisplayManager* display_manager, bool notify_on_pending_change_only)
...
DisplayManager::BeginEndNotifier::~BeginEndNotifier()
{
    if (--display_manager_->notify_depth_ == 0) {
        CHECK(display_manager_->pending_display_changes_.has_value());
        const bool has_pending_changes = !display_manager_->pending_display_changes_->IsEmpty();
        ...
        const DisplayManagerObserver::DisplayConfigurationChange config_change = CreateConfigChange();
...
DisplayManagerObserver::DisplayConfigurationChange DisplayManager::BeginEndNotifier::CreateConfigChange() const
{
    CHECK(display_manager_->pending_display_changes_.has_value());
    PendingDisplayChanges& pending_changes = display_manager_->pending_display_changes_.value();
    ...
    std::vector<DisplayManagerObserver::DisplayMetricsChange> display_metrics_changes;
    for (const auto& pair : pending_changes.display_metrics_changes) {
        if (display_manager_->IsDisplayIdValid(pair.first)) {
            display_metrics_changes.emplace_back(DisplayManagerObserver::DisplayMetricsChange(display_manager_->GetDisplayForId(pair.first), pair.second));
        }
    }

    return { std::move(added_displays), std::move(pending_changes.removed_displays), std::move(display_metrics_changes) };
}
...
{
    return added_display_ids.empty() && removed_displays.empty() && display_metrics_changes.empty();
}
    ...
    , layout_store_(new DisplayLayoutStore)
{
    ...
    unified_desktop_enabled_ = base::CommandLine::ForCurrentProcess()->HasSwitch(::switches::kEnableUnifiedDesktop);
...
bool DisplayManager::InitFromCommandLine()
{
    ...
    const std::string specs = command_line->GetSwitchValueASCII(::switches::kHostWindowBounds);

    // If the origin is not specified, put the host window next to the previous.
    int next_x = 0;
    for (const std::string& part : base::SplitString(specs, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
...
void DisplayManager::UpdateInternalDisplay(const ManagedDisplayInfo& display_info)
{
...
{
    gfx::SetFontRenderParamsDeviceScaleFactor(chromeos::GetRepresentativeDeviceScaleFactor(active_display_list_));
}
...
{
    return current_resolved_layout_ ? *current_resolved_layout_ : GetCurrentDisplayLayout();
}
...
bool DisplayManager::IsConnectedDisplayIdListInSyncWithCurrentState(const DisplayIdList& display_id_list) const
{
#if DCHECK_IS_ON()
    ...
    DisplayIdList software_mirroring_display_id_list = CreateDisplayIdList(software_mirroring_display_list_);
    connected_display_id_list.insert(connected_display_id_list.end(), software_mirroring_display_id_list.begin(), software_mirroring_display_id_list.end());
    connected_display_id_list.insert(connected_display_id_list.end(), hardware_mirroring_display_id_list_.begin(), hardware_mirroring_display_id_list_.end());
...
void DisplayManager::SetLayoutForCurrentDisplays(std::unique_ptr<DisplayLayout> layout)
{
    if (GetNumDisplays() == 1) {
        return;
    }
    ...
    const DisplayLayout& current_layout = layout_store_->GetRegisteredDisplayLayout(list);
    ...
    ApplyDisplayLayout(current_resolved_layout_.get(), &active_display_list_, &updated_ids);
    for (int64_t id : updated_ids) {
        NotifyMetricsChanged(GetDisplayForId(id), DisplayObserver::DISPLAY_METRIC_BOUNDS | DisplayObserver::DISPLAY_METRIC_WORK_AREA);
        CHECK(pending_display_changes_.has_value());
        pending_display_changes_->display_metrics_changes[id] |= DisplayObserver::DISPLAY_METRIC_BOUNDS | DisplayObserver::DISPLAY_METRIC_WORK_AREA;
    }
...
{
    auto* display = const_cast<DisplayManager*>(this)->FindDisplayForId(display_id);
...
{
    Display* display = const_cast<DisplayManager*>(this)->FindDisplayForId(display_id);
    return !!display;
}

const Display& DisplayManager::FindDisplayContainingPoint(const gfx::Point& point_in_screen) const
{
    const Displays& active_only_displays = active_only_display_list();
    auto iter = display::FindDisplayContainingPoint(active_only_displays, point_in_screen);
    return iter == active_only_displays.end() ? GetInvalidDisplay() : *iter;
}

bool DisplayManager::UpdateWorkAreaOfDisplay(int64_t display_id, const gfx::Insets& insets)
{
    BeginEndNotifier notifier(this);
    ...
    bool in_display_creation = in_creating_display_.has_value() && in_creating_display_.value() == display_id;
    ...
    if (workarea_changed && !in_display_creation) {
        ...
        CHECK(pending_display_changes_.has_value());
        pending_display_changes_->display_metrics_changes[display_id] |= DisplayObserver::DISPLAY_METRIC_WORK_AREA;
    }
    return workarea_changed;
}

void DisplayManager::SetOverscanInsets(int64_t display_id, const gfx::Insets& insets_in_dip)
{
    bool update = false;
...
void DisplayManager::SetDisplayRotation(int64_t display_id, Display::Rotation rotation, Display::RotationSource source)
{
    ...
    for (const auto& display : active_display_list_) {
        ManagedDisplayInfo info = GetDisplayInfo(display.id());
        if (info.id() == display_id) {
            if (info.GetRotation(source) == rotation && info.GetActiveRotation() == rotation) {
                return;
            }
...
void DisplayManager::OnScreenBrightnessChanged(float brightness)
{
    ...
    for (const auto& display : active_display_list_) {
        ...
        auto updated_display_color_spaces = UpdateMaxLuminanceValue(info.display_color_spaces(), brightness);
...
bool DisplayManager::SetDisplayMode(int64_t display_id, const ManagedDisplayMode& display_mode)
{
    DisplayInfoList display_info_list;
    bool display_property_changed = false;
        ...
        if (info.id() == display_id) {
            ...
            if (iter == info.display_modes().end()) {
                DLOG(WARNING) << "Unsupported display mode was requested:"
                              << "size=" << display_mode.size().ToString() << ", scale factor=" << display_mode.device_scale_factor();
                return false;
            }
            ...
            if (info.bounds_in_native().size() != display_mode.size()) {
                ...
                const DisplaySizeToZoomFactorMap& zoom_factor_map = info.zoom_factor_map();
...
void DisplayManager::RegisterDisplayProperty(int64_t display_id, Display::Rotation rotation, const gfx::Insets* overscan_insets,
    const gfx::Size& resolution_in_pixels, float device_scale_factor, float display_zoom_factor, const DisplaySizeToZoomFactorMap& display_zoom_factor_map,
    float refresh_rate, bool is_interlaced, VariableRefreshRateState variable_refresh_rate_state, const std::optional<float>& vsync_rate_min)
{
    if (display_info_.find(display_id) == display_info_.end()) {
        display_info_[display_id] = ManagedDisplayInfo(display_id, std::string(), false);
    }
    ...
    if (!resolution_in_pixels.IsEmpty()) {
        ...
        ManagedDisplayMode mode(resolution_in_pixels, refresh_rate, is_interlaced, false, device_scale_factor);
        display_modes_[display_id] = mode;
    }
}

bool DisplayManager::GetActiveModeForDisplayId(int64_t display_id, ManagedDisplayMode* mode) const
{
    ...
    const ManagedDisplayInfo::ManagedDisplayModeList& display_modes = info.display_modes();
    const ManagedDisplayMode current_mode(info.bounds_in_native().size(), info.refresh_rate(), info.is_interlaced(), info.native(), info.device_scale_factor());
...
void DisplayManager::RegisterDisplayRotationProperties(bool rotation_lock, Display::Rotation rotation)
{
    if (delegate_) {
        NotifyWillApplyDisplayChanges(false);
    }
...
bool DisplayManager::GetSelectedModeForDisplayId(int64_t display_id, ManagedDisplayMode* mode) const
{
...
void DisplayManager::SetSelectedModeForDisplayId(int64_t display_id, const ManagedDisplayMode& display_mode)
{
...
gfx::Insets DisplayManager::GetOverscanInsets(int64_t display_id) const
{
    ...
    return (it != display_info_.end()) ? it->second.overscan_insets_in_dip() : gfx::Insets();
}

void DisplayManager::OnNativeDisplaysChanged(const DisplayInfoList& updated_displays)
{
    DISPLAY_LOG(EVENT) << "Native displays updated"
                       << ". Unified desktop allowed: " << unified_desktop_enabled_ << ", Multi display mode: " << ToString(multi_display_mode_)
                       << ", count:" << updated_displays.size() << " currently active:" << active_display_list_.size();
    ...
    if (updated_displays.empty()) {
        ...
        if (active_display_list_.empty()) {
            ...
            init_displays.push_back(ManagedDisplayInfo::CreateFromSpec(std::string()));
            ...
            for (auto& display : active_display_list_) {
                if (display.detected()) {
                    ...
                    NotifyMetricsChanged(display, DisplayObserver::DISPLAY_METRIC_DETECTED);
                    CHECK(pending_display_changes_.has_value());
                    pending_display_changes_->display_metrics_changes[display.id()] |= DisplayObserver::DISPLAY_METRIC_DETECTED;
                }
            }
        }
        return;
    ...
    for (const auto& display_info : updated_displays) {
        ...
        ManagedDisplayMode new_mode(display_info.bounds_in_native().size(), display_info.refresh_rate(), display_info.is_interlaced(), display_info.native(),
            display_info.device_scale_factor());

        const ManagedDisplayInfo::ManagedDisplayModeList& display_modes = display_info.display_modes();
    ...
    if (HasInternalDisplay() && !internal_display_connected) {
        if (display_info_.find(Display::InternalDisplayId()) == display_info_.end()) {
            // Create a dummy internal display if the chrome restarted
            // in docked mode.
            ManagedDisplayInfo internal_display_info(
                Display::InternalDisplayId(), l10n_util::GetStringUTF8(IDS_DISPLAY_NAME_INTERNAL), false /*Internal display must not have overscan */);
            ...
            // active again.
            Display::Rotation user_rotation = display_info_[Display::InternalDisplayId()].GetRotation(Display::RotationSource::USER);
            display_info_[Display::InternalDisplayId()].SetRotation(user_rotation, Display::RotationSource::USER);
        }
    }

    if (!configure_displays_ && new_display_info_list.size() > 1 && hardware_mirroring_display_id_list.empty()) {
        ...
        bool should_enable_software_mirroring
            = base::CommandLine::ForCurrentProcess()->HasSwitch(::switches::kEnableSoftwareMirroring) || ShouldSetMirrorModeOn(list, unified_desktop_enabled_);
...
void DisplayManager::UpdateDisplaysWith(const DisplayInfoList& updated_display_info_list)
{
    base::AutoReset<bool> is_updating_displays_resetter(&is_updating_displays_, true);

    BeginEndNotifier notifier(this);

    DisplayInfoList new_display_info_list = updated_display_info_list;
    std::sort(active_display_list_.begin(), active_display_list_.end(), DisplaySortFunctor());
    std::sort(new_display_info_list.begin(), new_display_info_list.end(), DisplayInfoSortFunctor());

    DisplayIdList new_display_id_list = CreateDisplayIdList(new_display_info_list);
    ...
    if (num_connected_displays() > 1) {
        ...
        const DisplayLayout& layout = layout_store_->GetOrCreateRegisteredDisplayLayout(connected_display_id_list);
        current_default_multi_display_mode_ = (layout.default_unified && unified_desktop_enabled_) ? UNIFIED : EXTENDED;
    }

    if (multi_display_mode_ != MIRRORING
        || (mixed_mirror_mode_params_
            && ValidateParamsForMixedMirrorMode(new_display_id_list, *mixed_mirror_mode_params_) != MixedMirrorModeParamsErrors::kSuccess)) {
    ...
    UMA_HISTOGRAM_ENUMERATION("DisplayManager.MultiDisplayMode", multi_display_mode_, MULTI_DISPLAY_MODE_LAST + 1);
    ...
    while (curr_iter != active_display_list_.end() || new_info_iter != new_display_info_list.end()) {
        if (curr_iter == active_display_list_.end()) {
            ...
            new_displays.push_back(CreateDisplayFromDisplayInfoById(new_info_iter->id()));
            ++new_info_iter;
            ...
            // instance.
            const ManagedDisplayInfo current_display_info = GetDisplayInfo(current_display.id());
            InsertAndUpdateDisplayInfo(*new_info_iter);
            Display new_display = CreateDisplayFromDisplayInfoById(new_info_iter->id());
            const ManagedDisplayInfo& new_display_info = GetDisplayInfo(new_display.id());
            ...
            // time.
            if (force_bounds_changed_ || (current_display_info.bounds_in_native() != new_display_info.bounds_in_native())
                || (current_display_info.GetOverscanInsetsInPixel() != new_display_info.GetOverscanInsetsInPixel())
                || current_display.size() != new_display.size()) {
                metrics |= DisplayObserver::DISPLAY_METRIC_BOUNDS | DisplayObserver::DISPLAY_METRIC_WORK_AREA;
            }

            if (current_display.device_scale_factor() != new_display.device_scale_factor()) {
            ...
            if (!WithinEpsilon(current_display.display_frequency(), new_display.display_frequency())) {
                metrics |= DisplayObserver::DISPLAY_METRIC_REFRESH_RATE;
            }

            if (current_display_info.is_interlaced() != new_display_info.is_interlaced()) {
            ...
            if (current_display_info.variable_refresh_rate_state() != new_display_info.variable_refresh_rate_state()
                || current_display_info.vsync_rate_min() != new_display_info.vsync_rate_min()) {
                metrics |= DisplayObserver::DISPLAY_METRIC_VRR;
            }

            if (current_display_info.display_color_spaces() != new_display_info.display_color_spaces()) {
            ...
            if (metrics != DisplayObserver::DISPLAY_METRIC_NONE) {
                display_changes.insert(std::pair<size_t, uint32_t>(new_displays.size(), metrics));
            }
    ...
    bool clear_focus = !removed_displays.empty() && !(removed_displays.size() == 1 && added_display_indices.size() == 1);
    if (delegate_) {
    ...
    for (size_t updated_index : updated_indices) {
        if (!base::Contains(added_display_indices, updated_index)) {
            uint32_t metrics = DisplayObserver::DISPLAY_METRIC_BOUNDS | DisplayObserver::DISPLAY_METRIC_WORK_AREA;
    ...
    active_display_list_.insert(active_display_list_.end(), removed_displays.begin(), removed_displays.end());
    ...
    if (notify_primary_change) {
        ...
        if (primary.id() != old_primary.id()) {
            ...
            if (primary.size() != old_primary.size()) {
                primary_metrics |= (DisplayObserver::DISPLAY_METRIC_BOUNDS | DisplayObserver::DISPLAY_METRIC_WORK_AREA);
            }
    ...
    if (delegate_ && primary_metrics) {
        ...
        const auto primary_index_it = std::find(active_display_list_.begin(), active_display_list_.end(), primary);
        CHECK_EQ(primary.id(), screen_->GetPrimaryDisplay().id()) << "Primary changed during displays update.";
        CHECK(primary_index_it != active_display_list_.end()) << "Primary display not in display list.";
        const size_t primary_index = std::distance(active_display_list_.begin(), primary_index_it);
    ...
    // Populate the pending change structure.
    {
        ...
        for (const auto& pair : display_changes) {
            ...
            pending_display_changes_->display_metrics_changes[display_id] |= pair.second;
        }
    }

    if (mirror_mode) {
        UMA_HISTOGRAM_ENUMERATION(
            kMirroringImplementationHistogram, IsInSoftwareMirrorMode() ? MirroringImplementation::kSoftware : MirroringImplementation::kHardware);
        UMA_HISTOGRAM_ENUMERATION(
            kMirrorModeTypesHistogram, mixed_mirror_mode_params_ ? MirrorModeTypes::kMixed : MirrorModeTypes::kNormal, MirrorModeTypes::kCount);
    }
...
const Display& DisplayManager::GetPrimaryDisplayCandidate() const
{
    ...
    const DisplayLayout& layout = layout_store_->GetRegisteredDisplayLayout(GetConnectedDisplayIdList());
...
const Display& DisplayManager::GetFakePrimaryDisplay()
{
    ...
    if (!fake_display) {
        ...
        gfx::DisplayColorSpaces display_color_spaces(gfx::ColorSpace::CreateSRGB(), DisplaySnapshot::PrimaryFormat());
...
{
    if (multi_display_mode_ != MIRRORING || software_mirroring_display_list_.empty()) {
        return false;
    }
...
void DisplayManager::SetUnifiedDesktopEnabled(bool enable)
{
    ...
    DISPLAY_LOG(EVENT) << "Unified Desktop is now " << (enable ? "" : "not ") << "allowed."
                       << (IsInMirrorMode() ? " The displays will not be reconfigured since "
...
{
    return multi_display_mode_ == UNIFIED && !software_mirroring_display_list_.empty();
}

void DisplayManager::SetUnifiedDesktopMatrix(const UnifiedDesktopLayoutMatrix& matrix)
{
    current_unified_desktop_matrix_ = matrix;
    SetDefaultMultiDisplayModeForCurrentDisplays(UNIFIED);
}

Display DisplayManager::GetMirroringDisplayForUnifiedDesktop(DisplayPositionInUnifiedMatrix cell_position) const
{
    if (!IsInUnifiedMode()) {
        return Display();
    }
...
int DisplayManager::GetMirroringDisplayRowIndexInUnifiedMatrix(int64_t display_id) const
{
...
const ManagedDisplayInfo& DisplayManager::GetDisplayInfo(int64_t display_id) const
{
...
const Display DisplayManager::GetMirroringDisplayById(int64_t display_id) const
{
    auto iter = base::ranges::find(software_mirroring_display_list_, display_id, &Display::id);
    return iter == software_mirroring_display_list_.end() ? GetInvalidDisplay() : *iter;
}
...
bool DisplayManager::ShouldSetMirrorModeOn(const DisplayIdList& new_id_list, bool should_check_hardware_mirroring)
{
    ...
    if (should_restore_mirror_mode_from_display_prefs_ || num_connected_displays() <= 1) {
        ...
        for (int64_t id : new_id_list) {
            if (external_display_mirror_info_.count(GetDisplayIdWithoutOutputIndex(id))) {
                return true;
            }
        }
    }
    ...
    // connected displays.
    return IsInSoftwareMirrorMode() || (should_check_hardware_mirroring && IsInHardwareMirrorMode());
}

void DisplayManager::SetMirrorMode(MirrorMode mode, const std::optional<MixedMirrorModeParams>& mixed_params)
{
    ...
    if (mode != MirrorMode::kNormal && layout_store_->forced_mirror_mode_for_tablet()) {
    ...
    if (configure_displays_) {
        MultipleDisplayState new_state = enabled ? MULTIPLE_DISPLAY_STATE_MULTI_MIRROR : MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED;
        display_configurator_->SetMultipleDisplayState(new_state);
        return;
    }
    multi_display_mode_ = enabled ? MIRRORING : current_default_multi_display_mode_;
    ReconfigureDisplays();
}
...
void DisplayManager::AddRemoveDisplay()
{
    ...
    const ManagedDisplayInfo& first_display
        = IsInUnifiedMode() ? GetDisplayInfo(software_mirroring_display_list_[0].id()) : GetDisplayInfo(active_display_list_[0].id());
    ...
    if (num_connected_displays() == 1) {
        ...
        new_display_info_list.push_back(ManagedDisplayInfo::CreateFromSpec(base::StringPrintf(
            "%d+%d-%dx%d", host_bounds.x(), host_bounds.bottom() + kVerticalOffsetPx, host_bounds.height() + kExtraWidth, host_bounds.height())));
        // Reconnect the same display.
        new_display_info_list[1].set_display_id(new_display_info_list[0].id() + 0xFFFF);
    }
...
void DisplayManager::InitConfigurator(std::unique_ptr<NativeDisplayDelegate> delegate)
{
    display_configurator_ = std::make_unique<display::DisplayConfigurator>();
    display_configurator_->Init(std::move(delegate), false /* is_panel_fitting_enabled */);
    display_configurator_->SetConfigureDisplays(configure_displays_);
}

void DisplayManager::ForceInitialConfigureWithObservers(
    display::DisplayChangeObserver* display_change_observer, display::DisplayConfigurator::Observer* display_error_observer)
{
...
{
    SetMultiDisplayMode(enabled ? MIRRORING : current_default_multi_display_mode_);
}
...
bool DisplayManager::IsSoftwareMirroringEnforced() const
{
    ...
    return !!mixed_mirror_mode_params_ || layout_store_->forced_mirror_mode_for_tablet();
}

void DisplayManager::SetTouchCalibrationData(int64_t display_id, const TouchCalibrationData::CalibrationPointPairQuad& point_pair_quad,
    const gfx::Size& display_bounds, const ui::TouchscreenDevice& touchdevice, bool apply_spatial_calibration)
{
    ...
    // being performed.
    int64_t previous_display_id = touch_device_manager_->GetAssociatedDisplay(touchdevice);
    ...
    if (apply_spatial_calibration) {
        ...
        touch_device_manager_->AddTouchCalibrationData(touchdevice, display_id, calibration_data);
    } else {
    ...
    for (const auto& display : active_display_list_) {
        ...
        if (info.id() == display_id) {
            ...
            // supports touch.
            if (!touch_device_manager_->GetAssociatedTouchDevicesForDisplay(previous_display_id).empty()) {
    ...
    if (!update_add_support) {
        display_info_[display_id].set_touch_support(Display::TouchSupport::AVAILABLE);
    }
    if (!update_remove_support && !touch_device_manager_->GetAssociatedTouchDevicesForDisplay(previous_display_id).empty()) {
        display_info_[previous_display_id].set_touch_support(Display::TouchSupport::UNAVAILABLE);
    }
...
void DisplayManager::ClearTouchCalibrationData(int64_t display_id, std::optional<ui::TouchscreenDevice> touchdevice)
{
    if (touchdevice) {
...
void DisplayManager::UpdateZoomFactor(int64_t display_id, float zoom_factor)
{
    ...
    if (IsInternalDisplayId(display_id)) {
        on_display_zoom_modify_timeout_.Cancel();
        on_display_zoom_modify_timeout_.Reset(base::BindOnce(&OnInternalDisplayZoomChanged, zoom_factor));
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
            FROM_HERE, on_display_zoom_modify_timeout_.callback(), base::Seconds(kDisplayZoomModifyTimeoutSec));
    }
...
void DisplayManager::SetDefaultMultiDisplayModeForCurrentDisplays(MultiDisplayMode mode)
{
...
bool DisplayManager::UpdateDisplayBounds(int64_t display_id, const gfx::Rect& new_bounds)
{
    ...
    if (IsInSoftwareMirrorMode() && base::Contains(software_mirroring_display_list_, display_id, &Display::id)) {
        return false;
    }
    ...
    if (IsInUnifiedMode() && ContainsDisplayWithId(software_mirroring_display_list_, display_id)) {
    ...
    CHECK(pending_display_changes_.has_value());
    pending_display_changes_->display_metrics_changes[display->id()] |= DisplayObserver::DISPLAY_METRIC_BOUNDS;
    return true;
}
...
void DisplayManager::CreateMirrorWindowAsyncIfAny()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&DisplayManager::CreateMirrorWindowIfAny, weak_ptr_factory_.GetWeakPtr()));
}

void DisplayManager::UpdateInternalManagedDisplayModeListForTest()
{
    if (!HasInternalDisplay() || display_info_.count(Display::InternalDisplayId()) == 0) {
        return;
    }
...
void DisplayManager::ResetDisplayZoom(int64_t display_id)
{
    if (IsInUnifiedMode()) {
        ...
        const ManagedDisplayInfo::ManagedDisplayModeList& modes = display_info.display_modes();
...
void DisplayManager::CreateSoftwareMirroringDisplayInfo(DisplayInfoList* display_info_list)
{
    ...
    case MIRRORING: {
        ...
        if (mixed_mirror_mode_params_) {
            ...
            if (HasInternalDisplay()) {
                ...
                if (!base::Contains(*display_info_list, source_id, &ManagedDisplayInfo::id)) {
        ...
        for (auto iter = display_info_list->begin(); iter != display_info_list->end();) {
            if (destination_ids.count(iter->id())) {
                ...
                software_mirroring_display_list_.emplace_back(CreateMirroringDisplayFromDisplayInfoById(iter->id(), gfx::Point(), 1.0f));
...
void DisplayManager::CreateUnifiedDesktopDisplayInfo(DisplayInfoList* display_info_list)
{
    ...
    for (auto& display_info : *display_info_list) {
        ...
        if (it != display_info_.end()) {
            display_info.SetRotation(it->second.GetRotation(Display::RotationSource::USER), Display::RotationSource::USER);
            display_info.SetRotation(it->second.GetRotation(Display::RotationSource::ACTIVE), Display::RotationSource::ACTIVE);
            display_info.UpdateDisplaySize();
        }
    }

    if (!ValidateMatrix(current_unified_desktop_matrix_) || !ValidateMatrixForDisplayInfoList(*display_info_list, current_unified_desktop_matrix_)) {
    ...
    for (size_t i = 0; i < num_rows; ++i) {
        ...
        for (size_t j = 0; j < num_columns; ++j) {
            ...
            InsertAndUpdateDisplayInfo(*info);
            const float scale = info->size_in_pixel().height() / static_cast<float>(max_row_height);
            scales[i][j] = scale;

            const gfx::Point origin(this_row_bounds.right(), 0);
            const auto display_bounds = gfx::Rect(origin, gfx::ScaleToFlooredSize(info->size_in_pixel(), 1.0f / scale));
    ...
    for (size_t i = 0; i < num_rows; ++i) {
        ...
        for (size_t j = 0; j < num_columns; ++j) {
            ...
            const gfx::Point origin(row_displays_bounds.right(), unified_bounds.bottom());
            // The display is scaled to fit the unified desktop size.
            Display display = CreateMirroringDisplayFromDisplayInfoById(id, origin, 1.0f / scale);
    ...
    const int default_mode_param_index = internal_display_index != -1 ? internal_display_index : 0;
    ...
    ManagedDisplayInfo::ManagedDisplayModeList modes = CreateUnifiedManagedDisplayModeList(native_mode, modes_param_list);

    // Find the default mode.
    auto default_mode_iter = base::ranges::find_if(modes, &ManagedDisplayMode::native);
    DCHECK(default_mode_iter != modes.end());

    if (default_mode_iter != modes.end()) {
        const ManagedDisplayMode& default_mode = *default_mode_iter;
        unified_display_info.set_device_scale_factor(default_mode.device_scale_factor());
    ...
    // has changed.
    if (display_info_.count(kUnifiedDisplayId) != 0 && GetMaxNativeSize(display_info_[kUnifiedDisplayId]) != unified_bounds.size()) {
    ...
    if (GetSelectedModeForDisplayId(kUnifiedDisplayId, &selected_mode)
        && FindDisplayMode(unified_display_info, selected_mode) != unified_display_info.display_modes().end()) {
        unified_display_info.set_device_scale_factor(selected_mode.device_scale_factor());
        unified_display_info.SetBounds(gfx::Rect(selected_mode.size()));
    } else {
        display_modes_.erase(kUnifiedDisplayId);
    }

    const float unified_bounds_scale_y = unified_display_info.size_in_pixel().height() / static_cast<float>(unified_bounds.size().height());
    ...
    for (size_t i = 0; i < num_rows; ++i) {
        ...
        int max_height = std::numeric_limits<int>::min();
        for (size_t j = 0; j < num_columns; ++j) {
            Display& current_display = software_mirroring_display_list_[i * num_columns + j];
            ...
            current_display.set_bounds(gfx::Rect(origin, gfx::ToRoundedSize(scaled_size)));
    ...
    UMA_HISTOGRAM_ENUMERATION(
        "DisplayManager.UnifiedDesktopDisplayCountRange", GetDisplayCountRange(software_mirroring_display_list_.size()), DisplayCountRange::kCount);
}
...
void DisplayManager::AddMirrorDisplayInfoIfAny(DisplayInfoList* display_info_list)
{
    if (!IsInSoftwareMirrorMode()) {
        return;
    }
...
void DisplayManager::InsertAndUpdateDisplayInfo(const ManagedDisplayInfo& new_info)
{
...
void DisplayManager::ApplyDefaultZoomFactorIfNecessary(ManagedDisplayInfo& info)
{
...
Display DisplayManager::CreateDisplayFromDisplayInfoById(int64_t id)
{
    ...
    new_display.SetScaleAndBounds(device_scale_factor, gfx::Rect(bounds_in_native.size()));
    ...
    if (display_info.bits_per_channel() > kNormalBitDepthNumBitsPerChannel) {
        ...
        new_display.set_color_depth(display_info.bits_per_channel() * kRGBNumChannels);
    }
    if (internal_display_has_accelerometer_ && IsInternalDisplayId(id)) {
        new_display.set_accelerometer_support(Display::AccelerometerSupport::AVAILABLE);
    } else {
        new_display.set_accelerometer_support(Display::AccelerometerSupport::UNAVAILABLE);
    }
    return new_display;
}

Display DisplayManager::CreateMirroringDisplayFromDisplayInfoById(int64_t id, const gfx::Point& origin, float scale)
{
    DCHECK(display_info_.find(id) != display_info_.end()) << "id=" << id;
    const ManagedDisplayInfo& display_info = display_info_[id];

    Display new_display(display_info.id());
    new_display.SetScaleAndBounds(1.0f, gfx::Rect(origin, gfx::ScaleToFlooredSize(display_info.size_in_pixel(), scale)));
    new_display.set_touch_support(display_info.touch_support());
    new_display.set_maximum_cursor_size(display_info.maximum_cursor_size());
    new_display.set_rotation(display_info.GetActiveRotation());
    new_display.set_panel_rotation(display_info.GetLogicalActiveRotation());
...
void DisplayManager::UpdateNonPrimaryDisplayBoundsForLayout(Displays* display_list, std::vector<size_t>* updated_indices)
{
    ...
    DCHECK(IsConnectedDisplayIdListInSyncWithCurrentState(CreateDisplayIdList(*display_list)));
...
void DisplayManager::CreateMirrorWindowIfAny()
{
    ...
    DCHECK(IsConnectedDisplayIdListInSyncWithCurrentState(CreateDisplayIdList(active_display_list())));
}

void DisplayManager::ApplyDisplayLayout(DisplayLayout* layout, Displays* display_list, std::vector<int64_t>* updated_ids)
{
    ...
    if (mixed_mirror_mode_params_) {
        ...
        temp_layout->RemoveDisplayPlacements(mixed_mirror_mode_params_->destination_ids);
        temp_layout->ApplyToDisplayList(display_list, updated_ids, kMinimumOverlapForInvalidOffset);
    } else {
        layout->ApplyToDisplayList(display_list, updated_ids, kMinimumOverlapForInvalidOffset);
    }
}
...
void DisplayManager::SetTabletState(const TabletState& tablet_state)
{
    ...
    display_observers_.Notify(&DisplayObserver::OnDisplayTabletStateChanged, tablet_state);
}

void DisplayManager::NotifyMetricsChanged(const Display& display, uint32_t metrics)
{
    if (delegate_) {
        delegate_->UpdateDisplayMetrics(display, metrics);
    }

    display_observers_.Notify(&DisplayObserver::OnDisplayMetricsChanged, display, metrics);
}
...
{
    manager_observers_.Notify(&DisplayManagerObserver::OnWillProcessDisplayChanges);
}

void DisplayManager::NotifyDidProcessDisplayChanges(const DisplayManagerObserver::DisplayConfigurationChange& config_change)
{
    ...
    manager_observers_.Notify(&DisplayManagerObserver::OnDidProcessDisplayChanges, config_change);
}
...
void DisplayManager::AddDisplayManagerObserver(DisplayManagerObserver* manager_observer)
{
    manager_observers_.AddObserver(manager_observer);
}

void DisplayManager::RemoveDisplayManagerObserver(DisplayManagerObserver* manager_observer)
{
...
void DisplayManager::UpdatePrimaryDisplayIdIfNecessary()
{
    ...
    const display::DisplayLayout& layout = layout_store()->GetRegisteredDisplayLayout(list);
    layout_store()->UpdateDefaultUnified(list, layout.default_unified);
    if (delegate_ && GetNumDisplays() > 1) {
        delegate_->SetPrimaryDisplayId(layout.primary_id == display::kInvalidDisplayId ? list[0] : layout.primary_id);
    }
}
