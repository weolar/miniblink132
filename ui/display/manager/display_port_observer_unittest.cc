...
std::unique_ptr<DisplaySnapshot> CreateDisplaySnapshot(uint64_t base_connector_id, base::FilePath sys_path)
{
    return FakeDisplaySnapshot::Builder().SetId(1).SetNativeMode(gfx::Size(1024, 768)).SetBaseConnectorId(base_connector_id).SetSysPath(sys_path).Build();
}
...
public:
    ...
    void SetUp() override
    {
        display_port_observer_ = std::make_unique<DisplayPortObserver>(
            nullptr, base::BindLambdaForTesting([&](const std::vector<uint32_t>& port_nums) { cached_port_nums_ = port_nums; }));
...
TEST_F(DisplayPortObserverTest, OnNoDisplayConnected)
{
    ...
    display_port_observer_->OnDisplayConfigurationChanged(std::move(display_states));
