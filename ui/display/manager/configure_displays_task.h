...
struct DISPLAY_MANAGER_EXPORT DisplayConfigureRequest {
    DisplayConfigureRequest(DisplaySnapshot* display, const DisplayMode* mode, const gfx::Point& origin, bool enable_vrr);

    DisplayConfigureRequest(DisplaySnapshot* display, const DisplayMode* mode, const gfx::Point& origin);
...
class DISPLAY_MANAGER_EXPORT ConfigureDisplaysTask : public NativeDisplayObserver {
public:
    ...
    ConfigureDisplaysTask(NativeDisplayDelegate* delegate, const std::vector<DisplayConfigureRequest>& requests, ResponseCallback callback,
...
private:
    struct RequestToOriginalMode {
        RequestToOriginalMode(DisplayConfigureRequest* request, const DisplayMode* original_mode);
    ...
    using PartitionedRequestsQueue = std::queue<std::vector<RequestToOriginalMode>>;
    ...
    void OnFirstAttemptConfigured(const std::vector<DisplayConfigurationParams>& request_results, bool config_success);
    ...
    void OnRetryConfigured(const std::vector<DisplayConfigurationParams>& request_results, bool config_success);

    // Finalizes the configuration after a modeset attempt was made (as opposed to
    // test-modeset).
    void OnConfigured(const std::vector<DisplayConfigurationParams>& request_results, bool config_success);
    ...
    std::vector<display::DisplayConfigurationParams> last_successful_config_parameters_;
