...
void GetDisplayBounds(const std::string& spec, gfx::Rect* bounds, float* device_scale_factor)
{
    int width = 0;
    int height = 0;
    int x = 0;
    int y = 0;
    if (sscanf(spec.c_str(), "%dx%d*%f", &width, &height, device_scale_factor) >= 2
        || sscanf(spec.c_str(), "%d+%d-%dx%d*%f", &x, &y, &width, &height, device_scale_factor) >= 4) {
        bounds->SetRect(x, y, width, height);

        auto equals_within_epsilon = [device_scale_factor](float dsf) { return std::abs(*device_scale_factor - dsf) < 0.01f; };
...
ManagedDisplayMode::ManagedDisplayMode(const gfx::Size& size, float refresh_rate, bool is_interlaced, bool native)
    : size_(size)
...
ManagedDisplayMode::ManagedDisplayMode(const gfx::Size& size, float refresh_rate, bool is_interlaced, bool native, float device_scale_factor)
    : size_(size)
    , refresh_rate_(refresh_rate)
    , is_interlaced_(is_interlaced)
    , native_(native)
...
ManagedDisplayMode::ManagedDisplayMode(const ManagedDisplayMode& other) = default;

ManagedDisplayMode& ManagedDisplayMode::operator=(const ManagedDisplayMode& other) = default;

bool ManagedDisplayMode::operator==(const ManagedDisplayMode& other) const
{
    return size_ == other.size_ && is_interlaced_ == other.is_interlaced_ && native_ == other.native_ && IsWithinEpsilon(refresh_rate_, other.refresh_rate_)
...
bool ManagedDisplayMode::IsEquivalent(const ManagedDisplayMode& other) const
{
    ...
    return size_ == other.size_ && IsWithinEpsilon(device_scale_factor_, other.device_scale_factor_);
}

std::string ManagedDisplayMode::ToString() const
{
    return base::StringPrintf("DisplayMode{size: %s, refresh_rate: %f, interlaced:"
                              " %d, native: %d, device_scale_factor: %f}",
        size_.ToString().c_str(), refresh_rate_, is_interlaced_, native_, device_scale_factor_);
}
...
// static
ManagedDisplayInfo ManagedDisplayInfo::CreateFromSpecWithID(const std::string& spec, int64_t id)
{
    ...
    gfx::Rect bounds_in_native(kDefaultHostWindowX, kDefaultHostWindowY, kDefaultHostWindowWidth, kDefaultHostWindowHeight);
    std::string_view main_spec = spec;

    gfx::RoundedCornersF panel_corners_radii;
    std::vector<std::string_view> parts = base::SplitStringPiece(main_spec, "~", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    if (parts.size() == 2) {
        std::vector<std::string_view> radii_part = base::SplitStringPiece(parts[1], "|", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        ...
        panel_corners_radii = (radii_part.size() == 1) ? gfx::RoundedCornersF { radii[0] } : gfx::RoundedCornersF { radii[0], radii[1], radii[2], radii[3] };

        main_spec = parts[0];
    }

    float zoom_factor = 1.0f;
    parts = base::SplitStringPiece(main_spec, "@", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    if (parts.size() == 2) {
    ...
    if (!main_spec.empty()) {
        GetDisplayBounds(std::string(main_spec), &bounds_in_native, &device_scale_factor);
        ...
        if (parts.size() == 2) {
            ...
            parts = base::SplitStringPiece(resolution_list, "|", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
            for (size_t i = 0; i < parts.size(); ++i) {
                ...
                std::vector<std::string_view> resolution = base::SplitStringPiece(parts[i], "%", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
                float device_scale_factor_for_mode = device_scale_factor;
                GetDisplayBounds(std::string(resolution[0]), &mode_bounds, &device_scale_factor_for_mode);
                ...
                if (size.GetArea() >= largest_area && refresh_rate > highest_refresh_rate) {
                    // Use mode with largest area and highest refresh rate as native.
                    largest_area = size.GetArea();
                    highest_refresh_rate = refresh_rate;
                    native_mode = i;
                }
                display_modes.emplace_back(size, refresh_rate, is_interlaced, false, device_scale_factor_for_mode);
            }
            ManagedDisplayMode dm = display_modes[native_mode];
            display_modes[native_mode] = ManagedDisplayMode(dm.size(), dm.refresh_rate(), dm.is_interlaced(), true, dm.device_scale_factor());
        }
    }

    ManagedDisplayInfo display_info = id == kInvalidDisplayId ? CreateDisplayInfo(GetASynthesizedDisplayId()) : CreateDisplayInfo(id);
    ...
    if (has_hdr) {
        gfx::DisplayColorSpaces display_color_spaces { gfx::ColorSpace::CreateHDR10(), gfx::BufferFormat::BGRA_1010102 };
        display_info.set_display_color_spaces(display_color_spaces);
    }

    DVLOG(1) << "DisplayInfoFromSpec info=" << display_info.ToString() << ", spec=" << spec;
    return display_info;
}
...
ManagedDisplayInfo::ManagedDisplayInfo(int64_t id, const std::string& name, bool has_overscan)
    : id_(id)
    , name_(name)
    , year_of_manufacture_(kInvalidYearOfManufacture)
...
ManagedDisplayInfo::ManagedDisplayInfo(const ManagedDisplayInfo& other) = default;

ManagedDisplayInfo::~ManagedDisplayInfo() = default;

void ManagedDisplayInfo::SetRotation(Display::Rotation rotation, Display::RotationSource source)
{
...
{
    return GetRotationWithPanelOrientation(GetRotation(Display::RotationSource::ACTIVE));
}

Display::Rotation ManagedDisplayInfo::GetRotation(Display::RotationSource source) const
{
...
void ManagedDisplayInfo::AddZoomFactorForSize(const std::string& size, float zoom_factor)
{
...
void ManagedDisplayInfo::Copy(const ManagedDisplayInfo& native_info)
{
    ...
    panel_orientation_ = native_info.panel_orientation_, size_in_pixel_ = native_info.size_in_pixel_;
...
float ManagedDisplayInfo::GetEffectiveDeviceScaleFactor() const
{
    ...
    const int pixel_size = std::max(bounds_in_native_.width(), bounds_in_native_.height());
...
void ManagedDisplayInfo::UpdateZoomFactorToMatchTargetDPI()
{
    ...
    const float target_zoom_factor = device_dpi_ / kRecommendedDefaultExternalDisplayDpi;

    // Refine zoom factor based on available zoom factors in settings.
    const int display_larger_side = std::max(bounds_in_native_.width(), bounds_in_native_.height());
    const std::vector<float> avaialble_zoom_factors = GetDisplayZoomFactorsByDisplayWidth(display_larger_side);
    ...
    if (target_zoom_factor <= min_zoom_factor) {
        ...
        DCHECK(std::is_sorted(avaialble_zoom_factors.begin(), avaialble_zoom_factors.end()));
        for (size_t i = 0; i < avaialble_zoom_factors.size() - 1; i++) {
            ...
            zoom_factor_ = (target_zoom_factor - left_bound < right_bound - target_zoom_factor) ? left_bound : right_bound;
            break;
        }
    }
...
gfx::Size ManagedDisplayInfo::GetSizeInPixelWithPanelOrientation() const
{
    ...
    if (panel_orientation_ == display::PanelOrientation::kLeftUp || panel_orientation_ == display::PanelOrientation::kRightUp) {
...
void ManagedDisplayInfo::UpdateDisplaySize()
{
    ...
    if (GetActiveRotation() == Display::ROTATE_90 || GetActiveRotation() == Display::ROTATE_270) {
...
{
    return gfx::ToFlooredInsets(gfx::ConvertInsetsToPixels(overscan_insets_in_dip_, device_scale_factor_));
}

void ManagedDisplayInfo::SetSnapshotColorSpace(const gfx::ColorSpace& snapshot_color)
{
...
void ManagedDisplayInfo::SetManagedDisplayModes(const ManagedDisplayModeList& display_modes)
{
    display_modes_ = display_modes;
    std::sort(display_modes_.begin(), display_modes_.end(), ManagedDisplayModeSorter());
}
...
std::string ManagedDisplayInfo::ToString() const
{
    ...
    std::string result = base::StringPrintf("ManagedDisplayInfo[%lld] port_display_id=%lld, edid_display_id=%lld, "
                                            ...
                                            "color_space=%s",
        static_cast<long long int>(id_), static_cast<long long int>(port_display_id_), static_cast<long long int>(edid_display_id_),
        bounds_in_native_.ToString().c_str(), size_in_pixel_.ToString().c_str(), device_scale_factor_, zoom_factor_, overscan_insets_in_dip_.ToString().c_str(),
        rotation_degree,
        touch_support_ == Display::TouchSupport::AVAILABLE         ? "yes"
            : touch_support_ == Display::TouchSupport::UNAVAILABLE ? "no"
                                                                   : "unknown",
        panel_corners_radii_.ToString().c_str(), PanelOrientationToString(panel_orientation_).c_str(), detected_ ? "true" : "false",
...
std::string ManagedDisplayInfo::ToFullString() const
{
    ...
    for (const ManagedDisplayMode& m : display_modes_) {
        if (!display_modes_str.empty())
            display_modes_str += ",";
        base::StringAppendF(&display_modes_str, "(%dx%d@%g%c%s %g)", m.size().width(), m.size().height(), m.refresh_rate(), m.is_interlaced() ? 'I' : 'P',
            m.native() ? "(N)" : "", m.device_scale_factor());
    }
    return ToString() + ", display_modes==" + display_modes_str;
}

Display::Rotation ManagedDisplayInfo::GetRotationWithPanelOrientation(Display::Rotation rotation) const
{
    int offset = 0;
    ...
    return static_cast<Display::Rotation>((static_cast<int>(rotation) + offset) % 4);
}

ManagedDisplayInfo CreateDisplayInfo(int64_t id, const gfx::Rect& bounds)
{
    display::ManagedDisplayInfo info(id, base::StringPrintf("Display-%d", static_cast<int>(id)), false);
