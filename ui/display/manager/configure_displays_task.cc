...
const std::string GetUmaNamePrefixForRequest(const DisplayConfigureRequest& request)
{
    return request.display->type() == DISPLAY_CONNECTION_TYPE_INTERNAL ? std::string("ConfigureDisplays.Internal.Modeset.")
...
void LogIfInvalidRequestForInternalDisplay(const DisplayConfigureRequest& request)
{
    if (request.display->type() != DISPLAY_CONNECTION_TYPE_INTERNAL)
        return;
    ...
    if (request.display->native_mode() && *request.mode == *request.display->native_mode()) {
        return;
    }

    LOG(ERROR) << "A mode other than the preferred mode was requested for the "
                  "internal display: preferred="
               << request.display->native_mode()->ToString() << " vs. requested=" << request.mode->ToString()
               << ". Current mode=" << (request.display->current_mode() ? request.display->current_mode()->ToString() : "nullptr (disabled)") << ".";
}

// Samples used to define buckets used by DisplayResolution enum.
// The enum is used to record screen resolution statistics.
const int32_t kDisplayResolutionSamples[] = { 1024, 1280, 1440, 1920, 2560, 3840, 5120, 7680 };

void UpdateResolutionUma(const DisplayConfigureRequest& request, const std::string& uma_name)
{
    ...
    base::UmaHistogramExactLinear(uma_name, display_resolution_index, samples_list_size * samples_list_size + 2);
}
...
constexpr float kCommonDisplayRefreshRates[] = { 24.0, 25.0, 30.0, 45.0, 48.0, 50.0, 60.0, 75.0, 90.0, 100.0, 120.0, 144.0, 165.0, 200.0, 240.0 };

void UpdateRefreshRateUma(const DisplayConfigureRequest& request, const std::string& uma_name)
{
    // Display is powered off.
    if (!request.mode)
        return;

    base::HistogramBase* histogram = base::SparseHistogram::FactoryGet(uma_name, base::HistogramBase::kUmaTargetedHistogramFlag);

    // Check if the refresh value is within an epsilon from one of the common
    // refresh rate values.
    for (float common_rate : kCommonDisplayRefreshRates) {
        const bool is_within_epsilon = std::abs(request.mode->refresh_rate() - common_rate) < kRefreshRateEpsilon;
...
void UpdateAttemptSucceededUma(const std::vector<DisplayConfigureRequest>& requests, bool display_success)
{
    for (const auto& request : requests) {
        const std::string uma_name_prefix = GetUmaNamePrefixForRequest(request);
        base::UmaHistogramBoolean(uma_name_prefix + "AttemptSucceeded", display_success);

        VLOG(2) << "Configured status=" << display_success << " display=" << request.display->display_id() << " origin=" << request.origin.ToString()
                << " mode=" << (request.mode ? request.mode->ToString() : "null") << " enable_vrr=" << request.enable_vrr;
    }
}

void UpdateFinalStatusUma(const std::vector<RequestAndStatusList>& requests_and_statuses, ConfigureDisplaysTask::Status status)
{
    ...
    for (auto& request_and_status : requests_and_statuses) {
        ...
        const bool sst_display = request->display->base_connector_id() && request->display->path_topology().empty();
        if (!sst_display)
        ...
        base::UmaHistogramBoolean(uma_name_prefix + "FinalStatus", request_and_status.second);
    }

    base::UmaHistogramEnumeration("ConfigureDisplays.Modeset.FinalTaskStatus", status);

    base::UmaHistogramExactLinear("ConfigureDisplays.Modeset.TotalExternalDisplaysCount", base::checked_cast<int>(total_external_displays), kMaxDisplaysCount);

    base::UmaHistogramExactLinear("ConfigureDisplays.Modeset.MstExternalDisplaysCount", mst_external_displays, kMaxDisplaysCount);

    if (total_external_displays > 0) {
        const int mst_displays_percentage = 100.0 * mst_external_displays / total_external_displays;
        UMA_HISTOGRAM_PERCENTAGE("ConfigureDisplays.Modeset.MstExternalDisplaysPercentage", mst_displays_percentage);
    }
}
...
// match.
void UpdateSnapshotAfterConfiguration(DisplaySnapshot* display, const DisplayConfigurationParams& request)
{
    ...
    if (display->IsVrrCapable()) {
        display->set_variable_refresh_rate_state(request.enable_vrr ? VariableRefreshRateState::kVrrEnabled : VariableRefreshRateState::kVrrDisabled);
    }
}

} // namespace

DisplayConfigureRequest::DisplayConfigureRequest(DisplaySnapshot* display, const DisplayMode* mode, const gfx::Point& origin, bool enable_vrr)
    : display(display)
...
DisplayConfigureRequest::DisplayConfigureRequest(DisplaySnapshot* display, const DisplayMode* mode, const gfx::Point& origin)
    : DisplayConfigureRequest(display, mode, origin,
        /*enable_vrr=*/false)
{
}

DisplayConfigureRequest::DisplayConfigureRequest(const DisplayConfigureRequest& other)
    : DisplayConfigureRequest(other.display, other.mode.get(), other.origin, other.enable_vrr)
{
}
...
ConfigureDisplaysTask::ConfigureDisplaysTask(
    NativeDisplayDelegate* delegate, const std::vector<DisplayConfigureRequest>& requests, ResponseCallback callback, ConfigurationType configuration_type)
    : delegate_(delegate)
    , requests_(requests)
...
ConfigureDisplaysTask::RequestToOriginalMode::RequestToOriginalMode(DisplayConfigureRequest* request, const DisplayMode* original_mode)
    : request(request)
    , original_mode(original_mode ? original_mode->Clone() : nullptr)
{
}

ConfigureDisplaysTask::RequestToOriginalMode::RequestToOriginalMode(const RequestToOriginalMode& other)
    : RequestToOriginalMode(other.request, other.original_mode.get())
{
}

ConfigureDisplaysTask::RequestToOriginalMode::~RequestToOriginalMode() = default;
...
void ConfigureDisplaysTask::Run()
{
    ...
    for (const auto& request : requests_) {
        ...
        config_requests.emplace_back(request.display->display_id(), request.origin, request.mode.get(), request.enable_vrr);

        if (is_first_attempt) {
            const std::string uma_name_prefix = GetUmaNamePrefixForRequest(request);
            UpdateResolutionUma(request, uma_name_prefix + "OriginalRequest.Resolution");
        }
    }

    const auto& on_configured = is_first_attempt ? &ConfigureDisplaysTask::OnFirstAttemptConfigured : &ConfigureDisplaysTask::OnRetryConfigured;
    ...
    delegate_->Configure(config_requests, base::BindOnce(on_configured, weak_ptr_factory_.GetWeakPtr()), modeset_flags);
}
...
void ConfigureDisplaysTask::OnFirstAttemptConfigured(const std::vector<DisplayConfigurationParams>& request_results, bool config_success)
{
    ...
    if (!config_success) {
        ...
        for (const auto& pair : pending_display_group_requests_.front()) {
            pair.request->mode = pair.original_mode ? pair.original_mode->Clone() : nullptr;
        }
    ...
    if (configuration_type_ == kConfigurationTypeSeamless)
        modeset_flags.Put(display::ModesetFlag::kSeamlessModeset);
    delegate_->Configure(config_requests, base::BindOnce(&ConfigureDisplaysTask::OnConfigured, weak_ptr_factory_.GetWeakPtr()), modeset_flags);
}

void ConfigureDisplaysTask::OnRetryConfigured(const std::vector<DisplayConfigurationParams>& request_results, bool config_success)
{
    UpdateAttemptSucceededUma(requests_, config_success);
    ...
    if (!config_success) {
        ...
        for (const auto& request : requests_) {
            last_successful_config_parameters_.emplace_back(request.display->display_id(), request.origin, request.mode.get(), request.enable_vrr);
        }
    }
    ...
    if (configuration_type_ == kConfigurationTypeSeamless)
        modeset_flags.Put(display::ModesetFlag::kSeamlessModeset);
    delegate_->Configure(
        last_successful_config_parameters_, base::BindOnce(&ConfigureDisplaysTask::OnConfigured, weak_ptr_factory_.GetWeakPtr()), modeset_flags);
}

void ConfigureDisplaysTask::OnConfigured(const std::vector<DisplayConfigurationParams>& request_results, bool config_success)
{
    if (config_success) {
...
{
    auto cmp = [](DisplayConfigureRequest* lhs, DisplayConfigureRequest* rhs) { return *lhs->mode < *rhs->mode; };
    std::priority_queue<DisplayConfigureRequest*, std::vector<DisplayConfigureRequest*>, decltype(cmp)> sorted_requests(cmp);
