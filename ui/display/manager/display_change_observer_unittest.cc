...
float ComputeDpi(float diagonal_inch, const gfx::Size& resolution)
{
    ...
    float diagonal_pixel = std::sqrt(std::pow(resolution.width(), 2) + std::pow(resolution.height(), 2));
...
std::unique_ptr<DisplayMode> MakeDisplayMode(
    int width, int height, bool is_interlaced, float refresh_rate, const std::optional<float>& vsync_rate_min = std::nullopt)
{
    return std::make_unique<DisplayMode>(gfx::Size { width, height }, is_interlaced, refresh_rate, vsync_rate_min);
}
...
public:
    ...
    ManagedDisplayInfo CreateManagedDisplayInfo(DisplayChangeObserver* observer, const DisplaySnapshot* snapshot, const DisplayMode* mode_info)
    {
...
class DisplayChangeObserverTest : public DisplayChangeObserverTestBase, public testing::WithParamInterface<bool> {
public:
    DisplayChangeObserverTest() = default;

    DisplayChangeObserverTest(const DisplayChangeObserverTest&) = delete;
    DisplayChangeObserverTest& operator=(const DisplayChangeObserverTest&) = delete;
    ...
    void SetUp() override
    {
        if (GetParam()) {
            ...
            scoped_feature_list_.InitAndDisableFeature(features::kListAllDisplayModes);
        }
...
class DisplayChangeObserverPanelRadiiTest : public DisplayChangeObserverTestBase {
public:
    DisplayChangeObserverPanelRadiiTest() = default;

    DisplayChangeObserverPanelRadiiTest(const DisplayChangeObserverPanelRadiiTest&) = delete;
    DisplayChangeObserverPanelRadiiTest& operator=(const DisplayChangeObserverPanelRadiiTest&) = delete;
    ...
    {
        display_change_observer_ = std::make_unique<DisplayChangeObserver>(display_manager_.get());
    }
...
{
    command_line_.GetProcessCommandLine()->AppendSwitchASCII(switches::kDisplayProperties,
    ...
    std::unique_ptr<DisplaySnapshot> display_snapshot = FakeDisplaySnapshot::Builder()
                                                            ...
                                                            .SetType(display::DisplayConnectionType::DISPLAY_CONNECTION_TYPE_INTERNAL)
                                                            .Build();

    const ManagedDisplayInfo display_info = CreateManagedDisplayInfo(display_change_observer_.get(), display_snapshot.get(), default_display_mode_.get());

    EXPECT_EQ(display_info.panel_corners_radii(), gfx::RoundedCornersF(16, 16, 15, 15));
}
...
TEST_P(DisplayChangeObserverTest, GetExternalManagedDisplayModeList)
{
    ...
    ManagedDisplayInfo::ManagedDisplayModeList display_modes = DisplayChangeObserver::GetExternalManagedDisplayModeList(*display_snapshot);
...
TEST_P(DisplayChangeObserverTest, GetEmptyExternalManagedDisplayModeList)
{
    ...
    FakeDisplaySnapshot display_snapshot(
        ...
        /*connector_index=*/0x0001, gfx::Point(), gfx::Size(), DISPLAY_CONNECTION_TYPE_UNKNOWN,
        /*base_connector_id=*/1u, /*path_topology=*/ {}, false, false, PrivacyScreenState::kNotSupported, false, std::string(), base::FilePath(), {}, nullptr,
        nullptr, 0, gfx::Size(), color_info, VariableRefreshRateState::kVrrNotCapable, DrmFormatsAndModifiers());
...
{
    // Validation check
    EXPECT_EQ(1.25f, DisplayChangeObserver::FindDeviceScaleFactor(150, gfx::Size()));
    EXPECT_EQ(1.6f, DisplayChangeObserver::FindDeviceScaleFactor(180, gfx::Size()));
    EXPECT_EQ(kDsf_1_777, DisplayChangeObserver::FindDeviceScaleFactor(220, gfx::Size()));
    EXPECT_EQ(2.f, DisplayChangeObserver::FindDeviceScaleFactor(230, gfx::Size()));
    EXPECT_EQ(2.4f, DisplayChangeObserver::FindDeviceScaleFactor(270, gfx::Size()));
    EXPECT_EQ(kDsf_2_252, DisplayChangeObserver::FindDeviceScaleFactor(0, gfx::Size(3000, 2000)));
    EXPECT_EQ(kDsf_2_666, DisplayChangeObserver::FindDeviceScaleFactor(310, gfx::Size()));

    std::set<std::tuple<float, int, int>> dup_check;

    for (auto& entry : display_configs) {
        std::tuple<float, int, int> key { entry.diagonal_size, entry.resolution.width(), entry.resolution.height() };
        DCHECK(!dup_check.count(key));
        dup_check.emplace(key);

        SCOPED_TRACE(base::StringPrintf(
            "%dx%d, diag=%1.3f inch, expected=%1.10f", entry.resolution.width(), entry.resolution.height(), entry.diagonal_size, entry.expected_dsf));
        ...
        const gfx::Size dp_size = gfx::ScaleToCeiledSize(entry.resolution, 1.f / scale_factor);

        // Check Screenshot size.
        EXPECT_EQ(entry.expected_dp_size, dp_size);
        gfx::Transform transform;
        transform.Scale(scale_factor, scale_factor);
        const gfx::Size screenshot_size = cc::MathUtil::MapEnclosingClippedRect(transform, gfx::Rect(dp_size)).size();
        switch (entry.screenshot_size_error) {
        case kEpsilon: {
            EXPECT_NE(entry.resolution, screenshot_size);
            constexpr float kEpsilon = 0.001f;
            EXPECT_EQ(entry.resolution, cc::MathUtil::MapEnclosingClippedRectIgnoringError(transform, gfx::Rect(dp_size), kEpsilon).size());
            break;
        }
        case kExact:
    ...
    EXPECT_EQ(1.0f, DisplayChangeObserver::FindDeviceScaleFactor(-100.0f, gfx::Size()));
    EXPECT_EQ(1.0f, DisplayChangeObserver::FindDeviceScaleFactor(0.0f, gfx::Size()));
    EXPECT_EQ(max_scale_factor, DisplayChangeObserver::FindDeviceScaleFactor(10000.0f, gfx::Size()));
}

TEST_P(DisplayChangeObserverTest, FindExternalDisplayNativeModeWhenOverwritten)
{
    std::unique_ptr<DisplaySnapshot> display_snapshot = FakeDisplaySnapshot::Builder()
                                                            .SetId(123)
                                                            .SetNativeMode(MakeDisplayMode(1920, 1080, true, 60))
...
{
    const std::unique_ptr<DisplaySnapshot> display_snapshot = FakeDisplaySnapshot::Builder()
                                                                  .SetId(123)
    ...
    const ManagedDisplayInfo display_info = CreateManagedDisplayInfo(&observer, display_snapshot.get(), display_mode.get());
...
TEST_P(DisplayChangeObserverTest, HDRDisplayColorSpaces)
{
    ...
    scoped_feature_list.InitAndEnableFeature(features::kEnableExternalDisplayHDR10Mode);
    ...
    const std::unique_ptr<DisplaySnapshot> display_snapshot = FakeDisplaySnapshot::Builder()
                                                                  ...
                                                                  .SetHDRStaticMetadata({ 609.0, 500.0, 0.01,
    ...
    EXPECT_EQ(gfx::BufferFormat::RGBA_1010102,
        display_color_spaces.GetOutputBufferFormat(gfx::ContentColorUsage::kSRGB,
            /*needs_alpha=*/true));
    ...
    EXPECT_EQ(gfx::ColorSpace::CreateHDR10(),
        display_color_spaces.GetOutputColorSpace(gfx::ContentColorUsage::kHDR,
            /*needs_alpha=*/true));
    EXPECT_EQ(kDefaultHdrMaxLuminanceRelative, display_color_spaces.GetHDRMaxLuminanceRelative());
}
...
TEST_P(DisplayChangeObserverTest, VSyncRateMin)
{
    ...
    // present from the DisplayMode.
    {
        const std::unique_ptr<DisplaySnapshot> display_snapshot
            = FakeDisplaySnapshot::Builder().SetId(123).SetName("AmazingFakeDisplay").SetNativeMode(MakeDisplayMode(1920, 1080, true, 60)).Build();
        const std::unique_ptr<DisplayMode> display_mode = MakeDisplayMode(1920, 1080, true, 60);
        const ManagedDisplayInfo display_info = CreateManagedDisplayInfo(&observer, display_snapshot.get(), display_mode.get());
    ...
    // mode.
    {
        const std::unique_ptr<DisplaySnapshot> display_snapshot
            = FakeDisplaySnapshot::Builder().SetId(123).SetName("AmazingFakeDisplay").SetNativeMode(MakeDisplayMode(1920, 1080, true, 60)).Build();
        const std::unique_ptr<DisplayMode> display_mode = MakeDisplayMode(1920, 1080, true, 60, 48.000488f);
        const ManagedDisplayInfo display_info = CreateManagedDisplayInfo(&observer, display_snapshot.get(), display_mode.get());
...
TEST_P(DisplayChangeObserverTest, DisplayModeNativeCalculation)
{
    ...
    // true when they are the same.
    {
        ...
        const ManagedDisplayInfo display_info = CreateManagedDisplayInfo(&observer, display_snapshot.get(), display_mode);
    ...
    {
        const std::unique_ptr<DisplaySnapshot> display_snapshot
            = FakeDisplaySnapshot::Builder().SetId(123).SetType(DISPLAY_CONNECTION_TYPE_INTERNAL).SetCurrentMode(MakeDisplayMode(1920, 1080, true, 60)).Build();
...
INSTANTIATE_TEST_SUITE_P(All, DisplayChangeObserverTest, ::testing::Values(false, true));
...
auto CreateDisplay = [](const ManagedDisplayInfo& managed_display_info) {
    ...
    const float effective_scale = managed_display_info.GetEffectiveDeviceScaleFactor();
    display.SetScaleAndBounds(effective_scale, managed_display_info.bounds_in_native());
...
TEST_F(DisplayResolutionTest, CheckEffectiveResolutionUMAIndex)
{
    ...
    for (const auto& display_config : display_configs) {
        ...
        for (float zoom_level : zoom_levels) {
            ...
            const int landscape_key = logical_resolution.width() * logical_resolution.height();
...
TEST_F(DisplayResolutionTest, DisplayZoom)
{
    ...
    for (auto& config : display_configs) {
        ...
        auto managed_display_info = DisplayChangeObserver::CreateManagedDisplayInfo(snapshot.get(), native_mode,
            /*native=*/true, config.expected_dsf, dpi, std::string());
        const std::vector<float> zooms = GetDisplayZoomFactors(managed_display_info.display_modes()[0]);
        // For default scale factor, they should be the same.
        EXPECT_NEAR(config.expected_dsf, managed_display_info.GetEffectiveDeviceScaleFactor(), 0.0000001);
        ...
        for (auto zoom : zooms) {
            ...
            const float scale_factor = config.resolution.width() / static_cast<float>(display.size().width());
            EXPECT_NEAR(scale_factor, display.device_scale_factor(), kDeviceScaleFactorErrorTolerance);
        }
    }

    // Typical external display sizes.
    constexpr gfx::Size kExternalDisplaySizes[]
        = { { 4096, 2160 }, { 3840, 2160 }, { 3440, 1440 }, { 2560, 1600 }, { 2560, 1440 }, { 1920, 1200 }, { 1920, 1080 }, { 1600, 900 }, { 1440, 900 } };
    for (auto& size : kExternalDisplaySizes) {
        const auto snapshot = FakeDisplaySnapshot::Builder().SetId(10).SetType(DISPLAY_CONNECTION_TYPE_HDMI).SetNativeMode(size).SetCurrentMode(size).Build();
        const auto* native_mode = snapshot->native_mode();
        auto managed_display_info = DisplayChangeObserver::CreateManagedDisplayInfo(snapshot.get(), native_mode,
            /*native=*/true, /*device_scale_factor=*/1.0f, /*dpi=*/160, std::string());
        const std::vector<float> zooms = GetDisplayZoomFactors(managed_display_info.display_modes()[0]);
        ...
        for (auto zoom : zooms) {
            ...
            // Emulate how lacros computes the scale factor.
            const float scale_factor = size.width() / static_cast<float>(display.size().width());
            EXPECT_NEAR(scale_factor, display.device_scale_factor(), kDeviceScaleFactorErrorTolerance);
        }
    }
}
