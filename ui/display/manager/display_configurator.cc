...
bool IsDisplayIdInDisplayStateList(int64_t display_id, const DisplayConfigurator::DisplayStateList& display_list)
{
...
bool AreModesEqual(const DisplayMode& mode, const ManagedDisplayMode& managed_mode)
{
    return mode.size() == managed_mode.size() && mode.refresh_rate() == managed_mode.refresh_rate() && mode.is_interlaced() == managed_mode.is_interlaced();
}
...
const DisplayMode* FindExactMatchingMode(const DisplaySnapshot& display, const ManagedDisplayMode& managed_mode)
{
    if (managed_mode.native()) {
        return display.native_mode() && AreModesEqual(*display.native_mode(), managed_mode) ? display.native_mode() : nullptr;
    }
...
const int DisplayConfigurator::kSetDisplayPowerOnlyIfSingleInternalDisplay = 1 << 1;

////////////////////////////////////////////////////////////////////////////////
// DisplayConfigurator::DisplayLayoutManagerImpl implementation

class DisplayConfigurator::DisplayLayoutManagerImpl : public DisplayLayoutManager {
public:
    ...
    bool GetDisplayLayout(const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays, MultipleDisplayState new_display_state,
        chromeos::DisplayPowerState new_power_state, const base::flat_set<int64_t>& new_vrr_enabled_state,
...
private:
    ...
    // remove DisplayState.
    std::vector<DisplayState> ParseDisplays(const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays) const;
    ...
    bool AllDisplaysOnSameDevice(const std::vector<DisplayState*>& displays) const;

    // Return true if all displays have display mode.
    bool AllDisplaysHaveDisplayMode(const std::vector<DisplayState*>& displays) const;

    // Return true if |mode| has the same aspect ratio as the native mode of
    // |display|.
    bool HasSameAspectRatioAsNativeMode(const DisplaySnapshot* display, const DisplayMode* mode) const;
    ...
    bool FindExactMatchingMirrorMode(const std::vector<DisplayState*>& displays, bool preserve_native_aspect_ratio) const;
...
DisplayConfigurator::DisplayLayoutManagerImpl::DisplayLayoutManagerImpl(DisplayConfigurator* configurator)
...
DisplayConfigurator::SoftwareMirroringController* DisplayConfigurator::DisplayLayoutManagerImpl::GetSoftwareMirroringController() const
{
    return configurator_->mirroring_controller_;
}

DisplayConfigurator::StateController* DisplayConfigurator::DisplayLayoutManagerImpl::GetStateController() const
{
    return configurator_->state_controller_;
}

MultipleDisplayState DisplayConfigurator::DisplayLayoutManagerImpl::GetDisplayState() const
{
    return configurator_->current_display_state_;
}

chromeos::DisplayPowerState DisplayConfigurator::DisplayLayoutManagerImpl::GetPowerState() const
{
    return configurator_->current_power_state_;
}

std::vector<DisplayState> DisplayConfigurator::DisplayLayoutManagerImpl::ParseDisplays(
    const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& snapshots) const
{
    ...
    LOG_IF(WARNING, num_internal_displays >= 2) << "At least two internal displays detected.";
...
bool DisplayConfigurator::DisplayLayoutManagerImpl::GetDisplayLayout(const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays,
    MultipleDisplayState new_display_state, chromeos::DisplayPowerState new_power_state, const base::flat_set<int64_t>& new_vrr_enabled_state,
    std::vector<DisplayConfigureRequest>* requests) const
{
    ...
    VLOG(1) << "EnterState: display=" << MultipleDisplayStateToString(new_display_state) << " power=" << DisplayPowerStateToString(new_power_state);
    ...
    for (display::DisplaySnapshot* display : displays) {
        const bool enable_vrr
            = display->IsVrrCapable() && (::features::IsVariableRefreshRateAlwaysOn() || new_vrr_enabled_state.contains(display->display_id()));
        requests->emplace_back(display, display->current_mode(), gfx::Point(), enable_vrr);
    }

    switch (new_display_state) {
    case MULTIPLE_DISPLAY_STATE_INVALID:
        NOTREACHED() << "Ignoring request to enter invalid state with " << displays.size() << " connected display(s)";
    case MULTIPLE_DISPLAY_STATE_HEADLESS:
        if (displays.size() != 0) {
            LOG(WARNING) << "Ignoring request to enter headless mode with " << displays.size() << " connected display(s)";
            return false;
        }
        break;
    case MULTIPLE_DISPLAY_STATE_SINGLE: {
        ...
        if (displays.size() != 1 && num_on_displays != 1) {
            LOG(WARNING) << "Ignoring request to enter single mode with " << displays.size() << " connected displays and " << num_on_displays << " turned on";
            return false;
        }

        for (size_t i = 0; i < states.size(); ++i) {
            const DisplayState* state = &states[i];
            (*requests)[i].mode = display_power[i] && state->selected_mode ? state->selected_mode->Clone() : nullptr;
            ...
            if (display_power[i] || states.size() == 1) {
                ...
                if (mode_info->size() == gfx::Size(1024, 768)) {
                    VLOG(1) << "Potentially misdetecting display(1024x768):"
                            << " displays size=" << states.size() << ", num_on_displays=" << num_on_displays << ", current size:" << size.width() << "x"
                            << size.height() << ", i=" << i << ", display=" << state->display->ToString() << ", display_mode=" << mode_info->ToString();
                }
                size = mode_info->size();
            }
        }
    ...
    case MULTIPLE_DISPLAY_STATE_MULTI_MIRROR: {
        ...
        const bool can_set_mirror_mode = states.size() > 1 && num_on_displays != 1;
        if (!can_set_mirror_mode) {
            LOG(WARNING) << "Ignoring request to enter mirrored mode with " << states.size() << " connected display(s) and " << num_on_displays << " turned on";
            return false;
        }
        ...
        for (size_t i = 0; i < states.size(); ++i) {
            const DisplayState* state = &states[i];
            (*requests)[i].mode = display_power[i] && state->mirror_mode ? state->mirror_mode->Clone() : nullptr;
        }
        break;
    }
    case MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED: {
        if (states.size() < 2) {
            LOG(WARNING) << "Ignoring request to enter extended mode with " << states.size() << " connected display(s) and " << num_on_displays << " turned on";
            return false;
        }

        for (size_t i = 0; i < states.size(); ++i) {
            const DisplayState* state = &states[i];
            (*requests)[i].origin.set_y(size.height() ? size.height() + kVerticalGap : 0);
            (*requests)[i].mode = display_power[i] && state->selected_mode ? state->selected_mode->Clone() : nullptr;
...
DisplayConfigurator::DisplayStateList DisplayConfigurator::DisplayLayoutManagerImpl::GetDisplayStates() const
{
...
const DisplayMode* DisplayConfigurator::DisplayLayoutManagerImpl::GetUserSelectedMode(const DisplaySnapshot& display) const
{
    const DisplayMode* selected_mode = nullptr;
    auto* state_controller = GetStateController();
    if (state_controller) {
        ManagedDisplayMode mode;
        const bool mode_found = state_controller->GetSelectedModeForDisplayId(display.display_id(), &mode);
        if (display::features::IsListAllDisplayModesEnabled()) {
            ...
            selected_mode = mode_found ? FindDisplayModeMatchingSize(display, mode.size()) : nullptr;
        }
    }
...
bool DisplayConfigurator::DisplayLayoutManagerImpl::AllDisplaysOnSameDevice(const std::vector<DisplayState*>& displays) const
{
...
bool DisplayConfigurator::DisplayLayoutManagerImpl::AllDisplaysHaveDisplayMode(const std::vector<DisplayState*>& displays) const
{
...
bool DisplayConfigurator::DisplayLayoutManagerImpl::HasSameAspectRatioAsNativeMode(const DisplaySnapshot* display, const DisplayMode* mode) const
{
    return display->native_mode()->size().width() * mode->size().height() == display->native_mode()->size().height() * mode->size().width();
}

bool DisplayConfigurator::DisplayLayoutManagerImpl::FindExactMatchingMirrorMode(
    const std::vector<DisplayState*>& displays, bool preserve_native_aspect_ratio) const
{
    ...
    for (auto* d : displays) {
        ...
        for (auto& mode : d->display->modes()) {
            if (d->display->type() != DISPLAY_CONNECTION_TYPE_INTERNAL && preserve_native_aspect_ratio
        ...
        std::sort(mode_list.begin(), mode_list.end(), [](const DisplayMode* const& a, const DisplayMode* const& b) -> bool {
    ...
    for (; it_list[0] != mode_lists[0].end(); ++it_list[0]) {
        ...
        for (size_t i = 1; i < mode_lists.size(); ++i) {
            while (it_list[i] != mode_lists[i].end() && (*it_list[i])->size().GetArea() >= (*it_list[0])->size().GetArea()) {
                if ((*it_list[i])->size() == (*it_list[0])->size() && (*it_list[i])->is_interlaced() == (*it_list[0])->is_interlaced()) {
...
// static
const DisplayMode* DisplayConfigurator::FindDisplayModeMatchingSize(const DisplaySnapshot& display, const gfx::Size& size)
{
...
DisplayConfigurator::DisplayConfigurator()
    ...
    , content_protection_manager_(
          new ContentProtectionManager(layout_manager_.get(), base::BindRepeating(&DisplayConfigurator::configurator_disabled, base::Unretained(this))))
...
void DisplayConfigurator::SetDelegateForTesting(std::unique_ptr<NativeDisplayDelegate> display_delegate)
{
...
void DisplayConfigurator::SetInitialDisplayPower(chromeos::DisplayPowerState power_state)
{
...
void DisplayConfigurator::Init(std::unique_ptr<NativeDisplayDelegate> display_delegate, bool is_panel_fitting_enabled)
{
    ...
    content_protection_manager_->set_native_display_delegate(native_display_delegate_.get());
}
...
void DisplayConfigurator::TakeControl(DisplayControlCallback callback)
{
    ...
    if (!display_externally_controlled_) {
        LOG(ERROR) << __func__ << " failed. Displays are not controlled externally.";
    ...
    native_display_delegate_->TakeDisplayControl(
        base::BindOnce(&DisplayConfigurator::OnDisplayControlTaken, weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
}

void DisplayConfigurator::OnDisplayControlTaken(DisplayControlCallback callback, bool success)
{
    TRACE_EVENT1("ui", "DisplayConfigurator::OnDisplayControlTaken", "success", success);
    display_control_changing_ = false;
    ...
    if (success) {
        ...
        if (requested_power_state_) {
            ...
            SetDisplayPower(*requested_power_state_, kSetDisplayPowerNoFlags, base::DoNothing());
        }
    }
...
void DisplayConfigurator::RelinquishControl(DisplayControlCallback callback)
{
    ...
    if (display_externally_controlled_) {
        LOG(ERROR) << __func__ << " failed. Displays are already controlled externally.";
        std::move(callback).Run(true);
        return;
    }
    ...
    if (configuration_task_) {
        LOG(ERROR) << __func__ << " failed. There is a display configuration in progress.";
        std::move(callback).Run(false);
        return;
    }
    ...
    // them for output.
    SetDisplayPowerInternal(chromeos::DISPLAY_POWER_ALL_OFF, kSetDisplayPowerNoFlags,
        base::BindOnce(&DisplayConfigurator::SendRelinquishDisplayControl, weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
}

void DisplayConfigurator::GetSeamlessRefreshRates(int64_t display_id, GetSeamlessRefreshRatesCallback callback)
{
    native_display_delegate_->GetSeamlessRefreshRates(display_id, std::move(callback));
}

void DisplayConfigurator::SendRelinquishDisplayControl(DisplayControlCallback callback, bool success)
{
    TRACE_EVENT1("ui", "DisplayConfigurator::SendRelinquishDisplayControl", "success", success);
    if (success) {
        ...
        native_display_delegate_->RelinquishDisplayControl(
            base::BindOnce(&DisplayConfigurator::OnDisplayControlRelinquished, weak_ptr_factory_.GetWeakPtr(), std::move(callback)));
    } else {
        LOG(ERROR) << __func__
...
void DisplayConfigurator::OnDisplayControlRelinquished(DisplayControlCallback callback, bool success)
{
    TRACE_EVENT1("ui", "DisplayConfigurator::OnDisplayControlRelinquished", "success", success);
...
void DisplayConfigurator::ForceInitialConfigure()
{
    ...
    configuration_task_ = std::make_unique<UpdateDisplayConfigurationTask>(native_display_delegate_.get(), layout_manager_.get(), requested_display_state_,
        GetRequestedPowerState(), kSetDisplayPowerForceProbe, GetRequestedVrrState(), GetRequestedRefreshRateOverrides(),
        /*force_configure=*/true, kConfigurationTypeFull, base::BindOnce(&DisplayConfigurator::OnConfigured, weak_ptr_factory_.GetWeakPtr()));
    configuration_task_->Run();
}

void DisplayConfigurator::SetColorTemperatureAdjustment(int64_t display_id, const ColorTemperatureAdjustment& cta)
{
...
void DisplayConfigurator::SetColorCalibration(int64_t display_id, const ColorCalibration& calibration)
{
    if (!IsDisplayIdInDisplayStateList(display_id, cached_displays_)) {
        return;
    }
...
void DisplayConfigurator::SetPrivacyScreen(int64_t display_id, bool enabled, ConfigurationCallback callback)
{
#if DCHECK_IS_ON()
    ...
    native_display_delegate_->SetPrivacyScreen(display_id, enabled, std::move(callback));
}

chromeos::DisplayPowerState DisplayConfigurator::GetRequestedPowerState() const
{
...
void DisplayConfigurator::SetDisplayPowerInternal(chromeos::DisplayPowerState power_state, int flags, ConfigurationCallback callback)
{
    ...
    if (power_state == current_power_state_ && power_state == pending_power_state_ && !(flags & kSetDisplayPowerForceProbe)) {
        std::move(callback).Run(true);
        return;
    }
...
void DisplayConfigurator::SetDisplayPower(chromeos::DisplayPowerState power_state, int flags, ConfigurationCallback callback)
{
    if (configurator_disabled()) {
        std::move(callback).Run(false);
        return;
    ...
    VLOG(1) << "SetDisplayPower: power_state=" << DisplayPowerStateToString(power_state) << " flags=" << flags
            << ", configure timer=" << (configure_timer_.IsRunning() ? "Running" : "Stopped");

    requested_power_state_ = power_state;
    SetDisplayPowerInternal(*requested_power_state_, flags, std::move(callback));
}

void DisplayConfigurator::SetMultipleDisplayState(MultipleDisplayState new_state)
{
    if (configurator_disabled())
        return;

    VLOG(1) << "SetMultipleDisplayState: state=" << MultipleDisplayStateToString(new_state);
    if (current_display_state_ == new_state) {
        ...
        if (mirroring_controller_ && new_state == MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED)
...
void DisplayConfigurator::OnConfigurationChanged()
{
    ...
    configure_timer_.Start(FROM_HERE, base::Milliseconds(kConfigureDelayMs), this, &DisplayConfigurator::ConfigureDisplays);
}
...
void DisplayConfigurator::SuspendDisplays(ConfigurationCallback callback)
{
    ...
    SetDisplayPowerInternal(chromeos::DISPLAY_POWER_ALL_OFF, kSetDisplayPowerNoFlags, std::move(callback));
}
...
void DisplayConfigurator::ResumeDisplays()
{
    ...
    if (current_display_state_ == MULTIPLE_DISPLAY_STATE_MULTI_MIRROR || current_display_state_ == MULTIPLE_DISPLAY_STATE_MULTI_EXTENDED) {
        ...
        configure_timer_.Start(FROM_HERE, base::Milliseconds(kResumeConfigureMultiDisplayDelayMs), this, &DisplayConfigurator::ConfigureDisplays);
    }
...
void DisplayConfigurator::RunPendingConfiguration()
{
    ...
    configuration_task_ = std::make_unique<UpdateDisplayConfigurationTask>(native_display_delegate_.get(), layout_manager_.get(), requested_display_state_,
        pending_power_state_, pending_power_flags_, GetRequestedVrrState(), GetRequestedRefreshRateOverrides(), force_configure_, configuration_type,
        base::BindOnce(&DisplayConfigurator::OnConfigured, weak_ptr_factory_.GetWeakPtr()));
...
void DisplayConfigurator::OnConfigured(bool success, const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays,
    const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& unassociated_displays, MultipleDisplayState new_display_state,
    chromeos::DisplayPowerState new_power_state)
{
    VLOG(1) << "OnConfigured: success=" << success << " new_display_state=" << MultipleDisplayStateToString(new_display_state)
    ...
    if (success && !configure_timer_.IsRunning() && ShouldRunConfigurationTask()) {
        configure_timer_.Start(FROM_HERE, base::Milliseconds(kConfigureDelayMs), this, &DisplayConfigurator::RunPendingConfiguration);
    } else {
...
void DisplayConfigurator::UpdatePowerState(chromeos::DisplayPowerState new_power_state)
{
...
void DisplayConfigurator::NotifyDisplayStateObservers(bool success, MultipleDisplayState attempted_state)
{
    if (success) {
        observers_.Notify(&Observer::OnDisplayConfigurationChanged, cached_displays_);
    } else {
        observers_.Notify(&Observer::OnDisplayConfigurationChangeFailed, cached_displays_, attempted_state);
    }
}
...
void DisplayConfigurator::SetRefreshRateOverrides(const RefreshRateOverrideMap& requested_overrides)
{
    if (HasPendingFullConfiguration()) {
    ...
    for (auto& snapshot : cached_displays_) {
        ...
        if (it != requested_overrides.end() && it->second != snapshot->native_mode()->refresh_rate()) {
            effective_overrides[snapshot->display_id()] = it->second;
        }
    }

    LOG_IF(WARNING, requested_overrides != effective_overrides) << "Some requested overrides are invalid and have been removed.\n"
                                                                << "\tRequested: " << RefreshRateOverrideToString(requested_overrides) << "\n"
                                                                << "\tActual: " << RefreshRateOverrideToString(effective_overrides);

    const RefreshRateOverrideMap current_overrides = GetCurrentRefreshRateOverrideState();
...
void DisplayConfigurator::SetVrrEnabled(const base::flat_set<int64_t>& display_ids)
{
    ...
    for (const display::DisplaySnapshot* display : cached_displays_) {
        ...
        const bool vrr_should_be_enabled = display_ids.contains(display->display_id());
...
const base::flat_set<int64_t> DisplayConfigurator::GetRequestedVrrState() const
{
...
RefreshRateOverrideMap DisplayConfigurator::GetRequestedRefreshRateOverrides() const
{
...
RefreshRateOverrideMap DisplayConfigurator::GetCurrentRefreshRateOverrideState() const
{
    ...
    for (DisplaySnapshot* cached_display : cached_displays_) {
        ...
        if (native_mode->refresh_rate() != current_mode->refresh_rate()) {
            VLOG(3) << "Current override state for display with id (" << cached_display->display_id() << "): " << current_mode->refresh_rate();
...
{
    return configurator_->configure_timer_.IsRunning() ? configurator_->configure_timer_.GetCurrentDelay() : base::TimeDelta();
}

DisplayLayoutManager* DisplayConfigurator::TestApi::GetDisplayLayoutManager() const
{
