...
class DISPLAY_MANAGER_EXPORT DisplayConfigurator : public NativeDisplayObserver {
public:
    ...
    using GetSeamlessRefreshRatesCallback = base::OnceCallback<void(const std::optional<std::vector<float>>&)>;

    using DisplayStateList = std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>;
    ...
    public:
        ...
        virtual void OnDisplayConfigurationChanged(const DisplayStateList& displays)
        {
        }
        ...
        virtual void OnDisplayConfigurationChangeFailed(const DisplayStateList& displays, MultipleDisplayState failed_new_state)
        {
        }
    ...
    public:
        ...
        virtual MultipleDisplayState GetStateForDisplayIds(const DisplayConfigurator::DisplayStateList& outputs) = 0;

        virtual bool GetSelectedModeForDisplayId(int64_t display_id, ManagedDisplayMode* out_mode) const = 0;
    };
    ...
    static const DisplayMode* FindDisplayModeMatchingSize(const DisplaySnapshot& display, const gfx::Size& size);
    ...
    MultipleDisplayState display_state() const
    {
        return current_display_state_;
    }
    const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& cached_displays() const
    {
    ...
    bool has_unassociated_display() const
    {
        return has_unassociated_display_;
    }
    ...
    void SetDelegateForTesting(std::unique_ptr<NativeDisplayDelegate> display_delegate);
    ...
    void Init(std::unique_ptr<NativeDisplayDelegate> delegate, bool is_panel_fitting_enabled);
    ...
    // operation.
    void SetDisplayPower(chromeos::DisplayPowerState power_state, int flags, ConfigurationCallback callback);
    ...
    void GetSeamlessRefreshRates(int64_t display_id, GetSeamlessRefreshRatesCallback callback);
    ...
    void SetColorTemperatureAdjustment(int64_t display_id, const ColorTemperatureAdjustment& cta);

    // Sets the color calibration for the specified display;
    void SetColorCalibration(int64_t display_id, const ColorCalibration& calibration);
    ...
    void SetPrivacyScreen(int64_t display_id, bool enabled, ConfigurationCallback callback);
    ...
    std::optional<chromeos::DisplayPowerState> GetRequestedPowerStateForTest() const
    {
...
private:
    ...
    void SetDisplayPowerInternal(chromeos::DisplayPowerState power_state, int flags, ConfigurationCallback callback);

    // Configures displays. Invoked by |configure_timer_|.
    void ConfigureDisplays();

    // Notifies observers about an attempted state change.
    void NotifyDisplayStateObservers(bool success, MultipleDisplayState attempted_state);
    ...
    // in |power_state|.
    MultipleDisplayState ChooseDisplayState(chromeos::DisplayPowerState power_state) const;
    ...
    void OnConfigured(bool success, const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays,
    ...
    void OnDisplayControlRelinquished(DisplayControlCallback callback, bool success);
    ...
    // RelinquishDisplay().
    void SendRelinquishDisplayControl(DisplayControlCallback callback, bool success);
