...
std::string RefreshRateOverrideToString(const std::unordered_map<int64_t, float>& refresh_rate_override)
{
    std::vector<std::string> entries;
...
int GetDisplayPower(
    const std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>>& displays, chromeos::DisplayPowerState state, std::vector<bool>* display_power)
{
    ...
    for (size_t i = 0; i < displays.size(); ++i) {
        ...
        bool on = state == chromeos::DISPLAY_POWER_ALL_ON || (state == chromeos::DISPLAY_POWER_INTERNAL_OFF_EXTERNAL_ON && !internal)
...
bool GetContentProtectionMethods(DisplayConnectionType type, uint32_t* protection_mask)
{
    switch (type) {
...
std::vector<float> GetDisplayZoomFactors(const ManagedDisplayMode& mode)
{
    ...
    const int effective_width = std::round(static_cast<float>(std::max(mode.size().width(), mode.size().height())) / mode.device_scale_factor());
...
ManagedDisplayInfo::ManagedDisplayModeList CreateInternalManagedDisplayModeList(const ManagedDisplayMode& native_mode)
{
    ManagedDisplayMode mode(native_mode.size(), native_mode.refresh_rate(), native_mode.is_interlaced(), true, native_mode.device_scale_factor());
    return ManagedDisplayInfo::ManagedDisplayModeList { mode };
}

UnifiedDisplayModeParam::UnifiedDisplayModeParam(float dsf, float scale, bool is_default)
...
ManagedDisplayInfo::ManagedDisplayModeList CreateUnifiedManagedDisplayModeList(
    const ManagedDisplayMode& native_mode, const std::vector<UnifiedDisplayModeParam>& modes_param_list)
{
    ...
    for (auto& param : modes_param_list) {
        ...
        display_mode_list.emplace_back(gfx::ToFlooredSize(scaled_size), native_mode.refresh_rate(), native_mode.is_interlaced(),
            param.is_default_mode ? true : false /* native */, param.device_scale_factor);
    }
    // Sort the mode by the size in DIP.
    std::sort(display_mode_list.begin(), display_mode_list.end(),
        [](const ManagedDisplayMode& a, const ManagedDisplayMode& b) { return a.GetSizeInDIP().GetArea() < b.GetSizeInDIP().GetArea(); });
...
bool ForceFirstDisplayInternal()
{
    ...
    return command_line->HasSwitch(::switches::kUseFirstDisplayAsInternal)
        || (ash::system::StatisticsProvider::GetInstance()->IsRunningOnVm() && !command_line->HasSwitch(switches::kDRMVirtualConnectorIsExternal));
}

bool ComputeBoundary(const gfx::Rect& a_bounds, const gfx::Rect& b_bounds, gfx::Rect* a_edge, gfx::Rect* b_edge)
{
    // Find touching side.
...
bool ComputeBoundary(const Display& display_a, const Display& display_b, gfx::Rect* a_edge_in_screen, gfx::Rect* b_edge_in_screen)
{
    return ComputeBoundary(display_a.bounds(), display_b.bounds(), a_edge_in_screen, b_edge_in_screen);
}
...
{
    return GenerateDisplayIdList(updated_displays, &display::ManagedDisplayInfo::id);
}

void SortDisplayIdList(DisplayIdList* ids)
{
    std::sort(ids->begin(), ids->end(), [](int64_t a, int64_t b) { return CompareDisplayIds(a, b); });
}

bool IsDisplayIdListSorted(const DisplayIdList& list)
{
    return std::is_sorted(list.begin(), list.end(), [](int64_t a, int64_t b) { return CompareDisplayIds(a, b); });
}
...
MixedMirrorModeParams::MixedMirrorModeParams(int64_t src_id, const DisplayIdList& dst_ids)
    : source_id(src_id)
    , destination_ids(dst_ids)
{
}

MixedMirrorModeParams::MixedMirrorModeParams(const MixedMirrorModeParams& mixed_params) = default;

MixedMirrorModeParams::~MixedMirrorModeParams() = default;

MixedMirrorModeParamsErrors ValidateParamsForMixedMirrorMode(const DisplayIdList& connected_display_ids, const MixedMirrorModeParams& mixed_params)
{
