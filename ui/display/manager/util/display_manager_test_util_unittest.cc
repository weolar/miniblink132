...
TEST(DisplayManagerTestUtilTest, TestResetDisplayIdForTest)
{
    ...
    const int64_t kFirstEdidConnectorIndex = GetNextSynthesizedEdidDisplayConnectorIndex();
    ...
    EXPECT_EQ(kFirstEdidConnectorIndex, GetNextSynthesizedEdidDisplayConnectorIndex());
}
...
TEST(DisplayManagerTestUtilTest, TestGetASynthesizedDisplayId)
{
    ...
    feature_list.InitAndEnableFeature(display::features::kEnableEdidBasedDisplayIds);
    ResetDisplayIdForTest();
...
TEST(DisplayManagerTestUtilTest, TestSynthesizeDisplayIdFromSeed)
{
    ...
    const int64_t kSecondIdBase = static_cast<int64_t>(~static_cast<uint64_t>(0xFF) & kSecondId);
    ...
    constexpr int64_t kExpectedIdAfterInitialValue = kSynthesizedDisplayIdStart + 0x101;
    EXPECT_EQ(kExpectedIdAfterInitialValue, SynthesizeDisplayIdFromSeed(kSynthesizedDisplayIdStart));
}

TEST(DisplayManagerTestUtilTest, TestProduceAlternativeSchemeIdForId)
{
    EXPECT_EQ(1200000000, ProduceAlternativeSchemeIdForId(kSynthesizedDisplayIdStart));
