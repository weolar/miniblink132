...
int64_t GetASynthesizedDisplayId()
{
    ...
    next_synthesized_display_id
        = features::IsEdidBasedDisplayIdsEnabled() ? next_synthesized_display_id += 0x100 : SynthesizeDisplayIdFromSeed(next_synthesized_display_id);
    return id;
}
...
int64_t GetNextSynthesizedEdidDisplayConnectorIndex()
{
    ...
    DCHECK_LT(edid_device_index, 255) << "Connector index exceeded 65536. Cannot "
