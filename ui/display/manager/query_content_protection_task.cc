...
QueryContentProtectionTask::QueryContentProtectionTask(
    DisplayLayoutManager* layout_manager, NativeDisplayDelegate* native_display_delegate, int64_t display_id, ResponseCallback callback)
...
{
    if (callback_) {
        std::move(callback_).Run(Status::KILLED, DISPLAY_CONNECTION_TYPE_NONE, CONTENT_PROTECTION_METHOD_NONE);
    }
}
...
void QueryContentProtectionTask::Run()
{
    ...
    for (DisplaySnapshot* display : layout_manager_->GetDisplayStates()) {
        ...
        if (!GetContentProtectionMethods(display->type(), &protection_mask)) {
            std::move(callback_).Run(Status::FAILURE, DISPLAY_CONNECTION_TYPE_UNKNOWN, CONTENT_PROTECTION_METHOD_NONE);
            return;
        }
    ...
    if (pending_requests_ == 0) {
        std::move(callback_).Run(Status::SUCCESS, connection_mask_, CONTENT_PROTECTION_METHOD_NONE);
        return;
    }

    for (DisplaySnapshot* display : hdcp_capable_displays) {
        native_display_delegate_->GetHDCPState(*display, base::BindOnce(&QueryContentProtectionTask::OnGetHDCPState, weak_ptr_factory_.GetWeakPtr()));
    }
}

void QueryContentProtectionTask::OnGetHDCPState(bool success, HDCPState state, ContentProtectionMethod protection_method)
{
    success_ &= success;
    ...
    if ((protection_mask_ & kContentProtectionMethodHdcpAll) == kContentProtectionMethodHdcpAll) {
    ...
    std::move(callback_).Run(success_ ? Status::SUCCESS : Status::FAILURE, connection_mask_, protection_mask_);
}
