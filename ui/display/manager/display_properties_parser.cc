...
std::optional<gfx::RoundedCornersF> ParsePanelRadii(const base::Value* json_value)
{
...
std::optional<gfx::RoundedCornersF> ParseDisplayPanelRadii(const base::Value* json_value)
{
    ...
    std::optional<gfx::RoundedCornersF> panel_radii = ParsePanelRadii(display_info.Find(kDisplayRadiiKeyName));

    if (!panel_radii.has_value()) {
        LOG(ERROR) << base::StringPrintf("Invalid format of `%s` specified through display-properties "
                                         "switch",
