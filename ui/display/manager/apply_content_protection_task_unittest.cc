...
std::unique_ptr<DisplaySnapshot> CreateDisplaySnapshot(DisplayConnectionType type, int display_id = kDisplayId1)
{
    return FakeDisplaySnapshot::Builder().SetId(display_id).SetNativeMode(gfx::Size(1024, 768)).SetType(type).Build();
}
...
public:
    ...
    ApplyContentProtectionTaskTest(const ApplyContentProtectionTaskTest&) = delete;
    ApplyContentProtectionTaskTest& operator=(const ApplyContentProtectionTaskTest&) = delete;
...
TEST_F(ApplyContentProtectionTaskTest, ApplyHdcpToInternalDisplay)
{
    ...
    ApplyContentProtectionTask task(
        &layout_manager, &display_delegate_, request, base::BindOnce(&ApplyContentProtectionTaskTest::ResponseCallback, base::Unretained(this)));
    task.Run();
...
TEST_F(ApplyContentProtectionTaskTest, ApplyHdcpToExternalDisplay)
{
    ...
    EXPECT_EQ(Response::SUCCESS, response_);
    EXPECT_EQ(GetSetHDCPStateAction(kDisplayId1, HDCP_STATE_DESIRED, CONTENT_PROTECTION_METHOD_HDCP).c_str(), log_.GetActionsAndClear());
}
...
{
    std::vector<std::unique_ptr<DisplaySnapshot>> displays;
    displays.push_back(CreateDisplaySnapshot(DISPLAY_CONNECTION_TYPE_HDMI, kDisplayId1));
    displays.push_back(CreateDisplaySnapshot(DISPLAY_CONNECTION_TYPE_HDMI, kDisplayId2));
    display_delegate_.SetOutputs(std::move(displays));
    ...
    EXPECT_EQ(Response::SUCCESS, response_);
    EXPECT_EQ(JoinActions(GetSetHDCPStateAction(kDisplayId1, HDCP_STATE_DESIRED, CONTENT_PROTECTION_METHOD_HDCP_TYPE_0).c_str(),
                  GetSetHDCPStateAction(kDisplayId2, HDCP_STATE_DESIRED, CONTENT_PROTECTION_METHOD_HDCP_TYPE_1).c_str(), nullptr),
...
TEST_F(ApplyContentProtectionTaskTest, ApplyHdcpToMirroredMonitors)
{
    ...
    EXPECT_EQ(Response::SUCCESS, response_);
    EXPECT_EQ(JoinActions(GetSetHDCPStateAction(kDisplayId1, HDCP_STATE_DESIRED, CONTENT_PROTECTION_METHOD_HDCP_TYPE_1).c_str(),
                  GetSetHDCPStateAction(kDisplayId2, HDCP_STATE_DESIRED, CONTENT_PROTECTION_METHOD_HDCP_TYPE_1).c_str(), nullptr),
        log_.GetActionsAndClear());
}
