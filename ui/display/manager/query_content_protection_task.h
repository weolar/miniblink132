...
class DISPLAY_MANAGER_EXPORT QueryContentProtectionTask : public ContentProtectionManager::Task {
public:
    ...
    using ResponseCallback = base::OnceCallback<void(Status status, uint32_t connection_mask, uint32_t protection_mask)>;
    ...
    QueryContentProtectionTask& operator=(const QueryContentProtectionTask&) = delete;
...
private:
    void OnGetHDCPState(bool success, HDCPState state, ContentProtectionMethod protection_method);
