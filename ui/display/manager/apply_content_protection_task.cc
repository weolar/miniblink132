...
ApplyContentProtectionTask::ApplyContentProtectionTask(DisplayLayoutManager* layout_manager, NativeDisplayDelegate* native_display_delegate,
    ContentProtectionManager::ContentProtections requests, ResponseCallback callback)
...
void ApplyContentProtectionTask::Run()
{
    ...
    for (DisplaySnapshot* display : hdcp_capable_displays) {
        native_display_delegate_->GetHDCPState(
            *display, base::BindOnce(&ApplyContentProtectionTask::OnGetHDCPState, weak_ptr_factory_.GetWeakPtr(), display->display_id()));
    }
}

void ApplyContentProtectionTask::OnGetHDCPState(int64_t display_id, bool success, HDCPState state, ContentProtectionMethod protection_method)
{
    success_ &= success;

    bool hdcp_enabled = state != HDCP_STATE_UNDESIRED;
    uint32_t desired_hdcp_protections = GetDesiredProtectionMask(display_id) & kContentProtectionMethodHdcpAll;
    ...
    if (hdcp_enabled != !!desired_hdcp_protections || desired_hdcp_protections != protection_method) {
        ...
        hdcp_requests_.emplace_back(display_id, desired_hdcp_protections ? HDCP_STATE_DESIRED : HDCP_STATE_UNDESIRED, new_method);
    }
    ...
    std::vector<raw_ptr<DisplaySnapshot, VectorExperimental>> displays = layout_manager_->GetDisplayStates();
    std::vector<std::tuple<DisplaySnapshot*, HDCPState, ContentProtectionMethod>> hdcped_displays;
    // Lookup the displays again since display configuration may have changed.
    for (const auto& request : hdcp_requests_) {
        auto it = base::ranges::find(displays, request.display_id, &DisplaySnapshot::display_id);
        if (it == displays.end()) {
            std::move(callback_).Run(Status::FAILURE);
            return;
        }
    ...
    for (const auto& tuple : hdcped_displays) {
        native_display_delegate_->SetHDCPState(*std::get<0>(tuple), std::get<1>(tuple), std::get<2>(tuple),
            base::BindOnce(&ApplyContentProtectionTask::OnSetHDCPState, weak_ptr_factory_.GetWeakPtr()));
    }
}
...
uint32_t ApplyContentProtectionTask::GetDesiredProtectionMask(int64_t display_id) const
{
