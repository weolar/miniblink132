...
void AppendScreen(x11::RandRMonitorLayout& layout, const gfx::Size& resolution, const gfx::Vector2d& dpi)
{
    ...
    for (const auto& screen : layout.configs) {
        if (rightmost_layout == nullptr || screen.rect().right() > rightmost_layout->rect().right()) {
            rightmost_layout = &screen;
        }
    }
    layout.configs.emplace_back(
        std::nullopt, gfx::Rect(rightmost_layout->rect().right() + 1, rightmost_layout->rect().y(), resolution.width(), resolution.height()), dpi);
}
} // namespace
...
bool VirtualDisplayUtilLinux::IsAPIAvailable()
{
    ...
    x11::Response<x11::RandR::GetScreenResourcesCurrentReply> screen_resources
        = xrandr.GetScreenResourcesCurrent({ x11_connection->default_screen().root }).Sync();
    ...
    for (const auto& output : screen_resources.reply->outputs) {
        std::unique_ptr<x11::RandR::GetOutputInfoReply> output_reply = xrandr.GetOutputInfo(output, screen_resources.reply->config_timestamp).Sync().reply;
        if (output_reply && output_reply->connection == kConnected && output_reply->crtc == kDisabled) {
...
int64_t VirtualDisplayUtilLinux::AddDisplay(const DisplayParams& display_params)
{
    if (current_layout_.configs.size() - initial_layout_.configs.size() > kMaxDisplays) {
    ...
    AppendScreen(last_requested_layout_, display_params.resolution, display_params.dpi);
    randr_output_manager_->SetLayout(last_requested_layout_);
    size_t initial_detected_displays = detected_added_display_ids_.size();
    StartWaiting();
    CHECK_EQ(detected_added_display_ids_.size(), initial_detected_displays + 1u) << "Did not detect exactly one new display.";
    ...
    for (const auto& layout : current_layout_.configs) {
        auto was_added = std::find_if(
            prev_layout.configs.begin(), prev_layout.configs.end(), [&](const x11::RandRMonitorConfig& prev) { return prev.rect() == layout.rect(); });
...
void VirtualDisplayUtilLinux::RemoveDisplay(int64_t display_id)
{
    if (!display_id_to_randr_id_.contains(display_id)) {
        LOG(ERROR) << "Invalid display_id. Missing mapping for " << display_id << " to randr ID.";
        return;
    }
    last_requested_layout_ = current_layout_;
    x11::RandRMonitorConfig::ScreenId randr_id = display_id_to_randr_id_[display_id];
    std::erase_if(last_requested_layout_.configs, [&](const x11::RandRMonitorConfig& layout) { return layout.id() == randr_id; });
    randr_output_manager_->SetLayout(last_requested_layout_);
    StartWaiting();
}
...
void VirtualDisplayUtilLinux::OnDisplayAdded(const display::Display& new_display)
{
    detected_added_display_ids_.push_back(new_display.id());
    OnDisplayAddedOrRemoved(new_display.id());
}

void VirtualDisplayUtilLinux::OnDisplaysRemoved(const display::Displays& removed_displays)
{
    for (const auto& display : removed_displays) {
        base::EraseIf(display_id_to_randr_id_, [&](std::pair<DisplayId, x11::RandRMonitorConfig::ScreenId>& pair) { return pair.first == display.id(); });
        base::EraseIf(detected_added_display_ids_, [&](DisplayId& id) { return id == display.id(); });
...
bool VirtualDisplayUtilLinux::RequestedLayoutIsSet()
{
    ...
    return last_requested_layout_.configs.size() - initial_layout_.configs.size() == detected_added_display_ids_.size();
}
