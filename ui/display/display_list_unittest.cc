...
private:
    static void AddPartChange(uint32_t changed, uint32_t part, const std::string& description, std::string* changed_string)
    {
    ...
    void OnDisplayMetricsChanged(const Display& display, uint32_t changed_metrics) override
    {
        std::string parts;
        AddPartChange(changed_metrics, DISPLAY_METRIC_BOUNDS, "bounds", &parts);
        AddPartChange(changed_metrics, DISPLAY_METRIC_WORK_AREA, "work_area", &parts);
        AddPartChange(changed_metrics, DISPLAY_METRIC_DEVICE_SCALE_FACTOR, "scale_factor", &parts);
...
TEST(DisplayListTest, AddUpdateRemove)
{
    ...
    display_list.AddDisplay(Display(2, gfx::Rect(0, 0, 801, 802)), DisplayList::Type::PRIMARY);
    ...
    // Add another.
    display_list.AddDisplay(Display(3, gfx::Rect(0, 0, 809, 802)), DisplayList::Type::NOT_PRIMARY);
    ...
    display_list.UpdateDisplay(display_list.displays()[1], DisplayList::Type::PRIMARY);
...
{
    DisplayList display_list;
    EXPECT_DCHECK_DEATH(display_list.AddDisplay(Display(1), DisplayList::Type::NOT_PRIMARY));
}
...
TEST(DisplayListTest, DisplaysIdsMustBeUnique)
{
    ...
    EXPECT_DCHECK_DEATH(display_list.AddDisplay(Display(1), DisplayList::Type::PRIMARY));
}

TEST(DisplayListTest, GetPrimaryDisplayEmpty)
{
    DisplayList display_list;
    EXPECT_EQ(display_list.displays().end(), display_list.GetPrimaryDisplayIterator());
}
...
TEST(DisplayListTest, GetPrimaryDisplayOk)
{
    DisplayList display_list;
    display_list.AddDisplay(Display(1), DisplayList::Type::PRIMARY);
    EXPECT_NE(display_list.displays().end(), display_list.GetPrimaryDisplayIterator());
