...
ColorMixer::ColorMixer(MixerGetter previous_mixer_getter, MixerGetter input_mixer_getter)
...
{
    const ColorMixer* previous_mixer = previous_mixer_getter_ ? previous_mixer_getter_.Run() : nullptr;
    // Don't log transitions to previous mixers unless the logging level is a
    // little higher.
    DVLOG_IF(3, previous_mixer) << "GetInputColor: ColorId " << ColorIdName(id) << " not found. "
                                << "Checking previous mixer.";
    // If there's no previous mixer, always log color id misses.
    DVLOG_IF(2, !previous_mixer) << "GetInputColor: ColorId " << ColorIdName(id) << " not found. "
                                 << "Returning gfx::kPlaceholderColor.";
    return previous_mixer ? previous_mixer->GetResultColor(id) : gfx::kPlaceholderColor;
}
...
SkColor ColorMixer::GetResultColor(ColorId id) const
{
    ...
    const SkColor input_color = (recipe_in_mixer && i->second.Invariant()) ? gfx::kPlaceholderColor : GetInputColor(id);

    if (!recipe_in_mixer) {
        return input_color;
    }

    const ColorMixer* const mixer = input_mixer_getter_ ? input_mixer_getter_.Run() : nullptr;
...
std::set<ColorId> ColorMixer::GetDefinedColorIds() const
{
    ...
    base::ranges::transform(recipes_, std::inserter(color_ids, color_ids.end()), &std::pair<const ColorId, ColorRecipe>::first);
