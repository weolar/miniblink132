...
void AddGeneratedPalette(ColorProvider* provider, SkColor seed_color, ColorProviderKey::SchemeVariant variant)
{
...
void AddRefColorMixer(ColorProvider* provider, const ColorProviderKey& key)
{
    ...
    // is set.
    if (!key.user_color.has_value() || key.user_color_source == ColorProviderKey::UserColorSource::kBaseline
        ...
        auto variant = key.scheme_variant.value_or(ColorProviderKey::SchemeVariant::kTonalSpot);
