...
TEST(ColorTransformTest, AlphaBlend)
{
    const auto blend = [](SkAlpha alpha) {
        const ColorTransform transform = AlphaBlend(SK_ColorWHITE, SK_ColorBLACK, alpha);
...
{
    const ColorTransform transform = BlendForMinContrast(FromTransformInput(), kColorTest0);
    constexpr SkColor kBackground = SK_ColorWHITE;
    ColorMixer mixer;
    mixer[kColorTest0] = { kBackground };
    const auto verify_contrast = [&](SkColor input) {
        EXPECT_GE(color_utils::GetContrastRatio(transform.Run(input, mixer), kBackground), color_utils::kMinimumReadableContrastRatio);
    };
...
TEST(ColorTransformTest, BlendForMinContrastOptionalArgs)
{
    ...
    const ColorTransform transform = BlendForMinContrast(FromTransformInput(), kColorTest0, kColorTest1, kMinContrast);
    constexpr SkColor kBackground = SK_ColorWHITE;
    ColorMixer mixer;
    mixer[kColorTest0] = { kBackground };
    mixer[kColorTest1] = { gfx::kGoogleBlue900 };
    const auto verify_contrast = [&](SkColor input) { EXPECT_GE(color_utils::GetContrastRatio(transform.Run(input, mixer), kBackground), kMinContrast); };
    verify_contrast(SK_ColorBLACK);
    verify_contrast(SK_ColorWHITE);
...
TEST(ColorTransformTest, BlendForMinContrastWithSelf)
{
    ...
    const ColorTransform transform = BlendForMinContrastWithSelf(FromTransformInput(), kContrastRatio);
    const auto verify_blend = [&](SkColor input) {
        const SkColor target = color_utils::GetColorWithMaxContrast(input);
        EXPECT_LT(color_utils::GetContrastRatio(transform.Run(input, ColorMixer()), target), color_utils::GetContrastRatio(input, target));
    };
...
TEST(ColorTransformTest, BlendTowardMaxContrast)
{
    ...
    const ColorTransform transform = BlendTowardMaxContrast(FromTransformInput(), kAlpha);
    const auto verify_blend = [&](SkColor input) {
        const SkColor target = color_utils::GetColorWithMaxContrast(input);
        EXPECT_LT(color_utils::GetContrastRatio(transform.Run(input, ColorMixer()), target), color_utils::GetContrastRatio(input, target));
    };
...
TEST(ColorTransformTest, ContrastInvert)
{
    ...
    const auto verify_invert = [&](SkColor input) {
        ...
        const SkColor near_endpoint = color_utils::GetColorWithMaxContrast(far_endpoint);
        EXPECT_GE(color_utils::GetContrastRatio(transform.Run(input, ColorMixer()), near_endpoint), color_utils::GetContrastRatio(input, far_endpoint));
    };
...
TEST(ColorTransformTest, DeriveDefaultIconColor)
{
    ...
    const auto verify_derive = [&](SkColor input) { EXPECT_NE(input, transform.Run(input, ColorMixer())); };
...
TEST(ColorTransformTest, FromColor)
{
    ...
    const auto verify_color = [&](SkColor input) { EXPECT_EQ(kOutput, transform.Run(input, ColorMixer())); };
...
TEST(ColorTransformTest, FromColorId)
{
    ...
    const auto verify_color = [&](SkColor input) { EXPECT_EQ(kTest1Color, transform.Run(input, mixer)); };
    verify_color(SK_ColorBLACK);
    verify_color(SK_ColorWHITE);
    verify_color(SK_ColorRED);
} // namespace
...
TEST(ColorTransformTest, FromTransformInput)
{
    ...
    const auto verify_color = [&](SkColor input) { EXPECT_EQ(input, transform.Run(input, ColorMixer())); };
    verify_color(SK_ColorBLACK);
    verify_color(SK_ColorWHITE);
    verify_color(SK_ColorRED);
} // namespace
...
{
    const ColorTransform transform = GetColorWithMaxContrast(FromTransformInput());
    constexpr SkColor kNewDarkestColor = gfx::kGoogleGrey500;
    const SkColor default_darkest_color = color_utils::SetDarkestColorForTesting(kNewDarkestColor);
...
{
    const ColorTransform transform = GetResultingPaintColor(FromTransformInput(), kColorTest0);
    constexpr SkColor kBackground = SK_ColorWHITE;
    ColorMixer mixer;
    mixer[kColorTest0] = { kBackground };
    EXPECT_EQ(SK_ColorBLACK, transform.Run(SK_ColorBLACK, mixer));
    EXPECT_EQ(kBackground, transform.Run(SK_ColorTRANSPARENT, mixer));
    EXPECT_EQ(color_utils::AlphaBlend(SK_ColorBLACK, kBackground, SkAlpha { 0x80 }), transform.Run(SkColorSetA(SK_ColorBLACK, 0x80), mixer));
}
...
TEST(ColorTransformTest, SelectBasedOnDarkInput)
{
    ...
    const ColorTransform transform = SelectBasedOnDarkInput(FromTransformInput(), kDarkOutput, kLightOutput);
...
TEST(ColorTransformTest, PickGoogleColor)
{
    ...
    const ColorTransform transform = PickGoogleColor(FromTransformInput(), kBackground, kMinContrast);
...
TEST(ColorTransformTest, PickGoogleColorTwoBackgrounds)
{
    ...
    const ColorTransform transform = PickGoogleColorTwoBackgrounds(FromTransformInput(), kBackgroundA, kBackgroundB, kMinContrast);
...
TEST(ColorTransformTest, HSLShift)
{
    ...
    const auto verify_color = [&](SkColor input) { EXPECT_EQ(color_utils::HSLShift(input, kHsl), transform.Run(input, ColorMixer())); };
    verify_color(SK_ColorBLACK);
