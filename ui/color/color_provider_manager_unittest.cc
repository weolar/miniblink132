...
{
    return ColorProviderManager::GetForTesting().GetColorProviderFor(ui::ColorProviderKey());
}

class TestInitializerSupplier : public ColorProviderKey::InitializerSupplier {
    void AddColorMixers(ColorProvider* provider, const ColorProviderKey& key) const override
    {
    }
};
...
TEST_F(ColorProviderManagerTest, Persistence)
{
    ...
    histogram_tester.ExpectTotalCount("Views.Browser.TimeSpentInitializingColorProvider", 1);
}
...
TEST_F(ColorProviderManagerTest, SetInitializer)
{
    ColorProviderManager::GetForTesting().AppendColorProviderInitializer(
        base::BindRepeating([](ColorProvider* provider, const ColorProviderKey&) { provider->AddMixer()[kColorTest0] = { SK_ColorBLUE }; }));
...
TEST_F(ColorProviderManagerTest, Reset)
{
    ...
    EXPECT_EQ(SK_ColorBLUE, provider->GetColor(kColorTest0));
    ColorProviderManager::ResetForTesting();
    EXPECT_EQ(gfx::kPlaceholderColor, GetLightNormalColorProvider()->GetColor(kColorTest0));
    histogram_tester.ExpectTotalCount("Views.Browser.TimeSpentInitializingColorProvider", 2);
}
