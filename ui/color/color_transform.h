...
using Callback = base::RepeatingCallback<SkColor(SkColor input, const ColorMixer& mixer)>;
...
ColorTransform BlendForMinContrast(ColorTransform foreground_transform, ColorTransform background_transform,
    std::optional<ColorTransform> high_contrast_foreground_transform = std::nullopt, float contrast_ratio = color_utils::kMinimumReadableContrastRatio);
...
COMPONENT_EXPORT(COLOR)
ColorTransform PickGoogleColor(ColorTransform foreground_transform, ColorTransform background_transform = FromTransformInput(), float min_contrast = 0.0f,
...
ColorTransform PickGoogleColorTwoBackgrounds(ColorTransform foreground_transform, ColorTransform background_a_transform, ColorTransform background_b_transform,
    float min_contrast, float max_contrast_against_nearer = color_utils::kMaximumPossibleContrast);
