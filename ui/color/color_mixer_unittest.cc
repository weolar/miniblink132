...
{
    return base::BindRepeating([](const ui::ColorMixer* mixer) { return mixer; }, mixer);
}
...
TEST(ColorMixerTest, GetInputColorRespectsRecipePreviousMixer)
{
    ...
    EXPECT_EQ(color_utils::GetColorWithMaxContrast(gfx::kPlaceholderColor), mixer1.GetInputColor(kColorTest0));
}
...
TEST(ColorMixerTest, GetResultColorChained)
{
    ...
    mixer[kColorTest1] = BlendTowardMaxContrast(GetColorWithMaxContrast(SK_ColorWHITE), 0x29);
    mixer[kColorTest2] = BlendForMinContrast(gfx::kGoogleBlue500, kColorTest1, kColorTest0);
...
TEST(ColorMixerTest, GetResultColorWithInputGetter)
{
    ...
    const auto getter = base::BindRepeating([](const ColorMixer** mixer) { return *mixer; }, &front_mixer);
...
TEST(ColorMixerTest, GetDefinedColorIdsReturnsExpectedColorIds)
{
    ColorMixer mixer;
    mixer[kColorTest0] = { gfx::kGoogleBlue050 };
    mixer[kColorTest1] = BlendTowardMaxContrast(GetColorWithMaxContrast(FromTransformInput()), 0x29);
    mixer[kColorTest2] = BlendForMinContrast(gfx::kGoogleBlue500, kColorTest1, kColorTest0);
    EXPECT_EQ(std::set<ColorId>({ kColorTest0, kColorTest1, kColorTest2 }), mixer.GetDefinedColorIds());
}
