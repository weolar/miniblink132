...
// `hues_to_rotations`.
double GetRotatedHue(double source_hue, const base::flat_map<double, double>& hues_to_rotations)
{
    if (hues_to_rotations.size() == 1) {
        return material_color_utilities::SanitizeDegreesDouble(source_hue + hues_to_rotations.begin()->second);
    }
    ...
    return material_color_utilities::SanitizeDegreesDouble(source_hue + it->second);
}
...
// returned.
double GetAdjustedChroma(double source_hue, const base::flat_map<double, double>& hues_to_chromas)
{
...
public:
    CustomPalette(
        TonalPalette&& primary, TonalPalette&& secondary, TonalPalette&& tertiary, TonalPalette&& neutral, TonalPalette&& neutral_variant, TonalPalette&& error)
        : primary_(primary)
    ...
    const TonalPalette& primary() const override
    {
        return primary_;
    }

    const TonalPalette& secondary() const override
    {
        return secondary_;
    }

    const TonalPalette& tertiary() const override
    {
        return tertiary_;
    }

    const TonalPalette& neutral() const override
    {
        return neutral_;
    }
    ...
    const TonalPalette& error() const override
    {
        return error_;
    }
...
struct Transform {
    Transform() = default;
    Transform(double hue_rotation, double chroma, std::optional<base::flat_map<double, double>> hues_to_rotations = std::nullopt)
    ...
    double chroma = 0.0;
    std::optional<base::flat_map<double, double>> hues_to_rotations = std::nullopt;
...
TonalPalette MakePalette(double hue, const Transform& transform)
{
    ...
    if (transform.hues_to_rotations) {
        ...
        hue = material_color_utilities::SanitizeDegreesDouble(hue + transform.hue_rotation);
    }
...
std::unique_ptr<Palette> FromConfig(SkColor seed_color, const Config& config)
{
    ...
    return std::make_unique<CustomPalette>(MakePalette(hue, config.primary), MakePalette(hue, config.secondary), MakePalette(hue, config.tertiary),
        MakePalette(hue, config.neutral), MakePalette(hue, config.neutral_variant), TonalPalette(25.0, 84.0));
}

// Returns a `base::flat_map` from two arrays of equal size.
template <class S, class T, size_t N> base::flat_map<S, T> Zip(const std::array<S, N>& keys, const std::array<T, N>& values)
{
...
std::unique_ptr<Palette> GeneratePalette(SkColor seed_color, SchemeVariant variant)
{
    Config config;
    switch (variant) {
    case SchemeVariant::kTonalSpot:
        config = { Chroma(40.0), Chroma(16.0), Transform { 60.0, 24.0 }, Chroma(6.0), Chroma(8.0) };
        break;
    case SchemeVariant::kVibrant: {
        const auto hues = std::to_array<double>({ 0, 41, 61, 101, 131, 181, 251, 301, 360 });
        const auto secondary_rotations = std::to_array<double>({ 18, 15, 10, 12, 15, 18, 15, 12, 12 });
        const base::flat_map<double, double> secondary_hues_to_rotations = Zip(hues, secondary_rotations);

        const auto tertiary_rotations = std::to_array<double>({ 35, 30, 20, 25, 30, 35, 30, 25, 25 });
        const base::flat_map<double, double> tertiary_hues_to_rotations = Zip(hues, tertiary_rotations);

        config
            = { Chroma(200.0), Transform(0.0, 24.0, secondary_hues_to_rotations), Transform(0.0, 32.0, tertiary_hues_to_rotations), Chroma(8.0), Chroma(12.0) };
        break;
    }
    case SchemeVariant::kNeutral: {
        ...
        config = { Transform(std::move(chroma_transforms)), Chroma(8.0), Chroma(16.0), Chroma(2.0), Chroma(2.0) };
        break;
    }
    case SchemeVariant::kExpressive: {
        const auto hues = std::to_array<double>({ 0, 21, 51, 121, 151, 191, 271, 321, 360 });
        const auto secondary_rotations = std::to_array<double>({ 45, 95, 45, 20, 45, 90, 45, 45, 45 });
        const base::flat_map<double, double> secondary_hues_to_rotations = Zip(hues, secondary_rotations);

        const auto tertiary_rotations = std::to_array<double>({ 120, 120, 20, 45, 20, 15, 20, 120, 120 });
        const base::flat_map<double, double> tertiary_hues_to_rotations = Zip(hues, tertiary_rotations);
        config = { Transform(-90, 40.0), Transform(0.0, 24.0, secondary_hues_to_rotations), Transform(0.0, 32.0, tertiary_hues_to_rotations), Chroma(8.0),
            Chroma(12.0) };
        break;
    }
    }
