...
{
    EXPECT_EQ(
        std::string("--color-primary-background"), ui::ConvertColorProviderColorIdToCSSColorId(std::string(ui::ColorIdName(ui::kColorPrimaryBackground))));
}
...
TEST_F(ColorProviderUtilsTest, ConvertSkColorToCSSColor)
{
    ...
    EXPECT_EQ(std::string("#f2990025"), ui::ConvertSkColorToCSSColor(test_color_alpha));
}
...
TEST_F(ColorProviderUtilsTest, RendererColorMapGeneratesProvidersCorrectly)
{
    ...
    // at 0.
    constexpr uint32_t kTotaltRendererColorIds = static_cast<int32_t>(color::mojom::RendererColorId::kMaxValue) + 1;
    ...
    ASSERT_LT(kTotaltRendererColorIds, static_cast<uint32_t>(ui::kUiColorsEnd - ui::kUiColorsStart - 1));
    ...
    // RendererColorIds.
    ui::RendererColorMap renderer_color_map = ui::CreateRendererColorMap(color_provider);
    ...
    auto new_color_provider = ui::CreateColorProviderFromRendererColorMap(renderer_color_map);
    new_color_provider->GenerateColorMapForTesting();
    EXPECT_EQ(kTotaltRendererColorIds, new_color_provider->color_map_for_testing().size());
}
...
TEST_F(ColorProviderUtilsTest, ColorProviderRendererColorMapEquivalence)
{
    ...
    // equivalent mappings.
    ui::RendererColorMap renderer_color_map = ui::CreateRendererColorMap(color_provider);
    EXPECT_TRUE(IsRendererColorMappingEquivalent(&color_provider, renderer_color_map));

    // Providers with different renderer color mappings should not be flagged as
    // equivalent.
    ui::ColorProvider new_color_provider;
    EXPECT_FALSE(IsRendererColorMappingEquivalent(&new_color_provider, renderer_color_map));
}

TEST_F(ColorProviderUtilsTest, DefaultBlinkColorProviderColorMapsValidity)
{
    const auto has_valid_colors = [](const ui::RendererColorMap renderer_color_map) {
    ...
    ui::RendererColorMap light_color_map = ui::GetDefaultBlinkColorProviderColorMaps(/*dark_mode=*/false,
        /*is_forced_colors=*/false);
    ui::RendererColorMap dark_color_map = ui::GetDefaultBlinkColorProviderColorMaps(/*dark_mode=*/true,
        /*is_forced_colors=*/false);
    ui::RendererColorMap forced_colors_color_map = ui::GetDefaultBlinkColorProviderColorMaps(/*dark_mode=*/false,
    ...
    ui::RendererColorMap random_color_map = ui::CreateRendererColorMap(random_color_provider);
