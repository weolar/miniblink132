...
{
    color_map_.clear();
    mixers_.emplace_after(first_postprocessing_mixer_, base::BindRepeating([](const ColorMixer* mixer) { return mixer; }, GetLastNonPostprocessingMixer()),
        base::BindRepeating(&ColorProvider::GetLastNonPostprocessingMixer, base::Unretained(this)));
...
ColorMixer& ColorProvider::AddPostprocessingMixer()
{
    ...
    if (first_postprocessing_mixer_ == mixers_.before_begin()) {
        ...
        auto previous_mixer_getter = base::BindRepeating(&ColorProvider::GetLastNonPostprocessingMixer, base::Unretained(this));
        ...
        auto previous_mixer_getter = base::BindRepeating([](const ColorMixer* mixer) { return mixer; }, base::Unretained(&mixers_.front()));
        mixers_.emplace_front(previous_mixer_getter, previous_mixer_getter);
    }
...
SkColor ColorProvider::GetColor(ColorId id) const
{
    ...
    if (i == color_map_.end()) {
        ...
        DVLOG(2) << "ColorProvider::GetColor: Computing color for ColorId: " << ColorIdName(id);
    ...
    DVLOG(2) << "ColorProvider::GetColor: ColorId: " << ColorIdName(id) << " Value: " << SkColorName(i->second);
    return i->second;
}
