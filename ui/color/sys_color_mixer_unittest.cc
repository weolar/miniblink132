...
class SysColorMixerTest : public testing::Test, public testing::WithParamInterface<std::tuple<bool, SkColor>> {
public:
    // testing::Test:
    void SetUp() override
    {
        ...
        key.color_mode = std::get<0>(GetParam()) ? ColorProviderKey::ColorMode::kDark : ColorProviderKey::ColorMode::kLight;
...
TEST_P(SysColorMixerTest, SysColorContrast)
{
    ...
    auto check_sufficient_contrast = [&](ColorId id1, ColorId id2, float expected_contrast_ratio) {
        ...
        EXPECT_GE(contrast, expected_contrast_ratio) << ColorIdName(id1) << " - " << SkColorName(color1) << "\n"
                                                     << ColorIdName(id2) << " - " << SkColorName(color2);
    };

    for (const ColorId* ids : minimum_visible_contrasting_ids) {
        check_sufficient_contrast(ids[0], ids[1], color_utils::kMinimumVisibleContrastRatio);
    }
    for (const ColorId* ids : minimum_readable_contrasting_ids) {
        check_sufficient_contrast(ids[0], ids[1], color_utils::kMinimumReadableContrastRatio);
    }
}

INSTANTIATE_TEST_SUITE_P(All, SysColorMixerTest,
    testing::Combine(testing::Bool(),
        testing::Values(gfx::kGoogleBlue500, gfx::kGoogleRed500, gfx::kGoogleGreen500, gfx::kGoogleYellow500, gfx::kGoogleGrey500, gfx::kGoogleOrange500,
            gfx::kGooglePink500, gfx::kGooglePurple500, gfx::kGoogleCyan500, SK_ColorBLACK, SK_ColorWHITE)),
    [](const testing::TestParamInfo<std::tuple<bool, SkColor>>& info) {
        return base::StringPrintf("%s_%s", std::get<0>(info.param) ? "dark" : "light", SkColorName(std::get<1>(info.param)).c_str());
    });
