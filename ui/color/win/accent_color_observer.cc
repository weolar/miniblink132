...
{
    dwm_key_ = std::make_unique<base::win::RegKey>(HKEY_CURRENT_USER, L"SOFTWARE\\Microsoft\\Windows\\DWM", KEY_READ);
    if (dwm_key_->Valid()) {
        OnDwmKeyUpdated();
    } else {
        dwm_key_.reset();
...
base::CallbackListSubscription AccentColorObserver::Subscribe(base::RepeatingClosure callback)
{
    return callbacks_.Add(std::move(callback));
}

void AccentColorObserver::SetAccentColorForTesting(std::optional<SkColor> accent_color)
{
    accent_color_ = accent_color;
    callbacks_.Notify();
}

void AccentColorObserver::SetUseDwmFrameColorForTesting(bool use_dwm_frame_color)
{
...
void AccentColorObserver::OnDwmKeyUpdated()
{
    ...
    if ((dwm_key_->ReadValueDW(L"ColorizationColor", &colorization_color) == ERROR_SUCCESS)
        && (dwm_key_->ReadValueDW(L"ColorizationColorBalance", &colorization_color_balance) == ERROR_SUCCESS)) {
        ...
        accent_border_color_ = color_utils::AlphaBlend(input_color, SkColorSetRGB(0xd9, 0xd9, 0xd9), colorization_color_balance / 100.0f);
    }
    ...
    if (dwm_key_->ReadValueDW(L"AccentColor", &accent_color) == ERROR_SUCCESS) {
        ...
        if (dwm_key_->ReadValueDW(L"AccentColorInactive", &accent_color_inactive) == ERROR_SUCCESS) {
    ...
    use_dwm_frame_color_
        = accent_color_.has_value() && (dwm_key_->ReadValueDW(L"ColorPrevalence", &color_prevalence) == ERROR_SUCCESS) && color_prevalence == 1;
    ...
    if (!base::SequencedTaskRunner::HasCurrentDefault()
        || !dwm_key_->StartWatching(base::BindOnce(&AccentColorObserver::OnDwmKeyUpdated, base::Unretained(this)))) {
        dwm_key_.reset();
    }
}
