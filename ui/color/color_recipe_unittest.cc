...
TEST(ColorRecipeTest, EmptyRecipeIsPassthrough)
{
    ...
    const auto verify_passthrough = [&](SkColor input) { EXPECT_EQ(input, recipe.GenerateResult(input, ColorMixer())); };
...
TEST(ColorRecipeTest, OneTransform)
{
    ...
    const auto verify_transform = [&](SkColor input) { EXPECT_EQ(kOutput, recipe.GenerateResult(input, ColorMixer())); };
...
{
    ColorRecipe recipe = DeriveDefaultIconColor(FromTransformInput()) + BlendForMinContrast(FromTransformInput(), kColorTest0);
    ...
    const auto verify_chain = [&](SkColor input) {
        ...
        EXPECT_GE(color_utils::GetContrastRatio(color, kBackground), color_utils::kMinimumReadableContrastRatio);
    };
