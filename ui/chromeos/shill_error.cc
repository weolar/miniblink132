...
{
    return ash::NetworkHandler::Get()->network_state_handler()->GetNetworkStateFromGuid(network_id);
}
...
std::u16string GetShillErrorString(const std::string& error, const std::string& network_id)
{
    if (error.empty())
    ...
    if (error == shill::kErrorBadPassphrase || error == shill::kErrorResultInvalidPassphrase) {
    ...
    if (error == shill::kErrorActivationFailed) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_ACTIVATION_FAILED);
    }
    ...
    if (error == shill::kErrorNeedHomeNetwork) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_NEED_HOME_NETWORK);
    }
    ...
    if (error == shill::kErrorDNSLookupFailed) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_DNS_LOOKUP_FAILED);
    }
    if (error == shill::kErrorHTTPGetFailed) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_HTTP_GET_FAILED);
    }
    if (error == shill::kErrorIpsecPskAuthFailed) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_IPSEC_PSK_AUTH_FAILED);
    }
    ...
    if (error == shill::kErrorEapAuthenticationFailed) {
        const ash::NetworkState* network = network_id.empty() ? nullptr : GetNetworkState(network_id);
        ...
        if (network && network->eap_method() == shill::kEapMethodTLS) {
            ...
            return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_EAP_AUTH_FAILED);
        }
    }
    if (error == shill::kErrorEapLocalTlsFailed) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_EAP_LOCAL_TLS_FAILED);
    }
    if (error == shill::kErrorEapRemoteTlsFailed) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_EAP_REMOTE_TLS_FAILED);
    }
    if (error == shill::kErrorPppAuthFailed) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_PPP_AUTH_FAILED);
    }
    if (error == shill::kErrorResultNotOnHomeNetwork) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_NOT_ON_HOME_NETWORK);
    }
    ...
    if (error == shill::kErrorResultWepNotSupported) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_WEP_NOT_SUPPORTED);
    }
    ...
    if (error == shill::kErrorSuspectInactiveSim) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_SUSPECT_INACTIVE_SIM);
    }
    if (error == shill::kErrorSuspectSubscriptionError) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_SUSPECT_SUBSCRIPTION_ERROR);
    }
    if (error == shill::kErrorSuspectModemDisallowed) {
        return l10n_util::GetStringUTF16(IDS_CHROMEOS_NETWORK_ERROR_SUSPECT_MODEM_DISALLOWED);
    }
    ...
    return l10n_util::GetStringFUTF16(IDS_NETWORK_UNRECOGNIZED_ERROR, base::UTF8ToUTF16(error));
}
