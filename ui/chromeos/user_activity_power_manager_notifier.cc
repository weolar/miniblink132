...
power_manager::UserActivityType GetUserActivityTypeForEvent(const Event* event)
{
...
UserActivityPowerManagerNotifier::UserActivityPowerManagerNotifier(UserActivityDetector* detector, mojo::PendingRemote<device::mojom::Fingerprint> fingerprint)
    : detector_(detector)
    , fingerprint_(std::move(fingerprint))
{
    ...
    if (fingerprint_) {
        fingerprint_->AddFingerprintObserver(fingerprint_observer_receiver_.BindNewPipeAndPassRemote());
    }
}
...
void UserActivityPowerManagerNotifier::OnStylusStateChanged(ui::StylusState state)
{
...
void UserActivityPowerManagerNotifier::OnAuthScanDone(
    const device::mojom::FingerprintMessagePtr msg, const base::flat_map<std::string, std::vector<std::string>>& matches)
{
    MaybeNotifyUserActivity(power_manager::USER_ACTIVITY_OTHER);
}
...
void UserActivityPowerManagerNotifier::OnStatusChanged(device::mojom::BiometricsManagerStatus status)
{
}

void UserActivityPowerManagerNotifier::OnEnrollScanDone(device::mojom::ScanResult scan_result, bool enroll_session_complete, int percent_complete)
{
    MaybeNotifyUserActivity(power_manager::USER_ACTIVITY_OTHER);
}

void UserActivityPowerManagerNotifier::MaybeNotifyUserActivity(power_manager::UserActivityType user_activity_type)
{
    ...
    if (suspending_ || last_notify_time_.is_null() || (now - last_notify_time_).InSeconds() >= kNotifyIntervalSec) {
        chromeos::PowerManagerClient::Get()->NotifyUserActivity(user_activity_type);
        last_notify_time_ = now;
    }
}

void UserActivityPowerManagerNotifier::SuspendImminent(power_manager::SuspendImminent::Reason reason)
{
    suspending_ = true;
}

void UserActivityPowerManagerNotifier::SuspendDone(base::TimeDelta sleep_duration)
{
    suspending_ = false;
}
