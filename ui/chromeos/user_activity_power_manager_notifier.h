...
class UI_CHROMEOS_EXPORT UserActivityPowerManagerNotifier : public InputDeviceEventObserver,
...
public:
    ...
    UserActivityPowerManagerNotifier(UserActivityDetector* detector, mojo::PendingRemote<device::mojom::Fingerprint> fingerprint);

    UserActivityPowerManagerNotifier(const UserActivityPowerManagerNotifier&) = delete;
    UserActivityPowerManagerNotifier& operator=(const UserActivityPowerManagerNotifier&) = delete;
    ...
    void OnAuthScanDone(const device::mojom::FingerprintMessagePtr msg, const base::flat_map<std::string, std::vector<std::string>>& matches) override;
    void OnSessionFailed() override;
    void OnRestarted() override;
    void OnStatusChanged(device::mojom::BiometricsManagerStatus status) override;
    void OnEnrollScanDone(device::mojom::ScanResult scan_result, bool enroll_session_complete, int percent_complete) override;
...
private:
    ...
    void MaybeNotifyUserActivity(power_manager::UserActivityType user_activity_type);

    raw_ptr<UserActivityDetector> detector_; // not owned

    mojo::Remote<device::mojom::Fingerprint> fingerprint_;
    mojo::Receiver<device::mojom::FingerprintObserver> fingerprint_observer_receiver_ { this };
