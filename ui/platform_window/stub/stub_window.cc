...
StubWindow::StubWindow(PlatformWindowDelegate* delegate, bool use_default_accelerated_widget, const gfx::Rect& bounds)
    : bounds_(bounds)
{
    DCHECK(delegate);
...
void StubWindow::InitDelegate(PlatformWindowDelegate* delegate, bool use_default_accelerated_widget)
{
    DCHECK(delegate);
    delegate_ = delegate;
...
void StubWindow::InitDelegateWithWidget(PlatformWindowDelegate* delegate, gfx::AcceleratedWidget widget)
{
    DCHECK(delegate);
    delegate_ = delegate;
...
void StubWindow::SetFullscreen(bool fullscreen, int64_t target_display_id)
{
    ...
    window_state_ = fullscreen ? ui::PlatformWindowState::kFullScreen : ui::PlatformWindowState::kUnknown;
}
...
void StubWindow::SetWindowIcons(const gfx::ImageSkia& window_icon, const gfx::ImageSkia& app_icon)
{
}
