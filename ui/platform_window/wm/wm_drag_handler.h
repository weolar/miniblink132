...
public:
    ...
    using DragFinishedCallback = base::OnceCallback<void(mojom::DragOperation operation)>;
    ...
    virtual bool StartDrag(const OSExchangeData& data, int operations, mojom::DragEventSource source, gfx::NativeCursor cursor, bool can_grab_pointer,
        base::OnceClosure drag_started_callback, DragFinishedCallback drag_finished_callback, LocationDelegate* location_delegate)
        = 0;
    ...
    virtual void UpdateDragImage(const gfx::ImageSkia& image, const gfx::Vector2d& offset) = 0;
...
COMPONENT_EXPORT(WM)
void SetWmDragHandler(PlatformWindow* platform_window, WmDragHandler* drag_handler);
COMPONENT_EXPORT(WM)
