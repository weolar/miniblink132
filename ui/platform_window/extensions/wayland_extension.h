...
public:
    ...
    virtual void StartWindowDraggingSessionIfNeeded(ui::mojom::DragEventSource event_source, bool allow_system_drag) = 0;
    ...
    // feedback.
    virtual void ShowSnapPreview(WaylandWindowSnapDirection snap, bool allow_haptic_feedback) = 0;
    ...
    virtual void CommitSnap(WaylandWindowSnapDirection snap, float snap_ratio) = 0;
    ...
    // windows.
    virtual void SetFloatToLocation(WaylandFloatStartLocation float_start_location) = 0;
...
protected:
    ...
    // Sets the pointer to the extension as a property of the PlatformWindow.
    void SetWaylandToplevelExtension(PlatformWindow* window, WaylandToplevelExtension* extension);
};

COMPONENT_EXPORT(PLATFORM_WINDOW)
WaylandToplevelExtension* GetWaylandToplevelExtension(const PlatformWindow& window);
