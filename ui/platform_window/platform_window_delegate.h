...
public:
    ...
    struct COMPONENT_EXPORT(PLATFORM_WINDOW) State {
        ...
        {
            return std::tie(window_state, bounds_dip, size_px, window_scale, raster_scale, ui_scale, occlusion_state)
                == std::tie(rhs.window_state, rhs.bounds_dip, rhs.size_px, rhs.window_scale, rhs.raster_scale, rhs.ui_scale, rhs.occlusion_state);
        }
        ...
        // Occlusion state
        PlatformWindowOcclusionState occlusion_state = PlatformWindowOcclusionState::kUnknown;
    ...
    virtual gfx::Insets CalculateInsetsInDIP(PlatformWindowState window_state) const;
    ...
    virtual void OnWindowStateChanged(PlatformWindowState old_state, PlatformWindowState new_state) = 0;
    ...
    virtual void OnOcclusionStateChanged(PlatformWindowOcclusionState occlusion_state);
    ...
    virtual void OnTooltipShownOnServer(const std::u16string& text, const gfx::Rect& bounds);
    ...
    // coordinate.
    virtual gfx::PointF ConvertScreenPointToLocalDIP(const gfx::Point& screen_in_pixels) const;

    // Converts gfx::Insets in DIP to pixels.
    virtual gfx::Insets ConvertInsetsToPixels(const gfx::Insets& insets_dip) const;
