...
bool OverscrollGlow::OnOverscrolled(base::TimeTicks current_time, gfx::Vector2dF accumulated_overscroll, gfx::Vector2dF overscroll_delta,
    gfx::Vector2dF velocity, gfx::Vector2dF overscroll_location)
{
...
bool OverscrollGlow::Animate(base::TimeTicks current_time, cc::slim::Layer* parent_layer)
{
    DCHECK(parent_layer);
...
void OverscrollGlow::OnFrameUpdated(const gfx::SizeF& viewport_size, const gfx::SizeF& content_size, const gfx::PointF& content_scroll_offset)
{
    viewport_size_ = viewport_size;
    edge_offsets_[EdgeEffect::EDGE_TOP] = -content_scroll_offset.y();
    edge_offsets_[EdgeEffect::EDGE_LEFT] = -content_scroll_offset.x();
    edge_offsets_[EdgeEffect::EDGE_BOTTOM] = content_size.height() - content_scroll_offset.y() - viewport_size.height();
    edge_offsets_[EdgeEffect::EDGE_RIGHT] = content_size.width() - content_scroll_offset.x() - viewport_size.width();
...
void OverscrollGlow::Pull(base::TimeTicks current_time, const gfx::Vector2dF& overscroll_delta, const gfx::Vector2dF& overscroll_location)
{
    DCHECK(initialized_);
    ...
    for (size_t i = 0; i < EDGE_COUNT; ++i) {
        ...
        edge_effects_[i]->Pull(current_time, std::abs(edge_pull[i]), edge_displacement[i]);
        GetOppositeEdge(i)->Release(current_time);
    }
}

void OverscrollGlow::Absorb(base::TimeTicks current_time, const gfx::Vector2dF& velocity, bool x_overscroll_started, bool y_overscroll_started)
{
    DCHECK(initialized_);
