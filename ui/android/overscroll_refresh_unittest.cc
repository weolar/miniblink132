...
const float kDefaultEdgeWidth = OverscrollRefresh::kDefaultNavigationEdgeWidth * kDipScale;

class OverscrollRefreshTest : public OverscrollRefreshHandler, public testing::Test {
public:
    ...
    bool PullStart(OverscrollAction type, std::optional<BackGestureEventSwipeEdge> initiating_edge) override
    {
        started_ = true;
        return true;
    }
    ...
    void TestOverscrollBehavior(const cc::OverscrollBehavior& ob, const gfx::Vector2dF& scroll_delta, bool started)
    {
...
TEST_F(OverscrollRefreshTest, NotTriggeredIfInitialYOffsetIsNotZero)
{
    ...
    effect.OnFrameUpdated(viewport, nonzero_offset, content_size, overflow_y_hidden);
    effect.OnScrollBegin(kStartPos);

    effect.OnFrameUpdated(viewport, gfx::PointF(), content_size, overflow_y_hidden);
