...
public:
    ...
    // for unittests.
    static std::unique_ptr<FakeModalDialogManagerBridge> CreateForTab(WindowAndroid* window, bool use_empty_java_presenter);

    FakeModalDialogManagerBridge(const FakeModalDialogManagerBridge&) = delete;
    FakeModalDialogManagerBridge& operator=(const FakeModalDialogManagerBridge&) = delete;
...
private:
    FakeModalDialogManagerBridge(base::android::ScopedJavaLocalRef<jobject> j_fake_manager_, WindowAndroid* window);
