...
ViewAndroid::ScopedAnchorView::ScopedAnchorView(JNIEnv* env, const JavaRef<jobject>& jview, const JavaRef<jobject>& jdelegate)
    : view_(env, jview)
    , delegate_(env, jdelegate)
{
...
ViewAndroid::ScopedAnchorView& ViewAndroid::ScopedAnchorView::operator=(ScopedAnchorView&& other)
{
    if (this != &other) {
        view_ = other.view_;
        other.view_.reset();
...
const base::android::ScopedJavaLocalRef<jobject> ViewAndroid::ScopedAnchorView::view() const
{
    JNIEnv* env = base::android::AttachCurrentThread();
...
ViewAndroid::ViewAndroid()
    : ViewAndroid(LayoutType::kNormal)
{
}
...
{
    if (!event_forwarder_) {
        DCHECK(!RootPathHasEventForwarder(parent_)) << "The view tree path already has an event forwarder.";
        DCHECK(!SubtreeHasEventForwarder(this)) << "The view tree path already has an event forwarder.";
...
void ViewAndroid::AddChild(ViewAndroid* child)
{
    ...
    DCHECK(!RootPathHasEventForwarder(this) || !SubtreeHasEventForwarder(child)) << "Some view tree path will have more than one event forwarder "
    ...
    if (child->match_parent() && !bounds_.IsEmpty() && child->GetSize() != bounds_.size()) {
...
ViewAndroid::ScopedAnchorView ViewAndroid::AcquireAnchorView()
{
    ...
    JNIEnv* env = base::android::AttachCurrentThread();
    return ViewAndroid::ScopedAnchorView(env, Java_ViewAndroidDelegate_acquireView(env, delegate), delegate);
}

void ViewAndroid::SetAnchorRect(const JavaRef<jobject>& anchor, const gfx::RectF& bounds_dip)
{
    ScopedJavaLocalRef<jobject> delegate(GetViewAndroidDelegate());
    if (delegate.is_null())
        return;
    ...
    Java_ViewAndroidDelegate_setViewPosition(
        env, delegate, anchor, bounds_px.x(), bounds_px.y(), bounds_px.width(), bounds_px.height(), left_margin, top_margin);
}
...
gfx::Point ViewAndroid::GetLocationOfContainerViewInWindow()
{
    ...
    JNIEnv* env = base::android::AttachCurrentThread();
    gfx::Point result(Java_ViewAndroidDelegate_getXLocationOfContainerViewInWindow(env, delegate),
        Java_ViewAndroidDelegate_getYLocationOfContainerViewInWindow(env, delegate));
...
void ViewAndroid::RemoveChild(ViewAndroid* child)
{
    ...
    std::list<raw_ptr<ViewAndroid, CtnExperimental>>::iterator it = base::ranges::find(children_, child);
    CHECK(it != children_.end(), base::NotFatalUntil::M130);
    children_.erase(it);
...
void ViewAndroid::RequestUnbufferedDispatch(const MotionEventAndroid& event)
{
    ...
    if (delegate.is_null())
        return;
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_ViewAndroidDelegate_requestUnbufferedDispatch(env, delegate, event.GetJavaObject());
}
...
std::unique_ptr<viz::CopyOutputRequest> ViewAndroid::MaybeRequestCopyOfView(std::unique_ptr<viz::CopyOutputRequest> request)
{
...
const ScopedJavaLocalRef<jobject> ViewAndroid::GetViewAndroidDelegate() const
{
    JNIEnv* env = base::android::AttachCurrentThread();
    const ScopedJavaLocalRef<jobject> delegate = delegate_.get(env);
    if (!delegate.is_null())
...
bool ViewAndroid::StartDragAndDrop(const JavaRef<jobject>& jshadow_image, const JavaRef<jobject>& jdrop_data, jint cursor_offset_x, jint cursor_offset_y,
    jint drag_obj_rect_width, jint drag_obj_rect_height)
{
    ScopedJavaLocalRef<jobject> delegate(GetViewAndroidDelegate());
    if (delegate.is_null())
        return false;
    ...
    JNIEnv* env = base::android::AttachCurrentThread();
    return Java_ViewAndroidDelegate_startDragAndDrop(env, delegate, jshadow_image, jdrop_data, window_android ? window_android->GetJavaObject() : nullptr,
        cursor_offset_x, cursor_offset_y, drag_obj_rect_width, drag_obj_rect_height);
}
...
void ViewAndroid::OnCursorChanged(const Cursor& cursor)
{
    ...
    if (cursor.type() == mojom::CursorType::kCustom) {
        ...
        if (bitmap.drawsNothing()) {
            Java_ViewAndroidDelegate_onCursorChanged(env, delegate, static_cast<int>(mojom::CursorType::kPointer));
            return;
        }
        ScopedJavaLocalRef<jobject> java_bitmap = gfx::ConvertToJavaBitmap(bitmap);
        Java_ViewAndroidDelegate_onCursorChangedToCustom(env, delegate, java_bitmap, hotspot.x(), hotspot.y());
    } else {
        Java_ViewAndroidDelegate_onCursorChanged(env, delegate, static_cast<int>(cursor.type()));
    }
}
...
void ViewAndroid::NotifyHoverActionStylusWritable(bool stylus_writable)
{
    ...
    if (delegate.is_null())
        return;
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_ViewAndroidDelegate_notifyHoverActionStylusWritable(env, delegate, stylus_writable);
}
...
void ViewAndroid::OnControlsChanged(float top_controls_offset, float top_content_offset, float top_controls_min_height_offset, float bottom_controls_offset,
    float bottom_controls_min_height_offset)
{
    ...
    if (delegate.is_null())
        return;
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_ViewAndroidDelegate_onControlsChanged(env, delegate, std::round(top_controls_offset), std::round(top_content_offset),
        std::round(top_controls_min_height_offset), std::round(bottom_controls_offset), std::round(bottom_controls_min_height_offset));
}
...
void ViewAndroid::OnVerticalScrollDirectionChanged(bool direction_up, float current_scroll_ratio)
{
    ScopedJavaLocalRef<jobject> delegate(GetViewAndroidDelegate());
    if (delegate.is_null())
        return;
    JNIEnv* env = base::android::AttachCurrentThread();
    Java_ViewAndroidDelegate_onVerticalScrollDirectionChanged(env, delegate, direction_up, current_scroll_ratio);
}
...
void ViewAndroid::OnPhysicalBackingSizeChanged(const gfx::Size& size, std::optional<base::TimeDelta> deadline_override)
{
...
{
    return HitTest(base::BindRepeating(&ViewAndroid::SendDragEventToHandler), event, event.location());
}

// static
bool ViewAndroid::SendDragEventToHandler(EventHandlerAndroid* handler, const DragEventAndroid& event)
{
...
{
    return HitTest(base::BindRepeating(&ViewAndroid::SendTouchEventToHandler), event, event.GetPoint());
}

// static
bool ViewAndroid::SendTouchEventToHandler(EventHandlerAndroid* handler, const MotionEventAndroid& event)
{
...
{
    return HitTest(base::BindRepeating(&ViewAndroid::SendMouseEventToHandler), event, event.GetPoint());
}

// static
bool ViewAndroid::SendMouseEventToHandler(EventHandlerAndroid* handler, const MotionEventAndroid& event)
{
...
{
    return HitTest(base::BindRepeating(&ViewAndroid::SendMouseWheelEventToHandler), event, event.GetPoint());
}

// static
bool ViewAndroid::SendMouseWheelEventToHandler(EventHandlerAndroid* handler, const MotionEventAndroid& event)
{
...
{
    return HitTest(base::BindRepeating(&ViewAndroid::SendGestureEventToHandler), event, event.location());
}

// static
bool ViewAndroid::SendGestureEventToHandler(EventHandlerAndroid* handler, const GestureEventAndroid& event)
{
...
void ViewAndroid::NotifyVirtualKeyboardOverlayRect(const gfx::Rect& keyboard_rect)
{
    if (event_handler_)
...
template <typename E> bool ViewAndroid::HitTest(EventHandlerCallback<E> handler_callback, const E& event, const gfx::PointF& point)
{
    if (event_handler_) {
