...
UI_ANDROID_EXPORT constexpr int64_t kInvalidJavaColor = static_cast<int64_t>(std::numeric_limits<int32_t>::max()) + 1;

// Converts |color| to a CSS color string. If |color| is null, the empty string
// is returned.
UI_ANDROID_EXPORT std::string OptionalSkColorToString(const std::optional<SkColor>& color);
...
UI_ANDROID_EXPORT int64_t OptionalSkColorToJavaColor(const std::optional<SkColor>& skcolor);
UI_ANDROID_EXPORT std::optional<SkColor> JavaColorToOptionalSkColor(int64_t java_color);
