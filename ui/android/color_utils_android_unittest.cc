...
constexpr int64_t kAndroidInvalidColor = static_cast<int64_t>(std::numeric_limits<int32_t>::max()) + 1;
...
TEST(ColorHelpersTest, Roundtrip)
{
    ...
    EXPECT_EQ(kAndroidTransparent, OptionalSkColorToJavaColor(SK_ColorTRANSPARENT));
    EXPECT_EQ(SK_ColorTRANSPARENT, JavaColorToOptionalSkColor(kAndroidTransparent));
}
