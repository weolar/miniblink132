...
scoped_refptr<cc::slim::SurfaceLayer> CreateSurfaceLayer(const viz::SurfaceId& primary_surface_id, const viz::SurfaceId& fallback_surface_id,
    const gfx::Size& size_in_pixels, const cc::DeadlinePolicy& deadline_policy, bool surface_opaque)
{
...
blink::mojom::RecordContentToVisibleTimeRequestPtr ConsumeAndMergeRequests(
    blink::mojom::RecordContentToVisibleTimeRequestPtr request1, blink::mojom::RecordContentToVisibleTimeRequestPtr request2)
{
...
DelegatedFrameHostAndroid::DelegatedFrameHostAndroid(
    ui::ViewAndroid* view, viz::HostFrameSinkManager* host_frame_sink_manager, Client* client, const viz::FrameSinkId& frame_sink_id)
    ...
    , frame_evictor_(std::make_unique<viz::FrameEvictor>(this))
{
    ...
    content_layer_ = CreateSurfaceLayer(viz::SurfaceId(), viz::SurfaceId(), gfx::Size(), cc::DeadlinePolicy::UseDefaultDeadline(), is_transparent);
...
void DelegatedFrameHostAndroid::SetIsFrameSinkIdOwner(bool is_owner)
{
    ...
    if (owns_frame_sink_id_) {
        host_frame_sink_manager_->RegisterFrameSinkId(frame_sink_id_, this, viz::ReportFirstSurfaceActivation::kNo);
        host_frame_sink_manager_->SetFrameSinkDebugLabel(frame_sink_id_, "DelegatedFrameHostAndroid");
    }
}

void DelegatedFrameHostAndroid::RegisterOffsetTags(const cc::BrowserControlsOffsetTagsInfo& tags_info)
{
    const viz::OffsetTag top_controls_offset_tag = tags_info.top_controls_offset_tag;
    ...
    if (!top_controls_offset_tag.IsEmpty()) {
        ...
        viz::OffsetTagConstraints top_controls_constraints(0, 0, -(top_controls_height + top_controls_hairline_height), 0);
        content_layer_->RegisterOffsetTag(top_controls_offset_tag, top_controls_constraints);
    }

    if (!content_offset_tag.IsEmpty()) {
        int top_controls_height = tags_info.top_controls_height;
        viz::OffsetTagConstraints content_constraints(0, 0, -top_controls_height, 0);
        content_layer_->RegisterOffsetTag(content_offset_tag, content_constraints);
    }
}

void DelegatedFrameHostAndroid::UnregisterOffsetTags(const cc::BrowserControlsOffsetTagsInfo& tags_info)
{
    const viz::OffsetTag top_controls_offset_tag = tags_info.top_controls_offset_tag;
    if (!top_controls_offset_tag.IsEmpty()) {
...
void DelegatedFrameHostAndroid::CopyFromCompositingSurface(
    const gfx::Rect& src_subrect, const gfx::Size& output_size, base::OnceCallback<void(const SkBitmap&)> callback, bool capture_exact_surface_id)
{
    DCHECK(CanCopyFromCompositingSurface());

    const viz::SurfaceId surface_id(frame_sink_id_, local_surface_id_);

    ui::WindowAndroidCompositor::ScopedKeepSurfaceAliveCallback keep_surface_alive;
    if (view_->GetWindowAndroid() && view_->GetWindowAndroid()->GetCompositor()) {
        keep_surface_alive = view_->GetWindowAndroid()->GetCompositor()->TakeScopedKeepSurfaceAliveCallback(surface_id);
    }
    ...
    std::unique_ptr<viz::CopyOutputRequest> request
        = std::make_unique<viz::CopyOutputRequest>(viz::CopyOutputRequest::ResultFormat::RGBA, viz::CopyOutputRequest::ResultDestination::kSystemMemory,
            base::BindOnce(
                [](base::OnceCallback<void(const SkBitmap&)> copy_result, ui::WindowAndroidCompositor::ScopedKeepSurfaceAliveCallback keep_alive,
    ...
    viz::SetCopyOutoutRequestResultSize(request.get(), src_subrect, output_size, surface_size_in_pixels_);

    host_frame_sink_manager_->RequestCopyOfOutput(surface_id, std::move(request), capture_exact_surface_id);
}
...
void DelegatedFrameHostAndroid::EvictDelegatedFrame(const std::vector<viz::SurfaceId>& surface_ids)
{
    content_layer_->SetSurfaceId(viz::SurfaceId(), cc::DeadlinePolicy::UseDefaultDeadline());
    ...
    if (!pre_navigation_local_surface_id_.is_valid() && (!HasSavedFrame() || frame_evictor_->visible())) {
        return;
    }

    UMA_HISTOGRAM_COUNTS_100("MemoryAndroid.EvictedTreeSize2", surface_ids.size());
...
viz::FrameEvictorClient::EvictIds DelegatedFrameHostAndroid::CollectSurfaceIdsForEviction() const
{
...
viz::SurfaceId DelegatedFrameHostAndroid::GetFallbackSurfaceIdForTesting() const
{
    return content_layer_->oldest_acceptable_fallback().value_or(viz::SurfaceId());
}

viz::SurfaceId DelegatedFrameHostAndroid::GetCurrentSurfaceIdForTesting() const
{
    return GetCurrentSurfaceId();
}

viz::SurfaceId DelegatedFrameHostAndroid::GetFirstSurfaceIdAfterNavigationForTesting() const
{
    return viz::SurfaceId(frame_sink_id_, first_local_surface_id_after_navigation_);
}

void DelegatedFrameHostAndroid::ClearFallbackSurfaceForCommitPending()
{
    const std::optional<viz::SurfaceId> fallback_surface_id = content_layer_->oldest_acceptable_fallback();
...
void DelegatedFrameHostAndroid::ResetFallbackToFirstNavigationSurface()
{
    ...
    // navigation.
    if (content_layer_->oldest_acceptable_fallback() && content_layer_->oldest_acceptable_fallback()->frame_sink_id() == frame_sink_id_
        && content_layer_->oldest_acceptable_fallback()->local_surface_id().IsSameOrNewerThan(first_local_surface_id_after_navigation_)) {
        return;
    }

    // If we have a surface from before a navigation, evict it as well.
    if (pre_navigation_local_surface_id_.is_valid() && !first_local_surface_id_after_navigation_.is_valid()) {
        ...
        // BFCache.
        {
            ...
            // fixed.
            SCOPED_CRASH_KEY_STRING64("crbug/349073060", "bfc_fallback_crashed", bfcache_fallback_.ToString().c_str());
            SCOPED_CRASH_KEY_STRING64("crbug/349073060", "pre_nav_lsid_crashed", pre_navigation_local_surface_id_.ToString().c_str());
    ...
    content_layer_->SetOldestAcceptableFallback(viz::SurfaceId(frame_sink_id_, first_local_surface_id_after_navigation_));
}
...
void DelegatedFrameHostAndroid::AttachToCompositor(WindowAndroidCompositor* compositor)
{
    if (registered_parent_compositor_)
        DetachFromCompositor();
    ...
    if (content_to_visible_time_request_) {
        registered_parent_compositor_->PostRequestSuccessfulPresentationTimeForNextFrame(content_to_visible_time_recorder_.TabWasShown(
            /*has_saved_frames=*/true, std::move(content_to_visible_time_request_)));
    }
}
...
void DelegatedFrameHostAndroid::WasShown(const viz::LocalSurfaceId& new_local_surface_id, const gfx::Size& new_size_in_pixels, bool is_fullscreen,
    blink::mojom::RecordContentToVisibleTimeRequestPtr content_to_visible_time_request)
{
    if (content_to_visible_time_request) {
        PostRequestSuccessfulPresentationTimeForNextFrame(std::move(content_to_visible_time_request));
    }
    frame_evictor_->SetVisible(true);

    EmbedSurface(new_local_surface_id, new_size_in_pixels, cc::DeadlinePolicy::UseSpecifiedDeadline(FirstFrameTimeoutFrames()), is_fullscreen);
}

void DelegatedFrameHostAndroid::EmbedSurface(
    const viz::LocalSurfaceId& new_local_surface_id, const gfx::Size& new_size_in_pixels, cc::DeadlinePolicy deadline_policy, bool is_fullscreen)
{
    TRACE_EVENT2(
        "viz", "DelegatedFrameHostAndroid::EmbedSurface", "surface_id", new_local_surface_id.ToString(), "deadline_policy", deadline_policy.ToString());
    ...
    bool has_fallback_surface = (content_layer_->oldest_acceptable_fallback() && content_layer_->oldest_acceptable_fallback()->is_valid());
    ...
    if (!frame_evictor_->visible() || is_fullscreen) {
        ...
        // crbug.com/1218238.
        if (!content_layer_->bounds().IsEmpty() && surface_size_in_pixels_ != content_layer_->bounds()
            ...
            content_layer_->SetBackgroundColor(is_fullscreen ? SkColors::kBlack : SkColors::kTransparent);

            // Invalidates `bfcache_fallback_`, resize-while-hidden has given us the
            // latest `local_surface_id_`.
            bfcache_fallback_ = viz::ParentLocalSurfaceIdAllocator::InvalidLocalSurfaceId();
        }
    }
    ...
    if (bfcache_fallback_.is_valid()) {
        ...
        content_layer_->SetOldestAcceptableFallback(viz::SurfaceId(frame_sink_id_, bfcache_fallback_));
        bfcache_fallback_ = viz::ParentLocalSurfaceIdAllocator::InvalidLocalSurfaceId();
    }

    if (!current_primary_surface_id.is_valid() || current_primary_surface_id.local_surface_id() != local_surface_id_) {
        if (base::android::BuildInfo::GetInstance()->sdk_int() < base::android::SDK_VERSION_OREO) {
            ...
            // infinite.
            if (deadline_policy.policy_type() != cc::DeadlinePolicy::kUseInfiniteDeadline
                && (content_layer_->bounds().IsEmpty() || content_layer_->bounds() != surface_size_in_pixels_)) {
                deadline_policy = cc::DeadlinePolicy::UseSpecifiedDeadline(0u);
            }
        }
...
void DelegatedFrameHostAndroid::RequestSuccessfulPresentationTimeForNextFrame(
    blink::mojom::RecordContentToVisibleTimeRequestPtr content_to_content_to_visible_time_request)
{
    PostRequestSuccessfulPresentationTimeForNextFrame(std::move(content_to_content_to_visible_time_request));
}
...
void DelegatedFrameHostAndroid::OnFirstSurfaceActivation(const viz::SurfaceInfo& surface_info)
{
    NOTREACHED();
}

void DelegatedFrameHostAndroid::OnFrameTokenChanged(uint32_t frame_token, base::TimeTicks activation_time)
{
...
void DelegatedFrameHostAndroid::SetLocalSurfaceId(const viz::LocalSurfaceId& local_surface_id)
{
...
{
    return content_layer_->oldest_acceptable_fallback() && content_layer_->oldest_acceptable_fallback()->is_valid();
}

void DelegatedFrameHostAndroid::TakeFallbackContentFrom(DelegatedFrameHostAndroid* other)
{
    ...
    content_layer_->SetOldestAcceptableFallback(other->content_layer_->surface_id().ToSmallestId());
}
...
void DelegatedFrameHostAndroid::PostRequestSuccessfulPresentationTimeForNextFrame(
    blink::mojom::RecordContentToVisibleTimeRequestPtr content_to_visible_time_request)
{
    // Since we could receive multiple requests while awaiting
    // `registered_parent_compositor_` we merge them.
    auto request = ConsumeAndMergeRequests(std::move(content_to_visible_time_request_), std::move(content_to_visible_time_request));
