...
OverscrollRefresh::OverscrollRefresh(OverscrollRefreshHandler* handler, float edge_width)
...
void OverscrollRefresh::OnOverscrolled(const cc::OverscrollBehavior& behavior)
{
    ...
    if (in_y_direction) {
    ...
    } else if (in_x_direction && (scroll_begin_x_ < edge_width_ || viewport_width_ - scroll_begin_x_ < edge_width_)) {
        ...
        overscroll_edge = xdelta < 0 ? BackGestureEventSwipeEdge::RIGHT : BackGestureEventSwipeEdge::LEFT;
    }

    CHECK_EQ(overscroll_edge.has_value(), type == OverscrollAction::HISTORY_NAVIGATION);

    if (type != OverscrollAction::NONE) {
        scroll_consumption_state_ = handler_->PullStart(type, overscroll_edge) ? ENABLED : DISABLED;
    }
}

bool OverscrollRefresh::WillHandleScrollUpdate(const gfx::Vector2dF& scroll_delta)
{
    ...
    case AWAITING_SCROLL_UPDATE_ACK:
        if (std::abs(scroll_delta.y()) > std::abs(scroll_delta.x())) {
            ...
            bool is_pull_from_bottom_edge = scroll_delta.y() < 0 && bottom_at_scroll_start_ && !top_at_scroll_start_;

            // If the activation shouldn't have happened, stop here.
            if (overflow_y_hidden_ || (!is_pull_to_refresh && !is_pull_from_bottom_edge)) {
                scroll_consumption_state_ = DISABLED;
                return false;
            }
        }
...
void OverscrollRefresh::OnFrameUpdated(
    const gfx::SizeF& viewport_size, const gfx::PointF& content_scroll_offset, const gfx::SizeF& content_size, bool root_overflow_y_hidden)
{
    ...
    if (base::FeatureList::IsEnabled(kReportBottomOverscrolls)) {
        scrolled_to_bottom_ = content_size.height() <= content_scroll_offset.y() + viewport_size.height();
    }
