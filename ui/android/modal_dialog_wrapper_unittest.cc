...
TEST(ModalDialogWrapperTest, ShowTabModal)
{
    ...
    dialog_builder.SetTitle(u"title")
        ...
        .AddOkButton(base::BindLambdaForTesting([&ok_called]() { ok_called = true; }), ui::DialogModel::Button::Params().SetLabel(u"ok"))
        .AddCancelButton(base::DoNothing(), ui::DialogModel::Button::Params().SetLabel(u"cancel"))
        .SetCloseActionCallback(base::DoNothing());

    auto window = ui::WindowAndroid::CreateForTesting();
    auto fake_dialog_manager = FakeModalDialogManagerBridge::CreateForTab(window->get(), /*use_empty_java_presenter=*/false);
...
TEST(ModalDialogWrapperTest, CloseDialogFromNative)
{
    ...
    dialog_builder.SetTitle(u"title")
        .AddParagraph(ui::DialogModelLabel(u"paragraph"))
        .AddOkButton(base::DoNothing(), ui::DialogModel::Button::Params().SetLabel(u"ok"))
        .AddCancelButton(base::DoNothing(), ui::DialogModel::Button::Params().SetLabel(u"cancel"))
        .SetCloseActionCallback(base::BindLambdaForTesting([&closed]() { closed = true; }));
