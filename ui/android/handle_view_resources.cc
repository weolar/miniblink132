...
static SkBitmap CreateSkBitmapFromJavaBitmap(base::android::ScopedJavaLocalRef<jobject> jbitmap)
{
    return jbitmap.is_null() ? SkBitmap() : CreateSkBitmapFromJavaBitmap(gfx::JavaBitmap(jbitmap));
}
...
void HandleViewResources::LoadIfNecessary(const JavaRef<jobject>& context)
{
    ...
    left_bitmap_ = CreateSkBitmapFromJavaBitmap(Java_HandleViewResources_getLeftHandleBitmap(env, context));
    right_bitmap_ = CreateSkBitmapFromJavaBitmap(Java_HandleViewResources_getRightHandleBitmap(env, context));
    center_bitmap_ = CreateSkBitmapFromJavaBitmap(Java_HandleViewResources_getCenterHandleBitmap(env, context));
...
const SkBitmap& HandleViewResources::GetBitmap(ui::TouchHandleOrientation orientation)
{
    DCHECK(loaded_);
    switch (orientation) {
