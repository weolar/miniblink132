...
public:
    ...
    virtual ScopedKeepSurfaceAliveCallback TakeScopedKeepSurfaceAliveCallback(const viz::SurfaceId& surface_id) = 0;
    virtual void RequestCopyOfOutputOnRootLayer(std::unique_ptr<viz::CopyOutputRequest> request) = 0;
    ...
    virtual void OnUpdateSupportedRefreshRates(const std::vector<float>& supported_refresh_rates) = 0;
    virtual std::unique_ptr<ui::CompositorLock> GetCompositorLock(base::TimeDelta timeout) = 0;
    ...
    using SuccessfulPresentationTimeCallback = base::OnceCallback<void(const viz::FrameTimingDetails&)>;
    virtual void PostRequestSuccessfulPresentationTimeForNextFrame(SuccessfulPresentationTimeCallback callback) = 0;
    virtual void AddFrameSubmissionObserver(FrameSubmissionObserver* observer) = 0;
    virtual void RemoveFrameSubmissionObserver(FrameSubmissionObserver* observer) = 0;
};
