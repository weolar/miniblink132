...
public:
    using CopyViewCallback = base::RepeatingCallback<void(std::unique_ptr<viz::CopyOutputRequest>)>;
    ...
    public:
        ScopedAnchorView(JNIEnv* env, const base::android::JavaRef<jobject>& jview, const base::android::JavaRef<jobject>& jdelegate);
    ...
    {
        return frame_info_.content_offset;
    }
    gfx::SizeF viewport_size() const
    {
        return frame_info_.viewport_size;
    }
    ...
    bool StartDragAndDrop(const base::android::JavaRef<jobject>& jshadow_image, const base::android::JavaRef<jobject>& jdrop_data, jint cursor_offset_x,
        jint cursor_offset_y, jint drag_obj_rect_width, jint drag_obj_rect_height);
    ...
    void OnPhysicalBackingSizeChanged(const gfx::Size& size, std::optional<base::TimeDelta> deadline_override = std::nullopt);
    ...
    void OnControlsChanged(float top_controls_offset, float top_content_offset, float top_controls_min_height_offset, float bottom_controls_offset,
    ...
    void OnVerticalScrollDirectionChanged(bool direction_up, float current_scroll_ratio);
    ...
    std::unique_ptr<viz::CopyOutputRequest> MaybeRequestCopyOfView(std::unique_ptr<viz::CopyOutputRequest> request);
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(ViewAndroidBoundsTest, MatchesViewSizeOfkMatchParent);
    ...
    template <typename E> using EventHandlerCallback = const base::RepeatingCallback<bool(EventHandlerAndroid*, const E&)>;
    template <typename E> bool HitTest(EventHandlerCallback<E> handler_callback, const E& event, const gfx::PointF& point);

    static bool SendDragEventToHandler(EventHandlerAndroid* handler, const DragEventAndroid& event);
    static bool SendTouchEventToHandler(EventHandlerAndroid* handler, const MotionEventAndroid& event);
    static bool SendMouseEventToHandler(EventHandlerAndroid* handler, const MotionEventAndroid& event);
    static bool SendMouseWheelEventToHandler(EventHandlerAndroid* handler, const MotionEventAndroid& event);
    static bool SendGestureEventToHandler(EventHandlerAndroid* handler, const GestureEventAndroid& event);

    bool has_event_forwarder() const
    {
        return !!event_forwarder_;
    }

    bool match_parent() const
    {
        return layout_type_ == LayoutType::kMatchParent;
    }
    ...
    const base::android::ScopedJavaLocalRef<jobject> GetViewAndroidDelegate() const;
