...
gfx::Transform ComputeTransform(EdgeEffect::Edge edge, const gfx::SizeF& viewport_size, float offset)
{
    ...
    case EdgeEffect::EDGE_LEFT:
        return gfx::Transform::MakeTranslation(-viewport_size.height() / 2.f + offset, viewport_size.height() / 2.f) * gfx::Transform::Make270degRotation();
    case EdgeEffect::EDGE_BOTTOM:
        return gfx::Transform::MakeTranslation(0, viewport_size.height() + offset) * gfx::Transform::Make180degRotation();
    case EdgeEffect::EDGE_RIGHT:
        return gfx::Transform::MakeTranslation(-viewport_size.height() / 2.f + viewport_size.width() + offset, viewport_size.height() / 2.f)
...
gfx::SizeF ComputeOrientedSize(EdgeEffect::Edge edge, const gfx::SizeF& viewport_size)
{
    switch (edge) {
    case EdgeEffect::EDGE_TOP:
    case EdgeEffect::EDGE_BOTTOM:
...
void EdgeEffect::Pull(base::TimeTicks current_time, float delta_distance, float displacement)
{
    ...
    glow_alpha_ = glow_alpha_start_ = std::min(kMaxAlpha, glow_alpha_ + (abs_delta_distance * kPullDistanceAlphaGlowFactor));
...
void EdgeEffect::Absorb(base::TimeTicks current_time, float velocity)
{
    ...
    glow_alpha_finish_ = Clamp(glow_alpha_start_, velocity * kVelocityGlowFactor * .00001f, kMaxAlpha);
...
void EdgeEffect::ApplyToLayers(Edge edge, const gfx::SizeF& viewport_size, float offset)
{
    if (IsFinished())
        return;
    ...
    gfx::Size image_bounds(r, std::min(1.f, glow_scale_y_) * base_glow_scale * bounds_.height());
    ...
    gfx::PointF uv_top_left((clipped_top_left.x() - image_rect.x()) / image_rect.width(), (clipped_top_left.y() - image_rect.y()) / image_rect.height());
    gfx::PointF uv_bottom_right(
        (clipped_bottom_right.x() - image_rect.x()) / image_rect.width(), (clipped_bottom_right.y() - image_rect.y()) / image_rect.height());
    ...
    glow_->SetUIResourceId(resource_manager_->GetUIResourceId(ui::ANDROID_RESOURCE_TYPE_SYSTEM, kResourceId));
