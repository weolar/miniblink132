...
public:
    ...
    void SetResourceAsLoaded(AndroidResourceType res_type, int res_id)
    {
        ...
        OnResourceReady(nullptr, nullptr, res_type, res_id, gfx::ConvertToJavaBitmap(small_bitmap), 1, 1, reinterpret_cast<intptr_t>(new Resource()));
    }

protected:
    void PreloadResourceFromJava(AndroidResourceType res_type, int res_id) override
    {
    }

    void RequestResourceFromJava(AndroidResourceType res_type, int res_id) override
    {
...
public:
    ...
    {
        return resource_manager_.GetUIResourceId(ui::ANDROID_RESOURCE_TYPE_SYSTEM, type);
    }

    void SetResourceAsLoaded(ui::SystemUIResourceType type)
    {
        resource_manager_.SetResourceAsLoaded(ui::ANDROID_RESOURCE_TYPE_SYSTEM, type);
    }
...
TEST_F(ResourceManagerTest, GetResource)
{
    ...
    EXPECT_CALL(ui_resource_manager_, CreateUIResource(_)).WillOnce(Return(kResourceId)).RetiresOnSaturation();
...
TEST_F(ResourceManagerTest, TestOnMemoryDumpEmitsData)
{
    ...
    std::unique_ptr<base::trace_event::ProcessMemoryDump> process_memory_dump = std::make_unique<base::trace_event::ProcessMemoryDump>(dump_args);
    ...
    const char* system_allocator_pool_name = base::trace_event::MemoryDumpManager::GetInstance()->system_allocator_pool_name();
    size_t kExpectedDumpCount = 10;
    EXPECT_EQ(kExpectedDumpCount, allocator_dumps.size());
    for (const auto& dump : allocator_dumps) {
        ASSERT_TRUE(dump.first.find("ui/resource_manager") == 0 || dump.first.find(system_allocator_pool_name) == 0);
    }
}
