...
gfx::Size NinePatchResource::DrawSize(const gfx::Size& content_size) const
{
    ...
    // side.
    return gfx::Size(content_size.width() + size().width() - padding_.width(), content_size.height() + size().height() - padding_.height());
}

gfx::PointF NinePatchResource::DrawPosition(const gfx::Point& content_position) const
{
    // Offset the location of the layer by the amount taken by the left and top
    // margin.
    return gfx::PointF(content_position.x() - padding_.x(), content_position.y() - padding_.y());
}
...
gfx::Rect NinePatchResource::Border(const gfx::Size& bounds, const gfx::InsetsF& scale) const
{
    ...
    return gfx::Rect(aperture_.x() * left_scale, aperture_.y() * top_scale, (size().width() - aperture_.width()) * right_scale,
