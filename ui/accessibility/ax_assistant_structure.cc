...
bool IsRichTextEditable(const AXNode* node)
{
    ...
    return node->HasState(ax::mojom::State::kRichlyEditable) && (!parent || !parent->HasState(ax::mojom::State::kRichlyEditable));
}
...
{
    std::u16string value = node->GetString16Attribute(ax::mojom::StringAttribute::kValue);

    if (value.empty() && (IsTextField(node) || IsRichTextEditable(node)) && !IsAtomicTextField(node)) {
...
{
    if (node->GetRole() == ax::mojom::Role::kPdfRoot || node->GetRole() == ax::mojom::Role::kIframe
    ...
    if (node->GetRole() == ax::mojom::Role::kColorWell) {
        unsigned int color = static_cast<unsigned int>(node->GetIntAttribute(ax::mojom::IntAttribute::kColorValue));
        unsigned int red = color >> 16 & 0xFF;
        unsigned int green = color >> 8 & 0xFF;
        unsigned int blue = color >> 0 & 0xFF;
        return base::UTF8ToUTF16(base::StringPrintf("#%02X%02X%02X", red, green, blue));
    }

    std::u16string text = node->GetString16Attribute(ax::mojom::StringAttribute::kName);
    std::u16string description = node->GetString16Attribute(ax::mojom::StringAttribute::kDescription);
    ...
    if (node->GetRole() == ax::mojom::Role::kRootWebArea || node->GetRole() == ax::mojom::Role::kPdfRoot) {
        return text;
    }
    ...
    if (text.empty() && (IsLink(node->GetRole()) || node->GetRole() == ax::mojom::Role::kImage)) {
        std::u16string url = node->GetString16Attribute(ax::mojom::StringAttribute::kUrl);
...
void WalkAXTreeDepthFirst(const AXNode* node, const gfx::Rect& parent_absolute_clipped_rect, const gfx::Rect& parent_absolute_unclipped_rect,
    const int root_scroll_y, const AXTreeUpdate& update, const AXTree* tree, WalkAXTreeConfig* config, AssistantTree* assistant_tree, AssistantNode* result)
{
    result->text = GetText(node);
    result->class_name = AXRoleToAndroidClassName(node->GetRole(), node->GetUnignoredParent());
    ...
    if (node->HasFloatAttribute(ax::mojom::FloatAttribute::kFontSize)) {
        result->text_size = node->GetFloatAttribute(ax::mojom::FloatAttribute::kFontSize);
        result->color = node->GetIntAttribute(ax::mojom::IntAttribute::kColor);
        result->bgcolor = node->GetIntAttribute(ax::mojom::IntAttribute::kBackgroundColor);
        result->bold = node->HasTextStyle(ax::mojom::TextStyle::kBold);
        result->italic = node->HasTextStyle(ax::mojom::TextStyle::kItalic);
        result->line_through = node->HasTextStyle(ax::mojom::TextStyle::kLineThrough);
        result->underline = node->HasTextStyle(ax::mojom::TextStyle::kUnderline);
    }

    const gfx::Rect& absolute_clipped_rect = gfx::ToEnclosingRect(tree->GetTreeBounds(node));
    const gfx::Rect& absolute_unclipped_rect = gfx::ToEnclosingRect(tree->GetTreeBounds(node, nullptr, /* clip_bounds = */ false));
    ...
    if (!is_root) {
        parent_relative_clipped_rect.Offset(-parent_absolute_clipped_rect.OffsetFromOrigin());
        parent_relative_unclipped_rect.Offset(-parent_absolute_unclipped_rect.OffsetFromOrigin());
    }
    ...
    if (IsLeaf(node) && update.has_tree_data) {
        ...
        if (end_selection > 0)
            result->selection = std::make_optional<gfx::Range>(start_selection, end_selection);
    }

    result->html_tag = node->GetStringAttribute(ax::mojom::StringAttribute::kHtmlTag);
    result->css_display = node->GetStringAttribute(ax::mojom::StringAttribute::kDisplay);
    result->html_attributes = node->GetHtmlAttributes();

    // Always add root scroll values for debugging scrolling.
    result->html_attributes.emplace_back("root_scroll_y", base::NumberToString(root_scroll_y));

    std::string class_name = node->GetStringAttribute(ax::mojom::StringAttribute::kClassName);
    if (!class_name.empty())
    ...
    for (auto iter = node->UnignoredChildrenBegin(); iter != node->UnignoredChildrenEnd(); ++iter) {
        ...
        WalkAXTreeDepthFirst(iter.get(), absolute_clipped_rect, absolute_unclipped_rect, root_scroll_y, update, tree, config, assistant_tree, n);
    }
}
...
std::unique_ptr<AssistantTree> CreateAssistantTree(const AXTreeUpdate& update)
{
    ...
    int root_scroll_y = tree->root()->GetIntAttribute(ax::mojom::IntAttribute::kScrollY);

    WalkAXTreeDepthFirst(tree->root(), gfx::Rect(), gfx::Rect(), root_scroll_y, update, tree.get(), &config, assistant_tree.get(), root);
