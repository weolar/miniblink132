...
void AXTreeFuzzerGenerator::GenerateInitialUpdate(FuzzerData& fuzz_data, int node_count)
{
    ...
    while (fuzz_data.RemainingBytes() >= kMinimumNewNodeFuzzDataSize && max_assigned_node_id_ < node_count) {
        size_t extra_data_size = fuzz_data.RemainingBytes() - kMinimumNewNodeFuzzDataSize;
        ...
        AddRoleSpecificProperties(fuzz_data, node, parent.GetStringAttribute(ax::mojom::StringAttribute::kName), extra_data_size);
        ...
        case kSiblingToAncestor:
            ...
            for (size_t i = 0; i < ancestor_pop_count && parent_index_stack.size() > 1; ++i) {
...
void AXTreeFuzzerGenerator::RecursiveGenerateUpdate(
    const ui::AXNode* node, ui::AXTreeUpdate& tree_update, FuzzerData& fuzz_data, std::set<ui::AXNodeID>& updated_nodes)
{
    ...
    case kAddChild: {
        ...
        ui::AXNodeData child = CreateChildNodeData(parent, max_assigned_node_id_++);

        // Determine role.
        child.role = GetInterestingRole(fuzz_data.NextByte(), node->GetRole());

        // Add role-specific properties.
        AddRoleSpecificProperties(fuzz_data, child, node->GetStringAttribute(ax::mojom::StringAttribute::kName), extra_data_size);
        ...
        if (ui::CanHaveInlineTextBoxChildren(child.role)) {
            ...
            inline_text_data.SetName(child.GetStringAttribute(ax::mojom::StringAttribute::kName));
        }
    ...
    case kTextChange: {
        ...
        if (!child_inline_text || child_inline_text->GetRole() != ax::mojom::Role::kInlineTextBox) {
            break;
        }
        ui::AXNodeData static_text_data = node->data();
        ui::AXNodeData inline_text_data = child_inline_text->data();
        size_t text_size = kMinTextFuzzDataSize + fuzz_data.NextByte() % kMaxTextFuzzDataSize;
        if (text_size > extra_data_size)
            text_size = extra_data_size;
        extra_data_size -= text_size;
        inline_text_data.SetName(GenerateInterestingText(fuzz_data.NextBytes(text_size), text_size));
        static_text_data.SetName(inline_text_data.GetStringAttribute(ax::mojom::StringAttribute::kName));
    ...
    // Visit subtree.
    for (auto iter = node->AllChildrenBegin(); iter != node->AllChildrenEnd(); ++iter) {
...
bool AXTreeFuzzerGenerator::GenerateTreeUpdate(FuzzerData& fuzz_data, size_t node_count)
{
    ui::AXTreeUpdate tree_update;
    std::set<ui::AXNodeID> updated_nodes;
    RecursiveGenerateUpdate(tree_manager_.GetRoot(), tree_update, fuzz_data, updated_nodes);
...
ui::AXNodeData AXTreeFuzzerGenerator::CreateChildNodeData(ui::AXNodeData& parent, ui::AXNodeID new_node_id)
{
    ui::AXNodeData node;
    node.id = new_node_id;
...
AXTreeFuzzerGenerator::NextNodeRelationship AXTreeFuzzerGenerator::DetermineNextNodeRelationship(ax::mojom::Role role, unsigned char byte)
{
    ...
    NextNodeRelationship relationship = static_cast<NextNodeRelationship>(byte % 3);
...
AXTreeFuzzerGenerator::TreeUpdateOperation AXTreeFuzzerGenerator::DetermineTreeUpdateOperation(const ui::AXNode* node, unsigned char byte)
{
    switch (byte % 4) {
    case 0:
...
void AXTreeFuzzerGenerator::AddRoleSpecificProperties(FuzzerData& fuzz_data, ui::AXNodeData& node, const std::string& parentName, size_t extra_data_size)
{
    ...
    if (node.role == ax::mojom::Role::kInlineTextBox) {
        ...
        if (text_size > extra_data_size)
            text_size = extra_data_size;
        extra_data_size -= text_size;
        node.SetName(GenerateInterestingText(fuzz_data.NextBytes(text_size), text_size));
    }
}

ax::mojom::Role AXTreeFuzzerGenerator::GetInterestingRole(unsigned char byte, ax::mojom::Role parent_role)
{
...
std::u16string AXTreeFuzzerGenerator::GenerateInterestingText(const unsigned char* data, size_t size)
{
