...
constexpr int kMax_number_of_trees0 = TreeGenerator::ComputeUniqueTreeCount(kMax_tree_size,
    /* permutations */ false);
constexpr int kMax_number_of_trees1 = TreeGenerator::ComputeUniqueTreeCount(kMax_tree_size,
    /* permutations */ true);
...
std::string TreeToStringHelper(const AXNode* node)
{
    ...
    if (node->children().empty())
        return result;
    const auto add_children = [](const std::string& str, const AXNode* node) { return str + " " + TreeToStringHelper(node); };
    return result + " (" + std::accumulate(node->children().cbegin() + 1, node->children().cend(), TreeToStringHelper(node->children().front()), add_children)
        + ")";
}
...
{
    std::unique_ptr<AXTreeSource<const AXNode*, AXTreeData*, AXNodeData>> tree_source(tree.CreateTreeSource());
...
AXTreeUpdate MakeTreeUpdateFromIgnoredChanges(AXSerializableTree& tree0, AXSerializableTree& tree1)
{
...
void MakeTreeOfUnignoredNodesOnly(AXSerializableTree& src, AXSerializableTree* dst)
{
    AXTreeUpdate update;
...
TEST(AXGeneratedTreeTest, TestGeneratingTreesWithIgnoredNodes)
{
  ...
  const char* EXPECTED_TREES[] = {
    ...
    for (int i = 0; i < n; ++i) {
        int ignored_permutation_count = generator.IgnoredPermutationCountPerUniqueTree(i);
        for (int j = 0; j < ignored_permutation_count; j++) {
            AXTree tree;
            generator.BuildUniqueTreeWithIgnoredNodes(i, j, /* focused_node */ std::nullopt, &tree);
            std::string str = TreeToString(tree);
...
struct PermutationBlock {
    PermutationBlock(int first_unique_tree0, int last_unique_tree0, int first_unique_tree1, int last_unique_tree1)
...
class SerializeGeneratedTreesTest : public testing::TestWithParam<PermutationBlock> { };
        ...
        for (int j = first_tree1_; j < last_tree1_; j++) {
            ...
            for (int k = 0; k < tree_size; k++) {
                ...
                for (int l = 0; l <= tree_size; l++) {
                    SCOPED_TRACE(
                        "i=" + base::NumberToString(i) + " j=" + base::NumberToString(j) + " k=" + base::NumberToString(k) + " l=" + base::NumberToString(l));

                    // Start by serializing tree0 and unserializing it into a new
                    // empty tree |dst_tree|.
                    std::unique_ptr<AXTreeSource<const AXNode*, AXTreeData*, AXNodeData>> tree0_source(tree0.CreateTreeSource());
                    AXTreeSerializer<const AXNode*, std::vector<const AXNode*>, AXTreeUpdate*, AXTreeData*, AXNodeData> serializer(tree0_source.get());
                    AXTreeUpdate update0;
                    ASSERT_TRUE(serializer.SerializeChanges(tree0.root(), &update0));

                    AXTree dst_tree;
                    ASSERT_TRUE(dst_tree.Unserialize(update0)) << dst_tree.error() << "\n" << TreeToString(dst_tree) << "\nTree update: " << update0.ToString();
                    ...
                    std::unique_ptr<AXTreeSource<const AXNode*, AXTreeData*, AXNodeData>> tree1_source(tree1.CreateTreeSource());
                    ...
                    for (int k_index = 0; k_index < tree_size; ++k_index) {
                        ...
                        AXTreeUpdate update;
                        ASSERT_TRUE(serializer.SerializeChanges(tree1.GetFromId(id), &update));
...
INSTANTIATE_TEST_SUITE_P(AXGeneratedTreeTest0, SerializeGeneratedTreesTest,
    testing::Values(PermutationBlock(0, kMax_number_of_trees0 / 2, 0, kMax_number_of_trees1 / 2),
        PermutationBlock(0, kMax_number_of_trees0 / 2, kMax_number_of_trees1 / 2, kMax_number_of_trees1),
        PermutationBlock(kMax_number_of_trees0 / 2, kMax_number_of_trees0, 0, kMax_number_of_trees1 / 2),
        PermutationBlock(kMax_number_of_trees0 / 2, kMax_number_of_trees0, kMax_number_of_trees1 / 2, kMax_number_of_trees1)));
...
TEST(AXGeneratedTreeTest, GeneratedTreesWithIgnoredNodes)
{
    ...
    for (int tree_index = 0; tree_index < unique_tree_count; tree_index++) {
        ...
        int ignored_permutation_count = generator.IgnoredPermutationCountPerUniqueTree(tree_index);
        for (int perm_index0 = 0; perm_index0 < ignored_permutation_count; perm_index0++) {
            AXSerializableTree fat_tree;
            generator.BuildUniqueTreeWithIgnoredNodes(tree_index, perm_index0, /* focused_node */ std::nullopt, &fat_tree);
            ...
            for (int perm_index1 = 1; perm_index1 < ignored_permutation_count; perm_index1++) {
                AXSerializableTree fat_tree1;
                generator.BuildUniqueTreeWithIgnoredNodes(tree_index, perm_index1, /* focused_node */ std::nullopt, &fat_tree1);
                ...
                AXTreeUpdate update = MakeTreeUpdateFromIgnoredChanges(fat_tree, fat_tree1);
                ...
                for (const AXEventGenerator::TargetedEvent& event : event_generator) {
                    ...
                    if (node->IsIgnored() || event.event_params->event == AXEventGenerator::Event::IGNORED_CHANGED) {
                        continue;
                    }
                ...
                for (const AXEventGenerator::TargetedEvent& event : skinny_event_generator)
                ...
                for (auto& entry : expected_events) {
                    ...
                    for (auto& event_type : entry.second) {
                        EXPECT_TRUE(actual_events[node_id].find(event_type) != actual_events[node_id].end())
                ...
                for (auto& entry : actual_events) {
                    AXNodeID node_id = entry.first;
                    for (auto& event_type : entry.second) {
                        EXPECT_TRUE(expected_events[node_id].find(event_type) != expected_events[node_id].end())
                ...
                for (const AXNodeData& skinny_tree_node_data : skinny_tree_serialized.nodes) {
                    ...
                    // Check children.
                    EXPECT_EQ(skinny_tree_node->children().size(), fat_tree_node->GetUnignoredChildCount());
                    ...
                    if (skinny_tree_node->parent()) {
                        EXPECT_EQ(skinny_tree_node->parent()->id(), fat_tree_node->GetUnignoredParent()->id());
                    } else {
                        EXPECT_FALSE(fat_tree_node->GetUnignoredParent());
                    }

                    // Check index in parent.
                    EXPECT_EQ(skinny_tree_node->index_in_parent(), fat_tree_node->GetUnignoredIndexInParent());

                    // Unignored previous sibling.
                    size_t index_in_parent = skinny_tree_node->index_in_parent();
                    size_t num_siblings = skinny_tree_node->parent() ? skinny_tree_node->parent()->children().size() : 1;
                    if (index_in_parent > 0) {
                        AXNode* skinny_tree_previous_sibling = skinny_tree_node->parent()->children()[index_in_parent - 1];
                        AXNode* fat_tree_previous_sibling = fat_tree_node->GetPreviousUnignoredSibling();
                        EXPECT_TRUE(fat_tree_previous_sibling);
                        if (fat_tree_previous_sibling) {
                            EXPECT_EQ(skinny_tree_previous_sibling->id(), fat_tree_previous_sibling->id());
                        }
                    }

                    // Unignored next sibling.
                    if (index_in_parent < num_siblings - 1) {
                        AXNode* skinny_tree_next_sibling = skinny_tree_node->parent()->children()[index_in_parent + 1];
                        AXNode* fat_tree_next_sibling = fat_tree_node->GetNextUnignoredSibling();
                        EXPECT_TRUE(fat_tree_next_sibling);
                        if (fat_tree_next_sibling) {
                            EXPECT_EQ(skinny_tree_next_sibling->id(), fat_tree_next_sibling->id());
                        }
                    }
                }
            }
