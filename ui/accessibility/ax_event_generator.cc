...
bool HasEvent(const std::set<AXEventGenerator::EventParams>& node_events, AXEventGenerator::Event event)
{
    return node_events.count(AXEventGenerator::EventParams(event));
}

void RemoveEvent(std::set<AXEventGenerator::EventParams>* node_events, AXEventGenerator::Event event)
{
...
void RemoveEventsDueToIgnoredChanged(std::set<AXEventGenerator::EventParams>* node_events)
{
    RemoveEvent(node_events, AXEventGenerator::Event::ATK_TEXT_OBJECT_ATTRIBUTE_CHANGED);
    ...
    RemoveEvent(node_events, AXEventGenerator::Event::WIN_IACCESSIBLE_STATE_CHANGED);
}

// Add a particular AXEventGenerator::IgnoredChangedState to
// |ignored_changed_states|.
void AddIgnoredChangedState(const AXEventGenerator::IgnoredChangedState& state, AXEventGenerator::IgnoredChangedStatesBitset* ignored_changed_states)
{
...
bool HasIgnoredChangedState(const AXEventGenerator::IgnoredChangedStatesBitset& ignored_changed_states, const AXEventGenerator::IgnoredChangedState& state)
{
...
AXEventGenerator::EventParams::EventParams(
    const Event event, const ax::mojom::EventFrom event_from, const ax::mojom::Action event_from_action, const std::vector<AXEventIntent>& event_intents)
    : event(event)
...
AXEventGenerator::EventParams& AXEventGenerator::EventParams::operator=(const EventParams& other) = default;
...
AXEventGenerator::TargetedEvent::TargetedEvent(AXNodeID node_id, const EventParams& event_params)
    : node_id(node_id)
...
AXEventGenerator::Iterator::Iterator(
    std::map<AXNodeID, std::set<EventParams>>::const_iterator map_start_iter, std::map<AXNodeID, std::set<EventParams>>::const_iterator map_end_iter)
    : map_iter_(map_start_iter)
    , map_end_iter_(map_end_iter)
{
    if (map_iter_ != map_end_iter_)
        set_iter_ = map_iter_->second.begin();
}

AXEventGenerator::Iterator::Iterator(const AXEventGenerator::Iterator& other) = default;

AXEventGenerator::Iterator::~Iterator() = default;

AXEventGenerator::Iterator& AXEventGenerator::Iterator::operator=(const Iterator& other) = default;
...
bool operator==(const AXEventGenerator::Iterator& lhs, const AXEventGenerator::Iterator& rhs)
{
...
void swap(AXEventGenerator::Iterator& lhs, AXEventGenerator::Iterator& rhs)
{
    ...
    std::map<AXNodeID, std::set<AXEventGenerator::EventParams>>::const_iterator map_iter = lhs.map_iter_;
    lhs.map_iter_ = rhs.map_iter_;
    rhs.map_iter_ = map_iter;
    std::map<AXNodeID, std::set<AXEventGenerator::EventParams>>::const_iterator map_end_iter = lhs.map_end_iter_;
    lhs.map_end_iter_ = rhs.map_end_iter_;
    rhs.map_end_iter_ = map_end_iter;
    std::set<AXEventGenerator::EventParams>::const_iterator set_iter = lhs.set_iter_;
...
AXEventGenerator::Iterator AXEventGenerator::begin() const
{
    ...
    if (map_iter != tree_events_.end()) {
        ...
        // contains a non-empty set of events.
        while (map_iter != tree_events_.end() && set_iter == map_iter->second.end()) {
            map_iter++;
            if (map_iter != tree_events_.end())
...
void AXEventGenerator::AddEvent(AXNode* node, AXEventGenerator::Event event)
{
    ...
    node_events.emplace(event, tree_->event_data()->event_from, tree_->event_data()->event_from_action, tree_->event_data()->event_intents);
}
...
void AXEventGenerator::UnregisterEventOnNode(Event event_type, AXNodeID node_id)
{
...
void AXEventGenerator::OnIgnoredWillChange(AXTree* tree, AXNode* node, bool is_ignored_new_value, bool is_changing_unignored_parents_children)
{
...
void AXEventGenerator::OnNodeDataChanged(AXTree* tree, const AXNodeData& old_node_data, const AXNodeData& new_node_data)
{
    ...
    if (new_node_data.HasStringListAttribute(ax::mojom::StringListAttribute::kAriaNotificationAnnouncements)) {
    ...
    if (is_ignored) {
        ...
        DUMP_WILL_BE_CHECK(unignored_parent) << "The root cannot be ignored, so an unignored parent is always "
                                                "found.";
...
void AXEventGenerator::OnRoleChanged(AXTree* tree, AXNode* node, ax::mojom::Role old_role, ax::mojom::Role new_role)
{
    DCHECK_EQ(tree_, tree);
    AddEvent(node, new_role == ax::mojom::Role::kAlert ? Event::ALERT : Event::ROLE_CHANGED);
}

void AXEventGenerator::OnIgnoredChanged(AXTree* tree, AXNode* node, bool is_ignored_new_value)
{
    DCHECK_EQ(tree_, tree);
    ...
    // whether nodes are known to the AT. In fact, a node can be invisible and
    // have visible descendants.
    const bool was_in_invisible_subtree = !base::Contains(nodes_to_suppress_parent_changed_on_, node->id());
...
void AXEventGenerator::OnStateChanged(AXTree* tree, AXNode* node, ax::mojom::State state, bool new_value)
{
    DCHECK_EQ(tree_, tree);
    DCHECK_NE(state, ax::mojom::State::kIgnored) << "The ignored state should be handled in "
    ...
    case ax::mojom::State::kExpanded:
        if (node->data().HasState(ax::mojom::State::kCollapsed) || node->data().HasState(ax::mojom::State::kExpanded)) {
            AddEvent(node, new_value ? Event::EXPANDED : Event::COLLAPSED);
        }
        if (IsTableRow(node->GetRole()) || node->GetRole() == ax::mojom::Role::kTreeItem) {
...
void AXEventGenerator::OnStringAttributeChanged(
    AXTree* tree, AXNode* node, ax::mojom::StringAttribute attr, const std::string& old_value, const std::string& new_value)
{
    ...
    case ax::mojom::StringAttribute::kLiveStatus:
        ...
        if (!IsAlert(node->GetRole())) {
            bool was_off = !old_value.empty() ? old_value == "off" : !node->data().IsContainedInActiveLiveRegion();
            bool is_off = !new_value.empty() ? new_value == "off" : !node->data().IsContainedInActiveLiveRegion();
            if (was_off && !is_off)
    ...
    case ax::mojom::StringAttribute::kName:
        ...
        if (node->HasStringAttribute(ax::mojom::StringAttribute::kContainerLiveStatus)) {
...
void AXEventGenerator::OnIntAttributeChanged(AXTree* tree, AXNode* node, ax::mojom::IntAttribute attr, int32_t old_value, int32_t new_value)
{
    DCHECK_EQ(tree_, tree);
    ...
    case ax::mojom::IntAttribute::kRestriction: {
        bool was_enabled;
        bool was_readonly;
        GetRestrictionStates(static_cast<ax::mojom::Restriction>(old_value), &was_enabled, &was_readonly);
        bool is_enabled;
        bool is_readonly;
        GetRestrictionStates(static_cast<ax::mojom::Restriction>(new_value), &is_enabled, &is_readonly);
...
void AXEventGenerator::OnFloatAttributeChanged(AXTree* tree, AXNode* node, ax::mojom::FloatAttribute attr, float old_value, float new_value)
{
    DCHECK_EQ(tree_, tree);
...
void AXEventGenerator::OnBoolAttributeChanged(AXTree* tree, AXNode* node, ax::mojom::BoolAttribute attr, bool new_value)
{
    DCHECK_EQ(tree_, tree);
    ...
    case ax::mojom::BoolAttribute::kSelected: {
        ...
        AddEvent(node, Event::WIN_IACCESSIBLE_STATE_CHANGED);
        AXNode* container = node;
        while (container && !IsContainerWithSelectableChildren(container->GetRole()))
            container = container->parent();
        if (container)
...
void AXEventGenerator::OnIntListAttributeChanged(
    AXTree* tree, AXNode* node, ax::mojom::IntListAttribute attr, const std::vector<int32_t>& old_value, const std::vector<int32_t>& new_value)
{
    DCHECK_EQ(tree_, tree);
...
void AXEventGenerator::OnTreeDataChanged(AXTree* tree, const AXTreeData& old_tree_data, const AXTreeData& new_tree_data)
{
    DCHECK_EQ(tree_, tree);
    DCHECK(tree->root());
    ...
    if (new_tree_data.sel_is_backward != old_tree_data.sel_is_backward || new_tree_data.sel_anchor_object_id != old_tree_data.sel_anchor_object_id
        || new_tree_data.sel_anchor_offset != old_tree_data.sel_anchor_offset || new_tree_data.sel_anchor_affinity != old_tree_data.sel_anchor_affinity
        || new_tree_data.sel_focus_object_id != old_tree_data.sel_focus_object_id || new_tree_data.sel_focus_offset != old_tree_data.sel_focus_offset
        ...
        const AXNode* selection_focus = tree_->GetFromId(new_tree_data.sel_focus_object_id);
        if (selection_focus) {
...
void AXEventGenerator::OnAtomicUpdateFinished(AXTree* tree, bool root_changed, const std::vector<Change>& changes)
{
    DCHECK_EQ(tree_, tree);
    DCHECK(tree->root());
    ...
    for (const auto& change : changes) {
        ...
        if (change.node->GetBoolAttribute(ax::mojom::BoolAttribute::kSelected) && (change.type == SUBTREE_CREATED || change.type == NODE_CREATED)) {
            OnBoolAttributeChanged(tree, change.node, ax::mojom::BoolAttribute::kSelected,
                /*new_value*/ true);
        }
...
void AXEventGenerator::AddEventsForTesting(const AXNode& node, const std::set<EventParams>& events)
{
...
{
    std::string aria_relevant = node->GetStringAttribute(ax::mojom::StringAttribute::kContainerLiveRelevant);
    if (aria_relevant.empty())
        return false;
    std::vector<std::string> tokens = base::SplitString(aria_relevant, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    return std::any_of(tokens.begin(), tokens.end(), [](std::string token) { return token == "all" || token == "removals"; });
}

void AXEventGenerator::FireLiveRegionEvents(AXNode* node, bool is_removal)
{
    AXNode* live_root = node;
    std::string container_live = node->GetStringAttribute(ax::mojom::StringAttribute::kContainerLiveStatus);
    ...
    while (live_root && live_root->GetStringAttribute(ax::mojom::StringAttribute::kLiveStatus) != container_live)
        live_root = live_root->parent();

    if (live_root && !live_root->GetBoolAttribute(ax::mojom::BoolAttribute::kBusy)) {
...
void AXEventGenerator::FireActiveDescendantEvents()
{
    for (AXNode* node : active_descendant_changed_) {
        AXNode* descendant = tree_->GetFromId(node->GetIntAttribute(ax::mojom::IntAttribute::kActivedescendantId));
        if (!descendant)
            continue;
...
void AXEventGenerator::FireValueInTextFieldChangedEventIfNecessary(AXTree* tree, AXNode* target_node)
{
...
void AXEventGenerator::FireRelationSourceEvents(AXTree* tree, AXNode* target_node)
{
...
void AXEventGenerator::TrimEventsDueToAncestorIgnoredChanged(AXNode* node, std::map<AXNode*, IgnoredChangedStatesBitset>& ancestor_ignored_changed_map)
{
    DCHECK(node);
    ...
    if (node->parent() && !base::Contains(ancestor_ignored_changed_map, node->parent())) {
        TrimEventsDueToAncestorIgnoredChanged(node->parent(), ancestor_ignored_changed_map);
    }
    ...
    const auto& parent_map_iter = ancestor_ignored_changed_map.find(node->parent());
    ...
    IgnoredChangedStatesBitset& ancestor_ignored_changed_states = ancestor_ignored_changed_map[node];
    ...
    if (parent_map_iter != ancestor_ignored_changed_map.end()) {
        ...
        if (HasIgnoredChangedState(parent_map_iter->second, IgnoredChangedState::kHide)) {
            AddIgnoredChangedState(IgnoredChangedState::kHide, &ancestor_ignored_changed_states);
        }
        if (HasIgnoredChangedState(parent_map_iter->second, IgnoredChangedState::kShow)) {
            AddIgnoredChangedState(IgnoredChangedState::kShow, &ancestor_ignored_changed_states);
        }
        ...
        if (curr_events_iter != tree_events_.end() && HasEvent(curr_events_iter->second, Event::IGNORED_CHANGED)) {
            if ((HasIgnoredChangedState(parent_map_iter->second, IgnoredChangedState::kHide) && node->IsIgnored())
                || (HasIgnoredChangedState(parent_map_iter->second, IgnoredChangedState::kShow) && !node->IsIgnored())) {
...
void AXEventGenerator::PostprocessEvents()
{
    ...
    for (auto& iter : tree_events_) {
        ...
        if (HasEvent(node_events, Event::ALERT) || HasEvent(node_events, Event::LIVE_REGION_CREATED)) {
        ...
        if (parent && HasEvent(node_events, Event::TEXT_ATTRIBUTE_CHANGED) && tree_events_.find(parent->id()) != tree_events_.end()
        ...
        if (HasEvent(node_events, Event::PARENT_CHANGED)) {
            while (parent && (tree_events_.find(parent->id()) != tree_events_.end() || base::Contains(removed_parent_changed_nodes, parent))) {
                if ((base::Contains(removed_parent_changed_nodes, parent) || HasEvent(tree_events_[parent->id()], Event::PARENT_CHANGED))
        ...
        if (HasEvent(node_events, Event::SUBTREE_CREATED)) {
            while (parent && (tree_events_.find(parent->id()) != tree_events_.end() || base::Contains(removed_subtree_created_nodes, parent))) {
                if (base::Contains(removed_subtree_created_nodes, parent) || HasEvent(tree_events_[parent->id()], Event::SUBTREE_CREATED)) {
...
// static
void AXEventGenerator::GetRestrictionStates(ax::mojom::Restriction restriction, bool* is_enabled, bool* is_readonly)
{
    switch (restriction) {
...
// static
std::vector<int32_t> AXEventGenerator::ComputeIntListDifference(const std::vector<int32_t>& lhs, const std::vector<int32_t>& rhs)
{
    std::set<int32_t> sorted_lhs(lhs.cbegin(), lhs.cend());
    std::set<int32_t> sorted_rhs(rhs.cbegin(), rhs.cend());

    std::vector<int32_t> result;
    std::set_symmetric_difference(sorted_lhs.cbegin(), sorted_lhs.cend(), sorted_rhs.cbegin(), sorted_rhs.cend(), std::back_inserter(result));
    return result;
}
...
bool MaybeParseGeneratedEvent(const char* attribute, AXEventGenerator::Event* result)
{
    static base::NoDestructor<std::map<std::string, AXEventGenerator::Event>> attr_map;
    if (attr_map->empty()) {
        (*attr_map)[""] = AXEventGenerator::Event::NONE;
        for (int i = static_cast<int>(AXEventGenerator::Event::NONE); i <= static_cast<int>(AXEventGenerator::Event::MAX_VALUE); i++) {
