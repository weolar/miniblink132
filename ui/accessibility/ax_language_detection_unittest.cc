...
const std::string kTextEnglish = "This is text created using Google Translate, it is unlikely to be "
                                 "idiomatic in the given target language. This text is only used to "
                                 "test language detection";

const std::string kTextFrench = "Ce texte a été créé avec Google Translate, il est peu probable qu'il "
                                "soit idiomatique dans la langue cible indiquée Ce texte est "
                                "uniquement utilisé pour tester la détection de la langue.";

const std::string kTextGerman = "Dies ist ein mit Google Translate erstellter Text. Es ist "
...
const std::string kTextSpanish = "Este es un texto creado usando Google Translate, es poco probable que sea "
...
public:
    ...
    AXLanguageDetectionTestFixture(const AXLanguageDetectionTestFixture&) = delete;
    AXLanguageDetectionTestFixture& operator=(const AXLanguageDetectionTestFixture&) = delete;
...
protected:
    ...
    {
        tree.language_detection_manager->lang_info_stats_.disable_metric_clearing_ = true;
    }

    int count_detection_attempted(AXTree& tree) const
    {
        return tree.language_detection_manager->lang_info_stats_.count_detection_attempted_;
    }

    int count_detection_results(AXTree& tree) const
    {
        return tree.language_detection_manager->lang_info_stats_.count_detection_results_;
    }
    ...
    {
        return tree.language_detection_manager->lang_info_stats_.count_labelled_with_top_result_;
    }
    ...
    const std::unordered_set<std::string>& unique_top_lang_detected(AXTree& tree) const
    {
        return tree.language_detection_manager->lang_info_stats_.unique_top_lang_detected_;
    }
};

class AXLanguageDetectionTestStaticContent : public AXLanguageDetectionTestFixture {
public:
    AXLanguageDetectionTestStaticContent() = default;
    ~AXLanguageDetectionTestStaticContent() override = default;

    AXLanguageDetectionTestStaticContent(const AXLanguageDetectionTestStaticContent&) = delete;
    AXLanguageDetectionTestStaticContent& operator=(const AXLanguageDetectionTestStaticContent&) = delete;
...
class AXLanguageDetectionTestDynamicContent : public AXLanguageDetectionTestStaticContent {
public:
    AXLanguageDetectionTestDynamicContent() = default;
    ~AXLanguageDetectionTestDynamicContent() override = default;

    AXLanguageDetectionTestDynamicContent(const AXLanguageDetectionTestDynamicContent&) = delete;
    AXLanguageDetectionTestDynamicContent& operator=(const AXLanguageDetectionTestDynamicContent&) = delete;

    void SetUp() override
    {
        AXLanguageDetectionTestStaticContent::SetUp();

        base::CommandLine::ForCurrentProcess()->AppendSwitch(::switches::kEnableExperimentalAccessibilityLanguageDetectionDynamic);
    }
};
...
TEST_F(AXLanguageDetectionTestFixture, StaticContentFeatureFlag)
{
    ...
    EXPECT_FALSE(::switches::IsExperimentalAccessibilityLanguageDetectionEnabled());
    EXPECT_FALSE(IsStaticLanguageDetectionEnabled());

    base::CommandLine::ForCurrentProcess()->AppendSwitch(::switches::kEnableExperimentalAccessibilityLanguageDetection);

    EXPECT_TRUE(::switches::IsExperimentalAccessibilityLanguageDetectionEnabled());
...
{
    // TODO(crbug.com/41417304): Remove this test once this feature is stable
    EXPECT_FALSE(::switches::IsExperimentalAccessibilityLanguageDetectionDynamicEnabled());
    EXPECT_FALSE(IsDynamicLanguageDetectionEnabled());

    base::CommandLine::ForCurrentProcess()->AppendSwitch(::switches::kEnableExperimentalAccessibilityLanguageDetectionDynamic);

    EXPECT_TRUE(::switches::IsExperimentalAccessibilityLanguageDetectionDynamicEnabled());
...
TEST_F(AXLanguageDetectionTestFixture, FeatureFlag)
{
    ...
    scoped_feature_list.InitWithFeatures({ features::kEnableAccessibilityLanguageDetection }, {});
...
TEST_F(AXLanguageDetectionTestStaticContent, MetricCollection)
{
...

    {
        ...
        node6.AddStringAttribute(ax::mojom::StringAttribute::kName, "too short for detection.");
    }
    ...
    histograms.ExpectUniqueSample("Accessibility.LanguageDetection.CountDetectionAttempted", 5, 1);
    ...
    histograms.ExpectUniqueSample("Accessibility.LanguageDetection.PercentageLanguageDetected", 80, 1);

    // 3 nodes (any of 2, 3, 4, 5) should have been labelled.
    EXPECT_EQ(3, count_labelled(tree));
    histograms.ExpectUniqueSample("Accessibility.LanguageDetection.CountLabelled", 3, 1);
    ...
    histograms.ExpectUniqueSample("Accessibility.LanguageDetection.PercentageLabelledWithTop", 100, 1);
    ...
    histograms.ExpectUniqueSample("Accessibility.LanguageDetection.PercentageOverridden", 100, 1);

    // There should be 4 unique languages (de, en, fr, es).
    {
        const auto& top_lang = unique_top_lang_detected(tree);
        const std::unordered_set<std::string> expected_top_lang = { "de", "en", "es", "fr" };
        EXPECT_EQ(top_lang, expected_top_lang);
    }
    histograms.ExpectUniqueSample("Accessibility.LanguageDetection.LangsPerPage", 4, 1);
}
...
TEST_F(AXLanguageDetectionTestStaticContent, kLanguageUntouched)
{
...

    {
        ...
        EXPECT_EQ(node2->GetStringAttribute(ax::mojom::StringAttribute::kLanguage), "en");
        EXPECT_EQ(node2->GetLanguage(), "fr");
    }

    {
        ...
        EXPECT_EQ(node3->GetStringAttribute(ax::mojom::StringAttribute::kLanguage), "fr");
        EXPECT_EQ(node3->GetLanguage(), "en");
    }
}
...
TEST_F(AXLanguageDetectionTestDynamicContent, MetricCollection)
{
    ...
    histograms.ExpectUniqueSample("Accessibility.LanguageDetection.CountDetectionAttempted", 2, 1);

    // 2 nodes (2, 4) should have had detection results
    EXPECT_EQ(2, count_detection_results(tree));
    histograms.ExpectUniqueSample("Accessibility.LanguageDetection.PercentageLanguageDetected", 100, 1);

    // 2 nodes (2, 4) should have been labelled
    EXPECT_EQ(2, count_labelled(tree));
    histograms.ExpectUniqueSample("Accessibility.LanguageDetection.CountLabelled", 2, 1);
    ...
    histograms.ExpectUniqueSample("Accessibility.LanguageDetection.PercentageOverridden", 50, 1);
    ...
    histograms.ExpectUniqueSample("Accessibility.LanguageDetection.LangsPerPage", 2, 1);
}
...
TEST(AXLanguageDetectionTest, ShortLanguageDetectorLabeledTest)
{
    ...
    initial_state.nodes[1].id = 2;
    initial_state.nodes[1].AddStringAttribute(ax::mojom::StringAttribute::kName, "Hello");
    initial_state.nodes[1].AddStringAttribute(ax::mojom::StringAttribute::kLanguage, "en");
    AXTree tree(initial_state);
    ...
    ASSERT_NE(tree.language_detection_manager, nullptr);
    // Empty output.
    annotation = tree.language_detection_manager->GetLanguageAnnotationForStringAttribute(*item, ax::mojom::StringAttribute::kMathContent);
    ASSERT_EQ(0, (int)annotation.size());
    // Returns single AXLanguageSpan.
    annotation = tree.language_detection_manager->GetLanguageAnnotationForStringAttribute(*item, ax::mojom::StringAttribute::kName);
    ASSERT_EQ(1, (int)annotation.size());
    AXLanguageSpan* lang_span = &annotation[0];
    ASSERT_EQ("en", lang_span->language);
    std::string name = item->GetStringAttribute(ax::mojom::StringAttribute::kName);
    ASSERT_EQ("Hello", name.substr(lang_span->start_index, lang_span->end_index - lang_span->start_index));
}
...
TEST(AXLanguageDetectionTest, ShortLanguageDetectorCharacterTest)
{
    ...
    initial_state.nodes[0].child_ids = { 2 };
    initial_state.nodes[1].id = 2;
    initial_state.nodes[1].AddStringAttribute(ax::mojom::StringAttribute::kName, "δ");
    AXTree tree(initial_state);
    ...
    std::string name = item->GetStringAttribute(ax::mojom::StringAttribute::kName);
    ASSERT_EQ("δ", name.substr(lang_span->start_index, lang_span->end_index - lang_span->start_index));
}
...
TEST(AXLanguageDetectionTest, ShortLanguageDetectorMultipleLanguagesTest)
{
    ...
    AXNode* item = tree.GetFromId(2);
    ASSERT_NE(tree.language_detection_manager, nullptr);
    std::vector<AXLanguageSpan> annotation = tree.language_detection_manager->GetLanguageAnnotationForStringAttribute(*item, ax::mojom::StringAttribute::kName);
    ASSERT_EQ(3, (int)annotation.size());
    std::string name = item->GetStringAttribute(ax::mojom::StringAttribute::kName);
    AXLanguageSpan* lang_span = &annotation[0];
    ASSERT_EQ("This text should be read in English. ", name.substr(lang_span->start_index, lang_span->end_index - lang_span->start_index));
    lang_span = &annotation[1];
    ASSERT_EQ("차에 한하여 중임할 수. ", name.substr(lang_span->start_index, lang_span->end_index - lang_span->start_index));
    lang_span = &annotation[2];
    ASSERT_EQ("Followed by English.", name.substr(lang_span->start_index, lang_span->end_index - lang_span->start_index));
}
...
TEST(AXLanguageDetectionTest, DetectLanguagesForRoleTest)
{
    ...
    initial_state.nodes.resize(1);
    initial_state.nodes[0].id = 1;
    initial_state.nodes[0].AddStringAttribute(ax::mojom::StringAttribute::kValue, "どうぞよろしくお願いします.");
    AXTree tree(initial_state);

    AXNode* item = tree.GetFromId(1);
    ASSERT_NE(tree.language_detection_manager, nullptr);
    std::vector<AXLanguageSpan> annotation
        = tree.language_detection_manager->GetLanguageAnnotationForStringAttribute(*item, ax::mojom::StringAttribute::kValue);
    ASSERT_EQ(1, (int)annotation.size());
    std::string value = item->GetStringAttribute(ax::mojom::StringAttribute::kValue);
    AXLanguageSpan* lang_span = &annotation[0];
    ASSERT_EQ("どうぞよろしくお願いします.", value.substr(lang_span->start_index, lang_span->end_index - lang_span->start_index));
}
