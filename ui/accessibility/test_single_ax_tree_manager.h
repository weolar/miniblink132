...
public:
    ...
    TestSingleAXTreeManager& operator=(const TestSingleAXTreeManager& manager) = delete;
    ...
    AXTree* Init(const AXNodeData& node1, const AXNodeData& node2 = AXNodeData(), const AXNodeData& node3 = AXNodeData(),
    ...
    AXNodePosition::AXPositionInstance CreateTreePosition(const AXNode& anchor, int child_index) const;

    // Creates a tree position for the given |anchor_data| of a node from the
    // given tree.
    AXNodePosition::AXPositionInstance CreateTreePosition(const AXTree* tree, const AXNodeData& anchor_data, int child_index) const;

    // Creates a tree position for the given |anchor_data| of a node from the
    // current tree.
    AXNodePosition::AXPositionInstance CreateTreePosition(const AXNodeData& anchor_data, int child_index) const;

    // Creates a text position, a simple wrapper around
    // AXNodePosition::CreateTextPosition.
    AXNodePosition::AXPositionInstance CreateTextPosition(const AXNode& anchor, int text_offset, ax::mojom::TextAffinity affinity) const;
    ...
    // current tree.
    AXNodePosition::AXPositionInstance CreateTextPosition(const AXNodeData& anchor_data, int text_offset, ax::mojom::TextAffinity affinity) const;

    // Creates a text position for the given |anchor_id| of a node from the
    // given tree.
    AXNodePosition::AXPositionInstance CreateTextPosition(const AXNodeID& anchor_id, int text_offset, ax::mojom::TextAffinity affinity) const;
