...
namespace mojo {
template <typename DataViewType, typename T> struct UnionTraits;
}
...
public:
    ...
    const std::optional<base::UnguessableToken>& token() const
    {
        return token_;
    }
...
AX_BASE_EXPORT std::ostream& operator<<(std::ostream& stream, const AXTreeID& value);
