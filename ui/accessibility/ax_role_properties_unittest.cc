...
TEST(AXRolePropertiesTest, TestSupportsHierarchicalLevel)
{
    ...
    // hierarchical level.
    std::unordered_set<ax::mojom::Role> roles_expected_supports_hierarchical_level
        = { ax::mojom::Role::kComment, ax::mojom::Role::kListItem, ax::mojom::Role::kRow, ax::mojom::Role::kTreeItem };
    ...
    for (int role_idx = static_cast<int>(ax::mojom::Role::kMinValue); role_idx <= static_cast<int>(ax::mojom::Role::kMaxValue); role_idx++) {
        ...
        if (roles_expected_supports_hierarchical_level.find(role) != roles_expected_supports_hierarchical_level.end())
...
TEST(AXRolePropertiesTest, TestSupportsToggle)
{
    ...
    std::unordered_set<ax::mojom::Role> roles_expected_supports_toggle
        = { ax::mojom::Role::kCheckBox, ax::mojom::Role::kMenuItemCheckBox, ax::mojom::Role::kSwitch, ax::mojom::Role::kToggleButton };

    for (int role_idx = static_cast<int>(ax::mojom::Role::kMinValue); role_idx <= static_cast<int>(ax::mojom::Role::kMaxValue); role_idx++) {
        ax::mojom::Role role = static_cast<ax::mojom::Role>(role_idx);
        bool supports_toggle = SupportsToggle(role);

        SCOPED_TRACE(testing::Message() << "ax::mojom::Role=" << ToString(role) << ", Actual: supportsToggle=" << supports_toggle
                                        << ", Expected: supportsToggle=" << !supports_toggle);

        if (roles_expected_supports_toggle.find(role) != roles_expected_supports_toggle.end())
...
TEST(AXRolePropertiesTest, TestIsTableWithColumns)
{
    ...
    std::unordered_set<ax::mojom::Role> roles_expected_is_table_with_columns
        = { ax::mojom::Role::kGrid, ax::mojom::Role::kListGrid, ax::mojom::Role::kTable, ax::mojom::Role::kTreeGrid };
    ...
    for (int role_idx = static_cast<int>(ax::mojom::Role::kMinValue); role_idx <= static_cast<int>(ax::mojom::Role::kMaxValue); role_idx++) {
        ...
        SCOPED_TRACE(testing::Message() << "ax::mojom::Role=" << ToString(role) << ", Actual: isTableWithColumns=" << is_table_with_columns
                                        << ", Expected: isTableWithColumns=" << !is_table_with_columns);

        if (roles_expected_is_table_with_columns.find(role) != roles_expected_is_table_with_columns.end()) {
