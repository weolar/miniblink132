...
bool AXTreeCombiner::Combine()
{
    ...
    combined_->tree_data.focus_id = MapId(focused_tree_id, focused_tree->tree_data.focus_id);
    combined_->tree_data.sel_is_backward = MapId(focused_tree_id, focused_tree->tree_data.sel_is_backward);
    combined_->tree_data.sel_anchor_object_id = MapId(focused_tree_id, focused_tree->tree_data.sel_anchor_object_id);
    combined_->tree_data.sel_focus_object_id = MapId(focused_tree_id, focused_tree->tree_data.sel_focus_object_id);
    combined_->tree_data.sel_anchor_offset = focused_tree->tree_data.sel_anchor_offset;
    combined_->tree_data.sel_focus_offset = focused_tree->tree_data.sel_focus_offset;
...
void AXTreeCombiner::ProcessTree(AXTreeUpdate* tree, const std::map<AXTreeID, AXTreeUpdate*>& tree_id_map)
{
    AXTreeID tree_id = tree->tree_data.tree_id;
    for (auto& node : tree->nodes) {
        AXTreeID child_tree_id = AXTreeID::FromString(node.GetStringAttribute(ax::mojom::StringAttribute::kChildTreeId));
        ...
        if (node.relative_bounds.offset_container_id > 0) {
            node.relative_bounds.offset_container_id = MapId(tree_id, node.relative_bounds.offset_container_id);
        }
