...
public:
    ...
    constexpr uint32_t experimental_flags() const
    {
        return experimental_flags_;
    }

    constexpr bool is_mode_off() const
    {
        return !flags_ && !experimental_flags_;
    }
    ...
    constexpr AXMode operator~() const
    {
        return { ~flags_, ~experimental_flags_ };
    }
...
{
    return lhs.flags() == rhs.flags() && lhs.experimental_flags() == rhs.experimental_flags();
}
...
// not present.
inline constexpr AXMode kAXModeWebContentsOnly(AXMode::kWebContents | AXMode::kInlineTextBoxes | AXMode::kScreenReader);

// Used when an AT that requires full accessibility access, such as a screen
// reader, is present.
inline constexpr AXMode kAXModeComplete(AXMode::kNativeAPIs | AXMode::kWebContents | AXMode::kInlineTextBoxes | AXMode::kScreenReader);

// Used when tools that only need autofill functionality are present.
inline constexpr AXMode kAXModeFormControls(AXMode::kNativeAPIs | AXMode::kWebContents, AXMode::kExperimentalFormControls);

// If adding a new named set of mode flags, please update BundleHistogramValue.

// For debugging, test assertions, etc.
AX_BASE_EXPORT std::ostream& operator<<(std::ostream& stream, const AXMode& mode);
