...
#define ACCESSIBILITY_TREE_UNSERIALIZE_ERROR_HISTOGRAM(enum_value) base::UmaHistogramEnumeration("Accessibility.Reliability.Tree.UnserializeError", enum_value)
...
public:
    using IntReverseRelationMap = std::map<ax::mojom::IntAttribute, std::map<AXNodeID, std::set<AXNodeID>>>;
    using IntListReverseRelationMap = std::map<ax::mojom::IntListAttribute, std::map<AXNodeID, std::set<AXNodeID>>>;
    ...
    // and the tree.
    static bool ComputeNodeIsIgnored(const AXTreeData* optional_tree_data, const AXNodeData& node_data);
    ...
    base::ObserverList<AXTreeObserver>& observers()
    {
        return observers_;
    }
    ...
    gfx::RectF RelativeToTreeBounds(
        const AXNode* node, gfx::RectF node_bounds, bool* offscreen = nullptr, bool clip_bounds = true, bool skip_container_offset = false) const;
    ...
    std::set<AXNodeID> GetReverseRelations(ax::mojom::IntAttribute attr, AXNodeID dst_id) const;
    ...
    std::set<AXNodeID> GetReverseRelations(ax::mojom::IntListAttribute attr, AXNodeID dst_id) const;
...
private:
    ...
    void RecordError(const AXTreeUpdateState& update_state, std::string new_error, bool is_fatal = false);

    AXNode* CreateNode(AXNode* parent, AXNodeID id, size_t index_in_parent, AXTreeUpdateState* update_state);
    ...
    bool ComputePendingChanges(const AXTreeUpdate& update, AXTreeUpdateState* update_state);
    ...
    bool ComputePendingChangesToNode(const AXNodeData& new_data, bool is_new_root, AXTreeUpdateState* update_state);

    // This is called from within Unserialize(), it returns true on success.
    bool UpdateNode(const AXNodeData& src, bool is_new_root, AXTreeUpdateState* update_state);

    // Notify the delegate that the subtree rooted at |node| will be
    // destroyed or reparented.
    void NotifySubtreeWillBeReparentedOrDeleted(AXNode* node, const AXTreeUpdateState* update_state);

    // Notify the delegate that |node| will be destroyed or reparented.
    void NotifyNodeWillBeReparentedOrDeleted(AXNode& node, const AXTreeUpdateState& update_state);

    // Notify the delegate that |node| and all of its descendants will be
    // destroyed. This function is called during AXTree teardown.
    void RecursivelyNotifyNodeWillBeDeletedForTreeTeardown(AXNode& node, std::set<AXNodeID>& deleted_nodes);
    ...
    void NotifyNodeHasBeenReparentedOrCreated(AXNode* node, const AXTreeUpdateState* update_state);

    // Notify the delegate that `node` will change its data attributes, including
    // its ignored state.
    void NotifyNodeAttributesWillChange(AXNode* node, AXTreeUpdateState& update_state, const AXTreeData* optional_old_tree_data, const AXNodeData& old_data,
        const AXTreeData* new_tree_data, const AXNodeData& new_data);

    // Notify the delegate that `node` will change its its ignored state.
    void NotifyNodeIgnoredStateWillChange(
        AXNode* node, const AXTreeData* optional_old_tree_data, const AXNodeData& old_data, const AXTreeData* new_tree_data, const AXNodeData& new_data);

    // Notify the delegate that `node` has changed its data attributes, including
    // its ignored state.
    void NotifyNodeAttributesHaveBeenChanged(AXNode* node, AXTreeUpdateState& update_state, const AXTreeData* optional_old_tree_data,
        const AXNodeData& old_data, const AXTreeData* new_tree_data, const AXNodeData& new_data);

    // Update maps that track which relations are pointing to |node|.
    void UpdateReverseRelations(AXNode* node, const AXNodeData& new_data, bool is_new_node = false);
    ...
    void MarkSubtreeForDestruction(AXNodeID node_id, AXTreeUpdateState* update_state);

    // Modifies |update_state| so that it knows what nodes are
    // going to be destroyed for the subtree rooted at |node|.
    void MarkNodesForDestructionRecursive(AXNodeID node_id, AXTreeUpdateState* update_state);
    ...
    void DeleteOldChildren(AXNode* node, const std::vector<AXNodeID>& new_child_ids, AXTreeUpdateState* update_state);
    ...
    bool CreateNewChildVector(AXNode* node, const std::vector<AXNodeID>& new_child_ids, std::vector<raw_ptr<AXNode, VectorExperimental>>* new_children,
    ...
    gfx::RectF RelativeToTreeBoundsInternal(
        const AXNode* node, gfx::RectF node_bounds, bool* offscreen, bool clip_bounds, bool skip_container_offset, bool allow_recursion) const;
    ...
    mutable std::unordered_map<AXNodeID, std::unique_ptr<AXTableInfo>> table_info_map_;
    ...
    void PopulateOrderedSetItemsMap(const AXNode& original_node, const AXNode* ordered_set, OrderedSetItemsMap* items_map_to_be_populated) const;
    ...
    void RecursivelyPopulateOrderedSetItemsMap(const AXNode& original_node, const AXNode* ordered_set, const AXNode* local_parent,
    ...
    void ComputeSetSizePosInSetAndCache(const AXNode& node, const AXNode* ordered_set);
    ...
    void ComputeSetSizePosInSetAndCacheHelper(const OrderedSetContent& ordered_set_content);
    ...
    // Invalidated every time the tree is updated.
    mutable std::unordered_map<AXNodeID, NodeSetSizePosInSetInfo> node_set_size_pos_in_set_info_map_;
...
public:
    ...
    ScopedTreeUpdateInProgressStateSetter(const ScopedTreeUpdateInProgressStateSetter&) = delete;
    ScopedTreeUpdateInProgressStateSetter& operator=(const ScopedTreeUpdateInProgressStateSetter&) = delete;
