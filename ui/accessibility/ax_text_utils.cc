...
base::i18n::BreakIterator::BreakType ICUBreakTypeForBoundaryType(ax::mojom::TextBoundary boundary)
{
    switch (boundary) {
...
size_t FindAccessibleTextBoundary(const std::u16string& text, const std::vector<int>& line_breaks, ax::mojom::TextBoundary boundary, size_t start_offset,
    ax::mojom::MoveDirection direction, ax::mojom::TextAffinity affinity)
{
    ...
    base::i18n::BreakIterator::BreakType break_type = ICUBreakTypeForBoundaryType(boundary);
    base::i18n::BreakIterator break_iter(text, break_type);
    if (boundary == ax::mojom::TextBoundary::kCharacter || boundary == ax::mojom::TextBoundary::kSentenceStart
        || boundary == ax::mojom::TextBoundary::kWordStart || boundary == ax::mojom::TextBoundary::kWordStartOrEnd) {
        if (!break_iter.Init())
            return start_offset;
    }
    ...
    if (boundary == ax::mojom::TextBoundary::kLineStart) {
        if (direction == ax::mojom::MoveDirection::kForward) {
            for (int line_break : line_breaks) {
                size_t clamped_line_break = static_cast<size_t>(std::max(0, line_break));
                if ((affinity == ax::mojom::TextAffinity::kDownstream && clamped_line_break > start_offset)
                    || (affinity == ax::mojom::TextAffinity::kUpstream && clamped_line_break >= start_offset)) {
            ...
            for (size_t j = line_breaks.size(); j != 0; --j) {
                ...
                if ((affinity == ax::mojom::TextAffinity::kDownstream && line_break <= start_offset)
                    || (affinity == ax::mojom::TextAffinity::kUpstream && line_break < start_offset)) {
                    return line_break;
                }
            }
            return 0;
    ...
    for (;;) {
        ...
        case ax::mojom::TextBoundary::kCharacter:
            if (break_iter.IsGraphemeBoundary(result)) {
                ...
                if (direction == ax::mojom::MoveDirection::kBackward || result != start_offset)
                    return result;
            }
            break;
        case ax::mojom::TextBoundary::kWordStart:
        ...
        case ax::mojom::TextBoundary::kWordStartOrEnd:
            if (break_iter.IsStartOfWord(result)) {
                ...
                DCHECK_NE(direction, ax::mojom::MoveDirection::kNone);
                if (direction == ax::mojom::MoveDirection::kForward || result != start_offset)
                    return result;
            }
            break;
        case ax::mojom::TextBoundary::kSentenceStart:
            if (break_iter.IsSentenceBoundary(result)) {
                ...
                if (direction == ax::mojom::MoveDirection::kBackward || result != start_offset) {
                    ...
                    while (result < text_size && base::IsUnicodeWhitespace(text[result]))
                        result++;
                    return result;
                }
            }
...
std::vector<int> GetWordStartOffsets(const std::u16string& text)
{
    ...
    while (iter.Advance()) {
        if (!iter.IsWord())
            continue;
        word_starts.push_back(base::checked_cast<int>(iter.prev()) /* start index */);
    }
    return word_starts;
}
...
std::vector<int> GetSentenceStartOffsets(const std::u16string& text)
{
    ...
    base::i18n::BreakIterator iter(text, base::i18n::BreakIterator::BREAK_SENTENCE);
    if (!iter.Init())
        return sentence_starts;
    // iter.Advance() returns false if we've run past end of the text.
    while (iter.Advance()) {
        sentence_starts.push_back(base::checked_cast<int>(iter.prev()) /* start index */);
    }
    return sentence_starts;
}
...
std::vector<int> GetSentenceEndOffsets(const std::u16string& text)
{
    ...
    // iter.Advance() returns false if we've run past end of the text.
    while (iter.Advance()) {
        sentence_ends.push_back(base::checked_cast<int>(iter.pos()) /* end index */);
    }
    return sentence_ends;
}
} // namespace ui
