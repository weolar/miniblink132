...
bool isRowlessTable(AXNode* node)
{
    for (auto iter = node->UnignoredChildrenBegin(); iter != node->UnignoredChildrenEnd(); ++iter) {
        AXNode* child = iter.get();
        if (child->GetRole() == ax::mojom::Role::kGenericContainer || child->GetRole() == ax::mojom::Role::kGroup
            || child->GetRole() == ax::mojom::Role::kRowGroup) {
            return isRowlessTable(child);
        } else if (IsTableRow(child->GetRole())) {
            return false;
        } else if (child->GetRole() == ax::mojom::Role::kCell || child->GetRole() == ax::mojom::Role::kGridCell) {
...
// present.
void FindRows(AXNode* node, std::vector<raw_ptr<AXNode, VectorExperimental>>* row_node_list, AXNodeID& caption_node_id)
{
    for (auto iter = node->UnignoredChildrenBegin(); iter != node->UnignoredChildrenEnd(); ++iter) {
        AXNode* child = iter.get();
        if (child->GetRole() == ax::mojom::Role::kGenericContainer || child->GetRole() == ax::mojom::Role::kGroup
...
// 2-dimensional array.
void FindCells(std::vector<raw_ptr<AXNode, VectorExperimental>>* row_node_list, std::vector<std::vector<AXNode*>>* cell_nodes_per_row)
{
...
//    <Row>
// </Grid>
void FindCellsForRowlessTable(AXNode* grid_node, std::vector<std::vector<AXNode*>>* cell_nodes_per_row)
{
    int current_row = -1;
    int current_index = -1;
    base::queue<AXNode*> child_queue;
    for (auto iter = grid_node->UnignoredChildrenBegin(); iter != grid_node->UnignoredChildrenEnd(); ++iter) {
        ...
        while (!child_queue.empty()) {
            ...
            if (child->GetRole() == ax::mojom::Role::kGenericContainer || child->GetRole() == ax::mojom::Role::kGroup) {
                ...
                for (auto container_itr = child->UnignoredChildrenBegin(); container_itr != child->UnignoredChildrenEnd(); ++container_itr) {
                    child_queue.push(container_itr.get());
                }
                continue;
            } else if (IsCellOrTableHeader(child->GetRole())) {
                const int rowIndex = child->GetIntAttribute(ax::mojom::IntAttribute::kTableCellRowIndex);
                CHECK_GE(rowIndex, 0);
...
bool AXTableInfo::Update()
{
    ...
    // attribute.
    aria_row_count = int { table_node_->GetIntAttribute(IntAttribute::kAriaRowCount) };
    aria_col_count = int { table_node_->GetIntAttribute(IntAttribute::kAriaColumnCount) };
...
void AXTableInfo::BuildCellData(AXNode* cell, AXNode* row_or_first_cell, CellBuildState& state)
{
    ...
    cell_data.row_index = GetSizeTAttribute(*cell, IntAttribute::kTableCellRowIndex);
    cell_data.row_span = GetSizeTAttribute(*cell, IntAttribute::kTableCellRowSpan);
    cell_data.aria_row_index = GetSizeTAttribute(*cell, IntAttribute::kAriaCellRowIndex);
    cell_data.col_index = GetSizeTAttribute(*cell, IntAttribute::kTableCellColumnIndex);
    cell_data.aria_col_index = GetSizeTAttribute(*cell, IntAttribute::kAriaCellColumnIndex);
    cell_data.col_span = GetSizeTAttribute(*cell, IntAttribute::kTableCellColumnSpan);
    ...
    state.spanned_col_index = std::max(state.spanned_col_index, cell_data.col_index);
    ...
    if (state.is_first_cell_in_row) {
        ...
        if (!cell_data.aria_row_index) {
            cell_data.aria_row_index = GetSizeTAttribute(*row_or_first_cell, IntAttribute::kAriaCellRowIndex);
        }
        if (!cell_data.aria_col_index) {
            cell_data.aria_col_index = GetSizeTAttribute(*row_or_first_cell, IntAttribute::kAriaCellColumnIndex);
        }
        cell_data.aria_row_index = std::max(cell_data.aria_row_index, state.current_aria_row_index);
    ...
    while (true) {
        ...
        if (row_it == incremental_row_col_map_.end()) {
            ...
            if (col_it == row_it->second.end()) {
                ...
                state.spanned_col_index = spanned_cell_data.col_index + spanned_cell_data.col_span;
    ...
    for (size_t r = cell_data.row_index; r < (cell_data.row_index + cell_data.row_span); r++) {
        for (size_t c = cell_data.col_index; c < (cell_data.col_index + cell_data.col_span); c++) {
            incremental_row_col_map_[r][c] = cell_data;
        }
    }

    // Ensure the ARIA col index is incrementing.
    cell_data.aria_col_index = std::max(cell_data.aria_col_index, state.current_aria_col_index);
    ...
    if (aria_row_count != ax::mojom::kUnknownAriaColumnOrRowCount) {
        aria_row_count = std::max((aria_row_count), static_cast<int>(state.current_aria_row_index + cell_data.row_span - 1));
    }
    if (aria_col_count != ax::mojom::kUnknownAriaColumnOrRowCount) {
        aria_col_count = std::max((aria_col_count), static_cast<int>(state.current_aria_col_index + cell_data.col_span - 1));
    }
...
void AXTableInfo::BuildCellDataVectorFromCellNodes(const std::vector<std::vector<AXNode*>>& cell_nodes_per_row)
{
    // Iterate over the cells and build up an array of CellData
    // entries, one for each cell. Compute the actual row and column
    ...
    for (auto& cells_in_row : cell_nodes_per_row) {
        ...
        // the source tree.
        state.current_row_index = GetSizeTAttribute(*first_cell_node, ax::mojom::IntAttribute::kTableCellRowIndex);
...
void AXTableInfo::BuildCellDataVectorFromRowAndCellNodes(
    const std::vector<raw_ptr<AXNode, VectorExperimental>>& row_node_list, const std::vector<std::vector<AXNode*>>& cell_nodes_per_row)
{
    // Iterate over the cells and build up an array of CellData
    // entries, one for each cell. Compute the actual row and column
    // indices for each cell by taking the specified row and column
    ...
    for (size_t i = 0; i < cell_nodes_per_row.size(); i++) {
        ...
        // Make sure the row index is always at least as high as the one reported by
        // the source tree.
        row_id_to_index[row_node->id()] = std::max(next_row_index, GetSizeTAttribute(*row_node, IntAttribute::kTableRowIndex));
...
void AXTableInfo::UpdateExtraMacNodes()
{
    ...
    for (size_t i = 0; i < col_count; i++) {
        ...
        changes.emplace_back(new_extra_mac_nodes[i], AXTreeObserver::ChangeType::NODE_CREATED);
    }
    new_extra_mac_nodes.push_back(CreateExtraMacTableHeaderNode());
    changes.emplace_back(new_extra_mac_nodes[col_count], AXTreeObserver::ChangeType::NODE_CREATED);

    {
        ...
        data.AddIntListAttribute(ax::mojom::IntListAttribute::kIndirectChildIds, all_col_headers);
...
AXNode* AXTableInfo::CreateExtraMacColumnNode(size_t col_index)
{
    ...
    int32_t unignored_index_in_parent = col_index + table_node_->GetUnignoredChildCount();
    AXNode* node = new AXNode(tree_, table_node_, id, index_in_parent, unignored_index_in_parent);
    AXNodeData data;
    data.id = id;
...
AXNode* AXTableInfo::CreateExtraMacTableHeaderNode()
{
    ...
    int32_t unignored_index_in_parent = col_count + table_node_->GetUnignoredChildCount();
    AXNode* node = new AXNode(tree_, table_node_, id, index_in_parent, unignored_index_in_parent);
    AXNodeData data;
    data.id = id;
...
void AXTableInfo::UpdateExtraMacColumnNodeAttributes(size_t col_index)
{
    ...
    if (!col_headers[col_index].empty()) {
        data.AddIntAttribute(IntAttribute::kTableColumnHeaderId, col_headers[col_index][0]);
    }
    ...
    data.AddIntListAttribute(ax::mojom::IntListAttribute::kIndirectChildIds, col_nodes);
...
void AXTableInfo::ClearExtraMacNodes()
{
    ...
    for (AXTreeObserver& observer : tree_->observers()) {
        observer.OnAtomicUpdateFinished(tree_, /* root_changed= */ false, { { table_node_, AXTreeObserver::ChangeType::NODE_CHANGED } });
    }
}
