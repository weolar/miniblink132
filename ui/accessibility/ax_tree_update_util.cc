...
bool MergeAXTreeUpdates(const std::vector<AXTreeUpdate>& src, std::vector<AXTreeUpdate>* dst)
{
    SCOPED_UMA_HISTOGRAM_TIMER_MICROS("Accessibility.Performance.MergeAXTreeUpdates");
    ...
    for (size_t i = 1; i < src.size(); i++) {
        if (AXTreeUpdatesCanBeMerged(src[i - 1], src[i])) {
            ...
            if (features::IsUseMoveNotCopyInMergeTreeUpdateEnabled()) {
                std::vector<AXNodeData>& src_nodes = const_cast<std::vector<AXNodeData>&>(src[i].nodes);
