...
{
    return ax_tree_id != AXTreeIDUnknown() ? GetMap().GetManager(ax_tree_id) : nullptr;
}

// static
AXTreeManager* AXTreeManager::ForChildTree(const AXNode& parent_node)
{
    if (!parent_node.HasStringAttribute(ax::mojom::StringAttribute::kChildTreeId)) {
        return nullptr;
    }

    AXTreeID child_tree_id = AXTreeID::FromString(parent_node.GetStringAttribute(ax::mojom::StringAttribute::kChildTreeId));
    ...
    // the browser process.
    DCHECK(!child_tree_manager || !child_tree_manager->GetParentNodeFromParentTree()
        || child_tree_manager->GetParentNodeFromParentTree()->id() == parent_node.id());
...
{
    static base::NoDestructor<base::RepeatingClosure> g_focus_change_callback_for_testing;
    return *g_focus_change_callback_for_testing;
}

void AXTreeManager::SetFocusChangeCallbackForTesting(base::RepeatingClosure callback)
{
...
AXNode* AXTreeManager::RetargetForEvents(AXNode* node, RetargetEventType type) const
{
    return node;
}
...
void AXTreeManager::Initialize(const AXTreeUpdate& initial_tree)
{
    if (!ax_tree()->Unserialize(initial_tree)) {
        LOG(FATAL) << "No recovery is possible if the initial tree is broken: " << ax_tree()->error()
                   << ", AXTreeUpdate info: " << initial_tree.ToString().substr(0, 500);
    }
}
...
void AXTreeManager::SetLastFocusedNode(AXNode* node)
{
    ...
    static auto* const ax_crash_key_focus = base::debug::AllocateCrashKeyString("ax_focus", base::debug::CrashKeySize::Size256);
#endif
    static auto* const ax_crash_key_focus_top_frame = base::debug::AllocateCrashKeyString("ax_focus_top_frame", base::debug::CrashKeySize::Size256);
    static auto* const ax_crash_key_focus_frame = base::debug::AllocateCrashKeyString("ax_focus_frame", base::debug::CrashKeySize::Size256);
    if (node) {
        ...
        if (node->GetManager()->GetTreeID() != last_focused_node_tree_id_) {
            if (node->GetManager() && node->GetManager()->GetRootManager()) {
                ...
                base::debug::SetCrashKeyString(ax_crash_key_focus_top_frame, node_info_top_frame.str());
                if (!node->GetManager()->IsRoot()) {
                    ...
                    base::debug::SetCrashKeyString(ax_crash_key_focus_frame, node_info_frame.str());
                }
            }
        }
...
AXNode* AXTreeManager::GetLastFocusedNode()
{
    if (last_focused_node_id_) {
        DCHECK(last_focused_node_tree_id_);
        DCHECK(last_focused_node_tree_id_ != AXTreeIDUnknown());
        if (AXTreeManager* last_focused_manager = FromID(last_focused_node_tree_id_.value())) {
...
AXTreeManager::~AXTreeManager()
{
    ...
    if (HasValidTreeID()) {
        ...
        if (last_focused_node_tree_id_ && GetTreeID() == *last_focused_node_tree_id_) {
...
{
    if (!tree) {
        NOTREACHED() << "Attempting to set a new tree, but no tree has been provided.";
    }
...
std::unique_ptr<AXTree> AXTreeManager::SetTree(const AXTreeUpdate& initial_state)
{
    return SetTree(std::make_unique<AXTree>(initial_state));
}

void AXTreeManager::OnTreeDataChanged(AXTree* tree, const AXTreeData& old_data, const AXTreeData& new_data)
{
    ...
    if (last_focused_node_tree_id_ != AXTreeIDUnknown() && last_focused_node_tree_id_ == old_data.tree_id) {
        SetLastFocusedNode(nullptr);
    }
...
void AXTreeManager::OnAtomicUpdateFinished(AXTree* tree, bool root_changed, const std::vector<AXTreeObserver::Change>& changes)
{
    DCHECK_EQ(ax_tree(), tree);
    if (root_changed)
        connected_to_parent_tree_node_ = false;
...
AXNode* AXTreeManager::GetParentNodeFromParentTree() const
{
    ...
    std::set<int32_t> host_node_ids = parent_manager->ax_tree()->GetNodeIdsForChildTreeId(GetTreeID());
    if (host_node_ids.empty()) {
        // Parent tree has host node but the change has not yet been serialized.
        return nullptr;
    }

    CHECK_EQ(host_node_ids.size(), 1U) << "Multiple nodes cannot claim the same child tree ID.";

    AXNode* parent_node = parent_manager->GetNode(*(host_node_ids.begin()));
    DCHECK(parent_node);
    DCHECK_EQ(GetTreeID(), AXTreeID::FromString(parent_node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeId)))
...
void AXTreeManager::ParentConnectionChanged(AXNode* parent)
{
    ...
    parent = parent_manager->RetargetForEvents(parent, RetargetEventType::RetargetEventTypeGenerated);
    DCHECK(parent) << "RetargetForEvents shouldn't return a "
                      "null pointer when |parent| is not null.";
    parent_manager->FireGeneratedEvent(AXEventGenerator::Event::CHILDREN_CHANGED, parent);
}
