...
public:
    ...
    static AXNode* GetNodeFromTree(const AXTreeID& tree_id, const AXNodeID& node_id);
    ...
    bool AttachChildTree(const AXNodeID& host_node_id, AXTreeManagerBase& child_manager);
    ...
    std::optional<AXTreeManagerBase> AttachChildTree(const AXNodeID& host_node_id, const AXTreeUpdate& initial_state);
    std::optional<AXTreeManagerBase> AttachChildTree(AXNode& host_node, const AXTreeUpdate& initial_state);
...
private:
    static std::unordered_map<AXTreeID, AXTreeManagerBase*, AXTreeIDHash>& GetTreeManagerMapInstance();
