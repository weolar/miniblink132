...
AXNode::AXNode(AXTree* tree, AXNode* parent, AXNodeID id, size_t index_in_parent, size_t unignored_index_in_parent)
    : tree_(tree)
...
const std::vector<raw_ptr<AXNode, VectorExperimental>>& AXNode::GetAllChildren() const
{
...
size_t AXNode::GetUnignoredChildCountCrossingTreeBoundary() const
{
    ...
    const AXTreeManager* child_tree_manager = AXTreeManager::ForChildTree(*this);
    if (child_tree_manager) {
        DCHECK_EQ(unignored_child_count_, 0u) << "A node cannot be hosting both a child tree and other nodes as "
                                                 "children.";
...
AXNode* AXNode::GetChildAtIndexCrossingTreeBoundary(size_t index) const
{
    ...
    const AXTreeManager* child_tree_manager = AXTreeManager::ForChildTree(*this);
    if (child_tree_manager) {
        DCHECK_EQ(index, 0u) << "A node cannot be hosting both a child tree and other nodes as "
                                "children.";
...
AXNode* AXNode::GetUnignoredChildAtIndexCrossingTreeBoundary(size_t index) const
{
    // TODO(nektar): Should DCHECK that this node is not ignored.
    DCHECK(!tree_->GetTreeUpdateInProgressState());
...
AXNode* AXNode::GetDeepestFirstDescendantCrossingTreeBoundary() const
{
    ...
    while (deepest_descendant->GetChildCountCrossingTreeBoundary()) {
        deepest_descendant = deepest_descendant->GetFirstChildCrossingTreeBoundary();
        DCHECK(deepest_descendant);
    }
...
AXNode* AXNode::GetDeepestFirstUnignoredDescendantCrossingTreeBoundary() const
{
    ...
    while (deepest_descendant->GetUnignoredChildCountCrossingTreeBoundary()) {
        deepest_descendant = deepest_descendant->GetFirstUnignoredChildCrossingTreeBoundary();
        DCHECK(deepest_descendant);
    }
...
AXNode* AXNode::GetDeepestLastUnignoredDescendantCrossingTreeBoundary() const
{
    ...
    DCHECK(deepest_descendant);
    while (deepest_descendant->GetUnignoredChildCountCrossingTreeBoundary()) {
        deepest_descendant = deepest_descendant->GetLastUnignoredChildCrossingTreeBoundary();
        DCHECK(deepest_descendant);
    }
...
AXNode* AXNode::GetNextSibling() const
{
    ...
    if (!parent)
        return nullptr;
    DCHECK(parent || !GetIndexInParent()) << "Root nodes lack a parent. Their index_in_parent should be 0.";
...
{
    DCHECK(!tree_->GetTreeUpdateInProgressState());
    DCHECK(GetParent() || !GetIndexInParent()) << "Root nodes lack a parent. Their index_in_parent should be 0.";
...
AXNode::AllChildCrossingTreeBoundaryIterator AXNode::AllChildrenCrossingTreeBoundaryBegin() const
{
    DCHECK(!tree_->GetTreeUpdateInProgressState());
    return AllChildCrossingTreeBoundaryIterator(this, GetFirstChildCrossingTreeBoundary());
}

AXNode::AllChildCrossingTreeBoundaryIterator AXNode::AllChildrenCrossingTreeBoundaryEnd() const
{
    DCHECK(!tree_->GetTreeUpdateInProgressState());
...
AXNode::UnignoredChildCrossingTreeBoundaryIterator AXNode::UnignoredChildrenCrossingTreeBoundaryBegin() const
{
    DCHECK(!tree_->GetTreeUpdateInProgressState());
    return UnignoredChildCrossingTreeBoundaryIterator(this, GetFirstUnignoredChildCrossingTreeBoundary());
}

AXNode::UnignoredChildCrossingTreeBoundaryIterator AXNode::UnignoredChildrenCrossingTreeBoundaryEnd() const
{
    DCHECK(!tree_->GetTreeUpdateInProgressState());
...
AXNode* AXNode::GetLowestCommonAncestor(const AXNode& other)
{
    ...
    base::stack<AXNode*> our_ancestors = GetAncestorsCrossingTreeBoundaryAsStack();
    base::stack<AXNode*> other_ancestors = other.GetAncestorsCrossingTreeBoundaryAsStack();
...
std::optional<int> AXNode::CompareTo(const AXNode& other) const
{
    ...
    size_t other_uncommon_ancestor_index = other_ancestors.top()->GetIndexInParent();
    DCHECK_NE(this_uncommon_ancestor_index, other_uncommon_ancestor_index) << "Deepest uncommon ancestors should truly be uncommon, i.e. not be the "
                                                                              "same node.";
...
bool AXNode::IsLineBreak() const
{
    ...
    return GetRole() == ax::mojom::Role::kLineBreak
        || (GetRole() == ax::mojom::Role::kInlineTextBox && GetBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject));
}
...
void AXNode::SetLocation(AXNodeID offset_container_id, const gfx::RectF& location, gfx::Transform* transform)
{
    data_.relative_bounds.offset_container_id = offset_container_id;
    data_.relative_bounds.bounds = location;
    if (transform) {
        data_.relative_bounds.transform = std::make_unique<gfx::Transform>(*transform);
    } else {
...
void AXNode::SwapChildren(std::vector<raw_ptr<AXNode, VectorExperimental>>* children)
{
...
AXSelection AXNode::GetUnignoredSelection() const
{
    ...
    if (anchor && !anchor->IsLeaf()) {
        ...
        if (static_cast<size_t>(selection.anchor_offset) < anchor->GetChildCount()) {
            const AXNode* anchor_child = anchor->GetChildAtIndex(selection.anchor_offset);
            DCHECK(anchor_child);
            selection.anchor_offset = static_cast<int>(anchor_child->GetUnignoredIndexInParent());
        } else {
            selection.anchor_offset = static_cast<int>(anchor->GetUnignoredChildCount());
        }
    }
    ...
    if (focus && !focus->IsLeaf()) {
        ...
        if (static_cast<size_t>(selection.focus_offset) < focus->GetChildCount()) {
            const AXNode* focus_child = focus->GetChildAtIndex(selection.focus_offset);
            DCHECK(focus_child);
            selection.focus_offset = static_cast<int>(focus_child->GetUnignoredIndexInParent());
        } else {
            selection.focus_offset = static_cast<int>(focus->GetUnignoredChildCount());
        }
    }
    return selection;
}
...
bool AXNode::CanComputeIntAttribute(ax::mojom::IntAttribute attribute) const
{
    ...
    if (attribute != ax::mojom::IntAttribute::kNextOnLineId && attribute != ax::mojom::IntAttribute::kPreviousOnLineId) {
        return false;
    }
    ...
    if (this == GetParent()->GetFirstChild() && attribute == ax::mojom::IntAttribute::kPreviousOnLineId) {
        return GetParent()->data().HasIntAttribute(attribute);
    }

    if (this == GetParent()->GetLastChild() && attribute == ax::mojom::IntAttribute::kNextOnLineId) {
        return GetParent()->data().HasIntAttribute(attribute);
    }
...
bool AXNode::CanComputeStringAttribute(ax::mojom::StringAttribute attribute) const
{
    switch (attribute) {
    ...
    case ax::mojom::StringAttribute::kName:
        ...
        return ::features::IsAccessibilityPruneRedundantInlineTextEnabled() && data().role == ax::mojom::Role::kInlineTextBox
            && data().GetNameFrom() == ax::mojom::NameFrom::kContents && GetParent() && GetParent()->data().GetNameFrom() == ax::mojom::NameFrom::kContents
            && GetParent()->data().HasStringAttribute(ax::mojom::StringAttribute::kName);
...
const std::string& AXNode::GetStringAttribute(ax::mojom::StringAttribute attribute) const
{
    if (data().HasStringAttribute(attribute)) {
...
std::u16string AXNode::GetString16Attribute(ax::mojom::StringAttribute attribute) const
{
...
bool AXNode::HasInheritedStringAttribute(ax::mojom::StringAttribute attribute) const
{
    for (const AXNode* current_node = this; current_node; current_node = current_node->GetParent()) {
...
const std::string& AXNode::GetInheritedStringAttribute(ax::mojom::StringAttribute attribute) const
{
    for (const AXNode* current_node = this; current_node; current_node = current_node->GetParent()) {
        if (current_node->HasStringAttribute(attribute))
...
std::u16string AXNode::GetInheritedString16Attribute(ax::mojom::StringAttribute attribute) const
{
...
bool AXNode::CanComputeIntListAttribute(ax::mojom::IntListAttribute attribute) const
{
    switch (attribute) {
...
const std::vector<int32_t>& AXNode::GetIntListAttribute(ax::mojom::IntListAttribute attribute) const
{
    if (data().HasIntListAttribute(attribute)) {
...
const std::u16string& AXNode::GetHypertext() const
{
    ...
    if (IsLeaf() || IsChildOfLeaf()) {
        ...
        for (auto iter = first; iter != UnignoredChildrenCrossingTreeBoundaryEnd(); ++iter) {
            ...
            if (iter->IsText()) {
                ...
                auto inserted = hypertext_.hypertext_offset_to_hyperlink_child_index.emplace(character_offset, static_cast<int>(std::distance(first, iter)));
                DCHECK(inserted.second) << "An embedded object at " << character_offset << " has already been encountered.";
...
const std::map<int, int>& AXNode::GetHypertextOffsetToHyperlinkChildIndex() const
{
    // TODO(nektar): Introduce proper caching of hypertext via
    // `AXHypertext::needs_update`.
...
gfx::RectF AXNode::GetTextContentRangeBoundsUTF16(int start_offset, int end_offset) const
{
    DCHECK(!tree_->GetTreeUpdateInProgressState());
    DCHECK_LE(start_offset, end_offset) << "Invalid `start_offset` and `end_offset`.\n" << start_offset << ' ' << end_offset << "\nin\n" << *this;
    ...
    const std::vector<int32_t>& character_offsets = GetIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets);
    int character_offsets_length = base::checked_cast<int>(character_offsets.size());
    ...
    // itself.
    int start_pixel_offset = start_offset > 0 ? character_offsets[base::checked_cast<size_t>(start_offset - 1)] : 0;
    int end_pixel_offset = end_offset > 0 ? character_offsets[base::checked_cast<size_t>(end_offset - 1)] : 0;
    int max_pixel_offset = character_offsets_length > 0 ? character_offsets[character_offsets_length - 1] : 0;
    const gfx::RectF& node_bounds = data().relative_bounds.bounds;

    gfx::RectF out_bounds;
    switch (static_cast<ax::mojom::WritingDirection>(GetIntAttribute(ax::mojom::IntAttribute::kTextDirection))) {
    case ax::mojom::WritingDirection::kNone:
    case ax::mojom::WritingDirection::kLtr:
        out_bounds = gfx::RectF(start_pixel_offset, 0, end_pixel_offset - start_pixel_offset, node_bounds.height());
        break;
    ...
    case ax::mojom::WritingDirection::kTtb:
        out_bounds = gfx::RectF(0, start_pixel_offset, node_bounds.width(), end_pixel_offset - start_pixel_offset);
        break;
...
std::ostream& operator<<(std::ostream& stream, const AXNode& node)
{
    ...
    if (node.GetUnignoredChildCountCrossingTreeBoundary()) {
        ...
        for (auto it = node.UnignoredChildrenBegin(); it != node.UnignoredChildrenEnd(); ++it) {
            if (needs_comma) {
                stream << ",";
            } else {
                needs_comma = true;
...
AXNode* AXNode::GetTableCellFromCoords(int row_index, int col_index) const
{
    ...
    if (row_index < 0 || static_cast<size_t>(row_index) >= table_info->row_count || col_index < 0 || static_cast<size_t>(col_index) >= table_info->col_count) {
        return nullptr;
    }

    return tree_->GetFromId(table_info->cell_ids[static_cast<size_t>(row_index)][static_cast<size_t>(col_index)]);
}

AXNode* AXNode::GetTableCellFromAriaCoords(int aria_row_index, int aria_col_index) const
{
    DCHECK(!tree_->GetTreeUpdateInProgressState());
    const AXTableInfo* table_info = GetAncestorTableInfo();
    if (!table_info) {
    ...
    if (aria_row_index < 1 || aria_row_index > table_info->aria_row_count || aria_col_index < 1 || aria_col_index > table_info->aria_col_count) {
        return nullptr;
    }
    ...
    for (size_t row = 0; row < table_info->row_count; ++row) {
        for (size_t col = 0; col < table_info->col_count; ++col) {
            ...
            DCHECK(*current_aria_row == aria_row_index && *current_aria_col == aria_col_index);
            return node;
        }
    }
    return nullptr;
...
std::vector<AXNodeID> AXNode::GetTableColHeaderNodeIds() const
{
    ...
    for (std::vector<AXNodeID> col_headers_at_index : table_info->col_headers) {
        col_header_ids.insert(col_header_ids.end(), col_headers_at_index.begin(), col_headers_at_index.end());
    }
...
std::vector<AXNodeID> AXNode::GetTableColHeaderNodeIds(int col_index) const
{
    ...
    return std::vector<AXNodeID>(table_info->col_headers[static_cast<size_t>(col_index)]);
}
...
std::vector<AXNodeID> AXNode::GetTableRowHeaderNodeIds(int row_index) const
{
    ...
    return std::vector<AXNodeID>(table_info->row_headers[static_cast<size_t>(row_index)]);
}
...
const std::vector<raw_ptr<AXNode, VectorExperimental>>* AXNode::GetExtraMacNodes() const
{
    DCHECK(!tree_->GetTreeUpdateInProgressState());
...
bool AXNode::IsGenerated() const
{
    ...
    bool is_extra_mac_node_role = GetRole() == ax::mojom::Role::kColumn || GetRole() == ax::mojom::Role::kTableHeaderContainer;
...
std::optional<int> AXNode::GetTableCellColSpan() const
{
    ...
    if (col_span || HasIntAttribute(ax::mojom::IntAttribute::kTableCellColumnSpan)) {
        return col_span;
    }
    return 1;
}
...
std::optional<int> AXNode::GetTableCellAriaColIndex() const
{
    ...
    int aria_col_index = static_cast<int>(table_info->cell_data_vector[*index].aria_col_index);
    // |aria-colindex| attribute is one-based, value less than 1 is invalid.
    // https://www.w3.org/TR/wai-aria-1.2/#aria-colindex
    return (aria_col_index > 0) ? std::optional<int>(aria_col_index) : std::nullopt;
}
...
std::optional<int> AXNode::GetTableCellAriaRowIndex() const
{
    ...
    int aria_row_index = static_cast<int>(table_info->cell_data_vector[*index].aria_row_index);
    // |aria-rowindex| attribute is one-based, value less than 1 is invalid.
    // https://www.w3.org/TR/wai-aria-1.2/#aria-rowindex
    return (aria_row_index > 0) ? std::optional<int>(aria_row_index) : std::nullopt;
}
...
bool AXNode::IsCellOrHeaderOfAriaGrid() const
{
    ...
    return node->GetRole() == ax::mojom::Role::kGrid || node->GetRole() == ax::mojom::Role::kTreeGrid;
}
...
void AXNode::IdVectorToNodeVector(const std::vector<AXNodeID>& ids, std::vector<AXNode*>* nodes) const
{
    for (AXNodeID id : ids) {
...
{
    int hierarchical_level = GetIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel);
...
bool AXNode::SetRoleMatchesItemRole(const AXNode* ordered_set) const
{
    ...
    // ordered set items.
    if (IsRowInTreeGrid(ordered_set) || item_role == ax::mojom::Role::kDisclosureTriangle || item_role == ax::mojom::Role::kDisclosureTriangleGrouped) {
        return true;
    }
    ...
    case ax::mojom::Role::kGroup:
        return item_role == ax::mojom::Role::kComment || item_role == ax::mojom::Role::kListItem || item_role == ax::mojom::Role::kMenuItem
            || item_role == ax::mojom::Role::kMenuItemRadio || item_role == ax::mojom::Role::kMenuItemCheckBox || item_role == ax::mojom::Role::kListBoxOption
            || item_role == ax::mojom::Role::kTreeItem;
    case ax::mojom::Role::kMenu:
        return item_role == ax::mojom::Role::kMenuItem || item_role == ax::mojom::Role::kMenuItemRadio || item_role == ax::mojom::Role::kMenuItemCheckBox;
    ...
    case ax::mojom::Role::kMenuListPopup:
        return item_role == ax::mojom::Role::kMenuListOption || item_role == ax::mojom::Role::kMenuItem || item_role == ax::mojom::Role::kMenuItemRadio
...
{
    return IsIgnored() || IsEmbeddedGroup() || GetRole() == ax::mojom::Role::kDetails || GetRole() == ax::mojom::Role::kLabelText
        || GetRole() == ax::mojom::Role::kListItem || GetRole() == ax::mojom::Role::kGenericContainer || GetRole() == ax::mojom::Role::kScrollView
...
std::string AXNode::GetTextForRangeValue() const
{
    ...
    std::string range_value = GetStringAttribute(ax::mojom::StringAttribute::kValue);
    if (range_value.empty()) {
        float numeric_value = GetFloatAttribute(ax::mojom::FloatAttribute::kValueForRange);
        if (numeric_value != AXNode::kDefaultFloatValue || HasFloatAttribute(ax::mojom::FloatAttribute::kValueForRange)) {
...
std::string AXNode::GetValueForColorWell() const
{
    ...
    unsigned int color = static_cast<unsigned int>(GetIntAttribute(ax::mojom::IntAttribute::kColorValue));

    unsigned int red = SkColorGetR(color);
    unsigned int green = SkColorGetG(color);
    unsigned int blue = SkColorGetB(color);
    return base::StringPrintf("%d%% red %d%% green %d%% blue", red * 100 / 255, green * 100 / 255, blue * 100 / 255);
}
...
bool AXNode::IsIgnoredForTextNavigation() const
{
    ...
    if (GetRole() == ax::mojom::Role::kGenericContainer && !GetUnignoredChildCount() && !HasState(ax::mojom::State::kEditable)) {
        return true;
    }
...
bool AXNode::IsChildOfLeaf() const
{
    ...
    for (const AXNode* ancestor = GetUnignoredParent(); ancestor; ancestor = ancestor->GetUnignoredParent()) {
        if (ancestor->IsLeaf()) {
            return true;
        }
    }
...
bool AXNode::IsLeaf() const
{
    ...
    case ax::mojom::Role::kImage: {
        ...
        // ARIA-based images.
        const std::string role = GetStringAttribute(ax::mojom::StringAttribute::kRole);
...
{
    return HasState(ax::mojom::State::kFocusable) || IsLikelyARIAActiveDescendant();
}
...
bool AXNode::IsLikelyARIAActiveDescendant() const
{
    ...
    if (IsInvisibleOrIgnored() || GetIntAttribute(ax::mojom::IntAttribute::kRestriction) == static_cast<int>(ax::mojom::Restriction::kDisabled)) {
        return false;
    }
    ...
    for (AXNode* ancestor_node = GetUnignoredParent(); ancestor_node; ancestor_node = ancestor_node->GetUnignoredParent()) {
        // Check for an ancestor with aria-activedescendant.
        if (ancestor_node->HasIntAttribute(ax::mojom::IntAttribute::kActivedescendantId)) {
            return true;
        }
        ...
        if (ui::IsComboBoxContainer(ancestor_node->GetRole())) {
            std::set<AXNodeID> nodes_that_control_this_list = tree()->GetReverseRelations(ax::mojom::IntListAttribute::kControlsIds, ancestor_node->id());
            for (AXNodeID id : nodes_that_control_this_list) {
                if (AXNode* node = tree()->GetFromId(id)) {
                    if (ui::IsTextField(node->GetRole())) {
                        return node->HasIntAttribute(ax::mojom::IntAttribute::kActivedescendantId);
                    }
                }
            }
        }
...
bool AXNode::IsInListMarker() const
{
    ...
    return grandparent_node && grandparent_node->GetRole() == ax::mojom::Role::kListMarker;
}

bool AXNode::IsCollapsedMenuListSelect() const
{
    return HasState(ax::mojom::State::kCollapsed) && GetRole() == ax::mojom::Role::kComboBoxSelect;
}
...
AXNode* AXNode::GetLowestPlatformAncestor() const
{
    ...
    for (; lowest_unignored_node && lowest_unignored_node->IsIgnored(); lowest_unignored_node = lowest_unignored_node->GetParent()) {
    }
    ...
    for (AXNode* ancestor_node = lowest_unignored_node; ancestor_node; ancestor_node = ancestor_node->GetUnignoredParent()) {
...
AXNode* AXNode::GetTextFieldAncestor() const
{
    ...
    for (AXNode* ancestor = const_cast<AXNode*>(this); ancestor; ancestor = ancestor->GetUnignoredParent()) {
...
AXNode* AXNode::GetTextFieldInnerEditorElement() const
{
    ...
    if (text_container->GetRole() == ax::mojom::Role::kStaticText || text_container->GetRole() == ax::mojom::Role::kLineBreak) {
        text_container = text_container->GetUnignoredParent();
    }
