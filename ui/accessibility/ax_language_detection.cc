...
void AXLanguageInfoStats::RecordLabelStatistics(const std::string& labelled_lang, const std::string& author_lang, bool labelled_with_first_result)
{
...
void AXLanguageInfoStats::ReportMetrics()
{
    ...
    base::UmaHistogramCustomCounts("Accessibility.LanguageDetection.CountDetectionAttempted", count_detection_attempted_, 1, 1000, 50);

    int percentage_detected = count_detection_results_ * 100 / count_detection_attempted_;
    base::UmaHistogramPercentageObsoleteDoNotUse("Accessibility.LanguageDetection.PercentageLanguageDetected", percentage_detected);

    // 50 buckets exponentially covering the range from 1 to 1000.
    base::UmaHistogramCustomCounts("Accessibility.LanguageDetection.CountLabelled", count_labelled_, 1, 1000, 50);

    // If no nodes were labelled, then the percentage labelled with the top result
    // doesn't make sense to report.
    if (count_labelled_) {
        int percentage_top = count_labelled_with_top_result_ * 100 / count_labelled_;
        base::UmaHistogramPercentageObsoleteDoNotUse("Accessibility.LanguageDetection.PercentageLabelledWithTop", percentage_top);

        int percentage_overridden = count_overridden_ * 100 / count_labelled_;
        base::UmaHistogramPercentageObsoleteDoNotUse("Accessibility.LanguageDetection.PercentageOverridden", percentage_overridden);
    }

    // Exact count from 0 to 15, overflow is then truncated to 15.
    base::UmaHistogramExactLinear("Accessibility.LanguageDetection.LangsPerPage", unique_top_lang_detected_.size(), 15);
...
AXLanguageDetectionManager::AXLanguageDetectionManager(AXTree* tree)
    : short_text_language_identifier_(kShortTextIdentifierMinByteLength, kShortTextIdentifierMaxByteLength)
    , tree_(tree)
{
}
...
bool AXLanguageDetectionManager::IsStaticLanguageDetectionEnabled()
{
    ...
    return features::IsAccessibilityLanguageDetectionEnabled() || ::switches::IsExperimentalAccessibilityLanguageDetectionEnabled();
}
...
bool AXLanguageDetectionManager::IsDynamicLanguageDetectionEnabled()
{
    ...
    return features::IsAccessibilityLanguageDetectionEnabled() || ::switches::IsExperimentalAccessibilityLanguageDetectionDynamicEnabled();
}
...
void AXLanguageDetectionManager::RegisterLanguageDetectionObserver()
{
    ...
    language_detection_observer_ = std::make_unique<AXLanguageDetectionObserver>(tree_);
}
...
void AXLanguageDetectionManager::DetectLanguagesForSubtree(AXNode* subtree_root)
{
...
void AXLanguageDetectionManager::DetectLanguagesForNode(AXNode* node)
{
    ...
    const std::vector<Result> results = language_identifier_.FindTopNMostFreqLangs(text, kMaxDetectedLanguagesPerSpan);
...
void AXLanguageDetectionManager::LabelLanguagesForSubtree(AXNode* subtree_root)
{
...
void AXLanguageDetectionManager::LabelLanguagesForNode(AXNode* node)
{
    ...
    for (const auto& lang : lang_info->detected_languages) {
        if (lang_info_stats_.CheckLanguageWithinTop(lang)) {
            ...
            const std::string& author_lang = node->GetInheritedStringAttribute(ax::mojom::StringAttribute::kLanguage);
            lang_info_stats_.RecordLabelStatistics(lang, author_lang, labelled_with_first_result);
...
std::vector<AXLanguageSpan> AXLanguageDetectionManager::GetLanguageAnnotationForStringAttribute(const AXNode& node, ax::mojom::StringAttribute attr)
{
    ...
    if (node.HasStringAttribute(ax::mojom::StringAttribute::kLanguage)) {
        // Use author-provided language if present.
        language_annotation.push_back(AXLanguageSpan { 0 /* start_index */, static_cast<int>(attr_value.length()) /* end_index */,
            node.GetStringAttribute(ax::mojom::StringAttribute::kLanguage) /* language */, 1 /* probability */ });
        return language_annotation;
    }
    // Calculate top 3 languages.
    // TODO(akihiroota): What's a reasonable number of languages to have
    // cld_3 find? Should vary.
    std::vector<Result> top_languages = short_text_language_identifier_.FindTopNMostFreqLangs(attr_value, kMaxDetectedLanguagesPerPage);
    ...
    for (const auto& result : top_languages) {
        ...
        for (const auto& span_info : ranges) {
            language_annotation.push_back(AXLanguageSpan { span_info.start_index, span_info.end_index, result.language, span_info.probability });
        }
    }
    ...
    std::sort(language_annotation.begin(), language_annotation.end(),
        [](const AXLanguageSpan& left, const AXLanguageSpan& right) -> bool { return left.start_index < right.start_index; });
    // Ensure that AXLanguageSpans do not overlap.
    for (size_t i = 0; i < language_annotation.size(); ++i) {
        if (i > 0) {
            DCHECK(language_annotation[i].start_index <= language_annotation[i - 1].end_index);
        }
    }
    return language_annotation;
}
...
void AXLanguageDetectionObserver::OnAtomicUpdateFinished(AXTree* tree, bool root_changed, const std::vector<Change>& changes)
{
