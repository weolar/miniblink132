...
// links.
class FakeAXTreeSource : public AXTreeSource<const FakeAXNode*, AXTreeData*, AXNodeData> {
public:
    ...
    const FakeAXNode* GetRoot() const override
    {
    ...
    AXNodeID GetId(const FakeAXNode* node) const override
    {
        return node->id;
    }
    ...
    const FakeAXNode* ChildAt(const FakeAXNode* node, size_t index) const override
    {
    ...
    bool IsIgnored(const FakeAXNode* node) const override
    {
        return false;
    }

    bool IsEqual(const FakeAXNode* node1, const FakeAXNode* node2) const override
    {
        return node1 == node2;
    }

    const FakeAXNode* GetNull() const override
    {
        return nullptr;
    }

    void SerializeNode(const FakeAXNode* node, AXNodeData* out_data) const override
    {
        out_data->id = node->id;
...
TEST(AXTreeSourceCheckerTest, BadNodeIdOfRoot)
{
    ...
    EXPECT_FALSE(checker.CheckAndGetErrorString(&error_string));
    CleanAXNodeDataString(&error_string);
    EXPECT_EQ("Got a node with id 0, but all node IDs should be >= 1:\n"
...
TEST(AXTreeSourceCheckerTest, BadNodeIdOfChild)
{
    ...
    EXPECT_FALSE(checker.CheckAndGetErrorString(&error_string));
    CleanAXNodeDataString(&error_string);
    EXPECT_EQ("Got a node with id -5, but all node IDs should be >= 1:\n"
...
TEST(AXTreeSourceCheckerTest, RootShouldNotBeNodeWithParent)
{
    ...
    EXPECT_FALSE(checker.CheckAndGetErrorString(&error_string));
    CleanAXNodeDataString(&error_string);
    EXPECT_EQ("Node 2 is the root, so its parent should be invalid, "
...
TEST(AXTreeSourceCheckerTest, MissingParent)
{
    ...
    EXPECT_FALSE(checker.CheckAndGetErrorString(&error_string));
    CleanAXNodeDataString(&error_string);
    EXPECT_EQ("Node 2 is not the root, but its parent was invalid:\n"
...
TEST(AXTreeSourceCheckerTest, InvalidParent)
{
    ...
    EXPECT_FALSE(checker.CheckAndGetErrorString(&error_string));
    CleanAXNodeDataString(&error_string);
    EXPECT_EQ("Expected node 3 to have a parent of 1, but found a parent of 2.\n"
