...
public:
    ...
    // result.
    void RecordLabelStatistics(const std::string& labelled_lang, const std::string& author_lang, bool labelled_with_first_result);
...
public:
    ...
    AXLanguageDetectionObserver& operator=(const AXLanguageDetectionObserver&) = delete;
...
public:
    ...
    AXLanguageDetectionManager& operator=(const AXLanguageDetectionManager&) = delete;
    ...
    std::vector<AXLanguageSpan> GetLanguageAnnotationForStringAttribute(const AXNode& node, ax::mojom::StringAttribute attr);
