...
void AXActionHandlerRegistry::SetFrameIDForAXTreeID(const FrameID& frame_id, const AXTreeID& ax_tree_id)
{
...
void AXActionHandlerRegistry::RemoveObserver(AXActionHandlerObserver* observer)
{
...
AXActionHandlerRegistry::FrameID AXActionHandlerRegistry::GetFrameID(const AXTreeID& ax_tree_id)
{
...
AXTreeID AXActionHandlerRegistry::GetAXTreeID(AXActionHandlerRegistry::FrameID frame_id)
{
    auto it = frame_to_ax_tree_id_map_.find(frame_id);
    if (it != frame_to_ax_tree_id_map_.end())
        return it->second;
...
AXTreeID AXActionHandlerRegistry::GetOrCreateAXTreeID(AXActionHandlerBase* handler)
{
...
AXActionHandlerBase* AXActionHandlerRegistry::GetActionHandler(AXTreeID ax_tree_id)
{
...
void AXActionHandlerRegistry::SetAXTreeID(const AXTreeID& id, AXActionHandlerBase* action_handler)
{
