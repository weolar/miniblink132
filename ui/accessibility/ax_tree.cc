...
constexpr ax::mojom::IntListAttribute kReverseRelationIntListAttributes[]
    = { ax::mojom::IntListAttribute::kControlsIds, ax::mojom::IntListAttribute::kDetailsIds, ax::mojom::IntListAttribute::kDescribedbyIds,
          ax::mojom::IntListAttribute::kErrormessageIds, ax::mojom::IntListAttribute::kFlowtoIds, ax::mojom::IntListAttribute::kLabelledbyIds };
constexpr ax::mojom::IntAttribute kReverseRelationIntAttributes[] = { ax::mojom::IntAttribute::kActivedescendantId };
...
std::string TreeToStringHelper(const AXNode* node, int indent, bool verbose)
{
    ...
    return std::accumulate(node->children().cbegin(), node->children().cend(), std::string(2 * indent, ' ') + node->data().ToString(verbose) + "\n",
        [indent, verbose](const std::string& str, const AXNode* child) { return str + TreeToStringHelper(child, indent + 1, verbose); });
}

template <typename K, typename V> bool KeyValuePairsKeysMatch(std::vector<std::pair<K, V>> pairs1, std::vector<std::pair<K, V>> pairs2)
{
...
template <typename K, typename V> std::map<K, V> MapFromKeyValuePairs(std::vector<std::pair<K, V>> pairs)
{
    std::map<K, V> result;
...
void CallIfAttributeValuesChanged(
    const std::vector<std::pair<K, V>>& old_pairs, const std::vector<std::pair<K, V>>& new_pairs, const V& empty_value, F callback)
{
    ...
    std::sort(const_cast<VectorOfPairs>(old_pairs).begin(), const_cast<VectorOfPairs>(old_pairs).end(), comp);
    std::sort(const_cast<VectorOfPairs>(new_pairs).begin(), const_cast<VectorOfPairs>(new_pairs).end(), comp);
    for (size_t old_i = 0, new_i = 0; old_i < old_pairs.size() || new_i < new_pairs.size();) {
...
struct PendingStructureChanges {
    explicit PendingStructureChanges(const AXNode* node)
        ...
        , create_node_count(0)
        , node_exists(!!node)
        , parent_node_id((node && node->parent()) ? std::make_optional<AXNodeID>(node->parent()->id()) : std::nullopt)
    ...
    {
        return DoesNodeExpectSubtreeWillBeDestroyed() || DoesNodeExpectNodeWillBeDestroyed() || DoesNodeExpectNodeWillBeCreated();
    }
    ...
    {
        return DoesNodeExpectSubtreeWillBeDestroyed() || DoesNodeExpectNodeWillBeDestroyed();
    }
    ...
    {
        return node_exists && !last_known_data;
    }
...
struct AXTreeUpdateState {
    ...
    {
        DCHECK_EQ(AXTreePendingStructureStatus::kComplete, pending_update_status) << "This method should not be called before pending changes have "
                                                                                     "finished computing.";
    ...
    {
        DCHECK_EQ(AXTreePendingStructureStatus::kComputing, pending_update_status) << "This method should only be called while computing pending changes, "
    ...
    std::optional<AXNodeID> GetParentIdForPendingNode(AXNodeID node_id)
    {
        ...
        DCHECK(!data->parent_node_id || ShouldPendingNodeExistInTree(*data->parent_node_id));
    ...
    const AXNodeData& GetLastKnownPendingNodeData(AXNodeID node_id) const
    {
        ...
        PendingStructureChanges* data = GetPendingStructureChanges(node_id);
        return (data && data->last_known_data) ? *data->last_known_data : *empty_data;
    }
    ...
    void SetLastKnownPendingNodeData(const AXNodeData* node_data)
    {
        DCHECK_EQ(AXTreePendingStructureStatus::kComputing, pending_update_status) << "This method should only be called while computing pending changes, "
                                                                                      "before updates are made to the tree.";
        GetOrCreatePendingStructureChanges(node_data->id)->last_known_data = node_data;
    }
    ...
    bool IncrementPendingCreateNodeCount(AXNodeID node_id, std::optional<AXNodeID> parent_node_id)
    {
        DCHECK_EQ(AXTreePendingStructureStatus::kComputing, pending_update_status) << "This method should only be called while computing pending changes, "
                                                                                      "before updates are made to the tree.";
        PendingStructureChanges* data = GetOrCreatePendingStructureChanges(node_id);
    ...
    bool HasIgnoredChanged(const AXNodeData& new_data) const
    {
        ...
        return AXTree::ComputeNodeIsIgnored(old_tree_data ? &old_tree_data.value() : nullptr, old_data)
            != AXTree::ComputeNodeIsIgnored(new_tree_data ? &new_tree_data.value() : nullptr, new_data);
    }
    ...
    AXTreePendingStructureStatus pending_update_status = AXTreePendingStructureStatus::kNotStarted;
    ...
    std::map<AXNodeID, std::unique_ptr<PendingStructureChanges>> node_id_to_pending_data;
...
private:
    PendingStructureChanges* GetPendingStructureChanges(AXNodeID node_id) const
    {
        ...
        return (iter != node_id_to_pending_data.cend()) ? iter->second.get() : nullptr;
    }

    PendingStructureChanges* GetOrCreatePendingStructureChanges(AXNodeID node_id)
    {
        auto iter = node_id_to_pending_data.find(node_id);
        if (iter == node_id_to_pending_data.cend()) {
            const AXNode* node = tree->GetFromId(node_id);
            iter = node_id_to_pending_data.emplace(std::make_pair(node_id, std::make_unique<PendingStructureChanges>(node))).first;
        }
...
// static
bool AXTree::ComputeNodeIsIgnored(const AXTreeData* optional_tree_data, const AXNodeData& node_data)
{
    // A node with an ARIA presentational role (role="none") should also be
    // ignored.
    bool is_ignored = node_data.HasState(ax::mojom::State::kIgnored) || node_data.role == ax::mojom::Role::kNone;
    ...
    if (is_focused_node_always_unignored_ && is_ignored && optional_tree_data && optional_tree_data->focus_id != kInvalidAXNodeID
...
bool AXTree::ComputeNodeIsIgnoredChanged(
    const AXTreeData* optional_old_tree_data, const AXNodeData& old_node_data, const AXTreeData* optional_new_tree_data, const AXNodeData& new_node_data)
{
    ...
    const bool old_node_is_ignored = ComputeNodeIsIgnored(optional_old_tree_data, old_node_data);
    const bool new_node_is_ignored = ComputeNodeIsIgnored(optional_new_tree_data, new_node_data);
...
AXTree::AXTree()
{
    ...
    language_detection_manager = std::make_unique<AXLanguageDetectionManager>(this);
}
...
void AXTree::Destroy()
{
    ...
    observers_.Notify(&AXTreeObserver::OnAtomicUpdateStarting, this, deleting_node_ids, std::set<AXNodeID> {});
    ...
    UMA_HISTOGRAM_CUSTOM_TIMES("Accessibility.Performance.AXTree.Destroy2", timer.Elapsed(), base::Microseconds(1), base::Seconds(1), 50);
}
...
gfx::RectF AXTree::RelativeToTreeBoundsInternal(
    const AXNode* node, gfx::RectF bounds, bool* offscreen, bool clip_bounds, bool skip_container_offset, bool allow_recursion) const
{
    ...
    if (bounds.width() == 0 && bounds.height() == 0) {
        ...
        // bad state.
        if (bounds.IsEmpty() && !GetTreeUpdateInProgressState() && allow_recursion) {
            for (AXNode* child : node->children()) {
                gfx::RectF child_bounds = RelativeToTreeBoundsInternal(child, gfx::RectF(), /*offscreen=*/nullptr, clip_bounds, skip_container_offset,
        ...
        bounds.Offset(node->data().relative_bounds.bounds.x(), node->data().relative_bounds.bounds.y());
    }
    ...
    while (node != nullptr) {
        ...
        const AXNode* container = GetFromId(node->data().relative_bounds.offset_container_id);
        ...
        if (container->HasIntAttribute(ax::mojom::IntAttribute::kScrollX) && container->HasIntAttribute(ax::mojom::IntAttribute::kScrollY)) {
        ...
        if (container->GetBoolAttribute(ax::mojom::BoolAttribute::kClipsChildren) && intersection.IsEmpty() && !clipped.IsEmpty()) {
    ...
    if (bounds.width() == 0 && bounds.height() == 0) {
        ...
        if (ancestor && allow_recursion) {
            ...
            ancestor_bounds = RelativeToTreeBoundsInternal(ancestor, gfx::RectF(), &ignore_offscreen, clip_bounds, skip_container_offset,
                /* allow_recursion = */ false);
            ...
            if (original_bounds.x() == 0 && original_bounds.y() == 0) {
                ...
                bounds.set_height(std::max(0.0f, ancestor_bounds.bottom() - bounds.y()));
            }
            if (offscreen != nullptr)
                *offscreen |= true;
        }
...
gfx::RectF AXTree::RelativeToTreeBounds(const AXNode* node, gfx::RectF bounds, bool* offscreen, bool clip_bounds, bool skip_container_offset) const
{
    bool allow_recursion = true;
    return RelativeToTreeBoundsInternal(node, bounds, offscreen, clip_bounds, skip_container_offset, allow_recursion);
}

gfx::RectF AXTree::GetTreeBounds(const AXNode* node, bool* offscreen, bool clip_bounds) const
{
    return RelativeToTreeBounds(node, gfx::RectF(), offscreen, clip_bounds);
}

std::set<AXNodeID> AXTree::GetReverseRelations(ax::mojom::IntAttribute attr, AXNodeID dst_id) const
{
...
std::set<AXNodeID> AXTree::GetReverseRelations(ax::mojom::IntListAttribute attr, AXNodeID dst_id) const
{
...
std::set<AXNodeID> AXTree::GetNodeIdsForChildTreeId(AXTreeID child_tree_id) const
{
    // Conceptually, this is the "const" version of:
...
bool AXTree::Unserialize(const AXTreeUpdate& update)
{
    ...
    for (const auto& new_data : update.nodes)
        DCHECK(new_data.id != kInvalidAXNodeID) << "AXTreeUpdate contains invalid node: " << update.ToString();
    if (update.tree_data.tree_id != AXTreeIDUnknown() && data_.tree_id != AXTreeIDUnknown()) {
        DCHECK_EQ(update.tree_data.tree_id, data_.tree_id) << "Tree id mismatch between tree update and this tree.";
    }
#endif
    ...
    DCHECK(old_root_id != kInvalidAXNodeID || update.root_id != kInvalidAXNodeID) << "Tree must have a valid root or update must have a valid root.";
    ...
    SCOPED_UMA_HISTOGRAM_TIMER_MICROS("Accessibility.Performance.Tree.Unserialize2");
    ...
    for (const auto& new_data : update_state.updated_nodes) {
        const bool is_new_root = update_state.root_will_be_created && new_data.id == update.root_id;
        if (is_new_root) {
            continue;
        }
        ...
        if (node && !new_data.IsIgnored() && !node->data().IsIgnored() && notified_node_attributes_will_change.insert(new_data.id).second) {
            if (new_data.HasIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartOffsets)) {
                DCHECK(new_data.HasIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartOffsets));
                DCHECK(new_data.HasIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndOffsets));
                DCHECK(new_data.HasIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartAnchorIds));
                DCHECK(new_data.HasIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndAnchorIds));
                DCHECK(new_data.HasIntListAttribute(ax::mojom::IntListAttribute::kTextOperations));
                observers_.Notify(&AXTreeObserver::OnTextDeletionOrInsertion, *node, new_data);
            }
            NotifyNodeAttributesWillChange(node, update_state, update_state.old_tree_data ? &update_state.old_tree_data.value() : nullptr, node->data(),
                update_state.new_tree_data ? &update_state.new_tree_data.value() : nullptr, new_data);
        }
    }
    ...
    for (AXNodeID id : update_state.ignored_state_changed_ids) {
        ...
        if (node) {
            ...
            bool is_root_of_ignored_change = !node->parent() || !base::Contains(update_state.ignored_state_changed_ids, node->parent()->id())
                || node->IsIgnored() != node->parent()->IsIgnored();
            observers_.Notify(&AXTreeObserver::OnIgnoredWillChange, this, node, will_be_ignored, is_root_of_ignored_change);
        }
    }

    observers_.Notify(&AXTreeObserver::OnAtomicUpdateStarting, this, update_state.deleting_node_ids, update_state.reparenting_node_ids);
    ...
    std::vector<AXTreeObserver::Change> changes;
    {
        ...
        if (update.node_id_to_clear != kInvalidAXNodeID) {
            ...
            if (!GetFromId(node_id_to_clear) && update.root_id == node_id_to_clear && update.root_id != old_root_id && root_) {
        ...
        DCHECK_EQ(update.root_id != kInvalidAXNodeID && !GetFromId(update.root_id), update_state.root_will_be_created);

        // Update all of the nodes in the update.
        for (const AXNodeData& updated_node_data : update_state.updated_nodes) {
            const bool is_new_root = update_state.root_will_be_created && updated_node_data.id == update.root_id;
            if (!UpdateNode(updated_node_data, is_new_root, &update_state))
                return false;
        }

        if (!root_) {
            ACCESSIBILITY_TREE_UNSERIALIZE_ERROR_HISTOGRAM(AXTreeUnserializeError::kNoRoot);
        ...
        for (const AXNodeData& updated_node_data : update_state.updated_nodes) {
            ...
            if (is_new_node) {
                if (is_reparented_node) {
                    ...
                    bool is_subtree = !node->parent() || !update_state.IsCreatedNode(node->parent()) || (node->parent() == root_ && root_updated);
                    change = is_subtree ? AXTreeObserver::SUBTREE_REPARENTED : AXTreeObserver::NODE_REPARENTED;
                } else {
                    ...
                    // Note that we also need to check for the special case when we update
                    // the root without replacing it.
                    bool is_subtree = !node->parent() || !update_state.IsCreatedNode(node->parent()) || update_state.IsReparentedNode(node->parent()->id())
                        || (node->parent() == root_ && root_updated);
                    change = is_subtree ? AXTreeObserver::SUBTREE_CREATED : AXTreeObserver::NODE_CREATED;
                }
            }
        ...
        for (AXNodeID node_id : update_state.invalidate_unignored_cached_values_ids) {
            ...
            if (unignored_ancestor && updated_unignored_cached_values_ids.insert(unignored_ancestor->id()).second) {
                ...
                if (unignored_ancestor->id() != node_id && visited_observer_changes.emplace(unignored_ancestor->id()).second) {
                    changes.emplace_back(unignored_ancestor, AXTreeObserver::NODE_CHANGED);
                }
            }
        }
    ...
    if (update_state.old_tree_data) {
        DCHECK(update.has_tree_data) << "If `UpdateState::old_tree_data` exists, then there must be a "
        ...
        // information.
        observers_.Notify(&AXTreeObserver::OnTreeDataChanged, this, *update_state.old_tree_data, data_);
    }
    ...
    for (AXNodeID changed_id : update_state.node_data_changed_ids) {
        ...
        const bool is_new_root = update_state.root_will_be_created && changed_id == update.root_id;
        if (!is_new_root) {
            auto it = update_state.old_node_id_to_data.find(changed_id);
            if (it != update_state.old_node_id_to_data.end()) {
                NotifyNodeAttributesHaveBeenChanged(node, update_state, update_state.old_tree_data ? &update_state.old_tree_data.value() : nullptr, it->second,
                    update_state.new_tree_data ? &update_state.new_tree_data.value() : nullptr, node->data());
            }
        }
    ...
    observers_.Notify(&AXTreeObserver::OnAtomicUpdateFinished, this, root_->id() != old_root_id, changes);
...
void AXTree::CheckTreeConsistency(const AXTreeUpdate& update)
{
    ...
    msg << "After a tree update, there is a tree inconsistency.\n"
        ...
        << "\n* Slow nodes count: " << root_->GetSubtreeCount() << "\n* AXTreeUpdate: " << TreeToStringHelper(root_, 0, /*verbose*/ false);
...
AXNode* AXTree::CreateNode(AXNode* parent, AXNodeID id, size_t index_in_parent, AXTreeUpdateState* update_state)
{
    DCHECK(GetTreeUpdateInProgressState());
    ...
    DCHECK(!parent || update_state->InvalidatesUnignoredCachedValues(parent->id()));
    ...
    auto node = std::make_unique<AXNode>(this, parent, id, index_in_parent, parent ? 0 : index_in_parent);
...
bool AXTree::ComputePendingChanges(const AXTreeUpdate& update, AXTreeUpdateState* update_state)
{
    DCHECK_EQ(AXTreePendingStructureStatus::kNotStarted, update_state->pending_update_status) << "Pending changes have already started being computed.";
    ...
    base::AutoReset<std::optional<AXNodeID>> pending_root_id_resetter(
        &update_state->pending_root_id, root_ ? std::make_optional<AXNodeID>(root_->id()) : std::nullopt);
    ...
    if (update.node_id_to_clear != kInvalidAXNodeID) {
        if (AXNode* cleared_node = GetFromId(update.node_id_to_clear)) {
            DCHECK(root_);
            if (cleared_node == root_ && update.root_id != update_state->pending_root_id) {
            ...
            if (update_state->ShouldPendingNodeExistInTree(root_->id())) {
                update_state->invalidate_unignored_cached_values_ids.insert(cleared_node->id());
    ...
    if (is_focused_node_always_unignored_ && update_state->old_tree_data && update_state->new_tree_data) {
        ...
        if (update_state->old_tree_data->focus_id != kInvalidAXNodeID) {
            const AXNode* old_focus = GetFromId(update_state->old_tree_data->focus_id);
            if (old_focus && update_state->ShouldPendingNodeExistInTree(old_focus->id())
                && !base::Contains(update_state->updated_nodes, old_focus->id(), &AXNodeData::id)) {
                update_state->updated_nodes.push_back(old_focus->data());
            }
        }

        if (update_state->new_tree_data->focus_id != kInvalidAXNodeID) {
            const AXNode* new_focus = GetFromId(update_state->new_tree_data->focus_id);
            if (new_focus && update_state->ShouldPendingNodeExistInTree(new_focus->id())
                && !base::Contains(update_state->updated_nodes, new_focus->id(), &AXNodeData::id)) {
    ...
    if (update.root_id != kInvalidAXNodeID) {
        update_state->root_will_be_created = !GetFromId(update.root_id) || !update_state->ShouldPendingNodeExistInTree(update.root_id);
    }
    ...
    for (const AXNodeData& new_data : update_state->updated_nodes) {
        if (new_data.id == kInvalidAXNodeID)
            continue;
        bool is_new_root = update_state->root_will_be_created && new_data.id == update.root_id;
        if (!ComputePendingChangesToNode(new_data, is_new_root, update_state)) {
            update_state->pending_update_status = AXTreePendingStructureStatus::kFailed;
            return false;
        }
    ...
    base::UmaHistogramBoolean("Accessibility.InlineTextBoxes.PresentInUpdate", number_of_inline_textboxes > 0);

    if (number_of_inline_textboxes > 0) {
        base::UmaHistogramCounts1000("Accessibility.InlineTextBoxes.Count", number_of_inline_textboxes);
    }
...
bool AXTree::ComputePendingChangesToNode(const AXNodeData& new_data, bool is_new_root, AXTreeUpdateState* update_state)
{
    ...
    if (!update_state->ShouldPendingNodeExistInTree(new_data.id)) {
        if (!is_new_root) {
            ACCESSIBILITY_TREE_UNSERIALIZE_ERROR_HISTOGRAM(AXTreeUnserializeError::kNotInTree);
            RecordError(*update_state, base::StringPrintf("%d will not be in the tree and is not the new root", new_data.id));
            return false;
        }
        ...
        if (!update_state->IncrementPendingCreateNodeCount(new_data.id, std::nullopt)) {
            ACCESSIBILITY_TREE_UNSERIALIZE_ERROR_HISTOGRAM(AXTreeUnserializeError::kCreationPending);
            RecordError(*update_state, base::StringPrintf("Node %d is already pending for creation, cannot be the new root", new_data.id));
            return false;
        }
    ...
    for (AXNodeID new_child_id : new_data.child_ids) {
        if (base::Contains(new_child_id_set, new_child_id)) {
            ACCESSIBILITY_TREE_UNSERIALIZE_ERROR_HISTOGRAM(AXTreeUnserializeError::kDuplicateChild);
            RecordError(*update_state, base::StringPrintf("Node %d has duplicate child id %d", new_data.id, new_child_id));
            return false;
        }
    ...
    if (update_state->DoesPendingNodeRequireInit(new_data.id)) {
        ...
        for (AXNodeID child_id : new_child_id_set) {
            ...
            if (!update_state->IncrementPendingCreateNodeCount(child_id, new_data.id)) {
                ACCESSIBILITY_TREE_UNSERIALIZE_ERROR_HISTOGRAM(AXTreeUnserializeError::kCreationPendingForChild);
    ...
    const AXNodeData& old_data = update_state->GetLastKnownPendingNodeData(new_data.id);

    AXTreeData* old_tree_data = update_state->old_tree_data ? &update_state->old_tree_data.value() : nullptr;
    AXTreeData* new_tree_data = update_state->new_tree_data ? &update_state->new_tree_data.value() : nullptr;
    if (ComputeNodeIsIgnoredChanged(old_tree_data, old_data, new_tree_data, new_data)) {
    ...
    std::set<AXNodeID> old_child_id_set(old_data.child_ids.cbegin(), old_data.child_ids.cend());

    std::vector<AXNodeID> create_or_destroy_ids;
    std::set_symmetric_difference(
        old_child_id_set.cbegin(), old_child_id_set.cend(), new_child_id_set.cbegin(), new_child_id_set.cend(), std::back_inserter(create_or_destroy_ids));
    ...
    if (!create_or_destroy_ids.empty() || update_state->HasIgnoredChanged(new_data)) {
        update_state->invalidate_unignored_cached_values_ids.insert(new_data.id);
    ...
    for (AXNodeID child_id : create_or_destroy_ids) {
        if (base::Contains(new_child_id_set, child_id)) {
            ...
            if (update_state->ShouldPendingNodeExistInTree(child_id)) {
                ACCESSIBILITY_TREE_UNSERIALIZE_ERROR_HISTOGRAM(AXTreeUnserializeError::kReparent);
                RecordError(*update_state,
...
bool AXTree::UpdateNode(const AXNodeData& src, bool is_new_root, AXTreeUpdateState* update_state)
{
    DCHECK(GetTreeUpdateInProgressState());
    ...
    if (node) {
        ...
        if (!update_state->IsCreatedNode(node) || update_state->IsReparentedNode(node)) {
            update_state->old_node_id_to_data.insert(std::make_pair(node->id(), node->TakeData()));
        }
        node->SetData(src);
    } else {
        // Node is created.
        if (!is_new_root) {
            ACCESSIBILITY_TREE_UNSERIALIZE_ERROR_HISTOGRAM(AXTreeUnserializeError::kNotInTree);
            RecordError(*update_state, base::StringPrintf("%d is not in the tree and not the new root", src.id));
            return false;
        }
    ...
    bool success = CreateNewChildVector(node, src.child_ids, &new_children, update_state);
...
void AXTree::NotifySubtreeWillBeReparentedOrDeleted(AXNode* node, const AXTreeUpdateState* update_state)
{
    DCHECK(!GetTreeUpdateInProgressState());
    ...
    if (notify_removed && node->parent() && base::Contains(update_state->ignored_state_changed_ids, node->parent()->id()) && !node->parent()->IsIgnored()) {
...
void AXTree::NotifyNodeWillBeReparentedOrDeleted(AXNode& node, const AXTreeUpdateState& update_state)
{
    DCHECK(!GetTreeUpdateInProgressState());
    ...
    DCHECK(table_info_map_.find(id) == table_info_map_.end()) << "Table info should never be recreated during node deletion";
}

void AXTree::RecursivelyNotifyNodeWillBeDeletedForTreeTeardown(AXNode& node, std::set<AXNodeID>& deleted_nodes)
{
    ...
    for (ui::AXNode* child : node.children()) {
        RecursivelyNotifyNodeWillBeDeletedForTreeTeardown(CHECK_DEREF(child), deleted_nodes);
    }
}
...
void AXTree::NotifyNodeHasBeenReparentedOrCreated(AXNode* node, const AXTreeUpdateState* update_state)
{
    DCHECK(!GetTreeUpdateInProgressState());
    if (node->id() == kInvalidAXNodeID)
        return;
...
void AXTree::NotifyChildTreeConnectionChanged(AXNode* node, AXTree* child_tree)
{
    DCHECK(node->tree() == this);
    observers_.Notify(&AXTreeObserver::OnChildTreeConnectionChanged, node);
}

void AXTree::NotifyNodeAttributesWillChange(AXNode* node, AXTreeUpdateState& update_state, const AXTreeData* optional_old_tree_data, const AXNodeData& old_data,
    const AXTreeData* optional_new_tree_data, const AXNodeData& new_data)
{
    DCHECK(!GetTreeUpdateInProgressState());
    if (new_data.id == kInvalidAXNodeID)
        return;

    observers_.Notify(&AXTreeObserver::OnNodeDataWillChange, this, old_data, new_data);
}

void AXTree::NotifyNodeAttributesHaveBeenChanged(AXNode* node, AXTreeUpdateState& update_state, const AXTreeData* optional_old_tree_data,
    const AXNodeData& old_data, const AXTreeData* optional_new_tree_data, const AXNodeData& new_data)
{
    DCHECK(!GetTreeUpdateInProgressState());
    DCHECK(node);
    ...
    if (node->GetRole() == ax::mojom::Role::kRootWebArea && old_data.child_ids.empty() && !node->GetParentCrossingTreeBoundary()) {
        return;
    }

    observers_.Notify(&AXTreeObserver::OnNodeDataChanged, this, old_data, new_data);

    if (base::Contains(update_state.ignored_state_changed_ids, new_data.id)) {
        observers_.Notify(&AXTreeObserver::OnIgnoredChanged, this, node, node->IsIgnored());
    }
    ...
    if (old_data.role != new_data.role) {
        observers_.Notify(&AXTreeObserver::OnRoleChanged, this, node, old_data.role, new_data.role);
    }

    if (old_data.state != new_data.state) {
        for (int32_t i = static_cast<int32_t>(ax::mojom::State::kNone) + 1; i <= static_cast<int32_t>(ax::mojom::State::kMaxValue); ++i) {
            ...
            if (old_data.HasState(state) != new_data.HasState(state)) {
                observers_.Notify(&AXTreeObserver::OnStateChanged, this, node, state, new_data.HasState(state));
            }
        }
    }

    auto string_callback = [this, node](ax::mojom::StringAttribute attr, const std::string& old_string, const std::string& new_string) {
        DCHECK_NE(old_string, new_string);
        observers_.Notify(&AXTreeObserver::OnStringAttributeChanged, this, node, attr, old_string, new_string);
    };
    CallIfAttributeValuesChanged(old_data.string_attributes, new_data.string_attributes, std::string(), string_callback);

    auto bool_callback = [this, node](ax::mojom::BoolAttribute attr, const bool& old_bool, const bool& new_bool) {
        DCHECK_NE(old_bool, new_bool);
        observers_.Notify(&AXTreeObserver::OnBoolAttributeChanged, this, node, attr, new_bool);
    };
    CallIfAttributeValuesChanged(old_data.bool_attributes, new_data.bool_attributes, false, bool_callback);

    auto float_callback = [this, node](ax::mojom::FloatAttribute attr, const float& old_float, const float& new_float) {
        DCHECK_NE(old_float, new_float);
        observers_.Notify(&AXTreeObserver::OnFloatAttributeChanged, this, node, attr, old_float, new_float);
    };
    CallIfAttributeValuesChanged(old_data.float_attributes, new_data.float_attributes, 0.0f, float_callback);

    auto int_callback = [this, node](ax::mojom::IntAttribute attr, const int& old_int, const int& new_int) {
        DCHECK_NE(old_int, new_int);
        observers_.Notify(&AXTreeObserver::OnIntAttributeChanged, this, node, attr, old_int, new_int);
    };
    CallIfAttributeValuesChanged(old_data.int_attributes, new_data.int_attributes, 0, int_callback);

    auto intlist_callback = [this, node](ax::mojom::IntListAttribute attr, const std::vector<int32_t>& old_intlist, const std::vector<int32_t>& new_intlist) {
        observers_.Notify(&AXTreeObserver::OnIntListAttributeChanged, this, node, attr, old_intlist, new_intlist);
    };
    CallIfAttributeValuesChanged(old_data.intlist_attributes, new_data.intlist_attributes, std::vector<int32_t>(), intlist_callback);

    auto stringlist_callback
        = [this, node](ax::mojom::StringListAttribute attr, const std::vector<std::string>& old_stringlist, const std::vector<std::string>& new_stringlist) {
              observers_.Notify(&AXTreeObserver::OnStringListAttributeChanged, this, node, attr, old_stringlist, new_stringlist);
          };
    CallIfAttributeValuesChanged(old_data.stringlist_attributes, new_data.stringlist_attributes, std::vector<std::string>(), stringlist_callback);
}

void AXTree::UpdateReverseRelations(AXNode* node, const AXNodeData& new_data, bool is_new_node)
{
    DCHECK(GetTreeUpdateInProgressState());
...
bool AXTree::ValidatePendingChangesComplete(const AXTreeUpdateState& update_state)
{
    if (!update_state.pending_node_ids.empty()) {
        ACCESSIBILITY_TREE_UNSERIALIZE_ERROR_HISTOGRAM(AXTreeUnserializeError::kPendingNodes);
    ...
    if (!update_state.node_id_to_pending_data.empty()) {
        ...
        if (has_pending_changes) {
            ACCESSIBILITY_TREE_UNSERIALIZE_ERROR_HISTOGRAM(AXTreeUnserializeError::kPendingChanges);
            RecordError(update_state,
                base::StringPrintf("Changes left pending by the update; "
                                   "destroy subtrees: %s, destroy nodes: %s, create nodes: %s",
                    destroy_subtree_ids.c_str(), destroy_node_ids.c_str(), create_node_ids.c_str()));
        }
...
void AXTree::MarkSubtreeForDestruction(AXNodeID node_id, AXTreeUpdateState* update_state)
{
    update_state->IncrementPendingDestroySubtreeCount(node_id);
    MarkNodesForDestructionRecursive(node_id, update_state);
}

void AXTree::MarkNodesForDestructionRecursive(AXNodeID node_id, AXTreeUpdateState* update_state)
{
    ...
    const AXNodeData& last_known_data = update_state->GetLastKnownPendingNodeData(node_id);
...
void AXTree::DestroySubtree(AXNode* node, AXTreeUpdateState* update_state)
{
    ...
    DCHECK(!node->parent() || update_state->InvalidatesUnignoredCachedValues(node->parent()->id()));
    update_state->DecrementPendingDestroySubtreeCount(node->id());
    DestroyNodeAndSubtree(node, update_state);
}

void AXTree::DestroyNodeAndSubtree(AXNode* node, AXTreeUpdateState* update_state)
{
    AXNodeID id = node->id();
    ...
    if (update_state) {
        ...
        if (update_state->IsReparentedNode(node_to_delete.get())) {
            update_state->old_node_id_to_data.insert(std::make_pair(id, node_to_delete->TakeData()));
        }
    }
}

void AXTree::DeleteOldChildren(AXNode* node, const std::vector<AXNodeID>& new_child_ids, AXTreeUpdateState* update_state)
{
    DCHECK(GetTreeUpdateInProgressState());
    ...
    std::set<AXNodeID> new_child_id_set(new_child_ids.begin(), new_child_ids.end());
...
bool AXTree::CreateNewChildVector(
    AXNode* node, const std::vector<AXNodeID>& new_child_ids, std::vector<raw_ptr<AXNode, VectorExperimental>>* new_children, AXTreeUpdateState* update_state)
{
    DCHECK(GetTreeUpdateInProgressState());
    bool success = true;
    for (size_t i = 0; i < new_child_ids.size(); ++i) {
        ...
        if (child) {
            if (child->parent() != node) {
                ...
                if (child->parent()) {
                    RecordError(*update_state, base::StringPrintf("Node %d reparented from %d to %d", child->id(), child->parent()->id(), node->id()));
                } else {
...
void AXTree::PopulateOrderedSetItemsMap(const AXNode& original_node, const AXNode* ordered_set, OrderedSetItemsMap* items_map_to_be_populated) const
{
    ...
    // defined.
    std::optional<int> ordered_set_min_level = ordered_set->GetHierarchicalLevel();

    for (AXNode::UnignoredChildIterator child = ordered_set->UnignoredChildrenBegin(); child != ordered_set->UnignoredChildrenEnd(); ++child) {
        std::optional<int> child_level = child->GetHierarchicalLevel();
        if (child_level) {
            ordered_set_min_level = ordered_set_min_level ? std::min(child_level, ordered_set_min_level) : child_level;
        }
    }

    RecursivelyPopulateOrderedSetItemsMap(original_node, ordered_set, ordered_set, ordered_set_min_level, std::nullopt, items_map_to_be_populated);
    ...
    if (&original_node == ordered_set && !items_map_to_be_populated->HierarchicalLevelExists(ordered_set_min_level)) {
        items_map_to_be_populated->Add(ordered_set_min_level, OrderedSetContent(&original_node));
    }
}

void AXTree::RecursivelyPopulateOrderedSetItemsMap(const AXNode& original_node, const AXNode* ordered_set, const AXNode* local_parent,
    std::optional<int> ordered_set_min_level, std::optional<int> prev_level, OrderedSetItemsMap* items_map_to_be_populated) const
{
    ...
    // levels.
    if (ordered_set->GetRole() == local_parent->GetRole() && ordered_set != local_parent)
        return;

    for (AXNode::UnignoredChildIterator itr = local_parent->UnignoredChildrenBegin(); itr != local_parent->UnignoredChildrenEnd(); ++itr) {
        ...
        // are never higher.
        if (child->data().IsInvisible() && !IsCollapsed(local_parent) && !IsCollapsed(local_parent->parent())
            && (!local_parent->parent() || !IsCollapsed(local_parent->parent()->parent()))) {
            continue;
        }
        ...
        if (child->GetRole() == ax::mojom::Role::kComment
            || (original_node.GetRole() == ax::mojom::Role::kRadioButton && child->GetRole() == ax::mojom::Role::kRadioButton)
            || (original_node.GetRole() != ax::mojom::Role::kRadioButton && child->SetRoleMatchesItemRole(ordered_set))) {
            ...
            if (!items_map_to_be_populated->HierarchicalLevelExists(curr_level)) {
                bool use_ordered_set = child->SetRoleMatchesItemRole(ordered_set) && ordered_set_min_level == curr_level;
                const AXNode* child_ordered_set = use_ordered_set ? ordered_set : nullptr;
                items_map_to_be_populated->Add(curr_level, OrderedSetContent(child_ordered_set));
            }
        ...
        if (child->IsIgnoredContainerForOrderedSet()) {
            RecursivelyPopulateOrderedSetItemsMap(original_node, ordered_set, child, ordered_set_min_level, curr_level, items_map_to_be_populated);
        }
...
void AXTree::ComputeSetSizePosInSetAndCache(const AXNode& node, const AXNode* ordered_set)
{
    DCHECK(ordered_set);
    ...
    if (node.GetRole() != ax::mojom::Role::kComment && node.GetRole() != ax::mojom::Role::kDisclosureTriangle
        && node.GetRole() != ax::mojom::Role::kDisclosureTriangleGrouped && node.GetRole() != ax::mojom::Role::kRadioButton
    ...
    if (node.GetRole() == ax::mojom::Role::kComboBoxSelect && node.GetUnignoredChildCount() > 0) {
        ...
        OrderedSetContent* set_content = items_map_to_be_populated.GetFirstOrderedSetContent();
        if (set_content && set_content->set_items_.size() == 1) {
            ...
            if (menu_list_popup->GetRole() == ax::mojom::Role::kMenuListPopup) {
                ...
                PopulateOrderedSetItemsMap(node, menu_list_popup, &items_map_to_be_populated);
...
void AXTree::ComputeSetSizePosInSetAndCacheHelper(const OrderedSetContent& ordered_set_content)
{
    ...
    for (const AXNode* item : ordered_set_content.set_items_) {
        ...
        int32_t pos_in_set_value = std::max(num_elements + 1, item->GetIntAttribute(ax::mojom::IntAttribute::kPosInSet));
        ...
        if (item->GetHierarchicalLevel() && item->HasIntAttribute(ax::mojom::IntAttribute::kPosInSet))
            pos_in_set_value = item->GetIntAttribute(ax::mojom::IntAttribute::kPosInSet);
        ...
        node_set_size_pos_in_set_info_map_[item->id()].pos_in_set = pos_in_set_value;

        // Track the largest set size for this OrderedSetContent.
        max_item_set_size_from_attribute = std::max(max_item_set_size_from_attribute, item->GetIntAttribute(ax::mojom::IntAttribute::kSetSize));
    ...
    int32_t set_size_value = std::max(num_elements, max_item_set_size_from_attribute);
    ...
    if (const AXNode* ordered_set = ordered_set_content.ordered_set_) {
        ...
        if (node_set_size_pos_in_set_info_map_.find(ordered_set->id()) == node_set_size_pos_in_set_info_map_.end()) {
            node_set_size_pos_in_set_info_map_[ordered_set->id()] = NodeSetSizePosInSetInfo();
        ...
        node_set_size_pos_in_set_info_map_[ordered_set->id()].set_size = set_size_value;
    }
    ...
    for (const AXNode* item : ordered_set_content.set_items_) {
        ...
        if (item->GetHierarchicalLevel() && item->HasIntAttribute(ax::mojom::IntAttribute::kSetSize))
            node_set_size_pos_in_set_info_map_[item->id()].set_size = item->GetIntAttribute(ax::mojom::IntAttribute::kSetSize);
        else
...
std::optional<int> AXTree::GetPosInSet(const AXNode& node)
{
    ...
    if ((node.GetRole() == ax::mojom::Role::kComboBoxSelect || node.GetRole() == ax::mojom::Role::kPopUpButton) && node.GetUnignoredChildCount() == 0
        && node.HasIntAttribute(ax::mojom::IntAttribute::kPosInSet)) {
        return node.GetIntAttribute(ax::mojom::IntAttribute::kPosInSet);
    }

    if (node_set_size_pos_in_set_info_map_.find(node.id()) != node_set_size_pos_in_set_info_map_.end()) {
    ...
    std::optional<int> pos_in_set = node_set_size_pos_in_set_info_map_[node.id()].pos_in_set;
...
std::optional<int> AXTree::GetSetSize(const AXNode& node)
{
    ...
    if ((!node.IsOrderedSetItem() && !node.IsOrderedSet()) || node.IsEmbeddedGroup()) {
        return std::nullopt;
    }
    ...
    if (node.GetRole() == ax::mojom::Role::kPopUpButton || node.GetRole() == ax::mojom::Role::kComboBoxSelect) {
        const auto& controls_ids = node.GetIntListAttribute(ax::mojom::IntListAttribute::kControlsIds);
        if (controls_ids.size() == 1 && GetFromId(controls_ids[0]) && controls_ids[0] != node.id()) {
            ...
            node_set_size_pos_in_set_info_map_[node.id()].set_size = controlled_item_set_size;
    ...
    ComputeSetSizePosInSetAndCache(node, ordered_set);
    std::optional<int> set_size = node_set_size_pos_in_set_info_map_[node.id()].set_size;
...
void AXTree::NotifyTreeManagerWillBeRemoved(AXTreeID previous_tree_id)
{
    ...
    observers_.Notify(&AXTreeObserver::OnTreeManagerWillBeRemoved, previous_tree_id);
}

void AXTree::RecordError(const AXTreeUpdateState& update_state, std::string new_error, bool is_fatal)
{
    ...
    static auto* const ax_tree_error_key = base::debug::AllocateCrashKeyString("ax_tree_error", base::debug::CrashKeySize::Size256);
    static auto* const ax_tree_update_key = base::debug::AllocateCrashKeyString("ax_tree_update", base::debug::CrashKeySize::Size256);
    static auto* const ax_tree_key = base::debug::AllocateCrashKeyString("ax_tree", base::debug::CrashKeySize::Size256);
    static auto* const ax_tree_data_key = base::debug::AllocateCrashKeyString("ax_tree_data", base::debug::CrashKeySize::Size256);

    // Log additional crash keys so we can debug bad tree updates.
    base::debug::SetCrashKeyString(ax_tree_error_key, new_error);
    base::debug::SetCrashKeyString(ax_tree_update_key, update_state.pending_tree_update->ToString());
    base::debug::SetCrashKeyString(ax_tree_key, TreeToStringHelper(root_, 0, false));
