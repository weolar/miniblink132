...
TEST(AXModeTest, Equality)
{
    ...
    EXPECT_TRUE(AXMode(AXMode::kNone, AXMode::kExperimentalFormControls) == AXMode(AXMode::kNone, AXMode::kExperimentalFormControls));
    EXPECT_FALSE(AXMode(AXMode::kNativeAPIs, AXMode::kExperimentalFormControls) == AXMode(AXMode::kNone, AXMode::kExperimentalFormControls));
}
...
TEST(AXModeTest, Inequality)
{
    ...
    EXPECT_FALSE(AXMode(AXMode::kNone, AXMode::kExperimentalFormControls) != AXMode(AXMode::kNone, AXMode::kExperimentalFormControls));
    EXPECT_TRUE(AXMode(AXMode::kNativeAPIs, AXMode::kExperimentalFormControls) != AXMode(AXMode::kNone, AXMode::kExperimentalFormControls));
}

TEST(AXModeTest, Or)
{
    EXPECT_EQ(kAXModeBasic, AXMode(AXMode::kNativeAPIs) | AXMode(AXMode::kWebContents));
...
TEST(AXModeTest, And)
{
    EXPECT_EQ(kAXModeBasic, AXMode(AXMode::kNativeAPIs | AXMode::kWebContents) & AXMode(AXMode::kNativeAPIs | AXMode::kWebContents));
    EXPECT_EQ(AXMode(), kAXModeBasic & AXMode());
    EXPECT_EQ(kAXModeFormControls, AXMode(std::numeric_limits<uint32_t>::max(), std::numeric_limits<uint32_t>::max()) & kAXModeFormControls);
}
