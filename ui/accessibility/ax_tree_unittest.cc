...
std::string GetBoundsAsString(const AXTree& tree, int32_t id)
{
    ...
    return base::StringPrintf("(%.0f, %.0f) size (%.0f x %.0f)", bounds.x(), bounds.y(), bounds.width(), bounds.height());
}
...
public:
    ...
    void OnNodeDataWillChange(AXTree* tree, const AXNodeData& old_node_data, const AXNodeData& new_node_data) override
    {
    }
    void OnNodeDataChanged(AXTree* tree, const AXNodeData& old_node_data, const AXNodeData& new_node_data) override
    {
    }
    void OnTreeDataChanged(AXTree* tree, const AXTreeData& old_data, const AXTreeData& new_data) override
    {
    ...
    void OnNodeWillBeDeleted(AXTree* tree, AXNode* node) override
    {
        ...
        if (unignored_parent_id_before_node_deleted) {
            ...
            ASSERT_EQ(*unignored_parent_id_before_node_deleted, node->GetUnignoredParent()->id());
        }
    }
    ...
    {
        attribute_change_log_.push_back(base::StringPrintf("Role changed from %s to %s", ToString(old_role), ToString(new_role)));
    }

    void OnIgnoredChanged(AXTree* tree, AXNode* node, bool is_ignored_new_value) override
    {
        attribute_change_log_.push_back(base::StringPrintf("IsIgnored changed on node ID %d to %s", node->id(), is_ignored_new_value ? "true" : "false"));
    }

    void OnStateChanged(AXTree* tree, AXNode* node, ax::mojom::State state, bool new_value) override
    {
        attribute_change_log_.push_back(base::StringPrintf("%s changed to %s", ToString(state), new_value ? "true" : "false"));
    }

    void OnStringAttributeChanged(
        AXTree* tree, AXNode* node, ax::mojom::StringAttribute attr, const std::string& old_value, const std::string& new_value) override
    {
        attribute_change_log_.push_back(base::StringPrintf("%s changed from %s to %s", ToString(attr), old_value.c_str(), new_value.c_str()));
    }
    ...
    {
        attribute_change_log_.push_back(base::StringPrintf("%s changed from %d to %d", ToString(attr), old_value, new_value));
    }
    ...
    void OnFloatAttributeChanged(AXTree* tree, AXNode* node, ax::mojom::FloatAttribute attr, float old_value, float new_value) override
    {
        attribute_change_log_.push_back(
            base::StringPrintf("%s changed from %s to %s", ToString(attr), base::NumberToString(old_value).c_str(), base::NumberToString(new_value).c_str()));
    }

    void OnBoolAttributeChanged(AXTree* tree, AXNode* node, ax::mojom::BoolAttribute attr, bool new_value) override
    {
        attribute_change_log_.push_back(base::StringPrintf("%s changed to %s", ToString(attr), new_value ? "true" : "false"));
    }

    void OnIntListAttributeChanged(
        AXTree* tree, AXNode* node, ax::mojom::IntListAttribute attr, const std::vector<int32_t>& old_value, const std::vector<int32_t>& new_value) override
    {
        attribute_change_log_.push_back(
            base::StringPrintf("%s changed from %s to %s", ToString(attr), IntVectorToString(old_value).c_str(), IntVectorToString(new_value).c_str()));
    }

    bool tree_data_changed() const
    {
        return tree_data_changed_;
    }
    bool root_changed() const
    {
        return root_changed_;
    }
    const std::vector<int32_t>& deleted_ids()
    {
        return deleted_ids_;
    }
    ...
    const std::vector<int32_t>& created_ids()
    {
        return created_ids_;
    }
...
class AXTreeTestWithMultipleUTFEncodings : public ::testing::TestWithParam<TestEncoding> {
public:
    AXTreeTestWithMultipleUTFEncodings() = default;
    ~AXTreeTestWithMultipleUTFEncodings() override = default;
    AXTreeTestWithMultipleUTFEncodings(const AXTreeTestWithMultipleUTFEncodings& other) = delete;
    AXTreeTestWithMultipleUTFEncodings& operator=(const AXTreeTestWithMultipleUTFEncodings& other) = delete;
};
...
TEST(AXTreeTest, SerializeSimpleAXTree)
{
    ...
    std::unique_ptr<AXTreeSource<const AXNode*, AXTreeData*, AXNodeData>> tree_source(src_tree.CreateTreeSource());
...
TEST(AXTreeTest, SerializeAXTreeUpdate)
{
    ...
    EXPECT_EQ("AXTreeUpdate: root id 3\n"
...
TEST(AXTreeTest, LeaveOrphanedDeletedSubtreeFails)
{
    ...
    histogram_tester.ExpectTotalCount("Accessibility.Performance.Tree.Unserialize2", 1);
    ...
    EXPECT_DEATH_IF_SUPPORTED(tree.Unserialize(update), "Nodes left pending by the update: 2");
#else
    EXPECT_FALSE(tree.Unserialize(update));
    ASSERT_EQ("Nodes left pending by the update: 2", tree.error());
    histogram_tester.ExpectUniqueSample("Accessibility.Reliability.Tree.UnserializeError", AXTreeUnserializeError::kPendingNodes, 1);
    histogram_tester.ExpectTotalCount("Accessibility.Performance.Tree.Unserialize2", 2);
#endif
}
...
TEST(AXTreeTest, DuplicateChildIdFails)
{
    ...
    update.nodes[1].id = 2;
#if defined(AX_FAIL_FAST_BUILD)
    EXPECT_DEATH_IF_SUPPORTED(tree.Unserialize(update), "Node 1 has duplicate child id 2");
#else
    EXPECT_FALSE(tree.Unserialize(update));
    ASSERT_EQ("Node 1 has duplicate child id 2", tree.error());
    histogram_tester.ExpectUniqueSample("Accessibility.Reliability.Tree.UnserializeError", AXTreeUnserializeError::kDuplicateChild, 1);
    histogram_tester.ExpectTotalCount("Accessibility.Performance.Tree.Unserialize2", 1);
#endif
}
...
TEST(AXTreeTest, InvalidReparentingFails)
{
    ...
    update.nodes[2].id = 3;
#if defined(AX_FAIL_FAST_BUILD)
    EXPECT_DEATH_IF_SUPPORTED(tree.Unserialize(update), "Node 3 is not marked for destruction, would be reparented to 1");
#else
    EXPECT_FALSE(tree.Unserialize(update));
    ASSERT_EQ("Node 3 is not marked for destruction, would be reparented to 1", tree.error());
    histogram_tester.ExpectUniqueSample("Accessibility.Reliability.Tree.UnserializeError", AXTreeUnserializeError::kReparent, 1);
    histogram_tester.ExpectTotalCount("Accessibility.Performance.Tree.Unserialize2", 1);
#endif
}
...
TEST(AXTreeTest, ImplicitAttributeDelete)
{
    ...
    EXPECT_NE(tree.GetFromId(1), nullptr);
    EXPECT_EQ(tree.GetFromId(1)->GetStringAttribute(ax::mojom::StringAttribute::kName), "Node 1 name");
    ...
    EXPECT_NE(tree.GetFromId(1), nullptr);
    EXPECT_FALSE(tree.GetFromId(1)->HasStringAttribute(ax::mojom::StringAttribute::kName));
}
...
TEST(AXTreeTest, ReparentingDoesNotTriggerNodeCreated)
{
    ...
    std::vector<int> subtree_reparented = test_observer.subtree_reparented_finished_ids();
    std::vector<int> node_reparented = test_observer.node_reparented_finished_ids();
...
TEST(AXTreeTest, MultipleIgnoredChangesDoesNotBreakCache)
{
    ...
    TestAXTreeObserver test_observer(&tree);
#if DCHECK_IS_ON()
    EXPECT_DEATH_IF_SUPPORTED(tree.GetFromId(2)->GetUnignoredChildCount(), "Called unignored method on ignored node");
#else
...
TEST(AXTreeTest, DISABLED_BogusAXTree)
{
    ...
    AXTree tree;
#if DCHECK_IS_ON()
    EXPECT_DEATH_IF_SUPPORTED(tree.Unserialize(initial_state), "AXTreeUpdate contains invalid node");
#else
...
TEST(AXTreeTest, BogusAXTree2)
{
    ...
    AXTree tree;
#if defined(AX_FAIL_FAST_BUILD)
    EXPECT_DEATH_IF_SUPPORTED(tree.Unserialize(initial_state), "Node 1 has duplicate child id 1");
#else
...
TEST(AXTreeTest, BogusAXTree3)
{
    ...
    AXTree tree;
#if defined(AX_FAIL_FAST_BUILD)
    EXPECT_DEATH_IF_SUPPORTED(tree.Unserialize(initial_state), "Node 1 has duplicate child id 2");
#else
    EXPECT_FALSE(tree.Unserialize(initial_state));
...
TEST(AXTreeTest, RoleAndStateChangeCallbacks)
{
    ...
    const std::vector<std::string>& change_log = test_observer.attribute_change_log();
...
TEST(AXTreeTest, AttributeChangeCallbacks)
{
    ...
    initial_state.nodes.resize(1);
    initial_state.nodes[0].id = 1;
    initial_state.nodes[0].AddStringAttribute(ax::mojom::StringAttribute::kName, "N1");
    initial_state.nodes[0].AddStringAttribute(ax::mojom::StringAttribute::kDescription, "D1");
    initial_state.nodes[0].AddBoolAttribute(ax::mojom::BoolAttribute::kLiveAtomic, true);
    initial_state.nodes[0].AddBoolAttribute(ax::mojom::BoolAttribute::kBusy, false);
    initial_state.nodes[0].AddFloatAttribute(ax::mojom::FloatAttribute::kMinValueForRange, 1.0);
    initial_state.nodes[0].AddFloatAttribute(ax::mojom::FloatAttribute::kMaxValueForRange, 10.0);
    initial_state.nodes[0].AddFloatAttribute(ax::mojom::FloatAttribute::kStepValueForRange, 3.0);
    initial_state.nodes[0].AddIntAttribute(ax::mojom::IntAttribute::kScrollX, 5);
    initial_state.nodes[0].AddIntAttribute(ax::mojom::IntAttribute::kScrollXMin, 1);
    AXTree tree(initial_state);
    ...
    update0.nodes[0].AddStringAttribute(ax::mojom::StringAttribute::kDescription, "D2");
    update0.nodes[0].AddBoolAttribute(ax::mojom::BoolAttribute::kLiveAtomic, false);
    update0.nodes[0].AddBoolAttribute(ax::mojom::BoolAttribute::kBusy, true);
    update0.nodes[0].AddFloatAttribute(ax::mojom::FloatAttribute::kMinValueForRange, 2.0);
    update0.nodes[0].AddFloatAttribute(ax::mojom::FloatAttribute::kMaxValueForRange, 9.0);
    update0.nodes[0].AddFloatAttribute(ax::mojom::FloatAttribute::kStepValueForRange, 0.5);
    ...
    update1.nodes[0].AddStringAttribute(ax::mojom::StringAttribute::kDescription, "D3");
    ...
    update1.nodes[0].AddFloatAttribute(ax::mojom::FloatAttribute::kValueForRange, 5.0);
    update1.nodes[0].AddFloatAttribute(ax::mojom::FloatAttribute::kMaxValueForRange, 9.0);
    ...
    const std::vector<std::string>& change_log2 = test_observer2.attribute_change_log();
...
TEST(AXTreeTest, IntListChangeCallbacks)
{
    ...
    initial_state.nodes.resize(1);
    initial_state.nodes[0].id = 1;
    initial_state.nodes[0].AddIntListAttribute(ax::mojom::IntListAttribute::kControlsIds, one);
    initial_state.nodes[0].AddIntListAttribute(ax::mojom::IntListAttribute::kRadioGroupIds, two);
    AXTree tree(initial_state);
    ...
    update0.nodes.resize(1);
    update0.nodes[0].id = 1;
    update0.nodes[0].AddIntListAttribute(ax::mojom::IntListAttribute::kControlsIds, two);
    update0.nodes[0].AddIntListAttribute(ax::mojom::IntListAttribute::kRadioGroupIds, three);
    EXPECT_TRUE(tree.Unserialize(update0));
    ...
    update1.nodes.resize(1);
    update1.nodes[0].id = 1;
    update1.nodes[0].AddIntListAttribute(ax::mojom::IntListAttribute::kRadioGroupIds, two);
    update1.nodes[0].AddIntListAttribute(ax::mojom::IntListAttribute::kFlowtoIds, three);
    EXPECT_TRUE(tree.Unserialize(update1));
...
TEST(AXTreeTest, EmptyNodeBoundsIsUnionOfChildren)
{
    ...
    tree_update.nodes[0].child_ids.push_back(2);
    tree_update.nodes[1].id = 2;
    tree_update.nodes[1].relative_bounds.bounds = gfx::RectF(); // Deliberately empty.
...
TEST(AXTreeTest, EmptyNodeNotOffscreenEvenIfAllChildrenOffscreen)
{
    ...
    tree_update.nodes[0].AddBoolAttribute(ax::mojom::BoolAttribute::kClipsChildren, true);
    tree_update.nodes[0].child_ids.push_back(2);
    tree_update.nodes[1].id = 2;
    tree_update.nodes[1].relative_bounds.bounds = gfx::RectF(); // Deliberately empty.
    tree_update.nodes[1].child_ids.push_back(3);
...
TEST(AXTreeTest, GetBoundsWithTransform)
{
    ...
    tree_update.nodes[0].relative_bounds.transform = std::make_unique<gfx::Transform>();
    ...
    tree_update.nodes[2].relative_bounds.transform = std::make_unique<gfx::Transform>();
...
TEST(AXTreeTest, GetBoundsEmptyBoundsInheritsFromParent)
{
    ...
    tree_update.nodes[0].id = 1;
    tree_update.nodes[0].relative_bounds.bounds = gfx::RectF(0, 0, 800, 600);
    tree_update.nodes[1].AddBoolAttribute(ax::mojom::BoolAttribute::kClipsChildren, true);
    tree_update.nodes[0].child_ids.push_back(2);
    tree_update.nodes[1].id = 2;
...
TEST(AXTreeTest, GetBoundsCropsChildToRoot)
{
    ...
    tree_update.nodes[1].id = 2;
    tree_update.nodes[1].relative_bounds.bounds = gfx::RectF(-100, -100, 150, 150);
    ...
    EXPECT_EQ("(-100, -100) size (150 x 150)", GetUnclippedBoundsAsString(tree, 2));
...
TEST(AXTreeTest, IntReverseRelations)
{
    ...
    initial_state.nodes[1].id = 2;
    initial_state.nodes[2].id = 3;
    initial_state.nodes[2].AddIntAttribute(ax::mojom::IntAttribute::kMemberOfId, 1);
    initial_state.nodes[3].id = 4;
    initial_state.nodes[3].AddIntAttribute(ax::mojom::IntAttribute::kMemberOfId, 1);
    AXTree tree(initial_state);

    auto reverse_active_descendant = tree.GetReverseRelations(ax::mojom::IntAttribute::kActivedescendantId, 2);
    ASSERT_EQ(1U, reverse_active_descendant.size());
    EXPECT_TRUE(base::Contains(reverse_active_descendant, 1));

    reverse_active_descendant = tree.GetReverseRelations(ax::mojom::IntAttribute::kActivedescendantId, 1);
    ASSERT_EQ(0U, reverse_active_descendant.size());

    // Member of does not compute a reverse relation.
    auto reverse_member_of = tree.GetReverseRelations(ax::mojom::IntAttribute::kMemberOfId, 1);
    ASSERT_EQ(0U, reverse_member_of.size());

    AXTreeUpdate update = initial_state;
    update.nodes.resize(5);
    update.nodes[0].int_attributes.clear();
    update.nodes[0].AddIntAttribute(ax::mojom::IntAttribute::kActivedescendantId, 5);
    ...
    reverse_active_descendant = tree.GetReverseRelations(ax::mojom::IntAttribute::kActivedescendantId, 2);
    ASSERT_EQ(0U, reverse_active_descendant.size());

    reverse_active_descendant = tree.GetReverseRelations(ax::mojom::IntAttribute::kActivedescendantId, 5);
    ASSERT_EQ(1U, reverse_active_descendant.size());
    EXPECT_TRUE(base::Contains(reverse_active_descendant, 1));

    reverse_member_of = tree.GetReverseRelations(ax::mojom::IntAttribute::kMemberOfId, 1);
    ASSERT_EQ(0U, reverse_member_of.size());
}
...
TEST(AXTreeTest, DeletingNodeUpdatesReverseRelations)
{
    ...
    initial_state.nodes[1].id = 2;
    initial_state.nodes[2].id = 3;
    initial_state.nodes[2].AddIntAttribute(ax::mojom::IntAttribute::kActivedescendantId, 2);
    AXTree tree(initial_state);
...
TEST(AXTreeTest, ReverseRelationsDoNotKeepGrowing)
{
    ...
    initial_state.nodes[0].id = 1;
    initial_state.nodes[0].AddIntAttribute(ax::mojom::IntAttribute::kActivedescendantId, 2);
    initial_state.nodes[0].AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, { 2 });
    initial_state.nodes[0].child_ids.push_back(2);
    initial_state.nodes[1].id = 2;
    AXTree tree(initial_state);
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        update.nodes[0].AddIntAttribute(ax::mojom::IntAttribute::kActivedescendantId, update.nodes[1].id);
        update.nodes[0].AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, { update.nodes[1].id });
...
TEST(AXTreeTest, UnignoredChildIteratorIncrementDecrementPastEnd)
{
    ...
    {
        {
            AXNode::UnignoredChildIterator root_unignored_iter = root->UnignoredChildrenBegin();
            EXPECT_EQ(2, root_unignored_iter->id());
            EXPECT_EQ("text1", root_unignored_iter->GetStringAttribute(ax::mojom::StringAttribute::kName));
...
TEST(AXTreeTest, UnignoredChildIteratorIgnoredContainerSiblings)
{
...

    {
        ...
        EXPECT_EQ("text2", (*root_unignored_iter).GetStringAttribute(ax::mojom::StringAttribute::kName));

        EXPECT_EQ(7, (++root_unignored_iter)->id());
        EXPECT_EQ("text3", root_unignored_iter->GetStringAttribute(ax::mojom::StringAttribute::kName));
        EXPECT_EQ(root->UnignoredChildrenEnd(), ++root_unignored_iter);
    }

    {
        ...
        AXNode::UnignoredChildIterator unignored_iter = text1_ignored_container->UnignoredChildrenBegin();
        EXPECT_EQ(3, unignored_iter->id());
        EXPECT_EQ("text1", unignored_iter->GetStringAttribute(ax::mojom::StringAttribute::kName));
        // The next child of "text1" should be the end.
        EXPECT_EQ(text1_ignored_container->UnignoredChildrenEnd(), ++unignored_iter);
        ...
        EXPECT_EQ("text2", unignored_iter->GetStringAttribute(ax::mojom::StringAttribute::kName));
...
TEST(AXTreeTest, UnignoredChildIterator)
{
    ...
    AXNode::UnignoredChildIterator unignored_iterator = root->UnignoredChildrenBegin();
    ...
    AXNode::UnignoredChildIterator unignored_iterator2 = root->UnignoredChildrenBegin();
...
TEST(AXTreeTest, UnignoredNextPreviousChild)
{
    ...
    EXPECT_EQ(tree.GetFromId(12), tree.GetFromId(3)->GetPreviousUnignoredSibling());

    EXPECT_EQ(nullptr, tree.GetFromId(4)->GetNextUnignoredSibling());
    EXPECT_EQ(tree.GetFromId(3), tree.GetFromId(4)->GetPreviousUnignoredSibling());
    ...
    EXPECT_EQ(tree.GetFromId(5), tree.GetFromId(6)->GetPreviousUnignoredSibling());

    EXPECT_EQ(tree.GetFromId(3), tree.GetFromId(7)->GetNextUnignoredSibling());
    EXPECT_EQ(tree.GetFromId(6), tree.GetFromId(7)->GetPreviousUnignoredSibling());

    EXPECT_EQ(tree.GetFromId(3), tree.GetFromId(8)->GetNextUnignoredSibling());
    EXPECT_EQ(tree.GetFromId(12), tree.GetFromId(8)->GetPreviousUnignoredSibling());
    ...
    EXPECT_EQ(tree.GetFromId(6), tree.GetFromId(11)->GetPreviousUnignoredSibling());

    EXPECT_EQ(tree.GetFromId(3), tree.GetFromId(12)->GetNextUnignoredSibling());
    EXPECT_EQ(tree.GetFromId(15), tree.GetFromId(12)->GetPreviousUnignoredSibling());
    ...
    EXPECT_EQ(tree.GetFromId(6), tree.GetFromId(14)->GetPreviousUnignoredSibling());

    EXPECT_EQ(tree.GetFromId(12), tree.GetFromId(15)->GetNextUnignoredSibling());
    EXPECT_EQ(tree.GetFromId(14), tree.GetFromId(15)->GetPreviousUnignoredSibling());
...
TEST(AXTreeTest, GetSiblingsNoIgnored)
{
    ...
    EXPECT_EQ(tree.GetFromId(2), tree.GetFromId(3)->GetPreviousUnignoredSibling());
...
TEST(AXTreeTest, GetUnignoredSiblingsChildrenPromoted)
{
    ...
    EXPECT_EQ(tree.GetFromId(2), tree.GetFromId(3)->GetPreviousSibling());
    EXPECT_EQ(tree.GetFromId(5), tree.GetFromId(3)->GetPreviousUnignoredSibling());
    EXPECT_EQ(nullptr, tree.GetFromId(3)->GetNextSibling());
    EXPECT_EQ(nullptr, tree.GetFromId(3)->GetNextUnignoredSibling());
...
TEST(AXTreeTest, GetUnignoredSiblingsIgnoredChildSkipped)
{
    ...
    EXPECT_EQ(tree.GetFromId(2), tree.GetFromId(3)->GetPreviousSibling());
    EXPECT_EQ(tree.GetFromId(4), tree.GetFromId(3)->GetPreviousUnignoredSibling());
    EXPECT_EQ(nullptr, tree.GetFromId(3)->GetNextSibling());
    EXPECT_EQ(nullptr, tree.GetFromId(3)->GetNextUnignoredSibling());
...
TEST_P(AXTreeTestWithMultipleUTFEncodings, ComputedNodeData)
{
    ...
    rich_text_field.AddBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot, true);
    ...
    rich_text_field_text_container.child_ids = { rich_text_field_line_1.id, rich_text_field_line_break.id, rich_text_field_line_2.id };
    ...
    update.root_id = root.id;
    update.nodes = { root, rich_text_field, rich_text_field_text_container, rich_text_field_line_1, rich_text_field_line_break, rich_text_field_line_2,
        paragraph, paragraph_span, paragraph_span_text, paragraph_link, paragraph_link_text };
    ...
    if (GetParam() == TestEncoding::kUTF8) {
        EXPECT_EQ("Line 1\nLine 2", tree.root()->GetChildAtIndex(0)->GetTextContentUTF8());
        EXPECT_EQ(13, tree.root()->GetChildAtIndex(0)->GetTextContentLengthUTF8());
    } else if (GetParam() == TestEncoding::kUTF16) {
        EXPECT_EQ(u"Line 1\nLine 2", tree.root()->GetChildAtIndex(0)->GetTextContentUTF16());
        EXPECT_EQ(13, tree.root()->GetChildAtIndex(0)->GetTextContentLengthUTF16());
    }

    if (GetParam() == TestEncoding::kUTF8) {
        EXPECT_EQ("Link text", tree.root()->GetChildAtIndex(1)->GetTextContentUTF8());
        EXPECT_EQ(9, tree.root()->GetChildAtIndex(1)->GetTextContentLengthUTF8());
    } else if (GetParam() == TestEncoding::kUTF16) {
        EXPECT_EQ(u"Link text", tree.root()->GetChildAtIndex(1)->GetTextContentUTF16());
    ...
    rich_text_field_text_container.child_ids = { rich_text_field_line_1.id, rich_text_field_line_break.id };

    AXTreeUpdate update_2;
    update_2.node_id_to_clear = rich_text_field_line_2.id;
    update_2.nodes = { rich_text_field_text_container, rich_text_field_line_break, paragraph_span, paragraph_span_text, paragraph_link };
    ...
    if (GetParam() == TestEncoding::kUTF8) {
        ...
        EXPECT_EQ(u"Line 1", tree.root()->GetChildAtIndex(0)->GetTextContentUTF16());
        EXPECT_EQ(6, tree.root()->GetChildAtIndex(0)->GetTextContentLengthUTF16());
    }

    if (GetParam() == TestEncoding::kUTF8) {
        EXPECT_EQ("span textLink text", tree.root()->GetChildAtIndex(1)->GetTextContentUTF8());
        EXPECT_EQ(18, tree.root()->GetChildAtIndex(1)->GetTextContentLengthUTF8());
    } else if (GetParam() == TestEncoding::kUTF16) {
        EXPECT_EQ(u"span textLink text", tree.root()->GetChildAtIndex(1)->GetTextContentUTF16());
        EXPECT_EQ(18, tree.root()->GetChildAtIndex(1)->GetTextContentLengthUTF16());
    }

    const std::vector<std::string>& change_log = test_observer.attribute_change_log();
    EXPECT_THAT(change_log,
        ElementsAre("IsIgnored changed on node ID 5 to true", "IsIgnored changed on node ID 8 to false", "IsIgnored changed on node ID 9 to false",
            "IsIgnored changed on node ID 10 to true"));
}

INSTANTIATE_TEST_SUITE_P(MultipleUTFEncodingTest, AXTreeTestWithMultipleUTFEncodings, ::testing::Values(TestEncoding::kUTF8, TestEncoding::kUTF16));
...
TEST(AXTreeTest, OrderedSetReportsSetSize)
{
    ...
    tree_update.nodes[8].id = 9;
    tree_update.nodes[8].role = ax::mojom::Role::kList; // SetSize = 1 because only 1 item whose role
        // matches
...
TEST(AXTreeTest, SetSizePosInSetRadioButtons)
{
    ...
    tree_update.nodes[1].AddStringAttribute(ax::mojom::StringAttribute::kName, "sports");
    ...
    tree_update.nodes[2].AddStringAttribute(ax::mojom::StringAttribute::kName, "books");
    ...
    tree_update.nodes[12].AddStringAttribute(ax::mojom::StringAttribute::kName, "sports");
...
TEST(AXTreeTest, SetSizePosInSetRadioButtonsInList)
{
    ...
    tree_update.nodes.resize(6);
    tree_update.nodes[0].id = 1;
    tree_update.nodes[0].role = ax::mojom::Role::kList; // SetSize = 2, since only contains 2 ListItems
    tree_update.nodes[0].child_ids = { 2, 3, 4, 5, 6 };
...
TEST(AXTreeTest, SetSizePosInSetFlatTree)
{
    ...
    tree_update.nodes[1].AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, 1);
    tree_update.nodes[2].id = 3;
    tree_update.nodes[2].role = ax::mojom::Role::kTreeItem; // 1 of 1
    tree_update.nodes[2].AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, 2);
    tree_update.nodes[3].id = 4;
    tree_update.nodes[3].role = ax::mojom::Role::kTreeItem; // 1 of 1
    tree_update.nodes[3].AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, 3);
    AXTree tree(tree_update);
...
TEST(AXTreeTest, SetSizePosInSetFlatTreeLevelsOnly)
{
    ...
    tree_update.nodes[3].AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, 2);
    tree_update.nodes[4].id = 5;
    tree_update.nodes[4].role = ax::mojom::Role::kTreeItem; // 2 of 3
    tree_update.nodes[4].AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, 1);
    tree_update.nodes[5].id = 6;
    tree_update.nodes[5].role = ax::mojom::Role::kTreeItem; // 1 of 3
    tree_update.nodes[5].AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, 2);
    tree_update.nodes[6].id = 7;
    tree_update.nodes[6].role = ax::mojom::Role::kTreeItem; // 2 of 3
    tree_update.nodes[6].AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, 2);
    tree_update.nodes[7].id = 8;
    tree_update.nodes[7].role = ax::mojom::Role::kTreeItem; // 3 of 3
    tree_update.nodes[7].AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, 2);
    tree_update.nodes[8].id = 9;
    tree_update.nodes[8].role = ax::mojom::Role::kTreeItem; // 3 of 3
    tree_update.nodes[8].AddIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, 1);
    AXTree tree(tree_update);
...
TEST(AXTreeTest, SetSizePosInSetControls)
{
    ...
    tree_update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kControlsIds, three);
    ...
    tree_update.nodes[6].id = 7;
    tree_update.nodes[6].role = ax::mojom::Role::kPopUpButton; // Test an invalid controls id.
    tree_update.nodes[6].AddIntListAttribute(ax::mojom::IntListAttribute::kControlsIds, hundred);
    ...
    tree_update.nodes[7].AddIntListAttribute(ax::mojom::IntListAttribute::kControlsIds, eight);
    AXTree tree(tree_update);
...
TEST(AXTreeTest, OnNodeHasBeenDeleted)
{
    ...
    for (auto id : test_observer.deleted_ids()) {
        SCOPED_TRACE(::testing::Message() << "Node with id=" << id << ", should not exist in the tree");
...
#if defined(AX_FAIL_FAST_BUILD)
    EXPECT_DEATH_IF_SUPPORTED(tree.Unserialize(tree_update_3), "2 will not be in the tree and is not the new root");
#else
    EXPECT_FALSE(tree.Unserialize(tree_update_3));
    EXPECT_EQ("2 will not be in the tree and is not the new root", tree.error());
    histogram_tester.ExpectUniqueSample("Accessibility.Reliability.Tree.UnserializeError", AXTreeUnserializeError::kNotInTree, 1);
#endif
}
