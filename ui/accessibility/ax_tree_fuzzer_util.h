...
private:
    ...
    enum TreeUpdateOperation { kAddChild, kRemoveNode, kTextChange, kNoOperation };

    void RecursiveGenerateUpdate(const ui::AXNode* node, ui::AXTreeUpdate& tree_update, FuzzerData& fuzz_data, std::set<ui::AXNodeID>& updated_nodes);
    // TODO(janewman): Many of these can be made static.
    ui::AXNodeData CreateChildNodeData(ui::AXNodeData& parent, ui::AXNodeID new_node_id);
    NextNodeRelationship DetermineNextNodeRelationship(ax::mojom::Role role, unsigned char byte);
    TreeUpdateOperation DetermineTreeUpdateOperation(const ui::AXNode* node, unsigned char byte);
    void AddRoleSpecificProperties(FuzzerData& fuzz_data, ui::AXNodeData& node, const std::string& parentName, size_t extra_data_size);
    ax::mojom::Role GetInterestingRole(unsigned char byte, ax::mojom::Role parent_role);
    bool CanHaveChildren(ax::mojom::Role role);
    bool CanHaveText(ax::mojom::Role role);
    std::u16string GenerateInterestingText(const unsigned char* data, size_t size);
