...
TEST(AXNodeTest, TreeWalking)
{
    ...
    root.child_ids = { paragraph_0.id, paragraph_1_ignored.id, paragraph_2.id, paragraph_3_ignored.id };
    ...
    initial_state.nodes = { root, paragraph_0, static_text_0_0_ignored, paragraph_1_ignored, static_text_1_0, paragraph_2, static_text_2_0, paragraph_3_ignored,
        link_3_0_ignored, static_text_3_0_0, button_3_1 };
    ...
    EXPECT_THAT(root_node->GetAllChildren(),
        ElementsAre(HasAXNodeID(paragraph_0), HasAXNodeID(paragraph_1_ignored), HasAXNodeID(paragraph_2), HasAXNodeID(paragraph_3_ignored)));
    ...
    EXPECT_EQ(paragraph_0.id, root_node->GetChildAtIndexCrossingTreeBoundary(0)->id());
    EXPECT_EQ(paragraph_1_ignored.id, root_node->GetChildAtIndexCrossingTreeBoundary(1)->id());
    EXPECT_EQ(paragraph_2.id, root_node->GetChildAtIndexCrossingTreeBoundary(2)->id());
    EXPECT_EQ(paragraph_3_ignored.id, root_node->GetChildAtIndexCrossingTreeBoundary(3)->id());
    ...
    EXPECT_EQ(paragraph_0.id, root_node->GetUnignoredChildAtIndexCrossingTreeBoundary(0)->id());
    EXPECT_EQ(static_text_1_0.id, root_node->GetUnignoredChildAtIndexCrossingTreeBoundary(1)->id());
    EXPECT_EQ(paragraph_2.id, root_node->GetUnignoredChildAtIndexCrossingTreeBoundary(2)->id());
    EXPECT_EQ(static_text_3_0_0.id, root_node->GetUnignoredChildAtIndexCrossingTreeBoundary(3)->id());
    EXPECT_EQ(button_3_1.id, root_node->GetUnignoredChildAtIndexCrossingTreeBoundary(4)->id());
    EXPECT_EQ(nullptr, root_node->GetUnignoredChildAtIndexCrossingTreeBoundary(5));
    ...
    EXPECT_EQ(root_node, tree.GetFromId(paragraph_0.id)->GetParentCrossingTreeBoundary());
    EXPECT_EQ(root_node, tree.GetFromId(paragraph_0.id)->GetUnignoredParent());
    EXPECT_EQ(root_node, tree.GetFromId(paragraph_0.id)->GetUnignoredParentCrossingTreeBoundary());

    EXPECT_EQ(tree.GetFromId(paragraph_1_ignored.id), tree.GetFromId(static_text_1_0.id)->GetParent());
    EXPECT_EQ(tree.GetFromId(paragraph_1_ignored.id), tree.GetFromId(static_text_1_0.id)->GetParentCrossingTreeBoundary());
    EXPECT_EQ(root_node, tree.GetFromId(static_text_1_0.id)->GetUnignoredParent());
    EXPECT_EQ(root_node, tree.GetFromId(static_text_1_0.id)->GetUnignoredParentCrossingTreeBoundary());
    ...
    EXPECT_EQ(1u, tree.GetFromId(static_text_1_0.id)->GetUnignoredIndexInParent());
    EXPECT_EQ(2u, tree.GetFromId(paragraph_2.id)->GetUnignoredIndexInParent());

    EXPECT_EQ(paragraph_0.id, root_node->GetFirstChild()->id());
    EXPECT_EQ(paragraph_0.id, root_node->GetFirstChildCrossingTreeBoundary()->id());
    EXPECT_EQ(paragraph_0.id, root_node->GetFirstUnignoredChild()->id());
    EXPECT_EQ(paragraph_0.id, root_node->GetFirstUnignoredChildCrossingTreeBoundary()->id());

    EXPECT_EQ(paragraph_3_ignored.id, root_node->GetLastChild()->id());
    EXPECT_EQ(paragraph_3_ignored.id, root_node->GetLastChildCrossingTreeBoundary()->id());
    EXPECT_EQ(button_3_1.id, root_node->GetLastUnignoredChild()->id());
    EXPECT_EQ(button_3_1.id, root_node->GetLastUnignoredChildCrossingTreeBoundary()->id());

    EXPECT_EQ(static_text_0_0_ignored.id, root_node->GetDeepestFirstDescendant()->id());
    EXPECT_EQ(paragraph_0.id, root_node->GetDeepestFirstUnignoredDescendant()->id());

    EXPECT_EQ(button_3_1.id, root_node->GetDeepestLastDescendant()->id());
    EXPECT_EQ(button_3_1.id, root_node->GetDeepestLastUnignoredDescendant()->id());

    {
        std::vector<AXNode*> siblings;
        for (AXNode* sibling = tree.GetFromId(paragraph_0.id); sibling; sibling = sibling->GetNextSibling()) {
            siblings.push_back(sibling);
        }
        EXPECT_THAT(
            siblings, ElementsAre(HasAXNodeID(paragraph_0), HasAXNodeID(paragraph_1_ignored), HasAXNodeID(paragraph_2), HasAXNodeID(paragraph_3_ignored)));
    }

    {
        std::vector<AXNode*> siblings;
        for (AXNode* sibling = tree.GetFromId(paragraph_0.id); sibling; sibling = sibling->GetNextUnignoredSibling()) {
            siblings.push_back(sibling);
        }
        EXPECT_THAT(siblings,
            ElementsAre(
                HasAXNodeID(paragraph_0), HasAXNodeID(static_text_1_0), HasAXNodeID(paragraph_2), HasAXNodeID(static_text_3_0_0), HasAXNodeID(button_3_1)));
    }

    {
        std::vector<AXNode*> siblings;
        for (AXNode* sibling = tree.GetFromId(paragraph_3_ignored.id); sibling; sibling = sibling->GetPreviousSibling()) {
            siblings.push_back(sibling);
        }
        EXPECT_THAT(
            siblings, ElementsAre(HasAXNodeID(paragraph_3_ignored), HasAXNodeID(paragraph_2), HasAXNodeID(paragraph_1_ignored), HasAXNodeID(paragraph_0)));
    }

    {
        std::vector<AXNode*> siblings;
        for (AXNode* sibling = tree.GetFromId(button_3_1.id); sibling; sibling = sibling->GetPreviousUnignoredSibling()) {
            siblings.push_back(sibling);
        }
        EXPECT_THAT(siblings,
            ElementsAre(
                HasAXNodeID(button_3_1), HasAXNodeID(static_text_3_0_0), HasAXNodeID(paragraph_2), HasAXNodeID(static_text_1_0), HasAXNodeID(paragraph_0)));
    }

    {
        std::vector<AXNode::AllChildIterator> siblings;
        for (auto iter = root_node->AllChildrenBegin(); iter != root_node->AllChildrenEnd(); ++iter) {
...

    {
        ...
        for (auto iter = root_node->AllChildrenCrossingTreeBoundaryBegin(); iter != root_node->AllChildrenCrossingTreeBoundaryEnd(); ++iter) {
            siblings.push_back(iter);
        }
        EXPECT_THAT(
            siblings, ElementsAre(HasAXNodeID(paragraph_0), HasAXNodeID(paragraph_1_ignored), HasAXNodeID(paragraph_2), HasAXNodeID(paragraph_3_ignored)));
...

    {
        ...
        for (auto iter = root_node->UnignoredChildrenBegin(); iter != root_node->UnignoredChildrenEnd(); ++iter) {
            siblings.push_back(iter);
        }
        EXPECT_THAT(siblings,
            ElementsAre(
...

    {
        ...
        for (auto iter = root_node->UnignoredChildrenCrossingTreeBoundaryBegin(); iter != root_node->UnignoredChildrenCrossingTreeBoundaryEnd(); ++iter) {
            siblings.push_back(iter);
        }
        EXPECT_THAT(siblings,
            ElementsAre(
...
TEST(AXNodeTest, TreeWalkingCrossingTreeBoundary)
{
    ...
    EXPECT_EQ(root_node_2, root_node_1->GetUnignoredChildAtIndexCrossingTreeBoundary(0));
...
TEST(AXNodeTest, GetValueForControlTextField)
{
    ...
    rich_text_field_text_container.child_ids
        = { rich_text_field_line_1.id, rich_text_field_image.id, rich_text_field_line_break.id, rich_text_field_line_2.id };
    ...
    update.nodes = { root, rich_text_field, rich_text_field_text_container, rich_text_field_line_1, rich_text_field_image, rich_text_field_line_break,
    ...
    {
        const AXNode* text_field_node = manager.GetTree()->GetFromId(rich_text_field.id);
    ...
    rich_text_field.RemoveBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot);
    AXTreeUpdate update_2;
    update_2.nodes = { rich_text_field };

    ASSERT_TRUE(manager.GetTree()->Unserialize(update_2)) << manager.GetTree()->error();
    {
        const AXNode* text_field_node = manager.GetTree()->GetFromId(rich_text_field.id);
        ASSERT_NE(nullptr, text_field_node);
    ...
    ASSERT_TRUE(manager.GetTree()->Unserialize(update_3)) << manager.GetTree()->error();
    {
        const AXNode* text_field_node = manager.GetTree()->GetFromId(rich_text_field.id);
        ASSERT_NE(nullptr, text_field_node);
        EXPECT_EQ("Line 1\nLine 2", text_field_node->GetValueForControl());
...
TEST(AXNodeTest, GetLowestPlatformAncestor)
{
    ...
    AXTreeUpdate initial_state;
    initial_state.root_id = root.id;
    initial_state.nodes = { root, button, generic_container, static_text_1, inline_box_1, text_field, static_text_2, inline_box_2 };
    initial_state.has_tree_data = true;
    ...
    EXPECT_EQ(static_text_1_node, static_text_1_node->GetLowestPlatformAncestor());
...
TEST(AXNodeTest, GetTextContentRangeBounds)
{
    ...
    text_data1.AddIntAttribute(ax::mojom::IntAttribute::kTextDirection, static_cast<int32_t>(ax::mojom::WritingDirection::kLtr));
    text_data1.AddIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets, kEnglishCharacterOffsets);

    AXNodeData text_data2;
    text_data2.id = 3;
    text_data2.role = ax::mojom::Role::kStaticText;
    text_data2.SetName(kHindiText);
    text_data2.AddIntAttribute(ax::mojom::IntAttribute::kTextDirection, static_cast<int32_t>(ax::mojom::WritingDirection::kRtl));
    text_data2.AddIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets, kHindiCharacterOffsets);

    AXNodeData text_data3;
    text_data3.id = 4;
    text_data3.role = ax::mojom::Role::kStaticText;
    text_data3.SetName(kThaiText);
    text_data3.AddIntAttribute(ax::mojom::IntAttribute::kTextDirection, static_cast<int32_t>(ax::mojom::WritingDirection::kTtb));
    text_data3.AddIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets, kThaiCharacterOffsets);
    ...
    EXPECT_EQ(gfx::RectF(0, 0, 27, 0), text1_node->GetTextContentRangeBoundsUTF16(0, 3));
    EXPECT_EQ(gfx::RectF(12, 0, 7, 0), text1_node->GetTextContentRangeBoundsUTF16(1, 2));
    EXPECT_EQ(gfx::RectF(), text1_node->GetTextContentRangeBoundsUTF16(2, 4));

    // `kHindiText` is 6 code units in UTF-16.
    EXPECT_EQ(gfx::RectF(0, 0, 59, 0), text2_node->GetTextContentRangeBoundsUTF16(0, 6));
    EXPECT_EQ(gfx::RectF(0, 0, 19, 0), text2_node->GetTextContentRangeBoundsUTF16(2, 4));

    // `kThaiText` is 6 code units in UTF-16.
    EXPECT_EQ(gfx::RectF(0, 0, 0, 85), text3_node->GetTextContentRangeBoundsUTF16(0, 6));
    EXPECT_EQ(gfx::RectF(0, 66, 0, 10), text3_node->GetTextContentRangeBoundsUTF16(2, 4));
}
...
TEST(AXNodeTest, IsGridCellReadOnlyOrDisabled)
{
    ...
    gridcell_1.AddIntAttribute(ax::mojom::IntAttribute::kRestriction, static_cast<int32_t>(ax::mojom::Restriction::kNone));

    gridcell_2.role = ax::mojom::Role::kCell;
    gridcell_2.AddIntAttribute(ax::mojom::IntAttribute::kRestriction, static_cast<int32_t>(ax::mojom::Restriction::kReadOnly));

    gridcell_3.role = ax::mojom::Role::kCell;
    gridcell_3.AddIntAttribute(ax::mojom::IntAttribute::kRestriction, static_cast<int32_t>(ax::mojom::Restriction::kDisabled));
...
TEST(AXNodeTest, GetLowestCommonAncestor)
{
    ...
    AXTreeUpdate initial_state;
    initial_state.root_id = root.id;
    initial_state.nodes = { root, paragraph_0, static_text_0_0, paragraph_1, link_1_0, static_text_1_0_0, button_1_1 };
    initial_state.has_tree_data = true;
    ...
    ASSERT_EQ(paragraph_0_node, paragraph_0_node->GetLowestCommonAncestor(*paragraph_0_node));

    // Lowest common ancestor is reflexive.
    ASSERT_EQ(paragraph_1_node, link_1_0_node->GetLowestCommonAncestor(*button_1_1_node));
    ASSERT_EQ(paragraph_1_node, button_1_1_node->GetLowestCommonAncestor(*link_1_0_node));

    // Finds the lowest common ancestor for two nodes not on the same level.
    ASSERT_EQ(root_node, static_text_1_0_0_node->GetLowestCommonAncestor(*static_text_0_0_node));
    ASSERT_EQ(link_1_0_node, static_text_1_0_0_node->GetLowestCommonAncestor(*link_1_0_node));
}
...
TEST(AXNodeTest, DescendantOfNonAtomicTextField)
{
    ...
    spin_button_2.AddBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot, true);
    spin_button_2.child_ids = { spin_button_3.id, spin_button_4.id, generic_container_5.id };
    ...
    initial_state.nodes = { root_1, spin_button_2, spin_button_3, spin_button_4, generic_container_5 };
    initial_state.has_tree_data = true;
