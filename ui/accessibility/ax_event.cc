...
AXEvent::AXEvent(AXNodeData::AXID id, ax::mojom::Event event_type, ax::mojom::EventFrom event_from, ax::mojom::Action event_from_action,
    const std::vector<AXEventIntent>& event_intents, int action_request_id)
    : id(id)
...
std::string AXEvent::ToString() const
{
    ...
    if (event_from_action != ax::mojom::Action::kNone)
        result += std::string(" from accessibility action ") + ui::ToString(event_from_action);
