...
// static
std::unordered_map<AXTreeID, AXTreeManagerBase*, AXTreeIDHash>& AXTreeManagerBase::GetTreeManagerMapInstance()
{
    static base::NoDestructor<std::unordered_map<AXTreeID, AXTreeManagerBase*, AXTreeIDHash>> map_instance;
    return *map_instance;
}
...
std::unique_ptr<AXTree> AXTreeManagerBase::SetTree(std::unique_ptr<AXTree> tree)
{
    if (!tree) {
...
std::unique_ptr<AXTree> AXTreeManagerBase::SetTree(const AXTreeUpdate& initial_state)
{
...
// static
AXNode* AXTreeManagerBase::GetNodeFromTree(const AXTreeID& tree_id, const AXNodeID& node_id)
{
...
AXNode* AXTreeManagerBase::GetRootOfChildTree(const AXNodeID& host_node_id) const
{
...
{
    const AXTreeID& child_tree_id = AXTreeID::FromString(host_node.GetStringAttribute(ax::mojom::StringAttribute::kChildTreeId));
...
AXNode* AXTreeManagerBase::GetHostNode() const
{
    ...
    const std::set<AXNodeID>& host_node_ids = parent_manager->GetTree()->GetNodeIdsForChildTreeId(GetTreeID());
    ...
    DCHECK_EQ(host_node_ids.size(), 1u) << "Multiple nodes claim the same child tree ID.";
...
bool AXTreeManagerBase::AttachChildTree(const AXNodeID& host_node_id, AXTreeManagerBase& child_manager)
{
...
bool AXTreeManagerBase::AttachChildTree(AXNode& host_node, AXTreeManagerBase& child_manager)
{
    if (!tree_ || !child_manager.GetTree())
        return false;
    if (child_manager.GetParentTreeID().type() != ax::mojom::AXTreeIDType::kUnknown) {
        return false; // Child manager already attached to another host node.
    }

    DCHECK_EQ(GetNode(host_node.id()), &host_node) << "`host_node` should belong to this manager.";
    DCHECK(host_node.tree()) << "A node should always be attached to its owning tree.";
    DCHECK_NE(GetTreeID().type(), ax::mojom::AXTreeIDType::kUnknown);
    DCHECK_NE(child_manager.GetTreeID().type(), ax::mojom::AXTreeIDType::kUnknown);
    if (!host_node.IsLeaf()) {
...

    {
        ...
        DCHECK(!host_node.HasStringAttribute(ax::mojom::StringAttribute::kChildTreeId)) << "`AXNode::IsLeaf()` should mark all nodes with child tree IDs as "
                                                                                           "leaves.\n"
                                                                                        << host_node;
...

    {
        ...
        CHECK(child_manager.ApplyTreeUpdate(update)) << child_manager.GetTree()->error();
    }

    return true;
}

std::optional<AXTreeManagerBase> AXTreeManagerBase::AttachChildTree(const AXNodeID& host_node_id, const AXTreeUpdate& initial_state)
{
    AXNode* host_node = GetNode(host_node_id);
    if (host_node)
...
std::optional<AXTreeManagerBase> AXTreeManagerBase::AttachChildTree(AXNode& host_node, const AXTreeUpdate& initial_state)
{
...
AXTreeManagerBase* AXTreeManagerBase::DetachChildTree(const AXNodeID& host_node_id)
{
    AXNode* host_node = GetNode(host_node_id);
    if (host_node)
...
AXTreeManagerBase* AXTreeManagerBase::DetachChildTree(AXNode& host_node)
{
    ...
    {
        AXNodeData host_node_data = host_node.data();
        DCHECK_NE(child_manager->GetTreeData().parent_tree_id.type(), ax::mojom::AXTreeIDType::kUnknown) << "Child tree should be attached to its host node.\n"
                                                                                                         << host_node;
        host_node_data.RemoveStringAttribute(ax::mojom::StringAttribute::kChildTreeId);
        AXTreeUpdate update;
        update.nodes = { host_node_data };
        CHECK(ApplyTreeUpdate(update)) << GetTree()->error();
    }
