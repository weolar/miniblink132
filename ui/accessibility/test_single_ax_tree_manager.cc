...
TestSingleAXTreeManager::TestSingleAXTreeManager(TestSingleAXTreeManager&& manager)
...
TestSingleAXTreeManager& TestSingleAXTreeManager::operator=(TestSingleAXTreeManager&& manager)
{
    if (this == &manager) {
        return *this;
    }
...
AXTree* TestSingleAXTreeManager::Init(const AXNodeData& node1, const AXNodeData& node2 /* = AXNodeData() */, const AXNodeData& node3 /* = AXNodeData() */,
...
AXNodePosition::AXPositionInstance TestSingleAXTreeManager::CreateTreePosition(const AXNode& anchor, int child_index) const
{
    return AXNodePosition::CreateTreePosition(anchor, child_index);
}

AXNodePosition::AXPositionInstance TestSingleAXTreeManager::CreateTreePosition(const AXTree* tree, const AXNodeData& anchor_data, int child_index) const
{
    const AXNode* anchor = tree->GetFromId(anchor_data.id);
    return CreateTreePosition(*anchor, child_index);
}

AXNodePosition::AXPositionInstance TestSingleAXTreeManager::CreateTreePosition(const AXNodeData& anchor_data, int child_index) const
{
    return CreateTreePosition(ax_tree(), anchor_data, child_index);
}

AXNodePosition::AXPositionInstance TestSingleAXTreeManager::CreateTextPosition(const AXNode& anchor, int text_offset, ax::mojom::TextAffinity affinity) const
{
...
AXNodePosition::AXPositionInstance TestSingleAXTreeManager::CreateTextPosition(
    const AXTree* tree, const AXNodeData& anchor_data, int text_offset, ax::mojom::TextAffinity affinity) const
{
    const AXNode* anchor = tree->GetFromId(anchor_data.id);
...
AXNodePosition::AXPositionInstance TestSingleAXTreeManager::CreateTextPosition(
    const AXNodeData& anchor_data, int text_offset, ax::mojom::TextAffinity affinity) const
{
    return CreateTextPosition(ax_tree(), anchor_data, text_offset, affinity);
}

AXNodePosition::AXPositionInstance TestSingleAXTreeManager::CreateTextPosition(
    const AXNodeID& anchor_id, int text_offset, ax::mojom::TextAffinity affinity) const
{
...
AXNode* TestSingleAXTreeManager::GetParentNodeFromParentTree() const
{
    ...
    TestSingleAXTreeManager* parent_manager = static_cast<TestSingleAXTreeManager*>(AXTreeManager::FromID(parent_tree_id));
    if (!parent_manager) {
        return nullptr;
    }

    std::set<AXNodeID> host_node_ids = parent_manager->GetTree()->GetNodeIdsForChildTreeId(GetTreeID());
