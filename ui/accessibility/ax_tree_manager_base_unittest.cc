...
// static
void AXTreeManagerBaseTest::HostChildTreeAtNode(AXNode& host_node, AXTree& child_tree)
{
    ASSERT_NE(nullptr, host_node.tree());

    {
        AXNodeData host_node_data = host_node.data();
        ASSERT_NE(ax::mojom::AXTreeIDType::kUnknown, child_tree.GetAXTreeID().type());
...

    {
        ...
        ASSERT_NE(ax::mojom::AXTreeIDType::kUnknown, host_node.tree()->GetAXTreeID().type());
...
public:
    ...
    int manager_remove_count() const
    {
        return manager_remove_count_;
    }

    const AXTreeID& previous_tree_id() const
    {
        return previous_tree_id_;
    }
...
TEST_F(AXTreeManagerBaseTest, SetTree)
{
    ...
    std::unique_ptr<AXTree> old_tree = simple_manager_.SetTree(std::move(new_tree));
...
{
    EXPECT_EQ(simple_manager_.GetRoot(), GetNodeFromTree(simple_tree_id_, /* AXNodeID */ 1));
    EXPECT_EQ(complex_manager_.GetRoot(), GetNodeFromTree(complex_tree_id_, /* AXNodeID */ 1));
    EXPECT_EQ(simple_manager_.GetRoot(), simple_manager_.GetNode(/* AXNodeID */ 1));
...
TEST_F(AXTreeManagerBaseTest, ParentChildTreeRelationship)
{
    ...
    EXPECT_EQ(simple_manager_root, complex_manager_.GetRootOfChildTree(kIframeID));
...
TEST_F(AXTreeManagerBaseTest, AttachingAndDetachingChildTrees)
{
    ...
    EXPECT_FALSE(complex_manager_.AttachChildTree(root->id(), simple_manager_)) << "This particular rootnode is not a leaf node.";
    EXPECT_FALSE(complex_manager_.AttachChildTree(*root, simple_manager_)) << "This particular rootnode is not a leaf node.";

    EXPECT_FALSE(complex_manager_.AttachChildTree(kIframeID, empty_manager_)) << "Cannot attach an empty tree to any node.";
    EXPECT_FALSE(complex_manager_.AttachChildTree(*iframe, empty_manager_)) << "Cannot attach an empty tree to any node.";
    ...
    std::optional<AXTreeManagerBase> child_manager = complex_manager_.AttachChildTree(*iframe, CreateSimpleTreeUpdate());
    ASSERT_TRUE(child_manager.has_value());
    EXPECT_NE(nullptr, child_manager->GetTree());
    EXPECT_EQ(&(child_manager.value()), complex_manager_.DetachChildTree(*iframe));
    child_manager = complex_manager_.AttachChildTree(kIframeID, CreateSimpleTreeUpdate());
    EXPECT_NE(nullptr, child_manager->GetTree());
    ASSERT_TRUE(child_manager.has_value());
    ;
    EXPECT_EQ(&(child_manager.value()), complex_manager_.DetachChildTree(kIframeID));
}
...
TEST_F(AXTreeManagerBaseTest, Observers)
{
    ...
    EXPECT_EQ(2, observer.manager_remove_count()) << "Tree must have already been destroyed.";
    EXPECT_EQ(new_tree_id, observer.previous_tree_id());
}
