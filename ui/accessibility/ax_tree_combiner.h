...
public:
    ...
    std::optional<AXTreeUpdate>& combined()
    {
        return combined_;
    }
    const std::optional<AXTreeUpdate>& combined() const
    {
        return combined_;
    }
...
private:
    ...
    void ProcessTree(AXTreeUpdate* tree, const std::map<AXTreeID, AXTreeUpdate*>& tree_id_map);
