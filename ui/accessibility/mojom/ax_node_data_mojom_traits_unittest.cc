...
TEST(AXNodeDataMojomTraitsTest, StringAttributes)
{
    ...
    EXPECT_TRUE(SerializeAndDeserialize<ax::mojom::AXNodeData>(input, output));
    EXPECT_EQ("Mojo", output.GetStringAttribute(ax::mojom::StringAttribute::kName));
}
...
TEST(AXNodeDataMojomTraitsTest, FloatAttributes)
{
    ...
    EXPECT_EQ(100, output.GetFloatAttribute(ax::mojom::FloatAttribute::kFontWeight));
}
...
TEST(AXNodeDataMojomTraitsTest, IntListAttributes)
{
    ...
    EXPECT_TRUE(SerializeAndDeserialize<ax::mojom::AXNodeData>(input, output));
    EXPECT_EQ(std::vector<int32_t>({ 1, 2 }), output.GetIntListAttribute(ax::mojom::IntListAttribute::kControlsIds));
}
...
TEST(AXNodeDataMojomTraitsTest, IntListAttributesInvalid)
{
    ...
    EXPECT_FALSE(SerializeAndDeserialize<ax::mojom::AXNodeData>(input, output));
    input.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerEnds, { 1, 2, 3 });
    EXPECT_FALSE(SerializeAndDeserialize<ax::mojom::AXNodeData>(input, output));

    input.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerTypes, { static_cast<int32_t>(ax::mojom::MarkerType::kHighlight) });
...
{
    ui::AXNodeData input, output;
    input.AddStringListAttribute(ax::mojom::StringListAttribute::kCustomActionDescriptions, { "foo", "bar" });
    EXPECT_TRUE(SerializeAndDeserialize<ax::mojom::AXNodeData>(input, output));
    EXPECT_EQ(std::vector<std::string>({ "foo", "bar" }), output.GetStringListAttribute(ax::mojom::StringListAttribute::kCustomActionDescriptions));
}
