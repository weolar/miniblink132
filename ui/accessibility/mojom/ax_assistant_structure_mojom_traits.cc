...
// static
bool StructTraits<ax::mojom::AssistantTreeDataView, std::unique_ptr<ui::AssistantTree>>::Read(
    ax::mojom::AssistantTreeDataView data, std::unique_ptr<ui::AssistantTree>* out)
{
    DCHECK(!*out);
...
// static
bool StructTraits<ax::mojom::AssistantNodeDataView, std::unique_ptr<ui::AssistantNode>>::Read(
    ax::mojom::AssistantNodeDataView data, std::unique_ptr<ui::AssistantNode>* out)
{
    DCHECK(!out->get());
    *out = std::make_unique<ui::AssistantNode>();
    ...
    if (!data.ReadRect(&(*out)->rect) || !data.ReadText(&(*out)->text) || !data.ReadRole(&(*out)->role) || !data.ReadSelection(&(*out)->selection)
        || !data.ReadChildrenIndices(&(*out)->children_indices) || !data.ReadClassName(&(*out)->class_name)) {
        return false;
    }
