...
// static
bool StructTraits<ax::mojom::AXNodeDataDataView, ui::AXNodeData>::Read(ax::mojom::AXNodeDataDataView data, ui::AXNodeData* out)
{
    out->id = data.id();
    out->role = data.role();
    ...
    if (intlist_attributes.contains(ax::mojom::IntListAttribute::kMarkerTypes)) {
        if (!intlist_attributes.contains(ax::mojom::IntListAttribute::kMarkerStarts)) {
            return false;
        }
        if (!intlist_attributes.contains(ax::mojom::IntListAttribute::kMarkerEnds)) {
            return false;
        }
        auto& marker_types = intlist_attributes[ax::mojom::IntListAttribute::kMarkerTypes];
        auto& marker_starts = intlist_attributes[ax::mojom::IntListAttribute::kMarkerStarts];
        auto& marker_ends = intlist_attributes[ax::mojom::IntListAttribute::kMarkerEnds];
        if (marker_types.size() != marker_starts.size() || marker_types.size() != marker_ends.size()) {
            return false;
        }
        if (HasAnyHighlightEntries(marker_types)) {
            if (!intlist_attributes.contains(ax::mojom::IntListAttribute::kHighlightTypes)) {
                return false;
            }
            auto& highlight_types = intlist_attributes[ax::mojom::IntListAttribute::kHighlightTypes];
