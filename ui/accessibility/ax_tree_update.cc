...
std::string AXTreeUpdate::ToString(bool verbose) const
{
    ...
    if (node_id_to_clear != kInvalidAXNodeID) {
        result += "AXTreeUpdate: clear node " + base::NumberToString(node_id_to_clear) + "\n";
    }
    ...
    if (event_from_action != ax::mojom::Action::kNone)
        result += "event_from_action=" + std::string(ui::ToString(event_from_action)) + "\n";
...
void AXTreeUpdate::AccumulateSize(AXNodeData::AXNodeDataSize& node_data_size) const
{
