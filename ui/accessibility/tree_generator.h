...
public:
    ...
    static constexpr int ComputeUniqueTreeCount(int max_node_count, bool permutations)
    {
        int tree_count = 0;
    ...
    // should be focused.
    void BuildUniqueTreeWithIgnoredNodes(int tree_index, int ignored_index, std::optional<int> focused_node, AXTree* out_tree) const;

private:
    static constexpr int UniqueTreeCountForNodeCount(int node_count, bool permutations)
    {
    ...
    void BuildUniqueTreeUpdate(int tree_index, AXTreeUpdate* out_tree_update) const;
    void BuildUniqueTreeUpdateWithSize(int node_count, int tree_index, AXTreeUpdate* out_tree_update) const;
