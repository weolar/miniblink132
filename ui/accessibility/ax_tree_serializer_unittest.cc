...
using BasicAXTreeSerializer = AXTreeSerializer<const AXNode*, std::vector<raw_ptr<const AXNode, VectorExperimental>>, AXTreeUpdate*, AXTreeData*, AXNodeData>;
...
protected:
    ...
    std::unique_ptr<AXTreeSource<const AXNode*, AXTreeData*, AXNodeData>> tree0_source_;
    std::unique_ptr<AXTreeSource<const AXNode*, AXTreeData*, AXNodeData>> tree1_source_;
...
class AXTreeSourceWithInvalidId : public AXTreeSource<const AXNode*, AXTreeData*, AXNodeData> {
public:
    ...
    AXTreeSourceWithInvalidId& operator=(const AXTreeSourceWithInvalidId&) = delete;
    ...
    const AXNode* GetNull() const override
    {
        return nullptr;
    }
...
TEST(AXTreeSerializerInvalidDeathTest, InvalidChild)
{
    ...
    AXTreeUpdate update;
    EXPECT_CHECK_DEATH_WITH(serializer.SerializeChanges(tree.root(), &update), "child");
}
...
TEST_F(AXTreeSerializerTest, TestPartialSerialization)
{
    ...
    for (int max_node_count = 1; max_node_count <= 4; max_node_count++) {
        ...
        // be serialized.
        std::vector<AXNodeID> incomplete_node_ids = serializer_->GetIncompleteNodeIds();
        ...
        for (AXNodeID id : incomplete_node_ids) {
            update = AXTreeUpdate();
            ASSERT_TRUE(serializer_->SerializeChanges(tree1_->GetFromId(id), &update));
            EXPECT_TRUE(dst_tree.Unserialize(update)) << dst_tree.error();
        }

        // The result should be indistinguishable from the source tree.
        std::unique_ptr<AXTreeSource<const AXNode*, AXTreeData*, AXNodeData>> dst_tree_source(dst_tree.CreateTreeSource());
