...
void TreeGenerator::BuildUniqueTreeWithIgnoredNodes(int tree_index, int ignored_index, std::optional<int> focused_node, AXTree* out_tree) const
{
...
void TreeGenerator::BuildUniqueTreeUpdate(int tree_index, AXTreeUpdate* out_update) const
{
    ...
    for (int node_count = 1; node_count <= max_node_count_; ++node_count) {
        int unique_tree_count = unique_tree_count_by_size_[node_count];
        if (tree_index - unique_tree_count_so_far < unique_tree_count) {
            BuildUniqueTreeUpdateWithSize(node_count, tree_index - unique_tree_count_so_far, out_update);
            return;
        }
        unique_tree_count_so_far += unique_tree_count;
    }
...
void TreeGenerator::BuildUniqueTreeUpdateWithSize(int node_count, int tree_index, AXTreeUpdate* out_update) const
{
