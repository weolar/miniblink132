...
using OnNotifyEventCallbackMap = std::map<ax::mojom::Event,
    // A function to call when focus changes, for testing only.
    base::RepeatingClosure>;

OnNotifyEventCallbackMap& GetOnNotifyEventCallbackMap()
{
    static base::NoDestructor<OnNotifyEventCallbackMap> on_notify_event_for_testing;
    return *on_notify_event_for_testing;
}

// Check for descendant comment, using limited depth first search.
bool FindDescendantRoleWithMaxDepth(const AXPlatformNodeBase* node, ax::mojom::Role descendant_role, size_t max_depth, size_t max_children_to_check)
{
    ...
    size_t num_children_to_check = std::min(node->GetChildCount(), max_children_to_check);
    for (size_t index = 0; index < num_children_to_check; index++) {
        auto* child = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(node->ChildAtIndex(index)));
        if (child && FindDescendantRoleWithMaxDepth(child, descendant_role, max_depth - 1, max_children_to_check)) {
            return true;
        }
    }
...
using UniqueIdMap = std::unordered_map<int32_t, raw_ptr<AXPlatformNode, CtnExperimental>>;
base::LazyInstance<UniqueIdMap>::Leaky g_unique_id_map = LAZY_INSTANCE_INITIALIZER;

// Adds process-wide statistics about accessibility objects to traces.
class AXPlatformNodeMemoryDumpProvider : public base::trace_event::MemoryDumpProvider {
public:
    AXPlatformNodeMemoryDumpProvider(const AXPlatformNodeMemoryDumpProvider&) = delete;
    AXPlatformNodeMemoryDumpProvider& operator=(const AXPlatformNodeMemoryDumpProvider&) = delete;
...
bool AXPlatformNodeMemoryDumpProvider::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    auto* const dump = pmd->CreateAllocatorDump("accessibility/ax_platform_node");
    dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameObjectCount, base::trace_event::MemoryAllocatorDump::kUnitsObjects, id_to_node_->size());
    return true;
}

AXPlatformNodeMemoryDumpProvider::AXPlatformNodeMemoryDumpProvider(const UniqueIdMap& id_to_node)
    : id_to_node_(id_to_node)
{
    // Skip this in tests that don't set up a task runner on the main thread.
    if (base::SingleThreadTaskRunner::HasCurrentDefault()) {
        base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "AXPlatformNode", base::SingleThreadTaskRunner::GetCurrentDefault());
    }
}
...
// static
void AXPlatformNodeBase::SetOnNotifyEventCallbackForTesting(ax::mojom::Event event_type, base::RepeatingClosure callback)
{
...
void AXPlatformNodeBase::Init(AXPlatformNodeDelegate* delegate)
{
    ...
    static base::NoDestructor<AXPlatformNodeMemoryDumpProvider> dump_provider(g_unique_id_map.Get());
}
...
std::string AXPlatformNodeBase::GetName() const
{
    if (delegate_) {
        ...
        ax::mojom::ImageAnnotationStatus status = GetData().GetImageAnnotationStatus();
        switch (status) {
        ...
        case ax::mojom::ImageAnnotationStatus::kAnnotationProcessFailed:
            extra_text = base::UTF16ToUTF8(delegate_->GetLocalizedStringForImageAnnotationStatus(status));
            break;

        case ax::mojom::ImageAnnotationStatus::kAnnotationSucceeded:
            extra_text = GetStringAttribute(ax::mojom::StringAttribute::kImageAnnotation);
            break;
...
std::optional<size_t> AXPlatformNodeBase::GetIndexInParent()
{
    ...
    if (delegate_ && delegate_->GetNativeViewAccessible() != GetNativeViewAccessible()) {
        return std::nullopt;
    }
    ...
    DCHECK(false) << "Unable to find the child in the list of its parent's children.";
    return std::nullopt;
}
...
std::optional<int> AXPlatformNodeBase::CompareTo(AXPlatformNodeBase& other)
{
    ...
    auto* common_ancestor_platform_node = FromNativeViewAccessible(common_ancestor);
    ...
    if (!our_ancestors.empty() && !other_ancestors.empty()) {
        std::optional<int> this_index_in_parent = FromNativeViewAccessible(our_ancestors.top())->GetIndexInParent();
        std::optional<int> other_index_in_parent = FromNativeViewAccessible(other_ancestors.top())->GetIndexInParent();
        ...
        return std::optional<int>(this_uncommon_ancestor_index - other_uncommon_ancestor_index);
    }
...
AXPlatformNodeBase* AXPlatformNodeBase::GetActiveDescendant() const
{
    ...
    if (GetIntAttribute(ax::mojom::IntAttribute::kActivedescendantId, &active_descendant_id)) {
        active_descendant = static_cast<AXPlatformNodeBase*>(delegate_->GetFromNodeID(active_descendant_id));
    }

    if (GetRole() == ax::mojom::Role::kComboBoxSelect) {
        AXPlatformNodeBase* child = GetFirstChild();
        if (child && child->GetRole() == ax::mojom::Role::kMenuListPopup && !child->IsInvisibleOrIgnored()) {
            ...
            if (child->GetIntAttribute(ax::mojom::IntAttribute::kActivedescendantId, &active_descendant_id)) {
                active_descendant = static_cast<AXPlatformNodeBase*>(child->delegate_->GetFromNodeID(active_descendant_id));
            }
        }
    }
...
void AXPlatformNodeBase::NotifyAccessibilityEvent(ax::mojom::Event event_type)
{
    if (event_type == ax::mojom::Event::kAlert) {
        CHECK(IsAlert(GetRole())) << "On some platforms, the alert event does not work correctly unless "
                                     "it is fired on an object with an alert role. Role was "
                                  << GetRole();
    }

    OnNotifyEventCallbackMap& callback_map = GetOnNotifyEventCallbackMap();
    if (callback_map.find(event_type) != callback_map.end() && callback_map[event_type]) {
        callback_map[event_type].Run();
    }
}

#if BUILDFLAG(IS_APPLE)
void AXPlatformNodeBase::AnnounceTextAs(const std::u16string& text, AnnouncementType announcement_type)
{
}
#endif
...
AXPlatformNodeBase::AXPlatformNodeChildIterator AXPlatformNodeBase::AXPlatformNodeChildrenBegin() const
{
    return AXPlatformNodeChildIterator(this, GetFirstChild());
}

AXPlatformNodeBase::AXPlatformNodeChildIterator AXPlatformNodeBase::AXPlatformNodeChildrenEnd() const
{
...
bool AXPlatformNodeBase::GetBoolAttribute(ax::mojom::BoolAttribute attribute, bool* value) const
{
    if (!delegate_)
        return false;
...
bool AXPlatformNodeBase::GetFloatAttribute(ax::mojom::FloatAttribute attribute, float* value) const
{
    if (!delegate_)
        return false;
...
const std::vector<std::pair<ax::mojom::IntAttribute, int32_t>>& AXPlatformNodeBase::GetIntAttributes() const
{
    static const base::NoDestructor<const std::vector<std::pair<ax::mojom::IntAttribute, int32_t>>> empty_data;
    if (!delegate_)
        return *empty_data;
...
bool AXPlatformNodeBase::GetIntAttribute(ax::mojom::IntAttribute attribute, int* value) const
{
    if (!delegate_)
        return false;
...
const std::vector<std::pair<ax::mojom::StringAttribute, std::string>>& AXPlatformNodeBase::GetStringAttributes() const
{
    static const base::NoDestructor<const std::vector<std::pair<ax::mojom::StringAttribute, std::string>>> empty_data;
    if (!delegate_)
        return *empty_data;
...
const std::string& AXPlatformNodeBase::GetStringAttribute(ax::mojom::StringAttribute attribute) const
{
    if (!delegate_)
...
bool AXPlatformNodeBase::GetStringAttribute(ax::mojom::StringAttribute attribute, std::string* value) const
{
    if (!delegate_)
        return false;
...
std::u16string AXPlatformNodeBase::GetString16Attribute(ax::mojom::StringAttribute attribute) const
{
    if (!delegate_)
...
bool AXPlatformNodeBase::GetString16Attribute(ax::mojom::StringAttribute attribute, std::u16string* value) const
{
    if (!delegate_)
        return false;
...
bool AXPlatformNodeBase::HasInheritedStringAttribute(ax::mojom::StringAttribute attribute) const
{
...
const std::string& AXPlatformNodeBase::GetInheritedStringAttribute(ax::mojom::StringAttribute attribute) const
{
...
bool AXPlatformNodeBase::GetInheritedStringAttribute(ax::mojom::StringAttribute attribute, std::string* value) const
{
    // TODO(nektar): Switch to using `AXNode::GetInheritedStringAttribute` after
    // it has been modified to cross tree boundaries.
    const AXPlatformNodeBase* current_node = this;
...
std::u16string AXPlatformNodeBase::GetInheritedString16Attribute(ax::mojom::StringAttribute attribute) const
{
...
bool AXPlatformNodeBase::GetInheritedString16Attribute(ax::mojom::StringAttribute attribute, std::u16string* value) const
{
    // TODO(nektar): Switch to using `AXNode::GetInheritedString16Attribute` after
    // it has been modified to cross tree boundaries.
...
const std::vector<std::pair<ax::mojom::IntListAttribute, std::vector<int32_t>>>& AXPlatformNodeBase::GetIntListAttributes() const
{
    static const base::NoDestructor<const std::vector<std::pair<ax::mojom::IntListAttribute, std::vector<int32_t>>>> empty_data;
    if (!delegate_)
        return *empty_data;
    return delegate_->GetIntListAttributes();
}

bool AXPlatformNodeBase::HasIntListAttribute(ax::mojom::IntListAttribute attribute) const
{
    if (!delegate_)
        return false;
...
const std::vector<int32_t>& AXPlatformNodeBase::GetIntListAttribute(ax::mojom::IntListAttribute attribute) const
{
...
bool AXPlatformNodeBase::GetIntListAttribute(ax::mojom::IntListAttribute attribute, std::vector<int32_t>* value) const
{
    if (!delegate_)
        return false;
...
bool AXPlatformNodeBase::HasStringListAttribute(ax::mojom::StringListAttribute attribute) const
{
    if (!delegate_)
        return false;
...
const std::vector<std::string>& AXPlatformNodeBase::GetStringListAttribute(ax::mojom::StringListAttribute attribute) const
{
...
bool AXPlatformNodeBase::GetStringListAttribute(ax::mojom::StringListAttribute attribute, std::vector<std::string>* value) const
{
    if (!delegate_)
        return false;
...
bool AXPlatformNodeBase::HasNameFromOtherElement() const
{
    ...
    return nameFrom == ax::mojom::NameFrom::kCaption || nameFrom == ax::mojom::NameFrom::kRelatedElement;
}

// static
AXPlatformNodeBase* AXPlatformNodeBase::FromNativeViewAccessible(gfx::NativeViewAccessible accessible)
{
    return static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(accessible));
}

bool AXPlatformNodeBase::SetHypertextSelection(int start_offset, int end_offset)
{
    if (!delegate_)
        return false;
...
std::u16string AXPlatformNodeBase::GetRoleDescriptionFromImageAnnotationStatusOrFromAttribute() const
{
    if (GetRole() == ax::mojom::Role::kImage
        && (GetData().GetImageAnnotationStatus() == ax::mojom::ImageAnnotationStatus::kEligibleForAnnotation
            || GetData().GetImageAnnotationStatus() == ax::mojom::ImageAnnotationStatus::kSilentlyEligibleForAnnotation)) {
...
{
    DCHECK_EQ(GetRole(), ax::mojom::Role::kImage) << "Only call IsImageWithMap() on an image";
...
AXPlatformNodeBase* AXPlatformNodeBase::GetTableCell(int row, int column) const
{
    if (!delegate_) {
        return nullptr;
    }
...
AXPlatformNodeBase* AXPlatformNodeBase::GetAriaTableCell(int aria_row, int aria_column) const
{
    if (!delegate_) {
        return nullptr;
    }

    std::optional<int32_t> cell_id = delegate_->GetCellIdAriaCoords(aria_row, aria_column);
    if (!cell_id) {
        return nullptr;
    }
    return static_cast<AXPlatformNodeBase*>(delegate_->GetFromNodeID(*cell_id));
...
{
    return (HasIntAttribute(ax::mojom::IntAttribute::kScrollXMin) && HasIntAttribute(ax::mojom::IntAttribute::kScrollXMax)
               && HasIntAttribute(ax::mojom::IntAttribute::kScrollX))
        || (HasIntAttribute(ax::mojom::IntAttribute::kScrollYMin) && HasIntAttribute(ax::mojom::IntAttribute::kScrollYMax)
            && HasIntAttribute(ax::mojom::IntAttribute::kScrollY));
}

bool AXPlatformNodeBase::IsHorizontallyScrollable() const
{
    DCHECK_GE(GetIntAttribute(ax::mojom::IntAttribute::kScrollXMin), 0) << "Pixel sizes should be non-negative.";
    DCHECK_GE(GetIntAttribute(ax::mojom::IntAttribute::kScrollXMax), 0) << "Pixel sizes should be non-negative.";
    return IsScrollable() && GetIntAttribute(ax::mojom::IntAttribute::kScrollXMin) < GetIntAttribute(ax::mojom::IntAttribute::kScrollXMax);
}

bool AXPlatformNodeBase::IsVerticallyScrollable() const
{
    DCHECK_GE(GetIntAttribute(ax::mojom::IntAttribute::kScrollYMin), 0) << "Pixel sizes should be non-negative.";
    DCHECK_GE(GetIntAttribute(ax::mojom::IntAttribute::kScrollYMax), 0) << "Pixel sizes should be non-negative.";
    return IsScrollable() && GetIntAttribute(ax::mojom::IntAttribute::kScrollYMin) < GetIntAttribute(ax::mojom::IntAttribute::kScrollYMax);
}
...
void AXPlatformNodeBase::ComputeAttributes(PlatformAttributeList* attributes)
{
    ...
    // as "foo".
    AddAttributeToList(ax::mojom::StringAttribute::kDisplay, "display", attributes);
    AddAttributeToList(ax::mojom::StringAttribute::kHtmlTag, "tag", attributes);
    AddAttributeToList(ax::mojom::StringAttribute::kRole, "xml-roles", attributes);
    AddAttributeToList(ax::mojom::StringAttribute::kPlaceholder, "placeholder", attributes);

    AddAttributeToList(ax::mojom::StringAttribute::kAutoComplete, "autocomplete", attributes);
    if (!HasStringAttribute(ax::mojom::StringAttribute::kAutoComplete) && HasState(ax::mojom::State::kAutofillAvailable)) {
    ...
    std::u16string role_description = GetRoleDescriptionFromImageAnnotationStatusOrFromAttribute();
    if (!role_description.empty() || HasStringAttribute(ax::mojom::StringAttribute::kRoleDescription)) {
        AddAttributeToList("roledescription", base::UTF16ToUTF8(role_description), attributes);
    }
    ...
    if (GetIntAttribute(ax::mojom::IntAttribute::kDescriptionFrom, &desc_from)) {
        ...
        case ax::mojom::DescriptionFrom::kAriaDescription:
            ...
            AddAttributeToList(ax::mojom::StringAttribute::kDescription, "description", attributes);
    ...
    AddAttributeToList(ax::mojom::StringAttribute::kAriaBrailleLabel, "braillelabel", attributes);
    AddAttributeToList(ax::mojom::StringAttribute::kAriaBrailleRoleDescription, "brailleroledescription", attributes);

    AddAttributeToList(ax::mojom::StringAttribute::kKeyShortcuts, "keyshortcuts", attributes);
    AddAttributeToList(ax::mojom::IntAttribute::kHierarchicalLevel, "level", attributes);
    AddAttributeToList(ax::mojom::IntAttribute::kSetSize, "setsize", attributes);
    AddAttributeToList(ax::mojom::IntAttribute::kPosInSet, "posinset", attributes);
    ...
    AddAttributeToList(ax::mojom::StringAttribute::kLiveStatus, "live", attributes);
    AddAttributeToList(ax::mojom::StringAttribute::kLiveRelevant, "relevant", attributes);
    AddAttributeToList(ax::mojom::BoolAttribute::kLiveAtomic, "atomic", attributes);
    ...
    AddAttributeToList(ax::mojom::StringAttribute::kContainerLiveStatus, "container-live", attributes);
    AddAttributeToList(ax::mojom::StringAttribute::kContainerLiveRelevant, "container-relevant", attributes);
    AddAttributeToList(ax::mojom::BoolAttribute::kContainerLiveAtomic, "container-atomic", attributes);
    AddAttributeToList(ax::mojom::BoolAttribute::kContainerLiveBusy, "container-busy", attributes);
    ...
    if (GetIntAttribute(ax::mojom::IntAttribute::kAriaCurrentState, &aria_current_state)) {
    ...
    if (IsTableLike(GetRole())
        && (delegate_->GetTableAriaRowCount() != delegate_->GetTableRowCount() || delegate_->GetTableAriaColCount() != delegate_->GetTableColCount())) {
        AddAttributeToList(ax::mojom::IntAttribute::kAriaColumnCount, "colcount", attributes);
        AddAttributeToList(ax::mojom::IntAttribute::kAriaRowCount, "rowcount", attributes);
    }
    ...
    if (IsCellOrTableHeader(GetRole()) || IsTableRow(GetRole())) {
        ...
        if (aria_rowindex && physical_rowindex && aria_rowindex.value() - 1 != physical_rowindex.value()) {
            std::string str_value = base::NumberToString(*aria_rowindex);
            AddAttributeToList("rowindex", str_value, attributes);
        }

        if (!IsTableRow(GetRole()) && aria_colindex && physical_colindex && aria_colindex.value() - 1 != physical_colindex.value()) {
            AddAttributeToList(ax::mojom::IntAttribute::kAriaCellColumnIndex, "colindex", attributes);
        }
    }
    ...
    if (IsCellOrTableHeader(GetRole())) {
        ...
        AddAttributeToList(ax::mojom::StringAttribute::kAriaCellColumnIndexText, "coltext", attributes);
        AddAttributeToList(ax::mojom::StringAttribute::kAriaCellRowIndexText, "rowtext", attributes);
        // These newer names are consistent with the ARIA attribute names:
        AddAttributeToList(ax::mojom::StringAttribute::kAriaCellColumnIndexText, "colindextext", attributes);
        AddAttributeToList(ax::mojom::StringAttribute::kAriaCellRowIndexText, "rowindextext", attributes);

        AddAttributeToList(ax::mojom::IntAttribute::kAriaCellColumnSpan, "colspan", attributes);
        AddAttributeToList(ax::mojom::IntAttribute::kAriaCellRowSpan, "rowspan", attributes);
    }

    // Expose the value of a progress bar, slider, scroll bar or <select> element.
    if (GetData().IsRangeValueSupported() || GetRole() == ax::mojom::Role::kComboBoxMenuButton) {
    ...
    if (delegate_->HasIntAttribute(ax::mojom::IntAttribute::kDropeffectDeprecated)) {
        NOTREACHED();
    }
    ...
    AddAttributeToList(ax::mojom::StringAttribute::kDateTime, "datetime", attributes);
    ...
    if (delegate_->GetStringAttribute(ax::mojom::StringAttribute::kHtmlInputName, &input_name)) {
        AddAttributeToList("html-input-name", input_name, attributes);
    }

    std::string src;
    if (IsImage(GetRole()) && GetStringAttribute(ax::mojom::StringAttribute::kUrl, &src)) {
        AddAttributeToList("src", src, attributes);
    }

    if (delegate_->HasIntAttribute(ax::mojom::IntAttribute::kTextAlign)) {
        auto text_align = static_cast<ax::mojom::TextAlign>(delegate_->GetIntAttribute(ax::mojom::IntAttribute::kTextAlign));
        switch (text_align) {
    ...
    float text_indent;
    if (GetFloatAttribute(ax::mojom::FloatAttribute::kTextIndent, &text_indent) != 0.0f) {
    ...
    if (IsAtomicTextField() || IsDateOrTimeInput(GetRole())) {
        AddAttributeToList(ax::mojom::StringAttribute::kInputType, "text-input-type", attributes);
    }
    ...
    if (IsLink(GetRole())) {
        AddAttributeToList(ax::mojom::StringAttribute::kLinkTarget, "link-target", attributes);
    }

    // MathML content.
    AddAttributeToList(ax::mojom::StringAttribute::kMathContent, "math", attributes);
    ...
    if (int max_length = GetIntAttribute(ax::mojom::IntAttribute::kMaxLength)) {
        AddAttributeToList("maxlength", base::NumberToString(max_length), attributes);
    }
}

void AXPlatformNodeBase::AddAttributeToList(const ax::mojom::StringAttribute attribute, const char* name, PlatformAttributeList* attributes)
{
    DCHECK(attributes);
    std::string value;
...
void AXPlatformNodeBase::AddAttributeToList(const ax::mojom::BoolAttribute attribute, const char* name, PlatformAttributeList* attributes)
{
    DCHECK(attributes);
    bool value;
...
void AXPlatformNodeBase::AddAttributeToList(const ax::mojom::IntAttribute attribute, const char* name, PlatformAttributeList* attributes)
{
    DCHECK(attributes);
...
void AXPlatformNodeBase::AddAttributeToList(const char* name, const std::string& value, PlatformAttributeList* attributes)
{
...
AXLegacyHypertext& AXLegacyHypertext::operator=(const AXLegacyHypertext& other) = default;
...
void AXPlatformNodeBase::UpdateComputedHypertext() const
{
    ...
    for (AXPlatformNodeChildIterator child_iter = AXPlatformNodeChildrenBegin(); child_iter != AXPlatformNodeChildrenEnd(); ++child_iter) {
...
void AXPlatformNodeBase::AddAttributeToList(const char* name, const char* value, PlatformAttributeList* attributes)
{
}
...
// static
void AXPlatformNodeBase::SanitizeStringAttribute(const std::string& input, std::string* output)
{
    DCHECK(output);
...
int32_t AXPlatformNodeBase::GetHyperlinkIndexFromChild(AXPlatformNodeBase* child)
{
    if (hypertext_.hyperlinks.empty())
        return -1;

    auto iterator = base::ranges::find(hypertext_.hyperlinks, child->GetUniqueId());
...
int32_t AXPlatformNodeBase::GetHypertextOffsetFromHyperlinkIndex(int32_t hyperlink_index)
{
...
int32_t AXPlatformNodeBase::GetHypertextOffsetFromChild(AXPlatformNodeBase* child)
{
    ...
    if (child->IsText()) {
        ...
        for (auto child_iter = AXPlatformNodeChildrenBegin(); child_iter != AXPlatformNodeChildrenEnd() && child_iter.get() != child; ++child_iter) {
            if (child_iter->IsText()) {
                hypertext_offset += static_cast<int32_t>(child_iter->GetHypertext().size());
            } else {
                ++hypertext_offset;
            }
        }
...
int AXPlatformNodeBase::HypertextOffsetFromChildIndex(int child_index) const
{
    ...
    for (AXPlatformNodeChildIterator child_iter = AXPlatformNodeChildrenBegin(); child_iter != AXPlatformNodeChildrenEnd(); ++child_iter) {
        ...
        if (child_iter->IsText())
            child_text_len = base::checked_cast<int>(child_iter->GetHypertext().size());
...
int32_t AXPlatformNodeBase::GetHypertextOffsetFromDescendant(AXPlatformNodeBase* descendant)
{
    auto* parent_object = static_cast<AXPlatformNodeBase*>(FromNativeViewAccessible(descendant->GetDelegate()->GetParent()));
    while (parent_object && parent_object != this) {
        descendant = parent_object;
        parent_object = static_cast<AXPlatformNodeBase*>(FromNativeViewAccessible(descendant->GetParent()));
    }
    if (!parent_object)
        return -1;
...
int AXPlatformNodeBase::GetHypertextOffsetFromEndpoint(AXPlatformNodeBase* endpoint_object, int endpoint_offset)
{
    ...
    if (IsDescendantOf(endpoint_object)) {
        DCHECK_LE(endpoint_offset, static_cast<int>(endpoint_object->GetChildCount()));

        AXPlatformNodeBase* closest_ancestor = this;
        while (closest_ancestor) {
            AXPlatformNodeBase* parent = static_cast<AXPlatformNodeBase*>(FromNativeViewAccessible(closest_ancestor->GetParent()));
        ...
        std::optional<size_t> index_in_parent = closest_ancestor->GetIndexInParent();
        DCHECK(index_in_parent) << "No index in parent for ancestor: " << *closest_ancestor;
        if (index_in_parent && endpoint_offset > static_cast<int>(*index_in_parent)) {
    ...
    while (common_parent && !endpoint_object->IsDescendantOf(common_parent)) {
        ...
        common_parent = static_cast<AXPlatformNodeBase*>(FromNativeViewAccessible(common_parent->GetParent()));
    }
    if (!common_parent)
        return -1;
    ...
    if (common_parent == this) {
        ...
        auto* parent = static_cast<AXPlatformNodeBase*>(FromNativeViewAccessible(endpoint_object->GetParent()));
    ...
    for (auto child_iter = common_parent->AXPlatformNodeChildrenBegin(); child_iter != common_parent->AXPlatformNodeChildrenEnd(); ++child_iter) {
    ...
    DUMP_WILL_BE_NOTREACHED() << "Was not in descendant, so the endpoint_index_in_common_parent should "
                              ...
                              << "\n* Endpoint in common parent: " << endpoint_index_in_common_parent.value_or(-99);
    return -1;
}

AXPlatformNodeBase::AXPosition AXPlatformNodeBase::HypertextOffsetToEndpoint(int hypertext_offset) const
{
    ...
    for (auto child_iter = AXPlatformNodeChildrenBegin(); child_iter != AXPlatformNodeChildrenEnd() && current_hypertext_offset >= 0; ++child_iter) {
        int child_text_len = 1;
        if (child_iter->IsText())
            child_text_len = base::checked_cast<int>(child_iter->GetHypertext().size());
...
int AXPlatformNodeBase::GetSelectionAnchor(const AXSelection* selection)
{
    ...
    AXPlatformNodeBase* anchor_object = static_cast<AXPlatformNodeBase*>(delegate_->GetFromNodeID(anchor_id));
    if (!anchor_object)
        return -1;

    return GetHypertextOffsetFromEndpoint(anchor_object, selection->anchor_offset);
}
...
int AXPlatformNodeBase::GetSelectionFocus(const AXSelection* selection)
{
    ...
    AXPlatformNodeBase* focus_object = static_cast<AXPlatformNodeBase*>(GetDelegate()->GetFromNodeID(focus_id));
    if (!focus_object)
        return -1;
...
void AXPlatformNodeBase::GetSelectionOffsets(int* selection_start, int* selection_end)
{
    GetSelectionOffsets(nullptr, selection_start, selection_end);
}

void AXPlatformNodeBase::GetSelectionOffsets(const AXSelection* selection, int* selection_start, int* selection_end)
{
    DCHECK(selection_start && selection_end);

    if (IsAtomicTextField() && GetIntAttribute(ax::mojom::IntAttribute::kTextSelStart, selection_start)
...
void AXPlatformNodeBase::GetSelectionOffsetsFromTree(const AXSelection* selection, int* selection_start, int* selection_end)
{
    DCHECK(selection_start && selection_end);
    ...
    *selection_end = GetSelectionFocus(selection);
    ...
    const std::map<int, int>& offset_to_child_index = delegate_->GetHypertextOffsetToHyperlinkChildIndex();
    ...
    AXPlatformNodeBase* hyperlink = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(ChildAtIndex(child_index)));
    if (!hyperlink)
        return;

    int hyperlink_selection_start, hyperlink_selection_end;
    hyperlink->GetSelectionOffsets(selection, &hyperlink_selection_start, &hyperlink_selection_end);
    if (hyperlink_selection_start >= 0 && hyperlink_selection_end >= 0 && hyperlink_selection_start != hyperlink_selection_end) {
        ++(*largest_offset);
    }
}

bool AXPlatformNodeBase::IsSameHypertextCharacter(const AXLegacyHypertext& old_hypertext, size_t old_char_index, size_t new_char_index)
{
    if (old_char_index >= old_hypertext.hypertext.size() || new_char_index >= hypertext_.hypertext.size()) {
        return false;
    }
    ...
    const std::map<int32_t, int32_t>& old_offset_to_index = old_hypertext.hyperlink_offset_to_index;
    ...
    int old_child_id = (old_index >= 0 && old_index < old_hyperlinkscount) ? old_hyperlinks[old_index] : -1;

    const std::map<int32_t, int32_t>& new_offset_to_index = hypertext_.hyperlink_offset_to_index;
    ...
    int new_child_id = (new_index >= 0 && new_index < new_hyperlinkscount) ? new_hyperlinks[new_index] : -1;

    return old_child_id == new_child_id;
}

// Return true if the index represents a text character.
bool AXPlatformNodeBase::IsText(const std::u16string& text, size_t index, bool is_indexed_from_end)
{
...
void AXPlatformNodeBase::ComputeHypertextRemovedAndInserted(const AXLegacyHypertext& old_hypertext, size_t* start, size_t* old_len, size_t* new_len)
{
    *start = 0;
    *old_len = 0;
    *new_len = 0;
    ...
    while (prefix_index < old_text.size() && prefix_index < new_text.size() && IsSameHypertextCharacter(old_hypertext, prefix_index, prefix_index)) {
        ++prefix_index;
        if (allow_partial_text_node_changes || (!IsText(old_text, prefix_index) && !IsText(new_text, prefix_index))) {
    ...
    while (common_prefix + suffix_index < old_text.size() && common_prefix + suffix_index < new_text.size()
        && IsSameHypertextCharacter(old_hypertext, old_text.size() - suffix_index - 1, new_text.size() - suffix_index - 1)) {
        ++suffix_index;
        if (allow_partial_text_node_changes || (!IsText(old_text, suffix_index, true) && !IsText(new_text, suffix_index, true))) {
...
int AXPlatformNodeBase::FindTextBoundary(
    ax::mojom::TextBoundary boundary, int offset, ax::mojom::MoveDirection direction, ax::mojom::TextAffinity affinity) const
{
    ...
    AXMovementOptions options { AXBoundaryBehavior::kStopAtAnchorBoundary, AXBoundaryDetection::kDontCheckInitialPosition };
    ...
    const AXPosition boundary_position = position->CreatePositionAtTextBoundary(boundary, direction, options);
...
AXPlatformNodeBase* AXPlatformNodeBase::NearestLeafToPoint(gfx::Point point) const
{
    // First, scope the search to the node that contains point.
    AXPlatformNodeBase* nearest_node
        = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(GetDelegate()->HitTestSync(point.x(), point.y())));
    ...
    AXPlatformNodeBase* current_descendant = parent->GetChildCount() ? parent->GetFirstChild() : nullptr;
    ...
    while (parent && current_descendant) {
        ...
        float current_distance = current_descendant->GetDelegate()->GetClippedScreenBoundsRect().ManhattanDistanceToPoint(point);
...
int AXPlatformNodeBase::NearestTextIndexToPoint(gfx::Point point)
{
    ...
    float shortest_distance = GetDelegate()->GetInnerTextRangeBoundsRect(0, 1, coordinate_system, clipping_behavior).ManhattanDistanceToPoint(point);
    for (int i = 1, text_length = GetTextContentUTF16().length(); i < text_length; ++i) {
        float current_distance = GetDelegate()->GetInnerTextRangeBoundsRect(i, i + 1, coordinate_system, clipping_behavior).ManhattanDistanceToPoint(point);
...
TextAttributeList AXPlatformNodeBase::ComputeTextAttributes() const
{
    ...
    if ((color = delegate_->GetBackgroundColor())) {
        ...
        std::string color_value = "rgb(" + base::NumberToString(red) + ',' + base::NumberToString(green) + ',' + base::NumberToString(blue) + ')';
    ...
    if (font_family.empty()) {
        font_family = GetInheritedStringAttribute(ax::mojom::StringAttribute::kFontFamily);
    }
    ...
    if (font_size_in_points) {
        attributes.emplace_back(std::make_pair("font-size", base::NumberToString(*font_size_in_points) + "pt"));
    }
    ...
    if (text_style) {
        ...
        if (HasTextStyle(ax::mojom::TextStyle::kLineThrough)) {
            ...
            attributes.emplace_back(std::make_pair("text-line-through-style", "solid"));
        }
    ...
    auto text_direction = static_cast<ax::mojom::WritingDirection>(GetIntAttribute(ax::mojom::IntAttribute::kTextDirection));
    ...
    auto text_position = static_cast<ax::mojom::TextPosition>(GetIntAttribute(ax::mojom::IntAttribute::kTextPosition));
...
AXPlatformNodeBase* AXPlatformNodeBase::GetSelectedItem(int selected_index) const
{
...
int AXPlatformNodeBase::GetSelectedItems(int max_items, std::vector<AXPlatformNodeBase*>* out_selected_items) const
{
    int selected_count = 0;
    for (auto child_iter = AXPlatformNodeChildrenBegin(); child_iter != AXPlatformNodeChildrenEnd() && selected_count < max_items; ++child_iter) {
        if (!IsItemLike(child_iter->GetRole())) {
            selected_count += child_iter->GetSelectedItems(max_items - selected_count, out_selected_items);
        } else if (child_iter->GetBoolAttribute(ax::mojom::BoolAttribute::kSelected)) {
            selected_count++;
...
void AXPlatformNodeBase::SanitizeTextAttributeValue(const std::string& input, std::string* output) const
{
    DCHECK(output);
}
...
{
    const std::vector<int32_t>& description_ids = GetIntListAttribute(ax::mojom::IntListAttribute::kDescribedbyIds);

    std::string description_from;

    for (int id : description_ids) {
        AXPlatformNodeBase* description_object = static_cast<AXPlatformNodeBase*>(delegate_->GetFromNodeID(id));
        if (description_object && description_object->GetRole() == ax::mojom::Role::kTooltip) {
            return true;
        }
    }
...
{
    const std::vector<int32_t>& details_ids = GetIntListAttribute(ax::mojom::IntListAttribute::kDetailsIds);
    ...
    for (int id : details_ids) {
        AXPlatformNodeBase* detail_object = static_cast<AXPlatformNodeBase*>(delegate_->GetFromNodeID(id));
        if (!detail_object)
            continue;
        ...
        default:
            ...
            if (detail_object->GetDelegate()->node()->HasIntAttribute(ax::mojom::IntAttribute::kIsPopup)) {
    ...
    std::vector<std::string> details_roles_vector(details_roles_set.begin(), details_roles_set.end());
...
bool AXPlatformNodeBase::DescendantHasComment(const AXPlatformNodeBase* node)
{
    ...
    if (FindDescendantRoleWithMaxDepth(node, ax::mojom::Role::kComment, kMaxDepthToCheck, kMaxChildrenToCheck)) {
        return true;
    }
    return false;
}
