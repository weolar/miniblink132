...
#define AX_PLATFORM_NODE_AURALINUX(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), AX_PLATFORM_NODE_AURALINUX_TYPE, AXPlatformNodeAuraLinuxObject))
#define AX_PLATFORM_NODE_AURALINUX_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), AX_PLATFORM_NODE_AURALINUX_TYPE, AXPlatformNodeAuraLinuxClass))
#define IS_AX_PLATFORM_NODE_AURALINUX(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), AX_PLATFORM_NODE_AURALINUX_TYPE))
#define IS_AX_PLATFORM_NODE_AURALINUX_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), AX_PLATFORM_NODE_AURALINUX_TYPE))
#define AX_PLATFORM_NODE_AURALINUX_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), AX_PLATFORM_NODE_AURALINUX_TYPE, AXPlatformNodeAuraLinuxClass))
...
using GetRowColumnSpanFunc = bool (*)(AtkTableCell* cell, gint* row, gint* column, gint* row_span, gint* col_span);
...
AtkRole GetAtkRoleContentDeletion()
{
    ...
    return atk_version.CompareTo(base::Version("2.34.0")) >= 0 ? ATK_ROLE_CONTENT_DELETION : ATK_ROLE_SECTION;
}
...
{
    base::Version atk_version(atk_get_version());
    return atk_version.CompareTo(base::Version("2.34.0")) >= 0 ? ATK_ROLE_CONTENT_INSERTION : ATK_ROLE_SECTION;
}

AtkObject* FindAtkObjectParentFrame(AtkObject* atk_object)
{
    AXPlatformNodeAuraLinux* node = AXPlatformNodeAuraLinux::FromAtkObject(atk_object);
    while (node) {
...
bool IsFrameAncestorOfAtkObject(AtkObject* frame, AtkObject* atk_object)
{
    ...
    while (current_frame) {
        if (current_frame == frame)
            return true;
        AXPlatformNodeAuraLinux* frame_node = AXPlatformNodeAuraLinux::FromAtkObject(current_frame);
...
{
    static base::NoDestructor<std::map<AtkObject*, FindInPageResultInfo>> active_results;
...
const char* GetUniqueAccessibilityGTypeName(ImplementedAtkInterfaces interface_mask)
{
    // 37 characters is enough for "AXPlatformNodeAuraLinux%x" with any integer
    // value.
    static char name[37];
    snprintf(name, sizeof(name), "AXPlatformNodeAuraLinux%x", interface_mask.value());
    return name;
}
...
void SetWeakGPtrToAtkObject(AtkObject** weak_pointer, AtkObject* new_value)
{
    ...
    if (*weak_pointer) {
        g_object_remove_weak_pointer(G_OBJECT(*weak_pointer), reinterpret_cast<void**>(weak_pointer));
    }

    *weak_pointer = new_value;

    if (new_value) {
        g_object_add_weak_pointer(G_OBJECT(new_value), reinterpret_cast<void**>(weak_pointer));
    }
}
...
AXCoordinateSystem AtkCoordTypeToAXCoordinateSystem(AtkCoordType coordinate_type)
{
...
const char* BuildDescriptionFromHeaders(AXPlatformNodeDelegate* delegate, const std::vector<int32_t>& ids)
{
...
AtkAttributeSet* PrependAtkAttributeToAtkAttributeSet(const char* name, const char* value, AtkAttributeSet* attribute_set)
{
...
void PrependTextAttributeToSet(const std::string& attribute, const std::string& value, AtkAttributeSet** attributes)
{
    DCHECK(attributes);

    AtkAttribute* new_attribute = static_cast<AtkAttribute*>(g_malloc(sizeof(AtkAttribute)));
...
void PrependAtkTextAttributeToSet(const AtkTextAttribute attribute, const std::string& value, AtkAttributeSet** attributes)
{
    PrependTextAttributeToSet(atk_text_attribute_get_name(attribute), value, attributes);
}
...
AtkAttributeSet* ToAtkAttributeSet(const TextAttributeList& attributes)
{
    ...
    for (const auto& attribute : attributes) {
        if (attribute.first == "background-color") {
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_BG_COLOR, ToAtkTextAttributeColor(attribute.second), &copied_attributes);
        } else if (attribute.first == "color") {
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_FG_COLOR, ToAtkTextAttributeColor(attribute.second), &copied_attributes);
        } else if (attribute.first == "font-family") {
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_FAMILY_NAME, attribute.second, &copied_attributes);
        } else if (attribute.first == "font-size") {
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_SIZE, attribute.second, &copied_attributes);
        } else if (attribute.first == "font-weight" && attribute.second == "bold") {
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_WEIGHT, "700", &copied_attributes);
        } else if (attribute.first == "font-style") {
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_STYLE, "italic", &copied_attributes);
        } else if (attribute.first == "text-line-through-style") {
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_STRIKETHROUGH, "true", &copied_attributes);
        } else if (attribute.first == "text-underline-style") {
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_UNDERLINE, "single", &copied_attributes);
        } else if (attribute.first == "invalid") {
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_INVALID, attribute.second, &copied_attributes);
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_UNDERLINE, "error", &copied_attributes);
        } else if (attribute.first == "language") {
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_LANGUAGE, attribute.second, &copied_attributes);
        } else if (attribute.first == "writing-mode") {
            PrependAtkTextAttributeToSet(ATK_TEXT_ATTR_DIRECTION, attribute.second, &copied_attributes);
        } else if (attribute.first == "text-position") {
            PrependTextAttributeToSet(attribute.first, attribute.second, &copied_attributes);
        }
    }
...
void GetExtents(AtkComponent* atk_component, gint* x, gint* y, gint* width, gint* height, AtkCoordType coord_type)
{
    ...
    AXPlatformNodeAuraLinux* obj = AXPlatformNodeAuraLinux::FromAtkObject(atk_object);
    if (!obj)
        return;
...
void GetPosition(AtkComponent* atk_component, gint* x, gint* y, AtkCoordType coord_type)
{
    g_return_if_fail(ATK_IS_COMPONENT(atk_component));
...
AtkObject* RefAccesibleAtPoint(AtkComponent* atk_component, gint x, gint y, AtkCoordType coord_type)
{
...
gboolean ScrollTo(AtkComponent* atk_component, AtkScrollType scroll_type)
{
    ...
    AXPlatformNodeAuraLinux* obj = AXPlatformNodeAuraLinux::FromAtkObject(ATK_OBJECT(atk_component));
    if (!obj)
        return FALSE;
...
gboolean ScrollToPoint(AtkComponent* atk_component, AtkCoordType atk_coord_type, gint x, gint y)
{
    g_return_val_if_fail(ATK_IS_COMPONENT(atk_component), FALSE);
...
const GInterfaceInfo Info = { reinterpret_cast<GInterfaceInitFunc>(Init), nullptr, nullptr };
...
gboolean DoAction(AtkAction* atk_action, gint index)
{
    ...
    const std::vector<ax::mojom::Action> actions = obj->GetDelegate()->GetSupportedActions();
    const std::vector<int32_t>& aria_actions = obj->GetIntListAttribute(ax::mojom::IntListAttribute::kActionsIds);

    // Actions can be from Blink for the given markup, or from the aria-actions
    // attribute defined by the author.
    g_return_val_if_fail(index < static_cast<gint>(actions.size() + aria_actions.size()), FALSE);
    AXActionData data;
    ...
    if (AXPlatformNodeAuraLinux* aria_action_obj = obj->GetFromNodeID(aria_action_id)) {
...
gint GetNActions(AtkAction* atk_action)
{
    ...
    return static_cast<gint>(obj->GetDelegate()->GetSupportedActions().size() + obj->GetIntListAttribute(ax::mojom::IntListAttribute::kActionsIds).size());
}
...
const gchar* GetName(AtkAction* atk_action, gint index)
{
    ...
    int32_t aria_action_id = aria_actions[index - actions.size()];
    AXPlatformNodeAuraLinux* aria_action_obj = obj->GetFromNodeID(aria_action_id);
    std::string html_id = aria_action_obj->GetStringAttribute(ax::mojom::StringAttribute::kHtmlId);
    if (html_id.empty()) {
        ATK_AURALINUX_RETURN_STRING(AXPlatformNodeBase::kAriaActionsPrefix);
    }
    ATK_AURALINUX_RETURN_STRING(AXPlatformNodeBase::kAriaActionsPrefix + "#" + html_id);
}
...
const gchar* GetKeybinding(AtkAction* atk_action, gint index)
{
    ...
    if (index == 0 && obj->GetDelegate()->HasDefaultActionVerb()) {
        ...
        return obj->GetStringAttribute(ax::mojom::StringAttribute::kAccessKey).c_str();
    }
    return nullptr;
}
...
const gchar* GetDocumentAttributeValue(AtkDocument* atk_doc, const gchar* attribute)
{
...
void GetImagePosition(AtkImage* atk_img, gint* x, gint* y, AtkCoordType coord_type)
{
...
const gchar* GetImageDescription(AtkImage* atk_img)
{
    ...
    return obj->GetStringAttribute(ax::mojom::StringAttribute::kDescription).c_str();
}
...
void GetCurrentValue(AtkValue* atk_value, GValue* value)
{
    ...
    obj->GetFloatAttributeInGValue(ax::mojom::FloatAttribute::kValueForRange, value);
}
...
void GetMinimumValue(AtkValue* atk_value, GValue* value)
{
    ...
    obj->GetFloatAttributeInGValue(ax::mojom::FloatAttribute::kMinValueForRange, value);
}
...
void GetMaximumValue(AtkValue* atk_value, GValue* value)
{
    ...
    obj->GetFloatAttributeInGValue(ax::mojom::FloatAttribute::kMaxValueForRange, value);
}
...
void GetMinimumIncrement(AtkValue* atk_value, GValue* value)
{
    ...
    obj->GetFloatAttributeInGValue(ax::mojom::FloatAttribute::kStepValueForRange, value);
}
...
AtkHyperlink* GetLink(AtkHypertext* hypertext, int index)
{
    ...
    auto* link = static_cast<AXPlatformNodeAuraLinux*>(AXPlatformNodeBase::GetFromUniqueId(id));
    if (!link)
        return nullptr;
...
{
    g_return_val_if_fail(ATK_HYPERTEXT(hypertext), 0);
    AXPlatformNodeAuraLinux* obj = AXPlatformNodeAuraLinux::FromAtkObject(ATK_OBJECT(hypertext));
...
gchar* GetText(AtkText* atk_text, gint start_offset, gint end_offset)
{
    ...
    if (end_offset < 0) {
        ...
        end_offset = std::clamp(static_cast<int>(text.size()), start_offset, end_offset);
    }

    DCHECK_GE(start_offset, 0);
    DCHECK_GE(end_offset, start_offset);

    return g_strdup(base::UTF16ToUTF8(text.substr(start_offset, end_offset - start_offset)).c_str());
}
...
gunichar GetCharacterAtOffset(AtkText* atk_text, int offset)
{
    ...
    base::ReadUnicodeCharacter(text.c_str(), text_length + 1, &limited_offset, &code_point);
    return code_point;
}
...
char* GetCharacter(AtkText* atk_text, int offset, int* start_offset, int* end_offset)
{
    g_return_val_if_fail(ATK_IS_TEXT(atk_text), nullptr);
...
char* GetTextWithBoundaryType(AtkText* atk_text, int offset, ax::mojom::TextBoundary boundary, int* start_offset_ptr, int* end_offset_ptr)
{
    g_return_val_if_fail(ATK_IS_TEXT(atk_text), nullptr);
    ...
    int start_offset = obj->FindTextBoundary(boundary, offset, ax::mojom::MoveDirection::kBackward, ax::mojom::TextAffinity::kDownstream);
    int end_offset = obj->FindTextBoundary(boundary, offset, ax::mojom::MoveDirection::kForward, ax::mojom::TextAffinity::kDownstream);
    if (start_offset < 0 || end_offset < 0)
        return nullptr;

    DCHECK_LE(start_offset, end_offset) << "Start offset should be less than or equal the end offset.";
...
char* GetTextAtOffset(AtkText* atk_text, int offset, AtkTextBoundary atk_boundary, int* start_offset, int* end_offset)
{
    g_return_val_if_fail(ATK_IS_TEXT(atk_text), nullptr);
    ax::mojom::TextBoundary boundary = FromAtkTextBoundary(atk_boundary);
    return GetTextWithBoundaryType(atk_text, offset, boundary, start_offset, end_offset);
}

char* GetTextAfterOffset(AtkText* atk_text, int offset, AtkTextBoundary boundary, int* start_offset, int* end_offset)
{
    g_return_val_if_fail(ATK_IS_TEXT(atk_text), nullptr);
...
char* GetTextBeforeOffset(AtkText* atk_text, int offset, AtkTextBoundary boundary, int* start_offset, int* end_offset)
{
    g_return_val_if_fail(ATK_IS_TEXT(atk_text), nullptr);
...
gint GetCaretOffset(AtkText* atk_text)
{
    ...
    AXPlatformNodeAuraLinux* obj = AXPlatformNodeAuraLinux::FromAtkObject(ATK_OBJECT(atk_text));
    if (!obj)
        return -1;
...
gboolean SetCaretOffset(AtkText* atk_text, gint offset)
{
    ...
    obj->GrabFocusOrSetSequentialFocusNavigationStartingPointAtOffset(utf16_offset);
...
int GetNSelections(AtkText* atk_text)
{
    ...
    std::optional<FindInPageResultInfo> result = obj->GetSelectionOffsetsFromFindInPage();
...
gchar* GetSelection(AtkText* atk_text, int selection_num, int* start_offset, int* end_offset)
{
    g_return_val_if_fail(ATK_IS_TEXT(atk_text), nullptr);
...
gboolean RemoveSelection(AtkText* atk_text, int selection_num)
{
    ...
    int selection_end = obj->UTF16ToUnicodeOffsetInText(obj->GetIntAttribute(ax::mojom::IntAttribute::kTextSelEnd));
    return SetCaretOffset(atk_text, selection_end);
}

gboolean SetSelection(AtkText* atk_text, int selection_num, int start_offset, int end_offset)
{
    g_return_val_if_fail(ATK_IS_TEXT(atk_text), FALSE);
...
char* GetStringAtOffset(AtkText* atk_text, int offset, AtkTextGranularity atk_granularity, int* start_offset, int* end_offset)
{
    g_return_val_if_fail(ATK_IS_TEXT(atk_text), nullptr);
...
gfx::Rect GetUnclippedParentHypertextRangeBoundsRect(AXPlatformNodeDelegate* ax_platform_node_delegate, const int start_offset, const int end_offset)
{
    const AXPlatformNode* parent_platform_node = AXPlatformNode::FromNativeViewAccessible(ax_platform_node_delegate->GetParent());
    if (!parent_platform_node)
        return gfx::Rect();

    const AXPlatformNodeDelegate* parent_ax_platform_node_delegate = parent_platform_node->GetDelegate();
    if (!parent_ax_platform_node_delegate)
        return gfx::Rect();

    return ax_platform_node_delegate->GetHypertextRangeBoundsRect(start_offset, end_offset, AXCoordinateSystem::kRootFrame, AXClippingBehavior::kUnclipped)
        - parent_ax_platform_node_delegate->GetBoundsRect(AXCoordinateSystem::kRootFrame, AXClippingBehavior::kClipped).OffsetFromOrigin();
}

void GetCharacterExtents(AtkText* atk_text, int offset, int* x, int* y, int* width, int* height, AtkCoordType coordinate_type)
{
    ...
    if (obj) {
        switch (coordinate_type) {
        case ATK_XY_PARENT:
            rect = GetUnclippedParentHypertextRangeBoundsRect(obj->GetDelegate(), offset, offset + 1);
            break;
        default:
            rect = obj->GetDelegate()->GetHypertextRangeBoundsRect(obj->UnicodeToUTF16OffsetInText(offset), obj->UnicodeToUTF16OffsetInText(offset + 1),
                AtkCoordTypeToAXCoordinateSystem(coordinate_type), AXClippingBehavior::kUnclipped);
            break;
        }
    }
...
void GetRangeExtents(AtkText* atk_text, int start_offset, int end_offset, AtkCoordType coordinate_type, AtkTextRectangle* out_rectangle)
{
    g_return_if_fail(ATK_IS_TEXT(atk_text));
    ...
    if (obj) {
        switch (coordinate_type) {
        case ATK_XY_PARENT:
            rect = GetUnclippedParentHypertextRangeBoundsRect(obj->GetDelegate(), start_offset, end_offset);
            break;
        default:
            rect = obj->GetDelegate()->GetHypertextRangeBoundsRect(obj->UnicodeToUTF16OffsetInText(start_offset), obj->UnicodeToUTF16OffsetInText(end_offset),
                AtkCoordTypeToAXCoordinateSystem(coordinate_type), AXClippingBehavior::kUnclipped);
            break;
        }
    }
...
AtkAttributeSet* GetRunAttributes(AtkText* atk_text, gint offset, gint* start_offset, gint* end_offset)
{
    g_return_val_if_fail(ATK_IS_TEXT(atk_text), nullptr);
    ...
    return ToAtkAttributeSet(obj->GetTextAttributes(offset, start_offset, end_offset));
}
...
gboolean ScrollSubstringTo(AtkText* atk_text, gint start_offset, gint end_offset, AtkScrollType scroll_type)
{
    g_return_val_if_fail(ATK_IS_TEXT(atk_text), FALSE);
...
gboolean ScrollSubstringToPoint(AtkText* atk_text, gint start_offset, gint end_offset, AtkCoordType atk_coord_type, gint x, gint y)
{
    g_return_val_if_fail(ATK_IS_TEXT(atk_text), FALSE);
    ...
    return obj->ScrollSubstringToPoint(start_offset, end_offset, atk_coord_type, x, y);
}
...
gboolean AddSelection(AtkSelection* selection, gint index)
{
    ...
    AXPlatformNodeAuraLinux* obj = AXPlatformNodeAuraLinux::FromAtkObject(ATK_OBJECT(selection));
    if (!obj)
        return FALSE;
    if (index < 0 || static_cast<size_t>(index) >= obj->GetChildCount())
        return FALSE;

    AXPlatformNodeAuraLinux* child = AXPlatformNodeAuraLinux::FromAtkObject(obj->ChildAtIndex(index));
    if (!child)
        return FALSE;
...
gboolean ClearSelection(AtkSelection* selection)
{
    ...
    for (int i = 0; i < child_count; ++i) {
        AXPlatformNodeAuraLinux* child = AXPlatformNodeAuraLinux::FromAtkObject(obj->ChildAtIndex(i));
        if (!child)
            continue;
...
gboolean RemoveSelection(AtkSelection* selection, gint index_into_selected_children)
{
    g_return_val_if_fail(ATK_IS_SELECTION(selection), FALSE);
...
{
    g_return_val_if_fail(G_TYPE_CHECK_INSTANCE_TYPE((cell), atk_table_cell_get_type()), 0);

    if (const AXPlatformNodeBase* obj = AXPlatformNodeAuraLinux::FromAtkObject(ATK_OBJECT(cell))) {
...
GPtrArray* GetColumnHeaderCells(AtkTableCell* cell)
{
    ...
    const std::vector<int32_t> ids = obj->GetDelegate()->GetColHeaderNodeIds(*col_index);
    for (const auto& node_id : ids) {
...
{
    g_return_val_if_fail(G_TYPE_CHECK_INSTANCE_TYPE((cell), atk_table_cell_get_type()), FALSE);
...
GPtrArray* GetRowHeaderCells(AtkTableCell* cell)
{
    ...
    const std::vector<int32_t> ids = obj->GetDelegate()->GetRowHeaderNodeIds(*row_index);
    for (const auto& node_id : ids) {
        if (AXPlatformNode* node = obj->GetDelegate()->GetFromNodeID(node_id)) {
            if (AtkObject* atk_node = node->GetNativeViewAccessible()) {
                g_ptr_array_add(array, g_object_ref(atk_node));
...
const gchar* GetName(AtkObject* atk_object)
{
    ...
    if (obj->GetName().empty() && name_from != ax::mojom::NameFrom::kAttributeExplicitlyEmpty) {
        return nullptr;
    }
...
gint GetIndexInParent(AtkObject* atk_object)
{
    ...
    return index_in_parent.has_value() ? static_cast<gint>(index_in_parent.value()) : -1;
}
...
AtkStateSet* RefStateSet(AtkObject* atk_object)
{
    ...
    AtkStateSet* atk_state_set = ATK_OBJECT_CLASS(kAXPlatformNodeAuraLinuxParentClass)->ref_state_set(atk_object);
...
void Initialize(AtkObject* atk_object, gpointer data)
{
    if (ATK_OBJECT_CLASS(kAXPlatformNodeAuraLinuxParentClass)->initialize) {
        ATK_OBJECT_CLASS(kAXPlatformNodeAuraLinuxParentClass)->initialize(atk_object, data);
    }

    AX_PLATFORM_NODE_AURALINUX(atk_object)->m_object = reinterpret_cast<AXPlatformNodeAuraLinux*>(data);
}
...
GType GetType()
{
    ...
    if (g_once_init_enter(&type_id)) {
        ...
        GType type = g_type_register_static(ATK_TYPE_OBJECT, "AXPlatformNodeAuraLinux", &type_info, GTypeFlags(0));
...
// static
ImplementedAtkInterfaces AXPlatformNodeAuraLinux::GetGTypeInterfaceMask(const AXNodeData& data)
{
...
{
    static const GTypeInfo type_info = {
        sizeof(AXPlatformNodeAuraLinuxClass), (GBaseInitFunc) nullptr, (GBaseFinalizeFunc) nullptr, (GClassInitFunc) nullptr, (GClassFinalizeFunc) nullptr,
        nullptr, /* class data */
        sizeof(AXPlatformNodeAuraLinuxObject), /* instance size */
        0, /* nb preallocs */
        (GInstanceInitFunc) nullptr, nullptr /* value table */
    };
    ...
    type = g_type_register_static(AX_PLATFORM_NODE_AURALINUX_TYPE, atk_type_name, &type_info, GTypeFlags(0));
    ...
    if (interface_mask_.Implements(ImplementedAtkInterfaces::Value::kHyperlink)) {
        g_type_add_interface_static(type, ATK_TYPE_HYPERLINK_IMPL, &atk_hyperlink::Info);
    }
    ...
    if (interface_mask_.Implements(ImplementedAtkInterfaces::Value::kTableCell)) {
        g_type_add_interface_static(type, atk_table_cell_get_type(), &atk_table_cell::Info);
    }
...
void AXPlatformNodeAuraLinux::SetDocumentParentOnFrameIfNecessary()
{
    ...
    AXPlatformNodeAuraLinux* parent = AXPlatformNodeAuraLinux::FromAtkObject(parent_atk_object);
    if (!parent)
        return;
    ...
    AXPlatformNodeAuraLinux* frame = AXPlatformNodeAuraLinux::FromAtkObject(FindAtkObjectParentFrame(parent_atk_object));
    if (!frame)
        return;
...
bool AXPlatformNodeAuraLinux::IsWebDocumentForRelations()
{
    ...
    if (!parent || !GetDelegate()->IsWebContent() || GetAtkRole() != ATK_ROLE_DOCUMENT_WEB)
        return false;
...
{
    if (GetRole() != ax::mojom::Role::kApplication && !GetDelegate()->IsToplevelBrowserWindow()
...
{
    if (atk_hyperlink_) {
        ax_platform_atk_hyperlink_set_object(AX_PLATFORM_ATK_HYPERLINK(atk_hyperlink_.get()), nullptr);
...
// static
AXPlatformNodeAuraLinux* AXPlatformNodeAuraLinux::FromAtkObject(const AtkObject* atk_object)
{
    if (!atk_object)
        return nullptr;

    if (IS_AX_PLATFORM_NODE_AURALINUX(atk_object)) {
        AXPlatformNodeAuraLinuxObject* platform_object = AX_PLATFORM_NODE_AURALINUX(atk_object);
...
AtkRole AXPlatformNodeAuraLinux::GetAtkRole() const
{
    ...
    case ax::mojom::Role::kDisclosureTriangleGrouped:
        return ::features::IsAccessibilityExposeSummaryAsHeadingEnabled() ? ATK_ROLE_HEADING : ATK_ROLE_TOGGLE_BUTTON;
...
bool PlatformSupportsState(AtkStateType atk_state_type)
{
    ...
    if (!max_state_type.has_value()) {
        GEnumClass* enum_class = G_ENUM_CLASS(g_type_class_ref(atk_state_type_get_type()));
...
void AXPlatformNodeAuraLinux::GetAtkState(AtkStateSet* atk_state_set)
{
    ...
    if (menu_active && FindAtkObjectParentFrame(GetActiveMenus().back()) == atk_object_)
        atk_state_set_add_state(atk_state_set, ATK_STATE_ACTIVE);
    ...
    if ((HasState(ax::mojom::State::kEditable) || HasState(ax::mojom::State::kRichlyEditable))
    ...
    if (HasIntAttribute(ax::mojom::IntAttribute::kAriaCurrentState)
        && GetIntAttribute(ax::mojom::IntAttribute::kAriaCurrentState) != static_cast<int32_t>(ax::mojom::AriaCurrentState::kFalse)) {
        atk_state_set_add_state(atk_state_set, ATK_STATE_ACTIVE);
    }
    ...
    if (HasIntAttribute(ax::mojom::IntAttribute::kHasPopup) && PlatformSupportsState(ATK_STATE_HAS_POPUP))
    ...
    if (GetRole() == ax::mojom::Role::kProgressIndicator && !HasFloatAttribute(ax::mojom::FloatAttribute::kValueForRange)) {
    ...
    if (checked_state == ax::mojom::CheckedState::kTrue || checked_state == ax::mojom::CheckedState::kMixed) {
    ...
    if (GetData().GetRestriction() != ax::mojom::Restriction::kDisabled) {
        if (GetDelegate()->IsReadOnlySupported() && GetDelegate()->IsReadOnlyOrDisabled()) {
...
static AtkIntRelation kIntRelations[] = {
    { ax::mojom::IntAttribute::kMemberOfId, ATK_RELATION_MEMBER_OF, std::nullopt },
    { ax::mojom::IntAttribute::kPopupForId, ATK_RELATION_POPUP_FOR, std::nullopt },
};
...
static AtkIntListRelation kIntListRelations[] = {
    { ax::mojom::IntListAttribute::kControlsIds, ATK_RELATION_CONTROLLER_FOR, ATK_RELATION_CONTROLLED_BY },
    { ax::mojom::IntListAttribute::kDetailsIds, ATK_RELATION_DETAILS, ATK_RELATION_DETAILS_FOR },
    { ax::mojom::IntListAttribute::kDescribedbyIds, ATK_RELATION_DESCRIBED_BY, ATK_RELATION_DESCRIPTION_FOR },
    { ax::mojom::IntListAttribute::kErrormessageIds, ATK_RELATION_ERROR_MESSAGE, ATK_RELATION_ERROR_FOR },
    { ax::mojom::IntListAttribute::kFlowtoIds, ATK_RELATION_FLOWS_TO, ATK_RELATION_FLOWS_FROM },
    { ax::mojom::IntListAttribute::kLabelledbyIds, ATK_RELATION_LABELLED_BY, ATK_RELATION_LABEL_FOR },
};

void AXPlatformNodeAuraLinux::AddRelationToSet(AtkRelationSet* relation_set, AtkRelationType relation, AXPlatformNode* target)
{
    DCHECK(target);
    ...
    if (!max_relation_type.has_value()) {
        GEnumClass* enum_class = G_ENUM_CLASS(g_type_class_ref(atk_relation_type_get_type()));
    ...
    atk_relation_set_add_relation_by_type(relation_set, relation, target->GetNativeViewAccessible());
}
...
AtkRelationSet* AXPlatformNodeAuraLinux::GetAtkRelations()
{
    ...
    if (IsWebDocumentForRelations()) {
        ...
        if (parent_frame) {
            atk_relation_set_add_relation_by_type(relation_set, ATK_RELATION_EMBEDDED_BY, parent_frame);
        }
    }
    ...
    for (auto relation : kIntRelations) {
        if (AXPlatformNode* target = GetDelegate()->GetTargetNodeForRelation(relation.attribute))
            AddRelationToSet(relation_set, relation.relation, target);

        if (!relation.reverse_relation.has_value())
            continue;

        std::vector<AXPlatformNode*> target_ids = GetDelegate()->GetSourceNodesForReverseRelations(relation.attribute);
    ...
    for (const auto& relation : kIntListRelations) {
        std::vector<AXPlatformNode*> targets = GetDelegate()->GetTargetNodesForRelation(relation.attribute);
        ...
        std::vector<AXPlatformNode*> reverse_target_ids = GetDelegate()->GetSourceNodesForReverseRelations(relation.attribute);
...
{
    AXPlatformNode* parent = AXPlatformNode::FromNativeViewAccessible(GetParent());
    ...
    if (parent == AXPlatformNodeAuraLinux::application() && GetRole() == ax::mojom::Role::kUnknown
...
AXPlatformNodeAuraLinux* AXPlatformNodeAuraLinux::GetFromNodeID(int32_t node_id)
{
...
void AXPlatformNodeAuraLinux::OnCheckedStateChanged()
{
    ...
    atk_object_notify_state_change(ATK_OBJECT(obj), GetAtkStateTypeForCheckableNode(), GetData().GetCheckedState() != ax::mojom::CheckedState::kFalse);
}
...
void AXPlatformNodeAuraLinux::OnEnabledChanged()
{
    ...
    atk_object_notify_state_change(obj, ATK_STATE_ENABLED, GetData().GetRestriction() != ax::mojom::Restriction::kDisabled);

    atk_object_notify_state_change(obj, ATK_STATE_SENSITIVE, GetData().GetRestriction() != ax::mojom::Restriction::kDisabled);
}
...
void AXPlatformNodeAuraLinux::OnExpandedStateChanged(bool is_expanded)
{
    ...
    DCHECK(HasState(ax::mojom::State::kCollapsed) || HasState(ax::mojom::State::kExpanded));
...
void AXPlatformNodeAuraLinux::OnMenuPopupStart()
{
    ...
    if (previous_active_frame) {
        ...
        atk_object_notify_state_change(previous_active_frame, ATK_STATE_ACTIVE, FALSE);
    }
...
void AXPlatformNodeAuraLinux::OnMenuPopupEnd()
{
    ...
    std::vector<AtkObject*>& active_menus = GetActiveMenus();
    DCHECK(!active_menus.empty()) << "Asymmetrical menupopupend events -- too many";
...
{
    if (!GetActiveMenus().empty() && g_active_top_level_frame && ComputeActiveTopLevelFrame() != g_active_top_level_frame) {
        g_signal_emit_by_name(g_active_top_level_frame, "activate");
        atk_object_notify_state_change(g_active_top_level_frame, ATK_STATE_ACTIVE, TRUE);
    }
...
void AXPlatformNodeAuraLinux::OnWindowActivated()
{
    ...
    if (g_current_focused && IsFrameAncestorOfAtkObject(parent_frame, g_current_focused)) {
        g_signal_emit_by_name(g_current_focused, "focus-event", true);
        atk_object_notify_state_change(ATK_OBJECT(g_current_focused), ATK_STATE_FOCUSED, true);
    }
}
...
void AXPlatformNodeAuraLinux::OnFocused()
{
    ...
    if (g_current_focused) {
        ...
        atk_object_notify_state_change(ATK_OBJECT(g_current_focused), ATK_STATE_FOCUSED, false);
    }
...
void AXPlatformNodeAuraLinux::OnSelected()
{
    ...
    if (!atk_object)
        return;
    if (g_current_selected && !g_current_selected->GetBoolAttribute(ax::mojom::BoolAttribute::kSelected)) {
        atk_object_notify_state_change(ATK_OBJECT(g_current_selected->GetOrCreateAtkObject()), ATK_STATE_SELECTED, false);
    }

    g_current_selected = this;
    if (ATK_IS_OBJECT(atk_object)) {
        atk_object_notify_state_change(ATK_OBJECT(atk_object), ATK_STATE_SELECTED, true);
    }
}
...
void AXPlatformNodeAuraLinux::GetFullSelection(int32_t* anchor_node_id, int* anchor_offset, int32_t* focus_node_id, int* focus_offset)
{
    ...
    if (IsAtomicTextField() && GetIntAttribute(ax::mojom::IntAttribute::kTextSelStart, anchor_offset)
...
AXPlatformNodeAuraLinux& AXPlatformNodeAuraLinux::FindEditableRootOrDocument()
{
    if (GetAtkRole() == ATK_ROLE_DOCUMENT_WEB)
        return *this;
    if (GetBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot) && HasState(ax::mojom::State::kEditable))
        return *this;
...
AXPlatformNodeAuraLinux* AXPlatformNodeAuraLinux::FindCommonAncestor(AXPlatformNodeAuraLinux* other)
{
...
void AXPlatformNodeAuraLinux::UpdateSelectionInformation(int32_t anchor_node_id, int anchor_offset, int32_t focus_node_id, int focus_offset)
{
    had_nonzero_width_selection = focus_node_id != anchor_node_id || focus_offset != anchor_offset;
...
void AXPlatformNodeAuraLinux::EmitCaretChangedSignal()
{
    ...
    DCHECK(ATK_IS_TEXT(atk_object));
    g_signal_emit_by_name(atk_object, "text-caret-moved", UTF16ToUnicodeOffsetInText(selection.second));
}
...
void AXPlatformNodeAuraLinux::OnTextSelectionChanged()
{
    ...
    GetFullSelection(&anchor_node_id, &anchor_offset, &focus_node_id, &focus_offset);

    auto* anchor_node = static_cast<AXPlatformNodeAuraLinux*>(GetDelegate()->GetFromNodeID(anchor_node_id));
    auto* focus_node = static_cast<AXPlatformNodeAuraLinux*>(GetDelegate()->GetFromNodeID(focus_node_id));
    if (!anchor_node || !focus_node)
        return;

    AXPlatformNodeAuraLinux& editable_root = FindEditableRootOrDocument();
    AXPlatformNodeAuraLinux* common_ancestor = focus_node->FindCommonAncestor(anchor_node);
    if (common_ancestor) {
        common_ancestor->EmitSelectionChangedSignal(editable_root.HadNonZeroWidthSelection());
    }
    ...
    editable_root.UpdateSelectionInformation(anchor_node_id, anchor_offset, focus_node_id, focus_offset);
}

bool AXPlatformNodeAuraLinux::SupportsSelectionWithAtkSelection()
{
    return SupportsToggle(GetRole()) || GetRole() == ax::mojom::Role::kListBoxOption;
}
...
void AXPlatformNodeAuraLinux::OnDescriptionChanged()
{
    ...
    g_signal_emit_by_name(G_OBJECT(atk_object), "property-change::accessible-description", &property_values, nullptr);
...
void AXPlatformNodeAuraLinux::OnValueChanged()
{
    ...
    g_value_set_double(&property_values.new_value, static_cast<double>(float_val));
    g_signal_emit_by_name(G_OBJECT(atk_object), "property-change::accessible-value", &property_values, nullptr);
}
...
void AXPlatformNodeAuraLinux::OnNameChanged()
{
    ...
    if (!g_strcmp0(atk_object::GetName(atk_object), previous_accessible_name.c_str()))
        return;
...
void AXPlatformNodeAuraLinux::OnSubtreeCreated()
{
    ...
    gint index_gint = index_in_parent.has_value() ? static_cast<gint>(index_in_parent.value()) : -1;
    g_signal_emit_by_name(GetParent(), "children-changed::add", index_gint, atk_object);
}
...
void AXPlatformNodeAuraLinux::OnSubtreeWillBeDeleted()
{
    ...
    auto index_in_parent = GetIndexInParent();
    gint index_gint = index_in_parent.has_value() ? static_cast<gint>(index_in_parent.value()) : -1;
    g_signal_emit_by_name(GetParent(), "children-changed::remove", index_gint, atk_object);
}
...
void AXPlatformNodeAuraLinux::OnParentChanged()
{
    ...
    g_signal_emit_by_name(G_OBJECT(atk_object_.get()), "property-change::accessible-parent", &property_values, nullptr);
    g_value_unset(&property_values.new_value);
}
...
void AXPlatformNodeAuraLinux::OnReadonlyChanged()
{
    ...
    atk_object_notify_state_change(obj, ATK_STATE_READ_ONLY, GetData().GetRestriction() == ax::mojom::Restriction::kReadOnly);
}
...
void AXPlatformNodeAuraLinux::OnInvalidStatusChanged()
{
    ...
    atk_object_notify_state_change(ATK_OBJECT(atk_object), ATK_STATE_INVALID_ENTRY, GetData().GetInvalidState() != ax::mojom::InvalidState::kFalse);
}
...
void AXPlatformNodeAuraLinux::OnAriaCurrentChanged()
{
    ...
    ax::mojom::AriaCurrentState aria_current = static_cast<ax::mojom::AriaCurrentState>(GetIntAttribute(ax::mojom::IntAttribute::kAriaCurrentState));
    atk_object_notify_state_change(
        ATK_OBJECT(atk_object), ATK_STATE_ACTIVE, aria_current != ax::mojom::AriaCurrentState::kNone && aria_current != ax::mojom::AriaCurrentState::kFalse);
}

void AXPlatformNodeAuraLinux::OnAlertShown()
{
    atk_object_notify_state_change(ATK_OBJECT(GetOrCreateAtkObject()), ATK_STATE_SHOWING, TRUE);
}
...
void AXPlatformNodeAuraLinux::NotifyAccessibilityEvent(ax::mojom::Event event_type)
{
...
std::optional<std::pair<int, int>> AXPlatformNodeAuraLinux::GetEmbeddedObjectIndicesForId(int id)
{
    ...
    auto offset = base::ranges::find(hypertext_.hyperlink_offset_to_index, hyperlink_index, &AXLegacyHypertext::OffsetToIndex::value_type::second);
    if (offset == hypertext_.hyperlink_offset_to_index.end())
        return std::nullopt;

    return std::make_pair(UTF16ToUnicodeOffsetInText(offset->first), UTF16ToUnicodeOffsetInText(offset->first + 1));
}

std::optional<std::pair<int, int>> AXPlatformNodeAuraLinux::GetEmbeddedObjectIndices()
{
...
void AXPlatformNodeAuraLinux::UpdateHypertext()
{
    ...
    if ((!HasState(ax::mojom::State::kEditable) || GetData().GetRestriction() == ax::mojom::Restriction::kReadOnly) && !IsInLiveRegion()) {
        return;
    }
    ...
    ComputeHypertextRemovedAndInserted(old_hypertext, &shared_prefix, &old_len, &new_len);
    if (old_len > 0) {
        std::u16string removed_substring = old_hypertext.hypertext.substr(shared_prefix, old_len);
        ...
        g_signal_emit_by_name(atk_object, "text-remove",
    ...
    if (new_len > 0) {
        std::u16string inserted_substring = hypertext_.hypertext.substr(shared_prefix, new_len);
        size_t shared_unicode_prefix = UTF16ToUnicodeOffsetInText(shared_prefix);
        size_t shared_unicode_suffix = UTF16ToUnicodeOffsetInText(shared_prefix + new_len);
        g_signal_emit_by_name(atk_object, "text-insert",
...
const base::OffsetAdjuster::Adjustments& AXPlatformNodeAuraLinux::GetHypertextAdjustments()
{
    ...
    for (size_t i = 0; i < text_length; i++) {
        ...
        if ((i - original_i + 1) != 1) {
            text_unicode_adjustments_->push_back(base::OffsetAdjuster::Adjustment(original_i, i - original_i + 1, 1));
        }
    }
...
size_t AXPlatformNodeAuraLinux::UTF16ToUnicodeOffsetInText(size_t utf16_offset)
{
    size_t unicode_offset = utf16_offset;
    base::OffsetAdjuster::AdjustOffset(GetHypertextAdjustments(), &unicode_offset);
...
size_t AXPlatformNodeAuraLinux::UnicodeToUTF16OffsetInText(int unicode_offset)
{
    ...
    base::OffsetAdjuster::UnadjustOffset(GetHypertextAdjustments(), &utf16_offset);
    return utf16_offset;
}

int AXPlatformNodeAuraLinux::GetTextOffsetAtPoint(int x, int y, AtkCoordType atk_coord_type)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        atk_text::GetCharacterExtents(ATK_TEXT(atk_object), i, &out_x, &out_y, &out_width, &out_height, atk_coord_type);
...
gfx::Vector2d AXPlatformNodeAuraLinux::GetParentOriginInScreenCoordinates() const
{
    AtkObject* parent = GetParent();
    if (!parent)
        return gfx::Vector2d();

    const AXPlatformNode* parent_node = AXPlatformNode::FromNativeViewAccessible(parent);
    if (!parent)
        return gfx::Vector2d();

    return parent_node->GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped).OffsetFromOrigin();
}

gfx::Vector2d AXPlatformNodeAuraLinux::GetParentFrameOriginInScreenCoordinates() const
{
    AtkObject* frame = FindAtkObjectParentFrame(atk_object_);
    if (!frame)
        return gfx::Vector2d();

    const AXPlatformNode* frame_node = AXPlatformNode::FromNativeViewAccessible(frame);
    if (!frame_node)
        return gfx::Vector2d();

    return frame_node->GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped).OffsetFromOrigin();
}

gfx::Rect AXPlatformNodeAuraLinux::GetExtentsRelativeToAtkCoordinateType(AtkCoordType coord_type) const
{
    gfx::Rect extents = delegate_->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped);
    switch (coord_type) {
    case ATK_XY_SCREEN:
        break;
    case ATK_XY_WINDOW: {
...
void AXPlatformNodeAuraLinux::GetExtents(gint* x, gint* y, gint* width, gint* height, AtkCoordType coord_type)
{
...
void AXPlatformNodeAuraLinux::GetPosition(gint* x, gint* y, AtkCoordType coord_type)
{
    gfx::Rect extents = GetExtentsRelativeToAtkCoordinateType(coord_type);
    if (x)
        *x = extents.x();
...
gfx::NativeViewAccessible AXPlatformNodeAuraLinux::HitTestSync(gint x, gint y, AtkCoordType coord_type)
{
    ...
    while (true) {
        gfx::NativeViewAccessible hit_child = current_result->GetDelegate()->HitTestSync(scroll_to.x(), scroll_to.y());
        if (!hit_child)
            return nullptr;
...
bool AXPlatformNodeAuraLinux::FocusFirstFocusableAncestorInWebContent()
{
    ...
    for (auto child_iterator_ptr = parent->GetDelegate()->ChildrenBegin(); *child_iterator_ptr != *parent->GetDelegate()->ChildrenEnd();
         ++(*child_iterator_ptr)) {
...
{
    AXActionData action_data;
    action_data.action = ax::mojom::Action::kSetSequentialFocusNavigationStartingPoint;
    return delegate_->AccessibilityPerformAction(action_data);
}

bool AXPlatformNodeAuraLinux::GrabFocusOrSetSequentialFocusNavigationStartingPoint()
{
...
bool AXPlatformNodeAuraLinux::GrabFocusOrSetSequentialFocusNavigationStartingPointAtOffset(int offset)
{
...
const gchar* AXPlatformNodeAuraLinux::GetDefaultActionName()
{
    ...
    // with a key press.
    if (GetRole() == ax::mojom::Role::kButton && action == static_cast<int>(ax::mojom::DefaultActionVerb::kPress) && !IsFocusable()) {
        action = static_cast<int>(ax::mojom::DefaultActionVerb::kClick);
    }

    std::string action_verb = ui::ToString(static_cast<ax::mojom::DefaultActionVerb>(action));
...
const gchar* AXPlatformNodeAuraLinux::GetDocumentAttributeValue(const gchar* attribute) const
{
...
AtkAttributeSet* AXPlatformNodeAuraLinux::GetDocumentAttributes() const
{
    ...
    for (unsigned i = 0; i < G_N_ELEMENTS(doc_attributes); i++) {
        ...
        if (value) {
            attribute_set = PrependAtkAttributeToAtkAttributeSet(doc_attributes[i], value, attribute_set);
        }
    }
...
AtkHyperlink* AXPlatformNodeAuraLinux::GetAtkHyperlink()
{
    ...
    atk_hyperlink_ = ATK_HYPERLINK(g_object_new(AX_PLATFORM_ATK_HYPERLINK_TYPE, 0));
    ax_platform_atk_hyperlink_set_object(AX_PLATFORM_ATK_HYPERLINK(atk_hyperlink_.get()), this);
    return atk_hyperlink_;
}
...
void AXPlatformNodeAuraLinux::GetFloatAttributeInGValue(ax::mojom::FloatAttribute attr, GValue* value)
{
    float float_val;
...
void AXPlatformNodeAuraLinux::AddAttributeToList(const char* name, const char* value, AtkAttributeSet** attributes)
{
    *attributes = PrependAtkAttributeToAtkAttributeSet(name, value, *attributes);
}

void AXPlatformNodeAuraLinux::SetDocumentParent(AtkObject* new_document_parent)
{
...
int AXPlatformNodeAuraLinux::GetCaretOffset()
{
    if (!HasVisibleCaretOrSelection()) {
        std::optional<FindInPageResultInfo> result = GetSelectionOffsetsFromFindInPage();
        AtkObject* atk_object = GetOrCreateAtkObject();
        if (!atk_object)
            return -1;
...
bool AXPlatformNodeAuraLinux::SetTextSelectionForAtkText(int start_offset, int end_offset)
{
...
{
    std::pair<int, int> selection = GetSelectionOffsetsForAtk();
    return selection.first >= 0 && selection.second >= 0 && selection.first != selection.second;
}

void AXPlatformNodeAuraLinux::GetSelectionExtents(int* start_offset, int* end_offset)
{
    if (start_offset)
        *start_offset = 0;
    if (end_offset)
    ...
    std::pair<int, int> selection = GetSelectionOffsetsForAtk();
    if (selection.first < 0 || selection.second < 0 || selection.first == selection.second)
        return;
...
gchar* AXPlatformNodeAuraLinux::GetSelectionWithText(int* start_offset, int* end_offset)
{
    ...
    if (selection_start < 0 || selection_end < 0 || selection_start == selection_end) {
        std::optional<FindInPageResultInfo> find_in_page_result = GetSelectionOffsetsFromFindInPage();
        if (!find_in_page_result.has_value() || find_in_page_result->node != atk_object) {
            *start_offset = 0;
            *end_offset = 0;
            return nullptr;
        }
    ...
    return atk_text::GetText(ATK_TEXT(atk_object), selection_start, selection_end);
}
...
void AXPlatformNodeAuraLinux::ScrollToPoint(AtkCoordType atk_coord_type, int x, int y)
{
    gfx::Point scroll_to(x, y);
...
void AXPlatformNodeAuraLinux::ScrollNodeRectIntoView(gfx::Rect rect, AtkScrollType atk_scroll_type)
{
    AXActionData action_data;
    action_data.target_node_id = GetData().id;
    ...
    case ATK_SCROLL_TOP_LEFT:
        action_data.vertical_scroll_alignment = ax::mojom::ScrollAlignment::kScrollAlignmentTop;
        action_data.horizontal_scroll_alignment = ax::mojom::ScrollAlignment::kScrollAlignmentLeft;
        break;
    case ATK_SCROLL_BOTTOM_RIGHT:
        action_data.horizontal_scroll_alignment = ax::mojom::ScrollAlignment::kScrollAlignmentRight;
        action_data.vertical_scroll_alignment = ax::mojom::ScrollAlignment::kScrollAlignmentBottom;
        break;
...
void AXPlatformNodeAuraLinux::ScrollNodeIntoView(AtkScrollType atk_scroll_type)
{
    gfx::Rect rect = GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped);
    rect -= rect.OffsetFromOrigin();
    ScrollNodeRectIntoView(rect, atk_scroll_type);
}

std::optional<gfx::Rect> AXPlatformNodeAuraLinux::GetUnclippedHypertextRangeBoundsRect(int start_offset, int end_offset)
{
    start_offset = UnicodeToUTF16OffsetInText(start_offset);
    end_offset = UnicodeToUTF16OffsetInText(end_offset);
    ...
    return GetDelegate()->GetHypertextRangeBoundsRect(
        UnicodeToUTF16OffsetInText(start_offset), UnicodeToUTF16OffsetInText(end_offset), AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped);
}

bool AXPlatformNodeAuraLinux::ScrollSubstringIntoView(AtkScrollType atk_scroll_type, int start_offset, int end_offset)
{
    std::optional<gfx::Rect> optional_rect = GetUnclippedHypertextRangeBoundsRect(start_offset, end_offset);
    if (!optional_rect.has_value())
        return false;

    gfx::Rect rect = *optional_rect;
    gfx::Rect node_rect = GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped);
...
bool AXPlatformNodeAuraLinux::ScrollSubstringToPoint(int start_offset, int end_offset, AtkCoordType atk_coord_type, int x, int y)
{
    std::optional<gfx::Rect> optional_rect = GetUnclippedHypertextRangeBoundsRect(start_offset, end_offset);
    if (!optional_rect.has_value())
        return false;
    ...
    gfx::Rect rect = *optional_rect;
    gfx::Rect node_rect = GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped);
    ScrollToPoint(atk_coord_type, x - (rect.x() - node_rect.x()), y - (rect.y() - node_rect.y()));
...
void AXPlatformNodeAuraLinux::ComputeStylesIfNeeded()
{
    ...
    TextAttributeMap attributes_map = GetDelegate()->ComputeTextAttributeMap(default_text_attributes_);
    offset_to_text_attributes_.swap(attributes_map);
}

int AXPlatformNodeAuraLinux::FindStartOfStyle(int start_offset, ax::mojom::MoveDirection direction)
{
    ...
    case ax::mojom::MoveDirection::kForward: {
        const auto iterator = offset_to_text_attributes_.upper_bound(start_offset);
...
const TextAttributeList& AXPlatformNodeAuraLinux::GetTextAttributes(int offset, int* start_offset, int* end_offset)
{
    ComputeStylesIfNeeded();
    DCHECK(!offset_to_text_attributes_.empty());

    int utf16_offset = UnicodeToUTF16OffsetInText(offset);
    int style_start = FindStartOfStyle(utf16_offset, ax::mojom::MoveDirection::kBackward);
    int style_end = FindStartOfStyle(utf16_offset, ax::mojom::MoveDirection::kForward);

    auto iterator = offset_to_text_attributes_.find(style_start);
    CHECK(iterator != offset_to_text_attributes_.end(), base::NotFatalUntil::M130);

    SetIntPointerValueIfNotNull(start_offset, UTF16ToUnicodeOffsetInText(style_start));
    SetIntPointerValueIfNotNull(end_offset, UTF16ToUnicodeOffsetInText(style_end));
...
void AXPlatformNodeAuraLinux::ActivateFindInPageResult(int start_offset, int end_offset)
{
    AtkObject* atk_object = GetOrCreateAtkObject();
    if (!atk_object)
        return;
    ...
    std::map<AtkObject*, FindInPageResultInfo>& active_results = GetActiveFindInPageResults();
    ...
    g_signal_emit_by_name(atk_object, "text-selection-changed");
    g_signal_emit_by_name(atk_object, "text-caret-moved", UTF16ToUnicodeOffsetInText(end_offset));
}

std::optional<std::pair<int, int>> AXPlatformNodeAuraLinux::GetHypertextExtentsOfChild(AXPlatformNodeAuraLinux* child_to_find)
{
    int current_offset = 0;
    for (auto child_iterator_ptr = GetDelegate()->ChildrenBegin(); *child_iterator_ptr != *GetDelegate()->ChildrenEnd(); ++(*child_iterator_ptr)) {
        auto* child = FromAtkObject(child_iterator_ptr->GetNativeViewAccessible());
...
void AXPlatformNodeAuraLinux::ActivateFindInPageInParent(int start_offset, int end_offset)
{
    auto* parent = FromAtkObject(GetParent());
    if (!parent)
        return;

    std::optional<std::pair<int, int>> extents_in_parent = parent->GetHypertextExtentsOfChild(this);
    if (!extents_in_parent.has_value())
        return;

    DCHECK(IsText());
    parent->ActivateFindInPageResult(extents_in_parent->first + start_offset, extents_in_parent->first + end_offset);
}
...
std::optional<FindInPageResultInfo> AXPlatformNodeAuraLinux::GetSelectionOffsetsFromFindInPage()
{
    AtkObject* atk_object = GetOrCreateAtkObject();
    if (!atk_object)
        return std::nullopt;
...
gfx::Point AXPlatformNodeAuraLinux::ConvertPointToScreenCoordinates(const gfx::Point& point, AtkCoordType atk_coord_type)
{
...
std::pair<int, int> AXPlatformNodeAuraLinux::GetSelectionOffsetsForAtk()
{
    ...
    if (GetDelegate()->IsWebContent()) {
        ...
        GetSelectionOffsetsFromTree(&unignored_selection, &selection.first, &selection.second);
    } else {
