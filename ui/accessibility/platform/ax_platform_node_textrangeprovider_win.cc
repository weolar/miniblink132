...
#define UIA_VALIDATE_TEXTRANGEPROVIDER_CALL()                                                                                                                  \
    if (!GetOwner() || !GetOwner()->GetDelegate() || !start() || !start()->GetAnchor() || !end() || !end()->GetAnchor())                                       \
...
public:
    explicit AXRangePhysicalPixelRectDelegate(AXPlatformNodeTextRangeProviderWin* host)
        : host_(host)
    {
    }

    gfx::Rect GetInnerTextRangeBoundsRect(AXTreeID tree_id, AXNodeID node_id, int start_offset, int end_offset, AXClippingBehavior clipping_behavior,
        AXOffscreenResult* offscreen_result) override
    {
        AXPlatformNodeDelegate* delegate = host_->GetDelegate(tree_id, node_id);
        DCHECK(delegate);
        return delegate->GetInnerTextRangeBoundsRect(start_offset, end_offset, AXCoordinateSystem::kScreenPhysicalPixels, clipping_behavior, offscreen_result);
    }
    ...
    gfx::Rect GetBoundsRect(AXTreeID tree_id, AXNodeID node_id, AXOffscreenResult* offscreen_result) override
    {
        AXPlatformNodeDelegate* delegate = host_->GetDelegate(tree_id, node_id);
        DCHECK(delegate);
        return delegate->GetBoundsRect(AXCoordinateSystem::kScreenPhysicalPixels, AXClippingBehavior::kClipped, offscreen_result);
    }
...
void AXPlatformNodeTextRangeProviderWin::CreateTextRangeProvider(AXPositionInstance start, AXPositionInstance end, ITextRangeProvider** text_range_provider)
{
    DCHECK(text_range_provider);
    DCHECK_EQ(*text_range_provider, nullptr);
    *text_range_provider = nullptr;

    CComObject<AXPlatformNodeTextRangeProviderWin>* text_range_provider_win = nullptr;
    if (SUCCEEDED(CComObject<AXPlatformNodeTextRangeProviderWin>::CreateInstance(&text_range_provider_win))) {
...
void AXPlatformNodeTextRangeProviderWin::CreateTextRangeProviderForTesting(
    AXPlatformNodeWin* owner, AXPositionInstance start, AXPositionInstance end, ITextRangeProvider** text_range_provider)
{
    ...
    if (SUCCEEDED((*text_range_provider)->QueryInterface(IID_PPV_ARGS(&text_range_provider_win)))) {
...
HRESULT AXPlatformNodeTextRangeProviderWin::Compare(ITextRangeProvider* other, BOOL* result)
{
    ScopedAXEmbeddedObjectBehaviorSetter ax_embedded_object_behavior(AXEmbeddedObjectBehavior::kUIAExposeCharacterForTextContent);
    ...
    if (*start() == *(other_provider->start()) && *end() == *(other_provider->end())) {
        *result = TRUE;
    }
    return S_OK;
}
...
HRESULT AXPlatformNodeTextRangeProviderWin::CompareEndpoints(
    TextPatternRangeEndpoint this_endpoint, ITextRangeProvider* other, TextPatternRangeEndpoint other_endpoint, int* result)
{
    UIA_VALIDATE_TEXTRANGEPROVIDER_CALL_1_IN_1_OUT(other, result);
    ...
    WIN_ACCESSIBILITY_SOURCE_API_PERF_HISTOGRAM(UMA_API_TEXTRANGE_COMPAREENDPOINTS);
    ...
    const AXPositionInstance& this_provider_endpoint = (this_endpoint == TextPatternRangeEndpoint_Start) ? start() : end();
    const AXPositionInstance& other_provider_endpoint = (other_endpoint == TextPatternRangeEndpoint_Start) ? other_provider->start() : other_provider->end();

    std::optional<int> comparison = this_provider_endpoint->CompareTo(*other_provider_endpoint);
    if (!comparison)
        return UIA_E_INVALIDOPERATION;
...
HRESULT AXPlatformNodeTextRangeProviderWin::ExpandToEnclosingUnit(TextUnit unit)
{
    ...
    WIN_ACCESSIBILITY_SOURCE_API_PERF_HISTOGRAM(UMA_API_TEXTRANGE_EXPANDTOENCLOSINGUNIT);
    return ExpandToEnclosingUnitImpl(unit);
}

HRESULT AXPlatformNodeTextRangeProviderWin::ExpandToEnclosingUnitImpl(TextUnit unit)
{
    UIA_VALIDATE_TEXTRANGEPROVIDER_CALL();
    {
    ...
    case TextUnit_Character: {
        ...
        SetEnd(start()->CreateNextCharacterPosition({ AXBoundaryBehavior::kCrossBoundary, AXBoundaryDetection::kDontCheckInitialPosition }));
        ...
        if (end()->IsNullPosition()) {
            ...
            SetStart(start()->CreatePreviousCharacterPosition({ AXBoundaryBehavior::kCrossBoundary, AXBoundaryDetection::kDontCheckInitialPosition }));
    ...
    case TextUnit_Format:
        SetStart(start()->CreatePreviousFormatStartPosition({ AXBoundaryBehavior::kStopAtAnchorBoundary, AXBoundaryDetection::kCheckInitialPosition }));
        SetEnd(start()->CreateNextFormatEndPosition({ AXBoundaryBehavior::kStopAtLastAnchorBoundary, AXBoundaryDetection::kDontCheckInitialPosition }));
        break;
    case TextUnit_Word: {
        AXPositionInstance start_backup = start()->Clone();
        SetStart(start()->CreatePreviousWordStartPosition({ AXBoundaryBehavior::kStopAtAnchorBoundary, AXBoundaryDetection::kCheckInitialPosition }));
        ...
        SetEnd(start()->CreateNextWordStartPosition({ AXBoundaryBehavior::kStopAtAnchorBoundary, AXBoundaryDetection::kDontCheckInitialPosition }));
        break;
    }
    case TextUnit_Line:
        ...
        SetStart(start()->CreateBoundaryStartPosition({ AXBoundaryBehavior::kStopAtLastAnchorBoundary, AXBoundaryDetection::kCheckInitialPosition },
            ax::mojom::MoveDirection::kBackward, base::BindRepeating(&AtStartOfLinePredicate), base::BindRepeating(&AtEndOfLinePredicate)));
        // From the start we just walked backwards to, walk forwards to the line
        // end position.
        SetEnd(start()->CreateBoundaryEndPosition({ AXBoundaryBehavior::kStopAtLastAnchorBoundary, AXBoundaryDetection::kDontCheckInitialPosition },
            ax::mojom::MoveDirection::kForward, base::BindRepeating(&AtStartOfLinePredicate), base::BindRepeating(&AtEndOfLinePredicate)));
        break;
    case TextUnit_Paragraph:
        SetStart(start()->CreatePreviousParagraphStartPositionSkippingEmptyParagraphs(
            { AXBoundaryBehavior::kStopAtLastAnchorBoundary, AXBoundaryDetection::kCheckInitialPosition }));
    ...
    case TextUnit_Page: {
        ...
        if (common_anchor->tree()->HasPaginationSupport()) {
            SetStart(start()->CreatePreviousPageStartPosition({ AXBoundaryBehavior::kStopAtLastAnchorBoundary, AXBoundaryDetection::kCheckInitialPosition }));
            SetEnd(start()->CreateNextPageEndPosition({ AXBoundaryBehavior::kStopAtAnchorBoundary, AXBoundaryDetection::kCheckInitialPosition }));
            break;
        }
    }
        [[fallthrough]];
...
HRESULT AXPlatformNodeTextRangeProviderWin::FindAttribute(
    TEXTATTRIBUTEID text_attribute_id, VARIANT attribute_val, BOOL is_backward, ITextRangeProvider** result)
{
    ...
    *result = nullptr;
    ...
    auto expand_match = [&matched_range_start, &matched_range_end, is_backward](auto& current_start, auto& current_end) {
    ...
    HRESULT hr_result = is_backward ? FindAttributeRange(text_attribute_id, attribute_val, anchors.crbegin(), anchors.crend(), expand_match)
                                    : FindAttributeRange(text_attribute_id, attribute_val, anchors.cbegin(), anchors.cend(), expand_match);
    if (FAILED(hr_result))
        return E_FAIL;

    if (matched_range_start != nullptr && matched_range_end != nullptr) {
        CreateTextRangeProvider(std::move(matched_range_start), std::move(matched_range_end), result);
    }
    return S_OK;
}
...
HRESULT AXPlatformNodeTextRangeProviderWin::FindAttributeRange(
    const TEXTATTRIBUTEID text_attribute_id, VARIANT attribute_val, const AnchorIterator first, const AnchorIterator last, ExpandMatchLambda expand_match)
{
    ...
    for (auto it = first; it != last; ++it) {
        ...
        current_platform_node = static_cast<AXPlatformNodeWin*>(delegate->GetFromNodeID(current_start->GetAnchor()->id()));

        base::win::VariantVector current_attribute_value;
        if (FAILED(current_platform_node->GetTextAttributeValue(
                text_attribute_id, current_start->text_offset(), current_end->text_offset(), &current_attribute_value))) {
            return E_FAIL;
        }
...
HRESULT AXPlatformNodeTextRangeProviderWin::FindText(BSTR string, BOOL backwards, BOOL ignore_case, ITextRangeProvider** result)
{
    ...
    ScopedAXEmbeddedObjectBehaviorSetter ax_embedded_object_behavior_find_text(AXEmbeddedObjectBehavior::kSuppressCharacter);
    ...
    size_t find_start;
    size_t find_length;
    if (base::i18n::StringSearch(search_string, text_range, &find_start, &find_length, !ignore_case, !backwards)) {
        ...
        const AXNode* common_anchor = start()->LowestCommonAnchor(*end());
        AXPositionInstance start_ancestor_position = start()->CreateAncestorPosition(common_anchor, ax::mojom::MoveDirection::kForward);
        DCHECK(!start_ancestor_position->IsNullPosition());
        AXPositionInstance end_ancestor_position = end()->CreateAncestorPosition(common_anchor, ax::mojom::MoveDirection::kForward);
        DCHECK(!end_ancestor_position->IsNullPosition());
        const AXNode* anchor = start_ancestor_position->GetAnchor();
        DCHECK(anchor);
        const int start_offset = start_ancestor_position->text_offset() + find_start;
        const int end_offset = start_offset + find_length - GetAppendedNewLinesCountInRange(find_start, find_length, appended_newlines_indices);
        const int max_end_offset = end_ancestor_position->text_offset();
        DCHECK(start_offset <= end_offset && end_offset <= max_end_offset);

        AXPositionInstance start = AXNodePosition::CreateTextPosition(*anchor, start_offset, ax::mojom::TextAffinity::kDownstream)->AsLeafTextPosition();
        AXPositionInstance end = AXNodePosition::CreateTextPosition(*anchor, end_offset, ax::mojom::TextAffinity::kDownstream)->AsLeafTextPosition();
...
HRESULT AXPlatformNodeTextRangeProviderWin::GetAttributeValue(TEXTATTRIBUTEID attribute_id, VARIANT* value)
{
    ...
    WIN_ACCESSIBILITY_SOURCE_API_PERF_HISTOGRAM(UMA_API_TEXTRANGE_GETATTRIBUTEVALUE);
    ...
    // paragraph boundary.
    if (attribute_id == UIA_IsReadOnlyAttributeId && start()->anchor_id() != end()->anchor_id() && start()->AtEndOfParagraph() && end()->AtStartOfParagraph()
        && *start()->CreateNextCharacterPosition({ AXBoundaryBehavior::kCrossBoundary, AXBoundaryDetection::kDontCheckInitialPosition }) == *end()) {
        AXPlatformNodeWin* common_anchor = GetLowestAccessibleCommonPlatformNode();
        DCHECK(common_anchor);

        HRESULT hr = common_anchor->GetTextAttributeValue(attribute_id, std::nullopt, std::nullopt, &attribute_value);
    ...
    for (auto it = normalized_start->AsLeafTextPosition(); it->anchor_id() != end->anchor_id() || it->tree_id() != end->tree_id();
        ...
        AXPlatformNodeWin* platform_node = static_cast<AXPlatformNodeWin*>(delegate->GetFromNodeID(it->anchor_id()));
        DCHECK(platform_node);

        // Only get attributes for nodes in the tree. Exclude descendants of leaves
        // and ignored objects.
        platform_node = static_cast<AXPlatformNodeWin*>(AXPlatformNode::FromNativeViewAccessible(platform_node->GetDelegate()->GetLowestPlatformAncestor()));
        DCHECK(platform_node);

        base::win::VariantVector current_value;
        const bool at_end_leaf_text_anchor = it->anchor_id() == end_leaf_text_position->anchor_id() && it->tree_id() == end_leaf_text_position->tree_id();
        const std::optional<int> start_offset = it->IsTextPosition() ? std::make_optional(it->text_offset()) : std::nullopt;
        const std::optional<int> end_offset = at_end_leaf_text_anchor ? std::make_optional(end_leaf_text_position->text_offset()) : std::nullopt;
        HRESULT hr = platform_node->GetTextAttributeValue(attribute_id, start_offset, end_offset, &current_value);
        if (FAILED(hr))
            return E_FAIL;

        if (attribute_id == UIA_AnnotationTypesAttributeId && current_value.Type() == VT_EMPTY) {
...
HRESULT AXPlatformNodeTextRangeProviderWin::GetBoundingRectangles(SAFEARRAY** screen_physical_pixel_rectangles)
{
    ...
    WIN_ACCESSIBILITY_SOURCE_API_PERF_HISTOGRAM(UMA_API_TEXTRANGE_GETBOUNDINGRECTANGLES);
    ...
    *screen_physical_pixel_rectangles = nullptr;
    ...
    SAFEARRAY* safe_array = SafeArrayCreateVector(VT_R8 /* element type */, 0 /* lower bound */, rects.size() * 4);
    ...
    if (rects.size() > 0) {
        ...
        HRESULT hr = SafeArrayAccessData(safe_array, reinterpret_cast<void**>(&double_array));
...
HRESULT AXPlatformNodeTextRangeProviderWin::GetEnclosingElement(IRawElementProviderSimple** element)
{
    ...
    WIN_ACCESSIBILITY_SOURCE_API_PERF_HISTOGRAM(UMA_API_TEXTRANGE_GETENCLOSINGELEMENT);

    AXPlatformNodeWin* enclosing_node = GetLowestAccessibleCommonPlatformNode();
    if (!enclosing_node)
        return UIA_E_ELEMENTNOTAVAILABLE;

    enclosing_node->GetNativeViewAccessible()->QueryInterface(IID_PPV_ARGS(element));
...
HRESULT AXPlatformNodeTextRangeProviderWin::Move(TextUnit unit, int count, int* units_moved)
{
    ...
    HRESULT hr = MoveEndpointByUnitImpl(TextPatternRangeEndpoint_Start, unit, count, &start_units_moved);
    ...
    if (succeeded_move) {
        ...
        if (!is_degenerate_range) {
            ...
            if (forwards && start()->AtEndOfContent()) {
                ...
                hr = MoveEndpointByUnitImpl(TextPatternRangeEndpoint_Start, unit, -1, &current_start_units_moved);
                start_units_moved -= 1;
                succeeded_move = SUCCEEDED(hr) && current_start_units_moved == -1 && start_units_moved > 0;
            } else {
                ...
                hr = MoveEndpointByUnitImpl(TextPatternRangeEndpoint_End, unit, 1, &end_units_moved);
            ...
            if (start()->anchor_id() != end()->anchor_id() && (unit == TextUnit_Character || unit == TextUnit_Word)) {
...
HRESULT AXPlatformNodeTextRangeProviderWin::MoveEndpointByUnit(TextPatternRangeEndpoint endpoint, TextUnit unit, int count, int* units_moved)
{
    ScopedAXEmbeddedObjectBehaviorSetter ax_embedded_object_behavior(AXEmbeddedObjectBehavior::kUIAExposeCharacterForTextContent);
    UIA_VALIDATE_TEXTRANGEPROVIDER_CALL_1_OUT(units_moved);
    ...
    WIN_ACCESSIBILITY_SOURCE_API_PERF_HISTOGRAM(UMA_API_TEXTRANGE_MOVEENDPOINTBYUNIT);
    return MoveEndpointByUnitImpl(endpoint, unit, count, units_moved);
}

HRESULT AXPlatformNodeTextRangeProviderWin::MoveEndpointByUnitImpl(TextPatternRangeEndpoint endpoint, TextUnit unit, int count, int* units_moved)
{
    ScopedAXEmbeddedObjectBehaviorSetter ax_embedded_object_behavior(AXEmbeddedObjectBehavior::kUIAExposeCharacterForTextContent);
    ...
    AXPositionInstance position_to_move = is_start_endpoint ? start()->Clone() : end()->Clone();

    AXPositionInstance new_position;
    switch (unit) {
    case TextUnit_Character:
        new_position = MoveEndpointByCharacter(position_to_move, count, units_moved);
        break;
    case TextUnit_Format:
        new_position = MoveEndpointByFormat(position_to_move, is_start_endpoint, count, units_moved);
        break;
    case TextUnit_Word:
        new_position = MoveEndpointByWord(position_to_move, count, units_moved);
        break;
    case TextUnit_Line:
        new_position = MoveEndpointByLine(position_to_move, is_start_endpoint, count, units_moved);
        break;
    case TextUnit_Paragraph:
        new_position = MoveEndpointByParagraph(position_to_move, is_start_endpoint, count, units_moved);
        break;
    case TextUnit_Page:
        new_position = MoveEndpointByPage(position_to_move, is_start_endpoint, count, units_moved);
        break;
    case TextUnit_Document:
        new_position = MoveEndpointByDocument(position_to_move, count, units_moved);
        break;
    default:
        return UIA_E_NOTSUPPORTED;
    }
    ...
    std::optional<int> endpoint_comparison = AXNodeRange::CompareEndpoints(start().get(), end().get());
...
HRESULT AXPlatformNodeTextRangeProviderWin::MoveEndpointByRange(
    TextPatternRangeEndpoint this_endpoint, ITextRangeProvider* other, TextPatternRangeEndpoint other_endpoint)
{
    ...
    WIN_ACCESSIBILITY_SOURCE_API_PERF_HISTOGRAM(UMA_API_TEXTRANGE_MOVEENPOINTBYRANGE);
...
HRESULT AXPlatformNodeTextRangeProviderWin::Select()
{
    ...
    if (atomic_text_field && start_anchor != end_anchor) {
        ...
        if (non_atomic_text_field->GetTextFieldAncestor() == atomic_text_field) {
            if (start_anchor == atomic_text_field) {
                selection_end = selection_end->CreateAncestorPosition(start_anchor, ax::mojom::MoveDirection::kForward);
            } else {
                selection_start = selection_start->CreateAncestorPosition(end_anchor, ax::mojom::MoveDirection::kForward);
            }
        }
    }
    ...
    // is fixed.
    if (selection_start->GetAnchor()->IsInListMarker() || selection_end->GetAnchor()->IsInListMarker()) {
        return S_OK;
    }

    AXPlatformNodeDelegate* delegate = GetDelegate(selection_start->tree_id(), selection_start->anchor_id());
    DCHECK(delegate);

    AXNodeRange new_selection_range(std::move(selection_start), std::move(selection_end));
...
HRESULT AXPlatformNodeTextRangeProviderWin::ScrollIntoView(BOOL align_to_top)
{
    ...
    AXPlatformNode* start_platform_node = GetOwner()->GetDelegate()->GetFromTreeIDAndNodeID(start()->tree_id(), start()->GetAnchor()->id());
    AXPlatformNode* end_platform_node = GetOwner()->GetDelegate()->GetFromTreeIDAndNodeID(end()->tree_id(), end()->GetAnchor()->id());

    // If both anchors are onscreen, don't scroll.
    if (!start_platform_node->GetDelegate()->IsOffscreen() && !end_platform_node->GetDelegate()->IsOffscreen()) {
        return S_OK;
    }

    const AXPositionInstance start_common_ancestor = start()->LowestCommonAncestorPosition(*end(), ax::mojom::MoveDirection::kBackward);
    const AXPositionInstance end_common_ancestor = end()->LowestCommonAncestorPosition(*start(), ax::mojom::MoveDirection::kForward);
    if (start_common_ancestor->IsNullPosition() || end_common_ancestor->IsNullPosition()) {
        return E_INVALIDARG;
    }
    ...
    const AXPlatformNodeDelegate* root_delegate = GetRootDelegate(common_ancestor_tree_id);
    DCHECK(root_delegate);
    const gfx::Rect root_frame_bounds = root_delegate->GetBoundsRect(AXCoordinateSystem::kFrame, AXClippingBehavior::kUnclipped);
    UIA_VALIDATE_BOUNDS(root_frame_bounds);

    const AXPlatformNode* common_ancestor_platform_node
        = GetOwner()->GetDelegate()->GetFromTreeIDAndNodeID(common_ancestor_tree_id, common_ancestor_anchor->id());
    DCHECK(common_ancestor_platform_node);
    AXPlatformNodeDelegate* common_ancestor_delegate = common_ancestor_platform_node->GetDelegate();
    DCHECK(common_ancestor_delegate);
    const gfx::Rect text_range_container_frame_bounds = common_ancestor_delegate->GetBoundsRect(AXCoordinateSystem::kFrame, AXClippingBehavior::kUnclipped);
    ...
    if (align_to_top) {
        ...
        target_point = gfx::Point(root_frame_bounds.x(), root_frame_bounds.y() + root_frame_bounds.height());
    }

    if ((align_to_top && start()->GetAnchor()->IsText()) || (!align_to_top && end()->GetAnchor()->IsText())) {
        const gfx::Rect text_range_frame_bounds = common_ancestor_delegate->GetInnerTextRangeBoundsRect(
            start_common_ancestor->text_offset(), end_common_ancestor->text_offset(), AXCoordinateSystem::kFrame, AXClippingBehavior::kUnclipped);
        UIA_VALIDATE_BOUNDS(text_range_frame_bounds);

        if (align_to_top) {
            target_point.Offset(0, -(text_range_container_frame_bounds.height() - text_range_frame_bounds.height()));
        } else {
    ...
    const gfx::Rect root_screen_bounds = root_delegate->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped);
...
HRESULT AXPlatformNodeTextRangeProviderWin::GetChildren(SAFEARRAY** children)
{
    ...
    AXPlatformNodeWin* start_anchor = GetPlatformNodeFromAXNode(start()->GetAnchor());
    ...
    descendants = common_delegate->GetUIADirectChildrenInRange(start_delegate, end_delegate);

    SAFEARRAY* safe_array = SafeArrayCreateVector(VT_UNKNOWN, 0, descendants.size());
...
// static
bool AXPlatformNodeTextRangeProviderWin::AtStartOfLinePredicate(const AXPositionInstance& position)
{
    return !position->IsIgnored() && position->AtStartOfAnchor() && position->AtStartOfLine();
}

// static
bool AXPlatformNodeTextRangeProviderWin::AtEndOfLinePredicate(const AXPositionInstance& position)
{
    return !position->IsIgnored() && position->AtEndOfAnchor() && position->AtEndOfLine();
}

// static
AXPlatformNodeTextRangeProviderWin::AXPositionInstance AXPlatformNodeTextRangeProviderWin::GetNextTextBoundaryPosition(
    const AXPositionInstance& position, ax::mojom::TextBoundary boundary_type, AXMovementOptions options, ax::mojom::MoveDirection boundary_direction)
{
    ...
    case ax::mojom::TextBoundary::kLineStart:
        return position->CreateBoundaryStartPosition(
            options, boundary_direction, base::BindRepeating(&AtStartOfLinePredicate), base::BindRepeating(&AtEndOfLinePredicate));
    case ax::mojom::TextBoundary::kLineEnd:
        return position->CreateBoundaryEndPosition(
            options, boundary_direction, base::BindRepeating(&AtStartOfLinePredicate), base::BindRepeating(&AtEndOfLinePredicate));
    default:
        return position->CreatePositionAtTextBoundary(boundary_type, boundary_direction, options);
    }
}

std::u16string AXPlatformNodeTextRangeProviderWin::GetString(int max_count, std::vector<size_t>* appended_newlines_indices)
{
    AXNodeRange range(start()->Clone(), end()->Clone());
    return range.GetText(AXTextConcatenationBehavior::kWithParagraphBreaks, AXEmbeddedObjectBehavior::kUIAExposeCharacterForTextContent, max_count, false,
        appended_newlines_indices);
}

size_t AXPlatformNodeTextRangeProviderWin::GetAppendedNewLinesCountInRange(
    size_t find_start, size_t find_length, const std::vector<size_t>& appended_newlines_indices)
{
    ...
    for (size_t i = 0; i < appended_newlines_indices.size(); ++i) {
        ...
        if (appended_newlines_indices[i] >= find_start && appended_newlines_indices[i] < find_start + find_length) {
...
AXPlatformNodeWin* AXPlatformNodeTextRangeProviderWin::GetOwner() const
{
    ...
    const AXPositionInstance& position = !start()->IsNullPosition() ? start() : end();
    ...
    if (ax_tree_manager && ax_tree_manager->IsPlatformTreeManager()) {
        const AXPlatformTreeManager* platform_tree_manager = static_cast<const AXPlatformTreeManager*>(ax_tree_manager);
        DCHECK(platform_tree_manager);

        return static_cast<AXPlatformNodeWin*>(platform_tree_manager->GetPlatformNodeFromTree(*anchor));
    }

    return nullptr;
}

AXPlatformNodeDelegate* AXPlatformNodeTextRangeProviderWin::GetDelegate() const
{
    if (GetOwner()) {
...
AXPlatformNodeDelegate* AXPlatformNodeTextRangeProviderWin::GetDelegate(const AXPositionInstanceType* position) const
{
    return GetDelegate(position->tree_id(), position->anchor_id());
}

AXPlatformNodeDelegate* AXPlatformNodeTextRangeProviderWin::GetDelegate(const AXTreeID tree_id, const AXNodeID node_id) const
{
    AXPlatformNode* platform_node = GetOwner()->GetDelegate()->GetFromTreeIDAndNodeID(tree_id, node_id);
    if (!platform_node)
        return nullptr;
...
AXPlatformNodeTextRangeProviderWin::AXPositionInstance AXPlatformNodeTextRangeProviderWin::MoveEndpointByCharacter(
    const AXPositionInstance& endpoint, const int count, int* units_moved)
{
    return MoveEndpointByUnitHelper(std::move(endpoint), ax::mojom::TextBoundary::kCharacter, count, units_moved);
}

AXPlatformNodeTextRangeProviderWin::AXPositionInstance AXPlatformNodeTextRangeProviderWin::MoveEndpointByWord(
    const AXPositionInstance& endpoint, const int count, int* units_moved)
{
    return MoveEndpointByUnitHelper(std::move(endpoint), ax::mojom::TextBoundary::kWordStart, count, units_moved);
}

AXPlatformNodeTextRangeProviderWin::AXPositionInstance AXPlatformNodeTextRangeProviderWin::MoveEndpointByLine(
    const AXPositionInstance& endpoint, bool is_start_endpoint, const int count, int* units_moved)
{
    return MoveEndpointByUnitHelper(
        std::move(endpoint), is_start_endpoint ? ax::mojom::TextBoundary::kLineStart : ax::mojom::TextBoundary::kLineEnd, count, units_moved);
}

AXPlatformNodeTextRangeProviderWin::AXPositionInstance AXPlatformNodeTextRangeProviderWin::MoveEndpointByFormat(
    const AXPositionInstance& endpoint, const bool is_start_endpoint, const int count, int* units_moved)
{
    return MoveEndpointByUnitHelper(
        std::move(endpoint), is_start_endpoint ? ax::mojom::TextBoundary::kFormatStart : ax::mojom::TextBoundary::kFormatEnd, count, units_moved);
}

AXPlatformNodeTextRangeProviderWin::AXPositionInstance AXPlatformNodeTextRangeProviderWin::MoveEndpointByParagraph(
    const AXPositionInstance& endpoint, const bool is_start_endpoint, const int count, int* units_moved)
{
    return MoveEndpointByUnitHelper(std::move(endpoint), ax::mojom::TextBoundary::kParagraphStartSkippingEmptyParagraphs, count, units_moved);
}

AXPlatformNodeTextRangeProviderWin::AXPositionInstance AXPlatformNodeTextRangeProviderWin::MoveEndpointByPage(
    const AXPositionInstance& endpoint, const bool is_start_endpoint, const int count, int* units_moved)
{
    ...
    AXPositionInstance common_ancestor = start()->LowestCommonAncestorPosition(*end(), ax::mojom::MoveDirection::kBackward);
    if (!common_ancestor->GetAnchor()->tree()->HasPaginationSupport())
        return MoveEndpointByDocument(std::move(endpoint), count, units_moved);

    return MoveEndpointByUnitHelper(
        std::move(endpoint), is_start_endpoint ? ax::mojom::TextBoundary::kPageStart : ax::mojom::TextBoundary::kPageEnd, count, units_moved);
}

AXPlatformNodeTextRangeProviderWin::AXPositionInstance AXPlatformNodeTextRangeProviderWin::MoveEndpointByDocument(
    const AXPositionInstance& endpoint, const int count, int* units_moved)
{
    DCHECK_NE(count, 0);
...
AXPlatformNodeTextRangeProviderWin::AXPositionInstance AXPlatformNodeTextRangeProviderWin::MoveEndpointByUnitHelper(
    const AXPositionInstance& endpoint, const ax::mojom::TextBoundary boundary_type, const int count, int* units_moved)
{
    DCHECK_NE(count, 0);
    const ax::mojom::MoveDirection boundary_direction = (count > 0) ? ax::mojom::MoveDirection::kForward : ax::mojom::MoveDirection::kBackward;
    ...
    AXPositionInstance next_endpoint = GetNextTextBoundaryPosition(
        current_endpoint, boundary_type, { AXBoundaryBehavior::kStopAtLastAnchorBoundary, AXBoundaryDetection::kDontCheckInitialPosition }, boundary_direction);
    ...
    // to.
    while (iteration < std::abs(count) && !(next_endpoint->GetAnchor() == current_endpoint->GetAnchor() && *next_endpoint == *current_endpoint)) {
        is_ignored_for_text_navigation = false;
        current_endpoint = std::move(next_endpoint);

        next_endpoint = GetNextTextBoundaryPosition(current_endpoint, boundary_type,
            { AXBoundaryBehavior::kStopAtLastAnchorBoundary, AXBoundaryDetection::kDontCheckInitialPosition }, boundary_direction);
        DCHECK(next_endpoint->IsLeafTextPosition());
        ...
        is_ignored_for_text_navigation = boundary_type != ax::mojom::TextBoundary::kCharacter && current_endpoint->GetAnchor()->IsIgnoredForTextNavigation();
    ...
    *units_moved = (count > 0) ? iteration : -iteration;
    ...
    if (is_ignored_for_text_navigation && initial_endpoint != current_endpoint->GetAnchor()) {
...
void AXPlatformNodeTextRangeProviderWin::NormalizeTextRange(AXPositionInstance& start, AXPositionInstance& end)
{
    ...
    AXPositionInstance normalized_start = start->AsLeafTextPositionBeforeCharacter();
    ...
    AXPositionInstance normalized_end = is_degenerate ? normalized_start->Clone() : end->AsLeafTextPositionAfterCharacter();

    if (!normalized_start->IsNullPosition() && !normalized_end->IsNullPosition()) {
...
// static
void AXPlatformNodeTextRangeProviderWin::NormalizeAsUnignoredPosition(AXPositionInstance& position)
{
    if (position->IsNullPosition() || !position->IsValid())
        return;

    if (position->IsIgnored()) {
        AXPositionInstance normalized_position = position->AsUnignoredPosition(AXPositionAdjustmentBehavior::kMoveForward);
        if (normalized_position->IsNullPosition()) {
            normalized_position = position->AsUnignoredPosition(AXPositionAdjustmentBehavior::kMoveBackward);
        }
...
// static
void AXPlatformNodeTextRangeProviderWin::NormalizeAsUnignoredTextRange(AXPositionInstance& start, AXPositionInstance& end)
{
    if (!start->IsValid() || !end->IsValid())
        return;
...
AXPlatformNodeDelegate* AXPlatformNodeTextRangeProviderWin::GetRootDelegate(const AXTreeID tree_id)
{
    ...
    const AXPlatformNode* root_platform_node = GetOwner()->GetDelegate()->GetFromTreeIDAndNodeID(tree_id, root_node->id());
    DCHECK(root_platform_node);
    return root_platform_node->GetDelegate();
}

void AXPlatformNodeTextRangeProviderWin::SetStart(AXPositionInstance new_start)
{
...
void AXPlatformNodeTextRangeProviderWin::SnapStartAndEndToMaxTextOffsetIfBeyond()
{
    if (start()) {
...
void AXPlatformNodeTextRangeProviderWin::SetOwnerForTesting(AXPlatformNodeWin* owner)
{
...
AXNode* AXPlatformNodeTextRangeProviderWin::GetSelectionCommonAnchor()
{
    ...
    AXPlatformNode* anchor_object = delegate->GetFromNodeID(unignored_selection.anchor_object_id);
    AXPlatformNode* focus_object = delegate->GetFromNodeID(unignored_selection.focus_object_id);

    if (!anchor_object || !focus_object)
        return nullptr;

    AXNodePosition::AXPositionInstance start = anchor_object->GetDelegate()->CreateTextPositionAt(unignored_selection.anchor_offset);
    AXNodePosition::AXPositionInstance end = focus_object->GetDelegate()->CreateTextPositionAt(unignored_selection.focus_offset);
...
void AXPlatformNodeTextRangeProviderWin::RemoveFocusFromPreviousSelectionIfNeeded(const AXNodeRange& new_selection)
{
    const AXNode* old_selection_node = GetSelectionCommonAnchor();
    const AXNode* new_selection_node = new_selection.anchor()->LowestCommonAnchor(*new_selection.focus());
    ...
    if (old_text_field_ancestor && new_text_field_ancestor && old_text_field_ancestor == new_text_field_ancestor) {
        return;
    }

    if (!new_selection_node || (old_selection_node->HasState(ax::mojom::State::kFocusable) && !new_selection_node->HasState(ax::mojom::State::kFocusable))) {
        AXPlatformNodeDelegate* root_delegate = GetRootDelegate(old_selection_node->tree()->GetAXTreeID());
        DCHECK(root_delegate);

        AXPlatformNodeWin* old_selection_platform_node = GetPlatformNodeFromAXNode(old_selection_node);
        ...
        old_selection_platform_node->GetDelegate()->AccessibilityPerformAction(blur_action);
    }
}

AXPlatformNodeWin* AXPlatformNodeTextRangeProviderWin::GetPlatformNodeFromAXNode(const AXNode* node) const
{
    if (!node)
        return nullptr;
    ...
    AXPlatformNodeWin* platform_node = static_cast<AXPlatformNodeWin*>(
        AXPlatformNode::FromNativeViewAccessible(GetDelegate(node->tree()->GetAXTreeID(), node->id())->GetNativeViewAccessible()));
    DCHECK(platform_node);

    return platform_node;
}

AXPlatformNodeWin* AXPlatformNodeTextRangeProviderWin::GetLowestAccessibleCommonPlatformNode() const
{
    AXNode* common_anchor = start()->LowestCommonAnchor(*end());
    if (!common_anchor)
        return nullptr;

    return GetPlatformNodeFromAXNode(common_anchor)->GetLowestAccessibleElementForUIA();
}

bool AXPlatformNodeTextRangeProviderWin::HasTextRangeOrSelectionInAtomicTextField(
    const AXPositionInstance& start_position, const AXPositionInstance& end_position) const
{
    ...
    // field's root node.
    bool is_start_in_text_field = start_position->GetAnchor()->IsDescendantOfAtomicTextField();
    bool is_end_in_text_field = end_position->GetAnchor()->IsDescendantOfAtomicTextField();
    ...
    return (is_start_in_text_field && is_end_in_text_field) || (is_start_in_text_field && start_delegate && start_delegate->HasVisibleCaretOrSelection())
        || (is_end_in_text_field && end_delegate && end_delegate->HasVisibleCaretOrSelection());
}

// static
bool AXPlatformNodeTextRangeProviderWin::TextAttributeIsArrayType(TEXTATTRIBUTEID attribute_id)
{
    // https://docs.microsoft.com/en-us/windows/win32/winauto/uiauto-textattribute-ids
    return attribute_id == UIA_AnnotationObjectsAttributeId || attribute_id == UIA_AnnotationTypesAttributeId || attribute_id == UIA_TabsAttributeId;
}

// static
bool AXPlatformNodeTextRangeProviderWin::TextAttributeIsUiaReservedValue(const base::win::VariantVector& vector)
{
    ...
    return !vector.Compare(mixed_attribute_value_variant) || !vector.Compare(not_supported_value_variant);
}

// static
bool AXPlatformNodeTextRangeProviderWin::ShouldReleaseTextAttributeAsSafearray(TEXTATTRIBUTEID attribute_id, const base::win::VariantVector& attribute_value)
{
    // |vector| may be pre-populated with a UIA reserved value. In such a case, we
    // must release as a scalar variant.
    return TextAttributeIsArrayType(attribute_id) && !TextAttributeIsUiaReservedValue(attribute_value);
}
...
const AXPlatformNodeTextRangeProviderWin::AXPositionInstance& AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::GetStart()
{
    ValidateEndpointsAfterNodeDeletionIfNeeded();
    return start_;
}

const AXPlatformNodeTextRangeProviderWin::AXPositionInstance& AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::GetEnd()
{
    ValidateEndpointsAfterNodeDeletionIfNeeded();
    return end_;
}

void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::SetStart(AXPositionInstance new_start)
{
    ...
    // IsNullPosition().
    if (did_tree_change && start_->kind() != AXPositionKind::NULL_POSITION && start_->tree_id() != end_->tree_id()) {
        RemoveObserver(start_->tree_id());
    }

    start_ = std::move(new_start);

    if (did_tree_change && !start_->IsNullPosition() && start_->tree_id() != end_->tree_id()) {
        AddObserver(start_->tree_id());
    }
}

void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::SetEnd(AXPositionInstance new_end)
{
    bool did_tree_change = end_->tree_id() != new_end->tree_id();
    // TODO(bebeaudr): We can't use IsNullPosition() here because of
    // https://crbug.com/1152939. Once this is fixed, we can go back to
    // IsNullPosition().
    if (did_tree_change && end_->kind() != AXPositionKind::NULL_POSITION && end_->tree_id() != start_->tree_id()) {
        RemoveObserver(end_->tree_id());
    }

    end_ = std::move(new_end);

    if (did_tree_change && !end_->IsNullPosition() && start_->tree_id() != end_->tree_id()) {
        AddObserver(end_->tree_id());
    }
}

void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::AddObserver(const AXTreeID tree_id)
{
...
void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::RemoveObserver(const AXTreeID tree_id)
{
    AXTreeManager* ax_tree_manager = AXTreeManager::FromID(tree_id);
    if (ax_tree_manager)
...
void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::OnStringAttributeChanged(
    AXTree* tree, AXNode* node, ax::mojom::StringAttribute attr, const std::string& old_value, const std::string& new_value)
{
    if (attr != ax::mojom::StringAttribute::kName || new_value.length() >= old_value.length()) {
        return;
    }
    if (!start_->IsNullPosition() && start_->tree_id() == node->tree()->GetAXTreeID() && start_->anchor_id() == node->id()) {
        start_->SnapToMaxTextOffsetIfBeyond();
    }
    if (!end_->IsNullPosition() && end_->tree_id() == node->tree()->GetAXTreeID() && end_->anchor_id() == node->id()) {
...
void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::OnSubtreeWillBeDeleted(AXTree* tree, AXNode* node)
{
...
void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::AdjustEndpointForSubtreeDeletion(AXTree* tree, const AXNode* const node, bool is_start_endpoint)
{
    AXPositionInstance endpoint = is_start_endpoint ? start_->Clone() : end_->Clone();
    ...
    if (!node->GetParent() || !endpoint_anchor) {
        is_start_endpoint ? SetStart(AXNodePosition::CreateNullPosition()) : SetEnd(AXNodePosition::CreateNullPosition());
        return;
    }
    ...
    if (endpoint->kind() == AXPositionKind::TREE_POSITION && endpoint_anchor == node->GetParent()) {
        if (is_start_endpoint)
    ...
    AXPositionInstance new_endpoint = endpoint->CreateAncestorPosition(node, ax::mojom::MoveDirection::kForward);
    ...
    AXPositionInstance other_endpoint = is_start_endpoint ? end_->Clone() : start_->Clone();
    ...
    if (!endpoint_anchor || endpoint_anchor->IsDescendantOfCrossingTreeBoundary(node))
...
void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::OnNodeDeleted(AXTree* tree, AXNodeID node_id)
{
    DCHECK(tree);
    ...
    if (validation_necessary_for_start_.has_value() && validation_necessary_for_start_->tree_id == tree->GetAXTreeID()
        && validation_necessary_for_start_->node_id == node_id) {
        validation_necessary_for_start_->validation_needed = true;
    }

    if (validation_necessary_for_end_.has_value() && validation_necessary_for_end_->tree_id == tree->GetAXTreeID()
        && validation_necessary_for_end_->node_id == node_id) {
        validation_necessary_for_end_->validation_needed = true;
    }
}

void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::OnTreeManagerWillBeRemoved(AXTreeID previous_tree_id)
{
    if (start_->tree_id() == previous_tree_id || end_->tree_id() == previous_tree_id) {
        RemoveObserver(previous_tree_id);
    }
}

void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::OnTextDeletionOrInsertion(const AXNode& node, const AXNodeData& new_data)
{
    DCHECK(new_data.IsTextField());

    const std::vector<int>& start_offsets = new_data.GetIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartOffsets);
    const std::vector<int>& end_offsets = new_data.GetIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndOffsets);
    const std::vector<int>& start_ids = new_data.GetIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartAnchorIds);
    const std::vector<int>& end_ids = new_data.GetIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndAnchorIds);
    const std::vector<int>& op_vector = new_data.GetIntListAttribute(ax::mojom::IntListAttribute::kTextOperations);
    ...
    for (size_t i = 0; i < start_offsets.size(); i++) {
        ...
        DCHECK(op == ax::mojom::Command::kDelete || op == ax::mojom::Command::kInsert);
        AXNode* edit_start_node = manager->GetNode(edit_start_id);
        AXNode* edit_end_node = manager->GetNode(edit_end_id);

        AdjustEndpointForTextFieldEdit(node, GetStart(), edit_start_node, edit_end_node, edit_start, edit_end,
            /* is_start */ true, op);
        AdjustEndpointForTextFieldEdit(node, GetEnd(), edit_start_node, edit_end_node, edit_start, edit_end,
            /* is_start */ false, op);
    }
}

void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::AdjustEndpointForTextFieldEdit(const AXNode& text_field_node,
    const AXPositionInstance& current_position, AXNode* edit_start_anchor, AXNode* edit_end_anchor, int edit_start, int edit_end, bool is_start,
    ax::mojom::Command op)
{
    if (!edit_start_anchor || !edit_end_anchor || !current_position->GetAnchor()) {
        return;
    }
    DCHECK(op == ax::mojom::Command::kDelete || op == ax::mojom::Command::kInsert);
    ...
    if (current_position->GetAnchor()->id() != edit_start_anchor->id() && !current_position->GetAnchor()->IsDescendantOf(edit_start_anchor)
        && !edit_start_anchor->IsDescendantOf(current_position->GetAnchor())) {
        start_is_relevant = false;
    }
    // Then we check if the end is relevant.
    if (current_position->GetAnchor()->id() != edit_end_anchor->id() && !current_position->GetAnchor()->IsDescendantOf(edit_end_anchor)
    ...
    if (op == ax::mojom::Command::kInsert) {
        ...
        // to account for.
        AXPositionInstance deletion_start_position = AXNodePosition::CreateTextPosition(*edit_start_anchor, edit_start, current_position->affinity())
                                                         ->CreateAncestorPosition(&text_field_node, ax::mojom::MoveDirection::kForward);
        AXPositionInstance deletion_end_position = AXNodePosition::CreateTextPosition(*edit_end_anchor, edit_end, current_position->affinity())
                                                       ->CreateAncestorPosition(&text_field_node, ax::mojom::MoveDirection::kForward);
        AXPositionInstance current_text_field_position = current_position->CreateAncestorPosition(&text_field_node, ax::mojom::MoveDirection::kForward);
        deletion_encompasses_endpoint = deletion_start_position->text_offset() <= current_text_field_position->text_offset()
            && deletion_end_position->text_offset() >= current_text_field_position->text_offset();
    }
    ...
    if (edit_start < current_position->text_offset()) {
        ...
        if (is_start) {
            SetStart(AXNodePosition::CreateTextPosition(
                *current_position->GetAnchor(), current_position->text_offset() + difference_or_increase, current_position->affinity()));
            return;
        }
...
// Ensures that our endpoints are always valid (step 2, all scenarios). See
// comment in header file for more details.
void AXPlatformNodeTextRangeProviderWin::TextRangeEndpoints::ValidateEndpointsAfterNodeDeletionIfNeeded()
{
    if (validation_necessary_for_start_.has_value() && validation_necessary_for_start_->validation_needed) {
    ...
    if (validation_necessary_for_end_.has_value() && validation_necessary_for_end_->validation_needed) {
