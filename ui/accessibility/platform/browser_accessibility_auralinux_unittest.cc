...
public:
    ...
    BrowserAccessibilityAuraLinuxTest(const BrowserAccessibilityAuraLinuxTest&) = delete;
    BrowserAccessibilityAuraLinuxTest& operator=(const BrowserAccessibilityAuraLinuxTest&) = delete;
...
BrowserAccessibilityAuraLinuxTest::~BrowserAccessibilityAuraLinuxTest() = default;
...
TEST_F(BrowserAccessibilityAuraLinuxTest, TestSimpleAtkText)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root_data), node_id_delegate_, test_browser_accessibility_delegate_.get()));

    AXPlatformNodeAuraLinux* root_obj = ToBrowserAccessibilityAuraLinux(manager->GetBrowserAccessibilityRoot())->GetNode();
    ...
    auto verify_atk_text_contents = [&](const char* expected_text, int start_offset, int end_offset) {
...
TEST_F(BrowserAccessibilityAuraLinuxTest, TestComplexHypertext)
{
    ...
    const std::u16string string16_embed(1, AXPlatformNodeAuraLinux::kEmbeddedCharacter);
    const std::string embed = base::UTF16ToUTF8(string16_embed);
    const std::string root_hypertext = text1_name + embed + text2_name + embed + embed + embed;
    ...
    root.id = 1;
    root.role = ax::mojom::Role::kRootWebArea;
    root.child_ids = { text1.id, combo_box.id, text2.id, check_box.id, radio_button.id, link.id };

    std::unique_ptr<BrowserAccessibilityManager> manager(BrowserAccessibilityManager::Create(
        MakeAXTreeUpdateForTesting(root, text1, combo_box, text2, check_box, radio_button, radio_button_text, link, link_text), node_id_delegate_,
    ...
    EXPECT_EQ(g_utf8_strlen(root_hypertext.c_str(), -1), atk_text_get_character_count(atk_text));
    ...
    auto verify_atk_link_text = [&](const char* expected_text, int link_index, int expected_start_index) {
        ...
        AtkHyperlink* link_from_object = atk_hyperlink_impl_get_hyperlink(ATK_HYPERLINK_IMPL(link_object));
...
TEST_F(BrowserAccessibilityAuraLinuxTest, TestTextAttributesInButtons)
{
    ...
    empty_button.AddStringAttribute(ax::mojom::StringAttribute::kFontFamily, "Times");
    root.child_ids.push_back(empty_button.id);

    AXTreeUpdate update = MakeAXTreeUpdateForTesting(root, button, text, empty_button);
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(update, node_id_delegate_, test_browser_accessibility_delegate_.get()));

    BrowserAccessibilityAuraLinux* ax_root = ToBrowserAccessibilityAuraLinux(manager->GetBrowserAccessibilityRoot());

    BrowserAccessibilityAuraLinux* ax_button = ToBrowserAccessibilityAuraLinux(ax_root->PlatformGetChild(0));
    AtkObject* atk_button = ax_button->GetNode()->GetNativeViewAccessible();

    int start_offset, end_offset;
    AtkAttributeSet* attributes = atk_text_get_run_attributes(ATK_TEXT(atk_button), 0, &start_offset, &end_offset);
    ASSERT_EQ(1U, g_slist_length(attributes));
    atk_attribute_set_free(attributes);

    BrowserAccessibilityAuraLinux* ax_empty_button = ToBrowserAccessibilityAuraLinux(ax_root->PlatformGetChild(1));
    AtkObject* atk_empty_button = ax_empty_button->GetNode()->GetNativeViewAccessible();
    attributes = atk_text_get_run_attributes(ATK_TEXT(atk_empty_button), 0, &start_offset, &end_offset);
    ASSERT_EQ(1U, g_slist_length(attributes));
    atk_attribute_set_free(attributes);
}

TEST_F(BrowserAccessibilityAuraLinuxTest, TestTextAttributesInContentEditables)
{
    auto has_attribute = [](AtkAttributeSet* attributes, AtkTextAttribute text_attribute, std::optional<const char*> expected_value) {
        const char* name = atk_text_attribute_get_name(text_attribute);
        while (attributes) {
            const AtkAttribute* attribute = static_cast<AtkAttribute*>(attributes->data);
    ...
    BrowserAccessibilityAuraLinux* ax_div = ToBrowserAccessibilityAuraLinux(ax_root->PlatformGetChild(0));
    ASSERT_NE(nullptr, ax_div);
    ASSERT_EQ(3U, ax_div->PlatformChildCount());

    BrowserAccessibilityAuraLinux* ax_before = ToBrowserAccessibilityAuraLinux(ax_div->PlatformGetChild(0));
    ASSERT_NE(nullptr, ax_before);
    BrowserAccessibilityAuraLinux* ax_link = ToBrowserAccessibilityAuraLinux(ax_div->PlatformGetChild(1));
    ASSERT_NE(nullptr, ax_link);
    ASSERT_EQ(1U, ax_link->PlatformChildCount());
    BrowserAccessibilityAuraLinux* ax_after = ToBrowserAccessibilityAuraLinux(ax_div->PlatformGetChild(2));
    ASSERT_NE(nullptr, ax_after);

    BrowserAccessibilityAuraLinux* ax_link_text = ToBrowserAccessibilityAuraLinux(ax_link->PlatformGetChild(0));
    ...
    int start_offset, end_offset;
    AtkAttributeSet* attributes = atk_text_get_run_attributes(ATK_TEXT(root_atk_object), 0, &start_offset, &end_offset);

    ASSERT_EQ(0, start_offset);
    ASSERT_EQ(1, end_offset);

    ASSERT_TRUE(has_attribute(attributes, ATK_TEXT_ATTR_FAMILY_NAME, "Helvetica"));
    atk_attribute_set_free(attributes);

    // Test the style of text_before.
    for (int offset = 0; offset < 7; ++offset) {
        start_offset = end_offset = -1;
        attributes = atk_text_get_run_attributes(ATK_TEXT(ax_div_atk_object), offset, &start_offset, &end_offset);
    ...
    AtkObject* ax_link_text_atk_object = ax_link_text->GetNode()->GetNativeViewAccessible();
    for (int offset = 0; offset < 3; offset++) {
        start_offset = end_offset = -1;
        attributes = atk_text_get_run_attributes(ATK_TEXT(ax_link_text_atk_object), offset, &start_offset, &end_offset);
    ...
    for (int offset = 8; offset < 15; ++offset) {
        ...
        ASSERT_FALSE(has_attribute(attributes, ATK_TEXT_ATTR_WEIGHT, std::nullopt));
        ASSERT_FALSE(has_attribute(attributes, ATK_TEXT_ATTR_STYLE, std::nullopt));
        ASSERT_FALSE(has_attribute(attributes, ATK_TEXT_ATTR_UNDERLINE, std::nullopt));
    ...
    AtkObject* ax_before_atk_object = ax_before->GetNode()->GetNativeViewAccessible();
    start_offset = end_offset = -1;
    attributes = atk_text_get_run_attributes(ATK_TEXT(ax_before_atk_object), 6, &start_offset, &end_offset);
    EXPECT_EQ(0, start_offset);
    EXPECT_EQ(7, end_offset);
    ASSERT_TRUE(has_attribute(attributes, ATK_TEXT_ATTR_FAMILY_NAME, "Helvetica"));
    ASSERT_TRUE(has_attribute(attributes, ATK_TEXT_ATTR_WEIGHT, "700"));
    ...
    AtkObject* ax_after_atk_object = ax_after->GetNode()->GetNativeViewAccessible();
    attributes = atk_text_get_run_attributes(ATK_TEXT(ax_after_atk_object), 6, &start_offset, &end_offset);
    EXPECT_EQ(0, start_offset);
    EXPECT_EQ(7, end_offset);
    ASSERT_TRUE(has_attribute(attributes, ATK_TEXT_ATTR_FAMILY_NAME, "Helvetica"));
    ASSERT_FALSE(has_attribute(attributes, ATK_TEXT_ATTR_WEIGHT, std::nullopt));
...
TEST_F(BrowserAccessibilityAuraLinuxTest, TestAtkObjectsNotDeletedDuringTreeUpdate)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root, container, combo_box, menu_list, menu_option_1, menu_option_2), node_id_delegate_,
            test_browser_accessibility_delegate_.get()));

    AXPlatformNodeAuraLinux* combo_box_node = ToBrowserAccessibilityAuraLinux(manager->GetFromID(combo_box.id))->GetNode();
...
TEST_F(BrowserAccessibilityAuraLinuxTest, TestExistingMisspellingsInSimpleTextFields)
{
    ...
    BrowserAccessibilityAuraLinux* ax_combo_box = ToBrowserAccessibilityAuraLinux(ax_root->PlatformGetChild(0));
    ASSERT_NE(nullptr, ax_combo_box);

    AtkText* combo_box_text = ATK_TEXT(ax_combo_box->GetNode()->GetNativeViewAccessible());
    int start_offset, end_offset;

    auto contains_spelling_attribute = [](AtkAttributeSet* attributes) {
        const char* invalid_str = atk_text_attribute_get_name(ATK_TEXT_ATTR_INVALID);
        while (attributes) {
            AtkAttribute* attribute = static_cast<AtkAttribute*>(attributes->data);
            if (!g_strcmp0(attribute->name, invalid_str) && !g_strcmp0(attribute->value, "spelling"))
                return true;
            attributes = g_slist_next(attributes);
        }
    ...
    for (int offset = 0; offset < value1_length; ++offset) {
        AtkAttributeSet* attributes = atk_text_get_run_attributes(combo_box_text, offset, &start_offset, &end_offset);
    ...
    for (int offset = value1_length + 4; offset < combo_box_value_length; ++offset) {
        AtkAttributeSet* attributes = atk_text_get_run_attributes(combo_box_text, offset, &start_offset, &end_offset);
        EXPECT_FALSE(contains_spelling_attribute(attributes));
...
TEST_F(BrowserAccessibilityAuraLinuxTest, TextAtkStaticTextChange)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(BrowserAccessibilityManager::Create(
        MakeAXTreeUpdateForTesting(root, div_editable, text), node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    AXPlatformNodeAuraLinux* div_node = ToBrowserAccessibilityAuraLinux(manager->GetFromID(div_editable.id))->GetNode();
...
TEST_F(BrowserAccessibilityAuraLinuxTest, TestAtkTextGetOffesetAtPoint)
{
    ...
    inline_box1.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, std::vector<int32_t> { 5 });

    std::unique_ptr<BrowserAccessibilityManager> manager(BrowserAccessibilityManager::Create(
        MakeAXTreeUpdateForTesting(static_text1, inline_box1), node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    for (int offset = 0; offset < root_text_length; offset++) {
        atk_text_get_character_extents(atk_text, offset, &x, &y, &width, &height, ATK_XY_SCREEN);
