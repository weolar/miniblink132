...
const GUID GUID_ISimpleDOM = { 0x0c539790, 0x12e4, 0x11cf, { 0xb6, 0x61, 0x00, 0xaa, 0x00, 0x4c, 0xd6, 0xd8 } };
...
//
////////////////////////////////////////////////////////////////////////////////
class __declspec(uuid("562072fe-3390-43b1-9e2c-dd4118f5ac79")) BrowserAccessibilityComWin : public AXPlatformNodeWin,
...
public:
    ...
    BrowserAccessibilityComWin& operator=(const BrowserAccessibilityComWin&) = delete;
    ...
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP scrollTo(enum IA2ScrollType scroll_type) override;
    ...
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_toolkitVersion(BSTR* toolkit_version) override;
    ...
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_imagePosition(enum IA2CoordinateType coordinate_type, LONG* x, LONG* y) override;

    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_imageSize(LONG* height, LONG* width) override;
    ...
    IFACEMETHODIMP get_characterExtents(LONG offset, enum IA2CoordinateType coord_type, LONG* out_x, LONG* out_y, LONG* out_width, LONG* out_height) override;

    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_nSelections(LONG* n_selections) override;

    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_selection(LONG selection_index, LONG* start_offset, LONG* end_offset) override;

    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_text(LONG start_offset, LONG end_offset, BSTR* text) override;
    ...
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP scrollSubstringTo(LONG start_index, LONG end_index, enum IA2ScrollType scroll_type) override;
    ...
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP setSelection(LONG selection_index, LONG start_offset, LONG end_offset) override;

    // IAccessibleText methods not implemented.
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_attributes(LONG offset, LONG* start_offset, LONG* end_offset, BSTR* text_attributes) override;
    ...
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_hyperlinkIndex(LONG char_index, LONG* hyperlink_index) override;

    // IAccessibleHyperlink methods.
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_anchor(LONG index, VARIANT* anchor) override;
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_anchorTarget(LONG index, VARIANT* anchor_target) override;
    ...
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_description(LONG action_index, BSTR* description) override;
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_keyBinding(LONG action_index, LONG n_max_bindings, BSTR** key_bindings, LONG* n_bindings) override;
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_name(LONG action_index, BSTR* name) override;
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_localizedName(LONG action_index, BSTR* localized_name) override;
    ...
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_nameSpaceURIForID(SHORT name_space_id, BSTR* name_space_uri) override;
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP put_alternateViewMediaTypes(BSTR* comma_separated_media_types) override;
    ...
    COMPONENT_EXPORT(AX_PLATFORM)
    IFACEMETHODIMP
        get_nodeInfo(BSTR* node_name, SHORT* name_space_id, BSTR* node_value, unsigned int* num_children, unsigned int* unique_id, USHORT* node_type) override;

    COMPONENT_EXPORT(AX_PLATFORM)
    IFACEMETHODIMP get_attributes(USHORT max_attribs, BSTR* attrib_names, SHORT* name_space_id, BSTR* attrib_values, USHORT* num_attribs) override;

    COMPONENT_EXPORT(AX_PLATFORM)
    IFACEMETHODIMP get_attributesForNames(USHORT num_attribs, BSTR* attrib_names, SHORT* name_space_id, BSTR* attrib_values) override;

    COMPONENT_EXPORT(AX_PLATFORM)
    IFACEMETHODIMP get_computedStyle(
        USHORT max_style_properties, boolean use_alternate_view, BSTR* style_properties, BSTR* style_values, USHORT* num_style_properties) override;

    COMPONENT_EXPORT(AX_PLATFORM)
    IFACEMETHODIMP get_computedStyleForProperties(USHORT num_style_properties, boolean use_alternate_view, BSTR* style_properties, BSTR* style_values) override;
    ...
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_previousSibling(ISimpleDOMNode** node) override;

    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_nextSibling(ISimpleDOMNode** node) override;

    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_childAt(unsigned int child_index, ISimpleDOMNode** node) override;

    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_innerHTML(BSTR* innerHTML) override;

    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP get_localInterface(void** local_interface) override;
    ...
    COMPONENT_EXPORT(AX_PLATFORM)
    IFACEMETHODIMP
        get_clippedSubstringBounds(unsigned int start_index, unsigned int end_index, int* out_x, int* out_y, int* out_width, int* out_height) override;

    COMPONENT_EXPORT(AX_PLATFORM)
    IFACEMETHODIMP
        get_unclippedSubstringBounds(unsigned int start_index, unsigned int end_index, int* out_x, int* out_y, int* out_width, int* out_height) override;

    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP scrollToSubstring(unsigned int start_index, unsigned int end_index) override;
    ...
    COMPONENT_EXPORT(AX_PLATFORM) IFACEMETHODIMP QueryService(REFGUID guidService, REFIID riid, void** object) override;
    ...
    static COMPONENT_EXPORT(AX_PLATFORM) STDMETHODIMP InternalQueryInterface(void* this_ptr, const _ATL_INTMAP_ENTRY* entries, REFIID iid, void** object);
...
private:
    ...
    {
        return win_attributes_->name;
    }
    std::wstring description() const
    {
        return win_attributes_->description;
    }
    std::wstring value() const
    {
        return win_attributes_->value;
    }
    ...
    HRESULT GetStringAttributeAsBstr(ax::mojom::StringAttribute attribute, BSTR* value_bstr);
    ...
    struct UpdateState {
        UpdateState(std::unique_ptr<WinAttributes> old_win_attributes, AXLegacyHypertext old_hypertext);
...
COMPONENT_EXPORT(AX_PLATFORM) BrowserAccessibilityComWin* ToBrowserAccessibilityComWin(BrowserAccessibility* obj);
