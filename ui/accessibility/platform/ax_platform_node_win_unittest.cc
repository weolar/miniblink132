...
const std::u16string AXPlatformNodeWinTest::kEmbeddedCharacterAsString = { AXPlatformNodeBase::kEmbeddedCharacter };
...
#define EXPECT_UIA_EMPTY(node, property_id)                                                                                                                    \
    {                                                                                                                                                          \
        ...
        ASSERT_HRESULT_SUCCEEDED(node->GetPropertyValue(property_id, actual.Receive()));                                                                       \
...
#define EXPECT_UIA_DOUBLE_ARRAY_EQ(node, array_property_id, expected_property_values)                                                                          \
    {                                                                                                                                                          \
        ScopedVariant array;                                                                                                                                   \
        ASSERT_HRESULT_SUCCEEDED(node->GetPropertyValue(array_property_id, array.Receive()));                                                                  \
        ASSERT_EQ(VT_ARRAY | VT_R8, array.type());                                                                                                             \
        ASSERT_EQ(1u, SafeArrayGetDim(array.ptr()->parray));                                                                                                   \
        LONG array_lower_bound;                                                                                                                                \
        ASSERT_HRESULT_SUCCEEDED(SafeArrayGetLBound(array.ptr()->parray, 1, &array_lower_bound));                                                              \
        LONG array_upper_bound;                                                                                                                                \
        ASSERT_HRESULT_SUCCEEDED(SafeArrayGetUBound(array.ptr()->parray, 1, &array_upper_bound));                                                              \
        double* array_data;                                                                                                                                    \
        ASSERT_HRESULT_SUCCEEDED(::SafeArrayAccessData(array.ptr()->parray, reinterpret_cast<void**>(&array_data)));                                           \
...
#define EXPECT_UIA_ELEMENT_ARRAY_BSTR_EQ(array, element_test_property_id, expected_property_values)                                                            \
    {                                                                                                                                                          \
        ...
        LONG array_lower_bound;                                                                                                                                \
        ASSERT_HRESULT_SUCCEEDED(SafeArrayGetLBound(array, 1, &array_lower_bound));                                                                            \
        LONG array_upper_bound;                                                                                                                                \
        ASSERT_HRESULT_SUCCEEDED(SafeArrayGetUBound(array, 1, &array_upper_bound));                                                                            \
        IUnknown** array_data;                                                                                                                                 \
        ASSERT_HRESULT_SUCCEEDED(::SafeArrayAccessData(array, reinterpret_cast<void**>(&array_data)));                                                         \
        size_t count = array_upper_bound - array_lower_bound + 1;                                                                                              \
        ASSERT_EQ(expected_property_values.size(), count);                                                                                                     \
        for (size_t i = 0; i < count; ++i) {                                                                                                                   \
            ComPtr<IRawElementProviderSimple> element;                                                                                                         \
            ASSERT_HRESULT_SUCCEEDED(array_data[i]->QueryInterface(IID_PPV_ARGS(&element)));                                                                   \
            EXPECT_UIA_BSTR_EQ(element, element_test_property_id, expected_property_values[i].c_str());                                                        \
        }                                                                                                                                                      \
...
#define EXPECT_UIA_PROPERTY_ELEMENT_ARRAY_BSTR_EQ(node, array_property_id, element_test_property_id, expected_property_values)                                 \
    {                                                                                                                                                          \
        ...
        EXPECT_UIA_ELEMENT_ARRAY_BSTR_EQ(array.ptr()->parray, element_test_property_id, expected_property_values);                                             \
    }
...
#define EXPECT_UIA_PROPERTY_UNORDERED_ELEMENT_ARRAY_BSTR_EQ(node, array_property_id, element_test_property_id, expected_property_values)                       \
    {                                                                                                                                                          \
        ...
        for (size_t i = 0; i < count; ++i) {                                                                                                                   \
            ...
            ASSERT_HRESULT_SUCCEEDED(array_data[i]->QueryInterface(IID_PPV_ARGS(&element)));                                                                   \
            ScopedVariant actual;                                                                                                                              \
            ASSERT_HRESULT_SUCCEEDED(element->GetPropertyValue(element_test_property_id, actual.Receive()));                                                   \
            ASSERT_EQ(VT_BSTR, actual.type());                                                                                                                 \
            ASSERT_NE(nullptr, actual.ptr()->bstrVal);                                                                                                         \
            property_values.push_back(std::wstring(V_BSTR(actual.ptr()), SysStringLen(V_BSTR(actual.ptr()))));                                                 \
        }                                                                                                                                                      \
        ASSERT_HRESULT_SUCCEEDED(::SafeArrayUnaccessData(array.ptr()->parray));                                                                                \
        EXPECT_THAT(property_values, ::testing::UnorderedElementsAreArray(expected_property_values));                                                          \
    }
...
HRESULT
MockIRawElementProviderSimple::CreateMockIRawElementProviderSimple(IRawElementProviderSimple** provider)
{
    CComObject<MockIRawElementProviderSimple>* raw_element_provider = nullptr;
    HRESULT hr = CComObject<MockIRawElementProviderSimple>::CreateInstance(&raw_element_provider);
    if (SUCCEEDED(hr)) {
...
//
IFACEMETHODIMP MockIRawElementProviderSimple::GetPatternProvider(PATTERNID pattern_id, IUnknown** result)
{
    return E_NOTIMPL;
}

IFACEMETHODIMP MockIRawElementProviderSimple::GetPropertyValue(PROPERTYID property_id, VARIANT* result)
{
    return E_NOTIMPL;
}
...
IFACEMETHODIMP MockIRawElementProviderSimple::get_HostRawElementProvider(IRawElementProviderSimple** provider)
{
    return E_NOTIMPL;
}

AXPlatformNodeWinTest::AXPlatformNodeWinTest()
    : ax_embedded_object_behavior_(AXEmbeddedObjectBehavior::kExposeCharacterForHypertext)
{
...
template <typename T> ComPtr<T> AXPlatformNodeWinTest::QueryInterfaceFromNodeId(AXNodeID id)
{
    return QueryInterfaceFromNode<T>(GetNode(id));
}

template <typename T> ComPtr<T> AXPlatformNodeWinTest::QueryInterfaceFromNode(AXNode* node)
{
    AXPlatformNode* ax_platform_node = AXPlatformNodeFromNode(node);
    if (!ax_platform_node)
        return ComPtr<T>();
    ComPtr<T> result;
    EXPECT_HRESULT_SUCCEEDED(ax_platform_node->GetNativeViewAccessible()->QueryInterface(__uuidof(T), &result));
    return result;
}

ComPtr<IRawElementProviderSimple> AXPlatformNodeWinTest::GetRootIRawElementProviderSimple()
{
    return QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot());
}

ComPtr<IRawElementProviderSimple> AXPlatformNodeWinTest::GetIRawElementProviderSimpleFromChildIndex(int child_index)
{
    if (!GetRoot() || child_index < 0 || static_cast<size_t>(child_index) >= GetRoot()->children().size()) {
        return ComPtr<IRawElementProviderSimple>();
    }

    return QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[static_cast<size_t>(child_index)]);
}

Microsoft::WRL::ComPtr<IRawElementProviderSimple> AXPlatformNodeWinTest::GetIRawElementProviderSimpleFromId(const AXNodeID node_id)
{
    CHECK(GetTree()) << "No tree available";
    return QueryInterfaceFromNode<IRawElementProviderSimple>(GetTree()->GetFromId(node_id));
}

ComPtr<IRawElementProviderFragment> AXPlatformNodeWinTest::GetRootIRawElementProviderFragment()
{
    return QueryInterfaceFromNode<IRawElementProviderFragment>(GetRoot());
}

Microsoft::WRL::ComPtr<IRawElementProviderFragment> AXPlatformNodeWinTest::IRawElementProviderFragmentFromNode(AXNode* node)
{
    AXPlatformNode* platform_node = AXPlatformNodeFromNode(node);
    gfx::NativeViewAccessible native_view = platform_node->GetNativeViewAccessible();
...
ComPtr<IAccessible2> AXPlatformNodeWinTest::ToIAccessible2(ComPtr<IUnknown> unknown)
{
    CHECK(unknown);
    ComPtr<IServiceProvider> service_provider;
    unknown.As(&service_provider);
    ComPtr<IAccessible2> result;
    CHECK(SUCCEEDED(service_provider->QueryService(IID_IAccessible2, IID_PPV_ARGS(&result))));
    return result;
}

ComPtr<IAccessible2> AXPlatformNodeWinTest::ToIAccessible2(ComPtr<IAccessible> accessible)
{
    CHECK(accessible);
    ComPtr<IServiceProvider> service_provider;
...
ComPtr<IAccessible2_2> AXPlatformNodeWinTest::ToIAccessible2_2(ComPtr<IAccessible> accessible)
{
    CHECK(accessible);
    ComPtr<IServiceProvider> service_provider;
    accessible.As(&service_provider);
    ComPtr<IAccessible2_2> result;
    CHECK(SUCCEEDED(service_provider->QueryService(IID_IAccessible2_2, IID_PPV_ARGS(&result))));
    return result;
}

void AXPlatformNodeWinTest::CheckVariantHasName(const ScopedVariant& variant, const wchar_t* expected_name)
{
    ASSERT_NE(nullptr, variant.ptr());
    ComPtr<IAccessible> accessible;
    ASSERT_HRESULT_SUCCEEDED(V_DISPATCH(variant.ptr())->QueryInterface(IID_PPV_ARGS(&accessible)));
    ScopedBstr name;
    EXPECT_EQ(S_OK, accessible->get_accName(SELF, name.Receive()));
    EXPECT_STREQ(expected_name, name.Get());
}

void AXPlatformNodeWinTest::CheckIUnknownHasName(ComPtr<IUnknown> unknown, const wchar_t* expected_name)
{
...
void AXPlatformNodeWinTest::InitFragmentRoot()
{
    ...
    ax_fragment_root_.reset(InitNodeAsFragmentRoot(GetRoot(), test_fragment_root_delegate_.get()));
}

AXFragmentRootWin* AXPlatformNodeWinTest::InitNodeAsFragmentRoot(AXNode* node, TestFragmentRootDelegate* delegate)
{
    delegate->child_ = AXPlatformNodeFromNode(node)->GetNativeViewAccessible();
    if (node->parent())
        delegate->parent_ = AXPlatformNodeFromNode(node->parent())->GetNativeViewAccessible();

    return new AXFragmentRootWin(gfx::kMockAcceleratedWidget, delegate);
}

ComPtr<IRawElementProviderFragmentRoot> AXPlatformNodeWinTest::GetFragmentRoot()
{
    ComPtr<IRawElementProviderFragmentRoot> fragment_root_provider;
    ax_fragment_root_->GetNativeViewAccessible()->QueryInterface(IID_PPV_ARGS(&fragment_root_provider));
    return fragment_root_provider;
}

AXPlatformNodeWinTest::PatternSet AXPlatformNodeWinTest::GetSupportedPatternsFromNodeId(AXNodeID id)
{
    ComPtr<IRawElementProviderSimple> raw_element_provider_simple = QueryInterfaceFromNodeId<IRawElementProviderSimple>(id);
    ...
    for (LONG property_id : all_supported_patterns_) {
        ...
        if (SUCCEEDED(raw_element_provider_simple->GetPatternProvider(property_id, &provider)) && provider) {
...
{
    ASSERT_TRUE(role == ax::mojom::Role::kTable || role == ax::mojom::Role::kGrid);
    AXNodeData root;
    root.id = 1;
    root.role = role;
    ...
    ComPtr<ITableProvider> root_itableprovider(QueryInterfaceFromNode<ITableProvider>(GetRoot()));

    base::win::ScopedSafearray safearray;
    EXPECT_HRESULT_SUCCEEDED(root_itableprovider->GetColumnHeaders(safearray.Receive()));
    EXPECT_NE(nullptr, safearray.Get());

    std::vector<std::wstring> expected_names = { L"column_header" };
    EXPECT_UIA_ELEMENT_ARRAY_BSTR_EQ(safearray.Get(), UIA_NamePropertyId, expected_names);

    // Remove column_header's native event target and verify it's no longer
    // returned.
    TestAXNodeWrapper* column_header_wrapper = TestAXNodeWrapper::GetOrCreate(GetTree(), GetRoot()->children()[0]->children()[0]);
...
gfx::NativeViewAccessible TestFragmentRootDelegate::GetParentOfAXFragmentRoot()
{
    return parent_;
}
...
TEST_F(AXPlatformNodeWinTest, IAccessibleHitTest)
{
    ...
    ScopedVariant obj_2;
    std::unique_ptr<base::AutoReset<float>> scale_factor_reset = TestAXNodeWrapper::SetScaleFactor(1.5);
...
TEST_F(AXPlatformNodeWinTest, IAccessibleDescription)
{
    ...
    root.id = 1;
    root.role = ax::mojom::Role::kRootWebArea;
    root.AddStringAttribute(ax::mojom::StringAttribute::kDescription, "Description");
    Init(root);
...
TEST_F(AXPlatformNodeWinTest, IAccessibleShortcut)
{
    ...
    root.id = 1;
    root.role = ax::mojom::Role::kRootWebArea;
    root.AddStringAttribute(ax::mojom::StringAttribute::kKeyShortcuts, "Shortcut");
    Init(root);
    ...
    ScopedVariant bad_id(999);
    ScopedBstr k2;
    EXPECT_EQ(E_INVALIDARG, root_obj->get_accKeyboardShortcut(bad_id, k2.Receive()));
}
...
TEST_F(AXPlatformNodeWinTest, IAccessibleSelectionListBoxOptionMultipleSelected)
{
    ...
    ASSERT_HRESULT_SUCCEEDED(V_UNKNOWN(selection.ptr())->QueryInterface(IID_PPV_ARGS(&accessibles)));
    ...
    // Check out the first selected item.
    {
        ...
        ComPtr<IAccessible> accessible;
        ASSERT_HRESULT_SUCCEEDED(V_DISPATCH(item.ptr())->QueryInterface(IID_PPV_ARGS(&accessible)));
        ScopedBstr name;
        EXPECT_EQ(S_OK, accessible->get_accName(SELF, name.Receive()));
...
TEST_F(AXPlatformNodeWinTest, IAccessibleSelectionTableRowOneSelected)
{
    ...
    ASSERT_HRESULT_SUCCEEDED(V_DISPATCH(selection.ptr())->QueryInterface(IID_PPV_ARGS(&row)));
...
TEST_F(AXPlatformNodeWinTest, IAccessibleSelectionTableCellOneSelected)
{
    ...
    ASSERT_HRESULT_SUCCEEDED(V_DISPATCH(selection.ptr())->QueryInterface(IID_PPV_ARGS(&cell)));
...
TEST_F(AXPlatformNodeWinTest, IAccessibleRole)
{
    ...
    ScopedVariant bad_id(999);
    EXPECT_EQ(E_INVALIDARG, child_iaccessible->get_accRole(bad_id, role.Receive()));
}
...
TEST_F(AXPlatformNodeWinTest, IAccessibleLocation)
{
    ...
    EXPECT_EQ(S_OK, GetRootIAccessible()->accLocation(&x_left, &y_top, &width, &height, SELF));
    EXPECT_EQ(110, x_left);
    EXPECT_EQ(240, y_top);
    EXPECT_EQ(800, width);
    EXPECT_EQ(600, height);

    EXPECT_EQ(E_INVALIDARG, GetRootIAccessible()->accLocation(nullptr, &y_top, &width, &height, SELF));
    EXPECT_EQ(E_INVALIDARG, GetRootIAccessible()->accLocation(&x_left, nullptr, &width, &height, SELF));
    EXPECT_EQ(E_INVALIDARG, GetRootIAccessible()->accLocation(&x_left, &y_top, nullptr, &height, SELF));
    EXPECT_EQ(E_INVALIDARG, GetRootIAccessible()->accLocation(&x_left, &y_top, &width, nullptr, SELF));
    ScopedVariant bad_id(999);
    EXPECT_EQ(E_INVALIDARG, GetRootIAccessible()->accLocation(&x_left, &y_top, &width, &height, bad_id));
...
TEST_F(AXPlatformNodeWinTest, IAccessibleChildAndParent)
{
    ...
    ASSERT_LT(root_unique_id, 0);
    {
        ...
        EXPECT_EQ(E_INVALIDARG, button_iaccessible->get_accChild(root_id_variant, &result));
    }
...
TEST_F(AXPlatformNodeWinTest, IAccessible2IndexInParent)
{
    ...
    ComPtr<IAccessible> root_iaccessible(GetRootIAccessible());
    ComPtr<IAccessible2> root_iaccessible2 = ToIAccessible2(root_iaccessible);
    ComPtr<IAccessible> left_iaccessible(IAccessibleFromNode(GetRoot()->children()[0]));
    ComPtr<IAccessible2> left_iaccessible2 = ToIAccessible2(left_iaccessible);
    ComPtr<IAccessible> right_iaccessible(IAccessibleFromNode(GetRoot()->children()[1]));
...
TEST_F(AXPlatformNodeWinTest, AccNavigate)
{
    ...
    // Invalid arguments.
    EXPECT_EQ(E_INVALIDARG, ia_root->accNavigate(NAVDIR_NEXT, ScopedVariant::kEmptyVariant, nullptr));
    EXPECT_EQ(E_INVALIDARG, ia_child1->accNavigate(NAVDIR_NEXT, ScopedVariant::kEmptyVariant, end.AsInput()));
    EXPECT_EQ(VT_EMPTY, end.type());

    // Navigating to first/last child should only be from self.
    EXPECT_EQ(E_INVALIDARG, ia_root->accNavigate(NAVDIR_FIRSTCHILD, var_root, end.AsInput()));
    EXPECT_EQ(VT_EMPTY, end.type());
    EXPECT_EQ(E_INVALIDARG, ia_root->accNavigate(NAVDIR_LASTCHILD, var_root, end.AsInput()));
    EXPECT_EQ(VT_EMPTY, end.type());
    ...
    EXPECT_EQ(E_NOTIMPL, ia_child1->accNavigate(NAVDIR_RIGHT, SELF, end.AsInput()));
    EXPECT_EQ(E_NOTIMPL, ia_child2->accNavigate(NAVDIR_LEFT, SELF, end.AsInput()));
    EXPECT_EQ(VT_EMPTY, end.type());
    ...
    EXPECT_EQ(S_OK, ia_root->accNavigate(NAVDIR_NEXT, ScopedVariant(1), end.AsInput()));
    EXPECT_EQ(VT_DISPATCH, end.type());
    EXPECT_EQ(V_DISPATCH(var_child2.ptr()), V_DISPATCH(end.ptr()));

    EXPECT_EQ(S_OK, ia_root->accNavigate(NAVDIR_PREVIOUS, ScopedVariant(2), end.AsInput()));
    EXPECT_EQ(VT_DISPATCH, end.type());
    EXPECT_EQ(V_DISPATCH(var_child1.ptr()), V_DISPATCH(end.ptr()));

    // Test out-of-bounds.
    EXPECT_EQ(S_FALSE, ia_child1->accNavigate(NAVDIR_PREVIOUS, SELF, end.AsInput()));
    EXPECT_EQ(VT_EMPTY, end.type());
    EXPECT_EQ(S_FALSE, ia_child2->accNavigate(NAVDIR_NEXT, SELF, end.AsInput()));
    EXPECT_EQ(VT_EMPTY, end.type());

    EXPECT_EQ(S_FALSE, ia_root->accNavigate(NAVDIR_PREVIOUS, ScopedVariant(1), end.AsInput()));
    EXPECT_EQ(VT_EMPTY, end.type());
    EXPECT_EQ(S_FALSE, ia_root->accNavigate(NAVDIR_NEXT, ScopedVariant(2), end.AsInput()));
    EXPECT_EQ(VT_EMPTY, end.type());
}
...
TEST_F(AXPlatformNodeWinTest, IAccessible2TextFieldSetSelection)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(text_field->setSelection(0, IA2_TEXT_OFFSET_CARET, IA2_TEXT_OFFSET_LENGTH));
...
TEST_F(AXPlatformNodeWinTest, DISABLED_IAccessible2ContentEditableSetSelection)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(content_editable->setSelection(0, IA2_TEXT_OFFSET_CARET, IA2_TEXT_OFFSET_LENGTH));
...
TEST_F(AXPlatformNodeWinTest, IAccessible2ScrollToPoint)
{
    ...
    LONG x_left, y_top, width, height;
    EXPECT_EQ(S_OK, ax_child1->accLocation(&x_left, &y_top, &width, &height, SELF));
    EXPECT_EQ(10, x_left);
    EXPECT_EQ(10, y_top);
    EXPECT_EQ(10, width);
    EXPECT_EQ(10, height);
    ...
    ComPtr<IAccessible2> root_iaccessible2 = ToIAccessible2(root_iaccessible);
    EXPECT_EQ(S_OK, root_iaccessible2->scrollToPoint(IA2_COORDTYPE_SCREEN_RELATIVE, 600, 650));
    ...
    EXPECT_EQ(S_OK, root_iaccessible2->scrollToPoint(IA2_COORDTYPE_PARENT_RELATIVE, 0, 0));
...
TEST_F(AXPlatformNodeWinTest, IAccessibleTableGetRowColumnExtentsAtIndex)
{
    ...
    EXPECT_EQ(S_OK, result->get_rowColumnExtentsAtIndex(0, &row, &column, &row_extents, &column_extents, &is_selected));
    ...
    EXPECT_EQ(E_INVALIDARG, result->get_rowColumnExtentsAtIndex(-1, &row, &column, &row_extents, &column_extents, &is_selected));
}
...
TEST_F(AXPlatformNodeWinTest, IAccessibleTableCellGetColumnHeaderCells)
{
    ...
    LONG number_cells;
    EXPECT_EQ(S_OK, cell->get_columnHeaderCells(&cell_accessibles, &number_cells));
...
TEST_F(AXPlatformNodeWinTest, IAccessibleTableCellGetRowColumnExtent)
{
    ...
    LONG row, column, row_extents, column_extents;
    BOOLEAN is_selected = false;
    EXPECT_EQ(S_OK, cell->get_rowColumnExtents(&row, &column, &row_extents, &column_extents, &is_selected));
    EXPECT_EQ(1, row);
    EXPECT_EQ(1, column);
    EXPECT_EQ(1, row_extents);
    EXPECT_EQ(1, column_extents);
...
TEST_F(AXPlatformNodeWinTest, IAccessible2GetNRelations)
{
    ...
    root.AddIntListAttribute(ax::mojom::IntListAttribute::kDescribedbyIds, describedby_ids);
    ...
    EXPECT_HRESULT_SUCCEEDED(root_iaccessible2->get_relation(0, &describedby_relation));

    EXPECT_HRESULT_SUCCEEDED(describedby_relation->get_relationType(relation_type.Receive()));
    ...
    EXPECT_HRESULT_SUCCEEDED(ax_child1->get_relation(0, &description_for_relation));
    EXPECT_HRESULT_SUCCEEDED(description_for_relation->get_relationType(relation_type.Receive()));
    ...
    EXPECT_HRESULT_SUCCEEDED(ax_child2->get_relation(0, &description_for_relation));
    EXPECT_HRESULT_SUCCEEDED(description_for_relation->get_relationType(relation_type.Receive()));
    EXPECT_EQ(L"descriptionFor", std::wstring(relation_type.Get()));
    relation_type.Reset();
...
TEST_F(AXPlatformNodeWinTest, IAccessible2TestPopupForRelationMapsToControlledByRelation)
{
    AXNodeData root;
    root.id = 1;
    root.role = ax::mojom::Role::kRootWebArea;
    ...
    EXPECT_HRESULT_SUCCEEDED(controls_relation->get_relationType(relation_type.Receive()));
    ...
    EXPECT_HRESULT_SUCCEEDED(controlled_by_relation->get_relationType(relation_type.Receive()));
...
TEST_F(AXPlatformNodeWinTest, IAccessible2_TestRelationTargetsOfType)
{
    ...
    child2.AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, labelledby_ids);
...

    {
        ...
        IUnknown** targets;
        LONG n_targets;
        EXPECT_EQ(S_OK, root_iaccessible2->get_relationTargetsOfType(type.Get(), 0, &targets, &n_targets));
        ASSERT_EQ(2, n_targets);
        CoTaskMemFree(targets);

        EXPECT_EQ(S_OK, root_iaccessible2->get_relationTargetsOfType(type.Get(), 1, &targets, &n_targets));
...
TEST_F(AXPlatformNodeWinTest, DISABLED_TestRelationTargetsOfType)
{
...

    {
        ...
        IUnknown** targets;
        LONG n_targets;
        EXPECT_EQ(S_OK, ax_child2->get_relationTargetsOfType(type.Get(), 0, &targets, &n_targets));
        ASSERT_EQ(2, n_targets);
...

    {
        ...
        IUnknown** targets;
        LONG n_targets;
        EXPECT_EQ(S_OK, ax_child1->get_relationTargetsOfType(type.Get(), 0, &targets, &n_targets));
        ASSERT_EQ(2, n_targets);
        EXPECT_EQ(root_iaccessible2.Get(), targets[0]);
        EXPECT_EQ(ax_child3.Get(), targets[1]);
        CoTaskMemFree(targets);
...
TEST_F(AXPlatformNodeWinTest, IAccessibleTableGetSelectedColumnsZero)
{
    ...
    LONG* columns;
    LONG n_columns;
    EXPECT_EQ(S_OK, result->get_selectedColumns(max_columns, &columns, &n_columns));
...
TEST_F(AXPlatformNodeWinTest, IAccessible2GetGroupPosition)
{
    ...
    EXPECT_EQ(E_INVALIDARG, iaccessible2->get_groupPosition(nullptr, nullptr, nullptr));
}
...
TEST_F(AXPlatformNodeWinTest, IAccessible2GetLocalizedExtendedRole)
{
    AXNodeData root;
    root.id = 1;
    root.AddStringAttribute(ax::mojom::StringAttribute::kRoleDescription, "extended role");
    Init(root);
...
TEST_F(AXPlatformNodeWinTest, UnlabeledImageRoleDescription)
{
    ...
    tree.nodes[1].id = 2;
    tree.nodes[1].SetImageAnnotationStatus(ax::mojom::ImageAnnotationStatus::kEligibleForAnnotation);
    tree.nodes[1].role = ax::mojom::Role::kImage;

    tree.nodes[2].id = 3;
    tree.nodes[2].SetImageAnnotationStatus(ax::mojom::ImageAnnotationStatus::kSilentlyEligibleForAnnotation);
    ...
    for (int child_index = 0; child_index < static_cast<int>(tree.nodes[0].child_ids.size()); ++child_index) {
        ComPtr<IDispatch> child_dispatch;
        ASSERT_HRESULT_SUCCEEDED(ia2_root_obj->get_accChild(ScopedVariant(child_index + 1), &child_dispatch));
        ...
        ASSERT_EQ(S_OK, ia2_child->get_localizedExtendedRole(role_description.Receive()));
        EXPECT_STREQ(L"Unlabeled image", role_description.Get());

        ComPtr<IRawElementProviderSimple> uia_child = QueryInterfaceFromNode<IRawElementProviderSimple>(uia_root_node->children()[child_index]);
        EXPECT_UIA_BSTR_EQ(uia_child, UIA_LocalizedControlTypePropertyId, L"Unlabeled image");
    }
}
...
TEST_F(AXPlatformNodeWinTest, UnlabeledImageAttributes)
{
    ...
    for (int child_index = 0; child_index < static_cast<int>(tree.nodes[0].child_ids.size()); ++child_index) {
        ComPtr<IDispatch> child_dispatch;
        ASSERT_HRESULT_SUCCEEDED(root_obj->get_accChild(ScopedVariant(child_index + 1), &child_dispatch));
        ComPtr<IAccessible> child;
        ASSERT_HRESULT_SUCCEEDED(child_dispatch.As(&child));
        ComPtr<IAccessible2> ia2_child = ToIAccessible2(child);
        ...
        std::vector<std::wstring> attribute_vector = base::SplitString(attributes, L";", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
        EXPECT_TRUE(base::Contains(attribute_vector, L"roledescription:Unlabeled image"));
    }
}
...
TEST_F(AXPlatformNodeWinTest, AnnotatedImageName)
{
    ...
    tree.nodes[1].id = 2;
    tree.nodes[1].role = ax::mojom::Role::kImage;
    tree.nodes[1].AddStringAttribute(ax::mojom::StringAttribute::kImageAnnotation, "Annotation");
    tree.nodes[1].SetImageAnnotationStatus(ax::mojom::ImageAnnotationStatus::kEligibleForAnnotation);
    expected_names.push_back(L"To get missing image descriptions, open the context menu.");

    // If the status is EligibleForAnnotation, the discoverability string
    // should be appended to the existing name.
    tree.nodes[2].id = 3;
    tree.nodes[2].role = ax::mojom::Role::kImage;
    tree.nodes[2].AddStringAttribute(ax::mojom::StringAttribute::kImageAnnotation, "Annotation");
    tree.nodes[2].SetName("ExistingLabel");
    tree.nodes[2].SetImageAnnotationStatus(ax::mojom::ImageAnnotationStatus::kEligibleForAnnotation);
    expected_names.push_back(L"ExistingLabel. To get missing image descriptions, open the context "
                             L"menu.");
    ...
    tree.nodes[3].AddStringAttribute(ax::mojom::StringAttribute::kImageAnnotation, "Annotation");
    ...
    tree.nodes[4].AddStringAttribute(ax::mojom::StringAttribute::kImageAnnotation, "Annotation");
    tree.nodes[4].SetName("ExistingLabel");
    tree.nodes[4].SetImageAnnotationStatus(ax::mojom::ImageAnnotationStatus::kIneligibleForAnnotation);
    expected_names.push_back(L"ExistingLabel");
    ...
    tree.nodes[5].AddStringAttribute(ax::mojom::StringAttribute::kImageAnnotation, "Annotation");
    tree.nodes[5].SetName("ExistingLabel");
    tree.nodes[5].SetImageAnnotationStatus(ax::mojom::ImageAnnotationStatus::kAnnotationPending);
    ...
    tree.nodes[6].SetImageAnnotationStatus(ax::mojom::ImageAnnotationStatus::kAnnotationSucceeded);
    expected_names.push_back(L"ExistingLabel");
    ...
    tree.nodes[7].AddStringAttribute(ax::mojom::StringAttribute::kImageAnnotation, "Annotation");
    tree.nodes[7].SetName("ExistingLabel");
    tree.nodes[7].SetImageAnnotationStatus(ax::mojom::ImageAnnotationStatus::kAnnotationSucceeded);
    ...
    tree.nodes[8].AddStringAttribute(ax::mojom::StringAttribute::kImageAnnotation, "Annotation");
    tree.nodes[8].SetName("ExistingLabel");
    tree.nodes[8].SetImageAnnotationStatus(ax::mojom::ImageAnnotationStatus::kAnnotationEmpty);
    ...
    tree.nodes[9].AddStringAttribute(ax::mojom::StringAttribute::kImageAnnotation, "Annotation");
    ...
    tree.nodes[10].AddStringAttribute(ax::mojom::StringAttribute::kImageAnnotation, "Annotation");
    tree.nodes[10].SetName("ExistingLabel");
    tree.nodes[10].SetImageAnnotationStatus(ax::mojom::ImageAnnotationStatus::kAnnotationProcessFailed);
    expected_names.push_back(L"ExistingLabel. No description available.");
    ...
    for (int child_index = 0; child_index < child_count; child_index++) {
        ...
        ComPtr<IRawElementProviderSimple> uia_child = QueryInterfaceFromNode<IRawElementProviderSimple>(uia_root_node->children()[child_index]);
        EXPECT_UIA_BSTR_EQ(uia_child, UIA_NamePropertyId, expected_names[child_index]);
    }
}
...
TEST_F(AXPlatformNodeWinTest, IAccessibleTextGetOffsetAtPoint)
{
    ...
    inline_box1.AddIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets, character_offsets1);
    inline_box1.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, std::vector<int32_t> { 0, 5 });
    inline_box1.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, std::vector<int32_t> { 4, 6 });
    ...
    EXPECT_EQ(S_FALSE, root_text->get_offsetAtPoint(0, 0, IA2CoordinateType::IA2_COORDTYPE_PARENT_RELATIVE, &offset_result));
    ...
    EXPECT_EQ(S_FALSE, root_text->get_offsetAtPoint(0, 0, IA2CoordinateType::IA2_COORDTYPE_SCREEN_RELATIVE, &offset_result));
    EXPECT_EQ(-1, offset_result);
    ...
    EXPECT_EQ(S_FALSE, root_text->get_offsetAtPoint(25, 5, IA2CoordinateType::IA2_COORDTYPE_SCREEN_RELATIVE, &offset_result));
    EXPECT_EQ(-1, offset_result);
    ...
    EXPECT_EQ(S_OK, root_text->get_offsetAtPoint(0, 20, IA2CoordinateType::IA2_COORDTYPE_SCREEN_RELATIVE, &offset_result));
    ...
    EXPECT_EQ(S_FALSE, text1->get_offsetAtPoint(0, 0, IA2CoordinateType::IA2_COORDTYPE_SCREEN_RELATIVE, &offset_result));
    EXPECT_EQ(-1, offset_result);

    // Test point(30, 30) outside of any character bounds but on the text.
    EXPECT_EQ(S_OK, text1->get_offsetAtPoint(30, 30, IA2CoordinateType::IA2_COORDTYPE_SCREEN_RELATIVE, &offset_result));
    EXPECT_EQ(-1, offset_result);

    // Test point(0, 20) inside bounds of "l", text offset=0
    // character bounds={(0, 20), (5, 20), (0, 30), (5, 30)}
    EXPECT_HRESULT_SUCCEEDED(text1->get_offsetAtPoint(0, 20, IA2CoordinateType::IA2_COORDTYPE_SCREEN_RELATIVE, &offset_result));
    EXPECT_EQ(0, offset_result);

    // Test point(9, 20) inside bounds of "i", text offset=1
    // character bounds={(5, 20), (10, 20), (5, 30), (10, 30)}
    EXPECT_HRESULT_SUCCEEDED(text1->get_offsetAtPoint(9, 20, IA2CoordinateType::IA2_COORDTYPE_SCREEN_RELATIVE, &offset_result));
    EXPECT_EQ(1, offset_result);

    // Test point(10, 30) inside bounds of "n", text offset=2
    // character bounds={(10, 20), (15, 20), (10, 30), (15, 30)}
    EXPECT_HRESULT_SUCCEEDED(text1->get_offsetAtPoint(10, 29, IA2CoordinateType::IA2_COORDTYPE_SCREEN_RELATIVE, &offset_result));
    EXPECT_EQ(2, offset_result);

    // Test point(19, 29) inside bounds of "e", text offset=3
    // character bounds={(15, 20), (20, 20), (15, 30), (20, 30)
    EXPECT_HRESULT_SUCCEEDED(text1->get_offsetAtPoint(19, 29, IA2CoordinateType::IA2_COORDTYPE_SCREEN_RELATIVE, &offset_result));
    EXPECT_EQ(3, offset_result);

    // Test point(23, 25) inside bounds of " ", text offset=4
    // character bounds={(20, 20), (25, 20), (20, 30), (25, 30)}
    EXPECT_HRESULT_SUCCEEDED(text1->get_offsetAtPoint(23, 25, IA2CoordinateType::IA2_COORDTYPE_SCREEN_RELATIVE, &offset_result));
    EXPECT_EQ(4, offset_result);

    // Test point(25, 20) inside bounds of "1", text offset=5
    // character bounds={(25, 20), (30, 20), (25, 30), (30, 30)}
    EXPECT_HRESULT_SUCCEEDED(text1->get_offsetAtPoint(25, 20, IA2CoordinateType::IA2_COORDTYPE_SCREEN_RELATIVE, &offset_result));
...
TEST_F(AXPlatformNodeWinTest, IAccessibleTextTextFieldRemoveSelection)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(text_field->get_selection(0, &start_offset, &end_offset));
    ...
    EXPECT_EQ(E_INVALIDARG, text_field->get_selection(0, &start_offset, &end_offset));
}
...
TEST_F(AXPlatformNodeWinTest, IAccessibleTextTextFieldGetSelected)
{
    ...
    EXPECT_EQ(E_INVALIDARG, text_field->get_selection(1, &start_offset, &end_offset));
...
// to AXPlatformNodeWin because |hypertext_| is only initialized
// on the BrowserAccessibility side.
TEST_F(AXPlatformNodeWinTest, DISABLED_IAccessibleTextContentEditableAddSelection)
{
    Init(BuildContentEditable());
...
TEST_F(AXPlatformNodeWinTest, IAccessibleTextContentEditableGetNSelectionsZero)
{
    Init(BuildContentEditable());
...
TEST_F(AXPlatformNodeWinTest, IAccessibleTextContextEditableGetCaretOffsetNoCaret)
{
    Init(BuildContentEditable());
...
TEST_F(AXPlatformNodeWinTest, IAccessibleTextContentEditableGetCaretOffsetHasCaret)
{
    Init(BuildContentEditableWithSelectionRange(1, 2));
...
TEST_F(AXPlatformNodeWinTest, IGridProviderGetRowCount)
{
    ...
    // Empty Grid.
    ComPtr<IGridProvider> grid1_provider = QueryInterfaceFromNode<IGridProvider>(GetRoot()->children()[0]);

    // Grid with a cell that defines aria-rowindex (4) and aria-colindex (5).
    ComPtr<IGridProvider> grid2_provider = QueryInterfaceFromNode<IGridProvider>(GetRoot()->children()[1]);

    // Grid that specifies aria-rowcount (2) and aria-colcount (3).
    ComPtr<IGridProvider> grid3_provider = QueryInterfaceFromNode<IGridProvider>(GetRoot()->children()[2]);

    // Grid that specifies aria-rowcount and aria-colcount are both (-1).
    ComPtr<IGridProvider> grid4_provider = QueryInterfaceFromNode<IGridProvider>(GetRoot()->children()[3]);
...
TEST_F(AXPlatformNodeWinTest, IGridProviderGetItem)
{
    ...
    ComPtr<IGridProvider> root_igridprovider(QueryInterfaceFromNode<IGridProvider>(GetRoot()));

    ComPtr<IRawElementProviderSimple> cell1_irawelementprovidersimple(
        QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[0]->children()[0]));
...
TEST_F(AXPlatformNodeWinTest, IGridProviderGetItemCustomAria)
{
    ...
    EXPECT_EQ(QueryInterfaceFromNodeId<IRawElementProviderSimple>(5).Get(), grid_item);
...
TEST_F(AXPlatformNodeWinTest, ITableProviderNoHeaders)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(root_itableprovider->GetRowHeaders(safearray.Receive()));
    EXPECT_EQ(nullptr, safearray.Get());
}
...
TEST_F(AXPlatformNodeWinTest, ITableProviderGetColumnHeadersMultipleHeaders)
{
    ...
    Init(root, row1, row2, row3, header_r1c1, header_r1c2, header_r1c3, cell_r2c1, cell_r2c2, cell_r2c3, cell_r3c1, header_r3c2);
    ...
    // below.
    std::vector<std::wstring> expected_names = { L"header_r1c1", L"header_r1c2", L"header_r3c2", L"header_r1c3" };
    EXPECT_UIA_ELEMENT_ARRAY_BSTR_EQ(safearray.Get(), UIA_NamePropertyId, expected_names);
}
...
TEST_F(AXPlatformNodeWinTest, ITableProviderGetRowHeaders)
{
    ...
    TestAXNodeWrapper* row_header_wrapper = TestAXNodeWrapper::GetOrCreate(GetTree(), GetRoot()->children()[0]->children()[1]);
...
TEST_F(AXPlatformNodeWinTest, ITableItemProviderGetColumnHeaderItems)
{
    ...
    TestAXNodeWrapper* root_wrapper = TestAXNodeWrapper::GetOrCreate(GetTree(), GetRoot());
    root_wrapper->BuildAllWrappers(GetTree(), GetRoot());

    ComPtr<ITableItemProvider> cell_itableitemprovider(QueryInterfaceFromNode<ITableItemProvider>(GetRoot()->children()[1]->children()[0]));

    base::win::ScopedSafearray safearray;
    EXPECT_HRESULT_SUCCEEDED(cell_itableitemprovider->GetColumnHeaderItems(safearray.Receive()));
    EXPECT_NE(nullptr, safearray.Get());
...
TEST_F(AXPlatformNodeWinTest, ITableItemProviderGetRowHeaderItems)
{
    ...
    ComPtr<ITableItemProvider> cell_itableitemprovider(QueryInterfaceFromNode<ITableItemProvider>(GetRoot()->children()[0]->children()[1]));

    base::win::ScopedSafearray safearray;
    EXPECT_HRESULT_SUCCEEDED(cell_itableitemprovider->GetRowHeaderItems(safearray.Receive()));
    EXPECT_NE(nullptr, safearray.Get());
    ...
    // returned.
    TestAXNodeWrapper* row_header_wrapper = TestAXNodeWrapper::GetOrCreate(GetTree(), GetRoot()->children()[0]->children()[0]);
    row_header_wrapper->ResetNativeEventTarget();
...
TEST_F(AXPlatformNodeWinTest, IA2GetAttribute)
{
    ...
    root.AddStringAttribute(ax::mojom::StringAttribute::kRoleDescription, "Potato");
    Init(root);
    ...
    ASSERT_EQ(S_OK, iaccessible2->get_attribute(roledescription.Get(), result.Receive()));
    ...
    EXPECT_EQ(S_FALSE, iaccessible2->get_attribute(smoothness.Get(), result2.Receive()));
...
TEST_F(AXPlatformNodeWinTest, UIAGetPropertySimple)
{
    ...
    root.AddStringAttribute(ax::mojom::StringAttribute::kDescription, "fake description");
    root.AddIntAttribute(ax::mojom::IntAttribute::kSetSize, 2);
    root.AddIntAttribute(ax::mojom::IntAttribute::kInvalidState, 1);
    root.AddIntAttribute(ax::mojom::IntAttribute::kAriaCurrentState, static_cast<int>(ax::mojom::AriaCurrentState::kNone));
    root.id = 1;
    ...
    child1.AddIntAttribute(ax::mojom::IntAttribute::kAriaCurrentState, static_cast<int>(ax::mojom::AriaCurrentState::kPage));
    ...
    child2.AddIntAttribute(ax::mojom::IntAttribute::kAriaCurrentState, static_cast<int>(ax::mojom::AriaCurrentState::kFalse));
    root.child_ids.push_back(child2.id);

    Init(root, child1, child2);

    ComPtr<IRawElementProviderSimple> root_node = GetRootIRawElementProviderSimple();
    ...
    ASSERT_HRESULT_SUCCEEDED(root_node->GetPropertyValue(UIA_AutomationIdPropertyId, uia_id.Receive()));
    EXPECT_UIA_BSTR_EQ(root_node, UIA_AutomationIdPropertyId, uia_id.ptr()->bstrVal);
    EXPECT_UIA_BSTR_EQ(root_node, UIA_FullDescriptionPropertyId, L"fake description");
    ...
    EXPECT_UIA_INT_EQ(root_node, UIA_ControlTypePropertyId, int { UIA_ListControlTypeId });
    EXPECT_UIA_INT_EQ(root_node, UIA_OrientationPropertyId, int { OrientationType_None });
    EXPECT_UIA_INT_EQ(root_node, UIA_SizeOfSetPropertyId, 2);
    EXPECT_UIA_INT_EQ(root_node, UIA_ToggleToggleStatePropertyId, int { ToggleState_Off });
    ...
    ComPtr<IRawElementProviderSimple> child_node1 = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[0]);
    EXPECT_UIA_INT_EQ(child_node1, UIA_PositionInSetPropertyId, 1);
    EXPECT_UIA_BSTR_EQ(child_node1, UIA_AriaPropertiesPropertyId,
    ...
    ComPtr<IRawElementProviderSimple> child_node2 = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[1]);
    // aria-current="false" should not be serialized.
    EXPECT_UIA_BSTR_EQ(child_node2, UIA_AriaPropertiesPropertyId,
        L"readonly=true;expanded=false;multiline=false;multiselectable=false;"
...
TEST_F(AXPlatformNodeWinTest, UIAControlContentPropertyForTableElements)
{
    ...
    ComPtr<IRawElementProviderSimple> row = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[0]);
    EXPECT_UIA_BOOL_EQ(row, UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(row, UIA_IsContentElementPropertyId, true);

    ComPtr<IRawElementProviderSimple> cell = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[0]->children()[0]);
    ...
    ComPtr<IRawElementProviderSimple> generic_container_provider
        = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[0]->children()[0]->children()[0]);
    EXPECT_UIA_BOOL_EQ(generic_container_provider, UIA_IsControlElementPropertyId, false);
    EXPECT_UIA_BOOL_EQ(generic_container_provider, UIA_IsContentElementPropertyId, false);
}
...
TEST_F(AXPlatformNodeWinTest, IsUIAControlColorWellInsideTable)
{
    ...
    ComPtr<IRawElementProviderSimple> st_5_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(st_5_node);
    EXPECT_UIA_BOOL_EQ(st_5_provider, UIA_IsControlElementPropertyId, true);

    ComPtr<IRawElementProviderSimple> st_8_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(st_8_node);
    EXPECT_UIA_BOOL_EQ(st_8_provider, UIA_IsControlElementPropertyId, true);

    ComPtr<IRawElementProviderSimple> st_11_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(st_11_node);
...
TEST_F(AXPlatformNodeWinTest, IsUIAControlForTextNodes)
{
    ...
    ComPtr<IRawElementProviderSimple> text_3_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(text_3_node);
    EXPECT_UIA_BOOL_EQ(text_3_provider, UIA_IsControlElementPropertyId, true);

    ComPtr<IRawElementProviderSimple> text_5_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(text_5_node);
    EXPECT_UIA_BOOL_EQ(text_5_provider, UIA_IsControlElementPropertyId, true);

    ComPtr<IRawElementProviderSimple> text_7_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(text_7_node);
...
TEST_F(AXPlatformNodeWinTest, IsUIAControlForNonFocusableNodesInViews)
{
    ...
    ComPtr<IRawElementProviderSimple> root_1_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(root_1_node);
    EXPECT_UIA_BOOL_EQ(root_1_provider, UIA_IsControlElementPropertyId, true);

    ComPtr<IRawElementProviderSimple> button_2_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(button_2_node);
...
TEST_F(AXPlatformNodeWinTest, IsUIAControlForLiveRegionNodesInViews)
{
    ...
    ComPtr<IRawElementProviderSimple> status_2_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(status_2_node);
...
TEST_F(AXPlatformNodeWinTest, UIAGetPropertyValueClickablePoint)
{
    ...
    ComPtr<IRawElementProviderSimple> raw_element_provider_simple = GetRootIRawElementProviderSimple();
    ...
    EXPECT_UIA_DOUBLE_ARRAY_EQ(raw_element_provider_simple, UIA_ClickablePointPropertyId, expected_values);
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetPropertyValue_Histogram)
{
    ...
    // Log a known property ID
    {
        ...
        histogram_tester.ExpectUniqueSample("Accessibility.WinAPIs.GetPropertyValue", UIA_NamePropertyId, 1);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.WinAPIs.UMA_API_GET_PROPERTY_VALUE", 1);
    }
    ...
    // Collapse unknown property IDs to zero
    {
        ...
        histogram_tester.ExpectUniqueSample("Accessibility.WinAPIs.GetPropertyValue", 0, 1);
        histogram_tester.ExpectTotalCount("Accessibility.Performance.WinAPIs.UMA_API_GET_PROPERTY_VALUE", 1);
    }
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetPropertyValueIsDialog)
{
    ...
    EXPECT_UIA_BOOL_EQ(GetRootIRawElementProviderSimple(), UIA_IsDialogPropertyId, false);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromChildIndex(0), UIA_IsDialogPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromChildIndex(1), UIA_IsDialogPropertyId, true);
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetPropertyValueIsControlElementIgnoredInvisible)
{
    ...
    // control element.
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromChildIndex(0), UIA_IsControlElementPropertyId, true);

    // Button with ignored state (id=3). Should not be a control element.
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromChildIndex(1), UIA_IsControlElementPropertyId, false);

    // Button with invisible state (id=4). Should not be a control element.
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromChildIndex(2), UIA_IsControlElementPropertyId, false);

    // Button with invisible state, but focusable (id=5). Should not be a control
    // element.
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromChildIndex(3), UIA_IsControlElementPropertyId, false);

    // Generic container, focusable (id=6). Should be a control
    // element.
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromChildIndex(4), UIA_IsControlElementPropertyId, true);

    // Generic container, ignored but focusable (id=7). Should not be a control
    // element.
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromChildIndex(5), UIA_IsControlElementPropertyId, false);

    // Generic container, invisible and ignored, but focusable (id=8). Should not
    // be a control element.
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromChildIndex(6), UIA_IsControlElementPropertyId, false);
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetAnnotationObjectsPropertyId)
{
    ...
    ComPtr<IRawElementProviderSimple> root_node = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot());

    ScopedVariant array;
    ASSERT_HRESULT_SUCCEEDED(root_node->GetPropertyValue(UIA_AnnotationObjectsPropertyId, array.Receive()));
    ASSERT_EQ(VT_ARRAY | VT_UNKNOWN, array.type());
    std::vector<std::wstring> expected_names = { L"highlighted", L"comment" };
    EXPECT_UIA_ELEMENT_ARRAY_BSTR_EQ(array.ptr()->parray, UIA_NamePropertyId, expected_names);
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetControllerForPropertyId)
{
    ...
    tab.AddIntListAttribute(ax::mojom::IntListAttribute::kControlsIds, controller_ids);
    ...
    group1.AddIntListAttribute(ax::mojom::IntListAttribute::kErrormessageIds, std::vector<AXNodeID> { 6 });
    ...
    ComPtr<IRawElementProviderSimple> tab_node = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[0]);

    std::vector<std::wstring> expected_names_1 = { L"panel1", L"panel2" };
    EXPECT_UIA_PROPERTY_ELEMENT_ARRAY_BSTR_EQ(tab_node, UIA_ControllerForPropertyId, UIA_NamePropertyId, expected_names_1);

    // Remove panel1's native event target and verify it's no longer returned.
    TestAXNodeWrapper* panel1_wrapper = TestAXNodeWrapper::GetOrCreate(GetTree(), GetRoot()->children()[1]);
    panel1_wrapper->ResetNativeEventTarget();
    std::vector<std::wstring> expected_names_2 = { L"panel2" };
    EXPECT_UIA_PROPERTY_ELEMENT_ARRAY_BSTR_EQ(tab_node, UIA_ControllerForPropertyId, UIA_NamePropertyId, expected_names_2);

    // The aria-errormessage attribute should be exposed through the ControllerFor
    // UIA property.
    ComPtr<IRawElementProviderSimple> group1_node = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[3]);

    std::vector<std::wstring> expected_names_3 = { L"text1" };
    EXPECT_UIA_PROPERTY_ELEMENT_ARRAY_BSTR_EQ(group1_node, UIA_ControllerForPropertyId, UIA_NamePropertyId, expected_names_3);
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetDescribedByPropertyId)
{
    ...
    root.AddIntListAttribute(ax::mojom::IntListAttribute::kDetailsIds, describedby_ids);
    root.id = 1;
    ...
    EXPECT_UIA_PROPERTY_ELEMENT_ARRAY_BSTR_EQ(root_node, UIA_DescribedByPropertyId, UIA_NamePropertyId, expected_names);
}
...
TEST_F(AXPlatformNodeWinTest, UIAItemStatusPropertyId)
{
    ...
    row1.id = 2;
    row1.role = ax::mojom::Role::kRow;
    row1.AddIntAttribute(ax::mojom::IntAttribute::kSortDirection, static_cast<int>(ax::mojom::SortDirection::kAscending));
    root.child_ids.push_back(row1.id);

    AXNodeData header1;
    header1.id = 3;
    header1.role = ax::mojom::Role::kRowHeader;
    header1.AddIntAttribute(ax::mojom::IntAttribute::kSortDirection, static_cast<int>(ax::mojom::SortDirection::kAscending));
    row1.child_ids.push_back(header1.id);

    AXNodeData header2;
    header2.id = 4;
    header2.role = ax::mojom::Role::kColumnHeader;
    header2.AddIntAttribute(ax::mojom::IntAttribute::kSortDirection, static_cast<int>(ax::mojom::SortDirection::kDescending));
    row1.child_ids.push_back(header2.id);

    AXNodeData header3;
    header3.id = 5;
    header3.role = ax::mojom::Role::kColumnHeader;
    header3.AddIntAttribute(ax::mojom::IntAttribute::kSortDirection, static_cast<int>(ax::mojom::SortDirection::kOther));
    row1.child_ids.push_back(header3.id);

    AXNodeData header4;
    header4.id = 6;
    header4.role = ax::mojom::Role::kColumnHeader;
    header4.AddIntAttribute(ax::mojom::IntAttribute::kSortDirection, static_cast<int>(ax::mojom::SortDirection::kUnsorted));
    ...
    EXPECT_UIA_BSTR_EQ(QueryInterfaceFromNode<IRawElementProviderSimple>(row_node->children()[0]), UIA_ItemStatusPropertyId, L"ascending");

    EXPECT_UIA_BSTR_EQ(QueryInterfaceFromNode<IRawElementProviderSimple>(row_node->children()[1]), UIA_ItemStatusPropertyId, L"descending");

    EXPECT_UIA_BSTR_EQ(QueryInterfaceFromNode<IRawElementProviderSimple>(row_node->children()[2]), UIA_ItemStatusPropertyId, L"other");

    EXPECT_UIA_VALUE_EQ(QueryInterfaceFromNode<IRawElementProviderSimple>(row_node->children()[3]), UIA_ItemStatusPropertyId, ScopedVariant::kEmptyVariant);

    EXPECT_UIA_VALUE_EQ(QueryInterfaceFromNode<IRawElementProviderSimple>(row_node), UIA_ItemStatusPropertyId, ScopedVariant::kEmptyVariant);
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetFlowsToPropertyId)
{
    ...
    ComPtr<IRawElementProviderSimple> root_node = GetRootIRawElementProviderSimple();
    std::vector<std::wstring> expected_names = { L"child1", L"child2" };
    EXPECT_UIA_PROPERTY_ELEMENT_ARRAY_BSTR_EQ(root_node, UIA_FlowsToPropertyId, UIA_NamePropertyId, expected_names);
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetPropertyValueFlowsFromNone)
{
    ...
    ScopedVariant property_value;
    EXPECT_HRESULT_SUCCEEDED(root_node->GetPropertyValue(UIA_FlowsFromPropertyId, property_value.Receive()));
...
TEST_F(AXPlatformNodeWinTest, UIAGetPropertyValueFlowsFromSingle)
{
    ...
    EXPECT_UIA_PROPERTY_ELEMENT_ARRAY_BSTR_EQ(child_node1, UIA_FlowsFromPropertyId, UIA_NamePropertyId, expected_names);
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetPropertyValueFlowsFromMultiple)
{
    ...
    Init(root, child1, child2);
    ASSERT_NE(nullptr, TestAXNodeWrapper::GetOrCreate(GetTree(), GetRoot()));
    ASSERT_NE(nullptr, TestAXNodeWrapper::GetOrCreate(GetTree(), GetRoot()->children()[0]));

    ComPtr<IRawElementProviderSimple> child_node2 = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[1]);
    std::vector<std::wstring> expected_names_1 = { L"root", L"child1" };
    EXPECT_UIA_PROPERTY_UNORDERED_ELEMENT_ARRAY_BSTR_EQ(child_node2, UIA_FlowsFromPropertyId, UIA_NamePropertyId, expected_names_1);

    // Remove child1's native event target and verify it's no longer returned.
    TestAXNodeWrapper* child1_wrapper = TestAXNodeWrapper::GetOrCreate(GetTree(), GetRoot()->children()[0]);
    child1_wrapper->ResetNativeEventTarget();
    std::vector<std::wstring> expected_names_2 = { L"root" };
    EXPECT_UIA_PROPERTY_UNORDERED_ELEMENT_ARRAY_BSTR_EQ(child_node2, UIA_FlowsFromPropertyId, UIA_NamePropertyId, expected_names_2);
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetPropertyValueFrameworkId)
{
    ...
    ComPtr<IRawElementProviderSimple> root_raw_element_provider_simple = GetRootIRawElementProviderSimple();
    EXPECT_UIA_BSTR_EQ(root_raw_element_provider_simple, UIA_FrameworkIdPropertyId, L"Chrome");
}
...
TEST_F(AXPlatformNodeWinTest, GetPropertyValue_LabeledByTest)
{
    ...
    gc_2.AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, { static_text_3.id });
    ...
    gc_4.AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, { gc_5.id });
    ...
    alert_7.AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, { static_text_6.id });
    ...
    ComPtr<IRawElementProviderSimple> gc_2_provider = GetIRawElementProviderSimpleFromId(gc_2_node->id());
    ScopedVariant property_value;
    EXPECT_EQ(S_OK, gc_2_provider->GetPropertyValue(UIA_LabeledByPropertyId, property_value.Receive()));
    ASSERT_EQ(property_value.type(), VT_UNKNOWN);
    ComPtr<IRawElementProviderSimple> static_text_3_provider;
    EXPECT_EQ(S_OK, property_value.ptr()->punkVal->QueryInterface(IID_PPV_ARGS(&static_text_3_provider)));
    ...
    ComPtr<IRawElementProviderSimple> gc_4_provider = GetIRawElementProviderSimpleFromId(gc_4_node->id());
    property_value.Reset();
    EXPECT_EQ(S_OK, gc_4_provider->GetPropertyValue(UIA_LabeledByPropertyId, property_value.Receive()));
    ASSERT_EQ(property_value.type(), VT_UNKNOWN);
    ComPtr<IRawElementProviderSimple> static_text_6_provider;
    EXPECT_EQ(S_OK, property_value.ptr()->punkVal->QueryInterface(IID_PPV_ARGS(&static_text_6_provider)));
    ...
    ComPtr<IRawElementProviderSimple> alert_7_provider = GetIRawElementProviderSimpleFromId(alert_7_node->id());
    property_value.Reset();
    EXPECT_EQ(S_OK, alert_7_provider->GetPropertyValue(UIA_LabeledByPropertyId, property_value.Receive()));
    ...
    // Case 1.
    TestAXNodeWrapper* static_text_3_node_wrapper = TestAXNodeWrapper::GetOrCreate(GetTree(), static_text_3_node);
    ...
    // Case 2.
    TestAXNodeWrapper* static_text_6_node_wrapper = TestAXNodeWrapper::GetOrCreate(GetTree(), static_text_6_node);
...
TEST_F(AXPlatformNodeWinTest, GetPropertyValue_HelpText)
{
    ...
    update.nodes[1].AddIntAttribute(ax::mojom::IntAttribute::kNameFrom, static_cast<int>(ax::mojom::NameFrom::kTitle));
    update.nodes[2].AddIntAttribute(ax::mojom::IntAttribute::kNameFrom, static_cast<int>(ax::mojom::NameFrom::kTitle));
    update.nodes[3].AddIntAttribute(ax::mojom::IntAttribute::kNameFrom, static_cast<int>(ax::mojom::NameFrom::kPlaceholder));
    update.nodes[4].AddIntAttribute(ax::mojom::IntAttribute::kNameFrom, static_cast<int>(ax::mojom::NameFrom::kAttribute));
    update.nodes[5].AddIntAttribute(ax::mojom::IntAttribute::kNameFrom, static_cast<int>(ax::mojom::NameFrom::kAttribute));

    Init(update);

    auto* root_node = GetRoot();
    EXPECT_UIA_BSTR_EQ(QueryInterfaceFromNode<IRawElementProviderSimple>(root_node->children()[0]), UIA_HelpTextPropertyId, L"placeholder");
    EXPECT_UIA_BSTR_EQ(QueryInterfaceFromNode<IRawElementProviderSimple>(root_node->children()[1]), UIA_HelpTextPropertyId, L"name-from-title");
    EXPECT_UIA_BSTR_EQ(QueryInterfaceFromNode<IRawElementProviderSimple>(root_node->children()[2]), UIA_HelpTextPropertyId, L"name-from-placeholder");
    EXPECT_UIA_BSTR_EQ(QueryInterfaceFromNode<IRawElementProviderSimple>(root_node->children()[3]), UIA_HelpTextPropertyId, L"tooltip");
    EXPECT_UIA_VALUE_EQ(QueryInterfaceFromNode<IRawElementProviderSimple>(root_node->children()[4]), UIA_HelpTextPropertyId, ScopedVariant::kEmptyVariant);
}
...
TEST_F(AXPlatformNodeWinTest, GetPropertyValue_LocalizedControlType)
{
    ...
    ComPtr<IRawElementProviderSimple> root_node = GetRootIRawElementProviderSimple();
    EXPECT_UIA_BSTR_EQ(root_node, UIA_LocalizedControlTypePropertyId, L"rootRoleDescription");
    EXPECT_UIA_BSTR_EQ(
        QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[0]), UIA_LocalizedControlTypePropertyId, L"child1RoleDescription");
    EXPECT_UIA_EMPTY(QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[1]), UIA_LocalizedControlTypePropertyId);
}
...
TEST_F(AXPlatformNodeWinTest, GetPropertyValue_IsControlElement)
{
    ...
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(2), UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(3), UIA_IsControlElementPropertyId, false);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(4), UIA_IsControlElementPropertyId, false);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(5), UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(6), UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(7), UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(8), UIA_IsControlElementPropertyId, false);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(9), UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(10), UIA_IsControlElementPropertyId, false);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(11), UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(12), UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(13), UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(14), UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(15), UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(16), UIA_IsControlElementPropertyId, true);
    EXPECT_UIA_BOOL_EQ(GetIRawElementProviderSimpleFromId(17), UIA_IsControlElementPropertyId, true);
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetProviderOptions)
{
    ...
    EXPECT_EQ(
        ProviderOptions_ServerSideProvider | ProviderOptions_UseComThreading | ProviderOptions_RefuseNonClientSupport | ProviderOptions_HasNativeIAccessible,
        provider_options);
}
...
TEST_F(AXPlatformNodeWinTest, UIAGetHostRawElementProvider)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(root_node->get_HostRawElementProvider(&host_provider));
...
TEST_F(AXPlatformNodeWinTest, UIAGetBoundingRectangle)
{
    ...
    ComPtr<IRawElementProviderFragment> root_node = GetRootIRawElementProviderFragment();

    UiaRect bounding_rectangle;
    EXPECT_HRESULT_SUCCEEDED(root_node->get_BoundingRectangle(&bounding_rectangle));
...
TEST_F(AXPlatformNodeWinTest, UIAGetFragmentRoot)
{
    ...
    ComPtr<IRawElementProviderFragment> element1_provider = QueryInterfaceFromNode<IRawElementProviderFragment>(element1_node);
    ComPtr<IRawElementProviderFragmentRoot> expected_fragment_root = GetFragmentRoot();

    ComPtr<IRawElementProviderFragmentRoot> actual_fragment_root;
    EXPECT_HRESULT_SUCCEEDED(element1_provider->get_FragmentRoot(&actual_fragment_root));
    ...
    EXPECT_UIA_ELEMENTNOTAVAILABLE(element1_provider->get_FragmentRoot(&actual_fragment_root));

    // Test the case where the widget has gone away.
    TestAXNodeWrapper* element1_wrapper = TestAXNodeWrapper::GetOrCreate(GetTree(), element1_node);
...
TEST_F(AXPlatformNodeWinTest, UIAGetEmbeddedFragmentRoots)
{
    ...
    ComPtr<IRawElementProviderFragment> root_provider = GetRootIRawElementProviderFragment();

    base::win::ScopedSafearray embedded_fragment_roots;
    EXPECT_HRESULT_SUCCEEDED(root_provider->GetEmbeddedFragmentRoots(embedded_fragment_roots.Receive()));
...
TEST_F(AXPlatformNodeWinTest, UIAGetRuntimeIdForGeneratedId)
{
    ...
    LONG array_lower_bound;
    EXPECT_HRESULT_SUCCEEDED(::SafeArrayGetLBound(runtime_id.Get(), 1, &array_lower_bound));
    EXPECT_EQ(0, array_lower_bound);

    LONG array_upper_bound;
    EXPECT_HRESULT_SUCCEEDED(::SafeArrayGetUBound(runtime_id.Get(), 1, &array_upper_bound));
    EXPECT_EQ(3, array_upper_bound);

    int* array_data;
    EXPECT_HRESULT_SUCCEEDED(::SafeArrayAccessData(runtime_id.Get(), reinterpret_cast<void**>(&array_data)));
...
TEST_F(AXPlatformNodeWinTest, UIAIWindowProviderGetIsModalUnset)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(raw_element_provider_simple->GetPatternProvider(UIA_WindowPatternId, &window_provider));
...
TEST_F(AXPlatformNodeWinTest, UIAIWindowProviderNotSupported)
{
    ...
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_NOTSUPPORTED), window_provider->SetVisualState(WindowVisualState::WindowVisualState_Normal));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_NOTSUPPORTED), window_provider->Close());
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_NOTSUPPORTED), window_provider->WaitForInputIdle(0, &bool_result));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_NOTSUPPORTED), window_provider->get_CanMaximize(&bool_result));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_NOTSUPPORTED), window_provider->get_CanMinimize(&bool_result));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_NOTSUPPORTED), window_provider->get_WindowVisualState(&window_visual_state_result));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_NOTSUPPORTED), window_provider->get_WindowInteractionState(&window_interaction_state_result));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_NOTSUPPORTED), window_provider->get_IsTopmost(&bool_result));
}
...
TEST_F(AXPlatformNodeWinTest, UIANavigate)
{
    ...
    auto TestNavigate = [this](AXNode* element_node, AXNode* parent, AXNode* next_sibling, AXNode* prev_sibling, AXNode* first_child, AXNode* last_child) {
        ComPtr<IRawElementProviderFragment> element_provider = QueryInterfaceFromNode<IRawElementProviderFragment>(element_node);

        auto TestNavigateSingle = [&](NavigateDirection direction, AXNode* expected_node) {
            ComPtr<IRawElementProviderFragment> expected_provider = QueryInterfaceFromNode<IRawElementProviderFragment>(expected_node);

            ComPtr<IRawElementProviderFragment> navigated_to_fragment;
            EXPECT_HRESULT_SUCCEEDED(element_provider->Navigate(direction, &navigated_to_fragment));
    ...
    TestNavigate(element1_node, root_node, element2_node, nullptr, element3_node, element3_node);

    TestNavigate(element2_node, root_node, nullptr, element1_node, nullptr, nullptr);

    TestNavigate(element3_node, element1_node, nullptr, nullptr, nullptr, nullptr);
}
...
TEST_F(AXPlatformNodeWinTest, IAnnotationProvider)
{
    ...
    root.role = ax::mojom::Role::kRootWebArea;
    root.SetName("root");
    root.child_ids = { highlighted1.id, comment1.id, footnote.id, definition.id, button.id };

    highlighted1.role = ax::mojom::Role::kMark;
    highlighted1.AddIntListAttribute(ax::mojom::IntListAttribute::kDetailsIds, { comment1.id, footnote.id, definition.id });
    ...
    Init(root, highlighted1, highlighted1_text, comment1, comment1_text, footnote, footnote_text, definition, button);

    ComPtr<IRawElementProviderSimple> highlighted1_node = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[0]);
    ComPtr<IRawElementProviderSimple> comment1_node = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[1]);
    ComPtr<IRawElementProviderSimple> footnote_node = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[2]);
    ComPtr<IRawElementProviderSimple> definition_node = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[3]);
    ComPtr<IRawElementProviderSimple> button_node = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[4]);
...

    {
        ...
        // target.
        EXPECT_HRESULT_SUCCEEDED(highlighted1_node->GetPatternProvider(UIA_AnnotationPatternId, &annotation_provider));
...

    {
        ...
        EXPECT_HRESULT_SUCCEEDED(comment1_node->GetPatternProvider(UIA_AnnotationPatternId, &annotation_provider));
        ASSERT_NE(nullptr, annotation_provider.Get());
        EXPECT_HRESULT_SUCCEEDED(annotation_provider->get_AnnotationTypeId(&annotation_type));
        EXPECT_EQ(AnnotationType_Comment, annotation_type);
        EXPECT_HRESULT_SUCCEEDED(annotation_provider->get_AnnotationTypeName(name.Receive()));
...

    {
        ...
        // IAnnotationProvider.
        EXPECT_HRESULT_SUCCEEDED(footnote_node->GetPatternProvider(UIA_AnnotationPatternId, &annotation_provider));
        ASSERT_NE(nullptr, annotation_provider.Get());
        EXPECT_HRESULT_SUCCEEDED(annotation_provider->get_AnnotationTypeId(&annotation_type));
...

    {
        ...
        // IAnnotationProvider.
        EXPECT_HRESULT_SUCCEEDED(definition_node->GetPatternProvider(UIA_AnnotationPatternId, &annotation_provider));
        ASSERT_NE(nullptr, annotation_provider.Get());
...

    {
        ...
        EXPECT_HRESULT_SUCCEEDED(button_node->GetPatternProvider(UIA_AnnotationPatternId, &annotation_provider));
        ASSERT_EQ(nullptr, annotation_provider.Get());
    }
}
...
TEST_F(AXPlatformNodeWinTest, IAnnotationProviderMultipleTargets)
{
    ...
    highlighted1.role = ax::mojom::Role::kMark;
    highlighted1.AddIntListAttribute(ax::mojom::IntListAttribute::kDetailsIds, { comment1.id });
    highlighted1.child_ids = { highlighted1_text.id };
    ...
    highlighted2.AddIntListAttribute(ax::mojom::IntListAttribute::kDetailsIds, { comment1.id });
    ...
    endnote.AddIntListAttribute(ax::mojom::IntListAttribute::kDetailsIds, { comment1.id });
    ...
    Init(root, highlighted1, highlighted1_text, highlighted2, highlighted2_text, endnote, endnote_text, comment1, comment1_text);

    ComPtr<IRawElementProviderSimple> highlighted1_node = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[0]);
    ComPtr<IRawElementProviderSimple> comment1_node = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[3]);
...
TEST_F(AXPlatformNodeWinTest, ISelectionProviderCanSelectMultipleDefault)
{
    ...
    ComPtr<ISelectionProvider> selection_provider(QueryInterfaceFromNode<ISelectionProvider>(GetRoot()));

    BOOL multiple = TRUE;
    EXPECT_HRESULT_SUCCEEDED(selection_provider->get_CanSelectMultiple(&multiple));
    EXPECT_FALSE(multiple);
}

TEST_F(AXPlatformNodeWinTest, ISelectionProviderCanSelectMultipleTrue)
{
    const std::vector<ax::mojom::State> state = { ax::mojom::State::kMultiselectable, ax::mojom::State::kFocusable };
    Init(BuildListBox(/*option_1_is_selected*/ false,
        /*option_2_is_selected*/ false,
        /*option_3_is_selected*/ false,
...
TEST_F(AXPlatformNodeWinTest, ISelectionProviderIsSelectionRequiredDefault)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(selection_provider->get_IsSelectionRequired(&selection_required));
...
TEST_F(AXPlatformNodeWinTest, ISelectionProviderGetSelectionNoneSelected)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(selection_provider->GetSelection(selected_items.Receive()));
    EXPECT_NE(nullptr, selected_items.Get());

    LONG array_lower_bound;
    EXPECT_HRESULT_SUCCEEDED(::SafeArrayGetLBound(selected_items.Get(), 1, &array_lower_bound));
    EXPECT_EQ(0, array_lower_bound);

    LONG array_upper_bound;
    EXPECT_HRESULT_SUCCEEDED(::SafeArrayGetUBound(selected_items.Get(), 1, &array_upper_bound));
...
TEST_F(AXPlatformNodeWinTest, ISelectionProviderGetSelectionSingleItemSelected)
{
    ...
    ComPtr<ISelectionProvider> selection_provider(QueryInterfaceFromNode<ISelectionProvider>(GetRoot()));
    ComPtr<IRawElementProviderSimple> option2_provider(QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[1]));
    ...
    EXPECT_HRESULT_SUCCEEDED(::SafeArrayAccessData(selected_items.Get(), reinterpret_cast<void**>(&array_data)));
...
TEST_F(AXPlatformNodeWinTest, ISelectionProviderGetSelectionMultipleItemsSelected)
{
    ...
    ComPtr<ISelectionProvider> selection_provider(QueryInterfaceFromNode<ISelectionProvider>(GetRoot()));
    ComPtr<IRawElementProviderSimple> option1_provider(QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[0]));
    ComPtr<IRawElementProviderSimple> option2_provider(QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[1]));
    ComPtr<IRawElementProviderSimple> option3_provider(QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot()->children()[2]));
...
TEST_F(AXPlatformNodeWinTest, ComputeUIAControlType)
{
    ...
    Init(root, child1, child2, child3, child4, child5, child6, child7, child8, child9, child10);

    EXPECT_UIA_INT_EQ(QueryInterfaceFromNodeId<IRawElementProviderSimple>(child1.id), UIA_ControlTypePropertyId, int { UIA_TableControlTypeId });
    EXPECT_UIA_INT_EQ(QueryInterfaceFromNodeId<IRawElementProviderSimple>(child2.id), UIA_ControlTypePropertyId, int { UIA_GroupControlTypeId });
    EXPECT_UIA_INT_EQ(QueryInterfaceFromNodeId<IRawElementProviderSimple>(child3.id), UIA_ControlTypePropertyId, int { UIA_EditControlTypeId });
    EXPECT_UIA_INT_EQ(QueryInterfaceFromNodeId<IRawElementProviderSimple>(child4.id), UIA_ControlTypePropertyId, int { UIA_EditControlTypeId });
    EXPECT_UIA_INT_EQ(QueryInterfaceFromNodeId<IRawElementProviderSimple>(child5.id), UIA_ControlTypePropertyId, int { UIA_TitleBarControlTypeId });
    EXPECT_UIA_INT_EQ(QueryInterfaceFromNodeId<IRawElementProviderSimple>(child6.id), UIA_ControlTypePropertyId, int { UIA_WindowControlTypeId });
    EXPECT_UIA_INT_EQ(QueryInterfaceFromNodeId<IRawElementProviderSimple>(child7.id), UIA_ControlTypePropertyId, int { UIA_GroupControlTypeId });
    EXPECT_UIA_INT_EQ(QueryInterfaceFromNodeId<IRawElementProviderSimple>(child8.id), UIA_ControlTypePropertyId, int { UIA_SeparatorControlTypeId });
    EXPECT_UIA_INT_EQ(QueryInterfaceFromNodeId<IRawElementProviderSimple>(child9.id), UIA_ControlTypePropertyId, int { UIA_ThumbControlTypeId });
    EXPECT_UIA_INT_EQ(QueryInterfaceFromNodeId<IRawElementProviderSimple>(child10.id), UIA_ControlTypePropertyId, int { UIA_TextControlTypeId });
}
...
TEST_F(AXPlatformNodeWinTest, IsUIAControlForStatusRole)
{
    ...
    ComPtr<IRawElementProviderSimple> status_node_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(status_node);
    EXPECT_UIA_BOOL_EQ(status_node_provider, UIA_IsControlElementPropertyId, true);
}

TEST_F(AXPlatformNodeWinTest, UIALandmarkType)
{
    auto TestLandmarkType = [this](ax::mojom::Role node_role, std::optional<LONG> expected_landmark_type, const std::string& node_name = {}) {
        AXNodeData root_data;
        root_data.id = 1;
        ...
        ComPtr<IRawElementProviderSimple> root_provider = GetRootIRawElementProviderSimple();

        if (expected_landmark_type) {
            EXPECT_UIA_INT_EQ(root_provider, UIA_LandmarkTypePropertyId, expected_landmark_type.value());
        } else {
...
{
    auto TestLocalizedLandmarkType = [this](ax::mojom::Role node_role, const std::wstring& expected_localized_landmark, const std::string& node_name = {}) {
        AXNodeData root_data;
        root_data.id = 1;
        root_data.role = node_role;
        if (!node_name.empty())
        ...
        if (expected_localized_landmark.empty()) {
            ...
            EXPECT_UIA_BSTR_EQ(root_provider, UIA_LocalizedLandmarkTypePropertyId, expected_localized_landmark.c_str());
        }
    };
    ...
    TestLocalizedLandmarkType(ax::mojom::Role::kContentInfo, L"content information");
...
TEST_F(AXPlatformNodeWinTest, IRawElementProviderSimple2ShowContextMenu)
{
    ...
    ComPtr<IRawElementProviderSimple2> root_provider = QueryInterfaceFromNode<IRawElementProviderSimple2>(root_node);
    ComPtr<IRawElementProviderSimple2> element1_provider = QueryInterfaceFromNode<IRawElementProviderSimple2>(element1_node);
    ComPtr<IRawElementProviderSimple2> element2_provider = QueryInterfaceFromNode<IRawElementProviderSimple2>(element2_node);
...
TEST_F(AXPlatformNodeWinTest, UIAErrorHandling)
{
    ...
    ComPtr<IRawElementProviderSimple> simple_provider = GetRootIRawElementProviderSimple();
    ComPtr<IRawElementProviderSimple2> simple2_provider = QueryInterfaceFromNode<IRawElementProviderSimple2>(GetRoot());
    ComPtr<IRawElementProviderFragment> fragment_provider = GetRootIRawElementProviderFragment();
    ComPtr<IGridItemProvider> grid_item_provider = QueryInterfaceFromNode<IGridItemProvider>(GetRoot());
    ComPtr<IGridProvider> grid_provider = QueryInterfaceFromNode<IGridProvider>(GetRoot());
    ComPtr<IScrollItemProvider> scroll_item_provider = QueryInterfaceFromNode<IScrollItemProvider>(GetRoot());
    ComPtr<IScrollProvider> scroll_provider = QueryInterfaceFromNode<IScrollProvider>(GetRoot());
    ComPtr<ISelectionItemProvider> selection_item_provider = QueryInterfaceFromNode<ISelectionItemProvider>(GetRoot());
    ComPtr<ISelectionProvider> selection_provider = QueryInterfaceFromNode<ISelectionProvider>(GetRoot());
    ComPtr<ITableItemProvider> table_item_provider = QueryInterfaceFromNode<ITableItemProvider>(GetRoot());
    ComPtr<ITableProvider> table_provider = QueryInterfaceFromNode<ITableProvider>(GetRoot());
    ComPtr<IExpandCollapseProvider> expand_collapse_provider = QueryInterfaceFromNode<IExpandCollapseProvider>(GetRoot());
    ComPtr<IToggleProvider> toggle_provider = QueryInterfaceFromNode<IToggleProvider>(GetRoot());
    ComPtr<IValueProvider> value_provider = QueryInterfaceFromNode<IValueProvider>(GetRoot());
    ComPtr<IRangeValueProvider> range_value_provider = QueryInterfaceFromNode<IRangeValueProvider>(GetRoot());
    ComPtr<IWindowProvider> window_provider = QueryInterfaceFromNode<IWindowProvider>(GetRoot());
    ...
    // IGridItemProvider
    int int_result = 0;
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), grid_item_provider->get_Column(&int_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), grid_item_provider->get_ColumnSpan(&int_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), grid_item_provider->get_Row(&int_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), grid_item_provider->get_RowSpan(&int_result));

    // IExpandCollapseProvider
    ExpandCollapseState expand_collapse_state;
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), expand_collapse_provider->Collapse());
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), expand_collapse_provider->Expand());
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), expand_collapse_provider->get_ExpandCollapseState(&expand_collapse_state));

    // IGridProvider
    ComPtr<IRawElementProviderSimple> temp_simple_provider;
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), grid_provider->GetItem(0, 0, &temp_simple_provider));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), grid_provider->get_RowCount(&int_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), grid_provider->get_ColumnCount(&int_result));

    // IScrollItemProvider
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), scroll_item_provider->ScrollIntoView());

    // IScrollProvider
    BOOL bool_result = TRUE;
    double double_result = 3.14;
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), scroll_provider->SetScrollPercent(0, 0));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), scroll_provider->get_HorizontallyScrollable(&bool_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), scroll_provider->get_HorizontalScrollPercent(&double_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), scroll_provider->get_HorizontalViewSize(&double_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), scroll_provider->get_VerticallyScrollable(&bool_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), scroll_provider->get_VerticalScrollPercent(&double_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), scroll_provider->get_VerticalViewSize(&double_result));

    // ISelectionItemProvider
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), selection_item_provider->AddToSelection());
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), selection_item_provider->RemoveFromSelection());
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), selection_item_provider->Select());
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), selection_item_provider->get_IsSelected(&bool_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), selection_item_provider->get_SelectionContainer(&temp_simple_provider));

    // ISelectionProvider
    base::win::ScopedSafearray array_result;
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), selection_provider->GetSelection(array_result.Receive()));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), selection_provider->get_CanSelectMultiple(&bool_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), selection_provider->get_IsSelectionRequired(&bool_result));

    // ITableItemProvider
    RowOrColumnMajor row_or_column_major_result;
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), table_item_provider->GetColumnHeaderItems(array_result.Receive()));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), table_item_provider->GetRowHeaderItems(array_result.Receive()));

    // ITableProvider
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), table_provider->GetColumnHeaders(array_result.Receive()));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), table_provider->GetRowHeaders(array_result.Receive()));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), table_provider->get_RowOrColumnMajor(&row_or_column_major_result));
    ...
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), simple_provider->GetPatternProvider(UIA_WindowPatternId, &unknown));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), simple_provider->GetPropertyValue(UIA_FrameworkIdPropertyId, variant.Receive()));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), simple_provider->get_ProviderOptions(&options));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), simple_provider->get_HostRawElementProvider(&host_provider));

    // IRawElementProviderSimple2
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), simple2_provider->ShowContextMenu());
    ...
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), fragment_provider->Navigate(NavigateDirection_Parent, &navigated_to_fragment));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), fragment_provider->GetRuntimeId(safearray.Receive()));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), fragment_provider->get_BoundingRectangle(&bounding_rectangle));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), fragment_provider->GetEmbeddedFragmentRoots(safearray.Receive()));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), fragment_provider->get_FragmentRoot(&fragment_root));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), fragment_provider->SetFocus());

    // IValueProvider
    ScopedBstr bstr_value;
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), value_provider->SetValue(L"3.14"));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), value_provider->get_Value(bstr_value.Receive()));

    // IRangeValueProvider
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), range_value_provider->SetValue(double_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), range_value_provider->get_LargeChange(&double_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), range_value_provider->get_Maximum(&double_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), range_value_provider->get_Minimum(&double_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), range_value_provider->get_SmallChange(&double_result));
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), range_value_provider->get_Value(&double_result));

    // IToggleProvider
    ToggleState toggle_state;
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), toggle_provider->Toggle());
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), toggle_provider->get_ToggleState(&toggle_state));

    // IWindowProvider
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), window_provider->SetVisualState(WindowVisualState::WindowVisualState_Normal));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), window_provider->Close());
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), window_provider->WaitForInputIdle(0, nullptr));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), window_provider->get_CanMaximize(nullptr));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), window_provider->get_CanMinimize(nullptr));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), window_provider->get_IsModal(nullptr));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), window_provider->get_WindowVisualState(nullptr));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), window_provider->get_WindowInteractionState(nullptr));
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), window_provider->get_IsTopmost(nullptr));
}
...
TEST_F(AXPlatformNodeWinTest, GetPatternProviderSupportedPatterns)
{
    ...
    update.nodes[0].child_ids
        = { text_field_with_combo_box_id, meter_id, group_with_scroll_id, checkbox_id, link_id, table_without_header_id, table_with_header_id,
              grid_without_header_id, grid_with_header_id, button_with_value, button_without_value, tree_item_checked_id, tree_item_unchecked_id, tree_item_id,
              tab_id, toggle_button_with_popup_id, generic_container_id, button_with_expanded_state, button_without_expanded_state, list_grid_id };
    ...
    update.nodes[8].child_ids = { table_with_header_row_1_id, table_with_header_row_2_id };
    ...
    update.nodes[15].child_ids = { grid_with_header_row_1_id, grid_with_header_row_2_id };
    ...
    update.nodes[20].AddStringAttribute(ax::mojom::StringAttribute::kValue, "test");
    ...
    update.nodes[22].AddIntAttribute(ax::mojom::IntAttribute::kCheckedState, static_cast<int>(ax::mojom::CheckedState::kTrue));
    update.nodes[23].id = tree_item_unchecked_id;
    update.nodes[23].role = ax::mojom::Role::kTreeItem;
    update.nodes[23].AddIntAttribute(ax::mojom::IntAttribute::kCheckedState, static_cast<int>(ax::mojom::CheckedState::kFalse));
    ...
    update.nodes[26].AddIntAttribute(ax::mojom::IntAttribute::kHasPopup, static_cast<int32_t>(ax::mojom::HasPopup::kTrue));
    ...
    update.nodes[31].AddIntAttribute(ax::mojom::IntAttribute::kCheckedState, static_cast<int>(ax::mojom::CheckedState::kTrue));
    update.nodes[32].id = button_without_expanded_state;
    update.nodes[32].role = ax::mojom::Role::kButton;
    update.nodes[32].AddIntAttribute(ax::mojom::IntAttribute::kCheckedState, static_cast<int>(ax::mojom::CheckedState::kTrue));
    ...
    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_TextEditPatternId, UIA_TextPatternId, UIA_ValuePatternId }), GetSupportedPatternsFromNodeId(root_id));

    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_ValuePatternId, UIA_ExpandCollapsePatternId, UIA_TextChildPatternId, UIA_TextEditPatternId,
                  UIA_TextPatternId }),
        GetSupportedPatternsFromNodeId(text_field_with_combo_box_id));

    EXPECT_EQ(
        PatternSet({ UIA_ScrollItemPatternId, UIA_ValuePatternId, UIA_RangeValuePatternId, UIA_TextChildPatternId }), GetSupportedPatternsFromNodeId(meter_id));

    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_ScrollPatternId, UIA_TextChildPatternId }), GetSupportedPatternsFromNodeId(group_with_scroll_id));

    EXPECT_EQ(
        PatternSet({ UIA_ScrollItemPatternId, UIA_ValuePatternId, UIA_TextChildPatternId, UIA_TogglePatternId }), GetSupportedPatternsFromNodeId(checkbox_id));

    EXPECT_EQ(
        PatternSet({ UIA_ScrollItemPatternId, UIA_ValuePatternId, UIA_InvokePatternId, UIA_TextChildPatternId }), GetSupportedPatternsFromNodeId(link_id));

    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_GridPatternId, UIA_TablePatternId, UIA_TextChildPatternId }),
        GetSupportedPatternsFromNodeId(table_without_header_id));

    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_GridItemPatternId, UIA_TableItemPatternId, UIA_TextChildPatternId }),
    ...
    EXPECT_EQ(
        PatternSet({ UIA_ScrollItemPatternId, UIA_ValuePatternId, UIA_SelectionPatternId, UIA_GridPatternId, UIA_TablePatternId, UIA_TextChildPatternId }),
        GetSupportedPatternsFromNodeId(grid_without_header_id));

    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_GridItemPatternId, UIA_TableItemPatternId, UIA_TextChildPatternId, UIA_SelectionItemPatternId }),
    ...
    EXPECT_EQ(PatternSet({ UIA_ValuePatternId, UIA_ScrollItemPatternId, UIA_InvokePatternId, UIA_TextChildPatternId }),
        GetSupportedPatternsFromNodeId(button_with_value));

    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_InvokePatternId, UIA_TextChildPatternId }), GetSupportedPatternsFromNodeId(button_without_value));
    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_ExpandCollapsePatternId, UIA_TextChildPatternId, UIA_TogglePatternId }),
        GetSupportedPatternsFromNodeId(tree_item_checked_id));
    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_ExpandCollapsePatternId, UIA_TextChildPatternId, UIA_TogglePatternId }),
        GetSupportedPatternsFromNodeId(tree_item_checked_id));
    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_ExpandCollapsePatternId, UIA_TextChildPatternId }), GetSupportedPatternsFromNodeId(tree_item_id));
    EXPECT_EQ(PatternSet({ UIA_SelectionItemPatternId, UIA_ScrollItemPatternId, UIA_TextChildPatternId }), GetSupportedPatternsFromNodeId(tab_id));
    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_ExpandCollapsePatternId, UIA_TextChildPatternId }),
        GetSupportedPatternsFromNodeId(toggle_button_with_popup_id));
    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_TableItemPatternId, UIA_TextChildPatternId }), GetSupportedPatternsFromNodeId(cell_id));

    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_ExpandCollapsePatternId, UIA_TextChildPatternId }),
        GetSupportedPatternsFromNodeId(button_with_expanded_state));

    EXPECT_EQ(
        PatternSet({ UIA_ScrollItemPatternId, UIA_TextChildPatternId, UIA_TogglePatternId }), GetSupportedPatternsFromNodeId(button_without_expanded_state));

    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_TextChildPatternId, UIA_SelectionPatternId, UIA_GridPatternId, UIA_TablePatternId }),
        GetSupportedPatternsFromNodeId(list_grid_id));
    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_TextChildPatternId }), GetSupportedPatternsFromNodeId(list_grid_row_1_id));
    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_TextChildPatternId, UIA_SelectionItemPatternId, UIA_GridItemPatternId, UIA_TableItemPatternId }),
        GetSupportedPatternsFromNodeId(list_grid_column_header_id));
    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_TextChildPatternId }), GetSupportedPatternsFromNodeId(list_grid_row_2_id));
    EXPECT_EQ(PatternSet({ UIA_ScrollItemPatternId, UIA_TextChildPatternId, UIA_ValuePatternId, UIA_SelectionItemPatternId, UIA_GridItemPatternId,
                  UIA_TableItemPatternId }),
...
TEST_F(AXPlatformNodeWinTest, GetPatternProviderExpandCollapsePattern)
{
    ...
    Init(root, list_box, list_item, menu_item, menu_list_option, tree_item, combo_box_grouping, combo_box_menu_button, disclosure_triangle,
        text_field_with_combo_box);

    // list_box HasPopup set to false, does not support expand collapse.
    ComPtr<IRawElementProviderSimple> raw_element_provider_simple = GetIRawElementProviderSimpleFromChildIndex(0);
    ComPtr<IExpandCollapseProvider> expandcollapse_provider;
    EXPECT_HRESULT_SUCCEEDED(raw_element_provider_simple->GetPatternProvider(UIA_ExpandCollapsePatternId, &expandcollapse_provider));
...
TEST_F(AXPlatformNodeWinTest, GetPatternProviderInvokePattern)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(raw_element_provider_simple->GetPatternProvider(UIA_InvokePatternId, &invoke_provider));
...
TEST_F(AXPlatformNodeWinTest, GetPatternProviderGridCellValuePattern)
{
    ...
    cell_6.AddIntAttribute(ax::mojom::IntAttribute::kRestriction, static_cast<int>(ax::mojom::Restriction::kReadOnly));
    ...
    AXPlatformNodeWinTest::PatternSet cell_4_patterns = GetSupportedPatternsFromNodeId(cell_4.id);
    AXPlatformNodeWinTest::PatternSet cell_6_patterns = GetSupportedPatternsFromNodeId(cell_6.id);
    AXPlatformNodeWinTest::PatternSet cell_8_patterns = GetSupportedPatternsFromNodeId(cell_8.id);
...
TEST_F(AXPlatformNodeWinTest, IExpandCollapsePatternProviderAction)
{
    ...
    root.child_ids = { combo_box_grouping_has_popup.id, combo_box_grouping_expanded.id, combo_box_grouping_collapsed.id, combo_box_grouping_disabled.id,
        button_has_popup_menu.id, button_has_popup_menu_pressed.id, button_has_popup_true.id, generic_container_has_popup_menu.id };
    ...
    Init(root, combo_box_grouping_has_popup, combo_box_grouping_disabled, combo_box_grouping_expanded, combo_box_grouping_collapsed,
        combo_box_grouping_disabled, button_has_popup_menu, button_has_popup_menu_pressed, button_has_popup_true, generic_container_has_popup_menu);
    ...
    EXPECT_HRESULT_SUCCEEDED(expandcollapse_provider->get_ExpandCollapseState(&state));
...
TEST_F(AXPlatformNodeWinTest, IExpandCollapseProviderExpandedCollapsedStateMenuButton)
{
    ...
    Init(root, expanded_pressed_button, expanded_unpressed_button, collapsed_pressed_button, collapsed_unpressed_button, pressed_button, unpressed_button);
    ...
    for (size_t i = 0; i < root.child_ids.size(); ++i) {
        ...
        EXPECT_HRESULT_SUCCEEDED(raw_element_provider->GetPatternProvider(UIA_ExpandCollapsePatternId, &expandcollapse_provider));
        EXPECT_NE(expandcollapse_provider.Get(), nullptr);
        EXPECT_HRESULT_SUCCEEDED(expandcollapse_provider->get_ExpandCollapseState(&state));
        SCOPED_TRACE(::testing::Message() << "node index: " << i << ", Actual Expanded/Collapsed State: " << state
...
TEST_F(AXPlatformNodeWinTest, ISelectionItemProviderNotSupported)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(raw_element_provider_simple->GetPatternProvider(UIA_SelectionItemPatternId, &selection_item_provider));
...
TEST_F(AXPlatformNodeWinTest, ISelectionItemProviderDisabled)
{
    AXNodeData root;
    root.id = 1;
    root.AddIntAttribute(ax::mojom::IntAttribute::kRestriction, static_cast<int>(ax::mojom::Restriction::kDisabled));
...
TEST_F(AXPlatformNodeWinTest, ISelectionItemProviderSimple)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(raw_element_provider_simple->GetPatternProvider(UIA_SelectionItemPatternId, &option1_provider));
...
TEST_F(AXPlatformNodeWinTest, ISelectionItemProviderRadioButton)
{
    ...
    AXNodeData option1;
    option1.id = 2;
    option1.AddIntAttribute(ax::mojom::IntAttribute::kCheckedState, static_cast<int>(ax::mojom::CheckedState::kNone));
    ...
    AXNodeData option2;
    option2.id = 3;
    option2.AddIntAttribute(ax::mojom::IntAttribute::kCheckedState, static_cast<int>(ax::mojom::CheckedState::kFalse));
    ...
    AXNodeData option3;
    option3.id = 4;
    option3.AddIntAttribute(ax::mojom::IntAttribute::kCheckedState, static_cast<int>(ax::mojom::CheckedState::kTrue));
    ...
    AXNodeData option4;
    option4.id = 5;
    option4.AddIntAttribute(ax::mojom::IntAttribute::kCheckedState, static_cast<int>(ax::mojom::CheckedState::kMixed));
    ...
    ComPtr<ISelectionItemProvider> option1_provider;
    EXPECT_HRESULT_SUCCEEDED(GetIRawElementProviderSimpleFromChildIndex(0)->GetPatternProvider(UIA_SelectionItemPatternId, &option1_provider));
    ASSERT_EQ(nullptr, option1_provider.Get());

    // Option 2, CheckedState::kFalse.
    ComPtr<ISelectionItemProvider> option2_provider;
    EXPECT_HRESULT_SUCCEEDED(GetIRawElementProviderSimpleFromChildIndex(1)->GetPatternProvider(UIA_SelectionItemPatternId, &option2_provider));
    ...
    EXPECT_HRESULT_SUCCEEDED(GetIRawElementProviderSimpleFromChildIndex(2)->GetPatternProvider(UIA_SelectionItemPatternId, &option3_provider));
    ...
    EXPECT_HRESULT_SUCCEEDED(GetIRawElementProviderSimpleFromChildIndex(3)->GetPatternProvider(UIA_SelectionItemPatternId, &option4_provider));
...
TEST_F(AXPlatformNodeWinTest, ISelectionItemProviderTable)
{
    ...
    ComPtr<ISelectionItemProvider> selection_item_provider;
    EXPECT_HRESULT_SUCCEEDED(GetIRawElementProviderSimpleFromChildIndex(0)->GetPatternProvider(UIA_SelectionItemPatternId, &selection_item_provider));
    ASSERT_EQ(nullptr, selection_item_provider.Get());
}
...
TEST_F(AXPlatformNodeWinTest, ISelectionItemProviderGrid)
{
    ...
    ComPtr<IRawElementProviderSimple> raw_element_provider_simple = QueryInterfaceFromNode<IRawElementProviderSimple>(row->children()[0]);
...
TEST_F(AXPlatformNodeWinTest, ISelectionItemProviderGetSelectionContainer)
{
    ...
    ComPtr<IRawElementProviderSimple> container_provider = GetRootIRawElementProviderSimple();

    const auto* row = GetRoot()->children()[0].get();
    ComPtr<ISelectionItemProvider> item_provider = QueryInterfaceFromNode<ISelectionItemProvider>(row->children()[0]);
...
TEST_F(AXPlatformNodeWinTest, ISelectionItemProviderSelectFollowFocus)
{
    ...
    ComPtr<IRawElementProviderSimple> tab1_raw_element_provider_simple = QueryInterfaceFromNode<IRawElementProviderSimple>(tab1_node);
    ASSERT_NE(nullptr, tab1_raw_element_provider_simple.Get());

    ComPtr<IRawElementProviderFragment> tab1_raw_element_provider_fragment = IRawElementProviderFragmentFromNode(tab1_node);
    ASSERT_NE(nullptr, tab1_raw_element_provider_fragment.Get());

    ComPtr<ISelectionItemProvider> tab1_selection_item_provider;
    EXPECT_HRESULT_SUCCEEDED(tab1_raw_element_provider_simple->GetPatternProvider(UIA_SelectionItemPatternId, &tab1_selection_item_provider));
...
TEST_F(AXPlatformNodeWinTest, IValueProvider_GetValue)
{
    ...
    child3.AddIntAttribute(ax::mojom::IntAttribute::kRestriction, static_cast<int>(ax::mojom::Restriction::kReadOnly));
    root.child_ids.push_back(child3.id);
    ...
    EXPECT_HRESULT_SUCCEEDED(QueryInterfaceFromNode<IValueProvider>(GetRoot())->get_Value(bstr_value.Receive()));
    EXPECT_STREQ(url, base::UTF16ToASCII(base::as_u16cstr(bstr_value.Get())).c_str());
    bstr_value.Reset();

    EXPECT_HRESULT_SUCCEEDED(QueryInterfaceFromNode<IValueProvider>(GetRoot()->children()[0])->get_Value(bstr_value.Receive()));
    EXPECT_STREQ(L"3", bstr_value.Get());
    bstr_value.Reset();

    EXPECT_HRESULT_SUCCEEDED(QueryInterfaceFromNode<IValueProvider>(GetRoot()->children()[1])->get_Value(bstr_value.Receive()));
    EXPECT_STREQ(L"test", bstr_value.Get());
    bstr_value.Reset();

    EXPECT_HRESULT_SUCCEEDED(QueryInterfaceFromNode<IValueProvider>(GetRoot()->children()[2])->get_Value(bstr_value.Receive()));
    EXPECT_STREQ(L"test", bstr_value.Get());
    bstr_value.Reset();

    EXPECT_HRESULT_SUCCEEDED(QueryInterfaceFromNode<IValueProvider>(GetRoot()->children()[3])->get_Value(bstr_value.Receive()));
    EXPECT_STREQ(L"test", bstr_value.Get());
    bstr_value.Reset();
}
...
TEST_F(AXPlatformNodeWinTest, IAccessibleValue)
{
    ...
    ScopedVariant value;
    EXPECT_HRESULT_SUCCEEDED(QueryInterfaceFromNode<IAccessibleValue>(GetRoot()->children()[0])->get_currentValue(value.Receive()));
...
TEST_F(AXPlatformNodeWinTest, IValueProvider_SetValue)
{
    ...
    ComPtr<IValueProvider> root_provider = QueryInterfaceFromNode<IValueProvider>(GetRoot());
    ComPtr<IValueProvider> provider1 = QueryInterfaceFromNode<IValueProvider>(GetRoot()->children()[0]);
    ComPtr<IValueProvider> provider2 = QueryInterfaceFromNode<IValueProvider>(GetRoot()->children()[1]);
    ComPtr<IValueProvider> provider3 = QueryInterfaceFromNode<IValueProvider>(GetRoot()->children()[2]);
...
TEST_F(AXPlatformNodeWinTest, IValueProvider_IsReadOnly)
{
    ...
    child2.role = ax::mojom::Role::kTextField;
    child2.AddState(ax::mojom::State::kEditable);
    child2.AddIntAttribute(ax::mojom::IntAttribute::kRestriction, static_cast<int>(ax::mojom::Restriction::kReadOnly));
    root.child_ids.push_back(child2.id);
    ...
    child3.role = ax::mojom::Role::kTextField;
    child3.AddState(ax::mojom::State::kEditable);
    child3.AddIntAttribute(ax::mojom::IntAttribute::kRestriction, static_cast<int>(ax::mojom::Restriction::kDisabled));
    root.child_ids.push_back(child3.id);
    ...
    EXPECT_HRESULT_SUCCEEDED(QueryInterfaceFromNode<IValueProvider>(GetRoot())->get_IsReadOnly(&is_readonly));
    EXPECT_TRUE(is_readonly);

    EXPECT_HRESULT_SUCCEEDED(QueryInterfaceFromNode<IValueProvider>(GetRoot()->children()[0])->get_IsReadOnly(&is_readonly));
    EXPECT_FALSE(is_readonly);

    EXPECT_HRESULT_SUCCEEDED(QueryInterfaceFromNode<IValueProvider>(GetRoot()->children()[1])->get_IsReadOnly(&is_readonly));
    EXPECT_TRUE(is_readonly);

    EXPECT_HRESULT_SUCCEEDED(QueryInterfaceFromNode<IValueProvider>(GetRoot()->children()[2])->get_IsReadOnly(&is_readonly));
    EXPECT_TRUE(is_readonly);

    EXPECT_HRESULT_SUCCEEDED(QueryInterfaceFromNode<IValueProvider>(GetRoot()->children()[3])->get_IsReadOnly(&is_readonly));
    EXPECT_TRUE(is_readonly);
}
...
TEST_F(AXPlatformNodeWinTest, IScrollProviderSetScrollPercent)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(scroll_provider->get_HorizontalScrollPercent(&x_scroll_percent));
    EXPECT_EQ(x_scroll_percent, 0);
    EXPECT_HRESULT_SUCCEEDED(scroll_provider->get_VerticalScrollPercent(&y_scroll_percent));
...
TEST_F(AXPlatformNodeWinTest, AriaRoleForInsertionAndDeletion)
{
    ...
    EXPECT_UIA_BSTR_EQ(GetIRawElementProviderSimpleFromChildIndex(0), UIA_AriaRolePropertyId, L"insertion");
    EXPECT_UIA_BSTR_EQ(GetIRawElementProviderSimpleFromChildIndex(1), UIA_AriaRolePropertyId, L"deletion");
}
...
class TestIChromeAccessibleDelegate : public SequenceAffineComObjectRoot, public IDispatchImpl<IChromeAccessibleDelegate> {
...
TEST_F(AXPlatformNodeWinTest, DISABLED_BulkFetch)
{
    ...
    ComPtr<IChromeAccessible> chrome_accessible = QueryInterfaceFromNode<IChromeAccessible>(GetRoot());

    CComObject<TestIChromeAccessibleDelegate>* delegate = nullptr;
    ASSERT_HRESULT_SUCCEEDED(CComObject<TestIChromeAccessibleDelegate>::CreateInstance(&delegate));
    ...
    std::optional<base::Value> result_val = base::JSONReader::Read(response, base::JSON_ALLOW_TRAILING_COMMAS);
...
TEST_F(AXPlatformNodeWinTest, AsyncHitTest)
{
    ...
    ComPtr<IAccessible2> root_accessible = QueryInterfaceFromNode<IAccessible2>(GetRoot());
