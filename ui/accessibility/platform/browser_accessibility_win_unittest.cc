...
#define EXPECT_IA2_TEXT_AT_OFFSET(provider, index, text_boundary, expected_hr, start, end, text)                                                               \
    {                                                                                                                                                          \
        LONG actual_start;                                                                                                                                     \
        LONG actual_end;                                                                                                                                       \
        base::win::ScopedBstr actual_text;                                                                                                                     \
        EXPECT_EQ(expected_hr, provider->get_textAtOffset(index, text_boundary, &actual_start, &actual_end, actual_text.Receive()));                           \
        EXPECT_EQ(start, actual_start);                                                                                                                        \
        EXPECT_EQ(end, actual_end);                                                                                                                            \
        EXPECT_STREQ(text, actual_text.Get());                                                                                                                 \
    }

#define EXPECT_IA2_TEXT_BEFORE_OFFSET(provider, index, text_boundary, expected_hr, start, end, text)                                                           \
    {                                                                                                                                                          \
        LONG actual_start;                                                                                                                                     \
        LONG actual_end;                                                                                                                                       \
        base::win::ScopedBstr actual_text;                                                                                                                     \
        EXPECT_EQ(expected_hr, provider->get_textBeforeOffset(index, text_boundary, &actual_start, &actual_end, actual_text.Receive()));                       \
        EXPECT_EQ(start, actual_start);                                                                                                                        \
        EXPECT_EQ(end, actual_end);                                                                                                                            \
        EXPECT_STREQ(text, actual_text.Get());                                                                                                                 \
    }

#define EXPECT_IA2_TEXT_AFTER_OFFSET(provider, index, text_boundary, expected_hr, start, end, text)                                                            \
    {                                                                                                                                                          \
        LONG actual_start;                                                                                                                                     \
        LONG actual_end;                                                                                                                                       \
        base::win::ScopedBstr actual_text;                                                                                                                     \
        EXPECT_EQ(expected_hr, provider->get_textAfterOffset(index, text_boundary, &actual_start, &actual_end, actual_text.Receive()));                        \
        EXPECT_EQ(start, actual_start);                                                                                                                        \
        EXPECT_EQ(end, actual_end);                                                                                                                            \
        EXPECT_STREQ(text, actual_text.Get());                                                                                                                 \
    }

#define EXPECT_NATIVE_VIEW_ACCESSIBLE_VECTOR_EQ(descendants, expected_descendants)                                                                             \
    {                                                                                                                                                          \
        size_t count = descendants.size();                                                                                                                     \
        EXPECT_EQ(count, expected_descendants.size());                                                                                                         \
        for (size_t i = 0; i < count; ++i) {                                                                                                                   \
            EXPECT_EQ(AXPlatformNode::FromNativeViewAccessible(descendants[i])->GetDelegate()->GetData().ToString(),                                           \
                AXPlatformNode::FromNativeViewAccessible(expected_descendants[i])->GetDelegate()->GetData().ToString());                                       \
        }                                                                                                                                                      \
    }
...
public:
    ...
    BrowserAccessibilityWinTest& operator=(const BrowserAccessibilityWinTest&) = delete;

    ~BrowserAccessibilityWinTest() override;

protected:
    std::unique_ptr<TestAXPlatformTreeManagerDelegate> test_browser_accessibility_delegate_;
...
void BrowserAccessibilityWinTest::SetUp()
{
    ...
    test_browser_accessibility_delegate_ = std::make_unique<TestAXPlatformTreeManagerDelegate>();
}
...
TEST_F(BrowserAccessibilityWinTest, TestNoLeaks)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root, button, checkbox), node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    manager.reset(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root, button, checkbox), node_id_delegate_, test_browser_accessibility_delegate_.get()));
    IAccessible* root_accessible = ToBrowserAccessibilityWin(manager->GetBrowserAccessibilityRoot())->GetCOM();
...
TEST_F(BrowserAccessibilityWinTest, TestChildrenChange)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root, text), node_id_delegate_, test_browser_accessibility_delegate_.get()));

    // Query for the text IAccessible and verify that it returns "old text" as its
    // value.
    base::win::ScopedVariant one(1);
    Microsoft::WRL::ComPtr<IDispatch> text_dispatch;
    HRESULT hr = ToBrowserAccessibilityWin(manager->GetBrowserAccessibilityRoot())->GetCOM()->get_accChild(one, &text_dispatch);
    ASSERT_EQ(S_OK, hr);
    ...
    // as its value.
    hr = ToBrowserAccessibilityWin(manager->GetBrowserAccessibilityRoot())->GetCOM()->get_accChild(one, &text_dispatch);
    ASSERT_EQ(S_OK, hr);
...
TEST_F(BrowserAccessibilityWinTest, TestChildrenChangeNoLeaks)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(BrowserAccessibilityManager::Create(
        MakeAXTreeUpdateForTesting(root, div, text3, text4), node_id_delegate_, test_browser_accessibility_delegate_.get()));
...
TEST_F(BrowserAccessibilityWinTest, TestTextBoundaries)
{
    ...
    text_field.AddIntListAttribute(ax::mojom::IntListAttribute::kLineStarts, { 15 });
    text_field.child_ids = { text_container.id };

    text_container.role = ax::mojom::Role::kGenericContainer;
    text_container.child_ids = { static_text1.id, line_break1.id, static_text2.id, line_break2.id, static_text3.id };
    ...
    inline_box1.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, { 0, 4, 8 });

    line_break1.role = ax::mojom::Role::kLineBreak;
    line_break1.AddState(ax::mojom::State::kEditable);
    line_break1.SetName("\n");

    inline_box1.AddIntAttribute(ax::mojom::IntAttribute::kNextOnLineId, line_break1.id);
    line_break1.AddIntAttribute(ax::mojom::IntAttribute::kPreviousOnLineId, inline_box1.id);
    ...
    inline_box2.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, { 0, 5, 10 });

    line_break2.role = ax::mojom::Role::kLineBreak;
    line_break2.AddState(ax::mojom::State::kEditable);
    line_break2.SetName("\n");
    line_break2.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);

    inline_box2.AddIntAttribute(ax::mojom::IntAttribute::kNextOnLineId, line_break2.id);
    line_break2.AddIntAttribute(ax::mojom::IntAttribute::kPreviousOnLineId, inline_box2.id);
    ...
    inline_box3.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, { 0, 6, 12 });

    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root, text_field, text_container, static_text1, inline_box1, line_break1, static_text2,
                                                inline_box2, line_break2, static_text3, inline_box3),
            node_id_delegate_, test_browser_accessibility_delegate_.get()));

    BrowserAccessibilityWin* root_obj = ToBrowserAccessibilityWin(manager->GetBrowserAccessibilityRoot());
    ASSERT_NE(nullptr, root_obj);
    ASSERT_EQ(1U, root_obj->PlatformChildCount());

    BrowserAccessibilityComWin* text_field_obj = ToBrowserAccessibilityComWin(root_obj->PlatformGetChild(0));
    ...
    EXPECT_IA2_TEXT_AT_OFFSET(text_field_obj, text_len - 1, IA2_TEXT_BOUNDARY_WORD,
    ...
    EXPECT_IA2_TEXT_AT_OFFSET(text_field_obj, text_len - 1, IA2_TEXT_BOUNDARY_PARAGRAPH,
        /*expected_hr=*/S_OK, /*start=*/30, /*end=*/47,
        /*text=*/L"Seven eight nine.");

    EXPECT_IA2_TEXT_AT_OFFSET(text_field_obj, text_len, IA2_TEXT_BOUNDARY_PARAGRAPH,
        /*expected_hr=*/E_INVALIDARG, /*start=*/0,
        /*end=*/0,
        /*text=*/nullptr);
    ...
    EXPECT_IA2_TEXT_BEFORE_OFFSET(text_field_obj, text_len, IA2_TEXT_BOUNDARY_CHAR,
        /*expected_hr=*/E_INVALIDARG, /*start=*/0, /*end=*/0,
        /*text=*/nullptr);

    EXPECT_IA2_TEXT_BEFORE_OFFSET(text_field_obj, text_len, IA2_TEXT_BOUNDARY_WORD,
        /*expected_hr=*/E_INVALIDARG, /*start=*/0, /*end=*/0,
        /*text=*/nullptr);
    ...
    EXPECT_IA2_TEXT_BEFORE_OFFSET(text_field_obj, text_len - 1, IA2_TEXT_BOUNDARY_WORD,
    ...
    EXPECT_IA2_TEXT_BEFORE_OFFSET(text_field_obj, text_len - 1, IA2_TEXT_BOUNDARY_LINE,
        /*expected_hr=*/S_OK, /*start=*/15, /*end=*/30,
        /*text=*/L"Four five six.\n");
    ...
    EXPECT_IA2_TEXT_BEFORE_OFFSET(text_field_obj, text_len - 1, IA2_TEXT_BOUNDARY_PARAGRAPH,
        /*expected_hr=*/S_OK, /*start=*/15, /*end=*/30,
        /*text=*/L"Four five six.\n");
    ...
    EXPECT_IA2_TEXT_AFTER_OFFSET(text_field_obj, text_len - 1, IA2_TEXT_BOUNDARY_LINE,
        /*expected_hr=*/S_FALSE, /*start=*/0, /*end=*/0,
        /*text=*/nullptr);
    ...
    EXPECT_IA2_TEXT_AFTER_OFFSET(text_field_obj, text_len - 1, IA2_TEXT_BOUNDARY_PARAGRAPH,
        /*expected_hr=*/S_FALSE, /*start=*/0, /*end=*/0,
        /*text=*/nullptr);
...
TEST_F(BrowserAccessibilityWinTest, TestSimpleHypertext)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root, text1, text2), node_id_delegate_, test_browser_accessibility_delegate_.get()));

    BrowserAccessibilityComWin* root_obj = ToBrowserAccessibilityWin(manager->GetBrowserAccessibilityRoot())->GetCOM();
    ...
    EXPECT_EQ(text1_name + text2_name, base::WideToUTF8(std::wstring(text.Get())));
    ...
    EXPECT_EQ(E_INVALIDARG, root_obj->get_hyperlink(text_name_len + 1, &hyperlink));
    ...
    hyperlink_index = -2;
    EXPECT_EQ(E_INVALIDARG, root_obj->get_hyperlinkIndex(text_name_len, &hyperlink_index));
    EXPECT_EQ(-2, hyperlink_index);
    EXPECT_EQ(E_INVALIDARG, root_obj->get_hyperlinkIndex(-1, &hyperlink_index));
    EXPECT_EQ(-2, hyperlink_index);
    EXPECT_EQ(E_INVALIDARG, root_obj->get_hyperlinkIndex(text_name_len + 1, &hyperlink_index));
    EXPECT_EQ(-2, hyperlink_index);
...
TEST_F(BrowserAccessibilityWinTest, TestComplexHypertext)
{
    ...
    const std::u16string root_hypertext = text1_name + embed + text2_name + embed + embed + embed;
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root, text1, combo_box, text2, check_box, button, button_text, link, link_text),
            node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    EXPECT_EQ(S_OK, hypertext->get_text(0, IA2_TEXT_OFFSET_LENGTH, text.Receive()));
    ...
    // Invalid arguments should not be modified.
    hyperlink_index = -2;
    EXPECT_EQ(E_INVALIDARG, root_obj->get_hyperlinkIndex(root_hypertext_len, &hyperlink_index));
    EXPECT_EQ(-2, hyperlink_index);
...
TEST_F(BrowserAccessibilityWinTest, TestGetUIADirectChildrenInRange)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(BrowserAccessibilityManager::Create(
        MakeAXTreeUpdateForTesting(root, para1, text1, text2, link1, text3, link2, text4, button, para2, text5, image, text6), node_id_delegate_,
    ...
    expected_descendants = { link2_obj->GetNativeViewAccessible(), button_obj->GetNativeViewAccessible() };
    EXPECT_NATIVE_VIEW_ACCESSIBLE_VECTOR_EQ(descendants, expected_descendants);
...
{
    std::unique_ptr<BrowserAccessibilityManager> manager(
        new BrowserAccessibilityManagerWin(BrowserAccessibilityManagerWin::GetEmptyDocument(), node_id_delegate_, test_browser_accessibility_delegate_.get()));
...
TEST_F(BrowserAccessibilityWinTest, EmptyDocHasUniqueIdWin)
{
    ...
    AXPlatformNode* node = static_cast<AXPlatformNode*>(AXPlatformNodeWin::GetFromUniqueId(GetUniqueId(win_root)));
...
TEST_F(BrowserAccessibilityWinTest, TestIA2Attributes)
{
    ...
    pseudo_before.AddStringAttribute(ax::mojom::StringAttribute::kHtmlTag, "<pseudo:before>");
    pseudo_before.AddStringAttribute(ax::mojom::StringAttribute::kDisplay, "none");
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(BrowserAccessibilityManager::Create(
        MakeAXTreeUpdateForTesting(root, pseudo_before, checkbox), node_id_delegate_, test_browser_accessibility_delegate_.get()));

    ASSERT_NE(nullptr, manager->GetBrowserAccessibilityRoot());
    BrowserAccessibilityWin* root_accessible = ToBrowserAccessibilityWin(manager->GetBrowserAccessibilityRoot());
    ...
    HRESULT hr = pseudo_accessible->GetCOM()->get_attributes(attributes.Receive());
    EXPECT_EQ(S_OK, hr);
    ...
    EXPECT_EQ(L"checkable:true;name-from:attribute;explicit-name:true;", attributes_str);
...
TEST_F(BrowserAccessibilityWinTest, TestValueAttributeInTextControls)
{
    ...
    root.child_ids = { combo_box.id, search_box.id, text_field.id, link.id, slider.id };

    std::unique_ptr<BrowserAccessibilityManager> manager(BrowserAccessibilityManager::Create(
        MakeAXTreeUpdateForTesting(root, combo_box, combo_box_text_container, combo_box_text, search_box, search_box_text_container, search_box_text, new_line,
            text_field, text_field_text_container, link, link_text, slider, slider_text),
        node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    ASSERT_EQ(combo_box_accessible, ToBrowserAccessibilityWin(manager->GetFocus()));
    ...
    HRESULT hr = combo_box_accessible->GetCOM()->get_accValue(childid_self, value.Receive());
    EXPECT_EQ(S_OK, hr);
    EXPECT_STREQ(L"Combo box text", value.Get());
    value.Reset();
    hr = search_box_accessible->GetCOM()->get_accValue(childid_self, value.Receive());
    EXPECT_EQ(S_OK, hr);
    EXPECT_STREQ(L"Search box text\n", value.Get());
    value.Reset();
    hr = text_field_accessible->GetCOM()->get_accValue(childid_self, value.Receive());
    EXPECT_EQ(S_OK, hr);
...
TEST_F(BrowserAccessibilityWinTest, TestWordBoundariesInTextControls)
{
    ...
    textarea_line1.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, line1_word_starts);
    textarea_line2.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, line2_word_starts);
    ...
    text_field_line.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, line1_word_starts);
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root, textarea, textarea_div, textarea_text, textarea_line1, textarea_line2, text_field,
    ...
    EXPECT_HRESULT_SUCCEEDED(textarea_accessible->GetCOM()->QueryInterface(IID_PPV_ARGS(&textarea_object)));
    Microsoft::WRL::ComPtr<IAccessibleText> text_field_object;
    EXPECT_HRESULT_SUCCEEDED(text_field_accessible->GetCOM()->QueryInterface(IID_PPV_ARGS(&text_field_object)));

    LONG offset = 0;
    while (offset < static_cast<LONG>(text.length())) {
        LONG start, end;
        base::win::ScopedBstr word;
        EXPECT_EQ(S_OK, textarea_object->get_textAtOffset(offset, IA2_TEXT_BOUNDARY_WORD, &start, &end, word.Receive()));
...
TEST_F(BrowserAccessibilityWinTest, TextBoundariesOnlyEmbeddedObjectsNoCrash)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root_data, menu_data, button_1_data, button_2_data, static_text_data), node_id_delegate_,
            test_browser_accessibility_delegate_.get()));
    ...
    BrowserAccessibilityComWin* menu_accessible_com = ToBrowserAccessibilityComWin(root_accessible->PlatformGetChild(0));
...
TEST_F(BrowserAccessibilityWinTest, DISABLED_TestTextBoundariesEmbeddedCharacterText)
{
    ...
    body_data.child_ids = { static_text_1_data.id, menu_data_1.id, menu_data_2.id, static_text_2_data.id, static_text_3_data.id };
    menu_data_1.child_ids = { button_data.id };
    button_data.child_ids = { button_leaf_container_data.id, button_leaf_svg_data.id };
    ...
    update.nodes = { root_data, body_data, static_text_1_data, inline_text_1_data, menu_data_1, button_data, button_leaf_container_data, button_leaf_svg_data,
        menu_data_2, static_text_2_data, inline_text_2_data, static_text_3_data, inline_text_3_data };

    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(update, node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    BrowserAccessibilityWin* body_accessible = ToBrowserAccessibilityWin(root_accessible->PlatformGetChild(0));
    ...
    BrowserAccessibilityComWin* static_text_1_com = ToBrowserAccessibilityWin(body_accessible->PlatformGetChild(0))->GetCOM();
    ASSERT_NE(nullptr, static_text_1_com);
    ASSERT_EQ(ax::mojom::Role::kStaticText, static_text_1_com->GetRole());

    BrowserAccessibilityComWin* menu_1_accessible_com = ToBrowserAccessibilityWin(body_accessible->PlatformGetChild(1))->GetCOM();
    ASSERT_NE(nullptr, menu_1_accessible_com);
    ASSERT_EQ(ax::mojom::Role::kMenu, menu_1_accessible_com->GetRole());

    BrowserAccessibilityComWin* menu_2_accessible_com = ToBrowserAccessibilityWin(body_accessible->PlatformGetChild(2))->GetCOM();
    ASSERT_NE(nullptr, menu_2_accessible_com);
    ASSERT_EQ(ax::mojom::Role::kMenu, menu_2_accessible_com->GetRole());

    BrowserAccessibilityComWin* static_text_2_com = ToBrowserAccessibilityWin(body_accessible->PlatformGetChild(3))->GetCOM();
    ASSERT_NE(nullptr, static_text_2_com);
    ASSERT_EQ(ax::mojom::Role::kStaticText, static_text_2_com->GetRole());

    BrowserAccessibilityComWin* static_text_3_com = ToBrowserAccessibilityWin(body_accessible->PlatformGetChild(4))->GetCOM();
    ...
    // L"before" [obj] <[obj]> L"<a>fter" L"tail"
    EXPECT_IA2_TEXT_BEFORE_OFFSET(body_accessible_com, 7, IA2_TEXT_BOUNDARY_CHAR,
        /*expected_hr=*/S_OK, /*start=*/6, /*end=*/7,
        /*text=*/
        base::as_wcstr(std::u16string { AXPlatformNodeBase::kEmbeddedCharacter }));
    ...
    // <[obj]>
    EXPECT_IA2_TEXT_BEFORE_OFFSET(menu_1_accessible_com, 0, IA2_TEXT_BOUNDARY_CHAR,
        /*expected_hr=*/S_FALSE, /*start=*/0, /*end=*/0,
        /*text=*/nullptr);

    // L"<>"
    EXPECT_IA2_TEXT_BEFORE_OFFSET(menu_2_accessible_com, 0, IA2_TEXT_BOUNDARY_CHAR,
        /*expected_hr=*/E_INVALIDARG, /*start=*/0,
        /*end=*/0,
        /*text=*/nullptr);
...
TEST_F(BrowserAccessibilityWinTest, TestCaretAndSelectionInSimpleFields)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(BrowserAccessibilityManager::Create(
        MakeAXTreeUpdateForTesting(root, combo_box, text_field), node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    manager->ax_tree()->UpdateDataForTesting(data);
    ASSERT_EQ(text_field_accessible, ToBrowserAccessibilityWin(manager->GetFocus()));
    ...
    // Test get_selection.
    hr = combo_box_accessible->GetCOM()->get_selection(0L /* selection_index */, &selection_start, &selection_end);
    EXPECT_EQ(E_INVALIDARG, hr); // No selections available.
    hr = text_field_accessible->GetCOM()->get_selection(0L /* selection_index */, &selection_start, &selection_end);
    EXPECT_EQ(S_OK, hr);
...
TEST_F(BrowserAccessibilityWinTest, TestCaretInContentEditables)
{
    ...
    div_editable.AddBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot, true);
    ...
    AXTreeUpdate update = MakeAXTreeUpdateForTesting(root, div_editable, link, link_text, text);
    ...
    HRESULT hr = div_editable_accessible->GetCOM()->get_nSelections(&n_selections);
    EXPECT_EQ(S_OK, hr);
    EXPECT_EQ(0, n_selections);
    ...
    manager->ax_tree()->UpdateDataForTesting(data);
    ASSERT_EQ(div_editable_accessible, ToBrowserAccessibilityWin(manager->GetFocus()));
...
TEST_F(BrowserAccessibilityWinTest, TestSelectionInContentEditables)
{
    ...
    hr = div_editable_accessible->GetCOM()->get_selection(0L /* selection_index */, &selection_start, &selection_end);
    EXPECT_EQ(S_OK, hr);
    EXPECT_EQ(1, selection_start);
    // selection_end should be after embedded object character.
    EXPECT_EQ(7, selection_end);

    hr = text_accessible->GetCOM()->get_selection(0L /* selection_index */, &selection_start, &selection_end);
    EXPECT_EQ(S_OK, hr);
    EXPECT_EQ(1, selection_start);
    // No embedded character on this object, only the first part of the text.
    EXPECT_EQ(6, selection_end);
    hr = link_accessible->GetCOM()->get_selection(0L /* selection_index */, &selection_start, &selection_end);
    EXPECT_EQ(S_OK, hr);
    EXPECT_EQ(0, selection_start);
    EXPECT_EQ(4, selection_end);
    hr = link_text_accessible->GetCOM()->get_selection(0L /* selection_index */, &selection_start, &selection_end);
    EXPECT_EQ(S_OK, hr);
    EXPECT_EQ(0, selection_start);
    EXPECT_EQ(4, selection_end);
...
TEST_F(BrowserAccessibilityWinTest, TestIAccessibleHyperlink)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root, div, link, text), node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    EXPECT_HRESULT_FAILED(root_accessible->GetCOM()->QueryInterface(IID_PPV_ARGS(&hyperlink)));
    hyperlink.Reset();
    EXPECT_HRESULT_SUCCEEDED(div_accessible->GetCOM()->QueryInterface(IID_PPV_ARGS(&hyperlink)));
    hyperlink.Reset();
    EXPECT_HRESULT_FAILED(text_accessible->GetCOM()->QueryInterface(IID_PPV_ARGS(&hyperlink)));
    hyperlink.Reset();
    EXPECT_HRESULT_SUCCEEDED(link_accessible->GetCOM()->QueryInterface(IID_PPV_ARGS(&hyperlink)));
    hyperlink.Reset();
    ...
    EXPECT_HRESULT_FAILED(root_accessible->GetCOM()->get_anchor(0, anchor.Receive()));
    anchor.Reset();
    ...
    bstr.Reset();
    anchor.Reset();
    EXPECT_HRESULT_FAILED(text_accessible->GetCOM()->get_anchor(0, anchor.Receive()));
    anchor.Reset();
    ...
    bstr.Reset();
    anchor.Reset();
    EXPECT_HRESULT_FAILED(div_accessible->GetCOM()->get_anchor(1, anchor.Receive()));
    anchor.Reset();
    EXPECT_HRESULT_FAILED(link_accessible->GetCOM()->get_anchor(1, anchor.Receive()));
    anchor.Reset();

    EXPECT_HRESULT_FAILED(root_accessible->GetCOM()->get_anchorTarget(0, anchor_target.Receive()));
    anchor_target.Reset();
    ...
    bstr.Reset();
    anchor_target.Reset();
    EXPECT_HRESULT_FAILED(text_accessible->GetCOM()->get_anchorTarget(0, anchor_target.Receive()));
    anchor_target.Reset();
    ...
    bstr.Reset();
    anchor_target.Reset();
    EXPECT_HRESULT_FAILED(div_accessible->GetCOM()->get_anchorTarget(1, anchor_target.Receive()));
    anchor_target.Reset();
    EXPECT_HRESULT_FAILED(link_accessible->GetCOM()->get_anchorTarget(1, anchor_target.Receive()));
    anchor_target.Reset();

    EXPECT_HRESULT_FAILED(root_accessible->GetCOM()->get_startIndex(&start_index));
    EXPECT_HRESULT_SUCCEEDED(div_accessible->GetCOM()->get_startIndex(&start_index));
    EXPECT_EQ(0, start_index);
    EXPECT_HRESULT_FAILED(text_accessible->GetCOM()->get_startIndex(&start_index));
    EXPECT_HRESULT_SUCCEEDED(link_accessible->GetCOM()->get_startIndex(&start_index));
...
TEST_F(BrowserAccessibilityWinTest, TestTextAttributesInContentEditables)
{
    ...
    div_editable.AddBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot, true);
    div_editable.AddState(ax::mojom::State::kFocusable);
    div_editable.AddStringAttribute(ax::mojom::StringAttribute::kFontFamily, "Helvetica");
    ...
    std::vector<int32_t> marker_types { static_cast<int32_t>(ax::mojom::MarkerType::kSpelling) };
    std::vector<int32_t> marker_starts { 0 };
    std::vector<int32_t> marker_ends { 3 };
    link_text.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerTypes, marker_types);
    link_text.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerStarts, marker_starts);
    link_text.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerEnds, marker_ends);
    ...
    AXTreeUpdate update = MakeAXTreeUpdateForTesting(root, div_editable, text_before, link, link_text, text_after);

    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(update, node_id_delegate_, test_browser_accessibility_delegate_.get()));

    ASSERT_NE(nullptr, manager->GetBrowserAccessibilityRoot());
    BrowserAccessibilityWin* ax_root = ToBrowserAccessibilityWin(manager->GetBrowserAccessibilityRoot());
    ...
    hr = ax_root->GetCOM()->get_attributes(0, &start_offset, &end_offset, text_attributes.Receive());
    EXPECT_EQ(S_OK, hr);
    EXPECT_EQ(0, start_offset);
    EXPECT_EQ(1, end_offset);
    EXPECT_NE(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"font-family:Helvetica"));
    text_attributes.Reset();

    // Test the style of text_before.
    for (LONG offset = 0; offset < 7; ++offset) {
        hr = ax_div->GetCOM()->get_attributes(0, &start_offset, &end_offset, text_attributes.Receive());
        EXPECT_EQ(S_OK, hr);
        EXPECT_EQ(0, start_offset);
    ...
    hr = ax_link->GetCOM()->get_attributes(0, &start_offset, &end_offset, text_attributes.Receive());
    EXPECT_EQ(S_OK, hr);
    EXPECT_EQ(0, start_offset);
    EXPECT_EQ(3, end_offset);
    EXPECT_NE(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"font-family:Helvetica"));
    EXPECT_EQ(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"font-weight:"));
    EXPECT_EQ(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"font-style:"));
    EXPECT_NE(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"text-underline-style:solid"));
    EXPECT_EQ(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"text-underline-type:"));
    // For compatibility with Firefox, spelling attributes should also be
    // propagated to the parent of static text leaves.
    EXPECT_NE(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"invalid:spelling"));
    text_attributes.Reset();

    hr = ax_link_text->GetCOM()->get_attributes(2, &start_offset, &end_offset, text_attributes.Receive());
    EXPECT_EQ(S_OK, hr);
    EXPECT_EQ(0, start_offset);
    EXPECT_EQ(3, end_offset);
    EXPECT_NE(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"font-family:Helvetica"));
    ...
    for (LONG offset = 8; offset < 15; ++offset) {
        hr = ax_div->GetCOM()->get_attributes(offset, &start_offset, &end_offset, text_attributes.Receive());
        EXPECT_EQ(S_OK, hr);
        ...
        EXPECT_EQ(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"text-underline-style:solid"));
        EXPECT_EQ(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"text-underline-type:"));
    ...
    hr = ax_before->GetCOM()->get_attributes(6, &start_offset, &end_offset, text_attributes.Receive());
    EXPECT_EQ(S_OK, hr);
    EXPECT_EQ(0, start_offset);
    EXPECT_EQ(7, end_offset);
    EXPECT_NE(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"font-family:Helvetica"));
    EXPECT_NE(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"font-weight:bold"));
    EXPECT_NE(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"font-style:italic"));
    EXPECT_EQ(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"invalid:spelling"));
    text_attributes.Reset();

    hr = ax_after->GetCOM()->get_attributes(6, &start_offset, &end_offset, text_attributes.Receive());
    EXPECT_EQ(S_OK, hr);
    EXPECT_EQ(0, start_offset);
    EXPECT_EQ(7, end_offset);
    EXPECT_NE(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"font-family:Helvetica"));
    EXPECT_EQ(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"font-weight:"));
    EXPECT_EQ(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"font-style:"));
    EXPECT_EQ(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"text-underline-style:"));
    EXPECT_EQ(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"text-underline-type:"));
    EXPECT_EQ(std::wstring::npos, std::wstring(text_attributes.Get()).find(L"invalid:spelling"));
    text_attributes.Reset();
...
TEST_F(BrowserAccessibilityWinTest, TestExistingMisspellingsInSimpleTextFields)
{
    ...
    marker_types.push_back(static_cast<int32_t>(ax::mojom::MarkerType::kSpelling));
    std::vector<int32_t> marker_starts;
    marker_starts.push_back(0);
    std::vector<int32_t> marker_ends;
    marker_ends.push_back(4);
    static_text2.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerTypes, marker_types);
    static_text2.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerStarts, marker_starts);
    static_text2.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerEnds, marker_ends);
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(BrowserAccessibilityManager::Create(
        MakeAXTreeUpdateForTesting(root, combo_box, combo_box_div, static_text1, static_text2), node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    for (LONG offset = 0; offset < value1_length; ++offset) {
        hr = ax_combo_box->GetCOM()->get_attributes(offset, &start_offset, &end_offset, text_attributes.Receive());
    ...
    for (LONG offset = value1_length + 4; offset < combo_box_value_length; ++offset) {
        hr = ax_combo_box->GetCOM()->get_attributes(offset, &start_offset, &end_offset, text_attributes.Receive());
        EXPECT_TRUE(std::wstring(text_attributes.Get()).empty());
...
TEST_F(BrowserAccessibilityWinTest, TestDeepestFirstLastChild)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(BrowserAccessibilityManager::Create(
        MakeAXTreeUpdateForTesting(root, child1, child2, child2_child1, child2_child2), node_id_delegate_, test_browser_accessibility_delegate_.get()));

    BrowserAccessibility* root_accessible = manager->GetBrowserAccessibilityRoot();
    ASSERT_NE(nullptr, root_accessible);
    ASSERT_EQ(2U, root_accessible->PlatformChildCount());
    ...
    EXPECT_EQ(child2_child2_accessible, root_accessible->InternalDeepestLastChild());
    ...
    EXPECT_EQ(child2_child1_accessible, child2_accessible->InternalDeepestFirstChild());

    EXPECT_EQ(nullptr, child2_accessible->PlatformDeepestLastChild());
    EXPECT_EQ(child2_child2_accessible, child2_accessible->InternalDeepestLastChild());
...
TEST_F(BrowserAccessibilityWinTest, TestInheritedStringAttributes)
{
    ...
    EXPECT_EQ(u"en-US", root_accessible->GetInheritedString16Attribute(ax::mojom::StringAttribute::kLanguage));
    EXPECT_EQ(u"en-US", child1_accessible->GetInheritedString16Attribute(ax::mojom::StringAttribute::kLanguage));
    EXPECT_EQ(u"fr", child2_accessible->GetInheritedString16Attribute(ax::mojom::StringAttribute::kLanguage));
    EXPECT_EQ(u"fr", child2_child1_accessible->GetInheritedString16Attribute(ax::mojom::StringAttribute::kLanguage));
    EXPECT_EQ(u"fr", child2_child2_accessible->GetInheritedString16Attribute(ax::mojom::StringAttribute::kLanguage));

    // Test GetInheritedStringAttribute(attribute).
    EXPECT_EQ("Helvetica", root_accessible->GetInheritedStringAttribute(ax::mojom::StringAttribute::kFontFamily));
    EXPECT_EQ("Helvetica", child1_accessible->GetInheritedStringAttribute(ax::mojom::StringAttribute::kFontFamily));
    EXPECT_EQ("Arial", child2_accessible->GetInheritedStringAttribute(ax::mojom::StringAttribute::kFontFamily));
    EXPECT_EQ("Arial", child2_child1_accessible->GetInheritedStringAttribute(ax::mojom::StringAttribute::kFontFamily));
    EXPECT_EQ("Arial", child2_child2_accessible->GetInheritedStringAttribute(ax::mojom::StringAttribute::kFontFamily));
}
...
TEST_F(BrowserAccessibilityWinTest, AccChildOnlyReturnsDescendants)
{
    ...
    std::unique_ptr<BrowserAccessibilityManagerWin> manager(
        new BrowserAccessibilityManagerWin(MakeAXTreeUpdateForTesting(root_node, child_node), node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    EXPECT_EQ(E_INVALIDARG, ToBrowserAccessibilityWin(child)->GetCOM()->get_accChild(root_unique_id_variant, &result));

    base::win::ScopedVariant child_unique_id_variant(-GetUniqueId(child));
    EXPECT_EQ(S_OK, ToBrowserAccessibilityWin(root)->GetCOM()->get_accChild(child_unique_id_variant, &result));
}
...
TEST_F(BrowserAccessibilityWinTest, DISABLED_TestIAccessible2Relations)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root, child1, child2), node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    EXPECT_HRESULT_SUCCEEDED(ax_root->GetCOM()->get_relation(0, &describedby_relation));
    ...
    EXPECT_HRESULT_SUCCEEDED(ax_child1->GetCOM()->get_relation(0, &description_for_relation));
    ...
    EXPECT_HRESULT_SUCCEEDED(ax_child2->GetCOM()->get_relation(0, &description_for_relation));
    EXPECT_HRESULT_SUCCEEDED(description_for_relation->get_relationType(relation_type.Receive()));
    EXPECT_EQ(L"descriptionFor", std::wstring(relation_type.Get()));
    relation_type.Reset();
    ...
    child1.AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, labelledby_ids);
    AXUpdatesAndEvents event_bundle;
    event_bundle.updates.resize(1);
