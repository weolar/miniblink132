...
const char* GetNameForPlatformConstant(const PlatformConstantToNameEntry table[], size_t table_size, int32_t value)
{
...
#define ATSPI_CHECK_VERSION(major, minor, micro)                                                                                                               \
    (major) < ATSPI_MAJOR_VERSION || ((major) == ATSPI_MAJOR_VERSION && (minor) < ATSPI_MINOR_VERSION)                                                         \
        || ((major) == ATSPI_MAJOR_VERSION && (minor) == ATSPI_MINOR_VERSION && (micro) <= ATSPI_MICRO_VERSION)
...
const char* ATSPIRelationToString(AtspiRelationType relation)
{
    ...
    return GetNameForPlatformConstant(relation_table, std::size(relation_table), relation);
}
...
const char* const kRoleNames[] = {
    ...
    "accelerator label", "alert", "animation", "arrow", "calendar", "canvas", "check box", "check menu item", "color chooser", "column header", "combo box",
    "dateeditor", "desktop icon", "desktop frame", "dial", "dialog", "directory pane", "drawing area", "file chooser", "filler", "fontchooser", "frame",
    "glass pane", "html container", "icon", "image", "internal frame", "label", "layered pane", "list", "list item", "menu", "menu bar", "menu item",
    "option pane", "page tab", "page tab list", "panel", "password text", "popup menu", "progress bar", "push button", "radio button", "radio menu item",
    "root pane", "row header", "scroll bar", "scroll pane", "separator", "slider", "split pane", "spin button", "statusbar", "table", "table cell",
    "table column header", "table row header", "tear off menu item", "terminal", "text", "toggle button", "tool bar", "tool tip", "tree", "tree table",
    "unknown", "viewport", "window", "header", "footer", "paragraph", "ruler", "application", "autocomplete", "edit bar", "embedded component", "entry",
    "chart", "caption", "document frame", "heading", "page", "section", "redundant object", "form", "link", "input method window", "table row", "tree item",
    "document spreadsheet", "document presentation", "document text", "document web", "document email", "comment", "list box", "grouping", "image map",
    "notification", "info bar", "level bar", "title bar", "block quote", "audio", "video", "definition", "article", "landmark", "log", "marquee", "math",
    "rating", "timer", "description list", "description term", "description value", "static", "math fraction", "math root", "subscript", "superscript",
...
AtspiAccessible* FindActiveDocument(AtspiAccessible* node)
{
    ...
    if (role == ATSPI_ROLE_FRAME) {
        g_autoptr(GArray) relations = atspi_accessible_get_relation_set(node, &error);
        CHECK_ATSPI_ERROR_NULLPTR(error)
        if (!relations) {
            return nullptr;
        }
        ...
        for (guint idx = 0; idx < relations->len; idx++) {
            ...
            if (atspi_relation_get_relation_type(relation) == ATSPI_RELATION_EMBEDS && atspi_relation_get_n_targets(relation) > 0) {
...
AtspiAccessible* FindAccessible(const AXTreeSelector& selector)
{
    ...
    for (int i = 0; i < child_count; i++) {
        AtspiAccessible* child = atspi_accessible_get_child_at_index(desktop, i, &error);
        CHECK_ATSPI_ERROR_NULLPTR(error)
        ...
        if (selector.widget) {
            ...
            if (!error && name) {
                if ((!title.empty() && title == name) || (!selector.pattern.empty() && base::MatchPattern(name, selector.pattern))) {
