...
COMPONENT_EXPORT(AX_PLATFORM)
void IAccessible2StateToStringVector(int32_t ia_state, std::vector<std::wstring>* result);
...
template <typename ServiceType> HRESULT IA2QueryInterface(IUnknown* accessible, ServiceType** out_accessible)
{
...
public:
    ...
    IAccessible* AsIAccessible() const
    {
        return child_.Get();
    }
    const base::win::ScopedVariant& AsVariant() const
    {
        return child_variant_;
    }

    IAccessible* Parent() const
    {
        return parent_.Get();
    }
...
public:
    ...
    const MSAAChild& ChildAt(LONG index) const
    {
    ...
    public:
        ...
        const MSAAChild& operator*()
        {
            return children_->ChildAt(index_);
        }
