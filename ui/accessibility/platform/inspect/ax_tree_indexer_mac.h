...
struct AXNodeComparator {
    constexpr bool operator()(const gfx::NativeViewAccessible& lhs, const gfx::NativeViewAccessible& rhs) const
    {
...
using AXTreeIndexerMacBase
    = AXTreeIndexer<const gfx::NativeViewAccessible, AXElementWrapper::DOMIdOf, NSArray*, AXElementWrapper::ChildrenOf, AXNodeComparator>;
...
public:
    explicit AXTreeIndexerMac(const gfx::NativeViewAccessible node)
        : AXTreeIndexer(node)
        , type_(AXElementWrapper::TypeOf(node))
    {
    }
