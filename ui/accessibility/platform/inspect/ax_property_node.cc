...
// static
AXPropertyNode AXPropertyNode::From(const std::string& property, const std::vector<std::string>& line_indexes)
{
    std::string::const_iterator lvalue_end(property.end()), rvalue_begin(property.end());
    ...
    if (index > 0 && index != std::string::npos) {
        if (property[index - 1] != ':') {
            ...
            rvalue_begin = index == std::string::npos ? property.end() : property.begin() + index;
        }
    }
...
AXPropertyNode AXPropertyNode::From(const AXPropertyFilter& filter)
{
    ...
    // filtering.
    std::string property = filter.property_str.substr(0, filter.property_str.find_last_of('*'));
    ...
    return From(property, base::SplitString(filter.filter_str, std::string(1, ','), base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY));
}
...
bool AXPropertyNode::IsMatching(const std::string& pattern) const
{
    ...
    return name_or_value.compare(0, name_or_value.find_last_of('*'), pattern) == 0;
}
...
{
    if (name_or_value.size() > 1 && name_or_value[0] == '\'' && name_or_value[name_or_value.size() - 1] == '\'') {
...
std::optional<std::string> AXPropertyNode::FindStringKey(const char* refkey) const
{
    for (const auto& param : arguments) {
        if (param.key == refkey) {
...
// private
AXPropertyNode::AXPropertyNode(AXPropertyNode::iterator key_begin, AXPropertyNode::iterator key_end, const std::string& name_or_value)
    : key(key_begin, key_end)
    , name_or_value(name_or_value)
{
}
AXPropertyNode::AXPropertyNode(AXPropertyNode::iterator begin, AXPropertyNode::iterator end)
    : name_or_value(begin, end)
{
}
AXPropertyNode::AXPropertyNode(
    AXPropertyNode::iterator key_begin, AXPropertyNode::iterator key_end, AXPropertyNode::iterator value_begin, AXPropertyNode::iterator value_end)
    : key(key_begin, key_end)
    , name_or_value(value_begin, value_end)
{
}

// private static
AXPropertyNode::iterator AXPropertyNode::Parse(AXPropertyNode* node, AXPropertyNode::iterator begin, AXPropertyNode::iterator end)
{
    auto iter = begin;
    ...
    while (iter != end) {
        ...
        // arguments.
        if (*iter == '(') {
            AXPropertyNode* child_node = node->ConnectTo(state & kChain, key_begin, key_end, begin, iter);
        ...
        if (*iter == '[') {
            ...
            AXPropertyNode* child_node = node->ConnectTo(state & kChain, key_begin, key_end, "[]");
            key_begin = key_end = end;
            begin = iter = Parse(child_node, ++iter, end);
            continue;
        }
        ...
        // key2: value2} form.
        if (*iter == '{') {
            AXPropertyNode* child_node = node->AppendToArguments(key_begin, key_end, "{}");
            key_begin = key_end = end;
            begin = iter = Parse(child_node, ++iter, end);
            continue;
        }
