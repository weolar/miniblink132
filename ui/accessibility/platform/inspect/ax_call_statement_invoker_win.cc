...
#define DEFINE_IA2_QI_ENTRY(ia2_interface)                                                                                                                     \
    if (interface_name == #ia2_interface) {                                                                                                                    \
        ...
        return AXOptionalObject::Error("Unexpected error when querying " + interface_name);                                                                    \
    }

#define CHECK_ARGS_N(arg_count, property_node)                                                                                                                 \
    if (property_node.arguments.size() < arg_count) {                                                                                                          \
        return AXOptionalObject::Error("too few arguments to function " + property_node.name_or_value);                                                        \
    }                                                                                                                                                          \
    if (property_node.arguments.size() > arg_count) {                                                                                                          \
        return AXOptionalObject::Error("too many arguments to function " + property_node.name_or_value);                                                       \
    }
...
// static
std::string AXCallStatementInvokerWin::ToString(const AXOptionalObject& optional)
{
...
AXCallStatementInvokerWin::AXCallStatementInvokerWin(const AXTreeIndexerWin* indexer, std::map<std::string, Target>* storage)
    : indexer_(indexer)
    , storage_(storage)
{
}

AXOptionalObject AXCallStatementInvokerWin::Invoke(const AXPropertyNode& property_node) const
{
...
AXOptionalObject AXCallStatementInvokerWin::InvokeFor(const Target& target, const AXPropertyNode& property_node) const
{
    ...
    if (target.Is<IA2HypertextComPtr>())
        return InvokeForIA2Hypertext(target.As<IA2HypertextComPtr>(), property_node);
    ...
    if (target.Is<IA2TableCellComPtr>())
        return InvokeForIA2TableCell(target.As<IA2TableCellComPtr>(), property_node);
    ...
    if (target.Is<IA2TextSelectionContainerComPtr>()) {
        return InvokeForIA2TextSelectionContainer(target.As<IA2TextSelectionContainerComPtr>(), property_node);
    }
...
AXOptionalObject AXCallStatementInvokerWin::InvokeForAXElement(IAccessibleComPtr target, const AXPropertyNode& property_node) const
{
    ...
    if (property_node.name_or_value == "QueryInterface") {
        if (!property_node.arguments.size()) {
            LOG(ERROR) << "Error: " << property_node.name_or_value << "called without argument";
            return AXOptionalObject::Error();
        }
    ...
    LOG(ERROR) << "Error in '" << property_node.name_or_value << "' called on AXElement in '" << property_node.ToFlatString() << "' statement";
    return AXOptionalObject::Error();
}

AXOptionalObject AXCallStatementInvokerWin::InvokeForIA2(IA2ComPtr target, const AXPropertyNode& property_node) const
{
    if (property_node.name_or_value == "role")
...
AXOptionalObject AXCallStatementInvokerWin::InvokeForIA2Hypertext(IA2HypertextComPtr target, const AXPropertyNode& property_node) const
{
    return AXOptionalObject::Error();
}

AXOptionalObject AXCallStatementInvokerWin::InvokeForIA2Table(IA2TableComPtr target, const AXPropertyNode& property_node) const
{
...
AXOptionalObject AXCallStatementInvokerWin::InvokeForIA2TableCell(IA2TableCellComPtr target, const AXPropertyNode& property_node) const
{
    return AXOptionalObject::Error();
...
AXOptionalObject AXCallStatementInvokerWin::InvokeForIA2TextSelectionContainer(
    IA2TextSelectionContainerComPtr target, const AXPropertyNode& property_node) const
{
...
AXOptionalObject AXCallStatementInvokerWin::InvokeForIA2Text(IA2TextComPtr target, const AXPropertyNode& property_node) const
{
    return AXOptionalObject::Error();
}

AXOptionalObject AXCallStatementInvokerWin::InvokeForIA2Value(IA2ValueComPtr target, const AXPropertyNode& property_node) const
{
    return AXOptionalObject::Error();
...
AXOptionalObject AXCallStatementInvokerWin::GetRole(IAccessibleComPtr target) const
{
...
AXOptionalObject AXCallStatementInvokerWin::GetName(IAccessibleComPtr target) const
{
    base::win::ScopedVariant variant_self(CHILDID_SELF);
...
AXOptionalObject AXCallStatementInvokerWin::GetDescription(IAccessibleComPtr target) const
{
    base::win::ScopedVariant variant_self(CHILDID_SELF);
...
AXOptionalObject AXCallStatementInvokerWin::HasState(IAccessibleComPtr target, std::string state) const
{
    base::win::ScopedVariant variant_self(CHILDID_SELF);
    int32_t ia_state = 0;
    base::win::ScopedVariant ia_state_variant;
    if (target->get_accState(variant_self, ia_state_variant.Receive()) == S_OK && ia_state_variant.type() == VT_I4) {
...
AXOptionalObject AXCallStatementInvokerWin::QueryInterface(IAccessibleComPtr target, std::string interface_name) const
{
    ...
    return AXOptionalObject::Error("Unsupported " + interface_name + " interface");
}
...
AXOptionalObject AXCallStatementInvokerWin::GetIA2Role(IA2ComPtr target) const
{
    ...
    if (SUCCEEDED(target->role(&role)))
        return AXOptionalObject(Target(base::WideToUTF8(IAccessible2RoleToString(role))));

    return AXOptionalObject::Error();
}

AXOptionalObject AXCallStatementInvokerWin::GetIA2Attribute(IA2ComPtr target, const AXPropertyNode& property_node) const
{
    CHECK_ARGS_1(property_node)

    std::string attribute = property_node.arguments[0].name_or_value;
    std::optional<std::string> value = GetIAccessible2Attribute(target, attribute);
    if (value)
        return AXOptionalObject(Target(*value));
    return AXOptionalObject::Error();
}

AXOptionalObject AXCallStatementInvokerWin::HasIA2State(IA2ComPtr target, const AXPropertyNode& property_node) const
{
    CHECK_ARGS_1(property_node)
    std::string state = property_node.arguments[0].name_or_value;
...
AXOptionalObject AXCallStatementInvokerWin::GetSelectedColumns(const IA2TableComPtr target) const
{
    ScopedCoMemArray<LONG> columns;
    if (target->get_selectedColumns(INT_MAX, columns.Receive(), columns.ReceiveSize()) == S_OK) {
        return AXOptionalObject({ std::move(columns) });
    }
    return AXOptionalObject::Error();
}

AXOptionalObject AXCallStatementInvokerWin::GetSelections(IA2TextSelectionContainerComPtr target) const
{
    ScopedCoMemArray<IA2TextSelection> selections;
    if (target->get_selections(selections.Receive(), selections.ReceiveSize()) == S_OK) {
        return AXOptionalObject({ std::move(selections) });
    }
    return AXOptionalObject::Error();
}

AXOptionalObject AXCallStatementInvokerWin::SetSelections(const IA2TextSelectionContainerComPtr target, const AXPropertyNode& property_node) const
{
    CHECK_ARGS_1(property_node)

    std::vector<IA2TextSelection> selections = PropertyNodeToIA2TextSelectionArray(property_node.arguments[0]);
...
bool AXCallStatementInvokerWin::IsIAccessibleAndNotNull(const Target& target) const
{
    return target.Is<IAccessibleComPtr>() && target.As<IAccessibleComPtr>().Get() != nullptr;
}

std::optional<IA2TextSelection> AXCallStatementInvokerWin::PropertyNodeToIA2TextSelection(const AXPropertyNode& node) const
{
    if (!node.IsDict()) {
        return std::nullopt;
    }
    ...
    IA2TextComPtr start_obj = PropertyNodeToIAccessible<IAccessibleText>(*start_obj_node);
    if (!start_obj) {
        return std::nullopt;
    }
    ...
    IA2TextComPtr end_obj = PropertyNodeToIAccessible<IAccessibleText>(*end_obj_node);
    if (!end_obj) {
        return std::nullopt;
    }
...
std::vector<IA2TextSelection> AXCallStatementInvokerWin::PropertyNodeToIA2TextSelectionArray(const AXPropertyNode& node) const
{
    if (!node.IsArray()) {
        return {};
    }
    ...
    for (const auto& item_node : node.arguments) {
        std::optional<IA2TextSelection> item = PropertyNodeToIA2TextSelection(item_node);
        if (!item) {
            return {};
        }
