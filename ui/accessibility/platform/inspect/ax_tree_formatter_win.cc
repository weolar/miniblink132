...
void AXTreeFormatterWin::AddDefaultFilters(std::vector<AXPropertyFilter>* property_filters)
{
...
Microsoft::WRL::ComPtr<IAccessible> GetIAObject(AXPlatformNodeDelegate* node, LONG& root_x, LONG& root_y)
{
    DCHECK(node);
    ...
    HRESULT hr = static_cast<AXPlatformTreeManager*>(root_manager)
                     ...
                     ->accLocation(&root_x, &root_y, &root_width, &root_height, variant_self);
    DCHECK(SUCCEEDED(hr));

    return node->GetNativeViewAccessible();
}

base::Value::Dict AXTreeFormatterWin::BuildNode(AXPlatformNodeDelegate* node) const
{
    LONG root_x = 0, root_y = 0;
    Microsoft::WRL::ComPtr<IAccessible> node_ia = GetIAObject(node, root_x, root_y);
...
base::Value::Dict AXTreeFormatterWin::BuildTree(AXPlatformNodeDelegate* start) const
{
    LONG root_x = 0, root_y = 0;
    Microsoft::WRL::ComPtr<IAccessible> start_ia = GetIAObject(start, root_x, root_y);
    base::Value::Dict dict;
    if (!start_ia) {
        return dict;
    }
...
Microsoft::WRL::ComPtr<IAccessible> AXTreeFormatterWin::FindAccessibleRoot(const AXTreeSelector& selector) const
{
...
base::Value::Dict AXTreeFormatterWin::BuildTreeForSelector(const AXTreeSelector& selector) const
{
    base::Value::Dict dict;
...
std::string AXTreeFormatterWin::EvaluateScript(const AXTreeSelector& selector, const AXInspectScenario& scenario) const
{
    Microsoft::WRL::ComPtr<IAccessible> root = FindAccessibleRoot(selector);
    return EvaluateScript(root, scenario.script_instructions, 0 /* start_index */, scenario.script_instructions.size());
}

std::string AXTreeFormatterWin::EvaluateScript(
    AXPlatformNodeDelegate* root, const std::vector<AXScriptInstruction>& instructions, size_t start_index, size_t end_index) const
{
    DCHECK(root);
    return EvaluateScript(root->GetNativeViewAccessible(), instructions, start_index, end_index);
}

std::string AXTreeFormatterWin::EvaluateScript(
    Microsoft::WRL::ComPtr<IAccessible> root, const std::vector<AXScriptInstruction>& instructions, size_t start_index, size_t end_index) const
{
    if (!root)
    ...
    for (size_t index = start_index; index < end_index; index++) {
        ...
        scripts.Append(property_node.ToString() + "=" + AXCallStatementInvokerWin::ToString(value));
    }
...
void AXTreeFormatterWin::RecursiveBuildTree(const Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict, LONG root_x, LONG root_y) const
{
    AXPlatformNode* platform_node = AXPlatformNode::FromNativeViewAccessible(node.Get());
    ...
    for (const MSAAChild& msaa_child : MSAAChildren(node)) {
        ...
        if (child) {
            ...
            if (child_variant.type() == VT_EMPTY || child_variant.type() == VT_DISPATCH) {
                ...
                if (SUCCEEDED(node->get_accRole(child_variant, role_variant.Receive()))) {
...
void AXTreeFormatterWin::AddProperties(const Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict, LONG root_x, LONG root_y) const
{
...
void AXTreeFormatterWin::AddMSAAProperties(const Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict, LONG root_x, LONG root_y) const
{
    base::win::ScopedVariant variant_self(CHILDID_SELF);
    ...
    if (SUCCEEDED(node->get_accParent(&parent_dispatch))) {
        ...
        if (!parent_dispatch) {
            ...
            if (SUCCEEDED(parent_accessible->get_accRole(variant_self, parent_ia_role_variant.Receive())))
    ...
    if (node->get_accState(variant_self, ia_state_variant.Receive()) == S_OK && ia_state_variant.type() == VT_I4) {
...
void AXTreeFormatterWin::AddSimpleDOMNodeProperties(const Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict) const
{
...
bool AXTreeFormatterWin::AddIA2Properties(const Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict) const
{
    ...
    if (ia2->get_attributes(bstr.Receive()) == S_OK) {
        ...
        std::vector<std::u16string> ia2_attributes
            = base::SplitString(base::WideToUTF16(bstr.Get()), std::u16string(1, ';'), base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
    ...
    if (ia2->get_groupPosition(&group_level, &similar_items_in_group, &position_in_group) == S_OK) {
        dict->Set("group_level", static_cast<int>(group_level));
        dict->Set("similar_items_in_group", static_cast<int>(similar_items_in_group));
...
void AXTreeFormatterWin::AddIA2ActionProperties(const Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict) const
{
    ...
    base::win::ScopedBstr name;
    if (SUCCEEDED(ia2action->get_name(0 /* action_index */, name.Receive())) && name.Get()) {
        dict->Set("action_name", base::WideToUTF8(name.Get()));
    }
}

void AXTreeFormatterWin::AddIA2HypertextProperties(Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict) const
{
    ...
    if (SUCCEEDED(hr) && number_of_embeds > 0) {
        ...
        std::wstring embedded_character = base::UTF16ToWide(std::u16string(1, AXPlatformNodeBase::kEmbeddedCharacter));
        size_t character_index = 0;
        size_t hypertext_index = 0;
        while (hypertext_index < ia2_hypertext.length()) {
            if (ia2_hypertext[hypertext_index] != AXPlatformNodeBase::kEmbeddedCharacter) {
                ++character_index;
                ++hypertext_index;
                continue;
            }
            ...
            std::wstring child_index_str = (child_index >= 0) ? base::StrCat({ L"<obj", base::NumberToWString(child_index), L">" }) : std::wstring(L"<obj>");
            base::ReplaceFirstSubstringAfterOffset(&ia2_hypertext, hypertext_index, embedded_character, child_index_str);
            ++character_index;
...
void AXTreeFormatterWin::AddIA2RelationProperties(const Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict) const
{
    Microsoft::WRL::ComPtr<IAccessible2> ia2;
    if (S_OK != IA2QueryInterface<IAccessible2>(node.Get(), &ia2)) {
        return;
...
void AXTreeFormatterWin::AddIA2RelationProperty(const Microsoft::WRL::ComPtr<IAccessibleRelation> relation, base::Value::Dict* dict) const
{
...
void AXTreeFormatterWin::AddIA2TableProperties(const Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict) const
{
...
static std::u16string ProcessAccessiblesArray(IUnknown** accessibles, LONG num_accessibles)
{
...
void AXTreeFormatterWin::AddIA2TableCellProperties(const Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict) const
{
    ...
    if (SUCCEEDED(ia2cell->get_rowHeaderCells(&row_headers, &n_row_header_cells)) && n_row_header_cells > 0) {
        std::u16string accessibles_desc = ProcessAccessiblesArray(row_headers, n_row_header_cells);
    ...
    if (SUCCEEDED(ia2cell->get_columnHeaderCells(&column_headers, &n_column_header_cells)) && n_column_header_cells > 0) {
        std::u16string accessibles_desc = ProcessAccessiblesArray(column_headers, n_column_header_cells);
        CoTaskMemFree(column_headers); // Free the array manually.
        dict->Set("column_headers", accessibles_desc);
    }
}

void AXTreeFormatterWin::AddIA2TextProperties(const Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict) const
{
    ...
    while (current_offset < n_characters) {
        ...
        HRESULT hr = ia2text->get_attributes(current_offset, &start_offset, &end_offset, temp_bstr.Receive());
        ...
        if (hr == S_OK && temp_bstr.Get() && wcslen(temp_bstr.Get())) {
            ...
            std::u16string offset_str = u"offset:" + base::NumberToString16(start_offset);
            text_attributes.Append(offset_str);
            // Append name:value pairs.
            std::vector<std::wstring> name_val_pairs = SplitString(std::wstring(temp_bstr.Get()), L";", base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
...
void AXTreeFormatterWin::AddIA2ValueProperties(const Microsoft::WRL::ComPtr<IAccessible> node, base::Value::Dict* dict) const
{
    ...
    if (ia2value->get_currentValue(current_value.Receive()) == S_OK && isfinite(V_R8(current_value.ptr()))) {
        dict->Set("currentValue", V_R8(current_value.ptr()));
    }

    base::win::ScopedVariant minimum_value;
    if (ia2value->get_minimumValue(minimum_value.Receive()) == S_OK && isfinite(V_R8(minimum_value.ptr()))) {
        dict->Set("minimumValue", V_R8(minimum_value.ptr()));
    }

    base::win::ScopedVariant maximum_value;
    if (ia2value->get_maximumValue(maximum_value.Receive()) == S_OK && isfinite(V_R8(maximum_value.ptr()))) {
        dict->Set("maximumValue", V_R8(maximum_value.ptr()));
    }
}

std::string AXTreeFormatterWin::ProcessTreeForOutput(const base::Value::Dict& dict) const
{
    std::string line;
    ...
    for (const char* attribute_name : ALL_ATTRIBUTES) {
        ...
        case base::Value::Type::DICT: {
            ...
            if (strcmp(attribute_name, "size") == 0) {
                WriteAttribute(false, FormatCoordinates(dict_value, "size", "width", "height"), &line);
            } else if (strcmp(attribute_name, "location") == 0) {
                WriteAttribute(false, FormatCoordinates(dict_value, "location", "x", "y"), &line);
            }
            break;
        }
        default:
            WriteAttribute(false, base::StrCat({ attribute_name, "=", AXFormatValue(*value) }), &line);
            break;
        }
    }
...
Microsoft::WRL::ComPtr<IAccessible> AXTreeFormatterWin::FindActiveDocument(IAccessible* root) const
{
    for (const MSAAChild& child : MSAAChildren(root)) {
        ...
        if (role == IA2_ROLE_INTERNAL_FRAME) {
            ...
            if (SUCCEEDED(ia->get_accState(base::win::ScopedVariant(CHILDID_SELF), state_variant.Receive())) && state_variant.type() == VT_I4) {
        ...
        Microsoft::WRL::ComPtr<IAccessible> active_document = FindActiveDocument(ia);
        if (active_document)
