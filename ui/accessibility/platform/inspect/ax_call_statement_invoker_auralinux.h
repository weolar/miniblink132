...
using Target = absl::variant<absl::monostate, std::string, int, const AtspiAccessible*>;
...
public:
    ...
    AXCallStatementInvokerAuraLinux(const AXTreeIndexerAuraLinux* indexer, std::map<std::string, Target>* storage);

    // Invokes an attribute matching a property filter.
    AXOptionalObject Invoke(const AXPropertyNode& property_node, bool no_object_parse = false) const;
...
private:
    ...
    AXOptionalObject InvokeForAXElement(const AtspiAccessible* target, const AXPropertyNode& property_node) const;
    ...
    AXOptionalObject GetAttribute(const AtspiAccessible* target, std::string attribute) const;

    AXOptionalObject HasState(const AtspiAccessible* target, std::string state) const;

    AXOptionalObject GetRelation(const AtspiAccessible* target, std::string relation) const;

    AXOptionalObject HasInterface(const AtspiAccessible* target, std::string interface) const;
