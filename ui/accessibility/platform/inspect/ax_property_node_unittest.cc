...
TEST_F(AXPropertyNodeTest, ParseProperty)
{
    ...
    // Nested arguments
    ParseAndCheck("AXIndexForTextMarker(AXTextMarkerForIndex(0))", "AXIndexForTextMarker(AXTextMarkerForIndex(0))");
    ...
    // Dict: FindStringKey
    EXPECT_EQ(GetArgumentNode("Text({start: :1, dir: forward})").FindStringKey("start"), ":1");
    EXPECT_EQ(GetArgumentNode("Text({start: :1, dir: forward})").FindStringKey("dir"), "forward");
    EXPECT_EQ(GetArgumentNode("Text({start: :1, dir: forward})").FindStringKey("notexists"), std::nullopt);

    // Dict: FindIntKey
    EXPECT_EQ(GetArgumentNode("Text({loc: 3, len: 2})").FindIntKey("loc"), 3);
    EXPECT_EQ(GetArgumentNode("Text({loc: 3, len: 2})").FindIntKey("len"), 2);
    EXPECT_EQ(GetArgumentNode("Text({loc: 3, len: 2})").FindIntKey("notexists"), std::nullopt);

    // `AXPropertyNode::FindKey()`
    EXPECT_EQ(GetArgumentNode("Text({anchor: {:1, 0, up}})").FindKey("anchor")->ToFlatString(), "anchor: {}(:1, 0, up)");

    EXPECT_EQ(GetArgumentNode("Text({anchor: {:1, 0, up}})").FindKey("focus"), nullptr);
...
cellAt(
  ...
  columnIndex
    ...
    ParseAndCheckTree("table.cellAt(table.rowIndexFor(cell), table.columnIndexFor(cell))",
        R"~~(table.
cellAt(
  table.
  rowIndexFor(
...
AXRole)~~");
    ...
    ParseAndCheckTree("textarea.AXTextMarkerRangeForUIElement(textarea.AXChildren[0])",
        R"~~(textarea.
...
{
    // Statement
    ParseAndCheckTree("textmarker_range:= textarea.AXTextMarkerRangeForUIElement(textarea)",
