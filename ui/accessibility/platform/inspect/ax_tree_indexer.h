 ...
 */
template <typename AccessibilityObject, std::string (*GetDOMId)(const AccessibilityObject), typename ChildrenContainer,
    ChildrenContainer (*GetChildren)(const AccessibilityObject), typename Compare = std::less<AccessibilityObject>>
...
public:
    ...
    AccessibilityObject NodeBy(const std::string& identifier) const
    {
        for (auto& item : node_to_identifier_) {
            if (item.second.line_index == identifier || item.second.id == identifier) {
                return item.first;
            }
        }
        return nullptr;
...
private:
    void Build(const AccessibilityObject node, int* counter)
    {
        ...
        const std::string line_index = std::string(1, ':') + base::NumberToString(++(*counter));
    ...
    // attribute.
    std::map<const AccessibilityObject, NodeIdentifier, Compare> node_to_identifier_;
};
