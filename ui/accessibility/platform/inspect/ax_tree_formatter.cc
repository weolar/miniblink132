...
// static
bool AXTreeFormatter::MatchesPropertyFilters(const std::vector<AXPropertyFilter>& property_filters, const std::string& text, bool default_result)
{
    ...
    for (const auto& filter : property_filters) {
        ...
        if (base::MatchPattern(text, filter.match_str)
            || (filter.match_str.length() > 0 && filter.match_str.find('=') == std::string::npos && filter.match_str[filter.match_str.length() - 1] != '*'
...
// static
bool AXTreeFormatter::MatchesNodeFilters(const std::vector<AXNodeFilter>& node_filters, const base::Value::Dict& dict)
{
