...
// static
gboolean AXEventRecorderAuraLinux::OnATKEventReceived(GSignalInvocationHint* hint, unsigned int n_params, const GValue* params, gpointer data)
{
    GSignalQuery query;
    ...
    if (instance_) {
        ...
        gchar* signal_name = g_strcmp0(query.signal_name, "children-changed") ? g_strdup(query.signal_name)
                                                                              : g_strconcat(query.signal_name, ":", g_quark_to_string(hint->detail), nullptr);
...
AXEventRecorderAuraLinux::AXEventRecorderAuraLinux(base::WeakPtr<AXPlatformTreeManager> manager, base::ProcessId pid, const AXTreeSelector& selector)
    : manager_(manager)
    , pid_(pid)
    , selector_(selector)
{
...
std::string AXEventRecorderAuraLinux::AtkObjectToString(AtkObject* obj, bool include_name)
{
    std::string role = AtkRoleToString(atk_object_get_role(obj));
    base::ReplaceChars(role, " ", "_", &role);
    std::string str = base::StringPrintf("role=ROLE_%s", base::ToUpperASCII(role).c_str());
...
void AXEventRecorderAuraLinux::ProcessATKEvent(const char* event, unsigned int n_params, const GValue* params)
{
    ...
    if (event_name.find("property-change") != std::string::npos) {
        ...
        AtkPropertyValues* property_values = static_cast<AtkPropertyValues*>(g_value_get_pointer(&params[1]));

        if (g_strcmp0(property_values->property_name, "accessible-value") == 0) {
            log += "VALUE-CHANGED:";
            log += base::NumberToString(g_value_get_double(&property_values->new_value));
        } else if (g_strcmp0(property_values->property_name, "accessible-name") == 0) {
            const char* new_name = g_value_get_string(&property_values->new_value);
            log += "NAME-CHANGED:";
            log += (new_name) ? new_name : "(null)";
        } else if (g_strcmp0(property_values->property_name, "accessible-description") == 0) {
            const char* new_description = g_value_get_string(&property_values->new_value);
            log += "DESCRIPTION-CHANGED:";
            log += (new_description) ? new_description : "(null)";
        } else if (g_strcmp0(property_values->property_name, "accessible-parent") == 0) {
            log += "PARENT-CHANGED";
            if (AtkObject* new_parent = static_cast<AtkObject*>(g_value_get_object(&property_values->new_value)))
        ...
        } else if (event_name.find("text-insert") != std::string::npos || event_name.find("text-remove") != std::string::npos) {
            DCHECK_GE(n_params, 4u);
            log += base::StringPrintf(" (start=%i length=%i '%s')", g_value_get_int(&params[1]), g_value_get_int(&params[2]), g_value_get_string(&params[3]));
        }
    }
...
{
    atspi_init();
    atspi_event_listener_ = atspi_event_listener_new(OnATSPIEventReceived, this, nullptr);

    GError* error = nullptr;
    for (size_t i = 0; i < std::size(kEventNames); i++) {
        atspi_event_listener_register(atspi_event_listener_, kEventNames[i], &error);
        if (error) {
...
void AXEventRecorderAuraLinux::RemoveATSPIEventListeners()
{
    ...
    GError* error = nullptr;
    for (size_t i = 0; i < std::size(kEventNames); i++) {
        atspi_event_listener_deregister(atspi_event_listener_, kEventNames[i], nullptr);
        if (error) {
            LOG(ERROR) << "Could not deregister event listener for " << kEventNames[i];
            g_clear_error(&error);
        }
    }
...
void AXEventRecorderAuraLinux::ProcessATSPIEvent(const AtspiEvent* event)
{
    ...
    if (!pid_ && !selector_.empty()) {
        AtspiAccessible* application = atspi_accessible_get_application(event->source, &error);
        if (error) {
            g_clear_error(&error);
            return;
        }
    ...
    GHashTable* attributes = atspi_accessible_get_attributes(event->source, &error);
    ...
    if (!error && attributes) {
        ...
        if (char* id = static_cast<char*>(g_hash_table_lookup(attributes, "id")))
            html_id = id;
        if (char* class_chars = static_cast<char*>(g_hash_table_lookup(attributes, "class")))
