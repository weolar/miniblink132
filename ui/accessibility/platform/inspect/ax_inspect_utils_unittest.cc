...
TEST(AXInspectUtilsTest, FormatList)
{
    ...
    list.Append("item1");
    list.Append("item2");
    EXPECT_EQ(AXFormatValue(base::Value(std::move(list))), std::string("['item1', 'item2']"));
}
...
TEST(AXInspectUtilsTest, FormatDict)
{
    ...
    EXPECT_EQ(AXFormatValue(base::Value(std::move(dict))), std::string("{affinity: down, anchor: textbox, offset: 2}"));
}
...
TEST(AXInspectUtilsTest, FormatSet)
{
    ...
    EXPECT_EQ(AXFormatValue(base::Value(std::move(set))), std::string("{:1, 2, down}"));
}
...
TEST(AXInspectUtilsTest, FormatOrderedDict)
{
    ...
    EXPECT_EQ(AXFormatValue(base::Value(std::move(ordered_dict))), std::string("{w: 40, h: 30}"));
}
