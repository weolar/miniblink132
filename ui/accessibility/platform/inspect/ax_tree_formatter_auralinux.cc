...
base::Value::Dict AXTreeFormatterAuraLinux::BuildTreeForSelector(const AXTreeSelector& selector) const
{
...
std::string AXTreeFormatterAuraLinux::EvaluateScript(const AXTreeSelector& selector, const AXInspectScenario& scenario) const
{
    ...
    const std::vector<AXScriptInstruction>& instructions = scenario.script_instructions;
    ...
    for (size_t index = 0; index < end_index; index++) {
        ...
        scripts.emplace_back(property_node.ToString() + "=" + AXCallStatementInvokerAuraLinux::ToString(value));
    }
...
base::Value::Dict AXTreeFormatterAuraLinux::BuildTree(AXPlatformNodeDelegate* root) const
{
    base::Value::Dict dict;
...
base::Value::Dict AXTreeFormatterAuraLinux::BuildNode(AXPlatformNodeDelegate* node) const
{
    base::Value::Dict dict;
...
void AXTreeFormatterAuraLinux::RecursiveBuildTree(AtkObject* atk_node, base::Value::Dict* dict) const
{
    AXPlatformNodeAuraLinux* platform_node = AXPlatformNodeAuraLinux::FromAtkObject(atk_node);
...
void AXTreeFormatterAuraLinux::RecursiveBuildTree(AtspiAccessible* node, base::Value::Dict* dict) const
{
...
void AXTreeFormatterAuraLinux::AddHypertextProperties(AtkObject* atk_object, base::Value::Dict* dict) const
{
    ...
    if (link_count > 0) {
        for (int link_index = link_count - 1; link_index >= 0; link_index--) {
            ...
            base::ReplaceFirstSubstringAfterOffset(&text, offset, replacement_char, link_str);
        }
    }
...
void AXTreeFormatterAuraLinux::AddTextProperties(AtkObject* atk_object, base::Value::Dict* dict) const
{
    ...
    char* selection_text = atk_text_get_selection(atk_text, 0, &selection_start, &selection_end);
    if (selection_text) {
        g_free(selection_text);
        text_values.Append(base::StringPrintf("selection_start=%i", selection_start));
    ...
    auto add_attribute_set_values = [](gpointer value, gpointer list) {
        ...
        static_cast<base::Value::List*>(list)->Append(base::StringPrintf("%s=%s", attribute->name, attribute->value));
    };

    int current_offset = 0, start_offset, end_offset;
    while (current_offset < character_count) {
        AtkAttributeSet* text_attributes = atk_text_get_run_attributes(atk_text, current_offset, &start_offset, &end_offset);
...
void AXTreeFormatterAuraLinux::AddActionProperties(AtkObject* atk_object, base::Value::Dict* dict) const
{
...
void AXTreeFormatterAuraLinux::AddRelationProperties(AtkObject* atk_object, base::Value::Dict* dict) const
{
...
{
    std::string relation_name = atk_relation_type_get_name(relation->relationship);
    ...
    for (guint i = 0; i < relation_targets->len; i++) {
        AtkObject* atk_target = static_cast<AtkObject*>(g_ptr_array_index(relation_targets, i));
        DCHECK(atk_target);
    ...
    return base::StrCat({ relation_name, "=[", base::JoinString(target_roles, ","), "]" });
}

void AXTreeFormatterAuraLinux::AddValueProperties(AtkObject* atk_object, base::Value::Dict* dict) const
{
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    value_properties.Append(base::StringPrintf("current=%f", g_value_get_float(&current)));

    GValue minimum = G_VALUE_INIT;
    g_value_init(&minimum, G_TYPE_FLOAT);
    atk_value_get_minimum_value(value, &minimum);
    value_properties.Append(base::StringPrintf("minimum=%f", g_value_get_float(&minimum)));
...
#pragma clang diagnostic pop
    ...
    value_properties.Append(base::StringPrintf("maximum=%f", g_value_get_float(&maximum)));
    dict->Set("value", std::move(value_properties));
}

void AXTreeFormatterAuraLinux::AddTableProperties(AtkObject* atk_object, base::Value::Dict* dict) const
{
    ...
    table_properties.Append(base::StringPrintf("headers=(%s);", base::JoinString(col_headers, ", ").c_str()));
    ...
    table_properties.Append(base::StringPrintf("headers=(%s);", base::JoinString(row_headers, ", ").c_str()));

    // Caption details.
    AtkObject* caption = atk_table_get_caption(table);
    table_properties.Append(base::StringPrintf("caption=%s;", caption ? "true" : "false"));
    ...
    for (int r = 0; r < n_rows; r++) {
        for (int c = 0; c < n_cols; c++) {
            ...
            if (row_span != 1 || col_span != 1) {
                span_info.push_back(base::StringPrintf("cell at %i,%i: %ix%i", r, c, row_span, col_span));
            }
        }
    }
    if (!span_info.size())
    ...
    table_properties.Append(base::StringPrintf("spans=(%s)", base::JoinString(span_info, ", ").c_str()));
    dict->Set("table", std::move(table_properties));
}

void AXTreeFormatterAuraLinux::AddTableCellProperties(const AXPlatformNodeAuraLinux* node, AtkObject* atk_object, base::Value::Dict* dict) const
{
    AtkRole role = atk_object_get_role(atk_object);
    if (role != ATK_ROLE_TABLE_CELL && role != ATK_ROLE_COLUMN_HEADER && role != ATK_ROLE_ROW_HEADER) {
        return;
    }
    ...
    AtkTableCell* cell = G_TYPE_CHECK_INSTANCE_CAST((atk_object), atk_table_cell_get_type(), AtkTableCell);
    ...
    cell_properties.Append(base::StringPrintf("(%s)", base::JoinString(cell_info, ", ").c_str()));
    dict->Set("cell", std::move(cell_properties));
}

void AXTreeFormatterAuraLinux::AddProperties(AtkObject* atk_object, base::Value::Dict* dict) const
{
    AXPlatformNodeAuraLinux* platform_node = AXPlatformNodeAuraLinux::FromAtkObject(atk_object);
    DCHECK(platform_node);
    ...
    for (AtkAttributeSet* attr = attributes; attr; attr = attr->next) {
        ...
        dict->SetByDottedPath(std::string(kObjectAttributePrefix) + attribute->name, attribute->value);
    }
...
void AXTreeFormatterAuraLinux::AddProperties(AtspiAccessible* node, base::Value::Dict* dict) const
{
    GError* error = nullptr;
...
std::string AXTreeFormatterAuraLinux::ProcessTreeForOutput(const base::Value::Dict& node) const
{
    ...
    if (action_names_list) {
        ...
        if (actions_str.size()) {
            WriteAttribute(false, base::StringPrintf("actions=(%s)", actions_str.c_str()), &line);
        }
    }
    ...
    if (relations_value) {
        for (const auto& entry : *relations_value) {
            ...
            if (relation_value) {
                ...
                WriteAttribute(!relation_value->starts_with("embedded-by"), *relation_value, &line);
            }
        }
    }

    for (const char* attribute_name : ATK_OBJECT_ATTRIBUTES) {
        const std::string* attribute_value = node.FindString(std::string(kObjectAttributePrefix) + attribute_name);
        ...
        if (attribute_value) {
            WriteAttribute(false, base::StringPrintf("%s:%s", attribute_name, attribute_value->c_str()), &line);
        }
    }
