...
bool AXTreeFormatterBase::ShouldDumpNode(const AXPlatformNodeDelegate& node) const
{
    for (const std::pair<ax::mojom::StringAttribute, std::string>& string_attribute : node.GetStringAttributes()) {
...
bool AXTreeFormatterBase::ShouldDumpChildren(const AXPlatformNodeDelegate& node) const
{
    for (const std::pair<ax::mojom::StringAttribute, std::string>& string_attribute : node.GetStringAttributes()) {
...
std::string AXTreeFormatterBase::FormatNode(AXPlatformNodeDelegate* node) const
{
    return FormatTree(BuildNode(node));
}

std::string AXTreeFormatterBase::FormatNode(const AXTreeSelector& selector) const
{
    return FormatTree(BuildNodeForSelector(selector));
}

base::Value::Dict AXTreeFormatterBase::BuildNode(AXPlatformNodeDelegate* node) const
{
    return base::Value::Dict();
}

base::Value::Dict AXTreeFormatterBase::BuildNodeForSelector(const AXTreeSelector&) const
{
    return base::Value::Dict();
}

std::string AXTreeFormatterBase::FormatTree(const base::Value::Dict& dict) const
{
    std::string contents;
...
{
    NOTREACHED() << "Only supported when called on AccessibilityTreeFormatterBlink.";
}

std::string AXTreeFormatterBase::EvaluateScript(const AXTreeSelector& selector, const AXInspectScenario& scenario) const
{
    NOTIMPLEMENTED();
    return {};
}
...
void AXTreeFormatterBase::RecursiveFormatTree(const base::Value::Dict& dict, std::string* contents, int depth) const
{
...
void AXTreeFormatterBase::SetPropertyFilters(const std::vector<AXPropertyFilter>& property_filters, PropertyFilterSet default_filter_set)
{
    property_filters_.clear();
    if (default_filter_set == kFiltersDefaultSet) {
        AddDefaultFilters(&property_filters_);
    }
    property_filters_.insert(property_filters_.end(), property_filters.begin(), property_filters.end());
}

void AXTreeFormatterBase::SetNodeFilters(const std::vector<AXNodeFilter>& node_filters)
{
...
std::string AXTreeFormatterBase::DumpInternalAccessibilityTree(AXTreeID tree_id, const std::vector<AXPropertyFilter>& property_filters)
{
    NOTREACHED() << "Only supported when called on AccessibilityTreeFormatterBlink.";
}

std::vector<AXPropertyNode> AXTreeFormatterBase::PropertyFilterNodesFor(const std::string& line_index) const
{
    ...
    for (const auto& filter : property_filters_) {
        ...
        if (!property_node.line_indexes.empty() && !base::Contains(property_node.line_indexes, line_index)) {
            continue;
        }
...
bool AXTreeFormatterBase::MatchesPropertyFilters(const std::string& text, bool default_result) const
{
    return AXTreeFormatter::MatchesPropertyFilters(property_filters_, text, default_result);
}

bool AXTreeFormatterBase::MatchesNodeFilters(const base::Value::Dict& dict) const
{
    return AXTreeFormatter::MatchesNodeFilters(node_filters_, dict);
}

std::string AXTreeFormatterBase::FormatCoordinates(
    const base::Value::Dict& dict, const std::string& name, const std::string& x_name, const std::string& y_name) const
{
...
std::string AXTreeFormatterBase::FormatRectangle(const base::Value::Dict& dict, const std::string& name, const std::string& left_name,
    const std::string& top_name, const std::string& width_name, const std::string& height_name) const
{
    ...
    return base::StringPrintf("%s=(%d, %d, %d, %d)", name.c_str(), left, top, width, height);
}

bool AXTreeFormatterBase::WriteAttribute(bool include_by_default, const std::string& attr, std::string* line) const
{
    if (attr.empty())
        return false;
...
void AXTreeFormatterBase::AddPropertyFilter(std::vector<AXPropertyFilter>* property_filters, std::string filter, AXPropertyFilter::Type type)
{
    property_filters->push_back(AXPropertyFilter(filter, type));
}

void AXTreeFormatterBase::AddDefaultFilters(std::vector<AXPropertyFilter>* property_filters)
{
}
