...
public:
    ...
    base::FilePath GetExpectationFilePath(
        const base::FilePath& test_file_path, const base::FilePath::StringType& expectations_qualifier = FILE_PATH_LITERAL(""));
    ...
    // of conflict.
    AXInspectScenario ParseScenario(const std::vector<std::string>& lines, const std::vector<AXPropertyFilter>& default_filters = {});
    ...
    std::optional<AXInspectScenario> ParseScenario(const base::FilePath& scenario_path, const std::vector<AXPropertyFilter>& default_filters = {});
    ...
    static std::optional<std::vector<std::string>> LoadExpectationFile(const base::FilePath& expected_file);
    ...
    static bool ValidateAgainstExpectation(const base::FilePath& test_file_path, const base::FilePath& expected_file,
...
private:
    ...
    base::FilePath::StringType GetExpectedFileSuffix(const base::FilePath::StringType& expectations_qualifier = FILE_PATH_LITERAL("")) const;
    ...
    base::FilePath::StringType GetVersionSpecificExpectedFileSuffix(const base::FilePath::StringType& expectations_qualifier = FILE_PATH_LITERAL("")) const;
    ...
    static std::vector<int> DiffLines(const std::vector<std::string>& expected_lines, const std::vector<std::string>& actual_lines);
