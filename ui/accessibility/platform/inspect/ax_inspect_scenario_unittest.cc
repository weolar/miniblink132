...
{
    AXInspectScenario scenario = AXInspectScenario::From("@MAC-", { "@NO-LOAD-EXPECTED:broken.jpg" });
    EXPECT_THAT(scenario.no_load_expected, testing::ElementsAre("broken.jpg"));
}

TEST(AXInspectScenarioTest, WaitFor)
{
    AXInspectScenario scenario = AXInspectScenario::From("@MAC-", { "@WAIT-FOR:(0, 50" });
    EXPECT_THAT(scenario.wait_for, testing::ElementsAre("(0, 50"));
}

TEST(AXInspectScenarioTest, DefaultActionOn)
{
    AXInspectScenario scenario = AXInspectScenario::From("@MAC-", { "@DEFAULT-ACTION-ON:Show time picker" });
    EXPECT_THAT(scenario.default_action_on, testing::ElementsAre("Show time picker"));
}

TEST(AXInspectScenarioTest, ExecuteAndWaitFor)
{
    AXInspectScenario scenario = AXInspectScenario::From("@MAC-", { "@EXECUTE-AND-WAIT-FOR:open_modal()" });
    EXPECT_THAT(scenario.execute, testing::ElementsAre("open_modal()"));
}

TEST(AXInspectScenarioTest, PropertyFilters)
{
    AXInspectScenario scenario = AXInspectScenario::From("@MAC-", { "@MAC-ALLOW:AXRoleDescription", "@MAC-ALLOW-EMPTY:AXARIALive", "@MAC-DENY:AXTitle" });
...
{
    AXInspectScenario scenario = AXInspectScenario::From("@MAC-", { "@MAC-ALLOW:", "  AXRoleDescription", "  AXARIALive" });
...
{
    AXInspectScenario scenario = AXInspectScenario::From("@MAC-", { "@MAC-DENY-NODE:AXRole=AXCheckBox" });
...
{
    std::vector<std::string> lines({ "@MAC-SCRIPT:", "  text_range:= textarea.AXTextMarkerRangeForUIElement(textarea)",
        "  textarea.AXSelectedTextMarkerRange(text_range)", "  wait for AXSelectedTextChanged", "textarea.AXSelectedText" });
    AXInspectScenario scenario = AXInspectScenario::From("@MAC-", lines);
    EXPECT_EQ(scenario.script_instructions.size(), 4U);
    EXPECT_EQ(scenario.script_instructions[0].AsScript().ToString(), "text_range");
    EXPECT_EQ(scenario.script_instructions[1].AsScript().ToString(), "textarea.AXSelectedTextMarkerRange(text_range)");
    EXPECT_EQ(scenario.script_instructions[2].AsEvent(), "AXSelectedTextChanged");
    EXPECT_EQ(scenario.script_instructions[3].AsScript().ToString(), "textarea.AXSelectedText");
}
