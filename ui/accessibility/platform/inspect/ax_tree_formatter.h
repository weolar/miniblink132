...
public:
    ...
    // property filters.
    static bool MatchesPropertyFilters(const std::vector<AXPropertyFilter>& property_filters, const std::string& text, bool default_result);

    // Check if the given dictionary matches any of the supplied AXNodeFilter(s).
    static bool MatchesNodeFilters(const std::vector<AXNodeFilter>& node_filters, const base::Value::Dict& dict);
    ...
    virtual base::Value::Dict BuildNodeForSelector(const AXTreeSelector&) const = 0;
    ...
    //   } ]
    // }
    virtual base::Value::Dict BuildTreeForSelector(const AXTreeSelector&) const = 0;
    ...
    virtual std::string DumpInternalAccessibilityTree(AXTreeID tree_id, const std::vector<AXPropertyFilter>& property_filters) = 0;
    ...
    virtual std::string EvaluateScript(const AXTreeSelector& selector, const AXInspectScenario& scenario) const = 0;
    ...
    virtual void SetPropertyFilters(const std::vector<AXPropertyFilter>& property_filters, PropertyFilterSet default_filters_set = kFiltersEmptySet) = 0;

    // Set regular expression filters that apply to every node before output.
    virtual void SetNodeFilters(const std::vector<AXNodeFilter>& node_filters) = 0;
