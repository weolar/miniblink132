...
std::string AXCallStatementInvokerAuraLinux::ToString(AXOptionalObject& optional)
{
...
AXCallStatementInvokerAuraLinux::AXCallStatementInvokerAuraLinux(const AXTreeIndexerAuraLinux* indexer, std::map<std::string, Target>* storage)
    : indexer_(indexer)
    , storage_(storage)
{
}

AXOptionalObject AXCallStatementInvokerAuraLinux::Invoke(const AXPropertyNode& property_node, bool no_object_parse) const
{
...
AXOptionalObject AXCallStatementInvokerAuraLinux::InvokeFor(const Target target, const AXPropertyNode& property_node) const
{
    if (absl::holds_alternative<const AtspiAccessible*>(target)) {
        const AtspiAccessible* AXElement = absl::get<const AtspiAccessible*>(target);
...
AXOptionalObject AXCallStatementInvokerAuraLinux::GetRole(const AtspiAccessible* target) const
{
    GError* error = nullptr;
    char* role_name = atspi_accessible_get_role_name(const_cast<AtspiAccessible*>(target), &error);
    if (!error) {
...
AXOptionalObject AXCallStatementInvokerAuraLinux::GetName(const AtspiAccessible* target) const
{
    GError* error = nullptr;
    char* name = atspi_accessible_get_name(const_cast<AtspiAccessible*>(target), &error);
    if (!error) {
...
AXOptionalObject AXCallStatementInvokerAuraLinux::GetDescription(const AtspiAccessible* target) const
{
    GError* error = nullptr;
    char* description = atspi_accessible_get_description(const_cast<AtspiAccessible*>(target), &error);
    if (!error) {
...
AXOptionalObject AXCallStatementInvokerAuraLinux::GetParent(const AtspiAccessible* target) const
{
    GError* error = nullptr;
    AtspiAccessible* parent = atspi_accessible_get_parent(const_cast<AtspiAccessible*>(target), &error);
    if (!error) {
...
AXOptionalObject AXCallStatementInvokerAuraLinux::GetAttribute(const AtspiAccessible* target, std::string attribute) const
{
    GError* error = nullptr;

    GHashTable* attributes = atspi_accessible_get_attributes(const_cast<AtspiAccessible*>(target), &error);
    if (!error) {
        GHashTableIter i;
        void* key = nullptr;
...
AXOptionalObject AXCallStatementInvokerAuraLinux::HasState(const AtspiAccessible* target, std::string state) const
{
    AtspiStateSet* atspi_states = atspi_accessible_get_state_set(const_cast<AtspiAccessible*>(target));
...
AXOptionalObject AXCallStatementInvokerAuraLinux::GetRelation(const AtspiAccessible* target, std::string relation) const
{
    GError* error = nullptr;

    GArray* relations = atspi_accessible_get_relation_set(const_cast<AtspiAccessible*>(target), &error);

    if (!error) {
        for (guint idx = 0; idx < relations->len; idx++) {
            AtspiRelation* atspi_relation = g_array_index(relations, AtspiRelation*, idx);
            std::string relation_str = ATSPIRelationToString(atspi_relation_get_relation_type(atspi_relation));
...
AXOptionalObject AXCallStatementInvokerAuraLinux::HasInterface(const AtspiAccessible* target, std::string interface) const
{
    GArray* interfaces = atspi_accessible_get_interfaces(const_cast<AtspiAccessible*>(target));
...
AXOptionalObject AXCallStatementInvokerAuraLinux::InvokeForAXElement(const AtspiAccessible* target, const AXPropertyNode& property_node) const
{
