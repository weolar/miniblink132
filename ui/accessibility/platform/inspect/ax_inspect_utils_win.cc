...
std::wstring GetNameForPlatformConstant(const PlatformConstantToNameEntry table[], size_t table_size, int32_t value)
{
...
struct HwndWithProcId {
    HwndWithProcId(const base::ProcessId id)
        : pid(id)
        , hwnd(nullptr)
    {
    }
...
std::wstring IAccessibleRoleToString(int32_t ia_role)
{
    ...
    static const PlatformConstantToNameEntry ia_table[] = {
        QUOTE(ROLE_SYSTEM_ALERT),
        QUOTE(ROLE_SYSTEM_ANIMATION),
        QUOTE(ROLE_SYSTEM_APPLICATION),
        QUOTE(ROLE_SYSTEM_BORDER),
        QUOTE(ROLE_SYSTEM_BUTTONDROPDOWN),
        QUOTE(ROLE_SYSTEM_BUTTONDROPDOWNGRID),
        QUOTE(ROLE_SYSTEM_BUTTONMENU),
        QUOTE(ROLE_SYSTEM_CARET),
        QUOTE(ROLE_SYSTEM_CELL),
        QUOTE(ROLE_SYSTEM_CHARACTER),
        QUOTE(ROLE_SYSTEM_CHART),
        QUOTE(ROLE_SYSTEM_CHECKBUTTON),
        QUOTE(ROLE_SYSTEM_CLIENT),
        QUOTE(ROLE_SYSTEM_CLOCK),
        QUOTE(ROLE_SYSTEM_COLUMN),
        QUOTE(ROLE_SYSTEM_COLUMNHEADER),
        QUOTE(ROLE_SYSTEM_COMBOBOX),
        QUOTE(ROLE_SYSTEM_CURSOR),
        QUOTE(ROLE_SYSTEM_DIAGRAM),
        QUOTE(ROLE_SYSTEM_DIAL),
        QUOTE(ROLE_SYSTEM_DIALOG),
        QUOTE(ROLE_SYSTEM_DOCUMENT),
        QUOTE(ROLE_SYSTEM_DROPLIST),
        QUOTE(ROLE_SYSTEM_EQUATION),
        QUOTE(ROLE_SYSTEM_GRAPHIC),
        QUOTE(ROLE_SYSTEM_GRIP),
        QUOTE(ROLE_SYSTEM_GROUPING),
        QUOTE(ROLE_SYSTEM_HELPBALLOON),
        QUOTE(ROLE_SYSTEM_HOTKEYFIELD),
        QUOTE(ROLE_SYSTEM_INDICATOR),
        QUOTE(ROLE_SYSTEM_IPADDRESS),
        QUOTE(ROLE_SYSTEM_LINK),
        QUOTE(ROLE_SYSTEM_LIST),
        QUOTE(ROLE_SYSTEM_LISTITEM),
        QUOTE(ROLE_SYSTEM_MENUBAR),
        QUOTE(ROLE_SYSTEM_MENUITEM),
        QUOTE(ROLE_SYSTEM_MENUPOPUP),
        QUOTE(ROLE_SYSTEM_OUTLINE),
        QUOTE(ROLE_SYSTEM_OUTLINEBUTTON),
        QUOTE(ROLE_SYSTEM_OUTLINEITEM),
        QUOTE(ROLE_SYSTEM_PAGETAB),
        QUOTE(ROLE_SYSTEM_PAGETABLIST),
        QUOTE(ROLE_SYSTEM_PANE),
        QUOTE(ROLE_SYSTEM_PROGRESSBAR),
        QUOTE(ROLE_SYSTEM_PROPERTYPAGE),
        QUOTE(ROLE_SYSTEM_PUSHBUTTON),
        QUOTE(ROLE_SYSTEM_RADIOBUTTON),
        QUOTE(ROLE_SYSTEM_ROW),
        QUOTE(ROLE_SYSTEM_ROWHEADER),
        QUOTE(ROLE_SYSTEM_SCROLLBAR),
        QUOTE(ROLE_SYSTEM_SEPARATOR),
        QUOTE(ROLE_SYSTEM_SLIDER),
        QUOTE(ROLE_SYSTEM_SOUND),
        QUOTE(ROLE_SYSTEM_SPINBUTTON),
        QUOTE(ROLE_SYSTEM_SPLITBUTTON),
        QUOTE(ROLE_SYSTEM_STATICTEXT),
        QUOTE(ROLE_SYSTEM_STATUSBAR),
        QUOTE(ROLE_SYSTEM_TABLE),
        QUOTE(ROLE_SYSTEM_TEXT),
        QUOTE(ROLE_SYSTEM_TITLEBAR),
        QUOTE(ROLE_SYSTEM_TOOLBAR),
        QUOTE(ROLE_SYSTEM_TOOLTIP),
        QUOTE(ROLE_SYSTEM_WHITESPACE),
        QUOTE(ROLE_SYSTEM_WINDOW),
    };
...
void IAccessibleStateToStringVector(int32_t ia_state, std::vector<std::wstring>* result)
{
...
void IAccessible2StateToStringVector(int32_t ia2_state, std::vector<std::wstring>* result)
{
    // Note: for historical reasons these are in numerical order rather than
    // alphabetical order. Changing the order would change the order in which
    // the states are printed, which would affect a bunch of tests.
    static const PlatformConstantToNameEntry ia2_table[] = {
        QUOTE(IA2_STATE_ACTIVE), QUOTE(IA2_STATE_ARMED), QUOTE(IA2_STATE_CHECKABLE), QUOTE(IA2_STATE_DEFUNCT), QUOTE(IA2_STATE_EDITABLE),
        QUOTE(IA2_STATE_HORIZONTAL), QUOTE(IA2_STATE_ICONIFIED), QUOTE(IA2_STATE_INVALID_ENTRY), QUOTE(IA2_STATE_MANAGES_DESCENDANTS), QUOTE(IA2_STATE_MODAL),
        QUOTE(IA2_STATE_MULTI_LINE), QUOTE(IA2_STATE_REQUIRED), QUOTE(IA2_STATE_SELECTABLE_TEXT), QUOTE(IA2_STATE_SINGLE_LINE), QUOTE(IA2_STATE_STALE),
        QUOTE(IA2_STATE_SUPPORTS_AUTOCOMPLETION), QUOTE(IA2_STATE_TRANSIENT), QUOTE(IA2_STATE_VERTICAL),
...
{
    static const PlatformConstantToNameEntry id_table[] = { QUOTE(OrientationType_None), QUOTE(OrientationType_Horizontal), QUOTE(OrientationType_Vertical) };
    return GetNameForPlatformConstant(id_table, std::size(id_table), identifier);
}
...
{
    static const PlatformConstantToNameEntry id_table[] = { QUOTE(LiveSetting::Off), QUOTE(LiveSetting::Polite), QUOTE(LiveSetting::Assertive) };
    return GetNameForPlatformConstant(id_table, std::size(id_table), identifier);
}
...
BOOL CALLBACK MatchWindow(HWND hwnd, LPARAM lParam)
{
    ...
    if (title.ends_with(info->title) && (info->pattern.empty() || base::MatchPattern(base::AsStringPiece16(title), base::AsStringPiece16(info->pattern)))) {
...
HWND GetHWNDBySelector(const AXTreeSelector& selector)
{
    ...
    if (!selector.pattern.empty()) {
        ...
        LOG(ERROR) << selector.AppName() << " application is not supported on the system";
        return NULL;
    }
...
std::string RoleVariantToString(const base::win::ScopedVariant& role)
{
    ...
    if (role.type() == VT_BSTR)
        return base::WideToUTF8(std::wstring(V_BSTR(role.ptr()), SysStringLen(V_BSTR(role.ptr()))));
    return std::string();
}

COMPONENT_EXPORT(AX_PLATFORM)
std::optional<std::string> GetIAccessible2Attribute(Microsoft::WRL::ComPtr<IAccessible2> element, std::string attribute)
{
    base::win::ScopedBstr bstr;
    if (element->get_attributes(bstr.Receive()) == S_OK) {
        std::vector<std::string> ia2_attributes
            = base::SplitString(base::WideToUTF8(bstr.Get()), std::string(1, ';'), base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);

        for (const auto& str : ia2_attributes) {
            std::vector<std::string> ia2_attribute = base::SplitString(str, std::string(1, ':'), base::KEEP_WHITESPACE, base::SPLIT_WANT_ALL);
...
COMPONENT_EXPORT(AX_PLATFORM)
std::vector<Microsoft::WRL::ComPtr<IAccessible>> IAccessibleChildrenOf(Microsoft::WRL::ComPtr<IAccessible> parent)
{
...
MSAAChild::MSAAChild(IAccessible* parent, VARIANT&& variant)
    : parent_(parent)
{
...
MSAAChildren::MSAAChildren(IAccessible* parent)
{
    ...
    if (FAILED(AccessibleChildren(parent, 0, count_, children_variants.data(), &count_))) {
        count_ = 0;
        return;
    }
