...
static std::pair<uintptr_t, uintptr_t> GetModuleAddressRange(const wchar_t* module_name)
{
    MODULEINFO info;
    CHECK(GetModuleInformation(GetCurrentProcess(), GetModuleHandle(module_name), &info, sizeof(info)));
...
{
    CHECK(!base::subtle::NoBarrier_AtomicExchange(&instantiated_, 1)) << "There can be only one instance at a time.";
...
void AXEventRecorderWinUia::Thread::Init(AXEventRecorderWinUia* owner, HWND hwnd, base::RunLoop& initialization, base::RunLoop& shutdown)
{
    owner_ = owner;
    hwnd_ = hwnd;
...
void AXEventRecorderWinUia::Thread::ThreadMain()
{
    ...
    base::win::ScopedCOMInitializer com_init { base::win::ScopedCOMInitializer::kMTA };
    ...
    CHECK(SUCCEEDED(CComObject<EventHandler>::CreateInstance(&uia_event_handler_)));
    ...
    uia_->AddAutomationEventHandler(shutdown_sentinel_, root_.Get(), TreeScope::TreeScope_Subtree, cache_request_.Get(), uia_event_handler_.Get());

    // Subscribe to focus events
    uia_->AddFocusChangedEventHandler(cache_request_.Get(), uia_event_handler_.Get());
    ...
    uia_->AddPropertyChangedEventHandlerNativeArray(
        root_.Get(), TreeScope::TreeScope_Subtree, cache_request_.Get(), uia_event_handler_.Get(), &property_list[0], property_list.size());

    // Subscribe to all structure-change events
    uia_->AddStructureChangedEventHandler(root_.Get(), TreeScope_Subtree, cache_request_.Get(), uia_event_handler_.Get());
    ...
    for (EVENTID event_id = kMinEvent; event_id <= kMaxEvent; ++event_id) {
        if (event_id != UIA_StructureChangedEventId && event_id != UIA_LiveRegionChangedEventId) {
            uia_->AddAutomationEventHandler(event_id, root_.Get(), TreeScope::TreeScope_Subtree, cache_request_.Get(), uia_event_handler_.Get());
        }
    }
    ...
    uia_->AddAutomationEventHandler(UIA_LiveRegionChangedEventId, root_.Get(), TreeScope::TreeScope_Subtree, cache_request_.Get(), uia_event_handler_.Get());
...
AXEventRecorderWinUia::Thread::EventHandler::EventHandler()
{
    ...
    allowed_module_address_range_
        = GetModuleAddressRange((base::win::GetVersion() >= base::win::Version::WIN10_19H1) ? L"UIAutomationCore.dll" : L"RPCRT4.dll");
}

AXEventRecorderWinUia::Thread::EventHandler::~EventHandler() = default;

void AXEventRecorderWinUia::Thread::EventHandler::Init(AXEventRecorderWinUia::Thread* owner, Microsoft::WRL::ComPtr<IUIAutomationElement> root)
{
    owner_ = owner;
    root_ = root;
}
...
IFACEMETHODIMP
AXEventRecorderWinUia::Thread::EventHandler::HandleFocusChangedEvent(IUIAutomationElement* sender)
{
    ...
    owner_->uia_->CreatePropertyCondition(UIA_RuntimeIdPropertyId, id_variant, &condition);
    CHECK(condition);
    root_->FindFirst(TreeScope::TreeScope_Subtree, condition.Get(), &element_found);
    if (!element_found) {
    ...
    std::string log = base::StringPrintf("AutomationFocusChanged %s", GetSenderInfo(sender).c_str());
    owner_->OnEvent(log);
...
IFACEMETHODIMP
AXEventRecorderWinUia::Thread::EventHandler::HandlePropertyChangedEvent(IUIAutomationElement* sender, PROPERTYID property_id, VARIANT new_value)
{
    if (!owner_ || !IsCallerFromAllowedModule(RETURN_ADDRESS()))
        return S_OK;
    ...
    std::string log = base::StringPrintf("%s changed %s", prop_str.c_str(), GetSenderInfo(sender).c_str());
    owner_->OnEvent(log);
    return S_OK;
}

IFACEMETHODIMP
AXEventRecorderWinUia::Thread::EventHandler::HandleStructureChangedEvent(IUIAutomationElement* sender, StructureChangeType change_type, SAFEARRAY* runtime_id)
{
    if (!owner_ || !IsCallerFromAllowedModule(RETURN_ADDRESS()))
        return S_OK;
    ...
    std::string log = base::StringPrintf("StructureChanged/%s %s", type_str.c_str(), GetSenderInfo(sender).c_str());
    owner_->OnEvent(log);
    return S_OK;
}

IFACEMETHODIMP
AXEventRecorderWinUia::Thread::EventHandler::HandleAutomationEvent(IUIAutomationElement* sender, EVENTID event_id)
{
    if (!owner_ || !IsCallerFromAllowedModule(RETURN_ADDRESS()))
        return S_OK;
    ...
    if (event_id == owner_->shutdown_sentinel_) {
        ...
        std::string sender_info = event_id == UIA_MenuClosedEventId ? "" : GetSenderInfo(sender);
        std::string log = base::StringPrintf("%s %s", event_str.c_str(), sender_info.c_str());
        owner_->OnEvent(log);
    }
    return S_OK;
}
...
bool AXEventRecorderWinUia::Thread::EventHandler::IsCallerFromAllowedModule(void* return_address)
{
    const auto address = reinterpret_cast<uintptr_t>(return_address);
    return address >= allowed_module_address_range_.first && address < allowed_module_address_range_.second;
}

std::string AXEventRecorderWinUia::Thread::EventHandler::GetSenderInfo(IUIAutomationElement* sender)
{
    ...
    auto append_property = [&](const char* name, auto getter) {
        ...
        if (bstr.Length() > 0) {
            sender_info += base::StringPrintf("%s%s=%s", sender_info.empty() ? "" : ", ", name, BstrToUTF8(bstr.Get()).c_str());
        }
    };
