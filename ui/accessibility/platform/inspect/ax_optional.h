...
template <typename... Args> struct is_variant<absl::variant<Args...>> : std::true_type { };
...
template <typename ValueType> class COMPONENT_EXPORT(AX_PLATFORM) AXOptional final {
public:
    static constexpr AXOptional Unsupported()
    {
        return AXOptional(kUnsupported);
    }
    ...
    {
        return AXOptional(other_value_, other_value_ != nullptr ? kValue : kNotApplicable);
    }
    ...
    explicit constexpr AXOptional(ValueType&& value_)
        : value_(std::forward<ValueType>(value_))
        , state_(kValue)
    {
    }

    bool constexpr IsUnsupported() const
    {
        return state_ == kUnsupported;
    }
    bool constexpr IsNotApplicable() const
    {
        return state_ == kNotApplicable;
    }
    bool constexpr IsError() const
    {
        return state_ == kError;
    }

    template <typename T = ValueType> bool constexpr IsNotNull(typename std::enable_if<!is_variant_v<T>>::type* = 0) const
    {
    ...
    bool HasStateText() const
    {
        return !state_text_.empty();
    }
...
private:
    ...
    explicit constexpr AXOptional(State state, const std::string& state_text = {}, typename std::enable_if<!is_variant_v<T>>::type* = 0)
        : value_(nullptr)
        , state_(state)
