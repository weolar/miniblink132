...
AXTargetWin::AXTargetWin(std::nullptr_t)
    : value_(nullptr)
{
}
...
std::string AXTargetWin::ToString() const
{
    ...
    if (Is<ScopedCoMemArray<IA2TextSelection>>()) {
        std::string str;
        for (const IA2TextSelection& selection : As<ScopedCoMemArray<IA2TextSelection>>()) {
            if (!str.empty()) {
                str += ", ";
            }
            ...
            str += "{startObj: <obj>, startOffset: " + base::NumberToString(selection.startOffset)
                + ", endObj: <obj>, endOffset: " + base::NumberToString(selection.endOffset) + "}";
        }
        return '[' + str + ']';
    }
