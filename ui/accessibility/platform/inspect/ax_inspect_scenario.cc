...
AXInspectScenario::AXInspectScenario(const std::vector<AXPropertyFilter>& default_filters)
...
std::optional<AXInspectScenario> AXInspectScenario::From(
    const std::string& directive_prefix, const base::FilePath& scenario_path, const std::vector<AXPropertyFilter>& default_filters)
{
    ...
    std::string file_contents;
    {
        ...
        if (!base::ReadFileToString(scenario_path, &file_contents)) {
            LOG(ERROR) << "Failed to open a file to extract an inspect scenario: " << scenario_path;
            return std::nullopt;
        }
    }
    ...
    if (scenario_path.Extension() == FILE_PATH_LITERAL(".html")) {
        ...
        if (scenario_start != std::string::npos && scenario_end != std::string::npos) {
...
AXInspectScenario AXInspectScenario::From(
    const std::string& directive_prefix, const std::vector<std::string>& lines, const std::vector<AXPropertyFilter>& default_filters)
{
    ...
    for (const std::string& line : lines) {
        ...
        if (!line.starts_with("@")) {
            if (directive != kNone) {
                std::string_view trimmed_value = base::TrimWhitespaceASCII(line, base::TRIM_ALL);
...
// static
AXInspectScenario::Directive AXInspectScenario::ParseDirective(const std::string& directive_prefix, std::string_view directive)
{
    static constexpr auto kMapping = base::MakeFixedFlatMap<std::string_view, Directive>({ { "NO-LOAD-EXPECTED", kNoLoadExpected }, { "WAIT-FOR", kWaitFor },
        { "EXECUTE-AND-WAIT-FOR", kExecuteAndWaitFor }, { "DEFAULT-ACTION-ON", kDefaultActionOn }, { "ALLOW", kPropertyFilterAllow },
        { "ALLOW-EMPTY", kPropertyFilterAllowEmpty }, { "DENY", kPropertyFilterDeny }, { "SCRIPT", kScript }, { "DENY-NODE", kNodeFilter } });

    if (!directive.starts_with('@')) {
        return kNone;
    }

    auto trimmed_directive = directive.starts_with(directive_prefix) ? directive.substr(directive_prefix.size()) : directive.substr(1);
...
void AXInspectScenario::ProcessDirective(Directive directive, std::string_view value)
{
    switch (directive) {
    ...
    case kPropertyFilterAllow:
        property_filters.emplace_back(std::string(value), AXPropertyFilter::ALLOW);
        break;
    case kPropertyFilterAllowEmpty:
        property_filters.emplace_back(std::string(value), AXPropertyFilter::ALLOW_EMPTY);
        break;
    ...
    case kNodeFilter: {
        ...
        if (parts) {
            node_filters.emplace_back(
                std::string(base::TrimWhitespaceASCII(parts->first, base::TRIM_ALL)), std::string(base::TrimWhitespaceASCII(parts->second, base::TRIM_ALL)));
        } else {
