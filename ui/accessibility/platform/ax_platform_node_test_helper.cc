...
int AXPlatformNodeTestHelper::GetTreeSize(AXPlatformNode* ax_node)
{
    ...
    for (size_t i = 0; i < delegate->GetChildCount(); ++i) {
        AXPlatformNode* child_node = AXPlatformNode::FromNativeViewAccessible(delegate->ChildAtIndex(i));
...
// static
AXPlatformNode* AXPlatformNodeTestHelper::FindChildByName(AXPlatformNode* ax_node, const std::string& name)
{
    if (!ax_node)
        return nullptr;
    ...
    for (size_t i = 0; i < delegate->GetChildCount(); ++i) {
        AXPlatformNode* result_from_child = FindChildByName(AXPlatformNode::FromNativeViewAccessible(delegate->ChildAtIndex(i)), name);
