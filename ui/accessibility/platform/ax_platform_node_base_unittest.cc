...
TEST_F(AXPlatformNodeTest, GetHypertext)
{
    ...
    AXPlatformNodeBase* root = static_cast<AXPlatformNodeBase*>(TestAXNodeWrapper::GetOrCreate(tree, tree->root())->ax_platform_node());

    EXPECT_EQ(root->GetHypertext(), u"text1text2text3");

    AXPlatformNodeBase* text1 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(0)));
    EXPECT_EQ(text1->GetHypertext(), u"text1");

    AXPlatformNodeBase* text2 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(1)));
    EXPECT_EQ(text2->GetHypertext(), u"text2");

    AXPlatformNodeBase* text3 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(2)));
...
TEST_F(AXPlatformNodeTest, GetHypertextIgnoredContainerSiblings)
{
    ...
    AXTreeUpdate update;
    update.root_id = 1;
    update.nodes = { root_data, container1, container2, container3, item1, item2, item3 };
    ...
    AXPlatformNodeBase* text1_ignored_container = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(0)));
    EXPECT_EQ(text1_ignored_container->GetHypertext(), u"text1");

    AXPlatformNodeBase* text2_ignored_container = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(1)));
    EXPECT_EQ(text2_ignored_container->GetHypertext(), u"text2");

    AXPlatformNodeBase* text3_ignored_container = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(2)));
...
TEST_F(AXPlatformNodeTest, GetTextContentIgnoresInvisibleAndIgnored)
{
    ...
    AXTree* tree = Init(update);
    auto* root = static_cast<AXPlatformNodeBase*>(TestAXNodeWrapper::GetOrCreate(tree, tree->root())->ax_platform_node());
...
TEST_F(AXPlatformNodeTest, TestSelectedChildrenWithGroup)
{
    ...
    AXTreeUpdate update;
    update.root_id = 1;
    update.nodes = { root_data, group_1_data, group_2_data, item_1_data, item_2_data, item_3_data, item_4_data };
    Init(update);
    ...
    gfx::NativeViewAccessible first_group_child
        = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(0)))->ChildAtIndex(0);
    AXPlatformNodeBase* first_selected_node = root->GetSelectedItem(0);
    EXPECT_EQ(first_group_child, first_selected_node->GetNativeViewAccessible());

    gfx::NativeViewAccessible second_group_child
        = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(1)))->ChildAtIndex(1);
    AXPlatformNodeBase* second_selected_node = root->GetSelectedItem(1);
    EXPECT_EQ(second_group_child, second_selected_node->GetNativeViewAccessible());
}
...
TEST_F(AXPlatformNodeTest, TestSelectedChildrenMixed)
{
    ...
    AXTreeUpdate update;
    update.root_id = 1;
    update.nodes = { root_data, item_1_data, group_1_data, item_2_data, item_3_data, group_2_data, item_4_data, item_5_data, item_6_data };
    Init(update);
    ...
    gfx::NativeViewAccessible first_group_child
        = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(1)))->ChildAtIndex(0);
    AXPlatformNodeBase* second_selected_node = root->GetSelectedItem(1);
    EXPECT_EQ(first_group_child, second_selected_node->GetNativeViewAccessible());

    gfx::NativeViewAccessible second_group_child
        = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(2)))->ChildAtIndex(1);
...
TEST_F(AXPlatformNodeTest, CompareTo)
{
  ...
  // n4  n5  n6       n10
    ...
    AXTreeUpdate update;
    update.root_id = 1;
    update.nodes = { node1, node2, node3, node4, node5, node6, node7, node8, node9, node10 };

    Init(update);

    AXTree& tree = *GetTree();
    // Retrieve the nodes in a level-order traversal way.
    auto* n1 = static_cast<AXPlatformNodeBase*>(TestAXNodeWrapper::GetOrCreate(&tree, tree.root())->ax_platform_node());
    auto* n2 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(n1->ChildAtIndex(0)));
    auto* n3 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(n2->ChildAtIndex(0)));
    auto* n8 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(n2->ChildAtIndex(1)));
    auto* n9 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(n2->ChildAtIndex(2)));
    auto* n4 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(n3->ChildAtIndex(0)));
    auto* n5 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(n3->ChildAtIndex(1)));
    auto* n6 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(n3->ChildAtIndex(2)));
    auto* n10 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(n9->ChildAtIndex(0)));
    auto* n7 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(n6->ChildAtIndex(0)));

    // Test for two nodes that do not share the same root. They should not be
    // comparable.
    AXPlatformNodeDelegate detached_delegate;
    AXPlatformNodeBase* detached_node = static_cast<AXPlatformNodeBase*>(AXPlatformNode::Create(&detached_delegate));
    ...
    std::vector<AXPlatformNodeBase*> preorder_tree_nodes = { n1, n2, n3, n4, n5, n6, n7, n8, n9, n10 };
    ...
    for (auto* lhs : preorder_tree_nodes) {
        for (auto* rhs : preorder_tree_nodes) {
            ...
            SCOPED_TRACE(::testing::Message() << "lhs.id=" << base::NumberToString(lhs->GetData().id) << ", rhs.id=" << base::NumberToString(rhs->GetData().id)
                                              << ", lhs->CompareTo(*rhs)={actual:" << base::NumberToString(actual_result)
                                              << ", expected:" << base::NumberToString(expected_result) << "}");
...
TEST_F(AXPlatformNodeTest, HypertextOffsetFromEndpoint)
{
    ...
    auto* paragraph = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(root->ChildAtIndex(0)));

    auto* link = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(paragraph->ChildAtIndex(0)));

    auto* static_text = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(link->ChildAtIndex(0)));

    auto* static_text2 = static_cast<AXPlatformNodeBase*>(AXPlatformNode::FromNativeViewAccessible(link->ChildAtIndex(1)));
