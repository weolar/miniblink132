...
// static
AXPlatformNodeTextProviderWin* AXPlatformNodeTextProviderWin::Create(AXPlatformNodeWin* owner)
{
    CComObject<AXPlatformNodeTextProviderWin>* text_provider = nullptr;
    if (SUCCEEDED(CComObject<AXPlatformNodeTextProviderWin>::CreateInstance(&text_provider))) {
...
// static
void AXPlatformNodeTextProviderWin::CreateIUnknown(AXPlatformNodeWin* owner, IUnknown** unknown)
{
    Microsoft::WRL::ComPtr<AXPlatformNodeTextProviderWin> text_provider(Create(owner));
    if (text_provider)
...
HRESULT AXPlatformNodeTextProviderWin::GetSelection(SAFEARRAY** selection)
{
    ...
    *selection = nullptr;
    ...
    AXNodePosition::AXPositionInstance start = anchor_object->GetDelegate()->CreatePositionAt(start_offset);
    AXNodePosition::AXPositionInstance end = focus_object->GetDelegate()->CreatePositionAt(end_offset);
    ...
    AXPlatformNodeTextRangeProviderWin::CreateTextRangeProvider(std::move(start), std::move(end), &text_range_provider);
    ...
    base::win::ScopedSafearray selections_to_return(SafeArrayCreateVector(/* element type */ VT_UNKNOWN, /* lower bound */ 0,
    ...
    LONG index = 0;
    HRESULT hr = SafeArrayPutElement(selections_to_return.Get(), &index, text_range_provider.Get());
    DCHECK(SUCCEEDED(hr));
...
HRESULT AXPlatformNodeTextProviderWin::GetVisibleRanges(SAFEARRAY** visible_ranges)
{
    ...
    // correct bounds
    const gfx::Rect frame_rect = delegate->GetBoundsRect(AXCoordinateSystem::kFrame, AXClippingBehavior::kClipped);
    ...
    while (!current_line_start->IsNullPosition() && *current_line_start < *end) {
        auto current_line_end
            = current_line_start->CreateNextLineEndPosition({ AXBoundaryBehavior::kCrossBoundary, AXBoundaryDetection::kDontCheckInitialPosition });
        ...
        gfx::Rect current_rect = delegate->GetInnerTextRangeBoundsRect(
            current_line_start->text_offset(), current_line_end->text_offset(), AXCoordinateSystem::kFrame, AXClippingBehavior::kUnclipped);
        ...
        if (frame_rect.Intersects(current_rect)) {
            ...
            AXPlatformNodeTextRangeProviderWin::CreateTextRangeProvider(
                current_line_start->AsLeafTextPosition(), current_line_end->AsLeafTextPosition(), &text_range_provider);

            ranges.emplace_back(text_range_provider);
        }

        current_line_start
            = current_line_start->CreateNextLineStartPosition({ AXBoundaryBehavior::kCrossBoundary, AXBoundaryDetection::kDontCheckInitialPosition });
    }

    base::win::ScopedSafearray scoped_visible_ranges(SafeArrayCreateVector(/* element type */ VT_UNKNOWN, /* lower bound */ 0,
    ...
    for (Microsoft::WRL::ComPtr<ITextRangeProvider>& current_provider : ranges) {
        HRESULT hr = SafeArrayPutElement(scoped_visible_ranges.Get(), &index, current_provider.Get());
        DCHECK(SUCCEEDED(hr));
...
HRESULT AXPlatformNodeTextProviderWin::RangeFromChild(IRawElementProviderSimple* child, ITextRangeProvider** range)
{
...
HRESULT AXPlatformNodeTextProviderWin::RangeFromPoint(UiaPoint uia_point, ITextRangeProvider** range)
{
    ...
    *range = nullptr;
    ...
    AXPlatformNodeWin* nearest_node = static_cast<AXPlatformNodeWin*>(owner()->NearestLeafToPoint(point));
    DCHECK(nearest_node);
    DCHECK(nearest_node->IsLeaf());

    AXNodePosition::AXPositionInstance start, end;
    start = nearest_node->GetDelegate()->CreateTextPositionAt(nearest_node->NearestTextIndexToPoint(point));
    DCHECK(!start->IsNullPosition());
    end = start->Clone();

    AXPlatformNodeTextRangeProviderWin::CreateTextRangeProvider(std::move(start), std::move(end), range);

    return S_OK;
}

HRESULT AXPlatformNodeTextProviderWin::get_DocumentRange(ITextRangeProvider** range)
{
...
HRESULT AXPlatformNodeTextProviderWin::get_SupportedTextSelection(enum SupportedTextSelection* text_selection)
{
...
HRESULT AXPlatformNodeTextProviderWin::GetActiveComposition(ITextRangeProvider** range)
{
...
HRESULT AXPlatformNodeTextProviderWin::GetConversionTarget(ITextRangeProvider** range)
{
...
void AXPlatformNodeTextProviderWin::GetRangeFromChild(AXPlatformNodeWin* ancestor, AXPlatformNodeWin* descendant, ITextRangeProvider** range)
{
    DCHECK(ancestor);
    DCHECK(descendant);
    ...
    AXNodePosition::AXPositionInstance start = descendant->GetDelegate()->CreateTextPositionAt(0)->AsLeafTextPosition();
    ...
    if (descendant->IsPlatformDocument()) {
        ...
        end = start->CreatePositionAtEndOfContent()->AsUnignoredPosition(AXPositionAdjustmentBehavior::kMoveBackward);
    } else if (descendant->GetChildCount() == 0) {
        end = descendant->GetDelegate()->CreateTextPositionAt(0)->CreatePositionAtEndOfAnchor()->AsLeafTextPosition();
    } else {
        ...
        end = deepest_last_child->GetDelegate()->CreateTextPositionAt(0)->CreatePositionAtEndOfAnchor()->AsLeafTextPosition();
    }

    AXPlatformNodeTextRangeProviderWin::CreateTextRangeProvider(std::move(start), std::move(end), range);
}

void AXPlatformNodeTextProviderWin::CreateDegenerateRangeAtStart(AXPlatformNodeWin* node, ITextRangeProvider** text_range_provider)
{
    DCHECK(node);
...
HRESULT
AXPlatformNodeTextProviderWin::GetTextRangeProviderFromActiveComposition(ITextRangeProvider** range)
{
    *range = nullptr;
    ...
    if ((AXPlatformNode::FromNativeViewAccessible(owner()->GetDelegate()->GetFocus()) == static_cast<AXPlatformNode*>(owner()))
        && owner()->HasActiveComposition()) {
        gfx::Range active_composition_offset = owner()->GetActiveCompositionOffsets();
        AXNodePosition::AXPositionInstance start = owner()->GetDelegate()->CreateTextPositionAt(
            /*offset*/ active_composition_offset.start());
        AXNodePosition::AXPositionInstance end = owner()->GetDelegate()->CreateTextPositionAt(
