...
// (and vice versa).
std::map<std::string, std::pair<AXTreeID, int32_t>>& GetAppIDToParentTreeNodeMap()
{
    static base::NoDestructor<std::map<std::string, std::pair<AXTreeID, int32_t>>> app_id_to_tree_node_map;
    return *app_id_to_tree_node_map;
}

std::map<std::string, std::vector<AppNodeInfo>>& GetAppIDToTreeNodeMap()
{
    static base::NoDestructor<std::map<std::string, std::vector<AppNodeInfo>>> app_id_to_tree_node_map;
    return *app_id_to_tree_node_map;
}

AutomationAXTreeWrapper::AutomationAXTreeWrapper(AutomationTreeManagerOwner* owner)
    : AXTreeManager(std::make_unique<AXTree>())
    , owner_(owner)
{
}
...
AutomationAXTreeWrapper::~AutomationAXTreeWrapper()
{
    ...
    // tree).
    std::map<AXTreeID, AutomationAXTreeWrapper*>& child_tree_id_reverse_map = GetChildTreeIDReverseMap();
    const auto& child_tree_ids = ax_tree_->GetAllChildTreeIds();
    std::erase_if(child_tree_id_reverse_map, [&child_tree_ids](auto& pair) { return child_tree_ids.count(pair.first); });
}

// static
AutomationAXTreeWrapper* AutomationAXTreeWrapper::GetParentOfTreeId(AXTreeID tree_id)
{
    std::map<AXTreeID, AutomationAXTreeWrapper*>& child_tree_id_reverse_map = GetChildTreeIDReverseMap();
...
bool AutomationAXTreeWrapper::OnAccessibilityEvents(
    const AXTreeID& tree_id, const std::vector<AXTreeUpdate>& updates, const std::vector<AXEvent>& events, gfx::Point mouse_location)
{
    TRACE_EVENT0("accessibility", "AutomationAXTreeWrapper::OnAccessibilityEvents");

    std::optional<gfx::Rect> previous_accessibility_focused_global_bounds = owner_->GetAccessibilityFocusedLocation();
    ...
    for (const auto& update : updates) {
        ...
        if (!ax_tree_->Unserialize(update)) {
            static crash_reporter::CrashKeyString<4> crash_key("ax-tree-wrapper-unserialize-failed");
            crash_key.Set("yes");
        ...
        if (update.nodes.size() && did_send_tree_change_during_unserialization_) {
            owner_->SendTreeChangeEvent(ax::mojom::Mutation::kSubtreeUpdateEnd, ax_tree(), ax_tree_->root());
        }
    }
    ...
    for (const auto& targeted_event : event_generator_) {
        if (ShouldIgnoreGeneratedEventForAutomation(targeted_event.event_params->event)) {
            continue;
        }
        ...
        generated_event.event_from_action = targeted_event.event_params->event_from_action;
        generated_event.event_intents = targeted_event.event_params->event_intents;
        owner_->SendAutomationEvent(tree_id, mouse_location, generated_event, targeted_event.event_params->event);
    }
    event_generator_.ClearEvents();

    for (const auto& event : events) {
        if (event.event_type == ax::mojom::Event::kFocus || event.event_type == ax::mojom::Event::kBlur)
            continue;
    ...
    if (previous_accessibility_focused_global_bounds.has_value() && previous_accessibility_focused_global_bounds != owner_->GetAccessibilityFocusedLocation()) {
...
{
    return ax_tree_->root() ? ax_tree_->root()->GetRole() == ax::mojom::Role::kDesktop : false;
}
...
bool AutomationAXTreeWrapper::HasDeviceScaleFactor() const
{
    return ax_tree_->root() ?
        ...
        ax_tree_->root()->GetRole() != ax::mojom::Role::kDesktop && ax_tree_->root()->GetRole() != ax::mojom::Role::kClient
                            : true;
}
...
bool AutomationAXTreeWrapper::IsInFocusChain(int32_t node_id)
{
    ...
    while ((ancestor_tree = ancestor_tree->GetParentTree())) {
        ...
        AXNode* ancestor_tree_focused_node = ancestor_tree->ax_tree()->GetFromId(ancestor_tree_focus_id);
        if (!ancestor_tree_focused_node)
            return false;

        if (ancestor_tree_focused_node->HasStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId)) {
            ...
            AXNode* parent_node = descendant_tree->GetParentTreeNodeForAppID(
                ancestor_tree_focused_node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId), owner_);
            if (parent_node != ancestor_tree_focused_node)
                return false;
        } else if (AXTreeID::FromString(ancestor_tree_focused_node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeId)) != descendant_tree_id
            && ancestor_tree->ax_tree()->data().focused_tree_id != descendant_tree_id) {
...
{
    return accessibility_focused_id_ == kInvalidAXNodeID ? nullptr : ax_tree_->GetFromId(accessibility_focused_id_);
}
...
AutomationAXTreeWrapper* AutomationAXTreeWrapper::GetTreeWrapperWithUnignoredRoot()
{
...
void AutomationAXTreeWrapper::EventListenerAdded(const std::tuple<ax::mojom::Event, AXEventGenerator::Event>& event_type, AXNode* node)
{
...
void AutomationAXTreeWrapper::EventListenerRemoved(const std::tuple<ax::mojom::Event, AXEventGenerator::Event>& event_type, AXNode* node)
{
...
bool AutomationAXTreeWrapper::HasEventListener(const std::tuple<ax::mojom::Event, AXEventGenerator::Event>& event_type, AXNode* node)
{
    auto it = node_id_to_events_.find(node->id());
    if (it == node_id_to_events_.end())
        return false;
...
// static
std::map<AXTreeID, AutomationAXTreeWrapper*>& AutomationAXTreeWrapper::GetChildTreeIDReverseMap()
{
    static base::NoDestructor<std::map<AXTreeID, AutomationAXTreeWrapper*>> child_tree_id_reverse_map;
    return *child_tree_id_reverse_map;
}

// static
AXNode* AutomationAXTreeWrapper::GetParentTreeNodeForAppID(const std::string& app_id, const AutomationTreeManagerOwner* owner)
{
    ...
    AutomationAXTreeWrapper* wrapper = owner->GetAutomationAXTreeWrapperFromTreeID(it->second.first);
    if (!wrapper)
        return nullptr;
...
// static
AutomationAXTreeWrapper* AutomationAXTreeWrapper::GetParentTreeWrapperForAppID(const std::string& app_id, const AutomationTreeManagerOwner* owner)
{
    auto& map = GetAppIDToParentTreeNodeMap();
    auto it = map.find(app_id);
    if (it == map.end())
...
// static
std::vector<AXNode*> AutomationAXTreeWrapper::GetChildTreeNodesForAppID(const std::string& app_id, const AutomationTreeManagerOwner* owner)
{
    ...
    for (const AppNodeInfo& app_node_info : it->second) {
        AutomationAXTreeWrapper* wrapper = owner->GetAutomationAXTreeWrapperFromTreeID(app_node_info.tree_id);
        if (!wrapper)
            continue;
...
void AutomationAXTreeWrapper::OnNodeDataChanged(AXTree* tree, const AXNodeData& old_node_data, const AXNodeData& new_node_data)
{
    if (old_node_data.GetStringAttribute(ax::mojom::StringAttribute::kName) != new_node_data.GetStringAttribute(ax::mojom::StringAttribute::kName))
    ...
    AXTree* tree, AXNode* node, ax::mojom::StringAttribute attr, const std::string& old_value, const std::string& new_value)
{
    if (attr == ax::mojom::StringAttribute::kChildTreeNodeAppId) {
        if (new_value.empty()) {
            ...
            GetAppIDToParentTreeNodeMap()[new_value] = { tree->GetAXTreeID(), node->data().id };
        }
    }
    ...
    if (attr == ax::mojom::StringAttribute::kAppId) {
        if (new_value.empty()) {
            ...
            if (it != GetAppIDToTreeNodeMap().end()) {
                std::erase_if(it->second, [node](const AppNodeInfo& app_node_info) { return app_node_info.node_id == node->id(); });
                if (it->second.empty()) {
            ...
            GetAppIDToTreeNodeMap()[new_value].push_back({ tree->GetAXTreeID(), node->data().id });
...
{
    did_send_tree_change_during_unserialization_ |= owner_->SendTreeChangeEvent(ax::mojom::Mutation::kNodeRemoved, tree, node);
    deleted_node_ids_.push_back(node->id());
    node_id_to_events_.erase(node->id());

    if (node->HasStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId)) {
        GetAppIDToParentTreeNodeMap().erase(node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId));
    }

    if (node->HasStringAttribute(ax::mojom::StringAttribute::kAppId)) {
        const std::string& app_id = node->GetStringAttribute(ax::mojom::StringAttribute::kAppId);
...
{
    if (node->HasStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId)) {
        GetAppIDToParentTreeNodeMap()[node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId)] = { node->tree()->GetAXTreeID(), node->id() };
    }

    if (node->HasStringAttribute(ax::mojom::StringAttribute::kAppId)) {
        const std::string& app_id = node->GetStringAttribute(ax::mojom::StringAttribute::kAppId);
        GetAppIDToTreeNodeMap()[app_id].push_back({ node->tree()->GetAXTreeID(), node->id() });
        all_tree_node_app_ids_.insert(app_id);
    }
}

void AutomationAXTreeWrapper::OnAtomicUpdateFinished(AXTree* tree, bool root_changed, const std::vector<AXTreeObserver::Change>& changes)
{
    ...
    for (const auto& change : changes) {
        ...
        switch (change.type) {
        case NODE_CREATED:
            did_send_tree_change_during_unserialization_ |= owner_->SendTreeChangeEvent(ax::mojom::Mutation::kNodeCreated, tree, node);
            break;
        case SUBTREE_CREATED:
            did_send_tree_change_during_unserialization_ |= owner_->SendTreeChangeEvent(ax::mojom::Mutation::kSubtreeCreated, tree, node);
            break;
        case NODE_CHANGED:
            did_send_tree_change_during_unserialization_ |= owner_->SendTreeChangeEvent(ax::mojom::Mutation::kNodeChanged, tree, node);
            break;
        // Unhandled.
    ...
    for (int id : text_changed_node_ids_) {
        did_send_tree_change_during_unserialization_ |= owner_->SendTreeChangeEvent(ax::mojom::Mutation::kTextChanged, tree, tree->GetFromId(id));
    }
    text_changed_node_ids_.clear();
}

void AutomationAXTreeWrapper::OnIgnoredChanged(AXTree* tree, AXNode* node, bool is_ignored_new_value)
{
    owner_->SendTreeChangeEvent(is_ignored_new_value ? ax::mojom::Mutation::kNodeRemoved : ax::mojom::Mutation::kNodeCreated, tree, node);
}
