...
void AutomationTreeManagerOwner::SendNodesRemovedEvent(AXTree* tree, const std::vector<int>& ids)
{
    AXTreeID tree_id = tree->GetAXTreeID();
    GetAutomationV8Bindings()->SendNodesRemovedEvent(tree_id, ids);
}

bool AutomationTreeManagerOwner::SendTreeChangeEvent(ax::mojom::Mutation change_type, AXTree* tree, AXNode* node)
{
    ...
    if (node->HasStringAttribute(ax::mojom::StringAttribute::kChildTreeId)) {
        const std::string& child_tree_id_str = node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeId);
    ...
    for (const auto& observer : tree_change_observers_) {
        ...
        case TreeChangeObserverFilter::kLiveRegionTreeChanges:
            if (!node->HasStringAttribute(ax::mojom::StringAttribute::kContainerLiveStatus) && node->GetRole() != ax::mojom::Role::kAlert
        ...
        GetAutomationV8Bindings()->SendTreeChangeEvent(observer.id, tree_id, node->id(), change_type);
    }
...
void AutomationTreeManagerOwner::SendAutomationEvent(
    AXTreeID tree_id, const gfx::Point& mouse_location, const AXEvent& event, std::optional<AXEventGenerator::Event> generated_event_type)
{
    AutomationAXTreeWrapper* tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(tree_id);
    if (!tree_wrapper)
        return;

    // Event type is none if both the ax event and generated event (if it exists)
    // are type none or ignored.
    bool is_type_none = (event.event_type == ax::mojom::Event::kNone || ShouldIgnoreAXEventForAutomation(event.event_type))
        && (!generated_event_type.has_value()
            || (generated_event_type.value() == AXEventGenerator::Event::NONE || ShouldIgnoreGeneratedEventForAutomation(generated_event_type.value())));

    // These events get used internally to trigger other behaviors in js
    // so they should always be fired.
    bool fire_ax_event = (event.event_type == ax::mojom::Event::kHitTestResult || event.event_type == ax::mojom::Event::kMediaStartedPlaying
    ...
    std::tuple<ax::mojom::Event, AXEventGenerator::Event> event_type_tuple = MakeTupleForAutomationFromEventTypes(
        event.event_type, generated_event_type.has_value() ? generated_event_type.value() : AXEventGenerator::Event::NONE);
    ...
    GetAutomationV8Bindings()->SendAutomationEvent(tree_id, event, mouse_location, event_type_tuple);
}

AXNode* AutomationTreeManagerOwner::GetHostInParentTree(AutomationAXTreeWrapper** in_out_tree_wrapper) const
{
    AutomationAXTreeWrapper* parent_tree_wrapper = nullptr;

    AXTreeID parent_tree_id = (*in_out_tree_wrapper)->ax_tree()->data().parent_tree_id;
    if (parent_tree_id != AXTreeIDUnknown()) {
        ...
        parent_tree_wrapper = AutomationAXTreeWrapper::GetParentOfTreeId((*in_out_tree_wrapper)->GetTreeID());
    }

    if (!parent_tree_wrapper)
        return nullptr;

    std::set<int32_t> host_node_ids = parent_tree_wrapper->ax_tree()->GetNodeIdsForChildTreeId((*in_out_tree_wrapper)->GetTreeID());
    ...
    for (int32_t host_node_id : host_node_ids) {
        ...
        if (host_node) {
            DCHECK_EQ((*in_out_tree_wrapper)->GetTreeID(), AXTreeID::FromString(host_node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeId)));
...
AutomationAXTreeWrapper* AutomationTreeManagerOwner::GetAutomationAXTreeWrapperFromTreeID(AXTreeID tree_id) const
{
...
AXNode* AutomationTreeManagerOwner::GetParent(
    AXNode* node, AutomationAXTreeWrapper** in_out_tree_wrapper, bool should_use_app_id, bool requires_unignored) const
{
    if (should_use_app_id && node->HasStringAttribute(ax::mojom::StringAttribute::kAppId)) {
        AXNode* parent_app_node = AutomationAXTreeWrapper::GetParentTreeNodeForAppID(node->GetStringAttribute(ax::mojom::StringAttribute::kAppId), this);
        if (parent_app_node) {
            *in_out_tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(parent_app_node->tree()->GetAXTreeID());
...
void AutomationTreeManagerOwner::MaybeSendFocusAndBlur(AutomationAXTreeWrapper* tree, const AXTreeID& tree_id, const std::vector<AXTreeUpdate>& updates,
    const std::vector<AXEvent>& events, gfx::Point mouse_location)
{
    AXNode* old_node = nullptr;
    AutomationAXTreeWrapper* old_wrapper = GetAutomationAXTreeWrapperFromTreeID(focus_tree_id_);
    if (old_wrapper) {
    ...
    AutomationAXTreeWrapper* desktop_tree = GetAutomationAXTreeWrapperFromTreeID(desktop_tree_id_);
    ...
    if (!event_bundle_has_focus_or_blur && !lost_old_focus && !is_from_desktop && same_focused_tree)
        return;
...
std::optional<gfx::Rect> AutomationTreeManagerOwner::GetAccessibilityFocusedLocation() const
{
    if (accessibility_focused_tree_id_ == AXTreeIDUnknown())
        return std::nullopt;

    AutomationAXTreeWrapper* tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(accessibility_focused_tree_id_);
    if (!tree_wrapper)
        return std::nullopt;
...
void AutomationTreeManagerOwner::SendAccessibilityFocusedLocationChange(const gfx::Point& mouse_location)
{
    AutomationAXTreeWrapper* tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(accessibility_focused_tree_id_);
    if (!tree_wrapper)
        return;
...
bool AutomationTreeManagerOwner::GetFocusInternal(AutomationAXTreeWrapper* tree_wrapper, AutomationAXTreeWrapper** out_tree_wrapper, AXNode** out_node) const
{
    ...
    while (true) {
        ...
        const std::string& child_tree_id_str = focus->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeId);
        const std::string& child_tree_node_app_id_str = focus->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId);
        ...
        if (!child_tree_node_app_id_str.empty()) {
            std::vector<AXNode*> child_app_nodes = AutomationAXTreeWrapper::GetChildTreeNodesForAppID(child_tree_node_app_id_str, this);
            if (!child_app_nodes.empty()) {
                ...
                auto* wrapper = GetAutomationAXTreeWrapperFromTreeID(child_app_node->tree()->GetAXTreeID());
        ...
        AXTreeID focused_tree_id = child_tree_wrapper->ax_tree()->data().focused_tree_id;
        if (focused_tree_id != AXTreeIDUnknown() && !child_tree_wrapper->IsDesktopTree()) {
            AutomationAXTreeWrapper* focused_tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(child_tree_wrapper->ax_tree()->data().focused_tree_id);
...
gfx::Rect AutomationTreeManagerOwner::ComputeGlobalNodeBounds(
    AutomationAXTreeWrapper* tree_wrapper, AXNode* node, gfx::RectF local_bounds, bool* offscreen, bool clip_bounds) const
{
    gfx::RectF bounds = local_bounds;

    bool crossed_app_id = false;
    while (node) {
        bounds = tree_wrapper->ax_tree()->RelativeToTreeBounds(node, bounds, offscreen, clip_bounds,
        ...
        AXNode* parent_of_root = GetParent(tree_wrapper->ax_tree()->root(), &tree_wrapper, should_use_app_id);
        ...
        if (!parent_of_root && previous_tree_wrapper->GetParentTreeFromAnyAppID()) {
            ...
            while (found_node && !found_node->HasStringAttribute(ax::mojom::StringAttribute::kAppId)) {
                found_node = found_node->parent();
            }
            if (found_node) {
                const std::string& app_id = found_node->GetStringAttribute(ax::mojom::StringAttribute::kAppId);
                parent_of_root = AutomationAXTreeWrapper::GetParentTreeNodeForAppID(app_id, this);
                tree_wrapper = AutomationAXTreeWrapper::GetParentTreeWrapperForAppID(app_id, this);
                crossed_app_id = true;
            }
        }
        ...
        if (previous_tree_wrapper->HasDeviceScaleFactor() && !tree_wrapper->HasDeviceScaleFactor()) {
            // TODO(crbug/1234225): This calculation should be included in
            // |AXRelativeBounds::transform|.
            const float scale_factor = parent_of_root->data().GetFloatAttribute(ax::mojom::FloatAttribute::kChildTreeScale);
            if (scale_factor > 0)
...
std::vector<AXNode*> AutomationTreeManagerOwner::GetRootsOfChildTree(AXNode* node) const
{
    if (!node->HasStringAttribute(ax::mojom::StringAttribute::kChildTreeId) && !node->HasStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId)) {
    ...
    const std::string& child_tree_node_app_id_str = node->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId);
    ...
    AutomationAXTreeWrapper* child_tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(AXTreeID::FromString(child_tree_id_str));
...
AXNode* AutomationTreeManagerOwner::GetNextInTreeOrder(AXNode* start, AutomationAXTreeWrapper** in_out_tree_wrapper) const
{
    ...
    while ((parent = GetParent(walker, in_out_tree_wrapper))) {
        ...
        if ((walker->GetUnignoredIndexInParent() + 1) < parent->GetUnignoredChildCount()) {
            return parent->GetUnignoredChildAtIndex(walker->GetUnignoredIndexInParent() + 1);
        }
        walker = parent;
    }
...
AXNode* AutomationTreeManagerOwner::GetPreviousInTreeOrder(AXNode* start, AutomationAXTreeWrapper** in_out_tree_wrapper) const
{
    AXNode* walker = start;
    ...
    walker = parent->GetUnignoredChildAtIndex(walker->GetUnignoredIndexInParent() - 1);

    // Walks to deepest last child.
    while (true) {
        auto iter = walker->UnignoredChildrenBegin();
        if (iter != walker->UnignoredChildrenEnd() && --iter != walker->UnignoredChildrenEnd()) {
            walker = &(*iter);
...
std::vector<int> AutomationTreeManagerOwner::CalculateSentenceBoundary(AutomationAXTreeWrapper* tree_wrapper, AXNode* node, bool start_boundary)
{
    // Create an empty vector for storing final results and deal with the node
    // without a name.
    std::vector<int> sentence_boundary;
    std::u16string node_name = node->GetString16Attribute(ax::mojom::StringAttribute::kName);
    if (node_name.empty()) {
    ...
    AXNodePosition::AXPositionInstance head_pos
        = AXNodePosition::CreatePosition(*node, 0 /* child_index_or_text_offset */, ax::mojom::TextAffinity::kDownstream)->CreatePositionAtStartOfAnchor();
    ...
    if (!head_pos->AtStartOfParagraph()) {
        AXNodePosition::AXPositionInstance start_para_pos
            = head_pos->CreatePreviousParagraphStartPosition({ AXBoundaryBehavior::kStopAtLastAnchorBoundary, AXBoundaryDetection::kDontCheckInitialPosition });
        AXRange<AXPosition<AXNodePosition, AXNode>> pre_range(start_para_pos->Clone(), head_pos->Clone());
    ...
    AXNodePosition::AXPositionInstance end_para_pos
        = head_pos->CreateNextParagraphEndPosition({ AXBoundaryBehavior::kStopAtLastAnchorBoundary, AXBoundaryDetection::kDontCheckInitialPosition });
    AXRange<AXPosition<AXNodePosition, AXNode>> post_range(head_pos->Clone(), end_para_pos->Clone());
    post_str = post_range.GetText();

    // Calculate the boundary of the |combined_str|.
    std::u16string combined_str = pre_str + post_str;
    auto boundary_func = start_boundary ? &GetSentenceStartOffsets : &GetSentenceEndOffsets;
...
bool AutomationTreeManagerOwner::GetFocus(AXTreeID* focused_tree_id, int* node_id) const
{
    AutomationAXTreeWrapper* desktop_tree = GetAutomationAXTreeWrapperFromTreeID(desktop_tree_id_);
    AutomationAXTreeWrapper* focused_wrapper = nullptr;
    AXNode* focused_node = nullptr;
    if (desktop_tree && !GetFocusInternal(desktop_tree, &focused_wrapper, &focused_node))
        return false;
...
std::vector<int> AutomationTreeManagerOwner::GetChildIDs(AXNode* node, AXTreeID* result_tree_id) const
{
    ...
    if (!child_roots.empty()) {
        *result_tree_id = child_roots[0]->tree()->GetAXTreeID();
        ...
        for (auto iter = node->UnignoredChildrenBegin(); iter != node->UnignoredChildrenEnd(); ++iter) {
...
bool AutomationTreeManagerOwner::GetBoundsForRange(
    AutomationAXTreeWrapper* tree_wrapper, AXNode* node, int start, int end, bool clipped, gfx::Rect* result) const
{
    if (node->GetRole() != ax::mojom::Role::kInlineTextBox)
        return false;

    // Use character offsets to compute the local bounds of this subrange.
    gfx::RectF local_bounds(0, 0, node->data().relative_bounds.bounds.width(), node->data().relative_bounds.bounds.height());
    const std::string& name = node->GetStringAttribute(ax::mojom::StringAttribute::kName);
    std::vector<int> character_offsets = node->GetIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets);
    ...
    if (start >= 0 && start <= end && end <= len) {
        ...
        case ax::mojom::WritingDirection::kRtl:
            local_bounds.set_x(local_bounds.x() + local_bounds.width() - end_offset);
            local_bounds.set_width(end_offset - start_offset);
            break;
        ...
        case ax::mojom::WritingDirection::kBtt:
            local_bounds.set_y(local_bounds.y() + local_bounds.height() - end_offset);
            local_bounds.set_height(end_offset - start_offset);
            break;
        }
    }
    ...
    // transformations.
    *result = ComputeGlobalNodeBounds(tree_wrapper, node, local_bounds, nullptr, clipped /* clip_bounds */);
    return true;
}
...
bool AutomationTreeManagerOwner::GetNextTextMatch(
    AutomationAXTreeWrapper* tree_wrapper, AXNode* node, const std::string& search_str, bool backward, AXTreeID* result_tree_id, int* result_node_id) const
{
    std::u16string search_str_16 = base::UTF8ToUTF16(search_str);
    auto next = backward ? &AutomationTreeManagerOwner::GetPreviousInTreeOrder : &AutomationTreeManagerOwner::GetNextInTreeOrder;
    ...
    while (true) {
        ...
        std::u16string name = node->GetString16Attribute(ax::mojom::StringAttribute::kName);
        if (base::i18n::StringSearchIgnoringCaseAndAccents(search_str_16, name, nullptr, nullptr)) {
...
bool AutomationTreeManagerOwner::GetChildIDAtIndex(const AXTreeID& tree_id, int node_id, int index, AXTreeID* child_tree_id, int* child_node_id) const
{
    if (index < 0)
        return false;
...
bool AutomationTreeManagerOwner::GetAccessibilityFocus(AXTreeID* tree_id, int* node_id) const
{
    AutomationAXTreeWrapper* tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(accessibility_focused_tree_id_);
    if (!tree_wrapper)
        return false;
...
void AutomationTreeManagerOwner::AddTreeChangeObserver(int observer_id, TreeChangeObserverFilter filter)
{
...
{
    for (auto iter = tree_change_observers_.begin(); iter != tree_change_observers_.end(); ++iter) {
...
bool AutomationTreeManagerOwner::ShouldSendTreeChangeEvent(ax::mojom::Mutation change_type, AXTree* tree, AXNode* node) const
{
    ...
    auto* tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(tree->GetAXTreeID());
    if (!tree_wrapper || tree_wrapper->IsTreeIgnored())
        return false;

    bool has_filter = false;
    if (tree_change_observer_overall_filter_ & (1 << TreeChangeObserverFilter::kLiveRegionTreeChanges)) {
        if (node->HasStringAttribute(ax::mojom::StringAttribute::kContainerLiveStatus) || node->GetRole() == ax::mojom::Role::kAlert
            || change_type == ax::mojom::Mutation::kSubtreeUpdateEnd) {
            has_filter = true;
        }
    ...
    if (tree_change_observer_overall_filter_ & (1 << TreeChangeObserverFilter::kTextMarkerChanges)) {
        if (node->HasIntListAttribute(ax::mojom::IntListAttribute::kMarkerTypes))
            has_filter = true;
    }

    if (tree_change_observer_overall_filter_ & (1 << TreeChangeObserverFilter::kAllTreeChanges))
        has_filter = true;

    return has_filter;
}

void AutomationTreeManagerOwner::DestroyAccessibilityTree(const AXTreeID& tree_id)
{
    auto& child_tree_id_reverse_map = AutomationAXTreeWrapper::GetChildTreeIDReverseMap();
    std::erase_if(child_tree_id_reverse_map,
        [tree_id](const std::pair<AXTreeID, AutomationAXTreeWrapper*>& pair) { return pair.first == tree_id || pair.second->GetTreeID() == tree_id; });
...
bool AutomationTreeManagerOwner::CalculateNodeState(const AXTreeID& tree_id, int node_id, uint32_t* node_state, bool* offscreen, bool* focused) const
{
    AutomationAXTreeWrapper* tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(tree_id);
    if (!tree_wrapper)
        return false;
...
{
    if (tree_id != accessibility_focused_tree_id_ && accessibility_focused_tree_id_ != AXTreeIDUnknown()) {
        AutomationAXTreeWrapper* previous_tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(accessibility_focused_tree_id_);
...
void AutomationTreeManagerOwner::CacheAutomationTreeWrapperForTreeID(const AXTreeID& tree_id, AutomationAXTreeWrapper* tree_wrapper)
{
    tree_id_to_tree_wrapper_map_.insert(std::make_pair(tree_id, base::WrapUnique(tree_wrapper)));
}

void AutomationTreeManagerOwner::RemoveAutomationTreeWrapperFromCache(const AXTreeID& tree_id)
{
...
{
    auto& child_tree_id_reverse_map = AutomationAXTreeWrapper::GetChildTreeIDReverseMap();
    std::erase_if(child_tree_id_reverse_map, [this](const std::pair<AXTreeID, AutomationAXTreeWrapper*>& pair) { return pair.second->owner() == this; });

    ClearCachedAutomationTreeWrappers();
}

void AutomationTreeManagerOwner::TreeEventListenersChanged(AutomationAXTreeWrapper* tree_wrapper)
{
...
void AutomationTreeManagerOwner::MaybeSendOnAllAutomationEventListenersRemoved()
{
...
void AutomationTreeManagerOwner::DispatchTreeDestroyedEvent(const AXTreeID& tree_id)
{
    GetAutomationV8Bindings()->SendTreeDestroyedEvent(tree_id);
}

void AutomationTreeManagerOwner::DispatchAccessibilityEvents(
    const AXTreeID& tree_id, const std::vector<AXTreeUpdate>& updates, const gfx::Point& mouse_location, const std::vector<AXEvent>& events)
{
    AutomationAXTreeWrapper* tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(tree_id);
    ...
    if (!tree_wrapper->OnAccessibilityEvents(tree_id, updates, events, mouse_location)) {
...
void AutomationTreeManagerOwner::DispatchAccessibilityLocationChange(const AXTreeID& tree_id, int32_t node_id, const AXRelativeBounds& bounds)
{
    AutomationAXTreeWrapper* tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(tree_id);
    if (!tree_wrapper) {
        return;
    ...
    std::optional<gfx::Rect> previous_accessibility_focused_global_bounds = GetAccessibilityFocusedLocation();

    node->SetLocation(bounds.offset_container_id, bounds.bounds, bounds.transform.get());

    if (previous_accessibility_focused_global_bounds.has_value() && previous_accessibility_focused_global_bounds != GetAccessibilityFocusedLocation()) {
        SendAccessibilityFocusedLocationChange(gfx::Point());
    }
}

void AutomationTreeManagerOwner::DispatchAccessibilityScrollChange(const AXTreeID& tree_id, int32_t node_id, int32_t scroll_x, int32_t scroll_y)
{
    AutomationAXTreeWrapper* tree_wrapper = GetAutomationAXTreeWrapperFromTreeID(tree_id);
    if (!tree_wrapper) {
        return;
...
void AutomationTreeManagerOwner::DispatchActionResult(const AXActionData& data, bool result)
{
    GetAutomationV8Bindings()->SendActionResultEvent(data, result);
}

#if BUILDFLAG(IS_CHROMEOS)
void AutomationTreeManagerOwner::DispatchGetTextLocationResult(const AXActionData& data, const std::optional<gfx::Rect>& rect)
{
