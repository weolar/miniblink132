...
public:
    ...
    virtual TreeChangeObserverFilter ParseTreeChangeObserverFilter(const std::string& filter) const = 0;
    ...
    virtual std::string GetLocalizedStringForImageAnnotationStatus(ax::mojom::ImageAnnotationStatus status) const = 0;
    virtual std::string GetTreeChangeTypeString(ax::mojom::Mutation change_type) const = 0;
    virtual std::string GetEventTypeString(const std::tuple<ax::mojom::Event, AXEventGenerator::Event>& event_type) const = 0;

    //
    // Methods for routing Javascript methods to C++.
    //

    virtual void RouteHandlerFunction(const std::string& name, scoped_refptr<V8HandlerFunctionWrapper> handler_function_wrapper) = 0;
    ...
    virtual void DispatchEvent(const std::string& event_name, const base::Value::List& event_args) const = 0;
};
} // namespace ui
