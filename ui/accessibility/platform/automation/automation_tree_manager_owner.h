...
class COMPONENT_EXPORT(AX_PLATFORM) AutomationTreeManagerOwner : public ax::mojom::Automation {
public:
    ...
    bool SendTreeChangeEvent(ax::mojom::Mutation change_type, AXTree* tree, AXNode* node);

    // Sends an AXEvent to automation in V8.
    void SendAutomationEvent(AXTreeID tree_id, const gfx::Point& mouse_location, const AXEvent& event,
        std::optional<AXEventGenerator::Event> generated_event_type = std::optional<AXEventGenerator::Event>());

    // Gets the hosting node in a parent tree.
    AXNode* GetHostInParentTree(AutomationAXTreeWrapper** in_out_tree_wrapper) const;

    AutomationAXTreeWrapper* GetAutomationAXTreeWrapperFromTreeID(AXTreeID tree_id) const;
    ...
    AXNode* GetParent(AXNode* node, AutomationAXTreeWrapper** in_out_tree_wrapper, bool should_use_app_id = true, bool requires_unignored = true) const;

    void MaybeSendFocusAndBlur(AutomationAXTreeWrapper* tree, const AXTreeID& tree_id, const std::vector<AXTreeUpdate>& updates,
    ...
    gfx::Rect ComputeGlobalNodeBounds(
        AutomationAXTreeWrapper* tree_wrapper, AXNode* node, gfx::RectF local_bounds = gfx::RectF(), bool* offscreen = nullptr, bool clip_bounds = true) const;

    std::vector<int> CalculateSentenceBoundary(AutomationAXTreeWrapper* tree_wrapper, AXNode* node, bool start_boundary);
    ...
    bool GetBoundsForRange(AutomationAXTreeWrapper* tree_wrapper, AXNode* node, int start, int end, bool clipped, gfx::Rect* result) const;

    const char* GetName(AXNode* node) const;

    bool GetNextTextMatch(
    ...
    bool GetChildIDAtIndex(const AXTreeID& ax_tree_id, int node_id, int index, AXTreeID* child_tree_id, int* child_node_id) const;
    ...
    bool ShouldSendTreeChangeEvent(ax::mojom::Mutation change_type, AXTree* tree, AXNode* node) const;
    ...
    bool CalculateNodeState(const AXTreeID& tree_id, int node_id, uint32_t* node_state, bool* offscreen, bool* focused) const;

    void SetAccessibilityFocus(AXTreeID tree_id);

    void SetDesktopTreeId(AXTreeID tree_id)
    {
        desktop_tree_id_ = tree_id;
    }
...
protected:
    ...
    void DispatchAccessibilityEvents(
        const AXTreeID& tree_id, const std::vector<AXTreeUpdate>& updates, const gfx::Point& mouse_location, const std::vector<AXEvent>& events) override;
    void DispatchAccessibilityLocationChange(const AXTreeID& tree_id, int32_t node_id, const AXRelativeBounds& bounds) override;
    void DispatchAccessibilityScrollChange(const AXTreeID& tree_id, int32_t node_id, int32_t scroll_x, int32_t scroll_y) override;
    void DispatchActionResult(const AXActionData& data, bool result) override;

#if BUILDFLAG(IS_CHROMEOS)
    void DispatchGetTextLocationResult(const AXActionData& data, const std::optional<gfx::Rect>& rect) override;
...
private:
    ...
    AXNode* GetNextInTreeOrder(AXNode* start, AutomationAXTreeWrapper** in_out_tree_wrapper) const;

    AXNode* GetPreviousInTreeOrder(AXNode* start, AutomationAXTreeWrapper** in_out_tree_wrapper) const;
    ...
    bool GetFocusInternal(AutomationAXTreeWrapper* top_tree, AutomationAXTreeWrapper** out_tree, AXNode** out_node) const;

    void CacheAutomationTreeWrapperForTreeID(const AXTreeID& tree_id, AutomationAXTreeWrapper* tree_wrapper);
    ...
    std::map<AXTreeID, std::unique_ptr<AutomationAXTreeWrapper>> tree_id_to_tree_wrapper_map_;
