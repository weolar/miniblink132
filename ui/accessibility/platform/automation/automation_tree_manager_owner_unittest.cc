...
public:
    ...
    FakeAutomationTreeManagerOwner(const FakeAutomationTreeManagerOwner&) = delete;
    FakeAutomationTreeManagerOwner& operator=(const FakeAutomationTreeManagerOwner&) = delete;
...
public:
    FakeAutomationV8Router()
    {
        ...
        isolate_holder_ = std::make_unique<gin::IsolateHolder>(
            base::SingleThreadTaskRunner::GetCurrentDefault(), gin::IsolateHolder::kSingleThread, gin::IsolateHolder::IsolateType::kUtility);
    }
    ...
    TreeChangeObserverFilter ParseTreeChangeObserverFilter(const std::string& filter) const override
    {
    ...
    std::string GetLocalizedStringForImageAnnotationStatus(ax::mojom::ImageAnnotationStatus status) const override
    {
        return ToString(status);
    }

    std::string GetTreeChangeTypeString(ax::mojom::Mutation change_type) const override
    {
        return ToString(change_type);
    }

    std::string GetEventTypeString(const std::tuple<ax::mojom::Event, AXEventGenerator::Event>& event_type) const override
    {
    ...
    void RouteHandlerFunction(const std::string& name, scoped_refptr<V8HandlerFunctionWrapper> handler_function_wrapper) override
    {
    }

    void DispatchEvent(const std::string& event_name, const base::Value::List& event_args) const override
    {
        if (!notify_event_ && !notify_tree_destroyed_ && !notify_get_text_location_) {
            return;
        }

        if (notify_event_ && event_name == "automationInternal.onAccessibilityEvent") {
        ...
        if (notify_tree_destroyed_ && event_name == "automationInternal.onAccessibilityTreeDestroyed") {
        ...
        if (notify_get_text_location_ && event_name == "automationInternal.onGetTextLocationResult") {
    ...
    // For tests.
    void AddEventCallback(base::RepeatingCallback<void(const std::string&)> callback)
    {
        notify_event_ = std::move(callback);
    }

    // For tests.
    void AddTreeDestroyedCallback(base::RepeatingCallback<void(const AXTreeID&)> callback)
    {
        notify_tree_destroyed_ = std::move(callback);
    }

    // For tests.
    void AddGetTextLocationResultCallback(base::RepeatingCallback<void(const AXActionData&, const std::optional<gfx::Rect>&)> callback)
    {
...
private:
    ...
    base::RepeatingCallback<void(const AXActionData&, const std::optional<gfx::Rect>&)> notify_get_text_location_;
};
...
public:
    ...
    AutomationTreeManagerOwnerTest(const AutomationTreeManagerOwnerTest&) = delete;
    AutomationTreeManagerOwnerTest& operator=(const AutomationTreeManagerOwnerTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        bindings_ = std::make_unique<AutomationV8Bindings>(tree_manager_owner_.get(), router_.get());
...
protected:
    std::map<AXTreeID, std::unique_ptr<AutomationAXTreeWrapper>>& GetTreeIDToTreeMap()
    {
    ...
    {
        tree_manager_owner_->DispatchAccessibilityEvents(tree_id, updates, mouse_location, events);
    }
    ...
    void SendGetTextLocationResult(const AXActionData& data, const std::optional<gfx::Rect>& rect)
    {
    ...
    bool CallGetFocusInternal(AutomationAXTreeWrapper* top_wrapper, AutomationAXTreeWrapper** focused_wrapper, AXNode** focused_node)
    {
        return tree_manager_owner_->GetFocusInternal(top_wrapper, focused_wrapper, focused_node);
    }

    gfx::Rect CallComputeGlobalNodeBounds(AutomationAXTreeWrapper* wrapper, AXNode* node)
    {
...
TEST_F(AutomationTreeManagerOwnerTest, GetFocusMultipleTreesChildTreeConstruction)
{
    ...
    for (auto& updates : updates_list) {
        SendAccessibilityEvents(updates[0].tree_data.tree_id, updates, gfx::Point(), empty_events);
    }
    ...
    SendAccessibilityEvents(updates_list[0][0].tree_data.tree_id, updates_list[0], gfx::Point(), empty_events);
    CallGetFocusInternal(wrapper_0, &focused_wrapper, &focused_node);
    ASSERT_TRUE(focused_wrapper);
    ASSERT_TRUE(focused_node);
...
TEST_F(AutomationTreeManagerOwnerTest, GetFocusMultipleTreesAppIdConstruction)
{
    ...
    // same app id.
    wrapper0_button_data.AddStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId, "app1");
    wrapper1_link_data.AddStringAttribute(ax::mojom::StringAttribute::kAppId, "app1");
    wrapper0_link_data.AddStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId, "app2");
    wrapper2_button_data.AddStringAttribute(ax::mojom::StringAttribute::kAppId, "app2");
...
TEST_F(AutomationTreeManagerOwnerTest, GetBoundsAppIdConstruction)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        auto& node_data1 = tree_update.nodes.back();
        node_data1.id = 1;
        node_data1.role = i == 0 ? ax::mojom::Role::kDesktop : ax::mojom::Role::kRootWebArea;
        node_data1.child_ids.push_back(2);
    ...
    // same app id.
    wrapper0_button_data.AddStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId, "app1");
    wrapper1_button_data.AddStringAttribute(ax::mojom::StringAttribute::kAppId, "app1");

    wrapper0_button_data.AddFloatAttribute(ax::mojom::FloatAttribute::kChildTreeScale, 2.0);
    ...
    // bounds.
    EXPECT_EQ(gfx::Rect(50, 50, 100, 100), CallComputeGlobalNodeBounds(wrapper_1, wrapper1_button));
}
...
TEST_F(AutomationTreeManagerOwnerTest, GetBoundsNestedAppIdConstruction)
{
    ...
    wrapper0_client_data.AddStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId, "app2");
    wrapper1_root_data.AddStringAttribute(ax::mojom::StringAttribute::kAppId, "app2");
    ...
    EXPECT_EQ(gfx::Rect(200, 200, 100, 100), CallComputeGlobalNodeBounds(wrapper_1, wrapper1_root));
}
...
TEST_F(AutomationTreeManagerOwnerTest, GetMultipleChildRootsAppIdConstruction)
{
    ...
    // This construction requires the hosting and client nodes annotate with the
    // same app id.
    wrapper0_client_data.AddStringAttribute(ax::mojom::StringAttribute::kChildTreeNodeAppId, "app1");
    wrapper1_button_data.AddStringAttribute(ax::mojom::StringAttribute::kAppId, "app1");
    wrapper1_client_data.AddStringAttribute(ax::mojom::StringAttribute::kAppId, "app1");
...
TEST_F(AutomationTreeManagerOwnerTest, FireEventsWithListeners)
{
    ...
    AddEventCallback(base::BindLambdaForTesting([&](const std::string& event) { events.push_back(event); }));

    std::vector<AXEvent> ax_events;
    SendAccessibilityEvents(updates[0].tree_data.tree_id, updates, gfx::Point(), ax_events);
    ...
    // The button is id 2.
    std::tuple<ax::mojom::Event, AXEventGenerator::Event> event_type(ax::mojom::Event::kNone, AXEventGenerator::Event::ROLE_CHANGED);
    ...
    wrapper->EventListenerAdded(
        std::tuple<ax::mojom::Event, AXEventGenerator::Event>(ax::mojom::Event::kLoadComplete, AXEventGenerator::Event::NONE), tree->GetFromId(1));
    tree_update.nodes[0].role = ax::mojom::Role::kSwitch;
    SendAccessibilityEvents(updates[0].tree_data.tree_id, updates, gfx::Point(), ax_events);
    ...
    // button.
    wrapper->EventListenerAdded(
        std::tuple<ax::mojom::Event, AXEventGenerator::Event>(ax::mojom::Event::kClicked, AXEventGenerator::Event::NONE), tree->GetFromId(1));
    SendAccessibilityEvents(updates[0].tree_data.tree_id, updates, gfx::Point(), ax_events);
    ...
    AddGetTextLocationResultCallback(
        base::BindLambdaForTesting([&](const AXActionData& data, const std::optional<gfx::Rect>& rect) { text_location_sent = true; }));
    ...
    AddTreeDestroyedCallback(base::BindLambdaForTesting([&](const AXTreeID& tree_id) { tree_destroyed = true; }));
