...
UiaRegistrarWin::UiaRegistrarWin()
{
    ...
    if (FAILED(CoCreateInstance(CLSID_CUIAutomationRegistrar, nullptr, CLSCTX_INPROC_SERVER, IID_IUIAutomationRegistrar, &registrar)))
        return;

    // Register the custom UIA event that represents the test end event for the
    // UIA test suite.
    UIAutomationEventInfo test_complete_event_info = { kUiaEventTestCompleteSentinelGuid, L"kUiaTestCompleteSentinel" };
    ...
    UIAutomationPropertyInfo unique_id_property_info = { kUiaPropertyUniqueIdGuid, L"UniqueId", UIAutomationType_String };
    registrar->RegisterProperty(&unique_id_property_info, &unique_id_property_id_);
    ...
    if (features::IsAccessibilityAriaVirtualContentEnabled()) {
        ...
        UIAutomationPropertyInfo virtual_content_property_info = { kUiaPropertyVirtualContentGuid, L"VirtualContent", UIAutomationType_String };
        registrar->RegisterProperty(&virtual_content_property_info, &virtual_content_property_id_);
    }
}
