...
// alert event.
base::LazyInstance<AXPlatformNodeWinSet>::Leaky g_alert_targets = LAZY_INSTANCE_INITIALIZER;

base::LazyInstance<base::ObserverList<WinAccessibilityAPIUsageObserver>::Unchecked>::Leaky g_win_accessibility_api_usage_observer_list
...
template <typename T> void PatternProvider(AXPlatformNodeWin* node, IUnknown** result)
{
    node->AddRef();
...
void AXPlatformNodeWin::AddAttributeToList(const char* name, const char* value, PlatformAttributeList* attributes)
{
    std::string str_value = value;
    SanitizeStringAttribute(str_value, &str_value);
    attributes->push_back(base::UTF8ToWide(name) + L":" + base::UTF8ToWide(str_value));
}

// This also sets kNativeAPIs and kWebContents to ensure we don't have an
// incorrect combination of AXModes.
const uint32_t kScreenReaderAccessibilityMode = AXMode::kNativeAPIs | AXMode::kWebContents | AXMode::kScreenReader;
...
// static
base::ObserverList<WinAccessibilityAPIUsageObserver>::Unchecked& GetWinAccessibilityAPIUsageObserverList()
{
    return g_win_accessibility_api_usage_observer_list.Get();
}

// Used to simplify calling StartFiringUIAEvents and EndFiringEvents
WinAccessibilityAPIUsageScopedUIAEventsNotifier::WinAccessibilityAPIUsageScopedUIAEventsNotifier()
{
    GetWinAccessibilityAPIUsageObserverList().Notify(&WinAccessibilityAPIUsageObserver::StartFiringUIAEvents);
}
WinAccessibilityAPIUsageScopedUIAEventsNotifier::~WinAccessibilityAPIUsageScopedUIAEventsNotifier()
{
    GetWinAccessibilityAPIUsageObserverList().Notify(&WinAccessibilityAPIUsageObserver::EndFiringUIAEvents);
}
...
// Static
void AXPlatformNodeWin::SanitizeStringAttributeForUIAAriaProperty(const std::wstring& input, std::wstring* output)
{
    DCHECK(output);
...
void AXPlatformNodeWin::StringAttributeToUIAAriaProperty(
    std::vector<std::wstring>& properties, ax::mojom::StringAttribute attribute, const char* uia_aria_property)
{
    std::string value;
    if (GetStringAttribute(attribute, &value)) {
        ...
        properties.push_back(base::ASCIIToWide(uia_aria_property) + L"=" + wide_value);
    }
}

void AXPlatformNodeWin::BoolAttributeToUIAAriaProperty(std::vector<std::wstring>& properties, ax::mojom::BoolAttribute attribute, const char* uia_aria_property)
{
    bool value;
    if (GetBoolAttribute(attribute, &value)) {
        properties.push_back(base::ASCIIToWide(uia_aria_property) + L"=" + (value ? L"true" : L"false"));
    }
}

void AXPlatformNodeWin::IntAttributeToUIAAriaProperty(std::vector<std::wstring>& properties, ax::mojom::IntAttribute attribute, const char* uia_aria_property)
{
    int value;
    if (GetIntAttribute(attribute, &value)) {
        properties.push_back(base::ASCIIToWide(uia_aria_property) + L"=" + base::NumberToWString(value));
    }
}

void AXPlatformNodeWin::FloatAttributeToUIAAriaProperty(
    std::vector<std::wstring>& properties, ax::mojom::FloatAttribute attribute, const char* uia_aria_property)
{
    float value;
...
void AXPlatformNodeWin::StateToUIAAriaProperty(std::vector<std::wstring>& properties, ax::mojom::State state, const char* uia_aria_property)
{
    bool value = HasState(state);
    properties.push_back(base::ASCIIToWide(uia_aria_property) + L"=" + (value ? L"true" : L"false"));
}

std::vector<AXPlatformNodeWin*> AXPlatformNodeWin::CreatePlatformNodeVectorFromRelationIdVector(const std::vector<int32_t>& relation_id_list)
{
    ...
    for (int32_t id : relation_id_list) {
        ...
        if (IsValidUiaRelationTarget(platform_node)) {
            platform_node_list.push_back(static_cast<AXPlatformNodeWin*>(platform_node));
        }
    }
...
SAFEARRAY* AXPlatformNodeWin::CreateUIAElementsSafeArray(const std::vector<AXPlatformNodeWin*>& platform_node_list)
{
    if (platform_node_list.empty())
        return nullptr;

    SAFEARRAY* uia_array = SafeArrayCreateVector(VT_UNKNOWN, 0, platform_node_list.size());
    LONG i = 0;
    ...
    for (AXPlatformNodeWin* platform_node : platform_node_list) {
        ...
        SafeArrayPutElement(uia_array, &i, static_cast<IRawElementProviderSimple*>(platform_node));
        ++i;
    }
...
{
    std::vector<int32_t> relation_id_list = GetIntListAttribute(ax::mojom::IntListAttribute::kControlsIds);

    std::vector<AXPlatformNodeWin*> platform_node_list = CreatePlatformNodeVectorFromRelationIdVector(relation_id_list);

    if (GetActivePopupAxUniqueId() != std::nullopt) {
        AXPlatformNodeWin* view_popup_node_win = static_cast<AXPlatformNodeWin*>(GetFromUniqueId(GetActivePopupAxUniqueId().value()));
    ...
    if (HasIntListAttribute(ax::mojom::IntListAttribute::kErrormessageIds)) {
        std::vector<int32_t> error_message_node_ids = GetIntListAttribute(ax::mojom::IntListAttribute::kErrormessageIds);
        for (int32_t id : error_message_node_ids) {
            AXPlatformNodeWin* error_message_node_win = static_cast<AXPlatformNodeWin*>(GetDelegate()->GetFromNodeID(id));
...
SAFEARRAY* AXPlatformNodeWin::CreateUIAElementsArrayForRelation(const ax::mojom::IntListAttribute& attribute)
{
    return CreateUIAElementsSafeArray(CreatePlatformNodeVectorFromRelationIdVector(GetIntListAttribute(attribute)));
}

SAFEARRAY* AXPlatformNodeWin::CreateUIAElementsArrayForReverseRelation(const ax::mojom::IntListAttribute& attribute)
{
    std::vector<AXPlatformNode*> reverse_relations = GetDelegate()->GetSourceNodesForReverseRelations(attribute);

    std::vector<int32_t> id_list;
    base::ranges::transform(reverse_relations, std::back_inserter(id_list),
        [](AXPlatformNode* platform_node) { return static_cast<AXPlatformNodeWin*>(platform_node)->GetData().id; });

    return CreateUIAElementsSafeArray(CreatePlatformNodeVectorFromRelationIdVector(id_list));
}

SAFEARRAY* AXPlatformNodeWin::CreateClickablePointArray()
{
    SAFEARRAY* clickable_point_array = SafeArrayCreateVector(VT_R8, 0, 2);
    gfx::Point center = GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped).CenterPoint();

    double* double_array;
    SafeArrayAccessData(clickable_point_array, reinterpret_cast<void**>(&double_array));
...
gfx::Vector2d AXPlatformNodeWin::CalculateUIAScrollPoint(const ScrollAmount horizontal_amount, const ScrollAmount vertical_amount) const
{
    if (!GetDelegate() || !IsScrollable())
        return {};

    const gfx::Rect bounds = GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kClipped);
    ...
    DCHECK(hwnd);
    const float scale_factor = display::win::ScreenWin::GetScaleFactorForHWND(hwnd);
    const int small_change = base::ClampRound(kSmallScrollIncrement * scale_factor);
...
{
    TRACE_EVENT(
        "accessibility", "NotifyAccessibilityEvent", perfetto::Flow::FromPointer(this), "event_type", base::NumberToString(static_cast<int32_t>(event_type)));
    ...
    if (event_type == ax::mojom::Event::kSelection) {
        ...
        if (int role = MSAARole(); role == ROLE_SYSTEM_MENUITEM) {
            ...
            if (const AXPlatformNodeBase* container = GetSelectionContainer()) {
                if (container->GetRole() == ax::mojom::Role::kListBox && !container->HasState(ax::mojom::State::kMultiselectable)
                    && GetDelegate()->GetFocus() == GetNativeViewAccessible()) {
                    event_type = ax::mojom::Event::kFocus;
                }
            }
        } else if (auto* parent = GetParentPlatformNodeWin(); parent) {
            if (int parent_role = parent->MSAARole(); parent_role == ROLE_SYSTEM_MENUPOPUP || parent_role == ROLE_SYSTEM_LIST) {
                event_type = ax::mojom::Event::kFocus;
            }
        }
    }
    ...
    if (event_type == ax::mojom::Event::kValueChanged || event_type == ax::mojom::Event::kLiveRegionCreated
    ...
    if (std::optional<DWORD> native_event = MojoEventToMSAAEvent(event_type)) {
        ...
        TRACE_EVENT("accessibility", "NotifyWinEvent", "native_event", base::StringPrintf("0x%04lX", native_event.value()));
        ::NotifyWinEvent((*native_event), hwnd, OBJID_CLIENT, -GetUniqueId());
    }

    if (std::optional<PROPERTYID> uia_property = MojoEventToUIAProperty(event_type)) {
        ...
        ::UiaRaiseAutomationPropertyChangedEvent(this, (*uia_property), old_value, new_value);
    }
...
void AXPlatformNodeWin::OnActiveComposition(const gfx::Range& range, const std::u16string& active_composition_text, bool is_composition_committed)
{
    ...
    FireUiaTextEditTextChangedEvent(range, base::UTF16ToWide(active_composition_text), is_composition_committed);
}

void AXPlatformNodeWin::FireUiaTextEditTextChangedEvent(const gfx::Range& range, const std::wstring& active_composition_text, bool is_composition_committed)
{
    ...
    using UiaRaiseTextEditTextChangedEventFunction = HRESULT(WINAPI*)(IRawElementProviderSimple*, TextEditChangeType, SAFEARRAY*);
    UiaRaiseTextEditTextChangedEventFunction text_edit_text_changed_func = reinterpret_cast<UiaRaiseTextEditTextChangedEventFunction>(
        ::GetProcAddress(GetModuleHandle(L"uiautomationcore.dll"), "UiaRaiseTextEditTextChangedEvent"));
    if (!text_edit_text_changed_func) {
        return;
    }

    TextEditChangeType text_edit_change_type = is_composition_committed ? TextEditChangeType_CompositionFinalized : TextEditChangeType_Composition;
    // Composition has been finalized by TSF
    base::win::ScopedBstr composition_text(active_composition_text.c_str());
    base::win::ScopedSafearray changed_data(SafeArrayCreateVector(VT_BSTR /* element type */, 0 /* lower bound */, 1 /* number of elements */));
    if (!changed_data.Get()) {
        return;
    }

    LONG index = 0;
    HRESULT hr = SafeArrayPutElement(changed_data.Get(), &index, composition_text.Get());

    if (FAILED(hr)) {
        return;
    } else {
        // Fire the UiaRaiseTextEditTextChangedEvent
        text_edit_text_changed_func(this, text_edit_change_type, changed_data.Release());
    }
}

bool AXPlatformNodeWin::IsValidUiaRelationTarget(AXPlatformNode* ax_platform_node)
{
    if (!ax_platform_node)
        return false;
...
AXPlatformNodeWin::UIARoleProperties AXPlatformNodeWin::GetUIARoleProperties()
{
    ...
    case ax::mojom::Role::kAlert:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_TextControlTypeId, L"alert" };
    case ax::mojom::Role::kAlertDialog:
        ...
        return { UIALocalizationStrategy::kSupply, UIA_WindowControlTypeId, L"alert" };
    ...
    case ax::mojom::Role::kApplication:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_PaneControlTypeId, L"application" };

    case ax::mojom::Role::kArticle:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_GroupControlTypeId, L"article" };
    ...
    case ax::mojom::Role::kHeader:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"banner" };

    case ax::mojom::Role::kBlockquote:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"group" };

    case ax::mojom::Role::kButton:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_ButtonControlTypeId, L"button" };

    case ax::mojom::Role::kCanvas:
        return { UIALocalizationStrategy::kSupply, UIA_ImageControlTypeId, L"img" };

    case ax::mojom::Role::kCaption:
        return { UIALocalizationStrategy::kSupply, UIA_TextControlTypeId, L"description" };

    case ax::mojom::Role::kCaret:
        return { UIALocalizationStrategy::kSupply, UIA_PaneControlTypeId, L"region" };

    case ax::mojom::Role::kCell:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_DataItemControlTypeId, L"gridcell" };

    case ax::mojom::Role::kCode:
        return { UIALocalizationStrategy::kSupply, UIA_TextControlTypeId, L"code" };

    case ax::mojom::Role::kCheckBox:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_CheckBoxControlTypeId, L"checkbox" };

    case ax::mojom::Role::kClient:
        return { UIALocalizationStrategy::kSupply, UIA_PaneControlTypeId, L"region" };

    case ax::mojom::Role::kColorWell:
        return { UIALocalizationStrategy::kSupply, UIA_ButtonControlTypeId, L"textbox" };

    case ax::mojom::Role::kColumn:
        return { UIALocalizationStrategy::kSupply, UIA_PaneControlTypeId, L"region" };

    case ax::mojom::Role::kColumnHeader:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_DataItemControlTypeId, L"columnheader" };

    case ax::mojom::Role::kComboBoxGrouping:
    case ax::mojom::Role::kComboBoxMenuButton:
        return { UIALocalizationStrategy::kSupply, UIA_ComboBoxControlTypeId, L"combobox" };

    case ax::mojom::Role::kComboBoxSelect:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_ComboBoxControlTypeId, L"combobox" };

    case ax::mojom::Role::kComplementary:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_GroupControlTypeId, L"complementary" };

    case ax::mojom::Role::kContentDeletion:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"deletion" };

    case ax::mojom::Role::kContentInsertion:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"insertion" };

    case ax::mojom::Role::kContentInfo:
    case ax::mojom::Role::kFooter:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"contentinfo" };

    case ax::mojom::Role::kDate:
    case ax::mojom::Role::kDateTime:
        return { UIALocalizationStrategy::kSupply, UIA_EditControlTypeId, L"textbox" };

    case ax::mojom::Role::kDefinition:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_GroupControlTypeId, L"definition" };

    case ax::mojom::Role::kDescriptionList:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_ListControlTypeId, L"list" };

    case ax::mojom::Role::kDesktop:
        return { UIALocalizationStrategy::kSupply, UIA_DocumentControlTypeId, L"document" };

    case ax::mojom::Role::kDetails:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"group" };

    case ax::mojom::Role::kDialog:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_WindowControlTypeId, L"dialog" };

    case ax::mojom::Role::kDisclosureTriangle:
    case ax::mojom::Role::kDisclosureTriangleGrouped:
        if (::features::IsAccessibilityExposeSummaryAsHeadingEnabled()) {
            return { UIALocalizationStrategy::kSupply, UIA_ButtonControlTypeId, L"heading" };
        } else {
            return { UIALocalizationStrategy::kSupply, UIA_ButtonControlTypeId, L"button" };
        }
    ...
    case ax::mojom::Role::kDocNoteRef:
        return { UIALocalizationStrategy::kSupply, UIA_HyperlinkControlTypeId, L"link" };

    case ax::mojom::Role::kDocBiblioEntry:
    case ax::mojom::Role::kDocEndnote:
    case ax::mojom::Role::kDocFootnote:
        return { UIALocalizationStrategy::kSupply, UIA_ListItemControlTypeId, L"listitem" };

    case ax::mojom::Role::kDocPageBreak:
        return { UIALocalizationStrategy::kSupply, UIA_SeparatorControlTypeId, L"separator" };
    ...
    case ax::mojom::Role::kEmbeddedObject:
        if (GetDelegate()->GetChildCount()) {
            return { UIALocalizationStrategy::kSupply, UIA_PaneControlTypeId, L"group" };
        }
        return { UIALocalizationStrategy::kSupply, UIA_PaneControlTypeId, L"document" };

    case ax::mojom::Role::kEmphasis:
        return { UIALocalizationStrategy::kSupply, UIA_TextControlTypeId, L"emphasis" };

    case ax::mojom::Role::kFeed:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"feed" };

    case ax::mojom::Role::kFigcaption:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"description" };

    case ax::mojom::Role::kFigure:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"group" };

    case ax::mojom::Role::kForm:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"form" };
    ...
    case ax::mojom::Role::kGrid:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_DataGridControlTypeId, L"grid" };

    case ax::mojom::Role::kGridCell:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_DataItemControlTypeId, L"gridcell" };

    case ax::mojom::Role::kGroup:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_GroupControlTypeId, L"group" };

    case ax::mojom::Role::kHeading:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_TextControlTypeId, L"heading" };
    ...
    case ax::mojom::Role::kImage:
        ...
        // as 'Unlabeled'
        if (IsImageWithMap()) {
            return { UIALocalizationStrategy::kSupply, UIA_DocumentControlTypeId, L"img" };
        }
        return { UIALocalizationStrategy::kSupply, UIA_ImageControlTypeId, L"img" };
    ...
    case ax::mojom::Role::kInlineTextBox:
        return { UIALocalizationStrategy::kSupply, UIA_DocumentControlTypeId, L"textbox" };
    ...
    case ax::mojom::Role::kLayoutTableRow:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_GroupControlTypeId, L"group" };

    case ax::mojom::Role::kLink:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_HyperlinkControlTypeId, L"link" };

    case ax::mojom::Role::kList:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_ListControlTypeId, L"list" };

    case ax::mojom::Role::kListBox:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_ListControlTypeId, L"listbox" };

    case ax::mojom::Role::kListBoxOption:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_ListItemControlTypeId, L"option" };

    case ax::mojom::Role::kListGrid:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_DataGridControlTypeId, L"listview" };
    ...
    case ax::mojom::Role::kListMarker:
        ...
        return { UIALocalizationStrategy::kSupply, UIA_TextControlTypeId, L"group" };

    case ax::mojom::Role::kLog:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_GroupControlTypeId, L"log" };

    case ax::mojom::Role::kMain:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_GroupControlTypeId, L"main" };

    case ax::mojom::Role::kMark:
        return { UIALocalizationStrategy::kSupply, UIA_TextControlTypeId, L"description" };

    case ax::mojom::Role::kMarquee:
        return { UIALocalizationStrategy::kDeferToAriaRole, UIA_TextControlTypeId, L"marquee" };
    ...
    case ax::mojom::Role::kMenu:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_MenuControlTypeId, L"menu" };

    case ax::mojom::Role::kMenuBar:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_MenuBarControlTypeId, L"menubar" };

    case ax::mojom::Role::kMenuItem:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_MenuItemControlTypeId, L"menuitem" };

    case ax::mojom::Role::kMenuItemCheckBox:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_CheckBoxControlTypeId, L"menuitemcheckbox" };

    case ax::mojom::Role::kMenuItemRadio:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_RadioButtonControlTypeId, L"menuitemradio" };
    ...
    case ax::mojom::Role::kMeter:
        return { UIALocalizationStrategy::kSupply, UIA_ProgressBarControlTypeId, L"progressbar" };

    case ax::mojom::Role::kNavigation:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"navigation" };

    case ax::mojom::Role::kNote:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"note" };

    case ax::mojom::Role::kParagraph:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"group" };

    case ax::mojom::Role::kPdfActionableHighlight:
        return { UIALocalizationStrategy::kSupply, UIA_CustomControlTypeId, L"button" };
    ...
    case ax::mojom::Role::kRadioButton:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_RadioButtonControlTypeId, L"radio" };

    case ax::mojom::Role::kRadioGroup:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"radiogroup" };

    case ax::mojom::Role::kRegion:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"region" };

    case ax::mojom::Role::kRow:
        // Role changes depending on whether row is inside a treegrid
        // https://www.w3.org/TR/core-aam-1.1/#role-map-row
        if (IsInTreeGrid()) {
            return { UIALocalizationStrategy::kDeferToControlType, UIA_TreeItemControlTypeId, L"treeitem" };
        }
        return { UIALocalizationStrategy::kDeferToControlType, UIA_DataItemControlTypeId, L"row" };

    case ax::mojom::Role::kRowGroup:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"rowgroup" };

    case ax::mojom::Role::kRowHeader:
        return { UIALocalizationStrategy::kSupply, UIA_DataItemControlTypeId, L"rowheader" };
    ...
    case ax::mojom::Role::kScrollBar:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_ScrollBarControlTypeId, L"scrollbar" };

    case ax::mojom::Role::kScrollView:
        return { UIALocalizationStrategy::kSupply, UIA_PaneControlTypeId, L"region" };

    case ax::mojom::Role::kSearch:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"search" };

    case ax::mojom::Role::kSlider:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_SliderControlTypeId, L"slider" };

    case ax::mojom::Role::kSpinButton:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_SpinnerControlTypeId, L"spinbutton" };

    case ax::mojom::Role::kStrong:
        return { UIALocalizationStrategy::kSupply, UIA_TextControlTypeId, L"strong" };

    case ax::mojom::Role::kSwitch:
        return { UIALocalizationStrategy::kSupply, UIA_ButtonControlTypeId, L"switch" };

    case ax::mojom::Role::kStaticText:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_TextControlTypeId, L"description" };

    case ax::mojom::Role::kStatus:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_StatusBarControlTypeId, L"status" };

    case ax::mojom::Role::kSplitter:
        return { UIALocalizationStrategy::kDeferToControlType, IsFocusable() ? UIA_ThumbControlTypeId : UIA_SeparatorControlTypeId, L"separator" };
    ...
    case ax::mojom::Role::kTab:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_TabItemControlTypeId, L"tab" };

    case ax::mojom::Role::kTable:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_TableControlTypeId, L"grid" };

    case ax::mojom::Role::kTableHeaderContainer:
        return { UIALocalizationStrategy::kSupply, UIA_GroupControlTypeId, L"group" };

    case ax::mojom::Role::kTabList:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_TabControlTypeId, L"tablist" };

    case ax::mojom::Role::kTabPanel:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_PaneControlTypeId, L"tabpanel" };

    case ax::mojom::Role::kTerm:
        return { UIALocalizationStrategy::kSupply, UIA_ListItemControlTypeId, L"listitem" };

    case ax::mojom::Role::kTitleBar:
        return { UIALocalizationStrategy::kSupply, UIA_TitleBarControlTypeId, L"group" };

    case ax::mojom::Role::kToggleButton:
        return { UIALocalizationStrategy::kSupply, UIA_ButtonControlTypeId, L"button" };

    case ax::mojom::Role::kTextField:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_EditControlTypeId, L"textbox" };

    case ax::mojom::Role::kSearchBox:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_EditControlTypeId, L"searchbox" };
    ...
    case ax::mojom::Role::kTimer:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_PaneControlTypeId, L"timer" };

    case ax::mojom::Role::kToolbar:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_ToolBarControlTypeId, L"toolbar" };

    case ax::mojom::Role::kTooltip:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_ToolTipControlTypeId, L"tooltip" };

    case ax::mojom::Role::kTree:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_TreeControlTypeId, L"tree" };

    case ax::mojom::Role::kTreeGrid:
        return { UIALocalizationStrategy::kDeferToControlType, UIA_DataGridControlTypeId, L"treegrid" };
    ...
    case ax::mojom::Role::kImeCandidate: // Internal role, not used on Windows.
        return { UIALocalizationStrategy::kDeferToControlType, UIA_PaneControlTypeId, L"group" };
...
IFACEMETHODIMP AXPlatformNodeWin::accHitTest(LONG screen_physical_pixel_x, LONG screen_physical_pixel_y, VARIANT* child)
{
    ...
    if (!GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenPhysicalPixels, AXClippingBehavior::kClipped).Contains(point)) {
    ...
    while (true) {
        gfx::NativeViewAccessible hit_child = current_result->GetDelegate()->HitTestSync(screen_physical_pixel_x, screen_physical_pixel_y);
        if (!hit_child) {
            child->vt = VT_EMPTY;
            return S_FALSE;
        }

        AXPlatformNode* hit_child_node = AXPlatformNode::FromNativeViewAccessible(hit_child);
        if (!hit_child_node)
            break;
        ...
        bool is_same_hwnd
            = hit_child_node->GetDelegate()->GetTargetForNativeAccessibilityEvent() == current_result->GetDelegate()->GetTargetForNativeAccessibilityEvent();
...
IFACEMETHODIMP AXPlatformNodeWin::accLocation(LONG* physical_pixel_left, LONG* physical_pixel_top, LONG* width, LONG* height, VARIANT var_id)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("accLocation");
    WIN_ACCESSIBILITY_API_HISTOGRAM(UMA_API_ACC_LOCATION);
    AXPlatformNodeWin* target;
    COM_OBJECT_VALIDATE_VAR_ID_4_ARGS_AND_GET_TARGET(var_id, physical_pixel_left, physical_pixel_top, width, height, target);
    NotifyObserverForMSAAUsage();

    gfx::Rect bounds = target->GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenPhysicalPixels, AXClippingBehavior::kUnclipped);
...
IFACEMETHODIMP AXPlatformNodeWin::accNavigate(LONG nav_dir, VARIANT start, VARIANT* end)
{
    ...
    if ((nav_dir == NAVDIR_FIRSTCHILD || nav_dir == NAVDIR_LASTCHILD) && V_VT(&start) == VT_I4 && V_I4(&start) != CHILDID_SELF) {
    ...
    case NAVDIR_DOWN: {
        ...
        AXPlatformNodeBase* next = target->GetTableCell(*GetTableRow() + *GetTableRowSpan(), *GetTableColumn());
        if (!next)
            return S_OK;
    ...
    case NAVDIR_UP: {
        ...
        AXPlatformNodeBase* next = target->GetTableCell(*GetTableRow() - 1, *GetTableColumn());
        if (!next)
            return S_OK;
    ...
    case NAVDIR_LEFT: {
        ...
        AXPlatformNodeBase* next = target->GetTableCell(*GetTableRow(), *GetTableColumn() - 1);
        if (!next)
            return S_OK;
    ...
    case NAVDIR_RIGHT: {
        ...
        AXPlatformNodeBase* next = target->GetTableCell(*GetTableRow(), *GetTableColumn() + *GetTableColumnSpan());
        if (!next)
            return S_OK;
...
IFACEMETHODIMP AXPlatformNodeWin::get_accChild(VARIANT var_child, IDispatch** disp_child)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_accDefaultAction(VARIANT var_id, BSTR* def_action)
{
    ...
    int action;
    if (!target->GetIntAttribute(ax::mojom::IntAttribute::kDefaultActionVerb, &action)) {
        *def_action = nullptr;
        return S_FALSE;
    }

    std::wstring action_verb = base::UTF8ToWide(ToLocalizedString(static_cast<ax::mojom::DefaultActionVerb>(action)));
...
IFACEMETHODIMP AXPlatformNodeWin::get_accDescription(VARIANT var_id, BSTR* desc)
{
    ...
    return target->GetStringAttributeAsBstr(ax::mojom::StringAttribute::kDescription, desc);
}
...
IFACEMETHODIMP AXPlatformNodeWin::get_accFocus(VARIANT* focus_child)
{
    ...
    if (focus_accessible == this) {
        ...
        if (FAILED(focus_accessible->QueryInterface(IID_PPV_ARGS(&focus_idispatch)))) {
...
IFACEMETHODIMP AXPlatformNodeWin::get_accKeyboardShortcut(VARIANT var_id, BSTR* acc_key)
{
    ...
    return target->GetStringAttributeAsBstr(ax::mojom::StringAttribute::kKeyShortcuts, acc_key);
}
...
IFACEMETHODIMP AXPlatformNodeWin::get_accName(VARIANT var_id, BSTR* name_bstr)
{
    ...
    GetWinAccessibilityAPIUsageObserverList().Notify(&WinAccessibilityAPIUsageObserver::OnAccNameCalled);
...
IFACEMETHODIMP AXPlatformNodeWin::get_accValue(VARIANT var_id, BSTR* value)
{
    ...
    if (GetRole() == ax::mojom::Role::kProgressIndicator && !HasStringAttribute(ax::mojom::StringAttribute::kValue)
...
IFACEMETHODIMP AXPlatformNodeWin::get_accSelection(VARIANT* selected)
{
    ...
    for (size_t i = 0; i < GetDelegate()->GetChildCount(); ++i) {
        auto* node = static_cast<AXPlatformNodeWin*>(FromNativeViewAccessible(GetDelegate()->ChildAtIndex(i)));
    ...
    Microsoft::WRL::ComPtr<base::win::EnumVariant> enum_variant = Microsoft::WRL::Make<base::win::EnumVariant>(selected_count);
...
IFACEMETHODIMP AXPlatformNodeWin::get_accHelpTopic(BSTR* help_file, VARIANT var_id, LONG* topic_id)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_accHelpTopic");
    WIN_ACCESSIBILITY_API_HISTOGRAM(UMA_API_GET_ACC_HELP_TOPIC);
    AXPlatformNodeWin* target;
    COM_OBJECT_VALIDATE_VAR_ID_2_ARGS_AND_GET_TARGET(var_id, help_file, topic_id, target);
...
IFACEMETHODIMP AXPlatformNodeWin::get_relationTargetsOfType(BSTR type_bstr, LONG max_targets, IUnknown*** targets, LONG* n_targets)
{
    ...
    *targets = nullptr;
    ...
    if (type == L"alerts") {
        ...
        for (auto iter = g_alert_targets.Get().begin(); iter != g_alert_targets.Get().end(); ++iter) {
    ...
    int found = AXPlatformRelationWin::EnumerateRelationships(this, 0, type, &relation_type, &enumerated_targets);
    if (found == 0)
        return S_FALSE;
...
IFACEMETHODIMP AXPlatformNodeWin::get_nRelations(LONG* n_relations)
{
    ...
    int count = AXPlatformRelationWin::EnumerateRelationships(this, -1, std::wstring(), nullptr, nullptr);
    *n_relations = count;
    return S_OK;
}

IFACEMETHODIMP AXPlatformNodeWin::get_relation(LONG relation_index, IAccessibleRelation** relation)
{
    ...
    int found = AXPlatformRelationWin::EnumerateRelationships(this, relation_index, std::wstring(), &relation_type, &targets);
    if (found == 0)
        return E_INVALIDARG;
...
IFACEMETHODIMP AXPlatformNodeWin::get_relations(LONG max_relations, IAccessibleRelation** relations, LONG* n_relations)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_groupPosition(LONG* group_level, LONG* similar_items_in_group, LONG* position_in_group)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_groupPosition");
    WIN_ACCESSIBILITY_API_HISTOGRAM(UMA_API_GET_GROUP_POSITION);
    COM_OBJECT_VALIDATE_3_ARGS(group_level, similar_items_in_group, position_in_group);
    NotifyAddAXModeFlagsForIA2(kScreenReaderAccessibilityMode);
...
IFACEMETHODIMP AXPlatformNodeWin::get_localizedExtendedRole(BSTR* localized_extended_role)
{
    ...
    std::u16string role_description = GetRoleDescriptionFromImageAnnotationStatusOrFromAttribute();
...
IFACEMETHODIMP AXPlatformNodeWin::scrollToPoint(enum IA2CoordinateType coordinate_type, LONG x, LONG y)
{
    ...
    if (coordinate_type == IA2_COORDTYPE_PARENT_RELATIVE) {
        if (GetParent()) {
            ...
            scroll_to += base->GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped).OffsetFromOrigin();
        }
...
IFACEMETHODIMP AXPlatformNodeWin::get_extendedStates(LONG max_extended_states, BSTR** extended_states, LONG* n_extended_states)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_localizedExtendedStates(
    LONG max_localized_extended_states, BSTR** localized_extended_states, LONG* n_localized_extended_states)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_accessibleWithCaret(IUnknown** accessible, LONG* caret_offset)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_selectionRanges(IA2Range** ranges, LONG* nRanges)
{
    ...
    auto* anchor_node = static_cast<AXPlatformNodeWin*>(GetDelegate()->GetFromNodeID(anchor_id));
    if (!anchor_node)
        return E_FAIL;

    AXNodeID focus_id = unignored_selection.focus_object_id;
    auto* focus_node = static_cast<AXPlatformNodeWin*>(GetDelegate()->GetFromNodeID(focus_id));
    if (!focus_node)
        return E_FAIL;
...
IFACEMETHODIMP AXPlatformNodeWin::setSelectionRanges(LONG nRanges, IA2Range* ranges)
{
    ...
    const auto* anchor_node = static_cast<AXPlatformNodeWin*>(FromNativeViewAccessible(anchor.Get()));
    const auto* focus_node = static_cast<AXPlatformNodeWin*>(FromNativeViewAccessible(focus.Get()));
    ...
    if (anchor_node->IsLeaf()) {
        if (static_cast<size_t>(ranges->anchorOffset) > anchor_node->GetHypertext().length()) {
            return E_INVALIDARG;
        }
    } else {
        if (static_cast<size_t>(ranges->anchorOffset) > anchor_node->GetChildCount())
            return E_INVALIDARG;
    }

    if (focus_node->IsLeaf()) {
        if (static_cast<size_t>(ranges->activeOffset) > focus_node->GetHypertext().length())
            return E_INVALIDARG;
    } else {
...
IFACEMETHODIMP AXPlatformNodeWin::GetObjectForChild(LONG child_id, IAccessibleEx** result)
{
...
IFACEMETHODIMP AXPlatformNodeWin::GetIAccessiblePair(IAccessible** accessible, LONG* child_id)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_Target(IRawElementProviderSimple** target)
{
    ...
    std::vector<AXPlatformNode*> reverse_relations = GetDelegate()->GetSourceNodesForReverseRelations(ax::mojom::IntListAttribute::kDetailsIds);
...
IFACEMETHODIMP AXPlatformNodeWin::get_ExpandCollapseState(ExpandCollapseState* result)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_ExpandCollapseState");
    WIN_ACCESSIBILITY_API_HISTOGRAM(UMA_API_EXPANDCOLLAPSE_GET_EXPANDCOLLAPSESTATE);
...
IFACEMETHODIMP AXPlatformNodeWin::get_ContainingGrid(IRawElementProviderSimple** result)
{
...
IFACEMETHODIMP AXPlatformNodeWin::GetItem(int row, int column, IRawElementProviderSimple** result)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_ColumnCount(int* result)
{
    ...
    if (!column_count || *column_count == ax::mojom::kUnknownAriaColumnOrRowCount) {
        return E_UNEXPECTED;
    }
    *result = *column_count;
    return S_OK;
...
IFACEMETHODIMP AXPlatformNodeWin::ScrollIntoView()
{
    ...
    action_data.scroll_behavior = ax::mojom::ScrollBehavior::kDoNotScrollIfVisible;
    if (GetDelegate()->AccessibilityPerformAction(action_data))
        return S_OK;
    return E_FAIL;
}
...
IFACEMETHODIMP AXPlatformNodeWin::Scroll(ScrollAmount horizontal_amount, ScrollAmount vertical_amount)
{
    ...
    action_data.target_point = gfx::PointAtOffsetFromOrigin(CalculateUIAScrollPoint(horizontal_amount, vertical_amount));
    if (GetDelegate()->AccessibilityPerformAction(action_data))
        return S_OK;
    return E_FAIL;
}

IFACEMETHODIMP AXPlatformNodeWin::SetScrollPercent(double horizontal_percent, double vertical_percent)
{
    ...
    const int x = base::ClampRound(horizontal_percent / 100.0 * (x_max - x_min) + x_min);
    const int y = base::ClampRound(vertical_percent / 100.0 * (y_max - y_min) + y_min);
...
IFACEMETHODIMP AXPlatformNodeWin::get_HorizontalViewSize(double* result)
{
    ...
    gfx::RectF clipped_bounds(GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kClipped));
...
HRESULT AXPlatformNodeWin::ISelectionItemProviderSetSelected(bool selected) const
{
    UIA_VALIDATE_CALL();
    if (GetData().GetRestriction() == ax::mojom::Restriction::kDisabled)
        return UIA_E_ELEMENTNOTENABLED;
    ...
    // to be selected.
    if (selected == ISelectionItemProviderIsSelected() && !GetBoolAttribute(ax::mojom::BoolAttribute::kSelectedFromFocus))
        return S_OK;
...
bool AXPlatformNodeWin::ISelectionItemProviderIsSelected() const
{
    ...
    if (GetRole() == ax::mojom::Role::kRadioButton || GetRole() == ax::mojom::Role::kMenuItemRadio)
...
bool AXPlatformNodeWin::IsNodeInaccessibleForUIA() const
{
    ...
    return GetData().IsIgnored() || GetRole() == ax::mojom::Role::kInlineTextBox || IsChildOfLeaf();
}
...
IFACEMETHODIMP AXPlatformNodeWin::get_SelectionContainer(IRawElementProviderSimple** result)
{
...
IFACEMETHODIMP AXPlatformNodeWin::GetSelection(SAFEARRAY** result)
{
    ...
    *result = SafeArrayCreateVector(VT_UNKNOWN, 0, selected_children_count);
    ...
    for (LONG i = 0; i < selected_children_count; ++i) {
        AXPlatformNodeWin* children = static_cast<AXPlatformNodeWin*>(selected_children[i]);
        HRESULT hr = SafeArrayPutElement(*result, &i, static_cast<IRawElementProviderSimple*>(children));
        if (FAILED(hr)) {
...
IFACEMETHODIMP AXPlatformNodeWin::GetColumnHeaderItems(SAFEARRAY** result)
{
    ...
    *result = CreateUIAElementsSafeArray(CreatePlatformNodeVectorFromRelationIdVector(GetDelegate()->GetColHeaderNodeIds(*column)));
    return S_OK;
}
...
IFACEMETHODIMP AXPlatformNodeWin::GetRowHeaderItems(SAFEARRAY** result)
{
    ...
    *result = CreateUIAElementsSafeArray(CreatePlatformNodeVectorFromRelationIdVector(GetDelegate()->GetRowHeaderNodeIds(*row)));
    return S_OK;
}
...
IFACEMETHODIMP AXPlatformNodeWin::GetColumnHeaders(SAFEARRAY** result)
{
    ...
    *result = CreateUIAElementsSafeArray(CreatePlatformNodeVectorFromRelationIdVector(GetDelegate()->GetColHeaderNodeIds()));
    return S_OK;
}
...
IFACEMETHODIMP AXPlatformNodeWin::GetRowHeaders(SAFEARRAY** result)
{
    ...
    *result = CreateUIAElementsSafeArray(CreatePlatformNodeVectorFromRelationIdVector(GetDelegate()->GetRowHeaderNodeIds()));
    return S_OK;
}

IFACEMETHODIMP AXPlatformNodeWin::get_RowOrColumnMajor(RowOrColumnMajor* result)
{
...
IFACEMETHODIMP AXPlatformNodeWin::SetVisualState(WindowVisualState window_visual_state)
{
...
IFACEMETHODIMP AXPlatformNodeWin::WaitForInputIdle(int milliseconds, BOOL* result)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_WindowVisualState(WindowVisualState* result)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_WindowInteractionState(WindowInteractionState* result)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_LargeChange(double* result)
{
    ...
    UIA_VALIDATE_CALL_1_ARG(result);
    float attribute;
    if (GetFloatAttribute(ax::mojom::FloatAttribute::kStepValueForRange, &attribute)) {
        *result = attribute * kLargeChangeScaleFactor;
        ...
        std::string html_input_type = GetStringAttribute(ax::mojom::StringAttribute::kInputType);
...
IFACEMETHODIMP AXPlatformNodeWin::get_Maximum(double* result)
{
    ...
    UIA_VALIDATE_CALL_1_ARG(result);
    float attribute;
    if (GetFloatAttribute(ax::mojom::FloatAttribute::kMaxValueForRange, &attribute)) {
        *result = attribute;
        return S_OK;
    }
    return E_FAIL;
...
IFACEMETHODIMP AXPlatformNodeWin::get_Minimum(double* result)
{
    ...
    UIA_VALIDATE_CALL_1_ARG(result);
    float attribute;
    if (GetFloatAttribute(ax::mojom::FloatAttribute::kMinValueForRange, &attribute)) {
        *result = attribute;
        return S_OK;
    }
    return E_FAIL;
...
IFACEMETHODIMP AXPlatformNodeWin::get_Value(double* result)
{
    ...
    UIA_VALIDATE_CALL_1_ARG(result);
    float attribute;
    if (GetFloatAttribute(ax::mojom::FloatAttribute::kValueForRange, &attribute)) {
        *result = attribute;
        return S_OK;
    }
    return E_FAIL;
...
IFACEMETHODIMP
AXPlatformNodeWin::ConvertReturnedElement(IRawElementProviderSimple* element, IAccessibleEx** acc)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_accessibleAt(LONG row, LONG column, IUnknown** accessible)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_childIndex(LONG row, LONG column, LONG* cell_index)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_columnDescription(LONG column, BSTR* description)
{
    ...
    std::vector<int32_t> column_header_ids = GetDelegate()->GetColHeaderNodeIds(int { column });
    for (int32_t node_id : column_header_ids) {
        AXPlatformNodeWin* cell = static_cast<AXPlatformNodeWin*>(GetDelegate()->GetFromNodeID(node_id));
        if (!cell)
            continue;
        ...
        cell_name = base::UTF8ToWide(cell->GetStringAttribute(ax::mojom::StringAttribute::kDescription));
        if (!cell_name.empty()) {
            *description = SysAllocString(cell_name.c_str());
            return S_OK;
        }
...
IFACEMETHODIMP AXPlatformNodeWin::get_columnExtentAt(LONG row, LONG column, LONG* n_columns_spanned)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_columnHeader(IAccessibleTable** accessible_table, LONG* starting_row_index)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_columnIndex(LONG cell_index, LONG* column_index)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_rowDescription(LONG row, BSTR* description)
{
    ...
    std::vector<int32_t> row_header_ids = GetDelegate()->GetRowHeaderNodeIds(int { row });
...
IFACEMETHODIMP AXPlatformNodeWin::get_rowExtentAt(LONG row, LONG column, LONG* n_rows_spanned)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_rowHeader(IAccessibleTable** accessible_table, LONG* starting_column_index)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_rowIndex(LONG cell_index, LONG* row_index)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_selectedChildren(LONG max_children, LONG** children, LONG* n_children)
{
    ...
    return AllocateComArrayFromVector(results, max_children, children, n_children);
}

IFACEMETHODIMP AXPlatformNodeWin::get_selectedColumns(LONG max_columns, LONG** columns, LONG* n_columns)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_selectedRows(LONG max_rows, LONG** rows, LONG* n_rows)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_isColumnSelected(LONG column, boolean* is_selected)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_isRowSelected(LONG row, boolean* is_selected)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_isSelected(LONG row, LONG column, boolean* is_selected)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_isSelected");
    COM_OBJECT_VALIDATE_1_ARG(is_selected);
    NotifyAddAXModeFlagsForIA2(kScreenReaderAccessibilityMode);
...
IFACEMETHODIMP AXPlatformNodeWin::get_rowColumnExtentsAtIndex(
    LONG index, LONG* row, LONG* column, LONG* row_extents, LONG* column_extents, boolean* is_selected)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_rowColumnExtentsAtIndex");
    COM_OBJECT_VALIDATE_5_ARGS(row, column, row_extents, column_extents, is_selected);
    NotifyAddAXModeFlagsForIA2(kScreenReaderAccessibilityMode);
...
IFACEMETHODIMP AXPlatformNodeWin::get_modelChange(IA2TableModelChange* model_change)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_cellAt(LONG row, LONG column, IUnknown** cell)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_selectedCells(IUnknown*** cells, LONG* n_selected_cells)
{
    ...
    *cells = static_cast<IUnknown**>(CoTaskMemAlloc(selected.size() * sizeof(IUnknown*)));
...
IFACEMETHODIMP AXPlatformNodeWin::get_selectedColumns(LONG** columns, LONG* n_columns)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_selectedColumns");
...
IFACEMETHODIMP AXPlatformNodeWin::get_columnHeaderCells(IUnknown*** cell_accessibles, LONG* n_column_header_cells)
{
    ...
    std::vector<int32_t> column_header_ids = GetDelegate()->GetColHeaderNodeIds(*column);
    *cell_accessibles = static_cast<IUnknown**>(CoTaskMemAlloc(column_header_ids.size() * sizeof(IUnknown*)));
    int index = 0;
    for (int32_t node_id : column_header_ids) {
        AXPlatformNodeWin* node_win = static_cast<AXPlatformNodeWin*>(GetDelegate()->GetFromNodeID(node_id));
        if (node_win) {
...
IFACEMETHODIMP AXPlatformNodeWin::get_rowHeaderCells(IUnknown*** cell_accessibles, LONG* n_row_header_cells)
{
    ...
    std::vector<int32_t> row_header_ids = GetDelegate()->GetRowHeaderNodeIds(*row);
    *cell_accessibles = static_cast<IUnknown**>(CoTaskMemAlloc(row_header_ids.size() * sizeof(IUnknown*)));
    int index = 0;
    for (int32_t node_id : row_header_ids) {
        AXPlatformNodeWin* node_win = static_cast<AXPlatformNodeWin*>(GetDelegate()->GetFromNodeID(node_id));
        if (node_win) {
...
IFACEMETHODIMP AXPlatformNodeWin::get_rowColumnExtents(LONG* row_index, LONG* column_index, LONG* row_extents, LONG* column_extents, boolean* is_selected)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_rowColumnExtents");
    COM_OBJECT_VALIDATE_5_ARGS(row_index, column_index, row_extents, column_extents, is_selected);
    NotifyAddAXModeFlagsForIA2(kScreenReaderAccessibilityMode);
...
IFACEMETHODIMP AXPlatformNodeWin::get_nCharacters(LONG* n_characters)
{
    ...
    NotifyAddAXModeFlagsForIA2(kScreenReaderAccessibilityMode | AXMode::kInlineTextBoxes);
...
IFACEMETHODIMP AXPlatformNodeWin::get_selection(LONG selection_index, LONG* start_offset, LONG* end_offset)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_text(LONG start_offset, LONG end_offset, BSTR* text)
{
...
HRESULT AXPlatformNodeWin::IAccessibleTextGetTextForOffsetType(
    TextOffsetType text_offset_type, LONG offset, enum IA2TextBoundaryType boundary_type, LONG* start_offset, LONG* end_offset, BSTR* text)
{
    ...
    if (offset >= text_len) {
        ...
        const bool offset_for_line_text_len = offset == text_len && boundary_type == IA2_TEXT_BOUNDARY_LINE;
    ...
    case TextOffsetType::kAtOffset: {
        end = FindBoundary(boundary_type, offset, ax::mojom::MoveDirection::kForward);
        // Early return if the range will be degenerate containing no text.
        if (end <= 0)
            return S_FALSE;
        start = FindBoundary(boundary_type, offset, ax::mojom::MoveDirection::kBackward);
        break;
    }
    case TextOffsetType::kBeforeOffset: {
        ...
        end = FindBoundary(boundary_type, offset, ax::mojom::MoveDirection::kBackward);
        // Early return if the range will be degenerate containing no text,
        // or the range is after |offset|. Because the character at |offset| must
        // be excluded, |end| and |offset| may be equal.
        if (end <= 0 || end > offset)
            return S_FALSE;
        start = FindBoundary(boundary_type, end - 1, ax::mojom::MoveDirection::kBackward);
        break;
    }
    case TextOffsetType::kAfterOffset: {
        ...
        start = FindBoundary(boundary_type, offset, ax::mojom::MoveDirection::kForward);
        // Early return if the range will be degenerate containing no text,
        ...
        if (start >= text_len || start <= offset)
            return S_FALSE;
        end = FindBoundary(boundary_type, start, ax::mojom::MoveDirection::kForward);
        break;
    }
    }
...
IFACEMETHODIMP AXPlatformNodeWin::get_textAtOffset(LONG offset, enum IA2TextBoundaryType boundary_type, LONG* start_offset, LONG* end_offset, BSTR* text)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_textAtOffset");
    WIN_ACCESSIBILITY_API_HISTOGRAM(UMA_API_GET_TEXT_AT_OFFSET);
    return IAccessibleTextGetTextForOffsetType(TextOffsetType::kAtOffset, offset, boundary_type, start_offset, end_offset, text);
}

IFACEMETHODIMP AXPlatformNodeWin::get_textBeforeOffset(LONG offset, enum IA2TextBoundaryType boundary_type, LONG* start_offset, LONG* end_offset, BSTR* text)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_textBeforeOffset");
    WIN_ACCESSIBILITY_API_HISTOGRAM(UMA_API_GET_TEXT_BEFORE_OFFSET);
    return IAccessibleTextGetTextForOffsetType(TextOffsetType::kBeforeOffset, offset, boundary_type, start_offset, end_offset, text);
}

IFACEMETHODIMP AXPlatformNodeWin::get_textAfterOffset(LONG offset, enum IA2TextBoundaryType boundary_type, LONG* start_offset, LONG* end_offset, BSTR* text)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_textAfterOffset");
    WIN_ACCESSIBILITY_API_HISTOGRAM(UMA_API_GET_TEXT_AFTER_OFFSET);
    return IAccessibleTextGetTextForOffsetType(TextOffsetType::kAfterOffset, offset, boundary_type, start_offset, end_offset, text);
}

IFACEMETHODIMP AXPlatformNodeWin::get_offsetAtPoint(LONG x, LONG y, enum IA2CoordinateType coord_type, LONG* offset)
{
    ...
    *offset = -1;
    ...
    const AXPlatformNodeWin* hit_child = static_cast<AXPlatformNodeWin*>(FromNativeViewAccessible(GetDelegate()->HitTestSync(x, y)));

    if (!hit_child || !hit_child->IsText()) {
        return S_FALSE;
    }

    for (int i = 0, text_length = hit_child->GetTextContentUTF16().length(); i < text_length; ++i) {
        gfx::Rect char_bounds
            = hit_child->GetDelegate()->GetInnerTextRangeBoundsRect(i, i + 1, AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped);
...
IFACEMETHODIMP AXPlatformNodeWin::addSelection(LONG start_offset, LONG end_offset)
{
...
IFACEMETHODIMP AXPlatformNodeWin::setSelection(LONG selection_index, LONG start_offset, LONG end_offset)
{
    ...
    HandleSpecialTextOffset(&start_offset);
    HandleSpecialTextOffset(&end_offset);
    if (start_offset < 0 || start_offset > static_cast<LONG>(GetHypertext().length())) {
        return E_INVALIDARG;
    }
    if (end_offset < 0 || end_offset > static_cast<LONG>(GetHypertext().length())) {
        return E_INVALIDARG;
    }
...
IFACEMETHODIMP
AXPlatformNodeWin::get_selections(IA2TextSelection** selections, LONG* nSelections)
{
    ...
    AXNodeID anchor_id = unignored_selection.anchor_object_id;
    AXPlatformNodeWin* anchor_node = static_cast<AXPlatformNodeWin*>(GetDelegate()->GetFromNodeID(anchor_id));
    if (!anchor_node)
        return E_FAIL;
    ...
    if (anchor_node->IsDescendant(this)) {
        anchor_offset = anchor_node->GetHypertextOffsetFromEndpoint(anchor_node, anchor_offset);
    } else {
        anchor_offset = GetHypertextOffsetFromEndpoint(anchor_node, anchor_offset);
        anchor_node = this;
    }
    DCHECK_GE(anchor_offset, 0) << "This value is unexpected here, since we have already determined in "
                                   "this method that anchor_object is in the accessibility tree.";

    AXNodeID focus_id = unignored_selection.focus_object_id;
    AXPlatformNodeWin* focus_node = static_cast<AXPlatformNodeWin*>(GetDelegate()->GetFromNodeID(focus_id));
    if (!focus_node)
        return E_FAIL;

    int focus_offset = unignored_selection.focus_offset;
    if (focus_node->IsDescendant(this)) {
        focus_offset = focus_node->GetHypertextOffsetFromEndpoint(focus_node, focus_offset);
    } else {
        focus_offset = GetHypertextOffsetFromEndpoint(focus_node, focus_offset);
        focus_node = this;
    }
    DCHECK_GE(focus_offset, 0) << "This value is unexpected here, since we have already determined in "
    ...
    *selections = reinterpret_cast<IA2TextSelection*>(CoTaskMemAlloc(sizeof(IA2TextSelection)));
...
IFACEMETHODIMP AXPlatformNodeWin::setSelections(LONG nSelections, IA2TextSelection* selections)
{
    ...
    const auto* start_node = static_cast<AXPlatformNodeWin*>(FromNativeViewAccessible(start_obj.Get()));
    const auto* end_node = static_cast<AXPlatformNodeWin*>(FromNativeViewAccessible(end_obj.Get()));
    if (!start_node || !end_node)
        return E_INVALIDARG;

    AXPosition start_position = start_node->HypertextOffsetToEndpoint(selections->startOffset)->AsDomSelectionPosition();
    AXPosition end_position = end_node->HypertextOffsetToEndpoint(selections->endOffset)->AsDomSelectionPosition();
    ...
    int start_offset = start_position->IsTextPosition() ? start_position->text_offset() : start_position->child_index();
    int end_offset = end_position->IsTextPosition() ? end_position->text_offset() : end_position->child_index();
...
IFACEMETHODIMP AXPlatformNodeWin::get_hyperlink(LONG index, IAccessibleHyperlink** hyperlink)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_hyperlink");
    return E_NOTIMPL;
}

IFACEMETHODIMP AXPlatformNodeWin::get_hyperlinkIndex(LONG char_index, LONG* hyperlink_index)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_characterExtents(LONG offset, enum IA2CoordinateType coord_type, LONG* x, LONG* y, LONG* width, LONG* height)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_characterExtents");
    return E_NOTIMPL;
}
IFACEMETHODIMP AXPlatformNodeWin::scrollSubstringTo(LONG start_index, LONG end_index, enum IA2ScrollType scroll_type)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("scrollSubstringTo");
    return E_NOTIMPL;
}
IFACEMETHODIMP AXPlatformNodeWin::scrollSubstringToPoint(LONG start_index, LONG end_index, enum IA2CoordinateType coordinate_type, LONG x, LONG y)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("scrollSubstringToPoint");
    return E_NOTIMPL;
}
IFACEMETHODIMP AXPlatformNodeWin::get_attributes(LONG offset, LONG* start_offset, LONG* end_offset, BSTR* text_attributes)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_attributes");
    return E_NOTIMPL;
}
...
IFACEMETHODIMP AXPlatformNodeWin::get_minimumValue(VARIANT* value)
{
    ...
    float float_val;
    if (GetFloatAttribute(ax::mojom::FloatAttribute::kMinValueForRange, &float_val)) {
        value->vt = VT_R8;
        value->dblVal = float_val;
        return S_OK;
    }
...
IFACEMETHODIMP AXPlatformNodeWin::get_maximumValue(VARIANT* value)
{
    ...
    float float_val;
    if (GetFloatAttribute(ax::mojom::FloatAttribute::kMaxValueForRange, &float_val)) {
        value->vt = VT_R8;
        value->dblVal = float_val;
        return S_OK;
    }
...
IFACEMETHODIMP AXPlatformNodeWin::Navigate(NavigateDirection direction, IRawElementProviderFragment** element_provider)
{
  ...
  //   U6---U7   U8---U9
    ...
    case NavigateDirection_Parent: {
        ...
        AXFragmentRootWin* fragment_root = AXFragmentRootWin::GetFragmentRootParentOf(GetNativeViewAccessible());
    ...
    case NavigateDirection_NextSibling:
        ...
        if (AXFragmentRootWin::GetFragmentRootParentOf(GetNativeViewAccessible()) == nullptr) {
    ...
    if (neighbor) {
        if (direction != NavigateDirection_Parent) {
            ...
            AXFragmentRootWin* fragment_root = AXFragmentRootWin::GetFragmentRootParentOf(neighbor);
...
void AXPlatformNodeWin::GetRuntimeIdArray(AXPlatformNodeWin::RuntimeIdArray& runtime_id)
{
    ...
    if (dom_id) {
        ...
        if (tree_id != AXTreeIDUnknown()) {
            AXActionHandlerRegistry::FrameID frame_id = AXActionHandlerRegistry::GetInstance()->GetFrameID(tree_id);
...
IFACEMETHODIMP AXPlatformNodeWin::GetRuntimeId(SAFEARRAY** runtime_id)
{
    ...
    *runtime_id = ::SafeArrayCreateVector(VT_I4, 0, id_array.size());
    ...
    memcpy_s(array_data, runtime_id_byte_count, id_array.data(), runtime_id_byte_count);
...
IFACEMETHODIMP AXPlatformNodeWin::get_BoundingRectangle(UiaRect* screen_physical_pixel_bounds)
{
    ...
    gfx::Rect bounds = delegate_->GetBoundsRect(AXCoordinateSystem::kScreenPhysicalPixels, AXClippingBehavior::kUnclipped);
...
IFACEMETHODIMP AXPlatformNodeWin::GetEmbeddedFragmentRoots(SAFEARRAY** embedded_fragment_roots)
{
...
IFACEMETHODIMP AXPlatformNodeWin::get_FragmentRoot(IRawElementProviderFragmentRoot** fragment_root)
{
    ...
    gfx::AcceleratedWidget widget = delegate_->GetTargetForNativeAccessibilityEvent();
    if (widget) {
        AXFragmentRootWin* root = AXFragmentRootWin::GetForAcceleratedWidget(widget);
        if (root != nullptr) {
            root->GetNativeViewAccessible()->QueryInterface(IID_PPV_ARGS(fragment_root));
...
IFACEMETHODIMP AXPlatformNodeWin::GetPatternProvider(PATTERNID pattern_id, IUnknown** result)
{
...
HRESULT AXPlatformNodeWin::GetPatternProviderImpl(PATTERNID pattern_id, IUnknown** result)
{
    UIA_VALIDATE_CALL_1_ARG(result);

    *result = nullptr;

    PatternProviderFactoryMethod factory_method = GetPatternProviderFactoryMethod(pattern_id);
    if (factory_method)
...
IFACEMETHODIMP AXPlatformNodeWin::GetPropertyValue(PROPERTYID property_id, VARIANT* result)
{
    ...
    if (property_id >= kFirstKnownUiaPropertyId && property_id <= kLastKnownUiaPropertyId) {
        base::UmaHistogramSparse("Accessibility.WinAPIs.GetPropertyValue", property_id);
    } else {
...
HRESULT AXPlatformNodeWin::GetPropertyValueImpl(PROPERTYID property_id, VARIANT* result)
{
    UIA_VALIDATE_CALL_1_ARG(result);
    ...
    case UIA_AnnotationObjectsPropertyId: {
        ...
        result->parray = CreateUIAElementsArrayForRelation(ax::mojom::IntListAttribute::kDetailsIds);
        break;
    }
    ...
    case UIA_AutomationIdPropertyId: {
        ...
        std::u16string automation_id = GetRole() == ax::mojom::Role::kRootWebArea ? u"RootWebArea" : GetDelegate()->GetAuthorUniqueId();
        V_VT(result) = VT_BSTR;
    ...
    case UIA_ClassNamePropertyId:
        result->vt = VT_BSTR;
        GetStringAttributeAsBstr(ax::mojom::StringAttribute::kClassName, &result->bstrVal);
        break;
    ...
    case UIA_FlowsFromPropertyId:
        ...
        V_ARRAY(result) = CreateUIAElementsArrayForReverseRelation(ax::mojom::IntListAttribute::kFlowtoIds);
        break;

    case UIA_FlowsToPropertyId:
        result->vt = VT_ARRAY | VT_UNKNOWN;
        result->parray = CreateUIAElementsArrayForRelation(ax::mojom::IntListAttribute::kFlowtoIds);
        break;
    ...
    case UIA_HasKeyboardFocusPropertyId:
        result->vt = VT_BOOL;
        result->boolVal = (delegate_->GetFocus() == GetNativeViewAccessible()) ? VARIANT_TRUE : VARIANT_FALSE;
        break;

    case UIA_FullDescriptionPropertyId:
        result->vt = VT_BSTR;
        GetStringAttributeAsBstr(ax::mojom::StringAttribute::kDescription, &result->bstrVal);
        break;

    case UIA_HelpTextPropertyId:
        if (HasStringAttribute(ax::mojom::StringAttribute::kPlaceholder)) {
            V_VT(result) = VT_BSTR;
            GetStringAttributeAsBstr(ax::mojom::StringAttribute::kPlaceholder, &V_BSTR(result));
        } else if (GetNameFrom() == ax::mojom::NameFrom::kPlaceholder || GetNameFrom() == ax::mojom::NameFrom::kTitle) {
            V_VT(result) = VT_BSTR;
            GetNameAsBstr(&V_BSTR(result));
        } else if (HasStringAttribute(ax::mojom::StringAttribute::kTooltip)) {
            V_VT(result) = VT_BSTR;
            GetStringAttributeAsBstr(ax::mojom::StringAttribute::kTooltip, &V_BSTR(result));
        }
        break;
    ...
    case UIA_IsDataValidForFormPropertyId:
        if (GetIntAttribute(ax::mojom::IntAttribute::kInvalidState, &int_attribute)) {
            result->vt = VT_BOOL;
            result->boolVal = (static_cast<int>(ax::mojom::InvalidState::kFalse) == int_attribute) ? VARIANT_TRUE : VARIANT_FALSE;
        }
        break;
    ...
    case UIA_IsOffscreenPropertyId:
        result->vt = VT_BOOL;
        result->boolVal = GetDelegate()->IsOffscreen() ? VARIANT_TRUE : VARIANT_FALSE;
        break;
    ...
    case UIA_ItemStatusPropertyId: {
        ...
        if (IsTableHeader(GetRole()) && GetIntAttribute(ax::mojom::IntAttribute::kSortDirection, &sort_direction)) {
    ...
    case UIA_LocalizedControlTypePropertyId: {
        ...
        if (GetString16Attribute(ax::mojom::StringAttribute::kRoleDescription, &role_description)) {
            result->vt = VT_BSTR;
        ...
        // description.
        if (GetUIARoleProperties().localization_strategy == UIALocalizationStrategy::kSupply) {
            ...
            // "status".
            const std::string& html_tag = GetStringAttribute(ax::mojom::StringAttribute::kHtmlTag);
            std::u16string localized_control_type = html_tag == "output" ? l10n_util::GetStringUTF16(IDS_AX_ROLE_OUTPUT) : GetRoleDescription();

            if (!localized_control_type.empty()) {
                result->vt = VT_BSTR;
                result->bstrVal = SysAllocString(base::as_wcstr(localized_control_type));
            }
    ...
    case UIA_NamePropertyId:
        if (IsNameExposed()) {
            ...
            if (GetRole() == ax::mojom::Role::kListItem && !HasStringAttribute(ax::mojom::StringAttribute::kName)) {
    ...
    case UIA_OrientationPropertyId:
        if (SupportsOrientation(GetRole())) {
            if (HasState(ax::mojom::State::kHorizontal) && HasState(ax::mojom::State::kVertical)) {
    ...
    case UIA_IsPasswordPropertyId:
        result->vt = VT_BOOL;
        result->boolVal = HasState(ax::mojom::State::kProtected) ? VARIANT_TRUE : VARIANT_FALSE;
        break;

    case UIA_AcceleratorKeyPropertyId:
        if (HasStringAttribute(ax::mojom::StringAttribute::kKeyShortcuts)) {
            result->vt = VT_BSTR;
            GetStringAttributeAsBstr(ax::mojom::StringAttribute::kKeyShortcuts, &result->bstrVal);
        }
        break;
    ...
    case UIA_AccessKeyPropertyId:
        if (HasStringAttribute(ax::mojom::StringAttribute::kAccessKey)) {
            result->vt = VT_BSTR;
            GetStringAttributeAsBstr(ax::mojom::StringAttribute::kAccessKey, &result->bstrVal);
        }
        break;
    ...
    case UIA_LevelPropertyId:
        if (GetIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel, &int_attribute)) {
            result->vt = VT_I4;
    ...
    case UIA_LiveSettingPropertyId: {
        ...
        if (GetStringAttribute(ax::mojom::StringAttribute::kLiveStatus, &string_attribute)) {
    ...
    case UIA_SelectionItemIsSelectedPropertyId: {
        result->vt = VT_BOOL;
        result->boolVal = GetDelegate()->IsUIANodeSelected() ? VARIANT_TRUE : VARIANT_FALSE;
        break;
    }
    ...
    case UIA_LocalizedLandmarkTypePropertyId: {
        std::u16string localized_landmark_type = GetDelegate()->GetLocalizedStringForLandmarkType();
        if (!localized_landmark_type.empty()) {
            result->vt = VT_BSTR;
            result->bstrVal = SysAllocString(base::as_wcstr(localized_landmark_type));
        }
        break;
    }
    ...
    default:
        ...
        if (property_id == UiaRegistrarWin::GetInstance().GetUniqueIdPropertyId()) {
            ...
            result->vt = VT_BSTR;
            result->bstrVal = SysAllocString(base::NumberToWString(-GetUniqueId()).c_str());
        } else if (features::IsAccessibilityAriaVirtualContentEnabled() && property_id == UiaRegistrarWin::GetInstance().GetVirtualContentPropertyId()) {
            if (HasStringAttribute(ax::mojom::StringAttribute::kVirtualContent)) {
                V_VT(result) = VT_BSTR;
                GetStringAttributeAsBstr(ax::mojom::StringAttribute::kVirtualContent, &V_BSTR(result));
            }
        }
        break;
    }
...
IFACEMETHODIMP AXPlatformNodeWin::get_ProviderOptions(ProviderOptions* ret)
{
    ...
    *ret = ProviderOptions_ServerSideProvider | ProviderOptions_UseComThreading | ProviderOptions_RefuseNonClientSupport | ProviderOptions_HasNativeIAccessible;
    return S_OK;
}

IFACEMETHODIMP AXPlatformNodeWin::get_HostRawElementProvider(IRawElementProviderSimple** provider)
{
...
void SendBulkFetchResponse(Microsoft::WRL::ComPtr<IChromeAccessibleDelegate> delegate, LONG request_id, std::string json_result)
{
    std::wstring json_result_wide = base::UTF8ToWide(json_result);
    delegate->put_bulkFetchResult(request_id, SysAllocString(json_result_wide.c_str()));
}

IFACEMETHODIMP AXPlatformNodeWin::get_bulkFetch(BSTR input_json, LONG request_id, IChromeAccessibleDelegate* delegate)
{
    ...
    gfx::Rect bounds = GetDelegate()->GetBoundsRect(AXCoordinateSystem::kScreenDIPs, AXClippingBehavior::kUnclipped);
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&SendBulkFetchResponse, Microsoft::WRL::ComPtr<IChromeAccessibleDelegate>(delegate), request_id, json_result));
    return S_OK;
}

IFACEMETHODIMP AXPlatformNodeWin::get_hitTest(LONG screen_physical_pixel_x, LONG screen_physical_pixel_y, LONG request_id, IChromeAccessibleDelegate* delegate)
{
    ...
    AXPlatformNodeWin* hit_child
        = static_cast<AXPlatformNodeWin*>(FromNativeViewAccessible(GetDelegate()->HitTestSync(screen_physical_pixel_x, screen_physical_pixel_y)));
...
IFACEMETHODIMP AXPlatformNodeWin::QueryService(REFGUID guidService, REFIID riid, void** object)
{
    TRACE_EVENT("accessibility", "QueryService", perfetto::Flow::FromPointer(this), "guidService", base::WideToASCII(base::win::WStringFromGUID(guidService)),
    ...
    if (riid == IID_IAccessible2) {
        for (WinAccessibilityAPIUsageObserver& observer : GetWinAccessibilityAPIUsageObserverList()) {
    ...
    if (guidService == IID_IAccessible || guidService == IID_IAccessible2 || guidService == IID_IAccessible2_2 || guidService == IID_IAccessibleTable
        || guidService == IID_IAccessibleTable2 || guidService == IID_IAccessibleTableCell || guidService == IID_IAccessibleText
        || guidService == IID_IAccessibleTextSelectionContainer || guidService == IID_IAccessibleValue) {
...
// static
STDMETHODIMP AXPlatformNodeWin::InternalQueryInterface(void* this_ptr, const _ATL_INTMAP_ENTRY* entries, REFIID riid, void** object)
{
    if (!object)
        return E_INVALIDARG;
    *object = nullptr;
    AXPlatformNodeWin* accessible = reinterpret_cast<AXPlatformNodeWin*>(this_ptr);
    DCHECK(accessible);
    ...
    return CComObjectRootBase::InternalQueryInterface(this_ptr, entries, riid, object);
}

HRESULT AXPlatformNodeWin::GetTextAttributeValue(
    TEXTATTRIBUTEID attribute_id, const std::optional<int>& start_offset, const std::optional<int>& end_offset, base::win::VariantVector* result)
{
    ...
    case UIA_BackgroundColorAttributeId:
        result->Insert<VT_I4>(GetIntAttributeAsCOLORREF(ax::mojom::IntAttribute::kBackgroundColor));
        break;
    ...
    case UIA_FontWeightAttributeId:
        result->Insert<VT_I4>(GetFloatAttribute(ax::mojom::FloatAttribute::kFontWeight));
        break;
    case UIA_ForegroundColorAttributeId:
        result->Insert<VT_I4>(GetIntAttributeAsCOLORREF(ax::mojom::IntAttribute::kColor));
        break;
    ...
    case UIA_IsReadOnlyAttributeId: {
        ...
        const AXPlatformNodeWin* text_field = static_cast<AXPlatformNodeWin*>(FromNativeViewAccessible(GetDelegate()->GetTextFieldAncestor()));
        if (text_field) {
            result->Insert<VT_BOOL>(text_field->GetDelegate()->IsReadOnlyOrDisabled());
        } else {
    ...
    case UIA_IsSubscriptAttributeId:
        result->Insert<VT_BOOL>(GetData().GetTextPosition() == ax::mojom::TextPosition::kSubscript);
        break;
    case UIA_IsSuperscriptAttributeId:
        result->Insert<VT_BOOL>(GetData().GetTextPosition() == ax::mojom::TextPosition::kSuperscript);
        break;
    case UIA_OverlineStyleAttributeId:
        result->Insert<VT_I4>(GetUIATextDecorationStyle(ax::mojom::IntAttribute::kTextOverlineStyle));
        break;
    case UIA_StrikethroughStyleAttributeId:
        result->Insert<VT_I4>(GetUIATextDecorationStyle(ax::mojom::IntAttribute::kTextStrikethroughStyle));
        break;
    ...
    case UIA_HorizontalTextAlignmentAttributeId: {
        std::optional<HorizontalTextAlignment> horizontal_text_alignment = AXTextAlignToUIAHorizontalTextAlignment(GetData().GetTextAlign());
        if (horizontal_text_alignment)
            result->Insert<VT_I4>(*horizontal_text_alignment);
        break;
    }
    case UIA_UnderlineStyleAttributeId:
        result->Insert<VT_I4>(GetUIATextDecorationStyle(ax::mojom::IntAttribute::kTextUnderlineStyle));
        break;
    case UIA_TextFlowDirectionsAttributeId:
        result->Insert<VT_I4>(TextDirectionToFlowDirections(GetData().GetTextDirection()));
        break;
    default: {
...
void AXPlatformNodeWin::GetAnnotationObjectsAttribute(base::win::VariantVector* result)
{
    ...
    for (AXPlatformNodeWin* platform_node :
        CreatePlatformNodeVectorFromRelationIdVector(parent_platform_node->GetIntListAttribute(ax::mojom::IntListAttribute::kDetailsIds))) {
        Microsoft::WRL::ComPtr<IUnknown> annotation_object;
        if (SUCCEEDED(platform_node->QueryInterface(IID_PPV_ARGS(&annotation_object)))) {
...
HRESULT AXPlatformNodeWin::GetAnnotationTypesAttribute(
    const std::optional<int>& start_offset, const std::optional<int>& end_offset, base::win::VariantVector* result)
{
    ...
    MarkerTypeRangeResult highlight_spelling_result = MarkerTypeRangeResult::kNone;
    MarkerTypeRangeResult highlight_grammar_result = MarkerTypeRangeResult::kNone;

    if (IsText() || IsAtomicTextField()) {
        grammar_result = GetMarkerTypeFromRange(start_offset, end_offset, ax::mojom::MarkerType::kGrammar);
        spelling_result = GetMarkerTypeFromRange(start_offset, end_offset, ax::mojom::MarkerType::kSpelling);
        highlight_result = GetMarkerTypeFromRange(start_offset, end_offset, ax::mojom::MarkerType::kHighlight, ax::mojom::HighlightType::kHighlight);
        highlight_spelling_result
            = GetMarkerTypeFromRange(start_offset, end_offset, ax::mojom::MarkerType::kHighlight, ax::mojom::HighlightType::kSpellingError);
        highlight_grammar_result = GetMarkerTypeFromRange(start_offset, end_offset, ax::mojom::MarkerType::kHighlight, ax::mojom::HighlightType::kGrammarError);
    }

    if (grammar_result == MarkerTypeRangeResult::kMixed || spelling_result == MarkerTypeRangeResult::kMixed || highlight_result == MarkerTypeRangeResult::kMixed
        || highlight_spelling_result == MarkerTypeRangeResult::kMixed || highlight_grammar_result == MarkerTypeRangeResult::kMixed) {
    ...
    if (spelling_result == MarkerTypeRangeResult::kMatch || highlight_spelling_result == MarkerTypeRangeResult::kMatch)
        result->Insert<VT_I4>(AnnotationType_SpellingError);
    if (grammar_result == MarkerTypeRangeResult::kMatch || highlight_grammar_result == MarkerTypeRangeResult::kMatch)
...
{
    const std::u16string language = base::UTF8ToUTF16(GetDelegate()->GetLanguage());
    const LCID lcid = LocaleNameToLCID(base::as_wcstr(language), LOCALE_ALLOW_NEUTRAL_NAMES);
    if (!lcid)
        return std::nullopt;
...
COLORREF AXPlatformNodeWin::GetIntAttributeAsCOLORREF(ax::mojom::IntAttribute attribute) const
{
    SkColor color;
...
BulletStyle AXPlatformNodeWin::ComputeUIABulletStyle() const
{
    ...
    while (current_node && current_node->GetRole() != ax::mojom::Role::kListItem) {
        current_node = FromNativeViewAccessible(current_node->GetParent());
    }

    const ax::mojom::ListStyle list_style = current_node ? current_node->GetData().GetListStyle() : ax::mojom::ListStyle::kNone;
...
LONG AXPlatformNodeWin::ComputeUIAStyleId() const
{
    ...
    do {
        ...
        case ax::mojom::Role::kHeading:
            return AXHierarchicalLevelToUIAStyleId(current_node->GetIntAttribute(ax::mojom::IntAttribute::kHierarchicalLevel));
        case ax::mojom::Role::kListItem:
...
// static
std::optional<HorizontalTextAlignment> AXPlatformNodeWin::AXTextAlignToUIAHorizontalTextAlignment(ax::mojom::TextAlign text_align)
{
    switch (text_align) {
...
// static
LONG AXPlatformNodeWin::AXHierarchicalLevelToUIAStyleId(int32_t hierarchical_level)
{
...
// static
LONG AXPlatformNodeWin::AXListStyleToUIAStyleId(ax::mojom::ListStyle list_style)
{
    switch (list_style) {
    case ax::mojom::ListStyle::kNone:
        return StyleId_Normal;
...
// static
FlowDirections AXPlatformNodeWin::TextDirectionToFlowDirections(ax::mojom::WritingDirection text_direction)
{
...
// static
void AXPlatformNodeWin::AggregateRangesForMarkerType(AXPlatformNodeBase* node, ax::mojom::MarkerType marker_type, int offset_ranges_amount,
    std::vector<std::pair<int, int>>* ranges, const std::optional<ax::mojom::HighlightType>& highlight_type)
{
    DCHECK(node->IsText());
    const std::vector<int32_t>& marker_types = node->GetIntListAttribute(ax::mojom::IntListAttribute::kMarkerTypes);
    const std::vector<int32_t>& highlight_types = node->GetIntListAttribute(ax::mojom::IntListAttribute::kHighlightTypes);
    const std::vector<int>& marker_starts = node->GetIntListAttribute(ax::mojom::IntListAttribute::kMarkerStarts);
    const std::vector<int>& marker_ends = node->GetIntListAttribute(ax::mojom::IntListAttribute::kMarkerEnds);
    ...
    for (size_t i = 0; i < marker_types.size(); ++i) {
        ...
        if (marker_type == ax::mojom::MarkerType::kHighlight) {
            ...
            if (highlight_type != static_cast<ax::mojom::HighlightType>(highlight_types[i])) {
                continue;
            }
        }
...
AXPlatformNodeWin::MarkerTypeRangeResult AXPlatformNodeWin::GetMarkerTypeFromRange(const std::optional<int>& start_offset, const std::optional<int>& end_offset,
    ax::mojom::MarkerType marker_type, const std::optional<ax::mojom::HighlightType>& highlight_type)
{
    DCHECK(IsText() || IsAtomicTextField());
    std::vector<std::pair<int, int>> relevant_ranges;

    if (IsText()) {
        AggregateRangesForMarkerType(this, marker_type, /*offset_ranges_amount=*/0, &relevant_ranges, highlight_type);
    } else if (IsAtomicTextField()) {
        int offset_ranges_amount = 0;
        for (AXPlatformNodeBase* static_text = GetFirstTextOnlyDescendant(); static_text; static_text = static_text->GetNextSibling()) {
            ...
            AggregateRangesForMarkerType(static_text, marker_type, child_offset_ranges_amount, &relevant_ranges, highlight_type);
        }
    }

    // Sort the ranges by their start offset.
    const auto sort_ranges_by_start_offset = [](const std::pair<int, int>& a, const std::pair<int, int>& b) { return a.first < b.first; };
    std::sort(relevant_ranges.begin(), relevant_ranges.end(), sort_ranges_by_start_offset);
...
{
    return static_cast<AXPlatformNodeWin*>(AXPlatformNode::FromNativeViewAccessible(GetParent()));
}
...
int32_t AXPlatformNodeWin::ComputeIA2State()
{
    ...
    if (HasIntAttribute(ax::mojom::IntAttribute::kInvalidState)
        && GetIntAttribute(ax::mojom::IntAttribute::kInvalidState) != static_cast<int32_t>(ax::mojom::InvalidState::kFalse))
    ...
    if (!GetStringAttribute(ax::mojom::StringAttribute::kAutoComplete).empty() || HasState(ax::mojom::State::kAutofillAvailable)) {
    ...
    case ax::mojom::Role::kListBoxOption:
        ...
        if (!IsFocusable() || GetBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot))
...
std::wstring AXPlatformNodeWin::ComputeUIAProperties()
{
    ...
    BoolAttributeToUIAAriaProperty(properties, ax::mojom::BoolAttribute::kLiveAtomic, "atomic");
    BoolAttributeToUIAAriaProperty(properties, ax::mojom::BoolAttribute::kBusy, "busy");
    ...
    const auto restriction = static_cast<ax::mojom::Restriction>(GetIntAttribute(ax::mojom::IntAttribute::kRestriction));
    ...
    switch (static_cast<ax::mojom::HasPopup>(GetIntAttribute(ax::mojom::IntAttribute::kHasPopup))) {
    ...
    IntAttributeToUIAAriaProperty(properties, ax::mojom::IntAttribute::kHierarchicalLevel, "level");
    StringAttributeToUIAAriaProperty(properties, ax::mojom::StringAttribute::kLiveStatus, "live");
    StateToUIAAriaProperty(properties, ax::mojom::State::kMultiline, "multiline");
    StateToUIAAriaProperty(properties, ax::mojom::State::kMultiselectable, "multiselectable");
    IntAttributeToUIAAriaProperty(properties, ax::mojom::IntAttribute::kPosInSet, "posinset");
    StringAttributeToUIAAriaProperty(properties, ax::mojom::StringAttribute::kLiveRelevant, "relevant");
    StateToUIAAriaProperty(properties, ax::mojom::State::kRequired, "required");
    BoolAttributeToUIAAriaProperty(properties, ax::mojom::BoolAttribute::kSelected, "selected");
    IntAttributeToUIAAriaProperty(properties, ax::mojom::IntAttribute::kSetSize, "setsize");
    StateToUIAAriaProperty(properties, ax::mojom::State::kHasActions, "hasactions");
    ...
    if (GetData().IsRangeValueSupported()) {
        FloatAttributeToUIAAriaProperty(properties, ax::mojom::FloatAttribute::kMaxValueForRange, "valuemax");
        FloatAttributeToUIAAriaProperty(properties, ax::mojom::FloatAttribute::kMinValueForRange, "valuemin");
        StringAttributeToUIAAriaProperty(properties, ax::mojom::StringAttribute::kValue, "valuetext");
    ...
    if (GetIntAttribute(ax::mojom::IntAttribute::kAriaCurrentState, &aria_current_attribute)) {
        ax::mojom::AriaCurrentState aria_current_state = static_cast<ax::mojom::AriaCurrentState>(aria_current_attribute);
        if (aria_current_state != ax::mojom::AriaCurrentState::kNone && aria_current_state != ax::mojom::AriaCurrentState::kFalse) {
...
AXPlatformNodeWin* AXPlatformNodeWin::ComputeUIALabeledBy()
{
    ...
    for (int32_t id : GetIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds)) {
        auto* node_win = static_cast<AXPlatformNodeWin*>(GetDelegate()->GetFromNodeID(id));
        if (!node_win)
            continue;

        // If this node is a static text, then simply return the node itself.
        if (IsValidUiaRelationTarget(node_win) && node_win->GetRole() == ax::mojom::Role::kStaticText) {
            return node_win;
        }

        // Otherwise, find the first static text node in its descendants.
        for (auto iter = node_win->GetDelegate()->ChildrenBegin(); *iter != *node_win->GetDelegate()->ChildrenEnd(); ++(*iter)) {
            AXPlatformNodeWin* child = static_cast<AXPlatformNodeWin*>(AXPlatformNode::FromNativeViewAccessible(iter->GetNativeViewAccessible()));
            if (IsValidUiaRelationTarget(child) && child->GetRole() == ax::mojom::Role::kStaticText) {
                return child;
            }
        }
    }
...
bool AXPlatformNodeWin::IsUIAControl() const
{
    ...
    if (GetDelegate()->IsWebContent()) {
        ...
        if (IsText()) {
            ...
            while (ancestor) {
                ...
                case ax::mojom::Role::kListItem:
                    ...
                    return ancestor != FromNativeViewAccessible(GetDelegate()->GetParent());
                case ax::mojom::Role::kButton:
                case ax::mojom::Role::kCheckBox:
                case ax::mojom::Role::kHeading:
                case ax::mojom::Role::kLineBreak:
        ...
        if (IsControl(GetRole()) || ComputeUIALandmarkType() || IsUIATableLike(GetRole()) || IsList(GetRole())) {
            return true;
        }
        if (IsImage(GetRole())) {
            ...
            if (GetRole() == ax::mojom::Role::kImage && GetNameFrom() == ax::mojom::NameFrom::kAttributeExplicitlyEmpty) {
                return false;
            }
            return true;
        }
        ...
        if (!GetBoolAttribute(ax::mojom::BoolAttribute::kHasAriaAttribute) && !GetBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot)
            && GetName().empty() && GetStringAttribute(ax::mojom::StringAttribute::kDescription).empty() && !HasState(ax::mojom::State::kFocusable)
            && !GetData().IsClickable()) {
            return false;
        }
    ...
    // web content?
    return !(IsInvisibleOrIgnored() && !IsFocusable()) || GetRole() == ax::mojom::Role::kStatus;
}
...
std::optional<LONG> AXPlatformNodeWin::ComputeUIALandmarkType() const
{
    ...
    case ax::mojom::Role::kForm:
        ...
        if (HasStringAttribute(ax::mojom::StringAttribute::kName) || HasStringAttribute(ax::mojom::StringAttribute::kRole)) {
...
bool AXPlatformNodeWin::IsInaccessibleForUIA() const
{
    ...
    while (parent) {
        if (parent->IsNodeInaccessibleForUIA() || parent->ShouldHideChildrenForUIA()) {
            return true;
        }
...
{
    TRACE_EVENT("accessibility", "OnReferenced", perfetto::Flow::FromPointer(this), "UniqueId", base::NumberToString(GetUniqueId()));
}

void AXPlatformNodeWin::OnDereferenced()
{
    TRACE_EVENT("accessibility", "OnDereferenced", perfetto::TerminatingFlow::FromPointer(this));
}
...
int AXPlatformNodeWin::MSAAState() const
{
    ...
    default:
        ...
        // even if the node data isn't marked as read only, as long as the
        // node is not editable.
        if (!HasState(ax::mojom::State::kRichlyEditable) && ShouldHaveReadonlyStateByDefault(GetRole())) {
            msaa_state |= STATE_SYSTEM_READONLY;
        }
        break;
    }
    ...
    if (GetRole() == ax::mojom::Role::kPluginObject || GetRole() == ax::mojom::Role::kEmbeddedObject) {
        msaa_state |= STATE_SYSTEM_UNAVAILABLE;
    }
    ...
    if ((GetRole() == ax::mojom::Role::kListBoxOption || IsMenuItem(GetRole())) && GetBoolAttribute(ax::mojom::BoolAttribute::kSelected)) {
        AXPlatformNodeBase* container = FromNativeViewAccessible(GetParent());
        if (container && container->GetParent() == focus) {
            if ((container->GetRole() == ax::mojom::Role::kListBox || container->GetRole() == ax::mojom::Role::kMenu)
...
// static
std::optional<DWORD> AXPlatformNodeWin::MojoEventToMSAAEvent(ax::mojom::Event event)
{
    switch (event) {
...
// static
std::optional<EVENTID> AXPlatformNodeWin::MojoEventToUIAEvent(ax::mojom::Event event)
{
    if (!AXPlatform::GetInstance().IsUiaProviderEnabled()) {
        return std::nullopt;
    }
...
// static
std::optional<PROPERTYID> AXPlatformNodeWin::MojoEventToUIAProperty(ax::mojom::Event event)
{
    if (!AXPlatform::GetInstance().IsUiaProviderEnabled()) {
        return std::nullopt;
    }
...
BSTR AXPlatformNodeWin::GetValueAttributeAsBstr(AXPlatformNodeWin* target)
{
    if (target->IsPlatformDocument()) {
        std::wstring url = base::UTF8ToWide(target->GetDelegate()->GetTreeData().url);
    ...
    if (IsLink(target->GetRole())) {
        std::wstring url = base::UTF8ToWide(target->GetStringAttribute(ax::mojom::StringAttribute::kUrl));
        BSTR value = SysAllocString(url.c_str());
        DCHECK(value);
        return value;
    }
...
HRESULT AXPlatformNodeWin::GetStringAttributeAsBstr(ax::mojom::StringAttribute attribute, BSTR* value_bstr) const
{
    std::u16string str;
...
LONG AXPlatformNodeWin::FindBoundary(IA2TextBoundaryType ia2_boundary, LONG start_offset, ax::mojom::MoveDirection direction)
{
    HandleSpecialTextOffset(&start_offset);
    ...
    return static_cast<LONG>(FindTextBoundary(boundary, start_offset, direction, affinity));
}

AXPlatformNodeWin* AXPlatformNodeWin::GetTargetFromChildID(const VARIANT& var_id)
{
    ...
    if (child_id >= 1 && static_cast<size_t>(child_id) <= GetDelegate()->GetChildCount()) {
        // Positive child ids are a 1-based child index, used by clients
        // that want to enumerate all immediate children.
        AXPlatformNodeBase* base = FromNativeViewAccessible(GetDelegate()->ChildAtIndex(static_cast<size_t>(child_id - 1)));
    ...
    AXPlatformNodeBase* base = FromNativeViewAccessible(node->GetNativeViewAccessible());
...
HRESULT AXPlatformNodeWin::AllocateComArrayFromVector(std::vector<LONG>& results, LONG max, LONG** selected, LONG* n_selected)
{
    DCHECK_GT(max, 0);
    DCHECK(selected);
    DCHECK(n_selected);
...
{
    const std::wstring string = base::UTF8ToWide(GetInheritedStringAttribute(ax::mojom::StringAttribute::kFontFamily));
...
{
    std::u16string style_name = GetDelegate()->GetStyleNameAttributeAsLocalizedString();

    return SysAllocString(base::as_wcstr(style_name));
}

TextDecorationLineStyle AXPlatformNodeWin::GetUIATextDecorationStyle(const ax::mojom::IntAttribute int_attribute) const
{
    const ax::mojom::TextDecorationStyle text_decoration_style = static_cast<ax::mojom::TextDecorationStyle>(GetIntAttribute(int_attribute));
...
AXPlatformNodeWin::PatternProviderFactoryMethod AXPlatformNodeWin::GetPatternProviderFactoryMethod(PATTERNID pattern_id)
{
    switch (pattern_id) {
...
{
    for (auto* child = static_cast<AXPlatformNodeWin*>(GetFirstChild()); child; child = static_cast<AXPlatformNodeWin*>(child->GetNextSibling())) {
        if (child->IsText())
            return child;
...
void AXPlatformNodeWin::SanitizeTextAttributeValue(const std::string& input, std::string* output) const
{
...
{
    GetWinAccessibilityAPIUsageObserverList().Notify(&WinAccessibilityAPIUsageObserver::OnMSAAUsed);
}

void AXPlatformNodeWin::NotifyAddAXModeFlagsForIA2(const uint32_t ax_modes) const
{
    if (features::IsAccessibilityRestrictiveIA2AXModesEnabled()) {
...
void AXPlatformNodeWin::NotifyAPIObserverForPatternRequest(PATTERNID pattern_id) const
{
    if (!GetDelegate() || !GetDelegate()->IsWebContent()) {
        return;
    }
...
void AXPlatformNodeWin::NotifyAPIObserverForPropertyRequest(PROPERTYID property_id) const
{
    // Non-web content is always enabled, if a client isn't looking for web
    // content, don't enable.
    if (!GetDelegate() || !GetDelegate()->IsWebContent())
...
// static
void AXPlatformNodeWin::SanitizeStringAttributeForIA2(const std::string& input, std::string* output)
{
    DCHECK(output);
...
bool AXPlatformNodeWin::IsSelectionItemSupported() const
{
    ...
    case ax::mojom::Role::kRowHeader: {
        ...
        return table->GetRole() == ax::mojom::Role::kGrid || table->GetRole() == ax::mojom::Role::kListGrid || table->GetRole() == ax::mojom::Role::kTreeGrid;
    }
    // https://www.w3.org/TR/core-aam-1.1/#mapping_state-property_table
    ...
    case ax::mojom::Role::kRadioButton:
    case ax::mojom::Role::kMenuItemRadio: {
        if (GetData().GetCheckedState() == ax::mojom::CheckedState::kTrue || GetData().GetCheckedState() == ax::mojom::CheckedState::kFalse) {
            return true;
        }
        return false;
    }
