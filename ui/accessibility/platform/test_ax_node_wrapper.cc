...
TestAXNodeWrapper* TestAXNodeWrapper::GetOrCreate(AXTree* tree, AXNode* node)
{
    ...
    auto wrapper = std::unique_ptr<TestAXNodeWrapper>(new TestAXNodeWrapper(tree, node));
...
// static
std::unique_ptr<base::AutoReset<float>> TestAXNodeWrapper::SetScaleFactor(float value)
{
...
// static
void TestAXNodeWrapper::SetHitTestResult(AXNodeID src_node_id, AXNodeID dst_node_id)
{
...
AXNodePosition::AXPositionInstance TestAXNodeWrapper::CreatePositionAt(int offset, ax::mojom::TextAffinity affinity) const
{
    if (node_->IsLeaf()) {
...
AXNodePosition::AXPositionInstance TestAXNodeWrapper::CreateTextPositionAt(int offset, ax::mojom::TextAffinity affinity) const
{
    return AXNodePosition::CreateTextPosition(*node_, offset, affinity);
}
...
gfx::NativeViewAccessible TestAXNodeWrapper::GetParent() const
{
    ...
    TestAXNodeWrapper* parent_wrapper = GetOrCreate(tree_, node_->GetUnignoredParent());
...
gfx::Rect TestAXNodeWrapper::GetInnerTextRangeBoundsRect(const int start_offset, const int end_offset, const AXCoordinateSystem coordinate_system,
    const AXClippingBehavior clipping_behavior, AXOffscreenResult* offscreen_result) const
{
    ...
    case AXCoordinateSystem::kScreenDIPs: {
        ...
        } else if (GetRole() == ax::mojom::Role::kStaticText && InternalChildCount() > 0) {
            TestAXNodeWrapper* child = InternalGetChild(0);
            if (child != nullptr && child->GetRole() == ax::mojom::Role::kInlineTextBox) {
...
gfx::Rect TestAXNodeWrapper::GetHypertextRangeBoundsRect(const int start_offset, const int end_offset, const AXCoordinateSystem coordinate_system,
    const AXClippingBehavior clipping_behavior, AXOffscreenResult* offscreen_result) const
{
    switch (coordinate_system) {
    case AXCoordinateSystem::kScreenPhysicalPixels:
...
gfx::NativeViewAccessible TestAXNodeWrapper::HitTestSync(int screen_physical_pixel_x, int screen_physical_pixel_y) const
{
    const TestAXNodeWrapper* wrapper
        = const_cast<TestAXNodeWrapper*>(this)->HitTestSyncInternal(screen_physical_pixel_x / g_scale_factor, screen_physical_pixel_y / g_scale_factor);
    return wrapper ? wrapper->ax_platform_node()->GetNativeViewAccessible() : nullptr;
}

gfx::NativeViewAccessible TestAXNodeWrapper::GetFocus() const
{
    auto focused = g_focused_node_in_tree.find(tree_);
    if (focused != g_focused_node_in_tree.end() && focused->second->IsDescendantOf(node_)) {
        return GetOrCreate(tree_, focused->second)->ax_platform_node()->GetNativeViewAccessible();
    }
    return nullptr;
}
...
AXPlatformNode* TestAXNodeWrapper::GetFromTreeIDAndNodeID(const AXTreeID& ax_tree_id, int32_t id)
{
...
{
    return node_ ? std::make_optional(node_->GetUnignoredIndexInParent()) : std::nullopt;
}

void TestAXNodeWrapper::ReplaceIntAttribute(int32_t node_id, ax::mojom::IntAttribute attribute, int32_t value)
{
    if (!tree_)
        return;
    ...
    std::vector<std::pair<ax::mojom::IntAttribute, int32_t>>& attributes = new_data.int_attributes;

    std::erase_if(attributes, [attribute](auto& pair) { return pair.first == attribute; });

    new_data.AddIntAttribute(attribute, value);
    node->SetData(new_data);
}

void TestAXNodeWrapper::ReplaceFloatAttribute(ax::mojom::FloatAttribute attribute, float value)
{
    AXNodeData new_data = GetData();
    std::vector<std::pair<ax::mojom::FloatAttribute, float>>& attributes = new_data.float_attributes;
...
void TestAXNodeWrapper::ReplaceBoolAttribute(ax::mojom::BoolAttribute attribute, bool value)
{
    AXNodeData new_data = GetData();
    std::vector<std::pair<ax::mojom::BoolAttribute, bool>>& attributes = new_data.bool_attributes;
...
void TestAXNodeWrapper::ReplaceStringAttribute(ax::mojom::StringAttribute attribute, std::string value)
{
    AXNodeData new_data = GetData();
    std::vector<std::pair<ax::mojom::StringAttribute, std::string>>& attributes = new_data.string_attributes;
...
void TestAXNodeWrapper::ReplaceTreeDataTextSelection(int32_t anchor_node_id, int32_t anchor_offset, int32_t focus_node_id, int32_t focus_offset)
{
    if (!tree_)
        return;
...
std::vector<AXNodeID> TestAXNodeWrapper::GetColHeaderNodeIds(int col_index) const
{
...
std::vector<AXNodeID> TestAXNodeWrapper::GetRowHeaderNodeIds(int row_index) const
{
...
std::optional<int32_t> TestAXNodeWrapper::GetCellId(int row_index, int col_index) const
{
...
std::optional<int32_t> TestAXNodeWrapper::GetCellIdAriaCoords(int aria_row_index, int aria_col_index) const
{
...
gfx::AcceleratedWidget TestAXNodeWrapper::GetTargetForNativeAccessibilityEvent()
{
...
bool TestAXNodeWrapper::AccessibilityPerformAction(const AXActionData& data)
{
    ...
    case ax::mojom::Action::kSetScrollOffset: {
        ...
        int scroll_x = std::clamp(data.target_point.x(), scroll_x_min, scroll_x_max);
        int scroll_y = std::clamp(data.target_point.y(), scroll_y_min, scroll_y_max);

        ReplaceIntAttribute(node_->id(), ax::mojom::IntAttribute::kScrollX, scroll_x);
        ReplaceIntAttribute(node_->id(), ax::mojom::IntAttribute::kScrollY, scroll_y);
        return true;
    }
    ...
    case ax::mojom::Action::kDoDefault: {
        ...
        if (HasBoolAttribute(ax::mojom::BoolAttribute::kSelectedFromFocus))
            ReplaceBoolAttribute(ax::mojom::BoolAttribute::kSelectedFromFocus, false);

        switch (GetRole()) {
        case ax::mojom::Role::kListBoxOption:
        case ax::mojom::Role::kGridCell: {
            bool current_value = GetBoolAttribute(ax::mojom::BoolAttribute::kSelected);
        ...
        case ax::mojom::Role::kMenuItemRadio: {
            if (GetData().GetCheckedState() == ax::mojom::CheckedState::kTrue)
                ReplaceIntAttribute(node_->id(), ax::mojom::IntAttribute::kCheckedState, static_cast<int32_t>(ax::mojom::CheckedState::kFalse));
            else if (GetData().GetCheckedState() == ax::mojom::CheckedState::kFalse)
                ReplaceIntAttribute(node_->id(), ax::mojom::IntAttribute::kCheckedState, static_cast<int32_t>(ax::mojom::CheckedState::kTrue));
            break;
        }
        default:
            break;
    ...
    case ax::mojom::Action::kSetValue:
        if (GetData().IsRangeValueSupported()) {
            ReplaceFloatAttribute(ax::mojom::FloatAttribute::kValueForRange, std::stof(data.value));
    ...
    case ax::mojom::Action::kSetSelection: {
        ReplaceIntAttribute(data.anchor_node_id, ax::mojom::IntAttribute::kTextSelStart, data.anchor_offset);
        ReplaceIntAttribute(data.focus_node_id, ax::mojom::IntAttribute::kTextSelEnd, data.focus_offset);
        ReplaceTreeDataTextSelection(data.anchor_node_id, data.anchor_offset, data.focus_node_id, data.focus_offset);
        return true;
    }
    ...
    case ax::mojom::Action::kFocus: {
        ...
        if (IsSelectSupported(GetRole())) {
            ...
            ReplaceBoolAttribute(ax::mojom::BoolAttribute::kSelectedFromFocus, true);
        }
...
std::u16string TestAXNodeWrapper::GetLocalizedRoleDescriptionForUnlabeledImage() const
{
...
std::u16string TestAXNodeWrapper::GetLocalizedStringForImageAnnotationStatus(ax::mojom::ImageAnnotationStatus status) const
{
    switch (status) {
...
std::u16string TestAXNodeWrapper::GetStyleNameAttributeAsLocalizedString() const
{
...
std::vector<AXPlatformNode*> TestAXNodeWrapper::GetSourceNodesForReverseRelations(ax::mojom::IntAttribute attr)
{
    DCHECK(IsNodeIdIntAttribute(attr));
    return GetNodesFromRelationIdSet(tree_->GetReverseRelations(attr, GetData().id));
}

std::vector<AXPlatformNode*> TestAXNodeWrapper::GetSourceNodesForReverseRelations(ax::mojom::IntListAttribute attr)
{
...
const std::vector<gfx::NativeViewAccessible> TestAXNodeWrapper::GetUIADirectChildrenInRange(AXPlatformNodeDelegate* start, AXPlatformNodeDelegate* end)
{
    return {};
}
...
gfx::RectF TestAXNodeWrapper::GetInlineTextRect(const int start_offset, const int end_offset) const
{
    ...
    case ax::mojom::WritingDirection::kLtr: {
        int start_pixel_offset = start_offset > 0 ? character_offsets[start_offset - 1] : location.x();
        int end_pixel_offset = end_offset > 0 ? character_offsets[end_offset - 1] : location.x();
        bounds = gfx::RectF(start_pixel_offset, location.y(), end_pixel_offset - start_pixel_offset, location.height());
        break;
    }
    default:
        NOTIMPLEMENTED();
...
AXOffscreenResult TestAXNodeWrapper::DetermineOffscreenResult(gfx::RectF bounds) const
{
    if (!tree_ || !tree_->root())
        return AXOffscreenResult::kOnscreen;

    const AXNodeData& root_web_area_node_data = tree_->root()->data();
    gfx::RectF root_web_area_bounds = root_web_area_node_data.relative_bounds.bounds;
    ...
    if (!root_web_area_bounds.IsEmpty() && !Intersects(root_web_area_bounds, bounds)) {
