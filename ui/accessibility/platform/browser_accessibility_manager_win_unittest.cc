...
public:
    TestFragmentRootDelegate(ui::BrowserAccessibilityManager* browser_accessibility_manager)
    ...
    {
        return browser_accessibility_manager_->GetBrowserAccessibilityRoot()->GetNativeViewAccessible();
    }
...
public:
    ...
    BrowserAccessibilityManagerWinTest(const BrowserAccessibilityManagerWinTest&) = delete;
    BrowserAccessibilityManagerWinTest& operator=(const BrowserAccessibilityManagerWinTest&) = delete;
...
TEST_F(BrowserAccessibilityManagerWinTest, DynamicallyAddedIFrame)
{
    ...
    test_browser_accessibility_delegate_->accelerated_widget_ = gfx::kMockAcceleratedWidget;

    std::unique_ptr<BrowserAccessibilityManager> root_manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root), node_id_delegate_, test_browser_accessibility_delegate_.get()));

    TestFragmentRootDelegate test_fragment_root_delegate(root_manager.get());

    AXPlatformNode* root_document_root_node = AXPlatformNode::FromNativeViewAccessible(root_manager->GetBrowserAccessibilityRoot()->GetNativeViewAccessible());

    std::unique_ptr<AXPlatformNodeDelegate> fragment_root = std::make_unique<AXFragmentRootWin>(gfx::kMockAcceleratedWidget, &test_fragment_root_delegate);

    EXPECT_EQ(fragment_root->GetChildCount(), 1u);
    EXPECT_EQ(fragment_root->ChildAtIndex(0), root_document_root_node->GetNativeViewAccessible());
    ...
    std::unique_ptr<TestAXPlatformTreeManagerDelegate> iframe_delegate = std::make_unique<TestAXPlatformTreeManagerDelegate>();
    ...
    std::unique_ptr<BrowserAccessibilityManager> iframe_manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(root), node_id_delegate_, iframe_delegate.get()));
...
TEST_F(BrowserAccessibilityManagerWinTest, ChildTree)
{
    ...
    AXTreeUpdate parent_tree_update = MakeAXTreeUpdateForTesting(parent_tree_root);

    child_tree_update.tree_data.parent_tree_id = parent_tree_update.tree_data.tree_id;

    test_browser_accessibility_delegate_->accelerated_widget_ = gfx::kMockAcceleratedWidget;

    std::unique_ptr<BrowserAccessibilityManager> parent_manager(
        BrowserAccessibilityManager::Create(parent_tree_update, node_id_delegate_, test_browser_accessibility_delegate_.get()));

    TestFragmentRootDelegate test_fragment_root_delegate(parent_manager.get());

    AXPlatformNode* root_document_root_node
        = AXPlatformNode::FromNativeViewAccessible(parent_manager->GetBrowserAccessibilityRoot()->GetNativeViewAccessible());
    ...
    // Add the child tree.
    std::unique_ptr<TestAXPlatformTreeManagerDelegate> child_tree_delegate = std::make_unique<TestAXPlatformTreeManagerDelegate>();
    ...
    std::unique_ptr<BrowserAccessibilityManager> child_manager(
        BrowserAccessibilityManager::Create(child_tree_update, node_id_delegate_, child_tree_delegate.get()));
