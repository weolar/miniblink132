...
#define EXPECT_UIA_ELEMENT_ARRAY_BSTR_EQ(array, element_test_property_id, expected_property_values)                                                            \
    {                                                                                                                                                          \
        ...
        for (size_t i = 0; i < count; ++i) {                                                                                                                   \
            ...
            EXPECT_UIA_GETPROPERTYVALUE_EQ(element, element_test_property_id, expected_property_values[i].c_str());                                            \
        }                                                                                                                                                      \
...
#define EXPECT_UIA_SAFEARRAY_EQ(safearray, expected_property_values)                                                                                           \
    {                                                                                                                                                          \
        ...
        LONG array_lower_bound;                                                                                                                                \
        EXPECT_HRESULT_SUCCEEDED(SafeArrayGetLBound(safearray, 1, &array_lower_bound));                                                                        \
        LONG array_upper_bound;                                                                                                                                \
        EXPECT_HRESULT_SUCCEEDED(SafeArrayGetUBound(safearray, 1, &array_upper_bound));                                                                        \
        const size_t count = array_upper_bound - array_lower_bound + 1;                                                                                        \
        EXPECT_EQ(expected_property_values.size(), count);                                                                                                     \
        if (sizeof(T) == ::SafeArrayGetElemsize(safearray) && count == expected_property_values.size()) {                                                      \
            T* array_data;                                                                                                                                     \
            EXPECT_HRESULT_SUCCEEDED(::SafeArrayAccessData(safearray, reinterpret_cast<void**>(&array_data)));                                                 \
            for (size_t i = 0; i < count; ++i) {                                                                                                               \
...
#define EXPECT_UIA_TEXTATTRIBUTE_EQ(provider, attribute, variant)                                                                                              \
    {                                                                                                                                                          \
        ...
        EXPECT_HRESULT_SUCCEEDED(provider->GetAttributeValue(attribute, scoped_variant.Receive()));                                                            \
...
#define EXPECT_UIA_TEXTATTRIBUTE_MIXED(provider, attribute)                                                                                                    \
    {                                                                                                                                                          \
        ...
        EXPECT_HRESULT_SUCCEEDED(::UiaGetReservedMixedAttributeValue(&expected_mixed));                                                                        \
        base::win::ScopedVariant scoped_variant;                                                                                                               \
        EXPECT_HRESULT_SUCCEEDED(provider->GetAttributeValue(attribute, scoped_variant.Receive()));                                                            \
...
#define EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(provider, attribute)                                                                                             \
    {                                                                                                                                                          \
        ...
        EXPECT_HRESULT_SUCCEEDED(::UiaGetReservedNotSupportedValue(&expected_notsupported));                                                                   \
        base::win::ScopedVariant scoped_variant;                                                                                                               \
        EXPECT_HRESULT_SUCCEEDED(provider->GetAttributeValue(attribute, scoped_variant.Receive()));                                                            \
        EXPECT_EQ(VT_UNKNOWN, scoped_variant.type());                                                                                                          \
...
#define EXPECT_UIA_TEXTRANGE_EQ(provider, expected_content)                                                                                                    \
    {                                                                                                                                                          \
        ...
        EXPECT_HRESULT_SUCCEEDED(provider->GetText(-1, provider_content.Receive()));                                                                           \
...
#define EXPECT_UIA_FIND_TEXT(text_range_provider, search_term, ignore_case, owner)                                                                             \
    {                                                                                                                                                          \
        ...
        EXPECT_HRESULT_SUCCEEDED(text_range_provider->FindText(find_string.Get(), false, ignore_case, &text_range_provider_found));                            \
        SetOwner(owner, text_range_provider_found.Get());                                                                                                      \
        base::win::ScopedBstr found_content;                                                                                                                   \
        EXPECT_HRESULT_SUCCEEDED(text_range_provider_found->GetText(-1, found_content.Receive()));                                                             \
        if (ignore_case)                                                                                                                                       \
...
#define EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, endpoint, unit, count, expected_text, expected_count)                                            \
    {                                                                                                                                                          \
        int result_count;                                                                                                                                      \
        EXPECT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(endpoint, unit, count, &result_count));                                               \
...
#define EXPECT_UIA_MOVE(text_range_provider, unit, count, expected_text, expected_count)                                                                       \
    {                                                                                                                                                          \
        int result_count;                                                                                                                                      \
        EXPECT_HRESULT_SUCCEEDED(text_range_provider->Move(unit, count, &result_count));                                                                       \
        EXPECT_EQ(expected_count, result_count);                                                                                                               \
        EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, expected_text);                                                                                           \
    }
...
#define EXPECT_ENCLOSING_ELEMENT(ax_node_given, ax_node_expected)                                                                                              \
    {                                                                                                                                                          \
        ...
        ASSERT_HRESULT_SUCCEEDED(text_range_provider->GetEnclosingElement(&enclosing_element));                                                                \
        ComPtr<IRawElementProviderSimple> expected_text_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(ax_node_expected);                        \
...
public:
    ...
    AXPlatformNodeWin* GetOwner(const AXPlatformNodeTextRangeProviderWin* text_range)
    {
    ...
    void CopyOwnerToClone(ITextRangeProvider* source_range, ITextRangeProvider* destination_range)
    {
        ...
        destination_provider->QueryInterface(IID_PPV_ARGS(&destination_provider_internal));
        destination_provider_internal->SetOwnerForTesting(source_provider_internal->GetOwner());
    }
    ...
    void SetOwner(AXPlatformNodeWin* owner, ITextRangeProvider* destination_range)
    {
        ...
        destination_range->QueryInterface(IID_PPV_ARGS(&destination_provider_internal));
    ...
    void GetTextRangeProviderFromTextNode(ComPtr<ITextRangeProvider>& text_range_provider, AXNode* text_node)
    {
        ComPtr<IRawElementProviderSimple> provider_simple = QueryInterfaceFromNode<IRawElementProviderSimple>(text_node);
        ASSERT_NE(nullptr, provider_simple.Get());

        ComPtr<ITextProvider> text_provider;
        EXPECT_HRESULT_SUCCEEDED(provider_simple->GetPatternProvider(UIA_TextPatternId, &text_provider));
        ASSERT_NE(nullptr, text_provider.Get());

        EXPECT_HRESULT_SUCCEEDED(text_provider->get_DocumentRange(&text_range_provider));
        ASSERT_NE(nullptr, text_range_provider.Get());

        ComPtr<AXPlatformNodeTextRangeProviderWin> text_range_provider_interal;
        EXPECT_HRESULT_SUCCEEDED(text_range_provider->QueryInterface(IID_PPV_ARGS(&text_range_provider_interal)));
        AXPlatformNode* ax_platform_node = AXPlatformNodeFromNode(text_node);
        ASSERT_NE(ax_platform_node, nullptr);
        text_range_provider_interal->SetOwnerForTesting(static_cast<AXPlatformNodeWin*>(ax_platform_node));
    }

    void CreateTextRangeProviderWin(ComPtr<AXPlatformNodeTextRangeProviderWin>& text_range_provider_win, AXPlatformNodeWin* owner, const AXNode* start_anchor,
        int start_offset, ax::mojom::TextAffinity start_affinity, const AXNode* end_anchor, int end_offset, ax::mojom::TextAffinity end_affinity)
    {
        AXNodePosition::AXPositionInstance range_start = CreateTextPosition(*start_anchor, start_offset, start_affinity);
        AXNodePosition::AXPositionInstance range_end = CreateTextPosition(*end_anchor, end_offset, end_affinity);

        ComPtr<ITextRangeProvider> text_range_provider;
        AXPlatformNodeTextRangeProviderWin::CreateTextRangeProviderForTesting(owner, std::move(range_start), std::move(range_end), &text_range_provider);
    ...
    AXTreeUpdate BuildTextDocument(
        const std::vector<std::string>& text_nodes_content, bool build_word_boundaries_offsets = false, bool place_text_on_one_line = false)
    {
        int current_id = 0;
        ...
        for (const std::string& text_content : text_nodes_content) {
            ...
            if (build_word_boundaries_offsets) {
                ...
                ComputeWordBoundariesOffsets(text_content, word_start_offsets, word_end_offsets);
                inline_box_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, word_start_offsets);
                inline_box_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, word_end_offsets);
            }

            if (place_text_on_one_line && !update.nodes.empty()) {
                AXNodeData* previous_inline_box_data = &update.nodes.back();
                static_text_data.AddIntAttribute(ax::mojom::IntAttribute::kPreviousOnLineId, previous_inline_box_data->id);
                inline_box_data.AddIntAttribute(ax::mojom::IntAttribute::kPreviousOnLineId, previous_inline_box_data->id);
                previous_inline_box_data->AddIntAttribute(ax::mojom::IntAttribute::kNextOnLineId, inline_box_data.id);
            }
    ...
    AXTreeUpdate BuildAXTreeForBoundingRectangles()
    {
        ...
        button.AddIntAttribute(ax::mojom::IntAttribute::kNextOnLineId, check_box.id);
        ...
        check_box.AddIntAttribute(ax::mojom::IntAttribute::kPreviousOnLineId, button.id);
        ...
        text_field.AddIntListAttribute(ax::mojom::IntListAttribute::kLineStarts, std::vector<int32_t> { 0, 7 });
        ...
        inline_box1.AddIntAttribute(ax::mojom::IntAttribute::kNextOnLineId, line_break.id);
        ...
        line_break.AddIntAttribute(ax::mojom::IntAttribute::kPreviousOnLineId, inline_box1.id);
        line_break.child_ids.push_back(inline_box_line_break.id);

        inline_box_line_break.role = ax::mojom::Role::kInlineTextBox;
        inline_box_line_break.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        inline_box_line_break.SetName(LINE_BREAK_TEXT);
        inline_box_line_break.relative_bounds.bounds = gfx::RectF(250, 20, 0, 30);
        inline_box_line_break.AddIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets, { 0 });
        inline_box_line_break.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, std::vector<int32_t> { 0 });
        inline_box_line_break.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, std::vector<int32_t> { 0 });
        ...
        inline_box2.AddIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets, character_offsets2);
        inline_box2.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, std::vector<int32_t> { 0, 5 });
        inline_box2.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, std::vector<int32_t> { 4, 6 });

        AXTreeUpdate update;
        update.has_tree_data = true;
        update.root_id = ROOT_ID;
        update.nodes = { root, button, check_box, text_field, static_text1, inline_box1, line_break, inline_box_line_break, static_text2, inline_box2 };
        update.tree_data.tree_id = AXTreeID::CreateNewAXTreeID();
        return update;
    }

    const std::wstring tree_for_move_full_text = L"First line of text\nStandalone line\n"
                                                 L"bold text\nParagraph 1\nParagraph 2";
    ...
    AXTreeUpdate BuildAXTreeForMove()
    {
        ...
        group1_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        std::vector<int> word_start_offsets;
        ComputeWordBoundariesOffsets(text_content, word_start_offsets, word_end_offsets);
        text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, word_start_offsets);
        text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, word_end_offsets);
        ...
        line_break1_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        ComputeWordBoundariesOffsets(text_content, word_start_offsets, word_end_offsets);
        standalone_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, word_start_offsets);
        standalone_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, word_end_offsets);

        AXNodeData line_break2_data;
        line_break2_data.id = 7;
        line_break2_data.role = ax::mojom::Role::kLineBreak;
        line_break2_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        line_break2_data.SetName("\n");

        group2_data.child_ids = { line_break1_data.id, standalone_text_data.id, line_break2_data.id };
        standalone_text_data.AddIntAttribute(ax::mojom::IntAttribute::kNextOnLineId, line_break2_data.id);
        line_break2_data.AddIntAttribute(ax::mojom::IntAttribute::kPreviousOnLineId, standalone_text_data.id);

        AXNodeData bold_text_data;
        bold_text_data.id = 8;
        bold_text_data.role = ax::mojom::Role::kStaticText;
        bold_text_data.AddIntAttribute(ax::mojom::IntAttribute::kTextStyle, static_cast<int32_t>(ax::mojom::TextStyle::kBold));
        text_content = "bold text";
        bold_text_data.SetName(text_content);
        ComputeWordBoundariesOffsets(text_content, word_start_offsets, word_end_offsets);
        bold_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, word_start_offsets);
        bold_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, word_end_offsets);

        AXNodeData paragraph1_data;
        paragraph1_data.id = 9;
        paragraph1_data.role = ax::mojom::Role::kParagraph;
        paragraph1_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        ComputeWordBoundariesOffsets(text_content, word_start_offsets, word_end_offsets);
        paragraph1_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, word_start_offsets);
        paragraph1_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, word_end_offsets);
        paragraph1_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        paragraph2_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        ComputeWordBoundariesOffsets(text_content, word_start_offsets, word_end_offsets);
        paragraph2_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, word_start_offsets);
        paragraph2_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, word_end_offsets);
        paragraph1_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        root_data.id = 1;
        root_data.role = ax::mojom::Role::kRootWebArea;
        root_data.child_ids = { group1_data.id, group2_data.id, bold_text_data.id, paragraph1_data.id, paragraph2_data.id };

        AXTreeUpdate update;
        update.has_tree_data = true;
        update.root_id = root_data.id;
        update.nodes = { root_data, group1_data, text_data, group2_data, line_break1_data, standalone_text_data, line_break2_data, bold_text_data,
            paragraph1_data, paragraph1_text_data, ignored_text_data, paragraph2_data, paragraph2_text_data };
        update.tree_data.tree_id = AXTreeID::CreateNewAXTreeID();
        return update;
    }
    ...
    AXTreeUpdate BuildAXTreeForMoveByFormat()
    {
        ...
        group1_data.id = 2;
        group1_data.role = ax::mojom::Role::kGenericContainer;
        group1_data.AddStringAttribute(ax::mojom::StringAttribute::kFontFamily, "test font");
        group1_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        group2_data.id = 4;
        group2_data.role = ax::mojom::Role::kGenericContainer;
        group2_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        group3_data.AddIntAttribute(ax::mojom::IntAttribute::kTextStyle, static_cast<int32_t>(ax::mojom::TextStyle::kBold));
        group3_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        paragraph2_data.id = 12;
        paragraph2_data.role = ax::mojom::Role::kParagraph;
        paragraph2_data.AddFloatAttribute(ax::mojom::FloatAttribute::kFontSize, 1.0f);
        paragraph2_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        paragraph3_data.AddFloatAttribute(ax::mojom::FloatAttribute::kFontSize, 1.0f);
        paragraph3_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        paragraph4_data.AddFloatAttribute(ax::mojom::FloatAttribute::kFontSize, 2.0f);
        paragraph4_data.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
        ...
        root_data.role = ax::mojom::Role::kRootWebArea;
        root_data.child_ids
            = { group1_data.id, group2_data.id, group3_data.id, paragraph1_data.id, paragraph2_data.id, paragraph3_data.id, paragraph4_data.id };

        AXTreeUpdate update;
        update.has_tree_data = true;
        update.root_id = root_data.id;
        update.nodes = { root_data, group1_data, text_data, group2_data, line_break1_data, standalone_text_data, line_break2_data, group3_data, bold_text_data,
            paragraph1_data, paragraph1_text_data, paragraph2_data, paragraph2_text_data, paragraph3_data, paragraph3_text_data, paragraph4_data,
    ...
    AXTreeUpdate BuildAXTreeForMoveByPage()
    {
        ...
        update.has_tree_data = true;
        update.root_id = root_data.id;
        update.nodes = { root_data, page_1_data, page_1_text_data, page_2_data, page_2_text_data, page_3_data, page_3_text_data };
        update.tree_data.tree_id = AXTreeID::CreateNewAXTreeID();
        return update;
    }
...
class MockAXPlatformNodeTextRangeProviderWin : public SequenceAffineComObjectRoot, public ITextRangeProvider {
public:
    ...
    {
        CComObject<MockAXPlatformNodeTextRangeProviderWin>* text_range_provider = nullptr;
        HRESULT hr = CComObject<MockAXPlatformNodeTextRangeProviderWin>::CreateInstance(&text_range_provider);
        if (SUCCEEDED(hr)) {
    ...
    IFACEMETHODIMP CompareEndpoints(
        TextPatternRangeEndpoint this_endpoint, ITextRangeProvider* other, TextPatternRangeEndpoint other_endpoint, int* result) override
    {
        return E_NOTIMPL;
    }
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderClone)
{
    ...
    ComPtr<ITextRangeProvider> text_range_provider;
    GetTextRangeProviderFromTextNode(text_range_provider, GetRoot()->children()[0]);
...
TEST_F(AXPlatformNodeTextRangeProviderTest, CompareWithInvalidatedPositions)
{
    ...
    AXNodePosition::AXPositionInstance range_start = CreateTextPosition(/* anchor */ *st_node, /* text_offset*/ 0,
    ...
    AXNodePosition::AXPositionInstance range_end = CreateTextPosition(/* anchor */ *st_node, /* text_offset*/ 3,
        /* affinity*/ ax::mojom::TextAffinity::kDownstream);

    ComPtr<ITextRangeProvider> text_range_provider_b;
    AXPlatformNodeTextRangeProviderWin::CreateTextRangeProviderForTesting(
        static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(st_node)), std::move(range_start), std::move(range_end), &text_range_provider_b);

    BOOL are_same;
    text_range_provider_a->Compare(text_range_provider_b.Get(), &are_same);
}

TEST_F(AXPlatformNodeTextRangeProviderTest, CompareEndpointsWithInvalidatedPositions)
{
    TestAXTreeUpdate initial_state(std::string(R"HTML(
    ++1 kRootWebArea
    ++++2 kStaticText name="aa"
    ...
    int result;
    text_range_provider_a->CompareEndpoints(TextPatternRangeEndpoint_End, text_range_provider_b.Get(), TextPatternRangeEndpoint_End, &result);
}
...
TEST_F(AXPlatformNodeTextRangeProviderTest, MoveByRangeInvalidatedPositions)
{
    ...
    text_range_provider_a->MoveEndpointByRange(TextPatternRangeEndpoint_End, text_range_provider_b.Get(), TextPatternRangeEndpoint_End);
}

TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderCompareEndpoints)
{
    Init(BuildTextDocument({ "some text", "more text" }, false /* build_word_boundaries_offsets */, true /* place_text_on_one_line */));
    ...
    ComPtr<ITextRangeProvider> text_range_provider;
    GetTextRangeProviderFromTextNode(text_range_provider, root_node->children()[0]);

    // Get the textRangeProvider for "more text".
    ComPtr<ITextRangeProvider> more_text_range_provider;
    GetTextRangeProviderFromTextNode(more_text_range_provider, root_node->children()[1]);
    ...
    EXPECT_HRESULT_SUCCEEDED(document_text_range_provider->CompareEndpoints(
        TextPatternRangeEndpoint_Start, document_text_range_provider.Get(), TextPatternRangeEndpoint_Start, &result));
    EXPECT_EQ(0, result);

    EXPECT_HRESULT_SUCCEEDED(document_text_range_provider->CompareEndpoints(
        TextPatternRangeEndpoint_End, document_text_range_provider.Get(), TextPatternRangeEndpoint_End, &result));
    EXPECT_EQ(0, result);

    EXPECT_HRESULT_SUCCEEDED(document_text_range_provider->CompareEndpoints(
        TextPatternRangeEndpoint_Start, document_text_range_provider.Get(), TextPatternRangeEndpoint_End, &result));
    EXPECT_EQ(-1, result);

    EXPECT_HRESULT_SUCCEEDED(document_text_range_provider->CompareEndpoints(
        TextPatternRangeEndpoint_End, document_text_range_provider.Get(), TextPatternRangeEndpoint_Start, &result));
    EXPECT_EQ(1, result);
    ...
    EXPECT_HRESULT_SUCCEEDED(
        text_range_provider->CompareEndpoints(TextPatternRangeEndpoint_Start, more_text_range_provider.Get(), TextPatternRangeEndpoint_Start, &result));
    EXPECT_EQ(-1, result);

    EXPECT_HRESULT_SUCCEEDED(
        text_range_provider->CompareEndpoints(TextPatternRangeEndpoint_End, more_text_range_provider.Get(), TextPatternRangeEndpoint_Start, &result));
    EXPECT_EQ(0, result);
    ...
    EXPECT_HRESULT_SUCCEEDED(
        text_range_provider->CompareEndpoints(TextPatternRangeEndpoint_Start, document_text_range_provider.Get(), TextPatternRangeEndpoint_Start, &result));
    EXPECT_EQ(0, result);

    EXPECT_HRESULT_SUCCEEDED(
        text_range_provider->CompareEndpoints(TextPatternRangeEndpoint_End, document_text_range_provider.Get(), TextPatternRangeEndpoint_End, &result));
    EXPECT_EQ(-1, result);
    ...
    EXPECT_HRESULT_SUCCEEDED(
        more_text_range_provider->CompareEndpoints(TextPatternRangeEndpoint_End, document_text_range_provider.Get(), TextPatternRangeEndpoint_End, &result));
    EXPECT_EQ(0, result);
}
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderExpandToEnclosingCharacter)
{
    ...
    int count;
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ 2, &count));
    ASSERT_EQ(2, count);
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ 1, &count));
    ASSERT_EQ(1, count);
    ...
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ 9, &count));
    ASSERT_EQ(9, count);
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ 8, &count));
    ASSERT_EQ(8, count);
    ...
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ 9, &count));
    ASSERT_EQ(8, count);
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ 9, &count));
    ASSERT_EQ(9, count);
    ...
    ComPtr<IRawElementProviderSimple> more_text_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(root_node->children()[1]);
    ComPtr<IRawElementProviderSimple> enclosing_element;
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->GetEnclosingElement(&enclosing_element));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderExpandToEnclosingWord)
{
    ...
    ComPtr<ITextRangeProvider> text_range_provider;
    GetTextRangeProviderFromTextNode(text_range_provider, GetRoot()->children()[1]);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"definitely not text");

    // Start endpoint is already on a word's start boundary.
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->ExpandToEnclosingUnit(TextUnit_Word));
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"definitely ");

    // Start endpoint is between a word's start and end boundaries.
    int count;
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ -2, &count));
    ASSERT_EQ(-2, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"xtdefinitely ");

    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ 4, &count));
    ASSERT_EQ(4, count);
    ...
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ 18, &count));
    ASSERT_EQ(18, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"");

    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ 1, &count));
    ASSERT_EQ(1, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L" ");
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderExpandToEnclosingLine)
{
    ...
    int count;
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ -11, &count));
    ASSERT_EQ(-7, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"");

    ASSERT_HRESULT_SUCCEEDED(text_range_provider->ExpandToEnclosingUnit(TextUnit_Line));
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"line #1");

    // Start endpoint is between a line's start and end boundaries.
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ 13, &count));
    ASSERT_EQ(13, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"");
    ...
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ 29, &count));
    ASSERT_EQ(25, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"");
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderExpandToEnclosingParagraph)
{
    ...
    int count;
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Paragraph, /*count*/ -5, &count));
    EXPECT_EQ(-5, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"");

    ASSERT_HRESULT_SUCCEEDED(text_range_provider->ExpandToEnclosingUnit(TextUnit_Paragraph));
    ...
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Line, /*count*/ 2, &count));
    EXPECT_EQ(2, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"");
    ...
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Line, /*count*/ 1, &count));
    EXPECT_EQ(1, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"");
    ...
    // paragraph.
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Document, /*count*/ 1, &count));
    EXPECT_EQ(1, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"");
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->ExpandToEnclosingUnit(TextUnit_Paragraph));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderExpandToEnclosingFormat)
{
    ...
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->QueryInterface(IID_PPV_ARGS(&text_range_provider_internal)));
    ...
    int count;
    ASSERT_HRESULT_SUCCEEDED(units_a_b_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Line, /*count*/ 5, &count));
    ASSERT_EQ(5, count);
    ASSERT_HRESULT_SUCCEEDED(units_a_b_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Line, /*count*/ -1, &count));
    ASSERT_EQ(-1, count);
    EXPECT_UIA_TEXTRANGE_EQ(units_a_b_provider, L"Paragraph 1\nParagraph 2\nParagraph 3");
    ...
    ASSERT_HRESULT_SUCCEEDED(unit_a_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Line, /*count*/ -2, &count));
    ASSERT_EQ(-2, count);
    ...
    // Case 1: Degenerate range at start of A.
    {
        ...
        ASSERT_HRESULT_SUCCEEDED(
            test_case_provider->MoveEndpointByRange(TextPatternRangeEndpoint_End, test_case_provider.Get(), TextPatternRangeEndpoint_Start));
        EXPECT_UIA_TEXTRANGE_EQ(test_case_provider, L"");

        ASSERT_HRESULT_SUCCEEDED(test_case_provider->ExpandToEnclosingUnit(TextUnit_Format));
        BOOL are_same;
        ASSERT_HRESULT_SUCCEEDED(test_case_provider->Compare(unit_a_provider.Get(), &are_same));
    ...
    // Case 2: Range from start of A to middle of A.
    {
        ...
        ASSERT_HRESULT_SUCCEEDED(unit_a_provider->Clone(&test_case_provider));
        CopyOwnerToClone(unit_a_provider.Get(), test_case_provider.Get());
        ASSERT_HRESULT_SUCCEEDED(test_case_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ -7, &count));
        ASSERT_EQ(-7, count);
    ...
    // Case 4: Range from start of A to middle of B.
    {
        ...
        ASSERT_HRESULT_SUCCEEDED(unit_a_provider->Clone(&test_case_provider));
        CopyOwnerToClone(unit_a_provider.Get(), test_case_provider.Get());
        ASSERT_HRESULT_SUCCEEDED(test_case_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ 5, &count));
        ASSERT_EQ(5, count);
        EXPECT_UIA_TEXTRANGE_EQ(test_case_provider, L"Paragraph 1\nPara");
    ...
    // Case 5: Degenerate range in middle of A.
    {
        ...
        ASSERT_HRESULT_SUCCEEDED(unit_a_provider->Clone(&test_case_provider));
        CopyOwnerToClone(unit_a_provider.Get(), test_case_provider.Get());
        ASSERT_HRESULT_SUCCEEDED(test_case_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ 4, &count));
        ASSERT_EQ(4, count);
        ASSERT_HRESULT_SUCCEEDED(
            test_case_provider->MoveEndpointByRange(TextPatternRangeEndpoint_End, test_case_provider.Get(), TextPatternRangeEndpoint_Start));
        EXPECT_UIA_TEXTRANGE_EQ(test_case_provider, L"");
    ...
    // Case 6: Range from middle of A to middle of A.
    {
        ...
        ASSERT_HRESULT_SUCCEEDED(test_case_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ 4, &count));
        ASSERT_EQ(4, count);
        ASSERT_HRESULT_SUCCEEDED(test_case_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ -2, &count));
        ASSERT_EQ(-2, count);
    ...
    // Case 8: Range from middle of A to middle of B.
    {
        ...
        ASSERT_HRESULT_SUCCEEDED(unit_a_provider->Clone(&test_case_provider));
        CopyOwnerToClone(unit_a_provider.Get(), test_case_provider.Get());
        ASSERT_HRESULT_SUCCEEDED(test_case_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ 5, &count));
        ASSERT_EQ(5, count);
        ASSERT_HRESULT_SUCCEEDED(test_case_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ 5, &count));
        ASSERT_EQ(5, count);
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderExpandToEnclosingFormatWithEmptyObjects)
{
    ...
    int count;
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ 5, &count));
    ASSERT_EQ(5, count);
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ -1, &count));
    ASSERT_EQ(-1, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"");
    ...
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->ExpandToEnclosingUnit(TextUnit_Format));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderExpandToEnclosingDocument)
{
    ...
    for (auto& textunit : textunit_types) {
        ...
        ASSERT_HRESULT_SUCCEEDED(text_range_provider->ExpandToEnclosingUnit(textunit));
        EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"some textmore texteven more text");
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderIgnoredForTextNavigation)
{
    TestAXTreeUpdate update(std::string(R"HTML(
    ++1 kRootWebArea
    ++++2 kStaticText
    ...
    int count;
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Paragraph, /*count*/ 1, &count));
    ASSERT_EQ(1, count);
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderInvalidCalls)
{
    ...
    // validate call.
    {
        ...
        DestroyTree();
        ComPtr<ITextRangeProvider> text_range_provider_clone;
        EXPECT_UIA_ELEMENTNOTAVAILABLE(text_range_provider->Clone(&text_range_provider_clone));

        BOOL compare_result;
        EXPECT_UIA_ELEMENTNOTAVAILABLE(text_range_provider->Compare(text_range_provider.Get(), &compare_result));

        int compare_endpoints_result;
        EXPECT_UIA_ELEMENTNOTAVAILABLE(text_range_provider->CompareEndpoints(
            TextPatternRangeEndpoint_Start, text_range_provider.Get(), TextPatternRangeEndpoint_Start, &compare_endpoints_result));

        VARIANT attr_val;
        V_VT(&attr_val) = VT_BOOL;
        V_BOOL(&attr_val) = VARIANT_TRUE;
        ComPtr<ITextRangeProvider> matched_range_provider;
        EXPECT_UIA_ELEMENTNOTAVAILABLE(text_range_provider->FindAttribute(UIA_IsHiddenAttributeId, attr_val, true, &matched_range_provider));

        EXPECT_UIA_ELEMENTNOTAVAILABLE(
            text_range_provider->MoveEndpointByRange(TextPatternRangeEndpoint_Start, text_range_provider.Get(), TextPatternRangeEndpoint_Start));
    ...
    // performed on the other provider.
    {
        ...
        MockAXPlatformNodeTextRangeProviderWin::CreateMockTextRangeProvider(&other_provider_different_type);

        BOOL compare_result;
        EXPECT_UIA_INVALIDOPERATION(this_provider->Compare(other_provider_different_type.Get(), &compare_result));

        int compare_endpoints_result;
        EXPECT_UIA_INVALIDOPERATION(this_provider->CompareEndpoints(
            TextPatternRangeEndpoint_Start, other_provider_different_type.Get(), TextPatternRangeEndpoint_Start, &compare_endpoints_result));

        EXPECT_UIA_INVALIDOPERATION(
            this_provider->MoveEndpointByRange(TextPatternRangeEndpoint_Start, other_provider_different_type.Get(), TextPatternRangeEndpoint_Start));
    }
}
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderGetText)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetText(-1, text_content.Receive()));
    EXPECT_STREQ(text_content.Get(), L"some text");
    text_content.Reset();

    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetText(4, text_content.Receive()));
    EXPECT_STREQ(text_content.Get(), L"some");
    text_content.Reset();

    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetText(0, text_content.Receive()));
    EXPECT_STREQ(text_content.Get(), L"");
    text_content.Reset();

    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetText(9, text_content.Receive()));
    EXPECT_STREQ(text_content.Get(), L"some text");
    text_content.Reset();

    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetText(10, text_content.Receive()));
    EXPECT_STREQ(text_content.Get(), L"some text");
    text_content.Reset();

    EXPECT_HRESULT_FAILED(text_range_provider->GetText(-1, nullptr));

    EXPECT_HRESULT_FAILED(text_range_provider->GetText(-2, text_content.Receive()));
    text_content.Reset();

    ComPtr<ITextRangeProvider> document_textrange;
    GetTextRangeProviderFromTextNode(document_textrange, root_node);

    EXPECT_HRESULT_SUCCEEDED(document_textrange->GetText(-1, text_content.Receive()));
    EXPECT_STREQ(text_content.Get(), L"some textmore text");
    text_content.Reset();
}

TEST_F(AXPlatformNodeTextRangeProviderTest, TestGetVisibleRangesFindTextGetTextPipeline)
{
    TestAXTreeUpdate update(std::string(R"HTML(
    ++1 kRootWebArea
    ...
    ComPtr<ITextProvider> text_provider;
    EXPECT_HRESULT_SUCCEEDED(root_node->GetPatternProvider(UIA_TextPatternId, &text_provider));

    ComPtr<ITextRangeProvider> range;
    EXPECT_HRESULT_SUCCEEDED(text_provider->get_DocumentRange(&range));

    AXPlatformNodeWin* owner = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(GetNode(2)));
    ...
    EXPECT_HRESULT_SUCCEEDED(range->FindText(find_string.Get(), false, false, &text_range_provider_found));
    SetOwner(owner, text_range_provider_found.Get());
    ...
    EXPECT_HRESULT_SUCCEEDED(SafeArrayGetElement(selection.Get(), &index, static_cast<void**>(&selected_text_range_provider)));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveCharacter)
{
    ...
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_End, TextUnit_Character,
        /*count*/ -1,
        /*expected_text*/ L"",
        /*expected_count*/ -1);
    EXPECT_UIA_MOVE(text_range_provider, TextUnit_Character,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveFormat)
{
    ...
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_End, TextUnit_Format,
        /*count*/ -1,
        /*expected_text*/ L"",
        /*expected_count*/ -1);
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_End, TextUnit_Format,
    ...
    EXPECT_UIA_MOVE(text_range_provider, TextUnit_Format,
        ...
        /*expected_text*/ L"Paragraph 4",
        /*expected_count*/ 5);
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_Start, TextUnit_Format,
        /*count*/ 1,
        /*expected_text*/ L"",
        /*expected_count*/ 1);
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_Start, TextUnit_Format,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMovePage)
{
    ...
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_End, TextUnit_Page,
        /*count*/ -1,
    ...
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_Start, TextUnit_Character,
        /*count*/ 2,
        /*expected_text*/ L"me text on page",
        /*expected_count*/ 2);

    ASSERT_HRESULT_SUCCEEDED(text_range_provider->ExpandToEnclosingUnit(TextUnit_Page));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveWord)
{
    ...
    // Degenerate range moves.
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_End, TextUnit_Word,
        /*count*/ -1,
        /*expected_text*/ L"",
        /*expected_count*/ -1);
    EXPECT_UIA_MOVE(text_range_provider, TextUnit_Word,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveLine)
{
    ...
    // Degenerate range moves.
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_End, TextUnit_Line,
        /*count*/ -1,
        /*expected_text*/ L"",
        /*expected_count*/ -1);
    EXPECT_UIA_MOVE(text_range_provider, TextUnit_Line,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveParagraph)
{
    Init(BuildAXTreeForMove());
    AXNode* root_node = GetRoot();
    ...
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_End, TextUnit_Paragraph,
        /*count*/ -4,
        /*expected_text*/ L"First line of text\n",
        /*expected_count*/ -4);
    ...
    EXPECT_UIA_MOVE(text_range_provider, TextUnit_Paragraph,
        ...
        /*expected_text*/ L"Paragraph 2",
        /*expected_count*/ 4);
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_Start, TextUnit_Paragraph,
        /*count*/ 1,
        /*expected_text*/ L"",
        /*expected_count*/ 1);
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_Start, TextUnit_Paragraph,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveDocument)
{
    Init(BuildAXTreeForMove());
    AXNode* root_node = GetRoot();
    ...
    // Degenerate range moves.
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_End, TextUnit_Document,
        /*count*/ -2,
        /*expected_text*/ L"",
        /*expected_count*/ -1);
    EXPECT_UIA_MOVE(text_range_provider, TextUnit_Page,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveEndpointByDocument)
{
    ...
    for (auto& textunit : textunit_types) {
        ...
        EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_End, textunit,
            /*count*/ 0,
        ...
        EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_Start, textunit,
            /*count*/ 3,
            /*expected_text*/ L"",
            /*expected_count*/ 1);
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveEndpointByCharacterMultilingual)
{
    ...
    const std::string hindi = base::UTF16ToUTF8(u"\x0939\x093F\x0928\x094D\x0926\x0940");
    ...
    const std::string thai = base::UTF16ToUTF8(u"\x0E23\x0E39\x0E49\x0E2A\x0E36\x0E01");
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveEndpointByWord)
{
    Init(BuildTextDocument({ "some text", "more text", "even more text" },
        /*build_word_boundaries_offsets*/ true));
    ...
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_Start, TextUnit_Word,
        /*count*/ 1,
        /*expected_text*/ L"text",
        /*expected_count*/ 1);
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_Start, TextUnit_Word,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveEndpointByLine)
{
    Init(BuildTextDocument({ "0", "1", "2", "3", "4", "5", "6" }));

    ComPtr<ITextRangeProvider> text_range_provider;
    GetTextRangeProviderFromTextNode(text_range_provider, GetRoot()->children()[3]);
    ...
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_Start, TextUnit_Line,
        /*count*/ -2,
        /*expected_text*/ L"123",
        /*expected_count*/ -2);
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(text_range_provider, TextPatternRangeEndpoint_Start, TextUnit_Line,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveEndpointByUnitTextField)
{
    ...
    text_input_data.AddStringAttribute(ax::mojom::StringAttribute::kHtmlTag, "input");
    text_input_data.AddStringAttribute(ax::mojom::StringAttribute::kInputType, "text");
    ...
    ComputeWordBoundariesOffsets(text_content, word_start_offsets, word_end_offsets);
    more_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, word_start_offsets);
    more_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, word_end_offsets);
    ...
    ComputeWordBoundariesOffsets(text_content, word_start_offsets, word_end_offsets);
    empty_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordStarts, word_start_offsets);
    empty_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kWordEnds, word_end_offsets);
    ...
    update.has_tree_data = true;
    update.root_id = root_data.id;
    update.nodes = { root_data, group1_data, text_data, text_input_data, group2_data, more_text_data, empty_text_data };
    ...
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Word, /*count*/ 1, &count));
    ASSERT_EQ(1, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"some text\n\xFFFC\n");
    ...
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Line, /*count*/ 1, &count));
    ASSERT_EQ(1, count);
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"some text\n\xFFFC");
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestMoveByCharacterEmptyTextfield)
{
    ...
    int count;
    // Tests for TextUnit_Character.
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_Start, TextUnit_Character, /*count*/ 7, &count));
    ASSERT_EQ(7, count);
    // Note that by design, empty objects such as empty text fields, are placed in
    // their own paragraph for easier screen reader navigation.
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider, L"\nworld");
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveEndpointByFormat)
{
    Init(BuildAXTreeForMoveByFormat());
    AXNode* root_node = GetRoot();
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderCompare)
{
    ...
    // that they are equal.
    BOOL result;
    EXPECT_HRESULT_SUCCEEDED(document_text_range_provider->Compare(document_text_range_provider.Get(), &result));
    EXPECT_TRUE(result);

    // Compare the text range of the entire document with one of its child, which
    // should return that they are not equal.
    EXPECT_HRESULT_SUCCEEDED(document_text_range_provider->Compare(text_range_provider.Get(), &result));
    EXPECT_FALSE(result);
    ...
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->Compare(more_text_range_provider.Get(), &result));
    EXPECT_FALSE(result);
}
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderGetBoundingRectangles)
{
    ...
    // |---------------------||---------------------||----|   |------|
    GetTextRangeProviderFromTextNode(text_range_provider, GetRoot());
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetBoundingRectangles(rectangles.Receive()));
    ...
    // <button>Button</button><input type="checkbox">Line 1<br>Line 2
    // |---------------------||---------------------||----|   |----|
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Character, /*count*/ -1, &units_moved));
    ...
    // <button>Button</button><input type="checkbox">Line 1<br>Line 2
    // |---------------------||---------------------||--------|
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Line, /*count*/ -1, &units_moved));
    ASSERT_EQ(-1, units_moved);
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetBoundingRectangles(rectangles.Receive()));
    expected_values = { 20, 20, 200, 30, /* button */
        20, 50, 200, 30, /* check box */
    ...
    // <button>Button</button><input type="checkbox">Line 1<br>Line 2
    // |---------------------||---------------------|
    ASSERT_HRESULT_SUCCEEDED(text_range_provider->MoveEndpointByUnit(TextPatternRangeEndpoint_End, TextUnit_Word, /*count*/ -3, &units_moved));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderGetEnclosingElement)
{
    ...
    search_box.AddStringAttribute(ax::mojom::StringAttribute::kInputType, "search");
    ...
    update.has_tree_data = true;
    update.root_id = root_data.id;
    update.nodes = { root_data, paragraph_data, static_text_data1, inline_text_data1, link_data, static_text_data2, inline_text_data2, link_data2, list_data,
        list_item_data, static_text_data3, inline_text_data3, search_box, search_text, pdf_highlight_data, static_text_data4, inline_text_data4 };
    Init(update);
    ...
    AXPlatformNodeWin* owner = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(paragraph_node));
    ASSERT_NE(owner, nullptr);

    ComPtr<IRawElementProviderSimple> link_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(link_node);
    ComPtr<IRawElementProviderSimple> static_text_node_raw1 = QueryInterfaceFromNode<IRawElementProviderSimple>(static_text_node1);
    ComPtr<IRawElementProviderSimple> static_text_node_raw2 = QueryInterfaceFromNode<IRawElementProviderSimple>(static_text_node2);
    ComPtr<IRawElementProviderSimple> static_text_node_raw3 = QueryInterfaceFromNode<IRawElementProviderSimple>(static_text_node3);
    ComPtr<IRawElementProviderSimple> inline_text_node_raw1 = QueryInterfaceFromNode<IRawElementProviderSimple>(inline_text_node1);
    ComPtr<IRawElementProviderSimple> inline_text_node_raw2 = QueryInterfaceFromNode<IRawElementProviderSimple>(inline_text_node2);
    ComPtr<IRawElementProviderSimple> inline_text_node_raw3 = QueryInterfaceFromNode<IRawElementProviderSimple>(inline_text_node3);
    ComPtr<IRawElementProviderSimple> search_box_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(search_box_node);
    ComPtr<IRawElementProviderSimple> search_text_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(search_text_node);
    ComPtr<IRawElementProviderSimple> pdf_highlight_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(pdf_highlight_node);
    ComPtr<IRawElementProviderSimple> inline_text_node_raw4 = QueryInterfaceFromNode<IRawElementProviderSimple>(inline_text_node4);
    ...
    ComPtr<ITextProvider> text_provider;
    EXPECT_HRESULT_SUCCEEDED(inline_text_node_raw1->GetPatternProvider(UIA_TextPatternId, &text_provider));
    ...
    ComPtr<IRawElementProviderSimple> enclosing_element;
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetEnclosingElement(&enclosing_element));
    EXPECT_EQ(static_text_node_raw1.Get(), enclosing_element.Get());

    EXPECT_HRESULT_SUCCEEDED(inline_text_node_raw2->GetPatternProvider(UIA_TextPatternId, &text_provider));
    ...
    EXPECT_HRESULT_SUCCEEDED(inline_text_node_raw3->GetPatternProvider(UIA_TextPatternId, &text_provider));
    ...
    // search box
    EXPECT_HRESULT_SUCCEEDED(search_text_node_raw->GetPatternProvider(UIA_TextPatternId, &text_provider));

    EXPECT_HRESULT_SUCCEEDED(text_provider->get_DocumentRange(&text_range_provider));
    SetOwner(owner, text_range_provider.Get());

    EXPECT_HRESULT_SUCCEEDED(text_range_provider->ExpandToEnclosingUnit(TextUnit_Character));
    ...
    EXPECT_HRESULT_SUCCEEDED(inline_text_node_raw4->GetPatternProvider(UIA_TextPatternId, &text_provider));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderGetEnclosingElementRichButton)
{
    ...
    update.nodes = { root, button_1, static_text_1, inline_text_1, button_2, heading, static_text_2, inline_text_2 };
    Init(update);
    ...
    AXPlatformNodeWin* owner = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(button_1_node));
    ASSERT_NE(owner, nullptr);

    ComPtr<IRawElementProviderSimple> button_1_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(button_1_node);
    ComPtr<IRawElementProviderSimple> inline_text_1_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(inline_text_1_node);

    ComPtr<IRawElementProviderSimple> static_text_2_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(static_text_2_node);
    ComPtr<IRawElementProviderSimple> inline_text_2_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(inline_text_2_node);
    ...
    ComPtr<ITextProvider> text_provider;
    EXPECT_HRESULT_SUCCEEDED(inline_text_1_node_raw->GetPatternProvider(UIA_TextPatternId, &text_provider));
    ...
    // parent node.
    EXPECT_HRESULT_SUCCEEDED(inline_text_2_node_raw->GetPatternProvider(UIA_TextPatternId, &text_provider));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderMoveEndpointByRange)
{
    ...
    ComPtr<IRawElementProviderSimple> root_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(root_node);
    ComPtr<ITextProvider> document_provider;
    EXPECT_HRESULT_SUCCEEDED(root_node_raw->GetPatternProvider(UIA_TextPatternId, &document_provider));
    ComPtr<ITextRangeProvider> document_text_range_provider;
    ComPtr<AXPlatformNodeTextRangeProviderWin> document_text_range;

    // Text range related to "some text".
    ComPtr<IRawElementProviderSimple> text_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(text_node);
    ComPtr<ITextProvider> text_provider;
    EXPECT_HRESULT_SUCCEEDED(text_node_raw->GetPatternProvider(UIA_TextPatternId, &text_provider));
    ComPtr<ITextRangeProvider> text_range_provider;
    ComPtr<AXPlatformNodeTextRangeProviderWin> text_range;

    // Text range related to "more text".
    ComPtr<IRawElementProviderSimple> more_text_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(more_text_node);
    ComPtr<ITextProvider> more_text_provider;
    EXPECT_HRESULT_SUCCEEDED(more_text_node_raw->GetPatternProvider(UIA_TextPatternId, &more_text_provider));
    ComPtr<ITextRangeProvider> more_text_range_provider;
    ...
    // "some textmore text".
    EXPECT_HRESULT_SUCCEEDED(document_provider->get_DocumentRange(&document_text_range_provider));
    SetOwner(owner, document_text_range_provider.Get());

    EXPECT_HRESULT_SUCCEEDED(
        document_text_range_provider->MoveEndpointByRange(TextPatternRangeEndpoint_Start, document_text_range_provider.Get(), TextPatternRangeEndpoint_End));

    document_text_range_provider->QueryInterface(IID_PPV_ARGS(&document_text_range));
    EXPECT_EQ(*GetStart(document_text_range.Get()), *GetEnd(document_text_range.Get()));
    ...
    EXPECT_HRESULT_SUCCEEDED(more_text_provider->get_DocumentRange(&more_text_range_provider));
    SetOwner(owner, more_text_range_provider.Get());

    EXPECT_HRESULT_SUCCEEDED(
        document_text_range_provider->MoveEndpointByRange(TextPatternRangeEndpoint_Start, more_text_range_provider.Get(), TextPatternRangeEndpoint_Start));

    document_text_range_provider->QueryInterface(IID_PPV_ARGS(&document_text_range));
    more_text_range_provider->QueryInterface(IID_PPV_ARGS(&more_text_range));
    EXPECT_EQ(*GetStart(document_text_range.Get()), *GetStart(more_text_range.Get()));
    ...
    EXPECT_HRESULT_SUCCEEDED(
        document_text_range_provider->MoveEndpointByRange(TextPatternRangeEndpoint_End, text_range_provider.Get(), TextPatternRangeEndpoint_End));
    ...
    EXPECT_HRESULT_SUCCEEDED(
        more_text_range_provider->MoveEndpointByRange(TextPatternRangeEndpoint_End, text_range_provider.Get(), TextPatternRangeEndpoint_Start));
    ...
    EXPECT_HRESULT_SUCCEEDED(
        text_range_provider->MoveEndpointByRange(TextPatternRangeEndpoint_Start, more_text_range_provider.Get(), TextPatternRangeEndpoint_End));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderGetAttributeValue)
{
    AXNodeData text_data;
    text_data.id = 2;
    text_data.role = ax::mojom::Role::kStaticText;
    ...
    text_data.AddIntAttribute(ax::mojom::IntAttribute::kTextStrikethroughStyle, 2);
    text_data.AddIntAttribute(ax::mojom::IntAttribute::kTextUnderlineStyle, 3);
    text_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    ...
    text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerTypes,
        { (int)ax::mojom::MarkerType::kGrammar, (int)ax::mojom::MarkerType::kSpelling, (int)ax::mojom::MarkerType::kHighlight,
            (int)ax::mojom::MarkerType::kHighlight, (int)ax::mojom::MarkerType::kHighlight });
    text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kHighlightTypes,
        { (int)ax::mojom::HighlightType::kNone, (int)ax::mojom::HighlightType::kNone, (int)ax::mojom::HighlightType::kHighlight,
            (int)ax::mojom::HighlightType::kSpellingError, (int)ax::mojom::HighlightType::kGrammarError });
    text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerStarts, { 0, 5, 0, 14, 19 });
    text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerEnds, { 9, 9, 4, 18, 24 });
    ...
    heading_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    ...
    heading_text_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    heading_text_data.AddIntAttribute(ax::mojom::IntAttribute::kColor, 0xFFADC0DEU);
    ...
    heading_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerTypes, { (int)ax::mojom::MarkerType::kSpelling });
    heading_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerStarts, { 5 });
    heading_text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kMarkerEnds, { 9 });
    heading_text_data.SetName("more text");

    AXNodeData mark_data;
    mark_data.id = 5;
    mark_data.role = ax::mojom::Role::kMark;
    mark_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    ...
    mark_text_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    ...
    list_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    ...
    list_item_data.AddIntAttribute(ax::mojom::IntAttribute::kListStyle, static_cast<int>(ax::mojom::ListStyle::kOther));
    list_item_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    ...
    list_item_text_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    list_item_text_data.AddIntAttribute(ax::mojom::IntAttribute::kColor, 0xFFADC0DEU);
    ...
    list_item2_data.AddIntAttribute(ax::mojom::IntAttribute::kListStyle, static_cast<int>(ax::mojom::ListStyle::kDisc));
    list_item2_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    ...
    list_item2_text_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    list_item2_text_data.AddIntAttribute(ax::mojom::IntAttribute::kColor, 0xFFADC0DEU);
    ...
    input_text_data.AddIntAttribute(ax::mojom::IntAttribute::kNameFrom, static_cast<int>(ax::mojom::NameFrom::kPlaceholder));
    input_text_data.AddStringAttribute(ax::mojom::StringAttribute::kPlaceholder, "placeholder2");
    input_text_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    input_text_data.AddIntAttribute(ax::mojom::IntAttribute::kColor, 0xFFADC0DEU);
    input_text_data.AddStringAttribute(ax::mojom::StringAttribute::kHtmlTag, "input");
    input_text_data.AddStringAttribute(ax::mojom::StringAttribute::kInputType, "text");
    ...
    placeholder_text_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    placeholder_text_data.AddIntAttribute(ax::mojom::IntAttribute::kColor, 0xFFADC0DEU);
    ...
    input_text_data2.AddStringAttribute(ax::mojom::StringAttribute::kPlaceholder, "placeholder2");
    input_text_data2.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    input_text_data2.AddIntAttribute(ax::mojom::IntAttribute::kColor, 0xFFADC0DEU);
    input_text_data2.AddStringAttribute(ax::mojom::StringAttribute::kHtmlTag, "input");
    input_text_data2.AddStringAttribute(ax::mojom::StringAttribute::kInputType, "text");
    ...
    placeholder_text_data2.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    placeholder_text_data2.AddIntAttribute(ax::mojom::IntAttribute::kColor, 0xFFADC0DEU);
    placeholder_text_data2.SetName("placeholder2");

    AXNodeData link_data;
    link_data.id = 16;
    link_data.role = ax::mojom::Role::kLink;
    link_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    ...
    link_text_data.AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    ...
    GetTextRangeProviderFromTextNode(heading_text_range_provider, heading_text_node);
    ...
    GetTextRangeProviderFromTextNode(list_item_text_range_provider, list_item_text_node);
    ComPtr<ITextRangeProvider> list_item2_text_range_provider;
    GetTextRangeProviderFromTextNode(list_item2_text_range_provider, list_item2_text_node);

    ComPtr<ITextRangeProvider> placeholder_text_range_provider;
    GetTextRangeProviderFromTextNode(placeholder_text_range_provider, placeholder_text_node);

    ComPtr<ITextRangeProvider> placeholder_text_range_provider2;
    GetTextRangeProviderFromTextNode(placeholder_text_range_provider2, placeholder_text_node2);
    ...
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_BackgroundColorAttributeId, expected_variant);
    ...
    EXPECT_UIA_TEXTATTRIBUTE_EQ(document_range_provider, UIA_BackgroundColorAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(static_cast<int32_t>(BulletStyle::BulletStyle_None));
    EXPECT_UIA_TEXTATTRIBUTE_EQ(list_item_text_range_provider, UIA_BulletStyleAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(static_cast<int32_t>(BulletStyle::BulletStyle_FilledRoundBullet));
    EXPECT_UIA_TEXTATTRIBUTE_EQ(list_item2_text_range_provider, UIA_BulletStyleAttributeId, expected_variant);
    expected_variant.Reset();

    std::wstring font_name = L"sans";
    expected_variant.Set(SysAllocString(font_name.c_str()));
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_FontNameAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(12.0);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_FontSizeAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(300);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_FontWeightAttributeId, expected_variant);
    expected_variant.Reset();

    // SkColor is ARGB, COLORREF is 0BGR
    expected_variant.Set(static_cast<int32_t>(0x00DEC0ADU));
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_ForegroundColorAttributeId, expected_variant);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(document_range_provider, UIA_ForegroundColorAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(false);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_IsHiddenAttributeId, expected_variant);
    expected_variant.Reset();

    EXPECT_UIA_TEXTATTRIBUTE_MIXED(document_range_provider, UIA_IsHiddenAttributeId);

    expected_variant.Set(true);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_IsItalicAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(false);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(heading_text_range_provider, UIA_IsItalicAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(true);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_IsReadOnlyAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(false);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(heading_text_range_provider, UIA_IsReadOnlyAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(false);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(placeholder_text_range_provider, UIA_IsReadOnlyAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(true);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(placeholder_text_range_provider2, UIA_IsReadOnlyAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(true);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(link_text_range_provider, UIA_IsReadOnlyAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(HorizontalTextAlignment_Centered);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_HorizontalTextAlignmentAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(HorizontalTextAlignment_Justified);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(heading_text_range_provider, UIA_HorizontalTextAlignmentAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(true);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_IsSubscriptAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(false);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(heading_text_range_provider, UIA_IsSubscriptAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(false);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_IsSuperscriptAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(true);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(heading_text_range_provider, UIA_IsSuperscriptAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(TextDecorationLineStyle::TextDecorationLineStyle_Dot);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_OverlineStyleAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(TextDecorationLineStyle::TextDecorationLineStyle_Dash);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_StrikethroughStyleAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(TextDecorationLineStyle::TextDecorationLineStyle_Single);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_UnderlineStyleAttributeId, expected_variant);
    expected_variant.Reset();

    std::wstring style_name;
    expected_variant.Set(SysAllocString(style_name.c_str()));
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_StyleNameAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(static_cast<int32_t>(StyleId_Heading6));
    EXPECT_UIA_TEXTATTRIBUTE_EQ(heading_text_range_provider, UIA_StyleIdAttributeId, expected_variant);
    expected_variant.Reset();

    style_name = L"mark";
    expected_variant.Set(SysAllocString(style_name.c_str()));
    EXPECT_UIA_TEXTATTRIBUTE_EQ(mark_text_range_provider, UIA_StyleNameAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(static_cast<int32_t>(StyleId_NumberedList));
    EXPECT_UIA_TEXTATTRIBUTE_EQ(list_item_text_range_provider, UIA_StyleIdAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(static_cast<int32_t>(StyleId_BulletedList));
    EXPECT_UIA_TEXTATTRIBUTE_EQ(list_item2_text_range_provider, UIA_StyleIdAttributeId, expected_variant);
    expected_variant.Reset();

    expected_variant.Set(static_cast<int32_t>(FlowDirections::FlowDirections_RightToLeft));
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider, UIA_TextFlowDirectionsAttributeId, expected_variant);
    EXPECT_UIA_TEXTATTRIBUTE_MIXED(document_range_provider, UIA_TextFlowDirectionsAttributeId);
    expected_variant.Reset();
    ...
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(mark_text_range_provider, TextPatternRangeEndpoint_Start, TextUnit_Character,
        /*count*/ -1,
    ...
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(mark_text_range_provider, TextPatternRangeEndpoint_Start, TextUnit_Character,
        ...
        /*expected_count*/ 1);
    expected_variant.Set(false);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(mark_text_range_provider, UIA_IsSuperscriptAttributeId, expected_variant);
    expected_variant.Reset();

    // Same idea as above, but moving forth and back the end endpoint to force it
    // to be located at the start of the next anchor.
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(mark_text_range_provider, TextPatternRangeEndpoint_End, TextUnit_Character,
        /*count*/ 1,
...

    {
        ...
        EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetAttributeValue(UIA_AnnotationTypesAttributeId, annotation_types_variant.Receive()));

        EXPECT_EQ(annotation_types_variant.type(), VT_ARRAY | VT_I4);
        std::vector<int> expected_annotations = { AnnotationType_SpellingError, AnnotationType_GrammarError, AnnotationType_Highlighted };
        EXPECT_UIA_SAFEARRAY_EQ(V_ARRAY(annotation_types_variant.ptr()), expected_annotations);
    }

    {
        ...
        AXPlatformNodeWin* owner = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(text_node));
        ComPtr<AXPlatformNodeTextRangeProviderWin> range_with_annotations;
        CreateTextRangeProviderWin(range_with_annotations, owner,
        ...
        base::win::ScopedVariant annotation_types_variant;
        EXPECT_HRESULT_SUCCEEDED(range_with_annotations->GetAttributeValue(UIA_AnnotationTypesAttributeId, annotation_types_variant.Receive()));

        EXPECT_EQ(annotation_types_variant.type(), VT_ARRAY | VT_I4);
        std::vector<int> expected_annotations = { AnnotationType_SpellingError, AnnotationType_GrammarError };
        EXPECT_UIA_SAFEARRAY_EQ(V_ARRAY(annotation_types_variant.ptr()), expected_annotations);
    }

    {
        ...
        EXPECT_EQ(annotation_types_variant.type(), VT_ARRAY | VT_I4);
        std::vector<int> expected_annotations = { AnnotationType_GrammarError, AnnotationType_Highlighted };
        EXPECT_UIA_SAFEARRAY_EQ(V_ARRAY(annotation_types_variant.ptr()), expected_annotations);
    }
...

    {
        ...
        base::win::ScopedVariant annotation_types_variant;
        EXPECT_HRESULT_SUCCEEDED(heading_text_range_provider->GetAttributeValue(UIA_AnnotationTypesAttributeId, annotation_types_variant.Receive()));
...

    {
        ...
        AXPlatformNodeWin* owner = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(heading_text_node));
        ComPtr<AXPlatformNodeTextRangeProviderWin> range_with_annotations;
        CreateTextRangeProviderWin(range_with_annotations, owner,
...

    {
        ...
        EXPECT_UIA_TEXTATTRIBUTE_EQ(mark_text_range_provider, UIA_AnnotationTypesAttributeId, empty_variant);
    }
}

TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderGetAttributeValueAnnotationObjects)
{
    // rootWebArea id=1
    ...
    root.role = ax::mojom::Role::kRootWebArea;
    root.SetName("root");
    root.child_ids = { annotation_target.id, comment1.id, comment2.id, highlighted.id };

    annotation_target.role = ax::mojom::Role::kMark;
    annotation_target.child_ids = { some_text.id };
    annotation_target.AddIntListAttribute(ax::mojom::IntListAttribute::kDetailsIds, { comment1.id, comment2.id, highlighted.id });
    ...
    update.has_tree_data = true;
    update.root_id = root.id;
    update.nodes = { root, annotation_target, some_text, comment1, comment1_text, comment2, comment2_text, highlighted, highlighted_text };
    update.tree_data.tree_id = AXTreeID::CreateNewAXTreeID();
    ...
    AXPlatformNodeWin* owner = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(annotation_target_node));
    CreateTextRangeProviderWin(some_text_range_provider, owner,
    ...
    ComPtr<IRawElementProviderSimple> comment1_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(comment1_node);
    ASSERT_NE(nullptr, comment1_provider.Get());
    ComPtr<IRawElementProviderSimple> comment2_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(comment2_node);
    ASSERT_NE(nullptr, comment2_provider.Get());
    ComPtr<IRawElementProviderSimple> highlighted_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(highlighted_node);
    ...
    EXPECT_HRESULT_SUCCEEDED(comment1_provider->GetPatternProvider(UIA_AnnotationPatternId, &annotation_provider));
    ...
    EXPECT_HRESULT_SUCCEEDED(comment2_provider->GetPatternProvider(UIA_AnnotationPatternId, &annotation_provider));
    ASSERT_NE(nullptr, annotation_provider.Get());
    EXPECT_HRESULT_SUCCEEDED(annotation_provider->get_AnnotationTypeId(&annotation_type));
    EXPECT_EQ(AnnotationType_Comment, annotation_type);
    annotation_provider.Reset();
    ...
    // IAnnotationProvider.
    EXPECT_HRESULT_SUCCEEDED(highlighted_provider->GetPatternProvider(UIA_AnnotationPatternId, &annotation_provider));
    ASSERT_NE(nullptr, annotation_provider.Get());
    EXPECT_HRESULT_SUCCEEDED(annotation_provider->get_AnnotationTypeId(&annotation_type));
    ...
    EXPECT_HRESULT_SUCCEEDED(some_text_range_provider->GetAttributeValue(UIA_AnnotationObjectsAttributeId, annotation_objects_variant.Receive()));
    EXPECT_EQ(VT_UNKNOWN | VT_ARRAY, annotation_objects_variant.type());

    std::vector<std::wstring> expected_names = { L"comment 1", L"comment 2", L"highlighted" };
    EXPECT_UIA_ELEMENT_ARRAY_BSTR_EQ(V_ARRAY(annotation_objects_variant.ptr()), UIA_NamePropertyId, expected_names);
}

TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderGetAttributeValueAnnotationObjectsMixed)
{
    // rootWebArea id=1
    ...
    highlighted.AddIntListAttribute(ax::mojom::IntListAttribute::kDetailsIds, { comment1.id });
    ...
    update.has_tree_data = true;
    update.root_id = root.id;
    update.nodes = { root, highlighted, some_text, readonly_text, comment1, comment1_text };
    update.tree_data.tree_id = AXTreeID::CreateNewAXTreeID();
    ...
    // end  : TextPosition, anchor_id=2, text_offset=9, annotated_text=some text<>
    ComPtr<AXPlatformNodeTextRangeProviderWin> some_text_range_provider;
    AXPlatformNodeWin* owner = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(highlighted_node));
    CreateTextRangeProviderWin(some_text_range_provider, owner,
    ...
    GetTextRangeProviderFromTextNode(readonly_text_range_provider, readonly_text_node);
    ...
    EXPECT_HRESULT_SUCCEEDED(some_text_range_provider->GetAttributeValue(UIA_AnnotationObjectsAttributeId, expected_variant.Receive()));
    EXPECT_EQ(VT_UNKNOWN | VT_ARRAY, expected_variant.type());

    std::vector<std::wstring> expected_names = { L"comment 1" };
    EXPECT_UIA_ELEMENT_ARRAY_BSTR_EQ(V_ARRAY(expected_variant.ptr()), UIA_NamePropertyId, expected_names);
    expected_variant.Reset();
    ...
    expected_variant.Set(true);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(readonly_text_range_provider, UIA_IsReadOnlyAttributeId, expected_variant);
    ...
    CreateTextRangeProviderWin(mixed_text_range_provider, owner,
    ...
    EXPECT_UIA_TEXTATTRIBUTE_MIXED(mixed_text_range_provider, UIA_AnnotationObjectsAttributeId);
}
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderGetAttributeValueNotSupported)
{
    ...
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_AfterParagraphSpacingAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_AnimationStyleAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_BeforeParagraphSpacingAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_CapStyleAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_CaretBidiModeAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_CaretPositionAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_IndentationFirstLineAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_IndentationLeadingAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_IndentationTrailingAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_IsActiveAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_LineSpacingAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_LinkAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_MarginBottomAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_MarginLeadingAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_MarginTopAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_MarginTrailingAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_OutlineStylesAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_OverlineColorAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_SelectionActiveEndAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_StrikethroughColorAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_TabsAttributeId);
    EXPECT_UIA_TEXTATTRIBUTE_NOTSUPPORTED(document_range_provider, UIA_UnderlineColorAttributeId);
}
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderGetAttributeValueWithAncestorTextPosition)
{
    ...
    initial_state.nodes[3].AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    initial_state.nodes[4].id = 5;
    initial_state.nodes[4].role = ax::mojom::Role::kStaticText;
    initial_state.nodes[4].SetName("more text");
    initial_state.nodes[4].AddIntAttribute(ax::mojom::IntAttribute::kBackgroundColor, 0xFFADBEEFU);
    ...
    AXPlatformNodeWin* owner = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(tree->GetFromId(2)));
    ...
    CreateTextRangeProviderWin(text_range_provider_win, owner,
        /*start_anchor=*/some_text_node, /*start_offset=*/0,
        /*start_affinity*/ ax::mojom::TextAffinity::kDownstream,
    ...
    // SkColor is ARGB, COLORREF is 0BGR
    expected_variant.Set(static_cast<int32_t>(0x00EFBEADU));
    EXPECT_UIA_TEXTATTRIBUTE_EQ(text_range_provider_win, UIA_BackgroundColorAttributeId, expected_variant);
}
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderSelect)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(document_provider->get_DocumentRange(&document_text_range_provider));
    document_text_range_provider->QueryInterface(IID_PPV_ARGS(&document_text_range));
    AXPlatformNodeWin* owner_platform = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(root_node));
    ...
    GetTextRangeProviderFromTextNode(text_range_provider, root_node->children()[0]);
    ComPtr<AXPlatformNodeTextRangeProviderWin> text_range;
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->QueryInterface(IID_PPV_ARGS(&text_range)));
    ...
    AXPlatformNodeDelegate* delegate = GetOwner(document_text_range.Get())->GetDelegate();
    ...
    // Document text range "some textmore text2" performs select.
    {
        ...
        EXPECT_HRESULT_SUCCEEDED(SafeArrayGetElement(selection.Get(), &index, static_cast<void**>(&selected_text_range_provider)));
        SetOwner(owner_platform, selected_text_range_provider.Get());
        EXPECT_UIA_TEXTRANGE_EQ(selected_text_range_provider, L"some textmore text2");
    }

    // A degenerate text range performs select.
    {
        // Move the endpoint of text range so it becomes degenerate, then select.
        text_range_provider->MoveEndpointByRange(TextPatternRangeEndpoint_Start, text_range_provider.Get(), TextPatternRangeEndpoint_End);
        text_range_provider->Select();
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestSettingSelectionAcrossShadowDOM)
{
    ...
    AXPlatformNodeWin* owner = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(text_field));
    ComPtr<AXPlatformNodeTextRangeProviderWin> range;
    CreateTextRangeProviderWin(range, owner,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestScrollIntoViewOnOnscreenElement)
{
    TestAXTreeUpdate update(std::string(R"HTML(
    ++1 kRootWebArea
    ++++2 kGenericContainer
    ...
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->ScrollIntoView(/* align_to_top */ true));
    base::win::ScopedSafearray rectangles;
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderSelectListMarker)
{
    ...
    update.has_tree_data = true;
    update.root_id = root_data.id;
    update.nodes = { root_data, list_data, list_item_data, list_marker, static_text_data, list_item_text_data };
    Init(update);
    AXNode* root_node = GetRoot();
    ...
    GetTextRangeProviderFromTextNode(list_marker_text_range_provider, list_marker_node->children()[0]);
    ...
    ComPtr<ITextProvider> document_provider;
    EXPECT_HRESULT_SUCCEEDED(root_node_raw->GetPatternProvider(UIA_TextPatternId, &document_provider));
    EXPECT_HRESULT_SUCCEEDED(document_provider->GetSelection(selection.Receive()));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderFindText)
{
    ...
    Microsoft::WRL::ComPtr<ITextRangeProvider> text_range_provider_found;
    EXPECT_HRESULT_SUCCEEDED(range->FindText(find_string.Get(), false, false, &text_range_provider_found));
    Microsoft::WRL::ComPtr<AXPlatformNodeTextRangeProviderWin> text_range_provider_win;
    text_range_provider_found->QueryInterface(IID_PPV_ARGS(&text_range_provider_win));
    ASSERT_TRUE(GetStart(text_range_provider_win.Get())->IsTextPosition());
...
TEST_F(AXPlatformNodeTextRangeProviderTest, FindTextWithEmbeddedObjectCharacter)
{
    ...
    Microsoft::WRL::ComPtr<ITextRangeProvider> text_range_provider_found;
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->FindText(find_string.Get(), false, false, &text_range_provider_found));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestTextRangeProviderWinUnfocusableNodeForSelection)
{
    TestAXTreeUpdate update(std::string(R"HTML(
    ++1 kRootWebArea
    ...
    AXPlatformNodeWin* input_platform_node = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(input_node));

    AXPlatformNodeWin* root = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(tree->GetFromId(1)));
    ...
    CreateTextRangeProviderWin(text_range_provider, input_platform_node,
    ...
    text_range_provider->Select();
    // Verify selection.
    AXSelection unignored_selection = root->GetDelegate()->GetUnignoredSelection();
    EXPECT_EQ(5, unignored_selection.anchor_object_id);
    EXPECT_EQ(5, unignored_selection.focus_object_id);
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestTextRangeProviderWinFindTextInContentEditable)
{
    ...
    generic_container_2.AddBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot, true);
    ...
    update.nodes = { root_1, generic_container_2, paragraph_3, static_text_4, inline_box_5 };

    AXTree* tree = Init(update);
    AXNode* div_node = tree->GetFromId(generic_container_2.id);

    AXPlatformNodeWin* owner = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(div_node));
    ...
    ComPtr<AXPlatformNodeTextRangeProviderWin> text_range_provider;
    {
        ScopedAXEmbeddedObjectBehaviorSetter ax_embedded_object_behavior(AXEmbeddedObjectBehavior::kSuppressCharacter);
        CreateTextRangeProviderWin(text_range_provider, owner,
    ...
    Microsoft::WRL::ComPtr<ITextRangeProvider> text_range_provider_found;
    text_range_provider->FindText(find_string.Get(), false, false, &text_range_provider_found);
    ASSERT_TRUE(text_range_provider_found.Get());
}

TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderFindTextBackwards)
{
    Init(BuildTextDocument({ "text", "some", "text" }, false /* build_word_boundaries_offsets */, true /* place_text_on_one_line */));
    AXNode* root_node = GetRoot();
    ...
    EXPECT_HRESULT_SUCCEEDED(root_range_provider->FindText(find_string.Get(), false, false, &text_range_provider_found));
    ...
    range_equal = false;
    EXPECT_HRESULT_SUCCEEDED(text_range_provider_found->Compare(text_node1_range.Get(), &range_equal));
    EXPECT_TRUE(range_equal);

    // Backwards search finds the text_node3.
    EXPECT_HRESULT_SUCCEEDED(root_range_provider->FindText(find_string.Get(), true, false, &text_range_provider_found));
    CopyOwnerToClone(root_range_provider.Get(), text_range_provider_found.Get());
    EXPECT_UIA_TEXTRANGE_EQ(text_range_provider_found, find_string.Get());

    range_equal = false;
    EXPECT_HRESULT_SUCCEEDED(text_range_provider_found->Compare(text_node3_range.Get(), &range_equal));
    EXPECT_TRUE(range_equal);
}
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderFindAttribute)
{
    ...
    //       our searching criteria. And we return a degenerate range.
    {
        ...
        document_range_provider->FindAttribute(UIA_IsHiddenAttributeId, is_hidden_attr_val, is_search_backward, &matched_range_provider);
        ...
        CopyOwnerToClone(document_range_provider.Get(), matched_range_provider.Get());
    ...
    // Expected: "text4"
    {
        ...
        update.has_tree_data = true;
        update.root_id = root_data.id;
        update.nodes = { root_data, text_data1, text_data2, text_data3, text_data4, text_data5 };
...
TEST_F(AXPlatformNodeTextRangeProviderTest, ElementNotAvailable)
{
    ...
    ComPtr<IRawElementProviderSimple> raw_element_provider_simple = QueryInterfaceFromNode<IRawElementProviderSimple>(GetRoot());
    ASSERT_NE(nullptr, raw_element_provider_simple.Get());

    ComPtr<ITextProvider> text_provider;
    ASSERT_HRESULT_SUCCEEDED(raw_element_provider_simple->GetPatternProvider(UIA_TextPatternId, &text_provider));
    ASSERT_NE(nullptr, text_provider.Get());

    ComPtr<ITextRangeProvider> text_range_provider;
    ASSERT_HRESULT_SUCCEEDED(text_provider->get_DocumentRange(&text_range_provider));
    ASSERT_NE(nullptr, text_range_provider.Get());
    ...
    BOOL bool_arg = FALSE;
    ASSERT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), text_range_provider->ScrollIntoView(bool_arg));
}

TEST_F(AXPlatformNodeTextRangeProviderTest, TestITextRangeProviderIgnoredNodes)
{
    TestAXTreeUpdate update(std::string(R"HTML(
    ++1 kRootWebArea
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestNormalizeTextRangePastEndOfDocument)
{
    ...
    const AXNodePosition::AXPositionInstance start_after_move = GetStart(text_range_provider_win.Get())->Clone();
    const AXNodePosition::AXPositionInstance end_after_move = GetEnd(text_range_provider_win.Get())->Clone();
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestNormalizeTextRangeInsideIgnoredNodes)
{
    ...
    AXPlatformNodeWin* owner = static_cast<AXPlatformNodeWin*>(AXPlatformNodeFromNode(tree->GetFromId(1)));
    ...
    CreateTextRangeProviderWin(ignored_range_win, owner,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestNormalizeTextRangeSpanIgnoredNodes)
{
    ...
    CreateTextRangeProviderWin(range_span_ignored_nodes, owner,
    ...
    // |before||ignored1||ignored2||<a>fter|
    //                              |-|
    AXNodePosition::AXPositionInstance normalized_start = GetStart(range_span_ignored_nodes.Get())->Clone();
    AXNodePosition::AXPositionInstance normalized_end = GetEnd(range_span_ignored_nodes.Get())->Clone();
    NormalizeTextRange(range_span_ignored_nodes.Get(), normalized_start, normalized_end);
    // Verify that the original range was not changed by normalization.
    ExpectPositionsEqual(original_start, GetStart(range_span_ignored_nodes.Get()));
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestNormalizeTextRangeForceSameAnchorOnDegenerateRange)
{
    TestAXTreeUpdate update(std::string(R"HTML(
    ++1 kRootWebArea
    ...
    AXNodePosition::AXPositionInstance normalized_start = GetStart(range.Get())->Clone();
    AXNodePosition::AXPositionInstance normalized_end = GetEnd(range.Get())->Clone();
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestReplaceStartAndEndEndpointNode)
{
    ...
    // 3. Replace the node on which |start_| and |end_| is.
    {
        ...
        CreateTextRangeProviderWin(range_2, owner,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestDeleteSubtreeWithIgnoredAncestors)
{
    ...
    update.tree_data.tree_id = tree_id;
    update.has_tree_data = true;
    update.nodes = { root_1, text_2, gc_3, gc_4, gc_5, text_6, gc_7, text_8, gc_9, text_10, gc_11, text_12 };
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestDeleteSubtreeThatIncludesEndpointsNormalizeMoves)
{
    // This test updates the tree structure to ensure that the text range is still
    // valid after a subtree that includes the text range is deleted, resulting in
...
TEST_F(AXPlatformNodeTextRangeProviderTest, TestDeleteTreePositionPreviousSibling)
{
    ...
    generic_container_2.role = ax::mojom::Role::kGenericContainer;
    generic_container_2.child_ids = { heading_3.id, generic_container_6.id, button_7.id };
    ...
    generic_container_6.AddBoolAttribute(ax::mojom::BoolAttribute::kIsLineBreakingObject, true);
    ...
    update.has_tree_data = true;
    update.root_id = root_1.id;
    update.nodes = { root_1, generic_container_2, heading_3, static_text_4, inline_box_5, generic_container_6, button_7 };

    AXTree* tree = Init(update);

    AXNode* root_node = GetRoot();
    AXNodePosition::AXPositionInstance range_start = CreateTreePosition(generic_container_2,
        /*child_index*/ 3);
...
TEST_F(AXPlatformNodeTextRangeProviderTest, CaretAtEndOfTextFieldReadOnly)
{
    ...
    expected_variant.Set(false);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(range, UIA_IsReadOnlyAttributeId, expected_variant);

    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(range, TextPatternRangeEndpoint_Start, TextUnit_Character,
        /*count*/ 1,
        /*expected_text*/ L"",
        /*expected_count*/ 1);
    expected_variant.Set(false);
...
TEST_F(AXPlatformNodeTextRangeProviderTest, GeneratedNewlineReturnsCommonAnchorReadonly)
{
    ...
    CreateTextRangeProviderWin(range_1, owner,
    ...
    expected_variant.Set(true);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(range_1, UIA_IsReadOnlyAttributeId, expected_variant);
    expected_variant.Reset();
    ...
    expected_variant.Set(false);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(range_2, UIA_IsReadOnlyAttributeId, expected_variant);
    expected_variant.Reset();
    ...
    CreateTextRangeProviderWin(range_3, owner,
    ...
    expected_variant.Set(true);
    EXPECT_UIA_TEXTATTRIBUTE_EQ(range_3, UIA_IsReadOnlyAttributeId, expected_variant);
    expected_variant.Reset();
}

TEST_F(AXPlatformNodeTextRangeProviderTest, MoveEndpointToLastIgnoredForTextNavigationNode)
{
    ...
    EXPECT_UIA_MOVE_ENDPOINT_BY_UNIT(range, TextPatternRangeEndpoint_End, TextUnit_Paragraph,
        /*count*/ 1,
        /*expected_text*/ L"abc\n\xFFFC",
        /*expected_count*/ 1);
}

TEST_F(AXPlatformNodeTextRangeProviderTest, OnTextDeletionOrInsertionDeletionOnTwoRelevantNodes)
{
    ...
    CreateTextRangeProviderWin(original, owner,
        /*start_anchor*/ st_node, /*start_offset*/ 14,
        /*start_affinity*/ ax::mojom::TextAffinity::kDownstream,
        /*end_anchor*/ st_node, /*end_offset*/ 14,
        /*end_affinity*/ ax::mojom::TextAffinity::kDownstream);

    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartOffsets, std::vector<int> { 0 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndOffsets, std::vector<int> { 5 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartAnchorIds, std::vector<int> { 1 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndAnchorIds, std::vector<int> { 1 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperations, std::vector<int> { static_cast<int>(ax::mojom::Command::kDelete) });
    ...
    CreateTextRangeProviderWin(after_deletion_expected, owner,
...
TEST_F(AXPlatformNodeTextRangeProviderTest, OnTextDeletionOrInsertionDeletionOnTwoIrrelevantNodes)
{
    // This test is for `OnTextDeletionOrInsertion` which is called when
    // unserializing. This test covers the following scenario of text deletion:
    ...
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartOffsets, std::vector<int> { 0 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndOffsets, std::vector<int> { 5 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartAnchorIds, std::vector<int> { 3 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndAnchorIds, std::vector<int> { 3 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperations, std::vector<int> { static_cast<int>(ax::mojom::Command::kDelete) });
...
TEST_F(AXPlatformNodeTextRangeProviderTest, OnTextDeletionOrInsertionDeletionOnOneIrrelevantOneRelevant)
{
    // This test is for `OnTextDeletionOrInsertion` which is called when
    // unserializing. This test covers the following scenario of text deletion:
    ...
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartOffsets, std::vector<int> { 6 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndOffsets, std::vector<int> { 3 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartAnchorIds, std::vector<int> { 3 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndAnchorIds, std::vector<int> { 5 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperations, std::vector<int> { static_cast<int>(ax::mojom::Command::kDelete) });
...
TEST_F(AXPlatformNodeTextRangeProviderTest, OnTextDeletionOrInsertionDeletionInsideTextRangeProvider)
{
    // This test is for `OnTextDeletionOrInsertion` which is called when
    // unserializing. This test covers the following scenario of text deletion:
    // <div contenteditable><span>hello world</span> red green blue</div>
...
TEST_F(AXPlatformNodeTextRangeProviderTest, OnTextDeletionOrInsertionDeletionMultipleDeletionsAndInsertions)
{
    // This test is for `OnTextDeletionOrInsertion` which is called when
    // unserializing. This test covers the following scenario of text deletion:
    ...
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartOffsets, std::vector<int> { 20, 19, 18, 17, 16, 16, 17 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndOffsets, std::vector<int> { 21, 20, 19, 18, 17, 16, 17 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationStartAnchorIds, std::vector<int> { 3, 3, 3, 3, 3, 3, 3 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperationEndAnchorIds, std::vector<int> { 3, 3, 3, 3, 3, 3, 3 });
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kTextOperations,
        std::vector<int> { static_cast<int>(ax::mojom::Command::kDelete), static_cast<int>(ax::mojom::Command::kDelete),
            static_cast<int>(ax::mojom::Command::kDelete), static_cast<int>(ax::mojom::Command::kDelete), static_cast<int>(ax::mojom::Command::kDelete),
            static_cast<int>(ax::mojom::Command::kInsert), static_cast<int>(ax::mojom::Command::kInsert) });
