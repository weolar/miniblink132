...
#define DCHECK_IN_ON_ACCESSIBILITY_EVENTS()                                                                                                                    \
    DCHECK(in_on_accessibility_events_) << "This method should only be called during OnAccessibilityEvents "                                                   \
...
{
    for (BrowserAccessibility* current_node = &node; current_node; current_node = current_node->PlatformGetParent()) {
        if (ToBrowserAccessibilityWin(current_node)->GetCOM()->IsPatternProviderSupported(UIA_TextPatternId)) {
            return current_node;
        }
    }
...
{
    return new BrowserAccessibilityManagerWin(initial_tree, node_id_delegate, delegate);
}
...
{
    return new BrowserAccessibilityManagerWin(BrowserAccessibilityManagerWin::GetEmptyDocument(), node_id_delegate, delegate);
}
...
BrowserAccessibilityManagerWin::~BrowserAccessibilityManagerWin()
{
    ...
    BrowserAccessibility* parent = GetParentNodeFromParentTreeAsBrowserAccessibility();
    if (!parent) {
        return;
    }
    ...
    static_cast<AXPlatformNodeWin*>(parent_ax_platform_node)->ResetComputedHypertext();
}
...
void BrowserAccessibilityManagerWin::UserIsReloading()
{
    if (GetBrowserAccessibilityRoot())
        FireWinAccessibilityEvent(IA2_EVENT_DOCUMENT_RELOAD, GetBrowserAccessibilityRoot());
}

void BrowserAccessibilityManagerWin::FireAriaNotificationEvent(BrowserAccessibility* node, const std::string& announcement, const std::string& notification_id,
    ax::mojom::AriaNotificationInterrupt interrupt_property, ax::mojom::AriaNotificationPriority priority_property)
{
    DCHECK(node);

    // This API is only supported from Windows10 (version 1709) onwards.
    // Check if the function pointer is valid or not.
    using UiaRaiseNotificationEventFunction = HRESULT(WINAPI*)(IRawElementProviderSimple*, NotificationKind, NotificationProcessing, BSTR, BSTR);
    static const UiaRaiseNotificationEventFunction uia_raise_notification_event_func
        = reinterpret_cast<UiaRaiseNotificationEventFunction>(::GetProcAddress(GetModuleHandle(L"uiautomationcore.dll"), "UiaRaiseNotificationEvent"));
    if (!uia_raise_notification_event_func) {
        return;
    }

    auto MapPropertiesToUiaNotificationProcessing = [&]() -> NotificationProcessing {
    ...
    const base::win::ScopedBstr notification_id_bstr(base::UTF8ToWide(notification_id));

    uia_raise_notification_event_func(ToBrowserAccessibilityWin(node)->GetCOM(), NotificationKind_ActionCompleted, MapPropertiesToUiaNotificationProcessing(),
        announcement_bstr.Get(), notification_id_bstr.Get());
}
...
void BrowserAccessibilityManagerWin::FireBlinkEvent(ax::mojom::Event event_type, BrowserAccessibility* node, int action_request_id)
{
    DCHECK(CanFireEvents());

    BrowserAccessibilityManager::FireBlinkEvent(event_type, node, action_request_id);
    ...
    case ax::mojom::Event::kEndOfTest:
        ...
        FireUiaAccessibilityEvent(UiaRegistrarWin::GetInstance().GetTestCompleteEventId(), node);
        break;
...
void BrowserAccessibilityManagerWin::FireGeneratedEvent(AXEventGenerator::Event event_type, const AXNode* node)
{
    ...
    case AXEventGenerator::Event::CHECKED_STATE_CHANGED:
        ...
        if (IsRadio(wrapper->GetRole())) {
            HandleSelectedStateChanged(uia_selection_events_, wrapper, IsUIANodeSelected(wrapper));
        }
    ...
    case AXEventGenerator::Event::CHILDREN_CHANGED: {
        ...
        BrowserAccessibility* target_node = wrapper->IsIgnored() ? wrapper->PlatformGetParent() : wrapper;
        if (target_node) {
            FireWinAccessibilityEvent(EVENT_OBJECT_REORDER, target_node);
            FireUiaStructureChangedEvent(StructureChangeType_ChildrenReordered, target_node);
        }
        break;
    }
    case AXEventGenerator::Event::COLLAPSED:
    case AXEventGenerator::Event::EXPANDED:
        FireUiaPropertyChangedEvent(UIA_ExpandCollapseExpandCollapseStatePropertyId, wrapper);
        HandleAriaPropertiesChangedEvent(*wrapper);
        break;
    ...
    case AXEventGenerator::Event::DOCUMENT_SELECTION_CHANGED: {
        ...
        if (focus_object) {
            ...
            if (BrowserAccessibility* text_field = focus_object->PlatformGetTextFieldAncestor()) {
    ...
    case AXEventGenerator::Event::MULTISELECTABLE_STATE_CHANGED:
        FireUiaPropertyChangedEvent(UIA_SelectionCanSelectMultiplePropertyId, wrapper);
        HandleAriaPropertiesChangedEvent(*wrapper);
        break;
    ...
    case AXEventGenerator::Event::READONLY_CHANGED:
        if (wrapper->GetData().IsRangeValueSupported())
            FireUiaPropertyChangedEvent(UIA_RangeValueIsReadOnlyPropertyId, wrapper);
    ...
    case AXEventGenerator::Event::SCROLL_HORIZONTAL_POSITION_CHANGED:
        ...
        FireUiaPropertyChangedEvent(UIA_ScrollHorizontalScrollPercentPropertyId, wrapper);
        break;
    case AXEventGenerator::Event::SCROLL_VERTICAL_POSITION_CHANGED:
        FireWinAccessibilityEvent(EVENT_SYSTEM_SCROLLINGEND, wrapper);
        FireUiaPropertyChangedEvent(UIA_ScrollVerticalScrollPercentPropertyId, wrapper);
        break;
    case AXEventGenerator::Event::SELECTED_CHANGED:
        HandleSelectedStateChanged(ia2_selection_events_, wrapper, IsIA2NodeSelected(wrapper));
        HandleSelectedStateChanged(uia_selection_events_, wrapper, IsUIANodeSelected(wrapper));
        HandleAriaPropertiesChangedEvent(*wrapper);
        break;
...
void BrowserAccessibilityManagerWin::FireWinAccessibilityEvent(LONG win_event_type, BrowserAccessibility* node)
{
    ...
    TRACE_EVENT("accessibility", "NotifyWinEvent", perfetto::Flow::FromPointer(com), "win_event_type", base::StringPrintf("0x%04lX", win_event_type));
    ::NotifyWinEvent(win_event_type, hwnd, OBJID_CLIENT, -(com->GetUniqueId()));
}

bool BrowserAccessibilityManagerWin::IsIgnoredChangedNode(const BrowserAccessibility* node) const
{
    return base::Contains(ignored_changed_nodes_, const_cast<BrowserAccessibility*>(node));
}

void BrowserAccessibilityManagerWin::FireUiaAccessibilityEvent(LONG uia_event, BrowserAccessibility* node)
{
    ...
    ::UiaRaiseAutomationEvent(ToBrowserAccessibilityWin(node)->GetCOM(), uia_event);
}

void BrowserAccessibilityManagerWin::FireUiaPropertyChangedEvent(LONG uia_property, BrowserAccessibility* node)
{
    if (!AXPlatform::GetInstance().IsUiaProviderEnabled()) {
        return;
    }
    ...
    if (SUCCEEDED(provider->GetPropertyValueImpl(uia_property, new_value.Receive()))) {
        ::UiaRaiseAutomationPropertyChangedEvent(provider, uia_property, old_value, new_value);
    }
}

void BrowserAccessibilityManagerWin::FireUiaStructureChangedEvent(StructureChangeType change_type, BrowserAccessibility* node)
{
    if (!AXPlatform::GetInstance().IsUiaProviderEnabled()) {
        return;
    }
    ...
    case StructureChangeType_ChildRemoved: {
        ...
        if (parent && parent_com) {
            ...
            UiaRaiseStructureChangedEvent(parent_com, change_type, runtime_id.data(), runtime_id.size());
        }
        break;
    }
...
// static
bool BrowserAccessibilityManagerWin::IsUiaActiveTextPositionChangedEventSupported()
{
    return GetUiaActiveTextPositionChangedEventFunction();
}

// static
UiaRaiseActiveTextPositionChangedEventFunction BrowserAccessibilityManagerWin::GetUiaActiveTextPositionChangedEventFunction()
{
    // MSDN
    ...
    return reinterpret_cast<UiaRaiseActiveTextPositionChangedEventFunction>(
        ::GetProcAddress(::GetModuleHandle(L"uiautomationcore.dll"), "UiaRaiseActiveTextPositionChangedEvent"));
}

void BrowserAccessibilityManagerWin::FireUiaActiveTextPositionChangedEvent(BrowserAccessibility* node)
{
    if (!ShouldFireEventForNode(node))
        return;

    UiaRaiseActiveTextPositionChangedEventFunction active_text_position_changed_func = GetUiaActiveTextPositionChangedEventFunction();
    ...
    AXPlatformNodeTextProviderWin::CreateDegenerateRangeAtStart(target_node, &text_range);
...
{
    return BrowserAccessibilityManager::CanFireEvents() && GetDelegateFromRootManager() && GetDelegateFromRootManager()->AccessibilityGetAcceleratedWidget();
}

void BrowserAccessibilityManagerWin::OnSubtreeWillBeDeleted(AXTree* tree, AXNode* node)
{
...
void BrowserAccessibilityManagerWin::OnAtomicUpdateFinished(AXTree* tree, bool root_changed, const std::vector<AXTreeObserver::Change>& changes)
{
    BrowserAccessibilityManager::OnAtomicUpdateFinished(tree, root_changed, changes);
    ...
    for (auto* node : objs_to_update) {
        static_cast<BrowserAccessibilityComWin*>(node)->UpdateStep1ComputeWinAttributes();
    }
    ...
    for (auto* node : objs_to_update) {
        static_cast<BrowserAccessibilityComWin*>(node)->UpdateStep2ComputeHypertext();
    }
...
// static
bool BrowserAccessibilityManagerWin::IsIA2NodeSelected(BrowserAccessibility* node)
{
    return node->IsIA2NodeSelected();
}

// static
bool BrowserAccessibilityManagerWin::IsUIANodeSelected(BrowserAccessibility* node)
{
    return node->IsUIANodeSelected();
}

void BrowserAccessibilityManagerWin::FireIA2SelectionEvents(
    BrowserAccessibility* container, BrowserAccessibility* only_selected_child, const SelectionEvents& changes)
{
    if (only_selected_child) {
        ...
        const bool container_is_multiselectable = container && container->HasState(ax::mojom::State::kMultiselectable);
    ...
    BrowserAccessibility* container, BrowserAccessibility* only_selected_child, const SelectionEvents& changes)
{
    if (only_selected_child) {
        // Fire 'ElementSelected' on the only selected child.
        FireUiaAccessibilityEvent(UIA_SelectionItem_ElementSelectedEventId, only_selected_child);
        FireUiaPropertyChangedEvent(UIA_SelectionItemIsSelectedPropertyId, only_selected_child);
        for (BrowserAccessibility* item : changes.removed) {
        ...
        if ((changes.added.size() + changes.removed.size()) > kInvalidateLimit) {
            ...
            for (BrowserAccessibility* item : changes.added) {
                if (container_is_multiselectable) {
                    FireUiaAccessibilityEvent(UIA_SelectionItem_ElementAddedToSelectionEventId, item);
                } else {
                    FireUiaAccessibilityEvent(UIA_SelectionItem_ElementSelectedEventId, item);
                }
                FireUiaPropertyChangedEvent(UIA_SelectionItemIsSelectedPropertyId, item);
            }
            for (BrowserAccessibility* item : changes.removed) {
                FireUiaAccessibilityEvent(UIA_SelectionItem_ElementRemovedFromSelectionEventId, item);
                FireUiaPropertyChangedEvent(UIA_SelectionItemIsSelectedPropertyId, item);
            }
        }
    }
...
// static
void BrowserAccessibilityManagerWin::HandleSelectedStateChanged(SelectionEventsMap& selection_events_map, BrowserAccessibility* node, bool is_selected)
{
...
void BrowserAccessibilityManagerWin::FinalizeSelectionEvents(
    SelectionEventsMap& selection_events_map, IsSelectedPredicate is_selected_predicate, FirePlatformSelectionEventsCallback fire_platform_events_callback)
{
    for (auto&& selected : selection_events_map) {
        ...
        if (IsContainerWithSelectableChildren(key_node->GetRole())) {
            container = key_node;
            for (auto it = container->InternalChildrenBegin(); it != container->InternalChildrenEnd(); ++it) {
                auto* child = it.get();
...
void BrowserAccessibilityManagerWin::HandleAriaPropertiesChangedEvent(BrowserAccessibility& node)
{
    DCHECK_IN_ON_ACCESSIBILITY_EVENTS();
    aria_properties_events_.insert(&node);
}

void BrowserAccessibilityManagerWin::EnqueueTextChangedEvent(BrowserAccessibility& node)
{
    DCHECK_IN_ON_ACCESSIBILITY_EVENTS();
...
void BrowserAccessibilityManagerWin::EnqueueSelectionChangedEvent(BrowserAccessibility& node)
{
    DCHECK_IN_ON_ACCESSIBILITY_EVENTS();
    selection_changed_nodes_.insert(&node);
}

gfx::Rect BrowserAccessibilityManagerWin::GetViewBoundsInScreenCoordinates() const
{
    AXPlatformTreeManagerDelegate* delegate = GetDelegateFromRootManager();
    if (!delegate) {
        return gfx::Rect();
...
void BrowserAccessibilityManagerWin::BeforeAccessibilityEvents()
{
    ...
    for (const auto& targeted_event : event_generator()) {
        if (targeted_event.event_params->event == AXEventGenerator::Event::IGNORED_CHANGED) {
...
void BrowserAccessibilityManagerWin::FinalizeAccessibilityEvents()
{
    ...
    for (BrowserAccessibility* event_node : selection_changed_nodes_) {
        DCHECK(event_node);
        if (ToBrowserAccessibilityWin(event_node)->GetCOM()->IsPatternProviderSupported(UIA_TextPatternId)) {
    ...
    FinalizeSelectionEvents(ia2_selection_events_, base::BindRepeating(&IsIA2NodeSelected),
        base::BindRepeating(&BrowserAccessibilityManagerWin::FireIA2SelectionEvents, base::Unretained(this)));
    FinalizeSelectionEvents(uia_selection_events_, base::BindRepeating(&IsUIANodeSelected),
        base::BindRepeating(&BrowserAccessibilityManagerWin::FireUIASelectionEvents, base::Unretained(this)));
