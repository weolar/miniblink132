...
public:
        ...
        : ax_mode_setter_(kAXModeComplete)
    {
        ...
        TestAXNodeWrapper* wrapper = TestAXNodeWrapper::GetOrCreate(GetTree(), GetRoot());
        if (!wrapper) {
            NOTREACHED();
        }
...
TEST_F(AtkUtilAuraLinuxTest, KeySnooping)
{
    AtkKeySnoopFunc key_snoop_func
        = reinterpret_cast<AtkKeySnoopFunc>(+[](AtkKeyEventStruct* key_event, int* keyval_seen) { *keyval_seen = key_event->keyval; });
...
TEST_F(AtkUtilAuraLinuxTest, AtSpiReady)
{
    ...
    // as enabled.
    AtkKeySnoopFunc key_snoop_func = reinterpret_cast<AtkKeySnoopFunc>(+[](AtkKeyEventStruct* key_event) {});
