...
BrowserAccessibilityWin::BrowserAccessibilityWin(BrowserAccessibilityManager* manager, AXNode* node)
    : BrowserAccessibility(manager, node)
{
    win::CreateATLModuleIfNeeded();
    HRESULT hr = CComObject<BrowserAccessibilityComWin>::CreateInstance(&browser_accessibility_com_.AsEphemeralRawAddr());
    DCHECK(SUCCEEDED(hr));
...
std::wstring BrowserAccessibilityWin::ComputeListItemNameFromContent() const
{
    ...
    std::wstring str;
    int offset = 0;
    if (node()->GetFirstChild() && node()->GetFirstChild()->GetRole() == ax::mojom::Role::kListMarker) {
        offset = 1;
    }
    ...
    // children.
    str = base::UTF16ToWide(range.GetText(AXTextConcatenationBehavior::kWithoutParagraphBreaks, AXEmbeddedObjectBehavior::kSuppressCharacter));
...
const std::vector<gfx::NativeViewAccessible> BrowserAccessibilityWin::GetUIADirectChildrenInRange(AXPlatformNodeDelegate* start, AXPlatformNodeDelegate* end)
{
    ...
    if (!IsIgnored() && !ShouldHideChildrenForUIA() && PlatformChildCount() > 0) {
        ...
        for (auto it = PlatformChildrenBegin(); it != PlatformChildrenEnd(); ++it) {
            ...
            if (!in_range && (start_wrapper && (child == start_wrapper || start_wrapper->IsDescendantOf(child)))) {
                in_range = true;
            }
            ...
            if (end_wrapper && (child == end_wrapper || end_wrapper->IsDescendantOf(child))) {
                break;
            }
        }
    }
...
const BrowserAccessibilityWin* ToBrowserAccessibilityWin(const BrowserAccessibility* obj)
{
