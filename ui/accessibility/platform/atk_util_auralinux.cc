...
static guint AtkUtilAuraLinuxAddKeyEventListener(AtkKeySnoopFunc key_snoop_function, gpointer data)
{
    ...
    GetActiveKeySnoopFunctions()[current_key_event_listener_id] = std::make_pair(key_snoop_function, data);
...
bool AtkUtilAuraLinux::ShouldEnableAccessibility()
{
    ...
    if (connection) {
        GVariant* result = g_dbus_connection_call_sync(connection, "org.a11y.Bus", "/org/a11y/bus", "org.freedesktop.DBus.Properties", "Get",
            g_variant_new("(ss)", "org.a11y.Status", "IsEnabled"), nullptr, G_DBUS_CALL_FLAGS_NONE, -1, nullptr, nullptr);
    ...
    gschema = g_settings_schema_source_lookup(source, "org.gnome.desktop.interface", TRUE);
    if (gschema) {
        GSettings* settings = g_settings_new("org.gnome.desktop.interface");
        const bool accessibilityEnabled = g_settings_get_boolean(settings, "toolkit-accessibility");
...
__attribute__((no_sanitize("cfi-icall"))) DiscardAtkKeyEvent AtkUtilAuraLinux::HandleAtkKeyEvent(AtkKeyEventStruct* key_event)
{
    DCHECK(key_event);
