...
BrowserAccessibilityAuraLinux* ToBrowserAccessibilityAuraLinux(BrowserAccessibility* obj)
{
...
BrowserAccessibilityAuraLinux::BrowserAccessibilityAuraLinux(BrowserAccessibilityManager* manager, AXNode* node)
    : BrowserAccessibility(manager, node)
{
    platform_node_ = static_cast<AXPlatformNodeAuraLinux*>(AXPlatformNode::Create(this));
}
...
gfx::NativeViewAccessible BrowserAccessibilityAuraLinux::GetNativeViewAccessible()
{
    DCHECK(platform_node_);
...
{
    manager()->ToBrowserAccessibilityManagerAuraLinux()->SetPrimaryWebContentsForWindow(GetId());
}

bool BrowserAccessibilityAuraLinux::IsPrimaryWebContentsForWindow() const
{
    auto primary_id = manager()->ToBrowserAccessibilityManagerAuraLinux()->GetPrimaryWebContentsForWindow();
