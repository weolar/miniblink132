...
{
    caret_.reset(static_cast<AXPlatformNodeWin*>(AXPlatformNodeWin::Create(this)));
    ...
    if (event_target_) {
        ::NotifyWinEvent(EVENT_OBJECT_CREATE, event_target_, OBJID_CARET, -caret_->GetUniqueId());
    }
}
...
{
    if (event_target_) {
        ::NotifyWinEvent(EVENT_OBJECT_DESTROY, event_target_, OBJID_CARET, -caret_->GetUniqueId());
    }
}
...
void AXSystemCaretWin::MoveCaretTo(const gfx::Rect& bounds_physical_pixels)
{
    ...
    if (newly_visible) {
        ::NotifyWinEvent(EVENT_OBJECT_SHOW, event_target_, OBJID_CARET, -caret_->GetUniqueId());
    }
    ...
    if (data_.relative_bounds.bounds != new_location || newly_visible) {
        ...
        ::NotifyWinEvent(EVENT_OBJECT_LOCATIONCHANGE, event_target_, OBJID_CARET, -caret_->GetUniqueId());
    }
}
...
void AXSystemCaretWin::Hide()
{
    if (!data_.HasState(ax::mojom::State::kInvisible)) {
        ...
        if (event_target_) {
            ::NotifyWinEvent(EVENT_OBJECT_HIDE, event_target_, OBJID_CARET, -caret_->GetUniqueId());
        }
    }
}
...
gfx::NativeViewAccessible AXSystemCaretWin::GetParent() const
{
    ...
    HRESULT hr = ::AccessibleObjectFromWindow(event_target_, OBJID_WINDOW, IID_IAccessible, reinterpret_cast<void**>(&parent));
    if (SUCCEEDED(hr))
        return parent;
    return nullptr;
}
    ...
    const AXCoordinateSystem coordinate_system, const AXClippingBehavior clipping_behavior, AXOffscreenResult* offscreen_result) const
{
    ...
    case AXCoordinateSystem::kScreenDIPs:
        return display::win::ScreenWin::ScreenToDIPRect(event_target_, ToEnclosingRect(data_.relative_bounds.bounds));
...
gfx::AcceleratedWidget AXSystemCaretWin::GetTargetForNativeAccessibilityEvent()
{
