...
class AXFragmentRootPlatformNodeWin : public AXPlatformNodeWin, public IItemContainerProvider, public IRawElementProviderFragmentRoot {
public:
    ...
    static AXFragmentRootPlatformNodeWin* Create(AXPlatformNodeDelegate* delegate)
    {
        ...
        HRESULT hr = CComObject<AXFragmentRootPlatformNodeWin>::CreateInstance(&instance);
        CHECK(SUCCEEDED(hr));
    ...
    IFACEMETHODIMP FindItemByProperty(
        IRawElementProviderSimple* start_after_element, PROPERTYID property_id, VARIANT value, IRawElementProviderSimple** result) override
    {
        ...
        *result = nullptr;
        ...
        if (property_id == UiaRegistrarWin::GetInstance().GetUniqueIdPropertyId() && value.vt == VT_BSTR) {
            ...
            if (AXPlatformNodeWin* result_platform_node = static_cast<AXPlatformNodeWin*>(GetFromUniqueId(-ax_unique_id))) {
                if (start_after_element) {
                    Microsoft::WRL::ComPtr<AXPlatformNodeWin> start_after_platform_node;
                    if (!SUCCEEDED(start_after_element->QueryInterface(IID_PPV_ARGS(&start_after_platform_node))))
                        return E_INVALIDARG;
    ...
    IFACEMETHODIMP GetPropertyValue(PROPERTYID property_id, VARIANT* result) override
    {
        ...
        case UIA_IsContentElementPropertyId:
            ...
            result->vt = VT_BOOL;
            result->boolVal = static_cast<AXFragmentRootWin*>(GetDelegate())->IsControlElement() ? VARIANT_TRUE : VARIANT_FALSE;
            break;
        }
    ...
    IFACEMETHODIMP ElementProviderFromPoint(
        double screen_physical_pixel_x, double screen_physical_pixel_y, IRawElementProviderFragment** element_provider) override
    {
        ...
        WIN_ACCESSIBILITY_SOURCE_API_PERF_HISTOGRAM(UMA_API_ELEMENT_PROVIDER_FROM_POINT);
        ...
        *element_provider = nullptr;
        ...
        do {
            gfx::NativeViewAccessible test_result = node_to_test->GetDelegate()->HitTestSync(screen_physical_pixel_x, screen_physical_pixel_y);
    ...
    IFACEMETHODIMP GetFocus(IRawElementProviderFragment** focus) override
    {
        ...
        *focus = nullptr;
        ...
        AXPlatformNode* node_to_test = this;
        do {
            gfx::NativeViewAccessible test_result = node_to_test->GetDelegate()->GetFocus();
            if (test_result != nullptr && test_result != focused_element) {
                focused_element = test_result;
                node_to_test = AXPlatformNode::FromNativeViewAccessible(focused_element);
            } else {
                node_to_test = nullptr;
            }
        } while (node_to_test);
...
public:
    ...
    AXFragmentRootWin* GetFragmentRootParentOf(gfx::NativeViewAccessible accessible) const
    {
...
private:
    std::unordered_map<gfx::AcceleratedWidget, raw_ptr<AXFragmentRootWin, CtnExperimental>> map_;
};

AXFragmentRootWin::AXFragmentRootWin(gfx::AcceleratedWidget widget, AXFragmentRootDelegateWin* delegate)
    : widget_(widget)
    , delegate_(delegate)
{
...
AXFragmentRootWin* AXFragmentRootWin::GetForAcceleratedWidget(gfx::AcceleratedWidget widget)
{
    return AXFragmentRootMapWin::GetInstance().GetFragmentRoot(widget);
}

// static
AXFragmentRootWin* AXFragmentRootWin::GetFragmentRootParentOf(gfx::NativeViewAccessible accessible)
{
    return AXFragmentRootMapWin::GetInstance().GetFragmentRootParentOf(accessible);
}
...
gfx::AcceleratedWidget AXFragmentRootWin::GetTargetForNativeAccessibilityEvent()
{
    return widget_;
}

AXPlatformNode* AXFragmentRootWin::GetFromTreeIDAndNodeID(const AXTreeID& ax_tree_id, int32_t node_id)
{
    AXPlatformNodeDelegate* child_delegate = GetChildNodeDelegate();
    if (child_delegate)
...
size_t AXFragmentRootWin::GetIndexInParentOfChild() const
{
    ...
    if (child) {
        ...
        for (size_t child_index = 0; child_index < child_count; child_index++) {
            if (AXPlatformNode::FromNativeViewAccessible(parent->ChildAtIndex(child_index))->GetDelegate() == child) {
                return child_index;
            }
        }
    }
