...
const GUID GUID_IAccessibleContentDocument = { 0xa5d8e1f3, 0x3571, 0x4d8f, { 0x95, 0x21, 0x07, 0xed, 0x28, 0xfb, 0x07, 0x2e } };
...
#define WIN_ACCESSIBILITY_API_HISTOGRAM(enum_value) UMA_HISTOGRAM_ENUMERATION("Accessibility.WinAPIs", enum_value, UMA_API_MAX)

#define WIN_ACCESSIBILITY_API_PERF_HISTOGRAM(enum_value) SCOPED_UMA_HISTOGRAM_TIMER_MICROS("Accessibility.Performance.WinAPIs." #enum_value)
...
#define WIN_ACCESSIBILITY_SOURCE_API_PERF_HISTOGRAM(enum_value)                                                                                                \
    DCHECK(GetDelegate());                                                                                                                                     \
    absl::Cleanup record_metric = [node = (GetDelegate() ? GetDelegate()->node() : nullptr), timer = base::ElapsedTimer()] {                                   \
        base::UmaHistogramMicrosecondsTimes(node && !node->IsView() ? std::string_view("Accessibility.Performance.WinAPIs2."                                   \
...
// interfaces.
#define WIN_ACCESSIBILITY_API_TRACE_EVENT(function) TRACE_EVENT("accessibility", function, perfetto::Flow::FromPointer(this))
...
extern COMPONENT_EXPORT(AX_PLATFORM) base::ObserverList<WinAccessibilityAPIUsageObserver>::Unchecked& GetWinAccessibilityAPIUsageObserverList();

// Used to simplify calling StartFiringUIAEvents and EndFiringEvents
class COMPONENT_EXPORT(AX_PLATFORM) WinAccessibilityAPIUsageScopedUIAEventsNotifier {
public:
    WinAccessibilityAPIUsageScopedUIAEventsNotifier();
    ~WinAccessibilityAPIUsageScopedUIAEventsNotifier();
};

class COMPONENT_EXPORT(AX_PLATFORM) __declspec(uuid("26f5641a-246d-457b-a96d-07f3fae6acf2")) AXPlatformNodeWin
    : public SequenceAffineComObjectRoot,
      public IDispatchImpl<IAccessible2_4, &IID_IAccessible2_4, &LIBID_IAccessible2Lib>,
...
public:
    ...
    IFACEMETHODIMP accHitTest(LONG screen_physical_pixel_x, LONG screen_physical_pixel_y, VARIANT* child) override;

    // Performs the object's default action.
    IFACEMETHODIMP accDoDefaultAction(VARIANT var_id) override;

    // Retrieves the specified object's current screen location.
    IFACEMETHODIMP accLocation(LONG* physical_pixel_left, LONG* physical_pixel_top, LONG* width, LONG* height, VARIANT var_id) override;

    // Traverses to another UI element and retrieves the object.
    IFACEMETHODIMP accNavigate(LONG nav_dir, VARIANT start, VARIANT* end) override;

    // Retrieves an IDispatch interface pointer for the specified child.
    IFACEMETHODIMP get_accChild(VARIANT var_child, IDispatch** disp_child) override;

    // Retrieves the number of accessible children.
    IFACEMETHODIMP get_accChildCount(LONG* child_count) override;

    // Retrieves a string that describes the object's default action.
    IFACEMETHODIMP get_accDefaultAction(VARIANT var_id, BSTR* default_action) override;
    ...
    IFACEMETHODIMP get_accKeyboardShortcut(VARIANT var_id, BSTR* access_key) override;
    ...
    IFACEMETHODIMP get_accHelpTopic(BSTR* help_file, VARIANT var_id, LONG* topic_id) override;
    ...
    IFACEMETHODIMP get_relationTargetsOfType(BSTR type, LONG max_targets, IUnknown*** targets, LONG* n_targets) override;
    ...
    IFACEMETHODIMP get_relation(LONG relation_index, IAccessibleRelation** relation) override;

    IFACEMETHODIMP get_relations(LONG max_relations, IAccessibleRelation** relations, LONG* n_relations) override;
    ...
    IFACEMETHODIMP scrollToPoint(enum IA2CoordinateType coordinate_type, LONG x, LONG y) override;
    IFACEMETHODIMP get_groupPosition(LONG* group_level, LONG* similar_items_in_group, LONG* position_in_group) override;
    IFACEMETHODIMP get_localizedExtendedRole(BSTR* localized_extended_role) override;
    IFACEMETHODIMP get_nExtendedStates(LONG* n_extended_states) override;
    IFACEMETHODIMP get_extendedStates(LONG max_extended_states, BSTR** extended_states, LONG* n_extended_states) override;
    IFACEMETHODIMP get_localizedExtendedStates(
        LONG max_localized_extended_states, BSTR** localized_extended_states, LONG* n_localized_extended_states) override;
    IFACEMETHODIMP get_locale(IA2Locale* locale) override;
    IFACEMETHODIMP get_accessibleWithCaret(IUnknown** accessible, LONG* caret_offset) override;
    ...
    IFACEMETHODIMP GetObjectForChild(LONG child_id, IAccessibleEx** result) override;

    IFACEMETHODIMP GetIAccessiblePair(IAccessible** accessible, LONG* child_id) override;
    ...
    IFACEMETHODIMP get_ContainingGrid(IRawElementProviderSimple** result) override;
    ...
    IFACEMETHODIMP Scroll(ScrollAmount horizontal_amount, ScrollAmount vertical_amount) override;

    IFACEMETHODIMP SetScrollPercent(double horizontal_percent, double vertical_percent) override;
    ...
    IFACEMETHODIMP get_SelectionContainer(IRawElementProviderSimple** result) override;
    ...
    IFACEMETHODIMP get_WindowInteractionState(WindowInteractionState* result) override;
    ...
    IFACEMETHODIMP
    ConvertReturnedElement(IRawElementProviderSimple* element, IAccessibleEx** acc) override;
    ...
    IFACEMETHODIMP get_selection(LONG selection_index, LONG* start_offset, LONG* end_offset) override;

    IFACEMETHODIMP get_text(LONG start_offset, LONG end_offset, BSTR* text) override;

    IFACEMETHODIMP get_textAtOffset(LONG offset, enum IA2TextBoundaryType boundary_type, LONG* start_offset, LONG* end_offset, BSTR* text) override;

    IFACEMETHODIMP get_textBeforeOffset(LONG offset, enum IA2TextBoundaryType boundary_type, LONG* start_offset, LONG* end_offset, BSTR* text) override;

    IFACEMETHODIMP get_textAfterOffset(LONG offset, enum IA2TextBoundaryType boundary_type, LONG* start_offset, LONG* end_offset, BSTR* text) override;

    IFACEMETHODIMP get_offsetAtPoint(LONG x, LONG y, enum IA2CoordinateType coord_type, LONG* offset) override;
    ...
    IFACEMETHODIMP get_accessibleAt(LONG row, LONG column, IUnknown** accessible) override;

    IFACEMETHODIMP get_caption(IUnknown** accessible) override;

    IFACEMETHODIMP get_childIndex(LONG row_index, LONG column_index, LONG* cell_index) override;
    ...
    IFACEMETHODIMP
    get_columnHeader(IAccessibleTable** accessible_table, LONG* starting_row_index) override;
    ...
    IFACEMETHODIMP get_rowExtentAt(LONG row, LONG column, LONG* n_rows_spanned) override;

    IFACEMETHODIMP
    get_rowHeader(IAccessibleTable** accessible_table, LONG* starting_column_index) override;

    IFACEMETHODIMP get_rowIndex(LONG cell_index, LONG* row_index) override;

    IFACEMETHODIMP get_selectedChildren(LONG max_children, LONG** children, LONG* n_children) override;

    IFACEMETHODIMP get_selectedColumns(LONG max_columns, LONG** columns, LONG* n_columns) override;

    IFACEMETHODIMP get_selectedRows(LONG max_rows, LONG** rows, LONG* n_rows) override;
    ...
    IFACEMETHODIMP get_isSelected(LONG row, LONG column, boolean* is_selected) override;

    IFACEMETHODIMP
    get_rowColumnExtentsAtIndex(LONG index, LONG* row, LONG* column, LONG* row_extents, LONG* column_extents, boolean* is_selected) override;
    ...
    IFACEMETHODIMP
    get_columnHeaderCells(IUnknown*** cell_accessibles, LONG* n_column_header_cells) override;
    ...
    IFACEMETHODIMP
    get_rowHeaderCells(IUnknown*** cell_accessibles, LONG* n_row_header_cells) override;
    ...
    IFACEMETHODIMP
    get_rowColumnExtents(LONG* row, LONG* column, LONG* row_extents, LONG* column_extents, boolean* is_selected) override;
    ...
    IFACEMETHODIMP setSelections(LONG nSelections, IA2TextSelection* selections) override;
    ...
    IFACEMETHODIMP get_attributes(LONG offset, LONG* start_offset, LONG* end_offset, BSTR* text_attributes) override;
    IFACEMETHODIMP get_characterExtents(LONG offset, enum IA2CoordinateType coord_type, LONG* x, LONG* y, LONG* width, LONG* height) override;
    IFACEMETHODIMP removeSelection(LONG selection_index) override;
    IFACEMETHODIMP setCaretOffset(LONG offset) override;
    IFACEMETHODIMP setSelection(LONG selection_index, LONG start_offset, LONG end_offset) override;
    IFACEMETHODIMP scrollSubstringTo(LONG start_index, LONG end_index, enum IA2ScrollType scroll_type) override;
    IFACEMETHODIMP scrollSubstringToPoint(LONG start_index, LONG end_index, enum IA2CoordinateType coordinate_type, LONG x, LONG y) override;
    ...
    IFACEMETHODIMP Navigate(NavigateDirection direction, IRawElementProviderFragment** element_provider) override;
    IFACEMETHODIMP GetRuntimeId(SAFEARRAY** runtime_id) override;
    IFACEMETHODIMP get_BoundingRectangle(UiaRect* screen_physical_pixel_bounds) override;
    IFACEMETHODIMP GetEmbeddedFragmentRoots(SAFEARRAY** embedded_fragment_roots) override;
    ...
    IFACEMETHODIMP get_bulkFetch(BSTR input_json, LONG request_id, IChromeAccessibleDelegate* delegate) override;

    IFACEMETHODIMP get_hitTest(LONG screen_physical_pixel_x, LONG screen_physical_pixel_y, LONG request_id, IChromeAccessibleDelegate* delegate) override;

    //
    // IServiceProvider methods.
    //

    IFACEMETHODIMP QueryService(REFGUID guidService, REFIID riid, void** object) override;
    ...
    static STDMETHODIMP InternalQueryInterface(void* this_ptr, const _ATL_INTMAP_ENTRY* entries, REFIID riid, void** object);
    ...
    void OnActiveComposition(const gfx::Range& range, const std::u16string& active_composition_text, bool is_composition_committed);
    ...
    static std::optional<PROPERTYID> MojoEventToUIAProperty(ax::mojom::Event event);
...
protected:
    ...
    void SanitizeTextAttributeValue(const std::string& input, std::string* output) const override;

    // Escapes characters in string attributes as required by the IA2 Spec.
    // It's okay for input to be the same as output.
    static void SanitizeStringAttributeForIA2(const std::string& input, std::string* output);
    FRIEND_TEST_ALL_PREFIXES(AXPlatformNodeWinTest, SanitizeStringAttributeForIA2);
...
private:
    ...
    HRESULT GetStringAttributeAsBstr(ax::mojom::StringAttribute attribute, BSTR* value_bstr) const;
    ...
    static void SanitizeStringAttributeForUIAAriaProperty(const std::wstring& input, std::wstring* output);
    ...
    void StringAttributeToUIAAriaProperty(std::vector<std::wstring>& properties, ax::mojom::StringAttribute attribute, const char* uia_aria_property);

    // If the bool attribute |attribute| is present, add its value as a
    // UIA AriaProperties Property with the name |uia_aria_property|.
    void BoolAttributeToUIAAriaProperty(std::vector<std::wstring>& properties, ax::mojom::BoolAttribute attribute, const char* uia_aria_property);

    // If the int attribute |attribute| is present, add its value as a
    // UIA AriaProperties Property with the name |uia_aria_property|.
    void IntAttributeToUIAAriaProperty(std::vector<std::wstring>& properties, ax::mojom::IntAttribute attribute, const char* uia_aria_property);

    // If the float attribute |attribute| is present, add its value as a
    // UIA AriaProperties Property with the name |uia_aria_property|.
    void FloatAttributeToUIAAriaProperty(std::vector<std::wstring>& properties, ax::mojom::FloatAttribute attribute, const char* uia_aria_property);
    ...
    void StateToUIAAriaProperty(std::vector<std::wstring>& properties, ax::mojom::State state, const char* uia_aria_property);
    ...
    SAFEARRAY* CreateUIAElementsArrayForRelation(const ax::mojom::IntListAttribute& attribute);
    ...
    SAFEARRAY* CreateUIAElementsArrayForReverseRelation(const ax::mojom::IntListAttribute& attribute);
    ...
    std::vector<AXPlatformNodeWin*> CreatePlatformNodeVectorFromRelationIdVector(const std::vector<int32_t>& relation_id_list);
    ...
    SAFEARRAY* CreateUIAElementsSafeArray(const std::vector<AXPlatformNodeWin*>& platform_node_list);
    ...
    gfx::Vector2d CalculateUIAScrollPoint(const ScrollAmount horizontal_amount, const ScrollAmount vertical_amount) const;
    ...
    LONG FindBoundary(IA2TextBoundaryType ia2_boundary, LONG start_offset, ax::mojom::MoveDirection direction);
    ...
    HRESULT AllocateComArrayFromVector(std::vector<LONG>& results, LONG max, LONG** selected, LONG* n_selected);
    ...
    HRESULT GetAnnotationTypesAttribute(const std::optional<int>& start_offset, const std::optional<int>& end_offset, base::win::VariantVector* result);
    ...
    static std::optional<HorizontalTextAlignment> AXTextAlignToUIAHorizontalTextAlignment(ax::mojom::TextAlign text_align);
    ...
    static FlowDirections TextDirectionToFlowDirections(ax::mojom::WritingDirection);

    // Helper method for |GetMarkerTypeFromRange| which aggregates all
    // of the ranges for |marker_type| attached to |node|.
    static void AggregateRangesForMarkerType(AXPlatformNodeBase* node, ax::mojom::MarkerType marker_type, int offset_ranges_amount,
    ...
    MarkerTypeRangeResult GetMarkerTypeFromRange(const std::optional<int>& start_offset, const std::optional<int>& end_offset,
        ax::mojom::MarkerType marker_type, const std::optional<ax::mojom::HighlightType>& highlight_type = std::nullopt);
    ...
    TextDecorationLineStyle GetUIATextDecorationStyle(const ax::mojom::IntAttribute int_attribute) const;

    // IRawElementProviderSimple support methods.

    using PatternProviderFactoryMethod = void (*)(AXPlatformNodeWin*, IUnknown**);

    PatternProviderFactoryMethod GetPatternProviderFactoryMethod(PATTERNID pattern_id);

    // Fires UIA text edit event about composition (active or committed)
    void FireUiaTextEditTextChangedEvent(const gfx::Range& range, const std::wstring& active_composition_text, bool is_composition_committed);
    ...
    friend AXPlatformNode* AXPlatformNode::Create(AXPlatformNodeDelegate* delegate);
};
