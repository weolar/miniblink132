...
std::unordered_set<AXPlatformNodeId, typename AXPlatformNodeId::Hasher>& GetAssignedIds()
{
    static base::NoDestructor<std::unordered_set<AXPlatformNodeId, typename AXPlatformNodeId::Hasher>> assigned_ids;
    return *assigned_ids;
}
...
AXPlatformNodeId AXUniqueId::GetNextAXUniqueId(int32_t max_id)
{
    ...
    do {
        ...
        CHECK_NE(current_id, prev_id) << "There are over 2 billion available IDs, so the newly created ID "
                                         "cannot be equal to the most recently created ID.";
        // If it |has_wrapped| then we need to continue until we find the first
        // unassigned ID.
    } while (has_wrapped && base::Contains(assigned_ids, AXPlatformNodeId(current_id)));
