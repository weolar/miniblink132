...
public:
    ...
    void AnnounceTextAs(const std::u16string& text, AnnouncementType announcement_type) override;
#endif
    ...
    using AXPlatformNodeChildIterator = AXNode::ChildIteratorBase<AXPlatformNodeBase, &AXPlatformNodeBase::GetNextSibling,
        &AXPlatformNodeBase::GetPreviousSibling, &AXPlatformNodeBase::GetFirstChild, &AXPlatformNodeBase::GetLastChild>;
    ...
    static void SetOnNotifyEventCallbackForTesting(ax::mojom::Event event_type, base::RepeatingClosure callback);
    ...
    virtual int FindTextBoundary(ax::mojom::TextBoundary boundary, int offset, ax::mojom::MoveDirection direction, ax::mojom::TextAffinity affinity) const;
...
protected:
    ...
    void GetSelectionOffsets(const AXSelection* selection, int* selection_start, int* selection_end);
    void GetSelectionOffsetsFromTree(const AXSelection* selection, int* selection_start, int* selection_end);
    ...
    AXPlatformNodeBase::AXPosition HypertextOffsetToEndpoint(int hypertext_offset) const;

    bool IsSameHypertextCharacter(const AXLegacyHypertext& old_hypertext, size_t old_char_index, size_t new_char_index);
    void ComputeHypertextRemovedAndInserted(const AXLegacyHypertext& old_hypertext, size_t* start, size_t* old_len, size_t* new_len);
...
private:
    ...
    bool IsText(const std::u16string& text, size_t index, bool is_indexed_from_end = false);
