...
namespace content {
FORWARD_DECLARE_TEST(BrowserAccessibilityManagerTest, TestShouldFireEventForNode);
FORWARD_DECLARE_TEST(BrowserAccessibilityManagerTest, TestShouldFireEventForAlertEventWithEmptyName);
FORWARD_DECLARE_TEST(BrowserAccessibilityManagerTest, TestShouldFireEventForAlertEventWithNonEmptyName);
}
namespace ui {
...
using GeneratedEventCallbackForTesting = base::RepeatingCallback<void(BrowserAccessibilityManager*, AXEventGenerator::Event, AXNodeID)>;

COMPONENT_EXPORT(AX_PLATFORM)
AXTreeUpdate MakeAXTreeUpdateForTesting(const AXNodeData& node, const AXNodeData& node2 = AXNodeData(), const AXNodeData& node3 = AXNodeData(),
...
class COMPONENT_EXPORT(AX_PLATFORM) BrowserAccessibilityManager : public AXPlatformTreeManager {
public:
    // Creates the platform-specific BrowserAccessibilityManager.
    static BrowserAccessibilityManager* Create(const AXTreeUpdate& initial_tree, AXNodeIdDelegate& node_id_delegate, AXPlatformTreeManagerDelegate* delegate);
    static BrowserAccessibilityManager* Create(AXNodeIdDelegate& node_id_delegate, AXPlatformTreeManagerDelegate* delegate);
    ...
    BrowserAccessibilityManager& operator=(const BrowserAccessibilityManager&) = delete;
    ...
    virtual void FireAriaNotificationEvent(BrowserAccessibility* node, const std::string& announcement, const std::string& notification_id,
        ax::mojom::AriaNotificationInterrupt interrupt_property, ax::mojom::AriaNotificationPriority priority_property)
    {
    }

    virtual void FireBlinkEvent(ax::mojom::Event event_type, BrowserAccessibility* node, int action_request_id)
    {
    }

    // AXPlatformTreeManager overrides.
    void FireGeneratedEvent(AXEventGenerator::Event event_type, const AXNode* node) override;
    ...
    BrowserAccessibility* GetParentNodeFromParentTreeAsBrowserAccessibility() const;
    ...
    void SetGeneratedEventCallbackForTesting(const GeneratedEventCallbackForTesting& callback);
    ...
    void SetLocationChangeCallbackForTesting(const base::RepeatingClosure& callback);
    ...
    void GetImageData(const BrowserAccessibility& node, const gfx::Size& max_size);
    ...
    void Scroll(const BrowserAccessibility& node, ax::mojom::Action scroll_action);
    void ScrollToMakeVisible(const BrowserAccessibility& node, gfx::Rect subfocus,
        ax::mojom::ScrollAlignment horizontal_scroll_alignment = ax::mojom::ScrollAlignment::kScrollAlignmentCenter,
        ax::mojom::ScrollAlignment vertical_scroll_alignment = ax::mojom::ScrollAlignment::kScrollAlignmentCenter,
        ax::mojom::ScrollBehavior scroll_behavior = ax::mojom::ScrollBehavior::kDoNotScrollIfVisible);
    ...
    void SetSequentialFocusNavigationStartingPoint(const BrowserAccessibility& node);
    ...
    void StitchChildTree(const BrowserAccessibility& node, const AXTreeID& child_tree_id);
    ...
    virtual void OnFindInPageResult(int request_id, int match_index, int start_id, int start_offset, int end_id, int end_offset);
    ...
    BrowserAccessibilityManagerAuraLinux* ToBrowserAccessibilityManagerAuraLinux();
#endif
    ...
    // node controls.
    std::vector<BrowserAccessibility*> GetAriaControls(const BrowserAccessibility* focus) const;
    ...
    static BrowserAccessibility* NextInTreeOrder(const BrowserAccessibility* object);
    static BrowserAccessibility* NextNonDescendantInTreeOrder(const BrowserAccessibility* object);
    static BrowserAccessibility* PreviousInTreeOrder(const BrowserAccessibility* object, bool can_wrap_to_last_element);
    static BrowserAccessibility* NextTextOnlyObject(const BrowserAccessibility* object);
    static BrowserAccessibility* PreviousTextOnlyObject(const BrowserAccessibility* object);
    ...
    static bool FindIndicesInCommonParent(const BrowserAccessibility& object1, const BrowserAccessibility& object2, BrowserAccessibility** common_parent,
        size_t* child_index1, size_t* child_index2);
    ...
    static ax::mojom::TreeOrder CompareNodes(const BrowserAccessibility& object1, const BrowserAccessibility& object2);
    ...
    static std::u16string GetTextForRange(const BrowserAccessibility& start_object, const BrowserAccessibility& end_object);

    // If start and end offsets are greater than the text's length, returns all
    // the text.
    static std::u16string GetTextForRange(const BrowserAccessibility& start_object, int start_offset, const BrowserAccessibility& end_object, int end_offset);
    ...
    void OnAtomicUpdateStarting(AXTree* tree, const std::set<AXNodeID>& deleted_node_ids, const std::set<AXNodeID>& reparented_node_ids) override;
    void OnAtomicUpdateFinished(AXTree* tree, bool root_changed, const std::vector<AXTreeObserver::Change>& changes) override;
    ...
    AXPlatformTreeManagerDelegate* delegate() const
    {
        return delegate_;
    }
    ...
    BrowserAccessibility* CachingAsyncHitTest(const gfx::Point& physical_pixel_point) const;
    ...
    BrowserAccessibility* ApproximateHitTest(const gfx::Point& blink_screen_point) const;
    ...
    BrowserAccessibility* RetargetBrowserAccessibilityForEvents(BrowserAccessibility* node, RetargetEventType type) const;
...
protected:
    FRIEND_TEST_ALL_PREFIXES(content::BrowserAccessibilityManagerTest, TestShouldFireEventForNode);

    explicit BrowserAccessibilityManager(AXNodeIdDelegate& node_id_delegate, AXPlatformTreeManagerDelegate* delegate);
    ...
    virtual void SendLocationChangeEvents(const std::vector<AXLocationChange>& changes);
    ...
    virtual std::unique_ptr<BrowserAccessibility> CreateBrowserAccessibility(AXNode* node);
...
private:
    void BuildAXTreeHitTestCacheInternal(const BrowserAccessibility* node, std::vector<const BrowserAccessibility*>* storage);
    ...
    // called first.
    BrowserAccessibility* AXTreeHitTest(const gfx::Point& blink_screen_point) const;
