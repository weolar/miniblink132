...
protected:
    void SetOwner(AXPlatformNodeWin* owner, ITextRangeProvider* destination_range)
    {
        ...
        destination_provider->QueryInterface(IID_PPV_ARGS(&destination_provider_interal));
...
TEST_F(AXPlatformNodeTextProviderTest, CreateDegenerateRangeFromStart)
{
    ...
    ComPtr<IRawElementProviderSimple> text2_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(text2_node);

    ComPtr<AXPlatformNodeWin> root_platform_node;
    EXPECT_HRESULT_SUCCEEDED(root_node_raw->QueryInterface(IID_PPV_ARGS(&root_platform_node)));
    ComPtr<AXPlatformNodeWin> link_platform_node;
    EXPECT_HRESULT_SUCCEEDED(link_node_raw->QueryInterface(IID_PPV_ARGS(&link_platform_node)));
    ComPtr<AXPlatformNodeWin> text2_platform_node;
    EXPECT_HRESULT_SUCCEEDED(text2_node_raw->QueryInterface(IID_PPV_ARGS(&text2_platform_node)));

    // Degenerate range created on root node should be:
    // <>some-textmore-text
    ComPtr<ITextRangeProvider> text_range_provider;
    AXPlatformNodeTextProviderWin::CreateDegenerateRangeAtStart(root_platform_node.Get(), &text_range_provider);
    ...
    // <>some textmore text
    AXPlatformNodeTextProviderWin::CreateDegenerateRangeAtStart(link_platform_node.Get(), &text_range_provider);
    SetOwner(owner, text_range_provider.Get());
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetText(-1, text_content.Receive()));
    EXPECT_EQ(0, wcscmp(text_content.Get(), L""));
    text_range_provider->QueryInterface(IID_PPV_ARGS(&actual_range));
    ...
    // some text<>more text
    AXPlatformNodeTextProviderWin::CreateDegenerateRangeAtStart(text2_platform_node.Get(), &text_range_provider);
    SetOwner(owner, text_range_provider.Get());
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetText(-1, text_content.Receive()));
    EXPECT_EQ(0, wcscmp(text_content.Get(), L""));
    text_range_provider->QueryInterface(IID_PPV_ARGS(&actual_range));
...
TEST_F(AXPlatformNodeTextProviderTest, ITextProviderRangeFromChild)
{
    ...
    ComPtr<IRawElementProviderSimple> empty_text_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(empty_text_node);

    // Call RangeFromChild on the root with the text child passed in.
    ComPtr<ITextProvider> text_provider;
    EXPECT_HRESULT_SUCCEEDED(root_node_raw->GetPatternProvider(UIA_TextPatternId, &text_provider));

    ComPtr<ITextRangeProvider> text_range_provider;
    EXPECT_HRESULT_SUCCEEDED(text_provider->RangeFromChild(text_node_raw.Get(), &text_range_provider));
    SetOwner(owner, text_range_provider.Get());
    ...
    EXPECT_INVALIDARG(text_provider->RangeFromChild(root_node_raw.Get(), &text_range_provider));

    // Now test that a child with no text returns a degenerate range.
    EXPECT_HRESULT_SUCCEEDED(root_node_raw->GetPatternProvider(UIA_TextPatternId, &text_provider));

    EXPECT_HRESULT_SUCCEEDED(text_provider->RangeFromChild(empty_text_node_raw.Get(), &text_range_provider));
    SetOwner(owner, text_range_provider.Get());

    base::win::ScopedBstr empty_text_content;
    EXPECT_HRESULT_SUCCEEDED(text_range_provider->GetText(-1, empty_text_content.Receive()));
    ...
    MockIRawElementProviderSimple::CreateMockIRawElementProviderSimple(&other_root_node_raw);

    EXPECT_HRESULT_SUCCEEDED(root_node_raw->GetPatternProvider(UIA_TextPatternId, &text_provider));

    EXPECT_UIA_INVALIDOPERATION(text_provider->RangeFromChild(other_root_node_raw.Get(), &text_range_provider));
}
...
TEST_F(AXPlatformNodeTextProviderTest, ITextProviderRangeFromChildMultipleChildren)
{
    ...
    ComPtr<IRawElementProviderSimple> root_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(root_node);
    ComPtr<IRawElementProviderSimple> dialog_node_raw = QueryInterfaceFromNode<IRawElementProviderSimple>(dialog_node);
    ...
    ComPtr<ITextRangeProvider> text_range_provider;
    EXPECT_HRESULT_SUCCEEDED(text_provider->RangeFromChild(dialog_node_raw.Get(), &text_range_provider));
    SetOwner(owner, text_range_provider.Get());
...
TEST_F(AXPlatformNodeTextProviderTest, NearestTextIndexToPoint)
{
    ...
    // spacing: "t-e-x---t-"
    text_data.AddIntListAttribute(ax::mojom::IntListAttribute::kCharacterOffsets, { 2, 4, 8, 10 });
    ...
    NearestTextIndexTestData nodes[] = { { text_node, { { 0, 0 }, { 2, 0 }, { 3, 1 }, { 4, 1 }, { 5, 2 }, { 8, 2 }, { 9, 3 }, { 10, 3 } } },
        { root_node, { { 0, 0 }, { 2, 0 }, { 3, 0 }, { 4, 0 }, { 5, 0 }, { 8, 0 }, { 9, 0 }, { 10, 0 } } } };
    for (auto data : nodes) {
        ComPtr<IRawElementProviderSimple> element_provider = QueryInterfaceFromNode<IRawElementProviderSimple>(data.node);
        ComPtr<ITextProvider> text_provider;
        EXPECT_HRESULT_SUCCEEDED(element_provider->GetPatternProvider(UIA_TextPatternId, &text_provider));
        // get internal implementation to access helper for testing
        ComPtr<AXPlatformNodeTextProviderWin> platform_text_provider;
        EXPECT_HRESULT_SUCCEEDED(text_provider->QueryInterface(IID_PPV_ARGS(&platform_text_provider)));

        ComPtr<AXPlatformNodeWin> platform_node;
        EXPECT_HRESULT_SUCCEEDED(element_provider->QueryInterface(IID_PPV_ARGS(&platform_node)));

        for (auto pair : data.test_data) {
            EXPECT_EQ(pair.expected_index, platform_node->NearestTextIndexToPoint(gfx::Point(pair.point_offset_x, 0)));
        }
    }
}
...
TEST_F(AXPlatformNodeTextProviderTest, ITextProviderDocumentRangeTrailingIgnored)
{
    TestAXTreeUpdate update(std::string(R"HTML(
    ++1 kRootWebArea name="Document"
    ++++2 kGenericContainer
    ...
    update.nodes[1].AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, { 3 });
    update.nodes[3].AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, { 5 });
    ...
    EXPECT_HRESULT_SUCCEEDED(root_node->GetPatternProvider(UIA_TextPatternId, &root_text_provider));
    ...
    AXNodePosition::AXPositionInstance expected_start = owner->GetDelegate()->CreateTextPositionAt(0)->AsLeafTextPosition();
    AXNodePosition::AXPositionInstance expected_end = owner->GetDelegate()->CreateTextPositionAt(0)->CreatePositionAtEndOfAnchor()->AsLeafTextPosition();
...
TEST_F(AXPlatformNodeTextProviderTest, ITextProviderSupportedSelection)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(text_provider->get_SupportedTextSelection(&text_selection_mode));
...
TEST_F(AXPlatformNodeTextProviderTest, ITextProviderGetSelection)
{
    ...
    nonatomic_textfield_data.AddBoolAttribute(ax::mojom::BoolAttribute::kNonAtomicTextFieldRoot, true);
    ...
    update.nodes = { root_data, text_data, textbox_data, nonatomic_textfield_data, text_child_data };
    Init(update);
    ...
    AXPlatformNodeWin* owner = GetOwner(root_platform_node.Get());
    AXTreeData& selected_tree_data = const_cast<AXTreeData&>(owner->GetDelegate()->GetTreeData());
    ...
    LONG index = 0;
    ComPtr<ITextRangeProvider> text_range_provider;
    EXPECT_HRESULT_SUCCEEDED(SafeArrayGetElement(selections.Get(), &index, static_cast<void**>(&text_range_provider)));
    SetOwner(owner, text_range_provider.Get());
    ...
    ComPtr<IRawElementProviderSimple> text_edit_com = QueryInterfaceFromNode<IRawElementProviderSimple>(text_edit_node);

    ComPtr<ITextProvider> text_edit_provider;
    EXPECT_HRESULT_SUCCEEDED(text_edit_com->GetPatternProvider(UIA_TextPatternId, &text_edit_provider));

    selections.Reset();
    EXPECT_HRESULT_SUCCEEDED(text_edit_provider->GetSelection(selections.Receive()));
    ...
    EXPECT_HRESULT_SUCCEEDED(SafeArrayGetElement(selections.Get(), &index, static_cast<void**>(&text_edit_range_provider)));
    SetOwner(owner, text_edit_range_provider.Get());
    EXPECT_HRESULT_SUCCEEDED(text_edit_range_provider->GetText(-1, text_content.Receive()));
    ...
    EXPECT_EQ(static_cast<HRESULT>(UIA_E_ELEMENTNOTAVAILABLE), text_edit_provider->GetSelection(selections.Receive()));
}
...
TEST_F(AXPlatformNodeTextProviderTest, TestRemoveTextInvalidatingPositionForComparison)
{
...
TEST_F(AXPlatformNodeTextProviderTest, ITextProviderGetActiveComposition)
{
    ...
    EXPECT_HRESULT_SUCCEEDED(root_node->GetPatternProvider(UIA_TextEditPatternId, &root_text_edit_provider));
    ...
    ComPtr<AXPlatformNodeTextRangeProviderWin> actual_range;
    AXNodePosition::AXPositionInstance expected_start = owner->GetDelegate()->CreateTextPositionAt(0);
    AXNodePosition::AXPositionInstance expected_end = owner->GetDelegate()->CreateTextPositionAt(1);
    text_range_provider->QueryInterface(IID_PPV_ARGS(&actual_range));
    EXPECT_EQ(*GetStart(actual_range.Get()), *expected_start);
    EXPECT_EQ(*GetEnd(actual_range.Get()), *expected_end);
}
