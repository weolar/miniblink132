...
bool IsValuePatternSupported(AXPlatformNodeDelegate* delegate)
{
    ...
    if (delegate->IsCellOrHeaderOfAriaGrid() && !delegate->HasState(ax::mojom::State::kEditable)) {
        return false;
    }
    return delegate->GetData().IsRangeValueSupported() || delegate->IsReadOnlySupported() || IsLink(delegate->GetRole())
        || delegate->GetRole() == ax::mojom::Role::kColorWell || delegate->IsCellOrHeaderOfAriaGrid()
        || (delegate->GetRole() == ax::mojom::Role::kButton && delegate->HasStringAttribute(ax::mojom::StringAttribute::kValue))
        || (IsPlatformDocument(delegate->GetRole()) && delegate->GetRole() != ax::mojom::Role::kPdfRoot);
}
