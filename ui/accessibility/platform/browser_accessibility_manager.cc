...
AXTreeUpdate MakeAXTreeUpdateForTesting(const AXNodeData& node1, const AXNodeData& node2 /* = AXNodeData() */, const AXNodeData& node3 /* = AXNodeData() */,
    const AXNodeData& node4 /* = AXNodeData() */, const AXNodeData& node5 /* = AXNodeData() */, const AXNodeData& node6 /* = AXNodeData() */,
    const AXNodeData& node7 /* = AXNodeData() */, const AXNodeData& node8 /* = AXNodeData() */, const AXNodeData& node9 /* = AXNodeData() */,
    const AXNodeData& node10 /* = AXNodeData() */, const AXNodeData& node11 /* = AXNodeData() */, const AXNodeData& node12 /* = AXNodeData() */,
    const AXNodeData& node13 /* = AXNodeData() */, const AXNodeData& node14 /* = AXNodeData() */)
{
...
BrowserAccessibilityManager* BrowserAccessibilityManager::Create(
    const AXTreeUpdate& initial_tree, AXNodeIdDelegate& node_id_delegate, AXPlatformTreeManagerDelegate* delegate)
{
    BrowserAccessibilityManager* manager = new BrowserAccessibilityManager(node_id_delegate, delegate);
    manager->Initialize(initial_tree);
    return manager;
}

// static
BrowserAccessibilityManager* BrowserAccessibilityManager::Create(AXNodeIdDelegate& node_id_delegate, AXPlatformTreeManagerDelegate* delegate)
{
    BrowserAccessibilityManager* manager = new BrowserAccessibilityManager(node_id_delegate, delegate);
...
// static
BrowserAccessibilityManager* BrowserAccessibilityManager::FromID(AXTreeID ax_tree_id)
{
...
BrowserAccessibilityManager::BrowserAccessibilityManager(AXNodeIdDelegate& node_id_delegate, AXPlatformTreeManagerDelegate* delegate)
...
// static
bool BrowserAccessibilityManager::never_suppress_or_delay_events_for_testing_ = false;
...
void BrowserAccessibilityManager::FireFocusEventsIfNeeded()
{
    ...
    if (!never_suppress_or_delay_events_for_testing_ && !AXTreeManager::GetFocusChangeCallbackForTesting()) {
...
void BrowserAccessibilityManager::FireGeneratedEvent(AXEventGenerator::Event event_type, const AXNode* node)
{
    ...
    const auto& announcements = node_data.GetStringListAttribute(ax::mojom::StringListAttribute::kAriaNotificationAnnouncements);
    const auto& notification_ids = node_data.GetStringListAttribute(ax::mojom::StringListAttribute::kAriaNotificationIds);

    const auto& interrupt_properties = node_data.GetIntListAttribute(ax::mojom::IntListAttribute::kAriaNotificationInterruptProperties);
    const auto& priority_properties = node_data.GetIntListAttribute(ax::mojom::IntListAttribute::kAriaNotificationPriorityProperties);
    ...
    for (std::size_t i = 0; i < announcements.size(); ++i) {
        FireAriaNotificationEvent(wrapper, announcements[i], notification_ids[i], static_cast<ax::mojom::AriaNotificationInterrupt>(interrupt_properties[i]),
            static_cast<ax::mojom::AriaNotificationPriority>(priority_properties[i]));
    }
}

BrowserAccessibility* BrowserAccessibilityManager::GetBrowserAccessibilityRoot() const
{
    AXNode* root = GetRoot();
    return root ? GetFromID(root->id()) : nullptr;
}

BrowserAccessibility* BrowserAccessibilityManager::GetFromAXNode(const AXNode* node) const
{
    ...
    if (const AXTreeManager* manager = node->GetManager()) {
        ...
        if (manager->IsPlatformTreeManager()) {
            return static_cast<const BrowserAccessibilityManager*>(manager)->GetFromID(node->id());
        }
        return nullptr;
    }
...
BrowserAccessibility* BrowserAccessibilityManager::GetParentNodeFromParentTreeAsBrowserAccessibility() const
{
    ...
    BrowserAccessibilityManager* parent_manager_wrapper = static_cast<BrowserAccessibilityManager*>(parent_manager);
    BrowserAccessibility* parent_node = parent_manager_wrapper->GetFromAXNode(parent);
...
void BrowserAccessibilityManager ::SetUseRootScrollOffsetsWhenComputingBoundsForTesting(bool use)
{
    use_root_scroll_offsets_when_computing_bounds_ = use;
}

bool BrowserAccessibilityManager::OnAccessibilityEvents(const AXUpdatesAndEvents& details)
{
    TRACE_EVENT0(
        "accessibility", is_post_load_ ? "BrowserAccessibilityManager::OnAccessibilityEvents" : "BrowserAccessibilityManager::OnAccessibilityEventsLoading");
    SCOPED_UMA_HISTOGRAM_TIMER_MICROS("Accessibility.Performance.BrowserAccessibilityManager::"
    ...
    for (const AXTreeUpdate& tree_update : *tree_updates) {
        if (!ax_tree()->Unserialize(tree_update)) {
            ...
            CHECK(!ax_tree()->error().empty()) << "A failed serialization didn't supply the error via "
                                                  "AXTree::RecordError().";
            if (!delegate_)
    ...
    BrowserAccessibilityManager* parent_manager = has_parent_id ? BrowserAccessibilityManager::FromID(parent_id) : nullptr;
    if (IsRootFrameManager()) {
        CHECK(!has_parent_id) << "The root frame must be parentless, root url = " << GetTreeData().url
                              << "\nSupposed parent = " << (parent_manager ? parent_manager->GetTreeData().url : "[not in map for parent_tree_id]");
        CHECK(!connected_to_parent_tree_node_) << "Root manager must not be connected to a parent tree node.";
    } else {
        ...
        CHECK(connected_to_parent_tree_node_) << "Must be connected to parent tree node, otherwise could not reach "
    ...
    if (defer_load_complete_event_) {
        ...
        FireBlinkEvent(ax::mojom::Event::kLoadComplete, GetBrowserAccessibilityRoot(), -1);
    }
    ...
    for (const auto& targeted_event : event_generator()) {
        ...
        event_target = RetargetBrowserAccessibilityForEvents(event_target, RetargetEventType::RetargetEventTypeGenerated);
        if (!event_target)
    ...
    for (const AXEvent& event : details.events) {
        ...
        DCHECK(event_target) << "No event target for " << event.id << " with event type " << event.event_type;
        RetargetEventType type
            = event.event_type == ax::mojom::Event::kHover ? RetargetEventType::RetargetEventTypeBlinkHover : RetargetEventType::RetargetEventTypeBlinkGeneral;
        BrowserAccessibility* retargeted = RetargetBrowserAccessibilityForEvents(event_target, type);
        if (!retargeted)
            continue; // Drop the event if RetargetForEvents() returns nullptr.
...
void BrowserAccessibilityManager::OnLocationChanges(const AXLocationAndScrollUpdates& changes)
{
    TRACE_EVENT0("accessibility", is_post_load_ ? "BrowserAccessibilityManager::OnLocationChanges" : "BrowserAccessibilityManager::OnLocationChangesLoading");
    SCOPED_UMA_HISTOGRAM_TIMER_MICROS("Accessibility.Performance.BrowserAccessibilityManager::"
                                      "OnLocationChanges");
    ...
    for (auto& change : changes.scroll_changes) {
        ...
        if (can_fire_events) {
            if (change.scroll_x != old_scrollx) {
                FireGeneratedEvent(ui::AXEventGenerator::Event::SCROLL_HORIZONTAL_POSITION_CHANGED, node);
            }
            if (change.scroll_y != old_scrolly) {
                FireGeneratedEvent(ui::AXEventGenerator::Event::SCROLL_VERTICAL_POSITION_CHANGED, node);
            }
        }
    }
    ...
    for (auto& change : changes.location_changes) {
        ...
        AXNode* node = obj->node();
        node->SetLocation(change.new_location.offset_container_id, change.new_location.bounds, change.new_location.transform.get());
    }
    ...
    if (!location_change_callback_for_testing_.is_null() && !changes.location_changes.empty()) {
        location_change_callback_for_testing_.Run();
    }
}

void BrowserAccessibilityManager::SendLocationChangeEvents(const std::vector<AXLocationChange>& changes)
{
...
void BrowserAccessibilityManager::OnFindInPageResult(int request_id, int match_index, int start_id, int start_offset, int end_id, int end_offset)
{
...
BrowserAccessibility* BrowserAccessibilityManager::GetActiveDescendant(BrowserAccessibility* node) const
{
    if (!node)
        return nullptr;
    ...
    if (node->GetIntAttribute(ax::mojom::IntAttribute::kActivedescendantId, &active_descendant_id)) {
...
std::vector<BrowserAccessibility*> BrowserAccessibilityManager::GetAriaControls(const BrowserAccessibility* focus) const
{
    if (!focus)
        return {};

    std::vector<BrowserAccessibility*> aria_control_nodes;
    for (const auto& id : focus->GetIntListAttribute(ax::mojom::IntListAttribute::kControlsIds)) {
...
BrowserAccessibility* BrowserAccessibilityManager::GetFocusFromThisOrDescendantFrame() const
{
    ...
    if (obj->HasStringAttribute(ax::mojom::StringAttribute::kChildTreeId)) {
        AXTreeID child_tree_id = AXTreeID::FromString(obj->GetStringAttribute(ax::mojom::StringAttribute::kChildTreeId));
        const BrowserAccessibilityManager* child_manager = BrowserAccessibilityManager::FromID(child_tree_id);
        if (child_manager)
...
void BrowserAccessibilityManager::SetFocus(const BrowserAccessibility& node)
{
    ...
    base::RecordAction(base::UserMetricsAction("Accessibility.NativeApi.SetFocus"));
...
void BrowserAccessibilityManager::SetSequentialFocusNavigationStartingPoint(const BrowserAccessibility& node)
{
    if (!delegate_)
        return;
...
void BrowserAccessibilityManager::SetGeneratedEventCallbackForTesting(const GeneratedEventCallbackForTesting& callback)
{
    generated_event_callback_for_testing_ = callback;
}

void BrowserAccessibilityManager::SetLocationChangeCallbackForTesting(const base::RepeatingClosure& callback)
{
...
void BrowserAccessibilityManager::DoDefaultAction(const BrowserAccessibility& node)
{
    DCHECK(node.node()->data().GetDefaultActionVerb() != ax::mojom::DefaultActionVerb::kNone);

    if (!delegate_)
        return;

    base::RecordAction(base::UserMetricsAction("Accessibility.NativeApi.DoDefault"));
...
void BrowserAccessibilityManager::GetImageData(const BrowserAccessibility& node, const gfx::Size& max_size)
{
    if (!delegate_)
        return;
...
void BrowserAccessibilityManager::ShowContextMenu(const BrowserAccessibility& node)
{
    if (!delegate_)
        return;
...
void BrowserAccessibilityManager::Scroll(const BrowserAccessibility& node, ax::mojom::Action scroll_action)
{
    if (!delegate_)
        return;
...
void BrowserAccessibilityManager::ScrollToMakeVisible(const BrowserAccessibility& node, gfx::Rect subfocus,
    ax::mojom::ScrollAlignment horizontal_scroll_alignment, ax::mojom::ScrollAlignment vertical_scroll_alignment, ax::mojom::ScrollBehavior scroll_behavior)
{
    if (!delegate_)
        return;

    base::RecordAction(base::UserMetricsAction("Accessibility.NativeApi.ScrollToMakeVisible"));

    base::UmaHistogramBoolean("Accessibility.ScreenReader.ScrollToImage", node.GetRole() == ax::mojom::Role::kImage);
...
void BrowserAccessibilityManager::ScrollToPoint(const BrowserAccessibility& node, gfx::Point point)
{
    if (!delegate_)
        return;
...
void BrowserAccessibilityManager::SetScrollOffset(const BrowserAccessibility& node, gfx::Point offset)
{
    if (!delegate_)
        return;
...
void BrowserAccessibilityManager::SetValue(const BrowserAccessibility& node, const std::string& value)
{
    if (!delegate_)
        return;
...
void BrowserAccessibilityManager::SetSelection(const AXActionData& action_data)
{
    if (!delegate_)
        return;
    delegate_->AccessibilityPerformAction(action_data);
...
void BrowserAccessibilityManager::SetSelection(const BrowserAccessibility::AXRange& range)
{
...
void BrowserAccessibilityManager::StitchChildTree(const BrowserAccessibility& node, const AXTreeID& child_tree_id)
{
    if (!delegate_) {
        return;
    }
...
void BrowserAccessibilityManager::LoadInlineTextBoxes(const BrowserAccessibility& node)
{
    if (!delegate_)
        return;
...
void BrowserAccessibilityManager::SetAccessibilityFocus(const BrowserAccessibility& node)
{
    if (!delegate_)
        return;
...
void BrowserAccessibilityManager::ClearAccessibilityFocus(const BrowserAccessibility& node)
{
    if (!delegate_)
        return;
...
void BrowserAccessibilityManager::HitTest(const gfx::Point& frame_point, int request_id) const
{
    if (!delegate_)
        return;

    delegate_->AccessibilityHitTest(frame_point, ax::mojom::Event::kHover, request_id,
        /*opt_callback=*/ {});
    AXPlatform::GetInstance().NotifyAccessibilityApiUsage();
}

gfx::Rect BrowserAccessibilityManager::GetViewBoundsInScreenCoordinates() const
{
    AXPlatformTreeManagerDelegate* delegate = GetDelegateFromRootManager();
    if (delegate) {
...
BrowserAccessibility* BrowserAccessibilityManager::NextInTreeOrder(const BrowserAccessibility* object)
{
    if (!object)
        return nullptr;
...
BrowserAccessibility* BrowserAccessibilityManager::NextNonDescendantInTreeOrder(const BrowserAccessibility* object)
{
    if (!object)
        return nullptr;
...
BrowserAccessibility* BrowserAccessibilityManager::PreviousInTreeOrder(const BrowserAccessibility* object, bool can_wrap_to_last_element)
{
    if (!object)
        return nullptr;
    ...
    if (can_wrap_to_last_element && object->manager()->GetBrowserAccessibilityRoot() == object && object->PlatformChildCount() != 0) {
...
// static
BrowserAccessibility* BrowserAccessibilityManager::PreviousTextOnlyObject(const BrowserAccessibility* object)
{
...
// static
BrowserAccessibility* BrowserAccessibilityManager::NextTextOnlyObject(const BrowserAccessibility* object)
{
...
bool BrowserAccessibilityManager::FindIndicesInCommonParent(
    const BrowserAccessibility& object1, const BrowserAccessibility& object2, BrowserAccessibility** common_parent, size_t* child_index1, size_t* child_index2)
{
...
// static
ax::mojom::TreeOrder BrowserAccessibilityManager::CompareNodes(const BrowserAccessibility& object1, const BrowserAccessibility& object2)
{
    if (&object1 == &object2)
    ...
    size_t child_index1;
    size_t child_index2;
    if (FindIndicesInCommonParent(object1, object2, &common_parent, &child_index1, &child_index2)) {
...
std::vector<const BrowserAccessibility*> BrowserAccessibilityManager::FindTextOnlyObjectsInRange(
    const BrowserAccessibility& start_object, const BrowserAccessibility& end_object)
{
    ...
    if (&start_object != &end_object) {
        BrowserAccessibility* common_parent;
        if (!FindIndicesInCommonParent(start_object, end_object, &common_parent, &child_index1, &child_index2)) {
...
// static
std::u16string BrowserAccessibilityManager::GetTextForRange(const BrowserAccessibility& start_object, const BrowserAccessibility& end_object)
{
    return GetTextForRange(start_object, 0, end_object, end_object.GetTextContentUTF16().length());
}

// static
std::u16string BrowserAccessibilityManager::GetTextForRange(
    const BrowserAccessibility& start_object, int start_offset, const BrowserAccessibility& end_object, int end_offset)
{
    ...
    if (&start_object == &end_object && start_object.IsAtomicTextField()) {
        ...
        if (start_offset >= static_cast<int>(start_object.GetTextContentUTF16().length())
            || end_offset > static_cast<int>(start_object.GetTextContentUTF16().length())) {
            return std::u16string();
        }

        return start_object.GetTextContentUTF16().substr(start_offset, end_offset - start_offset);
    }
    ...
    if (text_only_objects.size() == 1) {
        ...
        if (start_offset < static_cast<int>(text_object->GetTextContentUTF16().length())
            && end_offset <= static_cast<int>(text_object->GetTextContentUTF16().length())) {
            return text_object->GetTextContentUTF16().substr(start_offset, end_offset - start_offset);
        }
    ...
    if (start_offset < static_cast<int>(start_text_object->GetTextContentUTF16().length())) {
    ...
    if (end_offset <= static_cast<int>(end_text_object->GetTextContentUTF16().length())) {
    ...
    const BrowserAccessibility& start_object, int start_offset, const BrowserAccessibility& end_object, int end_offset)
{
    ...
    if (&start_object == &end_object && start_object.IsAtomicTextField()) {
        ...
        return start_object.GetUnclippedRootFrameInnerTextRangeBoundsRect(start_offset, end_offset);
    }
    ...
    do {
        if (current->IsText()) {
            ...
            result.Union(current->GetUnclippedRootFrameInnerTextRangeBoundsRect(start_char_index, end_char_index));
        } else {
...
void BrowserAccessibilityManager::OnNodeCreated(AXTree* tree, AXNode* node)
{
    ...
    DCHECK(tree->GetFromId(node->id()) || node->IsGenerated()) << "Node must be in AXTree's map, unless it's an ExtraMacNode.";
...
void BrowserAccessibilityManager::OnNodeReparented(AXTree* tree, AXNode* node)
{
    ...
    SANITIZER_CHECK(iter != id_wrapper_map_.end()) << "Missing BrowserAccessibility* for node: " << *node << "\nTree: " << tree->ToString(/*verbose*/ false);
    if (iter == id_wrapper_map_.end()) {
        return;
    }
...
void BrowserAccessibilityManager::OnAtomicUpdateStarting(
    AXTree* tree, const std::set<AXNodeID>& deleted_node_ids, const std::set<AXNodeID>& reparented_node_ids)
{
...
void BrowserAccessibilityManager::OnAtomicUpdateFinished(AXTree* tree, bool root_changed, const std::vector<AXTreeObserver::Change>& changes)
{
...
AXPlatformNode* BrowserAccessibilityManager::GetPlatformNodeFromTree(AXNodeID node_id) const
{
...
AXPlatformNode* BrowserAccessibilityManager::GetPlatformNodeFromTree(const AXNode& node) const
{
...
BrowserAccessibilityManager* BrowserAccessibilityManager::GetManagerForRootFrame() const
{
    if (IsRootFrameManager())
        return const_cast<BrowserAccessibilityManager*>(this);

    BrowserAccessibilityManager* parent_manager = static_cast<BrowserAccessibilityManager*>(GetParentManager());
    if (!parent_manager) {
...
AXPlatformTreeManagerDelegate* BrowserAccessibilityManager::GetDelegateFromRootManager() const
{
    BrowserAccessibilityManager* root_manager = GetManagerForRootFrame();
    if (root_manager)
...
bool BrowserAccessibilityManager::IsRootFrameManager() const
{
    ...
    DCHECK(!is_root_tree || GetParentTreeID() == AXTreeIDUnknown()) << "Root tree has parent tree id of: " << GetParentTreeID();
    return is_root_tree;
}

AXTreeUpdate BrowserAccessibilityManager::SnapshotAXTreeForTesting()
{
    std::unique_ptr<AXTreeSource<const AXNode*, AXTreeData*, AXNodeData>> tree_source(ax_serializable_tree()->CreateTreeSource());
    AXTreeSerializer<const AXNode*, std::vector<const AXNode*>, AXTreeUpdate*, AXTreeData*, AXNodeData> serializer(tree_source.get());
    AXTreeUpdate update;
    serializer.SerializeChanges(GetRoot(), &update);
    return update;
}

void BrowserAccessibilityManager::UseCustomDeviceScaleFactorForTesting(float device_scale_factor)
{
    use_custom_device_scale_factor_for_testing_ = true;
    device_scale_factor_ = device_scale_factor;
}

BrowserAccessibility* BrowserAccessibilityManager::CachingAsyncHitTest(const gfx::Point& physical_pixel_point) const
{
    ...
    if (delegate_) {
        ...
        // http://www.chromium.org/developers/design-documents/blink-coordinate-spaces
        gfx::Point viewport_point = blink_screen_point - screen_view_bounds.OffsetFromOrigin();
        gfx::Point frame_point = gfx::ScaleToRoundedPoint(viewport_point, 1.0f / page_scale_factor_);
...
BrowserAccessibility* BrowserAccessibilityManager::ApproximateHitTest(const gfx::Point& blink_screen_point) const
{
...
void BrowserAccessibilityManager::BuildAXTreeHitTestCache()
{
    ...
    cached_node_rtree_->Build(
        storage.size(), [&storage](size_t index) { return storage[index]->GetUnclippedRootFrameBoundsRect(); },
        [&storage](size_t index) { return storage[index]->GetId(); });
}

void BrowserAccessibilityManager::BuildAXTreeHitTestCacheInternal(const BrowserAccessibility* node, std::vector<const BrowserAccessibility*>* storage)
{
...
BrowserAccessibility* BrowserAccessibilityManager::AXTreeHitTest(const gfx::Point& blink_screen_point) const
{
    ...
    cached_node_rtree_->Search(gfx::Rect(blink_screen_point.x(), blink_screen_point.y(), /*width=*/1,
                                   /*height=*/1),
        &results, &rects);
    ...
    auto rit = std::min_element(rects.rbegin(), rects.rend(), [](const gfx::Rect& a, const gfx::Rect& b) { return a.size().Area64() < b.size().Area64(); });
    return GetFromID(results[std::distance(rects.begin(), rit.base()) - 1]);
}

void BrowserAccessibilityManager::CacheHitTestResult(BrowserAccessibility* hit_test_result) const
{
...
void BrowserAccessibilityManager::CollectChangedNodesAndParentsForAtomicUpdate(
    AXTree* tree, const std::vector<AXTreeObserver::Change>& changes, std::set<AXPlatformNode*>* nodes_needing_update)
{
    ...
    for (const auto& change : changes) {
        ...
        while (editable_root->parent() && editable_root->parent()->HasState(ax::mojom::State::kEditable)) {
...
bool BrowserAccessibilityManager::ShouldFireEventForNode(BrowserAccessibility* node) const
{
    node = RetargetBrowserAccessibilityForEvents(node, RetargetEventType::RetargetEventTypeGenerated);
...
std::unique_ptr<BrowserAccessibility> BrowserAccessibilityManager::CreateBrowserAccessibility(AXNode* node)
{
...
BrowserAccessibility* BrowserAccessibilityManager::RetargetBrowserAccessibilityForEvents(BrowserAccessibility* node, RetargetEventType event_type) const
{
    if (!node) {
...
AXPlatformNodeId BrowserAccessibilityManager::GetNodeUniqueId(const BrowserAccessibility* node)
{
