...
const uint32_t kScreenReaderAccessibilityMode = ui::AXMode::kNativeAPIs | ui::AXMode::kWebContents | ui::AXMode::kScreenReader;
...
BrowserAccessibilityComWin::UpdateState::UpdateState(std::unique_ptr<WinAttributes> old_win_attributes, AXLegacyHypertext old_hypertext)
...
{
    TRACE_EVENT("accessibility", "OnReferenced", perfetto::Flow::FromPointer(this));
}
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_appName(BSTR* app_name)
{
    ...
    if (!app_name)
        return E_INVALIDARG;
    *app_name = SysAllocString(base::UTF8ToWide(AXPlatform::GetInstance().GetProductName()).c_str());
    DCHECK(*app_name);
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_appVersion(BSTR* app_version)
{
    ...
    *app_version = SysAllocString(base::UTF8ToWide(AXPlatform::GetInstance().GetProductVersion()).c_str());
    DCHECK(*app_version);
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_toolkitVersion(BSTR* toolkit_version)
{
    ...
    *toolkit_version = SysAllocString(base::UTF8ToWide(AXPlatform::GetInstance().GetToolkitVersion()).c_str());
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_imagePosition(IA2CoordinateType coordinate_type, LONG* x, LONG* y)
{
    ...
    if (coordinate_type == IA2_COORDTYPE_SCREEN_RELATIVE) {
        ...
        *y = bounds.y();
        ...
        gfx::Rect parent_bounds = GetOwner()->PlatformGetParent() ? GetOwner()->PlatformGetParent()->GetClippedRootFrameBoundsRect() : gfx::Rect();
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_imageSize(LONG* height, LONG* width)
{
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_characterExtents(
    LONG offset, IA2CoordinateType coordinate_type, LONG* out_x, LONG* out_y, LONG* out_width, LONG* out_height)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_characterExtents");
    WIN_ACCESSIBILITY_API_HISTOGRAM(UMA_API_GET_CHARACTER_EXTENTS);
    AddAccessibilityModeFlags(kScreenReaderAccessibilityMode | AXMode::kInlineTextBoxes);
    if (!GetOwner()) {
        return E_FAIL;
    }
    ...
    if (coordinate_type == IA2_COORDTYPE_SCREEN_RELATIVE) {
        character_bounds = GetOwner()->GetScreenHypertextRangeBoundsRect(offset, 1, AXClippingBehavior::kUnclipped);
    } else if (coordinate_type == IA2_COORDTYPE_PARENT_RELATIVE) {
        character_bounds = GetOwner()->GetRootFrameHypertextRangeBoundsRect(offset, 1, AXClippingBehavior::kUnclipped);
        if (GetOwner()->PlatformGetParent()) {
            character_bounds -= GetOwner()->PlatformGetParent()->GetUnclippedRootFrameBoundsRect().OffsetFromOrigin();
        }
    } else {
        return E_INVALIDARG;
    }
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_selection(LONG selection_index, LONG* start_offset, LONG* end_offset)
{
    return AXPlatformNodeWin::get_selection(selection_index, start_offset, end_offset);
}

IFACEMETHODIMP BrowserAccessibilityComWin::get_text(LONG start_offset, LONG end_offset, BSTR* text)
{
    ...
    std::u16string substr = text_str.substr(start_offset, end_offset - start_offset);
    if (substr.empty())
        return S_FALSE;
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_newText(IA2TextSegment* new_text)
{
    ...
    ComputeHypertextRemovedAndInserted(update_state_->old_hypertext, &start, &old_len, &new_len);
    if (new_len == 0)
        return E_FAIL;
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_oldText(IA2TextSegment* old_text)
{
...
IFACEMETHODIMP BrowserAccessibilityComWin::scrollSubstringTo(LONG start_index, LONG end_index, IA2ScrollType scroll_type)
{
...
IFACEMETHODIMP BrowserAccessibilityComWin::scrollSubstringToPoint(LONG start_index, LONG end_index, IA2CoordinateType coordinate_type, LONG x, LONG y)
{
    ...
    gfx::Rect string_bounds = GetOwner()->GetRootFrameHypertextRangeBoundsRect(start_index, length, AXClippingBehavior::kUnclipped);
    string_bounds -= GetOwner()->GetUnclippedRootFrameBoundsRect().OffsetFromOrigin();
    x -= string_bounds.x();
    y -= string_bounds.y();
...
IFACEMETHODIMP BrowserAccessibilityComWin::setSelection(LONG selection_index, LONG start_offset, LONG end_offset)
{
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_attributes(LONG offset, LONG* start_offset, LONG* end_offset, BSTR* text_attributes)
{
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_nHyperlinks(LONG* hyperlink_count)
{
    ...
    *hyperlink_count = hypertext_.hyperlink_offset_to_index.size();
    ...
    DCHECK(!IsIframe(GetOwner()->GetRole()) || *hyperlink_count <= 1) << "iframes should have 1 hyperlink, unless the child document is "
                                                                         "destroyed/unloaded, in which case it should have 0";

    return S_OK;
}

IFACEMETHODIMP BrowserAccessibilityComWin::get_hyperlink(LONG index, IAccessibleHyperlink** hyperlink)
{
    ...
    *hyperlink = nullptr;
    ...
    if (!hyperlink || index < 0 || index >= static_cast<LONG>(hypertext_.hyperlinks.size())) {
        return E_INVALIDARG;
    }

    DCHECK(!IsIframe(GetOwner()->GetRole()) || index == 0) << "An iframe cannot have more than 1 hyperlink";
    ...
    if (!node) {
        ...
        LOG(FATAL) << "Hyperlink error:\n index=" << index << " nHyperLinks=" << hypertext_.hyperlinks.size() << " hyperlink_id=" << id
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_hyperlinkIndex(LONG char_index, LONG* hyperlink_index)
{
    ...
    if (char_index < 0 || char_index >= static_cast<LONG>(GetHypertext().size())) {
        return E_INVALIDARG;
    }

    std::map<int32_t, int32_t>::iterator it = hypertext_.hyperlink_offset_to_index.find(char_index);
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_anchor(LONG index, VARIANT* anchor)
{
...
// Currently, only text links are supported.
IFACEMETHODIMP BrowserAccessibilityComWin::get_anchorTarget(LONG index, VARIANT* anchor_target)
{
    ...
    BSTR target;
    if (!(MSAAState() & STATE_SYSTEM_LINKED) || FAILED(GetStringAttributeAsBstr(ax::mojom::StringAttribute::kUrl, &target))) {
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_startIndex(LONG* index)
{
    ...
    if (parent) {
        hypertext_offset = ToBrowserAccessibilityComWin(parent)->GetHypertextOffsetFromChild(this);
    }
...
IFACEMETHODIMP BrowserAccessibilityComWin::nActions(LONG* n_actions)
{
    ...
    *n_actions = static_cast<LONG>(GetOwner()->GetSupportedActions().size() + GetOwner()->GetIntListAttribute(ax::mojom::IntListAttribute::kActionsIds).size());
    return S_OK;
}
...
IFACEMETHODIMP BrowserAccessibilityComWin::doAction(LONG action_index)
{
    ...
    const std::vector<ax::mojom::Action> actions = GetOwner()->GetSupportedActions();
    const std::vector<int32_t>& aria_actions = GetOwner()->GetIntListAttribute(ax::mojom::IntListAttribute::kActionsIds);

    // Actions can be from Blink for the given markup, or from the aria-actions
    // attribute defined by the author.
    if (action_index < 0 || action_index >= static_cast<LONG>(actions.size() + aria_actions.size())) {
        return E_INVALIDARG;
    }
...
IFACEMETHODIMP
BrowserAccessibilityComWin::get_description(LONG action_index, BSTR* description)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_description");
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_keyBinding(LONG action_index, LONG n_max_bindings, BSTR** key_bindings, LONG* n_bindings)
{
    ...
    *n_bindings = 0;
    ...
    if (action_index != 0 || !GetOwner()->HasDefaultActionVerb()
        || !GetOwner()->GetString16Attribute(ax::mojom::StringAttribute::kAccessKey, &key_binding_string)) {
        return S_FALSE;
    }
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_name(LONG action_index, BSTR* name)
{
    ...
    if (action_index == 0 && GetOwner()->GetIntAttribute(ax::mojom::IntAttribute::kDefaultActionVerb, &action)) {
        action_verb = ui::ToString(static_cast<ax::mojom::DefaultActionVerb>(action));
        ...
        action_verb = html_id.empty() ? AXPlatformNodeBase::kAriaActionsPrefix : AXPlatformNodeBase::kAriaActionsPrefix + "#" + html_id;
    }
...
IFACEMETHODIMP
BrowserAccessibilityComWin::get_localizedName(LONG action_index, BSTR* localized_name)
{
    ...
    if (action_index < static_cast<LONG>(actions.size())) {
        if (!GetOwner()->GetIntAttribute(ax::mojom::IntAttribute::kDefaultActionVerb, &action) || action_index != 0) {
        ...
        action_verb = ToLocalizedString(static_cast<ax::mojom::DefaultActionVerb>(action));
    } else {
        // action_index refers to a position in the combined Blink actions and
        // aria-actions vector. To find the corresponding index in the aria_actions
        // vector, subtract the number of Blink actions.
    ...
    *localized_name = SysAllocString(base::as_wcstr(base::UTF8ToUTF16(action_verb)));
...
IFACEMETHODIMP
BrowserAccessibilityComWin::get_nameSpaceURIForID(SHORT name_space_id, BSTR* name_space_uri)
{
...
IFACEMETHODIMP
BrowserAccessibilityComWin::put_alternateViewMediaTypes(BSTR* comma_separated_media_types)
{
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_nodeInfo(
    BSTR* node_name, SHORT* name_space_id, BSTR* node_value, unsigned int* num_children, unsigned int* unique_id, USHORT* node_type)
{
    ...
    std::u16string tag;
    if (GetOwner()->GetString16Attribute(ax::mojom::StringAttribute::kHtmlTag, &tag)) {
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_attributes(
    USHORT max_attribs, BSTR* attrib_names, SHORT* name_space_id, BSTR* attrib_values, USHORT* num_attribs)
{
    WIN_ACCESSIBILITY_API_TRACE_EVENT("get_attributes");
...
#define ADD_ATTRIBUTE(name, value)                                                                                                                             \
    ...
    USHORT index = 0;
    if (int max_length = GetOwner()->GetIntAttribute(ax::mojom::IntAttribute::kMaxLength)) {
    ...
    if (GetOwner()->GetData().GetNameFrom() == ax::mojom::NameFrom::kAttribute && !ui::IsImage(GetOwner()->GetRole())) {
        ADD_ATTRIBUTE("aria-label", GetOwner()->GetStringAttribute(ax::mojom::StringAttribute::kName));
    }

    // For OmniPass—rebranded as Fiserv Verifast. See https://crbug.com/378908266.
    std::string type_attr = GetOwner()->GetStringAttribute(ax::mojom::StringAttribute::kInputType);
    if (!type_attr.empty()) {
        ADD_ATTRIBUTE("type", type_attr);
    }
    std::string value_attr = GetOwner()->GetStringAttribute(ax::mojom::StringAttribute::kValue);
    if (!value_attr.empty()) {
        ADD_ATTRIBUTE("value", value_attr);
    }
    std::string name_attr = GetOwner()->GetStringAttribute(ax::mojom::StringAttribute::kHtmlInputName);
    if (!name_attr.empty()) {
    ...
    std::string id_attr = GetOwner()->GetStringAttribute(ax::mojom::StringAttribute::kHtmlId);
    if (!id_attr.empty()) {
...
// Returns HTML attributes -- not text attributes!
IFACEMETHODIMP BrowserAccessibilityComWin::get_attributesForNames(USHORT num_attribs, BSTR* attrib_names, SHORT* name_space_id, BSTR* attrib_values)
{
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_computedStyle(
    USHORT max_style_properties, boolean use_alternate_view, BSTR* style_properties, BSTR* style_values, USHORT* num_style_properties)
{
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_computedStyleForProperties(
    USHORT num_style_properties, boolean use_alternate_view, BSTR* style_properties, BSTR* style_values)
{
    // JAWS/NVDA no longer use this (and we only supported "display" property,
    // which is also exposed via the "display" object attribute).
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_parentNode(ISimpleDOMNode** node)
{
    ...
    *node = ToBrowserAccessibilityComWin(GetOwner()->PlatformGetParent())->NewReference();
    return S_OK;
}

IFACEMETHODIMP BrowserAccessibilityComWin::get_firstChild(ISimpleDOMNode** node)
{
    ...
    *node = ToBrowserAccessibilityComWin(GetOwner()->PlatformGetFirstChild())->NewReference();
    return S_OK;
}

IFACEMETHODIMP BrowserAccessibilityComWin::get_lastChild(ISimpleDOMNode** node)
{
    ...
    *node = ToBrowserAccessibilityComWin(GetOwner()->PlatformGetLastChild())->NewReference();
    return S_OK;
}

IFACEMETHODIMP BrowserAccessibilityComWin::get_previousSibling(ISimpleDOMNode** node)
{
    ...
    *node = ToBrowserAccessibilityComWin(GetOwner()->InternalGetPreviousSibling())->NewReference();
    return S_OK;
}

IFACEMETHODIMP BrowserAccessibilityComWin::get_nextSibling(ISimpleDOMNode** node)
{
    ...
    if (!index_in_parent.has_value() || (index_in_parent.value() + 1) >= GetOwner()->PlatformGetParent()->InternalChildCount()) {
        *node = NULL;
        return S_FALSE;
    }

    *node = ToBrowserAccessibilityComWin(GetOwner()->InternalGetNextSibling())->NewReference();
    return S_OK;
}

IFACEMETHODIMP BrowserAccessibilityComWin::get_childAt(unsigned int child_index, ISimpleDOMNode** node)
{
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_innerHTML(BSTR* innerHTML)
{
    ...
    if (GetOwner()->GetRole() != ax::mojom::Role::kMath && GetOwner()->GetRole() != ax::mojom::Role::kMathMLMath) {
        // TODO(nektar): Make sure we only get calls for Math nodes.
        return E_NOTIMPL;
    }

    std::u16string inner_html = GetOwner()->GetString16Attribute(ax::mojom::StringAttribute::kMathContent);
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_clippedSubstringBounds(
    unsigned int start_index, unsigned int end_index, int* out_x, int* out_y, int* out_width, int* out_height)
{
    ...
    return get_unclippedSubstringBounds(start_index, end_index, out_x, out_y, out_width, out_height);
}
    ...
    unsigned int start_index, unsigned int end_index, int* out_x, int* out_y, int* out_width, int* out_height)
{
    ...
    gfx::Rect bounds = GetOwner()->GetScreenHypertextRangeBoundsRect(start_index, end_index - start_index, AXClippingBehavior::kUnclipped);
    *out_x = bounds.x();
    *out_y = bounds.y();
...
IFACEMETHODIMP BrowserAccessibilityComWin::scrollToSubstring(unsigned int start_index, unsigned int end_index)
{
    ...
    manager->ScrollToMakeVisible(
        *GetOwner(), GetOwner()->GetRootFrameHypertextRangeBoundsRect(start_index, end_index - start_index, AXClippingBehavior::kUnclipped));
...
IFACEMETHODIMP BrowserAccessibilityComWin::get_fontFamily(BSTR* font_family)
{
    ...
    *font_family = nullptr;
    ...
    std::u16string family = GetOwner()->GetInheritedString16Attribute(ax::mojom::StringAttribute::kFontFamily);
    if (family.empty())
        return S_FALSE;
...
IFACEMETHODIMP BrowserAccessibilityComWin::QueryService(REFGUID guid_service, REFIID riid, void** object)
{
    TRACE_EVENT("accessibility", "QueryService", perfetto::Flow::FromPointer(this), "guidService", base::WideToASCII(base::win::WStringFromGUID(guid_service)),
    ...
    if (guid_service == GUID_IAccessibleContentDocument) {
        ...
        while (node->PlatformGetParent())
            node = node->PlatformGetParent()->manager()->GetBrowserAccessibilityRoot();
        return ToBrowserAccessibilityComWin(node)->QueryInterface(IID_IAccessible2, object);
    }

    if (guid_service == IID_IAccessible || guid_service == IID_IAccessible2 || guid_service == IID_IAccessibleAction
        || guid_service == IID_IAccessibleApplication || guid_service == IID_IAccessibleHyperlink || guid_service == IID_IAccessibleHypertext
        || guid_service == IID_IAccessibleImage || guid_service == IID_IAccessibleTable || guid_service == IID_IAccessibleTable2
        || guid_service == IID_IAccessibleTableCell || guid_service == IID_IAccessibleText || guid_service == IID_IAccessibleTextSelectionContainer
        || guid_service == IID_IAccessibleValue || guid_service == IID_ISimpleDOMDocument || guid_service == IID_ISimpleDOMNode
...
// static
STDMETHODIMP BrowserAccessibilityComWin::InternalQueryInterface(void* this_ptr, const _ATL_INTMAP_ENTRY* entries, REFIID iid, void** object)
{
    BrowserAccessibilityComWin* accessibility = reinterpret_cast<BrowserAccessibilityComWin*>(this_ptr);
    ...
    return AXPlatformNodeWin::InternalQueryInterface(this_ptr, entries, iid, object);
}
...
void BrowserAccessibilityComWin::ComputeStylesIfNeeded()
{
    ...
    TextAttributeList default_attributes = AXPlatformNodeWin::ComputeTextAttributes();
    TextAttributeMap attributes_map = GetDelegate()->ComputeTextAttributeMap(default_attributes);
...
void BrowserAccessibilityComWin::UpdateStep1ComputeWinAttributes()
{
    ...
    update_state_ = std::make_unique<UpdateState>(std::move(win_attributes_), std::move(hypertext_));
    ...
    win_attributes_->description = base::UTF8ToWide(GetOwner()->GetStringAttribute(ax::mojom::StringAttribute::kDescription));
...
void BrowserAccessibilityComWin::UpdateStep3FireEvents()
{
    ...
    if (ignored
        || (old_win_attributes.ignored != ignored && !GetOwner()->GetData().IsContainedInActiveLiveRegion()
    ...
    if (old_win_attributes.ia_role != 0) {
        ...
        int sx = 0;
        int sy = 0;
        if (GetOwner()->GetIntAttribute(ax::mojom::IntAttribute::kScrollX, &sx) && GetOwner()->GetIntAttribute(ax::mojom::IntAttribute::kScrollY, &sy)) {
        ...
        if (name() == old_win_attributes.name || GetNameFrom() == ax::mojom::NameFrom::kContents) {
            size_t start, old_len, new_len;
            ComputeHypertextRemovedAndInserted(update_state_->old_hypertext, &start, &old_len, &new_len);
            if (old_len > 0) {
...
BrowserAccessibilityComWin* BrowserAccessibilityComWin::GetTargetFromChildID(const VARIANT& var_id)
{
    if (!GetOwner()) {
        return nullptr;
    }
    ...
    if (child_id >= 1 && child_id <= static_cast<LONG>(GetOwner()->PlatformChildCount())) {
        return ToBrowserAccessibilityComWin(GetOwner()->PlatformGetChild(child_id - 1));
    }

    auto* child = static_cast<BrowserAccessibilityComWin*>(AXPlatformNodeWin::GetFromUniqueId(-child_id));
...
HRESULT BrowserAccessibilityComWin::GetStringAttributeAsBstr(ax::mojom::StringAttribute attribute, BSTR* value_bstr)
{
    if (!GetOwner()) {
        return E_FAIL;
    }
...
void BrowserAccessibilityComWin::SetIA2HypertextSelection(LONG start_offset, LONG end_offset)
{
    HandleSpecialTextOffset(&start_offset);
    HandleSpecialTextOffset(&end_offset);
...
LONG BrowserAccessibilityComWin::FindStartOfStyle(LONG start_offset, ax::mojom::MoveDirection direction)
{
...
BrowserAccessibilityComWin* BrowserAccessibilityComWin::GetFromID(int32_t id) const
{
    if (!GetOwner()) {
        return nullptr;
    }
...
void BrowserAccessibilityComWin::FireNativeEvent(LONG win_event_type) const
{
    ...
    if (GetOwner()->IsChildOfLeaf() && !GetOwner()->GetCollapsedMenuListSelectAncestor()) {
        return;
    }

    Manager()->ToBrowserAccessibilityManagerWin()->FireWinAccessibilityEvent(win_event_type, GetOwner());
}

BrowserAccessibilityComWin* ToBrowserAccessibilityComWin(BrowserAccessibility* obj)
{
    if (!obj)
        return nullptr;
