...
struct COMPONENT_EXPORT(AX_PLATFORM) AXTextEdit {
    AXTextEdit();
    AXTextEdit(std::u16string inserted_text, std::u16string deleted_text, id edit_text_marker);
    AXTextEdit(const AXTextEdit& other);
    ~AXTextEdit();

    bool IsEmpty() const
    {
        return inserted_text.empty() && deleted_text.empty();
    }
...
- (instancetype)initWithObject:(ui::BrowserAccessibility*)accessibility withPlatformNode:(ui::AXPlatformNodeMac*)platform_node;
...
- (bool)findRowIndex:(BrowserAccessibilityCocoa*)toFind withCurrentIndex:(int*)currentIndex;
