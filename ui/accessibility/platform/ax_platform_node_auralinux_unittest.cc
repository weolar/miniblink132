...
public:
    AXPlatformNodeAuraLinuxTest()
        : ax_mode_setter_(kAXModeComplete)
    {
    }
...
protected:
    ...
    {
        TestAXNodeWrapper* wrapper = TestAXNodeWrapper::GetOrCreate(GetTree(), node);
        if (!wrapper)
            return nullptr;
    ...
    AtkObject* GetRootAtkObject()
    {
        return AtkObjectFromNode(GetRoot());
    }
...
static void EnsureAtkObjectHasAttributeWithValue(AtkObject* atk_object, const gchar* attribute_name, const gchar* attribute_value)
{
...
static void EnsureAtkObjectDoesNotHaveAttribute(AtkObject* atk_object, const gchar* attribute_name)
{
    AtkAttributeSet* attributes = atk_object_get_attributes(atk_object);
    AtkAttributeSet* current = attributes;
    while (current) {
...
static void SetStringAttributeOnNode(
    AXNode* ax_node, ax::mojom::StringAttribute attribute, const char* attribute_value, std::optional<ax::mojom::Role> role = std::nullopt)
{
...
static void TestAtkObjectIntAttribute(AXNode* ax_node, AtkObject* atk_object, ax::mojom::IntAttribute mojom_attribute, const gchar* attribute_name,
    std::optional<ax::mojom::Role> role = std::nullopt)
{
    ...
    for (const auto& test : tests) {
        ...
        EnsureAtkObjectHasAttributeWithValue(atk_object, attribute_name, test.second);
    }
}

static void TestAtkObjectStringAttribute(AXNode* ax_node, AtkObject* atk_object, ax::mojom::StringAttribute mojom_attribute, const gchar* attribute_name,
    std::optional<ax::mojom::Role> role = std::nullopt)
{
    AXNodeData new_data = AXNodeData();
    new_data.role = role.value_or(ax::mojom::Role::kApplication);
...
static void TestAtkObjectBoolAttribute(AXNode* ax_node, AtkObject* atk_object, ax::mojom::BoolAttribute mojom_attribute, const gchar* attribute_name,
    std::optional<ax::mojom::Role> role = std::nullopt)
{
    AXNodeData new_data = AXNodeData();
    new_data.role = role.value_or(ax::mojom::Role::kApplication);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkObjectState)
{
    ...
    ASSERT_FALSE(atk_state_set_contains_state(state_set, ATK_STATE_INVALID_ENTRY));
    ...
    ASSERT_FALSE(atk_state_set_contains_state(state_set, ATK_STATE_MULTISELECTABLE));
    ...
    ASSERT_FALSE(atk_state_set_contains_state(state_set, ATK_STATE_SELECTABLE_TEXT));
    ...
    ASSERT_FALSE(atk_state_set_contains_state(state_set, ATK_STATE_SUPPORTS_AUTOCOMPLETION));
    ...
    ASSERT_TRUE(atk_state_set_contains_state(state_set, ATK_STATE_MULTISELECTABLE));
    ASSERT_TRUE(atk_state_set_contains_state(state_set, ATK_STATE_REQUIRED));
    ASSERT_TRUE(atk_state_set_contains_state(state_set, ATK_STATE_SUPPORTS_AUTOCOMPLETION));
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkObjectStringAttributes)
{
    ...
    std::pair<ax::mojom::StringAttribute, const char*> tests[] = {
        ...
        std::make_pair(ax::mojom::StringAttribute::kRoleDescription, "roledescription"),
        ...
        std::make_pair(ax::mojom::StringAttribute::kContainerLiveStatus, "container-live"),
        std::make_pair(ax::mojom::StringAttribute::kContainerLiveRelevant, "container-relevant"),
    };

    for (const auto& test : tests) {
        TestAtkObjectStringAttribute(root_node, root_atk_object, test.first, test.second);
    }
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkObjectBoolAttributes)
{
    ...
    std::pair<ax::mojom::BoolAttribute, const char*> tests[] = {
        ...
        std::make_pair(ax::mojom::BoolAttribute::kContainerLiveAtomic, "container-atomic"),
        std::make_pair(ax::mojom::BoolAttribute::kContainerLiveBusy, "container-busy"),
    };

    for (const auto& test : tests) {
        TestAtkObjectBoolAttribute(root_node, root_atk_object, test.first, test.second);
    }
...
TEST_F(AXPlatformNodeAuraLinuxTest, DISABLED_TestAtkObjectIntAttributes)
{
    ...
    TestAtkObjectIntAttribute(root_node, root_atk_object, ax::mojom::IntAttribute::kHierarchicalLevel, "level");
    TestAtkObjectIntAttribute(root_node, root_atk_object, ax::mojom::IntAttribute::kAriaColumnCount, "colcount", ax::mojom::Role::kTable);
    TestAtkObjectIntAttribute(root_node, root_atk_object, ax::mojom::IntAttribute::kAriaColumnCount, "colcount", ax::mojom::Role::kGrid);
    TestAtkObjectIntAttribute(root_node, root_atk_object, ax::mojom::IntAttribute::kAriaColumnCount, "colcount", ax::mojom::Role::kTreeGrid);

    TestAtkObjectIntAttribute(root_node, root_atk_object, ax::mojom::IntAttribute::kAriaRowCount, "rowcount", ax::mojom::Role::kTable);
    TestAtkObjectIntAttribute(root_node, root_atk_object, ax::mojom::IntAttribute::kAriaRowCount, "rowcount", ax::mojom::Role::kGrid);
    TestAtkObjectIntAttribute(root_node, root_atk_object, ax::mojom::IntAttribute::kAriaRowCount, "rowcount", ax::mojom::Role::kTreeGrid);

    TestAtkObjectIntAttribute(root_node, root_atk_object, ax::mojom::IntAttribute::kAriaCellColumnIndex, "colindex", ax::mojom::Role::kCell);
    TestAtkObjectIntAttribute(root_node, root_atk_object, ax::mojom::IntAttribute::kAriaCellRowIndex, "rowindex", ax::mojom::Role::kCell);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkComponentRefAtPoint)
{
    ...
    AtkObject* child_obj = atk_component_ref_accessible_at_point(ATK_COMPONENT(root_obj), 50, 50, ATK_XY_SCREEN);
    EXPECT_EQ(nullptr, child_obj);

    // this is directly on node 1.
    child_obj = atk_component_ref_accessible_at_point(ATK_COMPONENT(root_obj), 5, 5, ATK_XY_SCREEN);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkComponentsGetExtentsPositionSize)
{
    ...
    atk_component_get_extents(ATK_COMPONENT(root_obj), &x_left, &y_top, &width, &height, ATK_XY_SCREEN);
    EXPECT_EQ(110, x_left);
    EXPECT_EQ(240, y_top);
    EXPECT_EQ(800, width);
    EXPECT_EQ(600, height);

    AtkObject* hit_test_result = atk_component_ref_accessible_at_point(ATK_COMPONENT(root_obj), x_left, y_top, ATK_XY_SCREEN);
    ASSERT_EQ(hit_test_result, root_obj);
    g_object_unref(hit_test_result);

    atk_component_get_position(ATK_COMPONENT(root_obj), &x_left, &y_top, ATK_XY_SCREEN);
    EXPECT_EQ(110, x_left);
    EXPECT_EQ(240, y_top);

    atk_component_get_extents(ATK_COMPONENT(root_obj), &x_left, &y_top, &width, &height, ATK_XY_WINDOW);
    EXPECT_EQ(0, x_left);
    EXPECT_EQ(0, y_top);
    EXPECT_EQ(800, width);
    EXPECT_EQ(600, height);

    hit_test_result = atk_component_ref_accessible_at_point(ATK_COMPONENT(root_obj), x_left + 2, y_top + 2, ATK_XY_WINDOW);
    ASSERT_EQ(hit_test_result, root_obj);
    g_object_unref(hit_test_result);

    atk_component_get_position(ATK_COMPONENT(root_obj), &x_left, &y_top, ATK_XY_WINDOW);
    EXPECT_EQ(0, x_left);
    EXPECT_EQ(0, y_top);
    ...
    atk_component_get_extents(ATK_COMPONENT(child_obj), &x_left, &y_top, &width, &height, ATK_XY_SCREEN);
    EXPECT_EQ(200, x_left);
    EXPECT_EQ(350, y_top);
    EXPECT_EQ(200, width);
    EXPECT_EQ(200, height);

    hit_test_result = atk_component_ref_accessible_at_point(ATK_COMPONENT(root_obj), x_left, y_top, ATK_XY_SCREEN);
    ASSERT_EQ(hit_test_result, child_obj);
    g_object_unref(hit_test_result);

    atk_component_get_extents(ATK_COMPONENT(child_obj), &x_left, &y_top, &width, &height, ATK_XY_WINDOW);
    EXPECT_EQ(90, x_left);
    EXPECT_EQ(110, y_top);
    EXPECT_EQ(200, width);
    EXPECT_EQ(200, height);

    hit_test_result = atk_component_ref_accessible_at_point(ATK_COMPONENT(root_obj), x_left, y_top, ATK_XY_WINDOW);
    ASSERT_EQ(hit_test_result, child_obj);
    g_object_unref(hit_test_result);

    atk_component_get_extents(ATK_COMPONENT(child_obj), nullptr, &y_top, &width, &height, ATK_XY_SCREEN);
    EXPECT_EQ(200, height);
    atk_component_get_extents(ATK_COMPONENT(child_obj), &x_left, nullptr, &width, &height, ATK_XY_SCREEN);
    EXPECT_EQ(200, x_left);
    atk_component_get_extents(ATK_COMPONENT(child_obj), &x_left, &y_top, nullptr, &height, ATK_XY_SCREEN);
    EXPECT_EQ(350, y_top);
    atk_component_get_extents(ATK_COMPONENT(child_obj), &x_left, &y_top, &width, nullptr, ATK_XY_SCREEN);
    EXPECT_EQ(200, width);
...
typedef bool (*ScrollToPointFunc)(AtkComponent* component, AtkCoordType coords, gint x, gint y);
...
TEST_F(AXPlatformNodeAuraLinuxTest, AtkComponentScrollToPoint)
{
    ...
    atk_component_scroll_to_point(ATK_COMPONENT(child_obj), ATK_XY_SCREEN, 600, 650);
    atk_component_get_extents(ATK_COMPONENT(child_obj), &x_left, &y_top, &width, &height, ATK_XY_SCREEN);
    ...
    atk_component_scroll_to_point(ATK_COMPONENT(child_obj), ATK_XY_PARENT, 10, 10);
    atk_component_get_extents(ATK_COMPONENT(child_obj), &x_left, &y_top, &width, &height, ATK_XY_SCREEN);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkActionAriaAction)
{
    ...
    const gchar* action_localized_name = atk_action_get_localized_name(ATK_ACTION(root_obj), 2);
    EXPECT_STREQ("custom", action_name);
    EXPECT_STREQ("close", action_localized_name);
    EXPECT_TRUE(atk_action_do_action(ATK_ACTION(root_obj), 2));
    EXPECT_EQ(GetRoot()->GetChildAtIndex(0), TestAXNodeWrapper::GetNodeFromLastDefaultAction());

    // The fourth action refers to child2.
    action_name = atk_action_get_name(ATK_ACTION(root_obj), 3);
    action_localized_name = atk_action_get_localized_name(ATK_ACTION(root_obj), 3);
    EXPECT_STREQ("custom#open-button", action_name);
    EXPECT_STREQ("open", action_localized_name);
    EXPECT_TRUE(atk_action_do_action(ATK_ACTION(root_obj), 3));
    EXPECT_EQ(GetRoot()->GetChildAtIndex(1), TestAXNodeWrapper::GetNodeFromLastDefaultAction());
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkValueChangedSignal)
{
    ...
    g_signal_connect(root_object, "property-change::accessible-value",
        G_CALLBACK(+[](AtkObject*, void* property, bool* saw_value_change) { *saw_value_change = true; }), &saw_value_change);
    ...
    GetRootPlatformNode()->NotifyAccessibilityEvent(ax::mojom::Event::kValueChanged);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkHyperlink)
{
    ...
    AtkHyperlink* hyperlink(atk_hyperlink_impl_get_hyperlink(ATK_HYPERLINK_IMPL(root_obj)));
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkTextCharacterGranularity)
{
    ...
    EXPECT_EQ(static_cast<gunichar>('d'), atk_text_get_character_at_offset(atk_text, 2));
    ...
    EXPECT_EQ(static_cast<gunichar>(' '), atk_text_get_character_at_offset(atk_text, 24));

    auto verify_text = [&](const char* expected_text, char* text, int expected_start, int expected_end, int start, int end) {
    ...
    auto verify_text_at_offset = [&](const char* expected_text, int offset, int expected_start, int expected_end) {
        ::testing::Message message;
        message << "While checking at offset " << offset;
        SCOPED_TRACE(message);

        int start = 0, end = 0;
        char* text = atk_text_get_text_at_offset(atk_text, offset, ATK_TEXT_BOUNDARY_CHAR, &start, &end);
    ...
    auto verify_text_after_offset = [&](const char* expected_text, int offset, int expected_start, int expected_end) {
        ::testing::Message message;
        message << "While checking after offset " << offset;
        SCOPED_TRACE(message);

        int start = 0, end = 0;
        char* text = atk_text_get_text_after_offset(atk_text, offset, ATK_TEXT_BOUNDARY_CHAR, &start, &end);
        verify_text(expected_text, text, expected_start, expected_end, start, end);
    };
    ...
    auto verify_text_before_offset = [&](const char* expected_text, int offset, int expected_start, int expected_end) {
        ::testing::Message message;
        message << "While checking before offset " << offset;
        SCOPED_TRACE(message);

        int start = 0, end = 0;
        char* text = atk_text_get_text_before_offset(atk_text, offset, ATK_TEXT_BOUNDARY_CHAR, &start, &end);
        verify_text(expected_text, text, expected_start, expected_end, start, end);
    };
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkTextWordGranularity)
{
    ...
    root.id = 1;
    root.role = ax::mojom::Role::kTextField;
    root.AddStringAttribute(ax::mojom::StringAttribute::kValue, "A decently long string.");
    Init(root);
    ...
    static GetTextSegmentTest tests[] = { { 0, "A ", 0, 2 }, { 2, "decently ", 2, 11 }, { -1, nullptr, -1, -1 }, { 1000, nullptr, -1, -1 } };

    for (const auto& test : tests) {
        ::testing::Message message;
        message << "While checking at index " << test.offset << " for \'" << test.content << "\' at " << test.start_offset << '-' << test.end_offset << '.';
        SCOPED_TRACE(message);

        int start_offset = -1, end_offset = -1;
        char* content = atk_text_get_text_at_offset(atk_text, test.offset, ATK_TEXT_BOUNDARY_WORD_START, &start_offset, &end_offset);
    ...
    for (const auto& test : tests) {
        ...
        int start_offset = -1, end_offset = -1;
        char* content = atk_text_get_string_at_offset(atk_text, test.offset, ATK_TEXT_GRANULARITY_WORD, &start_offset, &end_offset);
        ASSERT_STREQ(content, test.content) << "with test offset=" << test.offset;
        ASSERT_EQ(start_offset, test.start_offset) << "with test offset=" << test.offset;
        ASSERT_EQ(end_offset, test.end_offset) << "with test offset=" << test.offset;
        g_free(content);
    }
#endif
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkTextSentenceGranularity)
{
    ...
    root.id = 1;
    root.role = ax::mojom::Role::kTextField;
    root.AddStringAttribute(ax::mojom::StringAttribute::kValue, "A short sentence. Another sentence.     A third...");
    Init(root);
    ...
    for (const auto& test : tests) {
        ...
        int start_offset = -1, end_offset = -1;
        char* content = atk_text_get_text_at_offset(atk_text, test.offset, ATK_TEXT_BOUNDARY_SENTENCE_START, &start_offset, &end_offset);
    ...
    for (const auto& test : tests) {
        ...
        int start_offset = -1, end_offset = -1;
        char* content = atk_text_get_string_at_offset(atk_text, test.offset, ATK_TEXT_GRANULARITY_SENTENCE, &start_offset, &end_offset);
        ASSERT_STREQ(content, test.content);
        ASSERT_EQ(start_offset, test.start_offset);
        ASSERT_EQ(end_offset, test.end_offset);
        g_free(content);
...
TEST_F(AXPlatformNodeAuraLinuxTest, DISABLED_TestAtkTextParagraphGranularity)
{
    ...
    root.id = 1;
    root.role = ax::mojom::Role::kTextField;
    root.AddStringAttribute(ax::mojom::StringAttribute::kValue, "A short paragraph. \nAnother paragraph.\nA third...");
    Init(root);
    ...
    for (const auto& test : tests) {
        int start_offset = -1, end_offset = -1;
        char* content = atk_text_get_string_at_offset(atk_text, test.offset, ATK_TEXT_GRANULARITY_PARAGRAPH, &start_offset, &end_offset);
        ASSERT_STREQ(content, test.content) << "with test offset=" << test.offset;
        ASSERT_EQ(start_offset, test.start_offset) << "with test offset=" << test.offset;
        ASSERT_EQ(end_offset, test.end_offset) << "with test offset=" << test.offset;
        g_free(content);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkTextWithNonBMPCharacters)
{
    ...
    static const char root_text[] = "\xF0\x9F\x83\x8f a decently long \xF0\x9F\x83\x8f string "
                                    "\xF0\x9F\x83\x8f.";
    ...
    for (int i = 0; i < root_text_length; i++) {
        ...
        gunichar expected_character = g_utf8_get_char_validated(g_utf8_offset_to_pointer(root_text, i), -1);
        ASSERT_EQ(character, expected_character);

        int start_offset = -1, end_offset = -1;
        char* char_string = atk_text_get_text_at_offset(atk_text, i, ATK_TEXT_BOUNDARY_CHAR, &start_offset, &end_offset);
        ...
        start_offset = -1;
        end_offset = -1;
        char_string = atk_text_get_string_at_offset(atk_text, i, ATK_TEXT_GRANULARITY_CHAR, &start_offset, &end_offset);
    ...
    static GetTextSegmentTest tests[] = { { 0, "\xF0\x9F\x83\x8f ", 0, 2 }, { 6, "decently ", 4, 13 } };

    for (const auto& test : tests) {
        int start_offset = -1, end_offset = -1;
        char* word = atk_text_get_text_at_offset(atk_text, test.offset, ATK_TEXT_BOUNDARY_WORD_START, &start_offset, &end_offset);
        ::testing::Message message;
        message << "Checking test with index=" << test.offset << " and expected text=\'" << test.content << "\' at " << tests[1].start_offset << '-'
                << tests[1].end_offset << '.';
        SCOPED_TRACE(message);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkTextCaretMoved)
{
    ...
    g_signal_connect(atk_text, "text-caret-moved", G_CALLBACK(+[](AtkText*, int new_position, gpointer data) {
    ...
    GetRootPlatformNode()->NotifyAccessibilityEvent(ax::mojom::Event::kTextSelectionChanged);
...
public:
    explicit ActivationTester(AtkObject* target)
        : target_(target)
    {
        auto callback = G_CALLBACK(+[](AtkWindow*, bool* flag) { *flag = true; });
        activate_id_ = g_signal_connect(target, "activate", callback, &saw_activate_);
        deactivate_id_ = g_signal_connect(target, "deactivate", callback, &saw_deactivate_);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkWindowActive)
{
    ...
    GetPlatformNode(checkbox_node)->NotifyAccessibilityEvent(ax::mojom::Event::kFocus);

    bool saw_active_focus_state_change = false;
    g_signal_connect(checkbox_atk_obj, "state-change", G_CALLBACK(+[](AtkObject* atkobject, gchar* state_changed, gboolean new_value, bool* flag) {
...

    {
        ...
        static_cast<AXPlatformNodeAuraLinux*>(GetRootPlatformNode())->NotifyAccessibilityEvent(ax::mojom::Event::kWindowActivated);
...

    {
        ...
        ActivationTester tester(root_atk_object);
        static_cast<AXPlatformNodeAuraLinux*>(GetRootPlatformNode())->NotifyAccessibilityEvent(ax::mojom::Event::kWindowDeactivated);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkWindowMinimized)
{
    ...
    g_signal_connect(root_atk_object, "state-change", G_CALLBACK(+[](AtkObject* atkobject, gchar* state_changed, gboolean new_value, bool* flag) {
    ...
    static_cast<AXPlatformNodeAuraLinux*>(root_node)->NotifyAccessibilityEvent(ax::mojom::Event::kWindowVisibilityChanged);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestFocusTriggersAtkWindowActive)
{
    ...
    {
        ActivationTester tester(root_atk_object);
        GetPlatformNode(child_node)->NotifyAccessibilityEvent(ax::mojom::Event::kFocus);
        EXPECT_FALSE(tester.saw_activate_);
        EXPECT_FALSE(tester.saw_deactivate_);
        EXPECT_FALSE(tester.IsActivatedInStateSet());
    }
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkPopupWindowActive)
{
    ...
    Init(root, window_node_data, document_node_data, menu_node_data, menu_item_data);
    ...
    g_signal_connect(document_atk_node, "focus-event", G_CALLBACK(+[](AtkObject* atkobject, gint focused, int* focus_events) {
        if (focused)
            *focus_events += 1;
    }),
    ...
    GetPlatformNode(window_node)->NotifyAccessibilityEvent(ax::mojom::Event::kWindowActivated);
    ...
    AtkObject* menu_atk_node(AtkObjectFromNode(menu_node));
    {
        ...
        GetPlatformNode(menu_node)->NotifyAccessibilityEvent(ax::mojom::Event::kMenuPopupStart);
        EXPECT_TRUE(tester.saw_activate_);
        EXPECT_FALSE(tester.saw_deactivate_);
        EXPECT_TRUE(tester.IsActivatedInStateSet());
    ...
    {
        ActivationTester tester(menu_atk_node);
        GetPlatformNode(menu_node)->NotifyAccessibilityEvent(ax::mojom::Event::kMenuPopupEnd);
        EXPECT_FALSE(tester.saw_activate_);
        EXPECT_TRUE(tester.saw_deactivate_);
        EXPECT_FALSE(tester.IsActivatedInStateSet());
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkObjectSetSizePosInSet)
{
    ...
    update.nodes[1].id = 2;
    update.nodes[1].role = ax::mojom::Role::kRadioButton; // kRadioButton posinset = 2, setsize = 5.
    update.nodes[1].AddIntAttribute(ax::mojom::IntAttribute::kPosInSet, 2);
    update.nodes[2].id = 3;
    update.nodes[2].role = ax::mojom::Role::kRadioButton; // kRadioButton posinset = 3, setsize = 5.
    update.nodes[3].id = 4;
    update.nodes[3].role = ax::mojom::Role::kRadioButton; // kRadioButton posinset = 5, stesize = 5
    ...
    // inferred.
    EnsureAtkObjectHasAttributeWithValue(radiobutton1_atk_object, "posinset", "2");
    EnsureAtkObjectHasAttributeWithValue(radiobutton1_atk_object, "setsize", "5");
    EnsureAtkObjectHasAttributeWithValue(radiobutton2_atk_object, "posinset", "3");
    EnsureAtkObjectHasAttributeWithValue(radiobutton2_atk_object, "setsize", "5");
    EnsureAtkObjectHasAttributeWithValue(radiobutton3_atk_object, "posinset", "5");
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkRelations)
{
    ...
    auto assert_contains_relation = [&](AtkObject* object, AtkObject* target, AtkRelationType relation) {
    ...
    if (PlatformSupportsRelation(ATK_RELATION_DETAILS)) {
        ...
        assert_contains_relation(atk_child1, root_atk_object, ATK_RELATION_DETAILS_FOR);
...
#endif
    assert_contains_relation(atk_child2, root_atk_object, ATK_RELATION_LABELLED_BY);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAllReverseAtkRelations)
{
    ...
    auto test_relation = [&](auto attribute_setter, AtkRelationType expected_relation, AtkRelationType expected_reverse_relation) {
        if (expected_relation > max_relation_type || expected_reverse_relation > max_relation_type)
            return;
        ...
        ASSERT_TRUE(atk_relation_set_contains_target(relations, expected_relation, target));
        g_object_unref(G_OBJECT(relations));

        relations = atk_object_ref_relation_set(target);
        ASSERT_TRUE(atk_relation_set_contains(relations, expected_reverse_relation));
        ASSERT_TRUE(atk_relation_set_contains_target(relations, expected_reverse_relation, source));
        g_object_unref(G_OBJECT(relations));
    };

    auto test_int_list_relation = [&](ax::mojom::IntListAttribute relation, AtkRelationType expected_relation, AtkRelationType expected_reverse_relation) {
    ...
    test_int_list_relation(ax::mojom::IntListAttribute::kDetailsIds, ATK_RELATION_DETAILS, ATK_RELATION_DETAILS_FOR);
    test_int_list_relation(ax::mojom::IntListAttribute::kErrormessageIds, ATK_RELATION_ERROR_MESSAGE, ATK_RELATION_ERROR_FOR);
    test_int_list_relation(ax::mojom::IntListAttribute::kControlsIds, ATK_RELATION_CONTROLLER_FOR, ATK_RELATION_CONTROLLED_BY);
    test_int_list_relation(ax::mojom::IntListAttribute::kDescribedbyIds, ATK_RELATION_DESCRIBED_BY, ATK_RELATION_DESCRIPTION_FOR);
    test_int_list_relation(ax::mojom::IntListAttribute::kFlowtoIds, ATK_RELATION_FLOWS_TO, ATK_RELATION_FLOWS_FROM);
    test_int_list_relation(ax::mojom::IntListAttribute::kLabelledbyIds, ATK_RELATION_LABELLED_BY, ATK_RELATION_LABEL_FOR);
}
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkRelationsTargetIndex)
{
    ...
    button1.AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, { 2, 3, 4 });
    root.child_ids.push_back(5);

    AXNodeData button2;
    button2.id = 6;
    button2.role = ax::mojom::Role::kButton;
    button2.AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, { 4, 3, 2 });
    root.child_ids.push_back(6);

    AXNodeData button3;
    button3.id = 7;
    button3.role = ax::mojom::Role::kButton;
    button3.AddIntListAttribute(ax::mojom::IntListAttribute::kLabelledbyIds, { 4, 4, 2, 2, 3 });
    root.child_ids.push_back(7);

    Init(root, label1, label2, label3, button1, button2, button3);

    auto test_index = [&](AtkObject* source, AtkObject* target, AtkRelationType relation_type, int index) {
        ...
        AtkRelation* relation = atk_relation_set_get_relation_by_type(relation_set, relation_type);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkTextContentEditableGetNSelectionsZero)
{
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkTextTextFieldSetSelection)
{
    ...
    g_signal_connect(atk_text, "text-selection-changed", G_CALLBACK(+[](AtkObject* atkobject, bool* flag) { *flag = true; }), &saw_selection_change);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkTextTextFieldGetSelection)
{
    ...
    gchar* selected_text = atk_text_get_selection(atk_text, 0, &selection_start, &selection_end);
    ...
    EXPECT_TRUE(atk_text_remove_selection(atk_text, 0));
    selected_text = atk_text_get_selection(atk_text, 0, &selection_start, &selection_end);
    ...
    selected_text = atk_text_get_selection(atk_text, 1, &selection_start, &selection_end);
    EXPECT_EQ(nullptr, selected_text);
    EXPECT_EQ(selection_start, 0);
    EXPECT_EQ(selection_end, 0);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkObjectExpandRebuildsPlatformNode)
{
    ...
    GetRootPlatformNode()->NotifyAccessibilityEvent(ax::mojom::Event::kExpandedChanged);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestReadonlyChanged)
{
    ...
    g_signal_connect(atk_object, "state-change", G_CALLBACK(+[](AtkObject* atkobject, gchar* state_changed, gboolean new_value, bool* flag) {
        if (!g_strcmp0(state_changed, "read-only"))
            *flag = new_value;
    }),
        &is_read_only);

    root_data.AddIntAttribute(ax::mojom::IntAttribute::kRestriction, static_cast<int32_t>(ax::mojom::Restriction::kReadOnly));
    root->SetData(root_data);
    node->OnReadonlyChanged();
    ASSERT_TRUE(is_read_only);

    root_data.AddIntAttribute(ax::mojom::IntAttribute::kRestriction, static_cast<int32_t>(ax::mojom::Restriction::kNone));
    root->SetData(root_data);
    node->OnReadonlyChanged();
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestAtkObjectParentChanged)
{
    ...
    g_signal_connect(atk_object, "property-change::accessible-parent",
        G_CALLBACK(+[](AtkObject*, void* property, bool* saw_parent_changed) { *saw_parent_changed = true; }), &saw_parent_changed);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestScrolledToAnchorEvent)
{
    ...
    g_signal_connect(
        atk_object, "text-caret-moved", G_CALLBACK(+[](AtkObject*, int position, bool* saw_caret_moved) { *saw_caret_moved = true; }), &saw_caret_moved);
...
TEST_F(AXPlatformNodeAuraLinuxTest, TestDialogActiveWhenChildFocused)
{
    ...
    g_signal_connect(dialog_obj, "state-change", G_CALLBACK(+[](AtkObject* atkobject, gchar* state_changed, gboolean new_value, bool* flag) {
    ...
    GetPlatformNode(entry_node)->NotifyAccessibilityEvent(ax::mojom::Event::kFocus);
    ...
    GetPlatformNode(outside_node)->NotifyAccessibilityEvent(ax::mojom::Event::kFocus);
    EXPECT_TRUE(saw_active_state_change);
