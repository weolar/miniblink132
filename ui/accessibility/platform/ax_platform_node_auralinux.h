...
struct FindInPageResultInfo {
    ...
    {
        return (node == other.node) && (start_offset == other.start_offset) && (end_offset == other.end_offset);
    }
};
...
class COMPONENT_EXPORT(AX_PLATFORM) AXPlatformNodeAuraLinux : public AXPlatformNodeBase {
public:
    ...
    void GetExtents(gint* x, gint* y, gint* width, gint* height, AtkCoordType coord_type);
    void GetPosition(gint* x, gint* y, AtkCoordType coord_type);
    void GetSize(gint* width, gint* height);
    gfx::NativeViewAccessible HitTestSync(gint x, gint y, AtkCoordType coord_type);
    bool GrabFocus();
    ...
    gfx::Rect GetExtentsRelativeToAtkCoordinateType(AtkCoordType coord_type) const;
    ...
    std::optional<gfx::Rect> GetUnclippedHypertextRangeBoundsRect(int start_offset, int end_offset);
    bool ScrollSubstringIntoView(AtkScrollType atk_scroll_type, int start_offset, int end_offset);
    bool ScrollSubstringToPoint(int start_offset, int end_offset, AtkCoordType atk_coord_type, int x, int y);
    ...
    const TextAttributeList& GetTextAttributes(int offset, int* start_offset, int* end_offset);
...
protected:
    ...
    std::optional<base::OffsetAdjuster::Adjustments> text_unicode_adjustments_ = std::nullopt;
...
private:
    ...
    void AddRelationToSet(AtkRelationSet*, AtkRelationType, AXPlatformNode* target);
    bool IsInLiveRegion();
    ...
    void GetFullSelection(int32_t* anchor_node_id, int* anchor_offset, int32_t* focus_node_id, int* focus_offset);
    ...
    void UpdateSelectionInformation(int32_t anchor_node_id, int anchor_offset, int32_t focus_node_id, int focus_offset);
    ...
    bool HadNonZeroWidthSelection() const
    {
        return had_nonzero_width_selection;
    }
    std::pair<int32_t, int> GetCurrentCaret() const
    {
        return current_caret_;
    }

    // If the given argument can be found as a child of this node, return its
    // hypertext extents, otherwise return std::nullopt;
    std::optional<std::pair<int, int>> GetHypertextExtentsOfChild(AXPlatformNodeAuraLinux* child);

    // The AtkStateType for a checkable node can vary depending on the role.
    AtkStateType GetAtkStateTypeForCheckableNode();

    gfx::Point ConvertPointToScreenCoordinates(const gfx::Point& point, AtkCoordType atk_coord_type);
