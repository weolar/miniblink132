...
std::optional<ax::mojom::Action> ConvertAction(fuchsia_accessibility_semantics::Action fuchsia_action)
{
    ...
    default:
        LOG(WARNING) << "Unknown fuchsia_accessibility_semantics::Action with value " << static_cast<int>(fuchsia_action);
        return {};
    }
}
...
AccessibilityBridgeFuchsiaImpl::AccessibilityBridgeFuchsiaImpl(aura::Window* window, fuchsia_ui_views::ViewRef view_ref,
    base::RepeatingCallback<void(bool)> on_semantics_enabled, OnConnectionClosedCallback on_connection_closed, inspect::Node inspect_node)
    : root_window_(window)
    , on_semantics_enabled_(std::move(on_semantics_enabled))
    , on_connection_closed_(std::move(on_connection_closed))
    , inspect_node_(std::move(inspect_node))
{
    semantic_provider_ = std::make_unique<AXFuchsiaSemanticProviderImpl>(std::move(view_ref), this);

    AccessibilityBridgeFuchsiaRegistry* registry = AccessibilityBridgeFuchsiaRegistry::GetInstance();
    DCHECK(registry);
    if (root_window_)
...
uint32_t AccessibilityBridgeFuchsiaImpl::MaybeToFuchsiaRootID(uint32_t node_id)
{
...
void AccessibilityBridgeFuchsiaImpl::UpdateNode(fuchsia_accessibility_semantics::Node node)
{
...
void AccessibilityBridgeFuchsiaImpl::OnAccessibilityHitTestResult(int hit_test_request_id, std::optional<uint32_t> result)
{
...
bool AccessibilityBridgeFuchsiaImpl::OnAccessibilityAction(uint32_t node_id, fuchsia_accessibility_semantics::Action action)
{
    ...
    AXPlatformNode* ax_platform_node = AXPlatformNodeFuchsia::GetFromUniqueId(node_id);
    AXPlatformNodeFuchsia* ax_platform_node_fuchsia = static_cast<AXPlatformNodeFuchsia*>(ax_platform_node);
    ...
    if (action == fuchsia_accessibility_semantics::Action::kShowOnScreen) {
        ...
        // origin.
        gfx::Rect local_bounds = gfx::ToEnclosedRectIgnoringError(ax_platform_node_fuchsia->GetData().relative_bounds.bounds, kRectConversionError);
...
void AccessibilityBridgeFuchsiaImpl::OnHitTest(fuchsia_math::PointF point, AXFuchsiaSemanticProvider::Delegate::HitTestCallback callback)
{
    AXPlatformNodeFuchsia* ax_platform_node_fuchsia = nullptr;

    if (root_node_id_) {
        // Target the root node.
        AXPlatformNode* ax_platform_node = AXPlatformNodeFuchsia::GetFromUniqueId(*root_node_id_);
        ax_platform_node_fuchsia = static_cast<AXPlatformNodeFuchsia*>(ax_platform_node);
    }
    ...
    pending_hit_test_completers_.insert_or_assign(action_data.request_id, std::move(callback));
...
bool AccessibilityBridgeFuchsiaImpl::OnSemanticsManagerConnectionClosed(zx_status_t status)
{
...
void AccessibilityBridgeFuchsiaImpl::set_semantic_provider_for_test(std::unique_ptr<AXFuchsiaSemanticProvider> semantic_provider)
{
...
{
    return inspect_node_.CreateChild(base::StringPrintf("AXTree-%d", next_inspect_tree_number_++));
}
