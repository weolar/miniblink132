...
class SemanticTreeEventHandler : public fidl::AsyncEventHandler<fuchsia_accessibility_semantics::SemanticTree> {
public:
    explicit SemanticTreeEventHandler(base::OnceCallback<void(fidl::UnbindInfo)> on_fidl_error_callback);
...
// fuchsia view.
class COMPONENT_EXPORT(AX_PLATFORM) AXFuchsiaSemanticProviderImpl : public AXFuchsiaSemanticProvider,
...
public:
    ...
    // semantic provider.
    AXFuchsiaSemanticProviderImpl(fuchsia_ui_views::ViewRef view_ref, Delegate* delegate);
    ~AXFuchsiaSemanticProviderImpl() override;

    // Returns true if Fuchsia has enabled semantics.
    bool semantic_updates_enabled() const
    {
        return semantic_updates_enabled_;
    }
...
private:
    ...
    public:
        ...
        void Apply(fidl::Client<fuchsia_accessibility_semantics::SemanticTree>* semantic_tree);
    ...
    void OnCommitComplete(fidl::Result<fuchsia_accessibility_semantics::SemanticTree::CommitUpdates>& result);
    ...
    // to be updated.
    void MarkChildrenAsNotReachable(const std::vector<uint32_t>& child_ids, uint32_t parent_id);
    ...
    // created.
    void MarkChildrenAsReachable(const std::vector<uint32_t>& child_ids, uint32_t parent_id);
    ...
    void OnAccessibilityActionRequested(OnAccessibilityActionRequestedRequest& request, OnAccessibilityActionRequestedCompleter::Sync& completer) override;

    // fuchsia_accessibility_semantics::SemanticListener:
    void HitTest(HitTestRequest& request, HitTestCompleter::Sync& completer) override;

    // fuchsia_accessibility_semantics::SemanticListener:
    void OnSemanticsModeChanged(OnSemanticsModeChangedRequest& request, OnSemanticsModeChangedCompleter::Sync& completer) override;

    const raw_ptr<Delegate> delegate_;

    std::optional<fidl::ServerBinding<fuchsia_accessibility_semantics::SemanticListener>> semantic_listener_binding_;
