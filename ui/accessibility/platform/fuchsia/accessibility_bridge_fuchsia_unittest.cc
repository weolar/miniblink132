...
public:
    ...
    const std::optional<fuchsia_accessibility_semantics::Node>& last_update() const
    {
        return last_update_;
    }
    ...
    const std::optional<fuchsia_accessibility_semantics::SemanticEvent>& last_event() const
    {
        return last_event_;
    }
...
public:
    ...
    {
        return ax_node_data_;
    }
    void SetData(AXNodeData ax_node_data)
    {
        ax_node_data_ = ax_node_data;
    }
...
public:
    ...
    void SetUp() override
    {
        mock_ax_platform_node_delegate_ = std::make_unique<FakeAXPlatformNodeDelegate>();
        ...
        accessibility_bridge_ = std::make_unique<AccessibilityBridgeFuchsiaImpl>(
            /*root_window=*/nullptr, fidl::HLCPPToNatural(std::move(view_ref)), base::RepeatingCallback<void(bool)>(),
            base::RepeatingCallback<bool(zx_status_t)>(), inspect::Node());
        accessibility_bridge_->set_semantic_provider_for_test(std::move(mock_semantic_provider));
    }
...
TEST_F(AccessibilityBridgeFuchsiaTest, UpdateNode)
{
    ...
    const std::optional<fuchsia_accessibility_semantics::Node>& last_update = mock_semantic_provider_->last_update();
...
TEST_F(AccessibilityBridgeFuchsiaTest, UpdateNodeReplaceNodeID)
{
    ...
    const std::optional<fuchsia_accessibility_semantics::Node>& last_update = mock_semantic_provider_->last_update();
    ASSERT_TRUE(last_update.has_value());
    EXPECT_EQ(last_update->node_id(), AXFuchsiaSemanticProvider::kFuchsiaRootNodeId);
}
...
TEST_F(AccessibilityBridgeFuchsiaTest, UpdateNodeReplaceOffsetContainerID)
{
    ...
    EXPECT_EQ(last_update->container_id().value(), AXFuchsiaSemanticProvider::kFuchsiaRootNodeId);
}
...
TEST_F(AccessibilityBridgeFuchsiaTest, SetRootIDDeletesOldRoot)
{
    ...
    const std::optional<uint32_t>& last_deletion = mock_semantic_provider_->last_deletion();
...
TEST_F(AccessibilityBridgeFuchsiaTest, HitTest)
{
    ...
    AXPlatformNode* root_platform_node = AXPlatformNode::Create(root_delegate.get());

    auto child_delegate = std::make_unique<FakeAXPlatformNodeDelegate>();
    AXPlatformNode* child_platform_node = AXPlatformNode::Create(child_delegate.get());
    ...
    accessibility_bridge_->OnHitTest(target_point,
        base::BindLambdaForTesting([&hit_test_result](const fidl::Response<fuchsia_accessibility_semantics::SemanticListener::HitTest>& response) {
    ...
    const std::optional<AXActionData>& action_data = root_delegate->last_action_data();
    ...
    uint32_t child_node_id = static_cast<uint32_t>(child_platform_node->GetUniqueId());
    accessibility_bridge_->OnAccessibilityHitTestResult(action_data->request_id, std::optional<uint32_t>(child_node_id));
...
TEST_F(AccessibilityBridgeFuchsiaTest, HitTestReturnsRoot)
{
    ...
    // Simulate a hit test result. This should invoke the callback.
    uint32_t root_node_id = static_cast<uint32_t>(root_platform_node->GetUniqueId());
    accessibility_bridge_->OnAccessibilityHitTestResult(action_data->request_id, std::optional<uint32_t>(root_node_id));
...
TEST_F(AccessibilityBridgeFuchsiaTest, HitTestReturnsEmptyResult)
{
    ...
    accessibility_bridge_->OnHitTest(
        target_point, base::BindLambdaForTesting([&callback_ran](const fidl::Response<fuchsia_accessibility_semantics::SemanticListener::HitTest>& response) {
            callback_ran = true;
...
TEST_F(AccessibilityBridgeFuchsiaTest, PerformActionOnRoot)
{
    ...
    accessibility_bridge_->OnAccessibilityAction(AXFuchsiaSemanticProvider::kFuchsiaRootNodeId, fuchsia_accessibility_semantics::Action::kDefault);
...
TEST_F(AccessibilityBridgeFuchsiaTest, ScrollToMakeVisible)
{
    ...
    accessibility_bridge_->OnAccessibilityAction(delegate->GetUniqueId(), fuchsia_accessibility_semantics::Action::kShowOnScreen);
