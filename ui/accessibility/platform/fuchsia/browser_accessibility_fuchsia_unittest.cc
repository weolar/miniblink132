...
AXTreeUpdate CreateTableUpdate()
{
    ...
    column_header_1.AddIntAttribute(ax::mojom::IntAttribute::kTableCellRowIndex, 0);
    column_header_1.AddIntAttribute(ax::mojom::IntAttribute::kTableCellColumnIndex, 0);

    auto& column_header_2 = update.nodes[5];
    column_header_2.id = 5;
    column_header_2.role = AXRole::kColumnHeader;
    column_header_2.AddIntAttribute(ax::mojom::IntAttribute::kTableCellRowIndex, 0);
    column_header_2.AddIntAttribute(ax::mojom::IntAttribute::kTableCellColumnIndex, 1);
...
private:
    ...
    BrowserAccessibilityFuchsiaTest(const BrowserAccessibilityFuchsiaTest&) = delete;
    BrowserAccessibilityFuchsiaTest& operator=(const BrowserAccessibilityFuchsiaTest&) = delete;
};
...
{
    std::map<ax::mojom::Role, fuchsia_accessibility_semantics::Role> role_mapping = { { AXRole::kButton, Role::kButton },
        { AXRole::kCheckBox, Role::kCheckBox }, { AXRole::kHeader, Role::kHeader }, { AXRole::kImage, Role::kImage }, { AXRole::kLink, Role::kLink },
        { AXRole::kList, Role::kList }, { AXRole::kListItem, Role::kListElement }, { AXRole::kListMarker, Role::kListElementMarker },
        { AXRole::kRadioButton, Role::kRadioButton }, { AXRole::kSlider, Role::kSlider }, { AXRole::kTextField, Role::kTextField },
        { AXRole::kSearchBox, Role::kSearchBox }, { AXRole::kTextFieldWithComboBox, Role::kTextFieldWithComboBox }, { AXRole::kTable, Role::kTable },
        { AXRole::kGrid, Role::kGrid }, { AXRole::kGridCell, Role::kCell }, { AXRole::kRow, Role::kTableRow }, { AXRole::kCell, Role::kCell },
        { AXRole::kColumnHeader, Role::kColumnHeader }, { AXRole::kRowGroup, Role::kRowGroup }, { AXRole::kParagraph, Role::kParagraph } };
    ...
    for (const auto& role_pair : role_mapping) {
        ...
        std::unique_ptr<BrowserAccessibilityManager> manager(
            BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(node), node_id_delegate_, test_browser_accessibility_delegate_.get()));

        BrowserAccessibilityFuchsia* browser_accessibility_fuchsia = ToBrowserAccessibilityFuchsia(manager->GetBrowserAccessibilityRoot());
...
TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesNodeActions)
{
    std::map<ax::mojom::Action, fuchsia_accessibility_semantics::Action> action_mapping
        = { { ax::mojom::Action::kDoDefault, fuchsia_accessibility_semantics::Action::kDefault },
              { ax::mojom::Action::kFocus, fuchsia_accessibility_semantics::Action::kSetFocus },
              { ax::mojom::Action::kSetValue, fuchsia_accessibility_semantics::Action::kSetValue },
              { ax::mojom::Action::kScrollToMakeVisible, fuchsia_accessibility_semantics::Action::kShowOnScreen } };
...
TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesLabels)
{
    ...
    node.AddStringAttribute(ax::mojom::StringAttribute::kDescription, kSecondaryLabel);
    ...
    EXPECT_EQ(fuchsia_node_data.attributes()->secondary_label().value(), kSecondaryLabel);
}

TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesRangeAttributes)
{
    ...
    const auto& range_attributes = fuchsia_node_data.attributes()->range().value();
...
TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesTableAttributes)
{
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(CreateTableUpdate(), node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    // Verify table node translation.
    {
        ...
        EXPECT_EQ(fuchsia_node_data.role(), fuchsia_accessibility_semantics::Role::kTable);
        ASSERT_TRUE(fuchsia_node_data.attributes().has_value());
        ASSERT_TRUE(fuchsia_node_data.attributes()->table_attributes().has_value());
        EXPECT_EQ(fuchsia_node_data.attributes()->table_attributes()->number_of_rows(), 2u);
        EXPECT_EQ(fuchsia_node_data.attributes()->table_attributes()->number_of_columns(), 2u);
    }

    // Verify table row node translation.
    {
        BrowserAccessibilityFuchsia* browser_accessibility_fuchsia = ToBrowserAccessibilityFuchsia(manager->GetFromID(kRowNodeId2));

        ASSERT_TRUE(browser_accessibility_fuchsia);
        auto fuchsia_node_data = browser_accessibility_fuchsia->ToFuchsiaNodeData();

        EXPECT_EQ(fuchsia_node_data.role(), fuchsia_accessibility_semantics::Role::kTableRow);
        ASSERT_TRUE(fuchsia_node_data.attributes().has_value());
        ASSERT_TRUE(fuchsia_node_data.attributes()->table_row_attributes().has_value());
        EXPECT_EQ(fuchsia_node_data.attributes()->table_row_attributes()->row_index(), 1u);
    }

    // Verify table cell node translation.
    {
        BrowserAccessibilityFuchsia* browser_accessibility_fuchsia = ToBrowserAccessibilityFuchsia(manager->GetFromID(kCellNodeId));

        ASSERT_TRUE(browser_accessibility_fuchsia);
        auto fuchsia_node_data = browser_accessibility_fuchsia->ToFuchsiaNodeData();

        EXPECT_EQ(fuchsia_node_data.role(), fuchsia_accessibility_semantics::Role::kCell);
        ASSERT_TRUE(fuchsia_node_data.attributes().has_value());
        ASSERT_TRUE(fuchsia_node_data.attributes()->table_cell_attributes().has_value());
        EXPECT_EQ(fuchsia_node_data.attributes()->table_cell_attributes()->row_index(), 1u);
        EXPECT_EQ(fuchsia_node_data.attributes()->table_cell_attributes()->column_index(), 1u);
    }
}

TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesListAttributes)
{
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(CreateListUpdate(), node_id_delegate_, test_browser_accessibility_delegate_.get()));
    ...
    // Verify that the list root was translated.
    {
        ...
        ASSERT_TRUE(browser_accessibility_fuchsia);
        auto fuchsia_node_data = browser_accessibility_fuchsia->ToFuchsiaNodeData();
        EXPECT_EQ(fuchsia_node_data.role(), fuchsia_accessibility_semantics::Role::kList);
        ASSERT_TRUE(fuchsia_node_data.attributes().has_value());
        ASSERT_TRUE(fuchsia_node_data.attributes()->list_attributes().has_value());
        ASSERT_FALSE(fuchsia_node_data.attributes()->list_element_attributes().has_value());
        EXPECT_EQ(fuchsia_node_data.attributes()->list_attributes()->size(), 2u);
    }

    // Verify that the list elements were translated.
    {
        BrowserAccessibilityFuchsia* browser_accessibility_fuchsia = ToBrowserAccessibilityFuchsia(manager->GetFromID(kListElementId2));
        ASSERT_TRUE(browser_accessibility_fuchsia);
        auto fuchsia_node_data = browser_accessibility_fuchsia->ToFuchsiaNodeData();
        EXPECT_EQ(fuchsia_node_data.role(), fuchsia_accessibility_semantics::Role::kListElement);
        ASSERT_TRUE(fuchsia_node_data.attributes().has_value());
        ASSERT_FALSE(fuchsia_node_data.attributes()->list_attributes().has_value());
        ASSERT_TRUE(fuchsia_node_data.attributes()->list_element_attributes().has_value());
        EXPECT_EQ(fuchsia_node_data.attributes()->list_element_attributes()->index(), 2u);
    }
}

TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesCheckedState)
{
    std::map<ax::mojom::CheckedState, fuchsia_accessibility_semantics::CheckedState> state_mapping
        = { { ax::mojom::CheckedState::kNone, fuchsia_accessibility_semantics::CheckedState::kNone },
              { ax::mojom::CheckedState::kTrue, fuchsia_accessibility_semantics::CheckedState::kChecked },
              { ax::mojom::CheckedState::kFalse, fuchsia_accessibility_semantics::CheckedState::kUnchecked },
              { ax::mojom::CheckedState::kMixed, fuchsia_accessibility_semantics::CheckedState::kMixed } };

    for (const auto state_pair : state_mapping) {
        AXNodeData node;
        node.id = kRootId;
        node.AddIntAttribute(ax::mojom::IntAttribute::kCheckedState, static_cast<int32_t>(state_pair.first));
        ...
        EXPECT_EQ(fuchsia_node_data.states()->checked_state().value(), state_pair.second);
    }
}

TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesSelectedState)
{
    AXNodeData node;
    node.id = kRootId;
...
TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesInvisibleState)
{
    AXNodeData node;
    node.id = kRootId;
...
TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesIgnoredState)
{
    AXNodeData node;
    node.id = kRootId;
...
{
    const auto full_value = std::string(fuchsia_accessibility_semantics::kMaxLabelSize + 1, 'a');
    const auto truncated_value = std::string(fuchsia_accessibility_semantics::kMaxLabelSize, 'a');
...
TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesScrollOffset)
{
    ...
    const auto& viewport_offset = fuchsia_node_data.states()->viewport_offset().value();
    EXPECT_EQ(viewport_offset.x(), kScrollX);
    EXPECT_EQ(viewport_offset.y(), kScrollY);
}

TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesSpatialInfo)
{
    ...
    node.relative_bounds.transform = std::make_unique<gfx::Transform>(
        gfx::Transform::RowMajor(x_scale, 0, 0, x_translation, 0, y_scale, 0, y_translation, 0, 0, z_scale, z_translation, 0, 0, 0, 1));
    node.relative_bounds.bounds = gfx::RectF(x_min, y_min, /* width = */ x_max - x_min, /* height = */ y_max - y_min);
    ...
    const auto& transform = fuchsia_node_data.node_to_container_transform()->matrix();
...
TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesOffsetContainerID)
{
    AXNodeData node;
    node.id = kRootId;
    node.child_ids = { 2 };
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(node, node_2, node_3), node_id_delegate_, test_browser_accessibility_delegate_.get()));

    // Verify that node 2's offset container was translated correctly.
    BrowserAccessibilityFuchsia* root = ToBrowserAccessibilityFuchsia(manager->GetFromID(kRootId));
    BrowserAccessibilityFuchsia* child = ToBrowserAccessibilityFuchsia(manager->GetFromID(2));
    ASSERT_TRUE(child);
    ...
    BrowserAccessibilityFuchsia* grandchild = ToBrowserAccessibilityFuchsia(manager->GetFromID(3));
    ...
    EXPECT_EQ(grandchild_node_data.container_id().value(), child->GetFuchsiaNodeID());
}

TEST_F(BrowserAccessibilityFuchsiaTest, ToleratesNonexistentOffsetContainerNodeID)
{
    AXNodeData node;
    node.id = kRootId;
    node.child_ids = { 2 };
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(node, node_2), node_id_delegate_, test_browser_accessibility_delegate_.get()));
...
TEST_F(BrowserAccessibilityFuchsiaTest, ToFuchsiaNodeDataTranslatesNodeIDAndChildIDs)
{
    AXNodeData node;
    node.id = kRootId;
    node.child_ids = { 2, 3 };
    ...
    BrowserAccessibilityFuchsia* root = ToBrowserAccessibilityFuchsia(manager->GetFromID(kRootId));
    BrowserAccessibilityFuchsia* child_1 = ToBrowserAccessibilityFuchsia(manager->GetFromID(2));
    BrowserAccessibilityFuchsia* child_2 = ToBrowserAccessibilityFuchsia(manager->GetFromID(3));

    ASSERT_TRUE(child_1);
    ASSERT_TRUE(child_2);
    EXPECT_EQ(fuchsia_node_data.node_id(), root->GetFuchsiaNodeID());
    ASSERT_EQ(fuchsia_node_data.child_ids()->size(), 2u);
    EXPECT_EQ(fuchsia_node_data.child_ids().value()[0], child_1->GetFuchsiaNodeID());
    EXPECT_EQ(fuchsia_node_data.child_ids().value()[1], child_2->GetFuchsiaNodeID());
}
...
TEST_F(BrowserAccessibilityFuchsiaTest, ChildTree)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> child_manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(node, node_2, node_3), node_id_delegate_, nullptr));
    ...
    std::unique_ptr<BrowserAccessibilityManager> parent_manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(node_4, node_5), node_id_delegate_, nullptr));
    ...
    BrowserAccessibilityFuchsia* browser_accessibility_fuchsia = ToBrowserAccessibilityFuchsia(parent_manager->GetFromID(5));

    {
        ASSERT_TRUE(browser_accessibility_fuchsia);
        fuchsia_accessibility_semantics::Node fuchsia_node_data = browser_accessibility_fuchsia->ToFuchsiaNodeData();

        // Get the root of the child tree to verify that it's present in the parent
        // node's children.
        BrowserAccessibilityFuchsia* child_root = ToBrowserAccessibilityFuchsia(child_manager->GetBrowserAccessibilityRoot());

        ASSERT_EQ(fuchsia_node_data.child_ids()->size(), 1u);
        EXPECT_EQ(fuchsia_node_data.child_ids().value()[0], child_root->GetFuchsiaNodeID());
    }
...
TEST_F(BrowserAccessibilityFuchsiaTest, ChildTreeMissing)
{
    ...
    std::unique_ptr<BrowserAccessibilityManager> parent_manager(
        BrowserAccessibilityManager::Create(MakeAXTreeUpdateForTesting(node_4, node_5), node_id_delegate_, test_browser_accessibility_delegate_.get()));
