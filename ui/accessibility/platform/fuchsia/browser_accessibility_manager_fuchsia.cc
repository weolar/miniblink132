...
{
    return new BrowserAccessibilityManagerFuchsia(initial_tree, node_id_delegate, delegate);
}
...
{
    return new BrowserAccessibilityManagerFuchsia(BrowserAccessibilityManagerFuchsia::GetEmptyDocument(), node_id_delegate, delegate);
}
    ...
    : BrowserAccessibilityManager(node_id_delegate, delegate)
{
    ...
    if (accessibility_bridge) {
        ...
        tree_dump_node_ = inspect_node_.CreateLazyNode("tree-data", [this]() {
            ...
            inspector.GetRoot().CreateString(GetTreeID().ToString(), std::move(str), &inspector);

            // Test to check if the string fit in memory.
            if (inspector.GetStats().failed_allocations > 0) {
                ZX_LOG(WARNING, ZX_OK) << "Inspector had failed allocations. Some semantic tree data may "
...
BrowserAccessibilityManagerFuchsia::~BrowserAccessibilityManagerFuchsia() = default;

AccessibilityBridgeFuchsia* BrowserAccessibilityManagerFuchsia::GetAccessibilityBridge() const
{
    ...
    gfx::NativeWindow top_level_native_window = delegate_ ? delegate_->GetTopLevelNativeWindow() : gfx::NativeWindow();

    aura::Window* accessibility_bridge_key = top_level_native_window ? top_level_native_window->GetRootWindow() : nullptr;
    if (!accessibility_bridge_key) {
        return nullptr;
    }

    AccessibilityBridgeFuchsiaRegistry* accessibility_bridge_registry = AccessibilityBridgeFuchsiaRegistry::GetInstance();
    DCHECK(accessibility_bridge_registry);

    return accessibility_bridge_registry->GetAccessibilityBridge(accessibility_bridge_key);
}
...
void BrowserAccessibilityManagerFuchsia::FireFocusEvent(AXNode* node)
{
    ...
    BrowserAccessibilityFuchsia* new_focus_fuchsia = ToBrowserAccessibilityFuchsia(GetFromAXNode(node));

    BrowserAccessibilityFuchsia* old_focus_fuchsia = ToBrowserAccessibilityFuchsia(GetFromAXNode(GetLastFocusedNode()));
...
void BrowserAccessibilityManagerFuchsia::FireBlinkEvent(ax::mojom::Event event_type, BrowserAccessibility* node, int action_request_id)
{
...
void BrowserAccessibilityManagerFuchsia::OnHitTestResult(int action_request_id, BrowserAccessibility* node)
{
    if (!GetAccessibilityBridge())
        return;
    ...
    if (node) {
        BrowserAccessibilityFuchsia* hit_result = ToBrowserAccessibilityFuchsia(node);
        DCHECK(hit_result);
        hit_result_id = hit_result->GetFuchsiaNodeID();
    }

    GetAccessibilityBridge()->OnAccessibilityHitTestResult(action_request_id, hit_result_id);
}
...
// static
void BrowserAccessibilityManagerFuchsia::SetAccessibilityBridgeForTest(AccessibilityBridgeFuchsia* accessibility_bridge_for_test)
{
    // Only allow transition from nullptr to non-nullptr, or vice versa.
    CHECK(!g_accessibility_bridge_for_test || !accessibility_bridge_for_test) << "Setting the accessibility bridge to two different values is not "
                                                                                 "allowed.";
