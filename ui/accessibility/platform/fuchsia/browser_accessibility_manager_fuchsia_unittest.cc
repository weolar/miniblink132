...
class MockBrowserAccessibilityDelegate : public TestAXPlatformTreeManagerDelegate {
public:
    ...
    void AccessibilityHitTest(const gfx::Point& point_in_frame_pixels, const ax::mojom::Event& opt_event_to_fire, int opt_request_id,
        base::OnceCallback<void(AXPlatformTreeManager* hit_manager, AXNodeID hit_node_id)> opt_callback) override
    {
    ...
    const std::optional<int>& last_request_id()
    {
...
public:
    ...
    void OnAccessibilityHitTestResult(int hit_test_request_id, std::optional<uint32_t> result) override
    {
    ...
    const std::vector<uint32_t>& node_deletions()
    {
    ...
    const std::optional<uint32_t>& old_focus()
    {
        return old_focus_;
    }

    const std::optional<uint32_t>& new_focus()
    {
        return new_focus_;
    }

    const std::optional<uint32_t>& root_node_id()
    {
...
private:
    ...
    std::map<int /* hit test request id */, std::optional<uint32_t> /* hit test result */> hit_test_results_;
...
protected:
    ...
    std::unique_ptr<BrowserAccessibilityManager> manager_ { BrowserAccessibilityManager::Create(node_id_delegate_, &mock_browser_accessibility_delegate_) };
};
...
TEST_F(BrowserAccessibilityManagerFuchsiaTest, TestEmitNodeUpdates)
{
...

    {
        ...
        BrowserAccessibilityFuchsia* node_1 = ToBrowserAccessibilityFuchsia(manager_->GetFromID(1));
        ASSERT_TRUE(node_1);
        ...
        EXPECT_EQ(*mock_accessibility_bridge.root_node_id(), static_cast<uint32_t>(node_1->GetFuchsiaNodeID()));
...

    {
        ...
        BrowserAccessibilityFuchsia* node_1 = ToBrowserAccessibilityFuchsia(manager_->GetFromID(1));
        ASSERT_TRUE(node_1);
        BrowserAccessibilityFuchsia* node_2 = ToBrowserAccessibilityFuchsia(manager_->GetFromID(2));
        ASSERT_TRUE(node_2);
        ...
        EXPECT_EQ(node_updates[1].child_ids().value()[0], node_2->GetFuchsiaNodeID());
...
TEST_F(BrowserAccessibilityManagerFuchsiaTest, TestLocationChange)
{
    ...
    {
        const std::vector<fuchsia_accessibility_semantics::Node>& node_updates = mock_accessibility_bridge.node_updates();
    ...
    relative_bounds.bounds = gfx::RectF(/*x=*/1, /*y=*/2, /*width=*/3, /*height=*/4);
...

    {
        ...
        const std::vector<fuchsia_accessibility_semantics::Node>& node_updates = mock_accessibility_bridge.node_updates();
        ASSERT_EQ(node_updates.size(), 3u);
        const fuchsia_accessibility_semantics::Node& node_update = node_updates.back();
        EXPECT_EQ(node_update.node_id(), static_cast<uint32_t>(node_2->GetFuchsiaNodeID()));
        ASSERT_TRUE(node_update.location());
        const fuchsia_ui_gfx::BoundingBox& location = node_update.location().value();
...
TEST_F(BrowserAccessibilityManagerFuchsiaTest, TestFocusChange)
{
...

    {
        ...
        EXPECT_EQ(node_updates.back().node_id().value(), node_1->GetFuchsiaNodeID());
...

    {
        ...
        const fuchsia_accessibility_semantics::Node& old_focus_node = node_updates[node_updates.size() - 2];
        ...
        EXPECT_EQ(node_updates.back().node_id().value(), node_2->GetFuchsiaNodeID());
        ASSERT_TRUE(node_updates.back().states());
        ASSERT_TRUE(node_updates.back().states()->has_input_focus().has_value());
        EXPECT_TRUE(node_updates.back().states()->has_input_focus().value());
    }
...
TEST_F(BrowserAccessibilityManagerFuchsiaTest, HitTest)
{
    ...
    AXPlatformNodeFuchsia* platform_node = static_cast<AXPlatformNodeFuchsia*>(AXPlatformNodeBase::GetFromUniqueId(node_1->GetFuchsiaNodeID()));
    ASSERT_TRUE(platform_node);

    platform_node->PerformAction(action_data);

    {
        std::optional<gfx::Point> last_target = mock_browser_accessibility_delegate_.last_hit_test_point();
        ASSERT_TRUE(last_target.has_value());
        EXPECT_EQ(last_target->x(), 1);
        EXPECT_EQ(last_target->y(), 2);

        std::optional<int> last_request_id = mock_browser_accessibility_delegate_.last_request_id();
    ...
    manager_->FireBlinkEvent(ax::mojom::Event::kHover, node_2, action_data.request_id);

    {
        const std::map<int, std::optional<uint32_t>>& hit_test_results = mock_accessibility_bridge.hit_test_results();
...
TEST_F(BrowserAccessibilityManagerFuchsiaTest, PerformAction)
{
    ...
    AXPlatformNodeFuchsia* platform_node = static_cast<AXPlatformNodeFuchsia*>(AXPlatformNodeBase::GetFromUniqueId(node_2->GetFuchsiaNodeID()));
    ASSERT_TRUE(platform_node);

    platform_node->PerformAction(action_data);

    {
        const std::optional<AXActionData> last_action_data = mock_browser_accessibility_delegate_.last_action_data();
        ASSERT_TRUE(last_action_data);
        EXPECT_EQ(last_action_data->action, ax::mojom::Action::kScrollToMakeVisible);
    }
}
