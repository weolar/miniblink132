...
BrowserAccessibilityFuchsia::BrowserAccessibilityFuchsia(BrowserAccessibilityManager* manager, AXNode* node)
    : BrowserAccessibility(manager, node)
{
    platform_node_ = static_cast<AXPlatformNodeFuchsia*>(AXPlatformNode::Create(this));
}

AccessibilityBridgeFuchsia* BrowserAccessibilityFuchsia::GetAccessibilityBridge() const
{
    BrowserAccessibilityManagerFuchsia* manager_fuchsia = static_cast<BrowserAccessibilityManagerFuchsia*>(manager());
...
fuchsia_accessibility_semantics::Node BrowserAccessibilityFuchsia::ToFuchsiaNodeData() const
{
    return { {
...
void BrowserAccessibilityFuchsia::OnDataChanged()
{
    ...
    // frame's tree.
    if (manager()->IsRootFrameManager() && manager()->GetBrowserAccessibilityRoot() == this) {
...
BrowserAccessibilityFuchsia* ToBrowserAccessibilityFuchsia(BrowserAccessibility* obj)
{
...
std::vector<uint32_t> BrowserAccessibilityFuchsia::GetFuchsiaChildIDs() const
{
    ...
    for (const auto* child : AllChildren()) {
        const BrowserAccessibilityFuchsia* fuchsia_child = static_cast<const BrowserAccessibilityFuchsia*>(child);
...
bool BrowserAccessibilityFuchsia::IsFuchsiaDefaultAction() const
{
    ...
    return verb != ax::mojom::DefaultActionVerb::kNone && verb != ax::mojom::DefaultActionVerb::kClickAncestor;
}

std::vector<fuchsia_accessibility_semantics::Action> BrowserAccessibilityFuchsia::GetFuchsiaActions() const
{
...
fuchsia_accessibility_semantics::Role BrowserAccessibilityFuchsia::GetFuchsiaRole() const
{
    auto role = GetRole();
...
fuchsia_accessibility_semantics::States BrowserAccessibilityFuchsia::GetFuchsiaStates() const
{
    ...
    if (HasIntAttribute(ax::mojom::IntAttribute::kCheckedState)) {
        ...
        case ax::mojom::CheckedState::kNone:
            states.checked_state(fuchsia_accessibility_semantics::CheckedState::kNone);
            break;
        case ax::mojom::CheckedState::kTrue:
            states.checked_state(fuchsia_accessibility_semantics::CheckedState::kChecked);
            break;
        case ax::mojom::CheckedState::kFalse:
            states.checked_state(fuchsia_accessibility_semantics::CheckedState::kUnchecked);
            break;
        case ax::mojom::CheckedState::kMixed:
            states.checked_state(fuchsia_accessibility_semantics::CheckedState::kMixed);
            break;
        }
    }
    ...
    if (GetData().IsSelectable() && HasBoolAttribute(ax::mojom::BoolAttribute::kSelected)) {
    ...
    if (HasStringAttribute(ax::mojom::StringAttribute::kValue)) {
        const std::string& value = GetStringAttribute(ax::mojom::StringAttribute::kValue);
        states.value(value.substr(0, fuchsia_accessibility_semantics::kMaxLabelSize));
    }

    // The value a range element currently has.
    if (HasFloatAttribute(ax::mojom::FloatAttribute::kValueForRange)) {
        states.range_value(GetFloatAttribute(ax::mojom::FloatAttribute::kValueForRange));
    }

    // The scroll offsets, if the element is a scrollable container.
    const float x_scroll_offset = GetIntAttribute(ax::mojom::IntAttribute::kScrollX);
    const float y_scroll_offset = GetIntAttribute(ax::mojom::IntAttribute::kScrollY);
...
fuchsia_accessibility_semantics::Attributes BrowserAccessibilityFuchsia::GetFuchsiaAttributes() const
{
    fuchsia_accessibility_semantics::Attributes attributes;
    if (HasStringAttribute(ax::mojom::StringAttribute::kName)) {
        const std::string& name = GetStringAttribute(ax::mojom::StringAttribute::kName);
        attributes.label(name.substr(0, fuchsia_accessibility_semantics::kMaxLabelSize));
    }

    if (HasStringAttribute(ax::mojom::StringAttribute::kDescription)) {
        const std::string& description = GetStringAttribute(ax::mojom::StringAttribute::kDescription);
        attributes.secondary_label(description.substr(0, fuchsia_accessibility_semantics::kMaxLabelSize));
    }
    ...
    if (GetData().IsRangeValueSupported()) {
        ...
        if (HasFloatAttribute(ax::mojom::FloatAttribute::kMinValueForRange)) {
            range_attributes.min_value(GetFloatAttribute(ax::mojom::FloatAttribute::kMinValueForRange));
        }
        if (HasFloatAttribute(ax::mojom::FloatAttribute::kMaxValueForRange)) {
            range_attributes.max_value(GetFloatAttribute(ax::mojom::FloatAttribute::kMaxValueForRange));
        }
        if (HasFloatAttribute(ax::mojom::FloatAttribute::kStepValueForRange)) {
            range_attributes.step_delta(GetFloatAttribute(ax::mojom::FloatAttribute::kStepValueForRange));
        }
...
fuchsia_ui_gfx::BoundingBox BrowserAccessibilityFuchsia::GetFuchsiaLocation() const
{
...
uint32_t BrowserAccessibilityFuchsia::GetOffsetContainerOrRootNodeID() const
{
    ...
    BrowserAccessibility* offset_container = offset_container_id == -1 ? manager()->GetBrowserAccessibilityRoot() : manager()->GetFromID(offset_container_id);

    BrowserAccessibilityFuchsia* fuchsia_container = ToBrowserAccessibilityFuchsia(offset_container);
    ...
    if (!fuchsia_container) {
        ZX_LOG(ERROR, ZX_OK) << "Node " << GetId() << " references non-existent offset container ID " << offset_container_id;
        return 0;
    }
...
bool BrowserAccessibilityFuchsia::AccessibilityPerformAction(const AXActionData& action_data)
{
