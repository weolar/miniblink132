...
static AXPlatformNodeAuraLinux* ToAXPlatformNodeAuraLinux(AXPlatformAtkHyperlink* atk_hyperlink)
{
    if (!atk_hyperlink)
        return nullptr;
...
static gchar* AXPlatformAtkHyperlinkGetUri(AtkHyperlink* atk_hyperlink, gint index)
{
    AXPlatformNodeAuraLinux* obj = ToAXPlatformNodeAuraLinux(AX_PLATFORM_ATK_HYPERLINK(atk_hyperlink));
    if (!obj)
        return nullptr;
    ...
    return g_strdup(obj->GetStringAttribute(ax::mojom::StringAttribute::kUrl).c_str());
}

static AtkObject* AXPlatformAtkHyperlinkGetObject(AtkHyperlink* atk_hyperlink, gint index)
{
    AXPlatformNodeAuraLinux* obj = ToAXPlatformNodeAuraLinux(AX_PLATFORM_ATK_HYPERLINK(atk_hyperlink));
    if (!obj)
        return nullptr;
...
static gboolean AXPlatformAtkHyperlinkIsSelectedLink(AtkHyperlink* atk_hyperlink)
{
    AXPlatformNodeAuraLinux* obj = ToAXPlatformNodeAuraLinux(AX_PLATFORM_ATK_HYPERLINK(atk_hyperlink));
    if (!obj)
        return false;
...
static int AXPlatformAtkHyperlinkGetStartIndex(AtkHyperlink* atk_hyperlink)
{
    ...
    std::optional<std::pair<int, int>> indices = link->priv->platform_node->GetEmbeddedObjectIndices();
...
static void AXPlatformAtkHyperlinkClassInit(AtkHyperlinkClass* klass)
{
    ...
    g_type_class_add_private(gobject_class, sizeof(AXPlatformAtkHyperlinkPrivate));
...
void ax_platform_atk_hyperlink_set_object(AXPlatformAtkHyperlink* atk_hyperlink, AXPlatformNodeAuraLinux* platform_node)
{
...
{
    AXPlatformAtkHyperlinkPrivate* priv = G_TYPE_INSTANCE_GET_PRIVATE(self, ax_platform_atk_hyperlink_get_type(), AXPlatformAtkHyperlinkPrivate);
    self->priv = priv;
...
GType ax_platform_atk_hyperlink_get_type()
{
    ...
    if (g_once_init_enter(&type_id)) {
        static const GTypeInfo tinfo = {
            sizeof(AXPlatformAtkHyperlinkClass), (GBaseInitFunc) nullptr, (GBaseFinalizeFunc) nullptr, (GClassInitFunc)AXPlatformAtkHyperlinkClassInit,
            (GClassFinalizeFunc) nullptr, nullptr, /* class data */
            sizeof(AXPlatformAtkHyperlink), /* instance size */
            0, /* nb preallocs */
            (GInstanceInitFunc)AXPlatformAtkHyperlinkInit, nullptr /* value table */
        };

        GType type = g_type_register_static(ATK_TYPE_HYPERLINK, "AXPlatformAtkHyperlink", &tinfo, GTypeFlags(0));
