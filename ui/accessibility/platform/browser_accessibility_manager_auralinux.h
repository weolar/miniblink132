...
class COMPONENT_EXPORT(AX_PLATFORM) BrowserAccessibilityManagerAuraLinux : public BrowserAccessibilityManager {
public:
    BrowserAccessibilityManagerAuraLinux(const AXTreeUpdate& initial_tree, AXNodeIdDelegate& node_id_delegate, AXPlatformTreeManagerDelegate* delegate);

    BrowserAccessibilityManagerAuraLinux(const BrowserAccessibilityManagerAuraLinux&) = delete;
    BrowserAccessibilityManagerAuraLinux& operator=(const BrowserAccessibilityManagerAuraLinux&) = delete;
    ...
    void OnFindInPageResult(int request_id, int match_index, int start_id, int start_offset, int end_id, int end_offset) override;
    void OnFindInPageTermination() override;

protected:
    FRIEND_TEST_ALL_PREFIXES(BrowserAccessibilityManagerAuraLinuxTest, TestEmitChildrenChanged);
