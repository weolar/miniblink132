...
{
    return new BrowserAccessibilityManagerAuraLinux(initial_tree, node_id_delegate, delegate);
}
...
{
    return new BrowserAccessibilityManagerAuraLinux(BrowserAccessibilityManagerAuraLinux::GetEmptyDocument(), node_id_delegate, delegate);
}

BrowserAccessibilityManagerAuraLinux* BrowserAccessibilityManager::ToBrowserAccessibilityManagerAuraLinux()
{
...
BrowserAccessibilityManagerAuraLinux::~BrowserAccessibilityManagerAuraLinux()
{
    if (IsRootFrameManager()) {
        ...
        gfx::NativeViewAccessible obj = GetBrowserAccessibilityRoot()->GetNativeViewAccessible();
...
void BrowserAccessibilityManagerAuraLinux::SetPrimaryWebContentsForWindow(AXNodeID node_id)
{
    DCHECK_NE(node_id, kInvalidAXNodeID);
    DCHECK(GetFromID(node_id));
    DCHECK(primary_web_contents_for_window_id_ == node_id || primary_web_contents_for_window_id_ == kInvalidAXNodeID);
    primary_web_contents_for_window_id_ = node_id;
}

AXNodeID BrowserAccessibilityManagerAuraLinux::GetPrimaryWebContentsForWindow() const
{
...
void BrowserAccessibilityManagerAuraLinux::FireSelectedEvent(BrowserAccessibility* node)
{
...
void BrowserAccessibilityManagerAuraLinux::FireBusyChangedEvent(BrowserAccessibility* node, bool is_busy)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnBusyStateChanged(is_busy);
}

void BrowserAccessibilityManagerAuraLinux::FireLoadingEvent(BrowserAccessibility* node, bool is_loading)
{
...
void BrowserAccessibilityManagerAuraLinux::FireEnabledChangedEvent(BrowserAccessibility* node)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnEnabledChanged();
}

void BrowserAccessibilityManagerAuraLinux::FireExpandedEvent(BrowserAccessibility* node, bool is_expanded)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnExpandedStateChanged(is_expanded);
}

void BrowserAccessibilityManagerAuraLinux::FireShowingEvent(BrowserAccessibility* node, bool is_showing)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnShowingStateChanged(is_showing);
}

void BrowserAccessibilityManagerAuraLinux::FireInvalidStatusChangedEvent(BrowserAccessibility* node)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnInvalidStatusChanged();
}

void BrowserAccessibilityManagerAuraLinux::FireAriaCurrentChangedEvent(BrowserAccessibility* node)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnAriaCurrentChanged();
}

void BrowserAccessibilityManagerAuraLinux::FireEvent(BrowserAccessibility* node, ax::mojom::Event event)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->NotifyAccessibilityEvent(event);
}

void BrowserAccessibilityManagerAuraLinux::FireBlinkEvent(ax::mojom::Event event_type, BrowserAccessibility* node, int action_request_id)
{
...
void BrowserAccessibilityManagerAuraLinux::FireNameChangedEvent(BrowserAccessibility* node)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnNameChanged();
}

void BrowserAccessibilityManagerAuraLinux::FireDescriptionChangedEvent(BrowserAccessibility* node)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnDescriptionChanged();
}

void BrowserAccessibilityManagerAuraLinux::FireParentChangedEvent(BrowserAccessibility* node)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnParentChanged();
}

void BrowserAccessibilityManagerAuraLinux::FireReadonlyChangedEvent(BrowserAccessibility* node)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnReadonlyChanged();
}

void BrowserAccessibilityManagerAuraLinux::FireSortDirectionChangedEvent(BrowserAccessibility* node)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnSortDirectionChanged();
}

void BrowserAccessibilityManagerAuraLinux::FireTextAttributesChangedEvent(BrowserAccessibility* node)
{
    ToBrowserAccessibilityAuraLinux(node)->GetNode()->OnTextAttributesChanged();
}

void BrowserAccessibilityManagerAuraLinux::FireSubtreeCreatedEvent(BrowserAccessibility* node)
{
...
void BrowserAccessibilityManagerAuraLinux::FireGeneratedEvent(AXEventGenerator::Event event_type, const AXNode* node)
{
    ...
    case AXEventGenerator::Event::BUSY_CHANGED: {
        ...
        if (wrapper->IsPlatformDocument())
            return;
        FireBusyChangedEvent(wrapper, wrapper->GetData().GetBoolAttribute(ax::mojom::BoolAttribute::kBusy));
        break;
    }
    ...
    case AXEventGenerator::Event::ROLE_CHANGED: {
        ...
        // be done manually.
        AXPlatformNodeAuraLinux* platform_node = ToBrowserAccessibilityAuraLinux(wrapper)->GetNode();
...
void BrowserAccessibilityManagerAuraLinux::OnNodeWillBeDeleted(ui::AXTree* tree, ui::AXNode* node)
{
...
void BrowserAccessibilityManagerAuraLinux::OnIgnoredWillChange(
    AXTree* tree, AXNode* node, bool is_ignored_new_value, bool is_changing_unignored_parents_children)
{
    ...
    if (is_ignored_new_value && is_changing_unignored_parents_children) {
        ...
        if (obj && obj->GetParent()) {
            ...
            g_signal_emit_by_name(
                obj->GetParent(), "children-changed::remove", static_cast<gint>(obj->GetIndexInParent().value()), obj->GetNativeViewAccessible());
        }
    }
}

void BrowserAccessibilityManagerAuraLinux::OnSubtreeWillBeDeleted(AXTree* tree, AXNode* node)
{
...
void BrowserAccessibilityManagerAuraLinux::OnAtomicUpdateFinished(AXTree* tree, bool root_changed, const std::vector<AXTreeObserver::Change>& changes)
{
...
void BrowserAccessibilityManagerAuraLinux::OnFindInPageResult(int request_id, int match_index, int start_id, int start_offset, int end_id, int end_offset)
{
    BrowserAccessibility* node = GetFromID(start_id);
    if (!node)
        return;
    AXPlatformNodeAuraLinux* platform_node = ToBrowserAccessibilityAuraLinux(node)->GetNode();
...
{
    static_cast<BrowserAccessibilityAuraLinux*>(GetBrowserAccessibilityRoot())->GetNode()->TerminateFindInPage();
}

bool BrowserAccessibilityManagerAuraLinux::CanEmitChildrenChanged(BrowserAccessibility* node) const
{
