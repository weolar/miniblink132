...
class COMPONENT_EXPORT(UI_WM) EasyResizeWindowTargeter : public aura::WindowTargeter {
public:
    // NOTE: the insets must be negative.
    EasyResizeWindowTargeter(const gfx::Insets& mouse_extend, const gfx::Insets& touch_extend);
...
private:
    ...
    bool EventLocationInsideBounds(aura::Window* target, const ui::LocatedEvent& event) const final;
