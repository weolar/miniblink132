...
public:
    ...
    gfx::NativeCursor cursor() const
    {
        return cursor_;
    }
    void set_cursor(gfx::NativeCursor cursor)
    {
        cursor_ = cursor;
    }
    ...
    ui::CursorSize cursor_size() const
    {
        return cursor_size_;
    }
    ...
    const gfx::Size& system_cursor_size() const
    {
        return system_cursor_size_;
    }
    ...
    bool mouse_events_enabled() const
    {
...
void CursorManager::ShowCursor()
{
    ...
    if (cursor_lock_count_ == 0 && IsCursorVisible() != state_on_unlock_->visible()) {
        ...
        if (GetCursor().type() != ui::mojom::CursorType::kNone) {
            ...
            observers_.Notify(&aura::client::CursorClientObserver::OnCursorVisibilityChanged, true);
        }
    }
}
...
void CursorManager::HideCursor()
{
    ...
    if (cursor_lock_count_ == 0 && IsCursorVisible() != state_on_unlock_->visible()) {
        delegate_->SetVisibility(state_on_unlock_->visible(), this);
        observers_.Notify(&aura::client::CursorClientObserver::OnCursorVisibilityChanged, false);
    }
}
...
void CursorManager::SetCursorSize(ui::CursorSize cursor_size)
{
    ...
    if (GetCursorSize() != state_on_unlock_->cursor_size()) {
        ...
        observers_.Notify(&aura::client::CursorClientObserver::OnCursorSizeChanged, cursor_size);
    }
}
...
void CursorManager::EnableMouseEvents()
{
    ...
    if (cursor_lock_count_ == 0 && IsMouseEventsEnabled() != state_on_unlock_->mouse_events_enabled()) {
        delegate_->SetMouseEventsEnabled(state_on_unlock_->mouse_events_enabled(), this);
    }
}
...
{
    display_ = display;
    observers_.Notify(&aura::client::CursorClientObserver::OnCursorDisplayChanged, display);
...
void CursorManager::AddObserver(aura::client::CursorClientObserver* observer)
{
    observers_.AddObserver(observer);
}

void CursorManager::RemoveObserver(aura::client::CursorClientObserver* observer)
{
    observers_.RemoveObserver(observer);
}

bool CursorManager::ShouldHideCursorOnKeyEvent(const ui::KeyEvent& event) const
{
    return false;
}

bool CursorManager::ShouldHideCursorOnTouchEvent(const ui::TouchEvent& event) const
{
...
void CursorManager::CommitVisibility(bool visible)
{
    ...
    observers_.Notify(&aura::client::CursorClientObserver::OnCursorVisibilityChanged, GetCursor().type() == ui::mojom::CursorType::kNone ? false : visible);
...
void CursorManager::CommitSystemCursorSize(const gfx::Size& system_cursor_size)
{
    current_state_->set_system_cursor_size(system_cursor_size);
    observers_.Notify(&aura::client::CursorClientObserver::OnSystemCursorSizeChanged, system_cursor_size);
}
...
void CursorManager::SetCursorImpl(gfx::NativeCursor cursor, bool forced)
{
    ...
    if (cursor_lock_count_ == 0 && (forced || GetCursor() != state_on_unlock_->cursor())) {
        delegate_->SetCursor(state_on_unlock_->cursor(), this);
        bool is_visible = cursor.type() != ui::mojom::CursorType::kNone;
        if (is_visible != previously_visible) {
            observers_.Notify(&aura::client::CursorClientObserver::OnCursorVisibilityChanged, is_visible);
        }
    }
}
