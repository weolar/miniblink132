...
void DefaultActivationClient::RemoveObserver(ActivationChangeObserver* observer)
{
...
{
    ActivateWindowImpl(ActivationChangeObserver::ActivationReason::ACTIVATION_CLIENT, window);
}

void DefaultActivationClient::ActivateWindowImpl(ActivationChangeObserver::ActivationReason reason, aura::Window* window)
{
    aura::Window* last_active = ActivationClient::GetActiveWindow();
    if (last_active == window)
        return;

    observers_.Notify(&ActivationChangeObserver::OnWindowActivating, reason, window, last_active);
    ...
    observers_.Notify(&ActivationChangeObserver::OnWindowActivated, reason, window, last_active);
...
void DefaultActivationClient::DeactivateWindow(aura::Window* window)
{
    ...
    if (observer) {
        observer->OnWindowActivated(ActivationChangeObserver::ActivationReason::ACTIVATION_CLIENT, nullptr, window);
    }
    if (last_active_)
...
aura::Window* DefaultActivationClient::GetActivatableWindow(aura::Window* window) const
{
    return nullptr;
}

const aura::Window* DefaultActivationClient::GetToplevelWindow(const aura::Window* window) const
{
    return nullptr;
}

bool DefaultActivationClient::CanActivateWindow(const aura::Window* window) const
{
    return true;
}
    ...
    if (window == GetActiveWindow()) {
        ...
        if (next_active && GetActivationChangeObserver(next_active)) {
            GetActivationChangeObserver(next_active)
                ->OnWindowActivated(ActivationChangeObserver::ActivationReason::WINDOW_DISPOSITION_CHANGED, next_active, nullptr);
        }
        return;
    }
