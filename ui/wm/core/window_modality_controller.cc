...
{
    return window->GetProperty(aura::client::kModalKey) == ui::mojom::ModalType::kWindow;
}

bool TransientChildIsSystemModal(const aura::Window* window)
{
    return window->GetProperty(aura::client::kModalKey) == ui::mojom::ModalType::kSystem;
}

bool TransientChildIsChildModal(const aura::Window* window)
{
    return window->GetProperty(aura::client::kModalKey) == ui::mojom::ModalType::kChild;
}
...
bool IsModalTransientChild(const aura::Window* transient, const aura::Window* original)
{
    return transient->IsVisible()
        && (TransientChildIsWindowModal(transient) || TransientChildIsSystemModal(transient)
            || (TransientChildIsChildModal(transient) && HasAncestor(original, GetModalParent(transient))));
}

const aura::Window* GetModalTransientChild(const aura::Window* activatable, const aura::Window* original)
{
    for (const aura::Window* transient : GetTransientChildren(activatable)) {
        if (IsModalTransientChild(transient, original)) {
            ...
            const aura::Window* modal_child = GetModalTransientChild(transient, original);
...
{
    return const_cast<aura::Window*>(GetModalTransient(const_cast<const aura::Window*>(window)));
}
...
WindowModalityController::WindowModalityController(ui::EventTarget* event_target, aura::Env* env)
...
void WindowModalityController::OnWindowPropertyChanged(aura::Window* window, const void* key, intptr_t old)
{
    // In tests, we sometimes create the modality relationship after a window is
    // visible.
    if (key == aura::client::kModalKey && window->GetProperty(aura::client::kModalKey) != ui::mojom::ModalType::kNone && window->IsVisible()) {
        ActivateWindow(window);
        CancelTouchesOnTransientWindowTree(window);
    }
}

void WindowModalityController::OnWindowVisibilityChanged(aura::Window* window, bool visible)
{
    if (visible && window->GetProperty(aura::client::kModalKey) != ui::mojom::ModalType::kNone) {
        CancelTouchesOnTransientWindowTree(window);
        ...
        if (capture_window) {
            ...
            if (window->GetProperty(aura::client::kModalKey) == ui::mojom::ModalType::kChild && !HasAncestor(capture_window, GetModalParent(window))) {
...
bool WindowModalityController::ProcessLocatedEvent(aura::Window* target, ui::LocatedEvent* event)
{
    if (event->handled())
        return false;
    aura::Window* modal_transient_child = GetModalTransient(target);
    if (modal_transient_child && (event->type() == ui::EventType::kMousePressed || event->type() == ui::EventType::kTouchPressed)) {
...
void WindowModalityController::CancelTouchesOnTransientWindowTree(aura::Window* window)
{
