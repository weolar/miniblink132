...
public:
    ...
    ConsumeGestureEventFilter& operator=(const ConsumeGestureEventFilter&) = delete;
...
private:
    ...
    void OnGestureEvent(ui::GestureEvent* e) override
    {
        e->StopPropagation();
    }
};
...
TEST_F(CompoundEventFilterTest, CursorVisibilityChange)
{
    ...
    ui::KeyEvent key = ui::KeyEvent::FromCharacter('a', ui::VKEY_A, ui::DomCode::NONE, ui::EF_NONE);
    DispatchEventUsingWindowDispatcher(&key);
    EXPECT_FALSE(cursor_client.IsCursorVisible());

    // Synthesized mouse event should not show the cursor.
    ui::MouseEvent enter(ui::EventType::kMouseEntered, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), 0, 0);
    enter.SetFlags(enter.flags() | ui::EF_IS_SYNTHESIZED);
    DispatchEventUsingWindowDispatcher(&enter);
    EXPECT_FALSE(cursor_client.IsCursorVisible());

    ui::MouseEvent move(ui::EventType::kMouseMoved, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), 0, 0);
    move.SetFlags(enter.flags() | ui::EF_IS_SYNTHESIZED);
    DispatchEventUsingWindowDispatcher(&move);
    EXPECT_FALSE(cursor_client.IsCursorVisible());

    // A real mouse event should show the cursor.
    ui::MouseEvent real_move(ui::EventType::kMouseMoved, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), 0, 0);
    ...
    key = ui::KeyEvent::FromCharacter('a', ui::VKEY_A, ui::DomCode::NONE, ui::EF_NONE);
    DispatchEventUsingWindowDispatcher(&key);
    EXPECT_TRUE(cursor_client.IsCursorVisible());
    ...
    ui::MouseEvent exit(ui::EventType::kMouseExited, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), 0, 0);
...
TEST_F(CompoundEventFilterTest, TouchHidesCursor)
{
    ...
    ui::MouseEvent mouse0(ui::EventType::kMouseMoved, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), 0, 0);
    ...
    // with kTouchId
    ui::TouchEvent press0(ui::EventType::kTouchPressed, gfx::Point(90, 90), GetTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
    DispatchEventUsingWindowDispatcher(&press0);
    EXPECT_FALSE(cursor_client.IsMouseEventsEnabled());
    EXPECT_FALSE(cursor_client.IsCursorVisible());

    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(10, 10), GetTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
    DispatchEventUsingWindowDispatcher(&move);
    EXPECT_FALSE(cursor_client.IsMouseEventsEnabled());
    EXPECT_FALSE(cursor_client.IsCursorVisible());

    ui::TouchEvent release(ui::EventType::kTouchReleased, gfx::Point(10, 10), GetTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
    ...
    ui::TouchEvent press1(ui::EventType::kTouchPressed, gfx::Point(90, 90), GetTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
...
{
    std::unique_ptr<CompoundEventFilter> compound_filter(new CompoundEventFilter);
    std::unique_ptr<ui::EventHandler> gesure_handler(new ConsumeGestureEventFilter);
