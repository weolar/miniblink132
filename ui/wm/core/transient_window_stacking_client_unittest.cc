...
public:
    ...
    TransientWindowStackingClientTest(const TransientWindowStackingClientTest&) = delete;
    TransientWindowStackingClientTest& operator=(const TransientWindowStackingClientTest&) = delete;
...
TEST_F(TransientWindowStackingClientTest, StackAboveWindowWithNULLLayerDelegate)
{
    std::unique_ptr<Window> parent(CreateTestWindowWithId(0, root_window()));
    std::unique_ptr<Window> w1(CreateTestWindowWithId(1, parent.get()));
    std::unique_ptr<Window> w2(CreateTestWindowWithId(2, parent.get()));
