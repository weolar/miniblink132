...
TEST_F(VisibilityControllerTest, AnimateTransparencyToZeroAndHideHides)
{
    ...
    std::unique_ptr<aura::Window> window(aura::test::CreateTestWindowWithDelegate(&d, -2, gfx::Rect(0, 0, 50, 50), root_window()));
...
TEST_F(VisibilityControllerTest, HideAnimationWindowBoundsTest)
{
    ...
    aura::test::TestWindowDelegate d;
    std::unique_ptr<aura::Window> window(aura::test::CreateTestWindowWithDelegate(&d, -2, expected_bounds, root_window()));
    window->Show();
    SetWindowVisibilityChangesAnimated(window.get());
    SetWindowVisibilityAnimationDuration(window.get(), base::Milliseconds(5));
    SetWindowVisibilityAnimationType(window.get(), WINDOW_VISIBILITY_ANIMATION_TYPE_DROP);
...
TEST_F(VisibilityControllerTest, SetWindowVisibilityChagnesAnimated)
{
    ...
    SetWindowVisibilityChangesAnimated(window.get());
    SetWindowVisibilityAnimationDuration(window.get(), base::Milliseconds(5));
    SetWindowVisibilityAnimationType(window.get(), WINDOW_VISIBILITY_ANIMATION_TYPE_FADE);
    window->Show();
