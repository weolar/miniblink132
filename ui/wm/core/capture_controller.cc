...
void CaptureController::SetCapture(aura::Window* new_capture_window)
{
    ...
    std::map<aura::Window*, raw_ptr<aura::client::CaptureDelegate, CtnExperimental>> delegates = delegates_;
    ...
    if (new_capture_window) {
        aura::Env::GetInstance()->gesture_recognizer()->CancelActiveTouchesExcept(new_capture_window);
    ...
    capture_delegate_ = delegates_.find(capture_root_window) == delegates_.end() ? nullptr : delegates_[capture_root_window].get();
    ...
    observers_.Notify(&aura::client::CaptureClientObserver::OnCaptureChanged, old_capture_window, capture_window_);
}
...
void CaptureController::AddObserver(aura::client::CaptureClientObserver* observer)
{
    observers_.AddObserver(observer);
}

void CaptureController::RemoveObserver(aura::client::CaptureClientObserver* observer)
{
...
void ScopedCaptureClient::TestApi::SetDelegate(aura::client::CaptureDelegate* delegate)
{
