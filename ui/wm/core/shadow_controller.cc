...
int GetShadowElevationForWindowLosingActive(aura::Window* losing_active, aura::Window* gaining_active)
{
...
// EnvObserver).
class ShadowController::Impl : public aura::EnvObserver, public aura::WindowObserver, public base::RefCounted<Impl> {
public:
...
private:
    ...
    void OnWindowActivated(ActivationReason reason, aura::Window* gained_active, aura::Window* lost_active);
    ...
    void HandleWindowActivationChange(aura::Window* gaining_active, aura::Window* losing_active);
    ...
    base::ScopedMultiSourceObservation<aura::Window, aura::WindowObserver> observation_manager_ { this };
...
void ShadowController::Impl::OnWindowParentChanged(aura::Window* window, aura::Window* parent)
{
    if (parent && window->IsVisible())
        HandlePossibleShadowVisibilityChange(window);
}

void ShadowController::Impl::OnWindowPropertyChanged(aura::Window* window, const void* key, intptr_t old)
{
    bool shadow_will_change = key == kShadowElevationKey && window->GetProperty(kShadowElevationKey) != old;

    if (key == aura::client::kShowStateKey) {
        shadow_will_change = window->GetProperty(aura::client::kShowStateKey) != static_cast<ui::mojom::WindowShowState>(old);
    }

    if (key == aura::client::kWindowCornerRadiusKey) {
        shadow_will_change = window->GetProperty(aura::client::kWindowCornerRadiusKey) != static_cast<int>(old);
    }

    shadow_will_change |= delegate_ && delegate_->ShouldUpdateShadowOnWindowPropertyChange(window, key, old);
...
void ShadowController::Impl::OnWindowVisibilityChanging(aura::Window* window, bool visible)
{
...
void ShadowController::Impl::OnWindowBoundsChanged(
    aura::Window* window, const gfx::Rect& old_bounds, const gfx::Rect& new_bounds, ui::PropertyChangeReason reason)
{
    ui::Shadow* shadow = GetShadowForWindow(window);
...
void ShadowController::Impl::OnWindowActivated(ActivationReason reason, aura::Window* gained_active, aura::Window* lost_active)
{
    if (gained_active) {
    ...
    if (lost_active) {
        ...
        if (shadow && GetShadowElevationConvertDefault(lost_active) == kShadowElevationInactiveWindow) {
            shadow->SetElevation(GetShadowElevationForWindowLosingActive(lost_active, gained_active));
        }
    }
}

bool ShadowController::Impl::ShouldShowShadowForWindow(aura::Window* window) const
{
    if (delegate_) {
    ...
    ui::mojom::WindowShowState show_state = window->GetProperty(aura::client::kShowStateKey);
    if (show_state == ui::mojom::WindowShowState::kFullscreen || show_state == ui::mojom::WindowShowState::kMaximized) {
        return false;
    }
...
void ShadowController::Impl::MaybeSetShadowRadiusForWindow(aura::Window* window) const
{
    ui::Shadow* shadow = GetShadowForWindow(window);
    CHECK(shadow);

    const int corner_radius = window->GetProperty(aura::client::kWindowCornerRadiusKey);
...
void ShadowController::Impl::HandlePossibleShadowVisibilityChange(aura::Window* window)
{
...
void ShadowController::Impl::CreateShadowForWindow(aura::Window* window)
{
    ...
    ui::Shadow* shadow = window->SetProperty(kShadowLayerKey, std::make_unique<ui::Shadow>());
...
// static
base::flat_set<ShadowController::Impl*>* ShadowController::Impl::GetInstances()
{
...
ui::Shadow::ElevationToColorsMap ShadowController::GenerateShadowColorsMap(const ui::ColorProvider* color_provider)
{
    ui::Shadow::ElevationToColorsMap color_map;
    color_map[kShadowElevationPopup] = std::make_pair(
        color_provider->GetColor(ui::kColorShadowValueKeyShadowElevationFour), color_provider->GetColor(ui::kColorShadowValueAmbientShadowElevationFour));
    color_map[kShadowElevationInactiveWindow] = std::make_pair(
        color_provider->GetColor(ui::kColorShadowValueKeyShadowElevationTwelve), color_provider->GetColor(ui::kColorShadowValueAmbientShadowElevationTwelve));
    color_map[kShadowElevationActiveWindow] = std::make_pair(color_provider->GetColor(ui::kColorShadowValueKeyShadowElevationTwentyFour),
        color_provider->GetColor(ui::kColorShadowValueAmbientShadowElevationTwentyFour));
    return color_map;
}

ShadowController::ShadowController(ActivationClient* activation_client, std::unique_ptr<ShadowControllerDelegate> delegate, aura::Env* env)
...
void ShadowController::OnWindowActivated(ActivationReason reason, aura::Window* gained_active, aura::Window* lost_active)
{
