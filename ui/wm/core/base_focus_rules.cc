...
bool BaseFocusRules::IsWindowConsideredVisibleForActivation(const aura::Window* window) const
{
...
bool BaseFocusRules::CanActivateWindow(const aura::Window* window) const
{
    ...
    if (GetActivationDelegate(window) && !GetActivationDelegate(window)->ShouldActivate()) {
        return false;
    }
...
bool BaseFocusRules::CanFocusWindow(const aura::Window* window, const ui::Event* event) const
{
...
const aura::Window* BaseFocusRules::GetToplevelWindow(const aura::Window* window) const
{
...
{
    return const_cast<aura::Window*>(GetActivatableWindow(const_cast<const aura::Window*>(window)));
}
...
aura::Window* BaseFocusRules::GetNextActivatableWindow(aura::Window* ignore) const
{
    DCHECK(ignore);
...
const aura::Window* BaseFocusRules::GetActivatableWindow(const aura::Window* window) const
{
    const aura::Window* parent = window->parent();
    const aura::Window* child = window;
    while (parent) {
        ...
        if (wm::GetTransientParent(child)) {
            ...
            const aura::Window* parent_modal_transient = GetModalTransient(wm::GetTransientParent(child));
