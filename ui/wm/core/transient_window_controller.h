...
class COMPONENT_EXPORT(UI_WM) TransientWindowController : public aura::client::TransientWindowClient {
public:
    ...
    static TransientWindowController* Get()
    {
        return instance_;
    }
    ...
    std::vector<raw_ptr<aura::Window, VectorExperimental>> GetTransientChildren(const aura::Window* parent) override;
    void AddObserver(aura::client::TransientWindowClientObserver* observer) override;
    void RemoveObserver(aura::client::TransientWindowClientObserver* observer) override;
...
private:
    ...
    base::ObserverList<aura::client::TransientWindowClientObserver>::Unchecked observers_;
};
