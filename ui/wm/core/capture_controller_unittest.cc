...
public:
    ...
    void SetDestroyOldCapture(bool destroy)
    {
        destroy_old_capture_ = destroy;
    }

    // aura::client::CaptureDelegate:
    void UpdateCapture(aura::Window* old_capture, aura::Window* new_capture) override
    {
...
public:
    ...
    void SetUp() override
    {
        ...
        second_host_ = aura::WindowTreeHost::Create(ui::PlatformWindowInitProperties { gfx::Rect(0, 0, 800, 600) });
        second_host_->InitHost();
        second_host_->window()->Show();
        second_host_->SetBoundsInPixels(gfx::Rect(800, 600));
        second_capture_client_ = std::make_unique<ScopedCaptureClient>(second_host_->window());
    }
    ...
    aura::Window* CreateNormalWindowWithBounds(int id, aura::Window* parent, const gfx::Rect& bounds, aura::WindowDelegate* delegate)
    {
...
TEST_F(CaptureControllerTest, ResetMouseEventHandlerOnCapture)
{
    ...
    std::unique_ptr<aura::Window> w1(CreateNormalWindow(1, root_window(), nullptr));

    // Make a synthesized mouse down event. Ensure that the WindowEventDispatcher
    // will dispatch further mouse events to |w1|.
    ui::MouseEvent mouse_pressed_event(ui::EventType::kMousePressed, gfx::Point(5, 5), gfx::Point(5, 5), ui::EventTimeForNow(), 0, 0);
    ...
    std::unique_ptr<aura::Window> w2(CreateNormalWindow(2, second_host_->window(), nullptr));
...
TEST_F(CaptureControllerTest, ReparentedWhileCaptured)
{
    ...
    ScopedCaptureClient::TestApi(capture_client_.get()).SetDelegate(delegate.get());
    std::unique_ptr<TestCaptureDelegate> delegate2(new TestCaptureDelegate);
    ScopedCaptureClient::TestApi(second_capture_client_.get()).SetDelegate(delegate2.get());

    std::unique_ptr<aura::Window> w(CreateNormalWindow(1, root_window(), nullptr));
    w->SetCapture();
...
class GestureEventDeleteWindowOnScrollEnd : public aura::test::TestWindowDelegate {
public:
    GestureEventDeleteWindowOnScrollEnd()
    {
    }

    GestureEventDeleteWindowOnScrollEnd(const GestureEventDeleteWindowOnScrollEnd&) = delete;
    GestureEventDeleteWindowOnScrollEnd& operator=(const GestureEventDeleteWindowOnScrollEnd&) = delete;
...
{
    std::unique_ptr<GestureEventDeleteWindowOnScrollEnd> delegate(new GestureEventDeleteWindowOnScrollEnd());
    const int kWindowWidth = 123;
    const int kWindowHeight = 45;
    gfx::Rect bounds(100, 200, kWindowWidth, kWindowHeight);
    std::unique_ptr<aura::Window> window1(CreateNormalWindowWithBounds(-1235, root_window(), bounds, nullptr));

    bounds.Offset(0, 100);
    std::unique_ptr<aura::Window> window2(CreateNormalWindowWithBounds(-1234, root_window(), bounds, delegate.get()));
    ...
    ui::MouseEvent mouse_press(ui::EventType::kMousePressed, gfx::Point(), gfx::Point(), base::TimeTicks(), 0, 0);
...
TEST_F(CaptureControllerTest, UpdateCaptureDestroysOldCaptureWindow)
{
    ...
    ScopedCaptureClient::TestApi(second_capture_client_.get()).SetDelegate(&delegate2);
    ...
    {
        // Create a window inside the WindowEventDispatcher.
        std::unique_ptr<aura::Window> capture_window(CreateNormalWindow(1, root_window(), nullptr));
        ui::test::EventGenerator event_generator(root_window());
        event_generator.PressTouch();
