...
public:
    ...
    TestTransientWindowObserver& operator=(const TestTransientWindowObserver&) = delete;
    ...
    int parent_change_count() const
    {
        return parent_change_count_;
    }
...
public:
    WindowVisibilityObserver(Window* observed_window, std::unique_ptr<Window> owned_window)
...
public:
    ...
    TransientWindowManagerTest& operator=(const TransientWindowManagerTest&) = delete;
...
protected:
    ...
    Window* CreateTransientChild(int id, Window* parent)
    {
        ...
        aura::client::ParentWindowWithContext(window, root_window(), gfx::Rect(), display::kInvalidDisplayId);
        return window;
    }
};
...
TEST_F(TransientWindowManagerTest, TransientChildren)
{
    ...
    wm::TransientWindowManager::GetOrCreate(w2)->set_parent_controls_visibility(true);
    w1->Hide();
    EXPECT_FALSE(w2->IsVisible());
    w2->Show();
    EXPECT_FALSE(w2->IsVisible());
...
TEST_F(TransientWindowManagerTest, CrashOnVisibilityChange)
{
    ...
    WindowVisibilityObserver visibility_observer(window1.get(), std::move(window2));
    root_window()->Hide();
}
...
public:
    explicit DestroyedTrackingDelegate(const std::string& name, std::vector<std::string>* results)
        : name_(name)
        , results_(results)
    {
    }
    ...
    DestroyedTrackingDelegate& operator=(const DestroyedTrackingDelegate&) = delete;
...
TEST_F(TransientWindowManagerTest, StackTransientsLayersRelativeToOtherTransients)
{
...
TEST_F(TransientWindowManagerTest, TransientWindowObserverNotified)
{
    ...
    TransientWindowManager::GetOrCreate(parent.get())->AddObserver(&test_parent_observer);
    TransientWindowManager::GetOrCreate(w1.get())->AddObserver(&test_child_observer);
    ...
    TransientWindowManager::GetOrCreate(parent.get())->RemoveObserver(&test_parent_observer);
    TransientWindowManager::GetOrCreate(parent.get())->RemoveObserver(&test_child_observer);
}
...
// manager.
TEST_F(TransientWindowManagerTest, TransientLifeTimeMayBeControlledByTransientParent)
{
    ...
    std::unique_ptr<Window> new_transient(CreateTransientChild(3, new_parent.get()));
    ...
    wm::TransientWindowManager::GetOrCreate(new_transient.get())->set_parent_controls_lifetime(false);
    new_parent.reset();
