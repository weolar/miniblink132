...
{
    if (window->GetProperty(aura::client::kModalKey) != ui::mojom::ModalType::kWindow) {
        return;
    }
...
FocusController::FocusController(FocusRules* rules)
    : rules_(rules)
    , focus_follows_cursor_(base::FeatureList::IsEnabled(features::kFocusFollowsCursor))
{
    DCHECK(rules);
}
...
aura::Window* FocusController::GetActivatableWindow(aura::Window* window) const
{
    return rules_->GetActivatableWindow(window);
}

const aura::Window* FocusController::GetToplevelWindow(const aura::Window* window) const
{
...
void FocusController::RemoveObserver(aura::client::FocusChangeObserver* observer)
{
...
{
    FocusAndActivateWindow(ActivationChangeObserver::ActivationReason::ACTIVATION_CLIENT, window,
        /*no_stacking=*/false);
}
...
{
    if ((event->type() == ui::EventType::kMousePressed || (event->type() == ui::EventType::kMouseEntered && focus_follows_cursor_)) && !event->handled()) {
        WindowFocusedFromInputEvent(static_cast<aura::Window*>(event->target()), event);
    }
}
...
{
    if (event->type() == ui::EventType::kGestureBegin && event->details().touch_points() == 1 && !event->handled()) {
        WindowFocusedFromInputEvent(static_cast<aura::Window*>(event->target()), event);
    }
}
...
void FocusController::OnWindowVisibilityChanged(aura::Window* window, bool visible)
{
    if (!visible)
...
void FocusController::OnWindowHierarchyChanging(const HierarchyChangeParams& params)
{
    if (params.receiver == active_window_ && params.target->Contains(params.receiver)
        && (!params.new_parent || aura::client::GetFocusClient(params.new_parent) != aura::client::GetFocusClient(params.receiver))) {
        WindowLostFocusFromDispositionChange(params.receiver, params.old_parent);
    }
}

void FocusController::OnWindowHierarchyChanged(const HierarchyChangeParams& params)
{
    if (params.receiver == focused_window_ && params.target->Contains(params.receiver)
        && (!params.new_parent || aura::client::GetFocusClient(params.new_parent) != aura::client::GetFocusClient(params.receiver))) {
        WindowLostFocusFromDispositionChange(params.receiver, params.old_parent);
    }
}
...
void FocusController::FocusAndActivateWindow(ActivationChangeObserver::ActivationReason reason, aura::Window* window, bool no_stacking)
{
    ...
    aura::Window* activatable = focusable ? rules_->GetActivatableWindow(focusable) : nullptr;
    ...
    if (!updating_focus_) {
        aura::Window* const new_active_window = pending_activation_.has_value() ? pending_activation_.value() : active_window_.get();
        const bool activation_changed_focus = last_focused_window != focused_window_;
...
void FocusController::SetFocusedWindow(aura::Window* window)
{
    ...
    if (focused_window_ && observation_manager_.IsObservingSource(focused_window_.get()) && focused_window_ != active_window_) {
        observation_manager_.RemoveObservation(focused_window_.get());
    }
    focused_window_ = window;
    if (focused_window_ && !observation_manager_.IsObservingSource(focused_window_.get()))
        observation_manager_.AddObservation(focused_window_.get());

    for (auto& observer : focus_observers_) {
        observer.OnWindowFocused(focused_window_, window_tracker.Contains(lost_focus) ? lost_focus : nullptr);
    }
...
bool FocusController::SetActiveWindow(ActivationChangeObserver::ActivationReason reason, aura::Window* requested_window, aura::Window* window, bool no_stacking)
{
    if (pending_activation_)
        return false;
    ...
    base::AutoReset<std::optional<aura::Window*>> updating_activation(&pending_activation_, std::make_optional(window));
    ...
    if (active_window_ && observation_manager_.IsObservingSource(active_window_.get()) && focused_window_ != active_window_) {
    ...
    if (observer) {
        observer->OnWindowActivated(reason, active_window_, window_tracker.Contains(lost_activation) ? lost_activation : nullptr);
    }

    MAYBE_ACTIVATION_INTERRUPTED();

    for (auto& activation_observer : activation_observers_) {
        activation_observer.OnWindowActivated(reason, active_window_, window_tracker.Contains(lost_activation) ? lost_activation : nullptr);
...
void FocusController::StackActiveWindow()
{
    ...
    for (const auto child_window : active_window_->parent()->children()) {
        ...
        if (has_found_window != HasTransientAncestor(child_window, active_window_)) {
            needs_restack = true;
            break;
        }
    }
...
void FocusController::WindowLostFocusFromDispositionChange(aura::Window* window, aura::Window* next)
{
    ...
    const bool is_pending_window_losing_focus = pending_activation_ && (window == pending_activation_.value());
    if (is_active_window_losing_focus || is_pending_window_losing_focus) {
        ...
        if (!SetActiveWindow(ActivationChangeObserver::ActivationReason::WINDOW_DISPOSITION_CHANGED, nullptr, next_activatable,
                /*no_stacking=*/false)) {
            return;
        }

        if (window == focused_window_ || !active_window_ || !active_window_->Contains(focused_window_)) {
...
void FocusController::WindowFocusedFromInputEvent(aura::Window* window, const ui::Event* event)
{
    ...
    if (is_mouse_entered_event && active_window_ && active_window_->Contains(window)) {
        return;
    }
    ...
    if (rules_->CanFocusWindow(GetToplevelWindow(window), event)) {
        FocusAndActivateWindow(ActivationChangeObserver::ActivationReason::INPUT_EVENT, window,
