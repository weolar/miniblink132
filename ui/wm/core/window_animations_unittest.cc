...
template <typename T> int GetZPosition(const T* child)
{
    ...
    const std::vector<raw_ptr<T, VectorExperimental>> children = parent->children();
...
{
    std::unique_ptr<aura::Window> window(aura::test::CreateTestWindowWithId(0, NULL));
...
{
    std::unique_ptr<aura::Window> parent(aura::test::CreateTestWindowWithId(0, NULL));

    std::unique_ptr<aura::Window> other(aura::test::CreateTestWindowWithId(1, parent.get()));

    std::unique_ptr<aura::Window> animating_window(aura::test::CreateTestWindowWithId(2, parent.get()));
    ...
    {
        ui::ScopedAnimationDurationScaleMode scale_mode(ui::ScopedAnimationDurationScaleMode::FAST_DURATION);
        ui::Layer* animating_layer = animating_window->layer();

        animating_window->Hide();
        EXPECT_TRUE(AnimateOnChildWindowVisibilityChanged(animating_window.get(), false));
        ...
        EXPECT_TRUE(base::Contains(parent->layer()->children(), animating_window->layer()));
...
TEST_F(WindowAnimationsTest, HideAnimationDetachLayersWithTransientChildren)
{
    ...
    std::unique_ptr<aura::Window> transient1(aura::test::CreateTestWindowWithId(3, parent.get()));
    std::unique_ptr<aura::Window> transient2(aura::test::CreateTestWindowWithId(4, parent.get()));
...
public:
    ...
    NotifyHideCompletedAnimationHost(const NotifyHideCompletedAnimationHost&) = delete;
    NotifyHideCompletedAnimationHost& operator=(const NotifyHideCompletedAnimationHost&) = delete;
    ...
    void SetHostTransitionOffsets(const gfx::Vector2d& top_left, const gfx::Vector2d& bottom_right) override
    {
    }

    bool hide_completed() const
    {
        return hide_completed_;
    }
...
TEST_F(WindowAnimationsTest, NotifyHideCompleted)
{
    ...
    wm::SetWindowVisibilityAnimationType(window.get(), WINDOW_VISIBILITY_ANIMATION_TYPE_FADE);
    AnimateOnChildWindowVisibilityChanged(window.get(), true);
...
TEST_F(WindowAnimationsTest, RotateHideNoLeak)
{
    ...
    std::unique_ptr<aura::Window> window(aura::test::CreateTestWindowWithId(0, nullptr));
    ui::Layer* animating_layer = window->layer();
    wm::SetWindowVisibilityAnimationType(window.get(), WINDOW_VISIBILITY_ANIMATION_TYPE_ROTATE);
...
TEST_F(WindowAnimationsTest, RotateHideNoCrash)
{
    ...
    wm::SetWindowVisibilityAnimationType(window.get(), WINDOW_VISIBILITY_ANIMATION_TYPE_ROTATE);
    AnimateOnChildWindowVisibilityChanged(window.get(), true);
    window->layer()->GetAnimator()->Step(base::TimeTicks::Now() + base::Seconds(5));
    AnimateOnChildWindowVisibilityChanged(window.get(), false);
    animating_layer->GetAnimator()->StopAnimating();
}
