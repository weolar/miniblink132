...
// animated.
DEFINE_UI_CLASS_PROPERTY_KEY(bool, kChildWindowVisibilityChangesAnimatedKey, false)
...
{
    return (window->parent() && window->parent()->GetProperty(kChildWindowVisibilityChangesAnimatedKey))
...
bool VisibilityController::CallAnimateOnChildWindowVisibilityChanged(aura::Window* window, bool visible)
{
    return AnimateOnChildWindowVisibilityChanged(window, visible);
}

void VisibilityController::UpdateLayerVisibility(aura::Window* window, bool visible)
{
    bool animated
        = window->GetType() != aura::client::WINDOW_TYPE_CONTROL && window->GetType() != aura::client::WINDOW_TYPE_UNKNOWN && ShouldAnimateWindow(window);
...
SuspendChildWindowVisibilityAnimations::SuspendChildWindowVisibilityAnimations(aura::Window* window)
    : window_(window)
    , original_enabled_(window->GetProperty(kChildWindowVisibilityChangesAnimatedKey))
{
