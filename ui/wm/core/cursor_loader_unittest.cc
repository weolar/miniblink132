...
TEST(CursorLoaderTest, InvisibleCursor)
{
    ...
    EXPECT_EQ(invisible_cursor.platform(), ui::CursorFactory::GetInstance()->GetDefaultCursor(CursorType::kNone));
}
...
TEST(CursorLoaderTest, GetCursorData)
{
    ...
    for (const float scale : { 0.8f, 1.0f, 1.25f, 1.5f, 2.0f, 3.0f }) {
        ...
        const float resource_scale = ui::GetScaleForResourceScaleFactor(ui::GetSupportedResourceScaleFactor(scale));
        for (const ui::CursorSize cursor_size : { ui::CursorSize::kNormal, ui::CursorSize::kLarge }) {
            SCOPED_TRACE(testing::Message() << "size " << static_cast<int>(cursor_size));
            cursor_loader.SetSize(cursor_size);

            const ui::Cursor invisible_cursor = CursorType::kNone;
            std::optional<ui::CursorData> cursor_loader_data = cursor_loader.GetCursorData(invisible_cursor);
            ...
            for (const ui::Cursor cursor : { CursorType::kPointer, CursorType::kWait }) {
                SCOPED_TRACE(cursor.type());
                cursor_loader_data = cursor_loader.GetCursorData(cursor);
                ASSERT_TRUE(cursor_loader_data);
                const auto cursor_data = GetCursorData(cursor.type(), cursor_size, resource_scale, std::nullopt, display.panel_rotation());
                ASSERT_TRUE(cursor_data);
                ASSERT_EQ(cursor_loader_data->bitmaps.size(), cursor_data->bitmaps.size());
                for (size_t i = 0; i < cursor_data->bitmaps.size(); i++) {
                    EXPECT_TRUE(gfx::BitmapsAreEqual(cursor_loader_data->bitmaps[i], cursor_data->bitmaps[i]));
                }
    ...
    std::optional<ui::CursorData> cursor_data = cursor_loader.GetCursorData(custom_cursor);
    ASSERT_TRUE(cursor_data);
    EXPECT_EQ(cursor_data->bitmaps[0].getGenerationID(), kBitmap.getGenerationID());
