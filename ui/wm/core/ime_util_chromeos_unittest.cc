...
TEST_F(ImeUtilChromeosTest, RestoreWindowBounds)
{
    ...
    aura::Window* window = aura::test::CreateTestWindowWithBounds(bounds, root_window());
...
TEST_F(ImeUtilChromeosTest, EnsureWindowNotInRect_MoveUp)
{
    ...
    aura::Window* window = aura::test::CreateTestWindowWithBounds(original_bounds, root_window());
    ...
    EXPECT_FALSE(gfx::IntersectRects(window->GetBoundsInScreen(), rect).IsEmpty());
    EnsureWindowNotInRect(window, rect);
...
TEST_F(ImeUtilChromeosTest, EnsureWindowNotInRect_MoveToTop)
{
    ...
    test_screen()->SetWorkAreaInsets(gfx::Insets::TLBR(kOccupiedTopHeight, 0, 0, 0));
    EnsureWindowNotInRect(window, rect);
