...
bool CursorLoader::SetDisplay(const display::Display& display)
{
    ...
    rotation_ = rotation;
    scale_ = scale;
    resource_scale_ = ui::GetScaleForResourceScaleFactor(ui::GetSupportedResourceScaleFactor(scale_));
...
std::optional<ui::CursorData> CursorLoader::GetCursorData(const ui::Cursor& cursor) const
{
    ...
    if (type == CursorType::kCustom) {
        return ui::CursorData({ cursor.custom_bitmap() }, cursor.custom_hotspot(), cursor.image_scale_factor());
    }
    ...
    if (use_platform_cursors_) {
        ...
        if (cursor_data) {
            ...
            return ui::CursorData(std::move(cursor_data->bitmaps), std::move(cursor_data->hotspot), scale_);
        }
    }
    ...
    return wm::GetCursorData(type, size_, resource_scale_, std::nullopt, display::Display::ROTATE_0);
}

scoped_refptr<ui::PlatformCursor> CursorLoader::CursorFromType(CursorType type)
{
...
scoped_refptr<ui::PlatformCursor> CursorLoader::LoadCursorFromAsset(CursorType type)
{
    std::optional<ui::CursorData> cursor_data = wm::GetCursorData(type, size_, resource_scale_, std::nullopt, rotation_);
    if (!cursor_data) {
        return nullptr;
    }

    if (cursor_data->bitmaps.size() == 1) {
        image_cursors_[type] = factory_->CreateImageCursor(type, cursor_data->bitmaps[0], cursor_data->hotspot, cursor_data->scale_factor);
    } else {
        image_cursors_[type]
            = factory_->CreateAnimatedCursor(type, cursor_data->bitmaps, cursor_data->hotspot, cursor_data->scale_factor, kAnimatedCursorFrameDelay);
    }
    return image_cursors_[type];
}
