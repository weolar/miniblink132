...
// static
gfx::NativeCursor CompoundEventFilter::CursorForWindowComponent(int window_component)
{
...
gfx::NativeCursor CompoundEventFilter::NoResizeCursorForWindowComponent(int window_component)
{
    switch (window_component) {
    case HTBOTTOM:
...
void CompoundEventFilter::UpdateCursor(aura::Window* target, ui::MouseEvent* event)
{
    ...
    if (cursor_client) {
        ...
        if ((event->flags() & ui::EF_IS_NON_CLIENT)) {
            if (target->delegate()) {
                ...
                if ((target->GetProperty(aura::client::kResizeBehaviorKey) & aura::client::kResizeBehaviorCanResize) != 0) {
...
void CompoundEventFilter::SetCursorVisibilityOnEvent(aura::Window* target, ui::Event* event, bool show)
{
...
void CompoundEventFilter::SetMouseEventsEnableStateOnEvent(aura::Window* target, ui::Event* event, bool enable)
{
    TRACE_EVENT2("ui,input", "CompoundEventFilter::SetMouseEventsEnableStateOnEvent", "event_flags", event->flags(), "enable", enable);
    if (event->flags() & ui::EF_IS_SYNTHESIZED)
        return;
    aura::client::CursorClient* client = aura::client::GetCursorClient(target->GetRootWindow());
    if (!client) {
        TRACE_EVENT_INSTANT0("ui,input", "CompoundEventFilter::SetMouseEventsEnableStateOnEvent - No Client", TRACE_EVENT_SCOPE_THREAD);
        return;
    }
...
{
    TRACE_EVENT2("ui,input", "CompoundEventFilter::OnMouseEvent", "event_type", event->type(), "event_flags", event->flags());
    ...
    if (!(event->flags() & ui::EF_FROM_TOUCH)
        && (event->type() == ui::EventType::kMouseEntered || event->type() == ui::EventType::kMouseMoved || event->type() == ui::EventType::kMousePressed
...
{
    TRACE_EVENT2("ui,input", "CompoundEventFilter::OnTouchEvent", "event_type", event->type(), "event_handled", event->handled());
    ...
    if (!event->handled() && event->type() == ui::EventType::kTouchPressed) {
        ...
        if (client && client->ShouldHideCursorOnTouchEvent(*event) && !aura::Env::GetInstance()->IsMouseButtonDown()) {
