...
AcceleratorFilter::AcceleratorFilter(std::unique_ptr<AcceleratorDelegate> delegate)
...
bool AcceleratorFilter::ShouldFilter(ui::KeyEvent* event)
{
    ...
    if (!event->target() || (type != ui::EventType::kKeyPressed && type != ui::EventType::kKeyReleased) || event->is_char() || !event->target() ||
