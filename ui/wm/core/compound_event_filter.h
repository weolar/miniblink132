...
public:
    ...
    static gfx::NativeCursor NoResizeCursorForWindowComponent(int window_component);
...
private:
    ...
    void SetCursorVisibilityOnEvent(aura::Window* target, ui::Event* event, bool show);

    // Enables or disables mouse events if the event is not synthesized.
    void SetMouseEventsEnableStateOnEvent(aura::Window* target, ui::Event* event, bool enable);
