...
DEFINE_OWNED_UI_CLASS_PROPERTY_KEY(TransientWindowManager, kPropertyKey, nullptr)
...
bool HasTransientCycles(const aura::Window* window)
{
    ...
    while (window) {
        ...
        auto* transient_window_manager = TransientWindowManager::GetIfExists(window);
        window = transient_window_manager ? transient_window_manager->transient_parent() : nullptr;
    }
    return false;
}
...
TransientWindowManager* TransientWindowManager::GetOrCreate(Window* window)
{
    ...
    return window->SetProperty(kPropertyKey, base::WrapUnique(new TransientWindowManager(window)));
}

// static
const TransientWindowManager* TransientWindowManager::GetIfExists(const Window* window)
{
...
void TransientWindowManager::AddTransientChild(Window* child)
{
    ...
    TransientWindowController::Get()->observers_.Notify(&aura::client::TransientWindowClientObserver::OnTransientChildWindowAdded, window_, child);
    ...
    observers_.Notify(&TransientWindowObserver::OnTransientChildAdded, window_, child);
    child_manager->observers_.Notify(&TransientWindowObserver::OnTransientParentChanged, window_);
}
...
void TransientWindowManager::RemoveTransientChild(Window* child)
{
    ...
    TransientWindowController::Get()->observers_.Notify(&aura::client::TransientWindowClientObserver::OnTransientChildWindowRemoved, window_, child);
    ...
    observers_.Notify(&TransientWindowObserver::OnTransientChildRemoved, window_, child);
    child_manager->observers_.Notify(&TransientWindowObserver::OnTransientParentChanged, nullptr);
}

bool TransientWindowManager::IsStackingTransient(const aura::Window* target) const
{
...
void TransientWindowManager::RestackTransientDescendants()
{
    ...
    aura::WindowTracker tracker(Window::Windows(parent->children().rbegin(), parent->children().rend()));
    while (!tracker.windows().empty()) {
        auto* child_window = tracker.Pop();
        if (child_window != window_ && HasTransientAncestor(child_window, window_)) {
            TransientWindowManager* descendant_manager = GetOrCreate(child_window);
            base::AutoReset<raw_ptr<Window>> resetter(&descendant_manager->stacking_target_, window_);
            parent->StackChildAbove(child_window, window_);
        }
    }
}

void TransientWindowManager::OnWindowHierarchyChanged(const HierarchyChangeParams& params)
{
    ...
    if (transient_parent_ && transient_parent_->parent() == new_parent) {
        TransientWindowManager* transient_parent_manager = GetOrCreate(transient_parent_);
        transient_parent_manager->RestackTransientDescendants();
    }
}

void TransientWindowManager::UpdateTransientChildVisibility(bool parent_visible)
{
...
void TransientWindowManager::OnWindowDestroying(Window* window)
{
    ...
    if (transient_parent_) {
        TransientWindowManager::GetOrCreate(transient_parent_)->RemoveTransientChild(window_);
    }
    ...
    // iterating.
    aura::WindowTracker tracker(Window::Windows(transient_children_.begin(), transient_children_.end()));
    while (!tracker.windows().empty()) {
