...
class COMPONENT_EXPORT(UI_WM) CursorLoader : public aura::client::CursorShapeClient, public ui::CursorFactoryObserver {
public:
    ...
    std::optional<ui::CursorData> GetCursorData(const ui::Cursor& cursor) const override;

private:
    // Resets the cursor cache.
    void UnloadCursors();
    scoped_refptr<ui::PlatformCursor> CursorFromType(ui::mojom::CursorType type);
    scoped_refptr<ui::PlatformCursor> LoadCursorFromAsset(ui::mojom::CursorType type);
    ...
    std::map<ui::mojom::CursorType, scoped_refptr<ui::PlatformCursor>> image_cursors_;
