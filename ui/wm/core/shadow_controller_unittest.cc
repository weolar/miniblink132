...
protected:
    ShadowController* shadow_controller()
    {
    ...
    void InstallShadowController(std::unique_ptr<ShadowControllerDelegate> delegate)
    {
        shadow_controller_ = std::make_unique<ShadowController>(GetActivationClient(root_window()), std::move(delegate));
    }
...
TEST_F(ShadowControllerTest, ShadowBounds)
{
    ...
    const ui::Shadow* shadow = ShadowController::GetShadowForWindow(window.get());
    ASSERT_TRUE(shadow != NULL);
    EXPECT_EQ(gfx::Rect(kOldBounds.size()).ToString(), shadow->content_bounds().ToString());
    ...
    EXPECT_EQ(gfx::Rect(kNewBounds.size()).ToString(), shadow->content_bounds().ToString());
}
...
TEST_F(ShadowControllerTest, ShadowBoundsDetached)
{
    ...
    std::unique_ptr<aura::Window> window(aura::test::CreateTestWindowWithBounds(kInitialBounds, root_window()));
    window->Show();
    const ui::Shadow* shadow = ShadowController::GetShadowForWindow(window.get());
    ASSERT_TRUE(shadow);
...
TEST_F(ShadowControllerTest, ShowState)
{
    ...
    window->SetProperty(aura::client::kShowStateKey, ui::mojom::WindowShowState::kMaximized);
    EXPECT_FALSE(shadow->layer()->visible());

    window->SetProperty(aura::client::kShowStateKey, ui::mojom::WindowShowState::kNormal);
    EXPECT_TRUE(shadow->layer()->visible());
...
TEST_F(ShadowControllerTest, SmallShadowsForTooltipsAndMenus)
{
    ...
    ui::Shadow* tooltip_shadow = ShadowController::GetShadowForWindow(tooltip_window.get());
    ...
    ui::Shadow* menu_shadow = ShadowController::GetShadowForWindow(tooltip_window.get());
...
TEST_F(ShadowControllerTest, SetColorsMapToShadow)
{
    ...
    shadow->SetElevationToColorsMap(ShadowController::GenerateShadowColorsMap(&color_provider));
...
public:
    ...
    TestShadowControllerDelegate& operator=(const TestShadowControllerDelegate&) = delete;
    ...
    bool ShouldUpdateShadowOnWindowPropertyChange(const aura::Window* window, const void* key, intptr_t old) override
    {
        return false;
    }
...
TEST_F(ShadowControllerTest, UpdateShadowWhenAddedToParent)
{
    ...
    EXPECT_TRUE(ShadowController::GetShadowForWindow(window1.get())->layer()->visible());
}
