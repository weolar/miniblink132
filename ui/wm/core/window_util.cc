...
void CloneChildren(ui::Layer* to_clone, ui::Layer* parent, const wm::MapLayerFunc& map_func)
{
...
//
// WARNING: It is assumed that |parent| is ultimately owned by a LayerTreeOwner.
void MirrorChildren(ui::Layer* to_mirror, ui::Layer* parent, bool sync_bounds)
{
...
void SetWindowFullscreen(aura::Window* window, bool fullscreen, int64_t target_display_id)
{
    DCHECK(window);
    // Should only specify display id when entering fullscreen.
    DCHECK(target_display_id == display::kInvalidDisplayId || fullscreen);

    ui::mojom::WindowShowState current_show_state = window->GetProperty(aura::client::kShowStateKey);
    const bool is_fullscreen = current_show_state == ui::mojom::WindowShowState::kFullscreen;
    if (fullscreen == is_fullscreen && target_display_id == display::kInvalidDisplayId) {
        return;
    }
    if (fullscreen) {
        ...
        int64_t current_display_id = window->GetRootWindow() ? display::Screen::GetScreen()->GetDisplayNearestWindow(window).id() : display::kInvalidDisplayId;
        ...
        if (current_show_state != ui::mojom::WindowShowState::kMinimized) {
            window->SetProperty(aura::client::kRestoreShowStateKey, current_show_state);
        }
        // Set fullscreen display id first, so it's available when the show state
        // property change is processed.
        window->SetProperty(aura::client::kFullscreenTargetDisplayIdKey, target_display_id);
        window->SetProperty(aura::client::kShowStateKey, ui::mojom::WindowShowState::kFullscreen);
...
bool WindowStateIs(const aura::Window* window, ui::mojom::WindowShowState state)
{
...
void Restore(aura::Window* window)
{
    ...
    window->SetProperty(aura::client::kShowStateKey, window->GetProperty(aura::client::kRestoreShowStateKey));
    window->ClearProperty(aura::client::kIsRestoringKey);
}

void Unminimize(aura::Window* window)
{
    DCHECK_EQ(window->GetProperty(aura::client::kShowStateKey), ui::mojom::WindowShowState::kMinimized);
    Restore(window);
}
...
{
    return const_cast<aura::Window*>(GetToplevelWindow(const_cast<const aura::Window*>(window)));
}
...
std::unique_ptr<ui::LayerTreeOwner> RecreateLayers(ui::LayerOwner* root)
{
    ...
    return RecreateLayersWithClosure(root, base::BindRepeating([](ui::LayerOwner* owner) { return owner->RecreateLayer(); }));
}

std::unique_ptr<ui::LayerTreeOwner> RecreateLayersWithClosure(ui::LayerOwner* root, const MapLayerFunc& map_func)
{
    DCHECK(root->OwnsLayer());
...
std::unique_ptr<ui::LayerTreeOwner> MirrorLayers(ui::LayerOwner* root, bool sync_bounds)
{
...
{
    return const_cast<aura::Window*>(GetTransientParent(const_cast<const aura::Window*>(window)));
}

const aura::Window* GetTransientParent(const aura::Window* window)
{
    const TransientWindowManager* manager = TransientWindowManager::GetIfExists(window);
    return manager ? manager->transient_parent() : nullptr;
}

const std::vector<raw_ptr<aura::Window, VectorExperimental>>& GetTransientChildren(const aura::Window* window)
{
    const TransientWindowManager* manager = TransientWindowManager::GetIfExists(window);
    if (manager)
        return manager->transient_children();

    static std::vector<raw_ptr<aura::Window, VectorExperimental>>* shared = new std::vector<raw_ptr<aura::Window, VectorExperimental>>;
    return *shared;
}
...
bool HasTransientAncestor(const aura::Window* window, const aura::Window* ancestor)
{
