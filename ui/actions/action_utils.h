...
template <typename A> bool IsActionItemClass(actions::ActionItem* action_item)
{
    return ui::metadata::IsClass<A, actions::ActionItem>(action_item);
}

template <typename A> std::unique_ptr<A> ToActionItemClass(std::unique_ptr<actions::ActionItem> action_item)
{
