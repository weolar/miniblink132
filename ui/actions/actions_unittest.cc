...
DEFINE_UI_CLASS_PROPERTY_KEY(ContextValues, kContextValueKey, ContextValues::kContextNone)
...
protected:
    ...
    void SetupInitializer()
    {
        ...
        initialization_subscription_ = manager.AppendActionItemInitializer(base::BindRepeating(&ActionManagerTest::InitializeActions, base::Unretained(this)));
    }

    void TearDown() override
    {
        ActionIdMap::ResetMapsForTesting();
    }
...
TEST_F(ActionManagerTest, InitializerTest)
{
    ...
    auto& manager = ActionManager::GetForTesting();
    auto subscription
        = manager.AppendActionItemInitializer(base::BindRepeating([](bool* called, ActionManager* manager) { *called = true; }, &initializer_called));
...
TEST_F(ActionManagerTest, ActionRegisterAndInvoke)
{
    ...
    auto subscription = manager.AppendActionItemInitializer(base::BindRepeating(
        [](int* invoked_count, const std::u16string& text, ActionManager* manager) {
            auto action = std::make_unique<ActionItem>(base::BindRepeating(
                [](int* invoked_count, actions::ActionItem* action, ActionInvocationContext context) {
                    ++*invoked_count;
...
TEST_F(ActionIdMapTest, MapBetweenEnumAndString)
{
    ...
    auto actual_action_id = ActionIdMap::StringToActionId(actual_action_string.value());
...
{
    auto test_action_map = base::flat_map<ActionId, std::string>(std::vector<std::pair<ActionId, std::string>> { TEST_ACTION_IDS });
...
TEST_F(ActionItemTest, TestItemBatchUpdate)
{
  ...
  auto builder = ActionItem::Builder()
    ...
    auto changed_subscription
        = root_action->AddActionChangedCallback(base::BindRepeating([](bool* action_item_changed) { *action_item_changed = true; }, &action_item_changed));
    {
...
TEST_F(ActionItemTest, TestGroupIdExclusion)
{
  ...
  auto builder = ActionItem::Builder()
    ...
    // clang-format on
    auto& manager = ActionManager::GetForTesting();
    manager.AddActions(std::move(builder).Build(), ActionItem::Builder().SetActionId(kActionTest4).Build());
    ASSERT_TRUE(action_test2);
    ASSERT_TRUE(action_test2);
...
TEST_F(ActionItemTest, TestActionItemPinnableKey)
{
  ...
  auto builder = ActionItem::Builder()
    ...
    // test using builder
    builder = ActionItem::Builder().SetText(kActionText).SetActionId(kActionTest2).SetProperty(kActionItemPinnableKey, true).SetVisible(true).SetEnabled(true);
...
TEST_F(ActionItemTest, TextActionInvocationContext)
{
  ...
  auto builder = ActionItem::Builder(
    ...
    manager.AddAction(std::move(builder).Build());
    ASSERT_TRUE(action_test1);
    action_test1->InvokeAction(ActionInvocationContext::Builder().SetProperty(kContextValueKey, ContextValues::kContextKeyboard).Build());
