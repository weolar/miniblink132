...
public:
    ...
    ~ActionList();
    const ActionListVector& children() const
    {
        return children_;
    }
    bool empty() const
...
class COMPONENT_EXPORT(ACTIONS) BaseAction : public ui::metadata::MetaDataProvider, public ActionList::Delegate, public ui::PropertyHandler {
public:
    ...
    const ActionList& GetChildren() const
    {
        return children_;
    }
...
// information.
class COMPONENT_EXPORT(ACTIONS) ActionInvocationContext : public ui::PropertyHandler {
public:
    ...
    public:
        ...
        template <typename T> ContextBuilder&& SetProperty(const ui::ClassProperty<T>* property, ui::metadata::ArgType<T> value) &&
        {
    ...
    private:
        ...
        ContextBuilder();
        std::unique_ptr<ActionInvocationContext> context_ = std::make_unique<ActionInvocationContext>();
    };

    static ContextBuilder Builder();
};

template <typename BuilderT, typename ActionItemClass> class BaseActionItemBuilderT {
public:
    using ChildList = std::vector<std::unique_ptr<BuilderT>>;
    using ActionChangedCallback = ui::metadata::PropertyChangedCallback;
    using InvokeActionCallback = base::RepeatingCallback<void(ActionItem*, ActionInvocationContext)>;
    ...
    template <typename Child, typename... Types> BuilderT& AddChildren(Child&& child, Types&&... args) &
    {
    ...
    template <typename ActionPtr> BuilderT& CopyAddressTo(ActionPtr* action_address) &
    {
    ...
    template <typename Action> BuilderT& CopyWeakPtrTo(base::WeakPtr<Action>* weak_ptr) &
    {
    ...
    template <typename T> BuilderT& SetProperty(const ui::ClassProperty<T>* property, ui::metadata::ArgType<T> value) &
    {
...
protected:
    template <typename... Args> BuilderT& AddChildrenImpl(Args*... args) &
    {
...
public:
    ...
    class COMPONENT_EXPORT(ACTIONS) ActionItemBuilder : public BaseActionItemBuilderT<ActionItemBuilder, ActionItem> {
    ...
    [[nodiscard]] base::CallbackListSubscription AddActionChangedCallback(ActionChangedCallback callback);
    ...
    // Invoke an action.
    void InvokeAction(ActionInvocationContext context = ActionInvocationContext());
...
public:
    ...
    class StatefulImageActionItemBuilder : public BaseActionItemBuilderT<StatefulImageActionItemBuilder, StatefulImageActionItem> {
    public:
        using BaseActionItemBuilderT::BaseActionItemBuilderT;

        StatefulImageActionItemBuilder& SetStatefulImage(const ui::ImageModel& image) &
        {
...
class COMPONENT_EXPORT(ACTIONS) ActionManager : public ui::metadata::MetaDataProvider {
public:
    METADATA_HEADER_BASE(ActionManager);

    using ActionItemInitializerList = base::RepeatingCallbackList<void(ActionManager*)>;
    ...
    ActionItem* FindAction(const ui::KeyEvent& key_event, ActionItem* scope = nullptr);
    void GetActions(ActionItemVector& items, ActionItem* scope = nullptr);

    ActionItem* AddAction(std::unique_ptr<ActionItem> action_item);
    std::unique_ptr<ActionItem> RemoveAction(ActionItem* action_item);
    template <typename Action, typename... Types> void AddActions(Action&& action, Types&&... args) &
    {
    ...
    [[nodiscard]] base::CallbackListSubscription AppendActionItemInitializer(ActionItemInitializerList::CallbackType initializer);
...
private:
    template <typename... Args> void AddActionsImpl(Args*... args)
    {
...
public:
    ...
    static std::optional<ActionId> StringToActionId(const std::string action_id_string);
    static std::vector<std::optional<std::string>> ActionIdsToStrings(std::vector<ActionId> action_ids);
    static std::vector<std::optional<ActionId>> StringsToActionIds(std::vector<std::string> action_id_strings);
    ...
    static std::pair<ActionId, bool> CreateActionId(const std::string& action_name);

    static void ResetMapsForTesting();

private:
    // Merges `map2` into `map1`.
    template <typename T, typename U> static void MergeMaps(base::flat_map<T, U>& map1, base::flat_map<T, U>& map2);
