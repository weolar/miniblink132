...
{
    auto result = std::find_if(children_.begin(), children_.end(), [action_item](auto& item) { return item.get() == action_item; });
...
ScopedActionUpdate::ScopedActionUpdate(ScopedActionUpdate&& scoped_action_update)
    : action_item_(std::move(scoped_action_update.action_item_))
{
    scoped_action_update.action_item_ = nullptr;
}

ScopedActionUpdate& ScopedActionUpdate::operator=(ScopedActionUpdate&& scoped_action_update) = default;
...
ActionInvocationContext::ContextBuilder::ContextBuilder(ContextBuilder&&) = default;

ActionInvocationContext::ContextBuilder& ActionInvocationContext::ContextBuilder::operator=(ContextBuilder&&) = default;
...
ActionInvocationContext::ActionInvocationContext(ActionInvocationContext&&) = default;

ActionInvocationContext& ActionInvocationContext::operator=(ActionInvocationContext&&) = default;
...
[[nodiscard]] base::CallbackListSubscription ActionItem::AddActionChangedCallback(ActionChangedCallback callback)
{
...
void StatefulImageActionItem::SetStatefulImage(const ui::ImageModel& stateful_image)
{
...
// static
std::optional<ActionIdMap::ActionIdToStringMap>& ActionIdMap::GetGlobalActionIdToStringMap()
{
    static base::NoDestructor<std::optional<ActionIdMap::ActionIdToStringMap>> map;
    return *map;
}

// static
std::optional<ActionIdMap::StringToActionIdMap>& ActionIdMap::GetGlobalStringToActionIdMap()
{
    static base::NoDestructor<std::optional<ActionIdMap::StringToActionIdMap>> map;
    return *map;
}
...
{
    std::optional<ActionIdMap::ActionIdToStringMap>& map = GetGlobalActionIdToStringMap();
    if (!map.has_value()) {
...
{
    std::optional<ActionIdMap::StringToActionIdMap>& map = GetGlobalStringToActionIdMap();
    if (!map.has_value()) {
...
// static
std::optional<std::string> ActionIdMap::ActionIdToString(const ActionId action_id)
{
...
// static
std::optional<ActionId> ActionIdMap::StringToActionId(const std::string action_id_string)
{
...
// static
std::vector<std::optional<std::string>> ActionIdMap::ActionIdsToStrings(std::vector<ActionId> action_ids)
{
...
// static
std::vector<std::optional<ActionId>> ActionIdMap::StringsToActionIds(std::vector<std::string> action_id_strings)
{
...
template <typename T, typename U> void ActionIdMap::MergeMaps(base::flat_map<T, U>& map1, base::flat_map<T, U>& map2)
{
...
// static
std::pair<ActionId, bool> ActionIdMap::CreateActionId(const std::string& action_name)
{
...
{
    if (root_action_parent_.GetChildren().children().empty() && !initializer_list_->empty()) {
...
ActionItem* ActionManager::FindAction(ActionId action_id, ActionItem* scope)
{
    ...
    const ActionList& action_list = scope ? scope->GetChildren() : root_action_parent_.GetChildren();
    return FindActionImpl(action_id, action_list);
}

ActionItem* ActionManager::FindAction(const ui::KeyEvent& key_event, ActionItem* scope)
{
    IndexActions();
    return nullptr;
}
...
std::unique_ptr<ActionItem> ActionManager::RemoveAction(ActionItem* action_item)
{
...
base::CallbackListSubscription ActionManager::AppendActionItemInitializer(ActionItemInitializerList::CallbackType initializer)
{
...
ActionItem* ActionManager::FindActionImpl(ActionId action_id, const ActionList& list)
{
