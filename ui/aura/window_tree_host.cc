...
public:
    explicit ScopedLocalSurfaceIdValidator(Window* window)
        : window_(window)
        , local_surface_id_(window ? window->GetLocalSurfaceId() : viz::LocalSurfaceId())
    {
    }

    ScopedLocalSurfaceIdValidator(const ScopedLocalSurfaceIdValidator&) = delete;
    ScopedLocalSurfaceIdValidator& operator=(const ScopedLocalSurfaceIdValidator&) = delete;
...
{
    if (host_) {
        if (NativeWindowOcclusionTracker::IsNativeWindowOcclusionTrackingAlwaysEnabled(host_.get())) {
...
const char WindowTreeHost::kWindowTreeHostUsesParent[] = "__AURA_WINDOW_TREE_HOST_USE_PARENT_OF_ACCELERATED_WIDGET__";
...
// static
WindowTreeHost* WindowTreeHost::GetForAcceleratedWidget(gfx::AcceleratedWidget widget)
{
#if BUILDFLAG(IS_WIN)
...
#endif // BUILDFLAG(IS_WIN)
    return reinterpret_cast<WindowTreeHost*>(ui::ViewProp::GetValue(widget, kWindowTreeHostForAcceleratedWidget));
}

void WindowTreeHost::InitHost()
{
    device_scale_factor_ = display::Screen::GetScreen()->GetPreferredScaleFactorForWindow(window()).value_or(1.f);
...
gfx::Transform WindowTreeHost::GetRootTransformForLocalEventCoordinates() const
{
    return GetRootTransform();
}

gfx::Transform WindowTreeHost::GetInverseRootTransformForLocalEventCoordinates() const
{
    gfx::Transform invert;
...
void WindowTreeHost::UpdateCompositorScaleAndSize(const gfx::Size& new_size_in_pixels)
{
    gfx::Rect new_bounds(new_size_in_pixels);
    if (compositor_->display_transform_hint() == gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90
        || compositor_->display_transform_hint() == gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_270) {
    ...
    compositor_->SetScaleAndSize(device_scale_factor_, new_bounds.size(), window_->GetLocalSurfaceId());
}
...
void WindowTreeHost::OnCursorVisibilityChanged(bool show)
{
    ...
    if (!show) {
        ui::EventDispatchDetails details = dispatcher()->DispatchMouseExitAtPoint(nullptr, dispatcher()->GetLastMouseLocationInRoot(), ui::EF_CURSOR_HIDE);
...
void WindowTreeHost::MoveCursorToLocationInDIP(const gfx::Point& location_in_dip)
{
...
void WindowTreeHost::MoveCursorToLocationInPixels(const gfx::Point& location_in_pixels)
{
...
ui::EventDispatchDetails WindowTreeHost::DispatchKeyEventPostIME(ui::KeyEvent* event)
{
    ...
    ui::EventDispatchDetails dispatch_details = GetEventSink()->OnEventFromSource(event);
...
std::unique_ptr<ScopedKeyboardHook> WindowTreeHost::CaptureSystemKeyEvents(std::optional<base::flat_set<ui::DomCode>> dom_codes)
{
...
void WindowTreeHost::SetNativeWindowOcclusionState(Window::OcclusionState raw_occlusion_state, const SkRegion& raw_occluded_region)
{
    raw_occlusion_state_ = raw_occlusion_state;
    raw_occluded_region_ = raw_occluded_region;

    auto state = video_capture_count_for_occlusion_tracking_ > 0 ? Window::OcclusionState::VISIBLE : raw_occlusion_state;
    auto occluded_region = video_capture_count_for_occlusion_tracking_ > 0 ? SkRegion() : raw_occluded_region;
    ...
    observers_.Notify(&WindowTreeHostObserver::OnOcclusionStateChanged, this, state, occluded_region);
}
...
void WindowTreeHost::UpdateRootWindowSize()
{
    ...
    ScopedLocalSurfaceIdValidator lsi_validator(compositor_inited ? window() : nullptr);
    window()->SetBounds(CalculateRootWindowBounds());
}

gfx::Rect WindowTreeHost::CalculateRootWindowBounds() const
{
    return GetTransformedRootWindowBoundsFromPixelSize(GetBoundsInPixels().size());
}
...
std::unique_ptr<ScopedEnableUnadjustedMouseEvents> WindowTreeHost::RequestUnadjustedMovement()
{
    NOTIMPLEMENTED();
    return nullptr;
}
...
std::unique_ptr<WindowTreeHost::VideoCaptureLock> WindowTreeHost::CreateVideoCaptureLock()
{
    if (NativeWindowOcclusionTracker::IsNativeWindowOcclusionTrackingAlwaysEnabled(this)) {
...
{ // See header for details on ownership.
    if (!window_) {
        window_ = new Window(nullptr);
    }
    device_scale_factor_ = display::Screen::GetScreen()->GetPreferredScaleFactorForWindow(window_).value_or(1.f);
#if BUILDFLAG(IS_WIN)
    ...
    native_window_occlusion_enabled_
        = !base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kHeadless) && base::FeatureList::IsEnabled(features::kCalculateNativeWinOcclusion);
#endif
}
...
void WindowTreeHost::CreateCompositor(
    bool force_software_compositor, bool use_external_begin_frame_control, bool enable_compositing_based_throttling, size_t memory_limit_when_visible_mb)
{
    Env* env = Env::GetInstance();
    ui::ContextFactory* context_factory = env->context_factory();
    DCHECK(context_factory);
    compositor_ = std::make_unique<ui::Compositor>(context_factory->AllocateFrameSinkId(), context_factory, base::SingleThreadTaskRunner::GetCurrentDefault(),
        ui::IsPixelCanvasRecordingEnabled(), use_external_begin_frame_control, force_software_compositor, enable_compositing_based_throttling,
...
void WindowTreeHost::InitCompositor()
{
    ...
    compositor_->SetScaleAndSize(device_scale_factor_, GetBoundsInPixels().size(), window()->GetLocalSurfaceId());
...
void WindowTreeHost::OnAcceleratedWidgetAvailable()
{
    ...
    prop_ = std::make_unique<ui::ViewProp>(GetAcceleratedWidget(), kWindowTreeHostForAcceleratedWidget, this);
    if (NativeWindowOcclusionTracker::IsNativeWindowOcclusionTrackingAlwaysEnabled(this)) {
...
void WindowTreeHost::OnHostResizedInPixels(const gfx::Size& new_size_in_pixels)
{
...
void WindowTreeHost::OnDisplayMetricsChanged(const display::Display& display, uint32_t metrics)
{
    if (metrics & DisplayObserver::DISPLAY_METRIC_COLOR_SPACE && compositor_ && display.id() == GetDisplayId())
        compositor_->SetDisplayColorSpaces(display.GetColorSpaces());
...
// do for other OSes.
    ...
    if (metrics & DISPLAY_METRIC_DEVICE_SCALE_FACTOR && display.id() == GetDisplayId())
        OnHostResizedInPixels(GetBoundsInPixels().size());
#endif
}

gfx::Rect WindowTreeHost::GetTransformedRootWindowBoundsFromPixelSize(const gfx::Size& size_in_pixels) const
{
...
{
    if (!compositor() || !accelerated_widget_made_visible_ || !NativeWindowOcclusionTracker::IsNativeWindowOcclusionTrackingAlwaysEnabled(this)) {
        return;
    }
...
{
#if BUILDFLAG(IS_WIN)
    if (!base::FeatureList::IsEnabled(features::kApplyNativeOcclusionToCompositor) || !IsNativeWindowOcclusionEnabled()) {
        return false;
    }

    const std::string type = features::kApplyNativeOcclusionToCompositorType.Get();
    return type == features::kApplyNativeOcclusionToCompositorTypeThrottle || type == features::kApplyNativeOcclusionToCompositorTypeThrottleAndRelease;
#else
    return false;
#endif
}
...
bool WindowTreeHost::NativeOcclusionAffectsVisibility() const
{
    ...
    const std::string type = features::kApplyNativeOcclusionToCompositorType.Get();
    return type == features::kApplyNativeOcclusionToCompositorTypeRelease || type == features::kApplyNativeOcclusionToCompositorTypeThrottleAndRelease;
#else
    return false;
#endif
}
...
bool WindowTreeHost::ShouldThrottle() const
{
    ...
    return video_capture_count_for_occlusion_tracking_ == 0 && occlusion_state_ == Window::OcclusionState::OCCLUDED;
}

// static
const base::flat_set<raw_ptr<WindowTreeHost, CtnExperimental>>& WindowTreeHost::GetThrottledHostsForTesting()
{
    return HostFrameRateThrottler::GetInstance().hosts();
}

void WindowTreeHost::MoveCursorToInternal(const gfx::Point& root_location, const gfx::Point& host_location)
{
...
void WindowTreeHost::OnFrameSinksToThrottleUpdated(const base::flat_set<viz::FrameSinkId>& ids)
{
    observers_.Notify(&WindowTreeHostObserver::OnCompositingFrameSinksToThrottleUpdated, this, ids);
}

void WindowTreeHost::OnSetPreferredRefreshRate(ui::Compositor*, float preferred_refresh_rate)
{
    observers_.Notify(&WindowTreeHostObserver::OnSetPreferredRefreshRate, this, preferred_refresh_rate);
}
