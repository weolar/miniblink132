...
public:
    ...
    void AddWindowEventDispatcherObserver(WindowEventDispatcherObserver* observer);
    void RemoveWindowEventDispatcherObserver(WindowEventDispatcherObserver* observer);
    base::ObserverList<WindowEventDispatcherObserver>::Unchecked& window_event_dispatcher_observers()
    {
    ...
    void SetLastTouchLocation(const aura::Window* target, const gfx::Point& last_touch_location);

    // Whether any touch device is currently down.
    bool is_touch_down() const
    {
        return is_touch_down_;
    }
    ...
    static void set_initial_throttle_input_on_resize_for_testing(bool throttle_input)
    {
    ...
    bool throttle_input_on_resize() const
    {
        return throttle_input_on_resize_;
    }
    ...
    void SetGestureRecognizer(std::unique_ptr<ui::GestureRecognizer> gesture_recognizer);
    ...
    // state.
    gfx::Point GetLastPointerPoint(ui::mojom::DragEventSource event_source, aura::Window* window, std::optional<gfx::Point> fallback);
    ...
    void AddEventObserver(ui::EventObserver* observer, ui::EventTarget* target, const std::set<ui::EventType>& types);
    ...
    const std::vector<raw_ptr<aura::WindowTreeHost, VectorExperimental>>& window_tree_hosts() const
    {
...
private:
    ...
    // observers.
    base::ObserverList<WindowEventDispatcherObserver>::Unchecked window_event_dispatcher_observers_;
    ...
    std::vector<raw_ptr<aura::WindowTreeHost, VectorExperimental>> window_tree_hosts_;
