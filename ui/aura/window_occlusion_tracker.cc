...
constexpr ui::LayerAnimationElement::AnimatableProperties kSkipWindowWhenPropertiesAnimated
    = ui::LayerAnimationElement::TRANSFORM | ui::LayerAnimationElement::BOUNDS | ui::LayerAnimationElement::OPACITY | ui::LayerAnimationElement::COLOR;
...
constexpr ui::LayerAnimationElement::AnimatableProperties kOcclusionCanChangeWhenPropertyAnimationEnds
    = kSkipWindowWhenPropertiesAnimated | ui::LayerAnimationElement::VISIBILITY;
...
gfx::Transform GetWindowTransformRelativeToRoot(Window* window, const gfx::Transform& parent_transform_relative_to_root, bool use_target_values)
{
    ...
    if (use_target_values) {
        translation.Translate(static_cast<float>(window->layer()->GetTargetBounds().x()), static_cast<float>(window->layer()->GetTargetBounds().y()));
        transform_relative_to_root = window->layer()->GetTargetTransform();
    } else {
        translation.Translate(static_cast<float>(window->layer()->bounds().x()), static_cast<float>(window->layer()->bounds().y()));
...
// bounds.
SkIRect ComputeTransformedBoundsEnclosing(const gfx::Rect& bounds, const gfx::Transform& transform_relative_to_root)
{
...
// bounds.
SkIRect ComputeTransformedBoundsEnclosed(const gfx::Rect& bounds, const gfx::Transform& transform_relative_to_root)
{
    DCHECK(transform_relative_to_root.Preserves2dAxisAlignment());
    return gfx::RectToSkIRect(gfx::ToEnclosedRect(transform_relative_to_root.MapRect(gfx::RectF(bounds))));
}
...
// integers are used.
SkIRect GetWindowBoundsInRootWindow(Window* window, const gfx::Transform& transform_relative_to_root, const SkIRect* clipped_bounds, bool use_target_values)
{
    // Compute the unclipped bounds of |window|.
    const gfx::Rect src_bounds = use_target_values ? window->layer()->GetTargetBounds() : window->bounds();
    const SkIRect transformed_bounds = ComputeTransformedBoundsEnclosed(gfx::Rect(src_bounds.size()), transform_relative_to_root);
...
SkIRect GetOpaqueBoundsInRootWindow(Window* window, const gfx::Transform& transform_relative_to_root, const SkIRect* clipped_bounds)
{
    ...
    const SkIRect transformed_bounds = ComputeTransformedBoundsEnclosing(opaque_region, transform_relative_to_root);
    return ComputeClippedBounds(transformed_bounds, clipped_bounds);
}
...
WindowOcclusionTracker::InnerClient::InnerClient(WindowOcclusionTracker* occlusion_tracker)
    : occlusion_tracker_(occlusion_tracker ? occlusion_tracker : Env::GetInstance()->GetWindowOcclusionTracker())
{
...
WindowOcclusionTracker::ScopedPause::ScopedPause(WindowOcclusionTracker* occlusion_tracker)
...
WindowOcclusionTracker::ScopedExclude::ScopedExclude(Window* window, WindowOcclusionTracker* occlusion_tracker)
    : InnerClient(occlusion_tracker)
    , window_(window)
{
...
WindowOcclusionTracker::ScopedForceVisible::ScopedForceVisible(Window* window, WindowOcclusionTracker* occlusion_tracker)
    : InnerClient(occlusion_tracker)
    , window_(window)
{
...
void WindowOcclusionTracker::ScopedForceVisible::OnWindowDestroying(Window* window)
{
    DCHECK_EQ(window_, window);
    Shutdown();
}
...
WindowOcclusionTracker::OcclusionData WindowOcclusionTracker::ComputeTargetOcclusionForWindow(Window* window)
{
    ...
    CHECK(tracked_window_iter != tracked_windows_.end(), base::NotFatalUntil::M130);

    base::AutoReset<OcclusionData> auto_reset_occlusion_data(&tracked_window_iter->second, OcclusionData());
    DCHECK(!target_occlusion_window_);
    base::AutoReset<raw_ptr<Window>> auto_reset_target_occlusion_window(&target_occlusion_window_, window);

    Window* root_window = window->GetRootWindow();
    SkRegion occluded_region;
    SkIRect root_window_clip = gfx::RectToSkIRect(root_window->bounds());
    RecomputeOcclusionImpl(root_window, gfx::Transform(), &root_window_clip, &occluded_region);
...
bool WindowOcclusionTracker::OcclusionStatesMatch(const base::flat_map<Window*, OcclusionData>& tracked_windows)
{
    for (const auto& tracked_window : tracked_windows) {
        if (tracked_window.second.occlusion_state != tracked_window.first->GetOcclusionState())
            return false;
    }
    return true;
}
...
{
    if (num_pause_occlusion_tracking_ || num_times_occlusion_recomputed_in_current_step_ != 0) {
        return;
    }

    base::AutoReset<int> auto_reset(&num_times_occlusion_recomputed_in_current_step_, 0);

    TRACE_EVENT1("ui", "WindowOcclusionTracker::MaybeComputeOcclusion", "this", reinterpret_cast<void*>(this));
    ...
    while (num_times_occlusion_recomputed_in_current_step_ <= kMaxRecomputeOcclusion) {
        const bool exceeded_max_num_times_occlusion_recomputed = num_times_occlusion_recomputed_in_current_step_ == kMaxRecomputeOcclusion;
        ...
        for (auto& root_window_pair : root_windows_) {
            if (root_window_pair.second.dirty) {
                ...
                if (!exceeded_max_num_times_occlusion_recomputed) {
                    ...
                    if (root_window_pair.second.occlusion_state == Window::OcclusionState::OCCLUDED) {
                        SetWindowAndDescendantsAreOccluded(root_window, /* is_occluded */ true, root_window->IsVisible());
// TODO(crbug.com/40262710): Enable for other platforms in a separate CL.
#if BUILDFLAG(IS_CHROMEOS)
                    } else if (root_window_pair.second.occlusion_state == Window::OcclusionState::HIDDEN) {
        ...
        std::unique_ptr<WindowOcclusionChangeBuilder> change_builder
            = occlusion_change_builder_factory_ ? occlusion_change_builder_factory_.Run() : WindowOcclusionChangeBuilder::Create();
        for (auto& it : tracked_windows_) {
            ...
            change_builder->Add(window, it.second.occlusion_state, it.second.occluded_region);
        }
    }
    ...
    DCHECK(occlusion_change_builder_factory_ || OcclusionStatesMatch(tracked_windows_));
}

bool WindowOcclusionTracker::RecomputeOcclusionImpl(
    Window* window, const gfx::Transform& parent_transform_relative_to_root, const SkIRect* clipped_bounds, SkRegion* occluded_region)
{
    DCHECK(window);
    ...
    const bool is_visible = force_visible || (ShouldUseTargetValues() ? window->layer()->GetTargetVisibility() : window->layer()->visible());
    if (!is_visible) {
    ...
    const gfx::Transform transform_relative_to_root = GetWindowTransformRelativeToRoot(window, parent_transform_relative_to_root, ShouldUseTargetValues());
    ...
    const SkIRect window_bounds
        = GetWindowBoundsInRootWindow(window, transform_relative_to_root, force_visible ? nullptr : clipped_bounds, ShouldUseTargetValues());

    // Compute children occlusion states.
    const SkIRect* clipped_bounds_for_children = window->layer()->GetMasksToBounds() ? &window_bounds : clipped_bounds;
    ...
    SkRegion* occluded_region_for_children = force_visible ? &region_for_forced_visible_windows : occluded_region;
    for (aura::Window* child : base::Reversed(window->children())) {
        has_visible_child |= RecomputeOcclusionImpl(child, transform_relative_to_root, clipped_bounds_for_children, occluded_region_for_children);
    }

    // Window is fully occluded.
    if (!force_visible && occluded_region->contains(window_bounds) && !has_visible_child) {
        SetOccluded(window, /* is_occluded */ true, /* is_parent_visible */ true, SkRegion());
        return false;
    }
    ...
    SetOccluded(window, false, /* is_parent_visible */ true, force_visible ? SkRegion() : occluded_region_before_traversing_children);

    if (!force_visible && VisibleWindowCanOccludeOtherWindows(window)) {
        const SkIRect occlusion_bounds
            = WindowHasOpaqueRegionsForOcclusion(window) ? GetOpaqueBoundsInRootWindow(window, transform_relative_to_root, clipped_bounds) : window_bounds;
        occluded_region->op(occlusion_bounds, SkRegion::kUnion_Op);
    }
    return true;
}

bool WindowOcclusionTracker::VisibleWindowCanOccludeOtherWindows(const Window* window) const
{
    DCHECK(window->layer());
    float combined_opacity = ShouldUseTargetValues() ? GetLayerCombinedTargetOpacity(window->layer()) : window->layer()->GetCombinedOpacity();
    ...
    if (window->layer()->type() == ui::LAYER_SOLID_COLOR) {
        auto color = ShouldUseTargetValues() ? window->layer()->GetTargetColor() : window->layer()->background_color();
        combined_opacity *= SkColorGetA(color) / 255.f;
    }
    return (!window->GetTransparent() && WindowHasContent(window) && combined_opacity == 1.0f &&
...
void WindowOcclusionTracker::CleanupAnimatedWindows()
{
    base::EraseIf(animated_windows_, [=, this](Window* window) {
        ...
        if (animator->IsAnimatingOnePropertyOf(kOcclusionCanChangeWhenPropertyAnimationEnds))
            return false;
...
void WindowOcclusionTracker::SetWindowAndDescendantsAreOccluded(Window* window, bool is_occluded, bool is_parent_visible)
{
    const bool force_visible = WindowIsForcedVisible(window);
    const bool is_visible = force_visible || (is_parent_visible && window->layer()->visible());
...
void WindowOcclusionTracker::SetOccluded(Window* window, bool is_occluded, bool is_parent_visible, const SkRegion& occluded_region)
{
    ...
    const bool is_visible = WindowIsForcedVisible(window) || (is_parent_visible && window->layer()->visible());
    if (!is_visible)
    ...
    DCHECK(tracked_window->second.occlusion_state == Window::OcclusionState::VISIBLE || tracked_window->second.occluded_region.isEmpty());
}
...
{
    return !ShouldUseTargetValues() && base::Contains(animated_windows_, window)
        && window->layer()->GetAnimator()->IsAnimatingOnePropertyOf(kSkipWindowWhenPropertiesAnimated);
}
...
template <typename Predicate> void WindowOcclusionTracker::MarkRootWindowAsDirtyAndMaybeComputeOcclusionIf(Window* window, Predicate predicate)
{
    Window* root_window = window->GetRootWindow();
    if (!root_window)
        return;
...
void WindowOcclusionTracker::MarkRootWindowStateAsDirty(RootWindowState* root_window_state)
{
    ...
    // stabilizing.
    DCHECK_LT(num_times_occlusion_recomputed_in_current_step_, kMaxRecomputeOcclusion);
...
bool WindowOcclusionTracker::WindowOrDescendantIsTrackedAndVisible(Window* window) const
{
    if (!WindowIsVisible(window))
        return false;
...
bool WindowOcclusionTracker::WindowOrDescendantCanOccludeOtherWindows(Window* window, bool assume_parent_opaque, bool assume_window_opaque) const
{
    const bool parent_window_is_opaque = assume_parent_opaque || !window->parent() || window->parent()->layer()->GetCombinedOpacity() == 1.0f;
    const bool window_is_opaque = parent_window_is_opaque && (assume_window_opaque || window->layer()->opacity() == 1.0f);

    if (!WindowIsVisible(window) || !window->layer() || !window_is_opaque || WindowIsAnimated(window)) {
        return false;
    }
    if ((!window->GetTransparent() && WindowHasContent(window)) || WindowHasOpaqueRegionsForOcclusion(window)) {
        return true;
    }
    for (Window* child_window : window->children()) {
...
bool WindowOcclusionTracker::WindowOpacityChangeMayAffectOcclusionStates(Window* window) const
{
    ...
    // excluded.
    return WindowIsVisible(window) && !WindowOrParentIsAnimated(window) && !WindowIsExcluded(window);
}

bool WindowOcclusionTracker::WindowMoveMayAffectOcclusionStates(Window* window) const
{
    return !WindowOrParentIsAnimated(window) && !WindowIsExcluded(window)
        && (WindowOrDescendantCanOccludeOtherWindows(window) || WindowOrDescendantIsTrackedAndVisible(window));
}
...
void WindowOcclusionTracker::TrackedWindowAddedToRoot(Window* window)
{
    ...
    if (root_window_state.num_tracked_windows == 1) {
        ...
        if (host) {
            ...
            if (!NativeWindowOcclusionTracker::IsNativeWindowOcclusionTrackingAlwaysEnabled(host)) {
...
void WindowOcclusionTracker::RemoveObserverFromWindowAndDescendants(Window* window)
{
    if (WindowIsTracked(window)) {
...
void WindowOcclusionTracker::OnLayerAnimationEnded(ui::LayerAnimationSequence* sequence)
{
    CleanupAnimatedWindows();
    MaybeComputeOcclusion();
}

void WindowOcclusionTracker::OnLayerAnimationAborted(ui::LayerAnimationSequence* sequence)
{
    CleanupAnimatedWindows();
    MaybeComputeOcclusion();
}

void WindowOcclusionTracker::OnLayerAnimationScheduled(ui::LayerAnimationSequence* sequence)
{
}
...
void WindowOcclusionTracker::OnWindowHierarchyChanged(const HierarchyChangeParams& params)
{
    Window* const window = params.target;
    Window* const root_window = window->GetRootWindow();
    if (root_window && base::Contains(root_windows_, root_window) && !window_observations_.IsObservingSource(window)) {
...
{
    MarkRootWindowAsDirtyAndMaybeComputeOcclusionIf(window, [=, this]() { return WindowMoveMayAffectOcclusionStates(window); });
}

void WindowOcclusionTracker::OnWillRemoveWindow(Window* window)
{
    MarkRootWindowAsDirtyAndMaybeComputeOcclusionIf(
        window, [=, this]() { return !WindowOrParentIsAnimated(window) && WindowOrDescendantCanOccludeOtherWindows(window); });
}

void WindowOcclusionTracker::OnWindowVisibilityChanged(Window* window, bool visible)
{
    ...
    MarkRootWindowAsDirtyAndMaybeComputeOcclusionIf(window, [=, this]() {
        ...
        return (!window->parent() || WindowIsVisible(window->parent())) && !WindowOrParentIsAnimated(window);
    });
}

void WindowOcclusionTracker::OnWindowBoundsChanged(Window* window, const gfx::Rect& old_bounds, const gfx::Rect& new_bounds, ui::PropertyChangeReason reason)
{
    ...
    const bool animation_started = (reason == ui::PropertyChangeReason::FROM_ANIMATION) && MaybeObserveAnimatedWindow(window);
    MarkRootWindowAsDirtyAndMaybeComputeOcclusionIf(window, [=, this]() { return animation_started || WindowMoveMayAffectOcclusionStates(window); });
}

void WindowOcclusionTracker::OnWindowOpacitySet(Window* window, ui::PropertyChangeReason reason)
{
    // Call MaybeObserveAnimatedWindow() outside the lambda so that the window can
    // be marked as animated even when its root is dirty.
    const bool animation_started = (reason == ui::PropertyChangeReason::FROM_ANIMATION) && MaybeObserveAnimatedWindow(window);
    MarkRootWindowAsDirtyAndMaybeComputeOcclusionIf(window, [=, this]() { return animation_started || WindowOpacityChangeMayAffectOcclusionStates(window); });
}

void WindowOcclusionTracker::OnWindowAlphaShapeSet(Window* window)
{
    MarkRootWindowAsDirtyAndMaybeComputeOcclusionIf(window, [=, this]() { return WindowOpacityChangeMayAffectOcclusionStates(window); });
}

void WindowOcclusionTracker::OnWindowTransparentChanged(Window* window, ui::PropertyChangeReason reason)
{
    // Call MaybeObserveAnimatedWindow() outside the lambda so that the window can
    // be marked as animated even when its root is dirty.
    const bool animation_started = (reason == ui::PropertyChangeReason::FROM_ANIMATION) && MaybeObserveAnimatedWindow(window);
...
void WindowOcclusionTracker::OnWindowTransformed(Window* window, ui::PropertyChangeReason reason)
{
    // Call MaybeObserveAnimatedWindow() outside the lambda so that the window can
    // be marked as animated even when its root is dirty.
    const bool animation_started = (reason == ui::PropertyChangeReason::FROM_ANIMATION) && MaybeObserveAnimatedWindow(window);
...
void WindowOcclusionTracker::OnWindowDestroyed(Window* window)
{
    ...
    DCHECK(!window->layer()->GetAnimator()->IsAnimatingOnePropertyOf(kOcclusionCanChangeWhenPropertyAnimationEnds));
...
void WindowOcclusionTracker::OnWindowRemovingFromRootWindow(Window* window, Window* new_root)
{
    DCHECK(window->GetRootWindow());
    if (WindowIsTracked(window))
...
void WindowOcclusionTracker::OnWindowOpaqueRegionsForOcclusionChanged(Window* window)
{
    ...
    MarkRootWindowAsDirtyAndMaybeComputeOcclusionIf(
        window, [=, this]() { return WindowOpacityChangeMayAffectOcclusionStates(window) || WindowMoveMayAffectOcclusionStates(window); });
}

void WindowOcclusionTracker::OnOcclusionStateChanged(WindowTreeHost* host, Window::OcclusionState new_state, const SkRegion& occluded_region)
{
...
void WindowOcclusionTracker::RemoveAnimationObservationForLayer(ui::Layer* layer)
{
