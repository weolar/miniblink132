...
Window* CreateTestWindow(SkColor color, int id, const gfx::Rect& bounds, Window* parent)
{
    return CreateTestWindowWithDelegate(new ColorTestWindowDelegate(color), id, bounds, parent);
}

Window* CreateTestWindowWithDelegate(WindowDelegate* delegate, int id, const gfx::Rect& bounds, Window* parent)
{
    return CreateTestWindowWithDelegateAndType(delegate, client::WINDOW_TYPE_NORMAL, id, bounds, parent, true);
}

Window* CreateTestWindowWithDelegateAndType(
    WindowDelegate* delegate, client::WindowType type, int id, const gfx::Rect& bounds, Window* parent, bool show_on_creation)
{
    Window* window = new Window(delegate, type);
    window->SetId(id);
    window->Init(ui::LAYER_TEXTURED);
    window->SetProperty(
        client::kResizeBehaviorKey, client::kResizeBehaviorCanResize | client::kResizeBehaviorCanMaximize | client::kResizeBehaviorCanFullscreen);
...
template <typename T> bool ObjectIsAbove(T* upper, T* lower)
{
