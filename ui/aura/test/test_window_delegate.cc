...
void TestWindowDelegate::OnBoundsChanged(const gfx::Rect& old_bounds, const gfx::Rect& new_bounds)
{
}
...
bool TestWindowDelegate::ShouldDescendIntoChildForEventHandling(Window* child, const gfx::Point& location)
{
    return true;
}
...
void TestWindowDelegate::OnDeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor)
{
}
...
void TestWindowDelegate::OnWindowOcclusionChanged(Window::OcclusionState old_occlusion_state, Window::OcclusionState new_occlusion_state)
{
...
void ColorTestWindowDelegate::OnBoundsChanged(const gfx::Rect& old_bounds, const gfx::Rect& new_bounds)
{
...
{
    std::string result = base::StringPrintf("%d %d %d", mouse_enter_count_, mouse_move_count_, mouse_leave_count_);
    mouse_enter_count_ = 0;
    mouse_move_count_ = 0;
    mouse_leave_count_ = 0;
    return result;
...
{
    std::string result = base::StringPrintf("%d %d", mouse_press_count_, mouse_release_count_);
    mouse_press_count_ = 0;
    mouse_release_count_ = 0;
    return result;
}
...
{
    std::string result = base::StringPrintf("%d %d", key_press_count_, key_release_count_);
    key_press_count_ = 0;
    key_release_count_ = 0;
    return result;
}
