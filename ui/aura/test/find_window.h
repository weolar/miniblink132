...
template <typename CallbackFunc> Window* FindFirstWindowMatching(Window* window, const CallbackFunc& cb)
{
    if (cb(window)) {
        return window;
    }
...
// environment.
template <typename CallbackFunc> Window* FindFirstWindowMatching(Env* env, const CallbackFunc& cb)
{
...
{
    return FindFirstWindowMatching(env, [title](Window* window) -> bool { return window->GetTitle() == title; });
}
