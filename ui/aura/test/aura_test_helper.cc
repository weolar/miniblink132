...
AuraTestHelper::AuraTestHelper(ui::ContextFactory* context_factory)
{
...
#if BUILDFLAG(IS_OZONE) && BUILDFLAG(IS_CHROMEOS) && !BUILDFLAG(IS_CHROMEOS_DEVICE)
    ...
    ui::test::EventGeneratorDelegate::SetFactoryFunction(base::BindRepeating(&EventGeneratorDelegateAura::Create));
...
void AuraTestHelper::SetUp()
{
    ...
    gfx::Size host_size(screen ? screen->GetPrimaryDisplay().GetSizeInPixel() : kDefaultHostSize);
    ...
    screen_position_client_ = std::make_unique<wm::DefaultScreenPositionClient>(root_window);
...
void AuraTestHelper::TearDown()
{
    ...
    ui::test::EventGeneratorDelegate::SetFactoryFunction(ui::test::EventGeneratorDelegate::FactoryFunction());
