...
{
    return rotation == display::Display::ROTATE_90 || rotation == display::Display::ROTATE_270;
}
...
{
    DCHECK(!host_);
    ui::PlatformWindowInitProperties properties(gfx::Rect(GetPrimaryDisplay().GetSizeInPixel()));
...
void TestScreen::SetDeviceScaleFactor(float device_scale_factor, bool resize_host)
{
...
void TestScreen::SetColorSpace(const gfx::ColorSpace& color_space, float sdr_white_level)
{
    display::Display display(GetPrimaryDisplay());
    gfx::DisplayColorSpaces display_color_spaces(color_space, gfx::BufferFormat::RGBA_8888);
...
void TestScreen::SetUIScale(float ui_scale)
{
    ...
    display::Display display(GetPrimaryDisplay());
    gfx::Rect bounds_in_pixel(display.GetSizeInPixel());
    gfx::Rect new_bounds = gfx::ToNearestRect(gfx::ScaleRect(gfx::RectF(bounds_in_pixel), 1.0f / ui_scale));
    display.SetScaleAndBounds(display.device_scale_factor(), new_bounds);
    display_list().UpdateDisplay(display);
    host_->SetRootTransform(GetUIScaleTransform() * GetRotationTransform());
}
...
void TestScreen::SetPreferredScaleFactorForWindow(gfx::NativeWindow window, float scale_factor)
{
...
{
    display::Display display = GetPrimaryDisplay();
    return display::CreateRotationTransform(display.rotation(), gfx::SizeF(display.size()));
}
...
void TestScreen::OnWindowBoundsChanged(Window* window, const gfx::Rect& old_bounds, const gfx::Rect& new_bounds, ui::PropertyChangeReason reason)
{
    DCHECK_EQ(host_->window(), window);
    display::Display display(GetPrimaryDisplay());
    display.SetSize(gfx::ScaleToFlooredSize(new_bounds.size(), display.device_scale_factor()));
    display_list().UpdateDisplay(display);
}
...
gfx::NativeWindow TestScreen::GetWindowForPoint(Window* window, const gfx::Point& local_point)
{
    DCHECK(window);
    if (!window->IsVisible()) {
        return nullptr;
...
gfx::NativeWindow TestScreen::GetLocalProcessWindowAtPoint(const gfx::Point& point, const std::set<gfx::NativeWindow>& ignore)
{
    return nullptr;
}

display::Display TestScreen::GetDisplayNearestWindow(gfx::NativeWindow window) const
{
...
std::optional<float> TestScreen::GetPreferredScaleFactorForWindow(gfx::NativeWindow window) const
{
    if (auto it = preferred_scale_factors_.find(window); it != preferred_scale_factors_.end()) {
        return it->second;
    }
