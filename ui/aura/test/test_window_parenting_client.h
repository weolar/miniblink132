...
public:
    ...
    TestWindowParentingClient& operator=(const TestWindowParentingClient&) = delete;

    ~TestWindowParentingClient() override;

    void set_default_parent(Window* parent)
    {
        default_parent_ = parent;
    }

    // Overridden from client::WindowParentingClient:
    Window* GetDefaultParent(Window* window, const gfx::Rect& bounds, const int64_t display_id) override;
