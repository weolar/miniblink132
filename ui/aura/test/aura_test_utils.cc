...
public:
    ...
    void disable_ime()
    {
        host_->dispatcher_->set_skip_ime(true);
    }
    ...
    static const base::flat_set<raw_ptr<WindowTreeHost, CtnExperimental>>& GetThrottledHosts()
    {
...
void SetHostDispatcher(WindowTreeHost* host, std::unique_ptr<WindowEventDispatcher> dispatcher)
{
    WindowTreeHostTestApi host_test_api(host);
...
const base::flat_set<raw_ptr<WindowTreeHost, CtnExperimental>>& GetThrottledHosts()
{
