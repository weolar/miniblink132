...
public:
    ...
    virtual void OnWindowEventDispatcherStartedProcessing(WindowEventDispatcher* dispatcher, const ui::Event& event)
    {
    }

    // Called when WindowEventDispatcher finishes processing an event.
    virtual void OnWindowEventDispatcherFinishedProcessingEvent(WindowEventDispatcher* dispatcher)
    {
    }

    // Called when the WindowEventDispatcher dispatches held events. See
    // WindowEventDispatcher::HoldMouseMoves() for more details.
    virtual void OnWindowEventDispatcherDispatchedHeldEvents(WindowEventDispatcher* dispatcher)
    {
    }
    ...
    virtual void OnWindowEventDispatcherIgnoredEvent(WindowEventDispatcher* dispatcher)
    {
    }
