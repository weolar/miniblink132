...
#if BUILDFLAG(IS_WIN)
        scoped_feature_list_.InitAndDisableFeature(features::kApplyNativeOcclusionToCompositor);
#endif
    }
...
public:
    TestWindowTreeHostObserver(WindowTreeHostPlatform* host, ui::PlatformWindow* platform_window)
        : host_(host)
    ...
    TestWindowTreeHostObserver& operator=(const TestWindowTreeHostObserver&) = delete;

    ~TestWindowTreeHostObserver() override
    {
...
public:
    explicit DeleteHostWindowTreeHostObserver(std::unique_ptr<TestWindowTreeHost> host)
        : host_(std::move(host))
    {
        host_->AddObserver(this);
    }

    DeleteHostWindowTreeHostObserver(const DeleteHostWindowTreeHostObserver&) = delete;
    DeleteHostWindowTreeHostObserver& operator=(const DeleteHostWindowTreeHostObserver&) = delete;
...
{
    std::unique_ptr<TestWindowTreeHost> host = std::make_unique<TestWindowTreeHost>();
