...
bool WindowTargeter::SubtreeShouldBeExploredForEvent(Window* window, const ui::LocatedEvent& event)
{
    return SubtreeCanAcceptEvent(window, event) && EventLocationInsideBounds(window, event);
}

bool WindowTargeter::GetHitTestRects(Window* window, gfx::Rect* hit_test_rect_mouse, gfx::Rect* hit_test_rect_touch) const
{
...
std::unique_ptr<WindowTargeter::HitTestRects> WindowTargeter::GetExtraHitTestShapeRects(Window* target) const
{
    return nullptr;
}
...
void WindowTargeter::SetInsets(const gfx::Insets& mouse_extend, const gfx::Insets& touch_extend)
{
...
Window* WindowTargeter::GetPriorityTargetInRootWindow(Window* root_window, const ui::LocatedEvent& event)
{
    ...
    if (event.IsPinchEvent()) {
        DCHECK_EQ(event.AsGestureEvent()->details().device_type(), ui::GestureDeviceType::DEVICE_TOUCHPAD);
        WindowEventDispatcher* dispatcher = root_window->GetHost()->dispatcher();
    ...
    if (event.IsTouchEvent()) {
        ...
        ui::GestureConsumer* consumer = Env::GetInstance()->gesture_recognizer()->GetTouchLockedTarget(touch);
        if (consumer)
...
Window* WindowTargeter::FindTargetInRootWindow(Window* root_window, const ui::LocatedEvent& event)
{
    DCHECK_EQ(root_window, root_window->GetRootWindow());
    ...
    if (event.IsTouchEvent()) {
        ...
        ui::GestureRecognizer* gesture_recognizer = Env::GetInstance()->gesture_recognizer();
        DCHECK(!gesture_recognizer->GetTouchLockedTarget(touch));
        ui::GestureConsumer* consumer = gesture_recognizer->GetTargetForLocation(event.location_f(), touch.source_device_id());
        if (consumer)
            return static_cast<Window*>(consumer);
        ...
        display::Display display = display::Screen::GetScreen()->GetDisplayNearestWindow(root_window);
        ...
        if (client::GetScreenPositionClient(root_window)) {
            client::GetScreenPositionClient(root_window)->ConvertPointToScreen(root_window, &screen_location);
        }
...
ui::EventTarget* WindowTargeter::FindTargetForEvent(ui::EventTarget* root, ui::Event* event)
{
    Window* window = static_cast<Window*>(root);
    return event->IsKeyEvent() ? FindTargetForKeyEvent(window) : FindTargetForNonKeyEvent(window, event);
}

ui::EventTarget* WindowTargeter::FindNextBestTarget(ui::EventTarget* previous_target, ui::Event* event)
{
    return nullptr;
}
...
Window* WindowTargeter::FindTargetForKeyEvent(Window* window)
{
    ...
    if (event_client && !event_client->GetCanProcessEventsWithinSubtree(focused_window)) {
...
Window* WindowTargeter::FindTargetForLocatedEvent(Window* window, ui::LocatedEvent* event)
{
    if (!window->parent()) {
        ...
        if (target) {
            ...
            if (window->IsRootWindow() && event->HasNativeEvent()) {
                ...
                ui::LocatedEvent* e = static_cast<ui::LocatedEvent*>(event->native_event());
                gfx::PointF native_point = e->location_f();
                aura::Window::ConvertNativePointToTargetHost(window, target, &native_point);
...
ui::EventSink* WindowTargeter::GetNewEventSinkForEvent(const ui::EventTarget* current_root, ui::EventTarget* target, ui::Event* in_out_event)
{
    const aura::Window* root_window = static_cast<const Window*>(current_root);
    aura::Window* target_window = static_cast<Window*>(target);
    if (!target_window || root_window->parent() || root_window->Contains(target_window)) {
    ...
    if (in_out_event->IsLocatedEvent()) {
        ...
        located_event->UpdateForRootTransform(window_tree_host->GetRootTransform(), window_tree_host->GetRootTransformForLocalEventCoordinates());
    }
    return new_root->GetHost()->GetEventSink();
}

bool WindowTargeter::SubtreeCanAcceptEvent(Window* window, const ui::LocatedEvent& event) const
{
    if (!window->IsVisible())
        return false;
    if (window->event_targeting_policy() == EventTargetingPolicy::kNone || window->event_targeting_policy() == EventTargetingPolicy::kTargetOnly) {
        return false;
    }
...
bool WindowTargeter::EventLocationInsideBounds(Window* window, const ui::LocatedEvent& event) const
{
...
// static
gfx::Point WindowTargeter::ConvertEventLocationToWindowCoordinates(Window* window, const ui::LocatedEvent& event)
{
...
Window* WindowTargeter::FindTargetForNonKeyEvent(Window* root_window, ui::Event* event)
{
    if (!event->IsLocatedEvent())
        return root_window;
    return FindTargetForLocatedEvent(root_window, static_cast<ui::LocatedEvent*>(event));
}

Window* WindowTargeter::FindTargetForLocatedEventRecursively(Window* root_window, ui::LocatedEvent* event)
{
    std::unique_ptr<ui::EventTargetIterator> iter = root_window->GetChildIterator();
    if (iter) {
        ui::EventTarget* target = root_window;
        for (ui::EventTarget* child = iter->GetNextTarget(); child; child = iter->GetNextTarget()) {
            WindowTargeter* targeter = static_cast<WindowTargeter*>(child->GetEventTargeter());
            if (!targeter)
                targeter = this;
            if (!targeter->SubtreeShouldBeExploredForEvent(static_cast<Window*>(child), *event)) {
                continue;
            }
...
bool WindowTargeter::PointInsideBounds(Window* window, BoundsType bounds_type, const gfx::Point& point) const
{
    gfx::Rect mouse_rect;
    gfx::Rect touch_rect;
    ...
    const bool point_in_rect
        = bounds_type == BoundsType::kTouch || bounds_type == BoundsType::kGesture ? touch_rect.Contains(point) : mouse_rect.Contains(point);
    if (point_in_rect) {
    ...
    if (ShouldUseExtendedBounds(window) && (!mouse_extend_.IsEmpty() || !touch_extend_.IsEmpty())) {
