...
{
    return consumer ? base::NumberToString(static_cast<Window*>(consumer)->GetId()) : "?";
}

#define EXPECT_0_EVENTS(events) EXPECT_EQ(0u, events.size())
...
public:
    ...
    GestureEventConsumeDelegate& operator=(const GestureEventConsumeDelegate&) = delete;
    ...
    const std::vector<ui::EventType>& events() const
    {
        return events_;
    }
    ...
    bool scroll_begin() const
    {
        return scroll_begin_;
    }
    bool scroll_update() const
    {
        return scroll_update_;
    }
    bool scroll_end() const
    {
        return scroll_end_;
    }
    bool pinch_begin() const
    {
        return pinch_begin_;
    }
    bool pinch_update() const
    {
        return pinch_update_;
    }
    bool pinch_end() const
    {
        return pinch_end_;
    }
    ...
    bool two_finger_tap() const
    {
        return two_finger_tap_;
    }
    bool show_press() const
    {
        return show_press_;
    }
    ...
    float scroll_velocity_x() const
    {
        return scroll_velocity_x_;
    }
    float scroll_velocity_y() const
    {
        return scroll_velocity_y_;
    }
    ...
    {
        return scroll_x_hint_;
    }
    float scroll_y_hint() const
    {
        return scroll_y_hint_;
    }
    float scale() const
    {
        return scale_;
...
public:
    ...
    void OnTouchEvent(ui::TouchEvent* event) override
    {
        ...
        if (synchronous_ack_for_next_event_ != AckState::PENDING) {
            aura::Env::GetInstance()->gesture_recognizer()->AckTouchEvent(event->unique_event_id(),
                synchronous_ack_for_next_event_ == AckState::CONSUMED ? ui::ER_CONSUMED : ui::ER_UNHANDLED, false /* is_source_touch_event_set_blocking */,
                window_);
            synchronous_ack_for_next_event_ = AckState::PENDING;
        } else {
...
private:
    ...
    void ReceivedAckImpl(bool prevent_defaulted)
    {
        ...
        dispatcher_->ProcessedTouchEvent(
            sent_event_id, window_, prevent_defaulted ? ui::ER_HANDLED : ui::ER_UNHANDLED, false /* is_source_touch_event_set_blocking */);
    }
...
public:
    ...
    GestureEventSynthDelegate& operator=(const GestureEventSynthDelegate&) = delete;
    ...
    bool mouse_enter() const
    {
        return mouse_enter_;
    }
    bool mouse_exit() const
    {
        return mouse_exit_;
    }
    bool mouse_press() const
    {
        return mouse_press_;
    }
...
public:
    ...
    void SendScrollEvents(
        ui::EventSink* sink, int x_start, int y_start, int dx, int dy, int touch_id, int time_step_ms, int num_steps, GestureEventConsumeDelegate* delegate)
    {
        float x = x_start;
        float y = y_start;
        ...
        for (int i = 0; i < num_steps; i++) {
            ...
            y += dy;
            ui::TouchEvent move(
                ui::EventType::kTouchMoved, gfx::Point(x, y), tick_clock_.NowTicks(), ui::PointerDetails(ui::EventPointerType::kTouch, touch_id));
    ...
    void SendScrollEvent(ui::EventSink* sink, float x, float y, int touch_id, GestureEventConsumeDelegate* delegate)
    {
        delegate->Reset();
        ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(), Now(), ui::PointerDetails(ui::EventPointerType::kTouch, touch_id));
...
public:
    ...
    int touch_released_count() const
    {
        return touch_released_count_;
    }
    int touch_pressed_count() const
    {
        return touch_pressed_count_;
    }
    int touch_moved_count() const
    {
        return touch_moved_count_;
    }
...
public:
    ...
    RemoveOnTouchCancelHandler& operator=(const RemoveOnTouchCancelHandler&) = delete;
...
void DelayByLongPressTimeout()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), config.gesture_detector_config.longpress_timeout * 2);
    run_loop.Run();
}
...
void DelayByShowPressTimeout()
{
    ui::GestureProvider::Config config;
    base::RunLoop run_loop;
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(FROM_HERE, run_loop.QuitClosure(), config.gesture_detector_config.showpress_timeout * 2);
    run_loop.Run();
}
...
void SetTouchRadius(ui::TouchEvent* event, float radius_x, float radius_y)
{
    ...
    // radii.
    ui::PointerDetails details(ui::EventPointerType::kTouch, event->pointer_details().id, radius_x, radius_y, event->pointer_details().force,
        event->pointer_details().twist, event->pointer_details().tilt_x, event->pointer_details().tilt_y, event->pointer_details().tangential_pressure);
...
public:
    ...
    void SetUp() override
    {
        ...
        ui::GestureConfiguration::GetInstance()->set_short_press_time(base::Milliseconds(3));
...
public:
    ...
    GestureRecognizerWithSwitchTest(const GestureRecognizerWithSwitchTest&) = delete;
    GestureRecognizerWithSwitchTest& operator=(const GestureRecognizerWithSwitchTest&) = delete;

    void SetUp() override
    {
        GestureRecognizerTest::SetUp();
        base::CommandLine::ForCurrentProcess()->AppendSwitch(switches::kCompensateForUnstablePinchZoom);
...
TEST_F(GestureRecognizerTest, TouchCancelCanDestroyWindow)
{
    ...
    // events.
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(delegate.get(), -1234, gfx::Rect(0, 0, 200, 200), root_window()));
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(101, 101), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press);

    // Cancel event, verify there is no crash.
    aura::Env::GetInstance()->gesture_recognizer()->CancelActiveTouchesExcept(nullptr);
...
{
    std::unique_ptr<GestureEventConsumeDelegate> delegate(new GestureEventConsumeDelegate());
    TimedEvents tes;
    const int kWindowWidth = 123;
    const int kWindowHeight = 45;
    const int kTouchId = 2;
    gfx::Rect bounds(100, 200, kWindowWidth, kWindowHeight);
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(delegate.get(), -1234, bounds, root_window()));

    delegate->Reset();
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press);
    ...
    ui::TouchEvent release(
        ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
...
TEST_F(GestureRecognizerTest, GestureEventTapRegion)
{
    ...
    {
        delegate->Reset();
        ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(365, 290), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        ...
        delegate->Reset();
        ui::TouchEvent release(
            ui::EventType::kTouchReleased, gfx::Point(367, 291), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    ...
    {
        delegate->Reset();
        ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(46, 205), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        ...
        delegate->Reset();
        ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(49, 204), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        ...
        delegate->Reset();
        ui::TouchEvent release(
            ui::EventType::kTouchReleased, gfx::Point(49, 204), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    ...
    {
        delegate->Reset();
        ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(400, 150), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        ...
        delegate->Reset();
        ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(397, 151), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        ...
        delegate->Reset();
        ui::TouchEvent move1(ui::EventType::kTouchMoved, gfx::Point(397, 149), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        ...
        delegate->Reset();
        ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(400, 150), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        ...
        delegate->Reset();
        ui::TouchEvent release(
            ui::EventType::kTouchReleased, gfx::Point(401, 149), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
...
TEST_F(GestureRecognizerTest, GestureEventScroll)
{
    ...
    // sqrt(5 * 5 + 5 * 5).
    ui::GestureConfiguration::GetInstance()->set_max_touch_move_in_pixels_for_click(sqrt(5.f * 5 + 5 * 5));
    std::unique_ptr<GestureEventConsumeDelegate> delegate(new GestureEventConsumeDelegate());
    TimedEvents tes;
    const int kWindowWidth = 123;
    const int kWindowHeight = 45;
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureBegin, ui::EventType::kGestureTapDown);
    ...
    EXPECT_3_EVENTS(delegate->events(), ui::EventType::kGestureTapCancel, ui::EventType::kGestureScrollBegin, ui::EventType::kGestureScrollUpdate);
    // The slop consumed 5 dips
    EXPECT_FLOAT_EQ(5.5, delegate->scroll_x());
    EXPECT_FLOAT_EQ(5.5, delegate->scroll_y());
    EXPECT_EQ(gfx::Point(1, 1).ToString(), delegate->scroll_begin_position().ToString());
    ...
    ui::TouchEvent release(
        ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kScrollFlingStart, ui::EventType::kGestureEnd);
    EXPECT_TRUE(delegate->bounding_box().IsEmpty());
}
...
TEST_F(GestureRecognizerTest, GestureEventScrollPrediction)
{
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(96, 196), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureBegin, ui::EventType::kGestureTapDown);
    delegate->Reset();

    // Get rid of touch slop.
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(111, 211), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move);
    EXPECT_3_EVENTS(delegate->events(), ui::EventType::kGestureTapCancel, ui::EventType::kGestureScrollBegin, ui::EventType::kGestureScrollUpdate);
...
TEST_F(GestureRecognizerTest, GestureEventScrollBoundingBox)
{
    ...
    for (int radius = 1; radius <= 10; ++radius) {
        ...
        delegate->Reset();
        ui::TouchEvent press(
            ui::EventType::kTouchPressed, gfx::Point(kPositionX, kPositionY), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        DispatchEventUsingWindowDispatcher(&press);
        EXPECT_EQ(gfx::Rect(kPositionX - radius, kPositionY - radius, radius * 2, radius * 2), delegate->bounding_box());

        const int kScrollAmount = 50;
        tes.SendScrollEvents(GetEventSink(), kPositionX, kPositionY, 1, 1, kTouchId, 1, kScrollAmount, delegate.get());
        EXPECT_EQ(gfx::Point(1, 1).ToString(), delegate->scroll_begin_position().ToString());
        EXPECT_EQ(gfx::Rect(kPositionX + kScrollAmount - radius, kPositionY + kScrollAmount - radius, radius * 2, radius * 2), delegate->bounding_box());

        // Release the touch. This should end the scroll.
        delegate->Reset();
        ui::TouchEvent release(ui::EventType::kTouchReleased, gfx::Point(kPositionX + kScrollAmount, kPositionY + kScrollAmount),
            press.time_stamp() + base::Milliseconds(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        DispatchEventUsingWindowDispatcher(&release);
        EXPECT_EQ(gfx::Rect(kPositionX + kScrollAmount - radius, kPositionY + kScrollAmount - radius, radius * 2, radius * 2), delegate->bounding_box());
    }
...
TEST_F(GestureRecognizerTest, GestureEventHorizontalRailFling)
{
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(0, 0), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press);

    // Get rid of touch slop.
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(10, 0), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move);
    delegate->Reset();
    ...
    tes.SendScrollEvents(GetEventSink(), 1, 1, 100, 10, kTouchId, 1, kScrollAmount, delegate.get());

    delegate->Reset();
    ui::TouchEvent release(ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release);
...
TEST_F(GestureRecognizerTest, GestureEventVerticalRailFling)
{
    ...
    // Get rid of touch slop.
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(0, 10), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move);
    delegate->Reset();
    ...
    const int kScrollAmount = 8;
    tes.SendScrollEvents(GetEventSink(), 1, 6, 10, 100, kTouchId, 1, kScrollAmount, delegate.get());
    EXPECT_EQ(0, delegate->scroll_velocity_x());

    delegate->Reset();
    ui::TouchEvent release(ui::EventType::kTouchReleased, gfx::Point(101, 206), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release);
...
{
    ui::GestureConfiguration::GetInstance()->set_max_touch_move_in_pixels_for_click(0);
    std::unique_ptr<GestureEventConsumeDelegate> delegate(new GestureEventConsumeDelegate());
    TimedEvents tes;
    const int kTouchId = 7;
    gfx::Rect bounds(0, 0, 1000, 1000);
    ...
    const int kScrollAmount = 8;
    tes.SendScrollEvents(GetEventSink(), 1, 1, 10, 100, kTouchId, 1, kScrollAmount, delegate.get());
...
TEST_F(GestureRecognizerTest, GestureEventLongPress)
{
    ...
    ui::TouchEvent press1(
        ui::EventType::kTouchPressed, gfx::Point(101, 201), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    ...
    ui::TouchEvent release1(
        ui::EventType::kTouchReleased, gfx::Point(101, 201), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
...
TEST_F(GestureRecognizerTest, GestureEventLongPressCancelledByScroll)
{
    ...
    ui::TouchEvent press1(ui::EventType::kTouchPressed, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press1);
    EXPECT_TRUE(delegate->tap_down());
    ...
    delegate->Reset();
    ui::TouchEvent release1(
        ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.LeapForward(10), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release1);
    EXPECT_FALSE(delegate->long_press());
    EXPECT_FALSE(delegate->tap_cancel());
}
...
{
    ui::GestureConfiguration::GetInstance()->set_max_touch_down_duration_for_click_in_ms(3);
    std::unique_ptr<GestureEventConsumeDelegate> delegate(new GestureEventConsumeDelegate());
    const int kWindowWidth = 123;
    const int kWindowHeight = 45;
    const int kTouchId = 2;
...
TEST_F(GestureRecognizerTest, GestureEventLongPressCancelledBySecondTap)
{
    ...
    delegate->Reset();
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&press);
    EXPECT_TRUE(delegate->tap_down());
    EXPECT_TRUE(delegate->begin());
    ...
    delegate->Reset();
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(10, 10), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    ...
    delegate->Reset();
    ui::TouchEvent release1(ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&release1);
    EXPECT_FALSE(delegate->long_press());
...
TEST_F(GestureRecognizerTest, GestureEventHorizontalRailScroll)
{
    ...
    // Get rid of touch slop.
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(5, 0), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    ...
    const int kScrollAmount = 8;
    tes.SendScrollEvents(GetEventSink(), 1, 1, 6, 100, kTouchId, 1, kScrollAmount, delegate.get());
...
TEST_F(GestureRecognizerTest, GestureEventVerticalRailScroll)
{
    ...
    // Get rid of touch slop.
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(0, 5), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move);
    delegate->Reset();
    ...
    // and we can break the rail
    const int kScrollAmount = 8;
    tes.SendScrollEvents(GetEventSink(), 1, 6, 100, 1, kTouchId, 1, kScrollAmount, delegate.get());
...
TEST_F(GestureRecognizerTest, GestureTapFollowedByScroll)
{
    ...
    delegate->Reset();
    ui::TouchEvent press1(
        ui::EventType::kTouchPressed, gfx::Point(101, 201), tes.LeapForward(1000), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press1);
    EXPECT_FALSE(delegate->tap());
    EXPECT_TRUE(delegate->tap_down());
    EXPECT_FALSE(delegate->tap_cancel());
    ...
    // Get rid of touch slop.
    ui::TouchEvent move_remove_slop(ui::EventType::kTouchMoved, gfx::Point(116, 216), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    ...
    // and not a rail scroll.
    delegate->Reset();
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(135, 235), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move);
    EXPECT_FALSE(delegate->tap());
    EXPECT_FALSE(delegate->tap_down());
    EXPECT_FALSE(delegate->tap_cancel());
    ...
    // Move some more to generate a few more scroll updates.
    delegate->Reset();
    ui::TouchEvent move1(ui::EventType::kTouchMoved, gfx::Point(115, 216), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move1);
    EXPECT_FALSE(delegate->tap());
    EXPECT_FALSE(delegate->tap_down());
    EXPECT_FALSE(delegate->tap_cancel());
    ...
    delegate->Reset();
    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(145, 220), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move2);
    EXPECT_FALSE(delegate->tap());
    EXPECT_FALSE(delegate->tap_down());
    EXPECT_FALSE(delegate->tap_cancel());
    ...
    // Release the touch. This should end the scroll.
    delegate->Reset();
    ui::TouchEvent release1(ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release1);
    EXPECT_FALSE(delegate->tap());
    EXPECT_FALSE(delegate->tap_down());
    EXPECT_FALSE(delegate->tap_cancel());
...
TEST_F(GestureRecognizerTest, DestroyGestureProviderAuraBeforeAck)
{
    ...
    std::unique_ptr<GestureEventConsumeDelegate> delegate(new GestureEventConsumeDelegate());
    std::unique_ptr<aura::Window> window1(CreateTestWindowWithDelegate(delegate.get(), /*id=*/-2345, /*bounds=*/gfx::Rect(0, 0, 50, 50),
        /*parent=*/root_window()));

    // Touch press then release on `window1`.
    constexpr gfx::Point touch_location(/*x=*/10, /*y=*/20);
    ui::TouchEvent press(ui::EventType::kTouchPressed, touch_location, /*time_stamp=*/tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    delegate->Reset();
    DispatchEventUsingWindowDispatcher(&press);
    EXPECT_TRUE(delegate->tap_down());
    delegate->Reset();
    ui::TouchEvent release(ui::EventType::kTouchReleased, touch_location,
        /*time_stamp=*/press.time_stamp() + base::Milliseconds(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release);
    EXPECT_FALSE(delegate->tap_down());

    // Verify that the gesture provider for `window1` is created.
    auto* gesture_recognizer = static_cast<ui::GestureRecognizerImpl*>(aura::Env::GetInstance()->gesture_recognizer());
    ...
    std::unique_ptr<QueueTouchEventDelegate> delegate2(new QueueTouchEventDelegate(host()->dispatcher()));
    const int kTouchId2 = 4;
    std::unique_ptr<aura::Window> window2(CreateTestWindowWithDelegate(delegate2.get(), /*id=*/-1234, /*bounds=*/gfx::Rect(100, 100, 500, 500), root_window()));
    delegate2->set_window(window2.get());

    // Send a press event on `window2`. Verify that the gesture provider for
    // `window2` is created.
    ui::TouchEvent press2(ui::EventType::kTouchPressed, /*location=*/gfx::Point(200, 200),
        /*time_stamp=*/tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&press2);
    EXPECT_NE(consumers.cend(), consumers.find(window2.get()));

    // Verify that `press2` is associated with a gesture provider raw pointer.
    const auto& event_provider_mappings = gesture_recognizer->event_to_gesture_provider_;
    EXPECT_NE(event_provider_mappings.cend(), event_provider_mappings.find(press2.unique_event_id()));

    // Before ACKing `press2`, replacing the gesture provider of `window2` with a
    // new value through event transferal.
    aura::Env::GetInstance()->gesture_recognizer()->TransferEventsTo(window1.get(), window2.get(), ui::TransferTouchesBehavior::kCancel);
...
{
    std::unique_ptr<QueueTouchEventDelegate> queued_delegate(new QueueTouchEventDelegate(host()->dispatcher()));
    TimedEvents tes;
    const int kWindowWidth = 123;
    const int kWindowHeight = 45;
    ...
    const int kTouchId2 = 4;
    gfx::Rect bounds(100, 200, kWindowWidth, kWindowHeight);
    std::unique_ptr<aura::Window> queue(CreateTestWindowWithDelegate(queued_delegate.get(), -1234, bounds, root_window()));
    ...
    queued_delegate->Reset();
    ui::TouchEvent release(ui::EventType::kTouchReleased, gfx::Point(101, 201), press.time_stamp() + base::Milliseconds(50),
    ...
    // tap-down gesture.
    std::unique_ptr<GestureEventConsumeDelegate> delegate(new GestureEventConsumeDelegate());
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(delegate.get(), -2345, gfx::Rect(0, 0, 50, 50), root_window()));
    delegate->Reset();
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(10, 20), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&press2);
    EXPECT_FALSE(delegate->tap());
    EXPECT_TRUE(delegate->tap_down());
    EXPECT_FALSE(delegate->tap_cancel());
    ...
    ui::TouchEvent release2(ui::EventType::kTouchReleased, gfx::Point(10, 20), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    ...
    queued_delegate->Reset();
    ui::TouchEvent press3(ui::EventType::kTouchPressed, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    ...
    queued_delegate->Reset();
    delegate->Reset();
    ui::TouchEvent press4(ui::EventType::kTouchPressed, gfx::Point(103, 203), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    ...
    queued_delegate->Reset();
    delegate->Reset();
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(203 + ui::GestureConfiguration::GetInstance()->max_touch_move_in_pixels_for_click(), 303),
...
TEST_F(GestureRecognizerTest, GestureEventPinchFromScroll)
{
    ...
    // should generate both SCROLL_BEGIN and SCROLL_UPDATE gestures.
    delegate->Reset();
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(130, 301), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move);
    EXPECT_3_EVENTS(delegate->events(), ui::EventType::kGestureTapCancel, ui::EventType::kGestureScrollBegin, ui::EventType::kGestureScrollUpdate);
    ...
    EXPECT_EQ(gfx::Rect(10, 10, 120, 291).ToString(), delegate->bounding_box().ToString());

    // Move the first finger.
    delegate->Reset();
    ui::TouchEvent move3(ui::EventType::kTouchMoved, gfx::Point(95, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move3);
    EXPECT_3_EVENTS(delegate->events(), ui::EventType::kGestureScrollUpdate, ui::EventType::kGesturePinchBegin, ui::EventType::kGesturePinchUpdate);
    EXPECT_EQ(gfx::Rect(10, 10, 85, 191).ToString(), delegate->bounding_box().ToString());

    // Now move the second finger.
    delegate->Reset();
    ui::TouchEvent move4(ui::EventType::kTouchMoved, gfx::Point(55, 15), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&move4);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureScrollUpdate, ui::EventType::kGesturePinchUpdate);
    EXPECT_EQ(gfx::Rect(55, 15, 40, 186).ToString(), delegate->bounding_box().ToString());

    // Release the first finger. This should end pinch.
    delegate->Reset();
    ui::TouchEvent release(ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&release);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGesturePinchEnd, ui::EventType::kGestureEnd);
    EXPECT_EQ(gfx::Rect(55, 15, 46, 186).ToString(), delegate->bounding_box().ToString());

    // Move the second finger. This should still generate a scroll.
    delegate->Reset();
    ui::TouchEvent move5(ui::EventType::kTouchMoved, gfx::Point(25, 10), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
...
TEST_F(GestureRecognizerTest, GestureEventPinchFromScrollFromPinch)
{
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(101, 301), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&press);
    delegate->Reset();
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(10, 10), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&press2);
    ...
    // Pinch again
    delegate->Reset();
    ui::TouchEvent press3(ui::EventType::kTouchPressed, gfx::Point(10, 10), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&press3);
    ...
    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(101, 50), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move2);
    EXPECT_TRUE(delegate->pinch_begin());
...
TEST_F(GestureRecognizerTest, GestureEventPinchFromTap)
{
    ...
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(10, 10), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&press2);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureTapCancel, ui::EventType::kGestureBegin);
    EXPECT_EQ(gfx::Rect(10, 10, 91, 291).ToString(), delegate->bounding_box().ToString());

    // Move the first finger.
    delegate->Reset();
    ui::TouchEvent move3(ui::EventType::kTouchMoved, gfx::Point(65, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move3);
    EXPECT_4_EVENTS(delegate->events(), ui::EventType::kGestureScrollBegin, ui::EventType::kGestureScrollUpdate, ui::EventType::kGesturePinchBegin,
        ui::EventType::kGesturePinchUpdate);
    EXPECT_EQ(gfx::Rect(10, 10, 55, 191).ToString(), delegate->bounding_box().ToString());
    ...
    DispatchEventUsingWindowDispatcher(&move4);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureScrollUpdate, ui::EventType::kGesturePinchUpdate);
    EXPECT_EQ(gfx::Rect(55, 15, 10, 186).ToString(), delegate->bounding_box().ToString());

    // Release the first finger. This should end pinch.
    delegate->Reset();
    ui::TouchEvent release(
        ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.LeapForward(10), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&release);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGesturePinchEnd, ui::EventType::kGestureEnd);
    EXPECT_EQ(gfx::Rect(55, 15, 46, 186).ToString(), delegate->bounding_box().ToString());
...
TEST_F(GestureRecognizerTest, GestureEventIgnoresDisconnectedEvents)
{
    ...
    ui::TouchEvent release1(ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, 6));
    DispatchEventUsingWindowDispatcher(&release1);
    EXPECT_FALSE(delegate->tap());
    EXPECT_FALSE(delegate->tap_down());
}
...
{
    ui::GestureRecognizer* gesture_recognizer = aura::Env::GetInstance()->gesture_recognizer();
    TimedEvents tes;

    ui::GestureConsumer* target;
    const int kNumWindows = 4;

    std::unique_ptr<GestureEventConsumeDelegate*[]> delegates(new GestureEventConsumeDelegate*[kNumWindows]);

    ui::GestureConfiguration::GetInstance()->set_max_separation_for_gesture_touches_in_pixels(499);
    ...
    for (int i = 0; i < kNumWindows; ++i) {
        ...
        windows[i] = CreateTestWindowWithDelegate(delegates[i], i, window_bounds[i], root_window());
        windows[i]->SetId(i);
        ui::TouchEvent press(ui::EventType::kTouchPressed, window_bounds[i].origin(), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, i));
        DispatchEventUsingWindowDispatcher(&press);
    }
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(0, 500), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kNumWindows));
    DispatchEventUsingWindowDispatcher(&press);
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(250, 250), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kNumWindows));
    DispatchEventUsingWindowDispatcher(&move);
    ...
    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(1000, 1000), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kNumWindows));
    DispatchEventUsingWindowDispatcher(&move2);
...
TEST_F(GestureRecognizerTest, GestureEventTouchLockIgnoresOtherScreens)
{
    ...
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(delegate.get(), 0, bounds, root_window()));

    const int kTouchId1 = 8;
    const int kTouchId2 = 2;
    TimedEvents tes;

    ui::TouchEvent press1(ui::EventType::kTouchPressed, gfx::Point(5, 5), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    ui::EventTestApi test_press1(&press1);
    test_press1.set_source_device_id(1);
    DispatchEventUsingWindowDispatcher(&press1);

    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(20, 20), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    ...
    EXPECT_NE(aura::Env::GetInstance()->gesture_recognizer()->GetTouchLockedTarget(press1),
        aura::Env::GetInstance()->gesture_recognizer()->GetTouchLockedTarget(press2));
}
...
{
    TimedEvents tes;
    std::unique_ptr<aura::Window> window(CreateTestWindowWithBounds(gfx::Rect(-100, -100, 2000, 2000), root_window()));

    gfx::Point pos1(-10, -10);
    ui::TouchEvent press1(ui::EventType::kTouchPressed, pos1, tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&press1);

    gfx::Point pos2(1000, 1000);
    ui::TouchEvent press2(ui::EventType::kTouchPressed, pos2, tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
    DispatchEventUsingWindowDispatcher(&press2);

    // As these presses were outside the root window, they should be
    // associated with the root window.
    EXPECT_EQ(root_window(), static_cast<aura::Window*>(aura::Env::GetInstance()->gesture_recognizer()->GetTouchLockedTarget(press1)));
    EXPECT_EQ(root_window(), static_cast<aura::Window*>(aura::Env::GetInstance()->gesture_recognizer()->GetTouchLockedTarget(press2)));
}

TEST_F(GestureRecognizerTest, NoTapWithPreventDefaultedRelease)
{
    std::unique_ptr<QueueTouchEventDelegate> delegate(new QueueTouchEventDelegate(host()->dispatcher()));
    TimedEvents tes;
    const int kTouchId = 2;
...
TEST_F(GestureRecognizerTest, PinchScrollWithPreventDefaultedRelease)
{
    ...
    {
        delegate->Reset();
        ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(15, 25), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
        ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(20, 95), tes.LeapForward(200), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
        ui::TouchEvent release(
            ui::EventType::kTouchReleased, gfx::Point(15, 25), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
        DispatchEventUsingWindowDispatcher(&press);
        DispatchEventUsingWindowDispatcher(&move);
        DispatchEventUsingWindowDispatcher(&release);
        delegate->Reset();
        ...
        delegate->ReceivedAckPreventDefaulted();
        EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureScrollEnd, ui::EventType::kGestureEnd);
    }

    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(15, 25), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(20, 95), tes.LeapForward(200), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    ui::TouchEvent release(ui::EventType::kTouchReleased, gfx::Point(15, 25), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(55, 25), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(145, 85), tes.LeapForward(1000), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    ui::TouchEvent release2(
        ui::EventType::kTouchReleased, gfx::Point(145, 85), tes.LeapForward(14), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
...
TEST_F(GestureRecognizerTest, GestureEndLocation)
{
    ...
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(&delegate, -1234, gfx::Rect(10, 10, 300, 300), root_window()));
    ...
    EXPECT_EQ((begin - window_offset).ToString(), delegate.scroll_begin_position().ToString());
    EXPECT_EQ((end - window_offset).ToString(), delegate.gesture_end_location().ToString());
}

TEST_F(GestureRecognizerTest, CaptureSendsGestureEnd)
{
    std::unique_ptr<GestureEventConsumeDelegate> delegate(new GestureEventConsumeDelegate());
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(delegate.get(), -1234, gfx::Rect(10, 10, 300, 300), root_window()));
    ...
    std::unique_ptr<aura::Window> capture(CreateTestWindowWithBounds(gfx::Rect(10, 10, 200, 200), root_window()));
    capture->SetCapture();
    RunAllPendingInMessageLoop();
...
TEST_F(GestureRecognizerTest, CaptureDoesNotCancelFinishedTouches)
{
    ...
    std::unique_ptr<aura::Window> window1(CreateTestWindowWithDelegate(delegate.get(), -1234, gfx::Rect(10, 10, 300, 300), root_window()));
    window1->SetCapture();
    ...
    std::unique_ptr<ui::TouchEvent> touch0(
        new ui::TouchEvent(ui::EventType::kTouchPressed, gfx::Point(20, 20), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, 0)));
    std::unique_ptr<ui::TouchEvent> touch1(
        new ui::TouchEvent(ui::EventType::kTouchPressed, gfx::Point(30, 30), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, 1)));
    ...
    // cover both cases.
    touch0
        = std::make_unique<ui::TouchEvent>(ui::EventType::kTouchReleased, gfx::Point(20, 20), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    touch1
        = std::make_unique<ui::TouchEvent>(ui::EventType::kTouchCancelled, gfx::Point(30, 30), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
    generator.Dispatch(touch0.get());
    generator.Dispatch(touch1.get());
    RunAllPendingInMessageLoop();
    ...
    std::unique_ptr<aura::Window> window2(CreateTestWindowWithBounds(gfx::Rect(100, 100, 300, 300), root_window()));
    window2->SetCapture();
    RunAllPendingInMessageLoop();
...
TEST_F(GestureRecognizerTest, PressDoesNotCrash)
{
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(45, 45), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, 7));
    SetTouchRadius(&press, 40, 0);
    DispatchEventUsingWindowDispatcher(&press);
    EXPECT_TRUE(delegate->tap_down());
    EXPECT_EQ(gfx::Rect(5, 5, 80, 80).ToString(), delegate->bounding_box().ToString());
    delegate->Reset();

    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(55, 45), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, 7));
    DispatchEventUsingWindowDispatcher(&press2);
...
TEST_F(GestureRecognizerTest, TwoFingerTap)
{
    ...
    delegate->Reset();
    ui::TouchEvent press1(ui::EventType::kTouchPressed, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&press1);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureBegin, ui::EventType::kGestureTapDown);

    delegate->Reset();
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(130, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&press2);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureTapCancel, ui::EventType::kGestureBegin);
    ...
    delegate->Reset();
    ui::TouchEvent move1(ui::EventType::kTouchMoved, gfx::Point(102, 202), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move1);
    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(131, 202), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&move2);
    ...
    delegate->Reset();
    ui::TouchEvent release1(
        ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));

    DispatchEventUsingWindowDispatcher(&release1);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureTwoFingerTap, ui::EventType::kGestureEnd);
    ...
    delegate->Reset();
    ui::TouchEvent release2(
        ui::EventType::kTouchReleased, gfx::Point(130, 201), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
...
TEST_F(GestureRecognizerTest, TwoFingerTapExpired)
{
    ...
    delegate->Reset();
    ui::TouchEvent release1(
        ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.LeapForward(1000), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
...
TEST_F(GestureRecognizerTest, TwoFingerTapChangesToPinch)
{
    ...
    // Test moving first finger
    {
        ...
        delegate->Reset();
        ui::TouchEvent release(
            ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
...
TEST_F(GestureRecognizerTest, MultiFingerSwipe)
{
    ...
    gfx::Point points[kTouchPoints] = { gfx::Point(10, 30), gfx::Point(30, 20), gfx::Point(50, 30), gfx::Point(80, 50) };
    ...
    for (int count = 2; count <= kTouchPoints; ++count) {
        generator.GestureMultiFingerScroll(count, points, 10, kSteps, 0, -11 * kSteps);
        ...
        generator.GestureMultiFingerScroll(count, points, 10, kSteps, -11 * kSteps, 0);
        ...
        generator.GestureMultiFingerScroll(count, points, 10, kSteps, 5 * kSteps, 12 * kSteps);
        EXPECT_FALSE(delegate->swipe_down());
        delegate->Reset();

        generator.GestureMultiFingerScroll(count, points, 10, kSteps, 4 * kSteps, 12 * kSteps);
        EXPECT_TRUE(delegate->swipe_down());
        delegate->Reset();

        generator.GestureMultiFingerScroll(count, points, 10, kSteps, 3 * kSteps, 12 * kSteps);
        EXPECT_TRUE(delegate->swipe_down());
        delegate->Reset();
    }
}
...
TEST_F(GestureRecognizerTest, TwoFingerTapCancelled)
{
    ...
    // Test canceling first finger.
    {
        ...
        delegate->Reset();
        ui::TouchEvent cancel(ui::EventType::kTouchCancelled, gfx::Point(130, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    ...
    // Test canceling second finger
    {
        ...
        delegate->Reset();
        ui::TouchEvent cancel(ui::EventType::kTouchCancelled, gfx::Point(130, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
        DispatchEventUsingWindowDispatcher(&cancel);
        EXPECT_FALSE(delegate->two_finger_tap());
...
TEST_F(GestureRecognizerTest, VeryWideTwoFingerTouchDownShouldBeAPinch)
{
    ...
    delegate->Reset();
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(430, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&press2);
    EXPECT_FALSE(delegate->tap());
    EXPECT_FALSE(delegate->tap_down()); // no touch down for second tap.
    EXPECT_TRUE(delegate->tap_cancel());
    ...
    delegate->Reset();
    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(530, 301), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&move2);
    EXPECT_FALSE(delegate->tap());
    EXPECT_FALSE(delegate->tap_down());
    EXPECT_FALSE(delegate->tap_cancel());
...
TEST_F(GestureRecognizerTest, FlushAllOnHide)
{
    ...
    ui::TouchEvent press1(ui::EventType::kTouchPressed, gfx::Point(10, 10), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&press1);
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(20, 20), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&press2);
    window->Hide();
    EXPECT_EQ(nullptr, aura::Env::GetInstance()->gesture_recognizer()->GetTouchLockedTarget(press1));
    EXPECT_EQ(nullptr, aura::Env::GetInstance()->gesture_recognizer()->GetTouchLockedTarget(press2));
}
...
public:
    ...
    ConsumesTouchMovesDelegate& operator=(const ConsumesTouchMovesDelegate&) = delete;

    ~ConsumesTouchMovesDelegate() override
    {
    }

    void set_consume_touch_move(bool consume)
    {
        consume_touch_move_ = consume;
    }
...
{
    std::unique_ptr<ConsumesTouchMovesDelegate> delegate(new ConsumesTouchMovesDelegate());
    const int kWindowWidth = 123;
    const int kWindowHeight = 45;
    const int kTouchId = 5;
    gfx::Rect bounds(100, 200, kWindowWidth, kWindowHeight);
    ...
    delegate->Reset();
    ui::TouchEvent release(
        ui::EventType::kTouchReleased, gfx::Point(130, 230), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release);
    EXPECT_FALSE(delegate->tap());
    EXPECT_FALSE(delegate->tap_down());
    EXPECT_FALSE(delegate->tap_cancel());
...
TEST_F(GestureRecognizerTest, GestureEventScrollTwoFingerTouchMoveConsumed)
{
    ...
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureTapCancel, ui::EventType::kGestureScrollBegin);

    delegate->Reset();
    // Second finger touches down and moves.
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(130, 201), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&press2);
    ...
    delegate->Reset();
    ui::TouchEvent release1(ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    ui::TouchEvent release2(ui::EventType::kTouchReleased, gfx::Point(130, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&release1);
    DispatchEventUsingWindowDispatcher(&release2);

    EXPECT_3_EVENTS(delegate->events(), ui::EventType::kGestureEnd, ui::EventType::kScrollFlingStart, ui::EventType::kGestureEnd);
}
...
TEST_F(GestureRecognizerTest, GestureEventScrollTouchMovePartialConsumed)
{
    ...
    EXPECT_EQ(gfx::Point(0, 0).ToString(), delegate->scroll_begin_position().ToString());
...
TEST_F(GestureRecognizerTest, GestureEventDoubleTap)
{
    ...
    ui::TouchEvent press1(ui::EventType::kTouchPressed, gfx::Point(104, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press1);
    ui::TouchEvent release1(
        ui::EventType::kTouchReleased, gfx::Point(104, 201), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release1);
    delegate->Reset();

    // Second tap
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(101, 203), tes.LeapForward(200), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press2);
    ui::TouchEvent release2(
        ui::EventType::kTouchReleased, gfx::Point(102, 206), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release2);
...
TEST_F(GestureRecognizerTest, GestureEventTripleTap)
{
    ...
    // Third tap
    ui::TouchEvent press3(ui::EventType::kTouchPressed, gfx::Point(102, 206), tes.LeapForward(200), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press3);
    ...
    for (int i = 4; i < 6; ++i) {
        ui::TouchEvent press4(
            ui::EventType::kTouchPressed, gfx::Point(102, 206), tes.LeapForward(200), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        DispatchEventUsingWindowDispatcher(&press4);
...
TEST_F(GestureRecognizerTest, TwoTapsFarApart)
{
    ...
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(201, 201), tes.LeapForward(200), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press2);
    ui::TouchEvent release2(
        ui::EventType::kTouchReleased, gfx::Point(201, 201), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release2);
...
TEST_F(GestureRecognizerTest, TwoTapsWithDelayBetween)
{
    ...
    ui::TouchEvent press2(
        ui::EventType::kTouchPressed, gfx::Point(101, 201), tes.LeapForward(2000), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press2);
    ui::TouchEvent release2(
        ui::EventType::kTouchReleased, gfx::Point(101, 201), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release2);
...
TEST_F(GestureRecognizerTest, BoundingBoxRadiusChange)
{
    ...
    ui::TouchEvent press2(
        ui::EventType::kTouchPressed, gfx::Point(201, 201), tes.LeapForward(400), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    SetTouchRadius(&press2, 5, 0);
    DispatchEventUsingWindowDispatcher(&press2);
    EXPECT_FALSE(delegate->pinch_begin());
    EXPECT_EQ(gfx::Rect(101, 196, 105, 10).ToString(), delegate->bounding_box().ToString());

    delegate->Reset();

    ui::TouchEvent move1(ui::EventType::kTouchMoved, gfx::Point(50, 50), tes.LeapForward(40), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move1);
    EXPECT_TRUE(delegate->pinch_begin());
    EXPECT_EQ(gfx::Rect(50, 50, 156, 156).ToString(), delegate->bounding_box().ToString());

    delegate->Reset();

    // The position doesn't move, but the radius changes.
    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(50, 50), tes.LeapForward(40), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
...
{
    ui::GestureConfiguration::GetInstance()->set_max_touch_move_in_pixels_for_click(3);
    std::unique_ptr<GestureEventConsumeDelegate> delegate(new GestureEventConsumeDelegate());
    const int kWindowWidth = 234;
    const int kWindowHeight = 345;
    const int kTouchId = 5;
    ...
    ui::TouchEvent press1(ui::EventType::kTouchPressed, gfx::Point(101, 208), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press1);
    EXPECT_TRUE(delegate->begin());

    delegate->Reset();

    ui::TouchEvent move1(ui::EventType::kTouchMoved, gfx::Point(101, 206), tes.LeapForward(40), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move1);
    EXPECT_FALSE(delegate->scroll_begin());

    delegate->Reset();

    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(101, 204), tes.LeapForward(40), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move2);
    EXPECT_TRUE(delegate->tap_cancel());
    EXPECT_TRUE(delegate->scroll_begin());
    EXPECT_TRUE(delegate->scroll_update());
    ...
    ui::TouchEvent move3(ui::EventType::kTouchMoved, gfx::Point(101, 204), tes.LeapForward(40), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move3);
    EXPECT_FALSE(delegate->scroll_update());

    delegate->Reset();

    ui::TouchEvent move4(ui::EventType::kTouchMoved, gfx::Point(101, 203), tes.LeapForward(40), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move4);
    EXPECT_TRUE(delegate->scroll_update());
    EXPECT_EQ(-1, delegate->scroll_y());
...
TEST_F(GestureRecognizerTest, CancelAllActiveTouches)
{
    ...
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureBegin, ui::EventType::kGestureTapDown);
    delegate->Reset();
    ui::TouchEvent p2(ui::EventType::kTouchPressed, gfx::Point(50, 50), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&p2);
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureTapCancel, ui::EventType::kGestureBegin);
    delegate->Reset();
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(350, 300), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&move);
    EXPECT_4_EVENTS(delegate->events(), ui::EventType::kGestureScrollBegin, ui::EventType::kGestureScrollUpdate, ui::EventType::kGesturePinchBegin,
        ui::EventType::kGesturePinchUpdate);
    EXPECT_EQ(2, handler->touch_pressed_count());
    ...
    ui::GestureRecognizer* gesture_recognizer = aura::Env::GetInstance()->gesture_recognizer();
    EXPECT_EQ(window.get(), gesture_recognizer->GetTouchLockedTarget(press));

    aura::Env::GetInstance()->gesture_recognizer()->CancelActiveTouchesExcept(nullptr);

    EXPECT_EQ(nullptr, gesture_recognizer->GetTouchLockedTarget(press));
    EXPECT_4_EVENTS(
        delegate->events(), ui::EventType::kGesturePinchEnd, ui::EventType::kGestureScrollEnd, ui::EventType::kGestureEnd, ui::EventType::kGestureEnd);
...
TEST_F(GestureRecognizerTest, GestureEventConsumedTouchMoveScrollTest)
{
    ...
    ui::TouchEvent move1(ui::EventType::kTouchMoved, gfx::Point(10, 10), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move1);
    delegate->ReceivedAckPreventDefaulted();

    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(20, 20), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move2);
    delegate->ReceivedAck();
...
TEST_F(GestureRecognizerTest, GestureEventConsumedTouchMoveTapTest)
{
    ...
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(2, 2), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move);
    delegate->ReceivedAckPreventDefaulted();

    ui::TouchEvent release(ui::EventType::kTouchReleased, gfx::Point(2, 2), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release);
    delegate->ReceivedAck();
...
TEST_F(GestureRecognizerTest, GestureEventConsumedTouchMoveLongPressTest)
{
    ...
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(103, 203), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move);
    delegate->ReceivedAckPreventDefaulted();
...
TEST_F(GestureRecognizerTest, TestExceedingSlopSlowly)
{
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(10, 10), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press);
    EXPECT_FALSE(delegate->scroll_begin());
    EXPECT_FALSE(delegate->scroll_update());
    delegate->Reset();

    ui::TouchEvent move1(ui::EventType::kTouchMoved, gfx::Point(11, 10), tes.LeapForward(40), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move1);
    EXPECT_FALSE(delegate->scroll_begin());
    EXPECT_FALSE(delegate->scroll_update());
    EXPECT_EQ(0, delegate->scroll_x());
    ...
    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(12, 10), tes.LeapForward(40), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move2);
    EXPECT_FALSE(delegate->scroll_begin());
    EXPECT_FALSE(delegate->scroll_update());
    EXPECT_EQ(0, delegate->scroll_x());
    ...
    ui::TouchEvent move3(ui::EventType::kTouchMoved, gfx::Point(), tes.LeapForward(40), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    ...
    ui::TouchEvent move4(ui::EventType::kTouchMoved, gfx::Point(14, 10), tes.LeapForward(40), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move4);
    EXPECT_FALSE(delegate->scroll_begin());
    EXPECT_TRUE(delegate->scroll_update());
...
TEST_F(GestureRecognizerTest, ScrollAlternatelyConsumedTest)
{
    ...
    ui::TouchEvent press1(ui::EventType::kTouchPressed, gfx::Point(x, y), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press1);
    delegate->ReceivedAck();
    EXPECT_FALSE(delegate->scroll_begin());
    EXPECT_FALSE(delegate->scroll_update());
    ...
    x += 100;
    y += 100;
    ui::TouchEvent move1(ui::EventType::kTouchMoved, gfx::Point(x, y), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&move1);
    delegate->ReceivedAck();
    EXPECT_TRUE(delegate->scroll_begin());
    EXPECT_TRUE(delegate->scroll_update());
    ...
    for (int i = 0; i < 3; ++i) {
        x += 10;
        y += 10;
        ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(x, y), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        DispatchEventUsingWindowDispatcher(&move2);
        delegate->ReceivedAck();
        EXPECT_FALSE(delegate->scroll_begin());
        EXPECT_TRUE(delegate->scroll_update());
        ...
        x += 20;
        y += 20;
        ui::TouchEvent move3(ui::EventType::kTouchMoved, gfx::Point(x, y), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
        DispatchEventUsingWindowDispatcher(&move3);
        delegate->ReceivedAckPreventDefaulted();
        EXPECT_FALSE(delegate->scroll_begin());
        EXPECT_FALSE(delegate->scroll_update());
...
TEST_F(GestureRecognizerTest, PinchAlternatelyConsumedTest)
{
    ...
    ui::TouchEvent press1(ui::EventType::kTouchPressed, gfx::Point(0, 0), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&press1);
    delegate->ReceivedAck();
    EXPECT_FALSE(delegate->scroll_begin());
    EXPECT_FALSE(delegate->scroll_update());
    ...
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(x, y), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&press2);
    delegate->ReceivedAck();
    EXPECT_FALSE(delegate->scroll_begin());
    EXPECT_FALSE(delegate->scroll_update());
    ...
    x += 100;
    y += 100;
    ui::TouchEvent move1(ui::EventType::kTouchMoved, gfx::Point(x, y), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&move1);
    delegate->ReceivedAck();
    EXPECT_TRUE(delegate->scroll_begin());
    EXPECT_TRUE(delegate->scroll_update());
    ...
    for (int i = 0; i < 3; ++i) {
        x += 50;
        y += 50;
        ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(x, y), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
        DispatchEventUsingWindowDispatcher(&move2);
        delegate->ReceivedAck();
        EXPECT_FALSE(delegate->scroll_begin());
        EXPECT_TRUE(delegate->scroll_update());
        ...
        x += 100;
        y += 100;
        ui::TouchEvent move3(ui::EventType::kTouchMoved, gfx::Point(x, y), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
        DispatchEventUsingWindowDispatcher(&move3);
        delegate->ReceivedAckPreventDefaulted();
        EXPECT_FALSE(delegate->scroll_begin());
        EXPECT_FALSE(delegate->scroll_update());
    ...
    for (int i = 0; i < 2; ++i) {
        y += delta_y[i];
        ui::TouchEvent move4(ui::EventType::kTouchMoved, gfx::Point(x, y), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
        DispatchEventUsingWindowDispatcher(&move4);
        delegate->ReceivedAck();
...
public:
    ...
    GestureEventDeleteWindowOnLongPress(const GestureEventDeleteWindowOnLongPress&) = delete;
    GestureEventDeleteWindowOnLongPress& operator=(const GestureEventDeleteWindowOnLongPress&) = delete;

    ~GestureEventDeleteWindowOnLongPress() override
    {
        DCHECK(!window_);
    }
...
TEST_F(GestureRecognizerTest, GestureEventLongPressDeletingWindow)
{
    ...
    const int kTouchId = 2;
    gfx::Rect bounds(100, 200, kWindowWidth, kWindowHeight);
    aura::Window* window(CreateTestWindowWithDelegate(&delegate, -1234, bounds, root_window()));
...
TEST_F(GestureRecognizerWithSwitchTest, GestureEventSmallPinchDisabled)
{
    ...
    ui::TouchEvent press1(ui::EventType::kTouchPressed, gfx::Point(101, 301), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&press1);
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(10, 10), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&press2);
    ...
    // Move the first finger.
    delegate->Reset();
    ui::TouchEvent move1(ui::EventType::kTouchMoved, gfx::Point(65, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move1);
    ...
    delegate->Reset();
    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(65, 202), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move2);
    EXPECT_1_EVENT(delegate->events(), ui::EventType::kGestureScrollUpdate);
}
...
TEST_F(GestureRecognizerTest, IgnoredEventsDontBreakGestureRecognition)
{
    ...
    ui::TouchEvent move3(ui::EventType::kTouchMoved, gfx::Point(65, 202), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move3);
...
TEST_F(GestureRecognizerTest, MixedSyncAndAsyncAcksDontCauseOutOfOrderDispatch)
{
    std::unique_ptr<QueueTouchEventDelegate> delegate(new QueueTouchEventDelegate(host()->dispatcher()));
    TimedEvents tes;
    const int kWindowWidth = 300;
    ...
    ui::TouchEvent move1(ui::EventType::kTouchMoved, gfx::Point(100, 100), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move1);
    delegate->ReceivedAck();
    ...
    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(200, 200), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move2);
    EXPECT_0_EVENTS(delegate->events());

    // Dispatch a touch move, but don't ack it.
    ui::TouchEvent move3(ui::EventType::kTouchMoved, gfx::Point(300, 300), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move3);

    // Dispatch two synchronously consumed touch moves, which should be ignored.
    delegate->set_synchronous_ack_for_next_event(true);
    ui::TouchEvent move4(ui::EventType::kTouchMoved, gfx::Point(400, 400), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move4);

    delegate->set_synchronous_ack_for_next_event(true);
    ui::TouchEvent move5(ui::EventType::kTouchMoved, gfx::Point(500, 500), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&move5);
    ...
    // Dispatch a touch move, but don't ack it.
    delegate->Reset();
    ui::TouchEvent move6(ui::EventType::kTouchMoved, gfx::Point(600, 600), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    ...
    ui::TouchEvent move7(ui::EventType::kTouchMoved, gfx::Point(700, 700), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    ...
    delegate->ReceivedAck();
    EXPECT_2_EVENTS(delegate->events(), ui::EventType::kGestureScrollUpdate, ui::EventType::kGestureScrollUpdate);
}
...
TEST_F(GestureRecognizerTest, GestureEventTwoWindowsActive)
{
    ...
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(queued_delegate.get(), -1234, bounds, root_window()));
    queued_delegate->set_window(window.get());

    // Touch down on the window. This should not generate any gesture event.
    queued_delegate->Reset();
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(151, 201), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId1));
    DispatchEventUsingWindowDispatcher(&press);
    EXPECT_FALSE(queued_delegate->tap());
    EXPECT_FALSE(queued_delegate->tap_down());
    EXPECT_FALSE(queued_delegate->tap_cancel());
    ...
    // gesture event.
    std::unique_ptr<QueueTouchEventDelegate> queued_delegate2(new QueueTouchEventDelegate(host()->dispatcher()));
    gfx::Rect bounds2(0, 0, kWindowWidth, kWindowHeight);
    std::unique_ptr<aura::Window> window2(CreateTestWindowWithDelegate(queued_delegate2.get(), -2345, bounds2, root_window()));
    queued_delegate2->set_window(window2.get());

    queued_delegate2->Reset();
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(1, 1), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId2));
    DispatchEventUsingWindowDispatcher(&press2);
...
{
    std::unique_ptr<QueueTouchEventDelegate> delegate_1(new QueueTouchEventDelegate(host()->dispatcher()));
    TimedEvents tes;
    const int kTouchId = 7;
    gfx::Rect bounds(0, 0, 1000, 1000);

    std::unique_ptr<aura::Window> window_1(CreateTestWindowWithDelegate(delegate_1.get(), -1234, bounds, root_window()));

    delegate_1->set_window(window_1.get());

    delegate_1->Reset();
    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(512, 512), tes.Now(), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&press);

    // Create a new consumer and Touch event delegate.
    std::unique_ptr<QueueTouchEventDelegate> delegate_2(new QueueTouchEventDelegate(host()->dispatcher()));
    std::unique_ptr<aura::Window> window_2(CreateTestWindowWithDelegate(delegate_2.get(), -2345, bounds, root_window()));
    delegate_2->set_window(window_2.get());

    // Transfer event sequence from previous window to the new window.
    aura::Env::GetInstance()->gesture_recognizer()->TransferEventsTo(window_1.get(), window_2.get(), ui::TransferTouchesBehavior::kDontCancel);
    ...
    EXPECT_2_EVENTS(delegate_1->events(), ui::EventType::kGestureBegin, ui::EventType::kGestureTapDown);

    delegate_1->Reset();

    ui::TouchEvent release(
        ui::EventType::kTouchReleased, gfx::Point(550, 512), tes.LeapForward(50), ui::PointerDetails(ui::EventPointerType::kTouch, kTouchId));
    DispatchEventUsingWindowDispatcher(&release);
    ...
    EXPECT_3_EVENTS(delegate_2->events(), ui::EventType::kGestureShowPress, ui::EventType::kGestureTap, ui::EventType::kGestureEnd);
...
TEST_F(GestureRecognizerTest, ResetGestureRecognizerWithGestureProvider)
{
    ...
    const int kTouchId = 4;
    std::unique_ptr<GestureEventConsumeDelegate> delegate(new GestureEventConsumeDelegate());
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(delegate.get(), /*id=*/-2345, /*bounds=*/gfx::Rect(0, 0, 50, 50),
        /*parent=*/root_window()));
    ...
    EXPECT_EQ(1u, static_cast<ui::GestureRecognizerImpl*>(aura::Env::GetInstance()->gesture_recognizer())->consumers_.size());

    // Destroy the current gesture recognizer.
    aura::Env::GetInstance()->SetGestureRecognizer(std::make_unique<ui::GestureRecognizerImpl>());
}
