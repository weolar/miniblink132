...
void DefaultCaptureClient::SetCapture(Window* window)
{
    ...
    if (capture_window_) {
        ...
        DCHECK(!root_window_ || root_window_ == old_capture_window->GetRootWindow());
    ...
    observers_.Notify(&CaptureClientObserver::OnCaptureChanged, old_capture_window, capture_window_);
}
