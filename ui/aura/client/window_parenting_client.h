...
public:
    ...
    // immediately added.
    virtual Window* GetDefaultParent(Window* window, const gfx::Rect& bounds, const int64_t display_id) = 0;
};

// Set/Get a window tree client for the RootWindow containing |window|. |window|
// must not be NULL.
AURA_EXPORT void SetWindowParentingClient(Window* window, WindowParentingClient* window_tree_client);
...
AURA_EXPORT void ParentWindowWithContext(Window* window, Window* context, const gfx::Rect& screen_bounds, const int64_t display_id);
