...
public:
    ...
    virtual void ConvertPointToScreen(const Window* window, gfx::PointF* point) = 0;
    virtual void ConvertPointFromScreen(const Window* window, gfx::PointF* point) = 0;
    ...
    virtual void ConvertHostPointToScreen(Window* root_window, gfx::Point* point) = 0;
    ...
    virtual void SetBounds(Window* window, const gfx::Rect& bounds, const display::Display& display) = 0;
    ...
    // hieraichy.
    void ConvertPointToScreenIgnoringTransforms(const Window* window, gfx::Point* point);
    void ConvertPointToRootWindowIgnoringTransforms(const Window* window, gfx::Point* point);

protected:
    // Returns the origin of the host platform-window in system DIP coordinates.
    virtual gfx::Point GetRootWindowOriginInScreen(const aura::Window* root_window) = 0;
};

// Sets/Gets the activation client on the Window.
AURA_EXPORT void SetScreenPositionClient(Window* root_window, ScreenPositionClient* client);
AURA_EXPORT ScreenPositionClient* GetScreenPositionClient(const Window* root_window);
