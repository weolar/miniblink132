...
public:
    ...
    DeletionOrder order() const
    {
        return order_;
    }
    bool property_deleted() const
    {
        return property_deleted_;
    }
    bool layout_manager_deleted() const
    {
        return layout_manager_deleted_;
    }
...
public:
    using AnimationCallback = base::RepeatingCallback<void(const gfx::Transform&)>;

    LayerTranslationAnimationNotifier(ui::Layer* animation_layer, const gfx::Transform& initial_transform_, const gfx::Transform& target_transform,
        AnimationCallback animation_start_callback, AnimationCallback animation_end_callback, AnimationCallback animation_progress_callback)
    ...
    LayerTranslationAnimationNotifier(const LayerTranslationAnimationNotifier&) = delete;
    LayerTranslationAnimationNotifier& operator=(const LayerTranslationAnimationNotifier&) = delete;
...
public:
    explicit DeletionTestProperty(DeletionTracker* tracker)
        : tracker_(tracker)
    {
    }
    ...
    ~DeletionTestProperty()
    {
        tracker_->PropertyDeleted();
    }
...
DEFINE_OWNED_UI_CLASS_PROPERTY_KEY(DeletionTestProperty, kDeletionTestPropertyKey, nullptr)
...
public:
    ...
    void SetUp() override
    {
        ...
        max_separation_ = ui::GestureConfiguration::GetInstance()->max_separation_for_gesture_touches_in_pixels();
        ui::GestureConfiguration::GetInstance()->set_max_separation_for_gesture_touches_in_pixels(0);
    }

    void TearDown() override
    {
        AuraTestBase::TearDown();
        ui::GestureConfiguration::GetInstance()->set_max_separation_for_gesture_touches_in_pixels(max_separation_);
    }
...
public:
    ...
    DestroyTrackingDelegateImpl& operator=(const DestroyTrackingDelegateImpl&) = delete;

    void clear_destroying_count()
    {
        destroying_count_ = 0;
    }
    int destroying_count() const
    {
        return destroying_count_;
    }

    void clear_destroyed_count()
    {
        destroyed_count_ = 0;
    }
    int destroyed_count() const
    {
        return destroyed_count_;
    }

    bool in_destroying() const
    {
        return in_destroying_;
    }
...
public:
    explicit ChildWindowDelegateImpl(DestroyTrackingDelegateImpl* parent_delegate)
...
public:
    ...
    CaptureWindowDelegateImpl& operator=(const CaptureWindowDelegateImpl&) = delete;
    ...
    int capture_lost_count() const
    {
        return capture_lost_count_;
    }
    int mouse_event_count() const
    {
        return mouse_event_count_;
    }
    int touch_event_count() const
    {
        return touch_event_count_;
    }
    int gesture_event_count() const
    {
        return gesture_event_count_;
    }
...
public:
    ...
    GestureTrackPositionDelegate& operator=(const GestureTrackPositionDelegate&) = delete;
...
public:
    ...
    SelfEventHandlingWindowDelegate(const SelfEventHandlingWindowDelegate&) = delete;
    SelfEventHandlingWindowDelegate& operator=(const SelfEventHandlingWindowDelegate&) = delete;

    bool ShouldDescendIntoChildForEventHandling(Window* child, const gfx::Point& location) override
    {
        return false;
    }
};
...
{
    std::unique_ptr<Window> w(CreateTestWindow(SK_ColorWHITE, 1, gfx::Rect(10, 10, 5, 5), root_window()));
...
{
    std::unique_ptr<Window> w1(CreateTestWindow(SK_ColorWHITE, 1, gfx::Rect(10, 10, 500, 500), root_window()));
    std::unique_ptr<Window> w11(CreateTestWindow(SK_ColorGREEN, 11, gfx::Rect(5, 5, 100, 100), w1.get()));
    std::unique_ptr<Window> w111(CreateTestWindow(SK_ColorCYAN, 111, gfx::Rect(5, 5, 75, 75), w11.get()));
    std::unique_ptr<Window> w1111(CreateTestWindow(SK_ColorRED, 1111, gfx::Rect(5, 5, 50, 50), w111.get()));

    Window* root = root_window();
    root->MoveCursorTo(gfx::Point(10, 10));
    EXPECT_EQ("10,10", display::Screen::GetScreen()->GetCursorScreenPoint().ToString());
    w1->MoveCursorTo(gfx::Point(10, 10));
    EXPECT_EQ("20,20", display::Screen::GetScreen()->GetCursorScreenPoint().ToString());
    w11->MoveCursorTo(gfx::Point(10, 10));
    EXPECT_EQ("25,25", display::Screen::GetScreen()->GetCursorScreenPoint().ToString());
    w111->MoveCursorTo(gfx::Point(10, 10));
    EXPECT_EQ("30,30", display::Screen::GetScreen()->GetCursorScreenPoint().ToString());
    w1111->MoveCursorTo(gfx::Point(10, 10));
    EXPECT_EQ("35,35", display::Screen::GetScreen()->GetCursorScreenPoint().ToString());
}

TEST_F(WindowTest, ContainsMouse)
{
    std::unique_ptr<Window> w(CreateTestWindow(SK_ColorWHITE, 1, gfx::Rect(10, 10, 500, 500), root_window()));
    w->Show();
...
{
    std::unique_ptr<Window> window(CreateTestWindow(SK_ColorWHITE, 1, gfx::Rect(10, 10, 300, 200), root_window()));
...
TEST_F(WindowTest, MoveCursorToWithTransformRootWindow)
{
    ...
    transform = transform * OverlayTransformToTransform(gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90, gfx::SizeF());
...
TEST_F(WindowTest, MoveCursorToWithTransformWindow)
{
    ...
    w1->MoveCursorTo(gfx::Point(10, 10));
    EXPECT_EQ("10,40", display::Screen::GetScreen()->GetCursorScreenPoint().ToString());

    gfx::Transform transform3;
    transform3 = transform3 * OverlayTransformToTransform(gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90, gfx::SizeF());
    w1->SetTransform(transform3);
    w1->MoveCursorTo(gfx::Point(5, 5));
    EXPECT_EQ("5,15", display::Screen::GetScreen()->GetCursorScreenPoint().ToString());

    gfx::Transform transform4;
    transform4.Translate(100.0, 100.0);
    transform4 = transform4 * OverlayTransformToTransform(gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90, gfx::SizeF());
    transform4.Scale(2.0, 5.0);
    w1->SetTransform(transform4);
    w1->MoveCursorTo(gfx::Point(10, 10));
    EXPECT_EQ("60,130", display::Screen::GetScreen()->GetCursorScreenPoint().ToString());
}
...
TEST_F(WindowTest, MoveCursorToWithComplexTransform)
{
    ...
    root_transform = root_transform * OverlayTransformToTransform(gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_270, gfx::SizeF());
...
#endif
    EXPECT_EQ("20,53", display::Screen::GetScreen()->GetCursorScreenPoint().ToString());
}
...
TEST_F(WindowTest, NoCrashOnWindowDelete)
{
    ...
    std::unique_ptr<Window> window(CreateTestWindowWithDelegate(&delegate, 0, gfx::Rect(0, 0, 20, 20), root_window()));
}
...
TEST_F(WindowTest, GetEventHandlerForPoint)
{
    ...
    std::unique_ptr<Window> w111(CreateTestWindow(SK_ColorCYAN, 111, gfx::Rect(5, 5, 75, 75), w11.get()));
    std::unique_ptr<Window> w1111(CreateTestWindow(SK_ColorRED, 1111, gfx::Rect(5, 5, 50, 50), w111.get()));
    std::unique_ptr<Window> w12(CreateTestWindow(SK_ColorMAGENTA, 12, gfx::Rect(10, 420, 25, 25), w1.get()));
    std::unique_ptr<Window> w121(CreateTestWindow(SK_ColorYELLOW, 121, gfx::Rect(5, 5, 5, 5), w12.get()));
    std::unique_ptr<Window> w13(CreateTestWindow(SK_ColorGRAY, 13, gfx::Rect(5, 470, 50, 50), w1.get()));
...
TEST_F(WindowTest, GetEventHandlerForPointInCornerOfChildBounds)
{
    ...
    // window edges.
    std::unique_ptr<Window> parent(CreateTestWindow(SK_ColorWHITE, 1, gfx::Rect(10, 20, 400, 500), root_window()));
    std::unique_ptr<Window> child(CreateTestWindow(SK_ColorRED, 2, gfx::Rect(0, 0, 60, 70), parent.get()));
...
{
    std::unique_ptr<SelfEventHandlingWindowDelegate> parent_delegate(new SelfEventHandlingWindowDelegate);
    std::unique_ptr<Window> parent(CreateTestWindowWithDelegate(parent_delegate.get(), 1, gfx::Rect(10, 20, 400, 500), root_window()));
    std::unique_ptr<Window> child(CreateTestWindow(SK_ColorRED, 2, gfx::Rect(0, 0, 390, 480), parent.get()));
...
TEST_F(WindowTest, GetToplevelWindow)
{
    ...
    std::unique_ptr<Window> w1(CreateTestWindowWithId(1, root_window()));
    std::unique_ptr<Window> w11(CreateTestWindowWithDelegate(&delegate, 11, kBounds, w1.get()));
    std::unique_ptr<Window> w111(CreateTestWindowWithId(111, w11.get()));
    std::unique_ptr<Window> w1111(CreateTestWindowWithDelegate(&delegate, 1111, kBounds, w111.get()));
...
public:
    ...
    AddedToRootWindowObserver& operator=(const AddedToRootWindowObserver&) = delete;
...
TEST_F(WindowTest, WindowAddedToRootWindowShouldNotifyChildAndNotParent)
{
    ...
    std::unique_ptr<Window> parent_window(CreateTestWindowWithId(1, root_window()));
...
TEST_F(WindowTest, DestroyTest)
{
    ...
    {
        std::unique_ptr<Window> parent(CreateTestWindowWithDelegate(&parent_delegate, 0, gfx::Rect(), root_window()));
...
TEST_F(WindowTest, OrphanedBeforeOnDestroyed)
{
    ...
    {
        std::unique_ptr<Window> parent(CreateTestWindowWithDelegate(&parent_delegate, 0, gfx::Rect(), root_window()));
        std::unique_ptr<Window> child(CreateTestWindowWithDelegate(&child_delegate, 0, gfx::Rect(), parent.get()));
...
TEST_F(WindowTest, CaptureTests)
{
    ...
    ui::TouchEvent touchev(ui::EventType::kTouchPressed, gfx::Point(50, 50), getTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ...
    ui::TouchEvent touchev2(ui::EventType::kTouchPressed, gfx::Point(250, 250), getTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
...
TEST_F(WindowTest, TouchCaptureCancelsOtherTouches)
{
    ...
    std::unique_ptr<Window> w1(CreateTestWindowWithDelegate(&delegate1, 0, gfx::Rect(0, 0, 50, 50), root_window()));
    CaptureWindowDelegateImpl delegate2;
    std::unique_ptr<Window> w2(CreateTestWindowWithDelegate(&delegate2, 0, gfx::Rect(50, 50, 50, 50), root_window()));

    // Press on w1.
    ui::TouchEvent press1(ui::EventType::kTouchPressed, gfx::Point(10, 10), getTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ...
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(10, 20), getTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ...
    ui::TouchEvent release(ui::EventType::kTouchReleased, gfx::Point(10, 20), getTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&release);
    EXPECT_EQ(0, delegate1.gesture_event_count());
    EXPECT_EQ(0, delegate2.gesture_event_count());

    // A new press is captured by w2.
    ui::TouchEvent press2(ui::EventType::kTouchPressed, gfx::Point(10, 10), getTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
...
{
    CaptureWindowDelegateImpl delegate;
    std::unique_ptr<Window> window(CreateTestWindowWithDelegate(&delegate, 0, gfx::Rect(0, 0, 50, 50), root_window()));
    base::TimeTicks time = getTime();
    const int kTimeDelta = 100;

    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(10, 10), time, ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ...
    ui::TouchEvent move(ui::EventType::kTouchMoved, gfx::Point(10, 20), time, ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&move);
    EXPECT_EQ(1, delegate.touch_event_count());
    ...
    time += base::Milliseconds(kTimeDelta);
    ui::TouchEvent move2(ui::EventType::kTouchMoved, gfx::Point(10, 30), time, ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ...
    time += base::Milliseconds(kTimeDelta);
    ui::TouchEvent release(ui::EventType::kTouchReleased, gfx::Point(10, 20), time, ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&release);
    EXPECT_EQ(1, delegate.touch_event_count());
    EXPECT_EQ(2, delegate.gesture_event_count());
}
...
TEST_F(WindowTest, TransferCaptureTouchEvents)
{
    ...
    std::unique_ptr<Window> w1(CreateTestWindowWithDelegate(&d1, 0, gfx::Rect(0, 0, 20, 20), root_window()));
    ui::TouchEvent p1(ui::EventType::kTouchPressed, gfx::Point(10, 10), getTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ...
    std::unique_ptr<Window> w2(CreateTestWindowWithDelegate(&d2, 0, gfx::Rect(40, 0, 40, 20), root_window()));
    ui::TouchEvent p2(ui::EventType::kTouchPressed, gfx::Point(41, 10), getTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
    ...
    std::unique_ptr<Window> w3(CreateTestWindowWithDelegate(&d3, 0, gfx::Rect(0, 0, 100, 101), root_window()));
    ...
    ui::TouchEvent m3(ui::EventType::kTouchMoved, gfx::Point(110, 105), getTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
    ...
    ui::TouchEvent m4(ui::EventType::kTouchMoved, gfx::Point(120, 105), getTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
...
TEST_F(WindowTest, ChangeCaptureWhileMouseDown)
{
    ...
    std::unique_ptr<Window> window(CreateTestWindowWithDelegate(&delegate, 0, gfx::Rect(0, 0, 20, 20), root_window()));
    CaptureWindowDelegateImpl delegate2;
    std::unique_ptr<Window> w2(CreateTestWindowWithDelegate(&delegate2, 0, gfx::Rect(20, 20, 20, 20), root_window()));
...
{
    std::unique_ptr<Window> viewport(CreateTestWindowWithBounds(gfx::Rect(0, 0, 300, 300), root_window()));
    std::unique_ptr<Window> child(CreateTestWindowWithBounds(gfx::Rect(0, 0, 100, 100), viewport.get()));
    // Sanity check.
...
TEST_F(WindowTest, GetBoundsInRootWindowWithLayers)
{
    ...
    std::unique_ptr<Window> widget(CreateTestWindowWithBounds(gfx::Rect(0, 0, 200, 200), viewport.get()));

    std::unique_ptr<Window> child(CreateTestWindowWithBounds(gfx::Rect(0, 0, 100, 100), widget.get()));
...
public:
    ...
    MouseEnterExitWindowDelegate& operator=(const MouseEnterExitWindowDelegate&) = delete;
...
TEST_F(WindowTest, MouseEnterExit)
{
    ...
    std::unique_ptr<Window> w1(CreateTestWindowWithDelegate(&d1, 1, gfx::Rect(10, 10, 50, 50), root_window()));
    MouseEnterExitWindowDelegate d2;
    std::unique_ptr<Window> w2(CreateTestWindowWithDelegate(&d2, 2, gfx::Rect(70, 70, 50, 50), root_window()));
...
TEST_F(WindowTest, WindowTreeHostExit)
{
    ...
    ui::MouseEvent exit_event(ui::EventType::kMouseExited, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), 0, 0);
...
TEST_F(WindowTest, MouseEnterExitWhenDeleteWithCapture)
{
    ...
    std::unique_ptr<Window> window(CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(10, 10, 50, 50), root_window()));
...
TEST_F(WindowTest, MouseEnterExitWithWindowAppearAndDelete)
{
    ...
    MouseEnterExitWindowDelegate d2;
    {
        std::unique_ptr<Window> w2(CreateTestWindowWithDelegate(&d2, 2, gfx::Rect(10, 10, 50, 50), root_window()));
...
TEST_F(WindowTest, MouseEnterExitWithParentHide)
{
    ...
    std::unique_ptr<Window> w1(CreateTestWindowWithDelegate(&d1, 1, gfx::Rect(10, 10, 50, 50), root_window()));
    MouseEnterExitWindowDelegate d2;
    Window* w2 = CreateTestWindowWithDelegate(&d2, 2, gfx::Rect(10, 10, 50, 50), w1.get());
    ui::test::EventGenerator generator(root_window());
...
TEST_F(WindowTest, GetEventHandlerForPoint_NoDelegate)
{
    ...
    std::unique_ptr<Window> w1(CreateTestWindowWithDelegate(nullptr, 1, gfx::Rect(0, 0, 500, 500), root_window()));
    std::unique_ptr<Window> w11(CreateTestWindowWithDelegate(nullptr, 11, gfx::Rect(0, 0, 500, 500), w1.get()));
    std::unique_ptr<Window> w111(CreateTestWindowWithDelegate(&d111, 111, gfx::Rect(50, 50, 450, 450), w11.get()));
    std::unique_ptr<Window> w12(CreateTestWindowWithDelegate(nullptr, 12, gfx::Rect(0, 0, 500, 500), w1.get()));
...
TEST_F(WindowTest, Visibility)
{
    ...
    std::unique_ptr<Window> w1(CreateTestWindowWithDelegate(&d, 1, gfx::Rect(), root_window()));
    std::unique_ptr<Window> w2(CreateTestWindowWithDelegate(&d2, 2, gfx::Rect(), w1.get()));
...
TEST_F(WindowTest, EventTargetingPolicy)
{
    ...
    std::unique_ptr<Window> w1(CreateTestWindowWithDelegate(nullptr, 1, gfx::Rect(0, 0, 500, 500), root_window()));
    std::unique_ptr<Window> w11(CreateTestWindowWithDelegate(&d11, 11, gfx::Rect(0, 0, 500, 500), w1.get()));
    std::unique_ptr<Window> w111(CreateTestWindowWithDelegate(&d111, 111, gfx::Rect(50, 50, 450, 450), w11.get()));
    std::unique_ptr<Window> w12(CreateTestWindowWithDelegate(&d12, 12, gfx::Rect(0, 0, 500, 500), w1.get()));
    std::unique_ptr<Window> w121(CreateTestWindowWithDelegate(&d121, 121, gfx::Rect(150, 150, 50, 50), w12.get()));
...
TEST_F(WindowTest, ScopedEventTargetingBlockerTest)
{
    ...
    std::unique_ptr<Window> window(CreateTestWindowWithDelegate(nullptr, 1, gfx::Rect(0, 0, 500, 500), root_window()));
    EXPECT_EQ(window->event_targeting_policy(), EventTargetingPolicy::kTargetAndDescendants);
    auto event_targeting_blocker1 = std::make_unique<ScopedWindowEventTargetingBlocker>(window.get());
    EXPECT_EQ(window->event_targeting_policy(), EventTargetingPolicy::kNone);
    auto event_targeting_blocker2 = std::make_unique<ScopedWindowEventTargetingBlocker>(window.get());
    EXPECT_EQ(window->event_targeting_policy(), EventTargetingPolicy::kNone);
    ...
    // targeting policy.
    auto event_targeting_blocker3 = std::make_unique<ScopedWindowEventTargetingBlocker>(window.get());
    EXPECT_EQ(window->event_targeting_policy(), EventTargetingPolicy::kNone);
    window->SetEventTargetingPolicy(EventTargetingPolicy::kTargetOnly);
    EXPECT_EQ(window->event_targeting_policy(), EventTargetingPolicy::kNone);
    event_targeting_blocker3.reset();
    EXPECT_EQ(window->event_targeting_policy(), EventTargetingPolicy::kTargetOnly);
}
...
TEST_F(WindowTest, Transform)
{
    ...
    EXPECT_EQ(gfx::Rect(size), display::Screen::GetScreen()->GetDisplayNearestPoint(gfx::Point()).bounds());

    // Rotate it clock-wise 90 degrees.
    host()->SetRootTransform(OverlayTransformToTransform(gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90, gfx::SizeF(size)));

    // The size should be the transformed size.
    gfx::Size transformed_size(size.height(), size.width());
    EXPECT_EQ(transformed_size.ToString(), root_window()->bounds().size().ToString());
    EXPECT_EQ(gfx::Rect(transformed_size).ToString(), display::Screen::GetScreen()->GetDisplayNearestPoint(gfx::Point()).bounds().ToString());
...
TEST_F(WindowTest, TransformGesture)
{
    ...
    std::unique_ptr<GestureTrackPositionDelegate> delegate(new GestureTrackPositionDelegate);
    std::unique_ptr<Window> window(CreateTestWindowWithDelegate(delegate.get(), -1234, gfx::Rect(0, 0, 20, 20), root_window()));

    // Rotate the root-window clock-wise 90 degrees.
    host()->SetRootTransform(OverlayTransformToTransform(gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90, gfx::SizeF(size)));

    ui::TouchEvent press(ui::EventType::kTouchPressed, gfx::Point(size.height() - 10, 10), getTime(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&press);
...
public:
    ...
    DeletionTestLayoutManager& operator=(const DeletionTestLayoutManager&) = delete;

    ~DeletionTestLayoutManager() override
    {
        tracker_->LayoutManagerDeleted();
    }
...
private:
    ...
    void SetChildBounds(Window* child, const gfx::Rect& requested_bounds) override
    {
    }
...
TEST_F(WindowTest, DeleteLayoutManagerBeforeOwnedProps)
{
    DeletionTracker tracker;
    {
        ...
        w.SetProperty(kDeletionTestPropertyKey, std::make_unique<DeletionTestProperty>(&tracker));
    }
...
TEST_F(WindowTest, SetBoundsInternalShouldCheckTargetBounds)
{
    ...
    ui::ScopedAnimationDurationScaleMode test_duration_mode(ui::ScopedAnimationDurationScaleMode::NON_ZERO_DURATION);

    std::unique_ptr<Window> w1(CreateTestWindowWithBounds(gfx::Rect(0, 0, 100, 100), root_window()));
...
class WindowObserverTest : public WindowTest, public WindowObserver {
public:
    ...
    struct WindowBoundsInfo {
        ...
        gfx::Rect old_bounds;
        gfx::Rect new_bounds;
        ui::PropertyChangeReason reason = ui::PropertyChangeReason::NOT_FROM_ANIMATION;
    };
    ...
    const CountAndWindow& alpha_shape_info() const
    {
        return alpha_shape_info_;
    }

    const WindowTargetTransformChangingInfo& window_target_transform_changing_info() const
    {
    ...
    {
        std::string result(base::StringPrintf("added=%d removing=%d removed=%d", added_count_, removing_count_, removed_count_));
...
private:
    ...
    void OnWindowRemoved(Window* removed_window) override
    {
        removed_count_++;
    }
    ...
    void OnWindowPropertyChanged(Window* window, const void* key, intptr_t old) override
    {
        property_key_ = key;
        old_property_value_ = old;
    }

    void OnWindowBoundsChanged(Window* window, const gfx::Rect& old_bounds, const gfx::Rect& new_bounds, ui::PropertyChangeReason reason) override
    {
    ...
    void OnWindowOpacitySet(Window* window, ui::PropertyChangeReason reason) override
    {
    ...
    void OnWindowTargetTransformChanging(Window* window, const gfx::Transform& new_transform) override
    {
    ...
    void OnWindowTransformed(Window* window, ui::PropertyChangeReason reason) override
    {
...
TEST_F(WindowObserverTest, PropertyChanged)
{
    ...
    EXPECT_EQ(PropertyChangeInfo(native_prop_key, 0), PropertyChangeInfoAndClear());
    w1->SetNativeWindowProperty(native_prop_key, nullptr);
    EXPECT_EQ(PropertyChangeInfo(native_prop_key, reinterpret_cast<intptr_t>(&*w1)), PropertyChangeInfoAndClear());
...
TEST_F(WindowObserverTest, WindowBoundsChanged)
{
    ...
    EXPECT_EQ(ui::PropertyChangeReason::NOT_FROM_ANIMATION, window_bounds_info().reason);
}
...
TEST_F(WindowObserverTest, WindowBoundsChangedAnimation)
{
    ...
    const gfx::Rect initial_bounds = window->bounds();
    constexpr gfx::Rect kTargetBounds(10, 20, 30, 40);
    const gfx::Rect step_bounds = gfx::Tween::RectValueBetween(0.5, initial_bounds, kTargetBounds);

    ui::ScopedAnimationDurationScaleMode scoped_animation_duration_scale_mode(ui::ScopedAnimationDurationScaleMode::NORMAL_DURATION);
    ui::ScopedLayerAnimationSettings settings(window->layer()->GetAnimator());
    window->layer()->SetBounds(kTargetBounds);
    ASSERT_EQ(0, window_opacity_info().changed_count);

    window->layer()->GetAnimator()->Step(window->layer()->GetAnimator()->last_step_time() + settings.GetTransitionDuration() / 2);
    ASSERT_EQ(1, window_bounds_info().changed_count);
    EXPECT_EQ(window.get(), window_bounds_info().window);
    EXPECT_EQ(initial_bounds, window_bounds_info().old_bounds);
    EXPECT_EQ(step_bounds, window_bounds_info().new_bounds);
    EXPECT_EQ(ui::PropertyChangeReason::FROM_ANIMATION, window_bounds_info().reason);

    window->layer()->GetAnimator()->StopAnimatingProperty(ui::LayerAnimationElement::BOUNDS);
...
TEST_F(WindowObserverTest, WindowOpacityChanged)
{
    ...
    EXPECT_EQ(ui::PropertyChangeReason::NOT_FROM_ANIMATION, window_opacity_info().reason);
}
...
TEST_F(WindowObserverTest, WindowOpacityChangedAnimation)
{
    ...
    ASSERT_EQ(1, window_opacity_info().changed_count);
    EXPECT_EQ(window.get(), window_opacity_info().window);
    EXPECT_EQ(ui::PropertyChangeReason::FROM_ANIMATION, window_opacity_info().reason);

    window->layer()->GetAnimator()->StopAnimatingProperty(ui::LayerAnimationElement::OPACITY);
...
TEST_F(WindowObserverTest, SetTransform)
{
    ...
    EXPECT_EQ(target_transform, window_target_transform_changing_info().new_transform);

    ASSERT_EQ(1, window_transformed_info().changed_count);
    EXPECT_EQ(window.get(), window_transformed_info().window);
    EXPECT_EQ(ui::PropertyChangeReason::NOT_FROM_ANIMATION, window_transformed_info().reason);
}
...
TEST_F(WindowObserverTest, SetTransformAnimation)
{
    ...
    window->layer()->GetAnimator()->StopAnimatingProperty(ui::LayerAnimationElement::TRANSFORM);

    EXPECT_EQ(1, window_target_transform_changing_info().changed_count);

    ASSERT_EQ(1, window_transformed_info().changed_count);
    EXPECT_EQ(window.get(), window_transformed_info().window);
    EXPECT_EQ(ui::PropertyChangeReason::FROM_ANIMATION, window_transformed_info().reason);
}
...
TEST_F(WindowObserverTest, OnWindowLayerRecreatedWithTransformAnimation)
{
    ...
    target_transform.Skew(10.0, 5.0);
    window->SetTransform(target_transform);
    EXPECT_TRUE(window->layer()->GetAnimator()->IsAnimatingProperty(ui::LayerAnimationElement::TRANSFORM));
...
{
    std::unique_ptr<Window> w(CreateTestWindow(SK_ColorWHITE, 1, gfx::Rect(0, 0, 100, 100), root_window()));
    std::unique_ptr<ui::Layer> old_layer(w->RecreateLayer());

    const std::vector<raw_ptr<ui::Layer, VectorExperimental>>& child_layers = root_window()->layer()->children();
...
TEST_F(WindowTest, MouseEventsOnLeafWindowChange)
{
    ...
    std::unique_ptr<Window> w1(CreateTestWindowWithDelegate(&d1, 1, gfx::Rect(0, 0, 100, 100), root_window()));
    RunAllPendingInMessageLoop();
    ...
    std::unique_ptr<Window> w11(CreateTestWindowWithDelegate(&d11, 1, gfx::Rect(0, 0, 100, 100), w1.get()));
    RunAllPendingInMessageLoop();
    ...
    // Add |w11|.
    w11.reset(CreateTestWindowWithDelegate(&d11, 1, gfx::Rect(0, 0, 100, 100), w1.get()));
    RunAllPendingInMessageLoop();
...
TEST_F(WindowTest, MouseEventsOnNonLeafWindowDelete)
{
    ...
    std::unique_ptr<Window> w2(CreateTestWindowWithDelegate(&d2, 1, gfx::Rect(0, 0, 100, 100), w1.get()));
    RunAllPendingInMessageLoop();
    EXPECT_EQ("0 0 1", d1.GetMouseMotionCountsAndReset());
    ...
    CreateTestWindowWithDelegate(&d3, 1, gfx::Rect(0, 0, 100, 100), w2.get());
    RunAllPendingInMessageLoop();
    EXPECT_EQ("0 0 0", d1.GetMouseMotionCountsAndReset());
...
public:
    ...
    RootWindowAttachmentObserver& operator=(const RootWindowAttachmentObserver&) = delete;
    ...
    void OnWindowRemovingFromRootWindow(Window* window, Window* new_root) override
    {
        ++removed_count_;
    }
...
public:
    ...
    BoundsChangedWindowObserver& operator=(const BoundsChangedWindowObserver&) = delete;
...
public:
    ...
    explicit DeleteOnVisibilityChangedObserver(Window* to_observe, Window* to_delete)
    ...
    DeleteOnVisibilityChangedObserver(const DeleteOnVisibilityChangedObserver&) = delete;
    DeleteOnVisibilityChangedObserver& operator=(const DeleteOnVisibilityChangedObserver&) = delete;
...
TEST_F(WindowTest, DelegateNotifiedAsBoundsChange)
{
    ...
    std::unique_ptr<Window> window(CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(0, 0, 100, 100), root_window()));
...
public:
    ...
    AddChildNotificationsObserver& operator=(const AddChildNotificationsObserver&) = delete;

    std::string CountStringAndReset()
    {
        std::string result = base::NumberToString(added_count_) + " " + base::NumberToString(removed_count_);
...
{
    std::unique_ptr<Window> w1(CreateTestWindowWithDelegate(new DestroyWindowDelegate(), 0, gfx::Rect(10, 20, 30, 40), root_window()));
}
...
public:
    ...
    ~HierarchyObserver() override
    {
        target_->RemoveObserver(this);
    }

    void ValidateState(int index, const WindowObserver::HierarchyChangeParams& params) const
    {
...
private:
    ...
    void ParamsMatch(const WindowObserver::HierarchyChangeParams& p1, const WindowObserver::HierarchyChangeParams& p2) const
    {
...
public:
    ...
    TestLayerAnimationObserver& operator=(const TestLayerAnimationObserver&) = delete;
    ...
    bool animation_aborted() const
    {
        return animation_aborted_;
    }
...
{
    EXPECT_EQ(host()->compositor()->frame_sink_id(), root_window()->GetFrameSinkId());
...
TEST_F(WindowTest, LocalSurfaceIdChanges)
{
    ...
    std::unique_ptr<cc::LayerTreeFrameSink> frame_sink(window.CreateLayerTreeFrameSink());
...
TEST_F(WindowTest, CreateLayerTreeFrameSink)
{
    ...
    std::unique_ptr<cc::LayerTreeFrameSink> layer_tree_frame_sink = window.CreateLayerTreeFrameSink();
...
public:
    explicit HandleGestureEndDelegate(base::OnceCallback<void(Window*)> on_gesture_end)
...
TEST_F(WindowTest, CleanupGestureStateChangesWindowHierarchy)
{
    ...
    window2.Show();
    HandleGestureEndDelegate delegate(base::BindLambdaForTesting([&](Window* target_window) { window2.AddChild(target_window); }));
...
TEST_F(WindowTest, CleanupGestureStateDeleteOtherWindows)
{
    ...
    child1->Show();
    HandleGestureEndDelegate delegate(base::BindLambdaForTesting([&](Window* target_window) { child1.reset(); }));
    ...
    void OnTouchEvent(ui::TouchEvent* event) override
    {
        if (event->type() == ui::EventType::kTouchCancelled) {
            if (on_touch_cancel_callback_) {
                std::move(on_touch_cancel_callback_).Run(static_cast<Window*>(event->target()));
            }
        }
    }
...
TEST_F(WindowTest, DeleteWindowWhenCancellingTouch)
{
    ...
    window_delegate.SetOnTouchCancelCallback(base::BindLambdaForTesting([&window](Window* target) {
    ...
    gesture_recognizer->TransferEventsTo(&window1, &window2, ui::TransferTouchesBehavior::kDontCancel);
}

class WindowActualScreenBoundsTest : public WindowTest,
...
public:
    ...
    WindowActualScreenBoundsTest& operator=(const WindowActualScreenBoundsTest&) = delete;
    ...
    {
        WindowTest::SetUp();
        viewport_ = std::unique_ptr<Window>(CreateTestWindowWithBounds(gfx::Rect(/*x=*/100, /*y=*/50, /*width=*/200, /*height=*/200), root_window()));
        child_ = std::unique_ptr<Window>(CreateTestWindowWithBounds(gfx::Rect(/*x=*/0, /*y=*/0, /*width=*/100, /*height*/ 100), viewport_.get()));
    }
    ...
    void OnTranslationAnimationProgressed(const gfx::Rect& child_initial_bounds, const gfx::Transform& transform) const
    {
        EXPECT_EQ(transform.MapRect(child_initial_bounds), child_->GetActualBoundsInScreen());
    }
...
TEST_P(WindowActualScreenBoundsTest, VerifyWindowActualBoundsDuringAnimation)
{
    ...
    if (GetParam()) {
        ...
        viewport_->SetBounds(gfx::Rect(/*x=*/50, /*y=*/0, /*width=*/200, /*height=*/200));
        ...
        child_initial_bounds = gfx::Rect(/*x=*/50, /*y=*/0, /*width=*/100, /*height=*/100);
    } else {
        ...
        // Calculate the child's initial screen bounds manually.
        child_initial_bounds = gfx::Rect(/*x=*/100, /*y=*/50, /*width=*/100, /*height=*/100);
    }

    LayerTranslationAnimationNotifier bounds_checker(viewport_layer, initial_transform, target_transform,
        base::BindRepeating(&WindowActualScreenBoundsTest::OnTranslationAnimationStarted, base::Unretained(this)),
        base::BindRepeating(&WindowActualScreenBoundsTest::OnTranslationAnimationEnded, base::Unretained(this)),
        base::BindRepeating(&WindowActualScreenBoundsTest::OnTranslationAnimationProgressed, base::Unretained(this), child_initial_bounds));
