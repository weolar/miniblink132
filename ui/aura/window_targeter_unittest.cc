...
private:
    // aura::WindowTargeter:
    Window* FindTargetForLocatedEvent(Window* window, ui::LocatedEvent* event) override
    {
        return window_;
    }
...
TEST_F(WindowTargeterTest, Basic)
{
    ...
    std::unique_ptr<Window> window(CreateNormalWindow(1, root_window(), &delegate));
    ...
    ui::MouseEvent press(ui::EventType::kMousePressed, gfx::Point(20, 20), gfx::Point(20, 20), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
...
TEST_F(WindowTargeterTest, FindTargetInRootWindow)
{
    ...
    ui::MouseEvent mouse1(ui::EventType::kMousePressed, inside, inside, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ui::TouchEvent touch1(ui::EventType::kTouchPressed, inside, ui::EventTimeForNow(), ui::PointerDetails());
    ...
    ui::MouseEvent mouse2(ui::EventType::kMousePressed, outside, outside, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ui::TouchEvent touch2(ui::EventType::kTouchPressed, outside, ui::EventTimeForNow(), ui::PointerDetails());
    touch2.set_root_location(outside);
    EXPECT_EQ(nullptr, targeter.FindTargetInRootWindow(root_window(), mouse2));
    EXPECT_EQ(root_window(), targeter.FindTargetInRootWindow(root_window(), touch2));
}
...
TEST_F(WindowTargeterTest, ScopedWindowTargeter)
{
    ...
    {
        ui::MouseEvent mouse(ui::EventType::kMouseMoved, event_location, event_location, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ...
    std::unique_ptr<ScopedWindowTargeter> scoped_targeter(
        new ScopedWindowTargeter(window.get(), std::unique_ptr<WindowTargeter>(new StaticWindowTargeter(window.get()))));
    {
        ui::MouseEvent mouse(ui::EventType::kMouseMoved, event_location, event_location, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
...
TEST_F(WindowTargeterTest, ScopedWindowTargeterWindowDestroyed)
{
    ...
    std::unique_ptr<Window> window(CreateNormalWindow(1, root_window(), &delegate));
    std::unique_ptr<ScopedWindowTargeter> scoped_targeter(
        new ScopedWindowTargeter(window.get(), std::unique_ptr<aura::WindowTargeter>(new StaticWindowTargeter(window.get()))));
...
TEST_F(WindowTargeterTest, TargetTransformedWindow)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<Window> window(CreateNormalWindow(2, root_window(), &delegate));
    ...
    EXPECT_EQ(gfx::RectF(100, 20, 200, 40).ToString(), GetEffectiveVisibleBoundsInRootWindow(window.get()).ToString());
    {
        ui::MouseEvent mouse(ui::EventType::kMouseMoved, event_location, event_location, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ...
    transform.Scale(0.5, 0.5);
    window->SetTransform(transform);
    EXPECT_EQ(gfx::RectF(300, 30, 200, 40).ToString(), GetEffectiveVisibleBoundsInRootWindow(window.get()).ToString());
    {
        ui::MouseEvent mouse(ui::EventType::kMouseMoved, event_location, event_location, ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
        EXPECT_EQ(window.get(), targeter->FindTargetForEvent(root_target, &mouse));
    }
...
public:
    explicit IdCheckingEventTargeter(int id)
        : id_(id)
    {
    }
...
protected:
    // WindowTargeter:
    bool SubtreeShouldBeExploredForEvent(Window* window, const ui::LocatedEvent& event) override
    {
        return (window->GetId() == id_ && WindowTargeter::SubtreeShouldBeExploredForEvent(window, event));
    }
...
{
    test::TestWindowDelegate delegate;
    std::unique_ptr<Window> parent(CreateNormalWindow(1, root_window(), &delegate));
    std::unique_ptr<Window> child(CreateNormalWindow(1, parent.get(), &delegate));
    std::unique_ptr<Window> grandchild(CreateNormalWindow(1, child.get(), &delegate));
    ...
    ui::MouseEvent mouse(ui::EventType::kMouseMoved, gfx::Point(1, 1), gfx::Point(1, 1), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ...
    // by |parent|.
    ui::MouseEvent mouse2(ui::EventType::kMouseMoved, gfx::Point(8, 8), gfx::Point(8, 8), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    std::unique_ptr<aura::WindowTargeter> original_targeter = child_r->SetEventTargeter(std::make_unique<IdCheckingEventTargeter>(2));
    ...
    ui::MouseEvent mouse3(ui::EventType::kMouseMoved, gfx::Point(8, 8), gfx::Point(8, 8), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ...
    ui::MouseEvent second(ui::EventType::kMouseMoved, gfx::Point(12, 12), gfx::Point(12, 12), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    EXPECT_EQ(grandchild_r, targeter->FindTargetForEvent(root_target, &second));

    // Target |child| location.
    ui::MouseEvent third(ui::EventType::kMouseMoved, gfx::Point(8, 8), gfx::Point(8, 8), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
...
TEST_F(WindowTargeterTest, NonFullyContainedBounds)
{
    ...
    auto* targeter = static_cast<ui::EventTarget*>(root_window())->GetEventTargeter();

    ui::MouseEvent mouse(ui::EventType::kMouseMoved, gfx::Point(17, 75), gfx::Point(17, 75), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
...
TEST_F(WindowTargeterTest, TargeterChecksOwningEventTarget)
{
    ...
    ui::MouseEvent mouse(ui::EventType::kMouseMoved, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    ...
    ui::MouseEvent mouse2(ui::EventType::kMouseMoved, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
