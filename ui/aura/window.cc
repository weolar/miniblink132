...
DEFINE_ENUM_CONVERTERS(aura::client::WindowType, { aura::client::WINDOW_TYPE_UNKNOWN, u"WINDOW_TYPE_UNKNOWN" },
    { aura::client::WINDOW_TYPE_NORMAL, u"WINDOW_TYPE_NORMAL" }, { aura::client::WINDOW_TYPE_POPUP, u"WINDOW_TYPE_POPUP" },
    { aura::client::WINDOW_TYPE_CONTROL, u"WINDOW_TYPE_CONTROL" }, { aura::client::WINDOW_TYPE_MENU, u"WINDOW_TYPE_MENU" },
    { aura::client::WINDOW_TYPE_TOOLTIP, u"WINDOW_TYPE_TOOLTIP" })

DEFINE_ENUM_CONVERTERS(aura::Window::OcclusionState, { aura::Window::OcclusionState::UNKNOWN, u"UNKNOWN" },
    { aura::Window::OcclusionState::VISIBLE, u"VISIBLE" }, { aura::Window::OcclusionState::OCCLUDED, u"OCCLUDED" },
    { aura::Window::OcclusionState::HIDDEN, u"HIDDEN" })

namespace ui {
namespace metadata {

template <> struct TypeConverter<viz::SurfaceId> : public BaseTypeConverter<true> {
    static std::u16string ToString(const viz::SurfaceId& source_value);
    static std::optional<viz::SurfaceId> FromString(const std::u16string& source_value);
    static ValidStrings GetValidStrings();
};

// static
std::u16string TypeConverter<viz::SurfaceId>::ToString(const viz::SurfaceId& source_value)
{
    return source_value.is_valid() ? base::UTF8ToUTF16(source_value.ToString()) : u"<none>";
}

// static
std::optional<viz::SurfaceId> TypeConverter<viz::SurfaceId>::FromString(const std::u16string& source_value)
{
    return std::nullopt;
}
...
public:
        ...
        , hid_cursor_(false)
    {
        if (!window_->IsRootWindow())
            return;
        const bool cursor_is_in_bounds = window_->GetBoundsInScreen().Contains(Env::GetInstance()->last_mouse_location());
...
Window::Window(WindowDelegate* delegate, client::WindowType type)
    : type_(type)
    , delegate_(delegate)
    , event_targeting_policy_(aura::EventTargetingPolicy::kTargetAndDescendants)
    , restore_event_targeting_policy_(aura::EventTargetingPolicy::kTargetAndDescendants)
    ,
...
{
    return const_cast<Window*>(static_cast<const Window*>(this)->GetRootWindow());
}
...
std::unique_ptr<WindowTargeter> Window::SetEventTargeter(std::unique_ptr<WindowTargeter> targeter)
{
...
void Window::SetBoundsInScreen(const gfx::Rect& new_bounds_in_screen, const display::Display& dst_display)
{
    if (auto* screen_position_client = aura::client::GetScreenPositionClient(GetRootWindow())) {
...
// static
void Window::ConvertPointToTarget(const Window* source, const Window* target, gfx::PointF* point)
{
    if (!source)
        return;
...
// static
void Window::ConvertPointToTarget(const Window* source, const Window* target, gfx::Point* point)
{
...
// static
void Window::ConvertRectToTarget(const Window* source, const Window* target, gfx::Rect* rect)
{
    DCHECK(rect);
...
// static
void Window::ConvertNativePointToTargetHost(const Window* source, const Window* target, gfx::PointF* point)
{
    if (!source || !target)
        return;
    ...
    point->Offset(-target->GetHost()->GetBoundsInPixels().x(), -target->GetHost()->GetBoundsInPixels().y());
}

// static
void Window::ConvertNativePointToTargetHost(const Window* source, const Window* target, gfx::Point* point)
{
    gfx::PointF point_float(*point);
...
void Window::SetEventTargetingPolicy(EventTargetingPolicy policy)
{
    ...
    const bool old_window_accepts_events
        = (event_targeting_policy_ == EventTargetingPolicy::kTargetOnly) || (event_targeting_policy_ == EventTargetingPolicy::kTargetAndDescendants);
    const bool new_window_accepts_events = (policy == EventTargetingPolicy::kTargetOnly) || (policy == EventTargetingPolicy::kTargetAndDescendants);
...
Window* Window::GetToplevelWindow()
{
    ...
    for (aura::Window* window = this; window != nullptr; window = window->parent()) {
        if (window->delegate())
...
std::unique_ptr<ScopedKeyboardHook> Window::CaptureSystemKeyEvents(std::optional<base::flat_set<ui::DomCode>> dom_codes)
{
    Window* root_window = GetRootWindow();
    if (!root_window)
        return nullptr;
...
void Window::OnDeviceScaleFactorChanged(float old_device_scale_factor, float new_device_scale_factor)
{
    if (!IsRootWindow() && last_device_scale_factor_ != new_device_scale_factor && IsEmbeddingExternalContent()) {
    ...
    if (delegate_) {
        delegate_->OnDeviceScaleFactorChanged(old_device_scale_factor, new_device_scale_factor);
    }
}
...
void Window::GetDebugInfo(
    const aura::Window* active_window, const aura::Window* focused_window, const aura::Window* capture_window, std::ostringstream* out) const
{
    std::string name(GetName());
    if (name.empty())
        name = "\"\"";
    const gfx::Vector2dF& subpixel_position_offset = layer()->GetSubpixelOffset();
    bool can_occlude_others = aura::Env::GetInstance()->GetWindowOcclusionTracker()->VisibleWindowCanOccludeOtherWindows(this);
    bool has_opaque_regions = !opaque_regions_for_occlusion().empty();
    *out << " " << name << "<" << GetId() << ">";
    *out << " (" << this << ")"
         << " type=" << aura::Window::WindowTypeToString(GetType());
    *out << ((this == active_window) ? " [active]" : "") << ((this == focused_window) ? " [focused]" : "") << ((this == capture_window) ? " [capture]" : "")
         << (GetTransparent() ? " [transparent]" : "") << (IsVisible() ? " [visible]" : "") << (has_opaque_regions ? " [opaque_regions]" : "")
         << (can_occlude_others ? " [occlude others]" : "")
         << (GetOcclusionState() != aura::Window::OcclusionState::UNKNOWN
                    ? (" " + base::UTF16ToUTF8(aura::Window::OcclusionStateToString(GetOcclusionState()))).c_str()
                    : "")
         << " " << bounds().ToString() << " scale=" + transform().To2dScale().ToString();
...
bool Window::HitTest(const gfx::Point& local_point)
{
    ...
    clip_region.setRect({ local_bounds.x(), local_bounds.y(), local_bounds.width(), local_bounds.height() });
...
void Window::SetBoundsInternal(const gfx::Rect& new_bounds)
{
    ...
    if (layer()->delegate() != this) {
        OnLayerBoundsChanged(old_bounds, ui::PropertyChangeReason::NOT_FROM_ANIMATION);
    }
}
...
void Window::SetOcclusionInfo(OcclusionState occlusion_state, const SkRegion& occluded_region)
{
    if (occlusion_state == occlusion_state_ && occluded_region_in_root_ == occluded_region) {
        return;
    }
...
void Window::StackChildRelativeTo(Window* child, Window* target, StackDirection direction)
{
    ...
    const size_t target_i = base::ranges::find(children_, target) - children_.begin();

    DCHECK_LT(child_i, children_.size()) << "Child was not in list of children!";
    DCHECK_LT(target_i, children_.size()) << "Target was not in list of children!";
...
void Window::StackChildLayerRelativeTo(Window* child, Window* target, StackDirection direction)
{
...
void Window::NotifyRemovingFromRootWindow(Window* new_root)
{
    ...
    for (Window::Windows::const_iterator it = children_.begin(); it != children_.end(); ++it) {
...
void Window::NotifyWindowHierarchyChange(const WindowObserver::HierarchyChangeParams& params)
{
...
void Window::NotifyWindowHierarchyChangeDown(const WindowObserver::HierarchyChangeParams& params)
{
...
void Window::NotifyWindowHierarchyChangeUp(const WindowObserver::HierarchyChangeParams& params)
{
    for (Window* window = this; window; window = window->parent())
        window->NotifyWindowHierarchyChangeAtReceiver(params);
}

void Window::NotifyWindowHierarchyChangeAtReceiver(const WindowObserver::HierarchyChangeParams& params)
{
...
void Window::NotifyWindowVisibilityChanged(aura::Window* target, bool visible)
{
...
bool Window::NotifyWindowVisibilityChangedAtReceiver(aura::Window* target, bool visible)
{
...
bool Window::NotifyWindowVisibilityChangedDown(aura::Window* target, bool visible)
{
...
void Window::NotifyWindowVisibilityChangedUp(aura::Window* target, bool visible)
{
...
std::unique_ptr<cc::LayerTreeFrameSink> Window::CreateLayerTreeFrameSink()
{
    ...
    if (!frame_sink_id_.is_valid()) {
        ...
        host_frame_sink_manager->RegisterFrameSinkId(frame_sink_id, this, viz::ReportFirstSurfaceActivation::kYes);
    ...
    mojo::PendingReceiver<viz::mojom::CompositorFrameSink> sink_receiver = sink_remote.InitWithNewPipeAndPassReceiver();
    mojo::PendingRemote<viz::mojom::CompositorFrameSinkClient> client_remote;
    mojo::PendingReceiver<viz::mojom::CompositorFrameSinkClient> client_receiver = client_remote.InitWithNewPipeAndPassReceiver();
    host_frame_sink_manager->CreateCompositorFrameSink(frame_sink_id_, std::move(sink_receiver), std::move(client_remote));

    cc::mojo_embedder::AsyncLayerTreeFrameSink::InitParams params;
    params.gpu_memory_buffer_manager = Env::GetInstance()->context_factory()->GetGpuMemoryBufferManager();
...
viz::ScopedSurfaceIdAllocator Window::GetSurfaceIdAllocator(base::OnceCallback<void()> allocation_task)
{
    return viz::ScopedSurfaceIdAllocator(parent_local_surface_id_allocator_.get(), std::move(allocation_task));
}
...
void Window::InvalidateLocalSurfaceId(bool also_invalidate_allocation_group)
{
    if (!parent_local_surface_id_allocator_)
        return;
    parent_local_surface_id_allocator_->Invalidate(also_invalidate_allocation_group);
}

void Window::UpdateLocalSurfaceIdFromEmbeddedClient(const std::optional<viz::LocalSurfaceId>& embedded_client_local_surface_id)
{
    if (embedded_client_local_surface_id) {
        parent_local_surface_id_allocator_->UpdateFromChild(*embedded_client_local_surface_id);
        UpdateLocalSurfaceId();
    } else {
        AllocateLocalSurfaceId();
    }
...
void Window::SetOpaqueRegionsForOcclusion(const std::vector<gfx::Rect>& opaque_regions_for_occlusion)
{
    ...
    DCHECK(GetTransparent() || layer()->type() == ui::LAYER_NOT_DRAWN || opaque_regions_for_occlusion.empty()
        || (opaque_regions_for_occlusion.size() == 1 && opaque_regions_for_occlusion[0] == gfx::Rect(bounds().size())));
...
void Window::OnLayerBoundsChanged(const gfx::Rect& old_bounds, ui::PropertyChangeReason reason)
{
    WindowOcclusionTracker::ScopedPause pause_occlusion_tracking;

    bounds_ = layer()->bounds();

    if (!IsRootWindow() && old_bounds.size() != bounds_.size() && IsEmbeddingExternalContent()) {
        parent_local_surface_id_allocator_->GenerateId();
        if (frame_sink_) {
            frame_sink_->SetLocalSurfaceId(GetCurrentLocalSurfaceId());
        }
...
void Window::OnLayerFillsBoundsOpaquelyChanged(ui::PropertyChangeReason reason)
{
...
void Window::OnLayerTransformed(const gfx::Transform& old_transform, ui::PropertyChangeReason reason)
{
    WindowOcclusionTracker::ScopedPause pause_occlusion_tracking;
    for (WindowObserver& observer : observers_)
...
void Window::ConvertEventToTarget(const ui::EventTarget* target, ui::LocatedEvent* event) const
{
...
std::unique_ptr<ui::Layer> Window::RecreateLayer()
{
    ...
    const bool was_animating_opacity = animator->IsAnimatingProperty(ui::LayerAnimationElement::OPACITY);
    const bool was_animating_transform = animator->IsAnimatingProperty(ui::LayerAnimationElement::TRANSFORM);
    ...
    if (was_animating_opacity) {
        for (WindowObserver& observer : observers_) {
            observer.OnWindowOpacitySet(this, ui::PropertyChangeReason::FROM_ANIMATION);
        }
    }
    if (was_animating_transform) {
        for (WindowObserver& observer : observers_) {
            observer.OnWindowTransformed(this, ui::PropertyChangeReason::FROM_ANIMATION);
        }
    }
...
void Window::OnFirstSurfaceActivation(const viz::SurfaceInfo& surface_info)
{
    ...
    layer()->SetShowSurface(
        surface_info.id(), bounds().size(), SK_ColorWHITE, cc::DeadlinePolicy::UseDefaultDeadline(), false /* stretch_content_to_fill_bounds */);
}

void Window::OnFrameTokenChanged(uint32_t frame_token, base::TimeTicks activation_time)
{
}
...
void Window::OnScopedWindowCaptureRequestAdded()
{
    if (++number_of_capture_requests_ == 1) {
        ...
        subtree_capture_id_ = Env::GetInstance()->context_factory()->AllocateSubtreeCaptureId();
        if (layer())
            layer()->SetSubtreeCaptureId(subtree_capture_id_);
    }
...
void Window::SetLayoutManagerImpl(std::unique_ptr<LayoutManager> layout_manager)
{
    ...
    for (Windows::const_iterator it = children_.begin(); it != children_.end(); ++it)
