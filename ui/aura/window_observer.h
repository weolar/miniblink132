...
public:
    struct HierarchyChangeParams {
        enum HierarchyChangePhase { HIERARCHY_CHANGING, HIERARCHY_CHANGED };

        raw_ptr<Window, DanglingUntriaged> target; // The window that was added or removed.
        raw_ptr<Window, DanglingUntriaged> new_parent;
        raw_ptr<Window, DanglingUntriaged> old_parent;
        HierarchyChangePhase phase;
        raw_ptr<Window, DanglingUntriaged> receiver; // The window receiving the notification.
    };
    ...
    virtual void OnWindowPropertyChanged(Window* window, const void* key, intptr_t old)
    {
    }
    ...
    virtual void OnWindowBoundsChanged(Window* window, const gfx::Rect& old_bounds, const gfx::Rect& new_bounds, ui::PropertyChangeReason reason)
    {
    }

    // Invoked before Window::SetTransform() sets the transform of a window.
    virtual void OnWindowTargetTransformChanging(Window* window, const gfx::Transform& new_transform)
    {
    }
    ...
    virtual void OnWindowTransformed(Window* window, ui::PropertyChangeReason reason)
    {
    }
    ...
    virtual void OnWindowOpacitySet(Window* window, ui::PropertyChangeReason reason)
    {
    }
    ...
    virtual void OnWindowTransparentChanged(Window* window, ui::PropertyChangeReason reason)
    {
    }
    ...
    // atomically.
    virtual void OnWindowRemovingFromRootWindow(Window* window, Window* new_root)
    {
    }
