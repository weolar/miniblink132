...
class AURA_EXPORT WindowOcclusionTracker : public ui::LayerAnimationObserver, public ui::LayerObserver, public WindowObserver, public WindowTreeHostObserver {
public:
    ...
    public:
        explicit ScopedExclude(Window* window, WindowOcclusionTracker* occlusion_tracker = nullptr);
    ...
    class AURA_EXPORT ScopedForceVisible : public WindowObserver, public InnerClient {
    public:
        explicit ScopedForceVisible(Window* window, WindowOcclusionTracker* occlusion_tracker = nullptr);
    ...
    bool HasIgnoredAnimatingWindows() const
    {
        return !animated_windows_.empty();
    }

    // Set the factory to create WindowOcclusionChangeBuilder.
    using OcclusionChangeBuilderFactory = base::RepeatingCallback<std::unique_ptr<WindowOcclusionChangeBuilder>()>;
    void set_occlusion_change_builder_factory(OcclusionChangeBuilderFactory factory)
    {
    ...
    bool IsPaused() const
    {
        return num_pause_occlusion_tracking_;
    }

private:
    friend class test::WindowOcclusionTrackerTestApi;
    friend class Env;
    friend void Window::GetDebugInfo(
    ...
    static bool OcclusionStatesMatch(const base::flat_map<Window*, OcclusionData>& tracked_windows);
    ...
    void SetWindowAndDescendantsAreOccluded(Window* window, bool is_occluded, bool is_parent_visible);
    ...
    // |tracked_windows_|.
    void SetOccluded(Window* window, bool is_occluded, bool is_parent_visible, const SkRegion& occluded_region);
    ...
    // this is called.
    template <typename Predicate> void MarkRootWindowAsDirtyAndMaybeComputeOcclusionIf(Window* window, Predicate predicate);
    ...
    // of |window| is 1.0f.
    bool WindowOrDescendantCanOccludeOtherWindows(Window* window, bool assume_parent_opaque = false, bool assume_window_opaque = false) const;
    ...
    void OnWindowTransparentChanged(Window* window, ui::PropertyChangeReason reason) override;
    ...
    base::ScopedMultiSourceObservation<Window, WindowObserver> window_observations_ { this };
    base::ScopedMultiSourceObservation<WindowTreeHost, WindowTreeHostObserver> window_tree_host_observations_ { this };
    base::ScopedMultiSourceObservation<ui::LayerAnimator, ui::LayerAnimationObserver> layer_animator_observations { this };
    base::ScopedMultiSourceObservation<ui::Layer, ui::LayerObserver> animated_layer_observations_ { this };
