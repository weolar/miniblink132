...
class DefaultWindowOcclusionChangeBuilder : public WindowOcclusionChangeBuilder {
public:
    DefaultWindowOcclusionChangeBuilder() = default;

    DefaultWindowOcclusionChangeBuilder(const DefaultWindowOcclusionChangeBuilder&) = delete;
    DefaultWindowOcclusionChangeBuilder& operator=(const DefaultWindowOcclusionChangeBuilder&) = delete;
    ...
    ~DefaultWindowOcclusionChangeBuilder() override
    {
        ...
        while (!windows_.windows().empty()) {
            ...
            if (it == changes_.end())
                continue;
            window->SetOcclusionInfo(it->second.occlusion_state, it->second.occluded_region);
        }
        changes_.clear();
    }
...
// static
std::unique_ptr<WindowOcclusionChangeBuilder> WindowOcclusionChangeBuilder::Create()
{
