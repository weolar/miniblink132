...
public:
    ...
    Window* mouse_pressed_handler()
    {
        return mouse_pressed_handler_;
    }
    Window* mouse_moved_handler()
    {
        return mouse_moved_handler_;
    }
    Window* touchpad_pinch_handler()
    {
        return touchpad_pinch_handler_;
    }

    WindowTargeter* event_targeter()
    {
    ...
    [[nodiscard]] ui::EventDispatchDetails DispatchMouseExitAtPoint(Window* target, const gfx::Point& point, int event_flags = ui::EF_NONE);
    ...
    // space, in DIPs.
    virtual void ProcessedTouchEvent(uint32_t unique_event_id, Window* window, ui::EventResult result, bool is_source_touch_event_set_blocking);
    ...
    void set_skip_ime(bool skip_ime)
    {
        skip_ime_ = skip_ime;
    }
    bool should_skip_ime() const
    {
        return skip_ime_;
    }

private:
    FRIEND_TEST_ALL_PREFIXES(WindowEventDispatcherTest, KeepTranslatedEventInRoot);
    ...
    [[nodiscard]] ui::EventDispatchDetails DispatchMouseEnterOrExit(Window* target, const ui::MouseEvent& event, ui::EventType type);
    [[nodiscard]] ui::EventDispatchDetails ProcessGestures(Window* target, ui::GestureRecognizer::Gestures gestures);
    ...
    ui::EventDispatchDetails PreDispatchEvent(ui::EventTarget* target, ui::Event* event) override;
    ui::EventDispatchDetails PostDispatchEvent(ui::EventTarget* target, const ui::Event& event) override;

    // Overridden from ui::GestureEventHelper.
    bool CanDispatchToConsumer(ui::GestureConsumer* consumer) override;
    void DispatchGestureEvent(ui::GestureConsumer* raw_input_consumer, ui::GestureEvent* event) override;
    ...
    ui::EventDispatchDetails PreDispatchLocatedEvent(Window* target, ui::LocatedEvent* event);
    ui::EventDispatchDetails PreDispatchMouseEvent(Window* target, ui::MouseEvent* event);
    ui::EventDispatchDetails PreDispatchPinchEvent(Window* target, ui::GestureEvent* event);
    ui::EventDispatchDetails PreDispatchTouchEvent(Window* target, ui::TouchEvent* event);
    ui::EventDispatchDetails PreDispatchKeyEvent(Window* target, ui::KeyEvent* event);

    std::unique_ptr<cc::EventsMetricsManager::ScopedMonitor> CreateScropedMetricsMonitorForEvent(const ui::Event& event);
    ...
    raw_ptr<ui::LocatedEvent, DanglingUntriaged> dispatching_held_event_ = nullptr;
    ...
    // previous one.
    std::unique_ptr<cc::EventsMetricsManager::ScopedMonitor> event_metrics_monitor_;
