...
SkRegion AdjustForClientAndConvertToDips(const SkRegion& region_pixels, float scale_factor, const gfx::Rect& window_tree_host_bounds_pixels)
{
    ...
    region.op(gfx::RectToSkIRect(window_tree_host_bounds_pixels), SkRegion::kIntersect_Op);
    region.translate(window_tree_host_bounds_pixels.x(), window_tree_host_bounds_pixels.y());
    ...
    while (!iter.done()) {
        ...
        rects.push_back(gfx::RectToSkIRect(gfx::ScaleToEnclosedRect(rect_pixels, 1.0f / scale_factor, 1.0f / scale_factor)));
        iter.next();
    }
    SkRegion result;
...
NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator* NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::instance_ = nullptr;

NativeWindowOcclusionTrackerWin* NativeWindowOcclusionTrackerWin::GetOrCreateInstance()
{
    if (!g_tracker)
...
void NativeWindowOcclusionTrackerWin::Enable(Window* window)
{
    ...
    update_occlusion_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &WindowOcclusionCalculator::EnableOcclusionTrackingForWindow, base::Unretained(WindowOcclusionCalculator::GetInstance()), root_window_hwnd));
}
...
void NativeWindowOcclusionTrackerWin::Disable(Window* window)
{
    ...
    update_occlusion_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            &WindowOcclusionCalculator::DisableOcclusionTrackingForWindow, base::Unretained(WindowOcclusionCalculator::GetInstance()), root_window_hwnd));
}

void NativeWindowOcclusionTrackerWin::OnWindowVisibilityChanged(Window* window, bool visible)
{
    if (!window->IsRootWindow())
        return;
    window->GetHost()->SetNativeWindowOcclusionState(visible ? Window::OcclusionState::UNKNOWN : Window::OcclusionState::HIDDEN, {});
    update_occlusion_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&WindowOcclusionCalculator::HandleVisibilityChanged, base::Unretained(WindowOcclusionCalculator::GetInstance()), visible));
}
...
NativeWindowOcclusionTrackerWin::NativeWindowOcclusionTrackerWin()
    ...
    // EventHookCallback.
    update_occlusion_task_runner_(base::ThreadPool::CreateCOMSTATaskRunner({ base::MayBlock(),
    ...
    , session_change_observer_(base::BindRepeating(&NativeWindowOcclusionTrackerWin::OnSessionChange, base::Unretained(this)))
    , power_setting_change_listener_(this)
{
    WindowOcclusionCalculator::CreateInstance(update_occlusion_task_runner_, base::SequencedTaskRunner::GetCurrentDefault(),
        base::BindRepeating(&NativeWindowOcclusionTrackerWin::UpdateOcclusionState, weak_factory_.GetWeakPtr()));
}
...
NativeWindowOcclusionTrackerWin::~NativeWindowOcclusionTrackerWin()
{
    ...
    DCHECK(hwnd_root_window_map_.empty()) << "Occlusion tracker torn down while a Window still exists";
    ...
    update_occlusion_task_runner_->PostTask(FROM_HERE, base::BindOnce(&WindowOcclusionCalculator::DeleteInstanceForTesting, &done_event));
    done_event.Wait();
}

// static
bool NativeWindowOcclusionTrackerWin::IsWindowVisibleAndFullyOpaque(HWND hwnd, gfx::Rect* window_rect)
{
    ...
    if (::GetWindowLong(hwnd, GWL_STYLE) & WS_POPUP) {
        ...
        if (!hwnd_class_name.starts_with(L"Chrome_WidgetWin_") && hwnd_class_name != L"Shell_TrayWnd") {
            return false;
        }
    }
...
void NativeWindowOcclusionTrackerWin::UpdateOcclusionState(const HwndToRootOcclusionStateMap& root_window_hwnds_occlusion_state, bool show_all_windows)
{
    ...
    for (const auto& root_window_pair : root_window_hwnds_occlusion_state) {
        ...
        if (it == hwnd_root_window_map_.end())
            continue;
        it->second->GetHost()->set_on_current_workspace(root_window_pair.second.on_current_workspace);
        ...
        if (!it->second->IsVisible()) {
            it->second->GetHost()->SetNativeWindowOcclusionState(Window::OcclusionState::HIDDEN, {});
            continue;
        }
        ...
        if (screen_locked_ || !display_on_) {
            ...
            occluded_region = AdjustForClientAndConvertToDips(root_window_pair.second.occluded_region_pixels, it->second->GetHost()->device_scale_factor(),
                it->second->GetHost()->GetBoundsInAcceleratedWidgetPixelCoordinates());
        }

        it->second->GetHost()->SetNativeWindowOcclusionState(occl_state, occluded_region);
        num_visible_root_windows_++;
    }
}

void NativeWindowOcclusionTrackerWin::OnSessionChange(WPARAM status_code, const bool* is_current_session)
{
    ...
    if (status_code == WTS_SESSION_UNLOCK) {
        ...
        update_occlusion_task_runner_->PostTask(
            FROM_HERE, base::BindOnce(&WindowOcclusionCalculator::ForceRecalculation, base::Unretained(WindowOcclusionCalculator::GetInstance())));
...
{
    static bool screen_power_listener_enabled = base::FeatureList::IsEnabled(features::kScreenPowerListenerForNativeWinOcclusion);
...
void NativeWindowOcclusionTrackerWin::OnResume()
{
    ...
    update_occlusion_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&WindowOcclusionCalculator::HandleResumeSuspend, base::Unretained(WindowOcclusionCalculator::GetInstance())));
}
...
void NativeWindowOcclusionTrackerWin::MarkNonIconicWindowsOccluded()
{
    ...
    for (const auto& root_window_hwnd_pair : hwnd_root_window_map_) {
        root_window_hwnd_pair.second->GetHost()->SetNativeWindowOcclusionState(
            IsIconic(root_window_hwnd_pair.first) ? Window::OcclusionState::HIDDEN : Window::OcclusionState::OCCLUDED, {});
    }
}

NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::WindowOcclusionCalculator(scoped_refptr<base::SequencedTaskRunner> task_runner,
    scoped_refptr<base::SequencedTaskRunner> ui_thread_task_runner, UpdateOcclusionStateCallback update_occlusion_state_callback)
    : task_runner_(task_runner)
    , ui_thread_task_runner_(ui_thread_task_runner)
    , calculate_occluded_region_(base::FeatureList::IsEnabled(features::kApplyNativeOccludedRegionToWindowTracker))
    , update_occlusion_state_callback_(update_occlusion_state_callback)
{
    ::CoCreateInstance(__uuidof(VirtualDesktopManager), nullptr, CLSCTX_ALL, IID_PPV_ARGS(&virtual_desktop_manager_));
    DETACH_FROM_SEQUENCE(sequence_checker_);
}

NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::~WindowOcclusionCalculator()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    UnregisterEventHooks();
}

void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::CreateInstance(scoped_refptr<base::SequencedTaskRunner> task_runner,
    scoped_refptr<base::SequencedTaskRunner> ui_thread_task_runner, UpdateOcclusionStateCallback update_occlusion_state_callback)
{
    DCHECK(!instance_);
    instance_ = new WindowOcclusionCalculator(task_runner, ui_thread_task_runner, update_occlusion_state_callback);
}

void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::DeleteInstanceForTesting(base::WaitableEvent* done_event)
{
    DCHECK(instance_);
    delete instance_;
    instance_ = nullptr;
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::EnableOcclusionTrackingForWindow(HWND hwnd)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::DisableOcclusionTrackingForWindow(HWND hwnd)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::HandleVisibilityChanged(bool visible)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::ForceRecalculation()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::HandleResumeSuspend()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::MaybeRegisterEventHooks()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (global_event_hooks_.empty())
        RegisterEventHooks();
...
// static
void CALLBACK NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::EventHookCallback(
...
// static
BOOL CALLBACK NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::ComputeNativeWindowOcclusionStatusCallback(HWND hwnd, LPARAM lParam)
{
    if (instance_) {
        return instance_->ProcessComputeNativeWindowOcclusionStatusCallback(hwnd, reinterpret_cast<base::flat_set<DWORD>*>(lParam));
    }
    return FALSE;
}

// static
BOOL CALLBACK NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::UpdateVisibleWindowProcessIdsCallback(HWND hwnd, LPARAM lParam)
{
    if (instance_) {
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::UpdateVisibleWindowProcessIds()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::ComputeNativeWindowOcclusionStatus()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (root_window_hwnds_occlusion_state_.empty())
        return;
    ...
    SkRegion screen_region = SkRegion(
        SkIRect::MakeLTRB(screen_left, screen_top, screen_left + GetSystemMetrics(SM_CXVIRTUALSCREEN), screen_top + GetSystemMetrics(SM_CYVIRTUALSCREEN)));
    ...
    for (auto& root_window_pair : root_window_hwnds_occlusion_state_) {
        ...
        root_window_pair.second.on_current_workspace = IsWindowOnCurrentVirtualDesktop(hwnd);
        ...
        if (IsIconic(hwnd)) {
            ...
            root_window_pair.second.occlusion_state = Window::OcclusionState::OCCLUDED;
    ...
    if (should_unregister_event_hooks) {
        ...
        // the windows.
        EnumWindows(&ComputeNativeWindowOcclusionStatusCallback, reinterpret_cast<LPARAM>(&current_pids_with_visible_windows));
        ...
        for (auto loc_change_pid : pids_for_location_change_hook_) {
            if (current_pids_with_visible_windows.find(loc_change_pid) == current_pids_with_visible_windows.end()) {
        ...
        if (!pids_to_remove.empty()) {
            ...
            base::EraseIf(pids_for_location_change_hook_, [&pids_to_remove](DWORD pid) { return pids_to_remove.find(pid) != pids_to_remove.end(); });
        }
    }
    // Post a task to the browser ui thread to update the window occlusion state
    // on the root windows.
    ui_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(update_occlusion_state_callback_, root_window_hwnds_occlusion_state_, showing_thumbnails_));
}

void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::ScheduleOcclusionCalculationIfNeeded()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!occlusion_update_timer_.IsRunning()) {
        occlusion_update_timer_.Start(FROM_HERE, kUpdateOcclusionDelay, this, &WindowOcclusionCalculator::ComputeNativeWindowOcclusionStatus);
    }
}

void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::RegisterGlobalEventHook(UINT event_min, UINT event_max)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    HWINEVENTHOOK event_hook = SetWinEventHook(event_min, event_max, nullptr, &EventHookCallback, 0, 0, WINEVENT_OUTOFCONTEXT);

    global_event_hooks_.push_back(event_hook);
}

void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::RegisterEventHookForProcess(DWORD pid)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    pids_for_location_change_hook_.insert(pid);
    process_event_hooks_[pid]
        = SetWinEventHook(EVENT_OBJECT_LOCATIONCHANGE, EVENT_OBJECT_LOCATIONCHANGE, nullptr, &EventHookCallback, pid, 0, WINEVENT_OUTOFCONTEXT);
}

void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::RegisterEventHooks()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::UnregisterEventHooks()
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::ProcessComputeNativeWindowOcclusionStatusCallback(
    HWND hwnd, base::flat_set<DWORD>* current_pids_with_visible_windows)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    gfx::Rect window_rect;
    const bool window_is_occluding = WindowCanOccludeOtherWindowsOnCurrentVirtualDesktop(hwnd, &window_rect);
    ...
    if (it == root_window_hwnds_occlusion_state_.end() || it->second.occlusion_state != Window::OcclusionState::UNKNOWN) {
        if (window_is_occluding) {
            unoccluded_desktop_region_.op(gfx::RectToSkIRect(window_rect), SkRegion::kDifference_Op);
        }
        return true;
    }
    ...
    if (!window_is_occluding) {
        ...
        if (::GetWindowRect(hwnd, &rect) != 0) {
            SkRegion window_region(SkIRect::MakeLTRB(rect.left, rect.top, rect.right, rect.bottom));
    ...
    occluded_window_region.op(gfx::RectToSkIRect(window_rect), SkRegion::kIntersect_Op);
    if (occluded_window_region.isEmpty())
        return true;

    occluded_window_region.op(gfx::RectToSkIRect(window_rect), SkRegion::kReverseDifference_Op);
    occluded_window_region.translate(-window_rect.x(), -window_rect.y());
    it->second.occluded_region_pixels.swap(occluded_window_region);
    return true;
}

void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::ProcessEventHookCallback(DWORD event, HWND hwnd, LONG id_object, LONG id_child)
{
    ...
    if (::GetWindowLong(hwnd, GWL_STYLE) & WS_POPUP) {
        std::wstring hwnd_class_name = gfx::GetClassName(hwnd);
        calculate_occlusion = hwnd_class_name.starts_with(L"Chrome_WidgetWin_") || hwnd_class_name == L"Shell_TrayWnd";
    }
    ...
    if (event == EVENT_OBJECT_SHOW) {
        ...
        if ((hwnd_class_name == "MultitaskingViewFrame" || hwnd_class_name == "TaskListThumbnailWnd")) {
            showing_thumbnails_ = true;
            ui_thread_task_runner_->PostTask(
                FROM_HERE, base::BindOnce(update_occlusion_state_callback_, root_window_hwnds_occlusion_state_, showing_thumbnails_));
        }
        return;
    ...
    // it when deciding if windows under it are occluded.
        ...
        if (event == EVENT_OBJECT_LOCATIONCHANGE || event == EVENT_OBJECT_STATECHANGE) {
            ...
            if ((root_window_hwnds_occlusion_state_.size() <= 1)
                || (root_window_hwnds_occlusion_state_.find(hwnd) == root_window_hwnds_occlusion_state_.end())) {
                return;
            }
        } else {
    ...
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&WindowOcclusionCalculator::ScheduleOcclusionCalculationIfNeeded, weak_factory_.GetWeakPtr()));
}

void NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::ProcessUpdateVisibleWindowProcessIdsCallback(HWND hwnd)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    gfx::Rect window_rect;
...
bool NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::WindowCanOccludeOtherWindowsOnCurrentVirtualDesktop(HWND hwnd, gfx::Rect* window_rect)
{
    return IsWindowVisibleAndFullyOpaque(hwnd, window_rect) && (IsWindowOnCurrentVirtualDesktop(hwnd) == true);
}

std::optional<bool> NativeWindowOcclusionTrackerWin::WindowOcclusionCalculator::IsWindowOnCurrentVirtualDesktop(HWND hwnd)
{
