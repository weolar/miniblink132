...
class AURA_EXPORT NativeWindowOcclusionTrackerWin : public WindowObserver, public ui::PowerSettingChangeListener {
public:
    static NativeWindowOcclusionTrackerWin* GetOrCreateInstance();

    static void DeleteInstanceForTesting();

    NativeWindowOcclusionTrackerWin(const NativeWindowOcclusionTrackerWin&) = delete;
    NativeWindowOcclusionTrackerWin& operator=(const NativeWindowOcclusionTrackerWin&) = delete;
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(NativeWindowOcclusionTrackerTest, DisplayOnOffHandling);
    ...
    public:
        using UpdateOcclusionStateCallback = base::RepeatingCallback<void(const HwndToRootOcclusionStateMap&, bool show_all_windows)>;

        // Creates WindowOcclusionCalculator instance. Must be called on UI thread.
        static void CreateInstance(scoped_refptr<base::SequencedTaskRunner> task_runner, scoped_refptr<base::SequencedTaskRunner> ui_thread_task_runner,
            UpdateOcclusionStateCallback update_occlusion_state_callback);

        // Returns existing WindowOcclusionCalculator instance.
        static WindowOcclusionCalculator* GetInstance()
        {
            return instance_;
        }
        ...
        WindowOcclusionCalculator& operator=(const WindowOcclusionCalculator&) = delete;
    ...
    private:
        WindowOcclusionCalculator(scoped_refptr<base::SequencedTaskRunner> task_runner, scoped_refptr<base::SequencedTaskRunner> ui_thread_task_runner,
            UpdateOcclusionStateCallback update_occlusion_state_callback);
        ...
        static void CALLBACK EventHookCallback(
            HWINEVENTHOOK hWinEventHook, DWORD event, HWND hwnd, LONG id_object, LONG id_child, DWORD dwEventThread, DWORD dwmsEventTime);
        ...
        static BOOL CALLBACK ComputeNativeWindowOcclusionStatusCallback(HWND hwnd, LPARAM lParam);

        // EnumWindows callback used to update the list of process ids with
        // visible hwnds, |pids_for_location_change_hook_|.
        static BOOL CALLBACK UpdateVisibleWindowProcessIdsCallback(HWND hwnd, LPARAM lParam);
        ...
        bool ProcessComputeNativeWindowOcclusionStatusCallback(HWND hwnd, base::flat_set<DWORD>* current_pids_with_visible_windows);
        ...
        // necessary.
        void ProcessEventHookCallback(DWORD event, HWND hwnd, LONG idObject, LONG idChild);
        ...
        bool WindowCanOccludeOtherWindowsOnCurrentVirtualDesktop(HWND hwnd, gfx::Rect* window_rect);
    ...
    void UpdateOcclusionState(const HwndToRootOcclusionStateMap& root_window_hwnds_occlusion_state, bool show_all_windows);
