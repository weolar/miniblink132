...
public:
    ...
    MockWindowTreeHostObserver& operator=(const MockWindowTreeHostObserver&) = delete;

    ~MockWindowTreeHostObserver() override
    {
        EXPECT_FALSE(is_expecting_call());
    }

    // WindowTreeHostObserver:
    void OnOcclusionStateChanged(WindowTreeHost* host, Window::OcclusionState new_state, const SkRegion& occluded_region) override
    {
        // Should only get notified when the occlusion state changes.
        EXPECT_NE(new_state, cur_state_);
        cur_state_ = new_state;
        if (expectation_ != Window::OcclusionState::UNKNOWN && cur_state_ == expectation_) {
    ...
    {
        return expectation_ != cur_state_;
    }
...
public:
    explicit MockWindowObserver(Window* window)
        : window_(window)
    {
...
private:
    ...
    BOOL ProcessWindowMessage(HWND window, UINT message, WPARAM w_param, LPARAM l_param, LRESULT& result, DWORD msg_map_id) override
    {
...
public:
    ...
    NativeWindowOcclusionTrackerTest(const NativeWindowOcclusionTrackerTest&) = delete;
    NativeWindowOcclusionTrackerTest& operator=(const NativeWindowOcclusionTrackerTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        scoped_feature_list_.InitWithFeatures({ features::kCalculateNativeWinOcclusion, features::kApplyNativeOccludedRegionToWindowTracker }, {});
    ...
    void SetNativeWindowBounds(HWND hwnd, const gfx::Rect& bounds)
    {
        ...
        SetWindowPos(hwnd, HWND_TOP, window_bounds.x(), window_bounds.y(), window_bounds.width(), window_bounds.height(), SWP_NOREPOSITION);
        EXPECT_TRUE(UpdateWindow(hwnd));
    }

    HWND CreateNativeWindowWithBounds(const gfx::Rect& bounds)
    {
        std::unique_ptr<TestNativeWindow> native_win = std::make_unique<TestNativeWindow>();
    ...
    Window* CreateTrackedAuraWindowWithBounds(MockWindowTreeHostObserver* observer, const gfx::Rect& bounds)
    {
        host()->Show();
    ...
    void MakeFullscreen(HWND hwnd)
    {
        ...
        SetWindowPos(hwnd, nullptr, window_rect.x(), window_rect.y(), window_rect.width(), window_rect.height(), SWP_FRAMECHANGED | SWP_ASYNCWINDOWPOS);
    }
...
TEST_F(NativeWindowOcclusionTrackerTest, OffscreenOcclusion)
{
    ...
    SetWindowPos(host()->GetAcceleratedWidget(), HWND_TOP, screen_left - 50, 0, 100, 100, SWP_NOZORDER | SWP_NOSIZE);
...
TEST_F(NativeWindowOcclusionTrackerTest, LockScreenVisibleOcclusion)
{
    ...
    PostMessage(gfx::SingletonHwnd::GetInstance()->hwnd(), WM_WTSSESSION_CHANGE, WTS_SESSION_LOCK, current_session_id);
    run_loop2.Run();
    EXPECT_FALSE(observer.is_expecting_call());
    host()->RemoveObserver(&observer);
}
...
TEST_F(NativeWindowOcclusionTrackerTest, LockScreenDifferentSession)
{
    ...
    DWORD current_session_id = 0;
    ProcessIdToSessionId(::GetCurrentProcessId(), &current_session_id);
    PostMessage(gfx::SingletonHwnd::GetInstance()->hwnd(), WM_WTSSESSION_CHANGE, WTS_SESSION_LOCK, current_session_id + 1);
...
TEST_F(NativeWindowOcclusionTrackerTest, DisplayOnOffHandling)
{
    ...
    NativeWindowOcclusionTrackerWin* occlusion_tracker = NativeWindowOcclusionTrackerWin::GetOrCreateInstance();
...
TEST_F(NativeWindowOcclusionTrackerTest, DISABLED_MovingWindowNotConsideredInCalculations)
{
    // Needed as this test triggers a native nested message loop.
    base::CurrentThread::ScopedAllowApplicationTasksInNativeNestedLoop allow_nesting;
    ...
    ASSERT_TRUE(ui_controls::SendMouseEvents(ui_controls::LEFT, ui_controls::DOWN));
    run_loop3.Run();
    EXPECT_FALSE(observer.is_expecting_call());
...
TEST_F(NativeWindowOcclusionTrackerTest, DISABLED_MaximizedOccludedByFullscreenWindow)
{
    // Create an aura window that is maximized.
    base::RunLoop run_loop1;
    MockWindowTreeHostObserver observer(run_loop1.QuitClosure());
    HWND hwnd_aura_window_maximized = CreateTrackedAuraWindowWithBounds(&observer, gfx::Rect(0, 0, 100, 100))->GetHost()->GetAcceleratedWidget();
    ...
    observer.set_quit_closure(run_loop2.QuitClosure());
    observer.set_expectation(Window::OcclusionState::OCCLUDED);
    HWND hwnd_native_window = CreateNativeWindowWithBounds(gfx::Rect(0, 0, 100, 100));
...
{
    Window* tracked_aura_window = CreateTrackedAuraWindowWithBounds(nullptr, gfx::Rect(20, 20, 200, 200));
    ...
    HWND obscuring_hwnd = CreateNativeWindowWithBounds(gfx::Rect(20, 20, 110, 110));
    run_loop.Run();
    EXPECT_EQ(Window::OcclusionState::OCCLUDED, tracked_aura_window->GetOcclusionState());
    ...
    run_loop2.Run();
    EXPECT_EQ(Window::OcclusionState::VISIBLE, tracked_aura_window->GetOcclusionState());
...
class NativeWindowOcclusionTrackerTestWithDpi2 : public NativeWindowOcclusionTrackerTest {
public:
    // NativeWindowOcclusionTrackerTest:
    void SetUp() override
    {
