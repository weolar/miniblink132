...
public:
    ...
    int non_client_count() const
    {
        return non_client_count_;
    }
    const gfx::Point& non_client_location() const
    {
        return non_client_location_;
    }
    int mouse_event_count() const
    {
        return mouse_event_count_;
    }
    const gfx::Point& mouse_event_location() const
    {
        return mouse_event_location_;
    }
    int mouse_event_flags() const
    {
        return mouse_event_flags_;
    }
...
public:
    ...
    ui::EventDispatchDetails DispatchKeyEventPostIME(ui::KeyEvent* event) override
    {
        ++dispatched_event_count_;
        return ui::EventDispatchDetails();
    }

    int dispatched_event_count() const
    {
...
gfx::Point GetLastTouchPoint(aura::Window* window, std::optional<gfx::Point> fallback = std::nullopt)
{
    return Env::GetInstance()->GetLastPointerPoint(ui::mojom::DragEventSource::kTouch, window, fallback);
}
...
TEST_F(WindowEventDispatcherTest, OnHostMouseEvent)
{
    ...
    std::unique_ptr<aura::Window> window1(CreateTestWindowWithDelegate(delegate1.get(), -1234, bounds1, root_window()));
    std::unique_ptr<aura::Window> window2(CreateTestWindowWithDelegate(delegate2.get(), -5678, bounds2, root_window()));

    // Send a mouse event to window1.
    gfx::Point point(101, 201);
    ui::MouseEvent event1(ui::EventType::kMousePressed, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
...
TEST_F(WindowEventDispatcherTest, RepostEvent)
{
    ...
    ui::MouseEvent event(ui::EventType::kMousePressed, point, point, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    host()->dispatcher()->RepostEvent(&event);
    RunAllPendingInMessageLoop();
    EXPECT_TRUE(Env::GetInstance()->IsMouseButtonDown());

    ui::TouchEvent touch_pressed_event(
        ui::EventType::kTouchPressed, gfx::Point(10, 10), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
...
{
    std::unique_ptr<WindowTreeHost> second_host = WindowTreeHost::Create(ui::PlatformWindowInitProperties { gfx::Rect(20, 30, 100, 50) });
    second_host->InitHost();
    second_host->window()->Show();

    ui::TouchEvent touch_pressed_event1(
        ui::EventType::kTouchPressed, gfx::Point(10, 10), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
    ui::TouchEvent touch_pressed_event2(
        ui::EventType::kTouchPressed, gfx::Point(10, 10), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 2));
    ui::TouchEvent touch_released_event1(
        ui::EventType::kTouchReleased, gfx::Point(10, 10), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
    ui::TouchEvent touch_released_event2(
        ui::EventType::kTouchReleased, gfx::Point(10, 10), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 2));
...
TEST_F(WindowEventDispatcherTest, MouseButtonState)
{
    ...
    event = std::make_unique<ui::MouseEvent>(
        ui::EventType::kMousePressed, location, location, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    DispatchEventUsingWindowDispatcher(event.get());
    EXPECT_TRUE(Env::GetInstance()->IsMouseButtonDown());

    // Additionally press the right.
    event = std::make_unique<ui::MouseEvent>(ui::EventType::kMousePressed, location, location, ui::EventTimeForNow(),
        ui::EF_LEFT_MOUSE_BUTTON | ui::EF_RIGHT_MOUSE_BUTTON, ui::EF_RIGHT_MOUSE_BUTTON);
    DispatchEventUsingWindowDispatcher(event.get());
    EXPECT_TRUE(Env::GetInstance()->IsMouseButtonDown());

    // Release the left button.
    event = std::make_unique<ui::MouseEvent>(
        ui::EventType::kMouseReleased, location, location, ui::EventTimeForNow(), ui::EF_RIGHT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    DispatchEventUsingWindowDispatcher(event.get());
    EXPECT_TRUE(Env::GetInstance()->IsMouseButtonDown());

    // Release the right button.  We should ignore the Shift-is-down flag.
    event = std::make_unique<ui::MouseEvent>(
        ui::EventType::kMouseReleased, location, location, ui::EventTimeForNow(), ui::EF_SHIFT_DOWN, ui::EF_RIGHT_MOUSE_BUTTON);
    DispatchEventUsingWindowDispatcher(event.get());
    EXPECT_FALSE(Env::GetInstance()->IsMouseButtonDown());

    // Press the middle button.
    event = std::make_unique<ui::MouseEvent>(
        ui::EventType::kMousePressed, location, location, ui::EventTimeForNow(), ui::EF_MIDDLE_MOUSE_BUTTON, ui::EF_MIDDLE_MOUSE_BUTTON);
    DispatchEventUsingWindowDispatcher(event.get());
    EXPECT_TRUE(Env::GetInstance()->IsMouseButtonDown());
}
...
{
    std::unique_ptr<Window> w1(test::CreateTestWindowWithDelegate(NULL, 1, gfx::Rect(50, 50, 100, 100), root_window()));

    gfx::Point origin(100, 100);
    ui::MouseEvent root(ui::EventType::kMousePressed, origin, origin, ui::EventTimeForNow(), 0, 0);

    EXPECT_EQ("100,100", root.location().ToString());
    EXPECT_EQ("100,100", root.root_location().ToString());

    ui::MouseEvent translated_event(root, static_cast<Window*>(root_window()), w1.get(), ui::EventType::kMouseEntered, root.flags());
...
public:
    ...
    {
        return const_cast<Window*>(static_cast<const TestEventClient*>(this)->GetLockWindow());
    }
...
TEST_F(WindowEventDispatcherTest, GetCanProcessEventsWithinSubtree)
{
    ...
    Window* w1 = test::CreateTestWindowWithBounds(gfx::Rect(10, 10, 20, 20), client.GetNonLockWindow());
    w1->SetId(1);
    Window* w2 = test::CreateTestWindowWithBounds(gfx::Rect(30, 30, 20, 20), client.GetNonLockWindow());
    w2->SetId(2);
    std::unique_ptr<Window> w3(test::CreateTestWindowWithDelegate(&d, 3, gfx::Rect(30, 30, 20, 20), client.GetLockWindow()));
...
TEST_F(WindowEventDispatcherTest, DontIgnoreUnknownKeys)
{
...
#if BUILDFLAG(IS_OZONE)
    ui::SetKeyboardImeFlagProperty(&properties, ui::kPropertyKeyboardImeIgnoredFlag);
#endif

    ConsumeKeyHandler handler;
    root_window()->AddPreTargetHandler(&handler);

    ui::KeyEvent unknown_event(ui::EventType::kKeyPressed, ui::VKEY_UNKNOWN, ui::EF_NONE);
    ...
    handler.Reset();
    ui::KeyEvent ime_event(ui::EventType::kKeyPressed, ui::VKEY_UNKNOWN, ui::EF_IME_FABRICATED_KEY);
    ...
    handler.Reset();
    ui::KeyEvent unknown_key_with_char_event(ui::EventType::kKeyPressed, ui::VKEY_UNKNOWN, ui::EF_NONE);
...
TEST_F(WindowEventDispatcherTest, TouchEventsOutsideBounds)
{
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, position, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&press);
    EXPECT_EQ(1, handler.num_touch_events());

    position = root_window()->bounds().origin();
    position.Offset(root_window()->bounds().width() + 10, root_window()->bounds().height() + 10);
    ui::TouchEvent release(ui::EventType::kTouchReleased, position, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
...
TEST_F(WindowEventDispatcherTest, ScrollEventDispatch)
{
    ...
    ui::ScrollEvent scroll1(ui::EventType::kScroll, gfx::Point(10, 10), now, 0, 0, -10, 0, -10, 2);
    ...
    ui::ScrollEvent scroll2(ui::EventType::kScroll, gfx::Point(25, 30), now, 0, -10, 0, -10, 0, 2);
...
TEST_F(WindowEventDispatcherTest, PreDispatchKeyEventToIme)
{
    ...
    ui::KeyEvent key_release(ui::EventType::kKeyReleased, ui::VKEY_A, ui::EF_NONE);
...
public:
    ...
    const EventLocations& mouse_locations() const
    {
        return mouse_locations_;
    }
    gfx::Point mouse_location(int i) const
    {
        return mouse_locations_[i];
    }
    const EventLocations& touch_locations() const
    {
        return touch_locations_;
    }
    const EventLocations& gesture_locations() const
    {
        return gesture_locations_;
    }
    const EventFlags& mouse_event_flags() const
    {
        return mouse_event_flags_;
    }
...
std::string EventTypeToString(ui::EventType type)
{
    ...
    default:
        ...
        NOTREACHED() << "Received unexpected event: " << base::to_underlying(type);
    }
}
...
TEST_F(WindowEventDispatcherTest, MAYBE(RepostTargetsCaptureWindow))
{
    ...
    const ui::MouseEvent press_event(
        ui::EventType::kMousePressed, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
...
TEST_F(WindowEventDispatcherTest, MouseMovesHeld)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(0, 0, 100, 100), root_window()));

    ui::MouseEvent mouse_move_event(ui::EventType::kMouseMoved, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), 0, 0);
    ...
    ui::MouseEvent mouse_dragged_event(ui::EventType::kMouseDragged, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), 0, 0);
    ...
    // of event.
    ui::MouseEvent mouse_pressed_event(ui::EventType::kMousePressed, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), 0, 0);
    DispatchEventUsingWindowDispatcher(&mouse_pressed_event);
    EXPECT_EQ("MOUSE_DRAGGED MOUSE_PRESSED", EventTypesToString(recorder.events()));
    recorder.Reset();
    ...
    mouse_dragged_event = ui::MouseEvent(ui::EventType::kMouseDragged, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), 0, 0);
    ui::MouseEvent mouse_dragged_event2(ui::EventType::kMouseDragged, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), 0, 0);
    DispatchEventUsingWindowDispatcher(&mouse_dragged_event);
    DispatchEventUsingWindowDispatcher(&mouse_dragged_event2);
    EXPECT_TRUE(recorder.events().empty());
    mouse_pressed_event = ui::MouseEvent(ui::EventType::kMousePressed, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), 0, 0);
    DispatchEventUsingWindowDispatcher(&mouse_pressed_event);
    EXPECT_EQ("MOUSE_DRAGGED MOUSE_PRESSED", EventTypesToString(recorder.events()));
    recorder.Reset();
    ...
    DispatchEventUsingWindowDispatcher(&mouse_dragged_event);
    host()->dispatcher()->ReleasePointerMoves();
    mouse_dragged_event2 = ui::MouseEvent(ui::EventType::kMouseDragged, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), 0, 0);
    DispatchEventUsingWindowDispatcher(&mouse_dragged_event2);
    EXPECT_EQ("MOUSE_DRAGGED", EventTypesToString(recorder.events()));
    recorder.Reset();
    RunAllPendingInMessageLoop();
    ...
    mouse_dragged_event = ui::MouseEvent(ui::EventType::kMouseDragged, gfx::Point(0, 0), gfx::Point(0, 0), ui::EventTimeForNow(), 0, 0);
    mouse_dragged_event2 = ui::MouseEvent(ui::EventType::kMouseDragged, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), 0, 0);
    ui::MouseEvent mouse_dragged_event3(ui::EventType::kMouseDragged, gfx::Point(28, 28), gfx::Point(28, 28), ui::EventTimeForNow(), 0, 0);
    host()->dispatcher()->HoldPointerMoves();
    DispatchEventUsingWindowDispatcher(&mouse_dragged_event);
    DispatchEventUsingWindowDispatcher(&mouse_dragged_event2);
...
TEST_F(WindowEventDispatcherTest, TouchMovesHeld)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(50, 50, 100, 100), root_window()));
    ...
    ui::TouchEvent touch_moved_event(
        ui::EventType::kTouchMoved, gfx::Point(10, 10), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ui::TouchEvent touch_moved_event2(
        ui::EventType::kTouchMoved, gfx::Point(11, 10), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ui::TouchEvent touch_moved_event3(
        ui::EventType::kTouchMoved, gfx::Point(12, 10), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ...
    ui::TouchEvent touch_released_event(
        ui::EventType::kTouchReleased, gfx::Point(10, 10), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    recorder.Reset();
    host()->dispatcher()->HoldPointerMoves();
    DispatchEventUsingWindowDispatcher(&touch_moved_event3);
    DispatchEventUsingWindowDispatcher(&touch_released_event);
    EXPECT_EQ("TOUCH_MOVED TOUCH_RELEASED GESTURE_TAP GESTURE_END", EventTypesToString(recorder.events()));
    recorder.Reset();
    host()->dispatcher()->ReleasePointerMoves();
    RunAllPendingInMessageLoop();
    EXPECT_TRUE(recorder.events().empty());
...
TEST_F(WindowEventDispatcherTest, MouseEventWithoutTargetWindow)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<aura::Window> window_first(CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(20, 10, 10, 20), root_window()));
    window_first->Show();
    window_first->AddPreTargetHandler(&recorder_first);

    std::unique_ptr<aura::Window> window_second(CreateTestWindowWithDelegate(&delegate, 2, gfx::Rect(20, 30, 10, 20), root_window()));
    window_second->Show();
    window_second->AddPreTargetHandler(&recorder_second);

    const gfx::Point event_location(22, 33);
    ui::MouseEvent mouse(ui::EventType::kMouseMoved, event_location, event_location, ui::EventTimeForNow(), 0, 0);
    DispatchEventUsingWindowDispatcher(&mouse);

    EXPECT_TRUE(recorder_first.events().empty());
    EXPECT_EQ("MOUSE_ENTERED MOUSE_MOVED", EventTypesToString(recorder_second.events()));
    ASSERT_EQ(2u, recorder_second.mouse_locations().size());
    EXPECT_EQ(gfx::Point(2, 3).ToString(), recorder_second.mouse_locations()[0].ToString());
...
TEST_F(WindowEventDispatcherTest, DispatchMouseExitWhenHidingWindow)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(10, 10, 50, 50), root_window()));
    window->Show();
    window->AddPreTargetHandler(&recorder);
    ...
    EXPECT_EQ(gfx::Point(12, 23).ToString(), recorder.mouse_locations()[0].ToString());
    window->RemovePreTargetHandler(&recorder);
}
...
TEST_F(WindowEventDispatcherTest, CallToProcessedTouchEvent)
{
    ...
    host()->dispatcher()->ProcessedTouchEvent(0, window.get(), ui::ER_UNHANDLED, false /* is_source_touch_event_set_blocking */);
}
...
public:
    HoldPointerOnScrollHandler(WindowEventDispatcher* dispatcher, EventFilterRecorder* filter)
    ...
    HoldPointerOnScrollHandler& operator=(const HoldPointerOnScrollHandler&) = delete;
...
private:
    ...
    {
        if (!holding_moves_ && gesture->type() == ui::EventType::kGestureScrollUpdate) {
            holding_moves_ = true;
...
TEST_F(WindowEventDispatcherTest, TouchMovesHeldOnScroll)
{
    ...
    ui::test::EventGenerator generator(root_window());
    generator.GestureScrollSequence(gfx::Point(60, 60), gfx::Point(10, 60), base::Milliseconds(100), 25);
    ...
    EXPECT_EQ("TOUCH_MOVED GESTURE_SCROLL_UPDATE TOUCH_RELEASED "
              "GESTURE_SCROLL_END GESTURE_END",
        EventTypesToString(events));
    ASSERT_EQ(2u, recorder.touch_locations().size());
    EXPECT_EQ(gfx::Point(-40, 10).ToString(), recorder.touch_locations()[0].ToString());
    EXPECT_EQ(gfx::Point(-40, 10).ToString(), recorder.touch_locations()[1].ToString());
...
TEST_F(WindowEventDispatcherTest, HeldTouchMoveContributesToGesture)
{
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, location, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&press);
    ...
    ui::TouchEvent move(ui::EventType::kTouchMoved, next_location, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
...
TEST_F(WindowEventDispatcherTest, DispatchSyntheticMouseEvents)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(&delegate, 1234, gfx::Rect(5, 5, 100, 100), root_window()));
    window->Show();
    window->SetCapture();
    ...
    ui::MouseEvent mouse1(ui::EventType::kMouseMoved, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), 0, 0);
    DispatchEventUsingWindowDispatcher(&mouse1);
    EXPECT_FALSE(recorder.events().empty());
    recorder.Reset();

    // Dispatch a synthetic mouse event when mouse events are enabled.
    ui::MouseEvent mouse2(ui::EventType::kMouseMoved, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), ui::EF_IS_SYNTHESIZED, 0);
...
TEST_F(WindowEventDispatcherTest, DoNotSynthesizeWhileButtonDown)
{
    ...
    ui::MouseEvent mouse1(
        ui::EventType::kMousePressed, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    DispatchEventUsingWindowDispatcher(&mouse1);
...
TEST_F(WindowEventDispatcherTest, DoNotDispatchInShutdown)
{
    ...
    ui::MouseEvent press(ui::EventType::kMousePressed, center, center, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
    DispatchEventUsingWindowDispatcher(&press);
...
TEST_F(WindowEventDispatcherTest, MAYBE(SynthesizeMouseEventsOnWindowBoundsChanged))
{
    test::TestCursorClient cursor_client(root_window());
    ...
    ui::MouseEvent mouse(ui::EventType::kMouseMoved, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), 0, 0);
    DispatchEventUsingWindowDispatcher(&mouse);
    EXPECT_FALSE(recorder.events().empty());
    recorder.Reset();
...
TEST_F(WindowEventDispatcherTest, DispatchMouseExitWhenCursorHidden)
{
    ...
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(&delegate, 1234, gfx::Rect(window_origin, gfx::Size(100, 100)), root_window()));
    window->Show();

    // Dispatch a mouse move event into the window.
    gfx::Point mouse_location(gfx::Point(15, 25));
    ui::MouseEvent mouse1(ui::EventType::kMouseMoved, mouse_location, mouse_location, ui::EventTimeForNow(), 0, 0);
    EXPECT_TRUE(recorder.events().empty());
    DispatchEventUsingWindowDispatcher(&mouse1);
    EXPECT_FALSE(recorder.events().empty());
    recorder.Reset();
...
TEST_F(WindowEventDispatcherTest, DispatchSyntheticMouseExitAfterMouseEventsDisabled)
{
    EventFilterRecorder recorder;
    root_window()->AddPreTargetHandler(&recorder);
    ...
    ui::MouseEvent mouse2(ui::EventType::kMouseExited, gfx::Point(150, 150), gfx::Point(150, 150), ui::EventTimeForNow(), ui::EF_IS_SYNTHESIZED, 0);
    DispatchEventUsingWindowDispatcher(&mouse2);
...
public:
    ...
    DetachesParentOnTapDelegate& operator=(const DetachesParentOnTapDelegate&) = delete;
...
TEST_F(WindowEventDispatcherTest, GestureRecognizerResetsTargetWhenParentHides)
{
    std::unique_ptr<Window> w1(CreateNormalWindow(1, root_window(), NULL));
...
public:
    NestedGestureDelegate(ui::test::EventGenerator* generator, const gfx::Point tap_location)
    ...
    int gesture_end_count() const
    {
        return gesture_end_count_;
    }
...
TEST_F(WindowEventDispatcherTest, RepostTapdownGestureTest)
{
    ...
    gfx::Point point(10, 10);
    ui::GestureEvent event(point.x(), point.y(), 0, ui::EventTimeForNow(), details);
    host()->dispatcher()->RepostEvent(&event);
    RunAllPendingInMessageLoop();
...
public:
    RepostGestureEventRecorder(aura::Window* repost_source, aura::Window* repost_target)
    ...
    RepostGestureEventRecorder& operator=(const RepostGestureEventRecorder&) = delete;
    ...
    void OnGestureEvent(ui::GestureEvent* event) override
    {
        EXPECT_EQ(done_cleanup_ ? repost_target_.get() : repost_source_.get(), event->target());
        if (event->type() == ui::EventType::kGestureTapDown) {
            if (!reposted_) {
...
TEST_F(WindowEventDispatcherTest, GestureRepostEventOrder)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<aura::Window> repost_target(CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(0, 0, 100, 100), root_window()));

    std::unique_ptr<aura::Window> repost_source(CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(0, 0, 50, 50), root_window()));

    RepostGestureEventRecorder repost_event_recorder(repost_source.get(), repost_target.get());
    ...
    scroll_generator.GestureScrollSequence(gfx::Point(80, 80), gfx::Point(100, 100), base::Milliseconds(100), 3);
    RunAllPendingInMessageLoop();
    ...
    EXPECT_EQ(kExpectedTargetEvents, EventTypesToString(repost_event_recorder.events()));
...
// event.
template <class T> class OnMouseExitDeletingEventFilter : public EventFilterRecorder {
public:
    ...
    OnMouseExitDeletingEventFilter(const OnMouseExitDeletingEventFilter&) = delete;
    OnMouseExitDeletingEventFilter& operator=(const OnMouseExitDeletingEventFilter&) = delete;
...
TEST_F(WindowEventDispatcherTest, DeleteWindowDuringMouseMovedDispatch)
{
    ...
    EXPECT_EQ("MOUSE_ENTERED MOUSE_MOVED MOUSE_EXITED", EventTypesToString(w1_filter.events()));
    w1->RemovePreTargetHandler(&w1_filter);
}
...
TEST_F(WindowEventDispatcherTest, DeleteDispatcherDuringPreDispatch)
{
    ...
    // on.
    WindowTreeHost* host = WindowTreeHost::Create(ui::PlatformWindowInitProperties { gfx::Rect(0, 0, 100, 100) }).release();
    host->InitHost();
    host->window()->Show();
    ...
    w1_filter.set_delete_closure(base::BindLambdaForTesting([&w1_filter, &w2_filter, &w1, &w2]() {
        w1->RemovePreTargetHandler(&w1_filter);
    ...
    ui::MouseEvent mouse_move(ui::EventType::kMouseMoved, gfx::Point(20, 20), gfx::Point(20, 20), base::TimeTicks(), 0, 0);
...
public:
    ValidRootDuringDestructionWindowObserver(bool* got_destroying, bool* has_valid_root)
        : got_destroying_(got_destroying)
        , has_valid_root_(has_valid_root)
    {
    }

    ValidRootDuringDestructionWindowObserver(const ValidRootDuringDestructionWindowObserver&) = delete;
    ValidRootDuringDestructionWindowObserver& operator=(const ValidRootDuringDestructionWindowObserver&) = delete;
...
TEST_F(WindowEventDispatcherTest, ValidRootDuringDestruction)
{
    ...
    ValidRootDuringDestructionWindowObserver observer(&got_destroying, &has_valid_root);
    {
        std::unique_ptr<WindowTreeHost> host = WindowTreeHost::Create(ui::PlatformWindowInitProperties { gfx::Rect(0, 0, 100, 100) });
        host->InitHost();
        host->window()->Show();
...
public:
    ...
    DontResetHeldEventWindowDelegate(const DontResetHeldEventWindowDelegate&) = delete;
    DontResetHeldEventWindowDelegate& operator=(const DontResetHeldEventWindowDelegate&) = delete;
    ...
    void OnMouseEvent(ui::MouseEvent* event) override
    {
        if ((event->flags() & ui::EF_SHIFT_DOWN) != 0 && mouse_event_count_++ == 0) {
            ui::MouseEvent mouse_event(ui::EventType::kMousePressed, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), ui::EF_SHIFT_DOWN, 0);
...
TEST_F(WindowEventDispatcherTest, DontResetHeldEvent)
{
    ...
    ui::MouseEvent pressed(ui::EventType::kMousePressed, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), ui::EF_SHIFT_DOWN, 0);
    root_window()->GetHost()->dispatcher()->RepostEvent(&pressed);
    ui::MouseEvent pressed2(ui::EventType::kMousePressed, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), 0, 0);
...
public:
    ...
    DeleteHostFromHeldMouseEventDelegate(const DeleteHostFromHeldMouseEventDelegate&) = delete;
    DeleteHostFromHeldMouseEventDelegate& operator=(const DeleteHostFromHeldMouseEventDelegate&) = delete;

    ~DeleteHostFromHeldMouseEventDelegate() override = default;

    bool got_mouse_event() const
    {
        return got_mouse_event_;
    }
    bool got_destroy() const
    {
        return got_destroy_;
    }
...
TEST_F(WindowEventDispatcherTest, DeleteHostFromHeldMouseEvent)
{
    ...
    WindowTreeHost* h2 = WindowTreeHost::Create(ui::PlatformWindowInitProperties { gfx::Rect(0, 0, 100, 100) }).release();
    h2->InitHost();
    h2->window()->Show();
...
TEST_F(WindowEventDispatcherTest, WindowHideCancelsActiveTouches)
{
    ...
    ui::TouchEvent press(ui::EventType::kTouchPressed, position1, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&press);

    EXPECT_EQ("TOUCH_PRESSED GESTURE_BEGIN GESTURE_TAP_DOWN", EventTypesToString(recorder.GetAndResetEvents()));
...
TEST_F(WindowEventDispatcherTest, WindowHideCancelsActiveGestures)
{
    ...
    ui::TouchEvent move(ui::EventType::kTouchMoved, position2, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&move);

    ui::TouchEvent press2(ui::EventType::kTouchPressed, position1, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 1));
...
TEST_F(WindowEventDispatcherTest, TouchpadPinchEventsRetargetOnCapture)
{
    ...
    std::unique_ptr<Window> window1(CreateNormalWindow(1, root_window(), nullptr));
    window1->SetBounds(gfx::Rect(0, 0, 40, 40));

    std::unique_ptr<Window> window2(CreateNormalWindow(2, root_window(), nullptr));
    ...
    ui::GestureEvent begin(position1.x(), position1.y(), 0, ui::EventTimeForNow(), begin_details);
    ...
    ui::GestureEvent update(position1.x(), position1.y(), 0, ui::EventTimeForNow(), update_details);
    ...
    ui::GestureEvent end(position1.x(), position1.y(), 0, ui::EventTimeForNow(), end_details);
    DispatchEventUsingWindowDispatcher(&end);

    EXPECT_EQ("GESTURE_PINCH_BEGIN", EventTypesToString(recorder1.events()));

    EXPECT_EQ("GESTURE_PINCH_UPDATE GESTURE_PINCH_END", EventTypesToString(recorder2.events()));
...
public:
    ...
    {
        capture_window_.reset(test::CreateTestWindowWithDelegate(this, -1234, gfx::Rect(20, 20, 20, 20), root_window));
...
public:
    ...
    ~RunLoopHandler() override
    {
        target_->RemovePreTargetHandler(this);
    }
    int num_scroll_updates() const
    {
        return num_scroll_updates_;
    }
...
TEST_F(WindowEventDispatcherTest, HeldTouchMoveWithRunLoop)
{
    ...
    ui::TouchEvent ev0(ui::EventType::kTouchPressed, point, ui::EventTimeForNow(), ui::PointerDetails());
    DispatchEventUsingWindowDispatcher(&ev0);

    point.Offset(10, 10);
    ui::TouchEvent ev1(ui::EventType::kTouchMoved, point, ui::EventTimeForNow(), ui::PointerDetails());
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindLambdaForTesting([&]() {
        ui::TouchEvent ev2(ui::EventType::kTouchMoved, point, base::TimeTicks::Now(), ui::PointerDetails());
...
public:
    ...
    ExitMessageLoopOnMousePress& operator=(const ExitMessageLoopOnMousePress&) = delete;
...
class WindowEventDispatcherTestWithMessageLoop : public WindowEventDispatcherTest {
public:
    WindowEventDispatcherTestWithMessageLoop()
    {
    }

    WindowEventDispatcherTestWithMessageLoop(const WindowEventDispatcherTestWithMessageLoop&) = delete;
    WindowEventDispatcherTestWithMessageLoop& operator=(const WindowEventDispatcherTestWithMessageLoop&) = delete;
    ...
    void RunTest(base::OnceClosure outer_loop_quit)
    {
        ...
        std::unique_ptr<ui::MouseEvent> mouse(
            new ui::MouseEvent(ui::EventType::kMousePressed, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE));
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
            FROM_HERE, base::BindOnce(&WindowEventDispatcherTestWithMessageLoop::RepostEventHelper, host()->dispatcher(), std::move(mouse)));
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, loop.QuitWhenIdleClosure());
...
private:
    ...
    static void RepostEventHelper(WindowEventDispatcher* dispatcher, std::unique_ptr<ui::MouseEvent> event)
    {
...
TEST_F(WindowEventDispatcherTestWithMessageLoop, EventRepostedInNonNestedLoop)
{
    ...
    base::RunLoop loop;
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
        FROM_HERE, base::BindOnce(&WindowEventDispatcherTestWithMessageLoop::RunTest, base::Unretained(this), loop.QuitWhenIdleClosure()));
    loop.Run();
}
...
{
    test::TestWindowDelegate delegate;
    std::unique_ptr<aura::Window> child(test::CreateTestWindowWithDelegate(&delegate, 1234, gfx::Rect(20, 20, 100, 100), root_window()));
    child->Show();
    ...
    {
        ui::MouseEvent move(ui::EventType::kMouseMoved, gfx::Point(30, 30), gfx::Point(30, 30), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
        DispatchEventUsingWindowDispatcher(&move);
    ...
    {
        ui::MouseEvent move(ui::EventType::kMouseMoved, gfx::Point(50, 50), gfx::Point(50, 50), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
        DispatchEventUsingWindowDispatcher(&move);
...
TEST_F(WindowEventDispatcherTestInHighDPI, TouchMovesHeldOnScroll)
{
    ...
    ui::test::EventGenerator generator(root_window());
    generator.GestureScrollSequence(gfx::Point(120, 120), gfx::Point(20, 120), base::Milliseconds(100), 25);
...
public:
    explicit TriggerNestedLoopOnRightMousePress(const base::RepeatingClosure& callback)
        : callback_(callback)
    {
    }

    TriggerNestedLoopOnRightMousePress(const TriggerNestedLoopOnRightMousePress&) = delete;
    TriggerNestedLoopOnRightMousePress& operator=(const TriggerNestedLoopOnRightMousePress&) = delete;

    ~TriggerNestedLoopOnRightMousePress() override
    {
    }

    const gfx::Point mouse_move_location() const
    {
        return mouse_move_location_;
    }
...
private:
    void OnMouseEvent(ui::MouseEvent* mouse) override
    {
        ...
        if (mouse->type() == ui::EventType::kMousePressed && mouse->IsOnlyRightMouseButton()) {
            base::RunLoop run_loop(base::RunLoop::Type::kNestableTasksAllowed);
            scoped_refptr<base::TaskRunner> task_runner = base::SingleThreadTaskRunner::GetCurrentDefault();
...
// correctly.
TEST_F(WindowEventDispatcherTestInHighDPI, EventsTransformedInRepostedEventTriggeredNestedLoop)
{
    std::unique_ptr<Window> window(CreateNormalWindow(1, root_window(), NULL));
    // Make sure the window is visible.
    RunAllPendingInMessageLoop();

    ui::MouseEvent mouse_move(ui::EventType::kMouseMoved, gfx::Point(80, 80), gfx::Point(80, 80), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
    base::RepeatingClosure callback_on_right_click
        = base::BindRepeating(base::IgnoreResult(&WindowEventDispatcherTestInHighDPI::DispatchEvent), base::Unretained(this), base::Unretained(&mouse_move));
    TriggerNestedLoopOnRightMousePress handler(callback_on_right_click);
    window->AddPreTargetHandler(&handler);

    std::unique_ptr<ui::MouseEvent> mouse(new ui::MouseEvent(
        ui::EventType::kMousePressed, gfx::Point(10, 10), gfx::Point(10, 10), ui::EventTimeForNow(), ui::EF_RIGHT_MOUSE_BUTTON, ui::EF_RIGHT_MOUSE_BUTTON));
    ...
    EXPECT_EQ(gfx::Point(40, 40).ToString(), handler.mouse_move_location().ToString());
    EXPECT_EQ(gfx::Point(40, 40).ToString(), Env::GetInstance()->last_mouse_location().ToString());
    window->RemovePreTargetHandler(&handler);
}
...
TEST_F(WindowEventDispatcherTest, SynthesizedLocatedEvent)
{
    ...
    ui::MouseEvent mouseev(ui::EventType::kMouseMoved, gfx::Point(), gfx::Point(), ui::EventTimeForNow(), ui::EF_IS_SYNTHESIZED, 0);
...
TEST_F(WindowEventDispatcherTest, DestroyWindowOnCaptureChanged)
{
    ...
    std::unique_ptr<aura::Window> window_second(test::CreateTestWindowWithId(2, root_window()));
    window_second->Show();
...
{
    test::TestWindowDelegate delegate;
    std::unique_ptr<Window> focused(CreateTestWindowWithDelegate(&delegate, 123, gfx::Rect(20, 30, 100, 50), NULL));
    ...
    EXPECT_EQ(focused.get(), client::GetFocusClient(root_window())->GetFocusedWindow());
...
private:
    ...
    void OnMouseEvent(ui::MouseEvent* mouse) override
    {
        if (mouse->type() == ui::EventType::kMouseMoved) {
            ui::MouseEvent move(
                ui::EventType::kMouseMoved, target_->bounds().CenterPoint(), target_->bounds().CenterPoint(), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
...
TEST_F(WindowEventDispatcherTest, NestedEventDispatchTargetMoved)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<Window> first(CreateTestWindowWithDelegate(&delegate, 123, gfx::Rect(20, 10, 10, 20), root_window()));
    std::unique_ptr<Window> second(CreateTestWindowWithDelegate(&delegate, 234, gfx::Rect(40, 10, 50, 20), root_window()));
    ...
    ui::MouseEvent move(
        ui::EventType::kMouseMoved, first->bounds().CenterPoint(), first->bounds().CenterPoint(), ui::EventTimeForNow(), ui::EF_NONE, ui::EF_NONE);
...
public:
    ...
    AlwaysMouseDownInputStateLookup(const AlwaysMouseDownInputStateLookup&) = delete;
    AlwaysMouseDownInputStateLookup& operator=(const AlwaysMouseDownInputStateLookup&) = delete;
...
TEST_F(WindowEventDispatcherTest, CursorVisibilityChangedWhileCaptureWindowInAnotherDispatcher)
{
    test::EventCountDelegate delegate;
    std::unique_ptr<Window> window(CreateTestWindowWithDelegate(&delegate, 123, gfx::Rect(20, 10, 10, 20), root_window()));
    window->Show();
    ...
    test::EnvTestHelper(Env::GetInstance()).SetInputStateLookup(std::unique_ptr<InputStateLookup>(new AlwaysMouseDownInputStateLookup()));
...
TEST_F(WindowEventDispatcherTest, RedirectedEventToDifferentDispatcherLocation)
{
    std::unique_ptr<WindowTreeHost> second_host = WindowTreeHost::Create(ui::PlatformWindowInitProperties { gfx::Rect(20, 30, 100, 50) });
    second_host->InitHost();
    second_host->window()->Show();
    ...
    // the end.
    client::CaptureClient* const old_capture_client = client::GetCaptureClient(root_window());
    {
        wm::ScopedCaptureClient scoped_capture_first(root_window());
        wm::ScopedCaptureClient scoped_capture_second(second_host->window());

        test::EventCountDelegate delegate;
        std::unique_ptr<Window> window_first(CreateTestWindowWithDelegate(&delegate, 123, gfx::Rect(20, 10, 10, 20), root_window()));
        window_first->Show();

        std::unique_ptr<Window> window_second(CreateTestWindowWithDelegate(&delegate, 12, gfx::Rect(10, 10, 20, 30), second_host->window()));
        window_second->Show();
        ...
        ui::MouseEvent mouse(
            ui::EventType::kMousePressed, event_location, event_location, ui::EventTimeForNow(), ui::EF_LEFT_MOUSE_BUTTON, ui::EF_LEFT_MOUSE_BUTTON);
        DispatchEventUsingWindowDispatcher(&mouse);
        EXPECT_TRUE(recorder_first.events().empty());
        ASSERT_EQ(1u, recorder_second.events().size());
        EXPECT_EQ(ui::EventType::kMousePressed, recorder_second.events()[0]);
        EXPECT_EQ(event_location.ToString(), recorder_second.mouse_locations()[0].ToString());
        window_first->RemovePreTargetHandler(&recorder_first);
        window_second->RemovePreTargetHandler(&recorder_second);
    }
...
private:
    void OnTouchEvent(ui::TouchEvent* event) override
    {
        ...
        dispatcher_->ProcessedTouchEvent(event->unique_event_id(), window_, ui::ER_UNHANDLED, false /* is_source_touch_event_set_blocking */);
        event->StopPropagation();
    }
...
TEST_F(WindowEventDispatcherTest, GestureEventCoordinates)
{
    ...
    HoldPointerOnScrollHandler handler(host()->dispatcher(), &recorder);
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(kWindowOffset, kWindowOffset, 100, 100), root_window()));
    window->AddPreTargetHandler(&handler);

    delegate.set_window(window.get());

    ui::TouchEvent touch_pressed_event(ui::EventType::kTouchPressed, gfx::Point(), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ...
    EXPECT_EQ(gfx::Point(kX - kWindowOffset, kY - kWindowOffset).ToString(), recorder.touch_locations()[0].ToString());

    ASSERT_EQ(2u, recorder.gesture_locations().size());
    EXPECT_EQ(gfx::Point(kX - kWindowOffset, kY - kWindowOffset).ToString(), recorder.gesture_locations()[0].ToString());
    root_window()->RemovePreTargetHandler(&recorder);
    window->RemovePreTargetHandler(&handler);
}
...
TEST_F(WindowEventDispatcherTest, TouchMovesMarkedWhenCausingScroll)
{
    ...
    ui::TouchEvent move(
        ui::EventType::kTouchMoved, location + gfx::Vector2d(100, 100), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&move);
    ...
    ui::TouchEvent move2(
        ui::EventType::kTouchMoved, location + gfx::Vector2d(200, 200), ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ...
    ui::TouchEvent release(ui::EventType::kTouchReleased, location + gfx::Vector2d(200, 200), ui::EventTimeForNow() + base::Seconds(1),
...
TEST_F(WindowEventDispatcherTest, TouchEventWithScaledWindow)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<Window> child(CreateNormalWindow(1, root_window(), &delegate));

    const gfx::Point child_position(-10, -10);
    const gfx::Rect& root_bounds = root_window()->bounds();
    gfx::Rect child_bounds(child_position, gfx::ScaleToCeiledSize(root_bounds.size(), 2));
    ...
    std::string expected_events = "TOUCH_PRESSED GESTURE_BEGIN GESTURE_TAP_DOWN TOUCH_RELEASED "
                                  "GESTURE_SHOW_PRESS GESTURE_TAP GESTURE_END";
    {
        ...
        ui::TouchEvent pressed_event(ui::EventType::kTouchPressed, touch_position, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
        ui::TouchEvent released_event(
            ui::EventType::kTouchReleased, touch_position, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
...
TEST_F(WindowEventDispatcherTest, TargetIsDestroyedByHeldEvent)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<aura::Window> mouse_target(CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(0, 0, 100, 100), root_window()));
    ...
    aura::Window* focused(test::CreateTestWindowWithBounds(gfx::Rect(200, 200, 100, 100), root_window()));
    ...
    ui::MouseEvent mouse_move_event(ui::EventType::kMouseMoved, gfx::Point(1, 1), gfx::Point(1, 1), ui::EventTimeForNow(), 0, 0);
    DispatchEventUsingWindowDispatcher(&mouse_move_event);
    // Discard MOUSE_ENTER.
    recorder.Reset();
    ...
    public:
        explicit Handler(aura::Window* focused)
            : focused_(focused)
        {
        }
...
TEST_F(WindowEventDispatcherTest, FilteredTouchProcessGesture)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<aura::Window> window(test::CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(100, 100), root_window()));
...
TEST_F(WindowEventDispatcherTest, LastTouchPoint)
{
    ...
    test::TestWindowDelegate delegate;
    std::unique_ptr<aura::Window> window(CreateTestWindowWithDelegate(&delegate, 1, gfx::Rect(10, 10, 100, 100), root_window()));
    ...
    ui::TouchEvent pressed(ui::EventType::kTouchPressed, location1, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    ...
    ui::TouchEvent move(ui::EventType::kTouchMoved, location2, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&move);
    EXPECT_EQ(location2, GetLastTouchPoint(window.get(), fallback));
    EXPECT_EQ(fallback, GetLastTouchPoint(root_window(), fallback));

    constexpr gfx::Point location3(00, 00);
    ui::TouchEvent move2(ui::EventType::kTouchMoved, location3, ui::EventTimeForNow(), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&move2);
    ...
    ui::TouchEvent release(
        ui::EventType::kTouchReleased, location3, ui::EventTimeForNow() + base::Seconds(1), ui::PointerDetails(ui::EventPointerType::kTouch, 0));
    DispatchEventUsingWindowDispatcher(&release);
