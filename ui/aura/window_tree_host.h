...
// aura.
class AURA_EXPORT WindowTreeHost : public ui::ImeKeyEventDispatcher, public ui::EventSource, public display::DisplayObserver, public ui::CompositorObserver {
public:
    ...
    static std::unique_ptr<WindowTreeHost> Create(ui::PlatformWindowInitProperties properties);
    ...
    {
        return const_cast<WindowEventDispatcher*>(const_cast<const WindowTreeHost*>(this)->dispatcher());
    }
    const WindowEventDispatcher* dispatcher() const
    {
    ...
    virtual gfx::Transform GetInverseRootTransformForLocalEventCoordinates() const;
    ...
    gfx::NativeCursor last_cursor() const
    {
        return last_cursor_;
    }
    ...
    virtual void SetNativeWindowOcclusionState(Window::OcclusionState raw_occlusion_state, const SkRegion& raw_occluded_region);
    ...
    const SkRegion& GetNativeOccludedRegion() const
    {
        return occluded_region_;
    }
    ...
    // multiple calls.
    virtual std::unique_ptr<ScopedEnableUnadjustedMouseEvents> RequestUnadjustedMovement();
...
protected:
    ...
    void CreateCompositor(bool force_software_compositor = false, bool use_external_begin_frame_control = false,
    ...
    // Moves the cursor to the specified location relative to the root window.
    virtual void MoveCursorToScreenLocationInPixels(const gfx::Point& location_in_pixels) = 0;
    ...
    void OnDisplayMetricsChanged(const display::Display& display, uint32_t metrics) override;

    // Begins capturing system key events.  Returns true if successful.
    virtual bool CaptureSystemKeyEventsImpl(std::optional<base::flat_set<ui::DomCode>> dom_codes) = 0;
    ...
    virtual gfx::Rect GetTransformedRootWindowBoundsFromPixelSize(const gfx::Size& size_in_pixels) const;
...
private:
    ...
    static const base::flat_set<raw_ptr<WindowTreeHost, CtnExperimental>>& GetThrottledHostsForTesting();

    // Moves the cursor to the specified location. This method is internally used
    // by MoveCursorToLocationInDIP() and MoveCursorToLocationInPixels().
    void MoveCursorToInternal(const gfx::Point& root_location, const gfx::Point& host_location);
    ...
    void OnFrameSinksToThrottleUpdated(const base::flat_set<viz::FrameSinkId>& ids) final;
    void OnSetPreferredRefreshRate(ui::Compositor*, float preferred_refresh_rate) override;
