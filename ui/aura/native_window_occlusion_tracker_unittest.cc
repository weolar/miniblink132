...
HWND TestWin32Window::Create(DWORD style)
{
    ...
    hwnd_ = CreateWindowEx(0, class_name, class_name, style, 0, 0, 100, 100, nullptr, nullptr, GetModuleHandle(nullptr), nullptr);
...
public:
    ...
    TestNativeWindow* native_win()
    {
        return native_win_.get();
    }

    HWND CreateNativeWindow(DWORD style, DWORD ex_style)
    {
        native_win_ = std::make_unique<TestNativeWindow>();
        native_win_->set_window_style(WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | style);
        ...
        if (GetWindowRgn(hwnd, region.get()) == COMPLEXREGION) {
            ...
            base::win::ScopedRegion rectangular_region(CreateRectRgnIndirect(&bounding_rect));
    ...
    {
        bool ret = NativeWindowOcclusionTrackerWin::IsWindowVisibleAndFullyOpaque(hwnd, win_rect);
...
TEST_F(NativeWindowOcclusionTrackerTest, UpdatedLayeredAlphaWindow)
{
    ...
    ::UpdateLayeredWindow(hwnd, hdc.Get(), nullptr, nullptr, nullptr, nullptr, RGB(0xFF, 0xFF, 0xFF), &blend, ULW_OPAQUE);
