...
public:
    ...
    std::unique_ptr<display::Screen::ScreenSaverSuspender> SuspendScreenSaver() override;
    ...
    std::optional<float> GetPreferredScaleFactorForWindow(gfx::NativeWindow window) const override;

    // Returns the NativeWindow associated with the AcceleratedWidget.
    virtual gfx::NativeWindow GetNativeWindowFromAcceleratedWidget(gfx::AcceleratedWidget widget) const;

    static bool IsOzoneInitialized();

protected:
    ui::PlatformScreen* platform_screen()
    {
        return platform_screen_.get();
    }

private:
#if BUILDFLAG(IS_LINUX)
    class ScreenSaverSuspenderOzone : public display::Screen::ScreenSaverSuspender {
    public:
        explicit ScreenSaverSuspenderOzone(std::unique_ptr<ui::PlatformScreen::PlatformScreenSaverSuspender> suspender);

        ScreenSaverSuspenderOzone(const ScreenSaverSuspenderOzone&) = delete;
        ScreenSaverSuspenderOzone& operator=(const ScreenSaverSuspenderOzone&) = delete;

        ~ScreenSaverSuspenderOzone() override;

    private:
        std::unique_ptr<ui::PlatformScreen::PlatformScreenSaverSuspender> suspender_;
    };
#endif // BUILDFLAG(IS_LINUX)

    gfx::AcceleratedWidget GetAcceleratedWidgetForWindow(aura::Window* window) const;
