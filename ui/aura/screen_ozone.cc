...
ScreenOzone::ScreenOzone()
{
    ...
    if (platform_screen_) {
        ...
        NOTREACHED() << "PlatformScreen is not implemented for this ozone platform.";
    }
}
...
{
    DCHECK(platform_screen_);
    return GetNativeWindowFromAcceleratedWidget(platform_screen_->GetAcceleratedWidgetAtScreenPoint(point));
}

gfx::NativeWindow ScreenOzone::GetLocalProcessWindowAtPoint(const gfx::Point& point, const std::set<gfx::NativeWindow>& ignore)
{
    DCHECK(platform_screen_);
    ...
    return GetNativeWindowFromAcceleratedWidget(platform_screen_->GetLocalProcessWidgetAtPoint(point, ignore_top_level));
}
...
display::Display ScreenOzone::GetDisplayNearestWindow(gfx::NativeWindow window) const
{
    DCHECK(platform_screen_);
    gfx::AcceleratedWidget widget = GetAcceleratedWidgetForWindow(window);
    if (!widget)
...
display::Display ScreenOzone::GetDisplayNearestView(gfx::NativeView view) const
{
    return GetDisplayNearestWindow(view);
}

display::Display ScreenOzone::GetDisplayNearestPoint(const gfx::Point& point) const
{
    return platform_screen_->GetDisplayNearestPoint(point);
}

display::Display ScreenOzone::GetDisplayMatching(const gfx::Rect& match_rect) const
{
...
ScreenOzone::ScreenSaverSuspenderOzone::ScreenSaverSuspenderOzone(std::unique_ptr<ui::PlatformScreen::PlatformScreenSaverSuspender> suspender)
...
std::unique_ptr<display::Screen::ScreenSaverSuspender> ScreenOzone::SuspendScreenSaver()
{
    return std::make_unique<ScreenSaverSuspenderOzone>(platform_screen_->SuspendScreenSaver());
}
...
base::Value::List ScreenOzone::GetGpuExtraInfo(const gfx::GpuExtraInfo& gpu_extra_info)
{
    return platform_screen_->GetGpuExtraInfo(gpu_extra_info);
}

std::optional<float> ScreenOzone::GetPreferredScaleFactorForWindow(gfx::NativeWindow window) const
{
    return ui::OzonePlatform::GetInstance()->GetPlatformRuntimeProperties().supports_per_window_scaling
        ? platform_screen_->GetPreferredScaleFactorForAcceleratedWidget(GetAcceleratedWidgetForWindow(window))
        : Screen::GetPreferredScaleFactorForWindow(window);
}

gfx::NativeWindow ScreenOzone::GetNativeWindowFromAcceleratedWidget(gfx::AcceleratedWidget widget) const
{
    return nullptr;
}

gfx::AcceleratedWidget ScreenOzone::GetAcceleratedWidgetForWindow(aura::Window* window) const
{
    if (!window)
