...
WindowEventDispatcher::ObserverNotifier::ObserverNotifier(WindowEventDispatcher* dispatcher, const ui::Event& event)
    : dispatcher_(dispatcher)
{
    Env::GetInstance()->window_event_dispatcher_observers().Notify(&WindowEventDispatcherObserver::OnWindowEventDispatcherStartedProcessing, dispatcher, event);
}

WindowEventDispatcher::ObserverNotifier::~ObserverNotifier()
{
    Env::GetInstance()->window_event_dispatcher_observers().Notify(&WindowEventDispatcherObserver::OnWindowEventDispatcherFinishedProcessingEvent, dispatcher_);
}
...
{
    DCHECK(event->type() == ui::EventType::kMousePressed || event->type() == ui::EventType::kGestureTapDown || event->type() == ui::EventType::kTouchPressed);
    ...
    if (event->type() == ui::EventType::kMousePressed) {
        held_repostable_event_ = std::make_unique<ui::MouseEvent>(*event->AsMouseEvent(), static_cast<aura::Window*>(event->target()), window());
    } else if (event->type() == ui::EventType::kTouchPressed) {
        held_repostable_event_ = std::make_unique<ui::TouchEvent>(*event->AsTouchEvent());
    } else {
    ...
    if (held_repostable_event_) {
        base::SingleThreadTaskRunner::GetCurrentDefault()->PostNonNestableTask(
            FROM_HERE, base::BindOnce(base::IgnoreResult(&WindowEventDispatcher::DispatchHeldEvents), repost_event_factory_.GetWeakPtr()));
    }
}
...
void WindowEventDispatcher::DispatchGestureEvent(ui::GestureConsumer* raw_input_consumer, ui::GestureEvent* event)
{
...
DispatchDetails WindowEventDispatcher::DispatchMouseExitAtPoint(Window* window, const gfx::Point& point, int event_flags)
{
    ui::MouseEvent event(ui::EventType::kMouseExited, point, point, ui::EventTimeForNow(), event_flags, ui::EF_NONE);
    return DispatchMouseEnterOrExit(window, event, ui::EventType::kMouseExited);
}

void WindowEventDispatcher::ProcessedTouchEvent(uint32_t unique_event_id, Window* window, ui::EventResult result, bool is_source_touch_event_set_blocking)
{
    ui::GestureRecognizer::Gestures gestures
        = Env::GetInstance()->gesture_recognizer()->AckTouchEvent(unique_event_id, result, is_source_touch_event_set_blocking, window);
...
void WindowEventDispatcher::HoldPointerMoves()
{
    ...
    ++move_hold_count_;
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("ui", "WindowEventDispatcher::HoldPointerMoves", TRACE_ID_LOCAL(this));
}
...
void WindowEventDispatcher::ReleasePointerMoves()
{
    ...
    if (!move_hold_count_) {
        ...
        if (held_move_event_) {
            ...
            // executes.
            base::SingleThreadTaskRunner::GetCurrentDefault()->PostNonNestableTask(
                FROM_HERE, base::BindOnce(base::IgnoreResult(&WindowEventDispatcher::DispatchHeldEvents), held_event_factory_.GetWeakPtr()));
        } else {
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END0("ui", "WindowEventDispatcher::HoldPointerMoves", TRACE_ID_LOCAL(this));
}
...
void WindowEventDispatcher::OnCursorMovedToRootLocation(const gfx::Point& root_location)
{
    Env::GetInstance()->env_controller()->SetLastMouseLocation(window(), root_location);
...
void WindowEventDispatcher::TransformEventForDeviceScaleFactor(ui::LocatedEvent* event)
{
    event->UpdateForRootTransform(host_->GetInverseRootTransform(), host_->GetInverseRootTransformForLocalEventCoordinates());
}
...
ui::EventDispatchDetails WindowEventDispatcher::DispatchMouseEnterOrExit(Window* target, const ui::MouseEvent& event, ui::EventType type)
{
    Env::GetInstance()->env_controller()->UpdateStateForMouseEvent(window(), event);
    if (!mouse_moved_handler_ || !mouse_moved_handler_->HasTargetHandler() || !window()->Contains(mouse_moved_handler_))
    ...
    if (!target)
        target = window();
    ui::MouseEvent translated_event(event, target, mouse_moved_handler_.get(), type, event.flags() | ui::EF_IS_SYNTHESIZED);
    return DispatchEvent(mouse_moved_handler_, &translated_event);
}

ui::EventDispatchDetails WindowEventDispatcher::ProcessGestures(Window* target, ui::GestureRecognizer::Gestures gestures)
{
...
void WindowEventDispatcher::OnWindowHidden(Window* invisible, WindowHiddenReason reason)
{
    ...
    if (reason != WINDOW_MOVING) {
        ...
        Window* capture_window = capture_client ? capture_client->GetCaptureWindow() : nullptr;

        if (!invisible_tracker.Contains(invisible) || invisible->Contains(event_dispatch_target_)) {
        ...
        // after this.
        if (invisible_tracker.Contains(invisible) && invisible->Contains(capture_window) && invisible != window()) {
            capture_window->ReleaseCapture();
        }
    }
}

bool WindowEventDispatcher::is_dispatched_held_event(const ui::Event& event) const
{
...
void WindowEventDispatcher::UpdateCapture(Window* old_capture, Window* new_capture)
{
    ...
    if (old_capture && old_capture->GetRootWindow() == window() && old_capture->delegate()) {
        ...
        ui::MouseEvent event(ui::EventType::kMouseCaptureChanged, location, location, ui::EventTimeForNow(), 0, 0);
...
ui::EventDispatchDetails WindowEventDispatcher::PreDispatchEvent(ui::EventTarget* target, ui::Event* event)
{
...
ui::EventDispatchDetails WindowEventDispatcher::PostDispatchEvent(ui::EventTarget* target, const ui::Event& event)
{
    DispatchDetails details;
    ...
    if (event.IsTouchEvent() && !details.target_destroyed) {
        ...
        if (is_dispatched_held_event(event) || !held_move_event_ || !held_move_event_->IsTouchEvent()) {
            ...
            if (!touchevent.synchronous_handling_disabled()) {
                ...
                auto event_result = touchevent.force_process_gesture() ? ui::ER_UNHANDLED : event.result();
                ui::GestureRecognizer::Gestures gestures = Env::GetInstance()->gesture_recognizer()->AckTouchEvent(
                    touchevent.unique_event_id(), event_result, false /* is_source_touch_event_set_blocking */, window);
    ...
    if (!details.dispatcher_destroyed && host_->compositor() && cc::CustomMetricRecorder::Get()) {
        event_metrics_monitor_.reset();
    }
...
bool WindowEventDispatcher::CanDispatchToConsumer(ui::GestureConsumer* consumer)
{
...
void WindowEventDispatcher::DispatchSyntheticTouchEvent(ui::TouchEvent* event)
{
    ...
    DCHECK(event->type() == ui::EventType::kTouchCancelled || event->type() == ui::EventType::kTouchPressed);
    event->UpdateForRootTransform(host_->GetRootTransform(), host_->GetRootTransformForLocalEventCoordinates());
...
void WindowEventDispatcher::OnWindowRemovingFromRootWindow(Window* detached, Window* new_root)
{
...
void WindowEventDispatcher::OnWindowVisibilityChanging(Window* window, bool visible)
{
    if (!host_->window()->Contains(window))
        return;
...
void WindowEventDispatcher::OnWindowVisibilityChanged(Window* window, bool visible)
{
    if (!host_->window()->Contains(window))
        return;
...
void WindowEventDispatcher::OnWindowBoundsChanged(Window* window, const gfx::Rect& old_bounds, const gfx::Rect& new_bounds, ui::PropertyChangeReason reason)
{
    if (!host_->window()->Contains(window))
        return;

    if (window == host_->window()) {
        TRACE_EVENT1("ui", "WindowEventDispatcher::OnWindowBoundsChanged(root)", "size", new_bounds.size().ToString());
    ...
    if (window->IsVisible() && window->event_targeting_policy() != EventTargetingPolicy::kNone) {
        gfx::Rect old_bounds_in_root = old_bounds, new_bounds_in_root = new_bounds;
        Window::ConvertRectToTarget(window->parent(), host_->window(), &old_bounds_in_root);
        Window::ConvertRectToTarget(window->parent(), host_->window(), &new_bounds_in_root);
        gfx::Point last_mouse_location = GetLastMouseLocationInRoot();
        if ((old_bounds_in_root.Contains(last_mouse_location) != new_bounds_in_root.Contains(last_mouse_location))
            || (new_bounds_in_root.Contains(last_mouse_location) && new_bounds_in_root.origin() != old_bounds_in_root.origin())) {
            PostSynthesizeMouseMove(window);
        }
    }
}

void WindowEventDispatcher::OnWindowTargetTransformChanging(Window* window, const gfx::Transform& new_transform)
{
...
void WindowEventDispatcher::OnWindowTransformed(Window* window, ui::PropertyChangeReason reason)
{
    ...
    if (window_transforming_ && !synthesize_mouse_move_ && host_->window()->Contains(window)) {
        SynthesizeMouseMoveAfterChangeToWindow(window);
    }
...
ui::EventDispatchDetails WindowEventDispatcher::DispatchHeldEvents()
{
    ...
    if (held_repostable_event_) {
        if (held_repostable_event_->type() == ui::EventType::kMousePressed || held_repostable_event_->type() == ui::EventType::kTouchPressed) {
            std::unique_ptr<ui::LocatedEvent> event = std::move(held_repostable_event_);
    ...
    if (held_move_event_) {
        ...
        if (event->IsTouchEvent() || (event->IsMouseEvent() && !synthesize_mouse_move_)) {
            dispatching_held_event_ = event.get();
            dispatch_details = OnEventFromSource(event.get());
        }
    }
    ...
    if (!dispatch_details.dispatcher_destroyed) {
        ...
        Env::GetInstance()->window_event_dispatcher_observers().Notify(&WindowEventDispatcherObserver::OnWindowEventDispatcherDispatchedHeldEvents, this);
        if (did_dispatch_held_move_event_callback_)
            std::move(did_dispatch_held_move_event_callback_).Run();
    }
...
void WindowEventDispatcher::PostSynthesizeMouseMove(Window* window)
{
    ...
    gfx::Point cursor_point = display::Screen::GetScreen()->GetCursorScreenPoint();
    gfx::NativeWindow window_under_cursor = display::Screen::GetScreen()->GetWindowAtScreenPoint(cursor_point);
    ...
    // mouse move event.
    if (window->ContainsPointInRoot(cursor_point) && (!window_under_cursor || !host_->window()->Contains(window_under_cursor))) {
        return;
    }
#endif
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostNonNestableTask(
        FROM_HERE, base::BindOnce(base::IgnoreResult(&WindowEventDispatcher::SynthesizeMouseMoveEvent), held_event_factory_.GetWeakPtr()));
}

void WindowEventDispatcher::SynthesizeMouseMoveAfterChangeToWindow(Window* window)
{
    if (in_shutdown_)
        return;
...
ui::EventDispatchDetails WindowEventDispatcher::SynthesizeMouseMoveEvent()
{
    ...
    client::CursorClient* cursor_client = client::GetCursorClient(host_->window());
    if (cursor_client && (!cursor_client->IsMouseEventsEnabled() || (!cursor_client->IsCursorVisible() && !cursor_client->IsCursorLocked()))) {
        return details;
    }
    ...
    gfx::Point mouse_location = display::Screen::GetScreen()->GetCursorScreenPoint();
    ...
    ui::MouseEvent event(ui::EventType::kMouseMoved, host_mouse_location, host_mouse_location, ui::EventTimeForNow(), ui::EF_IS_SYNTHESIZED, 0);
    return OnEventFromSource(&event);
}

DispatchDetails WindowEventDispatcher::PreDispatchLocatedEvent(Window* target, ui::LocatedEvent* event)
{
...
DispatchDetails WindowEventDispatcher::PreDispatchMouseEvent(Window* target, ui::MouseEvent* event)
{
    ...
    // cleared.
    if (cursor_client && !cursor_client->IsMouseEventsEnabled() && (event->flags() & ui::EF_IS_SYNTHESIZED) && (event->type() != ui::EventType::kMouseExited)) {
        event->SetHandled();
        return DispatchDetails();
    }
    ...
    if (IsEventCandidateForHold(*event) && !dispatching_held_event_) {
        if (move_hold_count_) {
            held_move_event_ = std::make_unique<ui::MouseEvent>(*event, target, window());
            event->SetHandled();
            return DispatchDetails();
        } else {
    ...
    case ui::EventType::kMouseExited:
        if (!target || target == window()) {
            DispatchDetails details = DispatchMouseEnterOrExit(target, *event, ui::EventType::kMouseExited);
            if (details.dispatcher_destroyed) {
                event->SetHandled();
                return details;
            }
    ...
    case ui::EventType::kMouseMoved:
        ...
        if (target != mouse_moved_handler_) {
            ...
            details = DispatchMouseEnterOrExit(target, *event, ui::EventType::kMouseEntered);
            if (details.dispatcher_destroyed || details.target_destroyed) {
                event->SetHandled();
                return details;
            }
...
DispatchDetails WindowEventDispatcher::PreDispatchPinchEvent(Window* target, ui::GestureEvent* event)
{
...
DispatchDetails WindowEventDispatcher::PreDispatchTouchEvent(Window* target, ui::TouchEvent* event)
{
    if (event->type() == ui::EventType::kTouchMoved && move_hold_count_ && !dispatching_held_event_) {
        held_move_event_ = std::make_unique<ui::TouchEvent>(*event, target, window());
        event->SetHandled();
        return DispatchDetails();
    }

    Env::GetInstance()->env_controller()->UpdateStateForTouchEvent(target, *event);

    ui::TouchEvent root_relative_event(*event);
    root_relative_event.set_location_f(event->root_location_f());
    Env* env = Env::GetInstance();
    if (!env->gesture_recognizer()->ProcessTouchEventPreDispatch(&root_relative_event, target)) {
        // The event is invalid - ignore it.
        event->StopPropagation();
        event->DisableSynchronousHandling();
        env->window_event_dispatcher_observers().Notify(&WindowEventDispatcherObserver::OnWindowEventDispatcherIgnoredEvent, this);
        return DispatchDetails();
    }
...
DispatchDetails WindowEventDispatcher::PreDispatchKeyEvent(Window* target, ui::KeyEvent* event)
{
    if (skip_ime_ || !host_->has_input_method() || (event->flags() & ui::EF_IS_SYNTHESIZED) || !host_->ShouldSendKeyEventToIme()
...
std::unique_ptr<cc::EventsMetricsManager::ScopedMonitor> WindowEventDispatcher::CreateScropedMetricsMonitorForEvent(const ui::Event& event)
{
    ...
    if (event.IsScrollGestureEvent() || event.IsPinchEvent()) {
        ...
        ui::ScrollInputType input_type
            = gesture->details().device_type() == ui::GestureDeviceType::DEVICE_TOUCHSCREEN ? ui::ScrollInputType::kTouchscreen : ui::ScrollInputType::kWheel;
        if (gesture->type() == ui::EventType::kGestureScrollUpdate) {
            metrics = cc::ScrollUpdateEventMetrics::CreateForBrowser(ui::EventType::kGestureScrollUpdate, input_type,
                /*is_inertial=*/false,
                has_seen_gesture_scroll_update_after_begin_ ? cc::ScrollUpdateEventMetrics::ScrollUpdateType::kContinued
                                                            : cc::ScrollUpdateEventMetrics::ScrollUpdateType::kStarted,
                gesture->details().scroll_y(), gesture->time_stamp(), base::IdType64<class ui::LatencyInfo>(event.latency()->trace_id()));
            has_seen_gesture_scroll_update_after_begin_ = true;
        } else if (gesture->IsScrollGestureEvent()) {
            metrics = cc::ScrollEventMetrics::CreateForBrowser(gesture->type(), input_type,
                /*is_inertial=*/false, gesture->time_stamp(), base::IdType64<class ui::LatencyInfo>(event.latency()->trace_id()));
            ...
            metrics = cc::PinchEventMetrics::Create(
                gesture->type(), input_type, gesture->time_stamp(), base::IdType64<class ui::LatencyInfo>(event.latency()->trace_id()));
        }
    } else {
        metrics = cc::EventMetrics::Create(event.type(), event.time_stamp(), base::IdType64<class ui::LatencyInfo>(event.latency()->trace_id()));
    }
    ...
    if (metrics) {
        ...
        metrics->SetDispatchStageTimestamp(cc::EventMetrics::DispatchStage::kRendererMainStarted);
        done_callback = base::BindOnce(
            [](std::unique_ptr<cc::EventMetrics> metrics, bool handled) {
                ...
                metrics->SetDispatchStageTimestamp(cc::EventMetrics::DispatchStage::kRendererMainFinished);
                return handled ? std::move(metrics) : nullptr;
            },
            std::move(metrics));
    }
    return host_->compositor()->GetScopedEventMetricsMonitor(std::move(done_callback));
}
