...
public:
    ...
    FakeWindowOcclusionChangeBuilder(const FakeWindowOcclusionChangeBuilder&) = delete;
    FakeWindowOcclusionChangeBuilder& operator=(const FakeWindowOcclusionChangeBuilder&) = delete;
    ~FakeWindowOcclusionChangeBuilder() override = default;

    // WindowOcclusionChangeBuilder:
    void Add(Window* window, Window::OcclusionState occlusion_state, SkRegion occluded_region) override
    {
    }
};
...
public:
    MockWindowDelegate()
        : test::ColorTestWindowDelegate(SK_ColorWHITE)
    {
    }
    ...
    ~MockWindowDelegate() override
    {
        EXPECT_FALSE(is_expecting_call());
    }
    ...
    void set_expectation(Window::OcclusionState occlusion_state, SkRegion occluded_region = SkRegion())
    {
    ...
    void OnWindowOcclusionChanged(Window::OcclusionState old_occlusion_state, Window::OcclusionState new_occlusion_state) override
    {
...
private:
    Window::OcclusionState expected_occlusion_state_ = Window::OcclusionState::UNKNOWN;
...
public:
    ...
    WindowOcclusionTrackerTest& operator=(const WindowOcclusionTrackerTest&) = delete;
    ...
    Window* CreateTrackedWindow(MockWindowDelegate* delegate, const gfx::Rect& bounds, Window* parent = nullptr, bool transparent = false,
        ui::LayerType layer_type = ui::LAYER_TEXTURED, WindowOcclusionTracker* secondary_occlusion_tracker = nullptr)
    {
    ...
    Window* CreateUntrackedWindow(const gfx::Rect& bounds, Window* parent = nullptr, ui::LayerType layer_type = ui::LAYER_TEXTURED)
    {
        if (layer_type == ui::LAYER_TEXTURED) {
            return test::CreateTestWindow(SK_ColorWHITE, 1, bounds, parent ? parent : root_window());
        }
    ...
    std::unique_ptr<WindowOcclusionTracker> CreateSecondaryOcclusionTracker()
    {
        ...
        occlusion_tracker->set_occlusion_change_builder_factory(
            base::BindRepeating([]() -> std::unique_ptr<WindowOcclusionChangeBuilder> { return std::make_unique<FakeWindowOcclusionChangeBuilder>(); }));
...
TEST_F(WindowOcclusionTrackerTest, NonOverlappingWindows)
{
    ...
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(15, 0, 10, 10)));
...
TEST_F(WindowOcclusionTrackerTest, PartiallyOverlappingWindow)
{
    ...
    MockWindowDelegate* delegate_b = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeWH(5, 5)));
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
...
class WindowOcclusionTrackerOpacityTest : public WindowOcclusionTrackerTest, public testing::WithParamInterface<bool> {
public:
    WindowOcclusionTrackerOpacityTest() = default;
    ~WindowOcclusionTrackerOpacityTest() override = default;
    WindowOcclusionTrackerOpacityTest(const WindowOcclusionTrackerOpacityTest&) = delete;
    WindowOcclusionTrackerOpacityTest& operator=(const WindowOcclusionTrackerOpacityTest&) = delete;
...
TEST_P(WindowOcclusionTrackerOpacityTest, SemiTransparentWindowCoversWindow)
{
    ...
    delegate_a->set_expectation(Window::OcclusionState::OCCLUDED, SkRegion());
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_b = CreateTrackedWindow(delegate_b, gfx::Rect(0, 0, 15, 15), nullptr, false, layer_type());
    EXPECT_FALSE(delegate_a->is_expecting_call());
    EXPECT_FALSE(delegate_b->is_expecting_call());
...
// tracked.
TEST_P(WindowOcclusionTrackerOpacityTest, SemiTransparentUntrackedWindowCoversWindow)
{
    // Create window a. Expect it to be non-occluded.
    MockWindowDelegate* delegate_a = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    ...
    delegate_a->set_expectation(Window::OcclusionState::OCCLUDED, SkRegion());
    Window* window_b = CreateUntrackedWindow(gfx::Rect(0, 0, 15, 15), nullptr, layer_type());
    EXPECT_FALSE(delegate_a->is_expecting_call());
...
TEST_F(WindowOcclusionTrackerTest, TwoWindowsOccludeOneWindow)
{
    ...
    MockWindowDelegate* delegate_b = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeWH(5, 10)));
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    ...
    delegate_a->set_expectation(Window::OcclusionState::OCCLUDED, SkRegion());
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(5, 0, 5, 10)));
...
TEST_F(WindowOcclusionTrackerTest, ChildAndSiblingOccludeOneWindow)
{
    ...
    MockWindowDelegate* delegate_c = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(10, 0, 10, 20)));
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(10, 0, 10, 20)));
    delegate_c->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
...
TEST_F(WindowOcclusionTrackerTest, ChildDoesNotOccludeParent)
{
    ...
    MockWindowDelegate* delegate_b = new MockWindowDelegate();
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_b = CreateTrackedWindow(delegate_b, gfx::Rect(0, 0, 10, 10), window_a);
    EXPECT_FALSE(delegate_b->is_expecting_call());

    // Create window c whose bounds don't overlap existing windows.
    MockWindowDelegate* delegate_c = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(15, 0, 10, 10)));
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(15, 0, 10, 10)));
    delegate_c->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
...
TEST_F(WindowOcclusionTrackerTest, TransparentParentStackingChanged)
{
    ...
    MockWindowDelegate* delegate_a = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_a = CreateTrackedWindow(delegate_a, gfx::Rect(0, 0, 10, 10), root_window(), true);
    EXPECT_FALSE(delegate_a->is_expecting_call());
    ...
    MockWindowDelegate* delegate_c = new MockWindowDelegate();
    delegate_c->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_c = CreateTrackedWindow(delegate_c, gfx::Rect(0, 0, 10, 10), root_window(), true);
    EXPECT_FALSE(delegate_c->is_expecting_call());
...
TEST_F(WindowOcclusionTrackerTest, BoundsChanged)
{
    ...
    MockWindowDelegate* delegate_b = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(0, 10, 10, 10)));
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
...
TEST_F(WindowOcclusionTrackerTest, OccludedWindowBoundsAnimated)
{
    ...
    ui::LayerAnimatorTestController test_controller(ui::LayerAnimator::CreateImplicitAnimator());
    ui::ScopedLayerAnimationSettings layer_animation_settings(test_controller.animator());
    ...
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(0, 10, 10, 10)));
    test_controller.Step(kTransitionDuration / 3);
    EXPECT_FALSE(delegate_a->is_expecting_call());
    EXPECT_FALSE(delegate_b->is_expecting_call());
...
TEST_F(WindowOcclusionTrackerTest, NonOccludedWindowBoundsAnimated)
{
    ...
    delegate_a->set_expectation(Window::OcclusionState::OCCLUDED, SkRegion());
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(0, 0, 10, 10)));
    test_controller.Step(kTransitionDuration / 3);
    EXPECT_FALSE(delegate_a->is_expecting_call());
    EXPECT_FALSE(delegate_b->is_expecting_call());
...
TEST_F(WindowOcclusionTrackerTest, TransparentParentBoundsChanged)
{
    ...
    MockWindowDelegate* delegate_b = new MockWindowDelegate();
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_b = CreateTrackedWindow(delegate_b, gfx::Rect(0, 10, 10, 10), root_window(), true);
    EXPECT_FALSE(delegate_b->is_expecting_call());

    // Create window c which has window b as parent and doesn't occlude any
    // window.
    MockWindowDelegate* delegate_c = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(0, 10, 5, 5)));
    delegate_c->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
...
TEST_F(WindowOcclusionTrackerTest, OccludedWindowTransformAnimated)
{
    ...
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    auto transform = std::make_unique<ui::InterpolatedScale>(gfx::Point3F(1, 1, 1), gfx::Point3F(2.0f, 2.0f, 1));
    transform->SetChild(std::make_unique<ui::InterpolatedTranslation>(gfx::PointF(), gfx::PointF(-0.0f, -10.0f)));
    test_controller.animator()->StartAnimation(
        new ui::LayerAnimationSequence(ui::LayerAnimationElement::CreateInterpolatedTransformElement(std::move(transform), kTransitionDuration)));
    test_controller.Step(kTransitionDuration / 3);
    EXPECT_FALSE(delegate_b->is_expecting_call());
    ...
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(0, 10, 5, 5)));
    test_controller.Step(kTransitionDuration / 3);
    EXPECT_FALSE(delegate_a->is_expecting_call());
    EXPECT_FALSE(delegate_b->is_expecting_call());
...
TEST_F(WindowOcclusionTrackerTest, NonOccludedWindowTransformAnimated)
{
    ...
    MockWindowDelegate* delegate_b = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(0, 20, 10, 10)));
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    ...
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    auto transform = std::make_unique<ui::InterpolatedScale>(gfx::Point3F(1, 1, 1), gfx::Point3F(2.0f, 2.0f, 1));
    transform->SetChild(std::make_unique<ui::InterpolatedTranslation>(gfx::PointF(), gfx::PointF(-0.0f, -20.0f)));
    test_controller.animator()->StartAnimation(
        new ui::LayerAnimationSequence(ui::LayerAnimationElement::CreateInterpolatedTransformElement(std::move(transform), kTransitionDuration)));
    test_controller.Step(kTransitionDuration / 3);
    EXPECT_FALSE(delegate_b->is_expecting_call());
    ...
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeWH(20, 20)));
    test_controller.Step(kTransitionDuration / 3);
    EXPECT_FALSE(delegate_a->is_expecting_call());
    EXPECT_FALSE(delegate_b->is_expecting_call());
...
TEST_F(WindowOcclusionTrackerTest, RemoveAndAddTrackedToRoot)
{
    ...
    delegate_c->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(0, 10, 5, 5)));
...
TEST_F(WindowOcclusionTrackerTest, ResizeChildFromObserver)
{
    ...
    MockWindowDelegate* delegate_c = new MockWindowDelegate();
    delegate_c->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_c = CreateTrackedWindow(delegate_c, gfx::Rect(0, 0, 5, 5), window_b);
    EXPECT_FALSE(delegate_b->is_expecting_call());
...
TEST_F(WindowOcclusionTrackerTest, HierarchyOfTransforms)
{
    ...
    SkRegion occluded_area = SkRegionFromSkIRects({ SkIRect::MakeXYWH(2, 2, 10, 10), SkIRect::MakeXYWH(4, 4, 4, 4), SkIRect::MakeXYWH(34, 36, 8, 10) });
...
TEST_F(WindowOcclusionTrackerTest, DestroyWindowWithPendingAnimation)
{
    ...
    test_controller.Step(kTransitionDuration / 3);
    EXPECT_TRUE(test_controller.animator()->IsAnimatingProperty(ui::LayerAnimationElement::BOUNDS));
...
TEST_F(WindowOcclusionTrackerTest, DestroyOcclusionTrackerWithPendingAnimation)
{
    ...
    Window* window = CreateTrackedWindow(new MockWindowDelegate, gfx::Rect(0, 0, 10, 10), /*parent=*/nullptr,
    ...
    window->SetBounds(gfx::Rect(10, 10, 5, 5));
    test_controller.Step(kTransitionDuration / 3);
    ASSERT_TRUE(test_controller.animator()->IsAnimatingProperty(ui::LayerAnimationElement::BOUNDS));
...
public:
    ...
    ObserverChangingWindowBounds& operator=(const ObserverChangingWindowBounds&) = delete;
...
class ObserverDestroyingWindowOnAnimationEnded : public ui::LayerAnimationObserver {
public:
    ObserverDestroyingWindowOnAnimationEnded(Window* window)
        : window_(window)
    {
    }

    ObserverDestroyingWindowOnAnimationEnded(const ObserverDestroyingWindowOnAnimationEnded&) = delete;
    ObserverDestroyingWindowOnAnimationEnded& operator=(const ObserverDestroyingWindowOnAnimationEnded&) = delete;
...
TEST_P(WindowOcclusionTrackerOpacityTest, DestroyTrackedWindowFromLayerAnimationObserver)
{
    ui::ScopedAnimationDurationScaleMode scoped_animation_duration_scale_mode(ui::ScopedAnimationDurationScaleMode::NORMAL_DURATION);
    ui::LayerAnimatorTestController test_controller(ui::LayerAnimator::CreateImplicitAnimator());
    ui::ScopedLayerAnimationSettings layer_animation_settings(test_controller.animator());
    ...
    MockWindowDelegate* delegate = new MockWindowDelegate();
    delegate->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window = CreateTrackedWindow(delegate, gfx::Rect(0, 0, 10, 10), nullptr, false, layer_type());
    EXPECT_FALSE(delegate->is_expecting_call());
    window->layer()->SetAnimator(test_controller.animator());
...
// window and deleted.
TEST_P(WindowOcclusionTrackerOpacityTest, DeleteNonTrackedAnimatedWindowRemovedFromTrackedRoot)
{
    ui::ScopedAnimationDurationScaleMode scoped_animation_duration_scale_mode(ui::ScopedAnimationDurationScaleMode::NORMAL_DURATION);
    ui::LayerAnimatorTestController test_controller(ui::LayerAnimator::CreateImplicitAnimator());
    ui::ScopedLayerAnimationSettings layer_animation_settings(test_controller.animator());
    ...
    delegate->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(10, 0, 10, 10)));
    Window* window = CreateUntrackedWindow(gfx::Rect(10, 0, 10, 10), nullptr, layer_type());
    EXPECT_FALSE(delegate->is_expecting_call());
...
TEST_P(WindowOcclusionTrackerOpacityTest, OpacityAnimationShouldNotOccludeWindow)
{
    ui::ScopedAnimationDurationScaleMode scoped_animation_duration_scale_mode(ui::ScopedAnimationDurationScaleMode::NORMAL_DURATION);
    ui::LayerAnimatorTestController test_controller(ui::LayerAnimator::CreateImplicitAnimator());
    ui::ScopedLayerAnimationSettings layer_animation_settings(test_controller.animator());
    ...
    Window* window = CreateUntrackedWindow(gfx::Rect(0, 0, 10, 10), nullptr, layer_type());
    window->SetName("B");
    EXPECT_FALSE(delegate->is_expecting_call());
...
public:
    ...
    WindowDelegateHidingWindowIfOccluded(const WindowDelegateHidingWindowIfOccluded&) = delete;
    WindowDelegateHidingWindowIfOccluded& operator=(const WindowDelegateHidingWindowIfOccluded&) = delete;

    // MockWindowDelegate:
    void OnWindowOcclusionChanged(Window::OcclusionState old_occlusion_state, Window::OcclusionState new_occlusion_state) override
    {
        MockWindowDelegate::OnWindowOcclusionChanged(old_occlusion_state, new_occlusion_state);
...
public:
    ...
    WindowDelegateWithQueuedExpectation(const WindowDelegateWithQueuedExpectation&) = delete;
    WindowDelegateWithQueuedExpectation& operator=(const WindowDelegateWithQueuedExpectation&) = delete;

    void set_queued_expectation(Window::OcclusionState occlusion_state, const SkRegion& occluded_region)
    {
    ...
    void OnWindowOcclusionChanged(Window::OcclusionState old_occlusion_state, Window::OcclusionState new_occlusion_state) override
    {
        ...
        if (queued_expected_occlusion_state_ != Window::OcclusionState::UNKNOWN) {
            set_expectation(queued_expected_occlusion_state_, queued_expected_occluded_region_);
...
private:
    Window::OcclusionState queued_expected_occlusion_state_ = Window::OcclusionState::UNKNOWN;
...
TEST_F(WindowOcclusionTrackerTest, HideFromOnWindowOcclusionChanged)
{
    ...
    WindowDelegateWithQueuedExpectation* delegate_a = new WindowDelegateWithQueuedExpectation();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_a = CreateTrackedWindow(delegate_a, gfx::Rect(0, 0, 10, 10));
    EXPECT_FALSE(delegate_a->is_expecting_call());

    // Create a tracked window. Expect it to be visible.
    MockWindowDelegate* delegate_b = new WindowDelegateHidingWindowIfOccluded(window_a);
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(10, 0, 10, 10)));
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    ...
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    delegate_a->set_queued_expectation(Window::OcclusionState::HIDDEN, SkRegion());
    delegate_b->set_expectation(Window::OcclusionState::HIDDEN, SkRegion());
    window_b->Hide();
...
public:
    ...
    WindowDelegateDeletingWindow& operator=(const WindowDelegateDeletingWindow&) = delete;

    void set_other_window(Window* other_window)
    {
        other_window_ = other_window;
    }
...
TEST_F(WindowOcclusionTrackerTest, DeleteFromOnWindowOcclusionChanged)
{
    ...
    // Create a tracked window. Expect it to be visible.
    MockWindowDelegate* delegate_c = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegionFromSkIRects({ SkIRect::MakeXYWH(10, 0, 10, 10), SkIRect::MakeXYWH(20, 0, 10, 10) }));
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(20, 0, 10, 10)));
    delegate_c->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
...
public:
    ...
    WindowDelegateChangingWindowVisibility(const WindowDelegateChangingWindowVisibility&) = delete;
    WindowDelegateChangingWindowVisibility& operator=(const WindowDelegateChangingWindowVisibility&) = delete;

    void set_window_to_update(Window* window)
    {
        window_to_update_ = window;
    }
...
{
    test::WindowOcclusionTrackerTestApi test_api(Env::GetInstance()->GetWindowOcclusionTracker());
    ...
    MockWindowDelegate* delegate_c = new MockWindowDelegate();
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(10, 0, 10, 10)));
    delegate_c->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    ...
    // VISIBLE.
    EXPECT_DCHECK_DEATH({
        delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(20, 0, 10, 10)));
...
TEST_F(WindowOcclusionTrackerTest, HideTreeBranch)
{
    ...
    MockWindowDelegate* delegate_b = new MockWindowDelegate();
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_b = CreateTrackedWindow(delegate_b, gfx::Rect(0, 10, 10, 10), window_a);
    EXPECT_FALSE(delegate_b->is_expecting_call());
...
TEST_F(WindowOcclusionTrackerTest, WindowWithParentAlphaShape)
{
    ...
    MockWindowDelegate* delegate_b = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeWH(10, 10)));
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_b = CreateTrackedWindow(delegate_b, gfx::Rect(0, 0, 10, 10));
    EXPECT_FALSE(delegate_a->is_expecting_call());
    EXPECT_FALSE(delegate_b->is_expecting_call());
...
public:
    ...
    WindowDelegateHidingWindow& operator=(const WindowDelegateHidingWindow&) = delete;
...
public:
    ...
    WindowDelegateAddingAndHidingChild(const WindowDelegateAddingAndHidingChild&) = delete;
    WindowDelegateAddingAndHidingChild& operator=(const WindowDelegateAddingAndHidingChild&) = delete;
    ...
    void OnWindowOcclusionChanged(Window::OcclusionState old_occlusion_state, Window::OcclusionState new_occlusion_state) override
    {
        ...
        Window* window = test_->CreateUntrackedWindow(gfx::Rect(0, 0, 5, 5), window_to_update_);
        window->Hide();
    }
...
// to be recomputed.
TEST_F(WindowOcclusionTrackerTest, HideWindowWithHiddenParentOnOcclusionChange)
{
    test::WindowOcclusionTrackerTestApi test_api(Env::GetInstance()->GetWindowOcclusionTracker());
...
TEST_F(WindowOcclusionTrackerTest, HideWindowChangesOcclusionRegionToBeFullyOccluded)
{
    MockWindowDelegate* delegate_a = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_a = CreateTrackedWindow(delegate_a, gfx::Rect(0, 0, 10, 10));
...
TEST_F(WindowOcclusionTrackerTest, WindowOccludesWindowPartially)
{
    ...
    MockWindowDelegate* delegate_b = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(6, 7, 8, 9)));
    delegate_b->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
...
TEST_F(WindowOcclusionTrackerTest, WindowWithAlphaShapeDoesNotPartiallyOccludeOtherWindows)
{
    // Create window a. Expect it to be non-occluded.
    MockWindowDelegate* delegate_a = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
...
TEST_F(WindowOcclusionTrackerTest, WindowCanBeOccludedByMultipleWindows)
{
    ...
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, window_a_occlusion);
    ...
    window_a_occlusion.op(SkIRect::MakeXYWH(14, 1, 5, 5), SkRegion::Op::kUnion_Op);
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, window_a_occlusion);
    CreateUntrackedWindow(gfx::Rect(14, 1, 5, 5));
    EXPECT_FALSE(delegate_a->is_expecting_call());

    window_a_occlusion.op(SkIRect::MakeXYWH(10, 10, 2, 3), SkRegion::Op::kUnion_Op);
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, window_a_occlusion);
...
TEST_P(WindowOcclusionTrackerOpacityTest, ExcludeWindow)
{
...

    {
        ...
        window_bb_occlusion.op(SkIRect::MakeXYWH(100, 100, 100, 100), SkRegion::kUnion_Op);
        delegate_bb->set_expectation(Window::OcclusionState::VISIBLE, window_bb_occlusion);
...

    {
        ...
        window_a_occlusion.op(SkIRect::MakeXYWH(5, 5, 100, 100), SkRegion::kUnion_Op);
        window_a_occlusion.op(SkIRect::MakeXYWH(100, 100, 100, 100), SkRegion::kUnion_Op);
        delegate_a->set_expectation(Window::OcclusionState::VISIBLE, window_a_occlusion);
    ...
    auto* window_d = CreateTrackedWindow(delegate_d, gfx::Rect(0, 0, 10, 10), nullptr, false, layer_type());
...
TEST_F(WindowOcclusionTrackerTest, ScopedForceVisibleWithOccludedSibling)
{
    ...
    // the same bounds.
    std::unique_ptr<WindowOcclusionTracker::ScopedPause> pause_occlusion_tracking = std::make_unique<WindowOcclusionTracker::ScopedPause>();
    MockWindowDelegate* parent_delegate = new MockWindowDelegate();
    Window* parent_window = CreateTrackedWindow(parent_delegate, gfx::Rect(0, 0, 10, 10));
    EXPECT_FALSE(parent_delegate->is_expecting_call());
    MockWindowDelegate* occluded_child_delegate = new MockWindowDelegate();
    CreateTrackedWindow(occluded_child_delegate, gfx::Rect(0, 0, 10, 10), parent_window);
    EXPECT_FALSE(occluded_child_delegate->is_expecting_call());
    MockWindowDelegate* visible_child_delegate = new MockWindowDelegate();
    CreateTrackedWindow(visible_child_delegate, gfx::Rect(0, 0, 10, 10), parent_window);
...
TEST_F(WindowOcclusionTrackerTest, ScopedForceVisibleHiddenContainer)
{
    ...
    Window* container = CreateTrackedWindow(container_delegate, gfx::Rect(0, 0, 100, 100), root);
    MockWindowDelegate* browser_delegate = new MockWindowDelegate();
    Window* browser = CreateTrackedWindow(browser_delegate, gfx::Rect(0, 0, 10, 10), container);
    MockWindowDelegate* webcontents_delegate = new MockWindowDelegate();
    Window* webcontents = CreateTrackedWindow(webcontents_delegate, gfx::Rect(0, 0, 10, 10), browser);
...
TEST_F(WindowOcclusionTrackerTest, ComputeTargetOcclusionForWindow)
{
    ...
    SkRegion window_a_occlusion = SkRegionFromSkIRects(
        { SkIRect::MakeXYWH(14, 14, 5, 5), SkIRect::MakeXYWH(1, 1, 5, 5), SkIRect::MakeXYWH(14, 1, 5, 5), SkIRect::MakeXYWH(10, 10, 2, 3) });

    auto& occlusion_tracker = GetOcclusionTracker();
    window_a->TrackOcclusionState();
    auto occlusion_data = occlusion_tracker.ComputeTargetOcclusionForWindow(window_a);
...
TEST_F(WindowOcclusionTrackerTest, ComputeTargetOcclusionForWindowUsesTargetBounds)
{
    MockWindowDelegate* delegate_a = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_a = CreateTrackedWindow(delegate_a, gfx::Rect(0, 0, 10, 10));
    ...
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(10, 10, 10, 10)));
    ...
    auto occlusion_data = occlusion_tracker.ComputeTargetOcclusionForWindow(window_a);
    EXPECT_EQ(Window::OcclusionState::VISIBLE, occlusion_data.occlusion_state);
    EXPECT_EQ(SkRegion(SkIRect::MakeXYWH(10, 10, 10, 10)), occlusion_data.occluded_region);
...
TEST_P(WindowOcclusionTrackerOpacityTest, ComputeTargetOcclusionForWindowUsesTargetOpacity)
{
    MockWindowDelegate* delegate_a = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_a = CreateTrackedWindow(delegate_a, gfx::Rect(0, 0, 10, 10));
    ...
    delegate_a->set_expectation(Window::OcclusionState::OCCLUDED, SkRegion());
    Window* window_b = CreateUntrackedWindow(gfx::Rect(0, 0, 10, 10), nullptr, layer_type());
    EXPECT_FALSE(delegate_a->is_expecting_call());
...
TEST_F(WindowOcclusionTrackerTest, ComputeTargetOcclusionForWindowUsesTargetVisibility)
{
    MockWindowDelegate* delegate_a = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    Window* window_a = CreateTrackedWindow(delegate_a, gfx::Rect(0, 0, 10, 10));
...
TEST_F(WindowOcclusionTrackerTest, ComputeTargetOcclusionForWindowTransformHierarchy)
{
    gfx::Transform scale_2x_transform;
    scale_2x_transform.Scale(2.0f, 2.0f);
    ...
    auto occlusion_data = occlusion_tracker.ComputeTargetOcclusionForWindow(window_d);
    EXPECT_EQ(Window::OcclusionState::VISIBLE, occlusion_data.occlusion_state);
    EXPECT_EQ(SkRegion(), occlusion_data.occluded_region);
    ...
    // Target occlusion should include them, however:
    SkRegion occluded_area_transformed
        = SkRegionFromSkIRects({ SkIRect::MakeXYWH(2, 2, 10, 10), SkIRect::MakeXYWH(4, 4, 8, 8), SkIRect::MakeXYWH(64, 68, 16, 20) });
    occlusion_data = occlusion_tracker.ComputeTargetOcclusionForWindow(window_d);
    EXPECT_EQ(Window::OcclusionState::VISIBLE, occlusion_data.occlusion_state);
...
TEST_F(WindowOcclusionTrackerTest, SetOpaqueRegionsForOcclusionAffectsOcclusionOfOtherWindows)
{
    MockWindowDelegate* delegate_a = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
...
TEST_F(WindowOcclusionTrackerTest, SetOpaqueRegionsForOcclusionOfAWindowDoesNotAffectOcclusionOfThatWindowItself)
{
    ...
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(5, 5, 5, 5)));
...
TEST_F(WindowOcclusionTrackerTest, SemiOpaqueSolidColorLayerDoesNotAffectChildOpacity)
{
    // Create window a. Expect it to be non-occluded.
    MockWindowDelegate* delegate_a = new MockWindowDelegate();
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion());
    ...
    // occluded.
    delegate_a->set_expectation(Window::OcclusionState::OCCLUDED, SkRegion());
    Window* window_b = CreateUntrackedWindow(gfx::Rect(0, 0, 15, 15), nullptr, ui::LAYER_SOLID_COLOR);
    EXPECT_FALSE(delegate_a->is_expecting_call());
...
TEST_F(WindowOcclusionTrackerTest, OccludedFractionalWindow)
{
    ...
    // window.
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(0, 0, 6, 6)));
...
TEST_F(WindowOcclusionTrackerTest, OccludingFractionalWindow)
{
    ...
    delegate_a->set_expectation(Window::OcclusionState::VISIBLE, SkRegion(SkIRect::MakeXYWH(0, 0, 11, 11)));
    window_a->SetBounds(gfx::Rect(0, 0, 12, 12));
    EXPECT_FALSE(delegate_a->is_expecting_call());
}
...
INSTANTIATE_TEST_SUITE_P(All, WindowOcclusionTrackerOpacityTest, testing::Bool());
