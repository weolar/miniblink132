...
public:
    ...
    virtual bool SubtreeShouldBeExploredForEvent(Window* window, const ui::LocatedEvent& event);
    ...
    virtual bool GetHitTestRects(Window* target, gfx::Rect* hit_test_rect_mouse, gfx::Rect* hit_test_rect_touch) const;
    ...
    virtual std::unique_ptr<HitTestRects> GetExtraHitTestShapeRects(Window* target) const;
    ...
    void SetInsets(const gfx::Insets& mouse_extend, const gfx::Insets& touch_extend);

    // If there is a target that takes priority over normal WindowTargeter (such
    // as a capture window) this returns it.
    Window* GetPriorityTargetInRootWindow(Window* root_window, const ui::LocatedEvent& event);

    Window* FindTargetInRootWindow(Window* root_window, const ui::LocatedEvent& event);

    // ui::EventTargeter:
    ui::EventTarget* FindTargetForEvent(ui::EventTarget* root, ui::Event* event) override;
    ui::EventTarget* FindNextBestTarget(ui::EventTarget* previous_target, ui::Event* event) override;
    ui::EventSink* GetNewEventSinkForEvent(const ui::EventTarget* current_root, ui::EventTarget* target, ui::Event* in_out_event) override;
...
protected:
    ...
    virtual Window* FindTargetForLocatedEvent(Window* window, ui::LocatedEvent* event);
    ...
    virtual bool SubtreeCanAcceptEvent(Window* window, const ui::LocatedEvent& event) const;
    ...
    virtual bool EventLocationInsideBounds(Window* target, const ui::LocatedEvent& event) const;
    ...
    const gfx::Insets& mouse_extend() const
    {
        return mouse_extend_;
    }
    const gfx::Insets& touch_extend() const
    {
        return touch_extend_;
    }

    static gfx::Point ConvertEventLocationToWindowCoordinates(Window* window, const ui::LocatedEvent& event);
...
private:
    ...
    Window* FindTargetForLocatedEventRecursively(Window* root_window, ui::LocatedEvent* event);

    // Whether |point| is inside |window| or its descendents using |bounds_type|
    // as a rect source. |point| should be relative to |window|.
    bool PointInsideBounds(Window* window, BoundsType bounds_type, const gfx::Point& point) const;
