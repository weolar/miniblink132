...
{
    constexpr gfx::Rect starting_bounds(aura::test::AuraTestHelper::kDefaultHostSize);
...
TEST_F(WindowTreeHostTest, ShouldHaveExactRootWindowBoundsWithDisplayRotation1xScale)
{
    ...
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().rotation(), display::Display::ROTATE_0);
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().GetSizeInPixel(), gfx::Size(400, 300));
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().bounds(), gfx::Rect(0, 0, 400, 300));
    EXPECT_EQ(gfx::Rect(400, 300), host()->window()->bounds());

    host()->SetBoundsInPixels(gfx::Rect(0, 0, 400, 300));
    test_screen()->SetDisplayRotation(display::Display::ROTATE_90);
    EXPECT_EQ(host()->GetBoundsInPixels(), gfx::Rect(0, 0, 400, 300));
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().rotation(), display::Display::ROTATE_90);
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().GetSizeInPixel(), gfx::Size(300, 400));
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().bounds(), gfx::Rect(0, 0, 300, 400));
    EXPECT_EQ(gfx::Rect(300, 400), host()->window()->bounds());

    host()->SetBoundsInPixels(gfx::Rect(0, 0, 400, 300));
    test_screen()->SetDisplayRotation(display::Display::ROTATE_180);
    EXPECT_EQ(host()->GetBoundsInPixels(), gfx::Rect(0, 0, 400, 300));
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().rotation(), display::Display::ROTATE_180);
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().GetSizeInPixel(), gfx::Size(400, 300));
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().bounds(), gfx::Rect(0, 0, 400, 300));
    EXPECT_EQ(gfx::Rect(400, 300), host()->window()->bounds());
    ...
    EXPECT_EQ(host()->GetBoundsInPixels(), gfx::Rect(0, 0, 400, 300));
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().rotation(), display::Display::ROTATE_270);
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().GetSizeInPixel(), gfx::Size(300, 400));
    EXPECT_EQ(display::Screen::GetScreen()->GetPrimaryDisplay().bounds(), gfx::Rect(0, 0, 300, 400));
    EXPECT_EQ(gfx::Rect(300, 400), host()->window()->bounds());
}
...
TEST_F(WindowTreeHostTest, ShouldHandleTextScale)
{
    ...
    auto asserter = [&](float test_scale_factor) {
        ...
        EXPECT_EQ(starting_bounds, host()->GetBoundsInPixels());
        // Size should be rounded up after scaling.
        EXPECT_EQ(gfx::ScaleToEnclosingRect(starting_bounds, 1.0f / test_scale_factor), root_window()->bounds());
...
TEST_F(WindowTreeHostTest, NoRewritesPostIME)
{
    ...
    ui::KeyEvent key_event = ui::KeyEvent::FromCharacter('A', ui::VKEY_A, ui::DomCode::NONE, 0);
    ui::EventDispatchDetails details = host()->GetInputMethod()->DispatchKeyEvent(&key_event);
...
TEST_F(WindowTreeHostTest, ConvertDIPToPixelsShouldRespectScaleFactor)
{
    ...
    EXPECT_EQ(ConvertDIPToPixels(host(), gfx::Point(width_in_dip, 0)), gfx::Point(width_in_pixels, 0));
    EXPECT_EQ(ConvertDIPToPixels(host(), gfx::Point(0, height_in_dip)), gfx::Point(0, height_in_pixels));
}
...
TEST_F(WindowTreeHostTest, ConvertDIPToPixelsShouldRespectRotation)
{
    ...
    EXPECT_EQ(ConvertDIPToPixels(host(), gfx::Point(0, 0)), gfx::Point(width_in_pixels, 0));
    EXPECT_EQ(ConvertDIPToPixels(host(), gfx::Point(width_in_dip, 0)), gfx::Point(width_in_pixels, height_in_pixels));
    EXPECT_EQ(ConvertDIPToPixels(host(), gfx::Point(width_in_dip, height_in_dip)), gfx::Point(0, height_in_pixels));
    EXPECT_EQ(ConvertDIPToPixels(host(), gfx::Point(0, height_in_dip)), gfx::Point(0, 0));
}
...
TEST_F(WindowTreeHostTest, ConvertDIPToPixelsShouldWorkWithPointF)
{
    ...
    EXPECT_EQ(ConvertDIPToPixels(host(), gfx::PointF(5.3f, 0)), gfx::PointF(10.6f, 0));
}
...
TEST_F(WindowTreeHostTest, ConvertPixelsToDIPShouldRespectScaleFactor)
{
    ...
    EXPECT_EQ(ConvertPixelsToDIP(host(), gfx::Point(width_in_pixels, 0)), gfx::Point(width_in_dip, 0));
    EXPECT_EQ(ConvertPixelsToDIP(host(), gfx::Point(0, height_in_pixels)), gfx::Point(0, height_in_dip));
}
...
TEST_F(WindowTreeHostTest, ConvertPixelsToDIPShouldRespectRotation)
{
    ...
    EXPECT_EQ(ConvertPixelsToDIP(host(), gfx::Point(0, 0)), gfx::Point(0, height_in_dip));
    EXPECT_EQ(ConvertPixelsToDIP(host(), gfx::Point(width_in_pixels, 0)), gfx::Point(0, 0));
    EXPECT_EQ(ConvertPixelsToDIP(host(), gfx::Point(width_in_pixels, height_in_pixels)), gfx::Point(width_in_dip, 0));
    EXPECT_EQ(ConvertPixelsToDIP(host(), gfx::Point(0, height_in_pixels)), gfx::Point(width_in_dip, height_in_dip));
}
...
TEST_F(WindowTreeHostTest, ConvertPixelsToDIPShouldWorkWithPointF)
{
    ...
    EXPECT_EQ(ConvertPixelsToDIP(host(), gfx::PointF(10.6f, 0)), gfx::PointF(5.3f, 0));
}
...
TEST_F(WindowTreeHostTest, LostCaptureDuringTearDown)
{
    ...
    scoped_feature_list.InitAndDisableFeature(features::kApplyNativeOcclusionToCompositor);
#endif
        ...
        scoped_feature_list_.InitWithFeaturesAndParameters(
            { { features::kCalculateNativeWinOcclusion, {} },
                { features::kApplyNativeOcclusionToCompositor,
                    { { features::kApplyNativeOcclusionToCompositorType.name, features::kApplyNativeOcclusionToCompositorTypeRelease } } },
...
TEST_F(WindowTreeHostWithReleaseTest, ToggleOccluded)
{
    ...
    ASSERT_TRUE(NativeWindowOcclusionTracker::IsNativeWindowOcclusionTrackingAlwaysEnabled(host()));
...
TEST_F(WindowTreeHostWithReleaseTest, VideoCaptureLockForcesVisible)
{
    ...
    host()->SetNativeWindowOcclusionState(Window::OcclusionState::OCCLUDED, {});
    EXPECT_FALSE(host()->compositor()->IsVisible());
    std::unique_ptr<WindowTreeHost::VideoCaptureLock> lock = host()->CreateVideoCaptureLock();
    EXPECT_TRUE(host()->compositor()->IsVisible());
    host()->SetNativeWindowOcclusionState(Window::OcclusionState::VISIBLE, {});
    EXPECT_TRUE(host()->compositor()->IsVisible());
    host()->SetNativeWindowOcclusionState(Window::OcclusionState::OCCLUDED, {});
...
TEST_F(WindowTreeHostWithReleaseTest, VideoCaptureLockAffectsOcclusionState)
{
    ...
    host()->Show();
    host()->SetNativeWindowOcclusionState(Window::OcclusionState::OCCLUDED, {});
    EXPECT_EQ(Window::OcclusionState::OCCLUDED, host()->GetNativeWindowOcclusionState());
    EXPECT_FALSE(host()->compositor()->IsVisible());

    std::unique_ptr<WindowTreeHost::VideoCaptureLock> lock = host()->CreateVideoCaptureLock();
    // VideoCaptureLock should force this to visible.
    EXPECT_EQ(Window::OcclusionState::VISIBLE, host()->GetNativeWindowOcclusionState());
    EXPECT_TRUE(host()->compositor()->IsVisible());
    lock.reset();
    ...
    host()->SetNativeWindowOcclusionState(Window::OcclusionState::HIDDEN, {});
    EXPECT_EQ(Window::OcclusionState::HIDDEN, host()->GetNativeWindowOcclusionState());
    EXPECT_FALSE(host()->compositor()->IsVisible());
        ...
        scoped_feature_list_.InitWithFeaturesAndParameters(
            {
                { features::kCalculateNativeWinOcclusion, {} },
                { features::kApplyNativeOcclusionToCompositor,
                    { { features::kApplyNativeOcclusionToCompositorType.name, features::kApplyNativeOcclusionToCompositorTypeThrottle } } },
                // Disable the headless check as the bots run with CHROME_HEADLESS
                // set.
                { features::kAlwaysTrackNativeWindowOcclusionForTest, {} },
            },
        ...
        scoped_feature_list_.InitWithFeaturesAndParameters(
            {
                { features::kCalculateNativeWinOcclusion, {} },
                { features::kApplyNativeOcclusionToCompositor,
                    { { features::kApplyNativeOcclusionToCompositorType.name, features::kApplyNativeOcclusionToCompositorTypeThrottleAndRelease } } },
                // Disable the headless check as the bots run with CHROME_HEADLESS
                // set.
                { features::kAlwaysTrackNativeWindowOcclusionForTest, {} },
            },
...
TEST_F(WindowTreeHostWithThrottleAndReleaseTest, VideoCaptureLockForcesVisible)
{
    ASSERT_TRUE(NativeWindowOcclusionTracker::IsNativeWindowOcclusionTrackingAlwaysEnabled(host()));
    // This test needs to drive native occlusion. If native occlusion is
    // used, it'll conflict with this test.
...
TEST_F(WindowTreeHostWithThrottleAndReleaseTest, VideoCaptureLockAffectsOcclusionState)
{
    ASSERT_TRUE(NativeWindowOcclusionTracker::IsNativeWindowOcclusionTrackingAlwaysEnabled(host()));
    // This test needs to drive native occlusion. If native occlusion is
    // used, it'll conflict with this test.
