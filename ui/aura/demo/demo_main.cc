...
public:
    explicit DemoWindowParentingClient(aura::Window* window)
        : window_(window)
    {
    ...
    DemoWindowParentingClient& operator=(const DemoWindowParentingClient&) = delete;
    ...
    {
        if (!capture_client_) {
            capture_client_ = std::make_unique<aura::client::DefaultCaptureClient>(window_->GetRootWindow());
        }
        return window_;
    }
...
int DemoMain()
{
    ...
    viz::FrameSinkManagerImpl frame_sink_manager { viz::FrameSinkManagerImpl::InitParams(&server_shared_bitmap_manager) };
    host_frame_sink_manager.SetLocalManager(&frame_sink_manager);
    frame_sink_manager.SetLocalClient(&host_frame_sink_manager);
    auto context_factory = std::make_unique<ui::InProcessContextFactory>(&host_frame_sink_manager, &frame_sink_manager, /*output_to_window=*/true);

    base::PowerMonitor::GetInstance()->Initialize(std::make_unique<base::PowerMonitorDeviceSource>());

    std::unique_ptr<aura::Env> env = aura::Env::CreateInstance();
    env->set_context_factory(context_factory.get());
    std::unique_ptr<aura::TestScreen> test_screen(aura::TestScreen::Create(gfx::Size()));
    display::Screen::SetScreenInstance(test_screen.get());
    std::unique_ptr<aura::WindowTreeHost> host(test_screen->CreateHostForPrimaryDisplay());
    ...
    window1.Show();
    aura::client::ParentWindowWithContext(&window1, host->window(), gfx::Rect(), display::kInvalidDisplayId);
    ...
    window2.Show();
    aura::client::ParentWindowWithContext(&window2, host->window(), gfx::Rect(), display::kInvalidDisplayId);
