...
TEST(TargetValueTest, VerifyLayerAnimationDelegateConstructor)
{
    ...
    const auto kTransform = gfx::Transform::Affine(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f);
    ...
    delegate.SetBoundsFromAnimation(kBounds, PropertyChangeReason::NOT_FROM_ANIMATION);
    delegate.SetTransformFromAnimation(kTransform, PropertyChangeReason::NOT_FROM_ANIMATION);
    delegate.SetOpacityFromAnimation(kOpacity, PropertyChangeReason::NOT_FROM_ANIMATION);
    delegate.SetVisibilityFromAnimation(kVisibility, PropertyChangeReason::NOT_FROM_ANIMATION);
    delegate.SetBrightnessFromAnimation(kBrightness, PropertyChangeReason::NOT_FROM_ANIMATION);
    delegate.SetGrayscaleFromAnimation(kGrayscale, PropertyChangeReason::NOT_FROM_ANIMATION);
    delegate.SetColorFromAnimation(kColor, PropertyChangeReason::NOT_FROM_ANIMATION);
    delegate.SetClipRectFromAnimation(kClipRect, PropertyChangeReason::NOT_FROM_ANIMATION);
    delegate.SetRoundedCornersFromAnimation(kRoundedCorners, PropertyChangeReason::NOT_FROM_ANIMATION);
...
TEST(LayerAnimationElementTest, TransformElement)
{
    ...
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateTransformElement(target_transform, delta);
    element->set_animation_group_id(1);

    for (int i = 0; i < 2; ++i) {
        start_time = effective_start_time + delta;
        element->set_requested_start_time(start_time);
        delegate.SetTransformFromAnimation(start_transform, PropertyChangeReason::NOT_FROM_ANIMATION);
        ...
        EXPECT_TRUE(element->IsFinished(effective_start_time + delta, &element_duration));
        ...
        CheckApproximatelyEqual(target_transform, delegate.GetTransformForAnimation());
        delegate.ExpectLastPropertyChangeReason(PropertyChangeReason::FROM_ANIMATION);
    }
...
TEST(LayerAnimationElementTest, BoundsElement)
{
    ...
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateBoundsElement(target, delta);

    for (int i = 0; i < 2; ++i) {
        start_time += delta;
        element->set_requested_start_time(start_time);
        delegate.SetBoundsFromAnimation(start, PropertyChangeReason::NOT_FROM_ANIMATION);
        element->Start(&delegate, 1);
        element->Progress(start_time, &delegate);
...
TEST(LayerAnimationElementTest, OpacityElement)
{
    ...
    base::TimeTicks effective_start_time;
    base::TimeDelta delta = base::Seconds(1);
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateOpacityElement(target, delta);

    for (int i = 0; i < 2; ++i) {
        start_time = effective_start_time + delta;
        element->set_requested_start_time(start_time);
        delegate.SetOpacityFromAnimation(start, PropertyChangeReason::NOT_FROM_ANIMATION);
        element->Start(&delegate, 1);
        element->Progress(start_time, &delegate);
...
TEST(LayerAnimationElementTest, VisibilityElement)
{
    ...
    base::TimeTicks start_time;
    base::TimeDelta delta = base::Seconds(1);
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateVisibilityElement(target, delta);

    for (int i = 0; i < 2; ++i) {
        start_time += delta;
        element->set_requested_start_time(start_time);
        delegate.SetVisibilityFromAnimation(start, PropertyChangeReason::NOT_FROM_ANIMATION);
        element->Start(&delegate, 1);
        element->Progress(start_time, &delegate);
...
TEST(LayerAnimationElementTest, BrightnessElement)
{
    ...
    base::TimeTicks start_time;
    base::TimeDelta delta = base::Seconds(1);
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateBrightnessElement(target, delta);

    for (int i = 0; i < 2; ++i) {
        start_time += delta;
        element->set_requested_start_time(start_time);
        delegate.SetBrightnessFromAnimation(start, PropertyChangeReason::NOT_FROM_ANIMATION);
        element->Start(&delegate, 1);
        element->Progress(start_time, &delegate);
...
TEST(LayerAnimationElementTest, GrayscaleElement)
{
    ...
    base::TimeTicks start_time;
    base::TimeDelta delta = base::Seconds(1);
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateGrayscaleElement(target, delta);

    for (int i = 0; i < 2; ++i) {
        start_time += delta;
        element->set_requested_start_time(start_time);
        delegate.SetGrayscaleFromAnimation(start, PropertyChangeReason::NOT_FROM_ANIMATION);
        element->Start(&delegate, 1);
        element->Progress(start_time, &delegate);
...
{
    LayerAnimationElement::AnimatableProperties properties = LayerAnimationElement::TRANSFORM | LayerAnimationElement::BOUNDS | LayerAnimationElement::OPACITY
        | LayerAnimationElement::BRIGHTNESS | LayerAnimationElement::GRAYSCALE;

    base::TimeTicks start_time;
    base::TimeDelta delta = base::Seconds(1);

    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreatePauseElement(properties, delta);
    ...
    CheckApproximatelyEqual(delegate.GetBoundsForAnimation(), copy.GetBoundsForAnimation());
    CheckApproximatelyEqual(delegate.GetTransformForAnimation(), copy.GetTransformForAnimation());
    EXPECT_FLOAT_EQ(delegate.GetOpacityForAnimation(), copy.GetOpacityForAnimation());
    EXPECT_FLOAT_EQ(delegate.GetBrightnessForAnimation(), copy.GetBrightnessForAnimation());
    EXPECT_FLOAT_EQ(delegate.GetGrayscaleForAnimation(), copy.GetGrayscaleForAnimation());
}
...
TEST(LayerAnimationElementTest, ClipRectElement)
{
    ...
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateClipRectElement(target, delta);

    for (int i = 0; i < 2; ++i) {
        start_time += delta;
        element->set_requested_start_time(start_time);
        delegate.SetClipRectFromAnimation(start, PropertyChangeReason::NOT_FROM_ANIMATION);
        element->Start(&delegate, 1);
        element->Progress(start_time, &delegate);
...
TEST(LayerAnimationElementTest, RoundedCornersElement)
{
    ...
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateRoundedCornersElement(target, delta);

    for (int i = 0; i < 2; ++i) {
        start_time += delta;
        element->set_requested_start_time(start_time);
        delegate.SetRoundedCornersFromAnimation(start, PropertyChangeReason::NOT_FROM_ANIMATION);
        element->Start(&delegate, 1);
        element->Progress(start_time, &delegate);
...
TEST(LayerAnimationElementTest, GradientMaskElement)
{
    ...
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateGradientMaskElement(target, delta);

    for (int i = 0; i < 2; ++i) {
        start_time += delta;
        element->set_requested_start_time(start_time);
        delegate.SetGradientMaskFromAnimation(start, PropertyChangeReason::NOT_FROM_ANIMATION);
        element->Start(&delegate, 1);
        element->Progress(start_time, &delegate);
...
TEST(LayerAnimationElementTest, AbortOpacityElement)
{
    ...
    // aborted.
    element->Abort(&delegate);
    EXPECT_FLOAT_EQ(gfx::Tween::CalculateValue(tween_type, 0.5), delegate.GetOpacityForAnimation());
    delegate.ExpectLastPropertyChangeReason(PropertyChangeReason::FROM_ANIMATION);
}
...
TEST(LayerAnimationElementTest, AbortTransformElement)
{
    ...
    // aborted.
    element->Abort(&delegate);
    target_transform.Blend(start_transform, gfx::Tween::CalculateValue(tween_type, 0.5));
    CheckApproximatelyEqual(target_transform, delegate.GetTransformForAnimation());
    delegate.ExpectLastPropertyChangeReason(PropertyChangeReason::FROM_ANIMATION);
}
...
TEST(LayerAnimationElementTest, ToString)
{
    ...
    EXPECT_EQ(base::StringPrintf("LayerAnimationElement{name=ThreadedOpacityTransition,"
                                 " id=%d, group=42, "
