...
CanvasPainter::CanvasPainter(SkBitmap* output, const gfx::Size& output_size, float device_scale_factor, SkColor clear_color, bool is_pixel_canvas)
    : output_(output)
    , pixel_output_size_(gfx::ScaleToCeiledSize(output_size, device_scale_factor))
    , raster_scale_(is_pixel_canvas ? 1.f : device_scale_factor)
    , clear_color_(clear_color)
    , list_(new cc::DisplayItemList)
    , context_(list_.get(), device_scale_factor, gfx::Rect(output_size), is_pixel_canvas)
{
}

CanvasPainter::~CanvasPainter()
{
    SkImageInfo info = SkImageInfo::MakeN32(pixel_output_size_.width(), pixel_output_size_.height(), kPremul_SkAlphaType);
