...
class COMPOSITOR_EXPORT HostBeginFrameObserver : public viz::mojom::BeginFrameObserver {
public:
    class SimpleBeginFrameObserver : public base::CheckedObserver {
    public:
        ~SimpleBeginFrameObserver() override = default;
        virtual void OnBeginFrame(base::TimeTicks frame_begin_time, base::TimeDelta frame_interval) = 0;
        virtual void OnBeginFrameSourceShuttingDown() = 0;
    };

    using SimpleBeginFrameObserverList = base::ObserverList<ui::HostBeginFrameObserver::SimpleBeginFrameObserver, true>;

    HostBeginFrameObserver(SimpleBeginFrameObserverList& observers, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
