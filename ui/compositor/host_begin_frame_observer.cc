...
HostBeginFrameObserver::HostBeginFrameObserver(SimpleBeginFrameObserverList& observers, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
void HostBeginFrameObserver::OnStandaloneBeginFrame(const viz::BeginFrameArgs& args)
{
    ...
    if ((base::TimeTicks::Now() - args.frame_time) > args.interval) {
        ...
        task_runner_->PostDelayedTask(
            FROM_HERE, base::BindOnce(&HostBeginFrameObserver::CoalescedBeginFrame, weak_factory_.GetWeakPtr()), base::Microseconds(1));
        return;
    }
...
mojo::PendingRemote<viz::mojom::BeginFrameObserver> HostBeginFrameObserver::GetBoundRemote()
{
...
{
    simple_begin_frame_observers_->Notify(&SimpleBeginFrameObserver::OnBeginFrame, args.frame_time, args.interval);
}
