...
public:
    using ReportRepeatingCallback = base::RepeatingCallback<void(const cc::FrameSequenceMetrics::CustomReportData&)>;
    using ReportOnceCallback = base::OnceCallback<void(const cc::FrameSequenceMetrics::CustomReportData&)>;

    explicit ThroughputReportChecker(AnimationThroughputReporterTestBase* test_base, bool fail_if_reported = false)
        : test_base_(test_base)
        , fail_if_reported_(fail_if_reported)
    {
    }
    ...
    {
        return base::BindRepeating(&ThroughputReportChecker::OnReport, base::Unretained(this));
    }
    ReportOnceCallback once_callback()
    {
        return base::BindOnce(&ThroughputReportChecker::OnReport, base::Unretained(this));
    }
