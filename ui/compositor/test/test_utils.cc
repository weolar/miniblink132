...
void CheckApproximatelyEqual(const gfx::Transform& lhs, const gfx::Transform& rhs)
{
    unsigned int errors = 0;
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            EXPECT_FLOAT_EQ(lhs.rc(i, j), rhs.rc(i, j)) << "(i, j) = (" << i << ", " << j << "), error count: " << ++errors;
        }
    }
...
void CheckApproximatelyEqual(const gfx::RoundedCornersF& lhs, const gfx::RoundedCornersF& rhs)
{
...
bool WaitForNextFrameToBePresented(ui::Compositor* compositor, std::optional<base::TimeDelta> timeout)
{
    bool frames_presented = false;
    base::RunLoop runloop;
    base::CancelableOnceCallback<void(const viz::FrameTimingDetails& frame_timing_details)> cancelable_callback(
        base::BindLambdaForTesting([&](const viz::FrameTimingDetails& frame_timing_details) {
            frames_presented = true;
            runloop.Quit();
        }));
    compositor->RequestSuccessfulPresentationTimeForNextFrame(cancelable_callback.callback());
