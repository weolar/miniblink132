...
public:
    ...
    StandaloneBeginFrameObserver& operator=(const StandaloneBeginFrameObserver&) = delete;
    ...
    void SetStandaloneObserver(mojo::PendingRemote<viz::mojom::BeginFrameObserver> observer)
    {
        TearDownObservation();
...
private:
    ...
    {
        if (begin_frame_source_ && remote_observer_.is_bound() && !is_observing_begin_frame_source_) {
...
class InProcessContextFactory::PerCompositorData : public viz::mojom::DisplayPrivate {
public:
    ...
    void SetBeginFrameSource(std::unique_ptr<viz::BeginFrameSource> begin_frame_source)
    {
        begin_frame_source_ = std::move(begin_frame_source);
        standalone_begin_frame_observer_.SetBeginFrameSource(begin_frame_source_.get());
    }
    ...
    void Bind(mojo::PendingAssociatedReceiver<viz::mojom::DisplayPrivate> remote)
    {
        receiver_.reset();
    ...
    SkM44 output_color_matrix()
    {
        return output_color_matrix_;
    }
    ...
    display::VariableRefreshRateState vrr_state() const
    {
        return vrr_state_;
    }
...
private:
    ...
    display::VariableRefreshRateState vrr_state_ = display::VariableRefreshRateState::kVrrNotCapable;

    mojo::AssociatedReceiver<viz::mojom::DisplayPrivate> receiver_ { this };
};

InProcessContextFactory::InProcessContextFactory(
    viz::HostFrameSinkManager* host_frame_sink_manager, viz::FrameSinkManagerImpl* frame_sink_manager, bool output_to_window)
    : frame_sink_id_allocator_(kDefaultClientId)
    , output_to_window_(output_to_window)
    , disable_vsync_(base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableVsyncForTests))
    , host_frame_sink_manager_(host_frame_sink_manager)
    , frame_sink_manager_(frame_sink_manager)
{
    DCHECK(host_frame_sink_manager);
    DCHECK_NE(gl::GetGLImplementation(), gl::kGLImplementationNone) << "If running tests, ensure that main() is calling "
...
void InProcessContextFactory::CreateLayerTreeFrameSink(base::WeakPtr<Compositor> compositor)
{
    // Try to reuse existing shared worker context provider.
    bool shared_worker_context_provider_lost = false;
    if (shared_worker_context_provider_wrapper_) {
        // Note: If context is lost, delete reference after releasing the lock.
        const scoped_refptr<viz::RasterContextProvider>& worker_context = shared_worker_context_provider_wrapper_->GetContext();
        base::AutoLock lock(*worker_context->GetLock());
        if (worker_context->RasterInterface()->GetGraphicsResetStatusKHR() != GL_NO_ERROR) {
            shared_worker_context_provider_lost = true;
        }
    }
    if (!shared_worker_context_provider_wrapper_ || shared_worker_context_provider_lost) {
        auto shared_worker_context_provider
            = base::MakeRefCounted<viz::TestInProcessContextProvider>(viz::TestContextType::kGpuRaster, /*support_locking=*/true);
        auto result = shared_worker_context_provider->BindToCurrentSequence();
        if (result != gpu::ContextResult::kSuccess) {
            shared_worker_context_provider_wrapper_ = nullptr;
        } else {
            shared_worker_context_provider_wrapper_ = base::MakeRefCounted<cc::RasterContextProviderWrapper>(std::move(shared_worker_context_provider), nullptr,
    ...
    auto skia_deps = std::make_unique<viz::SkiaOutputSurfaceDependencyImpl>(
        viz::TestGpuServiceHolder::GetInstance()->gpu_service(), output_to_window_ ? data->surface_handle() : gpu::kNullSurfaceHandle);
    auto display_dependency = std::make_unique<viz::DisplayCompositorMemoryAndTaskController>(std::move(skia_deps));
    std::unique_ptr<viz::OutputSurface> output_surface = viz::SkiaOutputSurfaceImpl::Create(display_dependency.get(), renderer_settings_, &debug_settings_);

    auto overlay_processor = std::make_unique<viz::OverlayProcessorStub>();

    std::unique_ptr<viz::BeginFrameSource> begin_frame_source;
    if (disable_vsync_) {
        begin_frame_source = std::make_unique<viz::BackToBackBeginFrameSource>(std::make_unique<viz::DelayBasedTimeSource>(compositor->task_runner().get()));
    } else {
        auto time_source = std::make_unique<viz::DelayBasedTimeSource>(compositor->task_runner().get());
        time_source->SetTimebaseAndInterval(base::TimeTicks(), base::Microseconds(base::Time::kMicrosecondsPerSecond / refresh_rate_));
        begin_frame_source = std::make_unique<viz::DelayBasedBeginFrameSource>(std::move(time_source), viz::BeginFrameSource::kNotRestartableId);
    }
    auto scheduler
        = std::make_unique<viz::DisplayScheduler>(begin_frame_source.get(), compositor->task_runner().get(), output_surface->capabilities().pending_swap_params,
            /*hint_session_factory=*/nullptr);

    data->SetDisplay(std::make_unique<viz::Display>(&shared_bitmap_manager_, &shared_image_manager_, &sync_point_manager_, &gpu_scheduler_, renderer_settings_,
        &debug_settings_, compositor->frame_sink_id(), std::move(display_dependency), std::move(output_surface), std::move(overlay_processor),
        std::move(scheduler), compositor->task_runner()));
    frame_sink_manager_->RegisterBeginFrameSource(begin_frame_source.get(), compositor->frame_sink_id());
    ...
    auto layer_tree_frame_sink
        = std::make_unique<DirectLayerTreeFrameSink>(compositor->frame_sink_id(), frame_sink_manager_, data->display(), SharedMainThreadRasterContextProvider(),
            shared_worker_context_provider_wrapper_, compositor->task_runner(), &gpu_memory_buffer_manager_, compositor->widget());
    compositor->SetLayerTreeFrameSink(std::move(layer_tree_frame_sink), std::move(display_private));

    data->Resize(compositor->size());
}

scoped_refptr<viz::RasterContextProvider> InProcessContextFactory::SharedMainThreadRasterContextProvider()
{
    if (shared_main_thread_contexts_ && shared_main_thread_contexts_->RasterInterface()->GetGraphicsResetStatusKHR() == GL_NO_ERROR) {
        return shared_main_thread_contexts_;
    }

    shared_main_thread_contexts_ = base::MakeRefCounted<viz::TestInProcessContextProvider>(viz::TestContextType::kSoftwareRaster, /*support_locking=*/false);
...
gpu::GpuMemoryBufferManager* InProcessContextFactory::GetGpuMemoryBufferManager()
{
...
SkM44 InProcessContextFactory::GetOutputColorMatrix(Compositor* compositor) const
{
...
gfx::DisplayColorSpaces InProcessContextFactory::GetDisplayColorSpaces(Compositor* compositor) const
{
    auto iter = per_compositor_data_.find(compositor);
    if (iter == per_compositor_data_.end())
...
base::TimeTicks InProcessContextFactory::GetDisplayVSyncTimeBase(Compositor* compositor) const
{
    auto iter = per_compositor_data_.find(compositor);
    if (iter == per_compositor_data_.end())
...
base::TimeDelta InProcessContextFactory::GetDisplayVSyncTimeInterval(Compositor* compositor) const
{
    auto iter = per_compositor_data_.find(compositor);
    if (iter == per_compositor_data_.end())
...
std::optional<base::TimeDelta> InProcessContextFactory::GetMaxVSyncInterval(Compositor* compositor) const
{
    auto iter = per_compositor_data_.find(compositor);
    if (iter == per_compositor_data_.end()) {
        return std::nullopt;
...
display::VariableRefreshRateState InProcessContextFactory::GetVrrState(Compositor* compositor) const
{
    auto iter = per_compositor_data_.find(compositor);
    if (iter == per_compositor_data_.end()) {
...
void InProcessContextFactory::ResetDisplayOutputParameters(Compositor* compositor)
{
    auto iter = per_compositor_data_.find(compositor);
    if (iter == per_compositor_data_.end())
        return;
...
InProcessContextFactory::PerCompositorData* InProcessContextFactory::CreatePerCompositorData(Compositor* compositor)
{
