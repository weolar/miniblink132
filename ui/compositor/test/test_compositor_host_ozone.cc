...
// AcceleratedWidget.
class TestCompositorHostOzone::StubPlatformWindowDelegate : public PlatformWindowDelegate {
public:
    ...
    StubPlatformWindowDelegate& operator=(const StubPlatformWindowDelegate&) = delete;
...
TestCompositorHostOzone::TestCompositorHostOzone(const gfx::Rect& bounds, ui::ContextFactory* context_factory)
    : bounds_(bounds)
    , compositor_(context_factory->AllocateFrameSinkId(), context_factory, base::SingleThreadTaskRunner::GetCurrentDefault(), false /* enable_pixel_canvas */)
...
void TestCompositorHostOzone::Show()
{
    ...
    window_ = ui::OzonePlatform::GetInstance()->CreatePlatformWindow(window_delegate_.get(), std::move(properties));
    window_->Show();
    DCHECK_NE(window_delegate_->widget(), gfx::kNullAcceleratedWidget);

    allocator_.GenerateId();
    compositor_.SetAcceleratedWidget(window_delegate_->widget());
    compositor_.SetScaleAndSize(1.0f, bounds_.size(), allocator_.GetCurrentLocalSurfaceId());
...
// static
TestCompositorHost* TestCompositorHost::Create(const gfx::Rect& bounds, ui::ContextFactory* context_factory)
{
