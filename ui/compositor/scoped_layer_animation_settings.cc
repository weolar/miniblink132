...
template <typename Trait> class ScopedLayerAnimationObserver : public ui::ImplicitAnimationObserver, public ui::LayerObserver {
public:
    ScopedLayerAnimationObserver(ui::Layer* layer)
        : layer_(layer)
    {
    ...
    ScopedLayerAnimationObserver& operator=(const ScopedLayerAnimationObserver&) = delete;
...
using ScopedRenderSurfaceCaching = ScopedLayerAnimationObserver<RenderSurfaceCachingTrait>;
...
using ScopedDeferredPainting = ScopedLayerAnimationObserver<DeferredPaintingTrait>;
...
using ScopedTrilinearFiltering = ScopedLayerAnimationObserver<TrilinearFilteringTrait>;

void AddObserverToSettings(ui::ScopedLayerAnimationSettings* settings, std::unique_ptr<ui::ImplicitAnimationObserver> observer)
{
    settings->AddObserver(observer.get());
    settings->GetAnimator()->AddOwnedObserver(std::move(observer));
}

void AddScopedDeferredPaintingObserverRecursive(ui::Layer* layer, ui::ScopedLayerAnimationSettings* settings)
{
...
ScopedLayerAnimationSettings::ScopedLayerAnimationSettings(scoped_refptr<LayerAnimator> animator)
    : animator_(animator)
    , old_is_transition_duration_locked_(animator->is_transition_duration_locked_)
...
{
    SetTransitionDuration(base::Milliseconds(kScopedLayerAnimationDefaultTransitionDurationMs));
}
...
void ScopedLayerAnimationSettings::AddObserver(ImplicitAnimationObserver* observer)
{
    observers_.insert(observer);
    animator_->AddObserver(observer);
}

void ScopedLayerAnimationSettings::SetTransitionDuration(base::TimeDelta duration)
{
...
void ScopedLayerAnimationSettings::SetPreemptionStrategy(LayerAnimator::PreemptionStrategy strategy)
{
    animator_->set_preemption_strategy(strategy);
}

LayerAnimator::PreemptionStrategy ScopedLayerAnimationSettings::GetPreemptionStrategy() const
{
...
{
    auto observer = std::make_unique<ScopedRenderSurfaceCaching>(animator_->delegate()->GetLayer());
    AddObserverToSettings(this, std::move(observer));
}

void ScopedLayerAnimationSettings::DeferPaint()
{
    AddScopedDeferredPaintingObserverRecursive(animator_->delegate()->GetLayer(), this);
}

void ScopedLayerAnimationSettings::TrilinearFiltering()
{
    auto observer = std::make_unique<ScopedTrilinearFiltering>(animator_->delegate()->GetLayer());
    AddObserverToSettings(this, std::move(observer));
}
