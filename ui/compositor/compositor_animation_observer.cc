...
#if !DCHECK_IS_ON() || defined(OFFICIAL_BUILD) || !defined(NDEBUG) || defined(ADDRESS_SANITIZER) || defined(MEMORY_SANITIZER) || defined(THREAD_SANITIZER)     \
...
CompositorAnimationObserver::CompositorAnimationObserver(const base::Location& location)
    : location_(location)
{
}
...
{
    if (!base::debug::BeingDebugged() && !base::subtle::ScopedTimeClockOverrides::overrides_active()) {
        TRACE_EVENT_BEGIN("ui", "LongCompositorAnimationObserved", perfetto::ThreadTrack::Current(), *start_);
        TRACE_EVENT_END("ui");
        LOG(DFATAL_OR_WARNING) << "CompositorAnimationObserver is active for too long (" << (base::TimeTicks::Now() - *start_).InSecondsF()
