...
base::TimeDelta DeltaFromNowToTarget(const base::TimeTicks start, int duration)
{
...
public:
    ...
    ~TestCompositorMonitor() override
    {
    ...
    void OnDidPresentCompositorFrame(uint32_t frame_token, const gfx::PresentationFeedback& feedback) override
    {
...
TotalAnimationThroughputReporter::ReportOnceCallback IgnoreTimestamps(ThroughputReportChecker::ReportOnceCallback original)
{
    return base::BindOnce([](ThroughputReportChecker::ReportOnceCallback original, const cc::FrameSequenceMetrics::CustomReportData& data,
                              base::TimeTicks first_animation_started_at, base::TimeTicks last_animation_finished_at) { std::move(original).Run(data); },
        std::move(original));
}

} // namespace

using TotalAnimationThroughputReporterTest = AnimationThroughputReporterTestBase;
...
TEST_F(TotalAnimationThroughputReporterTest, SingleAnimation)
{
    ...
    TotalAnimationThroughputReporter reporter(compositor(), checker.repeating_callback());
...
TEST_F(TotalAnimationThroughputReporterTest, DISABLED_AddAnimationWhileAnimating)
{
    Layer layer1;
    layer1.SetOpacity(0.5f);
    root_layer()->Add(&layer1);
...
public:
    ObserverChecker(ui::Compositor* compositor, ui::CompositorObserver* reporter_observer)
        : compositor_(compositor)
...
TEST_F(TotalAnimationThroughputReporterTest, OnceReporter)
{
    ...
    ThroughputReportChecker checker(this);
    TotalAnimationThroughputReporter reporter(compositor(), IgnoreTimestamps(checker.once_callback()),
        /*should_delete=*/false);
    auto scoped_blocker = reporter.NewScopedBlocker();
...
TEST_F(TotalAnimationThroughputReporterTest, OnceReporterShouldDelete)
{
    ...
    public:
        DeleteTestReporter(Compositor* compositor, ReportOnceCallback callback, bool* deleted)
            : TotalAnimationThroughputReporter(compositor, std::move(callback), true)
            , deleted_(deleted)
        {
        }
        ~DeleteTestReporter() override
        {
            *deleted_ = true;
        }
    ...
    bool deleted = false;
    TotalAnimationThroughputReporter* reporter = new DeleteTestReporter(compositor(),
        base::BindLambdaForTesting([&](const cc::FrameSequenceMetrics::CustomReportData&, base::TimeTicks, base::TimeTicks) { run_loop.Quit(); }), &deleted);
    auto scoped_blocker = reporter->NewScopedBlocker();
...
TEST_F(TotalAnimationThroughputReporterTest, ThreadCheck)
{
    ...
    ThroughputReportChecker::ReportOnceCallback callback = base::BindLambdaForTesting([&](const cc::FrameSequenceMetrics::CustomReportData& data) {
        // This call with fail if this is called on impl thread.
        c->ScheduleDraw();
        std::move(once_callback).Run(data);
    });

    TotalAnimationThroughputReporter reporter(c, IgnoreTimestamps(std::move(callback)),
        /*should_delete=*/false);
    auto scoped_blocker = reporter.NewScopedBlocker();
