...
#define SAFE_INVOKE_BOOL(function, running_anim) ((running_anim.is_sequence_alive()) ? function(running_anim.sequence()) : false)
#define SAFE_INVOKE_PTR(function, running_anim) ((running_anim.is_sequence_alive()) ? function(running_anim.sequence()) : nullptr)
...
{
    animation_ = cc::Animation::Create(cc::AnimationIdProvider::NextAnimationId());
}
...
{
    return base::MakeRefCounted<LayerAnimator>(base::Milliseconds(kLayerAnimatorDefaultTransitionDurationMs));
}
    ...
    {                                                                                                                                                          \
        ...
        if (duration.is_zero() && delegate() && (preemption_strategy_ != ENQUEUE_NEW_ANIMATION)) {                                                             \
            ...
            if (!weak_ptr || !delegate())                                                                                                                      \
                return;                                                                                                                                        \
            delegate()->Set##name##FromAnimation(value, PropertyChangeReason::NOT_FROM_ANIMATION);                                                             \
            return;                                                                                                                                            \
        }                                                                                                                                                      \
        std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::Create##name##Element(value, duration);                                        \
...
ANIMATED_PROPERTY(const gfx::Transform&, TRANSFORM, Transform, gfx::Transform, transform)
...
ANIMATED_PROPERTY(const gfx::RoundedCornersF&, ROUNDED_CORNERS, RoundedCorners, gfx::RoundedCornersF, rounded_corners)
ANIMATED_PROPERTY(const gfx::LinearGradient&, GRADIENT_MASK, GradientMask, gfx::LinearGradient, gradient_mask)
...
void LayerAnimator::AddThreadedAnimation(std::unique_ptr<cc::KeyframeModel> animation)
{
...
void LayerAnimator::ScheduleAnimation(LayerAnimationSequence* animation)
{
    ...
    OnScheduled(animation);
    if (is_animating()) {
        animation_queue_.push_back(std::unique_ptr<LayerAnimationSequence>(animation));
        ProcessQueue();
    } else {
...
void LayerAnimator::StartTogether(const std::vector<LayerAnimationSequence*>& animations)
{
    scoped_refptr<LayerAnimator> retain(this);
    ...
    StartAnimation(new LayerAnimationSequence(LayerAnimationElement::CreatePauseElement(animated_properties, base::TimeDelta())));

    bool wait_for_group_start = false;
    for (iter = animations.begin(); iter != animations.end(); ++iter)
        wait_for_group_start |= delegate_ && (*iter)->IsFirstElementThreaded(delegate_);
...
void LayerAnimator::ScheduleTogether(const std::vector<LayerAnimationSequence*>& animations)
{
    scoped_refptr<LayerAnimator> retain(this);
    ...
    ScheduleAnimation(new LayerAnimationSequence(LayerAnimationElement::CreatePauseElement(animated_properties, base::TimeDelta())));
...
void LayerAnimator::SchedulePauseForProperties(base::TimeDelta duration, LayerAnimationElement::AnimatableProperties properties_to_pause)
{
    ScheduleAnimation(new ui::LayerAnimationSequence(ui::LayerAnimationElement::CreatePauseElement(properties_to_pause, duration)));
}

bool LayerAnimator::IsAnimatingOnePropertyOf(LayerAnimationElement::AnimatableProperties properties) const
{
    for (auto& layer_animation_sequence : animation_queue_) {
...
void LayerAnimator::StopAnimatingProperty(LayerAnimationElement::AnimatableProperty property)
{
    scoped_refptr<LayerAnimator> retain(this);
    while (true) {
...
void LayerAnimator::RemoveObserver(LayerAnimationObserver* observer)
{
    ...
    for (AnimationQueue::iterator queue_iter = animation_queue_.begin(); queue_iter != animation_queue_.end(); ++queue_iter) {
        (*queue_iter)->RemoveObserver(observer);
    }
}

void LayerAnimator::AddOwnedObserver(std::unique_ptr<ImplicitAnimationObserver> animation_observer)
{
    owned_observer_list_.push_back(std::move(animation_observer));
}

void LayerAnimator::RemoveAndDestroyOwnedObserver(ImplicitAnimationObserver* animation_observer)
{
    std::erase_if(
        owned_observer_list_, [animation_observer](const std::unique_ptr<ImplicitAnimationObserver>& other) { return other.get() == animation_observer; });
}

base::CallbackListSubscription LayerAnimator::AddSequenceScheduledCallback(SequenceScheduledCallback callback)
{
    return sequence_scheduled_callbacks_.Add(std::move(callback));
}

void LayerAnimator::OnThreadedAnimationStarted(base::TimeTicks monotonic_time, cc::TargetProperty::Type target_property, int group_id)
{
    LayerAnimationElement::AnimatableProperty property = LayerAnimationElement::ToAnimatableProperty(target_property);
    ...
    running->sequence()->OnThreadedAnimationStarted(monotonic_time, target_property, group_id);
    ...
    // over now are alive.
    for (auto iter = running_animations_.begin(); iter != running_animations_.end(); ++iter) {
        ...
        if (((*iter).sequence()->animation_group_id() == group_id) && !(*iter).sequence()->IsFirstElementThreaded(delegate_)
...
void LayerAnimator::RemoveFromCollection(LayerAnimatorCollection* collection)
{
    ...
    DCHECK(!animation_->element_animations() || !animation_->element_animations()->HasTickingKeyframeEffect());
}

// LayerAnimator protected -----------------------------------------------------

void LayerAnimator::ProgressAnimation(LayerAnimationSequence* sequence, base::TimeTicks now)
{
...
{
    for (AnimationQueue::const_iterator queue_iter = animation_queue_.begin(); queue_iter != animation_queue_.end(); ++queue_iter) {
...
LayerAnimationSequence* LayerAnimator::RemoveAnimation(LayerAnimationSequence* sequence)
{
    ...
    if (!to_return.get() || !to_return->waiting_for_group_start() || !to_return->IsFirstElementThreaded(delegate_))
        return to_return.release();
    ...
    for (AnimationQueue::iterator queue_iter = animation_queue_.begin(); queue_iter != animation_queue_.end(); ++queue_iter) {
        if (((*queue_iter)->animation_group_id() == group_id) && (*queue_iter)->IsFirstElementThreaded(delegate_)) {
...
void LayerAnimator::FinishAnimation(LayerAnimationSequence* sequence, bool abort)
{
    scoped_refptr<LayerAnimator> retain(this);
...
void LayerAnimator::FinishAnyAnimationWithZeroDuration()
{
    ...
    for (size_t i = 0; i < running_animations_copy.size(); ++i) {
        ...
        if (running_animations_copy[i].sequence()->IsFinished(running_animations_copy[i].sequence()->start_time())) {
            SAFE_INVOKE_VOID(ProgressAnimationToEnd, running_animations_copy[i]);
            std::unique_ptr<LayerAnimationSequence> removed(SAFE_INVOKE_PTR(RemoveAnimation, running_animations_copy[i]));
        }
    }
    ProcessQueue();
    UpdateAnimationState();
...
LayerAnimator::RunningAnimation* LayerAnimator::GetRunningAnimation(LayerAnimationElement::AnimatableProperty property)
{
    PurgeDeletedAnimations();
    for (auto iter = running_animations_.begin(); iter != running_animations_.end(); ++iter) {
...
void LayerAnimator::AddToQueueIfNotPresent(LayerAnimationSequence* animation)
{
    ...
    if (!found_sequence) {
        animation_queue_.push_front(std::unique_ptr<LayerAnimationSequence>(animation));
    }
}

void LayerAnimator::RemoveAllAnimationsWithACommonProperty(LayerAnimationSequence* sequence, bool abort)
{
    ...
    for (size_t i = 0; i < running_animations_copy.size(); ++i) {
        ...
        if (running_animations_copy[i].sequence()->HasConflictingProperty(sequence->properties())) {
            std::unique_ptr<LayerAnimationSequence> removed(SAFE_INVOKE_PTR(RemoveAnimation, running_animations_copy[i]));
            if (abort)
    ...
    for (size_t i = 0; i < sequences.size(); ++i) {
        ...
        if (sequences[i]->HasConflictingProperty(sequence->properties())) {
            std::unique_ptr<LayerAnimationSequence> removed(RemoveAnimation(sequences[i].get()));
            if (abort)
...
void LayerAnimator::ImmediatelyAnimateToNewTarget(LayerAnimationSequence* sequence)
{
    // Need to detect if our sequence gets destroyed.
    base::WeakPtr<LayerAnimationSequence> weak_sequence_ptr = sequence->AsWeakPtr();
...
void LayerAnimator::ReplaceQueuedAnimations(LayerAnimationSequence* sequence)
{
    ...
    for (size_t i = 0; i < animation_queue_.size();) {
        ...
        bool is_running = false;
        for (RunningAnimations::const_iterator iter = running_animations_.begin(); iter != running_animations_.end(); ++iter) {
...
bool LayerAnimator::StartSequenceImmediately(LayerAnimationSequence* sequence)
{
    ...
    running_animations_.push_back(RunningAnimation(sequence->AsWeakPtr()));

    // Need to keep a reference to the animation.
    AddToQueueIfNotPresent(sequence);

    if (!sequence->waiting_for_group_start() || sequence->IsFirstElementThreaded(delegate_)) {
...
void LayerAnimator::GetTargetValue(LayerAnimationElement::TargetValue* target) const
{
    for (AnimationQueue::const_iterator iter = animation_queue_.begin(); iter != animation_queue_.end(); ++iter) {
...
void LayerAnimator::ClearAnimationsInternal()
{
    ...
    for (size_t i = 0; i < running_animations_copy.size(); ++i) {
        ...
        std::unique_ptr<LayerAnimationSequence> removed(RemoveAnimation(running_animations_copy[i].sequence()));
        if (removed.get())
...
void LayerAnimator::NotifyAnimationStarted(base::TimeTicks monotonic_time, int target_property, int group)
{
    OnThreadedAnimationStarted(monotonic_time, static_cast<cc::TargetProperty::Type>(target_property), group);
}

LayerAnimator::RunningAnimation::RunningAnimation(const base::WeakPtr<LayerAnimationSequence>& sequence)
    : sequence_(sequence)
{
}

LayerAnimator::RunningAnimation::RunningAnimation(const RunningAnimation& other) = default;
