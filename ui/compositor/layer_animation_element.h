...
public:
    ...
    static AnimatableProperty ToAnimatableProperty(cc::TargetProperty::Type property);
    ...
    static std::string AnimatablePropertiesToString(AnimatableProperties properties);

    // Creates an element that transitions to the given transform. The caller owns
    // the return value.
    static std::unique_ptr<LayerAnimationElement> CreateTransformElement(const gfx::Transform& transform, base::TimeDelta duration);
    ...
    static std::unique_ptr<LayerAnimationElement> CreateInterpolatedTransformElement(
    ...
    // the return value.
    static std::unique_ptr<LayerAnimationElement> CreateBoundsElement(const gfx::Rect& bounds, base::TimeDelta duration);

    // Creates an element that transitions to the given opacity. The caller owns
    // the return value.
    static std::unique_ptr<LayerAnimationElement> CreateOpacityElement(float opacity, base::TimeDelta duration);

    // Creates an element that sets visibily following a delay. The caller owns
    // the return value.
    static std::unique_ptr<LayerAnimationElement> CreateVisibilityElement(bool visibility, base::TimeDelta duration);

    // Creates an element that transitions to the given brightness.
    // The caller owns the return value.
    static std::unique_ptr<LayerAnimationElement> CreateBrightnessElement(float brightness, base::TimeDelta duration);

    // Creates an element that transitions to the given grayscale value.
    // The caller owns the return value.
    static std::unique_ptr<LayerAnimationElement> CreateGrayscaleElement(float grayscale, base::TimeDelta duration);

    // Creates an element that pauses the given properties. The caller owns the
    // return value.
    static std::unique_ptr<LayerAnimationElement> CreatePauseElement(AnimatableProperties properties, base::TimeDelta duration);

    // Creates an element that transitions to the given color. The caller owns the
    // return value.
    static std::unique_ptr<LayerAnimationElement> CreateColorElement(SkColor color, base::TimeDelta duration);

    // Creates an element that transitions the clip rect of the layer to the given
    // bounds. The caller owns the return value.
    static std::unique_ptr<LayerAnimationElement> CreateClipRectElement(const gfx::Rect& clip_rect, base::TimeDelta duration);

    // Creates an element that transitions the rounded corners of the layer to the
    // given ones. The caller owns the return value.
    static std::unique_ptr<LayerAnimationElement> CreateRoundedCornersElement(const gfx::RoundedCornersF& rounded_corners, base::TimeDelta duration);

    // Creates an element that transitions the gradient mask to the
    // given one. The caller owns the return value.
    static std::unique_ptr<LayerAnimationElement> CreateGradientMaskElement(const gfx::LinearGradient& gradient_mask, base::TimeDelta duration);
    ...
    base::TimeTicks requested_start_time() const
    {
        return requested_start_time_;
    }
    ...
    base::TimeTicks effective_start_time() const
    {
        return effective_start_time_;
    }
    ...
    AnimatableProperties properties() const
    {
        return properties_;
    }
    ...
    {
        return keyframe_model_id_;
    }
    int animation_group_id() const
    {
        return animation_group_id_;
    }
    void set_animation_group_id(int id)
    {
        animation_group_id_ = id;
    }
    ...
    double last_progressed_fraction() const
    {
        return last_progressed_fraction_;
    }
