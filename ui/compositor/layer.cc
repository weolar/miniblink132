...
public:
    ...
    SubpixelPositionOffsetCache& operator=(const SubpixelPositionOffsetCache&) = delete;
    ~SubpixelPositionOffsetCache() = default;

    gfx::Vector2dF GetSubpixelOffset(float device_scale_factor, const gfx::Point& origin, const gfx::Transform& tm) const
    {
        ...
        if (device_scale_factor == device_scale_factor_ && offset_from_parent == offset_from_parent_) {
            return offset_;
        }
        ...
        gfx::PointF snapped_offset_from_parent(gfx::ToRoundedPoint(scaled_offset_from_parent));

        gfx::Vector2dF offset = snapped_offset_from_parent - scaled_offset_from_parent;
...
Layer::Layer(LayerType type)
    ...
    , compositor_(nullptr)
    , parent_(nullptr)
    , subpixel_position_offset_(std::make_unique<SubpixelPositionOffsetCache>())
    , visible_(true)
...
std::unique_ptr<Layer> Layer::Clone() const
{
    ...
    if (surface_layer_) {
        clone->SetShowSurface(surface_layer_->surface_id(), frame_size_in_dip_, surface_layer_->background_color().toSkColor(),
            surface_layer_->deadline_in_frames() ? cc::DeadlinePolicy::UseSpecifiedDeadline(*surface_layer_->deadline_in_frames())
                                                 : cc::DeadlinePolicy::UseDefaultDeadline(),
            surface_layer_->stretch_content_to_fill_bounds());
        if (surface_layer_->oldest_acceptable_fallback())
            clone->SetOldestAcceptableFallback(*surface_layer_->oldest_acceptable_fallback());
...
std::unique_ptr<Layer> Layer::Mirror()
{
    ...
    if (!transfer_resource_.is_empty()) {
        ...
        mirror->SetTransferableResource(transfer_resource_, base::BindOnce([](const gpu::SyncToken& sync_token, bool is_lost) {}), frame_size_in_dip_);
    }
...
void Layer::SetShowReflectedLayerSubtree(Layer* subtree_reflected_layer)
{
    ...
    scoped_refptr<cc::MirrorLayer> new_layer = cc::MirrorLayer::Create(subtree_reflected_layer->cc_layer_.get());
    ...
    auto insert_pair = subtree_reflected_layer_->subtree_reflecting_layers_.insert(this);
...
void Layer::SetCompositor(Compositor* compositor, scoped_refptr<cc::Layer> root_layer)
{
...
void Layer::SetAnimator(scoped_refptr<LayerAnimator> animator)
{
    ...
    if (animator_) {
        if (compositor && compositor->animations_are_enabled() && !layer_mask_back_link())
...
{
    if (animator_ && animator_->IsAnimatingProperty(LayerAnimationElement::TRANSFORM)) {
...
{
    return subpixel_position_offset_->GetSubpixelOffset(device_scale_factor_, GetTargetBounds().origin(), GetTargetTransform());
}

gfx::Rect Layer::GetTargetBounds() const
{
    if (animator_ && animator_->IsAnimatingProperty(LayerAnimationElement::BOUNDS)) {
...
{
    if (animator_ && animator_->IsAnimatingProperty(LayerAnimationElement::CLIP)) {
...
{
    if (animator_ && animator_->IsAnimatingProperty(LayerAnimationElement::BRIGHTNESS)) {
...
{
    if (animator_ && animator_->IsAnimatingProperty(LayerAnimationElement::GRAYSCALE)) {
...
void Layer::SetLayerCustomColorMatrix(const cc::FilterOperation::Matrix& matrix)
{
    layer_custom_color_matrix_ = std::make_unique<cc::FilterOperation::Matrix>(matrix);
    SetLayerFilters();
}
...
void Layer::SetMaskLayer(Layer* layer_mask)
{
    ...
    DCHECK(!layer_mask || (!layer_mask->layer_mask_layer() && layer_mask->children().empty()));
    ...
    cc_layer_->SetMaskLayer(layer_mask ? layer_mask->content_layer_.get() : nullptr);
...
void Layer::SetLayerFilters()
{
    ...
    if (layer_custom_color_matrix_) {
        filters.Append(cc::FilterOperation::CreateColorMatrixFilter(*layer_custom_color_matrix_));
    }
    if (layer_hue_rotation_) {
        filters.Append(cc::FilterOperation::CreateHueRotateFilter(layer_hue_rotation_));
    }
    if (layer_saturation_) {
        filters.Append(cc::FilterOperation::CreateSaturateFilter(layer_saturation_));
    }
    if (layer_grayscale_) {
        filters.Append(cc::FilterOperation::CreateGrayscaleFilter(layer_grayscale_));
    }
    if (layer_inverted_)
    ...
    if (layer_blur_sigma_) {
        filters.Append(cc::FilterOperation::CreateBlurFilter(layer_blur_sigma_, SkTileMode::kClamp));
    }
    ...
    if (layer_brightness_) {
        filters.Append(cc::FilterOperation::CreateSaturatingBrightnessFilter(layer_brightness_));
    }
    if (alpha_shape_) {
        filters.Append(cc::FilterOperation::CreateAlphaThresholdFilter(*alpha_shape_));
    }
...
void Layer::SetLayerBackgroundFilters()
{
    ...
    if (background_blur_sigma_) {
        filters.Append(cc::FilterOperation::CreateBlurFilter(background_blur_sigma_, SkTileMode::kClamp));
    }
...
{
    if (animator_ && animator_->IsAnimatingProperty(LayerAnimationElement::OPACITY))
...
{
    if (animator_ && animator_->IsAnimatingProperty(LayerAnimationElement::VISIBILITY))
...
{
    if (animator_ && animator_->IsAnimatingProperty(LayerAnimationElement::ROUNDED_CORNERS)) {
...
bool Layer::GetTargetTransformRelativeTo(const Layer* ancestor, gfx::Transform* transform) const
{
    return GetTransformRelativeToImpl(ancestor, /*is_target_transform=*/true, transform);
}

bool Layer::GetTransformRelativeTo(const Layer* ancestor, gfx::Transform* transform) const
{
    return GetTransformRelativeToImpl(ancestor, /*is_target_transform=*/false, transform);
}

// static
void Layer::ConvertPointToLayer(const Layer* source, const Layer* target, bool use_target_transform, gfx::PointF* point)
{
    if (source == target)
        return;
    ...
    if (source_root_layer != target_root_layer) {
        ...
        LOG(ERROR) << "Source has different root than tareget: source chain=" << chain_name(source) << ", target chain=" << chain_name(target);
    }
    CHECK_EQ(source_root_layer, target_root_layer);

    if (source != source_root_layer) {
        source->ConvertPointForAncestor(source_root_layer, use_target_transform, point);
    }
    if (target != source_root_layer) {
        target->ConvertPointFromAncestor(source_root_layer, use_target_transform, point);
    }
}

void Layer::SetFillsBoundsOpaquely(bool fills_bounds_opaquely)
{
    SetFillsBoundsOpaquelyWithReason(fills_bounds_opaquely, PropertyChangeReason::NOT_FROM_ANIMATION);
}
...
bool Layer::SwitchToLayer(scoped_refptr<cc::Layer> new_layer)
{
    ...
    new_layer->SetSafeOpaqueBackgroundColor(cc_layer_->SafeOpaqueBackgroundColor());
...
void Layer::AddCacheRenderSurfaceRequest()
{
    ...
    TRACE_COUNTER_ID1("ui", "CacheRenderSurfaceRequests", this, cache_render_surface_requests_);
...
void Layer::AddDeferredPaintRequest()
{
    ...
    TRACE_COUNTER_ID1("ui", "DeferredPaintRequests", this, deferred_paint_requests_);
}
...
void Layer::AddTrilinearFilteringRequest()
{
    ...
    TRACE_COUNTER_ID1("ui", "TrilinearFilteringRequests", this, trilinear_filtering_request_);
...
void Layer::SetTransferableResource(const viz::TransferableResource& resource, viz::ReleaseCallback release_callback, gfx::Size texture_size_in_dip)
{
    ...
    if (!texture_layer_.get()) {
        scoped_refptr<cc::TextureLayer> new_layer = cc::TextureLayer::CreateForMailbox(this);
        if (!SwitchToLayer(new_layer))
            return;
    ...
    for (const auto& mirror : mirrors_) {
        ...
        mirror->dest()->SetTransferableResource(transfer_resource_, base::BindOnce([](const gpu::SyncToken& sync_token, bool is_lost) {}), frame_size_in_dip_);
    }
}
...
void Layer::SetShowSurface(const viz::SurfaceId& surface_id, const gfx::Size& frame_size_in_dip, SkColor default_background_color,
    const cc::DeadlinePolicy& deadline_policy, bool stretch_content_to_fill_bounds)
{
    DCHECK(type_ == LAYER_TEXTURED || type_ == LAYER_SOLID_COLOR);
    ...
    surface_layer_->SetBackgroundColor(SkColor4f::FromColor(default_background_color));
    surface_layer_->SetSafeOpaqueBackgroundColor(SkColor4f::FromColor(default_background_color));
    ...
    for (const auto& mirror : mirrors_) {
        mirror->dest()->SetShowSurface(surface_id, frame_size_in_dip, default_background_color, deadline_policy, stretch_content_to_fill_bounds);
    }
}

void Layer::SetShowSurface(
    const viz::SurfaceId& surface_id, SkColor default_background_color, const cc::DeadlinePolicy& deadline_policy, bool stretch_content_to_fill_bounds)
{
    DCHECK(type_ == LAYER_TEXTURED || type_ == LAYER_SOLID_COLOR);
    ...
    for (const auto& mirror : mirrors_) {
        mirror->dest()->SetShowSurface(surface_id, default_background_color, deadline_policy, stretch_content_to_fill_bounds);
    }
}
...
void Layer::SetShowReflectedSurface(const viz::SurfaceId& surface_id, const gfx::Size& frame_size_in_pixels)
{
    DCHECK(type_ == LAYER_TEXTURED || type_ == LAYER_SOLID_COLOR);
    ...
    surface_layer_->SetSurfaceId(surface_id, cc::DeadlinePolicy::UseInfiniteDeadline());
...
void Layer::UpdateNinePatchLayerImage(const gfx::ImageSkia& image)
{
    ...
    nine_patch_layer_->SetBitmap(image.GetRepresentation(device_scale_factor_).GetBitmap());
}
...
void Layer::UpdateNinePatchLayerAperture(const gfx::Rect& aperture_in_dip)
{
    ...
    gfx::Rect aperture_in_pixel = gfx::ToEnclosingRect(gfx::ConvertRectToPixels(aperture_in_dip, device_scale_factor()));
...
{
    if (animator_ && animator_->IsAnimatingProperty(LayerAnimationElement::COLOR))
...
{
    if (type_ == LAYER_NOT_DRAWN || (type_ == LAYER_SOLID_COLOR && !texture_layer_)) {
        return false;
    }
...
void Layer::CompleteAllAnimations()
{
    ...
    for (LayerAnimatorVector::const_iterator it = animators.begin(); it != animators.end(); ++it) {
        (*it)->StopAnimating();
    }
}

void Layer::StackChildrenAtBottom(const std::vector<Layer*>& new_leading_children)
{
    ...
    for (Layer* leading_child : new_leading_children) {
        ...
        new_cc_children_order.emplace_back(scoped_refptr<cc::Layer>(leading_child->cc_layer_.get()));
    }

    base::flat_set<raw_ptr<Layer, VectorExperimental>> reordered_children(new_children_order);
...
void Layer::OnDeviceScaleFactorChanged(float device_scale_factor)
{
    ...
    if (is_root_layer) {
        ...
        for (auto& weak_layer : flattened) {
            ...
            weak_layer->animator_->StopAnimatingProperty(LayerAnimationElement::TRANSFORM);
    ...
    if (delegate_) {
        delegate_->OnDeviceScaleFactorChanged(old_device_scale_factor, device_scale_factor);
    }
...
void Layer::SetDidScrollCallback(base::RepeatingCallback<void(const gfx::PointF&, const cc::ElementId&)> callback)
{
...
gfx::PointF Layer::CurrentScrollOffset() const
{
    ...
    gfx::PointF offset;
    if (compositor && compositor->GetScrollOffsetForLayer(cc_layer_->element_id(), &offset))
        return offset;
...
{
    Compositor* compositor = GetCompositor();
    bool scrolled_on_impl_side = compositor && compositor->ScrollLayerTo(cc_layer_->element_id(), offset);
...
void Layer::RequestCopyOfOutput(std::unique_ptr<viz::CopyOutputRequest> request)
{
    if (!request->has_result_task_runner()) {
        CHECK(GetCompositor()) << "A copy request must either have a task runner, or be added to the "
...
scoped_refptr<cc::DisplayItemList> Layer::PaintContentsToDisplayList()
{
    ...
    gfx::Rect invalidation(gfx::IntersectRects(paint_region_.bounds(), local_bounds));
    paint_region_.Clear();
    auto display_list = base::MakeRefCounted<cc::DisplayItemList>();
    if (delegate_) {
        delegate_->OnPaintLayer(PaintContext(display_list.get(), device_scale_factor_, invalidation, GetCompositor()->is_pixel_canvas()));
    }
...
bool Layer::FillsBoundsCompletely() const
{
    return fills_bounds_completely_;
}

bool Layer::PrepareTransferableResource(
    cc::SharedBitmapIdRegistrar* bitmap_registar, viz::TransferableResource* resource, viz::ReleaseCallback* release_callback)
{
    if (!transfer_release_callback_)
        return false;
...
void Layer::CollectAnimators(std::vector<scoped_refptr<LayerAnimator>>* animators)
{
...
void Layer::StackRelativeTo(Layer* child, Layer* other, bool above)
{
    ...
    const size_t child_i = base::ranges::find(children_, child) - children_.begin();
    const size_t other_i = base::ranges::find(children_, other) - children_.begin();
...
bool Layer::ConvertPointForAncestor(const Layer* ancestor, bool use_target_transform, gfx::PointF* point) const
{
    gfx::Transform transform;
    bool result = use_target_transform ? GetTargetTransformRelativeTo(ancestor, &transform) : GetTransformRelativeTo(ancestor, &transform);
    *point = transform.MapPoint(*point);
    return result;
}

bool Layer::ConvertPointFromAncestor(const Layer* ancestor, bool use_target_transform, gfx::PointF* point) const
{
    gfx::Transform transform;
    if (!(use_target_transform ? GetTargetTransformRelativeTo(ancestor, &transform) : GetTransformRelativeTo(ancestor, &transform))) {
        return false;
    }
    const std::optional<gfx::PointF> transformed_point = transform.InverseMapPoint(*point);
...
void Layer::SetBoundsFromAnimation(const gfx::Rect& bounds, PropertyChangeReason reason)
{
    if (bounds == bounds_)
        return;
...
void Layer::SetTransformFromAnimation(const gfx::Transform& new_transform, PropertyChangeReason reason)
{
...
void Layer::SetOpacityFromAnimation(float opacity, PropertyChangeReason reason)
{
...
void Layer::SetVisibilityFromAnimation(bool visible, PropertyChangeReason reason)
{
...
void Layer::SetBrightnessFromAnimation(float brightness, PropertyChangeReason reason)
{
    layer_brightness_ = brightness;
    SetLayerFilters();
}

void Layer::SetGrayscaleFromAnimation(float grayscale, PropertyChangeReason reason)
{
...
void Layer::SetClipRectFromAnimation(const gfx::Rect& clip_rect, PropertyChangeReason reason)
{
...
void Layer::SetRoundedCornersFromAnimation(const gfx::RoundedCornersF& rounded_corners, PropertyChangeReason reason)
{
...
void Layer::SetGradientMaskFromAnimation(const gfx::LinearGradient& gradient_mask, PropertyChangeReason reason)
{
...
SkColor Layer::GetColorForAnimation() const
{
    ...
    // TODO(crbug.com/40219248): Remove toSkColor and make all SkColor4f.
    return solid_color_layer_.get() ? solid_color_layer_->background_color().toSkColor() : SK_ColorBLACK;
}
...
void Layer::RecomputeDrawsContentAndUVRect()
{
    ...
    if (texture_layer_.get()) {
        ...
        gfx::PointF uv_bottom_right(
            static_cast<float>(size.width()) / frame_size_in_dip_.width(), static_cast<float>(size.height()) / frame_size_in_dip_.height());
...
{
    const auto it = base::ranges::find(mirrors_, mirror, &std::unique_ptr<LayerMirror>::get);
...
void Layer::ResetSubtreeReflectedLayer()
{
    ...
    size_t result = subtree_reflected_layer_->subtree_reflecting_layers_.erase(this);
    DCHECK_EQ(1u, result);
    subtree_reflected_layer_ = nullptr;
}

void Layer::GetFlattenedWeakList(std::vector<base::WeakPtr<Layer>>* flattened_list)
{
...
void Layer::SetFillsBoundsOpaquelyWithReason(bool fills_bounds_opaquely, PropertyChangeReason reason)
{
...
bool Layer::GetTransformRelativeToImpl(const Layer* ancestor, bool is_target_transform, gfx::Transform* transform) const
{
    const Layer* p = this;
    for (; p && p != ancestor; p = p->parent()) {
        gfx::Transform translation;
        translation.Translate(static_cast<float>(p->bounds().x()), static_cast<float>(p->bounds().y()));
        const gfx::Transform& layer_transform = is_target_transform ? p->GetTargetTransform() : p->transform();
