...
public:
    ...
    struct COMPOSITOR_EXPORT BucketParams {
        BucketParams();
        BucketParams(base::TimeDelta min_latency, base::TimeDelta max_latency, int num_buckets);
    ...
    explicit PresentationTimeRecorder(raw_ptr<PresentationTimeRecorderInternal> internal);
...
// with 50 buckets.
COMPOSITOR_EXPORT std::unique_ptr<PresentationTimeRecorder> CreatePresentationTimeHistogramRecorder(ui::Compositor* compositor,
    const char* presentation_time_histogram_name, const char* max_latency_histogram_name = "",
    PresentationTimeRecorder::BucketParams bucket_params = PresentationTimeRecorder::BucketParams(), bool emit_trace_event = false);
