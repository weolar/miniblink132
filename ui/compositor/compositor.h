...
public:
    ...
    virtual void CreateLayerTreeFrameSink(base::WeakPtr<Compositor> compositor) = 0;

    // Return a reference to a shared offscreen context provider usable from the
    // main thread.
    virtual scoped_refptr<viz::RasterContextProvider> SharedMainThreadRasterContextProvider() = 0;
...
public:
    Compositor(const viz::FrameSinkId& frame_sink_id, ui::ContextFactory* context_factory, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
        bool enable_pixel_canvas, bool use_external_begin_frame_control = false, bool force_software_compositor = false,
        bool enable_compositing_based_throttling = false, size_t memory_limit_when_visible_mb = 0);
    ...
    void SetLayerTreeFrameSink(std::unique_ptr<cc::LayerTreeFrameSink> surface, mojo::AssociatedRemote<viz::mojom::DisplayPrivate> display_private);
    void SetExternalBeginFrameController(mojo::AssociatedRemote<viz::mojom::ExternalBeginFrameController> external_begin_frame_controller);
    ...
    bool animations_are_enabled() const
    {
        return animations_are_enabled_;
    }
    bool IsAnimating() const
    {
        return animation_started_;
    }
    ...
    const SkM44& display_color_matrix() const
    {
    ...
    void SetScaleAndSize(float scale, const gfx::Size& size_in_pixel, const viz::LocalSurfaceId& local_surface_id);
    ...
    bool GetScrollOffsetForLayer(cc::ElementId element_id, gfx::PointF* offset) const;
    ...
    void SetDisplayVSyncParameters(base::TimeTicks timebase, base::TimeDelta interval);
    void AddVSyncParameterObserver(mojo::PendingRemote<viz::mojom::VSyncParameterObserver> observer);
    ...
    void SetMaxVSyncAndVrr(const std::optional<base::TimeDelta>& max_vsync_interval, display::VariableRefreshRateState vrr_state);
    ...
    {
        return force_software_compositor_;
    }
    ...
    std::unique_ptr<CompositorLock> GetCompositorLock(CompositorLockClient* client, base::TimeDelta timeout = base::Milliseconds(kCompositorLockTimeoutMs))
    {
        return lock_manager_.GetCompositorLock(client, timeout, base::DoNothingWithBoundArgs(host_->DeferMainFrameUpdate()));
    }
    ...
    using PresentationTimeCallback = base::OnceCallback<void(const gfx::PresentationFeedback&)>;
    void RequestPresentationTimeForNextFrame(PresentationTimeCallback callback);

    // Registers a callback that is run when the next frame successfully makes it
    // to the screen (it's entirely possible some frames may be dropped between
    // the time this is called and the callback is run).
    using SuccessfulPresentationTimeCallback = base::OnceCallback<void(const viz::FrameTimingDetails& frame_timing_details)>;
    void RequestSuccessfulPresentationTimeForNextFrame(SuccessfulPresentationTimeCallback callback);

    void IssueExternalBeginFrame(const viz::BeginFrameArgs& args, bool force, base::OnceCallback<void(const viz::BeginFrameAck&)> callback);
    ...
    std::unique_ptr<cc::EventsMetricsManager::ScopedMonitor> GetScopedEventMetricsMonitor(cc::EventsMetricsManager::ScopedMonitor::DoneCallback done_callback);
    ...
    void OnDeferCommitsChanged(bool, cc::PaintHoldingReason, std::optional<cc::PaintHoldingCommitTrigger>) override
    {
    }
    ...
    void DidCommit(int source_frame_number, base::TimeTicks, base::TimeTicks) override;
    ...
    void NotifyThroughputTrackerResults(cc::CustomTrackerResults results) override;
    ...
    void StartThroughputTracker(TrackerId tracker_id, ThroughputTrackerHost::ReportCallback callback) override;
    ...
    bool IsLocked()
    {
        return lock_manager_.IsLocked();
    }

    bool output_is_secure() const
    {
        return output_is_secure_;
    }
    ...
    bool is_pixel_canvas() const
    {
        return is_pixel_canvas_;
    }
    ...
    virtual void SetDelegatedInkPointRenderer(mojo::PendingReceiver<gfx::mojom::DelegatedInkPointRenderer> receiver);
    ...
    const cc::LayerTreeHost* host_for_testing() const
    {
        return host_.get();
    }

    void AddSimpleBeginFrameObserver(ui::HostBeginFrameObserver::SimpleBeginFrameObserver* obs);
    void RemoveSimpleBeginFrameObserver(ui::HostBeginFrameObserver::SimpleBeginFrameObserver* obs);
    ...
    void SetSeamlessRefreshRates(const std::vector<float>& seamless_refresh_rates);
...
private:
    ...
    void ReportMetricsForTracker(int tracker_id, const cc::FrameSequenceMetrics::CustomReportData& data);
    ...
    mojo::AssociatedRemote<viz::mojom::ExternalBeginFrameController> external_begin_frame_controller_;

    std::unique_ptr<PendingBeginFrameArgs> pending_begin_frame_args_;

    ui::HostBeginFrameObserver::SimpleBeginFrameObserverList simple_begin_frame_observers_;
    ...
    base::ObserverList<CompositorAnimationObserver>::Unchecked animation_observer_list_;
...
template <> struct ScopedObservationTraits<ui::Compositor, ui::CompositorAnimationObserver> {
    static void AddObserver(ui::Compositor* source, ui::CompositorAnimationObserver* observer)
    {
        source->AddAnimationObserver(observer);
    }
    static void RemoveObserver(ui::Compositor* source, ui::CompositorAnimationObserver* observer)
    {
