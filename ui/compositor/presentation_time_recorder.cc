...
class PresentationTimeRecorder::PresentationTimeRecorderInternal : public ui::CompositorObserver {
public:
    ...
    PresentationTimeRecorderInternal(const PresentationTimeRecorderInternal&) = delete;
    PresentationTimeRecorderInternal& operator=(const PresentationTimeRecorderInternal&) = delete;
...
protected:
    ...
    ~PresentationTimeRecorderInternal() override
    {
        ...
        VLOG(1) << "Finished Recording FrameTime: average latency=" << (average_latency ? average_latency->InMilliseconds() : 0)
...
private:
    ...
    public:
        ...
        ~Deleter()
        {
            recorder_internal_.ExtractAsDangling()->SelfDestruct();
        }
    ...
    void OnPresented(int count, base::TimeTicks requested_time, const viz::FrameTimingDetails& frame_timing_details);
    ...
    base::WeakPtrFactory<PresentationTimeRecorderInternal> weak_ptr_factory_ { this };
};
...
bool PresentationTimeRecorder::PresentationTimeRecorderInternal::RequestNext()
{
    ...
    VLOG(1) << "Start Next (" << request_count_ << ") state=" << (state_ == COMMITTED ? "Committed" : "Presented");
    state_ = REQUESTED;
    ...
    compositor_->RequestSuccessfulPresentationTimeForNextFrame(
        base::BindOnce(&PresentationTimeRecorderInternal::OnPresented, weak_ptr_factory_.GetWeakPtr(), request_count_++, now));
    return true;
}

void PresentationTimeRecorder::PresentationTimeRecorderInternal::OnPresented(
    int count, base::TimeTicks requested_time, const viz::FrameTimingDetails& frame_timing_details)
{
    ...
    if (delta.InMilliseconds() < 0) {
        LOG(ERROR) << "Invalid timestamp for presentation feedback (" << count << "): requested_time=" << requested_time
...
void PresentationTimeRecorder::PresentationTimeRecorderInternal::SelfDestruct()
{
    delete this;
}
...
PresentationTimeRecorder::PresentationTimeRecorder(raw_ptr<PresentationTimeRecorderInternal> internal)
...
std::optional<base::TimeDelta> PresentationTimeRecorder::GetAverageLatency() const
{
    return recorder_internal_->GetAverageLatency();
}

// static
void PresentationTimeRecorder::SetReportPresentationTimeImmediatelyForTest(bool enable)
{
...
base::HistogramBase* CreateTimesHistogram(const char* name, const PresentationTimeRecorder::BucketParams& bucket_params)
{
    return base::Histogram::FactoryTimeGet(
        name, bucket_params.min_latency, bucket_params.max_latency, bucket_params.num_buckets, base::HistogramBase::kUmaTargetedHistogramFlag);
}

// PresentationTimeHistogramRecorder ------------------------------------------

class PresentationTimeHistogramRecorder : public PresentationTimeRecorder::PresentationTimeRecorderInternal {
public:
    ...
    PresentationTimeHistogramRecorder(ui::Compositor* compositor, const char* presentation_time_histogram_name, const char* max_latency_histogram_name,
        const PresentationTimeRecorder::BucketParams& bucket_params, bool emit_trace_event)
        : PresentationTimeRecorderInternal(compositor)
        , presentation_time_histogram_(CreateTimesHistogram(presentation_time_histogram_name, bucket_params))
        , max_latency_histogram_name_(max_latency_histogram_name)
        , bucket_params_(bucket_params)
        , presentation_time_histogram_name_(emit_trace_event ? presentation_time_histogram_name : nullptr)
    {
        if (presentation_time_histogram_name_) {
            TRACE_EVENT_NESTABLE_ASYNC_BEGIN0("ui", presentation_time_histogram_name_, this);
        }
    }

    PresentationTimeHistogramRecorder(const PresentationTimeHistogramRecorder&) = delete;
    PresentationTimeHistogramRecorder& operator=(const PresentationTimeHistogramRecorder&) = delete;

    // PresentationTimeRecorderInternal:
    void ReportTime(base::TimeDelta delta) override
    {
        if (presentation_time_histogram_name_) {
            TRACE_EVENT_NESTABLE_ASYNC_END0("ui", presentation_time_histogram_name_, this);
        }
...
private:
    ~PresentationTimeHistogramRecorder() override
    {
        if (present_count() > 0 && !max_latency_histogram_name_.empty()) {
            CreateTimesHistogram(max_latency_histogram_name_.c_str(), bucket_params_)->AddTimeMillisecondsGranularity(base::Milliseconds(max_latency_ms()));
        }
    }
...
PresentationTimeRecorder::BucketParams::BucketParams(base::TimeDelta min_latency, base::TimeDelta max_latency, int num_buckets)
...
PresentationTimeRecorder::BucketParams::BucketParams(const BucketParams&) = default;

PresentationTimeRecorder::BucketParams& PresentationTimeRecorder::BucketParams::operator=(const BucketParams&) = default;

PresentationTimeRecorder::BucketParams::~BucketParams() = default;

// static
PresentationTimeRecorder::BucketParams PresentationTimeRecorder::BucketParams::CreateWithMaximum(base::TimeDelta max_latency)
{
    BucketParams params;
...
std::unique_ptr<PresentationTimeRecorder> CreatePresentationTimeHistogramRecorder(ui::Compositor* compositor, const char* presentation_time_histogram_name,
    const char* max_latency_histogram_name, PresentationTimeRecorder::BucketParams bucket_params, bool emit_trace_event)
{
    return std::make_unique<PresentationTimeRecorder>(
        new PresentationTimeHistogramRecorder(compositor, presentation_time_histogram_name, max_latency_histogram_name, bucket_params, emit_trace_event));
}
...
void PresentationTimeRecorder::TestApi::OnCompositingDidCommit(ui::Compositor* compositor)
{
    recorder_->recorder_internal_->OnCompositingDidCommit(compositor);
}

void PresentationTimeRecorder::TestApi::OnPresented(int count, base::TimeTicks requested_time, const viz::FrameTimingDetails& frame_timing_details)
{
    recorder_->recorder_internal_->OnPresented(count, requested_time, frame_timing_details);
}
