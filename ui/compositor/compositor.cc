...
struct PendingBeginFrameArgs {
    PendingBeginFrameArgs(const viz::BeginFrameArgs& args, bool force, base::OnceCallback<void(const viz::BeginFrameAck&)> callback)
        : args(args)
        , force(force)
...
Compositor::Compositor(const viz::FrameSinkId& frame_sink_id, ui::ContextFactory* context_factory, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    bool enable_pixel_canvas, bool use_external_begin_frame_control, bool force_software_compositor, bool enable_compositing_based_throttling,
    ...
    , lock_manager_(task_runner)
{
    ...
    host_frame_sink_manager->RegisterFrameSinkId(frame_sink_id_, this, viz::ReportFirstSurfaceActivation::kNo);
    ...
    settings.gpu_rasterization_disabled = !features::IsUiGpuRasterizationEnabled();

    if (command_line->HasSwitch(switches::kUIShowCompositedLayerBorders)) {
        std::string layer_borders_string = command_line->GetSwitchValueASCII(switches::kUIShowCompositedLayerBorders);
        std::vector<std::string_view> entries = base::SplitStringPiece(layer_borders_string, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
        if (entries.empty()) {
            ...
            for (const auto& entry : entries) {
                ...
                } kBorders[] = { { switches::kCompositedRenderPassBorders, cc::DebugBorderType::RENDERPASS },
                    { switches::kCompositedSurfaceBorders, cc::DebugBorderType::SURFACE }, { switches::kCompositedLayerBorders, cc::DebugBorderType::LAYER } };
    ...
    settings.initial_debug_state.show_fps_counter = command_line->HasSwitch(switches::kUIShowFPSCounter);
    settings.initial_debug_state.show_layer_animation_bounds_rects = command_line->HasSwitch(switches::kUIShowLayerAnimationBounds);
    settings.initial_debug_state.show_paint_rects = command_line->HasSwitch(switches::kUIShowPaintRects);
    settings.initial_debug_state.show_property_changed_rects = command_line->HasSwitch(switches::kUIShowPropertyChangedRects);
    settings.initial_debug_state.show_surface_damage_rects = command_line->HasSwitch(switches::kUIShowSurfaceDamageRects);
    settings.initial_debug_state.show_screen_space_rects = command_line->HasSwitch(switches::kUIShowScreenSpaceRects);

    settings.initial_debug_state.SetRecordRenderingStats(command_line->HasSwitch(switches::kEnableGpuBenchmarking));
    ...
    settings.use_partial_raster = !(settings.use_zero_copy || features::IsUsingRawDraw());

    settings.use_rgba_4444 = command_line->HasSwitch(switches::kUIEnableRGBA4444Textures);
    ...
    settings.use_gpu_memory_buffer_resources = features::IsDelegatedCompositingEnabled();
#endif
    ...
    settings.memory_policy.bytes_limit_when_visible = (memory_limit_when_visible_mb > 0 ? memory_limit_when_visible_mb : 512) * 1024 * 1024;

    settings.memory_policy.priority_cutoff_when_visible = gpu::MemoryAllocation::CUTOFF_ALLOW_NICE_TO_HAVE;

    settings.disallow_non_exact_resource_reuse = command_line->HasSwitch(switches::kDisallowNonExactResourceReuse);

    settings.wait_for_all_pipeline_stages_before_draw = command_line->HasSwitch(switches::kRunAllCompositorStagesBeforeDraw);

    if (features::IsPercentBasedScrollingEnabled()) {
        settings.percent_based_scrolling = true;
    }

    settings.enable_compositing_based_throttling = enable_compositing_based_throttling;
    ...
    settings.enable_shared_image_cache_for_gpu = base::FeatureList::IsEnabled(features::kUIEnableSharedImageCacheForGpu);
    ...
    const base::WeakPtr<cc::CompositorDelegateForInput>& compositor_delegate = host_->GetDelegateForInput();
    if (base::FeatureList::IsEnabled(features::kUiCompositorScrollWithLayers) && compositor_delegate) {
        input_handler_weak_ = cc::InputHandler::Create(*compositor_delegate);
        scroll_input_handler_ = std::make_unique<ScrollInputHandler>(input_handler_weak_);
    }

    animation_timeline_ = cc::AnimationTimeline::Create(cc::AnimationIdProvider::NextTimelineId());
    ...
    if (auto* power_monitor = base::PowerMonitor::GetInstance(); power_monitor->IsInitialized()) {
        power_monitor->AddPowerSuspendObserver(this);
    }

    if (command_line->HasSwitch(switches::kUISlowAnimations)) {
        slow_animations_ = std::make_unique<ScopedAnimationDurationScaleMode>(ScopedAnimationDurationScaleMode::SLOW_DURATION);
    }

    settings.disable_frame_rate_limit = command_line->HasSwitch(switches::kDisableFrameRateLimit);
}
...
Compositor::~Compositor()
{
    ...
    animation_observer_list_.Notify(&CompositorAnimationObserver::OnCompositingShuttingDown, this);

    simple_begin_frame_observers_.Notify(&ui::HostBeginFrameObserver::SimpleBeginFrameObserver::OnBeginFrameSourceShuttingDown);
    ...
    for (auto& client : child_frame_sinks_) {
        ...
        host_frame_sink_manager->UnregisterFrameSinkHierarchy(frame_sink_id_, client);
    }
...
{
    context_factory_->GetHostFrameSinkManager()->RegisterFrameSinkHierarchy(frame_sink_id_, frame_sink_id);
...
void Compositor::RemoveChildFrameSink(const viz::FrameSinkId& frame_sink_id)
{
    ...
    DCHECK(it->is_valid());
    context_factory_->GetHostFrameSinkManager()->UnregisterFrameSinkHierarchy(frame_sink_id_, *it);
    child_frame_sinks_.erase(it);
}

void Compositor::SetLayerTreeFrameSink(
    std::unique_ptr<cc::LayerTreeFrameSink> layer_tree_frame_sink, mojo::AssociatedRemote<viz::mojom::DisplayPrivate> display_private)
{
    ...
    if (display_private_) {
        ...
        display_private_->SetDisplayColorMatrix(gfx::SkM44ToTransform(display_color_matrix_));
        display_private_->SetOutputIsSecure(output_is_secure_);
#if BUILDFLAG(IS_MAC)
        display_private_->SetVSyncDisplayID(display_id_);
#endif
        if (has_vsync_params_) {
            display_private_->SetDisplayVSyncParameters(vsync_timebase_, vsync_interval_);
        }
...
void Compositor::SetExternalBeginFrameController(mojo::AssociatedRemote<viz::mojom::ExternalBeginFrameController> external_begin_frame_controller)
{
    ...
    if (pending_begin_frame_args_) {
        external_begin_frame_controller_->IssueExternalBeginFrame(
            pending_begin_frame_args_->args, pending_begin_frame_args_->force, std::move(pending_begin_frame_args_->callback));
...
void Compositor::SetScaleAndSize(float scale, const gfx::Size& size_in_pixel, const viz::LocalSurfaceId& local_surface_id)
{
    DCHECK_GT(scale, 0);
    ...
    if (!size_in_pixel.IsEmpty()) {
        ...
        size_ = size_in_pixel;
        host_->SetViewportRectAndScale(gfx::Rect(size_in_pixel), scale, local_surface_id);
...
void Compositor::SetDisplayColorSpaces(const gfx::DisplayColorSpaces& display_color_spaces)
{
    if (display_color_spaces_ == display_color_spaces)
        return;

    bool only_hdr_headroom_changed = gfx::DisplayColorSpaces::EqualExceptForHdrHeadroom(display_color_spaces_, display_color_spaces);
...
void Compositor::SetVisible(bool visible)
{
    ...
    if (changed) {
        observer_list_.Notify(&CompositorObserver::OnCompositorVisibilityChanging, this, visible);
    }
    ...
    if (changed) {
        observer_list_.Notify(&CompositorObserver::OnCompositorVisibilityChanged, this, visible);
    }
}
...
bool Compositor::ScrollLayerTo(cc::ElementId element_id, const gfx::PointF& offset)
{
    return input_handler_weak_ && input_handler_weak_->ScrollLayerTo(element_id, offset);
}

bool Compositor::GetScrollOffsetForLayer(cc::ElementId element_id, gfx::PointF* offset) const
{
    return input_handler_weak_ && input_handler_weak_->GetScrollOffsetForLayer(element_id, offset);
}

void Compositor::SetDisplayVSyncParameters(base::TimeTicks timebase, base::TimeDelta interval)
{
    static bool is_frame_rate_limit_disabled = base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableFrameRateLimit);
...
void Compositor::AddVSyncParameterObserver(mojo::PendingRemote<viz::mojom::VSyncParameterObserver> observer)
{
    if (display_private_)
        display_private_->AddVSyncParameterObserver(std::move(observer));
}

void Compositor::SetMaxVSyncAndVrr(const std::optional<base::TimeDelta>& max_vsync_interval, display::VariableRefreshRateState vrr_state)
{
...
void Compositor::SetAcceleratedWidget(gfx::AcceleratedWidget widget)
{
    ...
    if (layer_tree_frame_sink_requested_) {
        context_factory_->CreateLayerTreeFrameSink(context_creation_weak_ptr_factory_.GetWeakPtr());
    }
}
...
void Compositor::RemoveAnimationObserver(CompositorAnimationObserver* observer)
{
...
bool Compositor::HasAnimationObserver(const CompositorAnimationObserver* observer) const
{
    return animation_observer_list_.HasObserver(observer);
}

void Compositor::IssueExternalBeginFrame(const viz::BeginFrameArgs& args, bool force, base::OnceCallback<void(const viz::BeginFrameAck&)> callback)
{
    if (!external_begin_frame_controller_) {
        ...
        pending_begin_frame_args_ = std::make_unique<PendingBeginFrameArgs>(args, force, std::move(callback));
        return;
    }
    external_begin_frame_controller_->IssueExternalBeginFrame(args, force, std::move(callback));
}

ThroughputTracker Compositor::RequestNewThroughputTracker()
{
    return ThroughputTracker(next_throughput_tracker_id_++, weak_ptr_factory_.GetWeakPtr());
}
...
std::unique_ptr<cc::EventsMetricsManager::ScopedMonitor> Compositor::GetScopedEventMetricsMonitor(
...
{
    DCHECK(!IsLocked());
    animation_observer_list_.Notify(&CompositorAnimationObserver::OnAnimationStep, args.frame_time);
    if (!animation_observer_list_.empty()) {
        host_->SetNeedsAnimate();
        ...
        observer_list_.Notify(&CompositorObserver::OnFirstNonAnimatedFrameStarted, this);
    }
}
...
{
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&Compositor::RequestNewLayerTreeFrameSink, context_creation_weak_ptr_factory_.GetWeakPtr()));
}

void Compositor::DidCommit(int source_frame_number, base::TimeTicks, base::TimeTicks)
{
    DCHECK(!IsLocked());
    observer_list_.Notify(&CompositorObserver::OnCompositingDidCommit, this);
}

std::unique_ptr<cc::BeginMainFrameMetrics> Compositor::GetBeginMainFrameMetrics()
{
#if BUILDFLAG(IS_CHROMEOS)
...
void Compositor::DidPresentCompositorFrame(uint32_t frame_token, const viz::FrameTimingDetails& frame_timing_details)
{
    TRACE_EVENT_MARK_WITH_TIMESTAMP1("cc,benchmark", "FramePresented", frame_timing_details.presentation_feedback.timestamp, "environment", "browser");
    observer_list_.Notify(&CompositorObserver::OnDidPresentCompositorFrame, frame_token, frame_timing_details.presentation_feedback);
}

void Compositor::DidSubmitCompositorFrame()
{
    base::TimeTicks start_time = base::TimeTicks::Now();
    observer_list_.Notify(&CompositorObserver::OnCompositingStarted, this, start_time);
}
...
void Compositor::FrameSinksToThrottleUpdated(const base::flat_set<viz::FrameSinkId>& ids)
{
    observer_list_.Notify(&CompositorObserver::OnFrameSinksToThrottleUpdated, ids);
}
...
void Compositor::OnFrameTokenChanged(uint32_t frame_token, base::TimeTicks activation_time)
{
...
Compositor::TrackerState& Compositor::TrackerState::operator=(TrackerState&&) = default;
Compositor::TrackerState::~TrackerState() = default;

void Compositor::StartThroughputTracker(TrackerId tracker_id, ThroughputTrackerHost::ReportCallback callback)
{
...
{
    observer_list_.Notify(&CompositorObserver::OnCompositingCompleteSwapWithNewSize, this, size);
}
...
void Compositor::SetLayerTreeDebugState(const cc::LayerTreeDebugState& debug_state)
{
    host_->SetDebugState(debug_state);
}

void Compositor::RequestPresentationTimeForNextFrame(PresentationTimeCallback callback)
{
    host_->RequestPresentationTimeForNextFrame(std::move(callback));
}

void Compositor::RequestSuccessfulPresentationTimeForNextFrame(SuccessfulPresentationTimeCallback callback)
{
    host_->RequestSuccessfulPresentationTimeForNextFrame(std::move(callback));
}

void Compositor::ReportMetricsForTracker(int tracker_id, const cc::FrameSequenceMetrics::CustomReportData& data)
{
    auto it = throughput_tracker_map_.find(tracker_id);
...
void Compositor::SetDelegatedInkPointRenderer(mojo::PendingReceiver<gfx::mojom::DelegatedInkPointRenderer> receiver)
{
    if (display_private_)
...
void Compositor::AddSimpleBeginFrameObserver(ui::HostBeginFrameObserver::SimpleBeginFrameObserver* obs)
{
    DCHECK(obs);
...
void Compositor::RemoveSimpleBeginFrameObserver(ui::HostBeginFrameObserver::SimpleBeginFrameObserver* obs)
{
    DCHECK(obs);
...
void Compositor::MaybeUpdateObserveBeginFrame()
{
    ...
    host_begin_frame_observer_ = std::make_unique<ui::HostBeginFrameObserver>(simple_begin_frame_observers_, task_runner_);
    display_private_->SetStandaloneBeginFrameObserver(host_begin_frame_observer_->GetBoundRemote());
}

#if BUILDFLAG(IS_CHROMEOS)
void Compositor::SetSeamlessRefreshRates(const std::vector<float>& seamless_refresh_rates)
{
...
{
    observer_list_.Notify(&CompositorObserver::OnSetPreferredRefreshRate, this, refresh_rate);
}
#endif // BUILDFLAG(IS_CHROMEOS)
