...
public:
    ...
    virtual void OnLayerAnimationAborted(LayerAnimationSequence* sequence) = 0;

    // Called when the animation is scheduled.
    virtual void OnLayerAnimationScheduled(LayerAnimationSequence* sequence) = 0;

protected:
    typedef std::set<raw_ptr<LayerAnimationSequence, SetExperimental>> AttachedSequences;
...
private:
    ...
    void DetachedFromSequence(LayerAnimationSequence* sequence, bool send_notification);
...
class COMPOSITOR_EXPORT ImplicitAnimationObserver : public LayerAnimationObserver {
public:
...
private:
    ...
    void UpdatePropertyAnimationStatus(LayerAnimationSequence* sequence, AnimationStatus status);
    AnimationStatus AnimationStatusForProperty(LayerAnimationElement::AnimatableProperty property) const;

    bool active_ = false;

    typedef std::map<LayerAnimationElement::AnimatableProperty, AnimationStatus> PropertyAnimationStatusMap;
