...
TEST(LayerAnimationSequenceTest, NoElement)
{
    ...
    EXPECT_EQ(static_cast<LayerAnimationElement::AnimatableProperties>(LayerAnimationElement::UNKNOWN), sequence.properties());
...
TEST(LayerAnimationSequenceTest, SingleElement)
{
    ...
    sequence.AddElement(LayerAnimationElement::CreateBrightnessElement(target, delta));
    ...
    EXPECT_EQ(static_cast<LayerAnimationElement::AnimatableProperties>(LayerAnimationElement::BRIGHTNESS), sequence.properties());
}
...
TEST(LayerAnimationSequenceTest, SingleThreadedElement)
{
    ...
    base::TimeDelta delta = base::Seconds(1);
    sequence.AddElement(LayerAnimationElement::CreateOpacityElement(target, delta));
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        sequence.OnThreadedAnimationStarted(effective_start, cc::TargetProperty::OPACITY, sequence.animation_group_id());
    ...
    EXPECT_EQ(static_cast<LayerAnimationElement::AnimatableProperties>(LayerAnimationElement::OPACITY), sequence.properties());
}
...
TEST(LayerAnimationSequenceTest, MultipleElement)
{
    ...
    base::TimeDelta delta = base::Seconds(1);
    sequence.AddElement(LayerAnimationElement::CreateOpacityElement(target_opacity, delta));

    // Pause bounds for a second.
    sequence.AddElement(LayerAnimationElement::CreatePauseElement(LayerAnimationElement::BOUNDS, delta));

    gfx::Transform start_transform, target_transform;
    start_transform.Rotate(-30.0);
    target_transform.Rotate(30.0);

    sequence.AddElement(LayerAnimationElement::CreateTransformElement(target_transform, delta));
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        sequence.OnThreadedAnimationStarted(opacity_effective_start, cc::TargetProperty::OPACITY, sequence.animation_group_id());
        ...
        sequence.Progress(opacity_effective_start + delta + delta / 2, &delegate);
        ...
        sequence.OnThreadedAnimationStarted(transform_effective_start, cc::TargetProperty::TRANSFORM, sequence.animation_group_id());
...
TEST(LayerAnimationSequenceTest, AbortingRepeatingSequence)
{
    ...
    base::TimeTicks start_time;
    base::TimeDelta delta = base::Seconds(1);
    sequence.AddElement(LayerAnimationElement::CreateBrightnessElement(target_brightness, delta));
...
TEST(LayerAnimationSequenceTest, AddObserver)
{
    ...
    base::TimeDelta delta = base::Seconds(1);
    LayerAnimationSequence sequence;
    sequence.AddElement(LayerAnimationElement::CreateBrightnessElement(1.0f, delta));
    for (int i = 0; i < 2; ++i) {
        start_time += delta;
        sequence.set_start_time(start_time);
...
TEST(LayerAnimationSequenceTest, ToString)
{
    base::TimeDelta delta = base::Seconds(1);
    LayerAnimationSequence sequence;
    EXPECT_EQ("LayerAnimationSequence{size=0, properties=, elements=[], "
              "is_repeating=0, "
              "group_id=0}",
        sequence.ToString());

    std::unique_ptr<LayerAnimationElement> brightness = LayerAnimationElement::CreateBrightnessElement(1.0f, delta);
    int brightness_id = brightness->keyframe_model_id();
    sequence.AddElement(std::move(brightness));
    EXPECT_EQ(base::StringPrintf("LayerAnimationSequence{size=1, properties=BRIGHTNESS, "
    ...
    std::unique_ptr<LayerAnimationElement> opacity = LayerAnimationElement::CreateOpacityElement(1.0f, delta);
    int opacity_id = opacity->keyframe_model_id();
    sequence.AddElement(std::move(opacity));
    sequence.set_is_repeating(true);
    sequence.set_animation_group_id(1973);
    EXPECT_EQ(base::StringPrintf("LayerAnimationSequence{size=2, properties=OPACITY|BRIGHTNESS, "
                                 "elements=[LayerAnimationElement{name=BrightnessTransition, id=%d, "
                                 "group=0, last_progressed_fraction=0.00}, "
...
TEST(LayerAnimationSequenceTest, RepeatingWithZeroDuration)
{
    ...
    LayerAnimationSequence sequence;
    sequence.set_is_repeating(true);
    sequence.AddElement(LayerAnimationElement::CreateBrightnessElement(0.5f, base::TimeDelta()));
    sequence.AddElement(LayerAnimationElement::CreateBrightnessElement(final_brightness, base::TimeDelta()));
    TestLayerAnimationDelegate delegate;
    delegate.SetBrightnessFromAnimation(0.f, PropertyChangeReason::NOT_FROM_ANIMATION);
