...
class COMPOSITOR_EXPORT TotalAnimationThroughputReporter : public CompositorObserver {
public:
    ...
    public:
        explicit ScopedThroughputReporterBlocker(base::WeakPtr<TotalAnimationThroughputReporter> reporter);
        ScopedThroughputReporterBlocker(const ScopedThroughputReporterBlocker&) = delete;
        ~ScopedThroughputReporterBlocker();

        ScopedThroughputReporterBlocker& operator=(const ScopedThroughputReporterBlocker&) = delete;
    ...
    using ReportOnceCallback = base::OnceCallback<void(
        const cc::FrameSequenceMetrics::CustomReportData& data, base::TimeTicks first_animation_started_at, base::TimeTicks last_animation_finished_at)>;
    using ReportRepeatingCallback = base::RepeatingCallback<void(const cc::FrameSequenceMetrics::CustomReportData& data)>;
    ...
    // invoked.
    TotalAnimationThroughputReporter(Compositor* compositor, ReportOnceCallback callback, bool should_delete);
    ...
    TotalAnimationThroughputReporter(Compositor* compositor, ReportRepeatingCallback callback);

    TotalAnimationThroughputReporter(const TotalAnimationThroughputReporter&) = delete;
    TotalAnimationThroughputReporter& operator=(const TotalAnimationThroughputReporter&) = delete;
    ...
    {
        return bool { throughput_tracker_ };
    }
...
private:
    ...
    TotalAnimationThroughputReporter(Compositor* compositor, ReportRepeatingCallback repeating_callback, ReportOnceCallback once_callback, bool should_delete);
