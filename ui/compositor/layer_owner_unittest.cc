...
public:
    explicit TestLayerAnimationObserver(Layer* layer)
        : layer_(layer)
    {
    }
...
public:
    ...
    LayerOwnerTestWithCompositor& operator=(const LayerOwnerTestWithCompositor&) = delete;
...
void LayerOwnerTestWithCompositor::SetUp()
{
    ...
    compositor_ = std::make_unique<ui::Compositor>(
        context_factories_->GetContextFactory()->AllocateFrameSinkId(), context_factories_->GetContextFactory(), task_runner, false /* enable_pixel_canvas */);
...
TEST_F(LayerOwnerTestWithCompositor, RecreateRootLayerDuringAnimation)
{
    ...
    std::unique_ptr<TestLayerAnimationObserver> observer(new TestLayerAnimationObserver(child.get()));
    std::unique_ptr<ui::ScopedAnimationDurationScaleMode> long_duration_animation(
        new ui::ScopedAnimationDurationScaleMode(ui::ScopedAnimationDurationScaleMode::SLOW_DURATION));
    {
...
TEST_F(LayerOwnerTestWithCompositor, DetachTimelineOnAnimatorDeletion)
{
    ...
    scoped_refptr<cc::Animation> animation = layer->GetAnimator()->GetAnimationForTesting();
...
TEST_F(LayerOwnerTestWithCompositor, AttachTimelineIfAnimatorCreatedAfterSetCompositor)
{
    std::unique_ptr<Layer> root_layer(new Layer);
    compositor()->SetRootLayer(root_layer.get());
...
public:
    explicit TestLayerDelegate(ui::LayerOwner* owner)
        : owner_(owner)
    {
    }
    ...
    void OnLayerBoundsChanged(const gfx::Rect& old_bounds, PropertyChangeReason reason) override
    {
