...
class COMPOSITOR_EXPORT LayerAnimator : public base::RefCounted<LayerAnimator>, public LayerThreadedAnimationDelegate, public cc::AnimationDelegate {
public:
    enum PreemptionStrategy { IMMEDIATELY_SET_NEW_TARGET, IMMEDIATELY_ANIMATE_TO_NEW_TARGET, ENQUEUE_NEW_ANIMATION, REPLACE_QUEUED_ANIMATIONS };

    using SequenceScheduledCallbacks = base::RepeatingCallbackList<void(LayerAnimationSequence*)>;
    ...
    // End the list with -1.
    void SchedulePauseForProperties(base::TimeDelta duration, LayerAnimationElement::AnimatableProperties properties_to_pause);
    ...
    bool IsAnimatingProperty(LayerAnimationElement::AnimatableProperty property) const
    {
    ...
    bool IsAnimatingOnePropertyOf(LayerAnimationElement::AnimatableProperties properties) const;
    ...
    // animation.
    void StopAnimatingProperty(LayerAnimationElement::AnimatableProperty property);
    ...
    void AddOwnedObserver(std::unique_ptr<ImplicitAnimationObserver> animation_observer);
    void RemoveAndDestroyOwnedObserver(ImplicitAnimationObserver* animation_observer);
    ...
    [[nodiscard]] base::CallbackListSubscription AddSequenceScheduledCallback(SequenceScheduledCallback callback);

    // Called when a threaded animation is actually started.
    void OnThreadedAnimationStarted(base::TimeTicks monotonic_time, cc::TargetProperty::Type target_property, int group_id);
    ...
    // is Tween::LINEAR.
    void set_tween_type(gfx::Tween::Type tween_type)
    {
        tween_type_ = tween_type;
    }
    gfx::Tween::Type tween_type() const
    {
        return tween_type_;
    }
    ...
    base::TimeTicks last_step_time() const
    {
...
protected:
    ...
    const LayerAnimationDelegate* delegate() const
    {
        return delegate_;
    }

    // Virtual for testing.
    virtual void ProgressAnimation(LayerAnimationSequence* sequence, base::TimeTicks now);
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(LayerAnimatorTest, AnimatorRemovedFromCollectionWhenLayerIsDestroyed);
    ...
    public:
        ...
        bool is_sequence_alive() const
        {
            return !!sequence_.get();
        }
        LayerAnimationSequence* sequence() const
        {
            return sequence_.get();
        }
    ...
    using AnimationQueue = base::circular_deque<std::unique_ptr<LayerAnimationSequence>>;
    ...
    [[nodiscard]] LayerAnimationSequence* RemoveAnimation(LayerAnimationSequence* sequence);
    ...
    RunningAnimation* GetRunningAnimation(LayerAnimationElement::AnimatableProperty property);
    ...
    void RemoveAllAnimationsWithACommonProperty(LayerAnimationSequence* sequence, bool abort);
    ...
    void NotifyAnimationStarted(base::TimeTicks monotonic_time, int target_property, int group_id) override;
    void NotifyAnimationFinished(base::TimeTicks monotonic_time, int target_property, int group_id) override
    {
    }
    void NotifyAnimationAborted(base::TimeTicks monotonic_time, int target_property, int group_id) override
    {
    }
    ...
    void AddThreadedAnimation(std::unique_ptr<cc::KeyframeModel> keyframe_model) override;
    ...
    // related code.
    base::ObserverList<LayerAnimationObserver>::UncheckedAndDanglingUntriaged observers_;
