...
public:
    ...
    virtual void OnCompositingStarted(Compositor* compositor, base::TimeTicks start_time)
    {
    }
    ...
    virtual void OnCompositingCompleteSwapWithNewSize(ui::Compositor* compositor, const gfx::Size& size)
    {
    }
    ...
    virtual void OnDidPresentCompositorFrame(uint32_t frame_token, const gfx::PresentationFeedback& feedback)
    {
    }
    ...
    virtual void OnFrameSinksToThrottleUpdated(const base::flat_set<viz::FrameSinkId>& ids)
    {
    }
    ...
    // changed.
    virtual void OnCompositorVisibilityChanging(Compositor* compositor, bool visible)
    {
    }

    // Called when the compositor visibility has changed.
    virtual void OnCompositorVisibilityChanged(Compositor* compositor, bool visible)
    {
    }

    // Called when the compositor receives a new refresh rate preference.
    virtual void OnSetPreferredRefreshRate(Compositor* compositor, float refresh_rate)
    {
    }
};
