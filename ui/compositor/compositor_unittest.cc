...
public:
    MOCK_METHOD2(OnCompositorVisibilityChanging, void(Compositor* compositor, bool visible));
    MOCK_METHOD2(OnCompositorVisibilityChanged, void(Compositor* compositor, bool visible));
};
...
public:
    ...
    {
        compositor_ = std::make_unique<Compositor>(context_factories_->GetContextFactory()->AllocateFrameSinkId(), context_factories_->GetContextFactory(),
            CreateTaskRunner(), false /* enable_pixel_canvas */);
...
private:
    base::test::TaskEnvironment task_environment_ { base::test::TaskEnvironment::MainThreadType::UI, base::test::TaskEnvironment::TimeSource::MOCK_TIME };
};
...
TEST_F(CompositorTestWithMessageLoop, ShouldUpdateDisplayProperties)
{
    ...
    compositor()->SetScaleAndSize(1.0f, gfx::Size(10, 10), allocator.GetCurrentLocalSurfaceId());
    ...
    gfx::DisplayColorSpaces display_color_spaces(gfx::ColorSpace::CreateDisplayP3D65());
    ...
    compositor()->SetMaxVSyncAndVrr(max_vsync_interval, display::VariableRefreshRateState::kVrrEnabled);

    InProcessContextFactory* context_factory = static_cast<InProcessContextFactory*>(compositor()->context_factory());
    compositor()->ScheduleDraw();
    DrawWaiterForTest::WaitForCompositingEnded(compositor());
    EXPECT_EQ(color_matrix, context_factory->GetOutputColorMatrix(compositor()));
    EXPECT_EQ(display_color_spaces, context_factory->GetDisplayColorSpaces(compositor()));
    EXPECT_EQ(vsync_timebase, context_factory->GetDisplayVSyncTimeBase(compositor()));
    EXPECT_EQ(vsync_interval, context_factory->GetDisplayVSyncTimeInterval(compositor()));
    EXPECT_EQ(max_vsync_interval, context_factory->GetMaxVSyncInterval(compositor()));
    EXPECT_EQ(display::VariableRefreshRateState::kVrrEnabled, context_factory->GetVrrState(compositor()));
    ...
    EXPECT_EQ(gfx::kNullAcceleratedWidget, compositor()->ReleaseAcceleratedWidget());
...
TEST_F(CompositorTestWithMockedTime, ReleaseWidgetWithOutputSurfaceNeverCreated)
{
    compositor()->SetVisible(false);
    EXPECT_EQ(gfx::kNullAcceleratedWidget, compositor()->ReleaseAcceleratedWidget());
    compositor()->SetAcceleratedWidget(gfx::kNullAcceleratedWidget);
...
TEST_F(CompositorTestWithMessageLoop, MoveThroughputTracker)
{
    ...
    {
        auto tracker = compositor()->RequestNewThroughputTracker();
        tracker.Start(base::BindLambdaForTesting([&](const cc::FrameSequenceMetrics::CustomReportData& data) {
...
{
    auto tracker = compositor()->RequestNewThroughputTracker();
    tracker.Start(base::BindLambdaForTesting([&](const cc::FrameSequenceMetrics::CustomReportData& data) { ADD_FAILURE() << "No report should happen"; }));
...
TEST_F(CompositorTestWithMessageLoop, ThroughputTrackerCallbackStateChange)
{
    ...
    tracker.Start(base::BindLambdaForTesting([&](const cc::FrameSequenceMetrics::CustomReportData& data) {
        ...
        ThroughputTracker another_tracker = compositor()->RequestNewThroughputTracker();
...
#define MAYBE_CreateAndReleaseOutputSurface DISABLED_CreateAndReleaseOutputSurface
#else
...
{
    std::unique_ptr<Layer> root_layer = std::make_unique<Layer>(ui::LAYER_SOLID_COLOR);
    std::unique_ptr<Layer> child_layer = std::make_unique<Layer>(ui::LAYER_TEXTURED);
    std::unique_ptr<Layer> child_layer2 = std::make_unique<Layer>(ui::LAYER_SOLID_COLOR);
    LayerDelegateThatAddsDuringUpdateVisualState child_layer_delegate(root_layer.get());
...
TEST_F(CompositorTestWithMessageLoop, CompositorVisibilityChanges)
{
    ...
    EXPECT_CALL(observer, OnCompositorVisibilityChanging(compositor(), false)).Times(1);
    EXPECT_CALL(observer, OnCompositorVisibilityChanged(compositor(), false)).Times(1);
    compositor()->SetVisible(false);
    ::testing::Mock::VerifyAndClearExpectations(&observer);

    EXPECT_CALL(observer, OnCompositorVisibilityChanging(compositor(), true)).Times(1);
    EXPECT_CALL(observer, OnCompositorVisibilityChanged(compositor(), true)).Times(1);
    compositor()->SetVisible(true);
    ::testing::Mock::VerifyAndClearExpectations(&observer);

    // Verify no calls if visibility isn't changed.
    EXPECT_CALL(observer, OnCompositorVisibilityChanging(compositor(), _)).Times(0);
    EXPECT_CALL(observer, OnCompositorVisibilityChanged(compositor(), _)).Times(0);
    compositor()->SetVisible(true);
    ::testing::Mock::VerifyAndClearExpectations(&observer);
