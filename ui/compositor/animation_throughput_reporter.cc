...
class AnimationThroughputReporter::AnimationTracker : public CallbackLayerAnimationObserver {
public:
    AnimationTracker(LayerAnimator* animator, ReportCallback report_callback)
        : CallbackLayerAnimationObserver(base::BindRepeating(&AnimationTracker::OnAnimationEnded, base::Unretained(this)))
        , animator_(animator)
    ...
    void set_should_delete(bool should_delete)
    {
...
private:
    ...
    void OnAnimatorAttachedToTimeline() override
    {
        MaybeStartTracking();
    }
    ...
    void OnLayerAnimationStarted(LayerAnimationSequence* sequence) override
    {
        ...
        if (!first_animation_group_id_.has_value() && !sequence->IsFinished(base::TimeTicks::Now())) {
    ...
    void OnLayerAnimationAborted(LayerAnimationSequence* sequence) override
    {
        ...
        if (first_animation_group_id_.has_value() && first_animation_group_id_.value() <= sequence->animation_group_id()) {
    ...
    void MaybeStartTracking()
    {
        ...
        ui::Compositor* compositor = AnimationThroughputReporter::GetCompositor(animator_);
...
AnimationThroughputReporter::AnimationThroughputReporter(scoped_refptr<LayerAnimator> animator, ReportCallback report_callback)
    : animator_(std::move(animator))
    , animation_tracker_(std::make_unique<AnimationTracker>(animator_.get(), std::move(report_callback)))
{
...
// static
Compositor* AnimationThroughputReporter::GetCompositor(LayerAnimator* animator)
{
    return animator->delegate()->GetLayer()->GetCompositor();
}

// static
bool AnimationThroughputReporter::IsAnimatorAttachedToTimeline(LayerAnimator* animator)
{
