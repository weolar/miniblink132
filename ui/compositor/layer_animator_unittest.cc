...
{
    return base::StringPrintf("%d %d %d %d", SkColorGetA(color), SkColorGetR(color), SkColorGetG(color), SkColorGetB(color));
}

// Creates vector with two LayerAnimationSequences, based on |first| and
// |second| layer animation elements.
std::vector<LayerAnimationSequence*> CreateMultiSequence(std::unique_ptr<LayerAnimationElement> first, std::unique_ptr<LayerAnimationElement> second)
{
...
scoped_refptr<LayerAnimator> CreateDefaultTestAnimator(LayerAnimationDelegate* delegate, LayerAnimationObserver* observer)
{
...
scoped_refptr<LayerAnimator> CreateDefaultTestAnimator(LayerAnimationDelegate* delegate)
{
...
// |observer| are attached if non-null.
scoped_refptr<LayerAnimator> CreateImplicitTestAnimator(LayerAnimationDelegate* delegate, LayerAnimationObserver* observer)
{
    scoped_refptr<LayerAnimator> animator = LayerAnimator::CreateImplicitAnimator();
    animator->set_disable_timer_for_test(true);
    if (delegate)
        animator->SetDelegate(delegate);
    if (observer)
...
// attached if non-null.
scoped_refptr<LayerAnimator> CreateImplicitTestAnimator(LayerAnimationDelegate* delegate)
{
...
public:
    ...
    TestImplicitAnimationObserver& operator=(const TestImplicitAnimationObserver&) = delete;

    bool animations_completed() const
    {
        return animations_completed_;
    }
    ...
    bool WasAnimationAbortedForProperty(LayerAnimationElement::AnimatableProperty property) const
    {
        return ImplicitAnimationObserver::WasAnimationAbortedForProperty(property);
    }

    bool WasAnimationCompletedForProperty(LayerAnimationElement::AnimatableProperty property) const
    {
        return ImplicitAnimationObserver::WasAnimationCompletedForProperty(property);
    }
...
public:
    ...
    DeletingLayerAnimationObserver(const DeletingLayerAnimationObserver&) = delete;
    DeletingLayerAnimationObserver& operator=(const DeletingLayerAnimationObserver&) = delete;
...
class AbortAnimationsOnStartedLayerAnimationObserver : public LayerAnimationObserver {
public:
    explicit AbortAnimationsOnStartedLayerAnimationObserver(LayerAnimator* animator)
        : animator_(animator)
    {
    }

    AbortAnimationsOnStartedLayerAnimationObserver(const AbortAnimationsOnStartedLayerAnimationObserver&) = delete;
    AbortAnimationsOnStartedLayerAnimationObserver& operator=(const AbortAnimationsOnStartedLayerAnimationObserver&) = delete;
...
public:
    ...
    LayerAnimatorDestructionObserver(const LayerAnimatorDestructionObserver&) = delete;
    LayerAnimatorDestructionObserver& operator=(const LayerAnimatorDestructionObserver&) = delete;
...
public:
    ...
    void SetDestructionObserver(LayerAnimatorDestructionObserver* observer)
    {
...
protected:
    ...
    void ProgressAnimation(LayerAnimationSequence* sequence, base::TimeTicks now) override
    {
...
public:
    TestLayerAnimationSequence(std::unique_ptr<LayerAnimationElement> element, int* num_live_instances)
    ...
    TestLayerAnimationSequence& operator=(const TestLayerAnimationSequence&) = delete;

    ~TestLayerAnimationSequence() override
    {
        (*num_live_instances_)--;
    }
...
public:
    ...
    CountCheckingLayerAnimationObserver(const CountCheckingLayerAnimationObserver&) = delete;
    CountCheckingLayerAnimationObserver& operator=(const CountCheckingLayerAnimationObserver&) = delete;
    ...
    {
        ++successful_count_;
        EXPECT_LE(GetNumSequencesCompleted(), sequences_scheduled_) << " aborted_count_=" << aborted_count_ << " successful_count_=" << successful_count_;
        EXPECT_LE(GetNumSequencesCompleted(), attached_sequence_count_) << " aborted_count_=" << aborted_count_ << " successful_count_=" << successful_count_;
        if (observer_)
...
protected:
    ...
    void OnDetachedFromSequence(LayerAnimationSequence* sequence) override
    {
        ...
        EXPECT_LE(detached_sequence_count_, GetNumSequencesCompleted()) << " aborted_count_=" << aborted_count_ << " successful_count_=" << successful_count_;
        if (observer_)
...
TEST(LayerAnimatorTest, NoImplicitAnimation)
{
    ...
    EXPECT_FLOAT_EQ(delegate.GetBrightnessForAnimation(), 0.5);
    delegate.ExpectLastPropertyChangeReason(PropertyChangeReason::NOT_FROM_ANIMATION);
}
...
TEST(LayerAnimatorTest, AbortAllAnimations)
{
    ...
    delegate.SetOpacityFromAnimation(initial_opacity, PropertyChangeReason::NOT_FROM_ANIMATION);
    delegate.SetBoundsFromAnimation(initial_bounds, PropertyChangeReason::NOT_FROM_ANIMATION);
    scoped_refptr<LayerAnimator> animator = CreateImplicitTestAnimator(&delegate);
    double target_opacity(0.5);
    gfx::Rect target_bounds(0, 0, 50, 50);
    animator->SetOpacity(target_opacity);
...
TEST(LayerAnimatorTest, ScheduleAnimationThatCanRunImmediately)
{
    ...
    delegate.SetBrightnessFromAnimation(start_brightness, PropertyChangeReason::NOT_FROM_ANIMATION);

    animator->ScheduleAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(target_brightness, delta)));
...
TEST(LayerAnimatorTest, ScheduleThreadedAnimationThatCanRunImmediately)
{
    ...
    TestLayerAnimationDelegate delegate;
    LayerAnimatorTestController test_controller(CreateDefaultTestAnimator(&delegate));
    ...
    delegate.SetOpacityFromAnimation(start_opacity, PropertyChangeReason::NOT_FROM_ANIMATION);

    test_controller.animator()->ScheduleAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(target_opacity, delta)));
    ...
    test_controller.animator()->OnThreadedAnimationStarted(
        effective_start, cc::TargetProperty::OPACITY, test_controller.GetRunningSequence(LayerAnimationElement::OPACITY)->animation_group_id());

    animator->Step(effective_start + delta / 2);

    EXPECT_TRUE(test_controller.animator()->is_animating());
    EXPECT_NEAR(0.5, test_controller.GetRunningSequence(LayerAnimationElement::OPACITY)->last_progressed_fraction(), epsilon);
...
TEST(LayerAnimatorTest, ScheduleTwoAnimationsThatCanRunImmediately)
{
    ...
    delegate.SetBrightnessFromAnimation(start_brightness, PropertyChangeReason::NOT_FROM_ANIMATION);
    delegate.SetBoundsFromAnimation(start_bounds, PropertyChangeReason::NOT_FROM_ANIMATION);

    animator->ScheduleAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(target_brightness, delta)));

    animator->ScheduleAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateBoundsElement(target_bounds, delta)));
...
TEST(LayerAnimatorTest, ScheduleThreadedAndNonThreadedAnimations)
{
    ...
    animations.push_back(new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(target_opacity, delta)));

    animations.push_back(new LayerAnimationSequence(LayerAnimationElement::CreateBoundsElement(target_bounds, delta)));
...
TEST(LayerAnimatorTest, ScheduleTwoAnimationsOnSameProperty)
{
    ...
    animator->ScheduleAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(start_brightness, delta)));
...
TEST(LayerAnimatorTest, ScheduleBlockedAnimation)
{
    ...
    delegate.SetGrayscaleFromAnimation(start_grayscale, PropertyChangeReason::NOT_FROM_ANIMATION);
    delegate.SetBoundsFromAnimation(start_bounds, PropertyChangeReason::NOT_FROM_ANIMATION);

    animator->ScheduleAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateGrayscaleElement(target_grayscale, delta)));

    std::unique_ptr<LayerAnimationSequence> bounds_and_grayscale(
        new LayerAnimationSequence(LayerAnimationElement::CreateGrayscaleElement(start_grayscale, delta)));

    bounds_and_grayscale->AddElement(LayerAnimationElement::CreateBoundsElement(target_bounds, delta));

    animator->ScheduleAnimation(bounds_and_grayscale.release());

    animator->ScheduleAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateBoundsElement(start_bounds, delta)));
...
TEST(LayerAnimatorTest, ScheduleTogether)
{
    ...
    sequences.push_back(new LayerAnimationSequence(LayerAnimationElement::CreateGrayscaleElement(start_grayscale, delta)));
    sequences.push_back(new LayerAnimationSequence(LayerAnimationElement::CreateBoundsElement(target_bounds, delta)));
...
TEST(LayerAnimatorTest, StartAnimationThatCanRunImmediately)
{
    ...
    animator->StartAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(target_brightness, delta)));
...
TEST(LayerAnimatorTest, StartThreadedAnimationThatCanRunImmediately)
{
    ...
    test_controller.animator()->StartAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(target_opacity, delta)));
...
TEST(LayerAnimatorTest, PreemptBySettingNewTarget)
{
    ...
    animator->StartAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(target_opacity, delta)));

    animator->StartAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(start_opacity, delta)));
...
TEST(LayerAnimatorTest, PreemptByImmediatelyAnimatingToNewTarget)
{
    ...
    animator->set_preemption_strategy(LayerAnimator::IMMEDIATELY_ANIMATE_TO_NEW_TARGET);
    ...
    animator->StartAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(start_brightness, delta)));
    ...
    EXPECT_TRUE(animator->is_animating());
    EXPECT_FLOAT_EQ(delegate.GetBrightnessForAnimation(), 0.5 * (start_brightness + middle_brightness));
...
TEST(LayerAnimatorTest, PreemptThreadedByImmediatelyAnimatingToNewTarget)
{
    ...
    test_controller.animator()->set_preemption_strategy(LayerAnimator::IMMEDIATELY_ANIMATE_TO_NEW_TARGET);
    ...
    test_controller.animator()->StartAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(start_opacity, delta)));
    ...
    test_controller.animator()->OnThreadedAnimationStarted(
        second_effective_start, cc::TargetProperty::OPACITY, test_controller.GetRunningSequence(LayerAnimationElement::OPACITY)->animation_group_id());
...
TEST(LayerAnimatorTest, PreemptyByReplacingQueuedAnimations)
{
    ...
    animator->StartAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(middle_brightness, delta)));
...
TEST(LayerAnimatorTest, StartTogetherSetsLastStepTime)
{
    ...
    animator->StartTogether(CreateMultiSequence(
        LayerAnimationElement::CreateGrayscaleElement(target_grayscale, delta), LayerAnimationElement::CreateBrightnessElement(target_brightness, delta)));
...
TEST(LayerAnimatorTest, MultiPreemptBySettingNewTarget)
{
    ...
    animator->StartTogether(CreateMultiSequence(
        LayerAnimationElement::CreateOpacityElement(target_opacity, delta), LayerAnimationElement::CreateBrightnessElement(target_brightness, delta)));

    animator->StartTogether(CreateMultiSequence(
        LayerAnimationElement::CreateOpacityElement(start_opacity, delta), LayerAnimationElement::CreateBrightnessElement(start_brightness, delta)));
...
TEST(LayerAnimatorTest, MultiPreemptByImmediatelyAnimatingToNewTarget)
{
    ...
    animator->StartTogether(CreateMultiSequence(
        LayerAnimationElement::CreateGrayscaleElement(start_grayscale, delta), LayerAnimationElement::CreateBrightnessElement(start_brightness, delta)));
    ...
    EXPECT_TRUE(animator->is_animating());
    EXPECT_FLOAT_EQ(delegate.GetGrayscaleForAnimation(), 0.5 * (start_grayscale + middle_grayscale));
    EXPECT_FLOAT_EQ(delegate.GetBrightnessForAnimation(), 0.5 * (start_brightness + middle_brightness));
...
TEST(LayerAnimatorTest, MultiPreemptThreadedByImmediatelyAnimatingToNewTarget)
{
    ...
    test_controller.animator()->StartTogether(CreateMultiSequence(
        LayerAnimationElement::CreateOpacityElement(target_opacity, delta), LayerAnimationElement::CreateBrightnessElement(target_brightness, delta)));
    ...
    EXPECT_TRUE(test_controller.animator()->is_animating());
    EXPECT_NEAR(0.5, test_controller.GetRunningSequence(LayerAnimationElement::OPACITY)->last_progressed_fraction(), epsilon);
    EXPECT_NEAR(delegate.GetBrightnessForAnimation(), 0.5 * (start_brightness + middle_brightness), epsilon);
...
TEST(LayerAnimatorTest, MultiPreemptByReplacingQueuedAnimations)
{
    ...
    animator->StartTogether(CreateMultiSequence(
        LayerAnimationElement::CreateGrayscaleElement(middle_grayscale, delta), LayerAnimationElement::CreateBrightnessElement(middle_brightness, delta)));
...
TEST(LayerAnimatorTest, CyclicSequences)
{
    ...
    std::unique_ptr<LayerAnimationSequence> sequence(new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(target_brightness, delta)));

    sequence->AddElement(LayerAnimationElement::CreateBrightnessElement(start_brightness, delta));
...
TEST(LayerAnimatorTest, ThreadedCyclicSequences)
{
    ...
    std::unique_ptr<LayerAnimationSequence> sequence(new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(target_opacity, delta)));

    sequence->AddElement(LayerAnimationElement::CreateOpacityElement(start_opacity, delta));
    ...
    test_controller.animator()->OnThreadedAnimationStarted(
        third_effective_start, cc::TargetProperty::OPACITY, test_controller.GetRunningSequence(LayerAnimationElement::OPACITY)->animation_group_id());
    ...
    test_controller.animator()->OnThreadedAnimationStarted(
        fourth_effective_start, cc::TargetProperty::OPACITY, test_controller.GetRunningSequence(LayerAnimationElement::OPACITY)->animation_group_id());
    ...
    test_controller.animator()->OnThreadedAnimationStarted(
        fifth_effective_start, cc::TargetProperty::OPACITY, test_controller.GetRunningSequence(LayerAnimationElement::OPACITY)->animation_group_id());
    ...
    test_controller.animator()->StopAnimatingProperty(LayerAnimationElement::OPACITY);
...
TEST(LayerAnimatorTest, AddObserverExplicit)
{
    ...
    TestLayerAnimationDelegate delegate;
    scoped_refptr<LayerAnimator> animator(CreateDefaultTestAnimator(&delegate, &observer));
    ...
    delegate.SetBrightnessFromAnimation(0.0f, PropertyChangeReason::NOT_FROM_ANIMATION);

    LayerAnimationSequence* sequence = new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(1.0f, delta));
    ...
    sequence = new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(1.0f, delta));
...
TEST(LayerAnimatorTest, ImplicitAnimationObservers)
{
    ...
    EXPECT_TRUE(observer.WasAnimationCompletedForProperty(LayerAnimationElement::BRIGHTNESS));
...
TEST(LayerAnimatorTest, CacheRenderSurface)
{
    ...
    animator->StopAnimatingProperty(LayerAnimationElement::OPACITY);
    EXPECT_TRUE(observer.animations_completed());
    EXPECT_TRUE(observer.WasAnimationCompletedForProperty(LayerAnimationElement::OPACITY));
...
TEST(LayerAnimatorTest, ImplicitObserversAtAnimatorDestruction)
{
    ...
    EXPECT_TRUE(observer_notify.WasAnimationAbortedForProperty(LayerAnimationElement::BRIGHTNESS));
    EXPECT_FALSE(observer_do_not_notify.animations_completed());
}
...
TEST(LayerAnimatorTest, AbortedAnimationStatusInImplicitObservers)
{
    ...
    animator->AbortAllAnimations();
    EXPECT_TRUE(observer.animations_completed());
    EXPECT_TRUE(observer.WasAnimationAbortedForProperty(LayerAnimationElement::BRIGHTNESS));
    EXPECT_FALSE(observer.WasAnimationAbortedForProperty(LayerAnimationElement::OPACITY));
    ...
    EXPECT_TRUE(observer.animations_completed());
    EXPECT_TRUE(observer.WasAnimationAbortedForProperty(LayerAnimationElement::BRIGHTNESS));
    EXPECT_TRUE(observer.WasAnimationAbortedForProperty(LayerAnimationElement::OPACITY));
}
...
{
    TestLayerAnimationDelegate delegate;
    std::unique_ptr<TestLayerAnimationObserver> observer(new TestLayerAnimationObserver);
    scoped_refptr<LayerAnimator> animator(CreateDefaultTestAnimator(&delegate, observer.get()));

    delegate.SetOpacityFromAnimation(0.0f, PropertyChangeReason::NOT_FROM_ANIMATION);

    base::TimeDelta delta = base::Seconds(1);
    LayerAnimationSequence* sequence = new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(1.0f, delta));
...
TEST(LayerAnimatorTest, ObserverDetachedBeforeAnimationFinished)
{
    ...
    EXPECT_FALSE(observer.WasAnimationCompletedForProperty(LayerAnimationElement::BRIGHTNESS));
    EXPECT_FALSE(observer.WasAnimationAbortedForProperty(LayerAnimationElement::BRIGHTNESS));
}
...
{
    ScopedAnimationDurationScaleMode normal_duration_mode(ScopedAnimationDurationScaleMode::NORMAL_DURATION);
    ...
    std::unique_ptr<DeletingLayerAnimationObserver> observer(new DeletingLayerAnimationObserver(animator.get()));

    animator->AddObserver(observer.get());

    animator->StartAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(target_brightness, brightness_delta)));

    animator->StartAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateBoundsElement(target_bounds, bounds_delta)));
...
TEST(LayerAnimatorTest, CallbackDeletesAnimationInProgress)
{
    ...
    public:
        ...
        void SetBoundsFromAnimation(const gfx::Rect& bounds, PropertyChangeReason reason) override
        {
...
{
    ScopedAnimationDurationScaleMode test_duration_mode(ScopedAnimationDurationScaleMode::NON_ZERO_DURATION);
    scoped_refptr<LayerAnimator> animator(new TestLayerAnimator());
    animator->set_disable_timer_for_test(true);
    TestLayerAnimationDelegate delegate;
    animator->SetDelegate(&delegate);
...
TEST(LayerAnimatorTest, SettingPropertyDuringAnAnimation)
{
    ...
    std::unique_ptr<LayerAnimationSequence> sequence
        = std::make_unique<LayerAnimationSequence>(LayerAnimationElement::CreateOpacityElement(target_opacity, delta));
...
TEST(LayerAnimatorTest, ImmediatelySettingNewTargetDoesNotLeak)
{
    ...
    std::unique_ptr<TestLayerAnimationSequence> sequence
        = std::make_unique<TestLayerAnimationSequence>(LayerAnimationElement::CreateBoundsElement(target_bounds, delta), &num_live_instances);
...
TEST(LayerAnimatorTest, GetTargetOpacity)
{
    ...
    delegate.SetOpacityFromAnimation(0.0, PropertyChangeReason::NOT_FROM_ANIMATION);
...
TEST(LayerAnimatorTest, GetTargetBrightness)
{
    ...
    delegate.SetBrightnessFromAnimation(0.0, PropertyChangeReason::NOT_FROM_ANIMATION);
...
TEST(LayerAnimatorTest, GetTargetGrayscale)
{
    ...
    delegate.SetGrayscaleFromAnimation(0.0, PropertyChangeReason::NOT_FROM_ANIMATION);
...
TEST(LayerAnimatorTest, Color)
{
    ...
    delegate.SetColorFromAnimation(start_color, PropertyChangeReason::NOT_FROM_ANIMATION);

    animator->ScheduleAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateColorElement(target_color, delta)));

    EXPECT_TRUE(animator->is_animating());
    EXPECT_EQ(ColorToString(start_color), ColorToString(delegate.GetColorForAnimation()));
    ...
    EXPECT_TRUE(animator->is_animating());
    EXPECT_EQ(ColorToString(middle_color), ColorToString(delegate.GetColorForAnimation()));

    animator->Step(start_time + base::Milliseconds(1000));

    EXPECT_FALSE(animator->is_animating());
    EXPECT_EQ(ColorToString(target_color), ColorToString(delegate.GetColorForAnimation()));
}
...
TEST(LayerAnimatorTest, SchedulePauseForProperties)
{
    ...
    animator->set_preemption_strategy(LayerAnimator::ENQUEUE_NEW_ANIMATION);
    animator->SchedulePauseForProperties(base::Milliseconds(100), LayerAnimationElement::TRANSFORM | LayerAnimationElement::BOUNDS);
...
TEST(LayerAnimatorTest, IsAnimatingOnePropertyOf)
{
    ...
    EXPECT_TRUE(animator->IsAnimatingOnePropertyOf(LayerAnimationElement::LayerAnimationElement::BRIGHTNESS));
    EXPECT_TRUE(animator->IsAnimatingOnePropertyOf(LayerAnimationElement::LayerAnimationElement::GRAYSCALE));
    EXPECT_TRUE(
        animator->IsAnimatingOnePropertyOf(LayerAnimationElement::LayerAnimationElement::BRIGHTNESS | LayerAnimationElement::LayerAnimationElement::GRAYSCALE));
    EXPECT_TRUE(animator->IsAnimatingOnePropertyOf(LayerAnimationElement::LayerAnimationElement::BRIGHTNESS));
    EXPECT_FALSE(animator->IsAnimatingOnePropertyOf(LayerAnimationElement::LayerAnimationElement::BOUNDS));
}
...
public:
    ...
    LayerAnimator* animator()
    {
        return animator_owner_->animator();
    }
...
TEST(LayerAnimatorTest, ObserverDeletesAnimatorAfterFinishingAnimation)
{
    ...
    base::TimeDelta delta = base::Seconds(1);
    LayerAnimationSequence* brightness_sequence = new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(1.0f, delta));
    animator->StartAnimation(brightness_sequence);

    delta = base::Seconds(2);
    LayerAnimationSequence* bounds_sequence = new LayerAnimationSequence(LayerAnimationElement::CreateBoundsElement(target_bounds, delta));
...
TEST(LayerAnimatorTest, ObserverDeletesAnimatorAfterStoppingAnimating)
{
    ...
    base::TimeDelta delta = base::Seconds(1);
    LayerAnimationSequence* opacity_sequence = new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(1.0f, delta));
...
TEST(LayerAnimatorTest, ObserverDeletesAnimatorAfterScheduling)
{
    ...
    base::TimeDelta delta = base::Seconds(1);
    to_start.push_back(new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(1.0f, delta)));

    delta = base::Seconds(2);
    to_start.push_back(new LayerAnimationSequence(LayerAnimationElement::CreateBoundsElement(target_bounds, delta)));
...
TEST(LayerAnimatorTest, ObserverDeletesAnimatorAfterAborted)
{
    ...
    animator->StartAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(1.0f, delta)));
...
TEST(LayerAnimatorTest, TestSetterRespectEnqueueStrategy)
{
    ...
    ScopedLayerAnimationSettings settings(animator.get());
    settings.SetPreemptionStrategy(LayerAnimator::IMMEDIATELY_ANIMATE_TO_NEW_TARGET);
    settings.SetTransitionDuration(base::Seconds(1));
    animator->SetOpacity(target_opacity);

    EXPECT_EQ(start_opacity, delegate.GetOpacityForAnimation());

    settings.SetPreemptionStrategy(LayerAnimator::ENQUEUE_NEW_ANIMATION);
...
TEST(LayerAnimatorTest, SequencesAreRemovedFromQueueBeforeBeingNotifiedOfStarted)
{
    TestLayerAnimationDelegate delegate;
    scoped_refptr<LayerAnimator> animator = CreateDefaultTestAnimator(&delegate);

    AbortAnimationsOnStartedLayerAnimationObserver aborting_observer(animator.get());
    test::CountCheckingLayerAnimationObserver counting_observer(&aborting_observer);

    const base::TimeDelta sequence_duration = base::Seconds(1);

    animator->ScheduleAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(1.f, sequence_duration)));

    LayerAnimationSequence* observed_sequence = new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(0.f, sequence_duration));
...
TEST(LayerAnimatorTest, AnimatorStartedCorrectly)
{
    ...
    animator->ScheduleAnimation(new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(target_opacity, time_delta)));
...
TEST(LayerAnimatorTest, AnimatorRemovedFromCollectionWhenLayerIsDestroyed)
{
    ...
    EXPECT_TRUE(collection_delegate.GetLayerAnimatorCollection()->HasActiveAnimators());

    layer.reset();
    EXPECT_EQ(NULL, animator->delegate());
    EXPECT_FALSE(collection_delegate.GetLayerAnimatorCollection()->HasActiveAnimators());
}
...
TEST(LayerAnimatorTest, LayerMovedBetweenCompositorsDuringAnimation)
{
    ...
    std::unique_ptr<TestCompositorHost> host_1(TestCompositorHost::Create(bounds, context_factories.GetContextFactory()));
    std::unique_ptr<TestCompositorHost> host_2(TestCompositorHost::Create(bounds, context_factories.GetContextFactory()));
    host_1->Show();
    host_2->Show();
    ...
    cc::MutatorHost* mutator_host_1 = root_1.cc_layer_for_testing()->layer_tree_host()->mutator_host();

    Compositor* compositor_2 = host_2->GetCompositor();
    Layer root_2;
    compositor_2->SetRootLayer(&root_2);
    cc::MutatorHost* mutator_host_2 = root_2.cc_layer_for_testing()->layer_tree_host()->mutator_host();
    ...
    root_1.Add(&layer);
    EXPECT_FALSE(mutator_host_1->HasTickingKeyframeModelForTesting(layer.element_id()));
    EXPECT_FALSE(mutator_host_2->HasTickingKeyframeModelForTesting(layer.element_id()));
    ...
    EXPECT_FALSE(compositor_2->layer_animator_collection()->HasActiveAnimators());
    EXPECT_TRUE(mutator_host_1->HasTickingKeyframeModelForTesting(layer.element_id()));
    EXPECT_FALSE(mutator_host_2->HasTickingKeyframeModelForTesting(layer.element_id()));
    ...
    EXPECT_FALSE(mutator_host_1->HasTickingKeyframeModelForTesting(layer.element_id()));
    EXPECT_TRUE(mutator_host_2->HasTickingKeyframeModelForTesting(layer.element_id()));
...
TEST(LayerAnimatorTest, ThreadedAnimationSurvivesIfLayerRemovedAdded)
{
    ...
    TestContextFactories context_factories(enable_pixel_output);
    const gfx::Rect bounds(10, 10, 100, 100);
    std::unique_ptr<TestCompositorHost> host(TestCompositorHost::Create(bounds, context_factories.GetContextFactory()));
    host->Show();
    ...
    cc::MutatorHost* mutator = layer.cc_layer_for_testing()->layer_tree_host()->mutator_host();
...
public:
    ...
    LayerOwnerAnimationObserver& operator=(const LayerOwnerAnimationObserver&) = delete;
...
TEST(LayerAnimatorTest, ObserverDeletesLayerInStopAnimating)
{
    ...
    base::TimeDelta time_delta = base::Seconds(1);
    LayerAnimationSequence* opacity = new LayerAnimationSequence(LayerAnimationElement::CreateOpacityElement(target_opacity, time_delta));
    opacity->AddObserver(&observer);
    animator->ScheduleAnimation(opacity);
    time_delta = base::Seconds(2);
    LayerAnimationSequence* move = new LayerAnimationSequence(LayerAnimationElement::CreateBoundsElement(target_bounds, time_delta));
...
TEST(LayerAnimatorTest, SetPropertyWithObserverThatDeletesLayerInStopAnimating)
{
    scoped_refptr<LayerAnimator> animator = CreateDefaultTestAnimator();
    LayerOwnerAnimationObserver observer(animator.get());
    LayerAnimationDelegate* delegate = observer.animator_layer();
...
TEST(LayerAnimatorTest, SetPropertyWithObserverThatDeletesLayerOnImplicitAnimationCompletion)
{
    ...
    layer_animation_observer.SetAnimationsCompletedCallback(base::BindRepeating(base::IgnoreResult(&LayerOwner::ReleaseLayer), base::Unretained(&layer_owner)));
...
TEST(LayerAnimatorTest, ObserverGetsNotifiedOnCycleEnded)
{
    ...
    LayerAnimationSequence* sequence = new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(1.0f, kAnimationDuration));
    sequence->set_is_repeating(true);
    animator->StartAnimation(sequence);
...
TEST(LayerAnimatorObserverNotificationOrderTest, SuccessfulCompletionOfSequence)
{
    for (int is_cyclic = 0; is_cyclic <= 0; is_cyclic++) {
        ...
        LayerAnimationSequence* sequence = new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(1.0f, animation_duration));
        ...
        EXPECT_EQ(observer.last_repetition_ended_sequence(), is_cyclic ? sequence : nullptr);
...
TEST(LayerAnimatorObserverNotificationOrderTest, RunningASequenceThatIsQueuedForLaterStartTime)
{
    for (int is_cyclic = 0; is_cyclic <= 0; is_cyclic++) {
        ...
        LayerAnimationSequence* first_sequence = new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(1.0f, animation_duration));
        first_sequence->set_is_repeating(is_cyclic);

        LayerAnimationSequence* queued_sequence = new LayerAnimationSequence(LayerAnimationElement::CreateBrightnessElement(1.0f, animation_duration));
        ...
        EXPECT_EQ(observer.last_aborted_sequence(), nullptr);
        EXPECT_EQ(observer.last_ended_sequence(), is_cyclic ? nullptr : first_sequence);
        EXPECT_EQ(observer.last_repetition_ended_sequence(), is_cyclic ? first_sequence : nullptr);
...
TEST(LayerAnimatorObserverNotificationOrderTest, ObserverAddedAfterAnimationStarts)
{
    for (int is_cyclic = 0; is_cyclic <= 0; is_cyclic++) {
        TestLayerAnimationDelegate delegate;
        scoped_refptr<LayerAnimator> animator = CreateDefaultTestAnimator(&delegate);
