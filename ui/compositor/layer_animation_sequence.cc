...
LayerAnimationSequence::LayerAnimationSequence(std::unique_ptr<LayerAnimationElement> element)
    : properties_(LayerAnimationElement::UNKNOWN)
    , is_repeating_(false)
    , last_element_(0)
    , waiting_for_group_start_(false)
...
void LayerAnimationSequence::Start(LayerAnimationDelegate* delegate)
{
    ...
    DUMP_WILL_BE_CHECK(!(is_repeating_ && GetTotalDurationOfAllElements().is_zero())) << "A repeating animation with zero duration is not a supported "
...
void LayerAnimationSequence::Progress(base::TimeTicks now, LayerAnimationDelegate* delegate)
{
    DCHECK(start_time_ != base::TimeTicks());
    ...
    const auto animation_should_progress = [this, total_duration]() {
        ...
        // values.
        return (is_repeating_ && !total_duration.is_zero()) || last_element_ < elements_.size();
    };
...
void LayerAnimationSequence::GetTargetValue(LayerAnimationElement::TargetValue* target) const
{
    if (is_repeating_)
        return;
...
void LayerAnimationSequence::AddElement(std::unique_ptr<LayerAnimationElement> element)
{
    properties_ |= element->properties();
    elements_.push_back(std::move(element));
}

bool LayerAnimationSequence::HasConflictingProperty(LayerAnimationElement::AnimatableProperties other) const
{
    return (properties_ & other) != LayerAnimationElement::UNKNOWN;
}

bool LayerAnimationSequence::IsFirstElementThreaded(LayerAnimationDelegate* delegate) const
{
    if (!elements_.empty())
...
void LayerAnimationSequence::OnThreadedAnimationStarted(base::TimeTicks monotonic_time, cc::TargetProperty::Type target_property, int group_id)
{
    ...
    LayerAnimationElement::AnimatableProperty event_property = LayerAnimationElement::ToAnimatableProperty(target_property);
...
void LayerAnimationSequence::OnAnimatorAttached(LayerAnimationDelegate* delegate)
{
...
{
    return base::StringPrintf("LayerAnimationSequence{size=%zu, properties=%s, "
                              "elements=[%s], is_repeating=%d, group_id=%d}",
        size(), LayerAnimationElement::AnimatablePropertiesToString(properties_).c_str(), ElementsToString().c_str(), is_repeating_, animation_group_id_);
}
