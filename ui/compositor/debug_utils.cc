...
void PrintLayerHierarchyImp(const Layer* layer, int indent, const gfx::Point& mouse_location, std::ostringstream* out, DebugLayerChildCallback child_cb)
{
    std::string indent_str(indent, ' ');

    gfx::Point transformed_mouse_location = layer->transform().InverseMapPoint(mouse_location).value_or(mouse_location);
    const bool mouse_inside_layer_bounds = layer->bounds().Contains(transformed_mouse_location);
    const gfx::Point mouse_location_in_layer = transformed_mouse_location - layer->bounds().origin().OffsetFromOrigin();
    ...
    *out << ' ' << layer->bounds().width() << 'x' << layer->bounds().height();
    ...
    if (cc_layer) {
        ...
        // reasons.
        if (cc_layer->layer_tree_host() && !cc_layer->layer_tree_host()->property_trees()->needs_rebuild()) {
            cc::RenderSurfaceReason render_surface = cc_layer->GetRenderSurfaceReason();
            if (render_surface != cc::RenderSurfaceReason::kNone) {
                *out << " render-surface-reason: " << cc::RenderSurfaceReasonToString(render_surface);
            }
        }
    }
    ...
    if (mask) {
        *out << '\n' << property_indent_str;
        *out << "mask layer: " << std::setprecision(2) << mask->bounds().ToString() << mask->GetSubpixelOffset().ToString();
    }
    ...
    if (!layer->transform().IsIdentity()) {
        if (std::optional<gfx::DecomposedTransform> decomp = layer->transform().Decompose()) {
            *out << '\n' << property_indent_str;
    ...
    *out << '\n';
    ...
    std::vector<raw_ptr<ui::Layer, VectorExperimental>> children = child_cb ? child_cb.Run(layer) : layer->children();
    for (ui::Layer* child : children) {
        PrintLayerHierarchyImp(child, indent + 3, mouse_location_in_layer, out, child_cb);
    }
}
...
void PrintLayerHierarchy(const Layer* layer, const gfx::Point& mouse_location, std::ostringstream* out, DebugLayerChildCallback child_cb)
{
    *out << "Layer hierarchy:\n";
