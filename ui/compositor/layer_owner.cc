...
std::unique_ptr<Layer> LayerOwner::RecreateLayer()
{
    ...
    std::vector<raw_ptr<ui::Layer, VectorExperimental>> children_copy = old_layer->children();
    for (std::vector<raw_ptr<ui::Layer, VectorExperimental>>::const_iterator it = children_copy.begin(); it != children_copy.end(); ++it) {
        ui::Layer* child = *it;
        layer_->Add(child);
    }
