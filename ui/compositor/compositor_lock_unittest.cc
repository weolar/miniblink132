...
protected:
    ...
    CompositorLockManager* lock_manager()
    {
        return lock_manager_.get();
    }

    void DestroyLockManager()
    {
        lock_manager_.reset();
    }
    ...
    {
        return base::BindOnce([](CompositorLockTest* self) { self->num_releases_++; }, base::Unretained(this));
    }
...
TEST_F(CompositorLockTest, LocksTimeOut)
{
...

    {
        ...
        std::unique_ptr<CompositorLock> lock = lock_manager()->GetCompositorLock(&lock_client, timeout, CreateReleaseCallback());
...

    {
        ...
        std::unique_ptr<CompositorLock> lock = lock_manager()->GetCompositorLock(&lock_client, base::TimeDelta(), CreateReleaseCallback());
        EXPECT_TRUE(lock_manager()->IsLocked());
...
TEST_F(CompositorLockTest, MultipleLockClients)
{
    ...
    lock1 = lock_manager()->GetCompositorLock(&lock_client1, timeout, CreateReleaseCallback());
    lock2 = lock_manager()->GetCompositorLock(&lock_client2, timeout, CreateReleaseCallback());
    EXPECT_TRUE(lock_manager()->IsLocked());
...
TEST_F(CompositorLockTest, AddingLocksDoesNotExtendTimeout)
{
    ...
    lock1 = lock_manager()->GetCompositorLock(&lock_client1, timeout1, CreateReleaseCallback());
    EXPECT_TRUE(lock_manager()->IsLocked());

    // The second lock has a longer timeout, but since a lock is active,
    // the first one is used for both.
    lock2 = lock_manager()->GetCompositorLock(&lock_client2, timeout2, CreateReleaseCallback());
...
TEST_F(CompositorLockTest, CompositorLockOutlivesManager)
{
    ...
    lock1 = lock_manager()->GetCompositorLock(&lock_client1, base::TimeDelta(), CreateReleaseCallback());
