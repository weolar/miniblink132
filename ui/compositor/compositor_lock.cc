...
CompositorLockManager::CompositorLockManager(scoped_refptr<base::SingleThreadTaskRunner> task_runner)
...
std::unique_ptr<CompositorLock> CompositorLockManager::GetCompositorLock(
    CompositorLockClient* client, base::TimeDelta timeout, base::OnceClosure release_callback)
{
    ...
    auto lock = std::make_unique<CompositorLock>(client, weak_ptr_factory_.GetWeakPtr(), std::move(release_callback));
    ...
    if (should_extend_timeout) {
        ...
        task_runner_->PostDelayedTask(FROM_HERE, base::BindOnce(&CompositorLockManager::TimeoutLocks, lock_timeout_weak_ptr_factory_.GetWeakPtr()), timeout);
    }
    return lock;
}
...
void CompositorLockManager::TimeoutLocks()
{
    ...
    std::vector<raw_ptr<CompositorLock, VectorExperimental>> locks = active_locks_;
...
CompositorLock::CompositorLock(CompositorLockClient* client, base::WeakPtr<CompositorLockManager> manager, base::OnceClosure release_callback)
    : client_(client)
