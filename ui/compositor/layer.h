...
class COMPOSITOR_EXPORT Layer : public LayerAnimationDelegate, public cc::ContentLayerClient, public cc::TextureLayerClient {
public:
    ...
    {
        return const_cast<Compositor*>(const_cast<const Layer*>(this)->GetCompositor());
    }
    ...
    void SetCompositor(Compositor* compositor, scoped_refptr<cc::Layer> root_layer);
    ...
    // through the layer.
    float background_blur() const
    {
        return background_blur_sigma_;
    }
    void SetBackgroundBlur(float blur_sigma);

    // Blur pixels of this layer by 3 * this amount.
    float layer_blur() const
    {
        return layer_blur_sigma_;
    }
    ...
    float layer_saturation() const
    {
        return layer_saturation_;
    }
    ...
    // The effect of invert, brightness, greyscale, saturate, sepia, and
    // custom color matrix settings are combined.
    float layer_brightness() const
    {
        return layer_brightness_;
    }
    ...
    // The effect of invert, brightness, greyscale, saturate, sepia, and
    // custom color matrix settings are combined.
    float layer_grayscale() const
    {
        return layer_grayscale_;
    }
    ...
    float layer_hue_rotation() const
    {
        return layer_hue_rotation_;
    }
    ...
    const gfx::Point& layer_offset() const
    {
        return layer_offset_;
    }
    ...
    const ShapeRects* alpha_shape() const
    {
    ...
    // Invert the layer.
    bool layer_inverted() const
    {
        return layer_inverted_;
    }
    ...
    Layer* layer_mask_layer()
    {
        return layer_mask_;
    }
    const Layer* layer_mask_layer() const
    {
        return layer_mask_;
    }
    ...
    {
        return !cc_layer_->gradient_mask().IsEmpty();
    }
    ...
    // transform.
    static void ConvertPointToLayer(const Layer* source, const Layer* target, bool use_target_transform, gfx::PointF* point);
    ...
    bool GetTargetTransformRelativeTo(const Layer* ancestor, gfx::Transform* transform) const;
    ...
    void SetTransferableResource(const viz::TransferableResource& resource, viz::ReleaseCallback release_callback, gfx::Size texture_size_in_dip);
    ...
    void SetShowSurface(const viz::SurfaceId& surface_id, const gfx::Size& frame_size_in_dip, SkColor default_background_color,
    ...
    void SetShowReflectedSurface(const viz::SurfaceId& surface_id, const gfx::Size& frame_size_in_pixels);
    ...
    void SetDidScrollCallback(base::RepeatingCallback<void(const gfx::PointF&, const cc::ElementId&)> callback);
    ...
    cc::MirrorLayer* mirror_layer_for_testing()
    {
        return mirror_layer_.get();
    }
    cc::Layer* cc_layer_for_testing()
    {
        return cc_layer_;
    }
    const cc::Layer* cc_layer_for_testing() const
    {
        return cc_layer_;
    }
    ...
    bool PrepareTransferableResource(
        cc::SharedBitmapIdRegistrar* bitmap_registar, viz::TransferableResource* resource, viz::ReleaseCallback* release_callback) override;
    ...
    const Layer* layer_mask_back_link() const
    {
        return layer_mask_back_link_;
    }
...
private:
    ...
    bool ConvertPointForAncestor(const Layer* ancestor, bool use_target_transform, gfx::PointF* point) const;
    bool ConvertPointFromAncestor(const Layer* ancestor, bool use_target_transform, gfx::PointF* point) const;

    // Implementation of LayerAnimatorDelegate
    void SetBoundsFromAnimation(const gfx::Rect& bounds, PropertyChangeReason reason) override;
    void SetTransformFromAnimation(const gfx::Transform& new_transform, PropertyChangeReason reason) override;
    void SetOpacityFromAnimation(float opacity, PropertyChangeReason reason) override;
    void SetVisibilityFromAnimation(bool visibility, PropertyChangeReason reason) override;
    void SetBrightnessFromAnimation(float brightness, PropertyChangeReason reason) override;
    void SetGrayscaleFromAnimation(float grayscale, PropertyChangeReason reason) override;
    void SetColorFromAnimation(SkColor color, PropertyChangeReason reason) override;
    void SetClipRectFromAnimation(const gfx::Rect& clip_rect, PropertyChangeReason reason) override;
    void SetRoundedCornersFromAnimation(const gfx::RoundedCornersF& rounded_corners, PropertyChangeReason reason) override;
    void SetGradientMaskFromAnimation(const gfx::LinearGradient& gradient_mask, PropertyChangeReason reason) override;
    ...
    bool IsHitTestableForCC() const
    {
        return visible_ && accept_events_;
    }
    ...
    void SetFillsBoundsOpaquelyWithReason(bool fills_bounds_opaquely, PropertyChangeReason reason);
    ...
    // of this layer).
    bool GetTransformRelativeToImpl(const Layer* ancestor, bool is_target_transform, gfx::Transform* transform) const;
    ...
    base::ObserverList<LayerObserver>::UncheckedAndDanglingUntriaged observer_list_;
