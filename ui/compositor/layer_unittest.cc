...
public:
    ...
    LayerWithRealCompositorTest& operator=(const LayerWithRealCompositorTest&) = delete;
    ...
    void SetUp() override
    {
        ASSERT_TRUE(base::PathService::Get(base::DIR_SRC_TEST_DATA_ROOT, &test_data_dir_));
        ...
        context_factories_ = std::make_unique<TestContextFactories>(enable_pixel_output);

        const gfx::Rect host_bounds(10, 10, 500, 500);
        compositor_host_.reset(TestCompositorHost::Create(host_bounds, context_factories_->GetContextFactory()));
    ...
    {
        return compositor_host_->GetCompositor();
    }
    ...
    std::unique_ptr<Layer> CreateColorLayer(SkColor color, const gfx::Rect& bounds)
    {
    ...
    void ReadPixels(SkBitmap* bitmap, gfx::Rect source_rect)
    {
        ...
        std::unique_ptr<viz::CopyOutputRequest> request = std::make_unique<viz::CopyOutputRequest>(viz::CopyOutputRequest::ResultFormat::RGBA,
            viz::CopyOutputRequest::ResultDestination::kSystemMemory, base::BindOnce(&ReadbackHolder::OutputRequestCallback, holder));
    ...
    {
        layer->SchedulePaint(gfx::Rect(0, 0, layer->bounds().width(), layer->bounds().height()));
    }
    ...
    public:
        ...
        void OutputRequestCallback(std::unique_ptr<viz::CopyOutputResult> result)
        {
            if (result->IsEmpty()) {
                ...
                result_ = std::make_unique<SkBitmap>(scoped_bitmap.GetOutScopedBitmap());
            }
            run_loop_->Quit();
        }
...
public:
    ...
    MOCK_METHOD2(OnLayerBoundsChanged, void(const gfx::Rect&, PropertyChangeReason));
    MOCK_METHOD2(OnLayerTransformed, void(const gfx::Transform&, PropertyChangeReason));
...
private:
    ...
    void OnCompositingStarted(Compositor* compositor, base::TimeTicks start_time) override
    {
        started_ = true;
    }
...
public:
    ...
    TestCompositorAnimationObserver(const TestCompositorAnimationObserver&) = delete;
    TestCompositorAnimationObserver& operator=(const TestCompositorAnimationObserver&) = delete;
    ...
    size_t animation_step_count() const
    {
        return animation_step_count_;
    }
    bool shutdown() const
    {
        return shutdown_;
...
{
    std::unique_ptr<Layer> layer = CreateColorLayer(SK_ColorRED, gfx::Rect(20, 20, 50, 50));
    DrawTree(layer.get());
}
...
{
    std::unique_ptr<Layer> l1 = CreateColorLayer(SK_ColorRED, gfx::Rect(20, 20, 400, 400));
    std::unique_ptr<Layer> l2 = CreateColorLayer(SK_ColorBLUE, gfx::Rect(10, 10, 350, 350));
    std::unique_ptr<Layer> l3 = CreateColorLayer(SK_ColorYELLOW, gfx::Rect(5, 5, 25, 25));
    std::unique_ptr<Layer> l4 = CreateColorLayer(SK_ColorMAGENTA, gfx::Rect(300, 300, 100, 100));
...
TEST(LayerStandaloneTest, ReleaseMailboxOnDestruction)
{
    ...
    auto resource = viz::TransferableResource::MakeGpu(
        gpu::Mailbox::Generate(), GL_TEXTURE_2D, gpu::SyncToken(), size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
    layer->SetTransferableResource(resource, base::BindOnce(ReturnMailbox, &callback_run), gfx::Size(10, 10));
...
TEST_F(LayerWithDelegateTest, ConvertPointToLayer_Simple)
{
    ...
    Layer::ConvertPointToLayer(l2.get(), l1.get(), /*use_target_transform=*/true, &point1_in_l2_coords);
    gfx::PointF point1_in_l1_coords(15, 15);
    EXPECT_EQ(point1_in_l1_coords, point1_in_l2_coords);

    gfx::PointF point2_in_l1_coords(5, 5);
    Layer::ConvertPointToLayer(l1.get(), l2.get(), /*use_target_transform=*/true, &point2_in_l1_coords);
...
TEST_F(LayerWithDelegateTest, ConvertPointToLayer_Medium)
{
    std::unique_ptr<Layer> l1 = CreateColorLayer(SK_ColorRED, gfx::Rect(20, 20, 400, 400));
    std::unique_ptr<Layer> l2 = CreateColorLayer(SK_ColorBLUE, gfx::Rect(10, 10, 350, 350));
    std::unique_ptr<Layer> l3 = CreateColorLayer(SK_ColorYELLOW, gfx::Rect(10, 10, 100, 100));
    l1->Add(l2.get());
    l2->Add(l3.get());
    DrawTree(l1.get());

    gfx::PointF point1_in_l3_coords(5, 5);
    Layer::ConvertPointToLayer(l3.get(), l1.get(), /*use_target_transform=*/true, &point1_in_l3_coords);
    gfx::PointF point1_in_l1_coords(25, 25);
    EXPECT_EQ(point1_in_l1_coords, point1_in_l3_coords);

    gfx::PointF point2_in_l1_coords(5, 5);
    Layer::ConvertPointToLayer(l1.get(), l3.get(), /*use_target_transform=*/true, &point2_in_l1_coords);
...
TEST_F(LayerWithRealCompositorTest, Delegate)
{
    ...
    // |color_index|.
    std::unique_ptr<Layer> l1 = CreateColorLayer(SK_ColorBLACK, gfx::Rect(20, 20, 400, 400));
...
{
    std::unique_ptr<Layer> layer = CreateColorLayer(SK_ColorRED, gfx::Rect(20, 20, 400, 400));
...
TEST_F(LayerWithDelegateTest, Cloning)
{
    ...
    EXPECT_NE(layer->cc_layer_for_testing()->cache_render_surface(), clone->cc_layer_for_testing()->cache_render_surface());
    // Cloning should not preserve trilinear_filtering flag.
    EXPECT_NE(layer->cc_layer_for_testing()->trilinear_filtering(), clone->cc_layer_for_testing()->trilinear_filtering());
...
TEST_F(LayerWithDelegateTest, SurfaceLayerCloneAndMirror)
{
    ...
    layer->SetShowSurface(surface_id_one, gfx::Size(10, 10), SK_ColorWHITE, cc::DeadlinePolicy::UseDefaultDeadline(), false);
    ...
    layer->SetShowSurface(surface_id_two, gfx::Size(10, 10), SK_ColorWHITE, cc::DeadlinePolicy::UseDefaultDeadline(), true);
...
public:
    ...
    LayerWithNullDelegateTest& operator=(const LayerWithNullDelegateTest&) = delete;
    ...
    std::unique_ptr<Layer> CreateNoTextureLayer(const gfx::Rect& bounds) override
    {
        std::unique_ptr<Layer> layer = CreateLayer(LAYER_NOT_DRAWN);
        layer->SetBounds(bounds);
        return layer;
...
TEST_F(LayerWithNullDelegateTest, SwitchLayerPreservesCCLayerState)
{
    ...
    EXPECT_EQ(kSubtreeCaptureId, l1->cc_layer_for_testing()->subtree_capture_id());
    ...
    auto resource = viz::TransferableResource::MakeGpu(
        gpu::Mailbox::Generate(), GL_TEXTURE_2D, gpu::SyncToken(), size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
    l1->SetTransferableResource(resource, base::BindOnce(ReturnMailbox, &callback1_run), gfx::Size(10, 10));
    ...
    resource = viz::TransferableResource::MakeGpu(
        gpu::Mailbox::Generate(), GL_TEXTURE_2D, gpu::SyncToken(), size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
    l1->SetTransferableResource(resource, base::BindOnce(ReturnMailbox, &callback2_run), gfx::Size(10, 10));
    ...
    resource = viz::TransferableResource::MakeGpu(
        gpu::Mailbox::Generate(), GL_TEXTURE_2D, gpu::SyncToken(), size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
    l1->SetTransferableResource(resource, base::BindOnce(ReturnMailbox, &callback3_run), gfx::Size(10, 10));
...
TEST_F(LayerWithNullDelegateTest, EmptyDamagedRect)
{
    ...
    viz::ReleaseCallback callback
        = base::BindOnce([](base::RunLoop* run_loop, const gpu::SyncToken& sync_token, bool is_lost) { run_loop->Quit(); }, base::Unretained(&run_loop));

    std::unique_ptr<Layer> root = CreateLayer(LAYER_SOLID_COLOR);
    constexpr gfx::Size size(64, 64);
    auto resource = viz::TransferableResource::MakeGpu(
        gpu::Mailbox::Generate(), GL_TEXTURE_2D, gpu::SyncToken(), size, viz::SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
    root->SetTransferableResource(resource, std::move(callback), gfx::Size(10, 10));
...
{
    EXPECT_EQ(expected_color, actual_color) << "Pixel error at x=" << x << " y=" << y << "; "
                                            << "actual RGBA=(" << SkColorGetR(actual_color) << "," << SkColorGetG(actual_color) << ","
                                            << SkColorGetB(actual_color) << "," << SkColorGetA(actual_color) << "); "
                                            << "expected RGBA=(" << SkColorGetR(expected_color) << "," << SkColorGetG(expected_color) << ","
                                            << SkColorGetB(expected_color) << "," << SkColorGetA(expected_color) << ")";
}
...
TEST_F(LayerWithRealCompositorTest, DrawPixels)
{
    ...
    std::unique_ptr<Layer> layer = CreateColorLayer(SK_ColorRED, gfx::Rect(viewport_size));
    std::unique_ptr<Layer> layer2 = CreateColorLayer(SK_ColorBLUE, gfx::Rect(0, 0, viewport_size.width(), blue_height));
...
TEST_F(LayerWithRealCompositorTest, DrawAlphaBlendedPixels)
{
    ...
    std::unique_ptr<Layer> background_layer = CreateColorLayer(SK_ColorRED, gfx::Rect(viewport_size));
    std::unique_ptr<Layer> foreground_layer = CreateColorLayer(blue_with_alpha, gfx::Rect(viewport_size));
...
TEST_F(LayerWithRealCompositorTest, DrawAlphaThresholdFilterPixels)
{
    ...
    for (int x = 0; x < test_size; x++) {
        for (int y = 0; y < test_size; y++) {
            SkColor actual_color = bitmap.getColor(x, y);
            ExpectRgba(x, y, actual_color, y < blue_height ? blend_color : SK_ColorRED);
        }
    }
}
...
TEST_F(LayerWithRealCompositorTest, ModifyHierarchy)
{
    viz::ParentLocalSurfaceIdAllocator allocator;
    allocator.GenerateId();
    GetCompositor()->SetScaleAndSize(1.0f, gfx::Size(50, 50), allocator.GetCurrentLocalSurfaceId());

    // l0
    //  +-l11
    //  | +-l21
    //  +-l12
    std::unique_ptr<Layer> l0 = CreateColorLayer(SK_ColorRED, gfx::Rect(0, 0, 50, 50));
    std::unique_ptr<Layer> l11 = CreateColorLayer(SK_ColorGREEN, gfx::Rect(0, 0, 25, 25));
    std::unique_ptr<Layer> l21 = CreateColorLayer(SK_ColorMAGENTA, gfx::Rect(0, 0, 15, 15));
    std::unique_ptr<Layer> l12 = CreateColorLayer(SK_ColorBLUE, gfx::Rect(10, 10, 25, 25));
    ...
    EXPECT_TRUE(MatchesPNGFile(bitmap, ref_img1, cc::AlphaDiscardingExactPixelComparator()));
    ...
    EXPECT_TRUE(MatchesPNGFile(bitmap, ref_img2, cc::AlphaDiscardingExactPixelComparator()));
...
TEST_F(LayerWithRealCompositorTest, MAYBE_BackgroundBlur)
{
    ...
    const base::test::ScopedRunLoopTimeout increased_run_timeout(FROM_HERE, TestTimeouts::action_max_timeout());
#endif

    viz::ParentLocalSurfaceIdAllocator allocator;
    allocator.GenerateId();
    GetCompositor()->SetScaleAndSize(1.0f, gfx::Size(200, 200), allocator.GetCurrentLocalSurfaceId());
    // l0
    //  +-l1
    //  +-l2
    std::unique_ptr<Layer> l0 = CreateColorLayer(SK_ColorRED, gfx::Rect(0, 0, 200, 200));
    std::unique_ptr<Layer> l1 = CreateColorLayer(SK_ColorGREEN, gfx::Rect(100, 100, 100, 100));
    SkColor blue_with_alpha = SkColorSetARGB(40, 10, 20, 200);
    std::unique_ptr<Layer> l2 = CreateColorLayer(blue_with_alpha, gfx::Rect(50, 50, 100, 100));
    ...
    auto fuzzy_comparator = cc::FuzzyPixelComparator().DiscardAlpha().SetErrorPixelsPercentageLimit(25.f).SetAbsErrorLimit(3);
...
// TODO(crbug.com/40280155): Flaky on fuchsia-arm64 builds. Re-enable this test.
#if BUILDFLAG(IS_FUCHSIA) && defined(ARCH_CPU_ARM64)
#define MAYBE_BackgroundBlurChangeDeviceScale DISABLED_BackgroundBlurChangeDeviceScale
#else
...
TEST_F(LayerWithRealCompositorTest, MAYBE_BackgroundBlurChangeDeviceScale)
{
    ...
    base::FilePath ref_img1 = test_data_dir().AppendASCII("BackgroundBlur1.png");
    base::FilePath ref_img2 = test_data_dir().AppendASCII("BackgroundBlur1_zoom.png");
    SkBitmap bitmap;
    ...
    GetCompositor()->SetScaleAndSize(2.0f, gfx::Size(200, 200), allocator.GetCurrentLocalSurfaceId());
    DrawTree(l0.get());
    ReadPixels(&bitmap);
    ASSERT_FALSE(bitmap.empty());
    // WritePNGFile(bitmap, ref_img2, false);
...
TEST_F(LayerWithRealCompositorTest, Opacity)
{
    ...
    EXPECT_TRUE(MatchesPNGFile(bitmap, ref_img, cc::AlphaDiscardingExactPixelComparator()));
}
...
public:
    ...
    SchedulePaintLayerDelegate& operator=(const SchedulePaintLayerDelegate&) = delete;
    ...
    const gfx::Rect& last_clip_rect() const
    {
...
{
    std::unique_ptr<Layer> root = CreateColorLayer(SK_ColorRED, gfx::Rect(0, 0, 500, 500));
    SchedulePaintLayerDelegate child_delegate;
    std::unique_ptr<Layer> child = CreateColorLayer(SK_ColorBLUE, gfx::Rect(0, 0, 200, 200));
    ...
    WaitForCommit();
    EXPECT_EQ(1, child_delegate.GetPaintCountAndClear());
    EXPECT_TRUE(child_delegate.last_clip_rect().Contains(gfx::Rect(10, 10, 30, 30)));
}

TEST_F(LayerWithRealCompositorTest, ScaleUpDown)
{
    std::unique_ptr<Layer> root = CreateColorLayer(SK_ColorWHITE, gfx::Rect(10, 20, 200, 220));
    ...
    std::unique_ptr<Layer> l1 = CreateColorLayer(SK_ColorWHITE, gfx::Rect(10, 20, 140, 180));
    ...
    viz::ParentLocalSurfaceIdAllocator allocator;
    allocator.GenerateId();
    GetCompositor()->SetScaleAndSize(1.0f, gfx::Size(500, 500), allocator.GetCurrentLocalSurfaceId());
    ...
    allocator.GenerateId();
    GetCompositor()->SetScaleAndSize(2.0f, gfx::Size(500, 500), allocator.GetCurrentLocalSurfaceId());
    EXPECT_EQ("10,20 200x220", root->bounds().ToString());
    EXPECT_EQ("10,20 140x180", l1->bounds().ToString());
    ...
    // trigger repaint.
    allocator.GenerateId();
    GetCompositor()->SetScaleAndSize(1.0f, gfx::Size(1000, 1000), allocator.GetCurrentLocalSurfaceId());
    // No scale change, so no scale notification.
    EXPECT_EQ(0.0f, root_delegate.device_scale_factor());
    EXPECT_EQ(0.0f, l1_delegate.device_scale_factor());
}
...
{
    std::unique_ptr<Layer> root = CreateNoTextureLayer(gfx::Rect(0, 0, 1000, 1000));
...
TEST_F(LayerWithDelegateTest, ExternalContent)
{
    ...
    child->SetShowSurface(
        viz::SurfaceId(frame_sink_id, allocator.GetCurrentLocalSurfaceId()), gfx::Size(10, 10), SK_ColorWHITE, cc::DeadlinePolicy::UseDefaultDeadline(), false);
    ...
    allocator.GenerateId();
    child->SetShowSurface(viz::SurfaceId(frame_sink_id, allocator.GetCurrentLocalSurfaceId()), gfx::Size(10, 10), SK_ColorWHITE,
...
TEST_F(LayerWithDelegateTest, ExternalContentMirroring)
{
    ...
    viz::SurfaceId surface_id(viz::FrameSinkId(0, 1), viz::LocalSurfaceId(2, base::UnguessableToken::Create()));
    layer->SetShowSurface(surface_id, gfx::Size(10, 10), SK_ColorWHITE, cc::DeadlinePolicy::UseDefaultDeadline(), false);
    ...
    surface_id = viz::SurfaceId(viz::FrameSinkId(1, 2), viz::LocalSurfaceId(3, base::UnguessableToken::Create()));
    layer->SetShowSurface(surface_id, gfx::Size(20, 20), SK_ColorWHITE, cc::DeadlinePolicy::UseDefaultDeadline(), false);
...
TEST_F(LayerWithDelegateTest, TransferableResourceMirroring)
{
    ...
    layer->SetTransferableResource(resource, base::BindOnce(ReturnMailbox, &release_callback_run), gfx::Size(10, 10));
...
TEST_F(LayerWithDelegateTest, LayerFiltersSurvival)
{
    ...
    layer->SetShowSurface(viz::SurfaceId(), gfx::Size(10, 10), SK_ColorWHITE, cc::DeadlinePolicy::UseDefaultDeadline(), false);
    EXPECT_EQ(layer->layer_grayscale(), 0.5f);
    EXPECT_TRUE(layer->cc_layer_for_testing());
...
TEST_F(LayerWithRealCompositorTest, SwitchCCLayerSolidColorNotAnimating)
{
    ...
    EXPECT_FALSE(root->GetAnimator()->IsAnimatingProperty(LayerAnimationElement::COLOR));
...
TEST_F(LayerWithRealCompositorTest, SwitchCCLayerSolidColorWhileAnimating)
{
    ...
    auto long_duration_animation = std::make_unique<ui::ScopedAnimationDurationScaleMode>(ui::ScopedAnimationDurationScaleMode::SLOW_DURATION);
    {
    ...
    EXPECT_TRUE(root->fills_bounds_opaquely());
    EXPECT_TRUE(root->GetAnimator()->IsAnimatingProperty(LayerAnimationElement::COLOR));
...
TEST_F(LayerWithRealCompositorTest, SwitchCCLayerDeleteLayer)
{
    ...
    animation_observer.SetCallback(base::BindLambdaForTesting([&]() { l1.reset(); }));
...
TEST_F(LayerWithRealCompositorTest, TreeMutationDuringScaleFactorChange)
{
    ...
    animation_observer.SetCallback(base::BindLambdaForTesting([&]() { layer_to_delete.reset(); }));
    ...
    animation_observer.SetCallback(base::BindLambdaForTesting([&]() { root->StackChildrenAtBottom({ child2.get() }); }));

    long_duration_animation = std::make_unique<ui::ScopedAnimationDurationScaleMode>(ui::ScopedAnimationDurationScaleMode::SLOW_DURATION);
    {
        ui::ScopedLayerAnimationSettings animation(child->GetAnimator());
        animation.AddObserver(&animation_observer);
        animation.SetTransitionDuration(base::Milliseconds(1000));
...
{
    std::unique_ptr<Layer> root = CreateLayer(LAYER_SOLID_COLOR);
    std::unique_ptr<Layer> child = CreateColorLayer(SK_ColorRED, gfx::Rect(10, 10));
...
TEST_F(LayerWithDelegateTest, AddRemoveLayerUpdatesAnimatorsFromSubtree)
{
    std::unique_ptr<Layer> root = CreateLayer(LAYER_TEXTURED);
    std::unique_ptr<Layer> child = CreateLayer(LAYER_TEXTURED);
    std::unique_ptr<Layer> grandchild = CreateColorLayer(SK_ColorRED, gfx::Rect(10, 10));
    root->Add(child.get());
...
public:
    ...
    LayerRemovingLayerAnimationObserver(const LayerRemovingLayerAnimationObserver&) = delete;
    LayerRemovingLayerAnimationObserver& operator=(const LayerRemovingLayerAnimationObserver&) = delete;
...
TEST_F(LayerWithDelegateTest, NonAnimatingAnimatorsAreRemovedFromCollection)
{
    ...
    std::unique_ptr<LayerAnimationElement> element = ui::LayerAnimationElement::CreateOpacityElement(0.5f, base::Seconds(1));
    LayerAnimationSequence* sequence = new LayerAnimationSequence(std::move(element));
...
TEST_F(LayerWithRealCompositorTest, SnapLayerToPixels)
{
    ...
    viz::ParentLocalSurfaceIdAllocator allocator;
    allocator.GenerateId();
    GetCompositor()->SetScaleAndSize(1.25f, gfx::Size(100, 100), allocator.GetCurrentLocalSurfaceId());
    GetCompositor()->SetRootLayer(root.get());
    root->Add(c1.get());
    c1->Add(c11.get());
    ...
    EXPECT_EQ("-0.20 -0.20", Vector2dFTo100thPrecisionString(c11->GetSubpixelOffset()));

    allocator.GenerateId();
    GetCompositor()->SetScaleAndSize(1.5f, gfx::Size(100, 100), allocator.GetCurrentLocalSurfaceId());
    // 1 at 1.5 scale = 1.5 : (round(1.5) - 1.5) / 1.5 = 0.33
    EXPECT_EQ("0.33 0.33", Vector2dFTo100thPrecisionString(c11->GetSubpixelOffset()));

    c11->SetBounds(gfx::Rect(2, 2, 10, 10));
    // 2 at 1.5 scale = 3 : (round(3) - 3) / 1.5 = 0
    EXPECT_EQ("0.00 0.00", Vector2dFTo100thPrecisionString(c11->GetSubpixelOffset()));
}
...
TEST(LayerDelegateTest, OnLayerBoundsChanged)
{
    ...
    EXPECT_CALL(delegate, OnLayerBoundsChanged(initial_bounds, PropertyChangeReason::NOT_FROM_ANIMATION))
...
{
    ScopedAnimationDurationScaleMode scoped_animation_duration_scale_mode(ScopedAnimationDurationScaleMode::NORMAL_DURATION);
    LayerAnimatorTestController test_controller(LayerAnimator::CreateImplicitAnimator());
    ...
    // Start the animation.
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateBoundsElement(kTargetBounds, base::Seconds(1));
    ...
    EXPECT_CALL(delegate, OnLayerBoundsChanged(initial_bounds, PropertyChangeReason::FROM_ANIMATION))
        .WillOnce(testing::Invoke([&](const gfx::Rect&, PropertyChangeReason) {
            // Verify that |layer->bounds()| returns the correct value when the
            // delegate is notified.
    ...
    // End the animation.
    EXPECT_CALL(delegate, OnLayerBoundsChanged(step_bounds, PropertyChangeReason::FROM_ANIMATION))
        .WillOnce(testing::Invoke([&](const gfx::Rect&, PropertyChangeReason) {
            // Verify that |layer->bounds()| returns the correct value when the
            // delegate is notified.
            EXPECT_EQ(layer->bounds(), kTargetBounds);
...
TEST(LayerDelegateTest, OnLayerTransformed)
{
    ...
    {
        EXPECT_CALL(delegate, OnLayerTransformed(gfx::Transform(), PropertyChangeReason::NOT_FROM_ANIMATION))
            .WillOnce(testing::Invoke([&](const gfx::Transform& old_transform, PropertyChangeReason) {
    ...
    EXPECT_CALL(delegate, OnLayerTransformed(target_transform1, PropertyChangeReason::NOT_FROM_ANIMATION))
        .WillOnce(testing::Invoke([&](const gfx::Transform& old_transform, PropertyChangeReason) {
            // Verify that |layer->transform()| returns the correct value when
            // the delegate is notified.
...
TEST(LayerDelegateTest, OnLayerTransformedNonThreadedAnimation)
{
    ...
    const gfx::Transform initial_transform = interpolated_transform->Interpolate(0.0);
    const gfx::Transform step_transform = interpolated_transform->Interpolate(0.5);
    const gfx::Transform target_transform = interpolated_transform->Interpolate(1.0);

    // Start the animation.
    std::unique_ptr<LayerAnimationElement> element
        = LayerAnimationElement::CreateInterpolatedTransformElement(std::move(interpolated_transform), base::Seconds(1));
    // The LayerAnimationElement returned by CreateInterpolatedTransformElement()
    // is non-threaded.
    ASSERT_FALSE(element->IsThreaded(layer.get()));
    LayerAnimationElement* element_raw = element.get();
    EXPECT_CALL(delegate, OnLayerTransformed(gfx::Transform(), PropertyChangeReason::FROM_ANIMATION))
        .WillOnce(testing::Invoke([&](const gfx::Transform& old_transform, PropertyChangeReason) {
    ...
    // Progress the animation.
    EXPECT_CALL(delegate, OnLayerTransformed(initial_transform, PropertyChangeReason::FROM_ANIMATION))
        .WillOnce(testing::Invoke([&](const gfx::Transform& old_transform, PropertyChangeReason) {
            // Verify that |layer->transform()| returns the correct value when
            // the delegate is notified.
    ...
    // End the animation.
    EXPECT_CALL(delegate, OnLayerTransformed(step_transform, PropertyChangeReason::FROM_ANIMATION))
        .WillOnce(testing::Invoke([&](const gfx::Transform& old_transform, PropertyChangeReason) {
            // Verify that |layer->transform()| returns the correct value when the
            // delegate is notified.
            EXPECT_EQ(layer->transform(), target_transform);
            EXPECT_FALSE(animator->IsAnimatingProperty(LayerAnimationElement::TRANSFORM));
        }));
    test_controller.Step(element_raw->duration() / 2);
    testing::Mock::VerifyAndClear(&delegate);
}
...
TEST(LayerDelegateTest, OnLayerTransformedThreadedAnimation)
{
    ...
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateTransformElement(target_transform, base::Seconds(1));
    ...
    EXPECT_CALL(delegate, OnLayerTransformed(gfx::Transform(), PropertyChangeReason::FROM_ANIMATION)).Times(0);
    animator->StartAnimation(new LayerAnimationSequence(std::move(element)));
    testing::Mock::VerifyAndClear(&delegate);
    ...
    test_controller.Step(element_raw->duration() + (element_raw->effective_start_time() - animator->last_step_time()));
    testing::Mock::VerifyAndClear(&delegate);
}
...
TEST(LayerDelegateTest, OnLayerOpacityChanged)
{
    ...
    EXPECT_CALL(delegate, OnLayerOpacityChanged(PropertyChangeReason::NOT_FROM_ANIMATION)).WillOnce(testing::Invoke([&](PropertyChangeReason) {
...
TEST(LayerDelegateTest, OnLayerOpacityChangedAnimation)
{
    ...
    std::unique_ptr<LayerAnimationElement> element = LayerAnimationElement::CreateOpacityElement(kTargetOpacity, base::Seconds(1));
    ASSERT_TRUE(element->IsThreaded(layer.get()));
    LayerAnimationElement* element_raw = element.get();
    EXPECT_CALL(delegate, OnLayerOpacityChanged(PropertyChangeReason::FROM_ANIMATION)).WillOnce(testing::Invoke([&](PropertyChangeReason) {
        // Verify that |layer->opacity()| returns the correct value when the
        // delegate is notified.
        EXPECT_EQ(layer->opacity(), initial_opacity);
        EXPECT_TRUE(animator->IsAnimatingProperty(LayerAnimationElement::OPACITY));
    }));
    animator->StartAnimation(new LayerAnimationSequence(std::move(element)));
    testing::Mock::VerifyAndClear(&delegate);
    test_controller.StartThreadedAnimationsIfNeeded();
