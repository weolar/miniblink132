...
cc::ScrollState CreateScrollState(const ScrollEvent& scroll, bool is_begin)
{
    ...
    scroll_state_data.is_in_inertial_phase = scroll.momentum_phase() == EventMomentumPhase::INERTIAL_UPDATE;
...
ScrollInputHandler::ScrollInputHandler(const base::WeakPtr<cc::InputHandler>& input_handler)
...
{
    DCHECK(!input_handler_weak_ptr_) << "Pointer invalidated before WillShutdown() is called.";
}

bool ScrollInputHandler::OnScrollEvent(const ScrollEvent& event, Layer* layer_to_scroll)
{
    ...
    scroll_state_begin.data()->set_current_native_scrolling_element(layer_to_scroll->element_id());

    // Note: the WHEEL type covers both actual wheels as well as trackpad
    // scrolling.
    cc::InputHandler::ScrollStatus result = input_handler_weak_ptr_->ScrollBegin(&scroll_state_begin, ui::ScrollInputType::kWheel);
    ...
    input_handler_weak_ptr_->ScrollUpdate(CreateScrollState(event, false), base::TimeDelta());
...
void ScrollInputHandler::UpdateRootLayerStateForSynchronousInputHandler(const gfx::PointF& total_scroll_offset, const gfx::PointF& max_scroll_offset,
    const gfx::SizeF& scrollable_size, float page_scale_factor, float min_page_scale_factor, float max_page_scale_factor)
{
}

void ScrollInputHandler::DeliverInputForBeginFrame(const viz::BeginFrameArgs& args)
{
}
...
void ScrollInputHandler::SetScrollEventDispatchMode(cc::InputHandlerClient::ScrollEventDispatchMode mode)
{
}
