...
public:
    ...
    bool animation_cycle_ended() const
    {
        return animation_cycle_ended_;
    }
    bool animation_will_start_playing() const
    {
        return animation_will_start_playing_;
    }
    bool animation_resuming() const
    {
        return animation_resuming_;
    }
    bool animation_stopped() const
    {
        return animation_stopped_;
    }
    bool animation_is_deleted() const
    {
        return animation_is_deleted_;
    }
...
public:
    ...
    public:
        ...
        const std::optional<float>& last_frame_t() const
        {
            return last_frame_t_;
        }
    ...
    TestSkottieFrameDataProvider& operator=(const TestSkottieFrameDataProvider&) = delete;
    ~TestSkottieFrameDataProvider() override = default;

    scoped_refptr<ImageAsset> LoadImageAsset(std::string_view resource_id, const base::FilePath& resource_path, const std::optional<gfx::Size>& size) override
    {
...
public:
    ...
    void SetUp() override
    {
        canvas_ = std::make_unique<gfx::Canvas>(gfx::Size(kAnimationWidth, kAnimationHeight), 1.f, false);
        skottie_ = cc::SkottieWrapper::UnsafeCreateNonSerializable(base::as_bytes(base::make_span(kData, std::strlen(kData))));
    ...
    Animation::PlayState GetState() const
    {
        return animation_->state_;
    }
    ...
    bool IsPaused() const
    {
        return GetState() == Animation::PlayState::kPaused;
    }
    ...
    const base::TickClock* test_clock() const
    {
        return &test_clock_;
    }

    void AdvanceClock(base::TimeDelta advance)
    {
        test_clock_.Advance(advance);
    }
...
protected:
    void SetUp() override
    {
        ...
        animation_ = std::make_unique<Animation>(skottie_, cc::SkottieColorMap(), &frame_data_provider_);
    ...
    {
        return { .image = cc::CreateDiscardablePaintImage(animation_->GetOriginalSize()), .quality = cc::PaintFlags::FilterQuality::kHigh };
    }
...
TEST_F(AnimationTest, PlayLinearAnimation)
{
    ...
    EXPECT_TRUE(IsStopped());
    animation_->Start(Animation::PlaybackConfig::CreateWithStyle(Animation::Style::kLinear, *animation_));
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), kAdvance / kAnimationDuration);
    EXPECT_EQ(TimeDeltaSince(GetTimerPreviousTick()), base::TimeDelta());
    IsAllSameColor(SK_ColorGREEN, canvas()->GetBitmap());

    // Advance the clock to the end of the animation.
    constexpr auto kAdvanceToEnd = kAnimationDuration - kAdvance + base::Milliseconds(1);
...
TEST_F(AnimationTest, PlaySubsectionOfLinearAnimation)
{
    ...
    animation_->Start(Animation::PlaybackConfig({ { kStartTime, kStartTime + kDuration } }, /*initial_offset=*/kStartTime,
    ...
    EXPECT_FLOAT_EQ(GetTimerEndOffset(), (kStartTime + kDuration) / kAnimationDuration);
    ...
    EXPECT_EQ(TimeDeltaSince(GetTimerPreviousTick()), base::TimeDelta());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime + kAdvance) / kAnimationDuration);
    EXPECT_FALSE(observer.animation_cycle_ended());
    ...
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime + kAdvance + kAdvance2) / kAnimationDuration);
    EXPECT_FALSE(observer.animation_cycle_ended());

    // Reach the end of animation.
    constexpr auto kAdvanceToEnd = kDuration - kAdvance - kAdvance2 + base::Milliseconds(1);
    AdvanceClock(kAdvanceToEnd);
    EXPECT_EQ(TimeDeltaSince(GetTimerPreviousTick()), kAdvanceToEnd);
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
...
TEST_F(AnimationTest, PausingLinearAnimation)
{
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime + kAdvance * 2) / kAnimationDuration);
...
TEST_F(AnimationTest, PlayLoopAnimation)
{
    ...
    EXPECT_TRUE(IsStopped());
    animation_->Start(Animation::PlaybackConfig::CreateWithStyle(Animation::Style::kLoop, *animation_));
    EXPECT_TRUE(IsScheduledToPlay());
    EXPECT_FALSE(observer.animation_will_start_playing());
    ...
    EXPECT_EQ(TimeDeltaSince(GetTimerPreviousTick()), kAnimationDuration - kAdvance);
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
...
TEST_F(AnimationTest, PlaySubsectionOfLoopAnimation)
{
    ...
    EXPECT_EQ(TimeDeltaSince(GetTimerPreviousTick()), kDuration - kAdvance - kAdvance2);
...
TEST_F(AnimationTest, PlayDifferentSubsectionsOfLoopingAnimation)
{
    ...
    animation_->Start(Animation::PlaybackConfig({ { kStartTime1, kStartTime1 + kDuration1 }, { kStartTime2, kStartTime2 + kDuration2 } },
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), kStartTime1 / kAnimationDuration);

    // T: 600 ms
    AdvanceClock(base::Milliseconds(200));
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime1 + base::Milliseconds(200)) / kAnimationDuration);

    // T: 1399 ms (just before first cycle end)
    AdvanceClock(base::Milliseconds(799));
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime1 + kDuration1 - base::Milliseconds(1)) / kAnimationDuration);

    // T: 100 ms (start of second cycle)
    AdvanceClock(base::Milliseconds(1));
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), kStartTime2 / kAnimationDuration);

    // T: 300 ms
    AdvanceClock(base::Milliseconds(200));
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime2 + base::Milliseconds(200)) / kAnimationDuration);

    // T: 599 ms (just before second cycle end)
    AdvanceClock(base::Milliseconds(299));
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime2 + kDuration2 - base::Milliseconds(1)) / kAnimationDuration);
...
TEST_F(AnimationTest, HandlesLargeStepsInLoopingAnimation)
{
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime2 + base::Milliseconds(100)) / kAnimationDuration);
...
TEST_F(AnimationTest, PausingLoopAnimation)
{
    ...
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), kStartTime / kAnimationDuration);
...
TEST_F(AnimationTest, PlayThrobbingAnimation)
{
    ...
    animation_->Start(Animation::PlaybackConfig::CreateWithStyle(Animation::Style::kThrobbing, *animation_));
    EXPECT_TRUE(IsScheduledToPlay());
    EXPECT_FALSE(observer.animation_will_start_playing());
...
TEST_F(AnimationTest, PlaySubsectionOfThrobbingAnimation)
{
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime + kDuration - kAdvance3) / kAnimationDuration);
    EXPECT_TRUE(IsPlaying());
    EXPECT_FALSE(observer.animation_cycle_ended());
...
TEST_F(AnimationTest, PlayDifferentSubsectionsOfThrobbingAnimation)
{
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime2 + kDuration2) / kAnimationDuration);

    // T: 400 ms
    AdvanceClock(base::Milliseconds(200));
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime2 + kDuration2 - base::Milliseconds(200)) / kAnimationDuration);

    // T: 101 ms (just after start of second cycle start)
    AdvanceClock(base::Milliseconds(299));
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime2 + base::Milliseconds(1)) / kAnimationDuration);
...
TEST_F(AnimationTest, HandlesLargeStepsInThrobbingAnimation)
{
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime2 + kDuration2 - base::Milliseconds(100)) / kAnimationDuration);
...
TEST_F(AnimationTest, PausingThrobbingAnimation)
{
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime + kDuration - kAdvance) / kAnimationDuration);
    EXPECT_TRUE(IsPlaying());
    ...
    AdvanceClock(kAdvance2);
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime + kDuration - kAdvance - kAdvance2) / kAnimationDuration);
...
TEST_F(AnimationTest, NotifiesObserverFramePainted)
{
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(observer.last_frame_painted());
    EXPECT_FLOAT_EQ(*observer.last_frame_painted(), base::Milliseconds(1) / kAnimationDuration);
}
...
TEST_F(AnimationWithImageAssetsTest, PaintsAnimationImagesToCanvas)
{
    ...
    TestSkottieFrameDataProvider::ImageAssetImpl* asset_0 = frame_data_provider_.GetLoadedImageAsset("image_0");
    TestSkottieFrameDataProvider::ImageAssetImpl* asset_1 = frame_data_provider_.GetLoadedImageAsset("image_1");
    ...
    EXPECT_THAT(paint_record,
        ElementsAre(AllOf(PaintOpIs<cc::DrawSkottieOp>(),
            ResultOf(
                [](const cc::PaintOp& op) { return static_cast<const cc::DrawSkottieOp&>(op).images; }, ElementsAre(cc::SkottieImageIs("image_0", frame_0))))));
    ...
    EXPECT_THAT(paint_record,
        ElementsAre(AllOf(PaintOpIs<cc::DrawSkottieOp>(),
            ResultOf(
                [](const cc::PaintOp& op) { return static_cast<const cc::DrawSkottieOp&>(op).images; }, ElementsAre(cc::SkottieImageIs("image_1", frame_1))))));
}
...
TEST_F(AnimationWithImageAssetsTest, GracefullyHandlesNullImages)
{
    ...
    EXPECT_THAT(paint_record,
        ElementsAre(AllOf(PaintOpIs<cc::DrawSkottieOp>(),
            ResultOf([](const cc::PaintOp& op) { return static_cast<const cc::DrawSkottieOp&>(op).images; },
                ElementsAre(cc::SkottieImageIs("image_0", cc::SkottieFrameData()))))));
}
...
TEST_F(AnimationWithImageAssetsTest, LoadsCorrectFrameTimestamp)
{
    ...
    base::TimeDelta three_quarter_duration = animation_->GetAnimationDuration() * .75;
    ...
    EXPECT_THAT(asset_1_->last_frame_t().value(), FloatNear((three_quarter_duration - half_duration).InSecondsF(), kFrameTimestampToleranceSec));
    ...
    ASSERT_TRUE(asset_0_->last_frame_t().has_value());
    EXPECT_THAT(asset_0_->last_frame_t().value(), FloatNear(quarter_duration.InSecondsF(), kFrameTimestampToleranceSec));
}
...
TEST_F(AnimationWithImageAssetsTest, LoadsCorrectImageScale)
{
    ...
    EXPECT_THAT(asset_0_->last_frame_scale_factor().value(), FloatEq(kCanvasImageScale));
}
...
public:
    explicit AnimationRestarter(Animation* animation)
...
TEST_F(AnimationTest, GetPlaybackConfig)
{
    ...
    Animation::PlaybackConfig test_config({ { /*start_offset=*/kAnimationDuration / 4,
    ...
    EXPECT_THAT(*animation_->GetPlaybackConfig(),
        FieldsAre(ElementsAre(FieldsAre(test_config.scheduled_cycles.front().start_offset, test_config.scheduled_cycles.front().end_offset)),
            test_config.initial_offset, test_config.initial_completed_cycles, test_config.style));
    animation_->Stop();
    EXPECT_FALSE(animation_->GetPlaybackConfig());
...
TEST_F(AnimationTest, StartsAtArbitraryInitialTimestamp)
{
    ...
    animation_->Start(Animation::PlaybackConfig({ { kStartTime, kStartTime + kDuration } },
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime + base::Milliseconds(100)) / kAnimationDuration);

    // T: 600 ms
    AdvanceClock(base::Milliseconds(100));
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime + base::Milliseconds(200)) / kAnimationDuration);
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime + base::Milliseconds(500)) / kAnimationDuration);

    // T: 800 ms
    AdvanceClock(base::Milliseconds(100));
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_FLOAT_EQ(*animation_->GetCurrentProgress(), (kStartTime + base::Milliseconds(400)) / kAnimationDuration);
}
...
TEST_F(AnimationTest, GetCurrentCycleBoundaries)
{
    ...
    animation_->Start(Animation::PlaybackConfig({ { kStartTime1, kEndTime1 }, { kStartTime2, kEndTime2 } },
        /*initial_offset=*/kStartTime1,
        /*initial_completed_cycles=*/0, Animation::Style::kLoop));
    ...
    EXPECT_THAT(*animation_->GetCurrentCycleBoundaries(), FieldsAre(kStartTime1, kEndTime1));
    ...
    animation_->Paint(canvas(), NowTicks(), animation_->GetOriginalSize());
    ASSERT_TRUE(animation_->GetCurrentProgress());
    EXPECT_THAT(*animation_->GetCurrentCycleBoundaries(), FieldsAre(kStartTime2, kEndTime2));
