...
bool IsCycleValid(const Animation::CycleBoundaries& boundaries, const Animation& animation)
{
    return boundaries.start_offset >= base::TimeDelta() && boundaries.end_offset <= animation.GetAnimationDuration()
        && boundaries.start_offset < boundaries.end_offset;
}

bool IsInCycleBoundaries(base::TimeDelta offset, const Animation::CycleBoundaries& boundaries)
{
    return offset >= boundaries.start_offset && offset < boundaries.end_offset;
}

Animation::CycleBoundaries GetCycleAtIndex(const std::vector<Animation::CycleBoundaries>& scheduled_cycles, int cycle_idx)
{
    DCHECK(!scheduled_cycles.empty());
    return scheduled_cycles[std::min(cycle_idx, static_cast<int>(scheduled_cycles.size()) - 1)];
}

} // namespace

Animation::TimerControl::TimerControl(std::vector<CycleBoundaries> scheduled_cycles, base::TimeDelta initial_offset, int initial_completed_cycles,
    const base::TimeDelta& total_duration, const base::TimeTicks& start_timestamp, bool should_reverse, float playback_speed)
...
void Animation::TimerControl::Step(const base::TimeTicks& timestamp)
{
    ...
    while (!step_size.is_zero()) {
        base::TimeDelta time_until_current_cycle_end
            = IsPlayingInReverse() ? (current_cycle_progress_ - current_cycle_.start_offset) : (current_cycle_.end_offset - current_cycle_progress_);
        if (step_size >= time_until_current_cycle_end) {
            ++completed_cycles_;
            current_cycle_ = GetCycleAtIndex(scheduled_cycles_, completed_cycles_);
            current_cycle_progress_ = IsPlayingInReverse() ? current_cycle_.end_offset : current_cycle_.start_offset;
...
// static
Animation::CycleBoundaries Animation::CycleBoundaries::FullCycle(const Animation& animation)
{
    return {
...
// static
Animation::PlaybackConfig Animation::PlaybackConfig::CreateDefault(const Animation& animation)
{
...
// static
Animation::PlaybackConfig Animation::PlaybackConfig::CreateWithStyle(Style style, const Animation& animation)
{
...
Animation::PlaybackConfig::PlaybackConfig(
    std::vector<CycleBoundaries> scheduled_cycles, base::TimeDelta initial_offset, int initial_completed_cycles, Style style)
...
Animation::PlaybackConfig::PlaybackConfig(const PlaybackConfig& other) = default;

Animation::PlaybackConfig& Animation::PlaybackConfig::operator=(const PlaybackConfig& other) = default;

Animation::PlaybackConfig::~PlaybackConfig() = default;

Animation::Animation(scoped_refptr<cc::SkottieWrapper> skottie, cc::SkottieColorMap color_map, cc::SkottieFrameDataProvider* frame_data_provider)
    : skottie_(skottie)
    , color_map_(std::move(color_map))
    , text_map_(skottie_->GetCurrentTextPropertyValues())
{
    DCHECK(skottie_);
    bool animation_has_external_image_assets = !skottie_->GetImageAssetMetadata().asset_storage().empty();
    ...
    if (animation_has_external_image_assets) {
        ...
        for (const auto& asset_metadata_pair : skottie_->GetImageAssetMetadata().asset_storage()) {
            const std::string& asset_id = asset_metadata_pair.first;
            const cc::SkottieResourceMetadataMap::ImageAssetMetadata& asset_metadata = asset_metadata_pair.second;
            scoped_refptr<cc::SkottieFrameDataProvider::ImageAsset> new_asset
                = frame_data_provider->LoadImageAsset(asset_id, asset_metadata.resource_path, asset_metadata.size);
            DCHECK(new_asset);
            image_assets_.emplace(cc::HashSkottieResourceId(asset_id), std::move(new_asset));
        }
    }
}
...
std::optional<Animation::CycleBoundaries> Animation::GetCurrentCycleBoundaries() const
{
...
void Animation::Paint(gfx::Canvas* canvas, const base::TimeTicks& timestamp, const gfx::Size& size)
{
...
void Animation::PaintFrame(gfx::Canvas* canvas, float t, const gfx::Size& size)
{
    ...
    skottie_->Seek(t, base::BindRepeating(&Animation::LoadImageForAsset, base::Unretained(this), canvas, std::ref(all_frame_data)));
    canvas->DrawSkottie(skottie(), gfx::Rect(size), t, std::move(all_frame_data), color_map_, text_map_);
...
cc::SkottieWrapper::FrameDataFetchResult Animation::LoadImageForAsset(
    gfx::Canvas* canvas, cc::SkottieFrameDataMap& all_frame_data, cc::SkottieResourceIdHash asset_id, float t, sk_sp<SkImage>&, SkSamplingOptions&)
{
    TRACE_EVENT0("ui", "Animation::LoadImageForAsset");
    cc::SkottieFrameDataProvider::ImageAsset& image_asset = *image_assets_.at(asset_id);
    all_frame_data.emplace(asset_id, image_asset.GetFrameData(t, canvas->image_scale()));
...
{
    DCHECK(!timer_control_);
    timer_control_ = std::make_unique<TimerControl>(playback_config_.scheduled_cycles, playback_config_.initial_offset,
        playback_config_.initial_completed_cycles, GetAnimationDuration(), timestamp, playback_config_.style == Style::kThrobbing, playback_speed_);
}
...
void Animation::VerifyPlaybackConfigIsValid(const PlaybackConfig& playback_config) const
{
    ...
    DCHECK(IsInCycleBoundaries(playback_config.initial_offset, GetCycleAtIndex(playback_config.scheduled_cycles, playback_config.initial_completed_cycles)));
}
