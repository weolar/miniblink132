...
public:
    ...
    struct COMPONENT_EXPORT(UI_LOTTIE) PlaybackConfig {
        ...
        // |style|.
        static PlaybackConfig CreateWithStyle(Style style, const Animation& animation);

        PlaybackConfig();
        PlaybackConfig(std::vector<CycleBoundaries> scheduled_cycles, base::TimeDelta initial_offset, int initial_completed_cycles, Style style);
    ...
    explicit Animation(scoped_refptr<cc::SkottieWrapper> skottie, cc::SkottieColorMap color_map = cc::SkottieColorMap(),
    ...
    // |size|.
    void Paint(gfx::Canvas* canvas, const base::TimeTicks& timestamp, const gfx::Size& size);
    ...
    scoped_refptr<cc::SkottieWrapper> skottie() const
    {
        return skottie_;
    }
...
private:
    ...
    public:
        TimerControl(std::vector<CycleBoundaries> scheduled_cycles, base::TimeDelta initial_offset, int initial_completed_cycles,
        ...
        CycleBoundaries current_cycle() const
        {
    ...
    base::flat_map<cc::SkottieResourceIdHash, scoped_refptr<cc::SkottieFrameDataProvider::ImageAsset>> image_assets_;
