...
public:
    ...
    void SetOrientation(TouchHandleOrientation orientation, bool mirror_vertical, bool mirror_horizontal) override
    {
    }
    ...
    {
        return *intersects_rect_ ? gfx::RectF(-1000, -1000, 2000, 2000) : gfx::RectF(-1000, -1000, 0, 0);
    }
...
class TouchSelectionControllerTest : public testing::Test, public TouchSelectionControllerClient {
public:
    TouchSelectionControllerTest() = default;

    TouchSelectionControllerTest(const TouchSelectionControllerTest&) = delete;
    TouchSelectionControllerTest& operator=(const TouchSelectionControllerTest&) = delete;
    ...
    void OnDragUpdate(const TouchSelectionDraggable::Type type, const gfx::PointF& position) override
    {
    ...
    {
        controller_->WillHandleTouchEvent(MockMotionEvent(MotionEvent::Action::DOWN));
    }
    ...
    {
        dragging_enabled_ = enabled;
    }

    void ClearSelection()
    {
        controller_->OnSelectionBoundsChanged(gfx::SelectionBound(), gfx::SelectionBound());
    }
    ...
    void ChangeSelection(const gfx::RectF& start_rect, bool start_visible, const gfx::RectF& end_rect, bool end_visible)
    {
    ...
    void ChangeVerticalSelection(const gfx::RectF& start_rect, bool start_visible, const gfx::RectF& end_rect, bool end_visible)
    {
        gfx::SelectionBound start_bound, end_bound;
    ...
    {
        controller().HandleLongPressEvent(base::TimeTicks(), kIgnoredPoint);
    }
    ...
    {
        controller().HandleTapEvent(kIgnoredPoint, 3);
    }
    ...
    const gfx::PointF& GetLastCaretPosition() const
    {
        return caret_position_;
    }
    const gfx::PointF& GetLastSelectionStart() const
    {
        return selection_start_;
    }
    const gfx::PointF& GetLastSelectionEnd() const
    {
        return selection_end_;
    }
    const gfx::PointF& GetLastEventStart() const
    {
        return last_event_start_;
    }
    const gfx::PointF& GetLastEventEnd() const
    {
        return last_event_end_;
    }
...
TEST_F(TouchSelectionControllerTest, InsertionToSelectionTransition)
{
    ...
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(INSERTION_HANDLE_CLEARED, SELECTION_HANDLES_SHOWN));
    EXPECT_EQ(start_rect.bottom_left(), GetLastEventStart());

    ChangeInsertion(end_rect, visible);
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLES_CLEARED, INSERTION_HANDLE_SHOWN));
...
TEST_F(TouchSelectionControllerTest, InsertionDeactivatedWhileDragging)
{
    ...
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(INSERTION_HANDLE_DRAG_STOPPED, INSERTION_HANDLE_CLEARED));
...
TEST_F(TouchSelectionControllerTest, InsertionTapped)
{
    ...
    OnTapEvent();
    ChangeInsertion(start_rect, visible);
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(INSERTION_HANDLE_CLEARED, INSERTION_HANDLE_SHOWN));
    ...
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    event = MockMotionEvent(MockMotionEvent::Action::UP, event_time + base::Seconds(1), 0, 0);
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
...
TEST_F(TouchSelectionControllerTest, SelectionAllowedByTripleTapOnEditableArabicVowel)
{
    gfx::RectF start_rect(5, 5, 0, 10);
...
TEST_F(TouchSelectionControllerTest, SelectionDraggedWithOverlap)
{
    ...
    MockMotionEvent event(MockMotionEvent::Action::DOWN, event_time, touch_down_x, 0);
    SetDraggingEnabled(true);
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLE_DRAG_STARTED));
    EXPECT_FALSE(GetAndResetSelectionMoved());
    ...
    EXPECT_TRUE(GetAndResetSelectionMoved());
    EXPECT_EQ(fixed_offset, GetLastSelectionStart());
    EXPECT_EQ(end_offset - gfx::Vector2dF(touch_down_x, 0), GetLastSelectionEnd());
...
TEST_F(TouchSelectionControllerTest, SelectionDraggedToSwitchBaseAndExtent)
{
    ...
    MockMotionEvent event(MockMotionEvent::Action::DOWN, event_time, end_rect.x(), end_rect.bottom());
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_FALSE(GetAndResetSelectionMoved());
    ...
    event = MockMotionEvent(MockMotionEvent::Action::MOVE, event_time, end_rect.x(), end_rect.bottom() + 5);
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLE_DRAG_STARTED));
    EXPECT_TRUE(GetAndResetSelectionMoved());
    EXPECT_FALSE(GetAndResetSelectionPointsSwapped());
    ...
    event = MockMotionEvent(MockMotionEvent::Action::DOWN, event_time, start_rect.x(), start_rect.bottom());
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_FALSE(GetAndResetSelectionMoved());
    ...
    event = MockMotionEvent(MockMotionEvent::Action::MOVE, event_time, start_rect.x(), start_rect.bottom() + 5);
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLE_DRAG_STARTED));
    EXPECT_TRUE(GetAndResetSelectionMoved());
    EXPECT_FALSE(GetAndResetSelectionPointsSwapped());
    ...
    // Move the base, triggering a swap of points.
    event = MockMotionEvent(MockMotionEvent::Action::DOWN, event_time, end_rect.x(), end_rect.bottom());
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_FALSE(GetAndResetSelectionMoved());
    EXPECT_TRUE(GetAndResetSelectionPointsSwapped());
...
TEST_F(TouchSelectionControllerTest, SelectionDragExtremeLineSize)
{
    ...
    MockMotionEvent event(MockMotionEvent::Action::DOWN, event_time, small_line_rect.x(), small_line_rect.y());
    SetDraggingEnabled(true);
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLE_DRAG_STARTED));
    ...
    EXPECT_EQ(large_line_rect.bottom_left() - gfx::Vector2dF(0, 8.f), GetLastSelectionStart());
    EXPECT_EQ(small_line_rect.CenterPoint(), GetLastSelectionEnd());

    small_line_rect += gfx::Vector2dF(25.f, 0);
    event = MockMotionEvent(MockMotionEvent::Action::MOVE, event_time, small_line_rect.x(), small_line_rect.y());
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_TRUE(GetAndResetSelectionMoved());
    EXPECT_EQ(small_line_rect.CenterPoint(), GetLastSelectionEnd());
}
...
TEST_F(TouchSelectionControllerTest, LongPressDrag)
{
    ...
    EXPECT_TRUE(controller().WillHandleTouchEvent(event.MovePoint(0, 0, kDefaultTapSlop)));
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLE_DRAG_STARTED));
    EXPECT_EQ(fixed_offset, GetLastSelectionStart());
    EXPECT_EQ(end_offset, GetLastSelectionEnd());

    // Movement after the start of drag will be relative to the moved endpoint.
    EXPECT_TRUE(controller().WillHandleTouchEvent(event.MovePoint(0, 0, 2 * kDefaultTapSlop)));
    EXPECT_TRUE(GetAndResetSelectionMoved());
    EXPECT_EQ(end_offset + gfx::Vector2dF(0, kDefaultTapSlop), GetLastSelectionEnd());

    EXPECT_TRUE(controller().WillHandleTouchEvent(event.MovePoint(0, kDefaultTapSlop, 2 * kDefaultTapSlop)));
    EXPECT_TRUE(GetAndResetSelectionMoved());
    EXPECT_EQ(end_offset + gfx::Vector2dF(kDefaultTapSlop, kDefaultTapSlop), GetLastSelectionEnd());

    EXPECT_TRUE(controller().WillHandleTouchEvent(event.MovePoint(0, 2 * kDefaultTapSlop, 2 * kDefaultTapSlop)));
    EXPECT_TRUE(GetAndResetSelectionMoved());
    EXPECT_EQ(end_offset + gfx::Vector2dF(2 * kDefaultTapSlop, kDefaultTapSlop), GetLastSelectionEnd());
...
TEST_F(TouchSelectionControllerTest, DoublePressDrag)
{
    ...
    EXPECT_TRUE(controller().WillHandleTouchEvent(event.MovePoint(0, 0, 2 * kDefaultTapSlop)));
    EXPECT_TRUE(GetAndResetSelectionMoved());
    EXPECT_EQ(end_rect.CenterPoint() + gfx::Vector2dF(0, kDefaultTapSlop), GetLastSelectionEnd());

    EXPECT_TRUE(controller().WillHandleTouchEvent(event.MovePoint(0, kDefaultTapSlop, 2 * kDefaultTapSlop)));
    EXPECT_TRUE(GetAndResetSelectionMoved());
    EXPECT_EQ(end_rect.CenterPoint() + gfx::Vector2dF(kDefaultTapSlop, kDefaultTapSlop), GetLastSelectionEnd());

    EXPECT_TRUE(controller().WillHandleTouchEvent(event.MovePoint(0, 2 * kDefaultTapSlop, 2 * kDefaultTapSlop)));
    EXPECT_TRUE(GetAndResetSelectionMoved());
    EXPECT_EQ(end_rect.CenterPoint() + gfx::Vector2dF(2 * kDefaultTapSlop, kDefaultTapSlop), GetLastSelectionEnd());
...
TEST_F(TouchSelectionControllerTest, NoLongPressDragIfDisabled)
{
    ...
    EXPECT_FALSE(controller().WillHandleTouchEvent(event.MovePoint(0, 0, kDefaultTapSlop * 10)));
    EXPECT_THAT(GetAndResetEvents(), IsEmpty());
...
TEST_F(TouchSelectionControllerTest, SelectionNoOrientationChangeWhenSwapped)
{
    ...
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLES_SHOWN));
    EXPECT_EQ(start_rect.bottom_left(), GetLastEventStart());
    EXPECT_EQ(test_controller.GetStartHandleOrientation(), TouchHandleOrientation::LEFT);
    EXPECT_EQ(test_controller.GetEndHandleOrientation(), TouchHandleOrientation::RIGHT);

    SetDraggingEnabled(true);

    // Simulate moving the base, not triggering a swap of points.
    MockMotionEvent event(MockMotionEvent::Action::DOWN, event_time, start_rect.x(), start_rect.bottom());
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLE_DRAG_STARTED));
    ...
    event = MockMotionEvent(MockMotionEvent::Action::UP, event_time, offset_rect.x(), offset_rect.bottom());
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLE_DRAG_STOPPED));
    EXPECT_EQ(test_controller.GetStartHandleOrientation(), TouchHandleOrientation::LEFT);
    EXPECT_EQ(test_controller.GetEndHandleOrientation(), TouchHandleOrientation::RIGHT);
    ...
    event = MockMotionEvent(MockMotionEvent::Action::DOWN, event_time, offset_rect.x(), offset_rect.bottom());
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLE_DRAG_STARTED));
    ...
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLES_MOVED));
    EXPECT_EQ(test_controller.GetStartHandleOrientation(), TouchHandleOrientation::LEFT);
    EXPECT_EQ(test_controller.GetEndHandleOrientation(), TouchHandleOrientation::LEFT);
    ...
    ChangeSelection(offset_rect, visible, end_rect, visible);
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLES_MOVED));
    EXPECT_EQ(test_controller.GetStartHandleOrientation(), TouchHandleOrientation::RIGHT);
    EXPECT_EQ(test_controller.GetEndHandleOrientation(), TouchHandleOrientation::RIGHT);
...
TEST_F(TouchSelectionControllerTest, VerticalTextSelectionHandleSwap)
{
    ...
    MockMotionEvent event(MockMotionEvent::Action::DOWN, event_time, start_rect.right(), start_rect.bottom());
    EXPECT_TRUE(controller().WillHandleTouchEvent(event));
    EXPECT_THAT(GetAndResetEvents(), ElementsAre(SELECTION_HANDLE_DRAG_STARTED));
