...
public:
    ...
    virtual void SelectBetweenCoordinates(const gfx::PointF& base, const gfx::PointF& extent) = 0;
    virtual void OnSelectionEvent(SelectionEventType event) = 0;
    virtual void OnDragUpdate(const TouchSelectionDraggable::Type type, const gfx::PointF& position) = 0;
...
class UI_TOUCH_SELECTION_EXPORT TouchSelectionController : public TouchHandleClient, public LongPressDragSelectorClient {
public:
    enum ActiveStatus {
        INACTIVE,
        INSERTION_ACTIVE,
    ...
    TouchSelectionController(TouchSelectionControllerClient* client, const Config& config);
    ...
    void OnSelectionBoundsChanged(const gfx::SelectionBound& start, const gfx::SelectionBound& end);
    ...
    void HandleLongPressEvent(base::TimeTicks event_time, const gfx::PointF& location);

    // To be called before forwarding a double press event.
    void HandleDoublePressEvent(base::TimeTicks event_time, const gfx::PointF& location);
    ...
    const gfx::SelectionBound& start() const
    {
        return start_;
    }
    const gfx::SelectionBound& end() const
    {
        return end_;
    }

    ActiveStatus active_status() const
    {
...
private:
    ...
    void OnDragBegin(const TouchSelectionDraggable& draggable, const gfx::PointF& drag_position) override;
    void OnDragUpdate(const TouchSelectionDraggable& draggable, const gfx::PointF& drag_position) override;
    ...
    DragSelectorInitiatingGesture drag_selector_initiating_gesture_ = DragSelectorInitiatingGesture::kNone;
