...
LongPressDragSelector::LongPressDragSelector(LongPressDragSelectorClient* client)
    : client_(client)
    , state_(INACTIVE)
...
void LongPressDragSelector::OnLongPressEvent(base::TimeTicks event_time, const gfx::PointF& position)
{
    ...
    if (state_ == INITIATING_GESTURE_PENDING &&
        ...
        (touch_down_time_ < event_time + base::Microseconds(10)) && client_->IsWithinTapSlop(touch_down_position_ - position)) {
        SetState(SELECTION_PENDING);
    }
}

void LongPressDragSelector::OnDoublePressEvent(base::TimeTicks event_time, const gfx::PointF& position)
{
