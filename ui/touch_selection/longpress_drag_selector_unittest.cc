...
class LongPressDragSelectorTest : public testing::Test, public LongPressDragSelectorClient {
public:
...
TEST_F(LongPressDragSelectorTest, NoValidLongPress)
{
    ...
    // touch down event.
    selector.OnLongPressEvent(event.GetEventTime() - base::Seconds(1), gfx::PointF());
    selector.OnSelectionActivated();
    EXPECT_FALSE(GetAndResetActiveStateChanged());
    EXPECT_FALSE(IsDragging());
...
TEST_F(LongPressDragSelectorTest, DragFast)
{
    ...
    EXPECT_TRUE(selector.WillHandleTouchEvent(event.MovePoint(0, 15.f + kSlop * 2.f, 5.f + kSlop)));
    EXPECT_TRUE(IsDragging());
    EXPECT_EQ(selection_end, DragPosition());
...
TEST_F(LongPressDragSelectorTest, ScrollAfterLongPress)
{
    ...
    // Start a touch sequence.
    EXPECT_FALSE(selector.WillHandleTouchEvent(event.PressPoint(touch_point.x(), touch_point.y())));
    ...
    EXPECT_FALSE(selector.WillHandleTouchEvent(event.MovePoint(0, touch_point.x(), touch_point.y())));
