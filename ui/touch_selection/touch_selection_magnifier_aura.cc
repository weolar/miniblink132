...
gfx::ShadowValues GetMagnifierShadowValues(SkColor key_shadow_color, SkColor ambient_shadow_color)
{
    constexpr int kShadowElevation = 3;
    constexpr int kShadowBlur = 2 * kShadowElevation;
    return { gfx::ShadowValue(gfx::Vector2d(0, kShadowElevation), kShadowBlur, key_shadow_color),
...
{
    return gfx::ShadowValue::GetMargin(GetMagnifierShadowValues(gfx::kPlaceholderColor, gfx::kPlaceholderColor)).ToOutsets();
}
...
gfx::Rect GetZoomLayerBounds()
{
    ...
    return gfx::Rect(magnifier_shadow_outsets.left(), magnifier_shadow_outsets.top(), kMagnifierSize.width(), kMagnifierSize.height());
}
...
gfx::Rect GetMagnifierLayerBounds(const gfx::Size& parent_container_size, const gfx::Point& anchor_point)
{
    // Compute bounds for the magnifier zoomed contents, which we try to
    // horizontally center above `anchor_point`.
    const gfx::Point origin(anchor_point.x() - kMagnifierSize.width() / 2, anchor_point.y() - kMagnifierSize.height() + kMagnifierVerticalBoundsOffset);
...
gfx::Point GetMagnifierSourceCenter(const gfx::Size& parent_container_size, const gfx::Point& focus_center)
{
    const gfx::SizeF source_size(kMagnifierSize.width() / kMagnifierScale, kMagnifierSize.height() / kMagnifierScale);
    const gfx::PointF source_origin(focus_center.x() - source_size.width() / 2, focus_center.y() - source_size.height() / 2);
...
public:
    BorderRenderer()
    {
        UpdateTheme(NativeTheme::GetInstanceForNativeUi());
    }
    ...
    void OnPaintLayer(const PaintContext& context) override
    {
        ...
        shadow_flags.setLooper(gfx::CreateShadowDrawLooper(GetMagnifierShadowValues(key_shadow_color_, ambient_shadow_color)));
        recorder.canvas()->DrawRoundRect(zoom_layer_bounds, kMagnifierRadius, shadow_flags);
        ...
        recorder.canvas()->DrawRoundRect(zoom_layer_bounds, kMagnifierRadius, mask_flags);
        ...
        recorder.canvas()->DrawRoundRect(zoom_layer_bounds, kMagnifierRadius, border_flags);
    }
    ...
    {
        auto* color_provider = ColorProviderManager::Get().GetColorProviderFor(theme->GetColorProviderKey(nullptr));
        border_color_ = color_provider->GetColor(kColorSeparator);
        key_shadow_color_ = color_provider->GetColor(kColorShadowValueKeyShadowElevationThree);
        ambient_shadow_color = color_provider->GetColor(kColorShadowValueAmbientShadowElevationThree);
    }
...
void TouchSelectionMagnifierAura::ShowFocusBound(Layer* parent, const gfx::Point& focus_start, const gfx::Point& focus_end)
{
    DCHECK(parent);
    ...
    const gfx::Size magnifier_parent_size = magnifier_layer_->parent()->bounds().size();
    const gfx::Rect focus_rect = gfx::BoundingRect(focus_start, focus_end);
    const gfx::Rect magnifier_layer_bounds = GetMagnifierLayerBounds(magnifier_parent_size, focus_rect.top_center());
    ...
    const gfx::Point magnifier_source_center = GetMagnifierSourceCenter(magnifier_parent_size, focus_rect.CenterPoint());
    const gfx::Point zoom_layer_center = zoom_layer_bounds.CenterPoint() + magnifier_layer_bounds.OffsetFromOrigin();
    const gfx::Point zoom_offset = gfx::PointAtOffsetFromOrigin(zoom_layer_center - magnifier_source_center);
...
void TouchSelectionMagnifierAura::OnNativeThemeUpdated(NativeTheme* observed_theme)
{
    border_renderer_->UpdateTheme(observed_theme);
    border_layer_->SchedulePaint(gfx::Rect(border_layer_->size()));
}

gfx::Rect TouchSelectionMagnifierAura::GetZoomedContentsBoundsForTesting() const
{
    ...
    const gfx::SizeF contents_size(bounds.width() / kMagnifierScale, bounds.height() / kMagnifierScale);
    const gfx::PointF contents_origin(center.x() - contents_size.width() / 2, center.y() - contents_size.height() / 2);
    gfx::Rect contents_bounds = gfx::ToEnclosingRect(gfx::RectF(contents_origin, contents_size));
...
void TouchSelectionMagnifierAura::CreateMagnifierLayer()
{
    ...
    // zoomed contents.
    zoom_layer_->SetBackdropFilterBounds(gfx::RRectF { gfx::RectF(kMagnifierSize), kMagnifierRadius });
