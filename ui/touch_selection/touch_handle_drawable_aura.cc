...
{
    gfx::Rect window_bounds(gfx::ToRoundedPoint(targetable_origin_), handle_image_.Size());
...
void TouchHandleDrawableAura::SetOrientation(TouchHandleOrientation orientation, bool mirror_vertical, bool mirror_horizontal)
{
    ...
    handle_image_ = ::features::IsTouchTextEditingRedesignEnabled() ? GetHandleVectorIcon(orientation) : ImageModel::FromImage(*GetHandleImage(orientation));
    UpdateWindowBounds();
...
gfx::RectF TouchHandleDrawableAura::GetVisibleBounds() const
{
    ...
    if (::features::IsTouchTextEditingRedesignEnabled()) {
        targetable_bounds.Outset(gfx::OutsetsF::TLBR(0, kSelectionHandlePadding, kSelectionHandlePadding, kSelectionHandlePadding));
    }
...
float TouchHandleDrawableAura::GetDrawableHorizontalPaddingRatio() const
{
    ...
    return kSelectionHandlePadding / (window_->bounds().width() + 2.0f * kSelectionHandlePadding);
}
...
void TouchHandleDrawableAura::OnPaintLayer(const PaintContext& context)
{
    ...
    if (!handle_image_.IsEmpty()) {
        recorder.canvas()->DrawImageInt(
            handle_image_.Rasterize(ColorProviderManager::Get().GetColorProviderFor(NativeTheme::GetInstanceForNativeUi()->GetColorProviderKey(nullptr))), 0,
            0);
    }
}

void TouchHandleDrawableAura::OnNativeThemeUpdated(NativeTheme* observed_theme)
{
    if (!::features::IsTouchTextEditingRedesignEnabled()) {
        return;
    }
