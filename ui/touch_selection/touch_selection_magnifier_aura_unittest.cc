...
public:
    ...
    TouchSelectionMagnifierAuraTest(const TouchSelectionMagnifierAuraTest&) = delete;
    TouchSelectionMagnifierAuraTest& operator=(const TouchSelectionMagnifierAuraTest&) = delete;
...
TEST_F(TouchSelectionMagnifierAuraTest, BoundsForVerticalCaret)
{
    ...
    const gfx::Rect zoomed_contents_bounds = magnifier.GetZoomedContentsBoundsForTesting();
    EXPECT_EQ(zoomed_contents_bounds.CenterPoint().x(), kCaretTop.x());
    EXPECT_EQ(zoomed_contents_bounds.CenterPoint().y(), (kCaretTop.y() + kCaretBottom.y()) / 2);
}
...
TEST_F(TouchSelectionMagnifierAuraTest, BoundsUpdate)
{
    ...
    magnifier.ShowFocusBound(&magnifier_parent, kUpdatedCaretTop, kUpdatedCaretBottom);
    ...
    EXPECT_EQ(zoomed_contents_bounds.CenterPoint().y(), (kUpdatedCaretTop.y() + kUpdatedCaretBottom.y()) / 2);
}
...
TEST_F(TouchSelectionMagnifierAuraTest, StaysInsideParentLeftEdge)
{
    ...
    magnifier.ShowFocusBound(&magnifier_parent, gfx::Point(10, 200), gfx::Point(10, 210));
    ...
    EXPECT_TRUE(kParentBounds.Contains(magnifier.GetZoomedContentsBoundsForTesting()));
}
...
TEST_F(TouchSelectionMagnifierAuraTest, StaysInsideParentRightEdge)
{
    ...
    magnifier.ShowFocusBound(&magnifier_parent, gfx::Point(495, 200), gfx::Point(495, 210));
...
TEST_F(TouchSelectionMagnifierAuraTest, StaysInsideParentTopEdge)
{
    ...
    magnifier.ShowFocusBound(&magnifier_parent, gfx::Point(200, 2), gfx::Point(200, 12));
...
TEST_F(TouchSelectionMagnifierAuraTest, Size)
{
    ...
    // Show magnifier.
    magnifier.ShowFocusBound(&magnifier_parent, gfx::Point(300, 200), gfx::Point(300, 210));
    const gfx::Size magnifier_size = magnifier.GetMagnifierBoundsForTesting().size();
    const gfx::Size zoom_content_size = magnifier.GetZoomedContentsBoundsForTesting().size();
    // Move the caret near the edge of the parent container.
    magnifier.ShowFocusBound(&magnifier_parent, gfx::Point(10, 3), gfx::Point(10, 13));

    // Magnifier should remain the same size.
    EXPECT_EQ(magnifier.GetMagnifierBoundsForTesting().size(), magnifier_size);
    EXPECT_EQ(magnifier.GetZoomedContentsBoundsForTesting().size(), zoom_content_size);
}
...
TEST_F(TouchSelectionMagnifierAuraTest, SwitchesParentLayer)
{
    ...
    Layer magnifier_parent;
    magnifier_parent.SetBounds(gfx::Rect(500, 400));
    magnifier.ShowFocusBound(&magnifier_parent, gfx::Point(10, 20), gfx::Point(10, 30));
    // Reparent the magnifier.
    Layer new_parent;
    new_parent.SetBounds(gfx::Rect(600, 400));
    magnifier.ShowFocusBound(&new_parent, gfx::Point(200, 20), gfx::Point(200, 30));
