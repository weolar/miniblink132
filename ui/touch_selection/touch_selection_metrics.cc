...
bool IsSuccessfulSessionEndEvent(const Event& session_end_event)
{
    ...
    return session_end_event.AsKeyEvent()->GetDomKey().IsCharacter() || session_end_event.flags() & EF_IME_FABRICATED_KEY;
}
...
{
    base::UmaHistogramEnumeration(kTouchSelectionDragTypeHistogramName, drag_type);
}

void RecordTouchSelectionMenuCommandAction(int command_id)
{
    base::UmaHistogramEnumeration(kTouchSelectionMenuActionHistogramName, MapCommandIdToMenuAction(command_id));
}

void RecordTouchSelectionMenuEllipsisAction()
{
    base::UmaHistogramEnumeration(kTouchSelectionMenuActionHistogramName, TouchSelectionMenuAction::kEllipsis);
}

void RecordTouchSelectionMenuSmartAction()
{
    base::UmaHistogramEnumeration(kTouchSelectionMenuActionHistogramName, TouchSelectionMenuAction::kSmartAction);
}

TouchSelectionSessionMetricsRecorder::TouchSelectionSessionMetricsRecorder() = default;
...
void TouchSelectionSessionMetricsRecorder::OnMenuCommand(bool should_end_session)
{
    RefreshSessionStatus();
    if (!IsSessionActive()) {
        return;
...
void TouchSelectionSessionMetricsRecorder::OnSessionEndEvent(const Event& session_end_event)
{
    RefreshSessionStatus();
    if (!IsSessionActive()) {
        return;
...
void TouchSelectionSessionMetricsRecorder::RecordSessionMetrics() const
{
    ...
    base::UmaHistogramCustomCounts(
        active_status_ == ActiveStatus::kActiveCursor ? kTouchCursorSessionTouchDownCountHistogramName : kTouchSelectionSessionTouchDownCountHistogramName,
        session_touch_down_count_, kSessionTouchDownCountMin, kSessionTouchDownCountMax, kSessionTouchDownCountBuckets);
}
