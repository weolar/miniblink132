...
bool RectIntersectsCircle(const gfx::RectF& rect, const gfx::PointF& circle_center, float circle_radius)
{
...
static std::ostream& operator<<(std::ostream& os, const TouchHandleOrientation& orientation)
{
    switch (orientation) {
...
TouchHandle::TouchHandle(TouchHandleClient* client, TouchHandleOrientation orientation, const gfx::RectF& viewport_rect)
...
bool TouchHandle::WillHandleTouchEvent(const MotionEvent& event)
{
    ...
    case MotionEvent::Action::DOWN: {
        ...
        const float touch_radius = std::clamp(event.GetTouchMajor(), kMinTouchMajorForHitTesting, kMaxTouchMajorForHitTesting) * 0.5f;
...
bool TouchHandle::Animate(base::TimeTicks frame_time)
{
    ...
    const float position_u = (focus_bottom_ - fade_start_position_).LengthSquared() / kFadeDistanceSquared;
...
void TouchHandle::UpdateHandleLayout()
{
    ...
    if (!is_dragging_) {
        ...
        const float bottom_y_clipped = std::max(bottom_y_unmirrored - viewport_rect_.bottom(), 0.f);
        const float top_y_clipped = std::max(viewport_rect_.y() - top_y_mirrored, 0.f);

        mirror_vertical = top_y_clipped < bottom_y_clipped;

        if (orientation_ == TouchHandleOrientation::LEFT) {
            const float left_x_clipped = std::max(viewport_rect_.x() - (focus_bottom_.x() - handle_width), 0.f);
            if (left_x_clipped > 0)
                mirror_horizontal = true;
        } else if (orientation_ == TouchHandleOrientation::RIGHT) {
            const float right_x_clipped = std::max((focus_bottom_.x() + handle_width) - viewport_rect_.right(), 0.f);
...
gfx::PointF TouchHandle::ComputeHandleOrigin() const
{
    ...
    switch (orientation_) {
    case TouchHandleOrientation::LEFT:
        focal_offset_x = mirror_horizontal_ ? drawable_width * handle_horizontal_padding_ : drawable_width * (1.0f - handle_horizontal_padding_);
        break;
    case TouchHandleOrientation::RIGHT:
        focal_offset_x = mirror_horizontal_ ? drawable_width * (1.0f - handle_horizontal_padding_) : drawable_width * handle_horizontal_padding_;
        break;
    case TouchHandleOrientation::CENTER:
...
void TouchHandle::BeginFade()
{
    ...
    fade_end_time_ = base::TimeTicks::Now() + kFadeDuration * std::abs(target_alpha - alpha_);
