...
public:
    ...
    void OnDragBegin(const TouchSelectionDraggable& handler, const gfx::PointF& drag_position) override
    {
        dragging_ = true;
    }

    void OnDragUpdate(const TouchSelectionDraggable& handler, const gfx::PointF& drag_position) override
    {
        dragged_ = true;
    ...
    void UpdateHandleFocus(TouchHandle& handle, gfx::PointF& top, gfx::PointF& bottom)
    {
        handle.SetFocus(top, bottom);
        handle.UpdateHandleLayout();
    }

    void UpdateHandleOrientation(TouchHandle& handle, TouchHandleOrientation orientation)
    {
        handle.SetOrientation(orientation);
        handle.UpdateHandleLayout();
    }

    void UpdateHandleVisibility(TouchHandle& handle, bool visible, TouchHandle::AnimationStyle animation_style)
    {
    ...
    const gfx::PointF& DragPosition() const
    {
        return drag_position_;
    }
    bool NeedsAnimate() const
    {
        return needs_animate_;
    }

    const MockDrawableData& drawable()
    {
        return drawable_data_;
    }
...
{
    TouchHandle handle(this, TouchHandleOrientation::CENTER, kDefaultViewportRect);
...
TEST_F(TouchHandleTest, Enabled)
{
    ...
    MockMotionEvent event(MockMotionEvent::Action::DOWN, event_time, kOffset, kOffset);
...
TEST_F(TouchHandleTest, Drag)
{
    ...
    event = MockMotionEvent(MockMotionEvent::Action::MOVE, event_time, kOffset, kOffset);
    EXPECT_FALSE(handle.WillHandleTouchEvent(event));
    EXPECT_FALSE(IsDragging());

    // Start the drag.
    event = MockMotionEvent(MockMotionEvent::Action::DOWN, event_time, kOffset, kOffset);
    EXPECT_TRUE(handle.WillHandleTouchEvent(event));
    EXPECT_TRUE(IsDragging());

    event = MockMotionEvent(MockMotionEvent::Action::MOVE, event_time, kOffset + 10, kOffset + 15);
    EXPECT_TRUE(handle.WillHandleTouchEvent(event));
...
TEST_F(TouchHandleTest, DragTargettingUsesTouchSize)
{
    ...
    event = MockMotionEvent(MockMotionEvent::Action::DOWN, event_time, kDefaultDrawableSize / 2.f, kDefaultDrawableSize / 2.f);
    ...
    event = MockMotionEvent(MockMotionEvent::Action::DOWN, event_time, kDefaultDrawableSize / 2.f, -kTouchSize / 3.f);
    event.SetTouchMajor(kTouchSize);
    EXPECT_FALSE(handle.WillHandleTouchEvent(event));
    EXPECT_FALSE(IsDragging());
}
...
TEST_F(TouchHandleTest, Tap)
{
    ...
    event = MockMotionEvent(MockMotionEvent::Action::MOVE, event_time, kDefaultTapSlop / 2.f, 0);
    EXPECT_TRUE(handle.WillHandleTouchEvent(event));
    event = MockMotionEvent(MockMotionEvent::Action::UP, event_time, kDefaultTapSlop / 2.f, 0);
    EXPECT_TRUE(handle.WillHandleTouchEvent(event));
