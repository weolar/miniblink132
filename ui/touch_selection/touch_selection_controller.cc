...
{
    gfx::Vector2dF line_offset = gfx::ScaleVector2d(bound.edge_start() - bound.edge_end(), 0.5f);
...
TouchHandleOrientation ToTouchHandleOrientation(gfx::SelectionBound::Type type)
{
    switch (type) {
...
TouchSelectionController::TouchSelectionController(TouchSelectionControllerClient* client, const Config& config)
    : client_(client)
    , config_(config)
...
void TouchSelectionController::OnSelectionBoundsChanged(const gfx::SelectionBound& start, const gfx::SelectionBound& end)
{
    ...
    if (active_status_ == SELECTION_ACTIVE) {
        ...
        bool need_swap = (start_selection_handle_->IsActive() && end_.edge_end() == start.edge_end())
            || (end_selection_handle_->IsActive() && end.edge_end() == start_.edge_end());
        ...
        need_swap |= (start_selection_handle_->IsActive() && end_.edge_end() == start.edge_start())
            || (end_selection_handle_->IsActive() && end.edge_end() == start_.edge_start());
    ...
    response_pending_input_event_ = INPUT_EVENT_TYPE_NONE;
    base::AutoReset<InputEventType> auto_reset_response_pending_input_event(&response_pending_input_event_, causal_input_event);

    if ((start_orientation_ == TouchHandleOrientation::LEFT || start_orientation_ == TouchHandleOrientation::RIGHT)
        && (end_orientation_ == TouchHandleOrientation::RIGHT || end_orientation_ == TouchHandleOrientation::LEFT)) {
        OnSelectionChanged();
        return;
    }
...
void TouchSelectionController::OnViewportChanged(const gfx::RectF viewport_rect)
{
...
void TouchSelectionController::HandleTapEvent(const gfx::PointF& location, int tap_count)
{
    if (tap_count > 1) {
...
void TouchSelectionController::HandleLongPressEvent(base::TimeTicks event_time, const gfx::PointF& location)
{
...
void TouchSelectionController::HandleDoublePressEvent(base::TimeTicks event_time, const gfx::PointF& location)
{
    longpress_drag_selector_.OnDoublePressEvent(event_time, location);
    response_pending_input_event_ = LONG_PRESS;
    drag_selector_initiating_gesture_ = DragSelectorInitiatingGesture::kDoublePress;
}
...
bool TouchSelectionController::WillHandleTouchEventImpl(const MotionEvent& event)
{
...
void TouchSelectionController::OnDragBegin(const TouchSelectionDraggable& draggable, const gfx::PointF& drag_position)
{
...
void TouchSelectionController::OnDragUpdate(const TouchSelectionDraggable& draggable, const gfx::PointF& drag_position)
{
    ...
    gfx::Vector2dF line_offset = anchor_drag_to_selection_start_ ? GetStartLineOffset() : GetEndLineOffset();
    ...
    if (&draggable == &longpress_drag_selector_) {
        ...
        const gfx::SelectionBound* bound = anchor_drag_to_selection_start_ ? &start_ : &end_;
        const float x = bound->edge_start().x();
        const float y = (bound->edge_start().y() + bound->edge_end().y()) / 2.f;
        client_->OnDragUpdate(TouchSelectionDraggable::Type::kLongpress, gfx::PointF(x, y));
    } else {
        const float y = GetActiveHandleMiddleY();
        client_->OnDragUpdate(TouchSelectionDraggable::Type::kTouchHandle, gfx::PointF(drag_position.x(), y));
    }
}

void TouchSelectionController::OnDragEnd(const TouchSelectionDraggable& draggable)
{
    if (&draggable == insertion_handle_.get()) {
...
bool TouchSelectionController::IsWithinTapSlop(const gfx::Vector2dF& delta) const
{
...
std::unique_ptr<TouchHandleDrawable> TouchSelectionController::CreateDrawable()
{
...
void TouchSelectionController::OnInsertionChanged()
{
    ...
    client_->OnSelectionEvent(activated ? INSERTION_HANDLE_SHOWN : INSERTION_HANDLE_MOVED);
}
...
void TouchSelectionController::OnSelectionChanged()
{
    ...
    client_->OnSelectionEvent(activated ? SELECTION_HANDLES_SHOWN : SELECTION_HANDLES_MOVED);
}
...
bool TouchSelectionController::ActivateInsertionIfNecessary()
{
    ...
    if (!insertion_handle_) {
        insertion_handle_ = std::make_unique<TouchHandle>(this, TouchHandleOrientation::CENTER, viewport_rect_);
    }

    if (active_status_ == INACTIVE || response_pending_input_event_ == TAP || response_pending_input_event_ == LONG_PRESS) {
...
bool TouchSelectionController::ActivateSelectionIfNecessary()
{
    ...
    if (!start_selection_handle_) {
        start_selection_handle_ = std::make_unique<TouchHandle>(this, start_orientation_, viewport_rect_);
    } else {
    ...
    if (!end_selection_handle_) {
        end_selection_handle_ = std::make_unique<TouchHandle>(this, end_orientation_, viewport_rect_);
    } else {
...
TouchHandle::AnimationStyle TouchSelectionController::GetAnimationStyle(bool was_active) const
{
    return was_active && client_->SupportsAnimation() ? TouchHandle::ANIMATION_SMOOTH : TouchHandle::ANIMATION_NONE;
}

void TouchSelectionController::LogDragType(const TouchSelectionDraggable& draggable)
{
    if (&draggable == insertion_handle_.get()) {
        RecordTouchSelectionDrag(TouchSelectionDragType::kCursorHandleDrag);
    } else if (&draggable == start_selection_handle_.get() || &draggable == end_selection_handle_.get()) {
        RecordTouchSelectionDrag(TouchSelectionDragType::kSelectionHandleDrag);
    } else if (drag_selector_initiating_gesture_ == DragSelectorInitiatingGesture::kLongPress) {
        RecordTouchSelectionDrag(TouchSelectionDragType::kLongPressDrag);
    } else if (drag_selector_initiating_gesture_ == DragSelectorInitiatingGesture::kDoublePress) {
