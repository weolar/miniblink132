...
class UI_TOUCH_SELECTION_EXPORT LongPressDragSelectorClient : public TouchSelectionDraggableClient {
public:
...
class UI_TOUCH_SELECTION_EXPORT LongPressDragSelector : public TouchSelectionDraggable {
public:
    ...
    void OnLongPressEvent(base::TimeTicks event_time, const gfx::PointF& position);

    // Called just prior to a double press event being handled.
    void OnDoublePressEvent(base::TimeTicks event_time, const gfx::PointF& position);
...
private:
    enum SelectionState { INACTIVE, INITIATING_GESTURE_PENDING, SELECTION_PENDING, DRAG_PENDING, DRAGGING };
