...
LayeredWindowUpdaterImpl::LayeredWindowUpdaterImpl(HWND hwnd, mojo::PendingReceiver<mojom::LayeredWindowUpdater> receiver)
    : hwnd_(hwnd)
...
void LayeredWindowUpdaterImpl::OnAllocatedSharedMemory(const gfx::Size& pixel_size, base::UnsafeSharedMemoryRegion region, void* lock)
{
    canvas_.reset();
    ...
    bool size_result = ResourceSizes::MaybeSizeInBytes(pixel_size, SinglePlaneFormat::kRGBA_8888, &expected_bytes);
    if (!size_result)
        return;

    // The SkCanvas maps shared memory on creation and unmaps on destruction.
    canvas_
        = skia::CreatePlatformCanvasWithSharedSection(pixel_size.width(), pixel_size.height(), true, region.GetPlatformHandle(), skia::RETURN_NULL_ON_FAILURE);
...
void LayeredWindowUpdaterImpl::Draw(const gfx::Rect& damage_rect, DrawCallback draw_callback)
{
    ...
    UpdateLayeredWindow(hwnd_, nullptr, &position, &size, dib_dc, &zero, RGB(0xFF, 0xFF, 0xFF), &blend, ULW_ALPHA);
