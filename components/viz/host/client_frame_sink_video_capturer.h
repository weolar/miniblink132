...
// directory.
class VIZ_HOST_EXPORT ClientFrameSinkVideoCapturer : private mojom::FrameSinkVideoConsumer {
public:
    // A re-connectable FrameSinkVideoCaptureOverlay. See CreateOverlay().
    class VIZ_HOST_EXPORT Overlay final : public mojom::FrameSinkVideoCaptureOverlay {
    public:
        Overlay(base::WeakPtr<ClientFrameSinkVideoCapturer> client_capturer, int32_t stacking_index);
    ...
    using EstablishConnectionCallback = base::RepeatingCallback<void(mojo::PendingReceiver<mojom::FrameSinkVideoCapturer>)>;
    ...
    void SetResolutionConstraints(const gfx::Size& min_size, const gfx::Size& max_size, bool use_fixed_aspect_ratio);
    void SetAutoThrottlingEnabled(bool enabled);
    void ChangeTarget(const std::optional<VideoCaptureTarget>& target);
    void ChangeTarget(const std::optional<VideoCaptureTarget>& target, uint32_t sub_capture_target_version);
    void Stop();
    ...
    void Start(mojom::FrameSinkVideoConsumer* consumer, mojom::BufferFormatPreference buffer_format_preference);
    ...
    std::optional<media::VideoPixelFormat> GetFormat() const
    {
        return format_;
    }
...
private:
    struct ResolutionConstraints {
        ResolutionConstraints(const gfx::Size& min_size, const gfx::Size& max_size, bool use_fixed_aspect_ratio);
