...
public:
    ...
    public:
...
#endif
        virtual void BlockDomainsFrom3DAPIs(const std::set<GURL>& urls, gpu::DomainGuilt guilt) = 0;
        virtual std::string GetIsolationKey(int32_t client_id, const blink::WebGPUExecutionContextToken& token) = 0;
        ...
        virtual void BindDiscardableMemoryReceiver(mojo::PendingReceiver<discardable_memory::mojom::DiscardableSharedMemoryManager> receiver) = 0;
        virtual void BindInterface(const std::string& interface_name, mojo::ScopedMessagePipeHandle interface_pipe) = 0;
    ...
    using EstablishChannelCallback = base::OnceCallback<void(
        mojo::ScopedMessagePipeHandle, const gpu::GPUInfo&, const gpu::GpuFeatureInfo&, const gpu::SharedImageCapabilities&, EstablishChannelStatus)>;

    GpuHostImpl(Delegate* delegate, mojo::PendingRemote<mojom::VizMain> viz_main, InitParams params);
    ...
    void ConnectFrameSinkManager(mojo::PendingReceiver<mojom::FrameSinkManager> receiver, mojo::PendingRemote<mojom::FrameSinkManagerClient> client,
    ...
    void EstablishGpuChannel(int client_id, uint64_t client_tracing_id, bool is_gpu_host, bool sync, EstablishChannelCallback callback);
    ...
    // output stream.
    void StartVisualDebugStream(base::RepeatingCallback<void(base::Value)> callback);
    ...
    void BindInterface(const std::string& interface_name, mojo::ScopedMessagePipeHandle interface_pipe);

    mojom::GpuService* gpu_service();

#if BUILDFLAG(IS_WIN)
    mojom::InfoCollectionGpuService* info_collection_gpu_service();
    void AddChildWindow(gpu::SurfaceHandle parent_window, gpu::SurfaceHandle child_window);
#endif
...
private:
    ...
    void LoadedBlob(const gpu::GpuDiskCacheHandle& handle, const std::string& key, const std::string& data);
    void OnDiskCacheHandleDestoyed(const gpu::GpuDiskCacheHandle& handle);

    void OnChannelEstablished(int client_id, bool sync, mojo::ScopedMessagePipeHandle channel_handle, const gpu::GPUInfo& gpu_info,
    ...
    // mojom::GpuHost:
    void DidInitialize(const gpu::GPUInfo& gpu_info, const gpu::GpuFeatureInfo& gpu_feature_info, const std::optional<gpu::GPUInfo>& gpu_info_for_hardware_gpu,
        const std::optional<gpu::GpuFeatureInfo>& gpu_feature_info_for_hardware_gpu, const gfx::GpuExtraInfo& gpu_extra_info) override;
...
#endif
    void GetIsolationKey(int32_t client_id, const blink::WebGPUExecutionContextToken& token, GetIsolationKeyCallback cb) override;
    void StoreBlobToDisk(const gpu::GpuDiskCacheHandle& handle, const std::string& key, const std::string& blob) override;
...
#if BUILDFLAG(IS_WIN)
    mojo::Remote<mojom::InfoCollectionGpuService> info_collection_gpu_service_remote_;
#endif
