...
HostGpuMemoryBufferManager::PendingBufferInfo::PendingBufferInfo(PendingBufferInfo&&) = default;
HostGpuMemoryBufferManager::PendingBufferInfo::~PendingBufferInfo() = default;

HostGpuMemoryBufferManager::HostGpuMemoryBufferManager(GpuServiceProvider gpu_service_provider, int gpu_service_client_id,
    std::unique_ptr<gpu::GpuMemoryBufferSupport> gpu_memory_buffer_support, scoped_refptr<base::SingleThreadTaskRunner> task_runner)
    ...
    , task_runner_(std::move(task_runner))
{
    ...
    base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(this, "HostGpuMemoryBufferManager", task_runner_);
}
...
void HostGpuMemoryBufferManager::DestroyGpuMemoryBuffer(gfx::GpuMemoryBufferId id)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...
void HostGpuMemoryBufferManager::AllocateGpuMemoryBuffer(gfx::GpuMemoryBufferId id, const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage,
    gpu::SurfaceHandle surface_handle, base::OnceCallback<void(gfx::GpuMemoryBufferHandle)> callback, bool call_sync)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    ...
    if (call_sync) {
        gfx::GpuMemoryBufferHandle handle;
        {
            ...
            gpu_service->CreateGpuMemoryBuffer(id, size, format, usage, gpu_service_client_id_, surface_handle, &handle);
        }
        OnGpuMemoryBufferAllocated(gpu_service_version_, id, std::move(handle));
    } else {
        // Make an async request for the GMB to be created.
        gpu_service->CreateGpuMemoryBuffer(id, size, format, usage, gpu_service_client_id_, surface_handle,
            base::BindOnce(&HostGpuMemoryBufferManager::OnGpuMemoryBufferAllocated, weak_ptr_, gpu_service_version_, id));
    }
}

std::unique_ptr<gfx::GpuMemoryBuffer> HostGpuMemoryBufferManager::CreateGpuMemoryBuffer(
    const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage, gpu::SurfaceHandle surface_handle, base::WaitableEvent* cancel_event)
{
    ...
    auto reply_callback = base::BindOnce(
        [](scoped_refptr<base::RefCountedData<bool>> cancelled, gfx::GpuMemoryBufferHandle* handle, base::WaitableEvent* wait_event,
    ...
    auto allocate_callback = base::BindOnce(
        &HostGpuMemoryBufferManager::AllocateGpuMemoryBuffer, weak_ptr_, id, size, format, usage, surface_handle, std::move(reply_callback), call_sync);
    if (call_sync) {
        std::move(allocate_callback).Run();
    } else {
        task_runner_->PostTask(FROM_HERE, std::move(allocate_callback));
        base::ScopedAllowBaseSyncPrimitivesOutsideBlockingScope allow_base_sync_primitives;
        ...
        base::WaitableEvent* waitables[3] = { &completion_event, &shutdown_event_, cancel_event };
        size_t index = base::WaitableEvent::WaitMany(waitables, cancel_event ? 3 : 2);
        if (index > 0) {
            cancelled->data = true;
        }
    }
    ...
    return gpu_memory_buffer_support_->CreateGpuMemoryBufferImplFromHandle(std::move(handle), size, format, usage,
        base::BindPostTask(task_runner_, base::BindOnce(&HostGpuMemoryBufferManager::DestroyGpuMemoryBuffer, weak_ptr_, id)), this, pool_);
}
    ...
    gfx::GpuMemoryBufferHandle buffer_handle, base::UnsafeSharedMemoryRegion memory_region, base::OnceCallback<void(bool)> callback)
{
    if (!task_runner_->BelongsToCurrentThread()) {
        task_runner_->PostTask(FROM_HERE,
            base::BindOnce(
                &HostGpuMemoryBufferManager::CopyGpuMemoryBufferAsync, weak_ptr_, std::move(buffer_handle), std::move(memory_region), std::move(callback)));
        return;
    }

    if (auto* gpu_service = GetGpuService()) {
        gpu_service->CopyGpuMemoryBuffer(std::move(buffer_handle), std::move(memory_region),
...
bool HostGpuMemoryBufferManager::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    ...
    for (const auto& buffer_pair : allocated_buffers_) {
        ...
        if (!buffer_info.OnMemoryDump(pmd, gpu_service_client_id_, client_tracing_process_id)) {
            return false;
        }
    }
    return true;
...
mojom::GpuService* HostGpuMemoryBufferManager::GetGpuService()
{
    ...
    gpu_service_ = gpu_service_provider_.Run(base::BindOnce(&HostGpuMemoryBufferManager::OnConnectionError, weak_ptr_));
    return gpu_service_;
}
...
void HostGpuMemoryBufferManager::OnConnectionError()
{
    ...
    for (auto& buffer_pair : pending_buffers) {
        ...
        LOG(WARNING) << "Retrying allocation of GpuMemoryBuffer with id = " << buffer_pair.first.id << ", client_id = " << gpu_service_client_id_
                     << ", size = " << buffer.size.ToString() << ", format = " << gfx::BufferFormatToString(buffer.format)
                     << ", usage = " << gfx::BufferUsageToString(buffer.usage) << ", surface_handle = " << buffer.surface_handle << " due to connection error";
        AllocateGpuMemoryBuffer(buffer_pair.first, buffer.size, buffer.format, buffer.usage, buffer.surface_handle, std::move(buffer.callback));
    }
}

void HostGpuMemoryBufferManager::OnGpuMemoryBufferAllocated(int gpu_service_version, gfx::GpuMemoryBufferId id, gfx::GpuMemoryBufferHandle handle)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    ...
    if (!handle.is_null()) {
        ...
        gpu::AllocatedBufferInfo buffer_info(handle, pending_buffer.size, pending_buffer.format);
