...
class VIZ_HOST_EXPORT HostFrameSinkManager : public mojom::FrameSinkManagerClient, public HitTestDataProvider {
public:
    ...
    void SetServiceRunner(scoped_refptr<base::SingleThreadTaskRunner> service_runner)
    {
        service_runner_ = service_runner;
    }
    ...
    void BindAndSetManager(mojo::PendingReceiver<mojom::FrameSinkManagerClient> receiver, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    ...
    void RegisterFrameSinkId(const FrameSinkId& frame_sink_id, HostFrameSinkClient* client, ReportFirstSurfaceActivation report_activation);
    ...
    // closed.
    void InvalidateFrameSinkId(const FrameSinkId& frame_sink_id, HostFrameSinkClient* client);

    // |debug_label| is used when printing out the surface hierarchy so we know
    // which clients are contributing which surfaces.
    void SetFrameSinkDebugLabel(const FrameSinkId& frame_sink_id, const std::string& debug_label);
    ...
    // window.
    void CreateRootCompositorFrameSink(mojom::RootCompositorFrameSinkParamsPtr params, bool maybe_wait_on_destruction = true);
    ...
    // If there is already a CompositorFrameSink for |frame_sink_id| then calling
    // this will destroy the existing CompositorFrameSink and create a new one.
    void CreateCompositorFrameSink(const FrameSinkId& frame_sink_id, mojo::PendingReceiver<mojom::CompositorFrameSink> receiver,
        mojo::PendingRemote<mojom::CompositorFrameSinkClient> client, input::mojom::RenderInputRouterConfigPtr render_input_router_config = nullptr);
    ...
    // to `receiver`.
    void CreateBundledCompositorFrameSink(const FrameSinkId& frame_sink_id, const FrameSinkBundleId& bundle_id,
    ...
    bool RegisterFrameSinkHierarchy(const FrameSinkId& parent_frame_sink_id, const FrameSinkId& child_frame_sink_id);

    // Unregisters FrameSink hierarchy. Client must have registered frame sink
    // hierarchy before unregistering.
    void UnregisterFrameSinkHierarchy(const FrameSinkId& parent_frame_sink_id, const FrameSinkId& child_frame_sink_id);

    // Asks viz to send updates regarding video activity to |observer|.
    void AddVideoDetectorObserver(mojo::PendingRemote<mojom::VideoDetectorObserver> observer);

    // Creates a FrameSinkVideoCapturer instance in viz.
    void CreateVideoCapturer(mojo::PendingReceiver<mojom::FrameSinkVideoCapturer> receiver);
    ...
    void RequestCopyOfOutput(const SurfaceId& surface_id, std::unique_ptr<CopyOutputRequest> request, bool capture_exact_surface_id = false);
    ...
    using ScreenshotDestinationReadyCallback = base::OnceCallback<void(const SkBitmap& copy_output)>;
    ...
    void SetOnCopyOutputReadyCallback(const blink::SameDocNavigationScreenshotDestinationToken& destination_token, ScreenshotDestinationReadyCallback callback);
    ...
    void InvalidateCopyOutputReadyCallback(const blink::SameDocNavigationScreenshotDestinationToken& destination_token);
    ...
    void SetHitTestAsyncQueriedDebugRegions(const FrameSinkId& root_frame_sink_id, const std::vector<FrameSinkId>& hit_test_async_queried_debug_queue);
    ...
    void CreateHitTestQueryForSynchronousCompositor(const FrameSinkId& frame_sink_id);
    void EraseHitTestQueryForSynchronousCompositor(const FrameSinkId& frame_sink_id);
    ...
    void ClearUnclaimedViewTransitionResources(const blink::ViewTransitionToken& transition_token);
...
private:
    ...
    struct FrameSinkData {
        ...
        bool IsFrameSinkRegistered() const
        {
        ...
        bool IsEmpty() const
        {
            return !IsFrameSinkRegistered() && !has_created_compositor_frame_sink && children.empty();
        }
        ...
        ReportFirstSurfaceActivation report_activation = ReportFirstSurfaceActivation::kYes;
    ...
    void CreateFrameSink(const FrameSinkId& frame_sink_id, std::optional<FrameSinkBundleId> bundle_id,
        mojo::PendingReceiver<mojom::CompositorFrameSink> receiver, mojo::PendingRemote<mojom::CompositorFrameSinkClient> client,
    ...
    void OnFrameTokenChanged(const FrameSinkId& frame_sink_id, uint32_t frame_token, base::TimeTicks activation_time) override;
    ...
    mojo::Receiver<mojom::FrameSinkManagerClient> frame_sink_manager_client_receiver_ { this };
    ...
    std::unordered_map<FrameSinkId, FrameSinkData, FrameSinkIdHash> frame_sink_data_map_;
    ...
    scoped_refptr<base::SingleThreadTaskRunner> service_runner_;
    //=====
    ...
    base::flat_map<blink::SameDocNavigationScreenshotDestinationToken, ScreenshotDestinationReadyCallback> screenshot_destinations_;
