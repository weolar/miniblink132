...
public:
    ...
    int GetAllocationRequestsCount() const
    {
        return allocation_requests_.size();
    }

    bool IsAllocationRequestAt(size_t index, gfx::GpuMemoryBufferId id, int client_id) const
    {
    ...
    bool IsDestructionRequestAt(size_t index, gfx::GpuMemoryBufferId id, int client_id) const
    {
    ...
    void SatisfyAllocationRequestAt(size_t index, bool emulate_native_handle = false)
    {
        DCHECK_LT(index, allocation_requests_.size());
        ...
        handle.type = emulate_native_handle ? gfx::NATIVE_PIXMAP : gfx::SHARED_MEMORY_BUFFER;
    ...
    void CreateVideoEncodeAcceleratorProvider(mojo::PendingReceiver<media::mojom::VideoEncodeAcceleratorProvider> receiver) override
    {
    }
    ...
    void BindClientGmbInterface(mojo::PendingReceiver<gpu::mojom::ClientGmbInterface> receiver, int client_id) override
    {
    }
...
public:
    ...
    HostGpuMemoryBufferManagerTest(const HostGpuMemoryBufferManagerTest&) = delete;
    HostGpuMemoryBufferManagerTest& operator=(const HostGpuMemoryBufferManagerTest&) = delete;
    ...
    void SetUp() override
    {
        ...
        auto gpu_service_provider = base::BindRepeating(&TestGpuService::GetGpuService, base::Unretained(gpu_service_.get()));
        auto gpu_memory_buffer_support = std::make_unique<gpu::GpuMemoryBufferSupport>();
        gpu_memory_buffer_manager_ = std::make_unique<HostGpuMemoryBufferManager>(std::move(gpu_service_provider), kHostGpuMemoryBufferManagerId,
            std::move(gpu_memory_buffer_support), base::SingleThreadTaskRunner::GetCurrentDefault());
    }
...
#if BUILDFLAG(IS_OZONE)
        native_pixmap_supported = ui::OzonePlatform::GetInstance()->IsNativePixmapConfigSupported(gfx::BufferFormat::RGBA_8888, gfx::BufferUsage::GPU_READ);
    ...
    std::unique_ptr<gfx::GpuMemoryBuffer> AllocateShMemGpuMemoryBufferSync()
    {
        ...
        diff_thread.task_runner()->PostTask(FROM_HERE,
            base::BindOnce(
                [](HostGpuMemoryBufferManager* manager, std::unique_ptr<gfx::GpuMemoryBuffer>* out_buffer, base::OnceClosure callback) {
                    *out_buffer = manager->CreateGpuMemoryBuffer(
                        gfx::Size(64, 64), gfx::BufferFormat::YVU_420, gfx::BufferUsage::GPU_READ, gpu::kNullSurfaceHandle, nullptr);
    ...
    TestGpuService* gpu_service() const
    {
        return gpu_service_.get();
    }
...
TEST_F(HostGpuMemoryBufferManagerTest, GpuMemoryBufferDestroyedOnDifferentThread)
{
    auto buffer = AllocateShMemGpuMemoryBufferSync();
    EXPECT_TRUE(buffer);
...
TEST_F(HostGpuMemoryBufferManagerTest, AllocationRequestFromDeadGpuService)
{
    ...
    gpu_memory_buffer_manager()->AllocateGpuMemoryBuffer(buffer_id, size, format, usage, gpu::kNullSurfaceHandle,
        base::BindOnce(
            [](gfx::GpuMemoryBufferHandle* allocated_handle, gfx::GpuMemoryBufferHandle handle) { *allocated_handle = std::move(handle); }, &allocated_handle));
    EXPECT_EQ(1, gpu_service()->GetAllocationRequestsCount());
    EXPECT_TRUE(gpu_service()->IsAllocationRequestAt(0, buffer_id, kHostGpuMemoryBufferManagerId));
    ...
    EXPECT_TRUE(gpu_service()->IsAllocationRequestAt(1, buffer_id, kHostGpuMemoryBufferManagerId));
    EXPECT_TRUE(allocated_handle.is_null());
...
TEST_F(HostGpuMemoryBufferManagerTest, CancelRequestsForShutdown)
{
    ...
    for (auto& thread : threads) {
        ...
        // is destroyed.
        thread.task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting([this, &create_wait]() {
            create_wait.Signal();
            // This should block.
            gpu_memory_buffer_manager_->CreateGpuMemoryBuffer(
                gfx::Size(100, 100), gfx::BufferFormat::RGBA_8888, gfx::BufferUsage::SCANOUT, gpu::kNullSurfaceHandle, nullptr);
        }));
        create_wait.Wait();
    }
    ...
    base::RunLoop loop;
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, loop.QuitClosure());
    loop.Run();
}
