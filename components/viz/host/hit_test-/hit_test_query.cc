...
Target HitTestQuery::FindTargetForLocation(EventSource event_source, const gfx::PointF& location_in_root) const
{
    if (hit_test_data_.empty())
        return Target();
    return FindTargetForLocationStartingFromImpl(event_source, location_in_root, hit_test_data_[0].frame_sink_id,
    ...
    const std::vector<FrameSinkId>& target_ancestors, const gfx::PointF& location_in_root, gfx::PointF* transformed_location) const
{
    ...
    if (target_ancestors.size() == 0u || target_ancestors[target_ancestors.size() - 1] != hit_test_data_[0].frame_sink_id) {
        return false;
    }
    ...
    bool is_location_relative_to_parent, const FrameSinkId& root_view_frame_sink_id, Target* target) const
{
    ...
    if (is_location_relative_to_parent) {
        ...
        location_transformed = hit_test_data_[region_index].transform.MapPoint(location_transformed);
    ...
    if (!CheckChildCount(region_child_count, hit_test_data_.size() - region_index))
        return false;
    ...
    bool root_view_overlapped = hit_test_data_[region_index].frame_sink_id == root_view_frame_sink_id
        && hit_test_data_[region_index].async_hit_test_reasons == AsyncHitTestReasons::kOverlappedRegion;

    // Verify that async_hit_test_reasons is set if and only if there's
    // a kHitTestAsk flag.
    DCHECK_EQ(!!(flags & HitTestRegionFlags::kHitTestAsk), !!hit_test_data_[region_index].async_hit_test_reasons);
    ...
    while (child_region < child_region_end) {
        ...
        const int32_t child_region_child_count = hit_test_data_[child_region].child_count;
    ...
    if ((flags & HitTestRegionFlags::kHitTestMine) && !(flags & HitTestRegionFlags::kHitTestIgnore)) {
        ...
        if (root_view_overlapped) {
            DCHECK_EQ(hit_test_data_[region_index].async_hit_test_reasons, AsyncHitTestReasons::kOverlappedRegion);
...
{
    *location_in_target = hit_test_data_[region_index].transform.MapPoint(*location_in_target);
    if (!target_ancestor)
        return true;
    ...
    size_t child_region_end = child_region + region_child_count;
    while (child_region < child_region_end) {
        if (hit_test_data_[child_region].frame_sink_id == target_ancestors[target_ancestor - 1]) {
...
std::string HitTestQuery::PrintHitTestData() const
{
    ...
    for (uint32_t i = 0; i < hit_test_data_.size(); ++i) {
        ...
        while (!parents.empty() && parents.top() + hit_test_data_[parents.top()].child_count <= i) {
            tabs.pop_back();
            tabs.pop_back();
