...
class VIZ_HOST_EXPORT HostGpuMemoryBufferManager : public gpu::GpuMemoryBufferManager, public base::trace_event::MemoryDumpProvider {
public:
    ...
    using GpuServiceProvider = base::RepeatingCallback<mojom::GpuService*(base::OnceClosure connection_error_handler)>;
    ...
    // thread).
    HostGpuMemoryBufferManager(GpuServiceProvider gpu_service_provider, int gpu_service_client_id,
    ...
    HostGpuMemoryBufferManager& operator=(const HostGpuMemoryBufferManager&) = delete;
    ...
    std::unique_ptr<gfx::GpuMemoryBuffer> CreateGpuMemoryBuffer(const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage,
        gpu::SurfaceHandle surface_handle, base::WaitableEvent* shutdown_event) override;
...
private:
    ...
    FRIEND_TEST_ALL_PREFIXES(HostGpuMemoryBufferManagerTest, AllocationRequestsForDestroyedClient);
    FRIEND_TEST_ALL_PREFIXES(HostGpuMemoryBufferManagerTest, AllocationRequestFromDeadGpuService);

    void AllocateGpuMemoryBuffer(gfx::GpuMemoryBufferId id, const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage,
        gpu::SurfaceHandle surface_handle, base::OnceCallback<void(gfx::GpuMemoryBufferHandle)> callback, bool call_sync = false);
    ...
    using PendingBuffers = std::unordered_map<gfx::GpuMemoryBufferId, PendingBufferInfo, std::hash<gfx::GpuMemoryBufferId>>;

    using AllocatedBuffers = std::unordered_map<gfx::GpuMemoryBufferId, gpu::AllocatedBufferInfo, std::hash<gfx::GpuMemoryBufferId>>;
    ...
    void OnGpuMemoryBufferAllocated(int gpu_service_version, gfx::GpuMemoryBufferId id, gfx::GpuMemoryBufferHandle handle);
