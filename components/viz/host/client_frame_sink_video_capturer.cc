...
ClientFrameSinkVideoCapturer::ClientFrameSinkVideoCapturer(EstablishConnectionCallback callback)
...
void ClientFrameSinkVideoCapturer::SetMinCapturePeriod(base::TimeDelta min_capture_period)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void ClientFrameSinkVideoCapturer::SetMinSizeChangePeriod(base::TimeDelta min_period)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void ClientFrameSinkVideoCapturer::SetResolutionConstraints(const gfx::Size& min_size, const gfx::Size& max_size, bool use_fixed_aspect_ratio)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    resolution_constraints_.emplace(min_size, max_size, use_fixed_aspect_ratio);
    capturer_remote_->SetResolutionConstraints(min_size, max_size, use_fixed_aspect_ratio);
}
...
void ClientFrameSinkVideoCapturer::ChangeTarget(const std::optional<VideoCaptureTarget>& target)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void ClientFrameSinkVideoCapturer::ChangeTarget(const std::optional<VideoCaptureTarget>& target, uint32_t sub_capture_target_version)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void ClientFrameSinkVideoCapturer::Start(mojom::FrameSinkVideoConsumer* consumer, mojom::BufferFormatPreference buffer_format_preference)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(consumer);
...
std::unique_ptr<ClientFrameSinkVideoCapturer::Overlay> ClientFrameSinkVideoCapturer::CreateOverlay(int32_t stacking_index)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    // If there is an existing overlay at the same index, drop it.
    auto it = base::ranges::find(overlays_, stacking_index, &Overlay::stacking_index);
    if (it != overlays_.end()) {
        (*it)->DisconnectPermanently();
        overlays_.erase(it);
    }

    auto overlay = std::make_unique<Overlay>(weak_factory_.GetWeakPtr(), stacking_index);
...
ClientFrameSinkVideoCapturer::ResolutionConstraints::ResolutionConstraints(const gfx::Size& min_size, const gfx::Size& max_size, bool use_fixed_aspect_ratio)
    : min_size(min_size)
    , max_size(max_size)
...
void ClientFrameSinkVideoCapturer::OnFrameCaptured(media::mojom::VideoBufferHandlePtr data, media::mojom::VideoFrameInfoPtr info, const gfx::Rect& content_rect,
    mojo::PendingRemote<mojom::FrameSinkVideoConsumerFrameCallbacks> callbacks)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    consumer_->OnFrameCaptured(std::move(data), std::move(info), content_rect, std::move(callbacks));
}
...
void ClientFrameSinkVideoCapturer::OnNewSubCaptureTargetVersion(uint32_t sub_capture_target_version)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void ClientFrameSinkVideoCapturer::EstablishConnection()
{
    ...
    establish_connection_callback_.Run(capturer_remote_.BindNewPipeAndPassReceiver());
    capturer_remote_.set_disconnect_handler(base::BindOnce(&ClientFrameSinkVideoCapturer::OnConnectionError, base::Unretained(this)));
    if (format_)
    ...
    if (resolution_constraints_) {
        capturer_remote_->SetResolutionConstraints(
            resolution_constraints_->min_size, resolution_constraints_->max_size, resolution_constraints_->use_fixed_aspect_ratio);
    }
    if (auto_throttling_enabled_)
        capturer_remote_->SetAutoThrottlingEnabled(*auto_throttling_enabled_);
    if (target_) {
        capturer_remote_->ChangeTarget(target_.value(), sub_capture_target_version_);
    }
...
void ClientFrameSinkVideoCapturer::OnConnectionError()
{
    ...
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE, base::BindOnce(&ClientFrameSinkVideoCapturer::EstablishConnection, weak_factory_.GetWeakPtr()), kReEstablishConnectionDelay);
}
...
void ClientFrameSinkVideoCapturer::StartInternal()
{
    ...
    if (consumer_receiver_.is_bound())
        consumer_receiver_.reset();
    capturer_remote_->Start(consumer_receiver_.BindNewPipeAndPassRemote(), buffer_format_preference_);
}
...
ClientFrameSinkVideoCapturer::Overlay::Overlay(base::WeakPtr<ClientFrameSinkVideoCapturer> client_capturer, int32_t stacking_index)
    : client_capturer_(client_capturer)
    , stacking_index_(stacking_index)
{
}
...
void ClientFrameSinkVideoCapturer::Overlay::SetImageAndBounds(const SkBitmap& image, const gfx::RectF& bounds)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(!image.isNull());
...
void ClientFrameSinkVideoCapturer::Overlay::SetBounds(const gfx::RectF& bounds)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void ClientFrameSinkVideoCapturer::Overlay::OnCapturedMouseEvent(const gfx::Point& coordinates)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void ClientFrameSinkVideoCapturer::Overlay::EstablishConnection(mojom::FrameSinkVideoCapturer* capturer)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(client_capturer_);

    overlay_.reset();
    capturer->CreateOverlay(stacking_index_, overlay_.BindNewPipeAndPassReceiver());
