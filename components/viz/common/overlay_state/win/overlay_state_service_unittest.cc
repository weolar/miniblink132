...
class OverlayStateServiceUnittest : public testing::Test, public gpu::mojom::OverlayStateObserver {
public:
...
protected:
    ...
    base::OnceClosure SetPromotionHintClosure(const gpu::Mailbox& mailbox, bool promoted);
...
void OverlayStateServiceUnittest::PerformRegistration(const gpu::Mailbox& mailbox)
{
...
void OverlayStateServiceUnittest::SetPromotionHint(const gpu::Mailbox& mailbox, bool promoted)
{
    base::RunLoop run_loop;
    other_thread_task_runner_->PostTask(FROM_HERE, SetPromotionHintClosure(std::move(mailbox), promoted));
    other_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(run_loop.QuitClosure()));
    run_loop.Run();
}

base::OnceClosure OverlayStateServiceUnittest::SetPromotionHintClosure(const gpu::Mailbox& mailbox, bool promoted)
{
    return base::BindOnce(&OverlayStateServiceUnittest::SetPromotionHintWorker, base::Unretained(this), std::move(mailbox), promoted);
}

void OverlayStateServiceUnittest::SetPromotionHintWorker(const gpu::Mailbox& mailbox, bool promoted)
{
...
{
    base::RunLoop run_loop;
    other_thread_task_runner_->PostTask(FROM_HERE, DestroyMailboxClosure(std::move(mailbox)));
    other_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(run_loop.QuitClosure()));
    run_loop.Run();
}

base::OnceClosure OverlayStateServiceUnittest::DestroyMailboxClosure(const gpu::Mailbox& mailbox)
{
    return base::BindOnce(&OverlayStateServiceUnittest::DestroyMailboxWorker, base::Unretained(this), std::move(mailbox));
}

void OverlayStateServiceUnittest::DestroyMailboxWorker(const gpu::Mailbox& mailbox)
{
