...
public:
    ...
    // it is copied as-is.
    using FilterCallback = base::RepeatingCallback<bool(const DrawQuad&, CompositorRenderPass&)>;
    static std::unique_ptr<CompositorRenderPass> CopyPassWithQuadFiltering(const CompositorRenderPass& source_pass, FilterCallback filter_callback);

    static CompositorRenderPassId NextRenderPassId(const CompositorRenderPassId& id)
    {
        return CompositorRenderPassId(id.GetUnsafeValue() + 1);
    }

    static std::string RenderPassListToString(const CompositorRenderPassList& render_passes);
};
