...
public:
    ...
    constexpr SurfaceId(const FrameSinkId& frame_sink_id, const LocalSurfaceId& local_surface_id)
    ...
    size_t hash() const
    {
        return base::HashInts(static_cast<uint64_t>(frame_sink_id_.hash()), static_cast<uint64_t>(local_surface_id_.hash()));
    }
    ...
    friend std::strong_ordering operator<=>(const SurfaceId&, const SurfaceId&) = default;
...
VIZ_COMMON_EXPORT std::ostream& operator<<(std::ostream& out, const SurfaceId& surface_id);
