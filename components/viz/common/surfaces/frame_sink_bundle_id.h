...
public:
    ...
    constexpr FrameSinkBundleId& operator=(const FrameSinkBundleId& other) = default;
    ...
    {
        return client_id_ != 0 || bundle_id_ != 0;
    }
    constexpr uint32_t client_id() const
    {
        return client_id_;
    }
    constexpr uint32_t bundle_id() const
    {
        return bundle_id_;
    }

    friend std::strong_ordering operator<=>(const FrameSinkBundleId&, const FrameSinkBundleId&) = default;

    size_t hash() const
    {
        return base::HashInts(client_id_, bundle_id_);
    }
...
struct FrameSinkBundleIdHash {
    size_t operator()(const FrameSinkBundleId& key) const
    {
        return key.hash();
    }
};
