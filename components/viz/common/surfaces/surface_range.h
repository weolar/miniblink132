...
public:
    ...
    friend std::strong_ordering operator<=>(const SurfaceRange&, const SurfaceRange&) = default;
    ...
    const std::optional<SurfaceId>& start() const
    {
        return start_;
    }

    const SurfaceId& end() const
    {
        return end_;
    }
...
VIZ_COMMON_EXPORT std::ostream& operator<<(std::ostream& out, const SurfaceRange& surface_range);
