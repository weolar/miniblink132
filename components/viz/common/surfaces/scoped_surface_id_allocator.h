...
public:
    ...
    explicit ScopedSurfaceIdAllocator(ParentLocalSurfaceIdAllocator* allocator, base::OnceCallback<void()> allocation_task);

    ScopedSurfaceIdAllocator(const ScopedSurfaceIdAllocator& other) = delete;
    ScopedSurfaceIdAllocator& operator=(const ScopedSurfaceIdAllocator& other) = delete;
    ...
    friend void swap(ScopedSurfaceIdAllocator& first, ScopedSurfaceIdAllocator& second);
