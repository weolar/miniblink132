...
constexpr uint32_t kMaxParentSequenceNumber = std::numeric_limits<uint32_t>::max();
constexpr uint32_t kMaxChildSequenceNumber = std::numeric_limits<uint32_t>::max();
...
public:
    ...
    constexpr LocalSurfaceId(uint32_t parent_sequence_number, const base::UnguessableToken& embed_token)
    ...
    constexpr LocalSurfaceId(uint32_t parent_sequence_number, uint32_t child_sequence_number, const base::UnguessableToken& embed_token)
        : parent_sequence_number_(parent_sequence_number)
    ...
    {
        return LocalSurfaceId(kMaxParentSequenceNumber, kMaxChildSequenceNumber, base::UnguessableToken());
    }

    constexpr bool is_valid() const
    {
        return parent_sequence_number_ != kInvalidParentSequenceNumber && child_sequence_number_ != kInvalidChildSequenceNumber && !embed_token_.is_empty();
    }
    ...
    uint64_t embed_trace_id() const
    {
        return persistent_hash() << 1;
    }
    ...
    uint64_t submission_trace_id() const
    {
        return (persistent_hash() << 1) | 1;
    }

    friend std::strong_ordering operator<=>(const LocalSurfaceId&, const LocalSurfaceId&) = default;
...
private:
    friend struct mojo::StructTraits<mojom::LocalSurfaceIdDataView, LocalSurfaceId>;
...
VIZ_COMMON_EXPORT std::ostream& operator<<(std::ostream& out, const LocalSurfaceId& local_surface_id);
