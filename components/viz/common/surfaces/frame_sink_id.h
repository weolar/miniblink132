...
public:
    ...
    {
        return client_id_ != 0 || sink_id_ != 0;
    }
    ...
    constexpr uint32_t sink_id() const
    {
        return sink_id_;
    }

    friend std::strong_ordering operator<=>(const FrameSinkId&, const FrameSinkId&) = default;

    size_t hash() const
    {
        return base::HashInts(client_id_, sink_id_);
    }
...
VIZ_COMMON_EXPORT std::ostream& operator<<(std::ostream& out, const FrameSinkId& frame_sink_id);

struct FrameSinkIdHash {
    size_t operator()(const FrameSinkId& key) const
    {
        return key.hash();
    }
};
