...
public:
    SurfaceInfo() = default;
    SurfaceInfo(const SurfaceId& id, float device_scale_factor, const gfx::Size& size_in_pixels)
        : id_(id)
    ...
    bool is_valid() const
    {
        return id_.is_valid() && device_scale_factor_ != 0 && !size_in_pixels_.IsEmpty();
    }
...
VIZ_COMMON_EXPORT std::ostream& operator<<(std::ostream& out, const SurfaceInfo& surface_info);
