...
{
    return base::StringPrintf("SurfaceId(%s, %s)", frame_sink_id_.ToString().c_str(), local_surface_id_.ToString().c_str());
}

std::string SurfaceId::ToString(std::string_view frame_sink_debug_label) const
{
    return base::StringPrintf("SurfaceId(%s, %s)", frame_sink_id_.ToString(frame_sink_debug_label).c_str(), local_surface_id_.ToString().c_str());
}
...
{
    return local_surface_id_.embed_token() == other.local_surface_id_.embed_token();
}
