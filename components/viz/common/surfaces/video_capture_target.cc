...
VideoCaptureTarget::VideoCaptureTarget(FrameSinkId frame_sink_id, VideoCaptureSubTarget sub_target)
    : frame_sink_id(frame_sink_id)
    , sub_target(SubTargetIsValid(sub_target) ? sub_target : VideoCaptureSubTarget {})
{
    DCHECK(this->frame_sink_id.is_valid());
}

VideoCaptureTarget::VideoCaptureTarget() = default;
VideoCaptureTarget::VideoCaptureTarget(const VideoCaptureTarget& other) = default;
VideoCaptureTarget::VideoCaptureTarget(VideoCaptureTarget&& other) = default;
VideoCaptureTarget& VideoCaptureTarget::operator=(const VideoCaptureTarget& other) = default;
VideoCaptureTarget& VideoCaptureTarget::operator=(VideoCaptureTarget&& other) = default;
