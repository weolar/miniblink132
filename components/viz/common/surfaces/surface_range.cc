...
SurfaceRange::SurfaceRange(const std::optional<SurfaceId>& start, const SurfaceId& end)
    : start_(start)
    , end_(end)
{
}
...
bool SurfaceRange::IsInRangeExclusive(const SurfaceId& surface_id) const
{
    ...
    if (HasDifferentFrameSinkIds() || end_.local_surface_id().embed_token() != start_->local_surface_id().embed_token()) {
...
{
    return IsInRangeExclusive(surface_id) || end_ == surface_id || start_ == surface_id;
}
...
{
    return start_ && start_->local_surface_id().embed_token() != end_.local_surface_id().embed_token();
}
...
bool SurfaceRange::IsValid() const
{
    ...
    if (end_.local_surface_id().embed_token() != start_->local_surface_id().embed_token()) {
        return true;
    }
...
{
    return base::StringPrintf("SurfaceRange(start: %s, end: %s)", start_ ? start_->ToString().c_str() : "none", end_.ToString().c_str());
}
