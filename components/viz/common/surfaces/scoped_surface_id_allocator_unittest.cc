...
TEST(ScopedSurfaceIdAllocatorTest, SingleArgumentConstructorShouldCallCallback)
{
    bool callback_called = false;
    base::OnceCallback<void()> allocation_task = base::BindOnce([](bool* callback_called) { *callback_called = true; }, &callback_called);
...
// should be called.
TEST(ScopedSurfaceIdAllocatorTest, DualArgumentConstructorShouldSuppressParentAndCallCallback)
{
    bool callback_called = false;
    base::OnceCallback<void()> allocation_task = base::BindOnce([](bool* callback_called) { *callback_called = true; }, &callback_called);
    ...
    {
        ScopedSurfaceIdAllocator suppressing_allocator(&parent_allocator, std::move(allocation_task));
...
TEST(ScopedSurfaceIdAllocatorTest, MoveConstructorShouldTransferLifetimes)
{
    ...
    EXPECT_FALSE(parent_allocator.is_allocation_suppressed());
    {
        ScopedSurfaceIdAllocator moved_from_allocator(&parent_allocator, std::move(allocation_task));
        EXPECT_TRUE(parent_allocator.is_allocation_suppressed());

        {
            ScopedSurfaceIdAllocator moved_to_allocator(std::move(moved_from_allocator));
...
// lifetime.
TEST(ScopedSurfaceIdAllocatorTest, MoveAssignmentOperatorShouldTransferLifetimes)
{
    bool callback_called = false;
    base::OnceCallback<void()> allocation_task = base::BindOnce([](bool* callback_called) { *callback_called = true; }, &callback_called);
    bool second_callback_called = false;
    base::OnceCallback<void()> second_allocation_task
        = base::BindOnce([](bool* second_callback_called) { *second_callback_called = true; }, &second_callback_called);
    ParentLocalSurfaceIdAllocator parent_allocator;
    EXPECT_FALSE(parent_allocator.is_allocation_suppressed());
    {
        ScopedSurfaceIdAllocator moved_from_allocator(&parent_allocator, std::move(allocation_task));
        EXPECT_TRUE(parent_allocator.is_allocation_suppressed());
        {
            ScopedSurfaceIdAllocator moved_to_allocator(std::move(second_allocation_task));
