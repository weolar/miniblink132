...
protected:
        ...
        : context_(std::make_unique<gpu::GLInProcessContext>())
    {
        ...
        auto result = context_->Initialize(TestGpuServiceHolder::GetInstance()->task_executor(), attributes, gpu::SharedMemoryLimits());
    ...
    void StartTracing(const std::string& filter)
    {
        base::trace_event::TraceLog::GetInstance()->SetEnabled(
            base::trace_event::TraceConfig(filter, base::trace_event::RECORD_UNTIL_FULL), base::trace_event::TraceLog::RECORDING_MODE);
    }

    static void TraceDataCB(
        base::OnceClosure quit_closure, std::string* output, const scoped_refptr<base::RefCountedString>& json_events_str, bool has_more_events)
    {
    ...
    void EndTracing(std::map<std::string, int>* event_counts)
    {
        ...
        base::trace_event::TraceLog::GetInstance()->Flush(
            base::BindRepeating(&YUVReadbackTest::TraceDataCB, run_loop.QuitClosure(), base::Unretained(&json_data)));
        run_loop.Run();
        json_data.append("]");

        auto parsed_json = base::JSONReader::ReadAndReturnValueWithError(json_data);
        CHECK(parsed_json.has_value()) << "JSON parsing failed (" << parsed_json.error().message << ") JSON data:" << std::endl << json_data;
    ...
    int Channel(SkBitmap* pixels, int x, int y, int c)
    {
        if (pixels->bytesPerPixel() == 4) {
            uint32_t* data = pixels->getAddr32(std::clamp(x, 0, pixels->width() - 1), std::clamp(y, 0, pixels->height() - 1));
            return (*data) >> (c * 8) & 0xff;
        } else {
            DCHECK_EQ(pixels->bytesPerPixel(), 1);
            DCHECK_EQ(c, 0);
            return *pixels->getAddr8(std::clamp(x, 0, pixels->width() - 1), std::clamp(y, 0, pixels->height() - 1));
        }
    }
    ...
    void PrintPlane(const unsigned char* plane, int xsize, int stride, int ysize)
    {
    ...
    void ComparePlane(const unsigned char* truth, int truth_stride, const unsigned char* other, int other_stride, int maxdiff, int xsize, int ysize,
        SkBitmap* source, std::string message)
    {
        for (int x = 0; x < xsize; x++) {
            for (int y = 0; y < ysize; y++) {
                int a = other[y * other_stride + x];
                int b = truth[y * truth_stride + x];
                EXPECT_NEAR(a, b, maxdiff) << " x=" << x << " y=" << y << " " << message;
    ...
    // returns.
    void TestYUVReadback(int xsize, int ysize, int output_xsize, int output_ysize, int xmargin, int ymargin, int test_pattern, bool flip, bool use_mrt,
        gpu::GLHelper::ScalerQuality quality)
    {
        ...
        gl_->TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, xsize, ysize, 0, GL_RGBA, GL_UNSIGNED_BYTE, input_pixels.getPixels());

        std::string message = base::StringPrintf("input size: %dx%d "
                                                 "output size: %dx%d "
                                                 "margin: %dx%d "
                                                 "pattern: %d %s %s",
            xsize, ysize, output_xsize, output_ysize, xmargin, ymargin, test_pattern, flip ? "flip" : "noflip", use_mrt ? "mrt" : "nomrt");
        std::unique_ptr<gpu::ReadbackYUVInterface> yuv_reader = helper_->CreateReadbackPipelineYUV(flip, use_mrt);

        scoped_refptr<media::VideoFrame> output_frame = media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_I420,
            ...
            // on its coded size.
            gfx::Size((output_xsize + 15) & ~15, (output_ysize + 15) & ~15), gfx::Rect(0, 0, output_xsize, output_ysize), gfx::Size(output_xsize, output_ysize),
            base::Seconds(0));
        scoped_refptr<media::VideoFrame> truth_frame = media::VideoFrame::CreateFrame(media::PIXEL_FORMAT_I420, gfx::Size(output_xsize, output_ysize),
            gfx::Rect(0, 0, output_xsize, output_ysize), gfx::Size(output_xsize, output_ysize), base::Seconds(0));

        base::RunLoop run_loop;
        auto run_quit_closure = [](base::OnceClosure quit_closure, bool result) { std::move(quit_closure).Run(); };
        yuv_reader->ReadbackYUV(src_texture, gfx::Size(xsize, ysize), gfx::Rect(0, 0, xsize, ysize), output_frame->stride(media::VideoFrame::Plane::kY),
            output_frame->writable_data(media::VideoFrame::Plane::kY), output_frame->stride(media::VideoFrame::Plane::kU),
            output_frame->writable_data(media::VideoFrame::Plane::kU), output_frame->stride(media::VideoFrame::Plane::kV),
            output_frame->writable_data(media::VideoFrame::Plane::kV), gfx::Point(xmargin, ymargin), base::BindOnce(run_quit_closure, run_loop.QuitClosure()));

        const gfx::Rect paste_rect(gfx::Point(xmargin, ymargin), gfx::Size(xsize, ysize));
        ...
        unsigned char* Y = truth_frame->GetWritableVisibleData(media::VideoFrame::Plane::kY);
        unsigned char* U = truth_frame->GetWritableVisibleData(media::VideoFrame::Plane::kU);
        unsigned char* V = truth_frame->GetWritableVisibleData(media::VideoFrame::Plane::kV);
        ...
        for (int y = 0; y < ysize; y++) {
            for (int x = 0; x < xsize; x++) {
                Y[(y + ymargin) * y_stride + x + xmargin] = float_to_byte(ChannelAsFloat(&input_pixels, x, y, 0) * kRGBtoYColorWeights[0]
                    + ChannelAsFloat(&input_pixels, x, y, 1) * kRGBtoYColorWeights[1] + ChannelAsFloat(&input_pixels, x, y, 2) * kRGBtoYColorWeights[2]
        ...
        for (int y = 0; y < ysize / 2; y++) {
            for (int x = 0; x < xsize / 2; x++) {
                U[(y + ymargin / 2) * u_stride + x + xmargin / 2] = float_to_byte(Bilinear(&input_pixels, x * 2 + 1.0, y * 2 + 1.0, 0) * kRGBtoUColorWeights[0]
                    + Bilinear(&input_pixels, x * 2 + 1.0, y * 2 + 1.0, 1) * kRGBtoUColorWeights[1]
                    + Bilinear(&input_pixels, x * 2 + 1.0, y * 2 + 1.0, 2) * kRGBtoUColorWeights[2] + kRGBtoUColorWeights[3]);
                V[(y + ymargin / 2) * v_stride + x + xmargin / 2] = float_to_byte(Bilinear(&input_pixels, x * 2 + 1.0, y * 2 + 1.0, 0) * kRGBtoVColorWeights[0]
                    + Bilinear(&input_pixels, x * 2 + 1.0, y * 2 + 1.0, 1) * kRGBtoVColorWeights[1]
                    + Bilinear(&input_pixels, x * 2 + 1.0, y * 2 + 1.0, 2) * kRGBtoVColorWeights[2] + kRGBtoVColorWeights[3]);
            }
        }

        ComparePlane(Y, y_stride, output_frame->visible_data(media::VideoFrame::Plane::kY), output_frame->stride(media::VideoFrame::Plane::kY), 2, output_xsize,
            output_ysize, &input_pixels, message + " Y plane");
        ComparePlane(U, u_stride, output_frame->visible_data(media::VideoFrame::Plane::kU), output_frame->stride(media::VideoFrame::Plane::kU), 2,
            output_xsize / 2, output_ysize / 2, &input_pixels, message + " U plane");
        ComparePlane(V, v_stride, output_frame->visible_data(media::VideoFrame::Plane::kV), output_frame->stride(media::VideoFrame::Plane::kV), 2,
            output_xsize / 2, output_ysize / 2, &input_pixels, message + " V plane");
...
TEST_F(YUVReadbackTest, YUVReadbackOptTest)
{
    for (int use_mrt = 0; use_mrt <= 1; ++use_mrt) {
        ...
        StartTracing(TRACE_DISABLED_BY_DEFAULT("gpu.service") "," TRACE_DISABLED_BY_DEFAULT("gpu.decoder"));

        // Run a test with no size scaling, just planerization.
        TestYUVReadback(800, 400, 800, 400, 0, 0, 1, false, use_mrt == 1, gpu::GLHelper::SCALER_QUALITY_FAST);
...
class YUVReadbackPixelTest : public YUVReadbackTest, public ::testing::WithParamInterface<std::tuple<bool, bool, unsigned int, unsigned int>> { };
...
TEST_P(YUVReadbackPixelTest, Test)
{
    ...
    for (unsigned int ox = x; ox < std::size(kYUVReadbackSizes); ox++) {
        for (unsigned int oy = y; oy < std::size(kYUVReadbackSizes); oy++) {
            ...
            for (Margin xm = x < ox ? MarginLeft : MarginRight; xm <= MarginRight; xm = NextMargin(xm)) {
                for (Margin ym = y < oy ? MarginLeft : MarginRight; ym <= MarginRight; ym = NextMargin(ym)) {
                    for (int pattern = 0; pattern < 3; pattern++) {
                        TestYUVReadback(kYUVReadbackSizes[x], kYUVReadbackSizes[y], kYUVReadbackSizes[ox], kYUVReadbackSizes[oy],
                            compute_margin(kYUVReadbackSizes[x], kYUVReadbackSizes[ox], xm), compute_margin(kYUVReadbackSizes[y], kYUVReadbackSizes[oy], ym),
...
INSTANTIATE_TEST_SUITE_P(All, YUVReadbackPixelTest,
    ::testing::Combine(::testing::Bool(), ::testing::Bool(), ::testing::Range<unsigned int>(0, std::size(kYUVReadbackSizes)),
