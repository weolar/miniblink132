...
sk_sp<SkColorFilter> SkiaHelper::MakeOverdrawColorFilter()
{
    ...
    static const std::array<SkColor, kNumColors> colors = { /*no-color=*/0x00000000, /*no-color=*/0x00000000,
        /*blue=*/0x2f0000ff, /*green=*/0x2f00ff00,
        /*light-red=*/0x3fff0000,
        /*red=*/0x7fff0000, /*orange=*/0x9fff8c00,
        /*purple=*/0x9f6A0DAD, /*wine=*/0x9f2C041C };
    static constexpr char kOverdrawFilterCode[] = "uniform half4 color0, color1, color2, color3, color4, color5, color6, "
                                                  "color7, color8;"
    ...
    static auto* const overdraw_effect = SkRuntimeEffect::MakeForColorFilter(SkString(kOverdrawFilterCode)).effect.release();
    CHECK(overdraw_effect);

    SkRuntimeColorFilterBuilder builder(sk_ref_sp(overdraw_effect));
    for (int i = 0; i < kNumColors; i++) {
        builder.uniform(base::StrCat({ "color", base::NumberToString(i) })) = SkColor4f::FromColor(colors[i]).premul();
    }
