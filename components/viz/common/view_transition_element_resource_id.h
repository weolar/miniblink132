...
public:
    ...
    ViewTransitionElementResourceId(const blink::ViewTransitionToken& transition_token, uint32_t local_id);

    // Creates an invalid id.
    ViewTransitionElementResourceId();
    ~ViewTransitionElementResourceId();

    VIZ_COMMON_EXPORT friend bool operator==(const ViewTransitionElementResourceId&, const ViewTransitionElementResourceId&);
    friend auto operator<=>(const ViewTransitionElementResourceId&, const ViewTransitionElementResourceId&) = default;
...
using ViewTransitionElementResourceRects = std::unordered_map<ViewTransitionElementResourceId, gfx::RectF>;

} // namespace viz

namespace std {
template <> struct hash<viz::ViewTransitionElementResourceId> {
    size_t operator()(const viz::ViewTransitionElementResourceId& resource_id) const
    {
