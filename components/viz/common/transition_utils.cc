...
struct StackFrame {
    StackFrame(int list_index, SharedQuadStateList::ConstIterator sqs_iter, QuadList::ConstIterator quad_iter, int indent)
...
std::string SkColorToRGBAString(SkColor color)
{
    ...
    str << "rgba(" << SkColorGetR(color) << ", " << SkColorGetG(color) << ", " << SkColorGetB(color) << ", " << SkColorGetA(color) << ")";
    return str.str();
}
...
std::unordered_set<uint64_t> ProcessStack(std::ostringstream& str, std::vector<StackFrame>& stack, const CompositorRenderPassList& list)
{
    ...
    auto write_render_pass = [&str](const CompositorRenderPass* pass) {
        str << "(" << pass << ") render pass id=" << pass->id.GetUnsafeValue() << " output_rect=" << pass->output_rect.ToString();
    ...
    auto write_sqs = [&str](const SharedQuadState* sqs) {
        str << "(" << sqs << ") switched to sqs with opacity=" << sqs->opacity << ", blend_mode=" << BlendModeToString(sqs->blend_mode)
            << " quad_layer_rect=" << sqs->quad_layer_rect.ToString() << "\n";
    };
    auto write_shared_element_quad
        = [&str](const SharedElementDrawQuad* quad) { str << "(" << quad << ") SharedElementDrawQuad " << quad->element_resource_id.ToString() << "\n"; };
    auto write_solid_color_quad
        = [&str](const SolidColorDrawQuad* quad) { str << "(" << quad << ") SolidColorDrawQuad color=" << SkColorToRGBAString(quad->color) << "\n"; };
    auto write_compositor_render_pass_quad = [&str](const CompositorRenderPassDrawQuad* quad) { str << "(" << quad << ") CompositorRenderPassDrawQuad\n"; };
    ...
    while (!stack.empty()) {
        ...
        case DrawQuad::Material::kCompositorRenderPass: {
            auto* quad = CompositorRenderPassDrawQuad::MaterialCast((*frame.quad_iter));
            ...
            for (auto i = frame.list_index - 1; i >= 0; --i) {
                if (list[i]->id == quad->render_pass_id) {
                    found = true;
                    stack.emplace_back(i, list[i]->shared_quad_state_list.begin(), list[i]->quad_list.begin(), frame.indent + 2);
                    break;
                }
            }
        ...
        default:
            str << "DrawQuad, material: " << DrawQuadMaterialToString((*frame.quad_iter)->material) << "\n";
            break;
        }
    }
...
std::string TransitionUtils::RenderPassListToString(const CompositorRenderPassList& list)
{
    std::ostringstream str;

    if (list.size() > kMaxListToProcess || list.empty()) {
        str << "RenderPassList too large or too small (" << list.size() << "), max supported list length " << kMaxListToProcess;
        return str.str();
    }

    std::vector<StackFrame> stack;
    stack.emplace_back(list.size() - 1, list.back()->shared_quad_state_list.begin(), list.back()->quad_list.begin(), 0);
    ...
    std::unordered_set<uint64_t> seen_render_pass_ids = ProcessStack(str, stack, list);
    ...
    while (true) {
        ...
        DCHECK(stack.empty());
        stack.emplace_back(i, list[i]->shared_quad_state_list.begin(), list[i]->quad_list.begin(), 0);
        auto new_seen_pass_ids = ProcessStack(str, stack, list);
        seen_render_pass_ids.insert(new_seen_pass_ids.begin(), new_seen_pass_ids.end());
    }
    return str.str();
}

// static
std::unique_ptr<CompositorRenderPass> TransitionUtils::CopyPassWithQuadFiltering(const CompositorRenderPass& source_pass, FilterCallback filter_callback)
{
    ...
    auto copy_pass = CompositorRenderPass::Create(source_pass.shared_quad_state_list.size(), source_pass.quad_list.size());

    copy_pass->SetAll(source_pass.id, source_pass.output_rect, source_pass.damage_rect, source_pass.transform_to_root_target, source_pass.filters,
        source_pass.backdrop_filters, source_pass.backdrop_filter_bounds, source_pass.subtree_capture_id, source_pass.subtree_size,
        source_pass.view_transition_element_resource_id, source_pass.has_transparent_background, source_pass.cache_render_pass,
        source_pass.has_damage_from_contributing_content, source_pass.generate_mipmap, source_pass.has_per_quad_damage);

    if (source_pass.shared_quad_state_list.empty())
        return copy_pass;

    SharedQuadStateList::ConstIterator sqs_iter = source_pass.shared_quad_state_list.begin();
    ...
    *copy_shared_quad_state = **sqs_iter;
    ...
    for (auto* quad : source_pass.quad_list) {
        ...
        if (quad->material == DrawQuad::Material::kCompositorRenderPass) {
            ...
            copy_pass->CopyFromAndAppendRenderPassDrawQuad(pass_quad, pass_quad->render_pass_id);
        } else {
