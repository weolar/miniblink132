...
HitTestRegionList& HitTestRegionList::operator=(const HitTestRegionList&) = default;
...
bool HitTestRegionList::IsEqual(const HitTestRegionList& u, const HitTestRegionList& v)
{
    // Simple checks first.
    bool ret = u.flags == v.flags && u.bounds == v.bounds && u.transform == v.transform && u.regions.size() == v.regions.size();
    if (!ret)
        return false;
...
{
    return u.flags == v.flags && u.frame_sink_id == v.frame_sink_id && u.rect == v.rect && u.transform == v.transform;
}
