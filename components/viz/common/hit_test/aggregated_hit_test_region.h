...
struct AggregatedHitTestRegion {
    ...
    AggregatedHitTestRegion(const FrameSinkId& frame_sink_id, uint32_t flags, const gfx::Rect& rect, const gfx::Transform& transform, int32_t child_count,
        ...
        transform(transform)
    {
        DCHECK_EQ(!!(flags & HitTestRegionFlags::kHitTestAsk), !!async_hit_test_reasons);
    }
    ...
    {
        return (frame_sink_id == rhs.frame_sink_id && flags == rhs.flags && async_hit_test_reasons == rhs.async_hit_test_reasons && rect == rhs.rect
            && child_count == rhs.child_count && transform == rhs.transform);
    }
