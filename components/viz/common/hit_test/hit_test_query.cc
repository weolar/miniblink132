...
bool RegionMatchEventSource(EventSource event_source, uint32_t flags)
{
    ...
    return (flags & (HitTestRegionFlags::kHitTestMouse | HitTestRegionFlags::kHitTestTouch)) != 0u;
}

bool CheckChildCount(int32_t child_count, size_t child_count_max)
{
    return (child_count >= 0) && (static_cast<size_t>(child_count) < child_count_max);
}
...
void HitTestQuery::OnAggregatedHitTestRegionListUpdated(const std::vector<AggregatedHitTestRegion>& hit_test_data)
{
...
Target HitTestQuery::FindTargetForLocation(EventSource event_source, const gfx::PointF& location_in_root) const
{
    if (GetHitTestRegionData().empty()) {
        return Target();
    }
    return FindTargetForLocationStartingFromImpl(event_source, location_in_root, GetHitTestRegionData()[0].frame_sink_id,
        /* is_location_relative_to_parent */ true);
}

Target HitTestQuery::FindTargetForLocationStartingFrom(EventSource event_source, const gfx::PointF& location, const FrameSinkId& frame_sink_id) const
{
    return FindTargetForLocationStartingFromImpl(event_source, location, frame_sink_id,
        /* is_location_relative_to_parent */ false);
}

bool HitTestQuery::TransformLocationForTarget(
    const std::vector<FrameSinkId>& target_ancestors, const gfx::PointF& location_in_root, gfx::PointF* transformed_location) const
{
    if (GetHitTestRegionData().empty()) {
        return false;
    }
    ...
    if (target_ancestors.size() == 0u || target_ancestors[target_ancestors.size() - 1] != GetHitTestRegionData()[0].frame_sink_id) {
        return false;
    }
    ...
    *transformed_location = location_in_root;
    return TransformLocationForTargetRecursively(target_ancestors, target_ancestors.size() - 1, 0, transformed_location);
}

bool HitTestQuery::GetTransformToTarget(const FrameSinkId& target, gfx::Transform* transform) const
{
    if (GetHitTestRegionData().empty()) {
        return false;
    }
...
bool HitTestQuery::ContainsActiveFrameSinkId(const FrameSinkId& frame_sink_id) const
{
    for (auto& it : GetHitTestRegionData()) {
        if (it.frame_sink_id == frame_sink_id && !(it.flags & HitTestRegionFlags::kHitTestNotActive)) {
            return true;
        }
    }
    return false;
...
Target HitTestQuery::FindTargetForLocationStartingFromImpl(
    EventSource event_source, const gfx::PointF& location, const FrameSinkId& frame_sink_id, bool is_location_relative_to_parent) const
{
    if (GetHitTestRegionData().empty()) {
        return Target();
    }
    ...
    FindTargetInRegionForLocation(event_source, location, start_index, is_location_relative_to_parent, frame_sink_id, &target);
    return target;
}

bool HitTestQuery::FindTargetInRegionForLocation(EventSource event_source, const gfx::PointF& location, size_t region_index,
    bool is_location_relative_to_parent, const FrameSinkId& root_view_frame_sink_id, Target* target) const
{
    gfx::PointF location_transformed(location);

    // Exclude a region and all its descendants if the region has the ignore bit
    // set.
    if (GetHitTestRegionData()[region_index].flags & HitTestRegionFlags::kHitTestIgnore) {
        return false;
    }
    ...
    if (is_location_relative_to_parent) {
        ...
        location_transformed = GetHitTestRegionData()[region_index].transform.MapPoint(location_transformed);
        if (!gfx::RectF(GetHitTestRegionData()[region_index].rect).Contains(location_transformed)) {
            return false;
        }
    }

    const int32_t region_child_count = GetHitTestRegionData()[region_index].child_count;
    if (!CheckChildCount(region_child_count, GetHitTestRegionData().size() - region_index)) {
        return false;
    }
    ...
    bool root_view_overlapped = GetHitTestRegionData()[region_index].frame_sink_id == root_view_frame_sink_id
        && GetHitTestRegionData()[region_index].async_hit_test_reasons == AsyncHitTestReasons::kOverlappedRegion;

    // Verify that async_hit_test_reasons is set if and only if there's
    // a kHitTestAsk flag.
    DCHECK_EQ(!!(flags & HitTestRegionFlags::kHitTestAsk), !!GetHitTestRegionData()[region_index].async_hit_test_reasons);

    if ((flags & HitTestRegionFlags::kHitTestAsk) && !(flags & HitTestRegionFlags::kHitTestIgnore) && !root_view_overlapped) {
        target->frame_sink_id = GetHitTestRegionData()[region_index].frame_sink_id;
    ...
    while (child_region < child_region_end) {
        if (FindTargetInRegionForLocation(event_source, location_in_target, child_region,
                /*is_location_relative_to_parent=*/true, FrameSinkId(), target)) {
            return true;
        }

        const int32_t child_region_child_count = GetHitTestRegionData()[child_region].child_count;
    ...
    if ((flags & HitTestRegionFlags::kHitTestMine) && !(flags & HitTestRegionFlags::kHitTestIgnore)) {
        target->frame_sink_id = GetHitTestRegionData()[region_index].frame_sink_id;
        target->location_in_target = location_in_target;
        uint32_t target_flags = flags;
        if (root_view_overlapped) {
            DCHECK_EQ(GetHitTestRegionData()[region_index].async_hit_test_reasons, AsyncHitTestReasons::kOverlappedRegion);
...
bool HitTestQuery::TransformLocationForTargetRecursively(
    const std::vector<FrameSinkId>& target_ancestors, size_t target_ancestor, size_t region_index, gfx::PointF* location_in_target) const
{
    *location_in_target = GetHitTestRegionData()[region_index].transform.MapPoint(*location_in_target);
    if (!target_ancestor)
        return true;
    ...
    size_t child_region_end = child_region + region_child_count;
    while (child_region < child_region_end) {
        if (GetHitTestRegionData()[child_region].frame_sink_id == target_ancestors[target_ancestor - 1]) {
            return TransformLocationForTargetRecursively(target_ancestors, target_ancestor - 1, child_region, location_in_target);
        }
...
bool HitTestQuery::GetTransformToTargetRecursively(const FrameSinkId& target, size_t region_index, gfx::Transform* transform) const
{
    // TODO(crbug.com/41460941): Cache the matrix product such that the transform
    ...
    while (child_region < child_region_end) {
        ...
        if (GetTransformToTargetRecursively(target, child_region, &transform_to_child)) {
            gfx::Transform region_transform(GetHitTestRegionData()[region_index].transform);
...
const std::vector<AggregatedHitTestRegion>& HitTestQuery::GetHitTestRegionData() const
{
...
std::string HitTestQuery::PrintHitTestData() const
{
    ...
    for (uint32_t i = 0; i < GetHitTestRegionData().size(); ++i) {
        ...
        oss << tabs << "AsyncHitTestReasons: " << GetAsyncHitTestReasons(htr.async_hit_test_reasons) << '\n';
        ...
        while (!parents.empty() && parents.top() + GetHitTestRegionData()[parents.top()].child_count <= i) {
            tabs.pop_back();
            tabs.pop_back();
...
bool HitTestQuery::FindIndexOfFrameSink(const FrameSinkId& id, size_t* index) const
{
    for (uint32_t i = 0; i < GetHitTestRegionData().size(); ++i) {
