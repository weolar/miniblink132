...
std::optional<uint32_t> GetDeadlineToSynchronizeSurfaces()
{
    ...
    std::string deadline_to_synchronize_surfaces_string = command_line->GetSwitchValueASCII(switches::kDeadlineToSynchronizeSurfaces);
    ...
    if (!base::StringToUint(deadline_to_synchronize_surfaces_string, &activation_deadline_in_frames)) {
        return std::nullopt;
    }
...
std::optional<DelegatedInkRendererMode> GetDelegatedInkRendererMode()
{
    ...
    std::string mode = command_line->GetSwitchValueASCII(switches::kDelegatedInkRenderer);
    if (mode == "system") {
