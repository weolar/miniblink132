...
public:
    void TestCheckedWidthInBytes(int width, const TestFormat* test_formats)
    {
        for (int i = 0; i < kTestFormats; ++i) {
            size_t bytes = ResourceSizes::CheckedWidthInBytes<size_t>(width, test_formats[i].format);
    ...
    {
        for (int i = 0; i < kTestFormats; ++i) {
            size_t bytes = ResourceSizes::UncheckedWidthInBytes<size_t>(width, test_formats[i].format);
            EXPECT_EQ(bytes, test_formats[i].expected_bytes);
        }
    }

    void TestCheckedSizeInBytes(const gfx::Size& size, const TestFormat* test_formats)
    {
        for (int i = 0; i < kTestFormats; ++i) {
            size_t bytes = ResourceSizes::CheckedSizeInBytes<size_t>(size, test_formats[i].format);
            EXPECT_EQ(bytes, test_formats[i].expected_bytes);
        }
    }
};
...
TEST_F(ResourceUtilTest, SizeInBytesOverflow)
{
    ...
    EXPECT_FALSE(ResourceSizes::MaybeSizeInBytes<signed char>(size, SinglePlaneFormat::kRGBA_4444, &ignored_char));
    int ignored_int;
    EXPECT_TRUE(ResourceSizes::MaybeSizeInBytes<int>(size, SinglePlaneFormat::kRGBA_4444, &ignored_int));
}
...
TEST_F(ResourceUtilTest, WidthOverflowDoesNotCrash)
{
    ...
    // crash.
    int bytes;
    EXPECT_FALSE(ResourceSizes::MaybeSizeInBytes<int>(size, SinglePlaneFormat::kBGRA_8888, &bytes));
}
...
TEST_F(ResourceUtilTest, SizeInBitsOverflowBytesOk)
{
    ...
    int ignored;
    EXPECT_TRUE(ResourceSizes::MaybeSizeInBytes<int>(size, SinglePlaneFormat::kBGRA_8888, &ignored));
}
...
TEST_F(ResourceUtilTest, RoundingOverflows)
{
    ...
    int ignored;
    EXPECT_FALSE(ResourceSizes::MaybeSizeInBytes<int>(size, SinglePlaneFormat::kETC1, &ignored));
}
