...
unsigned int GLDataType(ResourceFormat format)
{
    ...
    static_assert(std::size(format_gl_data_type) == (RESOURCE_FORMAT_MAX + 1), "format_gl_data_type does not handle all cases.");
...
unsigned int GLDataFormat(ResourceFormat format)
{
    ...
    static_assert(std::size(format_gl_data_format) == (RESOURCE_FORMAT_MAX + 1), "format_gl_data_format does not handle all cases.");
...
unsigned int TextureStorageFormat(ResourceFormat format, bool use_angle_rgbx_format)
{
    switch (format) {
    case RGBA_8888:
        return GL_RGBA8_OES;
...
SkColorType ResourceFormatToClosestSkColorType(bool gpu_compositing, SharedImageFormat format)
{
    //return ResourceFormatToClosestSkColorType(gpu_compositing, format.resource_format());
    *(int*)1 = 1;
    return kRGBA_8888_SkColorType;
}
//
