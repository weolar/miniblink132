...
struct VIZ_COMMON_EXPORT TransferableResource {
    ...
    static TransferableResource MakeSoftwareSharedBitmap(const SharedBitmapId& id, const gpu::SyncToken& sync_token, const gfx::Size& size,
        SharedImageFormat format, ResourceSource source = ResourceSource::kUnknown);
    static TransferableResource MakeSoftwareSharedImage(const scoped_refptr<gpu::ClientSharedImage>& client_shared_image, const gpu::SyncToken& sync_token,
        const gfx::Size& size, SharedImageFormat format, ResourceSource source = ResourceSource::kUnknown);
    static TransferableResource MakeGpu(const gpu::Mailbox& mailbox, uint32_t texture_target, const gpu::SyncToken& sync_token, const gfx::Size& size,
        SharedImageFormat format, bool is_overlay_candidate, ResourceSource source = ResourceSource::kUnknown);
    static TransferableResource MakeGpu(const scoped_refptr<gpu::ClientSharedImage>& client_shared_image, uint32_t texture_target,
        const gpu::SyncToken& sync_token, const gfx::Size& size, SharedImageFormat format, bool is_overlay_candidate,
    ...
    static std::vector<ReturnedResource> ReturnResources(const std::vector<TransferableResource>& input);
    bool is_empty() const
    {
        return (absl::holds_alternative<gpu::Mailbox>(memory_buffer_id_) && mailbox().IsZero())
            || (absl::holds_alternative<SharedBitmapId>(memory_buffer_id_) && shared_bitmap_id().IsZero());
    }
    ...
    {
        return id == o.id && is_software == o.is_software && size == o.size && format == o.format && memory_buffer_id_ == o.memory_buffer_id_
            && sync_token_ == o.sync_token_ && texture_target_ == o.texture_target_ && color_space == o.color_space && hdr_metadata == o.hdr_metadata
            && is_overlay_candidate == o.is_overlay_candidate &&
#if BUILDFLAG(IS_ANDROID)
            is_backed_by_surface_texture == o.is_backed_by_surface_texture && wants_promotion_hint == o.wants_promotion_hint &&
#elif BUILDFLAG(IS_WIN)
            wants_promotion_hint == o.wants_promotion_hint &&
#endif
            synchronization_type == o.synchronization_type && resource_source == o.resource_source;
    }
    bool operator!=(const TransferableResource& o) const
    {
        return !(*this == o);
    ...
    const MemoryBufferId& memory_buffer_id() const
    {
        return memory_buffer_id_;
    }
