...
public:
    void TestToClosestSkColorType(std::vector<SkColorType> expected_types, SharedImageFormat format, bool gpu_compositing)
    {
        ...
        for (int plane_index = 0; plane_index < format.NumberOfPlanes(); plane_index++) {
            EXPECT_EQ(expected_types[plane_index], ToClosestSkColorType(gpu_compositing, format, plane_index));
        }
    }
};
...
TEST_F(SharedImageFormatUtilsTest, ToClosestSkColorTypeMultiPlaneNV12)
{
    ...
    std::vector<SkColorType> expected_types = { kAlpha_8_SkColorType, kR8G8_unorm_SkColorType };
...
TEST_F(SharedImageFormatUtilsTest, ToClosestSkColorTypeMultiPlaneYVU)
{
    ...
    std::vector<SkColorType> expected_types = { kAlpha_8_SkColorType, kAlpha_8_SkColorType, kAlpha_8_SkColorType };
    TestToClosestSkColorType(expected_types, format, /*gpu_compositing=*/true);
}
...
TEST_F(SharedImageFormatUtilsTest, ToClosestSkColorTypeMultiPlaneP010)
{
    ...
    std::vector<SkColorType> expected_types = { kA16_unorm_SkColorType, kR16G16_unorm_SkColorType };
    TestToClosestSkColorType(expected_types, format, /*gpu_compositing=*/true);
}
...
TEST_F(SharedImageFormatUtilsTest, ToClosestSkColorTypeMultiPlaneYUVBiplanar16bit)
{
...
TEST_F(SharedImageFormatUtilsTest, ToClosestSkColorTypeMultiPlaneYUVATriplanar)
{
    ...
    std::vector<SkColorType> expected_types = { kA16_float_SkColorType, kR16G16_float_SkColorType, kA16_float_SkColorType };
    TestToClosestSkColorType(expected_types, format, /*gpu_compositing=*/true);
}
...
TEST_F(SharedImageFormatUtilsTest, ToClosestSkColorTypeSoftwareYUV)
{
    ...
    // 10-bit 4:2:0 Y_UV biplanar format (P010)
    SharedImageFormat format = MultiPlaneFormat::kP010;
    std::vector<SkColorType> expected_types = { kN32_SkColorType, kN32_SkColorType };
    TestToClosestSkColorType(expected_types, format, /*gpu_compositing=*/false);
}
