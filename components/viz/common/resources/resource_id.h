...
inline constexpr ResourceId kVizReservedRangeStartId(std::numeric_limits<uint32_t>::max() - kNumReservedResourceIds);
...
public:
    ...
    ResourceId PeekNextValueForTesting() const
    {
        return ResourceId(next_id_);
    }
...
public:
    ...
    void set_next_id_for_test(uint32_t next_id)
    {
        next_id_ = next_id;
    }
