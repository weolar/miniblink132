...
#if 0 // BUILDFLAG(ENABLE_VULKAN)
#include <vulkan/vulkan_core.h>
#endif

namespace viz {

VIZ_RESOURCE_FORMAT_EXPORT SkColorType ResourceFormatToClosestSkColorType(bool gpu_compositing, ResourceFormat format);
...
VIZ_RESOURCE_FORMAT_EXPORT ResourceFormat SkColorTypeToResourceFormat(SkColorType color_type);
...
VIZ_RESOURCE_FORMAT_EXPORT gfx::BufferFormat BufferFormat(ResourceFormat format);
VIZ_RESOURCE_FORMAT_EXPORT bool IsResourceFormatCompressed(ResourceFormat format);

// |use_angle_rgbx_format| should be true when the GL_ANGLE_rgbx_internal_format
// extension is available.
VIZ_RESOURCE_FORMAT_EXPORT unsigned int TextureStorageFormat(ResourceFormat format, bool use_angle_rgbx_format);

// Returns whether the format can be used with GpuMemoryBuffer texture storage.
VIZ_RESOURCE_FORMAT_EXPORT bool IsGpuMemoryBufferFormatSupported(ResourceFormat format);
...
VIZ_RESOURCE_FORMAT_EXPORT ResourceFormat GetResourceFormat(gfx::BufferFormat format);
...
VIZ_RESOURCE_FORMAT_EXPORT SkColorType ResourceFormatToClosestSkColorType(bool gpu_compositing, SharedImageFormat format);
...
VIZ_RESOURCE_FORMAT_EXPORT unsigned int GLInternalFormat(SharedImageFormat format);

VIZ_RESOURCE_FORMAT_EXPORT gfx::BufferFormat BufferFormat(SharedImageFormat format);
VIZ_RESOURCE_FORMAT_EXPORT bool IsResourceFormatCompressed(SharedImageFormat format);

VIZ_RESOURCE_FORMAT_EXPORT unsigned int TextureStorageFormat(SharedImageFormat format, bool use_angle_rgbx_format);

VIZ_RESOURCE_FORMAT_EXPORT bool IsGpuMemoryBufferFormatSupported(SharedImageFormat format);
...
#if BUILDFLAG(IS_APPLE)
VIZ_RESOURCE_FORMAT_EXPORT unsigned int ToMTLPixelFormat(SharedImageFormat format);
#endif
