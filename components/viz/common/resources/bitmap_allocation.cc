...
void CollectMemoryUsageAndDie(const gfx::Size& size, SharedImageFormat format, size_t alloc_size)
{
#if BUILDFLAG(IS_WIN)
...
base::MappedReadOnlyRegion AllocateSharedBitmap(const gfx::Size& size, SharedImageFormat format)
{
    DCHECK(format.IsBitmapFormatSupported()) << "(format = " << format.ToString() << ")";
    size_t bytes = 0;
    ...
    base::MappedReadOnlyRegion shm = base::ReadOnlySharedMemoryRegion::Create(bytes);
    if (!shm.IsValid()) {
