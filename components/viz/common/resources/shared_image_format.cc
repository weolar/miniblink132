...
{
    return is_single_plane() && singleplanar_format() == mojom::SingleplanarFormat::RGBA_8888;
}
...
std::optional<size_t> SharedImageFormat::MaybeEstimatedPlaneSizeInBytes(int plane_index, const gfx::Size& size) const
{
    DCHECK(!size.IsEmpty());
    ...
    if (is_single_plane()) {
        ...
        base::CheckedNumeric<size_t> estimated_bytes = ConvertBitsToBytes(bits_per_row.ValueOrDie());
    ...
    base::CheckedNumeric<size_t> plane_estimated_bytes = bytes_per_element * NumChannelsInPlane(plane_index);
...
std::optional<size_t> SharedImageFormat::MaybeEstimatedSizeInBytes(const gfx::Size& size) const
{
    DCHECK(!size.IsEmpty());
    ...
    for (int plane_index = 0; plane_index < NumberOfPlanes(); ++plane_index) {
        std::optional<size_t> plane_estimated_bytes = MaybeEstimatedPlaneSizeInBytes(plane_index, size);
...
gfx::Size SharedImageFormat::GetPlaneSize(int plane_index, const gfx::Size& size) const
{
...
std::string SharedImageFormat::ToString() const
{
    ...
    case PlaneType::kMultiPlane:
        return base::StringPrintf("(%s, %s, %s, %s)", PlaneConfigToString(plane_config()), SubsamplingToString(subsampling()),
            ChannelFormatToString(channel_format()), PrefersExternalSamplerToString(*this));
    }
}
...
std::string SharedImageFormat::ToTestParamString() const
{
    ...
    case PlaneType::kSinglePlane:
        return SinglePlaneFormatToString(*this);
    case PlaneType::kMultiPlane:
        return base::StringPrintf("%s_%s_%s_%s", PlaneConfigToString(plane_config()), SubsamplingToString(subsampling()),
            ChannelFormatToString(channel_format()), PrefersExternalSamplerToString(*this));
    }
}
...
{
    return is_single_plane() && singleplanar_format() == mojom::SingleplanarFormat::ETC1;
}
...
std::weak_ordering SharedImageFormat::operator<=>(const SharedImageFormat& o) const
{
    if (plane_type_ != o.plane_type()) {
...
bool SharedImageFormat::SharedImageFormatUnion::MultiplanarFormat::operator==(const MultiplanarFormat& o) const
{
    return plane_config == o.plane_config && subsampling == o.subsampling && channel_format == o.channel_format;
}

std::weak_ordering SharedImageFormat::SharedImageFormatUnion::MultiplanarFormat::operator<=>(const MultiplanarFormat& o) const
{
    return std::tie(plane_config, subsampling, channel_format) <=> std::tie(o.plane_config, o.subsampling, o.channel_format);
}
