...
VIZ_COMMON_EXPORT bool GetScaledRegion(const gfx::Rect& rect, const gfx::QuadF* clip, gfx::QuadF* scaled_region);
// This takes a rounded rect and a rect that it lives in, and returns an
// equivalent rounded rect in the space -0.5->0.5.
VIZ_COMMON_EXPORT bool GetScaledRRectF(const gfx::Rect& space, const gfx::RRectF& rect, gfx::RRectF* scaled_rect);
...
VIZ_COMMON_EXPORT bool GatherFDStats(base::TimeDelta* delta_time_taken, int* fd_max, int* active_fd_count, int* rlim_cur);
...
VIZ_COMMON_EXPORT gfx::Rect GetExpandedRectWithPixelMovingForegroundFilter(const DrawQuad& rpdq, const cc::FilterOperations& filters);

// This transforms a rect from the view transition content surface/render_pass
// space to the shared element quad space.
VIZ_COMMON_EXPORT gfx::Transform GetViewTransitionTransform(gfx::Rect shared_element_quad, gfx::Rect view_transition_content_output);

// Returns true if the quad's visible rect bounds overlaps with at least one
// of the rounded corners bounding rects.
VIZ_COMMON_EXPORT bool QuadRoundedCornersBoundsIntersects(const DrawQuad* quad, const gfx::RectF& target_quad);
