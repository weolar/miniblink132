...
public:
    ...
    FrameDeadline(base::TimeTicks frame_start_time, uint32_t deadline_in_frames, base::TimeDelta frame_interval, bool use_default_lower_bound_deadline)
    ...
    // deadline in frames.
    base::TimeTicks ToWallTime(std::optional<uint32_t> default_deadline_in_frames = std::nullopt) const;

    friend bool operator==(const FrameDeadline&, const FrameDeadline&) = default;

    base::TimeTicks frame_start_time() const
    {
...
VIZ_COMMON_EXPORT std::ostream& operator<<(std::ostream& out, const FrameDeadline& frame_deadline);
