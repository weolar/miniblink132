...
SharedQuadState& SharedQuadState::operator=(const SharedQuadState& other) = default;

SharedQuadState::~SharedQuadState()
{
    TRACE_EVENT_OBJECT_DELETED_WITH_ID(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "viz::SharedQuadState", this);
}
...
bool SharedQuadState::Equals(const SharedQuadState& other) const
{
    ...
    return quad_to_target_transform == other.quad_to_target_transform && quad_layer_rect == other.quad_layer_rect
        && visible_quad_layer_rect == other.visible_quad_layer_rect && mask_filter_info == other.mask_filter_info && clip_rect == other.clip_rect
        && are_contents_opaque == other.are_contents_opaque && opacity == other.opacity && blend_mode == other.blend_mode
        && sorting_context_id == other.sorting_context_id && layer_id == other.layer_id && layer_namespace_id == other.layer_namespace_id
...
void SharedQuadState::SetAll(const gfx::Transform& transform, const gfx::Rect& layer_rect, const gfx::Rect& visible_layer_rect,
    const gfx::MaskFilterInfo& filter_info, const std::optional<gfx::Rect>& clip, bool contents_opaque, float opacity_f, SkBlendMode blend, int sorting_context,
    uint32_t layer, bool fast_rounded_corner)
{
...
void SharedQuadState::AsValueInto(base::trace_event::TracedValue* value) const
{
    ...
    cc::MathUtil::AddToTracedValue("layer_visible_content_rect", visible_quad_layer_rect, value);
    cc::MathUtil::AddToTracedValue("mask_filter_bounds", mask_filter_info.bounds(), value);
    if (mask_filter_info.HasRoundedCorners()) {
        cc::MathUtil::AddCornerRadiiToTracedValue("mask_filter_rounded_corners_radii", mask_filter_info.rounded_corner_bounds(), value);
    }
    if (mask_filter_info.HasGradientMask()) {
        cc::MathUtil::AddToTracedValue("mask_filter_gradient_mask", mask_filter_info.gradient_mask().value(), value);
    }
    ...
    TracedValue::MakeDictIntoImplicitSnapshotWithCategory(TRACE_DISABLED_BY_DEFAULT("viz.quads"), value, "viz::SharedQuadState", this);
}
