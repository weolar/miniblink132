...
{
    constexpr gfx::Transform quad_transform = gfx::Transform::Affine(1.0, 0.5, 0.0, 1.0, 0.5, 0.0);
    constexpr gfx::Rect layer_rect(26, 28);
    const gfx::MaskFilterInfo mask_filter_rounded_corners(gfx::RectF(5, 5), gfx::RoundedCornersF(2.5), gfx::LinearGradient());
    ...
    state->SetAll(quad_transform, layer_rect, visible_layer_rect, mask_filter_rounded_corners, clip_rect, are_contents_opaque, opacity, blend_mode,
        sorting_context_id, layer_id, is_fast_rounded_corner);
...
SharedQuadState* CreateSharedQuadState(CompositorRenderPass* render_pass)
{
    ...
    state->SetAll(quad_transform, layer_rect, visible_layer_rect, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity, blend_mode,
        sorting_context_id, layer_id, is_fast_rounded_corner);
    return state;
}

void CompareSharedQuadState(const SharedQuadState* source_sqs, const SharedQuadState* copy_sqs)
{
    ...
    EXPECT_EQ(source_sqs->visible_quad_layer_rect, copy_sqs->visible_quad_layer_rect);
    ...
    EXPECT_EQ(source_sqs->is_fast_rounded_corner, copy_sqs->is_fast_rounded_corner);
}
...
#define SETUP_AND_COPY_QUAD_NEW_RP(Type, quad, a)                                                                                                              \
    DrawQuad* copy_new = render_pass->CopyFromAndAppendRenderPassDrawQuad(quad_new, a);                                                                        \
    CompareDrawQuad(quad_new, copy_new);                                                                                                                       \
    [[maybe_unused]] const Type* copy_quad = Type::MaterialCast(copy_new);

#define SETUP_AND_COPY_QUAD_ALL_RP(Type, quad, a)                                                                                                              \
    DrawQuad* copy_all = render_pass->CopyFromAndAppendRenderPassDrawQuad(quad_all, a);                                                                        \
    CompareDrawQuad(quad_all, copy_all);                                                                                                                       \
...
#define CREATE_QUAD_ALL(Type, ...)                                                                                                                             \
    ...
    {                                                                                                                                                          \
        QUAD_DATA quad_all->SetAll(shared_state, quad_rect, quad_visible_rect, needs_blending, __VA_ARGS__);                                                   \
    }                                                                                                                                                          \
...
#define CREATE_QUAD_ALL_RP(Type, a, b, c, d, e, f, g, h, i, j, k, copy_a)                                                                                      \
    Type* quad_all = render_pass->CreateAndAppendDrawQuad<Type>();                                                                                             \
    {                                                                                                                                                          \
        QUAD_DATA quad_all->SetAll(shared_state, quad_rect, a, needs_blending, b, c, d, e, f, g, h, i, j, k);                                                  \
    }                                                                                                                                                          \
...
#define CREATE_QUAD_NEW_RP(Type, a, b, c, d, e, f, g, h, i, j, copy_a)                                                                                         \
    Type* quad_new = render_pass->CreateAndAppendDrawQuad<Type>();                                                                                             \
    {                                                                                                                                                          \
        QUAD_DATA quad_new->SetNew(shared_state, quad_rect, a, b, c, d, e, f, g, h, i, j);                                                                     \
    }                                                                                                                                                          \
...
TEST(DrawQuadTest, CopyRenderPassDrawQuad)
{
    ...
    CREATE_QUAD_ALL_RP(CompositorRenderPassDrawQuad, visible_rect, render_pass_id, mask_resource_id, mask_uv_rect, mask_texture_size, filters_scale,
        filters_origin, tex_coord_rect, force_anti_aliasing_off, backdrop_filter_quality, intersects_damage_under, copied_render_pass_id);
    ...
    EXPECT_EQ(mask_texture_size.ToString(), copy_quad->mask_texture_size.ToString());
...
TEST(DrawQuadTest, CopySolidColorDrawQuad)
{
    ...
    CREATE_QUAD_NEW(SolidColorDrawQuad, visible_rect, color, force_anti_aliasing_off);
...
{
    gfx::Rect visible_rect(40, 50, 30, 20);
    SurfaceId primary_surface_id(kArbitraryFrameSinkId, LocalSurfaceId(1234, base::UnguessableToken::Create()));
    SurfaceId fallback_surface_id(kArbitraryFrameSinkId, LocalSurfaceId(5678, base::UnguessableToken::Create()));
    CREATE_SHARED_STATE();

    CREATE_QUAD_NEW(
        SurfaceDrawQuad, visible_rect, SurfaceRange(fallback_surface_id, primary_surface_id), SkColors::kWhite, /*stretch_content_to_fill_bounds=*/true);
    ...
    CREATE_QUAD_ALL(SurfaceDrawQuad, SurfaceRange(fallback_surface_id, primary_surface_id), SkColors::kWhite, /*stretch_content_to_fill_bounds=*/false,
...
TEST(DrawQuadTest, CopyTextureDrawQuad)
{
    ...
    gfx::ProtectedVideoType protected_video_type = gfx::ProtectedVideoType::kSoftwareProtected;
    CREATE_SHARED_STATE();

    CREATE_QUAD_NEW(TextureDrawQuad, visible_rect, blending, resource_id, premultiplied_alpha, uv_top_left, uv_bottom_right, SkColors::kTransparent, y_flipped,
    ...
    CREATE_QUAD_ALL(TextureDrawQuad, resource_id, resource_size_in_pixels, premultiplied_alpha, uv_top_left, uv_bottom_right, SkColors::kTransparent, y_flipped,
        nearest_neighbor, secure_output_only, protected_video_type);
    EXPECT_EQ(DrawQuad::Material::kTextureContent, copy_quad->material);
    EXPECT_EQ(resource_id, copy_quad->resource_id());
...
TEST(DrawQuadTest, CopyTileDrawQuad)
{
    ...
    CREATE_QUAD_NEW(
        TileDrawQuad, visible_rect, blending, resource_id, tex_coord_rect, texture_size, contents_premultiplied, nearest_neighbor, force_anti_aliasing_off);
    ...
    CREATE_QUAD_ALL(TileDrawQuad, resource_id, tex_coord_rect, texture_size, contents_premultiplied, nearest_neighbor, force_anti_aliasing_off);
    EXPECT_EQ(DrawQuad::Material::kTiledContent, copy_quad->material);
    EXPECT_EQ(resource_id, copy_quad->resource_id());
    EXPECT_EQ(tex_coord_rect, copy_quad->tex_coord_rect);
    EXPECT_EQ(texture_size, copy_quad->texture_size);
...
TEST(DrawQuadTest, CopyPictureDrawQuad)
{
    ...
    auto display_item_list = cc::FakeRasterSource::CreateEmpty(gfx::Size(100, 100))->GetDisplayItemList();
    cc::ScrollOffsetMap raster_inducing_scroll_offsets = { { cc::ElementId(123), gfx::PointF(456.f, 789.f) } };
    CREATE_SHARED_STATE();

    CREATE_QUAD_NEW(PictureDrawQuad, visible_rect, blending, tex_coord_rect, texture_size, nearest_neighbor, content_rect, contents_scale, {},
    ...
    EXPECT_EQ(raster_inducing_scroll_offsets, copy_quad->raster_inducing_scroll_offsets);
}
...
TEST_F(DrawQuadIteratorTest, CompositorRenderPassDrawQuad)
{
    ...
    CREATE_SHARED_STATE();
    CREATE_QUAD_NEW_RP(CompositorRenderPassDrawQuad, visible_rect, render_pass_id, mask_resource_id, mask_uv_rect, mask_texture_size, filters_scale,
        filters_origin, tex_coord_rect, force_anti_aliasing_off, backdrop_filter_quality, copied_render_pass_id);
    ...
    gfx::Rect quad_rect(30, 40, 50, 60);
    quad_new->SetNew(shared_state, quad_rect, visible_rect, render_pass_id, new_mask_resource_id, mask_uv_rect, mask_texture_size, filters_scale,
        filters_origin, tex_coord_rect, force_anti_aliasing_off, backdrop_filter_quality);
    EXPECT_EQ(0, IterateAndCount(quad_new));
...
{
    gfx::Rect visible_rect(40, 50, 30, 20);
    SurfaceId surface_id(kArbitraryFrameSinkId, LocalSurfaceId(4321, base::UnguessableToken::Create()));

    CREATE_SHARED_STATE();
    CREATE_QUAD_NEW(SurfaceDrawQuad, visible_rect, SurfaceRange(std::nullopt, surface_id), SkColors::kWhite,
...
TEST_F(DrawQuadIteratorTest, TextureDrawQuad)
{
    ...
    CREATE_SHARED_STATE();
    CREATE_QUAD_NEW(TextureDrawQuad, visible_rect, needs_blending, resource_id, premultiplied_alpha, uv_top_left, uv_bottom_right, SkColors::kTransparent,
...
TEST_F(DrawQuadIteratorTest, TileDrawQuad)
{
    ...
    CREATE_SHARED_STATE();
    CREATE_QUAD_NEW(TileDrawQuad, visible_rect, needs_blending, resource_id, tex_coord_rect, texture_size, contents_premultiplied, nearest_neighbor,
...
TEST(DrawQuadTest, LargestQuadType)
{
    ...
    for (int i = 0; i <= static_cast<int>(DrawQuad::Material::kMaxValue); ++i) {
        ...
        case DrawQuad::Material::kTextureContent:
            ...
            break;
        case DrawQuad::Material::kCompositorRenderPass:
            LOG(ERROR) << "CompositorRenderPassDrawQuad " << sizeof(CompositorRenderPassDrawQuad);
            break;
        case DrawQuad::Material::kSolidColor:
...
class TextureDrawQuadTest : public testing::Test, public ::testing::WithParamInterface<std::tuple<RoundedDisplayMasksInfo, gfx::RectF, gfx::RectF>> {
public:
    TextureDrawQuadTest()
...
protected:
    void AddQuadWithRoundedDisplayMasks(gfx::Rect quad_rect, bool is_overlay_candidate, const gfx::Transform& quad_to_target_transform,
        const RoundedDisplayMasksInfo& rounded_display_masks_info, AggregatedRenderPass* render_pass)
    {
...
TEST_P(TextureDrawQuadTest, CorrectRoundedDisplayMaskBounds)
{
    ...
    AddQuadWithRoundedDisplayMasks(kTestQuadRect,
        /*is_overlay_candidate=*/true, identity, mask_info_, &render_pass);

    const auto mask_bounds = TextureDrawQuad::RoundedDisplayMasksInfo::GetRoundedDisplayMasksBounds(render_pass.quad_list.front());

    EXPECT_EQ(mask_bounds[RoundedDisplayMasksInfo::kOriginRoundedDisplayMaskIndex], expected_origin_mask_bounds_);
    EXPECT_EQ(mask_bounds[RoundedDisplayMasksInfo::kOtherRoundedDisplayMaskIndex], expected_other_mask_bounds_);
}

INSTANTIATE_TEST_SUITE_P(
    /*no_prefix*/, TextureDrawQuadTest,
    testing::Values(std::make_tuple(RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(
