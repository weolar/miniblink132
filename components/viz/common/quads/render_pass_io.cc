...
{
    return base::Value::Dict().Set("x", rect.x()).Set("y", rect.y()).Set("width", rect.width()).Set("height", rect.height());
}
...
bool RectFFromDict(const base::Value::Dict& dict, gfx::RectF* rect)
{
    ...
    rect->SetRect(static_cast<float>(x.value()), static_cast<float>(y.value()), static_cast<float>(width.value()), static_cast<float>(height.value()));
    return true;
}

base::Value::Dict SizeToDict(const gfx::Size& size)
{
    return base::Value::Dict().Set("width", size.width()).Set("height", size.height());
}
...
{
    return base::Value::Dict().Set("red", color.fR).Set("green", color.fG).Set("blue", color.fB).Set("alpha", color.fA);
}
...
bool ColorFromDict(const base::Value::Dict& dict, std::string_view key, SkColor4f* output_color)
{
...
bool FloatArrayFromList(const base::Value::List& list, size_t expected_count, float* data)
{
    DCHECK(data);
...
base::Value::Dict RRectFToDict(const gfx::RRectF& rect)
{
    ...
    if (rect.GetType() != gfx::RRectF::Type::kEmpty) {
        ...
        dict.Set("upper_left.x", rect.GetCornerRadii(gfx::RRectF::Corner::kUpperLeft).x());
        dict.Set("upper_left.y", rect.GetCornerRadii(gfx::RRectF::Corner::kUpperLeft).y());
        dict.Set("upper_right.x", rect.GetCornerRadii(gfx::RRectF::Corner::kUpperRight).x());
        dict.Set("upper_right.y", rect.GetCornerRadii(gfx::RRectF::Corner::kUpperRight).y());
        dict.Set("lower_right.x", rect.GetCornerRadii(gfx::RRectF::Corner::kLowerRight).x());
        dict.Set("lower_right.y", rect.GetCornerRadii(gfx::RRectF::Corner::kLowerRight).y());
        dict.Set("lower_left.x", rect.GetCornerRadii(gfx::RRectF::Corner::kLowerLeft).x());
        dict.Set("lower_left.y", rect.GetCornerRadii(gfx::RRectF::Corner::kLowerLeft).y());
    }
    return dict;
}
...
bool RRectFFromDict(const base::Value::Dict& dict, gfx::RRectF* out)
{
    ...
    if (!rect || !upper_left_x || !upper_left_y || !upper_right_x || !upper_right_y || !lower_right_x || !lower_right_y || !lower_left_x || !lower_left_y) {
        return false;
    }
    gfx::RectF rect_f;
    if (!RectFFromDict(*rect, &rect_f))
        return false;
    gfx::RRectF rrectf(rect_f, static_cast<float>(upper_left_x.value()), static_cast<float>(upper_left_y.value()), static_cast<float>(upper_right_x.value()),
        static_cast<float>(upper_right_y.value()), static_cast<float>(lower_right_x.value()), static_cast<float>(lower_right_y.value()),
        static_cast<float>(lower_left_x.value()), static_cast<float>(lower_left_y.value()));
...
base::Value::Dict LinearGradientToDict(const gfx::LinearGradient& gradient_mask)
{
    base::Value::List steps;
    for (size_t i = 0; i < gradient_mask.step_count(); ++i) {
        steps.Append(base::Value::Dict()
                         .Set("fraction", static_cast<double>(gradient_mask.steps()[i].fraction))
...
bool LinearGradientFromDict(const base::Value::Dict& dict, gfx::LinearGradient* out)
{
...
base::Value::Dict MaskFilterInfoToDict(const gfx::MaskFilterInfo& mask_filter_info)
{
    auto dict = base::Value::Dict().Set("rounded_corner_bounds", RRectFToDict(mask_filter_info.rounded_corner_bounds()));
    if (mask_filter_info.HasGradientMask()) {
        dict.Set("gradient_mask", LinearGradientToDict(*mask_filter_info.gradient_mask()));
    }
    return dict;
}

bool MaskFilterInfoFromDict(const base::Value::Dict& dict, gfx::MaskFilterInfo* out)
{
    DCHECK(out);
    const base::Value::Dict* rounded_corner_bounds = dict.FindDict("rounded_corner_bounds");
...
bool TransformFromList(const base::Value::List& list, gfx::Transform* transform)
{
    DCHECK(transform);
    if (list.size() != 16)
        return false;
...
base::Value::List ShapeRectsToList(const cc::FilterOperation::ShapeRects& shape)
{
    base::Value::List list;
...
bool ShapeRectsFromList(const base::Value::List& list, cc::FilterOperation::ShapeRects* shape)
{
    DCHECK(shape);
...
std::string PaintFilterToString(const sk_sp<cc::PaintFilter>& filter)
{
    ...
    cc::PaintOpWriter writer(buffer.data(), buffer.size(), options, true /* enable_security_constraints */);
    writer.Write(filter.get(), SkM44());
    if (writer.size() == 0)
        return "";
    buffer.resize(writer.size());

    return base::Base64Encode(base::span<const uint8_t>(buffer.data(), buffer.size()));
}
...
base::Value::Dict FilterOperationToDict(const cc::FilterOperation& filter)
{
    ...
    dict.Set("type", type);
    if (type != cc::FilterOperation::COLOR_MATRIX && type != cc::FilterOperation::REFERENCE && type != cc::FilterOperation::OFFSET) {
    ...
    case cc::FilterOperation::DROP_SHADOW:
        ...
        dict.Set("drop_shadow_color", SkColor4fToDict(filter.drop_shadow_color()));
        break;
    case cc::FilterOperation::REFERENCE:
        dict.Set("image_filter", PaintFilterToString(filter.image_filter()));
        break;
    case cc::FilterOperation::COLOR_MATRIX:
        dict.Set("matrix", FloatArrayToList(base::span<const float>(filter.matrix().data(), filter.matrix().size())));
        break;
...
bool FilterOperationFromDict(const base::Value& dict_value, cc::FilterOperation* out)
{
    DCHECK(out);
    if (!dict_value.is_dict()) {
        return false;
    ...
    if (!type)
        return false;
    cc::FilterOperation::FilterType filter_type = static_cast<cc::FilterOperation::FilterType>(type.value());
    filter.set_type(filter_type);
    if (filter_type != cc::FilterOperation::COLOR_MATRIX && filter_type != cc::FilterOperation::REFERENCE && filter_type != cc::FilterOperation::OFFSET) {
        if (!amount)
            return false;
    ...
    case cc::FilterOperation::BLUR:
        if (!blur_tile_mode)
            return false;
        filter.set_blur_tile_mode(static_cast<SkTileMode>(blur_tile_mode.value()));
        break;
    case cc::FilterOperation::OFFSET: {
...
bool FilterOperationsFromList(const base::Value::List& list, cc::FilterOperations* filters)
{
    DCHECK(filters);
...
bool TransferFunctionFromList(const base::Value::List& list, skcms_TransferFunction* fn)
{
    DCHECK(fn);
    float data[7];
...
base::Value::Dict ColorSpaceToDict(const gfx::ColorSpace& color_space)
{
    auto dict = base::Value::Dict()
                    .Set("primaries", ColorSpacePrimaryIdToString(color_space.GetPrimaryID()))
                    .Set("transfer", ColorSpaceTransferIdToString(color_space.GetTransferID()))
    ...
    if (color_space.GetTransferID() == gfx::ColorSpace::TransferID::CUSTOM || color_space.GetTransferID() == gfx::ColorSpace::TransferID::CUSTOM_HDR) {
...
bool ColorSpaceFromDict(const base::Value::Dict& dict, gfx::ColorSpace* color_space)
{
    DCHECK(color_space);
    ...
    bool uses_custom_primary_matrix = primary_id == static_cast<uint8_t>(gfx::ColorSpace::PrimaryID::CUSTOM);
    if (uses_custom_primary_matrix) {
        const base::Value::List* custom_primary_matrix = dict.FindList("custom_primary_matrix");
        if (!custom_primary_matrix || !Matrix3x3FromList(*custom_primary_matrix, &t_custom_primary_matrix)) {
            return false;
        }
    }
    skcms_TransferFunction t_custom_transfer_params;
    bool uses_custom_transfer_params = transfer_id == static_cast<uint8_t>(gfx::ColorSpace::TransferID::CUSTOM)
        || transfer_id == static_cast<uint8_t>(gfx::ColorSpace::TransferID::CUSTOM_HDR);
    if (uses_custom_transfer_params) {
        const base::Value::List* custom_transfer_params = dict.FindList("custom_transfer_params");
        if (!custom_transfer_params || !TransferFunctionFromList(*custom_transfer_params, &t_custom_transfer_params)) {
            return false;
        }
    }
    *color_space = gfx::ColorSpace(static_cast<gfx::ColorSpace::PrimaryID>(primary_id), static_cast<gfx::ColorSpace::TransferID>(transfer_id),
        static_cast<gfx::ColorSpace::MatrixID>(matrix_id), static_cast<gfx::ColorSpace::RangeID>(range_id),
        uses_custom_primary_matrix ? &t_custom_primary_matrix : nullptr, uses_custom_transfer_params ? &t_custom_transfer_params : nullptr);
    return true;
}

base::Value::List DrawQuadResourcesToList(const DrawQuad::Resources& resources)
{
    base::Value::List list;
...
bool DrawQuadResourcesFromList(const base::Value::List& list, DrawQuad::Resources* resources)
{
    DCHECK(resources);
    size_t size = list.size();
    if (size == 0u) {
...
base::Value::Dict SurfaceIdToDict(const SurfaceId& id)
{
    return base::Value::Dict()
        ...
        .Set("parent_seq", static_cast<int>(id.local_surface_id().parent_sequence_number()))
        .Set("child_seq", static_cast<int>(id.local_surface_id().child_sequence_number()))
        .Set("embed_token", base::UnguessableTokenToValue(id.local_surface_id().embed_token()));
}
...
std::optional<SurfaceId> SurfaceIdFromDict(const base::Value::Dict& dict)
{
    ...
    return SurfaceId(FrameSinkId(*client_id, *sink_id), LocalSurfaceId(*parent_seq, *child_seq, *token));
}
...
std::optional<SurfaceRange> SurfaceRangeFromDict(const base::Value::Dict& dict)
{
    const base::Value::Dict* start_dict = dict.FindDict("start");
    const base::Value::Dict* end_dict = dict.FindDict("end");
    if (!end_dict)
        return std::nullopt;
    std::optional<SurfaceId> start = start_dict ? SurfaceIdFromDict(*start_dict) : std::nullopt;
...
int GetSharedQuadStateIndex(const SharedQuadStateList& shared_quad_state_list, const SharedQuadState* shared_quad_state)
{
    for (auto iter = shared_quad_state_list.begin(); iter != shared_quad_state_list.end(); ++iter) {
...
void DrawQuadCommonToDict(const DrawQuad* draw_quad, base::Value::Dict* dict, const SharedQuadStateList& shared_quad_state_list)
{
    DCHECK(draw_quad);
    DCHECK(dict);
    ...
    int shared_quad_state_index = GetSharedQuadStateIndex(shared_quad_state_list, draw_quad->shared_quad_state);
...
void ContentDrawQuadCommonToDict(const ContentDrawQuadBase* draw_quad, base::Value::Dict* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
...
std::optional<DrawQuadCommon> GetDrawQuadCommonFromDict(const base::Value::Dict& dict, const SharedQuadStateList& shared_quad_state_list)
{
    ...
    std::optional<int> shared_quad_state_index = dict.FindInt("shared_quad_state_index");
    const base::Value::List* resources = dict.FindList("resources");
    if (!material || !rect || !visible_rect || !needs_blending || !shared_quad_state_index || !resources) {
        return std::nullopt;
    }
    ...
    if (sqs_index < 0 || static_cast<size_t>(sqs_index) >= shared_quad_state_list.size()) {
        return std::nullopt;
    }
    gfx::Rect t_rect, t_visible_rect;
    if (!RectFromDict(*rect, &t_rect) || !RectFromDict(*visible_rect, &t_visible_rect)) {
        return std::nullopt;
    }
    ...
    return DrawQuadCommon { static_cast<DrawQuad::Material>(material_index), t_rect, t_visible_rect, needs_blending.value(),
        shared_quad_state_list.ElementAt(sqs_index), t_resources };
}
...
std::optional<ContentDrawQuadCommon> GetContentDrawQuadCommonFromDict(const base::Value::Dict& dict)
{
    ...
    std::optional<bool> force_anti_aliasing_off = dict.FindBool("force_anti_aliasing_off");

    if (!tex_coord_rect || !texture_size || !is_premultiplied || !nearest_neighbor || !force_anti_aliasing_off) {
        return std::nullopt;
    }
    gfx::RectF t_tex_coord_rect;
    gfx::Size t_texture_size;
    if (!RectFFromDict(*tex_coord_rect, &t_tex_coord_rect) || !SizeFromDict(*texture_size, &t_texture_size)) {
        return std::nullopt;
    }

    return ContentDrawQuadCommon { t_tex_coord_rect, t_texture_size, is_premultiplied.value(), nearest_neighbor.value(), force_anti_aliasing_off.value() };
}

void CompositorRenderPassDrawQuadToDict(const CompositorRenderPassDrawQuad* draw_quad, base::Value::Dict* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
    dict->Set("render_pass_id", base::NumberToString(static_cast<uint64_t>(draw_quad->render_pass_id)));
...
void SolidColorDrawQuadToDict(const SolidColorDrawQuad* draw_quad, base::Value::Dict* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
...
void SurfaceDrawQuadToDict(const SurfaceDrawQuad* draw_quad, base::Value::Dict* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
    dict->Set("surface_range", SurfaceRangeToDict(draw_quad->surface_range));
    dict->Set("default_background_color", SkColor4fToDict(draw_quad->default_background_color));
...
void TextureDrawQuadToDict(const TextureDrawQuad* draw_quad, base::Value::Dict* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
    ...
    dict->Set("protected_video_type", ProtectedVideoTypeToString(draw_quad->protected_video_type));
    DCHECK_EQ(1u, draw_quad->resources.count);
    dict->Set("resource_size_in_pixels", SizeToDict(draw_quad->overlay_resources.size_in_pixels));
...
void TileDrawQuadToDict(const TileDrawQuad* draw_quad, base::Value::Dict* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
...
void VideoHoleDrawQuadToDict(const VideoHoleDrawQuad* draw_quad, base::Value::Dict* dict)
{
    DCHECK(draw_quad);
    DCHECK(dict);
    dict->Set("overlay_plane_id.empty", draw_quad->overlay_plane_id.is_empty());
    if (!draw_quad->overlay_plane_id.is_empty()) {
        dict->Set("overlay_plane_id.unguessable_token", base::UnguessableTokenToValue(draw_quad->overlay_plane_id));
    }
}
...
#define WRITE_DRAW_QUAD_TYPE_FIELDS(NAME, TYPE)                                                                                                                \
    case DrawQuad::Material::NAME:                                                                                                                             \
        ...
        break;
base::Value::Dict DrawQuadToDict(const DrawQuad* draw_quad, const SharedQuadStateList& shared_quad_state_list)
{
    DCHECK(draw_quad);
    base::Value::Dict dict;
    DrawQuadCommonToDict(draw_quad, &dict, shared_quad_state_list);
    switch (draw_quad->material) {
        WRITE_DRAW_QUAD_TYPE_FIELDS(kCompositorRenderPass, CompositorRenderPassDrawQuad)
...
base::Value::List QuadListToList(const QuadList& quad_list, const SharedQuadStateList& shared_quad_state_list)
{
    base::Value::List list;
    for (size_t ii = 0; ii < quad_list.size(); ++ii) {
        list.Append(DrawQuadToDict(quad_list.ElementAt(ii), shared_quad_state_list));
    }
    return list;
}

bool CompositorRenderPassDrawQuadFromDict(const base::Value::Dict& dict, const DrawQuadCommon& common, CompositorRenderPassDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    ...
    const base::Value::Dict* mask_texture_size = dict.FindDict("mask_texture_size");
    ...
    const base::Value::Dict* tex_coord_rect = dict.FindDict("tex_coord_rect");
    std::optional<double> backdrop_filter_quality = dict.FindDouble("backdrop_filter_quality");
    std::optional<bool> force_anti_aliasing_off = dict.FindBool("force_anti_aliasing_off");
    std::optional<bool> intersects_damage_under = dict.FindBool("intersects_damage_under");

    if (!render_pass_id || !mask_uv_rect || !mask_texture_size || !filters_scale || !filters_origin || !tex_coord_rect || !backdrop_filter_quality
    ...
    if (!base::StringToUint64(*render_pass_id, &render_pass_id_as_int) || !RectFFromDict(*mask_uv_rect, &t_mask_uv_rect)
        || !SizeFromDict(*mask_texture_size, &t_mask_texture_size) || !Vector2dFFromDict(*filters_scale, &t_filters_scale)
        || !PointFFromDict(*filters_origin, &t_filters_origin) || !RectFFromDict(*tex_coord_rect, &t_tex_coord_rect)) {
        return false;
    }
    ...
    draw_quad->SetAll(common.shared_quad_state, common.rect, common.visible_rect, common.needs_blending, t_render_pass_id, mask_resource_id, t_mask_uv_rect,
        t_mask_texture_size, t_filters_scale, t_filters_origin, t_tex_coord_rect, force_anti_aliasing_off.value(), backdrop_filter_quality.value(),
        intersects_damage_under ? intersects_damage_under.value() : false);
    return true;
}

bool SolidColorDrawQuadFromDict(const base::Value::Dict& dict, const DrawQuadCommon& common, SolidColorDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    std::optional<bool> force_anti_aliasing_off = dict.FindBool("force_anti_aliasing_off");
    ...
    draw_quad->SetAll(common.shared_quad_state, common.rect, common.visible_rect, common.needs_blending, t_color, force_anti_aliasing_off.value());
    return true;
}

bool SurfaceDrawQuadFromDict(const base::Value::Dict& dict, const DrawQuadCommon& common, SurfaceDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    ...
    if (!surface_range_dict)
        return false;
    std::optional<SurfaceRange> surface_range = SurfaceRangeFromDict(*surface_range_dict);
    ...
    if (!ColorFromDict(dict, "default_background_color", &t_default_background_color)) {
        return false;
    }

    draw_quad->SetAll(common.shared_quad_state, common.rect, common.visible_rect, common.needs_blending, *surface_range, t_default_background_color,
        *stretch_content, *is_reflection, *allow_merge);
    return true;
}

bool TextureDrawQuadFromDict(const base::Value::Dict& dict, const DrawQuadCommon& common, TextureDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    if (common.resources.count != 1u)
        return false;

    std::optional<bool> premultiplied_alpha = dict.FindBool("premultiplied_alpha");
    ...
    const std::string* protected_video_type = dict.FindString("protected_video_type");
    const base::Value::Dict* resource_size_in_pixels = dict.FindDict("resource_size_in_pixels");

    if (!premultiplied_alpha || !uv_top_left || !uv_bottom_right || !vertex_opacity || !y_flipped || !nearest_neighbor || !secure_output_only
        || !protected_video_type || !resource_size_in_pixels) {
        return false;
    }
    int protected_video_type_index = StringToProtectedVideoType(*protected_video_type);
    ...
    if (!PointFFromDict(*uv_top_left, &t_uv_top_left) || !PointFFromDict(*uv_bottom_right, &t_uv_bottom_right)
        || !SizeFromDict(*resource_size_in_pixels, &t_resource_size_in_pixels) || !ColorFromDict(dict, "background_color", &t_background_color)) {
        return false;
    }

    const size_t kIndex = TextureDrawQuad::kResourceIdIndex;
    ResourceId resource_id = common.resources.ids[kIndex];
    draw_quad->SetAll(common.shared_quad_state, common.rect, common.visible_rect, common.needs_blending, resource_id, t_resource_size_in_pixels,
        premultiplied_alpha.value(), t_uv_top_left, t_uv_bottom_right, t_background_color, y_flipped.value(), nearest_neighbor.value(),
        secure_output_only.value(), static_cast<gfx::ProtectedVideoType>(protected_video_type_index));
...
bool TileDrawQuadFromDict(const base::Value::Dict& dict, const DrawQuadCommon& common, TileDrawQuad* draw_quad)
{
    DCHECK(draw_quad);
    if (common.resources.count != 1u)
        return false;

    std::optional<ContentDrawQuadCommon> content_common = GetContentDrawQuadCommonFromDict(dict);
    if (!content_common)
        return false;
    ...
    draw_quad->SetAll(common.shared_quad_state, common.rect, common.visible_rect, common.needs_blending, resource_id, content_common->tex_coord_rect,
        content_common->texture_size, content_common->is_premultiplied, content_common->nearest_neighbor, content_common->force_anti_aliasing_off);
    return true;
}

bool VideoHoleDrawQuadFromDict(const base::Value::Dict& dict, const DrawQuadCommon& common, VideoHoleDrawQuad* draw_quad)
{
    DCHECK(draw_quad);

    std::optional<bool> overlay_plane_id_empty = dict.FindBool("overlay_plane_id.empty");
    ...
    if (!overlay_plane_id_empty.value()) {
        std::optional<base::UnguessableToken> deserialized_overlay_plane_id = base::ValueToUnguessableToken(dict.Find("overlay_plane_id.unguessable_token"));
        if (!deserialized_overlay_plane_id) {
            return false;
        }
        overlay_plane_id = deserialized_overlay_plane_id.value();
    }
    draw_quad->SetAll(common.shared_quad_state, common.rect, common.visible_rect, common.needs_blending, overlay_plane_id);
    return true;
}
...
bool QuadListFromList(const base::Value::List& list, QuadList* quad_list, const SharedQuadStateList& shared_quad_state_list)
{
    DCHECK(quad_list);
    size_t size = list.size();
    if (size == 0) {
    ...
    for (size_t ii = 0; ii < size; ++ii) {
        if (!list[ii].is_dict())
            return false;
        std::optional<DrawQuadCommon> common = GetDrawQuadCommonFromDict(list[ii].GetDict(), shared_quad_state_list);
        if (!common)
            return false;
...
{
    auto dict = base::Value::Dict()
                    .Set("quad_to_target_transform", TransformToList(sqs.quad_to_target_transform))
                    .Set("quad_layer_rect", RectToDict(sqs.quad_layer_rect))
                    .Set("visible_quad_layer_rect", RectToDict(sqs.visible_quad_layer_rect))
...
bool SharedQuadStateFromDict(const base::Value::Dict& dict, SharedQuadState* sqs)
{
    DCHECK(sqs);
    const base::Value::List* quad_to_target_transform = dict.FindList("quad_to_target_transform");
    const base::Value::Dict* quad_layer_rect = dict.FindDict("quad_layer_rect");
    const base::Value::Dict* visible_quad_layer_rect = dict.FindDict("visible_quad_layer_rect");
    ...
    std::optional<bool> are_contents_opaque = dict.FindBool("are_contents_opaque");
    ...
    std::optional<bool> is_fast_rounded_corner = dict.FindBool("is_fast_rounded_corner");

    if (!quad_to_target_transform || !quad_layer_rect || !visible_quad_layer_rect || !are_contents_opaque || !opacity || !blend_mode || !sorting_context_id
    ...
    if (!TransformFromList(*quad_to_target_transform, &t_quad_to_target_transform) || !RectFromDict(*quad_layer_rect, &t_quad_layer_rect)
        || !RectFromDict(*visible_quad_layer_rect, &t_visible_quad_layer_rect) || (clip_rect && !RectFromDict(*clip_rect, &t_clip_rect))) {
        return false;
    }

    gfx::MaskFilterInfo t_mask_filter_info;
    if (mask_filter_info && !MaskFilterInfoFromDict(*mask_filter_info, &t_mask_filter_info)) {
        return false;
    }
    ...
    sqs->SetAll(t_quad_to_target_transform, t_quad_layer_rect, t_visible_quad_layer_rect, t_mask_filter_info, clip_rect_opt, are_contents_opaque.value(),
        static_cast<float>(opacity.value()), t_blend_mode, sorting_context_id.value(), /*layer_id=*/0u, is_fast_rounded_corner.value());
    return true;
}

base::Value::List SharedQuadStateListToList(const SharedQuadStateList& shared_quad_state_list)
{
    base::Value::List list;
...
bool SharedQuadStateListFromList(const base::Value::List& list, SharedQuadStateList* shared_quad_state_list)
{
    DCHECK(shared_quad_state_list);
    size_t size = list.size();
    SharedQuadStateList states(alignof(SharedQuadState), sizeof(SharedQuadState), size);
    for (size_t ii = 0; ii < size; ++ii) {
        if (!list[ii].is_dict())
            return false;
...
base::Value::Dict GetRenderPassMetadata(const CompositorRenderPass& render_pass)
{
    return base::Value::Dict()
        .Set("render_pass_id", base::NumberToString(static_cast<uint64_t>(render_pass.id)))
        .Set("quad_count", static_cast<int>(render_pass.quad_list.size()))
        .Set("shared_quad_state_count", static_cast<int>(render_pass.shared_quad_state_list.size()));
}

base::Value::List GetRenderPassListMetadata(const CompositorRenderPassList& render_pass_list)
{
...
base::Value::Dict CompositorRenderPassToDict(const CompositorRenderPass& render_pass)
{
    base::Value::Dict dict;
    ...
    if (ProcessRenderPassField(kRenderPassTransformToRootTarget)) {
        dict.Set("transform_to_root_target", TransformToList(render_pass.transform_to_root_target));
    }
    ...
    if (ProcessRenderPassField(kRenderPassBackdropFilters)) {
        dict.Set("backdrop_filters", FilterOperationsToList(render_pass.backdrop_filters));
    }
    if (ProcessRenderPassField(kRenderPassBackdropFilterBounds) && render_pass.backdrop_filter_bounds) {
        dict.Set("backdrop_filter_bounds", RRectFToDict(render_pass.backdrop_filter_bounds.value()));
    }
    ...
    if (ProcessRenderPassField(kRenderPassHasTransparentBackground)) {
        dict.Set("has_transparent_background", render_pass.has_transparent_background);
    }
    ...
    if (ProcessRenderPassField(kRenderPassHasDamageFromContributingContent)) {
        dict.Set("has_damage_from_contributing_content", render_pass.has_damage_from_contributing_content);
    }
    ...
    if (ProcessRenderPassField(kRenderPassQuadList)) {
        dict.Set("quad_list", QuadListToList(render_pass.quad_list, render_pass.shared_quad_state_list));
    }
    if (ProcessRenderPassField(kRenderPassSharedQuadStateList)) {
        dict.Set("shared_quad_state_list", SharedQuadStateListToList(render_pass.shared_quad_state_list));
    }
    return dict;
}

std::unique_ptr<CompositorRenderPass> CompositorRenderPassFromDict(const base::Value::Dict& dict)
{
    ...
    if (ProcessRenderPassField(kRenderPassTransformToRootTarget)) {
        const base::Value::List* transform_to_root_target = dict.FindList("transform_to_root_target");
        if (!transform_to_root_target)
            return nullptr;
        if (!TransformFromList(*transform_to_root_target, &(pass->transform_to_root_target))) {
            return nullptr;
        }
    }
    ...
    if (ProcessRenderPassField(kRenderPassBackdropFilters)) {
        const base::Value::List* backdrop_filters = dict.FindList("backdrop_filters");
        if (!backdrop_filters)
            return nullptr;
        if (!FilterOperationsFromList(*backdrop_filters, &(pass->backdrop_filters))) {
            return nullptr;
        }
    }

    if (ProcessRenderPassField(kRenderPassBackdropFilterBounds)) {
        const base::Value::Dict* backdrop_filter_bounds = dict.FindDict("backdrop_filter_bounds");
    ...
    if (ProcessRenderPassField(kRenderPassHasTransparentBackground)) {
        const std::optional<bool> has_transparent_background = dict.FindBool("has_transparent_background");
    ...
    if (ProcessRenderPassField(kRenderPassCacheRenderPass)) {
        const std::optional<bool> cache_render_pass = dict.FindBool("cache_render_pass");
    ...
    if (ProcessRenderPassField(kRenderPassHasPreQuadDamage)) {
        const std::optional<bool> has_per_quad_damage = dict.FindBool("has_per_quad_damage");
        if (has_per_quad_damage)
            pass->has_per_quad_damage = has_per_quad_damage.value();
    }

    if (ProcessRenderPassField(kRenderPassHasDamageFromContributingContent)) {
        const std::optional<bool> has_damage_from_contributing_content = dict.FindBool("has_damage_from_contributing_content");
        if (!has_damage_from_contributing_content)
            return nullptr;
        pass->has_damage_from_contributing_content = has_damage_from_contributing_content.value();
    }

    if (ProcessRenderPassField(kRenderPassGenerateMipmap)) {
        const std::optional<bool> generate_mipmap = dict.FindBool("generate_mipmap");
        if (!generate_mipmap)
            return nullptr;
    ...
    if (ProcessRenderPassField(kRenderPassSharedQuadStateList)) {
        const base::Value::List* shared_quad_state_list = dict.FindList("shared_quad_state_list");
        if (!shared_quad_state_list)
            return nullptr;
        if (!SharedQuadStateListFromList(*shared_quad_state_list, &(pass->shared_quad_state_list))) {
            return nullptr;
        }
    }
    ...
    if (ProcessRenderPassField(kRenderPassQuadList)) {
        ...
        if (!quad_list)
            return nullptr;
        if (!QuadListFromList(*quad_list, &(pass->quad_list), pass->shared_quad_state_list)) {
            return nullptr;
        }
    }
...
base::Value::Dict CompositorRenderPassListToDict(const CompositorRenderPassList& render_pass_list)
{
    base::Value::List list;
...
bool CompositorRenderPassListFromDict(const base::Value::Dict& dict, CompositorRenderPassList* render_pass_list)
{
    ...
    for (const auto& item : *list) {
        ...
        std::unique_ptr<CompositorRenderPass> render_pass = CompositorRenderPassFromDict(*item_dict);
        if (!render_pass) {
            render_pass_list->clear();
            return false;
        }
...
base::Value::Dict CompositorFrameToDict(const CompositorFrame& compositor_frame)
{
    ...
    return base::Value::Dict()
        .Set("render_pass_list", CompositorRenderPassListToDict(compositor_frame.render_pass_list))
        .Set("metadata", base::Value::Dict().Set("referenced_surfaces", std::move(referenced_surfaces)));
}

bool CompositorFrameFromDict(const base::Value::Dict& dict, CompositorFrame* compositor_frame)
{
    ...
    if (!CompositorRenderPassListFromDict(*render_pass_list, &compositor_frame->render_pass_list)) {
        return false;
    }
    ...
    const base::Value::List* referenced_surfaces = metadata->FindList("referenced_surfaces");
    if (!referenced_surfaces) {
        return false;
    }
    for (auto& referenced_surface_dict : *referenced_surfaces) {
        auto referenced_surface = SurfaceRangeFromDict(referenced_surface_dict.GetDict());
        if (!referenced_surface) {
            return false;
        }
        compositor_frame->metadata.referenced_surfaces.push_back(*referenced_surface);
    }

    return true;
}

base::Value::List FrameDataToList(const std::vector<FrameData>& frame_data_list)
{
    base::Value::List list;
    ...
    for (auto& frame_data : frame_data_list) {
        list.Append(base::Value::Dict()
                        ...
                        .Set("compositor_frame", CompositorFrameToDict(frame_data.compositor_frame)));
    }
    return list;
}

bool FrameDataFromList(const base::Value::List& list, std::vector<FrameData>* frame_data_list)
{
    ...
    for (const auto& frame_data_value : list) {
        ...
        const base::Value::Dict* surface_id_dict = frame_data_dict->FindDict("surface_id");
        if (!surface_id_dict) {
            return false;
        }
        ...
        const base::Value::Dict* compositor_frame_dict = frame_data_dict->FindDict("compositor_frame");
        if (!compositor_frame_dict) {
            return false;
        }
        if (!CompositorFrameFromDict(*compositor_frame_dict, &frame_data.compositor_frame)) {
            return false;
        }
...
FrameData::FrameData(const SurfaceId& surface_id, const uint64_t frame_index, const CompositorFrame& compositor_frame)
