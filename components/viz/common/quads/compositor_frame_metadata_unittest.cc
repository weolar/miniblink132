...
{
    return a.frame_id == b.frame_id && a.trace_id == b.trace_id && a.has_damage == b.has_damage && a.preferred_frame_interval == b.preferred_frame_interval;
}

bool AreLatencyInfosEqual(const ui::LatencyInfo& a, const ui::LatencyInfo& b)
{
    return a.began() == b.began() && a.terminated() == b.terminated() && a.coalesced() == b.coalesced() && a.trace_id() == b.trace_id()
        && a.gesture_scroll_id() == b.gesture_scroll_id();
}

bool AreDelegatedInkMetadataEqual(const gfx::DelegatedInkMetadata& a, const gfx::DelegatedInkMetadata& b)
{
    return a.point() == b.point() && a.diameter() == b.diameter() && a.color() == b.color() && a.timestamp() == b.timestamp()
        && a.presentation_area() == b.presentation_area() && a.frame_time() == b.frame_time() && a.is_hovering() == b.is_hovering();
}

bool AreTransitionDirectivesEqual(const CompositorFrameTransitionDirective& a, const CompositorFrameTransitionDirective& b)
{
...
TEST(CompositorFrameMetadata, Clone)
{
    ...
    metadata.referenced_surfaces.emplace_back(SurfaceId(frame_sink_id, local_id1), SurfaceId(frame_sink_id, local_id2));
    metadata.activation_dependencies.emplace_back(SurfaceId(frame_sink_id, local_id1));
    metadata.deadline = FrameDeadline(base::TimeTicks() + base::Seconds(123), 15, base::Milliseconds(16), true);
    metadata.begin_frame_ack = BeginFrameAck(999, 888, true, 777);
    metadata.begin_frame_ack.preferred_frame_interval.emplace(base::Milliseconds(11));
    ...
    metadata.delegated_ink_metadata = std::make_unique<gfx::DelegatedInkMetadata>(
        gfx::PointF(88.8, 44.4), 1.f, SK_ColorRED, base::TimeTicks() + base::Seconds(125), gfx::RectF(1, 2, 3, 4), true);
    metadata.transition_directives.emplace_back(
        CompositorFrameTransitionDirective::CreateSave(blink::ViewTransitionToken(), /*maybe_cross_frame_sink=*/false, 4u, {}, {}));
    ...
    EXPECT_TRUE(AreLatencyInfosEqual(clone.latency_info[0], metadata.latency_info[0]));
    ...
    EXPECT_TRUE(AreBeginFrameAcksEqual(clone.begin_frame_ack, metadata.begin_frame_ack));

    EXPECT_EQ(clone.frame_token, metadata.frame_token);
    EXPECT_EQ(clone.send_frame_token_to_embedder, metadata.send_frame_token_to_embedder);
    EXPECT_FLOAT_EQ(clone.min_page_scale_factor, metadata.min_page_scale_factor);
    EXPECT_EQ(clone.top_controls_visible_height, metadata.top_controls_visible_height);
    EXPECT_FLOAT_EQ(*clone.top_controls_visible_height, *metadata.top_controls_visible_height);
    ...
    EXPECT_TRUE(AreDelegatedInkMetadataEqual(*clone.delegated_ink_metadata, *metadata.delegated_ink_metadata));

    EXPECT_EQ(clone.transition_directives.size(), metadata.transition_directives.size());
    EXPECT_TRUE(AreTransitionDirectivesEqual(clone.transition_directives[0], metadata.transition_directives[0]));
}
