...
public:
    ...
    bool IsDebugQuad() const
    {
        return material == Material::kDebugBorder;
    }

    bool ShouldDrawWithBlendingForReasonOtherThanMaskFilter() const
    {
        return needs_blending || shared_quad_state->opacity < 1.0f || shared_quad_state->blend_mode != SkBlendMode::kSrcOver;
    }

    bool ShouldDrawWithBlending() const
    {
        return ShouldDrawWithBlendingForReasonOtherThanMaskFilter() || !shared_quad_state->mask_filter_info.IsEmpty();
    }
    ...
    {
        return this->material == T::kMaterial ? static_cast<const T*>(this) : nullptr;
    }

protected:
    DrawQuad();

    void SetAll(const SharedQuadState* quad_state, Material m, const gfx::Rect& r, const gfx::Rect& visible_r, bool blending);
