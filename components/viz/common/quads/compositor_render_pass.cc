...
std::unique_ptr<CompositorRenderPass> CompositorRenderPass::Create(size_t num_layers)
{
    return base::WrapUnique(new CompositorRenderPass(num_layers));
}

std::unique_ptr<CompositorRenderPass> CompositorRenderPass::Create(size_t shared_quad_state_list_size, size_t quad_list_size)
{
    return base::WrapUnique(new CompositorRenderPass(shared_quad_state_list_size, quad_list_size));
}
...
CompositorRenderPass::CompositorRenderPass(size_t shared_quad_state_list_size, size_t quad_list_size)
...
{
    TRACE_EVENT_OBJECT_DELETED_WITH_ID(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "CompositorRenderPass", reinterpret_cast<void*>(static_cast<uint64_t>(id)));
}

void CompositorRenderPass::SetNew(
    CompositorRenderPassId pass_id, const gfx::Rect& output_rect, const gfx::Rect& damage_rect, const gfx::Transform& transform_to_root_target)
{
    DCHECK(pass_id);
...
void CompositorRenderPass::SetAll(CompositorRenderPassId pass_id, const gfx::Rect& output_rect, const gfx::Rect& damage_rect,
    ...
    const std::optional<gfx::RRectF>& backdrop_filter_bounds, SubtreeCaptureId capture_id, gfx::Size subtree_capture_size,
    ViewTransitionElementResourceId resource_id, bool has_transparent_background, bool cache_render_pass, bool has_damage_from_contributing_content,
    bool generate_mipmap, bool per_quad_damage)
{
    DCHECK(pass_id);
...
void CompositorRenderPass::AsValueInto(base::trace_event::TracedValue* value) const
{
    ...
    TracedValue::MakeDictIntoImplicitSnapshotWithCategory(
        TRACE_DISABLED_BY_DEFAULT("viz.quads"), value, "CompositorRenderPass", reinterpret_cast<void*>(static_cast<uint64_t>(id)));
}

CompositorRenderPassDrawQuad* CompositorRenderPass::CopyFromAndAppendRenderPassDrawQuad(
    const CompositorRenderPassDrawQuad* quad, CompositorRenderPassId render_pass_id)
{
    DCHECK(!shared_quad_state_list.empty());
...
DrawQuad* CompositorRenderPass::CopyFromAndAppendDrawQuad(const DrawQuad* quad)
{
    DCHECK(!shared_quad_state_list.empty());
...
std::unique_ptr<CompositorRenderPass> CompositorRenderPass::DeepCopy() const
{
    ...
    auto copy_pass = CompositorRenderPass::Create(shared_quad_state_list.size(), quad_list.size());
    copy_pass->SetAll(id, output_rect, damage_rect, transform_to_root_target, filters, backdrop_filters, backdrop_filter_bounds, subtree_capture_id,
        subtree_size, view_transition_element_resource_id, has_transparent_background, cache_render_pass, has_damage_from_contributing_content, generate_mipmap,
        has_per_quad_damage);
