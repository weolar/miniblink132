...
static void CompareRenderPassLists(const CompositorRenderPassList& expected_list, const CompositorRenderPassList& actual_list)
{
...
TEST(CompositorRenderPassTest, AggregatedCopyShouldBeIdenticalExceptIdAndQuads)
{
    AggregatedRenderPassId render_pass_id { 3u };
    gfx::Rect output_rect(45, 22, 120, 13);
    gfx::Transform transform_to_root = gfx::Transform::Affine(1.0, 0.5, 0.5, -0.5, -1.0, 0.0);
    ...
    std::optional<gfx::RRectF> backdrop_filter_bounds({ 10, 20, 130, 140, 1, 2, 3, 4, 5, 6, 7, 8 });
    ...
    pass->SetAll(render_pass_id, output_rect, damage_rect, transform_to_root, filters, backdrop_filters, backdrop_filter_bounds, content_color_usage,
        has_transparent_background, cache_render_pass, has_damage_from_contributing_content, generate_mipmap);
    pass->copy_requests.push_back(CopyOutputRequest::CreateStubForTesting());

    // Stick a quad in the pass, this should not get copied.
    SharedQuadState* shared_state = pass->CreateAndAppendSharedQuadState();
    shared_state->SetAll(gfx::Transform(), gfx::Rect(), gfx::Rect(), gfx::MaskFilterInfo(), /*clip=*/std::nullopt,
        /*contents_opaque=*/false, /*opacity_f=*/1, SkBlendMode::kSrcOver, /*sorting_context=*/0,
        /*layer_id=*/0u, /*fast_rounded_corner=*/false);

    auto* color_quad = pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
    color_quad->SetNew(pass->shared_quad_state_list.back(), gfx::Rect(), gfx::Rect(), SkColor4f(), false);
    ...
    EXPECT_TRUE(pass->backdrop_filter_bounds->ApproximatelyEqual(copy->backdrop_filter_bounds.value(), 0.001));
    ...
    EXPECT_EQ(pass->has_damage_from_contributing_content, copy->has_damage_from_contributing_content);
...
TEST(CompositorRenderPassTest, CopyAllShouldBeIdentical)
{
    ...
    pass->SetAll(id, output_rect, damage_rect, transform_to_root, filters, backdrop_filters, backdrop_filter_bounds, SubtreeCaptureId(base::Token(0u, 1u)),
        output_rect.size(), ViewTransitionElementResourceId(), has_transparent_background, cache_render_pass, has_damage_from_contributing_content,
    ...
    shared_state1->SetAll(gfx::Transform(), gfx::Rect(0, 0, 1, 1), gfx::Rect(), gfx::MaskFilterInfo(), /*clip=*/std::nullopt,
        /*contents_opaque=*/false, /*opacity_f=*/1, SkBlendMode::kSrcOver, /*sorting_context=*/0,
        /*layer_id=*/0u, /*fast_rounded_corner=*/false);

    auto* color_quad1 = pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
    color_quad1->SetNew(pass->shared_quad_state_list.back(), gfx::Rect(1, 1, 1, 1), gfx::Rect(1, 1, 1, 1), SkColor4f(), false);

    auto* color_quad2 = pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
    color_quad2->SetNew(pass->shared_quad_state_list.back(), gfx::Rect(2, 2, 2, 2), gfx::Rect(2, 2, 2, 2), SkColor4f(), false);

    // And two quads using another shared state.
    SharedQuadState* shared_state2 = pass->CreateAndAppendSharedQuadState();
    shared_state2->SetAll(gfx::Transform(), gfx::Rect(0, 0, 2, 2), gfx::Rect(), gfx::MaskFilterInfo(), /*clip=*/std::nullopt,
        /*contents_opaque=*/false, /*opacity_f=*/1, SkBlendMode::kSrcOver, /*sorting_context=*/0,
        /*layer_id=*/0u, /*fast_rounded_corner=*/false);

    auto* color_quad3 = pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
    color_quad3->SetNew(pass->shared_quad_state_list.back(), gfx::Rect(3, 3, 3, 3), gfx::Rect(3, 3, 3, 3), SkColor4f(), false);

    auto* color_quad4 = pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
    color_quad4->SetNew(pass->shared_quad_state_list.back(), gfx::Rect(4, 4, 4, 4), gfx::Rect(4, 4, 4, 4), SkColor4f(), false);

    // A second render pass with a quad.
    CompositorRenderPassId contrib_id { 4 };
    gfx::Rect contrib_output_rect(10, 15, 12, 17);
    gfx::Transform contrib_transform_to_root = gfx::Transform::Affine(1.0, 0.5, 0.5, -0.5, -1.0, 0.0);
    ...
    std::optional<gfx::RRectF> contrib_backdrop_filter_bounds({ 20, 30, 140, 150, 1, 2, 3, 4, 5, 6, 7, 8 });
    ...
    contrib->SetAll(contrib_id, contrib_output_rect, contrib_damage_rect, contrib_transform_to_root, contrib_filters, contrib_backdrop_filters,
        contrib_backdrop_filter_bounds, SubtreeCaptureId(base::Token(0u, 2u)), contrib_output_rect.size(), ViewTransitionElementResourceId(),
        contrib_has_transparent_background, contrib_cache_render_pass, contrib_has_damage_from_contributing_content, contrib_generate_mipmap,
        contrib_has_per_quad_damage);

    SharedQuadState* contrib_shared_state = contrib->CreateAndAppendSharedQuadState();
    contrib_shared_state->SetAll(gfx::Transform(), gfx::Rect(0, 0, 2, 2), gfx::Rect(), gfx::MaskFilterInfo(), /*clip=*/std::nullopt, /*contents_opaque=*/false,
        /*opacity_f=*/1, SkBlendMode::kSrcOver, /*sorting_context=*/0,
        /*layer_id=*/0u, /*fast_rounded_corner=*/false);

    auto* contrib_quad = contrib->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
    contrib_quad->SetNew(contrib->shared_quad_state_list.back(), gfx::Rect(3, 3, 3, 3), gfx::Rect(3, 3, 3, 3), SkColor4f(), false);

    // And a CompositorRenderPassDrawQuad for the contributing pass.
    auto pass_quad = std::make_unique<CompositorRenderPassDrawQuad>();
    pass_quad->SetNew(pass->shared_quad_state_list.back(), contrib_output_rect, contrib_output_rect, contrib_id, ResourceId(1u), gfx::RectF(), gfx::Size(),
        gfx::Vector2dF(1.0f, 1.0f), gfx::PointF(), gfx::RectF(), false, 1.0f);
...
TEST(CompositorRenderPassTest, CopyAllWithCulledQuads)
{
    ...
    bool has_per_quad_damage = false;
    auto pass = CompositorRenderPass::Create();
    pass->SetAll(id, output_rect, damage_rect, transform_to_root, filters, backdrop_filters, backdrop_filter_bounds, SubtreeCaptureId(), output_rect.size(),
        ViewTransitionElementResourceId(), has_transparent_background, cache_render_pass, has_damage_from_contributing_content, generate_mipmap,
        has_per_quad_damage);
    ...
    shared_state3->SetAll(gfx::Transform(), gfx::Rect(0, 0, 2, 2), gfx::Rect(), gfx::MaskFilterInfo(), /*clip=*/std::nullopt,
        /*contents_opaque=*/false, /*opacity_f=*/1, SkBlendMode::kSrcOver, /*sorting_context=*/0,
        /*layer_id=*/0u, /*fast_rounded_corner=*/false);

    // A last shared state with a quad again.
    SharedQuadState* shared_state4 = pass->CreateAndAppendSharedQuadState();
    shared_state4->SetAll(gfx::Transform(), gfx::Rect(0, 0, 2, 2), gfx::Rect(), gfx::MaskFilterInfo(), /*clip=*/std::nullopt,
        /*contents_opaque=*/false, /*opacity_f=*/1, SkBlendMode::kSrcOver, /*sorting_context=*/0,
        /*layer_id=*/0u, /*fast_rounded_corner=*/false);

    auto* color_quad2 = pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
    color_quad2->SetNew(pass->shared_quad_state_list.back(), gfx::Rect(3, 3, 3, 3), gfx::Rect(3, 3, 3, 3), SkColor4f(), false);
...
TEST(CompositorRenderPassTest, ReplacedQuadsShouldntMove)
{
    ...
    pass->ReplaceExistingQuadWithSolidColor(pass->quad_list.begin(), SkColor4f(), SkBlendMode::kSrcOver);
...
TEST(CompositorRenderPassTest, ReplacedQuadsGetColor)
{
    ...
    pass->ReplaceExistingQuadWithSolidColor(pass->quad_list.begin(), SkColors::kGreen, SkBlendMode::kSrcOver);
...
TEST(CompositorRenderPassTest, ReplacedQuadsGetBlendMode)
{
    ...
    const gfx::Rect quad_rect(1, 2, 3, 4);
    quad->SetNew(quad_state, quad_rect, quad_rect, SkColor4f(), false);
    pass->ReplaceExistingQuadWithSolidColor(pass->quad_list.begin(), SkColor4f(), SkBlendMode::kDstOut);
    EXPECT_EQ(SkBlendMode::kDstOut, quad->shared_quad_state->blend_mode);
}

TEST(CompositorRenderPassTest, ReplacedQuadsKeepOldSharedQuadStateWhenPossible)
{
    auto pass = CompositorRenderPass::Create();
    SharedQuadState* quad_state = pass->CreateAndAppendSharedQuadState();
    quad_state->are_contents_opaque = false;
    ...
    const gfx::Rect quad_rect(1, 2, 3, 4);
    quad->SetNew(quad_state, quad_rect, quad_rect, SkColors::kRed, false);
    pass->ReplaceExistingQuadWithSolidColor(pass->quad_list.begin(), SkColors::kGreen, SkBlendMode::kSoftLight);
...
TEST(CompositorRenderPassTest, HolePunchedQuadsShouldntBeOpaque)
{
    ...
    pass->ReplaceExistingQuadWithHolePunch(pass->quad_list.begin(), &quad_was_opaque);
    EXPECT_TRUE(quad_was_opaque);
    EXPECT_FALSE(pass->quad_list.begin()->shared_quad_state->are_contents_opaque);
    EXPECT_EQ(SkBlendMode::kSrcOver, quad->shared_quad_state->blend_mode);
    EXPECT_EQ(SolidColorDrawQuad::MaterialCast(quad)->color, SkColors::kTransparent);
}
