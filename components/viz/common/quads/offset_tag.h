...
public:
    ...
    constexpr explicit OffsetTag(const base::Token& token)
        : token_(token)
    {
    }
    ...
    friend constexpr bool operator==(const OffsetTag& lhs, const OffsetTag& rhs) = default;
    friend constexpr auto operator<=>(const OffsetTag& lhs, const OffsetTag& rhs) = default;
...
struct VIZ_COMMON_EXPORT OffsetTagDefinition {
    OffsetTagDefinition();
    OffsetTagDefinition(const OffsetTag& tag, const SurfaceRange& provider, const OffsetTagConstraints& constraints);
