...
RenderPassInternal::RenderPassInternal()
    : RenderPassInternal(kDefaultNumSharedQuadStatesToReserve, kDefaultNumQuadsToReserve)
{
}
...
RenderPassInternal::RenderPassInternal(size_t shared_quad_state_list_size, size_t quad_list_size)
    : quad_list(quad_list_size)
    , shared_quad_state_list(alignof(SharedQuadState), sizeof(SharedQuadState), shared_quad_state_list_size)
{
}
...
void RenderPassInternal::ReplaceExistingQuadWithSolidColor(QuadList::Iterator at, SkColor4f color, SkBlendMode blend_mode)
{
    const SharedQuadState* shared_quad_state = at->shared_quad_state;
    if (shared_quad_state->are_contents_opaque || shared_quad_state->blend_mode != blend_mode) {
        auto* new_shared_quad_state = shared_quad_state_list.AllocateAndCopyFrom(shared_quad_state);
    ...
    quad_list.ReplaceExistingElement<SolidColorDrawQuad>(at)->SetAll(shared_quad_state, rect, /*visible_rect=*/rect,
        /*needs_blending=*/false, color,
        /*force_anti_aliasing_off=*/true);
}

void RenderPassInternal::ReplaceExistingQuadWithHolePunch(QuadList::Iterator quad, bool* quad_was_opaque)
{
    ...
    if (quad->ShouldDrawWithBlending() && quad->shared_quad_state->blend_mode == SkBlendMode::kSrcOver) {
        ReplaceExistingQuadWithSolidColor(quad, SkColors::kBlack, SkBlendMode::kDstOut);
        if (quad_was_opaque) {
        ...
        // layer.
        ReplaceExistingQuadWithSolidColor(quad, SkColors::kTransparent, SkBlendMode::kSrcOver);
        if (quad_was_opaque) {
...
void RenderPassInternal::AsValueInto(base::trace_event::TracedValue* value) const
{
    ...
    value->SetBoolean("has_damage_from_contributing_content", has_damage_from_contributing_content);
    value->SetBoolean("generate_mipmap", generate_mipmap);
    value->SetInteger("copy_requests", base::saturated_cast<int>(copy_requests.size()));
    ...
    if (backdrop_filter_bounds.has_value()) {
        cc::MathUtil::AddToTracedValue("backdrop_filter_bounds", backdrop_filter_bounds.value(), value);
    }
