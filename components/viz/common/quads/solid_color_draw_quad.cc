...
void SolidColorDrawQuad::SetNew(
    const SharedQuadState* shared_quad_state, const gfx::Rect& rect, const gfx::Rect& visible_rect, SkColor4f c, bool anti_aliasing_off)
{
    // Clamp the alpha component of the color to the range of [0, 1].
    c.fA = std::clamp(c.fA, 0.0f, 1.0f);

    bool needs_blending = !c.isOpaque();
    DrawQuad::SetAll(shared_quad_state, DrawQuad::Material::kSolidColor, rect, visible_rect, needs_blending);
    color = c;
    force_anti_aliasing_off = anti_aliasing_off;
}

void SolidColorDrawQuad::SetAll(
    const SharedQuadState* shared_quad_state, const gfx::Rect& rect, const gfx::Rect& visible_rect, bool needs_blending, SkColor4f c, bool anti_aliasing_off)
{
    DrawQuad::SetAll(shared_quad_state, DrawQuad::Material::kSolidColor, rect, visible_rect, needs_blending);
    color = c;
    force_anti_aliasing_off = anti_aliasing_off;
...
void SolidColorDrawQuad::ExtendValue(base::trace_event::TracedValue* value) const
{
