...
{
    gfx::Transform quad_transform = gfx::Transform::Affine(1.0, 0.5, 0.0, 1.0, 0.5, 0.0);
    ...
    state->SetAll(quad_transform, content_rect, visible_layer_rect, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity, blend_mode,
        sorting_context_id,
...
public:
    ...
    void CreateRenderPass()
    {
        ...
        SharedQuadState* new_shared_state(CreateSharedQuadState(render_pass_.get()));
    ...
    void GenerateTextureDrawQuads(int count, std::vector<DrawQuad*>* quads)
    {
        for (int i = 0; i < count; ++i) {
            ...
            quad->SetNew(shared_state_, rect, rect, needs_blending, resource_id, premultiplied_alpha, uv_top_left, uv_bottom_right, background_color, y_flipped,
                nearest_neighbor,
