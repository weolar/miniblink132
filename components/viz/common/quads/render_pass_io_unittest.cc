...
TEST(RenderPassIOTest, FilterOperations)
{
    ...
    {
        // Add two filters.
        cc::FilterOperation grayscale = cc::FilterOperation::CreateGrayscaleFilter(0.25f);
        render_pass0->filters.Append(grayscale);
        cc::FilterOperation opacity = cc::FilterOperation::CreateOpacityFilter(0.8f);
        render_pass0->filters.Append(opacity);
    }
    {
        // Add three backdrop filters.
        cc::FilterOperation drop_shadow = cc::FilterOperation::CreateDropShadowFilter(gfx::Point(1.0f, 2.0f), 0.8f, SkColors::kYellow);
    ...
    EXPECT_TRUE(render_pass1);
    {
        ...
        EXPECT_EQ(cc::FilterOperation::GRAYSCALE, render_pass1->filters.at(0).type());
    ...
    }
    {
        ...
        EXPECT_EQ(cc::FilterOperation::DROP_SHADOW, render_pass1->backdrop_filters.at(0).type());
        EXPECT_EQ(0.8f, render_pass1->backdrop_filters.at(0).amount());
        EXPECT_EQ(SkColors::kYellow, render_pass1->backdrop_filters.at(0).drop_shadow_color());
        EXPECT_EQ(gfx::Point(1.0f, 2.0f), render_pass1->backdrop_filters.at(0).offset());
        EXPECT_EQ(cc::FilterOperation::INVERT, render_pass1->backdrop_filters.at(1).type());
        EXPECT_EQ(0.64f, render_pass1->backdrop_filters.at(1).amount());
        EXPECT_EQ(cc::FilterOperation::ZOOM, render_pass1->backdrop_filters.at(2).type());
    ...
    }
    {
        ...
        EXPECT_TRUE(render_pass0->backdrop_filter_bounds->Equals(render_pass1->backdrop_filter_bounds.value()));
        EXPECT_EQ(gfx::RRectF::Type::kSingle, render_pass1->backdrop_filter_bounds->GetType());
        EXPECT_EQ(1.5f, render_pass1->backdrop_filter_bounds->GetSimpleRadius());
        EXPECT_EQ(gfx::RectF(2.f, 3.f, 4.f, 5.f), render_pass1->backdrop_filter_bounds->rect());
    }
    base::Value::Dict dict1 = CompositorRenderPassToDict(*render_pass1);
    EXPECT_EQ(dict0, dict1);
}
...
TEST(RenderPassIOTest, SharedQuadStateList)
{
    auto render_pass0 = CompositorRenderPass::Create();
    {
        ...
        sqs1->SetAll(transform, gfx::Rect(0, 0, 640, 480), gfx::Rect(10, 10, 600, 400),
            gfx::MaskFilterInfo(gfx::RRectF(gfx::RectF(2.f, 3.f, 4.f, 5.f), 1.5f), gradient_mask), gfx::Rect(5, 20, 1000, 200), /*contents_opaque=*/false,
    ...
    ASSERT_TRUE(render_pass1);
    {
        ...
        const SharedQuadState* sqs0 = render_pass1->shared_quad_state_list.ElementAt(0);
        EXPECT_TRUE(sqs0);
        ...
        const SharedQuadState* sqs1 = render_pass1->shared_quad_state_list.ElementAt(1);
        EXPECT_TRUE(sqs1);
        ...
        EXPECT_EQ(gfx::RRectF::Type::kSingle, sqs1->mask_filter_info.rounded_corner_bounds().GetType());
        EXPECT_EQ(1.5f, sqs1->mask_filter_info.rounded_corner_bounds().GetSimpleRadius());
        ...
        EXPECT_EQ(gfx::LinearGradient::Step({ 0, 0 }), sqs1->mask_filter_info.gradient_mask()->steps()[0]);
        EXPECT_EQ(gfx::LinearGradient::Step({ 1, 255 }), sqs1->mask_filter_info.gradient_mask()->steps()[1]);
...
TEST(RenderPassIOTest, QuadList)
{
    ...
    auto render_pass0 = CompositorRenderPass::Create();
    {
        ...
        // Add to quad_list.
        {
            ...
            SolidColorDrawQuad* quad = render_pass0->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
            quad->SetAll(
                render_pass0->shared_quad_state_list.ElementAt(sqs_index), gfx::Rect(0, 0, 30, 40), gfx::Rect(1, 2, 20, 30), true, SkColors::kRed, false);
            ++quad_count;
        }
        {
            // 2. TextureDrawQuad with is_stream_video set to true.
            TextureDrawQuad* quad = render_pass0->CreateAndAppendDrawQuad<TextureDrawQuad>();
            quad->SetAll(render_pass0->shared_quad_state_list.ElementAt(sqs_index), gfx::Rect(10, 10, 300, 400), gfx::Rect(10, 10, 200, 400), false,
                ResourceId(100), gfx::Size(600, 800), false, gfx::PointF(0.f, 0.f), gfx::PointF(1.f, 1.f), SkColors::kTransparent, false, false, false,
        ...
        {
            // 3. VideoHoleDrawQuad
            VideoHoleDrawQuad* quad = render_pass0->CreateAndAppendDrawQuad<VideoHoleDrawQuad>();
            quad->SetAll(render_pass0->shared_quad_state_list.ElementAt(sqs_index), gfx::Rect(5, 5, 305, 405), gfx::Rect(15, 15, 205, 305), false,
        ...
        }
        {
            // 4. TextureDrawQuad
            TextureDrawQuad* quad = render_pass0->CreateAndAppendDrawQuad<TextureDrawQuad>();
            quad->SetAll(render_pass0->shared_quad_state_list.ElementAt(sqs_index), gfx::Rect(0, 0, 100, 50), gfx::Rect(0, 0, 100, 50), false, ResourceId(9u),
                gfx::Size(100, 50), false, gfx::PointF(0.f, 0.f), gfx::PointF(1.f, 1.f), SkColors::kBlue, false, true, false,
                gfx::ProtectedVideoType::kHardwareProtected);
        ...
        }
        {
            ...
            CompositorRenderPassDrawQuad* quad = render_pass0->CreateAndAppendDrawQuad<CompositorRenderPassDrawQuad>();
            quad->SetAll(render_pass0->shared_quad_state_list.ElementAt(sqs_index), gfx::Rect(2, 3, 100, 50), gfx::Rect(2, 3, 100, 50), true,
                CompositorRenderPassId { 198u }, ResourceId(81u), gfx::RectF(0.1f, 0.2f, 0.5f, 0.6f), gfx::Size(800, 600), gfx::Vector2dF(1.1f, 0.9f),
                gfx::PointF(0.01f, 0.02f), gfx::RectF(0.2f, 0.3f, 0.3f, 0.4f), true, 0.88f, true);
            ++sqs_index;
            ++quad_count;
        }
        {
            // 6. TileDrawQuad
            TileDrawQuad* quad = render_pass0->CreateAndAppendDrawQuad<TileDrawQuad>();
            quad->SetAll(render_pass0->shared_quad_state_list.ElementAt(sqs_index), gfx::Rect(0, 0, 256, 512), gfx::Rect(2, 2, 250, 500), true,
                ResourceId(512u), gfx::RectF(0.0f, 0.0f, 0.9f, 0.8f), gfx::Size(256, 512), true, true, true);
            ++quad_count;
        }
        {
            // 7. SurfaceDrawQuad
            SurfaceDrawQuad* quad = render_pass0->CreateAndAppendDrawQuad<SurfaceDrawQuad>();
            quad->SetAll(render_pass0->shared_quad_state_list.ElementAt(sqs_index), gfx::Rect(0, 0, 512, 256), gfx::Rect(2, 2, 500, 250), true,
                SurfaceRange(kSurfaceId1, kSurfaceId2), SkColors::kWhite, false, false, true);
            ++quad_count;
        }
        {
            // 8. SurfaceDrawQuad with no starting SurfaceId
            SurfaceDrawQuad* quad = render_pass0->CreateAndAppendDrawQuad<SurfaceDrawQuad>();
            quad->SetAll(render_pass0->shared_quad_state_list.ElementAt(sqs_index), gfx::Rect(10, 10, 512, 256), gfx::Rect(12, 12, 500, 250), true,
                SurfaceRange(std::nullopt, kSurfaceId1), SkColors::kBlack, true, true, false);
            ++quad_count;
        }
    ...
    for (size_t ii = 0; ii < quad_count; ++ii) {
        EXPECT_EQ(kQuadMaterials[ii], render_pass1->quad_list.ElementAt(ii)->material);
    }
    base::Value::Dict dict1 = CompositorRenderPassToDict(*render_pass1);
    EXPECT_EQ(dict0, dict1);
}
...
TEST(RenderPassIOTest, CompositorRenderPassList)
{
    ...
    base::FilePath json_path = test_data_dir.Append(FILE_PATH_LITERAL("render_pass_data"))
    ...
    EXPECT_TRUE(CompositorRenderPassListFromDict(dict0->GetDict(), &render_pass_list));
    ...
    for (auto& entry : *list) {
        ...
        for (auto& quad_entry : *quad_list) {
            if (base::Value* extra_value = quad_entry.GetDict().Find("intersects_damage_under")) {
