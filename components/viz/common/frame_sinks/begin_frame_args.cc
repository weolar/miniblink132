...
// namespace {
// perfetto::protos::pbzero::BeginFrameArgsV2::BeginFrameArgsType TypeToProtozeroEnum(BeginFrameArgs::BeginFrameArgsType type)
// {
...
{
    return (source_id == previous.source_id && sequence_number > previous.sequence_number);
}
...
PossibleDeadline::PossibleDeadline(int64_t vsync_id, base::TimeDelta latch_delta, base::TimeDelta present_delta)
    : vsync_id(vsync_id)
...
PossibleDeadline& PossibleDeadline::operator=(const PossibleDeadline& other) = default;
PossibleDeadline& PossibleDeadline::operator=(PossibleDeadline&& other) = default;
...
PossibleDeadlines& PossibleDeadlines::operator=(const PossibleDeadlines& other) = default;
PossibleDeadlines& PossibleDeadlines::operator=(PossibleDeadlines&& other) = default;
...
BeginFrameArgs::BeginFrameArgs(uint64_t source_id, uint64_t sequence_number, base::TimeTicks frame_time, base::TimeTicks deadline, base::TimeDelta interval,
...
BeginFrameArgs BeginFrameArgs::Create(BeginFrameArgs::CreationLocation location, uint64_t source_id, uint64_t sequence_number, base::TimeTicks frame_time,
    base::TimeTicks deadline, base::TimeDelta interval, BeginFrameArgs::BeginFrameArgsType type)
{
    DCHECK_NE(type, BeginFrameArgs::INVALID);
#ifdef NDEBUG
    return BeginFrameArgs(source_id, sequence_number, frame_time, deadline, interval, type);
#else
    BeginFrameArgs args = BeginFrameArgs(source_id, sequence_number, frame_time, deadline, interval, type);
    args.created_from = location;
    return args;
#endif
}

std::unique_ptr<base::trace_event::ConvertableToTraceFormat> BeginFrameArgs::AsValue() const
{
...
//     state->SetDouble("frame_time_us", frame_time.since_origin().InMicrosecondsF());
...
// }
//
// void BeginFrameArgs::AsProtozeroInto(perfetto::EventContext& ctx, perfetto::protos::pbzero::BeginFrameArgsV2* state) const
// {
...
// #ifndef NDEBUG
//     state->set_source_location_iid(base::trace_event::InternedSourceLocation::Get(&ctx, created_from));
// #endif
// }
...
BeginFrameAck::BeginFrameAck(uint64_t source_id, uint64_t sequence_number, bool has_damage, int64_t trace_id)
    : frame_id(BeginFrameId(source_id, sequence_number))
    , trace_id(trace_id)
    , has_damage(has_damage)
{
...
{
    return BeginFrameAck(BeginFrameArgs::kManualSourceId, BeginFrameArgs::kStartingFrameNumber, true);
}
