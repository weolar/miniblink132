...
// BeginFrame.
void FilterAndIssueBeginFrame(BeginFrameObserver* observer, const BeginFrameArgs& args)
{
...
bool CheckBeginFrameContinuity(BeginFrameObserver* observer, const BeginFrameArgs& args)
{
    const BeginFrameArgs& last_args = observer->LastUsedBeginFrameArgs();
    if (!last_args.IsValid() || (args.frame_time > last_args.frame_time)) {
        DCHECK(!last_args.frame_id.IsNextInSequenceTo(args.frame_id)) << "current " << args.ToString() << ", last " << last_args.ToString();
        return true;
    }
    return false;
}
...
void BeginFrameObserverBase::OnBeginFrame(const BeginFrameArgs& args)
{
    ...
    DCHECK(!last_begin_frame_args_.frame_id.IsNextInSequenceTo(args.frame_id))
        << "current " << args.ToString() << ", last " << last_begin_frame_args_.ToString();
...
void BeginFrameObserverBase::AsProtozeroInto(perfetto::EventContext& ctx, perfetto::protos::pbzero::BeginFrameObserverStateV2* state) const
{
    *(int*)1 = 1;
//     state->set_dropped_begin_frame_args(dropped_begin_frame_args_);
// 
//     last_begin_frame_args_.AsProtozeroInto(ctx, state->set_last_begin_frame_args());
}

BeginFrameArgs BeginFrameSource::BeginFrameArgsGenerator::GenerateBeginFrameArgs(
    uint64_t source_id, base::TimeTicks frame_time, base::TimeTicks deadline, base::TimeDelta vsync_interval)
{
    uint64_t sequence_number = next_sequence_number_ + EstimateTickCountsBetween(frame_time, next_expected_frame_time_, vsync_interval);
    next_expected_frame_time_ = deadline;
    next_sequence_number_ = sequence_number + 1;
    return BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, source_id, sequence_number, frame_time, deadline, vsync_interval, BeginFrameArgs::NORMAL);
}

uint64_t BeginFrameSource::BeginFrameArgsGenerator::EstimateTickCountsBetween(
    base::TimeTicks frame_time, base::TimeTicks next_expected_frame_time, base::TimeDelta vsync_interval)
{
    ...
    int ticks_since_estimated_frame_time = base::ClampFloor((frame_time + error_margin - next_expected_frame_time) / vsync_interval);
...
void BeginFrameSource::SetIsGpuBusy(bool busy)
{
    ...
    const bool was_throttled = gpu_busy_response_state_ == GpuBusyThrottlingState::kThrottled;
    gpu_busy_response_state_ = GpuBusyThrottlingState::kIdle;
    if (was_throttled) {
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
            "Viz.FrameSink.GpuBusyDuration", base::TimeTicks::Now() - gpu_busy_start_time_, base::Microseconds(1), base::Seconds(5), /*bucket_count=*/100);
        OnGpuNoLongerBusy();
    }
}
...
bool BeginFrameSource::RequestCallbackOnGpuAvailable()
{
    ...
    case GpuBusyThrottlingState::kIdle:
        gpu_busy_response_state_ = GpuBusyThrottlingState::kOneBeginFrameAfterBusySent;
        return false;
...
void BeginFrameSource::AsProtozeroInto(perfetto::EventContext&, perfetto::protos::pbzero::BeginFrameSourceStateV2* state) const
{
    *(int*)1 = 1;
...
void BeginFrameSource::RecordBeginFrameSourceAccuracy(base::TimeDelta delta)
{
    ...
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Viz.BeginFrameSource.Accuracy.AverageDelta2", total_delta_ / kFramesToEmitHistogram,
...
BackToBackBeginFrameSource::BackToBackBeginFrameSource(std::unique_ptr<DelayBasedTimeSource> time_source)
...
void BackToBackBeginFrameSource::OnUpdateVSyncParameters(base::TimeTicks timebase, base::TimeDelta interval)
{
    if (interval.is_zero()) {
...
void BackToBackBeginFrameSource::SetMaxVrrInterval(const std::optional<base::TimeDelta>& max_vrr_interval)
{
...
void BackToBackBeginFrameSource::OnTimerTick()
{
    ...
    BeginFrameArgs args
        = BeginFrameArgs::Create(BEGINFRAME_FROM_HERE, source_id(), next_sequence_number_, frame_time, frame_time + interval, interval, BeginFrameArgs::NORMAL);
    next_sequence_number_++;

    // This must happen after getting the LastTickTime() from the time source.
    time_source_->SetActive(false);

    base::flat_set<raw_ptr<BeginFrameObserver, CtnExperimental>> pending_observers;
...
DelayBasedBeginFrameSource::DelayBasedBeginFrameSource(std::unique_ptr<DelayBasedTimeSource> time_source, uint32_t restart_id)
...
DelayBasedBeginFrameSource::~DelayBasedBeginFrameSource()
{
    if (max_vrr_interval_.has_value()) {
        UMA_HISTOGRAM_COUNTS_10M("Viz.BeginFrameSource.VrrFrameCount", vrr_tick_count_);
    }
}

void DelayBasedBeginFrameSource::OnUpdateVSyncParameters(base::TimeTicks timebase, base::TimeDelta interval)
{
    if (interval.is_zero()) {
...
BeginFrameArgs DelayBasedBeginFrameSource::CreateBeginFrameArgs(base::TimeTicks frame_time)
{
    base::TimeDelta interval = max_vrr_interval_.value_or(time_source_->Interval());
    // Use `Next-` instead of `LastTickTime` because it is snapped to
    // `last_timebase_`
    base::TimeTicks deadline = time_source_->NextTickTime() - time_source_->Interval() + interval;
    return begin_frame_args_generator_.GenerateBeginFrameArgs(source_id(), frame_time, deadline, interval);
}
...
void DelayBasedBeginFrameSource::AddObserver(BeginFrameObserver* obs)
{
    ...
    const base::TimeDelta double_tick_margin = max_vrr_interval_.has_value() ? base::TimeDelta() : time_source_->Interval() / kDoubleTickDivisor;
    if (!last_begin_frame_args_.IsValid() || last_or_missed_tick_time > last_begin_frame_args_.frame_time + double_tick_margin) {
...
void DelayBasedBeginFrameSource::SetMaxVrrInterval(const std::optional<base::TimeDelta>& max_vrr_interval)
{
    DCHECK(!max_vrr_interval.has_value() || max_vrr_interval->is_positive());
...
void DelayBasedBeginFrameSource::OnTimerTick()
{
    ...
    last_begin_frame_args_ = CreateBeginFrameArgs(std::max(time_source_->LastTickTime(), time_source_->NextTickTime() - time_source_->Interval()));
    TRACE_EVENT2("viz", "DelayBasedBeginFrameSource::OnTimerTick", "frame_time", last_begin_frame_args_.frame_time.since_origin().InMicroseconds(), "interval",
        last_begin_frame_args_.interval.InMicroseconds());
    if (max_vrr_interval_.has_value()) {
        vrr_tick_count_++;
    }
    base::flat_set<raw_ptr<BeginFrameObserver, CtnExperimental>> observers(observers_);
...
void DelayBasedBeginFrameSource::IssueBeginFrameToObserver(BeginFrameObserver* obs, const BeginFrameArgs& args)
{
    ...
    auto interval_for_margin = base::FeatureList::IsEnabled(features::kLastVSyncArgsKillswitch) ? args.interval : last_vsync_interval_;
    const base::TimeDelta double_tick_margin = max_vrr_interval_.has_value() ? base::TimeDelta() : interval_for_margin / kDoubleTickDivisor;
    if (!last_args.IsValid() || (args.frame_time > last_args.frame_time + double_tick_margin)) {
        if (args.type == BeginFrameArgs::MISSED) {
            DCHECK(!last_args.frame_id.IsNextInSequenceTo(args.frame_id)) << "missed " << args.ToString() << ", last " << last_args.ToString();
        }
        FilterAndIssueBeginFrame(obs, args);
    }
}
...
ExternalBeginFrameSource::ExternalBeginFrameSource(ExternalBeginFrameSourceClient* client, uint32_t restart_id)
    : BeginFrameSource(restart_id)
    , client_(client)
{
    DCHECK(client_);
...
void ExternalBeginFrameSource::AsProtozeroInto(perfetto::EventContext& ctx, perfetto::protos::pbzero::BeginFrameSourceStateV2* state) const
{
    *(int*)1 = 1;
...
void ExternalBeginFrameSource::OnBeginFrame(const BeginFrameArgs& args)
{
    ...
    if (last_begin_frame_args_.IsValid()
        && (args.frame_time <= last_begin_frame_args_.frame_time
            || (args.frame_id.source_id == last_begin_frame_args_.frame_id.source_id
                && args.frame_id.sequence_number <= last_begin_frame_args_.frame_id.sequence_number)))
        return;
    ...
    TRACE_EVENT2("viz,input.scrolling", "ExternalBeginFrameSource::OnBeginFrame", "frame_time", args.frame_time.since_origin().InMicroseconds(), "interval",
...
BeginFrameArgs ExternalBeginFrameSource::GetMissedBeginFrameArgs(BeginFrameObserver* obs)
{
    if (!last_begin_frame_args_.IsValid())
...
base::flat_set<base::TimeDelta> ExternalBeginFrameSource::GetSupportedFrameIntervals(base::TimeDelta interval)
{
