...
public:
    ...
    CopyOutputResult(Format format, Destination destination, const gfx::Rect& rect, bool needs_lock_for_bitmap);
    ...
    struct VIZ_COMMON_EXPORT TextureResult {
        ...
        TextureResult(const gpu::Mailbox& mailbox, const gfx::ColorSpace& color_space);
    ...
    virtual bool ReadI420Planes(uint8_t* y_out, int y_out_stride, uint8_t* u_out, int u_out_stride, uint8_t* v_out, int v_out_stride) const;
    ...
    //
    // The color space is always Rec.709 (see gfx::ColorSpace::CreateREC709()).
    virtual bool ReadNV12Planes(uint8_t* y_out, int y_out_stride, uint8_t* uv_out, int uv_out_stride) const;
...
public:
    CopyOutputSkBitmapResult(Format format, const gfx::Rect& rect, SkBitmap bitmap);
...
public:
    ...
    CopyOutputTextureResult(Format format, const gfx::Rect& rect, TextureResult texture_result, ReleaseCallbacks release_callbacks);
