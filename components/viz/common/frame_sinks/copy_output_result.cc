...
CopyOutputResult::TextureResult::TextureResult(const CopyOutputResult::TextureResult& other) = default;
CopyOutputResult::TextureResult& CopyOutputResult::TextureResult::operator=(const CopyOutputResult::TextureResult& other) = default;

CopyOutputResult::TextureResult::TextureResult(const gpu::Mailbox& mailbox, const gfx::ColorSpace& color_space)
    : mailbox(mailbox)
    , color_space(color_space)
{
}

CopyOutputResult::CopyOutputResult(Format format, Destination destination, const gfx::Rect& rect, bool needs_lock_for_bitmap)
    : format_(format)
    , destination_(destination)
    , rect_(rect)
    , needs_lock_for_bitmap_(needs_lock_for_bitmap)
{
    DCHECK(format_ == Format::RGBA || format_ == Format::I420_PLANES || format == Format::NV12);
    DCHECK(destination_ == Destination::kSystemMemory || destination_ == Destination::kNativeTextures);
}
...
CopyOutputResult::ScopedSkBitmap CopyOutputResult::ScopedAccessSkBitmap() const
{
    return ScopedSkBitmap(this);
}

const CopyOutputResult::TextureResult* CopyOutputResult::GetTextureResult() const
{
    return nullptr;
}
...
bool CopyOutputResult::ReadI420Planes(uint8_t* y_out, int y_out_stride, uint8_t* u_out, int u_out_stride, uint8_t* v_out, int v_out_stride) const
{
    ...
    if (bitmap.colorType() == kBGRA_8888_SkColorType) {
        return 0
            == libyuv::ARGBToI420(pixels, bitmap.rowBytes(), y_out, y_out_stride, u_out, u_out_stride, v_out, v_out_stride, bitmap.width(), bitmap.height());
    } else if (bitmap.colorType() == kRGBA_8888_SkColorType) {
        return 0
            == libyuv::ABGRToI420(pixels, bitmap.rowBytes(), y_out, y_out_stride, u_out, u_out_stride, v_out, v_out_stride, bitmap.width(), bitmap.height());
    }

    // Other SkBitmap color types could be supported, but are currently never
    // being used.
    NOTIMPLEMENTED() << "Unsupported format, bitmap.colorType()=" << bitmap.colorType();
    return false;
}

bool CopyOutputResult::ReadNV12Planes(uint8_t* y_out, int y_out_stride, uint8_t* uv_out, int uv_out_stride) const
{
    auto scoped_sk_bitmap = ScopedAccessSkBitmap();
    const SkBitmap& bitmap = scoped_sk_bitmap.bitmap();
    if (!bitmap.readyToDraw())
    ...
    // space management (due to performance concerns).
    if (bitmap.colorType() == kBGRA_8888_SkColorType) {
        return 0 == libyuv::ARGBToNV12(pixels, bitmap.rowBytes(), y_out, y_out_stride, uv_out, uv_out_stride, bitmap.width(), bitmap.height());
    } else if (bitmap.colorType() == kRGBA_8888_SkColorType) {
        return 0 == libyuv::ABGRToNV12(pixels, bitmap.rowBytes(), y_out, y_out_stride, uv_out, uv_out_stride, bitmap.width(), bitmap.height());
    }
...
bool CopyOutputResult::ReadRGBAPlane(uint8_t* dest, int stride) const
{
    ...
    SkImageInfo image_info = SkImageInfo::MakeN32(bitmap.width(), bitmap.height(), kPremul_SkAlphaType, bitmap.refColorSpace());
...
CopyOutputSkBitmapResult::CopyOutputSkBitmapResult(const gfx::Rect& rect, SkBitmap bitmap)
    : CopyOutputSkBitmapResult(Format::RGBA, rect, std::move(bitmap))
{
}

CopyOutputSkBitmapResult::CopyOutputSkBitmapResult(Format format, const gfx::Rect& rect, SkBitmap bitmap)
...
const SkBitmap& CopyOutputSkBitmapResult::AsSkBitmap() const
{
    ...
    const SkImageInfo image_info = SkImageInfo::MakeN32Premul(rect().width(), rect().height(), bitmap->refColorSpace());
...
CopyOutputTextureResult::CopyOutputTextureResult(Format format, const gfx::Rect& rect, TextureResult texture_result, ReleaseCallbacks release_callbacks)
...
const CopyOutputResult::TextureResult* CopyOutputTextureResult::GetTextureResult() const
{
    return &texture_result_;
}

CopyOutputResult::ReleaseCallbacks CopyOutputTextureResult::TakeTextureOwnership()
{
...
CopyOutputResult::ScopedSkBitmap::ScopedSkBitmap(const CopyOutputResult* result)
{
    DCHECK(result);
...
CopyOutputResult::ScopedSkBitmap& CopyOutputResult::ScopedSkBitmap::operator=(ScopedSkBitmap&& other)
{
