...
protected:
    ...
    void SetUp() override
    {
        ...
        std::unique_ptr<FakeDelayBasedTimeSource> time_source
            = std::make_unique<FakeDelayBasedTimeSource>(task_runner_->GetMockTickClock(), task_runner_.get());

        delay_based_time_source_ = time_source.get();
        source_ = std::make_unique<BackToBackBeginFrameSource>(std::move(time_source));
    ...
    raw_ptr<FakeDelayBasedTimeSource> delay_based_time_source_; // Owned by |source_|.
};

const int64_t BackToBackBeginFrameSourceTest::kDeadline = BeginFrameArgs::DefaultInterval().InMicroseconds();

const int64_t BackToBackBeginFrameSourceTest::kInterval = BeginFrameArgs::DefaultInterval().InMicroseconds();
...
TEST_F(BackToBackBeginFrameSourceTest, AddObserverSendsBeginFrame)
{
    ...
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 1, 1000, 1000 + kDeadline, kInterval);
    task_runner_->RunUntilIdle();

    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 2, 1100, 1100 + kDeadline, kInterval);
    task_runner_->AdvanceMockTickClock(base::Microseconds(100));
    source_->DidFinishFrame(obs_.get());
    task_runner_->RunUntilIdle();
}

TEST_F(BackToBackBeginFrameSourceTest, RemoveObserverThenDidFinishFrameProducesNoFrame)
{
    EXPECT_BEGIN_FRAME_SOURCE_PAUSED(*obs_, false);
    source_->AddObserver(obs_.get());
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 1, 1000, 1000 + kDeadline, kInterval);
...
TEST_F(BackToBackBeginFrameSourceTest, DidFinishFrameThenRemoveObserverProducesNoFrame)
{
    EXPECT_BEGIN_FRAME_SOURCE_PAUSED(*obs_, false);
    source_->AddObserver(obs_.get());
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 1, 1000, 1000 + kDeadline, kInterval);
...
TEST_F(BackToBackBeginFrameSourceTest, TogglingObserverThenDidFinishFrameProducesCorrectFrame)
{
    EXPECT_BEGIN_FRAME_SOURCE_PAUSED(*obs_, false);
    source_->AddObserver(obs_.get());
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 1, 1000, 1000 + kDeadline, kInterval);
    ...
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 2, 1110, 1110 + kDeadline, kInterval);
    EXPECT_TRUE(task_runner_->HasPendingTask());
    task_runner_->RunUntilIdle();
}

TEST_F(BackToBackBeginFrameSourceTest, DidFinishFrameThenTogglingObserverProducesCorrectFrame)
{
    EXPECT_BEGIN_FRAME_SOURCE_PAUSED(*obs_, false);
    source_->AddObserver(obs_.get());
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 1, 1000, 1000 + kDeadline, kInterval);
    ...
    // last change to "now".
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 2, 1120, 1120 + kDeadline, kInterval);
    EXPECT_TRUE(task_runner_->HasPendingTask());
    task_runner_->RunUntilIdle();
}
...
TEST_F(BackToBackBeginFrameSourceTest, DidFinishFrameMultipleCallsIdempotent)
{
    ...
    source_->DidFinishFrame(obs_.get());
    source_->DidFinishFrame(obs_.get());
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 3, 1200, 1200 + kDeadline, kInterval);
    task_runner_->RunUntilIdle();
}
...
TEST_F(BackToBackBeginFrameSourceTest, MultipleObserversSynchronized)
{
    ...
    EXPECT_BEGIN_FRAME_USED(obs1, source_->source_id(), 1, 1000, 1000 + kDeadline, kInterval);
    EXPECT_BEGIN_FRAME_USED(obs2, source_->source_id(), 1, 1000, 1000 + kDeadline, kInterval);
    task_runner_->RunUntilIdle();

    task_runner_->AdvanceMockTickClock(base::Microseconds(100));
    source_->DidFinishFrame(&obs1);
    source_->DidFinishFrame(&obs2);
    EXPECT_BEGIN_FRAME_USED(obs1, source_->source_id(), 2, 1100, 1100 + kDeadline, kInterval);
    EXPECT_BEGIN_FRAME_USED(obs2, source_->source_id(), 2, 1100, 1100 + kDeadline, kInterval);
    task_runner_->RunUntilIdle();
...
TEST_F(BackToBackBeginFrameSourceTest, MultipleObserversInterleaved)
{
    ...
    task_runner_->AdvanceMockTickClock(base::Microseconds(100));
    source_->DidFinishFrame(&obs1);
    EXPECT_BEGIN_FRAME_USED(obs1, source_->source_id(), 3, 1200, 1200 + kDeadline, kInterval);
    task_runner_->RunUntilIdle();
    ...
    task_runner_->AdvanceMockTickClock(base::Microseconds(100));
    source_->DidFinishFrame(&obs2);
    EXPECT_BEGIN_FRAME_USED(obs2, source_->source_id(), 4, 1300, 1300 + kDeadline, kInterval);
    task_runner_->RunUntilIdle();
...
public:
    void SetUp() override
    {
        ...
        time_source->SetTimebaseAndInterval(base::TimeTicks(), base::Microseconds(10000));
        source_ = std::make_unique<DelayBasedBeginFrameSource>(std::move(time_source), BeginFrameSource::kNotRestartableId);
...
TEST_F(DelayBasedBeginFrameSourceTest, AddObserverCallsOnBeginFrameWithMissedTick)
{
    task_runner_->AdvanceMockTickClock(base::Microseconds(9010));
    EXPECT_BEGIN_FRAME_SOURCE_PAUSED(*obs_, false);
    EXPECT_BEGIN_FRAME_USED_MISSED(*obs_, source_->source_id(), 1, 10000, 20000, 10000);
...
{
    EXPECT_BEGIN_FRAME_SOURCE_PAUSED(*obs_, false);
    EXPECT_BEGIN_FRAME_USED_MISSED(*obs_, source_->source_id(), 1, 0, 10000, 10000);
    source_->AddObserver(obs_.get());
    EXPECT_EQ(TicksFromMicroseconds(10000), task_runner_->NowTicks() + task_runner_->NextPendingTaskDelay());
...
TEST_F(DelayBasedBeginFrameSourceTest, VSyncChanges)
{
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(27500), base::Microseconds(10001));
...
TEST_F(DelayBasedBeginFrameSourceTest, VSyncChangeTimebaseBeforeLastTick)
{
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(26000), base::Microseconds(5000));
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(42000), base::Microseconds(9000));
...
TEST_F(DelayBasedBeginFrameSourceTest, VSyncChangeTimebaseAfterNextTick)
{
    ...
    // Begin frame won't be used at 41000 because this is a double-tick.
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(41000), base::Microseconds(5000));
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 5, 40000, 41000, 5000);
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 7, 46000, 51000, 5000);
    task_runner_->FastForwardTo(TicksFromMicroseconds(46000));
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(60000), base::Microseconds(9000));
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 8, 51000, 60000, 9000);
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 9, 60000, 69000, 9000);
    EXPECT_BEGIN_FRAME_USED(*obs_, source_->source_id(), 10, 69000, 78000, 9000);
    task_runner_->FastForwardTo(TicksFromMicroseconds(70000));
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(100000), base::Microseconds(5000));
...
TEST_F(DelayBasedBeginFrameSourceTest, VSyncChangeTimebaseBetweenTicks)
{
    ...
    // and last tick time.
    // Begin frame won't be used at 41000 because this is a double-tick.
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(35000), base::Microseconds(6000));
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(49000), base::Microseconds(10000));
...
TEST_F(DelayBasedBeginFrameSourceTest, VSyncSkipped)
{
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(40000), base::Microseconds(11000));
...
TEST_F(DelayBasedBeginFrameSourceTest, MultipleObservers)
{
    ...
    EXPECT_BEGIN_FRAME_SOURCE_PAUSED(obs1, false);
    EXPECT_BEGIN_FRAME_USED_MISSED(obs1, source_->source_id(), 1, 10000, 20000, 10000);
    ...
    EXPECT_BEGIN_FRAME_USED_MISSED(obs2, source_->source_id(), 2, 20000, 30000, 10000);
...
TEST_F(DelayBasedBeginFrameSourceTest, DoubleTick)
{
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(5000), base::Microseconds(10000));
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(10000), base::Microseconds(10000));
...
TEST_F(DelayBasedBeginFrameSourceTest, DoubleTickMissedFrame)
{
    ...
    EXPECT_BEGIN_FRAME_USED_MISSED(obs, source_->source_id(), 2, 10000, 20000, 10000);
    source_->AddObserver(&obs);
    source_->RemoveObserver(&obs);
}
...
TEST_F(DelayBasedBeginFrameSourceTest, MultipleArgsInSameInterval)
{
    ...
    EXPECT_BEGIN_FRAME_USED_MISSED(obs2, source_->source_id(), 2, 10000, 20000, 10000);
    source_->AddObserver(&obs2);
...
TEST_F(DelayBasedBeginFrameSourceTest, ConsecutiveArgsDelayedByMultipleVsyncs)
{
    ...
    EXPECT_BEGIN_FRAME_SOURCE_PAUSED(obs, false);
    EXPECT_BEGIN_FRAME_USED_MISSED(obs, source_->source_id(), 10, 90000, 100000, 10000);
...
TEST_F(DelayBasedBeginFrameSourceTest, WithVrrInterval)
{
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(21000), base::Microseconds(10000));
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(32000), base::Microseconds(10000));
    ...
    source_->OnUpdateVSyncParameters(TicksFromMicroseconds(53000), base::Microseconds(10000));
...
// --------------------------------------------
class MockExternalBeginFrameSourceClient : public ExternalBeginFrameSourceClient {
public:
...
public:
    ExternalBeginFrameSourceTest()
        ...
        , source_(std::make_unique<ExternalBeginFrameSource>(client_.get(), BeginFrameSource::kNotRestartableId))
        , obs_(std::make_unique<MockBeginFrameObserver>())
    {
    }
...
TEST_F(ExternalBeginFrameSourceTest, OnAnimateOnlyBeginFrameOptIn)
{
    ...
    BeginFrameArgs args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 2, TicksFromMicroseconds(10000));
    args.animate_only = true;
    source_->OnBeginFrame(args);

    // When opting in, an observer receives animate_only BeginFrames.
    args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 3, TicksFromMicroseconds(10001));
    args.animate_only = true;
    EXPECT_CALL(*obs_, WantsAnimateOnlyBeginFrames()).WillOnce(::testing::Return(true));
...
{
    BeginFrameArgs args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 2, 10000, 10100, 100);
    source_->OnBeginFrame(args);
    ...
    args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 2, 9999, 10100, 101);
    source_->OnBeginFrame(args);
...
TEST_F(ExternalBeginFrameSourceTest, RootsNotifiedLast)
{
    ...
    {
        BeginFrameArgs args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 1, 10000, 10100, 100);
        ...
        EXPECT_CALL(obs2, IsRoot()).WillRepeatedly(::testing::Return(false));
        {
            ...
            InSequence s;
            EXPECT_CALL(obs2, OnBeginFrame(args)).WillOnce(::testing::SaveArg<0>(&(obs2.last_begin_frame_args)));
            EXPECT_CALL(obs1, OnBeginFrame(args)).WillOnce(::testing::SaveArg<0>(&(obs1.last_begin_frame_args)));
            source_->OnBeginFrame(args);
        }
    }

    {
        BeginFrameArgs args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 2, 10001, 10101, 100);
