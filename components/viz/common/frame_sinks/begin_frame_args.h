...
struct VIZ_COMMON_EXPORT BeginFrameId {
    ...
    friend std::strong_ordering operator<=>(const BeginFrameId&, const BeginFrameId&) = default;
...
struct VIZ_COMMON_EXPORT PossibleDeadline {
    PossibleDeadline(int64_t vsync_id, base::TimeDelta latch_delta, base::TimeDelta present_delta);
    ~PossibleDeadline();
...
struct VIZ_COMMON_EXPORT BeginFrameArgs {
    ...
    static BeginFrameArgs Create(CreationLocation location, uint64_t source_id, uint64_t sequence_number, base::TimeTicks frame_time, base::TimeTicks deadline,
        base::TimeDelta interval, BeginFrameArgsType type);
    ...
    static base::TimeDelta DefaultEstimatedDisplayDrawTime(base::TimeDelta interval)
    {
    ...
    void AsProtozeroInto(perfetto::EventContext& ctx, perfetto::protos::pbzero::BeginFrameArgsV2* args) const;
...
private:
    BeginFrameArgs(
        uint64_t source_id, uint64_t sequence_number, base::TimeTicks frame_time, base::TimeTicks deadline, base::TimeDelta interval, BeginFrameArgsType type);
};
...
struct VIZ_COMMON_EXPORT BeginFrameAck {
    ...
    BeginFrameAck(uint64_t source_id, uint64_t sequence_number, bool has_damage, int64_t trace_id = -1);
