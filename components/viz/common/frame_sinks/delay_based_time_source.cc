...
DelayBasedTimeSource::DelayBasedTimeSource(base::SingleThreadTaskRunner* task_runner)
    : client_(nullptr)
    , active_(false)
    ...
    , tick_closure_(base::BindRepeating(&DelayBasedTimeSource::OnTimerTick, base::Unretained(this)))
{
}
...
void DelayBasedTimeSource::PostNextTickTask(base::TimeTicks now)
{
    if (interval_.is_zero()) {
        next_tick_time_ = now;
        timer_.Start(FROM_HERE, base::TimeTicks(), tick_closure_, base::subtle::DelayPolicy::kPrecise);
    } else {
        ...
        timer_.Start(FROM_HERE, next_tick_time_, tick_closure_, base::subtle::DelayPolicy::kPrecise);
    }
}
...
void DelayBasedTimeSource::AsValueInto(base::trace_event::TracedValue* state) const
{
    ...
    state->SetDouble("last_tick_time_us", LastTickTime().since_origin().InMicrosecondsF());
    state->SetDouble("next_tick_time_us", NextTickTime().since_origin().InMicrosecondsF());
