...
protected:
    ...
    void AsProtozeroInto(perfetto::EventContext& ctx, perfetto::protos::pbzero::BeginFrameObserverStateV2* state) const;
...
public:
    ...
    public:
        ...
        BeginFrameArgs GenerateBeginFrameArgs(uint64_t source_id, base::TimeTicks frame_time, base::TimeTicks deadline, base::TimeDelta vsync_interval);

    private:
        static uint64_t EstimateTickCountsBetween(base::TimeTicks frame_time, base::TimeTicks next_expected_frame_time, base::TimeDelta vsync_interval);
    ...
    virtual void AsProtozeroInto(perfetto::EventContext& ctx, perfetto::protos::pbzero::BeginFrameSourceStateV2* state) const;
    ...
    virtual void SetUpdateVSyncParametersCallback(UpdateVSyncParametersCallback callback)
    {
    }
...
private:
    ...
    GpuBusyThrottlingState gpu_busy_response_state_ = GpuBusyThrottlingState::kIdle;
...
public:
    ...
    virtual void SetMaxVrrInterval(const std::optional<base::TimeDelta>& max_vrr_interval) = 0;
};

// A frame source which calls BeginFrame (at the next possible time) as soon as
// an observer acknowledges the prior BeginFrame.
class VIZ_COMMON_EXPORT BackToBackBeginFrameSource : public SyntheticBeginFrameSource, public DelayBasedTimeSourceClient {
public:
    explicit BackToBackBeginFrameSource(std::unique_ptr<DelayBasedTimeSource> time_source);

    BackToBackBeginFrameSource(const BackToBackBeginFrameSource&) = delete;
    BackToBackBeginFrameSource& operator=(const BackToBackBeginFrameSource&) = delete;
    ...
    void SetMaxVrrInterval(const std::optional<base::TimeDelta>& max_vrr_interval) override;
...
private:
    ...
    base::flat_set<raw_ptr<BeginFrameObserver, CtnExperimental>> pending_begin_frame_observers_;
...
class VIZ_COMMON_EXPORT DelayBasedBeginFrameSource : public SyntheticBeginFrameSource, public DelayBasedTimeSourceClient {
public:
    DelayBasedBeginFrameSource(std::unique_ptr<DelayBasedTimeSource> time_source, uint32_t restart_id);

    DelayBasedBeginFrameSource(const DelayBasedBeginFrameSource&) = delete;
    DelayBasedBeginFrameSource& operator=(const DelayBasedBeginFrameSource&) = delete;
    ...
    const DelayBasedTimeSource* time_source() const
    {
...
private:
    ...
    void IssueBeginFrameToObserver(BeginFrameObserver* obs, const BeginFrameArgs& args);
...
public:
    ...
    explicit ExternalBeginFrameSource(ExternalBeginFrameSourceClient* client, uint32_t restart_id = kNotRestartableId);
    ...
    void AsProtozeroInto(perfetto::EventContext& ctx, perfetto::protos::pbzero::BeginFrameSourceStateV2* state) const override;
    void OnGpuNoLongerBusy() override;
    ...
    virtual base::flat_set<base::TimeDelta> GetSupportedFrameIntervals(base::TimeDelta interval);
