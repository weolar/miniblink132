...
{
    return base::StringPrintf("sampling_bounds: %s, result_bounds: %s, result_selection: %s, "
                              "readback_offset: %s",
        sampling_bounds.ToString().c_str(), result_bounds.ToString().c_str(), result_selection.ToString().c_str(), readback_offset.ToString().c_str());
}

gfx::Rect ComputeResultRect(const gfx::Rect& area, const gfx::Vector2d& scale_from, const gfx::Vector2d& scale_to)
{
    ...
    const int64_t w = area.width() == 0 ? 0 : (CeilScale(area.right(), scale_to.x(), scale_from.x()) - x);
    const int64_t h = area.height() == 0 ? 0 : (CeilScale(area.bottom(), scale_to.y(), scale_from.y()) - y);
    ...
    if (x < -kMaxOriginOffset || x > kMaxOriginOffset || y < -kMaxOriginOffset || y > kMaxOriginOffset || w <= 0 || w > kMaxDimension || h <= 0
        || h > kMaxDimension) {
        return gfx::Rect();
    }

    return gfx::Rect(static_cast<int>(x), static_cast<int>(y), static_cast<int>(w), static_cast<int>(h));
}
