...
std::unique_ptr<SurfaceSavedFrame> CreateFrameWithResult()
{
    ...
    auto directive = CompositorFrameTransitionDirective::CreateSave(blink::ViewTransitionToken(), /*maybe_cross_frame_sink=*/false, 1, { element }, {});
...
public:
    ...
    bool HasBitmapResource(const TransferableResource& resource)
    {
        ...
        return !!shared_bitmap_manager_.GetSharedBitmapFromId(gfx::Size(1, 1), SinglePlaneFormat::kRGBA_8888, id);
    }
...
TEST_F(TransferableResourceTrackerTest, IdInRange)
{
    ...
    auto frame1 = tracker.ImportResources(result->TakeResult(), result->directive());
    ...
    auto frame2 = tracker.ImportResources(result->TakeResult(), result->directive());
...
{
    static_assert(std::is_same<decltype(kInvalidResourceId.GetUnsafeValue()), uint32_t>::value, "The test only makes sense if ResourceId is uint32_t");
    ...
    for (int i = 0; i < 10; ++i) {
        auto result = CreateFrameWithResult();
        auto frame = tracker.ImportResources(result->TakeResult(), result->directive());
...
TEST_F(TransferableResourceTrackerTest, ExhaustedIdLoopsButSkipsUnavailableIds)
{
    static_assert(std::is_same<decltype(kInvalidResourceId.GetUnsafeValue()), uint32_t>::value, "The test only makes sense if ResourceId is uint32_t");
    TransferableResourceTracker tracker(&shared_bitmap_manager_, &id_tracker_);

    auto result = CreateFrameWithResult();
    auto reserved = tracker.ImportResources(result->TakeResult(), result->directive());
