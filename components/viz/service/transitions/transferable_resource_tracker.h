...
public:
    ...
    struct VIZ_SERVICE_EXPORT ResourceFrame {
        ...
        // above.
        base::flat_map<ViewTransitionElementResourceId, TransferableResource> element_id_to_resource;
    };

    explicit TransferableResourceTracker(SharedBitmapManager* shared_bitmap_manager, ReservedResourceIdTracker* id_tracker);
    TransferableResourceTracker(const TransferableResourceTracker&) = delete;
    ~TransferableResourceTracker();

    TransferableResourceTracker& operator=(const TransferableResourceTracker&) = delete;
    ...
    // "returned".
    ResourceFrame ImportResources(SurfaceSavedFrame::FrameResult frame_result, CompositorFrameTransitionDirective directive);
    ...
    void UnrefResource(ResourceId id, int count, const gpu::SyncToken& sync_token);

    bool is_empty() const
    {
        return managed_resources_.empty();
    }

private:
    friend class TransferableResourceTrackerTest;

    PositionedResource ImportResource(SurfaceSavedFrame::OutputCopyResult output_copy);

    static_assert(std::is_same<decltype(kInvalidResourceId.GetUnsafeValue()), uint32_t>::value, "ResourceId underlying type should be uint32_t");

    const raw_ptr<SharedBitmapManager> shared_bitmap_manager_;

    struct TransferableResourceHolder {
        using ResourceReleaseCallback = base::OnceCallback<void(const TransferableResource&, const gpu::SyncToken&)>;

        TransferableResourceHolder();
        TransferableResourceHolder(const TransferableResource& resource, ResourceReleaseCallback release_callback);
