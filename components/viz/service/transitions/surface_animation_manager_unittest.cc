...
std::vector<CompositorFrameTransitionDirective> CreateSaveDirectiveAsVector(uint32_t sequence_id)
{
    std::vector<CompositorFrameTransitionDirective> result;
    result.emplace_back(sequence_id, CompositorFrameTransitionDirective::Type::kSave);
    return result;
}

std::vector<CompositorFrameTransitionDirective> CreateAnimateRendererDirectiveAsVector(uint32_t sequence_id)
{
    std::vector<CompositorFrameTransitionDirective> result;
    result.emplace_back(sequence_id, CompositorFrameTransitionDirective::Type::kAnimateRenderer);
    return result;
}
...
public:
    void SetUp() override
    {
        ...
        support_ = std::make_unique<CompositorFrameSinkSupport>(nullptr, &frame_sink_manager_, kArbitraryFrameSinkId, /*is_root=*/true);
...
{
    manager().ProcessTransitionDirectives(CreateSaveDirectiveAsVector(1), surface());

    storage()->ExpireForTesting();

    manager().ProcessTransitionDirectives(CreateAnimateRendererDirectiveAsVector(2), surface());
}
...
TEST_F(SurfaceAnimationManagerTest, RepeatedSavesAreOk)
{
    ...
    for (int i = 0; i < 200; ++i) {
        manager().ProcessTransitionDirectives(CreateSaveDirectiveAsVector(sequence_id), surface());
    }

    storage()->CompleteForTesting();

    manager().ProcessTransitionDirectives(CreateAnimateRendererDirectiveAsVector(sequence_id), surface());
}
