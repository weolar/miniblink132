...
public:
    ...
    void OnCreateCompositorFrameSink(const FrameSinkId& frame_sink_id, bool is_root, input::mojom::RenderInputRouterConfigPtr render_input_router_config,
        bool create_input_receiver, gpu::SurfaceHandle surface_handle);

    // FrameSinkObserver overrides.
    void OnDestroyedCompositorFrameSink(const FrameSinkId& frame_sink_id) override;
    ...
    FrameSinkId GetRootCompositorFrameSinkId(const FrameSinkId& child_frame_sink_id) override;
    ...
    void NotifyObserversOfInputEventAcks(const FrameSinkId& frame_sink_id, uint32_t grouping_id, blink::mojom::InputEventResultSource ack_source,
        blink::mojom::InputEventResultState ack_result, std::unique_ptr<blink::WebCoalescedInputEvent> event) override;
    void OnInvalidInputEventSource(const FrameSinkId& frame_sink_id, uint32_t grouping_id) override;
    ...
    input::RenderInputRouter* GetRenderInputRouterFromFrameSinkId(const FrameSinkId& id);
...
private:
    ...
    base::flat_map</*grouping_id=*/uint32_t, scoped_refptr<input::RenderWidgetHostInputEventRouter>> rwhier_map_;
    ...
    base::flat_map<FrameSinkId, std::unique_ptr<input::RenderInputRouter>> rir_map_;
    ...
    base::flat_map</*grouping_id=*/uint32_t, mojo::Remote<input::mojom::RenderInputRouterDelegateClient>> rir_delegate_remote_map_;
