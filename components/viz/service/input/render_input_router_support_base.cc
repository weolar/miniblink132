...
RenderInputRouterSupportBase::RenderInputRouterSupportBase(input::RenderInputRouter* rir, Delegate* delegate, const FrameSinkId& frame_sink_id)
    : frame_sink_id_(frame_sink_id)
    , delegate_(delegate)
    , rir_(*rir)
{
    TRACE_EVENT_INSTANT("input", "RenderInputRouterSupportBase::RenderInputRouterSupportBase", "frame_sink_id", frame_sink_id);
    CHECK(delegate_);
}
...
void RenderInputRouterSupportBase::NotifyHoverActionStylusWritable(bool stylus_writable)
{
    // Stylus input events are not going to be handled on VizCompositor thread
    // with the current scope of InputVizard.
    NOTREACHED();
...
base::WeakPtr<input::RenderWidgetHostViewInput> RenderInputRouterSupportBase::GetInputWeakPtr()
{
    return weak_factory_.GetWeakPtr();
}

input::RenderInputRouter* RenderInputRouterSupportBase::GetViewRenderInputRouter()
{
    return &*rir_;
}

void RenderInputRouterSupportBase::ProcessMouseEvent(const blink::WebMouseEvent& event, const ui::LatencyInfo& latency)
{
...
void RenderInputRouterSupportBase::ProcessMouseWheelEvent(const blink::WebMouseWheelEvent& event, const ui::LatencyInfo& latency)
{
...
void RenderInputRouterSupportBase::ProcessTouchEvent(const blink::WebTouchEvent& event, const ui::LatencyInfo& latency)
{
...
void RenderInputRouterSupportBase::ProcessGestureEvent(const blink::WebGestureEvent& event, const ui::LatencyInfo& latency)
{
    // Events are not routed to a prerendered page since we don't create RIR's for
    // them, hence we can directly process all input events here. See
...
const DisplayHitTestQueryMap& RenderInputRouterSupportBase::GetDisplayHitTestQuery() const
{
