...
class VIZ_SERVICE_EXPORT InfoCollectionGpuServiceImpl : public mojom::InfoCollectionGpuService {
public:
    InfoCollectionGpuServiceImpl(scoped_refptr<base::SingleThreadTaskRunner> main_runner, scoped_refptr<base::SingleThreadTaskRunner> io_runner,
        const gpu::DevicePerfInfo& device_perf_info, const gpu::GPUInfo::GPUDevice& gpu_device,
    ...
    InfoCollectionGpuServiceImpl& operator=(const InfoCollectionGpuServiceImpl&) = delete;
...
private:
    void BindOnIO(mojo::PendingReceiver<mojom::InfoCollectionGpuService> pending_receiver);

    void GetGpuSupportedDirectXVersionAndDevicePerfInfoOnMain(GetGpuSupportedDirectXVersionAndDevicePerfInfoCallback callback);

    void GetGpuSupportedVulkanVersionInfoOnMain(GetGpuSupportedVulkanVersionInfoCallback callback);
