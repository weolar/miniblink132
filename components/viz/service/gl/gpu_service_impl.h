...
public:
    ...
    GpuServiceImpl(const gpu::GpuPreferences& gpu_preferences, const gpu::GPUInfo& gpu_info, const gpu::GpuFeatureInfo& gpu_feature_info,
    ...
    void InitializeWithHost(mojo::PendingRemote<mojom::GpuHost> gpu_host, gpu::GpuProcessShmCount use_shader_cache_shm_count,
        scoped_refptr<gl::GLSurface> default_offscreen_surface, mojom::GpuServiceCreationParamsPtr creation_params,
#if BUILDFLAG(IS_ANDROID)
        gpu::SyncPointManager* sync_point_manager = nullptr, gpu::SharedImageManager* shared_image_manager = nullptr, gpu::Scheduler* scheduler = nullptr,
#endif
    ...
    void SetChannelDiskCacheHandle(int32_t client_id, const gpu::GpuDiskCacheHandle& handle) override;
    void OnDiskCacheHandleDestoyed(const gpu::GpuDiskCacheHandle& handle) override;
    void CloseChannel(int32_t client_id) override;
#if BUILDFLAG(IS_CHROMEOS_ASH)
#if BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
    void CreateArcVideoDecodeAccelerator(mojo::PendingReceiver<arc::mojom::VideoDecodeAccelerator> vda_receiver) override;
    void CreateArcVideoDecoder(mojo::PendingReceiver<arc::mojom::VideoDecoder> vd_receiver) override;
    void CreateArcVideoEncodeAccelerator(mojo::PendingReceiver<arc::mojom::VideoEncodeAccelerator> vea_receiver) override;
    void CreateArcVideoProtectedBufferAllocator(mojo::PendingReceiver<arc::mojom::VideoProtectedBufferAllocator> pba_receiver) override;
    void CreateArcProtectedBufferManager(mojo::PendingReceiver<arc::mojom::ProtectedBufferManager> pbm_receiver) override;
#endif // BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
    void CreateJpegDecodeAccelerator(mojo::PendingReceiver<chromeos_camera::mojom::MjpegDecodeAccelerator> jda_receiver) override;
    void CreateJpegEncodeAccelerator(mojo::PendingReceiver<chromeos_camera::mojom::JpegEncodeAccelerator> jea_receiver) override;
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
    ...
    void CreateVideoEncodeAcceleratorProvider(mojo::PendingReceiver<media::mojom::VideoEncodeAcceleratorProvider> vea_provider_receiver) override;

    void BindWebNNContextProvider(mojo::PendingReceiver<webnn::mojom::WebNNContextProvider> pending_receiver, int client_id) override;

    void BindClientGmbInterface(mojo::PendingReceiver<gpu::mojom::ClientGmbInterface> pending_receiver, int client_id) override;

    void CreateGpuMemoryBuffer(gfx::GpuMemoryBufferId id, const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage, int client_id,
        gpu::SurfaceHandle surface_handle, CreateGpuMemoryBufferCallback callback) override;
    void DestroyGpuMemoryBuffer(gfx::GpuMemoryBufferId id, int client_id) override;
...
#endif
    void LoadedBlob(const gpu::GpuDiskCacheHandle& handle, const std::string& key, const std::string& data) override;
    ...
    void WriteClangProfilingProfile(WriteClangProfilingProfileCallback callback) override;
#endif
    void Crash() override;
    void Hang() override;
    ...
    void GetIsolationKey(int client_id, const blink::WebGPUExecutionContextToken& token, GetIsolationKeyCallback cb) override;
    ...
    void MaybeExitOnContextLost(bool synthetic_loss, gpu::error::ContextLostReason context_lost_reason) override;
    ...
    using RequestBeginFrameForGpuServiceCB = base::RepeatingCallback<void(bool toggle)>;
    void SetRequestBeginFrameForGpuServiceCB(RequestBeginFrameForGpuServiceCB cb);
    void SetMjpegDecodeAcceleratorBeginFrameCB(std::optional<base::RepeatingClosure> cb);
    ...
    {
        return gpu_channel_manager_->scheduler();
    }
    ...
    {
        return compositor_gpu_thread() ? compositor_gpu_thread()->task_runner() : main_runner_;
    }
    ...
    bool in_host_process() const
    {
        return gpu_info_.in_process_gpu;
    }
    ...
    base::ProcessId host_process_id() const
    {
        return host_process_id_;
    }
...
private:
    ...
    class ClientGmbInterfaceImpl : public gpu::mojom::ClientGmbInterface, public base::trace_event::MemoryDumpProvider {
    public:
        ClientGmbInterfaceImpl(int client_id, mojo::PendingReceiver<gpu::mojom::ClientGmbInterface> pending_receiver, raw_ptr<GpuServiceImpl> gpu_service,
            scoped_refptr<base::SingleThreadTaskRunner> io_runner);
        ...
        void OnGpuMemoryBufferAllocated(gfx::GpuMemoryBufferId id, gfx::GpuMemoryBufferHandle handle);
    ...
    private:
        ...
        std::unordered_map<gfx::GpuMemoryBufferId, PendingBufferInfo, std::hash<gfx::GpuMemoryBufferId>> pending_buffers_;
        std::unordered_map<gfx::GpuMemoryBufferId, gpu::AllocatedBufferInfo, std::hash<gfx::GpuMemoryBufferId>> allocated_buffers_;

        base::WeakPtr<ClientGmbInterfaceImpl> weak_ptr_;
        base::WeakPtrFactory<ClientGmbInterfaceImpl> weak_ptr_factory_ { this };
    };

    bool IsNativeBufferSupported(gfx::BufferFormat format, gfx::BufferUsage usage);
    void RecordLogMessage(int severity, const std::string& header, const std::string& message);

#if BUILDFLAG(IS_CHROMEOS_ASH) && BUILDFLAG(USE_CHROMEOS_MEDIA_ACCELERATION)
    void CreateArcVideoDecodeAcceleratorOnMainThread(mojo::PendingReceiver<arc::mojom::VideoDecodeAccelerator> vda_receiver);
    void CreateArcVideoDecoderOnMainThread(mojo::PendingReceiver<arc::mojom::VideoDecoder> vd_receiver);
    void CreateArcVideoEncodeAcceleratorOnMainThread(mojo::PendingReceiver<arc::mojom::VideoEncodeAccelerator> vea_receiver);
    void CreateArcVideoProtectedBufferAllocatorOnMainThread(mojo::PendingReceiver<arc::mojom::VideoProtectedBufferAllocator> pba_receiver);
    void CreateArcProtectedBufferManagerOnMainThread(mojo::PendingReceiver<arc::mojom::ProtectedBufferManager> pbm_receiver);
#endif // BUILDFLAG(IS_CHROMEOS_ASH) &&                                                                                                                        \
    ...
    void GetPeakMemoryUsageOnMainThread(uint32_t sequence_num, GetPeakMemoryUsageCallback callback);
    ...
    std::optional<base::RepeatingClosure> mjpeg_decode_accelerator_begin_frame_cb_;
...
#if BUILDFLAG(USE_DAWN) || BUILDFLAG(SKIA_USE_DAWN)
    std::unique_ptr<gpu::webgpu::DawnCachingInterfaceFactory> dawn_caching_interface_factory_;
#endif
    ...
    std::unique_ptr<gpu::ImageDecodeAcceleratorWorker> image_decode_accelerator_worker_;
