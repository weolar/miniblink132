...
public:
    GpuServiceTest()
        ...
        , wait_(base::WaitableEvent::ResetPolicy::MANUAL, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }
    ...
    {
        wait_.Reset();
        io_runner()->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Wait, base::Unretained(&wait_)));
    }
    ...
    void SetUp() override
    {
        ...
        gpu_service_ = std::make_unique<GpuServiceImpl>(
            gpu::GpuPreferences(), gpu_info, gpu::GpuFeatureInfo(), gpu::GPUInfo(), gpu::GpuFeatureInfo(), gfx::GpuExtraInfo(), std::move(init_params));
    }
...
TEST_F(GpuServiceTest, ServiceDestroyedAfterBind)
{
    ...
    io_runner()->PostTask(FROM_HERE, base::BindOnce(&base::WaitableEvent::Signal, base::Unretained(&wait)));
    wait.Wait();
    DestroyService();
}
...
TEST_F(GpuServiceTest, LoseAllContexts)
{
    ...
    gpu_service()->InitializeWithHost(std::move(gpu_host_proxy), gpu::GpuProcessShmCount(),
        gl::init::CreateOffscreenGLSurface(gl::GetDefaultDisplay(), gfx::Size()), mojom::GpuServiceCreationParams::New());
...
TEST_F(GpuServiceTest, VisibilityCallbackCalled)
{
    ...
    gpu_service()->SetVisibilityChangedCallback(
        base::BindRepeating([](GpuServiceTest* test, bool visible) { test->visible_ = visible; }, base::Unretained(this)));
