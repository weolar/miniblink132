...
public:
    SinkGroup(FrameSinkManagerImpl& manager, FrameSinkBundleImpl& bundle, BeginFrameSource& source, mojom::FrameSinkBundleClient& client)
        : manager_(manager)
        , bundle_(bundle)
        , source_(source)
        , client_(client)
    ...
    void EnqueueDidReceiveCompositorFrameAck(uint32_t sink_id, std::vector<ReturnedResource> resources)
    {
        pending_received_frame_acks_.push_back(mojom::BundledReturnedResources::New(sink_id, std::move(resources)));
    ...
    void EnqueueOnBeginFrame(uint32_t sink_id, const BeginFrameArgs& args, const base::flat_map<uint32_t, FrameTimingDetails>& details, bool frame_ack,
        std::vector<ReturnedResource> resources)
    {
        pending_on_begin_frames_.push_back(mojom::BeginFrameInfo::New(sink_id, args, details, frame_ack, std::move(resources)));
        if (!defer_on_begin_frames_) {
            FlushMessages();
        }
    }

    void EnqueueReclaimResources(uint32_t sink_id, std::vector<ReturnedResource> resources)
    {
        // We always defer ReclaimResources until the next flush, whether it's done
        // for frame acks or OnBeginFrames.
        pending_reclaimed_resources_.push_back(mojom::BundledReturnedResources::New(sink_id, std::move(resources)));
    }
    ...
    void FlushMessages()
    {
        ...
        if (!pending_received_frame_acks.empty() || !pending_on_begin_frames.empty() || !pending_reclaimed_resources.empty()) {
            client_->FlushNotifications(std::move(pending_received_frame_acks), std::move(pending_on_begin_frames), std::move(pending_reclaimed_resources));
        }
    }

    void DidFinishFrame()
    {
        source_->DidFinishFrame(this);
    }
...
FrameSinkBundleImpl::FrameSinkBundleImpl(FrameSinkManagerImpl& manager, const FrameSinkBundleId& id, mojo::PendingReceiver<mojom::FrameSinkBundle> receiver,
...
{
    receiver_.set_disconnect_handler(base::BindOnce(&FrameSinkBundleImpl::OnDisconnect, base::Unretained(this)));
}

FrameSinkBundleImpl::~FrameSinkBundleImpl() = default;

void FrameSinkBundleImpl::SetSinkNeedsBeginFrame(uint32_t sink_id, bool needs_begin_frame)
{
...
void FrameSinkBundleImpl::AddFrameSink(CompositorFrameSinkSupport* support)
{
    ...
    if (!group) {
        group = std::make_unique<SinkGroup>(*manager_, *this, *source, *client_.get());
    }
    group->AddFrameSink(sink_id);
}

void FrameSinkBundleImpl::UpdateFrameSink(CompositorFrameSinkSupport* support, BeginFrameSource* old_source)
{
    uint32_t sink_id = support->frame_sink_id().sink_id();
...
void FrameSinkBundleImpl::InitializeCompositorFrameSinkType(uint32_t sink_id, mojom::CompositorFrameSinkType type)
{
...
void FrameSinkBundleImpl::SetNeedsBeginFrame(uint32_t sink_id, bool needs_begin_frame)
{
    if (auto* sink = GetFrameSink(sink_id)) {
...
void FrameSinkBundleImpl::Submit(std::vector<mojom::BundledFrameSubmissionPtr> submissions)
{
    ...
    for (auto& submission : submissions) {
        if (auto* sink = GetFrameSink(submission->sink_id)) {
            ...
            case mojom::BundledFrameSubmissionData::Tag::kFrame: {
                mojom::BundledCompositorFramePtr& frame = submission->data->get_frame();
                sink->SubmitCompositorFrame(frame->local_surface_id, std::move(frame->frame), std::move(frame->hit_test_region_list), frame->submit_time);
                break;
            }

            case mojom::BundledFrameSubmissionData::Tag::kDidNotProduceFrame:
                sink->DidNotProduceFrame(submission->data->get_did_not_produce_frame());
                break;

            case mojom::BundledFrameSubmissionData::Tag::kDidDeleteSharedBitmap:
                sink->DidDeleteSharedBitmap(submission->data->get_did_delete_shared_bitmap());
                break;
            }
        }
    }
...
void FrameSinkBundleImpl::DidAllocateSharedBitmap(uint32_t sink_id, base::ReadOnlySharedMemoryRegion region, const SharedBitmapId& id)
{
    if (auto* sink = GetFrameSink(sink_id)) {
...
void FrameSinkBundleImpl::SetThreads(uint32_t sink_id, const std::vector<Thread>& threads)
{
    if (auto* sink = GetFrameSink(sink_id)) {
...
void FrameSinkBundleImpl::EnqueueDidReceiveCompositorFrameAck(uint32_t sink_id, std::vector<ReturnedResource> resources)
{
    if (auto* group = GetSinkGroup(sink_id)) {
        ...
        acks.push_back(mojom::BundledReturnedResources::New(sink_id, std::move(resources)));
        client_->FlushNotifications(std::move(acks), {}, {});
    }
}

void FrameSinkBundleImpl::EnqueueOnBeginFrame(uint32_t sink_id, const BeginFrameArgs& args, const base::flat_map<uint32_t, FrameTimingDetails>& details,
    bool frame_ack, std::vector<ReturnedResource> resources)
{
    if (auto* group = GetSinkGroup(sink_id)) {
        group->EnqueueOnBeginFrame(sink_id, args, details, frame_ack, std::move(resources));
    } else {
        // The sink has no BeginFrameSource at the moment and therefore does not
        // belong to a SinkGroup. Forward directly without batching.
        std::vector<mojom::BeginFrameInfoPtr> begin_frames;
        begin_frames.push_back(mojom::BeginFrameInfo::New(sink_id, args, details, frame_ack, std::move(resources)));
        client_->FlushNotifications({}, std::move(begin_frames), {});
    }
}

void FrameSinkBundleImpl::EnqueueReclaimResources(uint32_t sink_id, std::vector<ReturnedResource> resources)
{
    if (auto* group = GetSinkGroup(sink_id)) {
        ...
        reclaims.push_back(mojom::BundledReturnedResources::New(sink_id, std::move(resources)));
        client_->FlushNotifications({}, {}, std::move(reclaims));
    }
}

void FrameSinkBundleImpl::SendOnBeginFramePausedChanged(uint32_t sink_id, bool paused)
{
    client_->OnBeginFramePausedChanged(sink_id, paused);
}

void FrameSinkBundleImpl::SendOnCompositorFrameTransitionDirectiveProcessed(uint32_t sink_id, uint32_t sequence_id)
{
    client_->OnCompositorFrameTransitionDirectiveProcessed(sink_id, sequence_id);
}

void FrameSinkBundleImpl::RemoveFrameSinkImpl(BeginFrameSource* source, uint32_t sink_id)
{
    if (!source) {
...
CompositorFrameSinkImpl* FrameSinkBundleImpl::GetFrameSink(uint32_t sink_id) const
{
    return manager_->GetFrameSinkImpl(FrameSinkId(id_.client_id(), sink_id));
}

CompositorFrameSinkSupport* FrameSinkBundleImpl::GetFrameSinkSupport(uint32_t sink_id) const
{
    return manager_->GetFrameSinkForId(FrameSinkId(id_.client_id(), sink_id));
}

FrameSinkBundleImpl::SinkGroup* FrameSinkBundleImpl::GetSinkGroup(uint32_t sink_id) const
{
