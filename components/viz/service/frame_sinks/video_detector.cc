...
public:
    ...
    bool ReportDrawnAndCheckForVideo(Surface* surface, base::TimeTicks now)
    {
        ...
        damage = gfx::ScaleToEnclosingRect(damage, 1.f / frame.device_scale_factor());
        ...
        const bool in_video = (buffer_size_ == kMinFramesPerSecond) && (now - update_times_[buffer_start_] <= base::Seconds(1));
...
VideoDetector::VideoDetector(const std::vector<FrameSinkId>& registered_frame_sink_ids, SurfaceManager* surface_manager, const base::TickClock* tick_clock,
...
void VideoDetector::AddObserver(mojo::PendingRemote<mojom::VideoDetectorObserver> pending_observer)
{
    mojo::Remote<mojom::VideoDetectorObserver> observer(std::move(pending_observer));
...
bool VideoDetector::OnSurfaceDamaged(const SurfaceId& surface_id, const BeginFrameAck& ack, HandleInteraction handle_interaction)
{
    return false;
}
...
void VideoDetector::OnSurfaceWillBeDrawn(Surface* surface)
{
    ...
    if (it->second->ReportDrawnAndCheckForVideo(surface, now)) {
        ...
        // overhead.
        if (!video_inactive_timer_.IsRunning() || (video_inactive_timer_.desired_run_time() - now) < kMinVideoTimeout) {
            video_inactive_timer_.Start(FROM_HERE, kMaxVideoTimeout, this, &VideoDetector::OnVideoActivityEnded);
        }
        if (!video_is_playing_) {
