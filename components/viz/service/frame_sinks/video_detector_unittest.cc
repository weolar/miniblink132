...
public:
    VideoDetectorTest()
        : surface_aggregator_(frame_sink_manager_.surface_manager(), &resource_provider_, false)
    {
    }
    ...
    void SetUp() override
    {
        mock_task_runner_ = base::MakeRefCounted<base::TestMockTimeTaskRunner>(base::Time() + base::Seconds(1), base::TimeTicks() + base::Seconds(1));

        detector_ = frame_sink_manager_.CreateVideoDetectorForTesting(mock_task_runner_->GetMockTickClock(), mock_task_runner_);
        ...
        ParentLocalSurfaceIdAllocator* allocator = allocators_.GetAllocator(root_frame_sink_->frame_sink_id());
        allocator->GenerateId();
        root_frame_sink_->SubmitCompositorFrame(allocator->GetCurrentLocalSurfaceId(), MakeDefaultCompositorFrame());
    }

protected:
    // Constants placed here for convenience.
    static constexpr int kMinFps = VideoDetector::kMinFramesPerSecond;
    static constexpr gfx::Rect kMinRect = gfx::Rect(VideoDetector::kMinDamageWidth, VideoDetector::kMinDamageHeight);
    static constexpr base::TimeDelta kMinDuration = VideoDetector::kMinVideoDuration;
    ...
    {
        surface_aggregator_.Aggregate(root_frame_sink_->last_activated_surface_id(), mock_task_runner_->NowTicks(), gfx::OVERLAY_TRANSFORM_NONE);
    }
    ...
    void SubmitRootFrame()
    {
        ...
        for (CompositorFrameSinkSupport* frame_sink : embedded_clients_) {
            ...
            quad->SetNew(shared_quad_state, gfx::Rect(0, 0, 10, 10), gfx::Rect(0, 0, 5, 5), SurfaceRange(std::nullopt, frame_sink->last_activated_surface_id()),
                SkColors::kMagenta, /*stretch_content_to_fill_bounds=*/false);
        }
        root_frame_sink_->SubmitCompositorFrame(root_frame_sink_->last_activated_local_surface_id(), std::move(frame));
    }

    void SendUpdate(CompositorFrameSinkSupport* frame_sink, const gfx::Rect& damage, bool may_contain_video, bool use_per_quad_damage)
    {
        LocalSurfaceId local_surface_id = frame_sink->last_activated_local_surface_id();
        if (!local_surface_id.is_valid()) {
            ParentLocalSurfaceIdAllocator* allocator = allocators_.GetAllocator(frame_sink->frame_sink_id());
            allocator->GenerateId();
            local_surface_id = allocator->GetCurrentLocalSurfaceId();
        }
        frame_sink->SubmitCompositorFrame(local_surface_id,
            use_per_quad_damage ? MakeDamagedCompositorFrameWithPerQuadDamage(damage, may_contain_video)
    ...
    void SendUpdates(CompositorFrameSinkSupport* frame_sink, const gfx::Rect& damage, bool may_contain_video, bool use_per_quad_damage, int updates_per_second,
        base::TimeDelta duration)
    {
        const base::TimeDelta time_between_updates = base::Seconds(1.0 / updates_per_second);
    ...
    std::unique_ptr<CompositorFrameSinkSupport> CreateFrameSink()
    {
        ...
        frame_sink_manager_.RegisterFrameSinkId(frame_sink_id, true /* report_activation */);
        auto frame_sink = std::make_unique<CompositorFrameSinkSupport>(&frame_sink_client_, &frame_sink_manager_, frame_sink_id, is_root);
...
protected:
    CompositorFrame MakeDamagedCompositorFrame(const gfx::Rect& damage, bool may_contain_video)
    {
        constexpr gfx::Rect kFrameSinkRect(10000, 10000);
        auto frame = CompositorFrameBuilder().AddRenderPass(kFrameSinkRect, damage).Build();
        frame.metadata.may_contain_video = may_contain_video;

        return frame;
    }

    CompositorFrame MakeDamagedCompositorFrameWithPerQuadDamage(const gfx::Rect& damage, bool may_contain_video)
    {
        constexpr gfx::Rect kFrameSinkRect(10000, 10000);
        auto frame = CompositorFrameBuilder()
                         .AddRenderPass(RenderPassBuilder(kFrameSinkRect).AddTextureQuad(kFrameSinkRect, ResourceId(1234)).SetQuadDamageRect(kFrameSinkRect))
                         .PopulateResources()
                         .Build();
        frame.metadata.may_contain_video = may_contain_video;
    ...
    std::set<raw_ptr<CompositorFrameSinkSupport, SetExperimental>> embedded_clients_;
