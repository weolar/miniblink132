...
const base::UnguessableToken kSurfaceTokenA = base::UnguessableToken::CreateForTesting(3, 42);
const base::UnguessableToken kSurfaceTokenB = base::UnguessableToken::CreateForTesting(5, 42);
const base::UnguessableToken kSurfaceTokenC = base::UnguessableToken::CreateForTesting(7, 42);
...
MATCHER_P(ForResource, id, "is for resource " + base::NumberToString(id.GetUnsafeValue()))
{
    return arg.id == id;
}
...
struct TestRootFrameSink {
    TestRootFrameSink(FrameSinkManagerImpl& manager, BeginFrameSource& begin_frame_source)
        : manager_(manager)
    {
    ...
    ~TestRootFrameSink()
    {
        manager_->InvalidateFrameSinkId(kRootFrame);
    }
...
struct TestFrameSink {
    TestFrameSink(
        FrameSinkManagerImpl& manager, const FrameSinkId& id, const FrameSinkId& parent_id, const std::optional<FrameSinkBundleId>& bundle_id = std::nullopt)
        : manager_(manager)
        , id_(id)
    {
        ...
        manager_->CreateCompositorFrameSink(id, bundle_id, frame_sink.BindNewPipeAndPassReceiver(), client_receiver_.BindNewPipeAndPassRemote(),
    ...
    mojo::Receiver<mojom::CompositorFrameSinkClient> client_receiver_ { &mock_client_ };
...
public:
    ...
    void WaitForNextFlush(std::vector<mojom::BundledReturnedResourcesPtr>* acks, std::vector<mojom::BeginFrameInfoPtr>* begin_frames,
    ...
    void FlushNotifications(std::vector<mojom::BundledReturnedResourcesPtr> acks, std::vector<mojom::BeginFrameInfoPtr> begin_frames,
        std::vector<mojom::BundledReturnedResourcesPtr> reclaimed_resources) override
    {
        if (acks_) {
            *acks_ = std::move(acks);
        } else {
        ...
        if (reclaimed_resources_) {
            *reclaimed_resources_ = std::move(reclaimed_resources);
            ...
            EXPECT_TRUE(reclaimed_resources.empty()) << "Got unexpected ReclaimResources";
        }
...
public:
    FrameSinkBundleImplTest()
    {
        ...
        manager_.CreateFrameSinkBundle(kBundleId, bundle_.BindNewPipeAndPassReceiver(), client_receiver_.BindNewPipeAndPassRemote());
    }
    ...
    {
        begin_frame_source_.TestOnBeginFrame(begin_frame_source_.CreateBeginFrameArgs(BEGINFRAME_FROM_HERE));
    }

    mojom::BundledFrameSubmissionPtr CreateFrameSubmission(
        const FrameSinkId& frame_sink_id, const LocalSurfaceId& surface_id, std::vector<ResourceId> resource_ids = {})
    {
        ...
        auto data = mojom::BundledCompositorFrame::New(surface_id, std::move(frame), std::nullopt, 0);
        return mojom::BundledFrameSubmission::New(frame_sink_id.sink_id(), mojom::BundledFrameSubmissionData::NewFrame(std::move(data)));
    }

    mojom::BundledFrameSubmissionPtr CreateDidNotSubmitFrame(const FrameSinkId& frame_sink_id)
    {
        return mojom::BundledFrameSubmission::New(
            frame_sink_id.sink_id(), mojom::BundledFrameSubmissionData::NewDidNotProduceFrame(BeginFrameAck::CreateManualAckWithDamage()));
    }
    ...
    TestBundleClient& test_client()
    {
        return test_client_;
    }
    mojo::Remote<mojom::FrameSinkBundle>& bundle()
    {
        return bundle_;
    }
...
private:
    ...
    FrameSinkManagerImpl manager_ { FrameSinkManagerImpl::InitParams(&shared_bitmap_manager_, &output_surface_provider_) };
...
TEST_F(FrameSinkBundleImplTest, OnBeginFrame)
{
    ...
    EXPECT_THAT(begin_frames, UnorderedElementsAre(ForSink(kSubFrameA), ForSink(kSubFrameB), ForSink(kSubFrameC)));
    ...
    IssueOnBeginFrame();
    test_client().WaitForNextFlush(nullptr, &begin_frames, nullptr);
    EXPECT_THAT(begin_frames, UnorderedElementsAre(ForSink(kSubFrameA), ForSink(kSubFrameC)));
...
class OnBeginFrameAcksFrameSinkBundleImplTest : public FrameSinkBundleImplTest, public testing::WithParamInterface<bool> {
public:
...
OnBeginFrameAcksFrameSinkBundleImplTest::OnBeginFrameAcksFrameSinkBundleImplTest()
{
...
TEST_P(OnBeginFrameAcksFrameSinkBundleImplTest, SubmitAndAck)
{
    ...
    if (BeginFrameAcksEnabled()) {
        ...
        EXPECT_THAT(acks, ElementsAre(ForSink(kSubFrameA), ForSink(kSubFrameB), ForSink(kSubFrameC)));
...
TEST_P(OnBeginFrameAcksFrameSinkBundleImplTest, ReclaimResourcesOnAck)
{
    ...
    submissions.clear();
    submissions.push_back(CreateFrameSubmission(kSubFrameC, kSurfaceC, { resource }));
    bundle()->Submit(std::move(submissions));
...
INSTANTIATE_TEST_SUITE_P(
    , OnBeginFrameAcksFrameSinkBundleImplTest, testing::Bool(), [](auto& info) { return info.param ? "BeginFrameAcks" : "CompositoFrameAcks"; });
} // namespace
} // namespace viz
