...
SharedImageInterfaceProvider::SharedImageInterfaceProvider(GpuServiceImpl* gpu_service)
...
gpu::SharedImageInterface* SharedImageInterfaceProvider::GetSharedImageInterface()
{
...
void SharedImageInterfaceProvider::CreateSharedImageInterface()
{
    ...
    if (!scheduler_sequence_) {
        ...
        scheduler_sequence_ = std::make_unique<gpu::SchedulerSequence>(gpu_service_->GetGpuScheduler(), gpu_service_->main_runner(),
            /*target_thread_is_always_available=*/true);
    }

    base::ScopedAllowBaseSyncPrimitives allow_wait;
    base::WaitableEvent event;
    scheduler_sequence_->ScheduleTask(base::BindOnce(&SharedImageInterfaceProvider::CreateSharedImageInterfaceOnGpu, base::Unretained(this), &event),
        /*sync_token_fences=*/ {}, gpu::SyncToken());
    event.Wait();
}

void SharedImageInterfaceProvider::CreateSharedImageInterfaceOnGpu(base::WaitableEvent* event)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(gpu_sequence_checker_);
    base::AutoLock hold(context_lock_);
    shared_context_state_ = gl::GetGLImplementation() != gl::kGLImplementationDisabled && gpu_service_ ? gpu_service_->GetContextState() : nullptr;
    context_lost_ = false;

    shared_image_interface_ = base::MakeRefCounted<gpu::SharedImageInterfaceInProcess>(scheduler_sequence_.get(), gpu_service_->gpu_preferences(),
        ...
        /*is_for_diplay_compositor=*/false, gpu::SharedImageInterfaceInProcess::OwnerThread::kGpu);
