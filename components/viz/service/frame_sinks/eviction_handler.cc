...
EvictionHandler::EvictionHandler(Display* display, CompositorFrameSinkSupport* support, ReservedResourceIdTracker* id_tracker)
    : display_(display)
    , support_(support)
...
bool EvictionHandler::WillEvictSurface(const SurfaceId& surface_id)
{
    ...
    CHECK(!display_->visible()) << "Expected display to be not visible - in_progress_: " << in_progress_;
    ...
    // check.
    if (surface_id.local_surface_id().parent_sequence_number() >= current_surface_id.local_surface_id().parent_sequence_number()) {
        std::optional<double> snapshot_scale = 0.4;// features::SnapshotEvictedRootSurfaceScale();
...
void EvictionHandler::TakeSnapshotForEviction(const SurfaceId& to_evict, double scale)
{
    snapshot_seq_id_++;
    auto request = std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kNativeTextures,
        base::BindOnce(&EvictionHandler::SubmitPlaceholderContentForEviction, weak_factory_.GetWeakPtr(), to_evict, snapshot_seq_id_));

    auto current_surface_id = display_->CurrentSurfaceId();
    auto* surface = support_->frame_sink_manager()->surface_manager()->GetSurfaceForId(current_surface_id);

    auto src_rect = surface->size_in_pixels();
    auto dst_rect = gfx::ScaleToRoundedSize(src_rect, scale);
    request->SetScaleRatio(gfx::Vector2d(src_rect.width(), src_rect.height()), gfx::Vector2d(dst_rect.width(), dst_rect.height()));
    ...
    support_->frame_sink_manager()->RequestCopyOfOutput(current_surface_id, std::move(request),
...
std::optional<TransferableResource> EvictionHandler::CreateTransferableResourceFromCopyOutputResult(
    CopyOutputResult* copy_result, scoped_refptr<gpu::ClientSharedImage>& output_software_shared_image)
{
    ...
    if (copy_result->GetTextureResult()) {
        return TransferableResource::MakeGpu(copy_result->GetTextureResult()->mailbox, GL_TEXTURE_2D, gpu::SyncToken(), copy_result->size(),
            SinglePlaneFormat::kRGBA_8888,
            /*is_overlay_candidate=*/false, TransferableResource::ResourceSource::kStaleContent);
    } else if (copy_result->destination() == CopyOutputResult::Destination::kSystemMemory) {
        ...
        auto [shared_image, mapping] = sii->CreateSharedImage(
            { SinglePlaneFormat::kRGBA_8888, size, gfx::ColorSpace(), gpu::SHARED_IMAGE_USAGE_CPU_WRITE, "CopyOutputResultAsSoftwareSharedImage" });
        if (!shared_image) {
            return std::nullopt;
        }
        ...
        return TransferableResource::MakeSoftwareSharedImage(
            shared_image, shared_image->creation_sync_token(), size, SinglePlaneFormat::kRGBA_8888, TransferableResource::ResourceSource::kStaleContent);
    }
    return std::nullopt;
}

void EvictionHandler::SubmitPlaceholderContentForEviction(SurfaceId to_evict, int64_t snapshot_seq_id, std::unique_ptr<CopyOutputResult> copy_result)
{
    ...
    auto resource_opt = CreateTransferableResourceFromCopyOutputResult(copy_result.get(), software_shared_image);
    if (resource_opt) {
        ...
        texture_quad->SetNew(quad_state, surface_rect, surface_rect,
    ...
    support_->SubmitCompositorFrameLocally(current_surface_id, std::move(frame), display_->settings());
...
void EvictionHandler::ReceiveFromChild(const std::vector<TransferableResource>& resources)
{
...
void EvictionHandler::RefResources(const std::vector<TransferableResource>& resources)
{
    for (const auto& resource : resources) {
        if (copy_output_results_.contains(resource.id)) {
            id_tracker_->RefId(resource.id, /*count=*/1);
...
void EvictionHandler::UnrefResources(const std::vector<ReturnedResource>& resources)
{
    for (const auto& resource : resources) {
        if (copy_output_results_.contains(resource.id)) {
