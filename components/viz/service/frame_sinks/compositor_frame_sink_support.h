...
class VIZ_SERVICE_EXPORT CompositorFrameSinkSupport : public BeginFrameObserver,
                                                      public SurfaceResourceHolderClient,
                                                      public SurfaceClient,
                                                      public CapturableFrameSink {
public:
    using AggregatedDamageCallback = base::RepeatingCallback<void(
        const LocalSurfaceId& local_surface_id, const gfx::Size& frame_size_in_pixels, const gfx::Rect& damage_rect, base::TimeTicks expected_display_time)>;
    ...
    CompositorFrameSinkSupport& operator=(const CompositorFrameSinkSupport&) = delete;
    ...
    // android webview).
    void SetUpHitTest(LatestLocalSurfaceIdLookupDelegate* local_surface_id_lookup_delegate);
    ...
    base::TimeDelta GetPreferredFrameInterval(mojom::CompositorFrameSinkType* type) const;
    ...
    void SetThreads(bool from_untrusted_client, std::vector<Thread> unverified_threads);
    ...
    bool ThrottleBeginFrame(base::TimeDelta interval, bool simple_cadence_only = false);
    ...
    void RefResources(const std::vector<TransferableResource>& resources) override;
    ...
    void ReceiveFromChild(const std::vector<TransferableResource>& resources) override;
    // Takes the CopyOutputRequests that were requested for a surface with at
    // most |local_surface_id|.
    std::vector<PendingCopyOutputRequest> TakeCopyOutputRequests(const LocalSurfaceId& local_surface_id) override;
    void OnFrameTokenChanged(uint32_t frame_token) override;
    void SendCompositorFrameAck() override;
    void OnSurfaceAggregatedDamage(Surface* surface, const LocalSurfaceId& local_surface_id, const CompositorFrame& frame, const gfx::Rect& damage_rect,
        base::TimeTicks expected_display_time) override;
    void OnSurfacePresented(
        uint32_t frame_token, base::TimeTicks draw_start_timestamp, const gfx::SwapTimings& swap_timings, const gfx::PresentationFeedback& feedback) override;
    ...
    void SubmitCompositorFrame(const LocalSurfaceId& local_surface_id, CompositorFrame frame,
        std::optional<HitTestRegionList> hit_test_region_list = std::nullopt, uint64_t submit_time = 0);
    // Returns false if the notification was not valid (a duplicate).
    bool DidAllocateSharedBitmap(base::ReadOnlySharedMemoryRegion region, const SharedBitmapId& id);
    ...
    void SubmitCompositorFrameLocally(const SurfaceId& surface_id, CompositorFrame frame, const RendererSettings& settings);
    ...
    SubmitResult MaybeSubmitCompositorFrame(const LocalSurfaceId& local_surface_id, CompositorFrame frame,
    ...
    const std::vector<PendingCopyOutputRequest>& copy_output_requests_for_testing() const
    {
...
private:
    ...
    void ProcessCompositorFrameTransitionDirective(const CompositorFrameTransitionDirective& directive, Surface* surface);
    void OnSaveTransitionDirectiveProcessed(const CompositorFrameTransitionDirective& directive);
    ...
    void DidRejectCompositorFrame(uint32_t frame_token, std::vector<TransferableResource> frame_resource_list, std::vector<ui::LatencyInfo> latency_info);
    ...
    bool ShouldSendBeginFrame(base::TimeTicks timestamp, base::TimeDelta vsync_interval);
    ...
    bool ShouldThrottleBeginFrameAsRequested(base::TimeTicks frame_time, base::TimeDelta vsync_interval);
    ...
    void UpdateThreadIdsPostVerification(std::vector<Thread> threads, bool passed_verification);

    void ForAllReservedResourceDelegates(base::FunctionRef<void(ReservedResourceDelegate&)> func);

    /*const*/ raw_ptr<mojom::CompositorFrameSinkClient> client_;
    base::Lock client_lock_;
    bool need_notif_client_destroy_ = false;
    ...
    // frames.
    std::vector<raw_ptr<CapturableFrameSink::Client, VectorExperimental>> capture_clients_;
    ...
    mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback compositor_frame_callback_;
    ...
    public:
        PendingFrameDetails(base::TimeTicks frame_submit_timestamp, SurfaceManager* surface_manager);
    ...
    private:
        void OnAddedSurfaceReference(const SurfaceId& parent_id, const SurfaceId& child_id) override;
    ...
    base::flat_map<uint32_t, std::unique_ptr<PendingFrameDetails>> pending_received_frame_times_;
    ...
    // has been drawn.
    static_assert(kFrameIndexStart > 1, "|last_drawn_frame_index| relies on kFrameIndexStart > 1");
    ...
    mojom::CompositorFrameSinkType frame_sink_type_ = mojom::CompositorFrameSinkType::kUnspecified;

    base::flat_map<blink::ViewTransitionToken, std::unique_ptr<SurfaceAnimationManager>> view_transition_token_to_animation_manager_;
    ...
    raw_ptr<ReservedResourceDelegate> external_reserved_resource_delegate_ = nullptr;
