...
const base::UnguessableToken kArbitraryToken = base::UnguessableToken::CreateForTesting(1, 2);
const base::UnguessableToken kAnotherArbitraryToken = base::UnguessableToken::CreateForTesting(2, 2);
...
{
    gpu::SyncToken token;
    token.Set(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(id), 1);
    return token;
}

bool BeginFrameArgsAreEquivalent(const BeginFrameArgs& first, const BeginFrameArgs& second)
{
    return first.frame_id == second.frame_id;
}

std::string PostTestCaseNameTuple(const ::testing::TestParamInfo<std::tuple<bool, bool>>& info)
{
    return base::StringPrintf("%s_%s", std::get<0>(info.param) ? "BeginFrameAcks" : "CompositorFrameAcks",
        std::get<1>(info.param) ? "AckOnSurfaceActivationWhenInteractive" : "DoNotAckOnSurfaceActivationWhenInteractive");
}
...
public:
    ...
    MockFrameSinkManagerClient& operator=(const MockFrameSinkManagerClient&) = delete;

    ~MockFrameSinkManagerClient() override = default;

    // mojom::FrameSinkManagerClient:
    MOCK_METHOD1(OnFirstSurfaceActivation, void(const SurfaceInfo&));
    MOCK_METHOD3(OnFrameTokenChanged, void(const FrameSinkId&, uint32_t, base::TimeTicks));
...
public:
    ...
    void SetUp() override
    {
        manager_ = std::make_unique<FrameSinkManagerImpl>(FrameSinkManagerImpl::InitParams(&shared_bitmap_manager_));
        surface_observer_ = std::make_unique<FakeSurfaceObserver>(manager_->surface_manager());
        ...
        manager_->RegisterFrameSinkId(kArbitraryFrameSinkId, true /* report_activation */);
        manager_->SetSharedImageInterfaceProviderForTest(&shared_image_interface_provider_);
        support_ = std::make_unique<CompositorFrameSinkSupport>(&fake_support_client_, manager_.get(), kArbitraryFrameSinkId, kIsRoot);
    ...
    void AddResourcesToFrame(CompositorFrame* frame, ResourceId* resource_ids, size_t num_resource_ids)
    {
    ...
    void SubmitCompositorFrameWithResources(ResourceId* resource_ids, size_t num_resource_ids)
    {
        ...
        EXPECT_EQ(surface_observer_->last_created_surface_id().local_surface_id(), local_surface_id_);
    }

    bool SubmitCompositorFrameWithCopyRequest(CompositorFrame frame, std::unique_ptr<CopyOutputRequest> request)
    {
        frame.render_pass_list.back()->copy_requests.push_back(std::move(request));
        const auto result = support_->MaybeSubmitCompositorFrame(
            local_surface_id_, std::move(frame), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
        switch (result) {
        ...
        case SubmitResult::COPY_OUTPUT_REQUESTS_NOT_ALLOWED:
            return false;
        default:
            ADD_FAILURE() << "Test broken; fail result not related to copy requests: " << CompositorFrameSinkSupport::GetSubmitResultAsString(result);
            break;
        }
        return false;
    }

    void UnrefResources(ResourceId* ids_to_unref, int* counts_to_unref, size_t num_ids_to_unref)
    {
    ...
    void CheckReturnedResourcesMatchExpected(ResourceId* expected_returned_ids, size_t expected_resources)
    {
        const std::vector<ReturnedResource>& actual_resources = fake_support_client_.returned_resources();
    ...
    void CheckReturnedResourcesMatchExpected(
        ResourceId* expected_returned_ids, int* expected_returned_counts, size_t expected_resources, gpu::SyncToken expected_sync_token)
    {
        const std::vector<ReturnedResource>& actual_resources = fake_support_client_.returned_resources();
        ASSERT_EQ(expected_resources, actual_resources.size());
        for (size_t i = 0; i < expected_resources; ++i) {
    ...
    {
        Surface* surface = GetSurfaceForId(SurfaceId(support_->frame_sink_id(), local_surface_id_));
    ...
    const BeginFrameArgs& GetLastUsedBeginFrameArgs(const CompositorFrameSinkSupport* support) const
    {
        return support->LastUsedBeginFrameArgs();
    }

    void SendPresentationFeedback(CompositorFrameSinkSupport* support, uint32_t frame_token)
    {
        ...
        support->DidPresentCompositorFrame(frame_token, draw_time, timings,
            gfx::PresentationFeedback(base::TimeTicks::Now(), base::Milliseconds(16),
                /*flags=*/0));
    }
    ...
    void ProcessCompositorFrameTransitionDirective(CompositorFrameSinkSupport* support, const CompositorFrameTransitionDirective& directive, Surface* surface)
    {
        support->ProcessCompositorFrameTransitionDirective(directive, surface);
    }

    bool SupportHasSurfaceAnimationManager(CompositorFrameSinkSupport* support) const
    {
...
class CompositorFrameSinkSupportTest : public testing::WithParamInterface<bool>, public CompositorFrameSinkSupportTestBase {
public:
...
class OnBeginFrameAcksCompositorFrameSinkSupportTest : public CompositorFrameSinkSupportTestBase, public testing::WithParamInterface<std::tuple<bool, bool>> {
public:
    ...
    bool client_needs_begin_frame(const CompositorFrameSinkSupport* support) const
    {
...
OnBeginFrameAcksCompositorFrameSinkSupportTest::OnBeginFrameAcksCompositorFrameSinkSupportTest()
{
    std::vector<base::test::FeatureRef> enabled_features;
    std::vector<base::test::FeatureRef> disabled_features;
...
void OnBeginFrameAcksCompositorFrameSinkSupportTest::MaybeSendCompositorFrameAck()
{
    if (!BeginFrameAcksEnabled() || ShouldAckOnSurfaceActivationWhenInteractive()) {
        return;
    }
    support_->SendCompositorFrameAck();
}

void OnBeginFrameAcksCompositorFrameSinkSupportTest::MaybeTestOnBeginFrame(uint64_t sequence_number)
{
    if (!BeginFrameAcksEnabled()) {
        return;
    }
    BeginFrameArgs args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, sequence_number);
...
TEST_P(OnBeginFrameAcksCompositorFrameSinkSupportTest, ResourceLifetimeSimple)
{
    ...
    SubmitCompositorFrameWithResources(first_frame_ids, std::size(first_frame_ids));
    ...
    ResourceId expected_returned_ids[] = { ResourceId(1), ResourceId(2), ResourceId(3) };
    int expected_returned_counts[] = { 1, 1, 1 };
    // Resources were never consumed so no sync token should be set.
    CheckReturnedResourcesMatchExpected(expected_returned_ids, expected_returned_counts, std::size(expected_returned_counts), gpu::SyncToken());

    ResourceId third_frame_ids[] = { ResourceId(4), ResourceId(5), ResourceId(6) };
    SubmitCompositorFrameWithResources(third_frame_ids, std::size(third_frame_ids));
    ...
    SubmitCompositorFrameWithResources(forth_frame_ids, std::size(forth_frame_ids));

    MaybeTestOnBeginFrame(2);
    ResourceId forth_expected_returned_ids[] = { ResourceId(4), ResourceId(5), ResourceId(6) };
    int forth_expected_returned_counts[] = { 1, 1, 1 };
    // Resources were never consumed so no sync token should be set.
    CheckReturnedResourcesMatchExpected(
        forth_expected_returned_ids, forth_expected_returned_counts, std::size(forth_expected_returned_counts), gpu::SyncToken());
}

// Tests submitting a frame with resources followed by one with no resources
// with the resource provider holding everything alive.
TEST_P(OnBeginFrameAcksCompositorFrameSinkSupportTest, ResourceLifetimeSimpleWithProviderHoldingAlive)
{
    ResourceId first_frame_ids[] = { ResourceId(1), ResourceId(2), ResourceId(3) };
    SubmitCompositorFrameWithResources(first_frame_ids, std::size(first_frame_ids));
    MaybeSendCompositorFrameAck();
    ...
    ResourceId expected_returned_ids[] = { ResourceId(1), ResourceId(2), ResourceId(3) };
    int expected_returned_counts[] = { 1, 1, 1 };
    CheckReturnedResourcesMatchExpected(expected_returned_ids, expected_returned_counts, std::size(expected_returned_counts), consumer_sync_token_);
}

// Tests referencing a resource, unref'ing it to zero, then using it again
// before returning it to the client.
TEST_P(OnBeginFrameAcksCompositorFrameSinkSupportTest, ResourceReusedBeforeReturn)
{
    ...
    const std::vector<ReturnedResource>& returned = fake_support_client_.returned_resources();
...
// multiple providers.
TEST_P(OnBeginFrameAcksCompositorFrameSinkSupportTest, ResourceRefMultipleTimes)
{
    ...
    SubmitCompositorFrameWithResources(second_frame_ids, std::size(second_frame_ids));
...
TEST_P(OnBeginFrameAcksCompositorFrameSinkSupportTest, ResourceLifetime)
{
    ...
    ResourceId third_frame_ids[] = { ResourceId(10), ResourceId(11), ResourceId(12), ResourceId(13) };
    SubmitCompositorFrameWithResources(third_frame_ids, std::size(third_frame_ids));

    {
        SCOPED_TRACE("third frame");
        MaybeTestOnBeginFrame(2);
        ResourceId expected_returned_ids[] = { ResourceId(2), ResourceId(3), ResourceId(4) };
    ...
    SubmitCompositorFrameWithResources(fourth_frame_ids, std::size(fourth_frame_ids));
    ...
    // first.
    {
        ResourceId ids_to_unref[] = { ResourceId(10), ResourceId(11), ResourceId(12), ResourceId(13) };
        int counts[] = { 1, 1, 1, 1 };
...

    {
        ...
        MaybeTestOnBeginFrame(3);
        ResourceId expected_returned_ids[] = { ResourceId(10), ResourceId(11), ResourceId(12), ResourceId(13) };
...
{
    manager_->RegisterFrameSinkId(kAnotherArbitraryFrameSinkId, true /* report_activation */);
    MockCompositorFrameSinkClient mock_client;
    auto support = std::make_unique<CompositorFrameSinkSupport>(&mock_client, manager_.get(), kAnotherArbitraryFrameSinkId, kIsRoot);
    if (BeginFrameAcksEnabled()) {
    ...
    auto submit_compositor_frame = [&]() {
        ...
        support->SubmitCompositorFrame(new_id, MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
        surface_manager->GarbageCollectSurfaces();
    };

    LocalSurfaceId local_surface_id(6, kArbitraryToken);
    support->SubmitCompositorFrame(local_surface_id, MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));

    if (BeginFrameAcksEnabled()) {
        EXPECT_CALL(mock_client, DidReceiveCompositorFrameAck(_)).Times(0);
    } else {
        EXPECT_CALL(mock_client, DidReceiveCompositorFrameAck(_)).WillOnce(submit_compositor_frame).WillRepeatedly(testing::Return());
    }
...
TEST_P(CompositorFrameSinkSupportTest, MonotonicallyIncreasingLocalSurfaceIds)
{
    ...
    auto result = support->MaybeSubmitCompositorFrame(
        local_surface_id1, MakeDefaultInteractiveCompositorFrame(), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    ...
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id2, MakeDefaultInteractiveCompositorFrame(), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);

    // LocalSurfaceId(7, 2): Parent-initiated synchronization.
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id3, MakeDefaultInteractiveCompositorFrame(), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);

    // LocalSurfaceId(5, 3): Submit rejected because not monotonically increasing.
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id4, MakeDefaultInteractiveCompositorFrame(), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    ...
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id5, MakeDefaultInteractiveCompositorFrame(), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::SURFACE_ID_DECREASED, result);

    // LocalSurfaceId(9, 3): Parent AND child-initiated synchronization.
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id6, MakeDefaultInteractiveCompositorFrame(), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);
...
// rejected.
TEST_P(OnBeginFrameAcksCompositorFrameSinkSupportTest, ProhibitsUnprivilegedCopyRequests)
{
    manager_->RegisterFrameSinkId(kAnotherArbitraryFrameSinkId, true /* report_activation */);
    MockCompositorFrameSinkClient mock_client;
    auto support = std::make_unique<CompositorFrameSinkSupport>(&mock_client, manager_.get(), kAnotherArbitraryFrameSinkId, false /* not root frame sink */);

    bool did_receive_aborted_copy_result = false;
    base::RunLoop aborted_copy_run_loop;
    auto request = std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(
            [](bool* got_nothing, base::OnceClosure finished, std::unique_ptr<CopyOutputResult> result) {
                *got_nothing = result->IsEmpty();
                std::move(finished).Run();
            },
            &did_receive_aborted_copy_result, aborted_copy_run_loop.QuitClosure()));

    auto frame = MakeDefaultInteractiveCompositorFrame();
    ResourceId frame_resource_ids[] = { ResourceId(1), ResourceId(2), ResourceId(3) };
    AddResourcesToFrame(&frame, frame_resource_ids, std::size(frame_resource_ids));

    EXPECT_FALSE(SubmitCompositorFrameWithCopyRequest(std::move(frame), std::move(request)));
    ...
    CheckReturnedResourcesMatchExpected(frame_resource_ids, std::size(frame_resource_ids));

    manager_->InvalidateFrameSinkId(kAnotherArbitraryFrameSinkId);
}

// Tests doing an EvictLastActivatedSurface before shutting down the factory.
TEST_P(OnBeginFrameAcksCompositorFrameSinkSupportTest, EvictLastActivatedSurface)
{
    manager_->RegisterFrameSinkId(kAnotherArbitraryFrameSinkId, true /* report_activation */);
    MockCompositorFrameSinkClient mock_client;
    auto support = std::make_unique<CompositorFrameSinkSupport>(&mock_client, manager_.get(), kAnotherArbitraryFrameSinkId, kIsRoot);
    ...
    EXPECT_EQ(surface_observer_->last_created_surface_id().local_surface_id(), local_surface_id);
    ...
    if (BeginFrameAcksEnabled()) {
        EXPECT_CALL(mock_client, DidReceiveCompositorFrameAck(_)).Times(0);
        EXPECT_CALL(mock_client, ReclaimResources(_)).WillOnce(expected_returned_resources);
    } else {
        EXPECT_CALL(mock_client, DidReceiveCompositorFrameAck(_)).WillOnce(expected_returned_resources);
    }
    support->EvictSurface(local_surface_id);
    ExpireAllTemporaryReferences();
...
TEST_P(CompositorFrameSinkSupportTest, ResurectAndImmediatelyEvict)
{
    ...
    auto frame = CompositorFrameBuilder().AddDefaultRenderPass().SetIsHandlingInteraction(true).Build();
    ...
    frame = CompositorFrameBuilder().AddDefaultRenderPass().SetIsHandlingInteraction(true).Build();
    support_->SubmitCompositorFrame(local_surface_id, std::move(frame));
...
TEST_P(CompositorFrameSinkSupportTest, EvictSurfaceWithTemporaryReference)
{
    ...
    manager_->RegisterFrameSinkId(parent_frame_sink_id, true /* report_activation */);
    ...
    support_->SubmitCompositorFrame(local_surface_id, MakeDefaultInteractiveCompositorFrame());
...
TEST_P(CompositorFrameSinkSupportTest, EvictOlderSurfaces)
{
    ...
    // When CompositorFrame is submitted, a temporary reference will be created.
    support_->SubmitCompositorFrame(local_surface_id1, MakeDefaultInteractiveCompositorFrame());
...
void CopyRequestTestCallback(bool* called, base::OnceClosure finished, std::unique_ptr<CopyOutputResult> result)
{
    *called = true;
    std::move(finished).Run();
}
...
TEST_P(CompositorFrameSinkSupportTest, CopyRequestOnSubtree)
{
    ...
    auto request = std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(&CopyRequestTestCallback, &called1, called1_run_loop.QuitClosure()));
    support_->RequestCopyOfOutput({ local_surface_id_, kSubtreeId1, std::move(request) });
    ...
    request = std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(&CopyRequestTestCallback, &called2, called2_run_loop.QuitClosure()));
    support_->RequestCopyOfOutput({ local_surface_id_, kSubtreeId2, std::move(request) });
    GetSurfaceForId(surface_id)->TakeCopyOutputRequestsFromClient();
...
TEST_P(CompositorFrameSinkSupportTest, DuplicateCopyRequest)
{
    ...
    {
        auto frame = CompositorFrameBuilder().AddDefaultRenderPass().SetReferencedSurfaces({ SurfaceRange(surface_id) }).SetIsHandlingInteraction(true).Build();
        support_->SubmitCompositorFrame(local_surface_id_, std::move(frame));
        EXPECT_EQ(surface_observer_->last_created_surface_id().local_surface_id(), local_surface_id_);
    }
    ...
    support_->RequestCopyOfOutput({ local_surface_id_, SubtreeCaptureId(), std::move(request) });
    GetSurfaceForId(surface_id)->TakeCopyOutputRequestsFromClient();
    EXPECT_FALSE(called1);
    ...
    request = std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(&CopyRequestTestCallback, &called3, called3_run_loop.QuitClosure()));
    request->set_source(source_id1);
...
TEST_P(CompositorFrameSinkSupportTest, SurfaceInfo)
{
    ...
    EXPECT_EQ(gfx::Size(7, 8), surface_observer_->last_surface_info().size_in_pixels());
}
...
TEST_P(OnBeginFrameAcksCompositorFrameSinkSupportTest, FrameSizeMismatch)
{
    ...
    auto frame = CompositorFrameBuilder().AddRenderPass(gfx::Rect(5, 5), gfx::Rect()).SetIsHandlingInteraction(true).Build();
    ...
    frame = CompositorFrameBuilder().AddRenderPass(gfx::Rect(5, 4), gfx::Rect()).SetIsHandlingInteraction(true).Build();
    ResourceId frame_resource_ids[] = { ResourceId(1), ResourceId(2), ResourceId(3) };
    AddResourcesToFrame(&frame, frame_resource_ids, std::size(frame_resource_ids));
...
TEST_P(CompositorFrameSinkSupportTest, DeviceScaleFactorMismatch)
{
    ...
    auto frame = CompositorFrameBuilder().AddDefaultRenderPass().SetDeviceScaleFactor(0.5f).SetIsHandlingInteraction(true).Build();
    auto result = support_->MaybeSubmitCompositorFrame(
        local_surface_id_, std::move(frame), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);
    EXPECT_TRUE(GetSurfaceForId(id));
    ...
    frame = CompositorFrameBuilder().AddDefaultRenderPass().SetDeviceScaleFactor(0.4f).SetIsHandlingInteraction(true).Build();
    result = support_->MaybeSubmitCompositorFrame(
        local_surface_id_, std::move(frame), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::SIZE_MISMATCH, result);
}
...
TEST_P(CompositorFrameSinkSupportTest, PassesOnBeginFrameAcks)
{
    ...
    // Issue a BeginFrame.
    BeginFrameArgs args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 1);
    begin_frame_source_.TestOnBeginFrame(args);
    ...
    CompositorFrame frame = CompositorFrameBuilder().AddDefaultRenderPass().SetBeginFrameAck(ack).SetIsHandlingInteraction(true).Build();
    support_->SubmitCompositorFrame(local_surface_id_, std::move(frame));
...
TEST_P(OnBeginFrameAcksCompositorFrameSinkSupportTest, NeedsBeginFrameResetAfterPresentationFeedback)
{
    // Request BeginFrames.
    support_->SetNeedsBeginFrame(true);
...
TEST_P(OnBeginFrameAcksCompositorFrameSinkSupportTest, AutoNeedsBeginFrameOnUnsolicitedFrame)
{
    support_->SetAutoNeedsBeginFrame();

    EXPECT_FALSE(client_needs_begin_frame(support_.get()));

    BeginFrameArgs args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 2, 1);
    begin_frame_source_.TestOnBeginFrame(args);

    EXPECT_EQ(fake_support_client_.begin_frame_count(), 0);

    CompositorFrame unsolicited_frame = MakeDefaultInteractiveCompositorFrame(BeginFrameArgs::kManualSourceId);
    support_->SubmitCompositorFrame(local_surface_id_, std::move(unsolicited_frame));
...
TEST_P(CompositorFrameSinkSupportTest, FrameIndexCarriedOverToNewSurface)
{
    ...
    support_->SubmitCompositorFrame(local_surface_id2, MakeDefaultInteractiveCompositorFrame());
...
// activated surface.
TEST_P(CompositorFrameSinkSupportTest, OldSurfaceTakesCopyOutputRequestsFromClient)
{
    LocalSurfaceId local_surface_id1(1, kArbitraryToken);
    LocalSurfaceId local_surface_id2(2, kArbitraryToken);
    SurfaceId id1(support_->frame_sink_id(), local_surface_id1);
    ...
    auto request = std::make_unique<CopyOutputRequest>(
        CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory, base::BindOnce(StubResultCallback));
    support_->RequestCopyOfOutput({ local_surface_id1, SubtreeCaptureId(), std::move(request) });
...
TEST_P(CompositorFrameSinkSupportTest, OldSurfaceDoesNotTakeCopyOutputRequestsFromNewLocalId)
{
    LocalSurfaceId local_surface_id1(1, kArbitraryToken);
    LocalSurfaceId local_surface_id2(2, kArbitraryToken);
    SurfaceId id1(support_->frame_sink_id(), local_surface_id1);
    ...
    auto request = std::make_unique<CopyOutputRequest>(
        CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory, base::BindOnce(StubResultCallback));
    support_->RequestCopyOfOutput({ local_surface_id2, SubtreeCaptureId(), std::move(request) });
...
// activated surface.
TEST_P(CompositorFrameSinkSupportTest, LastSurfaceTakesCopyOutputRequestsFromClient)
{
    LocalSurfaceId local_surface_id1(1, kArbitraryToken);
    LocalSurfaceId local_surface_id2(2, kArbitraryToken);
    SurfaceId id1(support_->frame_sink_id(), local_surface_id1);
...
TEST_P(CompositorFrameSinkSupportTest, OnFrameTokenUpdateAfterFirstSurfaceActivation)
{
    LocalSurfaceId local_surface_id(1, kArbitraryToken);
    auto frame = CompositorFrameBuilder().AddDefaultRenderPass().SetSendFrameTokenToEmbedder(true).SetIsHandlingInteraction(true).Build();
    uint32_t frame_token = frame.metadata.frame_token;
    ASSERT_NE(frame_token, 0u);

    testing::InSequence sequence;
    EXPECT_CALL(frame_sink_manager_client_, OnFirstSurfaceActivation(_));
    EXPECT_CALL(frame_sink_manager_client_, OnFrameTokenChanged(_, frame_token, _));
    support_->SubmitCompositorFrame(local_surface_id, std::move(frame));
}
...
// requested.
TEST_P(CompositorFrameSinkSupportTest, OnlyExactSurfaceCanTakeExactOutputRequest)
{
    LocalSurfaceId local_surface_id1(1, kArbitraryToken);
    LocalSurfaceId local_surface_id2(2, kArbitraryToken);
    SurfaceId id1(support_->frame_sink_id(), local_surface_id1);
    ...
    // or Surface2.
    support_->RequestCopyOfOutput({ local_surface_id1, SubtreeCaptureId(),
...
TEST_P(CompositorFrameSinkSupportTest, OnFrameTokenUpdate)
{
    ...
    EXPECT_CALL(frame_sink_manager_client_, OnFrameTokenChanged(_, _, _)).Times(0);
...
TEST_P(CompositorFrameSinkSupportTest, DisallowEmbedTokenReuseAcrossFrameSinks)
{
    auto result = support_->MaybeSubmitCompositorFrame(
        local_surface_id_, MakeDefaultInteractiveCompositorFrame(), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);
    ...
    result = support->MaybeSubmitCompositorFrame(
        local_surface_id, MakeDefaultInteractiveCompositorFrame(), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
...
TEST_P(CompositorFrameSinkSupportTest, SubmitAfterReparenting)
{
    ...
    CompositorFrame frame = CompositorFrameBuilder().AddDefaultRenderPass().SetIsHandlingInteraction(true).Build();
    SubmitResult result = support_->MaybeSubmitCompositorFrame(
        local_surface_id1, std::move(frame), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);

    frame = CompositorFrameBuilder().AddDefaultRenderPass().SetIsHandlingInteraction(true).Build();
    result = support_->MaybeSubmitCompositorFrame(
        local_surface_id2, std::move(frame), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
...
TEST_P(CompositorFrameSinkSupportTest, EvictThenReparent)
{
    ...
    // embed token.
    EXPECT_TRUE(GetSurfaceForId(SurfaceId(support_->frame_sink_id(), local_surface_id2)));
}
...
TEST_P(CompositorFrameSinkSupportTest, HitTestRegionValidation)
{
    ...
    auto support = std::make_unique<CompositorFrameSinkSupport>(&fake_support_client_, manager_.get(), frame_sink_id, kIsRoot);
    ...
    EXPECT_EQ(manager_->hit_test_manager()->submit_hit_test_region_list_index(), 0u);
    support->MaybeSubmitCompositorFrame(
        local_surface_id, MakeDefaultInteractiveCompositorFrame(), hit_test_region_list, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    // hit_test_region_1 is valid. Submitted region count increases.
    EXPECT_EQ(manager_->hit_test_manager()->submit_hit_test_region_list_index(), 1u);
    ...
    hit_test_region_3.async_hit_test_reasons = AsyncHitTestReasons::kOverlappedRegion;
    ...
    hit_test_region_4.async_hit_test_reasons = AsyncHitTestReasons::kOverlappedRegion;
    ...
    EXPECT_EQ(manager_->hit_test_manager()->submit_hit_test_region_list_index(), 2u);
}
...
TEST_P(CompositorFrameSinkSupportTest, ThrottleUnresponsiveClient)
{
    ...
    MockCompositorFrameSinkClient mock_client;
    auto support = std::make_unique<CompositorFrameSinkSupport>(&mock_client, manager_.get(), kAnotherArbitraryFrameSinkId,
        /*is_root=*/true);
    ...
    for (; sent_frames < BeginFrameTracker::kLimitThrottle; ++sent_frames) {
        ...
        args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, sequence_number++, frametime);
...
TEST_P(CompositorFrameSinkSupportTest, BeginFrameInterval)
{
    ...
    base::TimeDelta non_perfect_throttled_interval = base::Seconds(1) / non_perfect_cadence_fps;
    bool did_throttle = support->ThrottleBeginFrame(non_perfect_throttled_interval, /*perfect_cadence*/ true);
    ...
    did_throttle = support->ThrottleBeginFrame(throttled_interval, /*perfect_cadence*/ true);
    ...
    constexpr base::TimeDelta interval = BeginFrameArgs::DefaultInterval();
    const int num_expected_skipped_frames = (base::ClampRound<int>(interval.ToHz()) / fps) - 1;
    ...
    while (frame_time < end_time) {
        args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, sequence_number++, frame_time);

        BeginFrameArgs expected_args(args);
        expected_args.interval = throttled_interval;
        expected_args.deadline = frame_time + throttled_interval - BeginFrameArgs::DefaultEstimatedDisplayDrawTime(interval);
        expected_args.frames_throttled_since_last = frames_throttled_since_last;
        bool sent_frame = false;
        ON_CALL(mock_client, OnBeginFrame(_, _, _, _))
            .WillByDefault([&](const BeginFrameArgs& actual_args, const FrameTimingDetailsMap&, bool frame_ack, std::vector<ReturnedResource>) {
                EXPECT_THAT(actual_args, Eq(expected_args));
                support->SubmitCompositorFrame(local_surface_id_, MakeDefaultInteractiveCompositorFrame());
                ...
                if (!frame_time.is_null()) {
                    EXPECT_THAT(frames_throttled_since_last, Eq(num_expected_skipped_frames));
                }
...
TEST_P(CompositorFrameSinkSupportTest, HandlesSmallErrorInBeginFrameTimes)
{
    ...
    EXPECT_CALL(mock_client, OnBeginFrame(_, _, _, _)).WillOnce(submit_compositor_frame);
    begin_frame_source.TestOnBeginFrame(CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, sequence_number++, frame_time));
    testing::Mock::VerifyAndClearExpectations(&mock_client);
    if (!ShouldAckOnSurfaceActivationWhenInteractive()) {
        support->SendCompositorFrameAck();
    }
...
TEST_P(CompositorFrameSinkSupportTest, UsesThrottledIntervalInPresentationFeedback)
{
    ...
    // Issue a BeginFrame.
    BeginFrameArgs args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, 1, frame_time);
    begin_frame_source_.TestOnBeginFrame(args);
    ...
    ASSERT_THAT(fake_support_client_.all_frame_timing_details(), Contains(Key(token)));
    EXPECT_THAT(fake_support_client_.all_frame_timing_details().at(token).presentation_feedback.interval, Eq(kThrottledFrameInterval));
}
...
TEST_P(CompositorFrameSinkSupportTest, ForceFullFrameToActivateSurface)
{
    ...
    BeginFrameArgs args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, sequence_number, frame_time);
    ...
    EXPECT_CALL(mock_client, OnBeginFrame(testing::Field(&BeginFrameArgs::animate_only, testing::IsFalse()), _, _, _));
    begin_frame_source.TestOnBeginFrame(args_animate_only);
}

TEST_P(CompositorFrameSinkSupportTest, ReleaseTransitionDirectiveClearsFrameSinkManagerEntry)
{
    auto result = support_->MaybeSubmitCompositorFrame(
        local_surface_id_, MakeDefaultInteractiveCompositorFrame(), std::nullopt, 0, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    EXPECT_EQ(SubmitResult::ACCEPTED, result);
    ...
    std::unique_ptr<SurfaceAnimationManager> animation_manager
        = SurfaceAnimationManager::CreateWithSave(CompositorFrameTransitionDirective::CreateSave(transition_token, maybe_cross_frame_sink,
                                                      /*sequence_id=*/1, {}, {}),
            surface, &shared_bitmap_manager_, sii, &id_tracker, base::DoNothing());
    ASSERT_TRUE(animation_manager);

    EXPECT_FALSE(HasAnimationManagerForToken(transition_token));
    manager_->CacheSurfaceAnimationManager(transition_token, std::move(animation_manager));
    EXPECT_TRUE(HasAnimationManagerForToken(transition_token));

    auto release_directive = CompositorFrameTransitionDirective::CreateRelease(transition_token, maybe_cross_frame_sink, /*sequence_id=*/2);
    ProcessCompositorFrameTransitionDirective(support_.get(), release_directive, surface);
    EXPECT_FALSE(HasAnimationManagerForToken(transition_token));
...
TEST_P(CompositorFrameSinkSupportTest, ViewTransitionBlitRequestTextureQuad)
{
    ...
    auto* vt_quad = root_render_pass->CreateAndAppendDrawQuad<SharedElementDrawQuad>();
    ...
    auto* solid_quad = orphan_render_pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
    ...
    frame.metadata.transition_directives.push_back(CompositorFrameTransitionDirective::CreateSave(transition_token,
...
TEST_P(CompositorFrameSinkSupportTest, GetRequestRegionProperties_NoSurfaceWithActiveFrame)
{
    const auto props = support_->GetRequestRegionProperties(VideoCaptureSubTarget());
    EXPECT_EQ(std::nullopt, props);
}

TEST_P(CompositorFrameSinkSupportTest, GetRequestRegionProperties_SurfaceWithNoCaptureIdentifier)
{
    ResourceId first_frame_ids[] = { ResourceId(1), ResourceId(2), ResourceId(3), ResourceId(4), ResourceId(5) };
    SubmitCompositorFrameWithResources(first_frame_ids, std::size(first_frame_ids));
    const auto props_with_frame = support_->GetRequestRegionProperties(VideoCaptureSubTarget());
...
TEST_P(CompositorFrameSinkSupportTest, GetRequestRegionProperties_RenderPassWithSubtreeSize)
{
    ...
    const auto props_with_subtree = support_->GetRequestRegionProperties(kSubtreeId);
...
TEST_P(CompositorFrameSinkSupportTest, GetRequestRegionProperties_RenderPassWithNoSubtreeSize)
{
    ...
    frame.metadata.capture_bounds = RegionCaptureBounds { { { kSubtreeId.subtree_id(), gfx::Rect { 5, 6, 15, 14 } } } };
    ...
    support_->OnSurfaceAggregatedDamage(
        /*surface*/ nullptr, local_surface_id_, frame, kDefaultOutputRect, base::TimeTicks::Now());

    support_->SubmitCompositorFrame(local_surface_id_, std::move(frame));
    const auto region_properties = support_->GetRequestRegionProperties(kSubtreeId);
...
TEST_P(CompositorFrameSinkSupportTest, GetRequestRegionProperties_RenderPassWithNoSubtreeSizeShouldClipToViewport)
{
    constexpr SubtreeCaptureId kSubtreeId(base::Token(0, 7u));
    const SurfaceId surface_id(support_->frame_sink_id(), local_surface_id_);
    ...
    frame.metadata.capture_bounds = RegionCaptureBounds { { { kSubtreeId.subtree_id(), gfx::Rect { 12, 10, 15, 14 } } } };
...
TEST_P(CompositorFrameSinkSupportTest, GetRequestRegionProperties_RenderPassWithCaptureBounds)
{
    const SurfaceId surface_id(support_->frame_sink_id(), local_surface_id_);
    const auto crop_id = RegionCaptureCropId::CreateRandom();

    auto frame_with_crop_id = CompositorFrameBuilder()
                                  .AddDefaultRenderPass()
                                  .AddDefaultRenderPass()
                                  .SetReferencedSurfaces({ SurfaceRange(surface_id) })
                                  .SetIsHandlingInteraction(true)
    ...
    support_->SubmitCompositorFrame(local_surface_id_, std::move(frame_with_crop_id));

    // No capture bounds are set, so we shouldn't capture anything.
    const auto props_without_capture_bounds = support_->GetRequestRegionProperties(crop_id);
    EXPECT_FALSE(props_without_capture_bounds);

    // After setting capture bounds, we should be able to crop to it.
    auto frame_with_crop_id_and_bounds = CompositorFrameBuilder()
                                             .AddDefaultRenderPass()
                                             .AddDefaultRenderPass()
                                             .SetReferencedSurfaces({ SurfaceRange(surface_id) })
                                             .SetIsHandlingInteraction(true)
                                             .Build();
    frame_with_crop_id_and_bounds.render_pass_list.back()->output_rect = kDefaultOutputRect;
    frame_with_crop_id_and_bounds.metadata.capture_bounds = RegionCaptureBounds { { { crop_id, gfx::Rect { 0, 0, 13, 13 } } } };

    // Mark the surface as damaged to update the capture bounds.
    support_->OnSurfaceAggregatedDamage(
        /*surface*/ nullptr, local_surface_id_, frame_with_crop_id_and_bounds, kDefaultOutputRect, base::TimeTicks::Now());
...
INSTANTIATE_TEST_SUITE_P(, CompositorFrameSinkSupportTest, testing::Bool(), &PostTestCaseNameBool);

INSTANTIATE_TEST_SUITE_P(, OnBeginFrameAcksCompositorFrameSinkSupportTest, testing::Combine(testing::Bool(), testing::Bool()), &PostTestCaseNameTuple);
} // namespace viz
