...
FrameCounter::FrameCounter(base::TimeTicks start_time, base::TimeDelta bucket_size)
...
void FrameCounter::AddFrameSink(const FrameSinkId& frame_sink_id, mojom::CompositorFrameSinkType type, bool is_root, std::string_view debug_label)
{
    DCHECK(!base::Contains(frame_sink_data_, frame_sink_id));

    auto per_sink_data = mojom::FrameCountingPerSinkData::New(type, is_root, static_cast<std::string>(debug_label), 0, std::vector<uint16_t>());
    per_sink_data->presented_frames.reserve(kMaxFrameRecords);

    frame_sink_data_[frame_sink_id] = std::move(per_sink_data);
}

void FrameCounter::AddPresentedFrame(const FrameSinkId& frame_sink_id, base::TimeTicks present_timestamp)
{
    ...
    size_t bucket_index = (present_timestamp - start_time_).InSeconds() / bucket_size_.InSeconds();
...
void FrameCounter::SetFrameSinkType(const FrameSinkId& frame_sink_id, mojom::CompositorFrameSinkType type)
{
    frame_sink_data_[frame_sink_id]->type = type;
}

void FrameCounter::SetFrameSinkDebugLabel(const FrameSinkId& frame_sink_id, std::string_view debug_label)
{
