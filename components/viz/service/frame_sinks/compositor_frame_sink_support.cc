...
bool HasElapsedCadenceInterval(base::TimeDelta render_interval, base::TimeDelta frame_duration, base::TimeDelta time_since_last_render_interval)
{
    base::TimeDelta time_at_next_interval = time_since_last_render_interval + render_interval;
    base::TimeDelta tolerance = render_interval * 0.5;
    return time_at_next_interval > (frame_duration + tolerance);
}

bool media::VideoCadenceEstimator::HasSimpleCadence(base::TimeDelta, base::TimeDelta, base::TimeDelta)
{
    *(int*)1 = 1;
    return false;
}
...
{
    TRACE_EVENT2("viz", "SendBeginFrameDecision", "reason", reason, "should_send", should_send);
    return should_send;
}

void AdjustPresentationFeedback(gfx::PresentationFeedback* feedback, base::TimeTicks swap_start)
{
    ...
    feedback->available_timestamp = std::max(feedback->available_timestamp, swap_start);
    feedback->latch_timestamp = std::max(feedback->latch_timestamp, feedback->ready_timestamp);
}
...
// invertible.
gfx::Rect IntersectInSpace(const gfx::Rect& rect, const gfx::Transform& transform_to_space, const gfx::Rect& intersectee_in_space)
{
    const gfx::Rect rect_in_space = transform_to_space.MapRect(rect);

    const gfx::Rect intersected_in_space = gfx::IntersectRects(rect_in_space, intersectee_in_space);

    const std::optional<gfx::Rect> intersected = transform_to_space.InverseMapRect(intersected_in_space);

    return intersected.value_or(gfx::Rect {});
}

void RemoveSurfaceReferenceAndDispatchCopyOutputRequestCallback(base::WeakPtr<FrameSinkManagerImpl> frame_sink_manager, const SurfaceId& holds_ref_surface_id,
    const blink::SameDocNavigationScreenshotDestinationToken& destination_token, std::unique_ptr<CopyOutputResult> result)
{
    if (!frame_sink_manager) {
        return;
    }
    if (auto* surface_holds_ref = frame_sink_manager->surface_manager()->GetSurfaceForId(holds_ref_surface_id)) {
    ...
    frame_sink_manager->OnScreenshotCaptured(destination_token, std::move(result));
}

} // namespace

CompositorFrameSinkSupport::CompositorFrameSinkSupport(
    mojom::CompositorFrameSinkClient* client, FrameSinkManagerImpl* frame_sink_manager, const FrameSinkId& frame_sink_id, bool is_root)
    : client_(client)
    ...
    , use_blit_request_for_view_transition_(base::FeatureList::IsEnabled(features::kBlitRequestsForViewTransition))
{
    ...
    if (client_) // for single process viz
        need_notif_client_destroy_ = client_->SetServer(this);
}

CompositorFrameSinkSupport::~CompositorFrameSinkSupport()
{
    base::AutoLock locker(client_lock_);
    if (need_notif_client_destroy_ && client_)
        client_->OnServerDestroy();
    ...
    if (referenced_local_surface_id_.has_value()) {
        auto reference = MakeTopLevelRootReference(SurfaceId(frame_sink_id_, referenced_local_surface_id_.value()));
...
void CompositorFrameSinkSupport::SetUpHitTest(LatestLocalSurfaceIdLookupDelegate* local_surface_id_lookup_delegate)
{
    DCHECK(is_root_);
    hit_test_aggregator_
        = std::make_unique<HitTestAggregator>(frame_sink_manager_->hit_test_manager(), frame_sink_manager_, local_surface_id_lookup_delegate, frame_sink_id_);
}

void CompositorFrameSinkSupport::SetAggregatedDamageCallbackForTesting(AggregatedDamageCallback callback)
{
    aggregated_damage_callback_ = std::move(callback);
}

void CompositorFrameSinkSupport::SetBeginFrameSource(BeginFrameSource* begin_frame_source)
{
    if (begin_frame_source_ && added_frame_observer_) {
        begin_frame_source_->RemoveObserver(this);
...
void CompositorFrameSinkSupport::SetLastKnownVsync(base::TimeDelta vsync_interval)
{
...
bool CompositorFrameSinkSupport::ThrottleBeginFrame(base::TimeDelta interval, bool simple_cadence_only)
{
    ...
    if (media::VideoCadenceEstimator::HasSimpleCadence(last_known_vsync_interval_, interval, kMaxTimeUntilNextGlitch)) {
        begin_frame_interval_ = interval;
        return true;
    } else {
        begin_frame_interval_ = base::TimeDelta();
...
void CompositorFrameSinkSupport::ApplyPreferredFrameRate(uint64_t source_id)
{
    ...
    if ((last_known_frame_interval_ - preferred_frame_interval_).magnitude() > base::Milliseconds(2)) {
        TRACE_EVENT_INSTANT2("viz", "Set sink framerate", TRACE_EVENT_SCOPE_THREAD, "interval", preferred_frame_interval_, "sourceid", source_id);
...
void CompositorFrameSinkSupport::OnSurfaceActivated(Surface* surface)
{
    ...
    for (const auto& directive : surface->GetActiveFrameMetadata().transition_directives) {
    ...
    SurfaceAnimationManager::ReplaceSharedElementResources(surface, view_transition_token_to_animation_manager_);

    if (surface->surface_id() == last_activated_surface_id_)
        return;

    Surface* previous_surface = surface_manager_->GetSurfaceForId(last_activated_surface_id_);

    if (!previous_surface) {
        last_activated_surface_id_ = surface->surface_id();
    } else if (previous_surface->GetActiveFrameIndex() < surface->GetActiveFrameIndex()) {
        ...
        DCHECK_GT(previous_surface->GetActiveFrameIndex(), surface->GetActiveFrameIndex());
    ...
    if (is_root_ && (!referenced_local_surface_id_ || *referenced_local_surface_id_ != last_activated_surface_id_.local_surface_id())) {
...
void CompositorFrameSinkSupport::OnSurfaceAggregatedDamage(
    Surface* surface, const LocalSurfaceId& local_surface_id, const CompositorFrame& frame, const gfx::Rect& damage_rect, base::TimeTicks expected_display_time)
{
    DCHECK(!damage_rect.IsEmpty());

    const gfx::Size& frame_size_in_pixels = frame.size_in_pixels();
    if (aggregated_damage_callback_) {
        aggregated_damage_callback_.Run(local_surface_id, frame_size_in_pixels, damage_rect, expected_display_time);
    }

    current_capture_bounds_ = frame.metadata.capture_bounds;
    for (CapturableFrameSink::Client* client : capture_clients_) {
        client->OnFrameDamaged(frame_size_in_pixels, damage_rect, expected_display_time, frame.metadata);
    }
}
...
void CompositorFrameSinkSupport::OnSurfaceDestroyed(Surface* surface)
{
    ...
    if (!ShouldMergeBeginFrameWithAcks() || !client_ || surface_returned_resources_.empty()) {
        return;
    }
...
void CompositorFrameSinkSupport::OnSurfacePresented(
    uint32_t frame_token, base::TimeTicks draw_start_timestamp, const gfx::SwapTimings& swap_timings, const gfx::PresentationFeedback& feedback)
{
    ...
    if (frame_token != kLocalFrameToken) {
        DidPresentCompositorFrame(frame_token, draw_start_timestamp, swap_timings, feedback);
    }
}

void CompositorFrameSinkSupport::RefResources(const std::vector<TransferableResource>& resources)
{
    ForAllReservedResourceDelegates([&resources](ReservedResourceDelegate& delegate) { delegate.RefResources(resources); });

    surface_resource_holder_.RefResources(resources);
}

void CompositorFrameSinkSupport::UnrefResources(std::vector<ReturnedResource> resources)
{
    ...
    ForAllReservedResourceDelegates([&resources](ReservedResourceDelegate& delegate) { delegate.UnrefResources(resources); });

    surface_resource_holder_.UnrefResources(std::move(resources));
}

void CompositorFrameSinkSupport::ReturnResources(std::vector<ReturnedResource> resources)
{
    if (resources.empty())
        return;
    ...
    if (pending_frames_.empty() && client_ && (!ShouldMergeBeginFrameWithAcks() || (!needs_begin_frame_ || !begin_frame_source_))) {
        client_->ReclaimResources(std::move(resources));
        return;
    }

    std::move(resources.begin(), resources.end(), std::back_inserter(surface_returned_resources_));
}

void CompositorFrameSinkSupport::ReceiveFromChild(const std::vector<TransferableResource>& resources)
{
    ForAllReservedResourceDelegates([&resources](ReservedResourceDelegate& delegate) { delegate.ReceiveFromChild(resources); });

    surface_resource_holder_.ReceiveFromChild(resources);
}

std::vector<PendingCopyOutputRequest> CompositorFrameSinkSupport::TakeCopyOutputRequests(const LocalSurfaceId& latest_local_id)
{
    std::vector<PendingCopyOutputRequest> results;
    for (auto it = copy_output_requests_.begin(); it != copy_output_requests_.end();) {
...
void CompositorFrameSinkSupport::EvictSurface(const LocalSurfaceId& id)
{
    DCHECK(id.embed_token() != last_evicted_local_surface_id_.embed_token()
        || id.parent_sequence_number() >= last_evicted_local_surface_id_.parent_sequence_number());
...
void CompositorFrameSinkSupport::InitializeCompositorFrameSinkType(mojom::CompositorFrameSinkType type)
{
    if (frame_sink_type_ != mojom::CompositorFrameSinkType::kUnspecified || type == mojom::CompositorFrameSinkType::kUnspecified) {
        return;
    }
    frame_sink_type_ = type;

    if (frame_sink_manager_->frame_counter()) {
        frame_sink_manager_->frame_counter()->SetFrameSinkType(frame_sink_id_, frame_sink_type_);
    }
}

void CompositorFrameSinkSupport::BindLayerContext(mojom::PendingLayerContext& context)
{
    layer_context_ = std::make_unique<LayerContextImpl>(this, context);
}

void CompositorFrameSinkSupport::SetThreads(bool from_untrusted_client, std::vector<Thread> unverified_threads)
{
    ...
    frame_sink_manager_->VerifySandboxedThreadIds(
        thread_ids, base::BindOnce(&CompositorFrameSinkSupport::UpdateThreadIdsPostVerification, weak_factory_.GetWeakPtr(), unverified_threads));
}

void CompositorFrameSinkSupport::UpdateThreadIdsPostVerification(std::vector<Thread> threads, bool passed_verification)
{
    if (passed_verification) {
        ...
        TRACE_EVENT_INSTANT("viz,android.adpf", "FailedToUpdateThreadIdsPostVerification", "thread_ids", thread_ids);
    }
}

base::TimeDelta CompositorFrameSinkSupport::GetPreferredFrameInterval(mojom::CompositorFrameSinkType* type) const
{
    if (type)
...
{
    TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(ack.trace_id), [&](perfetto::EventContext ctx) {
        auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
        auto* data = event->set_chrome_graphics_pipeline();
        data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_DID_NOT_PRODUCE_FRAME);
    ...
    // for scheduling.
    if (last_activated_surface_id_.is_valid())
        surface_manager_->SurfaceModified(last_activated_surface_id_, modified_ack, SurfaceObserver::HandleInteraction::kNo);

    if (begin_frame_source_) {
        begin_frame_source_->DidFinishFrame(this);
        frame_sink_manager_->DidFinishFrame(frame_sink_id_, last_begin_frame_args_);
    }
    if (ack.preferred_frame_interval && frame_sink_type_ == mojom::CompositorFrameSinkType::kLayerTree) {
    ...
    const LocalSurfaceId& local_surface_id, CompositorFrame frame, std::optional<HitTestRegionList> hit_test_region_list, uint64_t submit_time)
{
    const auto result = MaybeSubmitCompositorFrame(
        local_surface_id, std::move(frame), std::move(hit_test_region_list), submit_time, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback());
    DCHECK_EQ(result, SubmitResult::ACCEPTED);
}

bool CompositorFrameSinkSupport::DidAllocateSharedBitmap(base::ReadOnlySharedMemoryRegion region, const SharedBitmapId& id)
{
    if (!frame_sink_manager_->shared_bitmap_manager()->ChildAllocatedSharedBitmap(region.Map(), id)) {
        return false;
    }
...
void CompositorFrameSinkSupport::DidDeleteSharedBitmap(const SharedBitmapId& id)
{
    frame_sink_manager_->shared_bitmap_manager()->ChildDeletedSharedBitmap(id);
    owned_bitmaps_.erase(id);
}

void CompositorFrameSinkSupport::SubmitCompositorFrameLocally(const SurfaceId& surface_id, CompositorFrame frame, const RendererSettings& settings)
{
    ...
    auto frame_rejected_callback = base::ScopedClosureRunner(base::BindOnce([] { NOTREACHED_IN_MIGRATION(); }));
    auto frame_index = ++last_frame_index_;
    Surface::QueueFrameResult result = surface->QueueFrame(std::move(frame), frame_index, std::move(frame_rejected_callback));
...
SubmitResult CompositorFrameSinkSupport::MaybeSubmitCompositorFrame(const LocalSurfaceId& local_surface_id, CompositorFrame frame,
    std::optional<HitTestRegionList> hit_test_region_list, uint64_t submit_time, mojom::CompositorFrameSink::SubmitCompositorFrameSyncCallback callback)
{
    if (!client_needs_begin_frame_ && auto_needs_begin_frame_) {
        ...
        if (frame.metadata.begin_frame_ack.frame_id.source_id == BeginFrameArgs::kManualSourceId && last_begin_frame_args_.frame_id != old_frame_id) {
    ...
    TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global((frame.metadata.begin_frame_ack.trace_id)),
        [&](perfetto::EventContext ctx) {
            auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
            auto* data = event->set_chrome_graphics_pipeline();
            data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_RECEIVE_COMPOSITOR_FRAME);
            frame_sink_id_.WriteIntoTrace(ctx.Wrap(data->set_frame_sink_id()));
            data->set_surface_frame_trace_id(frame.metadata.begin_frame_ack.trace_id);
        });
    ...
    if (frame.metadata.begin_frame_ack.frame_id.source_id == BeginFrameArgs::kManualSourceId) {
    ...
    pending_received_frame_times_.emplace(frame.metadata.frame_token, std::make_unique<PendingFrameDetails>(now_time, surface_manager_));

    // Override the has_damage flag (ignoring invalid data from clients).
    frame.metadata.begin_frame_ack.has_damage = true;
    DCHECK(frame.metadata.begin_frame_ack.frame_id.IsSequenceValid());

    if (!ui::LatencyInfo::Verify(frame.metadata.latency_info, "CompositorFrameSinkSupport::MaybeSubmitCompositorFrame")) {
    ...
    for (ui::LatencyInfo& latency : frame.metadata.latency_info) {
        if (latency.latency_components().size() > 0) {
            latency.AddLatencyNumberWithTimestamp(ui::DISPLAY_COMPOSITOR_RECEIVED_FRAME_COMPONENT, now_time);
        }
    }

    base::ScopedClosureRunner frame_rejected_callback(base::BindOnce(&CompositorFrameSinkSupport::DidRejectCompositorFrame, weak_factory_.GetWeakPtr(),
        frame.metadata.frame_token, frame.resource_list, frame.metadata.latency_info));
    ...
    if (!allow_copy_output_requests_ && frame.HasCopyOutputRequests()) {
        TRACE_EVENT_INSTANT0("viz", "CopyOutputRequests not allowed", TRACE_EVENT_SCOPE_THREAD);
    ...
    if (frame.metadata.begin_frame_ack.preferred_frame_interval) {
        preferred_frame_interval_ = *frame.metadata.begin_frame_ack.preferred_frame_interval;
    } else {
        preferred_frame_interval_ = BeginFrameArgs::MinInterval();
    }

    if (features::ShouldOnBeginFrameThrottleVideo() && frame_sink_type_ == mojom::CompositorFrameSinkType::kVideo) {
        ApplyPreferredFrameRate(frame.metadata.begin_frame_ack.frame_id.source_id);
    }

    Surface* prev_surface = surface_manager_->GetSurfaceForId(last_created_surface_id_);
    Surface* current_surface = nullptr;
    if (prev_surface && local_surface_id == last_created_surface_id_.local_surface_id()) {
        ...
        TRACE_EVENT_WITH_FLOW2(TRACE_DISABLED_BY_DEFAULT("viz.surface_id_flow"), "LocalSurfaceId.Submission.Flow",
            TRACE_ID_GLOBAL(local_surface_id.submission_trace_id()), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "step", "ReceiveCompositorFrame",
            "local_surface_id", local_surface_id.ToString());

        SurfaceId surface_id(frame_sink_id_, local_surface_id);
        SurfaceInfo surface_info(surface_id, frame.device_scale_factor(), frame.size_in_pixels());
        ...
        const LocalSurfaceId& last_created_local_surface_id = last_created_surface_id_.local_surface_id();

        bool child_initiated_synchronization_event
            = last_created_local_surface_id.is_valid() && local_surface_id.child_sequence_number() > last_created_local_surface_id.child_sequence_number();

        // Neither sequence numbers of the LocalSurfaceId can decrease and at least
        // one must increase.
        bool monotonically_increasing_id = (local_surface_id.parent_sequence_number() >= last_created_local_surface_id.parent_sequence_number()
                                               && local_surface_id.child_sequence_number() >= last_created_local_surface_id.child_sequence_number())
            && (local_surface_id.parent_sequence_number() > last_created_local_surface_id.parent_sequence_number() || child_initiated_synchronization_event);

        DCHECK(surface_info.is_valid());
        if (local_surface_id.embed_token() == last_created_local_surface_id.embed_token() && !monotonically_increasing_id) {
        ...
        if (IsEvicted(local_surface_id)) {
            TRACE_EVENT_INSTANT0("viz", "Submit rejected to evicted surface", TRACE_EVENT_SCOPE_THREAD);
            return SubmitResult::ACCEPTED;
        }

        const bool has_copy_request_against_prev_surface = frame.metadata.screenshot_destination.has_value() && prev_surface;

        current_surface = surface_manager_->CreateSurface(
            weak_factory_.GetWeakPtr(), surface_info, has_copy_request_against_prev_surface ? last_created_surface_id_ : SurfaceId());
        ...
        if (has_copy_request_against_prev_surface) {
            auto copy_request = std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory,
                base::BindOnce(&RemoveSurfaceReferenceAndDispatchCopyOutputRequestCallback, frame_sink_manager_->GetWeakPtr(), surface_info.id(),
                    frame.metadata.screenshot_destination.value()));
            if (const auto& size_for_testing = frame_sink_manager_->copy_output_request_result_size_for_testing(); // IN-TEST
                !size_for_testing.IsEmpty()) [[unlikely]] {
                SetCopyOutoutRequestResultSize(copy_request.get(), gfx::Rect(), size_for_testing, prev_surface->size_in_pixels());
            }

            copy_request->set_result_task_runner(base::SequencedTaskRunner::GetCurrentDefault());

            RequestCopyOfOutput(PendingCopyOutputRequest(last_created_surface_id_.local_surface_id(), SubtreeCaptureId {}, std::move(copy_request),
                /*capture_exact_id=*/true));
        }

        if (!current_surface) {
            TRACE_EVENT_INSTANT0("viz", "Surface belongs to another client", TRACE_EVENT_SCOPE_THREAD);
            return SubmitResult::SURFACE_OWNED_BY_ANOTHER_CLIENT;
        }
        last_created_surface_id_ = SurfaceId(frame_sink_id_, local_surface_id);

        surface_manager_->SurfaceDamageExpected(current_surface->surface_id(), last_begin_frame_args_);
    }
    ...
    pending_received_frame_times_[frame.metadata.frame_token]->set_surface_id(last_created_surface_id_);

    const int64_t trace_id = ~frame.metadata.begin_frame_ack.trace_id;
    TRACE_EVENT_WITH_FLOW1(TRACE_DISABLED_BY_DEFAULT("viz.hit_testing_flow"), "Event.Pipeline", TRACE_ID_GLOBAL(trace_id),
        TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "step", "ReceiveHitTestData");

    // QueueFrame can fail in unit tests, so SubmitHitTestRegionList has to be
    // called before that.
    frame_sink_manager()->SubmitHitTestRegionList(last_created_surface_id_, frame_index, std::move(hit_test_region_list));

    Surface::QueueFrameResult result = current_surface->QueueFrame(std::move(frame), frame_index, std::move(frame_rejected_callback));
    switch (result) {
...
SurfaceReference CompositorFrameSinkSupport::MakeTopLevelRootReference(const SurfaceId& surface_id)
{
...
{
    if (!compositor_frame_callback_ || !callback_received_begin_frame_ || !callback_received_receive_ack_) {
        return;
    }

    std::move(compositor_frame_callback_).Run(std::move(surface_returned_resources_));
    surface_returned_resources_.clear();
}
...
void CompositorFrameSinkSupport::DidReceiveCompositorFrameAck()
{
    ...
    bool was_pending_manual_begin_frame_source_ = pending_manual_begin_frame_source_;
    ...
    if (ShouldMergeBeginFrameWithAcks() && !was_pending_manual_begin_frame_source_ && !ack_pending_during_on_begin_frame_) {
    ...
    uint32_t frame_token, base::TimeTicks draw_start_timestamp, const gfx::SwapTimings& swap_timings, const gfx::PresentationFeedback& feedback)
{
    ...
    DCHECK((feedback.flags & gfx::PresentationFeedback::kFailure) || (!draw_start_timestamp.is_null() && !swap_timings.is_null()));
    ...
    details.embedded_frame_timestamp = is_root_ ? details.received_compositor_frame_timestamp : received_frame_timestamp->second->frame_embed_timestamp();
    details.draw_start_timestamp = draw_start_timestamp;
    details.swap_timings = swap_timings;
    details.presentation_feedback = feedback;
    AdjustPresentationFeedback(&details.presentation_feedback, swap_timings.swap_start);
    ...
    if (begin_frame_interval_.is_positive() && details.presentation_feedback.interval.is_positive()) {
    ...
    if (!feedback.failed() && frame_sink_manager_->frame_counter()) {
        frame_sink_manager_->frame_counter()->AddPresentedFrame(frame_sink_id_, feedback.timestamp);
    }
...
void CompositorFrameSinkSupport::DidRejectCompositorFrame(
    uint32_t frame_token, std::vector<TransferableResource> frame_resource_list, std::vector<ui::LatencyInfo> latency_info)
{
    TRACE_EVENT_INSTANT0("viz", "DidRejectCompositorFrame", TRACE_EVENT_SCOPE_THREAD);
    ...
    std::vector<ReturnedResource> resources = TransferableResource::ReturnResources(frame_resource_list);
    ReturnResources(std::move(resources));
    DidReceiveCompositorFrameAck();
    DidPresentCompositorFrame(frame_token, base::TimeTicks(), gfx::SwapTimings(), gfx::PresentationFeedback::Failure());
}

void CompositorFrameSinkSupport::UpdateDisplayRootReference(const Surface* surface)
{
    // Make the new SurfaceId reachable from the top-level root.
    surface_manager_->AddSurfaceReferences({ MakeTopLevelRootReference(surface->surface_id()) });

    // Make the old SurfaceId unreachable from the top-level root if applicable.
    if (referenced_local_surface_id_) {
        surface_manager_->RemoveSurfaceReferences({ MakeTopLevelRootReference(SurfaceId(frame_sink_id_, *referenced_local_surface_id_)) });
    }
...
void CompositorFrameSinkSupport::OnBeginFrame(const BeginFrameArgs& args)
{
    ...
    TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(trace_id), [trace_id](perfetto::EventContext ctx) {
        auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
        auto* data = event->set_chrome_graphics_pipeline();
        data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_ISSUE_BEGIN_FRAME);
    ...
    if (begin_frame_interval_.is_positive()) {
        ...
        base::TimeDelta offset_from_next_scheduled_frame = args.deadline - (args.frame_time + args.interval);
        adjusted_args.deadline = args.frame_time + begin_frame_interval_ + offset_from_next_scheduled_frame;
    }
    SetLastKnownVsync(args.interval);
    const bool should_send_begin_frame = ShouldSendBeginFrame(adjusted_args.frame_time, args.interval) && (client_ || layer_context_);
    if (should_send_begin_frame) {
        if (last_activated_surface_id_.is_valid())
            surface_manager_->SurfaceDamageExpected(last_activated_surface_id_, adjusted_args);
        ...
        if (client_) {
            if (ShouldMergeBeginFrameWithAcks()) {
                ...
                ack_pending_during_on_begin_frame_ = !frame_ack && !pending_frames_.empty();
                ...
                if (!handling_auto_needs_begin_frame_) {
                    client_->OnBeginFrame(adjusted_args, frame_timing_details_, frame_ack, std::move(surface_returned_resources_));
                    frame_timing_details_.clear();
                } else {
                    if (frame_ack) {
                        client_->DidReceiveCompositorFrameAck(std::move(surface_returned_resources_));
                ...
                client_->OnBeginFrame(adjusted_args, frame_timing_details_,
                    /*frame_ack=*/false, std::vector<ReturnedResource>());
                frame_timing_details_.clear();
            }
        }
...
const BeginFrameArgs& CompositorFrameSinkSupport::LastUsedBeginFrameArgs() const
{
...
void CompositorFrameSinkSupport::UpdateNeedsBeginFramesInternal()
{
    ...
    // return.
    needs_begin_frame_ = (client_needs_begin_frame_ || !frame_timing_details_.empty() || !pending_surfaces_.empty() || layer_context_wants_begin_frames_
        || (compositor_frame_callback_ && !callback_received_begin_frame_)
        || (ShouldMergeBeginFrameWithAcks() && (!surface_returned_resources_.empty() || ack_queued_for_client_count_)));
    ...
    if (bundle_id_.has_value()) {
        ...
        if (auto* bundle = frame_sink_manager_->GetFrameSinkBundle(*bundle_id_)) {
            bundle->SetSinkNeedsBeginFrame(frame_sink_id_.sink_id(), needs_begin_frame_);
        }
        return;
    }
...
void CompositorFrameSinkSupport::AttachCaptureClient(CapturableFrameSink::Client* client)
{
...
void CompositorFrameSinkSupport::DetachCaptureClient(CapturableFrameSink::Client* client)
{
...
std::optional<CapturableFrameSink::RegionProperties> CompositorFrameSinkSupport::GetRequestRegionProperties(const VideoCaptureSubTarget& sub_target) const
{
    if (!last_activated_surface_id_.is_valid())
        return {};

    Surface* current_surface = surface_manager_->GetSurfaceForId(last_activated_surface_id_);
    ...
    if (IsRegionCapture(sub_target)) {
        const auto it = current_capture_bounds_.bounds().find(absl::get<RegionCaptureCropId>(sub_target));
        if (it != current_capture_bounds_.bounds().end() && !it->second.IsEmpty() && gfx::Rect(out.root_render_pass_size).Contains(it->second)) {
    ...
    for (const auto& render_pass : frame.render_pass_list) {
        if (render_pass->subtree_capture_id == id) {
            ...
            if (!render_pass->subtree_size.IsEmpty()) {
                ...
                out.render_pass_subrect = IntersectInSpace(render_pass->output_rect, out.transform_to_root, gfx::Rect(out.root_render_pass_size));
            }

            if (!out.render_pass_subrect.IsEmpty() && render_pass->output_rect.Contains(out.render_pass_subrect)) {
                return out;
            }
        }
    }
...
void CompositorFrameSinkSupport::RequestCopyOfOutput(PendingCopyOutputRequest pending_copy_output_request)
{
    copy_output_requests_.push_back(std::move(pending_copy_output_request));
    if (last_activated_surface_id_.is_valid()) {
        BeginFrameAck ack;
        ack.has_damage = true;
        surface_manager_->SurfaceModified(last_activated_surface_id_, ack, SurfaceObserver::HandleInteraction::kNoChange);
    }
}

const CompositorFrameMetadata* CompositorFrameSinkSupport::GetLastActivatedFrameMetadata()
{
    if (!last_activated_surface_id_.is_valid())
        return nullptr;
    Surface* surface = surface_manager_->GetSurfaceForId(last_activated_surface_id_);
    DCHECK(surface);
...
// static
const char* CompositorFrameSinkSupport::GetSubmitResultAsString(SubmitResult result)
{
    switch (result) {
...
bool CompositorFrameSinkSupport::ShouldSendBeginFrame(base::TimeTicks frame_time, base::TimeDelta vsync_interval)
{
    ...
    const bool should_throttle_as_requested = ShouldThrottleBeginFrameAsRequested(frame_time, vsync_interval);
    ...
    const bool can_throttle_if_unresponsive_or_excessive = frame_time - last_frame_time_ < base::Seconds(1);
    ...
    if (last_activated_surface_id_.is_valid()) {
        ...
        should_throttle_undrawn_frames = can_throttle_if_unresponsive_or_excessive && num_undrawn_frames > kUndrawnFrameLimit
    ...
    // begin-frame.
    if (!frame_timing_details_.empty() && !should_throttle_as_requested && (!should_throttle_undrawn_frames || !frame_timing_details_all_failed)) {
...
void CompositorFrameSinkSupport::CheckPendingSurfaces()
{
    if (pending_surfaces_.empty())
        return;
    base::flat_set<raw_ptr<Surface, CtnExperimental>> pending_surfaces(pending_surfaces_);
...
{
    return features::IsOnBeginFrameAcksEnabled() && wants_begin_frame_acks_ && !layer_context_;
}

bool CompositorFrameSinkSupport::ShouldThrottleBeginFrameAsRequested(base::TimeTicks frame_time, base::TimeDelta vsync_interval)
{
    ...
    return !HasElapsedCadenceInterval(vsync_interval, begin_frame_interval_, time_since_last_frame);
}

void CompositorFrameSinkSupport::ProcessCompositorFrameTransitionDirective(const CompositorFrameTransitionDirective& directive, Surface* surface)
{
    ...
    case CompositorFrameTransitionDirective::Type::kSave:
        ...
        if (frame_sink_manager_->ClearSurfaceAnimationManager(transition_token)) [[unlikely]] {
            view_transition_token_to_animation_manager_.erase(transition_token);
            return;
        }
        if (view_transition_token_to_animation_manager_.erase(transition_token)) [[unlikely]] {
            return;
        }

        view_transition_token_to_animation_manager_[transition_token] = SurfaceAnimationManager::CreateWithSave(directive, surface,
            frame_sink_manager_->shared_bitmap_manager(), use_blit_request_for_view_transition_ ? frame_sink_manager_->GetSharedImageInterface() : nullptr,
            frame_sink_manager_->reserved_resource_id_tracker(),
            base::BindOnce(&CompositorFrameSinkSupport::OnSaveTransitionDirectiveProcessed, base::Unretained(this)));
        break;
    case CompositorFrameTransitionDirective::Type::kAnimateRenderer: {
        if (directive.maybe_cross_frame_sink()) {
            ...
            view_transition_token_to_animation_manager_[transition_token] = frame_sink_manager_->TakeSurfaceAnimationManager(transition_token);
        }

        auto it = view_transition_token_to_animation_manager_.find(transition_token);
    ...
    case CompositorFrameTransitionDirective::Type::kRelease:
        frame_sink_manager_->ClearSurfaceAnimationManager(directive.transition_token());
        view_transition_token_to_animation_manager_.erase(directive.transition_token());
        break;
    }
}

void CompositorFrameSinkSupport::OnSaveTransitionDirectiveProcessed(const CompositorFrameTransitionDirective& directive)
{
    DCHECK_EQ(directive.type(), CompositorFrameTransitionDirective::Type::kSave) << "Only save directives need to be ack'd back to the client";

    auto it = view_transition_token_to_animation_manager_.find(directive.transition_token());
    if (it == view_transition_token_to_animation_manager_.end()) {
        return;
    }
    ...
    if (client_) {
        client_->OnCompositorFrameTransitionDirectiveProcessed(directive.sequence_id());
    }

    if (directive.maybe_cross_frame_sink()) {
        frame_sink_manager_->CacheSurfaceAnimationManager(directive.transition_token(), std::move(it->second));
        view_transition_token_to_animation_manager_.erase(it);
    }
}

bool CompositorFrameSinkSupport::IsEvicted(const LocalSurfaceId& local_surface_id) const
{
    return local_surface_id.embed_token() == last_evicted_local_surface_id_.embed_token()
        && local_surface_id.parent_sequence_number() <= last_evicted_local_surface_id_.parent_sequence_number();
}
    ...
    for (auto& request : copy_output_requests_) {
        ...
        if (request.capture_exact_surface_id) {
            ...
            if (target_surface) {
                ...
                BeginFrameAck ack;
                ack.has_damage = true;
                surface_manager_->SurfaceModified(target_id, ack, SurfaceObserver::HandleInteraction::kNoChange);
            } else {
...
void CompositorFrameSinkSupport::SetExternalReservedResourceDelegate(ReservedResourceDelegate* delegate)
{
    external_reserved_resource_delegate_ = delegate;
}

void CompositorFrameSinkSupport::SetLayerContextWantsBeginFrames(bool wants_begin_frames)
{
...
void CompositorFrameSinkSupport::DestroySelf()
{
    ...
    frame_sink_manager_->DestroyCompositorFrameSink(frame_sink_id, base::DoNothing());
}

void CompositorFrameSinkSupport::ScheduleSelfDestruction()
{
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(&CompositorFrameSinkSupport::DestroySelf, weak_factory_.GetWeakPtr()));
}

void CompositorFrameSinkSupport::ForAllReservedResourceDelegates(base::FunctionRef<void(ReservedResourceDelegate&)> func)
{
...
CompositorFrameSinkSupport::PendingFrameDetails::PendingFrameDetails(base::TimeTicks frame_submit_timestamp, SurfaceManager* surface_manager)
...
void CompositorFrameSinkSupport::PendingFrameDetails::SetOrObserveFrameEmbedTimeStamp()
{
    frame_embed_timestamp_ = surface_manager_->GetSurfaceReferencedTimestamp(surface_id_);
...
void CompositorFrameSinkSupport::PendingFrameDetails::set_surface_id(SurfaceId surface_id)
{
...
void CompositorFrameSinkSupport::PendingFrameDetails::OnAddedSurfaceReference(const SurfaceId& parent_id, const SurfaceId& child_id)
{
...
void CompositorFrameSinkSupport::OnClientDestroy()
{
    base::AutoLock locker(client_lock_);
    client_ = nullptr;
}
