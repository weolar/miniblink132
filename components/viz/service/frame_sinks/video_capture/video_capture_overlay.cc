...
VideoCaptureOverlay::VideoCaptureOverlay(FrameSource& frame_source, mojo::PendingReceiver<mojom::FrameSinkVideoCaptureOverlay> receiver)
    : frame_source_(frame_source)
    , receiver_(this, std::move(receiver))
{
    receiver_.set_disconnect_handler(base::BindOnce(&FrameSource::OnOverlayConnectionLost, base::Unretained(frame_source_), this));
}

VideoCaptureOverlay::~VideoCaptureOverlay() = default;

void VideoCaptureOverlay::SetImageAndBounds(const SkBitmap& image, const gfx::RectF& bounds)
{
...
gfx::Rect ToAbsoluteBoundsForI420(const gfx::RectF& relative, const gfx::Rect& span)
{
    ...
    const float absolute_right = std::fma(relative.right(), span.width(), span.x());
    const float absolute_bottom = std::fma(relative.bottom(), span.height(), span.y());
    ...
    const int snapped_left = base::saturated_cast<int16_t>(std::ceil(absolute_left / 2.0f)) * 2;
    const int snapped_top = base::saturated_cast<int16_t>(std::ceil(absolute_top / 2.0f)) * 2;
    const int snapped_right = base::saturated_cast<int16_t>(std::floor(absolute_right / 2.0f)) * 2;
    const int snapped_bottom = base::saturated_cast<int16_t>(std::floor(absolute_bottom / 2.0f)) * 2;
    return gfx::Rect(snapped_left, snapped_top, std::max(0, snapped_right - snapped_left), std::max(0, snapped_bottom - snapped_top));
}
...
gfx::Rect Transform(const gfx::Rect& source, const gfx::Rect& from_region, const gfx::Rect& to_region)
{
    // Transforming from or to a zero space is undefined behavior.
    if (from_region.IsEmpty() || to_region.IsEmpty())
        return {};

    const gfx::Vector2dF scale { static_cast<float>(to_region.width()) / static_cast<float>(from_region.width()),
        static_cast<float>(to_region.height()) / static_cast<float>(from_region.height()) };

    const gfx::Rect old_translated = gfx::Rect(source.x() - from_region.x(), source.y() - from_region.y(), source.width(), source.height());
    const gfx::Rect scaled = gfx::ScaleToEnclosingRect(old_translated, scale.x(), scale.y());
    const gfx::Rect new_translated = gfx::Rect(scaled.x() + to_region.x(), scaled.y() + to_region.y(), scaled.width(), scaled.height());
...
{
    return base::StringPrintf("%s from %s into %s via transform %s, format %s", region_properties.render_pass_subrect.ToString().c_str(),
        region_properties.root_render_pass_size.ToString().c_str(), content_rect.ToString().c_str(), region_properties.transform_to_root.ToString().c_str(),
        media::VideoPixelFormatToString(format).c_str());
}

std::string VideoCaptureOverlay::BlendInformation::ToString() const
{
    return base::StringPrintf("source_region=%s, source_region_scaled=%s, "
                              "destination_region_content=%s",
        source_region.ToString().c_str(), source_region_scaled.ToString().c_str(), destination_region_content.ToString().c_str());
}

std::optional<VideoCaptureOverlay::BlendInformation> VideoCaptureOverlay::CalculateBlendInformation(const CapturedFrameProperties& properties) const
{
    const auto& compositor_frame_rect = gfx::Rect(properties.region_properties.root_render_pass_size);
    const gfx::Rect compositor_frame_subrect = properties.region_properties.transform_to_root.MapRect(properties.region_properties.render_pass_subrect);
    ...
    const gfx::Rect bounds_in_compositor_space = ToAbsoluteBoundsForI420(bounds_, compositor_frame_rect);
    ...
    const gfx::Rect bounds_in_content_space = Transform(bounds_in_compositor_space, compositor_frame_subrect, properties.content_rect);

    // If the sprite's size will be unreasonably large, punt.
    if (bounds_in_content_space.width() > media::limits::kMaxDimension || bounds_in_content_space.height() > media::limits::kMaxDimension) {
        return std::nullopt;
    }
    ...
    const gfx::Rect blit_rect = gfx::IntersectRects(bounds_in_content_space, properties.content_rect);
    ...
    const gfx::Rect source_region_scaled = gfx::Rect(blit_rect.origin() - bounds_in_content_space.OffsetFromOrigin(), blit_rect.size());
    ...
    float scale_x = static_cast<float>(image_.dimensions().width()) / bounds_in_content_space.width();
    float scale_y = static_cast<float>(image_.dimensions().height()) / bounds_in_content_space.height();
    const gfx::Rect source_region = gfx::ScaleToEnclosingRect(source_region_scaled, scale_x, scale_y);
    ...
    return BlendInformation { source_region, source_region_scaled, bounds_in_content_space };
}

VideoCaptureOverlay::OnceRenderer VideoCaptureOverlay::MakeRenderer(const CapturedFrameProperties& properties)
{
    std::optional<VideoCaptureOverlay::BlendInformation> blend_information = CalculateBlendInformation(properties);
    ...
    if (!sprite_ || sprite_->size() != dst_rect.size() || sprite_->format() != properties.format) {
        sprite_ = base::MakeRefCounted<Sprite>(image_, dst_rect.size(), properties.format);
    }
...
VideoCaptureOverlay::OnceRenderer VideoCaptureOverlay::MakeCombinedRenderer(
    const std::vector<VideoCaptureOverlay*>& overlays, const CapturedFrameProperties& properties)
{
    if (overlays.empty())
        return {};
...
gfx::Rect VideoCaptureOverlay::ComputeSourceMutationRect() const
{
    if (!image_.drawsNothing() && !bounds_.IsEmpty()) {
        ...
        gfx::Rect result = gfx::ToEnclosingRect(gfx::ScaleRect(bounds_, source_size.width(), source_size.height()));
...
VideoCaptureOverlay::Sprite::Sprite(const SkBitmap& image, const gfx::Size& size, const VideoPixelFormat format)
    : image_(image)
    , size_(size)
    , format_(format)
{
...
template <typename Pointer> Pointer PositionPointerInPlane(Pointer plane_begin, int stride, const gfx::Point& offset)
{
...
template <typename Pointer> Pointer PositionPointerARGB(Pointer pixels_begin, int stride, const gfx::Point& offset)
{
...
void VideoCaptureOverlay::Sprite::Blend(const gfx::Rect& src_rect, const gfx::Rect& dst_rect, VideoFrame* frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK(frame);
    CHECK(frame->visible_rect().Contains(dst_rect)) << "frame->visible_rect()=" << frame->visible_rect().ToString() << ", dst_rect=" << dst_rect.ToString();
    CHECK(gfx::Rect(size_).Contains(src_rect)) << "size_=" << size_.ToString() << ", src_rect=" << src_rect.ToString();

    CHECK_EQ(format_, frame->format());
    CHECK(!dst_rect.IsEmpty()) << ": frame->visible_rect()=" << frame->visible_rect().ToString() << ", dst_rect=" << dst_rect.ToString();
    CHECK(frame->ColorSpace().IsValid());

    TRACE_EVENT("gpu.capture", "VideoCaptureOverlay::Sprite::Blend", "x", dst_rect.x(), "y", dst_rect.y());
    ...
    case media::PIXEL_FORMAT_I420: {
        ...
        const auto BlitOntoPlane = [](const gfx::Size& blit_size, int src_stride, const float* src, const float* under_weight, int dst_stride, uint8_t* dst) {
            for (int row = 0; row < blit_size.height(); ++row, src += src_stride, under_weight += src_stride, dst += dst_stride) {
                for (int col = 0; col < blit_size.width(); ++col) {
                    dst[col] = base::saturated_cast<uint8_t>(dst[col] * under_weight[col] + 255.0f * src[col] + 0.5f);
                }
            }
        };
        ...
        const float* under_weight = PositionPointerInPlane(transformed_image_.get(), src_stride, src_origin);
        ...
        uint8_t* dst = PositionPointerInPlane(frame->GetWritableVisibleData(VideoFrame::Plane::kY), dst_stride, dst_rect.origin());
        BlitOntoPlane(dst_rect.size(), src_stride, src, under_weight, dst_stride, dst);
        ...
        under_weight = PositionPointerInPlane(transformed_image_.get() + 2 * num_pixels, src_stride, src_origin);
        const int num_chroma_pixels = size_.GetArea() / 4;
        src = under_weight + num_chroma_pixels;
        dst_stride = frame->stride(VideoFrame::Plane::kU);
        const gfx::Rect chroma_blit_rect(dst_rect.x() / 2, dst_rect.y() / 2, dst_rect.width() / 2, dst_rect.height() / 2);
        dst = PositionPointerInPlane(frame->GetWritableVisibleData(VideoFrame::Plane::kU), dst_stride, chroma_blit_rect.origin());
        BlitOntoPlane(chroma_blit_rect.size(), src_stride, src, under_weight, dst_stride, dst);
        src += num_chroma_pixels;
        dst_stride = frame->stride(VideoFrame::Plane::kV);
        dst = PositionPointerInPlane(frame->GetWritableVisibleData(VideoFrame::Plane::kV), dst_stride, chroma_blit_rect.origin());
        BlitOntoPlane(chroma_blit_rect.size(), src_stride, src, under_weight, dst_stride, dst);
    ...
    case media::PIXEL_FORMAT_ARGB: {
        ...
        const float* src = PositionPointerARGB(transformed_image_.get(), src_stride, src_origin);
        ...
        uint8_t* dst = PositionPointerARGB(frame->GetWritableVisibleData(VideoFrame::Plane::kARGB), dst_stride, dst_rect.origin());
        CHECK_EQ((dst - frame->visible_data(VideoFrame::Plane::kARGB)) % sizeof(uint32_t), 0u);
        ...
        // alpha.
        for (int row = 0; row < dst_rect.height(); ++row, src += src_stride, dst += dst_stride) {
            ...
            for (int col = 0; col < dst_rect.width(); ++col) {
                ...
                const float dst_weight = From255(dst_pixel[col] >> 24) * (1.0f - src_alpha);
                const float out_alpha = src_alpha + dst_weight;
                float out_red = std::fma(From255(dst_pixel[col] >> 16), dst_weight, src[src_idx + 1]);
                float out_green = std::fma(From255(dst_pixel[col] >> 8), dst_weight, src[src_idx + 2]);
                float out_blue = std::fma(From255(dst_pixel[col] >> 0), dst_weight, src[src_idx + 3]);
                if (out_alpha != 0.0f) {
                    out_red /= out_alpha;
                    out_green /= out_alpha;
                    out_blue /= out_alpha;
                ...
                dst_pixel[col]
                    = ((ToClamped255(out_alpha) << 24) | (ToClamped255(out_red) << 16) | (ToClamped255(out_green) << 8) | (ToClamped255(out_blue) << 0));
            }
        }
...
{
    TRACE_EVENT("gpu.capture", "VideoCaptureOverlay::Sprite::TransformImage", "width", size_.width(), "height", size_.height());
    ...
    const SkImageInfo scaled_image_format = SkImageInfo::Make(size_.width(), size_.height(), kN32_SkColorType, kUnpremul_SkAlphaType, image_.refColorSpace());
    SkBitmap scaled_image;
    if (image_.info() == scaled_image_format) {
        scaled_image = image_;
    } else {
        if (scaled_image.tryAllocPixels(scaled_image_format)
            && image_.pixmap().scalePixels(scaled_image.pixmap(), SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNearest))) {
    ...
    auto colors = base::HeapArray<gfx::ColorTransform::TriStim>::WithSize(num_pixels);
    if (scaled_image.drawsNothing()) {
        ...
        for (int y = 0; y < size_.height(); ++y) {
            ...
            for (int x = 0; x < size_.width(); ++x) {
                ...
                colors[pos].SetPoint(
                    ((pixel >> SK_R32_SHIFT) & 0xff) / 255.0f, ((pixel >> SK_G32_SHIFT) & 0xff) / 255.0f, ((pixel >> SK_B32_SHIFT) & 0xff) / 255.0f);
                ++pos;
            }
        }
    }
    ...
    if (!image_color_space.IsValid()) {
        ...
        image_color_space = gfx::ColorSpace(
            gfx::ColorSpace::PrimaryID::BT709, gfx::ColorSpace::TransferID::LINEAR, gfx::ColorSpace::MatrixID::RGB, gfx::ColorSpace::RangeID::FULL);
    }
    if (image_color_space != color_space_) {
        const auto color_transform = gfx::ColorTransform::NewColorTransform(image_color_space, color_space_);
    ...
    case media::PIXEL_FORMAT_I420: {
        ...
        const int num_chroma_pixels = size_.GetArea() / 4;
        transformed_image_.reset(new float[num_pixels * 2 + num_chroma_pixels * 3]);
        ...
        while (alpha_row0 < alpha_row_end) {
            ...
            for (int col = 0; col < size_.width(); col += 2) {
                ...
                *(out_uv_1_minus_alpha++) = std::fma(alpha_row0[col] + alpha_row0[col + 1] + alpha_row1[col] + alpha_row1[col + 1], -1.0f / 4.0f, 1.0f);
                ...
                *(out_u++) = ((color_row0[col].y() * alpha_row0[col]) + (color_row0[col + 1].y() * alpha_row0[col + 1])
                                 + (color_row1[col].y() * alpha_row1[col]) + (color_row1[col + 1].y() * alpha_row1[col + 1]))
                    / 4.0f;
                *(out_v++) = ((color_row0[col].z() * alpha_row0[col]) + (color_row0[col + 1].z() * alpha_row0[col + 1])
                                 + (color_row1[col].z() * alpha_row1[col]) + (color_row1[col + 1].z() * alpha_row1[col + 1]))
                    / 4.0f;
            }
