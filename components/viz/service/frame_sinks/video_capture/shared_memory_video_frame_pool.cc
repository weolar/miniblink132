...
scoped_refptr<VideoFrame> SharedMemoryVideoFramePool::ReserveVideoFrame(VideoPixelFormat format, const gfx::Size& size)
{
    ...
    for (auto it = available_buffers_.rbegin(); it != available_buffers_.rend(); ++it) {
    ...
    while (!available_buffers_.empty()) {
        const auto it = std::max_element(available_buffers_.rbegin(), available_buffers_.rend(),
            [](const PooledBuffer& a, const PooledBuffer& b) { return a.mapping.size() < b.mapping.size(); });
        available_buffers_.erase(it.base() - 1); // Release before allocating more.
        PooledBuffer reallocated = base::ReadOnlySharedMemoryRegion::Create(bytes_required);
        if (!reallocated.IsValid()) {
            LOG_IF(WARNING, CanLogSharedMemoryFailure()) << "Failed to re-allocate " << bytes_required << " bytes.";
    ...
    PooledBuffer additional = base::ReadOnlySharedMemoryRegion::Create(bytes_required);
    if (!additional.IsValid()) {
        LOG_IF(WARNING, CanLogSharedMemoryFailure()) << "Failed to allocate " << bytes_required << " bytes.";
        return nullptr;
    }
    return WrapBuffer(std::move(additional), format, size);
}

media::mojom::VideoBufferHandlePtr SharedMemoryVideoFramePool::CloneHandleForDelivery(const VideoFrame& frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    return media::mojom::VideoBufferHandle::NewReadOnlyShmemRegion(it->second.Duplicate());
}
...
scoped_refptr<VideoFrame> SharedMemoryVideoFramePool::WrapBuffer(PooledBuffer pooled_buffer, VideoPixelFormat format, const gfx::Size& size)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    scoped_refptr<VideoFrame> frame = VideoFrame::WrapExternalData(
        format, size, gfx::Rect(size), size, static_cast<uint8_t*>(pooled_buffer.mapping.memory()), pooled_buffer.mapping.size(), base::TimeDelta());
    CHECK(frame);
    ...
    frame->AddDestructionObserver(base::BindOnce(
        &SharedMemoryVideoFramePool::OnFrameWrapperDestroyed, weak_factory_.GetWeakPtr(), base::Unretained(frame.get()), std::move(pooled_buffer.mapping)));
    return frame;
}

void SharedMemoryVideoFramePool::OnFrameWrapperDestroyed(const VideoFrame* frame, base::WritableSharedMemoryMapping mapping)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    const auto it = utilized_buffers_.find(frame);
    CHECK(it != utilized_buffers_.end());
    available_buffers_.emplace_back(PooledBuffer { std::move(it->second), std::move(mapping) });
