...
#define UMA_HISTOGRAM_CAPTURE_DURATION(format, sample)                                                                                                         \
    do {                                                                                                                                                       \
        UMA_HISTOGRAM_CUSTOM_TIMES("Viz.FrameSinkVideoCapturer." format ".CaptureDuration", sample, base::Milliseconds(1), base::Seconds(1), 50);              \
        UMA_HISTOGRAM_CUSTOM_TIMES("Viz.FrameSinkVideoCapturer.CaptureDuration", sample, base::Milliseconds(1), base::Seconds(1), 50);                         \
    } while (false)
...
#define UMA_HISTOGRAM_CAPTURE_SUCCEEDED(format, success)                                                                                                       \
    do {                                                                                                                                                       \
        UMA_HISTOGRAM_BOOLEAN("Viz.FrameSinkVideoCapturer." format ".CaptureSucceeded", success);                                                              \
        UMA_HISTOGRAM_BOOLEAN("Viz.FrameSinkVideoCapturer.CaptureSucceeded", success);                                                                         \
    } while (false)
...
constexpr gfx::Rect kMaxRect = gfx::Rect(0, 0, std::numeric_limits<int>::max(), std::numeric_limits<int>::max());
...
std::unique_ptr<VideoFramePool> GetVideoFramePoolForFormat(
    media::VideoPixelFormat format, int capacity, mojom::BufferFormatPreference buffer_format_preference, GmbVideoFramePoolContextProvider* context_provider)
{
    CHECK(format == media::PIXEL_FORMAT_I420 || format == media::PIXEL_FORMAT_NV12 || format == media::PIXEL_FORMAT_ARGB);
    ...
    case media::PIXEL_FORMAT_ARGB: {
        ...
        case mojom::BufferFormatPreference::kPreferGpuMemoryBuffer:
            return std::make_unique<GpuMemoryBufferVideoFramePool>(capacity, format, gfx::ColorSpace::CreateSRGB(), context_provider);
    ...
    case media::PIXEL_FORMAT_NV12:
        return std::make_unique<GpuMemoryBufferVideoFramePool>(capacity, format, gfx::ColorSpace::CreateREC709(), context_provider);
    default:
        NOTREACHED();
    }
}

CopyOutputRequest::ResultFormat VideoPixelFormatToCopyOutputRequestFormat(media::VideoPixelFormat format)
{
    switch (format) {
    case media::PIXEL_FORMAT_I420:
...
bool IsCompatibleWithFormat(const gfx::Rect& rect, media::VideoPixelFormat format)
{
    CHECK(format == media::PIXEL_FORMAT_I420 || format == media::PIXEL_FORMAT_NV12 || format == media::PIXEL_FORMAT_ARGB);
    ...
    return rect.origin().x() % 2 == 0 && rect.origin().y() % 2 == 0 && rect.width() % 2 == 0 && rect.height() % 2 == 0;
}
...
{
    return perfetto::Track(static_cast<uint64_t>((metadata.capture_begin_time.value() - base::TimeTicks()).InMicroseconds()));
}

perfetto::Track CaptureTrack(const media::VideoFrameMetadata& metadata)
{
    return perfetto::Track(static_cast<uint64_t>((metadata.reference_time.value() - base::TimeTicks()).InMicroseconds()));
}

} // namespace

// static
constexpr media::VideoPixelFormat FrameSinkVideoCapturerImpl::kDefaultPixelFormat;
// static
...
FrameSinkVideoCapturerImpl::FrameSinkVideoCapturerImpl(FrameSinkVideoCapturerManager& frame_sink_manager,
    GmbVideoFramePoolContextProvider* gmb_video_frame_pool_context_provider, mojo::PendingReceiver<mojom::FrameSinkVideoCapturer> receiver,
    std::unique_ptr<media::VideoCaptureOracle> oracle, bool log_to_webrtc)
    : frame_sink_manager_(frame_sink_manager)
    , copy_request_source_(base::UnguessableToken::Create())
    , clock_(base::DefaultTickClock::GetInstance())
    , oracle_(std::move(oracle))
    , gmb_video_frame_pool_context_provider_(gmb_video_frame_pool_context_provider)
    , feedback_weak_factory_(oracle_.get())
    , log_to_webrtc_(log_to_webrtc)
{
    CHECK(oracle_);
    if (log_to_webrtc_) {
        oracle_->SetLogCallback(base::BindRepeating(&FrameSinkVideoCapturerImpl::OnLog, base::Unretained(this)));
    }
    ...
    if (receiver.is_valid()) {
        ...
        receiver_.set_disconnect_handler(
            base::BindOnce(&FrameSinkVideoCapturerManager::OnCapturerConnectionLost, base::Unretained(&*frame_sink_manager_), this));
    }
}
...
void FrameSinkVideoCapturerImpl::ResolveTarget()
{
    ...
    SetResolvedTarget(target_ ? frame_sink_manager_->FindCapturableFrameSink(target_.value()) : nullptr);
}
...
void FrameSinkVideoCapturerImpl::SetResolvedTarget(CapturableFrameSink* target)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    TRACE_EVENT_INSTANT("gpu.capture", "SetResolvedTarget", "current", resolved_target_ ? resolved_target_->GetFrameSinkId().ToString() : "None", "new",
        target ? target->GetFrameSinkId().ToString() : "None");
...
void FrameSinkVideoCapturerImpl::SetFormat(media::VideoPixelFormat format)
{
    ...
    if (format != media::PIXEL_FORMAT_I420 && format != media::PIXEL_FORMAT_ARGB && format != media::PIXEL_FORMAT_NV12) {
        ...
        CHECK(format != media::PIXEL_FORMAT_NV12 || gmb_video_frame_pool_context_provider_);
    ...
    if (format_changed) {
        ...
        if (video_capture_started_) {
            ...
            frame_pool_ = GetVideoFramePoolForFormat(pixel_format_, kFramePoolCapacity, buffer_format_preference_, gmb_video_frame_pool_context_provider_);
...
void FrameSinkVideoCapturerImpl::SetMinCapturePeriod(base::TimeDelta min_capture_period)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    constexpr base::TimeDelta kMinMinCapturePeriod = base::Microseconds(base::Time::kMicrosecondsPerSecond / media::limits::kMaxFramesPerSecond);
    ...
    if (!base::TimeTicks::IsHighResolution()) {
        constexpr base::TimeDelta kMinLowResCapturePeriod = base::Microseconds(base::Time::kMicrosecondsPerSecond / 30);
        if (min_capture_period < kMinLowResCapturePeriod) {
            min_capture_period = kMinLowResCapturePeriod;
        }
    }

    TRACE_EVENT_INSTANT("gpu.capture", "SetMinCapturePeriod", "min_capture_period", min_capture_period);
...
void FrameSinkVideoCapturerImpl::SetMinSizeChangePeriod(base::TimeDelta min_period)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    TRACE_EVENT_INSTANT("gpu.capture", "SetMinSizeChangePeriod", "min_size_change_period", min_period);

    oracle_->SetMinSizeChangePeriod(min_period);
}

void FrameSinkVideoCapturerImpl::SetResolutionConstraints(const gfx::Size& min_size, const gfx::Size& max_size, bool use_fixed_aspect_ratio)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    TRACE_EVENT_INSTANT("gpu.capture", "SetResolutionConstraints", "min_size", min_size.ToString(), "max_size", max_size.ToString());

    if (min_size.width() <= 0 || min_size.height() <= 0 || max_size.width() > media::limits::kMaxDimension || max_size.height() > media::limits::kMaxDimension
        || min_size.width() > max_size.width() || min_size.height() > max_size.height()) {
        LOG(DFATAL) << "Invalid resolutions constraints: " << min_size.ToString() << " must not be greater than " << max_size.ToString()
...
{
    TRACE_EVENT_INSTANT("gpu.capture", "SetAutoThrottlingEnabled", "autothrottling_enabled", enabled);

    oracle_->SetAutoThrottlingEnabled(enabled);
}

void FrameSinkVideoCapturerImpl::ChangeTarget(const std::optional<VideoCaptureTarget>& target, uint32_t sub_capture_target_version)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void FrameSinkVideoCapturerImpl::Start(mojo::PendingRemote<mojom::FrameSinkVideoConsumer> consumer, mojom::BufferFormatPreference buffer_format_preference)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK(consumer);
    ...
    TRACE_EVENT_INSTANT("gpu.capture", "Start", "buffer_format_preference", buffer_format_preference);
    ...
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN2(
        "gpu.capture", "FrameSinkVideoCapturerImpl::Start", this, "pixel_format_", pixel_format_, "buffer_format_preference_", buffer_format_preference_);

    // If we should start capture for NV12 format, we can only hand out GMBs so
    // the caller must tolerate them:
    CHECK(pixel_format_ != media::PIXEL_FORMAT_NV12 || buffer_format_preference_ == mojom::BufferFormatPreference::kPreferGpuMemoryBuffer);

    // If we are using ARGB format with GMB, we must have the pool context
    CHECK(pixel_format_ != media::PIXEL_FORMAT_ARGB || buffer_format_preference_ != mojom::BufferFormatPreference::kPreferGpuMemoryBuffer
    ...
    consumer_.set_disconnect_handler(base::BindOnce(&FrameSinkVideoCapturerImpl::Stop, base::Unretained(this)));
    RefreshEntireSourceNow();
}
...
void FrameSinkVideoCapturerImpl::Stop()
{
    ...
    TRACE_EVENT_NESTABLE_ASYNC_END0("gpu.capture", "FrameSinkVideoCapturerImpl::Start", this);
...
void FrameSinkVideoCapturerImpl::CreateOverlay(int32_t stacking_index, mojo::PendingReceiver<mojom::FrameSinkVideoCaptureOverlay> receiver)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);

    // This will cause an existing overlay with the same stacking index to be
    // dropped, per mojom-documented behavior.
    overlays_.emplace(stacking_index, std::make_unique<VideoCaptureOverlay>(*this, std::move(receiver)));
}
...
void FrameSinkVideoCapturerImpl::OnOverlayConnectionLost(VideoCaptureOverlay* overlay)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    base::EraseIf(overlays_, [&overlay](const decltype(overlays_)::value_type& entry) { return entry.second.get() == overlay; });
}
...
gfx::Rect FrameSinkVideoCapturerImpl::GetContentRectangle(const gfx::Rect& visible_rect, const gfx::Size& source_size, media::VideoPixelFormat pixel_format)
{
    CHECK(pixel_format == media::PIXEL_FORMAT_I420 || pixel_format == media::PIXEL_FORMAT_NV12 || pixel_format == media::PIXEL_FORMAT_ARGB);

    if (pixel_format == media::PIXEL_FORMAT_I420 || pixel_format == media::PIXEL_FORMAT_NV12) {
        return media::ComputeLetterboxRegionForI420(visible_rect, source_size);
    } else {
        CHECK_EQ(media::PIXEL_FORMAT_ARGB, pixel_format);
        const gfx::Rect content_rect = media::ComputeLetterboxRegion(visible_rect, source_size);

        // The media letterboxing computation explicitly allows for off-by-one
        // errors due to computation, so we address those here.
        return content_rect.ApproximatelyEqual(visible_rect, 1) ? visible_rect : content_rect;
    }
}
...
void FrameSinkVideoCapturerImpl::MaybeScheduleRefreshFrame()
{
    ...
    if (!refresh_frame_retry_timer_->IsRunning()) {
        ...
        refresh_frame_retry_timer_->Start(FROM_HERE, GetDelayBeforeNextRefreshAttempt(),
            base::BindOnce(&FrameSinkVideoCapturerImpl::RefreshInternal, base::Unretained(this), media::VideoCaptureOracle::Event::kRefreshRequest));
    }
}
...
base::TimeDelta FrameSinkVideoCapturerImpl::GetDelayBeforeNextRefreshAttempt() const
{
    ...
    return std::min(kMaxRefreshDelay, 2 * std::max(oracle_->estimated_frame_duration(), oracle_->min_capture_period()));
}
...
void FrameSinkVideoCapturerImpl::RefreshInternal(VideoCaptureOracle::Event event)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    CHECK(target_);
    const std::optional<CapturableFrameSink::RegionProperties> region_properties = resolved_target_->GetRequestRegionProperties(target_->sub_target);
    ...
    if (size != oracle_->source_size()) {
        ...
        InvalidateEntireSource();
        OnLog(base::StringPrintf("FrameSinkVideoCapturerImpl::RefreshInternal() "
                                 "changed active frame size: %s",
            size.ToString().c_str()));
    }

    MaybeCaptureFrame(event, gfx::Rect(), clock_->NowTicks(), *resolved_target_->GetLastActivatedFrameMetadata());
}

void FrameSinkVideoCapturerImpl::OnFrameDamaged(
    const gfx::Size& root_render_pass_size, const gfx::Rect& damage_rect, base::TimeTicks expected_display_time, const CompositorFrameMetadata& frame_metadata)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (size == oracle_->source_size()) {
        ...
        oracle_->SetSourceSize(size);
        InvalidateEntireSource();
        OnLog(base::StringPrintf("FrameSinkVideoCapturerImpl::OnFrameDamaged() changed frame size: %s", size.ToString().c_str()));
    }

    MaybeCaptureFrame(VideoCaptureOracle::kCompositorUpdate, damage_rect, expected_display_time, frame_metadata);
}
...
std::vector<VideoCaptureOverlay*> FrameSinkVideoCapturerImpl::GetOverlaysInOrder() const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
FrameSinkVideoCapturerImpl::FrameCapture::FrameCapture(int64_t capture_frame_number, OracleFrameNumber oracle_frame_number, int64_t content_version,
    gfx::Rect content_rect, CapturableFrameSink::RegionProperties region_properties, scoped_refptr<media::VideoFrame> frame, base::TimeTicks request_time)
...
FrameSinkVideoCapturerImpl::FrameCapture::FrameCapture(const FrameSinkVideoCapturerImpl::FrameCapture&) = default;
FrameSinkVideoCapturerImpl::FrameCapture::FrameCapture(FrameSinkVideoCapturerImpl::FrameCapture&&) = default;
FrameSinkVideoCapturerImpl::FrameCapture& FrameSinkVideoCapturerImpl::FrameCapture::operator=(const FrameSinkVideoCapturerImpl::FrameCapture&) = default;
FrameSinkVideoCapturerImpl::FrameCapture& FrameSinkVideoCapturerImpl::FrameCapture::operator=(FrameSinkVideoCapturerImpl::FrameCapture&&) = default;
FrameSinkVideoCapturerImpl::FrameCapture::~FrameCapture() = default;

bool FrameSinkVideoCapturerImpl::FrameCapture::operator<(const FrameSinkVideoCapturerImpl::FrameCapture& other) const
{
...
void FrameSinkVideoCapturerImpl::FrameCapture::CaptureFailed(CaptureResult result)
{
    CHECK(result_ == CaptureResult::kPending || result_ == CaptureResult::kSuccess);
    result_ = result;
    frame_metadata_ = frame->metadata();
    frame = nullptr;
}

const media::VideoFrameMetadata& FrameSinkVideoCapturerImpl::FrameCapture::frame_metadata() const
{
    return frame ? frame->metadata() : frame_metadata_.value();
}

void FrameSinkVideoCapturerImpl::MaybeCaptureFrame(
    VideoCaptureOracle::Event event, const gfx::Rect& damage_rect, base::TimeTicks event_time, const CompositorFrameMetadata& frame_metadata)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK(resolved_target_);
    CHECK(target_);
    ...
    if (oracle_->ObserveEventAndDecideCapture(event, damage_rect, event_time)) {
        ...
        TRACE_EVENT_INSTANT("gpu.capture", "FpsRateLimited", "trigger", VideoCaptureOracle::EventAsString(event));
    ...
    if (!consumer_) {
        TRACE_EVENT_INSTANT("gpu.capture", "NoConsumer", "trigger", VideoCaptureOracle::EventAsString(event));
        return;
    }
    ...
    if (!region_properties) {
        ...
        TRACE_EVENT_INSTANT("gpu.capture", "NoRegionProperties", "trigger", VideoCaptureOracle::EventAsString(event));
        return;
    }

    const gfx::Rect render_pass_in_root_space = region_properties->transform_to_root.MapRect(region_properties->render_pass_subrect);
    ...
    if (!gfx::Rect(region_properties->root_render_pass_size).Contains(render_pass_in_root_space)) {
        TRACE_EVENT_INSTANT("gpu.capture", "DroppingFrameWithUncontainedRegion", "root_render_pass_size", region_properties->root_render_pass_size.ToString(),
            "render_pass_subrect_in_root_space", render_pass_in_root_space.ToString());
        MaybeScheduleRefreshFrame();
        return;
    }
    ...
    const gfx::Size capture_size = AdjustSizeForPixelFormat(oracle_->capture_size());
    ...
    if (can_resurrect_content) {
        ...
        TRACE_EVENT_INSTANT("gpu.capture", "ReservingVideoFrame", "root_render_pass_size", region_properties->root_render_pass_size.ToString(),
            "render_pass_subrect", region_properties->render_pass_subrect.ToString());
        auto reserve_start_time = base::TimeTicks::Now();

        frame = frame_pool_->ReserveVideoFrame(pixel_format_, capture_size);

        UMA_HISTOGRAM_CUSTOM_TIMES(
            "Viz.FrameSinkVideoCapturer.ReserveFrameDuration", base::TimeTicks::Now() - reserve_start_time, base::Milliseconds(1), base::Milliseconds(250), 50);
    }

    UMA_HISTOGRAM_BOOLEAN("Viz.FrameSinkVideoCapturer.FrameResurrected", can_resurrect_content);
    ...
    // pipeline is full.
    if (!frame) {
        TRACE_EVENT_INSTANT("gpu.capture", "PipelineLimited", "trigger", VideoCaptureOracle::EventAsString(event), "utilization_pct", utilization_pct);
    ...
    // still proceed.
    if (utilization >= 1.0) {
        TRACE_EVENT_INSTANT("gpu.capture", "NearlyPipelineLimited", "trigger", VideoCaptureOracle::EventAsString(event), "utilization_pct", utilization_pct);
    }
    ...
    metadata.frame_sequence = frame_metadata.begin_frame_ack.frame_id.sequence_number;
    ...
    if (frame_metadata.top_controls_visible_height.has_value()) {
        last_top_controls_visible_height_ = *frame_metadata.top_controls_visible_height;
    }
    ...
    TRACE_EVENT_BEGIN("gpu.capture", "FrameInUse", FrameInUseTrack(metadata), "frame_number", capture_frame_number, "utilization_pct", utilization_pct);
    ...
    const gfx::Rect content_rect = GetContentRectangle(frame->visible_rect(), source_size, pixel_format_);
    TRACE_EVENT_INSTANT("gpu.capture", "ContentRectDeterminedForCapture", "content_rect", content_rect.ToString(), "source_size", source_size.ToString());

    TRACE_EVENT_BEGIN(
        "gpu.capture", "Capture", CaptureTrack(metadata), "frame_number", capture_frame_number, "trigger", VideoCaptureOracle::EventAsString(event));

    // Determine what rectangular region has changed since the last captured
    // frame.
    gfx::Rect update_rect;
    if (dirty_rect_ == kMaxRect || frame->visible_rect() != last_frame_visible_rect_) {
        ...
        update_rect = copy_output::ComputeResultRect(
            dirty_rect_, gfx::Vector2d(source_size.width(), source_size.height()), gfx::Vector2d(content_rect.width(), content_rect.height()));
        update_rect.Offset(content_rect.OffsetFromOrigin());
        if (pixel_format_ == media::PIXEL_FORMAT_I420 || pixel_format_ == media::PIXEL_FORMAT_NV12) {
    ...
    if (can_resurrect_content) {
        if (log_to_webrtc_) {
            ...
            switch (frame->format()) {
            case media::PIXEL_FORMAT_I420:
                strides = base::StringPrintf("strideY:%d StrideU:%d StrideV:%d", frame->stride(VideoFrame::Plane::kY), frame->stride(VideoFrame::Plane::kU),
                    frame->stride(VideoFrame::Plane::kV));
                break;
            case media::PIXEL_FORMAT_ARGB:
                strides = base::StringPrintf("strideARGB:%d", frame->stride(VideoFrame::Plane::kARGB));
                break;
            case media::PIXEL_FORMAT_NV12:
                strides = base::StringPrintf("strideY:%d StrideUV:%d", frame->stride(VideoFrame::Plane::kY), frame->stride(VideoFrame::Plane::kUV));
                break;
            default:
                strides = "strides:???";
            }
            consumer_->OnLog(base::StringPrintf("FrameSinkVideoCapturerImpl: Resurrecting frame format=%s "
                                                "frame_coded_size: %s "
                                                "frame_visible_rect: %s frame_natural_size: %s %s",
                VideoPixelFormatToString(frame->format()).c_str(), frame->coded_size().ToString().c_str(), frame->visible_rect().ToString().c_str(),
                frame->natural_size().ToString().c_str(), strides.c_str()));
        }

        FrameCapture frame_capture(
            capture_frame_number, oracle_frame_number, content_version_, content_rect, *region_properties, std::move(frame), capture_begin_time);
    ...
    if (content_rect.IsEmpty()) {
        ...
        FrameCapture frame_capture(
            capture_frame_number, oracle_frame_number, content_version_, gfx::Rect(), *region_properties, std::move(frame), capture_begin_time);
        frame_capture.CaptureSucceeded();
        OnFrameReadyForDelivery(frame_capture);
        return;
    }
    ...
    const bool is_same_frame_sink_as_requested = resolved_target_->GetFrameSinkId() == target_->frame_sink_id;
    if (IsRegionCapture(target_->sub_target) && is_same_frame_sink_as_requested) {
        ...
        metadata.region_capture_rect
            = scale_factor ? ScaleToEnclosingRect(region_properties->render_pass_subrect, 1.0f / scale_factor) : region_properties->render_pass_subrect;
        metadata.source_size = source_size;
    }
    ...
    const bool capture_texture_results = buffer_format_preference_ == mojom::BufferFormatPreference::kPreferGpuMemoryBuffer
        && (pixel_format_ == media::PIXEL_FORMAT_NV12 || pixel_format_ == media::PIXEL_FORMAT_ARGB);
    ...
    if (capture_texture_results) {
        ...
        blit_request = BlitRequest(content_rect.origin(), LetterboxingBehavior::kLetterbox, mailbox, sync_token, true);
        ...
        VideoCaptureOverlay::CapturedFrameProperties frame_properties
            = VideoCaptureOverlay::CapturedFrameProperties { frame_capture.region_properties, frame_capture.content_rect, pixel_format_ };

        for (const VideoCaptureOverlay* overlay : GetOverlaysInOrder()) {
            std::optional<VideoCaptureOverlay::BlendInformation> blend_information = overlay->CalculateBlendInformation(frame_properties);
            ...
            blit_request->AppendBlendBitmap(blend_information->source_region, blend_information->destination_region_content, overlay->bitmap().asImage());
        }
    }

    // Request a copy of the next frame from the frame sink.
    auto request = std::make_unique<CopyOutputRequest>(VideoPixelFormatToCopyOutputRequestFormat(pixel_format_),
        capture_texture_results ? CopyOutputRequest::ResultDestination::kNativeTextures : CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(&FrameSinkVideoCapturerImpl::DidCopyFrame, capture_weak_factory_.GetWeakPtr(), std::move(frame_capture)));
    ...
    request->SetScaleRatio(gfx::Vector2d(source_size.width(), source_size.height()), gfx::Vector2d(content_rect.width(), content_rect.height()));
    ...
    if (log_to_webrtc_) {
        ...
        const bool is_bitmap = buffer_format_preference_ == mojom::BufferFormatPreference::kDefault;
        consumer_->OnLog(base::StringPrintf("FrameSinkVideoCapturerImpl: Sending CopyRequest: "
                                            "format=%s (%s) area:%s "
                                            "scale_from: %s "
                                            "scale_to: %s "
                                            "frame pool utilization: %d",
            format.c_str(), is_bitmap ? "bitmap" : "GPU memory buffer", request->area().ToString().c_str(), request->scale_from().ToString().c_str(),
            request->scale_to().ToString().c_str(), utilization_pct));
    }

    const SubtreeCaptureId subtree_id = IsSubtreeCapture(target_->sub_target) ? absl::get<SubtreeCaptureId>(target_->sub_target) : SubtreeCaptureId();

    resolved_target_->RequestCopyOfOutput({ LocalSurfaceId(), subtree_id, std::move(request) });
}

void FrameSinkVideoCapturerImpl::DidCopyFrame(FrameCapture frame_capture, std::unique_ptr<CopyOutputResult> result)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    if (log_to_webrtc_ && consumer_) {
        ...
        consumer_->OnLog(base::StringPrintf("FrameSinkVideoCapturerImpl: got CopyOutputResult: format=%s size:%s "
                                            "frame_coded_size: %s frame_visible_rect: %s frame_natural_size: %s "
                                            "content_rect: %s %s",
            format.c_str(), result->size().ToString().c_str(), frame->coded_size().ToString().c_str(), frame->visible_rect().ToString().c_str(),
            frame->natural_size().ToString().c_str(), content_rect.ToString().c_str(), strides.c_str()));
    }
    ...
    if (pixel_format_ == media::PIXEL_FORMAT_I420) {
        ...
        uint8_t* const y = frame->GetWritableVisibleData(VideoFrame::Plane::kY) + content_rect.y() * y_stride + content_rect.x();
        const int u_stride = frame->stride(VideoFrame::Plane::kU);
        uint8_t* const u = frame->GetWritableVisibleData(VideoFrame::Plane::kU) + (content_rect.y() / 2) * u_stride + (content_rect.x() / 2);
        const int v_stride = frame->stride(VideoFrame::Plane::kV);
        uint8_t* const v = frame->GetWritableVisibleData(VideoFrame::Plane::kV) + (content_rect.y() / 2) * v_stride + (content_rect.x() / 2);
        bool success = result->ReadI420Planes(y, y_stride, u, u_stride, v, v_stride);
        if (success) {
            // Per CopyOutputResult header comments, I420_PLANES results are always in
            // the Rec.709 color space.
            frame->set_color_space(gfx::ColorSpace::CreateREC709());
            UMA_HISTOGRAM_CAPTURE_DURATION("I420", base::TimeTicks::Now() - frame_capture.request_time);
            frame_capture.CaptureSucceeded();
        } else {
        ...
        if (buffer_format_preference_ == mojom::BufferFormatPreference::kDefault) {
            ...
            uint8_t* const pixels = frame->GetWritableVisibleData(VideoFrame::Plane::kARGB) + content_rect.y() * stride + content_rect.x() * 4;
            bool success = result->ReadRGBAPlane(pixels, stride);
            if (success) {
                frame->set_color_space(result->GetRGBAColorSpace());
                UMA_HISTOGRAM_CAPTURE_DURATION("RGBA", base::TimeTicks::Now() - frame_capture.request_time);
                frame_capture.CaptureSucceeded();
            } else {
                frame_capture.CaptureFailed(CaptureResult::kARGBReadbackFailed);
            }
        } else {
            CHECK_EQ(buffer_format_preference_, mojom::BufferFormatPreference::kPreferGpuMemoryBuffer);
            // GMB ARGB results are written to the existing pool texture.
            if (result->IsEmpty()) {
                frame_capture.CaptureFailed(CaptureResult::kGpuMemoryBufferReadbackFailed);
            } else {
                UMA_HISTOGRAM_CAPTURE_DURATION("RGBA", base::TimeTicks::Now() - frame_capture.request_time);
                frame_capture.CaptureSucceeded();
            }
        ...
        UMA_HISTOGRAM_CAPTURE_DURATION("NV12", base::TimeTicks::Now() - frame_capture.request_time);
    ...
    if (frame_capture.success()) {
        ...
        if (!frame->HasMappableGpuBuffer()) {
            const VideoCaptureOverlay::CapturedFrameProperties frame_properties { frame_capture.region_properties, content_rect, frame->format() };
            ...
            auto overlay_renderer = VideoCaptureOverlay::MakeCombinedRenderer(GetOverlaysInOrder(), frame_properties);

            if (overlay_renderer) {
                TRACE_EVENT("gpu.capture", "BlendVideoCaptureOverlays", "frame_properties", frame_properties.ToString());
                std::move(overlay_renderer).Run(frame.get());
            }
        }

        const gfx::Rect result_rect = gfx::Rect(content_rect.origin(), result->size());
        DCHECK(IsCompatibleWithFormat(result_rect, pixel_format_));
        if (frame->visible_rect() != result_rect && !frame->HasMappableGpuBuffer()) {
...
void FrameSinkVideoCapturerImpl::OnFrameReadyForDelivery(const FrameCapture& frame_capture)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    CHECK_GE(frame_capture.capture_frame_number, next_delivery_frame_number_);
    ...
    if (frame_capture.success()) {
        ...
        base::TimeDelta sample = *frame->metadata().capture_end_time - *frame->metadata().capture_begin_time;

        if (frame->format() == media::PIXEL_FORMAT_I420) {
            UMA_HISTOGRAM_CUSTOM_TIMES("Viz.FrameSinkVideoCapturer.I420.TotalDuration", sample, base::Milliseconds(1), base::Seconds(1), 50);
        } else if (frame->format() == media::PIXEL_FORMAT_NV12) {
            UMA_HISTOGRAM_CUSTOM_TIMES("Viz.FrameSinkVideoCapturer.NV12.TotalDuration", sample, base::Milliseconds(1), base::Seconds(1), 50);
        }

        UMA_HISTOGRAM_CUSTOM_TIMES("Viz.FrameSinkVideoCapturer.TotalDuration", sample, base::Milliseconds(1), base::Seconds(1), 50);
    }
...
void FrameSinkVideoCapturerImpl::MaybeDeliverFrame(FrameCapture frame_capture)
{
    ...
    if (frame_capture.success()) {
        ...
        // a new frame.
        if (frame_capture.frame->metadata().sub_capture_target_version != sub_capture_target_version_) {
            frame_capture.CaptureFailed(CaptureResult::kSubCaptureTargetChanged);
        } else if (!oracle_->CompleteCapture(frame_capture.oracle_frame_number, frame_capture.success(), &media_ticks)) {
    ...
    if (!frame_capture.success()) {
        ...
        TRACE_EVENT_END("gpu.capture", CaptureTrack(metadata), "result", frame_capture.result());
        TRACE_EVENT_END(
            "gpu.capture", FrameInUseTrack(metadata), "frame_number", metadata.capture_counter, "utilization_pct", AsPercent(GetPipelineUtilization()));
        MaybeScheduleRefreshFrame();
        return;
    }
    ...
    TRACE_EVENT_END(
        "gpu.capture", CaptureTrack(frame_capture.frame_metadata()), "result", frame_capture.result(), "timestamp_micros", frame->timestamp().InMicroseconds());
    ...
    CHECK(handle);
    CHECK(!handle->is_read_only_shmem_region() || handle->get_read_only_shmem_region().IsValid());
    ...
    mojo::MakeSelfOwnedReceiver(
        std::make_unique<InFlightFrameDelivery>(
            base::BindOnce(&FrameSinkVideoCapturerImpl::NotifyFrameReleased, capture_weak_factory_.GetWeakPtr(), std::move(frame)),
            base::BindOnce(&VideoCaptureOracle::RecordConsumerFeedback, feedback_weak_factory_.GetWeakPtr(), frame_capture.oracle_frame_number)),
    ...
    consumer_->OnFrameCaptured(std::move(handle), std::move(info), frame_capture.content_rect, std::move(callbacks));
    consumer_informed_of_empty_region_ = false;
}

gfx::Size FrameSinkVideoCapturerImpl::AdjustSizeForPixelFormat(const gfx::Size& raw_size) const
{
    ...
    CHECK(media::PIXEL_FORMAT_I420 == pixel_format_ || media::PIXEL_FORMAT_NV12 == pixel_format_);
...
// static
gfx::Rect FrameSinkVideoCapturerImpl::ExpandRectToI420SubsampleBoundaries(const gfx::Rect& rect)
{
...
bool FrameSinkVideoCapturerImpl::ShouldMark(const media::VideoFrame& frame, int64_t content_version) const
{
    return !marked_frame_ || content_version > content_version_in_marked_frame_ || frame.coded_size() != marked_frame_->coded_size();
}

void FrameSinkVideoCapturerImpl::MarkFrame(scoped_refptr<media::VideoFrame> frame, int64_t content_version)
{
    marked_frame_ = frame;
    content_version_in_marked_frame_ = marked_frame_ ? content_version : -1;
}

bool FrameSinkVideoCapturerImpl::CanResurrectFrame(const gfx::Size& size) const
{
    return content_version_ == content_version_in_marked_frame_ && marked_frame_ && marked_frame_->coded_size() == size;
}

void FrameSinkVideoCapturerImpl::NotifyFrameReleased(scoped_refptr<media::VideoFrame> frame)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    num_frames_in_flight_--;
    const media::VideoFrameMetadata metadata = frame->metadata();
    TRACE_EVENT_END("gpu.capture", FrameInUseTrack(metadata), "frame_number", metadata.capture_counter, "utilization_pct", AsPercent(GetPipelineUtilization()));
}

float FrameSinkVideoCapturerImpl::GetPipelineUtilization() const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    return num_frames_in_flight_ / (kDesignLimitMaxFrames * kTargetPipelineUtilization);
}
...
void FrameSinkVideoCapturerImpl::MaybeInformConsumerOfEmptyRegion()
{
    ...
    if (!consumer_ || !target_ || !IsRegionCapture(target_->sub_target) || consumer_informed_of_empty_region_) {
        return;
    }
