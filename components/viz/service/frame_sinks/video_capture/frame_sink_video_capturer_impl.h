...
class VIZ_SERVICE_EXPORT FrameSinkVideoCapturerImpl final : public CapturableFrameSink::Client,
                                                            public VideoCaptureOverlay::FrameSource,
                                                            public mojom::FrameSinkVideoCapturer {
public:
    using GpuMemoryBufferVideoFramePoolContext = media::RenderableGpuMemoryBufferVideoFramePool::Context;
    ...
    // unit testing.
    FrameSinkVideoCapturerImpl(FrameSinkVideoCapturerManager& frame_sink_manager, GmbVideoFramePoolContextProvider* gmb_video_frame_pool_context_provider,
        mojo::PendingReceiver<mojom::FrameSinkVideoCapturer> receiver, std::unique_ptr<media::VideoCaptureOracle> oracle, bool log_to_webrtc);

    FrameSinkVideoCapturerImpl(const FrameSinkVideoCapturerImpl&) = delete;
    FrameSinkVideoCapturerImpl& operator=(const FrameSinkVideoCapturerImpl&) = delete;
    ...
    // can be resolved.
    const std::optional<VideoCaptureTarget>& target() const
    {
        return target_;
    }
    ...
    void ChangeTarget(const std::optional<VideoCaptureTarget>& target, uint32_t sub_capture_target_version) final;
    void Start(mojo::PendingRemote<mojom::FrameSinkVideoConsumer> consumer, mojom::BufferFormatPreference buffer_format_preference) final;
    void Stop() final;
    ...
    void CreateOverlay(int32_t stacking_index, mojo::PendingReceiver<mojom::FrameSinkVideoCaptureOverlay> receiver) final;
    ...
    static constexpr media::VideoPixelFormat kDefaultPixelFormat = media::PIXEL_FORMAT_I420;
    static constexpr gfx::ColorSpace kDefaultColorSpace = gfx::ColorSpace::CreateREC709();
    ...
    // `pixel_format`.
    static gfx::Rect GetContentRectangle(const gfx::Rect& visible_rect, const gfx::Size& source_size, media::VideoPixelFormat pixel_format);

private:
    friend class FrameSinkVideoCapturerTest;

    using OracleFrameNumber = decltype(std::declval<media::VideoCaptureOracle>().next_frame_number());
    ...
    void OnFrameDamaged(const gfx::Size& root_render_pass_size, const gfx::Rect& damage_rect, base::TimeTicks target_display_time,
    ...
    void MaybeCaptureFrame(
        media::VideoCaptureOracle::Event event, const gfx::Rect& damage_rect, base::TimeTicks event_time, const CompositorFrameMetadata& frame_metadata);
    ...
    class FrameCapture {
    public:
        FrameCapture(int64_t capture_frame_number, OracleFrameNumber oracle_frame_number, int64_t content_version, gfx::Rect content_rect,
            CapturableFrameSink::RegionProperties region_properties, scoped_refptr<media::VideoFrame> frame, base::TimeTicks request_time);
        ...
        bool finished() const
        {
            return result_ != CaptureResult::kPending;
        }

        // Returns true if the capture is successful so far.
        bool success() const
        {
            return result_ == CaptureResult::kSuccess;
        }
    ...
    void DidCopyFrame(FrameCapture capture, std::unique_ptr<CopyOutputResult> result);
    ...
    bool ShouldMark(const media::VideoFrame& frame, int64_t content_version) const;
    ...
    void MarkFrame(scoped_refptr<media::VideoFrame> frame, int64_t content_version = -1);
    ...
    mojom::BufferFormatPreference buffer_format_preference_ = mojom::BufferFormatPreference::kDefault;
    ...
    raw_ptr<GmbVideoFramePoolContextProvider> gmb_video_frame_pool_context_provider_;
