...
void ExpectValidHandleForDelivery(const base::ReadOnlySharedMemoryRegion& region)
{
    EXPECT_TRUE(region.IsValid());
    constexpr int kI420BitsPerPixel = 12;
    EXPECT_LE(static_cast<size_t>(kSize.GetArea() * kI420BitsPerPixel / 8), region.GetSize());
}

TEST(SharedMemoryVideoFramePoolTest, FramesConfiguredCorrectly)
{
    SharedMemoryVideoFramePool pool(1);
    const scoped_refptr<media::VideoFrame> frame = pool.ReserveVideoFrame(kFormat, kSize);
    ASSERT_TRUE(frame);
...
{
    constexpr gfx::Size kSmallerSize = gfx::Size(kSize.width() / 2, kSize.height() / 2);
    constexpr gfx::Size kBiggerSize = gfx::Size(kSize.width() * 2, kSize.height() * 2);

    SharedMemoryVideoFramePool pool(1);

    // Reserve a frame of baseline size and then free it to return it to the pool.
    scoped_refptr<media::VideoFrame> frame = pool.ReserveVideoFrame(kFormat, kSize);
    ASSERT_TRUE(frame);
    ...
    ASSERT_TRUE(frame);
    {
        auto handle = pool.CloneHandleForDelivery(*frame);
        ExpectValidHandleForDelivery(handle->get_read_only_shmem_region());
        EXPECT_EQ(baseline_bytes_allocated, handle->get_read_only_shmem_region().GetSize());
    }
    frame = nullptr; // Returns frame to pool.
    ...
    size_t larger_buffer_bytes_allocated;
    {
        auto handle = pool.CloneHandleForDelivery(*frame);
        ExpectValidHandleForDelivery(handle->get_read_only_shmem_region());
        larger_buffer_bytes_allocated = handle->get_read_only_shmem_region().GetSize();
    ...
    for (const auto& size : kTheSmallerSizes) {
        ...
        ASSERT_TRUE(frame);
        {
            auto handle = pool.CloneHandleForDelivery(*frame);
            ExpectValidHandleForDelivery(handle->get_read_only_shmem_region());
            EXPECT_EQ(larger_buffer_bytes_allocated, handle->get_read_only_shmem_region().GetSize());
        }
        frame = nullptr; // Returns frame to pool.
    }
}
...
{
    static_assert(VideoFrame::Plane::kU == (VideoFrame::Plane::kY + 1) && VideoFrame::Plane::kV == (VideoFrame::Plane::kU + 1),
    ...
    for (int plane = VideoFrame::Plane::kY; plane <= VideoFrame::Plane::kV; ++plane) {
        ...
        for (int y = 0; y < frame.rows(plane); ++y) {
            ...
            for (int x = 0; x < frame.row_bytes(plane); ++x) {
                EXPECT_EQ(expected_value, row[x]) << "at row " << y << " in plane " << plane;
...
{
    constexpr gfx::ColorSpace kArbitraryColorSpace1 = gfx::ColorSpace::CreateREC709();
    constexpr gfx::ColorSpace kArbitraryColorSpace2 = gfx::ColorSpace::CreateSRGB();
