...
{
    return (update_rect.x() % 2 == 0) && (update_rect.y() % 2 == 0) && (update_rect.width() % 2 == 0) && (update_rect.height() % 2 == 0);
}

// Returns true if |frame|'s device scale factor, page scale factor and root
// scroll offset are equal to the expected values.
bool CompareVarsInCompositorFrameMetadata(const VideoFrame& frame, float device_scale_factor, float page_scale_factor, const gfx::PointF& root_scroll_offset)
{
    ...
    bool valid = dsf.has_value() && psf.has_value() && rso_x.has_value() && rso_y.has_value();

    return valid && *dsf == device_scale_factor && *psf == page_scale_factor && gfx::PointF(*rso_x, *rso_y) == root_scroll_offset;
}
...
media::VideoPixelFormat CopyOutputRequestFormatToVideoPixelFormat(CopyOutputRequest::ResultFormat format)
{
    switch (format) {
...
gfx::Size GetBufferSizeInPixelsForVideoPixelFormat(media::VideoPixelFormat format, const gfx::Size& coded_size)
{
    switch (format) {
    ...
    case media::PIXEL_FORMAT_ARGB:
        return coded_size;
    case media::PIXEL_FORMAT_NV12:
        return { cc::MathUtil::CheckedRoundUp(coded_size.width(), 2), cc::MathUtil::CheckedRoundUp(coded_size.height(), 2) };
    default:
        NOTREACHED();
    }
}
...
const struct SizeSet {
    ...
    {
        return FrameSinkVideoCapturerImpl::GetContentRectangle(gfx::Rect(capture_size), source_size, format);
    }

    gfx::Rect ExpectedContentRect(CopyOutputRequest::ResultFormat format) const
    {
        return ExpectedContentRect(CopyOutputRequestFormatToVideoPixelFormat(format));
    }
...
} kSizeSets[5] = { { gfx::Size(100, 100), gfx::Size(32, 18) }, { gfx::Size(64, 18), gfx::Size(32, 18) }, { gfx::Size(64, 18), gfx::Size(64, 18) },
    { gfx::Size(100, 100), gfx::Size(16, 8) }, { gfx::Size(640, 478), gfx::Size(16, 16) } };
...
// made.
void PropagateMojoTasks(scoped_refptr<base::TestMockTimeTaskRunner> runner = nullptr)
{
    if (runner) {
        runner->RunUntilIdle();
    }
...
public:
    MOCK_METHOD1(FindCapturableFrameSink, CapturableFrameSink*(const VideoCaptureTarget& target));
    MOCK_METHOD1(OnCapturerConnectionLost, void(FrameSinkVideoCapturerImpl* capturer));
};
...
public:
    ...
    scoped_refptr<VideoFrame> TakeFrame(int i)
    {
...
private:
    void OnFrameCaptured(media::mojom::VideoBufferHandlePtr data, media::mojom::VideoFrameInfoPtr info, const gfx::Rect& expected_content_rect,
        mojo::PendingRemote<mojom::FrameSinkVideoConsumerFrameCallbacks> callbacks) final
    {
        CHECK(info);
        ...
        if (data->is_read_only_shmem_region()) {
            // kDefault + I420 / RGBA
            base::ReadOnlySharedMemoryRegion& shmem_region = data->get_read_only_shmem_region();
            ...
            auto required_bytes_to_hold_planes = media::VideoFrame::AllocationSize(info->pixel_format, info->coded_size);
            ...
            frame = media::VideoFrame::WrapExternalData(info->pixel_format, info->coded_size, info->visible_rect, info->visible_rect.size(),
                mapping.GetMemoryAs<const uint8_t>(), mapping.size(), info->timestamp);
            ASSERT_TRUE(frame);
            frame->AddDestructionObserver(base::BindOnce([](base::ReadOnlySharedMemoryMapping mapping) {}, std::move(mapping)));
            ...
            auto gmb_dummy = std::make_unique<media::FakeGpuMemoryBuffer>(
                GetBufferSizeInPixelsForVideoPixelFormat(info->pixel_format, info->coded_size), VideoPixelFormatToGfxBufferFormat(info->pixel_format).value());

            // The frame is only gonna tell Letterbox to skip the test.
            frame = media::VideoFrame::WrapExternalGpuMemoryBuffer(info->visible_rect, info->visible_rect.size(), std::move(gmb_dummy), info->timestamp);
            ASSERT_TRUE(frame);
        } else {
            NOTREACHED();
        }
        ...
        done_callbacks_.push_back(base::BindOnce(&mojom::FrameSinkVideoConsumerFrameCallbacks::Done, std::move(callbacks_remote)));
    }
...
public:
    FakeGpuCopyResult(Format format, const gfx::Rect rect)
        : CopyOutputResult(format, CopyOutputResult::Destination::kNativeTextures, rect, false)
        , format_(format)
        , result_(TextureResult(gpu::Mailbox {}, GetColorSpaceForPixelFormat(CopyOutputRequestFormatToVideoPixelFormat(format))))
    {
    }

    const TextureResult* GetTextureResult() const final
    {
        return &result_;
    }
...
public:
    SolidColorRGBAResult(const gfx::Rect rect, SkColor color)
        : CopyOutputResult(CopyOutputResult::Format::RGBA, CopyOutputResult::Destination::kSystemMemory, rect, false)
    {
        bitmap_.setInfo(SkImageInfo::MakeN32Premul(size().width(), size().height(), SkColorSpace::MakeSRGB()));
        bitmap_.allocPixels();
...
public:
    SolidColorI420Result(const gfx::Rect rect, YUVColor color)
        : CopyOutputResult(CopyOutputResult::Format::I420_PLANES, CopyOutputResult::Destination::kSystemMemory, rect, false)
        , color_(color)
    {
    }

    bool ReadI420Planes(uint8_t* y_out, int y_out_stride, uint8_t* u_out, int u_out_stride, uint8_t* v_out, int v_out_stride) const final
    {
        CHECK(y_out);
...
public:
    ...
    std::optional<CapturableFrameSink::RegionProperties> GetRequestRegionProperties(const VideoCaptureSubTarget& sub_target) const override
    {
    ...
    void RequestCopyOfOutput(PendingCopyOutputRequest pending_copy_output_request) override
    {
        ...
        auto expected_content_rect = gfx::Rect(size_set_.ExpectedContentRect(request->result_format()).size());
        ...
        case CopyOutputResult::Destination::kSystemMemory: {
            ...
            case CopyOutputRequest::ResultFormat::I420_PLANES: {
                result = std::make_unique<SolidColorI420Result>(request->result_selection(), RGBToYUV(color_));
                break;
            }
            case CopyOutputRequest::ResultFormat::RGBA: {
                result = std::make_unique<SolidColorRGBAResult>(request->result_selection(), color_);
                break;
            }
            default: {
                NOTREACHED();
        ...
        case CopyOutputResult::Destination::kNativeTextures: {
            ...
            result = std::make_unique<FakeGpuCopyResult>(request->result_format(), request->result_selection());
            break;
        }
        default: {
            NOTREACHED();
        ...
        results_.push_back(
            base::BindOnce([](std::unique_ptr<CopyOutputRequest> request, std::unique_ptr<CopyOutputResult> result) { request->SendResult(std::move(result)); },
    ...
    const gfx::Size& source_size() const
    {
        return size_set_.source_size;
    }
    ...
    void set_size_set(const SizeSet& size_set)
    {
        size_set_ = size_set;
    }
    ...
    void SetCopyOutputColor(SkColor color)
    {
        color_ = color;
    }
    ...
    int number_clients_capturing() const
    {
        return number_clients_capturing_;
    }
...
public:
    ...
    bool CompleteCapture(int frame_number, bool capture_was_successful, base::TimeTicks* frame_timestamp) override
    {
        capture_was_successful &= !return_false_on_complete_capture_;
        return media::VideoCaptureOracle::CompleteCapture(frame_number, capture_was_successful, frame_timestamp);
    }
...
namespace {
bool IsLetterboxedI420Plane(int plane, uint8_t component, gfx::Rect content_rect, const VideoFrame& frame, testing::MatchResultListener* result_listener)
{
    gfx::Rect content_rect_copy = content_rect;
    if (plane != VideoFrame::Plane::kY) {
        content_rect_copy = gfx::Rect(content_rect_copy.x() / 2, content_rect_copy.y() / 2, content_rect_copy.width() / 2, content_rect_copy.height() / 2);
    }
            ...
            if (content_rect_copy.Contains(gfx::Point(col, row))) {
                if (p[col] != component) {
                    *result_listener << " where pixel at (" << col << ", " << row
                                     ...
                                     << std::hex << static_cast<unsigned int>(component) << " but is 0x" << std::hex << static_cast<unsigned int>(p[col]);
                    return false;
                }
...
bool IsLetterboxedRGBA(SkColor color, gfx::Rect content_rect, const VideoFrame& frame, testing::MatchResultListener* result_listener)
{
    SkBitmap bitmap;
    auto bitmap_info = SkImageInfo::MakeN32Premul(frame.coded_size().width(), frame.coded_size().height());
    bitmap.installPixels(bitmap_info, const_cast<uint8_t*>(frame.visible_data(VideoFrame::Plane::kARGB)), frame.stride(VideoFrame::Plane::kARGB));
            ...
            if (content_rect.Contains(gfx::Point(col, row))) {
                if (pixel != color) {
                    *result_listener << " where pixel at (" << col << ", " << row
                                     ...
                                     << color_utils::SkColorToRgbString(color) << " but is " << color_utils::SkColorToRgbString(pixel);
                    return false;
                }
            } else { // Letterbox border around content.
                ...
                if (pixel != kLetterboxColor) {
                    *result_listener << " where pixel at (" << col << ", " << row
                                     << ") should be outside content rectangle and the "
                                        "component should match "
                                     << color_utils::SkColorToRgbString(kLetterboxColor) << " but is " << color_utils::SkColorToRgbString(pixel);
                    return false;
                }
            }
        }
...
MATCHER_P3(IsLetterboxedFrame, color, content_rect, pixel_format, "")
{
    ...
    case media::PIXEL_FORMAT_I420: {
        ...
        return IsLetterboxedI420Plane(VideoFrame::Plane::kY, yuvColor.y, content_rect, frame, result_listener)
            && IsLetterboxedI420Plane(VideoFrame::Plane::kU, yuvColor.u, content_rect, frame, result_listener)
            && IsLetterboxedI420Plane(VideoFrame::Plane::kV, yuvColor.v, content_rect, frame, result_listener);
    }
    default: {
        NOTREACHED();
    }
...
public:
    using PropertiesCallback = base::RepeatingCallback<void(const CapturedFrameProperties&)>;
    TestVideoCaptureOverlay(FrameSource& frame_source, mojo::PendingReceiver<mojom::FrameSinkVideoCaptureOverlay> receiver, PropertiesCallback properties_cb)
    ...
    OnceRenderer MakeRenderer(const CapturedFrameProperties& properties) override
    {
...
class TestGmbVideoFramePoolContext : public media::RenderableGpuMemoryBufferVideoFramePool::Context {
public:
    ...
    scoped_refptr<gpu::ClientSharedImage> CreateSharedImage(gfx::GpuMemoryBuffer* gpu_memory_buffer, const SharedImageFormat& si_format,
        const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, gpu::SharedImageUsageSet usage,
        gpu::SyncToken& sync_token) override
    {
        return context_provider_->SharedImageInterface()->CreateSharedImage(
            { si_format, gpu_memory_buffer->GetSize(), color_space, surface_origin, alpha_type, usage, "FrameSinkVideoCapturerImplUnittest" },
            gpu_memory_buffer->CloneHandle());
    }

    scoped_refptr<gpu::ClientSharedImage> CreateSharedImage(const gfx::Size& size, gfx::BufferUsage buffer_usage, const SharedImageFormat& si_format,
        const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, gpu::SharedImageUsageSet usage,
        gpu::SyncToken& sync_token) override
    {
        context_provider_->SharedImageInterface()->UseTestGMBInSharedImageCreationWithBufferUsage();
        return context_provider_->SharedImageInterface()->CreateSharedImage(
            { si_format, size, color_space, surface_origin, alpha_type, usage, "FrameSinkVideoCapturerImplUnittest" }, gpu::kNullSurfaceHandle, buffer_usage);
    }

    void DestroySharedImage(const gpu::SyncToken& sync_token, scoped_refptr<gpu::ClientSharedImage> shared_image) override
    {
        CHECK(shared_image);
...
class TestGmbVideoFramePoolContextProvider : public GmbVideoFramePoolContextProvider {
public:
    ~TestGmbVideoFramePoolContextProvider() override = default;

    std::unique_ptr<media::RenderableGpuMemoryBufferVideoFramePool::Context> CreateContext(base::OnceClosure on_context_lost) final
    {
...
class FrameSinkVideoCapturerTest : public testing::Test,
                                   public testing::WithParamInterface<std::tuple<mojom::BufferFormatPreference, media::VideoPixelFormat>> {
public:
    ...
    {
        auto oracle = std::make_unique<InstrumentedVideoCaptureOracle>(true /* enable_auto_throttling */);
        oracle_ = oracle.get();

        gmb_context_provider_ = std::make_unique<TestGmbVideoFramePoolContextProvider>();

        capturer_
            = std::make_unique<FrameSinkVideoCapturerImpl>(frame_sink_manager_, gmb_context_provider_.get(), mojo::NullReceiver(), std::move(oracle), false);
    }
    ...
    void SetUp() override
    {
        ...
        task_runner_
            = new base::TestMockTimeTaskRunner(base::Time::Now(), base::TimeTicks() + base::Seconds(1), base::TestMockTimeTaskRunner::Type::kStandalone);
        ...
        // task runner.
        capturer_->refresh_frame_retry_timer_.emplace(task_runner_->GetMockTickClock());
        ...
        ASSERT_EQ(FrameSinkVideoCapturerImpl::kDefaultPixelFormat, capturer_->pixel_format_);
        ...
        capturer_->SetResolutionConstraints(size_set_.capture_size, size_set_.capture_size, false);
    }
    ...
    {
        return buffer_format_preference_ == mojom::BufferFormatPreference::kPreferGpuMemoryBuffer;
    }
    ...
    const SizeSet& size_set()
    {
        return size_set_;
    }

    void NotifyFrameDamaged(gfx::Rect damage_rect, float device_scale_factor = kDefaultDeviceScaleFactor, float page_scale_factor = kDefaultPageScaleFactor,
        gfx::PointF root_scroll_offset = kDefaultRootScrollOffset)
    {
        ...
        capturer_->OnFrameDamaged(frame_sink_.source_size(), damage_rect, GetNextVsync(), metadata);
    }
    ...
    {
        return FrameSinkVideoCapturerImpl::ExpandRectToI420SubsampleBoundaries(rect);
    }

    void InsertOverlay(std::unique_ptr<VideoCaptureOverlay> overlay)
    {
        capturer_->overlays_.insert_or_assign(capturer_->overlays_.size() + 1, std::move(overlay));
    }
...
{
    EXPECT_CALL(frame_sink_manager_, FindCapturableFrameSink(kVideoCaptureTarget)).WillRepeatedly(Return(&frame_sink_));

    EXPECT_FALSE(capturer_->target());
    capturer_->ChangeTarget(kVideoCaptureTarget,
        /*sub_capture_target_version=*/0);
    EXPECT_EQ(kVideoCaptureTarget.frame_sink_id, capturer_->target()->frame_sink_id);
...
{
    EXPECT_CALL(frame_sink_manager_, FindCapturableFrameSink(kVideoCaptureTarget)).WillRepeatedly(Return(nullptr));
...
TEST_P(FrameSinkVideoCapturerTest, CapturesCompositedFrames)
{
    ...
    const int num_update_frames = 3 * FrameSinkVideoCapturerImpl::kDesignLimitMaxFrames;
    EXPECT_CALL(consumer, OnFrameCapturedMock()).Times(num_refresh_frames + num_update_frames);
    EXPECT_CALL(consumer, OnStopped()).Times(1);
    StartCapture(&consumer);
    ...
    EXPECT_THAT(consumer.TakeFrame(0), IsLetterboxedFrame(SkColorSetARGB(255, 128, 128, 128), size_set().ExpectedContentRect(pixel_format_), pixel_format_));
    consumer.SendDoneNotification(0);

    // Drive the capturer pipeline for a series of frame composites.
    base::TimeDelta last_timestamp;
    for (int i = num_refresh_frames; i < num_refresh_frames + num_update_frames; ++i) {
        SCOPED_TRACE(testing::Message() << "frame #" << i);

        // Move time forward to the next display vsync.
        AdvanceClockToNextVsync();
        const base::TimeTicks expected_reference_time = task_runner_->NowTicks() + kVsyncInterval;

        // Change the content of the frame sink and notify the capturer of the
        // damage.
        const SkColor color = SkColorSetARGB(255, static_cast<uint8_t>(i << 4), static_cast<uint8_t>((i << 4) + 0x10), static_cast<uint8_t>((i << 4) + 0x20));
        frame_sink_.SetCopyOutputColor(color);
        task_runner_->FastForwardBy(kVsyncInterval / 4);
        const base::TimeTicks expected_capture_begin_time = task_runner_->NowTicks();
        ...
        EXPECT_THAT(frame, IsLetterboxedFrame(color, size_set().ExpectedContentRect(pixel_format_), pixel_format_));
        ...
        EXPECT_NEAR(media::limits::kMaxFramesPerSecond, *metadata.frame_rate, 0.001);
...
TEST_P(FrameSinkVideoCapturerTest, DeliversFramesInOrder)
{
    ...
    ASSERT_EQ(num_refresh_frames, frame_sink_.num_copy_results());
    for (int i = num_refresh_frames; i < num_frames; ++i) {
        colors.push_back(SkColorSetARGB(255, static_cast<uint8_t>(i << 4), static_cast<uint8_t>((i << 4) + 0x10), static_cast<uint8_t>((i << 4) + 0x20)));
        frame_sink_.SetCopyOutputColor(colors.back());
        AdvanceClockToNextVsync();
        NotifyFrameDamaged(gfx::Rect(size_set().source_size));
    }
    ...
    const auto expected_content_rect = size_set().ExpectedContentRect(pixel_format_);
    frame_sink_.SendCopyOutputResult(0);
    ASSERT_EQ(1, consumer.num_frames_received());
    EXPECT_THAT(consumer.TakeFrame(0), IsLetterboxedFrame(colors[0], expected_content_rect, pixel_format_));
    ...
    EXPECT_THAT(consumer.TakeFrame(1), IsLetterboxedFrame(colors[1], expected_content_rect, pixel_format_));
    EXPECT_THAT(consumer.TakeFrame(2), IsLetterboxedFrame(colors[2], expected_content_rect, pixel_format_));
    EXPECT_THAT(consumer.TakeFrame(3), IsLetterboxedFrame(colors[3], expected_content_rect, pixel_format_));
    frame_sink_.SendCopyOutputResult(4);
    ASSERT_EQ(5, consumer.num_frames_received());
    EXPECT_THAT(consumer.TakeFrame(4), IsLetterboxedFrame(colors[4], expected_content_rect, pixel_format_));
...
TEST_P(FrameSinkVideoCapturerTest, CancelsInFlightCapturesOnStop)
{
    ...
    for (int i = 0; i < num_completed_captures; ++i) {
        ...
        frame_sink_.SendCopyOutputResult(i);
        ASSERT_EQ(i + 1, consumer.num_frames_received());
        EXPECT_THAT(consumer.TakeFrame(i), IsLetterboxedFrame(color1, size_set().ExpectedContentRect(pixel_format_), pixel_format_));
    }
    ...
    EXPECT_CALL(consumer2, OnFrameCapturedMock()).Times(num_captures_for_second_consumer);
    ...
    for (int i = 0; i < num_captures_for_second_consumer; ++i) {
        ...
        EXPECT_THAT(
            consumer2.TakeFrame(consumer2.num_frames_received() - 1), IsLetterboxedFrame(color2, size_set().ExpectedContentRect(pixel_format_), pixel_format_));
    }
...
{
    frame_sink_.SetCopyOutputColor(SkColorSetARGB(255, 128, 128, 128));
    ON_CALL(frame_sink_manager_, FindCapturableFrameSink(kVideoCaptureTarget)).WillByDefault(Return(&frame_sink_));
...
TEST_P(FrameSinkVideoCapturerTest, ResurrectsFramesForChangingCaptureResolution)
{
    frame_sink_.SetCopyOutputColor(SkColorSetARGB(255, 128, 128, 128));
    EXPECT_CALL(frame_sink_manager_, FindCapturableFrameSink(kVideoCaptureTarget)).WillRepeatedly(Return(&frame_sink_));
...
TEST_P(FrameSinkVideoCapturerTest, CompositorFrameMetadataReachesConsumer)
{
    ...
    EXPECT_TRUE(CompareVarsInCompositorFrameMetadata(
        *(consumer.TakeFrame(cur_frame_index)), kDefaultDeviceScaleFactor, kDefaultPageScaleFactor, kDefaultRootScrollOffset));
    ...
    AdvanceClockToNextVsync();
    NotifyFrameDamaged(gfx::Rect(size_set().source_size), kNewDeviceScaleFactor, kNewPageScaleFactor, kNewRootScrollOffset);
    frame_sink_.SendCopyOutputResult(++cur_frame_index);
    ++expected_frames_count;
    EXPECT_EQ(expected_frames_count, consumer.num_frames_received());
    EXPECT_TRUE(CompareVarsInCompositorFrameMetadata(*(consumer.TakeFrame(cur_frame_index)), kNewDeviceScaleFactor, kNewPageScaleFactor, kNewRootScrollOffset));
    consumer.SendDoneNotification(cur_frame_index);
    ...
    EXPECT_EQ(expected_frames_count, consumer.num_frames_received());
    EXPECT_TRUE(
        CompareVarsInCompositorFrameMetadata(*(consumer.TakeFrame(++cur_frame_index)), kNewDeviceScaleFactor, kNewPageScaleFactor, kNewRootScrollOffset));
    StopCapture();
}
...
TEST_P(FrameSinkVideoCapturerTest, DeliversUpdateRectAndCaptureCounter)
{
    ...
    // Expect to see the refresh frame delivered to the consumer, along with
    // default metadata values.
    int cur_capture_index = 0, cur_frame_index = 0, expected_frames_count = 1, previous_capture_counter_received = 0;
    frame_sink_.SendCopyOutputResult(cur_capture_index);
    EXPECT_EQ(expected_frames_count, consumer.num_frames_received());
    {
        auto received_frame = consumer.TakeFrame(cur_frame_index);
        EXPECT_EQ(gfx::Rect(size_set().capture_size), received_frame->metadata().capture_update_rect);
        previous_capture_counter_received = *received_frame->metadata().capture_counter;
    }
    consumer.SendDoneNotification(cur_frame_index);

    const gfx::Rect kSourceDamageRect = gfx::Rect(3, 7, 60, 45);
    gfx::Rect expected_frame_update_rect
        = copy_output::ComputeResultRect(kSourceDamageRect, gfx::Vector2d(size_set().source_size.width(), size_set().source_size.height()),
            gfx::Vector2d(size_set().ExpectedContentRect(pixel_format_).width(), size_set().ExpectedContentRect(pixel_format_).height()));
    expected_frame_update_rect.Offset(size_set().ExpectedContentRect(pixel_format_).OffsetFromOrigin());
    ...
    if (pixel_format_ != media::PIXEL_FORMAT_ARGB) {
        ...
        expected_frame_update_rect = ExpandRectToI420SubsampleBoundaries(expected_frame_update_rect);
    ...
    EXPECT_EQ(expected_frames_count, consumer.num_frames_received());
    {
        ...
        EXPECT_EQ(expected_frame_update_rect, *received_frame->metadata().capture_update_rect);
...
TEST_P(FrameSinkVideoCapturerTest, CaptureCounterSkipsWhenFramesAreDropped)
{
    ...
    // Expect to see the refresh frame delivered to the consumer, along with
    // default metadata values.
    int cur_capture_frame_index = 0, cur_receive_frame_index = 0, expected_frames_count = 1, previous_capture_counter_received = 0;
    ...
    {
        auto received_frame = consumer.TakeFrame(++cur_receive_frame_index);
        EXPECT_NE(previous_capture_counter_received + 1, *received_frame->metadata().capture_counter);
    }
    StopCapture();
}
...
TEST_P(FrameSinkVideoCapturerTest, RegionCaptureCropId)
{
    ...
    EXPECT_CALL(frame_sink_manager_, FindCapturableFrameSink(target)).WillRepeatedly(Return(&frame_sink_));
...
TEST_P(FrameSinkVideoCapturerTest, RegionCaptureTargetIsSetLaterWhenNotInitiallyAvailable)
{
    SwitchToSizeSet(kSizeSets[4]);
    ...
    // sinks yet.
    VideoCaptureTarget target(kVideoCaptureTarget.frame_sink_id, kCropId);
    EXPECT_CALL(frame_sink_manager_, FindCapturableFrameSink(target)).WillRepeatedly(Return(nullptr));
...
TEST_P(FrameSinkVideoCapturerTest, ProperlyHandlesCaptureSizeForOverlay)
{
    ...
    auto test_overlay = std::make_unique<TestVideoCaptureOverlay>(*capturer_, overlay_remote.BindNewPipeAndPassReceiver(),
        base::BindLambdaForTesting([&](const VideoCaptureOverlay::CapturedFrameProperties& properties) { frame_properties = properties; }));
    ...
    EXPECT_EQ(kSizeSets[4].source_size, frame_properties->region_properties.root_render_pass_size);
    EXPECT_EQ(kValidCropBounds, frame_properties->region_properties.render_pass_subrect);
...
TEST_P(FrameSinkVideoCapturerTest, ProperlyHandlesSubtreeSizeForOverlay)
{
    ...
    EXPECT_TRUE(frame_properties) << "didn't produce an overlay.";
    EXPECT_EQ(kCaptureBounds.size(), frame_properties->region_properties.root_render_pass_size);
    EXPECT_EQ(kCaptureBounds, frame_properties->region_properties.render_pass_subrect);
    EXPECT_EQ((gfx::Rect { 0, 2, 16, 12 }), frame_properties->content_rect);
}
...
INSTANTIATE_TEST_SUITE_P(All, FrameSinkVideoCapturerTest,
    testing::Values(std::tuple(mojom::BufferFormatPreference::kDefault, media::PIXEL_FORMAT_I420),
        std::tuple(mojom::BufferFormatPreference::kDefault, media::PIXEL_FORMAT_ARGB),
        std::tuple(mojom::BufferFormatPreference::kPreferGpuMemoryBuffer, media::PIXEL_FORMAT_NV12),
        std::tuple(mojom::BufferFormatPreference::kPreferGpuMemoryBuffer, media::PIXEL_FORMAT_ARGB)));
