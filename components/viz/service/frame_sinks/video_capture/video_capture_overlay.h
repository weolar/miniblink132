...
class VIZ_SERVICE_EXPORT VideoCaptureOverlay : public mojom::FrameSinkVideoCaptureOverlay {
public:
    ...
    VideoCaptureOverlay(FrameSource& frame_source, mojo::PendingReceiver<mojom::FrameSinkVideoCaptureOverlay> receiver);
    ...
    std::optional<BlendInformation> CalculateBlendInformation(const CapturedFrameProperties& properties) const;
    ...
    // nothing to render at this time.
    static OnceRenderer MakeCombinedRenderer(const std::vector<VideoCaptureOverlay*>& overlays, const CapturedFrameProperties& properties);
...
private:
    ...
    public:
        Sprite(const SkBitmap& image, const gfx::Size& size, const media::VideoPixelFormat format);
        ...
        // blended over.
        void Blend(const gfx::Rect& src_rect, const gfx::Rect& blit_rect, media::VideoFrame* frame);
