...
public:
    ...
    std::unique_ptr<VideoCaptureOverlay> CreateOverlay()
    {
        ...
        return std::make_unique<VideoCaptureOverlay>(frame_source(), overlay_remote.BindNewPipeAndPassReceiver());
    }
    ...
    static SkBitmap MakeTestBitmap(int cycle)
    {
        ...
        constexpr SkColor4f kTestImageBackground = SkColor4f { 1.0f, 1.0f, 1.0f, 1.0f };
        ...
        SkBitmap result;
        const SkImageInfo info = SkImageInfo::MakeN32Premul(kTestImageSize.width(), kTestImageSize.height(), GetLinearSRGB().ToSkColorSpace());
    ...
    {
        return gfx::ColorSpace(
            gfx::ColorSpace::PrimaryID::BT709, gfx::ColorSpace::TransferID::LINEAR, gfx::ColorSpace::MatrixID::BT709, gfx::ColorSpace::RangeID::LIMITED);
    }
...
{
    mojo::Remote<mojom::FrameSinkVideoCaptureOverlay> overlay_remote;
    VideoCaptureOverlay overlay(frame_source(), overlay_remote.BindNewPipeAndPassReceiver());
...
TEST_F(VideoCaptureOverlayTest, CapturedFramePropertiesAreStringifiable)
{
    ...
    VideoCaptureOverlay::CapturedFrameProperties frame_properties { .region_properties = CapturableFrameSink::RegionProperties { .root_render_pass_size = kSize,
    ...
    EXPECT_STREQ("23,24 27x26 from 100x75 into 10,20 40x30 via transform "
                 "[ 1 0 0 10\n  0 1 0 11\n  0 0 1 0\n  0 0 0 1 ]\n, "
...
{
    VideoCaptureOverlay::BlendInformation blend_info { gfx::Rect(1, 2, 3, 4), gfx::Rect(5, 6, 7, 8), gfx::Rect(9, 10, 11, 12) };

    EXPECT_STREQ("source_region=1,2 3x4, source_region_scaled=5,6 7x8, "
...
TEST_F(VideoCaptureOverlayTest, CalculateBlendInformation_ReturnsNulloptIfEmptyProperties)
{
    EXPECT_FALSE(CreateOverlay()->CalculateBlendInformation(VideoCaptureOverlay::CapturedFrameProperties {}));
}

TEST_F(VideoCaptureOverlayTest, CalculateBlendInformation_ReturnsNulloptIfNoImage)
{
...
TEST_F(VideoCaptureOverlayTest, CalculateBlendInformation_GoldenCase)
{
    ...
    const std::optional<VideoCaptureOverlay::BlendInformation> blend_info = overlay->CalculateBlendInformation(frame_properties);
...
TEST_F(VideoCaptureOverlayTest, DoesNotDoCombinedRenderIfNoOverlaysWouldRender)
{
    constexpr gfx::Size kSize = gfx::Size(100, 75);
    EXPECT_CALL(frame_source(), GetSourceSize()).WillRepeatedly(Return(kSize));
    ...
    const std::vector<VideoCaptureOverlay*> overlays { overlay0.get(), overlay1.get() };
...
class VideoCaptureOverlayRenderTest : public VideoCaptureOverlayTest, public testing::WithParamInterface<VideoPixelFormat> {
public:
    ...
    VideoCaptureOverlayRenderTest& operator=(const VideoCaptureOverlayRenderTest&) = delete;
    ...
    {
        auto frame = VideoFrame::CreateFrame(pixel_format(), size, gfx::Rect(size), size, base::TimeDelta());
    ...
    bool FrameMatchesPNG(const VideoFrame& frame, const char* golden_file)
    {
        ...
        const SkImageInfo canonical_format = SkImageInfo::Make(
            frame.visible_rect().width(), frame.visible_rect().height(), kN32_SkColorType, kUnpremul_SkAlphaType, png_color_space.ToSkColorSpace());
        ...
        case media::PIXEL_FORMAT_ARGB: {
            ...
            // representation.
            const SkImageInfo frame_format = SkImageInfo::Make(frame.visible_rect().width(), frame.visible_rect().height(), kBGRA_8888_SkColorType,
                kUnpremul_SkAlphaType, frame.ColorSpace().ToSkColorSpace());
            canonical_bitmap.writePixels(SkPixmap(frame_format, frame.visible_data(VideoFrame::Plane::kARGB), frame.stride(VideoFrame::Plane::kARGB)), 0, 0);
            break;
        }
        ...
        case media::PIXEL_FORMAT_I420: {
            ...
            std::unique_ptr<gfx::ColorTransform::TriStim[]> colors(new gfx::ColorTransform::TriStim[size.GetArea()]);
            int pos = 0;
            for (int row = 0; row < size.height(); ++row) {
                const uint8_t* y = frame.visible_data(VideoFrame::Plane::kY) + (row * frame.stride(VideoFrame::Plane::kY));
                const uint8_t* u = frame.visible_data(VideoFrame::Plane::kU) + ((row / 2) * frame.stride(VideoFrame::Plane::kU));
                const uint8_t* v = frame.visible_data(VideoFrame::Plane::kV) + ((row / 2) * frame.stride(VideoFrame::Plane::kV));
                for (int col = 0; col < size.width(); ++col) {
                    colors[pos].SetPoint(y[col] / 255.0f, u[col / 2] / 255.0f, v[col / 2] / 255.0f);
                    ++pos;
                }
            }

            // Execute the YUV→RGB conversion.
            gfx::ColorTransform::NewColorTransform(frame.ColorSpace(), png_color_space)->Transform(colors.get(), size.GetArea());
            ...
            for (int row = 0; row < size.height(); ++row) {
                ...
                for (int col = 0; col < size.width(); ++col) {
                    out[col] = ((UINT32_C(255) << SK_A32_SHIFT) | (ToClamped255(colors[pos].x()) << SK_R32_SHIFT)
                        | (ToClamped255(colors[pos].y()) << SK_G32_SHIFT) | (ToClamped255(colors[pos].z()) << SK_B32_SHIFT));
                    ++pos;
                }
            }
        ...
        golden_file_path = golden_file_path.Append(FILE_PATH_LITERAL("video_capture")).Append(base::FilePath::FromUTF8Unsafe(golden_file));
        ...
        if (is_argb_test() && base::CommandLine::ForCurrentProcess()->HasSwitch("video-overlay-capture-test-update-golden-files")) {
        ...
        const bool matches_golden_file = cc::MatchesPNGFile(canonical_bitmap, golden_file_path, comparator);
    ...
    void ExpectRendersAs(
        VideoCaptureOverlay::OnceRenderer* renderers, const char* const* expected_files, const std::size_t count, const gfx::Size& video_frame_size)
    {
...
TEST_P(VideoCaptureOverlayRenderTest, FullCover_NoScaling)
{
    ...
    EXPECT_CALL(frame_source, GetSourceSize()).WillRepeatedly(Return(kSourceSize));
    EXPECT_CALL(frame_source, InvalidateRect(gfx::Rect())).RetiresOnSaturation();
    EXPECT_CALL(frame_source, InvalidateRect(gfx::Rect(kSourceSize))).RetiresOnSaturation();
...
TEST_P(VideoCaptureOverlayRenderTest, FullCover_WithScaling)
{
    ...
    const SkBitmap test_bitmap = MakeTestBitmap(0);
    overlay.SetImageAndBounds(test_bitmap, gfx::RectF(0, 0, 1, 1));
    const gfx::Size output_size(test_bitmap.width() * 4, test_bitmap.height() * 4);
    VideoCaptureOverlay::OnceRenderer renderer =
      overlay.MakeRenderer(VideoCaptureOverlay::CapturedFrameProperties{
          .region_properties =
              CapturableFrameSink::RegionProperties{
...
TEST_P(VideoCaptureOverlayRenderTest, MovesAround)
{
    ...
    const SkBitmap test_bitmap = MakeTestBitmap(0);
    const gfx::Size video_frame_size(test_bitmap.width() * 4, test_bitmap.height() * 4);
    ...
    const gfx::RectF relative_image_bounds[6] = {
        ...
        gfx::RectF(2.0f / video_frame_size.width(), 1.0f / video_frame_size.height(), 0.5f, 0.5f),
        gfx::RectF(2.0f / video_frame_size.width(), 2.0f / video_frame_size.height(), 0.5f, 0.5f),
    ...
    constexpr std::array<const char*, 6> kGoldenFiles = {
        "overlay_moves_0_0.png",
        "overlay_moves_1_0.png",
        "overlay_moves_2_0.png",
        "overlay_moves_2_1.png",
        "overlay_moves_2_2.png",
        "overlay_moves_lr.png",
    };

    ExpectRendersAs(renderers, kGoldenFiles.data(), kGoldenFiles.size(), video_frame_size);
}
...
TEST_P(VideoCaptureOverlayRenderTest, ClipsToContentBounds)
{
    ...
    const SkBitmap test_bitmap = MakeTestBitmap(0);
    const gfx::Size video_frame_size(test_bitmap.width() * 4, test_bitmap.height() * 4);
    const gfx::Rect region_in_frame(test_bitmap.width(), test_bitmap.height(), test_bitmap.width() * 2, test_bitmap.height() * 2);
...
TEST_P(VideoCaptureOverlayRenderTest, ClipsToSubregionBounds)
{
    ...
    const SkBitmap test_bitmap = MakeTestBitmap(0);
    const gfx::Size frame_size(test_bitmap.width() * 4, test_bitmap.height() * 4);
    const gfx::Rect compositor_frame_subrect(test_bitmap.width(), test_bitmap.height(), test_bitmap.width() * 2, test_bitmap.height() * 2);
    const gfx::RectF relative_image_bounds[4] = {
    ...
    ExpectRendersAs(renderers, kGoldenFiles.data(), kGoldenFiles.size(), compositor_frame_subrect.size());
}
...
TEST_P(VideoCaptureOverlayRenderTest, ScalesToContentRegion)
{
    ...
    const gfx::Size video_frame_size(test_bitmap.width() * 4, test_bitmap.height() * 4);
    const gfx::Rect compositor_frame_subrect(test_bitmap.width(), test_bitmap.height(), test_bitmap.width() * 2, test_bitmap.height() * 2);
    const gfx::Rect content_rect(test_bitmap.width() * 2, test_bitmap.height() * 2, test_bitmap.width() * 6, test_bitmap.height() * 6);
    ...
    ExpectRendersAs(renderers, kGoldenFiles.data(), kGoldenFiles.size(), gfx::Size(content_rect.right(), content_rect.bottom()));
}

INSTANTIATE_TEST_SUITE_P(
    All, VideoCaptureOverlayRenderTest, testing::Values(VideoCaptureOverlayRenderTest::kARGBFormat, VideoCaptureOverlayRenderTest::kI420Format));
