...
public:
    ...
    GpuMemoryBufferVideoFramePool(const GpuMemoryBufferVideoFramePool&& other) = delete;
    GpuMemoryBufferVideoFramePool& operator=(const GpuMemoryBufferVideoFramePool& other) = delete;

    // VideoFramePool implementation:
    scoped_refptr<media::VideoFrame> ReserveVideoFrame(media::VideoPixelFormat format, const gfx::Size& size) override;

    media::mojom::VideoBufferHandlePtr CloneHandleForDelivery(const media::VideoFrame& frame) override;
...
private:
    ...
    raw_ptr<GmbVideoFramePoolContextProvider> context_provider_ GUARDED_BY_CONTEXT(sequence_checker_);

    std::unique_ptr<media::RenderableGpuMemoryBufferVideoFramePool> video_frame_pool_ GUARDED_BY_CONTEXT(sequence_checker_);
    ...
    uint32_t video_frame_pool_generation_ GUARDED_BY_CONTEXT(sequence_checker_) = 0;
