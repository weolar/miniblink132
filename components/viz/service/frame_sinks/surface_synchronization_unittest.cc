...
CompositorFrame MakeCompositorFrame(std::vector<SurfaceId> activation_dependencies, std::vector<SurfaceRange> referenced_surfaces,
    std::vector<TransferableResource> resource_list, const FrameDeadline& deadline = FrameDeadline(), bool is_handling_interaction = true)
{
...
public:
    ...
    SurfaceSynchronizationTest& operator=(const SurfaceSynchronizationTest&) = delete;
    ...
    {
        supports_[frame_sink_id] = std::make_unique<CompositorFrameSinkSupport>(&support_client_, frame_sink_manager_.get(), frame_sink_id, is_root);
    }
    ...
    const base::flat_set<SurfaceId>& GetReferencesFrom(const SurfaceId& surface_id)
    {
        return frame_sink_manager().surface_manager()->GetSurfacesReferencedByParent(surface_id);
    }
    ...
    // Returns all the references where |surface_id| is the parent.
    const base::flat_set<SurfaceId>& GetChildReferences(const SurfaceId& surface_id)
    {
        return frame_sink_manager().surface_manager()->GetSurfacesReferencedByParent(surface_id);
    }
    ...
    {
        return frame_sink_manager().surface_manager()->HasTemporaryReference(surface_id);
    }

    Surface* GetLatestInFlightSurface(const SurfaceRange& surface_range)
    {
        return frame_sink_manager().surface_manager()->GetLatestInFlightSurface(surface_range);
    }
    ...
    {
        return FrameDeadline(Now(), deadline_in_frames, BeginFrameArgs::DefaultInterval(), false);
    }
    ...
    void SendNextBeginFrame()
    {
        ...
        BeginFrameArgs args = begin_frame_source_->CreateBeginFrameArgs(BEGINFRAME_FROM_HERE, now_src_.get());
    ...
    FakeSurfaceObserver& surface_observer()
    {
        return *surface_observer_;
    }
    ...
    {
        testing::Test::SetUp();
        frame_sink_manager_ = std::make_unique<FrameSinkManagerImpl>(FrameSinkManagerImpl::InitParams(&shared_bitmap_manager_));
        surface_observer_ = std::make_unique<FakeSurfaceObserver>(surface_manager(), false);
        begin_frame_source_ = std::make_unique<FakeExternalBeginFrameSource>(0.f, false);
        now_src_ = std::make_unique<base::SimpleTestTickClock>();
        surface_manager()->SetTickClockForTesting(now_src_.get());

        supports_[kDisplayFrameSink] = std::make_unique<CompositorFrameSinkSupport>(&support_client_, frame_sink_manager_.get(), kDisplayFrameSink, kIsRoot);

        supports_[kParentFrameSink] = std::make_unique<CompositorFrameSinkSupport>(&support_client_, frame_sink_manager_.get(), kParentFrameSink, kIsChildRoot);

        supports_[kChildFrameSink1] = std::make_unique<CompositorFrameSinkSupport>(&support_client_, frame_sink_manager_.get(), kChildFrameSink1, kIsChildRoot);

        supports_[kChildFrameSink2] = std::make_unique<CompositorFrameSinkSupport>(&support_client_, frame_sink_manager_.get(), kChildFrameSink2, kIsChildRoot);
        ...
        frame_sink_manager().RegisterBeginFrameSource(begin_frame_source_.get(), kDisplayFrameSink);
        frame_sink_manager().RegisterFrameSinkHierarchy(kDisplayFrameSink, kParentFrameSink);
        frame_sink_manager().RegisterFrameSinkHierarchy(kDisplayFrameSink, kChildFrameSink1);
        frame_sink_manager().RegisterFrameSinkHierarchy(kDisplayFrameSink, kChildFrameSink2);
    }
    ...
    SurfaceId MakeSurfaceId(const FrameSinkId& frame_sink_id, uint32_t parent_sequence_number, uint32_t child_sequence_number = 1u)
    {
        return allocator_set_.MakeSurfaceId(frame_sink_id, parent_sequence_number, child_sequence_number);
    }
...
private:
    ...
    std::unordered_map<FrameSinkId, std::unique_ptr<CompositorFrameSinkSupport>, FrameSinkIdHash> supports_;
...
TEST_F(SurfaceSynchronizationTest, RootSurfaceReceivesReferences)
{
    ...
    display_support().SubmitCompositorFrame(display_id_first.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
    ...
    EXPECT_THAT(GetChildReferences(frame_sink_manager().surface_manager()->GetRootSurfaceId()), UnorderedElementsAre(display_id_first));

    // Submit a CompositorFrame for the second display root surface.
    display_support().SubmitCompositorFrame(display_id_second.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
    ...
    EXPECT_THAT(GetChildReferences(frame_sink_manager().surface_manager()->GetRootSurfaceId()), UnorderedElementsAre(display_id_second));
...
TEST_F(SurfaceSynchronizationTest, BlockedOnTwo)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id1, child_id2 }, empty_surface_ranges(), std::vector<TransferableResource>()));

    // parent_support is blocked on |child_id1| and |child_id2|.
    EXPECT_TRUE(parent_surface()->has_deadline());
    EXPECT_FALSE(parent_surface()->HasActiveFrame());
    EXPECT_TRUE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), UnorderedElementsAre(child_id1, child_id2));

    // Submit a CompositorFrame without any dependencies to |child_id1|.
    // parent_support should now only be blocked on |child_id2|.
    child_support1().SubmitCompositorFrame(child_id1.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));

    EXPECT_TRUE(parent_surface()->has_deadline());
    EXPECT_FALSE(parent_surface()->HasActiveFrame());
    EXPECT_TRUE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), UnorderedElementsAre(child_id2));

    // Submit a CompositorFrame without any dependencies to |child_id2|.
    // parent_support should be activated.
    child_support2().SubmitCompositorFrame(child_id2.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
...
TEST_F(SurfaceSynchronizationTest, BlockedChain)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id1 }, empty_surface_ranges(), std::vector<TransferableResource>()));

    // parent_support is blocked on |child_id1|.
    EXPECT_TRUE(parent_surface()->has_deadline());
    EXPECT_FALSE(parent_surface()->HasActiveFrame());
    EXPECT_TRUE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), UnorderedElementsAre(child_id1));
    // The parent should not report damage until it activates.
    EXPECT_FALSE(surface_observer().IsSurfaceDamaged(parent_id));

    child_support1().SubmitCompositorFrame(
        child_id1.local_surface_id(), MakeCompositorFrame({ child_id2 }, empty_surface_ranges(), std::vector<TransferableResource>()));
    ...
    EXPECT_THAT(child_surface1()->activation_dependencies(), UnorderedElementsAre(child_id2));
    ...
    child_support2().SubmitCompositorFrame(
        child_id2.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>()));
...
TEST_F(SurfaceSynchronizationTest, TwoBlockedOnOne)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id2 }, { SurfaceRange(std::nullopt, child_id2) }, std::vector<TransferableResource>()));
    ...
    // child_support1 should now be blocked on |child_id2|.
    child_support1().SubmitCompositorFrame(
        child_id1.local_surface_id(), MakeCompositorFrame({ child_id2 }, { SurfaceRange(std::nullopt, child_id2) }, std::vector<TransferableResource>()));
...
TEST_F(SurfaceSynchronizationTest, DeadlineHits)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id1 }, empty_surface_ranges(), std::vector<TransferableResource>(), MakeDefaultDeadline()));
    ...
    child_support1().SubmitCompositorFrame(
        child_id1.local_surface_id(), MakeCompositorFrame({ child_id2 }, empty_surface_ranges(), std::vector<TransferableResource>(), MakeDefaultDeadline()));
...
TEST_F(SurfaceSynchronizationTest, UnlimitedDeadline)
{
    ...
    frame_sink_manager().surface_manager()->SetActivationDeadlineInFramesForTesting(std::nullopt);
...
TEST_F(SurfaceSynchronizationTest, NewFrameOverridesOldDependencies)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ arbitrary_id }, empty_surface_ranges(), std::vector<TransferableResource>()));

    // Verify that the CompositorFrame is blocked on |arbitrary_id|.
    EXPECT_FALSE(parent_surface()->HasActiveFrame());
    EXPECT_TRUE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), UnorderedElementsAre(arbitrary_id));

    // Submit a CompositorFrame that has no dependencies.
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
...
class OnBeginFrameAcksSurfaceSynchronizationTest : public SurfaceSynchronizationTest, public testing::WithParamInterface<std::tuple<bool, bool>> {
public:
    OnBeginFrameAcksSurfaceSynchronizationTest();
    ~OnBeginFrameAcksSurfaceSynchronizationTest() override = default;

    bool BeginFrameAcksEnabled() const
    {
        return std::get<0>(GetParam());
    }
    bool AutoNeedsBeginFrame() const
    {
...
OnBeginFrameAcksSurfaceSynchronizationTest::OnBeginFrameAcksSurfaceSynchronizationTest()
{
...
TEST_P(OnBeginFrameAcksSurfaceSynchronizationTest, OnlyActiveFramesAffectSurfaceReferences)
{
    const SurfaceId parent_id = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId child_id1 = MakeSurfaceId(kChildFrameSink1, 1);
    const SurfaceId child_id2 = MakeSurfaceId(kChildFrameSink2, 1);
    ...
    EXPECT_CALL(support_client_, DidReceiveCompositorFrameAck(_)).Times(BeginFrameAcksEnabled() ? 0 : 1);
    child_support1().SubmitCompositorFrame(child_id1.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id2 }, { SurfaceRange(child_id1) }, std::vector<TransferableResource>()));
    EXPECT_FALSE(parent_surface()->HasActiveFrame());
    EXPECT_TRUE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), UnorderedElementsAre(child_id2));
    ...
    // on activation.
    EXPECT_CALL(support_client_, DidReceiveCompositorFrameAck(_)).Times(BeginFrameAcksEnabled() ? 0 : 2);
    child_support2().SubmitCompositorFrame(child_id2.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
    testing::Mock::VerifyAndClearExpectations(&support_client_);
...
TEST_P(OnBeginFrameAcksSurfaceSynchronizationTest, ResourcesOnlyReturnedOnce)
{
    ...
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), MakeCompositorFrame({ child_id }, empty_surface_ranges(), resource_list));

    // Verify that the CompositorFrame is blocked on |child_id|.
    EXPECT_FALSE(parent_surface()->HasActiveFrame());
    EXPECT_TRUE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), UnorderedElementsAre(child_id));

    child_support1().SubmitCompositorFrame(
        child_id.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>()));
    ...
    if (BeginFrameAcksEnabled()) {
        ...
        EXPECT_CALL(support_client_, OnBeginFrame(_, _, _, _))
            .WillRepeatedly([=](const BeginFrameArgs& args, const FrameTimingDetailsMap& timing_details, bool frame_ack, std::vector<ReturnedResource> got) {
        ...
        EXPECT_CALL(support_client_, DidReceiveCompositorFrameAck(_)).WillOnce([=](std::vector<ReturnedResource> got) {
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ empty_surface_ids() }, { empty_surface_ranges() }, std::vector<TransferableResource>()));
    EXPECT_TRUE(parent_surface()->HasActiveFrame());
    EXPECT_FALSE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), IsEmpty());
    ...
    if (BeginFrameAcksEnabled()) {
        ...
        ResourceId id = resource.ToReturnedResource().id;
        EXPECT_CALL(support_client_, OnBeginFrame(_, _, _, _))
            .WillOnce([=](const BeginFrameArgs& args, const FrameTimingDetailsMap& timing_details, bool frame_ack, std::vector<ReturnedResource> got) {
                EXPECT_EQ(1u, got.size());
                EXPECT_EQ(id, got[0].id);
            });
        SendNextBeginFrame();
...
// referenced.
TEST_P(OnBeginFrameAcksSurfaceSynchronizationTest, DropStaleReferencesAfterActivation)
{
    const SurfaceId parent_id = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId child_id1 = MakeSurfaceId(kChildFrameSink1, 1);
    const SurfaceId child_id2 = MakeSurfaceId(kChildFrameSink2, 1);
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), { SurfaceRange(child_id1) }, std::vector<TransferableResource>()));
    ...
    EXPECT_CALL(support_client_, DidReceiveCompositorFrameAck(_)).Times(0);
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id2 }, empty_surface_ranges(), std::vector<TransferableResource>()));
    testing::Mock::VerifyAndClearExpectations(&support_client_);
...
{
    const SurfaceId parent_id = MakeSurfaceId(kParentFrameSink, 1);
    const ui::LatencyComponentType latency_type1 = ui::INPUT_EVENT_LATENCY_ORIGINAL_COMPONENT;
    const ui::LatencyComponentType latency_type2 = ui::LATENCY_COMPONENT_TYPE_LAST;
    ...
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), std::move(frame));
    ...
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), std::move(frame2));
...
// frames.
TEST_F(SurfaceSynchronizationTest, LatencyInfoAggregation_NoUnresolvedDependencies)
{
    const SurfaceId parent_id1 = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId parent_id2 = MakeSurfaceId(kParentFrameSink, 2);
    const ui::LatencyComponentType latency_type1 = ui::INPUT_EVENT_LATENCY_RENDERER_SWAP_COMPONENT;
    const ui::LatencyComponentType latency_type2 = ui::LATENCY_COMPONENT_TYPE_LAST;
    ...
    CompositorFrame frame = CompositorFrameBuilder().AddDefaultRenderPass().AddLatencyInfo(info).SetIsHandlingInteraction(true).Build();
    ...
    CompositorFrame frame2 = CompositorFrameBuilder().AddDefaultRenderPass().AddLatencyInfo(info2).SetIsHandlingInteraction(true).Build();

    parent_support().SubmitCompositorFrame(parent_id2.local_surface_id(), std::move(frame2));
    ...
    std::vector<ui::LatencyInfo> info_list;
    surface->allocation_group()->TakeAggregatedLatencyInfoUpTo(surface, &info_list);
    ...
    EXPECT_TRUE(aggregated_latency_info.FindLatency(ui::DISPLAY_COMPOSITOR_RECEIVED_FRAME_COMPONENT, nullptr));
}
...
TEST_F(SurfaceSynchronizationTest, LatencyInfoAggregation_OldSurfaceHasPendingAndActiveFrame)
{
    const SurfaceId parent_id1 = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId parent_id2 = MakeSurfaceId(kParentFrameSink, 2);
    const SurfaceId child_id = MakeSurfaceId(kChildFrameSink1, 1);
    ...
    CompositorFrame frame = MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId);
    ...
    CompositorFrame frame2 = MakeCompositorFrame({ child_id }, empty_surface_ranges(), std::vector<TransferableResource>());
    frame2.metadata.latency_info.push_back(info2);

    parent_support().SubmitCompositorFrame(parent_id1.local_surface_id(), std::move(frame2));
    ...
    parent_support().SubmitCompositorFrame(parent_id2.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
...
TEST_F(SurfaceSynchronizationTest, LatencyInfoAggregation_NewSurfaceHasPendingFrame)
{
    const SurfaceId parent_id1 = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId parent_id2 = MakeSurfaceId(kParentFrameSink, 2);
    const SurfaceId child_id = MakeSurfaceId(kChildFrameSink1, 1);
    ...
    child_support1().SubmitCompositorFrame(child_id.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
    EXPECT_FALSE(surface->HasPendingFrame());
    EXPECT_TRUE(surface->HasActiveFrame());
...
// surface.
TEST_F(SurfaceSynchronizationTest, LatencyInfoAggregation_MultipleOldSurfacesWithPendingFrames)
{
    const SurfaceId parent_id1 = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId parent_id2 = MakeSurfaceId(kParentFrameSink, 2);
    ...
    CompositorFrame frame1 = MakeCompositorFrame({ child_id1 }, empty_surface_ranges(), std::vector<TransferableResource>());
    frame1.metadata.latency_info.push_back(info1);

    parent_support().SubmitCompositorFrame(parent_id1.local_surface_id(), std::move(frame1));

    // Submit a frame with unresolved dependencies to parent_id2.
    ui::LatencyInfo info2;
    info2.AddLatencyNumber(latency_type2);

    CompositorFrame frame2 = MakeCompositorFrame({ child_id2 }, empty_surface_ranges(), std::vector<TransferableResource>());
    frame2.metadata.latency_info.push_back(info2);
    ...
    parent_support().SubmitCompositorFrame(parent_id3.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
...
TEST_F(SurfaceSynchronizationTest, LatencyInfoNotCarriedOver)
{
    ...
    EXPECT_TRUE(info_list[0].FindLatency(ui::DISPLAY_COMPOSITOR_RECEIVED_FRAME_COMPONENT, nullptr));
...
TEST_P(OnBeginFrameAcksSurfaceSynchronizationTest, ReturnResourcesWithAck)
{
    ...
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), { resource }));
    ResourceId id = resource.ToReturnedResource().id;
...
TEST_P(OnBeginFrameAcksSurfaceSynchronizationTest, SubmitToDestroyedSurface)
{
    ...
    TransferableResource resource;
    resource.id = ResourceId(1234);
    child_support1().SubmitCompositorFrame(child_id.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), { resource }));
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id }, { SurfaceRange(child_id) }, std::vector<TransferableResource>()));
    ...
    {
        ResourceId id = resource.ToReturnedResource().id;
        EXPECT_CALL(support_client_, ReclaimResources(_)).WillOnce([=](std::vector<ReturnedResource> got) {
            EXPECT_EQ(1u, got.size());
            EXPECT_EQ(id, got[0].id);
        });
        frame_sink_manager().surface_manager()->GarbageCollectSurfaces();
...
TEST_F(SurfaceSynchronizationTest, DependencyTrackingGarbageCollection)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id1.local_surface_id(), MakeCompositorFrame({ child_id }, empty_surface_ranges(), std::vector<TransferableResource>(), MakeDefaultDeadline()));
    display_support().SubmitCompositorFrame(
        display_id.local_surface_id(), MakeCompositorFrame({ parent_id1 }, empty_surface_ranges(), std::vector<TransferableResource>(), MakeDefaultDeadline()));
    ...
    parent_support().SubmitCompositorFrame(
        parent_id2.local_surface_id(), MakeCompositorFrame({ child_id }, empty_surface_ranges(), std::vector<TransferableResource>(), MakeDefaultDeadline()));
    display_support().SubmitCompositorFrame(
        display_id.local_surface_id(), MakeCompositorFrame({ parent_id2 }, empty_surface_ranges(), std::vector<TransferableResource>(), MakeDefaultDeadline()));
...
TEST_F(SurfaceSynchronizationTest, GarbageCollectionOnDeadline)
{
    ...
    display_support().SubmitCompositorFrame(display_id.local_surface_id(),
        MakeCompositorFrame({ parent_id1 }, { SurfaceRange(parent_id1) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
...
TEST_F(SurfaceSynchronizationTest, OnlyBlockOnEmbeddedSurfaces)
{
    ...
    parent_support().SubmitCompositorFrame(parent_id1.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));

    display_support().SubmitCompositorFrame(
        display_id.local_surface_id(), MakeCompositorFrame({ parent_id2 }, { SurfaceRange(parent_id1) }, std::vector<TransferableResource>()));
    ...
    // but not |parent_id1|.
    EXPECT_THAT(display_surface()->activation_dependencies(), UnorderedElementsAre(parent_id2));
...
TEST_F(SurfaceSynchronizationTest, LateArrivingDependency)
{
    ...
    display_support().SubmitCompositorFrame(display_id.local_surface_id(),
        MakeCompositorFrame({ parent_id1 }, { SurfaceRange(std::nullopt, parent_id1) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    ...
    parent_support().SubmitCompositorFrame(parent_id1.local_surface_id(),
        MakeCompositorFrame({ child_id1 }, { SurfaceRange(std::nullopt, child_id1) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    EXPECT_FALSE(parent_surface()->has_deadline());
    EXPECT_FALSE(parent_surface()->HasPendingFrame());
    EXPECT_TRUE(parent_surface()->HasActiveFrame());
}
...
TEST_F(SurfaceSynchronizationTest, MultiLevelLateArrivingDependency)
{
    ...
    display_support().SubmitCompositorFrame(display_id.local_surface_id(),
        MakeCompositorFrame({ parent_id }, { SurfaceRange(std::nullopt, parent_id) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    EXPECT_TRUE(display_surface()->HasPendingFrame());
    EXPECT_FALSE(display_surface()->HasActiveFrame());
    EXPECT_TRUE(display_surface()->has_deadline());
    ...
    child_support1().SubmitCompositorFrame(child_id.local_surface_id(),
        MakeCompositorFrame({ arbitrary_id }, { SurfaceRange(std::nullopt, arbitrary_id) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    EXPECT_TRUE(child_surface1()->HasPendingFrame());
    EXPECT_FALSE(child_surface1()->HasActiveFrame());
    EXPECT_TRUE(child_surface1()->has_deadline());
    ...
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(),
        MakeCompositorFrame({ child_id }, { SurfaceRange(std::nullopt, child_id) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    EXPECT_FALSE(parent_surface()->HasPendingFrame());
    EXPECT_TRUE(parent_surface()->HasActiveFrame());
    EXPECT_FALSE(parent_surface()->has_deadline());
...
TEST_P(OnBeginFrameAcksSurfaceSynchronizationTest, FallbackSurfacesClosed)
{
    ...
    EXPECT_CALL(support_client_, DidReceiveCompositorFrameAck(_)).Times(0);
    child_support1().SubmitCompositorFrame(
        child_id1.local_surface_id(), MakeCompositorFrame({ arbitrary_id }, { SurfaceRange(std::nullopt, arbitrary_id) }, {}, MakeDefaultDeadline()));
    EXPECT_TRUE(child_surface1()->has_deadline());
    EXPECT_TRUE(child_surface1()->HasPendingFrame());
    EXPECT_FALSE(child_surface1()->HasActiveFrame());
    testing::Mock::VerifyAndClearExpectations(&support_client_);
    ...
    EXPECT_CALL(support_client_, DidReceiveCompositorFrameAck(_)).Times(BeginFrameAcksEnabled() ? 0 : 1);
    parent_support().SubmitCompositorFrame(parent_id1.local_surface_id(),
        MakeCompositorFrame({ child_id2 }, { SurfaceRange(child_id1, child_id2) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    EXPECT_TRUE(parent_surface()->has_deadline());
    EXPECT_TRUE(parent_surface()->HasPendingFrame());
    EXPECT_FALSE(parent_surface()->HasActiveFrame());
    EXPECT_FALSE(child_surface1()->HasPendingFrame());
    ...
    SendNextBeginFrame();
    EXPECT_CALL(support_client_, DidReceiveCompositorFrameAck(_)).Times((AutoNeedsBeginFrame() && BeginFrameAcksEnabled()) ? 0 : 1);
    child_support1().SubmitCompositorFrame(
        child_id1.local_surface_id(), MakeCompositorFrame({ arbitrary_id }, { SurfaceRange(std::nullopt, arbitrary_id) }, {}, MakeDefaultDeadline()));
    EXPECT_FALSE(child_surface1()->HasPendingFrame());
    EXPECT_TRUE(child_surface1()->HasActiveFrame());
...
TEST_F(SurfaceSynchronizationTest, IndependentDeadlines)
{
    ...
    parent_support().SubmitCompositorFrame(parent_id1.local_surface_id(),
        MakeCompositorFrame({ child_id1, child_id2 }, empty_surface_ranges(), std::vector<TransferableResource>(), MakeDefaultDeadline()));
    ...
    child_support1().SubmitCompositorFrame(child_id1.local_surface_id(),
        MakeCompositorFrame(
            { arbitrary_id }, empty_surface_ranges(), std::vector<TransferableResource>(), FrameDeadline(Now(), 3, BeginFrameArgs::DefaultInterval(), false)));
    EXPECT_TRUE(child_surface1()->HasPendingFrame());
    EXPECT_TRUE(child_surface1()->HasActiveFrame());
    EXPECT_TRUE(child_surface1()->has_deadline());
    ...
    child_support2().SubmitCompositorFrame(child_id2.local_surface_id(),
        MakeCompositorFrame({ arbitrary_id }, empty_surface_ranges(), std::vector<TransferableResource>(), MakeDefaultDeadline()));
...
TEST_F(SurfaceSynchronizationTest, InheritShorterDeadline)
{
    ...
    parent_support().SubmitCompositorFrame(parent_id1.local_surface_id(),
        MakeCompositorFrame({ child_id1 }, { SurfaceRange(std::nullopt, child_id1) }, std::vector<TransferableResource>(),
...
TEST_F(SurfaceSynchronizationTest, ChildDeadlineNotExtendedByInheritance)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id1 }, empty_surface_ranges(), std::vector<TransferableResource>(), MakeDeadline(10)));

    // Child1 blocks on Child2 with a deadline of 2.
    child_support1().SubmitCompositorFrame(child_id1.local_surface_id(),
        MakeCompositorFrame({ child_id2 }, { SurfaceRange(std::nullopt, child_id2) }, std::vector<TransferableResource>(), MakeDeadline(2)));
...
TEST_F(SurfaceSynchronizationTest, FrameActivationAfterFrameSinkDestruction)
{
    ...
    display_support().SubmitCompositorFrame(
        display_id.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), { SurfaceRange(parent_id) }, std::vector<TransferableResource>()));
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id }, empty_surface_ranges(), std::vector<TransferableResource>()));
    ...
    display_support().SubmitCompositorFrame(display_id.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
...
TEST_F(SurfaceSynchronizationTest, PreviousFrameSurfaceId)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id1.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>()));
    ...
    // |parent_id1|.
    parent_support().SubmitCompositorFrame(
        parent_id2.local_surface_id(), MakeCompositorFrame({ child_id }, empty_surface_ranges(), std::vector<TransferableResource>()));
    Surface* parent_surface2 = frame_sink_manager().surface_manager()->GetSurfaceForId(parent_id2);
...
TEST_F(SurfaceSynchronizationTest, FrameIndexWithPendingFrames)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>()));
    Surface* parent_surface = frame_sink_manager().surface_manager()->GetSurfaceForId(parent_id);
...
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurface)
{
    ...
    EXPECT_TRUE(HasTemporaryReference(child_id1));
    EXPECT_THAT(GetChildReferences(parent_id), IsEmpty());
    EXPECT_EQ(GetSurfaceForId(child_id1), GetLatestInFlightSurface(SurfaceRange(child_id1, child_id2)));
    ...
    child_support1().SubmitCompositorFrame(child_id2.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
    ...
    EXPECT_EQ(GetSurfaceForId(child_id2), GetLatestInFlightSurface(SurfaceRange(child_id1, child_id3)));

    // If the primary surface is old, then we shouldn't return an in-flight
    // surface that is newer than the primary.
    EXPECT_EQ(GetSurfaceForId(child_id1), GetLatestInFlightSurface(SurfaceRange(child_id1)));

    // Submit a child CompositorFrame to a new SurfaceId and verify that
    // GetLatestInFlightSurface returns the right surface.
    child_support1().SubmitCompositorFrame(child_id3.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));

    // Verify that there is a temporary reference for child_id3.
    EXPECT_TRUE(HasTemporaryReference(child_id3));

    EXPECT_EQ(GetSurfaceForId(child_id3), GetLatestInFlightSurface(SurfaceRange(child_id1, child_id4)));

    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id3 }, { SurfaceRange(std::nullopt, child_id3) }, std::vector<TransferableResource>()));

    EXPECT_THAT(GetChildReferences(parent_id), UnorderedElementsAre(child_id3));

    // If the primary surface is active, we return it.
    EXPECT_EQ(GetSurfaceForId(child_id3), GetLatestInFlightSurface(SurfaceRange(child_id1, child_id3)));
}
...
// exists.
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurfaceWithInvalidSurfaceRange)
{
    const SurfaceId parent_id = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId child_id1 = MakeSurfaceId(kChildFrameSink1, 1);
    const SurfaceId child_id2 = MakeSurfaceId(kChildFrameSink1, 2);
    ...
    EXPECT_EQ(nullptr, GetLatestInFlightSurface(SurfaceRange(bogus_child_id, child_id1)));

    // In this case, the end doesn't exist either. Still return nullptr.
    EXPECT_EQ(nullptr, GetLatestInFlightSurface(SurfaceRange(bogus_child_id, child_id2)));
}
...
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurfaceWithoutFallback)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id2 }, { SurfaceRange(child_id1, child_id2) }, std::vector<TransferableResource>()));
    ...
    EXPECT_EQ(GetSurfaceForId(child_id1), GetLatestInFlightSurface(SurfaceRange(std::nullopt, child_id2)));
    ...
    EXPECT_EQ(GetSurfaceForId(child_id2), GetLatestInFlightSurface(SurfaceRange(child_id1, child_id2)));

    // Fallback is not specified but primary exists so we return it.
    EXPECT_EQ(GetSurfaceForId(child_id2), GetLatestInFlightSurface(SurfaceRange(std::nullopt, child_id2)));
}
...
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurfaceWithGarbageFallback)
{
    ...
    EXPECT_FALSE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), IsEmpty());
    EXPECT_THAT(parent_surface()->active_referenced_surfaces(), UnorderedElementsAre(child_id1));
    EXPECT_FALSE(HasTemporaryReference(child_id1));
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), { SurfaceRange(child_id2) }, std::vector<TransferableResource>()));
    ...
    EXPECT_FALSE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), IsEmpty());
    EXPECT_THAT(parent_surface()->active_referenced_surfaces(), UnorderedElementsAre(child_id2));
    ...
    EXPECT_EQ(frame_sink_manager().surface_manager()->GetSurfaceForId(child_id1), nullptr);
...
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurfaceDifferentFrameSinkIds)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id4 }, { SurfaceRange(child_id1, child_id4) }, std::vector<TransferableResource>()));

    // Primary's frame sink id empty and |child_id2| is the latest in fallback's
    // frame sink.
    EXPECT_EQ(GetSurfaceForId(child_id2), GetLatestInFlightSurface(SurfaceRange(child_id1, child_id4)));

    // Activate |child_id3| which is in different frame sink.
    child_support2().SubmitCompositorFrame(child_id3.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
...
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurfaceUsesPersistentReferences)
{
    const SurfaceId parent_id = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId child_id1 = MakeSurfaceId(kChildFrameSink1, 1);
    const SurfaceId child_id2 = MakeSurfaceId(kChildFrameSink1, 2);
    ...
    EXPECT_TRUE(HasTemporaryReference(child_id1));
    EXPECT_TRUE(surface_manager()->GetSurfacesThatReferenceChildForTesting(child_id1).empty());
    ...
    EXPECT_TRUE(HasTemporaryReference(child_id2));
    EXPECT_TRUE(surface_manager()->GetSurfacesThatReferenceChildForTesting(child_id2).empty());
    ...
    EXPECT_FALSE(HasTemporaryReference(child_id2));
    EXPECT_FALSE(surface_manager()->GetSurfacesThatReferenceChildForTesting(child_id2).empty());
...
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurfaceSkipDifferentNonce)
{
    ...
    const SurfaceId child_id1 = SurfaceId(kChildFrameSink1, LocalSurfaceId(1, nonce1));
    const SurfaceId child_id2 = SurfaceId(kChildFrameSink1, LocalSurfaceId(2, nonce1));
    const SurfaceId child_id3 = SurfaceId(kChildFrameSink1, LocalSurfaceId(3, nonce2));
    const SurfaceId child_id4 = SurfaceId(kChildFrameSink1, LocalSurfaceId(4, nonce2));
    const SurfaceId child_id5 = SurfaceId(kChildFrameSink1, LocalSurfaceId(5, nonce3));
    ...
    EXPECT_EQ(GetSurfaceForId(child_id2), GetLatestInFlightSurface(SurfaceRange(child_id1, child_id5)));
}
...
TEST_F(SurfaceSynchronizationTest, DropDependenciesThatWillNeverArrive1)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id11, child_id21 }, empty_surface_ranges(), std::vector<TransferableResource>()));
    EXPECT_FALSE(parent_surface()->HasActiveFrame());
    EXPECT_TRUE(parent_surface()->HasPendingFrame());
    ...
    child_support1().SubmitCompositorFrame(
        child_id12.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>()));
    EXPECT_FALSE(parent_surface()->HasActiveFrame());
    EXPECT_TRUE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), UnorderedElementsAre(child_id21));

    // Finally, the second child submits a frame to the remaining dependency and
    // the parent activates.
    child_support2().SubmitCompositorFrame(
        child_id21.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>()));
    EXPECT_TRUE(parent_surface()->HasActiveFrame());
    EXPECT_FALSE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), IsEmpty());
}
...
TEST_F(SurfaceSynchronizationTest, ObserveDependenciesUntilArrival)
{
    ...
    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id22 }, empty_surface_ranges(), std::vector<TransferableResource>()));
    EXPECT_FALSE(parent_surface()->HasActiveFrame());
    EXPECT_TRUE(parent_surface()->HasPendingFrame());
    ...
    EXPECT_FALSE(parent_surface()->HasActiveFrame());
    EXPECT_TRUE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), UnorderedElementsAre(child_id22));

    // The child submits to |child_id22|. The parent should activate.
    child_support1().SubmitCompositorFrame(
        child_id22.local_surface_id(), MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>()));
    EXPECT_TRUE(parent_surface()->HasActiveFrame());
    EXPECT_FALSE(parent_surface()->HasPendingFrame());
    EXPECT_THAT(parent_surface()->activation_dependencies(), IsEmpty());
}
...
TEST_F(SurfaceSynchronizationTest, MarkPreviousSurfaceForDestructionAfterActivation)
{
    const SurfaceId parent_id1 = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId parent_id2 = MakeSurfaceId(kParentFrameSink, 2);
    const SurfaceId arbitrary_id = MakeSurfaceId(kArbitraryFrameSink, 1);
    ...
    EXPECT_FALSE(parent_surface2->HasActiveFrame());
    EXPECT_TRUE(parent_surface2->HasPendingFrame());
    EXPECT_THAT(parent_surface2->activation_dependencies(), UnorderedElementsAre(arbitrary_id));
    EXPECT_FALSE(IsMarkedForDestruction(parent_id1));
...
TEST_F(SurfaceSynchronizationTest, SetPreviousFrameSurfaceDoesntCrash)
{
    ...
    EXPECT_TRUE(parent_support().last_activated_surface_id().is_valid());
    parent_support().EvictSurface(parent_support().last_activated_surface_id().local_surface_id());
    EXPECT_FALSE(parent_support().last_activated_surface_id().is_valid());
...
TEST_F(SurfaceSynchronizationTest, SurfaceReferenceTracking_PrimaryEmbedTokenDoesntMatch)
{
    const SurfaceId parent_id = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId child_id1 = MakeSurfaceId(kChildFrameSink1, 1);
    const SurfaceId child_id2(kChildFrameSink2, LocalSurfaceId(2, base::UnguessableToken::Create()));
    const SurfaceId child_id3 = MakeSurfaceId(kChildFrameSink2, 5);

    // The parent embeds (child_id1, child_id3).
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(),
        MakeCompositorFrame(empty_surface_ids(), { SurfaceRange(child_id1, child_id3) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
...
TEST_F(SurfaceSynchronizationTest, SurfaceReferenceTracking_NewerSurfaceUpdatesReferences)
{
    const SurfaceId parent_id = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId child_id1 = MakeSurfaceId(kChildFrameSink1, 1);
    const SurfaceId child_id2 = MakeSurfaceId(kChildFrameSink1, 2);
    ...
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(),
        MakeCompositorFrame(empty_surface_ids(), { SurfaceRange(child_id1, child_id4) }, std::vector<TransferableResource>(), MakeDefaultDeadline()));
    ...
    child_support2().SubmitCompositorFrame(child_id4.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
...
TEST_F(SurfaceSynchronizationTest, InvalidatedFrameSinkShouldNotBlockActivation)
{
    const SurfaceId parent_id = MakeSurfaceId(kParentFrameSink, 1);
    const SurfaceId child_id1 = MakeSurfaceId(kChildFrameSink1, 1);
    ...
    EXPECT_FALSE(parent_support().last_activated_surface_id().is_valid());
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), MakeCompositorFrame({ child_id1 }, {}, {}));
...
TEST_F(SurfaceSynchronizationTest, SurfaceActivationDuringDeletion)
{
    ...
    child_support1().SubmitCompositorFrame(
        child_id2.local_surface_id(), MakeCompositorFrame({ arbitrary_id }, empty_surface_ranges(), std::vector<TransferableResource>()));
...
TEST_F(SurfaceSynchronizationTest, AllocationGroupCreationInitiatedBySubmitter)
{
    ...
    // Submit a CompositorFrame to |child_id1|.
    child_support1().SubmitCompositorFrame(surface_id.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
...
TEST_F(SurfaceSynchronizationTest, AllocationGroupCreationInitiatedByEmbedder)
{
    ...
    CompositorFrame frame = MakeCompositorFrame({}, { SurfaceRange(std::nullopt, child_id) }, {});
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), std::move(frame));
    ...
    EXPECT_TRUE(surface_manager()->GetAllocationGroupForSurfaceId(child_id)->IsReadyToDestroy());
...
TEST_F(SurfaceSynchronizationTest, FallbackAllocationGroupDestroyedAfterPrimaryAvailable)
{
    const SurfaceId parent_id = MakeSurfaceId(kParentFrameSink, 1, 1);
    const SurfaceId child_id1 = MakeSurfaceId(kChildFrameSink1, 1, 1);
    ...
    // of them.
    CompositorFrame frame = MakeCompositorFrame({}, { SurfaceRange(child_id1, child_id2) }, {});
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), std::move(frame));
    ...
    EXPECT_TRUE(surface_manager()->GetAllocationGroupForSurfaceId(child_id1)->IsReadyToDestroy());
    ASSERT_TRUE(surface_manager()->GetAllocationGroupForSurfaceId(child_id2));
    EXPECT_FALSE(surface_manager()->GetAllocationGroupForSurfaceId(child_id2)->IsReadyToDestroy());
    EXPECT_TRUE(allocation_groups_need_garbage_collection());
...
// the fallback at all.
TEST_F(SurfaceSynchronizationTest, FallbackAllocationGroupNotCreatedIfPrimaryAvailable)
{
    const SurfaceId parent_id = MakeSurfaceId(kParentFrameSink, 1, 1);
    const SurfaceId child_id1 = MakeSurfaceId(kChildFrameSink1, 1, 1);
    const SurfaceId child_id2 = MakeSurfaceId(kChildFrameSink2, 1, 1);
...
TEST_F(SurfaceSynchronizationTest, CheckLastActiveSurfaceAfterEmbedTokenChange)
{
    const SurfaceId parent_id1 = MakeSurfaceId(kParentFrameSink, 2);
    const SurfaceId parent_id2(kParentFrameSink, LocalSurfaceId(1, base::UnguessableToken::Create()));
    parent_support().SubmitCompositorFrame(parent_id1.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
    Surface* parent_surface1 = GetSurfaceForId(parent_id1);
    EXPECT_TRUE(parent_surface1->HasActiveFrame());
    EXPECT_FALSE(parent_surface1->HasPendingFrame());
...
TEST_F(SurfaceSynchronizationTest, LatestInFlightSurfaceConflict)
{
    ...
    parent_support().SubmitCompositorFrame(id1.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
    parent_support().SubmitCompositorFrame(id2.local_surface_id(), MakeDefaultInteractiveCompositorFrame(kBeginFrameSourceId));
    EXPECT_EQ(GetSurfaceForId(id1), GetLatestInFlightSurface(SurfaceRange(std::nullopt, id3)));
}

// Check that if two different SurfaceIds with the same embed token are
// embedded, viz doesn't crash. https://crbug.com/1001143
TEST_F(SurfaceSynchronizationTest, DuplicateAllocationGroupInActivationDependencies)
{
    const SurfaceId parent_id = MakeSurfaceId(kParentFrameSink, 1);
    ...
    CompositorFrame child1_frame = CompositorFrameBuilder()
                                       .AddDefaultRenderPass()
                                       .SetActivationDependencies({ child2_id1 })
                                       .SetReferencedSurfaces({ SurfaceRange(std::nullopt, child2_id1) })
                                       .SetIsHandlingInteraction(true)
                                       .Build();
    child_support1().SubmitCompositorFrame(child1_id1.local_surface_id(), std::move(child1_frame));

    // Submit a CompositorFrame to |parent_id| embedding both |child1_id1| and
    // |child1_id2|.
    CompositorFrame parent_frame = CompositorFrameBuilder()
                                       .AddDefaultRenderPass()
                                       .SetActivationDependencies({ child1_id1, child1_id2 })
                                       .SetReferencedSurfaces({ SurfaceRange(std::nullopt, child1_id1), SurfaceRange(std::nullopt, child1_id2) })
                                       .SetIsHandlingInteraction(true)
                                       .Build();
    // This shouldn't crash.
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), std::move(parent_frame));
...
class SurfaceSynchronizationTestMayAlwaysAckOnActivation : public SurfaceSynchronizationTest, public testing::WithParamInterface<bool> {
public:
...
SurfaceSynchronizationTestMayAlwaysAckOnActivation::SurfaceSynchronizationTestMayAlwaysAckOnActivation()
{
    std::vector<base::test::FeatureRef> enabled_features;
    std::vector<base::test::FeatureRef> disabled_features;
    if (ShouldAckOnSurfaceActivationWhenInteractive()) {
        enabled_features.push_back(features::kAckOnSurfaceActivationWhenInteractive);
    } else {
        disabled_features.push_back(features::kAckOnSurfaceActivationWhenInteractive);
    }
...
TEST_P(SurfaceSynchronizationTestMayAlwaysAckOnActivation, UnAckedSurfaceArrivesBeforeNewActivationDependencies)
{
    TestSurfaceIdAllocator parent_id(kParentFrameSink);
    TestSurfaceIdAllocator child_id(kChildFrameSink1);

    parent_support().SubmitCompositorFrame(
        parent_id.local_surface_id(), MakeCompositorFrame({ child_id }, { SurfaceRange(std::nullopt, child_id) }, std::vector<TransferableResource>()));
...
TEST_P(SurfaceSynchronizationTestMayAlwaysAckOnActivation, OnlyEarlyAckNonInteractiveSurface)
{
    TestSurfaceIdAllocator parent_id(kParentFrameSink);
    TestSurfaceIdAllocator child_id(kChildFrameSink1);
    ...
    EXPECT_CALL(support_client_, DidReceiveCompositorFrameAck(_)).Times(0);
    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(),
        MakeCompositorFrame({ child_id }, { SurfaceRange(std::nullopt, child_id) }, std::vector<TransferableResource>(), FrameDeadline(),
    ...
    child_support1().SubmitCompositorFrame(child_id.local_surface_id(),
        MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>(), FrameDeadline(),
            /*is_handling_interaction=*/false));
    testing::Mock::VerifyAndClearExpectations(&support_client_);
}

INSTANTIATE_TEST_SUITE_P(, SurfaceSynchronizationTestMayAlwaysAckOnActivation, testing::Bool(), [](const ::testing::TestParamInfo<bool>& info) -> std::string {
    return info.param ? "AckOnSurfaceActivationWhenInteractive" : "DoNotAckOnSurfaceActivationWhenInteractive";
});

class SurfaceSynchronizationTestDrawImmediatelyWithActivationAck : public SurfaceSynchronizationTest {
public:
    SurfaceSynchronizationTestDrawImmediatelyWithActivationAck()
    {
        scoped_feature_list_.InitWithFeatures({ features::kAckOnSurfaceActivationWhenInteractive, features::kDrawImmediatelyWhenInteractive }, {});
    }
    ~SurfaceSynchronizationTestDrawImmediatelyWithActivationAck() override = default;
...
TEST_F(SurfaceSynchronizationTestDrawImmediatelyWithActivationAck, AckOnSurfaceActivationDuringInteractionCooldown)
{
    ...
    auto frame1 = MakeCompositorFrame({ child_id }, { SurfaceRange(std::nullopt, child_id) }, std::vector<TransferableResource>());
    frame1.metadata.begin_frame_ack.frame_id.sequence_number = interactive_frame_number;

    parent_support().SubmitCompositorFrame(parent_id.local_surface_id(), std::move(frame1));
    ...
    // immediately.
    auto frame2 = MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>());
    frame2.metadata.begin_frame_ack.frame_id.sequence_number = interactive_frame_number - 1;
    frame2.metadata.is_handling_interaction = false;
    child_support1().SubmitCompositorFrame(child_id.local_surface_id(), std::move(frame2));
    ...
    auto frame3 = MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>());
    frame3.metadata.begin_frame_ack.frame_id.sequence_number = interactive_frame_number;
    frame3.metadata.is_handling_interaction = false;
    child_support1().SubmitCompositorFrame(child_id.local_surface_id(), std::move(frame3));
    ...
    auto frame4 = MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>());
    frame4.metadata.begin_frame_ack.frame_id.sequence_number = interactive_frame_number + 1;
    frame4.metadata.is_handling_interaction = false;
    child_support1().SubmitCompositorFrame(child_id.local_surface_id(), std::move(frame4));
    ...
    auto frame5 = MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>());
    std::optional<uint64_t> cooldown_frames = features::NumCooldownFramesForAckOnSurfaceActivationDuringInteraction();
    EXPECT_TRUE(cooldown_frames);
    frame5.metadata.begin_frame_ack.frame_id.sequence_number = interactive_frame_number + *cooldown_frames + 1;
    frame5.metadata.is_handling_interaction = false;
    child_support1().SubmitCompositorFrame(child_id.local_surface_id(), std::move(frame5));
...
TEST_P(OnBeginFrameAcksSurfaceSynchronizationTest, UnAckedOldActivationDependencyArrivesAfterNewDependencies)
{
    ...
    EXPECT_CALL(support_client_, DidReceiveCompositorFrameAck(_)).Times(BeginFrameAcksEnabled() ? 0 : 1);
    child_support1().SubmitCompositorFrame(old_child_id, MakeCompositorFrame(empty_surface_ids(), empty_surface_ranges(), std::vector<TransferableResource>()));
    testing::Mock::VerifyAndClearExpectations(&support_client_);
...
INSTANTIATE_TEST_SUITE_P(, OnBeginFrameAcksSurfaceSynchronizationTest, testing::Combine(testing::Bool(), testing::Bool()), [](auto& info) {
    std::string name = std::get<0>(info.param) ? "BeginFrameAcks" : "CompositoFrameAcks";
