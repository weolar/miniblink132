...
ExternalBeginFrameSourceMojo::ExternalBeginFrameSourceMojo(
    FrameSinkManagerImpl* frame_sink_manager, mojo::PendingAssociatedReceiver<mojom::ExternalBeginFrameController> controller_receiver, uint32_t restart_id)
...
void ExternalBeginFrameSourceMojo::IssueExternalBeginFrame(const BeginFrameArgs& args, bool force, base::OnceCallback<void(const BeginFrameAck&)> callback)
{
...
void ExternalBeginFrameSourceMojo::OnDestroyedCompositorFrameSink(const FrameSinkId& sink_id)
{
    pending_frame_sinks_.erase(sink_id);
    MaybeProduceFrameCallback();
}

void ExternalBeginFrameSourceMojo::OnFrameSinkDidBeginFrame(const FrameSinkId& sink_id, const BeginFrameArgs& args)
{
...
void ExternalBeginFrameSourceMojo::OnFrameSinkDidFinishFrame(const FrameSinkId& sink_id, const BeginFrameArgs& args)
{
    if (args.frame_id.source_id != original_source_id_)
        return;
    pending_frame_sinks_.erase(sink_id);
...
void ExternalBeginFrameSourceMojo::MaybeProduceFrameCallback()
{
    ...
    if (!display_->IsRootFrameMissing() && !display_->HasPendingSurfaces(last_begin_frame_args_)) {
        return;
    }
...
void ExternalBeginFrameSourceMojo::DispatchFrameCallback(const BeginFrameAck& ack)
{
...
void ExternalBeginFrameSourceMojo::OnDisplayDidFinishFrame(const BeginFrameAck& ack)
{
    if (!pending_frame_callback_)
        return;
    if (!pending_frame_sinks_.empty()) {
