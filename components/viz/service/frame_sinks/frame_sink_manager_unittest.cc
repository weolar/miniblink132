...
struct RootCompositorFrameSinkData {
    mojom::RootCompositorFrameSinkParamsPtr BuildParams(const FrameSinkId& frame_sink_id)
    {
        ...
        params->compositor_frame_sink = compositor_frame_sink.BindNewEndpointAndPassReceiver();
        params->compositor_frame_sink_client = compositor_frame_sink_client.BindInterfaceRemote();
...
public:
    FrameSinkManagerTest()
        : manager_(FrameSinkManagerImpl::InitParams(&shared_bitmap_manager_, &output_surface_provider_))
    {
    }
    ...
    std::unique_ptr<CompositorFrameSinkSupport> CreateCompositorFrameSinkSupport(const FrameSinkId& frame_sink_id)
    {
        return std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, frame_sink_id, false);
    }

    const BeginFrameSource* GetBeginFrameSource(const std::unique_ptr<CompositorFrameSinkSupport>& support)
    {
    ...
    {
        return base::Contains(manager_.sink_map_, frame_sink_id) || base::Contains(manager_.root_sink_map_, frame_sink_id);
    }
    ...
    bool InputManagerExists()
    {
        return manager_.GetInputManager();
    }
    ...
    // |interval|.
    void VerifyThrottling(base::TimeDelta interval, const std::vector<FrameSinkId>& ids)
    {
        for (auto& id : ids) {
    ...
    void CreateCompositorFrameSink(const FrameSinkId& frame_sink_id, input::mojom::RenderInputRouterConfigPtr config)
    {
        MockCompositorFrameSinkClient compositor_frame_sink_client;
        mojo::Remote<mojom::CompositorFrameSink> compositor_frame_sink;

        manager_.CreateCompositorFrameSink(frame_sink_id, /*bundle_id=*/std::nullopt, compositor_frame_sink.BindNewPipeAndPassReceiver(),
    ...
    base::flat_set<FrameSinkId> GetEmbeddedRenderInputRouters(const FrameSinkId& frame_sink_id)
    {
        auto rir_iterator = GetMockInputManager()->GetEmbeddedRenderInputRouters(frame_sink_id);

        return static_cast<RenderInputRouterIteratorImpl*>(rir_iterator.get())->GetRenderInputRoutersForTesting();
    }

    // testing::Test implementation.
    void SetUp() override
    {
        manager_.SetInputManagerForTesting(std::make_unique<MockInputManager>(&manager_));
    }
...
TEST_F(FrameSinkManagerTest, CreateRootCompositorFrameSink)
{
    ...
    manager_.CreateRootCompositorFrameSink(root_data.BuildParams(kFrameSinkIdRoot));
...
TEST_F(FrameSinkManagerTest, CompositorFrameSinkConnectionLost)
{
    ...
    MockCompositorFrameSinkClient compositor_frame_sink_client;
    mojo::Remote<mojom::CompositorFrameSink> compositor_frame_sink;
    manager_.CreateCompositorFrameSink(kFrameSinkIdA, /*bundle_id=*/std::nullopt, compositor_frame_sink.BindNewPipeAndPassReceiver(),
...
TEST_F(FrameSinkManagerTest, MultipleDisplays)
{
    ...
    manager_.RegisterFrameSinkHierarchy(root1->frame_sink_id(), client_a->frame_sink_id());
    EXPECT_EQ(GetBeginFrameSource(client_a), GetBeginFrameSource(root1));
    manager_.RegisterFrameSinkHierarchy(client_a->frame_sink_id(), client_b->frame_sink_id());
    EXPECT_EQ(GetBeginFrameSource(client_b), GetBeginFrameSource(root1));
    manager_.RegisterFrameSinkHierarchy(root2->frame_sink_id(), client_c->frame_sink_id());
    ...
    // root1 -> A -> B
    // root2 -> C -> A -> B
    manager_.RegisterFrameSinkHierarchy(client_c->frame_sink_id(), client_a->frame_sink_id());
    ...
    manager_.UnregisterFrameSinkHierarchy(root1->frame_sink_id(), client_a->frame_sink_id());
    ...
    // Cleanup hierarchy.
    manager_.UnregisterFrameSinkHierarchy(root2->frame_sink_id(), client_c->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(client_c->frame_sink_id(), client_a->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(client_a->frame_sink_id(), client_b->frame_sink_id());
}
...
TEST_F(FrameSinkManagerTest, FrameSinkParentChildRelationship)
{
    ...
    manager_.CreateRootCompositorFrameSink(root_data1.BuildParams(kFrameSinkIdRoot));
    EXPECT_TRUE(CompositorFrameSinkExists(kFrameSinkIdRoot));

    RootCompositorFrameSinkData root_data2;
    manager_.CreateRootCompositorFrameSink(root_data2.BuildParams(kFrameSinkIdRoot2));
    ...
    // root2 -> E
    manager_.RegisterFrameSinkHierarchy(root1->frame_sink_id(), client_a->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestParentByChildFrameId(client_a->frame_sink_id()), root1->frame_sink_id());
    manager_.RegisterFrameSinkHierarchy(client_a->frame_sink_id(), client_b->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestParentByChildFrameId(client_b->frame_sink_id()), client_a->frame_sink_id());
    manager_.RegisterFrameSinkHierarchy(client_b->frame_sink_id(), client_c->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestParentByChildFrameId(client_c->frame_sink_id()), client_b->frame_sink_id());
    manager_.RegisterFrameSinkHierarchy(client_b->frame_sink_id(), client_d->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestParentByChildFrameId(client_d->frame_sink_id()), client_b->frame_sink_id());
    manager_.RegisterFrameSinkHierarchy(root2->frame_sink_id(), client_e->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestParentByChildFrameId(client_e->frame_sink_id()), root2->frame_sink_id());

    EXPECT_EQ(manager_.GetOldestRootCompositorFrameSinkId(client_a->frame_sink_id()), root1->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestRootCompositorFrameSinkId(client_b->frame_sink_id()), root1->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestRootCompositorFrameSinkId(client_c->frame_sink_id()), root1->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestRootCompositorFrameSinkId(client_d->frame_sink_id()), root1->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestRootCompositorFrameSinkId(client_e->frame_sink_id()), root2->frame_sink_id());
    ...
    // root2 -> E -> A -> B -> C
    //                    + -> D
    manager_.RegisterFrameSinkHierarchy(client_e->frame_sink_id(), client_a->frame_sink_id());
    ...
    // root2 -> E -> A -> B -> C
    //                    + -> D
    EXPECT_EQ(manager_.GetOldestParentByChildFrameId(client_a->frame_sink_id()), client_e->frame_sink_id());

    EXPECT_EQ(manager_.GetOldestRootCompositorFrameSinkId(client_a->frame_sink_id()), root2->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestRootCompositorFrameSinkId(client_b->frame_sink_id()), root2->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestRootCompositorFrameSinkId(client_c->frame_sink_id()), root2->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestRootCompositorFrameSinkId(client_d->frame_sink_id()), root2->frame_sink_id());
    EXPECT_EQ(manager_.GetOldestRootCompositorFrameSinkId(client_e->frame_sink_id()), root2->frame_sink_id());

    manager_.UnregisterFrameSinkHierarchy(root2->frame_sink_id(), client_e->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(client_e->frame_sink_id(), client_a->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(client_a->frame_sink_id(), client_b->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(client_b->frame_sink_id(), client_d->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(client_b->frame_sink_id(), client_c->frame_sink_id());
...
TEST_F(FrameSinkManagerTest, ParentWithoutClientRetained_LateBeginFrameRegistration)
{
    StubBeginFrameSource root_source;
...
TEST_F(FrameSinkManagerTest, Throttle)
{
    ...
    manager_.RegisterFrameSinkHierarchy(root->frame_sink_id(), client_a->frame_sink_id());
    manager_.RegisterFrameSinkHierarchy(client_a->frame_sink_id(), client_b->frame_sink_id());
    manager_.RegisterFrameSinkHierarchy(root->frame_sink_id(), client_c->frame_sink_id());
    manager_.RegisterFrameSinkHierarchy(client_c->frame_sink_id(), client_d->frame_sink_id());

    constexpr base::TimeDelta interval = base::Hertz(20);

    std::vector<FrameSinkId> ids { kFrameSinkIdRoot, kFrameSinkIdA, kFrameSinkIdB, kFrameSinkIdC, kFrameSinkIdD };
    ...
    manager_.UnregisterFrameSinkHierarchy(root->frame_sink_id(), client_a->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(client_a->frame_sink_id(), client_b->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(root->frame_sink_id(), client_c->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(client_c->frame_sink_id(), client_d->frame_sink_id());
}
...
TEST_F(FrameSinkManagerTest, GlobalThrottle)
{
    ...
    VerifyThrottling(global_interval, { kFrameSinkIdRoot, kFrameSinkIdA, kFrameSinkIdB });
    ...
    manager_.RegisterFrameSinkHierarchy(root->frame_sink_id(), client_e->frame_sink_id());
    manager_.RegisterFrameSinkHierarchy(client_e->frame_sink_id(), client_f->frame_sink_id());
    VerifyThrottling(global_interval, { kFrameSinkIdRoot, kFrameSinkIdA, kFrameSinkIdB, kFrameSinkIdC, kFrameSinkIdD, kFrameSinkIdE, kFrameSinkIdF });
    ...
    manager_.UnregisterFrameSinkHierarchy(root->frame_sink_id(), client_c->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(client_c->frame_sink_id(), client_d->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(root->frame_sink_id(), client_e->frame_sink_id());
    manager_.UnregisterFrameSinkHierarchy(client_e->frame_sink_id(), client_f->frame_sink_id());
}
...
TEST_F(FrameSinkManagerTest, NoThrottleOnFrameSinksBeingCaptured)
{
    ...
    std::vector<FrameSinkId> ids { kFrameSinkIdRoot, kFrameSinkIdA, kFrameSinkIdB, kFrameSinkIdC };
    ...
    CapturableFrameSink* capturable_frame_sink = FindCapturableFrameSink(kFrameSinkIdB);
...
TEST_F(FrameSinkManagerTest, EvictRootSurfaceId)
{
    ...
    GetRootCompositorFrameSinkImpl()->SubmitCompositorFrame(local_surface_id, MakeDefaultCompositorFrame(), std::nullopt, 0);
    ...
    EXPECT_TRUE(base::test::RunUntil([&]() { return !GetRootCompositorFrameSinkImpl()->CurrentSurfaceId().is_valid(); }));
...
TEST_F(FrameSinkManagerTest, EvictNewerRootSurfaceId)
{
    ...
    EXPECT_EQ(surface_id, GetRootCompositorFrameSinkImpl()->CurrentSurfaceId());
    allocator.GenerateId();
    const LocalSurfaceId next_local_surface_id = allocator.GetCurrentLocalSurfaceId();
...
TEST_F(FrameSinkManagerTest, SubmitCompositorFrameWithEvictedSurfaceId)
{
    ...
    GetRootCompositorFrameSinkImpl()->SubmitCompositorFrame(local_surface_id2, MakeDefaultCompositorFrame(), std::nullopt, 0);
...
TEST_F(FrameSinkManagerTest, CopyOutputRequestPreservedAfterDiscardPendingCopyOfOutputRequests)
{
    StubBeginFrameSource source;
    ...
    manager_.GetFrameSinkForId(kFrameSinkIdA)->SubmitCompositorFrame(id1, MakeDefaultCompositorFrame());
    ...
    const auto& preserved_request = *(surface1->GetActiveFrame().render_pass_list.back()->copy_requests.back());
...
// non-exact requests.
TEST_F(FrameSinkManagerTest, ExactCopyOutputRequestQueuedInCompositorFrameSinkSupport)
{
    StubBeginFrameSource source;
    manager_.RegisterBeginFrameSource(&source, kFrameSinkIdA);
...
class AndroidFrameSinkManagerTest : public FrameSinkManagerTest, public ::testing::WithParamInterface<bool> {
public:
...
TEST_P(AndroidFrameSinkManagerTest, RenderInputRouterLifecycle)
{
    ...
    if (input::IsTransferInputToVizSupported()) {
        ...
        EXPECT_THAT(result.value(),
            testing::ElementsAre(testing::ElementsAre("cnt", "config_is_null", "client_id", "sink_id"),
                testing::ElementsAre("1", "0", base::NumberToString(kFrameSinkIdA.client_id()), base::NumberToString(kFrameSinkIdA.sink_id()))));
    } else {
    ...
    if (input::IsTransferInputToVizSupported()) {
        EXPECT_THAT(result2.value(),
            testing::ElementsAre(testing::ElementsAre("cnt", "client_id", "sink_id"),
                testing::ElementsAre("1", base::NumberToString(kFrameSinkIdA.client_id()), base::NumberToString(kFrameSinkIdA.sink_id()))));
    } else {
        EXPECT_EQ(result2.value()[1][0], "0");
    }
}

TEST_P(AndroidFrameSinkManagerTest, RenderInputRouterLifecycleNonLayerFrameSink)
{
    EXPECT_EQ(InputManagerExists(), ExpectedInputManagerCreation());
    ...
    if (input::IsTransferInputToVizSupported()) {
        EXPECT_THAT(result.value(),
            testing::ElementsAre(testing::ElementsAre("cnt", "config_is_null", "client_id", "sink_id"),
                testing::ElementsAre("1", "1", base::NumberToString(kFrameSinkIdB.client_id()), base::NumberToString(kFrameSinkIdB.sink_id()))));
    } else {
        EXPECT_EQ(result.value()[1][0], "0");
    }
}
...
TEST_P(AndroidFrameSinkManagerTest, VizRIRDelegateLifecycle)
{
    ...
    if (input::IsTransferInputToVizSupported()) {
        ...
        EXPECT_THAT(result.value(),
            testing::ElementsAre(testing::ElementsAre("name"),
                testing::ElementsAre("RenderInputRouterDelegateImpl::"
                                     "RenderInputRouterDelegateImpl"),
                testing::ElementsAre("RenderInputRouter::RenderInputRouter"), testing::ElementsAre("RenderInputRouter::~RenderInputRouter"),
                testing::ElementsAre("RenderInputRouterDelegateImpl::~"
                                     "RenderInputRouterDelegateImpl")));
    } else {
...
TEST_P(AndroidFrameSinkManagerTest, RenderInputRouterSupportTraversals)
{
    ...
    manager_.RegisterFrameSinkHierarchy(kFrameSinkIdA, kFrameSinkIdB);
    EXPECT_EQ(GetMockInputManager()->GetParentRenderInputRouterSupport(kFrameSinkIdB), GetMockInputManager()->GetSupportForFrameSink(kFrameSinkIdA));
    manager_.RegisterFrameSinkHierarchy(kFrameSinkIdB, kFrameSinkIdC);
    EXPECT_EQ(GetMockInputManager()->GetParentRenderInputRouterSupport(kFrameSinkIdC), GetMockInputManager()->GetSupportForFrameSink(kFrameSinkIdB));
    manager_.RegisterFrameSinkHierarchy(kFrameSinkIdB, kFrameSinkIdD);
    EXPECT_EQ(GetMockInputManager()->GetParentRenderInputRouterSupport(kFrameSinkIdD), GetMockInputManager()->GetSupportForFrameSink(kFrameSinkIdB));

    EXPECT_EQ(GetMockInputManager()->GetRootRenderInputRouterSupport(kFrameSinkIdB), GetMockInputManager()->GetSupportForFrameSink(kFrameSinkIdA));
    EXPECT_EQ(GetMockInputManager()->GetRootRenderInputRouterSupport(kFrameSinkIdC), GetMockInputManager()->GetSupportForFrameSink(kFrameSinkIdA));
    EXPECT_EQ(GetMockInputManager()->GetRootRenderInputRouterSupport(kFrameSinkIdD), GetMockInputManager()->GetSupportForFrameSink(kFrameSinkIdA));
    ...
    // root2 -> E -> A -> B -> C
    //                    + -> D
    EXPECT_EQ(GetMockInputManager()->GetRootRenderInputRouterSupport(kFrameSinkIdB), GetMockInputManager()->GetSupportForFrameSink(kFrameSinkIdE));
    EXPECT_EQ(GetMockInputManager()->GetRootRenderInputRouterSupport(kFrameSinkIdC), GetMockInputManager()->GetSupportForFrameSink(kFrameSinkIdE));
    EXPECT_EQ(GetMockInputManager()->GetRootRenderInputRouterSupport(kFrameSinkIdD), GetMockInputManager()->GetSupportForFrameSink(kFrameSinkIdE));
...
TEST_P(AndroidFrameSinkManagerTest, EmbeddedRenderInputRouters)
{
    ...
    EXPECT_THAT(GetEmbeddedRenderInputRouters(kFrameSinkIdA), testing::ContainerEq(base::flat_set<FrameSinkId>({ kFrameSinkIdB })));
    EXPECT_THAT(GetEmbeddedRenderInputRouters(kFrameSinkIdB), testing::ContainerEq(base::flat_set<FrameSinkId>({ kFrameSinkIdC, kFrameSinkIdD })));
    EXPECT_THAT(GetEmbeddedRenderInputRouters(kFrameSinkIdC), testing::ContainerEq(base::flat_set<FrameSinkId>({})));
    ...
    // root2 -> E -> A -> B -> C
    //                    + -> D
    EXPECT_THAT(GetEmbeddedRenderInputRouters(kFrameSinkIdE), testing::ContainerEq(base::flat_set<FrameSinkId>({ kFrameSinkIdA })));
    EXPECT_THAT(GetEmbeddedRenderInputRouters(kFrameSinkIdA), testing::ContainerEq(base::flat_set<FrameSinkId>({ kFrameSinkIdB })));
    EXPECT_THAT(GetEmbeddedRenderInputRouters(kFrameSinkIdB), testing::ContainerEq(base::flat_set<FrameSinkId>({ kFrameSinkIdC, kFrameSinkIdD })));
    EXPECT_THAT(GetEmbeddedRenderInputRouters(kFrameSinkIdRoot2), testing::ContainerEq(base::flat_set<FrameSinkId>({ kFrameSinkIdE })));
...
INSTANTIATE_TEST_SUITE_P(
    All, AndroidFrameSinkManagerTest, ::testing::Bool(), [](auto& info) { return info.param ? "InputOnViz_Enabled" : "InputOnViz_Disabled"; });
...
static const RegisterOrder kRegisterOrderList[] = { REGISTER_HIERARCHY_FIRST, REGISTER_CLIENTS_FIRST };
static const UnregisterOrder kUnregisterOrderList[] = { UNREGISTER_HIERARCHY_FIRST, UNREGISTER_CLIENTS_FIRST };
...
class FrameSinkManagerOrderingParamTest : public FrameSinkManagerOrderingTest,
                                          public ::testing::WithParamInterface<std::tuple<RegisterOrder, UnregisterOrder, BFSOrder>> { };
...
INSTANTIATE_TEST_SUITE_P(FrameSinkManagerOrderingParamTestInstantiation, FrameSinkManagerOrderingParamTest,
    ::testing::Combine(::testing::ValuesIn(kRegisterOrderList), ::testing::ValuesIn(kUnregisterOrderList), ::testing::ValuesIn(kBFSOrderList)));
