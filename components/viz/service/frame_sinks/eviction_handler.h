...
public:
    EvictionHandler(Display* display, CompositorFrameSinkSupport* support, ReservedResourceIdTracker* id_tracker);
...
private:
    ...
    std::optional<TransferableResource> CreateTransferableResourceFromCopyOutputResult(
    ...
    void SubmitPlaceholderContentForEviction(SurfaceId to_evict, int64_t snapshot_seq_id, std::unique_ptr<CopyOutputResult> copy_result);
    ...
    base::flat_map<ResourceId, absl::variant<scoped_refptr<gpu::ClientSharedImage>, std::unique_ptr<CopyOutputResult>>> copy_output_results_;
