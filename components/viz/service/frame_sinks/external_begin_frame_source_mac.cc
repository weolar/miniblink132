...
BASE_FEATURE(kForceMacVSyncTimerForDebugging, "ForceMacVSyncTimerForDebugging", base::FEATURE_DISABLED_BY_DEFAULT);

// Allow CADisplayLink to handle refresh rate within the range based on the app
// work load.
BASE_FEATURE(kUseRefreshRateRange, "UseRefreshRateRange", base::FEATURE_DISABLED_BY_DEFAULT);
...
{
    UMA_HISTOGRAM_ENUMERATION("Viz.ExternalBeginFrameSourceMac.DisplayLink", result);
}
...
void RecordVSyncCallbackDelay(base::TimeDelta delay)
{
    ...
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Viz.BeginFrameSource.VSyncCallbackDelay", delay,
...
ExternalBeginFrameSourceMac::ExternalBeginFrameSourceMac(uint32_t restart_id, int64_t display_id, OutputSurface* output_surface)
    ...
    , output_surface_(output_surface)
{
    ...
    if (display_id == display::kInvalidDisplayId) {
        ...
        DLOG(ERROR) << "DisplayLinkMac ID is not available. "
...
{
    if (!time_source_) {
        time_source_ = std::make_unique<DelayBasedTimeSource>(base::SingleThreadTaskRunner::GetCurrentDefault().get());
        time_source_->SetClient(this);
        time_source_->SetTimebaseAndInterval(base::TimeTicks::Now(), preferred_interval_);
    }
}
...
void ExternalBeginFrameSourceMac::SetVSyncDisplayID(int64_t display_id)
{
    ...
    if (display_link_mac_) {
        ...
        VLOG(kOutputLevel) << "ExternalBeginFrameSourceMac(" << this << ")"
                           << "::SetVSyncDisplayID: " << display_id_ << ", refresh_period_: " << nominal_refresh_period_;

        display_link_mac_->GetRefreshIntervalRange(min_refresh_interval_, max_refresh_interval_, granularity_);
        ...
        if (base::FeatureList::IsEnabled(kUseRefreshRateRange)) {
            hw_takes_any_refresh_rate_ = granularity_ <= base::Milliseconds(1) && min_refresh_interval_ != max_refresh_interval_;
        ...
        if (update_vsync_params_callback_) {
            update_vsync_params_callback_.Run(display_link_mac_->GetCurrentTime(), nominal_refresh_period_);
        }

        RecordDisplayLinkCreateStatus(DisplayLinkResult::kSuccess);
    } else {
        DisplayLinkResult display_link_result
            = display_id == display::kInvalidDisplayId ? DisplayLinkResult::kFailedInvalidDisplayId : DisplayLinkResult::kFailedCreateDisplayLink;
        RecordDisplayLinkCreateStatus(display_link_result);

        DLOG(ERROR) << "Fail to create DisplayLinkMac with DisplayID: " << display_id_ << ". Switch to DelayBasedTimeSource.";
...
void ExternalBeginFrameSourceMac::StartBeginFrame()
{
    if (display_link_mac_) {
        ...
        vsync_callback_mac_
            = display_link_mac_->RegisterCallback(base::BindRepeating(&ExternalBeginFrameSourceMac::OnDisplayLinkCallback, weak_ptr_factory_.GetWeakPtr()));
        ...
        DLOG(ERROR) << "Fail to start CVDisplayLink callback for DisplayID: " << display_id_ << ". Switch to the timer";
    }
...
void ExternalBeginFrameSourceMac::OnDisplayLinkCallback(ui::VSyncParamsMac params)
{
    if (!needs_begin_frames_) {
        return;
    }
    ...
    if (vsyncs_to_skip_ > 0) {
        TRACE_EVENT_INSTANT0("viz", "ExternalBeginFrameSourceMac::OnDisplayLinkCallback - skip_vsync", TRACE_EVENT_SCOPE_THREAD);
        vsyncs_to_skip_--;
        return;
    }
    ...
    if (params.callback_times_valid) {
        ...
        frame_time = now;
        interval = params.display_times_valid ? params.display_interval : nominal_refresh_period_;
    }

    auto callback_delay = params.callback_times_valid ? (now - frame_time) : base::Microseconds(0);
    auto callback_timebase_to_display = params.display_times_valid ? (params.display_timebase - frame_time) : base::Microseconds(0);
    TRACE_EVENT2("viz", "ExternalBeginFrameSourceMac::OnDisplayLinkCallback", "callback_timebase_to_display", callback_timebase_to_display.InMicroseconds(),
        "callback_delay", callback_delay.InMicroseconds());
    RecordVSyncCallbackDelay(callback_delay);

    bool display_link_frame_interval_changed = !AlmostEqual(nominal_refresh_period_, interval);
    ...
    OnBeginFrame(begin_frame_args_generator_.GenerateBeginFrameArgs(source_id(), frame_time, frame_time + interval, interval));
    ...
    if (display_link_frame_interval_changed) {
        ...
        VLOG(kOutputLevel) << "ExternalBeginFrameSourceMac(" << this << ")"
                           << "::OnDisplayLinkCallback: " << display_id_ << ", nominal_refresh_period_: " << nominal_refresh_period_;
...
BeginFrameArgs ExternalBeginFrameSourceMac::GetMissedBeginFrameArgs(BeginFrameObserver* obs)
{
    ...
    if (!last_begin_frame_args_.IsValid() || frame_time > last_begin_frame_args_.frame_time) {
        last_begin_frame_args_ = begin_frame_args_generator_.GenerateBeginFrameArgs(source_id(), frame_time, frame_time + interval, interval);
    }
...
void ExternalBeginFrameSourceMac::OnTimerTick()
{
    ...
    base::TimeTicks frame_time = std::max(time_source_->LastTickTime(), time_source_->NextTickTime() - time_source_->Interval());
    auto interval = time_source_->Interval();

    OnBeginFrame(begin_frame_args_generator_.GenerateBeginFrameArgs(source_id(), frame_time, time_source_->NextTickTime(), interval));
...
void ExternalBeginFrameSourceMac::SetPreferredInterval(base::TimeDelta interval)
{
    preferred_interval_ = interval;

    VLOG(kOutputLevel) << "ExternalBeginFrameSourceMac(" << this << ")"
                       << "::SetPreferredInterval: ID: " << display_id_ << ", Interval: " << interval;
    ...
    if (min_refresh_interval_ != max_refresh_interval_) {
        if (base::FeatureList::IsEnabled(kUseRefreshRateRange)) {
            ...
            display_link_mac_->SetPreferredIntervalRange(min_refresh_interval_, max_refresh_interval_, interval);
        } else {
    ...
    } else if (interval > kMaxSupportedFrameInterval && !AlmostEqual(interval, nominal_refresh_period_)) {
    ...
    vsync_subsampling_factor_ = adjusted_interval.IntDiv((nominal_refresh_period_ - kDeltaAlmostEqual));

    TRACE_EVENT1("gpu", "ExternalBeginFrameSourceMac::SetPreferredInterval", "vsync_subsampling_factor", vsync_subsampling_factor_);
}
...
void ExternalBeginFrameSourceMac::SetUpdateVSyncParametersCallback(UpdateVSyncParametersCallback callback)
{
    update_vsync_params_callback_ = callback;
}

void ExternalBeginFrameSourceMac::SetMultipleHWRefreshRatesCallback(MultipleHWRefreshRatesCallback callback)
{
    multiple_hw_refresh_rates_callback_ = callback;
}

base::flat_set<base::TimeDelta> ExternalBeginFrameSourceMac::GetSupportedFrameIntervals(base::TimeDelta current_interval)
{
    VLOG(kOutputLevel) << "ExternalBeginFrameSourceMac(" << this << ")"
    ...
    if (nominal_refresh_period_ > kMaxSupportedFrameInterval && min_refresh_interval_ == max_refresh_interval_) {
        VLOG(kOutputLevel) << "nominal_refresh_period_: " << nominal_refresh_period_;
    ...
    if (display_link_mac_ && min_refresh_interval_ != max_refresh_interval_) {
        ...
        for (base::TimeDelta interval = min_refresh_interval_; interval < upper_bound; interval += granularity_) {
...
DelayBasedBeginFrameSourceMac::DelayBasedBeginFrameSourceMac(std::unique_ptr<DelayBasedTimeSource> time_source, uint32_t restart_id)
...
void DelayBasedBeginFrameSourceMac::RequestTimeSourceParamsUpdate()
{
    ...
    time_source_updater_
        = display_link_->RegisterCallback(base::BindRepeating(&DelayBasedBeginFrameSourceMac::OnTimeSourceParamsUpdate, weak_factory_.GetWeakPtr()));
}

void DelayBasedBeginFrameSourceMac::OnTimeSourceParamsUpdate(ui::VSyncParamsMac params)
{
    ...
    VLOG(kOutputLevel) << "DelayBasedBeginFrameSourceMac(" << this << ")"
                       << "::OnTimeSourceParamsUpdate: ID: " << display_id_ << "last_hw_interval_," << last_hw_interval_ << "display_interval,"
    ...
    if (last_hw_interval_ == params.display_interval) {
        ...
        update_vsync_params_callback_.Run(params.display_timebase, params.display_interval);
    }
...
{
    if (last_begin_frame_args().interval == time_source()->Interval() && !just_started_begin_frame_) {
        base::TimeDelta delta = base::TimeTicks::Now() - last_begin_frame_args().deadline;
        RecordBeginFrameSourceAccuracy(delta);
    }
    just_started_begin_frame_ = false;
...
void DelayBasedBeginFrameSourceMac::SetUpdateVSyncParametersCallback(UpdateVSyncParametersCallback callback)
{
    update_vsync_params_callback_ = callback;
}
