...
public:
    ...
    SurfaceId CreateSurface(const FrameSinkId& frame_sink_id, uint32_t parent_id)
    {
        SurfaceId surface_id = allocator_set_.MakeSurfaceId(frame_sink_id, parent_id);
        GetCompositorFrameSinkSupport(frame_sink_id).SubmitCompositorFrame(surface_id.local_surface_id(), MakeDefaultCompositorFrame());
        return surface_id;
    }
    ...
    {
        GetCompositorFrameSinkSupport(surface_id.frame_sink_id()).EvictSurface(surface_id.local_surface_id());
    }

    CompositorFrameSinkSupport& GetCompositorFrameSinkSupport(const FrameSinkId& frame_sink_id)
    {
        auto& support_ptr = supports_[frame_sink_id];
        if (!support_ptr) {
            manager_->RegisterFrameSinkId(frame_sink_id, true /* report_activation */);
            constexpr bool is_root = false;
            support_ptr = std::make_unique<CompositorFrameSinkSupport>(nullptr, manager_.get(), frame_sink_id, is_root);
        }
        return *support_ptr;
    }
    ...
    void RemoveSurfaceReference(const SurfaceId& parent_id, const SurfaceId& child_id)
    {
        manager_->surface_manager()->RemoveSurfaceReferences({ SurfaceReference(parent_id, child_id) });
    }

    void AddSurfaceReference(const SurfaceId& parent_id, const SurfaceId& child_id)
    {
        manager_->surface_manager()->AddSurfaceReferences({ SurfaceReference(parent_id, child_id) });
    }
    ...
    const base::flat_set<SurfaceId> GetReferencesFor(const SurfaceId& surface_id)
    {
        return GetSurfaceManager().GetSurfacesThatReferenceChildForTesting(surface_id);
    }
...
TEST_F(SurfaceReferencesTest, AddReference)
{
    ...
    EXPECT_THAT(GetReferencesFor(id1), UnorderedElementsAre(GetSurfaceManager().GetRootSurfaceId()));
...
TEST_F(SurfaceReferencesTest, AddSurfaceThenRootReference)
{
    ...
    EXPECT_THAT(GetReferencesFrom(GetSurfaceManager().GetRootSurfaceId()), ElementsAre(surface_id));
}
...
TEST_F(SurfaceReferencesTest, AddTwoSurfacesThenOneReference)
{
    ...
    EXPECT_THAT(GetAllTempReferences(), UnorderedElementsAre(surface_id1, surface_id2));
    ...
    EXPECT_THAT(GetAllTempReferences(), UnorderedElementsAre(parent_id, surface_id2));
