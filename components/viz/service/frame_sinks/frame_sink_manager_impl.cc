...
FrameSinkManagerImpl::InitParams::InitParams(
    SharedBitmapManager* shared_bitmap_manager, OutputSurfaceProvider* output_surface_provider, GmbVideoFramePoolContextProvider* gmb_context_provider)
...
FrameSinkManagerImpl::InitParams::~InitParams() = default;
FrameSinkManagerImpl::InitParams& FrameSinkManagerImpl::InitParams::operator=(InitParams&& other) = default;

FrameSinkManagerImpl::FrameSinkSourceMapping::FrameSinkSourceMapping() = default;

FrameSinkManagerImpl::FrameSinkSourceMapping::FrameSinkSourceMapping(FrameSinkSourceMapping&& other) = default;

FrameSinkManagerImpl::FrameSinkSourceMapping::~FrameSinkSourceMapping() = default;

FrameSinkManagerImpl::FrameSinkSourceMapping& FrameSinkManagerImpl::FrameSinkSourceMapping::operator=(FrameSinkSourceMapping&& other) = default;
...
FrameSinkManagerImpl::FrameSinkData::FrameSinkData(FrameSinkData&& other) = default;
FrameSinkManagerImpl::FrameSinkData::~FrameSinkData() = default;
FrameSinkManagerImpl::FrameSinkData& FrameSinkManagerImpl::FrameSinkData::operator=(FrameSinkData&& other) = default;
...
FrameSinkManagerImpl::FrameSinkManagerImpl(const InitParams& params)
    ...
    , surface_manager_(this, params.activation_deadline_in_frames, params.max_uncommitted_frames)
    , hit_test_manager_(surface_manager())
    , restart_id_(params.restart_id)
    , run_all_compositor_stages_before_draw_(params.run_all_compositor_stages_before_draw)
...
CompositorFrameSinkImpl* FrameSinkManagerImpl::GetFrameSinkImpl(const FrameSinkId& id)
{
...
FrameSinkBundleImpl* FrameSinkManagerImpl::GetFrameSinkBundle(const FrameSinkBundleId& id)
{
...
void FrameSinkManagerImpl::BindAndSetClient(mojo::PendingReceiver<mojom::FrameSinkManager> receiver, scoped_refptr<base::SingleThreadTaskRunner> task_runner,
    mojo::PendingRemote<mojom::FrameSinkManagerClient> client, SharedImageInterfaceProvider* shared_image_interface_provider)
{
    DCHECK(!client_);
    ...
    frame_sink_manager_receiver_.Bind(std::move(receiver), std::move(task_runner));
    client_remote_.Bind(std::move(client));
    client_ = client_remote_.get();
}

void FrameSinkManagerImpl::SetLocalClient(mojom::FrameSinkManagerClient* client, scoped_refptr<base::SingleThreadTaskRunner> ui_task_runner)
{
...
// void FrameSinkManagerImpl::SetInputManagerForTesting(std::unique_ptr<InputManager> input_manager)
// {
//     if (!input::IsTransferInputToVizSupported()) {
//         return;
//     }
...
void FrameSinkManagerImpl::RegisterFrameSinkId(const FrameSinkId& frame_sink_id, bool report_activation)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void FrameSinkManagerImpl::InvalidateFrameSinkId(const FrameSinkId& frame_sink_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void FrameSinkManagerImpl::SetFrameSinkDebugLabel(const FrameSinkId& frame_sink_id, const std::string& debug_label)
{
...
void FrameSinkManagerImpl::CreateRootCompositorFrameSink(mojom::RootCompositorFrameSinkParamsPtr params)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    auto root_compositor_frame_sink = RootCompositorFrameSinkImpl::Create(
        std::move(params), this, output_surface_provider_, restart_id_, run_all_compositor_stages_before_draw_, &debug_settings_, hint_session_factory_);

    if (root_compositor_frame_sink) {
        root_sink_map_[frame_sink_id] = std::move(root_compositor_frame_sink);
//         if (GetInputManager()) {
//             GetInputManager()->OnCreateCompositorFrameSink(frame_sink_id,
//                 /*is_root=*/true,
//                 /*render_input_router_config=*/nullptr, create_input_receiver, widget);
//         }
    }
...
void FrameSinkManagerImpl::CreateFrameSinkBundle(
    const FrameSinkBundleId& bundle_id, mojo::PendingReceiver<mojom::FrameSinkBundle> receiver, mojo::PendingRemote<mojom::FrameSinkBundleClient> client)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (base::Contains(bundle_map_, bundle_id)) {
        uint32_t client_id = bundle_id.client_id();
        uint32_t bundle_id_value = bundle_id.bundle_id();
        frame_sink_manager_receiver_.ReportBadMessage("Duplicate FrameSinkBundle ID");
        base::debug::Alias(&client_id);
        base::debug::Alias(&bundle_id_value);
        return;
    }

    bundle_map_[bundle_id] = std::make_unique<FrameSinkBundleImpl>(*this, bundle_id, std::move(receiver), std::move(client));
}

void FrameSinkManagerImpl::CreateCompositorFrameSink(const FrameSinkId& frame_sink_id, const std::optional<FrameSinkBundleId>& bundle_id,
...
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    TRACE_EVENT("viz", "FrameSinkManagerImpl::CreateCompositorFrameSink", "frame_sink_id", frame_sink_id);
    ...
    sink_map_[frame_sink_id] = std::make_unique<CompositorFrameSinkImpl>(this, frame_sink_id, bundle_id, std::move(receiver), std::move(client));
...
void FrameSinkManagerImpl::DestroyCompositorFrameSink(const FrameSinkId& frame_sink_id, DestroyCompositorFrameSinkCallback callback)
{
    sink_map_.erase(frame_sink_id);
    root_sink_map_.erase(frame_sink_id);
...
void FrameSinkManagerImpl::RegisterFrameSinkHierarchy(const FrameSinkId& parent_frame_sink_id, const FrameSinkId& child_frame_sink_id)
{
    ...
    for (auto& observer : observer_list_) {
        observer.OnRegisteredFrameSinkHierarchy(parent_frame_sink_id, child_frame_sink_id);
    }
...
void FrameSinkManagerImpl::UnregisterFrameSinkHierarchy(const FrameSinkId& parent_frame_sink_id, const FrameSinkId& child_frame_sink_id)
{
    ...
    for (auto& observer : observer_list_) {
        observer.OnUnregisteredFrameSinkHierarchy(parent_frame_sink_id, child_frame_sink_id);
    }
    ...
    auto iter_find_parent = std::find(child_mapping.parent.begin(), child_mapping.parent.end(), parent_frame_sink_id);
    child_mapping.parent.erase(iter_find_parent);

    // Delete `child_frame_sink_id` entry from `frame_sink_source_map_` if empty.
    if (child_mapping.children.empty() && child_mapping.parent.empty() && !child_mapping.source) {
...
void FrameSinkManagerImpl::AddVideoDetectorObserver(mojo::PendingRemote<mojom::VideoDetectorObserver> observer)
{
    if (!video_detector_) {
        video_detector_ = std::make_unique<VideoDetector>(GetRegisteredFrameSinkIds(), &surface_manager_);
    }
    video_detector_->AddObserver(std::move(observer));
}

void FrameSinkManagerImpl::CreateVideoCapturer(mojo::PendingReceiver<mojom::FrameSinkVideoCapturer> receiver)
{
    *(int*)1 = 1;
//     video_capturers_.emplace(std::make_unique<FrameSinkVideoCapturerImpl>(*this, gmb_context_provider_, std::move(receiver),
//         std::make_unique<media::VideoCaptureOracle>(true /* enable_auto_throttling */), log_capture_pipeline_in_webrtc_));
}

void FrameSinkManagerImpl::EvictSurfaces(const std::vector<SurfaceId>& surface_ids)
{
...
void FrameSinkManagerImpl::RequestCopyOfOutput(const SurfaceId& surface_id, std::unique_ptr<CopyOutputRequest> request, bool capture_exact_surface_id)
{
    TRACE_EVENT0("viz", "FrameSinkManagerImpl::RequestCopyOfOutput");
    PendingCopyOutputRequest pending_request(surface_id.local_surface_id(), SubtreeCaptureId(), std::move(request), capture_exact_surface_id);
    ...
    if (capture_exact_surface_id) {
        ...
        if (exact_surface) {
            ...
            BeginFrameAck ack;
            ack.has_damage = true;
            surface_manager_.SurfaceModified(surface_id, ack, SurfaceObserver::HandleInteraction::kNoChange);
            return;
        }
    }
    ...
    if (it == support_map_.end()) {
        if (capture_exact_surface_id) {
            ...
            // "chrome://gpu".
            LOG(ERROR) << "The browser issued an exact CopyOutputRequest for " << surface_id << " but there is no such surface or a frame sink.";
        }
...
void FrameSinkManagerImpl::OnFirstSurfaceActivation(const SurfaceInfo& surface_info)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void FrameSinkManagerImpl::UpdateHitTestRegionData(const FrameSinkId& frame_sink_id, const std::vector<AggregatedHitTestRegion>& hit_test_data)
{
...
void FrameSinkManagerImpl::OnAggregatedHitTestRegionListUpdated(const FrameSinkId& frame_sink_id, const std::vector<AggregatedHitTestRegion>& hit_test_data)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
std::string_view FrameSinkManagerImpl::GetFrameSinkDebugLabel(const FrameSinkId& frame_sink_id) const
{
    auto it = frame_sink_data_.find(frame_sink_id);
    if (it != frame_sink_data_.end())
...
void FrameSinkManagerImpl::AddHitTestRegionObserver(HitTestRegionObserver* observer)
{
    hit_test_region_observers_.AddObserver(observer);
}

void FrameSinkManagerImpl::RemoveHitTestRegionObserver(HitTestRegionObserver* observer)
{
    hit_test_region_observers_.RemoveObserver(observer);
}

const DisplayHitTestQueryMap& FrameSinkManagerImpl::GetDisplayHitTestQuery() const
{
    return display_hit_test_query_;
}

void FrameSinkManagerImpl::RegisterCompositorFrameSinkSupport(const FrameSinkId& frame_sink_id, CompositorFrameSinkSupport* support)
{
    DCHECK(support);
...
//         if (capturer->target() && capturer->target()->frame_sink_id == frame_sink_id)
...
//     }
    ...
    if (global_throttle_interval_) {
        UpdateThrottlingRecursively(frame_sink_id, global_throttle_interval_.value());
    }

    if (frame_counter_) {
        frame_counter_->AddFrameSink(frame_sink_id, support->frame_sink_type(), support->is_root(), GetFrameSinkDebugLabel(frame_sink_id));
    }
}

void FrameSinkManagerImpl::UnregisterCompositorFrameSinkSupport(const FrameSinkId& frame_sink_id)
{
    DCHECK(base::Contains(support_map_, frame_sink_id));
...
void FrameSinkManagerImpl::RegisterBeginFrameSource(BeginFrameSource* source, const FrameSinkId& frame_sink_id)
{
    DCHECK(source);
...
void FrameSinkManagerImpl::UnregisterBeginFrameSource(BeginFrameSource* source)
{
    DCHECK(source);
...
void FrameSinkManagerImpl::RecursivelyAttachBeginFrameSource(const FrameSinkId& frame_sink_id, BeginFrameSource* source)
{
...
void FrameSinkManagerImpl::RecursivelyDetachBeginFrameSource(const FrameSinkId& frame_sink_id, BeginFrameSource* source)
{
...
CapturableFrameSink* FrameSinkManagerImpl::FindCapturableFrameSink(const VideoCaptureTarget& target)
{
    ...
    if (IsRegionCapture(target.sub_target)) {
        ...
        for (const auto& id_and_sink : support_map_) {
            const RegionCaptureBounds& bounds = id_and_sink.second->current_capture_bounds();
...
void FrameSinkManagerImpl::OnCapturerConnectionLost(FrameSinkVideoCapturerImpl* capturer)
{
    //video_capturers_.erase(capturer);
}

bool FrameSinkManagerImpl::ChildContains(const FrameSinkId& child_frame_sink_id, const FrameSinkId& search_frame_sink_id) const
{
...
void FrameSinkManagerImpl::SubmitHitTestRegionList(const SurfaceId& surface_id, uint64_t frame_index, std::optional<HitTestRegionList> hit_test_region_list)
{
    hit_test_manager_.SubmitHitTestRegionList(surface_id, frame_index, std::move(hit_test_region_list));
}

void FrameSinkManagerImpl::OnFrameTokenChangedDirect(const FrameSinkId& frame_sink_id, uint32_t frame_token, base::TimeTicks activation_time)
{
    if (client_)
        client_->OnFrameTokenChanged(frame_sink_id, frame_token, activation_time);
}

void FrameSinkManagerImpl::OnFrameTokenChanged(const FrameSinkId& frame_sink_id, uint32_t frame_token)
{
    if (client_remote_ || !ui_task_runner_) {
        ...
        ui_task_runner_->PostTask(FROM_HERE,
            base::BindOnce(&FrameSinkManagerImpl::OnFrameTokenChangedDirect, base::Unretained(this), frame_sink_id, frame_token,
                /* activation_time =*/base::TimeTicks::Now()));
    }
}

VideoDetector* FrameSinkManagerImpl::CreateVideoDetectorForTesting(const base::TickClock* tick_clock, scoped_refptr<base::SequencedTaskRunner> task_runner)
{
    DCHECK(!video_detector_);
    video_detector_ = std::make_unique<VideoDetector>(GetRegisteredFrameSinkIds(), surface_manager(), tick_clock, task_runner);
    return video_detector_.get();
}

void FrameSinkManagerImpl::DidBeginFrame(const FrameSinkId& frame_sink_id, const BeginFrameArgs& args)
{
    for (auto& observer : observer_list_)
        observer.OnFrameSinkDidBeginFrame(frame_sink_id, args);
}

void FrameSinkManagerImpl::DidFinishFrame(const FrameSinkId& frame_sink_id, const BeginFrameArgs& args)
{
    for (auto& observer : observer_list_)
...
std::vector<FrameSinkId> FrameSinkManagerImpl::GetRegisteredFrameSinkIds() const
{
...
FrameSinkId FrameSinkManagerImpl::GetOldestParentByChildFrameId(const FrameSinkId& child_frame_sink_id) const
{
...
FrameSinkId FrameSinkManagerImpl::GetOldestRootCompositorFrameSinkId(const FrameSinkId& child_frame_sink_id) const
{
    auto parent_id = GetOldestParentByChildFrameId(child_frame_sink_id);

    while (parent_id.is_valid() && root_sink_map_.find(parent_id) == root_sink_map_.end()) {
        parent_id = GetOldestParentByChildFrameId(parent_id);
    }
    return parent_id;
}

base::flat_set<FrameSinkId> FrameSinkManagerImpl::GetChildrenByParent(const FrameSinkId& parent_frame_sink_id) const
{
...
CompositorFrameSinkSupport* FrameSinkManagerImpl::GetFrameSinkForId(const FrameSinkId& frame_sink_id) const
{
...
base::TimeDelta FrameSinkManagerImpl::GetPreferredFrameIntervalForFrameSinkId(const FrameSinkId& id, mojom::CompositorFrameSinkType* type) const
{
...
void FrameSinkManagerImpl::DiscardPendingCopyOfOutputRequests(const BeginFrameSource* source)
{
...
void FrameSinkManagerImpl::VerifySandboxedThreadIds(
    const base::flat_set<base::PlatformThreadId>& thread_ids, base::OnceCallback<void(bool)> verification_callback)
{
#if BUILDFLAG(IS_ANDROID)
    ...
    client_->VerifyThreadIdsDoNotBelongToHost(tids, std::move(verification_callback));
#else
    std::move(verification_callback).Run(false);
#endif
}
...
#if BUILDFLAG(IS_ANDROID)
void FrameSinkManagerImpl::CacheBackBuffer(uint32_t cache_id, const FrameSinkId& root_frame_sink_id)
{
...
void FrameSinkManagerImpl::EvictBackBuffer(uint32_t cache_id, EvictBackBufferCallback callback)
{
...
void FrameSinkManagerImpl::UpdateDebugRendererSettings(const DebugRendererSettings& debug_settings)
{
    debug_settings_ = debug_settings;
}

void FrameSinkManagerImpl::UpdateThrottlingRecursively(const FrameSinkId& frame_sink_id, base::TimeDelta interval)
{
    auto it = support_map_.find(frame_sink_id);
    if (it != support_map_.end()) {
...
void FrameSinkManagerImpl::Throttle(const std::vector<FrameSinkId>& ids, base::TimeDelta interval)
{
...
void FrameSinkManagerImpl::StartThrottlingAllFrameSinks(base::TimeDelta interval)
{
...
void FrameSinkManagerImpl::UpdateThrottling()
{
    ...
    if (throttle_interval_.is_zero() && (!global_throttle_interval_ || global_throttle_interval_.value().is_zero()))
        return;
    ...
    if (!global_throttle_interval_ || throttle_interval_ > global_throttle_interval_) {
...
void FrameSinkManagerImpl::MaybeEraseHitTestQuery(const FrameSinkId& frame_sink_id)
{
//     if (!input::IsTransferInputToVizSupported()) {
//         return;
//     }
...
//     if (it != support_map_.end()) {
//         display_hit_test_query_[frame_sink_id] = std::make_unique<HitTestQuery>(it->second->GetHitTestAggregator()->GetDataProviderSafeRef());
//     }
    return;
}

void FrameSinkManagerImpl::CacheSurfaceAnimationManager(const blink::ViewTransitionToken& transition_token, std::unique_ptr<SurfaceAnimationManager> manager)
{
    if (transition_token_to_animation_manager_.contains(transition_token)) {
        LOG(ERROR) << "SurfaceAnimationManager already exists for |transition_token| : " << transition_token;
        return;
    }
...
std::unique_ptr<SurfaceAnimationManager> FrameSinkManagerImpl::TakeSurfaceAnimationManager(const blink::ViewTransitionToken& transition_token)
{
    auto it = transition_token_to_animation_manager_.find(transition_token);
    if (it == transition_token_to_animation_manager_.end()) {
        LOG(ERROR) << "SurfaceAnimationManager missing for |transition_token| : " << transition_token;
        return nullptr;
    }
...
bool FrameSinkManagerImpl::ClearSurfaceAnimationManager(const blink::ViewTransitionToken& transition_token)
{
    return transition_token_to_animation_manager_.erase(transition_token);
}

void FrameSinkManagerImpl::OnScreenshotCaptured(
    const blink::SameDocNavigationScreenshotDestinationToken& destination_token, std::unique_ptr<CopyOutputResult> copy_output_result)
{
    client_->OnScreenshotCaptured(destination_token, std::move(copy_output_result));
}

bool FrameSinkManagerImpl::IsFrameSinkIdInRootSinkMap(const FrameSinkId& frame_sink_id)
{
...
void FrameSinkManagerImpl::StartFrameCounting(base::TimeTicks start_time, base::TimeDelta bucket_size)
{
    ...
    for (auto& [sink_id, support] : support_map_) {
        ...
        frame_counter_->AddFrameSink(sink_id, support->frame_sink_type(), support->is_root(), GetFrameSinkDebugLabel(sink_id));
    }
}

void FrameSinkManagerImpl::StopFrameCounting(StopFrameCountingCallback callback)
{
...
void FrameSinkManagerImpl::StartOverdrawTracking(const FrameSinkId& root_frame_sink_id, base::TimeDelta bucket_size)
{
    auto iter = root_sink_map_.find(root_frame_sink_id);
    if (iter == root_sink_map_.end()) {
        LOG(ERROR) << "No RootCompositorFrameSink for root_frame_sink_id:" << root_frame_sink_id;
        return;
    }
...
void FrameSinkManagerImpl::StopOverdrawTracking(const FrameSinkId& root_frame_sink_id, StopOverdrawTrackingCallback callback)
{
    auto iter = root_sink_map_.find(root_frame_sink_id);
    if (iter == root_sink_map_.end()) {
        LOG(ERROR) << "No RootCompositorFrameSink for root_frame_sink_id:" << root_frame_sink_id;
...
void FrameSinkManagerImpl::HasUnclaimedViewTransitionResources(HasUnclaimedViewTransitionResourcesCallback callback)
{
    std::move(callback).Run(!transition_token_to_animation_manager_.empty());
}

void FrameSinkManagerImpl::SetSameDocNavigationScreenshotSize(const gfx::Size& result_size, SetSameDocNavigationScreenshotSizeCallback callback)
{
    copy_output_request_result_size_for_testing_ = result_size;
    std::move(callback).Run();
}

void FrameSinkManagerImpl::ClearUnclaimedViewTransitionResources(const blink::ViewTransitionToken& transition_token)
{
    transition_token_to_animation_manager_.erase(transition_token);
}

void FrameSinkManagerImpl::CreateMetricsRecorderForTest(mojo::PendingReceiver<mojom::FrameSinksMetricsRecorder> receiver)
{
    CHECK(!metrics_receiver_.is_bound());
    metrics_receiver_.Bind(std::move(receiver));
}

void FrameSinkManagerImpl::EnableFrameSinkManagerTestApi(mojo::PendingReceiver<mojom::FrameSinkManagerTestApi> receiver)
{
...
{
    *(int*)1 = 1;
    //input_manager_->SetupRenderInputRouterDelegateConnection(grouping_id, std::move(rir_delegate_client_remote));
}
