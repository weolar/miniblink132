...
SurfaceResourceHolder::SurfaceResourceHolder(SurfaceResourceHolderClient* client)
    : client_(client)
{
}
...
void SurfaceResourceHolder::ReceiveFromChild(const std::vector<TransferableResource>& resources)
{
...
void SurfaceResourceHolder::RefResources(const std::vector<TransferableResource>& resources)
{
    for (const auto& resource : resources) {
        ...
        DCHECK(count_it != resource_id_info_map_.end()) << "ResourceId: " << resource.id;
        count_it->second.refs_holding_resource_alive++;
    }
}

void SurfaceResourceHolder::UnrefResources(std::vector<ReturnedResource> resources)
{
