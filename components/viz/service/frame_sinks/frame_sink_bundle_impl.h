...
public:
    ...
    // `client_id`.
    FrameSinkBundleImpl(FrameSinkManagerImpl& manager, const FrameSinkBundleId& id, mojo::PendingReceiver<mojom::FrameSinkBundle> receiver,
    ...
    const FrameSinkBundleId& id() const
    {
        return id_;
    }
    ...
    void UpdateFrameSink(CompositorFrameSinkSupport* support, BeginFrameSource* old_source);
    void RemoveFrameSink(CompositorFrameSinkSupport* support);

    // mojom::FrameSinkBundle implementation:
    void InitializeCompositorFrameSinkType(uint32_t sink_id, mojom::CompositorFrameSinkType type) override;
    ...
    void Submit(std::vector<mojom::BundledFrameSubmissionPtr> submissions) override;
    void DidAllocateSharedBitmap(uint32_t sink_id, base::ReadOnlySharedMemoryRegion region, const SharedBitmapId& id) override;
#if BUILDFLAG(IS_ANDROID)
    void SetThreads(uint32_t sink_id, const std::vector<Thread>& threads) override;
#endif
    ...
    void SendOnCompositorFrameTransitionDirectiveProcessed(uint32_t sink_id, uint32_t sequence_id);
