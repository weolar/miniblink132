...
public:
    ...
    void AddFrameSink(const FrameSinkId& frame_sink_id, mojom::CompositorFrameSinkType type, bool is_root, std::string_view debug_label);

    // Add a presented frame for the frame sink.
    void AddPresentedFrame(const FrameSinkId& frame_sink_id, base::TimeTicks present_timetamp);

    // Takes the collected frame counts.
    mojom::FrameCountingDataPtr TakeData();

    // Sets a frame sink's type.
    void SetFrameSinkType(const FrameSinkId& frame_sink_id, mojom::CompositorFrameSinkType type);

    void SetFrameSinkDebugLabel(const FrameSinkId& frame_sink_id, std::string_view debug_label);
...
private:
    ...
    base::flat_map<FrameSinkId, mojom::FrameCountingPerSinkDataPtr> frame_sink_data_;
};
