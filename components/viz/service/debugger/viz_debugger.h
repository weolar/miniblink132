...
public:
    ...
    void StartDebugStream(mojo::PendingRemote<mojom::VizDebugOutput> pending_debug_output);
    void StopDebugStream();

    struct VIZ_SERVICE_EXPORT StaticSource {
        StaticSource(const char* anno_name, const char* file_name, int file_line, const char* func_name);
    ...
    void CompleteFrame(const uint64_t counter, const gfx::Size& window_pix, base::TimeTicks time_ticks);
    void Draw(const gfx::SizeF& obj_size, const gfx::Vector2dF& pos, const StaticSource* dcs, DrawOption option, int* id, const gfx::RectF& uv,
        const std::string& text);

    void AddLogMessage(std::string log, const StaticSource* dcs, DrawOption option);
...
private:
    ...
    VizDebugger();
    base::Value FrameAsJson(const uint64_t counter, const gfx::Size& window_pix, base::TimeTicks time_ticks);

    void AddFrame();
    void UpdateFilters();
    void RegisterSource(StaticSource* source);
    void DrawInternal(const gfx::SizeF& obj_size, const gfx::Vector2dF& pos, const StaticSource* dcs, DrawOption option, int* id, const gfx::RectF& uv,
        const std::string& text);
    ...
    struct DrawCall : public CallSubmitCommon {
        DrawCall() = default;
        DrawCall(int index, int source, int thread, DrawOption draw_option, gfx::SizeF size, gfx::Vector2dF position, int buffer_id, gfx::RectF uv_rect,
            std::string text_str)
    ...
    struct LogCall : public CallSubmitCommon {
        LogCall() = default;
        LogCall(int index, int source, int thread, DrawOption draw_option, std::string str)
            : CallSubmitCommon(index, source, thread, draw_option)
            , value(std::move(str))
        {
        }
    ...
    struct VIZ_SERVICE_EXPORT FilterBlock {
        FilterBlock(const std::string file_str, const std::string func_str, const std::string anno_str, bool is_active, bool is_enabled);
        ~FilterBlock();
...
#define DBG_DRAW_RECTANGLE_OPT_BUFF_UV_TEXT(anno, option, pos, size, id, uv, text)                                                                             \
    do {                                                                                                                                                       \
        if (viz::VizDebugger::IsEnabled()) {                                                                                                                   \
            static viz::VizDebugger::StaticSource dcs(anno, __FILE__, __LINE__, __func__);                                                                     \
            if (dcs.IsActive()) {                                                                                                                              \
                viz::VizDebugger::GetInstance()->Draw(size, pos, &dcs, option, id, uv, text);                                                                  \
            }                                                                                                                                                  \
        }                                                                                                                                                      \
    } while (0)
...
#define DBG_COMPLETE_BUFFERS(buff_id, buffer)                                                                                                                  \
    do {                                                                                                                                                       \
        if (viz::VizDebugger::IsEnabled()) {                                                                                                                   \
            viz::VizDebugger::GetInstance()->SubmitBuffer(buff_id, std::move(buffer));                                                                         \
        }                                                                                                                                                      \
    } while (0)
...
#define DBG_LOG_OPT(anno, option, format, ...)                                                                                                                 \
    do {                                                                                                                                                       \
        if (viz::VizDebugger::IsEnabled()) {                                                                                                                   \
            static viz::VizDebugger::StaticSource dcs(anno, __FILE__, __LINE__, __func__);                                                                     \
            if (dcs.IsActive()) {                                                                                                                              \
                viz::VizDebugger::GetInstance()->AddLogMessage(base::StringPrintf(format, ##__VA_ARGS__), &dcs, option);                                       \
            }                                                                                                                                                  \
        }                                                                                                                                                      \
    } while (0)

#define DBG_CONNECTED_OR_TRACING(is_enabled) is_enabled = VizDebugger::GetInstance()->IsEnabled();
...
public:
    ...
    inline void CompleteFrame(uint64_t counter, const gfx::Size& window_pix, base::TimeTicks time_ticks)
    {
    }
...
#define DBG_VIZ_DEBUGGER_TRACE_IMPL(anno, pos, size, text)                                                                                                     \
    TRACE_EVENT_INSTANT1(                                                                                                                                      \
        TRACE_DISABLED_BY_DEFAULT(VIZ_DEBUGGER_TRACING_CATEGORY), anno, TRACE_EVENT_FLAG_NONE, "args", viz::DrawRectToTraceValue(pos, size, text))
...
#define DBG_CONNECTED_OR_TRACING(enabled) TRACE_EVENT_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT(VIZ_DEBUGGER_TRACING_CATEGORY), (&enabled))
...
// compile paths
#define DBG_DRAW_RECTANGLE_OPT_BUFF_UV(anno, option, pos, size, id, uv) DBG_DRAW_RECTANGLE_OPT_BUFF_UV_TEXT(anno, option, pos, size, id, uv, "")

#define DBG_DRAW_RECTANGLE_OPT(anno, option, pos, size) DBG_DRAW_RECTANGLE_OPT_BUFF(anno, option, pos, size, nullptr)

#define DBG_DRAW_RECTANGLE(anno, pos, size) DBG_DRAW_RECTANGLE_OPT_BUFF(anno, DBG_OPT_BLACK, pos, size, nullptr)

#define DBG_DRAW_RECTANGLE_OPT_BUFF(anno, option, pos, size, id) DBG_DRAW_RECTANGLE_OPT_BUFF_UV(anno, option, pos, size, id, DBG_DEFAULT_UV)

#define DBG_DRAW_RECT_OPT_BUFF(anno, option, rect, id) DBG_DRAW_RECT_OPT_BUFF_UV(anno, option, rect, id, DBG_DEFAULT_UV)

#define DBG_DRAW_RECT_OPT(anno, option, rect) DBG_DRAW_RECT_OPT_BUFF(anno, option, rect, nullptr)

#define DBG_DRAW_RECT_BUFF(anno, rect, id) DBG_DRAW_RECT_OPT_BUFF(anno, DBG_OPT_BLACK, rect, id)

#define DBG_DRAW_RECT_BUFF_UV(anno, rect, id, uv) DBG_DRAW_RECT_OPT_BUFF_UV(anno, DBG_OPT_BLACK, rect, id, uv)

#define DBG_DRAW_RECT(anno, rect) DBG_DRAW_RECT_OPT(anno, DBG_OPT_BLACK, rect)

#define DBG_DRAW_TEXT_OPT(anno, option, pos, text)                                                                                                             \
    DBG_DRAW_RECTANGLE_OPT_BUFF_UV_TEXT(anno, option, gfx::Vector2dF(pos.x(), pos.y()), gfx::SizeF(-1, -1), nullptr, DBG_DEFAULT_UV, text)

#define DBG_DRAW_TEXT(anno, pos, text) DBG_DRAW_TEXT_OPT(anno, DBG_OPT_BLACK, pos, text)

#define DBG_LOG(anno, format, ...) DBG_LOG_OPT(anno, DBG_OPT_BLACK, format, ##__VA_ARGS__)

#define DBG_DRAW_RECT_OPT_BUFF_UV(anno, option, rect, id, uv)                                                                                                  \
    DBG_DRAW_RECTANGLE_OPT_BUFF_UV(anno, option, gfx::Vector2dF(rect.origin().x(), rect.origin().y()), gfx::SizeF(rect.size()), id, uv)
