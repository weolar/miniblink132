...
VizDebugger::FilterBlock::FilterBlock(const std::string file_str, const std::string func_str, const std::string anno_str, bool is_active, bool is_enabled)
...
base::Value::Dict VizDebugger::CallSubmitCommon::GetDictionaryValue() const
{
    return base::Value::Dict()
        ...
        .Set("option",
            base::Value::Dict().Set("color", base::StringPrintf("#%02x%02x%02x", option.color_r, option.color_g, option.color_b)).Set("alpha", option.color_a));
}

VizDebugger::StaticSource::StaticSource(const char* anno_name, const char* file_name, int file_line, const char* func_name)
    : anno(anno_name)
    , file(file_name)
    , func(func_name)
    , line(file_line)
...
base::Value VizDebugger::FrameAsJson(const uint64_t counter, const gfx::Size& window_pix, base::TimeTicks time_ticks)
{
    ...
    auto global_dict = base::Value::Dict()
                           ...
                           .Set("time", base::NumberToString(time_ticks.since_origin().InMicroseconds()));
    ...
    for (size_t i = last_sent_source_count_; i < sources_.size(); i++) {
        ...
        new_sources.Append(
            base::Value::Dict().Set("file", each->file).Set("line", each->line).Set("func", each->func).Set("anno", each->anno).Set("index", each->reg_index));
    }
    ...
    size_t const max_rect_calls_index = std::min(static_cast<int>(draw_calls_tail_idx_), static_cast<int>(draw_rect_calls_.size()));

    size_t const max_logs_index = std::min(static_cast<int>(logs_tail_idx_), static_cast<int>(logs_.size()));
    ...
    for (size_t i = 0; i < max_rect_calls_index; ++i) {
        ...
        dict.Set("size", base::Value::List().Append(draw_rect_calls_[i].obj_size.width()).Append(draw_rect_calls_[i].obj_size.height()));
        dict.Set("pos", base::Value::List().Append(static_cast<double>(draw_rect_calls_[i].pos.x())).Append(static_cast<double>(draw_rect_calls_[i].pos.y())));
        if (draw_rect_calls_[i].uv != DBG_DEFAULT_UV) {
            dict.Set("uv_size",
                base::Value::List().Append(static_cast<double>(draw_rect_calls_[i].uv.width())).Append(static_cast<double>(draw_rect_calls_[i].uv.height())));
            dict.Set(
                "uv_pos", base::Value::List().Append(static_cast<double>(draw_rect_calls_[i].uv.x())).Append(static_cast<double>(draw_rect_calls_[i].uv.y())));
        }
    ...
    for (auto&& each : buffers_) {
        ...
        bool result = SkPngEncoder::Encode(&stream, each.buffer_info.bitmap.pixmap(), SkPngEncoder::Options());
        if (!result) {
            DLOG(ERROR) << "encode failed";
            continue;
        }
        sk_sp<SkData> data = stream.detachAsData();
        std::string uri = "data:image/png;base64," + base::Base64Encode(base::span<const uint8_t>(static_cast<const uint8_t*>(data->data()), data->size()));
    ...
    for (auto&& thread_id : registered_threads) {
        std::string cur_thread_name = base::ThreadIdNameManager::GetInstance()->GetName(thread_id);
        new_threads.Append(base::Value::Dict().Set("thread_id", thread_id).Set("thread_name", cur_thread_name));
...
void VizDebugger::CompleteFrame(uint64_t counter, const gfx::Size& window_pix, base::TimeTicks time_ticks)
{
    if (!enabled_) {
        return;
    }
    ...
    gpu_thread_task_runner_->PostTask(FROM_HERE, base::BindOnce(&VizDebugger::AddFrame, base::Unretained(this)));
    read_write_lock_.WriteUnLock();
}
...
void VizDebugger::ApplyFilters(VizDebugger::StaticSource* src)
{
    ...
    auto simple_match = [](const char* source_str, const std::string& filter_match) {
    ...
    for (const auto& filter_block : cached_filters_) {
        if (simple_match(src->file, filter_block.file) && simple_match(src->func, filter_block.func) && simple_match(src->anno, filter_block.anno)) {
...
void VizDebugger::Draw(const gfx::SizeF& obj_size, const gfx::Vector2dF& pos, const VizDebugger::StaticSource* dcs, VizDebugger::DrawOption option, int* id,
    const gfx::RectF& uv, const std::string& text)
{
    DrawInternal(obj_size, pos, dcs, option, id, uv, text);
}

void VizDebugger::DrawInternal(const gfx::SizeF& obj_size, const gfx::Vector2dF& pos, const VizDebugger::StaticSource* dcs, VizDebugger::DrawOption option,
    int* id, const gfx::RectF& uv, const std::string& text)
{
    ...
    for (;;) {
        ...
        if (static_cast<size_t>(insertion_index) < draw_rect_calls_.size()) {
            ...
            draw_rect_calls_[insertion_index]
                = DrawCall { submission_count_++, dcs->reg_index, cur_thread_id, option, obj_size, pos, local_id_buffer, uv, text };
...
void VizDebugger::FilterDebugStream(base::Value::Dict json)
{
    ...
    for (const auto& entry : *filters) {
        ...
        if ((file && !file->is_string()) || (func && !func->is_string()) || (anno && !anno->is_string()) || !active->is_bool()) {
            LOG(ERROR) << "Filter props wrong type in json: " << json;
            continue;
        }

        auto check_str = [](const base::Value* filter_str) { return (filter_str ? filter_str->GetString() : std::string()); };

        std::optional<bool> enabled = filter.FindBool("enabled");
        new_filters_.emplace_back(check_str(file), check_str(func), check_str(anno), active->GetBool(), enabled.value_or(true));
    }
...
void VizDebugger::StartDebugStream(mojo::PendingRemote<mojom::VizDebugOutput> pending_debug_output)
{
    read_write_lock_.WriteLock();
    DCHECK(gpu_thread_task_runner_->RunsTasksInCurrentSequence());
    ...
    debug_output_->LogFrame(base::Value(base::Value::Dict().Set("connection", "ok")));
    enabled_.store(true);
    read_write_lock_.WriteUnLock();
}
...
void VizDebugger::AddLogMessage(std::string log, const VizDebugger::StaticSource* dcs, DrawOption option)
{
    //  Store atomic insertion index in local variable to use to insert into
    //  buffer.
    int insertion_index;
    ...
    for (;;) {
        ...
        if (static_cast<size_t>(insertion_index) < logs_.size()) {
            int cur_thread_id = base::PlatformThread::CurrentId();
            logs_[insertion_index] = LogCall { submission_count_++, dcs->reg_index, cur_thread_id, option, std::move(log) };
            // Return when call insertion is successful.
            read_write_lock_.ReadUnlock();
            return;
        }
...
std::unique_ptr<base::trace_event::ConvertableToTraceFormat> DrawRectToTraceValue(const gfx::Vector2dF& pos, const gfx::SizeF& size, const std::string& text)
{
