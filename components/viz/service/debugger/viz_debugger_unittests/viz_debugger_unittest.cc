...
TEST_F(VisualDebuggerTest, GeneralDrawSubmission)
{
    ...
    for (uint64_t frame_idx = 0; frame_idx < kNumFrames; frame_idx++) {
        ...
        for (int i = 0; i < kNumSubmission; i++) {
            ...
            DBG_DRAW_TEXT(kAnnoText, kTestRect.origin(), base::StringPrintf("Text %d", i));
        ...
        for (int i = 0; i < kNumSubmission; i++) {
            ...
            EXPECT_EQ(draw_calls_cache_[i * 2].pos, gfx::Vector2dF(kTestRect.origin().x(), kTestRect.origin().y()));
            ...
            EXPECT_EQ(draw_calls_cache_[i * 2 + 1].pos, gfx::Vector2dF(kTestRect.origin().x(), kTestRect.origin().y()));
            EXPECT_EQ(draw_calls_cache_[i * 2 + 1].source_index, 1);
            EXPECT_EQ(draw_calls_cache_[i * 2 + 1].draw_index, i * 3 + 1);
            EXPECT_EQ(draw_calls_cache_[i * 2 + 1].text, base::StringPrintf("Text %d", i));
...
TEST_F(VisualDebuggerTest, FilterDrawSubmission)
{
    ...
    auto check_draw = [](const VizDebuggerInternal::DrawCall& draw_call, const gfx::RectF& rect, int src_idx, int draw_idx) {
        EXPECT_EQ(draw_call.pos, gfx::Vector2dF(rect.origin().x(), rect.origin().y()));
...
TEST_F(VisualDebuggerTest, SingleBufferSync)
{
    ...
    buffer_info.bitmap.setInfo(SkImageInfo::MakeN32(kBufferWidth, kBufferHeight, kUnpremul_SkAlphaType));
...
TEST_F(VisualDebuggerTest, MultipleBuffersAsync)
{
    ...
    for (uint64_t frame_idx = 0; frame_idx < kNumFrames; frame_idx++) {
        ...
        for (int i = 0; i < kNumSubmission; i++) {
            ...
            uint8_t temp[4];
            base::RandBytes(temp);
            const auto kFillColor = SkColorSetARGB(temp[0], temp[1], temp[2], temp[3]);
            ...
            for (int i = 0; i < kNumSubmission; i++) {
                ...
                for (int jj = 0; jj < pixmap.height(); jj++) {
                    for (int ii = 0; ii < pixmap.width(); ii++) {
                        EXPECT_EQ(test_buffers_color[buffers_[i].id], pixmap.getColor(ii, jj));
                    }
                }
            }
        }
...
static_assert(flag_default_value_check() == false, "Default value when debugger is disabled is false.");
