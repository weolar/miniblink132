...
public:
    ...
    void ThreadMain() override
    {
        ...
        for (int i = 0; static_cast<uint32_t>(i) < thread_test_config_.dbg_commands[0].size(); ++i) {
            ...
            if (thread_test_config_.dbg_commands[0][i] == VizDebugCommandType::kDbgDrawRectCommand) {
                DBG_DRAW_RECTANGLE_OPT_BUFF_UV_TEXT(
                    "multithread rect", DBG_OPT_BLACK, testRect.OffsetFromOrigin(), testRect.size(), nullptr, DBG_DEFAULT_UV, "text");
...
public:
    ...
    void Init(VizDebuggerMultithreadTest* test_fixture_ptr, uint32_t num_writer_tries, uint32_t spin_amount)
    {
        spin_amount_ = spin_amount;
    ...
    void ThreadMain() override
    {
        for (uint32_t writer_try = 0; writer_try < num_writer_tries_; ++writer_try) {
...
TEST_F(VizDebuggerMultithreadTest, kReadersTest)
{
    ...
    uint32_t kNumCommandsPerFrame = kNumDrawRectCommandsPerFrame + kNumLogCommandsPerFrame;
    ...
    for (uint32_t i = 0; i < kNumDrawRectCommandsPerFrame; ++i) {
        test_config.dbg_commands[0].push_back(VizDebugCommandType::kDbgDrawRectCommand);
    }
    ...
    std::vector<VizDebuggerInternal::DrawCall> rect_calls_result = GetInternal()->GetDrawRectCalls();
    std::vector<VizDebuggerInternal::LogCall> logs_result = GetInternal()->GetLogs();

    size_t const kNumDrawCallSubmission = static_cast<size_t>(std::min(GetInternal()->GetRectCallsTailIdx(), GetInternal()->GetRectCallsSize()));
    size_t const kNumLogSubmission = static_cast<size_t>(std::min(GetInternal()->GetLogsTailIdx(), GetInternal()->GetLogsSize()));
    ...
    for (size_t i = 0; i < kNumDrawCallSubmission; ++i) {
        ...
        for (auto&& thread : threads) {
            ...
            if (static_cast<uint32_t>(thread.rect_command_idx) >= thread.draw_rect_command_history.size()) {
                continue;
            }
            DbgCommandIdentifier cur_thread_command_identifier = thread.draw_rect_command_history[thread.rect_command_idx];

            // Check if debug command ID matches. Refer to ReaderTestThread's
            // ThreadMain() function to see how command ID's are formatted.
            if (rect_calls_result[i].pos.x() == cur_thread_command_identifier.dbg_command_thread_id
                && rect_calls_result[i].pos.y() == cur_thread_command_identifier.dbg_command_id) {
    ...
    for (size_t i = 0; i < kNumLogSubmission; ++i) {
        ...
        for (auto&& thread : threads) {
            // If debug calls from a thread is exhausted, skip this thread.
            if (static_cast<uint32_t>(thread.log_command_idx) >= thread.log_command_history.size()) {
                continue;
            }
            DbgCommandIdentifier cur_thread_command_identifier = thread.log_command_history[thread.log_command_idx];
            ...
            int log_thread_id;
            int log_command_id;
            base::StringToInt(log_result_identifier.substr(0, log_split_position), &log_thread_id);
            base::StringToInt(log_result_identifier.substr(log_split_position + 1), &log_command_id);

            // Check if debug command ID matches.
            if (log_thread_id == cur_thread_command_identifier.dbg_command_thread_id && log_command_id == cur_thread_command_identifier.dbg_command_id) {
                valid_command_found = true;
...
TEST_F(VizDebuggerMultithreadTest, kReadersOneWriterCommandsSequenceTest)
{
    ...
    for (uint32_t i = 0; i < kNumReaderThreads; ++i) {
        ASSERT_TRUE(base::PlatformThread::Create(0, &reader_threads[i], &reader_thread_handles[i]));
    }
    writer_thread.Init(this, kNumWriterTries, kWriterSpinAmount);
    ASSERT_TRUE(base::PlatformThread::Create(0, &writer_thread, &writer_thread_handle));
    ...
    GetFrameData(false);
    int expected_submission_count = kNumCommandsPerFrame * kNumReaderThreads;
    int calls_cache_total_size = draw_calls_cache_.size() + log_calls_cache_.size();
    ...
    for (auto&& rect_call : draw_calls_cache_) {
        ...
        for (auto&& thread : reader_threads) {
            ...
            // Check if debug command ID matches. Refer to ReaderTestThread's
            // ThreadMain() function to see how command ID's are formatted.
            if (rect_call.pos.x() == cur_thread_command_identifier.dbg_command_thread_id && rect_call.pos.y() == cur_thread_command_identifier.dbg_command_id) {
                valid_command_found = true;
                ++thread.rect_command_idx;
                break;
            }
