...
TestFilter::TestFilter(const std::string& anno_, const std::string& func_, const std::string& file_)
    : anno(anno_)
    , func(func_)
    , file(file_)
{
...
TestFilter::TestFilter(const std::string& anno_, const std::string& func_, const std::string& file_, const bool& active_, const bool& enabled_)
    : anno(anno_)
    , func(func_)
    , file(file_)
    , active(active_)
...
void VisualDebuggerTestBase::SetFilter(std::vector<TestFilter> filters)
{
    ...
    for (auto&& each : filters) {
        ...
        filters_list.Append(base::Value::Dict().Set("selector", std::move(selector)).Set("active", each.active).Set("enabled", each.enabled));
    }

    GetInternal()->FilterDebugStream(base::Value::Dict().Set("filters", std::move(filters_list)));
...
void VisualDebuggerTestBase::GetFrameData(bool clear_cache)
{
    ...
    std::optional<base::Value> maybe_global_dict_val = GetInternal()->FrameAsJson(frame_counter_, gfx::Size(window_x_, window_y_), base::TimeTicks());
    ...
    auto func_common_call = [](const base::Value::Dict& dict, int* draw_index, int* source_index, int* thread_id, VizDebugger::DrawOption* option) {
        ...
        *thread_id = dict.FindInt("thread_id").value_or(kNoVal);
        ...
        uint32_t green;
        uint32_t blue;
        std::sscanf(option_dict->FindString("color")->c_str(), "#%x%x%x", &red, &green, &blue);

        option->color_r = red;
        option->color_g = green;
        option->color_b = blue;
        option->color_a = static_cast<uint8_t>(option_dict->FindInt("alpha").value_or(kNoVal));
    };
    ...
    for (size_t i = 0; i < kNumDrawCallSubmission; i++) {
        ...
        func_common_call(local_dict, &draw_index, &source_index, &thread_id, &option);
        ...
        VizDebuggerInternal::DrawCall draw_call(draw_index, source_index, thread_id, option, gfx::SizeF(size_x, size_y), gfx::Vector2dF(pos_x, pos_y),
            buffer_id ? buffer_id.value() : -1, gfx::RectF(uv_pos_x, uv_pos_y, uv_size_w, uv_size_h), text_str ? (*text_str) : std::string());
    ...
    if (buffer_map_dict) {
        for (base::Value::Dict::const_iterator itr = buffer_map_dict->begin(); itr != buffer_map_dict->end(); itr++) {
            ...
            std::string_view image_base64_encoded = std::string_view(image_data_uri).substr(strlen(kDataUriPrefix));
            const std::optional<std::vector<uint8_t>> image_bytes = base::Base64Decode(image_base64_encoded);
            ...
            sk_sp<SkData> data = SkData::MakeWithoutCopy(image_bytes->data(), image_bytes->size());
            SkCodec::Result decode_result;
            std::unique_ptr<SkCodec> codec = SkPngDecoder::Decode(data, &decode_result);
            EXPECT_EQ(SkCodec::Result::kSuccess, decode_result);

            VizDebuggerInternal::BufferInfo buff;
            buff.bitmap.allocPixels(codec->getInfo());
            const SkCodec::Result read_result = codec->getPixels(buff.bitmap.pixmap());
    ...
    for (size_t i = 0; i < kNumLogSubmission; i++) {
        ...
        VizDebuggerInternal::LogCall log_call(draw_index, source_index, thread_id, option, *local_dict.FindString("value"));
