...
public:
    ...
    HitTestAggregator(const HitTestManager* hit_test_manager, HitTestAggregatorDelegate* delegate,
        LatestLocalSurfaceIdLookupDelegate* local_surface_id_lookup_delegate, const FrameSinkId& frame_sink_id, uint32_t initial_region_size = 100,
...
private:
    ...
    void SetRegionAt(size_t index, const FrameSinkId& frame_sink_id, uint32_t flags, uint32_t reasons, const gfx::Rect& rect, const gfx::Transform& transform,
        int32_t child_count);
    ...
    std::optional<int64_t> GetTraceIdIfUpdated(const SurfaceId& surface_id, uint64_t active_frame_index);

    const raw_ptr<const HitTestManager> hit_test_manager_;

    const raw_ptr<HitTestAggregatorDelegate> delegate_;

    const raw_ptr<LatestLocalSurfaceIdLookupDelegate, AcrossTasksDanglingUntriaged> local_surface_id_lookup_delegate_;
