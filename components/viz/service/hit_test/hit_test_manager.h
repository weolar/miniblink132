...
public:
    ...
    void SubmitHitTestRegionList(const SurfaceId& surface_id, const uint64_t frame_index, std::optional<HitTestRegionList> hit_test_region_list);
    ...
    const HitTestRegionList* GetActiveHitTestRegionList(
        LatestLocalSurfaceIdLookupDelegate* delegate, const FrameSinkId& frame_sink_id, uint64_t* store_active_frame_index = nullptr) const;
...
private:
    bool ValidateHitTestRegionList(const SurfaceId& surface_id, HitTestRegionList* hit_test_region_list);

    const raw_ptr<SurfaceManager> surface_manager_;

    std::map<SurfaceId, base::flat_map<uint64_t, HitTestRegionList>> hit_test_region_lists_;
