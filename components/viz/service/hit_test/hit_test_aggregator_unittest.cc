...
public:
    ...
    const std::vector<AggregatedHitTestRegion>& regions()
    {
        return active_list_;
    }

    const FrameSinkId& buffer_frame_sink_id()
    {
        return buffer_frame_sink_id_;
    }
...
public:
    explicit TestFrameSinkManagerImpl(SharedBitmapManager* shared_bitmap_manager)
        : FrameSinkManagerImpl(FrameSinkManagerImpl::InitParams(shared_bitmap_manager))
    {
    }
    ...
    void OnAggregatedHitTestRegionListUpdated(const FrameSinkId& frame_sink_id, const std::vector<AggregatedHitTestRegion>& hit_test_data) override
    {
        ...
        if (host_client_) {
            host_client_->OnAggregatedHitTestRegionListUpdated(frame_sink_id, hit_test_data);
        }
    }
...
public:
    TestHitTestAggregator(const HitTestManager* manager, HitTestAggregatorDelegate* delegate,
        LatestLocalSurfaceIdLookupDelegate* local_surface_id_lookup_delegate, const FrameSinkId& frame_sink_id)
        : HitTestAggregator(manager, delegate, local_surface_id_lookup_delegate, frame_sink_id)
    ...
    {
        return hit_test_data_size_;
    }
    int GetHitTestRegionListCapacity()
    {
        return hit_test_data_capacity_;
    }
...
public:
    ...
    void SetUp() override
    {
        frame_sink_manager_ = std::make_unique<TestFrameSinkManagerImpl>(&shared_bitmap_manager_);
        host_frame_sink_manager_ = std::make_unique<TestHostFrameSinkManager>();
        local_surface_id_lookup_delegate_ = std::make_unique<TestLatestLocalSurfaceIdLookupDelegate>();
        frame_sink_manager_->SetLocalClient(host_frame_sink_manager_.get());
        support_ = std::make_unique<CompositorFrameSinkSupport>(nullptr /* client */, frame_sink_manager_.get(), kDisplayFrameSink, true /* is_root */);
        hit_test_aggregator_
            = std::make_unique<TestHitTestAggregator>(hit_test_manager(), frame_sink_manager(), local_surface_id_lookup_delegate(), kDisplayFrameSink);
    }
    ...
    int CreateAndSubmitHitTestRegionListWith8Children(uint32_t client_id, int depth)
    {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (depth > 0) {
                ...
                client_id = CreateAndSubmitHitTestRegionListWith8Children(client_id, depth - 1);
            } else {
        ...
        if (surface_id.frame_sink_id() == kDisplayFrameSink) {
            support()->SubmitCompositorFrame(surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(hit_test_region_list));
            local_surface_id_lookup_delegate()->SetSurfaceIdMap(surface_id);
        } else {
            auto support = std::make_unique<CompositorFrameSinkSupport>(nullptr, frame_sink_manager(), surface_id.frame_sink_id(), false /* is_root */);
            support->SubmitCompositorFrame(surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(hit_test_region_list));
            local_surface_id_lookup_delegate()->SetSurfaceIdMap(surface_id);
        }
        return client_id;
    ...
    {
        return allocator_set_.MakeSurfaceId(FrameSinkId(frame_sink_id_client_id, 0), 1);
    }
...
protected:
    ...
    TestLatestLocalSurfaceIdLookupDelegate* local_surface_id_lookup_delegate() const
    {
...
private:
    ...
    std::unique_ptr<TestLatestLocalSurfaceIdLookupDelegate> local_surface_id_lookup_delegate_;
...
TEST_F(HitTestAggregatorTest, OneSurface)
{
    ...
    support()->SubmitCompositorFrame(display_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(hit_test_region_list));
...
TEST_F(HitTestAggregatorTest, OneEmbedderTwoRegions)
{
    ...
    support()->SubmitCompositorFrame(e_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(e_hit_test_region_list));
...
TEST_F(HitTestAggregatorTest, OneEmbedderTwoChildren)
{
    ...
    auto support2 = std::make_unique<CompositorFrameSinkSupport>(nullptr, frame_sink_manager(), c1_surface_id.frame_sink_id(), false /* is_root */);
    support2->SubmitCompositorFrame(c1_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(c1_hit_test_region_list));
    local_surface_id_lookup_delegate()->SetSurfaceIdMap(c1_surface_id);
    support()->SubmitCompositorFrame(e_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(e_hit_test_region_list));
    local_surface_id_lookup_delegate()->SetSurfaceIdMap(e_surface_id);
    auto support3 = std::make_unique<CompositorFrameSinkSupport>(nullptr, frame_sink_manager(), c2_surface_id.frame_sink_id(), false /* is_root */);
    support3->SubmitCompositorFrame(c2_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(c2_hit_test_region_list));
...
TEST_F(HitTestAggregatorTest, OccludedChildFrame)
{
    ...
    // Submit in unexpected order.
    auto support2 = std::make_unique<CompositorFrameSinkSupport>(nullptr, frame_sink_manager(), c_surface_id.frame_sink_id(), false /* is_root */);
    support2->SubmitCompositorFrame(c_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(c_hit_test_region_list));
    ...
    region = host_regions()[2];
    EXPECT_EQ(HitTestRegionFlags::kHitTestChildSurface | HitTestRegionFlags::kHitTestMine, region.flags);
...
TEST_F(HitTestAggregatorTest, ClippedChildWithTabAndTransparentBackground)
{
    ...
    support2->SubmitCompositorFrame(c_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(c_hit_test_region_list));
    local_surface_id_lookup_delegate()->SetSurfaceIdMap(c_surface_id);
    auto support3 = std::make_unique<CompositorFrameSinkSupport>(nullptr, frame_sink_manager(), a_surface_id.frame_sink_id(), false /* is_root */);
    support3->SubmitCompositorFrame(a_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(a_hit_test_region_list));
    local_surface_id_lookup_delegate()->SetSurfaceIdMap(a_surface_id);
    auto support4 = std::make_unique<CompositorFrameSinkSupport>(nullptr, frame_sink_manager(), b_surface_id.frame_sink_id(), false /* is_root */);
    support4->SubmitCompositorFrame(b_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(b_hit_test_region_list));
    ...
    region = host_regions()[1];
    EXPECT_EQ(HitTestRegionFlags::kHitTestChildSurface | HitTestRegionFlags::kHitTestIgnore, region.flags);
    EXPECT_EQ(region.frame_sink_id, c_surface_id.frame_sink_id());
    EXPECT_EQ(region.rect, gfx::Rect(300, 100, 1600, 800));
    EXPECT_EQ(region.child_count, 2);

    EXPECT_EQ(gfx::Point(100, 200), region.transform.InverseMapPoint(gfx::Point(300, 300)));
...
TEST_F(HitTestAggregatorTest, ThreeChildrenDeep)
{
    ...
    support2->SubmitCompositorFrame(c1_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(c1_hit_test_region_list));
    local_surface_id_lookup_delegate()->SetSurfaceIdMap(c1_surface_id);
    auto support3 = std::make_unique<CompositorFrameSinkSupport>(nullptr, frame_sink_manager(), c3_surface_id.frame_sink_id(), false /* is_root */);
    support3->SubmitCompositorFrame(c3_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(c3_hit_test_region_list));
    local_surface_id_lookup_delegate()->SetSurfaceIdMap(c3_surface_id);
    support()->SubmitCompositorFrame(e_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(e_hit_test_region_list));
    local_surface_id_lookup_delegate()->SetSurfaceIdMap(e_surface_id);
    auto support4 = std::make_unique<CompositorFrameSinkSupport>(nullptr, frame_sink_manager(), c2_surface_id.frame_sink_id(), false /* is_root */);
    support4->SubmitCompositorFrame(c2_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(c2_hit_test_region_list));
    local_surface_id_lookup_delegate()->SetSurfaceIdMap(c2_surface_id);
...
TEST_F(HitTestAggregatorTest, MissingChildFrame)
{
    ...
    region = host_regions()[1];
    EXPECT_EQ(region.flags, HitTestRegionFlags::kHitTestChildSurface | HitTestRegionFlags::kHitTestAsk | HitTestRegionFlags::kHitTestNotActive);
    EXPECT_EQ(region.frame_sink_id, c_surface_id.frame_sink_id());
    EXPECT_EQ(region.rect, gfx::Rect(100, 100, 200, 500));
    EXPECT_EQ(region.child_count, 0);
...
TEST_F(HitTestAggregatorTest, DiscardedSurfaces)
{
    ...
    EXPECT_FALSE(hit_test_manager()->GetActiveHitTestRegionList(local_surface_id_lookup_delegate(), e_surface_id.frame_sink_id()));
    EXPECT_FALSE(hit_test_manager()->GetActiveHitTestRegionList(local_surface_id_lookup_delegate(), c_surface_id.frame_sink_id()));
    ...
    EXPECT_TRUE(hit_test_manager()->GetActiveHitTestRegionList(local_surface_id_lookup_delegate(), e_surface_id.frame_sink_id()));
    EXPECT_TRUE(hit_test_manager()->GetActiveHitTestRegionList(local_surface_id_lookup_delegate(), c_surface_id.frame_sink_id()));
...
TEST_F(HitTestAggregatorTest, TransparentOverlayRegions)
{
    ...
    support3->SubmitCompositorFrame(c2_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(c2_hit_test_region_list));
    local_surface_id_lookup_delegate()->SetSurfaceIdMap(c2_surface_id);
    auto support4 = std::make_unique<CompositorFrameSinkSupport>(nullptr, frame_sink_manager(), d1_surface_id.frame_sink_id(), false /* is_root */);
    support4->SubmitCompositorFrame(d1_surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(d1_hit_test_region_list));
...
TEST_F(HitTestAggregatorTest, HitTestDataNotUpdated)
{
    ...
    support()->SubmitCompositorFrame(surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::nullopt);
    aggregator->Aggregate(surface_id);
    ...
    support()->SubmitCompositorFrame(surface_id.local_surface_id(), MakeDefaultCompositorFrame(), std::move(hit_test_region_list_copy));
    aggregator->Aggregate(surface_id);
