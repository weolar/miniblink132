...
bool FlagsAndAsyncReasonsMatch(uint32_t flags, uint32_t async_hit_test_reasons)
{
...
void HitTestManager::SubmitHitTestRegionList(const SurfaceId& surface_id, const uint64_t frame_index, std::optional<HitTestRegionList> hit_test_region_list)
{
    if (!hit_test_region_list) {
        ...
        if (!frame_index_map.empty()) {
            ...
            HitTestRegionList last_hit_test_region_list = std::move(frame_index_map[last_frame_index]);
    ...
    hit_test_region_lists_[surface_id][frame_index] = std::move(*hit_test_region_list);
}

const HitTestRegionList* HitTestManager::GetActiveHitTestRegionList(
    LatestLocalSurfaceIdLookupDelegate* delegate, const FrameSinkId& frame_sink_id, uint64_t* store_active_frame_index) const
{
    if (!delegate)
        return nullptr;

    LocalSurfaceId local_surface_id = delegate->GetSurfaceAtAggregation(frame_sink_id);
...
bool HitTestManager::ValidateHitTestRegionList(const SurfaceId& surface_id, HitTestRegionList* hit_test_region_list)
{
    if (hit_test_region_list->regions.size() > kMaxRegionsPerSurface)
        return false;
    if (!FlagsAndAsyncReasonsMatch(hit_test_region_list->flags, hit_test_region_list->async_hit_test_reasons)) {
        return false;
    }
    for (auto& region : hit_test_region_list->regions) {
        ...
        if (region.frame_sink_id.client_id() == 0) {
            region.frame_sink_id = FrameSinkId(surface_id.frame_sink_id().client_id(), region.frame_sink_id.sink_id());
        }
