...
HitTestAggregator::HitTestAggregator(const HitTestManager* hit_test_manager, HitTestAggregatorDelegate* delegate,
    LatestLocalSurfaceIdLookupDelegate* local_surface_id_lookup_delegate, const FrameSinkId& frame_sink_id, uint32_t initial_region_size,
...
void HitTestAggregator::Aggregate(const SurfaceId& display_surface_id)
{
    ...
    uint64_t submit_hit_test_region_list_index = hit_test_manager_->submit_hit_test_region_list_index();

    if (submit_hit_test_region_list_index == last_submit_hit_test_region_list_index_) {
        return;
    }
...
const std::vector<AggregatedHitTestRegion>& HitTestAggregator::GetHitTestData() const
{
...
void HitTestAggregator::SendHitTestData()
{
    ...
    delegate_->OnAggregatedHitTestRegionListUpdated(root_frame_sink_id_, hit_test_data_);
}

std::optional<int64_t> HitTestAggregator::GetTraceIdIfUpdated(const SurfaceId& surface_id, uint64_t active_frame_index)
{
    bool enabled;
    TRACE_EVENT_CATEGORY_GROUP_ENABLED(TRACE_DISABLED_BY_DEFAULT("viz.hit_testing_flow"), &enabled);
    if (!enabled)
        return std::nullopt;
...
void HitTestAggregator::AppendRoot(const SurfaceId& surface_id)
{
    ...
    const HitTestRegionList* hit_test_region_list
        = hit_test_manager_->GetActiveHitTestRegionList(local_surface_id_lookup_delegate_, surface_id.frame_sink_id(), &active_frame_index);

    if (!hit_test_region_list)
        return;

    std::optional<int64_t> trace_id = GetTraceIdIfUpdated(surface_id, active_frame_index);
    TRACE_EVENT_WITH_FLOW1(TRACE_DISABLED_BY_DEFAULT("viz.hit_testing_flow"), "Event.Pipeline", TRACE_ID_GLOBAL(trace_id.value_or(-1)),
        trace_id ? TRACE_EVENT_FLAG_FLOW_IN : TRACE_EVENT_FLAG_NONE, "step", "AggregateHitTestData(Root)");
    ...
    SetRegionAt(0, surface_id.frame_sink_id(), hit_test_region_list->flags, hit_test_region_list->async_hit_test_reasons, hit_test_region_list->bounds,
        hit_test_region_list->transform, child_count);
}

size_t HitTestAggregator::AppendRegion(size_t region_index, const HitTestRegion& region)
{
    ...
    if (region.flags & HitTestRegionFlags::kHitTestChildSurface) {
        ...
        uint64_t active_frame_index;
        const HitTestRegionList* hit_test_region_list
            = hit_test_manager_->GetActiveHitTestRegionList(local_surface_id_lookup_delegate_, region.frame_sink_id, &active_frame_index);
        if (!hit_test_region_list) {
            ...
            flags |= (HitTestRegionFlags::kHitTestAsk | HitTestRegionFlags::kHitTestNotActive);
            ...
            if (enabled) {
                ...
                LocalSurfaceId local_surface_id = local_surface_id_lookup_delegate_->GetSurfaceAtAggregation(region.frame_sink_id);
                SurfaceId surface_id(region.frame_sink_id, local_surface_id);

                std::optional<int64_t> trace_id = GetTraceIdIfUpdated(surface_id, active_frame_index);
                TRACE_EVENT_WITH_FLOW1(TRACE_DISABLED_BY_DEFAULT("viz.hit_testing_flow"), "Event.Pipeline", TRACE_ID_GLOBAL(trace_id.value_or(-1)),
                    trace_id ? TRACE_EVENT_FLAG_FLOW_IN : TRACE_EVENT_FLAG_NONE, "step", "AggregateHitTestData");
            }
    ...
    SetRegionAt(parent_index, region.frame_sink_id, flags, reasons, region.rect, transform, child_count);
    return region_index;
}

void HitTestAggregator::SetRegionAt(size_t index, const FrameSinkId& frame_sink_id, uint32_t flags, uint32_t async_hit_test_reasons, const gfx::Rect& rect,
    const gfx::Transform& transform, int32_t child_count)
{
    hit_test_data_[index] = AggregatedHitTestRegion(frame_sink_id, flags, rect, transform, child_count, async_hit_test_reasons);
    hit_test_data_size_++;
}
