    ...
    , gpu_task_scheduler_(std::make_unique<gpu::GpuTaskSchedulerHelper>(skia_dependency_->CreateSequence()))
{
    ...
    shared_image_interface_ = base::MakeRefCounted<gpu::SharedImageInterfaceInProcess>(gpu_task_scheduler_->GetTaskSequence(), controller_on_gpu_.get());
}
...
DisplayCompositorMemoryAndTaskController::~DisplayCompositorMemoryAndTaskController()
{
    ...
    gpu_task_scheduler_->GetTaskSequence()->ScheduleTask(std::move(callback), /*sync_token_fences=*/ {}, gpu::SyncToken());
    event.Wait();
}
...
{
    DCHECK(event);
    controller_on_gpu_ = std::make_unique<gpu::DisplayCompositorMemoryAndTaskControllerOnGpu>(skia_dependency->GetSharedContextState(),
        skia_dependency->GetSharedImageManager(), skia_dependency->GetSyncPointManager(), skia_dependency->GetGpuPreferences(),
        skia_dependency->GetGpuDriverBugWorkarounds(), skia_dependency->GetGpuFeatureInfo());
    event->Signal();
}
