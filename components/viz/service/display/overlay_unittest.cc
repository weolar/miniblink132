...
public:
    ...
    void CheckOverlaySupportImpl(const PrimaryPlane* primary_plane, OverlayCandidateList* surfaces) override
    {
    }
...
{
    if (candidate.resource_size_in_pixels.IsEmpty() || candidate.uv_rect.IsEmpty()) {
        return 1.0f;
    }
    return std::min(candidate.display_rect.width() / (candidate.uv_rect.width() * candidate.resource_size_in_pixels.width()),
        candidate.display_rect.height() / (candidate.uv_rect.height() * candidate.resource_size_in_pixels.height()));
}

class DefaultOverlayProcessor : public TestOverlayProcessor {
public:
    DefaultOverlayProcessor()
        : expected_rects_(1, gfx::RectF(kOverlayRect))
    {
    }
    ...
    void CheckOverlaySupportImpl(const PrimaryPlane* primary_plane, OverlayCandidateList* surfaces) override
    {
        ...
        if (!scaling_sequence_.empty()) {
            ...
            EXPECT_LE(std::abs(scaling - scaling_sequence_.front().first), kAbsoluteError);
        ...
        for (const auto& r : expected_rects_) {
            ...
            if (std::abs(r.x() - candidate.display_rect.x()) <= kAbsoluteError && std::abs(r.y() - candidate.display_rect.y()) <= kAbsoluteError
                && std::abs(r.width() - candidate.display_rect.width()) <= kAbsoluteError
                && std::abs(r.height() - candidate.display_rect.height()) <= kAbsoluteError) {
                EXPECT_RECTF_EQ(BoundingRect(kUVTopLeft, kUVBottomRight), candidate.uv_rect);
...
public:
    ...
    void ProcessForOverlays(DisplayResourceProvider* resource_provider, AggregatedRenderPassList* render_passes, const SkM44& output_color_matrix,
        const FilterOperationsMap& render_pass_filters, const FilterOperationsMap& render_pass_backdrop_filters, SurfaceDamageRectList surface_damage_rect_list,
        OutputSurfaceOverlayPlane* output_surface_plane, CandidateList* overlay_candidates, gfx::Rect* damage_rect,
        std::vector<gfx::Rect>* content_bounds) override
    {
        ...
        OverlayProcessorUsingStrategy::ProcessForOverlays(resource_provider, render_passes, output_color_matrix, render_pass_filters,
            render_pass_backdrop_filters, surface_damage_rect_list, output_surface_plane, overlay_candidates, damage_rect, content_bounds);
    }
    ...
    void CheckOverlaySupportImpl(const PrimaryPlane* primary_plane, OverlayCandidateList* surfaces) override
    {
        ...
        for (size_t i = 0; i < surfaces->size(); ++i) {
            ...
            EXPECT_EQ(gfx::ToEnclosedRect(candidate.display_rect), expected_rects_[i]);
    ...
    void SortProposedOverlayCandidates(std::vector<OverlayProposedCandidate>* proposed_candidates) override
    {
        ...
        std::stable_sort(proposed_candidates->begin(), proposed_candidates->end(), [](const auto& a, const auto& b) {
            ...
            if (a.candidate.has_rounded_display_masks != b.candidate.has_rounded_display_masks) {
                return a.candidate.has_rounded_display_masks > b.candidate.has_rounded_display_masks;
            }
...
public:
    ...
    void SortProposedOverlayCandidates(std::vector<OverlayProposedCandidate>* proposed_candidates) override
    {
        // We want the power gains to be assigned for the OverlayCombinationCache.
        for (auto& proposed_candidate : *proposed_candidates) {
            proposed_candidate.relative_power_gain = proposed_candidate.candidate.display_rect.size().GetArea();
        }
        std::sort(proposed_candidates->begin(), proposed_candidates->end(),
            [](const auto& a, const auto& b) { return a.relative_power_gain > b.relative_power_gain; });
    }
...
class SizeSortedMultiOverlayProcessor : public SizeSortedMultiOverlayProcessorBase {
public:
...
class SizeSortedMultiUnderlayProcessor : public SizeSortedMultiOverlayProcessorBase {
public:
...
class TypeAndSizeSortedMultiOverlayProcessor : public MultiOverlayProcessorBase {
public:
    ...
    void SortProposedOverlayCandidates(std::vector<OverlayProposedCandidate>* proposed_candidates) override
    {
        ...
        std::sort(proposed_candidates->begin(), proposed_candidates->end(), [](const auto& a, const auto& b) {
            if (a.candidate.requires_overlay || b.candidate.requires_overlay) {
                return a.candidate.requires_overlay && !b.candidate.requires_overlay;
            }
...
// sorting them.
class AllowCandidateWithMasksSortedMultiOverlayProcessor : public MultiOverlayProcessorBase {
public:
    ...
    void SortProposedOverlayCandidates(std::vector<OverlayProposedCandidate>* proposed_candidates) override
    {
        ...
        // masks.
        std::erase_if(*proposed_candidates, [](OverlayProposedCandidate& cand) { return !cand.candidate.has_rounded_display_masks; });
...
public:
    TransparentUnderlayOverlayProcessor()
        : DefaultOverlayProcessor()
    {
        strategies_.push_back(std::make_unique<OverlayStrategyUnderlay>(this, OverlayStrategyUnderlay::OpaqueMode::AllowTransparentCandidates));
    }
};
...
static ResourceId CreateResourceInLayerTree(
    ClientResourceProvider* child_resource_provider, const gfx::Size& size, bool is_overlay_candidate, SharedImageFormat format)
{
    auto resource = TransferableResource::MakeGpu(gpu::Mailbox::Generate(), GL_TEXTURE_2D, gpu::SyncToken(), size, format, is_overlay_candidate);

    ResourceId resource_id = child_resource_provider->ImportResource(resource, base::DoNothing());

    return resource_id;
}

ResourceId CreateResource(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    RasterContextProvider* child_context_provider, const gfx::Size& size, bool is_overlay_candidate, SharedImageFormat format,
    SurfaceId test_surface_id = SurfaceId())
{
    ResourceId resource_id = CreateResourceInLayerTree(child_resource_provider, size, is_overlay_candidate, format);

    int child_id = parent_resource_provider->CreateChild(base::DoNothing(), test_surface_id);

    // Transfer resource to the parent.
    std::vector<ResourceId> resource_ids_to_transfer;
    resource_ids_to_transfer.push_back(resource_id);
    std::vector<TransferableResource> list;
    child_resource_provider->PrepareSendToParent(resource_ids_to_transfer, &list, child_context_provider);
    ...
    std::unordered_map<ResourceId, ResourceId, ResourceIdHasher> resource_map = parent_resource_provider->GetChildToParentMap(child_id);
    return resource_map[list[0].id];
}

ResourceId CreateResource(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    RasterContextProvider* child_context_provider, const gfx::Size& size, bool is_overlay_candidate)
{
    return CreateResource(parent_resource_provider, child_resource_provider, child_context_provider, size, is_overlay_candidate, SinglePlaneFormat::kRGBA_8888);
}

SolidColorDrawQuad* CreateSolidColorQuadAt(const SharedQuadState* shared_quad_state, SkColor4f color, AggregatedRenderPass* render_pass, const gfx::Rect& rect)
{
...
TextureDrawQuad* CreateCandidateQuadAt(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    RasterContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, const gfx::Rect& rect,
    gfx::ProtectedVideoType protected_video_type, SharedImageFormat format, const gfx::Size& resource_size_in_pixels, SurfaceId test_surface_id = SurfaceId())
{
    ...
    ResourceId resource_id = CreateResource(
        parent_resource_provider, child_resource_provider, child_context_provider, resource_size_in_pixels, is_overlay_candidate, format, test_surface_id);

    auto* overlay_quad = render_pass->CreateAndAppendDrawQuad<TextureDrawQuad>();
    overlay_quad->SetNew(shared_quad_state, rect, rect, needs_blending, resource_id, premultiplied_alpha, kUVTopLeft, kUVBottomRight, SkColors::kTransparent,
        flipped, nearest_neighbor, /*secure_output_only=*/false, protected_video_type);
...
TextureDrawQuad* CreateCandidateQuadAt(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    RasterContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, const gfx::Rect& rect,
    gfx::ProtectedVideoType protected_video_type, SharedImageFormat format, SurfaceId test_surface_id = SurfaceId())
{
    return CreateCandidateQuadAt(parent_resource_provider, child_resource_provider, child_context_provider, shared_quad_state, render_pass, rect,
        protected_video_type, format, rect.size(), test_surface_id);
}
...
{
    return CreateCandidateQuadAt(parent_resource_provider, child_resource_provider, child_context_provider, shared_quad_state, render_pass, rect,
        gfx::ProtectedVideoType::kClear, SinglePlaneFormat::kRGBA_8888, test_surface_id);
}

#if BUILDFLAG(ENABLE_CAST_OVERLAY_STRATEGY)
std::unique_ptr<AggregatedRenderPass> CreateRenderPassWithTransform(const gfx::Transform& transform)
{
    AggregatedRenderPassId render_pass_id { 1 };
    gfx::Rect output_rect(0, 0, 256, 256);
...
// expects.
VideoHoleDrawQuad* CreateVideoHoleDrawQuadAt(const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, const gfx::Rect& rect)
{
    base::UnguessableToken overlay_plane_id = base::UnguessableToken::Create();
    auto* overlay_quad = render_pass->CreateAndAppendDrawQuad<VideoHoleDrawQuad>();
    overlay_quad->SetNew(shared_quad_state, rect, rect, overlay_plane_id);
    return overlay_quad;
}
#endif

TextureDrawQuad* CreateTransparentCandidateQuadAt(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    RasterContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, const gfx::Rect& rect)
{
    ...
    ResourceId resource_id
        = CreateResource(parent_resource_provider, child_resource_provider, child_context_provider, resource_size_in_pixels, is_overlay_candidate);

    auto* overlay_quad = render_pass->CreateAndAppendDrawQuad<TextureDrawQuad>();
    overlay_quad->SetNew(shared_quad_state, rect, rect, needs_blending, resource_id, premultiplied_alpha, kUVTopLeft, kUVBottomRight, SkColors::kTransparent,
        flipped, nearest_neighbor, /*secure_output_only=*/false, gfx::ProtectedVideoType::kClear);
    overlay_quad->set_resource_size_in_pixels(resource_size_in_pixels);

    return overlay_quad;
}

TextureDrawQuad* CreateFullscreenCandidateQuad(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    RasterContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass)
{
    return CreateCandidateQuadAt(
        parent_resource_provider, child_resource_provider, child_context_provider, shared_quad_state, render_pass, render_pass->output_rect);
}

TextureDrawQuad* CreateQuadWithRoundedDisplayMasksAt(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    RasterContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, bool is_overlay_candidate,
    const gfx::Rect& rect, const RoundedDisplayMasksInfo& rounded_display_masks_info)
{
    bool needs_blending = true;
...
TextureDrawQuad* CreateFullscreenQuadWithRoundedDisplayMasks(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    RasterContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, bool is_overlay_candidate,
    const RoundedDisplayMasksInfo& rounded_display_masks_info)
{
    return CreateQuadWithRoundedDisplayMasksAt(parent_resource_provider, child_resource_provider, child_context_provider, shared_quad_state, render_pass,
        is_overlay_candidate, render_pass->output_rect, rounded_display_masks_info);
}

void CreateOpaqueQuadAt(
    DisplayResourceProvider* resource_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, const gfx::Rect& rect)
{
...
void CreateOpaqueQuadAt(DisplayResourceProvider* resource_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass,
    const gfx::Rect& rect, SkColor4f color)
{
...
void CreateFullscreenOpaqueQuad(DisplayResourceProvider* resource_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass)
{
    CreateOpaqueQuadAt(resource_provider, shared_quad_state, render_pass, render_pass->output_rect);
}

static void CompareRenderPassLists(const AggregatedRenderPassList& expected_list, const AggregatedRenderPassList& actual_list)
{
    ...
    for (size_t i = 0; i < actual_list.size(); ++i) {
        ...
        EXPECT_EQ(expected->transform_to_root_target, actual->transform_to_root_target);
        EXPECT_EQ(expected->damage_rect, actual->damage_rect);
        EXPECT_EQ(expected->has_transparent_background, actual->has_transparent_background);

        EXPECT_EQ(expected->shared_quad_state_list.size(), actual->shared_quad_state_list.size());
        EXPECT_EQ(expected->quad_list.size(), actual->quad_list.size());

        for (auto exp_iter = expected->quad_list.cbegin(), act_iter = actual->quad_list.cbegin(); exp_iter != expected->quad_list.cend();
             ++exp_iter, ++act_iter) {
            EXPECT_EQ(exp_iter->rect.ToString(), act_iter->rect.ToString());
            EXPECT_EQ(exp_iter->shared_quad_state->quad_layer_rect.ToString(), act_iter->shared_quad_state->quad_layer_rect.ToString());
        }
    }
}
...
template <typename OverlayProcessorType> class OverlayTest : public testing::Test {
protected:
    void SetUp() override
    {
        ...
        lock_set_for_external_use_.emplace(resource_provider_.get(), output_surface_.get());
    ...
    std::optional<DisplayResourceProviderSkia::LockSetForExternalUse> lock_set_for_external_use_;
...
template <typename OverlayProcessorType> class UseMultipleOverlaysTest : public OverlayTest<OverlayProcessorType> {
public:
...
using TransparentUnderlayTest = OverlayTest<TransparentUnderlayOverlayProcessor>;
#if BUILDFLAG(ENABLE_CAST_OVERLAY_STRATEGY)
...
using MultiSingleOnTopOverlayTest = UseMultipleOverlaysTest<MultiSingleOnTopProcessor>;
using SizeSortedMultiSingeOnTopOverlayTest = UseMultipleOverlaysTest<SizeSortedMultiSingleOnTopProcessor>;

using SizeSortedMultiUnderlayOverlayTest = UseMultipleOverlaysTest<SizeSortedMultiUnderlayProcessor>;
using SizeSortedMultiOverlayTest = UseMultipleOverlaysTest<SizeSortedMultiOverlayProcessor>;
using TypeAndSizeSortedMultiOverlayTest = UseMultipleOverlaysTest<TypeAndSizeSortedMultiOverlayProcessor>;
using AllowCandidateWithMasksSortedMultiOverlayTest = UseMultipleOverlaysTest<AllowCandidateWithMasksSortedMultiOverlayProcessor>;
...
TEST_F(FullscreenOverlayTest, DRMDefaultBlackOptimization)
{
    ...
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), sub_fullscreen);

    // Add a black solid color behind it.
    CreateSolidColorQuadAt(pass->shared_quad_state_list.back(), SkColors::kBlack, pass.get(), pass->output_rect);
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);

    if (base::FeatureList::IsEnabled(features::kUseDrmBlackFullscreenOptimization)) {
...
TEST_F(FullscreenOverlayTest, DRMDefaultBlackOptimizationWithRoundedDisplayMaskTextures)
{
    auto pass = CreateRenderPass();
    auto sub_fullscreen = pass->output_rect;
    ...
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, gfx::Rect(0, 0, radius, display_height),
        RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(radius, radius, /*is_horizontally_positioned=*/false));

    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, gfx::Rect(display_width - radius, 0, radius, display_height),
        RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(radius, radius, /*is_horizontally_positioned=*/false));
...
TEST_F(FullscreenOverlayTest, SuccessfulOverlay)
{
    ...
    TextureDrawQuad* original_quad = CreateFullscreenCandidateQuad(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get());
    ResourceId original_resource_id = original_quad->resource_id();

    // Add something behind it.
    CreateFullscreenOpaqueQuad(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get());
    ...
    gfx::Rect overlay_damage_rect = overlay_processor_->GetAndResetOverlayDamage();
...
TEST_F(FullscreenOverlayTest, FailIfFullscreenQuadHasRoundedDisplayMasks)
{
    ...
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, rect, RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(10, 0));
...
TEST_F(FullscreenOverlayTest, FailOnOutputColorMatrix)
{
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetNonIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);
    ASSERT_EQ(0U, candidate_list.size());
...
TEST_F(FullscreenOverlayTest, AlphaFail)
{
    ...
    CreateTransparentCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), pass->output_rect);
...
TEST_F(FullscreenOverlayTest, OnTopFail)
{
    ...
    CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kOverlayTopLeftRect);
...
TEST_F(FullscreenOverlayTest, NotCoveringFullscreenFail)
{
    ...
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), inset_rect);
...
TEST_F(SingleOverlayOnTopTest, SuccessfulOverlay)
{
    ...
    for (auto it = quad_list.BackToFrontBegin(); it != quad_list.BackToFrontEnd(); ++it) {
...
TEST_F(MultiSingleOnTopOverlayTest, SuccessfulOverlay_OccludedByOverlayRoundedDisplayMaskCandidate)
{
    auto pass = CreateRenderPass();

    // Add a quad with rounded-display masks.
    CreateFullscreenQuadWithRoundedDisplayMasks(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
        pass->shared_quad_state_list.back(), pass.get(),
        /*is_overlay_candidate=*/true, RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(10, 0));
...
TEST_F(MultiSingleOnTopOverlayTest, RoundedDisplayMaskCandidateNotDrawnOnTop)
{
    ...
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kSmallCandidateRect);
    ...
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, gfx::Rect(0, 0, 16, 100), RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(10, 0));

    const auto kMediumCandidateRect = gfx::Rect(16, 16, 32, 32);
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kMediumCandidateRect);
    ...
    // promoted.
    CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(),
        gfx::Rect(0, 16, 16, 16));
...
TEST_F(DeathMultiSingleOnTopOverlayTest, RoundedDisplayMaskCandidatesOverlapsEachOther)
{
    auto pass = CreateRenderPass();
    ...
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, kOverlayTopLeftRect, RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(10, 0));
    ...
    EXPECT_DCHECK_DEATH(overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters,
        render_pass_backdrop_filters, std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_));
}

TEST_F(DeathMultiSingleOnTopOverlayTest, RoundedDisplayMaskCandidateIsNotOverlayCandidate)
{
    auto pass = CreateRenderPass();
    ...
    CreateFullscreenQuadWithRoundedDisplayMasks(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
        pass->shared_quad_state_list.back(), pass.get(),
        /*is_overlay_candidate=*/false, RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(10, 0));
...
TEST_F(MultiSingleOnTopOverlayTest, PromoteRoundedDisplayMaskCandidateIfOccludeOtherCandidates)
{
    ...
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, kCandidateRect1, RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(0, 10));
    ...
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kCandidateRect2);

    // This candidate occludes quad with rounded-display masks.
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kOverlayTopRightRect);
...
TEST_F(MultiSingleOnTopOverlayTest, FailToPromoteRoundedDisplayMaskCandidatesIfTheyDontOccludeOtherCandidates)
{
    // Given different rect sizes, the candidate will be out of draw order once
    // sorted.
    auto pass = CreateRenderPass();
    ...
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, gfx::Rect(0, 0, 256, 10), RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(0, 10));

    // Add a quad with rounded-display masks that is an overlay candidate.
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, gfx::Rect(0, 20, 256, 10), RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(0, 10));

    // This candidate does not occludes any of the quads with rounded-display
    // masks.
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kOverlayTopLeftRect);
...
TEST_F(MultiSingleOnTopOverlayTest, OcclusionOptimizationForRoundedDisplayMaskCandidate)
{
    auto pass = CreateRenderPass();
    ...
    const auto* not_promoted_candidate = CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
        pass->shared_quad_state_list.back(), pass.get(),
        /*is_overlay_candidate=*/true, gfx::Rect(0, 0, 100, 10), RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(10, 10));
    ...
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, kCandidateWithMaskRect2, RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(10, 10));
    ...
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kCandidateRect1);
...
TEST_F(SingleOverlayOnTopTest, PrioritizeBiggerOne)
{
    ...
    TextureDrawQuad* quad_big = CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kBigCandidateRect);
...
TEST_F(SingleOverlayOnTopTest, CandidateIdCollision)
{
    ...
    TextureDrawQuad* quad_a = CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kCandidateRect);
    ...
    auto candidate_factory = OverlayCandidateFactory(
        pass.get(), resource_provider_.get(), &surface_damage_rect_list, &color_mat, gfx::RectF(pass->output_rect), &render_pass_filters, kTestOverlayContext);
    ...
    pass_list.push_back(std::move(pass));
    overlay_processor_->SetFrameSequenceNumber(1);
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, color_mat, render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);
    ASSERT_EQ(1U, candidate_list.size());
...
TEST_F(SingleOverlayOnTopTest, CandidateTrackIdUniqueSurface)
{
    auto pass = CreateRenderPass();
    const auto kCandidateRect = gfx::Rect(0, 0, 16, 16);
    TextureDrawQuad* quad_a = CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
        pass->shared_quad_state_list.back(), pass.get(), kCandidateRect, SurfaceId(FrameSinkId(1, 1), LocalSurfaceId()));
    TextureDrawQuad* quad_b = CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
        pass->shared_quad_state_list.back(), pass.get(), kCandidateRect, SurfaceId(FrameSinkId(2, 2), LocalSurfaceId()));
...
TEST_F(SingleOverlayOnTopTest, StablePrioritizeIntervalFrame)
{
    ...
    ResourceId resource_id_a = CreateResource(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), kCandidateRectA.size(), true /*is_overlay_candidate*/);
    ResourceId resource_id_b = CreateResource(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), kCandidateRectB.size(), true /*is_overlay_candidate*/);
    ...
    for (int i = 1; i < kNumFrames; i++) {
        auto pass = CreateRenderPass();
        SharedQuadState* shared_quad_state_a = pass->shared_quad_state_list.AllocateAndCopyFrom(pass->shared_quad_state_list.back());
        shared_quad_state_a->overlay_damage_index = 0;
        TextureDrawQuad* quad_small = pass->CreateAndAppendDrawQuad<TextureDrawQuad>();
        quad_small->SetNew(shared_quad_state_a, kCandidateRectA, kCandidateRectA, false /*needs_blending*/, resource_id_a, false /*premultiplied_alpha*/,
            kUVTopLeft, kUVBottomRight, SkColors::kTransparent, false /*flipped*/, false /*nearest_neighbor*/, false /*secure_output_only*/,
        ...
        SharedQuadState* shared_quad_state_b = pass->shared_quad_state_list.AllocateAndCopyFrom(pass->shared_quad_state_list.back());
        TextureDrawQuad* quad_big = pass->CreateAndAppendDrawQuad<TextureDrawQuad>();

        quad_big->SetNew(shared_quad_state_b, kCandidateRectB, kCandidateRectB, false /*needs_blending*/, resource_id_b, false /*premultiplied_alpha*/,
            kUVTopLeft, kUVBottomRight, SkColors::kTransparent, false /*flipped*/, false /*nearest_neighbor*/, false /*secure_output_only*/,
            gfx::ProtectedVideoType::kClear);
        ...
        // Add something behind it.
        SharedQuadState* default_shared_quad_state = pass->shared_quad_state_list.AllocateAndCopyFrom(pass->shared_quad_state_list.back());
        CreateFullscreenOpaqueQuad(resource_provider_.get(), default_shared_quad_state, pass.get());
        ...
        surface_damage_rect_list.push_back((i % 2) == 0 ? kCandidateRectA : gfx::Rect());
        surface_damage_rect_list.push_back((i % 2) == 0 ? gfx::Rect() : kCandidateRectB);

        overlay_processor_->SetFrameSequenceNumber(i);
        overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters,
            render_pass_backdrop_filters, std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);
        ASSERT_EQ(1U, candidate_list.size());
...
TEST_F(SingleOverlayOnTopTest, OpaqueOverlayDamageSubtract)
{
    ...
    const gfx::Rect kDamageRect[] = { gfx::Rect(10, 10, kCandidateSmall, kCandidateSmall), gfx::Rect(0, 10, kCandidateSmall, kCandidateSmall),
        gfx::Rect(6, 6, kCandidateSmall, kCandidateSmall) };

    const gfx::Rect kExpectedDamage[] = { gfx::Rect(), gfx::Rect(0, 10, 10, kCandidateSmall), gfx::Rect(6, 6, kCandidateSmall, kCandidateSmall) };
    ...
    for (size_t i = 0; i < std::size(kDamageRect); ++i) {
        ...
        auto pass = CreateRenderPass();
        SharedQuadState* damaged_shared_quad_state = pass->shared_quad_state_list.AllocateAndCopyFrom(pass->shared_quad_state_list.back());

        auto* quad = CreateCandidateQuadAt(
            resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), damaged_shared_quad_state, pass.get(), kOverlayDisplayRect);
...
TEST_F(SingleOverlayOnTopTest, NonOpaquePureOverlayFirstFrameDamage)
{
    ...
    for (size_t i = 0; i < std::size(kExpectedDamage); ++i) {
        ...
        auto* quad = CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
...
TEST_F(SingleOverlayOnTopTest, NonOpaquePureOverlayNonOccludingDamage)
{
    ...
    const gfx::Rect kExpectedDamage[] = { kInFrontDamage, kInFrontDamage,
...
TEST_F(SingleOverlayOnTopTest, DamageRect)
{
    ...
    SharedQuadState* default_damaged_shared_quad_state = pass->shared_quad_state_list.AllocateAndCopyFrom(pass->shared_quad_state_list.back());
    ...
    // Add something behind it.
    CreateFullscreenOpaqueQuad(resource_provider_.get(), default_damaged_shared_quad_state, pass.get());
    CreateFullscreenOpaqueQuad(resource_provider_.get(), pass->shared_quad_state_list.front(), pass.get());
...
TEST_F(SingleOverlayOnTopTest, DamageWithMutipleSurfaceDamage)
{
    ...
    auto* quad = CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get(), kOverlayTopLeftRect);
    quad->needs_blending = false;
    // Add something behind it.
    CreateFullscreenOpaqueQuad(resource_provider_.get(), default_damaged_shared_quad_state, pass.get());
...
TEST_F(UnderlayTest, AcceptBlackBackgroundColorWithRoundedCorners)
{
    ...
    sqs->mask_filter_info = gfx::MaskFilterInfo(gfx::RectF(kOverlayRect), gfx::RoundedCornersF(5.0f), gfx::LinearGradient::GetEmpty());

    TextureDrawQuad* quad = CreateFullscreenCandidateQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get());
    quad->background_color = SkColors::kBlack;
...
TEST_F(UnderlayTest, RejectBlackBackgroundColorWithGradient)
{
    ...
    auto* sqs = pass->shared_quad_state_list.back();
    sqs->mask_filter_info = gfx::MaskFilterInfo(gfx::RectF(kOverlayRect), gfx::RoundedCornersF(), gradient);
...
TEST_F(SingleOverlayOnTopTest, RejectNearestNeighbor)
{
    ...
    CreateFullscreenCandidateQuad(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get());
    static_cast<TextureDrawQuad*>(pass->quad_list.back())->nearest_neighbor = true;
...
TEST_F(SingleOverlayOnTopTest, RejectNonAxisAlignedTransform)
{
    ...
    CreateFullscreenCandidateQuad(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get());
    pass->shared_quad_state_list.back()->quad_to_target_transform.RotateAboutXAxis(45.f);
...
TEST_F(UnderlayTest, ReplacementQuad)
{
    ...
    EXPECT_EQ(SkColors::kTransparent, static_cast<SolidColorDrawQuad*>(pass_list.front()->quad_list.front())->color);
    EXPECT_FALSE(pass_list.front()->quad_list.front()->ShouldDrawWithBlending());
    EXPECT_FALSE(pass_list.front()->quad_list.front()->shared_quad_state->are_contents_opaque);
}
...
TEST_F(UnderlayTest, AllowVerticalFlip)
{
    ...
    auto pass = CreateRenderPass();
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), rect);
    pass->shared_quad_state_list.back()->quad_to_target_transform.Scale(2.0f, -1.0f);
    OverlayCandidateList candidate_list;
    OverlayProcessorInterface::FilterOperationsMap render_pass_filters;
    OverlayProcessorInterface::FilterOperationsMap render_pass_backdrop_filters;
    AggregatedRenderPassList pass_list;
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);
    ASSERT_EQ(1U, candidate_list.size());
    EXPECT_EQ(gfx::OVERLAY_TRANSFORM_FLIP_VERTICAL, absl::get<gfx::OverlayTransform>(candidate_list.back().transform));
}
...
TEST_F(UnderlayTest, AllowHorizontalFlip)
{
    ...
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), rect);
    pass->shared_quad_state_list.back()->quad_to_target_transform.Scale(-1.0f, 2.0f);
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);
    ASSERT_EQ(1U, candidate_list.size());
    EXPECT_EQ(gfx::OVERLAY_TRANSFORM_FLIP_HORIZONTAL, absl::get<gfx::OverlayTransform>(candidate_list.back().transform));
}
...
TEST_F(SingleOverlayOnTopTest, AcceptMirrorYTransform)
{
    ...
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), rect);
    pass->shared_quad_state_list.back()->quad_to_target_transform.Scale(1.f, -1.f);
...
TEST_F(UnderlayTest, Allow90DegreeRotation)
{
    ...
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), rect);
    pass->shared_quad_state_list.back()->quad_to_target_transform.RotateAboutZAxis(90.f);
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);
    ASSERT_EQ(1U, candidate_list.size());
    EXPECT_EQ(gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90, absl::get<gfx::OverlayTransform>(candidate_list.back().transform));
}
...
TEST_F(UnderlayTest, Allow180DegreeRotation)
{
    ...
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), rect);
    pass->shared_quad_state_list.back()->quad_to_target_transform.RotateAboutZAxis(180.f);
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);
    ASSERT_EQ(1U, candidate_list.size());
    EXPECT_EQ(gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_180, absl::get<gfx::OverlayTransform>(candidate_list.back().transform));
}
...
TEST_F(UnderlayTest, Allow270DegreeRotation)
{
    ...
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), rect);
    pass->shared_quad_state_list.back()->quad_to_target_transform.RotateAboutZAxis(270.f);
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect_, &content_bounds_);
    ASSERT_EQ(1U, candidate_list.size());
    EXPECT_EQ(gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_270, absl::get<gfx::OverlayTransform>(candidate_list.back().transform));
}
...
TEST_F(UnderlayTest, DisallowFilteredQuadOnTop)
{
    ...
    AggregatedRenderPassDrawQuad* quad = pass->CreateAndAppendDrawQuad<AggregatedRenderPassDrawQuad>();
    quad->SetNew(pass->shared_quad_state_list.back(), kOverlayRect, kOverlayRect, render_pass_id, kInvalidResourceId, gfx::RectF(), gfx::Size(),
...
TEST_F(UnderlayTest, AllowFilteredQuadOnTopForProtectedVideo)
{
    ...
    CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(),
        pass->output_rect, gfx::ProtectedVideoType::kHardwareProtected, MultiPlaneFormat::kNV12)
        ->needs_blending
        = false;
    pass->shared_quad_state_list.front()->opacity = 1.0;
...
TEST_F(SingleOverlayOnTopTest, AllowTransparentOnTop)
{
    ...
    CreateSolidColorQuadAt(shared_state, SkColors::kBlack, pass.get(), kOverlayBottomRightRect);
    shared_state = pass->CreateAndAppendSharedQuadState();
    shared_state->opacity = 1.f;
    CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), shared_state, pass.get(), kOverlayBottomRightRect);
...
TEST_F(SingleOverlayOnTopTest, AllowTransparentColorOnTop)
{
    ...
    auto pass = CreateRenderPass();
    CreateSolidColorQuadAt(pass->shared_quad_state_list.back(), SkColors::kTransparent, pass.get(), kOverlayBottomRightRect);
    CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(),
        kOverlayBottomRightRect);
...
TEST_F(SingleOverlayOnTopTest, RejectTransparentColorOnTopWithoutBlending)
{
    auto pass = CreateRenderPass();
    SharedQuadState* shared_state = pass->CreateAndAppendSharedQuadState();
    CreateSolidColorQuadAt(shared_state, SkColors::kTransparent, pass.get(), kOverlayBottomRightRect)->needs_blending = false;
    CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), shared_state, pass.get(), kOverlayBottomRightRect);
...
TEST_F(SingleOverlayOnTopTest, CheckPromotionHintBasic)
{
    ...
    for (size_t i = 0; i <= kTestRequiredAtFrame; ++i) {
        auto pass = CreateRenderPass();
        SurfaceDamageRectList surface_damage_rect_list;
        SharedQuadState* sqs_partial = pass->shared_quad_state_list.AllocateAndCopyFrom(pass->shared_quad_state_list.back());
        auto* tex_quad_partial = CreateCandidateQuadAt(
            resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs_partial, pass.get(), kOverlayTopLeftRect);
        ...
        tex_quad_partial->overlay_priority_hint = i > kTestRegularAtFrame ? OverlayPriority::kRequired : OverlayPriority::kRegular;

        // Full damaged quad with a different rect; specifically
        // |kOverlayBottomRightRect|.
        SharedQuadState* sqs_full = pass->shared_quad_state_list.AllocateAndCopyFrom(pass->shared_quad_state_list.back());
        auto* tex_quad_full = CreateCandidateQuadAt(
            resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs_full, pass.get(), kOverlayBottomRightRect);
        sqs_partial->overlay_damage_index = surface_damage_rect_list.size();
        tex_quad_full->overlay_priority_hint = i > kTestRegularAtFrame ? OverlayPriority::kRegular : OverlayPriority::kLow;
        ...
        if (i == kTestRegularAtFrame) {
            ...
            if (!candidate_list.empty()) {
                ...
                EXPECT_EQ(kOverlayTopLeftRect, gfx::ToRoundedRect(candidate_list.back().display_rect));
            }
...
TEST_F(ChangeSingleOnTopTest, DoNotPromoteIfContentsDontChange)
{
    ...
    size_t kFramesSkippedBeforeNotPromoting = overlay_processor_->TrackerConfigAccessor().max_num_frames_avg;
    ...
    for (size_t i = 0; i < 3 + kFramesSkippedBeforeNotPromoting; ++i) {
        ...
        if (i == 0 || i == 1) {
            ...
            resource_id = CreateResource(
                resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->output_rect.size(), true /*is_overlay_candidate*/);
        ...
        TextureDrawQuad* original_quad = main_pass->CreateAndAppendDrawQuad<TextureDrawQuad>();
        original_quad->SetNew(pass->shared_quad_state_list.back(), pass->output_rect, pass->output_rect, false /*needs_blending*/, resource_id,
            false /*premultiplied_alpha*/, kUVTopLeft, kUVBottomRight, SkColors::kTransparent, false /*flipped*/, false /*nearest_neighbor*/,
            false /*secure_output_only*/, gfx::ProtectedVideoType::kClear);
        original_quad->set_resource_size_in_pixels(pass->output_rect.size());

        // Add something behind it.
        CreateFullscreenOpaqueQuad(resource_provider_.get(), pass->shared_quad_state_list.back(), main_pass);
...
TEST_F(FullThresholdTest, ThresholdTestForPrioritization)
{
    ...
    const int kNumFramesClear
        = static_cast<int>(std::ceil(std::log(1.0f / config.max_num_frames_avg) / std::log((config.max_num_frames_avg - 1.0f) / config.max_num_frames_avg)));
    ...
    for (size_t i = 0; i < kSlowFrameTestEnd; ++i) {
        ...
        bool nearly_occluded = i >= kDamageFrameTestStart && i < kDamageFrameTestEnd;
        CreateSolidColorQuadAt(
            pass->shared_quad_state_list.back(), SkColors::kBlack, pass.get(), nearly_occluded ? nearly_occluding_quad : kOverlayTopLeftRect);

        // Create a quad with the resource ID selected above.
        TextureDrawQuad* quad_candidate = CreateCandidateQuadAt(
            resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kOverlayRect);
...
TEST_F(UnderlayTest, OverlayLayerUnderMainLayer)
{
    ...
    EXPECT_EQ(main_pass->quad_list.back()->material, DrawQuad::Material::kSolidColor);
...
TEST_F(UnderlayTest, AllowOnTop)
{
    ...
    EXPECT_EQ(-1, candidate_list[0].plane_z_order);
    // The overlay quad should have changed to a SOLID_COLOR quad.
    EXPECT_EQ(main_pass->quad_list.front()->material, DrawQuad::Material::kSolidColor);
...
TEST_F(TransitionOverlayTypeTest, DamageChangeOnTransistionOverlayType)
{
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (i >= kOverlayFrameStart && i < kOverlayFrameEnd) {
            CreateSolidColorQuadAt(pass->shared_quad_state_list.back(), SkColors::kBlack, pass.get(), kOverlayTopLeftRect);
        }

        CreateFullscreenCandidateQuad(
            resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.front(), pass.get());
...
TEST_F(TransitionOverlayTypeTest, DamageWhenOverlayBecomesTransparent)
{
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (i < kTransparentFrameStart) {
            ...
            CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get(), kTopLeft);
        } else {
            // Create transparent candidate in top left.
            auto* sqs = pass->CreateAndAppendSharedQuadState();
            sqs->overlay_damage_index = surface_damage_rect_list.size();
            surface_damage_rect_list.emplace_back(kTopLeft);
            CreateTransparentCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get(), kTopLeft);
        }
...
TEST_F(TransitionOverlayTypeTest, MaskFilterBringsUnderlay)
{
    ...
    sqs->overlay_damage_index = 0;
    surface_damage_rect_list.emplace_back(damage_rect_);
    sqs->mask_filter_info = gfx::MaskFilterInfo(gfx::RectF(kOverlayRect), gfx::RoundedCornersF(1.f), gfx::LinearGradient::GetEmpty());
    CreateFullscreenCandidateQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get());
    CreateFullscreenOpaqueQuad(resource_provider_.get(), default_damaged_shared_quad_state, pass.get());
    ...
    EXPECT_EQ(SkColors::kBlack, static_cast<SolidColorDrawQuad*>(pass_list.front()->quad_list.front())->color);
    EXPECT_FALSE(pass_list.front()->quad_list.front()->shared_quad_state->are_contents_opaque);
    EXPECT_EQ(SkBlendMode::kDstOut, pass_list.front()->quad_list.front()->shared_quad_state->blend_mode);
}
...
TEST_F(UnderlayTest, DamageNotExcludedForNonIdenticalConsecutiveUnderlays)
{
    ...
    gfx::Rect overlay_rects[] = { kOverlayBottomRightRect, kSmallTestRect, kOverlayTopLeftRect, kOverlayRect };
    gfx::Rect prev_rect;
    for (auto&& curr_rect : overlay_rects) {
        ...
        CreateCandidateQuadAt(
            resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), curr_rect);
...
TEST_F(UnderlayTest, DamageNotExcludedForNonConsecutiveIdenticalUnderlays)
{
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (i == 0) {
            ...
            EXPECT_EQ(damage_rect_.IsEmpty(), has_fullscreen_candidate[i] && has_fullscreen_candidate[i - 1]);
        }
    }
}
...
TEST_F(UnderlayTest, DamageNotExcludedForConsecutiveUnderlaysIfOneHasMaskFilterAndOtherDoesNot)
{
    constexpr bool kHasMaskFilter[] = { true, false, true, false, true, true, true, false, false, false };
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get(), kOverlayRect);
        CreateFullscreenOpaqueQuad(resource_provider_.get(), default_damaged_shared_quad_state, pass.get());
        ...
        if (i == 0) {
            ...
            EXPECT_EQ(damage_rect_.IsEmpty(), kHasMaskFilter[i] == kHasMaskFilter[i - 1]);
        }
    }
}
...
TEST_F(UnderlayTest, DamageExtractedWhenQuadsAboveDontOverlap)
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        CreateOpaqueQuadAt(resource_provider_.get(), default_damaged_shared_quad_state, pass.get(), kOverlayTopLeftRect);
        CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.front(), pass.get(),
            kOverlayBottomRightRect);
...
TEST_F(UnderlayTest, PrimaryPlaneOverlayIsTransparentWithUnderlay)
{
    auto pass = CreateRenderPass();
    gfx::Rect output_rect = pass->output_rect;
    CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), output_rect, SkColors::kWhite);
    ...
    auto output_surface_plane = overlay_processor_->ProcessOutputSurfaceAsOverlay(
        kDisplaySize, kDisplaySize, kDefaultSIFormat, gfx::ColorSpace(), false /* has_alpha */, 1.0f /* opacity */, gpu::Mailbox());
    OverlayProcessorInterface::OutputSurfaceOverlayPlane* primary_plane = &output_surface_plane;

    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), primary_plane, &candidate_list, &damage_rect_, &content_bounds_);
...
TEST_F(UnderlayTest, OverlayCandidateTemporalTracker)
{
    ...
    auto wait_inactive_frames = [&]() { frame_counter += config.max_num_frames_avg + 1; };
    ...
    for (int j = 0; j < 2; j++) {
        ...
        for (int i = 0; i < kNumFramesClear; i++) {
            wait_1_frame();
            tracker.AddRecord(frame_counter, kFullDamage, id_generator.GenerateNextId(), config);
        }

        EXPECT_TRUE(tracker.IsActivelyChanging(frame_counter, config));
        auto opaque_power_gain_60_full = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area, kIsFullscreen);
        ...
        auto opaque_power_gain_60_stutter = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area, kIsFullscreen);
        ...
        auto opaque_power_gain_60_inactive = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area, kIsFullscreen);
        ...
        auto opaque_power_gain_30_full = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area, kIsFullscreen);
        ...
        EXPECT_TRUE(tracker.IsActivelyChanging(frame_counter, config));
        auto opaque_power_gain_30_stutter = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area, kIsFullscreen);
        ...
        auto opaque_power_gain_30_inactive = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area, kIsFullscreen);
        // Simple test to make sure that power categorization is not completely
        // invalidated when candidate becomes inactive.
        ...
        for (int i = 0; i < kNumFramesClear; i++) {
            wait_1_frame();
            tracker.AddRecord(frame_counter, kAboveHighDamage, id_generator.GenerateNextId(), config);
        }

        auto opaque_power_gain_high_damage = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area, kIsFullscreen);
        ...
        for (int i = 0; i < kNumFramesClear; i++) {
            wait_1_frame();
            tracker.AddRecord(frame_counter, kBelowLowDamage, id_generator.GenerateNextId(), config);
        }

        auto opaque_power_gain_low_damage = tracker.GetModeledPowerGain(frame_counter, config, fake_display_area, kIsFullscreen);
        ...
        for (int i = 0; i < avg_range_tracker; i++) {
            ...
            tracker.AddRecord(frame_counter, dynamic_damage_ratio, id_generator.GenerateNextId(), config);
        }
    ...
    for (int i = 0; i < config.max_num_frames_avg; i++) {
        wait_1_frame();
        tracker.AddRecord(frame_counter, kDamageRatio, kFakeConstantResourceId, config, true);
    }
    ...
    for (int i = 0; i < config.max_num_frames_avg; i++) {
        wait_1_frame();
        tracker.AddRecord(frame_counter, 0.9f, kFakeConstantResourceId, config, false);
    }
...
TEST_F(UnderlayTest, UpdateDamageRectWhenNoPromotion)
{
    ...
    gfx::Rect damages[] = { gfx::Rect(0, 0, 32, 32), gfx::Rect(0, 0, 32, 32), gfx::Rect(0, 0, 312, 16) };
    gfx::Rect expected_damages[] = { gfx::Rect(0, 0, 256, 256), gfx::Rect(0, 0, 0, 0), gfx::Rect(0, 0, 312, 256) };
    ...
    for (size_t i = 0; i < std::size(expected_damages); ++i) {
        ...
        OverlayCandidateList candidate_list;
        OverlayProcessorInterface::FilterOperationsMap render_pass_filters;
        OverlayProcessorInterface::FilterOperationsMap render_pass_background_filters;
        AggregatedRenderPassList pass_list;
        pass_list.push_back(std::move(pass));

        overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters,
            render_pass_background_filters, std::move(surface_damage_rect_list), nullptr, &candidate_list, &damage_rect, &content_bounds_);
...
TEST_F(UnderlayTest, CandidateNoDamageWhenQuadSharedStateNoOccludingDamage)
{
    for (int i = 0; i < 4; ++i) {
        ...
        CreateSolidColorQuadAt(default_damaged_shared_quad_state, SkColors::kBlack, pass.get(), rect);
        ...
        if (i == 0 || i == 1) {
            EXPECT_EQ(candidate_list[0].overlay_damage_index, OverlayCandidate::kInvalidDamageIndex);
            EXPECT_FALSE(damage_rect_.IsEmpty());
        } else if (i == 3) {
...
{
    auto pass = CreateRenderPass();
    CreateVideoHoleDrawQuadAt(pass->shared_quad_state_list.back(), pass.get(), kOverlayRect);
...
TEST_F(UnderlayCastTest, BlackOutsideOverlayContentBounds)
{
    ...
    auto pass = CreateRenderPass();
    CreateVideoHoleDrawQuadAt(pass->shared_quad_state_list.back(), pass.get(), kOverlayBottomRightRect);
    CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kLeftSide, SkColors::kBlack);
    CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kTopRight, SkColors::kBlack);
...
TEST_F(UnderlayCastTest, OverlayOccludedUnionContentBounds)
{
    auto pass = CreateRenderPass();
    CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kOverlayTopLeftRect);
    CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kOverlayBottomRightRect);
    CreateVideoHoleDrawQuadAt(pass->shared_quad_state_list.back(), pass.get(), kOverlayRect);
...
TEST_F(UnderlayCastTest, RoundOverlayContentBounds)
{
    ...
    CreateVideoHoleDrawQuadAt(pass->shared_quad_state_list.back(), pass.get(), overlay_rect);
    CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), gfx::Rect(0, 0, 10, 10), SkColors::kWhite);
...
TEST_F(UnderlayCastTest, RoundContentBounds)
{
    ...
    auto pass = CreateRenderPassWithTransform(transform);
    CreateVideoHoleDrawQuadAt(pass->shared_quad_state_list.back(), pass.get(), overlay_rect);
    CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), gfx::Rect(0, 0, 255, 255), SkColors::kWhite);
...
TEST_F(UnderlayCastTest, PrimaryPlaneOverlayIsAlwaysTransparent)
{
    ...
    SurfaceDamageRectList surface_damage_rect_list;
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), &output_surface_plane, &candidate_list, &damage_rect_, &content_bounds_);
...
protected:
    ...
    {
        overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list_, GetIdentityColorMatrix(), render_pass_filters_,
            render_pass_backdrop_filters_, std::move(surface_damage_rect_list_), nullity, &ca_layer_list_, &damage_rect_, &content_bounds_);
    }
...
{
    quad_->SetNew(pass_->shared_quad_state_list.back(), kOverlayRect, kOverlayRect, render_pass_id_, kInvalidResourceId, gfx::RectF(), gfx::Size(),
        gfx::Vector2dF(1, 1), gfx::PointF(), gfx::RectF(), false, 1.0f);
    ProcessForOverlays();
...
TEST_F(CALayerOverlayRPDQTest, AggregatedRenderPassDrawQuadAllValidFilters)
{
    ...
    filters_.Append(cc::FilterOperation::CreateDropShadowFilter(gfx::Point(10, 20), 1.0f, SK_ColorGREEN));
...
TEST_F(CALayerOverlayRPDQTest, AggregatedRenderPassDrawQuadDropShadowFilterScale)
{
    filters_.Append(cc::FilterOperation::CreateDropShadowFilter(gfx::Point(10, 20), 1.0f, SK_ColorGREEN));
    render_pass_filters_[render_pass_id_] = &filters_;
    quad_->SetNew(pass_->shared_quad_state_list.back(), kOverlayRect, kOverlayRect, render_pass_id_, kInvalidResourceId, gfx::RectF(), gfx::Size(),
...
{
    quad_->SetNew(pass_->shared_quad_state_list.back(), kOverlayRect, kOverlayRect, render_pass_id_, ResourceId(2), gfx::RectF(), gfx::Size(),
        gfx::Vector2dF(1, 1), gfx::PointF(), gfx::RectF(), false, 1.0f);
    ProcessForOverlays();
    EXPECT_EQ(1U, ca_layer_list_.size());
}
...
TEST_F(CALayerOverlayRPDQTest, TooManyRenderPassDrawQuads)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        quad->SetNew(pass_->shared_quad_state_list.back(), kOverlayRect, kOverlayRect, render_pass_id_, ResourceId(2), gfx::RectF(), gfx::Size(),
            gfx::Vector2dF(1, 1), gfx::PointF(), gfx::RectF(), false, 1.0f);
    }
...
void AddQuad(gfx::Rect quad_rect, const gfx::Transform& quad_to_target_transform, AggregatedRenderPass* render_pass)
{
    ...
    solid_quad->SetNew(quad_state, quad_rect, quad_rect, SkColors::kBlack, false /* force_anti_aliasing_off */);
}
...
TEST_F(SingleOverlayOnTopTest, IsOverlayRequiredHwProtectedVideo)
{
    ...
    auto pass = CreateRenderPass();
    const auto kSmallCandidateRect = gfx::Rect(0, 0, 16, 16);
    auto* new_quad = CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(), kSmallCandidateRect, gfx::ProtectedVideoType::kHardwareProtected, MultiPlaneFormat::kNV12);
    SurfaceDamageRectList surface_damage_rect_list;
    OverlayCandidate candidate;
    auto color_mat = GetIdentityColorMatrix();
    OverlayProcessorInterface::FilterOperationsMap render_pass_filters;
...
TEST_F(SingleOverlayOnTopTest, RequiredOverlayClippingAndSubsampling)
{
    ...
    auto* new_quad = CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(), kVideoCandidateRect, gfx::ProtectedVideoType::kHardwareProtected, MultiPlaneFormat::kNV12);
    pass->shared_quad_state_list.back()->clip_rect = kOverlayClipRect;
    SurfaceDamageRectList surface_damage_rect_list;
    OverlayCandidate candidate;
    auto color_mat = GetIdentityColorMatrix();
    ...
    EXPECT_EQ(kTargetSrcRect,
        gfx::ToRoundedRect(gfx::ScaleRect(candidate.uv_rect, candidate.resource_size_in_pixels.width(), candidate.resource_size_in_pixels.height())));
    EXPECT_TRUE(candidate.requires_overlay);
    EXPECT_FALSE(candidate.clip_rect);
    EXPECT_EQ(gfx::ToRoundedRect(candidate.display_rect), kOverlayClipRect);
}

TEST_F(SingleOverlayOnTopTest, RequiredOverlayClippingAndSubsamplingWithPrimary)
{
    // Add a small quad.
    auto pass = CreateRenderPass();
    const auto kVideoCandidateRect = gfx::Rect(-19, -20, 320, 240);
    ...
    OverlayProcessorInterface::FilterOperationsMap render_pass_filters;
    auto candidate_factory = OverlayCandidateFactory(
        pass.get(), resource_provider_.get(), &surface_damage_rect_list, &color_mat, primary_rect, &render_pass_filters, kTestOverlayContext);
    candidate_factory.FromDrawQuad(new_quad, candidate);
...
TEST_F(UnderlayTest, EstimateOccludedDamage)
{
    ...
    AddQuad(gfx::Rect(100, 100, kOccluderWidth, kOccluderWidth), identity, pass.get());
    AddQuad(gfx::Rect(150, 150, kOccluderWidth, kOccluderWidth), identity, pass.get());

    const int kCandidateSmall = 50;
    const int kCandidateLarge = 100;
    const gfx::Rect kCandidateRects[] = { gfx::Rect(0, 0, kCandidateSmall, kCandidateSmall), gfx::Rect(100, 100, kCandidateSmall, kCandidateSmall),
        gfx::Rect(100, 100, kCandidateLarge, kCandidateLarge), kOverlayRect };

    const bool kCandidateUseSurfaceIndex[] = { true, true, true, false };

    const int kExpectedDamages[] = { kCandidateSmall * kCandidateSmall, kCandidateSmall * kCandidateSmall - kOccluderWidth * kOccluderWidth,
        kCandidateLarge * kCandidateLarge - kOccluderWidth * kOccluderWidth * 2,
        kOverlayRect.width() * kOverlayRect.height() - kOccluderWidth * kOccluderWidth * 2 };

    static_assert(std::size(kCandidateRects) == std::size(kCandidateUseSurfaceIndex), "Number of elements in each list should be the identical.");
    static_assert(std::size(kCandidateRects) == std::size(kExpectedDamages), "Number of elements in each list should be the identical.");
    ...
    for (size_t i = 0; i < std::size(kCandidateRects); ++i) {
        ...
        if (kCandidateUseSurfaceIndex[i]) {
            damaged_shared_quad_state->overlay_damage_index = surface_damage_rect_list.size();
        } else {
        ...
        auto* quad_candidate = CreateCandidateQuadAt(
            resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), damaged_shared_quad_state, pass.get(), kCandidateRects[i]);

        OverlayCandidate candidate;
        auto color_mat = GetIdentityColorMatrix();
        OverlayProcessorInterface::FilterOperationsMap render_pass_filters;
        auto candidate_factory = OverlayCandidateFactory(pass.get(), resource_provider_.get(), &surface_damage_rect_list, &color_mat,
        ...
        if (kCandidateUseSurfaceIndex[i]) {
            ASSERT_EQ(kCandidateRects[i].size().GetArea(), candidate.damage_area_estimate);
            ...
            ASSERT_FALSE(quad_candidate->shared_quad_state->overlay_damage_index.has_value());
        }

        // Now we test the opaque occlusion provided by 'EstimateOccludedDamage'
        // function.
        candidate.damage_area_estimate
            = candidate_factory.EstimateVisibleDamage(quad_candidate, candidate, quad_list.begin(), std::next(quad_list.begin(), occluder_iter_count));
...
TEST_F(UnderlayTest, ProtectedVideoOverlayScaling)
{
    ...
    for (auto initial_scaling : initial_scalings) {
        ...
        CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(),
            pass->output_rect, gfx::ProtectedVideoType::kHardwareProtected, MultiPlaneFormat::kNV12, gfx::ScaleToRoundedSize(kDisplaySize, res_scale))
            ->needs_blending
            = false;
        pass->shared_quad_state_list.front()->opacity = 1.0;
...
TEST_F(UnderlayTest, DisableOverlayWithRootCopies)
{
    ...
    constexpr int kDisableOverlayTestVectorSize = OverlayProcessorUsingStrategy::kCopyRequestSkipOverlayFrames + 5;
    ...
    for (int i = kDisableOverlayTestVectorSize - 3; i < kDisableOverlayTestVectorSize; i++) {
        expected_overlays[i] = true;
    }

    base::test::ScopedFeatureList feature_list;
    feature_list.InitAndEnableFeature(features::kTemporalSkipOverlaysWithRootCopyOutputRequests);
    ...
    for (size_t i = 0; i < copy_frames.size(); ++i) {
        ...
        CreateOpaqueQuadAt(resource_provider_.get(), render_pass_list[0]->shared_quad_state_list.back(), render_pass_list[0].get(),
            render_pass_list[0]->output_rect, SkColors::kWhite);

        CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            render_pass_list[0]->shared_quad_state_list.back(), render_pass_list[0].get(), kOverlayRect);

        // Add copy output request to root render pass on certain frames.
        if (copy_frames[i]) {
            render_pass_list[0]->copy_requests.push_back(CopyOutputRequest::CreateStubForTesting());
        }

        OverlayCandidateList candidate_list;
        overlay_processor_->ProcessForOverlays(resource_provider_.get(), &render_pass_list, GetIdentityColorMatrix(), render_pass_filters,
            render_pass_backdrop_filters, surface_damage_rect_list, primary_plane, &candidate_list, &damage_rect_, &content_bounds_);
...
TileDrawQuad* CreateTileCandidateQuadAt(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    RasterContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass, const gfx::Rect& rect)
{
    bool needs_blending = false;
    bool premultiplied_alpha = false;
    ...
    bool nearest_neighbor = false;
    bool is_overlay_candidate = true;
    ResourceId resource_id = CreateResource(parent_resource_provider, child_resource_provider, child_context_provider, rect.size(), is_overlay_candidate,
        SinglePlaneFormat::kRGBA_8888, SurfaceId());

    auto* overlay_quad = render_pass->CreateAndAppendDrawQuad<TileDrawQuad>();
    overlay_quad->SetNew(shared_quad_state, rect, rect, needs_blending, resource_id, gfx::RectF(0, 0, 1, 1), rect.size(), premultiplied_alpha, nearest_neighbor,
...
public:
    ...
    OverlayProcessorInterface::OutputSurfaceOverlayPlane* GetDefaultPrimaryPlane()
    {
...
TEST_F(DelegatedTest, ForwardMultipleBasic)
{
    ...
    pass_list.push_back(std::move(pass));
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), overlay_processor_->GetDefaultPrimaryPlane(), &candidate_list, &damage_rect_, &content_bounds_);
...
TEST_F(DelegatedTest, BlockDelegationWithNonRootCopies)
{
    ...
    CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), child_pass->shared_quad_state_list.back(),
        child_pass.get(), kOverlayTopLeftRect);

    auto root_pass = CreateRenderPass();
    AggregatedRenderPassDrawQuad* quad = root_pass->CreateAndAppendDrawQuad<AggregatedRenderPassDrawQuad>();
    quad->SetNew(root_pass->shared_quad_state_list.back(), kOverlayRect, kOverlayRect, child_id, kInvalidResourceId, gfx::RectF(), gfx::Size(),
        gfx::Vector2dF(1, 1), gfx::PointF(), gfx::RectF(), false, 1.0f);
    ...
    std::vector<bool> copy_frames = { true, false, true, false, false, false, false };
    ...
    for (size_t i = 0; i < copy_frames.size(); ++i) {
        ...
        if (copy_frames[i]) {
            pass_list[0]->copy_requests.push_back(CopyOutputRequest::CreateStubForTesting());
        }

        OverlayCandidateList candidate_list;
        overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters,
            render_pass_backdrop_filters, surface_damage_rect_list, overlay_processor_->GetDefaultPrimaryPlane(), &candidate_list, &damage_rect_,
            &content_bounds_);
...
TEST_F(DelegatedTest, TestClipHandCrafted)
{
    ...
    EXPECT_EQ(1U, candidate_list.size());
    EXPECT_RECTF_NEAR(gfx::RectF(kTestClip), candidate_list[0].display_rect, 0.01f);
...
TEST_F(DelegatedTest, TestClipComputed)
{
    ...
    gfx::RectF uv_rect = cc::MathUtil::ScaleRectProportional(
        BoundingRect(tex_rect->uv_top_left, tex_rect->uv_bottom_right), gfx::RectF(kSmallCandidateRect), gfx::RectF(expected_rect));
    EXPECT_RECTF_NEAR(gfx::RectF(expected_rect), candidate_list[0].display_rect, 0.01f);
    EXPECT_RECTF_NEAR(uv_rect, candidate_list[0].uv_rect, 0.01f);
}
...
TEST_F(DelegatedTest, TestClipAggregateRenderPass)
{
    ...
    AggregatedRenderPassId render_pass_id { 3 };
    AggregatedRenderPassDrawQuad* quad = pass->CreateAndAppendDrawQuad<AggregatedRenderPassDrawQuad>();
    quad->SetNew(pass->shared_quad_state_list.back(), kSmallCandidateRect, kSmallCandidateRect, render_pass_id, kInvalidResourceId, gfx::RectF(), gfx::Size(),
        gfx::Vector2dF(1, 1), gfx::PointF(), gfx::RectF(), false, 1.0f);
    ...
    EXPECT_RECTF_NEAR(gfx::RectF(5, 15, 65, 59), candidate_list[0].display_rect, 0.01f);
...
TEST_F(DelegatedTest, TestClipWithPrimary)
{
    ...
    const auto kOversizedCandidateRect = gfx::Rect(kDisplaySize.height() * 2, kDisplaySize.width() * 2);
    auto* tex_rect = CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(), kOversizedCandidateRect);
    tex_rect->uv_bottom_right = gfx::PointF(1, 1);
    tex_rect->uv_top_left = gfx::PointF(0, 0);
    // Check for potential candidates.
    OverlayCandidateList candidate_list;
    ...
    EXPECT_RECTF_NEAR(gfx::RectF(gfx::Rect(kDisplaySize)), candidate_list[0].display_rect, 0.01f);
    EXPECT_RECTF_NEAR(uv_rect, candidate_list[0].uv_rect, 0.01f);
}
...
TEST_F(DelegatedTest, ScaledBufferDamage)
{
    ...
    // quad.
    CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(),
        kSmallCandidateRect, gfx::ProtectedVideoType::kClear, SinglePlaneFormat::kRGBA_8888, kResourceSize);
    ...
    EXPECT_RECTF_NEAR(overlay_processor_->GetUnassignedDamage(), gfx::RectF(kDisplayDamage), 0.0001f);
}
...
TEST_F(DelegatedTest, QuadTypes)
{
    ...
    CreateTileCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kSmallCandidateRect);
    CreateSolidColorQuadAt(pass->shared_quad_state_list.back(), SkColors::kDkGray, pass.get(), kOverlayBottomRightRect);
    // Check for potential candidates.
    OverlayCandidateList candidate_list;
    OverlayProcessorInterface::FilterOperationsMap render_pass_filters;
    OverlayProcessorInterface::FilterOperationsMap render_pass_backdrop_filters;
...
TEST_F(DelegatedTest, NonAxisAlignedCandidateStatus)
{
    ...
    auto candidate_factory = OverlayCandidateFactory(
        pass.get(), resource_provider_.get(), &surface_damage_rect_list, &color_mat, gfx::RectF(pass->output_rect), &render_pass_filters, context);

    pass->shared_quad_state_list.back()->quad_to_target_transform = MakePerspectiveTransform();
    EXPECT_EQ(OverlayCandidate::CandidateStatus::kFailNotAxisAligned3dTransform, candidate_factory.FromDrawQuad(quad, candidate));

    pass->shared_quad_state_list.back()->quad_to_target_transform = MakeShearTransform();
    EXPECT_EQ(OverlayCandidate::CandidateStatus::kFailNotAxisAligned2dShear, candidate_factory.FromDrawQuad(quad, candidate));

    pass->shared_quad_state_list.back()->quad_to_target_transform = MakeRotationTransform();
    EXPECT_EQ(OverlayCandidate::CandidateStatus::kFailNotAxisAligned2dRotation, candidate_factory.FromDrawQuad(quad, candidate));
}
...
TEST_F(MultiUnderlayTest, DamageWhenDemotingTwoUnderlays)
{
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        }
        {
            ...
            CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get(), kTopRight);
...
TEST_F(MultiOverlayTest, DamageOnlyForNewUnderlays)
{
    ...
    for (int i = 0; i < 5; ++i) {
...

        {
            ...
            // be an underlay.
            CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kMidRight);
        }
        {
            // Create a candidate in the top left.
            auto* sqs = pass->CreateAndAppendSharedQuadState();
...
TEST_F(MultiOverlayTest, DamageMaskFilterChange)
{
    ...
    for (int i = 0; i < 7; ++i) {
        ...
        }
        {
            ...
            CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get(), kBottomLeft);
...
TEST_F(MultiOverlayTest, DamageOccluded)
{
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        }
        {
            ...
            sqs->overlay_damage_index = surface_damage_rect_list.size();
            surface_damage_rect_list.emplace_back(kBottomLeft);
            CreateTransparentCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get(), kBottomLeft);
        ...
        }
        {
            ...
            CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get(), kUnderBottomLeft);
        ...
        }
        {
            ...
            CreateOpaqueQuadAt(resource_provider_.get(), sqs, pass.get(), kUnderTopRight);
        }
        {
            // Add something behind it.
            auto* sqs = pass->CreateAndAppendSharedQuadState();
...
{
    constexpr gfx::Rect kCandRects[] { { 0, 0, 64, 64 }, { 64, 0, 64, 64 }, { 0, 64, 64, 64 }, { 64, 64, 64, 64 }, { 0, 128, 64, 64 }, { 64, 128, 64, 64 } };
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get(), kCandRects[i]);
...
TEST_F(MultiOverlayTest, RoundedDisplayMaskCandidateFailsToPromote)
{
    ...
    auto pass = CreateRenderPass();
    SurfaceDamageRectList surface_damage_rect_list;
    const auto kRoundedDisplayMaskInfo = RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(16, 16);

    // Create a candidate with rounded display masks in top left.
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, kRoundedDisplayMaskLeftRect, kRoundedDisplayMaskInfo);

    // Create a candidate with rounded display masks in top right.
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, kRoundedDisplayMaskRightRect, kRoundedDisplayMaskInfo);

    // This candidate is occluded by top left candidate with rounded display
    // masks.
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kNormalLeft);

    // This candidate is occluded by top right candidate with rounded display
    // masks.
    CreateCandidateQuadAt(
        resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kNormalRight);
    ...
    // draw order.
    EXPECT_EQ(gfx::ToRoundedRect(candidate_list[2].display_rect), kRoundedDisplayMaskLeftRect);
    EXPECT_EQ(candidate_list[2].plane_z_order, 2);
}

TEST_F(MultiOverlayTest, DontPromoteCandidatesWithMasksIfAreOnlyOverlayCandidatesNotRejected)
{
    overlay_processor_->SetMaximumOverlaysConsidered(6);
...
TEST_F(AllowCandidateWithMasksSortedMultiOverlayTest, DontPromoteCandidatesWithMasksIfAreOnlySingleOnTopCandidates)
{
    auto pass = CreateRenderPass();
    ...
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, kOverlayTopRightRect, RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(10, 0));
...
TEST_F(TypeAndSizeSortedMultiOverlayTest, PrioritizationWithRoundedDisplayMasks)
{
    ...
    constexpr std::pair<gfx::Rect, OverlayCandidateType> kCandidatesInfo[] { { kRoundedDisplayMaskRectSmall, OverlayCandidateType::kHasRoundedDisplayMasks },
        { kRoundedDisplayMaskRectSmallest, OverlayCandidateType::kHasRoundedDisplayMasks }, { kNormalBiggest, OverlayCandidateType::kNormal },
        { kNormalSmallest, OverlayCandidateType::kNormal }, { kNormalBig, OverlayCandidateType::kNormal }, { kNormalSmall, OverlayCandidateType::kNormal } };
    ...
    for (auto info : kCandidatesInfo) {
        ...
        case kNormal:
            CreateCandidateQuadAt(
                resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), info.first);
            break;
        case kHasRoundedDisplayMasks:
            CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
                pass->shared_quad_state_list.back(), pass.get(),
    ...
    // Candidates with masks are appended at the end of the `candidate_list` in
    // draw order.
    EXPECT_EQ(gfx::ToRoundedRect(candidate_list[2].display_rect), kRoundedDisplayMaskRectSmall);
    EXPECT_EQ(candidate_list[2].plane_z_order, 2);
    EXPECT_EQ(gfx::ToRoundedRect(candidate_list[3].display_rect), kRoundedDisplayMaskRectSmallest);
...
TEST_F(TransitionOverlayTypeTest, FullscreenFavored)
{
    ...
    for (int i = 0; i <= kLastIter; i++) {
        ...
        CreateSolidColorQuadAt(small_quad_sqs, SkColors::kWhite, pass.get(), kSmall);

        auto* fullscreen_sqs = pass->CreateAndAppendSharedQuadState();
        CreateFullscreenCandidateQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), fullscreen_sqs, pass.get());
...
TEST_F(SizeSortedMultiOverlayTest, OverlaysAreSorted)
{
    ...
    CreateQuadWithRoundedDisplayMasksAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(),
        pass.get(),
        /*is_overlay_candidate=*/true, kSmall, RoundedDisplayMasksInfo::CreateRoundedDisplayMasksInfo(16, 16));

    for (auto& cand_rect : kCandRects) {
        CreateCandidateQuadAt(
            resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), cand_rect);
    }
...
TEST_F(SizeSortedMultiUnderlayOverlayTest, UnderlaysAreSorted)
{
    ...
    for (auto& cand_rect : kCandRects) {
        ...
        CreateCandidateQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass.get(), cand_rect);
    }
    // Candidates will be sorted by surface area. All will be promoted.
    overlay_processor_->AddExpectedRect(kBiggest, true);
    overlay_processor_->AddExpectedRect(kBig, true);
...
class MultiUnderlayPromotedTest : public MultiUnderlayTest, public testing::WithParamInterface<bool> { };
...
INSTANTIATE_TEST_SUITE_P(PromotedTrueFalse, MultiUnderlayPromotedTest, testing::Bool());
