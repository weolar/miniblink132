...
struct ProposedCandidateKey {
    OverlayCandidate::TrackingId tracking_id = OverlayCandidate::kDefaultTrackingId;
    OverlayStrategy strategy_id = OverlayStrategy::kUnknown;

    bool operator==(const ProposedCandidateKey& other) const
    {
        return (tracking_id == other.tracking_id && strategy_id == other.strategy_id);
    }
...
struct ProposedCandidateKeyHasher {
    std::size_t operator()(const ProposedCandidateKey& k) const
    {
        ...
        static_assert(sizeof(ProposedCandidateKey) == sizeof(decltype(k.strategy_id)) + sizeof(decltype(k.tracking_id)));
...
public:
    OverlayProposedCandidate(QuadList::Iterator it, OverlayCandidate overlay_candidate, OverlayProcessorStrategy* overlay_strategy);
    ...
    static ProposedCandidateKey ToProposeKey(const OverlayProposedCandidate& proposed);
