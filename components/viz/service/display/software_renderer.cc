...
public:
    AnimatedImagesProvider(const PictureDrawQuad::ImageAnimationMap* image_animation_map)
        : image_animation_map_(image_animation_map)
    {
    }
    ~AnimatedImagesProvider() override = default;

    ImageProvider::ScopedResult GetRasterContent(const cc::DrawImage& draw_image) override
    {
        ...
        size_t frame_index = it == image_animation_map_->end() ? cc::PaintImage::kDefaultFrameIndex : it->second;
        return ScopedResult(cc::DecodedDrawImage(paint_image.GetSkImageForFrame(frame_index, cc::PaintImage::kDefaultGeneratorClientId), nullptr,
            SkSize::Make(0, 0), SkSize::Make(1.f, 1.f), draw_image.filter_quality(), true /* is_budgeted */));
    }
...
SoftwareRenderer::SoftwareRenderer(const RendererSettings* settings, const DebugRendererSettings* debug_settings, OutputSurface* output_surface,
    DisplayResourceProviderSoftware* resource_provider, OverlayProcessorInterface* overlay_processor)
...
void SoftwareRenderer::SetClipRect(const gfx::Rect& rect)
{
    ...
    DCHECK_EQ(current_canvas_->getDeviceClipBounds().width(), current_canvas_->imageInfo().width());
    DCHECK_EQ(current_canvas_->getDeviceClipBounds().height(), current_canvas_->imageInfo().height());
#endif
...
void SoftwareRenderer::SetClipRRect(const gfx::RRectF& rrect)
{
    ...
    SkRRect result;
    if (SkRRect(rrect).transform(gfx::AxisTransform2dToSkMatrix(current_frame()->target_to_device_transform), &result)) {
        // Skia applies the current matrix to clip rects so we reset it temporarily.
        SkMatrix current_matrix = current_canvas_->getTotalMatrix();
        current_canvas_->resetMatrix();
    ...
    const AggregatedRenderPass* render_pass, bool needs_clear, const gfx::Rect& render_pass_update_rect, const gfx::Size& viewport_size)
{
    if (render_pass == current_frame()->root_render_pass) {
        ...
        current_framebuffer_canvas_ = std::make_unique<SkCanvas>(bitmap, skia::LegacyDisplayGlobals::GetSkSurfaceProps());
...
void SoftwareRenderer::DoDrawQuad(const DrawQuad* quad, const gfx::QuadF* draw_region)
{
    if (!current_canvas_)
        return;
    ...
    bool do_save = draw_region || is_scissor_enabled_ || should_apply_rounded_corner;
    ...
    if (should_apply_rounded_corner)
        SetClipRRect(quad->shared_quad_state->mask_filter_info.rounded_corner_bounds());

    gfx::Transform quad_rect_matrix;
    QuadRectTransform(&quad_rect_matrix, quad->shared_quad_state->quad_to_target_transform, gfx::RectF(quad->rect));
    gfx::Transform contents_device_transform = quad_rect_matrix;
    contents_device_transform.PostConcat(current_frame()->target_to_device_transform);
    contents_device_transform.Flatten();
    SkMatrix sk_device_matrix = gfx::TransformToFlattenedSkMatrix(contents_device_transform);
    ...
    if (draw_region) {
        ...
        local_draw_region.Scale(1.0f / quad->visible_rect.width(), 1.0f / quad->visible_rect.height());
...
{
    current_canvas_->concat(SkMatrix::RectToRect(gfx::RectFToSkRect(quad->tex_coord_rect), gfx::RectFToSkRect(QuadVertexRect())));
    ...
    if (needs_transparency || disable_image_filtering) {
        ...
        opacity_canvas.emplace(raster_canvas, quad->shared_quad_state->opacity, disable_image_filtering);
    ...
    quad->display_item_list->Raster(raster_canvas, &image_provider, &quad->raster_inducing_scroll_offsets);
    raster_canvas->restore();
}

void SoftwareRenderer::DrawSolidColorQuad(const SolidColorDrawQuad* quad)
{
    gfx::RectF visible_quad_vertex_rect = cc::MathUtil::ScaleRectProportional(QuadVertexRect(), gfx::RectF(quad->rect), gfx::RectF(quad->visible_rect));
    current_paint_.setColor(quad->color);
    current_paint_.setAlphaf(quad->shared_quad_state->opacity * quad->color.fA);
    current_canvas_->drawRect(gfx::RectFToSkRect(visible_quad_vertex_rect), current_paint_);
}
...
void SoftwareRenderer::DrawTextureQuad(const TextureDrawQuad* quad)
{
    ...
    DisplayResourceProviderSoftware::ScopedReadLockSkImage lock(
        resource_provider(), quad->resource_id(), quad->premultiplied_alpha ? kPremul_SkAlphaType : kUnpremul_SkAlphaType);
    ...
    gfx::RectF visible_uv_rect = cc::MathUtil::ScaleRectProportional(uv_rect, gfx::RectF(quad->rect), gfx::RectF(quad->visible_rect));
    ...
    bool blend_background = quad->background_color != SkColors::kTransparent && !image->isOpaque();
    ...
    SkSamplingOptions sampling(quad->nearest_neighbor ? SkFilterMode::kNearest : SkFilterMode::kLinear);
    current_canvas_->drawImageRect(image, sk_uv_rect, quad_rect, sampling, &current_paint_, SkCanvas::kStrict_SrcRectConstraint);
    if (needs_layer)
...
void SoftwareRenderer::DrawTileQuad(const TileDrawQuad* quad)
{
    ...
    DisplayResourceProviderSoftware::ScopedReadLockSkImage lock(
        resource_provider(), quad->resource_id(), quad->is_premultiplied ? kPremul_SkAlphaType : kUnpremul_SkAlphaType);
    if (!lock.valid())
        return;

    gfx::RectF visible_tex_coord_rect = cc::MathUtil::ScaleRectProportional(quad->tex_coord_rect, gfx::RectF(quad->rect), gfx::RectF(quad->visible_rect));
    gfx::RectF visible_quad_vertex_rect = cc::MathUtil::ScaleRectProportional(QuadVertexRect(), gfx::RectF(quad->rect), gfx::RectF(quad->visible_rect));

    SkRect uv_rect = gfx::RectFToSkRect(visible_tex_coord_rect);
    SkSamplingOptions sampling(quad->nearest_neighbor ? SkFilterMode::kNearest : SkFilterMode::kLinear);
    current_canvas_->drawImageRect(
        lock.sk_image(), uv_rect, gfx::RectFToSkRect(visible_quad_vertex_rect), sampling, &current_paint_, SkCanvas::kStrict_SrcRectConstraint);
    ...
    if (software_toggle_capture()) {
        ...
        if (lock.sk_image()->peekPixels(&pixmap)) {
            ...
            visible_tex_coord_rect.Scale(1.0f / pixmap.width(), 1.0f / pixmap.height());
            DBG_DRAW_RECT_BUFF_UV("software.tile.buffer", display_rect, &buff_id, visible_tex_coord_rect);
            VizDebugger::BufferInfo buffer_info;
            buffer_info.bitmap.setInfo(SkImageInfo::Make(pixmap.width(), pixmap.height(), kBGRA_8888_SkColorType, kPremul_SkAlphaType));
...
void SoftwareRenderer::DrawRenderPassQuad(const AggregatedRenderPassDrawQuad* quad)
{
    ...
    SkRect dest_visible_rect
        = gfx::RectFToSkRect(cc::MathUtil::ScaleRectProportional(QuadVertexRect(), gfx::RectF(quad->rect), gfx::RectF(quad->visible_rect)));
    ...
    if (filters) {
        ...
        auto image_filter = paint_filter ? paint_filter->cached_sk_filter_ : nullptr;
        if (image_filter) {
            SkIRect result_rect;
            ...
            if (filter_image) {
                ...
                dest_rect = dest_visible_rect = gfx::RectFToSkRect(cc::MathUtil::ScaleRectProportional(QuadVertexRect(), gfx::RectF(quad->rect), rect));
    ...
    if (quad->mask_resource_id()) {
        DisplayResourceProviderSoftware::ScopedReadLockSkImage mask_lock(resource_provider(), quad->mask_resource_id(), kPremul_SkAlphaType);
        if (!mask_lock.valid())
            return;
        ...
        current_paint_.setMaskFilter(SkShaderMaskFilter::Make(mask_lock.sk_image()->makeShader(current_sampling_, mask_mat)));
    }

    // If we have a backdrop filter shader, render its results first.
    sk_sp<SkShader> backdrop_filter_shader = GetBackdropFilterShader(quad, SkTileMode::kClamp);
...
#endif
    current_paint_.setAlpha(quad->shared_quad_state->opacity * 255);
    current_canvas_->drawRect(gfx::RectFToSkRect(QuadVertexRect()), current_paint_);
}

void SoftwareRenderer::CopyDrawnRenderPass(const copy_output::RenderPassGeometry& geometry, std::unique_ptr<CopyOutputRequest> request)
{
    ...
    if (request->is_scaled()) {
            ...
            return;
        {
            render_pass_output = SkPixmap(
                render_pass_output.info().makeWH(geometry.sampling_bounds.width(), geometry.sampling_bounds.height()).makeColorSpace(std::move(color_space)),
                render_pass_output.addr(geometry.sampling_bounds.x(), geometry.sampling_bounds.y()), render_pass_output.rowBytes());
        }
        ...
        const bool is_downscale_in_both_dimensions = request->scale_to().x() < request->scale_from().x() && request->scale_to().y() < request->scale_from().y();
        const ImageOperations::ResizeMethod method = is_downscale_in_both_dimensions ? ImageOperations::RESIZE_BETTER : ImageOperations::RESIZE_BEST;
        bitmap = ImageOperations::Resize(render_pass_output, method, geometry.result_bounds.width(), geometry.result_bounds.height(),
            SkIRect { geometry.result_selection.x(), geometry.result_selection.y(), geometry.result_selection.right(), geometry.result_selection.bottom() });
    } else /* if (!request->is_scaled()) */ {
        SkImageInfo info = SkImageInfo::MakeN32Premul(geometry.result_selection.width(), geometry.result_selection.height(), std::move(color_space));
        if (!bitmap.tryAllocPixels(info))
            return;

        if (!current_canvas_->readPixels(bitmap, geometry.readback_offset.x(), geometry.readback_offset.y()))
            return;
    }
    ...
    if (request->result_destination() == CopyOutputResult::Destination::kNativeTextures && request->has_blit_request()) {
        *(int*)1 = 1;
//         const auto& blit_request = request->blit_request();
// 
//         auto representation = resource_provider()->GetSharedImageRepresentation(blit_request.mailbox(), blit_request.sync_token());
// 
//         if (!representation) {
...
//         }
// 
//         const auto dest_rect = gfx::Rect(blit_request.destination_region_offset(), gfx::Size(bitmap.width(), bitmap.height()));
// 
...
//         }
// 
//         auto surface = SkSurfaces::WrapPixels(read_access->pixmap().info(), read_access->pixmap().writable_addr(), read_access->pixmap().rowBytes(), nullptr);
// 
//         CHECK(surface);
// 
//         if (blit_request.letterboxing_behavior() == LetterboxingBehavior::kLetterbox) {
//             surface->getCanvas()->clear(SK_ColorBLACK);
//         }
// 
//         SkPaint blit_request_paint;
//         blit_request_paint.setBlendMode(SkBlendMode::kSrc);
//         surface->getCanvas()->drawImage(SkImages::RasterFromBitmap(bitmap), dest_rect.x(), dest_rect.y(), SkSamplingOptions(), &blit_request_paint);
// 
...
// 
//             surface->getCanvas()->drawImageRect(blend_bitmap.image(), gfx::RectToSkRect(blend_bitmap.source_region()),
//                 gfx::RectToSkRect(blend_bitmap.destination_region()), SkSamplingOptions(SkFilterMode::kLinear), &blend_bitmap_paint,
//                 SkCanvas::kFast_SrcRectConstraint);
//         }
// 
//         request->SendResult(std::make_unique<CopyOutputTextureResult>(CopyOutputResult::Format::RGBA, geometry.result_selection,
//             CopyOutputResult::TextureResult(request->blit_request().mailbox(), representation->color_space()), CopyOutputResult::ReleaseCallbacks()));
    ...
    request->SendResult(std::make_unique<CopyOutputSkBitmapResult>(request->result_format(), geometry.result_selection, std::move(bitmap)));
}
...
bool SoftwareRenderer::ShouldApplyBackdropFilters(const cc::FilterOperations* backdrop_filters, const AggregatedRenderPassDrawQuad* quad) const
{
    if (!backdrop_filters)
        return false;
...
sk_sp<SkImage> SoftwareRenderer::ApplyImageFilter(
    SkImageFilter* filter, const AggregatedRenderPassDrawQuad* quad, const SkBitmap& to_filter, bool offset_expanded_bounds, SkIRect* result_rect) const
{
    DCHECK(result_rect);
    if (!filter)
        return nullptr;
    ...
    *result_rect = filter->filterBounds(gfx::RectToSkIRect(quad->rect), local_matrix, SkImageFilter::kForward_MapDirection);
    gfx::Point canvas_offset = offset_expanded_bounds ? gfx::Point(result_rect->x(), result_rect->y()) : quad->rect.origin();
    SkImageInfo dst_info = SkImageInfo::MakeN32Premul(result_rect->width(), result_rect->height());
    ...
    surface->getCanvas()->drawImage(to_filter.asImage(), quad->rect.x(), quad->rect.y(), SkSamplingOptions(), &paint);
    return surface->makeImageSnapshot();
}

SkBitmap SoftwareRenderer::GetBackdropBitmap(const gfx::Rect& bounding_rect) const
{
    SkImageInfo info = SkImageInfo::MakeN32Premul(bounding_rect.width(), bounding_rect.height());
    SkBitmap bitmap;
    if (!bitmap.tryAllocPixels(info))
        base::TerminateBecauseOutOfMemory(info.computeMinByteSize());

    if (!current_canvas_->readPixels(bitmap, bounding_rect.x(), bounding_rect.y()))
        bitmap.reset();
    return bitmap;
}

gfx::Rect SoftwareRenderer::GetBackdropBoundingBoxForRenderPassQuad(const AggregatedRenderPassDrawQuad* quad, const cc::FilterOperations* backdrop_filters,
    std::optional<gfx::RRectF> backdrop_filter_bounds_input, gfx::Transform contents_device_transform, gfx::Transform* backdrop_filter_bounds_transform,
    std::optional<gfx::RRectF>* backdrop_filter_bounds, gfx::Rect* unclipped_rect) const
{
    ...
    *backdrop_filter_bounds = gfx::RRectF();
    if (!backdrop_filter_bounds_input || !GetScaledRRectF(quad->rect, backdrop_filter_bounds_input.value(), &backdrop_filter_bounds->value())) {
    ...
    gfx::Rect backdrop_rect = gfx::ToEnclosingRect(cc::MathUtil::MapClippedRect(contents_device_transform, QuadVertexRect()));

    *unclipped_rect = backdrop_rect;
    backdrop_rect.Intersect(MoveFromDrawToWindowSpace(current_frame()->current_render_pass->output_rect));

    // Shift to the space of the captured backdrop image.
    *backdrop_filter_bounds_transform = contents_device_transform;
    backdrop_filter_bounds_transform->PostTranslate(-backdrop_rect.x(), -backdrop_rect.y());

    return backdrop_rect;
}

sk_sp<SkShader> SoftwareRenderer::GetBackdropFilterShader(const AggregatedRenderPassDrawQuad* quad, SkTileMode content_tile_mode) const
{
    ...
    if (!ShouldApplyBackdropFilters(backdrop_filters, quad))
        return nullptr;
    std::optional<gfx::RRectF> backdrop_filter_bounds_input = BackdropFilterBoundsForPass(quad->render_pass_id);

    if (backdrop_filter_bounds_input.has_value()) {
        backdrop_filter_bounds_input->Scale(quad->filters_scale.x(), quad->filters_scale.y());
    }
    ...
    gfx::Rect backdrop_rect = GetBackdropBoundingBoxForRenderPassQuad(quad, backdrop_filters, backdrop_filter_bounds_input, contents_device_transform,
        &backdrop_filter_bounds_transform, &backdrop_filter_bounds, &unclipped_rect);
    ...
    SkMatrix filter_backdrop_transform = gfx::TransformToFlattenedSkMatrix(contents_device_transform_inverse);
    ...
    if (backdrop_filter_bounds.has_value()) {
        gfx::Rect filter_clip = gfx::ToEnclosingRect(cc::MathUtil::MapClippedRect(backdrop_filter_bounds_transform, backdrop_filter_bounds->rect()));
        filter_clip.Intersect(gfx::Rect(backdrop_bitmap.width(), backdrop_bitmap.height()));
        ...
        cropped_image = cropped_image->makeSubset(static_cast<GrDirectContext*>(nullptr), RectToSkIRect(filter_clip));
    ...
    // to BuildImageFilter().
    sk_sp<cc::PaintFilter> paint_filter = cc::RenderSurfaceFilters::BuildImageFilter(*backdrop_filters);
    if (!paint_filter)
        return nullptr;
    ...
    // check.
    DCHECK(!FiltersForPass(quad->render_pass_id)) << "Filters should always be in a separate Effect node";

    // TODO(crbug.com/40036319): Software renderer does not support/implement
    // kClamp_TileMode.
    SkIRect result_rect;
    sk_sp<SkImage> filtered_image = ApplyImageFilter(filter.get(), quad, backdrop_bitmap,
        /* offset_expanded_bounds = */ false, &result_rect);
    if (!filtered_image)
        return nullptr;

    // Use an SkBitmap to paint the rrect-clipped filtered image.
    SkImageInfo info = SkImageInfo::MakeN32Premul(backdrop_rect.width(), backdrop_rect.height());
    SkBitmap bitmap;
    if (!bitmap.tryAllocPixels(info))
        base::TerminateBecauseOutOfMemory(info.computeMinByteSize());
    ...
    if (backdrop_filter_bounds) {
        canvas.setMatrix(gfx::TransformToFlattenedSkMatrix(backdrop_filter_bounds_transform));
        canvas.clipRRect(SkRRect(*backdrop_filter_bounds), SkClipOp::kIntersect, true /* antialias */);
        canvas.resetMatrix();
    }
    ...
    if (quad->shared_quad_state->opacity < 1.0) {
        paint.setImageFilter(SkiaHelper::BuildOpacityFilter(quad->shared_quad_state->opacity));
    }

    // Now paint the pre-filtered image onto the canvas.
    SkRect src_rect = SkRect::MakeXYWH(0, 0, backdrop_bitmap.width(), backdrop_bitmap.height());
    SkRect dst_rect = src_rect.makeOffset(image_offset.x(), image_offset.y());
    canvas.drawImageRect(filtered_image, src_rect, dst_rect, SkSamplingOptions(), &paint, SkCanvas::kStrict_SrcRectConstraint);

    return SkImages::RasterFromBitmap(bitmap)->makeShader(content_tile_mode, content_tile_mode, SkSamplingOptions(), &filter_backdrop_transform);
}
    ...
    const AggregatedRenderPassList& render_passes_in_draw_order, const base::flat_map<AggregatedRenderPassId, RenderPassRequirements>& render_passes_in_frame)
{
    ...
    for (const auto& pair : render_pass_bitmaps_) {
        ...
        bool size_appropriate = bitmap.width() >= required_size.width() && bitmap.height() >= required_size.height();
        if (!size_appropriate)
            passes_to_delete.push_back(pair.first);
    }
...
void SoftwareRenderer::AllocateRenderPassResourceIfNeeded(const AggregatedRenderPassId& render_pass_id, const RenderPassRequirements& requirements)
{
    auto it = render_pass_bitmaps_.find(render_pass_id);
    if (it != render_pass_bitmaps_.end()) {
        DCHECK(it->second.bitmap.width() >= requirements.size.width() && it->second.bitmap.height() >= requirements.size.height());
        return;
    }
    ...
    SkImageInfo info = SkImageInfo::MakeN32(requirements.size.width(), requirements.size.height(), kPremul_SkAlphaType);
    SkBitmap bitmap;
    if (!bitmap.tryAllocPixels(info))
        base::TerminateBecauseOutOfMemory(info.computeMinByteSize());
...
bool SoftwareRenderer::IsRenderPassResourceAllocated(const AggregatedRenderPassId& render_pass_id) const
{
    auto it = render_pass_bitmaps_.find(render_pass_id);
    return it != render_pass_bitmaps_.end();
}

gfx::Size SoftwareRenderer::GetRenderPassBackingPixelSize(const AggregatedRenderPassId& render_pass_id)
{
    SkBitmap& bitmap = render_pass_bitmaps_.at(render_pass_id).bitmap;
    return gfx::Size(bitmap.width(), bitmap.height());
}

void SoftwareRenderer::SetRenderPassBackingDrawnRect(const AggregatedRenderPassId& render_pass_id, const gfx::Rect& drawn_rect)
{
    render_pass_bitmaps_.at(render_pass_id).drawn_rect = drawn_rect;
}

gfx::Rect SoftwareRenderer::GetRenderPassBackingDrawnRect(const AggregatedRenderPassId& render_pass_id) const
{
    auto it = render_pass_bitmaps_.find(render_pass_id);
    if (it != render_pass_bitmaps_.end()) {
