...
public:
    explicit DisplayCompositorMemoryAndTaskController(std::unique_ptr<SkiaOutputSurfaceDependency> skia_dependency);
    DisplayCompositorMemoryAndTaskController(const DisplayCompositorMemoryAndTaskController&) = delete;
    DisplayCompositorMemoryAndTaskController& operator=(const DisplayCompositorMemoryAndTaskController&) = delete;
...
private:
    void InitializeOnGpu(SkiaOutputSurfaceDependency* skia_dependency, base::WaitableEvent* event);
...
//     std::unique_ptr<gpu::DisplayCompositorMemoryAndTaskControllerOnGpu> controller_on_gpu_;
// 
