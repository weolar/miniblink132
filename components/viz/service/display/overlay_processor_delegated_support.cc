...
base::expected<std::optional<OverlayCandidate>, DelegationStatus> TryPromoteDrawQuadForDelegation(
    const OverlayCandidateFactory& candidate_factory, const DrawQuad* quad)
{
    ...
    DBG_DRAW_TEXT("delegated.overlay.type", gfx::Vector2dF(display_rect.origin().x(), display_rect.origin().y()),
        base::StringPrintf("m=%d rid=%d", static_cast<int>(quad->material), quad->resources.begin()->value()));

    OverlayCandidate candidate;
    OverlayCandidate::CandidateStatus candidate_status = candidate_factory.FromDrawQuad(quad, candidate);
    ...
    if (candidate_status == OverlayCandidate::CandidateStatus::kSuccess) {
        if (quad->material == DrawQuad::Material::kSolidColor) {
            ...
            if (!candidate.rounded_corners.IsEmpty()) {
                DBG_DRAW_RECT_OPT("delegated.overlay.candidate_rounded_corners", DBG_OPT_BLUE, candidate.rounded_corners.rect());

                using Corner = gfx::RRectF::Corner;
                const Corner corners[] = { Corner::kUpperLeft, Corner::kUpperRight, Corner::kLowerRight, Corner::kLowerLeft };
                for (auto corner : corners) {
                    auto corner_rect = candidate.rounded_corners.CornerBoundingRect(corner);
                    DBG_DRAW_RECT_OPT("delegated.overlay.candidate_rounded_corners", DBG_OPT_RED, corner_rect);
                }
            }
        }
        return base::ok(candidate);
    } else if (candidate_status == OverlayCandidate::CandidateStatus::kFailVisible) {
        return base::ok(std::nullopt);
    } else {
        DBG_DRAW_RECT("delegated.overlay.failed", display_rect);
        DBG_LOG("delegated.overlay.failed", "error code %d", static_cast<int>(candidate_status));
        ...
        case OverlayCandidate::CandidateStatus::kFailQuadNotSupported:
            return base::unexpected(DelegationStatus::kCompositedCandidateQuadMaterial);
        case OverlayCandidate::CandidateStatus::kFailBufferFormat:
            return base::unexpected(DelegationStatus::kCompositedCandidateBufferFormat);
        case OverlayCandidate::CandidateStatus::kFailNearFilter:
            return base::unexpected(DelegationStatus::kCompositedCandidateNearFilter);
        case OverlayCandidate::CandidateStatus::kFailMaskFilterNotSupported:
            return base::unexpected(DelegationStatus::kCompositedCandidateMaskFilter);
        case OverlayCandidate::CandidateStatus::kFailHasTransformButCantClip:
            return base::unexpected(DelegationStatus::kCompositedCandidateTransformCantClip);
        case OverlayCandidate::CandidateStatus::kFailRpdqWithTransform:
            return base::unexpected(DelegationStatus::kCompositedCandidateRpdqWithTransform);
        default:
...
void DebugLogBeforeDelegation(const gfx::Rect& incoming_root_damage, const SurfaceDamageRectList& surface_damage_rect_list)
{
...
void DebugLogAfterDelegation(DelegationStatus status, const OverlayCandidateList& candidates, const gfx::Rect& outgoing_root_damage)
{
    UMA_HISTOGRAM_ENUMERATION("Viz.DelegatedCompositing.Status", status);
    DBG_LOG("delegation_status", "delegation status: %d", static_cast<int>(status));
    ...
    TRACE_EVENT_INSTANT1(
        TRACE_DISABLED_BY_DEFAULT("viz.debug.overlay_planes"), "DelegatedCompositingStatus", TRACE_EVENT_SCOPE_THREAD, "delegated_status", status);
}
