...
public:
    ...
    AggregatedRenderPassId remapped_id() const
    {
        return fixed_.remapped_id;
    }
    ...
    AggregationPassData& aggregation()
    {
        return aggregation_;
    }
    const AggregationPassData& aggregation() const
    {
        return aggregation_;
    }
...
public:
    using OffsetTagLookupFn = base::FunctionRef<gfx::Vector2dF(const OffsetTagDefinition&)>;

    ResolvedFrameData(DisplayResourceProvider* resource_provider, Surface* surface, uint64_t prev_frame_index, AggregatedRenderPassId prev_root_pass_id);
    ~ResolvedFrameData();
    ...
    uint64_t previous_frame_index() const
    {
    ...
    void UpdateForAggregation(AggregatedRenderPassId::Generator& render_pass_id_generator);
    ...
    ResolvedPassData& GetRenderPassDataById(CompositorRenderPassId render_pass_id);
    const ResolvedPassData& GetRenderPassDataById(CompositorRenderPassId render_pass_id) const;
...
private:
    ...
    // will return false.
    void UpdateActiveFrame(AggregatedRenderPassId::Generator& render_pass_id_generator);
    ...
    void MovePersistentPassDataFromPreviousFrame(const std::vector<ResolvedPassData>& previoius_resolved_passes);
    ...
    base::flat_map<CompositorRenderPassId, raw_ptr<ResolvedPassData, CtnExperimental>> render_pass_id_map_;
    base::flat_map<CompositorRenderPassId, AggregatedRenderPassId> aggregated_id_map_;
