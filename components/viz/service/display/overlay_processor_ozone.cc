...
gfx::ColorSpace GetColorSpaceForOzone(gfx::BufferFormat format, const gfx::ColorSpace& orig_color_space)
{
#if BUILDFLAG(IS_CHROMEOS)
    ...
    if ((format == gfx::BufferFormat::YUV_420_BIPLANAR)
        && (!base::FeatureList::IsEnabled(features::kVulkan) || !base::FeatureList::IsEnabled(features::kDefaultANGLEVulkan)
            || !base::FeatureList::IsEnabled(features::kVulkanFromANGLE))) {
        return orig_color_space.GetWithMatrixAndRange(gfx::ColorSpace::MatrixID::SMPTE170M, orig_color_space.GetRangeID());
    }
#endif
...
void ConvertToOzoneOverlaySurface(const OverlayProcessorInterface::OutputSurfaceOverlayPlane& primary_plane, ui::OverlaySurfaceCandidate* ozone_candidate)
{
    ozone_candidate->transform = primary_plane.transform;
    ozone_candidate->format = SinglePlaneSharedImageFormatToBufferFormat(primary_plane.format);
    ozone_candidate->color_space = GetColorSpaceForOzone(ozone_candidate->format, /*orig_color_space=*/primary_plane.color_space);
...
void ConvertToOzoneOverlaySurface(const OverlayCandidate& overlay_candidate, ui::OverlaySurfaceCandidate* ozone_candidate)
{
    ozone_candidate->transform = overlay_candidate.transform;
    ozone_candidate->format = gpu::ToBufferFormat(overlay_candidate.format);
    ozone_candidate->color_space = GetColorSpaceForOzone(ozone_candidate->format,
    ...
    if (!overlay_candidate.is_solid_color && ozone_candidate->background_color && overlay_candidate.color) {
        ozone_candidate->background_color = overlay_candidate.color->toSkColor();
    }
}

void ConvertToTiledOzoneOverlaySurface(const OverlayCandidate& overlay_candidate, ui::OverlaySurfaceCandidate* ozone_candidate)
{
    ...
    ozone_candidate->buffer_size
        = gfx::Size(static_cast<int>(overlay_candidate.display_rect.width()), static_cast<int>(overlay_candidate.display_rect.height()));
...
{
    return (mailbox.name[0] << 24) + (mailbox.name[1] << 16) + (mailbox.name[2] << 8) + mailbox.name[3];
}
...
bool AllowColorSpaceCombination(SharedImageFormat source_format, const gfx::ColorSpace& source_color_space, const gfx::ColorSpace& destination_color_space)
{
    ...
    if ((source_format == MultiPlaneFormat::kNV12 || source_format == MultiPlaneFormat::kYV12 || source_format == MultiPlaneFormat::kP010) && is_yuv_color_space
        && (source_color_space.GetMatrixID() == gfx::ColorSpace::MatrixID::BT2020_NCL || source_color_space.GetRangeID() == gfx::ColorSpace::RangeID::FULL)) {
        return false;
    }
    ...
    if (base::FeatureList::IsEnabled(display::features::kEnableExternalDisplayHDR10Mode)
        && destination_color_space.GetContentColorUsage() == gfx::ContentColorUsage::kHDR) {
        return source_color_space.GetContentColorUsage() == destination_color_space.GetContentColorUsage();
    }
    ...
    // if it comes up.
    return source_color_space.GetContentColorUsage() == gfx::ContentColorUsage::kSRGB
        || source_color_space.GetContentColorUsage() == destination_color_space.GetContentColorUsage();
}
...
void OverlayProcessorOzone::NotifyOverlayPromotion(
    DisplayResourceProvider* display_resource_provider, const OverlayCandidateList& candidate_list, const QuadList& quad_list)
{
    ...
    overlay_candidates_->NotifyOverlayPromotion(std::move(promoted_overlay_types));
}

void OverlayProcessorOzone::CheckOverlaySupportImpl(const OverlayProcessorInterface::OutputSurfaceOverlayPlane* primary_plane, OverlayCandidateList* surfaces)
{
    ...
    ui::OverlayCandidatesOzone::OverlaySurfaceCandidateList ozone_surface_list(full_size);
    ...
    // Convert OverlayCandidateList to OzoneSurfaceCandidateList.
    {
        ...
        if (primary_plane) {
            ...
            if (shared_image_interface_) {
                bool result = SetNativePixmapForCandidate(&(*ozone_surface_iterator), primary_plane->mailbox,
                    /*is_primary=*/true);
#if BUILDFLAG(IS_CHROMEOS_ASH)
                if (!result) {
                    ...
                    for (auto surface_iterator = surfaces->cbegin(); surface_iterator < surfaces->cend(); surface_iterator++) {
                        if (surface_iterator->requires_overlay) {
                            DLOG(WARNING) << "Allowing required overlay with missing primary pixmap";
                            result = true;
                            break;
                        }
                    }
    ...
    // primary plane present.
        ...
        for (; ozone_surface_iterator < ozone_surface_list.end() && surface_iterator < surfaces->cend(); ozone_surface_iterator++, surface_iterator++) {
            if (surface_iterator->needs_detiling) {
                ConvertToTiledOzoneOverlaySurface(*surface_iterator, &(*ozone_surface_iterator));
                continue;
            }

            ConvertToOzoneOverlaySurface(*surface_iterator, &(*ozone_surface_iterator));
#if BUILDFLAG(IS_CHROMEOS_ASH)
...
#endif // BUILDFLAG(IS_CHROMEOS_ASH)
            if (shared_image_interface_) {
                bool result = SetNativePixmapForCandidate(&(*ozone_surface_iterator), surface_iterator->mailbox,
                    /*is_primary=*/false);
#if BUILDFLAG(IS_CHROMEOS_ASH)
        ...
        if (!has_independent_cursor_plane_) {
            auto highest_zindex_surface = std::max_element(
                ozone_surface_list.begin(), ozone_surface_list.end(), [](const auto& a, const auto& b) { return a.plane_z_order < b.plane_z_order; });
            if (highest_zindex_surface != ozone_surface_list.end()) {
                ...
                if (!display_rect.origin().IsOrigin() || buffer_size != gfx::ToFlooredSize(display_rect.size())) {
    ...
    // OverlayCandidateList.
    {
        ...
        for (; surface_iterator < surfaces->end() && ozone_surface_iterator < ozone_surface_list.cend(); surface_iterator++, ozone_surface_iterator++) {
            surface_iterator->overlay_handled = ozone_surface_iterator->overlay_handled;
...
void OverlayProcessorOzone::MaybeObserveHardwareCapabilities()
{
    ...
    if (overlay_candidates_) {
        overlay_candidates_->ObserveHardwareCapabilities(
            base::BindRepeating(&OverlayProcessorOzone::ReceiveHardwareCapabilities, weak_ptr_factory_.GetWeakPtr()));
    }
}

void OverlayProcessorOzone::ReceiveHardwareCapabilities(ui::HardwareCapabilities hardware_capabilities)
{
    if (hardware_capabilities.is_valid) {
        // Subtract 1 because one of these overlay capable planes will be needed for
        // the primary plane.
        int max_overlays_supported = hardware_capabilities.num_overlay_capable_planes - 1;
        max_overlays_considered_ = std::min(max_overlays_supported, max_overlays_config_);
        has_independent_cursor_plane_ = hardware_capabilities.has_independent_cursor_plane;

        DCHECK(overlay_candidates_);
        overlay_candidates_->SetSupportedBufferFormats(std::move(hardware_capabilities.supported_buffer_formats));
    } else {
...
gfx::Rect OverlayProcessorOzone::GetOverlayDamageRectForOutputSurface(const OverlayCandidate& overlay) const
{
...
bool OverlayProcessorOzone::SetNativePixmapForCandidate(ui::OverlaySurfaceCandidate* candidate, const gpu::Mailbox& mailbox, bool is_primary)
{
    DCHECK(shared_image_interface_);
    scoped_refptr<gfx::NativePixmap> native_pixmap = shared_image_interface_->GetNativePixmap(mailbox);
    ...
    if (is_primary && (candidate->buffer_size != native_pixmap->GetBufferSize() || candidate->format != native_pixmap->GetBufferFormat())) {
