...
const SharedImageFormat kOverlayFormats[] = { SinglePlaneFormat::kRGBX_8888, SinglePlaneFormat::kRGBA_8888, SinglePlaneFormat::kBGRX_8888,
    SinglePlaneFormat::kBGRA_8888, SinglePlaneFormat::kBGR_565, MultiPlaneFormat::kNV12, MultiPlaneFormat::kP010 };
...
gfx::OverlayTransform GetOverlayTransform(const gfx::Transform& quad_transform, bool y_flipped, bool supports_flip_rotate_transform)
{
...
OverlayCandidate::CandidateStatus GetReasonForTransformNotAxisAligned(const gfx::Transform& transform)
{
...
bool ShouldApplyRoundedCorner(OverlayCandidate& candidate, const DrawQuad* quad)
{
    const gfx::RectF target_rect = OverlayCandidate::DisplayRectInTargetSpace(candidate);
...
{
    return quad->ShouldDrawWithBlendingForReasonOtherThanMaskFilter() || quad->shared_quad_state->mask_filter_info.HasGradientMask();
}

} // namespace

OverlayCandidateFactory::OverlayContext::OverlayContext() = default;
OverlayCandidateFactory::OverlayContext::OverlayContext(const OverlayContext&) = default;

OverlayCandidate::CandidateStatus OverlayCandidateFactory::FromDrawQuad(const DrawQuad* quad, OverlayCandidate& candidate) const
{
    ...
    if (!context_.is_delegated_context && !cc::MathUtil::IsWithinEpsilon(sqs->opacity, 1.0f)) {
    ...
    if (!(sqs->blend_mode == SkBlendMode::kSrc || sqs->blend_mode == SkBlendMode::kSrcOver)) {
    ...
    candidate.overlay_damage_index = sqs->overlay_damage_index.value_or(OverlayCandidate::kInvalidDamageIndex);
    ...
    if (sqs->layer_id != 0) {
        ...
        candidate.aggregated_layer_id = static_cast<uint64_t>(sqs->layer_id) | (static_cast<uint64_t>(sqs->layer_namespace_id) << 32);
    }
    ...
    case DrawQuad::Material::kVideoHole:
        status = FromVideoHoleQuad(VideoHoleDrawQuad::MaterialCast(quad), candidate);
        break;
    case DrawQuad::Material::kSolidColor:
        if (context_.is_delegated_context) {
            status = FromSolidColorQuad(SolidColorDrawQuad::MaterialCast(quad), candidate);
        }
        break;
    case DrawQuad::Material::kAggregatedRenderPass:
        if (context_.is_delegated_context) {
            status = FromAggregateQuad(AggregatedRenderPassDrawQuad::MaterialCast(quad), candidate);
        }
        break;
...
OverlayCandidateFactory::OverlayCandidateFactory(const AggregatedRenderPass* render_pass, const DisplayResourceProvider* resource_provider,
    const SurfaceDamageRectList* surface_damage_rect_list, const SkM44* output_color_matrix, const gfx::RectF primary_rect,
    const OverlayProcessorInterface::FilterOperationsMap* render_pass_filters, const OverlayContext& context)
    ...
    , context_(context)
{
    ...
    DCHECK(!context_.disable_wire_size_optimization || context_.supports_arbitrary_transform);
...
float OverlayCandidateFactory::EstimateVisibleDamage(
    const DrawQuad* quad, const OverlayCandidate& candidate, QuadList::ConstIterator quad_list_begin, QuadList::ConstIterator quad_list_end) const
{
    ...
    for (auto overlap_iter = quad_list_begin; overlap_iter != quad_list_end; ++overlap_iter) {
        gfx::Rect overlap_rect
            = gfx::ToEnclosingRect(cc::MathUtil::MapClippedRect(overlap_iter->shared_quad_state->quad_to_target_transform, gfx::RectF(overlap_iter->rect)));

        // Opaque quad that (partially) occludes this candidate.
        if (!OverlayCandidate::IsInvisibleQuad(*overlap_iter) && !overlap_iter->ShouldDrawWithBlending()) {
    ...
    // below.
    return std::max(0.f, quad_damage.size().GetArea() - occluded_damage_estimate_total);
}

bool OverlayCandidateFactory::IsOccludedByFilteredQuad(const OverlayCandidate& candidate, QuadList::ConstIterator quad_list_begin,
    QuadList::ConstIterator quad_list_end,
    const base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>>& render_pass_backdrop_filters) const
{
    gfx::RectF target_rect = OverlayCandidate::DisplayRectInTargetSpace(candidate);
    for (auto overlap_iter = quad_list_begin; overlap_iter != quad_list_end; ++overlap_iter) {
        if (auto* render_pass_draw_quad = overlap_iter->DynamicCast<AggregatedRenderPassDrawQuad>()) {
            gfx::RectF overlap_rect = cc::MathUtil::MapClippedRect(overlap_iter->shared_quad_state->quad_to_target_transform, gfx::RectF(overlap_iter->rect));

            if (target_rect.Intersects(overlap_rect) && render_pass_backdrop_filters.count(render_pass_draw_quad->render_pass_id)) {
                return true;
            }
        }
    }
...
bool OverlayCandidateFactory::IsOccluded(
    const OverlayCandidate& candidate, QuadList::ConstIterator quad_list_begin, QuadList::ConstIterator quad_list_end) const
{
    // The rects are rounded as they're snapped by the compositor to pixel unless
    // it is AA'ed, in which case, it won't be overlaid.
    gfx::Rect target_rect = gfx::ToRoundedRect(OverlayCandidate::DisplayRectInTargetSpace(candidate));

    // Check that no visible quad overlaps the candidate.
    for (auto overlap_iter = quad_list_begin; overlap_iter != quad_list_end; ++overlap_iter) {
        gfx::Rect overlap_rect
            = gfx::ToRoundedRect(cc::MathUtil::MapClippedRect(overlap_iter->shared_quad_state->quad_to_target_transform, gfx::RectF(overlap_iter->rect)));

        if (!OverlayCandidate::IsInvisibleQuad(*overlap_iter) && target_rect.Intersects(overlap_rect)) {
            return true;
        }
    }
    return false;
...
OverlayCandidate::CandidateStatus OverlayCandidateFactory::FromDrawQuadResource(
    const DrawQuad* quad, ResourceId resource_id, bool y_flipped, OverlayCandidate& candidate) const
{
    if (resource_id != kInvalidResourceId && !resource_provider_->IsOverlayCandidate(resource_id)) {
    ...
    if (resource_id != kInvalidResourceId) {
        ...
        candidate.needs_detiling = resource_provider_->GetNeedsDetiling(resource_id);
        candidate.hdr_metadata = resource_provider_->GetHDRMetadata(resource_id);

        if (!context_.is_delegated_context && !base::Contains(kOverlayFormats, candidate.format)) {
    ...
    if (auto status = ApplyTransform(sqs->quad_to_target_transform, y_flipped, candidate); status != CandidateStatus::kSuccess) {
        return status;
    }

    candidate.is_opaque = !quad->ShouldDrawWithBlendingForReasonOtherThanMaskFilter();

    if (resource_id != kInvalidResourceId) {
        candidate.resource_size_in_pixels = resource_provider_->GetResourceBackedSize(resource_id);
    }
    ...
    if (resource_id != kInvalidResourceId) {
        ...
        if (!context_.is_delegated_context) {
            ...
            TrackingIdData track_data { quad->rect, resource_provider_->GetSurfaceId(resource_id).frame_sink_id() };
            // Assert that there is no padding - otherwise the bytes-based hash below
            // may differ for otherwise equal objects.
            static_assert(sizeof(track_data) == sizeof(decltype(track_data.rect)) + sizeof(decltype(track_data.frame_sink_id)));
            // Intentionally throwing away the high bits (assuming that hash entropy
            // is uniformly spread across all the bits).
            size_t original_hash = base::FastHash(base::byte_span_from_ref(track_data));
    ...
    if (context_.is_delegated_context) {
        const bool quad_within_window = primary_rect_.Contains(candidate.display_rect);
        const bool transform_supports_clipping = context_.supports_arbitrary_transform || absl::holds_alternative<gfx::OverlayTransform>(candidate.transform);
        ...
        bool can_delegate_clipping
            = context_.supports_clip_rect && (quad_within_window || context_.supports_out_of_window_clip_rect) && transform_supports_clipping;
        ...
        if (can_delegate_clipping) {
            ...
            if (candidate.clip_rect.has_value() && !OverlayCandidate::DisplayRectInTargetSpace(candidate).Intersects(gfx::RectF(*candidate.clip_rect))) {
                return CandidateStatus::kFailVisible;
            }
        } else {
    ...
    // is known.
    if (context_.disable_wire_size_optimization || ShouldApplyRoundedCorner(candidate, quad)) {
...
void OverlayCandidateFactory::SetDisplayRect(const DrawQuad& quad, OverlayCandidate& candidate) const
{
    if (context_.is_delegated_context && quad.visible_rect != quad.rect) {
        ...
        candidate.uv_rect = cc::MathUtil::ScaleRectProportional(candidate.uv_rect, gfx::RectF(quad.rect), gfx::RectF(quad.visible_rect));
    } else {
    ...
    if (context_.is_delegated_context) {
        ...
        if (auto* rpdq = quad.DynamicCast<AggregatedRenderPassDrawQuad>()) {
            ...
            if (filter_it != render_pass_filters_->end()) {
                candidate.display_rect = gfx::RectF(filter_it->second->ExpandRectForPixelMovement(quad.visible_rect));
...
OverlayCandidate::CandidateStatus OverlayCandidateFactory::DoGeometricClipping(const DrawQuad* quad, OverlayCandidate& candidate) const
{
...
OverlayCandidate::CandidateStatus OverlayCandidateFactory::ApplyTransform(
    const gfx::Transform& quad_to_target_transform, const bool y_flipped, OverlayCandidate& candidate) const
{
    ...
    if (!context_.disable_wire_size_optimization) {
        gfx::OverlayTransform overlay_transform = GetOverlayTransform(quad_to_target_transform, y_flipped, context_.supports_flip_rotate_transform);
        if (overlay_transform != gfx::OVERLAY_TRANSFORM_INVALID) {
            candidate.transform = overlay_transform;
            candidate.display_rect = quad_to_target_transform.MapRect(candidate.display_rect);
            return OverlayCandidate::CandidateStatus::kSuccess;
        }
    }

    // Otherwise, try to set an arbitrary transform, if possible.
    if (context_.supports_arbitrary_transform && (!quad_to_target_transform.HasPerspective() || quad_to_target_transform.Preserves2dAffine())) {
        gfx::Transform transform = quad_to_target_transform;
        if (y_flipped) {
            transform.PreConcat(gfx::OverlayTransformToTransform(gfx::OVERLAY_TRANSFORM_FLIP_VERTICAL, candidate.display_rect.size()));
        }
        candidate.transform = transform;
        return OverlayCandidate::CandidateStatus::kSuccess;
    }

    return context_.is_delegated_context ? GetReasonForTransformNotAxisAligned(quad_to_target_transform) : CandidateStatus::kFailNotAxisAligned;
}

OverlayCandidate::CandidateStatus OverlayCandidateFactory::FromAggregateQuad(const AggregatedRenderPassDrawQuad* quad, OverlayCandidate& candidate) const
{
...
OverlayCandidate::CandidateStatus OverlayCandidateFactory::FromSolidColorQuad(const SolidColorDrawQuad* quad, OverlayCandidate& candidate) const
{
    auto rtn = FromDrawQuadResource(quad, kInvalidResourceId, false, candidate);
...
// |candidate|.
OverlayCandidate::CandidateStatus OverlayCandidateFactory::FromVideoHoleQuad(const VideoHoleDrawQuad* quad, OverlayCandidate& candidate) const
{
    candidate.display_rect = gfx::RectF(quad->rect);
    const SharedQuadState* sqs = quad->shared_quad_state;
    if (context_.supports_arbitrary_transform) {
        ...
        gfx::OverlayTransform overlay_transform = GetOverlayTransform(sqs->quad_to_target_transform, false, context_.supports_flip_rotate_transform);
        if (overlay_transform == gfx::OVERLAY_TRANSFORM_INVALID)
            return CandidateStatus::kFailNotAxisAligned;
        candidate.transform = overlay_transform;
        candidate.display_rect = sqs->quad_to_target_transform.MapRect(candidate.display_rect);
    }
    candidate.is_opaque = !quad->ShouldDrawWithBlendingForReasonOtherThanMaskFilter();
...
OverlayCandidate::CandidateStatus OverlayCandidateFactory::FromTileQuad(const TileDrawQuad* quad, OverlayCandidate& candidate) const
{
    if (quad->nearest_neighbor)
        return CandidateStatus::kFailNearFilter;

    candidate.resource_size_in_pixels = resource_provider_->GetResourceBackedSize(quad->resource_id());
    candidate.uv_rect = gfx::ScaleRect(quad->tex_coord_rect, 1.f / candidate.resource_size_in_pixels.width(), 1.f / candidate.resource_size_in_pixels.height());

    auto rtn = FromDrawQuadResource(quad, quad->resource_id(), false, candidate);
    return rtn;
}

OverlayCandidate::CandidateStatus OverlayCandidateFactory::FromTextureQuad(const TextureDrawQuad* quad, OverlayCandidate& candidate) const
{
    if (!context_.is_delegated_context && quad->overlay_priority_hint == OverlayPriority::kLow) {
        // For current implementation low priority means this does not promote to
        // overlay.
        return CandidateStatus::kFailPriority;
    }

    if (!quad->rounded_display_masks_info.IsEmpty() && !context_.supports_rounded_display_masks) {
    ...
    if (context_.is_delegated_context) {
        ...
        && (quad->background_color != SkColors::kBlack || RequiresBlendingForReasonOtherThanRoundedCorners(quad))) {
    ...
    auto rtn = FromDrawQuadResource(quad, quad->resource_id(), quad->y_flipped, candidate);
    if (rtn == CandidateStatus::kSuccess) {
        ...
        candidate.is_video_in_surface_view = quad->is_stream_video && !resource_provider_->IsBackedBySurfaceTexture(quad->resource_id());
        ...
        candidate.has_rounded_display_masks = !quad->rounded_display_masks_info.IsEmpty();
    }
    return rtn;
}

void OverlayCandidateFactory::HandleClipAndSubsampling(OverlayCandidate& candidate) const
{
    ...
    if (candidate.format != MultiPlaneFormat::kNV12 && candidate.format != MultiPlaneFormat::kP010) {
        return;
    }
    ...
    if (candidate.needs_detiling && absl::holds_alternative<gfx::OverlayTransform>(candidate.transform)) {
        switch (absl::get<gfx::OverlayTransform>(candidate.transform)) {
        case gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90:
            candidate.uv_rect = gfx::RectF(1.0f - candidate.uv_rect.height(), candidate.uv_rect.x(), candidate.uv_rect.height(), candidate.uv_rect.width());
            break;
        case gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_180:
            candidate.uv_rect
                = gfx::RectF(1.0f - candidate.uv_rect.width(), 1.0f - candidate.uv_rect.height(), candidate.uv_rect.width(), candidate.uv_rect.height());
            break;
        case gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_270:
            candidate.uv_rect = gfx::RectF(candidate.uv_rect.y(), 1.0f - candidate.uv_rect.width(), candidate.uv_rect.height(), candidate.uv_rect.width());
            break;
        default:
            break;
        }
    ...
    gfx::RectF uv_rect = cc::MathUtil::ScaleRectProportional(candidate.uv_rect, candidate.display_rect, gfx::RectF(*candidate.clip_rect));
    ...
    candidate.display_rect.SetRect(rounded_display_rect.x(), rounded_display_rect.y(), rounded_display_rect.width(), rounded_display_rect.height());
    ...
    gfx::RectF src_rect = gfx::ScaleRect(candidate.uv_rect, candidate.resource_size_in_pixels.width(), candidate.resource_size_in_pixels.height());
...
void OverlayCandidateFactory::AssignDamage(const DrawQuad* quad, OverlayCandidate& candidate) const
{
    ...
    // damage.
    if (!context_.is_delegated_context) {
        candidate.damage_area_estimate = GetDamageEstimate(candidate).size().GetArea();
    }
}

gfx::RectF OverlayCandidateFactory::GetDamageEstimate(const OverlayCandidate& candidate) const
{
    ...
    return gfx::IntersectRects(OverlayCandidate::DisplayRectInTargetSpace(candidate), gfx::RectF(unassigned_surface_damage_));
}
