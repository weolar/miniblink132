...
TEST(OverlayProcessorSurfaceControlTest, Clipped)
{
    ...
    processor.CheckOverlaySupport(nullptr, &candidates);
    EXPECT_TRUE(candidates.at(0).overlay_handled);
    EXPECT_RECTF_EQ(candidates.at(0).display_rect, gfx::RectF(2.f, 2.f, 5.f, 5.f));
...
TEST(OverlayProcessorSurfaceControlTest, NegativeOffset)
{
    ...
    processor.CheckOverlaySupport(nullptr, &candidates);
    EXPECT_TRUE(candidates.at(0).overlay_handled);
    EXPECT_RECTF_EQ(candidates.at(0).display_rect, gfx::RectF(0.f, 0.f, 8.f, 6.f));
...
TEST(OverlayProcessorSurfaceControlTest, ClipAndNegativeOffset)
{
    ...
    processor.CheckOverlaySupport(nullptr, &candidates);
    EXPECT_TRUE(candidates.at(0).overlay_handled);
    EXPECT_RECTF_EQ(candidates.at(0).display_rect, gfx::RectF(0.f, 0.f, 5.f, 5.f));
    EXPECT_RECTF_EQ(candidates.at(0).uv_rect, gfx::RectF(0.75f, 0.75f, 0.25f, 0.25f));
}
...
TEST(OverlayProcessorSurfaceControlTest, DisplayTransformOverlayVFlip)
{
    ...
    candidates.back().transform = gfx::OVERLAY_TRANSFORM_FLIP_VERTICAL_CLOCKWISE_90;
    processor.CheckOverlaySupport(nullptr, &candidates);
    EXPECT_TRUE(candidates.back().overlay_handled);

    EXPECT_EQ(absl::get<gfx::OverlayTransform>(candidates.back().transform), gfx::OVERLAY_TRANSFORM_FLIP_VERTICAL);
...
TEST(OverlayProcessorSurfaceControlTest, DisplayTransformOverlay)
{
    ...
    processor.CheckOverlaySupport(nullptr, &candidates);
    EXPECT_TRUE(candidates.back().overlay_handled);
    EXPECT_EQ(absl::get<gfx::OverlayTransform>(candidates.back().transform), gfx::OVERLAY_TRANSFORM_NONE);
    EXPECT_RECTF_EQ(candidates.back().display_rect, gfx::RectF(10, 40, 100, 50));
}
...
TEST(OverlayProcessorSurfaceControlTest, DisplayTransformOutputSurfaceOverlay)
{
    ...
    std::optional<OverlayProcessorInterface::OutputSurfaceOverlayPlane> overlay_plane = candidate;
    ...
    EXPECT_EQ(overlay_plane.value().transform, gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90);
}
...
TEST(OverlayCandidateValidatorTest, OverlayDamageRectForOutputSurface)
{
    ...
    EXPECT_TRUE(candidates.back().overlay_handled);
    EXPECT_RECTF_EQ(candidates.back().display_rect, gfx::RectF(10, 40, 100, 50));
    EXPECT_EQ(processor.GetOverlayDamageRectForOutputSurface(candidates.back()), gfx::Rect(10, 10, 50, 100));
}
