...
public:
    ...
    ImageProvider::ScopedResult GetRasterContent(const cc::DrawImage& draw_image) override
    {
        ...
        return ScopedResult(cc::DecodedDrawImage(paint_image.GetSkImageForFrame(frame_index, cc::PaintImage::kDefaultGeneratorClientId), nullptr, SkSize::Make(0, 0),
            SkSize::Make(1.f, 1.f), draw_image.filter_quality(), true /* is_budgeted */));
    }
...
bool SoftwareRenderer::CanPartialSwap() { return true; }

void SoftwareRenderer::BeginDrawingFrame() { TRACE_EVENT0("viz", "SoftwareRenderer::BeginDrawingFrame"); }
...
void SoftwareRenderer::SwapBuffers(SwapFrameData swap_frame_data)
{
    ...
    output_frame.top_controls_visible_height_changed = swap_frame_data.top_controls_visible_height_changed;
    output_surface_->SwapBuffers(std::move(output_frame));
}

bool SoftwareRenderer::FlippedFramebuffer() const { return false; }

void SoftwareRenderer::EnsureScissorTestEnabled() { is_scissor_enabled_ = true; }

void SoftwareRenderer::EnsureScissorTestDisabled() { is_scissor_enabled_ = false; }
...
void SoftwareRenderer::BindFramebufferToTexture(const AggregatedRenderPassId render_pass_id)
{
#if 1
...
void SoftwareRenderer::PrepareSurfaceForPass(SurfaceInitializationMode initialization_mode, const gfx::Rect& render_pass_scissor)
{
...
bool SoftwareRenderer::IsSoftwareResource(ResourceId resource_id) { return resource_provider()->IsResourceSoftwareBacked(resource_id); }
...
void SoftwareRenderer::DoDrawQuad(const DrawQuad* quad, const gfx::QuadF* draw_region)
{
    ...
    //current_canvas_->setMatrix(sk_device_matrix); // weolar
...
void SoftwareRenderer::DrawSolidColorQuad(const SolidColorDrawQuad* quad)
{
    ...
    paint.setAlphaf(quad->shared_quad_state->opacity * quad->color.fA);
    paint.setColor(0xff112233);
    //current_canvas_->drawCircle(300, 300, 260, paint);// weolar
    //current_canvas_->drawRect(SkRect::MakeXYWH(0, 0, 200, 300), current_paint_);
}
...
void SoftwareRenderer::DrawTextureQuad(const TextureDrawQuad* quad)
{
    ...
    DisplayResourceProviderSoftware::ScopedReadLockSkImage lock(resource_provider(), quad->resource_id(), quad->premultiplied_alpha ? kPremul_SkAlphaType : kUnpremul_SkAlphaType);
...
void SoftwareRenderer::DrawTileQuad(const TileDrawQuad* quad)
{
    ...
    DisplayResourceProviderSoftware::ScopedReadLockSkImage lock(resource_provider(), quad->resource_id(), quad->is_premultiplied ? kPremul_SkAlphaType : kUnpremul_SkAlphaType);
    if (!lock.valid())
        return;
    ...
    SkSamplingOptions sampling(quad->nearest_neighbor ? SkFilterMode::kNearest : SkFilterMode::kLinear);
    //current_canvas_->drawImageRect(lock.sk_image(), uv_rect, gfx::RectFToSkRect(visible_quad_vertex_rect), sampling, &current_paint_, SkCanvas::kStrict_SrcRectConstraint);

    SkPaint paint;
    paint.setAntiAlias(true);
    paint.setAlphaf(1);
    paint.setColor(0xffff2233);
    current_canvas_->drawCircle(30, 30, 260, paint);// weolar
}
...
//     SkRect dest_visible_rect = gfx::RectFToSkRect(cc::MathUtil::ScaleRectProportional(QuadVertexRect(), gfx::RectF(quad->rect), gfx::RectF(quad->visible_rect)));
...
//         auto paint_filter = cc::RenderSurfaceFilters::BuildImageFilter(*filters, gfx::SizeF(source_bitmap.width(), source_bitmap.height()));
...
//         {
//             render_pass_output
//                 = SkPixmap(render_pass_output.info().makeWH(geometry.sampling_bounds.width(), geometry.sampling_bounds.height()).makeColorSpace(std::move(color_space)),
...
void SoftwareRenderer::GenerateMipmap() { NOTIMPLEMENTED(); }
...
gfx::Rect SoftwareRenderer::GetBackdropBoundingBoxForRenderPassQuad(const AggregatedRenderPassDrawQuad* quad, const cc::FilterOperations* backdrop_filters,
    absl::optional<gfx::RRectF> backdrop_filter_bounds_input, gfx::Transform contents_device_transform, gfx::Transform* backdrop_filter_bounds_transform,
    absl::optional<gfx::RRectF>* backdrop_filter_bounds, gfx::Rect* unclipped_rect) const
{
...
sk_sp<SkShader> SoftwareRenderer::GetBackdropFilterShader(const AggregatedRenderPassDrawQuad* quad, SkTileMode content_tile_mode) const
{
    ...
    if (!s_init)
        OutputDebugStringA("SoftwareRenderer::GetBackdropFilterShader not impl\n");
    s_init = true;
...
//         return nullptr;
//     absl::optional<gfx::RRectF> backdrop_filter_bounds_input = BackdropFilterBoundsForPass(quad->render_pass_id);
...
//         quad, backdrop_filters, backdrop_filter_bounds_input, contents_device_transform, &backdrop_filter_bounds_transform, &backdrop_filter_bounds, &unclipped_rect);
// 
...
//         cropped_image = cropped_image->makeSubset(RectToSkIRect(filter_clip), nullptr);
...
//         = cc::RenderSurfaceFilters::BuildImageFilter(*backdrop_filters, gfx::SizeF(backdrop_bitmap.width(), backdrop_bitmap.height()), clipping_offset);
//     if (!paint_filter)
//         return nullptr;
...
// 
//     return SkImage::MakeFromBitmap(bitmap)->makeShader(content_tile_mode, content_tile_mode, SkSamplingOptions(), &filter_backdrop_transform);
}
...
void SoftwareRenderer::AllocateRenderPassResourceIfNeeded(const AggregatedRenderPassId& render_pass_id, const RenderPassRequirements& requirements)
{
    auto it = render_pass_bitmaps_.find(render_pass_id);
    if (it != render_pass_bitmaps_.end()) {
        DCHECK(it->second.width() >= requirements.size.width() && it->second.height() >= requirements.size.height());
        return;
    }
