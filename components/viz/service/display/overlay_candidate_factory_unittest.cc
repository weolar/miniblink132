...
public:
    ...
    OverlayCandidateFactoryTestBase(const OverlayCandidateFactoryTestBase&) = delete;
    OverlayCandidateFactoryTestBase& operator=(const OverlayCandidateFactoryTestBase&) = delete;
...
protected:
    ...
    {
        scoped_refptr<RasterContextProvider> child_context_provider = TestContextProvider::Create();

        child_context_provider->BindToCurrentSequence();

        auto resource = TransferableResource::MakeGpu(
            gpu::Mailbox::Generate(), GL_TEXTURE_2D, gpu::SyncToken(), gfx::Size(1, 1), SinglePlaneFormat::kRGBA_8888, is_overlay_candidate);

        ResourceId resource_id = child_resource_provider_.ImportResource(resource, base::DoNothing());

        int child_id = resource_provider_.CreateChild(base::DoNothing(), SurfaceId());

        // Transfer resource to the parent.
        std::vector<ResourceId> resource_ids_to_transfer;
        resource_ids_to_transfer.push_back(resource_id);
        std::vector<TransferableResource> list;
        child_resource_provider_.PrepareSendToParent(resource_ids_to_transfer, &list, child_context_provider.get());
        ...
        std::unordered_map<ResourceId, ResourceId, ResourceIdHasher> resource_map = resource_provider_.GetChildToParentMap(child_id);

        return resource_map[list[0].id];
    }

    OverlayCandidateFactory CreateCandidateFactory(
        const AggregatedRenderPass& render_pass, const gfx::RectF& primary_rect, const OverlayCandidateFactory::OverlayContext& context)
    {
        return OverlayCandidateFactory(&render_pass, &resource_provider_, &surface_damage_list_, &identity_, primary_rect, &render_pass_filters_, context);
    }

    void RunRoundedCornerTest(bool disable_wire_size_optimization)
    {
        AggregatedRenderPass render_pass;
        render_pass.SetNew(AggregatedRenderPassId::FromUnsafeValue(1), gfx::Rect(0, 0, 100, 100), gfx::Rect(), gfx::Transform());
        ...
        for (const auto& tile_rect : tile_rects) {
            ...
            solid_quad->SetNew(sqs, /*rect=*/tile_rect, /*visible_rect=*/tile_rect, SkColors::kBlack,
        ...
        OverlayCandidateFactory factory = CreateCandidateFactory(render_pass, gfx::RectF(render_pass.output_rect), context);

        OverlayCandidateList candidates;
        for (const auto* quad : render_pass.quad_list) {
            OverlayCandidate candidate;
            OverlayCandidate::CandidateStatus status = factory.FromDrawQuad(quad, candidate);
        ...
        const bool expect_rounded_corner_on_inner_quads = disable_wire_size_optimization;
        EXPECT_EQ(!candidates[1].rounded_corners.IsEmpty(), expect_rounded_corner_on_inner_quads);
        EXPECT_EQ(!candidates[4].rounded_corners.IsEmpty(), expect_rounded_corner_on_inner_quads);
    }
...
SolidColorDrawQuad* AddQuad(const gfx::Rect quad_rect, const gfx::Transform& quad_to_target_transform, AggregatedRenderPass* render_pass,
    const std::optional<gfx::Rect> clip_rect, const gfx::Rect visible_rect)
{
    ...
    SolidColorDrawQuad* solid_quad = render_pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
    solid_quad->SetNew(quad_state, quad_rect, visible_rect, SkColors::kBlack, false /* force_anti_aliasing_off */);
    return solid_quad;
}

SolidColorDrawQuad* AddQuad(const gfx::Rect quad_rect, const gfx::Transform& quad_to_target_transform, AggregatedRenderPass* render_pass)
{
    return AddQuad(quad_rect, quad_to_target_transform, render_pass, std::nullopt, quad_rect);
}

AggregatedRenderPassDrawQuad* AddRenderPassQuad(
    gfx::Rect quad_rect, gfx::Transform transform, std::optional<gfx::Rect> clip_rect, AggregatedRenderPassId rpid, AggregatedRenderPass* render_pass)
{
    SharedQuadState* quad_state = render_pass->CreateAndAppendSharedQuadState();
    ...
    auto* rpdq = render_pass->CreateAndAppendDrawQuad<AggregatedRenderPassDrawQuad>();
    rpdq->SetNew(
        quad_state, quad_rect, quad_rect, rpid, kInvalidResourceId, gfx::RectF(), gfx::Size(), gfx::Vector2dF(1, 1), gfx::PointF(), gfx::RectF(), false, 1.0f);
    return rpdq;
}

OverlayCandidate CreateCandidate(float left, float top, float right, float bottom)
{
    OverlayCandidate candidate;
...
{
    AggregatedRenderPass render_pass;
    render_pass.SetNew(AggregatedRenderPassId::FromUnsafeValue(1), gfx::Rect(0, 0, 1, 1), gfx::Rect(), gfx::Transform());

    OverlayCandidateFactory factory = CreateCandidateFactory(render_pass, gfx::RectF(render_pass.output_rect), GetOverlayContextForTesting());
    ...
    EXPECT_FALSE(factory.IsOccluded(CreateCandidate(0.5f, 0.5f, 10.49f, 10.49f), render_pass.quad_list.begin(), render_pass.quad_list.end()));

    EXPECT_TRUE(factory.IsOccluded(CreateCandidate(0.49f, 0.5f, 10.49f, 10.49f), render_pass.quad_list.begin(), render_pass.quad_list.end()));

    EXPECT_TRUE(factory.IsOccluded(CreateCandidate(0.5f, 0.49f, 10.50f, 10.5f), render_pass.quad_list.begin(), render_pass.quad_list.end()));
    EXPECT_TRUE(factory.IsOccluded(CreateCandidate(0.5f, 0.5f, 10.5f, 10.49f), render_pass.quad_list.begin(), render_pass.quad_list.end()));

    EXPECT_TRUE(factory.IsOccluded(CreateCandidate(0.5f, 0.5f, 10.49f, 10.5f), render_pass.quad_list.begin(), render_pass.quad_list.end()));
}
...
TEST_F(OverlayCandidateFactoryTest, IsOccludedScaled)
{
    ...
    EXPECT_FALSE(factory.IsOccluded(CreateCandidate(2.f, 3.f, 14.f, 17.f), render_pass.quad_list.begin(), render_pass.quad_list.end()));
    EXPECT_TRUE(factory.IsOccluded(CreateCandidate(1.f, 3.f, 14.f, 17.f), render_pass.quad_list.begin(), render_pass.quad_list.end()));
    EXPECT_TRUE(factory.IsOccluded(CreateCandidate(2.f, 2.f, 14.f, 17.f), render_pass.quad_list.begin(), render_pass.quad_list.end()));
    EXPECT_TRUE(factory.IsOccluded(CreateCandidate(2.f, 3.f, 15.f, 17.f), render_pass.quad_list.begin(), render_pass.quad_list.end()));
    EXPECT_TRUE(factory.IsOccluded(CreateCandidate(2.f, 3.f, 15.f, 18.f), render_pass.quad_list.begin(), render_pass.quad_list.end()));
}
...
class OverlayCandidateFactoryArbitraryTransformTest : public OverlayCandidateFactoryTestBase {
protected:
    TextureDrawQuad CreateUnclippedDrawQuad(AggregatedRenderPass& render_pass, const gfx::Rect& quad_rect, const gfx::Transform& quad_to_target_transform)
    {
        SharedQuadState* sqs = render_pass.CreateAndAppendSharedQuadState();
        sqs->quad_to_target_transform = quad_to_target_transform;
        TextureDrawQuad quad;
        quad.SetNew(sqs, quad_rect, quad_rect, false, CreateResource(/*is_overlay_candidate=*/true), false, gfx::PointF(), gfx::PointF(1, 1),
            SkColors::kTransparent, false, false, false, gfx::ProtectedVideoType::kClear);
...
// possible.
TEST_F(OverlayCandidateFactoryArbitraryTransformTest, AxisAlignedNotBakedIntoDisplayRect)
{
    AggregatedRenderPass render_pass;
    render_pass.SetNew(AggregatedRenderPassId::FromUnsafeValue(1), gfx::Rect(0, 0, 10, 10), gfx::Rect(), gfx::Transform());
    ...
    OverlayCandidate candidate;
    OverlayCandidate::CandidateStatus result = factory.FromDrawQuad(&quad, candidate);
    ASSERT_EQ(result, OverlayCandidate::CandidateStatus::kSuccess);
    ASSERT_TRUE(absl::holds_alternative<gfx::OverlayTransform>(candidate.transform));
    EXPECT_EQ(absl::get<gfx::OverlayTransform>(candidate.transform), gfx::OverlayTransform::OVERLAY_TRANSFORM_NONE);
...
TEST_F(OverlayCandidateFactoryArbitraryTransformTest, TransformIncludesYFlip)
{
    ...
    ASSERT_TRUE(absl::holds_alternative<gfx::Transform>(candidate.transform));
    EXPECT_EQ(absl::get<gfx::Transform>(candidate.transform), transform_y_flipped);
    gfx::PointF display_rect_origin = absl::get<gfx::Transform>(candidate.transform).MapPoint(candidate.display_rect.origin());
    // Flip moves the origin to 0,1. The skew slides it out to 1,1.
    EXPECT_EQ(display_rect_origin, gfx::PointF(1, 1));
    EXPECT_EQ(candidate.display_rect, gfx::RectF(0, 0, 1, 1));
}

TEST_F(OverlayCandidateFactoryArbitraryTransformTest, UseArbitraryTransformWhenSupported)
{
    AggregatedRenderPass render_pass;
    render_pass.SetNew(AggregatedRenderPassId::FromUnsafeValue(1), gfx::Rect(0, 0, 1, 1), gfx::Rect(), gfx::Transform());
...
TEST_F(OverlayCandidateFactoryArbitraryTransformTest, UseOverlayTransformWhenPossibleForWireSize)
{
    AggregatedRenderPass render_pass;
    render_pass.SetNew(AggregatedRenderPassId::FromUnsafeValue(1), gfx::Rect(0, 0, 2, 2), gfx::Rect(), gfx::Transform());
    ...
    ASSERT_TRUE(absl::holds_alternative<gfx::OverlayTransform>(candidate.transform));
    EXPECT_EQ(absl::get<gfx::OverlayTransform>(candidate.transform), gfx::OVERLAY_TRANSFORM_NONE);
}

TEST_F(OverlayCandidateFactoryArbitraryTransformTest, Allow3DTransformNoPerspective)
{
    AggregatedRenderPass render_pass;
    render_pass.SetNew(AggregatedRenderPassId::FromUnsafeValue(1), gfx::Rect(0, 0, 10, 10), gfx::Rect(), gfx::Transform());
...
TEST_F(OverlayCandidateFactoryArbitraryTransformTest, TechnicallyFlatTransform)
{
    AggregatedRenderPass render_pass;
    render_pass.SetNew(AggregatedRenderPassId::FromUnsafeValue(1), gfx::Rect(0, 0, 10, 10), gfx::Rect(), gfx::Transform());
...
class OverlayCandidateFactoryInvalidContextTest : public OverlayCandidateFactoryTestBase {
protected:
    void CheckContext(const OverlayCandidateFactory::OverlayContext& context, const char* expected_assertion)
    {
        AggregatedRenderPass render_pass;
        render_pass.SetNew(AggregatedRenderPassId::FromUnsafeValue(1), gfx::Rect(0, 0, 1, 1), gfx::Rect(), gfx::Transform());
        EXPECT_DEATH(CreateCandidateFactory(render_pass, gfx::RectF(render_pass.output_rect), context), expected_assertion);
    }
};
...
TEST_F(OverlayCandidateFactoryArbitraryTransformTest, NoArbitraryTransformSupportFails)
{
    AggregatedRenderPass render_pass;
    render_pass.SetNew(AggregatedRenderPassId::FromUnsafeValue(1), gfx::Rect(0, 0, 1, 1), gfx::Rect(), gfx::Transform());
    ...
    OverlayCandidate candidate;
    OverlayCandidate::CandidateStatus result = factory.FromDrawQuad(&quad, candidate);
    ASSERT_EQ(result, OverlayCandidate::CandidateStatus::kFailNotAxisAligned2dRotation);
}

TEST_F(OverlayCandidateFactoryArbitraryTransformTest, OccludedByFilteredQuadWorksInTargetSpace)
{
    AggregatedRenderPassId render_pass_id = AggregatedRenderPassId::FromUnsafeValue(1);
    AggregatedRenderPass render_pass;
    render_pass.SetNew(render_pass_id, gfx::Rect(0, 0, 2, 2), gfx::Rect(), gfx::Transform());
    ...
    QuadList quad_list;
    AggregatedRenderPassDrawQuad* rpdq = quad_list.AllocateAndConstruct<AggregatedRenderPassDrawQuad>();
    rpdq->SetNew(render_pass.CreateAndAppendSharedQuadState(), gfx::Rect(1, 1, 1, 1), gfx::Rect(1, 1, 1, 1), render_pass_id, kInvalidResourceId, gfx::RectF(),
        gfx::Size(), gfx::Vector2dF(1, 1), gfx::PointF(0, 0), gfx::RectF(), false, 1.0);

    base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>> filter_map;
    ...
    // RPDQ.
    {
        ...
        OverlayCandidate::CandidateStatus result = factory.FromDrawQuad(&quad, candidate);
        ASSERT_EQ(result, OverlayCandidate::CandidateStatus::kSuccess);
        EXPECT_FALSE(factory.IsOccludedByFilteredQuad(candidate, quad_list.begin(), quad_list.end(), filter_map));
    }
    ...
    // Check that a transformed 1x1 quad intersects with the filtered RPDQ.
    {
        ...
        OverlayCandidate::CandidateStatus result = factory.FromDrawQuad(&quad, candidate);
        ASSERT_EQ(result, OverlayCandidate::CandidateStatus::kSuccess);
        EXPECT_TRUE(factory.IsOccludedByFilteredQuad(candidate, quad_list.begin(), quad_list.end(), filter_map));
    }
}

TEST_F(OverlayCandidateFactoryArbitraryTransformTest, UnassignedDamageWithArbitraryTransforms)
{
    AggregatedRenderPass render_pass;
    render_pass.SetNew(AggregatedRenderPassId::FromUnsafeValue(1), gfx::Rect(0, 0, 2, 2), gfx::Rect(), gfx::Transform());
    ...
    // not get any damage.
    {
        ...
        transform.Rotate(-45);
        auto quad = CreateUnclippedDrawQuad(render_pass, gfx::Rect(2, 2), transform);
        ...
        QuadList quad_list;
        EXPECT_EQ(factory.EstimateVisibleDamage(&quad, candidate, quad_list.begin(), quad_list.end()), 0);
    }
    ...
    // Ensure when that same rect does intersect with the damage picks up damage.
    {
        ...
        EXPECT_GT(factory.EstimateVisibleDamage(&quad, candidate, quad_list.begin(), quad_list.end()), 0);
    }
}
...
protected:
    TextureDrawQuad CreateClippedDrawQuad(AggregatedRenderPass& render_pass, const gfx::Rect& quad_rect, const gfx::Transform& quad_to_target_transform,
        const gfx::Rect& clip_rect, const gfx::RectF& quad_uv_rect)
    {
        SharedQuadState* sqs = render_pass.CreateAndAppendSharedQuadState();
        sqs->quad_to_target_transform = quad_to_target_transform;
        sqs->clip_rect = clip_rect;
        TextureDrawQuad quad;
        quad.SetNew(sqs, quad_rect, quad_rect, false, CreateResource(/*is_overlay_candidate=*/true), false, quad_uv_rect.origin(), quad_uv_rect.bottom_right(),
            SkColors::kTransparent, false, false, false, gfx::ProtectedVideoType::kClear);

        return quad;
    }

    void RunClipToTopLeftCornerTest(gfx::OverlayTransform overlay_transform, gfx::RectF quad_uvs, gfx::RectF expected_uvs, bool needs_detiling = false)
    {
        AggregatedRenderPass render_pass;
        gfx::Rect bounds(100, 100);
        render_pass.SetNew(AggregatedRenderPassId::FromUnsafeValue(1), bounds, gfx::Rect(), gfx::Transform());
        ...
        gfx::Transform transform = gfx::OverlayTransformToTransform(overlay_transform, gfx::SizeF(1, 1));
        transform.PostScale(100, 100);
        transform.PostTranslate(50, 50);
        auto quad = CreateClippedDrawQuad(render_pass, gfx::Rect(1, 1), transform, bounds, quad_uvs);
        ...
        ASSERT_EQ(result, OverlayCandidate::CandidateStatus::kSuccess);
        ASSERT_TRUE(absl::holds_alternative<gfx::OverlayTransform>(candidate.transform));
        EXPECT_EQ(absl::get<gfx::OverlayTransform>(candidate.transform), overlay_transform);
        EXPECT_EQ(candidate.display_rect, gfx::RectF(50, 50, 50, 50));
        EXPECT_TRUE(candidate.uv_rect.ApproximatelyEqual(expected_uvs, kEpsilon, kEpsilon));
    }
};

TEST_F(TransformedOverlayClipRectTest, NoTransform)
{
    RunClipToTopLeftCornerTest(gfx::OverlayTransform::OVERLAY_TRANSFORM_NONE, gfx::RectF(1, 1), gfx::RectF(0.0f, 0.0f, 0.5f, 0.5f));
}
...
TEST_F(TransformedOverlayClipRectTest, Rotate90)
{
    ...
    RunClipToTopLeftCornerTest(gfx::OverlayTransform::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90, gfx::RectF(1, 1), gfx::RectF(0.0f, 0.5f, 0.5f, 0.5f));
}
...
TEST_F(TransformedOverlayClipRectTest, Rotate180)
{
    ...
    RunClipToTopLeftCornerTest(gfx::OverlayTransform::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_180, gfx::RectF(1, 1), gfx::RectF(0.5f, 0.5f, 0.5f, 0.5f));
}
...
TEST_F(TransformedOverlayClipRectTest, Rotate270)
{
    ...
    RunClipToTopLeftCornerTest(gfx::OverlayTransform::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_270, gfx::RectF(1, 1), gfx::RectF(0.5f, 0.0f, 0.5f, 0.5f));
}
...
TEST_F(TransformedOverlayClipRectTest, ClippedUvs)
{
    ...
    RunClipToTopLeftCornerTest(
        gfx::OverlayTransform::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_180, gfx::RectF(0.1f, 0.2f, 0.4f, 0.4f), gfx::RectF(0.3f, 0.4f, 0.2f, 0.2f));
}
...
{
    RunClipToTopLeftCornerTest(
        gfx::OverlayTransform::OVERLAY_TRANSFORM_NONE, gfx::RectF(0.7f, 0.6f), gfx::RectF(0.0f, 0.0f, 0.35f, 0.3f), /*needs_detiling=*/true);
}

TEST_F(TransformedOverlayClipRectTest, Rotate90NeedsDetiling)
{
    RunClipToTopLeftCornerTest(gfx::OverlayTransform::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90, gfx::RectF(0.7f, 0.6f), gfx::RectF(0.0f, 0.3f, 0.35f, 0.3f),
        /*needs_detiling=*/true);
}

TEST_F(TransformedOverlayClipRectTest, Rotate180NeedsDetiling)
{
    RunClipToTopLeftCornerTest(gfx::OverlayTransform::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_180, gfx::RectF(0.7f, 0.6f), gfx::RectF(0.35f, 0.3f, 0.35f, 0.3f),
        /*needs_detiling=*/true);
}

TEST_F(TransformedOverlayClipRectTest, Rotate270NeedsDetiling)
{
    RunClipToTopLeftCornerTest(gfx::OverlayTransform::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_270, gfx::RectF(0.7f, 0.6f), gfx::RectF(0.35f, 0.0f, 0.35f, 0.3f),
        /*needs_detiling=*/true);
}
...
TEST_F(OverlayCandidateFactoryTest, RenderPassClipped)
{
    ...
    auto* rpdq = AddRenderPassQuad(gfx::Rect(100, 100), gfx::Transform(), clip_rect, rpid, &render_pass);

    OverlayCandidate candidate;
    OverlayCandidate::CandidateStatus result = factory.FromDrawQuad(rpdq, candidate);
...
TEST_F(OverlayCandidateFactoryTest, RenderPassOffscreen)
{
    ...
    auto* rpdq = AddRenderPassQuad(gfx::Rect(100, 100), transform, std::nullopt, rpid, &render_pass);
...
TEST_F(OverlayCandidateFactoryTest, ClipDelegation_Success)
{
    ...
    OverlayCandidateFactory::OverlayContext context;
    context.is_delegated_context = true;
    OverlayCandidateFactory noclip_factory = CreateCandidateFactory(render_pass, gfx::RectF(render_pass.output_rect), context);
    context.supports_clip_rect = true;
    OverlayCandidateFactory clip_factory = CreateCandidateFactory(render_pass, gfx::RectF(render_pass.output_rect), context);

    OverlayCandidate no_clip_cand;
    OverlayCandidate clip_cand;
    ASSERT_EQ(noclip_factory.FromDrawQuad(quad, no_clip_cand), OverlayCandidate::CandidateStatus::kSuccess);
    ASSERT_EQ(clip_factory.FromDrawQuad(quad, clip_cand), OverlayCandidate::CandidateStatus::kSuccess);
