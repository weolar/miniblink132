...
public:
    ...
    int GetModeledPowerGain(uint64_t curr_frame, const OverlayCandidateTemporalTracker::Config& config, int display_area, bool is_fullscreen) const;
    ...
    void AddRecord(uint64_t curr_frame, float damage_area_ratio, ResourceId resource_id, const Config& config, bool force_resource_update = false);
    ...
    float GetDamageRatioRate() const
    {
        return ratio_rate_category_;
    }
