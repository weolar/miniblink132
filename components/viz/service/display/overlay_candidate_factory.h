...
public:
    ...
    OverlayCandidateFactory(const AggregatedRenderPass* render_pass, const DisplayResourceProvider* resource_provider,
    ...
    CandidateStatus FromDrawQuad(const DrawQuad* quad, OverlayCandidate& candidate) const;
    ...
    bool IsOccludedByFilteredQuad(const OverlayCandidate& candidate, QuadList::ConstIterator quad_list_begin, QuadList::ConstIterator quad_list_end,
    ...
    bool IsOccluded(const OverlayCandidate& candidate, QuadList::ConstIterator quad_list_begin, QuadList::ConstIterator quad_list_end) const;

    gfx::Rect GetUnassignedDamage()
    {
        return unassigned_surface_damage_;
    }
...
private:
    CandidateStatus FromDrawQuadResource(const DrawQuad* quad, ResourceId resource_id, bool y_flipped, OverlayCandidate& candidate) const;

    CandidateStatus FromTextureQuad(const TextureDrawQuad* quad, OverlayCandidate& candidate) const;

    CandidateStatus FromTileQuad(const TileDrawQuad* quad, OverlayCandidate& candidate) const;

    CandidateStatus FromAggregateQuad(const AggregatedRenderPassDrawQuad* quad, OverlayCandidate& candidate) const;

    CandidateStatus FromSolidColorQuad(const SolidColorDrawQuad* quad, OverlayCandidate& candidate) const;

    CandidateStatus FromVideoHoleQuad(const VideoHoleDrawQuad* quad, OverlayCandidate& candidate) const;
    ...
    CandidateStatus DoGeometricClipping(const DrawQuad* quad, OverlayCandidate& candidate) const;

    // Apply |quad_to_target_transform| to the candidate, based on
    // |OverlayContext| settings.
    CandidateStatus ApplyTransform(const gfx::Transform& quad_to_target_transform, const bool y_flipped, OverlayCandidate& candidate) const;
    ...
    raw_ptr<const OverlayProcessorInterface::FilterOperationsMap> render_pass_filters_;
