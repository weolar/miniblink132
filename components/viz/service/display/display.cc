...
#if DCHECK_IS_ON()
bool schedule_task_disallowed = false;
#endif // DCHECK_IS_ON()
...
// gpu::SharedImageInterface* DisplayCompositorMemoryAndTaskController::shared_image_interface(void)
// {
//     *(int*)1 = 1;
//     return nullptr;
// }
// 
DisplayCompositorMemoryAndTaskController::~DisplayCompositorMemoryAndTaskController(void)
{
    *(int*)1 = 1;
}
...
gfx::PresentationFeedback SanitizePresentationFeedback(const gfx::PresentationFeedback& feedback, base::TimeTicks draw_time)
{
    ...
    const auto allowed_delta_from_future
        = ((feedback.flags & (gfx::PresentationFeedback::kHWClock | gfx::PresentationFeedback::kVSync)) != 0) ? kAllowedDeltaFromFuture : base::TimeDelta();
...
Display::PresentationGroupTiming::PresentationGroupTiming(Display::PresentationGroupTiming&& other) = default;
...
void Display::PresentationGroupTiming::OnDraw(base::TimeTicks frame_time, base::TimeTicks draw_start_timestamp,
    base::flat_set<base::PlatformThreadId> animation_thread_ids, base::flat_set<base::PlatformThreadId> renderer_main_thread_ids,
...
void Display::PresentationGroupTiming::OnSwap(gfx::SwapTimings timings, DisplaySchedulerBase* scheduler)
{
    ...
    if (frame_latency < base::Seconds(0)) {
        LOG(ERROR) << "Frame latency is negative: " << frame_latency.InMillisecondsF() << " ms";
        return;
    }
    // Can be nullptr in unittests.
    if (scheduler) {
        scheduler->ReportFrameTime(frame_latency, std::move(animation_thread_ids_), std::move(renderer_main_thread_ids_), draw_start_timestamp_, boost_type_);
    }
}
...
Display::Display(SharedBitmapManager* bitmap_manager, gpu::SharedImageManager* shared_image_manager, gpu::SyncPointManager* sync_point_manager,
    gpu::Scheduler* gpu_scheduler, const RendererSettings& settings, const DebugRendererSettings* debug_settings, const FrameSinkId& frame_sink_id,
    std::unique_ptr<DisplayCompositorMemoryAndTaskController> gpu_dependency, std::unique_ptr<OutputSurface> output_surface,
    std::unique_ptr<OverlayProcessorInterface> overlay_processor, std::unique_ptr<DisplaySchedulerBase> scheduler,
    ...
    , last_presented_trace_id_(swapped_trace_id_)
{
    ...
    if (!logged && output_surface_->capabilities().max_texture_size > 0) {
        logged = true;
        LOG(ERROR) << "Max Texture Size=" << output_surface_->capabilities().max_texture_size;
    }
#endif

    occlusion_culler_ = std::make_unique<OcclusionCuller>(overlay_processor_.get(), settings_.occlusion_culler_settings);
...
{
#if DCHECK_IS_ON()
    allow_schedule_gpu_task_during_destruction_.reset(new gpu::ScopedAllowScheduleGpuTask);
#endif
...
void Display::Initialize(DisplayClient* client, SurfaceManager* surface_manager, bool hw_support_for_multiple_refresh_rates)
{
    DCHECK(client);
...
#if BUILDFLAG(IS_ANDROID)
        output_surface_supports_set_frame_rate = OutputSurfaceSupportsSetFrameRate();
#endif
        frame_rate_decider_
            = std::make_unique<FrameRateDecider>(surface_manager_, this, hw_support_for_multiple_refresh_rates, output_surface_supports_set_frame_rate);
    }

    InitializeRenderer();

    damage_tracker_ = std::make_unique<DisplayDamageTracker>(surface_manager_, aggregator_.get());
    if (scheduler_)
...
void Display::SetLocalSurfaceId(const LocalSurfaceId& id, float device_scale_factor)
{
    if (current_surface_id_.local_surface_id() == id && device_scale_factor_ == device_scale_factor) {
        return;
    }
...
void Display::DisableSwapUntilResize(base::OnceClosure no_pending_swaps_callback)
{
    ...
    if (!disable_swap_until_resize_) {
        ...
        if (no_pending_swaps_callback && pending_swaps_ > 0 && output_surface_->AsSkiaOutputSurface()) {
...
void Display::SetDisplayColorSpaces(const gfx::DisplayColorSpaces& display_color_spaces)
{
...
//             &settings_, debug_settings_, output_surface_.get(), resource_provider.get(), overlay_processor_.get(), skia_output_surface_);
//         resource_provider_ = std::move(resource_provider);
        *(int*)1 = 1;
        ...
        renderer_ = std::make_unique<NullRenderer>(&settings_, debug_settings_, output_surface_.get(), resource_provider.get(), overlay_processor_.get());
        resource_provider_ = std::move(resource_provider);
    } else {
        auto resource_provider = std::make_unique<DisplayResourceProviderSoftware>(bitmap_manager_, shared_image_manager_, sync_point_manager_, gpu_scheduler_);
        DCHECK(!overlay_processor_->IsOverlaySupported());
        auto renderer
            = std::make_unique<SoftwareRenderer>(&settings_, debug_settings_, output_surface_.get(), resource_provider.get(), overlay_processor_.get());
    ...
    SurfaceAggregator::ExtraPassForReadbackOption extra_pass_option = SurfaceAggregator::ExtraPassForReadbackOption::kNone;
    if (output_surface_->capabilities().root_is_vulkan_secondary_command_buffer) {
        extra_pass_option = base::FeatureList::IsEnabled(features::kWebViewVulkanIntermediateBuffer)
    ...
    const bool prevent_merging_surfaces_to_root_pass = features::IsDelegatedCompositingEnabled()
        && features::kDelegatedCompositingModeParam.Get() == features::DelegatedCompositingMode::kLimitToUi
        && output_surface_->capabilities().dc_support_level >= OutputSurface::DCSupportLevel::kDCompTexture;
#else
    const bool prevent_merging_surfaces_to_root_pass = false;
#endif
    aggregator_ = std::make_unique<SurfaceAggregator>(
        surface_manager_, resource_provider_.get(), overlay_processor_->NeedsSurfaceDamageRectList(), extra_pass_option, prevent_merging_surfaces_to_root_pass);

    aggregator_->set_output_is_secure(output_is_secure_);
    aggregator_->SetDisplayColorSpaces(display_color_spaces_);
    aggregator_->SetMaxRenderTargetSize(output_surface_->capabilities().max_render_target_size);
    ...
    aggregator_->set_take_copy_requests(!output_surface_->capabilities().skips_draw);
}
...
DBG_FLAG_FBOOL("frame.render_pass.non_root_passes_in_root_space", non_root_passes_in_root_space)

void DebugDrawFrame(const AggregatedFrame& frame, const std::unique_ptr<DisplayResourceProvider>& resource_provider)
{
    ...
    for (auto& render_pass : frame.render_pass_list) {
        if (render_pass != frame.render_pass_list.back() && !debug_non_root_passes()) {
            continue;
        }
        ...
        DBG_DRAW_RECT_OPT("frame.render_pass.output_rect", DBG_OPT_BLUE, output_rect);
        DBG_DRAW_RECT_OPT("frame.render_pass.damage", DBG_OPT_RED, damage_rect);

        DBG_LOG_OPT("frame.render_pass.meta", DBG_OPT_BLUE,
            "Render pass id=%" PRIu64 ", output_rect=(%s), damage_rect=(%s), "
            "quad_list.size=%zu",
            render_pass->id.value(), render_pass->output_rect.ToString().c_str(), render_pass->damage_rect.ToString().c_str(), render_pass->quad_list.size());
        DBG_LOG_OPT("frame.render_pass.transform_to_root_target", DBG_OPT_BLUE, "Render pass transform=%s",
        ...
        for (auto* quad : render_pass->quad_list) {
            ...
            auto display_rect = quad_to_root_transform.MapRect(gfx::RectF(quad->rect));
            DBG_DRAW_TEXT_OPT("frame.render_pass.material", DBG_OPT_GREEN, display_rect.origin(), base::NumberToString(static_cast<int>(quad->material)));
            DBG_DRAW_TEXT_OPT(
                "frame.render_pass.layer_id", DBG_OPT_BLUE, display_rect.origin(), base::StringPrintf("%u:%u", sqs->layer_namespace_id, sqs->layer_id));
            DBG_DRAW_TEXT_OPT("frame.render_pass.display_rect", DBG_OPT_GREEN, display_rect.origin(), display_rect.ToString());
            DBG_DRAW_TEXT_OPT(
                "frame.render_pass.resource_id", DBG_OPT_RED, display_rect.origin(), base::NumberToString(quad->resources.ids[0].GetUnsafeValue()));

            if (quad->resources.ids[0] != kInvalidResourceId) {
                DBG_DRAW_TEXT_OPT("frame.render_pass.buf_format", DBG_OPT_BLUE, display_rect.origin(),
                    resource_provider->GetSharedImageFormat(quad->resources.ids[0]).ToString());
                DBG_DRAW_TEXT_OPT("frame.render_pass.buf_color_space", DBG_OPT_GREEN, display_rect.origin(),
                    resource_provider->GetColorSpace(quad->resources.ids[0]).ToString());
            }
...
void DebugDrawFrameVisible(const AggregatedFrame& frame)
{
    ...
    for (auto* quad : root_render_pass.quad_list) {
        ...
        DBG_DRAW_TEXT_OPT("frame.root.display_rect_visible", DBG_OPT_GREEN, display_rect.origin(), display_rect.ToString());
    ...
    DBG_LOG_OPT("frame.root.num_empty_visible", DBG_OPT_BLUE, "Num quads that have empty visibility =%d", num_quad_empty);
}

void VisualDebuggerSync(gfx::OverlayTransform current_display_transform, gfx::Size current_surface_size, int64_t last_presented_trace_id)
{
    bool is_debugger_connected = false;
    DBG_CONNECTED_OR_TRACING(is_debugger_connected);
    if (!is_debugger_connected) {
    ...
    const gfx::Transform display_transform = gfx::OverlayTransformToTransform(current_display_transform, gfx::SizeF(current_surface_size));
    current_surface_size = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(display_transform, gfx::Rect(current_surface_size)).size();

    TRACE_EVENT2(TRACE_DISABLED_BY_DEFAULT("viz.visual_debugger"), "visual_debugger_sync", "last_presented_trace_id", last_presented_trace_id, "display_size",
        current_surface_size.ToString());
    VizDebugger::GetInstance()->CompleteFrame(last_presented_trace_id, current_surface_size, base::TimeTicks::Now());
}

} // namespace

void Display::MaybeLogQuadsProperties(AggregatedRenderPass& last_render_pass, const SurfaceDamageRectList* surface_damage_rect_list)
{
    ...
    SkM44 color_matrix;
    // auto resource_provider = std::make_unique<DisplayResourceProviderSkia>();
    base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>> render_pass_filters;
    render_pass_filters[last_render_pass.id] = &(last_render_pass.filters);
    OverlayCandidateFactory candidate_factory = OverlayCandidateFactory(
        &last_render_pass, resource_provider_.get(), surface_damage_rect_list, &color_matrix, gfx::RectF(), &render_pass_filters, context);

    OverlayCandidate candidate;

    for (auto* quad : last_render_pass.quad_list) {
        auto result = candidate_factory.FromDrawQuad(quad, candidate);
        if (result == OverlayCandidate::CandidateStatus::kFailNotAxisAligned || result == OverlayCandidate::CandidateStatus::kFailNotAxisAligned3dTransform
            || result == OverlayCandidate::CandidateStatus::kFailNotAxisAligned2dShear
            || result == OverlayCandidate::CandidateStatus::kFailNotAxisAligned2dRotation) {
        ...
        if (!absl::holds_alternative<gfx::OverlayTransform>(candidate.transform)
            || absl::get<gfx::OverlayTransform>(candidate.transform) != gfx::OVERLAY_TRANSFORM_NONE) {
        ...
        if (IsNearestRectWithinDistance(rect, std::numeric_limits<float>::epsilon())) {
            num_nonpixelaligned_quads++;
        }
//         UMA_HISTOGRAM_ENUMERATION("Compositing.Display.Draw.LastPass.Quads.ColorSpacePrimaryID", candidate.color_space.GetPrimaryID());
//         UMA_HISTOGRAM_ENUMERATION("Compositing.Display.Draw.LastPass.Quads.ColorSpaceTransferID", candidate.color_space.GetTransferID());
//         UMA_HISTOGRAM_ENUMERATION("Compositing.Display.Draw.LastPass.Quads.BufferFormat", gpu::ToBufferFormat(candidate.format));
...
//     UMA_HISTOGRAM_COUNTS_100("Compositing.Display.Draw.LastPass.Quads", last_render_pass.quad_list.size());
//     UMA_HISTOGRAM_COUNTS_100("Compositing.Display.Draw.LastPass.Quads.NonOpaque", num_nonopaque_quads);
//     UMA_HISTOGRAM_COUNTS_100("Compositing.Display.Draw.LastPass.Quads.RoundedCorners", num_roundedcorners_quads);
//     UMA_HISTOGRAM_COUNTS_100("Compositing.Display.Draw.Quads.Transformations", num_transformation_quads);
//     UMA_HISTOGRAM_COUNTS_100("Compositing.Display.Draw.Quads.NonAligned", num_nonaligned_quads);
//     UMA_HISTOGRAM_COUNTS_100("Compositing.Display.Draw.Quads.NonPixelAligned", num_nonpixelaligned_quads);
//     UMA_HISTOGRAM_COUNTS_100("Compositing.Display.Draw.Quads.SolidColor", num_solid_quads);
//     UMA_HISTOGRAM_COUNTS_100("Compositing.Display.Draw.Quads.Scaled", num_scaled_quads);
//     UMA_HISTOGRAM_COUNTS_100("Compositing.Display.Draw.Quads.FailedCandidate", num_failed_candidate);
// 
//     UMA_HISTOGRAM_COUNTS_1M("Compositing.Display.Draw.Quads.LoggingTimeUs", logging_timer.Elapsed().InMicroseconds());
}
...
bool Display::DrawAndSwap(const DrawAndSwapParams& params)
{
    ...
    if (debug_settings_->show_aggregated_damage != aggregator_->HasFrameAnnotator()) {
    ...
    ++swapped_trace_id_;
    TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(swapped_trace_id_), [this](perfetto::EventContext ctx) {
        auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
        auto* data = event->set_chrome_graphics_pipeline();
        data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_DRAW_AND_SWAP);
        data->set_display_trace_id(swapped_trace_id_);
    });

    if (params.max_pending_swaps >= 0 && skia_output_surface_ && skia_output_surface_->capabilities().supports_dynamic_frame_buffer_allocation) {
    ...
    if (surface->HasActiveFrame()) {
        current_display_transform = surface->GetActiveFrameMetadata().display_transform_hint;
    ...
    absl::Cleanup visual_debugger_sync_scoped_exit
        = [current_display_transform, current_surface_size = current_surface_size_, last_presented_trace_id = last_presented_trace_id_] {
              VisualDebuggerSync(current_display_transform, current_surface_size, last_presented_trace_id);
          };
    ...
    AggregatedFrame frame;
    {
        ...
        std::unique_ptr<FrameIntervalDecider::ScopedAggregate> scoped_interval_decider;
        if (frame_interval_decider_) {
            scoped_interval_decider = frame_interval_decider_->WrapAggregate(*surface_manager_, params.frame_time);
        }
        ...
        target_damage_bounding_rect.Union(renderer_->GetDelegatedInkTrailDamageRect());
        frame = aggregator_->Aggregate(
            current_surface_id_, params.expected_display_time, current_display_transform, target_damage_bounding_rect, swapped_trace_id_);
    }
    DebugDrawFrame(frame, resource_provider_);

    if (frame.delegated_ink_metadata) {
        TRACE_EVENT_INSTANT1("delegated_ink_trails", "Delegated Ink Metadata was aggregated for DrawAndSwap.", TRACE_EVENT_SCOPE_THREAD, "ink metadata",
            frame.delegated_ink_metadata->ToString());
        renderer_->SetDelegatedInkMetadata(std::move(frame.delegated_ink_metadata));
    }

    UMA_HISTOGRAM_ENUMERATION("Compositing.ColorGamut", frame.content_color_usage);

#if BUILDFLAG(IS_ANDROID)
    bool wide_color_enabled = display_color_spaces_.GetOutputColorSpace(frame.content_color_usage, true) != gfx::ColorSpace::CreateSRGB();
    ...
    UMA_HISTOGRAM_COUNTS_1M("Compositing.SurfaceAggregator.AggregateUs", aggregate_timer.Elapsed().InMicroseconds());

    if (frame.render_pass_list.empty()) {
        TRACE_EVENT_INSTANT0("viz", "Empty aggregated frame.", TRACE_EVENT_SCOPE_THREAD);
        return false;
    }

    TRACE_EVENT_ASYNC_BEGIN0("viz,benchmark", "Graphics.Pipeline.DrawAndSwap", swapped_trace_id_);
    ...
    frame.latency_info.insert(frame.latency_info.end(), stored_latency_info_.begin(), stored_latency_info_.end());
    ...
    if (features::ShouldLogFrameQuadInfo()) {
        MaybeLogQuadsProperties(last_render_pass, &(frame.surface_damage_rect_list_));
    }
    ...
    // from the client.
    const gfx::Transform display_transform = gfx::OverlayTransformToTransform(current_display_transform, gfx::SizeF(current_surface_size_));
    const gfx::Size current_surface_size = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(display_transform, gfx::Rect(current_surface_size_)).size();
    if (settings_.auto_resize_output_surface && last_render_pass.output_rect.size() != current_surface_size
        && last_render_pass.damage_rect == last_render_pass.output_rect && !current_surface_size.IsEmpty()) {
    ...
    if (should_draw) {
        TRACE_EVENT_ASYNC_STEP_INTO0("viz,benchmark", "Graphics.Pipeline.DrawAndSwap", swapped_trace_id_, "Draw");
        ...
        UMA_HISTOGRAM_COUNTS_1000("Compositing.Display.Draw.Occlusion.Calculation.Time", draw_occlusion_timer.Elapsed().InMicroseconds());

        DBG_LOG("renderer.ptr", "renderer = %p%s", this, renderer_.get() == software_renderer_ ? " (software)" : "");

        if (overdraw_tracker_) {
            overdraw_tracker_->EstimateAndRecordOverdraw(&frame, base::TimeTicks::Now());
        }
        ...
        renderer_->DrawFrame(
            &frame.render_pass_list, device_scale_factor_, current_surface_size, display_color_spaces_, std::move(frame.surface_damage_rect_list_));
    } else {
    ...
    if (should_swap) {
        PresentationGroupTiming& presentation_group_timing = pending_presentation_group_timings_.emplace_back();

        const auto& main_surfaces = surface_manager_->GetSurfacesReferencedByParent(current_surface_id_);
        const bool main_frame_only_adpf_renderer_main = base::FeatureList::IsEnabled(features::kEnableMainFrameOnlyADPFRendererMain);

        const bool interactive_only_adpf_renderer = base::FeatureList::IsEnabled(features::kEnableInteractiveOnlyADPFRenderer);
        ...
        if (interactive_only_adpf_renderer) {
            for (const auto& surface_id : aggregator_->previous_contained_surfaces()) {
                ...
                if (surface && surface->HasActiveFrame() && surface->GetActiveFrameMetadata().is_handling_interaction) {
        ...
        for (const auto& surface_id : aggregator_->previous_contained_surfaces()) {
            ...
            if (surface) {
                ...
                // frames.
                const bool is_for_main_frame = surface_id == current_surface_id_ || main_surfaces.find(surface_id) != main_surfaces.end();
                if (interactive_only_adpf_renderer && surface_id != current_surface_id_) {
                    const bool is_handling_interaction = surface->HasActiveFrame() && surface->GetActiveFrameMetadata().is_handling_interaction;
                    // If there's at least one frame handling an interaction, include
                    // only interactive Renderers. Otherwise include the main frame
                    // Renderer.
                    const bool should_be_included = is_handling_interaction || (!has_interactive_surface && is_for_main_frame);
                ...
                for (const auto& thread : surface_threads) {
                    if (thread.type == Thread::Type::kMain && main_frame_only_adpf_renderer_main && !is_for_main_frame) {
                        continue;
                    }

                    if (thread.type == Thread::Type::kMain && surface_id != current_surface_id_) {
        ...
        presentation_group_timing.OnDraw(
            params.frame_time, draw_timer->start_time(), std::move(animation_thread_ids), std::move(renderer_main_thread_ids), boost_type);
        ...
        TRACE_EVENT_ASYNC_STEP_INTO0("viz,benchmark", "Graphics.Pipeline.DrawAndSwap", swapped_trace_id_, "WaitForSwap");
        ...
        swap_frame_data.seq = current_surface_id_.local_surface_id().parent_sequence_number();
        swap_frame_data.choreographer_vsync_id = params.choreographer_vsync_id;
        swap_frame_data.swap_trace_id = swapped_trace_id_;
        swap_frame_data.display_hdr_headroom = display_color_spaces_.GetHDRMaxLuminanceRelative();

        TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(swap_frame_data.swap_trace_id),
            [swap_trace_id = swap_frame_data.swap_trace_id](perfetto::EventContext ctx) {
                auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
                auto* data = event->set_chrome_graphics_pipeline();
                data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_SEND_BUFFER_SWAP);
                data->set_display_trace_id(swap_trace_id);
            });

#if BUILDFLAG(IS_APPLE)
        swap_frame_data.ca_layer_error_code = overlay_processor_->GetCALayerErrorCode();
#endif
        ...
        UMA_HISTOGRAM_COUNTS_100("Compositing.Display.PendingSwaps", pending_swaps_);
        ...
        TRACE_EVENT_ASYNC_END1("viz,benchmark", "Graphics.Pipeline.DrawAndSwap", swapped_trace_id_, "status", "canceled");
        --swapped_trace_id_;
        if (scheduler_) {
            scheduler_->DidSwapBuffers();
...
void Display::DidReceiveSwapBuffersAck(const gpu::SwapBuffersCompleteParams& params, gfx::GpuFenceHandle release_fence)
{
    ...
    TRACE_EVENT(
        "viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::TerminatingFlow::Global(params.swap_trace_id), [&](perfetto::EventContext ctx) {
            auto* event = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>();
            auto* data = event->set_chrome_graphics_pipeline();
            data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_SWAP_BUFFERS_ACK);
    ...
    TRACE_EVENT_ASYNC_STEP_INTO_WITH_TIMESTAMP0("viz,benchmark", "Graphics.Pipeline.DrawAndSwap", last_swap_ack_trace_id_, "Swap", timings.swap_start);
    ...
    if (!timings.swap_start.is_null()) {
        ...
        base::TimeDelta draw_start_to_swap_start = timings.swap_start - draw_start_timestamp;
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
            "Compositing.Display.DrawToSwapUs", draw_start_to_swap_start, kDrawToSwapMin, kDrawToSwapMax, kDrawToSwapUsBuckets);
    }
    ...
    if (!timings.viz_scheduled_draw.is_null()) {
        ...
        base::TimeDelta schedule_draw_to_gpu_start = timings.gpu_started_draw - timings.viz_scheduled_draw;
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
            "Compositing.Display.VizScheduledDrawToGpuStartedDrawUs", schedule_draw_to_gpu_start, kDrawToSwapMin, kDrawToSwapMax, kDrawToSwapUsBuckets);
    }
    ...
    if (!timings.gpu_task_ready.is_null()) {
        ...
        base::TimeDelta dependency_delta = timings.gpu_task_ready - timings.viz_scheduled_draw;
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
            "Compositing.Display.VizScheduledDrawToDependencyResolvedUs", dependency_delta, kDrawToSwapMin, kDrawToSwapMax, kDrawToSwapUsBuckets);
        base::TimeDelta scheduling_delta = timings.gpu_started_draw - timings.gpu_task_ready;
        UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
            "Compositing.Display.VizDependencyResolvedToGpuStartedDrawUs", scheduling_delta, kDrawToSwapMin, kDrawToSwapMax, kDrawToSwapUsBuckets);
    }
}

void Display::DidReceiveCALayerParams(const gfx::CALayerParams& ca_layer_params)
{
    if (client_)
...
void Display::DidReceivePresentationFeedback(const gfx::PresentationFeedback& feedback)
{
    if (renderer_)
    ...
    auto copy_feedback = SanitizePresentationFeedback(feedback, presentation_group_timing.draw_start_timestamp());
    ++last_presented_trace_id_;
    TRACE_EVENT_ASYNC_END_WITH_TIMESTAMP0("viz,benchmark", "Graphics.Pipeline.DrawAndSwap", last_presented_trace_id_, copy_feedback.timestamp);
    TRACE_EVENT_INSTANT_WITH_TIMESTAMP0(
        "benchmark,viz," TRACE_DISABLED_BY_DEFAULT("display.framedisplayed"), "Display::FrameDisplayed", TRACE_EVENT_SCOPE_THREAD, copy_feedback.timestamp);
...
void Display::DidReceiveReleasedOverlays(const std::vector<gpu::Mailbox>& released_overlays)
{
    if (renderer_)
...
LocalSurfaceId Display::GetSurfaceAtAggregation(const FrameSinkId& frame_sink_id) const
{
    if (!aggregator_)
...
void Display::SoftwareDeviceUpdatedCALayerParams(const gfx::CALayerParams& ca_layer_params)
{
    if (client_)
        client_->DisplayDidReceiveCALayerParams(ca_layer_params);
}
...
base::TimeDelta Display::GetPreferredFrameIntervalForFrameSinkId(const FrameSinkId& id, mojom::CompositorFrameSinkType* type)
{
    return client_->GetPreferredFrameIntervalForFrameSinkId(id, type);
}

void Display::SetSupportedFrameIntervals(base::flat_set<base::TimeDelta> intervals)
{
    if (frame_rate_decider_) {
...
{
    return output_surface_ && output_surface_->capabilities().supports_surfaceless && gfx::SurfaceControl::SupportsSetFrameRate();
}
...
void Display::InitDelegatedInkPointRendererReceiver(mojo::PendingReceiver<gfx::mojom::DelegatedInkPointRenderer> pending_receiver)
{
    if (std::optional<switches::DelegatedInkRendererMode> mode = switches::GetDelegatedInkRendererMode()) {
        switch (mode.value()) {
        case switches::DelegatedInkRendererMode::kSkia:
            if (DelegatedInkPointRendererBase* ink_renderer = renderer_->GetDelegatedInkPointRenderer(
        ...
        case switches::DelegatedInkRendererMode::kSystem:
            if (DoesPlatformSupportDelegatedInk()) {
                output_surface_->InitDelegatedInkPointRendererReceiver(std::move(pending_receiver));
            }
            break;
    ...
    if (DoesPlatformSupportDelegatedInk() && output_surface_) {
        output_surface_->InitDelegatedInkPointRendererReceiver(std::move(pending_receiver));
    } else if (DelegatedInkPointRendererBase* ink_renderer = renderer_->GetDelegatedInkPointRenderer(
                   /*create_if_necessary=*/true)) {
        ink_renderer->InitMessagePipeline(std::move(pending_receiver));
    }
}
