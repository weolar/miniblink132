...
BASE_FEATURE(kSkipBeginFramesFromOtherDisplays, "SkipBeginFramesFromOtherDisplays", base::FEATURE_ENABLED_BY_DEFAULT);

bool ShouldAccumulateInteraction(SurfaceObserver::HandleInteraction handle_interaction)
{
...
DisplayDamageTracker::DisplayDamageTracker(SurfaceManager* surface_manager, SurfaceAggregator* aggregator)
...
void DisplayDamageTracker::SetDisplayBeginFrameSourceId(uint64_t begin_frame_source_id)
{
...
{
    TRACE_EVENT1("viz", "DisplayDamageTracker::SetRootFrameMissing", "missing", missing);
...
void DisplayDamageTracker::SetRootSurfaceDamaged()
{
    ...
    ProcessSurfaceDamage(root_surface_id_, ack, true, HandleInteraction::kNoChange);
}
...
void DisplayDamageTracker::ProcessSurfaceDamage(
    const SurfaceId& surface_id, const BeginFrameAck& ack, bool display_damaged, HandleInteraction handle_interaction)
{
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("viz.surface_lifetime"), "DisplayDamageTracker::SurfaceDamaged", "surface_id", surface_id.ToString());

    has_surface_damage_due_to_interaction_ |= ShouldAccumulateInteraction(handle_interaction);
    ...
    if (valid_ack) {
        ...
        if (it != surface_states_.end() && !it->second.last_ack.frame_id.IsNextInSequenceTo(ack.frame_id)) {
...
bool DisplayDamageTracker::SurfaceHasUnackedFrame(const SurfaceId& surface_id) const
{
...
bool DisplayDamageTracker::HasPendingSurfaces(const BeginFrameArgs& begin_frame_args)
{
    for (auto& entry : surface_states_) {
        ...
        if (!state.last_args.IsValid() || state.last_args.frame_id != begin_frame_args.frame_id) {
            continue;
        }
        ...
        TRACE_EVENT_INSTANT2("viz", "DisplayDamageTracker::HasPendingSurfaces", TRACE_EVENT_SCOPE_THREAD, "has_pending_surfaces", true, "pending_surface_id",
            surface_id.ToString());

        return true;
    }

    TRACE_EVENT_INSTANT1("viz", "DisplayDamageTracker::HasPendingSurfaces", TRACE_EVENT_SCOPE_THREAD, "has_pending_surfaces", false);
...
void DisplayDamageTracker::OnSurfaceMarkedForDestruction(const SurfaceId& surface_id)
{
    auto it = surface_states_.find(surface_id);
...
bool DisplayDamageTracker::OnSurfaceDamaged(const SurfaceId& surface_id, const BeginFrameAck& ack, HandleInteraction handle_interaction)
{
    bool display_damaged = false;
    if (ack.has_damage) {
        // Display is damaged if we purged some resources or if this surface
        // contributes to this display.
        display_damaged = aggregator_->ForceReleaseResourcesIfNeeded(surface_id) || CheckForDisplayDamage(surface_id);
...
{
    return !begin_frame_source_id_ || source_id == *begin_frame_source_id_ || source_id == BeginFrameArgs::kManualSourceId;
}

void DisplayDamageTracker::OnSurfaceDamageExpected(const SurfaceId& surface_id, const BeginFrameArgs& args)
{
    TRACE_EVENT1(TRACE_DISABLED_BY_DEFAULT("viz.surface_lifetime"), "DisplayDamageTracker::SurfaceDamageExpected", "surface_id", surface_id.ToString());
