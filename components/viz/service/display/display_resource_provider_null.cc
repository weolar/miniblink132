...
std::vector<ReturnedResource> DisplayResourceProviderNull::DeleteAndReturnUnusedResourcesToChildImpl(
    Child& child_info, DeleteStyle style, const std::vector<ResourceId>& unused)
{
    ...
    for (ResourceId local_id : unused) {
        ...
        to_return.emplace_back(child_id, resource.sync_token(), std::move(resource.release_fence), resource.imported_count, /*is_lost=*/false);
