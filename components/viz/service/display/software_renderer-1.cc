...
void SoftwareRenderer::DrawTileQuad(const TileDrawQuad* quad)
{
    ...
    current_canvas_->drawCircle(130, 130, 60, paint); // weolar

    SkRect dst = SkRect::MakeWH(256, 256);
    current_canvas_->drawImageRect(lock.sk_image(), uv_rect, dst, sampling, nullptr, SkCanvas::kStrict_SrcRectConstraint);

    sk_sp<SkData> data = lock.sk_image()->encodeToData();
    if (0)
        saveDumpFile(L"G:\\mycode\\mb108\\content\\demo\\1.png", (const char*)data->data(), data->size());
}
