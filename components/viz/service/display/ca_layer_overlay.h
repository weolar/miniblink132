...
public:
    ...
    bool AreClipSettingsValid(const OverlayCandidate& ca_layer_overlay, OverlayCandidateList* ca_layer_overlay_list) const;
    void PutForcedOverlayContentIntoUnderlays(const DisplayResourceProvider* resource_provider, AggregatedRenderPass* render_pass,
    ...
    virtual bool ProcessForCALayerOverlays(AggregatedRenderPass* render_passes, const DisplayResourceProvider* resource_provider,
    ...
    gfx::CALayerResult ca_layer_result()
    {
        return ca_layer_result_;
    }

private:
    // Returns whether future candidate quads should be considered
    bool PutQuadInSeparateOverlay(QuadList::Iterator at, const DisplayResourceProvider* resource_provider, AggregatedRenderPass* render_pass,
        const gfx::RectF& display_rect, const DrawQuad* quad,
        const base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>>& render_pass_filters,
        const base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>>& render_pass_backdrop_filters,
