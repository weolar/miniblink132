...
bool IntersectsWithRoundedDisplayMask(const DrawQuad* quad, std::vector<DrawQuad*> quads_with_masks)
{
    ...
    for (const auto* mask_quad : quads_with_masks) {
        const auto mask_bounds = TextureDrawQuad::RoundedDisplayMasksInfo::GetRoundedDisplayMasksBounds(mask_quad);
...
OverlayStrategyFullscreen::OverlayStrategyFullscreen(OverlayProcessorUsingStrategy* capability_checker)
...
void OverlayStrategyFullscreen::Propose(const SkM44& output_color_matrix, const OverlayProcessorInterface::FilterOperationsMap& render_pass_filters,
    ...
    std::vector<OverlayProposedCandidate>* candidates, std::vector<gfx::Rect>* content_bounds)
{
    ...
    while (front != quad_list->end()) {
        const bool has_mask_textures = OverlayCandidate::QuadHasRoundedDisplayMasks(*front);
    ...
    if (candidate_factory.FromDrawQuad(quad, candidate) != OverlayCandidate::CandidateStatus::kSuccess) {
        return;
    }

    if (!candidate.display_rect.origin().IsOrigin() || gfx::ToRoundedSize(candidate.display_rect.size()) != render_pass->output_rect.size()) {
        ...
        while (after_front != quad_list->end()) {
            if (!(*after_front)->visible_rect.IsEmpty() && !OverlayCandidate::IsInvisibleQuad(*after_front)) {
                auto* solid_color_quad = (*after_front)->DynamicCast<SolidColorDrawQuad>();
                if (!solid_color_quad) {
                    return;
                }
...
bool OverlayStrategyFullscreen::Attempt(const SkM44& output_color_matrix, const OverlayProcessorInterface::FilterOperationsMap& render_pass_filters,
    const OverlayProcessorInterface::FilterOperationsMap& render_pass_backdrop_filters, const DisplayResourceProvider* resource_provider,
    AggregatedRenderPassList* render_pass_list, SurfaceDamageRectList* surface_damage_rect_list, const PrimaryPlane* primary_plane,
...
void OverlayStrategyFullscreen::CommitCandidate(const OverlayProposedCandidate& proposed_candidate, AggregatedRenderPass* render_pass)
{
