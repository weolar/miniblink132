...
{
    constexpr char kAverageOverdrawHistogramName[] = "Compositing.Display.Draw.AverageOverdraw2";
    ...
    UMA_HISTOGRAM_CUSTOM_COUNTS(kAverageOverdrawHistogramName, base::ClampRound<int>(overdraw * kConversionFactor),
...
// static
void OverdrawTracker::EstimateAndRecordOverdrawAsUMAMetric(const AggregatedFrame* frame)
{
...
float OverdrawTracker::EstimateOverdraw(const AggregatedFrame* frame)
{
    ...
    for (const auto& pass : frame->render_pass_list) {
        for (auto quad = pass->quad_list.begin(); quad != pass->quad_list.end(); ++quad) {
            ...
            auto quad_rect = gfx::ToEnclosingRect(quad_to_root_transform.MapRect(gfx::RectF(quad->visible_rect)));
            if (sqs->clip_rect) {
    ...
    return static_cast<float>(overdraw.ValueOrDefault(0)) / display_rect.size().Area64();
}
...
void OverdrawTracker::EstimateAndRecordOverdraw(const AggregatedFrame* frame, base::TimeTicks timestamp)
{
    const float overdraw = EstimateOverdraw(frame);
    Record(overdraw, timestamp);
}

OverdrawTracker::OverdrawTimeSeries OverdrawTracker::TakeDataAsTimeSeries() const
{
    ...
    std::transform(interval_overdraw_averages_.begin(), interval_overdraw_averages_.end(), averages.begin(),
...
void OverdrawTracker::Record(float overdraw, base::TimeTicks timestamp)
{
    ...
    DecomposedAverage& interval_average = interval_overdraw_averages_.at(interval_index);
    interval_average.AddValue(overdraw);
}

size_t OverdrawTracker::GetIntervalIndex(base::TimeTicks timestamp) const
{
    return (timestamp - start_time_).InSeconds() / settings_.interval_length_in_seconds;
}
