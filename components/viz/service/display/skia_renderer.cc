...
BASE_FEATURE(kDumpWithoutCrashingOnMissingRenderPassBacking, "DumpWithoutCrashingOnMissingRenderPassBacking", base::FEATURE_DISABLED_BY_DEFAULT);
...
bool IsTextureResource(DisplayResourceProviderSkia* resource_provider, ResourceId resource_id)
{
    return !resource_provider->IsResourceSoftwareBacked(resource_id);
}

unsigned GetCornerAAFlags(const DrawQuad* quad, const SkPoint& vertex, unsigned edge_mask)
{
    ...
    if (std::abs(vertex.x() - quad->shared_quad_state->quad_layer_rect.width()) < kAAEpsilon)
        mask |= SkCanvas::kRight_QuadAAFlag;
    if (std::abs(vertex.y()) < kAAEpsilon)
        mask |= SkCanvas::kTop_QuadAAFlag;
    if (std::abs(vertex.y() - quad->shared_quad_state->quad_layer_rect.height()) < kAAEpsilon)
...
{
    return transform.IsScaleOrTranslation() && transform.rc(0, 0) >= 0.0f && transform.rc(1, 1) >= 0.0f && transform.rc(2, 2) >= 0.0f;
}
...
unsigned GetRectilinearEdgeFlags(const DrawQuad* quad)
{
    ...
    unsigned mask = SkCanvas::kNone_QuadAAFlags;
    if (quad->IsLeftEdge() && std::abs(quad->rect.x() - quad->visible_rect.x()) < kAAEpsilon)
        mask |= SkCanvas::kLeft_QuadAAFlag;
    if (quad->IsTopEdge() && std::abs(quad->rect.y() - quad->visible_rect.y()) < kAAEpsilon)
        mask |= SkCanvas::kTop_QuadAAFlag;
    if (quad->IsRightEdge() && std::abs(quad->rect.right() - quad->visible_rect.right()) < kAAEpsilon)
        mask |= SkCanvas::kRight_QuadAAFlag;
    if (quad->IsBottomEdge() && std::abs(quad->rect.bottom() - quad->visible_rect.bottom()) < kAAEpsilon)
        mask |= SkCanvas::kBottom_QuadAAFlag;
...
void GetClippedEdgeFlags(const DrawQuad* quad, unsigned* edge_mask, SkDrawRegion* draw_region)
{
    ...
    if (draw_region->points[2] == draw_region->points[3]) {
        ...
        if (p0Mask && !(mask & SkCanvas::kLeft_QuadAAFlag) && !(mask & SkCanvas::kTop_QuadAAFlag)) {
        ...
        } else if (p1Mask && !(mask & SkCanvas::kTop_QuadAAFlag) && !(mask & SkCanvas::kRight_QuadAAFlag)) {
...
bool IsAAForcedOff(const DrawQuad* quad)
{
    ...
    case DrawQuad::Material::kCompositorRenderPass:
        ...
        return CompositorRenderPassDrawQuad::MaterialCast(quad)->force_anti_aliasing_off;
    case DrawQuad::Material::kAggregatedRenderPass:
        return AggregatedRenderPassDrawQuad::MaterialCast(quad)->force_anti_aliasing_off;
...
SkCanvas::SrcRectConstraint GetTextureConstraint(const SkImage* image, const gfx::RectF& vis_tex_coords, const gfx::RectF& valid_texel_bounds)
{
    ...
    // contains() check.
    if ((!fills_left && vis_tex_coords.x() < safe_texels.x()) || (!fills_right && vis_tex_coords.right() > safe_texels.right())) {
        return SkCanvas::kStrict_SrcRectConstraint;
    }
    if ((!fills_top && vis_tex_coords.y() < safe_texels.y()) || (!fills_bottom && vis_tex_coords.bottom() > safe_texels.bottom())) {
        return SkCanvas::kStrict_SrcRectConstraint;
    }
...
sk_sp<SkColorFilter> MakeOpacityFilter(float alpha, sk_sp<SkColorFilter> in)
{
    ...
    sk_sp<SkColorFilter> opacity = SkColorFilters::Blend(alpha_as_color.toSkColor(), SkBlendMode::kDstIn);
...
bool TransparentBlackAffectsOutput(SkBlendMode blendMode)
{
    ...
    return dst != SkBlendModeCoeff::kOne && dst != SkBlendModeCoeff::kISA && dst != SkBlendModeCoeff::kISC;
}
...
bool RenderPassPreservesContent(SkBlendMode blendMode)
{
    ...
    return src == SkBlendModeCoeff::kOne || src == SkBlendModeCoeff::kIDA || src == SkBlendModeCoeff::kIDC;
}
...
bool RenderPassRemainsTransparent(SkBlendMode blendMode)
{
    ...
    return src == SkBlendModeCoeff::kZero || src == SkBlendModeCoeff::kDA || src == SkBlendModeCoeff::kDC;
}

#if BUILDFLAG(ENABLE_VULKAN) && BUILDFLAG(IS_CHROMEOS) && BUILDFLAG(USE_V4L2_CODEC)
...
public:
    static void DebugLogDumpRenderPassBackings(const base::flat_map<AggregatedRenderPassId, RenderPassBacking>& render_pass_backings)
    {
        bool enabled;
        DBG_CONNECTED_OR_TRACING(enabled);
        if (enabled) {
            DBG_LOG("renderer.skia.render_pass_backings", "render_pass_backings_ = [");
            for (auto& [render_pass_id, backing] : render_pass_backings) {
                ...
                base::trace_event::TracedValue::Dictionary({
                                                               ...
                                                               { "alpha_type", backing.alpha_type == RenderPassAlphaType::kPremul ? "premul" : "opaque" },
                    ...
                    .WriteToValue(&value);
                DBG_LOG("renderer.skia.render_pass_backings", "%" PRIu64 ": %s", render_pass_id.value(), value.ToFormattedJSON().c_str());
            }
            DBG_LOG("renderer.skia.render_pass_backings", "]");
        }
    }

    static void DebugLogNewRenderPassBacking(const AggregatedRenderPassId& render_pass_id, const RenderPassRequirements& requirements)
    {
        bool enabled;
        DBG_CONNECTED_OR_TRACING(enabled);
        if (enabled) {
            ...
            base::trace_event::TracedValue::Dictionary({
                ...
                .WriteToValue(&value);
            DBG_LOG("renderer.skia.render_pass_backings", "allocate backing for render_pass %" PRIu64 ", %s", render_pass_id.value(),
                value.ToFormattedJSON().c_str());
        }
    }
};
...
SkiaRenderer::RenderPassBacking::RenderPassBacking(const SkiaRenderer::RenderPassBacking&) = default;

SkiaRenderer::RenderPassBacking& SkiaRenderer::RenderPassBacking::operator=(const SkiaRenderer::RenderPassBacking&) = default;

SkiaRenderer::RenderPassBacking::RenderPassBacking(gfx::Size size, bool generate_mipmap, gfx::ColorSpace color_space, RenderPassAlphaType alpha_type,
    SharedImageFormat format, gpu::Mailbox mailbox, bool is_root, bool is_scanout, bool scanout_dcomp_surface)
    : size(size)
...
struct SkiaRenderer::DrawQuadParams {
    ...
    DrawQuadParams(const gfx::Transform& cdt, const gfx::RectF& rect, const gfx::RectF& visible_rect, unsigned aa_flags, SkBlendMode blend_mode, float opacity,
        const SkSamplingOptions& sampling, const gfx::QuadF* draw_region);
    ...
    {
        if (draw_region) {
            return SkPath::Polygon(draw_region->points.data(), std::size(draw_region->points),
                /*isClosed=*/true);
        }
        return SkPath();
    }

    void ApplyScissor(const SkiaRenderer* renderer, const DrawQuad* quad, const std::optional<gfx::Rect>& scissor_to_apply);
};

SkiaRenderer::DrawQuadParams::DrawQuadParams(const gfx::Transform& cdt, const gfx::RectF& rect, const gfx::RectF& visible_rect, unsigned aa_flags,
    SkBlendMode blend_mode, float opacity, const SkSamplingOptions& sampling, const gfx::QuadF* draw_region)
...
public:
    ScopedSkImageBuilder(SkiaRenderer* skia_renderer, ResourceId resource_id, bool maybe_concurrent_reads, SkAlphaType alpha_type = kPremul_SkAlphaType,
        GrSurfaceOrigin origin = kTopLeft_GrSurfaceOrigin, sk_sp<SkColorSpace> override_color_space = nullptr, bool raw_draw_if_possible = false,
    ...
    const std::optional<SkColor4f>& clear_color() const
    {
        return clear_color_;
    }
...
SkiaRenderer::ScopedSkImageBuilder::ScopedSkImageBuilder(SkiaRenderer* skia_renderer, ResourceId resource_id, bool maybe_concurrent_reads,
    SkAlphaType alpha_type, GrSurfaceOrigin origin, sk_sp<SkColorSpace> override_color_space, bool raw_draw_if_possible, bool force_rgbx)
{
    if (!resource_id)
        return;
    ...
    auto* image_context = skia_renderer->lock_set_for_external_use_.LockResource(resource_id, maybe_concurrent_reads, raw_draw_if_possible);
    ...
    // conversion.
    skia_renderer->skia_output_surface_->MakePromiseSkImage(image_context, resource_provider->GetColorSpace(resource_id), force_rgbx);
    paint_op_buffer_ = image_context->paint_op_buffer();
    clear_color_ = image_context->clear_color();
    sk_image_ = image_context->image();
    LOG_IF(ERROR, !image_context->has_image() && !paint_op_buffer_) << "Failed to create the promise sk image or get paint ops.";
...
struct SkiaRenderer::DrawRPDQParams {
    ...
    DrawRPDQParams()
        : filter_bounds(SkRect::MakeEmpty())
    {
    }
    ...
    bool needs_bypass_clip(const gfx::RectF& content_rect) const
    {
        ...
        SkRect content_bounds = bypass_geometry->transform.mapRect(gfx::RectFToSkRect(content_rect));
        return !bypass_geometry->clip_rect.Contains(gfx::SkRectToRectF(content_bounds));
    }
    ...
    {
        return gfx::RectFToSkRect(bypass_geometry ? bypass_geometry->clip_rect : params->visible_rect);
    }
    ...
    // visuals.
    void SetBackdropFilterClip(SkCanvas* canvas, const DrawQuadParams* params) const;
    ...
    void ClearOutsideBackdropBounds(SkCanvas* canvas, const DrawQuadParams* params) const;
};

sk_sp<SkShader> SkiaRenderer::DrawRPDQParams::MaskShader::GetOrCreateSkShader(SkiaRenderer* renderer) const
{
    if (sk_shader_) {
        return sk_shader_;
    }
    ...
    CHECK(mask_image);
    sk_shader_
        = mask_image->makeShader(SkTileMode::kClamp, SkTileMode::kClamp, SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kNone), &mask_to_quad_matrix_);

    return sk_shader_;
}

void SkiaRenderer::DrawRPDQParams::SetBackdropFilterClip(SkCanvas* canvas, const DrawQuadParams* params) const
{
    if (!backdrop_filter) {
...
void SkiaRenderer::DrawRPDQParams::ClearOutsideBackdropBounds(SkCanvas* canvas, const DrawQuadParams* params) const
{
    if (!backdrop_filter) {
    ...
    if (params->draw_region) {
        ...
        if (!content.contains(filter_bounds) && (!backdrop_filter_bounds || !content.contains(backdrop_filter_bounds->rect()))) {
...
class SkiaRenderer::FrameResourceGpuCommandsCompletedFence : public ResourceFence {
public:
    explicit FrameResourceGpuCommandsCompletedFence(DisplayResourceProviderSkia* resource_provider)
        : ResourceFence(resource_provider)
    {
    }
    FrameResourceGpuCommandsCompletedFence() = delete;
    FrameResourceGpuCommandsCompletedFence(const FrameResourceGpuCommandsCompletedFence&) = delete;
    FrameResourceGpuCommandsCompletedFence& operator=(const FrameResourceGpuCommandsCompletedFence&) = delete;
...
public:
    explicit FrameResourceReleaseFence(DisplayResourceProviderSkia* resource_provider)
        : ResourceFence(resource_provider)
    {
    }
    FrameResourceReleaseFence() = delete;
    FrameResourceReleaseFence(const FrameResourceReleaseFence&) = delete;
    FrameResourceReleaseFence& operator=(const FrameResourceReleaseFence&) = delete;

    // ResourceFence implementation:
    bool HasPassed() override
    {
        return release_fence_.has_value();
    }
    gfx::GpuFenceHandle GetGpuFenceHandle() override
    {
...
SkiaRenderer::SkiaRenderer(const RendererSettings* settings, const DebugRendererSettings* debug_settings, OutputSurface* output_surface,
    DisplayResourceProviderSkia* resource_provider, OverlayProcessorInterface* overlay_processor, SkiaOutputSurface* skia_output_surface)
    : DirectRenderer(settings, debug_settings, output_surface, resource_provider, overlay_processor)
    ...
    , is_using_raw_draw_(features::IsUsingRawDraw())
{
    ...
    use_render_pass_drawn_rect_
        |= features::IsDelegatedCompositingEnabled() && features::kDelegatedCompositingModeParam.Get() == features::DelegatedCompositingMode::kLimitToUi;
#endif
    ...
    current_gpu_commands_completed_fence_ = base::MakeRefCounted<FrameResourceGpuCommandsCompletedFence>(resource_provider);
    current_release_fence_ = base::MakeRefCounted<FrameResourceReleaseFence>(resource_provider);
    this->resource_provider()->SetGpuCommandsCompletedFence(current_gpu_commands_completed_fence_.get());
    ...
    const bool want_buffer_queue
        = base::FeatureList::IsEnabled(kBufferQueue) && output_surface_->capabilities().dc_support_level >= OutputSurface::DCSupportLevel::kDCompTexture;
#else
    const bool want_buffer_queue = true;
#endif
    if (want_buffer_queue && output_surface->capabilities().renderer_allocates_images) {
        ...
        size_t number_of_buffers
            = output_surface->capabilities().supports_dynamic_frame_buffer_allocation ? 0 : output_surface->capabilities().number_of_buffers;
        buffer_queue_ = std::make_unique<BufferQueue>(skia_output_surface_, skia_output_surface_->GetSurfaceHandle(), number_of_buffers);
    }

#if BUILDFLAG(ENABLE_VULKAN) && BUILDFLAG(IS_CHROMEOS) && BUILDFLAG(USE_V4L2_CODEC)
    protected_buffer_queue_ = std::make_unique<BufferQueue>(skia_output_surface_, skia_output_surface_->GetSurfaceHandle(), 3,
        /*is_protected=*/true);
#endif
}
...
void SkiaRenderer::FinishDrawingFrame()
{
    ...
    if (current_frame()->output_surface_plane) {
        ...
        auto root_pass_backing = render_pass_backings_.find(current_frame()->root_render_pass->id);
        ...
        if (current_frame()->display_color_spaces.SupportsHDR() && current_frame()->root_render_pass->content_color_usage == gfx::ContentColorUsage::kHDR) {
            ...
            surface_candidate.hdr_metadata.extended_range->desired_headroom = gfx::HdrMetadataExtendedRange::kDefaultHdrHeadroom;
        }
        ...
        surface_candidate.damage_rect = use_partial_swap_ ? gfx::RectF(swap_buffer_rect_) : gfx::RectF(surface_plane.resource_size);
#if BUILDFLAG(IS_MAC)
...
#if BUILDFLAG(IS_CHROMEOS) && BUILDFLAG(ENABLE_VULKAN) && BUILDFLAG(USE_V4L2_CODEC)
...
gpu::Mailbox SkiaRenderer::GetProtectedSharedImage(bool is_10bit)
{
    ...
    protected_buffer_queue_->Reshape(gfx::Size(kMaxProtectedContentWidth, kMaxProtectedContentHeight), gfx::ColorSpace::CreateSRGB(),
        RenderPassAlphaType::kPremul,
        (is_10bit && base::FeatureList::IsEnabled(media::kEnableArmHwdrm10bitOverlays)) ? SinglePlaneFormat::kBGRA_1010102 : SinglePlaneFormat::kBGRA_8888);
...
{
    DCHECK(visible_);
    DCHECK(output_surface_->capabilities().supports_viewporter || viewport_size_for_swap_buffers() == surface_size_for_swap_buffers());
    ...
    if (delegated_ink_handler_ && !UsingSkiaForDelegatedInk()) {
        output_frame.delegated_ink_metadata = delegated_ink_handler_->TakeMetadata();
    }
    ...
    if (buffer_queue_) {
        gfx::Rect damage_rect = output_frame.sub_buffer_rect.value_or(gfx::Rect(surface_size_for_swap_buffers()));
    ...
    for (auto& overlay : available_render_pass_overlay_backings_) {
        skia_output_surface_->DestroySharedImage(overlay.render_pass_backing.mailbox);
    }
    ...
    if (protected_buffer_queue_) {
        ...
        protected_buffer_queue_->SwapBuffers(gfx::Rect(kMaxProtectedContentWidth, kMaxProtectedContentHeight));
    }
...
void SkiaRenderer::SwapBuffersComplete(const gpu::SwapBuffersCompleteParams& params, gfx::GpuFenceHandle release_fence)
{
    auto& read_lock_release_fence_overlay_locks = read_lock_release_fence_overlay_locks_.emplace_back();
    ...
    if (buffer_queue_) {
        if (params.swap_response.result == gfx::SwapResult::SWAP_NAK_RECREATE_BUFFERS) {
    ...
    if (!release_fence.is_null()) {
        ...
        read_fence_lock_iter
            = std::partition(committed_overlay_locks_.begin(), committed_overlay_locks_.end(), [](auto& lock) { return !lock.HasReadLockFence(); });
        read_lock_release_fence_overlay_locks.insert(read_lock_release_fence_overlay_locks.end(), std::make_move_iterator(read_fence_lock_iter),
    ...
    for (auto lock_iter = committed_overlay_locks_.begin(); lock_iter != read_fence_lock_iter; ++lock_iter) {
    ...
    {
        DisplayResourceProvider::ScopedBatchReturnResources returner(resource_provider_.get(), /*allow_access_to_gpu_thread=*/true);
...
void SkiaRenderer::DidReceiveReleasedOverlays(const std::vector<gpu::Mailbox>& released_overlays)
{
#if BUILDFLAG(IS_APPLE)
    DisplayResourceProvider::ScopedBatchReturnResources returner(resource_provider_.get(), /*allow_access_to_gpu_thread=*/true);
...
bool SkiaRenderer::NeedsLayerForColorConversion(const AggregatedRenderPass* render_pass)
{
    ...
    const gfx::ColorSpace& pass_color_space
        = it != render_pass_backings_.end() ? it->second.color_space : RenderPassColorSpace(current_frame()->root_render_pass);
...
{
    return hdr_color_conversion_layer_reset_ ? gfx::ColorSpace::CreateExtendedSRGB() : RenderPassColorSpace(current_frame()->current_render_pass);
}

void SkiaRenderer::BeginDrawingRenderPass(
    const AggregatedRenderPass* render_pass, bool needs_clear, const gfx::Rect& render_pass_update_rect, const gfx::Size& viewport_size)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::BeginDrawingRenderPass");
    ...
    if (is_root && !output_surface_->capabilities().renderer_allocates_images) {
        ...
        current_canvas_ = skia_output_surface_->BeginPaintRenderPass(render_pass->id, backing.size, backing.format, backing.alpha_type,
            backing.generate_mipmap ? skgpu::Mipmapped::kYes : skgpu::Mipmapped::kNo, backing.scanout_dcomp_surface, RenderPassBackingSkColorSpace(backing),
    ...
    if (NeedsLayerForColorConversion(render_pass)) {
        ...
        const gfx::ColorSpace blend_color_space = gfx::ColorSpace::CreateExtendedSRGB();
        CHECK(blend_color_space.IsSuitableForBlending());
        sk_sp<const SkColorSpace> color_space = blend_color_space.ToSkColorSpace();
        current_canvas_->saveLayer(SkCanvas::SaveLayerRec(&layer_bounds, &no_blend,
            /*backdrop=*/nullptr, color_space.get(), SkCanvas::SaveLayerFlagsSet::kF16ColorType));
    }
...
void SkiaRenderer::DoDrawQuad(const DrawQuad* quad, const gfx::QuadF* draw_region)
{
    if (!current_canvas_)
        return;
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::DoDrawQuad");
    DrawQuadParams params = CalculateDrawQuadParams(current_frame()->target_to_device_transform, scissor_rect_, quad, draw_region);
    // The outer DrawQuad will never have RPDQ params
    DrawQuadInternal(quad, /* rpdq */ nullptr, &params);
}

void SkiaRenderer::DrawQuadInternal(const DrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params)
{
    ...
    switch (quad->material) {
    case DrawQuad::Material::kAggregatedRenderPass:
        DrawRenderPassQuad(AggregatedRenderPassDrawQuad::MaterialCast(quad), rpdq_params, params);
        break;
    ...
    case DrawQuad::Material::kCompositorRenderPass:
        ...
        break;
    case DrawQuad::Material::kSolidColor:
        DrawSolidColorQuad(SolidColorDrawQuad::MaterialCast(quad), rpdq_params, params);
        break;
    case DrawQuad::Material::kTextureContent:
...
void SkiaRenderer::PrepareCanvas(
    const std::optional<gfx::Rect>& scissor_rect, const std::optional<gfx::MaskFilterInfo>& mask_filter_info, const gfx::Transform* cdt)
{
    ...
    if (mask_filter_info.has_value()) {
        current_canvas_->clipRRect(static_cast<SkRRect>(mask_filter_info->rounded_corner_bounds()),
            /*doAntiAlias=*/true);
...
void SkiaRenderer::PrepareGradient(const std::optional<gfx::MaskFilterInfo>& mask_filter_info)
{
    if (!mask_filter_info || !mask_filter_info->HasGradientMask())
        return;

    const gfx::RectF rect = mask_filter_info->bounds();
    const std::optional<gfx::LinearGradient>& gradient_mask = mask_filter_info->gradient_mask();
    ...
    sk_sp<SkShader> gradient = SkGradientShader::MakeLinear(start_end, gradient_colors.data(), positions.data(), /*count=*/i, SkTileMode::kClamp);
...
void SkiaRenderer::PrepareCanvasForRPDQ(const DrawRPDQParams& rpdq_params, DrawQuadParams* params)
{
    ...
    current_canvas_->saveLayer(
        SkCanvasPriv::ScaledBackdropLayer(&bounds, &layer_paint, rpdq_params.backdrop_filter.get(), rpdq_params.backdrop_filter_quality, 0));
...
void SkiaRenderer::PreparePaintOrCanvasForRPDQ(const DrawRPDQParams& rpdq_params, DrawQuadParams* params, SkPaint* paint)
{
    ...
    if (rpdq_params.mask_shader) {
        ...
        current_canvas_->clipShader(rpdq_params.mask_shader->GetOrCreateSkShader(this));
    }
    ...
    if (needs_save_layer) {
        ...
        if (rpdq_params.color_filter) {
            ...
            if (paint->getColorFilter()) {
                paint->setColorFilter(rpdq_params.color_filter->makeComposed(paint->refColorFilter()));
            } else {
            ...
            if (params->opacity != 1.f) {
                ...
                paint->setImageFilter(SkImageFilters::ColorFilter(MakeOpacityFilter(params->opacity, nullptr), rpdq_params.image_filter));
                paint->setAlphaf(1.f);
                params->opacity = 1.f;
            } else {
    ...
    if (needs_bypass_clip) {
        current_canvas_->clipRect(gfx::RectFToSkRect(rpdq_params.bypass_geometry->clip_rect), params->aa_flags != SkCanvas::kNone_QuadAAFlags);
    }
}

void SkiaRenderer::PrepareColorOrCanvasForRPDQ(const DrawRPDQParams& rpdq_params, DrawQuadParams* params, SkColor4f* content_color)
{
    ...
    bool needs_save_layer = rpdq_params.has_complex_image_filter() || rpdq_params.backdrop_filter;
    if (rpdq_params.mask_shader) {
        current_canvas_->clipShader(rpdq_params.mask_shader->GetOrCreateSkShader(this));
    }
    ...
    if (needs_save_layer) {
        ...
        *content_color = rpdq_params.color_filter->filterColor4f(*content_color, cs, cs);
    }
...
SkiaRenderer::DrawQuadParams SkiaRenderer::CalculateDrawQuadParams(
    const gfx::AxisTransform2d& target_to_device, const std::optional<gfx::Rect>& scissor_rect, const DrawQuad* quad, const gfx::QuadF* draw_region) const
{
    DrawQuadParams params(quad->shared_quad_state->quad_to_target_transform, gfx::RectF(quad->rect), gfx::RectF(quad->visible_rect),
        SkCanvas::kNone_QuadAAFlags, quad->shared_quad_state->blend_mode, quad->shared_quad_state->opacity, GetSampling(quad), draw_region);
...
void SkiaRenderer::DrawQuadParams::ApplyScissor(const SkiaRenderer* renderer, const DrawQuad* quad, const std::optional<gfx::Rect>& scissor_to_apply)
{
    ...
    if (const auto* quad_pass = quad->DynamicCast<AggregatedRenderPassDrawQuad>()) {
        ...
        if (const auto* filters = renderer->FiltersForPass(pass_id); filters && filters->HasFilterThatMovesPixels()) {
            return;
        }
    }
    ...
    std::optional<gfx::RectF> local_scissor = content_device_transform.InverseMapRect(gfx::RectF(*scissor_rect));
    if (!local_scissor) {
        return;
    }
    gfx::RectF remapped_scissor = content_device_transform.MapRect(*local_scissor);
...
const DrawQuad* SkiaRenderer::CanPassBeDrawnDirectly(const AggregatedRenderPass* pass, const RenderPassRequirements& requirements)
{
    ...
    // DrawRPDQParams.
    if (quad->material == DrawQuad::Material::kDebugBorder || quad->material == DrawQuad::Material::kPictureContent)
        return nullptr;
    ...
    SkMatrix flattened = gfx::TransformToFlattenedSkMatrix(quad->shared_quad_state->quad_to_target_transform);
    ...
    DCHECK(RenderPassPreservesContent(quad->shared_quad_state->blend_mode) || RenderPassRemainsTransparent(quad->shared_quad_state->blend_mode));
    ...
    if (const auto* render_pass_quad = quad->DynamicCast<AggregatedRenderPassDrawQuad>()) {
        ...
        if (!Is2dScaleTranslateTransform(render_pass_quad->shared_quad_state->quad_to_target_transform)) {
            return nullptr;
        }

        const auto nested_render_pass_id = render_pass_quad->render_pass_id;
        auto it = base::ranges::find_if(*current_frame()->render_passes_in_draw_order,
            [&nested_render_pass_id](const auto& render_pass) { return render_pass->id == nested_render_pass_id; });

        CHECK(it != current_frame()->render_passes_in_draw_order->end(), base::NotFatalUntil::M130);
        const auto& nested_render_pass = *it;
        if (!nested_render_pass->filters.IsEmpty() || !nested_render_pass->backdrop_filters.IsEmpty()) {
            return nullptr;
        }
    }
...
SkiaRenderer::BypassMode SkiaRenderer::CalculateBypassParams(const DrawQuad* bypass_quad, DrawRPDQParams* rpdq_params, DrawQuadParams* params) const
{
    // Depending on bypass_quad's blend mode, its content may be irrelevant
    if (RenderPassRemainsTransparent(bypass_quad->shared_quad_state->blend_mode)) {
    ...
    // blend mode instead.
    DCHECK(RenderPassPreservesContent(bypass_quad->shared_quad_state->blend_mode));
    ...
    SkMatrix bypass_to_rpdq = gfx::TransformToFlattenedSkMatrix(bypass_quad->shared_quad_state->quad_to_target_transform);
    ...
    if (params->draw_region) {
        ...
        rpdq_to_bypass.mapPoints(params->draw_region->points.data(), std::size(params->draw_region->points));
    }
    ...
    if (rpdq_params->bypass_geometry) {
        ...
        gfx::Transform bypass_transform = gfx::SkMatrixToTransform(bypass_geometry.transform);
        ...
        gfx::RectF bypass_visible_rect = bypass_transform.MapRect(params->visible_rect);
        ...
        if (bypass_quad->shared_quad_state->clip_rect) {
            ...
            // coordinate space.
            bypassed_quad_clip_rect = bypass_transform.MapRect(gfx::RectF(*bypass_quad->shared_quad_state->clip_rect));
        }
        ...
        rpdq_params->bypass_geometry = DrawRPDQParams::BypassGeometry { bypass_to_rpdq, params->visible_rect };
        ...
        if (bypass_quad->shared_quad_state->clip_rect) {
            ...
            bypassed_quad_clip_rect = gfx::RectF(*bypass_quad->shared_quad_state->clip_rect);
        }
    }
...
SkCanvas::ImageSetEntry SkiaRenderer::MakeEntry(const SkImage* image, int matrix_index, const DrawQuadParams& params) const
{
    return SkCanvas::ImageSetEntry({ sk_ref_sp(image), gfx::RectFToSkRect(params.vis_tex_coords), gfx::RectFToSkRect(params.visible_rect), matrix_index,
        params.opacity, params.aa_flags, params.draw_region.has_value() });
}

SkCanvas::SrcRectConstraint SkiaRenderer::ResolveTextureConstraints(const SkImage* image, const gfx::RectF& valid_texel_bounds, DrawQuadParams* params) const
{
    if (params->aa_flags == SkCanvas::kNone_QuadAAFlags && params->sampling == SkSamplingOptions()) {
    ...
    SkCanvas::SrcRectConstraint constraint = GetTextureConstraint(image, params->vis_tex_coords, valid_texel_bounds);
    ...
    // constraint
    if (constraint == SkCanvas::kFast_SrcRectConstraint || valid_texel_bounds == params->vis_tex_coords) {
        return constraint;
    }
    ...
    SkMatrix src_to_dst = SkMatrix::RectToRect(gfx::RectFToSkRect(params->vis_tex_coords), gfx::RectFToSkRect(params->visible_rect));
    params->visible_rect = gfx::SkRectToRectF(src_to_dst.mapRect(gfx::RectFToSkRect(valid_texel_bounds)));
    params->vis_tex_coords = valid_texel_bounds;

    return SkCanvas::kStrict_SrcRectConstraint;
}

bool SkiaRenderer::MustFlushBatchedQuads(const DrawQuad* new_quad, const DrawRPDQParams* rpdq_params, const DrawQuadParams& params) const
{
    if (batched_quads_.empty())
        return false;
    ...
    if (new_quad->material != DrawQuad::Material::kAggregatedRenderPass && new_quad->material != DrawQuad::Material::kTextureContent
        && new_quad->material != DrawQuad::Material::kTiledContent)
        return true;

    if (batched_quad_state_.blend_mode != params.blend_mode || batched_quad_state_.sampling != params.sampling)
        return true;
...
void SkiaRenderer::AddQuadToBatch(const SkImage* image, const gfx::RectF& valid_texel_bounds, DrawQuadParams* params)
{
    SkCanvas::SrcRectConstraint constraint = ResolveTextureConstraints(image, valid_texel_bounds, params);
    ...
    SkMatrix m = gfx::TransformToFlattenedSkMatrix(params->content_device_transform);
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::FlushBatchedQuads");

    SkAutoCanvasRestore acr(current_canvas_, true /* do_save */);
    PrepareCanvas(batched_quad_state_.scissor_rect, batched_quad_state_.mask_filter_info, nullptr);
    ...
    current_canvas_->experimental_DrawEdgeAAImageSet(&batched_quads_.front(), batched_quads_.size(), batched_draw_regions_.data(),
        &batched_cdt_matrices_.front(), batched_quad_state_.sampling, &paint, batched_quad_state_.constraint);
...
void SkiaRenderer::DrawColoredQuad(SkColor4f color, const DrawRPDQParams* rpdq_params, DrawQuadParams* params)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::DrawColoredQuad");
    DCHECK(batched_quads_.empty());

    SkAutoCanvasRestore acr(current_canvas_, true /* do_save */);
    PrepareCanvas(params->scissor_rect, params->mask_filter_info, &params->content_device_transform);
    ...
    if (content_color_filter) {
        ...
        color = content_color_filter->filterColor4f(color, SkColorSpace::MakeSRGB().get(), color_space);
    ...
    const SkPoint* draw_region = params->draw_region ? params->draw_region->points.data() : nullptr;

    current_canvas_->experimental_DrawEdgeAAQuad(
        gfx::RectFToSkRect(params->visible_rect), draw_region, static_cast<SkCanvas::QuadAAFlags>(params->aa_flags), color, params->blend_mode);
}

void SkiaRenderer::DrawSingleImage(
    const SkImage* image, const gfx::RectF& valid_texel_bounds, const DrawRPDQParams* rpdq_params, SkPaint* paint, DrawQuadParams* params)
{
    DCHECK(batched_quads_.empty());
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::DrawSingleImage");
    ...
    const SkPoint* draw_region = params->draw_region ? params->draw_region->points.data() : nullptr;
    current_canvas_->experimental_DrawEdgeAAImageSet(&entry, 1, draw_region, bypass_transform, params->sampling, paint, constraint);
}

void SkiaRenderer::DrawPaintOpBuffer(
    const cc::PaintOpBuffer* buffer, const std::optional<SkColor4f>& clear_color, const TileDrawQuad* quad, const DrawQuadParams* params)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::DrawPaintOpBuffer");
    if (!batched_quads_.empty())
        FlushBatchedQuads();
    ...
    float offset_x = params->visible_rect.x() - params->vis_tex_coords.x() * scale_x;
    float offset_y = params->visible_rect.y() - params->vis_tex_coords.y() * scale_y;
...
void SkiaRenderer::DrawDebugBorderQuad(const DebugBorderDrawQuad* quad, DrawQuadParams* params)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::DrawDebugBorderQuad");
    DCHECK(batched_quads_.empty());

    SkAutoCanvasRestore acr(current_canvas_, true /* do_save */);
    // We need to apply the matrix manually to have pixel-sized stroke width.
    PrepareCanvas(params->scissor_rect, params->mask_filter_info, nullptr);
    SkMatrix cdt = gfx::TransformToFlattenedSkMatrix(params->content_device_transform);

    SkPath path = params->draw_region ? params->draw_region_in_path() : SkPath::Rect(gfx::RectFToSkRect(params->visible_rect));
    path.transform(cdt);
...
void SkiaRenderer::DrawPictureQuad(const PictureDrawQuad* quad, DrawQuadParams* params)
{
    DCHECK(batched_quads_.empty());
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::DrawPictureQuad");
    ...
    const bool needs_transparency = params->opacity < 1.f || params->blend_mode != SkBlendMode::kSrcOver;
    ...
    if (params->draw_region) {
        current_canvas_->clipPath(params->draw_region_in_path(), paint.isAntiAlias());
    } else {
    ...
    raster_canvas->concat(SkMatrix::RectToRect(gfx::RectFToSkRect(quad->tex_coord_rect), gfx::RectToSkRect(quad->rect)));
    ...
    quad->display_item_list->Raster(raster_canvas, /*image_provider=*/nullptr, &quad->raster_inducing_scroll_offsets);
}

void SkiaRenderer::DrawSolidColorQuad(const SolidColorDrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params)
{
    DrawColoredQuad(quad->color, rpdq_params, params);
}

void SkiaRenderer::DrawTextureQuad(const TextureDrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::DrawTextureQuad");

    // We need only RGB portion of the color space, YUV conversion handled in
    // skia.
    const gfx::ColorSpace src_color_space = resource_provider()->GetColorSpace(quad->resource_id()).GetAsFullRangeRGB();
    const gfx::HDRMetadata& src_hdr_metadata = resource_provider()->GetHDRMetadata(quad->resource_id());
    const bool needs_color_conversion_filter = ((quad->is_video_frame && src_color_space.IsHDR()) || src_color_space.IsToneMappedByDefault()) &&
    ...
    if (!image)
        return;
    gfx::RectF uv_rect = gfx::ScaleRect(gfx::BoundingRect(quad->uv_top_left, quad->uv_bottom_right), image->width(), image->height());
    params->vis_tex_coords = cc::MathUtil::ScaleRectProportional(uv_rect, gfx::RectF(quad->rect), params->visible_rect);

    // Use provided resource size if not empty, otherwise use the full image size
    // as the content area
    gfx::RectF valid_texel_bounds
        = quad->resource_size_in_pixels().IsEmpty() ? gfx::RectF(image->width(), image->height()) : gfx::RectF(gfx::SizeF(quad->resource_size_in_pixels()));
    ...
    const bool blend_background = quad->background_color != SkColors::kTransparent && !image->isOpaque();
    ...
    if (needs_color_conversion_filter) {
        ...
        DCHECK(SkColorSpace::Equals(image->colorSpace(), dst_color_space.ToSkColorSpace().get()));
        sk_sp<SkColorFilter> color_filter
            = GetColorSpaceConversionFilter(src_color_space, std::nullopt, src_hdr_metadata, dst_color_space, quad->is_video_frame);
    ...
    if (blend_background) {
        ...
        sk_sp<SkColorFilter> cf = SkColorFilters::Blend(quad->background_color.toSkColor(), SkBlendMode::kDstOver);
        if (quad_alpha < 1.f) {
...
void SkiaRenderer::DrawTileDrawQuad(const TileDrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::DrawTileDrawQuad");
    DCHECK(!MustFlushBatchedQuads(quad, rpdq_params, *params));
    ...
    // use raw draw.
    bool raw_draw_if_possible = is_using_raw_draw_ && !quad->ShouldDrawWithBlending();
    ScopedSkImageBuilder builder(this, quad->resource_id(), /*maybe_concurrent_reads=*/false,
    ...
    params->vis_tex_coords = cc::MathUtil::ScaleRectProportional(quad->tex_coord_rect, gfx::RectF(quad->rect), params->visible_rect);

    bool using_raw_draw = builder.paint_op_buffer();
    if (is_using_raw_draw_) {
        UMA_HISTOGRAM_BOOLEAN("Compositing.SkiaRenderer.DrawTileDrawQuad.UsingRawDraw", using_raw_draw);
    }
    if (using_raw_draw) {
        DCHECK(!rpdq_params);
        DrawPaintOpBuffer(builder.paint_op_buffer(), builder.clear_color(), quad, params);
        return;
    }
...
void SkiaRenderer::DrawUnsupportedQuad(const DrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params)
{
#ifdef NDEBUG
...
void SkiaRenderer::ScheduleOverlays()
{
    ...
    for (auto& overlay : current_frame()->overlay_list) {
        ...
        if (overlay.needs_detiling) {
            ...
            skia_output_surface_->DetileOverlay(overlay.mailbox, overlay.resource_size_in_pixels, lock.sync_token(), detiled_image, overlay.display_rect,
                overlay.uv_rect, absl::get<gfx::OverlayTransform>(overlay.transform), is_10bit);
            overlay.uv_rect = gfx::RectF(static_cast<float>(overlay.display_rect.width()) / static_cast<float>(kMaxProtectedContentWidth),
                static_cast<float>(overlay.display_rect.height() / static_cast<float>(kMaxProtectedContentHeight)));
            overlay.mailbox = detiled_image;
            overlay.format = (is_10bit && base::FeatureList::IsEnabled(media::kEnableArmHwdrm10bitOverlays)) ? SinglePlaneFormat::kBGRA_1010102
        ...
        if (overlay.rpdq) {
            ...
            if (auto backing = GetRenderPassBackingForDirectScanout(overlay.rpdq->render_pass_id); backing) {
                DBG_LOG("delegated.overlay.log", "Pass %" PRIu64 ": RPDQ overlay can scanout directly", overlay.rpdq->render_pass_id.value());

                // SkiaRenderer might've allocated a larger backing than our render
                // pass' requested size.
                overlay.uv_rect = gfx::MapRect(overlay.rpdq->tex_coord_rect, gfx::RectF(backing->size), gfx::RectF(1, 1));
                ...
                if (overlay.rpdq->visible_rect != overlay.rpdq->rect) {
                    ...
                    overlay.uv_rect = gfx::MapRect(gfx::RectF(overlay.rpdq->visible_rect), gfx::RectF(overlay.rpdq->rect), overlay.uv_rect);
                }
        ...
        if (overlay.is_solid_color) {
            ...
            DCHECK(output_surface_->capabilities().supports_non_backed_solid_color_overlays || output_surface_->capabilities().supports_single_pixel_buffer);
            continue;
        }
    ...
    skia_output_surface_->ScheduleOverlays(std::move(current_frame()->overlay_list), std::move(sync_tokens));
}

sk_sp<SkColorFilter> SkiaRenderer::GetColorSpaceConversionFilter(const gfx::ColorSpace& src, std::optional<uint32_t> src_bit_depth,
    std::optional<gfx::HDRMetadata> src_hdr_metadata, const gfx::ColorSpace& dst, bool is_video_frame)
{
    ...
    if (is_video_frame && src.GetTransferID() == gfx::ColorSpace::TransferID::PQ && base::FeatureList::IsEnabled(features::kUseDisplaySDRMaxLuminanceNits)) {
        hdr_metadata = gfx::HDRMetadata::PopulateUnspecifiedWithDefaults(src_hdr_metadata);
        hdr_metadata->ndwl = gfx::HdrMetadataNdwl(current_frame()->display_color_spaces.GetSDRMaxLuminanceNits());
    }
    return color_filter_cache_.Get(src, dst, src_bit_depth, hdr_metadata, current_frame()->display_color_spaces.GetSDRMaxLuminanceNits(),
...
sk_sp<SkColorFilter> SkiaRenderer::GetContentColorFilter()
{
    ...
    bool is_root = current_frame()->current_render_pass == current_frame()->root_render_pass;

    if (is_root && output_surface_->color_matrix() != SkM44()) {
        color_transform = SkColorFilters::Matrix(ToColorMatrix(output_surface_->color_matrix()));
    }
    ...
    if (is_root && debug_settings_->tint_composited_content) {
        if (debug_settings_->tint_composited_content_modulate) {
            ...
            SkM44 mat44 = SkM44::ColMajor(cc::DebugColors::TintCompositedContentColorTransformMatrix().data());
            tint_transform = SkColorFilters::Matrix(ToColorMatrix(mat44));
        }
    }

    if (color_transform) {
        return tint_transform ? color_transform->makeComposed(tint_transform) : color_transform;
    } else {
...
SkiaRenderer::DrawRPDQParams SkiaRenderer::CalculateRPDQParams(
    const gfx::AxisTransform2d& target_to_device, const AggregatedRenderPassDrawQuad* quad, const DrawQuadParams* params)
{
    DrawRPDQParams rpdq_params(params->visible_rect);

    if (!quad->mask_resource_id().is_null()) {
        // Scale normalized uv rect into absolute texel coordinates.
        SkRect mask_rect = gfx::RectFToSkRect(gfx::ScaleRect(quad->mask_uv_rect, quad->mask_texture_size.width(), quad->mask_texture_size.height()));
        SkMatrix mask_to_quad_matrix = SkMatrix::RectToRect(mask_rect, gfx::RectToSkRect(quad->rect));
        rpdq_params.mask_shader.emplace(quad->mask_resource_id(), mask_to_quad_matrix);
    }

    const cc::FilterOperations* filters = FiltersForPass(quad->render_pass_id);
    const cc::FilterOperations* backdrop_filters = BackdropFiltersForPass(quad->render_pass_id);
    ...
    auto to_sk_image_filter = [](sk_sp<cc::PaintFilter> paint_filter, const SkMatrix& local_matrix) -> sk_sp<SkImageFilter> {
    ...
    if (filters) {
        ...
        rpdq_params.image_filter = to_sk_image_filter(std::move(paint_filter), local_matrix);
        ...
        if (rpdq_params.image_filter) {
            ...
            rpdq_params.filter_bounds = rpdq_params.image_filter->computeFastBounds(rpdq_params.filter_bounds);
    ...
    if (backdrop_filters) {
        ...
        if (local_matrix.invert(&inv_local_matrix)) {
            SkIRect filter_rect = inv_local_matrix.mapRect(gfx::RectToSkRect(quad->rect)).roundOut();
            auto bg_paint_filter = cc::RenderSurfaceFilters::BuildImageFilter(*backdrop_filters, gfx::SkIRectToRect(filter_rect));

            rpdq_params.backdrop_filter = to_sk_image_filter(std::move(bg_paint_filter), local_matrix);
        }
    }
    ...
    if (rpdq_params.backdrop_filter) {
        ...
        std::optional<gfx::RRectF> pass_bounds = BackdropFilterBoundsForPass(quad->render_pass_id);
        std::optional<SkRRect> backdrop_filter_bounds;
        if (pass_bounds) {
            // Scale by the filter's scale, but don't apply filter origin
            SkRRect result;
            if (!SkRRect(*pass_bounds).transform(local_matrix, &result) || !backdrop_rect.intersect(result.rect())) {
            ...
            if (backdrop_filter_bounds->contains(rpdq_params.filter_bounds)) {
                ...
                if (params->mask_filter_info) {
                    SkMatrix m = gfx::TransformToFlattenedSkMatrix(params->content_device_transform);
                    if (backdrop_filter_bounds->transform(m, &result) && SkRRect(params->mask_filter_info->rounded_corner_bounds()) == result) {
                        backdrop_filter_bounds.reset();
                    }
                }
            }
        ...
        SkIRect sk_src_rect = rpdq_params.backdrop_filter->filterBounds(sk_crop_rect, SkMatrix::I(), SkImageFilter::kReverse_MapDirection,
            /*inputRect=*/nullptr);
        if (!sk_crop_rect.contains(sk_src_rect)) {
            SkTileMode sk_tile_mode = base::FeatureList::IsEnabled(features::kBackdropFilterMirrorEdgeMode) ? SkTileMode::kMirror : SkTileMode::kClamp;
...
void SkiaRenderer::DrawRenderPassQuad(const AggregatedRenderPassDrawQuad* quad, const DrawRPDQParams* bypassed_rpdq_params, DrawQuadParams* params)
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::DrawRenderPassQuad");
    DrawRPDQParams rpdq_params = bypassed_rpdq_params ? *bypassed_rpdq_params : CalculateRPDQParams(current_frame()->target_to_device_transform, quad, params);
    ...
    if (bypass != render_pass_bypass_quads_.end()) {
        BypassMode mode = CalculateBypassParams(bypass->second, &rpdq_params, params);
    ...
    if (iter == render_pass_backings_.end()) {
        if (base::FeatureList::IsEnabled(kDumpWithoutCrashingOnMissingRenderPassBacking)) {
            // This can happen if we previously skipped drawing a render pass (and
            // allocating its backing) due to an empty update rect.
            LOG(ERROR) << "Could not find render pass id # " << quad->render_pass_id << " in the render pass overlay backings";

            SCOPED_CRASH_KEY_STRING32("missing rp backing", "0-seen before?", base::NumberToString(seen_render_pass_ids_.contains(quad->render_pass_id)));

            // This is derived from |DirectRenderer::ShouldSkipQuad|.
            gfx::Rect target_rect = quad->visible_rect;
            SCOPED_CRASH_KEY_STRING32("missing rp backing", "1-visible rect", target_rect.ToString());
            ...
            SCOPED_CRASH_KEY_STRING32(
                "missing rp backing", "2-filter expansion", filter_it != render_pass_filters_.end() ? target_rect.ToString() : "no filter expansion");

            const gfx::QuadF target_quad = quad->shared_quad_state->quad_to_target_transform.MapQuad(gfx::QuadF(gfx::RectF(target_rect)));
            SCOPED_CRASH_KEY_STRING256(
                "missing rp backing", "3-rpdq in draw", target_quad.IsRectilinear() ? target_quad.BoundingBox().ToString() : target_quad.ToString());

            gfx::Rect draw_rect_in_draw_space = OutputSurfaceRectInDrawSpace();
            SCOPED_CRASH_KEY_STRING32("missing rp backing", "4-output surface", draw_rect_in_draw_space.ToString());
            if (scissor_rect_) {
                draw_rect_in_draw_space = scissor_rect_.value();
                draw_rect_in_draw_space.Offset(current_frame()->current_render_pass->output_rect.OffsetFromOrigin());
            }
            SCOPED_CRASH_KEY_STRING32("missing rp backing", "5-with scissor?", scissor_rect_ ? draw_rect_in_draw_space.ToString() : "no scissor");

            if (quad->shared_quad_state->clip_rect) {
                draw_rect_in_draw_space.Intersect(*quad->shared_quad_state->clip_rect);
            }
            SCOPED_CRASH_KEY_STRING32(
                "missing rp backing", "6-with quad clip?", quad->shared_quad_state->clip_rect ? draw_rect_in_draw_space.ToString() : "no quad clip");

            const bool intersects = target_quad.IntersectsRect(gfx::RectF(draw_rect_in_draw_space));
            SCOPED_CRASH_KEY_STRING32("missing rp backing", "7-intersects?", base::NumberToString(intersects));

            SCOPED_CRASH_KEY_STRING32("missing rp backing", "8-quad-pass-id", base::NumberToString(quad->render_pass_id.value()));
            ...
            SCOPED_CRASH_KEY_STRING256("missing rp backing", "9-frame-pass-ids", base::JoinString(pass_ids, ","));

            pass_ids.clear();
            for (const auto& [id, pass] : render_pass_backings_) {
                pass_ids.push_back(base::NumberToString(id.value()));
            }
            SCOPED_CRASH_KEY_STRING256("missing rp backing", "10-backing-pass-ids", base::JoinString(pass_ids, ","));

            auto it = base::ranges::find(*current_frame()->render_passes_in_draw_order, quad->render_pass_id, &AggregatedRenderPass::id);
            SCOPED_CRASH_KEY_STRING256("missing rp backing", "11-rp transform",
                it != current_frame()->render_passes_in_draw_order->end() ? it->get()->transform_to_root_target.ToString() : "missing pass in frame");

            SCOPED_CRASH_KEY_STRING256("missing rp backing", "12-rpdq transform", quad->shared_quad_state->quad_to_target_transform.ToString());
    ...
    sk_sp<SkImage> content_image = skia_output_surface_->MakePromiseSkImageFromRenderPass(
        quad->render_pass_id, backing.size, backing.format, backing.generate_mipmap, RenderPassBackingSkColorSpace(backing), backing.mailbox);
    DLOG_IF(ERROR, !content_image) << "MakePromiseSkImageFromRenderPass() failed for render pass";

    if (!content_image)
        return;

    if (backing.generate_mipmap)
        params->sampling = SkSamplingOptions(SkFilterMode::kLinear, SkMipmapMode::kLinear);

    params->vis_tex_coords = cc::MathUtil::ScaleRectProportional(quad->tex_coord_rect, gfx::RectF(quad->rect), params->visible_rect);
    gfx::RectF valid_texel_bounds(content_image->width(), content_image->height());
    ...
    if (!rpdq_params.image_filter && !rpdq_params.backdrop_filter && !rpdq_params.mask_shader && !rpdq_params.bypass_geometry) {
    ...
    DrawSingleImage(content_image.get(), valid_texel_bounds, &rpdq_params, &paint, params);
}

void SkiaRenderer::CopyDrawnRenderPass(const copy_output::RenderPassGeometry& geometry, std::unique_ptr<CopyOutputRequest> request)
{
    // TODO(weiliangc): Make copy request work. (crbug.com/644851)
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::CopyDrawnRenderPass");
    ...
    skia_output_surface_->CopyOutput(geometry, RenderPassColorSpace(render_pass), std::move(request), mailbox);
}
...
{
    TRACE_EVENT0(TRACE_DISABLED_BY_DEFAULT("viz.quads"), "SkiaRenderer::FinishDrawingRenderPass");
    if (!current_canvas_)
        return;
    ...
    bool is_root_render_pass = current_frame()->current_render_pass == current_frame()->root_render_pass;
    ...
    if (UsingSkiaForDelegatedInk()) {
        if (const auto pass_id = delegated_ink_handler_->GetInkRenderer()->GetLatestMetadataRenderPassId();
            current_frame()->current_render_pass->id == pass_id) {
            gfx::Transform root_target_to_render_pass_draw_transform;
            if (current_frame()->current_render_pass->transform_to_root_target.GetInverse(&root_target_to_render_pass_draw_transform)) {
    ...
    bool is_overlay = skia_output_surface_->capabilities().renderer_allocates_images && is_root_render_pass;
...
void SkiaRenderer::UpdateRenderPassTextures(
    const AggregatedRenderPassList& render_passes_in_draw_order, const base::flat_map<AggregatedRenderPassId, RenderPassRequirements>& render_passes_in_frame)
{
    ...
    for (const auto& [backing_id, backing] : render_pass_backings_) {
        ...
        if (render_pass_it == render_passes_in_frame.end()) {
            passes_to_delete.push_back(backing_id);
            DBG_LOG("renderer.skia.render_pass_backings", "render_pass %" PRIu64 " is no longer in frame", backing_id.value());
            continue;
        }
        ...
        const bool size_is_sufficient = backing.size.width() >= requirements.size.width() && backing.size.height() >= requirements.size.height();
        bool size_appropriate = backing.is_root ? size_is_exact_match : size_is_sufficient;
        bool mipmap_appropriate = !requirements.generate_mipmap || backing.generate_mipmap;
        bool no_change_in_format = requirements.format == backing.format;
        bool no_change_in_alpha_type = requirements.alpha_type == backing.alpha_type;
        bool no_change_in_color_space = requirements.color_space == backing.color_space;
        bool scanout_appropriate = requirements.is_scanout == backing.is_scanout && requirements.scanout_dcomp_surface == backing.scanout_dcomp_surface;

        if (!size_appropriate || !mipmap_appropriate || !no_change_in_format || !no_change_in_alpha_type || !no_change_in_color_space || !scanout_appropriate) {
            passes_to_delete.push_back(backing_id);
            DBG_LOG("renderer.skia.render_pass_backings", "render_pass %" PRIu64 " allocation part not appropriate:%s%s%s%s%s%s", backing_id.value(),
                !size_appropriate ? " size" : "", !mipmap_appropriate ? " mipmap" : "", !no_change_in_format ? " format" : "",
                !no_change_in_alpha_type ? " alpha_type" : "", !no_change_in_color_space ? " color_space" : "", !scanout_appropriate ? " scanout" : "");
        }
    }
...
void SkiaRenderer::AllocateRenderPassResourceIfNeeded(const AggregatedRenderPassId& render_pass_id, const RenderPassRequirements& requirements)
{
    ...
    gpu::SharedImageUsageSet usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_DISPLAY_WRITE;
    ...
    if (requirements.is_scanout) {
        ...
        const bool dcomp_surface_unsupported_format = requirements.format == SinglePlaneFormat::kRGBA_1010102;

        if (requirements.scanout_dcomp_surface && !dcomp_surface_unsupported_format) {
    ...
    auto mailbox = skia_output_surface_->CreateSharedImage(
        requirements.format, requirements.size, requirements.color_space, requirements.alpha_type, usage, "RenderPassBacking", gpu::kNullSurfaceHandle);

    VizDebuggerLog::DebugLogNewRenderPassBacking(render_pass_id, requirements);

    render_pass_backings_.emplace(render_pass_id,
        RenderPassBacking(requirements.size, requirements.generate_mipmap, requirements.color_space, requirements.alpha_type, requirements.format, mailbox,
            is_root, requirements.is_scanout, requirements.scanout_dcomp_surface));
    if (base::FeatureList::IsEnabled(kDumpWithoutCrashingOnMissingRenderPassBacking)) {
...
bool SkiaRenderer::CanSkipRenderPassOverlay(
    AggregatedRenderPassId render_pass_id, const AggregatedRenderPassDrawQuad* rpdq, RenderPassOverlayParams** output_render_pass_overlay)
{
    ...
    for (auto rit = in_flight_render_pass_overlay_backings_.rbegin(); rit != in_flight_render_pass_overlay_backings_.rend(); ++rit) {
    ...
    if (!overlay_found) {
        int index = 0;
        for (auto rit = available_render_pass_overlay_backings_.rbegin(); rit != available_render_pass_overlay_backings_.rend(); ++rit, ++index) {
            if (rit->render_pass_id == render_pass_id) {
                found_in_available_backings = true;
                // Cannot use reverse_iterator. Convert it to const_iterator.
                it_to_delete = available_render_pass_overlay_backings_.begin() + (available_render_pass_overlay_backings_.size() - index - 1);
                overlay_found = &*rit;
                break;
            }
        }
    ...
    const cc::FilterOperations* backdrop_filters = BackdropFiltersForPass(render_pass_id);
    ...
    bool no_change_in_filters = filters ? (overlay_found->filters == *filters) : (overlay_found->filters == cc::FilterOperations());
    bool no_change_in_backdrop_filters
        = backdrop_filters ? (overlay_found->backdrop_filters == *backdrop_filters) : (overlay_found->backdrop_filters == cc::FilterOperations());

    if (no_change_in_rpdq && no_change_in_filters && no_change_in_backdrop_filters) {
        if (found_in_available_backings) {
            ...
            *output_render_pass_overlay = &in_flight_render_pass_overlay_backings_.back();
        } else {
...
std::optional<SkiaRenderer::RenderPassBacking> SkiaRenderer::GetRenderPassBackingForDirectScanout(const AggregatedRenderPassId& render_pass_id) const
{
#if BUILDFLAG(IS_WIN)
    if (auto backing_it = render_pass_backings_.find(render_pass_id); backing_it != render_pass_backings_.end()) {
        if (backing_it->second.is_scanout) {
            if (DCHECK_IS_ON()) {
                auto pass_it = base::ranges::find(*current_frame()->render_passes_in_draw_order, backing_it->first, &AggregatedRenderPass::id);
                ...
                DCHECK(!(pass_it->get()->will_backing_be_read_by_viz && backing_it->second.scanout_dcomp_surface));
            }
...
SkiaRenderer::RenderPassOverlayParams* SkiaRenderer::GetOrCreateRenderPassOverlayBacking(AggregatedRenderPassId render_pass_id,
    const AggregatedRenderPassDrawQuad* rpdq, SharedImageFormat buffer_format, gfx::ColorSpace color_space, const gfx::Size& buffer_size)
{
    RenderPassOverlayParams overlay_params;
    auto it
        = base::ranges::find_if(available_render_pass_overlay_backings_, [&buffer_format, &buffer_size, &color_space](const RenderPassOverlayParams& overlay) {
              auto& backing = overlay.render_pass_backing;
              return backing.format == buffer_format && backing.size == buffer_size && backing.color_space == color_space;
          });
    if (it == available_render_pass_overlay_backings_.end()) {
        // Allocate the image for render pass overlay if there is no existing
        // available one.
        auto kOverlayUsage = gpu::SHARED_IMAGE_USAGE_SCANOUT | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_DISPLAY_WRITE;

        auto mailbox = skia_output_surface_->CreateSharedImage(
            buffer_format, buffer_size, color_space, RenderPassAlphaType::kPremul, kOverlayUsage, "RenderPassOverlay", gpu::kNullSurfaceHandle);
    ...
    if (const cc::FilterOperations* filters = FiltersForPass(render_pass_id); filters) {
        overlay_params.filters = *filters;
    }
    if (const cc::FilterOperations* backdrop_filters = BackdropFiltersForPass(render_pass_id); backdrop_filters) {
...
void SkiaRenderer::PrepareRenderPassOverlay(OverlayProcessorInterface::PlatformOverlayCandidate* overlay)
{
    ...
    base::AutoReset<raw_ptr<const AggregatedRenderPass>> auto_reset_current_render_pass(
        &current_frame()->current_render_pass, current_frame()->root_render_pass);

    auto* shared_quad_state = const_cast<SharedQuadState*>(quad->shared_quad_state);
    ...
    if (shared_quad_state->quad_to_target_transform.IsInvertible()) {
        ...
        gfx::Transform flat_quad_to_target_transform(shared_quad_state->quad_to_target_transform);
        flat_quad_to_target_transform.Flatten();
        quad_to_target_transform_inverse = flat_quad_to_target_transform.GetCheckedInverse();
    }
    ...
    if (quad_to_target_transform_inverse) {
        ...
        gfx::RectF clip_rect(quad->shared_quad_state->clip_rect.value_or(current_frame()->current_render_pass->output_rect));
        ...
        auto_reset_clip_rect.emplace(&shared_quad_state->clip_rect, gfx::ToEnclosedRect(clip_rect));
    } else {
    ...
    if (!shared_quad_state->mask_filter_info.IsEmpty()) {
        if (quad_to_target_transform_inverse) {
            ...
            // the original value.
            auto_reset_mask_info.emplace(&shared_quad_state->mask_filter_info, shared_quad_state->mask_filter_info);
            ...
            shared_quad_state->mask_filter_info.ApplyTransform(*quad_to_target_transform_inverse);
        } else {
            // If we can't position the mask info into the render pass space, we
            // shouldn't use it when rendering.
            auto_reset_mask_info.emplace(&shared_quad_state->mask_filter_info, gfx::MaskFilterInfo());
        }
    }
    ...
    target_to_device.PostConcat(gfx::WindowTransform(/*x=*/0, /*y=*/0, /*width=*/viewport_size.width(),
    ...
    DrawRPDQParams rpdq_params { gfx::RectF() };
    {
        ...
        base::AutoReset<gfx::Transform> auto_reset_transform(&shared_quad_state->quad_to_target_transform, gfx::Transform());
    ...
    gfx::Rect filter_bounds = gfx::SkIRectToRect(rpdq_params.filter_bounds.roundOut());
    ...
    gfx::Size buffer_size(
        cc::MathUtil::CheckedRoundUp(filter_bounds.width(), kBufferMultiple), cc::MathUtil::CheckedRoundUp(filter_bounds.height(), kBufferMultiple));

    RenderPassOverlayParams* overlay_params = nullptr;
    bool can_skip_render_pass = CanSkipRenderPassOverlay(quad->render_pass_id, quad, &overlay_params);
    if (!can_skip_render_pass) {
        overlay_params = GetOrCreateRenderPassOverlayBacking(quad->render_pass_id, quad, si_format, color_space, buffer_size);
    }
    DCHECK(overlay_params);
    UMA_HISTOGRAM_BOOLEAN("Compositing.SkiaRenderer.SkipOverlayRenderPassDrawQuad", can_skip_render_pass);

    const RenderPassBacking& dst_overlay_backing = overlay_params->render_pass_backing;
    ...
    if (can_skip_render_pass) {
        ...
        UMA_HISTOGRAM_COUNTS_10M("Compositing.SkiaRenderer.SkippedOverlayRenderPassDrawQuadSize", pixel_size);
    } else {
        current_canvas_ = skia_output_surface_->BeginPaintRenderPass(quad->render_pass_id, dst_overlay_backing.size, dst_overlay_backing.format,
            dst_overlay_backing.alpha_type, skgpu::Mipmapped::kNo, dst_overlay_backing.scanout_dcomp_surface,
            RenderPassBackingSkColorSpace(dst_overlay_backing),
            /*is_overlay=*/true, overlay->mailbox);
        if (!current_canvas_) {
            DLOG(ERROR) << "BeginPaintRenderPass() in PrepareRenderPassOverlay() failed.";
            return;
        }
        ...
        params.content_device_transform.Translate(-filter_bounds.x(), -filter_bounds.y());
        ...
        if (bypass != render_pass_bypass_quads_.end()) {
            ...
            auto content_image = skia_output_surface_->MakePromiseSkImageFromRenderPass(quad->render_pass_id, src_quad_backing->size, src_quad_backing->format,
                src_quad_backing->generate_mipmap, RenderPassBackingSkColorSpace(*src_quad_backing), src_quad_backing->mailbox);
            if (!content_image) {
    ...
    if (absl::holds_alternative<gfx::OverlayTransform>(overlay->transform)) {
        ...
        // display_rect.
        overlay->display_rect = quad->shared_quad_state->quad_to_target_transform.MapRect(gfx::RectF(filter_bounds));
...
void SkiaRenderer::EndPaint(const gfx::Rect& update_rect, bool failed, bool is_overlay)
{
    ...
    if (!failed) {
        ...
        if (current_gpu_commands_completed_fence_->was_set()) {
            on_finished_callback = base::BindPostTask(base::SingleThreadTaskRunner::GetCurrentDefault(),
                base::BindOnce(&FrameResourceGpuCommandsCompletedFence::Signal, std::move(current_gpu_commands_completed_fence_)));
            current_gpu_commands_completed_fence_ = base::MakeRefCounted<FrameResourceGpuCommandsCompletedFence>(resource_provider());
            resource_provider()->SetGpuCommandsCompletedFence(current_gpu_commands_completed_fence_.get());
        }
        ...
        if (current_release_fence_->was_set()) {
            on_return_release_fence_cb = base::BindPostTask(base::SingleThreadTaskRunner::GetCurrentDefault(),
                base::BindOnce(&FrameResourceReleaseFence::SetReleaseFenceCallback, std::move(current_release_fence_)));
            current_release_fence_ = base::MakeRefCounted<FrameResourceReleaseFence>(resource_provider());
            resource_provider()->SetReleaseFence(current_release_fence_.get());
        }
    }
    skia_output_surface_->EndPaint(std::move(on_finished_callback), std::move(on_return_release_fence_cb), update_rect, is_overlay);
}

bool SkiaRenderer::IsRenderPassResourceAllocated(const AggregatedRenderPassId& render_pass_id) const
{
    auto it = render_pass_backings_.find(render_pass_id);
    return it != render_pass_backings_.end();
}

gfx::Size SkiaRenderer::GetRenderPassBackingPixelSize(const AggregatedRenderPassId& render_pass_id)
{
    auto it = render_pass_backings_.find(render_pass_id);
...
gfx::Rect SkiaRenderer::GetRenderPassBackingDrawnRect(const AggregatedRenderPassId& render_pass_id) const
{
    if (auto it = render_pass_backings_.find(render_pass_id); it != render_pass_backings_.end()) {
...
void SkiaRenderer::SetRenderPassBackingDrawnRect(const AggregatedRenderPassId& render_pass_id, const gfx::Rect& drawn_rect)
{
    auto it = render_pass_backings_.find(render_pass_id);
...
void SkiaRenderer::SetDelegatedInkPointRendererSkiaForTest(std::unique_ptr<DelegatedInkPointRendererSkia> renderer)
{
    DCHECK(!delegated_ink_handler_);
    delegated_ink_handler_ = std::make_unique<DelegatedInkHandler>(output_surface_->capabilities().supports_delegated_ink);
    delegated_ink_handler_->SetDelegatedInkPointRendererForTest(std::move(renderer));
}

void SkiaRenderer::DrawDelegatedInkTrail(const gfx::Transform& root_target_to_render_pass_transform)
{
    if (!delegated_ink_handler_ || !delegated_ink_handler_->GetInkRenderer())
        return;

    delegated_ink_handler_->GetInkRenderer()->DrawDelegatedInkTrail(current_canvas_, root_target_to_render_pass_transform);
}

DelegatedInkPointRendererBase* SkiaRenderer::GetDelegatedInkPointRenderer(bool create_if_necessary)
{
...
void SkiaRenderer::SetDelegatedInkMetadata(std::unique_ptr<gfx::DelegatedInkMetadata> metadata)
{
    if (!delegated_ink_handler_) {
        delegated_ink_handler_ = std::make_unique<DelegatedInkHandler>(output_surface_->capabilities().supports_delegated_ink);
    }
...
#endif
        buffer_queue_->Reshape(reshape_params.size, reshape_params.color_space, alpha_type, reshape_params.format);
    }
...
void SkiaRenderer::MaybeScheduleBackgroundImage(OverlayProcessorInterface::CandidateList& overlay_list)
{
    ...
    background_candidate.display_rect = gfx::RectF(gfx::SizeF(viewport_size_for_swap_buffers()));
    background_candidate.color = toggle_background_overlay_color() ? SkColors::kRed : SkColors::kTransparent;
    ...
    background_candidate.opacity = toggle_background_overlay_color() ? 1.0f : 0.0f;

    // ScheduleOverlays() will convert this to a buffer-backed solid color overlay
    // if necessary.
    background_candidate.is_solid_color = true;
    if (overlay_processor_) {
        background_candidate.damage_rect = overlay_processor_->GetUnassignedDamage();
...
#if BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_OZONE) || BUILDFLAG(IS_WIN)
SkiaRenderer::ScopedInFlightRenderPassOverlayBackingRef::ScopedInFlightRenderPassOverlayBackingRef(SkiaRenderer* renderer, const gpu::Mailbox& mailbox)
    : renderer_(renderer)
    , mailbox_(mailbox)
{
    CHECK(renderer_);
    ...
    auto it = base::ranges::find(renderer_->in_flight_render_pass_overlay_backings_, mailbox_,
        [](const RenderPassOverlayParams& overlay) { return overlay.render_pass_backing.mailbox; });
...
SkiaRenderer::ScopedInFlightRenderPassOverlayBackingRef::~ScopedInFlightRenderPassOverlayBackingRef()
{
    Reset();
}
...
SkiaRenderer::ScopedInFlightRenderPassOverlayBackingRef::ScopedInFlightRenderPassOverlayBackingRef::operator=(
    SkiaRenderer::ScopedInFlightRenderPassOverlayBackingRef&& other)
{
    Reset();
...
SkiaRenderer::OverlayLock::OverlayLock(DisplayResourceProvider* resource_provider, ResourceId resource_id)
{
...
SkiaRenderer::OverlayLock& SkiaRenderer::OverlayLock::OverlayLock::operator=(SkiaRenderer::OverlayLock&& other)
{
    resource_lock = std::move(other.resource_lock);
...
#if BUILDFLAG(IS_APPLE) || BUILDFLAG(IS_OZONE) || BUILDFLAG(IS_WIN)
SkiaRenderer::OverlayLock::OverlayLock(SkiaRenderer* renderer, const gpu::Mailbox& mailbox)
{
...
#if BUILDFLAG(IS_APPLE)
std::size_t SkiaRenderer::OverlayLockHash::operator()(const OverlayLock& o) const
{
    return std::hash<gpu::Mailbox> {}(o.mailbox());
}

std::size_t SkiaRenderer::OverlayLockHash::operator()(const gpu::Mailbox& m) const
{
    return std::hash<gpu::Mailbox> {}(m);
}

bool SkiaRenderer::OverlayLockKeyEqual::operator()(const OverlayLock& lhs, const OverlayLock& rhs) const
{
    return lhs.mailbox() == rhs.mailbox();
}

bool SkiaRenderer::OverlayLockKeyEqual::operator()(const OverlayLock& lhs, const gpu::Mailbox& rhs) const
{
