...
public:
    ...
    void CheckOverlaySupport(std::vector<ui::OverlaySurfaceCandidate>* candidates) override
    {
    ...
    void ObserveHardwareCapabilities(ui::HardwareCapabilitiesCallback receive_callback) override
    {
...
TEST(OverlayProcessorOzoneTest, PrimaryPlaneSizeAndFormatMatches)
{
    ...
    scoped_refptr<MockSharedImageInterface> sii = base::MakeRefCounted<MockSharedImageInterface>();
    scoped_refptr<gfx::NativePixmap> primary_plane_pixmap = base::MakeRefCounted<FakeNativePixmap>(size, gfx::BufferFormat::BGRA_8888);
    scoped_refptr<gfx::NativePixmap> candidate_pixmap = base::MakeRefCounted<FakeNativePixmap>(size, gfx::BufferFormat::BGRA_8888);
    EXPECT_CALL(*sii, GetNativePixmap(_)).WillOnce(Return(primary_plane_pixmap)).WillOnce(Return(candidate_pixmap));
    OverlayProcessorOzone processor(std::make_unique<FakeOverlayCandidatesOzone>(), {}, sii.get());
...
TEST(OverlayProcessorOzoneTest, PrimaryPlaneFormatMismatch)
{
    ...
    scoped_refptr<MockSharedImageInterface> sii = base::MakeRefCounted<MockSharedImageInterface>();
    scoped_refptr<gfx::NativePixmap> primary_plane_pixmap = base::MakeRefCounted<FakeNativePixmap>(size, gfx::BufferFormat::R_8);
...
TEST(OverlayProcessorOzoneTest, ColorSpaceMismatch)
{
    ...
    // Initialize a MockSharedImageInterface that returns a NativePixmap with
    // matching params to the primary plane.
    scoped_refptr<MockSharedImageInterface> sii = base::MakeRefCounted<::testing::NiceMock<MockSharedImageInterface>>();
    scoped_refptr<gfx::NativePixmap> primary_plane_pixmap = base::MakeRefCounted<FakeNativePixmap>(size, gfx::BufferFormat::BGRA_8888);
    scoped_refptr<gfx::NativePixmap> candidate_pixmap = base::MakeRefCounted<FakeNativePixmap>(size, gfx::BufferFormat::BGRA_8888);
    ON_CALL(*sii, GetNativePixmap(primary_plane.mailbox)).WillByDefault(Return(primary_plane_pixmap));
    ON_CALL(*sii, GetNativePixmap(candidate.mailbox)).WillByDefault(Return(candidate_pixmap));
    OverlayProcessorOzone processor(std::make_unique<FakeOverlayCandidatesOzone>(), {}, sii.get());
...
public:
    ...
    {
        return max_overlays_considered_;
    }
};
...
TEST(OverlayProcessorOzoneTest, ObserveHardwareCapabilites)
{
    ...
    TestOverlayProcessorOzone processor(std::move(fake_candidates_unique), {}, nullptr);
