...
DCLayerResult ValidateYUVOverlay(const gfx::ProtectedVideoType& protected_video_type, const gfx::ColorSpace& video_color_space,
    const SharedImageFormat si_format, const std::optional<gfx::HDRMetadata>& hdr_metadata, bool has_overlay_support, bool has_p010_video_processor_support,
    int allowed_yuv_overlay_count, int processed_yuv_overlay_count)
{
    ...
    if ((video_color_space.GetMatrixID() != gfx::ColorSpace::MatrixID::RGB) && !gfx::ColorSpaceWin::CanConvertToDXGIColorSpace(video_color_space)) {
    ...
    if (video_color_space.IsHDR()) {
        ...
        if ((video_color_space.GetPrimaryID() != gfx::ColorSpace::PrimaryID::BT2020)
            || (video_color_space.GetTransferID() != gfx::ColorSpace::TransferID::PQ)) {
    ...
    if (si_format == MultiPlaneFormat::kP010 && !has_p010_video_processor_support) {
...
DCLayerResult ValidateTextureQuad(const TextureDrawQuad* quad, const std::vector<gfx::Rect>& backdrop_filter_rects, bool has_overlay_support,
    bool has_p010_video_processor_support, int allowed_yuv_overlay_count, int processed_yuv_overlay_count, const DisplayResourceProvider* resource_provider)
{
    ...
    if (!quad->shared_quad_state->quad_to_target_transform.Preserves2dAxisAlignment()) {
    ...
    if (quad->is_video_frame) {
        const auto& color_space = resource_provider->GetColorSpace(quad->resource_id());
        const auto& hdr_metadata = resource_provider->GetHDRMetadata(quad->resource_id());
        auto si_format = resource_provider->GetSharedImageFormat(quad->resource_id());
        auto result = ValidateYUVOverlay(quad->protected_video_type, color_space, si_format, hdr_metadata, has_overlay_support,
            has_p010_video_processor_support, allowed_yuv_overlay_count, processed_yuv_overlay_count);
        return result;
    }
...
void FromTextureQuad(
    const TextureDrawQuad* quad, const gfx::Transform& transform_to_root_target, const DisplayResourceProvider* resource_provider, OverlayCandidate* dc_layer)
{
    ...
    dc_layer->uv_rect = gfx::BoundingRect(quad->uv_top_left, quad->uv_bottom_right);
    ...
    if (quad->y_flipped) {
        ...
        quad_to_root_transform.PostTranslate(0.0, dc_layer->resource_size_in_pixels.height());
    }
    quad_to_root_transform.PostConcat(quad->shared_quad_state->quad_to_target_transform);
    ...
    if (quad->shared_quad_state->clip_rect) {
        ...
        // space.
        dc_layer->clip_rect = transform_to_root_target.MapRect(quad->shared_quad_state->clip_rect.value_or(gfx::Rect()));
    }
    ...
    if (quad->is_stream_video) {
        dc_layer->color_space = gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT709, gfx::ColorSpace::TransferID::BT709);
...
{
    if (quad->ShouldDrawWithBlending() && !quad->shared_quad_state->mask_filter_info.HasRoundedCorners()) {
...
bool IsOccluded(const gfx::RectF& target_quad, QuadList::ConstIterator quad_list_begin, QuadList::ConstIterator quad_list_end,
    const DCLayerOverlayProcessor::FilterOperationsMap& render_pass_filters)
{
    ...
    for (auto overlap_iter = quad_list_begin; overlap_iter != quad_list_end; ++overlap_iter) {
        ...
        if (!render_pass_filters.empty() && quad->material == DrawQuad::Material::kAggregatedRenderPass) {
            ...
            if (render_pass_it != render_pass_filters.end()) {
                ...
                overlap_rect = gfx::RectF(GetExpandedRectWithPixelMovingForegroundFilter(*rpdq, *filters));
        ...
        if (quad->material == DrawQuad::Material::kSolidColor) {
            ...
            if (quad->ShouldDrawWithBlending() && alpha < std::numeric_limits<float>::epsilon())
                continue;
        }
...
bool HasOccludingDamageRect(
    const SharedQuadState* shared_quad_state, const SurfaceDamageRectList& surface_damage_rect_list, const gfx::Rect& quad_rect_in_target_space)
{
    ...
    gfx::Rect occluding_damage_rect = gfx::UnionRects(
        base::span<const gfx::Rect>(surface_damage_rect_list.data(), surface_damage_rect_list.size()).first(overlay_damage_index));
    occluding_damage_rect.Intersect(quad_rect_in_target_space);

    return !occluding_damage_rect.IsEmpty();
}

bool IsPossibleFullScreenLetterboxing(const QuadList::ConstIterator& it, QuadList::ConstIterator quad_list_end, const gfx::Rect& display_rect)
{
    ...
    if (beneath_overlay_it != quad_list_end) {
        if (beneath_overlay_it->material == DrawQuad::Material::kTiledContent
            || (beneath_overlay_it->material == DrawQuad::Material::kSolidColor
                && SolidColorDrawQuad::MaterialCast(*beneath_overlay_it)->color == SkColors::kBlack)) {
            gfx::RectF beneath_rect = ClippedQuadRectangleF(*beneath_overlay_it);
            return (beneath_rect.origin() == gfx::PointF(display_rect.origin())
                && (beneath_rect.width() == display_rect.width() || beneath_rect.height() == display_rect.height()));
        }
    }
...
void RecordVideoDCLayerResult(DCLayerResult result, gfx::ProtectedVideoType protected_video_type)
{
    switch (protected_video_type) {
    case gfx::ProtectedVideoType::kClear:
        UMA_HISTOGRAM_ENUMERATION("GPU.DirectComposition.DCLayerResult.Video.Clear", result);
        break;
    case gfx::ProtectedVideoType::kSoftwareProtected:
        UMA_HISTOGRAM_ENUMERATION("GPU.DirectComposition.DCLayerResult.Video.SoftwareProtected", result);
        break;
    case gfx::ProtectedVideoType::kHardwareProtected:
        UMA_HISTOGRAM_ENUMERATION("GPU.DirectComposition.DCLayerResult.Video.HardwareProtected", result);
        break;
    }
}
...
void RecordDCLayerResult(DCLayerResult result, const DrawQuad* quad)
{
    ...
    case DrawQuad::Material::kTextureContent: {
        ...
        if (tex_quad->is_stream_video) {
            UMA_HISTOGRAM_ENUMERATION("GPU.DirectComposition.DCLayerResult.StreamVideo", result);
        } else if (tex_quad->is_video_frame) {
            RecordVideoDCLayerResult(result, tex_quad->protected_video_type);
        } else {
            UMA_HISTOGRAM_ENUMERATION("GPU.DirectComposition.DCLayerResult.Texture", result);
        }
        break;
    }
    default:
...
void RecordOverlayHistograms(const DCLayerOverlayProcessor::RenderPassOverlayDataMap& render_pass_overlay_data_map, bool has_occluding_surface_damage)
{
    ...
    for (auto& [render_pass, overlay_data] : render_pass_overlay_data_map) {
        is_overlay = base::ranges::all_of(overlay_data.promoted_overlays, [](const auto& dc_layer) { return dc_layer.plane_z_order > 0; });
        if (!is_overlay) {
            break;
        }
    }

    bool damage_rects_empty = base::ranges::all_of(render_pass_overlay_data_map, [](const auto& data) { return data.second.damage_rect.IsEmpty(); });

    OverlayProcessorInterface::RecordOverlayDamageRectHistograms(is_overlay, has_occluding_surface_damage, damage_rects_empty);
}
...
QuadList::Iterator FindAnOverlayCandidateExcludingMediaFoundationVideoContent(QuadList& quad_list)
{
    QuadList::Iterator it = quad_list.end();
    for (auto quad_it = quad_list.begin(); quad_it != quad_list.end(); ++quad_it) {
        if (quad_it->material == DrawQuad::Material::kTextureContent && TextureDrawQuad::MaterialCast(*quad_it)->is_stream_video) {
            return quad_list.end();
        }
        if (it == quad_list.end() && (quad_it->material == DrawQuad::Material::kTextureContent)) {
            it = quad_it;
        }
    }
    return it;
...
{
    return quad->material == DrawQuad::Material::kTextureContent && TextureDrawQuad::MaterialCast(quad)->is_video_frame;
}
...
bool IsOverlayRequiredForQuad(const DrawQuad* quad)
{
    ...
    return quad->material == DrawQuad::Material::kTextureContent && TextureDrawQuad::MaterialCast(quad)->is_stream_video;
}
...
bool AllowRemoveClearVideoQuadCandidatesWhenMoving(const DisplayResourceProvider* resource_provider, const DrawQuad* quad, bool force_overlay_for_auto_hdr)
{
    ...
    case DrawQuad::Material::kTextureContent: {
        ...
        return !(resource_provider->GetColorSpace(texture_quad->resource_id()).IsHDR() || force_overlay_for_auto_hdr);
    }
    default:
        NOTREACHED();
    }
...
// be empty.
gfx::Rect PreviousFrameOverlayDamageContribution(const std::vector<DCLayerOverlayProcessor::OverlayRect>& previous_frame_overlay_rects)
{
...
bool IsPreviousFrameUnderlayRect(
    const std::vector<DCLayerOverlayProcessor::OverlayRect>& previous_frame_overlay_rects, const gfx::Rect& quad_rect, size_t index)
{
    if (index >= previous_frame_overlay_rects.size()) {
        ...
        // power.
        return (previous_frame_overlay_rects[index].rect == quad_rect) && (previous_frame_overlay_rects[index].is_overlay == false);
    }
}
...
ValidateDrawQuadResult ValidateDrawQuad(const DisplayResourceProvider* resource_provider, const QuadList::ConstIterator& it,
    const std::vector<gfx::Rect>& backdrop_filter_rects, const bool has_overlay_support, const bool has_p010_video_processor_support,
    const int allowed_yuv_overlay_count, const int processed_yuv_overlay_count, const bool allow_promotion_hinting)
{
    ...
    case DrawQuad::Material::kTextureContent: {
        ...
        if (tex_quad->is_stream_video) {
            ...
            result.code = ValidateTextureQuad(tex_quad, backdrop_filter_rects, has_overlay_support, has_p010_video_processor_support, allowed_yuv_overlay_count,
...
void FromDrawQuad(const DisplayResourceProvider* resource_provider, const AggregatedRenderPass* render_pass, bool is_page_fullscreen_mode,
    const QuadList::ConstIterator& it, int& processed_yuv_overlay_count, OverlayCandidate& dc_layer)
{
    dc_layer.possible_video_fullscreen_letterboxing
        = is_page_fullscreen_mode ? IsPossibleFullScreenLetterboxing(it, render_pass->quad_list.end(), render_pass->output_rect) : false;
    switch (it->material) {
    case DrawQuad::Material::kTextureContent: {
        const TextureDrawQuad* tex_quad = TextureDrawQuad::MaterialCast(*it);
        FromTextureQuad(tex_quad, render_pass->transform_to_root_target, resource_provider, &dc_layer);
...
std::optional<OverlayCandidate> DCLayerOverlayProcessor::FromTextureOrYuvQuad(const DisplayResourceProvider* resource_provider,
    const AggregatedRenderPass* render_pass, const QuadList::ConstIterator& it, bool is_page_fullscreen_mode) const
{
    ...
    ValidateDrawQuadResult result
        = ValidateDrawQuad(resource_provider, it, backdrop_filter_rects, true, has_p010_video_processor_support_, INT_MAX, INT_MIN, false);
    ...
    FromDrawQuad(resource_provider, render_pass, is_page_fullscreen_mode, it, ignore_processed_yuv_overlay_count, candidate);
    ...
    if (it->shared_quad_state->mask_filter_info.HasRoundedCorners()) {
        gfx::MaskFilterInfo mask_filter_info = it->shared_quad_state->mask_filter_info;
    ...
    if (it->visible_rect != it->rect) {
        ...
        candidate.uv_rect = gfx::MapRect(gfx::RectF(it->visible_rect), gfx::RectF(it->rect), candidate.uv_rect);
...
DCLayerOverlayProcessor::DCLayerOverlayProcessor(int allowed_yuv_overlay_count, bool disable_video_overlay_if_moving, bool skip_initialization_for_testing)
    : has_overlay_support_(skip_initialization_for_testing)
    , allowed_yuv_overlay_count_(allowed_yuv_overlay_count)
    , is_on_battery_power_(base::PowerMonitor::GetInstance()->AddPowerStateObserverAndReturnOnBatteryState(this))
    , no_undamaged_overlay_promotion_(false/*base::FeatureList::IsEnabled(features::kNoUndamagedOverlayPromotion)*/)
...
void DCLayerOverlayProcessor::UpdateSystemHDRStatus()
{
    ...
    system_hdr_disabled_on_any_display_ = dxgi_info->output_descs.size() > 0 ? hdr_disabled_on_any_display : true;
}

void DCLayerOverlayProcessor::UpdateP010VideoProcessorSupport()
{
    has_p010_video_processor_support_ = gl::CheckVideoProcessorFormatSupport(DXGI_FORMAT_P010);
}
...
void DCLayerOverlayProcessor::OnBatteryPowerStatusChange(base::PowerStateObserver::BatteryPowerStatus battery_power_status)
{
...
void DCLayerOverlayProcessor::RemoveOverlayDamageRect(const QuadList::Iterator& it, RenderPassCurrentFrameState& render_pass_state) const
{
    ...
    if (it->shared_quad_state->overlay_damage_index.has_value()) {
        size_t overlay_damage_index = it->shared_quad_state->overlay_damage_index.value();
        CHECK_LT(overlay_damage_index, render_pass_state.surface_damage_rect_list.size());
...
void DCLayerOverlayProcessor::UpdateDamageRect(AggregatedRenderPass* render_pass, const RenderPassPreviousFrameState& previous_frame_state,
    RenderPassOverlayData& overlay_data, RenderPassCurrentFrameState& current_frame_state) const
{
    ...
    const std::vector<OverlayRect>& previous_frame_overlay_rects = previous_frame_state.overlay_rects;

    bool should_add_previous_frame_overlay_damage = true;
    if (!current_frame_state.overlay_rects.empty() && current_frame_state.overlay_rects == previous_frame_overlay_rects
        ...
        if (render_pass->transform_to_root_target.Preserves2dAxisAlignment()) {
            ...
            for (auto surface_damage_rect : current_frame_state.surface_damage_rect_list) {
                ...
                for (const auto index_to_be_removed : current_frame_state.damages_to_be_removed) {
                    ...
                    if (surface_index == index_to_be_removed) {
                        ...
                        surface_damage_rect.Subtract(current_frame_state.surface_damage_rect_list[index_to_be_removed]);
                    }
                }
    ...
    if (should_add_previous_frame_overlay_damage) {
        overlay_data.damage_rect.Union(PreviousFrameOverlayDamageContribution(previous_frame_overlay_rects));
    }
    overlay_data.damage_rect.Intersect(render_pass->output_rect);
    current_frame_state.damages_to_be_removed.clear();
}

void DCLayerOverlayProcessor::RemoveClearVideoQuadCandidatesIfMoving(const DisplayResourceProvider* resource_provider,
    RenderPassOverlayDataMap& render_pass_overlay_data_map, RenderPassCurrentFrameStateMap& render_pass_state_map)
{
    ...
    for (auto& [render_pass, overlay_data] : render_pass_overlay_data_map) {
        std::vector<QuadList::Iterator>& candidates = render_pass_state_map[render_pass].candidates;
        current_overlay_candidate_rects.reserve(current_overlay_candidate_rects.size() + candidates.size());
        for (auto candidate_it : candidates) {
            if (AllowRemoveClearVideoQuadCandidatesWhenMoving(resource_provider, *candidate_it, force_overlay_for_auto_hdr())) {
                gfx::Rect quad_rect_in_target_space = ClippedQuadRectangle(*candidate_it);
                gfx::Rect quad_rect_in_root_space = cc::MathUtil::MapEnclosingClippedRect(render_pass->transform_to_root_target, quad_rect_in_target_space);
                current_overlay_candidate_rects.push_back(quad_rect_in_root_space);
            }
        }
    }

    if (previous_frame_overlay_candidate_rects_ != current_overlay_candidate_rects) {
        frames_since_last_overlay_candidate_rects_change_ = 0;
        std::swap(previous_frame_overlay_candidate_rects_, current_overlay_candidate_rects);
    } else {
        frames_since_last_overlay_candidate_rects_change_++;
    }

    if (frames_since_last_overlay_candidate_rects_change_ <= kFramesOfStabilityForOverlayPromotion) {
...
void DCLayerOverlayProcessor::CollectCandidates(const DisplayResourceProvider* resource_provider, AggregatedRenderPass* render_pass,
    const FilterOperationsMap& render_pass_backdrop_filters, RenderPassOverlayData& overlay_data, RenderPassCurrentFrameState& render_pass_state,
    GlobalOverlayState& global_overlay_state)
{
    ...
    if (ShouldSkipOverlay(render_pass)) {
        ...
        if (it != previous_frame_render_pass_states_.end()) {
            ...
            // for.
            overlay_data.damage_rect.Union(PreviousFrameOverlayDamageContribution(it->second.overlay_rects));
    ...
    for (auto it = quad_list->begin(); it != quad_list->end(); ++it) {
        if (it->material == DrawQuad::Material::kAggregatedRenderPass) {
            ...
            auto render_pass_it = render_pass_backdrop_filters.find(rpdq->render_pass_id);
        ...
        ValidateDrawQuadResult result = ValidateDrawQuad(resource_provider, it, backdrop_filter_rects, has_overlay_support_, has_p010_video_processor_support_,
            allowed_yuv_overlay_count_, global_overlay_state.processed_yuv_overlay_count, allow_promotion_hinting_);

        if (result.is_yuv_overlay) {
            global_overlay_state.yuv_quads++;
            if (no_undamaged_overlay_promotion_) {
                if (it->shared_quad_state->overlay_damage_index.has_value()
                    && !render_pass_state.surface_damage_rect_list[it->shared_quad_state->overlay_damage_index.value()].IsEmpty()) {
        ...
        if (!result.promotion_hint_mailbox.IsZero()) {
            ...
            overlay_state_service->SetPromotionHint(result.promotion_hint_mailbox, promoted);
        }
...
void DCLayerOverlayProcessor::PromoteCandidates(const DisplayResourceProvider* resource_provider, AggregatedRenderPass* render_pass,
    const FilterOperationsMap& render_pass_filters, const RenderPassPreviousFrameState& previous_frame_state, bool is_page_fullscreen_mode,
    RenderPassOverlayData& overlay_data, RenderPassCurrentFrameState& current_frame_state, GlobalOverlayState& global_overlay_state)
{
    QuadList* quad_list = &render_pass->quad_list;
    ...
    for (auto& it : current_frame_state.candidates) {
        ...
        bool undamaged = it->shared_quad_state->overlay_damage_index.has_value()
            && current_frame_state.surface_damage_rect_list[it->shared_quad_state->overlay_damage_index.value()].IsEmpty();

        if (global_overlay_state.yuv_quads > allowed_yuv_overlay_count_ && !global_overlay_state.has_non_clear_video_overlays && undamaged
        ...
        bool is_overlay = !IsOccluded(gfx::RectF(quad_rect_in_target_space), quad_list->begin(), it, render_pass_filters);
        ...
        // Used by a histogram.
        global_overlay_state.has_occluding_damage_rect = global_overlay_state.has_occluding_damage_rect
            || (!is_overlay && HasOccludingDamageRect(it->shared_quad_state, current_frame_state.surface_damage_rect_list, quad_rect_in_target_space));

        UpdateDCLayerOverlays(resource_provider, render_pass, it, quad_rect_in_target_space, previous_frame_state, is_overlay, is_page_fullscreen_mode,
    ...
    UpdateDamageRect(render_pass, previous_frame_state, overlay_data, current_frame_state);

    RenderPassPreviousFrameState& previous_frame_render_pass_state = previous_frame_render_pass_states_.at(render_pass->id);
    std::swap(previous_frame_render_pass_state.overlay_rects, current_frame_state.overlay_rects);
    previous_frame_render_pass_state.display_rect = render_pass->output_rect;
}

void DCLayerOverlayProcessor::Process(const DisplayResourceProvider* resource_provider, const FilterOperationsMap& render_pass_filters,
    const FilterOperationsMap& render_pass_backdrop_filters, const SurfaceDamageRectList& surface_damage_rect_list_in_root_space, bool is_page_fullscreen_mode,
    RenderPassOverlayDataMap& render_pass_overlay_data_map)
{
    ...
    for (auto& [render_pass, overlay_data] : render_pass_overlay_data_map) {
        ...
        RenderPassCurrentFrameState& current_frame_state = render_pass_state_map[render_pass];
        ...
        current_frame_state.surface_damage_rect_list = surface_damage_rect_list_in_root_space;
        ...
        CollectCandidates(resource_provider, render_pass, render_pass_backdrop_filters, overlay_data, current_frame_state, global_overlay_state);
    }
    ...
    // overlays
    if (global_overlay_state.yuv_quads > 1 && !global_overlay_state.has_non_clear_video_overlays) {
        if (no_undamaged_overlay_promotion_) {
            if (global_overlay_state.damaged_yuv_quads == global_overlay_state.processed_yuv_overlay_count) {
                frames_since_last_qualified_multi_overlays_++;
            } else {
                frames_since_last_qualified_multi_overlays_ = 0;
            }
            global_overlay_state.reject_overlays = frames_since_last_qualified_multi_overlays_ <= kDCLayerFramesDelayedBeforeOverlay;
        } else {
            if (global_overlay_state.yuv_quads != global_overlay_state.processed_yuv_overlay_count) {
    ...
    if (disable_video_overlay_if_moving_) {
        RemoveClearVideoQuadCandidatesIfMoving(resource_provider, render_pass_overlay_data_map, render_pass_state_map);
    }
    ...
    base::flat_map<AggregatedRenderPassId, RenderPassPreviousFrameState> previous_frame_render_pass_states;
    std::swap(previous_frame_render_pass_states_, previous_frame_render_pass_states);

    for (auto& [render_pass, overlay_data] : render_pass_overlay_data_map) {
        // Create an entry for the current frame. This is the only place where an
        // entry is created in this map.
        previous_frame_render_pass_states_.emplace(render_pass->id, RenderPassPreviousFrameState());

        PromoteCandidates(resource_provider, render_pass, render_pass_filters, previous_frame_render_pass_states[render_pass->id], is_page_fullscreen_mode,
            overlay_data, render_pass_state_map[render_pass], global_overlay_state);
    }

    if (global_overlay_state.processed_yuv_overlay_count > 0) {
        base::UmaHistogramExactLinear("GPU.DirectComposition.DCLayer.YUVOverlayCount",
            /*sample=*/global_overlay_state.processed_yuv_overlay_count,
            /*exclusive_max=*/10);

        RecordOverlayHistograms(render_pass_overlay_data_map, global_overlay_state.has_occluding_damage_rect);
    }
}

bool DCLayerOverlayProcessor::ShouldSkipOverlay(AggregatedRenderPass* render_pass) const
{
    QuadList* quad_list = &render_pass->quad_list;
    ...
    if (render_pass->HasCapture()) {
        ...
        if (it != quad_list->end()) {
            render_pass->video_capture_enabled ? RecordDCLayerResult(DC_LAYER_FAILED_VIDEO_CAPTURE_ENABLED, *it)
    ...
    if (render_pass->content_color_usage == gfx::ContentColorUsage::kHDR) {
        ...
        QuadList::Iterator it = FindAnOverlayCandidateExcludingMediaFoundationVideoContent(*quad_list);
        if (it != quad_list->end()) {
            ...
            bool supports_rgb10a2_overlay = gl::GetDirectCompositionOverlaySupportFlags(DXGI_FORMAT_R10G10B10A2_UNORM) != 0;
            ...
            if (system_hdr_disabled_on_any_display_) {
                RecordDCLayerResult(DC_LAYER_FAILED_YUV_VIDEO_QUAD_HDR_TONE_MAPPING, *it);
                return true;
            }
        }
    }
...
void DCLayerOverlayProcessor::UpdateDCLayerOverlays(const DisplayResourceProvider* resource_provider, AggregatedRenderPass* render_pass,
    const QuadList::Iterator& it, const gfx::Rect& quad_rect_in_target_space, const RenderPassPreviousFrameState& previous_frame_state, bool is_overlay,
    bool is_page_fullscreen_mode, RenderPassOverlayData& overlay_data, RenderPassCurrentFrameState& current_frame_state,
    GlobalOverlayState& global_overlay_state)
{
    ...
    FromDrawQuad(resource_provider, render_pass, is_page_fullscreen_mode, it, global_overlay_state.processed_yuv_overlay_count, dc_layer);

    // Underlays are less efficient, so attempt regular overlays first. We can
    // only check for occlusion within a render pass.
    if (is_overlay) {
        ProcessForOverlay(render_pass, it, previous_frame_state, current_frame_state);
    } else {
        ProcessForUnderlay(render_pass, it, quad_rect_in_target_space, previous_frame_state, global_overlay_state, overlay_data, current_frame_state, dc_layer);
    }

    current_frame_state.overlay_rects.push_back({ quad_rect_in_target_space, is_overlay });
...
void DCLayerOverlayProcessor::ProcessForOverlay(AggregatedRenderPass* render_pass, const QuadList::Iterator& it,
    const RenderPassPreviousFrameState& previous_frame_state, RenderPassCurrentFrameState& current_frame_state) const
{
    // The quad is on top, so promote it to an overlay and remove all damage
    // underneath it.
    const bool display_rect_changed = render_pass->output_rect != previous_frame_state.display_rect;
    const bool is_axis_aligned = it->shared_quad_state->quad_to_target_transform.Preserves2dAxisAlignment();
...
void DCLayerOverlayProcessor::ProcessForUnderlay(AggregatedRenderPass* render_pass, const QuadList::Iterator& it, const gfx::Rect& quad_rect_in_target_space,
    const RenderPassPreviousFrameState& previous_frame_state, const GlobalOverlayState& global_overlay_state, RenderPassOverlayData& overlay_data,
    RenderPassCurrentFrameState& current_frame_state, OverlayCandidate& dc_layer)
{
    ...
    const bool display_rect_unchanged = render_pass->output_rect == previous_frame_state.display_rect;
    const bool underlay_rect_unchanged
        = IsPreviousFrameUnderlayRect(previous_frame_state.overlay_rects, quad_rect_in_target_space, overlay_data.promoted_overlays.size());
    const bool is_axis_aligned = it->shared_quad_state->quad_to_target_transform.Preserves2dAxisAlignment();
    bool opacity_unchanged = (is_opaque == previous_frame_state.underlay_is_opaque);
    previous_frame_render_pass_states_.at(render_pass->id).underlay_is_opaque = is_opaque;

    if (is_axis_aligned && opacity_unchanged && underlay_rect_unchanged && display_rect_unchanged) {
        ...
        current_frame_state.surface_damage_rect_list.push_back(quad_rect_in_target_space);
    }
}

DCLayerOverlayProcessor::RenderPassOverlayData::RenderPassOverlayData() = default;
DCLayerOverlayProcessor::RenderPassOverlayData::~RenderPassOverlayData() = default;
DCLayerOverlayProcessor::RenderPassOverlayData::RenderPassOverlayData(RenderPassOverlayData&&) = default;
DCLayerOverlayProcessor::RenderPassOverlayData& DCLayerOverlayProcessor::RenderPassOverlayData::operator=(RenderPassOverlayData&&) = default;

DCLayerOverlayProcessor::RenderPassPreviousFrameState::RenderPassPreviousFrameState() = default;
DCLayerOverlayProcessor::RenderPassPreviousFrameState::~RenderPassPreviousFrameState() = default;
DCLayerOverlayProcessor::RenderPassPreviousFrameState::RenderPassPreviousFrameState(RenderPassPreviousFrameState&&) = default;
DCLayerOverlayProcessor::RenderPassPreviousFrameState& DCLayerOverlayProcessor::RenderPassPreviousFrameState::operator=(RenderPassPreviousFrameState&&)
    = default;

DCLayerOverlayProcessor::RenderPassCurrentFrameState::RenderPassCurrentFrameState() = default;
DCLayerOverlayProcessor::RenderPassCurrentFrameState::~RenderPassCurrentFrameState() = default;
DCLayerOverlayProcessor::RenderPassCurrentFrameState::RenderPassCurrentFrameState(RenderPassCurrentFrameState&&) = default;
DCLayerOverlayProcessor::RenderPassCurrentFrameState& DCLayerOverlayProcessor::RenderPassCurrentFrameState::operator=(RenderPassCurrentFrameState&&) = default;
} // namespace viz
