...
const std::optional<gfx::Rect>& GetOptionalDamageRectFromQuad(const DrawQuad* quad)
{
...
AggregationPassData& AggregationPassData::operator=(AggregationPassData&& other) = default;
AggregationPassData::~AggregationPassData() = default;
...
PersistentPassData& PersistentPassData::operator=(PersistentPassData& other) = default;
PersistentPassData& PersistentPassData::operator=(const PersistentPassData& other) = default;
PersistentPassData& PersistentPassData::operator=(PersistentPassData&& other) = default;
PersistentPassData::~PersistentPassData() = default;
...
ResolvedPassData& ResolvedPassData::operator=(ResolvedPassData&& other) = default;
...
ResolvedFrameData::ResolvedFrameData(
    DisplayResourceProvider* resource_provider, Surface* surface, uint64_t previous_frame_index, AggregatedRenderPassId prev_root_pass_id)
...
void ResolvedFrameData::UpdateForAggregation(AggregatedRenderPassId::Generator& render_pass_id_generator)
{
...
void ResolvedFrameData::UpdateActiveFrame(AggregatedRenderPassId::Generator& render_pass_id_generator)
{
    ...
    auto& child_to_parent_map = resource_provider_->GetChildToParentMap(child_resource_id_);
    ...
    for (size_t i = 0; i < num_render_pass; ++i) {
        ...
        for (auto* quad : render_pass->quad_list) {
            if (quad->material == DrawQuad::Material::kCompositorRenderPass) {
                ...
                auto quad_render_pass_id = CompositorRenderPassDrawQuad::MaterialCast(quad)->render_pass_id;
        ...
        if (!render_pass_id_map_.insert(std::make_pair(render_pass->id, &resolved_passes_.back())).second) {
    ...
    base::EraseIf(aggregated_id_map_, [this](auto& entry) { return render_pass_id_map_.find(entry.first) == render_pass_id_map_.end(); });
    ...
    resource_provider_->DeclareUsedResourcesFromChild(child_resource_id_, ResourceIdSet(std::move(referenced_resources)));
}

void ResolvedFrameData::UpdateOffsetTags(OffsetTagLookupFn lookup_value_fn)
{
    auto& offset_tags_to_find = surface_->GetActiveFrameMetadata().offset_tag_definitions;
    ...
    bool offset_tag_values_changed_from_last_frame
        = std::ranges::any_of(offset_tag_data_, [](auto& entry) { return entry.second.current_offset != entry.second.last_offset; });
    ...
    if (offset_tag_values_changed_from_last_frame) {
        ...
        for (size_t i = 0; i < offset_tag_render_passes_.size(); ++i) {
            resolved_passes_[i].SetCompositorRenderPass(offset_tag_render_passes_[i].get());
        }
...
void ResolvedFrameData::ComputeOffsetTagContainingRects()
{
    ...
    base::flat_set<OffsetTag> tag_set(base::ToVector(offset_tags_to_find, [](const OffsetTagDefinition& def) { return def.tag; }));
    ...
    for (auto& resolved_pass : resolved_passes_) {
        ...
        for (auto* sqs : render_pass.shared_quad_state_list) {
            if (sqs->offset_tag && tag_set.contains(sqs->offset_tag)) {
                gfx::Transform combined_transform = render_pass.transform_to_root_target * sqs->quad_to_target_transform;
                gfx::Rect containing_rect_in_root = cc::MathUtil::MapEnclosingClippedRect(combined_transform, sqs->visible_quad_layer_rect);
                offset_tag_data_[sqs->offset_tag].current_containing_rect.Union(containing_rect_in_root);
            }
        }
    }
            ...
            if (sqs->offset_tag && offset_tag_data_.contains(sqs->offset_tag)) {
                ...
                if (!tag_data.current_offset.IsZero()) {
                    ...
                    if (!sqs->mask_filter_info.IsEmpty()) {
                        ...
                        sqs->mask_filter_info.ApplyTransform(gfx::Transform::MakeTranslation(tag_data.current_offset));
                    }
                }
            }
        }
...
void ResolvedFrameData::RecomputeOffsetTagDamage()
{
    ...
    for (auto& [tag, data] : offset_tag_data_) {
        if (data.current_offset != data.last_offset) {
            ...
            offset_tag_added_damage_.Union(EnclosingOffsetRect(data.current_containing_rect, data.current_offset));
            offset_tag_added_damage_.Union(EnclosingOffsetRect(data.last_containing_rect, data.last_offset));
            ...
            offset_tag_added_damage_.Union(EnclosingOffsetRect(gfx::IntersectRects(data.current_containing_rect, surface_damage_rect), data.current_offset));

            if (!data.last_containing_rect.IsEmpty() && !data.current_containing_rect.Contains(data.last_containing_rect)) {
...
void ResolvedFrameData::ResetAfterAggregation()
{
    ...
    base::EraseIf(offset_tag_data_, [](auto& entry) { return !entry.second.defined_in_frame; });
...
ResolvedPassData& ResolvedFrameData::GetRenderPassDataById(CompositorRenderPassId render_pass_id)
{
    return const_cast<ResolvedPassData&>(const_cast<const ResolvedFrameData*>(this)->GetRenderPassDataById(render_pass_id));
}

const ResolvedPassData& ResolvedFrameData::GetRenderPassDataById(CompositorRenderPassId render_pass_id) const
{
    DCHECK(valid_);
...
FrameDamageType ResolvedFrameData::GetFrameDamageType() const
{
    ...
    } else if (previous_frame_index_ > kInvalidFrameIndex && frame_index_ == previous_frame_index_ + 1) {
...
gfx::Rect ResolvedFrameData::GetSurfaceDamage() const
{
    ...
    case FrameDamageType::kFrame:
        return gfx::UnionRects(resolved_passes_.back().render_pass().damage_rect, offset_tag_added_damage_);
...
{
    const CompositorRenderPassList& render_pass_list = surface_->GetActiveFrame().render_pass_list;
...
{
    child_resource_id_ = resource_provider_->CreateChild(base::BindRepeating(&SurfaceClient::UnrefResources, surface_->client()), surface_id_);
}

void ResolvedFrameData::MovePersistentPassDataFromPreviousFrame(const std::vector<ResolvedPassData>& previous_resolved_passes)
{
    for (const auto& previous_resolved_pass : previous_resolved_passes) {
        ...
        if (iter != render_pass_id_map_.end()) {
            iter->second->previous_persistent_data() = previous_resolved_pass.previous_persistent_data();
        }
    }
}
