...
// created on.
class VIZ_SERVICE_EXPORT DisplayResourceProvider : public base::trace_event::MemoryDumpProvider {
public:
    enum Mode {
        kGpu,
        kSoftware,
    ...
    void OnResourceFencePassed(ResourceFence* resource_fence, base::flat_set<ResourceId> resources);
...
public:
    ...
    public:
        ScopedReadLockSharedImage(DisplayResourceProvider* resource_provider, ResourceId resource_id);
    ...
    public:
        explicit ScopedBatchReturnResources(DisplayResourceProvider* resource_provider, bool allow_access_to_gpu_thread = false);
    ...
    const std::unordered_map<ResourceId, ResourceId, ResourceIdHasher>& GetChildToParentMap(int child) const;
    ...
    // wait on it.
    void ReceiveFromChild(int child, const std::vector<TransferableResource>& transferable_resources);
    ...
    void DeclareUsedResourcesFromChild(int child, const ResourceIdSet& resources_from_child);
...
protected:
    ...
    struct Child {
        ...
        int id;
        std::unordered_map<ResourceId, ResourceId, ResourceIdHasher> child_to_parent_map;
    ...
    struct ChildResource {
        ...
        bool is_gpu_resource_type() const
        {
            return !transferable.is_software;
        }
        const gpu::SyncToken& sync_token() const
        {
            return sync_token_;
        ...
        bool InUse() const
        {
            return lock_for_read_count > 0 || locked_for_external_use || lock_for_overlay_count > 0;
        }
    ...
    using ResourceMap = std::unordered_map<ResourceId, ChildResource, ResourceIdHasher>;
    ...
    void DeleteAndReturnUnusedResourcesToChild(ChildMap::iterator child_it, DeleteStyle style, const std::vector<ResourceId>& unused);
    virtual std::vector<ReturnedResource> DeleteAndReturnUnusedResourcesToChildImpl(Child& child_info, DeleteStyle style, const std::vector<ResourceId>& unused)
        = 0;
    CanDeleteNowResult CanDeleteNow(const Child& child_info, const ChildResource& resource, DeleteStyle style) const;
