...
gfx::CALayerResult FromRenderPassQuad(const DisplayResourceProvider* resource_provider, const AggregatedRenderPassDrawQuad* quad,
    const base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>>& render_pass_filters,
    const base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>>& render_pass_backdrop_filters,
    OverlayCandidate* ca_layer_overlay)
{
    ...
    if (shared_quad_state->clip_rect && !shared_quad_state->quad_to_target_transform.Preserves2dAxisAlignment()) {
        return gfx::kCALayerFailedQuadClipping;
    }

    // TODO(crbug.com/40769959): support not 2d axis aligned mask.
    if (!shared_quad_state->mask_filter_info.IsEmpty() && !shared_quad_state->quad_to_target_transform.Preserves2dAxisAlignment()) {
...
gfx::CALayerResult FromSolidColorDrawQuad(const SolidColorDrawQuad* quad, OverlayCandidate* ca_layer_overlay, bool* skip)
{
...
gfx::CALayerResult FromTextureQuad(const DisplayResourceProvider* resource_provider, const TextureDrawQuad* quad, OverlayCandidate* ca_layer_overlay)
{
    ...
    ca_layer_overlay->uv_rect = BoundingRect(quad->uv_top_left, quad->uv_bottom_right);
    ca_layer_overlay->color = quad->background_color;
    ca_layer_overlay->nearest_neighbor_filter = quad->nearest_neighbor;
    ca_layer_overlay->hdr_metadata = resource_provider->GetHDRMetadata(resource_id);
    if (quad->is_video_frame)
        ca_layer_overlay->protected_video_type = quad->protected_video_type;
    return gfx::kCALayerSuccess;
}

gfx::CALayerResult FromTileQuad(const DisplayResourceProvider* resource_provider, const TileDrawQuad* quad, OverlayCandidate* ca_layer_overlay)
{
    ResourceId resource_id = quad->resource_id();
    if (!resource_provider->IsOverlayCandidate(resource_id))
    ...
    ca_layer_overlay->uv_rect.InvScale(quad->texture_size.width(), quad->texture_size.height());
    ca_layer_overlay->nearest_neighbor_filter = quad->nearest_neighbor;
    return gfx::kCALayerSuccess;
}
...
public:
    gfx::CALayerResult FromDrawQuad(const DisplayResourceProvider* resource_provider, const gfx::RectF& display_rect, const DrawQuad* quad,
        const base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>>& render_pass_filters,
        const base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>>& render_pass_backdrop_filters,
        OverlayCandidate* ca_layer_overlay, bool* skip, bool* render_pass_draw_quad, int& yuv_draw_quad_count)
    {
        if (quad->shared_quad_state->blend_mode != SkBlendMode::kSrcOver)
            return gfx::kCALayerFailedQuadBlendMode;

        // Early-out for invisible quads.
        if (quad->shared_quad_state->opacity == 0.f || quad->visible_rect.IsEmpty()) {
            *skip = true;
            return gfx::kCALayerSuccess;
        }
        ...
        if (quad->shared_quad_state->mask_filter_info.HasRoundedCorners()) {
            if (quad->shared_quad_state->mask_filter_info.rounded_corner_bounds().GetType() > gfx::RRectF::Type::kSingle) {
        ...
        ca_layer_overlay->sorting_context_id = quad->shared_quad_state->sorting_context_id;
        ca_layer_overlay->clip_rect = quad->shared_quad_state->clip_rect;
        ca_layer_overlay->rounded_corners = quad->shared_quad_state->mask_filter_info.rounded_corner_bounds();
        ca_layer_overlay->transform = quad->shared_quad_state->quad_to_target_transform;

        ca_layer_overlay->display_rect = gfx::RectF(quad->rect);
        ca_layer_overlay->opacity = quad->shared_quad_state->opacity;

        *render_pass_draw_quad = quad->material == DrawQuad::Material::kAggregatedRenderPass;
        switch (quad->material) {
        case DrawQuad::Material::kTextureContent: {
            const TextureDrawQuad* texture_draw_quad = TextureDrawQuad::MaterialCast(quad);
            // Stream video and video frame counts as a yuv draw quad.
            if (texture_draw_quad->is_stream_video || texture_draw_quad->is_video_frame) {
                yuv_draw_quad_count += 1;
            }
            return FromTextureQuad(resource_provider, texture_draw_quad, ca_layer_overlay);
        }
        case DrawQuad::Material::kTiledContent:
            return FromTileQuad(resource_provider, TileDrawQuad::MaterialCast(quad), ca_layer_overlay);
        case DrawQuad::Material::kSolidColor:
            return FromSolidColorDrawQuad(SolidColorDrawQuad::MaterialCast(quad), ca_layer_overlay, skip);
        ...
        case DrawQuad::Material::kAggregatedRenderPass:
            return FromRenderPassQuad(
                resource_provider, AggregatedRenderPassDrawQuad::MaterialCast(quad), render_pass_filters, render_pass_backdrop_filters, ca_layer_overlay);
    ...
    bool video_with_odd_width()
    {
        return video_with_odd_width_;
    }
    bool video_with_odd_height()
    {
        return video_with_odd_height_;
    }
    bool video_with_odd_x()
    {
        return video_with_odd_x_;
    }
    bool video_with_odd_y()
    {
        return video_with_odd_y_;
    }
...
BASE_FEATURE(kCARenderer, "CoreAnimationRenderer", base::FEATURE_ENABLED_BY_DEFAULT);

// Control using the CoreAnimation renderer, which is the path that replaces
// all quads with CALayers.
BASE_FEATURE(kHDRUnderlays, "CoreAnimationHDRUnderlays", base::FEATURE_ENABLED_BY_DEFAULT);
    ...
    , enable_hdr_underlays_(base::FeatureList::IsEnabled(kHDRUnderlays))
{
    ...
    if (base::FeatureList::IsEnabled(features::kCALayerNewLimit)) {
        ...
        const int layer_limit_default_field_trial = features::kCALayerNewLimitDefault.Get();
        if (layer_limit_default_field_trial > 0) {
            layer_limit_default_ = layer_limit_default_field_trial;
        }

        const int layer_limit_with_many_videos_field_trial = features::kCALayerNewLimitManyVideos.Get();
...
bool CALayerOverlayProcessor::AreClipSettingsValid(const OverlayCandidate& ca_layer_overlay, OverlayCandidateList* ca_layer_overlay_list) const
{
    ...
    if (!ca_layer_overlay_list->empty()) {
        ...
        if (ca_layer_overlay.sorting_context_id && previous_ca_layer.sorting_context_id == ca_layer_overlay.sorting_context_id) {
...
void CALayerOverlayProcessor::PutForcedOverlayContentIntoUnderlays(const DisplayResourceProvider* resource_provider, AggregatedRenderPass* render_pass,
    ...
    OverlayCandidateList* ca_layer_overlays) const
{
    ...
    for (auto it = quad_list->begin(); it != quad_list->end(); ++it) {
        ...
        if (quad->material == ContentDrawQuadBase::Material::kTextureContent) {
            ...
            if (texture_quad->is_video_frame && texture_quad->protected_video_type != gfx::ProtectedVideoType::kClear) {
        ...
        if (force_quad_to_overlay) {
            if (!PutQuadInSeparateOverlay(it, resource_provider, render_pass, display_rect, quad, render_pass_filters, render_pass_backdrop_filters,
...
bool CALayerOverlayProcessor::ProcessForCALayerOverlays(AggregatedRenderPass* render_pass, const DisplayResourceProvider* resource_provider,
    const gfx::RectF& display_rect, const base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>>& render_pass_filters,
    const base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>>& render_pass_backdrop_filters,
    OverlayCandidateList* ca_layer_overlays)
{
    ...
    for (auto it = quad_list.BackToFrontBegin(); result == gfx::kCALayerSuccess && it != quad_list.BackToFrontEnd(); ++it) {
        const DrawQuad* quad = *it;
        OverlayCandidate ca_layer;
        bool skip = false;
        bool render_pass_draw_quad = false;
        result = processor.FromDrawQuad(resource_provider, display_rect, quad, render_pass_filters, render_pass_backdrop_filters, &ca_layer, &skip,
    ...
    size_t max_number = (yuv_draw_quad_count < kMaxNumVideos) ? layer_limit_default_ : layer_limit_with_many_videos_;
...
bool CALayerOverlayProcessor::PutQuadInSeparateOverlay(QuadList::Iterator at, const DisplayResourceProvider* resource_provider,
    AggregatedRenderPass* render_pass, const gfx::RectF& display_rect, const DrawQuad* quad,
    const base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>>& render_pass_filters,
    const base::flat_map<AggregatedRenderPassId, raw_ptr<cc::FilterOperations, CtnExperimental>>& render_pass_backdrop_filters,
    gfx::ProtectedVideoType protected_video_type, OverlayCandidateList* ca_layer_overlays) const
{
    CALayerOverlayProcessorInternal processor;
    OverlayCandidate ca_layer;
    bool skip = false;
    bool render_pass_draw_quad = false;
    int yuv_draw_quad_count = 0;
    gfx::CALayerResult result = processor.FromDrawQuad(resource_provider, display_rect, quad, render_pass_filters, render_pass_backdrop_filters, &ca_layer,
        &skip, &render_pass_draw_quad, yuv_draw_quad_count);
    if (result != gfx::kCALayerSuccess)
        return false;
