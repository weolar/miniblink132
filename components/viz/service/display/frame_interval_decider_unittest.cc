...
public:
    ...
    void SetResult(std::optional<Result> result)
    {
        result_ = result;
    }
...
class FrameIntervalDeciderTest : public testing::Test, public SurfaceManagerDelegate {
public:
    FrameIntervalDeciderTest()
...
protected:
    ...
    Surface* CreateSurface(const FrameSinkId& frame_sink_id, FrameIntervalInputs frame_interval_inputs)
    {
        LocalSurfaceId local_surface_id = LocalSurfaceId(1u, base::UnguessableToken::Create());
        ...
        Surface* surface = surface_manager_->CreateSurface(surface_client(), surface_info, SurfaceId());
    ...
    void UpdateFrame(Surface* surface, FrameIntervalInputs frame_interval_inputs)
    {
        ...
        ASSERT_TRUE(surface->QueueFrame(std::move(frame), frame_index, base::ScopedClosureRunner()));
        surface->ActivatePendingFrameForDeadline();
        ASSERT_EQ(surface->GetActiveFrameIndex(), frame_index);
    }

    void DrawSurfaces(std::vector<Surface*> surfaces, base::TimeTicks frame_time)
    {
        std::unique_ptr<FrameIntervalDecider::ScopedAggregate> scoped_aggregate = decider_->WrapAggregate(*surface_manager_, frame_time);
        for (auto* surface : surfaces) {
            static_cast<SurfaceObserver*>(scoped_aggregate.get())->OnSurfaceWillBeDrawn(surface);
        }
    }
    ...
    void InitializeDecider()
    {
        ...
        {
            auto matcher = std::make_unique<TestFrameIntervalMatcher>(FrameIntervalMatcherType::kInputBoost);
            matchers_.push_back(matcher->GetWeakPtr());
            matchers.push_back(std::move(matcher));
        }
        {
            auto matcher = std::make_unique<TestFrameIntervalMatcher>(FrameIntervalMatcherType::kOnlyVideo);
            matchers_.push_back(matcher->GetWeakPtr());
            matchers.push_back(std::move(matcher));
        }
        settings_.result_callback = base::BindRepeating(&FrameIntervalDeciderTest::SetFrameInterval, base::Unretained(this));
    ...
    {
        return matcher_type_.has_value();
    }
...
TEST_F(FrameIntervalDeciderTest, Basics)
{
    ...
    FrameIntervalMatcher::Inputs inputs_from_matcher = matchers_[0]->TakeLastMatcherInputs();
    EXPECT_EQ(kNow, inputs_from_matcher.aggregated_frame_time);
    EXPECT_EQ(settings_.increase_frame_interval_timeout, inputs_from_matcher.settings->increase_frame_interval_timeout);
    EXPECT_EQ(2u, inputs_from_matcher.inputs_map.size());
    for (Surface* surface : std::vector<Surface*> { surface1, surface2 }) {
        auto itr = inputs_from_matcher.inputs_map.find(surface->surface_id().frame_sink_id());
