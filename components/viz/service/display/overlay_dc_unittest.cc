...
std::unique_ptr<AggregatedRenderPass> CreateRenderPass(AggregatedRenderPassId render_pass_id = kDefaultRootPassId)
{
...
static ResourceId CreateResourceInLayerTree(ClientResourceProvider* child_resource_provider, const gfx::Size& size, const gfx::ColorSpace& color_space,
    const gfx::HDRMetadata& hdr_metadata, SharedImageFormat format, bool is_overlay_candidate)
{
...
ResourceId CreateResource(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    RasterContextProvider* child_context_provider, const gfx::Size& size, const gfx::ColorSpace& color_space, const gfx::HDRMetadata& hdr_metadata,
    SharedImageFormat format, bool is_overlay_candidate)
{
    ResourceId resource_id = CreateResourceInLayerTree(child_resource_provider, size, color_space, hdr_metadata, format, is_overlay_candidate);

    int child_id = parent_resource_provider->CreateChild(base::DoNothing(), SurfaceId());
...
TextureDrawQuad* CreateTextureQuadAt(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
...
{
    ResourceId resource_id = CreateResource(parent_resource_provider, child_resource_provider, child_context_provider, rect.size(), gfx::ColorSpace(),
        gfx::HDRMetadata(), SinglePlaneFormat::kRGBA_8888, is_overlay_candidate);
    ...
    quad->SetNew(shared_quad_state, rect, /*visible_rect=*/rect,
        ...
        /*nearest=*/false, /*secure_output=*/false, gfx::ProtectedVideoType::kClear);
    return quad;
}
...
TextureDrawQuad* CreateFullscreenCandidateYUVTextureQuad(DisplayResourceProvider* parent_resource_provider, ClientResourceProvider* child_resource_provider,
    RasterContextProvider* child_context_provider, const SharedQuadState* shared_quad_state, AggregatedRenderPass* render_pass,
    const gfx::ColorSpace& color_space = gfx::ColorSpace(), const gfx::HDRMetadata& hdr_metadata = gfx::HDRMetadata(),
    SharedImageFormat format = SinglePlaneFormat::kRGBA_8888)
{
    gfx::Rect rect = render_pass->output_rect;
    gfx::Size resource_size_in_pixels = rect.size();
    bool is_overlay_candidate = true;
    ResourceId resource_id = CreateResource(parent_resource_provider, child_resource_provider, child_context_provider, resource_size_in_pixels, color_space,
...
AggregatedRenderPassDrawQuad* CreateRenderPassDrawQuadAt(
    AggregatedRenderPass* render_pass, const SharedQuadState* shared_quad_state, const gfx::Rect& rect, AggregatedRenderPassId render_pass_id)
{
    AggregatedRenderPassDrawQuad* quad = render_pass->CreateAndAppendDrawQuad<AggregatedRenderPassDrawQuad>();
    quad->SetNew(
        shared_quad_state, rect, rect, render_pass_id, ResourceId(2), gfx::RectF(), gfx::Size(), gfx::Vector2dF(1, 1), gfx::PointF(), gfx::RectF(), false, 1.f);
    return quad;
}
...
protected:
    void InitializeDCLayerOverlayProcessor(int allowed_yuv_overlay_count = 1)
    {
        ...
        dc_layer_overlay_processor_ = std::make_unique<DCLayerOverlayProcessor>(allowed_yuv_overlay_count,
            /*disable_video_overlay_if_moving=*/false,
            /*skip_initialization_for_testing=*/true);

        dc_layer_overlay_processor_->set_frames_since_last_qualified_multi_overlays_for_testing(5);
    }
    ...
    DCLayerOverlayProcessor::RenderPassOverlayData ProcessRootPassForOverlays(const AggregatedRenderPassList* render_passes,
        const OverlayProcessorInterface::FilterOperationsMap& render_pass_filters,
        const OverlayProcessorInterface::FilterOperationsMap& render_pass_backdrop_filters, SurfaceDamageRectList surface_damage_rect_list_in_root_space)
    {
        DCLayerOverlayProcessor::RenderPassOverlayDataMap render_pass_overlay_data_map;
        auto emplace_pair = render_pass_overlay_data_map.emplace(render_passes->back().get(), DCLayerOverlayProcessor::RenderPassOverlayData());
        DCLayerOverlayProcessor::RenderPassOverlayData& root_render_pass_overlay_data = emplace_pair.first->second;

        root_render_pass_overlay_data.damage_rect = render_passes->back()->damage_rect;

        dc_layer_overlay_processor_->Process(resource_provider_.get(), render_pass_filters, render_pass_backdrop_filters,
        ...
        base::ranges::sort(root_render_pass_overlay_data.promoted_overlays, base::ranges::greater(), &OverlayCandidate::plane_z_order);
...
TEST_F(DCLayerOverlayProcessorTest, DisableVideoOverlayIfMovingWorkaround)
{
    ...
    auto ProcessForOverlaysSingleVideoRectWithOffset = [&](gfx::Vector2d video_rect_offset, bool is_hdr = false, bool is_sdr_to_hdr = false) {
        ...
        if (is_hdr) {
            ...
            hdr_metadata.smpte_st_2086 = gfx::HdrMetadataSmpteSt2086(SkNamedPrimariesExt::kRec2020, 1000, 0.0001);
            ...
            dc_layer_overlay_processor_->set_system_hdr_disabled_on_any_display_for_testing(false);

            // Device has video processor support.
            dc_layer_overlay_processor_->set_has_p010_video_processor_support_for_testing(true);
            ...
            dc_layer_overlay_processor_->set_is_on_battery_power_for_testing(false);

            // Device has at least one HDR-enabled display.
            dc_layer_overlay_processor_->set_system_hdr_enabled_on_any_display_for_testing(true);

            // Device has video processor auto hdr support.
            dc_layer_overlay_processor_->set_has_auto_hdr_video_processor_support_for_testing(true);
        }

        auto* video_quad = CreateFullscreenCandidateYUVTextureQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            pass->shared_quad_state_list.back(), pass.get(), color_space, hdr_metadata, format);
        ...
        auto overlay_data = ProcessRootPassForOverlays(&pass_list, render_pass_filters, render_pass_backdrop_filters, {});

        return std::move(overlay_data.promoted_overlays);
    };

    {
        dc_layer_overlay_processor_->set_disable_video_overlay_if_moving_for_testing(false);
    ...
    {
        dc_layer_overlay_processor_->set_disable_video_overlay_if_moving_for_testing(true);
...

    {
        ...
        EXPECT_EQ(1U, ProcessForOverlaysSingleVideoRectWithOffset({ 0, 0 }, /*is_hdr=*/false, /*is_sdr_to_hdr*/ true).size());
        ...
        // viz and GPU driver.
        EXPECT_EQ(1U, ProcessForOverlaysSingleVideoRectWithOffset({ 1, 0 }, /*is_hdr=*/false, /*is_sdr_to_hdr*/ true).size());
    }

    {
        ...
        // We expect an overlay promotion after a couple frames of no movement
        for (int i = 0; i < 10; i++) {
            ProcessForOverlaysSingleVideoRectWithOffset({ 0, 0 }, /*is_hdr=*/true).size();
        }
        EXPECT_EQ(1U, ProcessForOverlaysSingleVideoRectWithOffset({ 0, 0 }, /*is_hdr=*/true).size());
        ...
        EXPECT_EQ(1U, ProcessForOverlaysSingleVideoRectWithOffset({ 1, 0 }, /*is_hdr=*/true).size());
    }
}
...
TEST_F(DCLayerOverlayProcessorTest, Occluded)
{
    InitializeDCLayerOverlayProcessor();
    {
        ...
        CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), gfx::Rect(0, 3, 100, 100), SkColors::kWhite);

        SharedQuadState* second_shared_state = pass->CreateAndAppendSharedQuadState();
        ...
        first_video_quad->protected_video_type = gfx::ProtectedVideoType::kHardwareProtected;

        SharedQuadState* third_shared_state = pass->CreateAndAppendSharedQuadState();
        third_shared_state->overlay_damage_index = 2;
        auto* second_video_quad = CreateFullscreenCandidateYUVTextureQuad(
            resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get());
        // Set the protected video flag will force the quad to use hw overlay
        second_video_quad->protected_video_type = gfx::ProtectedVideoType::kHardwareProtected;
        ...
        AggregatedRenderPassList pass_list;
        pass_list.push_back(std::move(pass));
        SurfaceDamageRectList surface_damage_rect_list = { gfx::Rect(1, 1, 10, 10), gfx::Rect(0, 0, 0, 0), gfx::Rect(0, 0, 0, 0) };

        auto overlay_data = ProcessRootPassForOverlays(&pass_list, render_pass_filters, render_pass_backdrop_filters, std::move(surface_damage_rect_list));
    ...
    }
    {
        ...
        SharedQuadState* first_shared_state = pass->shared_quad_state_list.back();
        first_shared_state->overlay_damage_index = 0;
        CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), gfx::Rect(3, 3, 100, 100), SkColors::kWhite);
        ...
        auto* video_quad = CreateFullscreenCandidateYUVTextureQuad(
            resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get());
        // Set the protected video flag will force the quad to use hw overlay
        video_quad->protected_video_type = gfx::ProtectedVideoType::kHardwareProtected;
...
TEST_F(DCLayerOverlayProcessorTest, DamageRectWithoutVideoDamage)
{
    InitializeDCLayerOverlayProcessor();
    {
        ...
        CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), gfx::Rect(210, 210, 20, 20), SkColors::kWhite);
        ...
        AggregatedRenderPassList pass_list;
        pass_list.push_back(std::move(pass));
        SurfaceDamageRectList surface_damage_rect_list = { gfx::Rect(210, 210, 20, 20), gfx::Rect(0, 0, 0, 0) };
...
TEST_F(DCLayerOverlayProcessorTest, ClipRect)
{
    ...
    for (size_t i = 0; i < 2; ++i) {
        ...
        CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), gfx::Rect(0, 2, 100, 100), SkColors::kWhite);
        pass->shared_quad_state_list.back()->clip_rect = gfx::Rect(0, 3, 100, 100);

        SharedQuadState* shared_state = pass->CreateAndAppendSharedQuadState();
        shared_state->opacity = 1.f;
        shared_state->overlay_damage_index = 1;
        CreateFullscreenCandidateYUVTextureQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), shared_state, pass.get());
        ...
        AggregatedRenderPassList pass_list;
        pass_list.push_back(std::move(pass));
        SurfaceDamageRectList surface_damage_rect_list = { gfx::Rect(1, 3, 10, 8), gfx::Rect(1, 1, 10, 2) };
        ...
        // underlay.
        EXPECT_EQ(1, overlay_data.promoted_overlays.back().plane_z_order);
        EXPECT_EQ(gfx::Rect(0, 0, 100, 3), overlay_data.promoted_overlays.back().clip_rect);
        if (i == 1) {
...
TEST_F(DCLayerOverlayProcessorTest, UnderlayDamageRectWithQuadOnTopUnchanged)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        CreateSolidColorQuadAt(shared_state_on_top, SkColors::kRed, pass.get(), kOverlayBottomRightRect);
        ...
        SurfaceDamageRectList surface_damage_rect_list = { kOverlayBottomRightRect, kOverlayRect };
        if (i == 2) {
...
TEST_F(DCLayerOverlayProcessorTest, RoundedCorners)
{
    ...
    // Frame #0
    {
        ...
        pass->shared_quad_state_list.back()->overlay_damage_index = 0;
        // Rounded corners
        pass->shared_quad_state_list.back()->mask_filter_info = gfx::MaskFilterInfo(gfx::RRectF(gfx::RectF(0.f, 0.f, 20.f, 30.f), 5.f));
        ...
        EXPECT_EQ(SolidColorDrawQuad::MaterialCast(replaced_quad)->color, SkColors::kBlack);
    ...
    // Frame #1
    {
        auto pass = CreateRenderPass();
        // Create a solid quad.
        CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), gfx::Rect(0, 0, 32, 32), SkColors::kRed);
        ...
        AggregatedRenderPassList pass_list;
        pass_list.push_back(std::move(pass));
        SurfaceDamageRectList surface_damage_rect_list = { gfx::Rect(0, 0, 32, 32), gfx::Rect(0, 0, 256, 256) };
...
TEST_F(DCLayerOverlayProcessorTest, MultipleYUVOverlays)
{
    ...
    scoped_feature_list.InitAndEnableFeature(features::kNoUndamagedOverlayPromotion);
    {
        auto pass = CreateRenderPass();
        CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), gfx::Rect(0, 0, 256, 256), SkColors::kWhite);
...
TEST_F(DCLayerOverlayProcessorTest, PixelMovingForegroundFilter)
{
    ...
    filter_pass->SetNew(filter_render_pass_id, filter_rect, filter_rect, gfx::Transform());
    filter_pass->filters = blur_filter;

    // Add a solid quad to the non-root pass.
    SharedQuadState* shared_state_filter = filter_pass->CreateAndAppendSharedQuadState();
    CreateSolidColorQuadAt(shared_state_filter, SkColors::kRed, filter_pass.get(), filter_rect);
    ...
    CreateRenderPassDrawQuadAt(pass.get(), shared_quad_state_rpdq, filter_rect, filter_render_pass_id);
...
TEST_F(DCLayerOverlayProcessorTest, BackdropFilter)
{
    ...
    backdrop_filter_pass->SetNew(backdrop_filter_render_pass_id, backdrop_filter_rect, backdrop_filter_rect, gfx::Transform());
    backdrop_filter_pass->backdrop_filters = backdrop_filter;

    // Add a transparent solid quad to the non-root pass.
    SharedQuadState* shared_state_backdrop_filter = backdrop_filter_pass->CreateAndAppendSharedQuadState();
    CreateSolidColorQuadAt(shared_state_backdrop_filter, SkColors::kGreen, backdrop_filter_pass.get(), backdrop_filter_rect);
    ...
    CreateRenderPassDrawQuadAt(pass.get(), shared_quad_state_rpdq, backdrop_filter_rect, backdrop_filter_render_pass_id);
    ...
    OverlayProcessorInterface::FilterOperationsMap render_pass_filters;
    OverlayProcessorInterface::FilterOperationsMap render_pass_backdrop_filters;
    render_pass_backdrop_filters[backdrop_filter_render_pass_id] = &backdrop_filter;
    ...
    SurfaceDamageRectList surface_damage_rect_list = { backdrop_filter_rect, kOverlayRect };
...
TEST_F(DCLayerOverlayProcessorTest, VideoCaptureOnIsolatedRenderPass)
{
    ...
    // overlay.
    {
        ...
        // Create a solid quad.
        CreateOpaqueQuadAt(resource_provider_.get(), root_pass->shared_quad_state_list.back(), root_pass.get(), gfx::Rect(0, 0, 32, 32), SkColors::kRed);

        // Create a video YUV quad below the red solid quad.
        auto* video_quad = CreateFullscreenCandidateYUVTextureQuad(
            resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), root_pass->shared_quad_state_list.back(), root_pass.get());
        gfx::Rect rect(0, 0, 256, 256);
        video_quad->rect = rect;
        video_quad->visible_rect = rect;
...
void DCLayerOverlayProcessorTest::TestRenderPassRootTransform(bool is_overlay)
{
    ...
    const gfx::Transform kRenderPassToRootTransform = gfx::Transform::MakeTranslation(20, 45);
    // Surface damages in root space.
    const SurfaceDamageRectList kSurfaceDamageRectList = { // On top and does not intersect overlay. Translates to (110,5 20x10) in
        // render pass space.
    ...
    for (size_t frame = 0; frame < 3; frame++) {
        ...
        pass->shared_quad_state_list.back()->overlay_damage_index = kOverlayDamageIndex;

        if (!is_overlay) {
            // Create a quad that occludes the video to force it to an underlay.
            CreateOpaqueQuadAt(resource_provider_.get(), pass->shared_quad_state_list.back(), pass.get(), kOpaqueRect, SkColors::kWhite);
        }
        ...
        auto overlay_data = ProcessRootPassForOverlays(&pass_list, render_pass_filters, render_pass_backdrop_filters, std::move(surface_damage_rect_list));
        LOG(INFO) << "frame " << frame << " damage rect: " << overlay_data.damage_rect.ToString();

        EXPECT_EQ(overlay_data.promoted_overlays.size(), 1u);
        EXPECT_TRUE(absl::holds_alternative<gfx::Transform>(overlay_data.promoted_overlays[0].transform));
        EXPECT_EQ(absl::get<gfx::Transform>(overlay_data.promoted_overlays[0].transform), kRenderPassToRootTransform);
        if (is_overlay) {
...
TEST_F(DCLayerOverlayProcessorTest, MultipleRenderPassesOneOverlay)
{
    ...
    for (size_t frame = 0; frame < 3; frame++) {
        ...
        for (size_t id = 1; id <= num_render_passes; id++) {
            ...
            gfx::Rect quad_rect_in_root_space = gfx::Rect(0, 0, id * 16, pass->output_rect.height());

            if (id == 1) {
                // Create an overlay quad in the first render pass.
                auto* video_quad = CreateFullscreenCandidateYUVTextureQuad(
                    resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get());
                gfx::Rect quad_rect_in_quad_space = pass->transform_to_root_target.InverseMapRect(quad_rect_in_root_space).value();
                ...
                CreateSolidColorQuadAt(pass->shared_quad_state_list.back(), SkColors::kBlue, pass.get(),
                    pass->transform_to_root_target.InverseMapRect(quad_rect_in_root_space).value());
            }
        ...
        dc_layer_overlay_processor_->Process(resource_provider_.get(), render_pass_filters, render_pass_backdrop_filters, std::move(surface_damage_rect_list),
            /*is_page_fullscreen_mode=*/false, render_pass_overlay_data_map);

        for (auto& [render_pass, overlay_data] : render_pass_overlay_data_map) {
            LOG(INFO) << "frame " << frame << " render pass " << render_pass->id << " damage rect : " << overlay_data.damage_rect.ToString();
            LOG(INFO) << "frame " << frame << " render pass " << render_pass->id << " number of overlays: " << overlay_data.promoted_overlays.size();

            if (render_pass->id == AggregatedRenderPassId(1)) {
                // The render pass that contains an overlay.
                EXPECT_EQ(overlay_data.promoted_overlays.size(), 1u);
                EXPECT_EQ(absl::get<gfx::Transform>(overlay_data.promoted_overlays[0].transform), gfx::Transform::MakeTranslation(1, 0));
                EXPECT_GT(overlay_data.promoted_overlays[0].plane_z_order, 0);
                ...
                // x translation of -1.
                EXPECT_EQ(overlay_data.promoted_overlays[0].display_rect, gfx::RectF(-1, 0, 16, 256));
...
TEST_F(DCLayerOverlayProcessorTest, MultipleRenderPassesExceedsOverlayAllowance)
{
    const gfx::Rect output_rect = { 0, 0, 256, 256 };
    const size_t num_render_passes = 3;
    ...
    for (size_t frame = 1; frame <= 3; frame++) {
        ...
        for (size_t id = frame; id < num_render_passes + frame; id++) {
            ...
            gfx::Rect video_rect_in_root_space = gfx::Rect(0, 0, id * 16, pass->output_rect.height());

            gfx::Rect video_rect_in_render_pass_space = pass->transform_to_root_target.InverseMapRect(video_rect_in_root_space).value();
            auto* video_quad = CreateFullscreenCandidateYUVTextureQuad(
                resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->shared_quad_state_list.back(), pass.get());
        ...
        EXPECT_EQ(3U, dc_layer_overlay_processor_->get_previous_frame_render_pass_count());
        std::vector<AggregatedRenderPassId> previous_frame_render_pass_ids = dc_layer_overlay_processor_->get_previous_frame_render_pass_ids();
        std::sort(previous_frame_render_pass_ids.begin(), previous_frame_render_pass_ids.end());
        for (size_t id = frame; id < num_render_passes + frame; id++) {
    ...
    scoped_feature_list.InitAndEnableFeature(features::kNoUndamagedOverlayPromotion);
    {
        ...
        for (auto& dc : overlay_data.promoted_overlays) {
            if (dc.plane_z_order > 0) {
                ...
                EXPECT_EQ(gfx::Rect(150, 150, 50, 50), gfx::ToEnclosingRect(dc.display_rect));
                overlay_cnt++;
            }
        }
...
TEST_F(DCLayerOverlayProcessorTest, HDR10VideoOverlay)
{
    ...
    valid_hdr_metadata.smpte_st_2086 = gfx::HdrMetadataSmpteSt2086(SkNamedPrimariesExt::kRec2020, 1000, 0.0001);
    ...
    // Frame 1 should promote overlay as all conditions satisfied.
    {
        ...
        CreateFullscreenCandidateYUVTextureQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            pass->shared_quad_state_list.back(), pass.get(), gfx::ColorSpace::CreateHDR10(), valid_hdr_metadata, MultiPlaneFormat::kP010);
    ...
    // Frame 2 should skip overlay as bit depth not satisfied.
    {
        ...
        CreateFullscreenCandidateYUVTextureQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            pass->shared_quad_state_list.back(), pass.get(), gfx::ColorSpace::CreateHDR10(), valid_hdr_metadata, MultiPlaneFormat::kNV12);
    ...
    // Frame 3 should promote overlay even if hdr metadata is invalid.
    {
        ...
        CreateFullscreenCandidateYUVTextureQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            pass->shared_quad_state_list.back(), pass.get(), gfx::ColorSpace::CreateHDR10(), gfx::HDRMetadata(), MultiPlaneFormat::kP010);
    ...
    // Frame 4 should promote overlay as hdr metadata contains cta_861_3.
    {
        ...
        // Content is 10bit P010 content with HDR10 colorspace.
        CreateFullscreenCandidateYUVTextureQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            pass->shared_quad_state_list.back(), pass.get(), gfx::ColorSpace::CreateHDR10(), cta_861_3_hdr_metadata, MultiPlaneFormat::kP010);
    ...
    // Frame 5 should promote overlay as hdr metadata contains smpte_st_2086.
    {
        ...
        smpte_st_2086_hdr_metadata.smpte_st_2086 = gfx::HdrMetadataSmpteSt2086(SkNamedPrimariesExt::kRec2020, 1000, 0.0001);
        // Content is 10bit P010 content with HDR10 colorspace.
        CreateFullscreenCandidateYUVTextureQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            pass->shared_quad_state_list.back(), pass.get(), gfx::ColorSpace::CreateHDR10(), smpte_st_2086_hdr_metadata, MultiPlaneFormat::kP010);
    ...
    // Frame 6 should skip overlay as color space not satisfied.
    {
        ...
        // (not satisfied).
        CreateFullscreenCandidateYUVTextureQuad(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            pass->shared_quad_state_list.back(), pass.get(), gfx::ColorSpace::CreateHLG(), valid_hdr_metadata, MultiPlaneFormat::kP010);
    ...
    {
        dc_layer_overlay_processor_->set_has_p010_video_processor_support_for_testing(false);
    ...
    {
        dc_layer_overlay_processor_->set_system_hdr_disabled_on_any_display_for_testing(true);
...
// tests.
testing::Matcher<const OverlayCandidateList&> WhenCandidatesAreSortedResourceIdsAre(const std::vector<int>& expected_resource_ids)
{
    return testing::AllOf(
        PlaneZOrdersAreUnique(), testing::WhenSortedBy(test::PlaneZOrderAscendingComparator(), testing::Pointwise(ResourceIdEq(), expected_resource_ids)));
}
...
TEST_F(OverlayProcessorWinStaticTest, InsertSurfaceContentOverlay)
{
    ...
    DCLayerOverlayProcessor::RenderPassOverlayDataMap surface_content_render_passes;
    ...
    std::ignore = OverlayProcessorWin::InsertSurfaceContentOverlaysAndSetPlaneZOrderForTesting(std::move(surface_content_render_passes), candidates);
...
TEST_F(OverlayProcessorWinStaticTest, InsertSurfaceContentOverlaysWithGapsInZOrder)
{
    // Set up a dummy render pass and RPDQ
    AggregatedRenderPass pass;
    pass.id = AggregatedRenderPassId(1);
    ...
    EXPECT_THAT(candidates, WhenCandidatesAreSortedResourceIdsAre({ 1, 2, 3, 4, 5 }));
}

TEST_F(OverlayProcessorWinStaticTest, InsertSurfaceContentOverlaysWithNoPromotedOverlays)
{
    // Set up a dummy render pass and RPDQ
    AggregatedRenderPass pass;
    pass.id = AggregatedRenderPassId(1);
...
TEST_F(OverlayProcessorWinStaticTest, InsertSurfaceContentOverlaysWithUnderlays)
{
    // Set up a dummy render pass and RPDQ
    AggregatedRenderPass pass;
    pass.id = AggregatedRenderPassId(1);
    ...
    EXPECT_THAT(candidates, WhenCandidatesAreSortedResourceIdsAre({ 1, 2, 3, 4, 5, 6, 7, 8 }));
}

TEST_F(OverlayProcessorWinStaticTest, InsertSurfaceContentOverlaysMultipleSurfaces)
{
...
TEST_F(OverlayProcessorWinStaticTest, InsertSurfaceContentOverlaysSameSurfaceEmbeddedTwice)
{
    // Set up a dummy render pass and RPDQ
    AggregatedRenderPass pass;
    pass.id = AggregatedRenderPassId(1);
    ...
    EXPECT_THAT(candidates,
        WhenCandidatesAreSortedResourceIdsAre({ 1, 2, 3, 4,
...
public:
    explicit TestOverlayProcessorWin(int allowed_yuv_overlay_count, bool disable_video_overlay_if_moving)
        : OverlayProcessorWin(OutputSurface::DCSupportLevel::kDCompTexture, &debug_settings_,
            std::make_unique<DCLayerOverlayProcessor>(allowed_yuv_overlay_count, disable_video_overlay_if_moving,
                /*skip_initialization_for_testing=*/true))
    {
    }
...
protected:
    void SetUp() override
    {
        ...
        output_surface_plane_ = OverlayProcessorInterface::OutputSurfaceOverlayPlane();
    }
    ...
    OverlayProcessorInterface::OutputSurfaceOverlayPlane* GetOutputSurfacePlane()
    {
        EXPECT_TRUE(output_surface_plane_.has_value());
        return &output_surface_plane_.value();
    }

    std::optional<OverlayProcessorInterface::OutputSurfaceOverlayPlane> output_surface_plane_;
...
class OverlayProcessorWinSurfacePlaneTest : public OverlayProcessorWinTest, public testing::WithParamInterface<SurfaceTestMode> {
public:
    static std::string GetParamName(const testing::TestParamInfo<ParamType>& info)
    {
        switch (info.param) {
...
protected:
    OverlayProcessorWinSurfacePlaneTest()
    {
        ...
        case SurfaceTestMode::RootSurface:
            ...
            break;
        case SurfaceTestMode::SimulatePartiallyDelegated:
            feature_list_.InitAndEnableFeatureWithParameters(features::kDelegatedCompositing, { { "mode", "limit_to_ui" } });
            break;
        }
    }

    void ProcessForOverlays(AggregatedRenderPassList* render_passes, const OverlayProcessorInterface::FilterOperationsMap& render_pass_filters,
        const OverlayProcessorInterface::FilterOperationsMap& render_pass_backdrop_filters, SurfaceDamageRectList surface_damage_rect_list_in_root_space,
        OverlayCandidateList* candidates)
    {
        ...
        public:
            ScopedSimulateUnmergedWebContentsSurface(AggregatedRenderPassList* pass_list, OverlayCandidateList* candidates, gfx::Rect* damage_rect)
                ...
                , damage_rect_(damage_rect)
            {
                ...
                const AggregatedRenderPassId max_pass_id = base::ranges::max_element(*pass_list_, base::ranges::less(), &AggregatedRenderPass::id)->get()->id;
                const AggregatedRenderPassId unused_pass_id(max_pass_id.value() + 1);

                auto pass = std::make_unique<AggregatedRenderPass>();
                pass->SetNew(unused_pass_id, pass_list_->back()->output_rect, pass_list_->back()->damage_rect, gfx::Transform());

                auto* shared_quad_state = pass->CreateAndAppendSharedQuadState();
                shared_quad_state->SetAll(
                    /*transform=*/gfx::Transform(),
                    /*layer_rect=*/pass_list_->back()->output_rect,
                    /*visible_layer_rect=*/pass_list_->back()->output_rect, gfx::MaskFilterInfo(),
                    /*clip=*/std::nullopt, /*contents_opaque=*/false,
                    /*opacity_f=*/1.f, SkBlendMode::kSrc, /*sorting_context=*/0,
                    /*layer_id=*/0u,
                    /*fast_rounded_corner=*/false);

                auto* quad = pass->CreateAndAppendDrawQuad<AggregatedRenderPassDrawQuad>();
                quad->SetNew(shared_quad_state, /*rect=*/pass_list_->back()->output_rect,
                    /*visible_rect=*/pass_list_->back()->output_rect, pass_list_->back()->id,
            ...
            ~ScopedSimulateUnmergedWebContentsSurface()
            {
                auto it = base::ranges::find_if(
                    *candidates_, [](const auto& candidate) { return candidate.rpdq && candidate.rpdq->render_pass_id == kDefaultRootPassId; });
        ...
        std::optional<ScopedSimulateUnmergedWebContentsSurface> simulate_unmerged_web_contents_surface;
        if (GetParam() == SurfaceTestMode::SimulatePartiallyDelegated) {
            simulate_unmerged_web_contents_surface.emplace(render_passes, candidates, &damage_rect_);
        }

        output_surface_plane_ = OverlayProcessorInterface::OutputSurfaceOverlayPlane();
        overlay_processor_->ProcessForOverlays(resource_provider_.get(), render_passes, SkM44(), render_pass_filters, render_pass_backdrop_filters,
            std::move(surface_damage_rect_list_in_root_space), &output_surface_plane_.value(), candidates, &damage_rect_, &content_bounds_);
        ...
        base::ranges::sort(*candidates, base::ranges::greater(), &OverlayCandidate::plane_z_order);
    }
...
TEST_P(OverlayProcessorWinSurfacePlaneTest, PromoteOverlayFromSurface)
{
    AggregatedRenderPassList pass_list;
    auto pass = CreateRenderPass();
    CreateTextureQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), pass->CreateAndAppendSharedQuadState(), pass.get(),
        gfx::Rect(0, 0, 50, 50),
    ...
    OverlayProcessorInterface::FilterOperationsMap render_pass_filters;
    OverlayProcessorInterface::FilterOperationsMap render_pass_backdrop_filters;
    ProcessForOverlays(&pass_list, render_pass_filters, render_pass_backdrop_filters, SurfaceDamageRectList(), &dc_layer_list);
...
TEST_P(OverlayProcessorWinSurfacePlaneTest, ForceSwapChainForCapture)
{
    ...
    // succeeds.
    {
        ...
        pass_list.back()->copy_requests.push_back(CopyOutputRequest::CreateStubForTesting());
...
TEST_P(OverlayProcessorWinSurfacePlaneTest, UseDCompSurfaceWithVideo)
{
    ...
    for (int i = 0; i < 60; i++) {
        ...
        const gfx::Rect expected_damage = (i == 0) ? pass_list.back()->output_rect : gfx::Rect();
        ...
        if (GetParam() == SurfaceTestMode::RootSurface) {
            ...
            EXPECT_EQ(output_surface_plane_->enable_blending, pass_list.back()->has_transparent_background);
        } else {
    ...
    for (int i = 0; i < 65; i++) {
        ...
        quad->SetNew(pass->CreateAndAppendSharedQuadState(), damage_rect_, damage_rect_, SkColors::kRed, false);
        ...
        const gfx::Rect expected_damage = (i == 0 || (i + 1) == 60) ? pass_list.back()->output_rect : damage_rect_;

        const bool in_dc_layer_hysteresis = i + 1 < 60;

        ProcessForOverlays(&pass_list, render_pass_filters, render_pass_backdrop_filters, SurfaceDamageRectList(), &dc_layer_list);

        EXPECT_EQ(pass_list.back()->needs_synchronous_dcomp_commit, in_dc_layer_hysteresis);
        EXPECT_EQ(pass_list.back()->has_transparent_background, in_dc_layer_hysteresis);
        if (GetParam() == SurfaceTestMode::RootSurface) {
            EXPECT_TRUE(output_surface_plane_);
            EXPECT_EQ(output_surface_plane_->enable_blending, pass_list.back()->has_transparent_background);
        } else {
...
INSTANTIATE_TEST_SUITE_P(, OverlayProcessorWinSurfacePlaneTest, testing::Values(SurfaceTestMode::RootSurface, SurfaceTestMode::SimulatePartiallyDelegated),
    &OverlayProcessorWinSurfacePlaneTest::GetParamName);

class OverlayProcessorWinDelegatedCompositingTest : public OverlayProcessorWinTest {
protected:
    OverlayProcessorWinDelegatedCompositingTest()
    {
        feature_list_.InitAndEnableFeatureWithParameters(features::kDelegatedCompositing, { { "mode", "full" } });
    }

    class DelegationResult {
    public:
        DelegationResult(OverlayCandidateList candidates, bool delegation_succeeded, gfx::Rect original_root_surface_damage, gfx::Rect root_surface_damage)
        ...
        const OverlayCandidateList& candidates() const
        {
            return candidates_;
        }
    ...
    DelegationResult TryProcessForDelegatedOverlays(AggregatedRenderPassList& pass_list, SurfaceDamageRectList surface_damage_rect_list = {})
    {
        ...
        const gfx::Rect original_root_surface_damage = pass_list.back()->damage_rect;
        ...
        overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters,
            render_pass_backdrop_filters, std::move(surface_damage_rect_list), GetOutputSurfacePlane(), &candidates, &damage_rect_, &content_bounds_);

        overlay_processor_->AdjustOutputSurfaceOverlay(&output_surface_plane_);
        const bool delegation_succeeded = !output_surface_plane_.has_value();

        return DelegationResult(candidates, delegation_succeeded, original_root_surface_damage, damage_rect_);
    }

    testing::Matcher<const OverlayCandidateList&> WhenCandidatesAreSortedElementsAre(std::vector<testing::Matcher<const OverlayCandidate&>> element_matchers)
    {
        return testing::AllOf(
            PlaneZOrdersAreUnique(), testing::WhenSortedBy(test::PlaneZOrderAscendingComparator(), testing::ElementsAreArray(element_matchers)));
    }
...
TEST_F(OverlayProcessorWinDelegatedCompositingTest, SingleQuad)
{
    ...
    auto pass = CreateRenderPass();
    CreateSolidColorQuadAt(pass->CreateAndAppendSharedQuadState(), SkColors::kRed, pass.get(), gfx::Rect(0, 0, 50, 50));
    pass_list.push_back(std::move(pass));
...
TEST_F(OverlayProcessorWinDelegatedCompositingTest, TooManyQuads)
{
    ...
    for (int i = 0; i < 2049; i++) {
        CreateSolidColorQuadAt(sqs, SkColors::kRed, pass.get(), gfx::Rect(i, 0, 50, 50));
    }
    pass_list.push_back(std::move(pass));
...
TEST_F(OverlayProcessorWinDelegatedCompositingTest, TooManyComplexQuads)
{
    ...
    auto pass = CreateRenderPass();
    auto* sqs = pass->CreateAndAppendSharedQuadState();
    sqs->mask_filter_info = gfx::MaskFilterInfo(gfx::RRectF(gfx::RectF(0, 0, 50, 50), 50));
    for (int i = 0; i < 257; i++) {
        CreateSolidColorQuadAt(sqs, SkColors::kRed, pass.get(), gfx::Rect(i, 0, 50, 50));
    }
    pass_list.push_back(std::move(pass));
...
// videos to overlay.
TEST_F(OverlayProcessorWinDelegatedCompositingTest, DelegationFailStillPromotesVideos)
{
    AggregatedRenderPassList pass_list;
...

    {
        ...
        child_pass->backdrop_filters.Append(cc::FilterOperation::CreateBlurFilter(1.f));
        CreateRenderPassDrawQuadAt(pass.get(), pass->shared_quad_state_list.back(), gfx::Rect(0, 0, 50, 50), child_pass->id);
        pass_list.push_back(std::move(child_pass));

        CreateSolidColorQuadAt(pass->shared_quad_state_list.back(), SkColors::kGreen, pass.get(), gfx::Rect(0, 0, 50, 50));
    }
...
TEST_F(OverlayProcessorWinDelegatedCompositingTest, SkipNonVisibleOverlays)
{
    ...
    auto pass = CreateRenderPass();
    CreateSolidColorQuadAt(pass->CreateAndAppendSharedQuadState(), SkColors::kRed, pass.get(), gfx::Rect(0, 0, 0, 0));
    pass_list.push_back(std::move(pass));
...
TEST_F(OverlayProcessorWinDelegatedCompositingTest, OccludedByFilteredQuadNotSupported)
{
    AggregatedRenderPassList pass_list;
...

    {
        ...
        CreateRenderPassDrawQuadAt(pass.get(), pass->CreateAndAppendSharedQuadState(), rect, child_pass_id);

        // Create a quad that will be occluded by the backdrop-filtered RPDQ above.
        CreateSolidColorQuadAt(pass->CreateAndAppendSharedQuadState(), SkColors::kRed, pass.get(), rect);
...
TEST_F(OverlayProcessorWinDelegatedCompositingTest, BackingWillBeReadInViz)
{
    ...
    auto CreateNamedPass = [&](const char* name, bool embed_in_root, base::OnceCallback<void(AggregatedRenderPass*)> update_pass) {
    ...
    CreateNamedPass("video capture enabled", true, base::BindOnce([](AggregatedRenderPass* pass) { pass->video_capture_enabled = true; }));

    CreateNamedPass("filters", true, base::BindOnce([](AggregatedRenderPass* pass) {
        pass->filters = cc::FilterOperations({
            cc::FilterOperation::CreateGrayscaleFilter(1.0f),
        });
    }));

    CreateNamedPass("generate mipmaps", true, base::BindOnce([](AggregatedRenderPass* pass) { pass->generate_mipmap = true; }));

    auto non_overlay_embeddee_id = CreateNamedPass("normal pass with non-overlay embedder", true, base::DoNothing());
    CreateNamedPass("non-overlay embedder", false, base::BindLambdaForTesting([&](AggregatedRenderPass* pass) {
        CreateRenderPassDrawQuadAt(pass, pass->CreateAndAppendSharedQuadState(), pass->output_rect, non_overlay_embeddee_id);
    }));

    auto complex_mask_embeddee_id = CreateNamedPass("normal pass with gradient mask embedder", true, base::DoNothing());
    CreateNamedPass("gradient mask embedder", false, base::BindLambdaForTesting([&](AggregatedRenderPass* pass) {
        auto* sqs = pass->CreateAndAppendSharedQuadState();
        CreateRenderPassDrawQuadAt(pass, sqs, pass->output_rect, complex_mask_embeddee_id);
        ...
        sqs->mask_filter_info = gfx::MaskFilterInfo(gfx::RRectF(gfx::RectF(pass->output_rect)), gradient);
    }));

    CreateNamedPass("root pass", false, base::BindLambdaForTesting([&](AggregatedRenderPass* pass) {
        for (auto id : passes_to_embed_in_root) {
            CreateRenderPassDrawQuadAt(pass, pass->CreateAndAppendSharedQuadState(), pass->output_rect, id);
        }
    }));
    ...
    for (size_t i = 0u; i < pass_list.size(); i++) {
        SCOPED_TRACE(base::StringPrintf("pass_list[%zu]: %s", i, pass_names[pass_list[i]->id]));
...
class OverlayProcessorWinPartiallyDelegatedCompositingTest : public OverlayProcessorWinDelegatedCompositingTest {
protected:
    ...
    TextureDrawQuad* CreateOverlayQuadWithSurfaceDamageAt(AggregatedRenderPass* pass, SurfaceDamageRectList& surface_damage_rect_list, const gfx::Rect& rect)
    {
        SharedQuadState* sqs = pass->CreateAndAppendSharedQuadState();
        auto* quad = CreateTextureQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, pass, rect,
            /*is_overlay_candidate=*/true);

        pass->damage_rect.Union(sqs->quad_to_target_transform.MapRect(quad->visible_rect));
        ...
        surface_damage_rect_list.push_back(quad_to_root_transform.MapRect(quad->visible_rect));
...
TEST_F(OverlayProcessorWinPartiallyDelegatedCompositingTest, CandidatePromotedFromNonRootSurface)
{
    AggregatedRenderPassList pass_list;
    ...
    // Create a pass with just an overlay quad.
    {
        ...
        auto* texture_quad = CreateTextureQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            child_pass->CreateAndAppendSharedQuadState(), child_pass.get(), gfx::Rect(0, 0, 50, 50),
            /*is_overlay_candidate=*/true);
    ...
    {
        auto pass = CreateRenderPass();
        CreateRenderPassDrawQuadAt(pass.get(), pass->CreateAndAppendSharedQuadState(), gfx::Rect(0, 0, 50, 50), child_pass_id);
        pass_list.push_back(std::move(pass));
    }
...
TEST_F(OverlayProcessorWinPartiallyDelegatedCompositingTest, CandidatePromotedFromNonRootSurfaceAsUnderlay)
{
    AggregatedRenderPassList pass_list;
    ...
    // This forces the overlay candidate to appear as an underlay to the surface.
    {
        auto child_pass = CreateRenderPass(child_pass_id);
        child_pass->is_from_surface_root_pass = true;
        CreateSolidColorQuadAt(child_pass->CreateAndAppendSharedQuadState(), SkColors::kRed, child_pass.get(), gfx::Rect(5, 5, 10, 10));
        auto* texture_quad = CreateTextureQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            child_pass->CreateAndAppendSharedQuadState(), child_pass.get(), gfx::Rect(0, 0, 50, 50),
            /*is_overlay_candidate=*/true);
        child_pass_texture_id = texture_quad->resource_id();
...

    {
        auto pass = CreateRenderPass();
        CreateRenderPassDrawQuadAt(pass.get(), pass->CreateAndAppendSharedQuadState(), gfx::Rect(0, 0, 50, 50), child_pass_id);
        CreateSolidColorQuadAt(pass->CreateAndAppendSharedQuadState(), SkColors::kBlue, pass.get(), pass->output_rect);
        pass_list.push_back(std::move(pass));
    }
...
TEST_F(OverlayProcessorWinPartiallyDelegatedCompositingTest, CandidatesPromotedFromMultipleSurfaces)
{
    AggregatedRenderPassList pass_list;
    ...
    ResourceId other_child_pass_video_2_id;
    {
        ...
        auto* texture_quad = CreateTextureQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            other_child_pass->CreateAndAppendSharedQuadState(), other_child_pass.get(), gfx::Rect(10, 0, 50, 50),
            /*is_overlay_candidate=*/true);
        other_child_pass_video_id = texture_quad->resource_id();
        auto* texture_quad_2 = CreateTextureQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
            other_child_pass->CreateAndAppendSharedQuadState(), other_child_pass.get(), gfx::Rect(0, 0, 50, 50),
            /*is_overlay_candidate=*/true);
...

    {
        ...
        CreateRenderPassDrawQuadAt(pass.get(), pass->CreateAndAppendSharedQuadState(), gfx::Rect(0, 0, 50, 50), child_pass_id);
        CreateSolidColorQuadAt(pass->CreateAndAppendSharedQuadState(), SkColors::kBlue, pass.get(), pass->output_rect);
        CreateRenderPassDrawQuadAt(pass.get(), pass->CreateAndAppendSharedQuadState(), gfx::Rect(50, 0, 50, 50), other_child_pass_id);
        pass_list.push_back(std::move(pass));
    }
...
TEST_F(OverlayProcessorWinPartiallyDelegatedCompositingTest, CandidatePromotionRespectsAllowedYuvOverlayCount)
{
    AggregatedRenderPassList pass_list;
...
TEST_F(OverlayProcessorWinPartiallyDelegatedCompositingTest, CandidatesInheritSurfaceEmbeddersBounds)
{
    AggregatedRenderPassList pass_list;
    ...
    EXPECT_THAT(result.candidates(),
        WhenCandidatesAreSortedElementsAre({
            test::IsRenderPassOverlay(child_pass_id),
            testing::AllOf(test::OverlayHasResource(child_pass_texture_id), test::OverlayHasClip(expected_overlay_clip)),
        }));
}

TEST_F(OverlayProcessorWinPartiallyDelegatedCompositingTest, CandidatesInheritSurfaceEmbeddersClip)
{
    AggregatedRenderPassList pass_list;
    ...
    EXPECT_THAT(result.candidates(),
        WhenCandidatesAreSortedElementsAre({
            testing::AllOf(test::IsRenderPassOverlay(child_pass_id), test::OverlayHasClip(rpdq_clip_rect)),
            testing::AllOf(test::OverlayHasResource(child_pass_texture_id), test::OverlayHasClip(expected_overlay_clip)),
        }));
}

TEST_F(OverlayProcessorWinPartiallyDelegatedCompositingTest, CandidatesInheritSurfaceEmbeddersClipAndIntersect)
{
    AggregatedRenderPassList pass_list;
...

    {
        ...
        auto* texture_quad = CreateTextureQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, child_pass.get(),
            gfx::Rect(0, 0, 50, 50),
            /*is_overlay_candidate=*/true);
        child_pass_texture_id = texture_quad->resource_id();
        pass_list.push_back(std::move(child_pass));
...

    {
        ...
        expected_overlay_clip = sqs->quad_to_target_transform.MapRect(inner_quad_clip);
...
TEST_F(OverlayProcessorWinPartiallyDelegatedCompositingTest, CandidatesInheritSurfaceEmbeddersClipAndRoundedCorners)
{
    AggregatedRenderPassList pass_list;
...

    {
        ...
        // surface.
        sqs->mask_filter_info = gfx::MaskFilterInfo(gfx::RRectF(gfx::RectF(10, 10), 1));
        auto* texture_quad = CreateTextureQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), sqs, child_pass.get(),
            gfx::Rect(0, 0, 50, 50),
            /*is_overlay_candidate=*/true);
        child_pass_texture_id = texture_quad->resource_id();
...

    {
        ...
        CreateRenderPassDrawQuadAt(pass.get(), sqs, gfx::Rect(0, 0, 50, 50), child_pass_id);
    ...
    EXPECT_THAT(result.candidates(),
        WhenCandidatesAreSortedElementsAre({
            testing::AllOf(test::OverlayHasResource(child_pass_texture_id), test::OverlayHasClip(gfx::Rect(0, 0, 50, 50)),
                test::OverlayHasRoundedCorners(rpdq_rounded_corners)),
            testing::AllOf(test::IsRenderPassOverlay(child_pass_id), test::OverlayHasRoundedCorners(rpdq_rounded_corners)),
        }));
}

TEST_F(OverlayProcessorWinPartiallyDelegatedCompositingTest, DamageRemovedFromSurface)
{
    const AggregatedRenderPassId child_pass_id { 2 };
    ...
    for (int frame = 0; frame < 3; frame++) {
        ...
        auto child_pass = CreateRenderPass(child_pass_id);
        child_pass->transform_to_root_target = gfx::Transform::MakeTranslation(rpdq_quad.OffsetFromOrigin());
        child_pass->is_from_surface_root_pass = true;
        child_pass->damage_rect = gfx::Rect();
        auto* texture_quad = is_overlay_candidate_with_damage
            ? CreateOverlayQuadWithSurfaceDamageAt(child_pass.get(), surface_damage_rect_list, texture_quad_rect)
            : CreateTextureQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), child_pass->CreateAndAppendSharedQuadState(),
        ...
        auto pass = CreateRenderPass();
        CreateRenderPassDrawQuadAt(pass.get(), pass->CreateAndAppendSharedQuadState(), rpdq_quad, child_pass_id);
        pass_list.push_back(std::move(pass));

        switch (frame) {
        case 0:
        case 1:
            EXPECT_EQ(pass_list[0]->damage_rect, texture_quad_rect) << "The quad in the child surface contributes damage";
            break;

        case 2:
            EXPECT_EQ(pass_list[0]->damage_rect, gfx::Rect()) << "No damage on the child surface before overlay processing";
            break;
        }

        auto result = TryProcessForDelegatedOverlays(pass_list, std::move(surface_damage_rect_list));
        result.ExpectDelegationSuccess();

        switch (frame) {
        case 0:
            EXPECT_EQ(pass_list[0]->damage_rect, pass_list[0]->output_rect) << "Full damage is forced on the first frame";
            EXPECT_THAT(result.candidates(),
                WhenCandidatesAreSortedElementsAre({
                    test::IsRenderPassOverlay(child_pass_id),
                    test::OverlayHasResource(child_pass_texture_id),
        ...
        case 1:
            EXPECT_EQ(pass_list[0]->damage_rect, gfx::Rect()) << "Damage is removed when only from overlays";
            EXPECT_THAT(result.candidates(),
                WhenCandidatesAreSortedElementsAre({
                    test::IsRenderPassOverlay(child_pass_id),
                    test::OverlayHasResource(child_pass_texture_id),
        ...
        case 2:
            EXPECT_EQ(pass_list[0]->damage_rect, texture_quad_rect) << "Damage removed in frame 1 is re-added";
            EXPECT_THAT(result.candidates(),
                WhenCandidatesAreSortedElementsAre({
                    test::IsRenderPassOverlay(child_pass_id),
                }));
...
TEST_F(OverlayProcessorWinPartiallyDelegatedCompositingTest, DamageRemovedFromMultipleSurfaces)
{
    ...
    for (int frame = 0; frame < 4; frame++) {
        ...
        left_child_pass->transform_to_root_target = gfx::Transform::MakeTranslation(left_rpdq_quad.OffsetFromOrigin());
        ...
        auto* left_texture_quad = is_left_overlay_candidate_with_damage
            ? CreateOverlayQuadWithSurfaceDamageAt(left_child_pass.get(), surface_damage_rect_list, left_texture_quad_rect)
            : CreateTextureQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
                left_child_pass->CreateAndAppendSharedQuadState(), left_child_pass.get(), left_texture_quad_rect,
                /*is_overlay_candidate=*/false);
        ...
        right_child_pass->transform_to_root_target = gfx::Transform::MakeTranslation(right_rpdq_quad.OffsetFromOrigin());
        ...
        auto* right_texture_quad = is_right_overlay_candidate_with_damage
            ? CreateOverlayQuadWithSurfaceDamageAt(right_child_pass.get(), surface_damage_rect_list, right_texture_quad_rect)
            : CreateTextureQuadAt(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(),
                right_child_pass->CreateAndAppendSharedQuadState(), right_child_pass.get(), right_texture_quad_rect,
                /*is_overlay_candidate=*/false);
        ResourceId right_child_pass_texture_id = right_texture_quad->resource_id();
        pass_list.push_back(std::move(right_child_pass));

        CreateRenderPassDrawQuadAt(pass.get(), pass->CreateAndAppendSharedQuadState(), left_rpdq_quad, left_child_pass_id);
        CreateRenderPassDrawQuadAt(pass.get(), pass->CreateAndAppendSharedQuadState(), right_rpdq_quad, right_child_pass_id);
        pass_list.push_back(std::move(pass));

        switch (frame) {
        case 0:
        case 1:
            EXPECT_EQ(pass_list[0]->damage_rect, left_texture_quad_rect) << "The quad in the child surface contributes damage";
            EXPECT_EQ(pass_list[1]->damage_rect, right_texture_quad_rect) << "The quad in the child surface contributes damage";
            break;

        case 2:
            EXPECT_EQ(pass_list[0]->damage_rect, left_texture_quad_rect) << "The quad in the child surface contributes damage";
            EXPECT_EQ(pass_list[1]->damage_rect, gfx::Rect()) << "No damage on the child surface before overlay processing";
            break;
        ...
        case 0:
            EXPECT_EQ(pass_list[0]->damage_rect, pass_list[0]->output_rect) << "Full damage is forced on the first frame";
            EXPECT_EQ(pass_list[1]->damage_rect, pass_list[1]->output_rect) << "Full damage is forced on the first frame";
            EXPECT_THAT(result.candidates(),
                WhenCandidatesAreSortedElementsAre({
        ...
        case 1:
            EXPECT_EQ(pass_list[0]->damage_rect, gfx::Rect()) << "Damage is removed when only from overlays";
            EXPECT_EQ(pass_list[1]->damage_rect, gfx::Rect()) << "Damage is removed when only from overlays";
            EXPECT_THAT(result.candidates(),
                WhenCandidatesAreSortedElementsAre({
                    test::IsRenderPassOverlay(right_child_pass_id),
                    test::OverlayHasResource(right_child_pass_texture_id),
        ...
        case 2:
            EXPECT_EQ(pass_list[0]->damage_rect, gfx::Rect()) << "Damage is removed when only from overlays";
            EXPECT_EQ(pass_list[1]->damage_rect, right_texture_quad_rect) << "Damage removed in frame 1 is re-added";
            EXPECT_THAT(result.candidates(),
                WhenCandidatesAreSortedElementsAre({
                    test::IsRenderPassOverlay(right_child_pass_id),
                    test::IsRenderPassOverlay(left_child_pass_id),
        ...
        case 3:
            EXPECT_EQ(pass_list[0]->damage_rect, left_texture_quad_rect) << "Damage removed in frame 2 is re-added";
