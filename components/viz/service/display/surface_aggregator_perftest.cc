...
perf_test::PerfResultReporter SetUpSurfaceAggregatorReporter(const std::string& story)
{
...
public:
    ...
    {
        resource_provider_ = std::make_unique<DisplayResourceProviderSoftware>(&shared_bitmap_manager_, /*shared_image_manager=*/nullptr,
            /*sync_point_manager=*/nullptr, /*gpu_scheduler=*/nullptr);
    }

    void RunTest(int num_surfaces, int num_textures, float opacity, bool full_damage, const std::string& story, ExpectedOutput expected_output)
    {
        std::vector<std::unique_ptr<CompositorFrameSinkSupport>> child_supports(num_surfaces);
        std::vector<base::UnguessableToken> child_tokens(num_surfaces);
        for (int i = 0; i < num_surfaces; i++) {
            child_supports[i] = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, FrameSinkId(1, i + 1), /*is_root=*/false);
            child_tokens[i] = base::UnguessableToken::Create();
        }
        aggregator_ = std::make_unique<SurfaceAggregator>(manager_.surface_manager(), resource_provider_.get(), true);
        for (int i = 0; i < num_surfaces; i++) {
            ...
            for (int j = 0; j < num_textures; j++) {
                ...
                TransferableResource resource
                    = TransferableResource::MakeSoftwareSharedBitmap(SharedBitmap::GenerateId(), gpu::SyncToken(), size, SinglePlaneFormat::kRGBA_8888);
                ...
                gfx::Rect visible_rect = j % 2 == 0 ? gfx::Rect(0, 0, 1, 2) : gfx::Rect(0, 1, 1, 1);
                ...
                quad->SetAll(sqs, rect, visible_rect, needs_blending, ResourceId(j), gfx::Size(), premultiplied_alpha, uv_top_left, uv_bottom_right,
                    background_color, flipped, nearest_neighbor, /*secure_output_only=*/false, gfx::ProtectedVideoType::kClear);
            }
            ...
            if (i >= 1) {
                ...
                surface_quad->SetNew(sqs, gfx::Rect(0, 0, 1, 1), gfx::Rect(0, 0, 1, 1),
                    ...
                    SurfaceRange(std::nullopt, SurfaceId(FrameSinkId(1, i), LocalSurfaceId(i, child_tokens[i - 1]))), SkColors::kWhite,
                    /*stretch_content_to_fill_bounds=*/false);
            }

            frame_builder.AddRenderPass(std::move(pass));
            child_supports[i]->SubmitCompositorFrame(local_surface_id, frame_builder.Build());
        }

        auto root_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, FrameSinkId(1, num_surfaces + 1), /*is_root=*/true);
        auto root_token = base::UnguessableToken::Create();
        base::TimeTicks next_fake_display_time = base::TimeTicks() + base::Seconds(1);
        ...
        do {
            ...
            auto* sqs = pass->CreateAndAppendSharedQuadState();
            auto* surface_quad = pass->CreateAndAppendDrawQuad<SurfaceDrawQuad>();
            surface_quad->SetNew(sqs, gfx::Rect(0, 0, 100, 100), gfx::Rect(0, 0, 100, 100),
                SurfaceRange(std::nullopt,
                    // Root surface embeds surface at index num_surfaces - 1.
                    SurfaceId(FrameSinkId(1, num_surfaces), LocalSurfaceId(num_surfaces, child_tokens[num_surfaces - 1]))),
            ...
            root_support->SubmitCompositorFrame(LocalSurfaceId(num_surfaces + 1, root_token), std::move(frame));

            auto aggregated = aggregator_->Aggregate(
                SurfaceId(FrameSinkId(1, num_surfaces + 1), LocalSurfaceId(num_surfaces + 1, root_token)), next_fake_display_time, gfx::OVERLAY_TRANSFORM_NONE);
    ...
    void SetUpRenderPassListResources(FrameSinkId frame_sink_id, uint64_t frame_index, CompositorRenderPassList* render_pass_list)
    {
        ...
        for (auto& render_pass : *render_pass_list) {
            for (auto* quad : render_pass->quad_list) {
                for (ResourceId resource_id : quad->resources) {
                    ...
                    auto& created_resources = resource_data_map_[frame_sink_id].created_resources;
                    // Create the resource if we haven't yet.
                    if (created_resources.find(resource_id) == created_resources.end()) {
                        created_resources[resource_id] = TransferableResource::MakeSoftwareSharedBitmap(
                            SharedBitmap::GenerateId(), gpu::SyncToken(), quad->rect.size(), SinglePlaneFormat::kRGBA_8888);
                        created_resources[resource_id].id = resource_id;
                    }
                    resource_data_map_[frame_sink_id].resource_lists[frame_index].push_back(created_resources[resource_id]);
    ...
    void SubmitCompositorFrame(CompositorFrameSinkSupport* frame_sink, SurfaceId surface_id, uint64_t frame_index,
        const CompositorRenderPassList& render_pass_list, std::vector<SurfaceRange> referenced_surfaces, bool set_full_damage)
    {
        ...
        frame_builder.SetRenderPassList(std::move(local_list))
            .SetTransferableResources(resource_data_map_[surface_id.frame_sink_id()].resource_lists[frame_index])
            .SetReferencedSurfaces(std::move(referenced_surfaces));
        frame_sink->SubmitCompositorFrame(surface_id.local_surface_id(), frame_builder.Build());
    }

    void RunSingleSurfaceRenderPassListFromJson(const std::string& tag, const std::string& site, uint32_t year, size_t index)
    {
        CompositorRenderPassList render_pass_list;
        ASSERT_TRUE(CompositorRenderPassListFromJSON(tag, site, year, index, &render_pass_list));
        ...
        this->SetUpRenderPassListResources(root_frame_sink_id, /*frame_index=*/1, &render_pass_list);

        aggregator_ = std::make_unique<SurfaceAggregator>(manager_.surface_manager(), resource_provider_.get(), true);

        auto root_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, root_frame_sink_id, /*is_root=*/true);
        ...
        do {
            ...
            auto aggregated = aggregator_->Aggregate(root_surface_id, next_fake_display_time, gfx::OVERLAY_TRANSFORM_NONE);

            next_fake_display_time += BeginFrameArgs::DefaultInterval();
            timer_.NextLap();
        } while (!timer_.HasTimeLimitExpired());

        auto reporter = SetUpSurfaceAggregatorReporter(site + "_single_surface_json");
        reporter.AddResult(kMetricSpeedRunsPerS, timer_.LapsPerSecond());
    }
    ...
    void RunPerfTestFromJson(const std::string& group, const std::string& name, size_t frame_start, size_t frame_end)
    {
        ...
        for (size_t i = frame_start; i <= frame_end; ++i) {
            ...
            base::FilePath json_path = unzipped_folder->AppendASCII(base::StringPrintf("%04d.json", static_cast<int>(i)));
        ...
        std::map<FrameSinkId, std::unique_ptr<CompositorFrameSinkSupport>> frame_sinks;
        for (auto& frame : frames) {
            for (auto& frame_data : frame) {
                ...
                if (frame_sinks.find(frame_sink_id) == frame_sinks.end()) {
                    ...
                    frame_sinks[frame_sink_id] = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, frame_sink_id,
                        /*is_root=*/frame_sinks.empty());
                }

                this->SetUpRenderPassListResources(frame_sink_id, frame_data.frame_index, &frame_data.compositor_frame.render_pass_list);
            }
        }
        ...
        if (single_frame) {
            ...
            for (int i = frame.size() - 1; i >= 1; --i) {
                ...
                auto& referenced_surfaces = frame[i].compositor_frame.metadata.referenced_surfaces;
                SubmitCompositorFrame(frame_sinks[surface_id.frame_sink_id()].get(), surface_id, frame_index, render_pass_list, referenced_surfaces,
                    /*set_full_damage=*/true);
            }
        }
        ...
        base::HistogramBase* aggregate_histogram = base::Histogram::FactoryMicrosecondsTimeGet("AggregateUs", SurfaceAggregator::kHistogramMinTime,
            SurfaceAggregator::kHistogramMaxTime, SurfaceAggregator::kHistogramTimeBuckets, base::Histogram::kNoFlags);
        ...
        do {
            ...
            if (laps++ > 0) {
                for (auto& entry : frame_sinks) {
                    ...
                    frame_sink = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, frame_sink_id, is_root);
                }
            }
            ...
            for (auto& frame : frames) {
                ...
                for (auto& frame_data : frame) {
                    ...
                    auto& referenced_surfaces = frame_data.compositor_frame.metadata.referenced_surfaces;
                    // For multi-frame tests, only set the full damage for the first frame
                    // in the sequence.
                    bool set_full_damage = single_frame || frame_num == 0;

                    SubmitCompositorFrame(
                        frame_sinks[surface_id.frame_sink_id()].get(), surface_id, frame_index, render_pass_list, referenced_surfaces, set_full_damage);
                    surface_index++;
                }

                base::ElapsedTimer aggregate_timer;
                // Always aggregate the root surface.
                auto aggregated = aggregator_->Aggregate(frames[0][0].surface_id, next_fake_display_time, gfx::OVERLAY_TRANSFORM_NONE);
                aggregate_histogram->AddTimeMicrosecondsGranularity(aggregate_timer.Elapsed());
        ...
        reporter.AddResult(kMetricAggregateUs, GetHistogramStats(aggregate_histogram));
        reporter.AddResult(kMetricPrewalkUs, GetHistogramStats("PrewalkUs"));
        reporter.AddResult(kMetricDeclareResourcesUs, GetHistogramStats("DeclareResourcesUs"));
...
private:
    ...
    {
        auto* histogram = base::StatisticsRecorder::FindHistogram("Compositing.SurfaceAggregator." + histogram_name);
        CHECK(histogram);
        ...
        base::HistogramBase* temp_histogram = base::Histogram::FactoryMicrosecondsTimeGet(histogram_name, SurfaceAggregator::kHistogramMinTime,
            SurfaceAggregator::kHistogramMaxTime, SurfaceAggregator::kHistogramTimeBuckets, base::Histogram::kNoFlags);
...
{
    RunTest(100, 1, 1.f, false, "100_surfaces_1_quad_each", ExpectedOutput(1, 100));
}

TEST_F(SurfaceAggregatorPerfTest, ManySurfacesOpaque_300)
{
    RunTest(300, 1, 1.f, false, "300_surfaces_1_quad_each", ExpectedOutput(1, 300));
}

TEST_F(SurfaceAggregatorPerfTest, ManySurfacesManyQuadsOpaque_100)
{
    RunTest(100, 100, 1.f, false, "100_surfaces_100_quads_each", ExpectedOutput(1, 5000));
}

TEST_F(SurfaceAggregatorPerfTest, ManySurfacesManyQuadsOpaque_300)
{
    RunTest(300, 100, 1.f, false, "300_surfaces_100_quads_each", ExpectedOutput(1, 15000));
}

TEST_F(SurfaceAggregatorPerfTest, ManySurfacesTransparent)
{
    RunTest(20, 100, .5f, true, "many_surfaces_transparent", ExpectedOutput(20, 2019));
}
...
{
    RunTest(20, 100, 1.f, true, "many_surfaces_opaque_damage_calc", ExpectedOutput(1, 2000));
}

TEST_F(SurfaceAggregatorPerfTest, ManySurfacesTransparentDamageCalc)
{
    RunTest(20, 100, .5f, true, "many_surfaces_transparent_damage_calc", ExpectedOutput(20, 2019));
}

TEST_F(SurfaceAggregatorPerfTest, FewSurfacesDamageCalc)
{
    RunTest(3, 1000, 1.f, true, "few_surfaces_damage_calc", ExpectedOutput(1, 3000));
}

TEST_F(SurfaceAggregatorPerfTest, FewSurfacesAggregateDamaged)
{
    RunTest(3, 1000, 1.f, false, "few_surfaces_aggregate_damaged", ExpectedOutput(1, 1500));
}

#define TOP_REAL_WORLD_DESKTOP_RENDERER_PERF_TEST(SITE, FRAME)                                                                                                 \
    TEST_F(SurfaceAggregatorPerfTest, SITE##_SingleSurfaceTest)                                                                                                \
    {                                                                                                                                                          \
        this->RunSingleSurfaceRenderPassListFromJson(/*tag=*/"top_real_world_desktop", /*site=*/#SITE, /*year=*/2018, /*frame_index=*/FRAME);                  \
    }
...
#define MULTI_SURFACE_PERF_TEST(TESTNAME, NAME, FRAME_START, FRAME_END)                                                                                        \
    ...
    {                                                                                                                                                          \
        this->RunPerfTestFromJson("multi_surface_test", #NAME, FRAME_START, FRAME_END);                                                                        \
    }
...
#define TOP_REAL_WORLD_MOBILE_PERF_TEST(NAME, FRAME_START, FRAME_END)                                                                                          \
    ...
    {                                                                                                                                                          \
        this->RunPerfTestFromJson("top_real_world_mobile", #NAME, FRAME_START, FRAME_END);                                                                     \
    }
