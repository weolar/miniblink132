...
std::optional<base::FilePath> MakeTestDataJsonPath(const std::string& group, const std::string& name, size_t frame_index)
{
    ...
    return test_data_path.AppendASCII("render_pass_data")
        .AppendASCII(group)
        .AppendASCII(name)
        .AppendASCII(base::StringPrintf("%04d.json", static_cast<int>(frame_index)));
}
...
bool CompositorRenderPassListFromJSON(
    const std::string& tag, const std::string& site, uint32_t year, size_t frame_index, CompositorRenderPassList* render_pass_list)
{
    std::string name = site + "_" + base::NumberToString(year);
    std::optional<base::FilePath> json_path = MakeTestDataJsonPath(tag, name, frame_index);
    if (!json_path) {
        return false;
    }
...
std::optional<base::FilePath> UnzipFrameData(const std::string& group, const std::string& name)
{
    base::FilePath zip_path;
    if (!base::PathService::Get(Paths::DIR_TEST_DATA, &zip_path)) {
        return std::nullopt;
    }
    zip_path = zip_path.AppendASCII("render_pass_data").AppendASCII(group).AppendASCII(name).AppendASCII(name + ".zip");

    base::FilePath out_path;
    if (!base::GetTempDir(&out_path)) {
        return std::nullopt;
    }
    out_path = out_path.AppendASCII("render_pass_data").AppendASCII(group).AppendASCII(name);
...
bool FrameDataFromJson(base::FilePath& json_path, std::vector<FrameData>* frame_data_list)
{
...
base::TimeDelta TestTimeLimit()
{
    ...
    if (command_line->HasSwitch(kPerfTestTimeMillis)) {
        const std::string delay_millis_string(command_line->GetSwitchValueASCII(kPerfTestTimeMillis));
        int delay_millis;
        if (base::StringToInt(delay_millis_string, &delay_millis) && delay_millis > 0) {
