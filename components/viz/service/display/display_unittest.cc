...
public:
    ...
    gfx::Size viewport_pixel_size() const
    {
...
public:
    explicit TestDisplayScheduler(BeginFrameSource* begin_frame_source, base::SingleThreadTaskRunner* task_runner)
        : DisplayScheduler(begin_frame_source, task_runner, PendingSwapParams(1))
    {
    }
...
public:
    ...
    void DisplayWillDrawAndSwap(bool will_draw_and_swap, AggregatedRenderPassList* render_passes) override
    {
    }
    void DisplayDidDrawAndSwap() override
    {
    }
    void DisplayDidReceiveCALayerParams(const gfx::CALayerParams& ca_layer_params) override
    {
    }
    ...
    base::TimeDelta GetPreferredFrameIntervalForFrameSinkId(const FrameSinkId& id, mojom::CompositorFrameSinkType* type) override
    {
        return BeginFrameArgs::MinInterval();
    }
};

void CopyCallback(bool* called, base::OnceClosure finished, std::unique_ptr<CopyOutputResult> result)
{
    *called = true;
...
public:
    DisplayTest()
        ...
        , support_(std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId, true /* is_root */))
    ...
    void SetUpSoftwareDisplay(const RendererSettings& settings)
    {
        ...
        output_surface = std::make_unique<FakeSoftwareOutputSurface>(std::move(device));
        output_surface_ = output_surface.get();

        CreateDisplaySchedulerAndDisplay(settings, kArbitraryFrameSinkId, std::move(output_surface));
    }
    ...
    void SetUpGpuDisplay(const RendererSettings& settings)
    {
        ...
        std::unique_ptr<FakeSkiaOutputSurface> skia_output_surface = FakeSkiaOutputSurface::Create3d(std::move(provider));
        skia_output_surface_ = skia_output_surface.get();

        CreateDisplaySchedulerAndDisplay(settings, kArbitraryFrameSinkId, std::move(skia_output_surface));
    }

    void CreateDisplaySchedulerAndDisplay(const RendererSettings& settings, const FrameSinkId& frame_sink_id, std::unique_ptr<OutputSurface> output_surface)
    {
        begin_frame_source_ = std::make_unique<StubBeginFrameSource>();
        auto scheduler = std::make_unique<TestDisplayScheduler>(begin_frame_source_.get(), task_runner_.get());
        scheduler_ = scheduler.get();
        display_ = CreateDisplay(settings, kArbitraryFrameSinkId, std::move(scheduler), std::move(output_surface));
        manager_.RegisterBeginFrameSource(begin_frame_source_.get(), kArbitraryFrameSinkId);
    }

    std::unique_ptr<Display> CreateDisplay(const RendererSettings& settings, const FrameSinkId& frame_sink_id, std::unique_ptr<DisplayScheduler> scheduler,
        std::unique_ptr<OutputSurface> output_surface)
    {
        ...
        auto display = std::make_unique<Display>(&shared_bitmap_manager_, &shared_image_manager_, &sync_point_manager_, &gpu_scheduler_, settings,
            &debug_settings_, frame_sink_id, nullptr /* DisplayCompositorMemoryAndTaskController */, std::move(output_surface), std::move(overlay_processor),
            std::move(scheduler), task_runner_);
        display->SetVisible(true);
        return display;
    }

    bool ShouldSendBeginFrame(CompositorFrameSinkSupport* support, base::TimeTicks frame_time)
    {
        return support->ShouldSendBeginFrame(frame_time, base::Seconds(0));
    }

    void UpdateBeginFrameTime(CompositorFrameSinkSupport* support, base::TimeTicks frame_time)
    {
...
protected:
    ...
    void SubmitCompositorFrame(CompositorRenderPassList* pass_list, const LocalSurfaceId& local_surface_id)
    {
        CompositorFrame frame = CompositorFrameBuilder().SetRenderPassList(std::move(*pass_list)).Build();
        pass_list->clear();
...
TEST_F(DisplayTest, DisplayDamaged)
{
    ...
    EXPECT_EQ(gfx::Size(100, 100), software_output_device_->viewport_pixel_size());
    ...
    // result in latency info being stored for the next swap.
    {
        ...
        CompositorFrame frame = CompositorFrameBuilder().AddRenderPass(kOutputRect, kDamageRect).AddLatencyInfo(ui::LatencyInfo()).Build();
    ...
    // Pass has copy output request so should be swapped.
    {
        ...
        pass->copy_requests.push_back(std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA,
            CopyOutputRequest::ResultDestination::kSystemMemory, base::BindOnce(&CopyCallback, &copy_called, copy_run_loop.QuitClosure())));
        pass->id = CompositorRenderPassId { 1u };
    ...
    // previous size.
    {
        ...
        constexpr gfx::Rect kDamageRect(10, 10, 10, 10);
        CompositorFrame frame = CompositorFrameBuilder().AddRenderPass(kOutputRect, kDamageRect).Build();
...
void DisplayTest::LatencyInfoCapTest(bool over_capacity)
{
    ...
    CompositorFrame frame1 = CompositorFrameBuilder().AddRenderPass(kOutputRect, kDamageRect).Build();
    ...
    CompositorFrame frame2 = CompositorFrameBuilder().AddRenderPass(kOutputRect, kDamageRect).AddLatencyInfos(std::move(latency_info)).Build();
    support_->SubmitCompositorFrame(local_surface_id, std::move(frame2));

    EXPECT_TRUE(display_->DrawAndSwap({ base::TimeTicks::Now(), base::TimeTicks::Now() }));
    EXPECT_EQ(1u, output_surface_->num_sent_frames());
    EXPECT_EQ(0u, output_surface_->last_sent_frame()->latency_info.size());

    // Run a successful swap and verify whether or not LatencyInfo was discarded.
    display_->Resize(gfx::Size(100, 100));
    CompositorFrame frame3 = CompositorFrameBuilder().AddRenderPass(kOutputRect, kDamageRect).Build();
    ...
    EXPECT_EQ(2u, output_surface_->num_sent_frames());
    EXPECT_EQ(expected_size, output_surface_->last_sent_frame()->latency_info.size());
}
...
TEST_F(DisplayTest, DisableSwapUntilResize)
{
    ...
    display_->DisableSwapUntilResize(base::BindLambdaForTesting([&swap_callback_run]() { swap_callback_run = true; }));
    EXPECT_TRUE(scheduler_->swapped());
...
TEST_F(DisplayTest, BackdropFilterTest)
{
    ...
    settings.partial_swap_enabled = true;
    id_allocator_.GenerateId();
    const LocalSurfaceId local_surface_id(id_allocator_.GetCurrentLocalSurfaceId());
    ...
    const LocalSurfaceId sub_local_surface_id1(6, base::UnguessableToken::Create());
    const SurfaceId sub_surface_id1(kAnotherFrameSinkId, sub_local_surface_id1);
    auto sub_support1 = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kAnotherFrameSinkId, /*is_root=*/false);

    // Create frame sink for another sub surface.
    const LocalSurfaceId sub_local_surface_id2(7, base::UnguessableToken::Create());
    const SurfaceId sub_surface_id2(kAnotherFrameSinkId2, sub_local_surface_id2);
    auto sub_support2 = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kAnotherFrameSinkId2, /*is_root=*/false);
    ...
    for (size_t frame_num = 1; frame_num <= 2; ++frame_num) {
        ...
        ResetDamageForTest();
        {
            ...
            bd_pass->SetAll(render_pass_id_generator.GenerateNextId(), sub_surface_rect, no_damage, gfx::Transform(), cc::FilterOperations(), backdrop_filters,
                gfx::RRectF(gfx::RectF(sub_surface_rect), 0), SubtreeCaptureId(), sub_surface_rect.size(), ViewTransitionElementResourceId(), false, false,
                false, false, false);
            pass_list.push_back(std::move(bd_pass));

            CompositorFrame frame = CompositorFrameBuilder().SetRenderPassList(std::move(pass_list)).Build();
            sub_support1->SubmitCompositorFrame(sub_local_surface_id1, std::move(frame));
        }

        {
            ...
            CompositorFrame frame = CompositorFrameBuilder().SetRenderPassList(std::move(pass_list)).Build();
            sub_support2->SubmitCompositorFrame(sub_local_surface_id2, std::move(frame));
        }

        {
            ...
            shared_quad_state1->SetAll(gfx::Transform(), /*quad_layer_rect=*/sub_surface_rect,
            ...
            quad1->SetNew(shared_quad_state1, /*rect=*/sub_surface_rect,
                /*visible_rect=*/sub_surface_rect, SurfaceRange(std::nullopt, sub_surface_id1), SkColors::kBlack,
            ...
            quad2->SetNew(shared_quad_state2, /*rect=*/rect1,
                /*visible_rect=*/rect1, SurfaceRange(std::nullopt, sub_surface_id2), SkColors::kBlack,
                /*stretch_content_to_fill_bounds=*/false);
            ...
            // surface.
            auto expected_damage = first_frame ? gfx::Rect(display_size) : gfx::Rect(20, 20, 40, 40);
            ...
            auto expected_scissor_rect = first_frame ? gfx::Rect(display_size) : gfx::Rect(5, 5, 55, 55);
            EXPECT_EQ(expected_scissor_rect, display_->renderer_for_testing()->GetLastRootScissorRectForTesting());
        }
    }
}
...
TEST_F(DisplayTest, CompositorFrameDamagesCorrectDisplay)
{
    ...
    auto support2 = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kAnotherFrameSinkId, true /* is_root */);
    auto begin_frame_source2 = std::make_unique<StubBeginFrameSource>();
    auto scheduler_for_display2 = std::make_unique<TestDisplayScheduler>(begin_frame_source2.get(), task_runner_.get());
    TestDisplayScheduler* scheduler2 = scheduler_for_display2.get();
    StubDisplayClient client2; // Must outlive `display2`.
    auto display2 = CreateDisplay(settings, kAnotherFrameSinkId, std::move(scheduler_for_display2),
        std::make_unique<FakeSoftwareOutputSurface>(std::make_unique<TestSoftwareOutputDevice>()));
    manager_.RegisterBeginFrameSource(begin_frame_source2.get(), kAnotherFrameSinkId);
...
class OnBeginFrameAcksDisplayTest : public DisplayTest, public testing::WithParamInterface<bool> {
public:
...
TEST_P(OnBeginFrameAcksDisplayTest, CompositorFrameWithPresentationToken)
{
    ...
    // Create frame sink for a sub surface.
    const LocalSurfaceId sub_local_surface_id(6, base::UnguessableToken::Create());
    const SurfaceId sub_surface_id(kAnotherFrameSinkId, sub_local_surface_id);

    MockCompositorFrameSinkClient sub_client;

    auto sub_support = std::make_unique<CompositorFrameSinkSupport>(&sub_client, &manager_, kAnotherFrameSinkId, false /* is_root */);
    if (BeginFrameAcksEnabled()) {
    ...
    uint32_t frame_token_2 = kInvalidFrameToken;
    {
        CompositorFrame frame
            = CompositorFrameBuilder().AddRenderPass(gfx::Rect(sub_surface_size), gfx::Rect()).SetBeginFrameSourceId(kBeginFrameSourceId).Build();
        EXPECT_CALL(sub_client, DidReceiveCompositorFrameAck(_)).Times(BeginFrameAcksEnabled() ? 0 : 1);
...

    {
        ...
        quad1->SetNew(shared_quad_state1, rect1 /* rect */, rect1 /* visible_rect */, SkColors::kBlack, false /* force_anti_aliasing_off */);
        ...
        shared_quad_state2->SetAll(gfx::Transform(), /*quad_layer_rect=*/rect2,
            ...
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        auto* quad2 = pass->quad_list.AllocateAndConstruct<SurfaceDrawQuad>();
        quad2->SetNew(shared_quad_state2, rect2 /* rect */, rect2 /* visible_rect */, SurfaceRange(std::nullopt, sub_surface_id), SkColors::kBlack,
...

    {
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(gfx::Rect(sub_surface_size), gfx::Rect(sub_surface_size))
...
INSTANTIATE_TEST_SUITE_P(, OnBeginFrameAcksDisplayTest, testing::Bool(), [](auto& info) { return info.param ? "BeginFrameAcks" : "CompositoFrameAcks"; });
...
TEST_F(DisplayTest, BeginFrameThrottling)
{
    ...
    for (uint32_t i = 0; i < CompositorFrameSinkSupport::kUndrawnFrameLimit + 1; ++i) {
...
TEST_F(DisplayTest, DontThrottleWhenParentBlocked)
{
    ...
    for (uint32_t i = 0; i < CompositorFrameSinkSupport::kUndrawnFrameLimit + 1; ++i) {
        ...
        sub_support->SubmitCompositorFrame(sub_local_surface_id, MakeDefaultCompositorFrame());
        // Until we reach throttling we should return true.
        if (i < CompositorFrameSinkSupport::kUndrawnFrameLimit) {
            EXPECT_TRUE(ShouldSendBeginFrame(sub_support.get(), frame_time));
        } else {
    ...
    CompositorFrame frame = CompositorFrameBuilder()
                                ...
                                .SetDeadline(FrameDeadline(base::TimeTicks::Now(), std::numeric_limits<uint32_t>::max(), base::Seconds(1), false))
                                .Build();
    support_->SubmitCompositorFrame(id_allocator_.GetCurrentLocalSurfaceId(), std::move(frame));

    for (uint32_t i = 0; i < CompositorFrameSinkSupport::kUndrawnFrameLimit * 3; ++i) {
        frame_time = base::TimeTicks::Now();
        EXPECT_TRUE(ShouldSendBeginFrame(sub_support.get(), frame_time));
        UpdateBeginFrameTime(sub_support.get(), frame_time);
        sub_support->SubmitCompositorFrame(sub_local_surface_id, MakeDefaultCompositorFrame());
    ...
    EXPECT_TRUE(ShouldSendBeginFrame(sub_support.get(), frame_time));
    UpdateBeginFrameTime(sub_support.get(), frame_time);
    sub_support->SubmitCompositorFrame(sub_local_surface_id2, MakeDefaultCompositorFrame());
    frame_time = base::TimeTicks::Now();
    EXPECT_FALSE(ShouldSendBeginFrame(sub_support.get(), frame_time));
    UpdateBeginFrameTime(sub_support.get(), frame_time);
}
...
TEST_F(DisplayTest, DisplayTransformHint)
{
    ...
    constexpr gfx::Size kTransposedSize = gfx::Size(kSize.height(), kSize.width());
    ...
    for (const auto& test : kTestCases) {
        SCOPED_TRACE(
            testing::Message() << "support_display_transform=" << test.support_display_transform << ", display_transform_hint=" << test.display_transform_hint);

        output_surface_->set_support_display_transform_hint(test.support_display_transform);
        ...
        EXPECT_EQ(test.expected_size, software_output_device_->viewport_pixel_size());
    }
}
...
class UseMapRectDisplayTest : public DisplayTest, public testing::WithParamInterface<bool> {
public:
...
{
    if (GetParam()) {
        scoped_feature_list_.InitAndEnableFeature(features::kUseMapRectForPixelMovement);
    } else {
        scoped_feature_list_.InitAndDisableFeature(features::kUseMapRectForPixelMovement);
    }
}
...
TEST_P(UseMapRectDisplayTest, PixelMovingForegroundFilterTest)
{
    ...
    for (size_t frame_num = 1; frame_num <= 2; ++frame_num) {
        ...
        ResetDamageForTest();
        {
            ...
            foreground_filters.Append(cc::FilterOperation::CreateDropShadowFilter(gfx::Point(5, 10), 2.f, SkColors::kTransparent));
            bd_pass->SetAll(render_pass_id_generator.GenerateNextId(), sub_surface_rect, no_damage, gfx::Transform(), foreground_filters,
                cc::FilterOperations(), gfx::RRectF(gfx::RectF(sub_surface_rect), 0), SubtreeCaptureId(), sub_surface_rect.size(),
                ViewTransitionElementResourceId(), false, false, false, false, false);
            pass_list.push_back(std::move(bd_pass));

            CompositorFrame frame = CompositorFrameBuilder().SetRenderPassList(std::move(pass_list)).Build();
            sub_support1->SubmitCompositorFrame(sub_surface_id1.local_surface_id(), std::move(frame));
        }

        {
            ...
            CompositorFrame frame = CompositorFrameBuilder().SetRenderPassList(std::move(pass_list)).Build();
            sub_support2->SubmitCompositorFrame(sub_surface_id2.local_surface_id(), std::move(frame));
        }

        {
            auto frame = CompositorFrameBuilder()
                             .AddRenderPass(RenderPassBuilder(display_size)
                                                .AddSurfaceQuad(sub_surface_rect, SurfaceRange(std::nullopt, sub_surface_id1), { .allow_merge = false })
                                                .AddSurfaceQuad(gfx::Rect(display_size), SurfaceRange(std::nullopt, sub_surface_id2), { .allow_merge = false })
            ...
            auto expected_damage = first_frame ? gfx::Rect(display_size) : damage_rect;
            EXPECT_EQ(expected_damage, software_output_device_->damage_rect());
            // The scissor rect is expanded by direct_renderer to include the
            // overlapping pixel-moving foreground filter surface.
            auto expected_scissor_rect = first_frame ? gfx::Rect(display_size) : GetParam() ? gfx::Rect(4, 5, 56, 55) : gfx::Rect(0, 0, 60, 60);
            EXPECT_EQ(expected_scissor_rect, display_->renderer_for_testing()->GetLastRootScissorRectForTesting());
        }
    }
}
...
TEST_F(DisplayTest, CanSkipRenderPass)
{
    ...
    for (size_t frame_num = 1; frame_num <= 3; ++frame_num) {
        ...
        // Root render pass
        {
            ...
            auto* skipped = display_->renderer_for_testing()->GetLastSkippedRenderPassIdsForTesting();
...
public:
    ...
    void SetUpRenderers()
    {
        ...
        display_->renderer_for_testing()->SetDelegatedInkPointRendererSkiaForTest(std::move(renderer));
    }

    DelegatedInkPointRendererBase* ink_renderer()
    {
        return display_->renderer_for_testing()->GetDelegatedInkPointRenderer(/*create_if_necessary=*/
            false);
    }
    ...
    const std::map<base::TimeTicks, gfx::DelegatedInkPoint>& GetPointsForPointerId(int32_t pointer_id)
    {
        DCHECK(ink_renderer()->GetPointsMapForTest().find(pointer_id) != ink_renderer()->GetPointsMapForTest().end());
        return ink_renderer()->GetPointsMapForTest().find(pointer_id)->second.GetPoints();
    }

    void CreateAndStoreDelegatedInkPoint(const gfx::PointF& point, base::TimeTicks timestamp, int32_t pointer_id)
    {
    ...
    {
        ink_renderer()->SetDelegatedInkMetadata(std::make_unique<gfx::DelegatedInkMetadata>(metadata));
    }

    gfx::DelegatedInkMetadata MakeAndSendMetadataFromStoredInkPoint(int index, float diameter, SkColor4f color, const gfx::RectF& presentation_area)
    {
        DCHECK_EQ(static_cast<int>(ink_points_.size()), 1);
        return MakeAndSendMetadataFromStoredInkPoint(ink_points_.begin()->first, index, diameter, color, presentation_area);
    }

    gfx::DelegatedInkMetadata MakeAndSendMetadataFromStoredInkPoint(
        int32_t pointer_id, int index, float diameter, SkColor4f color, const gfx::RectF& presentation_area)
    {
        DCHECK(ink_points_.find(pointer_id) != ink_points_.end());
        EXPECT_GE(index, 0);
        ...
        gfx::DelegatedInkMetadata metadata(ink_points_[pointer_id][index].point(), diameter, color.toSkColor(), ink_points_[pointer_id][index].timestamp(),
    ...
    void HistogramCheck(const base::HistogramTester& histograms, base::TimeDelta expected_bucket, const char* histogram_name)
    {
    ...
    void FinalizePathAndCheckHistograms(base::TimeDelta expected_bucket_without_prediction, base::TimeDelta expected_bucket_with_prediction)
    {
        base::HistogramTester histograms;
        ink_renderer()->FinalizePathForDraw();
        HistogramCheck(histograms, expected_bucket_without_prediction, "Renderer.DelegatedInkTrail.LatencyImprovement.Skia.WithoutPrediction");
        HistogramCheck(histograms, expected_bucket_with_prediction, "Renderer.DelegatedInkTrail.LatencyImprovement.Skia.WithPrediction");
    }

    void DrawDelegatedInkTrail()
    {
        SkCanvas canvas;
        static_cast<DelegatedInkPointRendererSkia*>(ink_renderer())->DrawDelegatedInkTrail(&canvas, gfx::Transform());
    }

    int GetPathPointCount()
    {
        return ink_renderer()->GetPathPointCountForTest();
    }
    ...
    int points_to_predict() const
    {
        return kPointsToPredict;
    }

    const base::TimeDelta time_into_the_future() const
    {
        return base::Milliseconds((kMillisecondsIntoFuturePerPoint - kResampleLatency) * kPointsToPredict);
    }
...
TEST_F(SkiaDelegatedInkRendererTest, SkiaDelegatedInkRendererFilteringPoints)
{
    ...
    gfx::DelegatedInkMetadata metadata = MakeAndSendMetadataFromStoredInkPoint(kInkPointForMetadata, kDiameter, SkColors::kBlack, gfx::RectF());
    ...
    base::TimeDelta bucket_without_prediction = last_ink_point(kPointerId).timestamp() - metadata.timestamp();
    FinalizePathAndCheckHistograms(bucket_without_prediction, base::Milliseconds(7));

    EXPECT_EQ(kInitialDelegatedPoints - kInkPointForMetadata, StoredPointsForPointerId(kPointerId));
    EXPECT_EQ(metadata.point(), GetPointsForPointerId(kPointerId).begin()->second.point());
    EXPECT_EQ(last_ink_point(kPointerId).point(), GetPointsForPointerId(kPointerId).rbegin()->second.point());
    ...
    while (ink_points_size() < gfx::kMaximumNumberOfDelegatedInkPoints + kPointsBeyondMaxAllowed)
        CreateAndStoreDelegatedInkPointFromPreviousPoint(kPointerId);

    EXPECT_EQ(gfx::kMaximumNumberOfDelegatedInkPoints, StoredPointsForPointerId(kPointerId));
    EXPECT_EQ(ink_point(kPointsBeyondMaxAllowed).point(), GetPointsForPointerId(kPointerId).begin()->second.point());
    EXPECT_EQ(last_ink_point(kPointerId).point(), GetPointsForPointerId(kPointerId).rbegin()->second.point());
...
TEST_F(SkiaDelegatedInkRendererTest, SkiaDelegatedInkRendererFilteringPointsWithMultiplePointerIds)
{
    SetUpRenderers();
    ...
    for (uint64_t i = 0; i < kPointerIds.size(); ++i) {
        ...
        CreateAndStoreDelegatedInkPoint(gfx::PointF(i * 5, i * 10), timestamp, kPointerIds[i]);
        timestamp += base::Milliseconds(5);
    }
    ...
    for (int32_t pointer_id : kPointerIds) {
        EXPECT_EQ(ink_points_size(pointer_id), StoredPointsForPointerId(pointer_id));
    }
    ...
    const int kInkPointForMetadata = 1;
    const float kDiameter = 1.f;
    gfx::DelegatedInkMetadata metadata = MakeAndSendMetadataFromStoredInkPoint(kPointerIds[0], kInkPointForMetadata, kDiameter, SkColors::kBlack, gfx::RectF());
    ...
    base::TimeDelta bucket_without_prediction = last_ink_point(kPointerIds[0]).timestamp() - metadata.timestamp();
    FinalizePathAndCheckHistograms(bucket_without_prediction, bucket_without_prediction + time_into_the_future());

    // Confirm the size, first, and last points of the first pointer ID are what
    // we expect.
    EXPECT_EQ(kNumPointsForPointerId0 - kInkPointForMetadata, StoredPointsForPointerId(kPointerIds[0]));
    EXPECT_EQ(metadata.point(), GetPointsForPointerId(kPointerIds[0]).begin()->second.point());
    EXPECT_EQ(last_ink_point(kPointerIds[0]).point(), GetPointsForPointerId(kPointerIds[0]).rbegin()->second.point());

    // Confirm that neither of the other pointer ids were impacted.
    for (uint64_t i = 1; i < kPointerIds.size(); ++i) {
        EXPECT_EQ(ink_points_size(kPointerIds[i]), StoredPointsForPointerId(kPointerIds[i]));
    }
    ...
    SendMetadata(
        gfx::DelegatedInkMetadata(gfx::PointF(100, 100), 5.6f, SK_ColorBLACK, base::TimeTicks::Min(), gfx::RectF(), base::TimeTicks::Min(), /*hovering*/ false,
            /*render_pass_id=*/0));
    FinalizePathAndCheckHistograms(base::Milliseconds(0), base::TimeDelta::Min());
    EXPECT_EQ(kNumPointsForPointerId0 - kInkPointForMetadata, StoredPointsForPointerId(kPointerIds[0]));
    for (uint64_t i = 1; i < kPointerIds.size(); ++i) {
    ...
    SendMetadata(gfx::DelegatedInkMetadata(gfx::PointF(100, 100), 5.6f, SK_ColorBLACK, base::TimeTicks::Now() + base::Milliseconds(1000), gfx::RectF(),
...
TEST_F(SkiaDelegatedInkRendererTest, LatencyHistograms)
{
    ...
    // or points to draw.
    FinalizePathAndCheckHistograms(base::TimeDelta::Min(), base::TimeDelta::Min());
    ...
    CreateAndStoreDelegatedInkPoint(gfx::PointF(15, 19), timestamp + base::Milliseconds(8), kPointerId);
    CreateAndStoreDelegatedInkPoint(gfx::PointF(16, 28), timestamp + base::Milliseconds(16), kPointerId);
    CreateAndStoreDelegatedInkPoint(gfx::PointF(29, 35), timestamp + base::Milliseconds(24), kPointerId);

    // Provide a metadata so that points can be drawn, based on the first ink
    // point that was sent.
    const float kDiameter = 11.99f;
    MakeAndSendMetadataFromStoredInkPoint(/*index*/ 0, kDiameter, SkColors::kBlack, gfx::RectF());
    ...
    MakeAndSendMetadataFromStoredInkPoint(/*index*/ 3, kDiameter, SkColors::kBlack, gfx::RectF());
    bucket_without_prediction = base::Milliseconds(0);
    FinalizePathAndCheckHistograms(bucket_without_prediction, time_into_the_future());
    ...
    CreateAndStoreDelegatedInkPoint(gfx::PointF(96, 70), timestamp + base::Milliseconds(2), kPointerId);
    CreateAndStoreDelegatedInkPoint(gfx::PointF(112, 94), timestamp + base::Milliseconds(10), kPointerId);
    FinalizePathAndCheckHistograms(base::TimeDelta::Min(), base::TimeDelta::Min());
}
...
TEST_F(SkiaDelegatedInkRendererTest, DrawTrailWhenMetadataIsCloseEnough)
{
    ...
    CreateAndStoreDelegatedInkPoint(gfx::PointF(80.f, 70.f), timestamp2 + base::Milliseconds(8), kPointerId);

    gfx::DelegatedInkMetadata metadata(gfx::PointF(point.x() - 1.0f, point.y() - 1.0f), 45.f, SK_ColorBLACK, timestamp, gfx::RectF(0, 0, 100, 100),
        base::TimeTicks::Now(),
        /*hovering*/ false, /*render_pass_id=*/0);
    SendMetadata(metadata);
    ...
    // drawing.
    metadata = gfx::DelegatedInkMetadata(gfx::PointF(point2.x() - 1.01f, point2.y() - 1.0f), 45.f, SK_ColorBLACK, timestamp2, gfx::RectF(0, 0, 100, 100),
        base::TimeTicks::Now(),
        /*hovering*/ false, /*render_pass_id=*/0);
    SendMetadata(metadata);
...
{
    const std::string kHistogramName = "Renderer.DelegatedInkTrail.Skia.OutstandingPointsToDraw";
    ...
    CreateAndStoreDelegatedInkPoint(point, timestamp, kPointerId);
    SendMetadata(
        gfx::DelegatedInkMetadata(gfx::PointF(point.x(), point.y()), 45.f, SK_ColorBLACK, timestamp, gfx::RectF(0, 0, 100, 100), base::TimeTicks::Now(),
            /*hovering=*/false, /*render_pass_id=*/0));
    ink_renderer()->ReportPointsDrawn();
    ...
    CreateAndStoreDelegatedInkPoint(point + gfx::Vector2d(1, 1), timestamp + base::Milliseconds(10), kPointerId);
    ink_renderer()->ReportPointsDrawn();
    histogram_tester.ExpectTotalCount(kHistogramName, 2);
    histogram_tester.ExpectBucketCount(kHistogramName, 2, 1);
    CreateAndStoreDelegatedInkPoint(point + gfx::Vector2d(2, 2), timestamp + base::Milliseconds(20), kPointerId);
    ink_renderer()->ReportPointsDrawn();
...
{
    const std::string kHistogramName = "Renderer.DelegatedInkTrail.Skia.TimeToDrawPointsMillis";
    const base::HistogramTester histogram_tester;
    ...
    // previous).
    CreateAndStoreDelegatedInkPoint(point + gfx::Vector2d(1, 1), timestamp + base::Milliseconds(1), kPointerId);
    CreateAndStoreDelegatedInkPoint(point + gfx::Vector2d(2, 2), timestamp + base::Milliseconds(2), kPointerId);
    ink_renderer()->ReportPointsDrawn();
    histogram_tester.ExpectTotalCount(kHistogramName, 4);
}

TEST_F(SkiaDelegatedInkRendererTest, SkiaDelegatedInkTimeFromDelegatedInkToApiPaint)
{
    const std::string kHistogramName = "Renderer.DelegatedInkTrail.Skia.TimeFromDelegatedInkToApiPaint";
    const base::HistogramTester histogram_tester;
    constexpr int32_t kPointerId = 1u;
    const auto create_metadata = [](gfx::PointF& p, base::TimeTicks& t) {
        return gfx::DelegatedInkMetadata(p, /*diameter=*/45.f, SK_ColorBLACK, t, gfx::RectF(0, 0, 100, 100), t,
            /*hovering=*/false, /*render_pass_id=*/0);
    };
    SetUpRenderers();
    ...
    ink_renderer()->ReportPointsDrawn();
    EXPECT_NE(std::nullopt, GetPointsForPointerId(kPointerId).find(timestamp)->second.paint_timestamp());
    ink_renderer()->ReportPointsDrawn();
...
class DelegatedInkDisplayTest : public SkiaDelegatedInkRendererTest, public testing::WithParamInterface<DelegatedInkType> {
public:
    ...
    void SubmitCompositorFrameWithInkMetadata(
        CompositorRenderPassList* pass_list, const LocalSurfaceId& local_surface_id, const gfx::DelegatedInkMetadata& metadata)
    {
        CompositorFrame frame = CompositorFrameBuilder().SetRenderPassList(std::move(*pass_list)).AddDelegatedInkMetadata(metadata).Build();
        pass_list->clear();
...
struct DelegatedInkDisplayTestPassToString {
    std::string operator()(const testing::TestParamInfo<DelegatedInkType> type) const
    {
        return type.param == DelegatedInkType::kPlatformInk ? "PlatformInk" : "SkiaInk";
    }
};

INSTANTIATE_TEST_SUITE_P(DelegatedInkTrails, DelegatedInkDisplayTest, testing::Values(DelegatedInkType::kPlatformInk, DelegatedInkType::kSkiaInk),
...
TEST_P(DelegatedInkDisplayTest, MetadataOnlySentToSkiaRendererOrOutputSurface)
{
    ...
    gfx::DelegatedInkMetadata metadata(gfx::PointF(5, 5), 3.5f, SK_ColorBLACK, base::TimeTicks::Now(), gfx::RectF(0, 0, 20, 20), base::TimeTicks::Now(), false,
        /*render_pass_id=*/0);

    SubmitCompositorFrameWithInkMetadata(&pass_list, id_allocator_.GetCurrentLocalSurfaceId(), metadata);
    display_->DrawAndSwap({ base::TimeTicks::Now(), base::TimeTicks::Now() });
    ...
    const gfx::DelegatedInkMetadata* retrieved_metadata
        = GetParam() == DelegatedInkType::kPlatformInk ? skia_output_surface_->last_delegated_ink_metadata() : GetMetadataFromTestRenderer();
    ...
    EXPECT_EQ(retrieved_metadata->presentation_area(), metadata.presentation_area());
...
TEST_P(DelegatedInkDisplayTest, InkRendererRemoteGoesToSkiaRendererOrOutputSurface)
{
    SetUpGpuDisplay();

    mojo::Remote<gfx::mojom::DelegatedInkPointRenderer> ink_renderer_remote;
    display_->InitDelegatedInkPointRendererReceiver(ink_renderer_remote.BindNewPipeAndPassReceiver());

    if (GetParam() == DelegatedInkType::kPlatformInk) {
        EXPECT_TRUE(skia_output_surface_->ContainsDelegatedInkPointRendererReceiverForTesting());
        EXPECT_FALSE(ink_renderer());
    } else {
        EXPECT_FALSE(skia_output_surface_->ContainsDelegatedInkPointRendererReceiverForTesting());
...
// fails.
TEST_F(UnsupportedRendererDelegatedInkTest, DelegatedInkSilentlyFailsOnSoftwareRenderer)
{
    SetUpSoftwareDisplay(RendererSettings());
    display_->Initialize(client_.get(), manager_.surface_manager());
...
INSTANTIATE_TEST_SUITE_P(, UseMapRectDisplayTest, testing::Bool(), &PostTestCaseName);
