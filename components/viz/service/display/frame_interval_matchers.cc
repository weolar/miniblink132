...
namespace media {
bool HasSimpleCadence(base::TimeDelta render_interval, base::TimeDelta frame_duration, base::TimeDelta minimum_time_until_max_drift)
{
    *(int*)1 = 1;
    return false;
}
...
std::optional<FrameIntervalMatcher::Result> MatchContentIntervalType(const FrameIntervalMatcher::Inputs& matcher_inputs, ContentFrameIntervalType type)
{
    ...
    for (const auto& [frame_sink_id, inputs] : matcher_inputs.inputs_map) {
        ...
        if ((matcher_inputs.aggregated_frame_time - inputs.frame_time) > matcher_inputs.settings->ignore_frame_sink_timeout) {
            continue;
        }
        ...
        for (const ContentFrameIntervalInfo& content_frame_interval_info : inputs.content_interval_info) {
            ...
            if (content_interval
                && !FrameIntervalMatcher::AreAlmostEqual(
                    content_interval.value(), content_frame_interval_info.frame_interval, matcher_inputs.settings->epsilon)) {
                return std::nullopt;
            }
    ...
    return absl::visit(base::Overloaded(
                           ...
                           [&](const FrameIntervalMatcher::FixedIntervalSettings& fixed_interval_settings) {
                               // Pick the best interval from supported intervals using
                               // `HasSimpleCadence`.
                               std::optional<base::TimeDelta> best_interval;
                               for (auto supported_interval : fixed_interval_settings.supported_intervals) {
                                   bool simple_cadence = media::VideoCadenceEstimator::HasSimpleCadence(
                                       supported_interval, content_interval.value(), matcher_inputs.settings->max_time_until_next_glitch);
                                   if (simple_cadence && (!best_interval || supported_interval > best_interval)) {
                                       best_interval = supported_interval;
                                   }
                               }
                               return best_interval.value_or(fixed_interval_settings.default_interval);
                           },
                           [&](const FrameIntervalMatcher::ContinuousRangeSettings& continuous_range_settings) {
                               ...
                               if (content_interval.value() < range_min) {
                                   ...
                                   base::TimeDelta cadence_interval = cadence * content_interval.value();
                                   // Use the calculated cadence if it didn't overshoot the range
                                   // maximum. Otherwise use the range minimum as the closest
                                   // fallback.
                                   return cadence_interval <= range_max ? cadence_interval : range_min;
                               }
                               ...
                               if (content_interval.value() > range_max) {
                                   ...
                                   base::TimeDelta cadence_interval = content_interval.value() / cadence;
                                   // Use the calculated cadence if it didn't undershoot the range
                                   // minimum. Otherwise use the range maximum as the closest
                                   // fallback.
                                   return cadence_interval >= range_min ? cadence_interval : range_max;
                               }
...
FrameIntervalMatcher::FixedIntervalSettings::FixedIntervalSettings(const FixedIntervalSettings&) = default;
FrameIntervalMatcher::FixedIntervalSettings::~FixedIntervalSettings() = default;

FrameIntervalMatcher::ContinuousRangeSettings::ContinuousRangeSettings() = default;
FrameIntervalMatcher::ContinuousRangeSettings::ContinuousRangeSettings(const ContinuousRangeSettings&) = default;
FrameIntervalMatcher::ContinuousRangeSettings::~ContinuousRangeSettings() = default;

FrameIntervalMatcher::Settings::Settings() = default;
FrameIntervalMatcher::Settings::~Settings() = default;
FrameIntervalMatcher::Settings::Settings(const Settings& other) = default;
FrameIntervalMatcher::Settings& FrameIntervalMatcher::Settings::operator=(const Settings& other) = default;
FrameIntervalMatcher::Settings::Settings(Settings&& other) = default;
FrameIntervalMatcher::Settings& FrameIntervalMatcher::Settings::operator=(Settings&& other) = default;
...
FrameIntervalMatcher::Inputs& FrameIntervalMatcher::Inputs::operator=(const Inputs& other) = default;

void FrameIntervalMatcher::Inputs::WriteIntoTrace(perfetto::TracedValue trace_context) const
{
    *(int*)1 = 1;
//     auto dict = std::move(trace_context).WriteDictionary();
//     for (const auto& [frame_sink_id, interval_inputs] : inputs_map) {
//         auto frame_sink_dict = dict.AddDictionary(perfetto::DynamicString(frame_sink_id.ToString()));
//         frame_sink_dict.Add("time_diff_us", (aggregated_frame_time - interval_inputs.frame_time).InMicroseconds());
//         frame_sink_dict.Add("has_input", interval_inputs.has_input);
//         frame_sink_dict.Add("only_content", interval_inputs.has_only_content_frame_interval_updates);
// 
//         int index = 0;
//         for (const ContentFrameIntervalInfo& content_info : interval_inputs.content_interval_info) {
//             auto content_info_dict = dict.AddDictionary(perfetto::DynamicString(base::StringPrintf("content_info_%d", index)));
//             content_info_dict.Add("type", ContentFrameIntervalTypeToString(content_info.type));
//             content_info_dict.Add("interval_us", content_info.frame_interval.InMicroseconds());
...
std::string FrameIntervalMatcher::ResultToString(const Result& result)
{
    return absl::visit(base::Overloaded(
                           ...
                           [](base::TimeDelta interval) { return base::StringPrintf("%" PRId64 "us", interval.InMicroseconds()); }),
        result);
}

// static
std::string FrameIntervalMatcher::MatcherTypeToString(FrameIntervalMatcherType type)
{
    switch (type) {
...
// static
bool FrameIntervalMatcher::AreAlmostEqual(base::TimeDelta a, base::TimeDelta b, base::TimeDelta epsilon)
{
...
std::optional<FrameIntervalMatcher::Result> InputBoostMatcher::Match(const Inputs& matcher_inputs)
{
    for (const auto& [frame_sink_id, inputs] : matcher_inputs.inputs_map) {
        if (inputs.has_input && (matcher_inputs.aggregated_frame_time - inputs.frame_time) < matcher_inputs.settings->ignore_frame_sink_timeout) {
            return absl::visit(
                base::Overloaded([](const absl::monostate& monostate) -> Result { return FrameIntervalClass::kBoost; },
                    [](const FixedIntervalSettings& fixed_interval_settings) -> Result { return *fixed_interval_settings.supported_intervals.begin(); },
                    [](const ContinuousRangeSettings& continuous_range_settings) -> Result { return continuous_range_settings.min_interval; }),
                matcher_inputs.settings->interval_settings);
        }
    }
    return std::nullopt;
...
std::optional<FrameIntervalMatcher::Result> OnlyVideoMatcher::Match(const Inputs& matcher_inputs)
{
    return MatchContentIntervalType(matcher_inputs, ContentFrameIntervalType::kVideo);
}
...
DefineSimpleMatcherConstructorDestructor(VideoConferenceMatcher, kVideoConference);
std::optional<FrameIntervalMatcher::Result> VideoConferenceMatcher::Match(const Inputs& matcher_inputs)
{
    size_t num_videos = 0;
    ...
    for (const auto& [frame_sink_id, inputs] : matcher_inputs.inputs_map) {
        for (const ContentFrameIntervalInfo& content_frame_interval_info : inputs.content_interval_info) {
            // Ignore non-video.
            if (content_frame_interval_info.type != ContentFrameIntervalType::kVideo) {
                continue;
            }
            ...
            if ((matcher_inputs.aggregated_frame_time - inputs.frame_time) > 5 * content_frame_interval_info.frame_interval) {
                continue;
            }

            if (!min_interval || min_interval.value() > content_frame_interval_info.frame_interval) {
    ...
    return absl::visit(base::Overloaded([&](const absl::monostate& monostate) { return min_interval.value(); },
                           [&](const FixedIntervalSettings& fixed_interval_settings) {
                               ...
                               for (auto supported_interval : fixed_interval_settings.supported_intervals) {
                                   ...
                                   if ((AreAlmostEqual(min_interval.value(), supported_interval, matcher_inputs.settings->epsilon) || delta.is_positive())
                           ...
                           [&](const ContinuousRangeSettings& continuous_range_settings) {
                               ...
                               return std::clamp(min_interval.value(), continuous_range_settings.min_interval, continuous_range_settings.max_interval);
                           }),
        matcher_inputs.settings->interval_settings);
}

DefineSimpleMatcherConstructorDestructor(OnlyAnimatingImageMatcher, kOnlyAnimatingImage);
std::optional<FrameIntervalMatcher::Result> OnlyAnimatingImageMatcher::Match(const Inputs& matcher_inputs)
{
    return MatchContentIntervalType(matcher_inputs, ContentFrameIntervalType::kAnimatingImage);
}

DefineSimpleMatcherConstructorDestructor(OnlyScrollBarFadeOutAnimationMatcher, kOnlyScrollBarFadeOut);
std::optional<FrameIntervalMatcher::Result> OnlyScrollBarFadeOutAnimationMatcher::Match(const Inputs& matcher_inputs)
{
    return MatchContentIntervalType(matcher_inputs, ContentFrameIntervalType::kScrollBarFadeOutAnimation);
}
