...
static void CollectResources(std::vector<ReturnedResource>* array, std::vector<ReturnedResource> returned)
{
    array->insert(array->end(), std::make_move_iterator(returned.begin()), std::make_move_iterator(returned.end()));
}

class MockExternalUseClient : public ExternalUseClient {
public:
    MockExternalUseClient() = default;
    MOCK_METHOD1(ReleaseImageContexts, gpu::SyncToken(std::vector<std::unique_ptr<ImageContext>> image_contexts));
    MOCK_METHOD7(CreateImageContext,
        std::unique_ptr<ImageContext>(
            const gpu::MailboxHolder&, const gfx::Size&, SharedImageFormat, bool, const std::optional<gpu::VulkanYCbCrInfo>&, sk_sp<SkColorSpace>, bool));
};
...
public:
    ...
    static ReturnCallback GetReturnCallback(std::vector<ReturnedResource>* array)
    {
    ...
    TransferableResource CreateResource()
    {
        ...
        TransferableResource gl_resource
            = TransferableResource::MakeGpu(gpu_mailbox, GL_TEXTURE_2D, sync_token, size, SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
        return gl_resource;
    }

    gpu::SyncToken GenSyncToken()
    {
        gpu::SyncToken sync_token(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x123), next_fence_sync_++);
...
{
    gpu::SyncToken sync_token1(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x123), 0x42);
    auto mailbox = gpu::Mailbox::Generate();
    constexpr gfx::Size size(64, 64);
    TransferableResource gl_resource
        = TransferableResource::MakeGpu(mailbox, GL_TEXTURE_2D, sync_token1, size, SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
    ResourceId id1 = child_resource_provider_->ImportResource(gl_resource, base::DoNothing());
    std::vector<ReturnedResource> returned_to_child;
    int child_id = resource_provider_->CreateChild(GetReturnCallback(&returned_to_child), SurfaceId());

    // Transfer some resources to the parent.
    std::vector<TransferableResource> list;
    child_resource_provider_->PrepareSendToParent({ id1 }, &list, static_cast<RasterContextProvider*>(child_context_provider_.get()));
    ...
    auto owned_image_context = std::make_unique<ExternalUseClient::ImageContext>(gpu::MailboxHolder(mailbox, sync_token1, GL_TEXTURE_2D), size, format,
        /*ycbcr_info=*/std::nullopt, /*color_space=*/nullptr);
    auto* image_context = owned_image_context.get();

    gpu::MailboxHolder holder;
    EXPECT_CALL(client_, CreateImageContext(_, _, _, _, _, _, _)).WillOnce(DoAll(SaveArg<0>(&holder), Return(ByMove(std::move(owned_image_context)))));

    ExternalUseClient::ImageContext* locked_image_context = lock_set_->LockResource(parent_id, /*maybe_concurrent_reads=*/true,
    ...
    gpu::SyncToken sync_token2(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x234), 0x456);
    sync_token2.SetVerifyFlush();

    gpu::SyncToken sync_token3(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(0x234), 0x567);
    sync_token3.SetVerifyFlush();
    // We will get a second release of |parent_id| now that we've released our
    // external lock.
    EXPECT_CALL(client_, ReleaseImageContexts(testing::ElementsAre(SamePtr(locked_image_context)))).WillOnce(Return(sync_token3));
    ...
    child_resource_provider_->ReceiveReturnsFromParent(std::move(returned_to_child));
...
public:
    explicit TestGpuCommandsCompletedFence(DisplayResourceProviderSkia* resource_provider)
...
TEST_F(DisplayResourceProviderSkiaTest, ResourceFenceStopsReturnToChildOrDelete)
{
    const std::vector<TransferableResource::SynchronizationType> kSynchronizationTypes
        = { TransferableResource::SynchronizationType::kGpuCommandsCompleted, TransferableResource::SynchronizationType::kReleaseFence };
    for (auto sync_type : kSynchronizationTypes) {
        ...
        ResourceId id1 = child_resource_provider_->ImportResource(tran1, base::BindOnce(&MockReleaseCallback::Released, base::Unretained(&release)));

        TransferableResource tran2 = CreateResource();
        ASSERT_EQ(tran2.synchronization_type, TransferableResource::SynchronizationType::kSyncToken);
        ResourceId id2 = child_resource_provider_->ImportResource(tran2, base::BindOnce(&MockReleaseCallback::Released, base::Unretained(&release)));
        ...
        std::vector<TransferableResource> list;
        child_resource_provider_->PrepareSendToParent({ id1, id2 }, &list, static_cast<RasterContextProvider*>(child_context_provider_.get()));
        ...
        if (sync_type == TransferableResource::SynchronizationType::kGpuCommandsCompleted) {
            fence = base::MakeRefCounted<TestGpuCommandsCompletedFence>(resource_provider_.get());
            gpu_commands_completed_fence = static_cast<TestGpuCommandsCompletedFence*>(fence.get());
            resource_provider_->SetGpuCommandsCompletedFence(fence.get());
        } else {
            ASSERT_EQ(TransferableResource::SynchronizationType::kReleaseFence, sync_type);
...
TEST_F(DisplayResourceProviderSkiaTest, ResourceFenceOutlivesResourceProvider)
{
    ...
    TransferableResource tran1 = CreateResource();
    tran1.synchronization_type = TransferableResource::SynchronizationType::kGpuCommandsCompleted;
    ResourceId id1 = child_resource_provider_->ImportResource(tran1, base::BindOnce(&MockReleaseCallback::Released, base::Unretained(&release)));

    TransferableResource tran2 = CreateResource();
    tran2.synchronization_type = TransferableResource::SynchronizationType::kReleaseFence;
    ResourceId id2 = child_resource_provider_->ImportResource(tran2, base::BindOnce(&MockReleaseCallback::Released, base::Unretained(&release)));
    ...
    scoped_refptr<TestGpuCommandsCompletedFence> gpu_commands_completed_fence = base::MakeRefCounted<TestGpuCommandsCompletedFence>(resource_provider_.get());
    resource_provider_->SetGpuCommandsCompletedFence(gpu_commands_completed_fence.get());

    scoped_refptr<TestReleaseFence> release_fence = base::MakeRefCounted<TestReleaseFence>(resource_provider_.get());
...
TEST_F(DisplayResourceProviderSkiaTest, ScopedBatchReturnResourcesPreventsReturn)
{
    MockReleaseCallback release;
    ...
    for (auto& id : ids) {
        ...
        id = child_resource_provider_->ImportResource(tran, base::BindOnce(&MockReleaseCallback::Released, base::Unretained(&release)));
    }
    std::vector<ResourceId> resource_ids_to_transfer(ids, ids + kTotalResources);

    std::vector<TransferableResource> list;
    child_resource_provider_->PrepareSendToParent(resource_ids_to_transfer, &list, static_cast<RasterContextProvider*>(child_context_provider_.get()));
    ...
    // In DisplayResourceProvider's namespace, use the mapped resource id.
    std::unordered_map<ResourceId, ResourceId, ResourceIdHasher> resource_map = resource_provider_->GetChildToParentMap(child_id);
    std::vector<std::unique_ptr<DisplayResourceProvider::ScopedReadLockSharedImage>> read_locks;
    ...
    // batch.
    {
        DisplayResourceProvider::ScopedBatchReturnResources returner(resource_provider_.get());
        resource_provider_->DeclareUsedResourcesFromChild(child_id, ResourceIdSet(ids, ids + kUsedResources));
        EXPECT_EQ(0u, returned_to_child.size());
    }
    EXPECT_EQ(1u, returned_to_child.size());
    child_resource_provider_->ReceiveReturnsFromParent(std::move(returned_to_child));
    ...
    {
        DisplayResourceProvider::ScopedBatchReturnResources returner(resource_provider_.get());
        resource_provider_->DeclareUsedResourcesFromChild(child_id, ResourceIdSet(ids + kLockedResources, ids + kUsedResources));
