...
// static
void OverlayProcessorInterface::RecordOverlayDamageRectHistograms(bool is_overlay, bool has_occluding_surface_damage, bool zero_damage_rect)
{
    if (is_overlay) {
        UMA_HISTOGRAM_BOOLEAN("Viz.DisplayCompositor.RootDamageRect.Overlay", !zero_damage_rect);
    } else { // underlay
        ...
        UMA_HISTOGRAM_ENUMERATION("Viz.DisplayCompositor.RootDamageRect.Underlay", underlay_damage);
    }
}

OverlayProcessorInterface::OutputSurfaceOverlayPlane::OutputSurfaceOverlayPlane() = default;
OverlayProcessorInterface::OutputSurfaceOverlayPlane::OutputSurfaceOverlayPlane(const OutputSurfaceOverlayPlane&) = default;
OverlayProcessorInterface::OutputSurfaceOverlayPlane& OverlayProcessorInterface::OutputSurfaceOverlayPlane::operator=(const OutputSurfaceOverlayPlane&)
    = default;
OverlayProcessorInterface::OutputSurfaceOverlayPlane::~OutputSurfaceOverlayPlane() = default;

std::unique_ptr<OverlayProcessorInterface> OverlayProcessorInterface::CreateOverlayProcessor(OutputSurface* output_surface, gpu::SurfaceHandle surface_handle,
    const OutputSurface::Capabilities& capabilities, DisplayCompositorMemoryAndTaskController* display_controller,
    gpu::SharedImageInterface* shared_image_interface, const RendererSettings& renderer_settings, const DebugRendererSettings* debug_settings)
{
...
//             capabilities.allowed_yuv_overlay_count, display_controller->skia_dependency()->GetGpuDriverBugWorkarounds().disable_video_overlay_if_moving));
//     *(int*)1 = 1;
//     return nullptr;
// 
// #elif BUILDFLAG(IS_OZONE)
...
//     if (overlay_manager) {
//         overlay_candidates = overlay_manager->CreateOverlayCandidates(surface_handle);
...
//     }
// 
//     return std::make_unique<OverlayProcessorOzone>(std::move(overlay_candidates), std::move(renderer_settings.overlay_strategies), sii);
// 
// #elif BUILDFLAG(IS_ANDROID)
//     DCHECK(display_controller);
// 
...
OverlayProcessorInterface::OutputSurfaceOverlayPlane OverlayProcessorInterface::ProcessOutputSurfaceAsOverlay(const gfx::Size& viewport_size,
    const gfx::Size& resource_size, const SharedImageFormat si_format, const gfx::ColorSpace& color_space, bool has_alpha, float opacity,
    const gpu::Mailbox& mailbox)
{
    ...
    overlay_plane.uv_rect = gfx::RectF(
        0.f, 0.f, viewport_size.width() / static_cast<float>(resource_size.width()), viewport_size.height() / static_cast<float>(resource_size.height()));
    ...
    overlay_plane.display_rect = gfx::RectF(viewport_size.width(), viewport_size.height());
...
void OverlayProcessorInterface::ScheduleOverlays(DisplayResourceProvider* display_resource_provider)
{
}
