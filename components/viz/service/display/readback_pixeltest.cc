...
{
    return cc::FuzzyPixelComparator().SetErrorPixelsPercentageLimit(100.f).SetAvgAbsErrorLimit(kAvgAbsoluteErrorLimit).SetAbsErrorLimit(kMaxAbsoluteErrorLimit);
}
#endif
...
SharedQuadState* CreateSharedQuadState(AggregatedRenderPass* render_pass, const gfx::Rect& rect)
{
    const gfx::Rect layer_rect = rect;
    const gfx::Rect visible_layer_rect = rect;
    SharedQuadState* shared_state = render_pass->CreateAndAppendSharedQuadState();
    shared_state->SetAll(gfx::Transform(), layer_rect, visible_layer_rect, gfx::MaskFilterInfo(), /*clip=*/std::nullopt,
        /*contents_opaque=*/false, /*opacity_f=*/1.0f, SkBlendMode::kSrcOver,
        /*sorting_context=*/0,
...
void DeleteSharedImage(scoped_refptr<RasterContextProvider> context_provider, scoped_refptr<gpu::ClientSharedImage> client_shared_image,
...
struct ReadbackTextureInfo {
    ReadbackTextureInfo(const gfx::Size& size, SkColorType color_type, SkBitmap& out_bitmap)
        : size(size)
...
void ReadbackTexturesOnGpuThread(gpu::SharedImageManager* shared_image_manager, gpu::SharedContextState* context_state, const gpu::Mailbox& mailbox,
    base::span<ReadbackTextureInfo> readback_texture_infos)
{
    ...
    auto representation = shared_image_manager->ProduceSkia(mailbox, context_state->memory_type_tracker(), context_state);
    ...
    auto scoped_write = representation->BeginScopedWriteAccess(
        /*final_msaa_count=*/1, surface_props, &begin_semaphores, &end_semaphores, gpu::SharedImageRepresentation::AllowUnclearedAccess::kYes);

    context_state->gr_context()->wait(begin_semaphores.size(), begin_semaphores.data());
    ...
    for (size_t i = 0; i < readback_texture_infos.size(); i++) {
        ...
        DCHECK_EQ(static_cast<size_t>(out_bitmap.width() * out_bitmap.bytesPerPixel()), out_bitmap.rowBytes());
        SkPixmap pixmap(
            SkImageInfo::Make(texture_size.width(), texture_size.height(), color_type, representation->alpha_type()), out_bitmap.getAddr(0, 0), row_bytes);
    ...
    gpu::AddVulkanCleanupTaskForSkiaFlush(context_state->vk_context_provider(), &flush_info);
...
void ReadbackNV12Planes(
    TestGpuServiceHolder* gpu_service_holder, CopyOutputResult& result, const gfx::Size& texture_size, SkBitmap& out_luma_plane, SkBitmap& out_chroma_planes)
{
    ...
    // avoid contention.
    gpu_service_holder->ScheduleCompositorGpuTask(base::BindLambdaForTesting([&out_luma_plane, &out_chroma_planes, &result, &wait, &texture_size]() {
        auto* shared_image_manager = TestGpuServiceHolder::GetInstance()->gpu_service()->shared_image_manager();
        auto* context_state = TestGpuServiceHolder::GetInstance()->GetCompositorGpuThreadSharedContextState().get();

        std::vector<ReadbackTextureInfo> texture_infos;
        texture_infos.emplace_back(texture_size, kAlpha_8_SkColorType, out_luma_plane);
        texture_infos.emplace_back(gfx::Size(texture_size.width() / 2, texture_size.height() / 2), kR8G8_unorm_SkColorType, out_chroma_planes);

        ReadbackTexturesOnGpuThread(shared_image_manager, context_state, result.GetTextureResult()->mailbox, texture_infos);
...
void ReadbackResultRGBA(
    TestGpuServiceHolder* gpu_service_holder, bool is_software, CopyOutputResult& result, const gfx::Size& texture_size, SkBitmap& out_plane)
{
    ...
    if (is_software) {
        ...
        auto representation = gpu_service_holder->gpu_service()->shared_image_manager()->ProduceMemory(mailbox, memory_tracker.get());
        auto access = representation->BeginScopedReadAccess();
        memcpy(out_plane.pixmap().writable_addr(), access->pixmap().addr(), out_plane.pixmap().computeByteSize());
        return;
    }
    ...
    // image so it's better we issue the readback on the compositor GPU thread to
    // avoid contention.
    gpu_service_holder->ScheduleCompositorGpuTask(base::BindLambdaForTesting([&out_plane, &mailbox, &wait, &texture_size]() {
        auto* shared_image_manager = TestGpuServiceHolder::GetInstance()->gpu_service()->shared_image_manager();
        auto* context_state = TestGpuServiceHolder::GetInstance()->GetCompositorGpuThreadSharedContextState().get();

        std::vector<ReadbackTextureInfo> texture_infos;
        texture_infos.emplace_back(texture_size, kRGBA_8888_SkColorType, out_plane);

        ReadbackTexturesOnGpuThread(shared_image_manager, context_state, mailbox, texture_infos);
...
std::vector<uint8_t> GeneratePixels(size_t num_bytes, base::span<const uint8_t> pattern)
{
...
public:
    explicit ReadbackPixelTest(RendererType type)
        : VizPixelTest(type)
    {
    }
    ...
    {
        DCHECK(is_initialized_);
        gfx::Rect result(source_size_.width() / 2, source_size_.height() / 2, source_size_.width() / 4, source_size_.height() / 4);
...
protected:
    ...
    {
        return GetTestFilePath(scale_by_half_ ? FILE_PATH_LITERAL("half_of_one_of_16_color_rects.png") : FILE_PATH_LITERAL("one_of_16_color_rects.png"));
    }
    ...
    void SetUpReadbackPixeltest(bool scale_by_half)
    {
        ...
        source_bitmap_ = cc::ReadPNGFile(GetTestFilePath(FILE_PATH_LITERAL("16_color_rects.png")));
    ...
    std::unique_ptr<CopyOutputResult> IssueCopyOutputRequestAndRender(CopyOutputRequest::ResultFormat format, CopyOutputRequest::ResultDestination destination,
        base::OnceCallback<void(CopyOutputRequest&)> configure_request)
    {
        ...
        std::unique_ptr<CopyOutputResult> result;
        {
            ...
            base::RunLoop loop;
            auto request = std::make_unique<CopyOutputRequest>(format, destination,
                base::BindOnce(
                    [](std::unique_ptr<CopyOutputResult>* result_out, base::OnceClosure quit_closure, std::unique_ptr<CopyOutputResult> result_from_copier) {
            ...
            renderer_->DrawFrame(&pass_list, 1.0f, gfx::Size(bitmap.width(), bitmap.height()), gfx::DisplayColorSpaces(), std::move(surface_damage_rect_list));
    ...
    scoped_refptr<gpu::ClientSharedImage> CreateSharedImageWithPixels(
        SharedImageFormat format, gfx::Size size, const gfx::ColorSpace color_space, base::span<const uint8_t> pixels)
    {
        auto* sii = child_context_provider_->SharedImageInterface();
        CHECK(sii);

        if (is_software_renderer()) {
            auto result = sii->CreateSharedImage({ format, size, color_space, gpu::SHARED_IMAGE_USAGE_CPU_WRITE, "TestLabels" });
            memcpy(result.mapping.memory(), pixels.data(), pixels.size());
            return result.shared_image;
        } else {
            return sii->CreateSharedImage({ format, size, color_space, gpu::SHARED_IMAGE_USAGE_DISPLAY_READ, "TestLabels" }, pixels);
        }
    }

private:
    // Creates a RenderPass that embeds a single quad containing |bitmap|.
    std::unique_ptr<AggregatedRenderPass> GenerateRootRenderPass(const SkBitmap& bitmap)
    {
        const gfx::Size source_size = gfx::Size(bitmap.width(), bitmap.height());

        SharedImageFormat format = (bitmap.info().colorType() == kBGRA_8888_SkColorType) ? SinglePlaneFormat::kBGRA_8888 : SinglePlaneFormat::kRGBA_8888;
        ResourceId resource_id = CreateSharedImageResource(source_size, format, MakePixelSpan(bitmap));

        std::unordered_map<ResourceId, ResourceId, ResourceIdHasher> resource_map
            = cc::SendResourceAndGetChildToParentMap({ resource_id }, resource_provider_.get(), child_resource_provider_.get(), child_context_provider_.get());
        ResourceId mapped_resource_id = resource_map[resource_id];

        const gfx::Rect output_rect(source_size);
        auto pass = std::make_unique<AggregatedRenderPass>();
        pass->SetNew(AggregatedRenderPassId { 1 }, output_rect, output_rect, gfx::Transform());

        SharedQuadState* sqs = CreateSharedQuadState(pass.get(), output_rect);

        auto* quad = pass->CreateAndAppendDrawQuad<TileDrawQuad>();
        quad->SetNew(sqs, output_rect, output_rect, /*needs_blending=*/false, mapped_resource_id, gfx::RectF(output_rect), source_size,
    ...
    // resource id for it.
    ResourceId CreateSharedImageResource(const gfx::Size& size, SharedImageFormat format, base::span<const uint8_t> pixels)
    {
        scoped_refptr<gpu::ClientSharedImage> client_shared_image = CreateSharedImageWithPixels(format, size, gfx::ColorSpace(), pixels);
        gpu::SyncToken sync_token = child_context_provider_->SharedImageInterface()->GenUnverifiedSyncToken();

        TransferableResource resource = is_software_renderer() ? TransferableResource::MakeSoftwareSharedImage(client_shared_image, sync_token, size, format)
                                                               : TransferableResource::MakeGpu(client_shared_image, GL_TEXTURE_2D, sync_token, size, format,
                                                                   /*is_overlay_candidate=*/false);

        auto release_callback = base::BindOnce(&DeleteSharedImage, child_context_provider_, std::move(client_shared_image));
        return child_resource_provider_->ImportResource(resource, std::move(release_callback));
    }
...
class ReadbackPixelTestRGBA : public ReadbackPixelTest, public testing::WithParamInterface<std::tuple<RendererType, bool, CopyOutputResult::Destination>> {
public:
...
TEST_P(ReadbackPixelTestRGBA, ExecutesCopyRequest)
{
    ...
    std::unique_ptr<CopyOutputResult> result = IssueCopyOutputRequestAndRender(
        RequestFormat(), RequestDestination(), base::BindLambdaForTesting([this, &result_selection](CopyOutputRequest& request) {
    ...
    case CopyOutputResult::Destination::kNativeTextures: {
        ...
        actual.allocPixels(SkImageInfo::Make(size.width(), size.height(), kRGBA_8888_SkColorType, kUnpremul_SkAlphaType));
        ReadbackResultRGBA(gpu_service_holder_, is_software_renderer(), *result, result->size(), actual);
        break;
    }
#endif
    default:
    ...
    if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kRebaselinePixelTests)) {
...
INSTANTIATE_TEST_SUITE_P(, ReadbackPixelTestRGBA,
...
#else
        testing::Values(CopyOutputResult::Destination::kSystemMemory, CopyOutputResult::Destination::kNativeTextures)));
#endif

#if !BUILDFLAG(IS_ANDROID)
class ReadbackPixelTestRGBAWithBlit : public ReadbackPixelTest, public testing::WithParamInterface<std::tuple<RendererType, bool, LetterboxingBehavior, bool>> {
public:
...
TEST_P(ReadbackPixelTestRGBAWithBlit, ExecutesCopyRequestWithBlit)
{
    ...
    std::vector<uint8_t> pixels = GeneratePixels(format.EstimatedSizeInBytes(source_size), pattern);
    scoped_refptr<gpu::ClientSharedImage> blit_dest_shared_image = CreateSharedImageWithPixels(format, source_size, color_space, pixels);

    ASSERT_TRUE(blit_dest_shared_image);
    gpu::Mailbox mailbox = blit_dest_shared_image->mailbox();

    std::unique_ptr<CopyOutputResult> result = IssueCopyOutputRequestAndRender(RequestFormat(), RequestDestination(),
        base::BindLambdaForTesting([this, &result_selection, &destination_subregion, &mailbox](CopyOutputRequest& request) {
            // Build CopyOutputRequest based on test parameters.
            if (ScaleByHalf()) {
                request.SetUniformScaleRatio(2, 1);
            }
            ...
            request.set_blit_request(
                BlitRequest(destination_subregion.origin(), GetLetterboxingBehavior(), mailbox, gpu::SyncToken(), populates_gpu_memory_buffer()));
        }));
    ...
    ASSERT_EQ(result->destination(), CopyOutputResult::Destination::kNativeTextures);
    ...
    SkBitmap actual;
    actual.allocPixels(SkImageInfo::Make(source_size.width(), source_size.height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType));
    ReadbackResultRGBA(gpu_service_holder_, is_software_renderer(), *result, source_size, actual);
    ...
    SkBitmap expected_subregion = GLScalerTestUtil::CopyAndConvertToRGBA(GetExpectedOutputBitmap());
    ...
    expected.writePixels(expected_subregion.pixmap(), destination_subregion.x(), destination_subregion.y());

    EXPECT_TRUE(cc::MatchesBitmap(actual, expected, GetDefaultFuzzyPixelComparator()));
}

INSTANTIATE_TEST_SUITE_P(, ReadbackPixelTestRGBAWithBlit,
    testing::Combine(testing::Values(RendererType::kSoftware, RendererType::kSkiaGL),
        testing::Bool(), // Result scaling: Scale by half?
        testing::Values(LetterboxingBehavior::kDoNotLetterbox, LetterboxingBehavior::kLetterbox),
...
class ReadbackPixelTestNV12 : public ReadbackPixelTest, public testing::WithParamInterface<std::tuple<RendererType, bool, CopyOutputResult::Destination>> {
public:
...
TEST_P(ReadbackPixelTestNV12, ExecutesCopyRequest)
{
    ...
    // even.
    ASSERT_TRUE(result_selection.width() % 2 == 0 && result_selection.height() % 2 == 0)
        << " request size is not even, result_selection.size()=" << result_selection.size().ToString();

    // Additionally, the test uses helpers that assume pixel data can be packed (4
    // 8-bit values in 1 32-bit pixel).
    ASSERT_TRUE(result_selection.width() % 4 == 0) << " request width is not divisible by 4, result_selection.width()=" << result_selection.width();
    ...
    // Packed plane sizes:
    const gfx::Size luma_plane_size = gfx::Size(result->size().width() / 4, result->size().height());
    const gfx::Size chroma_planes_size = gfx::Size(luma_plane_size.width(), luma_plane_size.height() / 2);
    ...
    if (RequestDestination() == CopyOutputResult::Destination::kSystemMemory) {
        ...
        result->ReadNV12Planes(static_cast<uint8_t*>(luma_plane.getAddr(0, 0)), result->size().width(), static_cast<uint8_t*>(chroma_planes.getAddr(0, 0)),
            result->size().width());
    } else {
        luma_plane = GLScalerTestUtil::AllocateRGBABitmap(luma_plane_size);
        chroma_planes = GLScalerTestUtil::AllocateRGBABitmap(chroma_planes_size);

        ReadbackNV12Planes(gpu_service_holder_, *result, result->size(), luma_plane, chroma_planes);
    }
    ...
    SkBitmap expected = GLScalerTestUtil::CopyAndConvertToRGBA(GetExpectedOutputBitmap());
    GLScalerTestUtil::ConvertRGBABitmapToYUV(&expected);

    EXPECT_TRUE(cc::MatchesBitmap(actual, expected, GetDefaultFuzzyPixelComparator()));
}

INSTANTIATE_TEST_SUITE_P(, ReadbackPixelTestNV12,
    testing::Combine(testing::Values(RendererType::kSkiaGL),
        // Result scaling: Scale by half?
        testing::Values(true, false), testing::Values(CopyOutputResult::Destination::kSystemMemory, CopyOutputResult::Destination::kNativeTextures)));

class ReadbackPixelTestNV12WithBlit : public ReadbackPixelTest, public testing::WithParamInterface<std::tuple<RendererType, bool, LetterboxingBehavior, bool>> {
public:
...
TEST_P(ReadbackPixelTestNV12WithBlit, ExecutesCopyRequestWithBlit)
{
    ...
    ASSERT_TRUE(destination_subregion.x() % 2 == 0 && destination_subregion.y() % 2 == 0)
        << " The test case expects the blit region's origin to be even for NV12 "
           "blit requests";

    const SkColor4f yuv_red = { 0.245331, 0.399356, 0.939216, 1.0 };

    const std::vector<uint8_t> luma_pattern = { static_cast<uint8_t>(yuv_red.fR * 255.0f) };
    const std::vector<uint8_t> chromas_pattern = { static_cast<uint8_t>(yuv_red.fG * 255.0f), static_cast<uint8_t>(yuv_red.fB * 255.0f) };
    ...
    for (size_t i = 0; i < CopyOutputResult::kNV12MaxPlanes; ++i) {
        const gfx::Size plane_size = i == 0 ? source_size : gfx::Size(source_size.width() / 2, source_size.height() / 2);
        const size_t plane_size_in_bytes = plane_size.GetArea() * (i == 0 ? 1 : 2);

        pixels[i] = (i == 0) ? GeneratePixels(plane_size_in_bytes, luma_pattern) : GeneratePixels(plane_size_in_bytes, chromas_pattern);

        auto color_type = i == 0 ? kAlpha_8_SkColorType : kR8G8_unorm_SkColorType;
        size_t row_bytes = plane_size.width() * (i == 0 ? 1 : 2);
        pixmaps[i] = SkPixmap(SkImageInfo::Make(plane_size.width(), plane_size.height(), color_type, kUnpremul_SkAlphaType), pixels[i].data(), row_bytes);
    }

    auto shared_image = sii->CreateSharedImage(
        { MultiPlaneFormat::kNV12, source_size, gfx::ColorSpace::CreateREC709(), gpu::SHARED_IMAGE_USAGE_DISPLAY_READ, "TestLabels" }, gpu::kNullSurfaceHandle);
    CHECK(shared_image);
    ...
    SkYUVAInfo info = SkYUVAInfo({ source_size.width(), source_size.height() }, SkYUVAInfo::PlaneConfig::kY_UV, SkYUVAInfo::Subsampling::k420,
    ...
    std::unique_ptr<CopyOutputResult> result = IssueCopyOutputRequestAndRender(RequestFormat(), RequestDestination(),
        base::BindLambdaForTesting([this, &result_selection, &destination_subregion, &mailbox_holder](CopyOutputRequest& request) {
            // Build CopyOutputRequest based on test parameters.
            if (ScaleByHalf()) {
                request.SetUniformScaleRatio(2, 1);
            }
            ...
            request.set_blit_request(BlitRequest(
                destination_subregion.origin(), GetLetterboxingBehavior(), mailbox_holder.mailbox, mailbox_holder.sync_token, populates_gpu_memory_buffer()));
        }));
    ...
    // textures is caller-controlled, and we have issued a COR w/ blit request
    // that is supposed to write to an image of |source_size| size.
    const gfx::Size luma_plane_size = gfx::Size(source_size.width() / 4, source_size.height());
    const gfx::Size chroma_planes_size = gfx::Size(luma_plane_size.width(), luma_plane_size.height() / 2);

    SkBitmap luma_plane = GLScalerTestUtil::AllocateRGBABitmap(luma_plane_size);
    SkBitmap chroma_planes = GLScalerTestUtil::AllocateRGBABitmap(chroma_planes_size);

    ReadbackNV12Planes(gpu_service_holder_, *result, source_size, luma_plane, chroma_planes);
...
INSTANTIATE_TEST_SUITE_P(, ReadbackPixelTestNV12WithBlit,
    testing::Combine(testing::Values(RendererType::kSkiaGL),
        testing::Bool(), // Result scaling: Scale by half?
        testing::Values(LetterboxingBehavior::kDoNotLetterbox, LetterboxingBehavior::kLetterbox),
        testing::Bool() // Should behave as if COR is populating a GMB?
