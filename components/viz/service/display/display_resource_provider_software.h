...
class VIZ_SERVICE_EXPORT DisplayResourceProviderSoftware : public DisplayResourceProvider {
public:
    explicit DisplayResourceProviderSoftware(SharedBitmapManager* shared_bitmap_manager, gpu::SharedImageManager* shared_image_manager,
    ...
    public:
        ScopedReadLockSkImage(DisplayResourceProviderSoftware* resource_provider, ResourceId resource_id, SkAlphaType alpha_type);
        ~ScopedReadLockSkImage();

        ScopedReadLockSkImage(const ScopedReadLockSkImage&) = delete;
        ScopedReadLockSkImage& operator=(const ScopedReadLockSkImage& other) = delete;

        const SkImage* sk_image() const
        {
            return sk_image_.get();
        }
        sk_sp<SkImage> TakeSkImage()
        {
            return std::move(sk_image_);
        }
    ...
    std::unique_ptr<gpu::MemoryImageRepresentation> GetSharedImageRepresentation(const gpu::Mailbox& mailbox, const gpu::SyncToken& sync_token);
...
private:
    ...
    std::vector<ReturnedResource> DeleteAndReturnUnusedResourcesToChildImpl(
        Child& child_info, DeleteStyle style, const std::vector<ResourceId>& unused) override;

    void PopulateSkBitmapWithResource(SkBitmap* sk_bitmap, const ChildResource* resource, SkAlphaType alpha_type);
...
//         std::unique_ptr<gpu::MemoryImageRepresentation::ScopedReadAccess> read_access;
    };

    base::flat_map<ResourceId, std::unique_ptr<SharedImageAccess>> resource_shared_images_;
};
