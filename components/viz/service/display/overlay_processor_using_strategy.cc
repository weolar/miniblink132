...
// why we aren't.
enum class PromotingMaskCandidates { kYes = 0, kNoNotRequired = 1, kNoMultipleOverlaysDisabled = 2, kNoDrmRejected = 3, kMaxValue = kNoDrmRejected };

constexpr char kNumOverlaysPromotedHistogramName[] = "Compositing.Display.OverlayProcessorUsingStrategy.NumOverlaysPromoted";
constexpr char kNumOverlaysAttemptedHistogramName[] = "Compositing.Display.OverlayProcessorUsingStrategy.NumOverlaysAttempted";
constexpr char kNumOverlaysFailedHistogramName[] = "Compositing.Display.OverlayProcessorUsingStrategy.NumOverlaysFailed";
constexpr char kWorkingScaleFactorHistogramName[] = "Compositing.Display.OverlayProcessorUsingStrategy."
                                                    "WorkingScaleFactorForRequiredOverlays";
constexpr char kFramesAttemptingRequiredOverlaysHistogramName[] = "Compositing.Display.OverlayProcessorUsingStrategy."
                                                                  "FramesAttemptingRequiredOverlays";
constexpr char kFramesScalingRequiredOverlaysHistogramName[] = "Compositing.Display.OverlayProcessorUsingStrategy."
                                                               "FramesScalingRequiredOverlays";
constexpr char kFramesWithMaskCandidatesRequireOverlaysHistogramName[] = "Compositing.Display.OverlayProcessorUsingStrategy."
                                                                         "FramesWithMaskCandidatesRequireOverlays";
constexpr char kFramesWithMaskCandidatesHistogramName[] = "Compositing.Display.OverlayProcessorUsingStrategy."
                                                          "FramesWithMaskCandidates";
constexpr char kShouldPromoteCandidatesWithMasksHistogramName[] = "Compositing.Display.OverlayProcessorUsingStrategy."
                                                                  "ShouldPromoteCandidatesWithMasks";

using OverlayProposedCandidateIndex = std::vector<OverlayProposedCandidate>::size_type;
using ConstOverlayProposedCandidateIterator = std::vector<OverlayProposedCandidate>::const_iterator;

static void LogShouldPromoteCandidatesWithMasksEnumUMA(PromotingMaskCandidates attempt)
{
    UMA_HISTOGRAM_ENUMERATION(kShouldPromoteCandidatesWithMasksHistogramName, attempt);
}

static void LogFramesWithMaskCandidatesBoolUMA(const std::vector<OverlayProposedCandidate>& proposed_candidates)
{
    const bool have_mask_candidates = std::any_of(proposed_candidates.cbegin(), proposed_candidates.cend(),
        [](const OverlayProposedCandidate& candidate) { return candidate.candidate.has_rounded_display_masks; });

    UMA_HISTOGRAM_BOOLEAN(kFramesWithMaskCandidatesHistogramName, have_mask_candidates);
}
...
ConstOverlayProposedCandidateIterator MaybeAppendOccludingMaskCandidates(ConstOverlayProposedCandidateIterator candidates_wth_masks_begin,
    ConstOverlayProposedCandidateIterator candidates_wth_masks_end, std::vector<OverlayProposedCandidate>& test_candidates)
{
    // Keep track of the starting index of mask candidates in test_candidates`
    // list.
    OverlayProposedCandidateIndex begin_mask_candidates_index = test_candidates.size();

    bool appended_mask_candidates = false;
    for (auto& it = candidates_wth_masks_begin; it < candidates_wth_masks_end; it++) {
        auto mask_key = OverlayProposedCandidate::ToProposeKey(*it);
        for (OverlayProposedCandidateIndex i = 0; i < begin_mask_candidates_index; i++) {
            ...
            if (keys.contains(mask_key)) {
                ...
                LogShouldPromoteCandidatesWithMasksEnumUMA(PromotingMaskCandidates::kNoNotRequired);
            }
        }
    }

    UMA_HISTOGRAM_BOOLEAN(kFramesWithMaskCandidatesRequireOverlaysHistogramName, appended_mask_candidates);
...
bool IsOccludedByMaskCandidates(const OverlayProposedCandidate& candidate, const std::vector<OverlayProposedCandidate*>& mask_candidates)
{
    if (candidate.occluding_mask_keys.empty()) {
        return false;
    }

    return base::ranges::any_of(mask_candidates.begin(), mask_candidates.end(),
        [&candidate](const auto& iter) { return candidate.occluding_mask_keys.contains(OverlayProposedCandidate::ToProposeKey(*iter)); });
}
...
OverlayTestResults ProcessOverlayTestResults(std::vector<OverlayProposedCandidate>& test_candidates)
{
    ...
    for (auto& it : test_candidates) {
        if (it.candidate.has_rounded_display_masks) {
            ...
            LogShouldPromoteCandidatesWithMasksEnumUMA(it.candidate.overlay_handled ? PromotingMaskCandidates::kYes : PromotingMaskCandidates::kNoDrmRejected);
        }
    ...
    for (auto& it : test_candidates) {
        if (it.strategy->GetUMAEnum() == OverlayStrategy::kSingleOnTop && !it.candidate.has_rounded_display_masks && it.candidate.overlay_handled) {
...
// limitations.
void ScaleCandidateSrcRect(const gfx::RectF& org_src_rect, float scale_factor, OverlayCandidate* candidate)
{
    ...
    src_rect.set_x(kSubsamplingFactor * (std::lround(src_rect.x()) / kSubsamplingFactor));
    src_rect.set_y(kSubsamplingFactor * (std::lround(src_rect.y()) / kSubsamplingFactor));
    src_rect.set_width(kSubsamplingFactor * (std::lround(src_rect.width()) / kSubsamplingFactor));
    src_rect.set_height(kSubsamplingFactor * (std::lround(src_rect.height()) / kSubsamplingFactor));
    // Scale it back into UV space and set it in the candidate.
    candidate->uv_rect = gfx::ScaleRect(src_rect, 1.0f / candidate->resource_size_in_pixels.width(), 1.0f / candidate->resource_size_in_pixels.height());
}

void SyncOverlayCandidates(
    std::vector<OverlayProposedCandidate>& proposed_candidates, std::vector<OverlayCandidate>& candidates, bool copy_from_proposed_candidates)
{
...
static void LogFramesAttemptingRequiredCandidateBoolUMA(const std::vector<OverlayProposedCandidate>& proposed_candidates)
{
    const bool have_required_overlay_candidates = std::any_of(proposed_candidates.cbegin(), proposed_candidates.cend(),
        [](const OverlayProposedCandidate& candidate) { return candidate.candidate.requires_overlay; });

    UMA_HISTOGRAM_BOOLEAN(kFramesAttemptingRequiredOverlaysHistogramName, have_required_overlay_candidates);
}

static void LogWorkingScaleFactorCountUMA(float scale_factor)
{
    UMA_HISTOGRAM_CUSTOM_COUNTS(kWorkingScaleFactorHistogramName, scale_factor * 100, /*minimum=*/1,
        /*maximum=*/201, /*bucket_count=*/50);
}

static void LogFramesScalingRequiredCandidateBoolUMA(bool attempted_scaling)
{
    UMA_HISTOGRAM_BOOLEAN(kFramesScalingRequiredOverlaysHistogramName, attempted_scaling);
}
...
gfx::Rect OverlayProcessorUsingStrategy::GetPreviousFrameOverlaysBoundingRect() const
{
...
void OverlayProcessorUsingStrategy::NotifyOverlayPromotion(
    DisplayResourceProvider* display_resource_provider, const CandidateList& candidates, const QuadList& quad_list)
{
}

void OverlayProcessorUsingStrategy::SetFrameSequenceNumber(uint64_t frame_sequence_number)
{
...
void OverlayProcessorUsingStrategy::ProcessForOverlays(DisplayResourceProvider* resource_provider, AggregatedRenderPassList* render_passes,
    const SkM44& output_color_matrix, const OverlayProcessorInterface::FilterOperationsMap& render_pass_filters,
    const OverlayProcessorInterface::FilterOperationsMap& render_pass_backdrop_filters, SurfaceDamageRectList surface_damage_rect_list,
    OutputSurfaceOverlayPlane* output_surface_plane, CandidateList* candidates, gfx::Rect* damage_rect, std::vector<gfx::Rect>* content_bounds)
{
    ...
    UMA_HISTOGRAM_COUNTS_1000("Compositing.Display.OverlayProcessorUsingStrategy.NumQuadsConsidered", render_pass->quad_list.size());
    ...
    if (!skip_because_copy_request && !disable_overlay()) {
        success = AttemptWithStrategies(output_color_matrix, render_pass_filters, render_pass_backdrop_filters, resource_provider, render_passes,
            &surface_damage_rect_list, output_surface_plane, candidates, content_bounds, damage_rect);
    }

    DCHECK(candidates->empty() || success);
    UMA_HISTOGRAM_COUNTS_100(kNumOverlaysPromotedHistogramName, candidates->size());

    UpdateOverlayStatusMap(*candidates);
    UpdateDamageRect(surface_damage_rect_list, *damage_rect);

    NotifyOverlayPromotion(resource_provider, *candidates, render_pass->quad_list);
    ...
    TRACE_COUNTER1(TRACE_DISABLED_BY_DEFAULT("viz.debug.overlay_planes"), "Scheduled overlay planes", candidates->size());
}

void OverlayProcessorUsingStrategy::CheckOverlaySupport(
    const OverlayProcessorInterface::OutputSurfaceOverlayPlane* primary_plane, OverlayCandidateList* candidate_list)
{
#if BUILDFLAG(IS_CHROMEOS_ASH)
    // TODO(b/181974042):  Remove when color space is plumbed.
    if (primary_plane)
...
gfx::Rect OverlayProcessorUsingStrategy::ComputeDamageExcludingOverlays(const SurfaceDamageRectList& surface_damage_rect_list, const gfx::Rect& existing_damage)
{
    ...
    std::sort(overlay_status_iters.begin(), overlay_status_iters.end(),
        [](const auto& it1, const auto& it2) { return it1->second.damage_index < it2->second.damage_index; });
    ...
    for (size_t i = 0; i < surface_damage_rect_list.size(); i++) {
        size_t curr_overlay_damage_index
            = overlay_status_it != overlay_status_iters.end() ? (*overlay_status_it)->second.damage_index : OverlayCandidate::kInvalidDamageIndex;
        if (curr_overlay_damage_index == i) {
            ...
            bool is_occluded
                = base::ranges::any_of(occluding_rects, [&curr_surface_damage](const gfx::Rect& occluder) { return occluder.Contains(curr_surface_damage); });
            if (!is_occluded) {
...
OverlayProcessorUsingStrategy::OverlayStatus::OverlayStatus(const OverlayCandidate& candidate, const gfx::Rect& key, const OverlayStatusMap& prev_overlays)
{
...
OverlayProcessorUsingStrategy::OverlayStatus::OverlayStatus(const OverlayStatus&) = default;
OverlayProcessorUsingStrategy::OverlayStatus& OverlayProcessorUsingStrategy::OverlayStatus::operator=(const OverlayStatus&) = default;
OverlayProcessorUsingStrategy::OverlayStatus::~OverlayStatus() = default;

void OverlayProcessorUsingStrategy::UpdateOverlayStatusMap(const OverlayCandidateList& candidates)
{
...
void OverlayProcessorUsingStrategy::UpdateDamageRect(const SurfaceDamageRectList& surface_damage_rect_list, gfx::Rect& damage_rect)
{
    DCHECK_LE(curr_overlays_.size(), static_cast<size_t>(max_overlays_considered_));

    // Remove all damage caused by these overlays, and any damage they occlude.
    damage_rect = ComputeDamageExcludingOverlays(surface_damage_rect_list, damage_rect);
    ...
    for (auto& [key, status] : curr_overlays_) {
        ...
        if ((!status.prev_was_underlay && status.is_underlay) || (status.prev_was_opaque && !status.is_opaque)
            || (status.is_new && (status.is_underlay || !status.is_opaque)) || (status.has_mask_filter != status.prev_has_mask_filter)) {
    ...
    for (auto it : curr_overlays_) {
        ...
        if (status.plane_z_order != 0) {
            RecordOverlayDamageRectHistograms(status.plane_z_order > 0, status.damage_area_estimate != 0.f, damage_rect.IsEmpty());
        }
    }
}

void OverlayProcessorUsingStrategy::AdjustOutputSurfaceOverlay(std::optional<OutputSurfaceOverlayPlane>* output_surface_plane)
{
    ...
    if (last_successful_strategy_ && last_successful_strategy_->RemoveOutputSurfaceAsOverlay())
        output_surface_plane->reset();
}

void OverlayProcessorUsingStrategy::SortProposedOverlayCandidates(std::vector<OverlayProposedCandidate>* proposed_candidates)
{
    // Removes trackers for candidates that are no longer being rendered.
    for (auto it = tracked_candidates_.begin(); it != tracked_candidates_.end();) {
    ...
    for (auto it = proposed_candidates->begin(); it != proposed_candidates->end();) {
        ...
        DBG_DRAW_TEXT_OPT("candidate.surface.id", DBG_OPT_GREEN, it->candidate.display_rect.origin(),
            base::StringPrintf("%X , %d", key.tracking_id, static_cast<int>(key.strategy_id)).c_str());
        DBG_DRAW_TEXT_OPT("candidate.mean.damage", DBG_OPT_GREEN, it->candidate.display_rect.origin(),
            base::StringPrintf(" %f, %f %d", track_data.MeanFrameRatioRate(tracker_config_), track_data.GetDamageRatioRate(),
        ...
        const bool force_update = it->candidate.overlay_damage_index != OverlayCandidate::kInvalidDamageIndex && it->candidate.damage_area_estimate != 0.f;
        track_data.AddRecord(
            frame_sequence_number_, it->candidate.damage_area_estimate / display_area, it->candidate.resource_id, tracker_config_, force_update);
        ...
        // candidate.
        int power_gained
            = track_data.GetModeledPowerGain(frame_sequence_number_, tracker_config_, display_area, it->strategy->GetUMAEnum() == OverlayStrategy::kFullscreen);
        bool passes_min_threshold = ((track_data.IsActivelyChanging(frame_sequence_number_, tracker_config_) || !prioritization_config_.changing_threshold)
        ...
        if (it->candidate.requires_overlay || it->candidate.has_rounded_display_masks || passes_min_threshold) {
    ...
    std::stable_sort(proposed_candidates->begin(), proposed_candidates->end(), [prio_config](const auto& a, const auto& b) {
...
bool OverlayProcessorUsingStrategy::AttemptWithStrategies(const SkM44& output_color_matrix,
    const OverlayProcessorInterface::FilterOperationsMap& render_pass_filters,
    const OverlayProcessorInterface::FilterOperationsMap& render_pass_backdrop_filters, const DisplayResourceProvider* resource_provider,
    AggregatedRenderPassList* render_pass_list, SurfaceDamageRectList* surface_damage_rect_list,
    OverlayProcessorInterface::OutputSurfaceOverlayPlane* primary_plane, OverlayCandidateList* candidates, std::vector<gfx::Rect>* content_bounds,
    gfx::Rect* incoming_damage)
{
    last_successful_strategy_ = nullptr;
    std::vector<OverlayProposedCandidate> proposed_candidates;
    for (const auto& strategy : strategies_) {
        strategy->Propose(output_color_matrix, render_pass_filters, render_pass_backdrop_filters, resource_provider, render_pass_list, surface_damage_rect_list,
            primary_plane, &proposed_candidates, content_bounds);
    }

    size_t num_proposed_pre_sort = proposed_candidates.size();
    UMA_HISTOGRAM_COUNTS_1000("Viz.DisplayCompositor.OverlayNumProposedCandidates", num_proposed_pre_sort);
    ...
    if (proposed_candidates.size() == 0) {
        LogStrategyEnumUMA(num_proposed_pre_sort != 0 ? OverlayStrategy::kNoStrategyFailMin : OverlayStrategy::kNoStrategyUsed);
    }
    ...
    if (ShouldAttemptMultipleOverlays(proposed_candidates)) {
        ...
        return AttemptMultipleOverlays(proposed_candidates, primary_plane, render_pass, *candidates);
    }

    std::for_each(candidates->cbegin(), candidates->cend(), [](const OverlayCandidate& candidate) {
        if (candidate.has_rounded_display_masks) {
            LogShouldPromoteCandidatesWithMasksEnumUMA(PromotingMaskCandidates::kNoMultipleOverlaysDisabled);
        }
    });
    ...
    for (auto&& candidate : proposed_candidates) {
        ...
        DrawQuad::Material quad_material
            = candidate.strategy->GetUMAEnum() != OverlayStrategy::kUnknown ? candidate.quad_iter->material : DrawQuad::Material::kInvalid;
        if (candidate.candidate.requires_overlay) {
            has_required_overlay = true;
        }

        bool used_overlay = candidate.strategy->Attempt(output_color_matrix, render_pass_filters, render_pass_backdrop_filters, resource_provider,
            render_pass_list, surface_damage_rect_list, primary_plane, candidates, content_bounds, candidate);
        if (!used_overlay && candidate.candidate.requires_overlay) {
            ...
            if (scale_factor < 1.0f) {
                ...
                gfx::RectF org_src_rect = gfx::ScaleRect(candidate.candidate.uv_rect, candidate.candidate.resource_size_in_pixels);
                for (float new_scale_factor = std::min(min_working_scale_, std::max(max_failed_scale_, scale_factor) + kScaleAdjust); new_scale_factor < 1.0f;
                     new_scale_factor += kScaleAdjust) {
                    ...
                    if (candidate.strategy->Attempt(output_color_matrix, render_pass_filters, render_pass_backdrop_filters, resource_provider, render_pass_list,
                            surface_damage_rect_list, primary_plane, candidates, content_bounds, candidate)) {
                        used_overlay = true;
                        break;
                    } else {
        ...
        if (used_overlay) {
            ...
            UMA_HISTOGRAM_ENUMERATION("Viz.DisplayCompositor.OverlayQuadMaterial", quad_material);
            if (candidate.candidate.requires_overlay) {
                ...
                LogFramesScalingRequiredCandidateBoolUMA(attempted_scaling_required_overlays);
            }
...
bool OverlayProcessorUsingStrategy::ShouldAttemptMultipleOverlays(const std::vector<OverlayProposedCandidate>& sorted_candidates)
{
    ...
    for (auto& proposed : sorted_candidates) {
        ...
        if (type != OverlayStrategy::kSingleOnTop && type != OverlayStrategy::kUnderlay) {
            return false;
        }
    }
...
bool OverlayProcessorUsingStrategy::AttemptMultipleOverlays(const std::vector<OverlayProposedCandidate>& sorted_candidates,
    OverlayProcessorInterface::OutputSurfaceOverlayPlane* primary_plane, AggregatedRenderPass* render_pass, OverlayCandidateList& candidates)
{
    ...
    ConstOverlayProposedCandidateIterator first_candidate_without_masks = base::ranges::find_if(sorted_candidates.begin(), sorted_candidates.end(),
        [](const OverlayProposedCandidate& candidate) { return !candidate.candidate.has_rounded_display_masks; });

    int candidates_with_masks_count = std::distance(sorted_candidates.begin(), first_candidate_without_masks);
    int candidates_without_masks_count = sorted_candidates.size() - candidates_with_masks_count;
    ...
    if (candidates_without_masks_count == 0) {
        for (auto iter = sorted_candidates.begin(); iter != first_candidate_without_masks; iter++) {
            LogShouldPromoteCandidatesWithMasksEnumUMA(PromotingMaskCandidates::kNoNotRequired);
        }
    ...
    int max_overlays_without_mask_candidates = std::max(0, max_overlays_considered_ - candidates_with_masks_count);

    OverlayCombinationToTest result = overlay_combination_cache_.GetOverlayCombinationToTest(
        base::make_span(first_candidate_without_masks, sorted_candidates.end()), max_overlays_without_mask_candidates);

    std::vector<OverlayProposedCandidate> test_candidates = result.candidates_to_test;

    ConstOverlayProposedCandidateIterator begin_rounded_corner_candidate
        = MaybeAppendOccludingMaskCandidates(sorted_candidates.begin(), first_candidate_without_masks, test_candidates);
    ...
    for (auto it = test_candidates.begin(); it != test_candidates.end(); ++it) {
        ...
        case OverlayStrategy::kSingleOnTop:
            ...
            // plane_z_order=2.
            it->candidate.plane_z_order = it->candidate.has_rounded_display_masks ? 2 : 1;
            break;
    ...
    // display masks.
    overlay_combination_cache_.DeclarePromotedCandidates(base::make_span(test_candidates.begin(), begin_rounded_corner_candidate));
    ...
    std::erase_if(test_candidates, [](auto& proposed) -> bool { return !proposed.candidate.overlay_handled; });
    const int num_overlays_promoted = candidates.size();

    UMA_HISTOGRAM_COUNTS_100(kNumOverlaysAttemptedHistogramName, num_overlays_attempted);
    UMA_HISTOGRAM_COUNTS_100(kNumOverlaysFailedHistogramName, num_overlays_attempted - num_overlays_promoted);
    ...
    std::sort(test_candidates.begin(), test_candidates.end(),
        [](const OverlayProposedCandidate& c1, const OverlayProposedCandidate& c2) -> bool { return c1.quad_iter.index() > c2.quad_iter.index(); });
...
void OverlayProcessorUsingStrategy::AssignUnderlayZOrders(std::vector<std::vector<OverlayProposedCandidate>::iterator>& underlay_iters)
{
    // Sort the underlay iterators by DrawQuad order, frontmost first.
    std::sort(underlay_iters.begin(), underlay_iters.end(),
        [](const std::vector<OverlayProposedCandidate>::iterator& c1, const std::vector<OverlayProposedCandidate>::iterator& c2) -> bool {
...
gfx::Rect OverlayProcessorUsingStrategy::GetOverlayDamageRectForOutputSurface(const OverlayCandidate& overlay) const
{
    return ToEnclosedRect(overlay.display_rect);
}

void OverlayProcessorUsingStrategy::OnOverlaySwitchUMA(ProposedCandidateKey overlay_tracking_id)
{
    ...
    if (!(prev_overlay_tracking_id_ == overlay_tracking_id)) {
        ...
        UMA_HISTOGRAM_TIMES("Viz.DisplayCompositor.OverlaySwitchInterval", curr_tick - last_time_interval_switch_overlay_tick_);
        last_time_interval_switch_overlay_tick_ = curr_tick;
    }
}

void OverlayProcessorUsingStrategy::UpdateDownscalingCapabilities(float scale_factor, bool success)
{
    if (success) {
        ...
        min_working_scale_ = std::min(scale_factor + kScaleBoundsTolerance, min_working_scale_);
...
bool OverlayProcessorUsingStrategy::BlockForCopyRequests(const AggregatedRenderPass* root_render_pass)
{
    if (!base::FeatureList::IsEnabled(features::kTemporalSkipOverlaysWithRootCopyOutputRequests)) {
