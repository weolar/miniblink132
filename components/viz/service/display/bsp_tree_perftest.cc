...
public:
    ...
    void SetupTree() override
    {
        ...
        layer_tree_host()->SetViewportRectAndScale(gfx::Rect(viewport), 1.f, LocalSurfaceId());
        scoped_refptr<cc::Layer> root = ParseTreeFromJson(json_, &content_layer_client_);
    ...
    void DrawLayersOnThread(cc::LayerTreeHostImpl* host_impl) override
    {
        ...
        for (auto it = base_list.begin(); it != base_list.end(); ++it) {
            DrawPolygon* draw_polygon
                = new DrawPolygon(nullptr, gfx::RectF(gfx::SizeF((*it)->bounds())), (*it)->draw_properties().target_space_transform, polygon_counter++);
    ...
    void AfterTest() override
    {
        ...
        reporter.AddResult(kMetricCalcDrawPropsTimeUs, timer_.TimePerLap().InMicrosecondsF());
    }
