...
protected:
    ...
    void InitOcclusionCuller(RendererSettings::OcclusionCullerSettings settings)
    {
        ...
        occlusion_culler_ = std::make_unique<OcclusionCuller>(overlay_processor_.get(), settings);
    }

    OcclusionCuller* occlusion_culler()
    {
        return occlusion_culler_.get();
    }
...
TEST_F(OcclusionCullerTest, OcclusionCullingWithBlending)
{
    ...
    for (auto& render_pass : frame.render_pass_list) {
        ...
        src_sqs->SetAll(gfx::Transform(), src_rect, src_rect, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, are_contents_opaque, opacity, is_root_render_pass ? SkBlendMode::kSrcOver : SkBlendMode::kSrcIn,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        auto* dest_sqs = render_pass->CreateAndAppendSharedQuadState();
        dest_sqs->SetAll(gfx::Transform(), dest_rect, dest_rect, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, are_contents_opaque, opacity, is_root_render_pass ? SkBlendMode::kSrcOver : SkBlendMode::kDstIn,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        auto* src_quad = render_pass->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
        src_quad->SetNew(src_sqs, src_rect, src_rect, SkColors::kBlack, false);

        auto* dest_quad = render_pass->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
...
TEST_F(OcclusionCullerTest, OcclusionCullingWithIntersectingBackdropFilter)
{
    ...
    bd_render_pass->SetAll(AggregatedRenderPassId { 2 }, bd_filter_rect, gfx::Rect(), gfx::Transform(), cc::FilterOperations(), backdrop_filters,
        gfx::RRectF(gfx::RectF(bd_filter_rect), 0), gfx::ContentColorUsage::kSRGB, false, false, false, false);

    // Add quads to root render pass
    for (int i = 0; i < 3; i++) {
        shared_quad_states[i] = root_render_pass->CreateAndAppendSharedQuadState();
        shared_quad_states[i]->SetAll(gfx::Transform(), rects[i], rects[i], gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        if (i == 0) { // Backdrop filter quad
            auto* new_quad = root_render_pass->quad_list.AllocateAndConstruct<AggregatedRenderPassDrawQuad>();
            new_quad->SetNew(shared_quad_states[i], rects[i], rects[i], bd_render_pass->id, ResourceId(2), gfx::RectF(), gfx::Size(), gfx::Vector2dF(1, 1),
                gfx::PointF(), gfx::RectF(), false, 1.f);
            quads[i] = new_quad;
        } else {
            auto* new_quad = root_render_pass->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
            new_quad->SetNew(shared_quad_states[i], rects[i], rects[i], SkColors::kBlack, false);
            quads[i] = new_quad;
        }
    }
...
TEST_F(OcclusionCullerTest, OcclusionCullingWithNonCoveringDrawQuad)
{
    ...
    bool are_contents_opaque = true;
    float opacity = 1.f;
    SharedQuadState* shared_quad_state = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    auto* quad = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();

    // +----+
    // |    |
    // +----+
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
        ...
        // RemoveOverdrawQuads.
        EXPECT_EQ(1u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
    }

    SharedQuadState* shared_quad_state2 = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    auto* quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    ...
    //   +----+
    {
        ...
        shared_quad_state2->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect2, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
    }
    ...
    //   +--+                                    +--+
    {
        ...
        shared_quad_state2->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(gfx::Rect(25, 100, 50, 25), frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
    }
    ...
    //  +--+                                        +--+
    {
        shared_quad_state->SetAll(gfx::Transform(), rect7, rect7, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        shared_quad_state2->SetAll(gfx::Transform(), rect6, rect6, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        // same).
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect7, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect6, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
    }
    ...
    // +----+
    {
        ...
        shared_quad_state2->SetAll(gfx::Transform(), rect4, rect4, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect4, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
    }
    // +-----++
    // |     ||
    // +-----+|
    // +------+
    {
        ...
        shared_quad_state2->SetAll(gfx::Transform(), rect5, rect5, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect5, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
    }
}
...
TEST_F(OcclusionCullerTest, OcclusionCullingWithSingleOverlapBehindDisjointedDrawQuads)
{
    InitOcclusionCuller();
    ...
    for (const gfx::Rect& rect : rects) {
        SharedQuadState* shared_quad_state = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
        shared_quad_state->SetAll(gfx::Transform(), rect, rect, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
    ...
    //  +-----+     +-------+
    {
        ...
        EXPECT_EQ(rects[0], frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rects[1], frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
    }
}
...
// reduced to 1, since |rects[1]| will become the only occluder, and the quad
// defined by |rects[2]| will not be occluded (removed).
TEST_F(OcclusionCullerTest, OcclusionCullingWithMultipleOverlapBehindDisjointedDrawQuads)
{
    InitOcclusionCuller();
...
TEST_F(OcclusionCullerTest, CompositorFrameWithOverlapDrawQuad)
{
    ...
    //                         +-----+
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        shared_quad_state2->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
    ...
    //  +-----+
    {
        quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
...
TEST_F(OcclusionCullerTest, CompositorFrameWithTransformer)
{
...

    {
        ...
        shared_quad_state2->SetAll(half_scale, rect2, rect2, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...

    {
        ...
        shared_quad_state2->SetAll(double_scale, rect5, rect5, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect5, frame.render_pass_list.front()->quad_list.ElementAt(4)->visible_rect);
    }

    {
        ...
        shared_quad_state2->SetAll(double_scale, rect6, rect6, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(gfx::Rect(12, 50, 25, 12), frame.render_pass_list.front()->quad_list.ElementAt(4)->visible_rect);
    }

    {
        ...
        shared_quad_state2->SetAll(double_scale, rect7, rect7, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect7, frame.render_pass_list.front()->quad_list.ElementAt(4)->visible_rect);
    }

    {
        ...
        shared_quad_state2->SetAll(double_scale, rect8, rect8, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect8, frame.render_pass_list.front()->quad_list.ElementAt(4)->visible_rect);
    }

    {
        shared_quad_state->SetAll(double_scale, rect10, rect10, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        shared_quad_state2->SetAll(double_scale, rect9, rect9, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        // the same
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect10, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect9, frame.render_pass_list.front()->quad_list.ElementAt(4)->visible_rect);
    }
}
...
TEST_F(OcclusionCullerTest, CompositorFrameWithEpsilonScaleTransform)
{
...

    {
        ...
        shared_quad_state2->SetAll(zero_scale, rect, rect, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
    }

    {
        ...
        EXPECT_TRUE(cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(epsilon_scale, rect).IsEmpty());
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
    }

    {
        ...
        shared_quad_state2->SetAll(larger_epsilon_scale, rect, rect, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...
TEST_F(OcclusionCullerTest, CompositorFrameWithNegativeScaleTransform)
{
...

    {
        ...
        shared_quad_state2->SetAll(negative_scale, rect, rect, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...
TEST_F(OcclusionCullerTest, CompositorFrameWithAxisAlignmentRotation)
{
...

    {
        ...
        EXPECT_EQ(rect2_a, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
        EXPECT_EQ(rect2_b, frame.render_pass_list.front()->quad_list.ElementAt(2)->visible_rect);
...

    {
        ...
        shared_quad_state->SetAll(rotate_and_scale, rect1, rect1, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u,
            /*fast_rounded_corner=*/false);

        shared_quad_state2->SetAll(rotate_and_scale, rect2, rect2, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...
TEST_F(OcclusionCullerTest, CompositorFrameWithNonAxisAlignmentRotation)
{
...

    {
        ...
        shared_quad_state->SetAll(rotate, rect1, rect1, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        shared_quad_state2->SetAll(rotate, rect2, rect2, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...

    {
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect3, frame.render_pass_list.front()->quad_list.ElementAt(2)->visible_rect);
    }

    {
        ...
        shared_quad_state2->SetAll(rotate, rect3, rect3, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...
TEST_F(OcclusionCullerTest, CompositorFrameWithPerspective)
{
    ...
    {
        shared_quad_state->SetAll(perspective, rect1, rect1, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        shared_quad_state2->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
    }

    {
        ...
        shared_quad_state2->SetAll(perspective, rect2, rect2, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...
TEST_F(OcclusionCullerTest, CompositorFrameWithOpacityChange)
{
    ...
    auto* quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacityLess1, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        shared_quad_state2->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity1, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
    ...
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity1, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...
TEST_F(OcclusionCullerTest, CompositorFrameWithOpaquenessChange)
{
    ...
    auto* quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), std::nullopt, transparent_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        shared_quad_state2->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(),
    ...
    {
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...
TEST_F(OcclusionCullerTest, CompositorFrameZTranslate)
{
    ...
    //                         +-----+
    {
        shared_quad_state->SetAll(translate_back, rect1, rect1, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->rect);
        EXPECT_EQ(rect2, frame.render_pass_list.front()->quad_list.ElementAt(1)->rect);
    }
}
...
TEST_F(OcclusionCullerTest, CompositorFrameWithTranslateTransformer)
{
...

    {
        ...
        shared_quad_state->SetAll(translate_up, rect1, rect1, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...

    {
        ...
        shared_quad_state2->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(gfx::Rect(150, 100, 50, 20), frame.render_pass_list.front()->quad_list.ElementAt(2)->visible_rect);
    }
}
...
TEST_F(OcclusionCullerTest, CompositorFrameWithCombinedSharedQuadState)
{
    ...
    auto* quad3 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        ...
        shared_quad_state2->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state3->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...

    {
        ...
        //   +----+                            +----+
        //
        quad3 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
        shared_quad_state3->SetAll(gfx::Transform(), rect4, rect4, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect2, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
        EXPECT_EQ(gfx::Rect(160, 10, 30, 30), frame.render_pass_list.front()->quad_list.ElementAt(3)->visible_rect);
    }

    {
        ...
        //   +----+                            +-|--+  |
        //                                       +-----+
        shared_quad_state3->SetAll(gfx::Transform(), rect5, rect5, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect2, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
        EXPECT_EQ(gfx::Rect(10, 60, 120, 50), frame.render_pass_list.front()->quad_list.ElementAt(3)->visible_rect);
    }
}
...
TEST_F(OcclusionCullerTest, OcclusionCullingWithMultipleRenderPass)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        auto& render_pass = i == 3 ? frame.render_pass_list.back() : frame.render_pass_list.front();
        shared_quad_states[i] = render_pass->CreateAndAppendSharedQuadState();
        quads[i] = render_pass->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
        shared_quad_states[i]->SetAll(gfx::Transform(), rects[i], rects[i], gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, /*contents_opaque=*/true,
            /*opacity_f=*/1.f, SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        quads[i]->SetNew(shared_quad_states[i], rects[i], rects[i], SkColors::kBlack, false /*force_anti_aliasing_off*/);
    }
...
TEST_F(OcclusionCullerTest, CompositorFrameWithMultipleRenderPass)
{
    ...
    render_pass2->SetNew(AggregatedRenderPassId { 1 }, gfx::Rect(), gfx::Rect(), gfx::Transform());
    ...
    SharedQuadState* shared_quad_state = frame.render_pass_list.at(1)->CreateAndAppendSharedQuadState();
    auto* quad = frame.render_pass_list.at(1)->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    SharedQuadState* shared_quad_state2 = frame.render_pass_list.at(1)->CreateAndAppendSharedQuadState();
    auto* quad2 = frame.render_pass_list.at(1)->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    SharedQuadState* shared_quad_state3 = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    auto* quad3 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        ...
        EXPECT_EQ(2u, frame.render_pass_list.at(1)->quad_list.size());
        EXPECT_EQ(1u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.at(1)->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect2, frame.render_pass_list.at(1)->quad_list.ElementAt(1)->visible_rect);
        EXPECT_EQ(rect3, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
    }
}
...
TEST_F(OcclusionCullerTest, CompositorFrameWithCoveredRenderPass)
{
    ...
    auto* quad = frame.render_pass_list.at(1)->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    SharedQuadState* shared_quad_state2 = frame.render_pass_list.at(1)->CreateAndAppendSharedQuadState();
    auto* quad1 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<AggregatedRenderPassDrawQuad>();

    {
        ...
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state2->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad1->SetNew(shared_quad_state2, rect1, rect1, render_pass_id, mask_resource_id, gfx::RectF(), gfx::Size(), gfx::Vector2dF(1, 1), gfx::PointF(),
...
TEST_F(OcclusionCullerTest, CompositorFrameWithClip)
{
...

    {
        ...
        //
        quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(), clip_rect, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state2->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect2, frame.render_pass_list.front()->quad_list.ElementAt(2)->visible_rect);
    }

    {
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(gfx::Rect(60, 50, 10, 10), frame.render_pass_list.front()->quad_list.ElementAt(2)->visible_rect);
    }
}
...
TEST_F(OcclusionCullerTest, CompositorFrameWithCopyRequest)
{
    ...
    auto* quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        ...
        quad->SetNew(shared_quad_state, rect1, rect1, SkColors::kBlack, false);
        quad2->SetNew(shared_quad_state2, rect2, rect2, SkColors::kBlack, false);
        frame.render_pass_list.front()->copy_requests.push_back(CopyOutputRequest::CreateStubForTesting());
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
...
TEST_F(OcclusionCullerTest, CompositorFrameWithRenderPass)
{
    ...
    float opacity = 1.f;
    SharedQuadState* shared_quad_state = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    auto* R1 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<AggregatedRenderPassDrawQuad>();
    SharedQuadState* shared_quad_state2 = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    auto* R2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<AggregatedRenderPassDrawQuad>();
    SharedQuadState* shared_quad_state3 = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    auto* D1 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    SharedQuadState* shared_quad_state4 = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    auto* D2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    {
        ...
        shared_quad_state3->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state4->SetAll(gfx::Transform(), rect4, rect4, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        R1->SetNew(shared_quad_state, rect1, rect1, render_pass_id, mask_resource_id, gfx::RectF(), gfx::Size(), gfx::Vector2dF(1, 1), gfx::PointF(),
            gfx::RectF(), false, 1.0f);
        R2->SetNew(shared_quad_state, rect2, rect2, render_pass_id, mask_resource_id, gfx::RectF(), gfx::Size(), gfx::Vector2dF(1, 1), gfx::PointF(),
            gfx::RectF(), false, 1.0f);
        ...
        EXPECT_EQ(rect2, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
        EXPECT_EQ(rect3, frame.render_pass_list.front()->quad_list.ElementAt(2)->visible_rect);
        EXPECT_EQ(rect4, frame.render_pass_list.front()->quad_list.ElementAt(3)->visible_rect);
    }

    {
        ...
        // |   R2  |       R1  |
        // +-------+-----------+
        shared_quad_state->SetAll(gfx::Transform(), rect5, rect5, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state2->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
        ...
        shared_quad_state3->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state4->SetAll(gfx::Transform(), rect6, rect6, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);

        R1->SetNew(shared_quad_state, rect5, rect5, render_pass_id, mask_resource_id, gfx::RectF(), gfx::Size(), gfx::Vector2dF(1, 1), gfx::PointF(),
            gfx::RectF(), false, 1.0f);
        R2->SetNew(shared_quad_state, rect1, rect1, render_pass_id, mask_resource_id, gfx::RectF(), gfx::Size(), gfx::Vector2dF(1, 1), gfx::PointF(),
            gfx::RectF(), false, 1.0f);
        D1->SetNew(shared_quad_state3, rect3, rect3, SkColors::kBlack, false);
        ...
        // cannot be removed to reduce overdraw, |quad_list| remains unchanged.
        EXPECT_EQ(4u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect5, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect1, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
        EXPECT_EQ(rect3, frame.render_pass_list.front()->quad_list.ElementAt(2)->visible_rect);
        EXPECT_EQ(rect6, frame.render_pass_list.front()->quad_list.ElementAt(3)->visible_rect);
    }

    {
        ...
        shared_quad_state3->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state4->SetAll(gfx::Transform(), rect7, rect7, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...
TEST_F(OcclusionCullerTest, CompositorFrameWithMultipleDrawQuadInSharedQuadState)
{
    InitOcclusionCuller();
    ...
    SharedQuadState* shared_quad_state = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    SharedQuadState* shared_quad_state2 = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    auto* quad1 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    auto* quad2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    auto* quad3 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    auto* quad4 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    auto* quad5 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();

    {
        ...
        shared_quad_state->SetAll(gfx::Transform(), rect1, rect1, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state2->SetAll(gfx::Transform(), rect_in_rect1, rect_in_rect1, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        quad5->SetNew(shared_quad_state2, rect_in_rect1, rect_in_rect1, SkColors::kBlack, false);
        ...
        EXPECT_EQ(4u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect1_1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect1_2, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
        EXPECT_EQ(rect1_3, frame.render_pass_list.front()->quad_list.ElementAt(2)->visible_rect);
        EXPECT_EQ(rect1_4, frame.render_pass_list.front()->quad_list.ElementAt(3)->visible_rect);
    }

    {
        ...
        // |  |  |
        // +--+--+
        quad5 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
        shared_quad_state2->SetAll(gfx::Transform(), rect_intersects_rect1, rect_intersects_rect1, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        quad5->SetNew(shared_quad_state2, rect_intersects_rect1, rect_intersects_rect1, SkColors::kBlack, false);
        EXPECT_EQ(5u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        ...
        EXPECT_EQ(rect1_3, frame.render_pass_list.front()->quad_list.ElementAt(2)->visible_rect);
        EXPECT_EQ(rect1_4, frame.render_pass_list.front()->quad_list.ElementAt(3)->visible_rect);
        EXPECT_EQ(gfx::Rect(100, 0, 30, 30), frame.render_pass_list.front()->quad_list.ElementAt(5)->visible_rect);
    }

    {
        ...
        auto* quad6 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
        shared_quad_state->SetAll(gfx::Transform(), rect2, rect2, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state2->SetAll(gfx::Transform(), rect3, rect3, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
        ...
        quad5->SetNew(shared_quad_state2, rect3_1, rect3_1, SkColors::kBlack, false);
        quad6->SetNew(shared_quad_state2, rect3_2, rect3_2, SkColors::kBlack, false);
        ...
        EXPECT_EQ(6u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(rect2_1, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(rect2_2, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
        EXPECT_EQ(rect2_3, frame.render_pass_list.front()->quad_list.ElementAt(2)->visible_rect);
        EXPECT_EQ(rect2_4, frame.render_pass_list.front()->quad_list.ElementAt(3)->visible_rect);
        EXPECT_EQ(gfx::Rect(0, 0, 20, 30), frame.render_pass_list.front()->quad_list.ElementAt(5)->visible_rect);
        EXPECT_EQ(gfx::Rect(120, 0, 20, 30), frame.render_pass_list.front()->quad_list.ElementAt(6)->visible_rect);
    }
}
...
TEST_F(OcclusionCullerTest, CompositorFrameWithNonInvertibleTransform)
{
    ...
    SharedQuadState* shared_quad_state1 = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    SharedQuadState* shared_quad_state2 = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    SharedQuadState* shared_quad_state3 = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();

    {
        ...
        shared_quad_state1->SetAll(invertible, rect1, rect1, gfx::MaskFilterInfo(), std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state2->SetAll(invertible, rect2, rect2, gfx::MaskFilterInfo(), std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state3->SetAll(non_invertible, rect3, rect3, gfx::MaskFilterInfo(),
            /*clip=*/std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
...

    {
        ...
        shared_quad_state1->SetAll(invertible, rect1, rect1, gfx::MaskFilterInfo(), std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        shared_quad_state3->SetAll(non_invertible_miss_z, rect3, rect3, gfx::MaskFilterInfo(), std::nullopt, opaque_content, opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        quad1->SetNew(shared_quad_state1, rect1, rect1, SkColors::kBlack, false);
        quad3->SetNew(shared_quad_state3, rect3, rect3, SkColors::kBlack, false);
...
TEST_F(OcclusionCullerTest, OcclusionCullingWithRoundedCornerDoesNotOcclude)
{
    ...
    bool are_contents_opaque = true;
    float opacity = 1.f;
    SharedQuadState* shared_quad_state_with_rrect = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    SharedQuadState* shared_quad_state_occluded = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();

    auto* rounded_corner_quad = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    auto* occluded_quad = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();

    {
        shared_quad_state_occluded->SetAll(gfx::Transform(), quad_rect, quad_rect, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        occluded_quad->SetNew(shared_quad_state_occluded, quad_rect, quad_rect, SkColors::kRed, false);

        shared_quad_state_with_rrect->SetAll(gfx::Transform(), quad_rect, quad_rect, mask_filter_info,
            /*clip=*/std::nullopt, are_contents_opaque, opacity, SkBlendMode::kSrcOver, /*sorting_context=*/0, /*layer_id=*/0u,
            /*fast_rounded_corner=*/false);
        rounded_corner_quad->SetNew(shared_quad_state_with_rrect, quad_rect, quad_rect, SkColors::kBlue, false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(quad_rect, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(quad_rect, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
    }
}
...
TEST_F(OcclusionCullerTest, OcclusionCullingWithRoundedCornerDoesNotOccludeY)
{
    ...
    gfx::MaskFilterInfo mask_filter_info(gfx::RRectF(gfx::RectF(quad_rect), 10.f, 20.f));
    ...
    {
        shared_quad_state_occluded->SetAll(gfx::Transform(), occluded_quad_rect, occluded_quad_rect, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque,
            opacity, SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        occluded_quad->SetNew(shared_quad_state_occluded, occluded_quad_rect, occluded_quad_rect, SkColors::kRed, false);
        ...
        EXPECT_EQ(2u, NumVisibleRects(frame.render_pass_list.front()->quad_list));
        EXPECT_EQ(quad_rect, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        EXPECT_EQ(occluded_quad_rect, frame.render_pass_list.front()->quad_list.ElementAt(1)->visible_rect);
    }
}
...
TEST_F(OcclusionCullerTest, OcclusionCullingWithRoundedCornerDoesNotOccludeX)
{
    ...
    gfx::Rect quad_rect(10, 10, 100, 100);
    gfx::Rect occluded_quad_rect(13, 13, 994, 994);
    gfx::MaskFilterInfo mask_filter_info(gfx::RRectF(gfx::RectF(quad_rect), 20.f, 10.f));
...
TEST_F(OcclusionCullerTest, OcclusionCullingWithRoundedCornerDoesOcclude)
{
    ...
    {
        shared_quad_state_occluded->SetAll(gfx::Transform(), occluded_quad_rect, occluded_quad_rect, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque,
            opacity, SkBlendMode::kSrcOver, /*sorting_context=*/0, /*layer_id=*/0u,
            /*fast_rounded_corner=*/false);
        occluded_quad->SetNew(shared_quad_state_occluded, occluded_quad_rect, occluded_quad_rect, SkColors::kRed, false);
...
TEST_F(OcclusionCullerTest, OcclusionCullingSplit)
{
    ...
    SharedQuadState* shared_quad_state_occluder = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    SharedQuadState* shared_quad_state_occluded = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();

    SolidColorDrawQuad* quads[4];
    for (auto*& quad : quads) {
        quad = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    }

    {
        shared_quad_state_occluder->SetAll(gfx::Transform(), occluding_rect, occluding_rect, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        quads[0]->SetNew(shared_quad_state_occluder, occluding_rect, occluding_rect, SkColors::kRed, false);

        shared_quad_state_occluded->SetAll(gfx::Transform(), occluded_sqs_rect, occluded_sqs_rect, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque,
            opacity, SkBlendMode::kSrcOver, /*sorting_context=*/0, /*layer_id=*/0u,
            /*fast_rounded_corner=*/false);
        for (int i = 1; i < 4; i++) {
            quads[i]->SetNew(shared_quad_state_occluded, quad_rects[i - 1], quad_rects[i - 1], SkColors::kRed, false);
        }
        ...
        EXPECT_EQ(occluding_rect, frame.render_pass_list.front()->quad_list.ElementAt(0)->visible_rect);
        ...
        for (int i = 0; i < 5; i++) {
            EXPECT_EQ(expected_visible_rects[i], quad_list.ElementAt(i + 1)->visible_rect);
        }
    }
}
...
TEST_F(OcclusionCullerTest, FirstPassVisibleComplexityReduction)
{
    ...
    for (const auto& r : occluding_rects) {
        SharedQuadState* shared_quad_state_occluder = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
        shared_quad_state_occluder->SetAll(gfx::Transform(), r, r, gfx::MaskFilterInfo(), /*clip=*/std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        SolidColorDrawQuad* quad = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    ...
    {
        SharedQuadState* shared_quad_state_occluded = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
        shared_quad_state_occluded->SetAll(gfx::Transform(), occluded_rect, occluded_rect, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity,
            SkBlendMode::kSrcOver,
            /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        SolidColorDrawQuad* occluded_quad = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
        occluded_quad->SetNew(shared_quad_state_occluded, occluded_rect, occluded_rect, SkColors::kRed, false);
    }
    ...
    for (size_t i = 0; i < std::size(expected_visible_rects); ++i) {
        EXPECT_EQ(expected_visible_rects[i], frame.render_pass_list.front()->quad_list.ElementAt(i)->visible_rect);
    }
}
...
TEST_F(OcclusionCullerTest, OcclusionCullingWithRoundedCornerPartialOcclude)
{
    ...
    auto* rounded_corner_quad = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    auto* occluded_quad_1 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    auto* occluded_quad_2 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    auto* occluded_quad_3 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    auto* occluded_quad_4 = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();

    {
        shared_quad_state_occluded->SetAll(gfx::Transform(), occluded_sqs_rect, occluded_sqs_rect, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque,
            opacity, SkBlendMode::kSrcOver, /*sorting_context=*/0, /*layer_id=*/0u,
            /*fast_rounded_corner=*/false);
        occluded_quad_1->SetNew(shared_quad_state_occluded, occluded_quad_rect_1, occluded_quad_rect_1, SkColors::kRed, false);
        occluded_quad_2->SetNew(shared_quad_state_occluded, occluded_quad_rect_2, occluded_quad_rect_2, SkColors::kRed, false);
        occluded_quad_3->SetNew(shared_quad_state_occluded, occluded_quad_rect_3, occluded_quad_rect_3, SkColors::kRed, false);
        occluded_quad_4->SetNew(shared_quad_state_occluded, occluded_quad_rect_4, occluded_quad_rect_4, SkColors::kRed, false);
...
TEST_F(OcclusionCullerTest, OcclusionCullingSplitDeviceScaleFactorFractional)
{
    ...
    SharedQuadState* shared_quad_state_occluding = frame.render_pass_list.front()->CreateAndAppendSharedQuadState();
    SolidColorDrawQuad* occluding_quad = frame.render_pass_list.front()->quad_list.AllocateAndConstruct<SolidColorDrawQuad>();
    shared_quad_state_occluding->SetAll(gfx::Transform(), occluding_rect, occluding_rect, gfx::MaskFilterInfo(), std::nullopt, are_contents_opaque, opacity,
        SkBlendMode::kSrcOver,
        /*sorting_context=*/0, /*layer_id=*/0u, /*fast_rounded_corner=*/false);
    occluding_quad->SetNew(shared_quad_state_occluding, occluding_rect, occluding_rect, SkColors::kRed, false);
