...
bool AdpfCanUseSetThreads()
{
    ...
    return android_get_device_api_level() >= __ANDROID_API_U__ && base::FeatureList::IsEnabled(features::kEnableADPFSetThreads);
#else
    return false;
#endif
}
...
DisplayScheduler::DisplayScheduler(BeginFrameSource* begin_frame_source, base::SingleThreadTaskRunner* task_runner, PendingSwapParams pending_swap_params,
    HintSessionFactory* hint_session_factory, bool wait_for_all_surfaces_before_draw)
    ...
    , hint_session_factory_(hint_session_factory)
{
    ...
    begin_frame_deadline_closure_ = base::BindRepeating(&DisplayScheduler::OnBeginFrameDeadline, weak_ptr_factory_.GetWeakPtr());
    session_states_.emplace_back(HintSession::SessionType::kAnimation);
    if (base::FeatureList::IsEnabled(features::kEnableADPFSeparateRendererMainSession)) {
...
DisplayScheduler::AdpfSessionState::AdpfSessionState(HintSession::SessionType type)
    : type(type)
{
}
DisplayScheduler::AdpfSessionState::AdpfSessionState(AdpfSessionState&&) = default;
...
void DisplayScheduler::SetDamageTracker(DisplayDamageTracker* damage_tracker)
{
    ...
    damage_tracker->SetDisplayBeginFrameSourceId(begin_frame_source_->source_id());
}
...
base::TimeDelta DisplayScheduler::GetDeadlineOffset(base::TimeDelta interval) const
{
...
bool DisplayScheduler::UpdateHasPendingSurfaces()
{
    ...
    has_pending_surfaces_ = damage_tracker_->HasPendingSurfaces(current_begin_frame_args_);
    ...
    base::flat_set<base::PlatformThreadId> animation_thread_ids, base::flat_set<base::PlatformThreadId> renderer_main_thread_ids)
{
    ...
    if (!renderer_main_thread_ids.empty() && !base::FeatureList::IsEnabled(features::kEnableADPFSeparateRendererMainSession)) {
        animation_thread_ids.insert(renderer_main_thread_ids.begin(), renderer_main_thread_ids.end());
    ...
    for (auto& state : session_states_) {
        auto thread_ids = state.type == HintSession::SessionType::kAnimation ? animation_thread_ids : renderer_main_thread_ids;

        // Use SetThreads whenever possible - it's cheaper than recreating the
        // session.
        if (state.hint_session && state.thread_ids != thread_ids && AdpfCanUseSetThreads()) {
            state.thread_ids = std::move(thread_ids);
            state.hint_session->SetThreads(hint_session_factory_->GetSessionThreadIds(state.thread_ids, state.type));
            continue;
        }
        // If SetThreads cannot be used, (re)create the ADPF session - this is a
        // more a expensive operation.
        if ((!state.create_session_for_current_thread_ids_failed && !state.hint_session) || state.thread_ids != thread_ids) {
            state.hint_session.reset();
            state.thread_ids = std::move(thread_ids);
            state.hint_session = hint_session_factory_->CreateSession(state.thread_ids, adpf_target, state.type);
            state.create_session_for_current_thread_ids_failed = !state.hint_session;
        }
    }
}

void DisplayScheduler::ReportFrameTime(base::TimeDelta frame_time, base::flat_set<base::PlatformThreadId> animation_thread_ids,
    base::flat_set<base::PlatformThreadId> renderer_main_thread_ids, base::TimeTicks draw_start, HintSession::BoostType boost_type)
{
    MaybeCreateHintSessions(std::move(animation_thread_ids), std::move(renderer_main_thread_ids));
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("Compositing.Display.AdpfHintUs", frame_time, base::Microseconds(1), base::Milliseconds(50), 50);
    for (const auto& state : session_states_) {
        if (state.hint_session) {
            state.hint_session->ReportCpuCompletionTime(frame_time, draw_start, boost_type);
        }
    }
}
...
bool DisplayScheduler::DrawAndSwap()
{
    ...
    DCHECK_LT(pending_swaps_, std::max(pending_swap_params_.max_pending_swaps, pending_swap_params_.max_pending_swaps_120hz.value_or(0)));
    DCHECK(!output_surface_lost_);

    DrawAndSwapParams params { current_begin_frame_args_.frame_time, current_frame_display_time(), MaxPendingSwaps() };
    if (current_begin_frame_args_.possible_deadlines) {
        params.choreographer_vsync_id = current_begin_frame_args_.possible_deadlines->GetPreferredDeadline().vsync_id;
    }
...
bool DisplayScheduler::OnBeginFrame(const BeginFrameArgs& args)
{
    ...
    TRACE_EVENT2("viz", "DisplayScheduler::BeginFrame", "args", args.AsValue(), "now", now);
    ...
    if (inside_surface_damaged_) {
        ...
        missed_begin_frame_task_.Reset(base::BindOnce(base::IgnoreResult(&DisplayScheduler::OnBeginFrame),
    ...
    if (inside_begin_frame_deadline_interval_ && !task_runner_->BelongsToCurrentThread()) {
        task_runner_->PostTask(FROM_HERE, base::BindOnce(&DisplayScheduler::OnBeginFrameContinuation, weak_ptr_factory_.GetWeakPtr(), save_args));
    } else {
...
void DisplayScheduler::OnBeginFrameContinuation(const BeginFrameArgs& args)
{
    ...
    current_begin_frame_args_.deadline -= BeginFrameArgs::DefaultEstimatedDisplayDrawTime(args.interval);
...
int DisplayScheduler::MaxPendingSwaps() const
{
    ...
    if (current_begin_frame_args_.interval < k120HzInterval && pending_swap_params_.max_pending_swaps_120hz) {
        param_max_pending_swaps = pending_swap_params_.max_pending_swaps_120hz.value();
    } else if (current_begin_frame_args_.interval < k90HzInterval && pending_swap_params_.max_pending_swaps_90hz) {
        param_max_pending_swaps = pending_swap_params_.max_pending_swaps_90hz.value();
    } else {
    ...
    // time.
    const auto& deadline = current_begin_frame_args_.possible_deadlines->GetPreferredDeadline();
    ...
    int deadline_max_pending_swaps = (total_time_nanos + 0.8 * interval_nanos) / interval_nanos;
...
bool DisplayScheduler::ShouldDraw() const
{
    ...
    return needs_draw_ && !output_surface_lost_ && visible_ && !damage_tracker_->root_frame_missing();
}

// static
base::TimeTicks DisplayScheduler::DesiredBeginFrameDeadlineTime(BeginFrameDeadlineMode deadline_mode, BeginFrameArgs begin_frame_args)
{
...
DisplayScheduler::BeginFrameDeadlineMode DisplayScheduler::AdjustedBeginFrameDeadlineMode() const
{
    ...
    if (wait_for_all_surfaces_before_draw_ && (mode == BeginFrameDeadlineMode::kRegular || mode == BeginFrameDeadlineMode::kLate)) {
...
DisplayScheduler::BeginFrameDeadlineMode DisplayScheduler::DesiredBeginFrameDeadlineMode() const
{
    if (output_surface_lost_) {
        TRACE_EVENT_INSTANT0("viz", "Lost output surface", TRACE_EVENT_SCOPE_THREAD);
        return BeginFrameDeadlineMode::kImmediate;
    }

    const int max_pending_swaps = MaxPendingSwaps();
    if (pending_swaps_ >= max_pending_swaps) {
        TRACE_EVENT_INSTANT2("viz", "Swap throttled", TRACE_EVENT_SCOPE_THREAD, "pending_swaps", pending_swaps_, "max_pending_swaps", max_pending_swaps);
    ...
    bool wait_for_pending_surfaces = has_pending_surfaces_ && !(DrawImmediatelyWhenInteractive() && damage_tracker_->HasDamageDueToInteraction());

    bool all_surfaces_ready
        = !wait_for_pending_surfaces && damage_tracker_->IsRootSurfaceValid() && !damage_tracker_->expecting_root_surface_damage_because_of_resize();
    ...
    if (all_surfaces_ready && (needs_draw_ || allow_early_deadline_without_draw)) {
        TRACE_EVENT_INSTANT0("viz", "All active surfaces ready", TRACE_EVENT_SCOPE_THREAD);
        return BeginFrameDeadlineMode::kImmediate;
    }
    ...
    if (damage_tracker_->expecting_root_surface_damage_because_of_resize()) {
        TRACE_EVENT_INSTANT0("viz", "Entire display damaged", TRACE_EVENT_SCOPE_THREAD);
        return BeginFrameDeadlineMode::kLate;
    }

    TRACE_EVENT_INSTANT0("viz", "More damage expected soon", TRACE_EVENT_SCOPE_THREAD);
...
void DisplayScheduler::ScheduleBeginFrameDeadline()
{
    ...
    if (!inside_begin_frame_deadline_interval_) {
        TRACE_EVENT_INSTANT0("viz", "Waiting for next BeginFrame", TRACE_EVENT_SCOPE_THREAD);
    ...
    base::TimeTicks desired_deadline = DesiredBeginFrameDeadlineTime(deadline_mode, current_begin_frame_args_);

    // Avoid re-scheduling the deadline if it's already correctly scheduled.
    if (begin_frame_deadline_timer_.IsRunning() && desired_deadline == begin_frame_deadline_task_time_) {
    ...
    if (begin_frame_deadline_task_time_ == base::TimeTicks::Max()) {
        TRACE_EVENT_INSTANT0("viz", "Using infinite deadline", TRACE_EVENT_SCOPE_THREAD);
        return;
    }

    begin_frame_deadline_timer_.Start(FROM_HERE, desired_deadline, begin_frame_deadline_closure_, base::subtle::DelayPolicy::kPrecise);
    TRACE_EVENT2("viz", "Using new deadline", "deadline_mode", deadline_mode, "desired_deadline", desired_deadline);
}
