...
OverlayProcessorOnGpu::OverlayProcessorOnGpu(gpu::DisplayCompositorMemoryAndTaskControllerOnGpu* display_controller_on_gpu)
    : shared_image_representation_factory_(
        std::make_unique<gpu::SharedImageRepresentationFactory>(display_controller_on_gpu->shared_image_manager(), display_controller_on_gpu->memory_tracker()))
{
...
void OverlayProcessorOnGpu::ScheduleOverlays(CandidateList&& overlay_candidates)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
#if BUILDFLAG(IS_ANDROID)
    // TODO(weiliangc): Currently only implemented for Android Classic code path.
    for (auto& overlay : overlay_candidates) {
        auto shared_image_overlay = shared_image_representation_factory_->ProduceLegacyOverlay(overlay.mailbox);
        ...
        // SurfaceView/Dialog.
        shared_image_overlay->NotifyOverlayPromotion(true, ToNearestRect(overlay.display_rect));
...
#if BUILDFLAG(IS_ANDROID)
void OverlayProcessorOnGpu::NotifyOverlayPromotions(base::flat_set<gpu::Mailbox> promotion_denied, base::flat_map<gpu::Mailbox, gfx::Rect> possible_promotions)
{
    for (auto& denied : promotion_denied) {
        auto shared_image_overlay = shared_image_representation_factory_->ProduceLegacyOverlay(denied);
    ...
    for (auto& possible : possible_promotions) {
        auto shared_image_overlay = shared_image_representation_factory_->ProduceLegacyOverlay(possible.first);
        // When display is re-opened, the first few frames might not have video
        // resource ready. Possible investigation crbug.com/1023971.
        if (!shared_image_overlay)
            continue;
