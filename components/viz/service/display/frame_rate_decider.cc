...
FrameRateDecider::FrameRateDecider(
    SurfaceManager* surface_manager, Client* client, bool hw_support_for_multiple_refresh_rates, bool output_surface_supports_set_frame_rate)
    ...
    , client_(client)
    , hw_support_for_multiple_refresh_rates_(hw_support_for_multiple_refresh_rates)
    , output_surface_supports_set_frame_rate_(output_surface_supports_set_frame_rate)
{
...
void FrameRateDecider::SetSupportedFrameIntervals(base::flat_set<base::TimeDelta> supported_intervals)
{
...
void FrameRateDecider::OnSurfaceWillBeDrawn(Surface* surface)
{
    ...
    if (it == current_surface_id_to_active_index_.end()) {
        ...
        DCHECK_EQ(it->second, active_index) << "Same display frame should not draw a surface with different "
                                               "CompositorFrames";
    }

    it = prev_surface_id_to_active_index_.find(surface_id);
    if (it == prev_surface_id_to_active_index_.end() || it->second != active_index) {
...
void FrameRateDecider::UpdatePreferredFrameIntervalIfNeeded()
{
    ...
    for (const auto& frame_sink_id : frame_sinks_drawn_in_previous_frame_) {
        ...
        auto interval = client_->GetPreferredFrameIntervalForFrameSinkId(frame_sink_id, &type);
    ...
    ToggleFrameRateCase toggle_case = GetToggleFrameRateCase(fixed_interval_frame_sink_intervals);
    ...
    case ToggleFrameRateCase::kSingleVideoPerfectCadenceDiffersFromDisplay:
        should_toggle = base::FeatureList::IsEnabled(features::kSingleVideoFrameRateThrottling);
        break;
    ...
    if (toggle_case != ToggleFrameRateCase::kNone && metrics_subsampler_.ShouldSample(0.001)) {
        UMA_HISTOGRAM_ENUMERATION("Compositing.FrameRateDecider.ToggleFrameRateCase", toggle_case);
    }
    ...
    for (const auto& frame_sink_id : frame_sinks_updated_in_previous_frame_) {
        auto interval = client_->GetPreferredFrameIntervalForFrameSinkId(frame_sink_id);
    ...
    TRACE_EVENT_INSTANT1("viz", "FrameRateDecider::UpdatePreferredFrameIntervalIfNeeded", TRACE_EVENT_SCOPE_THREAD, "min_frame_sink_interval",
    ...
    can_set_preferred_frame_rate = all_frame_sinks_have_same_interval && hw_support_for_multiple_refresh_rates_;
#else
    ...
    can_set_preferred_frame_rate = all_frame_sinks_have_same_interval && output_surface_supports_set_frame_rate_;
#endif
    ...
    if (*min_frame_sink_interval != BeginFrameArgs::MinInterval()) {
        ...
        for (auto supported_interval : supported_intervals_) {
            if (toggle_case == ToggleFrameRateCase::kSingleVideoPerfectCadenceDiffersFromDisplay) {
                // For single video cases, pick the maximum interval with perfect
                // cadence from supported intervals.
                bool simple_cadence = media::VideoCadenceEstimator::HasSimpleCadence(supported_interval, *min_frame_sink_interval, kMaxTimeUntilNextGlitch);
                ...
                if ((AreAlmostEqual(*min_frame_sink_interval, supported_interval) || delta.is_positive()) && delta.magnitude() < min_delta) {
                    new_preferred_interval = supported_interval;
...
ToggleFrameRateCase FrameRateDecider::GetToggleFrameRateCase(const std::vector<base::TimeDelta>& fixed_interval_frame_sink_intervals) const
{
    int num_of_frame_sinks_with_fixed_interval = fixed_interval_frame_sink_intervals.size();
    ...
    if (num_of_frame_sinks_with_fixed_interval == 1) {
        ...
        for (auto supported_interval : supported_intervals_) {
            if (media::VideoCadenceEstimator::HasSimpleCadence(supported_interval, interval, kMaxTimeUntilNextGlitch)) {
                return ToggleFrameRateCase::kSingleVideoPerfectCadenceDiffersFromDisplay;
            }
        }
...
void FrameRateDecider::SetPreferredInterval(base::TimeDelta new_preferred_interval)
{
    TRACE_EVENT_INSTANT1(
        "viz", "FrameRateDecider::SetPreferredInterval", TRACE_EVENT_SCOPE_THREAD, "new_preferred_interval", new_preferred_interval.InMillisecondsF());

    if (AreAlmostEqual(new_preferred_interval, last_computed_preferred_frame_interval_)) {
    ...
    bool should_toggle = current_preferred_frame_interval_ > new_preferred_interval
        || num_of_frames_since_preferred_interval_changed_ >= min_num_of_frames_to_toggle_interval_;
    if (should_toggle) {
...
bool FrameRateDecider::multiple_refresh_rates_supported() const
{
    ...
    return supported_intervals_.size() > 1u || hw_support_for_multiple_refresh_rates_;
#else
    return output_surface_supports_set_frame_rate_ || supported_intervals_.size() > 1u;
#endif
}
