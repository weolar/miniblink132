...
OverlayCacheKey::OverlayCacheKey(const OverlayProposedCandidate& proposed_candidate)
{
...
OverlayCombinationToTest::~OverlayCombinationToTest() = default;
OverlayCombinationToTest::OverlayCombinationToTest(OverlayCombinationToTest&& other) = default;
OverlayCombinationToTest& OverlayCombinationToTest::operator=(OverlayCombinationToTest&& other) = default;

CombinationIdMapper::CombinationIdMapper() = default;
CombinationIdMapper::~CombinationIdMapper() = default;

OverlayCombinationCache::CandidateId CombinationIdMapper::GetCandidateId(const OverlayProposedCandidate& proposed_candidate)
{
...
void CombinationIdMapper::RemoveStaleIds(const OverlayCombinationCache::CandidateCombination& stale_candidates)
{
    if (stale_candidates.none()) {
        return;
    }
    std::erase_if(candidate_ids_, [&stale_candidates](auto& entry) { return stale_candidates[entry.second]; });
...
OverlayCombinationToTest OverlayCombinationCache::GetOverlayCombinationToTest(
    base::span<OverlayProposedCandidate const> sorted_candidates, int max_overlays_considered)
{
    DCHECK_LE(max_overlays_considered, static_cast<int>(kMaxTrackedCandidates));

    OverlayCombinationToTest result;

    size_t max_overlays_possible = std::min({ static_cast<size_t>(max_overlays_considered), sorted_candidates.size(), kMaxTrackedCandidates });
    if (max_overlays_possible == 0) {
        return result;
    }

    std::vector<OverlayProposedCandidate> considered_candidates = GetConsideredCandidates(sorted_candidates, max_overlays_possible);
    ...
    std::vector<std::pair<CandidateCombination, int>> power_sorted_combinations = GetPowerSortedCombinations(considered_candidates, considered_ids);
    ...
    for (auto& entry : power_sorted_combinations) {
        ...
        if (combo_result != CombinationResult::kFailed) {
            result.previously_succeeded = combo_result == CombinationResult::kPromoted;
...
std::vector<OverlayProposedCandidate> OverlayCombinationCache::GetConsideredCandidates(
    base::span<OverlayProposedCandidate const> sorted_candidates, size_t max_overlays_possible)
{
...
std::vector<OverlayCombinationCache::CandidateId> OverlayCombinationCache::GetIds(const std::vector<OverlayProposedCandidate>& considered_candidates)
{
...
std::vector<std::pair<OverlayCombinationCache::CandidateCombination, int>> OverlayCombinationCache::GetPowerSortedCombinations(
    const std::vector<OverlayProposedCandidate>& considered_candidates, const std::vector<CandidateId>& considered_ids)
{
    ...
    for (size_t i = 1; i < num_combinations; ++i) {
        ...
        combination_power_gains.emplace_back(std::make_pair(combo, total_power_gain));
    }
    ...
    std::stable_sort(combination_power_gains.begin(), combination_power_gains.end(), [](const auto& a, const auto& b) { return a.second > b.second; });

    return combination_power_gains;
}

void OverlayCombinationCache::DeclarePromotedCandidates(base::span<OverlayProposedCandidate const> attempted_candidates)
{
...
void OverlayCombinationCache::RemoveStaleCombinations(const CandidateCombination& stale_candidates)
{
    if (stale_candidates.none()) {
        return;
    }
