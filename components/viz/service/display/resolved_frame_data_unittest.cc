...
{
    return SurfaceId(kArbitraryFrameSinkId, LocalSurfaceId(1, 1, base::UnguessableToken::Create()));
}
...
CompositorFrame MakeSimpleFrame(const gfx::Rect& damage_rect = kOutputRect)
{
    return CompositorFrameBuilder()
        .AddRenderPass(RenderPassBuilder(kOutputRect).AddSolidColorQuad(kOutputRect, SkColors::kRed).SetDamageRect(kOutputRect))
        .Build();
}
...
std::unique_ptr<CompositorRenderPass> BuildRenderPass(int id)
{
    ...
    render_pass->SetNew(CompositorRenderPassId::FromUnsafeValue(id), kOutputRect, kOutputRect, gfx::Transform());
    return render_pass;
}

void AddRenderPassQuad(CompositorRenderPass* render_pass, CompositorRenderPassId render_pass_id)
{
    auto* sqs = render_pass->CreateAndAppendSharedQuadState();
    sqs->SetAll(gfx::Transform(), kOutputRect, kOutputRect, gfx::MaskFilterInfo(),
        /*clip=*/std::nullopt, /*contents_opaque=*/false, 1, SkBlendMode::kSrcOver, /*sorting_context=*/0,
        /*layer_id=*/0u, /*fast_rounded_corner=*/false);
    auto* quad = render_pass->CreateAndAppendDrawQuad<CompositorRenderPassDrawQuad>();
    quad->SetNew(sqs, kOutputRect, kOutputRect, render_pass_id, kInvalidResourceId, gfx::RectF(), gfx::Size(), gfx::Vector2dF(1.0f, 1.0f), gfx::PointF(),
        gfx::RectF(),
...
public:
    ResolvedFrameDataTest()
        : support_(std::make_unique<CompositorFrameSinkSupport>(nullptr, &frame_sink_manager_, surface_id_.frame_sink_id(),
            /*is_root=*/true))
    {
    }
...
protected:
    ...
    {
        support_->SubmitCompositorFrame(surface_id_.local_surface_id(), std::move(frame));

        Surface* surface = frame_sink_manager_.surface_manager()->GetSurfaceForId(surface_id_);
        EXPECT_TRUE(surface);
    ...
    FrameSinkManagerImpl frame_sink_manager_ { FrameSinkManagerImpl::InitParams(&shared_bitmap_manager_) };
...
{
    auto frame = CompositorFrameBuilder().AddRenderPass(BuildRenderPass(101)).AddRenderPass(BuildRenderPass(102)).AddRenderPass(BuildRenderPass(103)).Build();

    Surface* surface = SubmitCompositorFrame(std::move(frame));
    ResolvedFrameData resolved_frame(&resource_provider_, surface, 0u, AggregatedRenderPassId());
    ...
    for (auto& render_pass : surface->GetActiveFrame().render_pass_list) {
        const ResolvedPassData& resolved_pass = resolved_frame.GetRenderPassDataById(render_pass->id);
...
{
    auto frame = CompositorFrameBuilder().AddRenderPass(BuildRenderPass(1)).AddRenderPass(BuildRenderPass(1)).Build();
...
TEST_F(ResolvedFrameDataTest, RenderPassIdsSelfCycle)
{
    ...
    auto frame = CompositorFrameBuilder().AddRenderPass(std::move(render_pass)).Build();
...
TEST_F(ResolvedFrameDataTest, RenderPassIdsCycle)
{
    ...
    auto frame = CompositorFrameBuilder().AddRenderPass(std::move(render_pass1)).AddRenderPass(std::move(render_pass2)).Build();
    Surface* surface = SubmitCompositorFrame(std::move(frame));
    ResolvedFrameData resolved_frame(&resource_provider_, surface, 0u, AggregatedRenderPassId());
...
TEST_F(ResolvedFrameDataTest, RenderPassWithPerQuadDamage)
{
    ...
    ResolvedFrameData resolved_frame(&resource_provider_, surface, 1u, AggregatedRenderPassId());
...
{
    Surface* surface = SubmitCompositorFrame(MakeSimpleFrame());
    AggregatedRenderPassId prev_root_pass_id = render_pass_id_generator_.GenerateNextId();
    ResolvedFrameData resolved_frame(&resource_provider_, surface, 0u, prev_root_pass_id);

    resolved_frame.UpdateForAggregation(render_pass_id_generator_);

    EXPECT_EQ(resolved_frame.GetRootRenderPassData().remapped_id(), prev_root_pass_id);
...
TEST_F(ResolvedFrameDataTest, OffsetTags)
{
...

    {
        ...
        resolved_frame.UpdateOffsetTags([&first_offset](const OffsetTagDefinition&) { return first_offset; });

        ASSERT_THAT(resolved_frame.GetResolvedPasses(), testing::SizeIs(1));
        const CompositorRenderPass& resolved_render_pass = resolved_frame.GetResolvedPasses()[0].render_pass();
        ...
        EXPECT_THAT(resolved_render_pass.quad_list,
            testing::ElementsAre(testing::AllOf(IsSolidColorQuad(SkColors::kRed), HasTransform(gfx::Transform::MakeTranslation({}))),
                testing::AllOf(IsSolidColorQuad(SkColors::kBlack), HasTransform(gfx::Transform::MakeTranslation(first_offset)))));
...

    {
        ...
        resolved_frame.UpdateOffsetTags([&second_offset](const OffsetTagDefinition&) { return second_offset; });
        ...
        EXPECT_THAT(resolved_render_pass.quad_list,
            testing::ElementsAre(testing::AllOf(IsSolidColorQuad(SkColors::kRed), HasTransform(gfx::Transform::MakeTranslation({}))),
                testing::AllOf(IsSolidColorQuad(SkColors::kBlack), HasTransform(gfx::Transform::MakeTranslation(second_offset)))));
...

    {
        ...
        auto new_frame = CompositorFrameBuilder()
                             .AddRenderPass(RenderPassBuilder(kOutputRect).AddSolidColorQuad(gfx::Rect(10, 10), SkColors::kBlack).SetQuadOffsetTag(offset_tag))
                             .AddOffsetTagDefinition(tag_def)
                             .Build();
        ...
        EXPECT_THAT(resolved_render_pass.quad_list,
            testing::ElementsAre(testing::AllOf(IsSolidColorQuad(SkColors::kBlack), HasTransform(gfx::Transform::MakeTranslation(second_offset)))));
...
TEST_F(ResolvedFrameDataTest, OffsetTagValueIsClamped)
{
    ...
    resolved_frame.UpdateOffsetTags([&offset](const OffsetTagDefinition&) { return offset; });
    ...
    EXPECT_THAT(resolved_render_pass.quad_list,
        testing::ElementsAre(testing::AllOf(IsSolidColorQuad(SkColors::kBlack), HasTransform(gfx::Transform::MakeTranslation(clamped_offset)))));
...
TEST_F(ResolvedFrameDataTest, OffsetTagWithCopyRequest)
{
    ...
    resolved_frame.UpdateOffsetTags([](const OffsetTagDefinition&) { return gfx::Vector2dF(1, 1); });
    ...
    EXPECT_THAT(surface->GetActiveFrame().render_pass_list[0]->copy_requests, testing::SizeIs(1));
    EXPECT_THAT(resolved_frame.GetResolvedPasses()[0].render_pass().copy_requests, testing::IsEmpty());
...
TEST_F(ResolvedFrameDataTest, OffsetTagClientDamageIsOffset)
{
    ...
    {
        auto frame = CompositorFrameBuilder().AddRenderPass(RenderPassBuilder(kOutputRect).AddSolidColorQuad(kOutputRect, SkColors::kRed)).Build();
    ...
    {
        resolved_frame.UpdateForAggregation(render_pass_id_generator_);
        resolved_frame.UpdateOffsetTags([](const OffsetTagDefinition&) { return gfx::Vector2dF(); });
        resolved_frame.ResetAfterAggregation();
    }
...

    {
        ...
        resolved_frame.UpdateForAggregation(render_pass_id_generator_);
        resolved_frame.UpdateOffsetTags([](const OffsetTagDefinition&) { return gfx::Vector2dF(20, -20); });
...
TEST_F(ResolvedFrameDataTest, OffsetTagLayerRemovedDamage)
{
...

    {
        ...
        EXPECT_TRUE(resolved_frame.GetSurfaceDamage().Contains(gfx::Rect(20, 70, 50, 20)));
...
TEST_F(ResolvedFrameDataTest, OffsetTagContainingRectNonRootRenderPass)
{
    ...
    Surface* surface = nullptr;
    {
        gfx::Transform child_quad_to_child_pass = gfx::Transform::MakeTranslation(30, 30);
        ...
        auto frame = CompositorFrameBuilder()
                         .AddRenderPass(RenderPassBuilder(child_pass_id, kOutputRect)
                                            .SetTransformToRootTarget(child_pass_to_root_pass)
                                            .AddSolidColorQuad(gfx::Rect(10, 10), SkColors::kBlack)
                                            .SetQuadToTargetTransform(child_quad_to_child_pass)
                                            .SetQuadOffsetTag(tag_def.tag))
                         .AddRenderPass(RenderPassBuilder(root_pass_id, kOutputRect)
...

    {
        ...
        // 25,25 5x5.
        EXPECT_EQ(helper.GetCurrentContainingRect(tag_def.tag), gfx::Rect(25, 25, 5, 5));
...
TEST_F(ResolvedFrameDataTest, OffsetTagMaskFilterTranslated)
{
...

    {
        ...
        ASSERT_THAT(resolved_frame.GetResolvedPasses(), testing::SizeIs(1));
        auto& resolved_render_pass = resolved_frame.GetResolvedPasses()[0].render_pass();

        auto translated_mask_info = mask_info;
        translated_mask_info.ApplyTransform(gfx::Transform::MakeTranslation(offset));

        // Verify that the mask filter is translated.
        EXPECT_THAT(
            resolved_render_pass.quad_list, testing::ElementsAre(testing::AllOf(IsSolidColorQuad(SkColors::kRed), HasMaskFilterInfo(translated_mask_info))));
