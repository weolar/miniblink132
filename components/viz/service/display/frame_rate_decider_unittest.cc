...
class FrameRateDeciderTest : public testing::Test, public FrameRateDecider::Client, public SurfaceManagerDelegate {
public:
    FrameRateDeciderTest()
        : frame_(MakeDefaultCompositorFrame())
    {
    }
    ...
    void SetUp() override
    {
        surface_manager_ = std::make_unique<SurfaceManager>(this, /*activation_deadline_in_frames=*/std::nullopt,
            /*max_uncommitted_frames=*/0);
        bool hw_support_for_multiple_refresh_rates = true;
        frame_rate_decider_ = std::make_unique<FrameRateDecider>(surface_manager_.get(), this, hw_support_for_multiple_refresh_rates, false);
    ...
    base::TimeDelta GetPreferredFrameIntervalForFrameSinkId(const FrameSinkId& id, mojom::CompositorFrameSinkType* type) override
    {
        if (type) {
            ...
            *type = it == frame_sink_types_.end() ? mojom::CompositorFrameSinkType::kMediaStream : it->second;
        }
...
protected:
    ...
    Surface* CreateAndDrawSurface(const FrameSinkId& frame_sink_id, LocalSurfaceId local_surface_id = LocalSurfaceId(1u, base::UnguessableToken::Create()))
    {
        SurfaceId surface_id(frame_sink_id, local_surface_id);
        SurfaceInfo surface_info(surface_id, frame_.device_scale_factor(), frame_.size_in_pixels());
        auto* surface = surface_manager_->CreateSurface(surface_client(), surface_info, SurfaceId());
    ...
    void UpdateFrame(Surface* surface)
    {
        ...
        ASSERT_TRUE(surface->QueueFrame(MakeDefaultCompositorFrame(), frame_index, base::ScopedClosureRunner()));
...
TEST_F(FrameRateDeciderTest, ActiveSurfaceTrackingFrameIndexChange)
{
    ...
    const std::vector<base::TimeDelta> supported_intervals = { preferred_interval / 2, preferred_interval };
...
TEST_F(FrameRateDeciderTest, ActiveSurfaceTrackingSurfaceIdChange)
{
    ...
    LocalSurfaceId new_surface_id(prev_surface_id.parent_sequence_number() + 1, prev_surface_id.embed_token());
    CreateAndDrawSurface(frame_sink_id, new_surface_id);
    EXPECT_EQ(display_interval_, preferred_interval);
}

TEST_F(FrameRateDeciderTest, SurfaceWithMinIntervalPicksLowestSupportedInterval)
{
    base::TimeDelta min_supported_interval = base::Seconds(1);
    const std::vector<base::TimeDelta> supported_intervals = { min_supported_interval * 3, min_supported_interval * 2, min_supported_interval };
    frame_rate_decider_->SetSupportedFrameIntervals(supported_intervals);
    EXPECT_EQ(display_interval_, FrameRateDecider::UnspecifiedFrameInterval());
    ...
    preferred_intervals_[min_interval_frame_sink_id] = BeginFrameArgs::MinInterval();
...
{
    base::TimeDelta min_supported_interval = base::Seconds(1);
    const std::vector<base::TimeDelta> supported_intervals = { min_supported_interval * 2, min_supported_interval };
    frame_rate_decider_->SetSupportedFrameIntervals(supported_intervals);
    EXPECT_EQ(display_interval_, FrameRateDecider::UnspecifiedFrameInterval());
...
TEST_F(FrameRateDeciderTest, TogglesAfterMinNumOfFrames)
{
    ...
    // interval.
    frame_rate_decider_->set_min_num_of_frames_to_toggle_interval_for_testing(50u);
...
TEST_F(FrameRateDeciderTest, TogglesWithSyntheticBFS)
{
    ...
    frame_rate_decider_ = std::make_unique<FrameRateDecider>(surface_manager_.get(), this, hw_support_for_multiple_refresh_rate, false);
    frame_rate_decider_->set_min_num_of_frames_to_toggle_interval_for_testing(0u);
    base::TimeDelta min_supported_interval = base::Seconds(1);
    const std::vector<base::TimeDelta> supported_intervals = { min_supported_interval * 2, min_supported_interval };
    frame_rate_decider_->SetSupportedFrameIntervals(supported_intervals);
...
TEST_F(FrameRateDeciderTest, NoFixedIntervalSurfaces)
{
    ...
    for (int i = 0; i < 3; ++i) {
        FrameSinkId frame_sink_id(1u, i);
        preferred_intervals_[frame_sink_id] = FrameRateDecider::UnspecifiedFrameInterval();
        frame_sink_types_[frame_sink_id] = mojom::CompositorFrameSinkType::kLayerTree;
        surfaces[i] = CreateAndDrawSurface(frame_sink_id);
    }
...
TEST_F(FrameRateDeciderTest, NoHwSupportForMultiRefreshRates)
{
    ...
    const std::vector<base::TimeDelta> supported_intervals = { min_supported_interval * 4, min_supported_interval * 2, min_supported_interval };
    frame_rate_decider_->SetSupportedFrameIntervals(supported_intervals);
    EXPECT_EQ(display_interval_, FrameRateDecider::UnspecifiedFrameInterval());
    ...
    frame_sink_types_[video_frame_sink_id] = mojom::CompositorFrameSinkType::kVideo;
    ...
    frame_sink_types_[content_frame_sink_id] = mojom::CompositorFrameSinkType::kLayerTree;
    ...
    scoped_feature_list.InitAndEnableFeature(features::kSingleVideoFrameRateThrottling);
