...
void PointInterpolate(const gfx::Point3F& from, const gfx::Point3F& to, double delta, gfx::Point3F* out)
{
    out->SetPoint(from.x() + (to.x() - from.x()) * delta, from.y() + (to.y() - from.y()) * delta, from.z() + (to.z() - from.z()) * delta);
}
} // namespace
...
DrawPolygon::DrawPolygon(const DrawQuad* original, const std::vector<gfx::Point3F>& in_points, const gfx::Vector3dF& normal, int draw_order_index)
    : normal_(normal)
...
DrawPolygon::DrawPolygon(const DrawQuad* original_ref, const gfx::RectF& visible_layer_rect, const gfx::Transform& transform, int draw_order_index)
    ...
    , is_split_(false)
{
    ...
    cc::MathUtil::MapClippedQuad3d(transform, send_quad, points, &num_vertices_in_clipped_quad);
...
void DrawPolygon::TransformToLayerSpace(const gfx::Transform& inverse_transform)
{
...
// document order).
void DrawPolygon::SplitPolygon(
    std::unique_ptr<DrawPolygon> polygon, std::unique_ptr<DrawPolygon>* front, std::unique_ptr<DrawPolygon>* back, bool* is_coplanar) const
{
    ...
    const auto prev = [num_points](size_t i) { return (i + num_points - 1) % num_points; };
    ...
    *is_coplanar = false;
    ...
    front_begin = base::ranges::find_if(vertex_distance, [](float val) { return val > 0.0; }) - vertex_distance.begin();
    ...
    back_begin = base::ranges::find_if(vertex_distance, [](float val) { return val < 0.0; }) - vertex_distance.begin();
    ...
    PointInterpolate(polygon->points_[pre_front_begin], polygon->points_[front_begin],
        vertex_distance[pre_front_begin] / (vertex_distance[pre_front_begin] - vertex_distance[front_begin]), &pre_pos_intersection);
    ...
    PointInterpolate(polygon->points_[pre_back_begin], polygon->points_[back_begin],
        vertex_distance[pre_back_begin] / (vertex_distance[pre_back_begin] - vertex_distance[back_begin]), &pre_neg_intersection);
    ...
    *front = std::make_unique<DrawPolygon>(polygon->original_ref_, out_points, polygon->normal_, polygon->order_index_);
    ...
    *back = std::make_unique<DrawPolygon>(polygon->original_ref_, out_points, polygon->normal_, polygon->order_index_);
...
void DrawPolygon::ToQuads2D(std::vector<gfx::QuadF>* quads) const
{
    ...
    while (offset < points_.size() - 1) {
        ...
        quads->push_back(gfx::QuadF(first, gfx::PointF(points_[offset].x(), points_[offset].y()), gfx::PointF(points_[op1].x(), points_[op1].y()),
