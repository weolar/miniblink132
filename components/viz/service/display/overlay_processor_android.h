...
// gl compositing.
class VIZ_SERVICE_EXPORT OverlayProcessorAndroid : public OverlayProcessorUsingStrategy {
public:
    explicit OverlayProcessorAndroid(DisplayCompositorMemoryAndTaskController* display_controller);
    ...
    void ScheduleOverlays(DisplayResourceProvider* display_resource_provider) override;
    ...
    void CheckOverlaySupportImpl(const OverlayProcessorInterface::OutputSurfaceOverlayPlane* primary_plane, OverlayCandidateList* candidates) override;
    gfx::Rect GetOverlayDamageRectForOutputSurface(const OverlayCandidate& overlay) const override;
...
private:
    ...
    void InitializeOverlayProcessorOnGpu(gpu::DisplayCompositorMemoryAndTaskControllerOnGpu* display_controller_on_gpu, base::WaitableEvent* event);
    ...
    void NotifyOverlayPromotion(
        DisplayResourceProvider* display_resource_provider, const OverlayCandidateList& candidate_list, const QuadList& quad_list) override;
    ...
    using OverlayResourceLock = DisplayResourceProvider::ScopedReadLockSharedImage;
