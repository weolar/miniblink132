...
//             this, "cc::ResourceProvider", base::SingleThreadTaskRunner::GetCurrentDefault());
//     }
}
...
bool DisplayResourceProvider::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    *(int*)1 = 1;
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
//             uint64_t total_bytes = resource.transferable.format.EstimatedSizeInBytes(resource.transferable.size);
//             dump->AddScalar(
//                 base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, static_cast<uint64_t>(total_bytes));
//         }
// 
...
//         constexpr int kImportance = static_cast<int>(gpu::TracingImportance::kServiceOwner);
//         if (resource.transferable.is_software) {
//             pmd->CreateSharedMemoryOwnershipEdge(dump->guid(), resource.shared_bitmap_tracing_guid, kImportance);
//         } else {
...
bool DisplayResourceProvider::DoesResourceWantPromotionHint(ResourceId id) const
{
...
{
    const ChildResource* resource = GetResource(id);
    return children_.contains(resource->child_id) ? children_.at(resource->child_id).surface_id : SurfaceId();
}
...
const gfx::Size DisplayResourceProvider::GetResourceBackedSize(ResourceId id) const
{
    return GetResource(id)->transferable.size;
}

SharedImageFormat DisplayResourceProvider::GetSharedImageFormat(ResourceId id) const
{
    const ChildResource* resource = GetResource(id);
    return resource->transferable.format;
}

const gfx::ColorSpace& DisplayResourceProvider::GetColorSpace(ResourceId id) const
{
    const ChildResource* resource = GetResource(id);
...
const gfx::HDRMetadata& DisplayResourceProvider::GetHDRMetadata(ResourceId id) const
{
    const ChildResource* resource = GetResource(id);
    return resource->transferable.hdr_metadata;
}

int DisplayResourceProvider::CreateChild(ReturnCallback return_callback, const SurfaceId& surface_id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void DisplayResourceProvider::ReceiveFromChild(int child_id, const std::vector<TransferableResource>& resources)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    for (const TransferableResource& transferable_resource : resources) {
        auto resource_in_map_it = child_info.child_to_parent_map.find(transferable_resource.id);
        ...
        if (transferable_resource.is_software != IsSoftware() || transferable_resource.is_empty()) {
            TRACE_EVENT0("viz", "DisplayResourceProvider::ReceiveFromChild dropping invalid");
        ...
        DCHECK(!transferable_resource.is_software || transferable_resource.IsSoftwareSharedImage()
            || (!transferable_resource.IsSoftwareSharedImage() && transferable_resource.format.IsBitmapFormatSupported()));
        resources_.emplace(local_id, ChildResource(child_id, transferable_resource));
        child_info.child_to_parent_map[transferable_resource.id] = local_id;
    }
}

void DisplayResourceProvider::DeclareUsedResourcesFromChild(int child, const ResourceIdSet& resources_from_child)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
const std::unordered_map<ResourceId, ResourceId, ResourceIdHasher>& DisplayResourceProvider::GetChildToParentMap(int child) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
const DisplayResourceProvider::ChildResource* DisplayResourceProvider::GetResource(ResourceId id) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(id);
...
DisplayResourceProvider::ChildResource* DisplayResourceProvider::GetResource(ResourceId id)
{
    return const_cast<DisplayResourceProvider::ChildResource*>(const_cast<const DisplayResourceProvider*>(this)->GetResource(id));
}

const DisplayResourceProvider::ChildResource* DisplayResourceProvider::TryGetResource(ResourceId id) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    if (!id)
        return nullptr;
...
DisplayResourceProvider::ChildResource* DisplayResourceProvider::TryGetResource(ResourceId id)
{
    return const_cast<DisplayResourceProvider::ChildResource*>(const_cast<const DisplayResourceProvider*>(this)->TryGetResource(id));
}

void DisplayResourceProvider::OnResourceFencePassed(ResourceFence* resource_fence, base::flat_set<ResourceId> resources)
{
    for (auto local_id : resources) {
        auto it = resources_.find(local_id);
        if (it == resources_.end() || resource_fence != it->second.resource_fence.get()) {
            continue;
        }
...
void DisplayResourceProvider::TryReleaseResource(ResourceId id, ChildResource* resource)
{
...
bool DisplayResourceProvider::ResourceFenceHasPassed(const ChildResource* resource) const
{
    return !resource->resource_fence || resource->resource_fence->HasPassed();
}

DisplayResourceProvider::CanDeleteNowResult DisplayResourceProvider::CanDeleteNow(
    const Child& child_info, const ChildResource& resource, DeleteStyle style) const
{
    if (resource.InUse()) {
...
void DisplayResourceProvider::DeleteAndReturnUnusedResourcesToChild(ChildMap::iterator child_it, DeleteStyle style, const std::vector<ResourceId>& unused)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    DCHECK(child_it != children_.end());
    Child& child_info = child_it->second;
    ...
    std::vector<ReturnedResource> to_return = DeleteAndReturnUnusedResourcesToChildImpl(child_info, style, unused);

    if (!to_return.empty())
        child_info.return_callback.Run(std::move(to_return));

    if (child_info.marked_for_deletion && child_info.child_to_parent_map.empty()) {
        children_.erase(child_it);
    }
}

void DisplayResourceProvider::DestroyChildInternal(ChildMap::iterator it, DeleteStyle style)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
DisplayResourceProvider::ScopedReadLockSharedImage::ScopedReadLockSharedImage(DisplayResourceProvider* resource_provider, ResourceId resource_id)
...
DisplayResourceProvider::ScopedReadLockSharedImage::ScopedReadLockSharedImage(ScopedReadLockSharedImage&& other)
{
    *this = std::move(other);
}
...
DisplayResourceProvider::ScopedReadLockSharedImage& DisplayResourceProvider::ScopedReadLockSharedImage::operator=(ScopedReadLockSharedImage&& other)
{
    Reset();
...
void DisplayResourceProvider::ScopedReadLockSharedImage::SetReleaseFence(gfx::GpuFenceHandle release_fence)
{
    DCHECK(resource_);
    resource_->release_fence = std::move(release_fence);
}

bool DisplayResourceProvider::ScopedReadLockSharedImage::HasReadLockFence() const
{
    DCHECK(resource_);
    return resource_->transferable.synchronization_type == TransferableResource::SynchronizationType::kGpuCommandsCompleted;
}
...
DisplayResourceProvider::ScopedBatchReturnResources::ScopedBatchReturnResources(DisplayResourceProvider* resource_provider, bool allow_access_to_gpu_thread)
    : resource_provider_(resource_provider)
    , was_access_to_gpu_thread_allowed_(resource_provider_->can_access_gpu_thread_)
{
...
DisplayResourceProvider::ScopedBatchReturnResources::~ScopedBatchReturnResources()
{
    ...
    resource_provider_->SetAllowAccessToGPUThread(was_access_to_gpu_thread_allowed_);
}

DisplayResourceProvider::Child::Child() = default;
DisplayResourceProvider::Child::Child(Child&& other) = default;
DisplayResourceProvider::Child& DisplayResourceProvider::Child::operator=(Child&& other) = default;
DisplayResourceProvider::Child::~Child() = default;

DisplayResourceProvider::ChildResource::ChildResource(int child_id, const TransferableResource& transferable)
    : child_id(child_id)
...
DisplayResourceProvider::ChildResource::ChildResource(ChildResource&& other) = default;
DisplayResourceProvider::ChildResource::~ChildResource() = default;

void DisplayResourceProvider::ChildResource::UpdateSyncToken(const gpu::SyncToken& sync_token)
{
