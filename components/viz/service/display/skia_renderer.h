...
public:
    ...
    SkiaRenderer(const RendererSettings* settings, const DebugRendererSettings* debug_settings, OutputSurface* output_surface,
    ...
    void SwapBuffersComplete(const gpu::SwapBuffersCompleteParams& params, gfx::GpuFenceHandle release_fence) override;
    void BuffersPresented() override;
    void DidReceiveReleasedOverlays(const std::vector<gpu::Mailbox>& released_overlays) override;

    DelegatedInkPointRendererBase* GetDelegatedInkPointRenderer(bool create_if_necessary) override;
    void SetDelegatedInkMetadata(std::unique_ptr<gfx::DelegatedInkMetadata> metadata) override;
...
protected:
    ...
    void UpdateRenderPassTextures(const AggregatedRenderPassList& render_passes_in_draw_order,
        const base::flat_map<AggregatedRenderPassId, RenderPassRequirements>& render_passes_in_frame) override;
    void AllocateRenderPassResourceIfNeeded(const AggregatedRenderPassId& render_pass_id, const RenderPassRequirements& requirements) override;
    bool IsRenderPassResourceAllocated(const AggregatedRenderPassId& render_pass_id) const override;
    gfx::Size GetRenderPassBackingPixelSize(const AggregatedRenderPassId& render_pass_id) override;

    void SetRenderPassBackingDrawnRect(const AggregatedRenderPassId& render_pass_id, const gfx::Rect& drawn_rect) override;

    gfx::Rect GetRenderPassBackingDrawnRect(const AggregatedRenderPassId& render_pass_id) const override;
    void SetScissorTestRect(const gfx::Rect& scissor_rect) override;
    void BeginDrawingRenderPass(
        const AggregatedRenderPass* render_pass, bool needs_clear, const gfx::Rect& render_pass_update_rect, const gfx::Size& viewport_size) override;
    ...
    void CopyDrawnRenderPass(const copy_output::RenderPassGeometry& geometry, std::unique_ptr<CopyOutputRequest> request) override;
    void DidChangeVisibility() override;
    void SetDelegatedInkPointRendererSkiaForTest(std::unique_ptr<DelegatedInkPointRendererSkia> renderer) override;
...
private:
    ...
    void PrepareCanvas(const std::optional<gfx::Rect>& scissor_rect, const std::optional<gfx::MaskFilterInfo>& mask_filter_info, const gfx::Transform* cdt);
    void PrepareGradient(const std::optional<gfx::MaskFilterInfo>& mask_filter_info);
    ...
    void PrepareCanvasForRPDQ(const DrawRPDQParams& rpdq_params, DrawQuadParams* params);
    ...
    void PreparePaintOrCanvasForRPDQ(const DrawRPDQParams& rpdq_params, DrawQuadParams* params, SkPaint* paint);
    ...
    void PrepareColorOrCanvasForRPDQ(const DrawRPDQParams& rpdq_params, DrawQuadParams* params, SkColor4f* color);
    ...
    DrawRPDQParams CalculateRPDQParams(const gfx::AxisTransform2d& target_to_device, const AggregatedRenderPassDrawQuad* quad, const DrawQuadParams* params);
    // Modifies |params| and |rpdq_params| to apply correctly when drawing the
    // RenderPass directly via |bypass_quad|.
    BypassMode CalculateBypassParams(const DrawQuad* bypass_quad, DrawRPDQParams* rpdq_params, DrawQuadParams* params) const;

    SkCanvas::ImageSetEntry MakeEntry(const SkImage* image, int matrix_index, const DrawQuadParams& params) const;
    ...
    SkCanvas::SrcRectConstraint ResolveTextureConstraints(const SkImage* image, const gfx::RectF& valid_texel_bounds, DrawQuadParams* params) const;

    bool MustFlushBatchedQuads(const DrawQuad* new_quad, const DrawRPDQParams* rpdq_params, const DrawQuadParams& params) const;
    void AddQuadToBatch(const SkImage* image, const gfx::RectF& valid_texel_bounds, DrawQuadParams* params);
    ...
    void DrawQuadInternal(const DrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params);
    ...
    void DrawColoredQuad(SkColor4f color, const DrawRPDQParams* rpdq_params, DrawQuadParams* params);
    ...
    void DrawSingleImage(const SkImage* image, const gfx::RectF& valid_texel_bounds, const DrawRPDQParams* rpdq_params, SkPaint* paint, DrawQuadParams* params);

    void DrawPaintOpBuffer(
    ...
    void DrawRenderPassQuad(const AggregatedRenderPassDrawQuad* quad, const DrawRPDQParams* bypassed_rpdq_params, DrawQuadParams* params);
    ...
    void DrawDebugBorderQuad(const DebugBorderDrawQuad* quad, DrawQuadParams* params);
    ...
    void DrawSolidColorQuad(const SolidColorDrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params);

    void DrawTextureQuad(const TextureDrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params);
    void DrawTileDrawQuad(const TileDrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params);

    void DrawUnsupportedQuad(const DrawQuad* quad, const DrawRPDQParams* rpdq_params, DrawQuadParams* params);
    ...
    const DrawQuad* CanPassBeDrawnDirectly(const AggregatedRenderPass* pass, const RenderPassRequirements& requirements) override;

    void DrawDelegatedInkTrail(const gfx::Transform& root_target_to_render_pass_transform);
    ...
    sk_sp<SkColorFilter> GetColorSpaceConversionFilter(const gfx::ColorSpace& src, std::optional<uint32_t> src_bit_depth,
    ...
    struct RenderPassBacking {
        RenderPassBacking();
        RenderPassBacking(gfx::Size size, bool generate_mipmap, gfx::ColorSpace color_space, RenderPassAlphaType alpha_type, SharedImageFormat format,
            gpu::Mailbox mailbox, bool is_root, bool is_scanout, bool scanout_dcomp_surface);
    ...
    // overlay.
    std::optional<SkiaRenderer::RenderPassBacking> GetRenderPassBackingForDirectScanout(const AggregatedRenderPassId& render_pass_id) const;

    RenderPassOverlayParams* GetOrCreateRenderPassOverlayBacking(AggregatedRenderPassId render_pass_id, const AggregatedRenderPassDrawQuad* rpdq,
        SharedImageFormat buffer_format, gfx::ColorSpace color_space, const gfx::Size& buffer_size);

    void PrepareRenderPassOverlay(OverlayProcessorInterface::PlatformOverlayCandidate* overlay);
#endif
    ...
    void MaybeScheduleBackgroundImage(OverlayProcessorInterface::CandidateList& candidate_list);
#endif

    // A map from RenderPass id to the texture used to draw the RenderPass from.
    base::flat_map<AggregatedRenderPassId, RenderPassBacking> render_pass_backings_;
    sk_sp<SkColorSpace> RenderPassBackingSkColorSpace(const RenderPassBacking& backing)
    {
        return backing.color_space.GetWithSdrWhiteLevel(CurrentFrameSDRWhiteLevel()).ToSkColorSpace();
    }
    ...
    scoped_refptr<FrameResourceGpuCommandsCompletedFence> current_gpu_commands_completed_fence_;
    ...
    public:
        ScopedInFlightRenderPassOverlayBackingRef(SkiaRenderer* renderer, const gpu::Mailbox& mailbox);
        ~ScopedInFlightRenderPassOverlayBackingRef();

        ScopedInFlightRenderPassOverlayBackingRef(ScopedInFlightRenderPassOverlayBackingRef&& other);
        ScopedInFlightRenderPassOverlayBackingRef& operator=(ScopedInFlightRenderPassOverlayBackingRef&& other);

        ScopedInFlightRenderPassOverlayBackingRef(const ScopedInFlightRenderPassOverlayBackingRef&) = delete;
        ScopedInFlightRenderPassOverlayBackingRef& operator=(const ScopedInFlightRenderPassOverlayBackingRef&) = delete;
    ...
    struct OverlayLock {
        OverlayLock(DisplayResourceProvider* resource_provider, ResourceId resource_id);
        ...
        std::optional<DisplayResourceProviderSkia::ScopedReadLockSharedImage> resource_lock;
    ...
    base::circular_deque<std::vector<OverlayLock>> read_lock_release_fence_overlay_locks_;
    ...
    std::unordered_set<OverlayLock, OverlayLockHash, OverlayLockKeyEqual> awaiting_release_overlay_locks_;
