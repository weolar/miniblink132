...
public:
    ...
    virtual void MakePromiseSkImage(ExternalUseClient::ImageContext* image_context, const gfx::ColorSpace& yuv_color_space, bool force_rgbx) = 0;
    ...
    // paint render pass.
    virtual SkCanvas* BeginPaintRenderPass(const AggregatedRenderPassId& id, const gfx::Size& size, SharedImageFormat format, RenderPassAlphaType alpha_type,
        skgpu::Mipmapped mipmap, bool scanout_dcomp_surface, sk_sp<SkColorSpace> color_space, bool is_overlay, const gpu::Mailbox& mailbox)
        = 0;
    ...
    // this render pass.
    virtual sk_sp<SkImage> MakePromiseSkImageFromRenderPass(const AggregatedRenderPassId& id, const gfx::Size& size, SharedImageFormat format, bool mipmap,
        sk_sp<SkColorSpace> color_space, const gpu::Mailbox& mailbox)
        = 0;

    // Remove cached resources generated by BeginPaintRenderPass and
    // FinishPaintRenderPass.
    virtual void RemoveRenderPassResource(std::vector<AggregatedRenderPassId> ids) = 0;
    ...
    virtual void CopyOutput(const copy_output::RenderPassGeometry& geometry, const gfx::ColorSpace& color_space, std::unique_ptr<CopyOutputRequest> request,
    ...
    virtual void ScheduleOverlays(OverlayList overlays, std::vector<gpu::SyncToken> sync_tokens) = 0;
    ...
    virtual void ScheduleGpuTaskForTesting(base::OnceClosure callback, std::vector<gpu::SyncToken> sync_tokens) = 0;
    ...
    virtual gpu::Mailbox CreateSharedImage(SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space, RenderPassAlphaType alpha_type,
    ...
    virtual gpu::Mailbox CreateSolidColorSharedImage(const SkColor4f& color, const gfx::ColorSpace& color_space) = 0;
    ...
    virtual void SetSharedImagePurgeable(const gpu::Mailbox& mailbox, bool purgeable) = 0;

    virtual bool SupportsBGRA() const = 0;

#if BUILDFLAG(ENABLE_VULKAN) && BUILDFLAG(IS_CHROMEOS) && BUILDFLAG(USE_V4L2_CODEC)
    virtual void DetileOverlay(gpu::Mailbox input, const gfx::Size& input_visible_size, gpu::SyncToken input_sync_token, gpu::Mailbox output,
