...
{
    return cc::FuzzyPixelComparator(false, 100.f, 0.f, kAvgAbsoluteErrorLimit, kMaxAbsoluteErrorLimit, 0);
}
...
SharedQuadState* CreateSharedQuadState(AggregatedRenderPass* render_pass, const gfx::Rect& rect)
{
    ...
    shared_state->SetAll(gfx::Transform(), layer_rect, visible_layer_rect, gfx::MaskFilterInfo(), /*clip_rect=*/absl::nullopt,
        /*are_contents_opaque=*/false, /*opacity=*/1.0f, SkBlendMode::kSrcOver,
...
void DeleteSharedImage(scoped_refptr<ContextProvider> context_provider, gpu::Mailbox mailbox, const gpu::SyncToken& sync_token, bool is_lost)
{
...
void ReadbackTextureOnGpuThread(gpu::SharedImageManager* shared_image_manager, gpu::SharedContextState* context_state, const gpu::Mailbox& mailbox,
    const gfx::Size& texture_size, SkColorType color_type, SkBitmap& out_bitmap)
{
    DCHECK(color_type == kAlpha_8_SkColorType || color_type == kR8G8_unorm_SkColorType);
    ...
    size_t row_bytes = texture_size.width() * (color_type == kAlpha_8_SkColorType ? 1 : 2);
    ...
    SkPixmap pixmap(SkImageInfo::Make(texture_size.width(), texture_size.height(), color_type, kUnpremul_SkAlphaType), out_bitmap.getAddr(0, 0), row_bytes);
    ...
    TestGpuServiceHolder* gpu_service_holder, CopyOutputResult& result, const gfx::Size& texture_size, SkBitmap& out_luma_plane, SkBitmap& out_chroma_planes)
{
    ...
    gpu_service_holder->ScheduleCompositorGpuTask(base::BindLambdaForTesting([&out_luma_plane, &out_chroma_planes, &result, &wait, &texture_size]() {
        ...
        ReadbackTextureOnGpuThread(
            shared_image_manager, context_state, result.GetTextureResult()->planes[0].mailbox, texture_size, kAlpha_8_SkColorType, out_luma_plane);

        ReadbackTextureOnGpuThread(shared_image_manager, context_state, result.GetTextureResult()->planes[1].mailbox,
            gfx::Size(texture_size.width() / 2, texture_size.height() / 2), kR8G8_unorm_SkColorType, out_chroma_planes);
...
protected:
    ...
    void SetUpReadbackPixeltest(bool scale_by_half)
    {
        ...
        ASSERT_TRUE(cc::ReadPNGFile(GetTestFilePath(FILE_PATH_LITERAL("16_color_rects.png")), &source_bitmap_));
...
private:
    ...
    std::unique_ptr<AggregatedRenderPass> GenerateRootRenderPass(const SkBitmap& bitmap)
    {
        ...
        ResourceFormat format = (bitmap.info().colorType() == kBGRA_8888_SkColorType) ? ResourceFormat::BGRA_8888 : ResourceFormat::RGBA_8888;
        ResourceId resource_id = CreateGpuResource(source_size, format, MakePixelSpan(bitmap));
    ...
    ResourceId CreateGpuResource(const gfx::Size& size, ResourceFormat format, base::span<const uint8_t> pixels)
    {
        gpu::SharedImageInterface* sii = child_context_provider_->SharedImageInterface();
        DCHECK(sii);
        gpu::Mailbox mailbox = sii->CreateSharedImage(
            format, size, gfx::ColorSpace(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, gpu::SHARED_IMAGE_USAGE_DISPLAY_READ, pixels);
        gpu::SyncToken sync_token = sii->GenUnverifiedSyncToken();

        TransferableResource gl_resource = TransferableResource::MakeGpu(mailbox, GL_LINEAR, GL_TEXTURE_2D, sync_token, size, format,
            /*is_overlay_candidate=*/false);
        auto release_callback = base::BindOnce(&DeleteSharedImage, child_context_provider_, mailbox);
        return child_resource_provider_->ImportResource(gl_resource, std::move(release_callback));
    }
...
class SkiaReadbackPixelTestRGBA : public SkiaReadbackPixelTest, public testing::WithParamInterface<bool> {
public:
...
TEST_P(SkiaReadbackPixelTestRGBA, ExecutesCopyRequest)
{
    ...
    if (!cc::MatchesPNGFile(actual, expected_path, cc::ExactPixelComparator(false))) {
        LOG(ERROR) << "Entire source: " << cc::GetPNGDataUrl(GetSourceBitmap());
        ADD_FAILURE();
    }
}

INSTANTIATE_TEST_SUITE_P(All, SkiaReadbackPixelTestRGBA,
    // Result scaling: Scale by half?
    testing::Values(true, false));

class SkiaReadbackPixelTestNV12 : public SkiaReadbackPixelTest, public testing::WithParamInterface<std::tuple<bool, CopyOutputResult::Destination>> {
public:
    CopyOutputResult::Destination RequestDestination() const
    {
...
INSTANTIATE_TEST_SUITE_P(, SkiaReadbackPixelTestNV12,
    // Result scaling: Scale by half?
    testing::Combine(
...
class SkiaReadbackPixelTestNV12WithBlit : public SkiaReadbackPixelTest, public testing::WithParamInterface<std::tuple<bool, LetterboxingBehavior, bool>> {
public:
    CopyOutputResult::Destination RequestDestination() const
    {
...
TEST_P(SkiaReadbackPixelTestNV12WithBlit, ExecutesCopyRequestWithBlit)
{
    ...
    for (size_t i = 0; i < CopyOutputResult::kNV12MaxPlanes; ++i) {
        const auto resource_format = i == 0 ? ResourceFormat::RED_8 : ResourceFormat::RG_88;
        const gfx::Size plane_size = i == 0 ? source_size : gfx::Size(source_size.width() / 2, source_size.height() / 2);
        const size_t plane_size_in_bytes = plane_size.GetArea() * (resource_format == ResourceFormat::RED_8 ? 1 : 2);

        std::vector<uint8_t> pixels = (i == 0) ? GeneratePixels(plane_size_in_bytes, luma_pattern) : GeneratePixels(plane_size_in_bytes, chromas_pattern);

        mailboxes[i].mailbox = child_context_provider_->SharedImageInterface()->CreateSharedImage(resource_format, plane_size, gfx::ColorSpace::CreateREC709(),
            kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, gpu::SHARED_IMAGE_USAGE_DISPLAY_READ, pixels);
    ...
    std::unique_ptr<CopyOutputResult> result = IssueCopyOutputRequestAndRender(RequestFormat(), RequestDestination(),
        base::BindLambdaForTesting([this, &result_selection, &destination_subregion, &mailboxes](CopyOutputRequest& request) {
            // Build CopyOutputRequest based on test parameters.
            if (ScaleByHalf()) {
                request.SetUniformScaleRatio(2, 1);
            }
            ...
            request.set_blit_request(BlitRequest(destination_subregion.origin(), GetLetterboxingBehavior(), mailboxes, populates_gpu_memory_buffer()));
        }));
    ...
    for (size_t i = 0; i < CopyOutputResult::kNV12MaxPlanes; ++i) {
        child_context_provider_->SharedImageInterface()->DestroySharedImage(
            result->GetTextureResult()->planes[i].sync_token, result->GetTextureResult()->planes[i].mailbox);
    }
...
#if !BUILDFLAG(IS_ANDROID_EMULATOR)
INSTANTIATE_TEST_SUITE_P(, SkiaReadbackPixelTestNV12WithBlit,
...
// with NV12 format if the platform does not support GL_EXT_texture_rg extension
// in GL ES 2.0 (which is the case on Android emulator).
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(SkiaReadbackPixelTestNV12WithBlit);
#endif
