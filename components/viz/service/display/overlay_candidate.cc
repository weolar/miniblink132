...
bool OverlayCandidate::IsInvisibleQuad(const DrawQuad* quad)
{
    ...
    if (quad->material != DrawQuad::Material::kSolidColor)
        return false;
    const float alpha = SolidColorDrawQuad::MaterialCast(quad)->color.fA * opacity;
    return quad->ShouldDrawWithBlending() && cc::MathUtil::IsWithinEpsilon(alpha, 0.f);
}
...
// static
void OverlayCandidate::ApplyClip(OverlayCandidate& candidate, const gfx::RectF& clip_rect)
{
    ...
    if (!clip_rect.Contains(candidate.display_rect)) {
        ...
        gfx::Transform buffer_transform = gfx::OverlayTransformToTransform(absl::get<gfx::OverlayTransform>(candidate.transform), gfx::SizeF(1, 1));
        ...
        gfx::RectF uv_rect = cc::MathUtil::ScaleRectProportional(candidate.uv_rect, candidate.display_rect, intersect_clip_display);
        candidate.display_rect = intersect_clip_display;

        // Return |uv_rect| to buffer uv space.
        candidate.uv_rect = buffer_transform.InverseMapRect(uv_rect).value_or(uv_rect);
    }
}
...
bool OverlayCandidate::RequiresOverlay(const DrawQuad* quad)
{
    ...
    case DrawQuad::Material::kTextureContent:
        return TextureDrawQuad::MaterialCast(quad)->protected_video_type == gfx::ProtectedVideoType::kHardwareProtected
            || TextureDrawQuad::MaterialCast(quad)->overlay_priority_hint == OverlayPriority::kRequired;
...
// static
gfx::RectF OverlayCandidate::DisplayRectInTargetSpace(const OverlayCandidate& candidate)
{
    if (absl::holds_alternative<gfx::Transform>(candidate.transform)) {
        return absl::get<gfx::Transform>(candidate.transform).MapRect(candidate.display_rect);
    }
