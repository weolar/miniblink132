...
public:
    ...
    void SurfaceDamagedForTest(const SurfaceId& surface_id, const BeginFrameAck& ack, bool display_damaged, bool is_handling_interaction = false)
    {
        if (display_damaged)
            undrawn_surfaces_.insert(surface_id);
        HandleInteraction interaction = is_handling_interaction ? HandleInteraction::kYes : HandleInteraction::kNo;
        ProcessSurfaceDamage(surface_id, ack, display_damaged, interaction);
    }
    void ClearUndrawnSurfaces()
    {
...
public:
    ...
    int draw_and_swap_count() const
    {
        return draw_and_swap_count_;
    }

    void SetNextDrawAndSwapFails()
    {
        next_draw_and_swap_fails_ = true;
    }

    const BeginFrameAck& last_begin_frame_ack()
    {
...
public:
    TestDisplayScheduler(DisplayDamageTracker* damage_tracker, BeginFrameSource* begin_frame_source, SurfaceManager* surface_manager,
        base::SingleThreadTaskRunner* task_runner, int max_pending_swaps, bool wait_for_all_surfaces_before_draw)
        : DisplayScheduler(begin_frame_source, task_runner, PendingSwapParams(max_pending_swaps),
            /*hint_session_factory=*/nullptr, wait_for_all_surfaces_before_draw)
    ...
    base::TimeTicks DesiredBeginFrameDeadlineTimeForTest()
    {
        ...
        return DesiredBeginFrameDeadlineTime(deadline_mode, current_begin_frame_args_);
    }
...
public:
    explicit DisplaySchedulerTest(bool wait_for_all_surfaces_before_draw = false)
        ...
        , resource_provider_(&shared_bitmap_manager_, &shared_image_manager_, &sync_point_manager_, &gpu_scheduler_)
        , aggregator_(&surface_manager_, &resource_provider_, false)
        , damage_tracker_(std::make_unique<TestDisplayDamageTracker>(&surface_manager_, &aggregator_))
    ...
    void AdvanceTimeAndBeginFrameForTest(const std::vector<SurfaceId>& observing_surfaces)
    {
        now_src_.Advance(base::Microseconds(10000));
        // FakeBeginFrameSource deals with |source_id| and |sequence_number|.
        last_begin_frame_args_ = fake_begin_frame_source_.CreateBeginFrameArgs(BEGINFRAME_FROM_HERE, &now_src_);
        fake_begin_frame_source_.TestOnBeginFrame(last_begin_frame_args_);
        for (const auto& surface_id : observing_surfaces) {
            damage_tracker_->OnSurfaceDamageExpected(surface_id, last_begin_frame_args_);
        }
    }

    void SurfaceDamaged(const SurfaceId& surface_id)
    {
        damage_tracker_->SurfaceDamagedForTest(surface_id, AckForCurrentBeginFrame(), true);
    }
...
protected:
    ...
    FakeDisplaySchedulerClient& client()
    {
        return client_;
    }
...
void DisplaySchedulerTest::SetUp()
{
    scheduler_ = std::make_unique<TestDisplayScheduler>(
        damage_tracker_.get(), &fake_begin_frame_source_, &surface_manager_, task_runner_.get(), kMaxPendingSwaps, wait_for_all_surfaces_before_draw_);
...
{
    SurfaceId root_surface_id1(kArbitraryFrameSinkId, LocalSurfaceId(1, base::UnguessableToken::Create()));
    SurfaceId root_surface_id2(kArbitraryFrameSinkId, LocalSurfaceId(2, base::UnguessableToken::Create()));
    SurfaceId sid1(kArbitraryFrameSinkId, LocalSurfaceId(3, base::UnguessableToken::Create()));
    ...
    EXPECT_GE(now_src().NowTicks(), scheduler_->DesiredBeginFrameDeadlineTimeForTest());
    scheduler_->BeginFrameDeadlineForTest();
...
{
    SurfaceId root_surface_id(kArbitraryFrameSinkId, LocalSurfaceId(1, base::UnguessableToken::Create()));
    SurfaceId sid1(kArbitraryFrameSinkId, LocalSurfaceId(2, base::UnguessableToken::Create()));
    base::TimeTicks late_deadline;
...
TEST_F(DisplaySchedulerTest, SurfaceDamaged)
{
    SurfaceId root_surface_id(kArbitraryFrameSinkId, LocalSurfaceId(1, base::UnguessableToken::Create()));
    SurfaceId sid1(kArbitraryFrameSinkId, LocalSurfaceId(2, base::UnguessableToken::Create()));
    SurfaceId sid2(kArbitraryFrameSinkId, LocalSurfaceId(3, base::UnguessableToken::Create()));
    ...
    EXPECT_LT(now_src().NowTicks(), scheduler_->DesiredBeginFrameDeadlineTimeForTest());
    ...
    EXPECT_GE(now_src().NowTicks(), scheduler_->DesiredBeginFrameDeadlineTimeForTest());
    scheduler_->BeginFrameDeadlineForTest();
    EXPECT_EQ(BeginFrameAck(last_begin_frame_args_, true), client_.last_begin_frame_ack());
    ...
    EXPECT_LT(now_src().NowTicks(), scheduler_->DesiredBeginFrameDeadlineTimeForTest());
    scheduler_->BeginFrameDeadlineForTest();
    EXPECT_EQ(BeginFrameAck(last_begin_frame_args_, false), client_.last_begin_frame_ack());
    ...
    damage_tracker_->SurfaceDamagedForTest(sid1, AckForCurrentBeginFrame(), false);
    EXPECT_FALSE(scheduler_->inside_begin_frame_deadline_interval());

    // Deadline should trigger early if child surfaces are idle and
    // we get damage on the root surface.
    damage_tracker_->OnSurfaceDamageExpected(root_surface_id, last_begin_frame_args_);
    EXPECT_FALSE(scheduler_->inside_begin_frame_deadline_interval());
    SurfaceDamaged(root_surface_id);
    EXPECT_GE(now_src().NowTicks(), scheduler_->DesiredBeginFrameDeadlineTimeForTest());
    scheduler_->BeginFrameDeadlineForTest();
...
TEST_F(DisplaySchedulerWaitForAllSurfacesTest, WaitForAllSurfacesBeforeDraw)
{
    ...
    EXPECT_EQ(base::TimeTicks::Max(), scheduler_->DesiredBeginFrameDeadlineTimeForTest());
...
TEST_F(DisplaySchedulerTest, RootFrameMissing)
{
    ...
    EXPECT_TRUE(scheduler_->inside_begin_frame_deadline_interval());
    SurfaceDamaged(root_surface_id);
    EXPECT_EQ(base::TimeTicks(), scheduler_->DesiredBeginFrameDeadlineTimeForTest());
...
TEST_F(DisplaySchedulerTest, DidSwapBuffers)
{
    ...
    base::TimeTicks expected_deadline = last_begin_frame_args_.deadline - BeginFrameArgs::DefaultEstimatedDisplayDrawTime(last_begin_frame_args_.interval);
    EXPECT_EQ(expected_deadline, scheduler_->DesiredBeginFrameDeadlineTimeForTest());
...
TEST_F(DisplaySchedulerTest, DefaultBeginFrameArgsDeadline)
{
    ...
    BeginFrameArgs args = fake_begin_frame_source_.CreateBeginFrameArgsWithGenerator(frame_time, next_frame_time, kVSyncInterval);
...
TEST_F(ImmediateInteractiveDrawTest, DoNotWaitWhenInteracting)
{
    ...
    damage_tracker_->SurfaceDamagedForTest(sid1, ack, display_damaged, is_handling_interaction);
...
TEST_F(ImmediateInteractiveDrawTest, WaitWhenNotInteracting)
{
    ...
    EXPECT_TRUE(scheduler_->has_pending_surfaces());
    EXPECT_LT(base::TimeTicks(), scheduler_->DesiredBeginFrameDeadlineTimeForTest());
}
...
TEST_F(ImmediateInteractiveDrawTest, ResetScrollingBitAfterDrawAndSwap)
{
    ...
    damage_tracker_->SurfaceDamagedForTest(sid1, ack, display_damaged, is_handling_interaction);
    EXPECT_TRUE(scheduler_->has_pending_surfaces());
    EXPECT_NE(base::TimeTicks(), scheduler_->DesiredBeginFrameDeadlineTimeForTest());
}
