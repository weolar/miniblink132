...
void ExpectResult(const std::optional<Result> result_opt, FrameIntervalClass frame_interval_class)
{
...
void ExpectResult(const std::optional<Result> result_opt, base::TimeDelta interval)
{
    ASSERT_TRUE(result_opt.has_value());
    const Result& result = result_opt.value();
...
ContinuousRangeSettings BuildContinuousRangeSettings(base::TimeDelta min_interval = base::Hertz(120), base::TimeDelta max_interval = base::Hertz(40))
{
...
TEST(FrameIntervalMatchersTest, InputBoostIgnoreOldSinks)
{
    ...
    FrameIntervalInputs& frame_interval_inputs = inputs.inputs_map[FrameSinkId(0, 1)];
...
TEST(FrameIntervalMatchersTest, OnlyVideo)
{
    ...
    FrameIntervalInputs& frame_interval_inputs = inputs.inputs_map[FrameSinkId(0, 1)];
    frame_interval_inputs.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Milliseconds(32) });
...
TEST(FrameIntervalMatchersTest, OnlyVideoFixedInterval)
{
    ...
    FrameIntervalInputs& frame_interval_inputs = inputs.inputs_map[FrameSinkId(0, 1)];
    frame_interval_inputs.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Milliseconds(24) });
    frame_interval_inputs.has_only_content_frame_interval_updates = true;
...
TEST(FrameIntervalMatchersTest, OnlyVideoFixedIntervalNoSimpleCadence)
{
    ...
    FrameIntervalInputs& frame_interval_inputs = inputs.inputs_map[FrameSinkId(0, 1)];
    frame_interval_inputs.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Milliseconds(23) });
    frame_interval_inputs.has_only_content_frame_interval_updates = true;
...
TEST(FrameIntervalMatchersTest, OnlyVideoDifferentIntervals)
{
    ...
    interval_inputs1.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Milliseconds(32) });
    ...
    interval_inputs2.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Milliseconds(24) });
    interval_inputs2.has_only_content_frame_interval_updates = true;

    ExpectNullResult(matcher.Match(inputs));

    interval_inputs2.content_interval_info[0].frame_interval = base::Milliseconds(32);
    ExpectResult(matcher.Match(inputs), base::Milliseconds(32));
}
...
TEST(FrameIntervalMatchersTest, OnlyVideoContinuousRange)
{
    ...
    // supported range.
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/1u);
        FrameIntervalInputs& frame_interval_inputs = inputs.inputs_map[FrameSinkId(0, 1)];
        frame_interval_inputs.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(60) });
        frame_interval_inputs.has_only_content_frame_interval_updates = true;
    ...
    // interval falls above the supported range.
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/1u);
        FrameIntervalInputs& frame_interval_inputs = inputs.inputs_map[FrameSinkId(0, 1)];
        frame_interval_inputs.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(35) });
        frame_interval_inputs.has_only_content_frame_interval_updates = true;
    ...
    // Verify the same (where the expected cadence = 3).
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/1u);
        FrameIntervalInputs& frame_interval_inputs = inputs.inputs_map[FrameSinkId(0, 1)];
        frame_interval_inputs.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(15) });
        frame_interval_inputs.has_only_content_frame_interval_updates = true;
    ...
    // interval falls below the supported range.
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/1u);
        FrameIntervalInputs& frame_interval_inputs = inputs.inputs_map[FrameSinkId(0, 1)];
        frame_interval_inputs.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(160) });
        frame_interval_inputs.has_only_content_frame_interval_updates = true;
    ...
    // Verify the same (where the expected cadence = 4).
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/1u);
        FrameIntervalInputs& frame_interval_inputs = inputs.inputs_map[FrameSinkId(0, 1)];
        frame_interval_inputs.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(400) });
        frame_interval_inputs.has_only_content_frame_interval_updates = true;

        ExpectResult(matcher.Match(inputs), base::Hertz(100));
    }

    settings.interval_settings = BuildContinuousRangeSettings(base::Hertz(60), base::Hertz(48));
    ...
    // cadence when the target interval falls above the supported range.
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/1u);
        FrameIntervalInputs& frame_interval_inputs = inputs.inputs_map[FrameSinkId(0, 1)];
        frame_interval_inputs.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(40) });
        frame_interval_inputs.has_only_content_frame_interval_updates = true;
    ...
    // cadence when the target interval falls below the supported range.
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/1u);
        FrameIntervalInputs& frame_interval_inputs = inputs.inputs_map[FrameSinkId(0, 1)];
        frame_interval_inputs.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(80) });
        frame_interval_inputs.has_only_content_frame_interval_updates = true;
...
{
    Settings settings;
    FixedIntervalSettings fixed_interval_settings = BuildDenseFixedIntervalSettings();
    ...
    // interval.
    for (base::TimeDelta supported_interval : fixed_interval_settings.supported_intervals) {
        ASSERT_LE((supported_interval - input1_interval).magnitude(), settings.epsilon);
        ASSERT_LE((supported_interval - input2_interval).magnitude(), settings.epsilon);
    }
    ...
    // multiple options within epsilon.
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/3u);
        FrameIntervalInputs& interval_inputs1 = inputs.inputs_map[FrameSinkId(0, 1)];
        interval_inputs1.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, input1_interval });
        FrameIntervalInputs& interval_inputs2 = inputs.inputs_map[FrameSinkId(0, 2)];
        interval_inputs2.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, input1_interval });
    ...
    // Verify the same when the input interval is closer to the other side.
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/3u);
        FrameIntervalInputs& interval_inputs1 = inputs.inputs_map[FrameSinkId(0, 1)];
        interval_inputs1.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, input2_interval });
        FrameIntervalInputs& interval_inputs2 = inputs.inputs_map[FrameSinkId(0, 2)];
        interval_inputs2.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, input2_interval });
        ExpectResult(matcher.Match(inputs), base::Hertz(60));
    }
}
...
TEST(FrameIntervalMatchersTest, VideoConferenceDuplicateCount)
{
    ...
    FrameIntervalInputs& interval_inputs1 = inputs.inputs_map[FrameSinkId(0, 1)];
    interval_inputs1.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Milliseconds(32), 2u });
    ExpectResult(matcher.Match(inputs), base::Milliseconds(32));
}
...
TEST(FrameIntervalMatchersTest, VideoConferenceContinuousRange)
{
    ...
    // supported range.
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/3u);
        FrameIntervalInputs& interval_inputs1 = inputs.inputs_map[FrameSinkId(0, 1)];
        interval_inputs1.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(60) });
        FrameIntervalInputs& interval_inputs2 = inputs.inputs_map[FrameSinkId(0, 2)];
        interval_inputs2.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(50) });
        ExpectResult(matcher.Match(inputs), base::Hertz(60));
    }
    ...
    // interval falls below the supported range.
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/3u);
        FrameIntervalInputs& interval_inputs1 = inputs.inputs_map[FrameSinkId(0, 1)];
        interval_inputs1.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(150) });
        FrameIntervalInputs& interval_inputs2 = inputs.inputs_map[FrameSinkId(0, 2)];
        interval_inputs2.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(50) });
    ...
    // interval falls above the supported range.
    {
        Inputs inputs = BuildDefaultInputs(settings, /*num_sinks=*/3u);
        FrameIntervalInputs& interval_inputs1 = inputs.inputs_map[FrameSinkId(0, 1)];
        interval_inputs1.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(30) });
        FrameIntervalInputs& interval_inputs2 = inputs.inputs_map[FrameSinkId(0, 2)];
        interval_inputs2.content_interval_info.push_back({ ContentFrameIntervalType::kVideo, base::Hertz(35) });
