...
bool IsRightAngledRotationOrPositiveScaleOrTranslation(const gfx::Transform& transform)
{
    if (transform.IsPositiveScaleOrTranslation()) {
        return true;
    }

    const bool is_2d_and_has_no_perspective = cc::MathUtil::IsWithinEpsilon(transform.rc(3, 0), 0.0) && cc::MathUtil::IsWithinEpsilon(transform.rc(3, 1), 0.0)
        && cc::MathUtil::IsWithinEpsilon(transform.rc(3, 2), 0.0) && cc::MathUtil::IsWithinEpsilon(transform.rc(3, 3), 1.0) && // 4th row
        cc::MathUtil::IsWithinEpsilon(transform.rc(2, 0), 0.0) && cc::MathUtil::IsWithinEpsilon(transform.rc(2, 1), 0.0)
        && cc::MathUtil::IsWithinEpsilon(transform.rc(2, 2), 1.0) && cc::MathUtil::IsWithinEpsilon(transform.rc(2, 3), 0.0) && // 3rd row
        cc::MathUtil::IsWithinEpsilon(transform.rc(0, 2), 0.0) && cc::MathUtil::IsWithinEpsilon(transform.rc(1, 2), 0.0);

    if (!is_2d_and_has_no_perspective || !transform.NonDegeneratePreserves2dAxisAlignment()) {
        return false;
    }

    // Only scale, translation, mirroring and right angled rotations (90, 180,
    // 270) preserve axis alignment.
    const bool has_translation = std::abs(transform.rc(0, 3)) > kEpsilon || std::abs(transform.rc(1, 3)) > kEpsilon;

    // Inspect inner 2x2 matrix to check if the `transform` has rotation or
    // positive scale.
    const bool has_0_rotation_with_positive_scaling = transform.rc(0, 0) > kEpsilon && transform.rc(1, 1) > kEpsilon;
    const bool has_90_rotation_with_positive_scaling = transform.rc(0, 1) < kEpsilon && transform.rc(1, 0) > kEpsilon;
    const bool has_180_rotation_with_positive_scaling = transform.rc(0, 0) < kEpsilon && transform.rc(1, 1) < kEpsilon;
    const bool has_270_rotation_with_positive_scaling = transform.rc(0, 1) > kEpsilon && transform.rc(1, 0) < kEpsilon;

    return is_2d_and_has_no_perspective
        && (has_translation || has_0_rotation_with_positive_scaling || has_90_rotation_with_positive_scaling || has_180_rotation_with_positive_scaling
...
bool CanSplitQuad(const DrawQuad::Material quad_material, const std::vector<gfx::Rect>& visible_region_rects, const gfx::Size& visible_region_bounding_size,
    int minimum_fragments_reduced, const float device_scale_factor)
{
    ...
    base::CheckedNumeric<int> visible_region_bounding_area = visible_region_bounding_size.GetCheckedArea();
...
gfx::RectF GetOccludingRectForRRectF(const gfx::RRectF& bounds)
{
    ...
    const auto top_right = bounds.GetCornerRadii(gfx::RRectF::Corner::kUpperRight);
    const auto lower_right = bounds.GetCornerRadii(gfx::RRectF::Corner::kLowerRight);
    const auto lower_left = bounds.GetCornerRadii(gfx::RRectF::Corner::kLowerLeft);
    ...
    occluding_rect.Inset(gfx::InsetsF::TLBR(std::max(top_left.y(), top_right.y()) * kInsetCoeficient, std::max(top_left.x(), lower_left.x()) * kInsetCoeficient,
        std::max(lower_right.y(), lower_left.y()) * kInsetCoeficient, std::max(top_right.x(), lower_right.x()) * kInsetCoeficient));
...
bool ReduceComplexity(const cc::Region& region, size_t complexity_limit, std::vector<gfx::Rect>& reduced_region)
{
    reduced_region.clear();

    for (gfx::Rect r : region) {
        auto it = base::ranges::find_if(reduced_region, [&r](const gfx::Rect& a) { return a.SharesEdgeWith(r); });
...
OcclusionCuller::OcclusionCuller(OverlayProcessorInterface* overlay_processor, const RendererSettings::OcclusionCullerSettings& settings)
...
void OcclusionCuller::RemoveOverdrawQuads(AggregatedFrame* frame, float device_scale_factor)
{
    ...
    for (const auto& pass : frame->render_pass_list) {
        if (!pass->backdrop_filters.IsEmpty() && pass->backdrop_filters.HasFilterThatMovesPixels()) {
            backdrop_filter_rects[pass->id] = cc::MathUtil::MapEnclosingClippedRect(pass->transform_to_root_target, pass->output_rect);
        }
    }
        ...
        for (auto quad = pass->quad_list.begin(); quad != quad_list_end;) {
            ...
            if (last_sqs != quad->shared_quad_state) {
                if (last_sqs->opacity == 1 && last_sqs->are_contents_opaque
                    && (last_sqs->blend_mode == SkBlendMode::kSrcOver || last_sqs->blend_mode == SkBlendMode::kSrc)
                    && last_sqs->quad_to_target_transform.NonDegeneratePreserves2dAxisAlignment()) {
                    gfx::Rect sqs_rect_in_target
                        = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(last_sqs->quad_to_target_transform, last_sqs->visible_quad_layer_rect);
                    ...
                    if (last_sqs->mask_filter_info.HasRoundedCorners()) {
                        sqs_rect_in_target.Intersect(gfx::ToEnclosedRect(GetOccludingRectForRRectF(last_sqs->mask_filter_info.rounded_corner_bounds())));
                    }
                    ...
                    while (occlusion_in_target_space.GetRegionComplexity() > settings_.maximum_occluder_complexity) {
                ...
                const auto current_sqs_in_target_space = cc::MathUtil::MapEnclosingClippedRect(transform, last_sqs->visible_quad_layer_rect);
                current_sqs_intersects_occlusion = occlusion_in_target_space.Intersects(current_sqs_in_target_space);
                ...
                if (current_sqs_intersects_occlusion && IsRightAngledRotationOrPositiveScaleOrTranslation(transform)) {
                    // Given:
                    ...
                    DCHECK_LE(occlusion_in_target_space.GetRegionComplexity(), settings_.maximum_occluder_complexity);
                    ...
                    for (gfx::Rect rect_in_target_space : occlusion_in_target_space) {
                        if (current_sqs_in_target_space.Intersects(rect_in_target_space)) {
                            auto rect_in_content = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(reverse_transform, rect_in_target_space);
                            occlusion_in_quad_content_space.Union(SafeConvertRectForRegion(rect_in_content));
                        }
                    }
                    ...
                    // render pass quad.
                    if (current_sqs_in_target_space.Intersects(backdrop_filters_in_target_space.bounds())) {
            ...
            if (occlusion_in_quad_content_space.Contains(quad->visible_rect)) {
                ...
                const bool should_split_quads = !overlay_processor_->DisableSplittingQuads() && !visible_region.Intersects(render_pass_quads_in_content_space)
                    && ReduceComplexity(visible_region, settings_.quad_split_limit, reduced_visible_region)
                    && CanSplitQuad(
                        quad->material, reduced_visible_region, visible_region.bounds().size(), settings_.minimum_fragments_reduced, device_scale_factor);
                if (should_split_quads) {
                    auto new_quad = pass->quad_list.InsertCopyBeforeDrawQuad(quad, reduced_visible_region.size() - 1);
                ...
                && occlusion_in_target_space.Contains(cc::MathUtil::MapEnclosingClippedRect(transform, quad->visible_rect))) {
