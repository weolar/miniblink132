...
void DelegatedInkPointRendererBase::InitMessagePipeline(mojo::PendingReceiver<gfx::mojom::DelegatedInkPointRenderer> receiver)
{
...
void DelegatedInkPointRendererBase::SetDelegatedInkMetadata(std::unique_ptr<gfx::DelegatedInkMetadata> metadata)
{
    ...
    TRACE_EVENT_WITH_FLOW1("delegated_ink_trails", "DelegatedInkPointRendererBase::SetDelegatedInkMetadata", TRACE_ID_GLOBAL(metadata_->trace_id()),
        TRACE_EVENT_FLAG_FLOW_IN, "metadata", metadata_->ToString());

    // If we already have a cached pointer ID, check if the same pointer ID
    // matches the new metadata.
    if (pointer_id_.has_value() && pointer_ids_[pointer_id_.value()].ContainsMatchingPoint(metadata_.get())) {
        if (metadata_is_new) {
            metadata_paint_time_ = pointer_ids_[pointer_id_.value()].GetMatchingPoint(metadata_.get()).paint_timestamp();
        }
        return;
    }
    ...
    for (auto& it : pointer_ids_) {
        if (it.second.ContainsMatchingPoint(metadata_.get())) {
            ...
            metadata_paint_time_ = it.second.GetMatchingPoint(metadata_.get()).paint_timestamp();
            return;
        }
    }
...
std::vector<gfx::DelegatedInkPoint> DelegatedInkPointRendererBase::FilterPoints()
{
    ...
    for (const auto& [_, point] : trail_data.GetPoints()) {
        ...
        TRACE_EVENT_WITH_FLOW1(
            "delegated_ink_trails", "Filtering to draw point", TRACE_ID_GLOBAL(point.trace_id()), TRACE_EVENT_FLAG_FLOW_IN, "point", point.ToString());
    }
...
std::optional<AggregatedRenderPassId> DelegatedInkPointRendererBase::GetLatestMetadataRenderPassId() const
{
    if (metadata_) {
...
void DelegatedInkPointRendererBase::PredictPoints(std::vector<gfx::DelegatedInkPoint>* ink_points_to_draw)
{
    DCHECK(metadata_);

    if (!pointer_id_.has_value() || static_cast<int>(ink_points_to_draw->size()) == 0)
        return;

    pointer_ids_[pointer_id_.value()].PredictPoints(ink_points_to_draw, metadata_.get());
}
...
void DelegatedInkPointRendererBase::ResetPrediction()
{
    for (auto& it : pointer_ids_)
        it.second.Reset();
    TRACE_EVENT_INSTANT0("delegated_ink_trails", "Delegated ink prediction reset.", TRACE_EVENT_SCOPE_THREAD);
}
...
void DelegatedInkPointRendererBase::ReportPointsDrawn()
{
    ...
    if (metadata_paint_time_.has_value()) {
        base::UmaHistogramCustomTimes(
            "Renderer.DelegatedInkTrail.Skia.TimeFromDelegatedInkToApiPaint", now - metadata_paint_time_.value(), base::Milliseconds(1), base::Seconds(1), 50);
    ...
    base::UmaHistogramCounts100("Renderer.DelegatedInkTrail.Skia.OutstandingPointsToDraw", points_trail.size());

    for (auto& [_, point] : points_trail) {
        UMA_HISTOGRAM_TIMES("Renderer.DelegatedInkTrail.Skia.TimeToDrawPointsMillis", now - point.timestamp());
...
void DelegatedInkPointRendererBase::StoreDelegatedInkPoint(const gfx::DelegatedInkPoint& point)
{
    TRACE_EVENT_WITH_FLOW1("delegated_ink_trails", "DelegatedInkPointRendererImpl::StoreDelegatedInkPoint", TRACE_ID_GLOBAL(point.trace_id()),
