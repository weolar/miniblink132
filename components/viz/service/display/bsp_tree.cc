...
// data.
void BspTree::BuildTree(BspNode* node, base::circular_deque<std::unique_ptr<DrawPolygon>>* polygon_list)
{
    ...
    while (!polygon_list->empty()) {
        ...
        bool is_coplanar;
        node->node_data->SplitPolygon(std::move(polygon), &new_front, &new_back, &is_coplanar);
        if (is_coplanar) {
            if (new_front)
