...
public:
    ...
    explicit ScopedAllowGpuAccessForDisplayResourceProvider(DisplayResourceProvider* provider)
    {
...
std::vector<ReturnedResource> DisplayResourceProviderSkia::DeleteAndReturnUnusedResourcesToChildImpl(
    Child& child_info, DeleteStyle style, const std::vector<ResourceId>& unused)
{
    std::vector<ReturnedResource> to_return;
    std::vector<std::unique_ptr<ExternalUseClient::ImageContext>> image_contexts_to_return;
    ...
    for (ResourceId local_id : unused) {
        ...
        if (resource.transferable.synchronization_type == TransferableResource::SynchronizationType::kReleaseFence) {
    ...
    if (!image_contexts_to_return.empty()) {
        ...
        gpu::SyncToken sync_token = external_use_client_->ReleaseImageContexts(std::move(image_contexts_to_return));
...
DisplayResourceProviderSkia::LockSetForExternalUse::LockSetForExternalUse(DisplayResourceProviderSkia* resource_provider, ExternalUseClient* client)
...
ExternalUseClient::ImageContext* DisplayResourceProviderSkia::LockSetForExternalUse::LockResource(
    ResourceId id, bool maybe_concurrent_reads, bool raw_draw_is_possible)
{
    ...
    if (!resource.locked_for_external_use) {
        ...
        if (!resource.image_context) {
            ...
            // images.
            sk_sp<SkColorSpace> image_color_space = resource.transferable.color_space.GetAsFullRangeRGB().ToSkColorSpace();

            resource.image_context = resource_provider_->external_use_client_->CreateImageContext(
                gpu::MailboxHolder(resource.transferable.mailbox(), resource.transferable.sync_token(), resource.transferable.texture_target()),
                resource.transferable.size, resource.transferable.format, maybe_concurrent_reads, resource.transferable.ycbcr_info,
        ...
        case TransferableResource::SynchronizationType::kGpuCommandsCompleted:
            resource.resource_fence = resource_provider_->current_gpu_commands_completed_fence_;
            break;
...
void DisplayResourceProviderSkia::LockSetForExternalUse::UnlockResources(const gpu::SyncToken& sync_token)
{
...
DisplayResourceProviderSkia::ScopedExclusiveReadLockSharedImage::ScopedExclusiveReadLockSharedImage(
    DisplayResourceProviderSkia* resource_provider, ResourceId resource_id)
    : ScopedReadLockSharedImage(resource_provider, resource_id)
{
    ChildResource& resource = *this->resource();
    if (resource.image_context) {
        DCHECK(!resource.locked_for_external_use) << "Resource already locked, can't get exclusive lock!";
        DCHECK(resource_provider->can_access_gpu_thread_) << "Can't release |image_context| without access to gpu thread";
        ...
        gpu::SyncToken sync_token = resource_provider->external_use_client_->ReleaseImageContexts(std::move(image_contexts));
        resource.UpdateSyncToken(sync_token);
    }
}
DisplayResourceProviderSkia::ScopedExclusiveReadLockSharedImage::~ScopedExclusiveReadLockSharedImage() = default;

DisplayResourceProviderSkia::ScopedExclusiveReadLockSharedImage::ScopedExclusiveReadLockSharedImage(ScopedExclusiveReadLockSharedImage&& other) = default;
