...
class CopyOutputScalingPixelTest : public VizPixelTest,
                                   public testing::WithParamInterface<std::tuple<RendererType, gfx::Vector2d, gfx::Vector2d, CopyOutputResult::Format>> {
public:
    CopyOutputScalingPixelTest()
    ...
    void RunTest()
    {
        ...
        if (result_format_ == CopyOutputResult::Format::RGBA)
            ...
            NOTIMPLEMENTED();
        SCOPED_TRACE(testing::Message() << "scale_from=" << scale_from_.ToString() << ", scale_to=" << scale_to_.ToString()
        ...
        constexpr SkColor4f smaller_pass_colors[4] = { SkColors::kRed, SkColors::kGreen, SkColors::kBlue, SkColors::kYellow };
        ...
        for (int i = 0; i < 4; ++i, pass_id = AggregatedRenderPassId { pass_id.value() - 1 }) {
            smaller_pass_rects[i] = gfx::Rect(
                i % 2 == 0 ? x_block : (viewport_size.width() - 2 * x_block), i / 2 == 0 ? y_block : (viewport_size.height() - 2 * y_block), x_block, y_block);
            smaller_passes[i] = AddRenderPass(&list, pass_id, smaller_pass_rects[i], gfx::Transform(), cc::FilterOperations());
            cc::AddQuad(smaller_passes[i], smaller_pass_rects[i], smaller_pass_colors[i]);
        }

        // Create the root render pass and add all the child passes to it.
        auto* root_pass = cc::AddRenderPass(&list, pass_id, gfx::Rect(viewport_size), gfx::Transform(), cc::FilterOperations());
        for (int i = 0; i < 4; ++i)
        ...
        // correctly as well.
        const gfx::Rect copy_rect(x_block, y_block, viewport_size.width() - 2 * x_block, viewport_size.height() - 2 * y_block);
        std::unique_ptr<CopyOutputResult> result;
        {
            ...
            auto request = std::make_unique<CopyOutputRequest>(result_format_, CopyOutputRequest::ResultDestination::kSystemMemory,
                base::BindOnce(
            ...
            request = std::make_unique<CopyOutputRequest>(result_format_, CopyOutputRequest::ResultDestination::kSystemMemory,
                base::BindOnce(
                    [](bool* dummy_ran, std::unique_ptr<CopyOutputResult>* test_result, const base::RepeatingClosure& quit_closure,
            ...
            request->set_result_selection(copy_output::ComputeResultRect(copy_rect, scale_from_, scale_to_));
            ...
            renderer()->DrawFrame(&list, 1.0f, viewport_size, gfx::DisplayColorSpaces(), std::move(surface_damage_rect_list));
        ...
        // size.
        const gfx::Rect expected_result_rect = copy_output::ComputeResultRect(copy_rect, scale_from_, scale_to_);
        ...
        expected_bitmap.allocN32Pixels(expected_result_rect.width(), expected_result_rect.height());
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            expected_bitmap.erase(smaller_pass_colors[i], SkIRect { rect.x(), rect.y(), rect.right(), rect.bottom() });
        }
        ...
        EXPECT_EQ(0, num_bad_pixels) << "First failure position at: " << first_failure_position.ToString()
...
private:
    ...
    static SkBitmap ReadI420ResultToSkBitmap(const CopyOutputResult& result)
    {
        ...
        // Do the read.
        const bool success = result.ReadI420Planes(y_data.data(), y_stride, u_data.data(), u_stride, v_data.data(), v_stride);
        CHECK(success);

        // Convert to an SkBitmap.
        SkBitmap bitmap;
        bitmap.allocPixels(SkImageInfo::Make(result_width, result_height, kBGRA_8888_SkColorType, kPremul_SkAlphaType));
        const int error_code = libyuv::I420ToARGB(y_data.data(), y_stride, u_data.data(), u_stride, v_data.data(), v_stride,
            static_cast<uint8_t*>(bitmap.getPixels()), bitmap.rowBytes(), result_width, result_height);
...
const auto kParameters = testing::Combine(testing::ValuesIn(GetRendererTypes()),
    testing::Values(gfx::Vector2d(1, 1), gfx::Vector2d(2, 1), gfx::Vector2d(1, 2), gfx::Vector2d(2, 2)),
    testing::Values(gfx::Vector2d(1, 1), gfx::Vector2d(2, 1), gfx::Vector2d(1, 2)),
    testing::Values(CopyOutputResult::Format::RGBA, CopyOutputResult::Format::I420_PLANES));
