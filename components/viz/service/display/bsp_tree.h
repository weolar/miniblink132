...
public:
    ...
    template <typename ActionHandlerType> void TraverseWithActionHandler(ActionHandlerType* action_handler) const
    {
        if (root_) {
...
private:
    ...
    void BuildTree(BspNode* node, base::circular_deque<std::unique_ptr<DrawPolygon>>* data);

    template <typename ActionHandlerType> void WalkInOrderAction(ActionHandlerType* action_handler, DrawPolygon* item) const
    {
        (*action_handler)(item);
    }

    static bool OrderIndexLess(const std::unique_ptr<DrawPolygon>& x, const std::unique_ptr<DrawPolygon>& y)
    {
        return x->order_index() < y->order_index();
    }

    template <typename ActionHandlerType>
    void WalkInOrderVisitNodes(ActionHandlerType* action_handler, const BspNode* node, const BspNode* first_child, const BspNode* second_child,
        const std::vector<std::unique_ptr<DrawPolygon>>& first_coplanars, const std::vector<std::unique_ptr<DrawPolygon>>& second_coplanars) const
    {
        if (first_child) {
            WalkInOrderRecursion(action_handler, first_child);
        }
        DCHECK(std::is_sorted(first_coplanars.begin(), first_coplanars.end(), OrderIndexLess));
        ...
        DCHECK(std::is_sorted(second_coplanars.begin(), second_coplanars.end(), OrderIndexLess));
    ...
    template <typename ActionHandlerType> void WalkInOrderRecursion(ActionHandlerType* action_handler, const BspNode* node) const
    {
        ...
        if (GetCameraPositionRelative(*(node->node_data)) == BSP_FRONT) {
            WalkInOrderVisitNodes<ActionHandlerType>(
                action_handler, node, node->back_child.get(), node->front_child.get(), node->coplanars_back, node->coplanars_front);
        } else {
            WalkInOrderVisitNodes<ActionHandlerType>(
                action_handler, node, node->front_child.get(), node->back_child.get(), node->coplanars_back, node->coplanars_front);
        }
    }
