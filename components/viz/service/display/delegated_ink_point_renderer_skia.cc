...
void DelegatedInkPointRendererSkia::DrawDelegatedInkTrail(SkCanvas* canvas, const gfx::Transform& transform_to_render_pass)
{
    TRACE_EVENT1("viz", "DelegatedInkPointRendererSkia::DrawDelegatedInkTrail", "points", path_.countPoints());
...
base::TimeDelta GetImprovement(const std::vector<gfx::DelegatedInkPoint>* points_to_draw, const gfx::DelegatedInkMetadata* metadata)
{
...
std::vector<SkPoint> DelegatedInkPointRendererSkia::GetPointsToDraw()
{
    ...
    UMA_HISTOGRAM_TIMES("Renderer.DelegatedInkTrail.LatencyImprovement.Skia.WithoutPrediction", GetImprovement(&ink_points_to_draw, metadata_.get()));
...
void DelegatedInkPointRendererSkia::FinalizePathForDraw()
{
    ...
    TRACE_EVENT_INSTANT1("delegated_ink_trails", "Filtered and predicted points for delegated ink trail", TRACE_EVENT_SCOPE_THREAD, "points", sk_points.size());
    ...
    TRACE_EVENT_INSTANT1(
        "delegated_ink_trails", "DelegatedInkPointRendererSkia::FinalizePathForDraw", TRACE_EVENT_SCOPE_THREAD, "damage_rect", damage_rect.ToString());
