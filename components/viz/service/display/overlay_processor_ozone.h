...
class VIZ_SERVICE_EXPORT OverlayProcessorOzone : public OverlayProcessorUsingStrategy {
public:
    OverlayProcessorOzone(std::unique_ptr<ui::OverlayCandidatesOzone> overlay_candidates, std::vector<OverlayStrategy> available_strategies,
    ...
    void ReceiveHardwareCapabilities(ui::HardwareCapabilities hardware_capabilities);

    gfx::Rect GetOverlayDamageRectForOutputSurface(const OverlayCandidate& candidate) const override;
...
private:
    ...
    bool SetNativePixmapForCandidate(ui::OverlaySurfaceCandidate* candidate, const gpu::Mailbox& mailbox, bool is_primary);
