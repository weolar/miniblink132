...
std::unique_ptr<AggregatedRenderPass> CreateRenderPass()
{
    ...
    pass->SetNew(render_pass_id, kRenderPassOutputRect, kRenderPassOutputRect, gfx::Transform());
...
static ResourceId CreateResourceInLayerTree(ClientResourceProvider* child_resource_provider, const gfx::Size& size, bool is_overlay_candidate)
{
    auto resource
        = TransferableResource::MakeGpu(gpu::Mailbox::Generate(), GL_TEXTURE_2D, gpu::SyncToken(), size, SinglePlaneFormat::kRGBA_8888, is_overlay_candidate);
...
{
    ResourceId resource_id = CreateResourceInLayerTree(child_resource_provider, size, is_overlay_candidate);
...
{
    return CreateCandidateQuadAt(parent_resource_provider, child_resource_provider, child_context_provider, shared_quad_state, render_pass,
        render_pass->output_rect, gfx::ProtectedVideoType::kClear);
}
...
TEST_F(CALayerOverlayTest, AllowNonAxisAlignedTransform)
{
    ...
    pass->shared_quad_state_list.back()->quad_to_target_transform.RotateAboutZAxis(45.f);
    ...
    overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters, render_pass_backdrop_filters,
        std::move(surface_damage_rect_list), nullptr, &ca_layer_list, &damage_rect_, &content_bounds_);
...
TEST_F(CALayerOverlayTest, ThreeDTransform)
{
    ...
    gfx::Transform actual_transform(absl::get<gfx::Transform>(ca_layer_list.back().transform));
...
TEST_F(CALayerOverlayTest, NontrivialClip)
{
    ...
    EXPECT_EQ(gfx::Rect(), damage_rect_);
    EXPECT_EQ(1U, ca_layer_list.size());
    EXPECT_EQ(gfx::Rect(64, 64, 128, 128), ca_layer_list.back().clip_rect.value());
}
...
TEST_F(CALayerOverlayTest, TextureDrawQuadVideoOverlay)
{
    ...
    bool is_overlay_candidate = true;
    ResourceId resource_id = CreateResource(resource_provider_.get(), child_resource_provider_.get(), child_provider_.get(), size, is_overlay_candidate);
    ...
    // Video frames of TextureDrawQuad should be promoted to overlays.
    {
        ...
        texture_video_quad->SetNew(pass->shared_quad_state_list.back(), gfx::Rect(size), gfx::Rect(size),
            /*needs_blending=*/false, resource_id,
            /*premultiplied_alpha=*/false, kUVTopLeft, kUVBottomRight, SkColors::kTransparent,
        ...
        overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list, GetIdentityColorMatrix(), render_pass_filters,
            render_pass_backdrop_filters, std::move(surface_damage_rect_list), nullptr, &ca_layer_list, &damage_rect_, &content_bounds_);
        EXPECT_EQ(gfx::Rect(), damage_rect_);
        EXPECT_EQ(1U, ca_layer_list.size());
    }
}
...
protected:
    ...
    void ProcessForOverlays()
    {
        overlay_processor_->ProcessForOverlays(resource_provider_.get(), &pass_list_, GetIdentityColorMatrix(), render_pass_filters_,
            render_pass_backdrop_filters_, std::move(surface_damage_rect_list_), nullptr, &ca_layer_list_, &damage_rect_, &content_bounds_);
    }
...
TEST_F(CALayerOverlayRPDQTest, RenderPassDrawQuadAllValidFilters)
{
    ...
    filters_.Append(cc::FilterOperation::CreateDropShadowFilter(gfx::Point(10, 20), 1.0f, SkColors::kGreen));
