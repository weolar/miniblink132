...
public:
    DisplayDamageTrackerTest()
        ...
        , damage_tracker_(std::make_unique<DisplayDamageTracker>(manager_.surface_manager(), &aggregator_))
    {
        manager_.RegisterFrameSinkId(kRootFrameSinkId, false);
        manager_.RegisterBeginFrameSource(&fake_begin_frame_source_, kRootFrameSinkId);
...
protected:
    ...
    public:
        Client(FrameSinkManagerImpl* manager, FrameSinkId frame_sink_id)
            ...
            , support_(std::make_unique<CompositorFrameSinkSupport>(&client_, manager, frame_sink_id,
                  /*is_root=*/true))
        {
            MakeNewSurfaceId();
        ...
        void SubmitCompositorFrame(const BeginFrameId& frame_id)
        {
            ...
            CompositorFrame frame = CompositorFrameBuilder().SetRenderPassList(std::move(pass_list)).SetBeginFrameAck(ack).Build();
    ...
    void TickBeginFrame()
    {
        ...
        last_begin_frame_args_ = fake_begin_frame_source_.CreateBeginFrameArgs(BEGINFRAME_FROM_HERE);
...
TEST_F(DisplayDamageTrackerTest, Resize)
{
    ...
    EXPECT_FALSE(damage_tracker_->expecting_root_surface_damage_because_of_resize());

    // Resize display and expect there is root frame, but with expected damage
    damage_tracker_->DisplayResized();
    EXPECT_TRUE(damage_tracker_->expecting_root_surface_damage_because_of_resize());
    EXPECT_FALSE(damage_tracker_->root_frame_missing());
...
TEST_F(DisplayDamageTrackerTest, UnsolicitedFrameUnderAutoNeedsBeginFrame)
{
    ...
    root_client_.SubmitCompositorFrame(BeginFrameAck::CreateManualAckWithDamage().frame_id);
