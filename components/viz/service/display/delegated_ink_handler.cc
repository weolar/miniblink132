...
DelegatedInkHandler::DelegatedInkHandler(bool platform_supports_delegated_ink)
    : use_delegated_ink_renderer_((switches::GetDelegatedInkRendererMode() == switches::DelegatedInkRendererMode::kSkia) || !platform_supports_delegated_ink)
{
...
{
    if (use_delegated_ink_renderer_) {
        absl::get<RendererUniquePtr>(ink_data_)->SetDelegatedInkMetadata(std::move(metadata));
    } else {
...
void DelegatedInkHandler::SetDelegatedInkPointRendererForTest(RendererUniquePtr renderer)
{
