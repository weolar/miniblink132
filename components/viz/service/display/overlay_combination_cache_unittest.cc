...
class TestOverlayProcessor : public OverlayProcessorUsingStrategy {
    ...
    void CheckOverlaySupportImpl(const OverlayProcessorInterface::OutputSurfaceOverlayPlane* primary_plane, OverlayCandidateList* surfaces) override
    {
    }
...
protected:
    ...
    std::vector<OverlayProposedCandidate> GetConsideredCandidates(base::span<OverlayProposedCandidate const> sorted_candidates, size_t max_overlays_possible)
    {
        return combination_cache_.GetConsideredCandidates(sorted_candidates, max_overlays_possible);
    }
    std::vector<CandidateId> GetIds(const std::vector<OverlayProposedCandidate>& considered_candidates)
    {
    ...
    {
        return combination_cache_.GetPowerSortedCombinations(considered_candidates, considered_ids);
    }
    ...
    std::vector<OverlayProposedCandidate> MakeCandidatesWithPowerGains(std::vector<int> power_gains)
    {
        ...
        for (size_t i = 0; i < power_gains.size(); ++i) {
            ...
            OverlayProposedCandidate proposed_candidate(quad_iter, candidate, &on_top_strategy_);
        ...
        std::sort(proposed_candidates.begin(), proposed_candidates.end(),
            [](const auto& cand1, const auto& cand2) { return cand1.relative_power_gain > cand2.relative_power_gain; });

        return proposed_candidates;
    }

    void ExpectPowerGains(const std::vector<OverlayProposedCandidate>& result, const std::vector<int>& expected_power_gains) const
    {
...
TEST_F(OverlayCombinationCacheTest, NoCandidates)
{
    ...
    auto result = combination_cache_.GetOverlayCombinationToTest(sorted_candidates, 2);
...
TEST_F(OverlayCombinationCacheTest, AllCombosFail)
{
    ...
    std::vector<std::vector<int>> expectations = { { 10, 5, 1 }, { 10, 5 }, { 10, 1 }, { 10 }, { 5, 1 }, { 5 }, { 1 }, { 10 }, { 10 }, { 10 } };

    for (size_t i = 0; i < expectations.size(); ++i) {
        SCOPED_TRACE(i);
        auto result = combination_cache_.GetOverlayCombinationToTest(sorted_candidates, 3);
...
TEST_F(OverlayCombinationCacheTest, GetConsideredCandidatesWithDuplicates)
{
    ...
    OverlayProposedCandidate underlay_duplicate(front.quad_iter, front.candidate, &underlay_strategy_);
...
TEST_F(OverlayCombinationCacheTest, GetPowerSortedCombinations)
{
    ...
    std::vector<std::pair<CandidateCombination, int>> power_sorted_combinations = GetPowerSortedCombinations(considered_candidates, ids);
    ...
    power_sorted_combinations = GetPowerSortedCombinations(considered_candidates, different_ids);
...
TEST_F(OverlayCombinationCacheTest, IdMapper)
{
    ...
    EXPECT_EQ(id_mapper_.GetCandidateId(candidates[8]), OverlayCombinationCache::kInvalidCandidateId);
    EXPECT_EQ(id_mapper_.GetCandidateId(candidates[9]), OverlayCombinationCache::kInvalidCandidateId);
    // Id's are cached.
    EXPECT_EQ(id_mapper_.GetCandidateId(candidates[0]), 0u);
    ...
    EXPECT_EQ(id_mapper_.GetCandidateId(candidates[0]), OverlayCombinationCache::kInvalidCandidateId);
    ...
    // There's a maximum of 8 ids.
    EXPECT_EQ(id_mapper_.GetCandidateId(candidates[2]), OverlayCombinationCache::kInvalidCandidateId);
    EXPECT_EQ(id_mapper_.GetCandidateId(candidates[8]), OverlayCombinationCache::kInvalidCandidateId);
}
