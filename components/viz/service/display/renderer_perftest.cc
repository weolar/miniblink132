...
SharedQuadState* CreateTestSharedQuadState(
    gfx::Transform quad_to_target_transform, const gfx::Rect& rect, CompositorRenderPass* render_pass, const gfx::MaskFilterInfo& mask_filter_info)
{
...
TransferableResource CreateTestTexture(const gfx::Size& size, SkColor4f texel_color, bool premultiplied_alpha, ClientResourceProvider* child_resource_provider,
    scoped_refptr<RasterContextProvider> child_context_provider)
{
    using SkPMColor4f = SkRGBA4f<kPremul_SkAlphaType>;
    const SkPMColor4f pixel_color = premultiplied_alpha ? texel_color.premul() : SkPMColor4f { texel_color.fR, texel_color.fG, texel_color.fB, texel_color.fA };

    size_t num_pixels = static_cast<size_t>(size.width()) * size.height();
    std::vector<SkPMColor4f> pixels(num_pixels, pixel_color);

    gpu::SharedImageInterface* sii = child_context_provider->SharedImageInterface();
    DCHECK(sii);
    auto client_shared_image = sii->CreateSharedImage(
        { SinglePlaneFormat::kRGBA_8888, size, gfx::ColorSpace(), gpu::SHARED_IMAGE_USAGE_DISPLAY_READ, "TestLabel" }, base::as_byte_span(pixels));
    gpu::SyncToken sync_token = sii->GenVerifiedSyncToken();

    TransferableResource gl_resource
        = TransferableResource::MakeGpu(client_shared_image, GL_TEXTURE_2D, sync_token, size, SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
    gl_resource.color_space = gfx::ColorSpace();
    auto release_callback = base::BindOnce(&DeleteSharedImage, std::move(child_context_provider), std::move(client_shared_image));
    gl_resource.id = child_resource_provider->ImportResource(gl_resource, std::move(release_callback));
    return gl_resource;
}

void CreateTestTextureDrawQuad(ResourceId resource_id, const gfx::Rect& rect, SkColor4f background_color, bool premultiplied_alpha,
    const SharedQuadState* shared_state, CompositorRenderPass* render_pass)
{
    ...
    quad->SetNew(shared_state, rect, rect, needs_blending, resource_id, premultiplied_alpha, uv_top_left, uv_bottom_right, background_color, flipped,
        nearest_neighbor,
        /*secure_output=*/false, gfx::ProtectedVideoType::kClear);
}

void CreateTestTileDrawQuad(ResourceId resource_id, const gfx::Rect& rect, const gfx::Size& texture_size, bool premultiplied_alpha,
    const SharedQuadState* shared_state, CompositorRenderPass* render_pass)
{
    ...
    quad->SetNew(
        shared_state, rect, rect, needs_blending, resource_id, tex_coord_rect, texture_size, premultiplied_alpha, nearest_neighbor, force_anti_aliasing_off);
}
...
public:
    ...
    std::unique_ptr<SkiaOutputSurface> CreateOutputSurface(GpuServiceImpl* gpu_service, DisplayCompositorMemoryAndTaskController* display_controller)
    {
        return SkiaOutputSurfaceImpl::Create(display_controller, renderer_settings_, &debug_settings_);
    }
    ...
    void SetUp() override
    {
        ...
        child_context_provider_ = base::MakeRefCounted<TestInProcessContextProvider>(TestContextType::kSoftwareRaster, /*support_locking=*/false);
        child_context_provider_->BindToCurrentSequence();
        child_resource_provider_ = std::make_unique<ClientResourceProvider>();

        auto skia_deps = std::make_unique<SkiaOutputSurfaceDependencyImpl>(gpu_service, gpu::kNullSurfaceHandle);
        auto display_controller = std::make_unique<DisplayCompositorMemoryAndTaskController>(std::move(skia_deps));

        auto output_surface = CreateOutputSurface(gpu_service, display_controller.get());
        // WaitForSwapDisplayClient depends on this.
        output_surface->SetNeedsSwapSizeNotifications(true);
        auto overlay_processor = std::make_unique<OverlayProcessorStub>();
        display_ = std::make_unique<Display>(&shared_bitmap_manager_, /*shared_image_manager=*/nullptr,
            /*sync_point_manager=*/nullptr, /*gpu_scheduler=*/nullptr, renderer_settings_, &debug_settings_, kArbitraryFrameSinkId,
            std::move(display_controller), std::move(output_surface), std::move(overlay_processor),
            /*display_scheduler=*/nullptr, base::SingleThreadTaskRunner::GetCurrentDefault());
    ...
    {
        std::string story = ::testing::UnitTest::GetInstance()->current_test_info()->name();
        auto reporter = SetUpRendererReporter(story);
        reporter.AddResult(kMetricFps, timer_.LapsPerSecond());

        auto* histogram = base::StatisticsRecorder::FindHistogram("Compositing.Display.DrawToSwapUs");
        ...
        std::string temp_name = base::StringPrintf("%s.%s.DrawToSwapUs", info->test_suite_name(), info->name());
        auto samples = histogram->SnapshotDelta();
        base::HistogramBase* temp_histogram = base::Histogram::FactoryMicrosecondsTimeGet(
            temp_name, Display::kDrawToSwapMin, Display::kDrawToSwapMax, Display::kDrawToSwapUsBuckets, base::Histogram::kNoFlags);
        ...
        for (const auto& transferable_resource : resource_list_) {
            child_resource_provider_->RemoveImportedResource(transferable_resource.id);
        }
    ...
    {
        CompositorFrame frame = CompositorFrameBuilder().SetRenderPassList(std::move(pass_list)).SetTransferableResources(resource_list_).Build();
        support_->SubmitCompositorFrame(id_allocator_.GetCurrentLocalSurfaceId(), std::move(frame));
        ASSERT_TRUE(display_->DrawAndSwap({ base::TimeTicks::Now(), base::TimeTicks::Now() }));
    }

    ResourceId MapResourceId(base::flat_map<ResourceId, ResourceId>* resource_map, ResourceId recorded_id, const gfx::Size& texture_size, SkColor4f texel_color,
        bool premultiplied_alpha)
    {
        DCHECK(resource_map);
        ResourceId actual_id;
        if (resource_map->find(recorded_id) == resource_map->end()) {
            resource_list_.push_back(
                CreateTestTexture(texture_size, texel_color, premultiplied_alpha, child_resource_provider_.get(), child_context_provider_));
    ...
    void SetUpRenderPassListResources(CompositorRenderPassList* render_pass_list)
    {
            ...
            for (auto* quad : render_pass->quad_list) {
                ...
                case DrawQuad::Material::kTiledContent: {
                    ...
                    ResourceId actual_id = this->MapResourceId(
                        &resource_map, recorded_id, tile_quad->texture_size, SkColor4f { 0.0f, 1.0f, 0.0f, 0.5f }, tile_quad->is_premultiplied);
                    tile_quad->resources.ids[TileDrawQuad::kResourceIdIndex] = actual_id;
                } break;
                case DrawQuad::Material::kTextureContent: {
                    TextureDrawQuad* texture_quad = reinterpret_cast<TextureDrawQuad*>(quad);
                    ResourceId recorded_id = texture_quad->resource_id();
                    ResourceId actual_id = this->MapResourceId(
                        &resource_map, recorded_id, texture_quad->rect.size(), SkColor4f { 0.0f, 1.0f, 0.0f, 0.5f }, texture_quad->premultiplied_alpha);
                    texture_quad->resources.ids[TextureDrawQuad::kResourceIdIndex] = actual_id;
                } break;
                default:
                    ASSERT_TRUE(false);
                }
    ...
    {
        resource_list_.push_back(CreateTestTexture(kSurfaceSize,
            /*texel_color=*/SkColor4f { 0.0f, 1.0f, 0.0f, 0.5f },
            /*premultiplied_alpha=*/false, child_resource_provider_.get(), child_context_provider_));

        timer_.Reset();
        do {
            std::unique_ptr<CompositorRenderPass> pass = CreateTestRootRenderPass();

            SharedQuadState* shared_state = CreateTestSharedQuadState(gfx::Transform(), kSurfaceRect, pass.get(), gfx::MaskFilterInfo());
    ...
    {
        const gfx::Size kTextureSize = ScaleToCeiledSize(kSurfaceSize, /*x_scale=*/0.2, /*y_scale=*/0.2);
        ResourceId resource_ids[5][5];
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
                resource_list_.push_back(CreateTestTexture(kTextureSize,
                    /*texel_color=*/SkColor4f { 0.0f, 1.0f, 0.0f, 0.5f },
                    /*premultiplied_alpha=*/false, child_resource_provider_.get(), child_context_provider_));
        ...
        do {
            ...
            for (int i = 0; i < 5; i++) {
                for (int j = 0; j < 5; j++) {
                    CreateTestTextureDrawQuad(resource_ids[i][j],
                        gfx::Rect(i * kTextureSize.width(), j * kTextureSize.height(), kTextureSize.width(), kTextureSize.height()),
                        /*background_color=*/SkColors::kTransparent,
                        /*premultiplied_alpha=*/false, shared_state, pass.get());
                }
            }
    ...
    void RunTextureQuads5x5SameTex()
    {
        ...
        do {
            ...
            for (int i = 0; i < 5; i++) {
                for (int j = 0; j < 5; j++) {
                    CreateTestTextureDrawQuad(resource_id,
                        gfx::Rect(i * kTextureSize.width(), j * kTextureSize.height(), kTextureSize.width(), kTextureSize.height()),
                        /*background_color=*/SkColors::kTransparent,
                        /*premultiplied_alpha=*/false, shared_state, pass.get());
                }
    ...
    void RunTileQuads(int tile_count, const gfx::Transform& starting_transform, const gfx::Transform& transform_step, bool share_resources)
    {
        const gfx::Size kTextureSize = ScaleToCeiledSize(kSurfaceSize, /*x_scale=*/0.2, /*y_scale=*/0.2);
        // Make the tile size slightly smaller than the backing texture to simulate
        // undefined bottom/right edges
        const gfx::Rect kTileSize(kTextureSize.width() - 4, kTextureSize.height() - 4);
        if (share_resources) {
        ...
        do {
            ...
            for (int i = 0; i < tile_count; ++i) {
                ...
                // SharedQuadState
                SharedQuadState* shared_state = CreateTestSharedQuadState(current_transform, gfx::Rect(kSurfaceSize), pass.get(), gfx::MaskFilterInfo());
                ResourceId resource_id = share_resources ? resource_list_[0].id : resource_list_[i].id;
    ...
    void RunSingleRenderPassListFromJSON(const std::string& tag, const std::string& site, uint32_t year, size_t index)
    {
...
#define TOP_REAL_WORLD_DESKTOP_RENDERER_PERF_TEST(SITE, FRAME)                                                                                                 \
    ...
    {                                                                                                                                                          \
        this->RunSingleRenderPassListFromJSON(/*tag=*/"top_real_world_desktop", /*site=*/#SITE, /*year=*/2018, /*frame_index=*/FRAME);                         \
    }
