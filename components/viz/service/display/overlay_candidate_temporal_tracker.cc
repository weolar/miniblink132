...
int OverlayCandidateTemporalTracker::GetModeledPowerGain(
    uint64_t curr_frame, const OverlayCandidateTemporalTracker::Config& config, int display_area, bool is_fullscreen) const
{
    ...
    return static_cast<int>((ratio_rate_category_ - config.damage_rate_threshold) * display_area);
}

void OverlayCandidateTemporalTracker::CategorizeDamageRatioRate(uint64_t curr_frame, const OverlayCandidateTemporalTracker::Config& config)
{
    ...
    if (std::abs(mean_ratio_rate - ratio_rate_category_) >= damage_rate_hysteresis_range) {
        ratio_rate_category_ = mean_ratio_rate;
    }
}

bool OverlayCandidateTemporalTracker::IsActivelyChanging(uint64_t curr_frame, const OverlayCandidateTemporalTracker::Config& config) const
{
    return LastChangeFrameCount(curr_frame) < static_cast<uint64_t>(config.max_num_frames_avg);
}

float OverlayCandidateTemporalTracker::MeanFrameRatioRate(const OverlayCandidateTemporalTracker::Config& config) const
{
    return damage_record_avg_;
}

void OverlayCandidateTemporalTracker::AddRecord(
    uint64_t curr_frame, float damage_area_ratio, ResourceId resource_id, const OverlayCandidateTemporalTracker::Config& config, bool force_resource_update)
{
    if ((prev_resource_id_ != resource_id || force_resource_update) && frame_record_prev_ != curr_frame) {
        float damage_area_ratio_rate = damage_area_ratio / (curr_frame - frame_record_prev_);
        ...
        damage_record_avg_ = ((damage_record_avg_ * num_samples_) + damage_area_ratio_rate) / (num_samples_ + 1);
...
uint64_t OverlayCandidateTemporalTracker::LastChangeFrameCount(uint64_t curr_frame) const
{
