...
public:
    ...
    virtual std::unique_ptr<SharedBitmap> GetSharedBitmapFromId(const gfx::Size& size, SharedImageFormat format, const SharedBitmapId& id) = 0;
    virtual base::UnguessableToken GetSharedBitmapTracingGUIDFromId(const SharedBitmapId& id) = 0;
    // Used for locally allocated bitmaps that are not in shared memory.
    virtual bool LocalAllocatedSharedBitmap(SkBitmap bitmap, const SharedBitmapId& id) = 0;
    // Used in the display compositor to associate an id to a shm mapping.
    virtual bool ChildAllocatedSharedBitmap(base::ReadOnlySharedMemoryMapping mapping, const SharedBitmapId& id) = 0;
