...
void DelegatedInkTrailData::AddPoint(const gfx::DelegatedInkPoint& point)
{
    ...
    *(int*)1 = 1;
    //predictor_->Update(ui::InputPredictor::InputData(point.point(), point.timestamp()));
...
void DelegatedInkTrailData::PredictPoints(std::vector<gfx::DelegatedInkPoint>* ink_points_to_draw, gfx::DelegatedInkMetadata* metadata)
{
    *(int*)1 = 1;
...
//             base::TimeTicks timestamp = ink_points_to_draw->back().timestamp() + base::Milliseconds(kMillisecondsIntoFuturePerPoint);
//             std::unique_ptr<ui::InputPredictor::InputData> predicted_point = predictor_->GeneratePrediction(timestamp, base::Seconds(0));
//             metrics_handler_.AddPredictedEvent(predicted_point->pos, predicted_point->time_stamp, metadata->frame_time());
//             latency_improvement_with_prediction = predicted_point->time_stamp - metadata->timestamp();
//             ink_points_to_draw->emplace_back(predicted_point->pos, predicted_point->time_stamp, pointer_id_);
//         }
//     }
//     metrics_handler_.EvaluatePrediction();
//     base::UmaHistogramTimes("Renderer.DelegatedInkTrail.LatencyImprovement.Skia.WithPrediction", latency_improvement_with_prediction);
}
...
bool DelegatedInkTrailData::ContainsMatchingPoint(gfx::DelegatedInkMetadata* metadata) const
{
    const auto& point = points_.find(metadata->timestamp());
    return point != points_.end() && point->second.MatchesDelegatedInkMetadata(metadata);
}

gfx::DelegatedInkPoint DelegatedInkTrailData::GetMatchingPoint(gfx::DelegatedInkMetadata* metadata) const
{
    CHECK(ContainsMatchingPoint(metadata));
    return points_.find(metadata->timestamp())->second;
}

void DelegatedInkTrailData::ErasePointsOlderThanMetadata(gfx::DelegatedInkMetadata* metadata)
{
    ...
    // that can be drawn.
    while (!points_.empty() && points_.begin()->second.timestamp() < metadata->timestamp()) {
        points_.erase(points_.begin());
    }
}
...
void DelegatedInkTrailData::UpdateMetrics(gfx::DelegatedInkMetadata* metadata)
{
    for (const auto& [_, point] : points_) {
        metrics_handler_.AddRealEvent(point.point(), point.timestamp(), metadata->frame_time());
    }
}
