...
struct MaskFilterInfoExt {
    ...
    MaskFilterInfoExt(const gfx::MaskFilterInfo& mask_filter_info_arg, bool is_fast_rounded_corner_arg, const gfx::Transform target_transform)
    ...
    bool CanMergeMaskFilterInfo(const CompositorRenderPass& merge_render_pass, const gfx::Transform& parent_target_transform) const
    {
        ...
        for (const auto* sqs : merge_render_pass.shared_quad_state_list) {
            ...
            // filters.
            if (sqs->mask_filter_info.HasRoundedCorners() && !sqs->is_fast_rounded_corner) {
                return false;
            }
            ...
            rounded_corner_bounds = parent_target_transform.MapRect(rounded_corner_bounds);
            ...
            if (!mask_filter_info.rounded_corner_bounds().Contains(rounded_corner_bounds)) {
                return false;
            }
        }
        return true;
...
void MoveMatchingRequests(CompositorRenderPassId render_pass_id, std::multimap<CompositorRenderPassId, std::unique_ptr<CopyOutputRequest>>* copy_requests,
...
std::optional<gfx::Rect> CalculateClipRect(
    const std::optional<gfx::Rect> surface_clip, const std::optional<gfx::Rect> quad_clip, const gfx::Transform& target_transform)
{
    ...
    if (quad_clip) {
        ...
        gfx::Rect final_clip = cc::MathUtil::MapEnclosingClippedRect(target_transform, *quad_clip);
        if (out_clip)
...
SharedQuadState* CopyAndScaleSharedQuadState(const SharedQuadState* source_sqs, uint32_t client_namespace_id, const gfx::Transform& quad_to_target_transform,
    const gfx::Transform& target_transform, const gfx::Rect& quad_layer_rect, const gfx::Rect& visible_quad_layer_rect,
    const std::optional<gfx::Rect> added_clip_rect, const MaskFilterInfoExt& mask_filter_info_ext, AggregatedRenderPass* dest_render_pass)
{
    auto* shared_quad_state = dest_render_pass->CreateAndAppendSharedQuadState();
    auto new_clip_rect = CalculateClipRect(added_clip_rect, source_sqs->clip_rect, target_transform);
    ...
    shared_quad_state->SetAll(new_transform, quad_layer_rect, visible_quad_layer_rect, mask_filter_info_ext.mask_filter_info, new_clip_rect,
        source_sqs->are_contents_opaque, source_sqs->opacity, source_sqs->blend_mode, source_sqs->sorting_context_id, source_sqs->layer_id,
...
SharedQuadState* CopySharedQuadState(const SharedQuadState* source_sqs, uint32_t client_namespace_id, const gfx::Transform& target_transform,
    const std::optional<gfx::Rect> added_clip_rect, const MaskFilterInfoExt& mask_filter_info, AggregatedRenderPass* dest_render_pass)
{
    return CopyAndScaleSharedQuadState(source_sqs, client_namespace_id, source_sqs->quad_to_target_transform, target_transform, source_sqs->quad_layer_rect,
        source_sqs->visible_quad_layer_rect, added_clip_rect, mask_filter_info, dest_render_pass);
}

void UpdatePersistentPassDataMergeState(ResolvedPassData& resolved_pass, AggregatedRenderPass* dest_pass, bool is_merged_pass)
{
    auto& persistent_data = resolved_pass.current_persistent_data();

    PersistentPassData::MergeState merge_state = is_merged_pass ? PersistentPassData::kAlwaysMerged : PersistentPassData::kNotMerged;
...
{
    DCHECK(resolved_pass.current_persistent_data().merge_state != PersistentPassData::kInitState);
    // If this is the first frame and previous_merge_state is empty,
    // this function will returns false.
    auto current_merge_state = resolved_pass.current_persistent_data().merge_state;
    auto previous_merge_state = resolved_pass.previous_persistent_data().merge_state;

    // Check if this render pass is merged to its parent render pass in the
    // previous frame but is not in the current frame.
    bool change_in_merged_pass = previous_merge_state == PersistentPassData::kAlwaysMerged && current_merge_state == PersistentPassData::kNotMerged;
    ...
    change_in_merged_pass |= resolved_pass.current_persistent_data().merge_state == PersistentPassData::kSomeTimesMerged
        || resolved_pass.previous_persistent_data().merge_state == PersistentPassData::kSomeTimesMerged;

    return change_in_merged_pass;
}

void UpdateNeedsRedraw(ResolvedPassData& resolved_pass, AggregatedRenderPass* dest_pass, const std::optional<gfx::Rect> dest_root_target_clip_rect)
{
    ...
    auto& current_parent_clip_rect = resolved_pass.current_persistent_data().parent_clip_rect;
    current_parent_clip_rect.Union(dest_root_target_clip_rect.value());

    // Get the parent_clip_rect from the previous frame;
    auto& previous_parent_clip_rect = resolved_pass.previous_persistent_data().parent_clip_rect;

    // If the parent clip rect expands, the new area of the render pass output
    // buffer has never been updated. Redraw is needed.
    bool parent_clip_rect_expands = !previous_parent_clip_rect.Contains(current_parent_clip_rect);
    ...
    if (parent_clip_rect_expands || resolved_pass.aggregation().in_copy_request_pass || change_in_merged_pass) {
...
bool RenderPassNeedsFullDamage(ResolvedPassData& resolved_pass)
{
    ...
    const bool can_skip_render_pass = base::FeatureList::IsEnabled(features::kAllowUndamagedNonrootRenderPassToSkip);
    if (can_skip_render_pass) {
        ...
        return aggregation.in_cached_render_pass || aggregation.in_copy_request_pass || aggregation.in_pixel_moving_filter_pass
...
gfx::Rect ComputeDrawableRectForQuad(const DrawQuad* quad)
{
    ...
    gfx::Rect drawable_rect = cc::MathUtil::MapEnclosingClippedRect(sqs->quad_to_target_transform, quad->visible_rect);
    if (sqs->clip_rect)
...
// transform.
gfx::Rect TransformRectToDestRootTargetSpace(const gfx::Rect& rect_in_target_space, const gfx::Transform& target_to_dest_transform,
    const gfx::Transform& dest_to_root_target_transform, const std::optional<gfx::Rect> dest_root_target_clip_rect)
{
    gfx::Transform target_to_dest_root_target_transform = dest_to_root_target_transform * target_to_dest_transform;

    gfx::Rect rect_in_root_target_space = cc::MathUtil::MapEnclosingClippedRect(target_to_dest_root_target_transform, rect_in_target_space);
...
SurfaceAggregator::SurfaceAggregator(SurfaceManager* manager, DisplayResourceProvider* provider, bool needs_surface_damage_rect_list,
    ExtraPassForReadbackOption extra_pass_option, bool prevent_merging_surfaces_to_root_pass)
    : manager_(manager)
    , provider_(provider)
    , needs_surface_damage_rect_list_(needs_surface_damage_rect_list)
    , extra_pass_for_readback_option_(extra_pass_option)
    , prevent_merging_surfaces_to_root_pass_(prevent_merging_surfaces_to_root_pass)
{
    DCHECK(manager_);
    DCHECK(provider_);
...
void SurfaceAggregator::AddRenderPassFilterDamageToDamageList(const ResolvedFrameData& resolved_frame, const CompositorRenderPassDrawQuad* render_pass_quad,
    const gfx::Transform& parent_target_transform, const std::optional<gfx::Rect> dest_root_target_clip_rect,
    const gfx::Transform& dest_transform_to_root_target)
{
    ...
    const ResolvedPassData& child_resolved_pass = resolved_frame.GetRenderPassDataById(child_pass_id);
    const CompositorRenderPass& child_render_pass = child_resolved_pass.render_pass();

    // Add damages from render passes with pixel-moving foreground filters or
    // backdrop filters to the surface damage list.
    if (!child_render_pass.filters.HasFilterThatMovesPixels() && !child_render_pass.backdrop_filters.HasFilterThatMovesPixels()) {
        return;
    }
    ...
    if (child_render_pass.filters.HasFilterThatMovesPixels()) {
        ...
        damage_rect_in_target_space = GetExpandedRectWithPixelMovingForegroundFilter(*render_pass_quad, child_render_pass.filters);
        ...
        damage_rect_in_target_space = cc::MathUtil::MapEnclosingClippedRect(shared_quad_state->quad_to_target_transform, damage_rect);
        if (shared_quad_state->clip_rect) {
            damage_rect_in_target_space.Intersect(shared_quad_state->clip_rect.value());
        }
    }

    gfx::Rect damage_rect_in_root_target_space
        = TransformRectToDestRootTargetSpace(damage_rect_in_target_space, parent_target_transform, dest_transform_to_root_target, dest_root_target_clip_rect);
...
void SurfaceAggregator::AddSurfaceDamageToDamageList(const gfx::Rect& default_damage_rect, const gfx::Transform& parent_target_transform,
    const std::optional<gfx::Rect> dest_root_target_clip_rect, const gfx::Transform& dest_transform_to_root_target, ResolvedFrameData* resolved_frame)
{
    ...
    gfx::Rect damage_rect_in_root_target_space = TransformRectToDestRootTargetSpace(
        /*rect_in_target_space=*/damage_rect, parent_target_transform, dest_transform_to_root_target, dest_root_target_clip_rect);
...
const DrawQuad* SurfaceAggregator::FindQuadWithOverlayDamage(
    const CompositorRenderPass& source_pass, AggregatedRenderPass* dest_pass, const gfx::Transform& pass_to_root_target_transform, size_t* overlay_damage_index)
{
    ...
    if (target_quad->material == DrawQuad::Material::kCompositorRenderPass || target_quad->material == DrawQuad::Material::kSurfaceContent)
        return nullptr;
    ...
    if (!damage_rect_in_root_space.IsEmpty()) {
        gfx::Transform quad_to_root_transform = pass_to_root_target_transform * target_quad->shared_quad_state->quad_to_target_transform;
        if (!quad_to_root_transform.Preserves2dAxisAlignment()) {
            return nullptr;
        }

        gfx::RectF rect_in_root_space = cc::MathUtil::MapClippedRect(quad_to_root_transform, gfx::RectF(target_quad->rect));
        ...
        if (!gfx::ToEnclosingRectIgnoringError(rect_in_root_space).Contains(damage_rect_in_root_space)) {
            return nullptr;
        }
    }
...
bool SurfaceAggregator::CanPotentiallyMergePass(const SurfaceDrawQuad& surface_quad)
{
    const SharedQuadState* sqs = surface_quad.shared_quad_state;
    return surface_quad.allow_merge && base::IsApproximatelyEqual(sqs->opacity, 1.f, kOpacityEpsilon);
}
...
const ResolvedFrameData* SurfaceAggregator::GetLatestFrameData(const SurfaceId& surface_id)
{
    DCHECK(!is_inside_aggregate_);
    return GetResolvedFrame(surface_id);
}

ResolvedFrameData* SurfaceAggregator::GetResolvedFrame(const SurfaceRange& range)
{
...
ResolvedFrameData* SurfaceAggregator::GetResolvedFrame(const SurfaceId& surface_id)
{
    ...
    if (iter == resolved_frames_.end()) {
        ...
        if (surface->surface_id() != surface->previous_frame_surface_id()) {
            auto prev_resolved_frame_iter = resolved_frames_.find(surface->previous_frame_surface_id());
            if (prev_resolved_frame_iter != resolved_frames_.end()) {
                ...
                if (prev_resolved_frame.is_valid()) {
                    ...
                    prev_root_pass_id = prev_resolved_frame.GetRootRenderPassData().remapped_id();
                }
            }
        }

        iter = resolved_frames_
                   .emplace(std::piecewise_construct, std::forward_as_tuple(surface_id),
                       std::forward_as_tuple(provider_, surface, prev_frame_index, prev_root_pass_id))
                   .first;
    }
...
void SurfaceAggregator::HandleSurfaceQuad(const CompositorRenderPass& source_pass, const SurfaceDrawQuad* surface_quad, uint32_t embedder_client_namespace_id,
    float parent_device_scale_factor, const gfx::Transform& target_transform, const std::optional<gfx::Rect> added_clip_rect,
    const std::optional<gfx::Rect> dest_root_target_clip_rect, AggregatedRenderPass* dest_pass, const MaskFilterInfoExt& mask_filter_info)
{
    DCHECK(target_transform.Preserves2dAxisAlignment());

    SurfaceId primary_surface_id = surface_quad->surface_range.end();
    ResolvedFrameData* resolved_frame = GetResolvedFrame(surface_quad->surface_range);

    // |added_clip_rect| should be bounded by the output_rect of the render pass
    // that contains |surface_quad|.
    std::optional<gfx::Rect> surface_clip_rect = CalculateClipRect(added_clip_rect, source_pass.output_rect, target_transform);
    ...
    if (needs_surface_damage_rect_list_ && (!resolved_frame || resolved_frame->surface_id() != primary_surface_id)) {
        ...
        // root surface.
        AddSurfaceDamageToDamageList(surface_in_target_space, target_transform, dest_root_target_clip_rect, dest_pass->transform_to_root_target,
            /*resolved_frame=*/nullptr);
    }
    ...
    combined_clip_rect = TransformRectToDestRootTargetSpace(
        /*rect_in_target_space=*/surface_in_target_space, target_transform, dest_pass->transform_to_root_target, dest_root_target_clip_rect);
    ...
    if (!resolved_frame) {
        EmitDefaultBackgroundColorQuad(surface_quad, embedder_client_namespace_id, target_transform, surface_clip_rect, dest_pass, mask_filter_info);
        return;
    }

    if (resolved_frame->surface_id() != primary_surface_id && !surface_quad->stretch_content_to_fill_bounds) {
        gfx::Rect fallback_rect(resolved_frame->size_in_pixels());

        float scale_ratio = parent_device_scale_factor / resolved_frame->device_scale_factor();
        fallback_rect = gfx::ScaleToEnclosingRect(fallback_rect, scale_ratio, scale_ratio);
        fallback_rect = gfx::IntersectRects(fallback_rect, surface_quad->visible_rect);
        ...
        EmitGutterQuadsIfNecessary(surface_quad->visible_rect, fallback_rect, surface_quad->shared_quad_state, embedder_client_namespace_id, target_transform,
            surface_clip_rect, background_color, dest_pass, mask_filter_info);
    }

    EmitSurfaceContent(*resolved_frame, parent_device_scale_factor, surface_quad, embedder_client_namespace_id, target_transform, surface_clip_rect,
        combined_clip_rect, dest_pass, mask_filter_info);
}

void SurfaceAggregator::EmitSurfaceContent(ResolvedFrameData& resolved_frame, float parent_device_scale_factor, const SurfaceDrawQuad* surface_quad,
    uint32_t embedder_client_namespace_id, const gfx::Transform& target_transform, const std::optional<gfx::Rect> added_clip_rect,
    const std::optional<gfx::Rect> dest_root_target_clip_rect, AggregatedRenderPass* dest_pass, const MaskFilterInfoExt& mask_filter_info)
{
    ...
    if (surface_quad->stretch_content_to_fill_bounds) {
        ...
        extra_content_scale_x = surface_quad_rect.width() / static_cast<float>(resolved_frame.size_in_pixels().width());
        extra_content_scale_y = surface_quad_rect.height() / static_cast<float>(resolved_frame.size_in_pixels().height());
    } else {
        extra_content_scale_x = extra_content_scale_y = parent_device_scale_factor / resolved_frame.device_scale_factor();
    }
    ...
    gfx::Transform scaled_quad_to_target_transform(surface_quad_sqs->quad_to_target_transform);
    scaled_quad_to_target_transform.Scale(extra_content_scale_x, extra_content_scale_y);
    ...
    bool reflected_and_scaled = surface_quad->is_reflection && !scaled_quad_to_target_transform.IsIdentityOrTranslation();

    const bool pass_is_mergeable = CanPotentiallyMergePass(*surface_quad) && !reflected_and_scaled && combined_transform.Preserves2dAxisAlignment()
        && mask_filter_info.CanMergeMaskFilterInfo(resolved_frame.GetRootRenderPassData().render_pass(), combined_transform)
    ...
    const bool has_video_capture = !copy_requests.empty() || surface->IsVideoCaptureOnFromClient();
    ...
    // info.
    UpdatePersistentPassDataMergeState(resolved_frame.GetRootRenderPassData(), dest_pass, merge_pass);

    if (needs_surface_damage_rect_list_ && resolved_frame.WillDraw()) {
        AddSurfaceDamageToDamageList(
            /*default_damage_rect =*/gfx::Rect(), combined_transform, dest_root_target_clip_rect, dest_pass->transform_to_root_target, &resolved_frame);
    }

    if (frame_metadata.delegated_ink_metadata) {
        AggregatedRenderPassId render_pass_with_delegated_ink = merge_pass ? dest_pass->id : resolved_frame.GetRootRenderPassData().remapped_id();
        ...
        TransformAndStoreDelegatedInkMetadata(
            dest_pass->transform_to_root_target * combined_transform, frame_metadata.delegated_ink_metadata.get(), render_pass_with_delegated_ink);
    }
    ...
    size_t passes_to_copy = merge_pass ? num_render_passes - 1 : num_render_passes;
    for (size_t j = 0; j < passes_to_copy; ++j) {
        ...
        if (max_render_target_size_ > 0) {
            output_rect.set_width(std::min(output_rect.width(), max_render_target_size_));
            output_rect.set_height(std::min(output_rect.height(), max_render_target_size_));
        }
        copy_pass->SetAll(resolved_pass.remapped_id(), output_rect, output_rect, source.transform_to_root_target, source.filters, source.backdrop_filters,
            source.backdrop_filter_bounds, root_content_color_usage_, source.has_transparent_background, source.cache_render_pass,
        ...
        copy_pass->transform_to_root_target.PostConcat(dest_pass->transform_to_root_target);

        CopyQuadsToPass(resolved_frame, resolved_pass, copy_pass.get(), resolved_frame.device_scale_factor(), gfx::Transform(), {}, dest_root_target_clip_rect,
            MaskFilterInfoExt());
    ...
    const bool allow_forced_merge_pass = base::FeatureList::IsEnabled(features::kAllowForceMergeRenderPassWithRequireOverlayQuads);
    const bool force_merge_pass = allow_forced_merge_pass && !merge_pass && pass_is_mergeable
        && base::ranges::any_of(dest_pass_list_->back()->quad_list, &OverlayCandidate::RequiresOverlay);
    ...
    if (merge_pass || force_merge_pass) {
        ...
        std::optional<gfx::Rect> surface_quad_clip = CalculateClipRect(added_clip_rect, ComputeDrawableRectForQuad(surface_quad), target_transform);

        // UpdatePersistentPassDataMergeState() has been called earlier.
        CopyQuadsToPass(resolved_frame, resolved_root_pass, dest_pass, resolved_frame.device_scale_factor(), combined_transform, surface_quad_clip,
            dest_root_target_clip_rect, mask_filter_info);
    } else {
        auto* shared_quad_state = CopyAndScaleSharedQuadState(surface_quad_sqs, embedder_client_namespace_id, scaled_quad_to_target_transform, target_transform,
            gfx::ScaleToEnclosingRect(surface_quad_sqs->quad_layer_rect, inverse_extra_content_scale_x, inverse_extra_content_scale_y),
            gfx::ScaleToEnclosingRect(surface_quad_sqs->visible_quad_layer_rect, inverse_extra_content_scale_x, inverse_extra_content_scale_y), added_clip_rect,
            mask_filter_info, dest_pass);
        ...
        //   space.
        gfx::Rect quad_visible_rect(gfx::ScaleToEnclosingRect(surface_quad->visible_rect, inverse_extra_content_scale_x, inverse_extra_content_scale_y));
        ...
        if (quad_visible_rect.IsEmpty()) {
            std::erase_if(*dest_pass_list_, [&remapped_pass_id](const std::unique_ptr<AggregatedRenderPass>& pass) { return pass->id == remapped_pass_id; });
        } else {
            auto* quad = dest_pass->CreateAndAppendDrawQuad<AggregatedRenderPassDrawQuad>();
            quad->SetNew(shared_quad_state, quad_rect, quad_visible_rect, remapped_pass_id, kInvalidResourceId, gfx::RectF(), gfx::Size(),
                gfx::Vector2dF(1.0f, 1.0f), gfx::PointF(), tex_coord_rect,
...
void SurfaceAggregator::EmitDefaultBackgroundColorQuad(const SurfaceDrawQuad* surface_quad, uint32_t embedder_client_namespace_id,
    const gfx::Transform& target_transform, const std::optional<gfx::Rect> clip_rect, AggregatedRenderPass* dest_pass,
    const MaskFilterInfoExt& mask_filter_info)
{
    TRACE_EVENT1("viz", "SurfaceAggregator::EmitDefaultBackgroundColorQuad", "surface_range", surface_quad->surface_range.ToString());
    ...
    auto* shared_quad_state
        = CopySharedQuadState(surface_quad->shared_quad_state, embedder_client_namespace_id, target_transform, clip_rect, mask_filter_info, dest_pass);

    auto* solid_color_quad = dest_pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
    solid_color_quad->SetNew(shared_quad_state, surface_quad->rect, surface_quad->visible_rect, background_color, false);
}

void SurfaceAggregator::EmitGutterQuadsIfNecessary(const gfx::Rect& primary_rect, const gfx::Rect& fallback_rect,
    const SharedQuadState* primary_shared_quad_state, uint32_t embedder_client_namespace_id, const gfx::Transform& target_transform,
    const std::optional<gfx::Rect> clip_rect, SkColor4f background_color, AggregatedRenderPass* dest_pass, const MaskFilterInfoExt& mask_filter_info)
{
    ...
    if (fallback_rect.width() < primary_rect.width()) {
        ...
        gfx::Rect right_gutter_rect(fallback_rect.right(), primary_rect.y(), primary_rect.width() - fallback_rect.width(), primary_rect.height());

        SharedQuadState* shared_quad_state
            = CopyAndScaleSharedQuadState(primary_shared_quad_state, embedder_client_namespace_id, primary_shared_quad_state->quad_to_target_transform,
                target_transform, right_gutter_rect, right_gutter_rect, clip_rect, mask_filter_info, dest_pass);

        auto* right_gutter = dest_pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
        right_gutter->SetNew(shared_quad_state, right_gutter_rect, right_gutter_rect, background_color, false);
    }

    if (fallback_rect.height() < primary_rect.height()) {
        gfx::Rect bottom_gutter_rect(primary_rect.x(), fallback_rect.bottom(), fallback_rect.width(), primary_rect.height() - fallback_rect.height());

        SharedQuadState* shared_quad_state
            = CopyAndScaleSharedQuadState(primary_shared_quad_state, embedder_client_namespace_id, primary_shared_quad_state->quad_to_target_transform,
                target_transform, bottom_gutter_rect, bottom_gutter_rect, clip_rect, mask_filter_info, dest_pass);

        auto* bottom_gutter = dest_pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
        bottom_gutter->SetNew(shared_quad_state, bottom_gutter_rect, bottom_gutter_rect, background_color, false);
    }
}
...
void SurfaceAggregator::AddColorConversionPass()
{
    ...
    bool needs_color_conversion_pass
        = !display_color_spaces_.GetOutputColorSpace(root_render_pass->content_color_usage, root_render_pass->has_transparent_background)
    ...
    if (!color_conversion_render_pass_id_) {
        color_conversion_render_pass_id_ = render_pass_id_generator_.GenerateNextId();
    }

    AddRenderPassHelper(color_conversion_render_pass_id_, output_rect, root_render_pass->damage_rect, root_content_color_usage_,
        ...
        /*quad_state_contents_opaque=*/false, SkBlendMode::kSrc, root_render_pass->id);
}
...
void SurfaceAggregator::AddRootReadbackPass()
{
    ...
    for (auto* quad : root_render_pass->quad_list) {
        if (auto* render_pass_quad = quad->DynamicCast<AggregatedRenderPassDrawQuad>()) {
    ...
    if (extra_pass_for_readback_option_ == ExtraPassForReadbackOption::kAlwaysAddPass) {
        needs_readback_pass = true;
    }
    ...
    bool has_transparent_background = root_render_pass->has_transparent_background;
    root_render_pass->has_transparent_background = true;
    AddRenderPassHelper(readback_render_pass_id_, output_rect, root_render_pass->damage_rect, root_content_color_usage_, has_transparent_background,
        /*pass_is_color_conversion_pass=*/false,
        /*quad_state_to_target_transform=*/gfx::Transform(),
        /*quad_state_contents_opaque=*/false, SkBlendMode::kSrcOver, root_render_pass->id);
}
...
void SurfaceAggregator::AddDisplayTransformPass()
{
    ...
    if (!display_transform_render_pass_id_) {
        display_transform_render_pass_id_ = render_pass_id_generator_.GenerateNextId();
    }
    ...
    AddRenderPassHelper(display_transform_render_pass_id_,
        cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(root_surface_transform_, root_render_pass->output_rect),
        cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(root_surface_transform_, root_render_pass->damage_rect), root_render_pass->content_color_usage,
        root_render_pass->has_transparent_background,
        /*pass_is_color_conversion_pass=*/false, root_surface_transform_, are_contents_opaque, SkBlendMode::kSrcOver, root_render_pass->id);
}

void SurfaceAggregator::AddRenderPassHelper(AggregatedRenderPassId render_pass_id, const gfx::Rect& render_pass_output_rect,
    const gfx::Rect& render_pass_damage_rect, gfx::ContentColorUsage pass_color_usage, bool pass_has_transparent_background, bool pass_is_color_conversion_pass,
    const gfx::Transform& quad_state_to_target_transform, bool quad_state_contents_opaque, SkBlendMode quad_state_blend_mode,
    AggregatedRenderPassId quad_pass_id)
{
    ...
    render_pass->SetAll(render_pass_id, render_pass_output_rect, render_pass_damage_rect, gfx::Transform(),
        /*filters=*/cc::FilterOperations(),
        /*backdrop_filters=*/cc::FilterOperations(),
        /*backdrop_filter_bounds=*/gfx::RRectF(), pass_color_usage, pass_has_transparent_background,
    ...
    shared_quad_state->SetAll(quad_state_to_target_transform,
        /*layer_rect=*/current_output_rect,
        /*visible_layer_rect=*/current_output_rect, gfx::MaskFilterInfo(),
        /*clip=*/std::nullopt, quad_state_contents_opaque, /*opacity_f=*/1.f, quad_state_blend_mode, /*sorting_context=*/0, /*layer_id*/ 0u,
        /*fast_rounded_corner=*/false);

    auto* quad = render_pass->CreateAndAppendDrawQuad<AggregatedRenderPassDrawQuad>();
    quad->SetNew(shared_quad_state, current_output_rect, current_output_rect, quad_pass_id, kInvalidResourceId, gfx::RectF(), gfx::Size(),
        gfx::Vector2dF(1.0f, 1.0f), gfx::PointF(), gfx::RectF(current_output_rect),
        /*force_anti_aliasing_off=*/false,
        /*backdrop_filter_quality*/ 1.0f);
    dest_pass_list_->push_back(std::move(render_pass));
}

void SurfaceAggregator::CopyQuadsToPass(ResolvedFrameData& resolved_frame, ResolvedPassData& resolved_pass, AggregatedRenderPass* dest_pass,
    float parent_device_scale_factor, const gfx::Transform& target_transform, const std::optional<gfx::Rect> clip_rect,
    const std::optional<gfx::Rect> dest_root_target_clip_rect, const MaskFilterInfoExt& parent_mask_filter_info_ext)
{
...
#if DCHECK_IS_ON()
    const SharedQuadStateList& source_shared_quad_state_list = source_pass.shared_quad_state_list;
    ...
    for (auto* quad : source_quad_list) {
        while (sqs_iter != source_shared_quad_state_list.cend() && quad->shared_quad_state != *sqs_iter) {
            ++sqs_iter;
        }
    ...
    const gfx::Transform pass_to_dest_root_target_transform = dest_pass->transform_to_root_target * target_transform;
    ...
    if (needs_surface_damage_rect_list_ && resolved_pass.aggregation().will_draw && resolved_pass.is_root()) {
        ...
        quad_with_overlay_damage_index = FindQuadWithOverlayDamage(source_pass, dest_pass, pass_to_dest_root_target_transform, &overlay_damage_index);
    }

    // Add render pass |output_rect| to |dest_root_target_clip_rect|.
    auto new_dest_root_target_clip_rect
        = CalculateClipRect(dest_root_target_clip_rect, resolved_pass.render_pass().output_rect, pass_to_dest_root_target_transform);
    ...
    for (auto* quad : source_quad_list) {
        ...
        DCHECK(!quad->shared_quad_state->mask_filter_info.HasGradientMask() || !parent_mask_filter_info_ext.mask_filter_info.HasGradientMask());
        ...
        if (!quad->shared_quad_state->mask_filter_info.IsEmpty()) {
            new_mask_filter_info_ext
                = MaskFilterInfoExt(quad->shared_quad_state->mask_filter_info, quad->shared_quad_state->is_fast_rounded_corner, target_transform);
        }

        if (quad->material == DrawQuad::Material::kSharedElement) {
            // SharedElement quads should've been resolved before aggregation.
            continue;
        } else if (const auto* surface_quad = quad->DynamicCast<SurfaceDrawQuad>()) {
            ...
            HandleSurfaceQuad(source_pass, surface_quad, client_namespace_id, parent_device_scale_factor, target_transform, clip_rect,
                new_dest_root_target_clip_rect, dest_pass, new_mask_filter_info_ext);
        } else {
            ...
            bool needs_sqs = quad->shared_quad_state != last_copied_source_shared_quad_state;
            bool has_per_quad_damage
                = source_pass.has_per_quad_damage && GetOptionalDamageRectFromQuad(quad).has_value() && resolved_pass.aggregation().will_draw;

            if (needs_sqs || has_per_quad_damage) {
                SharedQuadState* dest_shared_quad_state
                    = CopySharedQuadState(quad->shared_quad_state, client_namespace_id, target_transform, clip_rect, new_mask_filter_info_ext, dest_pass);

                if (has_per_quad_damage) {
                    auto damage_rect_in_target_space = GetOptionalDamageRectFromQuad(quad);
                    dest_shared_quad_state->overlay_damage_index = surface_damage_rect_list_->size();
                    AddSurfaceDamageToDamageList(damage_rect_in_target_space.value(), target_transform, new_dest_root_target_clip_rect,
            ...
            if (const auto* pass_quad = quad->DynamicCast<CompositorRenderPassDrawQuad>()) {
                CompositorRenderPassId original_pass_id = pass_quad->render_pass_id;
                AggregatedRenderPassId remapped_pass_id = resolved_frame.GetRenderPassDataById(original_pass_id).remapped_id();

                dest_quad = dest_pass->CopyFromAndAppendRenderPassDrawQuad(pass_quad, remapped_pass_id);

                if (needs_surface_damage_rect_list_ && resolved_pass.aggregation().will_draw) {
                    AddRenderPassFilterDamageToDamageList(
                        resolved_frame, pass_quad, target_transform, new_dest_root_target_clip_rect, dest_pass->transform_to_root_target);
                }
            } else if (const auto* texture_quad = quad->DynamicCast<TextureDrawQuad>()) {
                if (texture_quad->secure_output_only && (!output_is_secure_ || resolved_pass.aggregation().in_copy_request_pass)) {
                    // If TextureDrawQuad requires secure output and the output is not
                    // secure then replace it with solid black.
                    auto* solid_color_quad = dest_pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();
                    solid_color_quad->SetNew(dest_pass->shared_quad_state_list.back(), quad->rect, quad->visible_rect, SkColors::kBlack, false);
                } else {
...
void SurfaceAggregator::CopyPasses(ResolvedFrameData& resolved_frame)
{
    ...
    const gfx::Transform surface_transform = IsRootSurface(surface) ? root_surface_transform_ : gfx::Transform();

    auto& root_resolved_pass = resolved_frame.GetRootRenderPassData();
    gfx::Rect root_output_rect = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(surface_transform, root_resolved_pass.render_pass().output_rect);
    ...
    if (frame_metadata.delegated_ink_metadata) {
        ...
        // prevents the delegated ink trail from flickering if a compositor frame
        // is not generated due to a delayed main frame.
        TransformAndStoreDelegatedInkMetadata(root_resolved_pass.render_pass().transform_to_root_target * surface_transform,
            frame_metadata.delegated_ink_metadata.get(), resolved_frame.GetRootRenderPassData().remapped_id());
    }
    ...
    for (auto& resolved_pass : resolved_frame.GetResolvedPasses()) {
        ...
        apply_surface_transform_to_root_pass = resolved_pass.is_root() && (copy_pass->copy_requests.empty() || surface_transform.IsIdentity());
        ...
        if (apply_surface_transform_to_root_pass) {
            ...
            transform_to_root_target = surface_transform * source.transform_to_root_target;
        }

        copy_pass->SetAll(resolved_pass.remapped_id(), output_rect, output_rect, transform_to_root_target, source.filters, source.backdrop_filters,
            source.backdrop_filter_bounds, root_content_color_usage_, source.has_transparent_background, source.cache_render_pass,
            resolved_pass.aggregation().has_damage, source.generate_mipmap);
        ...
        if (needs_surface_damage_rect_list_ && resolved_pass.is_root()) {
            AddSurfaceDamageToDamageList(
                ...
                /*dest_root_target_clip_rect=*/ {}, copy_pass->transform_to_root_target, &resolved_frame);
        }

        CopyQuadsToPass(resolved_frame, resolved_pass, copy_pass.get(), resolved_frame.device_scale_factor(),
            apply_surface_transform_to_root_pass ? surface_transform : gfx::Transform(), {}, /*dest_root_target_clip_rect*/ root_output_rect,
            MaskFilterInfoExt());
    ...
    dest_pass_list_->back()->video_capture_enabled = surface->IsVideoCaptureOnFromClient();

    if (!apply_surface_transform_to_root_pass)
        AddDisplayTransformPass();
}

void SurfaceAggregator::SetRenderPassDamageRect(AggregatedRenderPass* copy_pass, ResolvedPassData& resolved_pass)
{
    ...
    if (!RenderPassNeedsFullDamage(resolved_pass)) {
        ...
        if (copy_pass->transform_to_root_target.GetInverse(&inverse_transform)) {
            gfx::Rect damage_rect_in_render_pass_space = cc::MathUtil::ProjectEnclosingClippedRect(inverse_transform, root_damage_rect_);
            ...
            if (metrics_subsampler_.ShouldSample(0.001)) {
                gfx::Rect root_clip_in_render_pass_space
                    = cc::MathUtil::ProjectEnclosingClippedRect(inverse_transform, resolved_pass.previous_persistent_data().parent_clip_rect);
                ...
                bool is_output_rect = root_clip_in_render_pass_space == copy_pass->output_rect;

                UMA_HISTOGRAM_ENUMERATION(
                    "Compositing.SurfaceAggregator.RenderPassDamageType", is_output_rect ? RenderPassDamage::kOutputRect : RenderPassDamage::kRootClipped);

                const auto render_pass_overdamage = copy_pass->output_rect.size().Area64() - root_clip_in_render_pass_space.size().Area64();

                UMA_HISTOGRAM_COUNTS_10M("Compositing.SurfaceAggregator.ExcessPixelsClipped", render_pass_overdamage);
            }
        }
        ...
        static const bool can_skip_render_pass = base::FeatureList::IsEnabled(features::kAllowUndamagedNonrootRenderPassToSkip);
        ...
        UMA_HISTOGRAM_ENUMERATION("Compositing.SurfaceAggregator.RenderPassDamageType", RenderPassDamage::kForceFullOutputRect);
    }
}
...
void SurfaceAggregator::ProcessAddedAndRemovedSurfaces()
{
    ...
    std::erase_if(resolved_frames_, [](auto& entry) { return !entry.second.WasUsedInAggregation(); });
}

gfx::Rect SurfaceAggregator::PrewalkRenderPass(ResolvedFrameData& resolved_frame, ResolvedPassData& resolved_pass, const gfx::Rect& damage_from_parent,
    const gfx::Transform& target_to_root_transform, const ResolvedPassData* parent_pass, PrewalkResult& result)
{
    ...
    if (render_pass.cache_render_pass || (parent_pass && parent_pass->aggregation().in_cached_render_pass)) {
        resolved_pass.aggregation().in_cached_render_pass = true;
    }

    if (render_pass.filters.HasFilterThatMovesPixels() || (parent_pass && parent_pass->aggregation().in_pixel_moving_filter_pass)) {
    ...
    } else if (damage_type == FrameDamageType::kFrame && render_pass.has_damage_from_contributing_content) {
        resolved_pass.aggregation().has_damage = true;
    }
    ...
    if (!surface_root_rp_damage.IsEmpty()) {
        ...
        if (target_to_root_transform.GetInverse(&root_to_target_transform)) {
            surface_root_rp_damage = cc::MathUtil::ProjectEnclosingClippedRect(root_to_target_transform, surface_root_rp_damage);
        }
    }
    ...
    for (const DrawQuad* quad : base::Reversed(render_pass.quad_list)) {
        ...
        if (quad->material == DrawQuad::Material::kSurfaceContent) {
            ...
            ResolvedFrameData* child_resolved_frame = GetResolvedFrame(surface_quad->surface_range);
            ...
            // gutter.
            if (!child_resolved_frame || child_resolved_frame->surface_id() != surface_quad->surface_range.end()) {
            ...
            if (child_resolved_frame) {
                ...
                if (surface_quad->stretch_content_to_fill_bounds) {
                    ...
                    if (!child_size.IsEmpty()) {
                        x_scale = static_cast<float>(surface_quad->rect.width()) / child_size.width();
                        y_scale = static_cast<float>(surface_quad->rect.height()) / child_size.height();
                    }
                } else {
                    ...
                    x_scale = y_scale = resolved_frame.device_scale_factor() / child_resolved_frame->device_scale_factor();
                }
                ...
                if (CanPotentiallyMergePass(*surface_quad)) {
                    ...
                    if (!accumulated_damage_in_child_space.IsEmpty()) {
                        gfx::Transform inverse = quad->shared_quad_state->quad_to_target_transform.GetCheckedInverse();
                        inverse.PostScale(SK_Scalar1 / x_scale, SK_Scalar1 / y_scale);
                        accumulated_damage_in_child_space = cc::MathUtil::ProjectEnclosingClippedRect(inverse, accumulated_damage_in_child_space);
                    }
                }
                gfx::Rect child_rect = PrewalkSurface(*child_resolved_frame, &resolved_pass, accumulated_damage_in_child_space, result);
            ...
            if (parent_pass == nullptr && resolved_pass.is_root() && !result.page_fullscreen_mode) {
                ...
                if (surface_quad_on_target_space.ApproximatelyEqual(render_pass.output_rect, /*tolerance=*/8)) {
            ...
            if (prevent_merging_surfaces_to_root_pass_ && child_resolved_frame && resolved_pass.is_root() && IsRootSurface(resolved_frame.surface())) {
                child_resolved_frame->GetRootRenderPassData().aggregation().prevent_merge = true;
            }
#endif
        } else if (auto* render_pass_quad = quad->DynamicCast<CompositorRenderPassDrawQuad>()) {
            CompositorRenderPassId child_pass_id = render_pass_quad->render_pass_id;

            ResolvedPassData& child_resolved_pass = resolved_frame.GetRenderPassDataById(child_pass_id);
            const CompositorRenderPass& child_render_pass = child_resolved_pass.render_pass();

            gfx::Rect rect_in_target_space = cc::MathUtil::MapEnclosingClippedRect(quad->shared_quad_state->quad_to_target_transform, quad->rect);
            ...
            bool intersects_current_damage = rect_in_target_space.Intersects(damage_rect);
            bool intersects_damage_from_parent = rect_in_target_space.Intersects(damage_from_parent);
            ...
            // itself.
            bool intersects_damage_from_surface = rect_in_target_space.Intersects(surface_root_rp_damage);
            if (intersects_current_damage || intersects_damage_from_parent || intersects_damage_from_surface) {
            ...
            if (child_render_pass.filters.HasFilterThatMovesPixels()) {
                gfx::Rect expanded_rect_in_target_space = GetExpandedRectWithPixelMovingForegroundFilter(*render_pass_quad, child_render_pass.filters);

                if (expanded_rect_in_target_space.Intersects(damage_rect) || expanded_rect_in_target_space.Intersects(damage_from_parent)
            ...
            const gfx::Transform child_to_root_transform = target_to_root_transform * quad->shared_quad_state->quad_to_target_transform;
            quad_damage_rect = PrewalkRenderPass(resolved_frame, child_resolved_pass, gfx::Rect(), child_to_root_transform, &resolved_pass, result);
        ...
        if (!quad_damage_rect.IsEmpty()) {
            ...
            quad_target_space_damage_rect
                = cc::MathUtil::MapEnclosingClippedRectIgnoringError(quad->shared_quad_state->quad_to_target_transform, quad_damage_rect, kEpsilon);
        }

        if (!quad_target_space_damage_rect.IsEmpty()) {
            if (quad->shared_quad_state->clip_rect) {
                quad_target_space_damage_rect.Intersect(*quad->shared_quad_state->clip_rect);
            }
...
bool SurfaceAggregator::CheckFrameSinksChanged(const SurfaceId& surface_id)
{
    ...
    LocalSurfaceId& local_surface_id = contained_frame_sinks_[surface_id.frame_sink_id()];
    bool frame_sinks_changed = (!previous_contained_frame_sinks_.contains(surface_id.frame_sink_id()));
...
gfx::Rect SurfaceAggregator::PrewalkSurface(
    ResolvedFrameData& resolved_frame, ResolvedPassData* parent_pass, const gfx::Rect& damage_from_parent, PrewalkResult& result)
{
    Surface* surface = resolved_frame.surface();
    ...
    result.frame_sinks_changed |= CheckFrameSinksChanged(resolved_frame.surface_id());
    ...
    for (auto& resolved_pass : resolved_frame.GetResolvedPasses()) {
        ...
        if (resolved_pass.IsUnembedded()) {
            ...
            resolved_pass.aggregation().added_damage = PrewalkRenderPass(resolved_frame, resolved_pass,
    ...
    damage_rect.Union(PrewalkRenderPass(resolved_frame, root_resolved_pass, damage_from_parent, gfx::Transform(), parent_pass, result));
    ...
    if (!damage_rect.IsEmpty()) {
        ...
        if (IsRootSurface(surface)) {
            ...
            // transform.
            damage_rect = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(root_surface_transform_, damage_rect);
            gfx::Transform inverse = root_surface_transform_.GetCheckedInverse();
            damage_rect_surface_space = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(inverse, damage_rect);
        }
        ...
        surface->NotifyAggregatedDamage(damage_rect_surface_space, expected_display_time_);
    }
    ...
    for (const SurfaceRange& surface_range : frame_metadata.referenced_surfaces) {
        damage_ranges_[surface_range.end().frame_sink_id()].push_back(surface_range);
        if (surface_range.HasDifferentFrameSinkIds()) {
            damage_ranges_[surface_range.start()->frame_sink_id()].push_back(surface_range);
        }
    }
    ...
    for (const SurfaceId& surface_id : surface->active_referenced_surfaces()) {
        ...
        if (!contained_surfaces_.count(surface_id)) {
            ...
            if (undrawn_surface) {
                PrewalkSurface(*undrawn_surface, /*parent_pass=*/nullptr, gfx::Rect(), result);
            }
        }
    }
    ...
    result.content_color_usage = std::max(result.content_color_usage, frame_metadata.content_color_usage);
...
void SurfaceAggregator::CopyUndrawnSurfaces(PrewalkResult* prewalk_result)
{
    ...
    std::vector<SurfaceId> surfaces_to_copy(prewalk_result->undrawn_surfaces.begin(), prewalk_result->undrawn_surfaces.end());
...
void SurfaceAggregator::MarkAndPropagateCopyRequestPasses(ResolvedPassData& resolved_pass)
{
    if (resolved_pass.aggregation().in_copy_request_pass)
        return;

    resolved_pass.aggregation().in_copy_request_pass = true;
    for (ResolvedPassData* child_pass : resolved_pass.aggregation().embedded_passes) {
        MarkAndPropagateCopyRequestPasses(*child_pass);
    }
}

AggregatedFrame SurfaceAggregator::Aggregate(const SurfaceId& surface_id, base::TimeTicks expected_display_time, gfx::OverlayTransform display_transform,
    const gfx::Rect& target_damage, int64_t display_trace_id)
{
    ...
    TRACE_EVENT_BEGIN("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(display_trace_id_), [this](perfetto::EventContext ctx) {
        ...
        data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_SURFACE_AGGREGATION);
    ...
    absl::Cleanup surface_aggregation_trace_event_scoped_exit = [this] {
        TRACE_EVENT_END("viz,benchmark,graphics.pipeline", [this](perfetto::EventContext ctx) {
            auto* chrome_graphics_pipeline = ctx.event<perfetto::protos::pbzero::ChromeTrackEvent>()->set_chrome_graphics_pipeline();
            ...
            for (int64_t id : flow_ids_for_resolved_frames_) {
                chrome_graphics_pipeline->add_aggregated_surface_frame_trace_ids(id);
            }
            for (int64_t id : flow_ids_for_resolved_frames_) {
    ...
    auto& root_render_pass = resolved_frame->GetRootRenderPassData().render_pass();
    ...
    root_surface_transform_ = gfx::OverlayTransformToTransform(display_transform, gfx::SizeF(viewport_bounds));
    ...
    gfx::Rect prewalk_damage_rect = PrewalkSurface(*resolved_frame,
        /*parent_pass=*/nullptr,
    ...
    bool color_usage_changed = root_content_color_usage_ != prewalk_result.content_color_usage;
    if (color_usage_changed) {
        root_damage_rect_ = cc::MathUtil::MapEnclosedRectWith2dAxisAlignedTransform(root_surface_transform_, gfx::Rect(viewport_bounds));
    ...
    if (!color_usage_changed && !last_frame_had_delegated_ink_ && !RenderPassNeedsFullDamage(resolved_frame->GetRootRenderPassData())) {
    ...
    for (auto& contained_surface_id : previous_contained_surfaces_) {
        ...
        if (surface) {
            surface->allocation_group()->TakeAggregatedLatencyInfoUpTo(surface, &frame.latency_info);
        }
        if (!ui::LatencyInfo::Verify(frame.latency_info, "SurfaceAggregator::Aggregate")) {
            break;
        }
    }
...
{
    UMA_HISTOGRAM_COUNTS_100("Compositing.SurfaceAggregator.PrewalkedSurfaceCount", stats_->prewalked_surface_count);
    UMA_HISTOGRAM_COUNTS_100("Compositing.SurfaceAggregator.CopiedSurfaceCount", stats_->copied_surface_count);

    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
        "Compositing.SurfaceAggregator.PrewalkUs", stats_->prewalk_time, kHistogramMinTime, kHistogramMaxTime, kHistogramTimeBuckets);
    UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
        "Compositing.SurfaceAggregator.CopyUs", stats_->copy_time, kHistogramMinTime, kHistogramMaxTime, kHistogramTimeBuckets);

    UMA_HISTOGRAM_BOOLEAN("Compositing.SurfaceAggregator.HasCopyRequestsPerFrame", has_copy_requests_);
    UMA_HISTOGRAM_BOOLEAN("Compositing.SurfaceAggregator.HasPixelMovingFiltersPerFrame", stats_->has_pixel_moving_filter);
    UMA_HISTOGRAM_BOOLEAN("Compositing.SurfaceAggregator.HasPixelMovingBackdropFiltersPerFrame", has_pixel_moving_backdrop_filter_);
    UMA_HISTOGRAM_BOOLEAN("Compositing.SurfaceAggregator.HasUnembeddedRenderPassesPerFrame", stats_->has_unembedded_pass);
...
void SurfaceAggregator::SetDisplayColorSpaces(const gfx::DisplayColorSpaces& display_color_spaces)
{
...
bool SurfaceAggregator::ForceReleaseResourcesIfNeeded(const SurfaceId& surface_id)
{
    auto iter = resolved_frames_.find(surface_id);
    if (iter != resolved_frames_.end()) {
...
void SurfaceAggregator::SetFrameAnnotator(std::unique_ptr<FrameAnnotator> frame_annotator)
{
...
void SurfaceAggregator::TransformAndStoreDelegatedInkMetadata(const gfx::Transform& parent_quad_to_root_target_transform,
    const gfx::DelegatedInkMetadata* metadata, const AggregatedRenderPassId render_pass_with_delegated_ink)
{
    if (delegated_ink_metadata_) {
    ...
    gfx::PointF point = parent_quad_to_root_target_transform.MapPoint(metadata->point());
    gfx::RectF area = parent_quad_to_root_target_transform.MapRect(metadata->presentation_area());
    delegated_ink_metadata_ = std::make_unique<gfx::DelegatedInkMetadata>(point, metadata->diameter(), metadata->color(), metadata->timestamp(), area,
        metadata->frame_time(), metadata->is_hovering(), render_pass_with_delegated_ink.GetUnsafeValue());

    TRACE_EVENT_INSTANT2("viz", "SurfaceAggregator::TransformAndStoreDelegatedInkMetadata", TRACE_EVENT_SCOPE_THREAD, "original metadata", metadata->ToString(),
        "transformed metadata", delegated_ink_metadata_->ToString());
}

void SurfaceAggregator::DebugLogSurface(const Surface* surface, bool will_draw)
{
    DBG_LOG("aggregator.surface.log", "D%d - %s, %s draws=%s", static_cast<int>(referenced_surfaces_.size()), surface->surface_id().ToString().c_str(),
        surface->size_in_pixels().ToString().c_str(), will_draw ? "true" : "false");
}
