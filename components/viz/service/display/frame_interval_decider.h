...
public:
    ...
    private:
        ...
        ScopedAggregate(FrameIntervalDecider& decider, SurfaceManager& surface_manager, base::TimeTicks frame_time);
    ...
    void UpdateSettings(Settings settings, std::vector<std::unique_ptr<FrameIntervalMatcher>> matchers);
    std::unique_ptr<ScopedAggregate> WrapAggregate(SurfaceManager& surface_manager, base::TimeTicks frame_time);

private:
    void Decide(base::TimeTicks frame_time, base::flat_map<FrameSinkId, FrameIntervalInputs> inputs_map);

    // It's not possible to compare `FrameIntervalClass` and frame interval
    // directly, so assume true if result changes type.
    static bool MayDecreaseFrameInterval(const std::optional<Result>& from, const std::optional<Result>& to);
