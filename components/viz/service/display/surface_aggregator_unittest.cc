...
public:
    ...
    {
        return base::BindRepeating(&MockAggregatedDamageCallback::OnAggregatedDamage, weak_ptr_factory_.GetWeakPtr());
    }

    MOCK_METHOD4(OnAggregatedDamage,
        void(const LocalSurfaceId& local_surface_id, const gfx::Size& frame_size_in_pixels, const gfx::Rect& damage_rect,
...
public:
    base::TimeTicks next_display_time() const
    {
...
public:
    explicit SurfaceAggregatorTest(SurfaceAggregator::ExtraPassForReadbackOption extra_pass_option, bool prevent_merging_surfaces_to_root_pass)
        : root_sink_(std::make_unique<CompositorFrameSinkSupport>(&fake_client_, &manager_, kArbitraryRootFrameSinkId,
            /*is_root=*/true))
        , aggregator_(manager_.surface_manager(), &resource_provider_, true, extra_pass_option, prevent_merging_surfaces_to_root_pass)
    {
    }

    SurfaceAggregatorTest()
        : SurfaceAggregatorTest(SurfaceAggregator::ExtraPassForReadbackOption::kNone,
    ...
    AggregatedFrame AggregateFrame(const SurfaceId& surface_id, gfx::Rect target_damage = gfx::Rect())
    {
        AggregatedFrame aggregated_frame = aggregator_.Aggregate(surface_id, GetNextDisplayTimeAndIncrement(),
    ...
    struct Quad {
        ...
        static Quad TransparentSolidColorQuad(SkColor4f color, const gfx::Rect& rect, float opacity)
        {
            Quad quad;
            quad.material = DrawQuad::Material::kSolidColor;
            quad.color = color;
        ...
        static Quad TextureQuad(const gfx::Rect& rect, bool per_quad_damage_output = false)
        {
            Quad quad;
        ...
        static Quad SurfaceQuad(const SurfaceRange& surface_range, SkColor4f default_background_color, const gfx::Rect& primary_surface_rect,
            bool stretch_content_to_fill_bounds, bool allow_merge = true)
        {
            Quad quad;
        ...
        static Quad SurfaceQuad(const SurfaceRange& surface_range, SkColor4f default_background_color, const gfx::Rect& primary_surface_rect, float opacity,
            const gfx::Transform& transform, bool stretch_content_to_fill_bounds, const gfx::MaskFilterInfo& mask_filter_info, bool is_fast_rounded_corner)
        {
            Quad quad;
            quad.material = DrawQuad::Material::kSurfaceContent;
            quad.primary_surface_rect = primary_surface_rect;
        ...
        static Quad RenderPassQuad(CompositorRenderPassId id, const gfx::Transform& transform, bool intersects_damage_under)
        {
            Quad quad;
    ...
    struct Pass {
        Pass(const std::vector<Quad>& quads, CompositorRenderPassId id, const gfx::Size& size)
            : Pass(quads, id, gfx::Rect(size))
        {
        }
        Pass(const std::vector<Quad>& quads, CompositorRenderPassId id, const gfx::Rect& output_rect)
            : quads(quads)
            , id(id)
        ...
        Pass(const std::vector<Quad>& quads, const gfx::Size& size, const gfx::Rect& damage_rect)
            : quads(quads)
            , output_rect(size)
    ...
    static void AddQuadInPass(const Quad& desc, CompositorRenderPass* pass, std::vector<SurfaceRange>* referenced_surfaces)
    {
        ...
        case DrawQuad::Material::kSurfaceContent:
            ...
            AddSurfaceQuad(pass, desc.primary_surface_rect, desc.opacity, desc.to_target_transform, desc.surface_range, desc.default_background_color,
                desc.stretch_content_to_fill_bounds, desc.mask_filter_info, desc.is_fast_rounded_corner, desc.allow_merge);
            break;
        case DrawQuad::Material::kCompositorRenderPass:
            AddRenderPassQuad(pass, desc.render_pass_id, desc.transform, desc.intersects_damage_under);
            break;
        case DrawQuad::Material::kTextureContent:
            AddTextureDrawQuad(pass, desc.rect, desc.per_quad_damage_output, desc.to_target_transform);
            break;
        default:
    ...
    static void AddPasses(CompositorRenderPassList* pass_list, const std::vector<Pass>& passes, std::vector<SurfaceRange>* referenced_surfaces)
    {
        gfx::Transform root_transform;
        for (auto& pass : passes) {
            CompositorRenderPass* test_pass
                = AddRenderPassWithDamage(pass_list, pass.id, pass.output_rect, pass.damage_rect, root_transform, cc::FilterOperations());
            test_pass->has_transparent_background = pass.has_transparent_background;
            test_pass->has_damage_from_contributing_content = pass.has_damage_from_contributing_content;
            for (const auto& quad : pass.quads)
                AddQuadInPass(quad, test_pass, referenced_surfaces);
        }
    }

    static void TestQuadMatchesExpectations(Quad expected_quad, const DrawQuad* quad)
    {
        ...
        case DrawQuad::Material::kCompositorRenderPass: {
            ...
            const auto* render_pass_quad = AggregatedRenderPassDrawQuad::MaterialCast(quad);

            EXPECT_EQ(expected_quad.render_pass_id, CompositorRenderPassId { uint64_t { render_pass_quad->render_pass_id } });
            EXPECT_EQ(expected_quad.intersects_damage_under, render_pass_quad->intersects_damage_under);
            break;
        }
        default:
            NOTREACHED_IN_MIGRATION();
    ...
    static void TestPassMatchesExpectations(Pass expected_pass, const AggregatedRenderPass* pass)
    {
        ASSERT_EQ(expected_pass.quads.size(), pass->quad_list.size());
        for (auto iter = pass->quad_list.cbegin(); iter != pass->quad_list.cend(); ++iter) {
    ...
    static void TestPassesMatchExpectations(const std::vector<Pass>& expected_passes, const AggregatedRenderPassList* passes)
    {
...
private:
    static void AddSurfaceQuad(CompositorRenderPass* pass, const gfx::Rect& primary_surface_rect, float opacity, const gfx::Transform& transform,
        const SurfaceRange& surface_range, SkColor4f default_background_color, bool stretch_content_to_fill_bounds, const gfx::MaskFilterInfo& mask_filter_info,
        bool is_fast_rounded_corner, bool allow_merge)
    {
        ...
        shared_quad_state->SetAll(layer_to_target_transform, layer_bounds, visible_layer_rect, mask_filter_info, std::nullopt, are_contents_opaque, opacity,
        ...
        surface_quad->SetAll(pass->shared_quad_state_list.back(), primary_surface_rect, primary_surface_rect,
            /*needs_blending=*/true, surface_range, default_background_color, stretch_content_to_fill_bounds,
            /*is_reflection=*/false, allow_merge);
    }

    static void AddRenderPassQuad(
        CompositorRenderPass* pass, CompositorRenderPassId render_pass_id, const gfx::Transform& transform, bool intersects_damage_under)
    {
        gfx::Rect output_rect = gfx::Rect(0, 0, 5, 5);
        auto* shared_state = pass->CreateAndAppendSharedQuadState();
        shared_state->SetAll(transform, output_rect, output_rect, gfx::MaskFilterInfo(), std::nullopt, false, 1, SkBlendMode::kSrcOver, /*sorting_context=*/0,
        ...
        quad->SetAll(shared_state, output_rect, output_rect,
            /*needs_blending=*/true, render_pass_id, kInvalidResourceId, gfx::RectF(), gfx::Size(), gfx::Vector2dF(1.0f, 1.0f), gfx::PointF(), gfx::RectF(),
            /*force_anti_aliasing_off=*/false,
            /*backdrop_filter_quality=*/1.0f, intersects_damage_under);
    }

    static void AddTextureDrawQuad(CompositorRenderPass* pass, const gfx::Rect& output_rect, bool per_quad_damage_output, const gfx::Transform& transform)
    {
        auto* shared_state = pass->CreateAndAppendSharedQuadState();
        shared_state->SetAll(transform, output_rect, output_rect, gfx::MaskFilterInfo(), std::nullopt, false, 1, SkBlendMode::kSrcOver, /*sorting_context=*/0,
            /*layer_id=*/0u, /*fast_rounded_corner=*/false);
        ...
        quad->SetNew(shared_state, output_rect, output_rect, false /*needs_blending*/, ResourceId(1), false /*premultiplied_alpha*/, kUVTopLeft, kUVBottomRight,
            SkColors::kTransparent, false /*flipped*/, false /*nearest_neighbor*/, false /*secure_output_only*/, gfx::ProtectedVideoType::kClear);
...
protected:
    ...
    FrameSinkManagerImpl manager_ { FrameSinkManagerImpl::InitParams(&shared_bitmap_manager_) };
    DisplayResourceProviderSoftware resource_provider_ { &shared_bitmap_manager_, &shared_image_manager_, &sync_point_manager_, &gpu_scheduler_ };
...
public:
    SurfaceAggregatorValidSurfaceTest(SurfaceAggregator::ExtraPassForReadbackOption extra_pass_option, bool prevent_merging_surfaces_to_root_pass)
        : SurfaceAggregatorTest(extra_pass_option, prevent_merging_surfaces_to_root_pass)
        , child_sink_(std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryReservedFrameSinkId,
              /*is_root=*/false))
    ...
    SurfaceAggregatorValidSurfaceTest()
        : SurfaceAggregatorValidSurfaceTest(SurfaceAggregator::ExtraPassForReadbackOption::kNone,
            /*prevent_merging_surfaces_to_root_pass=*/false)
    {
    }
    ...
    void SetUp() override
    {
        ...
        root_surface_ = manager_.surface_manager()->GetSurfaceForId(root_surface_id_);
    }
    ...
    void AggregateAndVerify(const std::vector<Pass>& expected_passes, const std::vector<SurfaceId>& expected_surface_ids)
    {
        auto aggregated_frame = AggregateFrame(root_surface_id_);

        TestPassesMatchExpectations(expected_passes, &aggregated_frame.render_pass_list);
        VerifyQuadCoverSQS(&aggregated_frame);
        VerifyExpectedSurfaceIds(expected_surface_ids);
    }

    void VerifyExpectedSurfaceIds(const std::vector<SurfaceId>& expected_surface_ids)
    {
        EXPECT_THAT(aggregator_.previous_contained_surfaces(), testing::UnorderedElementsAreArray(expected_surface_ids));

        EXPECT_EQ(expected_surface_ids.size(), aggregator_.previous_contained_frame_sinks().size());
        for (const SurfaceId& surface_id : expected_surface_ids) {
            EXPECT_THAT(
                aggregator_.previous_contained_frame_sinks(), testing::Contains(testing::Pair(surface_id.frame_sink_id(), surface_id.local_surface_id())));
        }
    }

    void SubmitPassListAsFrame(CompositorFrameSinkSupport* support, const LocalSurfaceId& local_surface_id, CompositorRenderPassList* pass_list,
        std::vector<SurfaceRange> referenced_surfaces, float device_scale_factor)
    {
    ...
    CompositorRenderPassList GenerateRenderPassList(const std::vector<Pass>& passes, std::vector<SurfaceRange>* referenced_surfaces)
    {
    ...
    void SubmitCompositorFrame(
        CompositorFrameSinkSupport* support, const std::vector<Pass>& passes, const LocalSurfaceId& local_surface_id, float device_scale_factor)
    {
        std::vector<SurfaceRange> referenced_surfaces;
        CompositorRenderPassList pass_list = GenerateRenderPassList(passes, &referenced_surfaces);
        SubmitPassListAsFrame(support, local_surface_id, &pass_list, std::move(referenced_surfaces), device_scale_factor);
    }

    void QueuePassAsFrame(
        std::unique_ptr<CompositorRenderPass> pass, const LocalSurfaceId& local_surface_id, float device_scale_factor, CompositorFrameSinkSupport* support)
    {
        CompositorFrame child_frame = CompositorFrameBuilder().AddRenderPass(std::move(pass)).SetDeviceScaleFactor(device_scale_factor).Build();
...
class SurfaceAggregatorValidSurfaceWithMergingPassesTest : public SurfaceAggregatorValidSurfaceTest, public testing::WithParamInterface<bool> {
public:
...
TEST_F(SurfaceAggregatorValidSurfaceTest, SimpleFrame)
{
    CompositorFrame frame
        = CompositorFrameBuilder()
              .AddRenderPass(
                  RenderPassBuilder(kSurfaceSize).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kRed).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kBlue))
              .Build();

    root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame));

    // Add a callback for when the surface is damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());

    // Check that the AggregatedDamageCallback is called with the right arguments.
    EXPECT_CALL(
        aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, gfx::Rect(kSurfaceSize), next_display_time()));

    auto aggregated_frame = AggregateFrame(root_surface_id_);
    EXPECT_EQ(aggregated_frame.render_pass_list.size(), 1u);

    auto& render_pass = aggregated_frame.render_pass_list[0];
    EXPECT_THAT(render_pass->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kRed), IsSolidColorQuad(SkColors::kBlue)));
...
{
    ViewTransitionElementResourceId vt_resource_id(blink::ViewTransitionToken(), 1);

    CompositorFrame frame
        = CompositorFrameBuilder()
              .AddRenderPass(
                  RenderPassBuilder(kSurfaceSize).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kRed).AddSharedElementQuad(gfx::Rect(5, 5), vt_resource_id))
              .Build();
    ...
    auto& render_pass = aggregated_frame.render_pass_list[0];
    EXPECT_THAT(render_pass->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kRed)));
}
...
{
    auto embedded_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/true);
    TestSurfaceIdAllocator embedded_surface_id(embedded_support->frame_sink_id());

    {
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, gfx::Rect(kSurfaceSize))
                                                       .AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kGreen)
                                                       .AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kBlue))
                                    .Build();
        embedded_support->SubmitCompositorFrame(embedded_surface_id.local_surface_id(), std::move(frame));
    }

    {
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, gfx::Rect(kSurfaceSize))
                                                       .AddSurfaceQuad(gfx::Rect(5, 5), SurfaceRange(embedded_surface_id))
                                                       .SetQuadOpacity(0.5f))
                                    .Build();
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame));
    ...
    {
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 2 }, gfx::Rect(kSurfaceSize))
                                                       .AddSurfaceQuad(gfx::Rect(5, 5), SurfaceRange(embedded_surface_id))
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RotatedClip)
{
...

    {
        CompositorFrame frame
            = CompositorFrameBuilder()
                  .AddRenderPass(
                      RenderPassBuilder(kSurfaceSize).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kGreen).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kBlue))
                  .Build();
        embedded_support->SubmitCompositorFrame(embedded_surface_id.local_surface_id(), std::move(frame));
    }
    ...
    CompositorFrame frame = CompositorFrameBuilder()
                                .AddRenderPass(RenderPassBuilder(kSurfaceSize)
                                                   .AddSurfaceQuad(gfx::Rect(5, 5), SurfaceRange(std::nullopt, embedded_surface_id))
    ...
    EXPECT_THAT(embedded_pass->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kGreen), IsSolidColorQuad(SkColors::kBlue)));

    auto& root_pass = aggregated_frame.render_pass_list[1];
    EXPECT_THAT(root_pass->quad_list, ElementsAre(AllOf(IsAggregatedRenderPassQuad(), HasTransform(rotate))));
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, ClipMergedPasses)
{
    ...
    auto grandchild_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &this->manager_, kArbitraryFrameSinkId1, false);
    TestSurfaceIdAllocator grandchild_surface_id(grandchild_support->frame_sink_id());
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());

    {
        auto frame = CompositorFrameBuilder()
                         .AddRenderPass(RenderPassBuilder(grandchild_child_rect).AddSolidColorQuad(gfx::Rect(grandchild_child_rect), SkColors::kBlue))
                         .Build();
        grandchild_support->SubmitCompositorFrame(grandchild_surface_id.local_surface_id(), std::move(frame));
    }

    {
        ...
        auto frame = CompositorFrameBuilder()
                         .AddRenderPass(RenderPassBuilder(child_rect)
                                            .AddSurfaceQuad(grandchild_child_rect, SurfaceRange(grandchild_surface_id))
                                            .SetQuadToTargetTranslation(50, 50))
                         .Build();
        child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), std::move(frame));
    }

    {
        ...
        // surface.
        auto frame = CompositorFrameBuilder()
                         .AddRenderPass(RenderPassBuilder(root_size)
                                            .AddSurfaceQuad(grandchild_child_rect, SurfaceRange(child_surface_id))
                                            .SetQuadToTargetTranslation(50, 50)
    ...
    auto& render_pass = aggregated_frame.render_pass_list[0];
    EXPECT_THAT(render_pass->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kBlue), IsSolidColorQuad(SkColors::kWhite)));
    ...
    EXPECT_THAT(clipped_quad->shared_quad_state->clip_rect, testing::Optional(gfx::Rect(100, 100, 50, 50)));
}

TEST_F(SurfaceAggregatorValidSurfaceTest, MultiPassSimpleFrame)
{
    std::vector<Quad> quads[2] = { { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kLtGray, gfx::Rect(5, 5)) },
        { Quad::SolidColorQuad(SkColors::kGray, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kDkGray, gfx::Rect(5, 5)) } };
    std::vector<Pass> passes = { Pass(quads[0], CompositorRenderPassId { 1 }, kSurfaceSize), Pass(quads[1], CompositorRenderPassId { 2 }, kSurfaceSize) };

    constexpr float device_scale_factor = 1.0f;

    SubmitCompositorFrame(root_sink_.get(), passes, root_surface_id_.local_surface_id(), device_scale_factor);
...
TEST_F(SurfaceAggregatorValidSurfaceTest, MultiPassDeallocation)
{
    std::vector<Quad> quads[2] = { { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kLtGray, gfx::Rect(5, 5)) },
        { Quad::SolidColorQuad(SkColors::kGray, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kDkGray, gfx::Rect(5, 5)) } };
    std::vector<Pass> passes = { Pass(quads[0], CompositorRenderPassId { 2 }, kSurfaceSize), Pass(quads[1], CompositorRenderPassId { 1 }, kSurfaceSize) };
    ...
    std::vector<Pass> passes2 = { Pass(quads[0], CompositorRenderPassId { 3 }, kSurfaceSize), Pass(quads[1], CompositorRenderPassId { 1 }, kSurfaceSize) };

    SubmitCompositorFrame(root_sink_.get(), passes2, root_surface_id_.local_surface_id(), device_scale_factor);
...
TEST_F(SurfaceAggregatorValidSurfaceTest, MultiSurfacePassDeallocation)
{
    ...
    std::vector<Pass> child_passes = { Pass(quads[0], CompositorRenderPassId { 1 }, kSurfaceSize) };
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    SubmitCompositorFrame(child_sink_.get(), child_passes, child_surface_id.local_surface_id(), device_scale_factor);
    // Disallow merging so the child pass ids can be tested.
    std::vector<Quad> child_surface_quads = { Quad::SurfaceQuad(SurfaceRange(child_surface_id), SkColors::kBlack, gfx::Rect(5, 5),
    ...
    SubmitCompositorFrame(root_sink_.get(), root_embedding_passes, root_surface_id_.local_surface_id(), device_scale_factor);
    aggregated_frame = AggregateFrame(root_surface_id_);
...
TEST_F(SurfaceAggregatorValidSurfaceTest, SimpleSurfaceReference)
{
    ...
    {
        CompositorFrame frame
            = CompositorFrameBuilder().AddRenderPass(RenderPassBuilder(kSurfaceSize).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kGreen)).Build();
        embedded_support->SubmitCompositorFrame(embedded_surface_id.local_surface_id(), std::move(frame));
    }
    ...
    auto& render_pass = aggregated_frame.render_pass_list[0];
    EXPECT_THAT(
        render_pass->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kWhite), IsSolidColorQuad(SkColors::kGreen), IsSolidColorQuad(SkColors::kBlack)));
...
class TestVizClient {
public:
    TestVizClient(SurfaceAggregatorValidSurfaceTest* test, FrameSinkManagerImpl* manager, const FrameSinkId& frame_sink_id, const gfx::Rect& bounds)
        : test_(test)
        , manager_(manager)
        ...
        , bounds_(bounds)
    {
        ...
        root_sink_ = std::make_unique<CompositorFrameSinkSupport>(nullptr, manager_, frame_sink_id, is_root);
    ...
    {
        return manager_->surface_manager()->GetSurfaceForId(SurfaceId(frame_sink_id_, local_surface_id()));
    }
    ...
    void SubmitCompositorFrame(SkColor4f bgcolor)
    {
        ...
        for (const auto& embed : embedded_clients_) {
            if (embed.second) {
                embedded_quads.push_back(Quad::SurfaceQuad(SurfaceRange(std::nullopt, embed.first->surface_id()), SkColors::kWhite, embed.first->bounds(),
                    /*stretch_content_to_fill_bounds=*/false));
            } else {
                referenced_surfaces.emplace_back(SurfaceRange(std::nullopt, embed.first->surface_id()));
            }
        }
        std::vector<Pass> embedded_passes = { Pass(embedded_quads, bounds_.size()) };

        constexpr float device_scale_factor = 1.0f;
        CompositorRenderPassList pass_list = test_->GenerateRenderPassList(embedded_passes, &referenced_surfaces);
        test_->SubmitPassListAsFrame(root_sink_.get(), local_surface_id(), &pass_list, referenced_surfaces, device_scale_factor);
    }
    ...
    CopyOutputRequest* RequestCopyOfOutput()
    {
        ...
        root_sink_->RequestCopyOfOutput(PendingCopyOutputRequest { local_surface_id(), SubtreeCaptureId(), std::move(copy_request) });
        return copy_request_ptr;
    }

    SurfaceId surface_id() const
    {
        return { frame_sink_id_, local_surface_id() };
    }
...
{
    TestVizClient child(this, &manager_, kArbitraryFrameSinkId1, gfx::Rect(10, 10));
    ...
    TestVizClient parent(this, &manager_, kArbitraryFrameSinkId2, gfx::Rect(15, 15));
    parent.SetEmbeddedClient(&child, false);
    parent.SubmitCompositorFrame(SkColors::kGreen);

    std::vector<Quad> root_quads = { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, parent.surface_id()), SkColors::kWhite, parent.bounds(),
            /*stretch_content_to_fill_bounds=*/false),
        Quad::SolidColorQuad(SkColors::kBlack, gfx::Rect(5, 5)) };
    std::vector<Pass> root_passes = { Pass(root_quads, kSurfaceSize) };

    constexpr float device_scale_factor = 1.0f;
    SubmitCompositorFrame(root_sink_.get(), root_passes, root_surface_id_.local_surface_id(), device_scale_factor);

    std::vector<Quad> expected_quads = { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kGreen, parent.bounds()),
        Quad::SolidColorQuad(SkColors::kBlack, gfx::Rect(5, 5)) };
    std::vector<Pass> expected_passes = { Pass(expected_quads, kSurfaceSize) };
    AggregateAndVerify(expected_passes, { root_surface_id_, parent.surface_id(), child.surface_id() });
    ...
    expected_quads = { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kGreen, parent.bounds()),
        Quad::SolidColorQuad(SkColors::kBlue, child.bounds()), Quad::SolidColorQuad(SkColors::kBlack, gfx::Rect(5, 5)) };
    AggregateAndVerify({ Pass(expected_quads, kSurfaceSize) }, { root_surface_id_, parent.surface_id(), child.surface_id() });
...
TEST_F(SurfaceAggregatorValidSurfaceTest, UndrawnSurfacesWithCopyRequests)
{
    ...
    std::vector<Quad> expected_quads = { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kGreen, parent.bounds()),
        Quad::SolidColorQuad(SkColors::kBlack, gfx::Rect(5, 5)) };
    std::vector<Quad> expected_copy_quads = { Quad::SolidColorQuad(SkColors::kBlue, child.bounds()) };
    std::vector<Pass> expected_passes = { Pass(expected_copy_quads, kSurfaceSize), Pass(expected_quads, kSurfaceSize) };
    AggregateAndVerify(expected_passes, { root_surface_id_, parent.surface_id(), child.surface_id() });
    EXPECT_FALSE(child.GetSurface()->HasUndrawnActiveFrame());
    EXPECT_FALSE(parent.GetSurface()->HasUndrawnActiveFrame());
}

TEST_F(SurfaceAggregatorValidSurfaceTest, SurfacesWithMultipleEmbeddersBothVisibleAndInvisible)
{
    TestVizClient child(this, &manager_, kArbitraryFrameSinkId1, gfx::Rect(10, 10));
    child.SubmitCompositorFrame(SkColors::kBlue);
    ...
    // not provide a DrawQuad that embeds it.
    TestVizClient first_parent(this, &manager_, kArbitraryFrameSinkId2, gfx::Rect(15, 15));
    ...
    TestVizClient second_parent(this, &manager_, kArbitraryMiddleFrameSinkId, gfx::Rect(25, 25));
    ...
    std::vector<Quad> root_quads = { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, first_parent.surface_id()), SkColors::kCyan, first_parent.bounds(),
            /*stretch_content_to_fill_bounds=*/false),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, second_parent.surface_id()), SkColors::kMagenta, second_parent.bounds(),
            /*stretch_content_to_fill_bounds=*/false),
        Quad::SolidColorQuad(SkColors::kBlack, gfx::Rect(5, 5)) };
    std::vector<Pass> root_passes = { Pass(root_quads, kSurfaceSize) };
    ...
    std::vector<Quad> expected_quads = { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kGreen, first_parent.bounds()),
        Quad::SolidColorQuad(SkColors::kYellow, second_parent.bounds()), Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(10, 10)),
        Quad::SolidColorQuad(SkColors::kBlack, gfx::Rect(5, 5)) };
    std::vector<Quad> expected_copy_quads = {};
    std::vector<Pass> expected_passes = { Pass(expected_quads, kSurfaceSize) };
    AggregateAndVerify(expected_passes, { root_surface_id_, first_parent.surface_id(), second_parent.surface_id(), child.surface_id() });
    EXPECT_FALSE(child.GetSurface()->HasUndrawnActiveFrame());
...
{
    auto child_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/false);
    ...
    auto child_pass = RenderPassBuilder(CompositorRenderPassId { 1 }, child_surface_rect)
...

    {
        child_support->SubmitCompositorFrame(
            child_surface_id.local_surface_id(), CompositorFrameBuilder().AddRenderPass(child_pass->DeepCopy()).SetDeviceScaleFactor(2.0f).Build());
    }
    ...
    constexpr gfx::Rect root_quad_rect(10, 10);
    {
        auto pass = RenderPassBuilder(kSurfaceSize)
                        ...
                        .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(child_surface_id))
                        .Build();

        CompositorFrame frame = CompositorFrameBuilder().AddRenderPass(std::move(pass)).SetDeviceScaleFactor(1.0f).Build();
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame));
    }
...

    {
        ...
        child_support->SubmitCompositorFrame(
            child_surface_id.local_surface_id(), CompositorFrameBuilder().AddRenderPass(std::move(child_pass)).SetDeviceScaleFactor(2.0f).Build());
    }

    {
        ...
        EXPECT_TRUE(root_quad->shared_quad_state->quad_to_target_transform.IsIdentity());
        ...
        gfx::Rect scaled_child_quad_rect = cc::MathUtil::MapEnclosingClippedRect(child_quad->shared_quad_state->quad_to_target_transform, child_quad->rect);
...
TEST_F(SurfaceAggregatorValidSurfaceTest, LayerIds)
{
    ...
    gfx::Rect child_surface_rect(20, 20);
    {
        CompositorFrame frame
            = CompositorFrameBuilder()
                  .AddRenderPass(RenderPassBuilder(child_surface_rect).AddSolidColorQuad(gfx::Rect(20, 20), SkColors::kRed).SetQuadLayerId(1u))
                  .Build();
...

    {
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(RenderPassBuilder(kSurfaceSize)
                                                       .AddSurfaceQuad(child_surface_rect, SurfaceRange(child_surface_id))
                                                       .SetQuadLayerId(2)
                                                       .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kBlack)
                                                       .SetQuadLayerId(3))
                                    .Build();
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame));
    }

    {
        ...
        uint32_t root_surface_namespace = aggregator_.GetLatestFrameData(root_surface_id_)->GetClientNamespaceId();
        uint32_t child_surface_namespace = aggregator_.GetLatestFrameData(child_surface_id)->GetClientNamespaceId();
        ...
        EXPECT_THAT(render_pass->quad_list,
            ElementsAre(AllOf(IsSolidColorQuad(SkColors::kRed), HasLayerNamespaceId(child_surface_namespace), HasLayerId(1u)),
                AllOf(IsSolidColorQuad(SkColors::kBlack), HasLayerNamespaceId(root_surface_namespace), HasLayerId(3u))));
    }
    ...
    // render pass.
    {
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(RenderPassBuilder(kSurfaceSize)
                                                       .AddSurfaceQuad(child_surface_rect, SurfaceRange(child_surface_id), { .allow_merge = false })
                                                       .SetQuadLayerId(2)
                                                       .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kBlack)
                                                       .SetQuadLayerId(3))
                                    .Build();
...

    {
        ...
        EXPECT_THAT(child_pass->quad_list, ElementsAre(AllOf(IsSolidColorQuad(SkColors::kRed), HasLayerNamespaceId(child_surface_namespace), HasLayerId(1u))));
        ...
        EXPECT_THAT(root_pass->quad_list,
            ElementsAre(AllOf(IsAggregatedRenderPassQuad(), HasLayerNamespaceId(root_surface_namespace), HasLayerId(2u)),
                AllOf(IsSolidColorQuad(SkColors::kBlack), HasLayerNamespaceId(root_surface_namespace), HasLayerId(3u))));
    }
}
...
{
    auto primary_child_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/false);
    TestSurfaceIdAllocator primary_child_surface_id(primary_child_support->frame_sink_id());

    {
        CompositorFrame frame
            = CompositorFrameBuilder().AddRenderPass(RenderPassBuilder(gfx::Rect(20, 20)).AddSolidColorQuad(gfx::Rect(20, 20), SkColors::kRed)).Build();

        primary_child_support->SubmitCompositorFrame(primary_child_surface_id.local_surface_id(), std::move(frame));
    }
...

    {
        ...
        CompositorFrame frame
            = CompositorFrameBuilder()
                  .AddRenderPass(RenderPassBuilder(kSurfaceSize)
                                     .AddSurfaceQuad(surface_quad_rect, SurfaceRange(primary_child_surface_id), { .stretch_content_to_fill_bounds = true }))
                  .Build();
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame));
    }
    ...
    gfx::RectF output_rect = output_quad->shared_quad_state->quad_to_target_transform.MapRect(gfx::RectF(100.f, 100.f));
...
TEST_F(SurfaceAggregatorValidSurfaceTest, StretchContentToFillStretchedBounds)
{
...

    {
        ...
        pass->SetNew(CompositorRenderPassId { 1 }, gfx::Rect(0, 0, 20, 20), gfx::Rect(), gfx::Transform());
        auto* sqs = pass->CreateAndAppendSharedQuadState();
        auto* solid_color_quad = pass->CreateAndAppendDrawQuad<SolidColorDrawQuad>();

        solid_color_quad->SetNew(sqs, gfx::Rect(0, 0, 20, 20), gfx::Rect(0, 0, 20, 20), SkColors::kRed, false);

        CompositorFrame frame = CompositorFrameBuilder().AddRenderPass(std::move(pass)).Build();

        primary_child_support->SubmitCompositorFrame(primary_child_surface_id.local_surface_id(), std::move(frame));
    }

    constexpr gfx::Rect surface_quad_rect(10, 5);
    std::vector<Quad> root_quads = { Quad::SurfaceQuad(SurfaceRange(primary_child_surface_id), SkColors::kWhite, surface_quad_rect,
    ...
    SubmitCompositorFrame(root_sink_.get(), root_passes, root_surface_id_.local_surface_id(), 2.0f);
    ...
    // SurfaceAggregator should stretch the SolidColorDrawQuad to fit the bounds
    // of the parent's SurfaceDrawQuad.
    gfx::RectF output_rect = output_quad->shared_quad_state->quad_to_target_transform.MapRect(gfx::RectF(200.f, 200.f));
...
TEST_F(SurfaceAggregatorValidSurfaceTest, StretchContentToFillSquashedBounds)
{
    ...
    {
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(RenderPassBuilder(child_surface_rect).AddSolidColorQuad(child_surface_rect, SkColors::kRed))
    ...
    EXPECT_THAT(render_pass->quad_list, ElementsAre(AllOf(IsSolidColorQuad(), HasRect(child_surface_rect), HasTransform(expected_transform))));
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, ReflectedSurfaceDrawQuadScaled)
{
    ...
    {
        CompositorFrame frame = CompositorFrameBuilder().AddRenderPass(RenderPassBuilder(display_rect).AddSolidColorQuad(display_rect, SkColors::kRed)).Build();
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame));
    }

    auto mirror_display_sink = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, true);

    TestSurfaceIdAllocator mirror_display_surface_id(mirror_display_sink->frame_sink_id());
...

    {
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(RenderPassBuilder(mirror_display_rect)
                                                       .AddSurfaceQuad(display_rect, SurfaceRange(root_surface_id_),
                                                           { .stretch_content_to_fill_bounds = true, .is_reflection = true })
                                                       .SetQuadToTargetTransform(scale_transform))
                                    .Build();
        mirror_display_sink->SubmitCompositorFrame(mirror_display_surface_id.local_surface_id(), std::move(frame));
    }
    ...
    EXPECT_THAT(root_render_pass->quad_list, ElementsAre(IsAggregatedRenderPassQuad()));
    ...
    EXPECT_EQ(output_quad->shared_quad_state->quad_to_target_transform, scale_transform);
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, ReflectedSurfaceDrawQuadNotScaled)
{
...

    {
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(RenderPassBuilder(mirror_display_rect)
                                                       .AddSurfaceQuad(display_rect, SurfaceRange(std::nullopt, root_surface_id_),
                                                           { .stretch_content_to_fill_bounds = true, .is_reflection = true })
    ...
    EXPECT_THAT(root_render_pass->quad_list, ElementsAre(AllOf(IsSolidColorQuad(), HasTransform(translate_transform))));
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, FallbackSurfaceReferenceWithPrimary)
{
    auto primary_child_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/false);
    TestSurfaceIdAllocator primary_child_surface_id(primary_child_support->frame_sink_id());
    std::vector<Quad> primary_child_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(kSurfaceSize)) };
    constexpr gfx::Size primary_size(50, 50);
    std::vector<Pass> primary_child_passes = { Pass(primary_child_quads, primary_size) };

    // Submit a CompositorFrame to the primary Surface containing a green
    // SolidColorDrawQuad.
    constexpr float device_scale_factor = 1.0f;
    SubmitCompositorFrame(primary_child_support.get(), primary_child_passes, primary_child_surface_id.local_surface_id(), device_scale_factor);

    auto fallback_child_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId2, /*is_root=*/false);
    TestSurfaceIdAllocator fallback_child_surface_id(fallback_child_support->frame_sink_id());

    std::vector<Quad> fallback_child_quads = { Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(kSurfaceSize)) };
    std::vector<Pass> fallback_child_passes = { Pass(fallback_child_quads, kSurfaceSize) };

    // Submit a CompositorFrame to the fallback Surface containing a red
    // SolidColorDrawQuad.
    SubmitCompositorFrame(fallback_child_support.get(), fallback_child_passes, fallback_child_surface_id.local_surface_id(), device_scale_factor);

    // Try to embed |primary_child_surface_id| and if unavailabe, embed
    // |fallback_child_surface_id|.
    std::vector<Quad> root_quads
        = { Quad::SurfaceQuad(SurfaceRange(fallback_child_surface_id, primary_child_surface_id), SkColors::kWhite, gfx::Rect(kSurfaceSize),
            /*stretch_content_to_fill_bounds=*/false) };
    ...
    // SolidColorDrawQuad.
    EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), root_size, gfx::Rect(root_size), next_display_time()));

    auto aggregated_frame = AggregateFrame(root_surface_id_);

    ASSERT_EQ(aggregated_frame.render_pass_list.size(), 1u);
    EXPECT_THAT(aggregated_frame.render_pass_list[0]->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kGreen)));
    ...
    EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), root_size, gfx::Rect(primary_size), next_display_time()));
...
{
    auto embedded_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/false);
    TestSurfaceIdAllocator embedded_surface_id(embedded_support->frame_sink_id());

    CompositorFrame embedded_frame
        = CompositorFrameBuilder()
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kGreen))
              .Build();
    embedded_support->SubmitCompositorFrame(embedded_surface_id.local_surface_id(), std::move(embedded_frame));

    auto copy_request = CopyOutputRequest::CreateStubForTesting();
    auto* copy_request_ptr = copy_request.get();
    embedded_support->RequestCopyOfOutput({ embedded_surface_id.local_surface_id(), SubtreeCaptureId(), std::move(copy_request) });

    CompositorFrame root_frame = CompositorFrameBuilder()
                                     .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                                                        .AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kWhite)
                                                        .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(embedded_surface_id))
                                                        .AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kBlack)
                                                        .Build())
                                     .Build();
    root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(root_frame));

    auto aggregated_frame = AggregateFrame(root_surface_id_);

    std::vector<Quad> embedded_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
    std::vector<Quad> root_quads = { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)),
        Quad::RenderPassQuad(CompositorRenderPassId { uint64_t { aggregated_frame.render_pass_list[0]->id } }, gfx::Transform(), true),
        Quad::SolidColorQuad(SkColors::kBlack, gfx::Rect(5, 5)) };
    std::vector<Pass> expected_passes = { Pass(embedded_quads, kSurfaceSize), Pass(root_quads, kSurfaceSize) };
    TestPassesMatchExpectations(expected_passes, &aggregated_frame.render_pass_list);
    EXPECT_TRUE(aggregated_frame.has_copy_requests);
    ASSERT_EQ(2u, aggregated_frame.render_pass_list.size());
    ASSERT_EQ(1u, aggregated_frame.render_pass_list[0]->copy_requests.size());
    DCHECK_EQ(copy_request_ptr, aggregated_frame.render_pass_list[0]->copy_requests[0].get());

    VerifyExpectedSurfaceIds({ root_surface_id_, embedded_surface_id });
}

TEST_F(SurfaceAggregatorValidSurfaceTest, ShouldNotTakeCopyRequestIfTakeCopyRequestIsFalse)
{
    auto embedded_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/false);
    TestSurfaceIdAllocator embedded_surface_id(embedded_support->frame_sink_id());
    ...
    CompositorFrame embedded_frame
        = CompositorFrameBuilder()
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kGreen).Build())
              .Build();
    embedded_support->SubmitCompositorFrame(embedded_surface_id.local_surface_id(), std::move(embedded_frame));
...
TEST_F(SurfaceAggregatorValidSurfaceTest, CopyRequestOnEmbeddedSurfaceWithProtectedQuads)
{
    auto embedded_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId2, /*is_root=*/false);
    TestSurfaceIdAllocator embedded_surface_id(embedded_support->frame_sink_id());
        ...
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(RenderPassBuilder(kSurfaceSize)
                                                       .AddTextureQuad(gfx::Rect(20, 20), ResourceId(1),
                                                           TextureQuadParams { .protected_video_type = gfx::ProtectedVideoType::kHardwareProtected })
    ...
    // Embedded surface
    EXPECT_THAT(render_pass_list[0]->quad_list, ElementsAre(IsTextureQuad(), IsSolidColorQuad(SkColors::kGreen)));
    EXPECT_THAT(render_pass_list[0]->copy_requests, testing::SizeIs(1u));

    // Root pass
    EXPECT_THAT(render_pass_list[1]->quad_list, ElementsAre(IsTextureQuad(), IsSolidColorQuad(SkColors::kGreen)));
    ...
    const CompositorFrame& original_frame = manager_.surface_manager()->GetSurfaceForId(embedded_surface_id)->GetActiveFrame();
...
TEST_F(SurfaceAggregatorValidSurfaceTest, CopyRequestOnSurfaceEmbeddingSurfaceWithProtectedQuads)
{
    auto video_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId2, /*is_root=*/false);
    TestSurfaceIdAllocator video_surface_id(video_support->frame_sink_id());

    auto embedded_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId3, /*is_root=*/false);
    TestSurfaceIdAllocator embedded_surface_id(embedded_support->frame_sink_id());

    {
        ...
        video_support->SubmitCompositorFrame(video_surface_id.local_surface_id(), std::move(frame));
    }

    {
        CompositorFrame frame = CompositorFrameBuilder()
                                    .AddRenderPass(RenderPassBuilder(kSurfaceSize)
                                                       .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(video_surface_id),
                                                           { .default_background_color = SkColors::kYellow }))
                                    .Build();
    ...
    VerifyExpectedSurfaceIds({ root_surface_id_, embedded_surface_id, video_surface_id });
    ...
    // Ensure copy requests have been removed from the embedded surface.
    const CompositorFrame& original_frame = manager_.surface_manager()->GetSurfaceForId(root_surface_id_)->GetActiveFrame();
    const auto& original_pass_list = original_frame.render_pass_list;
    ASSERT_EQ(1u, original_pass_list.size());
    EXPECT_THAT(original_pass_list[0]->copy_requests, testing::IsEmpty());
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RootCopyRequest)
{
    ...
    {
        CompositorFrame frame = CompositorFrameBuilder().AddRenderPass(RenderPassBuilder(kSurfaceSize).AddSolidColorQuad(quad_rect, SkColors::kGreen)).Build();
        ...
        CompositorFrame frame
            = CompositorFrameBuilder()
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                                     ...
                                     .AddSurfaceQuad(quad_rect, SurfaceRange(embedded_surface_id), { .default_background_color = SkColors::kYellow })
                                     .AddSolidColorQuad(quad_rect, SkColors::kBlack))
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize)
    ...
    EXPECT_THAT(render_pass_list[0]->quad_list,
        ElementsAre(IsSolidColorQuad(SkColors::kWhite), IsSolidColorQuad(SkColors::kGreen), IsSolidColorQuad(SkColors::kBlack)));
    EXPECT_THAT(render_pass_list[0]->copy_requests, ElementsAre(testing::Pointer(child_copy_request_ptr.get())));

    EXPECT_THAT(render_pass_list[1]->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kRed)));
    EXPECT_THAT(render_pass_list[1]->copy_requests, ElementsAre(testing::Pointer(root_copy_request_ptr.get())));
...
TEST_F(SurfaceAggregatorValidSurfaceTest, ShouldNotTakeRootCopyRequestIfTakeCopyRequestIsFalse)
{
    auto embedded_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId2, /*is_root=*/false);
    TestSurfaceIdAllocator embedded_surface_id(embedded_support->frame_sink_id());
    ...
    CompositorFrame root_frame
        = CompositorFrameBuilder()
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kWhite).Build())
              .Build();
    root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(root_frame));

    auto copy_request = CopyOutputRequest::CreateStubForTesting();
    auto* copy_request_ptr = copy_request.get();
    root_sink_->RequestCopyOfOutput({ root_surface_id_.local_surface_id(), SubtreeCaptureId(), std::move(copy_request) });
    ...
    EXPECT_EQ(copy_request_ptr, original_frame.render_pass_list[0]->copy_requests[0].get());
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, VideoCapturePreventsMerge)
{
    ...
    // Frame #1: Video capture is enabled with a copy request
    {
        ...
        EXPECT_FALSE(aggregated_frame.render_pass_list.back()->video_capture_enabled);

        ASSERT_EQ(1u, aggregated_frame.render_pass_list[0]->copy_requests.size());
        EXPECT_EQ(copy_request_ptr, aggregated_frame.render_pass_list[0]->copy_requests[0].get());
    }
...
{
    auto embedded_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/false);
    auto parent_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId2, /*is_root=*/true);
    TestSurfaceIdAllocator embedded_surface_id(embedded_support->frame_sink_id());
    ...
    constexpr float device_scale_factor = 1.0f;
    SubmitCompositorFrame(embedded_support.get(), embedded_passes, embedded_surface_id.local_surface_id(), device_scale_factor);
    auto copy_request(CopyOutputRequest::CreateStubForTesting());
    auto* copy_request_ptr = copy_request.get();
    embedded_support->RequestCopyOfOutput({ embedded_surface_id.local_surface_id(), SubtreeCaptureId(), std::move(copy_request) });
    ...
    std::vector<Quad> parent_quads = { Quad::SolidColorQuad(SkColors::kGray, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, embedded_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false),
...

    {
        ...
        AddPasses(&frame.render_pass_list, parent_passes, &frame.metadata.referenced_surfaces);

        frame.metadata.referenced_surfaces.emplace_back(embedded_surface_id);

        parent_support->SubmitCompositorFrame(parent_surface_id.local_surface_id(), std::move(frame));
    }

    std::vector<Quad> root_quads = { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kBlack, gfx::Rect(5, 5)) };
    std::vector<Pass> root_passes = { Pass(root_quads, kSurfaceSize) };

    {
        CompositorFrame frame = MakeEmptyCompositorFrame();
        AddPasses(&frame.render_pass_list, root_passes, &frame.metadata.referenced_surfaces);
    ...
    VerifyExpectedSurfaceIds({ root_surface_id_, parent_surface_id, embedded_surface_id });
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, MultiPassSurfaceReference)
{
    ...
    CompositorRenderPassId pass_ids[] = { CompositorRenderPassId { 1 }, CompositorRenderPassId { 2 }, CompositorRenderPassId { 3 } };

    std::vector<Quad> embedded_quads[3] = { { Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 1.0f / 255.0f }, gfx::Rect(5, 5)),
                                                Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 2.0f / 255.0f }, gfx::Rect(5, 5)) },
        { Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 3.0f / 255.0f }, gfx::Rect(5, 5)), Quad::RenderPassQuad(pass_ids[0], gfx::Transform(), true) },
        { Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 4.0f / 255.0f }, gfx::Rect(5, 5)), Quad::RenderPassQuad(pass_ids[1], gfx::Transform(), true) } };
    std::vector<Pass> embedded_passes = { Pass(embedded_quads[0], pass_ids[0], kSurfaceSize), Pass(embedded_quads[1], pass_ids[1], kSurfaceSize),
        Pass(embedded_quads[2], pass_ids[2], kSurfaceSize) };

    constexpr float device_scale_factor = 1.0f;
    SubmitCompositorFrame(child_sink_.get(), embedded_passes, embedded_surface_id.local_surface_id(), device_scale_factor);

    std::vector<Quad> root_quads[3] = { { Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 5.0f / 255.0f }, gfx::Rect(5, 5)),
                                            Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 6.0f / 255.0f }, gfx::Rect(5, 5)) },
        { Quad::SurfaceQuad(SurfaceRange(std::nullopt, embedded_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
              /*stretch_content_to_fill_bounds=*/false),
            Quad::RenderPassQuad(pass_ids[0], gfx::Transform(), true) },
        { Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 7.0f / 255.0f }, gfx::Rect(5, 5)), Quad::RenderPassQuad(pass_ids[1], gfx::Transform(), true) } };
    std::vector<Pass> root_passes
        = { Pass(root_quads[0], pass_ids[0], kSurfaceSize), Pass(root_quads[1], pass_ids[1], kSurfaceSize), Pass(root_quads[2], pass_ids[2], kSurfaceSize) };
    ...
    AggregatedRenderPassId actual_pass_ids[]
        = { aggregated_pass_list[0]->id, aggregated_pass_list[1]->id, aggregated_pass_list[2]->id, aggregated_pass_list[3]->id, aggregated_pass_list[4]->id };
...

    {
        ...
        // else.
        TestPassMatchesExpectations(embedded_passes[0], aggregated_pass_list[1].get());
    }

    {
        ...
        TestQuadMatchesExpectations(embedded_quads[1][0], third_pass_quad_list.ElementAt(0));
        ...
        ASSERT_EQ(DrawQuad::Material::kAggregatedRenderPass, third_pass_quad_list.ElementAt(1)->material);
        const auto* third_pass_render_pass_draw_quad = AggregatedRenderPassDrawQuad::MaterialCast(third_pass_quad_list.ElementAt(1));
        EXPECT_EQ(actual_pass_ids[1], third_pass_render_pass_draw_quad->render_pass_id);
    }

    {
        ...
        // pass.
        TestQuadMatchesExpectations(embedded_quads[2][0], fourth_pass_quad_list.ElementAt(0));
        ...
        ASSERT_EQ(DrawQuad::Material::kAggregatedRenderPass, fourth_pass_quad_list.ElementAt(1)->material);
        const auto* fourth_pass_first_render_pass_draw_quad = AggregatedRenderPassDrawQuad::MaterialCast(fourth_pass_quad_list.ElementAt(1));
        EXPECT_EQ(actual_pass_ids[2], fourth_pass_first_render_pass_draw_quad->render_pass_id);

        // The last quad will be a render pass quad referencing the first pass from
        // the root surface, which is the first pass overall.
        ASSERT_EQ(DrawQuad::Material::kAggregatedRenderPass, fourth_pass_quad_list.ElementAt(2)->material);
        const auto* fourth_pass_second_render_pass_draw_quad = AggregatedRenderPassDrawQuad::MaterialCast(fourth_pass_quad_list.ElementAt(2));
        EXPECT_EQ(actual_pass_ids[0], fourth_pass_second_render_pass_draw_quad->render_pass_id);
    }

    {
        ...
        TestQuadMatchesExpectations(root_quads[2][0], fifth_pass_quad_list.ElementAt(0));
        ...
        // list.
        ASSERT_EQ(DrawQuad::Material::kAggregatedRenderPass, fifth_pass_quad_list.ElementAt(1)->material);
        const auto* fifth_pass_render_pass_draw_quad = AggregatedRenderPassDrawQuad::MaterialCast(fifth_pass_quad_list.ElementAt(1));
        EXPECT_EQ(actual_pass_ids[3], fifth_pass_render_pass_draw_quad->render_pass_id);
    }
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, InvalidSurfaceReference)
{
    ...
    std::vector<Quad> quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(SurfaceId(FrameSinkId(), LocalSurfaceId(0xdeadbeef, 0xdeadbeef, token))), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false),
    ...
    std::vector<Quad> expected_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(5, 5)) };
    std::vector<Pass> expected_passes = { Pass(expected_quads, kSurfaceSize) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, ValidSurfaceReferenceWithNoFrame)
{
    ...
    std::vector<Quad> quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, surface_with_no_frame_id), SkColors::kYellow, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false),
        Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(5, 5)) };
    std::vector<Pass> passes = { Pass(quads, kSurfaceSize) };
    ...
    std::vector<Quad> expected_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kYellow, gfx::Rect(5, 5)),
        Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(5, 5)) };
    std::vector<Pass> expected_passes = { Pass(expected_quads, kSurfaceSize) };
    AggregateAndVerify(expected_passes, { root_surface_id_ });
}
...
{
    const TestSurfaceIdAllocator surface_with_no_frame_id(root_sink_->frame_sink_id());

    std::vector<Quad> quads = { Quad::SurfaceQuad(SurfaceRange(surface_with_no_frame_id), SkColors::kYellow, gfx::Rect(5, 5),
        /*stretch_content_to_fill_bounds=*/false) };
    std::vector<Pass> passes = { Pass(quads, kSurfaceSize) };
...
{
    std::vector<Quad> quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, root_surface_id_), SkColors::kWhite, gfx::Rect(5, 5),
                                    /*stretch_content_to_fill_bounds=*/false),
        Quad::SolidColorQuad(SkColors::kYellow, gfx::Rect(5, 5)) };
    std::vector<Pass> passes = { Pass(quads, kSurfaceSize) };
    ...
    std::vector<Quad> expected_quads = { Quad::SolidColorQuad(SkColors::kYellow, gfx::Rect(5, 5)) };
    std::vector<Pass> expected_passes = { Pass(expected_quads, kSurfaceSize) };
    AggregateAndVerify(expected_passes, { root_surface_id_ });
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, TwoSurfaceCyclicalReference)
{
    ...
    std::vector<Quad> parent_quads = { Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false),
        Quad::SolidColorQuad(SkColors::kCyan, gfx::Rect(5, 5)) };
    std::vector<Pass> parent_passes = { Pass(parent_quads, kSurfaceSize) };

    constexpr float device_scale_factor = 1.0f;
    SubmitCompositorFrame(root_sink_.get(), parent_passes, root_surface_id_.local_surface_id(), device_scale_factor);

    std::vector<Quad> child_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, root_surface_id_), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false),
    ...
    std::vector<Quad> expected_quads = { Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)),
        Quad::SolidColorQuad(SkColors::kMagenta, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kCyan, gfx::Rect(5, 5)) };
    std::vector<Pass> expected_passes = { Pass(expected_quads, kSurfaceSize) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RenderPassIdMapping)
{
    ...
    CompositorRenderPassId child_pass_id[] = { CompositorRenderPassId { 1u }, CompositorRenderPassId { 2u } };
    std::vector<Quad> child_quad[2]
        = { { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) }, { Quad::RenderPassQuad(child_pass_id[0], gfx::Transform(), true) } };
    std::vector<Pass> surface_passes = { Pass(child_quad[0], child_pass_id[0], kSurfaceSize), Pass(child_quad[1], child_pass_id[1], kSurfaceSize) };

    constexpr float device_scale_factor = 1.0f;
    SubmitCompositorFrame(child_sink_.get(), surface_passes, child_surface_id.local_surface_id(), device_scale_factor);

    // Pass IDs from the parent surface may collide with ones from the child.
    CompositorRenderPassId parent_pass_id[] = { CompositorRenderPassId { 3u }, CompositorRenderPassId { 2u } };
    std::vector<Quad> parent_quad[2] = { { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
                                             /*stretch_content_to_fill_bounds=*/false) },
        { Quad::RenderPassQuad(parent_pass_id[0], gfx::Transform(), true) } };
    std::vector<Pass> parent_passes = { Pass(parent_quad[0], parent_pass_id[0], kSurfaceSize), Pass(parent_quad[1], parent_pass_id[1], kSurfaceSize) };
    ...
    AggregatedRenderPassId actual_pass_ids[] = { aggregated_pass_list[0]->id, aggregated_pass_list[1]->id, aggregated_pass_list[2]->id };
    // Make sure the aggregated frame's pass IDs are all unique.
    for (size_t i = 0; i < 3; ++i) {
        for (size_t j = 0; j < i; ++j) {
            EXPECT_NE(actual_pass_ids[j], actual_pass_ids[i]) << "pass ids " << i << " and " << j;
        }
    }

    // Make sure the render pass quads reference the remapped pass IDs.
    DrawQuad* render_pass_quads[] = { aggregated_pass_list[1]->quad_list.front(), aggregated_pass_list[2]->quad_list.front() };
    ASSERT_EQ(render_pass_quads[0]->material, DrawQuad::Material::kAggregatedRenderPass);
    EXPECT_EQ(actual_pass_ids[0], AggregatedRenderPassDrawQuad::MaterialCast(render_pass_quads[0])->render_pass_id);

    ASSERT_EQ(render_pass_quads[1]->material, DrawQuad::Material::kAggregatedRenderPass);
    EXPECT_EQ(actual_pass_ids[1], AggregatedRenderPassDrawQuad::MaterialCast(render_pass_quads[1])->render_pass_id);
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, AggregateSharedQuadStateProperties)
{
    ...
    auto grandchild_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/false);
    auto child_one_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId2, /*is_root=*/false);
    auto child_two_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId3, /*is_root=*/false);
    TestSurfaceIdAllocator grandchild_surface_id(grandchild_support->frame_sink_id());

    constexpr float device_scale_factor = 1.0f;

    auto grandchild_pass = RenderPassBuilder(kSurfaceSize).AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen).SetBlendMode(blend_modes[2]).Build();
    QueuePassAsFrame(std::move(grandchild_pass), grandchild_surface_id.local_surface_id(), device_scale_factor, grandchild_support.get());

    TestSurfaceIdAllocator child_one_surface_id(child_one_support->frame_sink_id());

    auto child_one_pass = RenderPassBuilder(kSurfaceSize)
                              .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                              .SetBlendMode(blend_modes[1])
                              .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(std::nullopt, grandchild_surface_id))
                              .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                              .SetBlendMode(blend_modes[3])
                              .Build();
    QueuePassAsFrame(std::move(child_one_pass), child_one_surface_id.local_surface_id(), device_scale_factor, child_one_support.get());

    TestSurfaceIdAllocator child_two_surface_id(child_two_support->frame_sink_id());

    auto child_two_pass = RenderPassBuilder(kSurfaceSize).AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen).SetBlendMode(blend_modes[5]).Build();
    QueuePassAsFrame(std::move(child_two_pass), child_two_surface_id.local_surface_id(), device_scale_factor, child_two_support.get());

    auto root_pass = RenderPassBuilder(kSurfaceSize)
                         .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                         .SetBlendMode(blend_modes[0])
                         .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(std::nullopt, child_one_surface_id))
                         .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                         .SetBlendMode(blend_modes[4])
                         .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(std::nullopt, child_two_surface_id))
                         .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                         .SetBlendMode(blend_modes[6])
                         .Build();
    QueuePassAsFrame(std::move(root_pass), root_surface_id_.local_surface_id(), device_scale_factor, root_sink_.get());
    ...
    for (auto iter = aggregated_quad_list.cbegin(); iter != aggregated_quad_list.cend(); ++iter) {
        EXPECT_EQ(blend_modes[iter.index()], iter->shared_quad_state->blend_mode) << iter.index();
    }
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, AggregateSharedQuadStateRoundedCornerBounds)
{
    const gfx::MaskFilterInfo kMaskFilterInfoWithFastRoundedCorners1(gfx::RRectF(0, 0, 640, 480, 5));
    const gfx::MaskFilterInfo kMaskFilterInfoWithFastRoundedCorners2(gfx::RRectF(6, 7, 100, 100, 2));
    const gfx::MaskFilterInfo kMaskFilterInfoWithFastRoundedCorners3(gfx::RRectF(41, 50, 600, 100, 7));
    const gfx::MaskFilterInfo kMaskFilterInfoWithRoundedCorners4(gfx::RRectF(0, 1, 10, 10, 3));

    auto child_root_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/false);
    auto child_one_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId2, /*is_root=*/false);
    auto child_two_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    auto child_three_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId3, /*is_root=*/false);
    auto child_four_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId4, /*is_root=*/false);
    auto child_five_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId5, /*is_root=*/false);

    constexpr float device_scale_factor = 1.0f;

    // Setup child five surface.
    TestSurfaceIdAllocator child_five_surface_id(child_five_support->frame_sink_id());

    auto child_five_pass = RenderPassBuilder(kSurfaceSize)
                               .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                               .SetMaskFilter(kMaskFilterInfoWithRoundedCorners4,
                                   /*is_fast_rounded_corner=*/false)
                               .Build();
    QueuePassAsFrame(std::move(child_five_pass), child_five_surface_id.local_surface_id(), device_scale_factor, child_five_support.get());

    // Setup child four surface.
    TestSurfaceIdAllocator child_four_surface_id(child_four_support->frame_sink_id());

    auto child_four_pass = RenderPassBuilder(kSurfaceSize)
                               .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                               .SetMaskFilter(kMaskFilterInfoWithFastRoundedCorners3,
                                   /*is_fast_rounded_corner=*/true)
                               .Build();
    QueuePassAsFrame(std::move(child_four_pass), child_four_surface_id.local_surface_id(), device_scale_factor, child_four_support.get());

    // Setup child three surface.
    TestSurfaceIdAllocator child_three_surface_id(child_three_support->frame_sink_id());

    auto child_three_pass = RenderPassBuilder(kSurfaceSize).AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen).Build();
    QueuePassAsFrame(std::move(child_three_pass), child_three_surface_id.local_surface_id(), device_scale_factor, child_three_support.get());
    ...
    auto child_one_pass = RenderPassBuilder(kSurfaceSize)
                              .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                              .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(std::nullopt, child_three_surface_id))
                              .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(std::nullopt, child_four_surface_id))
                              .Build();
    QueuePassAsFrame(std::move(child_one_pass), child_one_surface_id.local_surface_id(), device_scale_factor, child_one_support.get());
    ...
    auto child_two_pass = RenderPassBuilder(kSurfaceSize)
                              .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                              .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                              .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(std::nullopt, child_five_surface_id))
    ...
    TestSurfaceIdAllocator child_root_surface_id(child_root_support->frame_sink_id());

    auto child_root_pass = RenderPassBuilder(kSurfaceSize)
                               .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(std::nullopt, child_one_surface_id))
                               .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(std::nullopt, child_two_surface_id))
                               .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                               .Build();
    QueuePassAsFrame(std::move(child_root_pass), child_root_surface_id.local_surface_id(), device_scale_factor, child_root_support.get());

    auto root_pass = RenderPassBuilder(kSurfaceSize)
                         .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(std::nullopt, child_root_surface_id))
    ...
    // merged.
    const auto& aggregated_quad_list_of_surface1 = aggregated_pass_list[0]->quad_list;
    EXPECT_THAT(aggregated_quad_list_of_surface1, ElementsAre(HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners3)));

    // There was a mask filter with non-fast rounded corners, which are not
    // allowed to be merged.
    const auto& aggregated_quad_list_of_surface2 = aggregated_pass_list[1]->quad_list;
    EXPECT_THAT(aggregated_quad_list_of_surface2, ElementsAre(HasMaskFilterInfo(kMaskFilterInfoWithRoundedCorners4)));

    // Non-root pass that contains the aggregated render pass.
    const auto& aggregated_quad_list_of_surface3 = aggregated_pass_list[2]->quad_list;
    EXPECT_THAT(aggregated_quad_list_of_surface3,
        ElementsAre(HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners1), HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners1),
            HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners1), HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners1),
            HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners1), HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners2),
...
// embedding pass in.
TEST_F(SurfaceAggregatorValidSurfaceTest, AggregateSharedQuadStateRoundedCornerBoundsClipping)
{
    const gfx::MaskFilterInfo kMaskFilterInfoWithFastRoundedCorners1(gfx::RRectF(0, 0, 900, 800, 2.5));
    const gfx::MaskFilterInfo kMaskFilterInfoWithFastRoundedCorners2(gfx::RRectF(31, 319, 888, 743, 14));
    ...
    struct AuxiliaryTestData {
        ...
        { /*mask_will_merge=*/true, gfx::Rect(31, 319, 70, 80), gfx::Transform::MakeTranslation(0, 100) },
    };
    ...
    for (auto& test_data : kAuxiliaryTestData) {
        ...
        auto child_one_pass = RenderPassBuilder(kSurfaceSize1)
        ...
        auto child_root_pass = RenderPassBuilder(kSurfaceSize1)
                                   .AddSurfaceQuad(gfx::Rect(kSurfaceSize1), SurfaceRange(std::nullopt, child_one_surface_id))
                                   .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
                                   .Build();
        QueuePassAsFrame(std::move(child_root_pass), child_root_surface_id.local_surface_id(), device_scale_factor, child_root_support.get());

        auto root_pass = RenderPassBuilder(kSurfaceSize1)
                             .AddSurfaceQuad(gfx::Rect(kSurfaceSize1), SurfaceRange(std::nullopt, child_root_surface_id))
        ...
        if (test_data.mask_will_merge) {
            ...
            const auto& root_aggregated_quad_list_of_surface = aggregated_pass_list[0]->quad_list;

            gfx::MaskFilterInfo expected_second_mask = kMaskFilterInfoWithFastRoundedCorners2;
            expected_second_mask.ApplyTransform(test_data.parent_target_transform);
            EXPECT_THAT(root_aggregated_quad_list_of_surface,
                ElementsAre(HasMaskFilterInfo(expected_second_mask), HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners1)));
        } else {
            ...
            const auto& aggregated_quad_list_of_surface1 = aggregated_pass_list[0]->quad_list;
            EXPECT_THAT(aggregated_quad_list_of_surface1, ElementsAre(HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners2)));

            const auto& root_aggregated_quad_list_of_surface = aggregated_pass_list[1]->quad_list;
            EXPECT_THAT(root_aggregated_quad_list_of_surface,
                ElementsAre(HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners1), HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners1)));
        }
    }
...
TEST_F(SurfaceAggregatorValidSurfaceTest, AggregateSharedQuadStateRoundedCornerBounds2)
{
    const gfx::MaskFilterInfo kMaskFilterInfoWithFastRoundedCorners1(gfx::RRectF(0, 0, 110, 180, 5));
    const gfx::MaskFilterInfo kMaskFilterInfoWithFastRoundedCorners2(gfx::RRectF(2, 3, 100, 100, 2));
    const gfx::MaskFilterInfo kMaskFilterInfoWithFastRoundedCorners3(gfx::RRectF(4, 5, 50, 50, 20));
    ...
    const auto& non_root_aggregated_quad_list_of_surface = aggregated_pass_list[0]->quad_list;
    EXPECT_THAT(non_root_aggregated_quad_list_of_surface,
        ElementsAre(HasMaskFilterInfo(gfx::MaskFilterInfo()), HasMaskFilterInfo(kMaskFilterInfoWithFastRoundedCorners3),
...
{
    auto middle_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    // Innermost child surface.
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        CompositorRenderPassId child_pass_id[] = { CompositorRenderPassId { 1 }, CompositorRenderPassId { 2 } };
        ...
        std::vector<Pass> child_passes = { Pass(child_quads[0], child_pass_id[0], kSurfaceSize), Pass(child_quads[1], child_pass_id[1], kSurfaceSize) };

        CompositorFrame child_frame = MakeEmptyCompositorFrame();
        AddPasses(&child_frame.render_pass_list, child_passes, &child_frame.metadata.referenced_surfaces);

        auto* child_nonroot_pass = child_frame.render_pass_list[0].get();
        child_nonroot_pass->transform_to_root_target.Translate(8, 0);
        auto* child_nonroot_pass_sqs = child_nonroot_pass->shared_quad_state_list.front();
        ...
        child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), std::move(child_frame));
    }
    ...
    TestSurfaceIdAllocator middle_surface_id(middle_support->frame_sink_id());
    {
        std::vector<Quad> middle_quads
            = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5), /*stretch_content_to_fill_bounds=*/false) };
        std::vector<Pass> middle_passes = {
            Pass(middle_quads, kSurfaceSize),
        };

        CompositorFrame middle_frame = MakeEmptyCompositorFrame();
        AddPasses(&middle_frame.render_pass_list, middle_passes, &middle_frame.metadata.referenced_surfaces);
        ...
        auto* middle_root_pass_sqs = middle_root_pass->shared_quad_state_list.front();
        middle_root_pass_sqs->quad_to_target_transform.Scale(2, 3);

        middle_support->SubmitCompositorFrame(middle_surface_id.local_surface_id(), std::move(middle_frame));
    }

    // Root surface.
    std::vector<Quad> secondary_quads = { Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 1.0f / 255.0f }, gfx::Rect(5, 5)),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, middle_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false) };
    std::vector<Quad> root_quads = { Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 1.0f / 255.0f }, gfx::Rect(5, 5)) };
    std::vector<Pass> root_passes
        = { Pass(secondary_quads, CompositorRenderPassId(1), kSurfaceSize), Pass(root_quads, CompositorRenderPassId(2), kSurfaceSize) };

    CompositorFrame root_frame = MakeEmptyCompositorFrame();
    AddPasses(&root_frame.render_pass_list, root_passes, &root_frame.metadata.referenced_surfaces);

    root_frame.render_pass_list[0]->shared_quad_state_list.front()->quad_to_target_transform.Translate(0, 7);
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(1)->quad_to_target_transform.Translate(0, 10);
    DrawQuad* root_frame_quad = root_frame.render_pass_list[0]->quad_list.ElementAt(1);
    ...
    auto* aggregated_first_pass_sqs = aggregated_pass_list[0]->shared_quad_state_list.front();
    ...
    EXPECT_EQ(expected_aggregated_first_pass_sqs_transform.ToString(), aggregated_first_pass_sqs->quad_to_target_transform.ToString());
    ...
    EXPECT_EQ(expected_first_pass_transform_to_root_target.ToString(), aggregated_pass_list[0]->transform_to_root_target.ToString());
    ...
    for (auto iter = aggregated_pass_list[1]->quad_list.cbegin(); iter != aggregated_pass_list[1]->quad_list.cend(); ++iter) {
        EXPECT_EQ(expected_root_pass_quad_transforms[iter.index()].ToString(), iter->shared_quad_state->quad_to_target_transform.ToString()) << iter.index();
    }

    EXPECT_TRUE(aggregated_pass_list[1]->shared_quad_state_list.ElementAt(1)->clip_rect.has_value());
    ...
    EXPECT_EQ(gfx::Rect(0, 13, 8, 12), aggregated_pass_list[1]->shared_quad_state_list.ElementAt(1)->clip_rect);
}
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, FallbackSurfaceReference)
{
    auto primary_child_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/false);
    ...
    std::vector<Quad> fallback_child_quads = { Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(fallback_size)) };
    std::vector<Pass> fallback_child_passes = { Pass(fallback_child_quads, fallback_size) };

    // Submit a CompositorFrame to the fallback Surface containing a red
    // SolidColorDrawQuad.
    constexpr float device_scale_factor_1 = 1.0f;
    constexpr float device_scale_factor_2 = 2.0f;
    SubmitCompositorFrame(fallback_child_support.get(), fallback_child_passes, fallback_child_surface_id.local_surface_id(), device_scale_factor_2);
    ...
    std::vector<Quad> root_quads = { Quad::SurfaceQuad(SurfaceRange(fallback_child_surface_id, primary_child_surface_id), SkColors::kWhite, surface_quad_rect,
    ...
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    primary_child_support->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    fallback_child_support->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());

    SubmitCompositorFrame(root_sink_.get(), root_passes, root_surface_id_.local_surface_id(), device_scale_factor_1);
    ...
    Quad right_gutter_quad = Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 0, 7, 15));
    Quad bottom_gutter_quad = Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(0, 5, 5, 10));
    Quad render_pass_quad = Quad::RenderPassQuad(CompositorRenderPassId { 2 }, gfx::Transform(), true);
    Quad fallback_quad = Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(fallback_size));
    ...
    std::vector<Quad> expected_quads1 { right_gutter_quad, bottom_gutter_quad, AllowMerge() ? fallback_quad : render_pass_quad };
    ...
    EXPECT_CALL(aggregated_damage_callback,
        OnAggregatedDamage(fallback_child_surface_id.local_surface_id(), fallback_size, gfx::Rect(fallback_size), next_display_time()))
        .Times(1);
    EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(primary_child_surface_id.local_surface_id(), _, _, _)).Times(0);
    // The whole root surface should be damaged because this is the first
    // aggregation.
    EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, gfx::Rect(kSurfaceSize), next_display_time()))
        .Times(1);

    // The primary_surface will not be listed in previously contained surfaces.
    AggregateAndVerify(expected_passes1, { root_surface_id_, fallback_child_surface_id });
    ...
    EXPECT_CALL(
        aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, surface_quad_rect, testing::A<base::TimeTicks>()))
        .Times(1);

    std::vector<Quad> expected_quads2 { right_gutter_quad, bottom_gutter_quad, AllowMerge() ? fallback_quad : render_pass_quad };
    ...
    AggregateAndVerify(
        expected_passes2, { root_surface_id_, SurfaceId(fallback_child_support->frame_sink_id(), fallback_child_surface_id.local_surface_id()) });

    testing::Mock::VerifyAndClearExpectations(&aggregated_damage_callback);

    std::vector<Quad> primary_child_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
    constexpr gfx::Size primary_surface_size(5, 5);
    std::vector<Pass> primary_child_passes = { Pass(primary_child_quads, primary_surface_size) };

    // Submit a CompositorFrame to the primary Surface containing a green
    // SolidColorDrawQuad.
    SubmitCompositorFrame(primary_child_support.get(), primary_child_passes, primary_child_surface_id.local_surface_id(), device_scale_factor_2);
    ...
    std::vector<Quad> expected_quads3 { AllowMerge() ? primary_quad : render_pass_quad };
    ...
    EXPECT_CALL(aggregated_damage_callback,
        OnAggregatedDamage(primary_child_surface_id.local_surface_id(), primary_surface_size, gfx::Rect(primary_surface_size), next_display_time()))
        .Times(1);
    EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(fallback_child_surface_id.local_surface_id(), _, _, _)).Times(0);
    ...
    // has DSF=2.
    gfx::Rect scaled_primary_surface_rect = gfx::ScaleToEnclosingRect(gfx::Rect(primary_surface_size), 0.5, 0.5);
    EXPECT_CALL(
        aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, scaled_primary_surface_rect, next_display_time()))
        .Times(1);

    AggregateAndVerify(expected_passes3, { root_surface_id_, primary_child_surface_id });
}

// Tests that damage rects are aggregated correctly when surfaces change.
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, AggregateDamageRect)
{
    // Add a callback for when the surface is damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());

    auto parent_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);

    CompositorRenderPassList child_passes;
    child_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                               .AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kGreen)
    ...
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        CompositorFrame child_frame = MakeCompositorFrame(CopyRenderPasses(child_passes));
    ...
    CompositorFrame parent_surface_frame
        = CompositorFrameBuilder()
              .AddRenderPass(RenderPassBuilder(kSurfaceSize)
                                 .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(std::nullopt, child_surface_id), { .allow_merge = AllowMerge() }))
              .Build();

    TestSurfaceIdAllocator parent_surface_id(parent_support->frame_sink_id());
    parent_support->SubmitCompositorFrame(parent_surface_id.local_surface_id(), std::move(parent_surface_frame));

    CompositorRenderPassList root_passes;
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                              .AddSurfaceQuad(gfx::Rect(kSurfaceSize), SurfaceRange(std::nullopt, parent_surface_id), { .allow_merge = AllowMerge() })
                              .SetQuadToTargetTranslation(0, 10)
                              .SetDamageRect(gfx::Rect(5, 5, 95, 95))
                              .Build());
    root_passes.push_back(
        RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize).AddRenderPassQuad(gfx::Rect(kSurfaceSize), CompositorRenderPassId { 1 }).Build());

    {
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), MakeCompositorFrame(CopyRenderPasses(root_passes)));
    }
    ...
    // Damage rect for first aggregation should contain entire root surface.
    {
        ...
        EXPECT_CALL(
            aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, expected_root_damage, next_display_time()));
        auto aggregated_frame = AggregateFrame(root_surface_id_);
        testing::Mock::VerifyAndClearExpectations(&aggregated_damage_callback);
        const auto& aggregated_pass_list = aggregated_frame.render_pass_list;
...

    {
        ...
        EXPECT_CALL(
            aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, expected_damage_rect, next_display_time()));
        auto aggregated_frame = AggregateFrame(root_surface_id_);
        const auto& aggregated_pass_list = aggregated_frame.render_pass_list;
        ASSERT_EQ(expected_num_passes_after_aggregation, aggregated_pass_list.size());
    ...
    {
        CompositorFrame root_frame = MakeCompositorFrame(CopyRenderPasses(root_passes));
    ...
    {
        EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(_, _, _, _)).Times(0);
        auto aggregated_frame = AggregateFrame(root_surface_id_);
        const auto& aggregated_pass_list = aggregated_frame.render_pass_list;
        ASSERT_EQ(expected_num_passes_after_aggregation, aggregated_pass_list.size());
    ...
    // marked as damaged.
    {
        ...
        const auto& aggregated_pass_list = aggregated_frame.render_pass_list;
        ASSERT_EQ(expected_num_passes_after_aggregation, aggregated_pass_list.size());
        EXPECT_TRUE(aggregated_pass_list.back()->damage_rect.Contains(gfx::Rect(kSurfaceSize)));
    }
}

// Tests that damage rects are aggregated correctly when surfaces stretch to
// fit and device size is less than 1.
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, AggregateDamageRectWithSquashToFit)
{
    // Add a callback for when the surface is damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    ...
    auto parent_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    std::vector<Quad> child_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
    std::vector<Pass> child_passes = { Pass(child_quads, CompositorRenderPassId { 1 }, gfx::Size(kSurfaceSize)) };
    ...
    std::vector<Quad> parent_surface_quads = { Quad::SurfaceQuad(
        SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(kSurfaceSize), /*stretch_content_to_fill_bounds=*/false, AllowMerge()) };
    std::vector<Pass> parent_surface_passes = { Pass(parent_surface_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };

    // Parent surface is only used to test if the transform is applied correctly
    // to the child surface's damage.
    CompositorFrame parent_surface_frame = MakeEmptyCompositorFrame();
    AddPasses(&parent_surface_frame.render_pass_list, parent_surface_passes, &parent_surface_frame.metadata.referenced_surfaces);
    ...
    CompositorFrame root_frame
        = CompositorFrameBuilder()
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                                 .AddSurfaceQuad(gfx::Rect(50, 50), SurfaceRange(std::nullopt, parent_surface_id),
                                     { .stretch_content_to_fill_bounds = true, .allow_merge = AllowMerge() }))
              .AddRenderPass(
                  RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize).AddRenderPassQuad(gfx::Rect(kSurfaceSize), CompositorRenderPassId { 1 }))
              .Build();
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, AggregateDamageRectWithStretchToFit)
{
    // Add a callback for when the surface is damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    ...
    auto parent_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    std::vector<Quad> child_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
    std::vector<Pass> child_passes = { Pass(child_quads, CompositorRenderPassId { 1 }, gfx::Size(100, 100)) };
    ...
    CompositorFrame root_frame
        = CompositorFrameBuilder()
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                                 .AddSurfaceQuad(gfx::Rect(200, 200), SurfaceRange(std::nullopt, parent_surface_id),
                                     { .stretch_content_to_fill_bounds = true, .allow_merge = AllowMerge() }))
              .AddRenderPass(
                  RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize).AddRenderPassQuad(gfx::Rect(kSurfaceSize), CompositorRenderPassId { 1 }))
              .Build();
...
{
    {
        std::vector<Quad> root_render_pass_quads = { Quad::SolidColorQuad({ 1.0, 0.0, 1.0, 1.0f / 255.0f }, gfx::Rect(5, 5)) };

        std::vector<Pass> root_passes = { Pass(root_render_pass_quads, CompositorRenderPassId { 2 }, kSurfaceSize) };
...

    {
        ...
        // Damage rect for first aggregation should contain entire root surface.
        EXPECT_TRUE(aggregated_pass_list[0]->damage_rect.Contains(gfx::Rect(kSurfaceSize)));
    }
    ...
    TestSurfaceIdAllocator second_root_surface_id(root_sink_->frame_sink_id());
    {
        ...
        root_sink_->SubmitCompositorFrame(second_root_surface_id.local_surface_id(), std::move(root_frame));
    }
    {
...
TEST_F(SurfaceAggregatorValidSurfaceTest, SurfaceDamageSameFrameSinkId)
{
...

    {
        CompositorFrame frame
            = CompositorFrameBuilder()
                  .AddRenderPass(RenderPassBuilder(kSurfaceSize).AddSurfaceQuad(gfx::Rect(5, 5), SurfaceRange(fallback_surface_id, primary_surface_id)))
                  .Build();
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame));
    }
    ...
    EXPECT_FALSE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id1)));

    // |id2| is the fallback id so it should damage the display.
    EXPECT_TRUE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id2)));

    // |id3| is between fallback and primary so it should damage the display.
    EXPECT_TRUE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id3)));

    // |id4| is the primary id so it should damage the display.
    EXPECT_TRUE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id4)));

    // |id5| is newer than the primary surface so it shouldn't damage display.
    EXPECT_FALSE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id5)));

    // This FrameSinkId is not embedded at all so it shouldn't damage the display.
    EXPECT_FALSE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId3, id3)));
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, SurfaceDamageDifferentFrameSinkId)
{
    ...
    EXPECT_TRUE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId2, id3)));

    // |id4| is the primary id so it should damage the display.
    EXPECT_TRUE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId2, id4)));

    // This FrameSinkId is not embedded at all so it shouldn't damage the display.
    EXPECT_FALSE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId3, id4)));
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, SurfaceDamagePrimarySurfaceOnly)
{
    ...
    CompositorFrame frame = CompositorFrameBuilder()
                                .AddRenderPass(RenderPassBuilder(kSurfaceSize).AddSurfaceQuad(gfx::Rect(5, 5), SurfaceRange(std::nullopt, primary_surface_id)))
                                .Build();
    root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame));
    ...
    EXPECT_TRUE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id1)));

    // |id2| is the primary id so it should damage the display.
    EXPECT_TRUE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id2)));

    // |id3| is after the primary id so it shouldn't damage the display.
    EXPECT_FALSE(aggregator_.CheckForDisplayDamage(SurfaceId(kArbitraryFrameSinkId1, id3)));
...
TEST_F(SurfaceAggregatorValidSurfaceTest, SurfaceDamagePrimaryAndFallbackEqual)
{
    auto embedded_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/true);
    ParentLocalSurfaceIdAllocator allocator;
    allocator.GenerateId();
    ...
    {
        CompositorFrame frame
            = CompositorFrameBuilder().AddRenderPass(RenderPassBuilder(kSurfaceSize).AddSurfaceQuad(gfx::Rect(5, 5), SurfaceRange(surface_id))).Build();
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(frame));
    }
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, RPDQBackdropFilterCacheFlagTest1)
{
    // Add callbacks for when the root and child surfaces are damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    child_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    ...
    std::vector<Pass> child_passes = { Pass(child_quads, CompositorRenderPassId { 1 }, child_surface_size) };
    ...
    CompositorRenderPassList root_passes;
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 1 }, child_pass_rect).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kRed).Build());
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize)
                              .SetDamageRect(gfx::Rect())
                              // We will verify the correctness of the |intersects_damage_under|
                              // flag on this quad.
                              .AddRenderPassQuad(child_pass_rect, CompositorRenderPassId { 1 }, { .intersects_damage_under = false })
                              .SetQuadToTargetTranslation(20, 30)
                              .AddSurfaceQuad(gfx::Rect(90, 90), SurfaceRange(child_surface_id), { .allow_merge = AllowMerge() })
                              .Build());
    ...
    // First aggregation.
    {
        EXPECT_CALL(
            aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, gfx::Rect(kSurfaceSize), next_display_time()));
        EXPECT_CALL(aggregated_damage_callback,
            OnAggregatedDamage(child_surface_id.local_surface_id(), child_surface_size, gfx::Rect(child_surface_size), next_display_time()));
        auto aggregated_frame = AggregateFrame(root_surface_id_);
        testing::Mock::VerifyAndClearExpectations(&aggregated_damage_callback);
        const auto& aggregated_pass_list = aggregated_frame.render_pass_list;
        ASSERT_EQ(expected_num_passes_after_aggregation, aggregated_pass_list.size());
        ...
        const auto* rp_quad = AggregatedRenderPassDrawQuad::MaterialCast(quad_to_test);
    ...
    {
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), MakeCompositorFrame(std::move(root_passes)));
        EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), _, _, _)).Times(0);

        // No damage is expected from the child surface.
        EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(child_surface_id.local_surface_id(), _, _, _)).Times(0);
    ...
    // |intersects_damage_under|.
    {
        ...
        EXPECT_CALL(
            aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, expected_damage_rect, next_display_time()));
        EXPECT_CALL(
            aggregated_damage_callback, OnAggregatedDamage(child_surface_id.local_surface_id(), child_surface_size, expected_damage_rect, next_display_time()));
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, RPDQBackdropFilterCacheFlagTest2)
{
    auto second_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/false);

    // Add callbacks for when the surfaces are damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    child_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    second_support->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    ...
    child_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 1 }, child_surface_size).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kGreen).Build());
    child_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 2 }, child_surface_size)
                               .AddRenderPassQuad(gfx::Rect(child_surface_size), CompositorRenderPassId { 1 }, { .intersects_damage_under = false })
                               .SetDamageRect(gfx::Rect())
                               .Build());

    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());

    child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), MakeCompositorFrame(CopyRenderPasses(child_passes)));
    ...
    std::vector<Quad> second_surface_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };

    std::vector<Pass> second_surface_passes = { Pass(second_surface_quads, CompositorRenderPassId { 1 }, second_surface_size) };
    ...
    TestSurfaceIdAllocator second_surface_id(second_support->frame_sink_id());
    {
        ...
        AddPasses(&second_surface_frame.render_pass_list, second_surface_passes, &second_surface_frame.metadata.referenced_surfaces);

        second_support->SubmitCompositorFrame(second_surface_id.local_surface_id(), std::move(second_surface_frame));
    }
    ...
    // the second surface.
    std::vector<Quad> render_pass_quads = { // The |allow_merge| flag of the surface quad would be set to true/false
        // according to the parameter of the test.
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(90, 90),
            /*stretch_content_to_fill_bounds=*/false, AllowMerge()),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, second_surface_id), SkColors::kWhite, gfx::Rect(second_surface_size),
            /*stretch_content_to_fill_bounds=*/false,
            /*allow_merge=*/false)
    };

    std::vector<Pass> root_passes { Pass(render_pass_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };

    CompositorFrame root_frame = MakeEmptyCompositorFrame();
    AddPasses(&root_frame.render_pass_list, root_passes, &root_frame.metadata.referenced_surfaces);
    root_frame.render_pass_list.back()->shared_quad_state_list.front()->quad_to_target_transform.Translate(20, 30);
    root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(root_frame));
    ...
    // First aggregation.
    {
        ...
        EXPECT_CALL(aggregated_damage_callback,
            OnAggregatedDamage(child_surface_id.local_surface_id(), child_surface_size, expected_child_damage_rect, next_display_time()));
        EXPECT_CALL(aggregated_damage_callback,
            OnAggregatedDamage(second_surface_id.local_surface_id(), second_surface_size, gfx::Rect(second_surface_size), next_display_time()));
        ...
        // entire root rect.
        EXPECT_EQ(gfx::Rect(kSurfaceSize), aggregated_pass_list.back()->damage_rect);
        const auto* quad_to_test = aggregated_pass_list[AllowMerge() ? 2 : 1]->quad_list.front();
        const auto* rp_quad = AggregatedRenderPassDrawQuad::MaterialCast(quad_to_test);
    ...
    {
        child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), MakeCompositorFrame(std::move(child_passes)));
        EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), _, _, _)).Times(0);

        // There should be no damage on any surface.
        EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(child_surface_id.local_surface_id(), _, _, _)).Times(0);

        EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(second_surface_id.local_surface_id(), _, _, _)).Times(0);
    ...
    // |intersects_damage_under|.
    {
        CompositorFrame second_surface_frame = MakeEmptyCompositorFrame();
        AddPasses(&second_surface_frame.render_pass_list, second_surface_passes, &second_surface_frame.metadata.referenced_surfaces);
        auto* second_surface_root_pass = second_surface_frame.render_pass_list[0].get();
        ...
        EXPECT_CALL(aggregated_damage_callback,
            OnAggregatedDamage(second_surface_id.local_surface_id(), second_surface_size, expected_damage_rect, next_display_time()));
        auto aggregated_frame = AggregateFrame(root_surface_id_);
        const auto& aggregated_pass_list = aggregated_frame.render_pass_list;
        ASSERT_EQ(expected_num_passes_after_aggregation, aggregated_pass_list.size());
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, RPDQCanUseBackdropFilterCache)
{
    auto parent_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    ...
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    child_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    parent_support->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    ...
    std::vector<Quad> parent_surface_quads = { Quad::SurfaceQuad(
        SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(kSurfaceSize), /*stretch_content_to_fill_bounds=*/false, AllowMerge()) };
    std::vector<Pass> parent_surface_passes = { Pass(parent_surface_quads, CompositorRenderPassId { 1 }, parent_surface_size) };
    ...
    std::vector<Quad> render_pass_1_quads = { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)) };

    std::vector<Quad> render_pass_2_quads = { // Set the |intersects_damage_under| of this CompositorRenderPassDrawQuad
        ...
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, parent_surface_id), SkColors::kWhite, gfx::Rect(kSurfaceSize),
            /*stretch_content_to_fill_bounds=*/false, AllowMerge())
    };

    std::vector<Pass> root_passes { Pass(render_pass_1_quads, CompositorRenderPassId { 1 }, gfx::Size(50, 50)),
...

    {
        ...
        root_frame.render_pass_list[0]->shared_quad_state_list.front()->quad_to_target_transform.Translate(0, 10);
...

    {
        ...
        EXPECT_CALL(aggregated_damage_callback,
            OnAggregatedDamage(child_surface_id.local_surface_id(), child_surface_size, gfx::Rect(child_surface_size), next_display_time()));
        EXPECT_CALL(aggregated_damage_callback,
            OnAggregatedDamage(parent_surface_id.local_surface_id(), parent_surface_size, gfx::Rect(parent_surface_size), next_display_time()));
        auto aggregated_frame = AggregateFrame(root_surface_id_);
        testing::Mock::VerifyAndClearExpectations(&aggregated_damage_callback);
        const auto& aggregated_pass_list = aggregated_frame.render_pass_list;
    ...
    // (true).
    {
        ...
        EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), _, _, _)).Times(0);
        EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(child_surface_id.local_surface_id(), _, _, _)).Times(0);
        EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(parent_surface_id.local_surface_id(), _, _, _)).Times(0);
        auto aggregated_frame = AggregateFrame(root_surface_id_);
        const auto& aggregated_pass_list = aggregated_frame.render_pass_list;
        ASSERT_EQ(expected_num_passes_after_aggregation, aggregated_pass_list.size());
    ...
    // |intersects_damage_under| to become false.
    {
        ...
        EXPECT_CALL(
            aggregated_damage_callback, OnAggregatedDamage(child_surface_id.local_surface_id(), child_surface_size, expected_damage_rect, next_display_time()));
        EXPECT_CALL(aggregated_damage_callback,
            OnAggregatedDamage(parent_surface_id.local_surface_id(), parent_surface_size, expected_damage_rect, next_display_time()));

        auto aggregated_frame = AggregateFrame(root_surface_id_);
        const auto& aggregated_pass_list = aggregated_frame.render_pass_list;
        ASSERT_EQ(expected_num_passes_after_aggregation, aggregated_pass_list.size());
        EXPECT_EQ(expected_damage_rect.ToString(), aggregated_pass_list.back()->damage_rect.ToString());
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, RPDQCanUseBackdropFilterCacheTestWithMultiplyEmbeddedSurface)
{
    // Add a callback for when the surface is damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    ...
    CompositorRenderPassList root_passes;
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 4 }, gfx::Rect(5, 5)).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kRed).Build());
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 3 }, kSurfaceSize)
                              .AddSurfaceQuad(gfx::Rect(child_surface_size), SurfaceRange(std::nullopt, child_surface_id), { .allow_merge = AllowMerge() })
                              .Build());
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize)
                              ...
                              .Build());
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                              .AddRenderPassQuad(gfx::Rect(5, 5), CompositorRenderPassId { 4 }, { .intersects_damage_under = false })
    ...
    size_t expected_num_passes_after_aggregation = AllowMerge() ? 4u : 5u;
    {
        ...
        auto aggregated_frame = AggregateFrame(root_surface_id_);
        testing::Mock::VerifyAndClearExpectations(&aggregated_damage_callback);
        ASSERT_EQ(expected_num_passes_after_aggregation, aggregated_frame.render_pass_list.size());
        ...
        // (2,2 5x5).
        const auto* quad_to_test = aggregated_frame.render_pass_list.back()->quad_list.front();
        const auto* rp_quad = AggregatedRenderPassDrawQuad::MaterialCast(quad_to_test);
        EXPECT_TRUE(rp_quad->intersects_damage_under);
    }
...

    {
        ...
        AddPasses(&child_frame_2.render_pass_list, child_passes, &child_frame_2.metadata.referenced_surfaces);

        child_frame_2.render_pass_list.back()->damage_rect = gfx::Rect(10, 10, 10, 10);

        child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), std::move(child_frame_2));
        ...
        EXPECT_CALL(
            aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, expected_damage_rect, next_display_time()));
        EXPECT_CALL(aggregated_damage_callback,
            OnAggregatedDamage(child_surface_id.local_surface_id(), child_surface_size, gfx::Rect(expected_child_damage_rect), next_display_time()))
            .Times(2);
        auto aggregated_frame_2 = AggregateFrame(root_surface_id_);
        testing::Mock::VerifyAndClearExpectations(&aggregated_damage_callback);

        const auto* quad_to_test = aggregated_frame_2.render_pass_list.back()->quad_list.front();
        const auto* rp_quad = AggregatedRenderPassDrawQuad::MaterialCast(quad_to_test);
        EXPECT_FALSE(rp_quad->intersects_damage_under);
    }

    {
        ...
        // tested is now (12,12)
        gfx::Transform& tr = const_cast<gfx::Transform&>(root_passes.back()->quad_list.ElementAt(0)->shared_quad_state->quad_to_target_transform);
        tr.Translate(10, 10);
        CompositorFrame root_frame = MakeCompositorFrame(CopyRenderPasses(root_passes));
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, PixelMovingBackdropFilterDamageExpansion)
{
    // Add callbacks for when the surfaces are damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    ...
    CompositorFrame child_frame
        = CompositorFrameBuilder()
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, child_surface_size).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kGreen))
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 2 }, child_surface_size)
                                 .AddRenderPassQuad(gfx::Rect(child_surface_size), CompositorRenderPassId { 1 })
                                 .AddBackdropFilter(cc::FilterOperation::CreateBlurFilter(5)))
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 3 }, child_surface_size)
                                 .AddRenderPassQuad(gfx::Rect(child_surface_size), CompositorRenderPassId { 2 })
                                 .SetQuadToTargetTranslation(20, 30))
              .Build();
    ...
    root_render_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                                     .SetDamageRect(gfx::Rect(0, 0, 10, 20))
                                     .AddSurfaceQuad(gfx::Rect(90, 90), SurfaceRange(child_surface_id), { .allow_merge = AllowMerge() })
                                     .SetQuadToTargetTranslation(5, 5)
                                     .Build());

    root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), MakeCompositorFrame(CopyRenderPasses(root_render_passes)));
    ...
    {
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), MakeCompositorFrame(std::move(root_render_passes)));
        EXPECT_CALL(
            aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, gfx::Rect(0, 0, 10, 20), next_display_time()));
        ...
        const auto& aggregated_pass_list = aggregated_frame.render_pass_list;
        ASSERT_EQ(expected_num_passes_after_aggregation, aggregated_pass_list.size());
        EXPECT_EQ(gfx::Rect(0, 0, 10, 20), aggregated_pass_list.back()->damage_rect);
    }
}
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, PixelMovingBackdropFilterDamageExpansionWithSurfaceStretch)
{
    // Add callbacks for when the surfaces are damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
...

    {
        CompositorFrame root_frame = MakeEmptyCompositorFrame();
        AddPasses(&root_frame.render_pass_list, root_passes, &root_frame.metadata.referenced_surfaces);
        root_frame.render_pass_list.back()->shared_quad_state_list.front()->quad_to_target_transform.Translate(5, 5);
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(root_frame));
    }
    ...
    // First aggregation.
    {
        ...
        EXPECT_CALL(
            aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, expected_root_damage_rect, next_display_time()));
...
TEST_F(SurfaceAggregatorPartialSwapTest, ExpandByTargetDamage)
{
    ...
    // The child surface has one quad.
    {
        ...
        std::vector<Quad> child_quads1 = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
        std::vector<Pass> child_passes = { Pass(child_quads1, child_pass_id, gfx::Rect(5, 5)) };

        CompositorRenderPassList child_pass_list;
        std::vector<SurfaceRange> referenced_surfaces;
        AddPasses(&child_pass_list, child_passes, &referenced_surfaces);

        SubmitPassListAsFrame(child_sink_.get(), child_surface_id.local_surface_id(), &child_pass_list, std::move(referenced_surfaces), device_scale_factor);
    }

    {
        std::vector<Quad> root_quads = { Quad::SurfaceQuad(
            SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(kSurfaceSize), /*stretch_content_to_fill_bounds=*/false) };
        ...
        // produced.
        SubmitPassListAsFrame(root_sink_.get(), root_surface_id_.local_surface_id(), &root_pass_list, std::move(referenced_surfaces), device_scale_factor);
    }
...

    {
        ...
        ASSERT_EQ(1u, aggregated_pass_list.size());
        EXPECT_EQ(gfx::Rect(10, 10, 2, 2), aggregated_pass_list.back()->damage_rect);
    }

    // This pass has damage that does not intersect the quad in the child
    // surface.
    {
        std::vector<Quad> root_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(kSurfaceSize), false) };
...
public:
    SurfaceAggregatorWithResourcesTest()
        : SurfaceAggregatorTest(SurfaceAggregator::ExtraPassForReadbackOption::kNone, false)
    {
    ...
    {
        BeginFrameArgs args = CreateBeginFrameArgsForTesting(BEGINFRAME_FROM_HERE, 0, id);
        support->OnBeginFrame(args);
    }
};

CompositorFrame BuildCompositorFrameWithResources(const std::vector<ResourceId>& resource_ids, bool valid, SurfaceId child_id)
{
    CompositorFrame frame = MakeEmptyCompositorFrame();
    auto pass = CompositorRenderPass::Create();
    pass->SetNew(CompositorRenderPassId { 1 }, gfx::Rect(0, 0, 20, 20), gfx::Rect(), gfx::Transform());
    ...
    if (child_id.is_valid()) {
        ...
        surface_quad->SetNew(sqs, gfx::Rect(0, 0, 1, 1), gfx::Rect(0, 0, 1, 1), SurfaceRange(std::nullopt, child_id), SkColors::kWhite,
            /*stretch_content_to_fill_bounds=*/false);
    }

    for (ResourceId resource_id : resource_ids) {
        auto resource
            = TransferableResource::MakeSoftwareSharedBitmap(SharedBitmap::GenerateId(), gpu::SyncToken(), gfx::Size(1, 1), SinglePlaneFormat::kRGBA_8888);
        ...
        gfx::ProtectedVideoType protected_video_type = gfx::ProtectedVideoType::kClear;
        quad->SetAll(sqs, rect, visible_rect, needs_blending, resource_id, gfx::Size(), premultiplied_alpha, uv_top_left, uv_bottom_right, background_color,
            flipped, nearest_neighbor, secure_output_only, protected_video_type);
    }
...
void SubmitCompositorFrameWithResources(
    const std::vector<ResourceId>& resource_ids, bool valid, SurfaceId child_id, CompositorFrameSinkSupport* support, SurfaceId surface_id)
{
    auto frame = BuildCompositorFrameWithResources(resource_ids, valid, child_id);
    support->SubmitCompositorFrame(surface_id.local_surface_id(), std::move(frame));
}
...
TEST_F(SurfaceAggregatorWithResourcesTest, TakeResourcesOneSurface)
{
    ...
    std::vector<ResourceId> ids = { ResourceId(11), ResourceId(12), ResourceId(13) };
    SubmitCompositorFrameWithResources(ids, true, SurfaceId(), root_sink_.get(), surface_id);
    ...
    SubmitCompositorFrameWithResources({}, true, SurfaceId(), root_sink_.get(), surface_id);
    ...
    EXPECT_THAT(returned_ids, testing::WhenSorted(testing::ElementsAreArray(ids)));
}
...
TEST_F(SurfaceAggregatorWithResourcesTest, ReturnResourcesAsSurfacesChange)
{
    ...
    std::vector<ResourceId> ids = { ResourceId(11), ResourceId(12), ResourceId(13) };
    SubmitCompositorFrameWithResources(ids, true, SurfaceId(), root_sink_.get(), surface_id1);
    ...
    SubmitCompositorFrameWithResources({}, true, SurfaceId(), root_sink_.get(), surface_id2);
...
TEST_F(SurfaceAggregatorWithResourcesTest, TakeInvalidResources)
{
    ...
    CompositorFrame frame = CompositorFrameBuilder().AddDefaultRenderPass().AddTransferableResource(resource).Build();
...
TEST_F(SurfaceAggregatorWithResourcesTest, TwoSurfaces)
{
    ...
    auto support1 = std::make_unique<CompositorFrameSinkSupport>(&client, &manager_, FrameSinkId(3, 1), /*is_root=*/false);
    auto support2 = std::make_unique<CompositorFrameSinkSupport>(&client, &manager_, FrameSinkId(4, 2), /*is_root=*/false);
    ...
    std::vector<ResourceId> ids = { ResourceId(11), ResourceId(12), ResourceId(13) };
    SubmitCompositorFrameWithResources(ids, true, SurfaceId(), support1.get(), surface1_id);
    std::vector<ResourceId> ids2 = { ResourceId(14), ResourceId(15), ResourceId(16) };
    SubmitCompositorFrameWithResources(ids2, true, SurfaceId(), support2.get(), surface2_id);

    auto frame = AggregateFrame(surface1_id);

    SubmitCompositorFrameWithResources({}, true, SurfaceId(), support1.get(), surface1_id);
...
TEST_F(SurfaceAggregatorWithResourcesTest, InvalidChildSurface)
{
    ...
    std::vector<ResourceId> ids = { ResourceId(14), ResourceId(15), ResourceId(16) };
    SubmitCompositorFrameWithResources(ids, true, SurfaceId(), child_support.get(), child_surface_id);

    std::vector<ResourceId> ids2 = { ResourceId(17), ResourceId(18), ResourceId(19) };
    SubmitCompositorFrameWithResources(ids2, false, child_surface_id, middle_support.get(), middle_surface_id);

    std::vector<ResourceId> ids3 = { ResourceId(20), ResourceId(21), ResourceId(22) };
    SubmitCompositorFrameWithResources(ids3, true, middle_surface_id, root_sink_.get(), root_surface_id);
    ...
    SubmitCompositorFrameWithResources(ids2, true, child_surface_id, middle_support.get(), middle_surface_id);
...
{
    auto support1 = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, FrameSinkId(3, 1), /*is_root=*/false);
    auto support2 = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, FrameSinkId(4, 2), /*is_root=*/false);
    ...
    EXPECT_EQ(DrawQuad::Material::kTextureContent, render_pass->quad_list.back()->material);

    {
        ...
        surface_quad->SetNew(sqs, gfx::Rect(0, 0, 1, 1), gfx::Rect(0, 0, 1, 1), SurfaceRange(std::nullopt, surface1_id), SkColors::kWhite,
            /*stretch_content_to_fill_bounds=*/false);
        pass->copy_requests.push_back(CopyOutputRequest::CreateStubForTesting());

        CompositorFrame compositor_frame = CompositorFrameBuilder().AddRenderPass(std::move(pass)).Build();

        support2->SubmitCompositorFrame(local_frame2_id, std::move(compositor_frame));
    }
    ...
    EXPECT_EQ(DrawQuad::Material::kSolidColor, render_pass->quad_list.back()->material);
    ...
    EXPECT_EQ(DrawQuad::Material::kTextureContent, render_pass->quad_list.front()->material);
...
TEST_F(SurfaceAggregatorValidSurfaceTest, ColorSpaceTestWin)
{
    ...
    std::vector<Quad> quads[2] = { { Quad::SolidColorQuad(SkColors::kWhite, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kLtGray, gfx::Rect(5, 5)) },
        { Quad::SolidColorQuad(SkColors::kGray, gfx::Rect(5, 5)), Quad::TransparentSolidColorQuad(SkColors::kDkGray, gfx::Rect(5, 5), 0.5) } };

    gfx::DisplayColorSpaces display_color_spaces(gfx::ColorSpace::CreateSRGB());
    display_color_spaces.SetOutputColorSpaceAndBufferFormat(gfx::ContentColorUsage::kWideColorGamut, false /* needs_alpha */,
        gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT2020, gfx::ColorSpace::TransferID::SRGB), gfx::BufferFormat::RGBA_8888);
    display_color_spaces.SetOutputColorSpaceAndBufferFormat(
        gfx::ContentColorUsage::kWideColorGamut, true /* needs_alpha */, gfx::ColorSpace::CreateSRGBLinear(), gfx::BufferFormat::RGBA_8888);
    display_color_spaces.SetOutputColorSpaceAndBufferFormat(
        gfx::ContentColorUsage::kHDR, false /* needs_alpha */, gfx::ColorSpace::CreateHDR10(), gfx::BufferFormat::BGRA_1010102);
    display_color_spaces.SetOutputColorSpaceAndBufferFormat(
        gfx::ContentColorUsage::kHDR, true /* needs_alpha */, gfx::ColorSpace::CreateSRGBLinear(), gfx::BufferFormat::RGBA_F16);
    ...
    const bool has_color_conversion_pass = !base::FeatureList::IsEnabled(features::kColorConversionInRenderer);
    ...
    {
        SubmitCompositorFrame(root_sink_.get(), passes, root_surface_id_.local_surface_id(), device_scale_factor);
        SurfaceId surface_id(root_sink_->frame_sink_id(), root_surface_id_.local_surface_id());

        auto aggregated_frame = AggregateFrame(surface_id);

        EXPECT_EQ(has_color_conversion_pass ? 3u : 2u, aggregated_frame.render_pass_list.size());
        EXPECT_EQ(gfx::ContentColorUsage::kHDR, aggregated_frame.render_pass_list[0]->content_color_usage);
        EXPECT_EQ(gfx::ContentColorUsage::kHDR, aggregated_frame.render_pass_list[1]->content_color_usage);
        if (has_color_conversion_pass) {
            EXPECT_EQ(gfx::ContentColorUsage::kHDR, aggregated_frame.render_pass_list[2]->content_color_usage);
        }

        // All passes will have full damage for the first frame.
        if (has_color_conversion_pass) {
            EXPECT_EQ(full_damage_rect, aggregated_frame.render_pass_list[2]->damage_rect);
        }
        EXPECT_EQ(full_damage_rect, aggregated_frame.render_pass_list[1]->damage_rect);
        EXPECT_EQ(full_damage_rect, aggregated_frame.render_pass_list[0]->damage_rect);
    }
    ...
    passes[1].has_transparent_background = false;
    {
        ...
        if (has_color_conversion_pass) {
            ...
            EXPECT_EQ(partial_damage_rect, aggregated_frame.render_pass_list[2]->damage_rect);
        }
        EXPECT_EQ(partial_damage_rect, aggregated_frame.render_pass_list[1]->damage_rect);
    }
    ...
    display_color_spaces.SetOutputColorSpaceAndBufferFormat(gfx::ContentColorUsage::kHDR, false /* needs_alpha */,
        gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT2020, gfx::ColorSpace::TransferID::SRGB), gfx::BufferFormat::BGRA_1010102);
    display_color_spaces.SetOutputColorSpaceAndBufferFormat(
        gfx::ContentColorUsage::kHDR, true /* needs_alpha */, gfx::ColorSpace::CreateSRGBLinear(), gfx::BufferFormat::RGBA_F16);
    ...
    quads[1][1] = Quad::TransparentSolidColorQuad(SkColors::kDkGray, gfx::Rect(5, 5), 0.5);
    passes[1] = Pass(quads[1], CompositorRenderPassId { 1 }, kSurfaceSize);
    passes[1].has_transparent_background = true;
    passes[1].damage_rect = partial_damage_rect;
    {
...
{
    auto test_content_color_usage_aggregation = [this](gfx::ContentColorUsage content_color_usage) {
        std::vector<Quad> child_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
        std::vector<Pass> child_passes = { Pass(child_quads, CompositorRenderPassId { 1 }, gfx::Size(100, 100)) };
        ...
        ASSERT_EQ(aggregated_pass_list[0]->content_color_usage, content_color_usage);
    };
...
{
    std::vector<Quad> child_surface_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
    std::vector<Pass> child_surface_passes = { Pass(child_surface_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };

    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        CompositorFrame child_surface_frame = MakeEmptyCompositorFrame();
        AddPasses(&child_surface_frame.render_pass_list, child_surface_passes, &child_surface_frame.metadata.referenced_surfaces);

        child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), std::move(child_surface_frame));
    }

    std::vector<Quad> root_surface_quads
        = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5), /*stretch_content_to_fill_bounds=*/false) };
    std::vector<Pass> root_passes = { Pass(root_surface_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };
    ...
    {
        auto aggregated_frame = AggregateFrame(root_surface_id_);
        EXPECT_FALSE(aggregated_frame.render_pass_list[0]->has_damage_from_contributing_content);
    }
    ...
    // Change child_frame with damage should set the flag.
    {
        ...
        EXPECT_TRUE(aggregated_frame.render_pass_list[0]->has_damage_from_contributing_content);
    }
...
TEST_F(SurfaceAggregatorValidSurfaceTest, HasDamageByChangingGrandChildSurface)
{
    auto grand_child_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    ...
    std::vector<Quad> grand_child_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
    std::vector<Pass> grand_child_passes = { Pass(grand_child_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };
    TestSurfaceIdAllocator grand_child_surface_id(grand_child_support->frame_sink_id());
    {
        CompositorFrame grand_child_frame = MakeEmptyCompositorFrame();
        AddPasses(&grand_child_frame.render_pass_list, grand_child_passes, &grand_child_frame.metadata.referenced_surfaces);

        grand_child_support->SubmitCompositorFrame(grand_child_surface_id.local_surface_id(), std::move(grand_child_frame));

        std::vector<Quad> new_child_surface_quads = { child_surface_quads[0],
            Quad::SurfaceQuad(SurfaceRange(std::nullopt, grand_child_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
                /*stretch_content_to_fill_bounds=*/false) };
        std::vector<Pass> new_child_surface_passes = { Pass(new_child_surface_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };
        child_surface_frame = MakeEmptyCompositorFrame();
        AddPasses(&child_surface_frame.render_pass_list, new_child_surface_passes, &child_surface_frame.metadata.referenced_surfaces);
        child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), std::move(child_surface_frame));
...
TEST_F(SurfaceAggregatorValidSurfaceTest, HasDamageByChangingGrandChildSurfaceNoMerge)
{
    auto grand_child_sink = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    TestSurfaceIdAllocator grand_child_surface_id(grand_child_sink->frame_sink_id());
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());

    {
        CompositorFrame grandchild_frame
            = CompositorFrameBuilder().AddRenderPass(RenderPassBuilder(kSurfaceSize).AddSolidColorQuad(gfx::Rect(5, 5), SkColors::kGreen)).Build();
        grand_child_sink->SubmitCompositorFrame(grand_child_surface_id.local_surface_id(), std::move(grandchild_frame));

        CompositorFrame child_frame
            = CompositorFrameBuilder()
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize)
                                     .AddSurfaceQuad(gfx::Rect(5, 5), SurfaceRange(grand_child_surface_id), { .allow_merge = false }))
                  .AddRenderPass(
                      RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize).AddRenderPassQuad(gfx::Rect(kSurfaceSize), CompositorRenderPassId { 1 }))
                  .Build();
        child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), std::move(child_frame));
        ...
        CompositorFrame root_frame
            = CompositorFrameBuilder()
                  .AddRenderPass(RenderPassBuilder(kSurfaceSize).AddSurfaceQuad(gfx::Rect(5, 5), SurfaceRange(child_surface_id), { .allow_merge = false }))
                  .Build();
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(root_frame));
    ...
    // No Surface changed, so no damage should be given.
    {
        ...
        EXPECT_FALSE(aggregated_frame.render_pass_list[3]->has_damage_from_contributing_content);
    }
    ...
    // A new grandchild frame should damage the root render pass.
    {
        ...
        ASSERT_EQ(4u, aggregated_frame.render_pass_list.size());
        EXPECT_TRUE(aggregated_frame.render_pass_list[3]->has_damage_from_contributing_content);
    }
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RootPassNoDamage)
{
    constexpr gfx::Rect render_pass_rect(50, 0, 100, 100);
    {
        CompositorFrame root_frame
            = CompositorFrameBuilder()
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize).AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kBlue))
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize)
                                     .AddSolidColorQuad(gfx::Rect(kSurfaceSize), SkColors::kGreen)
        ...
        ASSERT_THAT(pass_list[0]->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kBlue)));
        ASSERT_THAT(pass_list[1]->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kGreen), IsAggregatedRenderPassQuad()));
    }

    {
        ...
        auto& pass_list = aggregated_frame.render_pass_list;
        ASSERT_THAT(pass_list[0]->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kBlue), IsSolidColorQuad(SkColors::kRed)));
...
{
    std::vector<Quad> child_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
    std::vector<Pass> child_passes = { Pass(child_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };
    ...
    std::vector<Quad> root_surface_quads
        = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5), /*stretch_content_to_fill_bounds=*/false) };
    std::vector<Quad> root_render_pass_quads = { Quad::RenderPassQuad(CompositorRenderPassId { 1 }, gfx::Transform(), true) };

    std::vector<Pass> root_passes
        = { Pass(root_surface_quads, CompositorRenderPassId { 1 }, kSurfaceSize), Pass(root_render_pass_quads, CompositorRenderPassId { 2 }, kSurfaceSize) };
    ...
    // No Surface changed, so no damage should be given.
    {
        auto aggregated_frame = AggregateFrame(root_surface_id_);
        EXPECT_FALSE(aggregated_frame.render_pass_list[0]->has_damage_from_contributing_content);
        EXPECT_FALSE(aggregated_frame.render_pass_list[1]->has_damage_from_contributing_content);
    }
    ...
    // Changing child_frame should damage both render_pass.
    {
        ...
        EXPECT_TRUE(aggregated_frame.render_pass_list[1]->has_damage_from_contributing_content);
    }
    ...
    // has_damage_from_contributing_content set to true.
    {
        ...
        AddPasses(&root_frame_2.render_pass_list, root_passes, &root_frame_2.metadata.referenced_surfaces);

        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(root_frame_2));
...
{
    CompositorRenderPassId pass_id[] = { CompositorRenderPassId { 1 }, CompositorRenderPassId { 2 } };
    std::vector<Quad> root_quads[2] = {
        { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) },
        { Quad::RenderPassQuad(pass_id[0], gfx::Transform(), true) },
    };
    std::vector<Pass> root_passes = { Pass(root_quads[0], pass_id[0], kSurfaceSize), Pass(root_quads[1], pass_id[1], kSurfaceSize) };
...

    {
        ...
        // be treated as completely damaged.
        EXPECT_TRUE(aggregated_pass_list[0]->damage_rect.Contains(gfx::Rect(kSurfaceSize)));
        EXPECT_TRUE(aggregated_pass_list[1]->damage_rect.Contains(gfx::Rect(kSurfaceSize)));
    }
...
TEST_F(SurfaceAggregatorValidSurfaceTest, DamageRectOfCachedRenderPassInChildSurface)
{
    CompositorRenderPassId pass_id[] = { CompositorRenderPassId { 1 }, CompositorRenderPassId { 2 } };
    std::vector<Quad> child_quads[2] = {
    ...
    std::vector<Pass> child_passes = { Pass(child_quads[0], pass_id[0], kSurfaceSize), Pass(child_quads[1], pass_id[1], kSurfaceSize) };
...
{
    std::vector<Quad> child_surface_quads = { Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(kSurfaceSize)) };
    std::vector<Pass> child_surface_passes = { Pass(child_surface_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };
    ...
    auto transform = gfx::Transform::MakeTranslation(20, 0) * gfx::Transform::MakeScale(0.5, 0.5);
    ...
    // Clipping is off
    {
        ...
        EXPECT_EQ(aggregated_frame.render_pass_list[0]->damage_rect, expected_damage_rect);
    }
    // Clipping is on
    {
        CompositorFrame child_surface_frame = MakeEmptyCompositorFrame();
...
TEST_F(SurfaceAggregatorValidSurfaceTest, DamageRectWithInvalidChildFrame)
{
    ...
    std::vector<Quad> root_surface_quads = { Quad::SurfaceQuad(
        SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(0, 0, 100, 100), /*stretch_content_to_fill_bounds=*/false) };
    ...
    // The damage rect of the very first frame is always the full rect.
    {
        ...
        EXPECT_EQ(gfx::Rect(gfx::Rect(0, 0, 100, 100)), output_root_pass->damage_rect);
        EXPECT_EQ(output_root_pass->damage_rect, DamageListUnion(aggregated_frame.surface_damage_rect_list_));
    }
    ...
    std::vector<Quad> child_surface_quads = { Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(0, 0, 100, 100)) };
    std::vector<Pass> child_surface_passes = { Pass(child_surface_quads, CompositorRenderPassId { 1 }, gfx::Rect(20, 20, 50, 50)) };
    {
        CompositorFrame child_surface_frame = MakeEmptyCompositorFrame();
        AddPasses(&child_surface_frame.render_pass_list, child_surface_passes, &child_surface_frame.metadata.referenced_surfaces);
        ...
        EXPECT_EQ(gfx::Rect(gfx::Rect(10, 10, 60, 60)), output_root_pass->damage_rect);
        EXPECT_EQ(output_root_pass->damage_rect, DamageListUnion(aggregated_frame.surface_damage_rect_list_));
    }
    // Frame # 3 - The primary surface is not available, with a different id.
    {
        ...
        std::vector<Quad> new_root_surface_quads = { Quad::SurfaceQuad(
            SurfaceRange(std::nullopt, child_surface_id2), SkColors::kWhite, gfx::Rect(0, 0, 100, 100), /*stretch_content_to_fill_bounds=*/false) };
        std::vector<Pass> new_root_passes = { Pass(new_root_surface_quads,
            /*size*/ gfx::Size(100, 100),
            /*damage_rect*/ gfx::Rect(10, 10, 20, 20)) };

        CompositorFrame root_frame = MakeEmptyCompositorFrame();
        AddPasses(&root_frame.render_pass_list, new_root_passes, &root_frame.metadata.referenced_surfaces);
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(root_frame));
...
{
    // Video quad
    std::vector<Quad> child_surface_quads = { Quad::TextureQuad(gfx::Rect(0, 0, 100, 100)) };

    std::vector<Pass> child_surface_passes = { Pass(child_surface_quads, /*size*/ gfx::Size(100, 100),
    ...
    // (10, 0, 80, 80).
    auto video_transform = gfx::Transform::MakeTranslation(10.f, 0) * gfx::Transform::MakeScale(0.8f);

    // root surface quads
    std::vector<Quad> root_surface_quads = { Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(60, 0, 40, 40)),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite,
...

    {
        ...
        const SharedQuadState* video_sqs = output_root_pass->quad_list.back()->shared_quad_state;

        // The whole root surface (0, 0, 200, 200) is damaged.
        EXPECT_EQ(gfx::Rect(0, 0, 200, 200), aggregated_frame.surface_damage_rect_list_[0]);
        ...
        EXPECT_EQ(1U, index);
        EXPECT_EQ(gfx::Rect(10, 0, 80, 80), aggregated_frame.surface_damage_rect_list_[index]);
    }
    ...
    // Frame #1 - Has occluding damage
    {
        ...
        EXPECT_EQ(gfx::Rect(60, 0, 40, 40), aggregated_frame.surface_damage_rect_list_[0]);
    ...
    // Frame #3 - The only quad on top is removed
    {
        ...
        std::vector<Quad> new_root_surface_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite,
            /*primary_surface_rect*/ gfx::Rect(0, 0, 100, 100),
            /*opacity*/ 1.f, video_transform,
            /*stretch_content_to_fill_bounds=*/false, gfx::MaskFilterInfo(),
            /*is_fast_rounded_corner=*/false) };
    ...
    // Frame #5 - Has occluding damage and clipping of surface on top is on
    {
        ...
        // root surface quads
        std::vector<Quad> new_root_surface_quads = { Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(60, 0, 100, 100)),
            Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite,
                /*primary_surface_rect*/ gfx::Rect(0, 0, 100, 100),
                /*opacity*/ 1.f, video_transform,
                /*stretch_content_to_fill_bounds=*/false, gfx::MaskFilterInfo(),
    ...
    child_surface_quads
        = std::vector<Quad>({ Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(0, 0, 50, 50)), Quad::TextureQuad(gfx::Rect(0, 0, 100, 100), true) });

    child_surface_passes = std::vector<Pass>({ Pass(child_surface_quads, /*size*/ gfx::Size(100, 100),
        /*damage_rect*/ gfx::Rect(0, 0, 100, 100)) });
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RenderPassHasPerQuadDamage)
{
    ...
    std::vector<Quad> child_surface_quads = { Quad::TextureQuad(surface_quad_rect) };

    std::vector<Pass> child_surface_passes = { Pass(child_surface_quads, /*size*/ gfx::Size(100, 100),
        /*damage_rect*/ gfx::Rect(0, 0, 100, 100)) };

    // Various rects configs that will be used to test per quad damage.
    gfx::Rect quad_rects[] = { gfx::Rect(60, 0, 40, 40), gfx::Rect(0, 0, 50, 50), gfx::Rect(0, 0, 75, 25), gfx::Rect(10, 0, 30, 30), gfx::Rect(0, 5, 50, 50) };
    ...
    gfx::Transform child_surface_transform = gfx::Transform::MakeTranslation(child_surface_offset.x(), child_surface_offset.y());
    ...
    // root surface quads
    std::vector<Quad> root_surface_quads = { Quad::SolidColorQuad(SkColors::kRed, quad_rects[0]),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite,
            /*primary_surface_rect*/ gfx::Rect(0, 0, 100, 100),
...

    {
        ...
        auto index = video_sqs->overlay_damage_index.value();
        EXPECT_EQ(1U, index);
        EXPECT_EQ(apply_transform(surface_quad_rect), aggregated_frame.surface_damage_rect_list_[index]);
    }
    ...
    child_surface_passes = std::vector<Pass>({ Pass(child_surface_quads, gfx::Size(100, 100), quad_rects[1]) });

    {
        ...
        auto* output_root_pass = aggregated_frame.render_pass_list.back().get();
        EXPECT_EQ(output_root_pass->damage_rect, DamageListUnion(aggregated_frame.surface_damage_rect_list_));
        EXPECT_EQ(apply_transform(quad_rects[1]), aggregated_frame.surface_damage_rect_list_[0]);
        ...
        for (auto* quad : output_root_pass->quad_list) {
            ...
            if (i >= 2) {
                ...
                EXPECT_EQ(i - 1, index);
                EXPECT_EQ(apply_transform(quad_rects[i]), aggregated_frame.surface_damage_rect_list_[i - 1]);
            }
            i++;
        }
    }
...
TEST_F(SurfaceAggregatorValidSurfaceTest, PerQuadDamageSameSharedQuadState)
{
    ...
    gfx::Rect damage_rects[] = { gfx::Rect(60, 0, 30, 30), gfx::Rect(0, 0, 20, 20) };

    auto pass = CompositorRenderPass::Create();
    pass->SetNew(CompositorRenderPassId { 1 }, gfx::Rect(0, 0, 200, 200), gfx::Rect(), gfx::Transform());
    ...
    for (int i = 0; i < 2; i++) {
        ...
        const gfx::PointF kUVTopLeft(0.1f, 0.2f);
        const gfx::PointF kUVBottomRight(1.0f, 1.0f);
        texure_quad->SetNew(sqs, quad_rects[i], quad_rects[i], false /*needs_blending*/, ResourceId(1), false /*premultiplied_alpha*/, kUVTopLeft,
            kUVBottomRight, SkColors::kTransparent, false /*flipped*/, false /*nearest_neighbor*/, false /*secure_output_only*/,
            gfx::ProtectedVideoType::kClear);

        texure_quad->damage_rect = damage_rects[i];
    }

    CompositorFrame root_frame = CompositorFrameBuilder().AddRenderPass(std::move(pass)).Build();
    ...
    for (auto* quad : output_root_pass->quad_list) {
        ...
        EXPECT_EQ(aggregated_frame.surface_damage_rect_list_[quad_sqs->overlay_damage_index.value()], damage_rects[draw_rect_index]);
        draw_rect_index++;
    }
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, QuadContainsSurfaceDamageRect)
{
    ...
    std::vector<Pass> child_surface_passes = { Pass({ video_quad }, /*size=*/surface_quad_rect.size(),
    ...
    { // First frame will have full damage for each surface.
        ...
        // root surface quads
        std::vector<Quad> root_surface_quads = { Quad::SolidColorQuad(SkColors::kRed, red_rect),
            Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite,
        ...
        EXPECT_EQ(gfx::Rect(0, 0, 200, 200), output_root_pass->damage_rect);
        EXPECT_EQ(gfx::Rect(0, 0, 200, 200), aggregated_frame.surface_damage_rect_list_[0]);
        EXPECT_EQ(apply_offset(surface_quad_rect), aggregated_frame.surface_damage_rect_list_[1]);
    ...
    { // Same frame submitted to child surface.
        ...
        auto index = video_sqs->overlay_damage_index.value();
        EXPECT_EQ(0U, index);
        EXPECT_EQ(apply_offset(video_quad_rect), aggregated_frame.surface_damage_rect_list_[index]);
    }
    ...
    { // Video quad on child surface scales to 110%
        ...
        child_surface_passes = std::vector<Pass>({ Pass({ scaled_video_quad }, /*size=*/surface_quad_rect.size(),
        ...
        auto index = video_sqs->overlay_damage_index.value();
        EXPECT_EQ(0U, index);
        EXPECT_EQ(apply_offset(scaled_video_rect), aggregated_frame.surface_damage_rect_list_[index]);
    }

    { // Video quad scales back to 100%
        child_surface_passes = std::vector<Pass>({ Pass({ video_quad }, /*size=*/surface_quad_rect.size(),
            /*damage_rect=*/scaled_video_rect) });
    ...
    { // Video moves 3px right
        ...
        child_surface_passes = std::vector<Pass>({ Pass({ moved_video_quad }, /*size=*/surface_quad_rect.size(),
    ...
    { // Stays at 3px right
        ...
        auto index = video_sqs->overlay_damage_index.value();
        EXPECT_EQ(0U, index);
        EXPECT_EQ(apply_offset(moved_video_rect), aggregated_frame.surface_damage_rect_list_[index]);
    }
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, OverlayDamageIndexFromNonRootSurface)
{
    ...
    const gfx::Transform transform_child_surface_to_root = gfx::Transform::MakeTranslation(0.0f, 10.0f);
    const gfx::Transform transform_video_to_child_surface = gfx::Transform::MakeTranslation(10.0f, 0.0f);
    const gfx::Rect video_rect_in_root = (transform_child_surface_to_root * transform_video_to_child_surface).MapRect(video_quad_rect);
    const gfx::Rect child_surface_rect_in_root = transform_child_surface_to_root.MapRect(child_surface_quad_rect);

    auto video_embedded_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/false);

    TestSurfaceIdAllocator video_surface_id(video_embedded_support->frame_sink_id());
    auto video_quad = Quad::TextureQuad(video_quad_rect);

    std::vector<Pass> video_surface_passes = { Pass({ video_quad }, /*size=*/video_quad_rect.size(),
        /*damage_rect=*/video_quad_rect) };
    {
        CompositorFrame video_surface_frame = MakeEmptyCompositorFrame();
        AddPasses(&video_surface_frame.render_pass_list, video_surface_passes, &video_surface_frame.metadata.referenced_surfaces);
        PopulateTransferableResources(video_surface_frame);

        video_embedded_support->SubmitCompositorFrame(video_surface_id.local_surface_id(), std::move(video_surface_frame));
    }

    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        auto surface_quad = Quad::SurfaceQuad(SurfaceRange(std::nullopt, video_surface_id), SkColors::kWhite,
        ...
        std::vector<Pass> child_surface_passes = { Pass({ surface_quad },
    ...
    // First frame will have full damage for each surface.
    {
        std::vector<Quad> root_surface_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite,
        ...
        EXPECT_EQ(DamageListUnion(aggregated_frame.surface_damage_rect_list_), output_root_pass->damage_rect);

        EXPECT_EQ(2u, aggregated_frame.render_pass_list.size()) << "Test assumes surface does not merge";
        EXPECT_THAT(aggregated_frame.render_pass_list[0]->quad_list, ElementsAre(IsTextureQuad()));
        EXPECT_THAT(aggregated_frame.render_pass_list[1]->quad_list, ElementsAre(IsAggregatedRenderPassQuad()));
    }
    ...
    // this damage rect.
    {
        ...
        const SharedQuadState* video_sqs = aggregated_frame.render_pass_list[0]->quad_list.back()->shared_quad_state;

        // Surface damage can be assigned to the video quad.
        ASSERT_TRUE(video_sqs->overlay_damage_index.has_value());
        auto index = video_sqs->overlay_damage_index.value();
        EXPECT_EQ(0U, index);
        EXPECT_EQ(video_rect_in_root, aggregated_frame.surface_damage_rect_list_[index]);
    }
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, NonRootRenderPassWithPerQuadDamage)
{
    ...
    CompositorRenderPassList root_passes;
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 1 }, child_pass_size)
    ...
    root_passes.push_back(RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize)
                              ...
                              .AddRenderPassQuad(gfx::Rect(child_pass_size), CompositorRenderPassId { 1 })
                              ...
                              .Build());
    {
        ...
        EXPECT_EQ(aggregated_frame.render_pass_list[1]->damage_rect, gfx::Rect(kSurfaceSize));
    }

    {
        ...
        // 10x10).
        ASSERT_EQ(aggregated_frame.render_pass_list.size(), 2u);
        EXPECT_EQ(aggregated_frame.render_pass_list[1]->damage_rect, gfx::Rect(30, 30, 50, 50));
    }
...
TEST_F(SurfaceAggregatorValidSurfaceTest, DisplayTransformDamageCallback)
{
    ...
    SubmitCompositorFrame(root_sink_.get(), root_passes, root_surface_id_.local_surface_id(), 0.5f);

    EXPECT_CALL(
        aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), surface_size, gfx::Rect(surface_size), next_display_time()));

    auto frame = aggregator_.Aggregate(root_surface_id_, GetNextDisplayTimeAndIncrement(), gfx::OVERLAY_TRANSFORM_ROTATE_CLOCKWISE_90);
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RoundedCornerTransformChange)
{
    ...
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        ...
        std::vector<Pass> child_passes = { Pass(child_quads[0], CompositorRenderPassId { 1 }, kSurfaceSize) };

        CompositorFrame child_frame = MakeEmptyCompositorFrame();
        AddPasses(&child_frame.render_pass_list, child_passes, &child_frame.metadata.referenced_surfaces);

        child_frame.render_pass_list[0]->shared_quad_state_list.front()->mask_filter_info = gfx::MaskFilterInfo(gfx::RRectF(0, 0, 100, 10, 5));

        child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), std::move(child_frame));
    }

    // Root surface.
    std::vector<Quad> surface_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5), false) };
    ...
    auto aggregated_frame = AggregateFrame(root_surface_id_);
    auto* aggregated_first_pass_sqs = aggregated_frame.render_pass_list[0]->shared_quad_state_list.front();

    EXPECT_EQ(gfx::RRectF(0, 7, 100, 10, 5), aggregated_first_pass_sqs->mask_filter_info.rounded_corner_bounds());
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RoundedCornerTransformedSurfaceQuad)
{
    ...
    TestSurfaceIdAllocator grandchild_surface_id(child_sink_->frame_sink_id());
    {
        ...
        child_sink_->SubmitCompositorFrame(grandchild_surface_id.local_surface_id(), std::move(child_frame));
    }
    ...
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        ...
        std::vector<Quad> child_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, grandchild_surface_id), SkColors::kWhite, gfx::Rect(5, 5), 0.5f,
            gfx::Transform(), false, gfx::MaskFilterInfo(gfx::RRectF(0, 0, 96, 10, 5)),
            /*is_fast_rounded_corner=*/false) };
    ...
    std::vector<Quad> secondary_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5), 1.f,
        surface_transform, false, gfx::MaskFilterInfo(),
        /*is_fast_rounded_corner=*/false) };

    std::vector<Pass> root_passes = { Pass(secondary_quads, kSurfaceSize) };

    CompositorFrame root_frame = CompositorFrameBuilder().SetDeviceScaleFactor(2.0f).Build();
    AddPasses(&root_frame.render_pass_list, root_passes, &root_frame.metadata.referenced_surfaces);
    ...
    auto aggregated_frame = AggregateFrame(root_surface_id_);
    auto* aggregated_first_pass_sqs = aggregated_frame.render_pass_list[1]->shared_quad_state_list.front();
    ...
    EXPECT_EQ(gfx::RRectF(3, 11, 192, 20, 10), aggregated_first_pass_sqs->mask_filter_info.rounded_corner_bounds());
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RoundedCornerTransformedMergedSurfaceQuad)
{
    auto middle_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    // Grandchild surface.
    TestSurfaceIdAllocator grandchild_surface_id(child_sink_->frame_sink_id());
    ...
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        std::vector<Quad> child_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, grandchild_surface_id), SkColors::kWhite, gfx::Rect(5, 5), 1.f,
            gfx::Transform(), false, gfx::MaskFilterInfo(gfx::RRectF(0, 0, 96, 10, 5)),
            /*is_fast_rounded_corner=*/false) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, TransformedRoundedSurfaceQuad)
{
    ...
    surface_transform.Translate(3, 4);
    std::vector<Quad> secondary_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, gfx::Rect(5, 5), 1.f,
        surface_transform, false, gfx::MaskFilterInfo(gfx::RRectF(0, 0, 96, 10, 5)),
        /*is_fast_rounded_corner=*/true) };
    ...
    EXPECT_EQ(gfx::RRectF(0, 0, 96, 10, 5), aggregated_first_pass_sqs->mask_filter_info.rounded_corner_bounds());
}
...
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, AggregateDamageRectWithMultiplyEmbeddedSurface)
{
    // Add a callback for when the surface is damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    ...
    CompositorFrame root_frame
        = CompositorFrameBuilder()
              ...
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 2 }, kSurfaceSize)
                                 ...
                                 .SetQuadToTargetTransform(scale))
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 3 }, kSurfaceSize)
                                 .AddRenderPassQuad(gfx::Rect(kSurfaceSize), CompositorRenderPassId { 2 })
                                 .SetQuadToTargetTranslation(30, 50)
                                 .AddRenderPassQuad(gfx::Rect(kSurfaceSize), CompositorRenderPassId { 1 }))
              .Build();
    ...
    EXPECT_CALL(aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, expected_damage_rect, next_display_time()));
    auto aggregated_frame_2 = AggregateFrame(root_surface_id_);
    testing::Mock::VerifyAndClearExpectations(&aggregated_damage_callback);
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, AggregateDamageRectWithRenderPassCycle)
{
    // Add a callback for when the surface is damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
    ...
    CompositorRenderPassId root_pass_ids[] = { CompositorRenderPassId { 1 }, CompositorRenderPassId { 2 } };
    std::vector<Quad> root_quads_1
        = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)), Quad::RenderPassQuad(root_pass_ids[1], gfx::Transform(), true) };
    std::vector<Quad> root_quads_2 = { Quad::RenderPassQuad(root_pass_ids[0], gfx::Transform(), true) };
    std::vector<Pass> root_passes = { Pass(root_quads_2, root_pass_ids[1], kSurfaceSize), Pass(root_quads_1, root_pass_ids[0], kSurfaceSize) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, AllowMerge)
{
    ...
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        std::vector<Quad> child_quads = { Quad::SolidColorQuad(SkColors::kGreen, child_rect) };
        std::vector<Pass> child_passes = { Pass(child_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };
    ...
    {
        auto pass = CompositorRenderPass::Create();
        pass->SetNew(CompositorRenderPassId { 1 }, root_rect, root_rect, gfx::Transform());
        auto* sqs = pass->CreateAndAppendSharedQuadState();
        sqs->opacity = 1.f;

        auto* surface_quad = pass->CreateAndAppendDrawQuad<SurfaceDrawQuad>();
        surface_quad->SetAll(sqs, child_rect, child_rect,
            /*needs_blending=*/false, SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite,
            /*stretch_content_to_fill_bounds=*/false,
            /*is_reflection=*/false,
            /*allow_merge=*/true);
...
TEST_F(SurfaceAggregatorValidSurfaceTest, SkipInvisibleSurface)
{
    ...
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        ...
        std::vector<Pass> child_passes = { Pass(child_quads, CompositorRenderPassId { 1 }, output_rect) };
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RenderPassDoesNotFillSurface)
{
    ...
    {
        std::vector<Quad> child_quads = { Quad::SolidColorQuad(SkColors::kGreen, child_rect) };
        std::vector<Pass> child_passes = { Pass(child_quads, CompositorRenderPassId { 1 }, child_rect) };
    ...
    // Submit a SurfaceDrawQuad that does not allow merging.
    {
        ...
        const auto* rpdq = AggregatedRenderPassDrawQuad::MaterialCast(aggregated_frame.render_pass_list[1]->quad_list.front());
...
// Tests that damage rects are aggregated correctly when surfaces change.
TEST_P(SurfaceAggregatorValidSurfaceWithMergingPassesTest, AggregateDamageRectWithBackdropFilter)
{
    // Add callbacks for when the surfaces are damaged.
    MockAggregatedDamageCallback aggregated_damage_callback;
    root_sink_->SetAggregatedDamageCallbackForTesting(aggregated_damage_callback.GetCallback());
...

    {
        ...
        for (int i = 1; i < 5; ++i) {
            root_pass_list.push_back(RenderPassBuilder(CompositorRenderPassId { i }, render_pass_rect)
                                         .AddSolidColorQuad(render_pass_rect, SkColors::kGreen)
                                         .AddBackdropFilter(cc::FilterOperation::CreateBlurFilter(5))
                                         .Build());
        }
        root_pass_list.push_back(RenderPassBuilder(CompositorRenderPassId { 5 }, gfx::Rect(kSurfaceSize))
                                     .AddRenderPassQuad(render_pass_rect, CompositorRenderPassId { 1 })
                                     ...
                                     .AddSurfaceQuad(gfx::Rect(100, 100), SurfaceRange(child_surface_id), { .allow_merge = AllowMerge() })
                                     .SetQuadToTargetTranslation(0, 85)
                                     .Build());

        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), MakeCompositorFrame(std::move(root_pass_list)));
    }
    ...
    EXPECT_CALL(aggregated_damage_callback,
        OnAggregatedDamage(child_surface_id.local_surface_id(), child_surface_size, gfx::Rect(child_surface_size), next_display_time()));
    EXPECT_CALL(
        aggregated_damage_callback, OnAggregatedDamage(root_surface_id_.local_surface_id(), kSurfaceSize, gfx::Rect(0, 0, 100, 100), next_display_time()));
    auto aggregated_frame = AggregateFrame(root_surface_id_);
    testing::Mock::VerifyAndClearExpectations(&aggregated_damage_callback);
    const auto& aggregated_pass_list = aggregated_frame.render_pass_list;
    ...
    EXPECT_EQ(expected_num_passes_after_aggregation, aggregated_pass_list2.size());
    EXPECT_EQ(expected_damage_rect, aggregated_pass_list2.back()->damage_rect);
}

TEST_F(SurfaceAggregatorValidSurfaceTest, ContainedFrameSinkChangeInvalidatesHitTestData)
{
    auto embedded_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId1, /*is_root=*/true);
    TestSurfaceIdAllocator embedded_surface_id(embedded_support->frame_sink_id());
    ...
    // and aggregate.
    {
        std::vector<Quad> embedded_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kGray, gfx::Rect(5, 5)) };
        std::vector<Pass> embedded_passes = { Pass(embedded_quads, kSurfaceSize) };
        SubmitCompositorFrame(embedded_support.get(), embedded_passes, embedded_surface_id.local_surface_id(), 1.0f);

        std::vector<Quad> root_quads = { Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(5, 5)) };
        std::vector<Pass> root_passes = { Pass(root_quads, kSurfaceSize) };
        SubmitCompositorFrame(root_sink_.get(), root_passes, root_surface_id_.local_surface_id(), 1.0f);
        AggregateFrame(root_surface_id_);
    }

    const HitTestManager* hit_test_manager = manager_.hit_test_manager();
    uint64_t hit_test_region_index = hit_test_manager->submit_hit_test_region_list_index();

    // Now submit a root frame that *does* reference the embedded frame, and
    // aggregate.
    {
        std::vector<Quad> root_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, embedded_surface_id), SkColors::kWhite, gfx::Rect(5, 5), false),
            Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(5, 5)), Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(5, 5)) };
        std::vector<Pass> root_passes = { Pass(root_quads, kSurfaceSize) };

        SubmitCompositorFrame(root_sink_.get(), root_passes, root_surface_id_.local_surface_id(), 1.0);
        AggregateFrame(root_surface_id_);
    }

    // Check that the HitTestManager was marked as needing to re-aggregate hit
    // test data.
    EXPECT_GT(hit_test_manager->submit_hit_test_region_list_index(), hit_test_region_index);
}

void ExpectDelegatedInkMetadataIsEqual(const gfx::DelegatedInkMetadata& lhs, const gfx::DelegatedInkMetadata& rhs)
{
    ...
    EXPECT_FLOAT_EQ(lhs.presentation_area().width(), rhs.presentation_area().width());
    EXPECT_FLOAT_EQ(lhs.presentation_area().height(), rhs.presentation_area().height());
...
TEST_F(SurfaceAggregatorValidSurfaceTest, DelegatedInkMetadataTest)
{
    ...
    CompositorFrame child_frame = MakeEmptyCompositorFrame();
    gfx::DelegatedInkMetadata metadata(gfx::PointF(100, 100), 1.5, SK_ColorRED, base::TimeTicks::Now(), gfx::RectF(10, 10, 200, 200), base::TimeTicks::Now(),
        /*hovering*/ true,
        /*render_pass_id=*/0);
    child_frame.metadata.delegated_ink_metadata = std::make_unique<gfx::DelegatedInkMetadata>(metadata);
    AddPasses(&child_frame.render_pass_list, child_passes, &child_frame.metadata.referenced_surfaces);
    ...
    std::vector<Pass> root_passes = { Pass(root_quads, CompositorRenderPassId { 1 }, gfx::Size(30, 30)) };

    CompositorFrame root_frame = MakeEmptyCompositorFrame();
    AddPasses(&root_frame.render_pass_list, root_passes, &root_frame.metadata.referenced_surfaces);

    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.Scale(1.5, 1.5);
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.Translate(70, 240);

    // Update the expected metadata to reflect the transforms to point and area
    // that are expected to occur.
    gfx::PointF pt = root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.MapPoint(metadata.point());
    gfx::RectF area = root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.MapRect(metadata.presentation_area());
    metadata = gfx::DelegatedInkMetadata(
        pt, metadata.diameter(), metadata.color(), metadata.timestamp(), area, metadata.frame_time(), metadata.is_hovering(), /*render_pass_id=*/0);

    root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(root_frame));

    auto aggregated_frame = AggregateFrame(root_surface_id_);

    std::unique_ptr<gfx::DelegatedInkMetadata> actual_metadata = std::move(aggregated_frame.delegated_ink_metadata);
    ...
    AddPasses(&blank_frame.render_pass_list, child_passes, &blank_frame.metadata.referenced_surfaces);
    child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), std::move(blank_frame));
...
TEST_F(SurfaceAggregatorValidSurfaceTest, RepeatedDelegatedInkMetadataTest)
{
    ...
    CompositorFrame child_frame = MakeEmptyCompositorFrame();
    gfx::DelegatedInkMetadata metadata(gfx::PointF(100, 100), /*diameter=*/1.5, SK_ColorRED, base::TimeTicks::Now(), gfx::RectF(10, 10, 200, 200),
    ...
    gfx::RectF area = root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.MapRect(metadata.presentation_area());
    metadata = gfx::DelegatedInkMetadata(
        pt, metadata.diameter(), metadata.color(), metadata.timestamp(), area, metadata.frame_time(), metadata.is_hovering(), /*render_pass_id=*/1);
...
TEST_F(SurfaceAggregatorValidSurfaceTest, TransformDelegatedInkMetadataTallTree)
{
    auto greatgrand_child_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId2, /*is_root=*/false);
    std::vector<Quad> greatgrandchild_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
    std::vector<Pass> greatgrandchild_passes = { Pass(greatgrandchild_quads, CompositorRenderPassId { 1 }, gfx::Size(100, 100)) };

    gfx::DelegatedInkMetadata metadata(gfx::PointF(100, 100), 1.5, SK_ColorRED, base::TimeTicks::Now(), gfx::RectF(10, 10, 200, 200), base::TimeTicks::Now(),
        /*hovering*/ false,
        /*render_pass_id=*/0);
    CompositorFrame greatgrandchild_frame = MakeEmptyCompositorFrame();
    greatgrandchild_frame.metadata.delegated_ink_metadata = std::make_unique<gfx::DelegatedInkMetadata>(metadata);
    AddPasses(&greatgrandchild_frame.render_pass_list, greatgrandchild_passes, &greatgrandchild_frame.metadata.referenced_surfaces);

    TestSurfaceIdAllocator great_grandchild_surface_id(greatgrand_child_support->frame_sink_id());
    greatgrand_child_support->SubmitCompositorFrame(great_grandchild_surface_id.local_surface_id(), std::move(greatgrandchild_frame));

    auto grand_child_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    std::vector<Quad> grandchild_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, great_grandchild_surface_id), SkColors::kWhite, gfx::Rect(7, 7),
        /*stretch_content_to_fill_bounds=*/false) };
    std::vector<Pass> grandchild_passes = { Pass(grandchild_quads, CompositorRenderPassId { 1 }, gfx::Size(100, 100)) };

    CompositorFrame grandchild_frame = MakeEmptyCompositorFrame();

    AddPasses(&grandchild_frame.render_pass_list, grandchild_passes, &grandchild_frame.metadata.referenced_surfaces);

    grandchild_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.Scale(1.5, 1.5);
    grandchild_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.Translate(37, 82);

    // Update the expected metadata to reflect the transforms to point and area
    // that are expected to occur.
    gfx::PointF pt = grandchild_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.MapPoint(metadata.point());
    gfx::RectF area = grandchild_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.MapRect(metadata.presentation_area());

    TestSurfaceIdAllocator grandchild_surface_id(grand_child_support->frame_sink_id());
    grand_child_support->SubmitCompositorFrame(grandchild_surface_id.local_surface_id(), std::move(grandchild_frame));

    std::vector<Quad> child_quads
        = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, grandchild_surface_id), SkColors::kWhite, gfx::Rect(7, 7), /*stretch_content_to_fill_bounds=*/false) };
    std::vector<Pass> child_passes = { Pass(child_quads, CompositorRenderPassId { 1 }, gfx::Size(30, 30)) };

    CompositorFrame child_frame = MakeEmptyCompositorFrame();
    AddPasses(&child_frame.render_pass_list, child_passes, &child_frame.metadata.referenced_surfaces);

    child_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.Translate(36, 15);

    pt = child_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.MapPoint(pt);
    area = child_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.MapRect(area);
    ...
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.Scale(0.7, 0.7);
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.Translate(70, 240);

    pt = root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.MapPoint(pt);
    area = root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.MapRect(area);
    ...
    // Send a compositor frame with no delegated ink metadata.
    CompositorFrame blank_frame = MakeEmptyCompositorFrame();
    AddPasses(&blank_frame.render_pass_list, greatgrandchild_passes, &blank_frame.metadata.referenced_surfaces);
    greatgrand_child_support->SubmitCompositorFrame(great_grandchild_surface_id.local_surface_id(), std::move(blank_frame));
...
TEST_F(SurfaceAggregatorValidSurfaceTest, DelegatedInkMetadataMultipleChildren)
{
    auto child_2_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    auto child_3_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId2, /*is_root=*/false);

    std::vector<Quad> child_1_quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
    std::vector<Pass> child_1_passes = { Pass(child_1_quads, CompositorRenderPassId { 1 }, gfx::Size(100, 100)) };

    CompositorFrame child_1_frame = MakeEmptyCompositorFrame();
    AddPasses(&child_1_frame.render_pass_list, child_1_passes, &child_1_frame.metadata.referenced_surfaces);

    TestSurfaceIdAllocator child_1_surface_id(child_sink_->frame_sink_id());
    child_sink_->SubmitCompositorFrame(child_1_surface_id.local_surface_id(), std::move(child_1_frame));

    std::vector<Quad> child_2_quads = { Quad::SolidColorQuad(SkColors::kMagenta, gfx::Rect(5, 5)) };
    std::vector<Pass> child_2_passes = { Pass(child_2_quads, CompositorRenderPassId { 1 }, gfx::Size(100, 100)) };

    gfx::DelegatedInkMetadata metadata(gfx::PointF(88, 34), 1.8, SK_ColorBLACK, base::TimeTicks::Now(), gfx::RectF(50, 50, 300, 300), base::TimeTicks::Now(),
        /*hovering*/ true,
        /*render_pass_id=*/0);
    CompositorFrame child_2_frame = MakeEmptyCompositorFrame();
    child_2_frame.metadata.delegated_ink_metadata = std::make_unique<gfx::DelegatedInkMetadata>(metadata);
    AddPasses(&child_2_frame.render_pass_list, child_2_passes, &child_2_frame.metadata.referenced_surfaces);

    TestSurfaceIdAllocator child_2_surface_id(child_2_support->frame_sink_id());
    child_2_support->SubmitCompositorFrame(child_2_surface_id.local_surface_id(), std::move(child_2_frame));

    std::vector<Quad> child_3_quads = { Quad::SolidColorQuad(SkColors::kCyan, gfx::Rect(5, 5)) };
    std::vector<Pass> child_3_passes = { Pass(child_3_quads, CompositorRenderPassId { 1 }, gfx::Size(100, 100)) };

    CompositorFrame child_3_frame = MakeEmptyCompositorFrame();
    AddPasses(&child_3_frame.render_pass_list, child_3_passes, &child_3_frame.metadata.referenced_surfaces);

    TestSurfaceIdAllocator child_3_surface_id(child_3_support->frame_sink_id());
    child_3_support->SubmitCompositorFrame(child_3_surface_id.local_surface_id(), std::move(child_3_frame));

    std::vector<Quad> root_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_1_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
                                         /*stretch_content_to_fill_bounds=*/false),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_2_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false),
        Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_3_surface_id), SkColors::kWhite, gfx::Rect(5, 5),
            /*stretch_content_to_fill_bounds=*/false) };
    ...
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.Translate(9, 87);
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(1)->quad_to_target_transform.Scale(0.7, 0.7);
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(1)->quad_to_target_transform.Translate(70, 240);
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(2)->quad_to_target_transform.Scale(2.7, 0.2);

    // Update the expected metadata to reflect the transforms to point and area
    // that are expected to occur.
    gfx::PointF pt = root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(1)->quad_to_target_transform.MapPoint(metadata.point());
    gfx::RectF area = root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(1)->quad_to_target_transform.MapRect(metadata.presentation_area());
    ...
    // Send a compositor frame with no delegated ink metadata.
    CompositorFrame blank_frame = MakeEmptyCompositorFrame();
    AddPasses(&blank_frame.render_pass_list, child_2_passes, &blank_frame.metadata.referenced_surfaces);
    child_2_support->SubmitCompositorFrame(child_2_surface_id.local_surface_id(), std::move(blank_frame));
...
TEST_F(SurfaceAggregatorValidSurfaceTest, MultipleChildrenHaveDelegatedInkMetadata)
{
    auto child_2_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryMiddleFrameSinkId, /*is_root=*/false);
    auto child_3_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &manager_, kArbitraryFrameSinkId2, /*is_root=*/false);
    ...
    gfx::DelegatedInkMetadata early_metadata(gfx::PointF(88, 34), 1.8, SK_ColorBLACK, base::TimeTicks::Now(), gfx::RectF(50, 50, 300, 300),
        base::TimeTicks::Now(), /*hovering*/ false,
        /*render_pass_id=*/0);
    gfx::DelegatedInkMetadata later_metadata(gfx::PointF(92, 35), 0.08, SK_ColorYELLOW, base::TimeTicks::Now() + base::Microseconds(50),
        gfx::RectF(35, 55, 128, 256), base::TimeTicks::Now() + base::Microseconds(52),
        /*hovering*/ true, /*render_pass_id=*/0);

    CompositorFrame child_2_frame = MakeEmptyCompositorFrame();
    child_2_frame.metadata.delegated_ink_metadata = std::make_unique<gfx::DelegatedInkMetadata>(later_metadata);
    AddPasses(&child_2_frame.render_pass_list, child_2_passes, &child_2_frame.metadata.referenced_surfaces);
    ...
    CompositorFrame child_3_frame = MakeEmptyCompositorFrame();
    child_3_frame.metadata.delegated_ink_metadata = std::make_unique<gfx::DelegatedInkMetadata>(early_metadata);
    AddPasses(&child_3_frame.render_pass_list, child_3_passes, &child_3_frame.metadata.referenced_surfaces);
    ...
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(0)->quad_to_target_transform.Translate(9, 87);
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(1)->quad_to_target_transform.Scale(1.4, 1.7);
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(1)->quad_to_target_transform.Translate(214, 144);
    root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(2)->quad_to_target_transform.Scale(2.7, 0.2);
    ...
    gfx::PointF pt = root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(1)->quad_to_target_transform.MapPoint(later_metadata.point());
    gfx::RectF area = root_frame.render_pass_list[0]->shared_quad_state_list.ElementAt(1)->quad_to_target_transform.MapRect(later_metadata.presentation_area());

    root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(root_frame));

    auto aggregated_frame = AggregateFrame(root_surface_id_);

    gfx::DelegatedInkMetadata expected_metadata(pt, later_metadata.diameter(), later_metadata.color(), later_metadata.timestamp(), area,
        later_metadata.frame_time(), later_metadata.is_hovering(), later_metadata.render_pass_id());
    ...
    CompositorFrame blank_frame = MakeEmptyCompositorFrame();
    AddPasses(&blank_frame.render_pass_list, child_3_passes, &blank_frame.metadata.referenced_surfaces);
    child_3_support->SubmitCompositorFrame(child_3_surface_id.local_surface_id(), std::move(blank_frame));
...
TEST_F(SurfaceAggregatorValidSurfaceTest, DelegatedInkMetadataOnUndrawnSurface)
{
    std::vector<Quad> child_quads = { Quad::SolidColorQuad(SkColors ::kGreen, gfx::Rect(5, 5)) };
    std::vector<Pass> child_passes = { Pass(child_quads, CompositorRenderPassId { 1 }, gfx::Size(100, 100)) };

    CompositorFrame child_frame = MakeEmptyCompositorFrame();
    gfx::DelegatedInkMetadata metadata(gfx::PointF(34, 89), 1.597, SK_ColorBLUE, base::TimeTicks::Now(), gfx::RectF(2.3, 3.2, 177, 212), base::TimeTicks::Now(),
    ...
    // undrawn.
    std::vector<Quad> root_quads = { Quad::SolidColorQuad(SkColors ::kMagenta, gfx::Rect(5, 5)) };

    std::vector<Pass> root_passes = { Pass(root_quads, CompositorRenderPassId { 1 }, gfx::Size(30, 30)) };

    CompositorFrame root_frame = MakeEmptyCompositorFrame();
    root_frame.metadata.referenced_surfaces.emplace_back(SurfaceRange(std::nullopt, child_surface_id));
    AddPasses(&root_frame.render_pass_list, root_passes, &root_frame.metadata.referenced_surfaces);
    ...
    auto copy_request = CopyOutputRequest::CreateStubForTesting();
    child_sink_->RequestCopyOfOutput({ child_surface_id.local_surface_id(), SubtreeCaptureId(), std::move(copy_request) });
...
TEST_F(SurfaceAggregatorValidSurfaceTest, HasUnembeddedRenderPass)
{
...

    {
        CompositorFrame frame
            = CompositorFrameBuilder()
                  ...
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, unembedded_rect).AddSolidColorQuad(root_rect, SkColors::kGreen).Build())
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 2 }, root_rect).AddSolidColorQuad(root_rect, SkColors::kBlue).Build())
                  .Build();
    ...
    EXPECT_THAT(unembedded_pass->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kGreen)));
    ...
    EXPECT_THAT(root_pass->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kBlue)));
    EXPECT_EQ(root_pass->output_rect, root_rect);
    EXPECT_TRUE(root_pass->has_damage_from_contributing_content);
}

TEST_F(SurfaceAggregatorValidSurfaceTest, HasDamageFromContributingPropagatedForUnembeddedRenderPass)
{
    constexpr gfx::Rect unembedded_rect(50, 50);
    constexpr gfx::Rect root_rect(kSurfaceSize);
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());

    {
        CompositorFrame frame
            = CompositorFrameBuilder()
                  ...
                  // be drawn to draw the root render pass.
                  .AddRenderPass(
                      RenderPassBuilder(CompositorRenderPassId { 1 }, unembedded_rect).AddSurfaceQuad(unembedded_rect, SurfaceRange(child_surface_id)).Build())
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 2 }, root_rect).AddSolidColorQuad(root_rect, SkColors::kBlue).Build())
                  .Build();
...

    {
        ...
        CompositorFrame frame
            = CompositorFrameBuilder()
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, unembedded_rect).AddSolidColorQuad(unembedded_rect, SkColors::kGreen).Build())
                  .Build();
...
TEST_F(SurfaceAggregatorValidSurfaceTest, CopyRequestWithSecureOutputForUnembeddedRenderPass)
{
    aggregator_.set_output_is_secure(true);

    {
        ...
        CompositorFrame frame
            = CompositorFrameBuilder()
                  ...
                  .AddRenderPass(
                      RenderPassBuilder(CompositorRenderPassId { 1 }, rect).AddTextureQuad(rect, ResourceId(1), { .secure_output_only = true }).Build())
                  // This render pass isn't embedded by the root but it embeds the
                  // render pass with TextureDrawQuad.
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 2 }, rect)
                                     .AddRenderPassQuad(rect, CompositorRenderPassId { 1 })
                                     .AddStubCopyOutputRequest()
                                     .Build())
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 3 }, rect).AddSolidColorQuad(rect, SkColors::kGreen).Build())
                  .PopulateResources()
                  .Build();
    ...
    EXPECT_THAT(render_pass_list[0]->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kBlack)));
    ...
    EXPECT_THAT(render_pass_list[1]->quad_list, ElementsAre(IsAggregatedRenderPassQuad()));
    ...
    EXPECT_THAT(render_pass_list[2]->quad_list, ElementsAre(IsSolidColorQuad(SkColors::kGreen)));
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, ColorUsageChangeFullFrameDamage)
{
    ...
    std::vector<Quad> quads = { Quad::SolidColorQuad(SkColors ::kRed, gfx::Rect(kSurfaceSize)) };
    std::vector<Pass> passes = { Pass(quads, kSurfaceSize) };
    ...
    // Second frame has partial damage.
    {
        ...
        EXPECT_EQ(gfx::ContentColorUsage::kHDR, aggregated_frame.render_pass_list[0]->content_color_usage);
        EXPECT_EQ(partial_damage_rect, aggregated_frame.render_pass_list[0]->damage_rect);
    }
    ...
    // frame should have full damage.
    {
        ...
        compositor_frame.metadata.content_color_usage = gfx::ContentColorUsage::kSRGB;
        AddPasses(&compositor_frame.render_pass_list, passes, &compositor_frame.metadata.referenced_surfaces);
        root_sink_->SubmitCompositorFrame(root_surface_id_.local_surface_id(), std::move(compositor_frame));
        auto aggregated_frame = AggregateFrame(root_surface_id_);

        EXPECT_EQ(gfx::ContentColorUsage::kSRGB, aggregated_frame.render_pass_list[0]->content_color_usage);
        EXPECT_EQ(full_damage_rect, aggregated_frame.render_pass_list[0]->damage_rect);
    }
}
...
TEST_F(SurfaceAggregatorValidSurfaceTest, ClipRectNonMergedPass)
{
    ...
    {
        auto frame = CompositorFrameBuilder()
                         .AddRenderPass(RenderPassBuilder(grandchild_rect).AddSolidColorQuad(gfx::Rect(grandchild_rect), SkColors ::kBlue))
                         .Build();
        grandchild_support->SubmitCompositorFrame(grandchild_surface_id.local_surface_id(), std::move(frame));
    }
    {
        ...
        auto frame
            = CompositorFrameBuilder()
                  .AddRenderPass(RenderPassBuilder(child_rect).AddSurfaceQuad(grandchild_rect, SurfaceRange(grandchild_surface_id), { .allow_merge = false }))
                  .Build();
        child_sink_->SubmitCompositorFrame(child_surface_id.local_surface_id(), std::move(frame));
    }
    ...
    {
        auto frame = CompositorFrameBuilder()
                         .AddRenderPass(RenderPassBuilder(root_rect)
    ...
    EXPECT_THAT(nonroot_render_pass->quad_list, ElementsAre(IsSolidColorQuad(SkColors ::kBlue)));
    ...
    EXPECT_THAT(root_render_pass->quad_list, ElementsAre(IsAggregatedRenderPassQuad(), IsSolidColorQuad(SkColors ::kWhite)));
    ...
    EXPECT_THAT(rpdq->shared_quad_state->clip_rect, testing::Optional(gfx::Rect(10, 10, 90, 90)));
}

INSTANTIATE_TEST_SUITE_P(, SurfaceAggregatorValidSurfaceWithMergingPassesTest, testing::Bool());
...
class SurfaceAggregatorPreventMergeTest : public SurfaceAggregatorValidSurfaceTest {
protected:
...
TEST_F(SurfaceAggregatorPreventMergeTest, PreventMerge)
{
    ...
    {
        std::vector<Quad> root_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child.surface_id()), SkColors::kWhite, child_rect, false, true) };
        std::vector<Pass> root_passes = { Pass(root_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };
...
TEST_F(SurfaceAggregatorPreventMergeTest, NonRootSurfacesCanMerge)
{
    ...
    TestVizClient inner_child(this, &manager_, kArbitraryFrameSinkId1, child_rect);
    ...
    TestSurfaceIdAllocator child_surface_id(child_sink_->frame_sink_id());
    {
        std::vector<Quad> child_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, inner_child.surface_id()), SkColors::kGreen, child_rect, false, true) };
        std::vector<Pass> child_passes = { Pass(child_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };
    ...
    // Submit a SurfaceDrawQuad that allows merging, but will be prevented.
    {
        std::vector<Quad> root_quads = { Quad::SurfaceQuad(SurfaceRange(std::nullopt, child_surface_id), SkColors::kWhite, child_rect, false, true) };
        std::vector<Pass> root_passes = { Pass(root_quads, CompositorRenderPassId { 1 }, kSurfaceSize) };
...
class SurfaceAggregatorVulkanSecondaryCB : public SurfaceAggregatorValidSurfaceTest {
public:
    SurfaceAggregatorVulkanSecondaryCB()
        : SurfaceAggregatorValidSurfaceTest(SurfaceAggregator::ExtraPassForReadbackOption::kAddPassForReadback, false)
    {
    }
};
...
TEST_F(SurfaceAggregatorVulkanSecondaryCB, DoNotAppendPassForFrameWithoutReadback)
{
    CompositorFrame frame
        = CompositorFrameBuilder()
              .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, kSurfaceSize).AddSolidColorQuad(gfx::Rect(5, 5), SkColors ::kGreen))
...
public:
    ...
    OnScreenshotCapturedWaiter& operator=(const OnScreenshotCapturedWaiter&) = delete;

    // mojom::FrameSinkManagerClient:
    void OnFirstSurfaceActivation(const SurfaceInfo&) override
    {
    }
    void OnFrameTokenChanged(const FrameSinkId&, uint32_t, base::TimeTicks) override
    {
    }
    void OnAggregatedHitTestRegionListUpdated(const FrameSinkId& frame_sink_id, const std::vector<AggregatedHitTestRegion>& hit_test_data) override
    {
    }
#if BUILDFLAG(IS_ANDROID)
    void VerifyThreadIdsDoNotBelongToHost(const std::vector<int32_t>& thread_ids, VerifyThreadIdsDoNotBelongToHostCallback callback) override
    {
    }
#endif
    void OnScreenshotCaptured(
        const blink::SameDocNavigationScreenshotDestinationToken& destination_token, std::unique_ptr<CopyOutputResult> copy_output_result) override
    {
...
class SurfaceAggregatorCopyRequestAgainstPreviousSurfaceTest : public SurfaceAggregatorValidSurfaceTest, public ::testing::WithParamInterface<bool> {
public:
    ...
    bool DestroyFrameSinkBeforeResult()
    {
        return GetParam();
    }
};
...
TEST_P(SurfaceAggregatorCopyRequestAgainstPreviousSurfaceTest, CopyAgainstPreviousSurface)
{
    ...
    // Submit one frame against the previous child surface.
    {
        ...
        std::vector<Quad> quads = { Quad::SolidColorQuad(SkColors::kGreen, gfx::Rect(5, 5)) };
        std::vector<Pass> passes = { Pass(quads, CompositorRenderPassId { 1 }, gfx::Size(100, 100)) };
        AddPasses(&new_frame.render_pass_list, passes, &new_frame.metadata.referenced_surfaces);
        child_sink_->SubmitCompositorFrame(prev_sid.local_surface_id(), std::move(new_frame));
    }
    ...
    // Submit a frame against the root surface.
    {
        ...
        std::vector<Quad> quads = { Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(5, 5)) };
        std::vector<Pass> passes = { Pass(quads, CompositorRenderPassId { 2 }, gfx::Size(100, 100)) };
        AddPasses(&new_frame.render_pass_list, passes, &new_frame.metadata.referenced_surfaces);
        root_sink_->SubmitCompositorFrame(root_sid.local_surface_id(), std::move(new_frame));
    }
    ...
    // reachable.
    {
        ...
        std::vector<Quad> quads = { Quad::SolidColorQuad(SkColors::kBlue, gfx::Rect(5, 5)) };
        std::vector<Pass> passes = { Pass(quads, CompositorRenderPassId { 3 }, gfx::Size(100, 100)) };
        AddPasses(&new_frame.render_pass_list, passes, &new_frame.metadata.referenced_surfaces);
        root_sink_->SubmitCompositorFrame(root_sid.local_surface_id(), std::move(new_frame));
    }
    ...
    const auto expected_token = base::UnguessableToken::Create();
    {
        ...
        CompositorFrame new_frame = MakeEmptyCompositorFrame();
        new_frame.metadata.screenshot_destination = blink::SameDocNavigationScreenshotDestinationToken(expected_token);
        std::vector<Quad> quads = { Quad::SolidColorQuad(SkColors::kRed, gfx::Rect(5, 5)) };
        std::vector<Pass> passes = { Pass(quads, CompositorRenderPassId { 4 }, gfx::Size(100, 100)) };
        AddPasses(&new_frame.render_pass_list, passes, &new_frame.metadata.referenced_surfaces);
        child_sink_->SubmitCompositorFrame(current_sid.local_surface_id(), std::move(new_frame));
    }

    // Check that the current child surface has `pending_copy_surface_id_` set.
    ASSERT_EQ(manager_.surface_manager()->GetSurfaceForId(current_sid)->pending_copy_surface_id_for_testing(), prev_sid);

    // Check the references.
    ASSERT_THAT(manager_.surface_manager()->GetSurfacesReferencedByParent(current_sid), ::testing::UnorderedElementsAre(prev_sid));
    ASSERT_THAT(manager_.surface_manager()->GetSurfacesThatReferenceChildForTesting(prev_sid), ::testing::UnorderedElementsAre(current_sid));
    ...
    if (DestroyFrameSinkBeforeResult()) {
        ...
        ASSERT_TRUE(manager_.surface_manager()->GetSurfacesReferencedByParent(current_sid).empty());
        ASSERT_TRUE(manager_.surface_manager()->GetSurfacesThatReferenceChildForTesting(prev_sid).empty());
    } else {
        auto empty_result = std::make_unique<CopyOutputResult>(CopyOutputResult::Format::RGBA, CopyOutputResult::Destination::kSystemMemory, gfx::Rect(),
            /*needs_lock_for_bitmap=*/false);
        result.render_pass_list[0]->copy_requests[0]->SendResult(std::move(empty_result));
        {
            SCOPED_TRACE("Waiting for OnScreenshotCaptured()");
            waiter.Wait();
        }
        ASSERT_EQ(waiter.observed_token().value(), expected_token);
        ASSERT_FALSE(manager_.surface_manager()->GetSurfaceForId(current_sid)->pending_copy_surface_id_for_testing().is_valid());
        ASSERT_TRUE(manager_.surface_manager()->GetSurfacesReferencedByParent(current_sid).empty());
        ASSERT_TRUE(manager_.surface_manager()->GetSurfacesThatReferenceChildForTesting(prev_sid).empty());
    }
}

INSTANTIATE_TEST_SUITE_P(All, SurfaceAggregatorCopyRequestAgainstPreviousSurfaceTest, ::testing::Bool(), &DescribeParam);
