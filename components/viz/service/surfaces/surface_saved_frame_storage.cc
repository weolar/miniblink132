...
base::flat_set<SharedElementResourceId> SurfaceSavedFrameStorage::ProcessSaveDirective(
    const CompositorFrameTransitionDirective& directive, SurfaceSavedFrame::TransitionDirectiveCompleteCallback directive_finished_callback)
{
    ...
    saved_frame_ = std::make_unique<SurfaceSavedFrame>(std::move(directive), std::move(directive_finished_callback));
    ...
    expiry_closure_.Reset(base::BindOnce(&SurfaceSavedFrameStorage::ExpireSavedFrame, base::Unretained(this)));
    base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(FROM_HERE, expiry_closure_.callback(), kExpiryTime);
