...
public:
    SurfaceManager(SurfaceManagerDelegate* delegate, std::optional<uint32_t> activation_deadline_in_frames, size_t max_uncommitted_frames);
    ...
    // deadline).
    void SetActivationDeadlineInFramesForTesting(std::optional<uint32_t> deadline);
    ...
    Surface* CreateSurface(base::WeakPtr<SurfaceClient> surface_client, const SurfaceInfo& surface_info, const SurfaceId& pending_copy_surface_id);
    ...
    void AddObserver(SurfaceObserver* obs)
    {
    ...
    bool SurfaceModified(const SurfaceId& surface_id, const BeginFrameAck& ack, SurfaceObserver::HandleInteraction handle_interaction);
    ...
    void SurfaceDamageExpected(const SurfaceId& surface_id, const BeginFrameArgs& args);
    ...
    const base::flat_set<SurfaceId>& GetSurfacesReferencedByParent(const SurfaceId& surface_id) const;
    ...
    base::flat_set<SurfaceId> GetSurfacesThatReferenceChildForTesting(const SurfaceId& surface_id) const;
    ...
    base::TimeTicks GetSurfaceReferencedTimestamp(const SurfaceId& surface_id) const;
    ...
    SurfaceAllocationGroup* GetOrCreateAllocationGroupForSurfaceId(const SurfaceId& surface_id);
    ...
    SurfaceAllocationGroup* GetAllocationGroupForSurfaceId(const SurfaceId& surface_id);
    ...
    void CommitFramesInRangeRecursively(const SurfaceRange& range, const CommitPredicate& predicate);
...
private:
    ...
    void RemoveTemporaryReferenceImpl(const SurfaceId& surface_id, RemovedReason reason);
    ...
    void SurfaceReferencesToStringImpl(const SurfaceId& surface_id, std::string indent, std::stringstream* str);
#endif
    ...
    base::flat_map<base::UnguessableToken, std::unique_ptr<SurfaceAllocationGroup>> embed_token_to_allocation_group_;
    base::flat_map<FrameSinkId, std::vector<raw_ptr<SurfaceAllocationGroup, VectorExperimental>>> frame_sink_id_to_allocation_groups_;
    ...
    std::unordered_map<SurfaceId, base::flat_set<SurfaceId>, SurfaceIdHash> references_;

    // A map of surfaces that have temporary references.
    std::unordered_map<SurfaceId, TemporaryReferenceData, SurfaceIdHash> temporary_references_;

    // A map of pair(the timestamp of the first time a surface gets referenced,
    // the number of references that surface has).
    std::unordered_map<SurfaceId, std::pair<base::TimeTicks, uint32_t>, SurfaceIdHash> surface_referenced_timestamps_;
    ...
    std::unordered_map<FrameSinkId, std::vector<LocalSurfaceId>, FrameSinkIdHash> temporary_reference_ranges_;
    ...
    std::optional<uint64_t> cooldown_frames_for_ack_on_activation_during_interaction_;
};
