...
public:
    using CopyFinishedCallback = base::OnceCallback<void(const CompositorFrameTransitionDirective&)>;
    ...
    static std::unique_ptr<SurfaceSavedFrame> CreateForTesting(CompositorFrameTransitionDirective directive);

    SurfaceSavedFrame(CompositorFrameTransitionDirective directive, gpu::SharedImageInterface* shared_image_interface);
    ~SurfaceSavedFrame();
    ...
    // frame.
    void RequestCopyOfOutput(Surface* surface, CopyFinishedCallback finished_callback);
...
private:
    explicit SurfaceSavedFrame(base::PassKey<SurfaceSavedFrame>, CompositorFrameTransitionDirective directive);
    ...
    void NotifyCopyOfOutputComplete(size_t shared_index, std::unique_ptr<CopyOutputResult> result);
    ...
    void PrepareForCopy(const CompositorRenderPassList& render_passes, CompositorRenderPassId& max_id,
        base::flat_map<CompositorRenderPassId, CompositorRenderPassId>& tainted_to_clean_pass_ids) const;
    ...
    bool FilterSharedElementAndTaintedQuads(const base::flat_map<CompositorRenderPassId, CompositorRenderPassId>* tainted_to_clean_pass_ids,
        const DrawQuad& quad, CompositorRenderPass& copy_pass) const;
    ...
    base::flat_map<size_t, scoped_refptr<gpu::ClientSharedImage>> blit_shared_images_;
