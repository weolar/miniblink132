...
void GetSurfaceReferenceDifference(const SurfaceId& parent_surface_id, const base::flat_set<SurfaceId>& old_referenced_surfaces,
    const base::flat_set<SurfaceId>& new_referenced_surfaces, std::vector<SurfaceReference>* references_to_add,
    std::vector<SurfaceReference>* references_to_remove)
{
    ...
    while (old_it != old_end || new_it != new_end) {
        if (old_it == old_end) {
            references_to_add->push_back(SurfaceReference(parent_surface_id, *new_it++));
        } else if (new_it == new_end) {
            references_to_remove->push_back(SurfaceReference(parent_surface_id, *old_it++));
