...
public:
    ...
    virtual void RefResources(const std::vector<TransferableResource>& resources) = 0;
    ...
    // compositor.
    virtual void ReceiveFromChild(const std::vector<TransferableResource>& resources) = 0;

    // Takes all the CopyOutputRequests made at the client level that happened for
    // a LocalSurfaceId preceeding the given one.
    virtual std::vector<PendingCopyOutputRequest> TakeCopyOutputRequests(const LocalSurfaceId& latest_surface_id) = 0;
    ...
    virtual void OnSurfaceAggregatedDamage(Surface* surface, const LocalSurfaceId& local_surface_id, const CompositorFrame& frame, const gfx::Rect& damage_rect,
