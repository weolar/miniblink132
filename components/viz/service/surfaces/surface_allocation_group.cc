...
SurfaceAllocationGroup::SurfaceAllocationGroup(SurfaceManager* surface_manager, const FrameSinkId& submitter, const base::UnguessableToken& embed_token)
...
{
    return surfaces_.empty() && active_embedders_.empty() && blocked_embedders_.empty();
}

void SurfaceAllocationGroup::RegisterSurface(Surface* surface)
{
    DCHECK_EQ(submitter_, surface->surface_id().frame_sink_id());
    DCHECK_EQ(embed_token_, surface->surface_id().local_surface_id().embed_token());
    DCHECK(!last_created_surface() || surface->surface_id().IsNewerThan(last_created_surface()->surface_id()));
...
void SurfaceAllocationGroup::RegisterBlockedEmbedder(Surface* surface, const SurfaceId& activation_dependency)
{
    blocked_embedders_[surface] = activation_dependency;
}

void SurfaceAllocationGroup::UnregisterBlockedEmbedder(Surface* surface, bool did_activate)
{
...
void SurfaceAllocationGroup::UpdateLastActiveReferenceAndMaybeActivate(const SurfaceId& surface_id)
{
    DCHECK_EQ(submitter_, surface_id.frame_sink_id());
    DCHECK_EQ(embed_token_, surface_id.local_surface_id().embed_token());
    if (last_active_reference_.is_valid() && last_active_reference_.IsSameOrNewerThan(surface_id)) {
        return;
    }
...
void SurfaceAllocationGroup::UpdateLastPendingReferenceAndMaybeActivate(const SurfaceId& surface_id)
{
    UpdateLastReferenceAndMaybeActivate(surface_id);
}
...
Surface* SurfaceAllocationGroup::FindLatestActiveSurfaceInRange(const SurfaceRange& range) const
{
    ...
    if (range.end().local_surface_id().embed_token() == embed_token_) {
        ...
        if (it != surfaces_.end() && (!range.start() || !range.start()->IsNewerThan((*it)->surface_id()))) {
            return *it;
        } else {
            return nullptr;
        }
...
void SurfaceAllocationGroup::TakeAggregatedLatencyInfoUpTo(Surface* surface, std::vector<ui::LatencyInfo>* out)
{
...
std::vector<raw_ptr<Surface, VectorExperimental>>::const_iterator SurfaceAllocationGroup::FindLatestSurfaceUpTo(const SurfaceId& surface_id) const
{
    DCHECK_EQ(submitter_, surface_id.frame_sink_id());
    DCHECK_EQ(embed_token_, surface_id.local_surface_id().embed_token());
...
std::vector<raw_ptr<Surface, VectorExperimental>>::const_iterator SurfaceAllocationGroup::FindLatestActiveSurfaceUpTo(const SurfaceId& surface_id) const
{
...
void SurfaceAllocationGroup::UpdateLastReferenceAndMaybeActivate(const SurfaceId& surface_id)
{
