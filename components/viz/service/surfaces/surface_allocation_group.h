...
public:
    SurfaceAllocationGroup(SurfaceManager* surface_manager, const FrameSinkId& submitter, const base::UnguessableToken& embed_token);
    ...
    // allocation group.
    const FrameSinkId& submitter_frame_sink_id() const
    {
        return submitter_;
    }
    ...
    void RegisterBlockedEmbedder(Surface* surface, const SurfaceId& activation_dependency);
    ...
    // |surface|.
    void TakeAggregatedLatencyInfoUpTo(Surface* surface, std::vector<ui::LatencyInfo>* out);
...
private:
    ...
    // yet.
    std::vector<raw_ptr<Surface, VectorExperimental>>::const_iterator FindLatestSurfaceUpTo(const SurfaceId& surface_id) const;

    // Returns an iterator to the latest active surface in |surfaces_| whose
    // SurfaceId is older than or equal to |surface_id|.
    std::vector<raw_ptr<Surface, VectorExperimental>>::const_iterator FindLatestActiveSurfaceUpTo(const SurfaceId& surface_id) const;
