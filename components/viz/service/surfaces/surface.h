...
public:
    ...
    public:
        PresentationHelper(base::WeakPtr<SurfaceClient> surface_client, uint32_t frame_token);
        ...
        void DidPresent(base::TimeTicks draw_start_timestamp, const gfx::SwapTimings& timings, const gfx::PresentationFeedback& feedback);
    ...
    using PresentedCallback = base::OnceCallback<void(const gfx::PresentationFeedback&)>;
    enum QueueFrameResult { REJECTED, ACCEPTED_ACTIVE, ACCEPTED_PENDING };

    using CommitPredicate = base::FunctionRef<bool(const SurfaceId&, const BeginFrameId&)>;

    // `pending_copy_surface_id`, when valid, becomes an
    // `active_referenced_surfaces_` of `this`.
    Surface(const SurfaceInfo& surface_info, SurfaceManager* surface_manager, SurfaceAllocationGroup* allocation_group,
    ...
    void SetDependencyDeadline(std::unique_ptr<SurfaceDependencyDeadline> deadline);
    ...
    base::WeakPtr<SurfaceClient> client()
    {
        return surface_client_;
    }

    bool has_deadline() const
    {
        return deadline_ && deadline_->has_deadline();
    }
    ...
    QueueFrameResult QueueFrame(CompositorFrame frame, uint64_t frame_index, base::ScopedClosureRunner frame_rejected_callback);
    ...
    void RequestCopyOfOutput(PendingCopyOutputRequest pending_copy_output_request);

    using CopyRequestsMap = std::multimap<CompositorRenderPassId, std::unique_ptr<CopyOutputRequest>>;
    ...
    {
        return active_frame_data_ ? active_frame_data_->frame_index : kInvalidFrameIndex;
    }

    void TakeActiveLatencyInfo(std::vector<ui::LatencyInfo>* latency_info);
    void TakeActiveAndPendingLatencyInfo(std::vector<ui::LatencyInfo>* latency_info);
    // Callers of this function must call |DidPresent| on the returned
    // PresentationHelper, at the appropriate point in the future.
    std::unique_ptr<Surface::PresentationHelper> TakePresentationHelperForPresentNotification();
    void SendAckToClient();
    void MarkAsDrawn();
    void NotifyAggregatedDamage(const gfx::Rect& damage_rect, base::TimeTicks expected_display_time);
    ...
    {
        return active_frame_data_.has_value();
    }
    bool HasPendingFrame() const
    {
        return pending_frame_data_.has_value();
    }
    ...
    SurfaceAllocationGroup* allocation_group() const
    {
        return allocation_group_;
    }
    ...
    // is already taken.
    bool is_latency_info_taken()
    {
        return is_latency_info_taken_;
    }
    ...
    void OnActivationDependencyResolved(const SurfaceId& activation_dependency, SurfaceAllocationGroup* group);
    ...
    base::WeakPtr<Surface> GetWeakPtr()
    {
        return weak_factory_.GetWeakPtr();
    }

    // Always placed the given |copy_request| on the root render pass.
    void RequestCopyOfOutputOnRootRenderPass(std::unique_ptr<CopyOutputRequest> copy_request);
    ...
    // otherwise.
    bool RequestCopyOfOutputOnActiveFrameRenderPassId(std::unique_ptr<CopyOutputRequest> copy_request, CompositorRenderPassId render_pass_id);
    ...
    std::optional<uint64_t> GetUncommitedFrameIndexNewerThan(uint64_t frame_index);
...
private:
    ...
    // appropriate.
    void UpdateReferencedAllocationGroups(std::vector<SurfaceAllocationGroup*> new_referenced_allocation_groups);
    ...
    static void TakeLatencyInfoFromFrame(CompositorFrame* frame, std::vector<ui::LatencyInfo>* latency_info);
    ...
    base::flat_set<raw_ptr<SurfaceAllocationGroup, CtnExperimental>> referenced_allocation_groups_;
    ...
    base::flat_set<raw_ptr<SurfaceAllocationGroup, CtnExperimental>> blocking_allocation_groups_;
