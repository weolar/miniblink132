...
void RequestCopyOfOutputOnRenderPass(std::unique_ptr<CopyOutputRequest> request, CompositorRenderPass& render_pass)
{
    if (request->has_source()) {
        const base::UnguessableToken& source = request->source();
        // Remove existing CopyOutputRequests made on the Surface by the same
        // source.
        std::erase_if(render_pass.copy_requests, [&source](const std::unique_ptr<CopyOutputRequest>& x) { return x->has_source() && x->source() == source; });
    }
...
Surface::PresentationHelper::PresentationHelper(base::WeakPtr<SurfaceClient> surface_client, uint32_t frame_token)
...
Surface::PresentationHelper::~PresentationHelper()
{
    ...
    DidPresent(base::TimeTicks(), gfx::SwapTimings(), gfx::PresentationFeedback::Failure());
}

void Surface::PresentationHelper::DidPresent(
    base::TimeTicks draw_start_timestamp, const gfx::SwapTimings& swap_timings, const gfx::PresentationFeedback& feedback)
{
    if (surface_client_) {
        surface_client_->OnSurfacePresented(frame_token_, draw_start_timestamp, swap_timings, feedback);
    }

    surface_client_ = nullptr;
}

Surface::Surface(const SurfaceInfo& surface_info, SurfaceManager* surface_manager, SurfaceAllocationGroup* allocation_group,
    base::WeakPtr<SurfaceClient> surface_client, const SurfaceId& pending_copy_surface_id, size_t max_uncommitted_frames)
...
{
    TRACE_EVENT_ASYNC_BEGIN1(TRACE_DISABLED_BY_DEFAULT("viz.surface_lifetime"), "Surface", this, "surface_info", surface_info.ToString());
    allocation_group_->RegisterSurface(this);
    is_fallback_ = allocation_group_->GetLastReference().IsNewerThan(surface_id());
}
...
Surface::~Surface()
{
    ...
    TRACE_EVENT_ASYNC_END1(TRACE_DISABLED_BY_DEFAULT("viz.surface_lifetime"), "Surface", this, "surface_info", surface_info_.ToString());
...
void Surface::SetDependencyDeadline(std::unique_ptr<SurfaceDependencyDeadline> deadline)
{
...
{
    const base::flat_set<SurfaceId>& existing_referenced_surfaces = surface_manager_->GetSurfacesReferencedByParent(surface_id());
    ...
    GetSurfaceReferenceDifference(surface_id(), existing_referenced_surfaces, active_referenced_surfaces(), &references_to_add, &references_to_remove);
...
void Surface::ActivateIfDeadlinePassed()
{
    ...
    if (!deadline_->HasDeadlinePassed())
        return;
    TRACE_EVENT1("viz", "Surface deadline passed", "FrameSinkId", surface_id().frame_sink_id().ToString());
    ActivatePendingFrameForDeadline();
}

Surface::QueueFrameResult Surface::QueueFrame(CompositorFrame frame, uint64_t frame_index, base::ScopedClosureRunner frame_rejected_callback)
{
    if (frame.size_in_pixels() != surface_info_.size_in_pixels() || frame.device_scale_factor() != surface_info_.device_scale_factor()) {
        TRACE_EVENT_INSTANT0("viz", "Surface invariants violation", TRACE_EVENT_SCOPE_THREAD);
    ...
    if (!max_uncommitted_frames_) {
        ...
        if (uncommitted_frames_.size() == max_uncommitted_frames_) {
            TRACE_EVENT_INSTANT1("viz", "DropUncommitedFrame", TRACE_EVENT_SCOPE_THREAD, "queue_length", uncommitted_frames_.size());

            UnrefFrameResourcesAndRunCallbacks(std::move(uncommitted_frames_.front()));
        ...
        if (uncommitted_frames_.size() < max_uncommitted_frames_) {
            TRACE_EVENT_INSTANT1("viz", "AckingUncommitedFrame", TRACE_EVENT_SCOPE_THREAD, "queue_length", uncommitted_frames_.size());
...
{
    TRACE_EVENT1("viz", "Surface::CommitFrame", "SurfaceId", surface_id().ToString());
    ...
    std::optional<FrameData> previous_pending_frame_data = std::move(pending_frame_data_);
    ...
    if (activation_dependencies_.empty()) {
        ...
        TRACE_EVENT_NESTABLE_ASYNC_BEGIN2("viz", "SurfaceQueuedPending", TRACE_ID_LOCAL(this), "LocalSurfaceId", surface_info_.id().ToString(),
            "ActivationDependencies", std::move(traced_value));
...
void Surface::RequestCopyOfOutput(PendingCopyOutputRequest pending_copy_output_request)
{
    TRACE_EVENT1("viz", "Surface::RequestCopyOfOutput", "has_active_frame_data", !!active_frame_data_);

    if (!pending_copy_output_request.subtree_capture_id.is_valid()) {
        RequestCopyOfOutputOnRootRenderPass(std::move(pending_copy_output_request.copy_output_request));
        return;
    }
    ...
    for (auto& render_pass : GetActiveFrame().render_pass_list) {
        if (render_pass->subtree_capture_id == pending_copy_output_request.subtree_capture_id) {
            RequestCopyOfOutputOnRenderPass(std::move(pending_copy_output_request.copy_output_request), *render_pass);
            return;
        }
    }
}

void Surface::RequestCopyOfOutputOnRootRenderPass(std::unique_ptr<CopyOutputRequest> copy_request)
{
    TRACE_EVENT1("viz", "Surface::RequestCopyOfOutputOnRootRenderPass", "has_active_frame_data", !!active_frame_data_);
    if (!active_frame_data_)
        return; // |copy_request| auto-sends empty result on out-of-scope.

    RequestCopyOfOutputOnRenderPass(std::move(copy_request), *GetActiveFrame().render_pass_list.back());
}

bool Surface::RequestCopyOfOutputOnActiveFrameRenderPassId(std::unique_ptr<CopyOutputRequest> copy_request, CompositorRenderPassId render_pass_id)
{
    TRACE_EVENT1("viz", "Surface::RequestCopyOfOurpurRenderPassId", "has_active_frame_data", !!active_frame_data_);
    if (!active_frame_data_)
        return false;
...
void Surface::OnActivationDependencyResolved(const SurfaceId& activation_dependency, SurfaceAllocationGroup* group)
{
...
void Surface::ActivatePendingFrameForDeadline()
{
    ...
    if (!activation_dependencies_.empty()) {
        TRACE_EVENT_NESTABLE_ASYNC_END0("viz", "SurfaceQueuedPending", TRACE_ID_LOCAL(this));
    }
...
void Surface::ActivatePendingFrame()
{
    ...
    if (duration.has_value()) {
        TRACE_EVENT_INSTANT2("viz", "SurfaceSynchronizationEvent", TRACE_EVENT_SCOPE_THREAD, "surface_id", surface_info_.id().ToString(), "duration_ms",
...
{
    TRACE_EVENT1("viz", "Surface::CommitFramesRecursively", "SurfaceId", surface_id().ToString());
    ...
    while (!uncommitted_frames_.empty()) {
        const auto& ack = uncommitted_frames_.front().frame.metadata.begin_frame_ack;
...
std::optional<uint64_t> Surface::GetUncommitedFrameIndexNewerThan(uint64_t frame_index)
{
    for (auto& frame : uncommitted_frames_) {
...
void Surface::UpdateReferencedAllocationGroups(std::vector<SurfaceAllocationGroup*> new_referenced_allocation_groups)
{
    base::flat_set<raw_ptr<SurfaceAllocationGroup, CtnExperimental>> new_set(new_referenced_allocation_groups.begin(), new_referenced_allocation_groups.end());
...
void Surface::RecomputeActiveReferencedSurfaces()
{
    ...
    for (const SurfaceRange& surface_range : active_frame_data_->frame.metadata.referenced_surfaces) {
        // Figure out what surface in the |surface_range| needs to be referenced.
        Surface* surface = surface_manager_->GetLatestInFlightSurface(surface_range);
        if (surface) {
        ...
        // referenced.
        SurfaceAllocationGroup* end_allocation_group = surface_manager_->GetOrCreateAllocationGroupForSurfaceId(surface_range.end());
        if (end_allocation_group) {
            new_referenced_allocation_groups.push_back(end_allocation_group);
            end_allocation_group->UpdateLastActiveReferenceAndMaybeActivate(surface_range.end());
        }
        // Only reference the allocation group for the start of SurfaceRange if the
        // current referenced surface is a part of it.
        if (surface_range.HasDifferentEmbedTokens() && (!surface || surface->surface_id().HasSameEmbedTokenAs(*surface_range.start()))) {
            SurfaceAllocationGroup* start_allocation_group = surface_manager_->GetOrCreateAllocationGroupForSurfaceId(*surface_range.start());
            if (start_allocation_group) {
                new_referenced_allocation_groups.push_back(start_allocation_group);
                start_allocation_group->UpdateLastActiveReferenceAndMaybeActivate(*surface_range.start());
            }
        }
    }
...
{
    TRACE_EVENT1("viz", "Surface::ActivateFrame", "SurfaceId", surface_id().ToString());
    ...
    if (!seen_first_frame_activation_) {
        TRACE_EVENT_WITH_FLOW2(TRACE_DISABLED_BY_DEFAULT("viz.surface_id_flow"), "LocalSurfaceId.Submission.Flow",
            TRACE_ID_GLOBAL(surface_info_.id().local_surface_id().submission_trace_id()), TRACE_EVENT_FLAG_FLOW_IN, "step", "FirstSurfaceActivation",
...
FrameDeadline Surface::ResolveFrameDeadline(const CompositorFrame& current_frame)
{
    ...
    // show.
    if (!HasActiveFrame() && allocation_group_->GetLastActiveReference() == surface_id()) {
        return FrameDeadline::MakeZero();
    }

    const std::optional<uint32_t>& default_deadline = surface_manager_->activation_deadline_in_frames();
    ...
    if (!default_deadline || deadline.use_default_lower_bound_deadline()) {
        deadline_in_frames = std::max(deadline_in_frames, default_deadline.value_or(std::numeric_limits<uint32_t>::max()));
    }

    return FrameDeadline(deadline.frame_start_time(), deadline_in_frames, deadline.frame_interval(), false /* use_default_lower_bound_deadline */);
}

void Surface::UpdateActivationDependencies(const CompositorFrame& current_frame)
{
    for (SurfaceAllocationGroup* group : blocking_allocation_groups_)
        group->UnregisterBlockedEmbedder(this, false /* did_activate */);
    blocking_allocation_groups_.clear();
    ...
    bool should_block_on_dependencies = ShouldBlockActivationOnDependenciesWhenInteractive() || !current_frame.metadata.is_handling_interaction;

    if (!should_block_on_dependencies) {
        return;
    }

    base::flat_set<raw_ptr<SurfaceAllocationGroup, CtnExperimental>> new_blocking_allocation_groups;
    std::vector<SurfaceId> new_activation_dependencies;
    for (const SurfaceId& surface_id : current_frame.metadata.activation_dependencies) {
        SurfaceAllocationGroup* group = surface_manager_->GetOrCreateAllocationGroupForSurfaceId(surface_id);
        ...
        TRACE_EVENT_WITH_FLOW2(TRACE_DISABLED_BY_DEFAULT("viz.surface_id_flow"), "LocalSurfaceId.Embed.Flow",
            TRACE_ID_GLOBAL(surface_id.local_surface_id().embed_trace_id()), TRACE_EVENT_FLAG_FLOW_IN | TRACE_EVENT_FLAG_FLOW_OUT, "step",
...
void Surface::TakeCopyOutputRequests(Surface::CopyRequestsMap* copy_requests)
{
    ...
    for (const auto& render_pass : GetActiveFrame().render_pass_list) {
        for (auto& request : render_pass->copy_requests) {
            copy_requests->insert(std::make_pair(render_pass->id, std::move(request)));
        }
...
void Surface::TakeCopyOutputRequestsFromClient()
{
    if (!surface_client_)
        return;
    for (PendingCopyOutputRequest& request_params : surface_client_->TakeCopyOutputRequests(surface_id().local_surface_id())) {
...
void Surface::TakeActiveLatencyInfo(std::vector<ui::LatencyInfo>* latency_info)
{
    if (!active_frame_data_)
        return;
...
void Surface::TakeActiveAndPendingLatencyInfo(std::vector<ui::LatencyInfo>* latency_info)
{
...
std::unique_ptr<Surface::PresentationHelper> Surface::TakePresentationHelperForPresentNotification()
{
    if (active_frame_data_ && !active_frame_data_->will_be_notified_of_presentation) {
        active_frame_data_->will_be_notified_of_presentation = true;
        return std::make_unique<PresentationHelper>(client(), active_frame_data_->frame.metadata.frame_token);
    }
    return nullptr;
}
...
void Surface::NotifyAggregatedDamage(const gfx::Rect& damage_rect, base::TimeTicks expected_display_time)
{
    if (!active_frame_data_ || !surface_client_)
        return;
    surface_client_->OnSurfaceAggregatedDamage(this, surface_id().local_surface_id(), active_frame_data_->frame, damage_rect, expected_display_time);
}
...
void Surface::UnrefFrameResourcesAndRunCallbacks(std::optional<FrameData> frame_data)
{
    if (!frame_data || !surface_client_)
        return;

    std::vector<ReturnedResource> resources = TransferableResource::ReturnResources(frame_data->frame.resource_list);
    ...
    if (!frame_data->will_be_notified_of_presentation && surface_client_) {
        surface_client_->OnSurfacePresented(
            frame_data->frame.metadata.frame_token, base::TimeTicks(), gfx::SwapTimings(), gfx::PresentationFeedback::Failure());
    }
...
void Surface::TakePendingLatencyInfo(std::vector<ui::LatencyInfo>* latency_info)
{
    if (!pending_frame_data_)
        return;
...
// static
void Surface::TakeLatencyInfoFromFrame(CompositorFrame* frame, std::vector<ui::LatencyInfo>* latency_info)
{
    if (latency_info->empty()) {
        frame->metadata.latency_info.swap(*latency_info);
        return;
    }
    base::ranges::copy(frame->metadata.latency_info, std::back_inserter(*latency_info));
    frame->metadata.latency_info.clear();
    if (!ui::LatencyInfo::Verify(*latency_info, "Surface::TakeLatencyInfoFromFrame")) {
...
void Surface::OnWillBeDrawn()
{
    if (!seen_first_surface_embedding_) {
        ...
        TRACE_EVENT_WITH_FLOW2(TRACE_DISABLED_BY_DEFAULT("viz.surface_id_flow"), "LocalSurfaceId.Embed.Flow",
            TRACE_ID_GLOBAL(surface_info_.id().local_surface_id().embed_trace_id()), TRACE_EVENT_FLAG_FLOW_IN, "step", "FirstSurfaceEmbedding", "surface_id",
