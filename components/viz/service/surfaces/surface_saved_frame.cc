...
constexpr auto kResultDestination = CopyOutputRequest::ResultDestination::kNativeTextures;
...
size_t GetSharedPassIndex(const std::vector<CompositorFrameTransitionDirective::SharedElement>& shared_elements, CompositorRenderPassId render_pass_id)
{
    size_t shared_element_index = 0;
    for (; shared_element_index < shared_elements.size(); ++shared_element_index) {
        if (shared_elements[shared_element_index].render_pass_id == render_pass_id) {
            break;
        }
    }
...
// static
std::unique_ptr<SurfaceSavedFrame> SurfaceSavedFrame::CreateForTesting(CompositorFrameTransitionDirective directive)
{
    return base::WrapUnique(new SurfaceSavedFrame(base::PassKey<SurfaceSavedFrame>(), std::move(directive)));
}

SurfaceSavedFrame::SurfaceSavedFrame(CompositorFrameTransitionDirective directive, gpu::SharedImageInterface* shared_image_interface)
    : directive_(std::move(directive))
    , shared_image_interface_(shared_image_interface)
    , use_blit_requests_(base::FeatureList::IsEnabled(features::kBlitRequestsForViewTransition))
{
...
SurfaceSavedFrame::SurfaceSavedFrame(base::PassKey<SurfaceSavedFrame>, CompositorFrameTransitionDirective directive)
    : directive_(std::move(directive))
    , use_blit_requests_(true)
{
...
base::flat_set<ViewTransitionElementResourceId> SurfaceSavedFrame::GetEmptyResourceIds() const
{
...
{
    return frame_result_ && (valid_result_count_ == ExpectedResultCount() || use_blit_requests_);
}

void SurfaceSavedFrame::RequestCopyOfOutput(Surface* surface, CopyFinishedCallback finished_callback)
{
    ...
    for (const auto& render_pass : active_frame.render_pass_list) {
        ...
        if (auto request = CreateCopyRequestIfNeeded(*render_pass, is_software, active_frame.metadata.content_color_usage)) {
            surface->RequestCopyOfOutputOnActiveFrameRenderPassId(std::move(request), render_pass->id);
            copy_request_count_++;
        }
    }
    ...
    if (use_blit_requests_) {
        for (auto& [index, shared_image] : blit_shared_images_) {
            ...
            slot->release_callback = base::BindOnce([](scoped_refptr<gpu::ClientSharedImage> image, const gpu::SyncToken& sync_token,
...
{
    base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE, base::BindOnce(std::move(directive_finished_callback_), directive_));
}

std::unique_ptr<CopyOutputRequest> SurfaceSavedFrame::CreateCopyRequestIfNeeded(
    const CompositorRenderPass& render_pass, bool is_software, gfx::ContentColorUsage content_color_usage)
{
    size_t shared_pass_index = GetSharedPassIndex(directive_.shared_elements(), render_pass.id);
    ...
    auto request = std::make_unique<CopyOutputRequest>(kResultFormat, kResultDestination,
...
bool SurfaceSavedFrame::IsSharedElementRenderPass(CompositorRenderPassId pass_id) const
{
...
void SurfaceSavedFrame::NotifyCopyOfOutputComplete(size_t shared_index, std::unique_ptr<CopyOutputResult> output_copy)
{
    ...
    if (output_copy->IsEmpty()) {
        LOG(ERROR) << "SurfaceSavedFrame copy output result for shared index " << shared_index << " is empty.";
        return;
    }
    ...
    OutputCopyResult* slot = &frame_result_->shared_results[shared_index].emplace();
    ...
    if (output_copy->destination() == CopyOutputResult::Destination::kSystemMemory) {
        ...
        CopyOutputResult::ReleaseCallbacks release_callbacks = output_copy->TakeTextureOwnership();
...
{
    SkBitmap bitmap;
    bitmap.allocPixels(SkImageInfo::MakeN32Premul(kDefaultTextureSizeForTesting.width(), kDefaultTextureSizeForTesting.height()));
...
SurfaceSavedFrame::RenderPassDrawData::RenderPassDrawData(const CompositorRenderPass& render_pass)
    : size(render_pass.output_rect.size())
{
}

SurfaceSavedFrame::OutputCopyResult::OutputCopyResult() = default;
SurfaceSavedFrame::OutputCopyResult::OutputCopyResult(OutputCopyResult&& other)
{
    *this = std::move(other);
}
...
SurfaceSavedFrame::OutputCopyResult& SurfaceSavedFrame::OutputCopyResult::operator=(OutputCopyResult&& other)
{
...
SurfaceSavedFrame::FrameResult& SurfaceSavedFrame::FrameResult::operator=(FrameResult&& other) = default;
