...
SurfaceObserver::HandleInteraction GetHandleInteraction(const CompositorFrameMetadata& metadata)
{
...
SurfaceManager::SurfaceManager(SurfaceManagerDelegate* delegate, std::optional<uint32_t> activation_deadline_in_frames, size_t max_uncommitted_frames)
    : delegate_(delegate)
    , activation_deadline_in_frames_(activation_deadline_in_frames)
    , root_surface_id_(FrameSinkId(0u, 0u), LocalSurfaceId(1u, base::UnguessableToken::Create()))
    , tick_clock_(base::DefaultTickClock::GetInstance())
    , max_uncommitted_frames_(max_uncommitted_frames)
    , cooldown_frames_for_ack_on_activation_during_interaction_(features::NumCooldownFramesForAckOnSurfaceActivationDuringInteraction())
{
...
SurfaceManager::~SurfaceManager()
{
    ...
    // mutate that set.
    base::flat_set<SurfaceId> children(GetSurfacesReferencedByParent(root_surface_id_));
...
void SurfaceManager::SetActivationDeadlineInFramesForTesting(std::optional<uint32_t> activation_deadline_in_frames)
{
    activation_deadline_in_frames_ = activation_deadline_in_frames;
}
...
Surface* SurfaceManager::CreateSurface(base::WeakPtr<SurfaceClient> surface_client, const SurfaceInfo& surface_info, const SurfaceId& pending_copy_surface_id)
{
    ...
    SurfaceAllocationGroup* allocation_group = GetOrCreateAllocationGroupForSurfaceId(surface_info.id());
    ...
    std::unique_ptr<Surface> surface
        = std::make_unique<Surface>(surface_info, this, allocation_group, surface_client, pending_copy_surface_id, max_uncommitted_frames_);
    surface->SetDependencyDeadline(std::make_unique<SurfaceDependencyDeadline>(tick_clock_));
...
void SurfaceManager::AddSurfaceReferences(const std::vector<SurfaceReference>& references)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void SurfaceManager::RemoveSurfaceReferences(const std::vector<SurfaceReference>& references)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void SurfaceManager::GarbageCollectSurfaces()
{
    ...
    for (auto iter = surfaces_to_destroy_.begin(); iter != surfaces_to_destroy_.end();) {
    ...
    for (const auto& iter : surfaces_to_delete) {
        ...
        UMA_HISTOGRAM_TIMES("Compositing.SurfaceManager.MarkForDestructionToDestroy", delta);
...
const base::flat_set<SurfaceId>& SurfaceManager::GetSurfacesReferencedByParent(const SurfaceId& surface_id) const
{
...
base::flat_set<SurfaceId> SurfaceManager::GetSurfacesThatReferenceChildForTesting(const SurfaceId& surface_id) const
{
...
base::TimeTicks SurfaceManager::GetSurfaceReferencedTimestamp(const SurfaceId& surface_id) const
{
    CHECK(surface_id.is_valid());
    auto surface_referenced_timestamp = surface_referenced_timestamps_.find(surface_id);
...
void SurfaceManager::AddSurfaceReferenceImpl(const SurfaceReference& reference)
{
    const SurfaceId& parent_id = reference.parent_id();
    const SurfaceId& child_id = reference.child_id();

    if (parent_id.frame_sink_id() == child_id.frame_sink_id() && !parent_id.IsNewerThan(child_id)) {
        // Only newer surfaces from the same client can keep an older surface alive.
        DLOG(ERROR) << "Cannot add self reference from " << parent_id << " to " << child_id;
        return;
    }
    ...
    if (surface_referenced_timestamps_.find(child_id) == surface_referenced_timestamps_.end()) {
        ...
        surface_referenced_timestamps_[child_id] = std::make_pair(base::TimeTicks::Now(), 1);
    } else {
...
void SurfaceManager::RemoveSurfaceReferenceImpl(const SurfaceReference& reference)
{
    const SurfaceId& parent_id = reference.parent_id();
    const SurfaceId& child_id = reference.child_id();
    ...
    CHECK(surface_referenced_timestamps_.find(child_id) != surface_referenced_timestamps_.end());
    surface_referenced_timestamps_[child_id].second--;
...
void SurfaceManager::AddTemporaryReference(const SurfaceId& surface_id)
{
    ...
    temporary_reference_ranges_[surface_id.frame_sink_id()].push_back(surface_id.local_surface_id());

    // Start timer to expire temporary references if it's not running.
    if (expire_timer_ && !expire_timer_->IsRunning()) {
        expire_timer_->Start(FROM_HERE, kExpireInterval, this, &SurfaceManager::ExpireOldTemporaryReferences);
    }
}

void SurfaceManager::RemoveTemporaryReferenceImpl(const SurfaceId& surface_id, RemovedReason reason)
{
    const FrameSinkId& frame_sink_id = surface_id.frame_sink_id();
    std::vector<LocalSurfaceId>& frame_sink_temp_refs = temporary_reference_ranges_[frame_sink_id];
    ...
    while (iter != frame_sink_temp_refs.end()) {
        ...
        if (!temp_id.HasSameEmbedTokenAs(surface_id) || temp_id.IsNewerThan(surface_id)) {
            ++iter;
            continue;
        }
...
Surface* SurfaceManager::GetLatestInFlightSurface(const SurfaceRange& surface_range)
{
    SurfaceAllocationGroup* end_allocation_group = GetAllocationGroupForSurfaceId(surface_range.end());
    if (end_allocation_group) {
        Surface* result = end_allocation_group->FindLatestActiveSurfaceInRange(surface_range);
        if (result)
            return result;
    }
    if (!surface_range.start() || surface_range.start()->local_surface_id().embed_token() == surface_range.end().local_surface_id().embed_token()) {
        return nullptr;
    }
    SurfaceAllocationGroup* start_allocation_group = GetAllocationGroupForSurfaceId(*surface_range.start());
...
void SurfaceManager::ExpireOldTemporaryReferences()
{
    ...
    for (auto& map_entry : temporary_references_) {
        ...
        if (data.marked_as_old) {
            ...
            if (delegate_) {
                frame_sink_debug_label = delegate_->GetFrameSinkDebugLabel(surface_id.frame_sink_id());
            }
            DLOG(ERROR) << "Old/orphaned temporary reference to " << surface_id.ToString(frame_sink_debug_label);
...
bool SurfaceManager::SurfaceModified(const SurfaceId& surface_id, const BeginFrameAck& ack, SurfaceObserver::HandleInteraction handle_interaction)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    bool changed = false;
...
void SurfaceManager::SurfaceActivated(Surface* surface)
{
    ...
    if (!SurfaceModified(surface->surface_id(), metadata.begin_frame_ack, GetHandleInteraction(metadata))) {
        TRACE_EVENT_INSTANT0("viz", "Damage not visible.", TRACE_EVENT_SCOPE_THREAD);
...
bool SurfaceManager::ShouldAckNonInteractiveFrame(const CompositorFrameMetadata& metadata) const
{
    if (!cooldown_frames_for_ack_on_activation_during_interaction_ || !last_interactive_frame_ || metadata.is_handling_interaction) {
        return false;
    }
    ...
    const uint64_t frames_since_interactive = metadata.begin_frame_ack.frame_id.sequence_number - last_interactive_frame_.value().sequence_number;
    return frames_since_interactive < cooldown_frames_for_ack_on_activation_during_interaction_.value();
}
...
void SurfaceManager::SurfaceDamageExpected(const SurfaceId& surface_id, const BeginFrameArgs& args)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    for (auto& observer : observer_list_)
...
#if DCHECK_IS_ON()
void SurfaceManager::SurfaceReferencesToStringImpl(const SurfaceId& surface_id, std::string indent, std::stringstream* str)
{
    *str << indent;
...
SurfaceAllocationGroup* SurfaceManager::GetOrCreateAllocationGroupForSurfaceId(const SurfaceId& surface_id)
{
    std::unique_ptr<SurfaceAllocationGroup>& allocation_group = embed_token_to_allocation_group_[surface_id.local_surface_id().embed_token()];
    if (allocation_group && allocation_group->submitter_frame_sink_id() != surface_id.frame_sink_id()) {
        DLOG(ERROR) << "Cannot reuse embed token across frame sinks";
        return nullptr;
    }
    if (!allocation_group) {
        allocation_group = std::make_unique<SurfaceAllocationGroup>(this, surface_id.frame_sink_id(), surface_id.local_surface_id().embed_token());
        frame_sink_id_to_allocation_groups_[surface_id.frame_sink_id()].push_back(allocation_group.get());
    }
    return allocation_group.get();
}

SurfaceAllocationGroup* SurfaceManager::GetAllocationGroupForSurfaceId(const SurfaceId& surface_id)
{
    auto it = embed_token_to_allocation_group_.find(surface_id.local_surface_id().embed_token());
...
void SurfaceManager::MaybeGarbageCollectAllocationGroups()
{
    ...
    for (auto it = embed_token_to_allocation_group_.begin(); it != embed_token_to_allocation_group_.end(); ++it) {
        if (!it->second->IsReadyToDestroy())
            continue;
        // Before destroying the allocation group, remove it from
        // |frame_sink_id_to_allocation_groups_|.
        auto list_it = frame_sink_id_to_allocation_groups_.find(it->second->submitter_frame_sink_id());
    ...
    if (did_destroy) {
        base::EraseIf(embed_token_to_allocation_group_, [](auto& entry) { return !entry.second; });
    }

    allocation_groups_need_garbage_collection_ = false;
}

bool SurfaceManager::HasBlockedEmbedder(const FrameSinkId& frame_sink_id) const
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    auto it = frame_sink_id_to_allocation_groups_.find(frame_sink_id);
    if (it == frame_sink_id_to_allocation_groups_.end())
...
void SurfaceManager::CommitFramesInRangeRecursively(const SurfaceRange& range, const CommitPredicate& predicate)
{
    ...
    if (range.start() && range.start()->local_surface_id().embed_token() != range.end().local_surface_id().embed_token()) {
        if (auto* allocation_group = GetAllocationGroupForSurfaceId(*range.start())) {
