...
public:
    SurfaceTest()
        : frame_sink_manager_(FrameSinkManagerImpl::InitParams(&shared_bitmap_manager_))
    {
    }
...
class OnBeginFrameAcksSurfaceTest : public SurfaceTest, public testing::WithParamInterface<bool> {
public:
...
TEST_P(OnBeginFrameAcksSurfaceTest, PresentationCallback)
{
    ...
    auto support = std::make_unique<CompositorFrameSinkSupport>(&client, &frame_sink_manager_, kArbitraryFrameSinkId, kIsRoot);
    if (BeginFrameAcksEnabled()) {
        support->SetWantsBeginFrameAcks();
    }
    uint32_t frame_token = kInvalidFrameToken;
    {
        CompositorFrame frame = CompositorFrameBuilder().AddRenderPass(gfx::Rect(kSurfaceSize), kDamageRect).SetBeginFrameSourceId(kBeginFrameSourceId).Build();
        frame_token = frame.metadata.frame_token;
        ASSERT_NE(frame_token, kInvalidFrameToken);
        EXPECT_CALL(client, DidReceiveCompositorFrameAck(testing::_)).Times(BeginFrameAcksEnabled() ? 0 : 1);
...
INSTANTIATE_TEST_SUITE_P(, OnBeginFrameAcksSurfaceTest, testing::Bool(), [](auto& info) { return info.param ? "BeginFrameAcks" : "CompositoFrameAcks"; });
...
void TestCopyResultCallback(bool* called, base::OnceClosure finished, std::unique_ptr<CopyOutputResult> result)
{
    *called = true;
...
TEST_F(SurfaceTest, CopyRequestLifetime)
{
    ...
    auto support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &frame_sink_manager_, kArbitraryFrameSinkId, kIsRoot);
    ...
    support->RequestCopyOfOutput(PendingCopyOutputRequest { local_surface_id, SubtreeCaptureId(),
        std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory,
            base::BindOnce(&TestCopyResultCallback, &copy_called, copy_runloop.QuitClosure())) });
    ...
    for (int i = 0; i < max_frame; ++i) {
        ...
        frame.render_pass_list.back()->id = CompositorRenderPassId { i * 3 + start_id };
        frame.render_pass_list.push_back(CompositorRenderPass::Create());
        frame.render_pass_list.back()->id = CompositorRenderPassId { i * 3 + start_id + 1 };
        frame.render_pass_list.push_back(CompositorRenderPass::Create());
        frame.render_pass_list.back()->SetNew(CompositorRenderPassId { i * 3 + start_id + 2 }, gfx::Rect(0, 0, 20, 20), gfx::Rect(), gfx::Transform());
        support->SubmitCompositorFrame(local_surface_id, std::move(frame));
    }
    ...
    // is called.
    EXPECT_FALSE(copy_called);
    EXPECT_EQ(1u, surface->GetActiveFrame().render_pass_list.back()->copy_requests.size());
...
TEST_F(SurfaceTest, ActiveSurfaceReferencesWithOverlappingReferences)
{
    ...
    auto root_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &frame_sink_manager_, kArbitraryFrameSinkId,
        /*is_root=*/true);
    TestSurfaceIdAllocator root_surface_id(kArbitraryFrameSinkId);

    auto child_support1 = std::make_unique<CompositorFrameSinkSupport>(nullptr, &frame_sink_manager_, FrameSinkId(2, 1), /*is_root=*/false);
    TestSurfaceIdAllocator child_surface_id1(child_support1->frame_sink_id());

    auto child_support2 = std::make_unique<CompositorFrameSinkSupport>(nullptr, &frame_sink_manager_, FrameSinkId(3, 1), /*is_root=*/false);
    ...
    auto root_render_pass = RenderPassBuilder(CompositorRenderPassId { 1 }, output_rect)
...

    {
        ...
        EXPECT_THAT(frame.metadata.referenced_surfaces, testing::ElementsAre(old_surface_range, navigation_surface_range));

        root_support->SubmitCompositorFrame(root_surface_id.local_surface_id(), std::move(frame));
    }
    ...
    child_support2->SubmitCompositorFrame(child_surface_id2.local_surface_id(), MakeDefaultCompositorFrame(kBeginFrameSourceId));
    EXPECT_TRUE(surface_manager->GetSurfaceForId(child_surface_id2));
    EXPECT_THAT(root_surface->active_referenced_surfaces(), testing::ElementsAre(child_surface_id2));
    ...
    child_support1->SubmitCompositorFrame(child_surface_id1.local_surface_id(), MakeDefaultCompositorFrame(kBeginFrameSourceId));
    EXPECT_TRUE(surface_manager->GetSurfaceForId(child_surface_id1));
    EXPECT_THAT(root_surface->active_referenced_surfaces(), testing::ElementsAre(child_surface_id1, child_surface_id2));

    // Resubmit root frame with the same SurfaceDrawQuads and verify active
    // surface references are unchanged.
    root_support->SubmitCompositorFrame(root_surface_id.local_surface_id(), MakeCompositorFrame(std::move(root_render_pass)));
    EXPECT_THAT(root_surface->active_referenced_surfaces(), testing::ElementsAre(child_surface_id1, child_surface_id2));
    ...
    // surfaces.
    {
        SurfaceRange post_navigation_surface_range(child_surface_id2, child_surface_id2);
        CompositorFrame frame
            = CompositorFrameBuilder()
                  .AddRenderPass(RenderPassBuilder(CompositorRenderPassId { 1 }, output_rect).AddSurfaceQuad(output_rect, post_navigation_surface_range))
                  .Build();
...
TEST_F(SurfaceTest, PendingCopySurfaceIncludedInActiveReferencedSurfaces)
{
    ...
    auto support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &frame_sink_manager_, kArbitraryFrameSinkId,
        /*is_root=*/false);
...

    {
        CompositorFrame frame = MakeCompositorFrame(
            RenderPassBuilder(CompositorRenderPassId { 1 }, rect).AddSolidColorQuad(rect, SkColors::kBlue).AddSolidColorQuad(rect, SkColors::kBlue).Build());
        support->SubmitCompositorFrame(prev_id.local_surface_id(), std::move(frame));
    }
    {
        CompositorFrame frame = MakeCompositorFrame(
            RenderPassBuilder(CompositorRenderPassId { 2 }, rect).AddSolidColorQuad(rect, SkColors::kBlue).AddSolidColorQuad(rect, SkColors::kBlue).Build());
        frame.metadata.screenshot_destination = blink::SameDocNavigationScreenshotDestinationToken(base::UnguessableToken::Create());
        support->SubmitCompositorFrame(curr_id.local_surface_id(), std::move(frame));
    }

    auto* curr_surface = surface_manager->GetSurfaceForId(curr_id);
    ASSERT_TRUE(curr_surface);
    ASSERT_THAT(curr_surface->active_referenced_surfaces(), ::testing::UnorderedElementsAre(prev_id));
...
class ImmediateActivationSurfaceTest : public SurfaceTest, public testing::WithParamInterface<bool> {
public:
    ImmediateActivationSurfaceTest()
    {
        if (GetParam()) {
            scoped_feature_list_.InitAndEnableFeature(features::kDrawImmediatelyWhenInteractive);
        } else {
            scoped_feature_list_.InitAndDisableFeature(features::kDrawImmediatelyWhenInteractive);
        }
    }
    ...
    void SetUp() override
    {
        ...
        frame_sink_manager_.surface_manager()->SetTickClockForTesting(now_src_.get());
    }
...
TEST_P(ImmediateActivationSurfaceTest, WithNoInteraction)
{
    ...
    auto child_support = std::make_unique<CompositorFrameSinkSupport>(nullptr, &frame_sink_manager_, FrameSinkId(2, 1), /*is_root=*/false);
    ...
    auto root_render_pass = RenderPassBuilder(CompositorRenderPassId { 1 }, output_rect).AddSurfaceQuad(output_rect, surface_range).Build();

    {
        CompositorFrame frame = MakeCompositorFrame(root_render_pass->DeepCopy());
        frame.metadata.activation_dependencies.push_back(child_surface_id);
        frame.metadata.deadline = FrameDeadline(Now(), 4u, BeginFrameArgs::DefaultInterval(), false);
        EXPECT_THAT(frame.metadata.referenced_surfaces, testing::ElementsAre(surface_range));
        root_support->SubmitCompositorFrame(root_surface_id.local_surface_id(), std::move(frame));
    }
