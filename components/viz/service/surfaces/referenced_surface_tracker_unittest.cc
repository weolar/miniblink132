...
base::flat_set<SurfaceId> MakeReferenceSet(std::initializer_list<SurfaceId> surface_ids)
{
...
{
    return SurfaceId(frame_sink_id, LocalSurfaceId(parent_id, base::UnguessableToken::Create()));
}
...
public:
    ...
    ReferencedSurfaceTrackerTest& operator=(const ReferencedSurfaceTrackerTest&) = delete;
    ...
    void UpdateReferences(
        const SurfaceId& surface_id, const base::flat_set<SurfaceId>& old_referenced_surfaces, const base::flat_set<SurfaceId>& new_referenced_surfaces)
    {
        references_to_add_.clear();
        references_to_remove_.clear();
        GetSurfaceReferenceDifference(surface_id, old_referenced_surfaces, new_referenced_surfaces, &references_to_add_, &references_to_remove_);
    }
...
TEST_F(ReferencedSurfaceTrackerTest, AddSurfaceReference)
{
    ...
    UpdateReferences(parent_id, MakeReferenceSet({}), MakeReferenceSet({ child_id1 }));
...
TEST_F(ReferencedSurfaceTrackerTest, RemoveSurfaceReference)
{
    ...
    UpdateReferences(parent_id, MakeReferenceSet({ child_id1 }), MakeReferenceSet({}));
    EXPECT_THAT(references_to_add(), IsEmpty());
...
TEST_F(ReferencedSurfaceTrackerTest, RemoveOneOfTwoSurfaceReferences)
{
    ...
    UpdateReferences(parent_id, MakeReferenceSet({ child_id1_first }), MakeReferenceSet({ child_id1_second }));
...
TEST_F(ReferencedSurfaceTrackerTest, AddTwoThenRemoveOneSurfaceReferences)
{
    ...
    EXPECT_THAT(references_to_remove(), IsEmpty());
    EXPECT_THAT(references_to_add(), UnorderedElementsAre(reference1, reference2));

    // Check that reference to |child_id1| is removed but not to |child_id2|.
    UpdateReferences(parent_id, initial_referenced, MakeReferenceSet({ child_id2 }));
