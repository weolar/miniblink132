...
public:
    ...
    MOCK_METHOD(void, OnAllocatedSharedMemory, (const gfx::Size& size, base::UnsafeSharedMemoryRegion memory_region), (override));
...
public:
    ...
    void SetUp() override
    {
        ...
        EXPECT_CALL(updater_, OnAllocatedSharedMemory(kDefaultSize, _)).WillOnce(testing::WithArg<1>([](base::UnsafeSharedMemoryRegion region) {
            EXPECT_TRUE(region.IsValid());
            size_t required_bytes = ResourceSizes::CheckedSizeInBytes<size_t>(kDefaultSize, SinglePlaneFormat::kRGBA_8888);
...
TEST_F(SoftwareOutputDeviceWinProxyTest, DrawWithSwap)
{
    ...
    // shouldn't run yet.
    bool called = false;
    device_.OnSwapBuffers(base::BindOnce([](bool* val, const gfx::Size& size) { *val = true; }, &called), gfx::FrameData());
    EXPECT_FALSE(called);
