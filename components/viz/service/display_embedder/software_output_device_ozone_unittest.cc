...
public:
    ...
    {
        surface_ = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(viewport_size.width(), viewport_size.height()));
    }
...
public:
    ...
    SoftwareOutputDeviceOzoneTest& operator=(const SoftwareOutputDeviceOzoneTest&) = delete;
...
{
    std::unique_ptr<TestSurfaceOzoneCanvas> surface_ozone = std::make_unique<TestSurfaceOzoneCanvas>();
    surface_ozone_ = surface_ozone.get();
    output_device_ = std::make_unique<SoftwareOutputDeviceOzone>(nullptr, std::move(surface_ozone));
}
...
TEST_F(SoftwareOutputDeviceOzoneTest, CheckCorrectResizeBehavior)
{
    ...
    ASSERT_TRUE(canvas);
    gfx::Size canvas_size(canvas->getBaseLayerSize().width(), canvas->getBaseLayerSize().height());
    ...
    canvas_size.SetSize(canvas->getBaseLayerSize().width(), canvas->getBaseLayerSize().height());
