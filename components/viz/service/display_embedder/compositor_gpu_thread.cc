...
// static
std::unique_ptr<CompositorGpuThread> CompositorGpuThread::MaybeCreate(const CreateParams& params)
{
    DCHECK(params.gpu_channel_manager);

    if (!features::IsDrDcEnabled() || params.gpu_channel_manager->gpu_driver_bug_workarounds().disable_drdc) {
        return nullptr;
    }
    ...
    // extension.
    if (gl::GetGLImplementation() == gl::kGLImplementationEGLANGLE && gl::GetANGLEImplementation() == gl::ANGLEImplementation::kOpenGLES) {
    ...
    auto compositor_gpu_thread = base::WrapUnique(new CompositorGpuThread(params.gpu_channel_manager, params.display, params.enable_watchdog));
    ...
    if (params.vulkan_implementation && params.device_queue) {
        ...
        auto compositor_thread_device_queue = std::make_unique<gpu::VulkanDeviceQueue>(device_queue->GetVulkanInstance());
        compositor_thread_device_queue->InitializeForCompositorGpuThread(device_queue->GetVulkanPhysicalDevice(), device_queue->GetVulkanDevice(),
            device_queue->GetVulkanQueue(), device_queue->GetVulkanQueueIndex(), device_queue->enabled_extensions(), device_queue->enabled_device_features_2(),
            device_queue->vma_allocator());
        compositor_gpu_thread->vulkan_context_provider_ = VulkanInProcessContextProvider::CreateForCompositorGpuThread(params.vulkan_implementation,
            std::move(compositor_thread_device_queue), params.gpu_channel_manager->gpu_preferences().vulkan_sync_cpu_memory_limit);
    }
#endif

#if BUILDFLAG(SKIA_USE_DAWN)
    if (params.gpu_channel_manager->gpu_preferences().gr_context_type == gpu::GrContextType::kGraphiteDawn) {
        compositor_gpu_thread->dawn_context_provider_ = gpu::DawnContextProvider::CreateWithSharedDevice(params.dawn_context_provider);
    }
#endif
...
CompositorGpuThread::CompositorGpuThread(gpu::GpuChannelManager* gpu_channel_manager, gl::GLDisplay* display, bool enable_watchdog)
...
scoped_refptr<gpu::SharedContextState> CompositorGpuThread::GetSharedContextState()
{
    ...
    const bool use_passthrough_decoder = gpu::gles2::PassthroughCommandDecoderSupported() && gpu_preferences.use_passthrough_cmd_decoder;
    gl::GLContextAttribs attribs = gpu::gles2::GenerateGLContextAttribsForCompositor(use_passthrough_decoder);
    attribs.angle_context_virtualization_group_number = gl::AngleContextVirtualizationGroup::kDrDc;
    ...
    auto context = gl::init::CreateGLContext(share_group.get(), surface.get(), attribs);
    ...
    auto shared_context_state = base::MakeRefCounted<gpu::SharedContextState>(std::move(share_group), std::move(surface), std::move(context),
        /*use_virtualized_gl_contexts=*/false, gpu_channel_manager_->GetContextLostCallback(), gpu_preferences.gr_context_type,
#if BUILDFLAG(ENABLE_VULKAN)
    ...
    auto gles2_feature_info = base::MakeRefCounted<gpu::gles2::FeatureInfo>(workarounds, gpu_feature_info);

    // Initialize GL.
    if (!shared_context_state->InitializeGL(gpu_preferences, std::move(gles2_feature_info))) {
        LOG(ERROR) << "Failed to initialize GL for DrDC SharedContextState";
        return nullptr;
    }

    // Initialize Skia.
    if (!shared_context_state->InitializeSkia(gpu_preferences, workarounds, gpu_channel_manager_->gr_shader_cache(),
...
void CompositorGpuThread::HandleMemoryPressure(base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level)
{
    DCHECK(task_runner()->BelongsToCurrentThread());

    // Context should be current for cache/memory cleanup.
    if (shared_context_state_ && shared_context_state_->MakeCurrent(nullptr, /*needs_gl=*/true)) {
...
void CompositorGpuThread::Init()
{
    ...
    if (enable_watchdog_ && gpu_channel_manager_->watchdog()) {
        watchdog_thread_
            = gpu::GpuWatchdogThread::Create(gpu_preferences.watchdog_starts_backgrounded, gpu_channel_manager_->watchdog(), "GpuWatchdog_Compositor");
    ...
    memory_pressure_listener_
        = std::make_unique<base::MemoryPressureListener>(FROM_HERE, base::BindRepeating(&CompositorGpuThread::HandleMemoryPressure, base::Unretained(this))),
...
void CompositorGpuThread::OnMemoryAllocatedChange(gpu::CommandBufferId id, uint64_t old_size, uint64_t new_size, gpu::GpuPeakMemoryAllocationSource source)
{
    gpu_channel_manager_->GetOnMemoryAllocatedChangeCallback().Run(id, old_size, new_size, source);
}
...
void CompositorGpuThread::OnBackgrounded()
{
    ...
    task_runner()->PostTask(FROM_HERE, base::BindOnce(&CompositorGpuThread::OnBackgroundedOnCompositorGpuThread, weak_ptr_factory_.GetWeakPtr()));
}
...
void CompositorGpuThread::LoseContext()
{
    if (!task_runner()->BelongsToCurrentThread()) {
        task_runner()->PostTask(FROM_HERE, base::BindOnce(&CompositorGpuThread::LoseContext, weak_ptr_factory_.GetWeakPtr()));
        return;
    }
