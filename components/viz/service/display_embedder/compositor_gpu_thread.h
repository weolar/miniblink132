...
class VIZ_SERVICE_EXPORT CompositorGpuThread : public base::Thread, public gpu::MemoryTracker::Observer {
public:
    struct CreateParams {
    ...
    static std::unique_ptr<CompositorGpuThread> MaybeCreate(const CreateParams& params);
    ...
    void OnMemoryAllocatedChange(gpu::CommandBufferId id, uint64_t old_size, uint64_t new_size, gpu::GpuPeakMemoryAllocationSource source) override;
...
private:
    CompositorGpuThread(gpu::GpuChannelManager* gpu_channel_manager, gl::GLDisplay* display, bool enable_watchdog);
