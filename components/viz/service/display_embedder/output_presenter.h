...
public:
    ...
    virtual void InitializeCapabilities(OutputSurface::Capabilities* capabilities) = 0;

    using ReshapeParams = SkiaOutputDevice::ReshapeParams;
    virtual bool Reshape(const ReshapeParams& params) = 0;
    virtual void Present(SwapCompletionCallback completion_callback, BufferPresentedCallback presentation_callback, gfx::FrameData data) = 0;
    ...
    virtual void ScheduleOverlayPlane(
        const OverlayPlaneCandidate& overlay_plane_candidate, ScopedOverlayAccess* access, std::unique_ptr<gfx::GpuFence> acquire_fence)
        = 0;
