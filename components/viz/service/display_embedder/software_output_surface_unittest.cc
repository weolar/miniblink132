...
public:
    ...
    VSyncSoftwareOutputDevice& operator=(const VSyncSoftwareOutputDevice&) = delete;
...
TEST(SoftwareOutputSurfaceTest, NoVSyncProvider)
{
    ...
    auto output_surface = std::make_unique<SoftwareOutputSurface>(std::make_unique<SoftwareOutputDevice>());
    output_surface->BindToClient(&output_surface_client);

    // Verify the callback is never called.
    output_surface->SetUpdateVSyncParametersCallback(base::BindRepeating([](base::TimeTicks timebase, base::TimeDelta interval) { EXPECT_TRUE(false); }));
...
{
    cc::FakeOutputSurfaceClient output_surface_client;
    auto output_surface = std::make_unique<SoftwareOutputSurface>(std::make_unique<VSyncSoftwareOutputDevice>());
    output_surface->BindToClient(&output_surface_client);

    int update_vsync_parameters_call_count = 0;
    output_surface->SetUpdateVSyncParametersCallback(base::BindLambdaForTesting(
        [&update_vsync_parameters_call_count](base::TimeTicks timebase, base::TimeDelta interval) { ++update_vsync_parameters_call_count; }));

    FakeVSyncProvider* vsync_provider = static_cast<FakeVSyncProvider*>(output_surface->software_device()->GetVSyncProvider());
