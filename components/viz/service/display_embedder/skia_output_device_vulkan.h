...
public:
    SkiaOutputDeviceVulkan(base::PassKey<SkiaOutputDeviceVulkan>, VulkanContextProvider* context_provider, gpu::SurfaceHandle surface_handle,
    ...
    static std::unique_ptr<SkiaOutputDeviceVulkan> Create(VulkanContextProvider* context_provider, gpu::SurfaceHandle surface_handle,
        gpu::MemoryTracker* memory_tracker, DidSwapBufferCompleteCallback did_swap_buffer_complete_callback);
...
private:
    ...
    bool Initialize();
    bool RecreateSwapChain(const SkImageInfo& image_info, int sample_count, gfx::OverlayTransform transform);
    void OnPostSubBufferFinished(OutputSurfaceFrame frame, gfx::SwapResult result);
