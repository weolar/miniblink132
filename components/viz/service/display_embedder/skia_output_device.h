...
public:
    ...
    public:
        ScopedPaint(std::vector<GrBackendSemaphore> end_semaphores, SkiaOutputDevice* device, SkSurface* sk_surface);
        ...
        bool Wait(int num_semaphores, const GrBackendSemaphore wait_semaphores[], bool delete_semaphores_after_wait);
        bool Draw(sk_sp<const GrDeferredDisplayList> ddl);

        // Graphite
        bool Draw(std::unique_ptr<skgpu::graphite::Recording> graphite_recording, base::OnceClosure on_finished);
    ...
    using ReleaseOverlaysCallback = base::RepeatingCallback<void(const std::vector<gpu::Mailbox>)>;

    SkiaOutputDevice(GrDirectContext* gr_context, skgpu::graphite::Context* graphite_context, gpu::MemoryTracker* memory_tracker,
        DidSwapBufferCompleteCallback did_swap_buffer_complete_callback, ReleaseOverlaysCallback release_overlays_callback = base::DoNothing());
    ...
    virtual void Present(const std::optional<gfx::Rect>& update_rect, BufferPresentedCallback feedback, OutputSurfaceFrame frame) = 0;
    ...
    // bookkeeping.
    virtual void SwapBuffersSkipped(BufferPresentedCallback feedback, OutputSurfaceFrame frame);

    bool is_emulated_rgbx() const
    {
...
protected:
    ...
    class SwapInfo {
    public:
        SwapInfo(uint64_t swap_id, BufferPresentedCallback feedback, base::TimeTicks viz_scheduled_draw, base::TimeTicks gpu_started_draw,
            base::TimeTicks task_ready);
        SwapInfo(SwapInfo&& other);
        ~SwapInfo();
        uint64_t SwapId();
        const gpu::SwapBuffersCompleteParams& Complete(
            gfx::SwapCompletionResult result, const std::optional<gfx::Rect>& damage_area, std::vector<gpu::Mailbox> released_overlays, int64_t swap_trace_id);
        void CallFeedback();
    ...
    virtual SkSurface* BeginPaint(std::vector<GrBackendSemaphore>* end_semaphores) = 0;
    ...
    virtual bool Wait(SkSurface* sk_surface, int num_semaphores, const GrBackendSemaphore wait_semaphores[], bool delete_semaphores_after_wait);
    virtual bool Draw(SkSurface* sk_surface, sk_sp<const GrDeferredDisplayList> ddl);
    virtual bool Draw(SkSurface* sk_surface, std::unique_ptr<skgpu::graphite::Recording> graphite_recording, base::OnceClosure on_finished);
    ...
    void FinishSwapBuffers(gfx::SwapCompletionResult result, const gfx::Size& size, OutputSurfaceFrame frame,
        const std::optional<gfx::Rect>& damage_area = std::nullopt, std::vector<gpu::Mailbox> released_overlays = {});
