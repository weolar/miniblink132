...
SoftwareOutputDeviceMac::SoftwareOutputDeviceMac(scoped_refptr<base::SequencedTaskRunner> task_runner)
...
void SoftwareOutputDeviceMac::Resize(const gfx::Size& pixel_size, float scale_factor)
{
...
void SoftwareOutputDeviceMac::UpdateAndCopyBufferDamage(Buffer* previous_paint_buffer, const SkRegion& new_damage_region)
{
    ...
    bool copy_region_nonempty = copy_region.op(new_damage_region, SkRegion::kDifference_Op);
...

    {
        ...
        IOReturn io_result = IOSurfaceLock(previous_io_surface, kIOSurfaceLockReadOnly | kIOSurfaceLockAvoidSync, nullptr);
        if (io_result) {
    ...
    uint8_t* pixels = static_cast<uint8_t*>(IOSurfaceGetBaseAddress(previous_io_surface));
    ...
    for (SkRegion::Iterator it(copy_region); !it.done(); it.next()) {
        ...
        current_paint_canvas_->writePixels(
            SkImageInfo::MakeN32Premul(rect.width(), rect.height()), pixels + bytes_per_element * rect.x() + stride * rect.y(), stride, rect.x(), rect.y());
    }

    {
        TRACE_EVENT0("browser", "IOSurfaceUnlock");
        IOReturn io_result = IOSurfaceUnlock(previous_io_surface, kIOSurfaceLockReadOnly | kIOSurfaceLockAvoidSync, nullptr);
        if (io_result)
            DLOG(ERROR) << "Failed to unlock previous IOSurface " << io_result;
    }
}

SkCanvas* SoftwareOutputDeviceMac::BeginPaint(const gfx::Rect& new_damage_rect)
{
    // Record the previous paint buffer.
    Buffer* previous_paint_buffer = buffer_queue_.empty() ? nullptr : buffer_queue_.back().get();
    ...
    if (!current_paint_buffer_) {
        ...
        new_buffer->io_surface = gfx::CreateIOSurface(pixel_size_, gfx::BufferFormat::BGRA_8888);
        if (!new_buffer->io_surface)
            return nullptr;
        // Set the initial damage to be the full buffer.
        new_buffer->accumulated_damage.setRect(gfx::RectToSkIRect(gfx::Rect(pixel_size_)));
    ...
    // |current_paint_canvas_|.
    {
        ...
        IOReturn io_result = IOSurfaceLock(current_paint_buffer_->io_surface.get(), kIOSurfaceLockAvoidSync, nullptr);
        if (io_result) {
    ...
    {
        SkPMColor* pixels = static_cast<SkPMColor*>(IOSurfaceGetBaseAddress(current_paint_buffer_->io_surface.get()));
        size_t stride = IOSurfaceGetBytesPerRow(current_paint_buffer_->io_surface.get());
        current_paint_canvas_ = SkCanvas::MakeRasterDirectN32(pixel_size_.width(), pixel_size_.height(), pixels, stride);
    }

    UpdateAndCopyBufferDamage(previous_paint_buffer, SkRegion(gfx::RectToSkIRect(new_damage_rect)));
...
void SoftwareOutputDeviceMac::EndPaint()
{
    ...
    {
        TRACE_EVENT0("browser", "IOSurfaceUnlock");
        IOReturn io_result = IOSurfaceUnlock(current_paint_buffer_->io_surface.get(), kIOSurfaceLockAvoidSync, nullptr);
        if (io_result)
    ...
    if (client_) {
        ...
        ca_layer_params.io_surface_mach_port.reset(IOSurfaceCreateMachPort(current_paint_buffer_->io_surface.get()));
