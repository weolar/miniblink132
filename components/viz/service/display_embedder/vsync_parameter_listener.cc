...
VSyncParameterListener::VSyncParameterListener(mojo::PendingRemote<mojom::VSyncParameterObserver> observer)
...
void VSyncParameterListener::OnVSyncParametersUpdated(base::TimeTicks timebase, base::TimeDelta interval)
{
    if (ShouldSendUpdate(timebase, interval))
        observer_->OnUpdateVSyncParameters(timebase, interval);
}

bool VSyncParameterListener::ShouldSendUpdate(base::TimeTicks timebase, base::TimeDelta interval)
{
