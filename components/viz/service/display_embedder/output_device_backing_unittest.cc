...
size_t GetViewportSizeInBytes(const gfx::Size& viewport_size)
{
    ...
    CHECK(ResourceSizes::MaybeSizeInBytes(viewport_size, SinglePlaneFormat::kRGBA_8888, &bytes));
    return bytes;
}
...
public:
    TestBackingClient(OutputDeviceBacking* backing, const gfx::Size& viewport_size)
        : backing_(backing)
    ...
    bool release_canvas_called() const
    {
        return release_canvas_called_;
    }
...
TEST(OutputDeviceBackingTest, GetMaxViewportBytes)
{
    ...
    EXPECT_EQ(GetViewportSizeInBytes(client_b.viewport_size()), backing.GetMaxViewportBytes());
}
...
{
    OutputDeviceBacking backing;
    auto client_a = std::make_unique<TestBackingClient>(&backing, gfx::Size(1920, 1080));
    auto client_b = std::make_unique<TestBackingClient>(&backing, gfx::Size(1080, 1920));
    auto client_c = std::make_unique<TestBackingClient>(&backing, gfx::Size(1024, 768));
    ...
    EXPECT_EQ(GetViewportSizeInBytes(client_c->viewport_size()), backing.GetMaxViewportBytes());
}
