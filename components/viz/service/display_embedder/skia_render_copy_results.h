...
class AsyncReadResultLock : public base::RefCountedThreadSafe<AsyncReadResultLock> {
public:
...
public:
    explicit AsyncReadResultHelper(SkiaOutputSurfaceImplOnGpu* impl_on_gpu, std::unique_ptr<const SkSurface::AsyncReadResult> result);
...
struct ReadPixelsContext {
    ReadPixelsContext(std::unique_ptr<CopyOutputRequest> request, const gfx::Rect& result_rect, const gfx::ColorSpace& color_space,
...
public:
    ...
    static void OnReadbackDone(void* c, std::unique_ptr<const SkSurface::AsyncReadResult> async_result);
...
public:
    ...
    ReadbackContextTexture(base::WeakPtr<SkiaOutputSurfaceImplOnGpu> impl_on_gpu, std::unique_ptr<CopyOutputRequest> request, const gfx::Rect& result_rect,
        const gpu::Mailbox& mailbox, const gfx::ColorSpace& color_space);
...
public:
    CopyOutputResultSkiaYUV(SkiaOutputSurfaceImplOnGpu* impl, const gfx::Rect& rect, std::unique_ptr<const SkSurface::AsyncReadResult> result);
    ~CopyOutputResultSkiaYUV() override;

    static void OnReadbackDone(void* c, std::unique_ptr<const SkSurface::AsyncReadResult> async_result);

    // CopyOutputResult implementation:
    bool ReadI420Planes(uint8_t* y_out, int y_out_stride, uint8_t* u_out, int u_out_stride, uint8_t* v_out, int v_out_stride) const override;
...
class NV12PlanesReadbackContext : public base::RefCounted<NV12PlanesReadbackContext> {
public:
    NV12PlanesReadbackContext(base::WeakPtr<SkiaOutputSurfaceImplOnGpu> impl_on_gpu, std::unique_ptr<CopyOutputRequest> request, const gfx::Rect& result_rect);

    void PlaneReadbackDone(int plane_index, std::unique_ptr<const SkSurface::AsyncReadResult> async_result);
...
private:
    ...
    std::array<std::unique_ptr<const SkSurface::AsyncReadResult>, CopyOutputResult::kNV12MaxPlanes> plane_results_;
};

// Represents a readback request for a specific NV12 plane.
struct NV12PlanePixelReadContext {
    NV12PlanePixelReadContext(scoped_refptr<NV12PlanesReadbackContext> nv12_planes_readback, int plane_index);
...
public:
    ...
    // CopyOutputResult implementation:
    bool ReadNV12Planes(uint8_t* y_out, int y_out_stride, uint8_t* uv_out, int uv_out_stride) const override;

    static void OnNV12PlaneReadbackDone(void* c, std::unique_ptr<const SkSurface::AsyncReadResult> async_result);
