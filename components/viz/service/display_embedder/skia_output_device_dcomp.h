...
public:
    SkiaOutputDeviceDComp(gpu::SharedImageRepresentationFactory* shared_image_representation_factory, gpu::SharedContextState* context_state,
...
private:
    ...
    std::optional<gl::DCLayerOverlayImage> BeginOverlayAccess(const gpu::Mailbox& mailbox);
    ...
    const raw_ptr<gpu::SharedImageRepresentationFactory> shared_image_representation_factory_;

    const raw_ptr<gpu::SharedContextState> context_state_;
    gfx::Size size_;

    // Completion callback for |DoPresent|.
    void OnPresentFinished(OutputSurfaceFrame frame, const gfx::Size& swap_size, gfx::SwapCompletionResult result);
