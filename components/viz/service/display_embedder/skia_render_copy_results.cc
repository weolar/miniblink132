...
AsyncReadResultHelper::AsyncReadResultHelper(SkiaOutputSurfaceImplOnGpu* impl_on_gpu, std::unique_ptr<const SkSurface::AsyncReadResult> result)
...
ReadPixelsContext::ReadPixelsContext(std::unique_ptr<CopyOutputRequest> request, const gfx::Rect& result_rect, const gfx::ColorSpace& color_space,
...
CopyOutputResultSkiaRGBA::CopyOutputResultSkiaRGBA(
    SkiaOutputSurfaceImplOnGpu* impl, const gfx::Rect& rect, std::unique_ptr<const SkSurface::AsyncReadResult> result, const gfx::ColorSpace& color_space)
    : CopyOutputResult(Format::RGBA, Destination::kSystemMemory, rect,
...
// static
void CopyOutputResultSkiaRGBA::OnReadbackDone(void* c, std::unique_ptr<const SkSurface::AsyncReadResult> async_result)
{
    ...
    auto result = std::make_unique<CopyOutputResultSkiaRGBA>(impl_on_gpu, context->result_rect, std::move(async_result), context->color_space);
...
const SkBitmap& CopyOutputResultSkiaRGBA::AsSkBitmap() const
{
    if (!result_) {
        ...
        *cached_bitmap() = SkBitmap();
        ...
        auto info =
#if BUILDFLAG(IS_IOS)
            SkImageInfo::Make(size().width(), size().height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType, color_space_);
#else
            SkImageInfo::MakeN32Premul(size().width(), size().height(), color_space_);
...
ReadbackContextTexture::ReadbackContextTexture(base::WeakPtr<SkiaOutputSurfaceImplOnGpu> impl_on_gpu, std::unique_ptr<CopyOutputRequest> request,
    const gfx::Rect& result_rect, const gpu::Mailbox& mailbox, const gfx::ColorSpace& color_space)
    : impl_on_gpu_(impl_on_gpu)
...
void ReadbackContextTexture::OnMailboxReadyInternal()
{
    ...
    request_->SendResult(std::make_unique<CopyOutputTextureResult>(
        request_->result_format(), result_rect_, CopyOutputResult::TextureResult(mailbox_, color_space_), CopyOutputResult::ReleaseCallbacks()));
}

CopyOutputResultSkiaYUV::CopyOutputResultSkiaYUV(
    SkiaOutputSurfaceImplOnGpu* impl, const gfx::Rect& rect, std::unique_ptr<const SkSurface::AsyncReadResult> result)
    : CopyOutputResult(Format::I420_PLANES, Destination::kSystemMemory, rect,
...
// static
void CopyOutputResultSkiaYUV::OnReadbackDone(void* c, std::unique_ptr<const SkSurface::AsyncReadResult> async_result)
{
    auto context = base::WrapUnique(static_cast<ReadPixelsContext*>(c));
    auto* impl_on_gpu = context->impl_on_gpu.get();
    ...
    auto result = std::make_unique<CopyOutputResultSkiaYUV>(impl_on_gpu, context->result_rect, std::move(async_result));
    context->request->SendResult(std::move(result));
}

// CopyOutputResult implementation:
bool CopyOutputResultSkiaYUV::ReadI420Planes(uint8_t* y_out, int y_out_stride, uint8_t* u_out, int u_out_stride, uint8_t* v_out, int v_out_stride) const
{
    ...
    libyuv::CopyPlane(data0, result_->rowBytes(0), y_out, y_out_stride, width(0), height(0));
    libyuv::CopyPlane(data1, result_->rowBytes(1), u_out, u_out_stride, width(1), height(1));
    libyuv::CopyPlane(data2, result_->rowBytes(2), v_out, v_out_stride, width(2), height(2));
    return true;
}

NV12PlanesReadbackContext::NV12PlanesReadbackContext(
    base::WeakPtr<SkiaOutputSurfaceImplOnGpu> impl_on_gpu, std::unique_ptr<CopyOutputRequest> request, const gfx::Rect& result_rect)
    : impl_on_gpu_(impl_on_gpu)
    , request_(std::move(request))
    , result_rect_(result_rect)
{
...
void NV12PlanesReadbackContext::PlaneReadbackDone(int plane_index, std::unique_ptr<const SkSurface::AsyncReadResult> async_result)
{
    ...
    if (outstanding_reads_ == 0) {
        auto result = std::make_unique<CopyOutputResultSkiaNV12>(impl_on_gpu_.get(), result_rect_, std::move(plane_results_));

        request_->SendResult(std::move(result));
    }
}

NV12PlanePixelReadContext::NV12PlanePixelReadContext(scoped_refptr<NV12PlanesReadbackContext> nv12_planes_readback, int plane_index)
...
CopyOutputResultSkiaNV12::CopyOutputResultSkiaNV12(
    SkiaOutputSurfaceImplOnGpu* impl, const gfx::Rect& rect, std::array<std::unique_ptr<const SkImage::AsyncReadResult>, 2> async_read_results)
    : CopyOutputResult(Format::I420_PLANES, Destination::kSystemMemory, rect,
        /*needs_lock_for_bitmap=*/false)
{
    DCHECK_EQ(0, size().width() % 2);
    ...
    for (auto& async_read_result : async_read_results) {
        plane_readback_results_.push_back(std::make_unique<AsyncReadResultHelper>(impl, std::move(async_read_result)));
    }
}

CopyOutputResultSkiaNV12::~CopyOutputResultSkiaNV12() = default;

bool CopyOutputResultSkiaNV12::ReadNV12Planes(uint8_t* y_out, int y_out_stride, uint8_t* uv_out, int uv_out_stride) const
{
    const auto plane_pointer = [y_out, uv_out](int plane_number) { return plane_number == 0 ? y_out : uv_out; };

    const auto plane_stride = [y_out_stride, uv_out_stride](int plane_number) { return plane_number == 0 ? y_out_stride : uv_out_stride; };
    ...
    for (size_t i = 0; i < kNV12MaxPlanes; ++i) {
        ...
        libyuv::CopyPlane(data, result->rowBytes(0), plane_pointer(i), plane_stride(i), width(i), height(i));
    }
...
// static
void CopyOutputResultSkiaNV12::OnNV12PlaneReadbackDone(void* c, std::unique_ptr<const SkSurface::AsyncReadResult> async_result)
{
    auto context = base::WrapUnique(static_cast<NV12PlanePixelReadContext*>(c));

    context->nv12_planes_readback->PlaneReadbackDone(context->plane_index, std::move(async_result));
}
