...
public:
    explicit LocalBitmapData(SkBitmap bitmap)
        ...
        , guid_(base::UnguessableToken::Create())
    {
    }
...
public:
    ...
    explicit ServerSharedBitmap(scoped_refptr<BitmapData> bitmap_data)
        : SharedBitmap(static_cast<uint8_t*>(const_cast<void*>(bitmap_data->GetMemory())))
...
std::unique_ptr<SharedBitmap> ServerSharedBitmapManager::GetSharedBitmapFromId(const gfx::Size& size, SharedImageFormat format, const SharedBitmapId& id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    size_t bitmap_size;
    if (!ResourceSizes::MaybeSizeInBytes(size, format, &bitmap_size) || bitmap_size > data->GetSize()) {
        return nullptr;
    }
...
base::UnguessableToken ServerSharedBitmapManager::GetSharedBitmapTracingGUIDFromId(const SharedBitmapId& id)
{
    auto it = handle_map_.find(id);
    if (it == handle_map_.end())
        return {};
...
bool ServerSharedBitmapManager::LocalAllocatedSharedBitmap(SkBitmap bitmap, const SharedBitmapId& id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
bool ServerSharedBitmapManager::ChildAllocatedSharedBitmap(base::ReadOnlySharedMemoryMapping mapping, const SharedBitmapId& id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    handle_map_[id] = base::MakeRefCounted<SharedMemoryBitmapData>(std::move(mapping));

    // Note: |region| will be destroyed at scope exit, releasing the fd.
    return true;
}

void ServerSharedBitmapManager::ChildDeletedSharedBitmap(const SharedBitmapId& id)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    handle_map_.erase(id);
}

bool ServerSharedBitmapManager::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
    DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
    ...
    for (const auto& pair : handle_map_) {
        ...
        std::string dump_str = base::StringPrintf("sharedbitmap/%s", base::HexEncode(base::as_byte_span(id.name)).c_str());
        base::trace_event::MemoryAllocatorDump* dump = pmd->CreateAllocatorDump(dump_str);
        if (!dump)
            return false;

        dump->AddScalar(base::trace_event::MemoryAllocatorDump::kNameSize, base::trace_event::MemoryAllocatorDump::kUnitsBytes, data->GetSize());
