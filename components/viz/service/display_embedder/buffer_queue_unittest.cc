...
constexpr gfx::ColorSpace kBufferQueueColorSpace = gfx::ColorSpace::CreateSRGB();

} // namespace

#if BUILDFLAG(IS_WIN)
const gpu::SurfaceHandle kFakeSurfaceHandle = reinterpret_cast<gpu::SurfaceHandle>(1);
#else
...
public:
    ...
    void SetUp() override
    {
        ...
        buffer_queue_ = std::make_unique<BufferQueue>(skia_output_surface_.get(), kFakeSurfaceHandle, 3);
    }
    ...
    {
        return buffer_queue_->current_buffer_ ? buffer_queue_->current_buffer_->mailbox : gpu::Mailbox();
    }
    const base::circular_deque<std::unique_ptr<BufferQueue::AllocatedBuffer>>& available_buffers()
    {
        return buffer_queue_->available_buffers_;
    }
    base::circular_deque<std::unique_ptr<BufferQueue::AllocatedBuffer>>& in_flight_buffers()
    {
    ...
    const gfx::Size size()
    {
        return buffer_queue_->size_;
    }

    int CountBuffers()
    {
        int n = available_buffers().size() + in_flight_buffers().size() + (displayed_frame() ? 1 : 0);
    ...
    bool CheckUnique()
    {
        ...
        if (displayed_frame() && !InsertUnique(&buffers, displayed_frame()->mailbox)) {
            return false;
        }
    ...
    void SendFullFrame()
    {
        SendDamagedFrame(gfx::Rect(buffer_queue_->size_));
    }
...
public:
    MockedSkiaOutputSurface()
        : FakeSkiaOutputSurface(nullptr)
    {
    }
    MOCK_METHOD7(CreateSharedImage,
        gpu::Mailbox(SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space, RenderPassAlphaType alpha_type,
            gpu::SharedImageUsageSet usage, std::string_view debug_label, gpu::SurfaceHandle surface_handle));
...
TEST(BufferQueueStandaloneTest, BufferCreationAndDestruction)
{
    ...
    std::unique_ptr<BufferQueue> buffer_queue = std::make_unique<BufferQueue>(mock_skia_output_surface.get(), kFakeSurfaceHandle, 1);
    ...
    const gpu::Mailbox expected_mailbox = gpu::Mailbox::Generate();
    {
        ...
        EXPECT_CALL(*mock_skia_output_surface,
            CreateSharedImage(_, _, _, _,
                ...
                gpu::SHARED_IMAGE_USAGE_SCANOUT | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_DISPLAY_WRITE, _, _))
            .WillOnce(Return(expected_mailbox));
        EXPECT_CALL(*mock_skia_output_surface, DestroySharedImage(expected_mailbox));
    }

    EXPECT_TRUE(buffer_queue->Reshape(screen_size, kBufferQueueColorSpace, RenderPassAlphaType::kPremul, kBufferQueueFormat));
    EXPECT_EQ(expected_mailbox, buffer_queue->GetCurrentBuffer());
}

TEST_F(BufferQueueTest, PartialSwapReuse)
{
    EXPECT_TRUE(buffer_queue_->Reshape(screen_size, kBufferQueueColorSpace, RenderPassAlphaType::kPremul, kBufferQueueFormat));
    SendFullFrame();
...
TEST_F(BufferQueueTest, CheckDoubleBuffering)
{
    ...
    buffer_queue_ = std::make_unique<BufferQueue>(skia_output_surface_.get(), kFakeSurfaceHandle, 2);
...
TEST_F(BufferQueueTest, ReshapeWithInFlightBuffers)
{
    ...
    EXPECT_TRUE(buffer_queue_->Reshape(gfx::Size(10, 20), kBufferQueueColorSpace, RenderPassAlphaType::kPremul, kBufferQueueFormat));
...
TEST_F(BufferQueueTest, DestroyBuffers)
{
    ...
    EXPECT_TRUE(buffer_queue_->Reshape(gfx::Size(20, 20), kBufferQueueColorSpace, RenderPassAlphaType::kPremul, kBufferQueueFormat));
    EXPECT_TRUE(buffer_queue_->GetLastSwappedBuffer().IsZero());
...
TEST_F(BufferQueueTest, SetPurgeable)
{
    ...
    skia_output_surface_->SetSharedImagePurgeableCallback(base::BindLambdaForTesting(mock.AsStdFunction()));
...
TEST_F(BufferQueueTest, SetPurgeableThenReshape)
{
    ...
    buffer_queue_->SetBuffersPurgeable();
    EXPECT_TRUE(buffer_queue_->Reshape(gfx::Size(1, 1), kBufferQueueColorSpace, RenderPassAlphaType::kPremul, kBufferQueueFormat));
