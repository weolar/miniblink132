...
void ReportLatency(const gfx::SwapTimings& timings, std::vector<ui::LatencyInfo> latency_info)
{
    for (auto& latency : latency_info) {
        latency.AddLatencyNumberWithTimestamp(ui::INPUT_EVENT_GPU_SWAP_BUFFER_COMPONENT, timings.swap_start);
        latency.AddLatencyNumberWithTimestamp(ui::INPUT_EVENT_LATENCY_FRAME_SWAP_COMPONENT, timings.swap_end);
    }
}

} // namespace

SkiaOutputDevice::ScopedPaint::ScopedPaint(std::vector<GrBackendSemaphore> end_semaphores, SkiaOutputDevice* device, SkSurface* sk_surface)
...
GrSemaphoresSubmitted SkiaOutputDevice::ScopedPaint::Flush(
    VulkanContextProvider* vulkan_context_provider, std::vector<GrBackendSemaphore> end_semaphores, base::OnceClosure on_finished)
{
    return device_->Flush(sk_surface_, vulkan_context_provider, std::move(end_semaphores), std::move(on_finished));
}

bool SkiaOutputDevice::ScopedPaint::Wait(int num_semaphores, const GrBackendSemaphore wait_semaphores[], bool delete_semaphores_after_wait)
{
    return device_->Wait(sk_surface_, num_semaphores, wait_semaphores, delete_semaphores_after_wait);
}

bool SkiaOutputDevice::ScopedPaint::Draw(sk_sp<const GrDeferredDisplayList> ddl)
{
    return device_->Draw(sk_surface_, std::move(ddl));
}

// bool SkiaOutputDevice::ScopedPaint::Draw(std::unique_ptr<skgpu::graphite::Recording> graphite_recording, base::OnceClosure on_finished)
// {
//     //return device_->Draw(sk_surface_, std::move(graphite_recording), std::move(on_finished));
// }

SkiaOutputDevice::SkiaOutputDevice(GrDirectContext* gr_context, skgpu::graphite::Context* graphite_context, gpu::MemoryTracker* memory_tracker,
    DidSwapBufferCompleteCallback did_swap_buffer_complete_callback, ReleaseOverlaysCallback release_overlays_callback)
    ...
    , memory_type_tracker_(std::make_unique<gpu::MemoryTypeTracker>(memory_tracker))
{
    *(int*)1 = 1;
//     if (gr_context_) {
...
std::unique_ptr<SkiaOutputDevice::ScopedPaint> SkiaOutputDevice::BeginScopedPaint()
{
    ...
    return std::make_unique<SkiaOutputDevice::ScopedPaint>(std::move(end_semaphores), this, sk_surface);
}
...
//     } else {
//         CHECK(graphite_context_);
//         graphite_context_->submit(sync_cpu ? skgpu::graphite::SyncToCpu::kYes : skgpu::graphite::SyncToCpu::kNo);
//     }
//     std::move(callback).Run();
}

void SkiaOutputDevice::ScheduleOverlays(SkiaOutputSurface::OverlayList overlays)
{
    NOTIMPLEMENTED();
}
...
void SkiaOutputDevice::SetDrawTimings(base::TimeTicks submitted, base::TimeTicks started)
{
...
void SkiaOutputDevice::ReadbackForTesting(base::OnceCallback<void(SkBitmap)> callback)
{
    NOTIMPLEMENTED();
}
...
{
    DCHECK_LT(static_cast<int>(pending_swaps_.size()), capabilities_.pending_swap_params.GetMax());

    pending_swaps_.emplace(++swap_id_, std::move(feedback), viz_scheduled_draw_, gpu_started_draw_, gpu_task_ready_);
    viz_scheduled_draw_ = base::TimeTicks();
    gpu_started_draw_ = base::TimeTicks();
    gpu_task_ready_ = base::TimeTicks();
}

void SkiaOutputDevice::FinishSwapBuffers(gfx::SwapCompletionResult result, const gfx::Size& size, OutputSurfaceFrame frame,
    const std::optional<gfx::Rect>& damage_area, std::vector<gpu::Mailbox> released_overlays)
{
    ...
    TRACE_EVENT("viz,benchmark,graphics.pipeline", "Graphics.Pipeline", perfetto::Flow::Global(frame.data.swap_trace_id),
        [swap_trace_id = frame.data.swap_trace_id](perfetto::EventContext ctx) {
            ...
            data->set_step(perfetto::protos::pbzero::ChromeGraphicsPipeline::StepName::STEP_FINISH_BUFFER_SWAP);
            data->set_display_trace_id(swap_trace_id);
        });

    auto release_fence = std::move(result.release_fence);
    const gpu::SwapBuffersCompleteParams& params
        = pending_swaps_.front().Complete(std::move(result), damage_area, std::move(released_overlays), frame.data.swap_trace_id);

    did_swap_buffer_complete_callback_.Run(params, size, std::move(release_fence));
    ...
    if (!pending_swaps_.empty()) {
        ...
        if (iter != skipped_swap_info_.end()) {
            ...
            FinishSwapBuffers(gfx::SwapCompletionResult(gfx::SwapResult::SWAP_SKIPPED), frame_size, std::move(output_frame));
        }
    }
}

void SkiaOutputDevice::SwapBuffersSkipped(BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    ...
    if (pending_swaps_.size() == 1) {
        ...
        FinishSwapBuffers(gfx::SwapCompletionResult(gfx::SwapResult::SWAP_SKIPPED), frame_size, std::move(frame));
    } else {
        skipped_swap_info_[swap_id_] = std::move(frame);
    }
}

SkiaOutputDevice::SwapInfo::SwapInfo(uint64_t swap_id, SkiaOutputDevice::BufferPresentedCallback feedback, base::TimeTicks viz_scheduled_draw,
    base::TimeTicks gpu_started_draw, base::TimeTicks gpu_task_ready)
...
const gpu::SwapBuffersCompleteParams& SkiaOutputDevice::SwapInfo::Complete(
    gfx::SwapCompletionResult result, const std::optional<gfx::Rect>& damage_rect, std::vector<gpu::Mailbox> released_overlays, int64_t swap_trace_id)
{
...
void SkiaOutputDevice::SwapInfo::CallFeedback()
{
    if (feedback_) {
        ...
        std::move(feedback_).Run(gfx::PresentationFeedback(params_.swap_response.timings.swap_start,
...
bool SkiaOutputDevice::Wait(SkSurface* sk_surface, int num_semaphores, const GrBackendSemaphore wait_semaphores[], bool delete_semaphores_after_wait)
{
    return sk_surface->wait(num_semaphores, wait_semaphores, delete_semaphores_after_wait);
}

bool SkiaOutputDevice::Draw(SkSurface* sk_surface, sk_sp<const GrDeferredDisplayList> ddl)
{
#if DCHECK_IS_ON()
...
#define CHECK_PROPERTY(name) DCHECK_EQ(characterization.name(), surface_characterization.name());
...
// bool SkiaOutputDevice::Draw(SkSurface* sk_surface, std::unique_ptr<skgpu::graphite::Recording> graphite_recording, base::OnceClosure on_finished)
// {
//     CHECK(sk_surface);
