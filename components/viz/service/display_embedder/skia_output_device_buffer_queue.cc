...
namespace {
BASE_FEATURE(kRestartReadAccessForConcurrentReadWrite, "RestartReadAccessForConcurrentReadWrite", base::FEATURE_ENABLED_BY_DEFAULT);
...
NOINLINE void CheckForLoopFailuresBufferQueue()
{
    ...
    if (!g_last_reshape_failure.is_null() && now - g_last_reshape_failure < threshold) {
        CHECK(false);
    }
...
public:
    ...
    OverlayData(std::unique_ptr<gpu::OverlayImageRepresentation> representation,
        std::unique_ptr<gpu::OverlayImageRepresentation::ScopedReadAccess> scoped_read_access, bool is_root_render_pass)
    ...
    void OnReuse() const
    {
        ...
        if (representation_->usage().Has(gpu::SHARED_IMAGE_USAGE_CONCURRENT_READ_WRITE)
    ...
    gpu::OverlayImageRepresentation::ScopedReadAccess* scoped_read_access() const
    {
...
private:
    ...
    mutable std::unique_ptr<gpu::OverlayImageRepresentation::ScopedReadAccess> scoped_read_access_;
    mutable int ref_ = 1;
    const bool is_root_render_pass_ = false;
};

SkiaOutputDeviceBufferQueue::SkiaOutputDeviceBufferQueue(std::unique_ptr<OutputPresenter> presenter, SkiaOutputSurfaceDependency* deps,
    gpu::SharedImageRepresentationFactory* representation_factory, gpu::MemoryTracker* memory_tracker,
    const DidSwapBufferCompleteCallback& did_swap_buffer_complete_callback, const ReleaseOverlaysCallback& release_overlays_callback)
    : SkiaOutputDevice(deps->GetSharedContextState()->gr_context(), deps->GetSharedContextState()->graphite_context(), memory_tracker,
        did_swap_buffer_complete_callback, release_overlays_callback)
...
{
#if BUILDFLAG(IS_OZONE)
    capabilities_.needs_background_image = ui::OzonePlatform::GetInstance()->GetPlatformRuntimeProperties().needs_background_image;
    capabilities_.supports_non_backed_solid_color_overlays
        = ui::OzonePlatform::GetInstance()->GetPlatformRuntimeProperties().supports_non_backed_solid_color_buffers;

    capabilities_.supports_single_pixel_buffer = ui::OzonePlatform::GetInstance()->GetPlatformRuntimeProperties().supports_single_pixel_buffer;
    ...
    capabilities_.pending_swap_params.max_pending_swaps = capabilities_.number_of_buffers - 1;
#if BUILDFLAG(IS_ANDROID)
    if (::features::IncreaseBufferCountForHighFrameRate() && capabilities_.number_of_buffers == 5) {
    ...
    DCHECK_LT(capabilities_.pending_swap_params.max_pending_swaps, capabilities_.number_of_buffers);
    DCHECK_LT(capabilities_.pending_swap_params.max_pending_swaps_90hz.value_or(0), capabilities_.number_of_buffers);
    DCHECK_LT(capabilities_.pending_swap_params.max_pending_swaps_120hz.value_or(0), capabilities_.number_of_buffers);
...
#if BUILDFLAG(IS_MAC)
    presenter_->SetMaxPendingSwaps(capabilities_.pending_swap_params.max_pending_swaps);
#endif
}
...
const SkiaOutputDeviceBufferQueue::OverlayData* SkiaOutputDeviceBufferQueue::GetOrCreateOverlayData(
    const gpu::Mailbox& mailbox, bool is_root_render_pass, bool* is_existing)
{
    if (is_existing)
        *is_existing = false;
    ...
    bool result;
    std::tie(it, result) = overlays_.emplace(std::move(shared_image), std::move(shared_image_access), is_root_render_pass);
    DCHECK(result);
    DCHECK(it->unique());
...
void SkiaOutputDeviceBufferQueue::ScheduleOverlays(SkiaOutputSurface::OverlayList overlays)
{
    ...
    for (const auto& overlay : overlays) {
        ...
        if (overlay.is_solid_color) {
            ...
            DCHECK(capabilities_.supports_non_backed_solid_color_overlays || capabilities_.supports_single_pixel_buffer);
        ...
        auto* overlay_data = GetOrCreateOverlayData(mailbox, overlay.is_root_render_pass, &overlay_has_been_submitted);
        if (overlay_data) {
        ...
        if (context_state_->GrContextIsGL() && access && !overlay_has_been_submitted
            && access->representation()->usage().Has(gpu::SHARED_IMAGE_USAGE_RASTER_DELEGATED_COMPOSITING) && gl::GLFence::IsGpuFenceSupported()) {
            ...
            acquire_fence = std::make_unique<gfx::GpuFence>(current_frame_fence->GetGpuFenceHandle().Clone());
        }
...
void SkiaOutputDeviceBufferQueue::Present(const std::optional<gfx::Rect>& update_rect, BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    StartSwapBuffers({});

    presenter_->Present(base::BindOnce(&SkiaOutputDeviceBufferQueue::DoFinishSwapBuffers, weak_ptr_.GetWeakPtr(), GetSwapBuffersSize(), std::move(frame),
...
void SkiaOutputDeviceBufferQueue::DoFinishSwapBuffers(
    const gfx::Size& size, OutputSurfaceFrame frame, std::vector<gpu::Mailbox> overlay_mailboxes, gfx::SwapCompletionResult result)
{
    ...
    // not used.
    std::erase_if(overlays_, [&result, &has_in_use_overlays, &released_overlays](auto& overlay) {
        if (!overlay.unique()) {
            return false;
        }
    ...
    (void)released_overlays;
        ...
        if (!result.release_fence.is_null()) {
            overlay.scoped_read_access()->SetReleaseFence(result.release_fence.Clone());
        }
        return true;
    });
...
{
    if (!base::FeatureList::IsEnabled(::features::kDeferredOverlaysRelease) || reclaim_overlays_timer_.IsRunning()
    ...
    reclaim_overlays_timer_.Start(FROM_HERE, kDelayForOverlaysReclaim, base::BindOnce(&SkiaOutputDeviceBufferQueue::ReleaseOverlays, base::Unretained(this)));
}
...
void SkiaOutputDeviceBufferQueue::ReleaseOverlays()
{
    ...
    if (swap_time_clock_->NowTicks() - last_swap_time_ < kDelayForOverlaysReclaim || has_overlays_scheduled_but_swap_not_finished_) {
        PostReleaseOverlays();
        return;
    }
    ...
    std::erase_if(overlays_, [&released_overlays](auto& overlay) {
        if (!overlay.unique() || overlay.IsInUseByWindowServer() || overlay.IsRootRenderPass()) {
            return false;
        }
...
void SkiaOutputDeviceBufferQueue::SetViewportSize(const gfx::Size& viewport_size)
{
    viewport_size_ = viewport_size;
}

SkSurface* SkiaOutputDeviceBufferQueue::BeginPaint(std::vector<GrBackendSemaphore>* end_semaphores)
{
    NOTREACHED();
}
...
size_t SkiaOutputDeviceBufferQueue::OverlayDataHash::operator()(const OverlayData& o) const
{
    return std::hash<gpu::Mailbox> {}(o.mailbox());
}

size_t SkiaOutputDeviceBufferQueue::OverlayDataHash::operator()(const gpu::Mailbox& m) const
{
    return std::hash<gpu::Mailbox> {}(m);
}

bool SkiaOutputDeviceBufferQueue::OverlayDataKeyEqual::operator()(const OverlayData& lhs, const OverlayData& rhs) const
{
    return lhs.mailbox() == rhs.mailbox();
}

bool SkiaOutputDeviceBufferQueue::OverlayDataKeyEqual::operator()(const OverlayData& lhs, const gpu::Mailbox& rhs) const
{
    return lhs.mailbox() == rhs;
}

bool SkiaOutputDeviceBufferQueue::OverlayDataKeyEqual::operator()(const gpu::Mailbox& lhs, const OverlayData& rhs) const
{
