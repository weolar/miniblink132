...
// static
std::unique_ptr<SkiaOutputDeviceVulkan> SkiaOutputDeviceVulkan::Create(VulkanContextProvider* context_provider, gpu::SurfaceHandle surface_handle,
    gpu::MemoryTracker* memory_tracker, DidSwapBufferCompleteCallback did_swap_buffer_complete_callback)
{
    auto output_device = std::make_unique<SkiaOutputDeviceVulkan>(
        base::PassKey<SkiaOutputDeviceVulkan>(), context_provider, surface_handle, memory_tracker, did_swap_buffer_complete_callback);
...
SkiaOutputDeviceVulkan::SkiaOutputDeviceVulkan(base::PassKey<SkiaOutputDeviceVulkan>, VulkanContextProvider* context_provider,
    gpu::SurfaceHandle surface_handle, gpu::MemoryTracker* memory_tracker, DidSwapBufferCompleteCallback did_swap_buffer_complete_callback)
    : SkiaOutputDevice(context_provider->GetGrContext(),
        /*graphite_context=*/nullptr, memory_tracker, did_swap_buffer_complete_callback)
...
SkiaOutputDeviceVulkan::~SkiaOutputDeviceVulkan()
{
    ...
    fence_helper->EnqueueVulkanObjectCleanupForSubmittedWork(std::move(vulkan_surface_));
}
...
bool SkiaOutputDeviceVulkan::Reshape(const ReshapeParams& params)
{
    ...
    return RecreateSwapChain(params.image_info, params.sample_count, params.transform);
}

void SkiaOutputDeviceVulkan::Submit(bool sync_cpu, base::OnceClosure callback)
{
    if (scoped_write_) [[likely]] {
        auto& sk_surface = sk_surface_size_pairs_[scoped_write_->image_index()].sk_surface;
        DCHECK(sk_surface);
        auto queue_index = context_provider_->GetDeviceQueue()->GetVulkanQueueIndex();
        skgpu::MutableTextureState state = skgpu::MutableTextureStates::MakeVulkan(VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, queue_index);
        if (GrDirectContext* direct_context = GrAsDirectContext(sk_surface->recordingContext())) {
...
void SkiaOutputDeviceVulkan::Present(const std::optional<gfx::Rect>& update_rect, BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    gfx::Rect rect = update_rect.value_or(gfx::Rect(vulkan_surface_->image_size()));
    ...
    if (is_new_swap_chain_ && rect == gfx::Rect(vulkan_surface_->image_size())) [[unlikely]] {
    ...
    if (!rect.IsEmpty()) [[likely]] {
        ...
        vulkan_surface_->PostSubBufferAsync(
            rect, base::BindOnce(&SkiaOutputDeviceVulkan::OnPostSubBufferFinished, weak_ptr_factory_.GetWeakPtr(), std::move(frame)), std::move(feedback));
    } else {
        OnPostSubBufferFinished(std::move(frame), gfx::SwapResult::SWAP_ACK);
        std::move(feedback).Run(gfx::PresentationFeedback(base::TimeTicks::Now(), vulkan_surface_->GetDisplayRefreshInterval(), 0));
    }
}

SkSurface* SkiaOutputDeviceVulkan::BeginPaint(std::vector<GrBackendSemaphore>* end_semaphores)
{
    DCHECK(vulkan_surface_);
    DCHECK(!scoped_write_);
    ...
    auto& sk_surface = sk_surface_size_pairs_[scoped_write.image_index()].sk_surface;

    if (!sk_surface) [[unlikely]] {
        SkSurfaceProps surface_props;
        const auto surface_format = vulkan_surface_->surface_format().format;
        DCHECK(surface_format == VK_FORMAT_B8G8R8A8_UNORM || surface_format == VK_FORMAT_R8G8B8A8_UNORM);
        ...
        GrBackendTexture backend_texture = GrBackendTextures::MakeVk(vk_image_size.width(), vk_image_size.height(), vk_image_info);

        // Estimate size of GPU memory needed for the GrBackendRenderTarget.
        VkMemoryRequirements requirements;
        vkGetImageMemoryRequirements(context_provider_->GetDeviceQueue()->GetVulkanDevice(), vk_image_info.fImage, &requirements);
        sk_surface_size_pairs_[scoped_write.image_index()].bytes_allocated = requirements.size;
        memory_type_tracker_->TrackMemAlloc(requirements.size);
        sk_surface = SkSurfaces::WrapBackendTexture(
            context_provider_->GetGrContext(), backend_texture, kTopLeft_GrSurfaceOrigin, sample_count_, color_type_, color_space_, &surface_props);
        if (!sk_surface) [[unlikely]] {
            return nullptr;
        }
    } else {
        auto backend = SkSurfaces::GetBackendRenderTarget(sk_surface.get(), SkSurfaces::BackendHandleAccess::kFlushRead);
        GrBackendRenderTargets::SetVkImageLayout(&backend, scoped_write.image_layout());
    }
    ...
    auto result = sk_surface->wait(1, &semaphore, /*deleteSemaphoresAfterWait=*/false);
    if (!result) [[unlikely]] {
        return nullptr;
    }

    DCHECK(scoped_write.end_semaphore() != VK_NULL_HANDLE);
    GrBackendSemaphore end_semaphore = GrBackendSemaphores::MakeVk(scoped_write.end_semaphore());
...
#if DCHECK_IS_ON()
    GrVkImageInfo vk_image_info;
    if (!context_provider_->GetGrContext()->abandoned() && !GrBackendRenderTargets::GetVkImageInfo(backend, &vk_image_info)) [[unlikely]] {
...
bool SkiaOutputDeviceVulkan::Initialize()
{
...
#if BUILDFLAG(IS_ANDROID)
    auto surface_record = gpu::GpuSurfaceLookup::GetInstance()->AcquireJavaSurface(surface_handle_);
...
#endif
    auto vulkan_surface = context_provider_->GetVulkanImplementation()->CreateViewSurface(accelerated_widget);
    if (!vulkan_surface) [[unlikely]] {
        LOG(ERROR) << "Failed to create vulkan surface.";
        return false;
    }
    auto result = vulkan_surface->Initialize(context_provider_->GetDeviceQueue(), gpu::VulkanSurface::FORMAT_RGBA_32);
    if (!result) [[unlikely]] {
    ...
    auto sk_color_type = surface_format == VK_FORMAT_R8G8B8A8_UNORM ? kRGBA_8888_SkColorType : kBGRA_8888_SkColorType;
    capabilities_.sk_color_type_map[SinglePlaneFormat::kRGBA_8888] = sk_color_type;
    capabilities_.sk_color_type_map[SinglePlaneFormat::kBGRA_8888] = sk_color_type;
    // BGRX_8888 is used on Windows.
    capabilities_.sk_color_type_map[SinglePlaneFormat::kBGRX_8888] = sk_color_type;
    return true;
}

bool SkiaOutputDeviceVulkan::RecreateSwapChain(const SkImageInfo& image_info, int sample_count, gfx::OverlayTransform transform)
{
    ...
    // necessary.
    if (!vulkan_surface_->Reshape(gfx::SkISizeToSize(image_info.dimensions()), transform)) [[unlikely]] {
        return false;
    }

    bool recreate = vulkan_surface_->swap_chain_generation() != generation || !SkColorSpace::Equals(image_info.colorSpace(), color_space_.get())
...
void SkiaOutputDeviceVulkan::OnPostSubBufferFinished(OutputSurfaceFrame frame, gfx::SwapResult result)
{
    if (result == gfx::SwapResult::SWAP_ACK) [[likely]] {
        auto image_index = vulkan_surface_->swap_chain()->current_image_index();
        FinishSwapBuffers(gfx::SwapCompletionResult(result), vulkan_surface_->image_size(), std::move(frame), damage_of_images_[image_index]);
    } else {
        FinishSwapBuffers(gfx::SwapCompletionResult(result), vulkan_surface_->image_size(), std::move(frame), gfx::Rect(vulkan_surface_->image_size()));
    }
}

SkiaOutputDeviceVulkan::SkSurfaceSizePair::SkSurfaceSizePair() = default;
SkiaOutputDeviceVulkan::SkSurfaceSizePair::SkSurfaceSizePair(const SkSurfaceSizePair& other) = default;
SkiaOutputDeviceVulkan::SkSurfaceSizePair::~SkSurfaceSizePair() = default;
