...
protected:
    ...
    ServerSharedBitmapManager* manager() const
    {
        return manager_.get();
    }
...
TEST_F(ServerSharedBitmapManagerTest, TestCreate)
{
    ...
    base::MappedReadOnlyRegion shm = bitmap_allocation::AllocateSharedBitmap(bitmap_size, SinglePlaneFormat::kRGBA_8888);
    ...
    large_bitmap = manager()->GetSharedBitmapFromId(gfx::Size(1024, 1024), SinglePlaneFormat::kRGBA_8888, id);
    EXPECT_FALSE(large_bitmap);

    std::unique_ptr<SharedBitmap> very_large_bitmap;
    very_large_bitmap = manager()->GetSharedBitmapFromId(gfx::Size(1, (1 << 30) | 1), SinglePlaneFormat::kRGBA_8888, id);
    EXPECT_FALSE(very_large_bitmap);

    std::unique_ptr<SharedBitmap> negative_size_bitmap;
    negative_size_bitmap = manager()->GetSharedBitmapFromId(gfx::Size(-1, 1024), SinglePlaneFormat::kRGBA_8888, id);
    EXPECT_FALSE(negative_size_bitmap);

    SharedBitmapId id2 = SharedBitmap::GenerateId();
    std::unique_ptr<SharedBitmap> invalid_bitmap;
    invalid_bitmap = manager()->GetSharedBitmapFromId(bitmap_size, SinglePlaneFormat::kRGBA_8888, id2);
    EXPECT_FALSE(invalid_bitmap);

    std::unique_ptr<SharedBitmap> shared_bitmap;
    shared_bitmap = manager()->GetSharedBitmapFromId(bitmap_size, SinglePlaneFormat::kRGBA_8888, id);
    ASSERT_TRUE(shared_bitmap);
    EXPECT_TRUE(std::equal(span.begin(), span.begin() + 4, shared_bitmap->pixels()));

    std::unique_ptr<SharedBitmap> large_bitmap2;
    large_bitmap2 = manager()->GetSharedBitmapFromId(gfx::Size(1024, 1024), SinglePlaneFormat::kRGBA_8888, id);
    EXPECT_FALSE(large_bitmap2);

    std::unique_ptr<SharedBitmap> shared_bitmap2;
    shared_bitmap2 = manager()->GetSharedBitmapFromId(bitmap_size, SinglePlaneFormat::kRGBA_8888, id);
...
TEST_F(ServerSharedBitmapManagerTest, TestLocalCreate)
{
...

    {
        ...
        SkImageInfo info = SkImageInfo::MakeN32Premul(bitmap_size.width(), bitmap_size.height());
        SkBitmap bitmap;
    ...
    std::unique_ptr<SharedBitmap> returned_bitmap = manager()->GetSharedBitmapFromId(bitmap_size, SinglePlaneFormat::kRGBA_8888, id);
...
TEST_F(ServerSharedBitmapManagerTest, AddDuplicate)
{
    ...
    base::MappedReadOnlyRegion shm2 = bitmap_allocation::AllocateSharedBitmap(bitmap_size, SinglePlaneFormat::kRGBA_8888);
...
TEST_F(ServerSharedBitmapManagerTest, SharedMemoryHandle)
{
    ...
    base::UnguessableToken tracing_guid = manager()->GetSharedBitmapTracingGUIDFromId(id);
...
TEST_F(ServerSharedBitmapManagerTest, InvalidScopedSharedBufferHandle)
{
    ...
    EXPECT_FALSE(manager()->ChildAllocatedSharedBitmap(std::move(invalid_mapping), id));
