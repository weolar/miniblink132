...
BufferQueue::BufferQueue(SkiaOutputSurface* skia_output_surface, gpu::SurfaceHandle surface_handle, size_t number_of_buffers, bool is_protected)
...
bool BufferQueue::Reshape(const gfx::Size& size, const gfx::ColorSpace& color_space, RenderPassAlphaType alpha_type, SharedImageFormat format)
{
    if (size == size_ && color_space == color_space_ && alpha_type == alpha_type_ && format == format_) {
        return false;
    }
...
void BufferQueue::AllocateBuffers(size_t n)
{
    ...
    const gpu::SharedImageUsageSet usage = gpu::SHARED_IMAGE_USAGE_DISPLAY_READ | gpu::SHARED_IMAGE_USAGE_DISPLAY_WRITE | gpu::SHARED_IMAGE_USAGE_SCANOUT
        | (is_protected_ ? gpu::SHARED_IMAGE_USAGE_PROTECTED_VIDEO : gpu::SharedImageUsageSet());

    available_buffers_.reserve(available_buffers_.size() + n);
    for (size_t i = 0; i < n; ++i) {
        const gpu::Mailbox mailbox
            = skia_output_surface_->CreateSharedImage(format_.value(), size_, color_space_, alpha_type_, usage, "VizBufferQueue", surface_handle_);
        DCHECK(!mailbox.IsZero());

        available_buffers_.push_back(std::make_unique<AllocatedBuffer>(mailbox, gfx::Rect(size_)));
    }
}
...
std::unique_ptr<BufferQueue::AllocatedBuffer> BufferQueue::GetNextBuffer()
{
    ...
    std::unique_ptr<AllocatedBuffer> buffer = std::move(available_buffers_.front());
...
void BufferQueue::RecreateBuffersIfDestroyed()
{
    ...
    if (buffers_destroyed_) {
        ...
        UMA_HISTOGRAM_TIMES("Compositing.BufferQueue.TimeUntilBuffersRecreatedMs", elapsed);
        destroyed_timer_.reset();
    }
}

BufferQueue::AllocatedBuffer::AllocatedBuffer(const gpu::Mailbox& mailbox, const gfx::Rect& rect)
    : mailbox(mailbox)
    , damage(rect)
{
}
