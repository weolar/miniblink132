...
public:
    ...
    bool TrackSwapBuffers()
    {
        ...
        return (num_swaps_in_current_swap_generation > 1)
            || (next_global_swap_generation - last_multi_window_swap_generation < kMultiWindowSwapEnableVSyncDelay);
    }
...
SkiaOutputDeviceGL::SkiaOutputDeviceGL(gpu::SharedContextState* context_state, scoped_refptr<gl::GLSurface> gl_surface,
    scoped_refptr<gpu::gles2::FeatureInfo> feature_info, gpu::MemoryTracker* memory_tracker, DidSwapBufferCompleteCallback did_swap_buffer_complete_callback)
    : SkiaOutputDevice(context_state->gr_context(), context_state->graphite_context(), memory_tracker, std::move(did_swap_buffer_complete_callback))
    ...
    , supports_async_swap_(gl_surface_->SupportsAsyncSwap())
{
    ...
    if (feature_info->workarounds().disable_post_sub_buffers_for_onscreen_surfaces) {
        capabilities_.supports_post_sub_buffer = false;
    }
    capabilities_.pending_swap_params.max_pending_swaps = gl_surface_->GetBufferCount() - 1;
    ...
    if (alpha_bits == 0) {
        color_type = gl_surface_->GetFormat().IsRGB565() ? kRGB_565_SkColorType : kRGB_888x_SkColorType;
        ...
        if (!context_state_->gr_context()->defaultBackendFormat(color_type, GrRenderable::kYes).isValid()) {
    ...
    // HDR10
    capabilities_.sk_color_type_map[SinglePlaneFormat::kRGBA_1010102] = kRGBA_1010102_SkColorType;
    // scRGB linear
    capabilities_.sk_color_type_map[SinglePlaneFormat::kRGBA_F16] = kRGBA_F16_SkColorType;
...
#if !BUILDFLAG(IS_ANDROID)
        multisurface_swapbuffers_tracker_ = std::make_unique<MultiSurfaceSwapBuffersTracker>();
#endif
    } else {
...
bool SkiaOutputDeviceGL::Reshape(const ReshapeParams& params)
{
    ...
    if (!gl_surface_->Resize(size, params.device_scale_factor, params.color_space, has_alpha)) {
    ...
    GrBackendFormat backend_format = gr_context->defaultBackendFormat(color_type, GrRenderable::kYes);
    ...
    auto render_target = GrBackendRenderTargets::MakeGL(size.width(), size.height(), params.sample_count,
        /*stencilBits=*/0, framebuffer_info);
    auto origin = (gl_surface_->GetOrigin() == gfx::SurfaceOrigin::kTopLeft) ? kTopLeft_GrSurfaceOrigin : kBottomLeft_GrSurfaceOrigin;
    sk_surface_ = SkSurfaces::WrapBackendRenderTarget(gr_context, render_target, origin, color_type, params.image_info.refColorSpace(), &surface_props);
    if (!sk_surface_) {
    ...
    memory_type_tracker_->TrackMemFree(backbuffer_estimated_size_);
    GLenum format = gpu::gles2::TextureManager::ExtractFormatFromStorageFormat(framebuffer_info.fFormat);
    GLenum type = gpu::gles2::TextureManager::ExtractTypeFromStorageFormat(framebuffer_info.fFormat);
    uint32_t estimated_size;
    gpu::gles2::GLES2Util::ComputeImageDataSizes(
        size.width(), size.height(), 1 /* depth */, format, type, 4 /* alignment */, &estimated_size, nullptr, nullptr);
...
void SkiaOutputDeviceGL::Present(const std::optional<gfx::Rect>& update_rect, BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    if (multisurface_swapbuffers_tracker_) {
        const bool multiple_surface_swaps = multisurface_swapbuffers_tracker_->TrackSwapBuffers();
        gl_surface_->SetVSyncEnabled(!multiple_surface_swaps);
    }

    StartSwapBuffers({});

    gfx::Size surface_size = gfx::Size(sk_surface_->width(), sk_surface_->height());

    auto data = frame.data;
    if (supports_async_swap_) {
        auto callback = base::BindOnce(&SkiaOutputDeviceGL::DoFinishSwapBuffersAsync, weak_ptr_factory_.GetWeakPtr(), surface_size, std::move(frame));

        if (update_rect) {
            gl_surface_->PostSubBufferAsync(
                update_rect->x(), update_rect->y(), update_rect->width(), update_rect->height(), std::move(callback), std::move(feedback), std::move(data));
        } else {
            gl_surface_->SwapBuffersAsync(std::move(callback), std::move(feedback), std::move(data));
        }
    } else {
        gfx::SwapResult result;
        if (update_rect) {
            result = gl_surface_->PostSubBuffer(
                update_rect->x(), update_rect->y(), update_rect->width(), update_rect->height(), std::move(feedback), std::move(data));
        } else {
            result = gl_surface_->SwapBuffers(std::move(feedback), std::move(data));
        }
        DoFinishSwapBuffers(surface_size, std::move(frame), gfx::SwapCompletionResult(result));
    }
}

void SkiaOutputDeviceGL::DoFinishSwapBuffersAsync(const gfx::Size& size, OutputSurfaceFrame frame, gfx::SwapCompletionResult result)
{
    DCHECK(result.release_fence.is_null());
    FinishSwapBuffers(std::move(result), size, std::move(frame));
}

void SkiaOutputDeviceGL::DoFinishSwapBuffers(const gfx::Size& size, OutputSurfaceFrame frame, gfx::SwapCompletionResult result)
{
    DCHECK(result.release_fence.is_null());
    FinishSwapBuffers(std::move(result), size, std::move(frame));
}

SkSurface* SkiaOutputDeviceGL::BeginPaint(std::vector<GrBackendSemaphore>* end_semaphores)
{
    DCHECK(sk_surface_);
