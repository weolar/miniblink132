...
class VIZ_SERVICE_EXPORT InProcessGpuMemoryBufferManager : public gpu::GpuMemoryBufferManager, public base::trace_event::MemoryDumpProvider {
public:
    // |gpu_memory_buffer_factory| and |sync_point_manager| must outlive |this|.
    InProcessGpuMemoryBufferManager(gpu::GpuMemoryBufferFactory* gpu_memory_buffer_factory, gpu::SyncPointManager* sync_point_manager);

    InProcessGpuMemoryBufferManager(const InProcessGpuMemoryBufferManager&) = delete;
    InProcessGpuMemoryBufferManager& operator=(const InProcessGpuMemoryBufferManager&) = delete;
...
private:
    ...
    base::flat_map<gfx::GpuMemoryBufferId, gpu::AllocatedBufferInfo> allocated_buffers_;
