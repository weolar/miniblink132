...
SoftwareOutputSurface::SoftwareOutputSurface(std::unique_ptr<SoftwareOutputDevice> device)
    : OutputSurface(std::move(device))
{
    capabilities_.pending_swap_params.max_pending_swaps = software_device()->MaxFramesPending();
    capabilities_.resize_based_on_root_surface = software_device()->SupportsOverridePlatformSize();
}
...
void SoftwareOutputSurface::SwapBuffers(OutputSurfaceFrame frame)
{
    ...
    for (auto& latency : frame.latency_info) {
        latency.AddLatencyNumberWithTimestamp(ui::INPUT_EVENT_GPU_SWAP_BUFFER_COMPONENT, swap_time);
        latency.AddLatencyNumberWithTimestamp(ui::INPUT_EVENT_LATENCY_FRAME_SWAP_COMPONENT, swap_time);
    }
    ...
    software_device()->OnSwapBuffers(
        base::BindOnce(&SoftwareOutputSurface::SwapBuffersCallback, weak_factory_.GetWeakPtr(), swap_time, frame.data.swap_trace_id), frame.data);

    gfx::VSyncProvider* vsync_provider = software_device()->GetVSyncProvider();
    if (vsync_provider && update_vsync_parameters_callback_) {
        vsync_provider->GetVSyncParameters(base::BindOnce(&SoftwareOutputSurface::UpdateVSyncParameters, weak_factory_.GetWeakPtr()));
    }
}
...
void SoftwareOutputSurface::UpdateVSyncParameters(base::TimeTicks timebase, base::TimeDelta interval)
{
    ...
    refresh_interval_ = interval.is_zero() ? BeginFrameArgs::DefaultInterval() : interval;
    update_vsync_parameters_callback_.Run(timebase, refresh_interval_);
}

void SoftwareOutputSurface::SetUpdateVSyncParametersCallback(UpdateVSyncParametersCallback callback)
{
...
#if BUILDFLAG(IS_LINUX)
void SoftwareOutputSurface::SetNeedsSwapSizeNotifications(bool needs_swap_size_notifications)
{
