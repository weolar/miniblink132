...
OutputSurfaceProviderImpl::OutputSurfaceProviderImpl(GpuServiceImpl* gpu_service_impl, bool headless)
...
OutputSurfaceProviderImpl::OutputSurfaceProviderImpl(bool headless)
    : OutputSurfaceProviderImpl(
        /*gpu_service_impl=*/nullptr, headless)
{
}

OutputSurfaceProviderImpl::~OutputSurfaceProviderImpl() = default;

std::unique_ptr<DisplayCompositorMemoryAndTaskController> OutputSurfaceProviderImpl::CreateGpuDependency(
...
//     auto skia_deps = std::make_unique<SkiaOutputSurfaceDependencyImpl>(gpu_service_impl_, surface_handle);
//     return std::make_unique<DisplayCompositorMemoryAndTaskController>(std::move(skia_deps));
}

std::unique_ptr<OutputSurface> OutputSurfaceProviderImpl::CreateOutputSurface(gpu::SurfaceHandle surface_handle, bool gpu_compositing,
    ...
    const DebugRendererSettings* debug_settings)
{
    ...
    if (!gpu_compositing) {
        return std::make_unique<SoftwareOutputSurface>(CreateSoftwareOutputDeviceForPlatform(surface_handle, display_client));
    } else {
        *(int*)1 = 1;
        return nullptr;
...
//         {
//             gpu::ScopedAllowScheduleGpuTask allow_schedule_gpu_task;
//             output_surface = SkiaOutputSurfaceImpl::Create(gpu_dependency, renderer_settings, debug_settings);
//         }
// 
...
//         // failure.
//         display_client->OnContextCreationResult(output_surface ? gpu::ContextResult::kSuccess : gpu::ContextResult::kSurfaceFailure);
...
std::unique_ptr<SoftwareOutputDevice> OutputSurfaceProviderImpl::CreateSoftwareOutputDeviceForPlatform(
    gpu::SurfaceHandle surface_handle, mojom::DisplayClient* display_client)
{
    if (headless_)
