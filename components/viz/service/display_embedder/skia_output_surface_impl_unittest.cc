...
public:
    ...
    gpu::SyncToken PaintRootRenderPass(
        const gfx::Rect& output_rect, base::OnceClosure closure, base::OnceCallback<void(gfx::GpuFenceHandle)> return_release_fence);

    void CheckSyncTokenOnGpuThread(const gpu::SyncToken& sync_token);
    void CopyRequestCallbackOnGpuThread(const gfx::Rect& output_rect, const gfx::ColorSpace& color_space, std::unique_ptr<CopyOutputResult> result);
...
{
    auto skia_deps = std::make_unique<SkiaOutputSurfaceDependencyImpl>(GetGpuService(), gpu::kNullSurfaceHandle);
    display_controller_ = std::make_unique<DisplayCompositorMemoryAndTaskController>(std::move(skia_deps));
    output_surface_ = SkiaOutputSurfaceImpl::Create(display_controller_.get(), RendererSettings(), &debug_settings_);
    ...
    const gfx::Rect& output_rect, base::OnceClosure closure, base::OnceCallback<void(gfx::GpuFenceHandle)> return_release_fence)
{
    ...
    output_surface_->EndPaint(std::move(closure), std::move(return_release_fence), output_rect,
        /*is_overlay=*/false);
...
void SkiaOutputSurfaceImplTest::CheckSyncTokenOnGpuThread(const gpu::SyncToken& sync_token)
{
    EXPECT_TRUE(GetGpuService()->sync_point_manager()->IsSyncTokenReleased(sync_token));
    UnblockMainThread();
}

void SkiaOutputSurfaceImplTest::CopyRequestCallbackOnGpuThread(
    const gfx::Rect& output_rect, const gfx::ColorSpace& color_space, std::unique_ptr<CopyOutputResult> result)
{
    ...
    SkBitmap expected;
    expected.allocPixels(SkImageInfo::MakeN32Premul(output_rect.width(), output_rect.height(), color_space.ToSkColorSpace()));
    expected.eraseColor(kOutputColor);

    EXPECT_TRUE(cc::MatchesBitmap(result_bitmap, expected, cc::ExactPixelComparator()));
...
TEST_F(SkiaOutputSurfaceImplTest, MAYBE_EndPaint)
{
    ...
    base::OnceClosure on_finished = base::BindOnce([](bool* result) { *result = true; }, &on_finished_called);
    base::OnceCallback<void(gfx::GpuFenceHandle)> return_release_fence_cb;

#if !BUILDFLAG(SKIA_USE_METAL)
    bool on_return_release_fence_called = false;
    // This callback is unsupported when using Metal.
    return_release_fence_cb = base::BindOnce([](bool* result, gfx::GpuFenceHandle handle) { *result = true; }, &on_return_release_fence_called);
#endif // !BUILDFLAG(SKIA_USE_METAL)

    gpu::SyncToken sync_token = PaintRootRenderPass(output_rect, std::move(on_finished), std::move(return_release_fence_cb));
    ...
    auto request = std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(&SkiaOutputSurfaceImplTest::CopyRequestCallbackOnGpuThread, base::Unretained(this), output_rect, color_space));
    request->set_result_task_runner(TestGpuServiceHolder::GetInstance()->gpu_main_thread_task_runner());
    ...
    output_surface_->CopyOutput(geometry, color_space, std::move(request), gpu::Mailbox());
    output_surface_->SwapBuffersSkipped(kSurfaceRect);
    output_surface_->Flush();
    BlockMainThread();

    // EndPaint draw is deferred until CopyOutput.
    base::OnceClosure closure = base::BindOnce(&SkiaOutputSurfaceImplTest::CheckSyncTokenOnGpuThread, base::Unretained(this), sync_token);
...
TEST_F(SkiaOutputSurfaceImplTest, SupportsColorSpaceChange)
{
    for (auto& color_space : { gfx::ColorSpace(), gfx::ColorSpace::CreateSRGB() }) {
        ...
        base::OnceCallback<void(gfx::GpuFenceHandle)> return_release_fence_cb;
        PaintRootRenderPass(kSurfaceRect, run_loop.QuitClosure(), std::move(return_release_fence_cb));
...
// TODO(crbug.com/40922049): Re-enable this test
#if defined(MEMORY_SANITIZER)
#define MAYBE_CopyOutputBitmapSupportedColorSpace DISABLED_CopyOutputBitmapSupportedColorSpace
#else
#define MAYBE_CopyOutputBitmapSupportedColorSpace CopyOutputBitmapSupportedColorSpace
#endif
...
TEST_F(SkiaOutputSurfaceImplTest, MAYBE_CopyOutputBitmapSupportedColorSpace)
{
    ...
    constexpr gfx::Rect output_rect(0, 0, 10, 10);
    const gfx::ColorSpace color_space = gfx::ColorSpace(gfx::ColorSpace::PrimaryID::BT709, gfx::ColorSpace::TransferID::LINEAR);
    base::RunLoop run_loop;
    std::unique_ptr<CopyOutputResult> result;
    auto request = std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(
            [](std::unique_ptr<CopyOutputResult>* result_out, base::OnceClosure quit_closure, std::unique_ptr<CopyOutputResult> tmp_result) {
    ...
    base::OnceCallback<void(gfx::GpuFenceHandle)> return_release_fence_cb;
    PaintRootRenderPass(kSurfaceRect, base::DoNothing(), std::move(return_release_fence_cb));
    output_surface_->CopyOutput(geometry, color_space, std::move(request), gpu::Mailbox());
    output_surface_->SwapBuffersSkipped(kSurfaceRect);
    output_surface_->Flush();
    run_loop.Run();
...
TEST_F(SkiaOutputSurfaceImplTest, CopyOutputBitmapUnsupportedColorSpace)
{
    ...
    constexpr gfx::Rect output_rect(0, 0, 10, 10);
    const gfx::ColorSpace color_space = gfx::ColorSpace::CreatePiecewiseHDR(gfx::ColorSpace::PrimaryID::BT2020, 0.5, 1.5);
    base::RunLoop run_loop;
    std::unique_ptr<CopyOutputResult> result;
    auto request = std::make_unique<CopyOutputRequest>(CopyOutputRequest::ResultFormat::RGBA, CopyOutputRequest::ResultDestination::kSystemMemory,
        base::BindOnce(
