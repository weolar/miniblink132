...
OutputPresenter::Image::Image(
    gpu::SharedImageFactory* factory, gpu::SharedImageRepresentationFactory* representation_factory, SkiaOutputSurfaceDependency* deps)
    : factory_(factory)
...
bool OutputPresenter::Image::Initialize(const gfx::Size& size, const gfx::ColorSpace& color_space, SharedImageFormat format, uint32_t shared_image_usage)
{
    auto mailbox = gpu::Mailbox::GenerateForSharedImage();

    if (!factory_->CreateSharedImage(
            mailbox, format, size, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, deps_->GetSurfaceHandle(), shared_image_usage)) {
        DLOG(ERROR) << "CreateSharedImage failed.";
        return false;
    }
    mailbox_ = mailbox;

    skia_representation_ = representation_factory_->ProduceSkia(mailbox_, deps_->GetSharedContextState());
...
void OutputPresenter::Image::BeginWriteSkia(int sample_count)
{
    ...
    scoped_skia_write_access_ = skia_representation_->BeginScopedWriteAccess(
        sample_count, surface_props, &begin_semaphores, &end_semaphores_, gpu::SharedImageRepresentation::AllowUnclearedAccess::kYes);
    DCHECK(scoped_skia_write_access_);
    if (!begin_semaphores.empty()) {
        scoped_skia_write_access_->surface()->wait(begin_semaphores.size(), begin_semaphores.data(),
...
{
    return scoped_skia_write_access_ ? scoped_skia_write_access_->surface() : nullptr;
}

std::vector<GrBackendSemaphore> OutputPresenter::Image::TakeEndWriteSkiaSemaphores()
{
...
void OutputPresenter::Image::EndWriteSkia(bool force_flush)
{
    ...
    if (!end_semaphores_.empty() || end_state || force_flush) {
        ...
        auto* direct_context = scoped_skia_write_access_->surface()->recordingContext()->asDirectContext();
...
std::unique_ptr<OutputPresenter::Image> OutputPresenter::AllocateSingleImage(gfx::ColorSpace color_space, gfx::Size image_size)
{
    return nullptr;
}
