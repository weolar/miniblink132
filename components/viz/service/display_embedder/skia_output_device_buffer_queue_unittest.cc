...
#define GTEST_TEST_GPU_(test_suite_name, test_name, parent_class, parent_id)                                                                                   \
    ...
    ::testing::TestInfo* const GTEST_TEST_CLASS_NAME_(test_suite_name, test_name)::test_info_                                                                  \
        = ::testing::internal::MakeAndRegisterTestInfo(#test_suite_name, #test_name, nullptr, nullptr, ::testing::internal::CodeLocation(__FILE__, __LINE__),  \
...
#define TEST_F_GPU(test_fixture, test_name) GTEST_TEST_GPU_(test_fixture, test_name, test_fixture, ::testing::internal::GetTypeId<test_fixture>())
...
protected:
    TestOnGpu()
        : wait_(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED)
    {
    }

    void TestBody() override
    {
        auto callback = base::BindLambdaForTesting([&]() { this->TestBodyOnGpu(); });
    ...
    {
        auto teardown = base::BindLambdaForTesting([&]() { this->TearDownOnGpu(); });
    ...
    {
        auto wrap = base::BindOnce(&TestOnGpu::CallOnGpuAndUnblockMain, base::Unretained(this), std::move(callback));
...
public:
    ...
    std::unique_ptr<gpu::SharedImageBacking> CreateSharedImage(const gpu::Mailbox& mailbox, SharedImageFormat format, gpu::SurfaceHandle surface_handle,
        ...
        std::string debug_label, bool is_thread_safe) override
    {
        ...
        return std::make_unique<gpu::TestImageBacking>(mailbox, format, size, color_space, surface_origin, alpha_type, usage, estimated_size);
    }
    std::unique_ptr<gpu::SharedImageBacking> CreateSharedImage(const gpu::Mailbox& mailbox, SharedImageFormat format, const gfx::Size& size,
    ...
    {
        return std::make_unique<gpu::TestImageBacking>(mailbox, format, size, color_space, surface_origin, alpha_type, usage, pixel_data.size());
    }
    std::unique_ptr<gpu::SharedImageBacking> CreateSharedImage(const gpu::Mailbox& mailbox, SharedImageFormat format, const gfx::Size& size,
        const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, gpu::SharedImageUsageSet usage, std::string debug_label,
    ...
    bool IsSupported(gpu::SharedImageUsageSet usage, SharedImageFormat format, const gfx::Size& size, bool thread_safe, gfx::GpuMemoryBufferType gmb_type,
        gpu::GrContextType gr_context_type, base::span<const uint8_t> pixel_data) override
    {
        return true;
    }
...
public:
    ...
    void Present(SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, gfx::FrameData data) override
    {
    ...
    bool ScheduleOverlayPlane(gl::OverlayImage image, std::unique_ptr<gfx::GpuFence> gpu_fence, const gfx::OverlayPlaneData& overlay_plane_data) override
    {
        return true;
    }
    ...
    void SwapComplete()
    {
        ...
        std::move(swap_completion_callbacks_.front()).Run(gfx::SwapCompletionResult(gfx::SwapResult::SWAP_ACK));
...
public:
    ...
    void SetUpOnMain() override
    {
        ...
        dependency_ = std::make_unique<SkiaOutputSurfaceDependencyImpl>(gpu_service_holder_->gpu_service(), surface_handle_);
    }

    virtual DidSwapBufferCompleteCallback GetDidSwapBuffersCompleteCallback()
    {
        return base::BindRepeating(&SkiaOutputDeviceBufferQueueTest::DidSwapBuffersComplete, base::Unretained(this));
    }

    virtual SkiaOutputDevice::ReleaseOverlaysCallback GetReleaseOverlaysCallback()
    {
        return base::BindRepeating(&SkiaOutputDeviceBufferQueueTest::ReleaseOverlays, base::Unretained(this));
    }
    ...
    void SetUpOnGpu() override
    {
        ...
        shared_image_factory_ = std::make_unique<gpu::SharedImageFactory>(dependency_->GetGpuPreferences(), dependency_->GetGpuDriverBugWorkarounds(),
            dependency_->GetGpuFeatureInfo(), dependency_->GetSharedContextState().get(), dependency_->GetSharedImageManager(), memory_tracker_.get(),
            /*is_for_display_compositor=*/true),
        shared_image_factory_->RegisterSharedImageBackingFactoryForTesting(&test_backing_factory_);
        shared_image_representation_factory_
            = std::make_unique<gpu::SharedImageRepresentationFactory>(dependency_->GetSharedImageManager(), memory_tracker_.get());

        auto present_callback = GetDidSwapBuffersCompleteCallback();
        auto release_callback = GetReleaseOverlaysCallback();

        output_device_ = std::make_unique<SkiaOutputDeviceBufferQueue>(std::make_unique<OutputPresenterGL>(presenter_, dependency_.get()), dependency_.get(),
            shared_image_representation_factory_.get(), memory_tracker_.get(), present_callback, release_callback);
    }
    ...
    const gpu::MemoryTracker& memory_tracker()
    {
    ...
    virtual void Present()
    {
        ...
        // empty one.
        output_device_->Present(gfx::Rect(), base::DoNothing(), OutputSurfaceFrame());
    }

    void PageFlipComplete()
    {
        presenter_->SwapComplete();
    }

    SkImageInfo CreateSkImageInfo(const gfx::Size size = kScreenSize)
    {
        return SkImageInfo::Make(size.width(), size.height(), kDefaultColorType, kPremul_SkAlphaType, nullptr);
    }

    void FirstReshape()
    {
        SkiaOutputDevice::ReshapeParams reshape_params = { .image_info = CreateSkImageInfo() };
    ...
    std::unique_ptr<gpu::OverlayImageRepresentation> MakeOverlay()
    {
        ...
        bool success = shared_image_factory_->CreateSharedImage(mailbox, SinglePlaneFormat::kRGBA_8888, gfx::Size(1000, 1000), gfx::ColorSpace::CreateSRGB(),
            GrSurfaceOrigin::kTopLeft_GrSurfaceOrigin, SkAlphaType::kPremul_SkAlphaType, gpu::kNullSurfaceHandle, gpu::SHARED_IMAGE_USAGE_SCANOUT, "TestLabel");
        CHECK(success);
...
protected:
    ...
    base::test::ScopedFeatureList feature_list { ::features::kDeferredOverlaysRelease };
    base::SimpleTestTickClock test_tick_clock_;
};

namespace {

SkiaOutputSurface::OverlayList MakeOverlayList(std::vector<gpu::Mailbox> mailboxes)
{
...
TEST_F_GPU(SkiaOutputDeviceBufferQueueTest, ScheduleOverlaysStillInUse)
{
    ...
    EXPECT_THAT(pending_overlay_mailboxes(), testing::ElementsAre(overlay_1->mailbox()));

    Present();
    EXPECT_THAT(pending_overlay_mailboxes(), testing::IsEmpty());
    EXPECT_THAT(committed_overlay_mailboxes(), testing::ElementsAre(overlay_1->mailbox()));

    PageFlipComplete();
    EXPECT_EQ(1u, params_.size());
    EXPECT_EQ(0u, params_[0].released_overlays.size());

    auto* overlay2 = static_cast<gpu::TestOverlayImageRepresentation*>(overlay_2.get());
    ...
    EXPECT_THAT(params_[1].released_overlays, testing::ElementsAre(overlay_1->mailbox()));
    ...
    EXPECT_THAT(params_[3].released_overlays, testing::ElementsAre(overlay_2->mailbox()));
}
...
TEST_F_GPU(SkiaOutputDeviceBufferQueueTest, InUseOverlaysAreCollected)
{
    ...
    EXPECT_THAT(released_overlays_params_[0], testing::ElementsAre(overlay_2->mailbox()));
}
