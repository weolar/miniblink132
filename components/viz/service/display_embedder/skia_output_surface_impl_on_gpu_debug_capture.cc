...
void DebuggerCaptureBufferCallback(void* si_info_raw, std::unique_ptr<const SkSurface::AsyncReadResult> async_result)
{
    DBG_LOG("skia_gpu.buffer_capture.debugging", "Readback request is %d", async_result != nullptr);
    if (!async_result) {
        return;
    }

    std::unique_ptr<DebuggerCaptureBufferSharedImageInfo> si_info(static_cast<DebuggerCaptureBufferSharedImageInfo*>(si_info_raw));
    int submit_id = 0;
    DBG_DRAW_RECTANGLE_OPT("skia_gpu.captured_buffer.black", viz::VizDebugger::DrawOption({ 0, 0, 0, 255 }), gfx::Vector2dF(), gfx::SizeF(si_info->size));
    DBG_DRAW_RECTANGLE_OPT_BUFF_UV("skia_gpu.captured_buffer.image", DBG_OPT_BLACK, gfx::Vector2dF(), gfx::SizeF(si_info->size), &submit_id, DBG_DEFAULT_UV);

    viz::VizDebugger::BufferInfo buffer_info;
    buffer_info.bitmap.setInfo(SkImageInfo::MakeN32(si_info->size.width(), si_info->size.height(), kPremul_SkAlphaType));
    buffer_info.bitmap.allocPixels();
    SkPixmap pixmap(buffer_info.bitmap.info(), async_result->data(0), buffer_info.bitmap.info().minRowBytes());
...
void AttemptDebuggerBufferCapture(
    ImageContextImpl* context, gpu::SharedContextState* context_state, gpu::SharedImageRepresentationFactory* representation_factory)
{
    CHECK(context_state);
    ...
    auto representation = representation_factory->ProduceSkia(context->mailbox_holder().mailbox, context_state);
    ...
    auto scoped_read = representation->BeginScopedReadAccess(&begin_semaphores_readback, &end_semaphores_readback);
    ...
    if (!begin_semaphores_readback.empty()) {
        bool result = context_state->gr_context()->wait(begin_semaphores_readback.size(), begin_semaphores_readback.data(),
    ...
    DBG_LOG("skia_gpu.buffer_capture.debugging", "Image %s", texture_size.ToString().c_str());

    auto resolution_div = buffer_capture_low_res() ? 2 : 1;
    SkImageInfo dst_info = SkImageInfo::Make(
        texture_size.width() / resolution_div, texture_size.height() / resolution_div, kBGRA_8888_SkColorType, kPremul_SkAlphaType, sk_sp<SkColorSpace>());
    ...
    if (auto* graphite_context = context_state->graphite_context()) {
        ...
        graphite_context->asyncRescaleAndReadPixels(skimage.get(), dst_info, SkIRect::MakeWH(texture_size.width(), texture_size.height()),
            SkSurface::RescaleGamma::kSrc, SkSurface::RescaleMode::kRepeatedLinear, &DebuggerCaptureBufferCallback, si_info.release());
    } else {
        skimage->asyncRescaleAndReadPixels(dst_info, SkIRect::MakeWH(texture_size.width(), texture_size.height()), SkSurface::RescaleGamma::kSrc,
            SkSurface::RescaleMode::kRepeatedLinear, &DebuggerCaptureBufferCallback, si_info.release());
    }
}
