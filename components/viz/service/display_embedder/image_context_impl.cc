...
bool DawnYCbCrVkDescriptorsAreEqual(wgpu::YCbCrVkDescriptor left, wgpu::YCbCrVkDescriptor right)
{
...
SkColor4f GetFallbackColorForPlane(viz::SharedImageFormat format, int plane_index)
{
    ...
    case viz::SharedImageFormat::PlaneConfig::kY_U_V_A:
        return (plane_index == 0 || plane_index == 3) ? SkColors::kWhite : SkColors::kGray;
...
ImageContextImpl::ImageContextImpl(const gpu::MailboxHolder& mailbox_holder, const gfx::Size& size, SharedImageFormat format, bool maybe_concurrent_reads,
    const std::optional<gpu::VulkanYCbCrInfo>& ycbcr_info, sk_sp<SkColorSpace> color_space, bool is_for_render_pass, bool raw_draw_if_possible)
...
void ImageContextImpl::SetPromiseImageTextures(std::vector<sk_sp<GrPromiseImageTexture>> promise_image_textures)
{
    ...
    if (fallback_context_state_) {
        if (fallback_context_state_->gr_context()) {
            ...
            for (auto& fallback_texture : graphite_fallback_textures_) {
                fallback_context_state_->gpu_main_graphite_recorder()->deleteBackendTexture(fallback_texture);
            }
        }
    }
    fallback_context_state_ = nullptr;
...
void ImageContextImpl::CreateFallbackImage(gpu::SharedContextState* context_state)
{
    ...
    if (context_state->graphite_context()) {
        ...
        if (tex_infos.size() != static_cast<size_t>(num_planes) || base::ranges::any_of(tex_infos, [](const auto& tex_info) { return !tex_info.isValid(); })) {
            DLOG(ERROR) << "Invalid Graphite texture infos for format: " << format().ToString();
            return;
        }

#if BUILDFLAG(SKIA_USE_DAWN)
        skgpu::graphite::DawnTextureInfo dawn_info;
        bool success = skgpu::graphite::TextureInfos::GetDawnTextureInfo(tex_infos[0], &dawn_info);
        ...
        for (int plane_index = 0; plane_index < num_planes; plane_index++) {
            SkISize sk_size = gfx::SizeToSkISize(format().GetPlaneSize(plane_index, size()));
            auto tex_info = gpu::FallbackGraphiteBackendTextureInfo(tex_infos[plane_index]);
            graphite_fallback_textures_.push_back(context_state->gpu_main_graphite_recorder()->createBackendTexture(sk_size, tex_info));

            SkColorType color_type = ToClosestSkColorType(/*gpu_compositing=*/true, format(), plane_index);

            auto sk_surface = SkSurfaces::WrapBackendTexture(context_state->gpu_main_graphite_recorder(), graphite_fallback_textures_[plane_index], color_type,
                color_space(),
                /*props=*/nullptr);
            if (!sk_surface) {
            ...
            sk_surface->getCanvas()->clear(GetFallbackColorForPlane(format(), plane_index));
        }
    ...
    for (int plane_index = 0; plane_index < num_planes; plane_index++) {
        ...
        auto fallback_texture = fallback_context_state_->gr_context()->createBackendTexture(
            size().width(), size().height(), formats[plane_index], GetFallbackColorForPlane(format(), plane_index), skgpu::Mipmapped::kNo, GrRenderable::kYes);
...
void ImageContextImpl::BeginAccessIfNecessary(gpu::SharedContextState* context_state, gpu::SharedImageRepresentationFactory* representation_factory,
    std::vector<GrBackendSemaphore>* begin_semaphores, std::vector<GrBackendSemaphore>* end_semaphores)
{
    ...
    if (!BeginAccessIfNecessaryInternal(context_state, representation_factory, begin_semaphores, end_semaphores)) {
        CreateFallbackImage(context_state);
    }
}

bool ImageContextImpl::BeginRasterAccess(gpu::SharedImageRepresentationFactory* representation_factory)
{
    if (paint_op_buffer()) {
    ...
    auto raster = raw_draw_if_possible_ ? representation_factory->ProduceRaster(mailbox_holder().mailbox) : nullptr;
    if (!raster)
        return false;
...
bool ImageContextImpl::BeginAccessIfNecessaryInternal(gpu::SharedContextState* context_state, gpu::SharedImageRepresentationFactory* representation_factory,
    std::vector<GrBackendSemaphore>* begin_semaphores, std::vector<GrBackendSemaphore>* end_semaphores)
{
    ...
    if (!promise_image_textures_.empty()) {
        CHECK_EQ(promise_image_textures_.size(), owned_promise_image_textures_.size());
        return true;
    }
    ...
    if (!representation_) {
        auto representation = representation_factory->ProduceSkia(mailbox_holder().mailbox, context_state);
        if (!representation) {
        ...
        if (representation->size() != size()) {
            DLOG(ERROR) << "Failed to fulfill the promise texture - SharedImage "
                        ...
                        << representation->size().ToString() << " vs " << size().ToString();
            return false;
        }
    ...
    representation_scoped_read_access_ = representation_->BeginScopedReadAccess(begin_semaphores, end_semaphores);
    if (!representation_scoped_read_access_) {
    ...
    int num_planes = format().PrefersExternalSampler() ? 1 : format().NumberOfPlanes();
    if (context_state->graphite_context()) {
#if BUILDFLAG(IS_ANDROID) && BUILDFLAG(SKIA_USE_DAWN)
        ...
        CHECK(skgpu::graphite::TextureInfos::GetDawnTextureInfo(representation_scoped_read_access_->graphite_texture(0).info(), &fulfillment_texture_info));

        wgpu::YCbCrVkDescriptor promise_texture_ycbcr_desc = {};
        if (ycbcr_info()) {
            promise_texture_ycbcr_desc = gpu::ToDawnYCbCrVkDescriptor(ycbcr_info().value());
        }
        wgpu::YCbCrVkDescriptor fulfillment_texture_ycbcr_desc = fulfillment_texture_info.fYcbcrVkDescriptor;

        if (!DawnYCbCrVkDescriptorsAreEqual(promise_texture_ycbcr_desc, fulfillment_texture_ycbcr_desc)) {
        ...
        for (int plane_index = 0; plane_index < num_planes; plane_index++) {
            graphite_textures_.push_back(representation_scoped_read_access_->graphite_texture(plane_index));
        }
    } else {
        CHECK(context_state->gr_context());
        for (int plane_index = 0; plane_index < num_planes; plane_index++) {
            promise_image_textures_.push_back(representation_scoped_read_access_->promise_image_texture(plane_index));
        }
    }
...
void ImageContextImpl::EndAccessIfNecessary()
{
    ...
    if (representation_->SupportsMultipleConcurrentReadAccess() && !is_for_render_pass_) {
        return;
    }
