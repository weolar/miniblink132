...
void SoftwareOutputDeviceWinBase::Resize(const gfx::Size& viewport_pixel_size, float scale_factor)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(!in_paint_);
...
SkCanvas* SoftwareOutputDeviceWinBase::BeginPaint(const gfx::Rect& damage_rect)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(!in_paint_);
...
SoftwareOutputDeviceWinDirect::SoftwareOutputDeviceWinDirect(HWND hwnd, OutputDeviceBacking* backing)
...
SkCanvas* SoftwareOutputDeviceWinDirect::BeginPaintDelegated()
{
    if (!canvas_) {
        ...
        base::UnsafeSharedMemoryRegion* region = backing_->GetSharedMemoryRegion(viewport_pixel_size_);
        if (region && region->IsValid()) {
            canvas_ = skia::CreatePlatformCanvasWithSharedSection(
                viewport_pixel_size_.width(), viewport_pixel_size_.height(), true, region->GetPlatformHandle(), skia::CRASH_ON_FAILURE);
        }
    }
    return canvas_.get();
}

void SoftwareOutputDeviceWinDirect::EndPaintDelegated(const gfx::Rect& damage_rect)
{
    if (!canvas_)
        return;
    ...
    skia::CopyHDC(dib_dc, hdc, damage_rect.x(), damage_rect.y(), canvas_->imageInfo().isOpaque(), src_rect, canvas_->getTotalMatrix());
...
SoftwareOutputDeviceWinProxy::SoftwareOutputDeviceWinProxy(HWND hwnd, mojo::PendingRemote<mojom::LayeredWindowUpdater> layered_window_updater)
    : SoftwareOutputDeviceWinBase(hwnd)
...
void SoftwareOutputDeviceWinProxy::OnSwapBuffers(SwapBuffersCallback swap_ack_callback, gfx::FrameData data)
{
...
void SoftwareOutputDeviceWinProxy::ResizeDelegated()
{
    ...
    if (!ResourceSizes::MaybeSizeInBytes(viewport_pixel_size_, SinglePlaneFormat::kRGBA_8888, &required_bytes)) {
        DLOG(ERROR) << "Invalid viewport size " << viewport_pixel_size_.ToString();
        return;
    }

    base::UnsafeSharedMemoryRegion region = base::UnsafeSharedMemoryRegion::Create(required_bytes);
    if (!region.IsValid()) {
    ...
    *(int*)1 = 1;
    layered_window_updater_->OnAllocatedSharedMemory(viewport_pixel_size_, std::move(region), nullptr);
}
...
void SoftwareOutputDeviceWinProxy::EndPaintDelegated(const gfx::Rect& damage_rect)
{
    ...
    layered_window_updater_->Draw(damage_rect, base::BindOnce(&SoftwareOutputDeviceWinProxy::DrawAck, base::Unretained(this)));
...
std::unique_ptr<SoftwareOutputDevice> CreateSoftwareOutputDeviceWin(HWND hwnd, OutputDeviceBacking* backing, mojom::DisplayClient* display_client)
{
    if (NeedsToUseLayerWindow(hwnd)) {
        ...
        return std::make_unique<SoftwareOutputDeviceWinProxy>(hwnd, std::move(layered_window_updater));
    } else {
