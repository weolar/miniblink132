...
constexpr wgpu::TextureFormat kSwapChainFormat = wgpu::TextureFormat::RGBA8Unorm;
#else
...
std::unique_ptr<SkiaOutputDeviceDawn> SkiaOutputDeviceDawn::Create(scoped_refptr<gpu::SharedContextState> context_state, gfx::SurfaceOrigin origin,
...
{
    auto output_device = std::make_unique<SkiaOutputDeviceDawn>(context_state, origin, memory_tracker, std::move(did_swap_buffer_complete_callback), PassKey());
...
SkiaOutputDeviceDawn::SkiaOutputDeviceDawn(scoped_refptr<gpu::SharedContextState> context_state, gfx::SurfaceOrigin origin, gpu::MemoryTracker* memory_tracker,
    DidSwapBufferCompleteCallback did_swap_buffer_complete_callback, base::PassKey<SkiaOutputDeviceDawn>)
    : SkiaOutputDevice(
        /*gr_context=*/nullptr, context_state->graphite_context(), memory_tracker, did_swap_buffer_complete_callback)
    , context_state_(std::move(context_state))
{
    ...
    capabilities_.sk_color_type_map[SinglePlaneFormat::kRGBA_8888] = kSurfaceColorType;
    capabilities_.sk_color_type_map[SinglePlaneFormat::kRGBX_8888] = kSurfaceColorType;
    capabilities_.sk_color_type_map[SinglePlaneFormat::kBGRA_8888] = kSurfaceColorType;
    capabilities_.sk_color_type_map[SinglePlaneFormat::kBGRX_8888] = kSurfaceColorType;
}
...
bool SkiaOutputDeviceDawn::Initialize(gpu::SurfaceHandle surface_handle)
{
    ...
    vsync_provider_ = std::make_unique<gl::VSyncProviderWin>(window_handle_to_draw_to);
    ...
    auto& scoped_java_surface = absl::get<gl::ScopedJavaSurface>(surface_record.surface_variant);
    ...
    wgpu::Status result = surface_.GetCapabilities(context_provider->GetAdapter(), &caps);
...
void SkiaOutputDeviceDawn::Present(const std::optional<gfx::Rect>& update_rect, BufferPresentedCallback feedback, OutputSurfaceFrame frame)
{
    DCHECK(!update_rect);
    StartSwapBuffers({});
    surface_.Present();
    ...
    if (vsync_provider_ && vsync_provider_->GetVSyncParametersIfAvailable(&vsync_timebase, &vsync_interval)) {
        ...
        flags = gfx::PresentationFeedback::kVSync | gfx::PresentationFeedback::kHWClock;
    }
    std::move(feedback).Run(gfx::PresentationFeedback(timestamp, vsync_interval, flags));
}

SkSurface* SkiaOutputDeviceDawn::BeginPaint(std::vector<GrBackendSemaphore>* end_semaphores)
{
    wgpu::SurfaceTexture texture;
    surface_.GetCurrentTexture(&texture);
    auto backend_texture = skgpu::graphite::BackendTextures::MakeDawn(texture.texture.Get());

    SkSurfaceProps surface_props;
    sk_surface_
        = SkSurfaces::WrapBackendTexture(context_state_->gpu_main_graphite_recorder(), backend_texture, kSurfaceColorType, sk_color_space_, &surface_props);
...
{
    CHECK(sk_surface_);
    if (GrDirectContext* direct_context = GrAsDirectContext(sk_surface_->recordingContext())) {
        direct_context->flush(sk_surface_.get(), SkSurfaces::BackendSurfaceAccess::kPresent, {});
    }
    sk_surface_.reset();
}
