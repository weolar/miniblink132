...
{
    size_t bytes;
    if (!ResourceSizes::MaybeSizeInBytes(viewport_size, SinglePlaneFormat::kRGBA_8888, &bytes)) {
        return false;
    }
...
base::UnsafeSharedMemoryRegion* OutputDeviceBacking::GetSharedMemoryRegion(const gfx::Size& viewport_size)
{
    ...
    if (!region_.IsValid()) {
        ...
        if (!region_.IsValid()) {
            LOG(ERROR) << "Shared memory region create failed on " << max_viewport_bytes << " bytes";
            return nullptr;
        }
