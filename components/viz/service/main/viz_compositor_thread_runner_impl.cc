...
std::unique_ptr<VizCompositorThreadType> CreateAndStartCompositorThread()
{
    ...
    auto thread = std::make_unique<base::android::JavaHandlerThread>(kThreadName, thread_type);
    thread->Start();
    return thread;
    ...
    thread_options.message_pump_type = platform->GetPlatformProperties().message_pump_type_for_viz_compositor;
...
VizCompositorThreadRunnerImpl::~VizCompositorThreadRunnerImpl()
{
    task_runner_->PostTask(FROM_HERE, base::BindOnce(&VizCompositorThreadRunnerImpl::TearDownOnCompositorThread, base::Unretained(this)));
    thread_->Stop();
}

bool VizCompositorThreadRunnerImpl::CreateHintSessionFactory(base::flat_set<base::PlatformThreadId> thread_ids, base::RepeatingClosure* wake_up_closure)
{
    base::WaitableEvent event;
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VizCompositorThreadRunnerImpl::CreateHintSessionFactoryOnCompositorThread, base::Unretained(this), std::move(thread_ids),
...
void VizCompositorThreadRunnerImpl::CreateHintSessionFactoryOnCompositorThread(
    base::flat_set<base::PlatformThreadId> thread_ids, base::RepeatingClosure* wake_up_closure, base::WaitableEvent* event)
{
    *(int*)1 = 1;
...
//         *wake_up_closure
//             = base::BindPostTask(task_runner_, base::BindRepeating(&VizCompositorThreadRunnerImpl::WakeUpOnCompositorThread, weak_factory_.GetWeakPtr()));
//     }
//     event->Signal();
}
...
void VizCompositorThreadRunnerImpl::CreateFrameSinkManager(mojom::FrameSinkManagerParamsPtr params, GpuServiceImpl* gpu_service)
{
    //shared_image_interface_provider_ = std::make_unique<SharedImageInterfaceProvider>(gpu_service);

    // All of the unretained objects are owned on the GPU thread and destroyed
    // after VizCompositorThread has been shutdown.
    task_runner_->PostTask(FROM_HERE,
        base::BindOnce(&VizCompositorThreadRunnerImpl::CreateFrameSinkManagerOnCompositorThread, base::Unretained(this), std::move(params),
            /*base::Unretained(gpu_service)*/nullptr));
}

void VizCompositorThreadRunnerImpl::CreateFrameSinkManagerOnCompositorThread(mojom::FrameSinkManagerParamsPtr params, GpuServiceImpl* gpu_service)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
    ...
    //base::trace_event::MemoryDumpManager::GetInstance()->RegisterDumpProvider(server_shared_bitmap_manager_.get(), "ServerSharedBitmapManager", task_runner_);

    base::CommandLine* command_line = base::CommandLine::ForCurrentProcess();
    const bool headless = command_line->HasSwitch(switches::kHeadless);
    const bool run_all_compositor_stages_before_draw = command_line->HasSwitch(switches::kRunAllCompositorStagesBeforeDraw);

    if (gpu_service) {
        *(int*)1 = 1;
//         // Create OutputSurfaceProvider usable for GPU + software compositing.
//         gpu_memory_buffer_manager_
//             = std::make_unique<InProcessGpuMemoryBufferManager>(gpu_service->gpu_memory_buffer_factory(), gpu_service->sync_point_manager());
//         output_surface_provider_ = std::make_unique<OutputSurfaceProviderImpl>(gpu_service, headless);
// 
...
//             = std::make_unique<GmbVideoFramePoolContextProviderImpl>(gpu_service, gpu_memory_buffer_manager_.get(), gpu_service->gpu_memory_buffer_factory());
    } else {
        // Create OutputSurfaceProvider usable for software compositing only.
        output_surface_provider_ = std::make_unique<OutputSurfaceProviderImpl>(headless);
    }
    ...
    if (params->use_activation_deadline) {
        init_params.activation_deadline_in_frames = params->activation_deadline_in_frames;
    }
    init_params.output_surface_provider = output_surface_provider_.get();
    init_params.gpu_service = gpu_service;
    init_params.gmb_context_provider = gmb_video_frame_pool_context_provider_.get();
    init_params.restart_id = params->restart_id;
    init_params.run_all_compositor_stages_before_draw = run_all_compositor_stages_before_draw;
    init_params.log_capture_pipeline_in_webrtc = features::ShouldWebRtcLogCapturePipeline();
    ...
    frame_sink_manager_->BindAndSetClient(
        std::move(params->frame_sink_manager), nullptr, std::move(params->frame_sink_manager_client), /*shared_image_interface_provider_.get()*/nullptr);
}

void VizCompositorThreadRunnerImpl::RequestBeginFrameForGpuService(bool toggle)
{
    task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&VizCompositorThreadRunnerImpl::RequestBeginFrameForGpuServiceOnCompositorThread, base::Unretained(this), toggle));
}

void VizCompositorThreadRunnerImpl::RequestBeginFrameForGpuServiceOnCompositorThread(bool toggle)
{
    DCHECK(task_runner_->BelongsToCurrentThread());
...
void VizCompositorThreadRunnerImpl::TearDownOnCompositorThread()
{
    ...
    if (server_shared_bitmap_manager_) {
        //base::trace_event::MemoryDumpManager::GetInstance()->UnregisterDumpProvider(server_shared_bitmap_manager_.get());
    }
