...
public:
    MOCK_METHOD1(PostCompositorThreadCreated, void(base::SingleThreadTaskRunner*));
...
public:
    explicit MockVizCompositorThreadRunner(base::SingleThreadTaskRunner* task_runner)
    ...
    MOCK_METHOD2(CreateFrameSinkManager, void(mojom::FrameSinkManagerParamsPtr, GpuServiceImpl*));
...
public:
    explicit MockPowerMonitorSource(bool* leak_guard)
        ...
        , leak_guard_(leak_guard)
    {
        *leak_guard_ = true;
    }

    ~MockPowerMonitorSource() override
    {
        *leak_guard_ = false;
    }

    base::PowerStateObserver::BatteryPowerStatus GetBatteryPowerStatus() const override
    {
...
TEST(VizMainImplTest, OopVizDependencyInjection)
{
    ...
    std::unique_ptr<MockUkmRecorder> mock_ukm_recorder = std::make_unique<MockUkmRecorder>(*factory);
    ...
    external_deps.power_monitor_source = std::make_unique<MockPowerMonitorSource>(&mock_source_is_alive);
    ...
    VizMainImpl impl(&mock_delegate, std::move(external_deps), std::move(gpu_init));
