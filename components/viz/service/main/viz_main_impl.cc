...
std::unique_ptr<base::Thread> CreateAndStartIOThread()
{
    ...
    io_thread->task_runner()->PostTask(FROM_HERE, base::BindOnce([]() {
...
VizMainImpl::ExternalDependencies::ExternalDependencies(ExternalDependencies&& other) = default;

VizMainImpl::ExternalDependencies& VizMainImpl::ExternalDependencies::operator=(ExternalDependencies&& other) = default;

VizMainImpl::VizMainImpl(Delegate* delegate, ExternalDependencies dependencies, std::unique_ptr<gpu::GpuInit> gpu_init)
    : delegate_(delegate)
    , dependencies_(std::move(dependencies))
    //, gpu_init_(std::move(gpu_init))
    , gpu_thread_task_runner_(base::SingleThreadTaskRunner::GetCurrentDefault())
{
    //DCHECK(gpu_init_);
    *(int*)1 = 1;
    ...
    if (dependencies_.power_monitor_source) {
        base::PowerMonitor::GetInstance()->Initialize(std::move(dependencies_.power_monitor_source));
    }
    ...
    if (!viz_compositor_thread_runner_) {
        viz_compositor_thread_runner_impl_ = std::make_unique<VizCompositorThreadRunnerImpl>();
        viz_compositor_thread_runner_ = viz_compositor_thread_runner_impl_.get();
    }
    if (delegate_) {
        delegate_->PostCompositorThreadCreated(viz_compositor_thread_runner_->task_runner());
    }
...
//         ukm::DelegatingUkmRecorder::Get()->AddDelegate(dependencies_.ukm_recorder->GetWeakPtr());
//     }
// 
...
// #endif
//     init_params.exit_callback = base::BindOnce(&VizMainImpl::ExitProcess, base::Unretained(this));
// 
//     init_params.vulkan_implementation = gpu_init_->vulkan_implementation();
//     gpu_service_ = std::make_unique<GpuServiceImpl>(gpu_init_->gpu_preferences(), gpu_init_->gpu_info(), gpu_init_->gpu_feature_info(),
//         gpu_init_->gpu_info_for_hardware_gpu(), gpu_init_->gpu_feature_info_for_hardware_gpu(), gpu_init_->gpu_extra_info(), std::move(init_params));
//     gpu_service_->SetRequestBeginFrameForGpuServiceCB(base::BindRepeating(&VizMainImpl::RequestBeginFrameForGpuService, base::Unretained(this)));
...
//         ukm::DelegatingUkmRecorder::Get()->RemoveDelegate(dependencies_.ukm_recorder.get());
}
...
void VizMainImpl::CreateGpuService(mojo::PendingReceiver<mojom::GpuService> pending_receiver, mojo::PendingRemote<mojom::GpuHost> pending_gpu_host,
    mojo::PendingRemote<discardable_memory::mojom::DiscardableSharedMemoryManager> discardable_memory_manager,
    base::UnsafeSharedMemoryRegion use_shader_cache_shm_region, mojom::GpuServiceCreationParamsPtr params)
{
    *(int*)1 = 1;
//     DCHECK(gpu_thread_task_runner_->BelongsToCurrentThread());
// 
...
//             = base::MakeRefCounted<discardable_memory::ClientDiscardableSharedMemoryManager>(std::move(discardable_memory_manager), io_task_runner());
//         base::DiscardableMemoryAllocator::SetInstance(discardable_shared_memory_manager_.get());
//     }
// 
//     gpu_service_->InitializeWithHost(gpu_host.Unbind(), gpu::GpuProcessShmCount(std::move(use_shader_cache_shm_region)),
//         gpu_init_->TakeDefaultOffscreenSurface(), std::move(params),
// #if BUILDFLAG(IS_ANDROID)
//         dependencies_.sync_point_manager, dependencies_.shared_image_manager, dependencies_.scheduler,
// #endif
...
// 
//         CompositorGpuThread* compositor_gpu_thread = gpu_service_->compositor_gpu_thread();
// 
//         if (compositor_gpu_thread && base::FeatureList::IsEnabled(::features::kEnableADPFGpuCompositorThread)) {
...
//             base::BindOnce(
//                 [](base::PlatformThreadId* io_thread_id, base::WaitableEvent* event) {
...
//         if (viz_compositor_thread_runner_->CreateHintSessionFactory(std::move(gpu_process_thread_ids), &wake_up_closure)) {
//             gpu_service_->SetWakeUpGpuClosure(std::move(wake_up_closure));
//         }
//     }
// 
//     if (!pending_frame_sink_manager_params_.is_null()) {
//         CreateFrameSinkManagerInternal(std::move(pending_frame_sink_manager_params_));
...
void VizMainImpl::SetRenderParams(gfx::FontRenderParams::SubpixelRendering subpixel_rendering, float text_contrast, float text_gamma)
{
    skia::LegacyDisplayGlobals::SetCachedParams(gfx::FontRenderParams::SubpixelRenderingToSkiaPixelGeometry(subpixel_rendering), text_contrast, text_gamma);
}

#if BUILDFLAG(IS_WIN)
void VizMainImpl::CreateInfoCollectionGpuService(mojo::PendingReceiver<mojom::InfoCollectionGpuService> pending_receiver)
{
    *(int*)1 = 1;
//     DCHECK(gpu_thread_task_runner_->BelongsToCurrentThread());
...
//         gpu_thread_task_runner_, io_task_runner(), gpu_init_->device_perf_info().value(), gpu_init_->gpu_info().active_gpu(), std::move(pending_receiver));
}
#endif
...
void VizMainImpl::CreateFrameSinkManager(mojom::FrameSinkManagerParamsPtr params)
{
...
void VizMainImpl::CreateFrameSinkManagerInternal(mojom::FrameSinkManagerParamsPtr params)
{
    //DCHECK(gpu_service_);
    DCHECK(gpu_thread_task_runner_->BelongsToCurrentThread());
    ...
    viz_compositor_thread_runner_->CreateFrameSinkManager(std::move(params), /*gpu_service_.get()*/nullptr);
}
...
void VizMainImpl::StartDebugStream(mojo::PendingRemote<mojom::VizDebugOutput> pending_debug_output)
{
...
void VizMainImpl::ExitProcess(/*ExitCode*/int immediate_exit_code)
{
    DCHECK(gpu_thread_task_runner_->BelongsToCurrentThread());
...
//         base::Process::TerminateCurrentProcessImmediately(static_cast<int>(immediate_exit_code));
//     }
...
discardable_memory::ClientDiscardableSharedMemoryManager* VizMainImpl::discardable_shared_memory_manager()
{
