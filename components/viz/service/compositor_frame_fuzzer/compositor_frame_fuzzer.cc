...
public:
    Env()
    {
        ...
        auto png_dir_path
            = command_line->HasSwitch("dump-to-png") ? std::optional<base::FilePath>(command_line->GetSwitchValuePath("dump-to-png")) : std::nullopt;
...
DEFINE_BINARY_PROTO_FUZZER(const viz::proto::CompositorRenderPass& render_pass_spec)
{
    static base::NoDestructor<Env> env;

    viz::FuzzedData fuzzed_frame = viz::BuildFuzzedCompositorFrame(render_pass_spec);

    env->browser_process->EmbedFuzzedCompositorFrame(std::move(fuzzed_frame.frame), std::move(fuzzed_frame.allocated_bitmaps));
}
