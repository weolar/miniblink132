...
public:
    ...
    void EndPaint() override
    {
        ...
        std::optional<std::vector<uint8_t>> output = gfx::PNGCodec::Encode(static_cast<const unsigned char*>(input_pixmap.addr()), color_format,
            gfx::Size(input_pixmap.width(), input_pixmap.height()), input_pixmap.rowBytes(),
...
private:
    ...
    base::FilePath NextOutputFilePath()
    {
        ...
        std::string file_name = base::StringPrintf("%0*u.png", kPaddedIntLength, next_file_id_++);
...
FuzzerSoftwareOutputSurfaceProvider::FuzzerSoftwareOutputSurfaceProvider(std::optional<base::FilePath> png_dir_path)
    : png_dir_path_(png_dir_path)
{
}

FuzzerSoftwareOutputSurfaceProvider::~FuzzerSoftwareOutputSurfaceProvider() = default;

std::unique_ptr<DisplayCompositorMemoryAndTaskController> FuzzerSoftwareOutputSurfaceProvider::CreateGpuDependency(
    bool gpu_compositing, gpu::SurfaceHandle surface_handle)
{
    return nullptr;
}

std::unique_ptr<OutputSurface> FuzzerSoftwareOutputSurfaceProvider::CreateOutputSurface(gpu::SurfaceHandle surface_handle, bool gpu_compositing,
    mojom::DisplayClient* display_client, DisplayCompositorMemoryAndTaskController* gpu_dependency, const RendererSettings& renderer_settings,
    const DebugRendererSettings* debug_settings)
{
    std::unique_ptr<SoftwareOutputDevice> software_output_device
        = png_dir_path_ ? std::make_unique<PNGSoftwareOutputDevice>(*png_dir_path_) : std::make_unique<SoftwareOutputDevice>();
    return std::make_unique<SoftwareOutputSurface>(std::move(software_output_device));
}

gpu::SharedImageManager* FuzzerSoftwareOutputSurfaceProvider::GetSharedImageManager()
{
    return nullptr;
}

gpu::SyncPointManager* FuzzerSoftwareOutputSurfaceProvider::GetSyncPointManager()
{
    return nullptr;
}
