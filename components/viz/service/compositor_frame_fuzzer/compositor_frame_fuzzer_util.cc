...
{
    return gfx::Rect(proto_rect.x(), proto_rect.y(), proto_rect.width(), proto_rect.height());
}

gfx::Transform GetTransformFromProtobuf(const proto::Transform& proto_transform)
{
    gfx::Transform transform;

    // Note: There are no checks here that disallow a non-invertible transform
    // (for instance, if |scale_x| or |scale_y| is 0).
    transform.Scale(MakeNormal(proto_transform.scale_x()), MakeNormal(proto_transform.scale_y()));

    transform.Rotate(MakeNormal(proto_transform.rotate()));

    transform.Translate(MakeNormal(proto_transform.translate_x()), MakeNormal(proto_transform.translate_y()));
...
public:
    ...
    FuzzedCompositorFrameBuilder& operator=(const FuzzedCompositorFrameBuilder&) = delete;
...
private:
    CompositorRenderPassId AddRenderPass(const proto::CompositorRenderPass& render_pass_spec);

    // Helper methods for AddRenderPass. Try* methods may return before
    // creating the quad in order to adhere to memory limits.
    void AddSolidColorDrawQuad(CompositorRenderPass* pass, const gfx::Rect& rect, const gfx::Rect& visible_rect, const proto::DrawQuad& quad_spec);
    void TryAddTileDrawQuad(CompositorRenderPass* pass, const gfx::Rect& rect, const gfx::Rect& visible_rect, const proto::DrawQuad& quad_spec);
    void TryAddRenderPassDrawQuad(CompositorRenderPass* pass, const gfx::Rect& rect, const gfx::Rect& visible_rect, const proto::DrawQuad& quad_spec);
    ...
    // identity transform.
    void ConfigureSharedQuadState(SharedQuadState* shared_quad_state, const proto::DrawQuad& quad_spec);
...
FuzzedData FuzzedCompositorFrameBuilder::Build(const proto::CompositorRenderPass& render_pass_spec)
{
    static FrameTokenGenerator next_frame_token;

    data_.frame.metadata.begin_frame_ack.frame_id = BeginFrameId(BeginFrameArgs::kManualSourceId, BeginFrameArgs::kStartingFrameNumber);
...
CompositorRenderPassId FuzzedCompositorFrameBuilder::AddRenderPass(const proto::CompositorRenderPass& render_pass_spec)
{
    auto pass = CompositorRenderPass::Create();
    gfx::Rect rp_output_rect = GetRectFromProtobuf(render_pass_spec.output_rect());
    gfx::Rect rp_damage_rect = GetRectFromProtobuf(render_pass_spec.damage_rect());
    ...
    gfx::Transform transform_to_root_target
        = render_pass_spec.has_transform_to_root_target() ? GetTransformFromProtobuf(render_pass_spec.transform_to_root_target()) : gfx::Transform();
    pass->SetNew(pass_id_generator_.GenerateNextId(), rp_output_rect, rp_damage_rect, transform_to_root_target);
    ...
    for (const proto::DrawQuad& quad_spec : render_pass_spec.quad_list()) {
        ...
        case proto::DrawQuad::kSolidColorQuad: {
            AddSolidColorDrawQuad(pass.get(), quad_rect, quad_visible_rect, quad_spec);
            break;
        }
        ...
        case proto::DrawQuad::kRenderPassQuad: {
            TryAddRenderPassDrawQuad(pass.get(), quad_rect, quad_visible_rect, quad_spec);
            break;
        }
...
void FuzzedCompositorFrameBuilder::AddSolidColorDrawQuad(
    CompositorRenderPass* pass, const gfx::Rect& rect, const gfx::Rect& visible_rect, const proto::DrawQuad& quad_spec)
{
    ...
    quad->force_anti_aliasing_off = quad_spec.solid_color_quad().force_anti_aliasing_off();
}

void FuzzedCompositorFrameBuilder::TryAddTileDrawQuad(
    CompositorRenderPass* pass, const gfx::Rect& rect, const gfx::Rect& visible_rect, const proto::DrawQuad& quad_spec)
{
    gfx::Size tile_size = GetSizeFromProtobuf(quad_spec.tile_quad().texture_size());
    ...
    if (!TryReserveBitmapBytes(tile_size)) {
        VLOG(1) << "Skipping TileDrawQuad: bitmap of size " << tile_size.ToString() << " can't be allocated.";
        return;
    }

    FuzzedBitmap* fuzzed_bitmap = AllocateFuzzedBitmap(tile_size, GetColorFromProtobuf(quad_spec.tile_quad().texture_color()));
    TransferableResource transferable_resource
        = TransferableResource::MakeSoftwareSharedBitmap(fuzzed_bitmap->id, gpu::SyncToken(), fuzzed_bitmap->size, SinglePlaneFormat::kRGBA_8888);
    ...
    quad->tex_coord_rect = gfx::RectF(GetRectFromProtobuf(quad_spec.tile_quad().tex_coord_rect()));
    ...
    quad->force_anti_aliasing_off = quad_spec.tile_quad().force_anti_aliasing_off();
    ...
    CompositorRenderPass* pass, const gfx::Rect& rect, const gfx::Rect& visible_rect, const proto::DrawQuad& quad_spec)
{
    ...
    gfx::Size render_pass_size = GetRectFromProtobuf(quad_spec.render_pass_quad().render_pass().output_rect()).size();

    // RenderPass texture dimensions are rounded up to a
    // multiple of 64 pixels to reduce fragmentation.
    constexpr int multiple = 64;
    if (!cc::MathUtil::VerifyRoundup(render_pass_size.width(), multiple) || !cc::MathUtil::VerifyRoundup(render_pass_size.height(), multiple)) {
        VLOG(1) << "Skipping CompositorRenderPassDrawQuad: bitmap of size " << render_pass_size.ToString() << " can't be allocated.";
        return;
    }
    render_pass_size
        = gfx::Size(cc::MathUtil::UncheckedRoundUp(render_pass_size.width(), multiple), cc::MathUtil::UncheckedRoundUp(render_pass_size.height(), multiple));
    ...
    CompositorRenderPassId child_pass_id = AddRenderPass(quad_spec.render_pass_quad().render_pass());
    ...
    gfx::RectF tex_coord_rect
        = gfx::RectF(quad_spec.render_pass_quad().has_tex_coord_rect() ? GetRectFromProtobuf(quad_spec.render_pass_quad().tex_coord_rect()) : rect);
...
void FuzzedCompositorFrameBuilder::ConfigureSharedQuadState(SharedQuadState* shared_quad_state, const proto::DrawQuad& quad_spec)
{
    if (quad_spec.has_sqs()) {
        shared_quad_state->quad_to_target_transform = GetTransformFromProtobuf(quad_spec.sqs().transform());
        shared_quad_state->quad_layer_rect = GetRectFromProtobuf(quad_spec.sqs().layer_rect());
        shared_quad_state->visible_quad_layer_rect = GetRectFromProtobuf(quad_spec.sqs().visible_rect());
        if (quad_spec.sqs().is_clipped()) {
            shared_quad_state->clip_rect = GetRectFromProtobuf(quad_spec.sqs().clip_rect());
        }
        shared_quad_state->are_contents_opaque = quad_spec.sqs().are_contents_opaque();
        shared_quad_state->opacity = Normalize(quad_spec.sqs().opacity());
        shared_quad_state->blend_mode = SkBlendMode::kSrcOver;
        shared_quad_state->sorting_context_id = quad_spec.sqs().sorting_context_id();
    } else {
        if (quad_spec.quad_case() == proto::DrawQuad::kRenderPassQuad && quad_spec.render_pass_quad().render_pass().has_transform_to_root_target()) {
            shared_quad_state->quad_to_target_transform = GetTransformFromProtobuf(quad_spec.render_pass_quad().render_pass().transform_to_root_target());
        }

        shared_quad_state->quad_layer_rect = GetRectFromProtobuf(quad_spec.rect());
        shared_quad_state->visible_quad_layer_rect = GetRectFromProtobuf(quad_spec.visible_rect());
    }
}

bool FuzzedCompositorFrameBuilder::TryReserveBitmapBytes(const gfx::Size& size)
{
    uint64_t bitmap_bytes;
    if (!ResourceSizes::MaybeSizeInBytes<uint64_t>(size, SinglePlaneFormat::kRGBA_8888, &bitmap_bytes) || bitmap_bytes > kMaxTextureMemory - reserved_bytes_) {
        return false;
    }
...
FuzzedBitmap* FuzzedCompositorFrameBuilder::AllocateFuzzedBitmap(const gfx::Size& size, SkColor4f color)
{
    SharedBitmapId shared_bitmap_id = SharedBitmap::GenerateId();
    base::MappedReadOnlyRegion shm = bitmap_allocation::AllocateSharedBitmap(size, SinglePlaneFormat::kRGBA_8888);
    ...
    data_.allocated_bitmaps.push_back({ shared_bitmap_id, size, std::move(shm.region) });
...
FuzzedBitmap::FuzzedBitmap(const SharedBitmapId& id, const gfx::Size& size, base::ReadOnlySharedMemoryRegion shared_region)
    : id(id)
    , size(size)
    , shared_region(std::move(shared_region))
{
}
...
FuzzedData BuildFuzzedCompositorFrame(const proto::CompositorRenderPass& render_pass_spec)
{
