...
protected:
    ...
    void InitProvider()
    {
        ...
        provider_ = std::make_unique<ClientResourceProvider>(base::SingleThreadTaskRunner::GetCurrentDefault(), base::ThreadPool::CreateSequencedTaskRunner({}),
            base::BindRepeating(&MockFlushCallback::FlushCallback, base::Unretained(&mock_flush_callback_)));
    }
    ...
    TransferableResource MakeTransferableResource(bool gpu, char mailbox_char, uint32_t sync_token_value)
    {
    ...
    ClientResourceProvider& provider() const
    {
        return *provider_;
    }
    ...
    void ExpectFlush()
    {
        EXPECT_CALL(mock_flush_callback_, FlushCallback());
    }
...
INSTANTIATE_TEST_SUITE_P(ClientResourceProviderTests, ClientResourceProviderTest, ::testing::Values(false, true));

class MockReleaseCallback {
public:
    MOCK_METHOD2(Released, void(const gpu::SyncToken& token, bool lost));
    MOCK_METHOD3(ReleasedWithId, void(ResourceId id, const gpu::SyncToken& token, bool lost));
...
TEST_P(ClientResourceProviderTest, TransferableResourceReleased)
{
    ...
    ResourceId id = provider().ImportResource(tran, base::BindOnce(&MockReleaseCallback::Released, base::Unretained(&release)));
...
{
    TransferableResource tran[] = { MakeTransferableResource(use_gpu(), 'a', 15), MakeTransferableResource(use_gpu(), 'b', 16) };
...
TEST_P(ClientResourceProviderTest, TransferableResourceLostOnShutdownIfExported)
{
    MockReleaseCallback release;
    TransferableResource tran = MakeTransferableResource(use_gpu(), 'a', 15);
    ResourceId id = provider().ImportResource(tran, base::BindOnce(&MockReleaseCallback::Released, base::Unretained(&release)));
...
TEST_P(ClientResourceProviderTest, TransferableResourceSendToParentManyUnsent)
{
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        data[i].id = provider().ImportResource(data[i].tran, base::BindOnce(&MockReleaseCallback::Released, base::Unretained(&release)));
    }
    std::sort(std::begin(data), std::end(data), [](const Data& a, const Data& b) { return a.id < b.id; });
...
TEST_P(ClientResourceProviderTest, ReturnedSyncTokensArePassedToClient)
{
    ...
    // in production.
    gpu::Mailbox mailbox = sii->CreateSharedImage({ SinglePlaneFormat::kRGBA_8888, gfx::Size(1, 1), gfx::ColorSpace(),
                                                      gpu::SHARED_IMAGE_USAGE_RASTER_WRITE | gpu::SHARED_IMAGE_USAGE_DISPLAY_READ, "TestLabel" },
                                  gpu::kNullSurfaceHandle)
                               ->mailbox();
    gpu::SyncToken sync_token = sii->GenUnverifiedSyncToken();

    constexpr gfx::Size size(64, 64);
    auto tran = TransferableResource::MakeGpu(mailbox, GL_TEXTURE_2D, sync_token, size, SinglePlaneFormat::kRGBA_8888, false /* is_overlay_candidate */);
    ResourceId resource = provider().ImportResource(tran, base::BindOnce(&MockReleaseCallback::Released, base::Unretained(&release)));
    ...
    EXPECT_EQ(0, memcmp(mailbox.name, list[0].mailbox().name, sizeof(mailbox.name)));

    // Make a new texture id from the mailbox.
    context_provider()->RasterInterface()->WaitSyncTokenCHROMIUM(list[0].sync_token().GetConstData());
    context_provider()->RasterInterface()->GenSyncTokenCHROMIUM(list[0].mutable_sync_token().GetData());
    EXPECT_TRUE(list[0].sync_token().HasData());

    // Receive the resource, then delete it, expect the SyncTokens to be
    // consistent.
    provider().ReceiveReturnsFromParent(TransferableResource::ReturnResources(list));

    gpu::SyncToken returned_sync_token;
    EXPECT_CALL(release, Released(_, false)).WillOnce(testing::SaveArg<0>(&returned_sync_token));
    provider().RemoveImportedResource(resource);
    EXPECT_GE(returned_sync_token.release_count(), list[0].sync_token().release_count());
}
...
TEST_P(ClientResourceProviderTest, ReleaseMultipleResources)
{
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        resources[i] = provider().ImportResource(tran, base::BindOnce(&MockReleaseCallback::ReleasedWithId, base::Unretained(&release), ResourceId(i)));
    }

    // Transfer some resources to the parent, but not in the sorted order.
    std::vector<TransferableResource> list;
    provider().PrepareSendToParent({ resources[2], resources[0], resources[4] }, &list, context_provider());
...
TEST_P(ClientResourceProviderTest, EvictionUnlocksResources)
{
    ...
    TransferableResource resource = MakeTransferableResource(use_gpu(), 'a', sync_token_value);
    ResourceId id = provider().ImportResource(resource, base::BindOnce(&MockReleaseCallback::Released, base::Unretained(&release)), ReleaseCallback(),
        base::BindOnce(&MockReleaseCallback::Evicted, base::Unretained(&release)));
...
TEST_P(ClientResourceProviderTest, LockedReturnedResourcesReleasedDuringEviction)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitWithFeatures({ features::kEvictionUnlocksResources }, {});
    // Mark visible so eviction path is not inadvertently triggered.
...
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitWithFeatures({ features::kEvictionUnlocksResources, features::kBatchMainThreadReleaseCallbacks }, {});
    ...
    const uint32_t sync_token_value = 1u;
    TransferableResource resource = MakeTransferableResource(use_gpu(), 'a', sync_token_value);
    ResourceId id = provider().ImportResource(resource, ReleaseCallback(), base::BindOnce(&MockReleaseCallback::Released, base::Unretained(&release)),
        base::BindOnce(&MockReleaseCallback::Evicted, base::Unretained(&release)));
...
TEST_P(ClientResourceProviderTest, BatchedCallbacksDoNotFireImmediately)
{
...

    {
        ...
        ClientResourceProvider::ScopedBatchResourcesRelease batch = provider().CreateScopedBatchResourcesRelease();
...
TEST_P(ClientResourceProviderTest, BatchedCallbacksFireUponProviderDestruction)
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitWithFeatures({ features::kBatchResourceRelease }, {});
    // Mark visible so eviction path is not inadvertently triggered.
