...
FrameEvictorClient::EvictIds& FrameEvictorClient::EvictIds::operator=(EvictIds&& other) = default;
...
std::vector<SurfaceId> FrameEvictor::CollectSurfaceIdsForEviction() const
{
    ...
    DCHECK(output_ids.empty() || !current.is_valid() || base::Contains(output_ids, current));
