...
public:
    ...
    public:
        ScopedBatchResourcesRelease(const ScopedBatchResourcesRelease& other) = delete;
        ScopedBatchResourcesRelease& operator=(const ScopedBatchResourcesRelease& other) = delete;
        ScopedBatchResourcesRelease(ScopedBatchResourcesRelease&& other);
        ScopedBatchResourcesRelease& operator=(ScopedBatchResourcesRelease&& other) = default;
        ~ScopedBatchResourcesRelease();

    protected:
        explicit ScopedBatchResourcesRelease(base::OnceClosure batch_release_callback);
    ...
    ClientResourceProvider(scoped_refptr<base::SequencedTaskRunner> main_task_runner, scoped_refptr<base::SequencedTaskRunner> impl_task_runner,
    ...
    static gpu::SyncToken GenerateSyncTokenHelper(gpu::raster::RasterInterface* ri);
    ...
    void PrepareSendToParent(
        const std::vector<ResourceId>& resource_ids, std::vector<TransferableResource>* transferable_resources, RasterContextProvider* context_provider);
    ...
    // wait on it.
    void ReceiveReturnsFromParent(std::vector<ReturnedResource> transferable_resources);
    ...
    ResourceId ImportResource(const TransferableResource& resource, ReleaseCallback impl_release_callback, ReleaseCallback main_thread_release_callback = {},
...
private:
    ...
    void PrepareSendToParentInternal(const std::vector<ResourceId>& export_ids, std::vector<TransferableResource>* list,
    ...
    void BatchMainReleaseCallbacks(std::vector<base::OnceClosure> release_callbacks);
