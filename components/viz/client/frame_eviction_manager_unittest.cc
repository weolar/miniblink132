...
public:
    ...
    TestFrameEvictionManagerClient(const TestFrameEvictionManagerClient&) = delete;
    TestFrameEvictionManagerClient& operator=(const TestFrameEvictionManagerClient&) = delete;
...
TEST_F(FrameEvictionManagerTest, ScopedPause)
{
    ...
    std::vector<TestFrameEvictionManagerClient> frames(kFrames);
    {
        ...
        EXPECT_EQ(kFrames, base::ranges::count_if(frames, &TestFrameEvictionManagerClient::has_frame));
    }

    // Frame eviction happens when |scoped_pause| goes out of scope.
    EXPECT_EQ(kMaxSavedFrames, base::ranges::count_if(frames, &TestFrameEvictionManagerClient::has_frame));
}
...
TEST_F(FrameEvictionManagerTest, PeriodicCulling)
{
    ...
    TestFrameEvictionManagerClient frame1 { &manager }, frame2 { &manager }, frame3 { &manager };
...
TEST_F(FrameEvictionManagerTest, MemoryPressure)
{
    ...
    manager->OnMemoryPressure(base::MemoryPressureListener::MEMORY_PRESSURE_LEVEL_CRITICAL);
    EXPECT_FALSE(frame1.has_frame());
    EXPECT_FALSE(frame2.has_frame());
}
