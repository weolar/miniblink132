...
void FrameEvictionManager::AddFrame(FrameEvictionManagerClient* frame, bool locked)
{
    RemoveFrame(frame);
    if (locked)
...
{
    if (base::Contains(unlocked_frames_, frame, [](const auto& p) { return p.first; })) {
...
void FrameEvictionManager::StartFrameCullingTimer()
{
    ...
    idle_frame_culling_timer_.Start(FROM_HERE, kPeriodicCullingDelay, base::BindOnce(&FrameEvictionManager::CullOldUnlockedFrames, base::Unretained(this)));
}

void FrameEvictionManager::RegisterUnlockedFrame(FrameEvictionManagerClient* frame)
{
...
FrameEvictionManager::FrameEvictionManager()
    : memory_pressure_listener_(
        new base::MemoryPressureListener(FROM_HERE, base::BindRepeating(&FrameEvictionManager::OnMemoryPressure, base::Unretained(this))))
{
...
//             this, "FrameEvictionManager", base::SingleThreadTaskRunner::GetCurrentDefault());
//     }
}
...
#if BUILDFLAG(IS_ANDROID)
void FrameEvictionManager::CullOldUnlockedFrames(base::MemoryReductionTaskContext task_type)
{
    const bool should_cull_all = task_type == base::MemoryReductionTaskContext::kProactive;
#else
void FrameEvictionManager::CullOldUnlockedFrames()
{
    const bool should_cull_all = false;
#endif
    DCHECK(std::is_sorted(unlocked_frames_.begin(), unlocked_frames_.end(), [](const auto& a, const auto& b) { return a.second >= b.second; }));
    ...
    while (!unlocked_frames_.empty() && (should_cull_all || now - unlocked_frames_.back().second >= kPeriodicCullingDelay)) {
        size_t old_size = unlocked_frames_.size();
        auto* frame = unlocked_frames_.back().first;
        frame->EvictCurrentFrame();
...
void FrameEvictionManager::OnMemoryPressure(base::MemoryPressureListener::MemoryPressureLevel memory_pressure_level)
{
...
void FrameEvictionManager::SetOverridesForTesting(scoped_refptr<base::SingleThreadTaskRunner> task_runner, const base::TickClock* clock)
{
...
bool FrameEvictionManager::OnMemoryDump(const base::trace_event::MemoryDumpArgs& args, base::trace_event::ProcessMemoryDump* pmd)
{
