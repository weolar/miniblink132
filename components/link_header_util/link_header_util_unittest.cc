...
void SplitLinkHeaderForTesting(const std::string& header, std::vector<std::string>* values)
{
...
bool ParseLinkHeaderValueForTesting(std::string value, std::string* url, std::unordered_map<std::string, absl::optional<std::string>>* params)
{
...
{
    std::vector<std::string> values;
    SplitLinkHeaderForTesting("\"f\\\"oo,bar\", 'b\\'ar,foo', <hel\\>,lo>", &values);
...
TEST_P(SimpleParseTest, Simple)
{
    ...
    EXPECT_EQ(test.valid, ParseLinkHeaderValueForTesting(test.link, &url, &params));
    if (test.valid) {
...
INSTANTIATE_TEST_SUITE_P(LinkHeaderTest, SimpleParseTest, testing::ValuesIn(simple_parse_tests));
