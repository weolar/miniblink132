...
TEST(TextFragmentsUtilsTest, SplitUrlTextFragmentDirective_NoDelimiter)
{
    ...
    EXPECT_FALSE(SplitUrlTextFragmentDirective(url, &webpage_url, &highlight_directive));
    EXPECT_TRUE(webpage_url.spec().empty());
    EXPECT_TRUE(highlight_directive.empty());
}

TEST(TextFragmentsUtilsTest, SplitUrlTextFragmentDirective_NormalUrlWithFragment)
{
    GURL webpage_url;
    std::string highlight_directive;

    std::string url = "https://www.example.com/hello/#:~:text=blahblah";
    EXPECT_TRUE(SplitUrlTextFragmentDirective(url, &webpage_url, &highlight_directive));
...
TEST(TextFragmentsUtilsTest, SplitUrlTextFragmentDirective_OtherAnchorBeforeFragment)
{
    GURL webpage_url;
    std::string highlight_directive;
...
{
    GURL url_with_fragment("https://www.example.com/#idFrag:~:text=text%201&text=text%202");
    base::Value result = ParseTextFragments(url_with_fragment);
    ASSERT_EQ(2u, result.GetList().size());
    EXPECT_EQ("text 1", result.GetList()[0].FindKey(kFragmentTextStartKey)->GetString());
    EXPECT_EQ("text 2", result.GetList()[1].FindKey(kFragmentTextStartKey)->GetString());
...
TEST(TextFragmentsUtilsTest, ExtractTextFragments)
{
    ...
    EXPECT_EQ(expected, ExtractTextFragments("#id:~:text=test1&text=test2&text=test3"));
    EXPECT_EQ(expected, ExtractTextFragments("#id:~:text=test1&text=test2&text=test3&"));

    // Test that empty tokens (&& or &text=&) are discarded
    EXPECT_EQ(expected, ExtractTextFragments("#id:~:text=test1&&text=test2&text=&text=test3"));
...
TEST(TextFragmentsUtilsTest, RemoveTextFragmentsAndSelectorFragmentsAndKeepOtherSelectors)
{
    ...
    url_with_fragment = GURL { R"(https://www.example.org#:~:lorem_directive_string&selector(type=CssSelector,value=img[src$="example.org"]))" };
...
TEST(TextFragmentsUtilsTest, RemoveTextFragmentsParameterIsSubstring)
{
    ...
    GURL result = RemoveFragmentSelectorDirectives(url);
    EXPECT_EQ("https://www.example.com/#:~:case_insensitive_text=foo", result.spec());
}
...
TEST(TextFragmentsUtilsTest, RemoveTextFragmentsWithNonTextFragment)
{
    ...
    GURL result = RemoveFragmentSelectorDirectives(url);
    EXPECT_EQ("https://example.com/?not_a_frag_directive:~:#no_text_fragments", result.spec());
}
...
TEST(TextFragmentsUtilsTest, AppendFragmentDirectivesOneFragment)
{
    ...
    EXPECT_EQ("https://www.chromium.org/#:~:text=only%20start", created_url.spec());
}
...
TEST(TextFragmentsUtilsTest, AppendFragmentDirectivesURLWithPoundAndValue)
{
    ...
    GURL created_url = AppendFragmentDirectives(base_url, { test_fragment });
    EXPECT_EQ("https://www.chromium.org/#SomeAnchor:~:text=only%20start", created_url.spec());
}

TEST(TextFragmentsUtilsTest, AppendFragmentDirectivesURLWithPoundAndExistingFragment)
{
    GURL base_url("https://www.chromium.org/#:~:text=some%20value");
    TextFragment test_fragment("only start");

    GURL created_url = AppendFragmentDirectives(base_url, { test_fragment });
    EXPECT_EQ("https://www.chromium.org/"
              "#:~:text=only%20start",
        created_url.spec());
}

TEST(TextFragmentsUtilsTest, AppendFragmentDirectivesURLWithPoundAndExistingFragmentAndAnchor)
{
...
TEST(TextFragmentsUtilsTest, AppendFragmentDirectivesTwoFragments)
{
    ...
    GURL created_url = AppendFragmentDirectives(base_url, { first_test_fragment, second_test_fragment });
    EXPECT_EQ("https://www.chromium.org/"
              "#:~:text=only%20start&text=only%2C%2D%20start%20%232",
        created_url.spec());
}

TEST(TextFragmentsUtilsTest, AppendSelectorsURLWithPoundAndExistingFragmentAndAnchor)
{
    GURL base_url("https://www.chromium.org/#SomeAnchor:~:text=some%20value");
...
TEST(TextFragmentsUtilsTest, AppendSelectorsTwoFragments)
{
    ...
    GURL created_url = AppendSelectors(base_url, { first_test_selector, second_test_selector });
    EXPECT_EQ("https://www.chromium.org/"
              "#:~:text=only%20start&text=only%2C%2D%20start%20%232",
        created_url.spec());
}
