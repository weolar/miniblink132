...
TextFragment::TextFragment(const std::string& text_start, const std::string& text_end, const std::string& prefix, const std::string& suffix)
...
TextFragment::TextFragment(const TextFragment& other)
    : TextFragment(other.text_start(), other.text_end(), other.prefix(), other.suffix())
{
}
...
// static
absl::optional<TextFragment> TextFragment::FromEscapedString(std::string escaped_string)
{
    ...
    std::vector<std::string> pieces = base::SplitString(escaped_string, ",", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    if (prefix.find_first_of("&-,") != std::string::npos || text_start.find_first_of("&-,") != std::string::npos
        || text_end.find_first_of("&-,") != std::string::npos || suffix.find_first_of("&-,") != std::string::npos) {
        // Malformed if any of the pieces contain characters that are supposed to be
        // URL-encoded.
        return absl::nullopt;
    }

    return TextFragment(Unescape(text_start), Unescape(text_end), Unescape(prefix), Unescape(suffix));
}
...
absl::optional<TextFragment> TextFragment::FromValue(const base::Value* value)
{
    ...
    return TextFragment(*text_start, ValueOrDefault(text_end), ValueOrDefault(prefix), ValueOrDefault(suffix));
}
