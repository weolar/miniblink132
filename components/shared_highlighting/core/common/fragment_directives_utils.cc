...
base::Value ParseTextFragments(const GURL& url)
{
    ...
    for (const std::string& fragment : fragments) {
        absl::optional<TextFragment> opt_frag = TextFragment::FromEscapedString(fragment);
        if (opt_frag.has_value()) {
            base::Value::List* parsed_list = parsed.GetIfList();
            parsed_list->Append(opt_frag->ToValue());
        }
    }
...
bool SplitUrlTextFragmentDirective(const std::string& full_url, GURL* webpage_url, std::string* highlight_directive)
{
    ...
    *highlight_directive = full_url.substr(pos + kFragmentsUrlDelimiterLength + kTextDirectiveParameterNameLength);
    return true;
}
...
{
    const std::vector<std::string_view> directive_parameter_names { kTextDirectiveParameterName, kSelectorDirectiveParameterName };
    ...
    for (const std::string& directive : base::SplitString(fragment_directive, kSelectorJoinDelimeter, base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL)) {
        if (base::ranges::none_of(directive_parameter_names,
                [&directive](std::string_view directive_parameter_name) { return base::StartsWith(directive, directive_parameter_name); })) {
...
GURL AppendFragmentDirectives(const GURL& base_url, std::vector<TextFragment> fragments)
{
...
GURL AppendFragmentDirectives(const GURL& base_url, std::vector<std::string> directives)
{
    std::string fragments_string = base::JoinString(directives, kSelectorJoinDelimeter);
