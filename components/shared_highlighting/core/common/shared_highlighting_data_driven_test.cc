...
const base::FilePath::CharType kFeatureName[] = FILE_PATH_LITERAL("shared_highlighting");
const base::FilePath::CharType kTestName[] = FILE_PATH_LITERAL("generate_navigate");
...
const base::FilePath GetTestDataDir()
{
    static base::NoDestructor<base::FilePath> dir([]() {
        ...
        return dir.AppendASCII("components").AppendASCII("test").AppendASCII("data");
    }());
    return *dir;
}
...
const std::vector<base::FilePath> SharedHighlightingDataDrivenTest::GetTestFiles()
{
    base::FilePath dir = GetInputDir().AppendASCII("test-params");
    base::FileEnumerator input_files(dir, false, base::FileEnumerator::FILES, kFileNamePattern);
    std::vector<base::FilePath> files;
    for (base::FilePath input_file = input_files.Next(); !input_file.empty(); input_file = input_files.Next()) {
...
void SharedHighlightingDataDrivenTest::GenerateResults(const std::string& input, std::string* output)
{
    ...
    auto results = GenerateAndNavigate(html_content, start_parent_id, start_offset_in_parent, start_text_offset, end_parent_id, end_offset_in_parent,
        end_text_offset, selected_text, highlight_text);

    *output = "GENERATION: " + (results.generation_success ? kTestSuccess : kTestFailure) + "\n"
        + "HIGHLIGHTING: " + (results.highlighting_success ? kTestSuccess : kTestFailure) + "\n";
}
