...
// Java counterpart will be auto-generated for this enum.
// GENERATED_JAVA_ENUM_PACKAGE: org.chromium.chrome.browser.share.link_to_text
enum class LinkGenerationReadyStatus { kRequestedBeforeReady = 0, kRequestedAfterReady = 1 };
...
enum class LinkGenerationCopiedLinkType { kCopiedFromNewGeneration = 0, kCopiedFromExistingHighlight = 1, kMaxValue = kCopiedFromExistingHighlight };
...
void LogRequestedFailureMetrics(ukm::SourceId source_id, LinkGenerationError error);
...
// be recorded.
void LogLinkOpenedUkmEvent(ukm::SourceId source_id, const GURL& referrer, bool success);
...
void LogLinkOpenedUkmEvent(ukm::UkmRecorder* recorder, ukm::SourceId source_id, const GURL& referrer, bool success);
...
void LogLinkGeneratedSuccessUkmEvent(ukm::UkmRecorder* recorder, ukm::SourceId source_id);
...
// will be recorded.
void LogLinkGeneratedErrorUkmEvent(ukm::SourceId source_id, LinkGenerationError reason);
...
void LogLinkGeneratedErrorUkmEvent(ukm::UkmRecorder* recorder, ukm::SourceId source_id, LinkGenerationError reason);
...
void LogLinkGeneratedRequestedErrorUkmEvent(ukm::SourceId source_id, LinkGenerationError reason);

// Records whether link to text was requested before or after link generation
// was complete with corresponding success status.
void LogLinkRequestedBeforeStatus(LinkGenerationStatus status, LinkGenerationReadyStatus ready_status);
