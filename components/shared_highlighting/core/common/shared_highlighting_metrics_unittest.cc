...
protected:
    ...
    void ValidateLinkOpenedUkm(const ukm::TestAutoSetUkmRecorder& recorder, ukm::SourceId source_id, bool success, TextFragmentLinkOpenSource source)
    {
        auto entries = recorder.GetEntriesByName(ukm::builders::SharedHighlights_LinkOpened::kEntryName);
        ASSERT_EQ(1u, entries.size());
        const ukm::mojom::UkmEntry* entry = entries[0];
        EXPECT_EQ(source_id, entry->source_id);
        recorder.ExpectEntryMetric(entry, kSuccessUkmMetric, success);
        recorder.ExpectEntryMetric(entry, kSourceUkmMetric, static_cast<int64_t>(source));
    }
...
TEST_F(SharedHighlightingMetricsTest, LogTextFragmentAmbiguousMatch)
{
    ...
    histogram_tester_.ExpectBucketCount("TextFragmentAnchor.AmbiguousMatch", 1, 1);

    LogTextFragmentAmbiguousMatch(false);
    histogram_tester_.ExpectBucketCount("TextFragmentAnchor.AmbiguousMatch", 0, 1);
...
TEST_F(SharedHighlightingMetricsTest, LogTextFragmentLinkOpenSource)
{
    ...
    histogram_tester_.ExpectBucketCount("TextFragmentAnchor.LinkOpenSource", TextFragmentLinkOpenSource::kSearchEngine, 1);

    GURL non_search_engine_url("https://example.com");
    LogTextFragmentLinkOpenSource(non_search_engine_url);
    histogram_tester_.ExpectBucketCount("TextFragmentAnchor.LinkOpenSource", TextFragmentLinkOpenSource::kUnknown, 1);
    histogram_tester_.ExpectTotalCount("TextFragmentAnchor.LinkOpenSource", 2);

    GURL empty_gurl("");
    LogTextFragmentLinkOpenSource(empty_gurl);
    histogram_tester_.ExpectBucketCount("TextFragmentAnchor.LinkOpenSource", TextFragmentLinkOpenSource::kUnknown, 2);
    ...
    histogram_tester_.ExpectBucketCount("TextFragmentAnchor.LinkOpenSource", TextFragmentLinkOpenSource::kUnknown, 3);
...
TEST_F(SharedHighlightingMetricsTest, LogTextFragmentSelectorCount)
{
    ...
    histogram_tester_.ExpectBucketCount("TextFragmentAnchor.SelectorCount", 20, 1);
...
TEST_F(SharedHighlightingMetricsTest, LogLinkGenerationStatus)
{
    ...
    histogram_tester_.ExpectUniqueSample("SharedHighlights.LinkGenerated", true, 1);

    LogLinkGenerationStatus(LinkGenerationStatus::kFailure);
    histogram_tester_.ExpectBucketCount("SharedHighlights.LinkGenerated", false, 1);
...
TEST_F(SharedHighlightingMetricsTest, LogLinkGenerationErrorReason)
{
    ...
    histogram_tester_.ExpectBucketCount("SharedHighlights.LinkGenerated.Error", LinkGenerationError::kIncorrectSelector, 1);

    LogLinkGenerationErrorReason(LinkGenerationError::kEmptySelection);
    histogram_tester_.ExpectBucketCount("SharedHighlights.LinkGenerated.Error", LinkGenerationError::kEmptySelection, 1);
...
TEST_F(SharedHighlightingMetricsTest, LinkOpenedUkmSuccessSearchEngine)
{
    ...
    ValidateLinkOpenedUkm(ukm_recorder, source_id, success, TextFragmentLinkOpenSource::kSearchEngine);
}
...
TEST_F(SharedHighlightingMetricsTest, LinkOpenedUkmSuccessUnknownSource)
{
    ...
    ValidateLinkOpenedUkm(ukm_recorder, source_id, success, TextFragmentLinkOpenSource::kUnknown);
}
...
TEST_F(SharedHighlightingMetricsTest, LinkOpenedUkmInvalidSourceId)
{
    ...
    auto entries = ukm_recorder.GetEntriesByName(ukm::builders::SharedHighlights_LinkOpened::kEntryName);
...
TEST_F(SharedHighlightingMetricsTest, LinkOpenedUkmCustomRecorder)
{
    ...
    auto static_entries = static_ukm_recorder.GetEntriesByName(ukm::builders::SharedHighlights_LinkOpened::kEntryName);
    EXPECT_EQ(0U, static_entries.size());

    auto custom_entries = custom_ukm_recorder.GetEntriesByName(ukm::builders::SharedHighlights_LinkOpened::kEntryName);
...
TEST_F(SharedHighlightingMetricsTest, LinkGeneratedUkmCustomRecorder)
{
    ...
    LogLinkGeneratedErrorUkmEvent(&custom_ukm_recorder, source_id, LinkGenerationError::kEmptySelection);

    auto static_entries = static_ukm_recorder.GetEntriesByName(ukm::builders::SharedHighlights_LinkGenerated::kEntryName);
    EXPECT_EQ(0U, static_entries.size());

    auto custom_entries = custom_ukm_recorder.GetEntriesByName(ukm::builders::SharedHighlights_LinkGenerated::kEntryName);
...
TEST_F(SharedHighlightingMetricsTest, LinkGeneratedSuccessLatency)
{
    ...
    histogram_tester_.ExpectTimeBucketCount("SharedHighlights.LinkGenerated.TimeToGenerate", test_delta, 1);
}
...
TEST_F(SharedHighlightingMetricsTest, LinkGeneratedErrorLatency)
{
    ...
    histogram_tester_.ExpectTimeBucketCount("SharedHighlights.LinkGenerated.Error.TimeToGenerate", test_delta, 1);
}
...
TEST_F(SharedHighlightingMetricsTest, LogRequestedFailureMetrics)
{
    ...
    histogram_tester_.ExpectBucketCount("SharedHighlights.LinkGenerated.Requested", false, 1);
    histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.Requested", 1);
    histogram_tester_.ExpectBucketCount("SharedHighlights.LinkGenerated.Error.Requested", LinkGenerationError::kEmptySelection, 1);
    histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.Error.Requested", 1);

    // Check UKM
    auto entries = ukm_recorder.GetEntriesByName(ukm::builders::SharedHighlights_LinkGenerated_Requested::kEntryName);
    ASSERT_EQ(1u, entries.size());
    const ukm::mojom::UkmEntry* entry = entries[0];
    EXPECT_EQ(source_id, entry->source_id);
    ukm_recorder.ExpectEntryMetric(entry, kSuccessUkmMetric, false);
    ukm_recorder.ExpectEntryMetric(entry, kErrorUkmMetric, static_cast<int64_t>(LinkGenerationError::kEmptySelection));
}
...
TEST_F(SharedHighlightingMetricsTest, LogRequestedSuccessMetrics)
{
    ...
    histogram_tester_.ExpectBucketCount("SharedHighlights.LinkGenerated.Requested", true, 1);
    histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.Requested", 1);
...
{
    LogLinkRequestedBeforeStatus(LinkGenerationStatus::kSuccess, LinkGenerationReadyStatus::kRequestedBeforeReady);

    histogram_tester_.ExpectBucketCount("SharedHighlights.LinkGenerated.RequestedBeforeReady", true, 1);
    histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.RequestedBeforeReady", 1);
    histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.RequestedAfterReady", 0);

    LogLinkRequestedBeforeStatus(LinkGenerationStatus::kFailure, LinkGenerationReadyStatus::kRequestedBeforeReady);
    histogram_tester_.ExpectBucketCount("SharedHighlights.LinkGenerated.RequestedBeforeReady", false, 1);
    histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.RequestedBeforeReady", 2);
    histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.RequestedAfterReady", 0);

    LogLinkRequestedBeforeStatus(LinkGenerationStatus::kSuccess, LinkGenerationReadyStatus::kRequestedAfterReady);
    histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.RequestedBeforeReady", 2);
    histogram_tester_.ExpectBucketCount("SharedHighlights.LinkGenerated.RequestedAfterReady", true, 1);
    histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.RequestedAfterReady", 1);

    LogLinkRequestedBeforeStatus(LinkGenerationStatus::kFailure, LinkGenerationReadyStatus::kRequestedAfterReady);
    histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.RequestedBeforeReady", 2);
    histogram_tester_.ExpectBucketCount("SharedHighlights.LinkGenerated.RequestedAfterReady", false, 1);
    histogram_tester_.ExpectTotalCount("SharedHighlights.LinkGenerated.RequestedAfterReady", 2);
}
...
TEST_F(SharedHighlightingMetricsTest, LogLinkToTextReshareStatus)
{
    ...
    histogram_tester_.ExpectBucketCount("SharedHighlights.ObtainReshareLink.Status", LinkToTextReshareStatus::kSuccess, 1);
    histogram_tester_.ExpectTotalCount("SharedHighlights.ObtainReshareLink.Status", 1);

    LogLinkToTextReshareStatus(LinkToTextReshareStatus::kTimeout);
    histogram_tester_.ExpectBucketCount("SharedHighlights.ObtainReshareLink.Status", LinkToTextReshareStatus::kTimeout, 1);
    histogram_tester_.ExpectTotalCount("SharedHighlights.ObtainReshareLink.Status", 2);
}
