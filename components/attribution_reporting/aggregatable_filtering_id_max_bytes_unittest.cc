...
TEST(AggregatableFilteringIdsMaxBytes, Parse)
{
    const struct {
        ...
        const char* json;
        ::testing::Matcher<base::expected<AggregatableFilteringIdsMaxBytes, TriggerRegistrationError>> matches;
    } kTestCases[] {
        {
            "empty",
            R"json({})json",
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_THAT(AggregatableFilteringIdsMaxBytes::Parse(input), test_case.matches);
    }
}
...
TEST(AggregatableFilteringIdsMaxBytes, CanEncompass)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_EQ(test_case.max_bytes.CanEncompass(test_case.filtering_id), test_case.expected);
    }
}
