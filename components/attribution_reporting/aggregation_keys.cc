...
{
    return keys.size() <= kMaxAggregationKeysPerSource && base::ranges::all_of(keys, [](const auto& key) { return AggregationKeyIdHasValidLength(key.first); });
}
...
void RecordAggregatableKeysPerSource(base::HistogramBase::Sample count)
{
    ...
    static_assert(kMaxAggregationKeysPerSource < kExclusiveMaxHistogramValue, "Bump the version for histogram Conversions.AggregatableKeysPerSource");
...
// static
base::expected<AggregationKeys, SourceRegistrationError> AggregationKeys::FromJSON(const base::Value* value)
{
    if (!value)
    ...
    if (!dict)
        return base::unexpected(SourceRegistrationError::kAggregationKeysDictInvalid);
    ...
    for (auto [key_id, maybe_string_value] : *dict) {
        if (!AggregationKeyIdHasValidLength(key_id)) {
            return base::unexpected(SourceRegistrationError::kAggregationKeysKeyTooLong);
        }

        ASSIGN_OR_RETURN(
            absl::uint128 key, ParseAggregationKeyPiece(maybe_string_value), [](ParseError) { return SourceRegistrationError::kAggregationKeysValueInvalid; });
...
AggregationKeys::AggregationKeys(Keys keys)
    : keys_(std::move(keys))
{
    CHECK(IsValid(keys_));
}
