...
TEST(PrivacyMathTest, GetRandomizedResponseRate)
{
    const struct {
        ...
        { .num_states = std::numeric_limits<uint32_t>::max(), .epsilon = 14, .expected = 0.99972007548289821 } };

    for (const auto& test_case : kTestCases) {
        EXPECT_EQ(test_case.expected, GetRandomizedResponseRate(test_case.num_states, test_case.epsilon));
    }
}
...
TEST(PrivacyMathTest, ComputeChannelCapacity)
{
    ...
    for (const auto& test_case : kTestCases) {
        double rate = GetRandomizedResponseRate(test_case.num_states, test_case.epsilon);

        EXPECT_EQ(test_case.expected, internal::ComputeChannelCapacity(test_case.num_states, rate));
    }
}
...
TEST(PrivacyMathTest, ComputeChannelCapacityWithScopes)
{
    ...
    for (const auto& test_case : kTestCases) {
        EXPECT_EQ(
            test_case.expected, internal::ComputeChannelCapacityScopes(test_case.num_states, test_case.max_event_states, test_case.attribution_scopes_limit));
    }
}
...
TEST(PrivacyMathTest, GetFakeReportsForSequenceIndex)
{
    ...
    for (int i = 0; const auto& test_case : kTestCases) {
        ...
        EXPECT_THAT(internal::GetFakeReportsForSequenceIndex(
                        TriggerSpecs(test_case.source_type, *EventReportWindows::FromDefaults(base::Days(30), test_case.source_type),
...
void RunRandomFakeReportsTest(const TriggerSpecs& specs, const int num_samples, const double tolerance)
{
...
TEST(PrivacyMathTest, GetRandomFakeReports_Event_MatchesExpectedDistribution)
{
    ...
    // at most 1e-9.
    RunRandomFakeReportsTest(TriggerSpecs(SourceType::kEvent, *EventReportWindows::FromDefaults(base::Days(30), SourceType::kEvent), MaxEventLevelReports(1)),
        /*num_samples=*/100'000,
        /*tolerance=*/0.03);
}

TEST(PrivacyMathTest, GetRandomFakeReports_Navigation_MatchesExpectedDistribution)
{
    ...
    RunRandomFakeReportsTest(
        TriggerSpecs(SourceType::kNavigation, *EventReportWindows::FromDefaults(base::Days(30), SourceType::kNavigation), MaxEventLevelReports(3)),
        /*num_samples=*/150'000,
        /*tolerance=*/0.9);
}
...
TEST(PrivacyMathTest, GetRandomFakeReports_Custom_MatchesExpectedDistribution)
{
    ...
    // For the distribution check, the probability of failure with `tolerance` is
    // at most 1e-9.
    const std::vector<TriggerSpec> kSpecList = { TriggerSpec(*EventReportWindows::Create(
...
const struct {
    ...
    { MaxEventLevelReports(20), { 5, 5, 5, 1 }, base::unexpected(RandomizedResponseError::kExceedsTriggerStateCardinalityLimit) },
};
...
TEST(PrivacyMathTest, NumStatesForTriggerSpecs_UniqueSampling)
{
    for (const auto& test_case : kNumStateTestCases) {
        ...
        if (!test_case.expected_num_states.has_value() || *test_case.expected_num_states > 10000) {
            continue;
        }
        ...
        for (uint32_t i = 0; i < *test_case.expected_num_states; i++) {
            if (auto output = internal::GetFakeReportsForSequenceIndex(specs, i, map); output.has_value()) {
                seen_outputs.insert(*std::move(output));
            }
        }
        EXPECT_EQ(static_cast<size_t>(*test_case.expected_num_states), seen_outputs.size());
    }
}
...
TEST(PrivacyMathTest, NonDefaultTriggerDataForSingleSharedSpec)
{
    ...
    const auto kSpecs = *TriggerSpecs::Create({ { /*trigger_data=*/123, /*index=*/0 } }, { TriggerSpec() }, MaxEventLevelReports(1));
    ...
    do {
        ...
        ASSERT_OK_AND_ASSIGN(RandomizedResponseData response_data,
            internal::DoRandomizedResponseWithCache(kSpecs,
                /*epsilon=*/0, map, SourceType::kNavigation,
                /*scopes_data=*/std::nullopt, PrivacyMathConfig()));
...
TEST(PrivacyMathTest, RandomizedResponse_ExceedsChannelCapacity)
{
    ...
    auto channel_capacity_response = DoRandomizedResponse(TriggerSpecs(SourceType::kNavigation, EventReportWindows(),
                                                              /*max_reports=*/MaxEventLevelReports(1)),
        /*epsilon=*/14, SourceType::kNavigation,
        /*scopes_data=*/std::nullopt, kConfig);

    EXPECT_THAT(channel_capacity_response, ErrorIs(RandomizedResponseError::kExceedsChannelCapacityLimit));
}
...
TEST(PrivacyMathTest, RandomizedResponse_ExceedsScopesChannelCapacity)
{
    ...
    EXPECT_THAT(channel_capacity_response, ErrorIs(RandomizedResponseError::kExceedsScopesChannelCapacityLimit));

    // Event
    channel_capacity_response = DoRandomizedResponse(TriggerSpecs(SourceType::kEvent, EventReportWindows(),
                                                         /*max_reports=*/MaxEventLevelReports(1)),
...
TEST(PrivacyMathTest, UnaryChannel)
{
    const struct {
        ...
        {
            .desc = "empty-specs",
            .trigger_specs = *TriggerSpecs::Create(TriggerSpecs::TriggerDataIndices(), std::vector<TriggerSpec>(), MaxEventLevelReports(20)),
        },
        {
            .desc = "zero-max-reports",
            .trigger_specs = TriggerSpecs(SourceType::kNavigation, EventReportWindows(), MaxEventLevelReports(0)),
        },
    };
    ...
    for (const auto& test_case : kTestCases) {
        ...
        ASSERT_OK_AND_ASSIGN(const uint32_t num_states, GetNumStates(test_case.trigger_specs));
...
{
    const TriggerSpecs kSpecs(SourceType::kNavigation, *EventReportWindows::FromDefaults(base::Days(30), SourceType::kNavigation), MaxEventLevelReports(1));
