...
{
    return !starts.empty() && base::MakeStrictNum(starts.size()) <= static_cast<int>(MaxEventLevelReports::Max()) && *starts.begin() > 0;
}

} // namespace

base::expected<SummaryOperator, SourceRegistrationError> ParseSummaryOperator(const base::Value::Dict& dict)
{
    ...
    if (!str) {
        return base::unexpected(SourceRegistrationError::kSummaryOperatorValueInvalid);
...
// static
base::expected<SummaryBuckets, SourceRegistrationError> SummaryBuckets::Parse(const base::Value::Dict& dict, MaxEventLevelReports max_event_level_reports)
{
    ...
    if (!list || list->empty() || base::MakeStrictNum(list->size()) > static_cast<int>(max_event_level_reports)) {
        return base::unexpected(SourceRegistrationError::kSummaryBucketsListInvalid);
    }
    ...
    for (const base::Value& item : *list) {
        ASSIGN_OR_RETURN(uint32_t start, ParseUint32(item), [](ParseError) { return SourceRegistrationError::kSummaryBucketsValueInvalid; });

        if (start <= prev) {
            return base::unexpected(SourceRegistrationError::kSummaryBucketsValueInvalid);
        }
    ...
    return SummaryBuckets(base::flat_set<uint32_t>(base::sorted_unique, std::move(starts)));
}

SummaryBuckets::SummaryBuckets(const MaxEventLevelReports max_event_level_reports)
{
    std::vector<uint32_t> starts;
