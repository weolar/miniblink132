...
base::expected<void, SourceRegistrationError> ParseTriggerData(const base::Value& value, TriggerSpecs::TriggerDataIndices& trigger_data_indices,
    const uint8_t spec_index, const bool allow_empty, SourceRegistrationError list_error, SourceRegistrationError duplicate_error,
    SourceRegistrationError excessive_error)
{
    ...
    for (const base::Value& item : *list) {
        ASSIGN_OR_RETURN(uint32_t trigger_data, ParseUint32(item), [&](ParseError) { return list_error; });

        auto [_, inserted] = trigger_data_indices.try_emplace(trigger_data, spec_index);
        if (!inserted) {
...
bool AreSpecsValid(const TriggerSpecs::TriggerDataIndices& trigger_data_indices, const std::vector<TriggerSpec>& specs)
{
    return trigger_data_indices.size() <= kMaxTriggerDataPerSource
        && base::ranges::all_of(trigger_data_indices, [&specs](const auto& pair) { return pair.second < specs.size(); });
}

base::expected<void, SourceRegistrationError> ValidateSpecsForTriggerDataMatching(
    const TriggerSpecs::TriggerDataIndices& trigger_data_indices, TriggerDataMatching trigger_data_matching)
{
    switch (trigger_data_matching) {
    case TriggerDataMatching::kExact:
        return base::ok();
    case TriggerDataMatching::kModulus:
        for (uint32_t i = 0; const auto& [trigger_data, _] : trigger_data_indices) {
            if (trigger_data != i) {
                return base::unexpected(SourceRegistrationError::kInvalidTriggerDataForMatchingMode);
            }
            ++i;
        }
        return base::ok();
...
base::expected<TriggerDataMatching, SourceRegistrationError> ParseTriggerDataMatching(const base::Value::Dict& dict)
{
    ...
    if (!str) {
        return base::unexpected(SourceRegistrationError::kTriggerDataMatchingValueInvalid);
...
void Serialize(base::Value::Dict& dict, TriggerDataMatching trigger_data_matching)
{
    switch (trigger_data_matching) {
    case TriggerDataMatching::kExact:
...
TriggerSpecs::const_iterator TriggerSpecs::find(uint64_t trigger_data, TriggerDataMatching trigger_data_matching) const
{
    switch (trigger_data_matching) {
    case TriggerDataMatching::kExact:
    ...
    case TriggerDataMatching::kModulus:
        ...
        // random-access.
        return Iterator(*this, std::next(trigger_data_indices_.begin(), trigger_data % trigger_data_indices_.size()));
    }
}
...
// static
base::expected<TriggerSpecs, SourceRegistrationError> TriggerSpecs::ParseFullFlexForTesting(const base::Value::Dict& registration, SourceType source_type,
    base::TimeDelta expiry, EventReportWindows default_report_windows, TriggerDataMatching trigger_data_matching)
{
    const base::Value* trigger_specs = registration.Find(kTriggerSpecs);
    if (!trigger_specs) {
        return ParseTopLevelTriggerData(registration, source_type, std::move(default_report_windows), trigger_data_matching);
    }

    if (registration.contains(kTriggerData)) {
        return base::unexpected(SourceRegistrationError::kTopLevelTriggerDataAndTriggerSpecs);
    }

    ASSIGN_OR_RETURN(auto max_event_level_reports, MaxEventLevelReports::Parse(registration, source_type));
    ...
    if (list->size() > kMaxTriggerDataPerSource) {
        return base::unexpected(SourceRegistrationError::kTriggerSpecExcessiveTriggerData);
    }
    ...
    for (const base::Value& item : *list) {
        ...
        if (!trigger_data) {
            return base::unexpected(SourceRegistrationError::kTriggerSpecTriggerDataMissing);
        }

        RETURN_IF_ERROR(ParseTriggerData(*trigger_data, trigger_data_indices,
            /*spec_index=*/base::checked_cast<uint8_t>(specs.size()),
            /*allow_empty=*/false, SourceRegistrationError::kTriggerSpecTriggerDataListInvalid, SourceRegistrationError::kTriggerSpecDuplicateTriggerData,
            SourceRegistrationError::kTriggerSpecExcessiveTriggerData));

        ASSIGN_OR_RETURN(auto event_report_windows, EventReportWindows::ParseWindows(*dict, expiry, default_report_windows));

        specs.emplace_back(std::move(event_report_windows));
    }

    RETURN_IF_ERROR(ValidateSpecsForTriggerDataMatching(trigger_data_indices, trigger_data_matching));

    return TriggerSpecs(std::move(trigger_data_indices), std::move(specs), max_event_level_reports);
}

// static
base::expected<TriggerSpecs, SourceRegistrationError> TriggerSpecs::ParseTopLevelTriggerData(
    const base::Value::Dict& registration, SourceType source_type, EventReportWindows default_report_windows, TriggerDataMatching trigger_data_matching)
{
    ASSIGN_OR_RETURN(auto max_event_level_reports, MaxEventLevelReports::Parse(registration, source_type));
    ...
    if (!trigger_data) {
        return TriggerSpecs(source_type, std::move(default_report_windows), max_event_level_reports);
    }

    TriggerDataIndices trigger_data_indices;
    RETURN_IF_ERROR(ParseTriggerData(*trigger_data, trigger_data_indices,
        /*spec_index=*/0,
        /*allow_empty=*/true, SourceRegistrationError::kTriggerDataListInvalid, SourceRegistrationError::kDuplicateTriggerData,
...
TriggerSpecs::TriggerSpecs(SourceType source_type, EventReportWindows event_report_windows, MaxEventLevelReports max_event_level_reports)
...
std::optional<TriggerSpecs> TriggerSpecs::Create(
    TriggerDataIndices trigger_data_indices, std::vector<TriggerSpec> specs, MaxEventLevelReports max_event_level_reports)
{
...
TriggerSpecs::TriggerSpecs(TriggerDataIndices trigger_data_indices, std::vector<TriggerSpec> specs, MaxEventLevelReports max_event_level_reports)
...
base::Value::List TriggerSpecs::ToJson() const
{
    ...
    for (const auto& [trigger_data, index] : trigger_data_indices_) {
        spec_list[index].GetDict().FindList(kTriggerData)->Append(Uint32ToJson(trigger_data));
    }
...
TriggerSpecs::Iterator::Iterator(const TriggerSpecs& specs, TriggerDataIndices::const_iterator it)
    : specs_(specs)
    , it_(it)
{
}
