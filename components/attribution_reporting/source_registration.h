...
struct /*COMPONENT_EXPORT(ATTRIBUTION_REPORTING)*/ SourceRegistration {
    ...
    static base::expected<SourceRegistration, mojom::SourceRegistrationError> Parse(base::Value, mojom::SourceType);

    // Logs metric on parsing failures.
    static base::expected<SourceRegistration, mojom::SourceRegistrationError> Parse(std::string_view json, mojom::SourceType);
    ...
    friend bool operator==(const SourceRegistration&, const SourceRegistration&) = default;
    ...
    mojom::TriggerDataMatching trigger_data_matching = mojom::TriggerDataMatching::kModulus;
