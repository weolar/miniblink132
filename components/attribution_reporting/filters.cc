...
base::expected<void, FilterValuesError> ValidateForSource(const FilterValues& filter_values)
{
...
void RecordFiltersPerFilterData(base::HistogramBase::Sample count)
{
    ...
    static_assert(kMaxFiltersPerSource < kExclusiveMaxHistogramValue, "Bump the version for histogram Conversions.FiltersPerFilterData");
...
void RecordValuesPerFilter(base::HistogramBase::Sample count)
{
    ...
    static_assert(kMaxValuesPerFilter < kExclusiveMaxHistogramValue, "Bump the version for histogram Conversions.ValuesPerFilter");
...
base::expected<FilterValues, FilterValuesError> ParseFilterValuesFromJSON(
    base::Value::Dict dict, const size_t max_filters, const size_t max_string_size, const size_t max_set_size)
{
    ...
    for (auto [filter, value] : dict) {
        ...
        ASSIGN_OR_RETURN(base::flat_set<std::string> values, ExtractStringSet(std::move(*list), max_string_size, max_set_size), [](StringSetError error) {
            switch (error) {
...
// static
base::expected<FilterData, FilterValuesError> FilterData::CreateForTesting(FilterValues filter_values)
{
...
// static
base::expected<FilterData, SourceRegistrationError> FilterData::FromJSON(base::Value* input_value)
{
    if (!input_value) {
        return FilterData();
    }
...
bool FilterData::Matches(
    mojom::SourceType source_type, const base::Time& source_time, const base::Time& trigger_time, const FiltersDisjunction& filters, bool negated) const
{
    if (filters.empty()) {
        return true;
    }
    ...
    const base::TimeDelta duration_since_source_registration = (source_time < trigger_time) ? trigger_time - source_time : base::Microseconds(0);
    ...
    return base::ranges::any_of(filters, [&](const FilterConfig& config) {
        if (config.lookback_window()) {
            if (duration_since_source_registration > config.lookback_window().value()) {
                if (!negated) {
                    return false;
                }
            } else if (negated) {
        ...
        return base::ranges::all_of(config.filter_values(), [&](const auto& trigger_filter) {
            if (trigger_filter.first == kSourceTypeFilterKey) {
                bool has_intersection
                    = base::ranges::any_of(trigger_filter.second, [&](const std::string& value) { return value == SourceTypeName(source_type); });
            ...
            bool has_intersection
                = base::ranges::any_of(trigger_filter.second, [&](const std::string& value) { return base::Contains(source_filter->second, value); });
...
bool FilterData::MatchesForTesting(
    mojom::SourceType source_type, const base::Time& source_time, const base::Time& trigger_time, const FiltersDisjunction& filters, bool negated) const
{
    return Matches(source_type, source_time, trigger_time, filters, negated);
}

bool FilterData::Matches(mojom::SourceType source_type, const base::Time& source_time, const base::Time& trigger_time, const FilterPair& filters) const
{
...
std::optional<FilterConfig> FilterConfig::Create(FilterValues filter_values, std::optional<base::TimeDelta> lookback_window)
{
...
FilterConfig::FilterConfig(FilterValues filter_values, std::optional<base::TimeDelta> lookback_window)
...
base::expected<FiltersDisjunction, TriggerRegistrationError> FiltersFromJSON(base::Value* input_value)
{
    if (!input_value) {
        return FiltersDisjunction();
    }

    const auto map_errors = [](FilterValuesError error, TriggerRegistrationError value_error, TriggerRegistrationError reserved_key_error) {
        switch (error) {
        case FilterValuesError::kValueWrongType:
        case FilterValuesError::kListWrongType:
            return value_error;
    ...
    const auto append_if_valid = [&](base::Value& value, TriggerRegistrationError value_error, TriggerRegistrationError lookback_window_error,
        ...
        if (std::optional<base::Value> lookback_window_value = dict->Extract(FilterConfig::kLookbackWindowKey)) {
            ASSIGN_OR_RETURN(lookback_window, ParseDuration(*lookback_window_value), [lookback_window_error](ParseError) { return lookback_window_error; });
        ...
        ASSIGN_OR_RETURN(auto filter_values,
            ParseFilterValuesFromJSON(std::move(*dict),
                ...
                .transform_error([&](FilterValuesError error) { return map_errors(error, value_error, reserved_key_error); }));

        if (!filter_values.empty() || lookback_window.has_value()) {
            auto config = FilterConfig::Create(std::move(filter_values), lookback_window);
    ...
    if (base::Value::List* list = input_value->GetIfList()) {
        ...
        for (base::Value& item : *list) {
            RETURN_IF_ERROR(append_if_valid(item, TriggerRegistrationError::kFiltersListValueInvalid,
                TriggerRegistrationError::kFiltersListLookbackWindowValueInvalid, TriggerRegistrationError::kFiltersListUsingReservedKey));
        }
    } else {
        RETURN_IF_ERROR(append_if_valid(*input_value, TriggerRegistrationError::kFiltersValueInvalid,
            TriggerRegistrationError::kFiltersLookbackWindowValueInvalid, TriggerRegistrationError::kFiltersUsingReservedKey));
    }
...
base::Value::List ToJson(const FiltersDisjunction& filters)
{
    ...
    for (const auto& filter_config : filters) {
        ...
        if (filter_config.lookback_window().has_value()) {
            dict.Set(FilterConfig::kLookbackWindowKey, static_cast<int>(filter_config.lookback_window().value().InSeconds()));
        }
...
// static
base::expected<FilterPair, TriggerRegistrationError> FilterPair::FromJSON(base::Value::Dict& dict)
{
...
base::expected<FiltersDisjunction, TriggerRegistrationError> FiltersFromJSONForTesting(base::Value* input_value)
{
