...
TEST(AggregatableNamedBudgetDefsTest, Parse)
{
    ...
    EXPECT_THAT(AggregatableNamedBudgetDefs::FromJSON(/*value=*/nullptr), ValueIs(AggregatableNamedBudgetDefs()));
    const struct {
        const char* desc;
        const char* json;
        ::testing::Matcher<base::expected<AggregatableNamedBudgetDefs, SourceRegistrationError>> matches;
    } kTestCases[] = {
        {
            "wrong_type",
            R"json([])json",
            ErrorIs(SourceRegistrationError::kAggregatableNamedBudgetsDictInvalid),
        },
        {
            "empty_dict",
            R"json({})json",
        ...
        },
        {
            ...
            ValueIs(AggregatableNamedBudgetDefs::FromBudgetMap({ { "a", 65536 }, { "b", 1 } })),
        },
    };
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_THAT(AggregatableNamedBudgetDefs::FromJSON(&value), test_case.matches);
    }
}
