...
TEST(SuitableOriginTest, Create)
{
    ...
    for (const auto& test_case : kTestCases) {
        EXPECT_EQ(test_case.expected_suitable, SuitableOrigin::IsSuitable(test_case.origin)) << test_case.origin;

        std::optional<SuitableOrigin> actual = SuitableOrigin::Create(test_case.origin);

        EXPECT_EQ(test_case.expected_suitable, actual.has_value()) << test_case.origin;
...
TEST(SuitableOriginTest, Deserialize_Serialize)
{
    ...
    for (const auto& test_case : kTestCases) {
        std::optional<SuitableOrigin> actual = SuitableOrigin::Deserialize(test_case.str);

        EXPECT_EQ(test_case.expected.has_value(), actual.has_value()) << test_case.str;

        if (test_case.expected.has_value()) {
            EXPECT_EQ(test_case.expected, *actual.value()) << test_case.str;
            EXPECT_EQ(test_case.expected_serialization, actual->Serialize()) << test_case.str;
        }
    }
}
...
TEST(SuitableOriginTest, IsSitePotentiallySuitable)
{
    ...
    for (const auto& test_case : kTestCases) {
        EXPECT_EQ(IsSitePotentiallySuitable(test_case.site), test_case.expected_suitable) << test_case.site;
    }
}
