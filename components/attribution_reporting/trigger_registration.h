...
struct COMPONENT_EXPORT(ATTRIBUTION_REPORTING) TriggerRegistration {
    ...
    static base::expected<TriggerRegistration, mojom::TriggerRegistrationError> Parse(base::Value);

    // Logs metric on parsing failures.
    static base::expected<TriggerRegistration, mojom::TriggerRegistrationError> Parse(std::string_view json);
    ...
    friend bool operator==(const TriggerRegistration&, const TriggerRegistration&) = default;
    ...
    std::vector<AggregatableNamedBudgetCandidate> aggregatable_named_budget_candidates;
};
