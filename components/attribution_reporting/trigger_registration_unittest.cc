...
TriggerRegistration TriggerRegistrationWith(base::FunctionRef<void(TriggerRegistration&)> f)
{
...
TEST(TriggerRegistrationTest, Parse)
{
    ...
    scoped_feature_list.InitAndEnableFeature(features::kAttributionReportingAggregatableFilteringIds);
    const struct {
        const char* description;
        const char* json;
        ::testing::Matcher<base::expected<TriggerRegistration, TriggerRegistrationError>> matches;
    } kTestCases[] = {
        {
            "invalid_json",
            "!",
        ...
        },
        {
            "empty",
            R"json({})json",
            ValueIs(AllOf(Field(&TriggerRegistration::filters, FilterPair()), Field(&TriggerRegistration::debug_key, std::nullopt),
                Field(&TriggerRegistration::aggregatable_dedup_keys, IsEmpty()), Field(&TriggerRegistration::event_triggers, IsEmpty()),
                Field(&TriggerRegistration::aggregatable_trigger_data, IsEmpty()), Field(&TriggerRegistration::aggregatable_values, IsEmpty()),
                Field(&TriggerRegistration::debug_reporting, false), Field(&TriggerRegistration::aggregation_coordinator_origin, std::nullopt),
                Field(&TriggerRegistration::aggregatable_trigger_config, AggregatableTriggerConfig()),
                Field(&TriggerRegistration::aggregatable_debug_reporting_config, AggregatableDebugReportingConfig()))),
        },
        {
            "filters_valid",
           ...
           "a":1, "b": {"value": 2, "filtering_id": "3"}}})json",
            ValueIs(Field(&TriggerRegistration::aggregatable_values,
                ElementsAre(*AggregatableValues::Create(
                    { { "a", *AggregatableValuesValue::Create(1, /*filtering_id=*/0u) }, { "b", *AggregatableValuesValue::Create(2, 3) } }, FilterPair())))),
        },
        {
        ...
        },
        {
            ...
            ErrorIs(TriggerRegistrationError::kAggregatableValuesListValueInvalid),
        },
        {
        ...
        },
        {
            ...
            ValueIs(
                Field(&TriggerRegistration::aggregatable_dedup_keys, ElementsAre(AggregatableDedupKey(), AggregatableDedupKey(/*dedup_key=*/5, FilterPair())))),
        },
        {
        ...
        },
        {
            ...
            ValueIs(Field(&TriggerRegistration::aggregatable_trigger_config,
                Property(&AggregatableTriggerConfig::source_registration_time_config, SourceRegistrationTimeConfig::kInclude))),
        },
        {
            // Tested more thoroughly in
            // `aggregatable_trigger_config_unittest.cc`.
            ...
            ErrorIs(TriggerRegistrationError::kAggregatableSourceRegistrationTimeValueInvalid),
        },
        {
            // Tested more thoroughly in
            ...
            ErrorIs(TriggerRegistrationError::kAggregatableFilteringIdMaxBytesInvalidValue),
        },
    };

    static constexpr char kTriggerRegistrationErrorMetric[] = "Conversions.TriggerRegistrationError11";
    ...
    for (const auto& test_case : kTestCases) {
        ...
        if (trigger.has_value()) {
            ...
            histograms.ExpectUniqueSample(kTriggerRegistrationErrorMetric, trigger.error(), 1);
        }
    }
}
    ...
    const struct {
        ...
        },
        {
            TriggerRegistrationWith([](TriggerRegistration& r) {
                r.aggregatable_dedup_keys = { AggregatableDedupKey(/*dedup_key=*/1, FilterPair()) };
                r.aggregatable_trigger_data = { AggregatableTriggerData() };
                r.aggregatable_values = { *AggregatableValues::Create({ { "a", *AggregatableValuesValue::Create(2, /*filtering_id=*/0u) } }, FilterPair()),
                    *AggregatableValues::Create({ { "b", *AggregatableValuesValue::Create(3, /*filtering_id=*/0u) } }, FilterPair()) };
                r.debug_key = 3;
                r.debug_reporting = true;
                r.event_triggers = { EventTriggerData() };
                r.filters.positive = { *FilterConfig::Create({ { { "b", {} } } }) };
                r.filters.negative = { *FilterConfig::Create({ { { "c", {} } } }, /*lookback_window=*/base::Seconds(2)) };
                r.aggregatable_trigger_config = *AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kExclude,
                    /*trigger_context_id=*/"123", *AggregatableFilteringIdsMaxBytes::Create(2));
                r.aggregatable_debug_reporting_config = AggregatableDebugReportingConfig(
                    /*key_piece=*/1,
                    /*debug_data=*/
...
TEST(TriggerRegistrationTest, ParseAggregationCoordinator)
{
    const struct {
    ...
    } kTestCases[] = {
        {
            ...
            ValueIs(Field(&TriggerRegistration::aggregation_coordinator_origin, *SuitableOrigin::Create(GURL("https://a.test")))),
        },
        {
            "aggregation_coordinator_origin_wrong_type",
            R"json({"aggregation_coordinator_origin":123})json",
            ErrorIs(TriggerRegistrationError::kAggregationCoordinatorValueInvalid),
        },
        {
...
{
    base::test::ScopedFeatureList scoped_feature_list;
    scoped_feature_list.InitAndDisableFeature(features::kAttributionReportingAggregatableFilteringIds);
    const struct {
        TriggerRegistration input;
        const char* expected_json;
    } kTestCases[] = {
        ...
        {
            TriggerRegistrationWith([](TriggerRegistration& r) { r.aggregation_coordinator_origin = SuitableOrigin::Create(GURL("https://a.test")); }),
            R"json({
            "aggregatable_source_registration_time": "exclude",
...
TEST(TriggerRegistrationTest, ParseAggregatableDebugReportingConfig)
{
    const struct {
    ...
    } kTestCases[] = {
        {
            ...
            ValueIs(Field(&TriggerRegistration::aggregatable_debug_reporting_config, Field(&AggregatableDebugReportingConfig::key_piece, 2))),
        },
        {
            "invalid",
            R"json({
            ...
            ValueIs(Field(&TriggerRegistration::aggregatable_debug_reporting_config, AggregatableDebugReportingConfig())),
        },
    };
...
TEST(TriggerRegistrationTest, ParseAttributionScopesConfig)
{
    const struct {
    ...
    } kTestCases[] = {
        {
            ...
            ValueIs(Field(&TriggerRegistration::attribution_scopes, AttributionScopesSet({ "123" }))),
        },
        {
            "empty",
            R"json({})json",
            ValueIs(Field(&TriggerRegistration::attribution_scopes, AttributionScopesSet())),
        },
        {
    ...
    for (const auto& test_case : kTestCases) {
        ...
        if (trigger.has_value()) {
            histograms.ExpectUniqueSample("Conversions.ScopesPerTriggerRegistration", trigger->attribution_scopes.scopes().size(), 1);
        }
    }
}
...
TEST(TriggerRegistrationTest, ParseAggregatableNamedBudgetCandidate)
{
    const struct {
    ...
    } kTestCases[] = {
        {
            ...
            ValueIs(Field(&TriggerRegistration::aggregatable_named_budget_candidates,
                ElementsAre(AggregatableNamedBudgetCandidate(/*name=*/"a", FilterPair()),
        ...
        },
        {
            "empty",
            R"json({})json",
            ValueIs(Field(&TriggerRegistration::aggregatable_named_budget_candidates, IsEmpty())),
        },
        {
            ...
            ErrorIs(TriggerRegistrationError::kAggregatableNamedBudgetNameInvalid),
        },
    };
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_THAT(trigger, test_case.matches);
        if (trigger.has_value()) {
            histograms.ExpectUniqueSample("Conversions.NamedBudgetsPerTriggerRegistration", trigger->aggregatable_named_budget_candidates.size(), 1);
        }
    }
}
...
TEST(TriggerRegistrationTest, SerializeAggregatableNamedBudgetCandidate)
{
    const struct {
        ...
        {
            TriggerRegistrationWith([](TriggerRegistration& r) {
                r.aggregatable_named_budget_candidates = { AggregatableNamedBudgetCandidate(
                    /*name=*/"a",
                    FilterPair(
                        /*positive=*/FiltersDisjunction(),
                        /*negative=*/ { *FilterConfig::Create({ { { "a", { "b" } } } }) })) };
