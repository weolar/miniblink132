...
TEST(RegistrationInfoTest, ParseInfo)
{
    const struct {
        ...
        ::testing::Matcher<base::expected<RegistrationInfo, RegistrationInfoError>> matches_cross_app_web_enabled;
        ::testing::Matcher<base::expected<RegistrationInfo, RegistrationInfoError>> matches_cross_app_web_disabled;
    } kTestCases[] = {
        {
            "empty",
            "",
    ...
    static constexpr char kRegistrationInfoErrorMetric[] = "Conversions.RegistrationInfoError";
    ...
    for (const auto& test_case : kTestCases) {
        SCOPED_TRACE(test_case.description);
        {
            ...
            if (info.has_value()) {
                ...
                histograms.ExpectUniqueSample(kRegistrationInfoErrorMetric, info.error(), 1);
            }
        }
