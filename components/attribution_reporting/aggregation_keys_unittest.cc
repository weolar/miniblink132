...
{
    EXPECT_THAT(AggregationKeys::FromJSON(nullptr), ValueIs(Property(&AggregationKeys::keys, IsEmpty())));

    const struct {
        const char* description;
        const char* json;
        ::testing::Matcher<base::expected<AggregationKeys, SourceRegistrationError>> matches;
    } kTestCases[] = {
        {
            "Not a dictionary",
            "[]",
        ...
        },
        {
            "One valid key",
            R"({"key":"0x159"})",
            ValueIs(Property(&AggregationKeys::keys, ElementsAre(Pair("key", absl::MakeUint128(/*high=*/0, /*low=*/345))))),
        },
        {
            "Two valid keys",
            R"({"key1":"0x159","key2":"0x50000000000000159"})",
            ValueIs(Property(&AggregationKeys::keys,
                ElementsAre(Pair("key1", absl::MakeUint128(/*high=*/0, /*low=*/345)), Pair("key2", absl::MakeUint128(/*high=*/5, /*low=*/345))))),
        },
        {
            "Second key invalid",
            R"({"key1":"0x159","key2":""})",
...
TEST(AggregationKeysTest, FromJSON_CheckSize)
{
    ...
    const TestCase kTestCases[] = {
        ...
        { "excessive_key_size", false, 1, AggregationKeys::kMaxBytesPerAggregationKeyId + 1 },
    };
...
TEST(AggregationKeysTest, FromJSON_RecordsMetric)
{
    ...
    EXPECT_THAT(histograms.GetAllSamples("Conversions.AggregatableKeysPerSource"), ElementsAre(Bucket(2, 1)));
}
