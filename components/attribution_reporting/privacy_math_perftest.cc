...
using ScopedCollapsibleTriggerConfig = std::tuple</*collapse=*/bool, TriggerConfig, std::optional<ScopesConfig>>;
...
class PrivacyMathPerfTest : public testing::Test, public testing::WithParamInterface<ScopedCollapsibleTriggerConfig> {
protected:
    template <typename Func> void Run(const std::string& metric_basename, Func&& func) const
    {
        const auto& [collapse, tc, sc] = GetParam();
        const TriggerSpecs specs = SpecsFromWindowList(std::vector<int>(/*count=*/tc.trigger_data_cardinality,
                                                           /*value=*/tc.num_windows),
            collapse, MaxEventLevelReports(tc.max_reports));
        std::optional<AttributionScopesData> scopes;
        if (sc.has_value()) {
            scopes = AttributionScopesData::Create(AttributionScopesSet({ "1" }), sc->attribution_scope_limit, sc->max_event_states);
            ASSERT_TRUE(scopes);
        }
        ...
        perf_test::PerfResultReporter reporter(metric_basename, StoryName(GetParam()));
...
TEST_P(PrivacyMathPerfTest, NumStates)
{
    ...
    Run("AttributionReporting.NumStates", [](const TriggerSpecs& specs, const std::optional<AttributionScopesData>& scopes) { return GetNumStates(specs); });
}

TEST_P(PrivacyMathPerfTest, RandomizedResponse)
{
    constexpr PrivacyMathConfig kConfig {
        .max_channel_capacity_navigation = std::numeric_limits<double>::infinity(),
        .max_channel_capacity_event = std::numeric_limits<double>::infinity(),
    };

    Run("AttributionReporting.RandomizedResponse", [&](const TriggerSpecs& specs, const std::optional<AttributionScopesData>& scopes) {
        return DoRandomizedResponse(specs,
            /*epsilon=*/0,
            /*source_type=*/mojom::SourceType::kNavigation, scopes, kConfig);
    });
}

INSTANTIATE_TEST_SUITE_P(, PrivacyMathPerfTest,
    testing::Combine(/*collapse=*/testing::Bool(), testing::ValuesIn(kTriggerConfigs), testing::ValuesIn(kScopeConfigs)),
    [](const testing::TestParamInfo<ScopedCollapsibleTriggerConfig>& info) { return StoryName(info.param); });
