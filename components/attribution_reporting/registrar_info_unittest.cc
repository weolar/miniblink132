...
TEST(RegistrarInfoTest, GetRegistrar)
{
    ...
    static const char kSourceRegistrationMetric[] = "Conversions.SourceRegistrationRegistrarIssue";
    static const char kTriggerRegistrationMetric[] = "Conversions.TriggerRegistrationRegistrarIssue";

    for (const bool is_source : { true, false }) {
        SCOPED_TRACE(is_source);

        const char* metric = is_source ? kSourceRegistrationMetric : kTriggerRegistrationMetric;
        ...
        for (const auto& test_case : kTestCases) {
            ...
            EXPECT_EQ(
                RegistrarInfo::Get(test_case.has_web_header, test_case.has_os_header, is_source, test_case.preferred_platform, test_case.support), expected);
