...
// static
base::expected<MaxEventLevelReports, SourceRegistrationError> MaxEventLevelReports::Parse(const base::Value::Dict& dict, SourceType source_type)
{
    ...
    ASSIGN_OR_RETURN(int i, ParseInt(*value), [](ParseError) { return SourceRegistrationError::kMaxEventLevelReportsValueInvalid; });

    if (!IsMaxEventLevelReportsValid(i)) {
        return base::unexpected(SourceRegistrationError::kMaxEventLevelReportsValueInvalid);
    }
