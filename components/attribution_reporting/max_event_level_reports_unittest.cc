...
TEST(MaxEventLevelReportsTest, Parse)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_THAT(MaxEventLevelReports::Parse(input, test_case.source_type), test_case.matches);
    }
}
...
TEST(MaxEventLevelReportsTest, Serialize)
{
    ...
    EXPECT_THAT(dict, base::test::IsJson(R"json({"max_event_level_reports": 5})json"));
}
