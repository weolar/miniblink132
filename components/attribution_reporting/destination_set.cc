...
{
    return !destinations.empty() && destinations.size() <= kMaxDestinations && base::ranges::all_of(destinations, &IsSitePotentiallySuitable);
}

} // namespace

// static
std::optional<DestinationSet> DestinationSet::Create(Destinations destinations)
{
...
// static
base::expected<DestinationSet, SourceRegistrationError> DestinationSet::FromJSON(const base::Value* v)
{
    if (!v) {
    ...
    const auto append_if_valid = [&](const std::string& str, SourceRegistrationError error) -> AppendIfValidResult {
    ...
    RETURN_IF_ERROR(v->Visit(base::Overloaded {
        [&](const std::string& str) { return append_if_valid(str, SourceRegistrationError::kDestinationUntrustworthy); },
        [&](const base::Value::List& list) -> AppendIfValidResult {
            ...
            for (const auto& item : list) {
                ...
                RETURN_IF_ERROR(append_if_valid(*str, SourceRegistrationError::kDestinationListUntrustworthy));
        ...
        [](const auto&) -> AppendIfValidResult { return base::unexpected(SourceRegistrationError::kDestinationWrongType); },
    }));
