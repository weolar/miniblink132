...
SourceRegistration SourceRegistrationWith(DestinationSet destination_set, base::FunctionRef<void(SourceRegistration&)> f)
{
...
{
    const DestinationSet destination = *DestinationSet::Create({ net::SchemefulSite::Deserialize("https://d.example") });

    const struct {
        const char* desc;
        const char* json;
        ::testing::Matcher<base::expected<SourceRegistration, SourceRegistrationError>> matches;
        ...
        },
        {
            ...
            ValueIs(AllOf(Field(&SourceRegistration::source_event_id, 0), Field(&SourceRegistration::destination_set, destination),
                Field(&SourceRegistration::expiry, base::Days(30)),
                Field(&SourceRegistration::trigger_specs,
                    TriggerSpecs(SourceType::kNavigation, *EventReportWindows::FromDefaults(base::Days(30), SourceType::kNavigation),
                        MaxEventLevelReports(SourceType::kNavigation))),
                Field(&SourceRegistration::aggregatable_report_window, base::Days(30)), Field(&SourceRegistration::priority, 0),
                Field(&SourceRegistration::filter_data, FilterData()), Field(&SourceRegistration::debug_key, std::nullopt),
                Field(&SourceRegistration::aggregation_keys, AggregationKeys()), Field(&SourceRegistration::debug_reporting, false),
                Field(&SourceRegistration::trigger_data_matching, mojom::TriggerDataMatching::kModulus),
                Field(&SourceRegistration::aggregatable_debug_reporting_config, SourceAggregatableDebugReportingConfig()),
                Field(&SourceRegistration::attribution_scopes_data, std::nullopt), Field(&SourceRegistration::destination_limit_priority, 0),
                Field(&SourceRegistration::aggregatable_named_budget_defs, AggregatableNamedBudgetDefs()))),
        },
        {
        ...
        },
        {
          ...
          "destination":"https://d.example"})json",
            ValueIs(Field(&SourceRegistration::trigger_specs,
                TriggerSpecs(SourceType::kEvent, *EventReportWindows::FromDefaults(base::Seconds(86401), SourceType::kEvent),
        ...
        },
        {
            ...
            ValueIs(Field(&SourceRegistration::trigger_specs,
                TriggerSpecs(SourceType::kNavigation, *EventReportWindows::Create(base::Seconds(0), { base::Seconds(86401) }),
                    MaxEventLevelReports(SourceType::kNavigation)))),
        },
        {
          ...
          "destination":"https://d.example"})json",
            ValueIs(Field(&SourceRegistration::aggregatable_report_window, base::Seconds(86401))),
        },
        {
        ...
        },
        {
          ...
          "destination":"https://d.example"})json",
            ErrorIs(SourceRegistrationError::kAggregatableReportWindowValueInvalid),
        },
        {
        ...
        },
        {
            ...
            ValueIs(Field(&SourceRegistration::aggregatable_report_window, base::Seconds(3600))),
        },
        {
            ...
            ValueIs(Field(&SourceRegistration::aggregatable_report_window, base::Seconds(172800))),
        },
        {
        ...
        },
        {
            ...
            ValueIs(Field(&SourceRegistration::filter_data, *FilterData::Create({ { "a", { "b" } } }))),
        },
        {
        ...
        },
        {
            ...
            ValueIs(Field(&SourceRegistration::aggregation_keys, *AggregationKeys::FromKeys({ { "a", absl::MakeUint128(0, 1) } }))),
        },
        {
    ...
    static constexpr char kSourceRegistrationErrorMetric[] = "Conversions.SourceRegistrationError13";

    for (const auto& test_case : kTestCases) {
        SCOPED_TRACE(test_case.desc);
        base::HistogramTester histograms;

        auto source = SourceRegistration::Parse(test_case.json, test_case.source_type);
        EXPECT_THAT(source, test_case.matches);

        if (source.has_value()) {
            histograms.ExpectTotalCount(kSourceRegistrationErrorMetric, 0);
        } else {
            histograms.ExpectUniqueSample(kSourceRegistrationErrorMetric, source.error(), 1);
        }
    }
}
...
TEST(SourceRegistrationTest, ToJson)
{
        ...
        },
        {
            SourceRegistrationWith(destination,
                [](SourceRegistration& r) {
                    ...
                    r.trigger_specs = TriggerSpecs(SourceType::kNavigation, EventReportWindows(), MaxEventLevelReports(8));
                    r.aggregatable_debug_reporting_config = *SourceAggregatableDebugReportingConfig::Create(
                        /*budget=*/123,
    ...
    for (const auto& test_case : kTestCases) {
        EXPECT_THAT(test_case.input.ToJson(), base::test::IsJson(test_case.expected_json));
    }
}
...
TEST(SourceRegistrationTest, ParseDestinationLimitPriority)
{
    ...
    for (const auto& test_case : kTestCases) {
        SCOPED_TRACE(test_case.desc);
        EXPECT_THAT(SourceRegistration::Parse(test_case.json, SourceType::kNavigation), test_case.matches);
    }
}
...
TEST(SourceRegistrationTest, SerializeDestinationLimit)
{
    ...
    const struct {
        ...
        {
            SourceRegistrationWith(destination, [](SourceRegistration& r) { r.destination_limit_priority = 123; }),
            R"json({
              "aggregatable_report_window": 2592000,
              "debug_reporting": false,
              "destination":"https://d.example",
...
TEST(SourceRegistrationTest, IsValid)
{
    ...
    EXPECT_FALSE(SourceRegistrationWith(destination, [](SourceRegistration& r) {
        ...
        r.trigger_specs
            = TriggerSpecs(SourceType::kEvent, *EventReportWindows::FromDefaults(r.expiry, SourceType::kEvent), MaxEventLevelReports(SourceType::kEvent));
    }).IsValid());
    ...
    EXPECT_FALSE(SourceRegistrationWith(destination, [](SourceRegistration& r) {
        ...
        r.aggregatable_report_window = r.expiry;
        r.trigger_specs
            = TriggerSpecs(SourceType::kEvent, *EventReportWindows::FromDefaults(base::Days(30), SourceType::kEvent), MaxEventLevelReports(SourceType::kEvent));
    }).IsValid());
    ...
    EXPECT_FALSE(
        SourceRegistrationWith(destination, [](SourceRegistration& r) { r.aggregatable_report_window = base::Hours(1) - base::Microseconds(1); }).IsValid());
    ...
    EXPECT_FALSE(SourceRegistrationWith(destination, [](SourceRegistration& r) {
        r.expiry = base::Days(1);
        r.aggregatable_report_window = r.expiry;
        r.trigger_specs = TriggerSpecs(SourceType::kEvent, *EventReportWindows::FromDefaults(r.expiry + base::Microseconds(1), SourceType::kEvent),
            MaxEventLevelReports(SourceType::kEvent));
    }).IsValid());

    EXPECT_TRUE(SourceRegistrationWith(destination, [](SourceRegistration& r) { r.aggregatable_report_window = base::Hours(1); }).IsValid());
}
...
TEST(SourceRegistrationTest, ParseAggregatableDebugReportingConfig)
{
    const struct {
    ...
    } kTestCases[] = {
        {
            ...
            ValueIs(Field(&SourceRegistration::aggregatable_debug_reporting_config,
                AllOf(Property(&SourceAggregatableDebugReportingConfig::budget, 1),
                    Property(&SourceAggregatableDebugReportingConfig::config, Field(&AggregatableDebugReportingConfig::key_piece, 2))))),
        },
        {
            "invalid",
            R"json({
            ...
            ValueIs(Field(&SourceRegistration::aggregatable_debug_reporting_config, SourceAggregatableDebugReportingConfig())),
        },
    };
...
TEST(SourceRegistrationTest, ParseAttributionScopesConfig)
{
    const struct {
        ...
        },
        {
            ...
            ValueIs(Field(&SourceRegistration::attribution_scopes_data, std::nullopt)),
        },
        {
            "invalid",
            R"json({
    ...
    base::test::ScopedFeatureList scoped_feature_list(features::kAttributionScopes);

    for (const auto& test_case : kTestCases) {
        base::HistogramTester histograms;
        SCOPED_TRACE(test_case.desc);

        auto source = SourceRegistration::Parse(test_case.json, SourceType::kNavigation);
        EXPECT_THAT(source, test_case.matches);

        if (source.has_value()) {
            histograms.ExpectUniqueSample("Conversions.ScopesPerSourceRegistration", source->attribution_scopes_data.has_value() ? 1 : 0, 1);
        }
    }
}
...
TEST(SourceRegistrationTest, ParseAggregatableNamedBudgetDefs)
{
    const struct {
    ...
    } kTestCases[] = {
        {
            ...
            ValueIs(Field(&SourceRegistration::aggregatable_named_budget_defs, *AggregatableNamedBudgetDefs::FromBudgetMap({ { "a", 65536 } }))),
        },
        {
            "no_budgets",
            R"json({
            ...
            ValueIs(Field(&SourceRegistration::aggregatable_named_budget_defs, *AggregatableNamedBudgetDefs::FromBudgetMap({}))),
        },
        {
            ...
            ErrorIs(SourceRegistrationError::kAggregatableNamedBudgetsValueInvalid),
        },
    };

    base::test::ScopedFeatureList scoped_feature_list(features::kAttributionAggregatableNamedBudgets);
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_THAT(source, test_case.matches);
        if (source.has_value()) {
            histograms.ExpectUniqueSample("Conversions.NamedBudgetsPerSourceRegistration", source->aggregatable_named_budget_defs.budgets().size(), 1);
        }
    }
}
...
TEST(SourceRegistrationTest, SerializeAggregatableNamedBudgetDefs)
{
    ...
    const struct {
        ...
        },
        {
            SourceRegistrationWith(destination,
                [](SourceRegistration& r) {
                    r.aggregatable_named_budget_defs = *AggregatableNamedBudgetDefs::FromBudgetMap({ { "a", 65536 } });
                }),
            R"json({
            "aggregatable_report_window": 2592000,
            "debug_reporting": false,
