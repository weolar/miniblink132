...
TEST(AggregatableUtilsTest, RoundDownToWholeDaySinceUnixEpoch)
{
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_EQ(RoundDownToWholeDaySinceUnixEpoch(base::Time::UnixEpoch() + test_case.time), base::Time::UnixEpoch() + test_case.expected);
    }
}

TEST(AggregatableUtilsTest, GetNullAggregatableReports_IncludeSourceRegistrationTime)
{
    auto config = *AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kInclude,
    ...
    EXPECT_THAT(GetNullAggregatableReports(config,
        ...
        UnorderedElementsAre(NullAggregatableReport(now), NullAggregatableReport(now - base::Days(5))));
...
TEST(AggregatableUtilsTest, GetNullAggregatableReports_ExcludeSourceRegistrationTime)
{
    auto config = *AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kExclude,
        /*trigger_context_id=*/std::nullopt, kFilteringIdMaxBytes);
    base::Time now = base::Time::Now();
...
TEST(AggregatableUtilsTest, GetNullAggregatableReports_RoundedTime)
{
    auto config = *AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kInclude,
        /*trigger_context_id=*/std::nullopt, kFilteringIdMaxBytes);
    const auto generate_func = [](int day) { return day == 3 || day == 4 || day == 5; };

    const base::Time whole_day = base::Time::UnixEpoch() + base::Days(14288);

    const std::vector<base::TimeDelta> time_deltas = { base::TimeDelta(), base::Seconds(1), base::Hours(12), base::Days(1) - base::Seconds(1) };
    ...
    for (base::TimeDelta source_time_delta : time_deltas) {
        ...
        for (base::TimeDelta trigger_time_delta : time_deltas) {
            ...
            EXPECT_THAT(GetNullAggregatableReports(config, trigger_time,
                            ...
                            whole_day + source_time_delta, generate_func),
                UnorderedElementsAre(NullAggregatableReport(trigger_time - base::Days(3)), NullAggregatableReport(trigger_time - base::Days(5))));
        }
    }
}
...
TEST(AggregatableUtilsTest, GetNullAggregatableReportsUnconditionally)
{
    const struct {
    ...
    } kTestCases[] = {
        {
            ...
            .config = *AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kExclude,
                /*trigger_context_id=*/"", kFilteringIdMaxBytes),
        },
        {
            .description = "Non-default filtering id max bytes",
            .config = *AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kExclude,
                /*trigger_context_id=*/std::nullopt, *AggregatableFilteringIdsMaxBytes::Create(3)),
        },
    };
...
class AggregatableUtilsNullReportsTest : public testing::Test, public testing::WithParamInterface<NullReportsTestCase> {
};
...
TEST_P(AggregatableUtilsNullReportsTest, ExpectedDistribution)
{
    ...
    const auto generate = [&](int lookback_day) { return GenerateWithRate(test_case.rate); };
    ...
    for (int i = 0; i < num_samples; i++) {
        auto result = GetNullAggregatableReports(test_case.config, trigger_time, attributed_source_time, generate);
        actual_count += result.size();
    }

    const auto source_registration_time_config = test_case.config.source_registration_time_config();

    int num_total_samples = num_samples * GetNumLookbackDays(source_registration_time_config);
    ...
    EXPECT_NEAR(actual_count, expected_count, 2.575 * sqrt(num_total_samples * test_case.rate * (1. - test_case.rate)));
}

INSTANTIATE_TEST_SUITE_P(, AggregatableUtilsNullReportsTest, testing::ValuesIn(kNullReportsTestCases));
