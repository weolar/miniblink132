...
public:
    ...
    // to `Parse()`.
    static base::expected<TriggerSpecs, mojom::SourceRegistrationError> ParseFullFlexForTesting(
        const base::Value::Dict&, mojom::SourceType, base::TimeDelta expiry, EventReportWindows default_report_windows, mojom::TriggerDataMatching);

    // Parses the top-level `trigger_data` field. The resulting value is either
    // `empty()` or `SingleSharedSpec()`.
    static base::expected<TriggerSpecs, mojom::SourceRegistrationError> ParseTopLevelTriggerData(
        const base::Value::Dict&, mojom::SourceType, EventReportWindows default_report_windows, mojom::TriggerDataMatching);

    static std::optional<TriggerSpecs> Create(TriggerDataIndices, std::vector<TriggerSpec>, MaxEventLevelReports);
    ...
    bool empty() const
    {
        return trigger_data_indices_.empty();
    }

    size_t size() const
    {
        return trigger_data_indices_.size();
    }
    ...
    public:
        ...
        uint8_t index() const
        {
            return base::checked_cast<uint8_t>(std::distance(specs_->trigger_data_indices().cbegin(), it_));
        }
    ...
    const std::vector<TriggerSpec>& specs() const
    {
        return specs_;
    }
    ...
    void SetMaxEventLevelReportsForTesting(MaxEventLevelReports max_event_level_reports)
    {
...
private:
    TriggerSpecs(TriggerDataIndices, std::vector<TriggerSpec>, MaxEventLevelReports);
...
base::expected<mojom::TriggerDataMatching, mojom::SourceRegistrationError> ParseTriggerDataMatching(const base::Value::Dict&);
