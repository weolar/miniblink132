...
struct COMPONENT_EXPORT(ATTRIBUTION_REPORTING) OsRegistrationItem {
    ...
    friend bool operator==(const OsRegistrationItem&, const OsRegistrationItem&) = default;
};
...
base::expected<std::vector<OsRegistrationItem>, mojom::OsRegistrationError> ParseOsSourceOrTriggerHeader(std::string_view);

// Same as the above, but using an already-parsed structured-header list.
COMPONENT_EXPORT(ATTRIBUTION_REPORTING)
base::expected<std::vector<OsRegistrationItem>, mojom::OsRegistrationError> ParseOsSourceOrTriggerHeader(const net::structured_headers::List&);
