...
TEST(AttributionScopesSetTest, ParseSource)
{
    const struct {
        const char* desc;
        const char* json;
        ::testing::Matcher<base::expected<AttributionScopesSet, SourceRegistrationError>> matches;
        ...
        },
        {
            ...
            ValueIs(Property(&AttributionScopesSet::scopes, UnorderedElementsAre("1"))),
        },
        {
            "scopes_list_empty",
            R"json({"values": []})json",
        ...
        },
        {
            ...
            ValueIs(Property(&AttributionScopesSet::scopes, UnorderedElementsAre("1", "2", "3"))),
        },
        {
            "too_many_scopes",
            R"json({
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_THAT(AttributionScopesSet::FromJSON(value, test_case.scope_limit), test_case.matches);
    }
}
...
TEST(AttributionScopesSetTest, ParseTrigger)
{
    const struct {
        const char* desc;
        const char* json;
        ::testing::Matcher<base::expected<AttributionScopesSet, TriggerRegistrationError>> matches;
    } kTestCases[] = {
        {
            "empty",
            R"json({})json",
        ...
        },
        {
            ...
            ValueIs(Property(&AttributionScopesSet::scopes, UnorderedElementsAre("This string exceeds 50 characters to pass the limit"))),
        },
    };
...
TEST(AttributionScopesSetTest, HasIntersection)
{
    const struct {
        ...
        bool expected;
    } kTestCases[] = { { AttributionScopesSet(), AttributionScopesSet(), false }, { AttributionScopesSet({ "a" }), AttributionScopesSet(), false },
        { AttributionScopesSet({ "a" }), AttributionScopesSet({ "a" }), true },
        { AttributionScopesSet({ "a", "c" }), AttributionScopesSet({ "b", "c" }), true },
        { AttributionScopesSet({ "a" }), AttributionScopesSet({ "b" }), false } };
    for (const auto& test_case : kTestCases) {
        EXPECT_THAT(test_case.set_1.HasIntersection(test_case.set_2), test_case.expected);
        EXPECT_THAT(test_case.set_2.HasIntersection(test_case.set_1), test_case.expected);
    }
}
