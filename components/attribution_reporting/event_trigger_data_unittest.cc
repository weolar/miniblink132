...
TEST(EventTriggerDataTest, FromJSON)
{
    const struct {
        ...
        const char* json;
        ::testing::Matcher<base::expected<EventTriggerData, TriggerRegistrationError>> matches;
    } kTestCases[] = {
        {
            "empty",
            R"json({})json",
            ValueIs(AllOf(Field(&EventTriggerData::data, 0), Field(&EventTriggerData::priority, 0), Field(&EventTriggerData::dedup_key, std::nullopt),
        ...
        },
        {
            ...
            ValueIs(Field(&EventTriggerData::filters,
                FilterPair(
                    /*positive=*/ { *FilterConfig::Create({ { "a", { "b" } } }, /*lookback_window=*/base::Seconds(1)) },
    ...
    const struct {
        ...
        },
        {
            EventTriggerData(
                ...
                /*dedup_key=*/3,
                FilterPair(
                    /*positive=*/ { *FilterConfig::Create({ { "a", {} } }, /*lookback_window=*/base::Seconds(2)) },
...
TEST(EventTriggerValueTest, Parse)
{
    const struct {
        const char* description;
        const char* json;
        ::testing::Matcher<base::expected<EventTriggerValue, TriggerRegistrationError>> matches;
    } kTestCases[] = {
        {
            "empty",
            R"json({})json",
