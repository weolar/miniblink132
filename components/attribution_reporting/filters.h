...
public:
    ...
    static base::expected<FilterData, FilterValuesError> CreateForTesting(FilterValues);

    static base::expected<FilterData, mojom::SourceRegistrationError> FromJSON(base::Value*);
    ...
    const FilterValues& filter_values() const
    {
    ...
    bool Matches(mojom::SourceType, const base::Time& source_time, const base::Time& trigger_time, const FilterPair&) const;

    bool MatchesForTesting(mojom::SourceType, const base::Time& source_time, const base::Time& trigger_time, const FiltersDisjunction&, bool negated) const;
...
private:
    ...
    bool Matches(mojom::SourceType, const base::Time& source_time, const base::Time& trigger_time, const FiltersDisjunction&, bool negated) const;
...
struct COMPONENT_EXPORT(ATTRIBUTION_REPORTING) FilterPair {
    ...
    // dict, if present.
    static base::expected<FilterPair, mojom::TriggerRegistrationError> FromJSON(base::Value::Dict&);
...
public:
    ...
    static std::optional<FilterConfig> Create(FilterValues, std::optional<base::TimeDelta> lookback_window = std::nullopt);
...
private:
    explicit FilterConfig(FilterValues, std::optional<base::TimeDelta> lookback_window);
    std::optional<base::TimeDelta> lookback_window_;
    FilterValues filter_values_;
};

COMPONENT_EXPORT(ATTRIBUTION_REPORTING)
base::expected<FiltersDisjunction, mojom::TriggerRegistrationError> FiltersFromJSONForTesting(base::Value* input_value);
