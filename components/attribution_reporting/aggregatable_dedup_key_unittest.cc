...
TEST(AggregatableDedupKeyTest, FromJSON)
{
    const struct {
        ...
        const char* json;
        ::testing::Matcher<base::expected<AggregatableDedupKey, TriggerRegistrationError>> matches;
    } kTestCases[] = {
        {
            "empty",
            R"json({})json",
            ValueIs(AllOf(Field(&AggregatableDedupKey::dedup_key, std::nullopt), Field(&AggregatableDedupKey::filters, FilterPair()))),
        },
        {
            "dedup_key_valid",
        ...
        },
        {
            ...
            ValueIs(Field(&AggregatableDedupKey::filters,
                FilterPair(/*positive=*/ { *FilterConfig::Create({ { { "a", { "b" } } } },
