...
public:
    static std::optional<AttributionScopesData> Create(AttributionScopesSet, uint32_t attribution_scope_limit, uint32_t max_event_states);

    static base::expected<AttributionScopesData, mojom::SourceRegistrationError> FromJSON(base::Value&);
    ...
    {
        return max_event_states_;
    }

    base::Value::Dict ToJson() const;

    friend bool operator==(const AttributionScopesData&, const AttributionScopesData&) = default;

private:
    AttributionScopesData(AttributionScopesSet, uint32_t attribution_scope_limit, uint32_t max_event_states);
