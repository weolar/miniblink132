...
TEST(AggregatableNamedBudgetCandidateTest, Parse)
{
    const struct {
        const char* desc;
        const char* json;
        ::testing::Matcher<base::expected<AggregatableNamedBudgetCandidate, TriggerRegistrationError>> matches;
    } kTestCases[] = {
        {
            "wrong_type",
            R"json([])json",
        ...
        },
        {
            "empty_dict",
            R"json({})json",
            ValueIs(AggregatableNamedBudgetCandidate(/*name=*/std::nullopt, FilterPair())),
        },
        {
            "basic",
            R"json({"name": "a"})json",
    ...
    for (const auto& test_case : kTestCases) {
        ...
        EXPECT_THAT(AggregatableNamedBudgetCandidate::FromJSON(value), test_case.matches);
    }
}
