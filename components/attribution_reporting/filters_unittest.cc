...
const struct {
    ...
    base::expected<FiltersDisjunction, TriggerRegistrationError> expected_filters_list;
} kParseTestCases[] = {
    {
        "Null",
        std::nullopt,
    ...
    },
    {
        ...
        base::unexpected(SourceRegistrationError::kFilterDataKeyReserved),
        FiltersDisjunction({ *FilterConfig::Create({}, /*lookback_window=*/base::Seconds(1)) }),
        FiltersDisjunction({ *FilterConfig::Create({}, /*lookback_window=*/base::Seconds(1)) }),
    },
    {
        "reserved_key",
        ...
        base::unexpected(TriggerRegistrationError::kFiltersListUsingReservedKey),
    },
    {
        "lookback_window_list",
        base::test::ParseJson(R"json({"_lookback_window": ["a"]})json"),
        base::unexpected(SourceRegistrationError::kFilterDataKeyReserved),
        base::unexpected(TriggerRegistrationError::kFiltersLookbackWindowValueInvalid),
        base::unexpected(TriggerRegistrationError::kFiltersListLookbackWindowValueInvalid),
    },
    {
...
{
    EXPECT_TRUE(FilterData::Create(CreateFilterValues(kMaxFiltersPerSource)).has_value());

    EXPECT_FALSE(FilterData::Create(CreateFilterValues(kMaxFiltersPerSource + 1)).has_value());
}

TEST(FilterDataTest, FromJSON)
{
    for (auto& test_case : kParseTestCases) {
        std::optional<base::Value> json_copy = test_case.json ? std::make_optional(test_case.json->Clone()) : std::nullopt;
        EXPECT_EQ(FilterData::FromJSON(base::OptionalToPtr(json_copy)), test_case.expected_filter_data) << test_case.description;
    }

    for (auto& test_case : kSizeTestCases) {
        std::optional<base::Value> json_copy = test_case.json ? std::make_optional(test_case.json->Clone()) : std::nullopt;
        EXPECT_THAT(FilterData::FromJSON(base::OptionalToPtr(json_copy)), base::test::ErrorIs(test_case.expected_filter_data_error)) << test_case.description;
    }
    ...
    // was erroneously checked before deduplication instead of after.
    {
        ...
        EXPECT_THAT(FilterData::FromJSON(&value), ValueIs(Property(&FilterData::filter_values, ElementsAre(Pair("a", ElementsAre("0", "1", "2", "3", "4"))))));
    }
...
TEST(FilterDataTest, FromJSON_RecordsMetrics)
{
    ...
    EXPECT_THAT(histograms.GetAllSamples("Conversions.FiltersPerFilterData"), ElementsAre(Bucket(4, 1)));

    EXPECT_THAT(histograms.GetAllSamples("Conversions.ValuesPerFilter"), ElementsAre(Bucket(0, 1), Bucket(1, 2), Bucket(3, 1)));
}
...
TEST(FiltersTest, FromJSON)
{
    for (auto& test_case : kParseTestCases) {
        ...
        std::optional<base::Value> json_copy = test_case.json ? std::make_optional(test_case.json->Clone()) : std::nullopt;
        EXPECT_EQ(FiltersFromJSONForTesting(base::OptionalToPtr(json_copy)), test_case.expected_filters);
    }
...
TEST(FiltersTest, FromJSON_list)
{
    ...
    for (auto& test_case : kParseTestCases) {
        ...
        EXPECT_EQ(FiltersFromJSONForTesting(&json_copy), test_case.expected_filters_list) << test_case.description << " within a list";
    }
    {
        auto multiple_valid_filter_values = base::test::ParseJson(R"json([{"a":["b"]},{"c":["d"]}])json");
    ...
    {
        auto one_valid_and_one_invalid_filter_values = base::test::ParseJson(R"json([{"a":["b"]},"invalid"])json");
        auto actual = FiltersFromJSONForTesting(&one_valid_and_one_invalid_filter_values);
        EXPECT_THAT(actual, base::test::ErrorIs(TriggerRegistrationError::kFiltersWrongType));
    }
}
...
TEST(FilterDataTest, ToJson)
{
    ...
    for (const auto& test_case : kTestCases) {
        EXPECT_THAT(FilterData::Create(test_case.input)->ToJson(), base::test::IsJson(test_case.expected_json));
    }
}
...
TEST(FiltersTest, ToJson)
{
    const struct {
        ...
        {
            { *FilterConfig::Create(FilterValues({ { "a", {} } })), *FilterConfig::Create(FilterValues({ { "b", { "c" } } })) },
            R"json([{"a":[]},{"b":["c"]}])json",
        },
        {
    ...
    for (const auto& test_case : kTestCases) {
        EXPECT_THAT(ToJsonForTesting(test_case.input), base::test::IsJson(test_case.expected_json));
    }
}
...
TEST(FilterDataTest, EmptyOrMissingAttributionFilters)
{
    ...
    } kTestCases[] = { { "No source filters, no trigger filters", empty_filter, empty_filter },
        { "No source filters, trigger filter without values", empty_filter, empty_filter_values },
        { "No source filters, trigger filter with value", empty_filter, one_filter },

        { "Source filter without values, no trigger filters", empty_filter_values, empty_filter },

        { "Source filter with value, no trigger filters", one_filter, empty_filter } };

    // Behavior should match for negated and non-negated filters as it
    // requires a value on each side.
    for (const auto& test_case : kTestCases) {
        std::optional<FilterData> filter_data = FilterData::Create(test_case.filter_data);
        ASSERT_TRUE(filter_data) << test_case.description;

        FiltersDisjunction filters({ *FilterConfig::Create(test_case.filters) });
        EXPECT_TRUE(filter_data->MatchesForTesting(SourceType::kNavigation, kSourceTime, kTriggerTime, filters,
            /*negated=*/false))
...
TEST(FilterDataTest, AttributionFilterDataMatch)
{
    ...
    const auto two_filters = FilterValues({ { "filter1", { "value1" } }, { "filter2", { "value2" } } });

    const auto one_mismatched_filter = FilterValues({ { "filter1", { "value1" } }, { "filter2", { "value3" } } });

    const auto two_mismatched_filter = FilterValues({ { "filter1", { "value3" } }, { "filter2", { "value4" } } });
    ...
    const struct {
        ...
        { "Source filter without values, trigger filter with value", empty_filter_values, one_filter, false },
        { "Source filter without values, trigger filter without values", empty_filter_values, empty_filter_values, true },
        { "Source filter with value, trigger filter without values", one_filter, empty_filter_values, false },

        { "One filter with matching values", one_filter, one_filter, true },
        { "One filter with no matching values", one_filter, one_filter_different, false },

        { "Two filters with matching values", two_filters, two_filters, true },
        { "Two filters no matching values", one_mismatched_filter, two_mismatched_filter, false },

        { "One filter not present in source, other matches", one_filter, two_filters, true },
        { "One filter not present in trigger, other matches", two_filters, one_filter, true },

        { "Two filters one filter no match", two_filters, one_mismatched_filter, false },
    };
    for (const auto& test_case : kTestCases) {
        std::optional<FilterData> filter_data = FilterData::Create(test_case.filter_data);
        ASSERT_TRUE(filter_data) << test_case.description;
        ...
        EXPECT_EQ(test_case.match_expected,
            filter_data->MatchesForTesting(SourceType::kNavigation, kSourceTime, kTriggerTime, filters,
                /*negated=*/false))
            << test_case.description;
    }
}
...
TEST(FilterDataTest, AttributionFilterDataMatch_Disjunction)
{
    ...
    for (const auto& test_case : kTestCases) {
        EXPECT_EQ(
            test_case.match_expected, filter_data.MatchesForTesting(SourceType::kEvent, kSourceTime, kTriggerTime, test_case.disjunction, test_case.negated))
            << test_case.description;
    }
}
...
TEST(FilterDataTest, NegatedAttributionFilterDataMatch)
{
    ...
    const auto one_filter_one_different = FilterValues({ { "filter1", { "value1", "value2" } } });

    const auto one_filter_multiple_different = FilterValues({ { "filter1", { "value2", "value3" } } });
    ...
    const struct {
        ...
        { "Source filter without values, trigger filter with value", empty_filter_values, one_filter, true },
        { "Source filter without values, trigger filter without values", empty_filter_values, empty_filter_values, false },
        { "Source filter with value, trigger filter without values", one_filter, empty_filter_values, true },

        { "One filter with matching values", one_filter, one_filter, false },
        { "One filter with non-matching value", one_filter, one_filter_different, true },
        { "One filter with one non-matching value", one_filter, one_filter_one_different, false },
        { "One filter with multiple non-matching values", one_filter, one_filter_multiple_different, true },

        { "Two filters with matching values", two_filters, two_filters, false },
        { "Two filters no matching values", one_mismatched_filter, two_mismatched_filter, true },

        { "One filter not present in source, other matches", one_filter, two_filters, false },
        { "One filter not present in trigger, other matches", two_filters, one_filter, false },
...
TEST(FilterDataTest, AttributionFilterDataMatch_SourceType)
{
    ...
    for (const auto& test_case : kTestCases) {
        EXPECT_EQ(
            test_case.match_expected, FilterData().MatchesForTesting(test_case.source_type, kSourceTime, kTriggerTime, test_case.filters, test_case.negated))
            << test_case.description;
    }
}
...
TEST(FilterDataTest, AttributionFilterDataMatch_LookbackWindow)
{
    ...
    const struct {
    ...
    } kTestCases[] = {
        {
            ...
            .filter_data = {},
            .filters = FiltersDisjunction({ *FilterConfig::Create({}, /*lookback_window=*/kTriggerTime - kSourceTime + base::Microseconds(1)) }),
            .negated = false,
            .match_expected = true,
        },
        {
        ...
        },
        {
            ...
            .filter_data = {},
            .filters = FiltersDisjunction({ *FilterConfig::Create({}, /*lookback_window=*/kTriggerTime - kSourceTime) }),
            .negated = false,
            .match_expected = true,
        },
        {
        ...
        },
        {
            ...
            .filters = FiltersDisjunction({ *FilterConfig::Create({ one_filter }, /*lookback_window=*/kTriggerTime - kSourceTime) }),
            .negated = false,
            .match_expected = true,
        },
        {
            .description = "duration-equal-to-window-with-matching-filter-negated",
            .filter_data = *FilterData::Create(one_filter),
            .filters = FiltersDisjunction({ *FilterConfig::Create({ one_filter }, /*lookback_window=*/kTriggerTime - kSourceTime) }),
            .negated = true,
            .match_expected = false,
        },
        {
            ...
            .filter_data = *FilterData::Create(one_filter),
            .filters = FiltersDisjunction({ *FilterConfig::Create({ one_filter_different },
                /*lookback_window=*/kTriggerTime - kSourceTime) }),
            .negated = false,
            .match_expected = false,
        },
        {
            .description = "duration-equal-to-window-with-non-matching-filter-negated",
            .filter_data = *FilterData::Create(one_filter),
            .filters = FiltersDisjunction({ *FilterConfig::Create({ one_filter_different },
                /*lookback_window=*/kTriggerTime - kSourceTime) }),
            .negated = true,
        ...
        },
        {
            ...
            .filter_data = *FilterData::Create(one_filter),
            .filters = FiltersDisjunction({ *FilterConfig::Create({ one_filter }, /*lookback_window=*/kTriggerTime - kSourceTime - base::Microseconds(1)) }),
            .negated = false,
            .match_expected = false,
        },
        {
            .description = "duration-greater-than-window-with-matching-filter-negated",
            .filter_data = *FilterData::Create(one_filter),
            .filters = FiltersDisjunction({ *FilterConfig::Create({ one_filter }, /*lookback_window=*/kTriggerTime - kSourceTime - base::Microseconds(1)) }),
            .negated = true,
            .match_expected = false,
        ...
        {
            .description = "duration-greater-than-window-with-non-matching-filter",
            .filter_data = *FilterData::Create(one_filter),
            .filters = FiltersDisjunction({ *FilterConfig::Create({ one_filter_different },
                /*lookback_window=*/kTriggerTime - kSourceTime - base::Microseconds(1)) }),
            .negated = false,
            .match_expected = false,
        },
        {
            .description = "duration-greater-than-window-with-non-matching-filter-negated",
            .filter_data = *FilterData::Create(one_filter),
            .filters = FiltersDisjunction({ *FilterConfig::Create({ one_filter_different },
                /*lookback_window=*/kTriggerTime - kSourceTime - base::Microseconds(1)) }),
            .negated = true,
    ...
    for (const auto& test_case : kTestCases) {
        EXPECT_EQ(test_case.match_expected,
            FilterData(test_case.filter_data).MatchesForTesting(SourceType::kEvent, kSourceTime, kTriggerTime, test_case.filters, test_case.negated))
            << test_case.description;
    }
}
...
// implementation.
TEST(FilterDataTest, AttributionFilterDataMatch_SourceTimeGreaterThanTriggerTime)
{
    const auto one_filter = FilterValues({ { "filter1", { "value1" } } });
    const auto filter_data = *FilterData::Create(one_filter);
    const auto filters = FiltersDisjunction({ *FilterConfig::Create({ one_filter }, /*lookback_window=*/kTriggerTime - kSourceTime) });
    EXPECT_TRUE(FilterData(filter_data)
                    .MatchesForTesting(SourceType::kEvent, kSourceTime,
                        /*trigger_time=*/kSourceTime - base::Microseconds(1), filters, /*negated=*/false));
}
