...
FiltersDisjunction FiltersForSourceType(mojom::SourceType, std::optional<base::TimeDelta> lookback_window = std::nullopt);
...
// value).
TriggerSpecs SpecsFromWindowList(const std::vector<int>& windows_per_type, bool collapse_into_single_spec, MaxEventLevelReports);
...
std::ostream& operator<<(std::ostream&, const AggregatableNamedBudgetCandidate&);
...
std::ostream& operator<<(std::ostream& out, const AggregatableDebugReportingConfig&);

std::ostream& operator<<(std::ostream& out, const SourceAggregatableDebugReportingConfig&);

std::ostream& operator<<(std::ostream& out, const AggregatableDebugReportingContribution&);
