...
TEST(AggregatableTriggerConfigTest, ParseAggregatableSourceRegistrationTime)
{
    const struct {
        const char* desc;
        const char* json;
        ::testing::Matcher<base::expected<AggregatableTriggerConfig, TriggerRegistrationError>> matches;
    } kTestCases[] = {
        {
            "empty",
            R"json({})json",
        ...
        },
        {
            ...
            ValueIs(Property(&AggregatableTriggerConfig::source_registration_time_config, SourceRegistrationTimeConfig::kInclude)),
        },
        {
            "aggregatable_source_registration_time_exclude",
            R"json({"aggregatable_source_registration_time":"exclude"})json",
            ValueIs(Property(&AggregatableTriggerConfig::source_registration_time_config, SourceRegistrationTimeConfig::kExclude)),
        },
        {
...
TEST(AggregatableTriggerConfigTest, ParseTriggerContextId)
{
    const struct {
        const char* desc;
        const char* json;
        ::testing::Matcher<base::expected<AggregatableTriggerConfig, TriggerRegistrationError>> enabled_matches;
        ::testing::Matcher<base::expected<AggregatableTriggerConfig, TriggerRegistrationError>> disabled_matches;
    } kTestCases[] = {
        {
            "empty",
            R"json({})json",
        ...
        },
        {
            ...
            ValueIs(Property(&AggregatableTriggerConfig::trigger_context_id, Optional(std::string("123")))),
            ValueIs(AggregatableTriggerConfig()),
        },
        {
        ...
        },
        {
            ...
            ErrorIs(TriggerRegistrationError::kTriggerContextIdInvalidSourceRegistrationTimeConfig),
            ValueIs(*AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kInclude,
    ...
    for (const auto& test_case : kTestCases) {
        ...
        base::Value::Dict input = base::test::ParseJsonDict(test_case.json);
        EXPECT_THAT(AggregatableTriggerConfig::Parse(input), test_case.enabled_matches);
    }
}

TEST(AggregatableTriggerConfigTest, ParseAggregatableFilteringIdMaxByte_FilteringIdsEnabled)
{
    ...
    const struct {
    ...
    } kTestCases[] = {
        {
            ...
            ValueIs(Property(&AggregatableTriggerConfig::aggregatable_filtering_id_max_bytes, *AggregatableFilteringIdsMaxBytes::Create(3))),
        },
        {
        ...
        },
        {
            ...
            ErrorIs(TriggerRegistrationError::kAggregatableFilteringIdsMaxBytesInvalidSourceRegistrationTimeConfig),
        },
        {
            ...
            ValueIs(Property(&AggregatableTriggerConfig::aggregatable_filtering_id_max_bytes, AggregatableFilteringIdsMaxBytes())),
        },
    };
...
TEST(AggregatableTriggerConfigTest, ParseAggregatableFilteringIdMaxByte_FilteringIdsDisabled)
{
    base::test::ScopedFeatureList scoped_feature_list;
...
TEST(AggregatableTriggerConfigTest, Create)
{
    const struct {
    ...
    } kTestCases[] = {
        {
            ...
            *AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kExclude, "123", kFilteringIdMaxBytes),
        ...
        },
        {
            ...
            /*trigger_context_id=*/std::nullopt,
            kFilteringIdMaxBytes,
            *AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kInclude, std::nullopt, kFilteringIdMaxBytes),
            /*should_cause_a_report_to_be_sent_unconditionally=*/false,
        },
        {
        ...
        },
        {
            ...
            *AggregatableFilteringIdsMaxBytes::Create(2),
            /*expected=*/
            *AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kExclude, std::nullopt, *AggregatableFilteringIdsMaxBytes::Create(2)),
            /*should_cause_a_report_to_be_sent_unconditionally=*/true,
        },
        {
            ...
            *AggregatableFilteringIdsMaxBytes::Create(2),
            /*expected=*/
            *AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kExclude, "123", *AggregatableFilteringIdsMaxBytes::Create(2)),
            /*should_cause_a_report_to_be_sent_unconditionally=*/true,
        },
    };
    ...
    for (const auto& test_case : kTestCases) {
        ...
        ASSERT_EQ(test_case.expected.has_value(), test_case.should_cause_a_report_to_be_sent_unconditionally.has_value());

        auto actual
            = AggregatableTriggerConfig::Create(test_case.source_registration_time_config, test_case.trigger_context_id, test_case.filtering_id_max_bytes);
        EXPECT_EQ(actual, test_case.expected);
        if (test_case.expected.has_value()) {
            EXPECT_EQ(actual->ShouldCauseAReportToBeSentUnconditionally(), test_case.should_cause_a_report_to_be_sent_unconditionally);
        }
    }
}
...
TEST(AggregatableTriggerConfigTest, Parse_TriggerContextIdLength)
{
    ...
    EXPECT_THAT(AggregatableTriggerConfig::Parse(dict), ValueIs(Property(&AggregatableTriggerConfig::trigger_context_id, Optional(std::string(64, 'a')))));

    dict.Set(kTriggerContextId, std::string(65, 'a'));
    EXPECT_THAT(AggregatableTriggerConfig::Parse(dict), ErrorIs(TriggerRegistrationError::kTriggerContextIdInvalidValue));
}
    ...
    const struct {
        ...
        },
        {
            *AggregatableTriggerConfig::Create(SourceRegistrationTimeConfig::kExclude,
                /*trigger_context_id=*/"123", *AggregatableFilteringIdsMaxBytes::Create(3u)),
            R"json({
            "aggregatable_source_registration_time":"exclude",
            "aggregatable_filtering_id_max_bytes": 3,
            "trigger_context_id": "123"
