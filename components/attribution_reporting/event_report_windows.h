...
public:
    ...
    static std::optional<EventReportWindows> Create(base::TimeDelta start_time, std::vector<base::TimeDelta> end_times);

    // Uses default windows based on the source type, but truncated at
    // `report_window`.
    static std::optional<EventReportWindows> FromDefaults(base::TimeDelta report_window, mojom::SourceType);

    static base::expected<EventReportWindows, mojom::SourceRegistrationError> FromJSON(
        const base::Value::Dict& registration, base::TimeDelta expiry, mojom::SourceType);

    static base::expected<EventReportWindows, mojom::SourceRegistrationError> ParseWindows(
        const base::Value::Dict&, base::TimeDelta expiry, const EventReportWindows& default_if_absent);
    ...
    // source.
    base::Time ComputeReportTime(base::Time source_time, base::Time trigger_time) const;
    ...
    friend bool operator==(const EventReportWindows&, const EventReportWindows&) = default;

private:
    EventReportWindows(base::TimeDelta start_time, base::flat_set<base::TimeDelta> end_times);

    EventReportWindows(base::TimeDelta report_window, mojom::SourceType);

    static base::expected<EventReportWindows, mojom::SourceRegistrationError> ParseWindowsJSON(const base::Value&, base::TimeDelta expiry);
