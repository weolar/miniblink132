...
fuzztest::Domain<std::string> AnyFilterString(std::optional<size_t> max_size)
{
    ...
    return fuzztest::Filter([](const std::string& s) { return base::IsStringUTF8AllowingNoncharacters(s); }, std::move(string));
}

template <typename KeyPred>
fuzztest::Domain<FilterValues> AnyFilterValues(
    const fuzztest::Domain<std::string>& string, KeyPred&& key_pred, std::optional<size_t> max_values_per_key, std::optional<size_t> max_keys)
{
    ...
    auto key_values = fuzztest::ContainerOf<FilterValues>(fuzztest::PairOf(fuzztest::Filter(std::move(key_pred), string), std::move(values_per_key)));
...
{
    return fuzztest::Map([](int64_t micros) { return base::Microseconds(micros); }, fuzztest::Positive<int64_t>());
}
...
{
    return fuzztest::ConstructorOf<MaxEventLevelReports>(fuzztest::InRange(0, static_cast<int>(MaxEventLevelReports::Max())));
}
...
fuzztest::Domain<FilterData> AnyFilterData()
{
    return fuzztest::Map(
        [](const FilterValues& filter_values) {
            ...
            CHECK(result.has_value()) << static_cast<int>(result.error()) << ": " << FilterValuesToJson(filter_values);
            return *std::move(result);
        },
        AnyFilterValues(
            AnyFilterString(kMaxBytesPerFilterString), [](const std::string& key) { return key != FilterData::kSourceTypeFilterKey; }, kMaxValuesPerFilter,
            kMaxFiltersPerSource));
}

fuzztest::Domain<FilterConfig> AnyFilterConfig()
{
    return fuzztest::Map([](FilterValues&& filter_values,
                             std::optional<base::TimeDelta> lookback_window) { return *FilterConfig::Create(std::move(filter_values), lookback_window); },
        AnyFilterValues(
            AnyFilterString(/*max_size=*/std::nullopt), [](const std::string& key) { return !base::StartsWith(key, FilterConfig::kReservedKeyPrefix); },
