...
base::expected<absl::uint128, ParseError> ParseAggregationKeyPiece(const base::Value&);

COMPONENT_EXPORT(ATTRIBUTION_REPORTING)
std::string HexEncodeAggregationKey(absl::uint128);

template <typename T> requires(std::integral<T>) constexpr T ValueOrZero(std::optional<T> value)
{
    return value.value_or(0);
}

COMPONENT_EXPORT(ATTRIBUTION_REPORTING)
base::expected<std::optional<uint64_t>, ParseError> ParseUint64(const base::Value::Dict&, std::string_view key);

COMPONENT_EXPORT(ATTRIBUTION_REPORTING)
base::expected<std::optional<int64_t>, ParseError> ParseInt64(const base::Value::Dict&, std::string_view key);
...
base::expected<std::optional<uint64_t>, ParseError> ParseDeduplicationKey(const base::Value::Dict&);
...
base::expected<base::TimeDelta, ParseError> ParseLegacyDuration(const base::Value&, base::TimeDelta clamp_min, base::TimeDelta clamp_max);
...
base::expected<std::optional<SuitableOrigin>, ParseError> ParseAggregationCoordinator(const base::Value::Dict&);
...
void SerializeDeduplicationKey(base::Value::Dict&, std::optional<uint64_t> dedup_key);
...
base::expected<base::flat_set<std::string>, StringSetError> ExtractStringSet(base::Value::List, size_t max_string_size, size_t max_set_size);
