...
base::expected<std::optional<Registrar>, ParseError> ParsePreferredPlatform(const net::structured_headers::Dictionary& dict, bool cross_app_web_enabled)
{
...
base::expected<bool, ParseError> ParseReportHeaderErrors(const net::structured_headers::Dictionary& dict)
{
...
// static
base::expected<RegistrationInfo, RegistrationInfoError> RegistrationInfo::ParseInfo(std::string_view header, bool cross_app_web_enabled)
{
    if (header.empty()) {
        return RegistrationInfo();
    }

    base::expected<RegistrationInfo, RegistrationInfoError> info = base::unexpected(RegistrationInfoError::kRootInvalid);
...
// static
base::expected<RegistrationInfo, RegistrationInfoError> RegistrationInfo::ParseInfo(const net::structured_headers::Dictionary& dict, bool cross_app_web_enabled)
{
    ASSIGN_OR_RETURN(std::optional<Registrar> preferred_platform,
        ParsePreferredPlatform(dict, cross_app_web_enabled).transform_error([](ParseError) { return RegistrationInfoError::kInvalidPreferredPlatform; }));

    ASSIGN_OR_RETURN(
        bool report_header_errors, ParseReportHeaderErrors(dict).transform_error([](ParseError) { return RegistrationInfoError::kInvalidReportHeaderErrors; }));
