...
template <typename T>
base::expected<std::optional<T>, ParseError> ParseIntegerFromString(const base::Value::Dict& dict, std::string_view key, bool (*parse)(std::string_view, T*))
{
    ...
    T parsed_val;
    if (const std::string* str = value->GetIfString(); !str || !parse(*str, &parsed_val)) {
...
base::expected<absl::uint128, ParseError> ParseAggregationKeyPiece(const base::Value& value)
{
    ...
    if (!base::StartsWith(*str, "0x", base::CompareCase::INSENSITIVE_ASCII) || !base::HexStringToUInt128(*str, &key_piece)) {
        return base::unexpected(ParseError());
    }
...
base::expected<std::optional<uint64_t>, ParseError> ParseUint64(const base::Value::Dict& dict, std::string_view key)
{
    return ParseIntegerFromString<uint64_t>(dict, key, &base::StringToUint64);
}

base::expected<std::optional<int64_t>, ParseError> ParseInt64(const base::Value::Dict& dict, std::string_view key)
{
    return ParseIntegerFromString<int64_t>(dict, key, &base::StringToInt64);
}

base::expected<int64_t, ParseError> ParsePriority(const base::Value::Dict& dict)
{
...
base::expected<std::optional<uint64_t>, ParseError> ParseDeduplicationKey(const base::Value::Dict& dict)
{
...
template <typename T> base::expected<T, ParseError> ParseIntFromIntOrDouble(const base::Value& value)
{
    ...
    if (std::optional<int> int_value = value.GetIfInt()) {
        ...
        if (HasFractionalPart(*double_value) || !base::IsValueInRangeForNumericType<T>(*double_value)) {
            return base::unexpected(ParseError());
        }
...
base::expected<base::TimeDelta, ParseError> ParseLegacyDuration(const base::Value& value, const base::TimeDelta clamp_min, const base::TimeDelta clamp_max)
{
...
base::expected<base::TimeDelta, ParseError> ParseDuration(const base::Value& value)
{
    if (std::optional<int> int_value = value.GetIfInt()) {
...
base::expected<std::optional<SuitableOrigin>, ParseError> ParseAggregationCoordinator(const base::Value::Dict& dict)
{
    ...
    std::optional<url::Origin> aggregation_coordinator = aggregation_service::ParseAggregationCoordinator(*str);
    if (!aggregation_coordinator.has_value()) {
        return base::unexpected(ParseError());
    }
    auto aggregation_coordinator_origin = SuitableOrigin::Create(*std::move(aggregation_coordinator));
...
void SerializeUint64(base::Value::Dict& dict, std::string_view key, uint64_t value)
{
    dict.Set(key, base::NumberToString(value));
}

void SerializeInt64(base::Value::Dict& dict, std::string_view key, int64_t value)
{
    dict.Set(key, base::NumberToString(value));
}
...
void SerializeDebugKey(base::Value::Dict& dict, std::optional<uint64_t> debug_key)
{
    if (debug_key) {
...
void SerializeDeduplicationKey(base::Value::Dict& dict, std::optional<uint64_t> dedup_key)
{
    if (dedup_key) {
...
void SerializeTimeDeltaInSeconds(base::Value::Dict& dict, std::string_view key, base::TimeDelta value)
{
...
base::expected<uint32_t, ParseError> ParsePositiveUint32(const base::Value& value)
{
...
base::Value Uint32ToJson(uint32_t value)
{
    ...
    return base::IsValueInRangeForNumericType<int>(value) ? base::Value(static_cast<int>(value)) : base::Value(static_cast<double>(value));
}

base::expected<base::flat_set<std::string>, StringSetError> ExtractStringSet(base::Value::List list, const size_t max_string_size, const size_t max_set_size)
{
