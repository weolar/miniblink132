...
public:
    ...
    virtual SymKey* AsSymKey()
    {
        return nullptr;
    }
    virtual AsymKey* AsAsymKey()
    {
        return nullptr;
    }
};
...
public:
    ...
    explicit AsymKey(bssl::UniquePtr<EVP_PKEY> pkey)
        : pkey_(std::move(pkey))
    {
    }
...
blink::WebCryptoKeyHandle* CreateSymmetricKeyHandle(base::span<const uint8_t> key_bytes)
{
    return new SymKey(key_bytes);
}

blink::WebCryptoKeyHandle* CreateAsymmetricKeyHandle(bssl::UniquePtr<EVP_PKEY> pkey)
{
