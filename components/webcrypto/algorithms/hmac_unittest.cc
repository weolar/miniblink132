...
blink::WebCryptoAlgorithm CreateHmacKeyGenAlgorithm(blink::WebCryptoAlgorithmId hash_id, unsigned int key_length_bits)
{
    DCHECK(blink::WebCryptoAlgorithm::IsHash(hash_id));
    // key_length_bytes == 0 means unspecified
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(
        blink::kWebCryptoAlgorithmIdHmac, new blink::WebCryptoHmacKeyGenParams(CreateAlgorithm(hash_id), (key_length_bits != 0), key_length_bits));
}

blink::WebCryptoAlgorithm CreateHmacImportAlgorithmWithLength(blink::WebCryptoAlgorithmId hash_id, unsigned int length_bits)
{
    DCHECK(blink::WebCryptoAlgorithm::IsHash(hash_id));
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(
        blink::kWebCryptoAlgorithmIdHmac, new blink::WebCryptoHmacImportParams(CreateAlgorithm(hash_id), true, length_bits));
}

blink::WebCryptoKey GenerateHmacKey(blink::WebCryptoAlgorithmId hash, size_t key_length_bits)
{
    blink::WebCryptoKey key;
    auto status = GenerateSecretKey(CreateHmacKeyGenAlgorithm(hash, key_length_bits), true, blink::kWebCryptoKeyUsageSign, &key);
...
const HmacKnownAnswer kHmacKnownAnswers[] = {
    ...
    { blink::kWebCryptoAlgorithmIdSha1, "00", "", "fbdb1d1b18aa6c08324b7d64b71fb76370690e1d" },
    { blink::kWebCryptoAlgorithmIdSha256, "00", "", "b613679a0814d9ec772f95d778c35fc5ff1697c493715653c6c712144292c5ad" },
...
blink::WebCryptoKey HmacKeyFromHexBytes(blink::WebCryptoAlgorithmId hash, const char* key)
{
    return ImportSecretKeyFromRaw(
        HexStringToBytes(key), CreateHmacImportAlgorithmNoLength(hash), blink::kWebCryptoKeyUsageSign | blink::kWebCryptoKeyUsageVerify);
}
...
std::vector<uint8_t> HmacSign(blink::WebCryptoKey key, const std::vector<uint8_t>& message)
{
    std::vector<uint8_t> output;
    auto status = Sign(CreateAlgorithm(blink::kWebCryptoAlgorithmIdHmac), key, message, &output);
    CHECK(status == Status::Success());
    return output;
}

bool HmacVerify(blink::WebCryptoKey key, const std::vector<uint8_t>& message, const std::vector<uint8_t>& hmac)
{
    bool match = false;
    auto status = Verify(CreateAlgorithm(blink::kWebCryptoAlgorithmIdHmac), key, hmac, message, &match);
    CHECK(status == Status::Success());
    return match;
}
...
TEST_F(WebCryptoHmacTest, KnownAnswers)
{
    for (const auto& test : kHmacKnownAnswers) {
        ...
        std::vector<uint8_t> truncated_hmac(expected_hmac.begin(), expected_hmac.end() - 1);
...
TEST_F(WebCryptoHmacTest, GeneratedKeysHaveExpectedProperties)
{
    ...
    EXPECT_EQ(blink::kWebCryptoAlgorithmIdSha1, key.Algorithm().HmacParams()->GetHash().Id());
...
TEST_F(WebCryptoHmacTest, GeneratedKeysAreRandomIsh)
{
    ...
    for (int i = 0; i < 16; ++i) {
        std::vector<uint8_t> key_bytes = BytesFromHmacKey(GenerateHmacKey(blink::kWebCryptoAlgorithmIdSha1, 512));
...
{
    std::vector<uint8_t> key_bytes = BytesFromHmacKey(GenerateHmacKey(blink::kWebCryptoAlgorithmIdSha1, 1));
...
{
    blink::WebCryptoKey key;
    blink::WebCryptoAlgorithm algorithm = CreateHmacKeyGenAlgorithm(blink::kWebCryptoAlgorithmIdSha512, 0);
    ASSERT_EQ(Status::ErrorCreateKeyEmptyUsages(), GenerateSecretKey(algorithm, true, 0, &key));
}
...
TEST_F(WebCryptoHmacTest, ImportKeyEmptyUsage)
{
    ...
    EXPECT_EQ(Status::ErrorCreateKeyEmptyUsages(),
        ImportKey(blink::kWebCryptoKeyFormatRaw, HexStringToBytes(key_raw_hex_in), CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha1), true, 0,
            &key));
}
...
TEST_F(WebCryptoHmacTest, ImportKeyJwkKeyOpsSignVerify)
{
    ...
    EXPECT_EQ(Status::Success(),
        ImportKeyJwkFromDict(dict, CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha256), false, blink::kWebCryptoKeyUsageSign, &key));
    ...
    EXPECT_EQ(Status::Success(),
        ImportKeyJwkFromDict(dict, CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha256), false, blink::kWebCryptoKeyUsageVerify, &key));
...
TEST_F(WebCryptoHmacTest, ImportKeyJwkUseInconsisteWithKeyOps)
{
    ...
    EXPECT_EQ(Status::ErrorJwkUseAndKeyopsInconsistent(),
        ImportKeyJwkFromDict(dict, CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha256), false,
            blink::kWebCryptoKeyUsageSign | blink::kWebCryptoKeyUsageVerify, &key));
}
...
TEST_F(WebCryptoHmacTest, ImportKeyJwkUseSig)
{
    ...
    EXPECT_EQ(blink::kWebCryptoKeyUsageSign | blink::kWebCryptoKeyUsageVerify, key.Usages());
}
...
TEST_F(WebCryptoHmacTest, ImportJwkInputConsistency)
{
    ...
    blink::WebCryptoAlgorithm algorithm = CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha256);
    ...
    EXPECT_EQ(Status::Success(), ImportKey(blink::kWebCryptoKeyFormatJwk, json_vec, algorithm, extractable, usages, &key));
    EXPECT_TRUE(key.Handle());
    EXPECT_EQ(blink::kWebCryptoKeyTypeSecret, key.GetType());
    EXPECT_EQ(extractable, key.Extractable());
    EXPECT_EQ(blink::kWebCryptoAlgorithmIdHmac, key.Algorithm().Id());
    EXPECT_EQ(blink::kWebCryptoAlgorithmIdSha256, key.Algorithm().HmacParams()->GetHash().Id());
    ...
    EXPECT_EQ(Status::ErrorJwkExtInconsistent(), ImportKey(blink::kWebCryptoKeyFormatJwk, json_vec, algorithm, true, usages, &key));
    EXPECT_EQ(Status::Success(), ImportKey(blink::kWebCryptoKeyFormatJwk, json_vec, algorithm, false, usages, &key));
    EXPECT_FALSE(key.Extractable());
    dict.Set("ext", true);
    EXPECT_EQ(Status::Success(), ImportKeyJwkFromDict(dict, algorithm, true, usages, &key));
    EXPECT_TRUE(key.Extractable());
    EXPECT_EQ(Status::Success(), ImportKeyJwkFromDict(dict, algorithm, false, usages, &key));
    EXPECT_FALSE(key.Extractable());
    ...
    EXPECT_EQ(Status::ErrorJwkAlgorithmInconsistent(),
        ImportKeyJwkFromDict(dict, CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesCbc), extractable, blink::kWebCryptoKeyUsageEncrypt, &key));
    // Fail: Input usage (encrypt) is inconsistent with JWK value (use=sig).
    EXPECT_EQ(Status::ErrorJwkUseInconsistent(),
        ImportKey(
            blink::kWebCryptoKeyFormatJwk, json_vec, CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesCbc), extractable, blink::kWebCryptoKeyUsageEncrypt, &key));

    // Fail: Input algorithm (HMAC SHA1) is inconsistent with JWK value
    // (HMAC SHA256).
    EXPECT_EQ(Status::ErrorJwkAlgorithmInconsistent(),
        ImportKey(blink::kWebCryptoKeyFormatJwk, json_vec, CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha1), extractable, usages, &key));

    // Pass: JWK alg missing but input algorithm specified: use input value
    dict.Remove("alg");
    EXPECT_EQ(Status::Success(), ImportKeyJwkFromDict(dict, CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha256), extractable, usages, &key));
    ...
    EXPECT_EQ(
        Status::ErrorCreateKeyBadUsages(), ImportKey(blink::kWebCryptoKeyFormatJwk, json_vec, algorithm, extractable, blink::kWebCryptoKeyUsageEncrypt, &key));
    ...
    usages = blink::kWebCryptoKeyUsageEncrypt | blink::kWebCryptoKeyUsageSign | blink::kWebCryptoKeyUsageVerify;
    EXPECT_EQ(Status::ErrorCreateKeyBadUsages(), ImportKey(blink::kWebCryptoKeyFormatJwk, json_vec, algorithm, extractable, usages, &key));
...
TEST_F(WebCryptoHmacTest, ImportJwkHappy)
{
    ...
    ASSERT_EQ(Status::Success(), ImportKeyJwkFromDict(dict, algorithm, extractable, usages, &key));

    EXPECT_EQ(blink::kWebCryptoAlgorithmIdSha256, key.Algorithm().HmacParams()->GetHash().Id());

    const std::vector<uint8_t> message_raw = HexStringToBytes("b1689c2591eaf3c9e66070f8a77954ffb81749f1b00346f9dfe0b2ee905dcc288baf4a"
    ...
    ASSERT_EQ(Status::Success(), Sign(CreateAlgorithm(blink::kWebCryptoAlgorithmIdHmac), key, message_raw, &output));

    const std::string mac_raw = "769f00d3e6a6cc1fb426a14a4f76c6462e6149726e0dee0ec0cf97a16605ac8b";
...
TEST_F(WebCryptoHmacTest, ImportExportJwk)
{
    ...
    ImportExportJwkSymmetricKey(
        256, CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha1), blink::kWebCryptoKeyUsageSign | blink::kWebCryptoKeyUsageVerify, "HS1");

    // HMAC SHA-384
    ImportExportJwkSymmetricKey(384, CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha384), blink::kWebCryptoKeyUsageSign, "HS384");

    // HMAC SHA-512
    ImportExportJwkSymmetricKey(512, CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha512), blink::kWebCryptoKeyUsageVerify, "HS512");
}
...
TEST_F(WebCryptoHmacTest, ExportJwkEmptyKey)
{
    ...
    ASSERT_TRUE(DeserializeKeyForClone(
        blink::WebCryptoKeyAlgorithm::CreateHmac(blink::kWebCryptoAlgorithmIdSha1, 0), blink::kWebCryptoKeyTypeSecret, true, usages, {}, &key));

    // Export the key in JWK format and validate.
    std::vector<uint8_t> json;
    ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatJwk, key, &json));
    EXPECT_TRUE(VerifySecretJwk(json, "HS1", "", usages));

    // Now try re-importing the JWK key.
    key = blink::WebCryptoKey::CreateNull();
    EXPECT_EQ(Status::ErrorHmacImportEmptyKey(),
        ImportKey(blink::kWebCryptoKeyFormatJwk, json, CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha1), true, usages, &key));
}

// Imports an HMAC key contaning no byte data.
TEST_F(WebCryptoHmacTest, ImportRawEmptyKey)
{
    const blink::WebCryptoAlgorithm import_algorithm = CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha1);

    blink::WebCryptoKeyUsageMask usages = blink::kWebCryptoKeyUsageSign;
    blink::WebCryptoKey key;

    ASSERT_EQ(Status::ErrorHmacImportEmptyKey(), ImportKey(blink::kWebCryptoKeyFormatRaw, {}, import_algorithm, true, usages, &key));
}
...
{
    const blink::WebCryptoAlgorithm import_algorithm = CreateHmacImportAlgorithm(blink::kWebCryptoAlgorithmIdSha1, 0);

    blink::WebCryptoKeyUsageMask usages = blink::kWebCryptoKeyUsageSign;
    blink::WebCryptoKey key;

    std::vector<uint8_t> key_data(1);
    ASSERT_EQ(Status::ErrorHmacImportBadLength(), ImportKey(blink::kWebCryptoKeyFormatRaw, key_data, import_algorithm, true, usages, &key));
}
...
TEST_F(WebCryptoHmacTest, ImportRawKeyTooLarge)
{
    ...
    base::span<const uint8_t> big_data(static_cast<const uint8_t*>(invalid_data), UINT_MAX);

    blink::WebCryptoKey key;
    EXPECT_EQ(Status::ErrorDataTooLarge(),
        ImportKey(blink::kWebCryptoKeyFormatRaw, big_data, CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha1), true,
...
TEST_F(WebCryptoHmacTest, ImportRawKeyLengthTooLarge)
{
    ...
    EXPECT_EQ(Status::ErrorHmacImportBadLength(),
        ImportKey(blink::kWebCryptoKeyFormatRaw, std::vector<uint8_t>(15), CreateHmacImportAlgorithmWithLength(blink::kWebCryptoAlgorithmIdSha1, 128), true,
            blink::kWebCryptoKeyUsageSign, &key));
}
...
TEST_F(WebCryptoHmacTest, ImportRawKeyLengthTooSmall)
{
    blink::WebCryptoKey key;
    EXPECT_EQ(Status::ErrorHmacImportBadLength(),
        ImportKey(blink::kWebCryptoKeyFormatRaw, std::vector<uint8_t>(16), CreateHmacImportAlgorithmWithLength(blink::kWebCryptoAlgorithmIdSha1, 120), true,
            blink::kWebCryptoKeyUsageSign, &key));
}
...
TEST_F(WebCryptoHmacTest, ImportRawKeyTruncation)
{
    ...
    blink::WebCryptoKey key;
    EXPECT_EQ(Status::Success(),
        ImportKey(blink::kWebCryptoKeyFormatRaw, data, CreateHmacImportAlgorithmWithLength(blink::kWebCryptoAlgorithmIdSha1, 12), true,
            blink::kWebCryptoKeyUsageSign, &key));
...
TEST_F(WebCryptoHmacTest, ImportJwkKeyTruncation)
{
    ...
    blink::WebCryptoKey key;
    EXPECT_EQ(Status::Success(),
        ImportKeyJwkFromDict(dict, CreateHmacImportAlgorithmWithLength(blink::kWebCryptoAlgorithmIdSha1, 12), true, blink::kWebCryptoKeyUsageSign, &key));
