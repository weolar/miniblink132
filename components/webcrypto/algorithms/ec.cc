...
Status ReadJwkCrv(const JwkReader& jwk, blink::WebCryptoNamedCurve* named_curve)
{
...
Status WebCryptoCurveToJwkCrv(blink::WebCryptoNamedCurve named_curve, std::string* jwk_crv)
{
    for (const auto& mapping : kJwkCrvMappings) {
...
Status VerifyEcKeyAfterSpkiOrPkcs8Import(EVP_PKEY* pkey, blink::WebCryptoNamedCurve expected_named_curve)
{
...
Status CreateEC_KEY(blink::WebCryptoNamedCurve named_curve, bssl::UniquePtr<EC_KEY>* ec)
{
    int curve_nid = 0;
...
Status ReadPaddedBIGNUM(const JwkReader& jwk, const std::string& member_name, size_t expected_length, bssl::UniquePtr<BIGNUM>* out)
{
...
Status GetPublicKey(EC_KEY* ec, bssl::UniquePtr<BIGNUM>* x, bssl::UniquePtr<BIGNUM>* y)
{
    const EC_GROUP* group = EC_KEY_get0_group(ec);
...
{
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(algorithm.Id(), new blink::WebCryptoEcKeyImportParams(algorithm.EcParams()->NamedCurve()));
}

} // namespace

Status EcAlgorithm::GenerateKey(
    const blink::WebCryptoAlgorithm& algorithm, bool extractable, blink::WebCryptoKeyUsageMask combined_usages, GenerateKeyResult* result) const
{
    ...
    if (!private_pkey || !EVP_PKEY_set1_EC_KEY(private_pkey.get(), ec_private_key.get())) {
        return Status::OperationError();
    }
    ...
    if (status.IsError())
        return status;
    if (!EC_KEY_set_public_key(ec_public_key.get(), EC_KEY_get0_public_key(ec_private_key.get()))) {
        return Status::OperationError();
    }
    if (!public_pkey || !EVP_PKEY_set1_EC_KEY(public_pkey.get(), ec_public_key.get())) {
        return Status::OperationError();
    }
    ...
    blink::WebCryptoKeyAlgorithm key_algorithm = blink::WebCryptoKeyAlgorithm::CreateEc(algorithm.Id(), params->NamedCurve());

    // Note that extractable is unconditionally set to true. This is because per
    // the WebCrypto spec generated public keys are always extractable.
    status = CreateWebCryptoPublicKey(std::move(public_pkey), key_algorithm, true, public_usages, &public_key);
    if (status.IsError())
        return status;

    status = CreateWebCryptoPrivateKey(std::move(private_pkey), key_algorithm, extractable, private_usages, &private_key);
    if (status.IsError())
        return status;
...
Status EcAlgorithm::ImportKey(blink::WebCryptoKeyFormat format, base::span<const uint8_t> key_data, const blink::WebCryptoAlgorithm& algorithm,
...
Status EcAlgorithm::ExportKey(blink::WebCryptoKeyFormat format, const blink::WebCryptoKey& key, std::vector<uint8_t>* buffer) const
{
    switch (format) {
    case blink::kWebCryptoKeyFormatRaw:
...
Status EcAlgorithm::ImportKeyRaw(base::span<const uint8_t> key_data, const blink::WebCryptoAlgorithm& algorithm, bool extractable,
    blink::WebCryptoKeyUsageMask usages, blink::WebCryptoKey* key) const
{
    ...
    const blink::WebCryptoEcKeyImportParams* params = algorithm.EcKeyImportParams();
    ...
    if (!EC_POINT_oct2point(EC_KEY_get0_group(ec.get()), point.get(), key_data.data(), key_data.size(), nullptr)) {
    ...
    return CreateWebCryptoPublicKey(std::move(pkey), key_algorithm, extractable, usages, key);
}

Status EcAlgorithm::ImportKeyPkcs8(base::span<const uint8_t> key_data, const blink::WebCryptoAlgorithm& algorithm, bool extractable,
    blink::WebCryptoKeyUsageMask usages, blink::WebCryptoKey* key) const
{
    ...
    status = VerifyEcKeyAfterSpkiOrPkcs8Import(private_key.get(), params->NamedCurve());
    if (status.IsError())
        return status;

    return CreateWebCryptoPrivateKey(
        std::move(private_key), blink::WebCryptoKeyAlgorithm::CreateEc(algorithm.Id(), params->NamedCurve()), extractable, usages, key);
}

Status EcAlgorithm::ImportKeySpki(base::span<const uint8_t> key_data, const blink::WebCryptoAlgorithm& algorithm, bool extractable,
    blink::WebCryptoKeyUsageMask usages, blink::WebCryptoKey* key) const
{
    Status status = CheckKeyCreationUsages(all_public_key_usages_, usages);
    if (status.IsError())
    ...
    status = VerifyEcKeyAfterSpkiOrPkcs8Import(public_key.get(), params->NamedCurve());
    if (status.IsError())
        return status;

    return CreateWebCryptoPublicKey(
        std::move(public_key), blink::WebCryptoKeyAlgorithm::CreateEc(algorithm.Id(), params->NamedCurve()), extractable, usages, key);
}

// The format for JWK EC keys is given by:
// https://tools.ietf.org/html/draft-ietf-jose-json-web-algorithms-36#section-6.2
Status EcAlgorithm::ImportKeyJwk(base::span<const uint8_t> key_data, const blink::WebCryptoAlgorithm& algorithm, bool extractable,
    blink::WebCryptoKeyUsageMask usages, blink::WebCryptoKey* key) const
{
    crypto::OpenSSLErrStackTracer err_tracer(FROM_HERE);
    ...
    JwkReader jwk;
    Status status = jwk.Init(key_data, extractable, usages, "EC", GetJwkAlgorithm(params->NamedCurve()));
    if (status.IsError())
        return status;
    ...
    // Wrap the EVP_PKEY into a WebCryptoKey
    if (is_private_key) {
        return CreateWebCryptoPrivateKey(std::move(pkey), key_algorithm, extractable, usages, key);
    }
    return CreateWebCryptoPublicKey(std::move(pkey), key_algorithm, extractable, usages, key);
}
...
Status EcAlgorithm::ExportKeySpki(const blink::WebCryptoKey& key, std::vector<uint8_t>* buffer) const
{
    if (key.GetType() != blink::kWebCryptoKeyTypePublic)
        return Status::ErrorUnexpectedKeyType();
...
// The format for JWK EC keys is given by:
// https://tools.ietf.org/html/draft-ietf-jose-json-web-algorithms-36#section-6.2
Status EcAlgorithm::ExportKeyJwk(const blink::WebCryptoKey& key, std::vector<uint8_t>* buffer) const
{
    crypto::OpenSSLErrStackTracer err_tracer(FROM_HERE);
    ...
    // Set the crv
    std::string crv;
    Status status = WebCryptoCurveToJwkCrv(key.Algorithm().EcParams()->NamedCurve(), &crv);
    if (status.IsError())
        return status;
...
Status EcAlgorithm::DeserializeKeyForClone(const blink::WebCryptoKeyAlgorithm& algorithm, blink::WebCryptoKeyType type, bool extractable,
    blink::WebCryptoKeyUsageMask usages, base::span<const uint8_t> key_data, blink::WebCryptoKey* key) const
{
    ...
    if (algorithm.EcParams()->NamedCurve() != key->Algorithm().EcParams()->NamedCurve()) {
        return Status::ErrorUnexpected();
    }
