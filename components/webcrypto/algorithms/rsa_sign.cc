...
Status GetPKeyAndDigest(const blink::WebCryptoKey& key, EVP_PKEY** pkey, const EVP_MD** digest)
{
...
Status ApplyRsaPssOptions(const blink::WebCryptoKey& key, const EVP_MD* const mgf_digest, unsigned int salt_length_bytes, EVP_PKEY_CTX* pctx)
{
    ...
    if (key.Algorithm().Id() != blink::kWebCryptoAlgorithmIdRsaPss) {
        ...
        DCHECK_EQ(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, key.Algorithm().Id());
        return Status::Success();
    }
    ...
    if (1 != EVP_PKEY_CTX_set_rsa_padding(pctx, RSA_PKCS1_PSS_PADDING) || 1 != EVP_PKEY_CTX_set_rsa_mgf1_md(pctx, mgf_digest)
        || 1 != EVP_PKEY_CTX_set_rsa_pss_saltlen(pctx, salt_length_bytes_int.ValueOrDie())) {
        return Status::OperationError();
    }
...
Status RsaSign(const blink::WebCryptoKey& key, unsigned int pss_salt_length_bytes, base::span<const uint8_t> data, std::vector<uint8_t>* buffer)
{
    ...
    if (!EVP_DigestSign(ctx.get(), buffer->data(), &sig_len, data.data(), data.size())) {
        return Status::OperationError();
    }
...
Status RsaVerify(const blink::WebCryptoKey& key, unsigned int pss_salt_length_bytes, base::span<const uint8_t> signature, base::span<const uint8_t> data,
    bool* signature_match)
{
    ...
    *signature_match = 1 == EVP_DigestVerify(ctx.get(), signature.data(), signature.size(), data.data(), data.size());
    return Status::Success();
}
