...
Status GetPKeyAndDigest(const blink::WebCryptoAlgorithm& algorithm, const blink::WebCryptoKey& key, EVP_PKEY** pkey, const EVP_MD** digest)
{
...
public:
    EcdsaImplementation()
        : EcAlgorithm(blink::kWebCryptoKeyUsageVerify, blink::kWebCryptoKeyUsageSign)
    {
    }

    const char* GetJwkAlgorithm(const blink::WebCryptoNamedCurve curve) const override
    {
        switch (curve) {
        ...
        const blink::WebCryptoAlgorithm& algorithm, const blink::WebCryptoKey& key, base::span<const uint8_t> data, std::vector<uint8_t>* buffer) const override
    {
        ...
        size_t sig_len = 0;
        if (!EVP_DigestSignInit(ctx.get(), nullptr, digest, nullptr, private_key) || !EVP_DigestSign(ctx.get(), nullptr, &sig_len, data.data(), data.size())) {
            return Status::OperationError();
        }
        ...
        base::span<const uint8_t> data, bool* signature_match) const override
    {
        ...
        status = ConvertWebCryptoSignatureToDerSignature(public_key, signature, &der_signature, &incorrect_length_signature);
        if (status.IsError())
            return status;
        ...
        *signature_match = 1 == EVP_DigestVerify(ctx.get(), der_signature.data(), der_signature.size(), data.data(), data.size());
        return Status::Success();
    }
};
