...
blink::WebCryptoAlgorithm CreateRsaOaepAlgorithm(const std::vector<uint8_t>& label)
{
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(blink::kWebCryptoAlgorithmIdRsaOaep, new blink::WebCryptoRsaOaepParams(!label.empty(), label));
}
...
std::string Base64EncodeUrlSafe(const std::vector<uint8_t>& input)
{
    ...
    base::Base64UrlEncode(
        std::string_view(reinterpret_cast<const char*>(input.data()), input.size()), base::Base64UrlEncodePolicy::OMIT_PADDING, &base64url_encoded);
    return base64url_encoded;
}

base::Value::Dict CreatePublicKeyJwkDict(base::flat_map<std::string, std::string> extra_properties)
{
    base::Value::Dict jwk;
    jwk.Set("kty", "RSA");
...
TEST_F(WebCryptoRsaOaepTest, ImportPkcs8WithRsaEncryption)
{
    ...
    ASSERT_EQ(Status::Success(),
        ImportKey(blink::kWebCryptoKeyFormatPkcs8, HexStringToBytes(kPrivateKeyPkcs8DerHex),
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaOaep, blink::kWebCryptoAlgorithmIdSha1), true, blink::kWebCryptoKeyUsageDecrypt,
            &private_key));
}
...
TEST_F(WebCryptoRsaOaepTest, ImportPublicJwkWithNoAlg)
{
    ...
    ASSERT_EQ(Status::Success(),
        ImportKeyJwkFromDict(CreatePublicKeyJwkDict({}), CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaOaep, blink::kWebCryptoAlgorithmIdSha1),
...
TEST_F(WebCryptoRsaOaepTest, ImportPublicJwkWithMatchingAlg)
{
    blink::WebCryptoKey public_key;
    ASSERT_EQ(Status::Success(),
        ImportKeyJwkFromDict(CreatePublicKeyJwkDict({ { "alg", "RSA-OAEP" } }),
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaOaep, blink::kWebCryptoAlgorithmIdSha1), true, blink::kWebCryptoKeyUsageEncrypt,
            &public_key));
}
...
TEST_F(WebCryptoRsaOaepTest, ImportPublicJwkWithMismatchedAlgFails)
{
    ...
    ASSERT_EQ(Status::ErrorJwkAlgorithmInconsistent(),
        ImportKeyJwkFromDict(CreatePublicKeyJwkDict({ { "alg", "RSA-OAEP-512" } }),
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaOaep, blink::kWebCryptoAlgorithmIdSha1), true, blink::kWebCryptoKeyUsageEncrypt,
            &public_key));
}
...
TEST_F(WebCryptoRsaOaepTest, ImportPublicJwkWithMismatchedTypeFails)
{
    ...
    ASSERT_EQ(Status::ErrorJwkUnexpectedKty("RSA"),
        ImportKeyJwkFromDict(CreatePublicKeyJwkDict({ { "kty", "oct" }, { "alg", "RSA-OAEP" } }),
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaOaep, blink::kWebCryptoAlgorithmIdSha1), true, blink::kWebCryptoKeyUsageEncrypt,
            &public_key));
}
...
TEST_F(WebCryptoRsaOaepTest, ExportPublicJwk)
{
    ...
    } kTestData[] = { { blink::kWebCryptoAlgorithmIdSha1, "RSA-OAEP" }, { blink::kWebCryptoAlgorithmIdSha256, "RSA-OAEP-256" },
        { blink::kWebCryptoAlgorithmIdSha384, "RSA-OAEP-384" }, { blink::kWebCryptoAlgorithmIdSha512, "RSA-OAEP-512" } };
    for (const auto& test_data : kTestData) {
        ...
        blink::WebCryptoKey public_key;
        ASSERT_EQ(Status::Success(),
            ImportKeyJwkFromDict(CreatePublicKeyJwkDict({ { "alg", test_data.expected_jwk_alg } }),
                CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaOaep, test_data.hash_alg), true, blink::kWebCryptoKeyUsageEncrypt, &public_key));

        // Now export the key as JWK and verify its contents
        std::vector<uint8_t> jwk_data;
        ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatJwk, public_key, &jwk_data));
        EXPECT_TRUE(VerifyPublicJwk(jwk_data, test_data.expected_jwk_alg, kPublicKeyModulusHex, kPublicKeyExponentHex, blink::kWebCryptoKeyUsageEncrypt));
    }
}
...
TEST_F(WebCryptoRsaOaepTest, EncryptDecryptKnownAnswerTest)
{
    for (const auto& test : kRsaOaepKnownAnswers) {
        ...
        blink::WebCryptoAlgorithm import_algorithm = CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaOaep, test.hash);
        blink::WebCryptoKey public_key;
        blink::WebCryptoKey private_key;

        ASSERT_NO_FATAL_FAILURE(ImportRsaKeyPair(public_key_der, private_key_der, import_algorithm, false, blink::kWebCryptoKeyUsageEncrypt,
            blink::kWebCryptoKeyUsageDecrypt, &public_key, &private_key));

        blink::WebCryptoAlgorithm op_algorithm = CreateRsaOaepAlgorithm(label);
        std::vector<uint8_t> decrypted_data;
        ASSERT_EQ(Status::Success(), Decrypt(op_algorithm, private_key, ciphertext, &decrypted_data));
        EXPECT_BYTES_EQ(plaintext, decrypted_data);
        std::vector<uint8_t> encrypted_data;
        ASSERT_EQ(Status::Success(), Encrypt(op_algorithm, public_key, plaintext, &encrypted_data));
        std::vector<uint8_t> redecrypted_data;
        ASSERT_EQ(Status::Success(), Decrypt(op_algorithm, private_key, encrypted_data, &redecrypted_data));
...
TEST_F(WebCryptoRsaOaepTest, EncryptWithLargeMessageFails)
{
    ...
    blink::WebCryptoKey public_key;
    ASSERT_EQ(Status::Success(),
        ImportKeyJwkFromDict(CreatePublicKeyJwkDict({}), CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaOaep, kHash), true,
            blink::kWebCryptoKeyUsageEncrypt, &public_key));
    ...
    ASSERT_EQ(Status::OperationError(), Encrypt(op_algorithm, public_key, large_message, &ciphertext));
}
...
TEST_F(WebCryptoRsaOaepTest, EncryptWithLargeDigestFails)
{
    ...
    ASSERT_EQ(Status::OperationError(), Encrypt(op_algorithm, public_key, small_message, &ciphertext));
}
...
{
    blink::WebCryptoAlgorithm import_algorithm = CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaOaep, blink::kWebCryptoAlgorithmIdSha1);
    blink::WebCryptoKey public_key;
    blink::WebCryptoKey private_key;
    ...
    ASSERT_NO_FATAL_FAILURE(ImportRsaKeyPair(HexStringToBytes(kPublicKeySpkiDerHex), HexStringToBytes(kPrivateKeyPkcs8DerHex), import_algorithm, false,
        blink::kWebCryptoKeyUsageEncrypt | blink::kWebCryptoKeyUsageWrapKey, blink::kWebCryptoKeyUsageDecrypt | blink::kWebCryptoKeyUsageUnwrapKey, &public_key,
        &private_key));
    ...
    blink::WebCryptoKey key = ImportSecretKeyFromRaw(HexStringToBytes(key_hex), key_algorithm, blink::kWebCryptoKeyUsageEncrypt);
    ASSERT_FALSE(key.IsNull());

    std::vector<uint8_t> wrapped_key;
    ASSERT_EQ(Status::Success(), WrapKey(blink::kWebCryptoKeyFormatRaw, key, public_key, wrapping_algorithm, &wrapped_key));
    ...
    ASSERT_EQ(Status::Success(), Decrypt(wrapping_algorithm, private_key, wrapped_key, &decrypted_key));
    EXPECT_BYTES_EQ_HEX(key_hex, decrypted_key);

    // Now attempt to unwrap the key, which should also decrypt the data.
    blink::WebCryptoKey unwrapped_key;
    ASSERT_EQ(Status::Success(),
        UnwrapKey(blink::kWebCryptoKeyFormatRaw, wrapped_key, private_key, wrapping_algorithm, key_algorithm, true, blink::kWebCryptoKeyUsageEncrypt,
            &unwrapped_key));
    ASSERT_FALSE(unwrapped_key.IsNull());

    std::vector<uint8_t> raw_key;
    ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatRaw, unwrapped_key, &raw_key));
...
TEST_F(WebCryptoRsaOaepTest, WrapUnwrapJwkSymKey)
{
    ...
    // id-rsaEncryption OID
    const char kPublicKey2048SpkiDerHex[] = "30820122300d06092a864886f70d01010105000382010f003082010a0282010100c5d8ce"
                                            ...
                                            "5d0203010001";
    const char kPrivateKey2048Pkcs8DerHex[] = "308204bd020100300d06092a864886f70d0101010500048204a7308204a3020100028201"
    ...
    ASSERT_NO_FATAL_FAILURE(ImportRsaKeyPair(HexStringToBytes(kPublicKey2048SpkiDerHex), HexStringToBytes(kPrivateKey2048Pkcs8DerHex), import_algorithm, false,
        blink::kWebCryptoKeyUsageEncrypt | blink::kWebCryptoKeyUsageWrapKey, blink::kWebCryptoKeyUsageDecrypt | blink::kWebCryptoKeyUsageUnwrapKey, &public_key,
        &private_key));
    ...
    std::vector<uint8_t> wrapped_key;
    ASSERT_EQ(Status::Success(), WrapKey(blink::kWebCryptoKeyFormatJwk, key, public_key, wrapping_algorithm, &wrapped_key));

    // Verify that |wrapped_key| can be decrypted and yields a valid JWK object.
    // Because |private_key| supports both decrypt and unwrap, this is valid.
    std::vector<uint8_t> decrypted_jwk;
    ASSERT_EQ(Status::Success(), Decrypt(wrapping_algorithm, private_key, wrapped_key, &decrypted_jwk));
    EXPECT_TRUE(VerifySecretJwk(decrypted_jwk, "A128CBC", key_hex, blink::kWebCryptoKeyUsageEncrypt));

    // Now attempt to unwrap the key, which should also decrypt the data.
    blink::WebCryptoKey unwrapped_key;
    ASSERT_EQ(Status::Success(),
        UnwrapKey(blink::kWebCryptoKeyFormatJwk, wrapped_key, private_key, wrapping_algorithm, key_algorithm, true, blink::kWebCryptoKeyUsageEncrypt,
            &unwrapped_key));
    ASSERT_FALSE(unwrapped_key.IsNull());
...
TEST_F(WebCryptoRsaOaepTest, ImportExportJwkRsaPublicKey)
{
    ...
    const TestCase kTests[] = { { blink::kWebCryptoAlgorithmIdSha1, blink::kWebCryptoKeyUsageEncrypt, "RSA-OAEP" },
        { blink::kWebCryptoAlgorithmIdSha256, blink::kWebCryptoKeyUsageEncrypt, "RSA-OAEP-256" },
        { blink::kWebCryptoAlgorithmIdSha384, blink::kWebCryptoKeyUsageEncrypt, "RSA-OAEP-384" },
        { blink::kWebCryptoAlgorithmIdSha512, blink::kWebCryptoKeyUsageEncrypt, "RSA-OAEP-512" } };

    for (const auto& test : kTests) {
        SCOPED_TRACE(&test - &kTests[0]);

        const blink::WebCryptoAlgorithm import_algorithm = CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaOaep, test.hash);
