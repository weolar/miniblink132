...
#include "third_party/openssl/openssl/include/openssl/ecdh.h"
...
public:
    EcdhImplementation()
        : EcAlgorithm(0, blink::kWebCryptoKeyUsageDeriveKey | blink::kWebCryptoKeyUsageDeriveBits)
    {
    }
        ...
        unsigned int optional_length_bits, std::vector<uint8_t>* derived_bytes) const override
    {
        ...
        const blink::WebCryptoKey& public_key = algorithm.EcdhKeyDeriveParams()->PublicKey();
        ...
        // curves must match.
        if (public_key.Algorithm().EcParams()->NamedCurve() != base_key.Algorithm().EcParams()->NamedCurve()) {
            return Status::ErrorEcdhCurveMismatch();
        }

        EC_KEY* public_key_ec = (EC_KEY*)EVP_PKEY_get0_EC_KEY(GetEVP_PKEY(public_key));

        const EC_POINT* public_key_point = EC_KEY_get0_public_key(public_key_ec);

        EC_KEY* private_key_ec = (EC_KEY*)EVP_PKEY_get0_EC_KEY(GetEVP_PKEY(base_key));
        ...
        int field_size_bytes = NumBitsToBytes(EC_GROUP_get_degree(EC_KEY_get0_group(private_key_ec)));

        // If a desired key length was not specified, default to the field size
        // (rounded up to nearest byte).
        unsigned int length_bits = has_optional_length_bits ? optional_length_bits : field_size_bytes * 8;
        ...
        int result = ECDH_compute_key(derived_bytes->data(), derived_bytes->size(), public_key_point, private_key_ec, nullptr);
