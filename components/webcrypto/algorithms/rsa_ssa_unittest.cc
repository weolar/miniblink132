...
{
    return CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256);
}
...
blink::WebCryptoKey ImportJwkRS256OrDie(std::string_view jwk)
{
    ...
    Status status = ImportKey(blink::kWebCryptoKeyFormatJwk, jwk_bytes, RS256Algorithm(), true, blink::kWebCryptoKeyUsageSign, &key);
...
{
    blink::WebCryptoKey key;
    Status status = ImportKeyJwkFromDict(jwk, RS256Algorithm(), false, blink::kWebCryptoKeyUsageSign, &key);
    CHECK(status.IsSuccess()) << StatusToString(status);
    return key;
}
...
{
    blink::WebCryptoKey key;
    return ImportKeyJwkFromDict(jwk, RS256Algorithm(), false, blink::kWebCryptoKeyUsageSign, &key);
}
...
Status ImportSpkiRS256MustFail(std::string_view spki)
{
    ...
    Status status = ImportKey(blink::kWebCryptoKeyFormatSpki, spki_bytes, RS256Algorithm(), true, blink::kWebCryptoKeyUsageVerify, &key);
    CHECK(!status.IsSuccess());
    return status;
}
...
Status ImportPkcs8RS256MustFail(std::string_view pkcs8)
{
    ...
    blink::WebCryptoKey key;
    Status status = ImportKey(blink::kWebCryptoKeyFormatPkcs8, pkcs8_bytes, RS256Algorithm(), true, blink::kWebCryptoKeyUsageSign, &key);
    CHECK(!status.IsSuccess());
    return status;
}
...
TEST_F(WebCryptoRsaSsaTest, ImportExportSpki)
{
    ...
    ASSERT_EQ(Status::Success(),
        ImportKey(blink::kWebCryptoKeyFormatSpki, HexStringToBytes(kPublicKeySpkiDerHex),
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256), true,
    ...
    EXPECT_EQ(kModulusLengthBits, key.Algorithm().RsaHashedParams()->ModulusLengthBits());
    EXPECT_BYTES_EQ_HEX("010001", key.Algorithm().RsaHashedParams()->PublicExponent());

    // Failing case: Import RSA key but provide an inconsistent input algorithm.
    EXPECT_EQ(Status::ErrorUnsupportedImportKeyFormat(),
        ImportKey(blink::kWebCryptoKeyFormatSpki, HexStringToBytes(kPublicKeySpkiDerHex), CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesCbc), true,
            blink::kWebCryptoKeyUsageEncrypt, &key));

    // Passing case: Export a previously imported RSA public key in SPKI format
    // and compare to original data.
    std::vector<uint8_t> output;
    ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatSpki, key, &output));
    EXPECT_BYTES_EQ_HEX(kPublicKeySpkiDerHex, output);

    // Failing case: Try to export a previously imported RSA public key in raw
    // format (not allowed for a public key).
    EXPECT_EQ(Status::ErrorUnsupportedExportKeyFormat(), ExportKey(blink::kWebCryptoKeyFormatRaw, key, &output));

    // Failing case: Try to export a non-extractable key
    ASSERT_EQ(Status::Success(),
        ImportKey(blink::kWebCryptoKeyFormatSpki, HexStringToBytes(kPublicKeySpkiDerHex),
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256), false,
            blink::kWebCryptoKeyUsageVerify, &key));
    EXPECT_TRUE(key.Handle());
    EXPECT_FALSE(key.Extractable());
    EXPECT_EQ(Status::ErrorKeyNotExtractable(), ExportKey(blink::kWebCryptoKeyFormatSpki, key, &output));
...
TEST_F(WebCryptoRsaSsaTest, ImportExportPkcs8)
{
    ...
    blink::WebCryptoKey key;
    ASSERT_EQ(Status::Success(),
        ImportKey(blink::kWebCryptoKeyFormatPkcs8, HexStringToBytes(kPrivateKeyPkcs8DerHex),
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha1), true, blink::kWebCryptoKeyUsageSign,
            &key));
    EXPECT_TRUE(key.Handle());
    EXPECT_EQ(blink::kWebCryptoKeyTypePrivate, key.GetType());
    EXPECT_TRUE(key.Extractable());
    EXPECT_EQ(blink::kWebCryptoKeyUsageSign, key.Usages());
    EXPECT_EQ(blink::kWebCryptoAlgorithmIdSha1, key.Algorithm().RsaHashedParams()->GetHash().Id());
    EXPECT_EQ(kModulusLengthBits, key.Algorithm().RsaHashedParams()->ModulusLengthBits());
    EXPECT_BYTES_EQ_HEX("010001", key.Algorithm().RsaHashedParams()->PublicExponent());

    std::vector<uint8_t> exported_key;
    ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatPkcs8, key, &exported_key));
    ...
    EXPECT_EQ(Status::ErrorUnsupportedImportKeyFormat(),
        ImportKey(blink::kWebCryptoKeyFormatPkcs8, HexStringToBytes(kPrivateKeyPkcs8DerHex), CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesCbc), true,
...
TEST_F(WebCryptoRsaSsaTest, ImportRsaPrivateKeyJwkToPkcs8RoundTrip)
{
    ...
    ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatJwk, key, &exported_key_jwk));

    // All of the optional parameters (p, q, dp, dq, qi) should be present in the
    // output.
    const char* expected_jwk = "{\"alg\":\"RS1\",\"d\":\"M6UEKpCyfU9UUcqbu9C0R3GhAa-IQ0Cu-YhfKku-"
    ...
    EXPECT_BYTES_EQ(base::as_bytes(base::make_span(std::string_view(expected_jwk))), exported_key_jwk);
    ...
    ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatPkcs8, key, &exported_key_pkcs8));
...
const char kRsa512Pkcs8_0[] = "30820156020100300D06092A864886F70D0101010500048201403082013C020100024100C8"
...
const char kRsa512Pkcs8_1[] = "30820155020100300D06092A864886F70D01010105000482013F3082013B020100024100CE"
...
TEST_F(WebCryptoRsaSsaTest, GenerateKeyPairRsa)
{
    ...
    blink::WebCryptoAlgorithm algorithm
        = CreateRsaHashedKeyGenAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256, modulus_length, public_exponent);
    bool extractable = true;
    const blink::WebCryptoKeyUsageMask public_usages = blink::kWebCryptoKeyUsageVerify;
    const blink::WebCryptoKeyUsageMask private_usages = blink::kWebCryptoKeyUsageSign;
    const blink::WebCryptoKeyUsageMask usages = public_usages | private_usages;
    blink::WebCryptoKey public_key;
    blink::WebCryptoKey private_key;

    EXPECT_EQ(Status::Success(), GenerateKeyPair(algorithm, extractable, usages, &public_key, &private_key));
    ...
    EXPECT_EQ(modulus_length, public_key.Algorithm().RsaHashedParams()->ModulusLengthBits());
    EXPECT_EQ(modulus_length, private_key.Algorithm().RsaHashedParams()->ModulusLengthBits());
    EXPECT_EQ(blink::kWebCryptoAlgorithmIdSha256, public_key.Algorithm().RsaHashedParams()->GetHash().Id());
    EXPECT_EQ(blink::kWebCryptoAlgorithmIdSha256, private_key.Algorithm().RsaHashedParams()->GetHash().Id());
    ...
    EXPECT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatSpki, public_key, &public_key_spki));

    public_key = blink::WebCryptoKey::CreateNull();
    ASSERT_EQ(Status::Success(),
        ImportKey(blink::kWebCryptoKeyFormatSpki, public_key_spki,
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256), true, public_usages, &public_key));
    EXPECT_EQ(modulus_length, public_key.Algorithm().RsaHashedParams()->ModulusLengthBits());

    std::vector<uint8_t> private_key_pkcs8;
    EXPECT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatPkcs8, private_key, &private_key_pkcs8));
    private_key = blink::WebCryptoKey::CreateNull();
    ASSERT_EQ(Status::Success(),
        ImportKey(blink::kWebCryptoKeyFormatPkcs8, private_key_pkcs8,
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256), true, private_usages,
            &private_key));
    EXPECT_EQ(modulus_length, private_key.Algorithm().RsaHashedParams()->ModulusLengthBits());

    // Fail with bad modulus.
    algorithm = CreateRsaHashedKeyGenAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256, 0, public_exponent);
    EXPECT_EQ(Status::ErrorGenerateRsaUnsupportedModulus(), GenerateKeyPair(algorithm, extractable, usages, &public_key, &private_key));
    ...
    algorithm = CreateRsaHashedKeyGenAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256, modulus_length, long_exponent);
    EXPECT_EQ(Status::ErrorGenerateKeyPublicExponent(), GenerateKeyPair(algorithm, extractable, usages, &public_key, &private_key));

    // Fail with bad exponent: empty.
    const std::vector<uint8_t> empty_exponent;
    algorithm = CreateRsaHashedKeyGenAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256, modulus_length, empty_exponent);
    EXPECT_EQ(Status::ErrorGenerateKeyPublicExponent(), GenerateKeyPair(algorithm, extractable, usages, &public_key, &private_key));

    // Fail with bad exponent: all zeros.
    std::vector<uint8_t> exponent_with_leading_zeros(15, 0x00);
    algorithm = CreateRsaHashedKeyGenAlgorithm(
        blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256, modulus_length, exponent_with_leading_zeros);
    EXPECT_EQ(Status::ErrorGenerateKeyPublicExponent(), GenerateKeyPair(algorithm, extractable, usages, &public_key, &private_key));

    // Key generation success using exponent with leading zeros.
    exponent_with_leading_zeros.insert(exponent_with_leading_zeros.end(), public_exponent.begin(), public_exponent.end());
    algorithm = CreateRsaHashedKeyGenAlgorithm(
        blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256, modulus_length, exponent_with_leading_zeros);
    EXPECT_EQ(Status::Success(), GenerateKeyPair(algorithm, extractable, usages, &public_key, &private_key));
    ...
    algorithm = CreateRsaHashedKeyGenAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha1, modulus_length, public_exponent);
    EXPECT_EQ(Status::Success(), GenerateKeyPair(algorithm, false, usages, &public_key, &private_key));
    EXPECT_FALSE(public_key.IsNull());
    EXPECT_FALSE(private_key.IsNull());
    EXPECT_EQ(blink::kWebCryptoKeyTypePublic, public_key.GetType());
    EXPECT_EQ(blink::kWebCryptoKeyTypePrivate, private_key.GetType());
    EXPECT_EQ(modulus_length, public_key.Algorithm().RsaHashedParams()->ModulusLengthBits());
    EXPECT_EQ(modulus_length, private_key.Algorithm().RsaHashedParams()->ModulusLengthBits());
    EXPECT_EQ(blink::kWebCryptoAlgorithmIdSha1, public_key.Algorithm().RsaHashedParams()->GetHash().Id());
    EXPECT_EQ(blink::kWebCryptoAlgorithmIdSha1, private_key.Algorithm().RsaHashedParams()->GetHash().Id());
    ...
    std::vector<uint8_t> output;
    EXPECT_EQ(Status::ErrorKeyNotExtractable(), ExportKey(blink::kWebCryptoKeyFormatSpki, private_key, &output));

    // Re-generate an extractable private_key and try to export it as SPKI format.
    // This should fail since spki is for public keys.
    EXPECT_EQ(Status::Success(), GenerateKeyPair(algorithm, true, usages, &public_key, &private_key));
    EXPECT_EQ(Status::ErrorUnexpectedKeyType(), ExportKey(blink::kWebCryptoKeyFormatSpki, private_key, &output));
}
...
TEST_F(WebCryptoRsaSsaTest, GenerateKeyPairRsaBadModulusLength)
{
    ...
    for (auto modulus_length_bits : kBadModulusBits) {
        blink::WebCryptoAlgorithm algorithm = CreateRsaHashedKeyGenAlgorithm(
            blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256, modulus_length_bits, public_exponent);
        bool extractable = true;
...
TEST_F(WebCryptoRsaSsaTest, GenerateKeyPairRsaBadExponent)
{
    ...
    for (auto* const exponent : kPublicExponents) {
        ...
        blink::WebCryptoAlgorithm algorithm = CreateRsaHashedKeyGenAlgorithm(
            blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256, modulus_length, HexStringToBytes(exponent));

        blink::WebCryptoKey public_key;
        blink::WebCryptoKey private_key;

        EXPECT_EQ(Status::ErrorGenerateKeyPublicExponent(), GenerateKeyPair(algorithm, true, blink::kWebCryptoKeyUsageSign, &public_key, &private_key));
    }
}
...
{
    // Import a key pair.
    blink::WebCryptoAlgorithm import_algorithm = CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha1);
    blink::WebCryptoKey public_key;
    blink::WebCryptoKey private_key;
    ASSERT_NO_FATAL_FAILURE(ImportRsaKeyPair(HexStringToBytes(kPublicKeySpkiDerHex), HexStringToBytes(kPrivateKeyPkcs8DerHex), import_algorithm, false,
        blink::kWebCryptoKeyUsageVerify, blink::kWebCryptoKeyUsageSign, &public_key, &private_key));

    blink::WebCryptoAlgorithm algorithm = CreateAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5);
    ...
    EXPECT_EQ(Status::Success(), Verify(algorithm, public_key, base::make_span(signature).first(signature.size() - 1), data, &signature_match));
    EXPECT_FALSE(signature_match);

    // Ensure truncated signature does not verify by passing no bytes.
    EXPECT_EQ(Status::Success(), Verify(algorithm, public_key, {}, data, &signature_match));
    EXPECT_FALSE(signature_match);

    // Ensure corrupted signature does not verify.
    std::vector<uint8_t> corrupt_sig = signature;
    corrupt_sig[corrupt_sig.size() / 2] ^= 0x1;
    EXPECT_EQ(Status::Success(), Verify(algorithm, public_key, corrupt_sig, data, &signature_match));
    EXPECT_FALSE(signature_match);
    ...
    EXPECT_EQ(Status::Success(), Verify(algorithm, public_key, kLongSignature, data, &signature_match));
    EXPECT_FALSE(signature_match);

    // Ensure that signing and verifying with an incompatible algorithm fails.
    algorithm = CreateAlgorithm(blink::kWebCryptoAlgorithmIdRsaOaep);

    EXPECT_EQ(Status::ErrorUnexpected(), Sign(algorithm, private_key, data, &signature));
    EXPECT_EQ(Status::ErrorUnexpected(), Verify(algorithm, public_key, signature, data, &signature_match));
    ...
    EXPECT_EQ(Status::Success(), Sign(CreateAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5), private_key, data, &signature));

    blink::WebCryptoKey public_key_256;
    EXPECT_EQ(Status::Success(),
        ImportKey(blink::kWebCryptoKeyFormatSpki, HexStringToBytes(kPublicKeySpkiDerHex),
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256), true,
            blink::kWebCryptoKeyUsageVerify, &public_key_256));
    ...
    EXPECT_EQ(blink::kWebCryptoAlgorithmIdSha1, private_key.Algorithm().RsaHashedParams()->GetHash().Id());
    EXPECT_EQ(blink::kWebCryptoAlgorithmIdSha256, public_key_256.Algorithm().RsaHashedParams()->GetHash().Id());

    bool is_match;
    EXPECT_EQ(Status::Success(), Verify(CreateAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5), public_key_256, signature, data, &is_match));
...
{
    const blink::WebCryptoAlgorithm algorithm = CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256);
    ...
    for (auto usage : kBadUsages) {
        ...
        ASSERT_EQ(Status::ErrorCreateKeyBadUsages(),
            ImportKey(blink::kWebCryptoKeyFormatSpki, HexStringToBytes(kPublicKeySpkiDerHex), algorithm, false, usage, &public_key));
    }
}
...
TEST_F(WebCryptoRsaSsaTest, ImportRsaSsaPublicKeyBadUsage_JWK)
{
    ...
    for (auto usage : kBadUsages) {
        blink::WebCryptoKey public_key;
        ASSERT_EQ(Status::ErrorCreateKeyBadUsages(), ImportKeyJwkFromDict(jwk, algorithm, false, usage, &public_key));
    }
}
    ...
    for (auto usage : kBadUsages) {
        ...
        ASSERT_EQ(Status::ErrorCreateKeyBadUsages(),
            GenerateKeyPair(CreateRsaHashedKeyGenAlgorithm(
                                blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256, modulus_length, public_exponent),
...
TEST_F(WebCryptoRsaSsaTest, GenerateKeyPairIntersectUsages)
{
    ...
    ASSERT_EQ(Status::Success(),
        GenerateKeyPair(
            CreateRsaHashedKeyGenAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256, modulus_length, public_exponent),
            true, blink::kWebCryptoKeyUsageSign | blink::kWebCryptoKeyUsageVerify, &public_key, &private_key));
    ...
    ASSERT_EQ(Status::Success(),
        GenerateKeyPair(
            CreateRsaHashedKeyGenAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256, modulus_length, public_exponent),
            true, blink::kWebCryptoKeyUsageSign, &public_key, &private_key));
...
TEST_F(WebCryptoRsaSsaTest, ImportKeyEmptyUsages)
{
    ...
    ASSERT_EQ(Status::Success(),
        ImportKey(blink::kWebCryptoKeyFormatSpki, HexStringToBytes(kPublicKeySpkiDerHex),
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256), true, 0, &public_key));
    EXPECT_EQ(0, public_key.Usages());

    // Private empty usage will throw an error.
    ASSERT_EQ(Status::ErrorCreateKeyEmptyUsages(),
        ImportKey(blink::kWebCryptoKeyFormatPkcs8, HexStringToBytes(kPrivateKeyPkcs8DerHex),
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha1), true, 0, &private_key));

    std::vector<uint8_t> public_jwk;
    ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatJwk, public_key, &public_jwk));
    ...
    ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatJwk, private_key, &private_jwk));
...
TEST_F(WebCryptoRsaSsaTest, ImportExportJwkRsaPublicKey)
{
    ...
    const TestCase kTests[] = { { blink::kWebCryptoAlgorithmIdSha1, blink::kWebCryptoKeyUsageVerify, "RS1" },
        { blink::kWebCryptoAlgorithmIdSha256, blink::kWebCryptoKeyUsageVerify, "RS256" },
        { blink::kWebCryptoAlgorithmIdSha384, blink::kWebCryptoKeyUsageVerify, "RS384" },
        { blink::kWebCryptoAlgorithmIdSha512, blink::kWebCryptoKeyUsageVerify, "RS512" } };

    for (const auto& test : kTests) {
        SCOPED_TRACE(&test - &kTests[0]);

        const blink::WebCryptoAlgorithm import_algorithm = CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, test.hash);

        // Import the spki to create a public key
        blink::WebCryptoKey public_key;
        ASSERT_EQ(Status::Success(),
            ImportKey(blink::kWebCryptoKeyFormatSpki, HexStringToBytes(kPublicKeySpkiDerHex), import_algorithm, true, test.usage, &public_key));

        // Export the public key as JWK and verify its contents
        std::vector<uint8_t> jwk;
        ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatJwk, public_key, &jwk));
        EXPECT_TRUE(VerifyPublicJwk(jwk, test.jwk_alg, kPublicKeyModulusHex, kPublicKeyExponentHex, test.usage));

        // Import the JWK back in to create a new key
        blink::WebCryptoKey public_key2;
        ASSERT_EQ(Status::Success(), ImportKey(blink::kWebCryptoKeyFormatJwk, jwk, import_algorithm, true, test.usage, &public_key2));
        ...
        ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatSpki, public_key2, &spki));
...
{
    blink::WebCryptoAlgorithm algorithm = CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256);
    ...
    // Baseline pass.
    EXPECT_EQ(Status::Success(), ImportKeyJwkFromDict(BuildTestJwkPublicKey(), algorithm, false, usages, &key));
    ...
    for (auto* const param : { "n", "e" }) {
        ...
        jwk.Remove(param);
        EXPECT_NE(Status::Success(), ImportKeyJwkFromDict(jwk, algorithm, false, usages, &key));
        ...
        jwk.Set(param, "");
        EXPECT_EQ(Status::ErrorJwkEmptyBigInteger(param), ImportKeyJwkFromDict(jwk, algorithm, false, usages, &key));
    }
}
...
TEST_F(WebCryptoRsaSsaTest, ImportRsaSsaJwkBadUsageAndData)
{
    ...
    ASSERT_EQ(Status::ErrorJwkNotDictionary(),
        ImportKey(blink::kWebCryptoKeyFormatJwk, base::as_bytes(base::make_span(bad_data)),
            CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256), true,
            blink::kWebCryptoKeyUsageVerify | blink::kWebCryptoKeyUsageSign, &key));
}
...
TEST_F(WebCryptoRsaSsaTest, ImportInvalidJwkPrivateKey_MissingMostOptionals)
{
    ...
    for (const auto* param : { "q", "dp", "dq", "qi" })
        key.Remove(param);
    EXPECT_EQ(StatusToString(ImportJwkRS256MustFail(key)), "DataError: The required JWK member \"q\" was missing");
}
...
TEST_F(WebCryptoRsaSsaTest, ImportInvalidJwkPrivateKey_MissingAllOptionals)
{
    ...
    for (const auto* param : { "p", "q", "dp", "dq", "qi" })
        key.Remove(param);
    EXPECT_EQ(StatusToString(ImportJwkRS256MustFail(key)), "DataError: The required JWK member \"p\" was missing");
}
...
TEST_F(WebCryptoRsaSsaTest, ImportInvalidJwkPrivateKey_LeadingZeroesOnE)
{
    ...
    EXPECT_EQ("DataError: The JWK \"e\" member contained a leading zero.", StatusToString(ImportJwkRS256MustFail(key)));
}
...
TEST_F(WebCryptoRsaSsaTest, ImportInvalidJwkPrivateKey_Base64PaddingInD)
{
    ...
    key.Set("d", d + "==");
    EXPECT_EQ("DataError: The JWK member \"d\" could not be base64url decoded or"
              " contained padding",
...
TEST_F(WebCryptoRsaSsaTest, ImportInvalidJwkPrivateKey_Base64UrlInN)
{
    ...
    key.Set("n", n);
    EXPECT_EQ("DataError: The JWK member \"n\" could not be base64url decoded or"
              " contained padding",
        StatusToString(ImportJwkRS256MustFail(key)));
}
...
TEST_F(WebCryptoRsaSsaTest, ImportInvalidJwkPrivateKey_Base64UrlInDQ)
{
    ...
    key.Set("dq", dq);
    EXPECT_EQ("DataError: The JWK member \"dq\" could not be base64url decoded or"
              " contained padding",
        StatusToString(ImportJwkRS256MustFail(key)));
}
...
{
    EXPECT_EQ("DataError",
        StatusToString(ImportSpkiRS256MustFail("3059301306072A8648CE3D020106082A8648CE3D030107034200049CB0CF69"
...
{
    EXPECT_EQ("DataError",
        StatusToString(ImportSpkiRS256MustFail("30819F300D06092A864886F70D010101050003818D0030818902818100A56E4A0E70"
...
{
    EXPECT_EQ("DataError", StatusToString(ImportPkcs8RS256MustFail("618333c4cb")));
}

TEST_F(WebCryptoRsaSsaTest, ImportInvalidPkcs8_CRTValuesAreZero)
{
    EXPECT_EQ("DataError",
        StatusToString(ImportPkcs8RS256MustFail("30820138020100300D06092A864886F70D0101010500048201223082011E02010002"
...
{
    EXPECT_EQ("DataError",
        StatusToString(ImportPkcs8RS256MustFail("308187020100301306072A8648CE3D020106082A8648CE3D030107046D306B"
...
{
    EXPECT_EQ("DataError",
        StatusToString(ImportPkcs8RS256MustFail("30820275020100300D06092A864886F70D01010105000482025F3082025B02010002"
                                                "818100A56E4A0E701017589A5187DC7EA841D156F2EC0E36AD52A44DFEB1E61F7AD9"
                                                "91D8C51056FFEDB162B4C0F283A12A88A394DFF526AB7291CBB307CEABFCE0B1DFD5"
                                                "CD9508096D5B2B8B6DF5D671EF6377C0921CB23C270A70E2598E6FF89D19F105ACC2"
                                                "D3F0CB35F29280E1386B6F64C4EF22E1E1F20D0CE8CFFB2249BD9A21370203010001"
...
TEST_F(WebCryptoRsaSsaTest, ImportInvalidPkcs8_CorruptFields)
{
    ...
    for (const auto& test : kTestCases) {
        ...
        EXPECT_EQ("DataError", StatusToString(ImportPkcs8RS256MustFail(key))) << "byte flip should have invalidated key: " << test.name;
    }
}
