...
bool Base64DecodeUrlSafe(std::string_view input, std::string* output)
{
    ...
    return base::Base64UrlDecode(input, base::Base64UrlDecodePolicy::DISALLOW_PADDING, output);
}
...
std::optional<base::Value> ReadJsonTestFile(const char* test_file_name)
{
    ...
    base::FilePath file_path
        = test_data_dir.AppendASCII("components").AppendASCII("test").AppendASCII("data").AppendASCII("webcrypto").AppendASCII(test_file_name);
...
bool operator==(const Status& a, const Status& b)
{
    ...
    if (a.IsSuccess())
        return true;
    return a.error_type() == b.error_type() && a.error_details() == b.error_details();
}
...
blink::WebCryptoAlgorithm CreateRsaHashedKeyGenAlgorithm(blink::WebCryptoAlgorithmId algorithm_id, const blink::WebCryptoAlgorithmId hash_id,
    unsigned int modulus_length, const std::vector<uint8_t>& public_exponent)
{
    DCHECK(blink::WebCryptoAlgorithm::IsHash(hash_id));
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(
        algorithm_id, new blink::WebCryptoRsaHashedKeyGenParams(CreateAlgorithm(hash_id), modulus_length, public_exponent));
}
...
std::vector<uint8_t> GetBytesFromHexString(const base::Value::Dict& dict, std::string_view property_name)
{
...
blink::WebCryptoAlgorithm GetDigestAlgorithm(const base::Value::Dict& dict, const char* property_name)
{
...
blink::WebCryptoAlgorithm CreateAesKeyGenAlgorithm(blink::WebCryptoAlgorithmId aes_alg_id, uint16_t length)
{
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(aes_alg_id, new blink::WebCryptoAesKeyGenParams(length));
}
...
const char* const kPublicKeySpkiDerHex = "30819f300d06092a864886f70d010101050003818d0030818902818100a5"
...
const char* const kPrivateKeyPkcs8DerHex = "30820275020100300d06092a864886f70d01010105000482025f3082025b"
...
const char* const kPublicKeyModulusHex = "A56E4A0E701017589A5187DC7EA841D156F2EC0E36AD52A44DFEB1E61F7AD991D8C51056"
...
blink::WebCryptoKey ImportSecretKeyFromRaw(const std::vector<uint8_t>& key_raw, const blink::WebCryptoAlgorithm& algorithm, blink::WebCryptoKeyUsageMask usage)
{
    blink::WebCryptoKey key;
    bool extractable = true;
    EXPECT_EQ(Status::Success(), ImportKey(blink::kWebCryptoKeyFormatRaw, key_raw, algorithm, extractable, usage, &key));
...
void ImportRsaKeyPair(const std::vector<uint8_t>& spki_der, const std::vector<uint8_t>& pkcs8_der, const blink::WebCryptoAlgorithm& algorithm, bool extractable,
    blink::WebCryptoKeyUsageMask public_key_usages, blink::WebCryptoKeyUsageMask private_key_usages, blink::WebCryptoKey* public_key,
    blink::WebCryptoKey* private_key)
{
    ASSERT_EQ(Status::Success(), ImportKey(blink::kWebCryptoKeyFormatSpki, spki_der, algorithm, true, public_key_usages, public_key));
    ...
    ASSERT_EQ(Status::Success(), ImportKey(blink::kWebCryptoKeyFormatPkcs8, pkcs8_der, algorithm, extractable, private_key_usages, private_key));
...
Status ImportKeyJwkFromDict(
    const base::ValueView& dict, const blink::WebCryptoAlgorithm& algorithm, bool extractable, blink::WebCryptoKeyUsageMask usages, blink::WebCryptoKey* key)
{
    return ImportKey(blink::kWebCryptoKeyFormatJwk, MakeJsonVector(dict), algorithm, extractable, usages, key);
}

std::optional<base::Value::Dict> GetJwkDictionary(const std::vector<uint8_t>& json)
{
    std::string_view json_string(reinterpret_cast<const char*>(json.data()), json.size());
...
::testing::AssertionResult VerifyJwk(
    const base::Value::Dict& dict, std::string_view kty_expected, std::string_view alg_expected, blink::WebCryptoKeyUsageMask use_mask_expected)
{
    // ---- kty
    ...
    if (*value_string != kty_expected)
        return ::testing::AssertionFailure() << "Expected 'kty' to be " << kty_expected << "but found " << *value_string;
    ...
    if (*value_string != alg_expected)
        return ::testing::AssertionFailure() << "Expected 'alg' to be " << alg_expected << " but found " << *value_string;
    ...
    if (!ext_value.value())
        return ::testing::AssertionFailure() << "Expected 'ext' to be true but found false";
    ...
    Status status = GetWebCryptoUsagesFromJwkKeyOpsForTest(*key_ops, &key_ops_mask);
    if (status.IsError())
        return ::testing::AssertionFailure() << "Failure extracting 'key_ops'";
    if (key_ops_mask != use_mask_expected)
        return ::testing::AssertionFailure() << "Expected 'key_ops' mask to be " << use_mask_expected << " but found " << key_ops_mask << " (" << value_string
                                             << ")";
...
::testing::AssertionResult VerifySecretJwk(
    const std::vector<uint8_t>& json, std::string_view alg_expected, std::string_view k_expected_hex, blink::WebCryptoKeyUsageMask use_mask_expected)
{
    ...
    if (!base::EqualsCaseInsensitiveASCII(base::HexEncode(k_value), k_expected_hex)) {
        return ::testing::AssertionFailure() << "Expected 'k' to be " << k_expected_hex << " but found something different";
    }

    return VerifyJwk(dict.value(), "oct", alg_expected, use_mask_expected);
}

::testing::AssertionResult VerifyPublicJwk(const std::vector<uint8_t>& json, std::string_view alg_expected, std::string_view n_expected_hex,
    std::string_view e_expected_hex, blink::WebCryptoKeyUsageMask use_mask_expected)
{
    std::optional<base::Value::Dict> dict = GetJwkDictionary(json);
    if (!dict.has_value() || dict.value().empty())
        return ::testing::AssertionFailure() << "JSON parsing failed";
    ...
    if (!base::EqualsCaseInsensitiveASCII(base::HexEncode(e_value), e_expected_hex)) {
        return ::testing::AssertionFailure() << "Expected 'e' to be " << e_expected_hex << " but found something different";
    }
...
void ImportExportJwkSymmetricKey(
    int key_len_bits, const blink::WebCryptoAlgorithm& import_algorithm, blink::WebCryptoKeyUsageMask usages, std::string_view jwk_alg)
{
    ...
    case 128:
        ...
        break;
    case 256:
        key_hex = "bd08286b81a74783fd1ccf46b7e05af84ee25ae021210074159e0c4d9d907692";
        break;
    case 384:
        key_hex = "a22c5441c8b185602283d64c7221de1d0951e706bfc09539435ec0e0ed614e1d40"
                  "6623f2b31d31819fec30993380dd82";
        break;
    case 512:
        key_hex = "5834f639000d4cf82de124fbfd26fb88d463e99f839a76ba41ac88967c80a3f61e"
    ...
    // Import a raw key.
    blink::WebCryptoKey key = ImportSecretKeyFromRaw(HexStringToBytes(key_hex), import_algorithm, usages);
    ...
    ASSERT_EQ(Status::Success(), ImportKey(blink::kWebCryptoKeyFormatJwk, json, import_algorithm, true, usages, &key));
    EXPECT_TRUE(key.Handle());
    EXPECT_EQ(blink::kWebCryptoKeyTypeSecret, key.GetType());
    ...
    ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatRaw, key, &key_raw_out));
    EXPECT_BYTES_EQ_HEX(key_hex, key_raw_out);
}

Status GenerateSecretKey(const blink::WebCryptoAlgorithm& algorithm, bool extractable, blink::WebCryptoKeyUsageMask usages, blink::WebCryptoKey* key)
{
...
Status GenerateKeyPair(const blink::WebCryptoAlgorithm& algorithm, bool extractable, blink::WebCryptoKeyUsageMask usages, blink::WebCryptoKey* public_key,
    blink::WebCryptoKey* private_key)
{
    GenerateKeyResult result;
    Status status = GenerateKey(algorithm, extractable, usages, &result);
...
blink::WebCryptoKeyFormat GetKeyFormatFromJsonTestCase(const base::Value::Dict& test)
{
...
std::vector<uint8_t> GetKeyDataFromJsonTestCase(const base::Value::Dict& test, blink::WebCryptoKeyFormat key_format)
{
...
blink::WebCryptoNamedCurve GetCurveNameFromDictionary(const base::Value::Dict& dict)
{
...
blink::WebCryptoAlgorithm CreateHmacImportAlgorithm(blink::WebCryptoAlgorithmId hash_id, unsigned int length_bits)
{
    DCHECK(blink::WebCryptoAlgorithm::IsHash(hash_id));
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(
...
blink::WebCryptoAlgorithm CreateHmacImportAlgorithmNoLength(blink::WebCryptoAlgorithmId hash_id)
{
    DCHECK(blink::WebCryptoAlgorithm::IsHash(hash_id));
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(
        blink::kWebCryptoAlgorithmIdHmac, new blink::WebCryptoHmacImportParams(CreateAlgorithm(hash_id), false, 0));
}
...
blink::WebCryptoAlgorithm CreateRsaHashedImportAlgorithm(blink::WebCryptoAlgorithmId id, blink::WebCryptoAlgorithmId hash_id)
{
    DCHECK(blink::WebCryptoAlgorithm::IsHash(hash_id));
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(id, new blink::WebCryptoRsaHashedImportParams(CreateAlgorithm(hash_id)));
}

blink::WebCryptoAlgorithm CreateEcImportAlgorithm(blink::WebCryptoAlgorithmId id, blink::WebCryptoNamedCurve named_curve)
{
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(id, new blink::WebCryptoEcKeyImportParams(named_curve));
}
