...
public:
    AesKwImplementation()
        : AesAlgorithm(blink::kWebCryptoKeyUsageWrapKey | blink::kWebCryptoKeyUsageUnwrapKey, "KW")
    {
    }
        ...
        const blink::WebCryptoAlgorithm& algorithm, const blink::WebCryptoKey& key, base::span<const uint8_t> data, std::vector<uint8_t>* buffer) const override
    {
        ...
        AES_KEY aes_key;
        if (AES_set_encrypt_key(raw_key.data(), static_cast<unsigned>(raw_key.size() * 8), &aes_key) < 0) {
        ...
        if (AES_wrap_key(&aes_key, nullptr /* default IV */, buffer->data(), data.data(), data.size()) < 0) {
            return Status::OperationError();
        }
        ...
        const blink::WebCryptoAlgorithm& algorithm, const blink::WebCryptoKey& key, base::span<const uint8_t> data, std::vector<uint8_t>* buffer) const override
    {
        ...
        const std::vector<uint8_t>& raw_key = GetSymmetricKeyData(key);
        AES_KEY aes_key;
        if (AES_set_decrypt_key(raw_key.data(), static_cast<unsigned>(raw_key.size() * 8), &aes_key) < 0) {
            return Status::OperationError();
        }
        ...
        if (AES_unwrap_key(&aes_key, nullptr /* default IV */, buffer->data(), data.data(), data.size()) < 0) {
            return Status::OperationError();
        }
