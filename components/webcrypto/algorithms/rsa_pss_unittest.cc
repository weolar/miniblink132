...
blink::WebCryptoAlgorithm CreateRsaPssAlgorithm(unsigned int salt_length_bytes)
{
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(blink::kWebCryptoAlgorithmIdRsaPss, new blink::WebCryptoRsaPssParams(salt_length_bytes));
}
...
TEST_F(WebCryptoRsaPssTest, SignIsRandom)
{
    ...
    ImportRsaKeyPair(HexStringToBytes(kPublicKeySpkiDerHex), HexStringToBytes(kPrivateKeyPkcs8DerHex),
        CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaPss, blink::kWebCryptoAlgorithmIdSha1), true, blink::kWebCryptoKeyUsageVerify,
        blink::kWebCryptoKeyUsageSign, &public_key, &private_key);
    ...
    ASSERT_EQ(Status::Success(), Verify(params, public_key, signature1, message, &is_match));
    EXPECT_TRUE(is_match);

    ASSERT_EQ(Status::Success(), Verify(params, public_key, signature2, message, &is_match));
    EXPECT_TRUE(is_match);

    // Corrupt the signature and verification must fail.
    ASSERT_EQ(Status::Success(), Verify(params, public_key, Corrupted(signature2), message, &is_match));
...
TEST_F(WebCryptoRsaPssTest, SignEmptyMessage)
{
    ...
    // Make sure that verification works.
    bool is_match = false;
    ASSERT_EQ(Status::Success(), Verify(params, public_key, signature, message, &is_match));
    EXPECT_TRUE(is_match);

    // Corrupt the signature and verification must fail.
    ASSERT_EQ(Status::Success(), Verify(params, public_key, Corrupted(signature), message, &is_match));
    EXPECT_FALSE(is_match);
}

// RSA-PSS known answer tests:
const char kKey1[] = "30819F300D06092A864886F70D010101050003818D0030818902818100A56E4A0E70101758"
...
const char kKey2[] = "30819D300D06092A864886F70D010101050003818B0030818702818100BE499B5E7F06C83F"
...
const char kNistTestMessage[] = "c7f5270fca725f9bd19f519a8d7cca3cc5c079024029f3bae510f9b02140fe238908e4f6c1"
...
blink::WebCryptoKey RsaPssKeyFromBytes(blink::WebCryptoAlgorithmId hash, const char* key)
{
    auto pubkey = blink::WebCryptoKey::CreateNull();
    webcrypto::Status result = ImportKey(blink::kWebCryptoKeyFormatSpki, HexStringToBytes(key),
        CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaPss, hash), true, blink::kWebCryptoKeyUsageVerify, &pubkey);
    CHECK(result == Status::Success());
    return pubkey;
}

bool VerifySignature(size_t salt_length, blink::WebCryptoKey pubkey, const std::vector<uint8_t>& signature, const std::vector<uint8_t>& message)
{
    bool is_match = false;
    auto result = Verify(CreateRsaPssAlgorithm(salt_length), pubkey, signature, message, &is_match);
    CHECK(result == Status::Success());
    return is_match;
}
...
TEST_F(WebCryptoRsaPssTest, VerifyKnownAnswer)
{
    for (const auto& test : kRsaPssKnownAnswers) {
        ...
        EXPECT_FALSE(VerifySignature(test.salt_length, pubkey, signature, Corrupted(message)));
        EXPECT_FALSE(VerifySignature(test.salt_length, pubkey, Corrupted(signature), message));
    }
}
