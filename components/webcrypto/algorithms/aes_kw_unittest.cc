...
{
    return CreateAesKeyGenAlgorithm(blink::kWebCryptoAlgorithmIdAesKw, key_length_bits);
}
...
TEST_F(WebCryptoAesKwTest, GenerateKeyBadLength)
{
    ...
    for (auto len : { 0, 127, 257 }) {
        SCOPED_TRACE(len);
        EXPECT_EQ(Status::ErrorGenerateAesKeyLength(), GenerateSecretKey(CreateAesKwKeyGenAlgorithm(len), true, blink::kWebCryptoKeyUsageWrapKey, &key));
    }
}
...
{
    blink::WebCryptoKey key;
    EXPECT_EQ(Status::ErrorCreateKeyEmptyUsages(), GenerateSecretKey(CreateAesKwKeyGenAlgorithm(256), true, 0, &key));
}
...
TEST_F(WebCryptoAesKwTest, ImportKeyEmptyUsage)
{
    ...
    EXPECT_EQ(Status::ErrorCreateKeyEmptyUsages(),
        ImportKey(blink::kWebCryptoKeyFormatRaw, std::vector<uint8_t>(16), CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesKw), true, 0, &key));
}
...
TEST_F(WebCryptoAesKwTest, ImportKeyJwkKeyOpsWrapUnwrap)
{
    ...
    EXPECT_EQ(Status::Success(), ImportKeyJwkFromDict(dict, CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesKw), false, blink::kWebCryptoKeyUsageWrapKey, &key));
    ...
    EXPECT_EQ(
        Status::Success(), ImportKeyJwkFromDict(dict, CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesKw), false, blink::kWebCryptoKeyUsageUnwrapKey, &key));
...
{
    const blink::WebCryptoAlgorithm algorithm = CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesKw);

    // AES-KW 128
    ImportExportJwkSymmetricKey(128, algorithm, blink::kWebCryptoKeyUsageWrapKey | blink::kWebCryptoKeyUsageUnwrapKey, "A128KW");

    // AES-KW 256
    ImportExportJwkSymmetricKey(256, algorithm, blink::kWebCryptoKeyUsageWrapKey | blink::kWebCryptoKeyUsageUnwrapKey, "A256KW");
}
...
TEST_F(WebCryptoAesKwTest, AesKwKeyImport)
{
    ...
    ASSERT_EQ(
        Status::Success(), ImportKey(blink::kWebCryptoKeyFormatRaw, HexStringToBytes(key_raw_hex_in), algorithm, true, blink::kWebCryptoKeyUsageWrapKey, &key));
    ...
    ASSERT_EQ(Status::ErrorAes192BitUnsupported(),
        ImportKey(blink::kWebCryptoKeyFormatRaw, HexStringToBytes(key_raw_hex_in), algorithm, true, blink::kWebCryptoKeyUsageWrapKey, &key));

    // Import a 256-bit Key Encryption Key (KEK)
    key_raw_hex_in = "e11fe66380d90fa9ebefb74e0478e78f95664d0c67ca20ce4a0b5842863ac46f";
    ASSERT_EQ(
        Status::Success(), ImportKey(blink::kWebCryptoKeyFormatRaw, HexStringToBytes(key_raw_hex_in), algorithm, true, blink::kWebCryptoKeyUsageWrapKey, &key));
    EXPECT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatRaw, key, &key_raw_out));
    EXPECT_BYTES_EQ_HEX(key_raw_hex_in, key_raw_out);
    ...
    EXPECT_EQ(Status::ErrorImportAesKeyLength(),
        ImportKey(blink::kWebCryptoKeyFormatRaw, HexStringToBytes(""), algorithm, true, blink::kWebCryptoKeyUsageWrapKey, &key));
...
TEST_F(WebCryptoAesKwTest, UnwrapFailures)
{
    ...
    // should fail.
    blink::WebCryptoKey wrapping_key = ImportSecretKeyFromRaw(test_kek, CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesKw), blink::kWebCryptoKeyUsageUnwrapKey);
    EXPECT_EQ(Status::ErrorUnexpected(),
        UnwrapKey(blink::kWebCryptoKeyFormatRaw, test_ciphertext, wrapping_key, CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesCbc),
...
TEST_F(WebCryptoAesKwTest, AesKwRawSymkeyWrapUnwrapKnownAnswer)
{
    for (const auto& test : kAesKwKnownAnswers) {
        ...
        const auto test_ciphertext = HexStringToBytes(test.ciphertext);
        const blink::WebCryptoAlgorithm wrapping_algorithm = CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesKw);

        // Import the wrapping key.
        blink::WebCryptoKey wrapping_key
            = ImportSecretKeyFromRaw(test_kek, wrapping_algorithm, blink::kWebCryptoKeyUsageWrapKey | blink::kWebCryptoKeyUsageUnwrapKey);

        // Import the key to be wrapped.
        blink::WebCryptoKey key
            = ImportSecretKeyFromRaw(test_key, CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha1), blink::kWebCryptoKeyUsageSign);

        // Wrap the key and verify the ciphertext result against the known answer.
        std::vector<uint8_t> wrapped_key;
        ASSERT_EQ(Status::Success(), WrapKey(blink::kWebCryptoKeyFormatRaw, key, wrapping_key, wrapping_algorithm, &wrapped_key));
        EXPECT_BYTES_EQ(test_ciphertext, wrapped_key);

        // Unwrap the known ciphertext to get a new test_key.
        blink::WebCryptoKey unwrapped_key;
        ASSERT_EQ(Status::Success(),
            UnwrapKey(blink::kWebCryptoKeyFormatRaw, test_ciphertext, wrapping_key, wrapping_algorithm,
                CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha1), true, blink::kWebCryptoKeyUsageSign, &unwrapped_key));
        ...
        EXPECT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatRaw, unwrapped_key, &raw_key));
...
TEST_F(WebCryptoAesKwTest, AesKwRawSymkeyUnwrapSignVerifyHmac)
{
    ...
    // Import the wrapping key.
    blink::WebCryptoKey wrapping_key = ImportSecretKeyFromRaw(test_kek, wrapping_algorithm, blink::kWebCryptoKeyUsageUnwrapKey);

    // Unwrap the known ciphertext.
    blink::WebCryptoKey key;
    ASSERT_EQ(Status::Success(),
        UnwrapKey(blink::kWebCryptoKeyFormatRaw, test_ciphertext, wrapping_key, wrapping_algorithm,
            CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha1), false, blink::kWebCryptoKeyUsageSign | blink::kWebCryptoKeyUsageVerify, &key));
    ...
    ASSERT_EQ(Status::Success(), Sign(CreateAlgorithm(blink::kWebCryptoAlgorithmIdHmac), key, test_message, &signature));

    EXPECT_GT(signature.size(), 0u);

    bool verify_result;
    ASSERT_EQ(Status::Success(), Verify(CreateAlgorithm(blink::kWebCryptoAlgorithmIdHmac), key, signature, test_message, &verify_result));
}
...
TEST_F(WebCryptoAesKwTest, AesKwRawSymkeyWrapUnwrapErrors)
{
    ...
    const blink::WebCryptoAlgorithm wrapping_algorithm = CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesKw);
    const blink::WebCryptoAlgorithm key_algorithm = CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesCbc);
    // Import the wrapping key.
    blink::WebCryptoKey wrapping_key
        = ImportSecretKeyFromRaw(test_kek, wrapping_algorithm, blink::kWebCryptoKeyUsageWrapKey | blink::kWebCryptoKeyUsageUnwrapKey);
    // Import the key to be wrapped.
    blink::WebCryptoKey key = ImportSecretKeyFromRaw(test_key, CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesCbc), blink::kWebCryptoKeyUsageEncrypt);

    // Unwrap with wrapped data too small must fail.
    const std::vector<uint8_t> small_data(test_ciphertext.begin(), test_ciphertext.begin() + 23);
    blink::WebCryptoKey unwrapped_key;
    EXPECT_EQ(Status::ErrorDataTooSmall(),
        UnwrapKey(blink::kWebCryptoKeyFormatRaw, small_data, wrapping_key, wrapping_algorithm, key_algorithm, true, blink::kWebCryptoKeyUsageEncrypt,
            &unwrapped_key));

    // Unwrap with wrapped data size not a multiple of 8 bytes must fail.
    const std::vector<uint8_t> unaligned_data(test_ciphertext.begin(), test_ciphertext.end() - 2);
    EXPECT_EQ(Status::ErrorInvalidAesKwDataLength(),
        UnwrapKey(blink::kWebCryptoKeyFormatRaw, unaligned_data, wrapping_key, wrapping_algorithm, key_algorithm, true, blink::kWebCryptoKeyUsageEncrypt,
            &unwrapped_key));
}
...
TEST_F(WebCryptoAesKwTest, AesKwRawSymkeyUnwrapCorruptData)
{
    ...
    EXPECT_EQ(Status::OperationError(),
        UnwrapKey(blink::kWebCryptoKeyFormatRaw, Corrupted(test_ciphertext), wrapping_key, wrapping_algorithm,
            CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesCbc), true, blink::kWebCryptoKeyUsageEncrypt, &unwrapped_key));
}
...
TEST_F(WebCryptoAesKwTest, AesKwJwkSymkeyUnwrapKnownData)
{
    ...
    const std::vector<uint8_t> key_data = HexStringToBytes("000102030405060708090A0B0C0D0E0F000102030405060708090A0B0C0D0E0F");
    const std::vector<uint8_t> wrapped_key_data = HexStringToBytes("14E6380B35FDC5B72E1994764B6CB7BFDD64E7832894356AAEE6C3768FC3D0F115E6B0"
    ...
    const std::vector<uint8_t> wrapping_key_data = HexStringToBytes("000102030405060708090A0B0C0D0E0F");
    const blink::WebCryptoAlgorithm wrapping_algorithm = CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesKw);

    // Import the wrapping key.
    blink::WebCryptoKey wrapping_key = ImportSecretKeyFromRaw(wrapping_key_data, wrapping_algorithm, blink::kWebCryptoKeyUsageUnwrapKey);

    // Unwrap the known wrapped key data to produce a new key
    blink::WebCryptoKey unwrapped_key;
    ASSERT_EQ(Status::Success(),
        UnwrapKey(blink::kWebCryptoKeyFormatJwk, wrapped_key_data, wrapping_key, wrapping_algorithm,
            CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha256), true, blink::kWebCryptoKeyUsageVerify, &unwrapped_key));
    ...
    EXPECT_EQ(blink::kWebCryptoAlgorithmIdSha256, unwrapped_key.Algorithm().HmacParams()->GetHash().Id());
...
{
    const blink::WebCryptoAlgorithm unwrap_algorithm = CreateAlgorithm(blink::kWebCryptoAlgorithmIdAesKw);
    ...
    blink::WebCryptoKey wrapping_key;
    ASSERT_EQ(Status::Success(),
        ImportKey(blink::kWebCryptoKeyFormatRaw, std::vector<uint8_t>(16), unwrap_algorithm, true, blink::kWebCryptoKeyUsageUnwrapKey, &wrapping_key));
    ...
    for (auto usage : kBadUsages) {
        ...
        ASSERT_EQ(Status::ErrorCreateKeyBadUsages(),
            UnwrapKey(blink::kWebCryptoKeyFormatJwk, HexStringToBytes(kWrappedJwk), wrapping_key, unwrap_algorithm,
                CreateHmacImportAlgorithmNoLength(blink::kWebCryptoAlgorithmIdSha256), true, usage, &key));
    }
}
...
TEST_F(WebCryptoAesKwTest, UnwrapRsaSsaPublicKeyBadUsage_JWK)
{
    ...
    const char* kWrappedJwk = "CE8DAEF99E977EE58958B8C4494755C846E883B2ECA575C5366622839AF71AB30875F152"
    ...
    for (auto usage : kBadUsages) {
        ...
        ASSERT_EQ(Status::ErrorCreateKeyBadUsages(),
            UnwrapKey(blink::kWebCryptoKeyFormatJwk, HexStringToBytes(kWrappedJwk), wrapping_key, unwrap_algorithm,
                CreateRsaHashedImportAlgorithm(blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5, blink::kWebCryptoAlgorithmIdSha256), true, usage, &key));
    }
}
