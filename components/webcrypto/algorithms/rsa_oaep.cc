...
typedef int (*EncryptDecryptFunc)(EVP_PKEY_CTX* ctx, unsigned char* out, size_t* outlen, const unsigned char* in, size_t inlen);
...
Status CommonEncryptDecrypt(InitFunc init_func, EncryptDecryptFunc encrypt_decrypt_func, const blink::WebCryptoAlgorithm& algorithm,
    const blink::WebCryptoKey& key, base::span<const uint8_t> data, std::vector<uint8_t>* buffer)
{
    ...
    if (!init_func(ctx.get()) || !EVP_PKEY_CTX_set_rsa_padding(ctx.get(), RSA_PKCS1_OAEP_PADDING) || !EVP_PKEY_CTX_set_rsa_oaep_md(ctx.get(), digest)
        || !EVP_PKEY_CTX_set_rsa_mgf1_md(ctx.get(), digest)) {
        return Status::OperationError();
    }

    const blink::WebVector<uint8_t>& label = algorithm.RsaOaepParams()->OptionalLabel();
    ...
    if (label.size()) {
        ...
        if (1 != EVP_PKEY_CTX_set0_rsa_oaep_label(ctx.get(), label_copy.release(), label.size())) {
            return Status::OperationError();
        }
    }
    ...
    size_t outlen = 0;
    if (!encrypt_decrypt_func(ctx.get(), nullptr, &outlen, data.data(), data.size())) {
        return Status::OperationError();
    }
    buffer->resize(outlen);

    // Do the actual encryption/decryption.
    if (!encrypt_decrypt_func(ctx.get(), buffer->data(), &outlen, data.data(), data.size())) {
        return Status::OperationError();
    }
    buffer->resize(outlen);
...
public:
    RsaOaepImplementation()
        : RsaHashedAlgorithm(
            blink::kWebCryptoKeyUsageEncrypt | blink::kWebCryptoKeyUsageWrapKey, blink::kWebCryptoKeyUsageDecrypt | blink::kWebCryptoKeyUsageUnwrapKey)
    {
    }

    const char* GetJwkAlgorithm(const blink::WebCryptoAlgorithmId hash) const override
    {
        switch (hash) {
        ...
        const blink::WebCryptoAlgorithm& algorithm, const blink::WebCryptoKey& key, base::span<const uint8_t> data, std::vector<uint8_t>* buffer) const override
    {
        ...
        return CommonEncryptDecrypt(EVP_PKEY_encrypt_init, EVP_PKEY_encrypt, algorithm, key, data, buffer);
    }
        ...
        const blink::WebCryptoAlgorithm& algorithm, const blink::WebCryptoKey& key, base::span<const uint8_t> data, std::vector<uint8_t>* buffer) const override
    {
        ...
        return CommonEncryptDecrypt(EVP_PKEY_decrypt_init, EVP_PKEY_decrypt, algorithm, key, data, buffer);
    }
};
