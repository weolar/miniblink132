...
#include "third_party/openssl/openssl/include/openssl/cipher.h"
...
Status AesCtrEncrypt128BitCounter(const EVP_CIPHER* cipher, base::span<const uint8_t> raw_key, base::span<const uint8_t> input,
    base::span<const uint8_t, 16> counter, base::span<uint8_t> output)
{
    DCHECK(cipher);
    ...
    if (!EVP_CipherInit_ex(context.get(), cipher, nullptr, raw_key.data(), counter.data(), ENCRYPT)) {
        return Status::OperationError();
    }

    int output_len = 0;
    if (!EVP_CipherUpdate(context.get(), output.data(), &output_len, input.data(), base::checked_cast<int>(input.size()))) {
        return Status::OperationError();
    }
    int final_output_chunk_len = 0;
    if (!EVP_CipherFinal_ex(context.get(), output.data() + output_len, &final_output_chunk_len)) {
        return Status::OperationError();
    }
...
absl::uint128 GetCounter(base::span<const uint8_t, 16> counter_block, unsigned int counter_length_bits)
{
...
std::array<uint8_t, AES_BLOCK_SIZE> BlockWithZeroedCounter(base::span<const uint8_t, AES_BLOCK_SIZE> counter_block, unsigned int counter_length_bits)
{
    ...
    const blink::WebCryptoAlgorithm& algorithm, const blink::WebCryptoKey& key, base::span<const uint8_t> data, std::vector<uint8_t>* buffer)
{
    ...
    base::span<const uint8_t, AES_BLOCK_SIZE> counter_block(params->Counter().data(), params->Counter().size());
    ...
    absl::uint128 current_counter = GetCounter(counter_block, counter_length_bits);

    if (counter_length_bits == 128) {
        return AesCtrEncrypt128BitCounter(cipher, base::span<const uint8_t>(raw_key.data(), raw_key.size()),
            data, counter_block, base::span<uint8_t>(buffer->data(), buffer->size()));
    }
    ...
    // size.
    size_t input_size_part1 = static_cast<size_t>(num_blocks_until_reset * AES_BLOCK_SIZE);
    DCHECK_LT(input_size_part1, data.size());
    base::span<uint8_t> output_part1 = base::span<uint8_t>(buffer->data(), buffer->size()).first(input_size_part1);
    base::span<uint8_t> output_part2 = base::span<uint8_t>(buffer->data(), buffer->size()).subspan(input_size_part1);

    // Encrypt the first part (before wrap-around).
    Status status = AesCtrEncrypt128BitCounter(cipher, base::span<const uint8_t>(raw_key.data(), raw_key.size()), data.first(input_size_part1), counter_block, output_part1);
    if (status.IsError())
        return status;

    // Encrypt the second part (after wrap-around).
    std::array<uint8_t, AES_BLOCK_SIZE> counter_block_part2 = BlockWithZeroedCounter(counter_block, counter_length_bits);

    return AesCtrEncrypt128BitCounter(cipher, base::span<const uint8_t>(raw_key.data(), raw_key.size()), data.subspan(input_size_part1), counter_block_part2, output_part2);
}

class AesCtrImplementation : public AesAlgorithm {
public:
    AesCtrImplementation()
        : AesAlgorithm("CTR")
    {
    }
