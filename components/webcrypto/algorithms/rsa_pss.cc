...
public:
    RsaPssImplementation()
        : RsaHashedAlgorithm(blink::kWebCryptoKeyUsageVerify, blink::kWebCryptoKeyUsageSign)
    {
    }
    ...
    {
        return RsaSign(key, algorithm.RsaPssParams()->SaltLengthBytes(), data, buffer);
    }
    ...
    {
        return RsaVerify(key, algorithm.RsaPssParams()->SaltLengthBytes(), signature, data, signature_match);
    }
};
