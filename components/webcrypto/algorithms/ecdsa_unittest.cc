...
blink::WebCryptoAlgorithm CreateEcdsaKeyGenAlgorithm(blink::WebCryptoNamedCurve named_curve)
{
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(blink::kWebCryptoAlgorithmIdEcdsa, new blink::WebCryptoEcKeyGenParams(named_curve));
}

blink::WebCryptoAlgorithm CreateEcdsaImportAlgorithm(blink::WebCryptoNamedCurve named_curve)
{
    return CreateEcImportAlgorithm(blink::kWebCryptoAlgorithmIdEcdsa, named_curve);
}

blink::WebCryptoAlgorithm CreateEcdsaAlgorithm(blink::WebCryptoAlgorithmId hash_id)
{
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(blink::kWebCryptoAlgorithmIdEcdsa, new blink::WebCryptoEcdsaParams(CreateAlgorithm(hash_id)));
}
...
TEST_F(WebCryptoEcdsaTest, GenerateKeyIsRandom)
{
    ...
    for (int j = 0; j < 4; ++j) {
        ...
        ASSERT_EQ(Status::Success(), GenerateKeyPair(CreateEcdsaKeyGenAlgorithm(named_curve), true, blink::kWebCryptoKeyUsageSign, &public_key, &private_key));
        ...
        ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatJwk, public_key, &key_bytes));
        serialized_keys.push_back(key_bytes);

        ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatJwk, private_key, &key_bytes));
        serialized_keys.push_back(key_bytes);
    }
...
TEST_F(WebCryptoEcdsaTest, GenerateKeyEmptyUsage)
{
    ...
    blink::WebCryptoKey public_key;
    blink::WebCryptoKey private_key;
    ASSERT_EQ(Status::ErrorCreateKeyEmptyUsages(), GenerateKeyPair(CreateEcdsaKeyGenAlgorithm(named_curve), true, 0, &public_key, &private_key));
}
...
TEST_F(WebCryptoEcdsaTest, SignatureIsRandom)
{
    ...
    // using the first one.
    base::Value::List private_keys = ReadJsonTestFileAsList("ec_private_keys.json");
    ...
    blink::WebCryptoKey private_key;
    ASSERT_EQ(Status::Success(), ImportKeyJwkFromDict(*key_jwk, CreateEcdsaImportAlgorithm(curve), true, blink::kWebCryptoKeyUsageSign, &private_key));
    ...
    blink::WebCryptoKey public_key;
    ASSERT_EQ(Status::Success(), ImportKeyJwkFromDict(key_jwk_copy, CreateEcdsaImportAlgorithm(curve), true, blink::kWebCryptoKeyUsageVerify, &public_key));

    // Sign twice
    std::vector<uint8_t> message(10);
    blink::WebCryptoAlgorithm algorithm = CreateEcdsaAlgorithm(blink::kWebCryptoAlgorithmIdSha1);

    std::vector<uint8_t> signature1;
    std::vector<uint8_t> signature2;
    ASSERT_EQ(Status::Success(), Sign(algorithm, private_key, message, &signature1));
    ASSERT_EQ(Status::Success(), Sign(algorithm, private_key, message, &signature2));
    ...
    ASSERT_EQ(Status::Success(), Verify(algorithm, public_key, signature1, message, &signature_matches));
    EXPECT_TRUE(signature_matches);
    ASSERT_EQ(Status::Success(), Verify(algorithm, public_key, signature2, message, &signature_matches));
    EXPECT_TRUE(signature_matches);
}
...
blink::WebCryptoKeyUsageMask GetExpectedUsagesForKeyImport(blink::WebCryptoKeyFormat key_format, const base::Value::Dict& test)
{
...
TEST_F(WebCryptoEcdsaTest, ImportBadKeys)
{
    ...
    for (const auto& test_value : tests) {
        ...
        std::vector<uint8_t> key_data = GetKeyDataFromJsonTestCase(test, key_format);
        const std::string* expected_error = test.FindString("error");
        ASSERT_TRUE(expected_error);

        blink::WebCryptoKey key;
        Status status = ImportKey(key_format, key_data, CreateEcdsaImportAlgorithm(curve), true, GetExpectedUsagesForKeyImport(key_format, test), &key);
...
TEST_F(WebCryptoEcdsaTest, ImportExportPrivateKey)
{
    ...
    for (const auto& test_value : tests) {
        ...
        std::vector<uint8_t> pkcs8_bytes = GetBytesFromHexString(test, test.contains("exported_pkcs8") ? "exported_pkcs8" : "pkcs8");
        ...
        ASSERT_EQ(Status::Success(),
            ImportKey(blink::kWebCryptoKeyFormatJwk, jwk_bytes, CreateEcdsaImportAlgorithm(curve), true, blink::kWebCryptoKeyUsageSign, &key));

        // Export the key as JWK
        std::vector<uint8_t> exported_bytes;
        ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatJwk, key, &exported_bytes));
        ...
        ASSERT_EQ(Status::Success(), ExportKey(blink::kWebCryptoKeyFormatPkcs8, key, &exported_bytes));
        ...
        // during export).
        std::vector<uint8_t> pkcs8_input_bytes = GetBytesFromHexString(test, test.contains("original_pkcs8") ? "original_pkcs8" : "pkcs8");
        base::span<const uint8_t> pkcs8_input_data(pkcs8_input_bytes.empty() ? pkcs8_bytes : pkcs8_input_bytes);

        // Import the key using PKCS8
        ASSERT_EQ(Status::Success(),
            ImportKey(blink::kWebCryptoKeyFormatPkcs8, pkcs8_input_data, CreateEcdsaImportAlgorithm(curve), true, blink::kWebCryptoKeyUsageSign, &key));
