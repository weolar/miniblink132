...
blink::WebCryptoAlgorithm CreateRsaHashedImportAlgorithm(blink::WebCryptoAlgorithmId id, blink::WebCryptoAlgorithmId hash_id)
{
    ...
    return blink::WebCryptoAlgorithm::AdoptParamsAndCreate(
        id, new blink::WebCryptoRsaHashedImportParams(blink::WebCryptoAlgorithm::AdoptParamsAndCreate(hash_id, nullptr)));
}
...
blink::WebCryptoKeyUsageMask GetCompatibleKeyUsages(blink::WebCryptoKeyFormat format)
{
    ...
    // algorithm.
    return format == blink::kWebCryptoKeyFormatSpki ? blink::kWebCryptoKeyUsageVerify : blink::kWebCryptoKeyUsageSign;
}

void ImportEcKeyFromDerFuzzData(const uint8_t* data, size_t size, blink::WebCryptoKeyFormat format)
{
    DCHECK(format == blink::kWebCryptoKeyFormatSpki || format == blink::kWebCryptoKeyFormatPkcs8);
    EnsureInitialized();
    ...
    webcrypto::Status status = webcrypto::ImportKey(format, base::make_span(data, size), CreateEcImportAlgorithm(algorithm_id, curve), true, usages, &key);
...
void ImportEcKeyFromRawFuzzData(const uint8_t* data, size_t size)
{
    ...
    webcrypto::Status status
        = webcrypto::ImportKey(blink::kWebCryptoKeyFormatRaw, base::make_span(data, size), CreateEcImportAlgorithm(algorithm_id, curve), true, usages, &key);
...
void ImportRsaKeyFromDerFuzzData(const uint8_t* data, size_t size, blink::WebCryptoKeyFormat format)
{
    DCHECK(format == blink::kWebCryptoKeyFormatSpki || format == blink::kWebCryptoKeyFormatPkcs8);
    EnsureInitialized();
    ...
    // for non-JWK formats.
    blink::WebCryptoAlgorithmId algorithm_id = blink::kWebCryptoAlgorithmIdRsaSsaPkcs1v1_5;
    ...
    blink::WebCryptoKey key;
    webcrypto::Status status
        = webcrypto::ImportKey(format, base::make_span(data, size), CreateRsaHashedImportAlgorithm(algorithm_id, hash_id), true, usages, &key);
