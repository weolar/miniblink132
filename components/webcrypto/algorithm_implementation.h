...
public:
    ...
    virtual Status Verify(const blink::WebCryptoAlgorithm& algorithm, const blink::WebCryptoKey& key, base::span<const uint8_t> signature,
    ...
    virtual Status Digest(const blink::WebCryptoAlgorithm& algorithm, base::span<const uint8_t> data, std::vector<uint8_t>* buffer) const;

    // This is what is run whenever the spec says:
    //    "Let result be the result of executing the generate key operation"
    //
    // (crypto.subtle.generateKey() dispatches to this)
    virtual Status GenerateKey(
    ...
    virtual Status DeriveBits(const blink::WebCryptoAlgorithm& algorithm, const blink::WebCryptoKey& base_key, bool has_optional_length_bits,
    ...
    virtual Status GetKeyLength(const blink::WebCryptoAlgorithm& key_length_algorithm, bool* has_length_bits, unsigned int* length_bits) const;

    // This is what is run whenever the spec says:
    //    "Let result be the result of performing the import key operation"
    //
    // (crypto.subtle.importKey() dispatches to this).
    virtual Status ImportKey(blink::WebCryptoKeyFormat format, base::span<const uint8_t> key_data, const blink::WebCryptoAlgorithm& algorithm, bool extractable,
    ...
    virtual Status ExportKey(blink::WebCryptoKeyFormat format, const blink::WebCryptoKey& key, std::vector<uint8_t>* buffer) const;
    ...
    Status SerializeKeyForClone(const blink::WebCryptoKey& key, blink::WebVector<uint8_t>* key_data) const;
    ...
    virtual Status DeserializeKeyForClone(const blink::WebCryptoKeyAlgorithm& algorithm, blink::WebCryptoKeyType type, bool extractable,
