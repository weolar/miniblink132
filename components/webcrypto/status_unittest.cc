...
TEST(WebCryptoStatusTest, Basic)
{
    ...
    EXPECT_EQ(Status::ErrorJwkMemberWrongType("kty", "string"), Status::ErrorJwkMemberWrongType("kty", "string"));
    ...
    EXPECT_TRUE(status.IsError());
    EXPECT_EQ("The required JWK member \"kty\" was missing", status.error_details());
    EXPECT_EQ(blink::kWebCryptoErrorTypeData, status.error_type());
    ...
    EXPECT_TRUE(status.IsError());
    EXPECT_EQ("The JWK member \"n\" could not be base64url decoded or contained "
              "padding",
