...
public:
    ...
    bool ShouldDeleteCookieOnExit(const ContentSettingsForOneType& cookie_settings, const std::string& domain, bool is_https) const;
    ...
    bool IsFullCookieAccessAllowed(const GURL& url, const GURL& first_party_url, QueryReason query_reason) const;
    ...
    ContentSetting GetCookieSetting(const GURL& url, const GURL& first_party_url, content_settings::SettingSource* source, QueryReason query_reason) const;
    ...
    net::CookieAccessSemantics GetCookieAccessSemanticsForDomain(const std::string& cookie_domain) const;
    ...
    // removed.
    virtual ContentSetting GetSettingForLegacyCookieAccess(const std::string& cookie_domain) const = 0;
    ...
    // embedding context.
    virtual bool ShouldIgnoreSameSiteRestrictions(const GURL& url, const net::SiteForCookies& site_for_cookies) const = 0;
...
protected:
    ...
    static bool IsThirdPartyRequest(const GURL& url, const net::SiteForCookies& site_for_cookies);
    ...
    static GURL GetFirstPartyURL(const net::SiteForCookies& site_for_cookies, const url::Origin* top_frame_origin);

private:
    virtual ContentSetting GetCookieSettingInternal(
        const GURL& url, const GURL& first_party_url, bool is_third_party_request, content_settings::SettingSource* source, QueryReason query_reason) const
        = 0;
