...
const char* const kSchemeNames[] = { "wildcard", "other", url::kHttpScheme, url::kHttpsScheme, url::kFileScheme, "chrome-extension", "chrome-search", "chrome",
    "chrome-untrusted", "devtools" };

static_assert(std::size(kSchemeNames) == ContentSettingsPattern::SCHEME_MAX, "kSchemeNames should have SCHEME_MAX elements");
...
bool IsSubDomainOrEqual(base::StringPiece sub_domain, base::StringPiece domain)
{
    ...
    return sub_domain.length() > domain.length() && sub_domain[sub_domain.length() - domain.length() - 1] == '.'
...
std::tuple<absl::optional<base::StringPiece>, base::StringPiece> SplitDomainOnLastDot(const base::StringPiece domain)
{
    size_t index_of_last_dot = domain.rfind('.');
    if (index_of_last_dot == base::StringPiece::npos)
        return std::make_tuple(absl::nullopt, domain);
    return std::make_tuple(domain.substr(0, index_of_last_dot), domain.substr(index_of_last_dot + 1));
}
...
//
class ContentSettingsPattern::Builder : public ContentSettingsPattern::BuilderInterface {
public:
    Builder();
...
BuilderInterface* ContentSettingsPattern::Builder::WithPort(const std::string& port)
{
    parts_.port = port;
...
BuilderInterface* ContentSettingsPattern::Builder::WithHost(const std::string& host)
{
    parts_.host = host;
    return this;
}
...
BuilderInterface* ContentSettingsPattern::Builder::WithScheme(const std::string& scheme)
{
...
BuilderInterface* ContentSettingsPattern::Builder::WithPath(const std::string& path)
{
    parts_.path = path;
...
ContentSettingsPattern ContentSettingsPattern::Builder::Build()
{
    ...
    DCHECK(ContentSettingsPattern(std::move(twice_canonicalized_parts), true) == ContentSettingsPattern(parts_, true));
#endif
...
bool ContentSettingsPattern::Builder::Canonicalize(PatternParts* parts)
{
    ...
    if (parts->scheme == url::kFileScheme && !parts->is_path_wildcard) {
        ...
        while (true) {
            std::string url_spec = base::StrCat({ url::kFileScheme, url::kStandardSchemeSeparator, parts->path });
            GURL url(url_spec);
            if (!url.is_valid())
                return false;
    ...
    std::string canonicalized_host(net::CanonicalizeHost(parts->host, &host_info));
    ...
    if (base::EndsWith(canonicalized_host, "..", base::CompareCase::SENSITIVE)) {
        return false;
    } else if (canonicalized_host.size() >= 2u && base::EndsWith(canonicalized_host, ".", base::CompareCase::SENSITIVE)) {
        canonicalized_host.pop_back();
    }

    if ((parts->host.find('*') == std::string::npos) && !canonicalized_host.empty()) {
        // Valid host.
...
bool ContentSettingsPattern::Builder::Validate(const PatternParts& parts)
{
    ...
    if ((parts.is_scheme_wildcard && !parts.scheme.empty()) || (parts.is_port_wildcard && !parts.port.empty())
    ...
    if (parts.scheme == url::kFileScheme) {
        ...
        return (!parts.path.empty() && parts.path != "/" && parts.path.find('*') == std::string::npos);
    }

    // If the pattern is for a URL with a non-wildcard domain without a port,
    // test if it is valid.
    if (IsNonWildcardDomainNonPortScheme(parts.scheme) && parts.port.empty() && !parts.is_port_wildcard) {
        return true;
    }

    // Non-file patterns are invalid if either the scheme, host or port part is
    // empty.
    if ((parts.scheme.empty() && !parts.is_scheme_wildcard) || (parts.host.empty() && !parts.has_domain_wildcard)
    ...
    if (!parts.is_scheme_wildcard && parts.scheme != url::kHttpScheme && parts.scheme != url::kHttpsScheme) {
        return false;
    }
    return true;
}
...
ContentSettingsPattern::PatternParts::PatternParts(const PatternParts& other) = default;
ContentSettingsPattern::PatternParts::PatternParts(PatternParts&& other) = default;

ContentSettingsPattern::PatternParts::~PatternParts()
{
}

ContentSettingsPattern::PatternParts& ContentSettingsPattern::PatternParts::operator=(const PatternParts& other) = default;
ContentSettingsPattern::PatternParts& ContentSettingsPattern::PatternParts::operator=(PatternParts&& other) = default;
...
ContentSettingsPattern ContentSettingsPattern::FromURL(const GURL& url)
{
    ...
    if (local_url->SchemeIsFile()) {
        ...
        if (local_url->HostIsIPAddress()) {
            ...
            builder.WithSchemeWildcard()->WithDomainWildcard()->WithHost(local_url->host());
        } else if (local_url->SchemeIs(url::kHttpsScheme)) {
            builder.WithScheme(local_url->scheme())->WithDomainWildcard()->WithHost(local_url->host());
        } else {
            // Unsupported scheme
        }
...
// static
ContentSettingsPattern ContentSettingsPattern::FromURLNoWildcard(const GURL& url)
{
    ContentSettingsPattern::Builder builder;
    const GURL* local_url = &url;
    if (url.SchemeIsFileSystem() && url.inner_url()) {
...
// static
ContentSettingsPattern ContentSettingsPattern::FromString(base::StringPiece pattern_spec)
{
    ContentSettingsPattern::Builder builder;
...
// static
void ContentSettingsPattern::SetNonWildcardDomainNonPortSchemes(const char* const* schemes, size_t count)
{
...
// static
bool ContentSettingsPattern::IsNonWildcardDomainNonPortScheme(base::StringPiece scheme)
{
    DCHECK(g_non_domain_wildcard_non_port_schemes || g_non_domain_wildcard_non_port_schemes_count == 0);
...
// static
ContentSettingsPattern ContentSettingsPattern::ToDomainWildcardPattern(const ContentSettingsPattern& pattern)
{
    DCHECK(pattern.IsValid());
    std::string registrable_domain
        = net::registry_controlled_domains::GetDomainAndRegistry(pattern.GetHost(), net::registry_controlled_domains::INCLUDE_PRIVATE_REGISTRIES);

    if (registrable_domain.empty()) {
        return CreateBuilder()->Invalid()->Build();
    }

    return CreateBuilder()->WithHost(registrable_domain)->WithDomainWildcard()->WithSchemeWildcard()->WithPortWildcard()->WithPathWildcard()->Build();
}

// static
ContentSettingsPattern ContentSettingsPattern::ToHostOnlyPattern(const ContentSettingsPattern& pattern)
{
    DCHECK(pattern.IsValid());
...
bool ContentSettingsPattern::Matches(const GURL& url) const
{
    ...
    if (!parts_.is_scheme_wildcard && parts_.scheme != local_url->scheme_piece()) {
        return false;
    }
    ...
    if (!parts_.is_scheme_wildcard && local_url->scheme_piece() == url::kFileScheme)
    ...
    // specified.
    const base::StringPiece port = local_url->port_piece().empty() ? GetDefaultPort(local_url->scheme_piece()) : local_url->port_piece();
...
{
    return !parts_.is_scheme_wildcard && !parts_.has_domain_wildcard && !parts_.is_port_wildcard && !parts_.is_path_wildcard;
}
...
ContentSettingsPattern::Relation ContentSettingsPattern::Compare(const ContentSettingsPattern& other) const
{
    ...
    if (host_relation == DISJOINT_ORDER_PRE || host_relation == DISJOINT_ORDER_POST)
        return host_relation;

    Relation port_relation = ComparePort(parts_, other.parts_);
    if (port_relation == DISJOINT_ORDER_PRE || port_relation == DISJOINT_ORDER_POST)
        return port_relation;

    Relation scheme_relation = CompareScheme(parts_, other.parts_);
    if (scheme_relation == DISJOINT_ORDER_PRE || scheme_relation == DISJOINT_ORDER_POST)
        return scheme_relation;

    Relation path_relation = ComparePath(parts_, other.parts_);
    if (path_relation == DISJOINT_ORDER_PRE || path_relation == DISJOINT_ORDER_POST)
...
bool ContentSettingsPattern::operator==(const ContentSettingsPattern& other) const
{
...
ContentSettingsPattern::Relation ContentSettingsPattern::CompareScheme(
    const ContentSettingsPattern::PatternParts& parts, const ContentSettingsPattern::PatternParts& other_parts)
{
    ...
    const ContentSettingsPattern::PatternParts& parts, const ContentSettingsPattern::PatternParts& other_parts)
{
    ...
    if (parts.scheme != url::kFileScheme || other_parts.scheme != url::kFileScheme) {
        return ContentSettingsPattern::IDENTITY;
    }
