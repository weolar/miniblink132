...
TEST(ContentSettingsPatternTest, FromURL)
{
    ...
    ContentSettingsPattern pattern = ContentSettingsPattern::FromURL(GURL("http://www.youtube.com"));
...
{
    ContentSettingsPattern pattern = ContentSettingsPattern::FromURL(GURL("http://www.google.com"));
    EXPECT_TRUE(pattern.Matches(GURL("filesystem:http://www.google.com/temporary/")));
    EXPECT_TRUE(pattern.Matches(GURL("filesystem:http://foo.www.google.com/temporary/")));
    EXPECT_TRUE(pattern.Matches(GURL("filesystem:http://www.google.com:80/temporary/")));
    EXPECT_TRUE(pattern.Matches(GURL("filesystem:http://www.google.com:81/temporary/")));

    pattern = ContentSettingsPattern::FromURL(GURL("https://www.google.com"));
    EXPECT_TRUE(pattern.Matches(GURL("filesystem:https://www.google.com/temporary/")));
    EXPECT_TRUE(pattern.Matches(GURL("filesystem:https://www.google.com:443/temporary/")));
    EXPECT_TRUE(pattern.Matches(GURL("filesystem:https://foo.www.google.com/temporary/")));
    EXPECT_FALSE(pattern.Matches(GURL("filesystem:https://www.google.com:81/temporary/")));

    // A pattern from a filesystem URLs is equivalent to a pattern from the inner
    // URL of the filesystem URL.
    ContentSettingsPattern pattern2 = ContentSettingsPattern::FromURL(GURL("filesystem:https://www.google.com/temporary/"));
    ...
    pattern = ContentSettingsPattern::FromURL(GURL("filesystem:file:///temporary/foo/bar"));
    ...
    pattern2 = ContentSettingsPattern::FromURL(GURL("filesystem:file:///persistent/foo2/bar2"));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, pattern.Compare(pattern2));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_POST, pattern2.Compare(pattern));
}
...
TEST(ContentSettingsPatternTest, FromURLNoWildcard)
{
    ...
    // with a wildcard.
    ContentSettingsPattern pattern = ContentSettingsPattern::FromURLNoWildcard(GURL("http://www.example.com"));
    EXPECT_TRUE(pattern.IsValid());
    ...
    pattern = ContentSettingsPattern::FromURLNoWildcard(GURL("https://www.example.com"));
    EXPECT_TRUE(pattern.IsValid());
    ...
    pattern = ContentSettingsPattern::FromURLNoWildcard(GURL("filesystem:http://www.google.com/temporary/"));
    EXPECT_TRUE(pattern.IsValid());
    EXPECT_TRUE(pattern.Matches(GURL("http://www.google.com")));
    EXPECT_FALSE(pattern.Matches(GURL("http://foo.www.google.com")));
    EXPECT_TRUE(pattern.Matches(GURL("filesystem:http://www.google.com/persistent/")));
    EXPECT_FALSE(pattern.Matches(GURL("filesystem:https://www.google.com/persistent/")));
    EXPECT_FALSE(pattern.Matches(GURL("filesystem:https://www.google.com:81/temporary/")));
    EXPECT_FALSE(pattern.Matches(GURL("filesystem:https://foo.www.google.com/temporary/")));

    pattern = ContentSettingsPattern::FromURLNoWildcard(GURL("chrome://test"));
    EXPECT_TRUE(pattern.IsValid());
    EXPECT_TRUE(pattern.Matches(GURL("chrome://test")));

    pattern = ContentSettingsPattern::FromURLNoWildcard(GURL("chrome-untrusted://test"));
    EXPECT_TRUE(pattern.IsValid());
    EXPECT_TRUE(pattern.Matches(GURL("chrome-untrusted://test")));

    pattern = ContentSettingsPattern::FromURLNoWildcard(GURL("devtools://devtools"));
    EXPECT_TRUE(pattern.IsValid());
...
{
    std::unique_ptr<ContentSettingsPattern::BuilderInterface> builder = ContentSettingsPattern::CreateBuilder();
    builder->WithSchemeWildcard()->WithDomainWildcard()->WithPortWildcard()->WithPathWildcard();
...
TEST(ContentSettingsPatternTest, Wildcard)
{
    ...
    EXPECT_TRUE(ContentSettingsPattern::Wildcard().Matches(GURL("http://www.google.com")));
    EXPECT_TRUE(ContentSettingsPattern::Wildcard().Matches(GURL("https://www.google.com")));
    EXPECT_TRUE(ContentSettingsPattern::Wildcard().Matches(GURL("https://myhost:8080")));
    EXPECT_TRUE(ContentSettingsPattern::Wildcard().Matches(GURL("file:///foo/bar.txt")));

    EXPECT_STREQ("*", ContentSettingsPattern::Wildcard().ToString().c_str());

    EXPECT_EQ(ContentSettingsPattern::IDENTITY, ContentSettingsPattern::Wildcard().Compare(ContentSettingsPattern::Wildcard()));
}

TEST(ContentSettingsPatternTest, TrimEndingDotFromHost)
{
    EXPECT_TRUE(Pattern("www.example.com").IsValid());
    EXPECT_TRUE(Pattern("www.example.com").Matches(GURL("http://www.example.com")));
    EXPECT_TRUE(Pattern("www.example.com").Matches(GURL("http://www.example.com.")));

    EXPECT_TRUE(Pattern("www.example.com.").IsValid());
    EXPECT_STREQ("www.example.com", Pattern("www.example.com.").ToString().c_str());
...
TEST(ContentSettingsPatternTest, FromString_WithNoWildcards)
{
    ...
    EXPECT_STREQ("http://www.example.com:80", Pattern("http://www.example.com:80").ToString().c_str());
    // HTTP patterns with none default port.
    EXPECT_TRUE(Pattern("http://www.example.com:81").IsValid());
    EXPECT_STREQ("http://www.example.com:81", Pattern("http://www.example.com:81").ToString().c_str());

    // HTTPS patterns with default port.
    EXPECT_TRUE(Pattern("https://www.example.com:443").IsValid());
    EXPECT_STREQ("https://www.example.com:443", Pattern("https://www.example.com:443").ToString().c_str());
    // HTTPS patterns with none default port.
    EXPECT_TRUE(Pattern("https://www.example.com:8080").IsValid());
    EXPECT_STREQ("https://www.example.com:8080", Pattern("https://www.example.com:8080").ToString().c_str());
}
...
TEST(ContentSettingsPatternTest, FromString_FilePatterns)
{
    ...
    EXPECT_TRUE(Pattern("file:///foo/bar/file.html").Matches(GURL("file://localhost/foo/bar/file.html")));
    EXPECT_TRUE(Pattern("file:///foo/bar/file.html").Matches(GURL("file://example.com/foo/bar/file.html")));
    EXPECT_FALSE(Pattern("file:///foo/bar/file.html").Matches(GURL("file://localhost/foo/bar/other.html")));
    EXPECT_FALSE(Pattern("file:///foo/bar/file.html").Matches(GURL("file://example.com/foo/bar/other.html")));

    ContentSettingsPattern pattern = ContentSettingsPattern::FromURL(GURL("file:///tmp/test.html"));
    ...
    ContentSettingsPattern pattern2 = ContentSettingsPattern::FromString("file:///tmp/test.html");
    ContentSettingsPattern pattern3 = ContentSettingsPattern::FromString("file:///tmp/other.html");

    EXPECT_EQ(ContentSettingsPattern::IDENTITY, pattern.Compare(pattern));
    EXPECT_EQ(ContentSettingsPattern::IDENTITY, pattern.Compare(pattern2));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, pattern.Compare(pattern3));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_POST, pattern3.Compare(pattern));
    EXPECT_EQ(ContentSettingsPattern::SUCCESSOR, file_wildcard.Compare(pattern));
    EXPECT_EQ(ContentSettingsPattern::PREDECESSOR, pattern.Compare(file_wildcard));
}

TEST(ContentSettingsPatternTest, FromString_ExtensionPatterns)
{
    EXPECT_TRUE(Pattern("chrome-extension://peoadpeiejnhkmpaakpnompolbglelel/").IsValid());
    EXPECT_EQ("chrome-extension://peoadpeiejnhkmpaakpnompolbglelel/", Pattern("chrome-extension://peoadpeiejnhkmpaakpnompolbglelel/").ToString());
    EXPECT_TRUE(Pattern("chrome-extension://peoadpeiejnhkmpaakpnompolbglelel/").Matches(GURL("chrome-extension://peoadpeiejnhkmpaakpnompolbglelel/")));
}

TEST(ContentSettingsPatternTest, FromString_SearchPatterns)
{
    EXPECT_TRUE(Pattern("chrome-search://local-ntp/").IsValid());
    EXPECT_EQ("chrome-search://local-ntp/", Pattern("chrome-search://local-ntp/").ToString());
    EXPECT_TRUE(Pattern("chrome-search://local-ntp/").Matches(GURL("chrome-search://local-ntp/")));
}
...
TEST(ContentSettingsPatternTest, FromString_WithIPAdresses)
{
    ...
    EXPECT_STREQ("https://192.168.0.1:8080", Pattern("https://192.168.0.1:8080").ToString().c_str());
    ...
    EXPECT_STREQ("https://[::1]:8080", Pattern("https://[::1]:8080").ToString().c_str());
}
...
TEST(ContentSettingsPatternTest, FromString_WithWildcards)
{
    ...
    EXPECT_TRUE(Pattern("*").IsValid());
    EXPECT_STREQ("*", Pattern("*").ToString().c_str());
    EXPECT_EQ(ContentSettingsPattern::IDENTITY, Pattern("*").Compare(ContentSettingsPattern::Wildcard()));

    // Patterns with port wildcard.
    EXPECT_TRUE(Pattern("http://example.com:*").IsValid());
    EXPECT_STREQ("http://example.com", Pattern("http://example.com:*").ToString().c_str());

    EXPECT_TRUE(Pattern("https://example.com").IsValid());
    EXPECT_STREQ("https://example.com", Pattern("https://example.com").ToString().c_str());

    EXPECT_TRUE(Pattern("*://www.google.com.com:8080").IsValid());
    EXPECT_STREQ("www.google.com:8080", Pattern("*://www.google.com:8080").ToString().c_str());
    EXPECT_TRUE(Pattern("*://www.google.com:8080").Matches(GURL("http://www.google.com:8080")));
    EXPECT_TRUE(Pattern("*://www.google.com:8080").Matches(GURL("https://www.google.com:8080")));
    EXPECT_FALSE(Pattern("*://www.google.com").Matches(GURL("file:///foo/bar.html")));
    ...
    EXPECT_TRUE(Pattern("[*.]example.com").Matches(GURL("http://foo.example.com/")));
    EXPECT_FALSE(Pattern("[*.]example.com").Matches(GURL("http://example.org/")));

    EXPECT_TRUE(Pattern("[*.]google.com:80").Matches(GURL("http://mail.google.com:80")));
    EXPECT_FALSE(Pattern("[*.]google.com:80").Matches(GURL("http://mail.google.com:81")));
    EXPECT_TRUE(Pattern("[*.]google.com:80").Matches(GURL("http://www.google.com")));

    EXPECT_TRUE(Pattern("[*.]google.com:8080").Matches(GURL("http://mail.google.com:8080")));

    EXPECT_TRUE(Pattern("[*.]google.com:443").Matches(GURL("https://mail.google.com:443")));
    EXPECT_TRUE(Pattern("[*.]google.com:443").Matches(GURL("https://www.google.com")));

    EXPECT_TRUE(Pattern("[*.]google.com:4321").Matches(GURL("https://mail.google.com:4321")));
    EXPECT_TRUE(Pattern("[*.]example.com").Matches(GURL("http://example.com/")));
    EXPECT_TRUE(Pattern("[*.]example.com").Matches(GURL("http://www.example.com/")));
...
TEST(ContentSettingsPatternTest, FromString_Canonicalized)
{
    ...
    EXPECT_STREQ("[*.]xn--ira-ppa.com", Pattern("[*.]\xC4\x87ira.com").ToString().c_str());
    EXPECT_TRUE(Pattern("\xC4\x87ira.com").IsValid());
    EXPECT_STREQ("xn--ira-ppa.com", Pattern("\xC4\x87ira.com").ToString().c_str());
    EXPECT_TRUE(Pattern("file:///\xC4\x87ira.html").IsValid());
    EXPECT_STREQ("file:///%C4%87ira.html", Pattern("file:///\xC4\x87ira.html").ToString().c_str());

    // File path normalization.
    EXPECT_TRUE(Pattern("file:///tmp/bar/../test.html").IsValid());
    EXPECT_STREQ("file:///tmp/test.html", Pattern("file:///tmp/bar/../test.html").ToString().c_str());
}

TEST(ContentSettingsPatternTest, FromString_WebUISchemes)
{
    const char* patterns[] = { "chrome://test/", "chrome-untrusted://test/", "devtools://devtools/" };
...
TEST(ContentSettingsPatternTest, ToDomainWildcardPattern)
{
    ...
    ContentSettingsPattern pattern = Pattern("https://www.google.com:81/temporary/");
    ContentSettingsPattern domain_wildcard_pattern = ContentSettingsPattern::ToDomainWildcardPattern(pattern);
    ...
    domain_wildcard_pattern = ContentSettingsPattern::ToDomainWildcardPattern(pattern);
    EXPECT_TRUE(domain_wildcard_pattern.IsValid());
...
TEST(ContentSettingsPatternTest, ToHostOnlyPattern)
{
    // Patterns with scheme, port and path.
    ContentSettingsPattern pattern = Pattern("https://www.google.com:81/temporary/");
    ContentSettingsPattern host_only_pattern = ContentSettingsPattern::ToHostOnlyPattern(pattern);
...
TEST(ContentSettingsPatternTest, UnequalOperator)
{
    ...
    EXPECT_TRUE(Pattern("http://www.foo.com*") != ContentSettingsPattern::Wildcard());

    EXPECT_TRUE(Pattern("http://www.foo.com") != ContentSettingsPattern::Wildcard());

    EXPECT_TRUE(Pattern("http://www.foo.com") != Pattern("www.foo.com"));
    EXPECT_TRUE(Pattern("http://www.foo.com") != Pattern("http://www.foo.com:80"));
...
TEST(ContentSettingsPatternTest, Compare)
{
    ...
    EXPECT_EQ(ContentSettingsPattern::IDENTITY, Pattern("http://www.google.com:80").Compare(Pattern("http://www.google.com:80")));
    EXPECT_EQ(ContentSettingsPattern::IDENTITY, Pattern("*://[*.]google.com:*").Compare(Pattern("*://[*.]google.com:*")));

    ContentSettingsPattern invalid_pattern1;
    ContentSettingsPattern invalid_pattern2 = ContentSettingsPattern::FromString("google.com*");

    // Compare invalid patterns.
    EXPECT_TRUE(!invalid_pattern1.IsValid());
    EXPECT_TRUE(!invalid_pattern2.IsValid());
    EXPECT_EQ(ContentSettingsPattern::IDENTITY, invalid_pattern1.Compare(invalid_pattern2));
    EXPECT_TRUE(invalid_pattern1 == invalid_pattern2);

    // Compare a pattern with an IPv4 addresse to a pattern with a domain name.
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_POST, Pattern("http://www.google.com").Compare(Pattern("127.0.0.1")));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, Pattern("127.0.0.1").Compare(Pattern("http://www.google.com")));
    ...
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_POST, Pattern("http://www.google.com").Compare(Pattern("[::1]")));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, Pattern("[::1]").Compare(Pattern("http://www.google.com")));
    ...
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, Pattern("127.0.0.1").Compare(Pattern("[::1]")));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_POST, Pattern("[::1]").Compare(Pattern("127.0.0.1")));
    EXPECT_TRUE(Pattern("[::1]") < Pattern("127.0.0.1"));
    EXPECT_TRUE(Pattern("127.0.0.1") > Pattern("[::1]"));

    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, Pattern("http://www.google.com").Compare(Pattern("http://www.youtube.com")));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, Pattern("http://[*.]google.com").Compare(Pattern("http://[*.]youtube.com")));

    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_POST, Pattern("http://[*.]host.com").Compare(Pattern("http://[*.]evilhost.com")));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_POST, Pattern("*://www.google.com:80").Compare(Pattern("*://www.google.com:8080")));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, Pattern("https://www.google.com:80").Compare(Pattern("http://www.google.com:80")));

    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, Pattern("http://[*.]google.com:90").Compare(Pattern("http://mail.google.com:80")));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, Pattern("https://[*.]google.com:80").Compare(Pattern("http://mail.google.com:80")));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, Pattern("https://mail.google.com:*").Compare(Pattern("http://mail.google.com:80")));

    // Test patterns with different precedences.
    EXPECT_EQ(ContentSettingsPattern::PREDECESSOR, Pattern("mail.google.com").Compare(Pattern("[*.]google.com")));
    EXPECT_EQ(ContentSettingsPattern::SUCCESSOR, Pattern("[*.]google.com").Compare(Pattern("mail.google.com")));
    ...
    EXPECT_EQ(ContentSettingsPattern::PREDECESSOR, Pattern("mail.google.com:80").Compare(Pattern("mail.google.com:*")));
    EXPECT_EQ(ContentSettingsPattern::SUCCESSOR, Pattern("mail.google.com:*").Compare(Pattern("mail.google.com:80")));
    EXPECT_TRUE(Pattern("mail.google.com:80") > Pattern("mail.google.com:*"));

    EXPECT_EQ(ContentSettingsPattern::PREDECESSOR, Pattern("https://mail.google.com:*").Compare(Pattern("*://mail.google.com:*")));
    EXPECT_EQ(ContentSettingsPattern::SUCCESSOR, Pattern("*://mail.google.com:*").Compare(Pattern("https://mail.google.com:*")));

    EXPECT_EQ(ContentSettingsPattern::PREDECESSOR, Pattern("*://mail.google.com:80").Compare(Pattern("https://mail.google.com:*")));
    EXPECT_EQ(ContentSettingsPattern::SUCCESSOR, Pattern("https://mail.google.com:*").Compare(Pattern("*://mail.google.com:80")));
}

TEST(ContentSettingsPatternTest, CompareSubdomains)
{
    EXPECT_EQ(ContentSettingsPattern::IDENTITY, Pattern("https://[*.]a.b").Compare(Pattern("https://[*.]a.b")));

    EXPECT_EQ(ContentSettingsPattern::PREDECESSOR, Pattern("https://[*.]b.a.a.a").Compare(Pattern("https://[*.]a.a")));
    EXPECT_EQ(ContentSettingsPattern::SUCCESSOR, Pattern("https://[*.]a.a").Compare(Pattern("https://[*.]b.a.a.a")));

    EXPECT_EQ(ContentSettingsPattern::PREDECESSOR, Pattern("https://[*.]a.b.a.b").Compare(Pattern("https://[*.]a.b")));
    EXPECT_EQ(ContentSettingsPattern::SUCCESSOR, Pattern("https://[*.]a.b").Compare(Pattern("https://[*.]a.b.a.b")));

    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, Pattern("https://[*.]a.a").Compare(Pattern("https://[*.]b.a.a.b")));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_POST, Pattern("https://[*.]b.a.a.b").Compare(Pattern("https://[*.]a.a")));

    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_PRE, Pattern("https://[*.]a.b").Compare(Pattern("https://[*.]aa.b")));
    EXPECT_EQ(ContentSettingsPattern::DISJOINT_ORDER_POST, Pattern("https://[*.]aa.b").Compare(Pattern("https://[*.]a.b")));
}

TEST(ContentSettingsPatternTest, CompareWithWildcard)
{
    EXPECT_EQ(ContentSettingsPattern::IDENTITY, ContentSettingsPattern::Wildcard().Compare(ContentSettingsPattern::Wildcard()));
    EXPECT_EQ(ContentSettingsPattern::IDENTITY, ContentSettingsPattern::Wildcard().Compare(Pattern("*")));

    EXPECT_EQ(ContentSettingsPattern::PREDECESSOR, Pattern("[*.]google.com").Compare(ContentSettingsPattern::Wildcard()));
    EXPECT_EQ(ContentSettingsPattern::PREDECESSOR, Pattern("[*.]google.com").Compare(Pattern("*")));

    EXPECT_EQ(ContentSettingsPattern::SUCCESSOR, ContentSettingsPattern::Wildcard().Compare(Pattern("[*.]google.com")));
    EXPECT_EQ(ContentSettingsPattern::SUCCESSOR, Pattern("*").Compare(Pattern("[*.]google.com")));

    EXPECT_EQ(ContentSettingsPattern::PREDECESSOR, Pattern("mail.google.com").Compare(ContentSettingsPattern::Wildcard()));
    EXPECT_EQ(ContentSettingsPattern::PREDECESSOR, Pattern("mail.google.com").Compare(Pattern("*")));

    EXPECT_EQ(ContentSettingsPattern::SUCCESSOR, ContentSettingsPattern::Wildcard().Compare(Pattern("mail.google.com")));
    EXPECT_EQ(ContentSettingsPattern::SUCCESSOR, Pattern("*").Compare(Pattern("mail.google.com")));
}
...
TEST(ContentSettingsPatternTest, PatternSupport_Legacy)
{
    ...
    EXPECT_TRUE(Pattern("[*.]example.com").Matches(GURL("http://www.example.com/")));
    EXPECT_TRUE(Pattern("www.example.com").Matches(GURL("http://www.example.com/")));
    EXPECT_TRUE(Pattern("file:///tmp/test.html").Matches(GURL("file:///tmp/test.html")));
    EXPECT_FALSE(Pattern(std::string()).Matches(GURL("http://www.example.com/")));
    EXPECT_FALSE(Pattern("[*.]example.com").Matches(GURL("http://example.org/")));
    EXPECT_FALSE(Pattern("example.com").Matches(GURL("http://example.org/")));
    EXPECT_FALSE(Pattern("file:///tmp/test.html").Matches(GURL("file:///tmp/other.html")));
    EXPECT_FALSE(Pattern("file:///tmp/test.html").Matches(GURL("http://example.org/")));
}
...
TEST(ContentSettingsPatternTest, CanonicalizePattern_Legacy)
{
    ...
    EXPECT_STREQ("192.168.1.1", Pattern("192.168.1.1").ToString().c_str());
    EXPECT_STREQ("[::1]", Pattern("[::1]").ToString().c_str());
    EXPECT_STREQ("file:///tmp/file.html", Pattern("file:///tmp/file.html").ToString().c_str());
...
{
    EXPECT_EQ(ContentSettingsPattern::SCHEME_HTTP, Pattern("http://www.example.com").GetScheme());
    EXPECT_EQ(ContentSettingsPattern::SCHEME_HTTPS, Pattern("https://www.example.com").GetScheme());
    EXPECT_EQ(ContentSettingsPattern::SCHEME_FILE, Pattern("file:///tmp/file.html").GetScheme());
    EXPECT_EQ(ContentSettingsPattern::SCHEME_CHROMEEXTENSION, Pattern("chrome-extension://peoadpeiejnhkmpaakpnompolbglelel/").GetScheme());
    EXPECT_EQ(ContentSettingsPattern::SCHEME_CHROMESEARCH, Pattern("chrome-search://local-ntp/").GetScheme());
    EXPECT_EQ(ContentSettingsPattern::SCHEME_WILDCARD, Pattern("192.168.0.1").GetScheme());
    EXPECT_EQ(ContentSettingsPattern::SCHEME_WILDCARD, Pattern("www.example.com").GetScheme());
    EXPECT_EQ(ContentSettingsPattern::SCHEME_OTHER, Pattern("filesystem:http://www.google.com/temporary/").GetScheme());
    EXPECT_EQ(ContentSettingsPattern::SCHEME_CHROME, Pattern("chrome://sample/").GetScheme());
    EXPECT_EQ(ContentSettingsPattern::SCHEME_CHROMEUNTRUSTED, Pattern("chrome-untrusted://sample/").GetScheme());
    EXPECT_EQ(ContentSettingsPattern::SCHEME_DEVTOOLS, Pattern("devtools://devtools/").GetScheme());
}
...
TEST(ContentSettingsPatternTest, MatchesSingleOrigin)
{
    ...
    // URL conversion.
    EXPECT_FALSE(ContentSettingsPattern::FromURL(GURL("https://example.com")).MatchesSingleOrigin());
    EXPECT_TRUE(ContentSettingsPattern::FromURLNoWildcard(GURL("https://example.com")).MatchesSingleOrigin());
    EXPECT_TRUE(ContentSettingsPattern::FromURL(GURL("file:///foo/bar/example.txt")).MatchesSingleOrigin());
}
