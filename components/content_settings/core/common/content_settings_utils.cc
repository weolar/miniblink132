...
bool ParseContentSettingValue(const base::Value& value, ContentSetting* setting)
{
    if (value.is_none()) {
...
{
    if (setting <= CONTENT_SETTING_DEFAULT || setting >= CONTENT_SETTING_NUM_SETTINGS) {
        return base::Value();
    }
...
bool PatternAppliesToSingleOrigin(const ContentSettingsPattern& primary_pattern, const ContentSettingsPattern& secondary_pattern)
{
