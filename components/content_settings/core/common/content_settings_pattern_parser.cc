...
void PatternParser::Parse(base::StringPiece pattern_spec, ContentSettingsPattern::BuilderInterface* builder)
{
    if (pattern_spec == "*") {
    ...
    const base::StringPiece standard_scheme_separator(url::kStandardSchemeSeparator);
    ...
    current_pos = pattern_spec.find_first_of(kUrlPortAndPathSeparator, current_pos);
    if (current_pos == base::StringPiece::npos) {
    ...
    if (!host_piece.empty()) {
        if (host_piece == kHostWildcard) {
            if (ContentSettingsPattern::IsNonWildcardDomainNonPortScheme(scheme_piece)) {
                builder->Invalid();
                return;
            }

            builder->WithDomainWildcard();
        } else if (base::StartsWith(host_piece, kDomainWildcard, base::CompareCase::SENSITIVE)) {
            if (ContentSettingsPattern::IsNonWildcardDomainNonPortScheme(scheme_piece)) {
                builder->Invalid();
                return;
            }

            if (base::StartsWith(host_piece, kDomainWildcardWithSuperfluousDot, base::CompareCase::SENSITIVE)) {
                builder->Invalid();
                return;
            }
    ...
    } else if (!ContentSettingsPattern::IsNonWildcardDomainNonPortScheme(scheme_piece) && !base::EqualsCaseInsensitiveASCII(scheme_piece, url::kFileScheme)) {
        builder->WithPortWildcard();
    }
...
// static
std::string PatternParser::ToString(const ContentSettingsPattern::PatternParts& parts)
{
    // Return the most compact form to support legacy code and legacy pattern
    // strings.
    if (parts.is_scheme_wildcard && parts.has_domain_wildcard && parts.host.empty() && parts.is_port_wildcard) {
        return "*";
    }
