...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    ContentSettingsPattern recanonicalized_pattern = Parse(canonical_pattern_spec);
    CHECK(recanonicalized_pattern.IsValid()) << "Could not recanonicalize\n  '" << canonical_pattern_spec << "' (originally '" << pattern_spec << "')";
    CHECK_EQ(recanonicalized_pattern.ToString(), canonical_pattern_spec) << "\n  (originally '" << pattern_spec << "')";
    CHECK_EQ(recanonicalized_pattern.Compare(canonical_pattern), ContentSettingsPattern::Relation::IDENTITY)
        << "Canonical pattern\n"
