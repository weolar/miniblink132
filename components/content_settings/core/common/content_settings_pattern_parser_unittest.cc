...
TEST(ContentSettingsPatternParserTest, ParsePatterns)
{
    ...
    // case.)
    EXPECT_CALL(builder, WithSchemeWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithDomainWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPortWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("*", &builder);
    ::testing::Mock::VerifyAndClear(&builder);

    EXPECT_CALL(builder, WithScheme("http")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithHost("www.youtube.com")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPort("8080")).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("http://www.youtube.com:8080", &builder);
    ::testing::Mock::VerifyAndClear(&builder);

    EXPECT_CALL(builder, WithSchemeWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithHost("www.gmail.com")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPort("80")).Times(1).WillOnce(::testing::Return(&builder));
    ...
    EXPECT_CALL(builder, WithScheme("http")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithDomainWildcard()).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithHost("google.com")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPort("80")).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("http://[*.]google.com:80", &builder);
    ::testing::Mock::VerifyAndClear(&builder);

    EXPECT_CALL(builder, WithScheme("https")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithHost("[::1]")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPort("8080")).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("https://[::1]:8080", &builder);
    ::testing::Mock::VerifyAndClear(&builder);

    EXPECT_CALL(builder, WithScheme("http")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithHost("127.0.0.1")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPort("8080")).Times(1).WillOnce(::testing::Return(&builder));
    ...
    EXPECT_CALL(builder, WithSchemeWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithDomainWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithHost("youtube.com")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPortWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    ...
    EXPECT_CALL(builder, WithSchemeWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, Invalid()).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("*youtube.com", &builder);
    ::testing::Mock::VerifyAndClear(&builder);
...
TEST(ContentSettingsPatternParserTest, ParseFilePatterns)
{
    ...
    EXPECT_CALL(builder, WithScheme("file")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPath("/foo/bar/test.html")).Times(1).WillOnce(::testing::Return(&builder));
    ...
    EXPECT_CALL(builder, WithScheme("file")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithDomainWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPath("/")).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("file://*/", &builder);
    ::testing::Mock::VerifyAndClear(&builder);

    EXPECT_CALL(builder, WithScheme("file")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithDomainWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPathWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("file://*/*", &builder);
    ::testing::Mock::VerifyAndClear(&builder);
...
TEST(ContentSettingsPatternParserTest, ParseChromePatterns)
{
    ...
    EXPECT_CALL(builder, WithScheme("chrome-extension")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithHost("peoadpeiejnhkmpaakpnompolbglelel")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPath("/")).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("chrome-extension://peoadpeiejnhkmpaakpnompolbglelel/", &builder);
    ::testing::Mock::VerifyAndClear(&builder);

    // Valid chrome-search:// URL.
    EXPECT_CALL(builder, WithScheme("chrome-search")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithHost("local-ntp")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPath("/local-ntp.html")).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("chrome-search://local-ntp/local-ntp.html", &builder);
    ::testing::Mock::VerifyAndClear(&builder);

    // Not a non-domain wildcard non-port scheme implies a port is parsed.
    EXPECT_CALL(builder, WithScheme("chrome-not-search")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithHost("local-ntp")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPath("/local-ntp.html")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, WithPortWildcard()).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("chrome-not-search://local-ntp/local-ntp.html", &builder);
    ::testing::Mock::VerifyAndClear(&builder);
}

TEST(ContentSettingsPatternParserTest, ParseInvalidNonDomainWildcardNonPortPatterns)
{
    ::testing::StrictMock<MockBuilder> builder;
    ...
    EXPECT_CALL(builder, WithScheme("chrome-search")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, Invalid()).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("chrome-search://*/local-ntp.html", &builder);
    ::testing::Mock::VerifyAndClear(&builder);
    ...
    EXPECT_CALL(builder, WithHost("local-ntp")).Times(1).WillOnce(::testing::Return(&builder));
    EXPECT_CALL(builder, Invalid()).Times(1).WillOnce(::testing::Return(&builder));
    content_settings::PatternParser::Parse("chrome-search://local-ntp:65535/local-ntp.html", &builder);
    ::testing::Mock::VerifyAndClear(&builder);
...
TEST(ContentSettingsPatternParserTest, SerializePatterns)
{
    ...
    parts.port = "8080";
    EXPECT_STREQ("http://www.youtube.com:8080", content_settings::PatternParser::ToString(parts).c_str());

    parts = ContentSettingsPattern::PatternParts();
    parts.scheme = "file";
    parts.path = "/foo/bar/test.html";
    EXPECT_STREQ("file:///foo/bar/test.html", content_settings::PatternParser::ToString(parts).c_str());
    ...
    EXPECT_EQ("chrome-search://local-ntp/", content_settings::PatternParser::ToString(parts));

    parts = ContentSettingsPattern::PatternParts();
    parts.scheme = "chrome-extension";
    parts.host = "peoadpeiejnhkmpaakpnompolbglelel";
    EXPECT_EQ("chrome-extension://peoadpeiejnhkmpaakpnompolbglelel/", content_settings::PatternParser::ToString(parts));
}

TEST(ContentSettingsPatternParserTest, IdempotencyOfCanonicalization)
{
    const std::string pattern_specs[] = {
        "abc", "https://chromium.org", "file:///foo/", "file:///foo/:/bar/:/baz", "https://foo/:/bar/:/baz", "file://:/path",
    ...
    for (const std::string& spec : pattern_specs) {
        ...
        EXPECT_EQ(pattern.Compare(pattern2), ContentSettingsPattern::Relation::IDENTITY);
    }
}
