...
VKAPI_ATTR VkBool32 VKAPI_CALL VulkanErrorCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT object_type, uint64_t object, size_t location,
...
VKAPI_ATTR VkBool32 VKAPI_CALL VulkanWarningCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT object_type, uint64_t object, size_t location,
    int32_t message_code, const char* layer_prefix, const char* message, void* user_data)
{
    LOG(WARNING) << message;
    return VK_FALSE;
...
bool VulkanInstance::Initialize(
    const base::FilePath& vulkan_loader_library_path, const std::vector<const char*>& required_extensions, const std::vector<const char*>& required_layers)
{
...
bool VulkanInstance::BindUnassignedFunctionPointers(const base::FilePath& vulkan_loader_library_path)
{
    VulkanFunctionPointers* vulkan_function_pointers = gpu::GetVulkanFunctionPointers();
    if (is_from_angle_) {
        ...
        if (!vulkan_function_pointers->BindUnassociatedFunctionPointersFromGetProcAddr(proc)) {
            return false;
        }
    } else {
        base::NativeLibraryLoadError error;
        loader_library_ = base::LoadNativeLibrary(vulkan_loader_library_path, &error);
        if (!loader_library_) {
            LOG(ERROR) << "Failed to load '" << vulkan_loader_library_path << "': " << error.ToString();
            return false;
        }
        if (!vulkan_function_pointers->BindUnassociatedFunctionPointersFromLoaderLib(loader_library_)) {
            return false;
        }
    }
    return true;
...
bool VulkanInstance::InitializeInstace(const std::vector<const char*>& required_extensions, const std::vector<const char*>& required_layers)
{
    if (is_from_angle_)
...
bool VulkanInstance::CreateInstance(const std::vector<const char*>& required_extensions, const std::vector<const char*>& required_layers)
{
    DCHECK(!vk_instance_);
    ...
    for (const VkExtensionProperties& ext_property : vulkan_info_.instance_extensions) {
    ...
    gfx::ExtensionSet enabled_extensions(std::begin(vulkan_info_.enabled_instance_extensions), std::end(vulkan_info_.enabled_instance_extensions));
    ...
    if (debug_report_enabled_) {
        ...
        result = vkCreateDebugReportCallbackEXT(vk_instance_, &cb_create_info, nullptr, &error_callback_);
        if (VK_SUCCESS != result) {
        ...
        result = vkCreateDebugReportCallbackEXT(vk_instance_, &cb_create_info, nullptr, &warning_callback_);
        if (VK_SUCCESS != result) {
...
bool VulkanInstance::InitializeFromANGLE(const std::vector<const char*>& required_extensions, const std::vector<const char*>& required_layers)
{
...
bool VulkanInstance::CollectBasicInfo(const std::vector<const char*>& required_layers)
{
    ...
    gpu::crash_keys::vulkan_api_version.Set(VkVersionToString(vulkan_info_.api_version));
    ...
    for (const char* layer_name : all_required_layers) {
        ...
        if (VK_SUCCESS != result) {
            DLOG(ERROR) << "vkEnumerateInstanceExtensionProperties(" << layer_name << ") failed: " << result;
            return false;
        }
        ...
        result = vkEnumerateInstanceExtensionProperties(layer_name, &num_instance_exts, &vulkan_info_.instance_extensions.data()[previous_extension_count]);
        if (VK_SUCCESS != result) {
            DLOG(ERROR) << "vkEnumerateInstanceExtensionProperties(" << layer_name << ") failed: " << result;
            return false;
        }
    ...
    if (VK_SUCCESS != result) {
        DLOG(ERROR) << "vkEnumerateInstanceLayerProperties(NULL) failed: " << result;
        return false;
    }
...
bool VulkanInstance::CollectDeviceInfo(VkPhysicalDevice physical_device)
{
    ...
    for (VkPhysicalDevice device : physical_devices) {
        ...
        DLOG_IF(ERROR, result != VK_SUCCESS) << "vkEnumerateDeviceExtensionProperties failed: " << result;
        ...
        if (info.properties.apiVersion >= kVulkanRequiredApiVersion) {
            ...
            VkPhysicalDeviceProtectedMemoryFeatures protected_memory_feature = { VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROTECTED_MEMORY_FEATURES };
            ...
            info.feature_sampler_ycbcr_conversion = ycbcr_conversion_features.samplerYcbcrConversion;
...
{
#if DCHECK_IS_ON()
    if (debug_report_enabled_ && (error_callback_ != VK_NULL_HANDLE || warning_callback_ != VK_NULL_HANDLE)) {
