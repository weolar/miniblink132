...
public:
    ...
    bool BindUnassignedFunctionPointers(const base::FilePath& vulkan_loader_library_path);
    bool InitializeInstace(const std::vector<const char*>& required_extensions, const std::vector<const char*>& required_layers);

    const VulkanInfo& vulkan_info() const
    {
        return vulkan_info_;
    }
    ...
    {
        return is_from_angle_;
    }

private:
    bool CreateInstance(const std::vector<const char*>& required_extensions, const std::vector<const char*>& required_layers);
    bool InitializeFromANGLE(const std::vector<const char*>& required_extensions, const std::vector<const char*>& required_layers);
