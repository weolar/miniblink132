...
VkPipelineStageFlags GetPipelineStageFlags(const VulkanDeviceQueue* device_queue, const VkImageLayout layout)
{
    switch (layout) {
...
VkAccessFlags GetAccessMask(const VkImageLayout layout)
{
    ...
    case VK_IMAGE_LAYOUT_GENERAL:
        ...
        return VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT | VK_ACCESS_TRANSFER_WRITE_BIT | VK_ACCESS_TRANSFER_READ_BIT
            | VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_HOST_WRITE_BIT | VK_ACCESS_HOST_READ_BIT | VK_ACCESS_INPUT_ATTACHMENT_READ_BIT
            | VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_MEMORY_READ_BIT | VK_ACCESS_MEMORY_WRITE_BIT;
    case VK_IMAGE_LAYOUT_PREINITIALIZED:
...
VulkanCommandBuffer::VulkanCommandBuffer(VulkanDeviceQueue* device_queue, VulkanCommandPool* command_pool, bool primary)
    : primary_(primary)
...
bool VulkanCommandBuffer::Initialize()
{
    ...
    VkCommandBufferAllocateInfo command_buffer_info = {
        ...
        .level = primary_ ? VK_COMMAND_BUFFER_LEVEL_PRIMARY : VK_COMMAND_BUFFER_LEVEL_SECONDARY,
...
bool VulkanCommandBuffer::Submit(uint32_t num_wait_semaphores, VkSemaphore* wait_semaphores, uint32_t num_signal_semaphores, VkSemaphore* signal_semaphores)
{
    DCHECK(primary_);

    std::vector<VkPipelineStageFlags> wait_dst_stage_mask(num_wait_semaphores, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT);
    ...
    result = vkQueueSubmit(device_queue_->GetVulkanQueue(), 1, &submit_info, fence);
...
void VulkanCommandBuffer::TransitionImageLayout(
    VkImage image, VkImageLayout old_layout, VkImageLayout new_layout, uint32_t src_queue_family_index, uint32_t dst_queue_family_index)
{
    ...
    vkCmdPipelineBarrier(command_buffer_, GetPipelineStageFlags(device_queue_, old_layout), GetPipelineStageFlags(device_queue_, new_layout), 0, 0, nullptr, 0,
        nullptr, 1, &barrier);
}

void VulkanCommandBuffer::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t buffer_width, uint32_t buffer_height, uint32_t width, uint32_t height)
{
    ...
    vkCmdCopyBufferToImage(command_buffer_, buffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
}

void VulkanCommandBuffer::CopyImageToBuffer(VkBuffer buffer, VkImage image, uint32_t buffer_width, uint32_t buffer_height, uint32_t width, uint32_t height)
{
    VkBufferImageCopy region = {};
    region.bufferOffset = 0;
    region.bufferRowLength = buffer_width;
    ...
    region.imageOffset = { 0, 0, 0 };
    region.imageExtent = { width, height, 1 };
    vkCmdCopyImageToBuffer(command_buffer_, image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, buffer, 1, &region);
}
...
ScopedMultiUseCommandBufferRecorder::ScopedMultiUseCommandBufferRecorder(VulkanCommandBuffer& command_buffer)
...
ScopedSingleUseCommandBufferRecorder::ScopedSingleUseCommandBufferRecorder(VulkanCommandBuffer& command_buffer)
    : CommandBufferRecorderBase(command_buffer)
{
