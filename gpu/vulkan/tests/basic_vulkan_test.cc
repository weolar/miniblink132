...
void BasicVulkanTest::SetUp()
{
...
#if defined(USE_OZONE)
    supports_swapchain = ui::OzonePlatform::GetInstance()->GetPlatformProperties().supports_vulkan_swap_chain;
#endif

    bool use_swiftshader = base::CommandLine::ForCurrentProcess()->HasSwitch("use-swiftshader");
    ...
    device_queue_ = gpu::CreateVulkanDeviceQueue(vulkan_implementation_.get(), flags);
...
std::unique_ptr<VulkanSurface> BasicVulkanTest::CreateViewSurface(gfx::AcceleratedWidget window)
{
    return window_ != gfx::kNullAcceleratedWidget ? vulkan_implementation_->CreateViewSurface(window) : nullptr;
}
