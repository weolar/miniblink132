...
public:
    ...
    using GetPresentationSupportCallback
        = base::RepeatingCallback<bool(VkPhysicalDevice, const std::vector<VkQueueFamilyProperties>&, uint32_t queue_family_index)>;
    bool Initialize(uint32_t options, const GPUInfo* gpu_info, const std::vector<const char*>& required_extensions,
        ...
        uint32_t heap_memory_limit, const bool is_thread_safe = false);

    bool InitializeFromANGLE();

    bool InitializeForWebView(
    ...
    bool InitializeForCompositorGpuThread(VkPhysicalDevice vk_physical_device, VkDevice vk_device, VkQueue vk_queue, uint32_t vk_queue_index,
    ...
    {
        DCHECK_NE(static_cast<VkPhysicalDevice>(VK_NULL_HANDLE), vk_physical_device_);
    ...
    const VkPhysicalDeviceDriverProperties& vk_physical_device_driver_properties() const
    {
    ...
    uint32_t GetVulkanQueueIndex() const
    {
        return vk_queue_index_;
    }
    ...
    VulkanFenceHelper* GetFenceHelper() const
    {
        return cleanup_helper_.get();
    }
    ...
    bool allow_protected_memory() const
    {
        return allow_protected_memory_;
    }

private:
    // Common Init method to be used by both webview and compositor gpu thread.
    bool InitCommon(VkPhysicalDevice vk_physical_device, VkDevice vk_device, VkQueue vk_queue, uint32_t vk_queue_index, gfx::ExtensionSet enabled_extensions);
    ...
    VkPhysicalDeviceFeatures2 enabled_device_features_2_ { VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2 };
    raw_ptr<const VkPhysicalDeviceFeatures2> enabled_device_features_2_from_angle_ = nullptr;
    ...
    VkPhysicalDeviceProtectedMemoryFeatures protected_memory_features_ { VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SAMPLER_YCBCR_CONVERSION_FEATURES };
};
