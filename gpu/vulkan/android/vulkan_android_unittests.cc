...
public:
    void SetUp() override
    {
        ...
        vk_context_provider_ = viz::VulkanInProcessContextProvider::Create(vk_implementation_.get());
        ...
        vk_phy_device_ = vk_context_provider_->GetDeviceQueue()->GetVulkanPhysicalDevice();
...
TEST_F(VulkanImplementationAndroidTest, ExportImportSyncFd)
{
    ...
    EXPECT_TRUE(SubmitSignalVkSemaphore(vk_context_provider_->GetDeviceQueue()->GetVulkanQueue(), semaphore1));

    // Export a handle from the semaphore.
    SemaphoreHandle handle = vk_implementation_->GetSemaphoreHandle(vk_device_, semaphore1);
    EXPECT_TRUE(handle.is_valid());

    // Import the above semaphore handle into a new semaphore.
    VkSemaphore semaphore2 = vk_implementation_->ImportSemaphoreHandle(vk_device_, std::move(handle));
...
TEST_F(VulkanImplementationAndroidTest, CreateVkImageFromAHB)
{
    ...
    auto vulkan_image = VulkanImage::CreateFromGpuMemoryBufferHandle(device_queue, std::move(gmb_handle), size, VK_FORMAT_R8G8B8A8_UNORM,
