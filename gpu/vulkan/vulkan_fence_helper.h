...
public:
    ...
    using CleanupTask = base::OnceCallback<void(VulkanDeviceQueue* device_queue, bool device_lost)>;
    ...
    void EnqueueImageCleanupForSubmittedWork(VkImage image, VkDeviceMemory memory);
    void EnqueueBufferCleanupForSubmittedWork(VkBuffer buffer, VmaAllocation allocation);
    // Helpers for VulkanCommandBuffer, VulkanCommandPool, etc
    template <typename T> void EnqueueVulkanObjectCleanupForSubmittedWork(std::unique_ptr<T> obj);
...
template <typename T> void VulkanFenceHelper::EnqueueVulkanObjectCleanupForSubmittedWork(std::unique_ptr<T> obj)
{
    if (!obj)
        return;
    EnqueueCleanupTaskForSubmittedWork(
        base::BindOnce([](std::unique_ptr<T> obj, VulkanDeviceQueue* device_queue, bool device_lost) { obj->Destroy(); }, std::move(obj)));
}
