...
VkSemaphore CreateSemaphore(VkDevice vk_device)
{
    ...
    constexpr VkSemaphoreCreateInfo semaphore_create_info = { VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO };

    VkSemaphore vk_semaphore = VK_NULL_HANDLE;
    auto result = vkCreateSemaphore(vk_device, &semaphore_create_info,
        /*pAllocator=*/nullptr, &vk_semaphore);
    LOG_IF(FATAL, VK_SUCCESS != result) << "vkCreateSemaphore() failed: " << result;
    return vk_semaphore;
}
...
bool VulkanSwapChain::Initialize(VulkanDeviceQueue* device_queue, VkSurfaceKHR surface, const VkSurfaceFormatKHR& surface_format, const gfx::Size& image_size,
    uint32_t min_image_count, VkImageUsageFlags image_usage_flags, VkSurfaceTransformFlagBitsKHR pre_transform, VkCompositeAlphaFlagBitsKHR composite_alpha,
    std::unique_ptr<VulkanSwapChain> old_swap_chain)
{
    ...
    is_incremental_present_supported_ = gfx::HasExtension(device_queue_->enabled_extensions(), VK_KHR_INCREMENTAL_PRESENT_EXTENSION_NAME);
    device_queue_->GetFenceHelper()->ProcessCleanupTasks();
    return InitializeSwapChain(
               surface, surface_format, image_size, min_image_count, image_usage_flags, pre_transform, composite_alpha, std::move(old_swap_chain))
...
void VulkanSwapChain::Destroy()
{
    ...
    if (UNLIKELY(!pending_semaphores_queue_.empty())) {
        ...
        fence_helper->EnqueueCleanupTaskForSubmittedWork(base::BindOnce(
            [](base::circular_deque<PendingSemaphores> pending_semaphores_queue, VulkanDeviceQueue* device_queue, bool device_lost) {
...
void VulkanSwapChain::PostSubBufferAsync(const gfx::Rect& rect, PostSubBufferCompletionCallback callback)
{
    base::AutoLock auto_lock(lock_);
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    if (UNLIKELY(!PresentBuffer(rect))) {
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(std::move(callback), gfx::SwapResult::SWAP_FAILED));
        return;
    }
    ...
    post_sub_buffer_task_runner_->PostTask(FROM_HERE,
        base::BindOnce(
            [](VulkanSwapChain* self, scoped_refptr<base::SingleThreadTaskRunner> task_runner, PostSubBufferCompletionCallback callback) {
                base::AutoLock auto_lock(self->lock_);
                DCHECK(self->has_pending_post_sub_buffer_);
                auto swap_result = self->AcquireNextImage() ? gfx::SwapResult::SWAP_ACK : gfx::SwapResult::SWAP_FAILED;
                task_runner->PostTask(FROM_HERE, base::BindOnce(std::move(callback), swap_result));
                self->has_pending_post_sub_buffer_ = false;
                self->condition_variable_.Signal();
            },
            base::Unretained(this), base::ThreadTaskRunnerHandle::Get(), std::move(callback)));
}

bool VulkanSwapChain::InitializeSwapChain(VkSurfaceKHR surface, const VkSurfaceFormatKHR& surface_format, const gfx::Size& image_size, uint32_t min_image_count,
    VkImageUsageFlags image_usage_flags, VkSurfaceTransformFlagBitsKHR pre_transform, VkCompositeAlphaFlagBitsKHR composite_alpha,
    std::unique_ptr<VulkanSwapChain> old_swap_chain)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    VkSwapchainCreateInfoKHR swap_chain_create_info = {
        ...
        .imageExtent = { static_cast<uint32_t>(image_size.width()), static_cast<uint32_t>(image_size.height()) },
        .imageArrayLayers = 1,
    ...
    if (LIKELY(old_swap_chain)) {
        ...
        pending_semaphores_queue_ = std::move(old_swap_chain->pending_semaphores_queue_);
    ...
    if (LIKELY(old_swap_chain)) {
        auto* fence_helper = device_queue_->GetFenceHelper();
        fence_helper->EnqueueVulkanObjectCleanupForSubmittedWork(std::move(old_swap_chain));
    }
    ...
    size_ = gfx::Size(swap_chain_create_info.imageExtent.width, swap_chain_create_info.imageExtent.height);

    if (UNLIKELY(!post_sub_buffer_task_runner_)) {
        post_sub_buffer_task_runner_
            = base::ThreadPool::CreateSequencedTaskRunner({ base::TaskPriority::USER_BLOCKING, base::TaskShutdownBehavior::BLOCK_SHUTDOWN, base::MayBlock() });
    }
...
bool VulkanSwapChain::InitializeSwapImages(const VkSurfaceFormatKHR& surface_format)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    result = vkGetSwapchainImagesKHR(device, swap_chain_, &image_count, images.data());
    if (UNLIKELY(VK_SUCCESS != result)) {
...
bool VulkanSwapChain::BeginWriteCurrentImage(VkImage* image, uint32_t* image_index, VkImageLayout* image_layout, VkImageUsageFlags* image_usage,
    VkSemaphore* begin_semaphore, VkSemaphore* end_semaphore)
{
    ...
    if (UNLIKELY(!new_acquired_)) {
        ...
        device_queue_->GetFenceHelper()->EnqueueSemaphoreCleanupForSubmittedWork(current_image_data.acquire_semaphore);
        ...
        current_image_data.present_semaphore = CreateSemaphore(device_queue_->GetVulkanDevice());
...
bool VulkanSwapChain::PresentBuffer(const gfx::Rect& rect)
{
    ...
    VkRectLayerKHR rect_layer = {
        ...
        .extent = { static_cast<uint32_t>(rect.width()), static_cast<uint32_t>(rect.height()) },
        .layer = 0,
    };
    ...
    auto result = ({
        static auto* kCrashKey = base::debug::AllocateCrashKeyString("inside_queue_present", base::debug::CrashKeySize::Size32);
...
bool VulkanSwapChain::AcquireNextImage()
{
    ...
    auto result = ({
        ...
        static auto* kCrashKey = base::debug::AllocateCrashKeyString("inside_acquire_next_image", base::debug::CrashKeySize::Size32);
        base::debug::ScopedCrashKeyString scoped_crash_key(kCrashKey, "1");
        vkAcquireNextImageKHR(device, swap_chain_, acquire_next_image_timeout_ns_, acquire_semaphore, /*fence=*/VK_NULL_HANDLE, &next_image);
    });
    ...
    ReturnSemaphores(current_image_data.acquire_semaphore, current_image_data.present_semaphore);
...
bool VulkanSwapChain::GetOrCreateSemaphores(VkSemaphore* acquire_semaphore, VkSemaphore* present_semaphore)
{
...
void VulkanSwapChain::ReturnSemaphores(VkSemaphore acquire_semaphore, VkSemaphore present_semaphore)
{
    DCHECK_EQ(acquire_semaphore != VK_NULL_HANDLE, present_semaphore != VK_NULL_HANDLE);
...
{
    success_ = swap_chain_->BeginWriteCurrentImage(&image_, &image_index_, &image_layout_, &image_usage_, &begin_semaphore_, &end_semaphore_);
    if (LIKELY(success_)) {
...
const VulkanSwapChain::ScopedWrite& VulkanSwapChain::ScopedWrite::operator=(ScopedWrite&& other)
{
    Reset();
