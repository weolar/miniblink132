...
public:
    explicit VulkanImplementation(bool use_swiftshader = false, bool allow_protected_memory = false);
    ...
    virtual std::unique_ptr<VulkanSurface> CreateViewSurface(gfx::AcceleratedWidget window) = 0;
    ...
    virtual std::unique_ptr<gfx::GpuFence> ExportVkFenceToGpuFence(VkDevice vk_device, VkFence vk_fence) = 0;
    ...
    virtual VkSemaphore ImportSemaphoreHandle(VkDevice vk_device, SemaphoreHandle handle) = 0;

    // Export a platform-specific handle for a Vulkan semaphore. Returns a null
    // handle in case of a failure.
    virtual SemaphoreHandle GetSemaphoreHandle(VkDevice vk_device, VkSemaphore vk_semaphore) = 0;
    ...
    virtual bool CanImportGpuMemoryBuffer(VulkanDeviceQueue* device_queue, gfx::GpuMemoryBufferType memory_buffer_type) = 0;
    ...
    virtual bool GetSamplerYcbcrConversionInfo(const VkDevice& vk_device, base::android::ScopedHardwareBufferHandle ahb_handle, VulkanYCbCrInfo* ycbcr_info)
        = 0;
#endif
    ...
    virtual std::unique_ptr<SysmemBufferCollection> RegisterSysmemBufferCollection(VkDevice device, gfx::SysmemBufferCollectionId id, zx::channel token,
        gfx::BufferFormat format, gfx::BufferUsage usage, gfx::Size size, size_t min_buffer_count, bool register_with_image_pipe)
        = 0;
#endif // BUILDFLAG(IS_FUCHSIA)

    bool use_swiftshader() const
    {
...
std::unique_ptr<VulkanDeviceQueue> CreateVulkanDeviceQueue(VulkanImplementation* vulkan_implementation, uint32_t option, const GPUInfo* gpu_info = nullptr,
    uint32_t heap_memory_limit = 0, const bool is_thread_safe = false);
