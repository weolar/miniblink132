...
VulkanFenceHelper::FenceHandle::FenceHandle(VkFence fence, uint64_t generation_id)
    : fence_(fence)
...
void VulkanFenceHelper::ProcessCleanupTasks(uint64_t retired_generation_id)
{
    ...
    for (const auto& tasks_for_fence : cleanup_tasks_) {
        ...
        if (result == VK_SUCCESS) {
            retired_generation_id = std::max(tasks_for_fence.generation_id, retired_generation_id);
            continue;
        }
...
base::OnceClosure VulkanFenceHelper::CreateExternalCallback()
{
    ...
    return base::BindOnce(
        [](base::WeakPtr<VulkanFenceHelper> fence_helper, uint64_t generation_id) {
            if (!fence_helper)
                return;
...
void VulkanFenceHelper::EnqueueSemaphoreCleanupForSubmittedWork(VkSemaphore semaphore)
{
...
void VulkanFenceHelper::EnqueueSemaphoresCleanupForSubmittedWork(std::vector<VkSemaphore> semaphores)
{
    ...
    EnqueueCleanupTaskForSubmittedWork(base::BindOnce(
        [](std::vector<VkSemaphore> semaphores, VulkanDeviceQueue* device_queue, bool /* is_lost */) {
...
void VulkanFenceHelper::EnqueueImageCleanupForSubmittedWork(VkImage image, VkDeviceMemory memory)
{
    ...
    EnqueueCleanupTaskForSubmittedWork(base::BindOnce(
        [](VkImage image, VkDeviceMemory memory, VulkanDeviceQueue* device_queue, bool /* is_lost */) {
...
void VulkanFenceHelper::EnqueueBufferCleanupForSubmittedWork(VkBuffer buffer, VmaAllocation allocation)
{
    ...
    EnqueueCleanupTaskForSubmittedWork(base::BindOnce([](VkBuffer buffer, VmaAllocation allocation, VulkanDeviceQueue* device_queue,
                                                          bool /* is_lost */) { vma::DestroyBuffer(device_queue->vma_allocator(), buffer, allocation); },
        buffer, allocation));
}
...
VulkanFenceHelper::TasksForFence::TasksForFence(FenceHandle handle, std::vector<CleanupTask> tasks)
    : fence(handle.fence_)
    , generation_id(handle.generation_id_)
    , tasks(std::move(tasks))
{
}
VulkanFenceHelper::TasksForFence::TasksForFence(uint64_t generation_id, std::vector<CleanupTask> tasks)
