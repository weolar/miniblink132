...
SemaphoreHandle::SemaphoreHandle(VkExternalSemaphoreHandleTypeFlagBits type, PlatformHandle handle)
    : type_(type)
...
SemaphoreHandle::SemaphoreHandle(gfx::GpuFenceHandle fence_handle)
{
    ...
    Init(VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_ZIRCON_EVENT_BIT_FUCHSIA, std::move(fence_handle.owned_event));
#elif BUILDFLAG(IS_POSIX)
    Init(VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_FD_BIT_KHR, std::move(fence_handle.owned_fd));
#elif BUILDFLAG(IS_WIN)
    Init(VK_EXTERNAL_SEMAPHORE_HANDLE_TYPE_OPAQUE_WIN32_BIT, std::move(fence_handle.owned_handle));
#endif // BUILDFLAG(IS_FUCHSIA)
}

void SemaphoreHandle::Init(VkExternalSemaphoreHandleTypeFlagBits type, PlatformHandle handle)
{
    type_ = type;
    handle_ = std::move(handle);
}
...
#if BUILDFLAG(IS_POSIX)
    return SemaphoreHandle(type_, base::ScopedFD(HANDLE_EINTR(dup(handle_.get()))));
#elif BUILDFLAG(IS_WIN)
    HANDLE handle_dup;
    if (!::DuplicateHandle(::GetCurrentProcess(), handle_.Get(), ::GetCurrentProcess(), &handle_dup, 0, FALSE, DUPLICATE_SAME_ACCESS)) {
        return SemaphoreHandle();
    }
