...
VulkanDeviceQueue::VulkanDeviceQueue(VulkanInstance* instance)
    : vk_instance_(instance->vk_instance())
    , instance_(instance)
{
}
...
bool VulkanDeviceQueue::Initialize(uint32_t options, const GPUInfo* gpu_info, const std::vector<const char*>& required_extensions,
    const std::vector<const char*>& optional_extensions, bool allow_protected_memory, const GetPresentationSupportCallback& get_presentation_support,
    uint32_t heap_memory_limit, const bool is_thread_safe)
{
    DCHECK_EQ(static_cast<VkPhysicalDevice>(VK_NULL_HANDLE), vk_physical_device_);
    ...
    for (size_t i = 0; i < info.physical_devices.size(); ++i) {
        ...
        if (gpu_info && (device_properties.vendorID != gpu_info->gpu.vendor_id || device_properties.deviceID != gpu_info->gpu.device_id)) {
            continue;
        }
#endif

        if (device_properties.deviceType < 0 || device_properties.deviceType > VK_PHYSICAL_DEVICE_TYPE_CPU) {
            DLOG(ERROR) << "Unsupported device type: " << device_properties.deviceType;
            continue;
        }
        ...
        for (size_t n = 0; n < device_info.queue_families.size(); ++n) {
            if ((device_info.queue_families[n].queueFlags & queue_flags) != queue_flags) {
                continue;
            }

            if (options & DeviceQueueOption::PRESENTATION_SUPPORT_QUEUE_FLAG && !get_presentation_support.Run(device, device_info.queue_families, n)) {
                continue;
            }
    ...
    vk_physical_device_driver_properties_ = physical_device_info.driver_properties;
    ...
    queue_create_info.flags = allow_protected_memory ? VK_DEVICE_QUEUE_CREATE_PROTECTED_BIT : 0;

    std::vector<const char*> enabled_extensions;
    for (const char* extension : required_extensions) {
        if (base::ranges::none_of(
                physical_device_info.extensions, [extension](const VkExtensionProperties& p) { return std::strcmp(extension, p.extensionName) == 0; })) {
            ...
            DLOG(ERROR) << "Required Vulkan extension " << extension << " is not supported.";
            return false;
#endif
        }
    ...
    for (const char* extension : optional_extensions) {
        if (base::ranges::none_of(
                physical_device_info.extensions, [extension](const VkExtensionProperties& p) { return std::strcmp(extension, p.extensionName) == 0; })) {
            DLOG(ERROR) << "Optional Vulkan extension " << extension << " is not supported.";
        } else {
            enabled_extensions.push_back(extension);
        }
    }

    crash_keys::vulkan_device_api_version.Set(VkVersionToString(vk_physical_device_properties_.apiVersion));
    if (vk_physical_device_properties_.vendorID == 0x10DE) {
        ...
        crash_keys::vulkan_device_driver_version.Set(base::StringPrintf("%d.%d.%d.%d", major, minor, secondary_branch, tertiary_branch));
    } else {
        crash_keys::vulkan_device_driver_version.Set(VkVersionToString(vk_physical_device_properties_.driverVersion));
    }
    crash_keys::vulkan_device_vendor_id.Set(base::StringPrintf("0x%04x", vk_physical_device_properties_.vendorID));
    crash_keys::vulkan_device_id.Set(base::StringPrintf("0x%04x", vk_physical_device_properties_.deviceID));
    ...
    if (allow_protected_memory) {
        ...
        protected_memory_features_ = { VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROTECTED_MEMORY_FEATURES };
    ...
    result = vkCreateDevice(vk_physical_device_, &device_create_info, nullptr, &owned_vk_device_);
    if (VK_SUCCESS != result) {
        DLOG(ERROR) << "vkCreateDevice failed. result:" << result;
        return false;
    }

    enabled_extensions_ = gfx::ExtensionSet(std::begin(enabled_extensions), std::end(enabled_extensions));

    if (!gpu::GetVulkanFunctionPointers()->BindDeviceFunctionPointers(owned_vk_device_, info.used_api_version, enabled_extensions_)) {
    ...
    std::vector<VkDeviceSize> heap_size_limit(VK_MAX_MEMORY_HEAPS, heap_memory_limit ? heap_memory_limit : VK_WHOLE_SIZE);
    vma::CreateAllocator(vk_physical_device_, vk_device_, vk_instance_, heap_size_limit.data(), is_thread_safe, &owned_vma_allocator_);
...
bool VulkanDeviceQueue::InitCommon(
    VkPhysicalDevice vk_physical_device, VkDevice vk_device, VkQueue vk_queue, uint32_t vk_queue_index, gfx::ExtensionSet enabled_extensions)
{
    DCHECK_EQ(static_cast<VkPhysicalDevice>(VK_NULL_HANDLE), vk_physical_device_);
    DCHECK_EQ(static_cast<VkDevice>(VK_NULL_HANDLE), owned_vk_device_);
    DCHECK_EQ(static_cast<VkDevice>(VK_NULL_HANDLE), vk_device_);
...
bool VulkanDeviceQueue::InitializeFromANGLE()
{
    ...
    if (!gpu::GetVulkanFunctionPointers()->BindDeviceFunctionPointers(vk_device, info.used_api_version, enabled_extensions)) {
        return false;
    }

    enabled_device_features_2_from_angle_ = gl::QueryVkEnabledDeviceFeaturesFromANGLE();
    if (!enabled_device_features_2_from_angle_)
        return false;

    return InitCommon(vk_physical_device, vk_device, vk_queue, vk_queue_index, enabled_extensions);
}
...
bool VulkanDeviceQueue::InitializeForCompositorGpuThread(VkPhysicalDevice vk_physical_device, VkDevice vk_device, VkQueue vk_queue, uint32_t vk_queue_index,
    gfx::ExtensionSet enabled_extensions, const VkPhysicalDeviceFeatures2& vk_physical_device_features2, VmaAllocator vma_allocator)
{
    ...
    // access map here.
    GetVulkanFunctionPointers()->per_queue_lock_map[vk_queue] = std::make_unique<base::Lock>();
