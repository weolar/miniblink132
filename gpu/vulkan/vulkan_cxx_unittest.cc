...
public:
    ...
    void SetUp() override
    {
        use_swiftshader_ = base::CommandLine::ForCurrentProcess()->HasSwitch("use-swiftshader");
        base::FilePath path;
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_ANDROID) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_FUCHSIA)
        if (use_swiftshader_) {
...
#error "Not supported platform"
        ...
        vulkan_function_pointers->vulkan_loader_library = base::LoadNativeLibrary(path, &native_library_load_error);
...
TEST_F(VulkanCXXTest, CreateInstanceUnique)
{
    ...
    vk::ApplicationInfo app_info("VulkanCXXTest", 0, nullptr, 0, kVulkanRequiredApiVersion);
    ...
    EXPECT_TRUE(vulkan_function_pointers->BindInstanceFunctionPointers(instance.get(), kVulkanRequiredApiVersion, gfx::ExtensionSet()));
