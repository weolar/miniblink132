...
TEST_F(VulkanFenceHelperTest, BasicFenceUsage)
{
    ...
    ASSERT_TRUE(VK_SUCCESS == vkQueueSubmit(GetDeviceQueue()->GetVulkanQueue(), 0, nullptr, fence));
    VulkanFenceHelper::FenceHandle fence_handle = fence_helper->EnqueueFence(fence);
...
TEST_F(VulkanFenceHelperTest, TestBasicCallback)
{
    ...
    fence_helper->EnqueueCleanupTaskForSubmittedWork(
        base::BindOnce([](bool* cleanup_run, VulkanDeviceQueue* device_queue, bool is_lost) { *cleanup_run = true; }, &cleanup_run));
...
TEST_F(VulkanFenceHelperTest, TestMultipleCallbacks)
{
    ...
    auto increment_cleanups_callback = [](uint32_t expected_index, uint32_t* cleanups_run, VulkanDeviceQueue* device_queue, bool is_lost) {
    ...
    // Enqueue 5 callbacks.
    for (int i = 0; i < 5; i++) {
        fence_helper->EnqueueCleanupTaskForSubmittedWork(base::BindOnce(increment_cleanups_callback, i, &cleanups_run));
    }
