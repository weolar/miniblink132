...
public:
    ...
    GpuMemoryBufferFactoryStub& operator=(const GpuMemoryBufferFactoryStub&) = delete;
...
// static
std::unique_ptr<GpuMemoryBufferFactory> GpuMemoryBufferFactory::CreateNativeType(
    viz::VulkanContextProvider* vulkan_context_provider, scoped_refptr<base::SingleThreadTaskRunner> io_runner)
{
#if BUILDFLAG(IS_APPLE)
    ...
    return std::make_unique<GpuMemoryBufferFactoryNativePixmap>(vulkan_context_provider);
...
gfx::GpuMemoryBufferHandle GpuMemoryBufferFactory::CreateNativeGmbHandle(
    MappableSIClientGmbId id, const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage)
{
    ...
    auto handle = CreateGpuMemoryBuffer(gmb_id, size, /*framebuffer_size=*/size, format, usage, kMappableSIClientId, gpu::kNullSurfaceHandle);
...
void GpuMemoryBufferFactory::CreateGpuMemoryBufferAsync(gfx::GpuMemoryBufferId id, const gfx::Size& size, gfx::BufferFormat format, gfx::BufferUsage usage,
    int client_id, SurfaceHandle surface_handle, CreateGpuMemoryBufferAsyncCallback callback)
{
    ...
    std::move(callback).Run(CreateGpuMemoryBuffer(id, size, /*framebuffer_size=*/size, format, usage, client_id, surface_handle));
}
