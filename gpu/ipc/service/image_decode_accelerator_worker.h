...
public:
    ...
    using CompletedDecodeCB = base::OnceCallback<void(std::unique_ptr<DecodeResult>)>;
    ...
    // returned.
    virtual std::vector<ImageDecodeAcceleratorSupportedProfile> GetSupportedProfiles() = 0;
    ...
    virtual void Decode(std::vector<uint8_t> encoded_data, const gfx::Size& output_size, CompletedDecodeCB decode_cb) = 0;
};
