...
WebGPUCommandBufferStub::WebGPUCommandBufferStub(GpuChannel* channel, const mojom::CreateCommandBufferParams& init_params, CommandBufferId command_buffer_id,
    ...
    CommandBufferStub* share_command_buffer_stub, const mojom::CreateCommandBufferParams& init_params, base::UnsafeSharedMemoryRegion shared_state_shm)
{
    ...
    scoped_refptr<SharedContextState> shared_context_state = manager->GetSharedContextState(&result);
    ...
    webgpu::DawnCacheOptions dawn_cache_options
        = { manager->dawn_caching_interface_factory(), channel_->GetCacheHandleForType(gpu::GpuDiskCacheType::kDawnWebGPU) };

    command_buffer_ = std::make_unique<CommandBufferService>(this, memory_tracker_.get());
    std::unique_ptr<webgpu::WebGPUDecoder> decoder(webgpu::WebGPUDecoder::Create(this, command_buffer_.get(), manager->shared_image_manager(),
        memory_tracker_.get(), manager->outputter(), manager->gpu_preferences(), std::move(shared_context_state), dawn_cache_options, channel_));
