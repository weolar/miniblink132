...
class GPU_IPC_SERVICE_EXPORT CommandBufferStub : public CommandBufferServiceClient, public DecoderClient, public mojom::CommandBuffer {
public:
    ...
    CommandBufferStub(GpuChannel* channel, const mojom::CreateCommandBufferParams& init_params, CommandBufferId command_buffer_id, SequenceId sequence_id,
        int32_t stream_id, int32_t route_id);
    ...
    virtual gpu::ContextResult Initialize(
        CommandBufferStub* share_group, const mojom::CreateCommandBufferParams& params, base::UnsafeSharedMemoryRegion shared_state_shm)
        = 0;

    // Establish Mojo bindings for the receiver and client endpoints.
    void BindEndpoints(mojo::PendingAssociatedReceiver<mojom::CommandBuffer> receiver, mojo::PendingAssociatedRemote<mojom::CommandBufferClient> client,
    ...
    void ExecuteDeferredRequest(mojom::DeferredCommandBufferRequestParams& params, FenceSyncReleaseDelegate* release_delegate);
    ...
    using WaitForStateCallback = base::OnceCallback<void(const gpu::CommandBuffer::State&)>;
    void WaitForTokenInRange(int32_t start, int32_t end, WaitForStateCallback callback);
    ...
    void WaitForGetOffsetInRange(uint32_t set_get_buffer_count, int32_t start, int32_t end, WaitForStateCallback callback);
    ...
    using MemoryTrackerFactory = base::RepeatingCallback<std::unique_ptr<MemoryTracker>()>;
    ...
    DecoderContext* decoder_context() const
    {
        return decoder_context_.get();
    }
    GpuChannel* channel() const
    {
        return channel_;
    }
...
protected:
    ...
    mojom::CommandBufferClient& client()
    {
        return *client_.get();
    }
    ...
    void RegisterTransferBuffer(int32_t id, base::UnsafeSharedMemoryRegion transfer_buffer) override;
    void CreateGpuFenceFromHandle(uint32_t id, gfx::GpuFenceHandle handle) override;
    ...
    virtual void OnSetDefaultFramebufferSharedImage(const Mailbox& mailbox, int samples_count, bool preserve, bool needs_depth, bool needs_stencil)
    {
    }
...
private:
    ...
    void CreateCacheUse(std::optional<gles2::ProgramCache::ScopedCacheUse>& cache_use);

    // Message handlers:
    void OnAsyncFlush(int32_t put_offset, uint32_t flush_id, const std::vector<SyncToken>& sync_token_fences);
    ...
    static void SetContextGpuFeatureInfo(gl::GLContext* context, const GpuFeatureInfo& gpu_feature_info);
    ...
    // This is intended for mocking the MemoryTracker in tests.
    static MemoryTrackerFactory SetOrGetMemoryTrackerFactory(MemoryTrackerFactory factory);
