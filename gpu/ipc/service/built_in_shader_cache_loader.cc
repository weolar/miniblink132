...
public:
    ...
    {
        file_.Initialize(AdjustPath(path), base::File::FLAG_OPEN | base::File::FLAG_READ);
        if (!file_.IsValid()) {
...
private:
    ...
    {
        return path.empty() ? base::apple::PathForFrameworkBundleResource(kShaderCacheFileName) : path;
    }
    ...
    bool ReadBytes(uint32_t size, char* data)
    {
        ...
        while (!AtEndOrErrored()) {
            ...
            const uint32_t bytes_to_copy = std::min(static_cast<uint32_t>(data_end - data_ptr), RemainingBytesInBuffer());
    ...
    bool AtEndOrErrored() const
    {
        return at_end_or_errored_;
    }
...
void BuiltInShaderCacheLoader::StartLoading()
{
    ...
    auto path = base::CommandLine::ForCurrentProcess()->GetSwitchValuePath(switches::kShaderCachePath);
    base::ThreadPool::PostTask(
        FROM_HERE, { base::TaskPriority::USER_BLOCKING, base::MayBlock() }, base::BindOnce(&BuiltInShaderCacheLoader::Load, base::Unretained(g_loader), path));
}

// static
std::unique_ptr<std::vector<BuiltInShaderCacheLoader::CacheEntry>> BuiltInShaderCacheLoader::TakeEntries()
{
...
std::unique_ptr<std::vector<BuiltInShaderCacheLoader::CacheEntry>> BuiltInShaderCacheLoader::TakeEntriesImpl()
{
    const base::TimeTicks start_time = base::TimeTicks::Now();
    loaded_signaler_.Wait();
    const base::TimeTicks end_time = base::TimeTicks::Now();
    base::UmaHistogramCustomMicrosecondsTimes("Gpu.MetalShaderCache.WaitTime", end_time - start_time, base::Microseconds(1), base::Milliseconds(100), 100);
    std::unique_ptr<std::vector<CacheEntry>> entries = std::make_unique<std::vector<CacheEntry>>(std::move(entries_));
    return entries;
}
...
void BuiltInShaderCacheLoader::Load(const base::FilePath& path)
{
    ...
    LoadImpl(path);
    const base::TimeTicks end_time = base::TimeTicks::Now();
    base::UmaHistogramCustomMicrosecondsTimes("Gpu.MetalShaderCache.LoadTime", end_time - start_time, base::Microseconds(10), base::Milliseconds(100), 100);
    base::UmaHistogramCounts100("Gpu.MetalShaderCache.NumEntriesInCache", entries_.size());
...
void BuiltInShaderCacheLoader::LoadImpl(const base::FilePath& path)
{
    ...
    for (;;) {
        CacheEntry entry;
        if (!reader.ReadKeyOrValue(entry.key) || !reader.ReadKeyOrValue(entry.value)) {
            return;
        }
