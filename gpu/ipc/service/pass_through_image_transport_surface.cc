...
PassThroughImageTransportSurface::PassThroughImageTransportSurface(
    base::WeakPtr<ImageTransportSurfaceDelegate> delegate, gl::GLSurface* surface, bool override_vsync_for_multi_window_swap)
    : GLSurfaceAdapter(surface)
    , is_gpu_vsync_disabled_(!features::UseGpuVsync())
    , is_multi_window_swap_vsync_override_enabled_(override_vsync_for_multi_window_swap)
    , delegate_(delegate)
{
}
...
gfx::SwapResult PassThroughImageTransportSurface::SwapBuffers(PresentationCallback callback, gl::FrameData data)
{
    gfx::SwapResponse response;
    StartSwapBuffers(&response);
    gfx::SwapResult result = gl::GLSurfaceAdapter::SwapBuffers(
        base::BindOnce(&PassThroughImageTransportSurface::BufferPresented, weak_ptr_factory_.GetWeakPtr(), std::move(callback), local_swap_id_),
        std::move(data));
...
void PassThroughImageTransportSurface::SwapBuffersAsync(
    SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, gl::FrameData data)
{
    gfx::SwapResponse response;
    StartSwapBuffers(&response);
    ...
    gl::GLSurfaceAdapter::SwapBuffersAsync(base::BindOnce(&PassThroughImageTransportSurface::FinishSwapBuffersAsync, weak_ptr_factory_.GetWeakPtr(),
                                               std::move(completion_callback), response, local_swap_id_),
        base::BindOnce(&PassThroughImageTransportSurface::BufferPresented, weak_ptr_factory_.GetWeakPtr(), std::move(presentation_callback), local_swap_id_),
        std::move(data));
}

gfx::SwapResult PassThroughImageTransportSurface::SwapBuffersWithBounds(const std::vector<gfx::Rect>& rects, PresentationCallback callback, gl::FrameData data)
{
    gfx::SwapResponse response;
    StartSwapBuffers(&response);
...
gfx::SwapResult PassThroughImageTransportSurface::PostSubBuffer(int x, int y, int width, int height, PresentationCallback callback, gl::FrameData data)
{
    gfx::SwapResponse response;
    StartSwapBuffers(&response);
    gfx::SwapResult result = gl::GLSurfaceAdapter::PostSubBuffer(x, y, width, height,
        base::BindOnce(&PassThroughImageTransportSurface::BufferPresented, weak_ptr_factory_.GetWeakPtr(), std::move(callback), local_swap_id_),
        std::move(data));
    response.result = result;
    FinishSwapBuffers(response, local_swap_id_,
...
void PassThroughImageTransportSurface::PostSubBufferAsync(
    int x, int y, int width, int height, SwapCompletionCallback completion_callback, PresentationCallback presentation_callback, gl::FrameData data)
{
    gfx::SwapResponse response;
    StartSwapBuffers(&response);
    gl::GLSurfaceAdapter::PostSubBufferAsync(x, y, width, height,
        base::BindOnce(&PassThroughImageTransportSurface::FinishSwapBuffersAsync, weak_ptr_factory_.GetWeakPtr(), std::move(completion_callback), response,
            local_swap_id_),
        base::BindOnce(&PassThroughImageTransportSurface::BufferPresented, weak_ptr_factory_.GetWeakPtr(), std::move(presentation_callback), local_swap_id_),
        std::move(data));
}

gfx::SwapResult PassThroughImageTransportSurface::CommitOverlayPlanes(PresentationCallback callback, gl::FrameData data)
{
    gfx::SwapResponse response;
    StartSwapBuffers(&response);
...
void PassThroughImageTransportSurface::CommitOverlayPlanesAsync(SwapCompletionCallback callback, PresentationCallback presentation_callback, gl::FrameData data)
{
    gfx::SwapResponse response;
    StartSwapBuffers(&response);
    gl::GLSurfaceAdapter::CommitOverlayPlanesAsync(base::BindOnce(&PassThroughImageTransportSurface::FinishSwapBuffersAsync, weak_ptr_factory_.GetWeakPtr(),
                                                       std::move(callback), response, local_swap_id_),
        base::BindOnce(&PassThroughImageTransportSurface::BufferPresented, weak_ptr_factory_.GetWeakPtr(), std::move(presentation_callback), local_swap_id_),
        std::move(data));
}
...
void PassThroughImageTransportSurface::TrackMultiSurfaceSwap()
{
    ...
    multiple_surfaces_swapped_ = (g_num_swaps_in_current_swap_generation_ > 1)
        || (g_current_swap_generation_ - g_last_multi_window_swap_generation_ < kMultiWindowSwapEnableVSyncDelay);
}
...
void PassThroughImageTransportSurface::StartSwapBuffers(gfx::SwapResponse* response)
{
...
void PassThroughImageTransportSurface::FinishSwapBuffers(gfx::SwapResponse response, uint64_t local_swap_id, gfx::GpuFenceHandle release_fence)
{
    ...
    if (delegate_) {
        auto blocked_time_since_last_swap = delegate_->GetGpuBlockedTimeSinceLastSwap();

        if (!multiple_surfaces_swapped_) {
            static constexpr base::TimeDelta kTimingMetricsHistogramMin = base::Microseconds(5);
            static constexpr base::TimeDelta kTimingMetricsHistogramMax = base::Milliseconds(500);
            static constexpr uint32_t kTimingMetricsHistogramBuckets = 50;

            base::TimeDelta delta = response.timings.swap_end - response.timings.swap_start;
            UMA_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES(
                "GPU.SwapTimeUs", delta, kTimingMetricsHistogramMin, kTimingMetricsHistogramMax, kTimingMetricsHistogramBuckets);
            ...
            if (!blocked_time_since_last_swap.is_min()) {
                LOCAL_HISTOGRAM_CUSTOM_MICROSECONDS_TIMES("GPU.GpuBlockedBetweenSwapsUs2", blocked_time_since_last_swap, kTimingMetricsHistogramMin,
...
void PassThroughImageTransportSurface::FinishSwapBuffersAsync(
    SwapCompletionCallback callback, gfx::SwapResponse response, uint64_t local_swap_id, gfx::SwapCompletionResult result)
{
...
void PassThroughImageTransportSurface::BufferPresented(
    GLSurface::PresentationCallback callback, uint64_t local_swap_id, const gfx::PresentationFeedback& feedback)
{
#if DCHECK_IS_ON()
    ...
    // callback.
    DCHECK(pending_local_swap_ids_.empty() || pending_local_swap_ids_.front() > local_swap_id);
#endif
