...
public:
    ...
    virtual bool EnsureSandboxInitialized(GpuWatchdogThread* watchdog_thread, const GPUInfo* gpu_info, const GpuPreferences& gpu_prefs) = 0;
};
...
public:
    ...
    bool InitializeAndStartSandbox(base::CommandLine* command_line, const GpuPreferences& gpu_preferences);
    void InitializeInProcess(base::CommandLine* command_line, const GpuPreferences& gpu_preferences);
    ...
    const gfx::GpuExtraInfo& gpu_extra_info() const
    {
    ...
    const std::optional<GpuFeatureInfo>& gpu_feature_info_for_hardware_gpu() const
    {
