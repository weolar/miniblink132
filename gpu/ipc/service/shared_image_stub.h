...
public:
    ...
    using SharedImageDestructionCallback = base::OnceCallback<void(const gpu::SyncToken&)>;

    static std::unique_ptr<SharedImageStub> Create(GpuChannel* channel, int32_t route_id);
    ...
    SharedImageFactory* factory() const
    {
        return factory_.get();
    }
    ...
    const scoped_refptr<gpu::GpuChannelSharedImageInterface>& shared_image_interface();

    SharedImageDestructionCallback GetSharedImageDestructionCallback(const Mailbox& mailbox);

    bool CreateSharedImage(const Mailbox& mailbox, gfx::GpuMemoryBufferHandle handle, viz::SharedImageFormat format, const gfx::Size& size,
    ...
    bool UpdateSharedImage(const Mailbox& mailbox, gfx::GpuFenceHandle in_fence_handle);

#if BUILDFLAG(IS_WIN)
    void CopyToGpuMemoryBufferAsync(const Mailbox& mailbox, base::OnceCallback<void(bool)> callback);
#endif
...
private:
    ...
    void OnCreateSharedImageWithData(mojom::CreateSharedImageWithDataParamsPtr params);
    void OnCreateSharedImageWithBuffer(mojom::CreateSharedImageWithBufferParamsPtr params);
    void OnUpdateSharedImage(const Mailbox& mailbox, gfx::GpuFenceHandle in_fence_handle);
    ...
    void OnRegisterDxgiFence(const Mailbox& mailbox, gfx::DXGIHandleToken dxgi_token, gfx::GpuFenceHandle fence_handle);
    void OnUpdateDxgiFence(const Mailbox& mailbox, gfx::DXGIHandleToken dxgi_token, uint64_t fence_value);
    void OnUnregisterDxgiFence(const Mailbox& mailbox, gfx::DXGIHandleToken dxgi_token);
    ...
    scoped_refptr<GpuChannelSharedImageInterface> gpu_channel_shared_image_interface_;
    ...
    base::flat_map<Mailbox, base::flat_set<scoped_refptr<gfx::D3DSharedFence>>> registered_dxgi_fences_;
#endif
