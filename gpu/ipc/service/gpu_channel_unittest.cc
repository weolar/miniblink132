...
TEST_F(GpuChannelTest, CreateViewCommandBufferAllowed)
{
    ...
    int32_t kRouteId = static_cast<int32_t>(GpuChannelReservedRoutes::kMaxValue) + 1;
    ...
    CreateCommandBuffer(*channel, std::move(init_params), kRouteId, GetSharedMemoryRegion(), &result, &capabilities, &gl_capabilities);
...
TEST_F(GpuChannelTest, IncompatibleStreamIds)
{
    ...
    int32_t kRouteId1 = static_cast<int32_t>(GpuChannelReservedRoutes::kMaxValue) + 1;
    ...
    gpu::Capabilities capabilities;
    gpu::GLCapabilities gl_capabilities;
    CreateCommandBuffer(*channel, std::move(init_params), kRouteId1, GetSharedMemoryRegion(), &result, &capabilities, &gl_capabilities);
    EXPECT_EQ(result, gpu::ContextResult::kSuccess);
    ...
    CreateCommandBuffer(*channel, std::move(init_params2), kRouteId2, GetSharedMemoryRegion(), &result, &capabilities, &gl_capabilities);
    EXPECT_EQ(result, gpu::ContextResult::kFatalFailure);
...
TEST_F(GpuChannelTest, CreateFailsIfSharedContextIsLost)
{
    ...
    int32_t kSharedRouteId = static_cast<int32_t>(GpuChannelReservedRoutes::kMaxValue) + 1;
    {
        ...
        gpu::Capabilities capabilities;
        gpu::GLCapabilities gl_capabilities;
        CreateCommandBuffer(*channel, std::move(init_params), kSharedRouteId, GetSharedMemoryRegion(), &result, &capabilities, &gl_capabilities);
        EXPECT_EQ(result, gpu::ContextResult::kSuccess);
    }
    ...
    int32_t kFriendlyRouteId = kSharedRouteId + 1;
    {
        ...
        gpu::Capabilities capabilities;
        gpu::GLCapabilities gl_capabilities;
        CreateCommandBuffer(*channel, std::move(init_params), kFriendlyRouteId, GetSharedMemoryRegion(), &result, &capabilities, &gl_capabilities);
        EXPECT_EQ(result, gpu::ContextResult::kSuccess);
    }
    ...
    int32_t kAnotherRouteId = kFriendlyRouteId + 1;
    {
        ...
        gpu::Capabilities capabilities;
        gpu::GLCapabilities gl_capabilities;
        CreateCommandBuffer(*channel, std::move(init_params), kAnotherRouteId, GetSharedMemoryRegion(), &result, &capabilities, &gl_capabilities);
...
public:
    GpuChannelExitForContextLostTest()
        : GpuChannelTestCommon({ EXIT_ON_CONTEXT_LOST } /* enabled_workarounds */, true /* use_stub_bindings */)
    {
    }
};

TEST_F(GpuChannelExitForContextLostTest, CreateFailsDuringLostContextShutdown_1)
{
    int32_t kClientId = 1;
    GpuChannel* channel = CreateChannel(kClientId, false);
    ASSERT_TRUE(channel);
    ...
    channel_manager()->OnContextLost(-1 /* context_lost_count */, false /* synthetic_loss */, error::ContextLostReason::kUnknown);
...
TEST_F(GpuChannelExitForContextLostTest, CreateFailsDuringLostContextShutdown_2)
{
