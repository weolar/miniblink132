...
template <typename GpuMemoryBufferFactoryType> class GpuMemoryBufferFactoryTest : public testing::Test {
public:
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_OZONE)
...
TYPED_TEST_P(GpuMemoryBufferFactoryTest, CreateGpuMemoryBuffer)
{
    ...
    for (auto format : gfx::GetBufferFormatsForTesting()) {
        ...
        for (auto usage : usages) {
            if (!gpu::GpuMemoryBufferSupport::IsNativeGpuMemoryBufferConfigurationSupported(format, usage)) {
                continue;
            }

            gfx::GpuMemoryBufferHandle handle = TestFixture::factory_.CreateGpuMemoryBuffer(
                kBufferId, buffer_size, /*framebuffer_size=*/buffer_size, format, usage, kClientId, gpu::kNullSurfaceHandle);
