...
void InitializePlatformOverlaySettings(GPUInfo* gpu_info, const GpuFeatureInfo& gpu_feature_info)
{
#if BUILDFLAG(IS_WIN)
    ...
    gl::DirectCompositionOverlayWorkarounds workarounds = { .disable_sw_video_overlays
        = gpu_feature_info.IsWorkaroundEnabled(DISABLE_DIRECT_COMPOSITION_SW_VIDEO_OVERLAYS),
        .disable_decode_swap_chain = gpu_feature_info.IsWorkaroundEnabled(DISABLE_DECODE_SWAP_CHAIN),
        .enable_bgra8_overlays_with_yuv_overlay_support = gpu_feature_info.IsWorkaroundEnabled(gpu::ENABLE_BGRA8_OVERLAYS_WITH_YUV_OVERLAY_SUPPORT),
        .force_nv12_overlay_support = gpu_feature_info.IsWorkaroundEnabled(gpu::FORCE_NV12_OVERLAY_SUPPORT),
        .force_rgb10a2_overlay_support = gpu_feature_info.IsWorkaroundEnabled(gpu::FORCE_RGB10A2_OVERLAY_SUPPORT),
...
public:
    ...
    void SetGpuWatchdogPtr(GpuWatchdogThread* ptr)
    {
        watchdog_ptr_ = ptr;
    }
...
void DisableInProcessGpuVulkan(GpuFeatureInfo* gpu_feature_info, GpuPreferences* gpu_preferences)
{
    if (gpu_feature_info->status_values[GPU_FEATURE_TYPE_VULKAN] == kGpuFeatureStatusEnabled || gpu_preferences->gr_context_type == GrContextType::kVulkan) {
...
void DisableInProcessGpuGraphite(GpuFeatureInfo& gpu_feature_info, GpuPreferences& gpu_preferences)
{
    if (gpu_feature_info.status_values[GPU_FEATURE_TYPE_SKIA_GRAPHITE] == kGpuFeatureStatusEnabled
        || gpu_preferences.gr_context_type == GrContextType::kGraphiteDawn) {
        LOG(ERROR) << "Graphite not supported with in process gpu";
        gpu_feature_info.status_values[GPU_FEATURE_TYPE_SKIA_GRAPHITE] = kGpuFeatureStatusDisabled;
        gpu_preferences.gr_context_type = GrContextType::kGL;
    }
}
#endif
...
{
    auto pattern_strings = base::SplitString(patterns, "|", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
...
void SetupGLDisplayManagerEGL(const GPUInfo& gpu_info, const GpuFeatureInfo& gpu_feature_info)
{
    const GPUInfo::GPUDevice* gpu_high_perf = gpu_info.GetGpuByPreference(gl::GpuPreference::kHighPerformance);
    const GPUInfo::GPUDevice* gpu_low_power = gpu_info.GetGpuByPreference(gl::GpuPreference::kLowPower);
#if BUILDFLAG(IS_WIN)
    // On Windows the default GPU may not be the low power GPU.
    const GPUInfo::GPUDevice* gpu_default = &(gpu_info.gpu);
    uint64_t system_device_id_high_perf = gpu_high_perf ? CHROME_LUID_to_uint64_t(gpu_high_perf->luid) : 0;
    uint64_t system_device_id_low_power = gpu_low_power ? CHROME_LUID_to_uint64_t(gpu_low_power->luid) : 0;
    uint64_t system_device_id_default = CHROME_LUID_to_uint64_t(gpu_default->luid);
#else // IS_MAC
    const GPUInfo::GPUDevice* gpu_default = gpu_low_power ? gpu_low_power : &(gpu_info.gpu);
    uint64_t system_device_id_high_perf = gpu_high_perf ? gpu_high_perf->system_device_id : 0;
    uint64_t system_device_id_low_power = gpu_low_power ? gpu_low_power->system_device_id : 0;
    uint64_t system_device_id_default = gpu_default->system_device_id;
#endif
    DCHECK(gpu_default);

    if (gpu_info.GpuCount() <= 1) {
        gl::SetGpuPreferenceEGL(gl::GpuPreference::kDefault, system_device_id_default);
        return;
    }
    if (gpu_feature_info.IsWorkaroundEnabled(FORCE_LOW_POWER_GPU) && system_device_id_low_power) {
        gl::SetGpuPreferenceEGL(gl::GpuPreference::kDefault, system_device_id_low_power);
        return;
    }
    if (gpu_feature_info.IsWorkaroundEnabled(FORCE_HIGH_PERFORMANCE_GPU) && system_device_id_high_perf) {
    ...
    if (system_device_id_high_perf) {
        gl::SetGpuPreferenceEGL(gl::GpuPreference::kHighPerformance, system_device_id_high_perf);
    }
    return;
}
#endif // IS_WIN || IS_MAC
...
bool GpuInit::InitializeAndStartSandbox(base::CommandLine* command_line, const GpuPreferences& gpu_preferences)
{
    ...
    IntelGpuSeriesType intel_gpu_series_type = GetIntelGpuSeriesType(gpu_info_.active_gpu().vendor_id, gpu_info_.active_gpu().device_id);
    ...
    // info.
    gpu_feature_info_ = ComputeGpuFeatureInfo(gpu_info_, gpu_preferences_, command_line, &needs_more_info);
    ...
    gl_use_swiftshader_ = EnableSwiftShaderIfNeeded(command_line, gpu_feature_info_, gpu_preferences_.disable_software_rasterizer, needs_more_info);

    bool enable_watchdog = !gpu_preferences_.disable_gpu_watchdog && !command_line->HasSwitch(switches::kHeadless);
    ...
    if (enable_watchdog && !delayed_watchdog_enable) {
        watchdog_thread_ = GpuWatchdogThread::Create(gpu_preferences_.watchdog_starts_backgrounded, gl_use_swiftshader_, "GpuWatchdog");
    ...
    if (gpu_preferences_.gpu_sandbox_start_early) {
        gpu_info_.sandboxed = sandbox_helper_->EnsureSandboxInitialized(watchdog_thread_.get(), &gpu_info_, gpu_preferences_);
    ...
    params.enable_native_gpu_memory_buffers = gpu_preferences_.enable_native_gpu_memory_buffers;
    params.handle_overlays_swap_failure = base::FeatureList::IsEnabled(features::kHandleOverlaysSwapFailure);
...
#if BUILDFLAG(IS_WIN)
    if (gpu_preferences_.gr_context_type == GrContextType::kGraphiteDawn && features::kSkiaGraphiteDawnBackendValidation.Get()) {
    ...
    const bool need_fallback_from_graphite = [this]() {
        ...
        if (gpu_preferences_.gr_context_type != GrContextType::kGraphiteDawn && gpu_preferences_.gr_context_type != GrContextType::kGraphiteMetal) {
            return false;
        }
...
#endif
        if (kRequiredANGLEImplementation == gl::ANGLEImplementation::kNone || gl::GetANGLEImplementation() == kRequiredANGLEImplementation) {
        ...
        if (base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableSkiaGraphite)) {
            return false;
        }
    ...
    if (gpu_preferences_.use_passthrough_cmd_decoder || is_swangle) {
        gpu_info_.passthrough_cmd_decoder = gles2::PassthroughCommandDecoderSupported();
#if BUILDFLAG(IS_ANDROID)
...
#endif
            << "Passthrough is not supported, GL is " << gl::GetGLImplementationGLName(gl::GetGLImplementationParts()) << ", ANGLE is "
            << gl::GetGLImplementationANGLEName(gl::GetGLImplementationParts());
    } else {
        gpu_info_.passthrough_cmd_decoder = false;
    }
    gpu_preferences_.use_passthrough_cmd_decoder = gpu_info_.passthrough_cmd_decoder;
#else
    ...
    if (!gl_disabled) {
        if (!gl_use_swiftshader_) {
            ...
            SetKeysForCrashLogging(gpu_info_);
            gpu_feature_info_ = ComputeGpuFeatureInfo(gpu_info_, gpu_preferences_, command_line, nullptr);
            gl_use_swiftshader_ = EnableSwiftShaderIfNeeded(command_line, gpu_feature_info_, gpu_preferences_.disable_software_rasterizer, false);
            if (gl_use_swiftshader_) {
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
                ...
                if (watchdog_thread_.get()) {
                    ...
                    watchdog_thread_ = GpuWatchdogThread::Create(gpu_preferences_.watchdog_starts_backgrounded,
            ...
            case VulkanImplementationName::kNative: {
                ...
                auto gpu_feature_info = ComputeGpuFeatureInfo(gpu_info, gpu_preferences_, command_line, nullptr);
                gpu_feature_info_.status_values[GPU_FEATURE_TYPE_VULKAN] = gpu_feature_info.status_values[GPU_FEATURE_TYPE_VULKAN];
                break;
            }
            case VulkanImplementationName::kForcedNative:
            case VulkanImplementationName::kSwiftshader:
                gpu_feature_info_.status_values[GPU_FEATURE_TYPE_VULKAN] = kGpuFeatureStatusEnabled;
                break;
    ...
    if (gpu_feature_info_.status_values[GPU_FEATURE_TYPE_VULKAN] != kGpuFeatureStatusEnabled || !InitializeVulkan()) {
        ...
        gpu_feature_info_.enabled_gpu_driver_bug_workarounds.push_back(EXIT_ON_CONTEXT_LOST);
    }
    ...
    if (!gl_disabled) {
        if (!gpu_feature_info_.disabled_extensions.empty()) {
            gl::init::SetDisabledExtensionsPlatform(gpu_feature_info_.disabled_extensions);
        }
        if (!gl::init::InitializeExtensionSettingsOneOffPlatform(gl_display)) {
            VLOG(1) << "gl::init::InitializeExtensionSettingsOneOffPlatform failed";
            return false;
        }
        default_offscreen_surface_ = gl::init::CreateOffscreenGLSurface(gl_display, gfx::Size());
    ...
    std::vector<gfx::BufferFormat> supported_buffer_formats_for_texturing
        = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->GetSupportedFormatsForTexturing();
    std::vector<gfx::BufferFormat> supported_buffer_formats_for_gl_native_pixmap_import
        = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->GetSupportedFormatsForGLNativePixmapImport();
#endif // BUILDFLAG(IS_OZONE)
    ...
    if (kGpuFeatureStatusEnabled != gpu_feature_info_.status_values[GPU_FEATURE_TYPE_ACCELERATED_VIDEO_DECODE]) {
        gpu_preferences_.disable_accelerated_video_decode = true;
    }

    if (kGpuFeatureStatusEnabled != gpu_feature_info_.status_values[GPU_FEATURE_TYPE_ACCELERATED_VIDEO_ENCODE]) {
        gpu_preferences_.disable_accelerated_video_encode = true;
    }

    DEPRECATED_UMA_HISTOGRAM_MEDIUM_TIMES("GPU.InitializeOneOffMediumTime", elapsed_timer.Elapsed());
...
#if BUILDFLAG(IS_LINUX) || (BUILDFLAG(IS_CHROMEOS) && !BUILDFLAG(IS_CHROMEOS_DEVICE))
    ...
    if (gl_use_swiftshader_ || gl::IsSoftwareGLImplementation(gl::GetGLImplementationParts())) {
    ...
    if (recreate_watchdog) {
        watchdog_thread_ = nullptr;
        watchdog_init.SetGpuWatchdogPtr(nullptr);
        watchdog_thread_ = GpuWatchdogThread::Create(gpu_preferences_.watchdog_starts_backgrounded, software_rendering, "GpuWatchdog");
        watchdog_init.SetGpuWatchdogPtr(watchdog_thread_.get());
    }
    ...
    if (gpu_preferences_.gr_context_type == GrContextType::kGraphiteDawn) {
        if (!InitializeDawn()) {
            if (gpu_feature_info_.status_values[GPU_FEATURE_TYPE_SKIA_GRAPHITE] == kGpuFeatureStatusEnabled) {
                return false;
            }
    ...
    gpu_feature_info_.supported_buffer_formats_for_allocation_and_texturing = std::move(supported_buffer_formats_for_texturing);
    gpu_feature_info_.supported_buffer_formats_for_gl_native_pixmap_import = std::move(supported_buffer_formats_for_gl_native_pixmap_import);
    [[maybe_unused]] auto* factory = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone();
    bool filter_set = false;
#if BUILDFLAG(ENABLE_VULKAN)
    if (gpu_feature_info_.status_values[GPU_FEATURE_TYPE_VULKAN] == kGpuFeatureStatusEnabled && factory->SupportsDrmModifiersFilter()) {
        CHECK(!filter_set);
        DCHECK(vulkan_implementation_ && vulkan_implementation_->GetVulkanInstance()
            && vulkan_implementation_->GetVulkanInstance()->vk_instance() != VK_NULL_HANDLE);
        factory->SetDrmModifiersFilter(std::make_unique<DrmModifiersFilterVulkan>(vulkan_implementation_.get()));
        filter_set = true;
    }
#endif // BUILDFLAG(ENABLE_VULKAN)
#if BUILDFLAG(SKIA_USE_DAWN) && BUILDFLAG(IS_CHROMEOS)
    if (dawn_context_provider_ && factory->SupportsDrmModifiersFilter()) {
        CHECK(!filter_set);
        factory->SetDrmModifiersFilter(std::make_unique<DrmModifiersFilterDawn>(dawn_context_provider_->GetDevice().GetAdapter()));
        filter_set = true;
    }
...
#if BUILDFLAG(IS_ANDROID)
void GpuInit::InitializeInProcess(base::CommandLine* command_line, const GpuPreferences& gpu_preferences)
{
    gpu_preferences_ = gpu_preferences;
    init_successful_ = true;
    DCHECK(!EnableSwiftShaderIfNeeded(command_line, gpu_feature_info_, gpu_preferences_.disable_software_rasterizer, false));

    gl::GLDisplay* gl_display = InitializeGLThreadSafe(command_line, gpu_preferences_, &gpu_info_, &gpu_feature_info_);
...
void GpuInit::InitializeInProcess(base::CommandLine* command_line, const GpuPreferences& gpu_preferences)
{
    ...
    const std::vector<gfx::BufferFormat> supported_buffer_formats_for_texturing
        = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->GetSupportedFormatsForTexturing();
    const std::vector<gfx::BufferFormat> supported_buffer_formats_for_gl_native_pixmap_import
        = ui::OzonePlatform::GetInstance()->GetSurfaceFactoryOzone()->GetSupportedFormatsForGLNativePixmapImport();
    gpu_feature_info_.supported_buffer_formats_for_allocation_and_texturing = std::move(supported_buffer_formats_for_texturing);
    gpu_feature_info_.supported_buffer_formats_for_gl_native_pixmap_import = std::move(supported_buffer_formats_for_gl_native_pixmap_import);
#endif
...
bool GpuInit::InitializeDawn()
{
...
#if BUILDFLAG(IS_ANDROID)
    auto validate_adapter_fn = [this](wgpu::BackendType backend_type, wgpu::Adapter adapter) {
    ...
    dawn_context_provider_ = gpu::DawnContextProvider::Create(
        gpu_preferences_, validate_adapter_fn, GpuDriverBugWorkarounds(gpu_feature_info_.enabled_gpu_driver_bug_workarounds));
...
{
#if BUILDFLAG(ENABLE_VULKAN)
    DCHECK_EQ(gpu_feature_info_.status_values[GPU_FEATURE_TYPE_VULKAN], kGpuFeatureStatusEnabled);
    DCHECK_NE(gpu_preferences_.use_vulkan, VulkanImplementationName::kNone);
    bool vulkan_use_swiftshader = gpu_preferences_.use_vulkan == VulkanImplementationName::kSwiftshader;
    bool forced_native = gpu_preferences_.use_vulkan == VulkanImplementationName::kForcedNative;
    ...
    if (!use_swiftshader && !forced_native) {
        ...
        const base::FeatureParam<std::string> disable_by_renderer(&features::kVulkan, "disable_by_gl_renderer", "");
        if (MatchGLInfo(gpu_info_.gl_renderer, disable_by_renderer.Get())) {
            return false;
        }
    }

    vulkan_implementation_ = CreateVulkanImplementation(vulkan_use_swiftshader, gpu_preferences_.enable_vulkan_protected_memory);
    if (!vulkan_implementation_ || !vulkan_implementation_->InitializeVulkanInstance(!gpu_preferences_.disable_vulkan_surface)) {
    ...
    auto& vulkan_info = vulkan_implementation_->GetVulkanInstance()->vulkan_info();
    if (vulkan_info.physical_devices.empty()) {
        return false;
    }

    VulkanPhysicalDeviceProperties device_properties(vulkan_info.physical_devices.front().properties);
    if (!use_swiftshader && !forced_native && !CheckVulkanCompatibilities(device_properties, gpu_info_)) {
        vulkan_implementation_.reset();
        return false;
    }

    gpu_info_.hardware_supports_vulkan = true;
    gpu_info_.vulkan_info = vulkan_implementation_->GetVulkanInstance()->vulkan_info();
