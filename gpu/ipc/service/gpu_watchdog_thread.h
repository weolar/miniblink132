...
public:
    static std::unique_ptr<GpuWatchdogThread> Create(bool start_backgrounded, bool software_rendering, const std::string& thread_name);

    // Use the existing GpuWatchdogThread to create a second one. This is used
    // for DrDC thread only.
    static std::unique_ptr<GpuWatchdogThread> Create(bool start_backgrounded, const GpuWatchdogThread* existing_watchdog, const std::string& thread_name);

    static std::unique_ptr<GpuWatchdogThread> Create(
        bool start_backgrounded, base::TimeDelta timeout, int restart_factor, bool test_mode, const std::string& thread_name);
...
private:
    ...
    GpuWatchdogThread(base::TimeDelta timeout, int restart_factor, bool test_mode, const std::string& thread_name);
    ...
    void WatchedThreadNeedsMoreThreadTimeHistogram(bool no_gpu_hang_detected, bool start_of_more_thread_time);
#endif
