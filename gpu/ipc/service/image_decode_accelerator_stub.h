...
class GPU_IPC_SERVICE_EXPORT ImageDecodeAcceleratorStub : public base::RefCountedThreadSafe<ImageDecodeAcceleratorStub> {
public:
    ...
    ImageDecodeAcceleratorStub(ImageDecodeAcceleratorWorker* worker, GpuChannel* channel, int32_t route_id);

    ImageDecodeAcceleratorStub(const ImageDecodeAcceleratorStub&) = delete;
    ImageDecodeAcceleratorStub& operator=(const ImageDecodeAcceleratorStub&) = delete;
    ...
    void ScheduleImageDecode(mojom::ScheduleImageDecodeParamsPtr params, uint64_t release_count);
...
private:
    ...
    void OnDecodeCompleted(gfx::Size expected_output_size, std::unique_ptr<ImageDecodeAcceleratorWorker::DecodeResult> result);
    ...
    base::queue<std::unique_ptr<ImageDecodeAcceleratorWorker::DecodeResult>> pending_completed_decodes_ GUARDED_BY(lock_);
