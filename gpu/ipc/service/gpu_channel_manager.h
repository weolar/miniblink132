...
class GPU_IPC_SERVICE_EXPORT GpuChannelManager : public raster::GrShaderCache::Client {
public:
    using OnMemoryAllocatedChangeCallback
        = base::OnceCallback<void(gpu::CommandBufferId id, uint64_t old_size, uint64_t new_size, gpu::GpuPeakMemoryAllocationSource source)>;

    GpuChannelManager(const GpuPreferences& gpu_preferences, GpuChannelManagerDelegate* delegate, GpuWatchdogThread* watchdog,
        ...
        ImageDecodeAcceleratorWorker* image_decode_accelerator_worker, viz::VulkanContextProvider* vulkan_context_provider = nullptr,
        viz::MetalContextProvider* metal_context_provider = nullptr, DawnContextProvider* dawn_context_provider = nullptr,
        webgpu::DawnCachingInterfaceFactory* dawn_caching_interface_factory = nullptr);
    ...
    GpuChannelManagerDelegate* delegate() const
    {
        return delegate_;
    }
    GpuWatchdogThread* watchdog() const
    {
        return watchdog_;
    }

    GpuChannel* EstablishChannel(const base::UnguessableToken& channel_token, int client_id, uint64_t client_tracing_id, bool is_gpu_host,
    ...
    void SetChannelDiskCacheHandle(int client_id, const gpu::GpuDiskCacheHandle& handle);
    void OnDiskCacheHandleDestoyed(const gpu::GpuDiskCacheHandle& handle);

    void PopulateCache(const gpu::GpuDiskCacheHandle& handle, const std::string& key, const std::string& program);
    ...
    void OnContextLost(int context_lost_count, bool synthetic_loss, error::ContextLostReason context_lost_reason);
    ...
    const GpuFeatureInfo& gpu_feature_info() const
    {
    ...
    bool application_backgrounded() const
    {
        return application_backgrounded_;
    }
    // Make sure that delayed cleanup is happening now. Expensive.
    void PerformImmediateCleanup();

    gl::GLShareGroup* share_group() const
    {
    ...
    {
        return gpu_preferences_.use_passthrough_cmd_decoder && gles2::PassthroughCommandDecoderSupported();
    }

    // Retrieve GPU Resource consumption statistics for the task manager
    void GetVideoMemoryUsageStats(VideoMemoryUsageStats* video_memory_usage_stats) const;
    ...
    base::flat_map<GpuPeakMemoryAllocationSource, uint64_t> GetPeakMemoryUsage(uint32_t sequence_num, uint64_t* out_peak_memory);

    scoped_refptr<SharedContextState> GetSharedContextState(ContextResult* result);
    ...
    void SetImageDecodeAcceleratorWorkerForTesting(ImageDecodeAcceleratorWorker* worker);

    void LoseAllContexts();

    SharedContextState::ContextLostCallback GetContextLostCallback();
    GpuChannelManager::OnMemoryAllocatedChangeCallback GetOnMemoryAllocatedChangeCallback();
...
private:
    ...
    class GPU_IPC_SERVICE_EXPORT GpuPeakMemoryMonitor : public MemoryTracker::Observer {
    public:
        GpuPeakMemoryMonitor(GpuChannelManager* channel_manager, scoped_refptr<base::SingleThreadTaskRunner> task_runner);
    ...
    private:
        ...
        public:
            SequenceTracker(uint64_t current_memory, base::flat_map<GpuPeakMemoryAllocationSource, uint64_t> current_memory_per_source);
            SequenceTracker(const SequenceTracker&);
            ~SequenceTracker();

            uint64_t initial_memory_ = 0u;
            uint64_t total_memory_ = 0u;
            base::flat_map<GpuPeakMemoryAllocationSource, uint64_t> initial_memory_per_source_;
            base::flat_map<GpuPeakMemoryAllocationSource, uint64_t> peak_memory_per_source_;
        };
        std::unique_ptr<base::trace_event::TracedValue> StartTrackingTracedValue();
        std::unique_ptr<base::trace_event::TracedValue> StopTrackingTracedValue(SequenceTracker& sequence);
        // MemoryTracker::Observer:
        void OnMemoryAllocatedChange(
            CommandBufferId id, uint64_t old_size, uint64_t new_size, GpuPeakMemoryAllocationSource source = GpuPeakMemoryAllocationSource::UNKNOWN) override;
        ...
        base::flat_map<GpuPeakMemoryAllocationSource, uint64_t> current_memory_per_source_;
    ...
    raw_ptr<ImageDecodeAcceleratorWorker> image_decode_accelerator_worker_ = nullptr;
