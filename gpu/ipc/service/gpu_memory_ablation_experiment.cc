...
// GPU ablations.
BASE_FEATURE(kGPUMemoryAblationFeature, "GPUMemoryAblation", base::FEATURE_DISABLED_BY_DEFAULT);
...
constexpr viz::SharedImageFormat kFormat = viz::SharedImageFormat::SinglePlane(viz::ResourceFormat::RGBA_8888);
...
bool GpuMemoryAblationExperiment::ExperimentSupported()
{
    ...
    // possible.
    if (gl_impl == gl::kGLImplementationNone || gl_impl == gl::kGLImplementationMockGL || gl_impl == gl::kGLImplementationStubGL
...
GpuMemoryAblationExperiment::GpuMemoryAblationExperiment(GpuChannelManager* channel_manager, scoped_refptr<base::SequencedTaskRunner> task_runner)
...
void GpuMemoryAblationExperiment::OnMemoryAllocated(uint64_t old_size, uint64_t new_size)
{
    ...
    if (new_size > old_size) {
        ...
        if (!mailboxes_.empty()) {
            ...
            task_runner_->PostTask(FROM_HERE, base::BindOnce(&GpuMemoryAblationExperiment::DeleteGpuMemory, weak_factory_.GetWeakPtr()));
        }
    }
}

uint64_t GpuMemoryAblationExperiment::GetPeakMemory(uint32_t sequence_num) const
{
...
void GpuMemoryAblationExperiment::AllocateGpuMemory()
{
    ...
    std::unique_ptr<ui::ScopedMakeCurrent> scoped_current = ScopedMakeContextCurrent();
    if (!scoped_current)
        return;
    ...
    if (!factory_->CreateSharedImage(mailbox, kFormat, size_, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, gpu::kNullSurfaceHandle, kUsage)) {
        return;
    }
    ...
    auto write_access = skia_rep->BeginScopedWriteAccess(
        /*begin_semaphores=*/ {}, /*end_semaphores=*/ {}, SharedImageRepresentation::AllowUnclearedAccess::kYes);
    if (!write_access)
        return;
...
bool GpuMemoryAblationExperiment::InitGpu(GpuChannelManager* channel_manager)
{
    ...
    int arg_value = base::GetFieldTrialParamByFeatureAsInt(kGPUMemoryAblationFeature, kGPUMemoryAblationFeatureSizeParam, default_value);
    if (arg_value == default_value)
        return false;
    int texture_size = std::min(256 * arg_value, context_state_->gr_context()->maxTextureSize());
    ...
    gpu::GpuMemoryBufferFactory* gmb_factory = channel_manager->gpu_memory_buffer_factory();
    factory_ = std::make_unique<SharedImageFactory>(channel_manager->gpu_preferences(), channel_manager->gpu_driver_bug_workarounds(),
        channel_manager->gpu_feature_info(), context_state_.get(), channel_manager->shared_image_manager(),
        gmb_factory ? gmb_factory->AsImageFactory() : nullptr, this,
        /*is_for_display_compositor=*/false);

    rep_factory_ = std::make_unique<SharedImageRepresentationFactory>(channel_manager->shared_image_manager(), this);
    return true;
}

std::unique_ptr<ui::ScopedMakeCurrent> GpuMemoryAblationExperiment::ScopedMakeContextCurrent()
{
    std::unique_ptr<ui::ScopedMakeCurrent> scoped_current = std::make_unique<ui::ScopedMakeCurrent>(context_state_->context(), context_state_->surface());
