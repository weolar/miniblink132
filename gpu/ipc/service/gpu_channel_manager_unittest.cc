...
public:
    static constexpr uint64_t kUInt64_T_Max = std::numeric_limits<uint64_t>::max();
    ...
    uint64_t GetManagersPeakMemoryUsage(uint32_t sequence_num)
    {
        ...
        auto allocation = channel_manager()->GetPeakMemoryUsage(sequence_num, &peak_memory);
        return peak_memory;
    }
    ...
    uint64_t GetMonitorsPeakMemoryUsage(uint32_t sequence_num)
    {
        // Set default as max so that invalid cases can properly test 0u returns.
        uint64_t peak_memory = kUInt64_T_Max;
        auto allocation = channel_manager()->peak_memory_monitor_.GetPeakMemoryUsage(sequence_num, &peak_memory);
        return peak_memory;
    }

    // Helpers to call MemoryTracker::Observer methods of
    // GpuChannelManager::GpuPeakMemoryMonitor.
    void OnMemoryAllocatedChange(CommandBufferId id, uint64_t old_size, uint64_t new_size)
    {
        static_cast<MemoryTracker::Observer*>(gpu_peak_memory_monitor())
            ->OnMemoryAllocatedChange(id, old_size, new_size, GpuPeakMemoryAllocationSource::UNKNOWN);
    }

#if BUILDFLAG(IS_ANDROID)
    void TestApplicationBackgrounded(ContextType type, bool should_destroy_channel)
    {
        ...
        auto raster_decoder_state = channel_manager()->GetSharedContextState(&result);
        EXPECT_EQ(result, ContextResult::kSuccess);
        ...
        ASSERT_NE(channel_manager()->GetSharedContextState(&result).get(), raster_decoder_state.get());
    }
#endif
...
TEST_F(GpuChannelManagerTest, EstablishChannel)
{
    ...
    GpuChannel* channel = channel_manager()->EstablishChannel(
        base::UnguessableToken::Create(), kClientId, kClientTracingId, false, gfx::GpuExtraInfo(), /*gpu_memory_buffer_factory=*/nullptr);
    EXPECT_TRUE(channel);
...
TEST_F(GpuChannelManagerTest, GpuPeakMemoryOnlyReportedForValidSequence)
{
    ...
    const CommandBufferId buffer_id = CommandBufferIdFromChannelAndRoute(42, 1337);
    ...
    EXPECT_THAT(result.value(),
        ::testing::ElementsAre(std::vector<std::string> { "start", "has_start_sources", "peak", "has_end_sources" },
            std::vector<std::string> { base::StringPrintf("%" PRIu64, current_memory), "1", base::StringPrintf("%" PRIu64, current_memory), "1" }));
}

// Tests that while a channel may exist for longer than a request to monitor,
// that only peaks seen are reported.
TEST_F(GpuChannelManagerTest, GpuPeakMemoryOnlyReportsPeaksFromObservationTime)
{
    GpuChannelManager* manager = channel_manager();
