...
[[maybe_unused]] constexpr auto kGpuWatchdogTimeoutForTestingSlow = base::Milliseconds(240);
[[maybe_unused]] constexpr auto kExtraGPUJobTimeForTestingSlow = base::Milliseconds(1000);

// For Fuchsia in which GpuWatchdogTest.GpuInitializationAndRunningTasks test
// is flaky.
[[maybe_unused]] constexpr auto kGpuWatchdogTimeoutForTestingSlowest = base::Milliseconds(1000);
[[maybe_unused]] constexpr auto kExtraGPUJobTimeForTestingSlowest = base::Milliseconds(4000);
...
public:
    ...
    void LongTaskWithReportProgress(base::TimeDelta duration, base::TimeDelta report_delta);

#if BUILDFLAG(IS_ANDROID)
    void LongTaskFromBackgroundToForeground(base::TimeDelta duration, base::TimeDelta extra_time, base::TimeDelta time_to_switch_to_foreground);
#endif
...
public:
    ...
    void LongTaskOnResume(base::TimeDelta duration, base::TimeDelta extra_time, base::TimeDelta time_to_power_resume);
...
void GpuWatchdogTest::SetUp()
{
    ...
    std::optional<base::SysInfo::HardwareModelNameSplit> split = base::SysInfo::SplitHardwareModelNameDoNotUse(model_str);

    if (split && split.value().category == "MacBookPro" && split.value().model < 14) {
        timeout_type = kSlow;
    }
...
void GpuWatchdogTest::LongTaskWithReportProgress(base::TimeDelta duration, base::TimeDelta report_delta)
{
...
#if BUILDFLAG(IS_ANDROID)
void GpuWatchdogTest::LongTaskFromBackgroundToForeground(base::TimeDelta duration, base::TimeDelta extra_time, base::TimeDelta time_to_switch_to_foreground)
{
...
void GpuWatchdogPowerTest::LongTaskOnResume(base::TimeDelta duration, base::TimeDelta extra_time, base::TimeDelta time_to_power_resume)
{
...
TEST_F(GpuWatchdogTest, GpuInitializationAndRunningTasks)
{
    ...
    task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&SimpleTask, normal_task_time,
    ...
    task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&GpuWatchdogTest::LongTaskWithReportProgress, base::Unretained(this), normal_long_task_time,
            /*report_progress_time*/ timeout_ / 4));

    task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE, run_loop.QuitClosure());
    run_loop.Run();
...
TEST_F(GpuWatchdogTest, GpuRunningATaskHang)
{
    ...
    task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE, base::BindOnce(&SimpleTask, allowed_time, extra_gpu_job_time_));
...
TEST_F(GpuWatchdogTest, ChromeInBackground)
{
    ...
    // Chrome is running in the background.
    task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&SimpleTask, normal_long_task_time,
            /*extra_time=*/base::TimeDelta()));
    task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE, run_loop.QuitClosure());
    run_loop.Run();
...
TEST_F(GpuWatchdogTest, GpuSwitchingToForegroundHang)
{
    ...
    task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&GpuWatchdogTest::LongTaskFromBackgroundToForeground, base::Unretained(this), /*duration*/ allowed_time,
...
TEST_F(GpuWatchdogPowerTest, GpuOnResumeHang)
{
    ...
    auto allowed_time = timeout_ * (kRestartFactor + 1) + full_thread_time_on_windows_;

    task_environment_.GetMainThreadTaskRunner()->PostTask(FROM_HERE,
        base::BindOnce(&GpuWatchdogPowerTest::LongTaskOnResume, base::Unretained(this),
