...
class GPU_IPC_SERVICE_EXPORT BuiltInShaderCacheWriter : public gles2::PassthroughProgramCache::ValueAddedHook {
public:
    // Written to beginning of file to ensure file was created by this code.
    static const uint32_t kSignature;

    explicit BuiltInShaderCacheWriter(const base::FilePath& path = base::FilePath());
    ~BuiltInShaderCacheWriter() override;

    void OnValueAddedToCache(const std::vector<uint8_t>& key, const std::vector<uint8_t>& value) override;
