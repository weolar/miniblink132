...
public:
    DCOMPTextureRepresentation(
        SharedImageManager* manager, SharedImageBacking* backing, MemoryTypeTracker* tracker, scoped_refptr<gl::DCOMPSurfaceProxy> dcomp_surface_proxy)
    ...
    {
        return std::make_optional<gl::DCLayerOverlayImage>(size(), dcomp_surface_proxy_);
    }
...
public:
    DCOMPTextureBacking(scoped_refptr<gl::DCOMPSurfaceProxy> dcomp_surface_proxy, const Mailbox& mailbox, const gfx::Size& size)
        : ClearTrackingSharedImageBacking(mailbox, viz::SinglePlaneFormat::kBGRA_8888, size, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin,
            kPremul_SkAlphaType, gpu::SHARED_IMAGE_USAGE_SCANOUT, {},
            /*estimated_size=*/0,
    ...
    {
        return std::make_unique<DCOMPTextureRepresentation>(manager, this, tracker, dcomp_surface_proxy_);
    }
...
// static
scoped_refptr<DCOMPTexture> DCOMPTexture::Create(GpuChannel* channel, int route_id, mojo::PendingAssociatedReceiver<mojom::DCOMPTexture> receiver)
{
    ...
    return base::WrapRefCounted(new DCOMPTexture(channel, route_id, std::move(receiver), std::move(context_state)));
}

DCOMPTexture::DCOMPTexture(
    GpuChannel* channel, int32_t route_id, mojo::PendingAssociatedReceiver<mojom::DCOMPTexture> receiver, scoped_refptr<SharedContextState> context_state)
    : channel_(channel)
    , route_id_(route_id)
    ...
    , receiver_(this)
{
    auto runner = base::MakeRefCounted<SchedulerTaskRunner>(*channel_->scheduler(), sequence_);
...
void DCOMPTexture::ResetSizeIfNeeded()
{
    ...
    if (!channel_ && protected_video_type_ == gfx::ProtectedVideoType::kHardwareProtected
        && base::TimeTicks::Now() - last_power_change_time_ < kPowerChangeDetectionGracePeriod) {
        DVLOG(1) << __func__
...
void DCOMPTexture::StartListening(mojo::PendingAssociatedRemote<mojom::DCOMPTextureClient> client)
{
...
void DCOMPTexture::SetDCOMPSurfaceHandle(const base::UnguessableToken& token, SetDCOMPSurfaceHandleCallback callback)
{
    DVLOG(1) << __func__;

    base::win::ScopedHandle surface_handle = gl::DCOMPSurfaceRegistry::GetInstance()->TakeDCOMPSurfaceHandle(token);
...
gpu::Mailbox DCOMPTexture::CreateSharedImage()
{
    ...
    auto shared_image = std::make_unique<DCOMPTextureBacking>(this, mailbox, size_);
...
void DCOMPTexture::SetParentWindow(HWND parent)
{
    if (last_parent_ != parent) {
        ...
        if (!window_pos_timer_.IsRunning()) {
            window_pos_timer_.Start(FROM_HERE, kParentWindowPosPollingPeriod, this, &DCOMPTexture::OnUpdateParentWindowRect);
        }
    }
}
...
void DCOMPTexture::SetProtectedVideoType(gfx::ProtectedVideoType protected_video_type)
{
    if (protected_video_type == protected_video_type_)
        return;

    DVLOG(2) << __func__ << ": protected_video_type=" << static_cast<int>(protected_video_type);
