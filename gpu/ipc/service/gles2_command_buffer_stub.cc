...
GLES2CommandBufferStub::GLES2CommandBufferStub(GpuChannel* channel, const mojom::CreateCommandBufferParams& init_params, CommandBufferId command_buffer_id,
    SequenceId sequence_id, int32_t stream_id, int32_t route_id)
    : CommandBufferStub(channel, init_params, command_buffer_id, sequence_id, stream_id, route_id)
...
gpu::ContextResult GLES2CommandBufferStub::Initialize(
    CommandBufferStub* share_command_buffer_stub, const mojom::CreateCommandBufferParams& init_params, base::UnsafeSharedMemoryRegion shared_state_shm)
{
    ...
    if (share_command_buffer_stub) {
        context_group_ = share_command_buffer_stub->decoder_context()->GetContextGroup();
        if (!context_group_) {
            LOG(ERROR) << "ContextResult::kFatalFailure: attempt to create a GLES2 "
                          "context sharing with a non-GLES2 context";
            return gpu::ContextResult::kFatalFailure;
        }
        if (context_group_->bind_generates_resource() != init_params.attribs.bind_generates_resource) {
            LOG(ERROR) << "ContextResult::kFatalFailure: attempt to create a shared "
                          "GLES2 context with inconsistent bind_generates_resource";
            return gpu::ContextResult::kFatalFailure;
        }
    } else {
        scoped_refptr<gles2::FeatureInfo> feature_info = new gles2::FeatureInfo(manager->gpu_driver_bug_workarounds(), manager->gpu_feature_info());
        context_group_ = new gles2::ContextGroup(manager->gpu_preferences(), gles2::PassthroughCommandDecoderSupported(), CreateMemoryTracker(),
            manager->shader_translator_cache(), manager->framebuffer_completeness_cache(), feature_info, init_params.attribs.bind_generates_resource,
            manager->watchdog() /* progress_reporter */, manager->gpu_feature_info(), manager->discardable_manager(),
            manager->passthrough_discardable_manager(), manager->shared_image_manager());
    }
    ...
    gles2_decoder_ = gles2::GLES2Decoder::Create(this, command_buffer_.get(), manager->outputter(), context_group_.get());
    set_decoder_context(std::unique_ptr<DecoderContext>(gles2_decoder_));

    scoped_sync_point_client_state_ = channel_->scheduler()->CreateSyncPointClientState(sequence_id_, CommandBufferNamespace::GPU_IO, command_buffer_id_);
    ...
    if (gpu_preference == gl::GpuPreference::kLowPower || gpu_preference == gl::GpuPreference::kNone || force_default_display) {
    ...
    gl::GLDisplay* display = gl::init::GetOrInitializeGLOneOffPlatformImplementation(
    ...
    if (manager->gpu_preferences().force_separate_egl_display_for_webgl_testing && features::SupportsEGLDualGPURendering()) {
    ...
    if (!offscreen) {
        ...
        surface_ = ImageTransportSurface::CreateNativeGLSurface(display, init_params.surface_handle, surface_format);
    ...
    if (use_virtualized_gl_context_ && share_group_) {
        ...
        if (context && (!context->MakeCurrent(surface_.get()) || context->CheckStickyGraphicsResetStatus() != GL_NO_ERROR)) {
            context = nullptr;
        }
        if (!context) {
            context
                = gl::init::CreateGLContext(share_group_.get(), surface_.get(), GenerateGLContextAttribsForDecoder(init_params.attribs, context_group_.get()));
            if (!context) {
        ...
        context = base::MakeRefCounted<GLContextVirtual>(share_group_.get(), context.get(), gles2_decoder_->AsWeakPtr());
        if (!context->Initialize(surface_.get(), GenerateGLContextAttribsForDecoder(init_params.attribs, context_group_.get()))) {
        ...
        context = gl::init::CreateGLContext(share_group_.get(), surface_.get(), GenerateGLContextAttribsForDecoder(init_params.attribs, context_group_.get()));
        if (!context) {
            // TODO(piman): This might not be fatal, we could recurse into
            // CreateGLContext to get more info, tho it should be exceedingly
            // rare and may not be recoverable anyway.
    ...
    // also not used.
    if (!context->GetGLStateRestorer() && !context_group_->use_passthrough_cmd_decoder()) {
        context->SetGLStateRestorer(new GLStateRestorerImpl(gles2_decoder_->AsWeakPtr()));
    }
    ...
    auto result = gles2_decoder_->Initialize(surface_, context, offscreen, gpu::gles2::DisallowedFeatures(), init_params.attribs);
    ...
    base::WritableSharedMemoryMapping shared_state_mapping = shared_state_shm.MapAt(0, kSharedStateSize);
    if (!shared_state_mapping.IsValid()) {
        LOG(ERROR) << "ContextResult::kFatalFailure: "
                      "Failed to map shared state buffer.";
        return gpu::ContextResult::kFatalFailure;
    }
    command_buffer_->SetSharedStateBuffer(MakeBackingFromSharedMemory(std::move(shared_state_shm), std::move(shared_state_mapping)));
    ...
    if (IsWebGLContextType(init_params.attribs.context_type)) {
        ...
        if (display_egl) {
            UMA_HISTOGRAM_ENUMERATION("GPU.WebGLDisplayType", display_egl->GetDisplayType(), gl::DISPLAY_TYPE_MAX);
            ...
            if (surface_area >= kLargeCanvasNumPixels) {
                UMA_HISTOGRAM_ENUMERATION("GPU.WebGLDisplayTypeLarge", display_egl->GetDisplayType(), gl::DISPLAY_TYPE_MAX);
            }
        }
    }
...
void GLES2CommandBufferStub::OnGpuSwitched(gl::GpuPreference active_gpu_heuristic)
{
    client().OnGpuSwitched(active_gpu_heuristic);
}

void GLES2CommandBufferStub::OnSetDefaultFramebufferSharedImage(const Mailbox& mailbox, int samples_count, bool preserve, bool needs_depth, bool needs_stencil)
{
    gles2_decoder_->SetDefaultFramebufferSharedImage(mailbox, samples_count, preserve, needs_depth, needs_stencil);
}

void GLES2CommandBufferStub::CreateGpuFenceFromHandle(uint32_t gpu_fence_id, gfx::GpuFenceHandle handle)
{
    ...
    if (gles2_decoder_->GetGpuFenceManager()->CreateGpuFenceFromHandle(gpu_fence_id, std::move(handle)))
        return;
...
void GLES2CommandBufferStub::GetGpuFenceHandle(uint32_t gpu_fence_id, GetGpuFenceHandleCallback callback)
{
    ScopedContextOperation operation(*this);
    if (!operation.is_context_current())
        return;
    ...
    if (manager->IsValidGpuFence(gpu_fence_id)) {
        std::unique_ptr<gfx::GpuFence> gpu_fence = manager->GetGpuFence(gpu_fence_id);
