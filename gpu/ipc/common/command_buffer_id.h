...
inline CommandBufferId CommandBufferIdFromChannelAndRoute(int channel_id, int32_t route_id)
{
    return CommandBufferId::FromUnsafeValue((static_cast<uint64_t>(channel_id) << 32) | route_id);
}

inline int ChannelIdFromCommandBufferId(gpu::CommandBufferId command_buffer_id)
{
    return static_cast<int>(command_buffer_id.GetUnsafeValue() >> 32);
}

inline int32_t RouteIdFromCommandBufferId(gpu::CommandBufferId command_buffer_id)
{
