...
{
    if (absl::holds_alternative<gpu::GpuDiskCacheGlShaderHandle>(handle)) {
        const auto& gl_shader_handle = absl::get<gpu::GpuDiskCacheGlShaderHandle>(handle);
        return gl_shader_handle == kDisplayCompositorGpuDiskCacheHandle || gl_shader_handle == kGrShaderGpuDiskCacheHandle;
    }
    if (absl::holds_alternative<gpu::GpuDiskCacheDawnGraphiteHandle>(handle)) {
        const auto& dawn_graphite_handle = absl::get<gpu::GpuDiskCacheDawnGraphiteHandle>(handle);
