...
template <> struct GPU_EXPORT StructTraits<gpu::mojom::SyncTokenDataView, gpu::SyncToken> {
    ...
    static gpu::mojom::CommandBufferNamespace namespace_id(const gpu::SyncToken& token)
    {
        return static_cast<gpu::mojom::CommandBufferNamespace>(token.namespace_id());
    }
    ...
    {
        *out = gpu::SyncToken(static_cast<gpu::CommandBufferNamespace>(data.namespace_id()), gpu::CommandBufferId::FromUnsafeValue(data.command_buffer_id()),
            data.release_count());
        if (out->HasData()) {
