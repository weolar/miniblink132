...
private:
    // TraitsTestService:
    void EchoGpuDevice(const GPUInfo::GPUDevice& g, EchoGpuDeviceCallback callback) override
    {
    ...
    void EchoMailboxHolder(const MailboxHolder& r, EchoMailboxHolderCallback callback) override
    {
        std::move(callback).Run(r);
    }

    void EchoSyncToken(const SyncToken& s, EchoSyncTokenCallback callback) override
    {
        std::move(callback).Run(s);
    }

    void EchoVideoDecodeAcceleratorSupportedProfile(
        const VideoDecodeAcceleratorSupportedProfile& v, EchoVideoDecodeAcceleratorSupportedProfileCallback callback) override
    {
        std::move(callback).Run(v);
    }

    void EchoVideoDecodeAcceleratorCapabilities(const VideoDecodeAcceleratorCapabilities& v, EchoVideoDecodeAcceleratorCapabilitiesCallback callback) override
    {
        std::move(callback).Run(v);
    }

    void EchoVideoEncodeAcceleratorSupportedProfile(
        const VideoEncodeAcceleratorSupportedProfile& v, EchoVideoEncodeAcceleratorSupportedProfileCallback callback) override
    {
        std::move(callback).Run(v);
    }

    void EchoGpuPreferences(const GpuPreferences& prefs, EchoGpuPreferencesCallback callback) override
    {
...
TEST_F(StructTraitsTest, GpuInfo)
{
    ...
    const gl::GLImplementationParts gl_implementation_parts(gl::ANGLEImplementation::kSwiftShader);
...
#endif
    const VideoDecodeAcceleratorSupportedProfiles video_decode_accelerator_supported_profiles;
    const VideoEncodeAcceleratorSupportedProfiles video_encode_accelerator_supported_profiles;
...
#endif
    input.video_decode_accelerator_supported_profiles = video_decode_accelerator_supported_profiles;
    input.video_encode_accelerator_supported_profiles = video_encode_accelerator_supported_profiles;
    ...
    EXPECT_EQ(gl_reset_notification_strategy, output.gl_reset_notification_strategy);
    ...
    EXPECT_EQ(rgb10a2_overlay_support, output.overlay_info.rgb10a2_overlay_support);
    EXPECT_EQ(p010_overlay_support, output.overlay_info.p010_overlay_support);
#endif
    for (size_t i = 0; i < video_decode_accelerator_supported_profiles.size(); i++) {
        const gpu::VideoDecodeAcceleratorSupportedProfile& expected = video_decode_accelerator_supported_profiles[i];
        const gpu::VideoDecodeAcceleratorSupportedProfile& actual = output.video_decode_accelerator_supported_profiles[i];
        EXPECT_EQ(expected.encrypted_only, actual.encrypted_only);
    }
    EXPECT_EQ(output.video_decode_accelerator_supported_profiles.size(), video_decode_accelerator_supported_profiles.size());
    EXPECT_EQ(output.video_encode_accelerator_supported_profiles.size(), video_encode_accelerator_supported_profiles.size());
}
...
{
    const gpu::VideoCodecProfile profile = gpu::VideoCodecProfile::H264PROFILE_MAIN;
...
TEST_F(StructTraitsTest, VideoDecodeAcceleratorCapabilities)
{
    ...
    input.flags = flags;
    input.supported_profiles.push_back(gpu::VideoDecodeAcceleratorSupportedProfile());
    input.supported_profiles.push_back(gpu::VideoDecodeAcceleratorSupportedProfile());
...
TEST_F(StructTraitsTest, GpuFeatureInfo)
{
    ...
    input.status_values[GPU_FEATURE_TYPE_ACCELERATED_2D_CANVAS] = gpu::kGpuFeatureStatusBlocklisted;
    input.status_values[GPU_FEATURE_TYPE_ACCELERATED_WEBGL] = gpu::kGpuFeatureStatusUndefined;
    input.status_values[GPU_FEATURE_TYPE_GPU_TILE_RASTERIZATION] = gpu::kGpuFeatureStatusDisabled;

    GpuFeatureInfo output;
    ASSERT_TRUE(mojom::GpuFeatureInfo::Deserialize(mojom::GpuFeatureInfo::Serialize(&input), &output));
