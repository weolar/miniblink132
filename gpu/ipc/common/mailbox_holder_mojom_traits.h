...
template <> struct GPU_EXPORT StructTraits<gpu::mojom::MailboxHolderDataView, gpu::MailboxHolder> {
    ...
    static bool Read(gpu::mojom::MailboxHolderDataView data, gpu::MailboxHolder* out)
    {
        if (!data.ReadMailbox(&out->mailbox) || !data.ReadSyncToken(&out->sync_token))
            return false;
