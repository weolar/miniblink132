...
template <> struct GPU_EXPORT StructTraits<gpu::mojom::ExportedSharedImageDataView, gpu::ExportedSharedImage> {
    static const gpu::Mailbox& mailbox(const gpu::ExportedSharedImage& holder)
    {
        return holder.mailbox_;
    }

    static const gpu::SharedImageMetadata& metadata(const gpu::ExportedSharedImage& shared_image)
    {
        return shared_image.metadata_;
    }

    static const gpu::SyncToken& creation_sync_token(const gpu::ExportedSharedImage& shared_image)
    {
    ...
    static bool Read(gpu::mojom::ExportedSharedImageDataView data, gpu::ExportedSharedImage* out)
    {
        if (!data.ReadMailbox(&out->mailbox_) || !data.ReadMetadata(&out->metadata_) || !data.ReadCreationSyncToken(&out->creation_sync_token_)) {
            return false;
        }
