...
bool GpuSurfaceTracker::IsValidSurfaceHandle(gpu::SurfaceHandle surface_handle) const
{
    base::AutoLock lock(surface_map_lock_);
...
SurfaceRecord GpuSurfaceTracker::AcquireJavaSurface(gpu::SurfaceHandle surface_handle)
{
    base::AutoLock lock(surface_map_lock_);
    ...
    return absl::visit(base::Overloaded { [&](const gl::ScopedJavaSurface& surface) {
                                             DCHECK(surface.IsValid());
                                             return SurfaceRecord(
                                                 surface.CopyRetainOwnership(), it->second.can_be_used_with_surface_control, it->second.host_input_token);
                                         },
                           [&](const gl::ScopedJavaSurfaceControl& surface_control) { return SurfaceRecord(surface_control.CopyRetainOwnership()); } },
