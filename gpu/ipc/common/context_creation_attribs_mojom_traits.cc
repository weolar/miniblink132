...
bool StructTraits<gpu::mojom::ContextCreationAttribsDataView, gpu::ContextCreationAttribs>::Read(
    gpu::mojom::ContextCreationAttribsDataView data, gpu::ContextCreationAttribs* out)
{
    if (!data.ReadGpuPreference(&out->gpu_preference) || !data.ReadContextType(&out->context_type)) {
        return false;
    }
