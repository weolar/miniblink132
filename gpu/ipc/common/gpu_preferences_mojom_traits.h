...
template <> struct GPU_EXPORT EnumTraits<gpu::mojom::GrContextType, gpu::GrContextType> {
    ...
    static bool FromMojom(gpu::mojom::GrContextType input, gpu::GrContextType* out)
    {
        switch (input) {
...
template <> struct GPU_EXPORT EnumTraits<gpu::mojom::VulkanImplementationName, gpu::VulkanImplementationName> {
    static gpu::mojom::VulkanImplementationName ToMojom(gpu::VulkanImplementationName input)
    {
        switch (input) {
    ...
    static bool FromMojom(gpu::mojom::VulkanImplementationName input, gpu::VulkanImplementationName* out)
    {
        switch (input) {
...
template <> struct GPU_EXPORT EnumTraits<gpu::mojom::WebGPUAdapterName, gpu::WebGPUAdapterName> {
    ...
    static bool FromMojom(gpu::mojom::WebGPUAdapterName input, gpu::WebGPUAdapterName* out)
    {
        switch (input) {
...
template <> struct GPU_EXPORT EnumTraits<gpu::mojom::WebGPUPowerPreference, gpu::WebGPUPowerPreference> {
    static gpu::mojom::WebGPUPowerPreference ToMojom(gpu::WebGPUPowerPreference input)
    {
        switch (input) {
    ...
    static bool FromMojom(gpu::mojom::WebGPUPowerPreference input, gpu::WebGPUPowerPreference* out)
    {
        switch (input) {
...
template <> struct GPU_EXPORT EnumTraits<gpu::mojom::DawnBackendValidationLevel, gpu::DawnBackendValidationLevel> {
    static gpu::mojom::DawnBackendValidationLevel ToMojom(gpu::DawnBackendValidationLevel input)
    {
        switch (input) {
    ...
    static bool FromMojom(gpu::mojom::DawnBackendValidationLevel input, gpu::DawnBackendValidationLevel* out)
    {
        switch (input) {
...
template <> struct GPU_EXPORT StructTraits<gpu::mojom::GpuPreferencesDataView, gpu::GpuPreferences> {
    static bool Read(gpu::mojom::GpuPreferencesDataView prefs, gpu::GpuPreferences* out)
    {
        out->disable_accelerated_video_decode = prefs.disable_accelerated_video_decode();
        out->disable_accelerated_video_encode = prefs.disable_accelerated_video_encode();
        ...
        out->log_gpu_control_list_decisions = prefs.log_gpu_control_list_decisions();
        out->compile_shader_always_succeeds = prefs.compile_shader_always_succeeds();
        ...
        out->enable_gpu_service_logging_gpu = prefs.enable_gpu_service_logging_gpu();
        out->enable_gpu_driver_debug_logging = prefs.enable_gpu_driver_debug_logging();
        ...
        out->force_gpu_mem_discardable_limit_bytes = prefs.force_gpu_mem_discardable_limit_bytes();
        ...
        out->enable_threaded_texture_mailboxes = prefs.enable_threaded_texture_mailboxes();
        out->gl_shader_interm_output = prefs.gl_shader_interm_output();
        out->enable_android_surface_control = prefs.enable_android_surface_control();
        ...
        out->enable_vulkan_protected_memory = prefs.enable_vulkan_protected_memory();
        out->disable_vulkan_surface = prefs.disable_vulkan_surface();
        out->disable_vulkan_fallback_to_gl_for_testing = prefs.disable_vulkan_fallback_to_gl_for_testing();
        ...
        out->enable_gpu_benchmarking_extension = prefs.enable_gpu_benchmarking_extension();
        out->enable_webgpu = prefs.enable_webgpu();
        out->enable_unsafe_webgpu = prefs.enable_unsafe_webgpu();
        out->enable_webgpu_developer_features = prefs.enable_webgpu_developer_features();
        out->enable_webgpu_experimental_features = prefs.enable_webgpu_experimental_features();
        if (!prefs.ReadUseWebgpuAdapter(&out->use_webgpu_adapter))
            return false;
        if (!prefs.ReadUseWebgpuPowerPreference(&out->use_webgpu_power_preference)) {
            return false;
        }
        out->force_webgpu_compat = prefs.force_webgpu_compat();
        if (!prefs.ReadEnableDawnBackendValidation(&out->enable_dawn_backend_validation))
            return false;
        ...
        out->enable_native_gpu_memory_buffers = prefs.enable_native_gpu_memory_buffers();

        out->force_separate_egl_display_for_webgl_testing = prefs.force_separate_egl_display_for_webgl_testing();

        return true;
    }

    static bool disable_accelerated_video_decode(const gpu::GpuPreferences& prefs)
    {
        return prefs.disable_accelerated_video_decode;
    }
    static bool disable_accelerated_video_encode(const gpu::GpuPreferences& prefs)
    {
    ...
    static bool enable_gpu_driver_debug_logging(const gpu::GpuPreferences& prefs)
    {
    ...
    static uint32_t force_gpu_mem_available_bytes(const gpu::GpuPreferences& prefs)
    {
        return prefs.force_gpu_mem_available_bytes;
    }
    static uint32_t force_gpu_mem_discardable_limit_bytes(const gpu::GpuPreferences& prefs)
    {
    ...
    static bool enable_threaded_texture_mailboxes(const gpu::GpuPreferences& prefs)
    {
    ...
    static gpu::VulkanImplementationName use_vulkan(const gpu::GpuPreferences& prefs)
    {
    ...
    static bool disable_vulkan_fallback_to_gl_for_testing(const gpu::GpuPreferences& prefs)
    {
    ...
    static uint32_t vulkan_sync_cpu_memory_limit(const gpu::GpuPreferences& prefs)
    {
        return prefs.vulkan_sync_cpu_memory_limit;
    }
    static bool enable_gpu_benchmarking_extension(const gpu::GpuPreferences& prefs)
    {
    ...
    static bool enable_webgpu_developer_features(const gpu::GpuPreferences& prefs)
    {
        return prefs.enable_webgpu_developer_features;
    }
    static bool enable_webgpu_experimental_features(const gpu::GpuPreferences& prefs)
    {
        return prefs.enable_webgpu_experimental_features;
    }
    static gpu::WebGPUAdapterName use_webgpu_adapter(const gpu::GpuPreferences& prefs)
    {
        return prefs.use_webgpu_adapter;
    }
    static gpu::WebGPUPowerPreference use_webgpu_power_preference(const gpu::GpuPreferences& prefs)
    {
    ...
    static gpu::DawnBackendValidationLevel enable_dawn_backend_validation(const gpu::GpuPreferences& prefs)
    {
        return prefs.enable_dawn_backend_validation;
    }
    static const std::vector<std::string>& enabled_dawn_features_list(const gpu::GpuPreferences& prefs)
    {
        return prefs.enabled_dawn_features_list;
    }
    static const std::vector<std::string>& disabled_dawn_features_list(const gpu::GpuPreferences& prefs)
    {
...
#if BUILDFLAG(IS_OZONE)
    static base::MessagePumpType message_pump_type(const gpu::GpuPreferences& prefs)
    {
        return prefs.message_pump_type;
    }
#endif
    static bool enable_native_gpu_memory_buffers(const gpu::GpuPreferences& prefs)
    {
        return prefs.enable_native_gpu_memory_buffers;
    }
    static bool force_separate_egl_display_for_webgl_testing(const gpu::GpuPreferences& prefs)
    {
