...
template <> struct GPU_EXPORT StructTraits<gpu::mojom::VideoMemoryProcessStatsDataView, gpu::VideoMemoryUsageStats::ProcessStats> {
    static uint64_t video_memory_bytes(const gpu::VideoMemoryUsageStats::ProcessStats& state)
    {
        return state.video_memory;
    }

    static bool has_duplicates(const gpu::VideoMemoryUsageStats::ProcessStats& state)
    {
        return state.has_duplicates;
    }

    static bool Read(gpu::mojom::VideoMemoryProcessStatsDataView data, gpu::VideoMemoryUsageStats::ProcessStats* out)
    {
...
template <> struct GPU_EXPORT StructTraits<gpu::mojom::VideoMemoryUsageStatsDataView, gpu::VideoMemoryUsageStats> {
    static std::map<int32_t, gpu::VideoMemoryUsageStats::ProcessStats> process_map(const gpu::VideoMemoryUsageStats& stats)
    {
    ...
    static bool Read(gpu::mojom::VideoMemoryUsageStatsDataView data, gpu::VideoMemoryUsageStats* out)
    {
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_FUCHSIA)
