...
public:
    D3D11ScopedTextureUnmap(Microsoft::WRL::ComPtr<ID3D11DeviceContext> context, Microsoft::WRL::ComPtr<ID3D11Texture2D> texture);
...
public:
    DXGIScopedReleaseKeyedMutex(Microsoft::WRL::ComPtr<IDXGIKeyedMutex> keyed_mutex, UINT64 key);

    DXGIScopedReleaseKeyedMutex(const DXGIScopedReleaseKeyedMutex&) = delete;

    DXGIScopedReleaseKeyedMutex& operator=(const DXGIScopedReleaseKeyedMutex&) = delete;
...
GPU_EXPORT bool CopyD3D11TexToMem(ID3D11Texture2D* input_texture, uint8_t* dst_buffer, size_t buffer_size, ID3D11Device* d3d11_device,
