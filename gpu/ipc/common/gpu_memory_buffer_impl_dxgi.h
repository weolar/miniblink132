...
public:
    ...
    static constexpr gfx::GpuMemoryBufferType kBufferType = gfx::DXGI_SHARED_HANDLE;

    static std::unique_ptr<GpuMemoryBufferImplDXGI> CreateFromHandle(gfx::GpuMemoryBufferHandle handle, const gfx::Size& size, gfx::BufferFormat format,
        gfx::BufferUsage usage, DestructionCallback callback, gpu::GpuMemoryBufferManager* gpu_memory_buffer_manager,
        scoped_refptr<base::UnsafeSharedMemoryPool> pool, base::span<uint8_t> premapped_memory = base::span<uint8_t>());
...
private:
    GpuMemoryBufferImplDXGI(gfx::GpuMemoryBufferId id, const gfx::Size& size, gfx::BufferFormat format, DestructionCallback callback,
        base::win::ScopedHandle dxgi_handle, gfx::DXGIHandleToken dxgi_token, GpuMemoryBufferManager* gpu_memory_buffer_manager,
    ...
    std::optional<base::OnceCallback<void(void)>> DoMapAsync(base::OnceCallback<void(bool)>);
    ...
    std::vector<base::OnceCallback<void(bool)>> map_callbacks_ GUARDED_BY(map_lock_);
