...
GPU_EXPORT std::ostream& operator<<(std::ostream& s, const GpuDiskCacheType& type);
...
GPU_EXPORT base::FilePath::StringType GetGpuDiskCacheSubdir(GpuDiskCacheType type);
...
using GpuDiskCacheGlShaderHandle = base::IdType<class GpuDiskCacheGlShader, int32_t, std::numeric_limits<int32_t>::min(), 1>;

// Dawn WebGPU cache handles (for the most part, these should be 1:1 per
// profile).
using GpuDiskCacheDawnWebGPUHandle = base::IdType<class GpuDiskCacheDawnWebGPU, int32_t, std::numeric_limits<int32_t>::min(), 1>;

// Dawn Graphite cache handles (for the most part, these should be 1:1 per
// profile).
using GpuDiskCacheDawnGraphiteHandle = base::IdType<class GpuDiskCacheDawnGraphite, int32_t, std::numeric_limits<int32_t>::min(), 1>;
//
// Variant handle that encompasses all possible handles, and utilities.
//
using GpuDiskCacheHandle = absl::variant<absl::monostate, GpuDiskCacheGlShaderHandle, GpuDiskCacheDawnWebGPUHandle, GpuDiskCacheDawnGraphiteHandle>;
...
GPU_EXPORT std::ostream& operator<<(std::ostream& s, const GpuDiskCacheHandle& handle);
