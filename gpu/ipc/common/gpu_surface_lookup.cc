...
SurfaceRecord::SurfaceRecord(gl::ScopedJavaSurface surface, bool can_be_used_with_surface_control, const base::android::JavaRef<jobject>& host_input_token)
    ...
    , host_input_token(host_input_token)
{
    ...
    if (host_input_token) {
        DCHECK(env->IsInstanceOf(host_input_token.obj(), android_window_InputTransferToken_clazz(env)));
    }
#endif // DCHECK_IS_ON()
}
