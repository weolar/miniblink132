...
protected:
    ...
    void InitCache()
    {
        ...
        handle_ = factory_->GetCacheHandle(GpuDiskCacheType::kGlShaders, cache_path());
    }
...
TEST_F(GpuDiskCacheTest, ClearByPathTriggersCallback)
{
    ...
    factory()->ClearByPath(cache_path(), base::Time(), base::Time::Max(), base::BindLambdaForTesting([&]() { test_callback.callback().Run(1); }));
...
{
    net::TestCompletionCallback test_callback;
    factory()->ClearByPath(base::FilePath(), base::Time(), base::Time::Max(), base::BindLambdaForTesting([&]() { test_callback.callback().Run(1); }));
    ASSERT_TRUE(test_callback.WaitForResult());
}
...
TEST_F(GpuDiskCacheTest, MultipleLoaderCallbacks)
{
    ...
    int loaded_calls = 0;
    cache = factory()->Create(handle_,
        base::BindLambdaForTesting([&loaded_calls](const GpuDiskCacheHandle& handle, const std::string& key, const std::string& value) { ++loaded_calls; }));
    ASSERT_TRUE(cache.get() != nullptr);
    net::TestCompletionCallback available_cb2;
    int rv2 = cache->SetAvailableCallback(available_cb2.callback());
    ASSERT_EQ(net::OK, available_cb2.GetResult(rv2));
...
TEST_F(GpuDiskCacheTest, DestroyedCallbackCalledOneInstance)
{
    ...
    base::RunLoop run_loop;
    {
        scoped_refptr<GpuDiskCache> cache
            = factory()->Create(handle_, base::DoNothing(), base::BindLambdaForTesting([&destroyed, &run_loop](const GpuDiskCacheHandle&) {
                  destroyed = true;
                  run_loop.Quit();
              }));
        ASSERT_TRUE(cache.get() != nullptr);
