...
public:
    GpuDiskCacheEntry(GpuDiskCache* cache, const std::string& key, const std::string& blob);
...
public:
    ...
    GpuDiskCacheReadHelper(GpuDiskCache* cache, const BlobLoadedCallback& callback);
...
private:
    enum OpType { TERMINATE, OPEN_NEXT, OPEN_NEXT_COMPLETE, READ_COMPLETE, ITERATION_FINISHED };
...
public:
    GpuDiskCacheClearHelper(
        GpuDiskCacheFactory* factory, scoped_refptr<GpuDiskCache> cache, base::Time delete_begin, base::Time delete_end, base::OnceClosure callback);
...
GpuDiskCacheEntry::GpuDiskCacheEntry(GpuDiskCache* cache, const std::string& key, const std::string& blob)
    : cache_(cache)
    , key_(key)
    , blob_(blob)
    , entry_(nullptr)
...
int GpuDiskCacheEntry::OpenEntry()
{
    ...
    auto callback = base::BindOnce(&GpuDiskCacheEntry::OnEntryOpenComplete, weak_ptr_factory_.GetWeakPtr());

    disk_cache::EntryResult result = cache_->backend()->OpenEntry(key_, net::HIGHEST, std::move(callback));
...
int GpuDiskCacheEntry::OpenCallback(int rv)
{
    ...
    disk_cache::EntryResult create_result = cache_->backend()->CreateEntry(key_, net::HIGHEST, std::move(callback));
...
int GpuDiskCacheEntry::WriteCallback(int rv)
{
    ...
    return entry_->WriteData(1, 0, io_buf.get(), blob_.length(), base::BindOnce(&GpuDiskCacheEntry::OnOpComplete, weak_ptr_factory_.GetWeakPtr()), false);
}
...
GpuDiskCacheReadHelper::GpuDiskCacheReadHelper(GpuDiskCache* cache, const BlobLoadedCallback& callback)
    : cache_(cache)
...
void GpuDiskCacheReadHelper::OnEntryOpenComplete(disk_cache::EntryResult result)
{
    int rv = result.net_error();
    entry_ = result.ReleaseEntry();
    OnOpComplete(rv);
...
int GpuDiskCacheReadHelper::OpenNextEntry()
{
    ...
    auto callback = base::BindOnce(&GpuDiskCacheReadHelper::OnEntryOpenComplete, weak_ptr_factory_.GetWeakPtr());
...
int GpuDiskCacheReadHelper::OpenNextEntryComplete(int rv)
{
    ...
    return entry_->ReadData(1, 0, buf_.get(), buf_->size(), base::BindOnce(&GpuDiskCacheReadHelper::OnOpComplete, weak_ptr_factory_.GetWeakPtr()));
}
...
int GpuDiskCacheReadHelper::ReadComplete(int rv)
{
    ...
    if (rv && rv == buf_->size() && !blob_loaded_callback_.is_null()) {
        blob_loaded_callback_.Run(entry_->GetKey(), std::string(buf_->data(), buf_->size()));
    }
...
void GpuDiskCacheClearHelper::DoClearGpuCache(int rv)
{
    ...
    while (rv != net::ERR_IO_PENDING) {
        ...
        case VERIFY_CACHE_SETUP:
            rv = cache_->SetAvailableCallback(base::BindOnce(&GpuDiskCacheClearHelper::DoClearGpuCache, weak_ptr_factory_.GetWeakPtr()));
            op_type_ = DELETE_CACHE;
            break;
        case DELETE_CACHE:
            rv = cache_->Clear(delete_begin_, delete_end_, base::BindOnce(&GpuDiskCacheClearHelper::DoClearGpuCache, weak_ptr_factory_.GetWeakPtr()));
            op_type_ = TERMINATE;
            break;
        case TERMINATE:
...
GpuDiskCacheFactory::GpuDiskCacheFactory(const HandleToPathMap& reserved_handles)
{
...
GpuDiskCacheHandle GpuDiskCacheFactory::GetCacheHandle(GpuDiskCacheType type, const base::FilePath& path)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
scoped_refptr<GpuDiskCache> GpuDiskCacheFactory::Get(const GpuDiskCacheHandle& handle)
{
...
scoped_refptr<GpuDiskCache> GpuDiskCacheFactory::Create(
    const GpuDiskCacheHandle& handle, const BlobLoadedForCacheCallback& blob_loaded_cb, CacheDestroyedCallback cache_destroyed_cb)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    return GetOrCreateByPath(it->second, base::BindRepeating(blob_loaded_cb, handle), base::BindOnce(std::move(cache_destroyed_cb), handle));
}

scoped_refptr<GpuDiskCache> GpuDiskCacheFactory::GetOrCreateByPath(
    const base::FilePath& path, const GpuDiskCache::BlobLoadedCallback& blob_loaded_cb, base::OnceClosure cache_destroyed_cb)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    auto cache = base::WrapRefCounted(new GpuDiskCache(this, path, blob_loaded_cb, std::move(cache_destroyed_cb)));

    cache->Init();
    return cache;
}

void GpuDiskCacheFactory::AddToCache(const base::FilePath& key, GpuDiskCache* cache)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
...
void GpuDiskCacheFactory::ClearByCache(scoped_refptr<GpuDiskCache> cache, base::Time delete_begin, base::Time delete_end, base::OnceClosure callback)
{
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    ...
    auto helper = std::make_unique<GpuDiskCacheClearHelper>(this, cache, delete_begin, delete_end, std::move(callback));
...
void GpuDiskCacheFactory::ClearByPath(const base::FilePath& path, base::Time delete_begin, base::Time delete_end, base::OnceClosure callback)
{
    ...
    ClearByCache(iter->second.get(), delete_begin, delete_end, std::move(callback));
}
...
GpuDiskCache::GpuDiskCache(
    GpuDiskCacheFactory* factory, const base::FilePath& cache_path, const BlobLoadedCallback& blob_loaded_cb, base::OnceClosure cache_destroyed_cb)
    : factory_(factory)
...
void GpuDiskCache::Init()
{
    ...
    disk_cache::BackendResult rv = disk_cache::CreateCacheBackend(net::SHADER_CACHE, net::CACHE_BACKEND_DEFAULT,
        /*file_operations=*/nullptr, cache_path_, GetDefaultGpuDiskCacheSize(), disk_cache::ResetHandling::kResetOnError,
        /*net_log=*/nullptr, base::BindOnce(&GpuDiskCache::CacheCreatedCallback, this));
...
int GpuDiskCache::Clear(base::Time begin_time, base::Time end_time, net::CompletionOnceCallback completion_callback)
{
    int rv;
    if (begin_time.is_null()) {
        rv = backend_->DoomAllEntries(std::move(completion_callback));
    } else {
        rv = backend_->DoomEntriesBetween(begin_time, end_time, std::move(completion_callback));
    }
    return rv;
}
...
int GpuDiskCache::SetCacheCompleteCallback(net::CompletionOnceCallback callback)
{
    if (entries_.empty()) {
        return net::OK;
    }
