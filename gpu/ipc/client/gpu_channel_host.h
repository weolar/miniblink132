...
using GpuChannelEstablishedCallback = base::OnceCallback<void(scoped_refptr<GpuChannelHost>)>;
...
// IO thread.
class GPU_EXPORT GpuChannelHost : public base::RefCountedThreadSafe<GpuChannelHost> {
public:
    GpuChannelHost(int channel_id, const gpu::GPUInfo& gpu_info, const gpu::GpuFeatureInfo& gpu_feature_info,
    ...
    bool IsLost() const
    {
        return !connection_tracker_->is_connected();
    }
    ...
    // release.
    uint32_t OrderingBarrier(int32_t route_id, int32_t put_offset, std::vector<SyncToken> sync_token_fences, uint64_t release_count);
    ...
    uint32_t EnqueueDeferredMessage(mojom::DeferredRequestParamsPtr params, std::vector<SyncToken> sync_token_fences, uint64_t release_count);
    ...
    void CreateGpuMemoryBuffer(const gfx::Size& size, const viz::SharedImageFormat& format, gfx::BufferUsage buffer_usage, gfx::GpuMemoryBufferHandle* handle);
    ...
    // Virtual for testing.
    virtual scoped_refptr<ClientSharedImageInterface> CreateClientSharedImageInterface();
...
private:
    ...
    void EstablishSharedMemoryForFlushVerification() EXCLUSIVE_LOCKS_REQUIRED(shared_memory_version_lock_);
    ...
    struct ConnectionTracker : public base::RefCountedThreadSafe<ConnectionTracker> {
        ConnectionTracker();
    ...
    private:
        ...
        std::vector<GpuChannelLostObserver*> GUARDED_BY(channel_obs_lock_) observer_list_;
    };
    ...
    public:
        ...
        void Initialize(mojo::ScopedMessagePipeHandle handle, mojo::PendingAssociatedReceiver<mojom::GpuChannel> receiver,
    ...
    std::optional<mojo::SharedMemoryVersionClient> shared_memory_version_client_ GUARDED_BY(shared_memory_version_lock_);
    ...
    std::vector<mojom::DeferredRequestPtr> deferred_messages_ GUARDED_BY(deferred_message_lock_);
    std::optional<OrderingBarrierInfo> pending_ordering_barrier_ GUARDED_BY(deferred_message_lock_);
