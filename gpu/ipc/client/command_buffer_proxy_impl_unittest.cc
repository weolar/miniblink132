...
public:
    explicit TestGpuChannelHost(mojom::GpuChannel& gpu_channel)
        : GpuChannelHost(0 /* channel_id */, GPUInfo(), GpuFeatureInfo(), SharedImageCapabilities(),
            mojo::ScopedMessagePipeHandle(mojo::MessagePipeHandle(mojo::kInvalidHandleValue)))
        , gpu_channel_(gpu_channel)
    {
    }

    mojom::GpuChannel& GetGpuChannel() override
    {
        return *gpu_channel_;
    }
...
class CommandBufferProxyImplTest : public testing::WithParamInterface<std::tuple<bool, bool>>, public testing::Test {
public:
    CommandBufferProxyImplTest()
    {
        ...
        (std::get<0>(GetParam()) ? enabled_features : disabled_features).push_back(features::kConditionallySkipGpuChannelFlush);
        ...
        skip_flush_if_possible_ = base::FeatureList::IsEnabled(features::kConditionallySkipGpuChannelFlush);
    ...
    std::unique_ptr<CommandBufferProxyImpl> CreateAndInitializeProxy(MockCommandBuffer* mock_command_buffer = nullptr)
    {
        auto proxy = std::make_unique<CommandBufferProxyImpl>(channel_, 0 /* stream_id */, base::SingleThreadTaskRunner::GetCurrentDefault());
        ...
        EXPECT_CALL(mock_gpu_channel_, CreateCommandBuffer(_, _, _, _, _, _, _, _))
            .Times(1)
            .WillOnce(
                Invoke([&](mojom::CreateCommandBufferParamsPtr params, int32_t routing_id, base::UnsafeSharedMemoryRegion shared_state,
                           ...
                           ContextResult* result, Capabilities* capabilities, GLCapabilities* gl_capabilities) -> bool {
        ...
        proxy->Initialize(kNullSurfaceHandle, nullptr, SchedulingPriority::kNormal, ContextCreationAttribs(), GURL());
    ...
    void ExpectOrderingBarrier(const mojom::DeferredRequest& request, int32_t route_id, int32_t put_offset)
    {
        ASSERT_TRUE(request.params->is_command_buffer_request());

        const auto& command_buffer_request = *request.params->get_command_buffer_request();
        ASSERT_TRUE(command_buffer_request.params->is_async_flush());
        EXPECT_EQ(command_buffer_request.routing_id, route_id);

        const auto& flush_request = *command_buffer_request.params->get_async_flush();
    ...
    void ExpectFlush(int count)
    {
        if (skip_flush_if_possible_) {
            ...
            // Flush()
            EXPECT_CALL(mock_gpu_channel_, GetSharedMemoryForFlushId(Matcher<::base::ReadOnlySharedMemoryRegion*>(_))).Times(count);
        } else {
            EXPECT_CALL(mock_gpu_channel_, Flush()).Times(count).WillRepeatedly(Return(true));
        }
    }
...
protected:
    ...
    std::vector<mojo::PendingAssociatedRemote<mojom::CommandBufferClient>> clients_;
};
...
TEST_P(CommandBufferProxyImplTest, OrderingBarriersAreCoalescedWithFlush)
{
    ...
    EXPECT_CALL(mock_gpu_channel_, FlushDeferredRequests(_, _)).Times(1).WillOnce(Invoke([&](std::vector<mojom::DeferredRequestPtr> requests, int32_t id) {
    ...
    // Once for each proxy.
    EXPECT_CALL(mock_gpu_channel_, DestroyCommandBuffer(_)).Times(2).WillRepeatedly(Return(true));
...
TEST_P(CommandBufferProxyImplTest, EnqueueDeferredMessageEnqueuesPendingOrderingBarriers)
{
    auto proxy1 = CreateAndInitializeProxy();

    proxy1->OrderingBarrier(10);
    proxy1->OrderingBarrier(20);
    channel_->EnqueueDeferredMessage(mojom::DeferredRequestParams::NewCommandBufferRequest(mojom::DeferredCommandBufferRequest::New(
                                         proxy1->route_id(), mojom::DeferredCommandBufferRequestParams::NewDestroyTransferBuffer(3))),
    ...
    EXPECT_CALL(mock_gpu_channel_, DestroyCommandBuffer(_)).Times(1).WillOnce(Return(true));
...
{
    auto gpu_control_client = std::unique_ptr<MockGpuControlClient>(new testing::StrictMock<MockGpuControlClient>());
    ...
    EXPECT_CALL(*gpu_control_client, OnGpuControlLostContext()).Times(1).RetiresOnSaturation();

    // Passing kReturnNullOnOOM should not cause the context to be lost
    EXPECT_CALL(*gpu_control_client, OnGpuControlLostContextMaybeReentrant()).Times(0);

    int32_t id = -1;
    scoped_refptr<gpu::Buffer> transfer_buffer_oom
        = proxy->CreateTransferBuffer(std::numeric_limits<uint32_t>::max(), &id, 0, TransferBufferAllocationOption::kReturnNullOnOOM);
    ...
    scoped_refptr<gpu::Buffer> transfer_buffer = proxy->CreateTransferBuffer(16, &id);

    EXPECT_NE(transfer_buffer, nullptr);
    EXPECT_NE(id, -1);

    // Now, allocating with kLoseContextOnOOM should cause the context to be lost.
    EXPECT_CALL(*gpu_control_client, OnGpuControlLostContextMaybeReentrant()).Times(1).RetiresOnSaturation();

    transfer_buffer_oom = proxy->CreateTransferBuffer(std::numeric_limits<uint32_t>::max(), &id, 0, TransferBufferAllocationOption::kLoseContextOnOOM);
...
INSTANTIATE_TEST_SUITE_P(All, CommandBufferProxyImplTest, testing::Combine(testing::Values(false, true), testing::Values(false, true)));
} // namespace
} // namespace gpu
