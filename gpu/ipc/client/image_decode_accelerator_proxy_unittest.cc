...
public:
    ...
    TestGpuChannelHost(mojom::GpuChannel& gpu_channel, const GPUInfo& info)
        : GpuChannelHost(
            kChannelId, info, GpuFeatureInfo(), SharedImageCapabilities(), mojo::ScopedMessagePipeHandle(mojo::MessagePipeHandle(mojo::kInvalidHandleValue)))
...
public:
    ImageDecodeAcceleratorProxyTest()
        : gpu_channel_host_(base::MakeRefCounted<TestGpuChannelHost>(mock_gpu_channel_))
        , proxy_(gpu_channel_host_.get(), static_cast<int32_t>(GpuChannelReservedRoutes::kImageDecodeAccelerator))
    {
    }
...
TEST_F(ImageDecodeAcceleratorProxyTest, ScheduleImageDecodeSendsMessage)
{
    ...
    base::span<const uint8_t> encoded_data = base::span<const uint8_t>(image, std::size(image));
    ...
    EXPECT_CALL(mock_gpu_channel_, ScheduleImageDecode(ImageDecodeParamsEqualTo(expected_params), Eq(kExpectedReleaseCount))).Times(1);

    SyncToken token = proxy_.ScheduleImageDecode(encoded_data, kOutputSize, CommandBufferIdFromChannelAndRoute(kChannelId, kRasterCmdBufferRouteId),
    ...
    EXPECT_EQ(ChannelIdFromCommandBufferId(token.command_buffer_id()), kChannelId);
    EXPECT_EQ(RouteIdFromCommandBufferId(token.command_buffer_id()), static_cast<int32_t>(GpuChannelReservedRoutes::kImageDecodeAccelerator));
    EXPECT_EQ(token.release_count(), 1u);
}

class ImageDecodeAcceleratorProxySubsamplingTest : public testing::TestWithParam<cc::YUVSubsampling> {
public:
...
TEST_P(ImageDecodeAcceleratorProxySubsamplingTest, JPEGSubsamplingIsSupported)
{
    ...
    auto gpu_channel_host = base::MakeRefCounted<TestGpuChannelHost>(mock_gpu_channel_, gpu_info);
    ImageDecodeAcceleratorProxy proxy(gpu_channel_host.get(), static_cast<int32_t>(GpuChannelReservedRoutes::kImageDecodeAccelerator));

    EXPECT_TRUE(proxy.IsImageSupported(&image_metadata));
}

TEST_P(ImageDecodeAcceleratorProxySubsamplingTest, JPEGSubsamplingIsNotSupported)
{
    cc::ImageHeaderMetadata image_metadata;
    image_metadata.yuv_subsampling = GetParam();
    image_metadata.image_type = cc::ImageType::kJPEG;
...
INSTANTIATE_TEST_SUITE_P(ImageDecodeAcceleratorProxySubsample, ImageDecodeAcceleratorProxySubsamplingTest,
    testing::Values(cc::YUVSubsampling::k410, cc::YUVSubsampling::k411, cc::YUVSubsampling::k420, cc::YUVSubsampling::k422, cc::YUVSubsampling::k440,
        cc::YUVSubsampling::k444, cc::YUVSubsampling::kUnknown));
