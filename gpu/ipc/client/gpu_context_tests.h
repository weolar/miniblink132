...
CONTEXT_TEST_F(SignalTest, InvalidSignalSyncTokenTest)
{
    ...
    // Signalling something that doesn't exist should run the callback
    // immediately.
    gpu::SyncToken sync_token(gpu::CommandBufferNamespace::GPU_IO, gpu::CommandBufferId::FromUnsafeValue(1297824234), 9123743439);
    TestSignalSyncToken(sync_token);
}
...
public:
    ...
    void TestReceive(base::OnceClosure quit_cb, std::unique_ptr<gfx::GpuFence> gpu_fence)
    {
...
CONTEXT_TEST_F(GpuFenceTest, BasicGpuFenceTest)
{
    ...
    base::OnceCallback<void(std::unique_ptr<gfx::GpuFence>)> callback
        = base::BindOnce(&GpuFenceTest::TestReceive, base::Unretained(this), run_loop.QuitClosure());
    ...
    GLuint id2 = gl_->CreateClientGpuFenceCHROMIUM(received_gpu_fence_->AsClientGpuFence());
