...
// CommandBufferStub.
class GPU_EXPORT CommandBufferProxyImpl : public gpu::CommandBuffer, public gpu::GpuControl, public mojom::CommandBufferClient {
public:
    ...
    CommandBufferProxyImpl(scoped_refptr<GpuChannelHost> channel, int32_t stream_id, scoped_refptr<base::SequencedTaskRunner> task_runner,
    ...
    ContextResult Initialize(gpu::SurfaceHandle surface_handle, CommandBufferProxyImpl* share_group, gpu::SchedulingPriority stream_priority,
    ...
    void SetDefaultFramebufferSharedImage(
    ...
    const scoped_refptr<GpuChannelHost>& channel() const
    {
        return channel_;
    }
...
private:
    ...
    std::pair<base::UnsafeSharedMemoryRegion, base::WritableSharedMemoryMapping> AllocateAndMapSharedMemory(
        size_t size, base::SharedMemoryMapper* mapper = nullptr);
    ...
    void OnGetGpuFenceHandleComplete(uint32_t gpu_fence_id, base::OnceCallback<void(std::unique_ptr<gfx::GpuFence>)> callback, gfx::GpuFenceHandle);
    ...
    // has an error.
    void TryUpdateStateDontReportError() EXCLUSIVE_LOCKS_REQUIRED(last_state_lock_);
    // Sets the state, and calls OnGpuStateError() if the new state has an error.
    void SetStateFromMessageReply(const CommandBuffer::State& state) EXCLUSIVE_LOCKS_REQUIRED(last_state_lock_);
    ...
    void OnGpuAsyncMessageError(gpu::error::ContextLostReason reason, gpu::error::Error error) EXCLUSIVE_LOCKS_REQUIRED(last_state_lock_);
    ...
    // errors.
    void OnClientError(gpu::error::Error error) EXCLUSIVE_LOCKS_REQUIRED(last_state_lock_);

    // Helper methods, don't call these directly.
    void DisconnectChannelInFreshCallStack() EXCLUSIVE_LOCKS_REQUIRED(last_state_lock_);
    ...
    {
        return const_cast<gpu::CommandBufferSharedState*>(std::as_const(*this).shared_state());
    }
    ...
    raw_ptr<base::HistogramBase> uma_histogram_ensure_work_visible_duration_ = nullptr;
