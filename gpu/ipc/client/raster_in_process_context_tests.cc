...
constexpr viz::SharedImageFormat kSharedImageFormat = viz::SinglePlaneFormat::kRGBA_8888;
...
public:
    RasterInProcessCommandBufferTest()
    {
        ...
        gpu_feature_info->status_values[gpu::GPU_FEATURE_TYPE_GPU_TILE_RASTERIZATION] = gpu::kGpuFeatureStatusEnabled;
    }
    ...
    std::unique_ptr<RasterInProcessContext> CreateRasterInProcessContext()
    {
        ...
        auto result = context->Initialize(gpu_thread_holder_.GetTaskExecutor(), attributes, SharedMemoryLimits(),
    ...
    void SetUp() override
    {
        if (!RasterInProcessContext::SupportedInTest())
            return;
        gpu_thread_holder_.GetGpuPreferences()->gr_context_type = GrContextType::kGL;
...
TEST_F(RasterInProcessCommandBufferTest, AllowedBetweenBeginEndRasterCHROMIUM)
{
    ...
    SharedImageUsageSet flags = gpu::SHARED_IMAGE_USAGE_RASTER_READ | gpu::SHARED_IMAGE_USAGE_RASTER_WRITE | gpu::SHARED_IMAGE_USAGE_OOP_RASTERIZATION;
    scoped_refptr<gpu::ClientSharedImage> shared_image
        = sii->CreateSharedImage({ kSharedImageFormat, kBufferSize, color_space, flags, "TestLabel" }, kNullSurfaceHandle);
