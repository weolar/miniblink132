...
public:
    ...
    GpuInProcessThreadServiceDelegate(const GpuInProcessThreadServiceDelegate&) = delete;
    GpuInProcessThreadServiceDelegate& operator=(const GpuInProcessThreadServiceDelegate&) = delete;
...
class GL_IN_PROCESS_CONTEXT_EXPORT GpuInProcessThreadService : public CommandBufferTaskExecutor {
public:
    GpuInProcessThreadService(GpuInProcessThreadServiceDelegate* delegate, scoped_refptr<base::SingleThreadTaskRunner> task_runner, Scheduler* scheduler,
        SyncPointManager* sync_point_manager, gl::GLSurfaceFormat share_group_surface_format, const GpuFeatureInfo& gpu_feature_info,
    ...
    GpuInProcessThreadService& operator=(const GpuInProcessThreadService&) = delete;
