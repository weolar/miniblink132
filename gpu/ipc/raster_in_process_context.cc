...
ContextResult RasterInProcessContext::Initialize(CommandBufferTaskExecutor* task_executor, const ContextCreationAttribs& attribs,
    const SharedMemoryLimits& memory_limits, gpu::raster::GrShaderCache* gr_shader_cache, GpuProcessShmCount* use_shader_cache_shm_count)
{
    ...
    command_buffer_ = std::make_unique<InProcessCommandBuffer>(task_executor, GURL());
    auto result = command_buffer_->Initialize(attribs, base::SingleThreadTaskRunner::GetCurrentDefault(), gr_shader_cache, use_shader_cache_shm_count);
    ...
    auto raster_helper = std::make_unique<raster::RasterCmdHelper>(command_buffer_.get());
    ...
    raster_implementation_ = std::make_unique<raster::RasterImplementation>(raster_helper.get(), transfer_buffer_.get(), bind_generates_resource,
        attribs.lose_context_when_out_of_memory, command_buffer_.get(), nullptr /* image_decode_accelerator */);
    result = raster_implementation_->Initialize(memory_limits);
    raster_implementation_->SetLostContextCallback(base::BindOnce([]() { EXPECT_TRUE(false) << "Unexpected lost context."; }));
...
InProcessCommandBuffer* RasterInProcessContext::GetCommandBufferForTest() const
{
...
bool RasterInProcessContext::SupportedInTest()
{
    ...
    return !gpu_preferences.use_passthrough_cmd_decoder || !gles2::PassthroughCommandDecoderSupported();
}
