...
#if BUILDFLAG(IS_WIN)
enum class OverlaySupport { kNone = 0, kDirect = 1, kScaling = 2, kSoftware = 3 };
...
struct GPU_EXPORT OverlayInfo {
    ...
    {
        return direct_composition == other.direct_composition && supports_overlays == other.supports_overlays
            && yuy2_overlay_support == other.yuy2_overlay_support && nv12_overlay_support == other.nv12_overlay_support
            && bgra8_overlay_support == other.bgra8_overlay_support && rgb10a2_overlay_support == other.rgb10a2_overlay_support;
    }
    bool operator!=(const OverlayInfo& other) const
    {
        return !(*this == other);
...
// Whether the browser was built with ASAN or not.
    ...
    public:
        ...
        virtual void AddBinary(const char* name, const base::span<const uint8_t>& blob) = 0;
