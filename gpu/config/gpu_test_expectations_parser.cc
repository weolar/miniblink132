...
{
    // Some sanity check.
    DCHECK_EQ(static_cast<unsigned int>(kNumberOfExactMatchTokens), sizeof(kTokenData) / sizeof(kTokenData[0]));
    DCHECK_EQ(static_cast<unsigned int>(kNumberOfErrors), sizeof(kErrorMessage) / sizeof(kErrorMessage[0]));
}
...
bool GPUTestExpectationsParser::LoadTestExpectations(const std::string& data)
{
    ...
    std::vector<std::string> lines = base::SplitString(data, "\n", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    bool rt = true;
...
bool GPUTestExpectationsParser::LoadTestExpectations(const base::FilePath& path)
{
    entries_.clear();
    error_messages_.clear();
...
int32_t GPUTestExpectationsParser::GetTestExpectation(const std::string& test_name, const GPUTestBotConfig& bot_config) const
{
...
{
    DCHECK(config);
    std::vector<std::string> tokens = base::SplitString(config_data, base::kWhitespaceASCII, base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
{
    std::vector<std::string> tokens = base::SplitString(line_data, base::kWhitespaceASCII, base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    ...
    for (size_t i = 0; i < tokens.size() && !comments_encountered; ++i) {
        ...
        case kExpectationSkip:
            ...
            if ((kTokenData[token].flag & entry.test_expectation) != 0) {
                PushErrorMessage(kErrorMessage[kErrorEntryWithExpectationConflicts], line_number);
                return false;
            }
...
bool GPUTestExpectationsParser::UpdateTestConfig(GPUTestConfig* config, int32_t token, size_t line_number)
{
    DCHECK(config);
    switch (token) {
    case kConfigWinXP:
    ...
    case kConfigValidating:
        if ((config->command_decoder() & kTokenData[token].flag) != 0) {
            PushErrorMessage(kErrorMessage[kErrorEntryWithCommandDecoderConflicts], line_number);
            return false;
        }
        config->set_command_decoder(config->command_decoder() | kTokenData[token].flag);
        break;
    default:
        DCHECK(false);
        break;
...
bool GPUTestExpectationsParser::UpdateTestConfig(GPUTestConfig* config, const std::string& gpu_device_id, size_t line_number)
{
    DCHECK(config);
    ...
    for (size_t i = 0; i < entries_.size(); ++i) {
        for (size_t j = i + 1; j < entries_.size(); ++j) {
            if (entries_[i].test_name == entries_[j].test_name && entries_[i].test_config.OverlapsWith(entries_[j].test_config)) {
                PushErrorMessage(kErrorMessage[kErrorEntriesOverlap], entries_[i].line_number, entries_[j].line_number);
                rt = true;
            }
        }
    }
...
{
    error_messages_.push_back(base::StringPrintf("Line %d : %s", static_cast<int>(line_number), message.c_str()));
}

void GPUTestExpectationsParser::PushErrorMessage(const std::string& message, size_t entry1_line_number, size_t entry2_line_number)
{
    error_messages_.push_back(
        base::StringPrintf("Line %d and %d : %s", static_cast<int>(entry1_line_number), static_cast<int>(entry2_line_number), message.c_str()));
}
