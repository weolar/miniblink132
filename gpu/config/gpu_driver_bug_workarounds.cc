...
void IntSetToWorkarounds(const std::vector<int32_t>& enabled_workarounds, gpu::GpuDriverBugWorkarounds* workarounds)
{
    DCHECK(workarounds);
...
GpuDriverBugWorkarounds::GpuDriverBugWorkarounds(const std::vector<int>& enabled_driver_bug_workarounds)
{
    IntSetToWorkarounds(enabled_driver_bug_workarounds, this);
}

GpuDriverBugWorkarounds::GpuDriverBugWorkarounds(const GpuDriverBugWorkarounds& other) = default;
...
#undef GPU_OP
    ...
    webgl_or_caps_max_texture_size = LowerMax(webgl_or_caps_max_texture_size, extra.webgl_or_caps_max_texture_size);
    max_copy_texture_chromium_size = LowerMax(max_copy_texture_chromium_size, extra.max_copy_texture_chromium_size);
    max_3d_array_texture_size = LowerMax(max_3d_array_texture_size, extra.max_3d_array_texture_size);
}
