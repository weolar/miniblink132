...
void RecurseDiagnosticTree(DxDiagNode* output, IDxDiagContainer* container, int depth)
{
    HRESULT hr;
    ...
    if (depth > 0) {
        ...
        if (SUCCEEDED(hr)) {
            for (DWORD i = 0; i < child_count; i++) {
                ...
                hr = container->EnumChildContainerNames(i, child_name16, std::size(child_name16));
                if (SUCCEEDED(hr)) {
...
bool GetDxDiagnostics(DxDiagNode* output)
{
    ...
    hr = CoCreateInstance(CLSID_DxDiagProvider, nullptr, CLSCTX_INPROC_SERVER, IID_IDxDiagProvider, reinterpret_cast<void**>(&provider));
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                hr = root->GetChildContainer(L"DxDiag_DisplayDevices", &display_devices);
                if (SUCCEEDED(hr)) {
