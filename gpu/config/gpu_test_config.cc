...
{
    DCHECK_EQ(0, command_decoder & ~(kCommandDecoderPassthrough | kCommandDecoderValidating));
...
bool GPUTestBotConfig::Matches(const GPUTestConfig& config) const
{
    ...
    if (config.api() != 0 && (api() & config.api()) == 0)
        return false;
    if (config.command_decoder() != 0 && command_decoder() != config.command_decoder())
        return false;
    return true;
}
...
#else
        GPUInfo my_gpu_info;
        if (!CollectBasicGraphicsInfo(base::CommandLine::ForCurrentProcess(), &my_gpu_info)) {
