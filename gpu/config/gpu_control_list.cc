...
bool ProcessVersionString(const std::string& version_string, char splitter, std::vector<std::string>* version)
{
    DCHECK(version);
    *version = base::SplitString(version_string, std::string(1, splitter), base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    if (version->size() == 0)
        return false;
...
//        -1 if number < number_ref.
int CompareNumericalNumberStrings(const std::string& number, const std::string& number_ref)
{
    unsigned value1 = 0;
    unsigned value2 = 0;
...
int CompareLexicalNumberStrings(const std::string& number, const std::string& number_ref)
{
...
bool StringMismatch(const std::string& input, const std::string& pattern)
{
    if (input.empty() || pattern.empty())
        return false;
    static crash_reporter::CrashKeyString<128> crash_key("StringMismatch::pattern");
...
bool ProcessANGLEGLRenderer(const std::string& gl_renderer, std::string* vendor, std::string* renderer, std::string* version)
{
    ...
    std::string vendor_renderer_version = gl_renderer.substr(sizeof(kANGLEPrefix) - 1, len - sizeof(kANGLEPrefix));
    segments = base::SplitString(vendor_renderer_version, ",", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    if (segments.size() != 3) {
...
bool GpuControlList::Version::Contains(const std::string& version_string, char splitter) const
{
    if (op == kUnknown)
        return false;
    if (op == kAny)
...
// static
int GpuControlList::Version::Compare(const std::vector<std::string>& version, const std::vector<std::string>& version_ref, VersionStyle version_style)
{
...
bool GpuControlList::More::GLVersionInfoMismatch(const std::string& gl_version_string) const
{
    if (gl_version_string.empty())
        return false;
    if (!gl_version.IsSpecified() && gl_type == kGLTypeNone)
        return false;
    std::vector<std::string> segments = base::SplitString(gl_version_string, " ", base::TRIM_WHITESPACE, base::SPLIT_WANT_ALL);
    std::string number;
...
void GpuControlList::Entry::LogControlListMatch(const std::string& control_list_logging_name) const
{
    static const char kControlListMatchMessage[] = "Control list match for rule #%u in %s.";
    VLOG(1) << base::StringPrintf(kControlListMatchMessage, id, control_list_logging_name.c_str());
}

bool GpuControlList::DriverInfo::Contains(const std::vector<GPUInfo::GPUDevice>& gpus) const
{
    for (auto& gpu : gpus) {
        if (StringMismatch(gpu.driver_vendor, driver_vendor))
            continue;

        if (driver_version.IsSpecified() && !gpu.driver_version.empty() && !driver_version.Contains(gpu.driver_version)) {
            continue;
        }
        return true;
    }
...
bool GpuControlList::GLStrings::Contains(const GPUInfo& gpu_info) const
{
    ...
    std::string renderer;
    std::string version;
    bool is_angle_gl = ProcessANGLEGLRenderer(gpu_info.gl_renderer, &vendor, &renderer, &version);
    if (StringMismatch(is_angle_gl ? vendor : gpu_info.gl_vendor, gl_vendor)) {
        return false;
    }
    if (StringMismatch(is_angle_gl ? renderer : gpu_info.gl_renderer, gl_renderer)) {
        return false;
    }
...
bool GpuControlList::MachineModelInfo::Contains(const GPUInfo& gpu_info) const
{
    if (machine_model_name_size > 0) {
        ...
        for (size_t ii = 0; ii < machine_model_name_size; ++ii) {
            if (RE2::FullMatch(gpu_info.machine_model_name, machine_model_names[ii])) {
                found_match = true;
                break;
            }
        }
    ...
    if (machine_model_version.IsSpecified() && (gpu_info.machine_model_version.empty() || !machine_model_version.Contains(gpu_info.machine_model_version))) {
        return false;
    }
    return true;
}
...
bool GpuControlList::More::Contains(const GPUInfo& gpu_info) const
{
    ...
    bool is_angle_gl = ProcessANGLEGLRenderer(gpu_info.gl_renderer, nullptr, nullptr, &gl_version_string);
    if (GLVersionInfoMismatch(is_angle_gl ? gl_version_string : gpu_info.gl_version)) {
        return false;
    }

    if (gl_reset_notification_strategy != 0 && gl_reset_notification_strategy != gpu_info.gl_reset_notification_strategy) {
        return false;
    }
    ...
    if (direct_rendering_version.IsSpecified() && !direct_rendering_version.Contains(gpu_info.direct_rendering_version)) {
        return false;
    }
    if (in_process_gpu && !gpu_info.in_process_gpu) {
        return false;
    }
    if (pixel_shader_version.IsSpecified() && !pixel_shader_version.Contains(gpu_info.pixel_shader_version)) {
        return false;
    }
    ...
    if ((subpixel_font_rendering == kUnsupported && gpu_info.subpixel_font_rendering)
        || (subpixel_font_rendering == kSupported && !gpu_info.subpixel_font_rendering)) {
        return false;
    }
    return true;
}

bool GpuControlList::Conditions::Contains(OsType target_os_type, const std::string& target_os_version, const GPUInfo& gpu_info) const
{
    ...
    if (vendor_id != 0 || intel_gpu_series_list_size > 0 || intel_gpu_generation.IsSpecified()) {
        bool found = false;
        if (intel_gpu_series_list_size > 0) {
            for (size_t ii = 0; !found && ii < candidates.size(); ++ii) {
                IntelGpuSeriesType candidate_series = GetIntelGpuSeriesType(candidates[ii].vendor_id, candidates[ii].device_id);
            ...
            for (auto& candidate : candidates) {
                std::string candidate_generation = GetIntelGpuGeneration(candidate.vendor_id, candidate.device_id);
            ...
            if (device_size == 0) {
                ...
                for (size_t ii = 0; !found && ii < device_size; ++ii) {
                    ...
                    for (auto& candidate : candidates) {
                        if (vendor_id != candidate.vendor_id || device_id != candidate.device_id)
                            continue;
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_CHROMEOS)
    ...
    if (driver_info) {
        ...
        auto behind_last = std::remove_if(
            candidates.begin(), candidates.end(), [vendor_id = vendor_id](const GPUInfo::GPUDevice& gpu) { return (vendor_id && vendor_id != gpu.vendor_id); });
...
bool GpuControlList::Entry::Contains(OsType target_os_type, const std::string& target_os_version, const GPUInfo& gpu_info) const
{
    static crash_reporter::CrashKeyString<8> crash_key("GpuControlList::Entry::id");
    crash_reporter::ScopedCrashKeyString scoped_crash_key(&crash_key, base::StringPrintf("%d", id));
    if (!conditions.Contains(target_os_type, target_os_version, gpu_info)) {
        return false;
    }
    for (size_t ii = 0; ii < exception_size; ++ii) {
        if (exceptions[ii].Contains(target_os_type, target_os_version, gpu_info) && !exceptions[ii].NeedsMoreInfo(gpu_info)) {
            return false;
        }
    }
    return true;
...
bool GpuControlList::Entry::AppliesToTestGroup(uint32_t target_test_group) const
{
...
bool GpuControlList::Conditions::NeedsMoreInfo(const GPUInfo& gpu_info) const
{
    ...
    if (driver_info) {
        ...
        if (driver_info->driver_version.IsSpecified() && active_gpu.driver_version.empty()) {
            return true;
        }
    }
    if (((more && more->gl_version.IsSpecified()) || (gl_strings && gl_strings->gl_version)) && gpu_info.gl_version.empty()) {
        return true;
    }
    ...
    if (gl_strings && gl_strings->gl_renderer && gpu_info.gl_renderer.empty())
        return true;
    if (more && more->pixel_shader_version.IsSpecified() && gpu_info.pixel_shader_version.empty()) {
        return true;
    }
    return false;
}

bool GpuControlList::Entry::NeedsMoreInfo(const GPUInfo& gpu_info, bool consider_exceptions) const
{
...
base::Value::List GpuControlList::Entry::GetFeatureNames(const FeatureMap& feature_map) const
{
    ...
    for (size_t ii = 0; ii < disabled_extension_size; ++ii) {
        std::string name = base::StringPrintf("disable(%s)", disabled_extensions[ii]);
...
std::set<int32_t> GpuControlList::MakeDecision(GpuControlList::OsType os, const std::string& os_version, const GPUInfo& gpu_info)
{
    return MakeDecision(os, os_version, gpu_info, 0);
}

std::set<int32_t> GpuControlList::MakeDecision(GpuControlList::OsType os, const std::string& os_version, const GPUInfo& gpu_info, uint32_t target_test_group)
{
...
std::vector<uint32_t> GpuControlList::GetEntryIDsFromIndices(const std::vector<uint32_t>& entry_indices) const
{
...
std::vector<std::string> GpuControlList::GetDisabledExtensions()
{
    ...
    return std::vector<std::string>(disabled_extensions.begin(), disabled_extensions.end());
}
...
std::vector<std::string> GpuControlList::GetDisabledWebGLExtensions()
{
    ...
    return std::vector<std::string>(disabled_webgl_extensions.begin(), disabled_webgl_extensions.end());
}

void GpuControlList::GetReasons(base::Value::List& problem_list, const std::string& tag, const std::vector<uint32_t>& entries) const
{
    for (auto index : entries) {
        ...
        for (size_t jj = 0; jj < entry.cr_bug_size; ++jj)
            cr_bugs.Append(base::Int64ToValue(static_cast<int64_t>(entry.cr_bugs[jj])));
...
void GpuControlList::AddSupportedFeature(const std::string& feature_name, int feature_id)
{
    feature_map_[feature_id] = feature_name;
}

// static
bool GpuControlList::AreEntryIndicesValid(const std::vector<uint32_t>& entry_indices, size_t total_entries)
{
    for (auto index : entry_indices) {
