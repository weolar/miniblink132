...
{
    EXPECT_EQ(left.disable_accelerated_video_decode, right.disable_accelerated_video_decode);
    EXPECT_EQ(left.disable_accelerated_video_encode, right.disable_accelerated_video_encode);
    ...
    EXPECT_EQ(left.enable_zero_copy_dxgi_video, right.enable_zero_copy_dxgi_video);
    EXPECT_EQ(left.enable_nv12_dxgi_video, right.enable_nv12_dxgi_video);
    EXPECT_EQ(left.enable_media_foundation_vea_on_windows7, right.enable_media_foundation_vea_on_windows7);
    EXPECT_EQ(left.disable_software_rasterizer, right.disable_software_rasterizer);
    EXPECT_EQ(left.log_gpu_control_list_decisions, right.log_gpu_control_list_decisions);
    EXPECT_EQ(left.compile_shader_always_succeeds, right.compile_shader_always_succeeds);
    ...
    EXPECT_EQ(left.disable_shader_name_hashing, right.disable_shader_name_hashing);
    ...
    EXPECT_EQ(left.enable_gpu_service_logging_gpu, right.enable_gpu_service_logging_gpu);
    EXPECT_EQ(left.enable_gpu_driver_debug_logging, right.enable_gpu_driver_debug_logging);
    ...
    EXPECT_EQ(left.force_gpu_mem_available_bytes, right.force_gpu_mem_available_bytes);
    EXPECT_EQ(left.force_gpu_mem_discardable_limit_bytes, right.force_gpu_mem_discardable_limit_bytes);
    EXPECT_EQ(left.gpu_program_cache_size, right.gpu_program_cache_size);
    EXPECT_EQ(left.disable_gpu_shader_disk_cache, right.disable_gpu_shader_disk_cache);
    EXPECT_EQ(left.enable_threaded_texture_mailboxes, right.enable_threaded_texture_mailboxes);
    ...
    EXPECT_EQ(left.use_passthrough_cmd_decoder, right.use_passthrough_cmd_decoder);
    EXPECT_EQ(left.disable_biplanar_gpu_memory_buffers_for_video_frames, right.disable_biplanar_gpu_memory_buffers_for_video_frames);
    EXPECT_EQ(left.texture_target_exception_list, right.texture_target_exception_list);
    EXPECT_EQ(left.ignore_gpu_blocklist, right.ignore_gpu_blocklist);
    EXPECT_EQ(left.watchdog_starts_backgrounded, right.watchdog_starts_backgrounded);
    EXPECT_EQ(left.gr_context_type, right.gr_context_type);
    EXPECT_EQ(left.use_vulkan, right.use_vulkan);
    EXPECT_EQ(left.enable_vulkan_protected_memory, right.enable_vulkan_protected_memory);
    EXPECT_EQ(left.vulkan_heap_memory_limit, right.vulkan_heap_memory_limit);
    EXPECT_EQ(left.vulkan_sync_cpu_memory_limit, right.vulkan_sync_cpu_memory_limit);
    EXPECT_EQ(left.enable_gpu_benchmarking_extension, right.enable_gpu_benchmarking_extension);
    ...
    EXPECT_EQ(left.disabled_dawn_features_list, right.disabled_dawn_features_list);
    EXPECT_EQ(left.enable_gpu_blocked_time_metric, right.enable_gpu_blocked_time_metric);
    EXPECT_EQ(left.enable_perf_data_collection, right.enable_perf_data_collection);
#if defined(USE_OZONE)
    EXPECT_EQ(left.message_pump_type, right.message_pump_type);
#endif
    EXPECT_EQ(left.enable_native_gpu_memory_buffers, right.enable_native_gpu_memory_buffers);
#if BUILDFLAG(IS_CHROMEOS)
    EXPECT_EQ(left.enable_chromeos_direct_video_decoder, right.enable_chromeos_direct_video_decoder);
#endif
}
...
#define GPU_PREFERENCES_FIELD_ENUM(name, value, mojom_value)                                                                                                   \
    ...
    EXPECT_EQ(static_cast<uint32_t>(input_prefs.name), static_cast<uint32_t>(prefs_mojom.name));
        ...
        GPU_PREFERENCES_FIELD(gpu_program_cache_size, kDefaultMaxProgramCacheMemoryBytes - 1)
        ...
        GPU_PREFERENCES_FIELD(disable_biplanar_gpu_memory_buffers_for_video_frames, true)
        GPU_PREFERENCES_FIELD(ignore_gpu_blocklist, true)
        GPU_PREFERENCES_FIELD(watchdog_starts_backgrounded, true)
        GPU_PREFERENCES_FIELD_ENUM(gr_context_type, GrContextType::kVulkan, mojom::GrContextType::kVulkan)
        GPU_PREFERENCES_FIELD_ENUM(use_vulkan, VulkanImplementationName::kNative, mojom::VulkanImplementationName::kNative)
        ...
        GPU_PREFERENCES_FIELD_ENUM(enable_dawn_backend_validation, DawnBackendValidationLevel::kPartial, mojom::DawnBackendValidationLevel::kPartial)
        GPU_PREFERENCES_FIELD(enable_gpu_blocked_time_metric, true)
        GPU_PREFERENCES_FIELD(enable_perf_data_collection, true)
#if defined(USE_OZONE)
        GPU_PREFERENCES_FIELD_ENUM(message_pump_type, base::MessagePumpType::UI, base::MessagePumpType::UI)
#endif
        ...
        input_prefs.texture_target_exception_list.emplace_back(gfx::BufferUsage::SCANOUT, gfx::BufferFormat::RGBA_8888);
        input_prefs.texture_target_exception_list.emplace_back(gfx::BufferUsage::GPU_READ, gfx::BufferFormat::BGRA_8888);
...
TEST(GpuPreferencesTest, DISABLED_DecodePreferences)
{
    ...
    if (!command_line->HasSwitch(switches::kGpuPreferences)) {
        LOG(ERROR) << "Please specify the preferences to decode via " << switches::kGpuPreferences;
        return;
    }

    const auto preferences = command_line->GetSwitchValueASCII(switches::kGpuPreferences);
...
#define PRINT_INT(key) printf("  %s: %d\n", #key, static_cast<uint32_t>(gpu_preferences.key))
    ...
    for (size_t i = 0; i < gpu_preferences.texture_target_exception_list.size(); ++i) {
