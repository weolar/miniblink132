...
TEST_F(GpuDriverBugListTest, CurrentListForARM)
{
    ...
    std::set<int> bugs = list->MakeDecision(GpuControlList::kOsAndroid, "4.1", gpu_info);
...
TEST_F(GpuDriverBugListTest, AppendSingleWorkaround)
{
    ...
    command_line.AppendSwitch(GpuDriverBugWorkaroundTypeToString(DISABLE_CHROMIUM_FRAMEBUFFER_MULTISAMPLE));
...
{
    base::CommandLine command_line(0, nullptr);
    command_line.AppendSwitch(GpuDriverBugWorkaroundTypeToString(FORCE_HIGH_PERFORMANCE_GPU));
    std::set<int> workarounds;
    workarounds.insert(EXIT_ON_CONTEXT_LOST);
