    ...
    void RunFeatureTest(GpuFeatureType feature_type)
    {
        ...
        const GpuControlList::Entry kTestEntries[1] = { {
            ...
            nullptr, // CrBugs
            {
                ...
                0, // gpu_series size
                nullptr, // gpu_series
                { GpuControlList::kUnknown, GpuControlList::kVersionStyleNumerical, GpuControlList::kVersionSchemaCommon, nullptr,
        ...
        std::set<int> type = blocklist->MakeDecision(GpuBlocklist::kOsMacosx, "10.12.3", gpu_info());
...
#define GPU_BLOCKLIST_FEATURE_TEST(test_name, feature_type)                                                                                                    \
    TEST_F(GpuBlocklistTest, test_name)                                                                                                                        \
    {                                                                                                                                                          \
        RunFeatureTest(feature_type);                                                                                                                          \
    }

GPU_BLOCKLIST_FEATURE_TEST(Accelerated2DCanvas, GPU_FEATURE_TYPE_ACCELERATED_2D_CANVAS)

GPU_BLOCKLIST_FEATURE_TEST(AcceleratedWebGL, GPU_FEATURE_TYPE_ACCELERATED_WEBGL)

GPU_BLOCKLIST_FEATURE_TEST(AcceleratedVideoDecode, GPU_FEATURE_TYPE_ACCELERATED_VIDEO_DECODE)

GPU_BLOCKLIST_FEATURE_TEST(AcceleratedVideoEncode, GPU_FEATURE_TYPE_ACCELERATED_VIDEO_ENCODE)
...
GPU_BLOCKLIST_FEATURE_TEST(AcceleratedWebGPU, GPU_FEATURE_TYPE_ACCELERATED_WEBGPU)
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (const auto* gl_strings = entry.conditions.gl_strings) {
            if (gl_strings->gl_vendor) {
                EXPECT_TRUE(RE2(gl_strings->gl_vendor).ok()) << "gl_vendor=" << gl_strings->gl_vendor;
            }
            if (gl_strings->gl_renderer) {
                EXPECT_TRUE(RE2(gl_strings->gl_renderer).ok()) << "gl_renderer=" << gl_strings->gl_renderer;
            }
            if (gl_strings->gl_extensions) {
                EXPECT_TRUE(RE2(gl_strings->gl_extensions).ok()) << "gl_extensions=" << gl_strings->gl_extensions;
            }
            if (gl_strings->gl_version) {
                EXPECT_TRUE(RE2(gl_strings->gl_version).ok()) << "gl_version=" << gl_strings->gl_version;
            }
        }
...
{
    TestBlockList(kSoftwareRenderingListEntries, kSoftwareRenderingListEntryCount);
