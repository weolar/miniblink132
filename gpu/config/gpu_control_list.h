...
public:
    ...
    enum OsType { kOsLinux, kOsMacosx, kOsWin, kOsChromeOS, kOsAndroid, kOsFuchsia, kOsAny };
    ...
    enum VersionStyle { kVersionStyleNumerical, kVersionStyleLexical, kVersionStyleUnknown };
    ...
    struct GPU_EXPORT Version {
        ...
        static int Compare(const std::vector<std::string>& version, const std::vector<std::string>& version_ref, VersionStyle version_style);
    };
    ...
    struct GPU_EXPORT Conditions {
        ...
        bool Contains(OsType os_type, const std::string& os_version, const GPUInfo& gpu_info) const;
    ...
    struct GPU_EXPORT Entry {
        ...
        void LogControlListMatch(const std::string& control_list_logging_name) const;
    };
    ...
    // current OS version.
    std::set<int32_t> MakeDecision(OsType os, const std::string& os_version, const GPUInfo& gpu_info);
    ...
    std::set<int32_t> MakeDecision(OsType os, const std::string& os_version, const GPUInfo& gpu_info, uint32_t target_test_group);
    ...
    std::vector<uint32_t> GetEntryIDsFromIndices(const std::vector<uint32_t>& entry_indices) const;
    ...
    void GetReasons(base::Value::List& problem_list, const std::string& tag, const std::vector<uint32_t>& entries) const;
    ...
    bool needs_more_info() const
    {
        return needs_more_info_;
    }
...
protected:
    ...
    static bool AreEntryIndicesValid(const std::vector<uint32_t>& entry_indices, size_t total_entries);
...
struct GPU_EXPORT GpuControlListData {
    ...
    GpuControlListData(size_t a_entry_count, const GpuControlList::Entry* a_entries)
        : entry_count(a_entry_count)
        , entries(a_entries)
    {
    }
};
