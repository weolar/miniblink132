...
public:
    ...
    static std::unique_ptr<GpuDriverBugList> Create(const GpuControlListData& data);

    // Append |workarounds| with these passed in through the
    // |command_line|.
    static void AppendWorkaroundsFromCommandLine(std::set<int>* workarounds, const base::CommandLine& command_line);
