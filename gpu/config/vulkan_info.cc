...
VulkanPhysicalDeviceInfo::VulkanPhysicalDeviceInfo(const VulkanPhysicalDeviceInfo& other) = default;
VulkanPhysicalDeviceInfo::~VulkanPhysicalDeviceInfo() = default;
VulkanPhysicalDeviceInfo& VulkanPhysicalDeviceInfo::operator=(const VulkanPhysicalDeviceInfo& info) = default;
...
void VulkanInfo::SetEnabledInstanceExtensions(const std::vector<const char*>& extensions)
{
    ...
    for (const auto* const extension : extensions) {
        ...
        if (!found) {
            LOG(ERROR) << "The enabled extension '" << extension << "' is not in instance_extensions!";
        }
    }
}

void VulkanInfo::SetEnabledInstanceExtensions(const std::vector<base::StringPiece>& extensions)
{
    enabled_instance_extensions.clear();
