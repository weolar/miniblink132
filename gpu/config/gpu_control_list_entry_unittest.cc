...
constexpr GpuControlList::OsType kAllOsType[] = { kOsMacosx, kOsWin, kOsLinux, kOsChromeOS, kOsAndroid, kOsFuchsia };
...
TEST_F(GpuControlListEntryTest, VendorOnLinuxEntry)
{
    ...
    const GpuControlList::OsType os_types[] = { kOsMacosx, kOsWin, kOsChromeOS, kOsAndroid, kOsFuchsia };
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_AllExceptNVidiaOnLinuxEntry);
    EXPECT_EQ(kOsLinux, entry.conditions.os_type);
    const GpuControlList::OsType os_type[] = { kOsMacosx, kOsWin, kOsLinux, kOsChromeOS, kOsAndroid };
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_AllExceptIntelOnLinuxEntry);
    EXPECT_EQ(kOsLinux, entry.conditions.os_type);
    const GpuControlList::OsType os_types[] = { kOsMacosx, kOsWin, kOsChromeOS, kOsAndroid, kOsFuchsia };
    for (auto os_type : os_types)
        EXPECT_FALSE(entry.Contains(os_type, "10.6", gpu_info()));
...
TEST_F(GpuControlListEntryTest, ChromeOSEntry)
{
    ...
    const GpuControlList::OsType os_types[] = { kOsMacosx, kOsWin, kOsLinux, kOsAndroid, kOsFuchsia };
    for (auto os_type : os_types)
        EXPECT_FALSE(entry.Contains(os_type, "10.6", gpu_info()));
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_GlRendererCaseInsensitive);
    GPUInfo gpu_info;
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_LexicalDriverVersionEntry);
    EXPECT_EQ(kOsLinux, entry.conditions.os_type);
    GPUInfo gpu_info;
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_NeedsMoreInfoForExceptionsEntry);
    GPUInfo gpu_info;
    gpu_info.gpu.vendor_id = 0x8086;
    EXPECT_TRUE(entry.NeedsMoreInfo(gpu_info, true));
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_NeedsMoreInfoForGlVersionEntry);
    GPUInfo gpu_info;
    EXPECT_TRUE(entry.NeedsMoreInfo(gpu_info, true));
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_FeatureTypeAllEntryWithExceptions);
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_MachineModelNameException);
    EXPECT_EQ(kOsAny, entry.conditions.os_type);
    GPUInfo gpu_info;
    gpu_info.machine_model_name = "Nexus 4";
    EXPECT_FALSE(entry.Contains(kOsAndroid, "4.1", gpu_info));
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_MachineModelVersionException);
    EXPECT_EQ(kOsMacosx, entry.conditions.os_type);
    GPUInfo gpu_info;
    gpu_info.machine_model_name = "MacBookPro";
...
{
    const Entry& entry_intel = GetEntry(kGpuControlListEntryDualGPUTest_CategoryAny_Intel);
    EntryShouldApply(entry_intel);
    const Entry& entry_nvidia = GetEntry(kGpuControlListEntryDualGPUTest_CategoryAny_NVidia);
    EntryShouldApply(entry_nvidia);
}

TEST_F(GpuControlListEntryDualGPUTest, CategoryPrimarySecondary)
{
    const Entry& entry_secondary = GetEntry(kGpuControlListEntryDualGPUTest_CategorySecondary);
    EntryShouldApply(entry_secondary);
    const Entry& entry_primary = GetEntry(kGpuControlListEntryDualGPUTest_CategoryPrimary);
    EntryShouldNotApply(entry_primary);
    const Entry& entry_default = GetEntry(kGpuControlListEntryDualGPUTest_CategoryDefault);
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryDualGPUTest_ActiveSecondaryGPU);
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryDualGPUTest_VendorOnlyActiveSecondaryGPU);
    // By default, secondary GPU is active.
    EntryShouldApply(entry);
    ActivatePrimaryGPU();
    EntryShouldNotApply(entry);
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryDualGPUTest_ActivePrimaryGPU);
    // By default, secondary GPU is active.
    EntryShouldNotApply(entry);
    ActivatePrimaryGPU();
    EntryShouldApply(entry);
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryDualGPUTest_VendorOnlyActivePrimaryGPU);
    // By default, secondary GPU is active.
    EntryShouldNotApply(entry);
    ActivatePrimaryGPU();
    EntryShouldApply(entry);
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_ExceptionWithoutVendorId);
...
TEST_F(GpuControlListEntryTest, MultiGpuStyleAMDSwitchable)
{
    ...
    { // amd_switchable_discrete entry
        const Entry& entry = GetEntry(kGpuControlListEntryTest_MultiGpuStyleAMDSwitchableDiscrete);
    ...
    { // amd_switchable_integrated entry
        const Entry& entry = GetEntry(kGpuControlListEntryTest_MultiGpuStyleAMDSwitchableIntegrated);
        // Discrete GPU is active
        gpu_info.gpu.active = true;
        gpu_info.secondary_gpus[0].active = false;
        EXPECT_FALSE(entry.Contains(kOsWin, "6.0", gpu_info));
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_SubpixelFontRenderingDontCare);
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_IntelDriverVendorEntry);
    GPUInfo gpu_info;
    gpu_info.gpu.vendor_id = 0x8086;
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_IntelDriverVersionEntry);
    GPUInfo gpu_info;
    gpu_info.gpu.vendor_id = 0x8086;
...
{
    const Entry& entry = GetEntry(kGpuControlListEntryTest_DeviceRevisionUnspecifiedEntry);
    GPUInfo gpu_info;
    gpu_info.gpu.vendor_id = 0x1002;
    gpu_info.gpu.device_id = 0x15DD;
    gpu_info.gpu.revision = 0x86;
