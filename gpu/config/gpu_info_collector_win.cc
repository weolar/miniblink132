...
#include <SetupAPI.h> // SUPPORT_XP_CODE
#include <cfgmgr32.h>
#include "base/strings/string_number_conversions.h"
#include "base/strings/utf_string_conversions.h"
#include "gpu/config/nvml_info.h"
#include "patch_code/dxgi_xp.h"
...
inline D3D12FeatureLevel ConvertToHistogramFeatureLevel(uint32_t d3d_feature_level)
{
...
bool GetAMDSwitchableInfo(bool* is_switchable, uint32_t* active_vendor_id, uint32_t* active_device_id);
#else
bool GetAMDSwitchableInfo(bool* is_switchable, uint32_t* active_vendor_id, uint32_t* active_device_id)
{
    return false;
...
void CollectHardwareOverlayInfo(OverlayInfo* overlay_info)
{
    if (gl::GetGLImplementation() == gl::kGLImplementationEGLANGLE) {
        ...
        overlay_info->nv12_overlay_support
            = FlagsToOverlaySupport(overlay_info->supports_overlays, gl::GetDirectCompositionOverlaySupportFlags(DXGI_FORMAT_NV12));
        overlay_info->yuy2_overlay_support
            = FlagsToOverlaySupport(overlay_info->supports_overlays, gl::GetDirectCompositionOverlaySupportFlags(DXGI_FORMAT_YUY2));
        overlay_info->bgra8_overlay_support
            = FlagsToOverlaySupport(overlay_info->supports_overlays, gl::GetDirectCompositionOverlaySupportFlags(DXGI_FORMAT_B8G8R8A8_UNORM));
        overlay_info->rgb10a2_overlay_support
            = FlagsToOverlaySupport(overlay_info->supports_overlays, gl::GetDirectCompositionOverlaySupportFlags(DXGI_FORMAT_R10G10B10A2_UNORM));
    }
}
...
bool CollectDriverInfoD3D(GPUInfo* gpu_info)
{
    ...
    HRESULT hr = ::CreateDXGIFactory1Xp(IID_PPV_ARGS(&dxgi_factory));
    if (FAILED(hr))
        return false;
    ...
    for (i = 0; SUCCEEDED(dxgi_factory->EnumAdapters(i, &dxgi_adapter)); i++) {
        ...
        device.luid = CHROME_LUID { desc.AdapterLuid.LowPart, desc.AdapterLuid.HighPart };

        LARGE_INTEGER umd_version;
        hr = dxgi_adapter->CheckInterfaceSupport(__uuidof(IDXGIDevice), &umd_version);
        if (SUCCEEDED(hr)) {
            device.driver_version = base::StringPrintf(
                "%d.%d.%d.%d", HIWORD(umd_version.HighPart), LOWORD(umd_version.HighPart), HIWORD(umd_version.LowPart), LOWORD(umd_version.LowPart));
        } else {
            DLOG(ERROR) << "Unable to retrieve the umd version of adapter: " << desc.Description << " HR: " << std::hex << hr;
        }
    ...
    if (gpu_info->GpuCount() > 1 && SUCCEEDED(dxgi_factory.As(&dxgi_factory6))) {
        if (SUCCEEDED(dxgi_factory6->EnumAdapterByGpuPreference(0, DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE, IID_PPV_ARGS(&dxgi_adapter)))) {
            DXGI_ADAPTER_DESC desc;
            CHECK_EQ(S_OK, dxgi_adapter->GetDesc(&desc));
            GPUInfo::GPUDevice* device = gpu_info->FindGpuByLuid(desc.AdapterLuid.LowPart, desc.AdapterLuid.HighPart);
            DCHECK(device);
            device->gpu_preference = gl::GpuPreference::kHighPerformance;
        }
        if (SUCCEEDED(dxgi_factory6->EnumAdapterByGpuPreference(0, DXGI_GPU_PREFERENCE_MINIMUM_POWER, IID_PPV_ARGS(&dxgi_adapter)))) {
            DXGI_ADAPTER_DESC desc;
            CHECK_EQ(S_OK, dxgi_adapter->GetDesc(&desc));
            GPUInfo::GPUDevice* device = gpu_info->FindGpuByLuid(desc.AdapterLuid.LowPart, desc.AdapterLuid.HighPart);
            DCHECK(device);
...
bool CanCreateD3D12Device(IDXGIAdapter* dxgi_adapter)
{
    ...
    LARGE_INTEGER umd_version;
    HRESULT hr = dxgi_adapter->CheckInterfaceSupport(__uuidof(IDXGIDevice), &umd_version);
    if (FAILED(hr)) {
        return false;
    }
    ...
    if (HIWORD(umd_version.LowPart) == 100 && LOWORD(umd_version.LowPart) >= 9416 && LOWORD(umd_version.LowPart) < 9664) {
...
void GetGpuSupportedD3D12Version(uint32_t& d3d12_feature_level, uint32_t& highest_shader_model_version)
{
    ...
    base::ScopedNativeLibrary d3d12_library(base::FilePath(FILE_PATH_LITERAL("d3d12.dll")));
    ...
    PFN_D3D12_CREATE_DEVICE D3D12CreateDevice = reinterpret_cast<PFN_D3D12_CREATE_DEVICE>(d3d12_library.GetFunctionPointer("D3D12CreateDevice"));
    ...
    if (D3D12CreateDevice) {
        ...
        for (auto level : feature_levels) {
            if (SUCCEEDED(D3D12CreateDevice(dxgi_adapter.Get(), level, _uuidof(ID3D12Device), &d3d12_device))) {
    ...
    if (d3d12_device) {
        ...
        for (auto model : shader_models) {
            ...
            if (SUCCEEDED(d3d12_device->CheckFeatureSupport(D3D12_FEATURE_SHADER_MODEL, &shader_model_data, sizeof(shader_model_data)))) {
...
bool BadAMDVulkanDriverVersion()
{
    ...
    std::unique_ptr<FileVersionInfoWin> file_version_info = FileVersionInfoWin::CreateFileVersionInfoWin(kAmdDriver64);
    if (!file_version_info) {
        file_version_info = FileVersionInfoWin::CreateFileVersionInfoWin(kAmdDriver32);
        if (!file_version_info)
            return false;
    }
...
bool InitVulkan(
    base::NativeLibrary* vulkan_library, PFN_vkGetInstanceProcAddr* vkGetInstanceProcAddr, PFN_vkCreateInstance* vkCreateInstance, uint32_t* vulkan_version)
{
    *vulkan_version = 0;

    *vulkan_library = base::LoadNativeLibrary(base::FilePath(L"vulkan-1.dll"), nullptr);

    if (!(*vulkan_library)) {
        return false;
    }

    *vkGetInstanceProcAddr = reinterpret_cast<PFN_vkGetInstanceProcAddr>(base::GetFunctionPointerFromNativeLibrary(*vulkan_library, "vkGetInstanceProcAddr"));
...
bool InitVulkanInstanceProc(const VkInstance& vk_instance, const PFN_vkGetInstanceProcAddr& vkGetInstanceProcAddr,
    PFN_vkEnumeratePhysicalDevices* vkEnumeratePhysicalDevices, PFN_vkEnumerateDeviceExtensionProperties* vkEnumerateDeviceExtensionProperties)
{
...
uint32_t GetGpuSupportedVulkanVersion(const gpu::GPUInfo::GPUDevice& gpu_device)
{
    ...
    if (!InitVulkan(&vulkan_library, &vkGetInstanceProcAddr, &vkCreateInstance, &vulkan_version)) {
        return 0;
    }
    ...
    const std::vector<const char*> enabled_instance_extensions = { "VK_KHR_surface", "VK_KHR_win32_surface" };
    ...
    for (int minor_version = highest_minor_version; minor_version >= 1; --minor_version) {
        ...
        if (result == VK_SUCCESS && vk_instance
            && InitVulkanInstanceProc(vk_instance, vkGetInstanceProcAddr, &vkEnumeratePhysicalDevices, &vkEnumerateDeviceExtensionProperties)) {
...
void RecordGpuSupportedDx12VersionHistograms(uint32_t d3d12_feature_level, uint32_t highest_shader_model_version)
{
    bool supports_dx12 = (d3d12_feature_level >= D3D_FEATURE_LEVEL_12_0) ? true : false;
    UMA_HISTOGRAM_BOOLEAN("GPU.SupportsDX12", supports_dx12);
    UMA_HISTOGRAM_ENUMERATION("GPU.D3D12FeatureLevel", ConvertToHistogramFeatureLevel(d3d12_feature_level));

    UMA_HISTOGRAM_ENUMERATION("GPU.D3D12HighestShaderModel2", ConvertToHistogramShaderVersion(highest_shader_model_version));
}

bool CollectD3D11FeatureInfo(D3D_FEATURE_LEVEL* d3d11_feature_level, bool* has_discrete_gpu)
{
    Microsoft::WRL::ComPtr<IDXGIFactory1> dxgi_factory;
    if (FAILED(::CreateDXGIFactory1(IID_PPV_ARGS(&dxgi_factory))))
        return false;

    base::ScopedNativeLibrary d3d11_library(base::FilePath(FILE_PATH_LITERAL("d3d11.dll")));
    if (!d3d11_library.is_valid())
        return false;
    PFN_D3D11_CREATE_DEVICE D3D11CreateDevice = reinterpret_cast<PFN_D3D11_CREATE_DEVICE>(d3d11_library.GetFunctionPointer("D3D11CreateDevice"));
    ...
    const D3D_FEATURE_LEVEL kFeatureLevels[] = { D3D_FEATURE_LEVEL_12_1, D3D_FEATURE_LEVEL_12_0, D3D_FEATURE_LEVEL_11_1, D3D_FEATURE_LEVEL_11_0,
    ...
    Microsoft::WRL::ComPtr<IDXGIAdapter> dxgi_adapter;
    for (UINT ii = 0; SUCCEEDED(dxgi_factory->EnumAdapters(ii, &dxgi_adapter)); ++ii) {
        DXGI_ADAPTER_DESC desc;
        CHECK_EQ(S_OK, dxgi_adapter->GetDesc(&desc));
        ...
        if (FAILED(D3D11CreateDevice(dxgi_adapter.Get(), D3D_DRIVER_TYPE_UNKNOWN,
                /*Software=*/0,
                /*Flags=*/0, kFeatureLevels, _countof(kFeatureLevels), D3D11_SDK_VERSION, &d3d11_device, &returned_feature_level,
        ...
        if (FAILED(d3d11_device_3->CheckFeatureSupport(D3D11_FEATURE_D3D11_OPTIONS2, &data, sizeof(data)))) {
            continue;
        }
...
static void DeviceIDToVendorAndDevice(const std::wstring& id, uint32_t* vendor_id, uint32_t* device_id)
{
    *vendor_id = 0;
    *device_id = 0;
    if (id.length() < 21)
        return;
    std::u16string vendor_id_string = (const char16_t*)id.substr(8, 4).c_str();
    std::u16string device_id_string = (const char16_t*)id.substr(17, 4).c_str();
    int vendor = 0;
    int device = 0;
...
static std::string ParseNVIDIARegistryDriverVersion(std::string registry_version)
{
    ...
    if (!RE2::FullMatch(registry_version, "\\d+\\.\\d+\\.(\\d+)\\.(\\d+)", &second_to_last_digits, &last_digits)) {
...
bool CollectDriverInfoD3DXp(const std::wstring& device_id, GPUInfo* gpu_info)
{
    TRACE_EVENT0("gpu", "CollectDriverInfoD3D");
    ...
    const HDEVINFO device_info = ::SetupDiGetClassDevs(&display_class, nullptr, nullptr, DIGCF_PRESENT);
    ...
    while (SetupDiEnumDeviceInfo(device_info, index++, &device_info_data)) {
        WCHAR value[255];
        if (SetupDiGetDeviceRegistryPropertyW(device_info, &device_info_data, SPDRP_DRIVER, nullptr, reinterpret_cast<PBYTE>(value), sizeof(value), nullptr)) {
            HKEY key;
            std::wstring driver_key = L"System\\CurrentControlSet\\Control\\Class\\";
            driver_key += value;
            LONG result = RegOpenKeyExW(HKEY_LOCAL_MACHINE, driver_key.c_str(), 0, KEY_QUERY_VALUE, &key);
            if (result == ERROR_SUCCESS) {
                GPUInfo::GPUDevice device;
                DWORD dwcb_data = sizeof(value);
                result = RegQueryValueExW(key, L"DriverVersion", nullptr, nullptr, reinterpret_cast<LPBYTE>(value), &dwcb_data);
                if (result == ERROR_SUCCESS)
                    device.driver_version = base::UTF16ToASCII(std::u16string((const char16_t*)value));

                dwcb_data = sizeof(value);
                result = RegQueryValueExW(key, L"DriverDate", nullptr, nullptr, reinterpret_cast<LPBYTE>(value), &dwcb_data);
                //         if (result == ERROR_SUCCESS)
                //           device.driver_date = base::UTF16ToASCII(std::wstring(value));

                dwcb_data = sizeof(value);
                result = RegQueryValueExW(key, L"ProviderName", nullptr, nullptr, reinterpret_cast<LPBYTE>(value), &dwcb_data);
                if (result == ERROR_SUCCESS)
                    device.driver_vendor = base::UTF16ToASCII(std::u16string((const char16_t*)value));

                wchar_t new_device_id[MAX_DEVICE_ID_LEN];
                CONFIGRET status = CM_Get_Device_ID(device_info_data.DevInst, new_device_id, MAX_DEVICE_ID_LEN, 0);
                ...
                if (status == CR_SUCCESS) {
                    ...
                    if (device.vendor_id == 0x10de) {
                        ...
                        bool nvml_success
                            = GetNvmlDeviceInfo(device.device_id, &nvml_driver_version, &major_cuda_compute_capability, &minor_cuda_compute_capability);
                        if (nvml_success) {
                            ...
                            device.driver_version = ParseNVIDIARegistryDriverVersion(device.driver_version);
                        }
                    }
...
bool CollectContextGraphicsInfo(GPUInfo* gpu_info)
{
    ...
    if (RE2::FullMatch(gpu_info->gl_renderer, "ANGLE \\(.*\\)") && RE2::PartialMatch(gpu_info->gl_renderer, " Direct3D(\\w+)", &direct3d_version)
        && RE2::PartialMatch(gpu_info->gl_renderer, " vs_(\\d+)_(\\d+)", &vertex_shader_major_version, &vertex_shader_minor_version)
        && RE2::PartialMatch(gpu_info->gl_renderer, " ps_(\\d+)_(\\d+)", &pixel_shader_major_version, &pixel_shader_minor_version)) {
        gpu_info->vertex_shader_version = base::StringPrintf("%d.%d", vertex_shader_major_version, vertex_shader_minor_version);
        gpu_info->pixel_shader_version = base::StringPrintf("%d.%d", pixel_shader_major_version, pixel_shader_minor_version);

        DCHECK(!gpu_info->vertex_shader_version.empty());
        // Note: do not reorder, used by UMA_HISTOGRAM below
        enum ShaderModel { SHADER_MODEL_UNKNOWN, SHADER_MODEL_2_0, SHADER_MODEL_3_0, SHADER_MODEL_4_0, SHADER_MODEL_4_1, SHADER_MODEL_5_0, NUM_SHADER_MODELS };
        ...
        UMA_HISTOGRAM_ENUMERATION("GPU.D3DShaderModel", shader_model, NUM_SHADER_MODELS);
...
bool CollectBasicGraphicsInfo(GPUInfo* gpu_info)
{
    ...
    if (id.length() <= 20) {
        ...
        if (wcscmp(dd.DeviceString, L"RDPUDD Chained DD") != 0 && wcscmp(dd.DeviceString, L"Citrix Systems Inc. Display Driver") != 0) {
    ...
    DeviceIDToVendorAndDevice(id, &gpu_info->gpu.vendor_id, &gpu_info->gpu.device_id);
    // TODO(zmo): we only need to call CollectDriverInfoD3D() if we use ANGLE.
    return CollectDriverInfoD3DXp(id, gpu_info);
#endif
}
...
bool IdentifyActiveGPUWithLuid(GPUInfo* gpu_info)
{
    ...
    if (gpu_info->gpu.luid.HighPart == luid.HighPart && gpu_info->gpu.luid.LowPart == luid.LowPart) {
        gpu_info->gpu.active = true;
        return true;
    }

    for (size_t i = 0; i < gpu_info->secondary_gpus.size(); i++) {
        if (gpu_info->secondary_gpus[i].luid.HighPart == luid.HighPart && gpu_info->secondary_gpus[i].luid.LowPart == luid.LowPart) {
