...
typedef decltype(&nvmlDeviceGetCudaComputeCapability) DEVICEGETCUDACOMPUTECAPABILITYPROC;

bool GetNvmlDeviceInfo(uint32_t pci_device_id, std::string* driver_version, int* major_cuda_compute_capability, int* minor_cuda_compute_capability)
{
    ...
    *minor_cuda_compute_capability = 0;
    ...
    std::unique_ptr<FileVersionInfoWin> file_version_info = FileVersionInfoWin::CreateFileVersionInfoWin(dll_path);
    ...
    base::Version dll_version = file_version_info->GetFileVersion();
    ...
    SHUTDOWNPROC fnNvmlShutdown = (SHUTDOWNPROC)GetProcAddress(hinstNVML, "nvmlShutdown");
    GETDRIVERVERSIONPROC fnNvmlSystemGetDriverVersion = (GETDRIVERVERSIONPROC)GetProcAddress(hinstNVML, "nvmlSystemGetDriverVersion");
    DEVICEGETCOUNTPROC fnNvmlDeviceGetCount = (DEVICEGETCOUNTPROC)GetProcAddress(hinstNVML, "nvmlDeviceGetCount");
    DEVICEGETHANDLEBYINDEXPROC fnNvmlDeviceGetHandleByIndex = (DEVICEGETHANDLEBYINDEXPROC)GetProcAddress(hinstNVML, "nvmlDeviceGetHandleByIndex");
    DEVICEGETPCIINFOPROC fnNvmlDeviceGetPciInfo = (DEVICEGETPCIINFOPROC)GetProcAddress(hinstNVML, "nvmlDeviceGetPciInfo");
    DEVICEGETCUDACOMPUTECAPABILITYPROC fnNvmlDeviceGetCudaComputeCapability
        = (DEVICEGETCUDACOMPUTECAPABILITYPROC)GetProcAddress(hinstNVML, "nvmlDeviceGetCudaComputeCapability");
    ...
    result = fnNvmlSystemGetDriverVersion(driver_version_temp, kDriverVersionCapacity);
    if (NVML_SUCCESS != result) {
        fnNvmlShutdown();
        return false;
    }
    ...
    *driver_version = driver_version_temp;
    ...
    if (fnNvmlDeviceGetCount && fnNvmlDeviceGetHandleByIndex && fnNvmlDeviceGetPciInfo && fnNvmlDeviceGetCudaComputeCapability) {
        ...
        if (NVML_SUCCESS == result) {
            for (unsigned int i = 0; i < device_count; i++) {
                ...
                result = fnNvmlDeviceGetCudaComputeCapability(device, major_cuda_compute_capability, minor_cuda_compute_capability);
                if (NVML_SUCCESS != result) {
                    *major_cuda_compute_capability = 0;
                    *minor_cuda_compute_capability = 0;
                }
