...
GPU_EXPORT bool CollectBasicGraphicsInfo(const base::CommandLine* command_line, GPUInfo* gpu_info);
...
GPU_EXPORT void GetGpuSupportedD3D12Version(uint32_t& d3d12_feature_level, uint32_t& highest_shader_model_version);
GPU_EXPORT void RecordGpuSupportedDx12VersionHistograms(uint32_t d3d12_feature_level, uint32_t highest_shader_model_version);
GPU_EXPORT uint32_t GetGpuSupportedVulkanVersion(const gpu::GPUInfo::GPUDevice& gpu_device);
...
GPU_EXPORT bool CollectD3D11FeatureInfo(D3D_FEATURE_LEVEL* d3d11_feature_level, bool* has_discrete_gpu);
...
GPU_EXPORT bool CollectGraphicsInfoGL(GPUInfo* gpu_info, gl::GLDisplay* display);
...
GPU_EXPORT bool CollectGpuExtraInfo(gfx::GpuExtraInfo* gpu_extra_info, const GpuPreferences& prefs);

// Collect Dawn Toggle name info for about:gpu
GPU_EXPORT void CollectDawnInfo(const gpu::GpuPreferences& gpu_preferences, std::vector<std::string>* dawn_info_list);
