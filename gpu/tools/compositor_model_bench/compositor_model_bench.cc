...
public:
    ...
    void Run()
    {
        ...
        single_thread_task_executor_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&Simulator::ProcessEvents, weak_factory_.GetWeakPtr()));
        run_loop_.Run();
    }
...
private:
    ...
    bool InitX11()
    {
        ...
        connection_->CreateWindow({
            ...
            .event_mask = x11::EventMask::Exposure | x11::EventMask::KeyPress | x11::EventMask::StructureNotify,
        });
        x11::SetStringProperty(window_, x11::Atom::WM_NAME, x11::Atom::STRING, "Compositor Model Bench");
    ...
    bool InitGLContext()
    {
        ...
        if (!glx_config)
            return false;
        auto* visual = glXGetVisualFromFBConfig(connection_->GetXlibDisplay(), glx_config);
        DCHECK(visual);

        gl_context_ = glXCreateContext(connection_->GetXlibDisplay(x11::XlibDisplayType::kSyncing), visual, nullptr, true /* Direct rendering */);
    ...
    bool InitializeNextTest()
    {
        ...
        current_sim_ = ConstructSimulationModel(spec.model_under_test, std::move(root), window_width_, window_height_);
        return !!current_sim_;
    }
    ...
    void UpdateCurrentTest()
    {
        ...
        base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&Simulator::UpdateLoop, weak_factory_.GetWeakPtr()));
    }
    ...
    void DumpOutput()
    {
        ...
        while (sims_completed_.size()) {
            ...
            fprintf(f,
                ...
                "\t\t\t\"frames_drawn\":%d\n"
                "\t\t},\n",
                i.simulation_name.c_str(), ModelToString(i.model_under_test), i.frames_rendered);
            sims_completed_.pop();
        }
