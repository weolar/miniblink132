...
bool VerifyDictionaryEntry(const base::Value& node, const std::string& key, Value::Type type)
{
...
bool VerifyListEntry(const base::Value& list, int index, Value::Type type, const char* listName = nullptr)
{
    ...
    if (list.GetListDeprecated()[index].type() != type) {
        LOG(ERROR) << (listName ? listName : "List") << "element " << index << " did not have the expected type (expected " << base::Value::GetTypeName(type)
                   << ")\n";
        return false;
    }
...
{
    if (!VerifyDictionaryEntry(node, "layerID", Value::Type::INTEGER) || !VerifyDictionaryEntry(node, "width", Value::Type::INTEGER)
        || !VerifyDictionaryEntry(node, "height", Value::Type::INTEGER) || !VerifyDictionaryEntry(node, "drawsContent", Value::Type::BOOLEAN)
        || !VerifyDictionaryEntry(node, "targetSurfaceID", Value::Type::INTEGER) || !VerifyDictionaryEntry(node, "transform", Value::Type::LIST)) {
        return false;
    }
    ...
    if (!VerifyListEntry(*dim, 0, Value::Type::INTEGER, "Tile dimension") || !VerifyListEntry(*dim, 1, Value::Type::INTEGER, "Tile dimension")) {
        return false;
    }
    ...
    for (unsigned int i = 0; i < tiles->GetListDeprecated().size(); ++i) {
        ...
        if (!VerifyDictionaryEntry(tdict, "x", Value::Type::INTEGER) || !VerifyDictionaryEntry(tdict, "y", Value::Type::INTEGER)) {
            return false;
        }
        Tile t;
...
{
    if (!VerifyDictionaryEntry(node, "vertex_shader", Value::Type::STRING) || !VerifyDictionaryEntry(node, "fragment_shader", Value::Type::STRING)
    ...
    for (unsigned int i = 0; i < textures->GetListDeprecated().size(); ++i) {
        ...
        if (!VerifyDictionaryEntry(tex, "texID", Value::Type::INTEGER) || !VerifyDictionaryEntry(tex, "height", Value::Type::INTEGER)
            || !VerifyDictionaryEntry(tex, "width", Value::Type::INTEGER) || !VerifyDictionaryEntry(tex, "format", Value::Type::STRING)) {
            return false;
        }
        Texture t;
        ...
        if (t.format == GL_INVALID_ENUM) {
            LOG(ERROR) << "Unrecognized texture format in layer " << c->layerID() << " (format: " << *format_name
                       << ")\n"
                          "The layer had "
...
std::unique_ptr<RenderNode> InterpretContentLayer(const base::Value& node)
{
    ...
    if (!VerifyDictionaryEntry(node, "type", Value::Type::STRING) || !VerifyDictionaryEntry(node, "skipsDraw", Value::Type::BOOLEAN)
...
std::unique_ptr<RenderNode> InterpretNode(const base::Value& node)
{
    ...
    std::string outjson;
    JSONWriter::WriteWithOptions(node, base::JSONWriter::OPTIONS_PRETTY_PRINT, &outjson);
...
std::unique_ptr<RenderNode> BuildRenderTreeFromFile(const base::FilePath& path)
{
    ...
    auto result = JSONReader::ReadAndReturnValueWithError(contents, base::JSON_ALLOW_TRAILING_COMMAS);
    if (!result.has_value()) {
