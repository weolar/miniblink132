...
INSTANTIATE_TEST_SUITE_P(GLCubeMapTextureTests, GLCubeMapTextureTest, ::testing::ValuesIn(kCubeMapTextureTargets));
...
TEST_P(GLCubeMapTextureTest, TexImage2DAfterFBOBinding)
{
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, cube_map_target, texture_, 0);
    EXPECT_EQ(static_cast<GLenum>(GL_NO_ERROR), glGetError());
    ...
    glTexImage2D(cube_map_target, 0, GL_RGBA, width_, width_, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels_);
    EXPECT_EQ(static_cast<GLenum>(GL_NO_ERROR), glGetError());
}
...
TEST_P(GLCubeMapTextureTest, ReadPixels)
{
    ...
    for (unsigned i = 0; i < std::size(kCubeMapTextureTargets); i++) {
        glTexImage2D(kCubeMapTextureTargets[i], 0, GL_RGBA, width_, width_, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels_);
        EXPECT_EQ(static_cast<GLenum>(GL_NO_ERROR), glGetError());
    }
...
TEST_P(GLCubeMapTextureTest, ReadPixelsFromIncompleteCubeTexture)
{
    ...
    EXPECT_EQ(static_cast<GLenum>(GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT), glCheckFramebufferStatus(GL_FRAMEBUFFER));
    GLsizei size = width_ * width_ * 4;
    std::unique_ptr<uint8_t[]> pixels(new uint8_t[size]);
    glReadPixels(0, 0, width_, width_, GL_RGBA, GL_UNSIGNED_BYTE, pixels.get());
    EXPECT_EQ(static_cast<GLenum>(GL_INVALID_FRAMEBUFFER_OPERATION), glGetError());
}
