...
public:
    ...
    CommandBuffer::State WaitForGetOffsetInRange(uint32_t set_get_buffer_count, int32_t start, int32_t end) override
    {
        DCHECK_EQ(mode_, kDirect);
        return CommandBufferDirect::WaitForGetOffsetInRange(set_get_buffer_count, start, end);
    }
    ...
    int32_t saved_put_offset() const
    {
        return saved_put_offset_;
    }
    Mode mode() const
    {
        return mode_;
...
GpuPreferences GetGpuPreferences()
{
    ...
    if (gles2::UsePassthroughCommandDecoder(base::CommandLine::ForCurrentProcess()))
...
public:
        ...
        , translator_cache_(gpu_preferences_)
    {
        ...
        if (base::CommandLine::ForCurrentProcess()->HasSwitch("use-stub")) {
            ...
            scoped_refptr<gl::GLContextStub> context_stub = new gl::GLContextStub(share_group_.get());
            ...
            context_ = gl::init::CreateGLContext(share_group_.get(), surface_.get(), attribs);
        }
        ...
        scoped_refptr<gles2::ContextGroup> context_group = new gles2::ContextGroup(gpu_preferences_, true, &mailbox_manager_, nullptr /* memory_tracker */,
            &translator_cache_, &completeness_cache_, feature_info, bind_generates_resource, nullptr /* image_factory */, nullptr /* progress_reporter */,
            GpuFeatureInfo(), &discardable_manager_, &passthrough_discardable_manager_, &shared_image_manager_);
        ...
        ContextResult result = decoder_->Initialize(surface_.get(), context_.get(), true, gles2::DisallowedFeatures(), attrib_helper);
        DCHECK_EQ(result, ContextResult::kSuccess);
        capabilities_ = decoder_->GetCapabilities();

        const SharedMemoryLimits limits;
        gles2_helper_ = std::make_unique<gles2::GLES2CmdHelper>(command_buffer_.get());
        ...
        gles2_implementation_ = std::make_unique<gles2::GLES2Implementation>(
            gles2_helper_.get(), nullptr, transfer_buffer_.get(), bind_generates_resource, lose_context_when_out_of_memory, support_client_side_arrays, this);
    ...
    void Replay()
    {
        command_buffer_->Replay();
    }

    gles2::GLES2Implementation* gl()
    {
...
private:
    ...
    void GetGpuFence(uint32_t gpu_fence_id, base::OnceCallback<void(std::unique_ptr<gfx::GpuFence>)> callback) override
    {
        NOTREACHED();
    }
    ...
    void SignalSyncToken(const gpu::SyncToken& sync_token, base::OnceClosure callback) override
    {
        NOTREACHED();
    }
...
public:
        ...
        , iterations_(iterations)
    {
        ...
        for_linux_perf_ = base::CommandLine::ForCurrentProcess()->HasSwitch("for-linux-perf");
...
public:
    ...
    void SetUp() override
    {
        ...
        gl_->FramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, renderbuffer_);
    ...
    void StartRecord()
    {
        context_->StartRecord();
    }

    void StartReplay()
    {
        context_->StartReplay();
    }
    ...
    GLuint CompileShader(GLenum type, const char* source)
    {
        ...
        if (!compile_status) {
            ...
            if (log_length) {
                ...
                gl_->GetShaderInfoLog(shader, log_length, &returned_log_length, log.get());
    ...
    GLuint CreateAndLinkProgram(const char* vertex_shader, const char* fragment_shader, const std::initializer_list<AttribBinding>& attrib_bindings)
    {
    ...
    void CreateBasicTexture(GLuint texture)
    {
        ...
        gl_->TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 4, 4, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    }
...
constexpr const char kVertexShader[] = "attribute vec2 position;\n"
                                       "uniform vec2 scale;\n"
                                       "uniform vec2 offset;\n"
...
constexpr const char kFragmentShader[] = "precision mediump float;\n"
                                         "varying vec2 texcoords;\n"
...
{
    GLuint program = CreateAndLinkProgram(kVertexShader, kFragmentShader, { { "postition", 0 } });
    ...
    gl_->BufferData(GL_ARRAY_BUFFER, sizeof(kVertices), kVertices, GL_STATIC_DRAW);
...
{
    GLuint program = CreateAndLinkProgram(kVertexShader, kFragmentShader, { { "position", 0 } });
    gl_->UseProgram(program);
...
{
    const char kVertexShader2[] = "attribute vec2 position;\n"
                                  "uniform vec2 scale;\n"
                                  "uniform vec2 offset;\n"
                                  "void main () {\n"
                                  "  gl_Position = vec4(position * scale + offset, 0.0, 1.0);\n"
    ...
    const char kFragmentShader2[] = "precision mediump float;\n"
                                    "uniform vec4 color;\n"
                                    "void main() {\n"
                                    "  gl_FragColor = color;\n"
                                    "}\n";
    ...
    GLuint programs[2];
    programs[0] = CreateAndLinkProgram(kVertexShader, kFragmentShader, { { "position", 0 } });
    ...
    programs[1] = CreateAndLinkProgram(kVertexShader2, kFragmentShader2, { { "position", 0 } });
    ...
    GLint offset_locations[2] = { gl_->GetUniformLocation(programs[0], "offset"), gl_->GetUniformLocation(programs[1], "offset") };
