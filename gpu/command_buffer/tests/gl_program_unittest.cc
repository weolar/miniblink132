...
TEST_F(GLProgramTest, GetSetUniform)
{
    static const char* v_shader_str = SHADER(
        attribute vec4 a_vertex; attribute vec3 a_normal;
        ...
        uniform MyStruct u_struct; uniform float u_array[4];
    ...
    static const char* f_shader_str = SHADER(varying mediump vec3 v_normal;

                                             void main() { gl_FragColor = vec4(v_normal / 2.0 + vec3(0.5), 1); });
...
TEST_F(GLProgramTest, NewShaderInCurrentProgram)
{
    ...
    static const char* f_red_shader_str = SHADER(void main() { gl_FragColor = vec4(1, 0, 0, 1); });
    static const char* f_blue_shader_str = SHADER(void main() { gl_FragColor = vec4(0, 0, 1, 1); });
    ...
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, 1, 1, 0, expected_color, nullptr));
    GLTestHelper::CheckGLError("no errors", __LINE__);
}

TEST_F(GLProgramTest, ShaderLengthSpecified)
{
    const std::string valid_shader_str = SHADER(attribute vec4 a_position; void main() { gl_Position = a_position; });
