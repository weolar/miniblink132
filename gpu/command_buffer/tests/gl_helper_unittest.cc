...
protected:
    void SetUp() override
    {
        ...
        auto result
            = context_->Initialize(viz::TestGpuServiceHolder::GetInstance()->task_executor(), attributes, SharedMemoryLimits(), /*image_factory=*/nullptr);
    ...
    int Channel(SkBitmap* pixels, int x, int y, int c)
    {
        if (pixels->bytesPerPixel() == 4) {
            uint32_t* data = pixels->getAddr32(base::clamp(x, 0, pixels->width() - 1), base::clamp(y, 0, pixels->height() - 1));
            return (*data) >> (c * 8) & 0xff;
        } else {
            DCHECK_EQ(pixels->bytesPerPixel(), 1);
            DCHECK_EQ(c, 0);
            return *pixels->getAddr8(base::clamp(x, 0, pixels->width() - 1), base::clamp(y, 0, pixels->height() - 1));
        }
    }
    ...
    std::string PrintStages(const std::vector<GLHelperScaling::ScalerStage>& scaler_stages)
    {
        std::string ret;
        for (size_t i = 0; i < scaler_stages.size(); i++) {
            ret.append(base::StringPrintf("%dx%d -> %dx%d ", scaler_stages[i].scale_from.x(), scaler_stages[i].scale_from.y(), scaler_stages[i].scale_to.x(),
    ...
    void ValidateScalerStages(GLHelper::ScalerQuality quality, const std::vector<GLHelperScaling::ScalerStage>& scaler_stages,
        const gfx::Vector2d& overall_scale_from, const gfx::Vector2d& overall_scale_to, const std::string& message)
    {
        ...
        for (size_t i = 0; i < scaler_stages.size(); i++) {
            ...
            double x_scale = static_cast<double>(scaler_stages[i].scale_from.x()) / static_cast<double>(scaler_stages[i].scale_to.x());
            combined_x_scale *= x_scale;
            double y_scale = static_cast<double>(scaler_stages[i].scale_from.y()) / static_cast<double>(scaler_stages[i].scale_to.y());
            ...
            if (x_samples) {
                EXPECT_TRUE(CheckScale(x_scale, x_samples, scaled_x)) << "x_scale = " << x_scale;
            }
            if (y_samples) {
                EXPECT_TRUE(CheckScale(y_scale, y_samples, scaled_y)) << "y_scale = " << y_scale;
            }
        ...
        const double expected_x_scale = static_cast<double>(overall_scale_from.x()) / static_cast<double>(overall_scale_to.x());
        const double expected_y_scale = static_cast<double>(overall_scale_from.y()) / static_cast<double>(overall_scale_to.y());
    ...
    // and |message|.
    void Compare(
        SkBitmap* truth, SkBitmap* other, int maxdiff, SkBitmap* source, const std::vector<GLHelperScaling::ScalerStage>& scaler_stages, std::string message)
    {
        EXPECT_EQ(truth->width(), other->width());
        EXPECT_EQ(truth->height(), other->height());
        bool swizzle = (truth->colorType() == kRGBA_8888_SkColorType && other->colorType() == kBGRA_8888_SkColorType)
            || (truth->colorType() == kBGRA_8888_SkColorType && other->colorType() == kRGBA_8888_SkColorType);
        ...
        for (int x = 0; x < truth->width(); x++) {
            for (int y = 0; y < truth->height(); y++) {
                for (int c = 0; c < bpp; c++) {
                    ...
                    int b = swizzle && (c == 0 || c == 2) ? Channel(other, x, y, (c + 2) & 2) : Channel(other, x, y, c);
                    EXPECT_NEAR(a, b, maxdiff) << " x=" << x << " y=" << y << " c=" << c << " " << message;
    ...
    float Bilinear(SkBitmap* pixels, float x, float y, int c)
    {
        ...
        x -= base_x;
        y -= base_y;
        return (ChannelAsFloat(pixels, base_x, base_y, c) * (1 - x) * (1 - y) + ChannelAsFloat(pixels, base_x + 1, base_y, c) * x * (1 - y)
            + ChannelAsFloat(pixels, base_x, base_y + 1, c) * (1 - x) * y + ChannelAsFloat(pixels, base_x + 1, base_y + 1, c) * x * y);
    }

    // Very slow bicubic / bilinear scaler for reference.
    void ScaleSlow(SkBitmap* input, const gfx::Rect& source_rect, GLHelper::ScalerQuality quality, SkBitmap* output)
    {
            ...
            for (int dst_x = 0; dst_x < output->width(); dst_x++) {
                for (int channel = 0; channel < 4; channel++) {
                    ...
                    case GLHelper::SCALER_QUALITY_BEST:
                        for (int src_y = source_rect.y() - 10; src_y < source_rect.bottom() + 10; ++src_y) {
                            float coeff_y = Bicubic((src_y + 0.5f - dst_y_in_src) * clamped_yscale);
                            if (coeff_y == 0.0f) {
                                continue;
                            }
                            for (int src_x = source_rect.x() - 10; src_x < source_rect.right() + 10; ++src_x) {
                                float coeff = coeff_y * Bicubic((src_x + 0.5f - dst_x_in_src) * clamped_xscale);
                                if (coeff == 0.0f) {
                                    continue;
                                }
                                sum += coeff;
                    ...
                    case GLHelper::SCALER_QUALITY_GOOD: {
                        ...
                        for (int x = 0; x < xmag; x++) {
                            for (int y = 0; y < ymag; y++) {
                                value += Bilinear(input, source_rect.x() + (dst_x * xmag + x + 0.5) * xscale / xmag,
                                    source_rect.y() + (dst_y * ymag + y + 0.5) * yscale / ymag, channel);
                                sum += 1.0;
                            }
                        }
                        break;
                    ...
                    value /= sum;
                    SetChannel(output, dst_x, dst_y, channel, static_cast<int>(value * 255.0f + 0.5f));
                }
            }
        }
    }
    ...
    void FlipSKBitmap(SkBitmap* bitmap)
    {
        ...
        while (top_line < bottom_line) {
            for (int x = 0; x < bitmap->width(); x++) {
                bpp == 4 ? std::swap(*bitmap->getAddr32(x, top_line), *bitmap->getAddr32(x, bottom_line))
                         : std::swap(*bitmap->getAddr8(x, top_line), *bitmap->getAddr8(x, bottom_line));
            }
            top_line++;
            bottom_line--;
        }
    ...
    void ScaleSlowRecursive(SkBitmap* input, const gfx::Rect& source_rect, GLHelper::ScalerQuality quality, SkBitmap* output)
    {
        if (quality == GLHelper::SCALER_QUALITY_FAST || quality == GLHelper::SCALER_QUALITY_GOOD) {
        ...
        if ((xscale == 1.0f && yscale == 1.0f) || (xscale == 0.5f && yscale == 1.0f) || (xscale == 1.0f && yscale == 0.5f) || (xscale >= 1.0f && yscale == 1.0f)
    ...
    // for overscan.
    std::unique_ptr<SkBitmap> CreateTestBitmap(const gfx::Size& size, const gfx::Point& origin, int test_pattern)
    {
        std::unique_ptr<SkBitmap> bitmap(new SkBitmap);
        bitmap->allocPixels(SkImageInfo::Make(size.width(), size.height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType));
    ...
    void TestScale(const gfx::Rect& source_rect, const gfx::Size& scaled_size, int test_pattern, size_t quality_index, bool flip_output)
    {
        ...
        std::unique_ptr<SkBitmap> input_pixels = CreateTestBitmap(framebuffer_size, source_rect.origin(), test_pattern);
        GLuint src_texture;
        gl_->GenTextures(1, &src_texture);
        gl_->BindTexture(GL_TEXTURE_2D, src_texture);
        gl_->TexImage2D(
            GL_TEXTURE_2D, 0, GL_RGBA, framebuffer_size.width(), framebuffer_size.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, input_pixels->getPixels());

        std::string message = base::StringPrintf("source rect: %s "
                                                 "output size: %s "
                                                 "pattern: %d quality: %s %s",
            source_rect.ToString().c_str(), scaled_size.ToString().c_str(), test_pattern, kQualityNames[quality_index], flip_output ? "flipout" : "noflipout");

        std::vector<GLHelperScaling::ScalerStage> stages;
        const auto scale_from = gfx::Vector2d(source_rect.width(), source_rect.height());
        const auto scale_to = gfx::Vector2d(scaled_size.width(), scaled_size.height());
        helper_scaling_->ComputeScalerStages(kQualities[quality_index], scale_from, scale_to, false, flip_output, false, &stages);
        ValidateScalerStages(kQualities[quality_index], stages, scale_from, scale_to, message);

        // Scale the source texture, producing the results in a new output
        // texture.
        std::unique_ptr<GLHelper::ScalerInterface> scaler = helper_->CreateScaler(kQualities[quality_index], scale_from, scale_to, false, flip_output, false);
        ...
        gl_->TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, scaled_size.width(), scaled_size.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
        scaler->Scale(src_texture, framebuffer_size, source_rect.OffsetFromOrigin(), dst_texture, gfx::Rect(scaled_size));

        SkBitmap output_pixels;
        output_pixels.allocPixels(SkImageInfo::Make(scaled_size.width(), scaled_size.height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType));

        EXPECT_TRUE(ReadBackTexture(
            dst_texture, scaled_size, static_cast<unsigned char*>(output_pixels.getPixels()), output_pixels.rowBytes(), flip_output, kRGBA_8888_SkColorType));

        // If the bitmap shouldn't have changed - compare against input.
        if (source_rect == gfx::Rect(scaled_size)) {
            Compare(input_pixels.get(), &output_pixels, 0, nullptr, stages, message + " comparing against input");
            return;
        }

        // Now scale the bitmap using the reference implementation.
        SkBitmap truth_pixels;
        truth_pixels.allocPixels(SkImageInfo::Make(scaled_size.width(), scaled_size.height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType));
        ScaleSlowRecursive(input_pixels.get(), source_rect, kQualities[quality_index], &truth_pixels);
        ...
        Compare(&truth_pixels, &output_pixels, maxdiff, input_pixels.get(), stages, message + " comparing against scaled");
    ...
    void TestScalePatching(const gfx::Vector2d& scale_from, const gfx::Vector2d& scale_to, int test_pattern, size_t quality_index, bool flipped_source)
    {
        ...
        std::unique_ptr<SkBitmap> test_bitmap = CreateTestBitmap(framebuffer_size, gfx::Point(), test_pattern);
        if (flipped_source)
            FlipSKBitmap(test_bitmap.get());
        GLuint src_texture;
        gl_->GenTextures(1, &src_texture);
        gl_->BindTexture(GL_TEXTURE_2D, src_texture);
        gl_->TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, framebuffer_size.width(), framebuffer_size.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, test_bitmap->getPixels());

        const std::unique_ptr<GLHelper::ScalerInterface> scaler
            = helper_->CreateScaler(kQualities[quality_index], scale_from, scale_to, flipped_source, false, false);
        ...
        std::vector<GLHelperScaling::ScalerStage> stages;
        helper_scaling_->ComputeScalerStages(kQualities[quality_index], scale_from, scale_to, flipped_source, false, false, &stages);
        ...
        gl_->TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, entire_output_size.width(), entire_output_size.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
        scaler->Scale(src_texture, framebuffer_size, gfx::Vector2dF(), dst_texture, gfx::Rect(entire_output_size));
        SkBitmap entire_output;
        entire_output.allocPixels(SkImageInfo::Make(entire_output_size.width(), entire_output_size.height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType));

        EXPECT_TRUE(ReadBackTexture(dst_texture, entire_output_size, static_cast<unsigned char*>(entire_output.getPixels()), entire_output.rowBytes(),
            /*flip_y=*/false, kRGBA_8888_SkColorType));

        const std::string human_readable_test_params = base::StringPrintf("scale from: %s "
                                                                          "scale to: %s "
                                                                          "pattern: %d quality: %s %s",
            scale_from.ToString().c_str(), scale_to.ToString().c_str(), test_pattern, kQualityNames[quality_index], flipped_source ? "flippedsource" : "");

        // Check the entire output image against the reference implementation.
        SkBitmap entire_output_ref;
        entire_output_ref.allocPixels(SkImageInfo::Make(entire_output_size.width(), entire_output_size.height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType));
        ScaleSlowRecursive(test_bitmap.get(), gfx::Rect(framebuffer_size), kQualities[quality_index], &entire_output_ref);
        Compare(&entire_output_ref, &entire_output, 2, test_bitmap.get(), stages, human_readable_test_params + " ENTIRE OUTPUT");
        if (HasFailure())
            return;
        ...
        gl_->BindTexture(GL_TEXTURE_2D, dst_texture);
        gl_->TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, patch_size.width(), patch_size.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
        for (int xoffset = 0; xoffset < scale_to.x(); ++xoffset) {
            for (int yoffset = 0; yoffset < scale_to.y(); ++yoffset) {
                ...
                scaler->Scale(src_texture, framebuffer_size, gfx::Vector2dF(), dst_texture, patch_rect);
                SkBitmap patch_output;
                patch_output.allocPixels(SkImageInfo::Make(patch_size.width(), patch_size.height(), kRGBA_8888_SkColorType, kPremul_SkAlphaType));
                EXPECT_TRUE(ReadBackTexture(dst_texture, patch_size, static_cast<unsigned char*>(patch_output.getPixels()), patch_output.rowBytes(),
                    /*flip_y=*/false, kRGBA_8888_SkColorType));
                SkBitmap expected;
                SkIRect expected_subrect { patch_rect.x(), patch_rect.y(), patch_rect.right(), patch_rect.bottom() };
                if (flipped_source) {
                ...
                Compare(
                    &expected, &patch_output, 2, test_bitmap.get(), stages, "METHOD1 " + human_readable_test_params + " patch rect: " + patch_rect.ToString());
                if (HasFailure())
                    return;
                ...
                scaler->ComputeRegionOfInfluence(framebuffer_size, gfx::Vector2dF(), patch_rect, &sampling_rect, &offset);
                ...
                if (offset.x() == std::floor(offset.x()) && offset.y() == std::floor(offset.y())) {
                    GLuint src_subset_texture;
                    gl_->GenTextures(1, &src_subset_texture);
                    gl_->BindTexture(GL_TEXTURE_2D, src_subset_texture);
                    gl_->TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, sampling_rect.width(), sampling_rect.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
                    gl_->CopySubTextureCHROMIUM(src_texture, 0 /* source_level */, GL_TEXTURE_2D, src_subset_texture, 0 /* dest_level */, 0 /* xoffset */,
                        0 /* yoffset */, sampling_rect.x(), sampling_rect.y(), sampling_rect.width(), sampling_rect.height(), false, false, false);
                    scaler->Scale(src_subset_texture, sampling_rect.size(), offset, dst_texture, gfx::Rect(patch_size));
                    gl_->DeleteTextures(1, &src_subset_texture);
                    EXPECT_TRUE(ReadBackTexture(dst_texture, patch_size, static_cast<unsigned char*>(patch_output.getPixels()), patch_output.rowBytes(),
                        /*flip_y=*/false, kRGBA_8888_SkColorType));
                    Compare(&expected, &patch_output, 2, test_bitmap.get(), stages,
                        "METHOD2 " + human_readable_test_params + " patch rect: " + patch_rect.ToString());
                    if (HasFailure())
                        return;
                }
            }
    ...
    void TestScalerPipeline(size_t quality, int xsize, int ysize, int dst_xsize, int dst_ysize)
    {
        std::vector<GLHelperScaling::ScalerStage> stages;
        helper_scaling_->ComputeScalerStages(
            kQualities[quality], gfx::Vector2d(xsize, ysize), gfx::Vector2d(dst_xsize, dst_ysize), false, false, false, &stages);
        ValidateScalerStages(kQualities[quality], stages, gfx::Vector2d(xsize, ysize), gfx::Vector2d(dst_xsize, dst_ysize),
            base::StringPrintf("input size: %dx%d "
                               "output size: %dx%d "
                               "quality: %s",
                xsize, ysize, dst_xsize, dst_ysize, kQualityNames[quality]));
    }

    // Create a scaling pipeline and make sure that the steps
    // are exactly the steps we expect.
    void CheckPipeline(GLHelper::ScalerQuality quality, int xsize, int ysize, int dst_xsize, int dst_ysize, const std::string& description)
    {
        std::vector<GLHelperScaling::ScalerStage> stages;
        helper_scaling_->ComputeScalerStages(quality, gfx::Vector2d(xsize, ysize), gfx::Vector2d(dst_xsize, dst_ysize), false, false, false, &stages);
        ValidateScalerStages(GLHelper::SCALER_QUALITY_GOOD, stages, gfx::Vector2d(xsize, ysize), gfx::Vector2d(dst_xsize, dst_ysize), "");
        EXPECT_EQ(PrintStages(stages), description);
    }

    void DrawGridToBitmap(int w, int h, SkColor background_color, SkColor grid_color, int grid_pitch, int grid_width, const SkBitmap& bmp)
    {
    ...
    void DrawCheckerToBitmap(int w, int h, SkColor color1, SkColor color2, int rect_w, int rect_h, const SkBitmap& bmp)
    {
        ASSERT_GT(rect_w, 0);
        ASSERT_GT(rect_h, 0);
    ...
    void BindAndAttachTextureWithPixels(GLuint src_texture, SkColorType color_type, const gfx::Size& src_size, const SkBitmap& input_pixels)
    {
        gl_->BindTexture(GL_TEXTURE_2D, src_texture);
        const GLenum format = (color_type == kBGRA_8888_SkColorType) ? GL_BGRA_EXT : GL_RGBA;
        gl_->TexImage2D(GL_TEXTURE_2D, 0, format, src_size.width(), src_size.height(), 0, format, GL_UNSIGNED_BYTE, input_pixels.getPixels());
    }

    bool ReadBackTexture(GLuint src_texture, const gfx::Size& src_size, unsigned char* pixels, size_t pixels_stride, bool flip_y, SkColorType color_type)
    {
        DCHECK(color_type == kRGBA_8888_SkColorType || color_type == kBGRA_8888_SkColorType);
        ...
        helper_->ReadbackTextureAsync(src_texture, GL_TEXTURE_2D, src_size, pixels, pixels_stride, flip_y, format,
            base::BindOnce(
    ...
    bool TestTextureFormatReadback(const gfx::Size& src_size, SkColorType color_type)
    {
        SkImageInfo info = SkImageInfo::Make(src_size.width(), src_size.height(), color_type, kPremul_SkAlphaType);
        GLuint src_texture;
        gl_->GenTextures(1, &src_texture);
        ...
        BindAndAttachTextureWithPixels(src_texture, color_type, src_size, input_pixels);
        SkBitmap output_pixels;
        ...
        if (!ReadBackTexture(src_texture, src_size, pixels, output_pixels.rowBytes(), /*flip_y=*/false, color_type) || !IsEqual(input_pixels, output_pixels)) {
        ...
        DrawGridToBitmap(src_size.width(), src_size.height(), color2, color1, src_grid_pitch, src_grid_width, input_pixels);
        BindAndAttachTextureWithPixels(src_texture, color_type, src_size, input_pixels);
        if (!ReadBackTexture(src_texture, src_size, pixels, output_pixels.rowBytes(), /*flip_y=*/false, color_type) || !IsEqual(input_pixels, output_pixels)) {
        ...
        DrawCheckerToBitmap(src_size.width(), src_size.height(), color1, color2, rect_w, rect_h, input_pixels);
        BindAndAttachTextureWithPixels(src_texture, color_type, src_size, input_pixels);
        if (!ReadBackTexture(src_texture, src_size, pixels, output_pixels.rowBytes(), /*flip_y=*/false, color_type) || !IsEqual(input_pixels, output_pixels)) {
    ...
    void CheckPipeline2(int xsize, int ysize, int dst_xsize, int dst_ysize, const std::string& description)
    {
        std::vector<GLHelperScaling::ScalerStage> stages;
        helper_scaling_->ConvertScalerOpsToScalerStages(GLHelper::SCALER_QUALITY_GOOD, gfx::Vector2d(xsize, ysize), &x_ops_, &y_ops_, &stages);
...
TEST_F(GLHelperTest, RGBAASyncReadbackTest)
{
    ...
    bool result = TestTextureFormatReadback(gfx::Size(kTestSize, kTestSize), kRGBA_8888_SkColorType);
...
{
    const int kTestSize = 64;
    bool result = TestTextureFormatReadback(gfx::Size(kTestSize, kTestSize), kBGRA_8888_SkColorType);
    EXPECT_EQ(result, true);
}

int kRGBReadBackSizes[] = { 3, 6, 16 };

class GLHelperPixelReadbackTest : public GLHelperPixelTest,
                                  public ::testing::WithParamInterface<std::tuple<unsigned int, unsigned int, unsigned int, unsigned int, unsigned int>> { };
        ...
        for (int pattern = 0; pattern < 3; ++pattern) {
            for (int xoffset = 0; xoffset < 4; ++xoffset) {
                for (int yoffset = 0; yoffset < 4; ++yoffset) {
                    TestScale(gfx::Rect(xoffset, yoffset, kRGBReadBackSizes[x], kRGBReadBackSizes[y]),
                        gfx::Size(kRGBReadBackSizes[dst_x], kRGBReadBackSizes[dst_y]), pattern, q_index, !!flip_output);
                    if (HasFailure()) {
                        return;
                    }
                }
...
TEST_P(GLHelperPixelReadbackTest, DISABLED_ScalePatching)
{
    for (int flipped_source = 0; flipped_source <= 1; ++flipped_source) {
        for (int pattern = 0; pattern < 3; ++pattern) {
            TestScalePatching(gfx::Vector2d(kRGBReadBackSizes[std::get<1>(GetParam())], kRGBReadBackSizes[std::get<2>(GetParam())]),
                gfx::Vector2d(kRGBReadBackSizes[std::get<3>(GetParam())], kRGBReadBackSizes[std::get<4>(GetParam())]), pattern, std::get<0>(GetParam()),
                !!flipped_source);
            if (HasFailure()) {
                return;
            }
...
INSTANTIATE_TEST_SUITE_P(All, GLHelperPixelReadbackTest,
    ::testing::Combine(::testing::Range<unsigned int>(0, std::size(kQualities)), ::testing::Range<unsigned int>(0, std::size(kRGBReadBackSizes)),
        ::testing::Range<unsigned int>(0, std::size(kRGBReadBackSizes)), ::testing::Range<unsigned int>(0, std::size(kRGBReadBackSizes)),
            ...
            for (size_t y = 0; y < std::size(sizes); y++) {
                for (size_t dst_x = 0; dst_x < std::size(sizes); dst_x++) {
                    for (size_t dst_y = 0; dst_y < std::size(sizes); dst_y++) {
                        TestScalerPipeline(q, sizes[x], sizes[y], sizes[dst_x], sizes[dst_y]);
                        if (HasFailure()) {
                            return;
                        }
                    }
...
TEST_F(GLHelperTest, CheckSpecificPipelines)
{
    ...
    CheckPipeline(GLHelper::SCALER_QUALITY_GOOD, 1024, 700, 1280, 720, "1024x700 -> 1280x720 bilinear\n");
    // Slight downscale should use BILINEAR2X2.
    CheckPipeline(GLHelper::SCALER_QUALITY_GOOD, 1280, 720, 1024, 700, "1280x720 -> 1024x700 bilinear2x2\n");
    ...
    for (int allow3 = 0; allow3 <= 1; allow3++) {
        for (int dst = 1; dst < 2049; dst += 1 + (dst >> 3)) {
            for (int src = 1; src < 2049; src++) {
                ...
                if (HasFailure()) {
                    LOG(ERROR) << "Failed for src=" << src << " dst=" << dst << " allow3=" << allow3;
                    return;
                }
            }
        }
