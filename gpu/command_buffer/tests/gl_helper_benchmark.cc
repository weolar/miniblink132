...
protected:
    ...
    void LoadPngFileToSkBitmap(const base::FilePath& filename, SkBitmap* bitmap)
    {
        ...
        ASSERT_TRUE(gfx::PNGCodec::Decode(reinterpret_cast<const unsigned char*>(compressed.data()), compressed.size(), bitmap));
    }
    ...
    void SaveToFile(SkBitmap* bitmap, const base::FilePath& filename)
    {
        ...
        ASSERT_TRUE(gfx::PNGCodec::Encode(static_cast<unsigned char*>(bitmap->getPixels()), gfx::PNGCodec::FORMAT_BGRA,
            gfx::Size(bitmap->width(), bitmap->height()), static_cast<int>(bitmap->rowBytes()), true, std::vector<gfx::PNGCodec::Comment>(), &compressed));
        ASSERT_TRUE(compressed.size());
        FILE* f = base::OpenFile(filename, "wb");
        ASSERT_TRUE(f);
        ASSERT_EQ(fwrite(&*compressed.begin(), 1, compressed.size(), f), compressed.size());
        base::CloseFile(f);
    }
    ...
    for (size_t q = 0; q < std::size(kQualities); q++) {
        for (size_t outsize = 0; outsize < std::size(output_sizes); outsize += 2) {
            for (size_t insize = 0; insize < std::size(input_sizes); insize += 2) {
                ...
                const gfx::Size dst_size(output_sizes[outsize], output_sizes[outsize + 1]);
                SkBitmap input;
                ...
                gl_->TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, dst_size.width(), dst_size.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
                gl_->BindTexture(GL_TEXTURE_2D, src_texture);
                gl_->TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, src_size.width(), src_size.height(), 0, GL_RGBA, GL_UNSIGNED_BYTE, input.getPixels());

                std::unique_ptr<GLHelper::ScalerInterface> scaler = helper_->CreateScaler(
                    kQualities[q], gfx::Vector2d(src_size.width(), src_size.height()), gfx::Vector2d(dst_size.width(), dst_size.height()), false, false, false);
                // Scale once beforehand before we start measuring.
                const gfx::Rect output_rect(dst_size);
                scaler->Scale(src_texture, src_size, gfx::Vector2dF(), dst_texture, output_rect);
                gl_->Finish();
                ...
                std::string name;
                name
                    = base::StringPrintf("scale_%dx%d_to_%dx%d_%s", src_size.width(), src_size.height(), dst_size.width(), dst_size.height(), kQualityNames[q]);
