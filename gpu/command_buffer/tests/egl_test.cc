...
TEST_F(EGLTest, GetDisplay)
{
    ...
    EGLNativeDisplayType invalid_display_type = static_cast<EGLNativeDisplayType>(0x1);
#else
    EGLNativeDisplayType invalid_display_type = reinterpret_cast<EGLNativeDisplayType>(0x1);
#endif
...
TEST_F(EGLTest, ChooseConfigUninitialized)
{
    ...
    EXPECT_FALSE(eglChooseConfig(display, attrib_list, configs, kConfigsSize, &num_config));
    EXPECT_EQ(EGL_NOT_INITIALIZED, eglGetError());

    EXPECT_FALSE(eglChooseConfig(display, attrib_list, configs, kConfigsSize, nullptr));
    EXPECT_EQ(EGL_NOT_INITIALIZED, eglGetError());
}
...
{
    EGLint attrib_list[] = { EGL_NONE };
    EXPECT_FALSE(eglChooseConfig(display_, attrib_list, configs_, kConfigsSize, nullptr));
    EXPECT_EQ(EGL_BAD_PARAMETER, eglGetError());
}
...
TEST_F(EGLConfigTest, ChooseConfig)
{
    EGLint num_config = 0;
    EGLint attrib_list[] = { EGL_NONE };
    EXPECT_TRUE(eglChooseConfig(display_, attrib_list, configs_, kConfigsSize, &num_config));
    EXPECT_GT(num_config, 0);
    CheckConfigsExist(num_config);
}
...
TEST_F(EGLConfigTest, ChooseConfigInvalidAttrib)
{
    ...
    EXPECT_FALSE(eglChooseConfig(display_, invalid_attrib_list, configs_, kConfigsSize, &num_config));
...
{
    EGLint num_config = 0;
    EGLint attrib_list[] = { EGL_SURFACE_TYPE, EGL_PBUFFER_BIT | EGL_WINDOW_BIT, EGL_NONE };
    EXPECT_TRUE(eglChooseConfig(display_, attrib_list, configs_, kConfigsSize, &num_config));
...
{
    static const EGLint kConfigAttribs[] = { EGL_RED_SIZE, 8, EGL_GREEN_SIZE, 8, EGL_BLUE_SIZE, 8, EGL_ALPHA_SIZE, 8, EGL_DEPTH_SIZE, 8, EGL_STENCIL_SIZE, 8,
        EGL_SAMPLE_BUFFERS, 1, EGL_SAMPLES, 4, EGL_NONE };
    EGLint num_config = 0;
    EXPECT_TRUE(eglChooseConfig(display_, kConfigAttribs, configs_, kConfigsSize, &num_config));
...
TEST_F(EGLTest, MakeCurrent)
{
    ...
    //  eglMakeCurrent."
    EXPECT_TRUE(eglMakeCurrent(display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT));
    EGLDisplay invalid_display = reinterpret_cast<EGLDisplay>(0x1);
    EXPECT_FALSE(eglMakeCurrent(invalid_display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT));
    EXPECT_EQ(EGL_BAD_DISPLAY, eglGetError());
...
void EGLSurfaceTest::CreateSurfaceAndContext(EGLSurface* surface, EGLContext* context)
{
    static const EGLint config_attribs[] = { EGL_SURFACE_TYPE, EGL_PBUFFER_BIT, EGL_NONE };
    EGLint num_config;
    EGLConfig config;
    EXPECT_TRUE(eglChooseConfig(display_, config_attribs, &config, 1, &num_config));
    ASSERT_GT(num_config, 0);
    static const EGLint surface_attribs[] = { EGL_WIDTH, 1, EGL_HEIGHT, 1, EGL_NONE };
...
{
    EXPECT_TRUE(eglMakeCurrent(display_, surface1_, surface1_, context1_));
    EXPECT_TRUE(eglMakeCurrent(display_, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT));
    EXPECT_TRUE(eglMakeCurrent(display_, surface2_, surface2_, context2_));
    EXPECT_TRUE(eglMakeCurrent(display_, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT));
    EXPECT_TRUE(eglMakeCurrent(display_, surface1_, surface1_, context2_));
    EXPECT_TRUE(eglMakeCurrent(display_, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT));
...
TEST_F(EGLMultipleSurfacesContextsTest, MakeCurrentSurfaceFails)
{
    ...
    EXPECT_FALSE(eglMakeCurrent(invalid_display, surface1_, surface1_, context1_));
    EXPECT_EQ(EGL_BAD_DISPLAY, eglGetError());
...
TEST_F(EGLMultipleSurfacesContextsTest, CallGLOnMultipleContextNoCrash)
{
    ...
    glEnableProc glEnable = reinterpret_cast<glEnableProc>(eglGetProcAddress("glEnable"));
...
public:
    ...
    void OtherThreadMakeCurrent(EGLSurface surface, EGLContext context, EGLBoolean* result, base::WaitableEvent*);
...
EGLThreadTest::EGLThreadTest()
    : EGLSurfaceTest()
    , other_thread_("EGLThreadTest thread")
{
}
...
void EGLThreadTest::TearDown()
{
    ...
    other_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&EGLThreadTest::OtherThreadTearDown, base::Unretained(this), &completion));
    completion.Wait();
    other_thread_.Stop();
...
void EGLThreadTest::OtherThreadMakeCurrent(EGLSurface surface, EGLContext context, EGLBoolean* result, base::WaitableEvent* completion)
{
    *result = eglMakeCurrent(display_, surface, surface, context);
    completion->Signal();
}

void EGLThreadTest::OtherThreadGetError(EGLint* result, base::WaitableEvent* completion)
{
    *result = eglGetError();
    completion->Signal();
}
...
TEST_F(EGLThreadTest, Basic)
{
    ...
    base::WaitableEvent completion(base::WaitableEvent::ResetPolicy::AUTOMATIC, base::WaitableEvent::InitialState::NOT_SIGNALED);

    EGLBoolean result = EGL_FALSE;
    other_thread_.task_runner()->PostTask(
        FROM_HERE, base::BindOnce(&EGLThreadTest::OtherThreadMakeCurrent, base::Unretained(this), surface, context, &result, &completion));
    completion.Wait();
    EXPECT_FALSE(result);
    EXPECT_EQ(EGL_SUCCESS, eglGetError());

    EGLint error = EGL_NONE;
    other_thread_.task_runner()->PostTask(FROM_HERE, base::BindOnce(&EGLThreadTest::OtherThreadGetError, base::Unretained(this), &error, &completion));
    completion.Wait();
...
TEST_F(EGLTest, WindowlessNativeWindows)
{
    ...
    static const EGLint config_attribs[] = { EGL_SURFACE_TYPE, EGL_WINDOW_BIT, EGL_NONE };
    EGLint num_config;
    EGLConfig config;
    EXPECT_TRUE(eglChooseConfig(display, config_attribs, &config, 1, &num_config));
    ASSERT_GT(num_config, 0);
    ...
    EGLSurface surface = eglCreateWindowSurface(display, config, win, surface_attribs);
    EXPECT_NE(EGL_NO_SURFACE, surface);
