    ...
    void WaitForQueryCallback(int q, base::OnceClosure cb)
    {
        ...
        if (done) {
            ...
            base::ThreadTaskRunnerHandle::Get()->PostDelayedTask(
                FROM_HERE, base::BindOnce(&GLReadbackTest::WaitForQueryCallback, base::Unretained(this), q, std::move(cb)), base::Milliseconds(3));
        }
    }
...
TEST_F(GLReadbackTest, ReadPixelsWithPBOAndQuery)
{
    ...
    glBufferData(GL_PIXEL_PACK_TRANSFER_BUFFER_CHROMIUM, kWidth * kHeight * kBytesPerPixel, nullptr, GL_STREAM_READ);
    ...
    unsigned char* data = static_cast<unsigned char*>(glMapBufferCHROMIUM(GL_PIXEL_PACK_TRANSFER_BUFFER_CHROMIUM, GL_READ_ONLY));
    EXPECT_TRUE(data);
...
TEST_F(GLReadbackTest, MAYBE_ReadPixelsFloat)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(quad_vertices), reinterpret_cast<void*>(quad_vertices), GL_STATIC_DRAW);

    GLint position_location = glGetAttribLocation(program, "a_position");
    glVertexAttribPointer(position_location, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), nullptr);
    ...
    for (size_t ii = 0; ii < test_count; ++ii) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, test_formats[ii].format, kTextureSize, kTextureSize, 0, test_formats[ii].format, test_formats[ii].type, nullptr);
        ...
        if (glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE) {
            ...
            if ((read_format == GL_RGB || read_format == GL_RGBA) && read_type == test_formats[ii].type) {
                ...
                case GL_HALF_FLOAT_OES: {
                    std::unique_ptr<GLushort[]> buf(new GLushort[kTextureSize * kTextureSize * read_comp_count]);
                    glReadPixels(0, 0, kTextureSize, kTextureSize, read_format, read_type, buf.get());
                    EXPECT_EQ(glGetError(), GLenum(GL_NO_ERROR));
                    for (uint32_t jj = 0; jj < kTextureSize * kTextureSize; ++jj) {
                        for (uint32_t kk = 0; kk < test_formats[ii].comp_count; ++kk) {
                            EXPECT_LE(std::abs(HalfToFloat32(buf[jj * read_comp_count + kk]) - kDrawColor[kk]), std::abs(kDrawColor[kk] * kEpsilon));
                        }
                    }
                    break;
                }
                case GL_FLOAT: {
                    std::unique_ptr<GLfloat[]> buf(new GLfloat[kTextureSize * kTextureSize * read_comp_count]);
                    glReadPixels(0, 0, kTextureSize, kTextureSize, read_format, read_type, buf.get());
                    EXPECT_EQ(glGetError(), GLenum(GL_NO_ERROR));
                    for (uint32_t jj = 0; jj < kTextureSize * kTextureSize; ++jj) {
                        for (uint32_t kk = 0; kk < test_formats[ii].comp_count; ++kk) {
                            EXPECT_LE(std::abs(buf[jj * read_comp_count + kk] - kDrawColor[kk]), std::abs(kDrawColor[kk] * kEpsilon));
                        }
                    }
                    break;
                }
