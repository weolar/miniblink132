...
protected:
    ...
    {
        return GLTestHelper::HasExtension("GL_CHROMIUM_framebuffer_mixed_samples") && GLTestHelper::HasExtension("GL_OES_rgb8_rgba8");
    }
    ...
    {
        // clang-format on
        static const char* kVertexShaderSource = SHADER(attribute mediump vec4 position; void main() { gl_Position = position; });
        static const char* kFragmentShaderSource = SHADER(uniform mediump vec4 color; void main() { gl_FragColor = color; });
        // clang-format on
        ...
        if (fbo_type == MixedSampleFBO) {
            ...
            glRenderbufferStorageMultisampleCHROMIUM(GL_RENDERBUFFER, num_samples, GL_STENCIL_INDEX8, kWidth, kHeight);
            GLint param = 0;
