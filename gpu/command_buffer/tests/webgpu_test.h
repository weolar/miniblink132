...
public:
    struct Options {
        ...
        SharedMemoryLimits shared_memory_limits = SharedMemoryLimits::ForWebGPUContext();
...
protected:
    ...
    static std::map<std::pair<WGPUDevice, WGPUErrorType>, /* matched */ bool> s_expected_errors;
...
#define EXPECT_WEBGPU_ERROR(device, type, statement)                                                                                                           \
    do {                                                                                                                                                       \
        PollUntilIdle();                                                                                                                                       \
        auto it = s_expected_errors.insert({ std::make_pair(device.Get(), type), false });                                                                     \
        EXPECT_TRUE(it.second) << "Only one expectation per-device-per-type supported.";                                                                       \
        statement;                                                                                                                                             \
        PollUntilIdle();                                                                                                                                       \
        EXPECT_TRUE(it.first->second) << "Expected error (" << type << ") in `" #statement "`";                                                                \
