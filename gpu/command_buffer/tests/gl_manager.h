...
public:
    ...
    std::unique_ptr<gfx::GpuMemoryBuffer> CreateGpuMemoryBuffer(const gfx::Size& size, gfx::BufferFormat format);

    void Initialize(const Options& options);
    void InitializeWithWorkarounds(const Options& options, const GpuDriverBugWorkarounds& workarounds);
    void Destroy();

    bool IsInitialized() const
    {
        return gles2_implementation() != nullptr;
    }
    ...
    void set_use_native_pixmap_memory_buffers(bool use_native_pixmap_memory_buffers)
    {
...
private:
    ...
    void InitializeWithWorkaroundsImpl(const Options& options, const GpuDriverBugWorkarounds& workarounds);
