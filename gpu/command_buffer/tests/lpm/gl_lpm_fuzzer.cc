...
{
    std::vector<std::string> lines = base::SplitString(log, "\n", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
...
{
    GLuint program = gpu::GLTestHelper::LinkProgram(vertex_shader, fragment_shader);
    ...
    if (linked == 0) {
        if (CHECK_FOR_UNKNOWN_ERRORS) {
            ...
            auto buffer = std::make_unique<GLchar[]>(log_length);
            glGetProgramInfoLog(program, log_length, /*length=*/nullptr, buffer.get());
            base::StringPiece log(buffer.get(), log_length);
...
DEFINE_PROTO_FUZZER(const fuzzing::Session& session)
{
    ...
    GLuint vertex_shader_id = LoadShader(GL_VERTEX_SHADER, session.vertex_shader());
    GLuint fragment_shader_id = LoadShader(GL_FRAGMENT_SHADER, session.fragment_shader());
