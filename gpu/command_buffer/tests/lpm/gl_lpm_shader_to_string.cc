...
{
    return os << "vec3(" << vec3.first() << ", " << vec3.second() << ", " << vec3.third() << ")";
}

std::ostream& operator<<(std::ostream& os, const fuzzing::Vec4& vec4)
{
    return os << "vec4(" << vec4.first() << ", " << vec4.second() << ", " << vec4.third() << ", " << vec4.fourth() << ")";
}
...
std::ostream& operator<<(std::ostream& os, const fuzzing::Const& cons)
{
    ...
    case fuzzing::Const::kBvec2: {
        os << "bvec2(" << BoolToString(cons.bvec2().first()) << ", " << BoolToString(cons.bvec2().second()) << ")";
        break;
    }
    case fuzzing::Const::kBvec3: {
        os << "bvec3(" << BoolToString(cons.bvec3().first()) << ", " << BoolToString(cons.bvec3().second()) << ", " << BoolToString(cons.bvec3().third())
           << ")";
        break;
    }
    case fuzzing::Const::kBvec4: {
        os << "bvec4(" << BoolToString(cons.bvec4().first()) << ", " << BoolToString(cons.bvec4().second()) << ", " << BoolToString(cons.bvec4().third())
           << ", " << BoolToString(cons.bvec4().fourth()) << ")";
        break;
    }
    case fuzzing::Const::kIvec2: {
        os << "ivec2(" << base::NumberToString(cons.ivec2().first()) << ", " << base::NumberToString(cons.ivec2().second()) << ")";
        break;
    }
    case fuzzing::Const::kIvec3: {
        os << "ivec3(" << base::NumberToString(cons.ivec3().first()) << ", " << base::NumberToString(cons.ivec3().second()) << ", "
           << base::NumberToString(cons.ivec3().third()) << ")";
        break;
    }
    case fuzzing::Const::kIvec4: {
        os << "ivec4(" << base::NumberToString(cons.ivec4().first()) << ", " << base::NumberToString(cons.ivec4().second()) << ", "
           << base::NumberToString(cons.ivec4().third()) << ", " << base::NumberToString(cons.ivec4().fourth()) << ")";
        break;
    }
    case fuzzing::Const::kUvec2: {
        os << "uvec2(" << base::NumberToString(cons.uvec2().first()) << "u, " << base::NumberToString(cons.uvec2().second()) << "u)";
        break;
    }
    case fuzzing::Const::kUvec3: {
        os << "uvec3(" << base::NumberToString(cons.uvec3().first()) << "u, " << base::NumberToString(cons.uvec3().second()) << "u, "
           << base::NumberToString(cons.uvec3().third()) << "u)";
        break;
    }
    case fuzzing::Const::kUvec4: {
        os << "uvec4(" << base::NumberToString(cons.uvec4().first()) << "u, " << base::NumberToString(cons.uvec4().second()) << "u, "
           << base::NumberToString(cons.uvec4().third()) << "u, " << base::NumberToString(cons.uvec4().fourth()) << "u)";
        break;
    }
    ...
    case fuzzing::Const::kMat3: {
        os << "mat3(" << cons.mat3().first() << ", " << cons.mat3().second() << ", " << cons.mat3().third() << ")";
        break;
    }
    case fuzzing::Const::kMat4: {
        os << "mat4(" << cons.mat4().first() << ", " << cons.mat4().second() << ", " << cons.mat4().third() << ", " << cons.mat4().fourth() << ")";
        break;
    }
    ...
    case fuzzing::Const::kMat3X2: {
        os << "mat3x2(" << cons.mat3x2().first() << ", " << cons.mat3x2().second() << ", " << cons.mat3x2().third() << ")";
        break;
    }
    case fuzzing::Const::kMat3X3: {
        os << "mat3x3(" << cons.mat3x3().first() << ", " << cons.mat3x3().second() << ", " << cons.mat3x3().third() << ")";
        break;
    }
    case fuzzing::Const::kMat3X4: {
        os << "mat3x4(" << cons.mat3x4().first() << ", " << cons.mat3x4().second() << ", " << cons.mat3x4().third() << ")";
        break;
    }
    case fuzzing::Const::kMat4X2: {
        os << "mat4x2(" << cons.mat4x2().first() << ", " << cons.mat4x2().second() << ", " << cons.mat4x2().third() << ", " << cons.mat4x2().fourth() << ")";
        break;
    }
    case fuzzing::Const::kMat4X3: {
        os << "mat4x3(" << cons.mat4x3().first() << ", " << cons.mat4x3().second() << ", " << cons.mat4x3().third() << ", " << cons.mat4x3().fourth() << ")";
        break;
    }
    case fuzzing::Const::kMat4X4: {
        os << "mat4x4(" << cons.mat4x4().first() << ", " << cons.mat4x4().second() << ", " << cons.mat4x4().third() << ", " << cons.mat4x4().fourth() << ")";
        break;
    }
    }
    return os;
...
{
    return os << "(" << binary_op.left() << " " << GetBinaryOp(binary_op.op()) << " " << binary_op.right() << ")";
}
...
std::ostream& operator<<(std::ostream& os, const fuzzing::Assignment& assignment)
{
...
void GetDeclaration(std::ostream& os, const fuzzing::Declare& declare, bool is_global)
{
    ...
    os << GetType(declare.type(), /*void_ok=*/false) << " " << GetVar(declare.var());
    // Const must be initialized, other types cannot be
    if (declare.qualifier() == fuzzing::Declare::CONST_QUALIFIER || (declare.qualifier() == fuzzing::Declare::NO_QUALIFIER && declare.has_rvalue())) {
        os << " = " << declare.rvalue();
    }
    os << ";\n";
}
...
{
    os << GetType(function.type(), /*void_ok=*/true) << " " << GetFunctionName(function.function_name()) << "() {\n";
    os << function.block();
