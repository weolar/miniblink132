...
protected:
    ...
    void PrepareForDraw()
    {
        static const char* v_shader_str = SHADER0(attribute vec4 a_Position; void main() { gl_Position = a_Position; });
        static const char* f_shader_str = SHADER0(precision mediump float; uniform vec4 color; void main() { gl_FragColor = color; });
        ...
        glRenderbufferStorageMultisampleCHROMIUM(GL_RENDERBUFFER, num_samples, GL_RGBA8_OES, kWidth, kHeight);
        GLint param = 0;
        ...
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, sample_rb);
    ...
    void PrepareForVerify()
    {
        ...
        glClear(GL_COLOR_BUFFER_BIT);
        glBlitFramebufferCHROMIUM(0, 0, kWidth, kHeight, 0, 0, kWidth, kHeight, GL_COLOR_BUFFER_BIT, GL_NEAREST);
        glBindFramebuffer(GL_READ_FRAMEBUFFER, resolve_fbo_);
    }

    bool IsApplicable() const
    {
        return GLTestHelper::HasExtension("GL_EXT_multisample_compatibility") && GLTestHelper::HasExtension("GL_CHROMIUM_framebuffer_multisample")
            && GLTestHelper::HasExtension("GL_OES_rgb8_rgba8") && !GPUTestBotConfig::CurrentConfigMatches("AMD");
    }
    GLuint sample_fbo_;
    GLuint resolve_fbo_;
    GLuint color_loc_;
...
TEST_F(EXTMultisampleCompatibilityTest, DrawAlphaOneAndResolve)
{
    ...
    if (renderer.find("NVIDIA Tegra") != std::string::npos && version.find("OpenGL ES 3.2 NVIDIA 361.00") != std::string::npos)
        return;
#endif
