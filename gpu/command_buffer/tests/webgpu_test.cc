...
std::map<std::pair<WGPUDevice, WGPUErrorType>, /* matched */ bool> WebGPUTest::s_expected_errors = {};
...
bool WebGPUTest::WebGPUSharedImageSupported() const
{
    ...
    // and ChromeOS.
#if (BUILDFLAG(IS_MAC) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS) || BUILDFLAG(IS_WIN)) && BUILDFLAG(USE_DAWN)
...
void WebGPUTest::Initialize(const Options& options)
{
    ...
    gpu_preferences.use_passthrough_cmd_decoder = gles2::UsePassthroughCommandDecoder(base::CommandLine::ForCurrentProcess());
    ...
    gpu_preferences.texture_target_exception_list = gpu::CreateBufferUsageAndFormatExceptionList();

    gpu_service_holder_ = std::make_unique<viz::TestGpuServiceHolder>(gpu_preferences);
    ...
    ContextResult result = context_->Initialize(gpu_service_holder_->task_executor(), attributes, options.shared_memory_limits, image_factory);
    ASSERT_EQ(result, ContextResult::kSuccess) << "Context failed to initialize";

    cmd_helper_ = std::make_unique<webgpu::WebGPUCmdHelper>(context_->GetCommandBufferForTest());

    webgpu()->SetLostContextCallback(base::BindLambdaForTesting([]() { GTEST_FAIL() << "Context lost unexpectedly."; }));
    ...
    auto* callback = webgpu::BindWGPUOnceCallback(
        [](WebGPUTest* test, bool force_fallback_adapter, bool* done, WGPURequestAdapterStatus status, WGPUAdapter adapter, const char* message) {
    ...
    instance_.RequestAdapter(&ra_options, callback->UnboundCallback(), callback->AsUserdata());
...
void WebGPUTest::WaitForCompletion(wgpu::Device device)
{
    ...
    queue.OnSubmittedWorkDone(
        0u, [](WGPUQueueWorkDoneStatus, void* userdata) { *static_cast<bool*>(userdata) = true; }, &done);
...
void WebGPUTest::PollUntilIdle()
{
    ...
    gpu_service_holder_->ScheduleGpuMainTask(base::BindLambdaForTesting([&wait, decoder = GetDecoder()]() {
...
wgpu::Device WebGPUTest::GetNewDevice()
{
    ...
    auto* callback = webgpu::BindWGPUOnceCallback(
        [](wgpu::Device* device_out, bool* done, WGPURequestDeviceStatus status, WGPUDevice device, const char* message) {
    ...
    adapter_.RequestDevice(&device_desc, callback->UnboundCallback(), callback->AsUserdata());
    webgpu()->FlushCommands();
    while (!done) {
        base::PlatformThread::Sleep(TestTimeouts::tiny_timeout());
        RunPendingTasks();
    ...
    device.SetDeviceLostCallback(
        [](WGPUDeviceLostReason reason, const char* message, void*) {
            ...
            GTEST_FAIL() << "Unexpected device lost (" << reason << "): " << message;
        },
        nullptr);
    device.SetUncapturedErrorCallback(
        [](WGPUErrorType type, const char* message, void* userdata) {
            auto it = s_expected_errors.find(std::make_pair(static_cast<WGPUDevice>(userdata), type));
...
{
    auto ExpectSPIRVDisallowedError = [](WGPUErrorType type, const char* message, void* userdata) {
