...
TEST_F(DepthTextureTest, RenderTo)
{
    ...
    bool have_depth_stencil = GLTestHelper::HasExtension("GL_OES_packed_depth_stencil");

    static const char* v_shader_str = SHADER(attribute vec4 v_position; void main() { gl_Position = v_position; });
    static const char* f_shader_str = SHADER(precision mediump float; uniform sampler2D u_texture; uniform vec2 u_resolution; void main() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, kResolution, kResolution, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    ...
    for (size_t ii = 0; ii < std::size(format_types); ++ii) {
        ...
        glBindTexture(GL_TEXTURE_2D, depth_texture);
        glTexImage2D(GL_TEXTURE_2D, 0, format, kResolution, kResolution, 0, format, type, nullptr);

        glBindFramebuffer(GL_FRAMEBUFFER, fbo);
        GLenum status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
        EXPECT_EQ(static_cast<GLenum>(GL_FRAMEBUFFER_COMPLETE), status) << "iteration: " << ii;
        ...
        glReadPixels(0, 0, kResolution, kResolution, GL_RGBA, GL_UNSIGNED_BYTE, actual_pixels);
        ...
        for (GLint yy = 0; bad_count < 16 && yy < kResolution; ++yy) {
            for (GLint xx = 0; bad_count < 16 && xx < kResolution; ++xx) {
                ...
                if (xx > 0 && xx < kResolution - 1) {
                    EXPECT_GT(actual[0], left[0]) << "pixel at " << xx << ", " << yy << " actual[0] =" << static_cast<unsigned>(actual[0])
                                                  << " left[0] =" << static_cast<unsigned>(left[0]) << " actual =" << reinterpret_cast<const void*>(actual)
        ...
        EXPECT_GT(actual_pixels[(kResolution * kResolution - 1) * 4] - actual_pixels[0], 0xC0);
