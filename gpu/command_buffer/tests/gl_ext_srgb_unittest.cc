...
TEST_F(GLEXTSRGBTest, TexImageSRGBALPHAFormat)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB_ALPHA_EXT, kWidth, kHeight, 0, GL_SRGB_ALPHA_EXT, GL_UNSIGNED_BYTE, pixels);
    EXPECT_EQ(static_cast<GLenum>(GL_NO_ERROR), glGetError());

    memset(pixels, kSubImageColor[0], sizeof(pixels));
    glTexSubImage2D(GL_TEXTURE_2D, 0, kSubImageX, kSubImageY, kSubImageWidth, kSubImageHeight, GL_SRGB_ALPHA_EXT, GL_UNSIGNED_BYTE, pixels);
    EXPECT_EQ(static_cast<GLenum>(GL_NO_ERROR), glGetError());
    ...
    GLTestHelper::CheckPixels(kSubImageX, kSubImageY, kSubImageWidth, kSubImageHeight, 0, kSubImageColor, nullptr);
}
