...
class GpuOESEGLImageTest : public testing::Test, public gpu::GpuCommandBufferTestEGL {
protected:
    void SetUp() override
    {
...
// clang-format on
const char kVertexShader[] = SHADER(attribute vec4 a_position; varying vec2 v_texCoord; void main() {
    gl_Position = a_position;
    v_texCoord = vec2((a_position.x + 1.0) * 0.5, (a_position.y + 1.0) * 0.5);
});

const char* kFragmentShader
    = SHADER(precision mediump float; uniform sampler2D a_texture; varying vec2 v_texCoord; void main() { gl_FragColor = texture2D(a_texture, v_texCoord); });
// clang-format on
...
TEST_F(GpuOESEGLImageTest, EGLImageToTexture)
{
    ...
    scoped_refptr<gl::GLImageNativePixmap> image = CreateGLImageNativePixmap(format, size, pixels.get());
    EXPECT_TRUE(image);
    ...
    gl_.decoder()->SetLevelInfo(texture_id, 0 /* level */, image->GetInternalFormat(), size.width(), size.height(), 1 /* depth */, image->GetDataFormat(),
        image->GetDataType(), gfx::Rect(size));
    gl_.decoder()->BindImage(texture_id, GL_TEXTURE_2D, image.get(), true /* can_bind_to_sampler */);

    // Build program, buffers and draw the texture.
    GLuint vertex_shader = gpu::GLTestHelper::LoadShader(GL_VERTEX_SHADER, kVertexShader);
    GLuint fragment_shader = gpu::GLTestHelper::LoadShader(GL_FRAGMENT_SHADER, kFragmentShader);
    GLuint program = gpu::GLTestHelper::SetupProgram(vertex_shader, fragment_shader);
    ...
    GLuint vbo = gpu::GLTestHelper::SetupUnitQuad(glGetAttribLocation(program, "a_position"));
    ASSERT_NE(0u, vbo);
    ...
    gpu::GLTestHelper::CheckPixels(0, 0, kImageWidth, kImageHeight, 0, pixel, nullptr);
    EXPECT_TRUE(GL_NO_ERROR == glGetError());

    // Release the image.
    gl_.decoder()->BindImage(texture_id, GL_TEXTURE_2D, image.get(), false /* can_bind_to_sampler */);
