...
template <int factor, int index> float Weight(float /*dst*/[4], float src[4], float src1[4])
{
...
template <int RGBs, int RGBd, int As, int Ad> void BlendEquationFuncAdd(float dst[4], float src[4], float src1[4], uint8_t result[4])
{
    float r[4];
    ...
    for (int i = 0; i < 4; ++i) {
        result[i] = static_cast<uint8_t>(std::floor(base::clamp(r[i], 0.0f, 1.0f) * 255.0f));
    }
}
...
protected:
    ...
    {
        // clang-format on
        static const char* kVertexShader = SHADER(attribute vec4 position; void main() { gl_Position = position; });
        // clang-format on
    ...
    {
        GLuint vertex_shader = GLTestHelper::LoadShader(GL_VERTEX_SHADER, GetVertexShader());
        GLuint fragment_shader = GLTestHelper::LoadShader(GL_FRAGMENT_SHADER, fragment_shader_str);
    ...
    void DrawAndVerify()
    {
        ...
        // Verify.
        uint8_t color[4];
        BlendEquationFuncAdd<GL_SRC1_COLOR_EXT, GL_SRC_ALPHA, GL_ONE_MINUS_SRC1_COLOR_EXT, GL_ONE_MINUS_SRC1_ALPHA_EXT>(kDst, kSrc, kSrc1, color);

        EXPECT_TRUE(GLTestHelper::CheckPixels(kWidth / 4, (3 * kHeight) / 4, 1, 1, 1, color, nullptr));
        EXPECT_TRUE(GLTestHelper::CheckPixels(kWidth - 1, 0, 1, 1, 1, color, nullptr));
    }
...
TEST_P(EXTBlendFuncExtendedDrawTest, ESSL1FragColor)
{
    ...
    if (bot_config.LoadCurrentConfig(nullptr) && bot_config.Matches("linux amd")) {
        return;
    }

    // clang-format on
    static const char* kFragColorShader = BFE_SHADER(precision mediump float; uniform vec4 src; uniform vec4 src1; void main() {
        gl_FragColor = src;
...
TEST_P(EXTBlendFuncExtendedDrawTest, ESSL1FragData)
{
    ...
    gpu::GPUTestBotConfig bot_config;
    if (bot_config.LoadCurrentConfig(nullptr) && bot_config.Matches("linux intel")) {
        return;
    }
...
TEST_P(EXTBlendFuncExtendedES3DrawTest, ES3Getters)
{
  ...
  static const char* kFragColorShader =
    ...
    for (int i = 0; i < 2; ++i) {
        SCOPED_TRACE(testing::Message() << "Testing getters after link, iteration " << i);
...
TEST_P(EXTBlendFuncExtendedES3DrawTest, ES3GettersArray)
{
    ...
    // https://bugs.freedesktop.org/show_bug.cgi?id=96765
    gpu::GPUTestBotConfig bot_config;
    if (bot_config.LoadCurrentConfig(nullptr) && (bot_config.Matches("linux amd") || bot_config.Matches("linux intel"))) {
        return;
    }
  ...
  static const char* kFragColorShader =
    ...
    for (int testcase = 0; testcase < 4; ++testcase) {
        if (testcase == 0) {
            ...
            glBindFragDataLocationIndexedEXT(program_, kUnusedLocation, 0, "FragData[0]");
            glBindFragDataLocationIndexedEXT(program_, kFragData0Location, 0, "FragData");
            glBindFragDataLocationIndexedEXT(program_, kFragData1Location, 0, "FragData[1]");
        } else if (testcase == 3) {
            glBindFragDataLocationIndexedEXT(program_, kUnusedLocation, 0, "FragData");
            glBindFragDataLocationIndexedEXT(program_, kFragData0Location, 0, "FragData[0]");
            glBindFragDataLocationIndexedEXT(program_, kFragData1Location, 0, "FragData[1]");
        }
        ...
        EXPECT_EQ(kFragData0Location, glGetFragDataLocation(program_, "FragData[0]"));
        EXPECT_EQ(0, glGetFragDataIndexEXT(program_, "FragData[0]"));
        EXPECT_EQ(kFragData1Location, glGetFragDataLocation(program_, "FragData[1]"));
...
TEST_P(EXTBlendFuncExtendedES3DrawTest, ES3Conflicts)
{
  ...
  static const char* kFragColorShader =
    ...
    glBindFragDataLocationIndexedEXT(program_, kColorName0Location, 0, "FragData0");
    glBindFragDataLocationIndexedEXT(program_, kColorName0Location, 0, "FragData1");
    EXPECT_EQ(static_cast<GLenum>(GL_NO_ERROR), glGetError());
    EXPECT_FALSE(LinkProgram());

    glBindFragDataLocationIndexedEXT(program_, kColorName0Location, 1, "FragData0");
    glBindFragDataLocationIndexedEXT(program_, kColorName0Location, 1, "FragData1");
    EXPECT_EQ(static_cast<GLenum>(GL_NO_ERROR), glGetError());
    EXPECT_FALSE(LinkProgram());
...
INSTANTIATE_TEST_SUITE_P(TranslatorVariants, EXTBlendFuncExtendedDrawTest, ::testing::Bool());

INSTANTIATE_TEST_SUITE_P(TranslatorVariants, EXTBlendFuncExtendedES3DrawTest, ::testing::Bool());
