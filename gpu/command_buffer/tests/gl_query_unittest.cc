...
TEST_F(QueryTest, MultipleQueries)
{
    ...
    EXPECT_TRUE(GLTestHelper::HasExtension("GL_CHROMIUM_command_buffer_latency_query"));
    ...
    available = 0;
    result = 0;
    glGetQueryObjectuivEXT(commands_issue_query, GL_QUERY_RESULT_AVAILABLE_EXT, &available);
    ...
    result = 0;
    available = 0;
    glGetQueryObjectuivEXT(error_query, GL_QUERY_RESULT_AVAILABLE_EXT, &available);
    EXPECT_TRUE(available);
