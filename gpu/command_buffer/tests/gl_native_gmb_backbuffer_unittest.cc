...
TEST_F(GLNativeGMBTest, TestNativeGMBBackbufferWithDifferentConfigurations)
{
    ...
    if (bot_config.LoadCurrentConfig(nullptr) && bot_config.Matches("linux mac passthrough")) {
        return;
    }
    ...
    for (int has_alpha = 0; has_alpha <= 1; ++has_alpha) {
        for (int msaa = 0; msaa <= 1; ++msaa) {
            for (int preserve_backbuffer = 0; preserve_backbuffer <= 1; ++preserve_backbuffer) {
