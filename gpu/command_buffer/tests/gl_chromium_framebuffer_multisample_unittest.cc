...
{
    if (!(GLTestHelper::HasExtension("GL_CHROMIUM_framebuffer_multisample") && GLTestHelper::HasExtension("GL_OES_rgb8_rgba8"))) {
        return;
    }
    ...
    glRenderbufferStorageMultisampleCHROMIUM(GL_RENDERBUFFER, num_samples, GL_RGBA8_OES, width, height);
    GLint param = 0;
    ...
    glClear(GL_COLOR_BUFFER_BIT);
    glBlitFramebufferCHROMIUM(0, 0, width, height, 0, 0, width, height, GL_COLOR_BUFFER_BIT, GL_NEAREST);

    // Verify.
    const uint8_t green[] = { 0, 255, 0, 255 };
    const uint8_t black[] = { 0, 0, 0, 0 };
    glBindFramebuffer(GL_READ_FRAMEBUFFER, resolve_fbo);
    EXPECT_TRUE(GLTestHelper::CheckPixels(width / 4, (3 * height) / 4, 1, 1, 0, green, nullptr));
