...
class RequestExtensionCHROMIUMTest : public testing::TestWithParam<ContextType> {
protected:
    void SetUp() override
    {
...
TEST_P(RequestExtensionCHROMIUMTest, Basic)
{
    ...
    std::string requestable_extensions_string = reinterpret_cast<const char*>(glGetRequestableExtensionsCHROMIUM());
    std::vector<std::string> requestable
        = base::SplitString(requestable_extensions_string, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
    for (const auto& to_request : requestable) {
        ...
        size_t extensions_size_before_request
            = base::SplitString(extension_string, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY).size();
        ...
        extension_string = reinterpret_cast<const char*>(glGetString(GL_EXTENSIONS));
        std::vector<std::string> extensions = base::SplitString(extension_string, base::kWhitespaceASCII, base::TRIM_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
        std::set<std::string> extensions_from_string(extensions.begin(), extensions.end());

        if (GetParam() == CONTEXT_TYPE_WEBGL2 || GetParam() == CONTEXT_TYPE_OPENGLES3) {
            ...
            for (int i = 0; i < num_extensions; ++i) {
                extensions_from_stringi.insert(reinterpret_cast<const char*>(glGetStringi(GL_EXTENSIONS, i)));
            }
...
INSTANTIATE_TEST_SUITE_P(WithContextTypes, RequestExtensionCHROMIUMTest,
    ::testing::Values(CONTEXT_TYPE_WEBGL1, CONTEXT_TYPE_WEBGL2, CONTEXT_TYPE_OPENGLES2, CONTEXT_TYPE_OPENGLES3));
}
