...
TEST_F(GLTest, BasicFBO)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 16, 16, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels.get());
...
TEST_F(GLTest, SimpleShader)
{
    ...
    glClear(GL_COLOR_BUFFER_BIT);
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, 1, 1, 1, expected_clear, nullptr));
...
{
    scoped_refptr<gles2::FeatureInfo> features = new gles2::FeatureInfo(gl_.workarounds(), GpuFeatureInfo());
    ...
    EXPECT_EQ(caps.blend_equation_advanced_coherent, flags.blend_equation_advanced_coherent);
    ...
    EXPECT_EQ(caps.render_buffer_format_bgra8888, flags.ext_render_buffer_format_bgra8888);
    EXPECT_EQ(caps.occlusion_query_boolean, flags.occlusion_query_boolean);
}

TEST_F(GLTest, GetString)
{
    EXPECT_STREQ("OpenGL ES 2.0 Chromium", reinterpret_cast<const char*>(glGetString(GL_VERSION)));
    EXPECT_STREQ("OpenGL ES GLSL ES 1.0 Chromium", reinterpret_cast<const char*>(glGetString(GL_SHADING_LANGUAGE_VERSION)));
}
