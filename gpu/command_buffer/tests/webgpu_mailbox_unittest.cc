...
std::unique_ptr<testing::StrictMock<MockBufferMapCallback>> mock_buffer_map_callback;
...
std::unique_ptr<testing::StrictMock<MockUncapturedErrorCallback>> mock_device_error_callback;
void ToMockUncapturedErrorCallback(WGPUErrorType type, const char* message, void* userdata)
{
...
std::ostream& operator<<(std::ostream& os, const WebGPUMailboxTestParams& options)
{
...
class WebGPUMailboxTest : public WebGPUTest, public testing::WithParamInterface<WebGPUMailboxTestParams> {
public:
...
protected:
    void SetUp() override
    {
        ...
        mock_buffer_map_callback = std::make_unique<testing::StrictMock<MockBufferMapCallback>>();
        mock_device_error_callback = std::make_unique<testing::StrictMock<MockUncapturedErrorCallback>>();
    }
    ...
    template <typename T> static error::Error ExecuteCmd(webgpu::WebGPUDecoder* decoder, const T& cmd)
    {
        static_assert(T::kArgFlags == cmd::kFixed, "T::kArgFlags should equal cmd::kFixed");
        int entries_processed = 0;
        return decoder->DoCommands(1, (const void*)&cmd, ComputeNumEntries(sizeof(cmd)), &entries_processed);
    }

    template <typename T> static error::Error ExecuteImmediateCmd(webgpu::WebGPUDecoder* decoder, const T& cmd, size_t data_size)
    {
        static_assert(T::kArgFlags == cmd::kAtLeastN, "T::kArgFlags should equal cmd::kAtLeastN");
        int entries_processed = 0;
        return decoder->DoCommands(1, (const void*)&cmd, ComputeNumEntries(sizeof(cmd) + data_size), &entries_processed);
    }

    void InitializeTextureColor(wgpu::Device device, const Mailbox& mailbox, wgpu::Color clearValue)
    {
        gpu::webgpu::ReservedTexture reservation = webgpu()->ReserveTexture(device.Get());

        webgpu()->AssociateMailbox(reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation,
            WGPUTextureUsage_RenderAttachment, webgpu::WEBGPU_MAILBOX_NONE, reinterpret_cast<const GLbyte*>(&mailbox));
...
TEST_P(WebGPUMailboxTest, AssociateMailboxCmd)
{
    ...
    Mailbox mailbox = sii->CreateSharedImage(GetParam().format, { 1, 1 }, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType,
        SHARED_IMAGE_USAGE_WEBGPU, kNullSurfaceHandle);
    ...
    GetGpuServiceHolder()->ScheduleGpuMainTask(base::BindOnce(
        [](webgpu::WebGPUDecoder* decoder, webgpu::ReservedTexture reservation, gpu::Mailbox mailbox) {
            // Error case: device client id doesn't exist.
            {
                AssociateMailboxCmdStorage cmd;
                cmd.cmd.Init(reservation.deviceId + 1, reservation.deviceGeneration, reservation.id, reservation.generation, WGPUTextureUsage_TextureBinding,
                    webgpu::WEBGPU_MAILBOX_NONE, mailbox.name);
                EXPECT_EQ(error::kInvalidArguments, ExecuteImmediateCmd(decoder, cmd.cmd, sizeof(mailbox.name)));
            }

            // Error case: device generation is invalid.
            {
                AssociateMailboxCmdStorage cmd;
                cmd.cmd.Init(reservation.deviceId, reservation.deviceGeneration + 1, reservation.id, reservation.generation, WGPUTextureUsage_TextureBinding,
                    webgpu::WEBGPU_MAILBOX_NONE, mailbox.name);
                EXPECT_EQ(error::kInvalidArguments, ExecuteImmediateCmd(decoder, cmd.cmd, sizeof(mailbox.name)));
            }
            ...
            {
                AssociateMailboxCmdStorage cmd;
                cmd.cmd.Init(reservation.deviceId, reservation.deviceGeneration, reservation.id + 1, reservation.generation, WGPUTextureUsage_TextureBinding,
                    webgpu::WEBGPU_MAILBOX_NONE, mailbox.name);
                EXPECT_EQ(error::kInvalidArguments, ExecuteImmediateCmd(decoder, cmd.cmd, sizeof(mailbox.name)));
            }
            ...
            {
                AssociateMailboxCmdStorage cmd;
                cmd.cmd.Init(reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation, WGPUTextureUsage_Force32,
                    webgpu::WEBGPU_MAILBOX_NONE, mailbox.name);
                EXPECT_EQ(error::kInvalidArguments, ExecuteImmediateCmd(decoder, cmd.cmd, sizeof(mailbox.name)));
            }
            ...
            {
                AssociateMailboxCmdStorage cmd;
                cmd.cmd.Init(reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation, WGPUTextureUsage_TextureBinding,
                    webgpu::WEBGPU_MAILBOX_NONE, mailbox.name);
                EXPECT_EQ(error::kNoError, ExecuteImmediateCmd(decoder, cmd.cmd, sizeof(mailbox.name)));
            }
    ...
    GetGpuServiceHolder()->gpu_main_thread_task_runner()->RunsTasksInCurrentSequence();
}
    ...
    GetGpuServiceHolder()->ScheduleGpuMainTask(base::BindOnce(
        [](webgpu::WebGPUDecoder* decoder, webgpu::ReservedTexture reservation, gpu::Mailbox mailbox) {
            // Error case: invalid mailbox
            {
                ...
                EXPECT_EQ(error::kNoError, ExecuteImmediateCmd(decoder, cmd.cmd, sizeof(bad_mailbox.name)));
            }
        },
        GetDecoder(), reservation, mailbox));
...
TEST_P(WebGPUMailboxTest, WriteToMailboxThenReadFromIt)
{
    ...
    // Part 2: Read back the texture using Dawn
    {
        ...
        gpu::webgpu::ReservedTexture reservation = webgpu()->ReserveTexture(device_.Get());

        webgpu()->AssociateMailbox(reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation, WGPUTextureUsage_CopySrc,
        ...
        readback_buffer.MapAsync(wgpu::MapMode::Read, 0, 4, ToMockBufferMapCallback, nullptr);
        EXPECT_CALL(*mock_buffer_map_callback, Call(WGPUBufferMapAsyncStatus_Success, nullptr)).Times(1);
...
TEST_P(WebGPUMailboxTest, ReadUninitializedSharedImage)
{
    ...
    // to uncleared.
    webgpu()->AssociateMailbox(reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation, WGPUTextureUsage_CopySrc,
        webgpu::WEBGPU_MAILBOX_DISCARD, reinterpret_cast<const GLbyte*>(&mailbox));
    wgpu::Texture texture = wgpu::Texture::Acquire(reservation.texture);
    ...
    // Map the buffer and assert the pixel is the correct value.
    readback_buffer.MapAsync(wgpu::MapMode::Read, 0, buffer_desc.size, ToMockBufferMapCallback, nullptr);
    EXPECT_CALL(*mock_buffer_map_callback, Call(WGPUBufferMapAsyncStatus_Success, nullptr)).Times(1);

    WaitForCompletion(device_);

    const uint8_t* data = static_cast<const uint8_t*>(readback_buffer.GetConstMappedRange(0, buffer_desc.size));
...
TEST_P(WebGPUMailboxTest, ReadWritableUninitializedSharedImage)
{
    ...
    // to uncleared.
    webgpu()->AssociateMailbox(reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation,
        WGPUTextureUsage_CopySrc | WGPUTextureUsage_RenderAttachment, webgpu::WEBGPU_MAILBOX_DISCARD, reinterpret_cast<const GLbyte*>(&mailbox));
    wgpu::Texture texture = wgpu::Texture::Acquire(reservation.texture);
...
TEST_P(WebGPUMailboxTest, ErrorWhenUsingTextureAfterDissociate)
{
    ...
    EXPECT_CALL(*mock_device_error_callback, Call(WGPUErrorType_Validation, testing::_, testing::_)).Times(1);
    WaitForCompletion(device_);
}
...
TEST_P(WebGPUMailboxTest, UseA_UseB_DestroyA_DestroyB)
{
    ...
    SharedImageInterface* sii = GetSharedImageInterface();
    Mailbox mailbox_a = sii->CreateSharedImage(GetParam().format, { 1, 1 }, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType,
        SHARED_IMAGE_USAGE_WEBGPU, kNullSurfaceHandle);
    Mailbox mailbox_b = sii->CreateSharedImage(GetParam().format, { 1, 1 }, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType,
        SHARED_IMAGE_USAGE_WEBGPU, kNullSurfaceHandle);

    // Associate both mailboxes
    gpu::webgpu::ReservedTexture reservation_a = webgpu()->ReserveTexture(device_.Get());
    webgpu()->AssociateMailbox(reservation_a.deviceId, reservation_a.deviceGeneration, reservation_a.id, reservation_a.generation,
        WGPUTextureUsage_RenderAttachment, webgpu::WEBGPU_MAILBOX_NONE, reinterpret_cast<const GLbyte*>(&mailbox_a));

    gpu::webgpu::ReservedTexture reservation_b = webgpu()->ReserveTexture(device_.Get());
    webgpu()->AssociateMailbox(reservation_b.deviceId, reservation_b.deviceGeneration, reservation_b.id, reservation_b.generation,
        WGPUTextureUsage_RenderAttachment, webgpu::WEBGPU_MAILBOX_NONE, reinterpret_cast<const GLbyte*>(&mailbox_b));
...
TEST_P(WebGPUMailboxTest, AssociateOnTwoDevicesAtTheSameTime)
{
    ...
    // Associate both mailboxes
    gpu::webgpu::ReservedTexture reservation_a = webgpu()->ReserveTexture(device_a.Get());
    webgpu()->AssociateMailbox(reservation_a.deviceId, reservation_a.deviceGeneration, reservation_a.id, reservation_a.generation,
        WGPUTextureUsage_RenderAttachment, webgpu::WEBGPU_MAILBOX_NONE, reinterpret_cast<const GLbyte*>(&mailbox_a));

    gpu::webgpu::ReservedTexture reservation_b = webgpu()->ReserveTexture(device_b.Get());
    webgpu()->AssociateMailbox(reservation_b.deviceId, reservation_b.deviceGeneration, reservation_b.id, reservation_b.generation,
        WGPUTextureUsage_RenderAttachment, webgpu::WEBGPU_MAILBOX_NONE, reinterpret_cast<const GLbyte*>(&mailbox_b));
...
TEST_P(WebGPUMailboxTest, ReflectionOfDescriptor)
{
    ...
    gpu::webgpu::ReservedTexture reservation1 = webgpu()->ReserveTexture(device_.Get(), reinterpret_cast<const WGPUTextureDescriptor*>(&desc1));
    ...
    gpu::webgpu::ReservedTexture reservation2 = webgpu()->ReserveTexture(device_.Get(), reinterpret_cast<const WGPUTextureDescriptor*>(&desc2));
    ...
    // reflection.
    SharedImageInterface* sii = GetSharedImageInterface();
    Mailbox mailbox1 = sii->CreateSharedImage(GetParam().format, { 1, 1 }, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType,
        SHARED_IMAGE_USAGE_WEBGPU, kNullSurfaceHandle);
    Mailbox mailbox2 = sii->CreateSharedImage(GetParam().format, { 1, 1 }, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType,
        SHARED_IMAGE_USAGE_WEBGPU, kNullSurfaceHandle);
    webgpu()->AssociateMailbox(reservation1.deviceId, reservation1.deviceGeneration, reservation1.id, reservation1.generation,
        static_cast<WGPUTextureUsage>(desc1.usage), webgpu::WEBGPU_MAILBOX_NONE, reinterpret_cast<const GLbyte*>(&mailbox1));
    webgpu()->AssociateMailbox(reservation2.deviceId, reservation2.deviceGeneration, reservation2.id, reservation2.generation,
        static_cast<WGPUTextureUsage>(desc2.usage), webgpu::WEBGPU_MAILBOX_NONE, reinterpret_cast<const GLbyte*>(&mailbox2));
}
...
TEST_P(WebGPUMailboxTest, AssociateDissociateMailboxWhenNotCurrent)
{
    ...
    auto CreateAndMakeGLContextCurrent = [&](scoped_refptr<gl::GLContext>* gl_context_out, scoped_refptr<gl::GLSurface>* gl_surface_out) {
        GetGpuServiceHolder()->ScheduleGpuMainTask(base::BindOnce(
            [](scoped_refptr<gl::GLContext>* gl_context_out, scoped_refptr<gl::GLSurface>* gl_surface_out) {
                auto gl_surface = gl::init::CreateOffscreenGLSurface(gl::GetDefaultDisplay(), gfx::Size(4, 4));
                auto gl_context = gl::init::CreateGLContext(nullptr, gl_surface.get(), gl::GLContextAttribs());

                EXPECT_TRUE(gl_context->MakeCurrent(gl_surface.get())) << "Failed to make GL context current";
    ...
    webgpu()->AssociateMailbox(reservation.deviceId, reservation.deviceGeneration, reservation.id, reservation.generation, WGPUTextureUsage_RenderAttachment,
        webgpu::WEBGPU_MAILBOX_NONE, reinterpret_cast<const GLbyte*>(&mailbox));
    wgpu::Texture texture = wgpu::Texture::Acquire(reservation.texture);
    ...
    GetGpuServiceHolder()->ScheduleGpuMainTask(base::BindOnce([](scoped_refptr<gl::GLContext> gl_context1, scoped_refptr<gl::GLContext> gl_context2,
                                                                  scoped_refptr<gl::GLSurface> gl_surface1, scoped_refptr<gl::GLSurface> gl_surface2) {},
        std::move(gl_context1), std::move(gl_context2), std::move(gl_surface1), std::move(gl_surface2)));
}

INSTANTIATE_TEST_SUITE_P(, WebGPUMailboxTest, ::testing::ValuesIn(WebGPUMailboxTest::TestParams()), ::testing::PrintToStringParamName());
