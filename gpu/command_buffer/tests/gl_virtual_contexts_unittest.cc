...
class GLVirtualContextsTest : public testing::TestWithParam<GpuDriverBugWorkarounds> {
protected:
    ...
    {
        static const char* v_shader_str = SHADER(attribute vec4 a_position; attribute vec4 a_color; varying vec4 v_color; void main() {
            gl_Position = a_position;
            v_color = a_color;
        });

        static const char* f_shader_str = SHADER(precision mediump float; varying vec4 v_color; void main() { gl_FragColor = v_color; });
...
void SetupSimpleShader(const uint8_t* color)
{
    ...
    static const char* f_shader_str = SHADER(precision mediump float; uniform vec4 u_color; void main() { gl_FragColor = u_color; });
    ...
    glUniform4f(color_loc, color[0] / 255.0f, color[1] / 255.0f, color[2] / 255.0f, color[3] / 255.0f);
}
...
void TestDraw(int size)
{
    ...
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, size, size, 1, expected_clear, nullptr));
...
TEST_P(GLVirtualContextsTest, Basic)
{
    ...
    for (int ii = 0; ii < kNumTests; ++ii) {
        ...
        GLManager* gl_manager = test.manager;
        gl_manager->MakeCurrent();
        EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, test.size, test.size, 0, test.color, nullptr));
    }
...
TEST_P(GLVirtualContextsTest, VertexArrayObjectRestore)
{
    ...
    glDrawArrays(GL_TRIANGLES, 0, 6);
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, kSize1, kSize1, 0, kExpectedRed, nullptr));
    glFinish();
    GLTestHelper::CheckGLError("no errors", __LINE__);
    ...
    glDrawArrays(GL_TRIANGLES, 0, 6);
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, kSize2, kSize2, 0, kExpectedGreen, nullptr));
    glFinish();
    GLTestHelper::CheckGLError("no errors", __LINE__);
}
...
TEST_P(GLVirtualContextsTest, VirtualQueries)
{
    ...
    for (GLenum query_target : query_targets) {
        ...
        EXPECT_TRUE(GL_NO_ERROR == glGetError()) << "Virtualized Query " << query_target << " failed.";
        ...
        glGetQueryObjectuivEXT(query1, GL_QUERY_RESULT_AVAILABLE_EXT, &query1_available);
        ...
        glGetQueryObjectuivEXT(query2, GL_QUERY_RESULT_AVAILABLE_EXT, &query2_available);
...
TEST_P(GLVirtualContextsTest, Texture2DArrayAnd3DRestore)
{
    ...
    glGetTexParameteriv(GL_TEXTURE_2D_ARRAY, GL_TEXTURE_MIN_FILTER, &tex_2d_array_params);
...
INSTANTIATE_TEST_SUITE_P(WithWorkarounds, GLVirtualContextsTest, ::testing::ValuesIn(workarounds_cases));
