...
TEST_F(GLSurfaceTextureTest, SimpleTest)
{
    ...
    scoped_refptr<gl::SurfaceTexture> surface_texture(gl::SurfaceTexture::Create(texture));
    gfx::AcceleratedWidget window = surface_texture->CreateSurface();
    EXPECT_TRUE(window != nullptr);

    scoped_refptr<gl::GLSurface> gl_surface = gl::init::CreateViewGLSurface(gl::GetDefaultDisplayEGL(), window);
