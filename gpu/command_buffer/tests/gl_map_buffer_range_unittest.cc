...
protected:
    ...
    {
        static const char* v_shader_src = SHADER(attribute vec2 a_position; void main() { gl_Position = vec4(a_position, 0.0, 1.0); });
        static const char* f_shader_src = SHADER(precision mediump float; void main() { gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0); });
    ...
    {
        static const char* v_shader_src = "#version 300 es\n"
                                          "in vec2 a_position;\n"
                                          "out vec3 var0;\n"
                                          "out vec2 var1;\n"
                                          "void main() {\n"
                                          ...
                                          "}";
        static const char* f_shader_src = "#version 300 es\n"
...
TEST_F(ES3MapBufferRangeTest, DrawArraysAndInstanced)
{
    ...
    GLTestHelper::CheckGLError("no errors", __LINE__);
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, kCanvasSize, kCanvasSize, 1, kRedColor, nullptr));
    ...
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, kCanvasSize, kCanvasSize, 1, kBlackColor, nullptr));
...
TEST_F(ES3MapBufferRangeTest, DrawElementsAndInstanced)
{
    ...
    std::vector<GLuint> buffers = GLTestHelper::SetupIndexedUnitQuad(position_loc);
    GLTestHelper::CheckGLError("no errors", __LINE__);
    ...
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    glDrawElementsInstancedANGLE(GL_TRIANGLES, 6, GL_UNSIGNED_BYTE, 0, kPrimCount);
    GLTestHelper::CheckGLError("no errors", __LINE__);
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, kCanvasSize, kCanvasSize, 1, kRedColor, nullptr));
...
TEST_F(ES3MapBufferRangeTest, ReadPixels)
{
    ...
    glBufferData(GL_PIXEL_PACK_BUFFER, 4 * kCanvasSize * kCanvasSize, 0, GL_STATIC_READ);
...
TEST_F(ES3MapBufferRangeTest, TexImageAndSubImage2D)
{
    ...
    glBufferData(GL_PIXEL_UNPACK_BUFFER, 4 * kTextureSize * kTextureSize, 0, GL_STATIC_READ);

    GLuint tex = 0;
    glGenTextures(1, &tex);
    EXPECT_LT(0u, tex);
    glBindTexture(GL_TEXTURE_2D, tex);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, kTextureSize, kTextureSize, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
    GLTestHelper::CheckGLError("no errors", __LINE__);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, kTextureSize, kTextureSize, GL_RGBA, GL_UNSIGNED_BYTE, 0);
    GLTestHelper::CheckGLError("no errors", __LINE__);
...
TEST_F(ES3MapBufferRangeTest, TexImageAndSubImage3D)
{
    ...
    EXPECT_LT(0u, buffer);
    glBindBuffer(GL_PIXEL_UNPACK_BUFFER, buffer);
    glBufferData(GL_PIXEL_UNPACK_BUFFER, 4 * kTextureSize * kTextureSize * kTextureSize, 0, GL_STATIC_READ);

    GLuint tex = 0;
    glGenTextures(1, &tex);
    EXPECT_LT(0u, tex);
    glBindTexture(GL_TEXTURE_3D, tex);
    glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA8, kTextureSize, kTextureSize, kTextureSize, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
    GLTestHelper::CheckGLError("no errors", __LINE__);
    glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0, kTextureSize, kTextureSize, kTextureSize, GL_RGBA, GL_UNSIGNED_BYTE, 0);
    GLTestHelper::CheckGLError("no errors", __LINE__);
...
TEST_F(ES3MapBufferRangeTest, TransformFeedback)
{
    ...
    glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 0, transform_feedback_buffer[0]);
    glBufferData(GL_TRANSFORM_FEEDBACK_BUFFER, 3 * 6 * sizeof(float), 0, GL_STATIC_READ);
    glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, 1, transform_feedback_buffer[1]);
    glBufferData(GL_TRANSFORM_FEEDBACK_BUFFER, 2 * 6 * sizeof(float), 0, GL_STATIC_READ);
    GLTestHelper::CheckGLError("no errors", __LINE__);
...
TEST_F(ES3MapBufferRangeTest, CopyBufferSubData)
{
    ...
    const uint8_t* map_ptr = static_cast<uint8_t*>(glMapBufferRange(GL_ARRAY_BUFFER, 0, kSize, GL_MAP_READ_BIT));
    ...
    glCopyBufferSubData(GL_ARRAY_BUFFER, GL_ARRAY_BUFFER, kReadOffset, kWriteOffset, kCopySize);
    GLTestHelper::CheckGLError("no errors", __LINE__);

    // Verify the data is copied.
    map_ptr = static_cast<uint8_t*>(glMapBufferRange(GL_ARRAY_BUFFER, 0, kSize, GL_MAP_READ_BIT));
    ASSERT_NE(nullptr, map_ptr);
...
TEST_F(ES3MapBufferRangeTest, Delete)
{
    ...
    const GLuint* map_ptr_0 = static_cast<GLuint*>(glMapBufferRange(GL_COPY_READ_BUFFER, 0, kSize, GL_MAP_READ_BIT));
    ...
    const GLuint* map_ptr_1 = static_cast<GLuint*>(glMapBufferRange(GL_COPY_READ_BUFFER, 0, kSize, GL_MAP_READ_BIT));
    ASSERT_NE(nullptr, map_ptr_1);
    EXPECT_EQ(buffers[1], *map_ptr_1);
    glUnmapBuffer(GL_COPY_READ_BUFFER);

    const GLuint* map_ptr_2 = static_cast<GLuint*>(glMapBufferRange(GL_ARRAY_BUFFER, 0, kSize, GL_MAP_READ_BIT));
