...
protected:
    GLWebGLMultiDrawTest()
        : max_transfer_buffer_size_(16 * 1024)
    {
    ...
    void SetUp() override
    {
        ...
        options.shared_memory_limits.min_transfer_buffer_size = max_transfer_buffer_size_ / 2;
        options.shared_memory_limits.max_transfer_buffer_size = max_transfer_buffer_size_;
        options.shared_memory_limits.start_transfer_buffer_size = max_transfer_buffer_size_ / 2;
...
TEST_F(GLWebGLMultiDrawTest, MultiDrawLargerThanTransferBuffer)
{
    ...
    if (!GLTestHelper::HasExtension("GL_ANGLE_multi_draw")) {
        if (requestable_extensions_string.find("GL_ANGLE_multi_draw ") == std::string::npos) {
            return;
        }
    ...
    if (!GLTestHelper::HasExtension("GL_WEBGL_multi_draw")) {
        if (requestable_extensions_string.find("GL_WEBGL_multi_draw ") == std::string::npos) {
            return;
        }
    ...
    ASSERT_GT(firsts.size() * sizeof(GLint) + counts.size() * sizeof(GLsizei), max_transfer_buffer_size());

    glClearColor(0, 0, 0, 0);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    gles2_implementation()->MultiDrawArraysWEBGL(GL_TRIANGLES, firsts.data(), counts.data(), canvas_size() * canvas_size());
    glFlush();

    std::vector<std::array<uint8_t, 4>> pixels(canvas_size() * canvas_size());
    glReadPixels(0, 0, canvas_size(), canvas_size(), GL_RGBA, GL_UNSIGNED_BYTE, pixels.data());
    unsigned int expected = 0;
    for (const auto& pixel : pixels) {
        unsigned int id = ((pixel[0] << 24) + (pixel[1] << 16) + (pixel[2] << 8) + pixel[3]);
        EXPECT_EQ(id, expected);
        expected++;
    }

    GLTestHelper::CheckGLError("GLWebGLMultiDrawTest.MultiDrawLargerThanTransferBuffer", __LINE__);
}
