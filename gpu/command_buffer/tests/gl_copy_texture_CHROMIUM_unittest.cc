...
static const char* kSimpleVertexShaderES2 = "attribute vec2 a_position;\n"
...
std::string GetFragmentShaderSource(GLenum target, GLenum format, bool is_es3)
{
    ...
    source += "uniform mediump SamplerType u_texture;\n"
              "VARYING vec2 v_texCoord;\n"
              "void main() {\n";
    ...
    source += "  FRAGCOLOR = vec4(color) / ScaleValue;\n"
              "}\n";
    return source;
}
...
void getExpectedColorAndMask(GLenum src_internal_format, GLenum dest_internal_format, const uint8_t* color, uint8_t* expected_color, uint8_t* expected_mask)
{
    ...
    case GL_RGB10_A2: {
        ...
        setColor(color[0] >> 2, color[1] >> 2, color[2] >> 2, alpha_value, adjusted_color);
        break;
    }
    default:
    ...
    case GL_RGB32F:
    case GL_RGB8UI:
        setColor(adjusted_color[0], adjusted_color[1], adjusted_color[2], 0, expected_color);
    ...
    case GL_RGBA32F:
    case GL_RGBA8UI:
        setColor(adjusted_color[0], adjusted_color[1], adjusted_color[2], adjusted_color[3], expected_color);
    ...
    case GL_RGB10_A2: {
        ...
        setColor(adjusted_color[0], adjusted_color[1], adjusted_color[2], alpha_value, expected_color);
        ...
        setColor(1, 1, 1, src_internal_format != GL_LUMINANCE_ALPHA, expected_mask);
#else
        setColor(1, 1, 1, 1, expected_mask);
#endif
        break;
    ...
    case GL_RGB5_A1:
        setColor(adjusted_color[0], adjusted_color[1], adjusted_color[2], (adjusted_color[3] >> 7) ? 0xFF : 0x0, expected_color);
...
void getTextureDataAndExpectedRGBAs(FormatType src_format_type, FormatType dest_format_type, GLsizei width, GLsizei height, std::vector<uint8_t>* texture_data,
    std::vector<uint8_t>* expected_rgba_pixels, uint8_t* expected_mask)
{
    DCHECK(texture_data);
    DCHECK(expected_rgba_pixels);

    const uint32_t src_channel_count = gles2::GLES2Util::ElementsPerGroup(src_format_type.format, src_format_type.type);
    constexpr uint8_t color[4] = { 1u, 63u, 127u, 255u };
    uint8_t expected_color[4];
    constexpr uint8_t alt_color[4] = { 200u, 100u, 0u, 255u };

    getExpectedColorAndMask(src_format_type.internal_format, dest_format_type.internal_format, color, expected_color, expected_mask);
    ...
    if (src_format_type.type == GL_UNSIGNED_BYTE) {
        ...
        getExpectedColorAndMask(src_format_type.internal_format, dest_format_type.internal_format, alt_color, alt_expected_color, expected_mask);
            ...
            for (int x = 0; x < width; ++x) {
                ...
                for (uint32_t j = 0; j < 4; ++j) {
                    if (j < src_channel_count) {
                        texture_data->at((width * y + x) * src_channel_count + j) = (alt ? alt_color : color)[j];
                    }
                    expected_rgba_pixels->at((width * y + x) * 4 + j) = (alt ? alt_expected_color : expected_color)[j];
                }
            }
        }
        ...
        constexpr uint16_t color_16bit[4] = { color[0] << 8, color[1] << 8, color[2] << 8, color[3] << 8 };

        texture_data->resize(num_pixels * src_channel_count * sizeof(uint16_t));
        uint16_t* texture_data16 = reinterpret_cast<uint16_t*>(texture_data->data());
        int16_t flip_sign = -1;
        for (uint32_t i = 0; i < num_pixels * src_channel_count; i += src_channel_count) {
        ...
        constexpr uint32_t color_rgb10_a2 = ((color[3] & 0x3) << 30) + (color[2] << 20) + (color[1] << 10) + color[0];
        texture_data->resize(num_pixels * sizeof(uint32_t));
        uint32_t* texture_data32 = reinterpret_cast<uint32_t*>(texture_data->data());
...
class GLCopyTextureCHROMIUMTest : public testing::Test, public ::testing::WithParamInterface<CopyType> {
protected:
    void CreateAndBindDestinationTextureAndFBO(GLenum target)
    {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, target, textures_[1], 0);
    }
    ...
    void RunCopyTexture(
        GLenum dest_target, CopyType copy_type, FormatType src_format_type, GLint source_level, FormatType dest_format_type, GLint dest_level, bool is_es3)
    {
        std::vector<uint8_t> expected_pixels;
        std::vector<uint8_t> texture_data;
        uint8_t mask[4];
        getTextureDataAndExpectedRGBAs(src_format_type, dest_format_type, width_, height_, &texture_data, &expected_pixels, mask);
        ...
        glTexImage2D(source_target, 0, src_format_type.internal_format, width_ << source_level, height_ << source_level, 0, src_format_type.format,
            src_format_type.type, nullptr);
#endif
        glTexImage2D(source_target, source_level, src_format_type.internal_format, width_, height_, 0, src_format_type.format, src_format_type.type,
            texture_data.data());
        EXPECT_TRUE(glGetError() == GL_NO_ERROR);
        GLenum dest_binding_target = gles2::GLES2Util::GLFaceTargetToTextureTarget(dest_target);
        ...
        if (is_es3) {
            ...
            if (dest_binding_target == GL_TEXTURE_CUBE_MAP) {
                for (int i = 0; i < 6; ++i) {
                    ...
                    glTexImage2D(
                        face, dest_level, dest_format_type.internal_format, width_, height_, 0, dest_format_type.format, dest_format_type.type, nullptr);
                }
            }
#if BUILDFLAG(IS_MAC)
            // TODO(qiankun.miao@intel.com): Remove this workaround for Mac OSX, once
            // framebuffer complete bug is fixed on Mac OSX: crbug.com/678526.
            glTexImage2D(dest_target, 0, dest_format_type.internal_format, width_ << dest_level, height_ << dest_level, 0, dest_format_type.format,
                dest_format_type.type, nullptr);
#endif
        } else {
            glTexParameteri(dest_binding_target, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_NEAREST);
            // For cube map textures, make the texture cube complete.
            if (dest_binding_target == GL_TEXTURE_CUBE_MAP) {
                for (int i = 0; i < 6; ++i) {
                    GLenum face = GL_TEXTURE_CUBE_MAP_POSITIVE_X + i;
                    glTexImage2D(face, 0, dest_format_type.internal_format, width_ << dest_level, height_ << dest_level, 0, dest_format_type.format,
                        dest_format_type.type, nullptr);
                }
            } else {
                glTexImage2D(dest_target, 0, dest_format_type.internal_format, width_ << dest_level, height_ << dest_level, 0, dest_format_type.format,
        ...
        if (copy_type == TexImage) {
            glCopyTextureCHROMIUM(textures_[0], source_level, dest_target, textures_[1], dest_level, dest_format_type.internal_format, dest_format_type.type,
                false, false, false);
        } else {
            glBindTexture(dest_binding_target, textures_[1]);
            glTexImage2D(
                dest_target, dest_level, dest_format_type.internal_format, width_, height_, 0, dest_format_type.format, dest_format_type.type, nullptr);

            // Split large blits into two in order to expose bugs at lower levels.
            constexpr int sub_rect_width = 8;
            if (width_ <= sub_rect_width) {
                glCopySubTextureCHROMIUM(textures_[0], source_level, dest_target, textures_[1], dest_level, 0, 0, 0, 0, width_, height_, false, false, false);
            } else {
                glCopySubTextureCHROMIUM(
                    textures_[0], source_level, dest_target, textures_[1], dest_level, 0, 0, 0, 0, width_ - sub_rect_width, height_, false, false, false);
                glCopySubTextureCHROMIUM(textures_[0], source_level, dest_target, textures_[1], dest_level, width_ - sub_rect_width, 0, width_ - sub_rect_width,
                    0, sub_rect_width, height_, false, false, false);
            }
        }
        const GLenum last_error = glGetError();
        EXPECT_TRUE(last_error == GL_NO_ERROR) << gl::GLEnums::GetStringError(last_error);
        ...
        EXPECT_EQ(static_cast<GLenum>(GL_FRAMEBUFFER_COMPLETE), glCheckFramebufferStatus(GL_FRAMEBUFFER));
        glViewport(0, 0, width_, height_);

        glBindTexture(dest_binding_target, textures_[1]);
        std::string fragment_shader_source = GetFragmentShaderSource(dest_binding_target, dest_format_type.internal_format, is_es3);
        GLTestHelper::DrawTextureQuad(
            dest_target, is_es3 ? kSimpleVertexShaderES3 : kSimpleVertexShaderES2, fragment_shader_source.c_str(), "a_position", "u_texture", "u_face");
        EXPECT_TRUE(GL_NO_ERROR == glGetError());

        uint8_t tolerance = dest_format_type.internal_format == GL_RGBA4 ? 20 : 7;
        EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, width_, height_, tolerance, expected_pixels, mask))
            << " dest_target : " << gles2::GLES2Util::GetStringEnum(dest_target)
            << " src_internal_format: " << gles2::GLES2Util::GetStringEnum(src_format_type.internal_format) << " source_level: " << source_level
            << " dest_internal_format: " << gles2::GLES2Util::GetStringEnum(dest_format_type.internal_format) << " dest_level: " << dest_level;
...
protected:
    ...
    {
        DCHECK(!ShouldSkipTest());
        return !gl_.decoder()->GetFeatureInfo()->feature_flags().ext_texture_format_bgra8888;
    }
    ...
    {
        DCHECK(!ShouldSkipTest());
#if (BUILDFLAG(IS_MAC) || BUILDFLAG(IS_WIN) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)) && (defined(ARCH_CPU_X86) || defined(ARCH_CPU_X86_64))
        // Make sure it's tested; it is safe to assume that the flag is always true
        // on desktop.
        EXPECT_TRUE(gl_.decoder()->GetFeatureInfo()->feature_flags().ext_texture_norm16);
#endif
    ...
    {
        DCHECK(!ShouldSkipTest());
#if (BUILDFLAG(IS_MAC) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)) && (defined(ARCH_CPU_X86) || defined(ARCH_CPU_X86_64))
    ...
    {
        DCHECK(!ShouldSkipTest());
        const gl::GLVersionInfo& gl_version_info = gl_.decoder()->GetFeatureInfo()->gl_version_info();
        ...
        const bool supports_rgb10_a2
            = gl_version_info.IsAtLeastGL(3, 3) || gl_version_info.IsAtLeastGLES(3, 0) || GLTestHelper::HasExtension("GL_EXT_texture_type_2_10_10_10_REV");
...
INSTANTIATE_TEST_SUITE_P(CopyType, GLCopyTextureCHROMIUMTest, ::testing::ValuesIn(kCopyTypes));

INSTANTIATE_TEST_SUITE_P(CopyType, GLCopyTextureCHROMIUMES3Test, ::testing::ValuesIn(kCopyTypes));
...
TEST_P(GLCopyTextureCHROMIUMTest, Basic)
{
    ...
    if (copy_type == TexImage) {
        glCopyTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, GL_RGBA, GL_UNSIGNED_BYTE, false, false, false);
    } else {
        glBindTexture(GL_TEXTURE_2D, textures_[1]);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);

        glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 0, 0, 0, 0, 1, 1, false, false, false);
    }
    EXPECT_TRUE(glGetError() == GL_NO_ERROR);
...
TEST_P(GLCopyTextureCHROMIUMES3Test, FormatCombinations)
{
    ...
    if (gl_.gpu_preferences().use_passthrough_cmd_decoder) {
        ...
        LOG(INFO) << "Passthrough command decoder expected failure. Skipping test...";
        return;
    }
    ...
    for (auto src_format_type : src_format_types) {
        for (auto dest_format_type : dest_format_types) {
            if ((src_format_type.internal_format == GL_BGRA_EXT || src_format_type.internal_format == GL_BGRA8_EXT
                    || dest_format_type.internal_format == GL_BGRA_EXT || dest_format_type.internal_format == GL_BGRA8_EXT)
                && ShouldSkipBGRA()) {
                continue;
            }
            if (gles2::GLES2Util::IsFloatFormat(dest_format_type.internal_format) && ShouldSkipFloatFormat()) {
                continue;
            }
            if ((dest_format_type.internal_format == GL_SRGB_EXT || dest_format_type.internal_format == GL_SRGB_ALPHA_EXT) && ShouldSkipSRGBEXT()) {
                continue;
            }
            if ((src_format_type.internal_format == GL_R16_EXT || src_format_type.internal_format == GL_RG16_EXT
                    || src_format_type.internal_format == GL_RGBA16_EXT)
                && ShouldSkipNorm16())
                continue;
            if (src_format_type.internal_format == GL_RGB10_A2 && ShouldSkipRGB10A2())
                continue;
            if (src_format_type.internal_format == GL_RGBA16_EXT && dest_format_type.internal_format == GL_RGB10_A2 && ShouldSkipRGBA16ToRGB10A2())
                continue;

            RunCopyTexture(GL_TEXTURE_2D, copy_type, src_format_type, 0, dest_format_type, 0, true);
        }
    }
}
...
TEST_P(GLCopyTextureCHROMIUMTest, ImmutableTexture)
{
    ...
    for (auto src_internal_format : src_internal_formats) {
        for (auto dest_internal_format : dest_internal_formats) {
            ...
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 1, 1, ExtractFormatFrom(src_internal_format), GL_UNSIGNED_BYTE, pixels);
            ...
            if (copy_type == TexImage) {
                glCopyTextureCHROMIUM(
                    textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, ExtractFormatFrom(dest_internal_format), GL_UNSIGNED_BYTE, false, false, false);
...
{
    CopyType copy_type = GetParam();
    constexpr GLint src_formats[] = { GL_ALPHA, GL_RGB, GL_RGBA, GL_LUMINANCE, GL_LUMINANCE_ALPHA, GL_BGRA_EXT };
    ...
    for (const auto src_format : src_formats) {
        for (const auto dst_format : dest_formats) {
            CreateAndBindDestinationTextureAndFBO(GL_TEXTURE_2D);
            glBindTexture(GL_TEXTURE_2D, textures_[0]);
            glTexImage2D(GL_TEXTURE_2D, 0, src_format, 1, 1, 0, src_format, GL_UNSIGNED_BYTE, nullptr);
            EXPECT_TRUE(GL_NO_ERROR == glGetError());

            if (copy_type == TexImage) {
                glCopyTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, dst_format, GL_UNSIGNED_BYTE, false, false, false);
            } else {
                glBindTexture(GL_TEXTURE_2D, textures_[1]);
                glTexImage2D(GL_TEXTURE_2D, 0, dst_format, 1, 1, 0, dst_format, GL_UNSIGNED_BYTE, nullptr);
                EXPECT_TRUE(GL_NO_ERROR == glGetError());

                glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 0, 0, 0, 0, 1, 1, false, false, false);
            }

            EXPECT_TRUE(GL_NO_ERROR == glGetError()) << "src_format: " << gl::GLEnums::GetStringEnum(src_format)
...
TEST_P(GLCopyTextureCHROMIUMTest, InternalFormatRGBFloat)
{
    if (!GLTestHelper::HasExtension("GL_CHROMIUM_color_buffer_float_rgb")) {
        LOG(INFO) << "GL_CHROMIUM_color_buffer_float_rgb not supported. Skipping test...";
        return;
    }
    ...
    if (!gl_.decoder()->GetFeatureInfo()->feature_flags().enable_texture_float_linear) {
    ...
    RunCopyTexture(GL_TEXTURE_2D, copy_type, src_format_type, 0, dest_format_type, 0, false);
}
...
TEST_P(GLCopyTextureCHROMIUMTest, InternalFormatNotSupported)
{
    ...
    for (size_t dest_index = 0; dest_index < std::size(unsupported_dest_formats); dest_index++) {
        if (copy_type == TexImage) {
            glCopyTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, unsupported_dest_formats[dest_index], GL_UNSIGNED_BYTE, false, false, false);
            EXPECT_THAT(glGetError(), testing::AnyOf(GL_INVALID_VALUE, GL_INVALID_OPERATION)) << "dest_index:" << dest_index;
        } else {
            glBindTexture(GL_TEXTURE_2D, textures_[1]);
            glTexImage2D(GL_TEXTURE_2D, 0, unsupported_dest_formats[dest_index], 1, 1, 0, unsupported_dest_formats[dest_index], GL_UNSIGNED_BYTE, nullptr);
            // clear GL_INVALID_ENUM error from glTexImage2D on ES2 devices
            glGetError();
            glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 0, 0, 0, 0, 1, 1, false, false, false);
            EXPECT_TRUE(GL_INVALID_OPERATION == glGetError()) << "dest_index:" << dest_index;
        }
    }
    glDeleteTextures(2, textures_);
    glDeleteFramebuffers(1, &framebuffer_id_);
...
TEST_F(GLCopyTextureCHROMIUMTest, InternalFormatTypeCombinationNotSupported)
{
    ...
    for (size_t dest_index = 0; dest_index < std::size(unsupported_format_types); dest_index++) {
        glCopyTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, unsupported_format_types[dest_index].format,
...
TEST_P(GLCopyTextureCHROMIUMTest, CopyTextureLevel)
{
    ...
    for (GLint dest_level = 0; dest_level < 3; dest_level++) {
        for (auto dest_format_type : dest_format_types) {
            RunCopyTexture(GL_TEXTURE_2D, copy_type, src_format_type, source_level, dest_format_type, dest_level, false);
        }
    }
}
        ...
        for (GLint dest_level = 0; dest_level < 3; dest_level++) {
            for (auto dest_format_type : dest_format_types) {
                ...
                if (dest_level > 0)
                    continue;
#endif
                RunCopyTexture(GL_TEXTURE_2D, copy_type, src_format_type, source_level, dest_format_type, dest_level, true);
            }
        }
    }
}
...
TEST_P(GLCopyTextureCHROMIUMTest, CopyTextureCubeMap)
{
    ...
    GLenum dest_targets[] = { GL_TEXTURE_CUBE_MAP_POSITIVE_X, GL_TEXTURE_CUBE_MAP_NEGATIVE_X, GL_TEXTURE_CUBE_MAP_POSITIVE_Y, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y,
    ...
    for (auto dest_format_type : dest_format_types) {
        for (auto dest_target : dest_targets) {
            RunCopyTexture(dest_target, copy_type, src_format_type, source_level, dest_format_type, dest_level, false);
        }
    }
}
...
TEST_P(GLCopyTextureCHROMIUMES3Test, CopyTextureCubeMap)
{
    ...
    for (auto dest_format_type : dest_format_types) {
        for (auto dest_target : dest_targets) {
            RunCopyTexture(dest_target, copy_type, src_format_type, source_level, dest_format_type, dest_level, true);
        }
    }
}
...
{
    uint8_t pixels[4 * 4] = { 255u, 0u, 0u, 255u, 255u, 0u, 0u, 255u, 255u, 0u, 0u, 255u, 255u, 0u, 0u, 255u };
...
TEST_P(GLCopyTextureCHROMIUMES3Test, SamplerStatePreserved)
{
    ...
    if (copy_type == TexImage) {
        glCopyTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, GL_RGBA, GL_UNSIGNED_BYTE, false, false, true);
    } else {
        glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 0, 0, 0, 0, 1, 1, false, false, true);
    }
    EXPECT_TRUE(GL_NO_ERROR == glGetError());
...
TEST_P(GLCopyTextureCHROMIUMTest, FBOStatePreserved)
{
    ...
    EXPECT_TRUE(GL_FRAMEBUFFER_COMPLETE == glCheckFramebufferStatus(GL_FRAMEBUFFER));
    ...
    GLint fbo_params = 0;
    glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE, &fbo_params);
    EXPECT_EQ(GL_TEXTURE, fbo_params);

    fbo_params = 0;
    glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME, &fbo_params);
    EXPECT_EQ(texture_id, static_cast<GLuint>(fbo_params));

    fbo_params = 0;
    glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE, &fbo_params);
    EXPECT_EQ(GL_RENDERBUFFER, fbo_params);

    fbo_params = 0;
    glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME, &fbo_params);
...
TEST_P(GLCopyTextureCHROMIUMTest, UninitializedSource)
{
    ...
    if (copy_type == TexImage) {
        ...
        glBindTexture(GL_TEXTURE_2D, textures_[1]);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, kWidth, kHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
        glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 0, 0, 0, 0, kWidth, kHeight, false, false, false);
    }
    EXPECT_TRUE(GL_NO_ERROR == glGetError());
...
TEST_F(GLCopyTextureCHROMIUMTest, CopySubTextureDimension)
{
    ...
    glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 1, 1, 0, 0, 1, 1, false, false, false);
    EXPECT_TRUE(GL_NO_ERROR == glGetError());
    ...
    // x < 0
    glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 1, 1, -1, 0, 1, 1, false, false, false);
    EXPECT_TRUE(glGetError() == GL_INVALID_VALUE);

    // xoffset + width > dest_width
    glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 2, 2, 0, 0, 2, 2, false, false, false);
    EXPECT_TRUE(glGetError() == GL_INVALID_VALUE);

    // x + width > source_width
    glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 0, 0, 1, 1, 2, 2, false, false, false);
    EXPECT_TRUE(glGetError() == GL_INVALID_VALUE);
...
TEST_F(GLCopyTextureCHROMIUMTest, CopyTextureInvalidTextureIds)
{
    ...
    glCopyTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, 99993, 0, GL_RGBA, GL_UNSIGNED_BYTE, false, false, false);
    EXPECT_TRUE(GL_INVALID_VALUE == glGetError());

    glCopyTextureCHROMIUM(99994, 0, GL_TEXTURE_2D, textures_[1], 0, GL_RGBA, GL_UNSIGNED_BYTE, false, false, false);
    EXPECT_TRUE(GL_INVALID_VALUE == glGetError());

    glCopyTextureCHROMIUM(99995, 0, GL_TEXTURE_2D, 99996, 0, GL_RGBA, GL_UNSIGNED_BYTE, false, false, false);
    EXPECT_TRUE(GL_INVALID_VALUE == glGetError());
...
TEST_F(GLCopyTextureCHROMIUMTest, CopySubTextureInvalidTextureIds)
{
    ...
    glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, 99993, 0, 1, 1, 0, 0, 1, 1, false, false, false);
    EXPECT_TRUE(GL_INVALID_VALUE == glGetError());

    glCopySubTextureCHROMIUM(99994, 0, GL_TEXTURE_2D, textures_[1], 0, 1, 1, 0, 0, 1, 1, false, false, false);
    EXPECT_TRUE(GL_INVALID_VALUE == glGetError());

    glCopySubTextureCHROMIUM(99995, 0, GL_TEXTURE_2D, 99996, 0, 1, 1, 0, 0, 1, 1, false, false, false);
    EXPECT_TRUE(GL_INVALID_VALUE == glGetError());
...
{
    uint8_t rgba_pixels[4 * 4] = { 255u, 0u, 0u, 255u, 0u, 255u, 0u, 255u, 0u, 0u, 255u, 255u, 0u, 0u, 0u, 255u };
    CreateAndBindDestinationTextureAndFBO(GL_TEXTURE_2D);
    glBindTexture(GL_TEXTURE_2D, textures_[0]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 2, 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, rgba_pixels);

    uint8_t transparent_pixels[4 * 4] = { 0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u };
    glBindTexture(GL_TEXTURE_2D, textures_[1]);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 2, 2, 0, GL_RGBA, GL_UNSIGNED_BYTE, transparent_pixels);

    glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 1, 1, 0, 0, 1, 1, false, false, false);
    EXPECT_TRUE(glGetError() == GL_NO_ERROR);
    glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 1, 0, 1, 0, 1, 1, false, false, false);
    EXPECT_TRUE(glGetError() == GL_NO_ERROR);
    glCopySubTextureCHROMIUM(textures_[0], 0, GL_TEXTURE_2D, textures_[1], 0, 0, 1, 0, 1, 1, 1, false, false, false);
    EXPECT_TRUE(glGetError() == GL_NO_ERROR);
