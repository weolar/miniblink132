...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
class GpuMemoryBufferTestEGL : public testing::Test, public gpu::GpuCommandBufferTestEGL {
public:
...
TEST_P(GpuMemoryBufferTest, MapUnmap)
{
    ...
    if (libyuv_fourcc == static_cast<uint32_t>(libyuv::FOURCC_ANY)) {
        LOG(WARNING) << gfx::BufferFormatToString(buffer_format) << " not supported, skipping test";
        return;
    }

    std::unique_ptr<gfx::GpuMemoryBuffer> buffer(gl_.CreateGpuMemoryBuffer(gfx::Size(kImageWidth, kImageHeight), buffer_format));
    ...
    for (size_t plane = 0; plane < num_planes; ++plane) {
        gl::GLImageTestSupport::SetBufferDataToColor(
            kImageWidth, kImageHeight, buffer->stride(plane), plane, buffer_format, color_rgba, static_cast<uint8_t*>(buffer->memory(plane)));
    }
    buffer->Unmap();
    ...
    const int result = libyuv::ConvertToARGB(data, stride * kImageWidth, argb, kImageWidth /* dst_stride_argb */, 0 /* crop_x */, 0 /* crop_y */, kImageWidth,
        kImageHeight, kImageWidth /* rop_width */, kImageHeight /* crop_height */, libyuv::kRotate0, libyuv_fourcc);
    ...
    for (int y = 0; y < kImageHeight; ++y) {
        for (int x = 0; x < kImageWidth; ++x) {
            ...
            for (int c = 0; c < 4; ++c) {
                ...
                EXPECT_NEAR(expected, actual, max_error) << " at " << x << ", " << y << " channel " << c;
...
INSTANTIATE_TEST_SUITE_P(GpuMemoryBufferTests, GpuMemoryBufferTest,
    ::testing::Values(gfx::BufferFormat::R_8, gfx::BufferFormat::BGR_565, gfx::BufferFormat::RGBA_4444, gfx::BufferFormat::RGBA_8888,
        gfx::BufferFormat::RGBA_1010102, gfx::BufferFormat::BGRA_1010102, gfx::BufferFormat::BGRA_8888, gfx::BufferFormat::RGBA_F16, gfx::BufferFormat::YVU_420,
