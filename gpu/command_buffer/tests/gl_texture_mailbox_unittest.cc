...
TEST_F(GLTextureMailboxTest, ProduceAndConsumeTexture)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, &source_pixel);
...
TEST_F(GLTextureMailboxTest, ProduceAndConsumeTextureRGB)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 1, 1, 0, GL_RGB, GL_UNSIGNED_BYTE, &source_pixel);
...
TEST_F(GLTextureMailboxTest, SharedTextures)
{
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 1, 1, GL_RGBA, GL_UNSIGNED_BYTE, &source_pixel);
    EXPECT_EQ(static_cast<GLenum>(GL_NO_ERROR), glGetError());
    glFlush();
