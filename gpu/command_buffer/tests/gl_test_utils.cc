...
gl::GLDisplay* GLTestHelper::InitializeGL(gl::GLImplementation gl_impl)
{
    ...
    if (gl_impl == gl::GLImplementation::kGLImplementationNone) {
        ...
        if (!gl::init::InitializeStaticGLBindingsImplementation(gl::GLImplementationParts(gl_impl),
    ...
    gpu::GLManager::g_gpu_feature_info = gpu::ComputeGpuFeatureInfo(gpu_info, gpu::GpuPreferences(), base::CommandLine::ForCurrentProcess(),
        nullptr // needs_more_info
    );

    gl::init::SetDisabledExtensionsPlatform(gpu::GLManager::g_gpu_feature_info.disabled_extensions);
...
{
    return GLTestHelper::InitializeGL(gl::GLImplementation::kGLImplementationNone);
}
...
bool GLTestHelper::CheckGLError(const char* msg, int line)
{
    ...
    while ((error = glGetError()) != GL_NO_ERROR) {
        success = false;
        EXPECT_EQ(static_cast<GLenum>(GL_NO_ERROR), error) << "GL ERROR in " << msg << " at line " << line << " : " << error;
    }
    return success;
}
...
GLuint GLTestHelper::SetupProgram(GLuint vertex_shader, GLuint fragment_shader)
{
...
GLuint GLTestHelper::LoadProgram(const char* vertex_shader_source, const char* fragment_shader_source)
{
...
std::vector<GLuint> GLTestHelper::SetupIndexedUnitQuad(GLint position_location)
{
...
GLuint GLTestHelper::SetupColorsForUnitQuad(GLint location, const GLfloat color[4], GLenum usage)
{
    GLuint vbo = 0;
    glGenBuffers(1, &vbo);
    glBindBuffer(GL_ARRAY_BUFFER, vbo);
...
bool GLTestHelper::CheckPixels(GLint x, GLint y, GLsizei width, GLsizei height, GLint tolerance, const uint8_t* color, const uint8_t* mask)
{
...
bool GLTestHelper::CheckPixels(GLint x, GLint y, GLsizei width, GLsizei height, GLint tolerance, const std::vector<uint8_t>& expected, const uint8_t* mask)
{
        ...
        for (GLint xx = 0; xx < width; ++xx) {
            ...
            for (int jj = 0; jj < 4; ++jj) {
                ...
                if ((!mask || mask[jj]) && diff > tolerance) {
                    EXPECT_EQ(static_cast<int>(expected_component), static_cast<int>(actual)) << " at " << (xx + x) << ", " << (yy + y) << " channel " << jj;
                    ++bad_count;
...
bool GLTestHelper::SaveBackbufferAsBMP(const char* filename, int width, int height)
{
...
void GLTestHelper::DrawTextureQuad(
    const GLenum texture_target, const char* vertex_src, const char* fragment_src, const char* position_name, const char* sampler_name, const char* face_name)
{
    ...
    GLint face_loc = -1;
    if (gpu::gles2::GLES2Util::GLFaceTargetToTextureTarget(texture_target) == GL_TEXTURE_CUBE_MAP) {
...
bool GpuCommandBufferTestEGL::InitializeEGL(int width, int height)
{
    ...
    if (!(current_impl == gl::kGLImplementationEGLGLES2 || current_impl == gl::kGLImplementationEGLANGLE)) {
        gpu::GPUInfo gpu_info;
        gpu::CollectContextGraphicsInfo(&gpu_info);
        gpu::CollectBasicGraphicsInfo(base::CommandLine::ForCurrentProcess(), &gpu_info);
    ...
    DCHECK(current_impl == gl::kGLImplementationEGLGLES2 || current_impl == gl::kGLImplementationEGLANGLE);
    ...
    gl_extensions_ = gfx::MakeExtensionSet(gl::GetGLExtensionsFromCurrentContext());
    gl::GLVersionInfo gl_version_info(
        reinterpret_cast<const char*>(glGetString(GL_VERSION)), reinterpret_cast<const char*>(glGetString(GL_RENDERER)), gl_extensions_);
    bool result = gl::init::GetGLWindowSystemBindingInfo(gl_version_info, &window_system_binding_info_);
    DCHECK(result);

    egl_extensions_ = gfx::MakeExtensionSet(window_system_binding_info_.extensions);
...
#if BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS)
scoped_refptr<gl::GLImageNativePixmap> GpuCommandBufferTestEGL::CreateGLImageNativePixmap(gfx::BufferFormat format, gfx::Size size, uint8_t* pixels) const
{
...
gfx::NativePixmapHandle GpuCommandBufferTestEGL::CreateNativePixmapHandle(gfx::BufferFormat format, gfx::Size size, uint8_t* pixels)
{
    scoped_refptr<gl::GLImageNativePixmap> image = CreateGLImageNativePixmap(format, size, pixels);
    EXPECT_TRUE(image);
