...
protected:
    void SetUp() override
    {
        ...
        ContextResult result
            = gl_context_->Initialize(GetGpuServiceHolder()->task_executor(), attributes, option.shared_memory_limits, /*image_factory=*/nullptr);
...
// Windows is the only platform enabled passthrough in this test.
    ...
    {
        return gl_context_->GetImplementation();
    }
...
TEST_F(SharedImageGLBackingProduceDawnTest, Basic)
{
    ...
    Mailbox gl_mailbox = sii->CreateSharedImage(viz::ResourceFormat::RGBA_8888, { 1, 1 }, gfx::ColorSpace::CreateSRGB(), kTopLeft_GrSurfaceOrigin,
        kPremul_SkAlphaType, SHARED_IMAGE_USAGE_GLES2, kNullSurfaceHandle);
    SyncToken mailbox_produced_token = sii->GenVerifiedSyncToken();
    gl()->WaitSyncTokenCHROMIUM(mailbox_produced_token.GetConstData());
    GLuint texture = gl()->CreateAndTexStorage2DSharedImageCHROMIUM(gl_mailbox.name);

    gl()->BeginSharedImageAccessDirectCHROMIUM(texture, GL_SHARED_IMAGE_ACCESS_MODE_READWRITE_CHROMIUM);
    GLuint fbo = 0;
    gl()->GenFramebuffers(1, &fbo);
    gl()->BindFramebuffer(GL_FRAMEBUFFER, fbo);

    // Attach the texture to FBO.
    gl()->FramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, /* Hard code */
        texture, 0);
