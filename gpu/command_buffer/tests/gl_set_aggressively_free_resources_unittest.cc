...
TEST_F(SetAggressivelyFreeResourcesTest, FreeAllMemory_TransferBuffer)
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 4, 4, 0, GL_RGBA, GL_UNSIGNED_BYTE, kPixels);
...
TEST_F(SetAggressivelyFreeResourcesTest, FreeAllMemory_MappedMemory)
{
    ...
    void* data = glMapBufferSubDataCHROMIUM(GL_ARRAY_BUFFER, 0, sizeof(kData), GL_WRITE_ONLY);
    ASSERT_TRUE(data);
