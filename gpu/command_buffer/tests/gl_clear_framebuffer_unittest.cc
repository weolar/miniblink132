...
protected:
    ...
    bool IsApplicable()
    {
        ...
        return !GetParam() || !gl_.context()->GetVersionInfo()->is_desktop_core_profile;
    }
...
{
    static const char* v_shader_str = "attribute vec4 a_Position;\n"
...
void GLClearFramebufferTest::SetDrawColor(GLfloat r, GLfloat g, GLfloat b, GLfloat a)
{
...
INSTANTIATE_TEST_SUITE_P(GLClearFramebufferTestWithParam, GLClearFramebufferTest, ::testing::Values(true, false));
...
TEST_P(GLClearFramebufferTest, ClearColor)
{
    ...
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, 1, 1, 1 /* tolerance */, expected, nullptr));
}
...
TEST_P(GLClearFramebufferTest, ClearColorWithMask)
{
    ...
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, 1, 1, 0 /* tolerance */, expected, nullptr));
}
...
TEST_P(GLClearFramebufferTest, ClearColorWithScissor)
{
    ...
    if (bot_config.LoadCurrentConfig(nullptr) && bot_config.Matches("linux passthrough")) {
        return;
    }
...
TEST_P(GLClearFramebufferTest, ClearDepthStencil)
{
    ...
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, 1, 1, 0 /* tolerance */, kRed, nullptr));
    ...
    EXPECT_TRUE(GLTestHelper::CheckPixels(0, 0, 1, 1, 0 /* tolerance */, kGreen, nullptr));
...
{
    const char* extension_string = reinterpret_cast<const char*>(glGetString(GL_EXTENSIONS));
    gfx::ExtensionSet extensions = gfx::MakeExtensionSet(extension_string);
    bool has_separate_framebuffer = gfx::HasExtension(extensions, "GL_CHROMIUM_framebuffer_multisample");
    ...
    EXPECT_NE(glCheckFramebufferStatus(GL_READ_FRAMEBUFFER), static_cast<GLenum>(GL_FRAMEBUFFER_COMPLETE));
    ...
    EXPECT_EQ(glCheckFramebufferStatus(GL_READ_FRAMEBUFFER), static_cast<GLenum>(GL_FRAMEBUFFER_COMPLETE));
