...
RingBuffer::RingBuffer(scoped_refptr<gpu::Buffer> buffer, uint32_t alignment, Offset base_offset, CommandBufferHelper* helper)
    : helper_(helper)
    , buffer_(buffer)
...
void RingBuffer::FreeOldestBlock()
{
    ...
    DCHECK(block.state != IN_USE) << "attempt to allocate more than maximum memory";
...
void* RingBuffer::Alloc(uint32_t size)
{
    ...
    DCHECK_LE(size, size_) << "attempt to allocate more than maximum memory after rounding";
...
void RingBuffer::FreePendingToken(void* pointer, uint32_t token)
{
    ...
    for (Container::reverse_iterator it = blocks_.rbegin(); it != blocks_.rend(); ++it) {
        Block& block = *it;
        if (block.offset == offset) {
            DCHECK(block.state == IN_USE) << "block that corresponds to offset already freed";
            block.token = token;
...
void RingBuffer::DiscardBlock(void* pointer)
{
    ...
    for (Container::reverse_iterator it = blocks_.rbegin(); it != blocks_.rend(); ++it) {
        Block& block = *it;
        if (block.offset == offset) {
            DCHECK(block.state != PADDING) << "block that corresponds to offset already discarded";
...
uint32_t RingBuffer::GetLargestFreeSizeNoWaitingInternal()
{
    while (!blocks_.empty()) {
        Block& block = blocks_.front();
        if (!helper_->HasTokenPassed(block.token) || block.state == IN_USE)
            break;
        FreeOldestBlock();
    }
