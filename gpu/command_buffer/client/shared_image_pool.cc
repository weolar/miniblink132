...
gpu::ImageInfo GetImageInfo(scoped_refptr<gpu::ClientImage> image)
{
    ...
    return gpu::ImageInfo(shared_image->size(), shared_image->format(), shared_image->usage(), shared_image->color_space(), shared_image->surface_origin(),
...
SharedImagePoolBase::SharedImagePoolBase(const ImageInfo& image_info, const scoped_refptr<SharedImageInterface> sii, std::optional<uint8_t> max_pool_size)
...
scoped_refptr<ClientSharedImage> SharedImagePoolBase::CreateSharedImageInternal()
{
    CHECK(sii_);
    if (image_info_.buffer_usage.has_value()) {
        ...
        // merged after that.
        return sii_->CreateSharedImage({ image_info_.format, image_info_.size, image_info_.color_space, image_info_.surface_origin, image_info_.alpha_type,
                                           image_info_.usage, "SharedImagePoolMappable" },
            gpu::kNullSurfaceHandle, image_info_.buffer_usage.value());
    } else {
        return sii_->CreateSharedImage({ image_info_.format, image_info_.size, image_info_.color_space, image_info_.surface_origin, image_info_.alpha_type,
                                           image_info_.usage, "SharedImagePool" },
...
void SharedImagePoolBase::ReleaseImageInternal(scoped_refptr<ClientImage> image)
{
    ...
    if (!max_pool_size_.has_value() || image_pool_.size() < max_pool_size_.value()) {
