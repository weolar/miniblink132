...
protected:
    ...
    void SetUp() override
    {
        ...
        program_info_manager_->CreateInfo(kClientProgramId);
        {
            ...
            program_ = program_info_manager_->GetProgramInfo(nullptr, kClientProgramId, ProgramInfoManager::kNone);
...
TEST_F(ProgramInfoManagerTest, UpdateES2)
{
    ...
    EXPECT_TRUE(program_->GetProgramiv(GL_ACTIVE_UNIFORM_MAX_LENGTH, &active_uniform_max_length));
    ...
    for (uint32_t ii = 0; ii < data.header.num_uniforms; ++ii) {
        ...
        EXPECT_LT(kNames[0].length(), static_cast<size_t>(active_uniform_max_length));
        EXPECT_EQ(kNames[ii], info->name);
        EXPECT_EQ(kNames[ii].back() == ']', info->is_array);
        EXPECT_EQ(data.uniforms[ii].size, static_cast<int32_t>(info->element_locations.size()));
...
TEST_F(ProgramInfoManagerTest, UpdateES3UniformBlocks)
{
    ...
    EXPECT_TRUE(program_->GetProgramiv(GL_ACTIVE_UNIFORM_BLOCKS, &uniform_block_count));
    EXPECT_EQ(data.header.num_uniform_blocks, static_cast<uint32_t>(uniform_block_count));
    GLint max_name_length = 0;
    EXPECT_TRUE(program_->GetProgramiv(GL_ACTIVE_UNIFORM_BLOCK_MAX_NAME_LENGTH, &max_name_length));
    for (uint32_t ii = 0; ii < data.header.num_uniform_blocks; ++ii) {
        ...
        EXPECT_EQ(data.entry[ii].active_uniforms, info->active_uniform_indices.size());
        for (uint32_t uu = 0; uu < data.entry[ii].active_uniforms; ++uu) {
            EXPECT_EQ(kIndices[ii][uu], info->active_uniform_indices[uu]);
        }
        EXPECT_EQ(data.entry[ii].referenced_by_vertex_shader, static_cast<GLboolean>(info->referenced_by_vertex_shader));
        EXPECT_EQ(data.entry[ii].referenced_by_fragment_shader, static_cast<GLboolean>(info->referenced_by_fragment_shader));
...
TEST_F(ProgramInfoManagerTest, UpdateES3TransformFeedbackVaryings)
{
    ...
    std::vector<int8_t> result(sizeof(data));
    memcpy(&result[0], &data, sizeof(data));
    EXPECT_FALSE(program_->IsCached(ProgramInfoManager::kES3TransformFeedbackVaryings));
    program_->UpdateES3TransformFeedbackVaryings(result);
    EXPECT_TRUE(program_->IsCached(ProgramInfoManager::kES3TransformFeedbackVaryings));

    GLint transform_feedback_buffer_mode = 0;
    EXPECT_TRUE(program_->GetProgramiv(GL_TRANSFORM_FEEDBACK_BUFFER_MODE, &transform_feedback_buffer_mode));
    EXPECT_EQ(data.header.transform_feedback_buffer_mode, static_cast<uint32_t>(transform_feedback_buffer_mode));
    GLint transform_feedback_varying_count = 0;
    EXPECT_TRUE(program_->GetProgramiv(GL_TRANSFORM_FEEDBACK_VARYINGS, &transform_feedback_varying_count));
    EXPECT_EQ(data.header.num_transform_feedback_varyings, static_cast<uint32_t>(transform_feedback_varying_count));
    GLint max_name_length = 0;
    EXPECT_TRUE(program_->GetProgramiv(GL_TRANSFORM_FEEDBACK_VARYING_MAX_LENGTH, &max_name_length));
    ...
    EXPECT_EQ(nullptr, program_->GetTransformFeedbackVarying(data.header.num_transform_feedback_varyings));
}
...
TEST_F(ProgramInfoManagerTest, GetUniformBlockIndexCached)
{
    ...
    EXPECT_EQ(0u, program_info_manager_->GetUniformBlockIndex(nullptr, kClientProgramId, data.name0));
    EXPECT_EQ(1u, program_info_manager_->GetUniformBlockIndex(nullptr, kClientProgramId, data.name1));
    EXPECT_EQ(GL_INVALID_INDEX, program_info_manager_->GetUniformBlockIndex(nullptr, kClientProgramId, "BadName"));
}
...
TEST_F(ProgramInfoManagerTest, GetActiveUniformBlockNameCached)
{
    ...
    GLsizei length = 0;
    EXPECT_EQ(true, program_info_manager_->GetActiveUniformBlockName(nullptr, kClientProgramId, 0, buf_size, &length, &buffer[0]));
    EXPECT_EQ(static_cast<GLsizei>(strlen(data.name0)), length);
    EXPECT_STREQ(data.name0, &buffer[0]);

    EXPECT_EQ(true, program_info_manager_->GetActiveUniformBlockName(nullptr, kClientProgramId, 1, buf_size, &length, &buffer[0]));
    EXPECT_EQ(static_cast<GLsizei>(strlen(data.name1)), length);
    EXPECT_STREQ(data.name1, &buffer[0]);

    // Test length == nullptr.
    EXPECT_EQ(true, program_info_manager_->GetActiveUniformBlockName(nullptr, kClientProgramId, 0, buf_size, nullptr, &buffer[0]));
    EXPECT_STREQ(data.name0, &buffer[0]);

    // Test buffer == nullptr.
    EXPECT_EQ(true, program_info_manager_->GetActiveUniformBlockName(nullptr, kClientProgramId, 0, buf_size, &length, nullptr));
    EXPECT_EQ(0, length);
...
TEST_F(ProgramInfoManagerTest, GetActiveUniformBlockivCached)
{
    ...
    for (uint32_t ii = 0; ii < data.header.num_uniform_blocks; ++ii) {
        ...
        GLint params[2];
        EXPECT_TRUE(program_info_manager_->GetActiveUniformBlockiv(nullptr, kClientProgramId, ii, GL_UNIFORM_BLOCK_BINDING, params));
        EXPECT_EQ(data.entry[ii].binding, static_cast<uint32_t>(params[0]));

        EXPECT_TRUE(program_info_manager_->GetActiveUniformBlockiv(nullptr, kClientProgramId, ii, GL_UNIFORM_BLOCK_DATA_SIZE, params));
        EXPECT_EQ(data.entry[ii].data_size, static_cast<uint32_t>(params[0]));

        EXPECT_TRUE(program_info_manager_->GetActiveUniformBlockiv(nullptr, kClientProgramId, ii, GL_UNIFORM_BLOCK_NAME_LENGTH, params));
        EXPECT_EQ(strlen(kName[ii]) + 1, static_cast<uint32_t>(params[0]));

        EXPECT_TRUE(program_info_manager_->GetActiveUniformBlockiv(nullptr, kClientProgramId, ii, GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS, params));
        EXPECT_EQ(data.entry[ii].active_uniforms, static_cast<uint32_t>(params[0]));

        EXPECT_TRUE(program_info_manager_->GetActiveUniformBlockiv(nullptr, kClientProgramId, ii, GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES, params));
        for (uint32_t uu = 0; uu < data.entry[ii].active_uniforms; ++uu) {
            EXPECT_EQ(kIndices[ii][uu], static_cast<uint32_t>(params[uu]));
        }

        EXPECT_TRUE(program_info_manager_->GetActiveUniformBlockiv(nullptr, kClientProgramId, ii, GL_UNIFORM_BLOCK_REFERENCED_BY_VERTEX_SHADER, params));
        EXPECT_EQ(data.entry[ii].referenced_by_vertex_shader, static_cast<uint32_t>(params[0]));

        EXPECT_TRUE(program_info_manager_->GetActiveUniformBlockiv(nullptr, kClientProgramId, ii, GL_UNIFORM_BLOCK_REFERENCED_BY_FRAGMENT_SHADER, params));
        EXPECT_EQ(data.entry[ii].referenced_by_fragment_shader, static_cast<uint32_t>(params[0]));
    }
}
...
TEST_F(ProgramInfoManagerTest, GetTransformFeedbackVaryingCached)
{
    ...
    for (uint32_t ii = 0; ii < data.header.num_transform_feedback_varyings; ++ii) {
        ...
        GLsizei size = 0;
        GLenum type = 0;
        EXPECT_EQ(true, program_info_manager_->GetTransformFeedbackVarying(nullptr, kClientProgramId, ii, buf_size, &length, &size, &type, &buffer[0]));
...
TEST_F(ProgramInfoManagerTest, GetUniformIndices)
{
    ...
    { // Original order.
        ...
        EXPECT_TRUE(program_info_manager_->GetUniformIndices(nullptr, kClientProgramId, kCount, kNames, indices));
...
TEST_F(ProgramInfoManagerTest, GetActiveUniformsivCached)
{
    ...
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(
        nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_BLOCK_INDEX, &block_index[0]));
    std::vector<GLint> offset(count);
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_OFFSET, &offset[0]));
    std::vector<GLint> array_stride(count);
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(
        nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_ARRAY_STRIDE, &array_stride[0]));
    std::vector<GLint> matrix_stride(count);
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(
        nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_MATRIX_STRIDE, &matrix_stride[0]));
    std::vector<GLint> is_row_major(count);
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(
        nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_IS_ROW_MAJOR, &is_row_major[0]));
    ...
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_SIZE, &size[0]));
    std::vector<GLint> type(count);
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_TYPE, &type[0]));
    std::vector<GLint> name_length(count);
    EXPECT_TRUE(program_info_manager_->GetActiveUniformsiv(
        nullptr, kClientProgramId, static_cast<GLsizei>(count), &indices[0], GL_UNIFORM_NAME_LENGTH, &name_length[0]));

    for (uint32_t ii = 0; ii < count; ++ii) {
        EXPECT_EQ(data_es2.uniforms[ii].size, size[ii]);
        EXPECT_EQ(data_es2.uniforms[ii].type, static_cast<uint32_t>(type[ii]));
        EXPECT_EQ(data_es2.uniforms[ii].name_length + 1, static_cast<uint32_t>(name_length[ii]));
    }
}
