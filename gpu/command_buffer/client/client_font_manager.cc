...
ClientFontManager::ClientFontManager(Client* client, CommandBuffer* command_buffer)
    : client_(client)
...
{
    auto client_handle = client_discardable_manager_.CreateHandle(command_buffer_);
...
void ClientFontManager::Serialize()
{
    ...
    const uint32_t num_handles_created = last_allocated_handle_id_ - last_serialized_handle_id_;
    if (strike_data.size() == 0u && num_handles_created == 0u && locked_handles_.size() == 0u) {
    ...
    checked_bytes_required += sizeof(uint32_t) + alignof(uint32_t) + alignof(SerializableSkiaHandle);
    checked_bytes_required += base::CheckMul(num_handles_created, sizeof(SerializableSkiaHandle));

    // num of handles locked + DiscardableHandleIds.
    checked_bytes_required += sizeof(uint32_t) + alignof(uint32_t) + alignof(SkDiscardableHandleId);
    checked_bytes_required += base::CheckMul(locked_handles_.size(), sizeof(SkDiscardableHandleId));
    ...
    for (SkDiscardableHandleId handle_id = last_serialized_handle_id_ + 1; handle_id <= last_allocated_handle_id_; handle_id++) {
        auto it = discardable_handle_map_.find(handle_id);
        ...
        SerializableSkiaHandle handle(handle_id, client_handle.shm_id(), client_handle.byte_offset());
