...
struct GPU_EXPORT ImageInfo {
    ...
    ImageInfo(gfx::Size size, viz::SharedImageFormat format, SharedImageUsageSet usage, std::optional<gfx::BufferUsage> buffer_usage = std::nullopt)
        : size(size)
        , format(format)
        , usage(usage)
    ...
    ImageInfo(gfx::Size size, viz::SharedImageFormat format, SharedImageUsageSet usage, gfx::ColorSpace color_space, GrSurfaceOrigin surface_origin,
        SkAlphaType alpha_type, std::optional<gfx::BufferUsage> buffer_usage = std::nullopt)
        : size(size)
        , format(format)
        , usage(usage)
    ...
    {
        return size == other.size && format == other.format && usage == other.usage && color_space == other.color_space
            && surface_origin == other.surface_origin && alpha_type == other.alpha_type && buffer_usage == other.buffer_usage;
    }
};
...
protected:
    SharedImagePoolBase(const ImageInfo& image_info, const scoped_refptr<SharedImageInterface> sii, std::optional<uint8_t> max_pool_size);
...
template <typename ClientImageType = ClientImage> class GPU_EXPORT SharedImagePool : public SharedImagePoolBase {
public:
    static std::unique_ptr<SharedImagePool<ClientImageType>> Create(
        const ImageInfo& image_info, const scoped_refptr<SharedImageInterface> sii, std::optional<uint8_t> max_pool_size = std::nullopt)
    {
        CHECK(sii);
        return base::WrapUnique<SharedImagePool<ClientImageType>>(new SharedImagePool(image_info, std::move(sii), std::move(max_pool_size)));
    }
...
private:
    SharedImagePool(const ImageInfo& image_info, scoped_refptr<SharedImageInterface> sii, std::optional<uint8_t> max_pool_size)
        : SharedImagePoolBase(image_info, std::move(sii), std::move(max_pool_size))
    {
    }

    base::WeakPtrFactory<SharedImagePool<ClientImageType>> weak_ptr_factory_ { this };
};
