...
void* ClientTransferCache::MapEntry(MappedMemoryManager* mapped_memory, uint32_t size)
{
    DCHECK(!mapped_ptr_);
...
void* ClientTransferCache::MapTransferBufferEntry(TransferBufferInterface* transfer_buffer, uint32_t size)
{
    DCHECK(!mapped_ptr_);
    DCHECK(!transfer_buffer_ptr_);
    transfer_buffer_ptr_.emplace(size, client_->cmd_buffer_helper(), transfer_buffer);
...
void ClientTransferCache::UnmapAndCreateEntry(uint32_t type, uint32_t id)
{
    ...
    if (mapped_ptr_) {
        ...
        client_->IssueCreateTransferCacheEntry(
            type, id, handle.shm_id(), handle.byte_offset(), mapped_ptr_->shm_id(), mapped_ptr_->offset(), mapped_ptr_->size());
        mapped_ptr_ = std::nullopt;
    } else {
        DCHECK(!mapped_ptr_);
        client_->IssueCreateTransferCacheEntry(
            type, id, handle.shm_id(), handle.byte_offset(), transfer_buffer_ptr_->shm_id(), transfer_buffer_ptr_->offset(), transfer_buffer_ptr_->size());
        transfer_buffer_ptr_ = std::nullopt;
    }
}

void ClientTransferCache::AddTransferCacheEntry(uint32_t type, uint32_t id, uint32_t shm_id, uint32_t shm_offset, uint32_t size)
{
    DCHECK(!mapped_ptr_);
    EntryKey key(type, id);
    ...
    client_->IssueCreateTransferCacheEntry(type, id, handle.shm_id(), handle.byte_offset(), shm_id, shm_offset, size);
}

void ClientTransferCache::StartTransferCacheEntry(uint32_t type, uint32_t id, base::OnceCallback<void(ClientDiscardableHandle)> create_entry_cb)
{
    DCHECK(!mapped_ptr_);
    EntryKey key(type, id);
...
ClientDiscardableHandle ClientTransferCache::CreateDiscardableHandle(const EntryKey& key)
{
    ClientDiscardableHandle::Id discardable_handle_id = discardable_manager_.CreateHandle(client_->command_buffer());
    ...
    ClientDiscardableHandle handle = discardable_manager_.GetHandle(discardable_handle_id);
...
void ClientTransferCache::UnlockEntries(const std::vector<std::pair<uint32_t, uint32_t>>& entries)
{
    base::AutoLock hold(lock_);
...
ClientDiscardableHandle::Id ClientTransferCache::FindDiscardableHandleId(const EntryKey& key)
{
