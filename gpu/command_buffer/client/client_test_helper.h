...
public:
    ...
    scoped_refptr<gpu::Buffer> CreateTransferBufferHelper(uint32_t size, int32_t* id);
...
class MockClientCommandBuffer : public CommandBuffer, public FakeCommandBufferServiceBase {
public:
    ...
    int32_t GetServicePutOffset()
    {
        return put_offset_;
    }

    void SetTokenForSetGetBuffer(int32_t token)
    {
        token_ = token;
    }
...
public:
    ...
    MOCK_METHOD3(CreateImage, int32_t(ClientBuffer buffer, size_t width, size_t height));
    MOCK_METHOD1(DestroyImage, void(int32_t id));

    // Workaround for move-only args in GMock.
    MOCK_METHOD2(DoSignalQuery, void(uint32_t query, base::OnceClosure* callback));
    ...
    // Workaround for move-only args in GMock.
    MOCK_METHOD2(DoSignalSyncToken, void(const SyncToken& sync_token, base::OnceClosure* callback));
    ...
    MOCK_METHOD2(CreateGpuFence, void(uint32_t gpu_fence_id, ClientGpuFence source));
