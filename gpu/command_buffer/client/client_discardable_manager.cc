...
{
    if (free_ranges_.size() != 1 || free_ranges_.begin()->start != 0 || free_ranges_.begin()->end != element_count_)
        return true;
...
ClientDiscardableHandle::Id GetNextHandleId()
{
    ...
    // invalid.
    return ClientDiscardableHandle::Id::FromUnsafeValue(g_next_handle_id.GetNext() + 1);
}

} // namespace

struct ClientDiscardableManager::Allocation {
    Allocation(uint32_t element_count)
        : free_offsets(element_count)
    {
    }
...
ClientDiscardableHandle::Id ClientDiscardableManager::CreateHandle(CommandBuffer* command_buffer)
{
    scoped_refptr<Buffer> buffer;
    int32_t shm_id;
    uint32_t offset = 0;
...
bool ClientDiscardableManager::LockHandle(ClientDiscardableHandle::Id handle_id)
{
...
void ClientDiscardableManager::FreeHandle(ClientDiscardableHandle::Id handle_id)
{
    auto found = handles_.find(handle_id);
    if (found == handles_.end())
        return;
...
bool ClientDiscardableManager::HandleIsValid(ClientDiscardableHandle::Id handle_id) const
{
    return base::Contains(handles_, handle_id);
}

ClientDiscardableHandle ClientDiscardableManager::GetHandle(ClientDiscardableHandle::Id handle_id)
{
    auto found = handles_.find(handle_id);
    if (found == handles_.end())
...
bool ClientDiscardableManager::HandleIsDeleted(ClientDiscardableHandle::Id handle_id)
{
    auto found = handles_.find(handle_id);
    if (found == handles_.end())
        return true;
...
bool ClientDiscardableManager::HandleIsDeletedForTracing(ClientDiscardableHandle::Id handle_id) const
{
    auto found = handles_.find(handle_id);
    if (found == handles_.end())
        return true;
...
bool ClientDiscardableManager::FindAllocation(CommandBuffer* command_buffer, scoped_refptr<Buffer>* buffer, int32_t* shm_id, uint32_t* offset)
{
    ...
    allocation->buffer = command_buffer->CreateTransferBuffer(base::checked_cast<uint32_t>(allocation_size_), &allocation->shm_id);
...
bool ClientDiscardableManager::FindExistingAllocation(CommandBuffer* command_buffer, scoped_refptr<Buffer>* buffer, int32_t* shm_id, uint32_t* offset)
{
...
void ClientDiscardableManager::ReturnAllocation(CommandBuffer* command_buffer, const ClientDiscardableHandle& handle)
{
    for (auto it = allocations_.begin(); it != allocations_.end(); ++it) {
        ...
        allocation->free_offsets.ReturnFreeOffset(static_cast<uint32_t>(handle.byte_offset() / element_size_));
...
{
    while (pending_handles_.size() > 0 && pending_handles_.front().CanBeReUsed()) {
