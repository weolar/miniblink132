...
bool TransferBuffer::Initialize(
    unsigned int default_buffer_size, unsigned int result_size, unsigned int min_buffer_size, unsigned int max_buffer_size, unsigned int alignment)
{
...
void TransferBuffer::AllocateRingBuffer(unsigned int size)
{
    for (; size >= min_buffer_size_; size /= 2) {
        int32_t id = -1;
        scoped_refptr<gpu::Buffer> buffer = helper_->command_buffer()->CreateTransferBuffer(size, &id, alignment_);
        if (id != -1) {
            last_allocated_size_ = size;
            DCHECK(buffer.get());
            buffer_ = buffer;
            ring_buffer_ = std::make_unique<RingBuffer>(buffer_, alignment_, result_size_, helper_);
            buffer_id_ = id;
...
static unsigned int ComputePOTSize(unsigned int dimension)
{
    ...
    // undefined behavior.
    return (dimension == 0) ? 0 : 1 << std::min(static_cast<int>(sizeof(dimension) * CHAR_BIT - 1), base::bits::Log2Ceiling(dimension));
}
...
void TransferBuffer::ReallocateRingBuffer(unsigned int size, bool shrink)
{
    ...
    DCHECK_EQ(needed_buffer_size % alignment_, 0u) << "Buffer size is not a multiple of alignment_";
...
{
    while (!previous_ring_buffers_.empty() && previous_ring_buffers_.front()->GetUsedSize() == 0) {
...
void TransferBuffer::ShrinkOrExpandRingBufferIfNecessary(unsigned int size_to_allocate)
{
    // We should never attempt to shrink the buffer if someone has a result
    // pointer that hasn't been released.
    DCHECK(!outstanding_result_pointer_);
    ...
    high_water_mark_ = std::max(high_water_mark_, last_allocated_size_ - available_size + size_to_allocate + GetPreviousRingBufferUsedBytes());
    if (size_to_allocate > available_size) {
        ...
        bytes_since_last_shrink_ = 0;
        ReallocateRingBuffer(high_water_mark_ + high_water_mark_ / 4, true /* shrink */);
        high_water_mark_ = size_to_allocate + GetPreviousRingBufferUsedBytes();
    }
}

void* TransferBuffer::AllocUpTo(unsigned int size, unsigned int* size_allocated)
{
...
ScopedTransferBufferPtr::ScopedTransferBufferPtr(ScopedTransferBufferPtr&& other)
