...
public:
    ...
    RingBuffer(scoped_refptr<gpu::Buffer> buffer, uint32_t alignment, Offset base_offset, CommandBufferHelper* helper);
    ...
    {
        return size_ - GetLargestFreeSizeNoWaiting();
    }

    uint32_t NumUsedBlocks() const
    {
...
private:
    enum State { IN_USE, PADDING, FREE_PENDING_TOKEN };
