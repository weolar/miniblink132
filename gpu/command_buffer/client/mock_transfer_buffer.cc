...
MockTransferBuffer::MockTransferBuffer(CommandBuffer* command_buffer, unsigned int size, unsigned int result_size, unsigned int alignment, bool initialize_fail)
    ...
    , initialize_fail_(initialize_fail)
{
    ...
    for (int ii = 0; ii < kNumBuffers; ++ii) {
        buffers_[ii] = command_buffer_->CreateTransferBuffer(size_ + ii * alignment_, &buffer_ids_[ii]);
...
bool MockTransferBuffer::Initialize(
    unsigned int starting_buffer_size, unsigned int result_size, unsigned int /* min_buffer_size */, unsigned int /* max_buffer_size */, unsigned int alignment)
{
...
void* MockTransferBuffer::AllocUpTo(unsigned int size, unsigned int* size_allocated)
{
...
MockTransferBuffer::ExpectedMemoryInfo MockTransferBuffer::GetExpectedMemory(uint32_t size)
{
    ExpectedMemoryInfo mem;
    mem.offset = AllocateExpectedTransferBuffer(size);
    mem.id = GetExpectedTransferBufferId();
    mem.ptr = static_cast<uint8_t*>(GetExpectedTransferAddressFromOffset(mem.offset, size));
    return mem;
}

MockTransferBuffer::ExpectedMemoryInfo MockTransferBuffer::GetExpectedResultMemory(uint32_t size)
{
    ExpectedMemoryInfo mem;
...
void* MockTransferBuffer::GetExpectedTransferAddressFromOffset(uint32_t offset, uint32_t size)
{
