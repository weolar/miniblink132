...
protected:
    ...
    void SetUp() override
    {
        ...
        EXPECT_CALL(*api_mock_.get(), DoCommand(cmd::kSetToken, 1, _))
            .WillRepeatedly(DoAll(Invoke(api_mock_.get(), &AsyncAPIMock::SetToken), Return(error::kNoError)));
...
protected:
    ...
    void SetUp() override
    {
        ...
        base::UnsafeSharedMemoryRegion shared_memory_region = base::UnsafeSharedMemoryRegion::Create(kBufferSize);
        base::WritableSharedMemoryMapping shared_memory_mapping = shared_memory_region.Map();
        buffer_ = MakeBufferFromSharedMemory(std::move(shared_memory_region), std::move(shared_memory_mapping));
    ...
    {
        return static_cast<uint8_t*>(buffer_->memory());
    }
...
TEST_F(MemoryChunkTest, Basic)
{
    ...
    EXPECT_GE(kBufferSize, static_cast<uint8_t*>(pointer) - buffer_memory() + kSize);
...
protected:
    ...
    {
        MappedMemoryTestBase::SetUp();
        manager_ = std::make_unique<MappedMemoryManager>(helper_.get(), MappedMemoryManager::kNoLimit);
    }
