...
class DawnClientMemoryTransferService::ReadHandleImpl : public dawn::wire::client::MemoryTransferService::ReadHandle {
public:
    ReadHandleImpl(void* ptr, MemoryTransferHandle handle, DawnClientMemoryTransferService* service)
        : ReadHandle()
        , ptr_(ptr)
        , handle_(handle)
        , service_(service)
    ...
    bool DeserializeDataUpdate(const void* deserialize_pointer, size_t deserialize_size, size_t offset, size_t size) override
    {
...
class DawnClientMemoryTransferService::WriteHandleImpl : public dawn::wire::client::MemoryTransferService::WriteHandle {
public:
    WriteHandleImpl(void* ptr, MemoryTransferHandle handle, DawnClientMemoryTransferService* service)
        : WriteHandle()
        , ptr_(ptr)
        , handle_(handle)
        , service_(service)
    ...
    void SerializeDataUpdate(void* serialize_pointer, size_t offset, size_t size) override
    {
        // No data is serialized because we're using shared memory.
    }
...
DawnClientMemoryTransferService::DawnClientMemoryTransferService(MappedMemoryManager* mapped_memory)
...
dawn::wire::client::MemoryTransferService::ReadHandle* DawnClientMemoryTransferService::CreateReadHandle(size_t size)
{
...
dawn::wire::client::MemoryTransferService::WriteHandle* DawnClientMemoryTransferService::CreateWriteHandle(size_t size)
{
    MemoryTransferHandle handle = {};
    void* ptr = AllocateHandle(size, &handle);
    if (ptr == nullptr) {
...
void* DawnClientMemoryTransferService::AllocateHandle(size_t size, MemoryTransferHandle* handle)
{
    ...
    return mapped_memory_->Alloc(alloc_size, &handle->shm_id, &handle->shm_offset, TransferBufferAllocationOption::kReturnNullOnOOM);
}
...
{
    std::vector<raw_ptr<void, VectorExperimental>> to_free = std::move(free_blocks_);
    if (to_free.size() > 0) {
