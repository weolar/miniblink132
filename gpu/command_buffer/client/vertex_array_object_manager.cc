...
template <typename T> static T RoundUpToMultipleOf4(T size)
{
    return (size + 3) & ~3;
}
...
public:
    ...
    public:
        ...
        void SetInfo(GLuint buffer_id, GLint size, GLenum type, GLboolean normalized, GLsizei gl_stride, const GLvoid* pointer, GLboolean integer)
        {
    ...
    void SetAttribPointer(GLuint buffer_id, GLuint index, GLint size, GLenum type, GLboolean normalized, GLsizei stride, const void* ptr, GLboolean integer);
...
void VertexArrayObject::SetAttribPointer(
    GLuint buffer_id, GLuint index, GLint size, GLenum type, GLboolean normalized, GLsizei stride, const void* ptr, GLboolean integer)
{
    if (index < vertex_attribs_.size()) {
        VertexAttrib& attrib = vertex_attribs_[index];
...
bool VertexArrayObject::GetVertexAttrib(GLuint index, GLenum pname, uint32_t* param) const
{
...
bool VertexArrayObject::GetAttribPointer(GLuint index, GLenum pname, void** ptr) const
{
    const VertexAttrib* attrib = GetAttrib(index);
...
const VertexArrayObject::VertexAttrib* VertexArrayObject::GetAttrib(GLuint index) const
{
    if (index < vertex_attribs_.size()) {
...
VertexArrayObjectManager::VertexArrayObjectManager(
    GLuint max_vertex_attribs, GLuint array_buffer_id, GLuint element_array_buffer_id, bool support_client_side_arrays)
    ...
    , default_vertex_array_object_(std::make_unique<VertexArrayObject>(max_vertex_attribs))
...
void VertexArrayObjectManager::GenVertexArrays(GLsizei n, const GLuint* arrays)
{
    DCHECK_GE(n, 0);
    for (GLsizei i = 0; i < n; ++i) {
        std::pair<VertexArrayObjectMap::iterator, bool> result
            = vertex_array_objects_.insert(std::make_pair(arrays[i], std::make_unique<VertexArrayObject>(max_vertex_attribs_)));
        DCHECK(result.second);
    }
}

void VertexArrayObjectManager::DeleteVertexArrays(GLsizei n, const GLuint* arrays)
{
    DCHECK_GE(n, 0);
    for (GLsizei i = 0; i < n; ++i) {
        GLuint id = arrays[i];
...
bool VertexArrayObjectManager::GetVertexAttrib(GLuint index, GLenum pname, uint32_t* param)
{
    return bound_vertex_array_object_->GetVertexAttrib(index, pname, param);
}

bool VertexArrayObjectManager::GetAttribPointer(GLuint index, GLenum pname, void** ptr) const
{
    ...
    GLuint buffer_id, GLuint index, GLint size, GLenum type, GLboolean normalized, GLsizei stride, const void* ptr, GLboolean integer)
{
    ...
    bound_vertex_array_object_->SetAttribPointer(buffer_id, index, size, type, normalized, stride, ptr, integer);
    return true;
}
...
// bytes collected.
GLsizei VertexArrayObjectManager::CollectData(const void* data, GLsizei bytes_per_element, GLsizei real_stride, GLsizei num_elements)
{
...
{
    return support_client_side_arrays_ && bound_vertex_array_object_->HaveEnabledClientSideBuffers();
}

// Returns true if buffers were setup.
bool VertexArrayObjectManager::SetupSimulatedClientSideBuffers(
    const char* function_name, GLES2Implementation* gl, GLES2CmdHelper* gl_helper, GLsizei num_elements, GLsizei primcount, bool* simulated)
{
    *simulated = false;
    ...
    if (!IsDefaultVAOBound()) {
        gl->SetGLError(GL_INVALID_OPERATION, function_name, "client side arrays not allowed with vertex array object");
        return false;
    }
    *simulated = true;
    ...
    for (GLuint ii = 0; ii < vertex_attribs.size(); ++ii) {
        ...
        if (attrib.IsClientSide() && attrib.enabled()) {
            uint32_t bytes_per_element = GLES2Util::GetGroupSizeForBufferType(attrib.size(), attrib.type());
    ...
    if (!checked_total_size.AssignIfValid(&total_size)) {
        gl->SetGLError(GL_INVALID_OPERATION, function_name, "size overflow for client side arrays");
        return false;
    }
    ...
    for (GLuint ii = 0; ii < vertex_attribs.size(); ++ii) {
        ...
        if (attrib.IsClientSide() && attrib.enabled()) {
            ...
            GLsizei elements = (primcount && attrib.divisor() > 0) ? ((primcount - 1) / attrib.divisor() + 1) : num_elements;
            GLsizei bytes_collected = CollectData(attrib.pointer(), bytes_per_element, real_stride, elements);
            gl->BufferSubDataHelper(GL_ARRAY_BUFFER, array_buffer_offset_, bytes_collected, collection_buffer_.get());
            gl_helper->VertexAttribPointer(ii, attrib.size(), attrib.type(), attrib.normalized(), 0, array_buffer_offset_);
...
bool VertexArrayObjectManager::SetupSimulatedIndexAndClientSideBuffers(const char* function_name, GLES2Implementation* gl, GLES2CmdHelper* gl_helper,
    GLsizei count, GLenum type, GLsizei primcount, const void* indices, GLuint* offset, bool* simulated)
{
    *simulated = false;
    *offset = ToGLuint(indices);
    ...
    if (bound_vertex_array_object_->bound_element_array_buffer() == 0) {
        ...
        *offset = 0;
        ...
        case GL_UNSIGNED_INT: {
            ...
            for (GLsizei ii = 0; ii < count; ++ii) {
                ...
                if (src[ii] > max_glsizei) {
                    gl->SetGLError(GL_INVALID_OPERATION, function_name, "index too large.");
                    return false;
                }
        ...
        if (!base::CheckMul(bytes_per_element, count).AssignIfValid(&bytes_needed)) {
            gl->SetGLError(GL_INVALID_OPERATION, function_name, "size overflow for client side index arrays");
            return false;
        }
        if (bytes_needed > element_array_buffer_size_) {
            element_array_buffer_size_ = bytes_needed;
            gl->BufferDataHelper(GL_ELEMENT_ARRAY_BUFFER, bytes_needed, nullptr, GL_DYNAMIC_DRAW);
        }
        gl->BufferSubDataHelper(GL_ELEMENT_ARRAY_BUFFER, 0, bytes_needed, indices);
        ...
        num_elements = gl->GetMaxValueInBufferCHROMIUMHelper(bound_vertex_array_object_->bound_element_array_buffer(), count, type, ToGLuint(indices)) + 1;
    }

    bool simulated_client_side_buffers = false;
    SetupSimulatedClientSideBuffers(function_name, gl, gl_helper, num_elements, primcount, &simulated_client_side_buffers);
