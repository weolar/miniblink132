...
QuerySyncManager::Bucket::Bucket(QuerySync* sync_mem, int32_t shm_id, unsigned int shm_offset)
    : syncs(sync_mem)
    , shm_id(shm_id)
...
void QuerySyncManager::Bucket::FreePendingSyncs()
{
    std::erase_if(pending_syncs, [this](const PendingSync& pending) {
        ...
        if (base::subtle::Acquire_Load(&sync->process_count) == pending.submit_count) {
...
bool QuerySyncManager::Alloc(QuerySyncManager::QueryInfo* info)
{
    ...
    if (!bucket) {
        ...
        void* mem = mapped_memory_->Alloc(kSyncsPerBucket * sizeof(QuerySync), &shm_id, &shm_offset);
        if (!mem) {
            return false;
        }
...
void QuerySyncManager::Free(const QuerySyncManager::QueryInfo& info)
{
    ...
    if (base::subtle::Acquire_Load(&info.sync->process_count) != info.submit_count) {
        // When you delete a query you can't mark its memory as unused until it's
        // completed.
        info.bucket->pending_syncs.push_back(Bucket::PendingSync { index_in_bucket, info.submit_count });
    } else {
...
QueryTracker::Query::Query(GLuint id, GLenum target, const QuerySyncManager::QueryInfo& info)
    : id_(id)
    , target_(target)
    , info_(info)
...
void QueryTracker::Query::QueryCounter(QueryTrackerClient* client)
{
    ...
    flush_count_ = client->cmd_buffer_helper()->flush_generation();
    int32_t submit_count = NextSubmitCount();
    client->IssueQueryCounter(id(), target(), shm_id(), shm_offset(), submit_count);
    MarkAsPending(client->cmd_buffer_helper()->InsertToken(), submit_count);
}

bool QueryTracker::Query::CheckResultsAvailable(CommandBufferHelper* helper, bool flush_if_pending)
{
    if (Pending()) {
        bool processed_all = base::subtle::Acquire_Load(&info_.sync->process_count) == submit_count();
        ...
        if (processed_all || helper->IsContextLost()) {
            ...
            if (flush_if_pending && (helper->flush_generation() - flush_count_ - 1) >= 0x80000000) {
                helper->Flush();
            } else {
...
QueryTracker::Query* QueryTracker::CreateQuery(GLuint id, GLenum target)
{
    ...
    std::pair<QueryIdMap::iterator, bool> result = queries_.emplace(id, std::move(query));
...
bool QueryTracker::BeginQuery(GLuint id, GLenum target, QueryTrackerClient* client)
{
    QueryTracker::Query* query = GetQuery(id);
    if (!query) {
        query = CreateQuery(id, target);
        if (!query) {
            client->SetGLError(GL_OUT_OF_MEMORY, "glBeginQueryEXT", "transfer buffer allocation failed");
            return false;
        }
    } else if (query->target() != target) {
        client->SetGLError(GL_INVALID_OPERATION, "glBeginQueryEXT", "target does not match");
        return false;
    }
...
bool QueryTracker::EndQuery(GLenum target, QueryTrackerClient* client)
{
    ...
    if (target_it == current_queries_.end()) {
        client->SetGLError(GL_INVALID_OPERATION, "glEndQueryEXT", "no active query");
        return false;
    }
...
bool QueryTracker::QueryCounter(GLuint id, GLenum target, QueryTrackerClient* client)
{
    QueryTracker::Query* query = GetQuery(id);
    if (!query) {
        query = CreateQuery(id, target);
        if (!query) {
            client->SetGLError(GL_OUT_OF_MEMORY, "glQueryCounterEXT", "transfer buffer allocation failed");
            return false;
        }
    } else if (query->target() != target) {
        client->SetGLError(GL_INVALID_OPERATION, "glQueryCounterEXT", "target does not match");
        return false;
    }
...
bool QueryTracker::SetDisjointSync(QueryTrackerClient* client)
{
    if (!disjoint_count_sync_) {
        ...
        int32_t shm_id = -1;
        uint32_t shm_offset;
        void* mem = mapped_memory_->Alloc(sizeof(*disjoint_count_sync_), &shm_id, &shm_offset);
        if (mem) {
