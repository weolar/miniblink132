...
void UnlockClientHandleForTesting(const ClientDiscardableHandle& client_handle)
{
...
bool DeleteClientHandleForTesting(const ClientDiscardableHandle& client_handle)
{
    ServiceDiscardableHandle service_handle(client_handle.BufferForTesting(), client_handle.byte_offset(), client_handle.shm_id());
    return service_handle.Delete();
}

void UnlockAndDeleteClientHandleForTesting(const ClientDiscardableHandle& client_handle)
{
...
TEST(ClientDiscardableTextureManagerTest, BasicUsage)
{
    ...
    {
        ClientDiscardableHandle handle = manager.InitializeTexture(&command_buffer, 1);
        EXPECT_TRUE(handle.IsLockedForTesting());
        EXPECT_EQ(handle.shm_id(), 1);
        EXPECT_FALSE(DeleteClientHandleForTesting(handle));
        UnlockClientHandleForTesting(handle);
    ...
    manager.DiscardableManagerForTesting()->CheckPendingForTesting(&command_buffer);
}
