...
public:
    ...
    TestGpuMemoryBufferManager& operator=(const TestGpuMemoryBufferManager&) = delete;

    ~TestGpuMemoryBufferManager() override;

    std::unique_ptr<TestGpuMemoryBufferManager> CreateClientGpuMemoryBufferManager();
    int GetClientId()
    {
        return client_id_;
    }
...
private:
    ...
    int client_id_ = -1;
    raw_ptr<TestGpuMemoryBufferManager> parent_gpu_memory_buffer_manager_ = nullptr;
