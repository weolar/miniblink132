...
TEST_F(FencedAllocatorTest, TestGetLargestFreeOrPendingSize)
{
    ...
    allocator_->Free(offset);
    allocator_->Free(offset1);
    EXPECT_EQ(kBufferSize - 3 * kSize, allocator_->GetLargestFreeOrPendingSize());
...
protected:
    void SetUp() override
    {
        ...
        // something.
        buffer_.reset(static_cast<char*>(base::AlignedAlloc(kBufferSize, kAllocAlignment)));
        allocator_ = std::make_unique<FencedAllocatorWrapper>(kBufferSize, helper_.get(), buffer_.get());
    }
...
TEST_F(FencedAllocatorWrapperTest, TestBasic)
{
    ...
    EXPECT_GE(buffer_.get() + kBufferSize, reinterpret_cast<char*>(pointer_uint + kSize));

    // Check that it did allocate kSize * sizeof(unsigned int). We can't tell
    // directly, except from the remaining size.
    EXPECT_EQ(kBufferSize - kSize * sizeof(*pointer_uint), allocator_->GetLargestFreeSize());
