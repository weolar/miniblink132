...
template <typename T> class SizedResultHelper {
public:
...
public:
    ...
    static void Delete(GLES2Implementation* gl_impl, GLsizei n, const GLuint* ids)
    {
...
protected:
    ...
    public:
        ...
        bool Initialize(ShareGroup* share_group, bool bind_generates_resource_client, bool bind_generates_resource_service,
            bool lose_context_when_out_of_memory, bool transfer_buffer_initialize_fail, bool sync_query, bool occlusion_query_boolean, bool timer_queries,
            int major_version, int minor_version)
        {
            SharedMemoryLimits limits = SharedMemoryLimitsForTesting();
            command_buffer_.reset(new StrictMock<MockClientCommandBuffer>());

            transfer_buffer_.reset(new MockTransferBuffer(command_buffer_.get(), kTransferBufferSize, GLES2Implementation::kStartingOffset,
                GLES2Implementation::kAlignment, transfer_buffer_initialize_fail));
            ...
            gl_capabilities_.VisitPrecisions([](GLenum shader, GLenum type, GLCapabilities::ShaderPrecision* precision) {
                precision->min_range = 3;
                precision->max_range = 5;
                precision->precision = 7;
            });
            gl_capabilities_.max_combined_texture_image_units = kMaxCombinedTextureImageUnits;
            gl_capabilities_.max_cube_map_texture_size = kMaxCubeMapTextureSize;
            gl_capabilities_.max_fragment_uniform_vectors = kMaxFragmentUniformVectors;
            ...
            gl_capabilities_.max_vertex_texture_image_units = kMaxVertexTextureImageUnits;
            ...
            gl_capabilities_.num_compressed_texture_formats = kNumCompressedTextureFormats;
            gl_capabilities_.num_shader_binary_formats = kNumShaderBinaryFormats;
            gl_capabilities_.max_transform_feedback_separate_attribs = kMaxTransformFeedbackSeparateAttribs;
            gl_capabilities_.max_uniform_buffer_bindings = kMaxUniformBufferBindings;
            gl_capabilities_.bind_generates_resource_chromium = bind_generates_resource_service ? 1 : 0;
            ...
            EXPECT_CALL(*gpu_control_, GetCapabilities()).WillOnce(ReturnRef(capabilities_));
            EXPECT_CALL(*gpu_control_, GetGLCapabilities()).WillOnce(ReturnRef(gl_capabilities_));

            {
                InSequence sequence;

                const bool support_client_side_arrays = true;
                gl_.reset(new GLES2Implementation(helper_.get(), share_group, transfer_buffer_.get(), bind_generates_resource_client,
                    lose_context_when_out_of_memory, support_client_side_arrays, gpu_control_.get()));
            }
            ...
            commands_ = static_cast<CommandBufferEntry*>(ring_buffer->memory()) + command_buffer()->GetServicePutOffset();
            ClearCommands();
        ...
        void TearDown()
        {
            ...
            // For command buffer.
            EXPECT_CALL(*command_buffer(), DestroyTransferBuffer(_)).Times(AtLeast(1));
    ...
    {
        bool success = true;
        share_group_ = new ShareGroup(init_options.bind_generates_resource_client, 0 /* tracing_id */);

        for (int i = 0; i < kNumTestContexts; i++) {
            if (!test_contexts_[i].Initialize(share_group_.get(), init_options.bind_generates_resource_client, init_options.bind_generates_resource_service,
                    init_options.lose_context_when_out_of_memory, init_options.transfer_buffer_initialize_fail, init_options.sync_query,
                    init_options.occlusion_query_boolean, init_options.timer_queries, init_options.major_version, init_options.minor_version))
                success = false;
        }
    ...
    int GetNextToken()
    {
        return test_contexts_[0].GetNextToken();
    }
    ...
    ExpectedMemoryInfo GetExpectedMappedMemory(size_t size)
    {
        ...
        mem.ptr = static_cast<uint8_t*>(gl_->mapped_memory_->Alloc(size, &mem.id, &mem.offset));
    ...
    int CheckError()
    {
        ...
        EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result.ptr, GLuint(GL_NO_ERROR))).RetiresOnSaturation();
        return gl_->GetError();
    }
    ...
    void ResetErrorMessageCallback()
    {
        gl_->error_message_callback_.Reset();
    }
...
protected:
    ...
    template <class ResApi> void FlushGenerationTest()
    {
        GLuint id1, id2, id3;
    ...
    template <class ResApi> void CrossContextGenerationTest()
    {
    ...
    template <class ResApi> void CrossContextGenerationAutoFlushTest()
    {
        GLES2Implementation* gl1 = test_contexts_[0].gl_.get();
        GLES2Implementation* gl2 = test_contexts_[1].gl_.get();
        GLuint id1, id2, id3;
    ...
    template <class ResApi> void DeletingInvalidIdGeneratesError()
    {
        GLES2Implementation* gl1 = test_contexts_[0].gl_.get();
        GLuint id1;
        ResApi::Gen(gl1, 1, &id1);
    ...
    template <class ResApi> void DoubleDeletingIdGeneratesError()
    {
        GLES2Implementation* gl1 = test_contexts_[0].gl_.get();
        GLuint id1;
        ResApi::Gen(gl1, 1, &id1);
...
TEST_F(GLES2ImplementationTest, GetBucketContents)
{
    ...
    ExpectedMemoryInfo mem2 = GetExpectedMemory(kTestSize - MaxTransferBufferSize());

    Cmds expected;
    expected.get_bucket_start.Init(kBucketId, result1.id, result1.offset, MaxTransferBufferSize(), mem1.id, mem1.offset);
    expected.set_token1.Init(GetNextToken());
    expected.get_bucket_data.Init(kBucketId, MaxTransferBufferSize(), kTestSize - MaxTransferBufferSize(), mem2.id, mem2.offset);
    expected.set_bucket_size2.Init(kBucketId, 0);
    expected.set_token2.Init(GetNextToken());

    EXPECT_CALL(*command_buffer(), OnFlush())
        .WillOnce(DoAll(SetMemory(result1.ptr, kTestSize), SetMemoryFromArray(mem1.ptr, expected_data, MaxTransferBufferSize())))
        .WillOnce(SetMemoryFromArray(mem2.ptr, expected_data + MaxTransferBufferSize(), kTestSize - MaxTransferBufferSize()))
        .RetiresOnSaturation();
...
TEST_F(GLES2ImplementationTest, GetShaderPrecisionFormat)
{
    ...
    expected1.cmd.Init(GL_FRAGMENT_SHADER, kDummyType1, client_result1.id, client_result1.offset);
    Result server_result1 = { true, 14, 14, 10 };
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(client_result1.ptr, server_result1)).RetiresOnSaturation();
    gl_->GetShaderPrecisionFormat(GL_FRAGMENT_SHADER, kDummyType1, range1, &precision1);
    ...
    expected3.cmd.Init(GL_FRAGMENT_SHADER, kDummyType2, result3.id, result3.offset);
    Result result3_source = { true, 62, 62, 16 };
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result3.ptr, result3_source)).RetiresOnSaturation();
    gl_->GetShaderPrecisionFormat(GL_FRAGMENT_SHADER, kDummyType2, range3, &precision3);
    ...
    GLint range4[2] = { 0, 0 };
    GLint precision4 = 0;
    gl_->GetShaderPrecisionFormat(GL_FRAGMENT_SHADER, GL_MEDIUM_FLOAT, range4, &precision4);
...
TEST_F(GLES2ImplementationTest, GetShaderSource)
{
    ...
    EXPECT_CALL(*command_buffer(), OnFlush())
        .WillOnce(DoAll(SetMemory(result1.ptr, uint32_t(sizeof(kString))), SetMemory(mem1.ptr, kString)))
        .RetiresOnSaturation();
...
TEST_F(GLES2ImplementationTest, DrawArraysClientSideBuffers)
{
    ...
    const GLsizei kSize1 = std::size(verts) * kNumComponents1 * sizeof(verts[0][0]);
    const GLsizei kSize2 = std::size(verts) * kNumComponents2 * sizeof(verts[0][0]);
    ...
    expected.copy_data1.Init(GL_ARRAY_BUFFER, kEmuOffset1, kSize1, mem1.id, mem1.offset);
    expected.set_token1.Init(GetNextToken());
    expected.set_pointer1.Init(kAttribIndex1, kNumComponents1, GL_FLOAT, GL_FALSE, 0, kEmuOffset1);
    expected.copy_data2.Init(GL_ARRAY_BUFFER, kEmuOffset2, kSize2, mem2.id, mem2.offset);
    expected.set_token2.Init(GetNextToken());
    expected.set_pointer2.Init(kAttribIndex2, kNumComponents2, GL_FLOAT, GL_FALSE, 0, kEmuOffset2);
    ...
    gl_->VertexAttribPointer(kAttribIndex1, kNumComponents1, GL_FLOAT, GL_FALSE, kClientStride, verts);
    gl_->VertexAttribPointer(kAttribIndex2, kNumComponents2, GL_FLOAT, GL_FALSE, kClientStride, verts);
...
TEST_F(GLES2ImplementationTest, DrawElementsClientSideBuffers)
{
    ...
    expected.set_index_size.Init(GL_ELEMENT_ARRAY_BUFFER, kIndexSize, 0, 0, GL_DYNAMIC_DRAW);
    expected.copy_data0.Init(GL_ELEMENT_ARRAY_BUFFER, 0, kIndexSize, mem1.id, mem1.offset);
    expected.set_token0.Init(GetNextToken());
    expected.bind_to_emu.Init(GL_ARRAY_BUFFER, kEmuBufferId);
    expected.set_size.Init(GL_ARRAY_BUFFER, kTotalSize, 0, 0, GL_DYNAMIC_DRAW);
    expected.copy_data1.Init(GL_ARRAY_BUFFER, kEmuOffset1, kSize1, mem2.id, mem2.offset);
    expected.set_token1.Init(GetNextToken());
    expected.set_pointer1.Init(kAttribIndex1, kNumComponents1, GL_FLOAT, GL_FALSE, 0, kEmuOffset1);
    expected.copy_data2.Init(GL_ARRAY_BUFFER, kEmuOffset2, kSize2, mem3.id, mem3.offset);
    expected.set_token2.Init(GetNextToken());
    expected.set_pointer2.Init(kAttribIndex2, kNumComponents2, GL_FLOAT, GL_FALSE, 0, kEmuOffset2);
...
TEST_F(GLES2ImplementationTest, DrawElementsClientSideBuffersServiceSideIndices)
{
    static const float verts[][4] = {
        {
            12.0f,
    ...
    expected.get_max.Init(kClientIndexBufferId, kCount, GL_UNSIGNED_SHORT, kIndexOffset, mem1.id, mem1.offset);
    expected.bind_to_emu.Init(GL_ARRAY_BUFFER, kEmuBufferId);
    expected.set_size.Init(GL_ARRAY_BUFFER, kTotalSize, 0, 0, GL_DYNAMIC_DRAW);
    expected.copy_data1.Init(GL_ARRAY_BUFFER, kEmuOffset1, kSize1, mem2.id, mem2.offset);
    expected.set_token1.Init(GetNextToken());
    ...
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(mem1.ptr, kMaxIndex)).RetiresOnSaturation();
    ...
    gl_->VertexAttribPointer(kAttribIndex1, kNumComponents1, GL_FLOAT, GL_FALSE, kClientStride, verts);
    gl_->VertexAttribPointer(kAttribIndex2, kNumComponents2, GL_FLOAT, GL_FALSE, kClientStride, verts);
    gl_->DrawElements(GL_POINTS, kCount, GL_UNSIGNED_SHORT, reinterpret_cast<const void*>(kIndexOffset));
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}
...
TEST_F(GLES2ImplementationTest, DrawElementsInstancedANGLEClientSideBuffers)
{
    ...
    gl_->VertexAttribPointer(kAttribIndex2, kNumComponents2, GL_FLOAT, GL_FALSE, kClientStride, verts);
    gl_->VertexAttribDivisorANGLE(kAttribIndex2, kDivisor);
    gl_->DrawElementsInstancedANGLE(GL_POINTS, kCount, GL_UNSIGNED_SHORT, indices, 1);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}
...
TEST_F(GLES2ImplementationTest, GetVertexBufferPointerv)
{
    ...
    expected.set_pointer.Init(kAttribIndex2, kNumComponents2, GL_FLOAT, GL_FALSE, kStride2, kOffset2);

    // Set one client side buffer.
    gl_->VertexAttribPointer(kAttribIndex1, kNumComponents1, GL_FLOAT, GL_FALSE, kStride1, verts);
    // Set one VBO
    gl_->BindBuffer(GL_ARRAY_BUFFER, kBufferId);
    gl_->VertexAttribPointer(kAttribIndex2, kNumComponents2, GL_FLOAT, GL_FALSE, kStride2, reinterpret_cast<const void*>(kOffset2));
    // now get them both.
    void* ptr1 = nullptr;
    void* ptr2 = nullptr;

    gl_->GetVertexAttribPointerv(kAttribIndex1, GL_VERTEX_ATTRIB_ARRAY_POINTER, &ptr1);
    gl_->GetVertexAttribPointerv(kAttribIndex2, GL_VERTEX_ATTRIB_ARRAY_POINTER, &ptr2);
...
TEST_F(GLES2ImplementationTest, GetVertexAttrib)
{
    ...
    expected.bind.Init(GL_ARRAY_BUFFER, kBufferId);
    expected.set_pointer.Init(kAttribIndex2, kNumComponents2, GL_FLOAT, GL_FALSE, kStride2, kOffset2);
    expected.get2.Init(kAttribIndex1, GL_CURRENT_VERTEX_ATTRIB, mem2.id, mem2.offset);
    ...
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(mem2.ptr, SizedResultHelper<FourFloats>(current_attrib))).RetiresOnSaturation();
    ...
    gl_->GetVertexAttribiv(kAttribIndex2, GL_VERTEX_ATTRIB_ARRAY_BUFFER_BINDING, &buffer_id);
    EXPECT_EQ(kBufferId, static_cast<GLuint>(buffer_id));
    gl_->GetVertexAttribiv(kAttribIndex1, GL_VERTEX_ATTRIB_ARRAY_BUFFER_BINDING, &buffer_id);
    gl_->GetVertexAttribiv(kAttribIndex1, GL_VERTEX_ATTRIB_ARRAY_ENABLED, &enabled);
    gl_->GetVertexAttribiv(kAttribIndex1, GL_VERTEX_ATTRIB_ARRAY_SIZE, &size);
    gl_->GetVertexAttribiv(kAttribIndex1, GL_VERTEX_ATTRIB_ARRAY_STRIDE, &stride);
    gl_->GetVertexAttribiv(kAttribIndex1, GL_VERTEX_ATTRIB_ARRAY_TYPE, &type);
    gl_->GetVertexAttribiv(kAttribIndex1, GL_VERTEX_ATTRIB_ARRAY_NORMALIZED, &normalized);
    gl_->GetVertexAttribfv(kAttribIndex1, GL_CURRENT_VERTEX_ATTRIB, &current[0]);
...
TEST_F(GLES2ImplementationTest, ReservedIds)
{
    ...
    ExpectedMemoryInfo mem1 = GetExpectedResultMemory(sizeof(cmds::GetError::Result));

    expected.get.Init(mem1.id, mem1.offset);

    // One call to flush to wait for GetError
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(mem1.ptr, GLuint(GL_NO_ERROR))).RetiresOnSaturation();

    gl_->BindBuffer(GL_ARRAY_BUFFER, GLES2Implementation::kClientSideArrayId);
    gl_->BindBuffer(GL_ARRAY_BUFFER, GLES2Implementation::kClientSideElementArrayId);
...
TEST_F(GLES2ImplementationTest, ReadPixels2Reads)
{
    ...
    Cmds expected;
    expected.read1.Init(0, 0, kWidth, kHeight / 2, kFormat, kType, mem1.id, mem1.offset, result1.id, result1.offset, false);
    expected.set_token1.Init(GetNextToken());
    expected.read2.Init(0, kHeight / 2, kWidth, kHeight / 2, kFormat, kType, mem2.id, mem2.offset, result2.id, result2.offset, false);
    expected.set_token2.Init(GetNextToken());
    auto buffer = base::HeapArray<int8_t>::Uninit(kWidth * kHeight * kBytesPerPixel);
...
TEST_F(GLES2ImplementationTest, ReadPixelsBadFormatType)
{
    ...
    Cmds expected;
    expected.read.Init(0, 0, kWidth, kHeight, kFormat, kType, mem1.id, mem1.offset, result1.id, result1.offset, false);
...
TEST_F(GLES2ImplementationTest, FreeUnusedSharedMemory)
{
    ...
    Cmds expected;
    expected.buf.Init(kTarget, kOffset, kSize, mem1.id, mem1.offset);
    expected.set_token.Init(GetNextToken());

    void* mem = gl_->MapBufferSubDataCHROMIUM(kTarget, kOffset, kSize, GL_WRITE_ONLY);
    ASSERT_TRUE(mem != nullptr);
    gl_->UnmapBufferSubDataCHROMIUM(mem);
    EXPECT_CALL(*command_buffer(), DestroyTransferBuffer(_)).Times(1).RetiresOnSaturation();
...
TEST_F(GLES2ImplementationTest, MapUnmapBufferSubDataCHROMIUM)
{
    ...
    uint32_t offset = 0;
    Cmds expected;
    expected.buf.Init(kTarget, kOffset, kSize, command_buffer()->GetNextFreeTransferBufferId(), offset);
    expected.set_token.Init(GetNextToken());
...
TEST_F(GLES2ImplementationTest, MapUnmapTexSubImage2DCHROMIUM)
{
    ...
    Cmds expected;
    expected.tex.Init(
        GL_TEXTURE_2D, kLevel, kXOffset, kYOffset, kWidth, kHeight, kFormat, kType, command_buffer()->GetNextFreeTransferBufferId(), offset, GL_FALSE);
    expected.set_token.Init(GetNextToken());

    void* mem = gl_->MapTexSubImage2DCHROMIUM(GL_TEXTURE_2D, kLevel, kXOffset, kYOffset, kWidth, kHeight, kFormat, kType, GL_WRITE_ONLY);
    ASSERT_TRUE(mem != nullptr);
...
TEST_F(GLES2ImplementationTest, MapUnmapTexSubImage2DCHROMIUMBadArgs)
{
    ...
    void* mem;
    mem = gl_->MapTexSubImage2DCHROMIUM(GL_TEXTURE_2D, -1, kXOffset, kYOffset, kWidth, kHeight, kFormat, kType, GL_WRITE_ONLY);
    EXPECT_TRUE(mem == nullptr);
    EXPECT_EQ(static_cast<GLenum>(GL_INVALID_VALUE), gl_->GetError());
    mem = gl_->MapTexSubImage2DCHROMIUM(GL_TEXTURE_2D, kLevel, -1, kYOffset, kWidth, kHeight, kFormat, kType, GL_WRITE_ONLY);
    EXPECT_TRUE(mem == nullptr);
    EXPECT_EQ(static_cast<GLenum>(GL_INVALID_VALUE), gl_->GetError());
    mem = gl_->MapTexSubImage2DCHROMIUM(GL_TEXTURE_2D, kLevel, kXOffset, -1, kWidth, kHeight, kFormat, kType, GL_WRITE_ONLY);
    EXPECT_TRUE(mem == nullptr);
    EXPECT_EQ(static_cast<GLenum>(GL_INVALID_VALUE), gl_->GetError());
    mem = gl_->MapTexSubImage2DCHROMIUM(GL_TEXTURE_2D, kLevel, kXOffset, kYOffset, -1, kHeight, kFormat, kType, GL_WRITE_ONLY);
    EXPECT_TRUE(mem == nullptr);
    EXPECT_EQ(static_cast<GLenum>(GL_INVALID_VALUE), gl_->GetError());
    mem = gl_->MapTexSubImage2DCHROMIUM(GL_TEXTURE_2D, kLevel, kXOffset, kYOffset, kWidth, -1, kFormat, kType, GL_WRITE_ONLY);
    EXPECT_TRUE(mem == nullptr);
    EXPECT_EQ(static_cast<GLenum>(GL_INVALID_VALUE), gl_->GetError());
    mem = gl_->MapTexSubImage2DCHROMIUM(GL_TEXTURE_2D, kLevel, kXOffset, kYOffset, kWidth, kHeight, kFormat, kType, GL_READ_ONLY);
    EXPECT_TRUE(mem == nullptr);
    EXPECT_EQ(static_cast<GLenum>(GL_INVALID_ENUM), gl_->GetError());
    const char* kPtr = "something";
...
TEST_F(GLES2ImplementationTest, GetIntegerCacheRead)
{
    ...
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result1.ptr, GLuint(GL_NO_ERROR))).RetiresOnSaturation();
    EXPECT_EQ(static_cast<GLenum>(GL_NO_ERROR), gl_->GetError());
}
...
static bool CheckRect(int width, int height, GLenum format, GLenum type, int alignment, const uint8_t* r1, const uint8_t* r2)
{
    uint32_t size = 0;
    uint32_t unpadded_row_size = 0;
    uint32_t padded_row_size = 0;
    if (!GLES2Util::ComputeImageDataSizes(width, height, 1, format, type, alignment, &size, &unpadded_row_size, &padded_row_size)) {
        return false;
    }
...
{
    EXPECT_TRUE(CheckRect(width, height, format, type, alignment, r1, r2));
}
...
TEST_F(GLES2ImplementationTest, TexImage2D)
{
    ...
    Cmds expected;
    expected.tex_image_2d.Init(kTarget, kLevel, kFormat, kWidth, kHeight, kFormat, kType, mem1.id, mem1.offset);
    expected.set_token.Init(GetNextToken());
    gl_->TexImage2D(kTarget, kLevel, kFormat, kWidth, kHeight, kBorder, kFormat, kType, pixels);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
    EXPECT_TRUE(CheckRect(kWidth, kHeight, kFormat, kType, kPixelStoreUnpackAlignment, pixels, mem1.ptr));
}
...
TEST_F(GLES2ImplementationTest, TexImage2DViaMappedMem)
{
    ...
    uint32_t unpadded_row_size = 0;
    uint32_t padded_row_size = 0;
    ASSERT_TRUE(GLES2Util::ComputeImageDataSizes(kWidth, 2, 1, kFormat, kType, kPixelStoreUnpackAlignment, &size, &unpadded_row_size, &padded_row_size));
    const GLsizei kMaxHeight = (MaxTransferBufferSize() / padded_row_size) * 2;
    const GLsizei kHeight = kMaxHeight * 2;
    ASSERT_TRUE(GLES2Util::ComputeImageDataSizes(kWidth, kHeight, 1, kFormat, kType, kPixelStoreUnpackAlignment, &size, &unpadded_row_size, &padded_row_size));
    ...
    expected.tex_image_2d.Init(kTarget, kLevel, kFormat, kWidth, kHeight, kFormat, kType, mem1.id, mem1.offset);
    expected.set_token.Init(GetNextToken());
    gl_->TexImage2D(kTarget, kLevel, kFormat, kWidth, kHeight, kBorder, kFormat, kType, pixels.data());
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
    EXPECT_TRUE(CheckRect(kWidth, kHeight, kFormat, kType, kPixelStoreUnpackAlignment, pixels.data(), mem1.ptr));
}
...
TEST_F(GLES2ImplementationTest, TexImage2DViaTexSubImage2D)
{
    ...
    ASSERT_TRUE(GLES2Util::ComputeImageDataSizes(kWidth, kHeight, 1, kFormat, kType, kPixelStoreUnpackAlignment, &size, nullptr, nullptr));
    uint32_t half_size = 0;
    ASSERT_TRUE(GLES2Util::ComputeImageDataSizes(kWidth, kHeight / 2, 1, kFormat, kType, kPixelStoreUnpackAlignment, &half_size, nullptr, nullptr));
    ...
    Cmds expected;
    expected.tex_image_2d.Init(kTarget, kLevel, kFormat, kWidth, kHeight, kFormat, kType, 0, 0);
    expected.tex_sub_image_2d1.Init(kTarget, kLevel, 0, 0, kWidth, kHeight / 2, kFormat, kType, mem1.id, mem1.offset, true);
    expected.set_token1.Init(GetNextToken());
    expected.tex_sub_image_2d2.Init(kTarget, kLevel, 0, kHeight / 2, kWidth, kHeight / 2, kFormat, kType, mem2.id, mem2.offset, true);
    expected.set_token2.Init(GetNextToken());
    ...
    gl_->TexImage2D(kTarget, kLevel, kFormat, kWidth, kHeight, kBorder, kFormat, kType, pixels.data());
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
    EXPECT_TRUE(CheckRect(kWidth, kHeight / 2, kFormat, kType, kPixelStoreUnpackAlignment, pixels.data() + kHeight / 2 * padded_row_size, mem2.ptr));
}
...
TEST_F(GLES2ImplementationTest, SubImage2DUnpack)
{
    ...
    ASSERT_TRUE(
        GLES2Util::ComputeImageDataSizesES3(kSrcWidth, kSrcSubImageY1, 1, kFormat, kType, pixel_params, &pixel_size, nullptr, nullptr, nullptr, nullptr));
    ...
    for (int sub = 0; sub < 2; ++sub) {
        for (size_t a = 0; a < std::size(unpack_alignments); ++a) {
            ...
            uint32_t client_skip_size;
            {
                ...
                ASSERT_TRUE(GLES2Util::ComputeImageDataSizesES3(kSrcSubImageWidth, kSrcSubImageHeight, 1, kFormat, kType, params, &client_size,
                    &client_unpadded_row_size, &client_padded_row_size, &client_skip_size, nullptr));
            ...
            uint32_t service_skip_size;
            {
                ...
                params.alignment = alignment;
                ASSERT_TRUE(GLES2Util::ComputeImageDataSizesES3(kSrcSubImageWidth, kSrcSubImageHeight, 1, kFormat, kType, params, &service_size,
                    &service_unpadded_row_size, &service_padded_row_size, &service_skip_size, nullptr));
            ...
            if (sub) {
                gl_->TexImage2D(GL_TEXTURE_2D, kLevel, kFormat, kTexWidth, kTexHeight, kBorder, kFormat, kType, nullptr);
                gl_->TexSubImage2D(
                    GL_TEXTURE_2D, kLevel, kTexSubXOffset, kTexSubYOffset, kSrcSubImageWidth, kSrcSubImageHeight, kFormat, kType, src_pixels.data());
                texSubImageExpected.pixel_store_i.Init(GL_UNPACK_ALIGNMENT, alignment);
                texSubImageExpected.tex_image_2d.Init(GL_TEXTURE_2D, kLevel, kFormat, kTexWidth, kTexHeight, kFormat, kType, 0, 0);
                texSubImageExpected.tex_sub_image_2d.Init(
                    GL_TEXTURE_2D, kLevel, kTexSubXOffset, kTexSubYOffset, kSrcSubImageWidth, kSrcSubImageHeight, kFormat, kType, mem.id, mem.offset, GL_FALSE);
                EXPECT_EQ(0, memcmp(&texSubImageExpected, commands, sizeof(texSubImageExpected)));
            } else {
                gl_->TexImage2D(GL_TEXTURE_2D, kLevel, kFormat, kSrcSubImageWidth, kSrcSubImageHeight, kBorder, kFormat, kType, src_pixels.data());
                texImageExpected.pixel_store_i.Init(GL_UNPACK_ALIGNMENT, alignment);
                texImageExpected.tex_image_2d.Init(GL_TEXTURE_2D, kLevel, kFormat, kSrcSubImageWidth, kSrcSubImageHeight, kFormat, kType, mem.id, mem.offset);
                EXPECT_EQ(0, memcmp(&texImageExpected, commands, sizeof(texImageExpected)));
            }
            for (int y = 0; y < kSrcSubImageHeight; ++y) {
                const uint8_t* src_row = src_pixels.data() + client_skip_size + y * client_padded_row_size;
...
TEST_F(GLES3ImplementationTest, SubImage3DUnpack)
{
    ...
    ASSERT_TRUE(GLES2Util::ComputeImageDataSizesES3(
        kSrcWidth, kSrcHeight, kSrcSubImageZ1, kFormat, kType, pixel_params, &pixel_size, nullptr, nullptr, nullptr, nullptr));
    auto src_pixels = base::HeapArray<uint8_t>::WithSize(pixel_size);
    for (size_t i = 0; i < pixel_size; ++i) {
        src_pixels[i] = static_cast<uint8_t>(i % 255);
    }
    ...
    for (int sub = 0; sub < 2; ++sub) {
        for (size_t a = 0; a < std::size(unpack_alignments); ++a) {
            ...
            uint32_t client_skip_size;
            {
                ...
                ASSERT_TRUE(GLES2Util::ComputeImageDataSizesES3(kSrcSubImageWidth, kSrcSubImageHeight, kSrcSubImageDepth, kFormat, kType, params, &client_size,
                    &client_unpadded_row_size, &client_padded_row_size, &client_skip_size, nullptr));
                ASSERT_TRUE(client_size + client_skip_size <= pixel_size);
            }
            ...
            uint32_t service_skip_size;
            {
                ...
                // parameters except for UNPACK_ALIGNMENT.
                params.alignment = alignment;
                ASSERT_TRUE(GLES2Util::ComputeImageDataSizesES3(kSrcSubImageWidth, kSrcSubImageHeight, kSrcSubImageDepth, kFormat, kType, params, &service_size,
                    &service_unpadded_row_size, &service_padded_row_size, &service_skip_size, nullptr));
                ASSERT_TRUE(service_size <= MaxTransferBufferSize());
                ASSERT_TRUE(service_skip_size == 0);
                ASSERT_TRUE(client_unpadded_row_size == service_unpadded_row_size);
            ...
            ExpectedMemoryInfo mem = GetExpectedMemory(service_size);
            if (sub) {
                gl_->TexImage3D(GL_TEXTURE_3D, kLevel, kFormat, kTexWidth, kTexHeight, kTexDepth, kBorder, kFormat, kType, nullptr);
                gl_->TexSubImage3D(GL_TEXTURE_3D, kLevel, kTexSubXOffset, kTexSubYOffset, kTexSubZOffset, kSrcSubImageWidth, kSrcSubImageHeight,
                    kSrcSubImageDepth, kFormat, kType, src_pixels.data());
                texSubImageExpected.pixel_store_i[0].Init(GL_UNPACK_ALIGNMENT, alignment);
                texSubImageExpected.pixel_store_i[1].Init(GL_UNPACK_ROW_LENGTH, kSrcWidth);
                texSubImageExpected.pixel_store_i[2].Init(GL_UNPACK_IMAGE_HEIGHT, kSrcHeight);
                texSubImageExpected.tex_image_3d.Init(GL_TEXTURE_3D, kLevel, kFormat, kTexWidth, kTexHeight, kTexDepth, kFormat, kType, 0, 0);
                texSubImageExpected.tex_sub_image_3d.Init(GL_TEXTURE_3D, kLevel, kTexSubXOffset, kTexSubYOffset, kTexSubZOffset, kSrcSubImageWidth,
                    kSrcSubImageHeight, kSrcSubImageDepth, kFormat, kType, mem.id, mem.offset, GL_FALSE);
                EXPECT_EQ(0, memcmp(&texSubImageExpected, commands, sizeof(texSubImageExpected)));
            } else {
                gl_->TexImage3D(
                    GL_TEXTURE_3D, kLevel, kFormat, kSrcSubImageWidth, kSrcSubImageHeight, kSrcSubImageDepth, kBorder, kFormat, kType, src_pixels.data());
                texImageExpected.pixel_store_i[0].Init(GL_UNPACK_ALIGNMENT, alignment);
                texImageExpected.pixel_store_i[1].Init(GL_UNPACK_ROW_LENGTH, kSrcWidth);
                texImageExpected.pixel_store_i[2].Init(GL_UNPACK_IMAGE_HEIGHT, kSrcHeight);
                texImageExpected.tex_image_3d.Init(
                    GL_TEXTURE_3D, kLevel, kFormat, kSrcSubImageWidth, kSrcSubImageHeight, kSrcSubImageDepth, kFormat, kType, mem.id, mem.offset);
                EXPECT_EQ(0, memcmp(&texImageExpected, commands, sizeof(texImageExpected)));
            }
            for (int z = 0; z < kSrcSubImageDepth; ++z) {
                for (int y = 0; y < kSrcSubImageHeight; ++y) {
                    const uint8_t* src_row = src_pixels.data() + client_skip_size + (kSrcHeight * z + y) * client_padded_row_size;
...
TEST_F(GLES2ImplementationTest, TextureInvalidArguments)
{
    ...
    // Use invalid border.
    gl_->TexImage2D(kTarget, kLevel, kFormat, kWidth, kHeight, kInvalidBorder, kFormat, kType, pixels);
    ...
    gl_->CompressedTexImage2D(kTarget, kLevel, kCompressedFormat, kWidth, kHeight, kBorder, std::size(pixels), pixels);
    ...
    // checks.
    gl_->CompressedTexImage2D(kTarget, kLevel, kCompressedFormat, kWidth, kHeight, kInvalidBorder, std::size(pixels), pixels);
    EXPECT_TRUE(NoCommandsWritten());
    EXPECT_EQ(GL_INVALID_VALUE, CheckError());
}
...
TEST_F(GLES2ImplementationTest, TexImage3DSingleCommand)
{
    ...
    uint32_t unpadded_row_size = 0;
    uint32_t padded_row_size = 0;
    ASSERT_TRUE(GLES2Util::ComputeImageDataSizes(kWidth, 2, kDepth, kFormat, kType, kPixelStoreUnpackAlignment, &size, &unpadded_row_size, &padded_row_size));
    ...
    ASSERT_TRUE(GLES2Util::ComputeImageDataSizes(kWidth, kHeight, kDepth, kFormat, kType, kPixelStoreUnpackAlignment, &size, nullptr, nullptr));
    ...
    Cmds expected;
    expected.tex_image_3d.Init(kTarget, kLevel, kFormat, kWidth, kHeight, kDepth, kFormat, kType, mem.id, mem.offset);

    gl_->TexImage3D(kTarget, kLevel, kFormat, kWidth, kHeight, kDepth, kBorder, kFormat, kType, pixels.data());

    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
    EXPECT_TRUE(CheckRect(kWidth, kHeight * kDepth, kFormat, kType, kPixelStoreUnpackAlignment, reinterpret_cast<uint8_t*>(pixels.data()), mem.ptr));
}
...
TEST_F(GLES2ImplementationTest, TexImage3DViaTexSubImage3D)
{
    ...
    Cmds expected;
    expected.tex_image_3d.Init(kTarget, kLevel, kFormat, kWidth, kHeight, 1, kFormat, kType, 0, 0);
    expected.tex_sub_image_3d1.Init(kTarget, kLevel, 0, 0, 0, kWidth, kHeight - 3, 1, kFormat, kType, mem1.id, mem1.offset, GL_TRUE);
    expected.tex_sub_image_3d2.Init(kTarget, kLevel, 0, kHeight - 3, 0, kWidth, 3, 1, kFormat, kType, mem2.id, mem2.offset, GL_TRUE);
    expected.set_token.Init(GetNextToken());

    gl_->TexImage3D(kTarget, kLevel, kFormat, kWidth, kHeight, 1, kBorder, kFormat, kType, pixels.data());
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}
...
TEST_F(GLES2ImplementationTest, TexSubImage3D4Writes)
{
    ...
    expected.tex_sub_image_3d1_1.Init(
        kTarget, kLevel, kXOffset, kYOffset, kZOffset, kWidth, kHeight - 2, 1, kFormat, kType, mem1_1.id, mem1_1.offset, GL_FALSE);
    expected.tex_sub_image_3d1_2.Init(
        kTarget, kLevel, kXOffset, kYOffset + kHeight - 2, kZOffset, kWidth, 2, 1, kFormat, kType, mem1_2.id, mem1_2.offset, GL_FALSE);
    expected.tex_sub_image_3d2_1.Init(
        kTarget, kLevel, kXOffset, kYOffset, kZOffset + 1, kWidth, kHeight - 2, 1, kFormat, kType, mem2_1.id, mem2_1.offset, GL_FALSE);
    expected.tex_sub_image_3d2_2.Init(
        kTarget, kLevel, kXOffset, kYOffset + kHeight - 2, kZOffset + 1, kWidth, 2, 1, kFormat, kType, mem2_2.id, mem2_2.offset, GL_FALSE);
    expected.set_token1.Init(GetNextToken());
    expected.set_token2.Init(GetNextToken());
    expected.set_token3.Init(GetNextToken());

    gl_->TexSubImage3D(kTarget, kLevel, kXOffset, kYOffset, kZOffset, kWidth, kHeight, kDepth, kFormat, kType, pixels.data());

    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
    uint32_t offset_to_last = first_size + second_size + third_size;
    EXPECT_TRUE(CheckRect(kWidth, 2, kFormat, kType, kPixelStoreUnpackAlignment, reinterpret_cast<uint8_t*>(pixels.data()) + offset_to_last, mem2_2.ptr));
}
...
TEST_F(GLES2ImplementationStrictSharedTest, CrossContextGenerationTestRenderbuffers)
{
    CrossContextGenerationTest<GenRenderbuffersAPI>();
}
TEST_F(GLES2ImplementationStrictSharedTest, CrossContextGenerationTestTextures)
{
...
// in testing.
TEST_F(GLES2ImplementationStrictSharedTest, CrossContextGenerationAutoFlushTestBuffers)
{
    CrossContextGenerationAutoFlushTest<GenBuffersAPI>();
}
TEST_F(GLES2ImplementationStrictSharedTest, CrossContextGenerationAutoFlushTestRenderbuffers)
{
    CrossContextGenerationAutoFlushTest<GenRenderbuffersAPI>();
}
TEST_F(GLES2ImplementationStrictSharedTest, CrossContextGenerationAutoFlushTestTextures)
{
    CrossContextGenerationAutoFlushTest<GenTexturesAPI>();
}

// Test deleting an invalid ID.
TEST_F(GLES2ImplementationStrictSharedTest, DeletingInvalidIdGeneratesErrorBuffers)
{
    DeletingInvalidIdGeneratesError<GenBuffersAPI>();
}
TEST_F(GLES2ImplementationStrictSharedTest, DeletingInvalidIdGeneratesErrorRenderbuffers)
{
    DeletingInvalidIdGeneratesError<GenRenderbuffersAPI>();
}
TEST_F(GLES2ImplementationStrictSharedTest, DeletingInvalidIdGeneratesErrorTextures)
{
    DeletingInvalidIdGeneratesError<GenTexturesAPI>();
}

// Test double-deleting the same ID.
TEST_F(GLES2ImplementationStrictSharedTest, DoubleDeletingIdGeneratesErrorBuffers)
{
    DoubleDeletingIdGeneratesError<GenBuffersAPI>();
}
TEST_F(GLES2ImplementationStrictSharedTest, DoubleDeletingIdGeneratesErrorRenderbuffers)
{
    DoubleDeletingIdGeneratesError<GenRenderbuffersAPI>();
}
TEST_F(GLES2ImplementationStrictSharedTest, DoubleDeletingIdGeneratesErrorTextures)
{
...
TEST_F(GLES2ImplementationTest, BufferDataLargerThanTransferBuffer)
{
    ...
    Cmds expected;
    expected.set_size.Init(GL_ARRAY_BUFFER, std::size(buf), 0, 0, GL_DYNAMIC_DRAW);
    expected.copy_data1.Init(GL_ARRAY_BUFFER, 0, kUsableSize, mem1.id, mem1.offset);
    expected.set_token1.Init(GetNextToken());
    expected.copy_data2.Init(GL_ARRAY_BUFFER, kUsableSize, kUsableSize, mem2.id, mem2.offset);
    expected.set_token2.Init(GetNextToken());
...
TEST_F(GLES2ImplementationTest, MultiDrawArraysWEBGLLargerThanTransferBuffer)
{
    ...
    expected.draw1.Init(GL_TRIANGLES, mem1.id, mem1.offset, mem1.id, mem1.offset + kCountsOffset, kChunkDrawCount);
    expected.set_token1.Init(GetNextToken());
    expected.draw2.Init(GL_TRIANGLES, mem2.id, mem2.offset, mem2.id, mem2.offset + kCountsOffset, kChunkDrawCount);
    expected.set_token2.Init(GetNextToken());
    expected.end.Init();
...
TEST_F(GLES2ImplementationTest, BeginEndQueryEXT)
{
    ...
    expected_begin_cmds.begin_query.Init(GL_ANY_SAMPLES_PASSED_EXT, id1, query->shm_id(), query->shm_offset());
    ...
    gl_->GetQueryivEXT(GL_ANY_SAMPLES_PASSED_CONSERVATIVE_EXT, GL_CURRENT_QUERY_EXT, &param);
    EXPECT_EQ(0, param);
    ...
    expected_end_cmds.end_query.Init(GL_ANY_SAMPLES_PASSED_EXT, query->submit_count());
...
TEST_F(GLES2ImplementationTest, SetDisjointSync)
{
    ...
    const void* commands = GetPut();
    gl_->SetDisjointValueSyncCHROMIUM();
    expected_disjoint_sync_cmd.disjoint_sync.Init(GetQueryTracker()->DisjointCountSyncShmID(), GetQueryTracker()->DisjointCountSyncShmOffset());
...
TEST_F(GLES2ImplementationTest, QueryCounterEXT)
{
    ...
    QueryTracker::Query* query = GetQuery(id1);
    ASSERT_TRUE(query != nullptr);
    expected_query_counter_cmds.query_counter.Init(id1, GL_TIMESTAMP_EXT, query->shm_id(), query->shm_offset(), query->submit_count());
    ...
    expected_query_counter_cmds.query_counter.Init(id2, GL_TIMESTAMP_EXT, query2->shm_id(), query2->shm_offset(), query2->submit_count());
    EXPECT_EQ(0, memcmp(&expected_query_counter_cmds, commands, sizeof(expected_query_counter_cmds)));
    ClearCommands();
    ...
    expected_query_counter_cmds.query_counter.Init(id3, GL_COMMANDS_ISSUED_TIMESTAMP_CHROMIUM, query3->shm_id(), query3->shm_offset(), query3->submit_count());
    EXPECT_EQ(0, memcmp(&expected_query_counter_cmds, commands, sizeof(expected_query_counter_cmds)));
    ClearCommands();
...
TEST_F(GLES2ImplementationTest, ErrorQuery)
{
    ...
    EndCmds expected_end_cmds;
    expected_end_cmds.begin_query.Init(GL_GET_ERROR_QUERY_CHROMIUM, id, query->shm_id(), query->shm_offset());
    expected_end_cmds.end_query.Init(GL_GET_ERROR_QUERY_CHROMIUM, query->submit_count());
    EXPECT_EQ(0, memcmp(&expected_end_cmds, commands, sizeof(expected_end_cmds)));
    ClearCommands();
...
TEST_F(GLES2ImplementationTest, VertexArrays)
{
    ...
    gl_->VertexAttribPointer(kAttribIndex1, kNumComponents1, GL_FLOAT, GL_FALSE, kClientStride, reinterpret_cast<const void*>(4));
    EXPECT_EQ(GL_INVALID_OPERATION, CheckError());

    gl_->VertexAttribPointer(kAttribIndex1, kNumComponents1, GL_FLOAT, GL_FALSE, kClientStride, nullptr);
    EXPECT_EQ(GL_NO_ERROR, CheckError());
}
#endif
...
TEST_F(GLES2ImplementationTest, LimitSizeAndOffsetTo32Bit)
{
    ...
    gl_->BufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_DYNAMIC_DRAW);
    EXPECT_EQ(GL_NO_ERROR, CheckError());
    ...
    EXPECT_TRUE(nullptr == gl_->MapBufferSubDataCHROMIUM(GL_ARRAY_BUFFER, offset, 1, GL_WRITE_ONLY));
    EXPECT_EQ(GL_INVALID_OPERATION, CheckError());
    EXPECT_STREQ(kOffsetOverflowMessage, GetLastError().c_str());

    // MapBufferSubDataCHROMIUM: size
    EXPECT_EQ(GL_NO_ERROR, CheckError());
    EXPECT_TRUE(nullptr == gl_->MapBufferSubDataCHROMIUM(GL_ARRAY_BUFFER, 0, size, GL_WRITE_ONLY));
    EXPECT_EQ(GL_INVALID_OPERATION, CheckError());
    EXPECT_STREQ(kSizeOverflowMessage, GetLastError().c_str());
    ...
    // DrawElements: offset
    gl_->DrawElements(GL_POINTS, 1, GL_UNSIGNED_BYTE, reinterpret_cast<void*>(offset));
    EXPECT_EQ(GL_INVALID_OPERATION, CheckError());
    EXPECT_STREQ(kOffsetOverflowMessage, GetLastError().c_str());
    ...
    gl_->DrawElementsInstancedANGLE(GL_POINTS, 1, GL_UNSIGNED_BYTE, reinterpret_cast<void*>(offset), 1);
    EXPECT_EQ(GL_INVALID_OPERATION, CheckError());
    EXPECT_STREQ(kOffsetOverflowMessage, GetLastError().c_str());
    ...
    gl_->VertexAttribPointer(kAttribIndex, 1, GL_FLOAT, GL_FALSE, kStride, nullptr);
    EXPECT_EQ(GL_NO_ERROR, CheckError());

    // VertexAttribPointer: offset
    gl_->VertexAttribPointer(kAttribIndex, 1, GL_FLOAT, GL_FALSE, kStride, reinterpret_cast<void*>(offset));
    EXPECT_EQ(GL_INVALID_OPERATION, CheckError());
    EXPECT_STREQ(kOffsetOverflowMessage, GetLastError().c_str());
}
...
TEST_F(GLES2ImplementationTest, TraceBeginCHROMIUM)
{
    ...
    expected.category_data.Init(kCategoryBucketId, 0, category_name.size() + 1, mem1.id, mem1.offset);
    expected.set_token1.Init(GetNextToken());
    expected.name_size1.Init(kNameBucketId, trace_name.size() + 1);
    expected.name_data.Init(kNameBucketId, 0, trace_name.size() + 1, mem2.id, mem2.offset);
    expected.set_token2.Init(GetNextToken());
...
TEST_F(GLES2ImplementationTest, SetActiveURLCHROMIUM)
{
    ...
    const size_t kPaddedStringSize = transfer_buffer_->RoundToAlignment(url.size());
    ...
    EXPECT_EQ(0, memcmp(url.c_str(), reinterpret_cast<char*>(mem.ptr), url.size()));
...
TEST_F(GLES2ImplementationTest, GenSyncTokenCHROMIUM)
{
    ...
    EXPECT_CALL(*gpu_control_, GetNamespaceID()).WillRepeatedly(Return(kNamespaceId));
    EXPECT_CALL(*gpu_control_, GetCommandBufferID()).WillRepeatedly(Return(kCommandBufferId));
    ...
    EXPECT_CALL(*gpu_control_, GenerateFenceSyncRelease()).WillOnce(Return(kFenceSync));
...
{
    ExpectedMemoryInfo result = GetExpectedResultMemory(sizeof(cmds::GetError::Result));
    EXPECT_CALL(*command_buffer(), OnFlush()).WillRepeatedly(SetMemory(result.ptr, GLuint(GL_NO_ERROR))).RetiresOnSaturation();
    ...
    ClearCommands();
    EXPECT_CALL(*gpu_control_, CanWaitUnverifiedSyncToken(sync_token)).WillOnce(Return(false));
    ...
    ClearCommands();
    EXPECT_CALL(*gpu_control_, CanWaitUnverifiedSyncToken(sync_token)).WillOnce(Return(true));
    EXPECT_CALL(*gpu_control_, EnsureWorkVisible());
...
TEST_F(GLES2ImplementationTest, VerifySyncTokensCHROMIUM_Sequence)
{
    ...
    EXPECT_CALL(*gpu_control_, GenerateFenceSyncRelease()).WillOnce(Return(kFenceSync1));
    ...
    EXPECT_CALL(*gpu_control_, GenerateFenceSyncRelease()).WillOnce(Return(kFenceSync2));
    ...
    Sequence sequence;
    EXPECT_CALL(*gpu_control_, CanWaitUnverifiedSyncToken(sync_token1)).InSequence(sequence).WillOnce(Return(true));
    EXPECT_CALL(*gpu_control_, CanWaitUnverifiedSyncToken(sync_token2)).InSequence(sequence).WillOnce(Return(true));
...
TEST_F(GLES2ImplementationTest, WaitSyncTokenCHROMIUM)
{
    ...
    EXPECT_CALL(*gpu_control_, GetCommandBufferID()).WillOnce(Return(kCommandBufferId));
    EXPECT_CALL(*gpu_control_, GenerateFenceSyncRelease()).WillOnce(Return(kFenceSync));
    EXPECT_CALL(*gpu_control_, EnsureWorkVisible());
...
TEST_F(GLES2ImplementationTest, WaitSyncTokenCHROMIUMErrors)
{
    ...
    ClearCommands();
    gpu::SyncToken unverified_sync_token(CommandBufferNamespace::GPU_IO, gpu::CommandBufferId(), 0);
    EXPECT_CALL(*gpu_control_, CanWaitUnverifiedSyncToken(unverified_sync_token)).WillOnce(Return(false));
...
TEST_F(GLES2ImplementationTest, IsEnabled)
{
    ...
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result1.ptr, uint32_t(GL_TRUE))).RetiresOnSaturation();
...
TEST_F(GLES2ImplementationTest, ClientWaitSync)
{
    ...
    expected.cmd.Init(client_sync_id, GL_SYNC_FLUSH_COMMANDS_BIT, kTimeout, result1.id, result1.offset);

    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result1.ptr, uint32_t(GL_CONDITION_SATISFIED))).RetiresOnSaturation();

    GLenum result = gl_->ClientWaitSync(reinterpret_cast<GLsync>(client_sync_id), GL_SYNC_FLUSH_COMMANDS_BIT, kTimeout);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
...
TEST_F(GLES2ImplementationTest, MapBufferRangeUnmapBufferWrite)
{
    ...
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result.ptr, uint32_t(1))).RetiresOnSaturation();
...
TEST_F(GLES2ImplementationTest, MapBufferRangeWriteWithInvalidateBit)
{
    ...
    GLsizeiptr kSize = 64;
    void* mem = gl_->MapBufferRange(GL_ARRAY_BUFFER, 10, kSize, GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_RANGE_BIT);
    EXPECT_TRUE(mem != nullptr);
...
TEST_F(GLES2ImplementationTest, MapBufferRangeWriteWithGLError)
{
    ...
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result.ptr, uint32_t(0))).RetiresOnSaturation();
...
TEST_F(GLES2ImplementationTest, GetInternalformativ)
{
    ...
    expected.cmd.Init(123, GL_RGBA8, GL_NUM_SAMPLE_COUNTS, result1.id, result1.offset);
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result1.ptr, SizedResultHelper<ResultType>(kNumSampleCounts))).RetiresOnSaturation();
...
TEST_F(GLES2ImplementationTest, SignalSyncToken)
{
    ...
    EXPECT_CALL(*gpu_control_, DoSignalSyncToken(_, _)).WillOnce(Invoke([&signal_closure](const SyncToken& sync_token, base::OnceClosure* callback) {
        signal_closure = std::move(*callback);
    }));
    gl_->SignalSyncToken(sync_token, base::BindOnce(&CountCallback, &signaled_count));
...
TEST_F(GLES2ImplementationTest, DiscardableMemoryDelete)
{
    ...
    EXPECT_FALSE(share_group_->discardable_texture_manager()->TextureIsValid(texture_id));
    gl_->InitializeDiscardableTextureCHROMIUM(texture_id);
    EXPECT_TRUE(share_group_->discardable_texture_manager()->TextureIsValid(texture_id));
...
TEST_F(GLES2ImplementationTest, DiscardableTextureLockFail)
{
    ...
    ClientDiscardableHandle client_handle = share_group_->discardable_texture_manager()->GetHandleForTesting(texture_id);
    ServiceDiscardableHandle service_handle(client_handle.BufferForTesting(), client_handle.byte_offset(), client_handle.shm_id());
...
struct ErrorMessageCounter {
    explicit ErrorMessageCounter(GLES2Implementation* gl)
        : gl(gl)
    {
    }
...
TEST_F(GLES2ImplementationTest, ReentrantErrorCallbacksShouldNotCrash)
{
    ...
    gl_->SetErrorMessageCallback(base::BindRepeating(&ErrorMessageCounter::Callback, base::Unretained(&counter)));
