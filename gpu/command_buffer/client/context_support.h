...
public:
    ...
    virtual void SignalSyncToken(const SyncToken& sync_token, base::OnceClosure callback) = 0;
    ...
    virtual void GetGpuFence(uint32_t gpu_fence_id, base::OnceCallback<void(std::unique_ptr<gfx::GpuFence>)> callback) = 0;
    ...
    // flushed.
    virtual void SetAggressivelyFreeResources(bool aggressively_free_resources) = 0;
    ...
    virtual void SetErrorMessageCallback(base::RepeatingCallback<void(const char*, int32_t)> callback) = 0;
    ...
    virtual void CompleteLockDiscardableTexureOnContextThread(uint32_t texture_id) = 0;

    // Checks if a discardable handle is deleted. For use in tracing code.
    virtual bool ThreadsafeDiscardableTextureIsDeletedForTracing(uint32_t texture_id) = 0;
    ...
    virtual void UnlockTransferCacheEntries(const std::vector<std::pair<uint32_t, uint32_t>>& entries) = 0;
    ...
    virtual bool CanDecodeWithHardwareAcceleration(const cc::ImageHeaderMetadata* image_metadata) const = 0;
