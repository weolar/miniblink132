...
protected:
    ...
    void SetUp() override
    {
        ...
        mapped_memory_ = std::make_unique<MappedMemoryManager>(helper_.get(), MappedMemoryManager::kNoLimit);
...
TEST_F(QuerySyncManagerTest, Shrink)
{
    ...
    FencedAllocator::State state = mapped_memory_->GetPointerStatusForTest(syncs, nullptr);
...
TEST_F(QueryTrackerTest, Basic)
{
    ...
    QueryTracker::Query* query = query_tracker_->CreateQuery(kId1, GL_ANY_SAMPLES_PASSED_EXT);
...
TEST_F(QueryTrackerTest, ManyQueries)
{
    ...
    static_assert(kTestSize > QuerySyncManager::kSyncsPerBucket, "We want to use more than one bucket");
