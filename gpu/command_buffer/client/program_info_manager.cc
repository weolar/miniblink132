...
// aligned for `T`.
template <typename T> T Load(base::span<const int8_t> data, size_t offset)
{
...
class DataIterator {
public:
    explicit DataIterator(base::span<const int8_t> data)
        : data_(data)
    {
    }
...
void FillNameAndLength(GLsizei bufsize, GLsizei* length, char* name, const std::string& string)
{
...
ProgramInfoManager::Program::VertexAttrib::VertexAttrib(GLsizei _size, GLenum _type, const std::string& _name, GLint _location)
    : size(_size)
    , type(_type)
    , location(_location)
    , name(_name)
...
ProgramInfoManager::Program::UniformInfo::UniformInfo(GLsizei _size, GLenum _type, const std::string& _name)
    : size(_size)
    , type(_type)
    , name(_name)
{
...
ProgramInfoManager::Program::UniformInfo::UniformInfo(const UniformInfo& other) = default;
...
ProgramInfoManager::Program::UniformBlock::UniformBlock(const UniformBlock& other) = default;

ProgramInfoManager::Program::UniformBlock::~UniformBlock() = default;

ProgramInfoManager::Program::TransformFeedbackVarying::TransformFeedbackVarying()
    : size(0)
    , type(0)
{
}

ProgramInfoManager::Program::TransformFeedbackVarying::~TransformFeedbackVarying() = default;
...
GLint ProgramInfoManager::Program::GetAttribLocation(const std::string& name) const
{
...
const ProgramInfoManager::Program::VertexAttrib* ProgramInfoManager::Program::GetAttribInfo(GLint index) const
{
    return (static_cast<size_t>(index) < attrib_infos_.size()) ? &attrib_infos_[index] : nullptr;
}

const ProgramInfoManager::Program::UniformInfo* ProgramInfoManager::Program::GetUniformInfo(GLint index) const
{
    return (static_cast<size_t>(index) < uniform_infos_.size()) ? &uniform_infos_[index] : nullptr;
}

const ProgramInfoManager::Program::UniformBlock* ProgramInfoManager::Program::GetUniformBlock(GLuint index) const
{
    return (index < uniform_blocks_.size()) ? &uniform_blocks_[index] : nullptr;
}

GLint ProgramInfoManager::Program::GetUniformLocation(const std::string& name) const
{
...
GLuint ProgramInfoManager::Program::GetUniformIndex(const std::string& name) const
{
...
GLint ProgramInfoManager::Program::GetFragDataIndex(const std::string& name) const
{
...
void ProgramInfoManager::Program::CacheFragDataIndex(const std::string& name, GLint index)
{
    frag_data_indices_[name] = index;
}

GLint ProgramInfoManager::Program::GetFragDataLocation(const std::string& name) const
{
    std::unordered_map<std::string, GLint>::const_iterator iter = frag_data_locations_.find(name);
    if (iter == frag_data_locations_.end())
        return -1;
    return iter->second;
}

void ProgramInfoManager::Program::CacheFragDataLocation(const std::string& name, GLint loc)
{
    frag_data_locations_[name] = loc;
}

bool ProgramInfoManager::Program::GetProgramiv(GLenum pname, GLint* params)
{
    switch (pname) {
    case GL_LINK_STATUS:
...
GLuint ProgramInfoManager::Program::GetUniformBlockIndex(const std::string& name) const
{
...
void ProgramInfoManager::Program::UniformBlockBinding(GLuint index, GLuint binding)
{
...
const ProgramInfoManager::Program::TransformFeedbackVarying* ProgramInfoManager::Program::GetTransformFeedbackVarying(GLuint index) const
{
    return (index < transform_feedback_varyings_.size()) ? &transform_feedback_varyings_[index] : nullptr;
}

bool ProgramInfoManager::Program::GetUniformsiv(GLsizei count, const GLuint* indices, GLenum pname, GLint* params)
{
    if (count == 0) {
    ...
    case GL_UNIFORM_NAME_LENGTH:
        DCHECK_EQ(num_uniforms, uniform_infos_.size());
        for (GLsizei ii = 0; ii < count; ++ii) {
            params[ii] = static_cast<GLint>(uniform_infos_[indices[ii]].name.length() + 1);
        }
        return true;
    }
...
void ProgramInfoManager::Program::UpdateES2(base::span<const int8_t> result)
{
    ...
    DataIterator inputs(result.subspan(sizeof(header), sizeof(ProgramInput) * (header.num_attribs + header.num_uniforms)));
    for (uint32_t ii = 0; ii < header.num_attribs; ++ii) {
        ...
        std::string name(ToStringView(result.subspan(input.name_offset, input.name_length)));
        attrib_infos_.push_back(VertexAttrib(input.size, input.type, name, location));
        max_attrib_name_length_ = std::max(static_cast<GLsizei>(name.size() + 1), max_attrib_name_length_);
    }
    for (uint32_t ii = 0; ii < header.num_uniforms; ++ii) {
        auto input = inputs.Get<ProgramInput>();
        DataIterator locations(result.subspan(input.location_offset, sizeof(int32_t) * input.size));
        std::string name(ToStringView(result.subspan(input.name_offset, input.name_length)));
        UniformInfo info(input.size, input.type, name);
        max_uniform_name_length_ = std::max(static_cast<GLsizei>(name.size() + 1), max_uniform_name_length_);
...
void ProgramInfoManager::Program::UpdateES3UniformBlocks(base::span<const int8_t> result)
{
    ...
    for (uint32_t ii = 0; ii < header.num_uniform_blocks; ++ii) {
        ...
        uniform_blocks_[ii].referenced_by_vertex_shader = static_cast<GLboolean>(entry.referenced_by_vertex_shader);
        uniform_blocks_[ii].referenced_by_fragment_shader = static_cast<GLboolean>(entry.referenced_by_fragment_shader);
        ...
        for (uint32_t uu = 0; uu < entry.active_uniforms; ++uu) {
            uniform_blocks_[ii].active_uniform_indices[uu] = static_cast<GLuint>(data.Get<uint32_t>());
        }
    }
    DCHECK(data.empty());
...
void ProgramInfoManager::Program::UpdateES3Uniformsiv(base::span<const int8_t> result)
{
...
void ProgramInfoManager::Program::UpdateES3TransformFeedbackVaryings(base::span<const int8_t> result)
{
    ...
    for (uint32_t ii = 0; ii < header.num_transform_feedback_varyings; ++ii) {
        ...
        base::span<const int8_t> name = data.GetBytes(entry.name_length);
        transform_feedback_varyings_[ii].name = ToStringView(name.first(entry.name_length - 1));
    }
    DCHECK(data.empty());
...
ProgramInfoManager::Program* ProgramInfoManager::GetProgramInfo(GLES2Implementation* gl, GLuint program, ProgramInfoType type)
{
...
bool ProgramInfoManager::GetProgramiv(GLES2Implementation* gl, GLuint program, GLenum pname, GLint* params)
{
    base::AutoLock auto_lock(lock_);
...
bool ProgramInfoManager::GetActiveUniformsiv(GLES2Implementation* gl, GLuint program, GLsizei count, const GLuint* indices, GLenum pname, GLint* params)
{
    base::AutoLock auto_lock(lock_);
    ProgramInfoType type = kNone;
    switch (pname) {
...
GLint ProgramInfoManager::GetAttribLocation(GLES2Implementation* gl, GLuint program, const char* name)
{
    {
        base::AutoLock auto_lock(lock_);
...
GLint ProgramInfoManager::GetUniformLocation(GLES2Implementation* gl, GLuint program, const char* name)
{
    {
        base::AutoLock auto_lock(lock_);
        Program* info = GetProgramInfo(gl, program, kES2);
...
GLint ProgramInfoManager::GetFragDataIndex(GLES2Implementation* gl, GLuint program, const char* name)
{
...
GLint ProgramInfoManager::GetFragDataLocation(GLES2Implementation* gl, GLuint program, const char* name)
{
...
bool ProgramInfoManager::GetActiveAttrib(
    GLES2Implementation* gl, GLuint program, GLuint index, GLsizei bufsize, GLsizei* length, GLint* size, GLenum* type, char* name)
{
    {
        base::AutoLock auto_lock(lock_);
        Program* info = GetProgramInfo(gl, program, kES2);
    ...
    return gl->GetActiveAttribHelper(program, index, bufsize, length, size, type, name);
}
    ...
    GLES2Implementation* gl, GLuint program, GLuint index, GLsizei bufsize, GLsizei* length, GLint* size, GLenum* type, char* name)
{
    ...
    return gl->GetActiveUniformHelper(program, index, bufsize, length, size, type, name);
}

GLuint ProgramInfoManager::GetUniformBlockIndex(GLES2Implementation* gl, GLuint program, const char* name)
{
    {
        base::AutoLock auto_lock(lock_);
...
bool ProgramInfoManager::GetActiveUniformBlockName(GLES2Implementation* gl, GLuint program, GLuint index, GLsizei buf_size, GLsizei* length, char* name)
{
    ...
    return gl->GetActiveUniformBlockNameHelper(program, index, buf_size, length, name);
}

bool ProgramInfoManager::GetActiveUniformBlockiv(GLES2Implementation* gl, GLuint program, GLuint index, GLenum pname, GLint* params)
{
    {
        base::AutoLock auto_lock(lock_);
        Program* info = GetProgramInfo(gl, program, kES3UniformBlocks);
            ...
            if (uniform_block && valid_pname && params) {
                ...
                case GL_UNIFORM_BLOCK_NAME_LENGTH:
                    *params = static_cast<GLint>(uniform_block->name.size()) + 1;
                    break;
                case GL_UNIFORM_BLOCK_ACTIVE_UNIFORMS:
                    *params = static_cast<GLint>(uniform_block->active_uniform_indices.size());
                    break;
                case GL_UNIFORM_BLOCK_ACTIVE_UNIFORM_INDICES:
                    for (size_t ii = 0; ii < uniform_block->active_uniform_indices.size(); ++ii) {
                        params[ii] = static_cast<GLint>(uniform_block->active_uniform_indices[ii]);
                    }
                    break;
                case GL_UNIFORM_BLOCK_REFERENCED_BY_VERTEX_SHADER:
                    *params = static_cast<GLint>(uniform_block->referenced_by_vertex_shader);
                    break;
                case GL_UNIFORM_BLOCK_REFERENCED_BY_FRAGMENT_SHADER:
                    *params = static_cast<GLint>(uniform_block->referenced_by_fragment_shader);
                    break;
                default:
                    NOTREACHED();
                }
...
void ProgramInfoManager::UniformBlockBinding(GLES2Implementation* gl, GLuint program, GLuint index, GLuint binding)
{
    GLuint max_bindings = static_cast<GLuint>(gl->gl_capabilities().max_uniform_buffer_bindings);
...
bool ProgramInfoManager::GetTransformFeedbackVarying(
    GLES2Implementation* gl, GLuint program, GLuint index, GLsizei bufsize, GLsizei* length, GLsizei* size, GLenum* type, char* name)
{
    {
        base::AutoLock auto_lock(lock_);
    ...
    return gl->GetTransformFeedbackVaryingHelper(program, index, bufsize, length, size, type, name);
}

bool ProgramInfoManager::GetUniformIndices(GLES2Implementation* gl, GLuint program, GLsizei count, const char* const* names, GLuint* indices)
{
    {
        base::AutoLock auto_lock(lock_);
        Program* info = GetProgramInfo(gl, program, kES2);
...
bool ProgramInfoManager::GetProgramInterfaceiv(GLES2Implementation* gl, GLuint program, GLenum program_interface, GLenum pname, GLint* params)
{
...
GLuint ProgramInfoManager::GetProgramResourceIndex(GLES2Implementation* gl, GLuint program, GLenum program_interface, const char* name)
{
    // TODO(jiajie.hu@intel.com): The info is not cached for now, so always
    // fallback to the IPC path.
...
bool ProgramInfoManager::GetProgramResourceName(
    GLES2Implementation* gl, GLuint program, GLenum program_interface, GLuint index, GLsizei bufsize, GLsizei* length, char* name)
{
    // TODO(jiajie.hu@intel.com): The info is not cached for now, so always
    // fallback to the IPC path.
    return gl->GetProgramResourceNameHelper(program, program_interface, index, bufsize, length, name);
}

bool ProgramInfoManager::GetProgramResourceiv(GLES2Implementation* gl, GLuint program, GLenum program_interface, GLuint index, GLsizei prop_count,
    const GLenum* props, GLsizei bufsize, GLsizei* length, GLint* params)
{
    // TODO(jiajie.hu@intel.com): The info is not cached for now, so always
    // fallback to the IPC path.
    return gl->GetProgramResourceivHelper(program, program_interface, index, prop_count, props, bufsize, length, params);
}

GLint ProgramInfoManager::GetProgramResourceLocation(GLES2Implementation* gl, GLuint program, GLenum program_interface, const char* name)
{
    // TODO(jiajie.hu@intel.com): The info is not cached for now, so always
    // fallback to the IPC path.
...
void ProgramInfoManager::UpdateProgramInfo(GLuint program, base::span<const int8_t> data, ProgramInfoType type)
{
    base::AutoLock auto_lock(lock_);
    ProgramInfoMap::iterator it = program_infos_.find(program);
