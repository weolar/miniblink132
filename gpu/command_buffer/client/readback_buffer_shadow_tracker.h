...
public:
    class Buffer final {
    public:
        explicit Buffer(GLuint buffer_id, MappedMemoryManager* mapped_memory, GLES2CmdHelper* helper);
        ...
        uint32_t Alloc(int32_t* shm_id, uint32_t* shm_offset, bool* already_allocated);
        void Free();
    ...
    ReadbackBufferShadowTracker(MappedMemoryManager* mapped_memory, GLES2CmdHelper* helper);

    ReadbackBufferShadowTracker(const ReadbackBufferShadowTracker&) = delete;
    ReadbackBufferShadowTracker& operator=(const ReadbackBufferShadowTracker&) = delete;
    ...
    uint64_t buffer_shadow_serial() const
    {
        return buffer_shadow_serial_;
    }
    void IncrementSerial()
    {
        buffer_shadow_serial_++;
    }
