...
{
    return static_cast<gpu::gles2::GLES2Interface*>(gpu::ThreadLocalGetValue(g_gl_context_key));
}
...
{
    for (const NameToFunc* named_function = g_gles2_function_table; named_function->name; ++named_function) {
