...
protected:
    ...
    bool Initialize()
    {
        ...
        transfer_buffer_ = std::make_unique<MockTransferBuffer>(
            command_buffer_.get(), kTransferBufferSize, ImplementationBase::kStartingOffset, ImplementationBase::kAlignment, false);
...

        {
            ...
            gl_ = std::make_unique<WebGPUImplementation>(helper_.get(), transfer_buffer_.get(), gpu_control_.get());
        }
        ...
        commands_ = static_cast<CommandBufferEntry*>(ring_buffer->memory()) + command_buffer_->GetServicePutOffset();
        ClearCommands();
