...
TEST_F(GLES2ImplementationTest, FramebufferRenderbuffer)
{
    ...
    gl_->FramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, 4);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}
...
TEST_F(GLES2ImplementationTest, FramebufferTexture2D)
{
    ...
    gl_->FramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 4, 5);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}
...
TEST_F(GLES2ImplementationTest, GetBooleanv)
{
    ...
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result1.ptr, SizedResultHelper<ResultType>(1))).RetiresOnSaturation();
...
TEST_F(GLES2ImplementationTest, GetFramebufferAttachmentParameteriv)
{
    ...
    Cmds expected;
    ExpectedMemoryInfo result1 = GetExpectedResultMemory(sizeof(uint32_t) + sizeof(ResultType));
    expected.cmd.Init(123, GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE, result1.id, result1.offset);
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result1.ptr, SizedResultHelper<ResultType>(1))).RetiresOnSaturation();
    gl_->GetFramebufferAttachmentParameteriv(123, GL_COLOR_ATTACHMENT0, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE, &result);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
    EXPECT_EQ(static_cast<ResultType>(1), result);
}
...
TEST_F(GLES2ImplementationTest, GetSynciv)
{
    ...
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result1.ptr, SizedResultHelper<ResultType>(1))).RetiresOnSaturation();
    gl_->GetSynciv(reinterpret_cast<GLsync>(123), GL_SYNC_STATUS, 3, nullptr, &result);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
    EXPECT_EQ(static_cast<ResultType>(1), result);
}
...
TEST_F(GLES2ImplementationTest, GetVertexAttribfv)
{
    ...
    Cmds expected;
    ExpectedMemoryInfo result1 = GetExpectedResultMemory(sizeof(uint32_t) + sizeof(ResultType));
    expected.cmd.Init(123, GL_VERTEX_ATTRIB_ARRAY_NORMALIZED, result1.id, result1.offset);
    EXPECT_CALL(*command_buffer(), OnFlush()).WillOnce(SetMemory(result1.ptr, SizedResultHelper<ResultType>(1))).RetiresOnSaturation();
...
TEST_F(GLES2ImplementationTest, ShaderSource)
{
    ...
    expected.set_bucket_header.Init(kBucketId, 0, kHeaderSize, mem0.id, mem0.offset);
    expected.set_token1.Init(GetNextToken());
    expected.set_bucket_data1.Init(kBucketId, kHeaderSize, kString1Size, mem1.id, mem1.offset);
    expected.set_token2.Init(GetNextToken());
    expected.set_bucket_data2.Init(kBucketId, kHeaderSize + kString1Size, kString2Size, mem2.id, mem2.offset);
...
TEST_F(GLES2ImplementationTest, ShaderSourceWithLength)
{
    ...
    expected.set_bucket_header.Init(kBucketId, 0, kHeaderSize, mem0.id, mem0.offset);
    expected.set_token1.Init(GetNextToken());
    expected.set_bucket_data.Init(kBucketId, kHeaderSize, kStringSize + 1, mem1.id, mem1.offset);
    expected.set_token2.Init(GetNextToken());
...
TEST_F(GLES2ImplementationTest, RenderbufferStorageMultisampleCHROMIUM)
{
    ...
    gl_->RenderbufferStorageMultisampleCHROMIUM(GL_RENDERBUFFER, 2, GL_RGBA4, 4, 5);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}
...
TEST_F(GLES2ImplementationTest, RenderbufferStorageMultisampleAdvancedAMD)
{
    ...
    gl_->RenderbufferStorageMultisampleAdvancedAMD(GL_RENDERBUFFER, 2, 3, GL_RGBA4, 5, 6);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}
...
TEST_F(GLES2ImplementationTest, FramebufferTexture2DMultisampleEXT)
{
    ...
    Cmds expected;
    expected.cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 4, 5, 6);

    gl_->FramebufferTexture2DMultisampleEXT(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 4, 5, 6);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}
...
TEST_F(GLES2ImplementationTest, CopyTextureCHROMIUM)
{
    ...
    Cmds expected;
    expected.cmd.Init(1, 2, GL_TEXTURE_2D, 4, 5, GL_ALPHA, GL_UNSIGNED_BYTE, true, true, true);

    gl_->CopyTextureCHROMIUM(1, 2, GL_TEXTURE_2D, 4, 5, GL_ALPHA, GL_UNSIGNED_BYTE, true, true, true);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}
...
TEST_F(GLES2ImplementationTest, CopySubTextureCHROMIUM)
{
    ...
    Cmds expected;
    expected.cmd.Init(1, 2, GL_TEXTURE_2D, 4, 5, 6, 7, 8, 9, 10, 11, true, true, true);

    gl_->CopySubTextureCHROMIUM(1, 2, GL_TEXTURE_2D, 4, 5, 6, 7, 8, 9, 10, 11, true, true, true);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}
...
TEST_F(GLES2ImplementationTest, LoseContextCHROMIUM)
{
    ...
    gl_->LoseContextCHROMIUM(GL_GUILTY_CONTEXT_RESET_ARB, GL_GUILTY_CONTEXT_RESET_ARB);
    EXPECT_EQ(0, memcmp(&expected, commands_, sizeof(expected)));
}
...
TEST_F(GLES2ImplementationTest, GetFramebufferPixelLocalStorageParameterfvANGLE)
{
    struct Cmds {
        cmds::GetFramebufferPixelLocalStorageParameterfvANGLE cmd;
    };
    typedef cmds::GetFramebufferPixelLocalStorageParameterfvANGLE::Result::Type ResultType;
    ResultType result = 0;
    Cmds expected;
    ExpectedMemoryInfo result1 = GetExpectedResultMemory(sizeof(uint32_t) + sizeof(ResultType));
    expected.cmd.Init(123, 2, result1.id, result1.offset);
...
TEST_F(GLES2ImplementationTest, GetFramebufferPixelLocalStorageParameterivANGLE)
{
    struct Cmds {
        cmds::GetFramebufferPixelLocalStorageParameterivANGLE cmd;
    };
    typedef cmds::GetFramebufferPixelLocalStorageParameterivANGLE::Result::Type ResultType;
    ResultType result = 0;
    Cmds expected;
    ExpectedMemoryInfo result1 = GetExpectedResultMemory(sizeof(uint32_t) + sizeof(ResultType));
    expected.cmd.Init(123, 2, result1.id, result1.offset);
