...
protected:
    void SetUp() override
    {
        ...
        api_mock_ = std::make_unique<AsyncAPIMock>(true, command_buffer_.get(), command_buffer_->service());

        // ignore noops in the mock - we don't want to inspect the internals of the
        // helper.
        EXPECT_CALL(*api_mock_, DoCommand(cmd::kNoop, _, _)).WillRepeatedly(Return(error::kNoError));
    ...
    void AddCommandWithExpect(error::Error _return, unsigned int command, int arg_count, CommandBufferEntry* args)
    {
        ...
        EXPECT_CALL(*api_mock_, DoCommand(command, arg_count, Truly(AsyncAPIMock::IsArgs(arg_count, args)))).InSequence(sequence_).WillOnce(Return(_return));
    }
    ...
    void AddUniqueCommandWithExpect(error::Error _return, int cmd_size)
    {
        ...
        auto args_ptr = std::make_unique<CommandBufferEntry[]>(arg_count ? arg_count : 1);
        ...
        AddCommandWithExpect(_return, test_command_next_id_++, arg_count, args_ptr.get());
    ...
    void TestCommandWrappingFull(int32_t cmd_size, int32_t start_commands)
    {
        ...
        for (int32_t ii = 0; ii < start_commands; ++ii) {
            AddCommandWithExpect(error::kNoError, ii + kUnusedCommandId, num_args, &args[0]);
        }
        helper_->Finish();

        EXPECT_EQ(GetPutOffset(), (start_commands * cmd_size) % kTotalNumCommandEntries);
        EXPECT_EQ(GetGetOffset(), (start_commands * cmd_size) % kTotalNumCommandEntries);
        ...
        for (int32_t ii = 0; ii < kTotalNumCommandEntries / cmd_size + 2; ++ii) {
            AddCommandWithExpect(error::kNoError, start_commands + ii + kUnusedCommandId, num_args, &args[0]);
        }
    ...
    int32_t GetPutOffset()
    {
        return command_buffer_->GetServicePutOffset();
    }

    int32_t GetHelperGetOffset()
    {
        return helper_->cached_get_offset_;
    }

    int32_t GetHelperPutOffset()
    {
        return helper_->put_;
    }

    uint32_t GetHelperFlushGeneration()
    {
        return helper_->flush_generation();
    }
    ...
    CommandBufferOffset get_helper_put()
    {
        return helper_->put_;
    }
...
TEST_F(CommandBufferHelperTest, TestCalcImmediateEntriesGetBeforePut)
{
    ...
    helper_->Finish();
    AddUniqueCommandWithExpect(error::kNoError, kTotalNumCommandEntries - kInitGetOffset);
...
TEST_F(CommandBufferHelperTest, TestCalcImmediateEntriesAutoFlushingOrderingBarrier)
{
...
TEST_F(CommandBufferHelperTest, TestCommandWrapping)
{
    ...
    static_assert(kTotalNumCommandEntries % 3 != 0, "kTotalNumCommandEntries must not be a multiple of 3");
...
TEST_F(CommandBufferHelperTest, TestCommandWrappingExactMultiple)
{
    ...
    static_assert(kTotalNumCommandEntries % kCommandSize == 0, "kTotalNumCommandEntries should be a multiple of kCommandSize");
    ...
    for (unsigned int i = 0; i < 5; ++i) {
        AddCommandWithExpect(error::kNoError, i + kUnusedCommandId, kNumArgs, args1);
    }
...
TEST_F(CommandBufferHelperTest, TestToken)
{
    ...
    EXPECT_CALL(*api_mock_.get(), DoCommand(cmd::kSetToken, 1, _)).WillOnce(DoAll(Invoke(api_mock_.get(), &AsyncAPIMock::SetToken), Return(error::kNoError)));
    // Add another command.
    AddCommandWithExpect(error::kNoError, kUnusedCommandId + 4, 2, args);
...
TEST_F(CommandBufferHelperTest, FreeRingBuffer)
{
    ...
    EXPECT_FALSE(helper_->HaveRingBuffer());
    EXPECT_EQ(command_buffer_->FlushCount(), old_flush_count + 2);
    EXPECT_EQ(command_buffer_->GetLastState().get_offset, helper_->GetPutOffsetForTest());
}
