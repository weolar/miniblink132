...
public:
    class Client {
    public:
        virtual void IssueCreateTransferCacheEntry(
            ...
            = 0;
        virtual void IssueDeleteTransferCacheEntry(GLuint entry_type, GLuint entry_id) = 0;
        virtual void IssueUnlockTransferCacheEntry(GLuint entry_type, GLuint entry_id) = 0;
    ...
    void AddTransferCacheEntry(uint32_t type, uint32_t id, uint32_t shm_id, uint32_t shm_offset, uint32_t size);
    ...
    void StartTransferCacheEntry(uint32_t type, uint32_t id, base::OnceCallback<void(ClientDiscardableHandle)> create_entry_cb);

    // Map(of either type) must always be followed by an Unmap.
    void* MapEntry(MappedMemoryManager* mapped_memory, uint32_t size);
    void* MapTransferBufferEntry(TransferBufferInterface* transfer_buffer, uint32_t size);
...
private:
    ...
    ClientDiscardableHandle::Id FindDiscardableHandleId(const EntryKey& key) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ClientDiscardableHandle CreateDiscardableHandle(const EntryKey& key) EXCLUSIVE_LOCKS_REQUIRED(lock_);
    ...
    std::map<EntryKey, ClientDiscardableHandle::Id> discardable_handle_id_map_ GUARDED_BY(lock_);
};
