...
protected:
    ...
    {
        for (std::vector<raw_ptr<const volatile void, VectorExperimental>>::iterator it = set_token_arguments_.begin(); it != set_token_arguments_.end();
             ++it) {
    ...
    void SetUp() override
    {
        ...
        // helper.
        EXPECT_CALL(*api_mock_, DoCommand(cmd::kNoop, 0, _)).WillRepeatedly(Return(error::kNoError));
        // Forward the SetToken calls to the engine
        EXPECT_CALL(*api_mock_.get(), DoCommand(cmd::kSetToken, 1, _))
            .WillRepeatedly(DoAll(Invoke(this, &BaseRingBufferTest::SetToken), Return(error::kNoError)));
    ...
    std::vector<raw_ptr<const volatile void, VectorExperimental>> set_token_arguments_;
...
protected:
    void SetUp() override
    {
        ...
        buffer_ = base::MakeRefCounted<gpu::Buffer>(std::make_unique<MemoryBufferBacking>(kBufferSize + kBaseOffset));
        allocator_.reset(new RingBuffer(buffer_, kAlignment, kBaseOffset, helper_.get()));
    }
...
TEST_F(RingBufferTest, TestCanAllocGetLargestFreeOrPendingSize)
{
    ...
    buffer_ = base::MakeRefCounted<gpu::Buffer>(std::make_unique<MemoryBufferBacking>(kBufferSize + 2 + kBaseOffset));
    allocator_.reset(new RingBuffer(buffer_, kAlignment, kBaseOffset, helper_.get()));
...
TEST_F(RingBufferTest, TestFreePendingToken)
{
    ...
    EXPECT_EQ(kBufferSize - (kSize * kAllocCount), allocator_->GetLargestFreeSizeNoWaiting());
...
TEST_F(RingBufferTest, TestFreeBug)
{
    ...
    EXPECT_EQ(kBufferSize - kAlloc1 - kAlloc2, allocator_->GetLargestFreeSizeNoWaiting());
    allocator_->FreePendingToken(pointer, helper_.get()->InsertToken());
...
TEST_F(RingBufferTest, DiscardFrontTest)
{
    ...
    EXPECT_EQ(static_cast<uint8_t*>(ptr1) + kAlloc1, static_cast<uint8_t*>(ptr2));
    EXPECT_EQ(kBufferSize - kAlloc1 - kAlloc2, allocator_->GetLargestFreeSizeNoWaiting());
    allocator_->FreePendingToken(ptr2, helper_.get()->InsertToken());
    EXPECT_EQ(allocator_->NumUsedBlocks(), 0u);

    void* ptr3 = allocator_->Alloc(kAlloc3);
    EXPECT_EQ(static_cast<uint8_t*>(ptr2) + kAlloc2, static_cast<uint8_t*>(ptr3));
    EXPECT_EQ(0u, allocator_->GetLargestFreeSizeNoWaiting());
    EXPECT_EQ(allocator_->NumUsedBlocks(), 1u);
