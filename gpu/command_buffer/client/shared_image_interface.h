...
struct SharedImageInfo {
    SharedImageInfo(const viz::SharedImageFormat& format, gfx::Size size, const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin,
        SkAlphaType alpha_type, SharedImageUsageSet usage, std::string_view debug_label)
        : meta(format, size, color_space, surface_origin, alpha_type, usage)
        , debug_label(debug_label)
    {
    }
    SharedImageInfo(
        const viz::SharedImageFormat& format, gfx::Size size, const gfx::ColorSpace& color_space, SharedImageUsageSet usage, std::string_view debug_label)
        : meta(format, size, color_space, kTopLeft_GrSurfaceOrigin, kPremul_SkAlphaType, usage)
        , debug_label(debug_label)
    {
    }
...
class GPU_EXPORT SharedImageInterface : public base::RefCountedThreadSafe<SharedImageInterface> {
public:
    ...
    virtual scoped_refptr<ClientSharedImage> CreateSharedImage(const SharedImageInfo& si_info, gpu::SurfaceHandle surface_handle) = 0;
    ...
    virtual scoped_refptr<ClientSharedImage> CreateSharedImage(const SharedImageInfo& si_info, base::span<const uint8_t> pixel_data) = 0;
    ...
    // `DestroySharedImage()` is called or the interface itself is destroyed (e.g.
    // the GPU channel is lost).
    virtual scoped_refptr<ClientSharedImage> CreateSharedImage(const SharedImageInfo& si_info, gfx::GpuMemoryBufferHandle buffer_handle) = 0;
    ...
    struct GPU_EXPORT SharedImageMapping {
        ...
        SharedImageMapping(scoped_refptr<ClientSharedImage> shared_image, base::WritableSharedMemoryMapping mapping);
    ...
    // only.
    virtual SharedImageMapping CreateSharedImage(const SharedImageInfo& si_info) = 0;
    ...
    virtual void UpdateSharedImage(const SyncToken& sync_token, const Mailbox& mailbox) = 0;
    ...
    virtual void UpdateSharedImage(const SyncToken& sync_token, std::unique_ptr<gfx::GpuFence> acquire_fence, const Mailbox& mailbox) = 0;
    ...
    // native GMBs.
    virtual void CopyToGpuMemoryBuffer(const SyncToken& sync_token, const Mailbox& mailbox);
    ...
    virtual void CopyToGpuMemoryBufferAsync(const SyncToken& sync_token, const Mailbox& mailbox, base::OnceCallback<void(bool)> callback);
    ...
    virtual bool CopyNativeGmbToSharedMemorySync(gfx::GpuMemoryBufferHandle buffer_handle, base::UnsafeSharedMemoryRegion memory_region);
    ...
    virtual void DestroySharedImage(const SyncToken& sync_token, const Mailbox& mailbox) = 0;

    // Same behavior as the above, except that this version takes
    // a |client_shared_image| parameter (which holds a mailbox).
    virtual void DestroySharedImage(const SyncToken& sync_token, scoped_refptr<ClientSharedImage> client_shared_image) = 0;
    ...
    // SharedImage.
    virtual scoped_refptr<ClientSharedImage> AddReferenceToSharedImage(const SyncToken& sync_token, const Mailbox& mailbox, viz::SharedImageFormat format,
    ...
    virtual scoped_refptr<ClientSharedImage> ImportSharedImage(const ExportedSharedImage& exported_shared_image) = 0;

    struct GPU_EXPORT SwapChainSharedImages {
        SwapChainSharedImages(scoped_refptr<gpu::ClientSharedImage> front_buffer, scoped_refptr<gpu::ClientSharedImage> back_buffer);
    ...
    virtual SwapChainSharedImages CreateSwapChain(viz::SharedImageFormat format, const gfx::Size& size, const gfx::ColorSpace& color_space,
        GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, gpu::SharedImageUsageSet usage)
        = 0;
    ...
    virtual void PresentSwapChain(const SyncToken& sync_token, const Mailbox& mailbox) = 0;
    ...
    virtual void UpdateSharedImage(const SyncToken& sync_token, scoped_refptr<gfx::D3DSharedFence> d3d_shared_fence, const Mailbox& mailbox);
    ...
    virtual scoped_refptr<gfx::NativePixmap> GetNativePixmap(const gpu::Mailbox& mailbox) = 0;
#endif
    ...
    virtual scoped_refptr<ClientSharedImage> NotifyMailboxAdded(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size,
        const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, SharedImageUsageSet usage);
    virtual scoped_refptr<ClientSharedImage> NotifyMailboxAdded(const Mailbox& mailbox, viz::SharedImageFormat format, const gfx::Size& size,
        const gfx::ColorSpace& color_space, GrSurfaceOrigin surface_origin, SkAlphaType alpha_type, SharedImageUsageSet usage, uint32_t texture_target);
...
class GPU_EXPORT SharedImageInterfaceHolder : public base::RefCountedThreadSafe<SharedImageInterfaceHolder> {
public:
