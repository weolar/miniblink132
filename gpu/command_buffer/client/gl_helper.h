...
public:
    ...
    typedef void (gles2::GLES2Interface::*DeleteFunc)(GLsizei n, const GLuint* ids);
    ScopedGLuint(gles2::GLES2Interface* gl, GenFunc gen_func, DeleteFunc delete_func)
        : gl_(gl)
        , id_(0u)
...
public:
    explicit ScopedBuffer(gles2::GLES2Interface* gl)
        : ScopedGLuint(gl, &gles2::GLES2Interface::GenBuffers, &gles2::GLES2Interface::DeleteBuffers)
    {
    }
};
...
public:
    explicit ScopedFramebuffer(gles2::GLES2Interface* gl)
        : ScopedGLuint(gl, &gles2::GLES2Interface::GenFramebuffers, &gles2::GLES2Interface::DeleteFramebuffers)
    {
    }
};
...
public:
    explicit ScopedTexture(gles2::GLES2Interface* gl)
        : ScopedGLuint(gl, &gles2::GLES2Interface::GenTextures, &gles2::GLES2Interface::DeleteTextures)
    {
    }
};

template <GLenum Target> class ScopedBinder {
public:
...
template <GLenum Target> class ScopedBufferBinder : ScopedBinder<Target> {
public:
...
template <GLenum Target> class ScopedFramebufferBinder : ScopedBinder<Target> {
public:
...
template <GLenum Target> class ScopedTextureBinder : ScopedBinder<Target> {
public:
...
public:
    ...
    public:
        ...
        void Scale(GLuint src_texture, const gfx::Size& src_texture_size, const gfx::Vector2dF& src_offset, GLuint dest_texture, const gfx::Rect& output_rect)
        {
            ScaleToMultipleOutputs(src_texture, src_texture_size, src_offset, dest_texture, 0, output_rect);
        }

        // Same as above, but for shaders that output to two textures at once.
        virtual void ScaleToMultipleOutputs(GLuint src_texture, const gfx::Size& src_texture_size, const gfx::Vector2dF& src_offset, GLuint dest_texture_0,
            GLuint dest_texture_1, const gfx::Rect& output_rect)
            = 0;
        ...
        virtual void ComputeRegionOfInfluence(const gfx::Size& src_texture_size, const gfx::Vector2dF& src_offset, const gfx::Rect& output_rect,
            gfx::Rect* sampling_rect, gfx::Vector2dF* offset) const
            = 0;

        // Returns true if from:to represent the same scale ratio as that provided
        // by this scaler.
        virtual bool IsSameScaleRatio(const gfx::Vector2d& from, const gfx::Vector2d& to) const = 0;
    ...
    std::unique_ptr<ScalerInterface> CreateScaler(
    ...
    std::unique_ptr<I420Converter> CreateI420Converter(bool flipped_source, bool flip_output, bool swizzle, bool use_mrt);
    ...
    std::unique_ptr<ReadbackYUVInterface> CreateReadbackPipelineYUV(bool vertically_flip_texture, bool use_mrt);
...
public:
    ...
    // textures.
    virtual void Convert(GLuint src_texture, const gfx::Size& src_texture_size, const gfx::Vector2dF& src_offset, GLHelper::ScalerInterface* optional_scaler,
        const gfx::Rect& output_rect, GLuint y_plane_texture, GLuint u_plane_texture, GLuint v_plane_texture)
        = 0;
...
public:
    ...
    //      modified).
    virtual void ReadbackYUV(GLuint texture, const gfx::Size& src_texture_size, const gfx::Rect& output_rect, int y_plane_row_stride_bytes,
        ...
        const gfx::Point& paste_location, base::OnceCallback<void(bool)> callback)
        = 0;
};
