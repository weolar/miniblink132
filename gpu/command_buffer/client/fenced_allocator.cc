...
{
    return (base::CheckedNumeric<uint32_t>(size) + (FencedAllocator::kAllocAlignment - 1)) & ~(FencedAllocator::kAllocAlignment - 1);
}
...
void FencedAllocator::FreePendingToken(FencedAllocator::Offset offset, int32_t token)
{
    BlockIndex index = GetBlockByOffset(offset);
    Block& block = blocks_[index];
    DCHECK_EQ(block.offset, offset);
...
FencedAllocator::State FencedAllocator::GetBlockStatusForTest(Offset offset, int32_t* token_if_pending)
{
    BlockIndex index = GetBlockByOffset(offset);
    Block& block = blocks_[index];
...
FencedAllocator::BlockIndex FencedAllocator::CollapseFreeBlock(BlockIndex index)
{
...
FencedAllocator::BlockIndex FencedAllocator::WaitForTokenAndFreeBlock(BlockIndex index)
{
    Block& block = blocks_[index];
...
void FencedAllocator::FreeUnused()
{
    ...
    for (uint32_t i = 0; i < blocks_.size();) {
        Block& block = blocks_[i];
        if (block.state == FREE_PENDING_TOKEN && helper_->HasCachedTokenPassed(block.token)) {
            block.state = FREE;
...
FencedAllocator::Offset FencedAllocator::AllocInBlock(BlockIndex index, uint32_t size)
{
    Block& block = blocks_[index];
...
FencedAllocator::BlockIndex FencedAllocator::GetBlockByOffset(Offset offset)
{
    ...
    Container::iterator it = std::lower_bound(blocks_.begin(), blocks_.end(), templ, OffsetCmp());
