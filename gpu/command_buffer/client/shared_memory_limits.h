...
// We can't call AmountOfPhysicalMemory under NACL, so leave the default.
        ...
        max_mapped_memory_for_texture_upload = base::SysInfo::AmountOfPhysicalMemory() > 1024ULL * 1024 * 1024
            ? base::saturated_cast<uint32_t>(base::SysInfo::AmountOfPhysicalMemory() / 20)
            : 0;
    ...
    static SharedMemoryLimits ForDisplayCompositor(const gfx::Size& screen_size)
    {
        ...
        const uint32_t full_screen_texture_size_in_bytes = screen_size.width() * screen_size.height() * kBytesPerPixel;
